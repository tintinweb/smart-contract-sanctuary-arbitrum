...
abstract contract MainBase is Storage {
    ...
    bytes32 internal constant IMPLEMENTATION_MAPPING_SLOT =
    ...
    function collectCryptoFee(address toAddress) external onlyOwner {
        require(
            isRelayer(toAddress),
            "swapContract: fee can be sent only to a relayer"
        );
    ...
    function collectTokenFee() external onlyOwner {
        IERC20 _Rubic = IERC20(
            address(uint160(uint256(RubicAddresses[numOfThisBlockchain])))
        );
        _Rubic.safeTransfer(msg.sender, accTokenFee - 1);
     ...
     */
    function approveTokenToRouter(IERC20 _token, address _router)
        external
        onlyOwnerAndManager
    {
    ...
    function poolBalancing(uint256 amount) external onlyOwner {
        IERC20(address(uint160(uint256(RubicAddresses[numOfThisBlockchain]))))
...
contract MainContract is MainBase {
    ...
    function getStorageValue(bytes4 sig, bytes32 slot)
        internal
        view
        returns (Implementation storage impl)
    {
        assembly {
    ...
    function getInfoAboutSig(bytes4 sig, bytes32 slot)
        external
        view
        returns (address implementationAddress, address router)
    {
    ...
    function addInstance(
        ...
        Implementation storage impl = getStorageValue(
            sig,
            IMPLEMENTATION_MAPPING_SLOT
        );
    ...
    function setRouter(bytes4 sig, address _router)
        external
        onlyOwnerAndManager
    {
    ...
    fallback() external payable {
        ...
        require(blockchainRouter != address(0), "no such instance");
        assembly {
