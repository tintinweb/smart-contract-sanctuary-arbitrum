...
contract Fiat24BuyByEth is AccessControl, Pausable {
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");

    ISwapRouter public constant uniswapRouter = ISwapRouter(0xE592427A0AEce92De3Edee1F18E0157C05861564);
    IPeripheryPayments public constant uniswapPeripheryPayments = IPeripheryPayments(0xE592427A0AEce92De3Edee1F18E0157C05861564);
    IQuoter public constant uniswapQuoter = IQuoter(0xb27308f9F90D607463bb33eA1BeBb41C27CE5AB6);
    address public WETH9;

    Fiat24Account public fiat24account;
    F24 public f24;
    Fiat24PriceList public pricelist;

    constructor (address fiat24account_, address f24_, address fiat24pricelist_, address weth_) Pausable(){
        fiat24account = Fiat24Account(fiat24account_);
        f24 = F24(f24_);
        pricelist = Fiat24PriceList(fiat24pricelist_);
        WETH9 =  weth_;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(OPERATOR_ROLE, _msgSender());
    }

    function buy(uint256 accountNumber) external payable  {
        uint256 price = pricelist.getPrice(accountNumber);
        buyF24ByEth(price);
        (bool success_approve,) = address(f24).delegatecall(abi.encodeWithSignature('approve(address,uint256)',address(fiat24account),price));
        require(success_approve, 'Fiat24BuyByEth: failed to approve F24 amount');
        (bool success_mint,) = address(fiat24account).delegatecall(abi.encodeWithSignature('mintByClientWithF24(uint256)',accountNumber));
        require(success_mint, 'Fiat24BuyByEth: failed to mint Fiat24 account');
        fiat24account.mintByClientWithF24(accountNumber);
    }

    function buyF24ByEth(uint256 price) private {
        require(msg.value > 0, "Fiat24BuyByEth: must pass non 0 ETH amount");

        uint256 deadline = block.timestamp + 15; // using 'now' for convenience, for mainnet pass deadline from frontend!
        address tokenIn = WETH9;
        address tokenOut = address(f24);
        uint24 fee = 3000;
        ...
        uniswapPeripheryPayments.refundETH();
    ...
    function pause() public {
        require(hasRole(OPERATOR_ROLE, msg.sender), "Fiat24BuyByEth: Not an operator");
        _pause();
    }
