...
contract ToadzMinter is Initializable, ToadzMinterContracts {
    ...
    {
        require(
            !addressToClaimed[msg.sender].claimedAll,
            "ToadzMinter: Already claimed all Toadz"
        );

        if(addressToClaimed[msg.sender].leftToClaim == 0) {
            addressToClaimed[msg.sender].leftToClaim = _amount;
        }

        uint256 _leftToClaim = addressToClaimed[msg.sender].leftToClaim;
        ...
        addressToClaimed[msg.sender].leftToClaim -= _batchSize;

        if(addressToClaimed[msg.sender].leftToClaim == 0) {
            addressToClaimed[msg.sender].claimedAll = true;
        }
    ...
    returns(uint256)
    {
        if(addressToClaimed[msg.sender].claimedAll) {
            return 0;
        }
        ...
        if(addressToClaimed[msg.sender].leftToClaim == 0) {
            ...
            return addressToClaimed[msg.sender].leftToClaim;
        }
    }
...
abstract contract ToadzMinterState is Initializable, IToadzMinter, AdminableUpgradeable {
    ...
    mapping(address => Claim) public addressToClaimed;
