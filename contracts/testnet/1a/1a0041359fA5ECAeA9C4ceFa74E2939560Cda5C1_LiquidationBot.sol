...
contract LiquidationBot is Controllable {
    ...
    function fetchUnhealthyAccounts(
        ...
        (uint256 price, uint8 decimals) = vault.getCollPrice();
        BaseVault.UserDetails[] memory accounts = vault.getUsersDetailsInRange(_startIndex, _startIndex + offset);
        uint256 vaultMinCollRatio = vault.MIN_COL_RATIO();

        uint256 j;
        for (uint256 i; i < accounts.length; ++i) {
            if (TauMath._computeCR(accounts[i].collateral, accounts[i].debt, price, decimals) < vaultMinCollRatio) j++;
        }
        ...
        for (uint256 i; i < accounts.length; i++) {
            if (TauMath._computeCR(accounts[i].collateral, accounts[i].debt, price, decimals) < vaultMinCollRatio)
                unhealthyAccounts[j++] = accounts[i].userAddress;
        }
    ...
    function liquidate(LiqParams memory _liqParams) external onlyKeeper {
        ...
        vault.liquidate(_liqParams.accountAddr, newCalcAmt, _liqParams.minExchangeRate);
    }
