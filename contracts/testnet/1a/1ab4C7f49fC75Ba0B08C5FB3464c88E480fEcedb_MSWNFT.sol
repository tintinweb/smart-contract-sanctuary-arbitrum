...
contract MSWNFT is Ownable, ERC721Enumerable {
    ...
    function upgrade(uint tokenId_, uint toCardId_) public {
        ...
        require(
            cardInfoes[cardIdMap[tokenId_]].cardId < toCardId_,
            "MSW: wrong lv"
        );

        uint diff = cardInfoes[toCardId_].price -
            cardInfoes[cardIdMap[tokenId_]].price;
        cardIdMap[tokenId_] = toCardId_;
        cardInfoes[tokenId_].currentAmount--;
        cardInfoes[toCardId_].currentAmount++;

        IERC20(WALLET).transferFrom(msg.sender, address(this), diff);
