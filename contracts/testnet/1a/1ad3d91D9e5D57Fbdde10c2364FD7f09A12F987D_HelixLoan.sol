...
contract HelixLoan is BaseUpgradeablePausable, EIP712Upgradeable, IHelixLoan, IVersioned {
  ...
  // bytes32 public constant INDICATIVE_INTEREST_TYPEHASH = keccak256("IndicativeInterest(address investor,address fund,uint256 investmentDate,uint256 amount,uint256 signingTime,uint256 expirationTime)");
   ...
   */
  function initialize(
    // config - manager wallet - borrower wallet - loan wallet - token
    ...
    loanWallet = _addresses[3];
    token = IERC20withDec(_addresses[4]);
  ...
  function getInvestmentHash(
   ...
   function encodeInvestmentData(
    ...
    return abi.encodePacked(bytes1(0x19), bytes1(0x01), DOMAIN_SEPARATOR(), investmentHash);
  }
