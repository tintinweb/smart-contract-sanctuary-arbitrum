...
contract MatchOrders is OwnableUpgradeable {
    ...
    function linkVertexSigner(address externalAccount, uint64 nonce) internal {
        ...
        IEndpoint.LinkSigner memory linkSigner = IEndpoint.LinkSigner(externalSubaccount, contractSubaccount, nonce);
    ...
    function setLinkerSigner(address externalAccount, uint64 nonce) external onlyOwner {
        linkVertexSigner(externalAccount, nonce);
    }

    function swapAMM(address sender, uint32 productId, int128 amount, int128 priceX18) external onlyOwner {
