...
//issue when deploying with proxy, import does not work, need to investigate
interface IWater {
    /* ##################################################################
                                BARTENDER FUNCTIONS
    ################################################################## */
...
contract Liquor is OwnableUpgradeable {
    ...
    function initialize(address _usdcToken, address _water, address _keeper, address _bartender) external initializer {
        water = IWater(_water);
    ...
    function setBartender(address _bartender) external onlyOwner {
        require(_bartender != address(0), "NO_DEAD_ADDRESS");
        bartender = IBartender(_bartender);
    }
    ...
    modifier onlyKeeper() {
        require(keeper == msg.sender, "Not Keeper");
        _;
    }

    function liquidateSakeVault(
        uint256 _id
    ) public returns (uint256 AmountToSakeUsers, uint256 amountToSafe, uint256 outstandingAmountToWater) {
        (uint256 newDebt, uint256 newRatio) = getDebtInfo(_id);
        // check if the liquidation threshold has reached
        require(newRatio >= (90 * RATE_PRECISION) / 100, "Liquidation Threshold Has Not Reached");
        (uint256 totalAmountOfVLP, address[] memory users) = getSakeUsers(_id);

        // sell all VLP for the sub sake
        address sakeVault = bartender.sakeVault(_id);
        (, uint256 usdcAmount) = ISake(sakeVault).withdraw(address(this), totalAmountOfVLP);
        ...
        if (usdcAmount >= newDebt) {
            water.repayDebt(newDebt);
            amountToSafe = ((usdcAmount - newDebt) / 2);
            usdcToken.safeTransfer(keeper, amountToSafe);
            AmountToSakeUsers = amountToSafe;

            // uint256 length = sakeInfo.users.length;
            for (uint256 i; i < users.length; ) {
                // address sakeUser = users[i];
                uint256 userShares = getUserShares(_id, users[i]);
                recordLiquidatedValue(_id, users[i], userShares, AmountToSakeUsers);
            ...
            emit Liquidate(_id, address(sakeVault), AmountToSakeUsers, amountToSafe, 0);

            return (AmountToSakeUsers, amountToSafe, 0);
        } else {
            //if liquidation < debt;
            // pay the debt, safe/keepr will safeTransfer the additional amount to water;
            water.repayDebt(usdcAmount);
            outstandingAmountToWater = newDebt - usdcAmount;
            return (0, 0, outstandingAmountToWater);
        }
    }

    function getDebtInfo(uint256 id) public view returns (uint256 newDebt, uint256 newRatio) {
        (, newDebt, newRatio, , ) = bartender.getDebtInfo(id);
    }

    function getSakeUsers(uint256 _id) public view returns (uint256 totalAmountOfVLP, address[] memory users) {
        (totalAmountOfVLP, users) = bartender.getSakeUsers(_id);
    }

    function getUserShares(uint256 _id, address _user) public view returns (uint256 shares) {
        (, , , shares, ) = bartender.depositInfo(_user, _id);
    ...
    function recordLiquidatedValue(uint256 _id, address _user, uint256 shares, uint256 totalAmount) internal {
        ...
        // usdcToken.safeTransfer(_user, _amount);
    }
    ...
    function withdraw(uint256 _id) public returns (uint256 wihtdrawnAmount) {
        ...
        (address feeRecipient, bool feeEnabled, , uint128 withdrawFeeBPS) = getFeeStatus();
        if (!feeEnabled) {
            withdrawableAmount[msg.sender][_id] = 0;
            usdcToken.safeTransfer(msg.sender, amount);
        } else {
            uint256 fee = amount.mulDiv(withdrawFeeBPS, MAX_BPS);
            amount -= fee;
            withdrawableAmount[msg.sender][_id] = 0;
    ...
    function getFeeStatus()
        public
        view
        returns (address feeRecipient, bool feeEnabled, uint128 depositFeeBPS, uint128 withdrawFeeBPS)
    {
