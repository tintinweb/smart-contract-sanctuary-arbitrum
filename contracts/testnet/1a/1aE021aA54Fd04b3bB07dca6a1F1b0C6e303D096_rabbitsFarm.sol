...
contract rabbitsFarm is Ownable {
    // emit payment events\
    ...
    uint256 public claimTaxCarrot = 8;
    ...
    //constructor\
    constructor (
        address _carrot, //address of a standard erc20 to use in the platform\
    ...
    function getCarrotBalance() external view returns (uint256) {
	return carrot.balanceOf(pair);
    }
    ...
    function getPrice() public view returns (uint256) {
        uint256 carrotBalance = carrot.balanceOf(pair);
        uint256 usdcBalance = usdc.balanceOf(pair);
        require(carrotBalance > 0, "divison by zero error");
        uint256 price = usdcBalance * 1e30 / carrotBalance;
        return price;
    }
    ...
    function setTokenAddr(address tokenAddress) public {
        require(msg.sender == treasury, 'Can only be used by Rabbits.Money Treasury');
        carrot = IERC20(tokenAddress);
    }
    ...
    function setCarrotTax(uint256 _claimTaxCarrot) public onlyOwner {
        claimTaxCarrot = _claimTaxCarrot;
    }
    ...
    function buyNode(uint256 _amount) external payable {  
        ...
        uint256 nodesOwned = farmers[msg.sender].carrotNodes + farmers[msg.sender].bondNodes + _amount;
        ...
        uint256 toBurn = transactionTotal;
        _transferFrom(carrot, msg.sender, address(burnaddress), toBurn);
        farmers[msg.sender] = farmer;
        updateClaims(msg.sender);
        farmers[msg.sender].carrotNodes += _amount;
        totalNodes += _amount;
    }
    ...
    function awardNode(address _address, uint256 _amount) public onlyOwner {
        uint256 nodesOwned = farmers[_address].carrotNodes + farmers[_address].bondNodes + _amount;
        require(nodesOwned < 101, "Max Rabbits Owned");
        Farmer memory farmer;
        ...
        updateClaims(_address);
        farmers[_address].carrotNodes += _amount;
        totalNodes += _amount;
    ...
    function compoundNode() public {
        ...
        uint256 nodesOwned = farmers[msg.sender].carrotNodes + farmers[msg.sender].bondNodes;
        require(pendingClaims>nodeCost, "Not enough pending Carrot to compound");
        require(nodesOwned < 100, "Max Rabbits Owned");
        updateClaims(msg.sender);
        if (farmers[msg.sender].claimsCarrot > nodeCost) {
            farmers[msg.sender].claimsCarrot -= nodeCost;
            farmers[msg.sender].carrotNodes++;
        } else {
            uint256 difference = nodeCost - farmers[msg.sender].claimsCarrot;
            farmers[msg.sender].claimsCarrot = 0;
    ...
    function updateClaims(address _address) internal {
        ...
        if (timerFrom > 0)
            farmers[_address].claimsCarrot += farmers[_address].carrotNodes * nodeBase * dailyInterest * (time - timerFrom) / 8640000;
    ...
    function getTotalClaimable() public view returns (uint256) {
        uint256 time = block.timestamp;
        uint256 pendingCarrot = farmers[msg.sender].carrotNodes * nodeBase * dailyInterest * (time - farmers[msg.sender].lastUpdate) / 8640000;
        uint256 pendingBond = farmers[msg.sender].bondNodes * nodeBase * dailyInterest * (time - farmers[msg.sender].lastUpdate) / 8640000;
        uint256 pending = pendingCarrot + pendingBond;
        return farmers[msg.sender].claimsCarrot + farmers[msg.sender].claimsBond + pending;
	}
    ...
    function getTaxEstimate() external view returns (uint256) {
        ...
        uint256 pendingCarrot = farmers[msg.sender].carrotNodes * nodeBase * dailyInterest * (time - farmers[msg.sender].lastUpdate) / 8640000;
        uint256 pendingBond = farmers[msg.sender].bondNodes * nodeBase * dailyInterest * (time - farmers[msg.sender].lastUpdate) / 8640000;
        uint256 claimableCarrot = pendingCarrot + farmers[msg.sender].claimsCarrot;
        uint256 claimableBond = pendingBond + farmers[msg.sender].claimsBond;
        uint256 taxCarrot = claimableCarrot / 100 * claimTaxCarrot;
        uint256 taxBond = claimableBond / 100 * claimTaxBond;
        return taxCarrot + taxBond;
	}

    function calculateTax() public returns (uint256) {
        updateClaims(msg.sender);
        uint256 taxCarrot = farmers[msg.sender].claimsCarrot / 100 * claimTaxCarrot;
        uint256 taxBond = farmers[msg.sender].claimsBond / 100 * claimTaxBond;
        uint256 tax = taxCarrot + taxBond;
        return tax;
    }
    ...
    function claim() external payable {
		...
		uint256 reward = farmers[msg.sender].claimsCarrot + farmers[msg.sender].claimsBond;
        uint256 toBurn = tax;
        uint256 toFarmer = reward - tax;
		if (reward > 0) {
            farmers[msg.sender].claimsCarrot = 0;		
            farmers[msg.sender].claimsBond = 0;
            _transfer(carrot, msg.sender, toFarmer);
            _transfer(carrot, address(burnaddress), toBurn);
		}
    }
    ...
    function getOwnedNodes() external view returns (uint256) {
        uint256 ownedNodes = farmers[msg.sender].carrotNodes + farmers[msg.sender].bondNodes;
        return ownedNodes;
    }
    ...
    function getCarrotClaimTax() external view returns (uint256) {
        return claimTaxCarrot;
    }
