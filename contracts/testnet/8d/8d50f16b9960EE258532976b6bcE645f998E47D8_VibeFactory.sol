...
contract NFTMintSale is MintSaleBase {
    ...
    function init(bytes calldata data) public payable {
        ...
        {
            (address treasury, uint96 feeTake, uint64 mintingFee)= NFTMintSale(vibeFactory.masterContractOf(address(this))).fees();
...
abstract contract MintSaleBase is Ownable {
    ...
    function getPayment(uint256 amount) internal {
        if (address(paymentToken) == address(WETH)) {
            if (fees.mintingFee > 0) {
                // amount = amount + fees.mintingFee;
                require(
                    msg.value == amount + fees.mintingFee,
                    "Not enough value"
                );

                (bool success, ) = fees.vibeTreasury.call{
                    value: fees.mintingFee
                }("");
            ...
            // WETH.transfer(fees.vibeTreasury, fees.mintingFee);
        } else {
            if (fees.mintingFee > 0) {
                require(msg.value == fees.mintingFee, "Not enough value");
                // WETH.deposit{value: fees.mintingFee}();
                // WETH.transfer(fees.vibeTreasury, fees.mintingFee);
