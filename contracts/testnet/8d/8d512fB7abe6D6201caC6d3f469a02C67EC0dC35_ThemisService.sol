...
interface IThemisProject {
    ...
    event WinningAmountModified(uint256 projectId, uint48 newWinningAmount);
...
interface IThemisTeam {
    event TeamCreated(uint256 teamId, address creater, uint256 signId);

    event MembersManaged(
        uint256 teamId,
        address teamOwner,
        address[] members,
        bool[] isEntereds,
    ...
    event ManageMemberFailed(uint256 i, string revertReason);
    ...
    event LeftTeam(uint256 teamId, address member);
...
contract ThemisProject is VRFConsumerBaseV2, ThemisTeam, IThemisProject {
    ...
    // projectId => teamId
    mapping(uint256 => uint256) public projectIdToTeamIds;
    ...
    mapping(uint256 => mapping(address => uint48[])) public userNumbers;
    ...
    function createProject(
        ProjectInfo calldata projectInfo
    ) external returns (uint256) {
        address creater = _msgSender();
        uint256 teamId = userToTeamIds[creater];

        require(teamId != 0, "ThemisProject: No team");

        projectIdCounter.increment();
        uint256 projectId = projectIdCounter.current();

        projectIdToTeamIds[projectId] = teamId;
    ...
    function modifyTimestamp(
        ...
        require(
            block.timestamp < startCampaign &&
                startCampaign < endCampaign &&
                endCampaign < startRaffle &&
                startRaffle < endRaffle &&
                endRaffle < startClaim &&
                startClaim < endClaim,
            "Themis: input error"
        );

        require(
            userToTeamIds[_msgSender()] == projectIdToTeamIds[projectId],
            "Themis: Not in the team"
        );

        Project memory project = projects[projectId];
        require(
            block.timestamp < project.startCampaign,
            "Themis: Campaign started"
        );

        project.startCampaign = startCampaign;
        project.endCampaign = endCampaign;
        project.startRaffle = startRaffle;
        project.endRaffle = endRaffle;
        project.startClaim = startClaim;
        project.endClaim = endClaim;
    ...
    function modifyWinningAmount(
        ...
        emit WinningAmountModified(projectId, newWinningAmount);
    }
    ...
    function modifyAirDrop(
        ...
        airDrops[projectId].tokenAddr = newTokenAddr;
        airDrops[projectId].tokenAmount = newTokenAmount;

        emit AirDropModified(projectId, newTokenAddr, newTokenAmount);
    }
    ...
    function requestRandom(uint256 projectId) external {
        ...
        require(!project.isRaffled, "Themis: The draw has been completed");
    ...
    function uploadVote(
        ...
        uint48 currentVoteId = project.currentVoteId;

        require(voteAmount > 0, "Themis: No new votes added");

        for (uint256 i = 0; i < voteAmount; ++i) {
            currentVoteId++;
            userNumbers[projectId][user].push(currentVoteId);
        }
        ...
        uint256 latestIndex = userNumbers[projectId][user].length - 1;
    ...
    function claimResult(
        ...
        for (uint256 i = 0; i < luckCodesLength; ++i) {
            ...
            if (
                userNumbers[projectId][user][userNumberIndexs[i]] == luckNumber
            ) {
                delete userNumbers[projectId][user][userNumberIndexs[i]];
                successAmount++;
    ...
    function getLuckNumber(
        uint256 projectId,
        uint256 code
    ) external view returns (uint256 luckNumber) {
        Project memory project = projects[projectId];
        require(project.isRaffled, "Themis: The draw was not completed");

        luckNumber =
            (uint256(keccak256(abi.encodePacked(project.randomWord, code))) %
                project.currentVoteId) +
            1;
    }
...
import "./interfaces/IThemisTeam.sol";
import "@openzeppelin/contracts/utils/Counters.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/cryptography/ECDSA.sol";

contract ThemisTeam is Ownable, IThemisTeam {
    using ECDSA for bytes32;

    using Counters for Counters.Counter;
    Counters.Counter public teamIdCounter;

    mapping(address => bool) private _executors;

    //Signature verification
    mapping(uint256 => bool) private _isUsedCreateTeamSignature;

    // user => teamId
    mapping(address => uint256) public userToTeamIds;

    // teamId => owner
    mapping(uint256 => address) private _teamOwner;
    ...
    function createTeam(
        ...
        //Step 1: Verify
        require(
            _isUsedCreateTeamSignature[signId] == false,
            "Themis: signature is used"
        );
        _isUsedCreateTeamSignature[signId] = true;
        address creater = _msgSender();

        address signer = keccak256(abi.encodePacked("THEMIS", signId, creater))
            .toEthSignedMessageHash()
            .recover(signature);
        require(isExecutor(signer), "Themis: signer is wrong.");

        //Step 2: createTeam
        require(userToTeamIds[creater] == 0, "ThemisTeam: Already in the team");

        teamIdCounter.increment();
        uint256 teamId = teamIdCounter.current();

        // Becoming a team owner
        _teamOwner[teamId] = creater;
        userToTeamIds[creater] = teamId;

        emit TeamCreated(teamId, creater, signId);

        return teamId;
    }

    /**
     * @notice As a team owner and managing team members, you can remove team members from the team or
     *         add people without a team to the team.
     * @param members Address of personnel to be operated
     * @param isEntereds  choose true to join the team, and choose false to remove the team
     */
    function manageMembers(
        address[] calldata members,
        bool[] calldata isEntereds
    ) external {
        address teamOwner = _msgSender();
        uint256 teamId = userToTeamIds[teamOwner];

        require(
            teamId != 0 && _teamOwner[teamId] == teamOwner,
            "ThemisTeam: Not the team owner"
        );

        uint256 successCounter;
        bool[] memory isSuccesses = new bool[](members.length);
        for (uint256 i = 0; i < members.length; ++i) {
            try
                this.manageMember(teamOwner, teamId, members[i], isEntereds[i])
            {
                isSuccesses[i] = true;
                successCounter += 1;
            } catch Error(string memory revertReason) {
                emit ManageMemberFailed(i, revertReason);
            }
        }
        require(successCounter > 0, "ThemisTeam: All manages failed");
    ...
    function manageMember(
        ...
        require(msg.sender == address(this), "ThemisTeam: Not the themis");

        require(member != teamOwner, "ThemisTeam: Unable to operate oneself");

        if (isEntered) {
            require(
                userToTeamIds[member] == 0,
                "ThemisTeam: Already in the team"
            );
            userToTeamIds[member] = teamId;
        } else {
            require(
                userToTeamIds[member] == teamId,
                "ThemisTeam: Not in the team"
            );
            delete userToTeamIds[member];
        }
    }

    function transferTeamOwnership(address member) external {
        address teamOwner = _msgSender();
        require(member != teamOwner, "ThemisTeam: Unable to operate oneself");
        ...
        require(userToTeamIds[member] == teamId, "ThemisTeam: Not in the team");

        _teamOwner[teamId] = member;

        emit TeamOwnershipTransferred(teamId, teamOwner, member);
    }

    function leaveTeam() external {
        address member = _msgSender();

        uint256 teamId = userToTeamIds[member];
        require(
            teamId != 0 && _teamOwner[teamId] != member,
            "ThemisTeam: Not in the team or team owner"
        );

        delete userToTeamIds[member];

        emit LeftTeam(teamId, member);
    }
