...
contract November721a is ERC721A, Managerial {
  ...
  /// Constant identifiers for the various rights used in this contract
   ...
   * @param _oldReceiver The old royalty receiver
   ...
   * @param _oldProxyRegistry The old proxy registry
   ...
   * @param _oldUri The old base URI
   ...
   * @param _oldFeeBps The old royalty fee
   ...
   */
  event TokenBurned(uint256 _tokenId, address _burner);
   ...
   * @param _owner The address to become the contract owner
  ...
  function setURI(string memory _uri) external hasValidPermit(SET_URI) {
    URI = _uri;
    emit TokenUriUpdated(URI, _uri, msg.sender);
  }
  ...
  function setContractURI(
    ...
    contractURI = _contractURI;
    emit ContractUriUpdated(contractURI, _contractURI, msg.sender);
  }
  ...
  function setPriceWei(
    ...
    priceWei = _priceWei;
    emit PriceUpdated(priceWei, _priceWei, msg.sender);
  }
  ...
  function setRoyaltyReceiver(
    ...
    emit RoyaltyReceiverUpdated(royaltyReceiver, _receiver, msg.sender);
  }
  ...
  function setRoyaltyFeeBps(
    ...
    royaltyFeeBps = _royaltyFeeBps;
    emit RoyaltyFeeUpdated(royaltyFeeBps, _royaltyFeeBps, msg.sender);
  }
  ...
  function burnTokensSudo(
    ...
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      _burn(_tokenIds[i]);
      emit TokenBurned(_tokenIds[i], msg.sender);
    }
  }
  ...
  function setProxyRegistry(
    ...
    address oldProxyRegistry = proxyRegistryAddress;
    proxyRegistryAddress = _proxyRegistryAddress;
    emit ProxyRegistryUpdated(
      oldProxyRegistry,
