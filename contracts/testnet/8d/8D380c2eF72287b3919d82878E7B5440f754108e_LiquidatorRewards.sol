...
contract LiquidatorRewards is ILiquidatorRewards, Owned, MixinSystemSettings, ReentrancyGuard {
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        ...
        newAddresses[2] = CONTRACT_SYNTHETIX;
    ...
    function earnedCrossChainInUSDC(address account) external view returns (uint256) {
        uint256 crossEarned = synthrAggregator().earned(account, synthetixDebtShare().balanceOfCrossChain(account));
    ...
    function rewardRestitution(address _to, uint256 _amount) external onlySynthetix returns (bool) {
    ...
    function notifyRewardAmount(uint256 reward) external onlySynthetix {
        uint256 sharesSupply = synthetixDebtShare().totalSupplyCrossChain();
