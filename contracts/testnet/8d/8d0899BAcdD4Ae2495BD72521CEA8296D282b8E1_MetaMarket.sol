...
interface IMetamarket {
    ...
    function mintMMT(address nftContract, uint256 numberOfTokens) external;
...
contract MetaMarket is IMetamarket, Ownable, ReentrancyGuard {
    ...
    function mintMMT(address nftContract, uint256 numberOfTokens) public nonReentrant {
        if (block.timestamp < _publicDrops[nftContract].startTime || block.timestamp > _publicDrops[nftContract].endTime) {
            // Revert if the drop stage is not active.
            revert NotActive(block.timestamp, _publicDrops[nftContract].startTime, _publicDrops[nftContract].endTime);
        }
