...
contract NFT721DIEM is ERC721Holder, ERC721Enumerable, Pausable, Ownable, ERC721Burnable
{
...
contract PointSystem is AccessControl {
    NFT721DIEM public nft721;
    bytes32 public constant PKT_ROLE = keccak256("PKT_ROLE");
    bytes32 public constant PDT_ROLE = keccak256("PDT_ROLE");
    ...
    struct PointOfStudentDetail {
        uint256 subjectId;
        uint256 point;
    }

    uint256[] public listSubject;
    string[] listStudents;
    mapping (uint256 => Subject) public subjects;
    mapping (string => Student) public students;
    mapping (string => mapping (uint256 => uint256)) public pointSubject;
    mapping (string => string) public nftPointStudent;
     ...
     modifier onlyPDT() {
        require(hasRole(PDT_ROLE, _msgSender()), "It is Not an PDT role");
        _;
    }

    modifier onlyPKT() {
        require(hasRole(PKT_ROLE, _msgSender()), "It is Not an PKT role");
        _;
    }    

    constructor() {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(PDT_ROLE, msg.sender);
        _setupRole(PKT_ROLE, msg.sender);
    }

    function addSubject(uint256 subjectId, string memory name) external onlyPKT {
        Subject storage subject = subjects[subjectId];
        subject.subjectId = subjectId;
        subject.subjectName = name;

        listSubject.push(subjectId);
    }

    function updateInfoStudent(string memory studentId, string memory name) public onlyPDT{
    ...
    function updatePoint(PointDetail[] memory pointDetail , string memory studentId) public onlyPKT {
        for (uint256 i =0 ; i < pointDetail.length; i ++) {
            require(pointDetail[i].point > 0 && pointDetail[i].point <= 10 , "wrong point");
            pointSubject[studentId][pointDetail[i].subjectId] = pointDetail[i].point;
        }
    ...
    function getPointWithSubject(string memory studentId, uint256 subjectId) public view returns(uint256) {
        uint256 point = pointSubject[studentId][subjectId];
        return point;
    }
    ...
    function getPointOfStudent(string memory studentId) public view returns( string memory, uint256, PointOfStudentDetail[] memory, string memory) {
        ...
        PointOfStudentDetail[] memory pointDetail = new PointOfStudentDetail[](listSubject.length);
    ...
    function getAllStudent() public view returns(Student[] memory) {
        Student[] memory student = new Student[](listStudents.length);
        
        for(uint256 i = 0 ; i < listStudents.length; i++) {
            student[i].studentId = listStudents[i];
            student[i].studentName = students[listStudents[i]].studentName;
        }

        return student;
    }

    function setContractNFT(NFT721DIEM _nft721) public onlyAdmin {
        nft721 = _nft721;
    }

    function changeOwnershipNFTContract(address _newOwner) external onlyAdmin {
        nft721.transferOwnership(_newOwner);
    }

    function mintNFT(string memory URI, string memory studentId) public onlyPDT returns(uint256)  {
        uint256 tokenId = nft721.safeMint(msg.sender, URI);

        nftPointStudent[studentId] = URI;
