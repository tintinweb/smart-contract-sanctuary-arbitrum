...
// File: contracts/uyux/UxuyRewardSelf.sol
...
interface UxuyToken {
    function reward(uint256 amount, address toUser) external;
}

/**
 * @title UxuyRewardSelf
 */
contract UxuyRewardSelf is ChainlinkClient, ConfirmedOwner {
    using Chainlink for Chainlink.Request;
    ...
    address private linkOracle;

    /// events
    event RequestBalanceBefore(
    ...
    /// @notice request cost pay for oracle job
    uint256 public requestPrice = 0.00001 ether;
    ...
    constructor(
        address _linkToken,
        address _linkOracle,
        address _uxuyTokenAddress,
        string memory _balanceRequestUrl
    ) ConfirmedOwner(msg.sender) {
        setChainlinkToken(_linkToken);
        setChainlinkOracle(_linkOracle);
        jobId = "cd6001af64614695bb56cb2fdf6a688b";
        linkFee = (1 * LINK_DIVISIBILITY) / 1000; // 0.001
        linkOracle = _linkOracle;
        _uxuyToken = UxuyToken(_uxuyTokenAddress);
        balanceRequestUrl = _balanceRequestUrl;
        superOperators[msg.sender] = true;
    }

    /**
     * @notice check from server
     */
    function claim(
        address toUser,
        uint256 amount,
        ...
        require(msg.value >= requestPrice, "Not enough ETH sent for request");
        
        // check linkToken balance
        LinkTokenInterface link = LinkTokenInterface(chainlinkTokenAddress());
        require(
            link.balanceOf(address(this)) >= linkFee,
            " Not enough LinkToken sent for request "
        );
        ...
        // Sends the request
        requestId = sendChainlinkRequestTo(linkOracle, req, linkFee);

        requestIdToAttemptee[requestId] = toUser;
        requestIdToTransId[requestId] = transId;
        emit RequestBalanceBefore(requestId, toUser, transId);
    }
        ...
        returns (uint)
    {
        ...
        require(
            requestIdToAttemptee[_requestId] != address(0),
            "Error request fulfill!"
        );

        if (_volume == 0) {
            emit ClaimedFailed(
                requestIdToAttemptee[_requestId],
                _requestId,
                requestIdToTransId[_requestId]
            );
        ...
        require(_volume > 0, "reward UxuyToken is zero");
    ...
    /**
     * @notice reward uxuyToken
     */
    function rewardUxuyToken(
        uint256 amount,
        address account,
        bytes32 requestId,
        string memory transId
    ) internal {
        // mint and transfer to wallet
        try _uxuyToken.reward(amount, account) {
     ...
     */
    function setBalanceRequestUrl(string memory _balanceRequestUrl)
        public
        onlyOwner
    {
        balanceRequestUrl = _balanceRequestUrl;
    ...
    function setLinkFee(uint256 _fee) public onlyOwner {
        linkFee = (_fee * LINK_DIVISIBILITY) / 100;
    }
    ...
    /// @notice set request price for oracle job cost
