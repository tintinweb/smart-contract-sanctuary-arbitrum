...
contract PixelMushrohmStaking is IPixelMushrohmStaking, PixelMushrohmAccessControlled, ReentrancyGuard {
    ...
    uint256 public stakingPrice;
    uint256 public levelUpPrice;
    ...
    {
        if (paymentToken != address(0) && stakingPrice != 0) {
            IERC20(paymentToken).safeTransferFrom(msg.sender, address(this), stakingPrice);
        }
        ...
        updateTotalMushrohmsStaked(false)
    {
        ...
        stakedTokenData[_tokenId].lastLevel = levelEarned(_tokenId);
    ...
    function levelUp(uint256 _tokenId) external override whenNotPaused nonReentrant onlyPixelMushrohmOwner(_tokenId) {
        ...
        if (paymentToken != address(0) && levelUpPrice != 0) {
            IERC20(paymentToken).safeTransferFrom(msg.sender, address(this), levelUpPrice);
        }

        pixelMushrohm.updateLevel(_tokenId);
        stakedTokenData[_tokenId].lastLevel = 0;
    ...
    function sporePowerEarned(uint256 _tokenId) public view override returns (uint256 sporePower) {
        require(address(pixelMushrohm) != address(0), "PixelMushrohm: Invalid pixelMushrohm address");
        if (stakedTokenData[_tokenId].timestampStake == 0) return 0;
        ...
        sporePower = (
            (
                pixelMushrohm.getSporePowerPerWeek(_tokenId).mul(timeDelta).mul(
                    pixelMushrohm.getLevelMultiplier(_tokenId)
                )
            ).div(1e9)
        ).div(WEEK);
    }

    function levelEarned(uint256 _tokenId) public view override returns (uint256 level) {
        if (stakedTokenData[_tokenId].timestampLevel == 0) return stakedTokenData[_tokenId].lastLevel;
        uint256 timeDelta = block.timestamp.sub(stakedTokenData[_tokenId].timestampLevel);
        level = (levelPerWeek(_tokenId).mul(timeDelta).div(WEEK)).add(stakedTokenData[_tokenId].lastLevel);
    }
    ...
    function isEligibleForLevelUp(uint256 _tokenId) public view override returns (bool) {
        ...
        return levelEarned(_tokenId) >= pixelMushrohm.getLevelCost();
    }
}
