...
// FUNCTIONS
        ...
        onlyWhenActive(recipeID)
    {
        ...
        require (keccak256(abi.encode(recipes[recipeID].recipeItems)) == keccak256(abi.encode(itemIDs)), "Items should match the recipe");
        require (keccak256(abi.encode(recipes[recipeID].recipeAmounts)) == keccak256(abi.encode(amounts)),"Items Amounts should match the recipe");
...
import "../tokens/interfaces/IGold.sol";
...
// MODIFIERS
    ...
    modifier onlyIfRecipeExists(uint256 recipeID) {
        require(recipeID < totalRecipes,"Recipe does not exist");
        _;
    }
...
// ADMIN FUNCTIONS
    ...
    /// @param recipe the recipe
    function updateItem(uint256 recipeID, Recipe calldata recipe)
        external 
        onlyOwner
    ...
    /// @param recipeID ID of the recipe
    function deleteItem(uint256 recipeID)
        external 
        onlyOwner
        onlyIfRecipeExists(recipeID)
    {
