...
contract XMY is IERC20, Ownable {
    ...
    uint256 oneMineReward = 10000 *10**_decimals;
    uint256 twoMineReward = 20000 *10**_decimals;
    uint256 threeMineReward = 30000 *10**_decimals;
    uint256 fourMineReward = 40000 *10**_decimals;
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                if(presaleList[from] || presaleList[to]){
                    require(block.timestamp < startPreTradeTime); //小于.  //撤车子与买B重合收费。//ETH撤池子不收费
                }else{
