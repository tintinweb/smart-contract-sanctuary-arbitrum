...
interface ICardMintingStation {
    function mintCollectible(address _tokenReceiver, uint256 _percent) external returns (uint256);
}
...
interface IMintingStation {
    function mintCollectible(address _tokenReceiver, uint256 _level, uint256 _type) external returns (uint256);
}

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.4;

interface IMintingStationBase {
    function mintCollectible(address _tokenReceiver) external returns (uint256);
}
...
interface INFT {
    function levels(uint256 _address) external;
    function types(uint256 _address) external;
...
contract Market is ERC721Holder, Ownable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    mapping(address => bool) public allowBuyTokens;
    ...
    mapping(address => uint256) public totalPendingRevenue;
    ...
    mapping(address => mapping(uint256 => EnumerableSet.UintSet))
        private _askTokenIds; // Set of tokenIds for a collection
    mapping(address => mapping(uint256 => Collection)) private _collections; // Details about the collections
    mapping(address => mapping(address => EnumerableSet.UintSet))
        private _tokenIdsOfSellerForCollection;

    mapping(address => mapping(uint256 => AskHistoryDetails))
        private askHistory;

    mapping(address => EnumerableSet.UintSet) private tokenListed;

    struct Ask {
        address seller; // address of the seller
        uint256 price; // price of the token
        address tokenBuy; // token buy
    }
    ...
    struct TransferDetails {
        address collectionAddress; // address of the nft
        uint256[] tokenId; // list token Id
    }

    struct BuyTokensDetails {
        address collectionAddress; // address collection of nft
    ...
    event CollectionClose(address indexed collection, uint256 collectionId);
    ...
    // Transfer list token Id belong to one address success
    ...
    // event buy tokens
    event BuyTokensEvent(BuyTokensDetails[] buyTokensDetail);
    ...
    function setAllowBuyTokens(
        address _token,
        bool _status
    ) external onlyAdmin {
        allowBuyTokens[_token] = _status;
    }
    ...
    function buyToken(
        ...
        require(_tokenBuy == askOrder.tokenBuy, "Buy: Incorrect Token");

        IERC20(askOrder.tokenBuy).safeTransferFrom(
            address(msg.sender),
            address(this),
            _price
        );
    ...
    function cancelAskOrder(
        ...
        _askTokenIds[_collection][_collectionId].remove(_tokenId);
        tokenListed[_collection].remove(_tokenId);
        ...
        // Emit event
        emit AskCancel(_collection, msg.sender, _tokenId, _collectionId);
    }
    ...
    function claimPendingRevenue(address _token) external nonReentrant {
        ...
        totalPendingRevenue[_token] -= revenueToClaim;
    ...
    function modifyAskOrder(
        ...
        require(
            _collections[_collection][_collectionId].status ==
        ...
        require(
            allowBuyTokens[_newTokenBuy] == true,
            "Order: token buy not allow"
        );

        require(
            askHistory[_collection][_tokenId].collectionId == _collectionId,
            "wrong collection"
        );
    ...
    function addCollection(
        ...
        require(
            _collections[_collection][_collectionId].creatorAddress ==
                address(0),
            "Operations: Collection already listed"
        );
        //require((!_collectionAddressSet));
        require(
        ...
        _collections[_collection][_collectionId] = Collection({
    ...
    function closeCollectionForTradingAndListing(
        ...
        _collections[_collection][_collectionId].status = CollectionStatus
            .Close;
    ...
    function modifyCollection(
        ...
        require(
            _collections[_collection][_collectionId].collectionId != 0,
            "Operations: Collection not listed"
        );
        require(
            (_creatorFee == 0 && _creator == address(0)) ||
        ...
        Collection storage collection = _collections[_collection][
            _collectionId
        ];

        require(
            msg.sender == collection.creatorAddress ||
    ...
    function recoverNonFungibleToken(
        ...
        require(
            !tokenListed[_collection].contains(_tokenId),
        ...
        emit NonFungibleTokenRecovery(_collection, _tokenId);
    }
    ...
    function viewAsksByCollectionAndTokenIds(
        ...
        for (uint256 i = 0; i < length; i++) {
            if (_askTokenIds[collection][collectionId].contains(tokenIds[i])) {
                statuses[i] = true;
            } else {
                statuses[i] = false;
            }
        ...
        returns (uint256[] memory tokenIds, Ask[] memory askInfo, uint256)
    {
        ...
        if (length > _askTokenIds[collection][collectionId].length() - cursor) {
            length = _askTokenIds[collection][collectionId].length() - cursor;
        }
        ...
        for (uint256 i = 0; i < length; i++) {
            tokenIds[i] = _askTokenIds[collection][collectionId].at(cursor + i);
            askInfo[i] = _askDetails[collection][tokenIds[i]];
        }
    ...
    {
        if (
            _collections[collection][collectionId].status !=
            CollectionStatus.Open
        ) {
            return (0, 0, 0);
        }
        ...
        return (
            _calculatePriceAndFeesForCollection(collection, price, collectionId)
        );
    }
    ...
    function canTokensBeListed(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            listingStatuses[i] = _canTokenBeListed(
                _collection,
                _tokenIds[i],
                _collectionId
            );
    ...
    function _buyToken(
        ...
        // Transfer TOKEN
        IERC20(askOrder.tokenBuy).safeTransfer(askOrder.seller, netPrice);

        // Update pending revenues for treasury/creator (if any!)
        if (creatorFee != 0) {
            pendingRevenue[
                _collections[_collection][_collectionId].creatorAddress
            ][askOrder.tokenBuy] += creatorFee;
        }

        // Update trading fee if not equal to 0
        if (tradingFee != 0) {
            pendingRevenue[treasuryAddress][askOrder.tokenBuy] += tradingFee;
        }

        totalPendingRevenue[askOrder.tokenBuy] =
            totalPendingRevenue[askOrder.tokenBuy] +
            tradingFee +
            creatorFee;
    ...
    {
        tradingFee =
            (_askPrice * _collections[_collection][_collectionId].tradingFee) /
            10000;
        creatorFee =
            (_askPrice * _collections[_collection][_collectionId].creatorFee) /
            10000;
    ...
    function batchTransfer(
        TransferDetails[] memory transferDetail,
        address receiver
    ) external {
        for (uint256 i = 0; i < transferDetail.length; i++) {
            for (uint256 j = 0; j < transferDetail[i].tokenId.length; j++) {
                address collection = transferDetail[i].collectionAddress;
                uint256 tokenId = transferDetail[i].tokenId[j];
        ...
        emit BatchTransferEvent(transferDetail, receiver);
    }

    function buyTokens(BuyTokensDetails[] memory buyTokensDetail) external {
        for (uint256 i = 0; i < buyTokensDetail.length; i++) {
            address collection = buyTokensDetail[i].collectionAddress;
            uint256 tokenId = buyTokensDetail[i].tokenId;
            uint256 price = buyTokensDetail[i].price;
            address tokenBuy = buyTokensDetail[i].tokenBuy;
            uint256 collectionid = buyTokensDetail[i].collectionId;
            buyToken(collection, tokenId, price, tokenBuy, collectionid);
        }

        //emit event
        emit BuyTokensEvent(buyTokensDetail);
    }

    /**
     * @notice Create ask order
     * @param _collection: contract address of the NFT
     * @param _tokenIds: array of tokenId of the NFT
     * @param _askPrices: array of price for listing (in wei)
     * @param _tokenBuy: token buy NFT
     */
    function createAskOrders(
        address _collection,
        ...
        for (uint256 index = 0; index < _tokenIds.length; index++) {
            uint256 _askPrice = _askPrices[index];
            ...
            require(
                allowBuyTokens[_tokenBuy] == true,
                "Order: token buy not allow"
            );

            if (askHistory[_collection][_tokenId].hasHistory == false) {
                askHistory[_collection][_tokenId].collectionId = _collectionId;
                askHistory[_collection][_tokenId].hasHistory = true;
            } else {
                require(
                    askHistory[_collection][_tokenId].collectionId ==
                        _collectionId,
                    "wrong collection"
                );
            }
            ...
            // Adjust the information
            _tokenIdsOfSellerForCollection[msg.sender][_collection].add(
                _tokenId
            );
            ...
            _askTokenIds[_collection][_collectionId].add(_tokenId);
            tokenListed[_collection].add(_tokenId);
...
contract Tournament is ERC721, AccessControl, ReentrancyGuard, ERC721URIStorage {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Counters for Counters.Counter;
    ...
    struct RedeemRequest {
        uint256 requestId;
        uint256[] listTypeOfNFTsId;
        mapping(uint256 => uint256) numberOfTicket;
    ...
    mapping(uint256 => TicketDetail) public ticketDetails;
    mapping(uint256 => TypeOfNFT) public listTypeOfNFTs;
    ...
    uint256[] listTicketId;
    
    event createTypeOfNFTEvent(uint256 ticketId, uint256 price);
    event TournamentEvent(TournamentDetail tournament);
    ...
    function createTypeOfNFT(
        uint256 price
    ) public onlyAdmin {
        require(price > 0, 
        "The value of priceTicket must be greater than 0");
        _tokenIdCounter.increment();
        uint256 ticketId = _tokenIdCounter.current();

        TypeOfNFT storage TypeOfNFT = listTypeOfNFTs[ticketId];

        TypeOfNFT.ticketId = ticketId;
        TypeOfNFT.price = price;
        TypeOfNFT.status = TicketCollectionStatus.OPEN;

        listTicketId.push(ticketId);

        emit createTypeOfNFTEvent(ticketId, price);
    ...
    function cleseTournament(uint256 tournamentId) public onlyAdmin {
        TournamentDetail storage tournament = tournamentDetails[tournamentId];
        require(
            tournament.status == TournamentStatus.OPEN,
    ...
    function buyTypeOfNFT(uint256 ticketId, address tokenBuy, uint256 quantity, string memory URI) public 
    {
        TypeOfNFT storage typeOfNFT = listTypeOfNFTs[ticketId];
        require(typeOfNFT.status == TicketCollectionStatus.OPEN, "");
        require(whiteListPaymentToken[tokenBuy], "");
        ...
        SafeERC20.safeTransferFrom(
            ...
            msg.sender,
            address(this),
            quantity.mul(typeOfNFT.price)
        );    
    }

    function buyTicket(uint256 ticketId, uint256[] memory tokenId) public {
    ...
    function useTicket(
        ...
        if(whiteListPaymentToken[token] && ticketId ==0) {
            TypeOfNFT storage typeOfNFT = listTypeOfNFTs[ticketId];
            SafeERC20.safeTransferFrom(
            IERC20(token),
            msg.sender,
    ...
    function createRedeemRequest(ArgRequest[] memory argRequest) public {
        for(uint256 i =0; i < argRequest.length; i ++) {
            uint256 ticketId = argRequest[i].ticketId;
            uint256 quantity = argRequest[i].quantity;
        }
    }
...
contract Tournament111 is AccessControl, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
