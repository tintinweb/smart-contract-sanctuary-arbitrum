    ...
    OwnableUpgradeable
{
    ...
    function withdrawETH() external onlyOwner {
        // Calculate the amount to withdraw, you can specify the amount you want to withdraw
        uint256 amountToWithdraw = address(this).balance;
        require(amountToWithdraw > 0, "No ETH to withdraw");

        // Transfer the ETH to the contract owner
        address payable ownerAddress = payable(owner());
        sendValue(ownerAddress, amountToWithdraw);

        // You can also emit an event to log the withdrawal
        // emit EthWithdrawn(ownerAddress, amountToWithdraw);
    }
}