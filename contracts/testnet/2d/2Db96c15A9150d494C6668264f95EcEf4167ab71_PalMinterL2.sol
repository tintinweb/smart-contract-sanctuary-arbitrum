...
import "./PalMinter.sol";
// import "@arbitrum/nitro-contracts/src/libraries/AddressAliasHelper.sol";
// import "@arbitrum/nitro-contracts/src/precompiles/ArbSys.sol";
import "./LayerZero/lzApp/NonblockingLzApp.sol";

contract PalMinterL2 is PalMinter, NonblockingLzApp {
    address public l1Contract;
    address public endpoint;
    uint16 public srcChainId;

    mapping(uint256 => address) public genesisOwner;

    event GenesisOwnershipTransferred(uint256 genesisTokenId, address newOwner, bytes srcAddress);
    ...
    function updateL1Contract(address _l1Contract) public onlyOwner {
        l1Contract = _l1Contract;
    }

    // function mintPalFromL1(uint256 _genesis, PalToken.Rarity _rarity) external payable whenNotPaused {
    //     require(
    //         msg.sender == AddressAliasHelper.applyL1ToL2Alias(l1Contract),
    //         "Can be called only by l1Contract"
    //     );
    ...
    function _nonblockingLzReceive(
        ...
        (uint256 _genesis, address _newOwner) = abi.decode(_payload, (uint256, address));
        genesisOwner[_genesis] = _newOwner;
        emit GenesisOwnershipTransferred(_genesis, _newOwner, _srcAddress);
    }

    function setGenesisOwnership(bytes memory _payload, bytes memory _srcAddress) public {
        (uint256 _genesis, address _newOwner) = abi.decode(_payload, (uint256, address));
        genesisOwner[_genesis] = _newOwner;
        emit GenesisOwnershipTransferred(_genesis, _newOwner, _srcAddress);
    }
...
contract PalMinter is Ownable, Pausable {
    ...
    constructor(address _genesisToken, address _palToken, address _treasury, address _daoWallet, uint256 _treasuryPercentage) {
        genesisToken = IERC721(_genesisToken);
    ...
    function mintPal(uint256 _genesis, PalToken.Rarity _rarity) external payable whenNotPaused {
        require(genesisToken.ownerOf(_genesis) == _msgSender(), "PalMinter: Must be an owner of the Genesis to mint a Pal");
