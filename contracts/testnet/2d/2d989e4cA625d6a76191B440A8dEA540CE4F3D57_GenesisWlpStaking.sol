...
contract GenesisWlpStaking is Access, ReentrancyGuard {
  ...
  constructor(
    ...
    claimWLPAddress = _claimWlpAddress;
  ...
  function pendingRewards(address _staker) public view returns (uint256 _vWINR, uint256 _WLP) {
    ...
    if (staker.amount > 0) {
      _vWINR =
        ((staker.amount * pool.accumulatedRewardsPerShare) / ACC_REWARD_PRECISION) -
        staker.rewardDebt;
  ...
  function claimWLP() public {
    // TODO: funtion to claim wlp holder rewards from fee collector
    // Fee collector should return amount
    // store returned value for computation for reward wlp
  ...
  function fundReward(uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
