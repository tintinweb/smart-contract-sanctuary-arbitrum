...
contract AoriCall is ERC20, ReentrancyGuard {
    ...
    constructor(
        ...
        decimalDiff = (10**UNDERLYING.decimals()) / (10**6); //The underlying decimals must be greater than or equal to USDC's decimals.
        oracle = _oracle;
...
contract AoriPut is ERC20, ReentrancyGuard {
    ...
    function mintPut(uint256 quantityOfUSDC, uint256 seatId) public nonReentrant returns (uint256) {
        ...
        if (msg.sender == AORISEATSADD.ownerOf(seatId)) {
            ...
            // Calculating the fees out of 100 to go to the seat owner
            feeToSeat = (refRate * mintingFee) / 100;     
            optionsToMint = ((quantityOfUSDC - mintingFee) * 10**6) / strikeInUSDC; //(1e6*1e6) / 1e6
    ...
    function buyerSettlementITM(uint256 optionsToSettle) public nonReentrant returns (uint256) {
        ...
        uint256 USDCOwed = ((optionsToSettle / decimalDiff) * profitPerOption) / 10**6; //((1e18 / 1e12) * 1e6) / 1e6
        //transfers
        _burn(msg.sender, optionsToSettle);
    ...
    function sellerSettlementITM() public nonReentrant returns (uint256) {
        ...
        uint256 USDCToReceive = ((optionsToSettle * settlementPrice) / decimalDiff) / 10**6; //((1e18 / 1e12) * 1e6) / 1e6
        //store the settlement
        optionSellers[msg.sender] = 0;
    ...
    function sellerSettlementOTM() public nonReentrant returns (uint256) {
        ...
        //settle
        uint256 USDCOwed = ((optionsSold / decimalDiff) * strikeInUSDC) / 10**6; //((1e18 / 1e12) * 1e6) / 1e6
        USDC.transfer(msg.sender, USDCOwed);
...
contract Ask is ReentrancyGuard {
    ...
    AoriSeats public immutable AORISEATSADD = AoriSeats(0x4234B576ebAef1aa258F1B2A53463b07e359EC26);
...
contract Orderbook is Ownable {
    ...
    function getActiveAsks() external view returns (Ask[] memory) {
        ...
        for (uint256 i; i < asks.length; i++) {
            Ask ask = Ask(asks[i]);
            if (ask.isFunded() && !ask.hasEnded() && address(ask) != address(0x0)) {
    ...
    function getActiveBids() external view returns (Bid[] memory) {
        ...
        for (uint256 i; i < bids.length; i++) {
            Bid bid = Bid(bids[i]);
            if (bid.isFunded() && !bid.hasEnded() && address(bid) != address(0x0)) {
...
contract OrderbookFactory is Ownable {
    ...
    */
    function createOrderbook(
        ...
        emit AoriOrderbookCreated(address(orderbook), fee, OPTION_);
        return (orderbook);
    }
