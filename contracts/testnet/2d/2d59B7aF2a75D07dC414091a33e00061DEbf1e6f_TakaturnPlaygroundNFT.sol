...
/// @title A Takaturn NFT using ERC721
/// @author Kevin Tan
/// @notice You can use this contract for mint and transfer NFT if user made the tx in takaturn.
/// @dev All function calls are currently implemented without side effects
contract TakaturnPlaygroundNFT is ERC721, Ownable, Pausable {
    using Strings for uint256;
    ...
    /// @notice Owner can only mint their the NFT and transfer NFT to receiver without tokenId.
    /// @param tokenId the default tokenId
    ...
    function mintNFT(uint256 tokenId, address receiver) external onlyOwner whenNotPaused returns (uint256) {
        ...
        require(tokenId == 0, 'Default TokenId should be Zero.');
    ...
    function burn(uint256 tokenId) external whenNotPaused {
        require(ownerOf(tokenId) == msg.sender || owner() == msg.sender, 'Only owner can burn their nft');
        _burn(tokenId);
    }

    /// @notice Owner can update the mint end time using this function.
    /// @param _mintEndTime mint end time
    function updateMintEndTime(uint256 _mintEndTime) external onlyOwner whenNotPaused {
        require(_mintEndTime > mintStartTime && _mintEndTime > block.timestamp, 'Invalid mintEndTime');
        mintEndTime = _mintEndTime;
    }

    /// @notice Owner can update the max mint count using this function.
    /// @param _maxMintAmount max value
    function updateMaxMintAmount(uint256 _maxMintAmount) external onlyOwner whenNotPaused {
        require(0 < _maxMintAmount, 'Invalid maxMintAmount');
        require(block.timestamp < mintEndTime, 'The event was finished');
        maxMintAmount = _maxMintAmount;
    }

    /// @notice factory can destroy this event.
    function destroy() external onlyOwner whenNotPaused {
