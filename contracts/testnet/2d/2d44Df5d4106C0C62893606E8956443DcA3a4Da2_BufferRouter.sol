...
contract BufferRouter is AccessControl, IBufferRouter {
    ...
    mapping(uint256 => QueueIdMapping) public queueIdToOptionId;
    mapping(uint256 => bool) public cancelledQueueIds;
    ...
    mapping(uint256 => uint256) public limitOrder;
    ...
    function verifyTrade(
        ...
        require(
            _validateUser(
                user,
                isOneCT ? accountMapping[user] : user,
                params,
                userSignature,
                signatureTimestamp
            ),
    ...
    ) external view returns (TradeData[] memory trades) {
        uint256 latestIndex = queuedTrades[latestQueueId].userQueueIndex;

        trades = new TradeData[](userQueuedIds[user].length - latestIndex);
        if (trades.length > 0) {
            for (
                uint256 index = latestIndex;
                index < userQueuedIds[user].length;
                index++
            ) {
                QueuedTrade storage queuedTrade = queuedTrades[
                    userQueuedIds[user][index]
                ];
                QueueIdMapping memory queueIdMapping = queueIdToOptionId[
                    queuedTrade.queueId
                ];

                (
                    IBufferBinaryOptions.State state,
                    uint256 strike,
                    uint256 amount,
                    ...
                    uint256 totalFee,
                    uint256 createdAt
                ) = IBufferBinaryOptions(queuedTrade.targetContract).options(
                        queueIdMapping.optionId
                    );
                IBufferBinaryOptions.Option memory option;
                trades[index - latestIndex] = TradeData(
                    queueIdMapping.optionId,
                    queuedTrade.queueId,
                    queuedTrade,
                    queueIdMapping.isSet
                        ? IBufferBinaryOptions.Option(
                            state,
                            strike,
                            amount,
                            lockedAmount,
                        ...
                        : option,
                    cancelledQueueIds[queuedTrade.queueId]
                );
            }
        }
    }
    ...
    function registerAccount(address oneCT) external {
        accountMapping[msg.sender] = oneCT;
    ...
    function openLimitOrder(
        ...
        bytes memory userSignature,
        bool isOneCT,
        uint256 limitOrderTimestamp
    ) public returns (uint256 queueId) {
        _validateKeeper();

        verifyTrade(params, user, userSignature, signatureTimestamp, isOneCT);
        queueId = _initiateTrade(params, queuedTimestamp, user);
        prevSignatures[userSignature] = true;
        limitOrder[queueId] = limitOrderTimestamp;
    }
    ...
    function _validateUser(
        ...
        if (error == ECDSA.RecoverError.NoError) {
            return recoveredSigner == signatureUser;
        } else {
            return false;
        }
    }
    ...
    function _openQueuedTrade(uint256 queueId, uint256 price) internal {
        ...
        queueIdToOptionId[queueId] = QueueIdMapping({
            optionId: optionId,
            isSet: true
        });
    ...
    function _resolveQueuedTrade(OpenTradeParams memory params) internal {
        ...
        uint256 limitOrderExpiry = limitOrder[params.queueId];
        ...
        if (
            ...
            (limitOrderExpiry == 0 &&
                params.timestamp != queuedTrade.queuedTime) ||
            (limitOrderExpiry > 0 &&
                params.timestamp > limitOrderExpiry &&
                params.timestamp < queuedTrade.queuedTime)
        ) {
        ...
        if (
            (limitOrderExpiry == 0 &&
                block.timestamp - queuedTrade.queuedTime <= MAX_WAIT_TIME) ||
            (limitOrderExpiry > 0 && block.timestamp <= limitOrderExpiry)
        ) {
    ...
    function _cancelQueuedTrade(uint256 queueId) internal {
        ...
        cancelledQueueIds[queueId] = true;
    }
}
