...
contract BountySystem is IBountySystem, GameRegistryConsumerUpgradeable {
    ...
    function isBountyAvailable(
        ...
        bool value = enabledComponent.getValue(bountyId);
        if (!value) {
            return false;
        }
    ...
    function startBounty(
        ...
        _verifyNftInputs(account, entities, activeBountyId);
    ...
    function _verifyNftInputs(
        ...
        for (uint256 i = 0; i < entityNfts.length; ++i) {
            // Check Pirate NFT
            _checkPirateNft(traitsProvider, entityNfts[i], account);
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.enabledcomponent.v1")
);
...
library EnabledComponentStorage {
 ...
 */
contract EnabledComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param value Whether the entity is enabled or not
     */
    function setValue(
        uint256 entity,
        bool value
    ...
    function getLayoutValue(
        ...
        value = EnabledComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Whether the entity is enabled or not
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (bool value) {
        if (has(entity)) {
            Layout memory s = EnabledComponentStorage.layout().entityIdToStruct[
                entity
            ];
            (value) = abi.decode(_getEncodedValues(s), (bool));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = EnabledComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete EnabledComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete EnabledComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
