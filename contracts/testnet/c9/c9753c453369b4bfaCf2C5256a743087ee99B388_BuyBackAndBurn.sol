...
contract BuyBackAndBurn {
    ...
    mapping(address => mapping(uint => UserInfo)) public userParticipateInfos;  //users addresse => index => info
    ...
    event Deposited(address from, uint256 wethAmount, uint256 agiAmount);
    ...
    function deposit(uint amountIn, uint minAmountOut, Route[] calldata routes, address ref) external nonReentrant {
    ...
    function depositETH(address ref) external payable nonReentrant {
    ...
    function getEarned(address user) public view returns(uint, uint, uint){
        ...
        for(uint i = 0; i < count; i++){
            ...
            totalVest += _userinfo.vestAmount;
            claimed += _userinfo.claimed;
        }
        return (earned, totalVest, claimed);
    }
    ...
    function _updateForDeposit(address user, address ref, uint wethAmount) internal{
        ...
        emit Deposited(user, wethAmount, _agiEarn);
    }
