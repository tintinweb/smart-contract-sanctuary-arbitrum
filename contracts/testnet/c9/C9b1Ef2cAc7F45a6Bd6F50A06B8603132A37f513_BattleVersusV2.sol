...
contract BattleVersusV2 is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  function fight(
    ...
    fightData.lpBonusMultiplier = _getLpBonusMultiplier();
  ...
  function _getLpBonusMultiplier() internal view returns (uint256 multiplier) {
    ...
    uint256 balancePercentage = (lpToken.balanceOf(msg.sender) * ONE_HUNDRED) / totalSupply;
  ...
  function updateLpBonusThresholds(uint256[] calldata _lpBonusThresholds) external onlyAdmin {
  ...
  function _calculateAnima(
    ...
    uint256 animaBonus = hasBonus
      ? ((animaReward * (_getAnimaMultiplier(MID - data.probability) + bonusLpMultiplier)) /
        PRECISION)
      : 0;
