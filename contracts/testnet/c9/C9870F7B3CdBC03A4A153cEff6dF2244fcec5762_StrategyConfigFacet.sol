...
import {InvalidParam} from "../errors/GenericErrors.sol";
...
contract StrategyConfigFacet is IStrategyConfig {
    ...
    function initConfig(address _usdcToken, address _wrappedNative) external {
        LibAccessControlEnumerable.checkRole(Constants.DEPLOYER_ROLE);
        LibStrategyConfig.Layout storage l = LibStrategyConfig.layout();
        if (_usdcToken == address(0) || _wrappedNative == address(0)) {
            revert InvalidParam();
        }
        l.usdcToken = _usdcToken;
        l.wrappedNativeToken = _wrappedNative;
    }

    // function isSupportCollateralToken(address token) external view returns (bool) {
    //     return LibStrategyConfig.layout().isSupportCollateralToken[token];
    // }
    ...
    function removeMarket(address underlying_) external {
        LibAccessControlEnumerable.checkRole(Constants.ADMIN_ROLE);
        LibStrategyConfig.Layout storage l = LibStrategyConfig.layout();
        // _ensureListed(l.markets[underlying_]);
        LibStrategyConfig._ensureListed(underlying_);
