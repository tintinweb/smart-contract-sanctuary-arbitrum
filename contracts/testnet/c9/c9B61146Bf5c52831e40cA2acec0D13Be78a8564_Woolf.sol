...
contract Woolf is IWoolf, ERC721Enumerable, Ownable, Pausable {
  ...
  function upgradeLevelCost(uint256 tokenId, uint256 upgradeLevels) public view returns (uint256) {
      require(upgradeLevels > 0, "level should > 0");

      uint256 tokenCost = 0;
      uint256 fromLevel = wolf_level[tokenId];
      uint256 toLevel = fromLevel + upgradeLevels;

      // 距离上次升级超过24小时，则升级费用重置
      if((block.timestamp - lastUpgradeTimestamp) > MINIMUM_TO_RESET_UPGRAGE)
        tokenCost = 1e18 * (toLevel - fromLevel + 1) * (toLevel - fromLevel) * 1000 / 2;
      else
        tokenCost = 1e18 * (toLevel + fromLevel + 1) * (toLevel - fromLevel) * 1000 / 2;
   ...
   // 升级消耗的代币直接打入黑洞
   function upgradeLevel(uint256 tokenId, uint256 upgradeLevels) internal {
      require(barn.isStaking(tokenId), "unstake first!");
