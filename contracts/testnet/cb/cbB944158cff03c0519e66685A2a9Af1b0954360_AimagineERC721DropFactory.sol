...
/**
 * @title AimagineERC721Drop
 * @dev The AimagineERC721Drop contract is an implementation of the ERC-721A standard with additional features
 * specific to the Aimagine project. It allows the creation and management of non-fungible tokens (NFTs)
...
import {IAimagineERC721Drop} from "./interfaces/IAimagineERC721Drop.sol";

contract AimagineERC721Drop is ERC721A, IERC2981, ReentrancyGuard, AccessControl, IAimagineERC721Drop, Ownable {
    // Storage variables

    /**
     * @dev Configuration struct to store various contract settings.
     */
    IAimagineERC721Drop.Configuration public config;

    /**
     * @dev SalesConfiguration struct to store sales-related settings.
     */
    IAimagineERC721Drop.SalesConfiguration public salesConfig;

    /**
     * @dev CollectionMeta struct to store coillection meta data.
     */
    IAimagineERC721Drop.CollectionMeta public collectionMeta;
    ...
    /**
     * @dev Immutable variables for the Aimagine minting fee and its recipient.
     */
    uint256 private immutable AIMAGINE_MINT_FEE;
    address payable private immutable AIMAGINE_MINT_FEE_RECIPIENT;
    ...
    constructor(
        ...
        IAimagineERC721Drop.SalesConfiguration memory _salesConfig,
        IAimagineERC721Drop.CollectionMeta memory _collectionMeta
    ) ERC721A(_contractName, _contractSymbol) {
        AIMAGINE_MINT_FEE = _mintFeeAmount;
        AIMAGINE_MINT_FEE_RECIPIENT = _mintFeeRecipient;
     ...
     */
    function saleDetails() external view returns (IAimagineERC721Drop.SaleDetails memory) {
        return IAimagineERC721Drop.SaleDetails({
    ...
    function mintedPerAddress(address minter)
        ...
        view
        override
        returns (IAimagineERC721Drop.AddressMintDetails memory)
    {
        return IAimagineERC721Drop.AddressMintDetails({
    ...
    /**
     * @notice Returns the AIMAGINE minting fee for a given quantity of tokens.
     ...
     */
    function aimagineFeeForAmount(uint256 quantity) public view returns (address payable recipient, uint256 fee) {
        recipient = AIMAGINE_MINT_FEE_RECIPIENT;
        fee = AIMAGINE_MINT_FEE * quantity;
    }
     ...
     * @param recipient The address of the recipient who will receive the purchased tokens.
     * @param quantity The quantity of tokens to purchase.
     * @param comment The comment associated with the purchase.
     * @return The ID of the first purchased token.
     */
    function _handlePurchase(address recipient, uint256 quantity, string memory comment) internal returns (uint256) {
        ...
        if (msg.value != (salePrice + AIMAGINE_MINT_FEE) * quantity) {
            revert Purchase_WrongPrice((salePrice + AIMAGINE_MINT_FEE) * quantity);
        }
        ...
        _payoutAimagineFee(quantity);

        emit IAimagineERC721Drop.Sale({
            to: recipient,
            quantity: quantity,
        ...
        if (bytes(comment).length > 0) {
            emit IAimagineERC721Drop.MintComment({
                sender: _msgSender(),
    ...
    function _handlePurchasePresale(
        ...
        if (msg.value != (pricePerToken + AIMAGINE_MINT_FEE) * quantity) {
            revert Purchase_WrongPrice((pricePerToken + AIMAGINE_MINT_FEE) * quantity);
        }
    ...
    /**
     * @notice Internal function to pay out the AIMAGINE minting fee.
     * @param quantity The quantity of tokens for which the minting fee is to be paid.
     */
    function _payoutAimagineFee(uint256 quantity) internal {
        (, uint256 aimagineFee) = aimagineFeeForAmount(quantity);
        (bool success,) = AIMAGINE_MINT_FEE_RECIPIENT.call{value: aimagineFee}("");
        emit MintFeePayout(aimagineFee, AIMAGINE_MINT_FEE_RECIPIENT, success);
    }
        ...
        returns (bool)
    {
        return super.supportsInterface(interfaceId) || type(IERC2981).interfaceId == interfaceId
            || bytes4(0x49064906) == interfaceId || type(IAimagineERC721Drop).interfaceId == interfaceId;
    }
...
/**
 * @title AimagineERC721DropFactory
 * @dev The AimagineERC721DropFactory contract allows the deployment of new AimagineERC721Drop contracts,
 * which are non-fungible tokens (NFTs) conforming to the ERC-721 standard. Each AimagineERC721Drop contract
...
import {AimagineERC721Drop} from "./AimagineERC721Drop.sol";
import {IAimagineERC721Drop} from "./interfaces/IAimagineERC721Drop.sol";

contract AimagineERC721DropFactory is Ownable, Pausable {
    // Array to keep track of deployed AimagineERC721Drop contract addresses.
    address[] public deployedContracts;
    // Address of the most recently deployed AimagineERC721Drop contract.
    address public lastDeployedContractAddress;

    event AimagineERC721DropDeployed(
    ...
    /**
     * @dev Deploys a new AimagineERC721Drop contract with customizable parameters.
     ...
     * @param _mintFeeAmount The amount of Ether required to mint a new NFT in this collection.
     * @param _mintFeeRecipient The address where the minting fees will be sent to.
     * @return aimagineERC721DropContract The address of the newly deployed AimagineERC721Drop contract.
     */
    function deployAimagineERC721Drop(
        string memory _contractName,
        string memory _contractSymbol,
        string memory _contractURI,
        address _initialOwner,
        ...
        IAimagineERC721Drop.SalesConfiguration memory _salesConfig,
        IAimagineERC721Drop.CollectionMeta memory _collectionMeta
    ) external whenNotPaused returns (address aimagineERC721DropContract) {
        // Deploy a new AimagineERC721Drop contract with the specified parameters.
        AimagineERC721Drop deployedContract =
        new AimagineERC721Drop(_contractName, _contractSymbol, _contractURI, _initialOwner, _fundsRecipient, _editionSize, _royaltyBPS, _mintFeeAmount, _mintFeeRecipient, _salesConfig, _collectionMeta);
        ...
        /* Emit an event to notify that a new AimagineERC721Drop contract has been deployed. */
        emit AimagineERC721DropDeployed(
            _contractName,
            _contractSymbol,
            _contractURI,
            _initialOwner,
    ...
    /**
     * @dev Gets the number of deployed AimagineERC721Drop contracts.
     * @return count The number of deployed AimagineERC721Drop contracts.
     */
