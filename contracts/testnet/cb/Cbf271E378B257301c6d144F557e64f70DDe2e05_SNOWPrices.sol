...
import './interfaces/ISNOWPrices.sol';
...
/** 
  ...
  @dev
  Adheres to -
  ISNOWPrices: General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.

  @dev
  Inherits from -
  Ownable: Includes convenience functionality for checking a message sender's permissions before executing certain transactions.
*/
contract SNOWPrices is Ownable, ISNOWPrices {
  //*********************************************************************//
  // --------------------------- custom errors ------------------------- //
  //*********************************************************************//
  ...
  */
  mapping(uint256 => mapping(uint256 => ISNOWPriceFeed)) public override feedFor;
  ...
  */
  function priceFor(
    ...
    ISNOWPriceFeed _feed = feedFor[_currency][_base];

    // If it exists, return the price.
    if (_feed != ISNOWPriceFeed(address(0))) return _feed.currentPrice(_decimals);

    // Get the inverse feed.
    _feed = feedFor[_base][_currency];

    // If it exists, return the inverse price.
    if (_feed != ISNOWPriceFeed(address(0)))
  ...
  */
  function addFeedFor(
    uint256 _currency,
    uint256 _base,
    ISNOWPriceFeed _feed
  ) external override onlyOwner {
    // There can't already be a feed for the specified currency.
    if (feedFor[_currency][_base] != ISNOWPriceFeed(address(0))) revert PRICE_FEED_ALREADY_EXISTS();
