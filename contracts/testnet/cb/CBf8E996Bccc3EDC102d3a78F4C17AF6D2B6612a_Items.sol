...
// USER FUNCTIONS
        ...
        onlyAllowed
    {
        ...
        uint256 itemID;
        for(uint256 i = 0; i < total;){                                                // Loop through the contracts
...
// EVENTS
    ...
    function addItem(ItemType calldata item)
        ...
        onlyOwner
    {
        items[totalItems] = item;    // Set the item details
        unchecked { totalItems++; }  // Increment the total items
    }
    ...
    function updateItem(uint256 itemID, ItemType calldata item)
        ...
        onlyIfItemExists(itemID)
    {
        items[itemID] = item;   // Set the item details
    }
    ...
    function deleteItem(uint256 itemID)
        ...
        onlyIfItemExists(itemID)
    {
        delete items[itemID];   // delete the item mapping
    }
        ...
        onlyOwner
    {
        ...
        for (uint256 itemID = 0; itemID < total;) {        // Loop through all items
        ...
        onlyAllowed
    {
        if (inUse) {
            ...
            getItemsInUse[account][itemID] += amount;        // Store the amount in use by Account
            getItemsInUseByToken[tokenID][itemID] += amount; // Store the amount in use by TokenID
        } else {
            delete getItemsInUse[account][itemID];           // Remove the amount in use by Account
            delete getItemsInUseByToken[tokenID][itemID];    // Remove the amount in use by TokenID
        }
        emit ItemSetInUse(account, tokenID, itemID, amount, inUse); // Write an event
    }
        ...
        onlyAllowed
    {
        ...
        for(uint256 i = 0; i < total;){
            setItemInUse(accounts[i], tokenIDs[i], itemIDs[i], amounts[i], inUse[i]);   // Call function to set item in use
            unchecked { i++; }
        }
    }
...
// GETTERS
    ...
    /// @param tokenID Id of NFT
    function getItemsByTokenID(uint256 tokenID)
        external
        view
        returns (uint256[] memory itemsByToken)
    {
        ...
        if ( total == 0 ) {
            return new uint256[](0);                                       // if no items added, return an empty array
        } else {
            for(uint256 i = 0; i < total;) {                               // Loop through the items to determine the count
                if (getItemsInUseByToken[tokenID][i] > 0) {                // Check if item is in use
                    totalByToken++;                                        // Increment total
                }
                unchecked { i++; } 
            }
            uint256[] memory _itemsByToken = new uint256[](totalByToken);
            uint256 index = 0;
            for(uint256 i = 0; i < total;) {                               // Loop through the items
                if (getItemsInUseByToken[tokenID][i] > 0) {                // Check if item is in use
                    _itemsByToken[index] = i;                                  // Add itemID to array
                    unchecked { index++; }
                }
                unchecked { i++; }
            }
            return _itemsByToken;                                          // Return the array
        }
    }
...
// ADMIN FUNCTIONS
        ...
        returns (string memory output) 
    {
        ...
        string memory json1 = string(abi.encodePacked( // Encode the metadata
            '{',
            '"name": "',            item.name, '",',
        ...
        if (item.itemType == 0) {
            ...
            itemType = "Useful Item";
        } else if (item.itemType == 2) {
            itemType = "Health Item";
        } else if (item.itemType == 3) {
            itemType = "Hazardous Item";
        }
        string memory json2 = string(abi.encodePacked( // Encode the metadata
            '"attributes": [',
                '{ "id": 0, "trait_type": "Rarity", "value": "' ,rarity, '" },',
                '{ "id": 1, "trait_type": "Type", "value": "'   ,itemType, '" }',
                ']',
            '}'
        ));
        output = Base64.encode(abi.encodePacked(json1, json2));   
