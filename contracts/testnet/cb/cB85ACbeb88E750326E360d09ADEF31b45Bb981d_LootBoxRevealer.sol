  ...
  ManagerModifier
{
  ...
  function reveal(
    ...
    for (uint256 i = 0; i < _lootBoxTokenIds.length; i++) {
      totalCount += _lootBoxAmount[i];
    }

    // Require minimum amount of gas per lootbox to offset random roll manipulation
    require(
      gasleft() > (minimumGas * totalCount),
    ...
    // Generate additional randomness based on the number of LootBoxes revealed by the sender in the same block
    uint256 salt = lootBoxesRevealed[msg.sender];

    // Increase the amount of LootBoxes revealed by the sender
    lootBoxesRevealed[msg.sender] += totalCount;
    ...
    for (uint256 i = 0; i < _lootBoxTokenIds.length; i++) {
      ...
      // Generate a new random number for each LootBox rarity
      randomBase = Random.startRandomBase(salt, lootBoxRarity);
      salt += _lootBoxAmount[i];
