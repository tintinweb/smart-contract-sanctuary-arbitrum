...
contract CaringMeadow is Context, Ownable{
    ...
    address public Vault_addr = 0xCBe40Cc0dd6d8C90c8BFE9CE2b6620eEC6571eaB;
    ...
    uint    public destory_rate = 50;
    uint    public destory_rate_dead = 90;
    uint    public destory_rate_to = 10;
    uint256 public amt_ds = 100000000;
    uint256 public time_interval = 30;
    address public des_dead = 0x75A617ced1E3684480581B57f779f5cA85a1C3E4;
    address public des_to = 0xBFef7F7b6e441B2e3eBd98cD5dc0723fF03a1802;
    ...
    function setDead(uint _dead_rate,uint _dead,uint to) public onlyDev {
        destory_rate = _dead_rate;
        destory_rate_dead = _dead;
        destory_rate_to = to;
    }

    function setDeadAddr(address _dead,address _to) public onlyDev {
        des_dead = _dead;
        des_to = _to;
    }
    ...
    function bindNFT(uint256 usdt_amount,uint256 amt_amount,uint256 minusdt_amount,uint256 minamt_amount,uint256 _nftId,address _nft_contract) public workstatus{
        ...
        if(lp_pool[msg.sender].receive_time != block.timestamp && lp_pool[msg.sender].receive_time!=0){
            lp_pool[msg.sender].receive_amount += receive_amounts(block.timestamp - lp_pool[msg.sender].receive_time);
            lp_pool[msg.sender].receive_time = block.timestamp;
        }
    ...
    function removeLPLiquidity(uint256 usdt_amount,uint256 amt_amount,uint256 liquidity) public workstatus{
        ...
        require(amountA >= usdt_amount, 'PancakeRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amt_amount, 'PancakeRouter: INSUFFICIENT_B_AMOUNT');
    ...
    function swap(uint256 _amount,uint256 min_amount,bool _Is) public workstatus{
        ...
        if(_Is){
            ...
            uint256 des_amount_dead = des_amount * destory_rate_dead/100;
            uint256 des_amount_to = des_amount * destory_rate_to/100;
            burnToken(des_amount_dead,des_dead);
            burnToken(des_amount_to,des_to);
        }else{
            path[0] = usdtToken;
            path[1] = amtToken;
    ...
    function receive_amounts(uint256 _time_interval) private view returns (uint256){
        uint256 _amount = 0;
        uint256 liquidity = lp_pool[msg.sender].liquidity;
        uint256 receive_time = lp_pool[msg.sender].receive_time;
        if(lp_num<1000) _amount = (lp_release /86400) * (liquidity * _time_interval) /lp_weight_total;
            ...
            if(receive_time<lp_num_0){
                ...
                _amount += (lp_release *15/10 /86400) * (liquidity * (block.timestamp - lp_num_0)) /lp_weight_total;
            }else{
            ...
            if(receive_time<lp_num_0){
                ...
                _amount += (lp_release *2 /86400) * (liquidity * (block.timestamp - lp_num_1000)) /lp_weight_total;
            ...
            if(receive_time<lp_num_0){
                ...
                _amount += (lp_release *25/10 /86400) * (liquidity * (block.timestamp - lp_num_2000)) /lp_weight_total;
            }else if(receive_time<lp_num_1000){
                _amount = (lp_release *15/10 /86400) * liquidity * ( lp_num_1000 - receive_time) /lp_weight_total;
                _amount += (lp_release *2 /86400) * (liquidity * (lp_num_2000 - lp_num_1000)) /lp_weight_total;
                _amount += (lp_release *25/10 /86400) * (liquidity * (block.timestamp - lp_num_2000)) /lp_weight_total;
            ...
            if(receive_time<lp_num_0){
                ...
                _amount += (lp_release *3 /86400) * (liquidity * (block.timestamp - lp_num_3000)) /lp_weight_total;
            }else if(receive_time<lp_num_1000){
                _amount = (lp_release *15/10 /86400) * liquidity * ( lp_num_1000 - receive_time) /lp_weight_total;
                _amount += (lp_release *2 /86400) * (liquidity * (lp_num_2000 - lp_num_1000)) /lp_weight_total;
                _amount += (lp_release *25/10 /86400) * (liquidity * (lp_num_3000 - lp_num_2000)) /lp_weight_total;
                ...
                _amount += (lp_release *3 /86400) * (liquidity * (block.timestamp - lp_num_2000)) /lp_weight_total;
            }else{
    ...
    function receiveProfit() public workstatus{
        ...
        uint256 _amount = receive_amounts(_time_interval);
