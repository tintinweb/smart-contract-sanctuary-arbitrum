...
contract DistributionContract {
    ...
    address public teamAddress; // Adresse de l'équipe
    ...
    function distributeETH() private {
        uint256 teamShare = address(this).balance / 2; // Calculer 50% des ETH
        payable(teamAddress).transfer(teamShare); // Envoyer à l'adresse de l'équipe
    ...
    function distributeArbotTokens() private {
        ...
        for (uint i = 0; i < holdersCount; i++) {
            ...
            if (!excludedAddresses[holder]) {
                ...
                uint256 proportion = (holderBalance * (10**18)) / totalSupply;
                uint256 amount = (value * proportion) / (10**18);
                arbotReceived[holder] += amount;
                arbotToken.transfer(holder, amount);
            }
        }
    }
}