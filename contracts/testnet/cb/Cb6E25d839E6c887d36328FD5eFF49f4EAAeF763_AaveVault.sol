...
contract AaveVault is Ownable, ReentrancyGuard {
  ...
  constructor(address _dusd, address _dai, address _aArbDai, address _tokenTransferProxy, address _augustusAddr, address _aavePool, address _fluidTreasury) {
    DUSD = IERC20(_dusd);
    DAI = IERC20(_dai);
  ...
  function withdraw() public nonReentrant returns(uint256) {
    ...
    // balancerSwap(amountWithdraw, address(DAI), address(DUSD), balancerPoolId);

    if(amountWithdraw > originalDaiBalance) {
      uint256 reward = amountWithdraw - originalDaiBalance;
      reward = reward * fee / 10000;
  ...
  function balancerSwap(uint256 amount, address assetIn, address assetOut, bytes32 poolId) internal returns (uint256) {
