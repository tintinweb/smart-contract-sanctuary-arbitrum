...
contract Router is BaseRouter, IRouter, ReentrancyGuard {
    ...
    function addPosition(
        ...
        //Scope to avoid stack too deep error
        {
            ...
            (, prices) = _getPricesAndCheckFastExecute(_path);
            _modifyPosition(
                msg.sender,
                _isLong,
                _posId,
    ...
    function updateTrailingStop(
        ...
        require(msg.sender == executor || msg.sender == bonds[_getPositionKey(_account, _indexToken, _isLong, _posId)].owner, "IVLPO"); //Invalid positionOwner
        bool isFastExecute;
        ...
        {
            uint256 price;
            (isFastExecute, price) = _getPriceAndCheckFastExecute(_indexToken);
            prices[0] = price;
        }
    ...
    */
    function triggerPosition(
        ...
        _modifyPosition(
            ...
            bonds[_key].isLong, 
            bonds[_key].posId,
            _getTxTypeForExecuting(_txType),
            false,
    ...
    function _createPrepareTransaction(
        ...
        if (!isFastExecute || _txType == ADD_POSITION) {
            ...
            if (_txType == TRIGGER_POSITION && positionKeeper.getPositionSize(key) == 0) {
                ...
                require(transaction.status == TRANSACTION_STATUS_NONE 
                    || transaction.status == TRANSACTION_STATUS_EXECUTE_REVERTED, "IVLPTS/PRCS"); //Invalid prepare transaction status, processing
            }
            
            transaction.txType = _txType;
            transaction.startTime = block.timestamp;
            transaction.status = isFastExecute ? TRANSACTION_STATUS_NONE : TRANSACTION_STATUS_PENDING;
        }

        if (!_isNotRequirePreparePath(_txType) && (!isFastExecute || _isDelayPosition(_txType))) {
