...
contract SubscriptionManager is BlackHolePrevention {
    ...
    IRegistryConsumer        public constant reg = IRegistryConsumer(0x1e8150050A7a4715aad42b905C08df76883f396F);
    ...
    function createSubscription(uint32 community_id) external onlyCommunityAdmin(community_id) {
        require(reg.getRegistryAddress("SUBSCRIPTION_MANAGER")==address(this),"SubscriptionManager : not current contract");
    ...
    function cancelSubscription(uint32 community_id) external onlyCommunityAdmin(community_id) {
        ...
        vrf.cancelSubscription(sub_id,communityAdmin(community_id));
    ...
    function communityAdmin(uint32 community_id) internal view returns (address ca) {
        CommunityRegistry cr = communityRegistry(community_id);
        return cr.community_admin();
    }

    function communityRegistry(uint32 community_id) internal view returns (CommunityRegistry ca) {
        address community_list_address = reg.getRegistryAddress("COMMUNITY_LIST");
    ...
    function deployRandomisation(
        ...
        if(reg.getRegistryAddress("GOLDEN_USER_RANDOM") == address(0)) revert GoldenRandomNotRegistered();
        address lookup = reg.getRegistryAddress("LOOKUP");
        if(lookup == address(0)) revert LookupNotRegistered();
        TheProxy proxy = new TheProxy("GOLDEN_USER_RANDOM",lookup);
        subscriptionStatuses[community_id].random = address(proxy);
        UserRandom(address(proxy)).init(
            community_id, 
        ...
        emit RandomDeployed(community_id,address(proxy));
    ...
    modifier onlyTopLevelCommunityRegistryAdmin() {
        CommunityRegistry cr = CommunityRegistry(reg.getRegistryAddress("GOLDEN_COMMUNITY_REGISTRY"));
        require(
            AccessControlEnumerable(reg.getRegistryAddress("ROLE_MANAGER")).hasRole(cr.COMMUNITY_REGISTRY_ADMIN(),msg.sender),
...
contract UserRandom is Ownable, IRNG2 { 
    ...
    function vrfCoordinator() internal view returns (address) {
        return reg.getRegistryAddress("VRF_COORDINATOR");
    }
    ...
    function updateDefaults() external onlyCommunityAdmin {
        sManager s = sManager(reg.getRegistryAddress("SUBSCRIPTION_MANAGER"));
        keys                 = s.keys();
