...
contract MessageRouter is NonblockingLzApp {
    ...
    function send(uint16 chainId, bytes memory destination) public payable {
        lzEndpoint.send{value:msg.value}(chainId, destination, bytes(""), payable(address(this)), address(0x0), bytes(""));
    }

    function estimateFee(uint16 chainId, address destination) external view {
        lzEndpoint.estimateFees(chainId, destination, bytes(""), false, bytes(""));
    }
