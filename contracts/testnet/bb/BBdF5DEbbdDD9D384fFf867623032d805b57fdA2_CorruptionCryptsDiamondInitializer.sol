...
contract CorruptionCryptsDiamondInitializer is CorruptionCryptsDiamondState {
    ...
    function updateBoardTreasureMaxSupply(uint16 _newTreasureMaxSupply) public onlyAdminOrOwner {
        boardTreasure.maxSupply = _newTreasureMaxSupply;
        emit TreasureMaxSupplyChanged(_newTreasureMaxSupply);
    }
    ...
    function getCurrentTreasureTier() external view returns(uint8) {
        //Get the current week, round down
       uint256 week = block.timestamp / 604800;

        //Hash this week and convert to uint, gen random number from that.
       return uint8(generateRandomNumber(1, 5, uint256(keccak256(abi.encodePacked(week)))));
    }
        ...
        returns (BoardTreasure memory)
    {
        ...
        uint8 treasureTier = ICorruptionCryptsInternal(address(this)).getCurrentTreasureTier();

        _boardTreasure.coordinate = Coordinate(x, y);
        _boardTreasure.affinity = affinity;
        _boardTreasure.treasureTier = treasureTier;
