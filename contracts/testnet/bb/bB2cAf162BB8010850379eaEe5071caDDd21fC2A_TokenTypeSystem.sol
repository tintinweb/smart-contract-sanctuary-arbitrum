...
struct BaseImageURIs {
    string imageURI;
    string animatedImageURI;
}

struct TokenTypeMetadata {
    BaseImageURIs imageURIs;
}
...
contract TokenTypeSystem is GameRegistryConsumerUpgradeable {
    ...
    function getTokenType(
        ...
        return _getTokenType().getValue(tokenEntity);
    }

    function getTokenTypeMetadata(
        uint256 tokenEntity
    ) external view returns (TokenTypeMetadata memory) {
        return
            TokenTypeMetadata({
                imageURIs: _getBaseImageURIs(
                    EntityLibrary.entityToAddress(tokenEntity),
                    _getTokenType().getValue(tokenEntity)
                )
            });
    }
    ...
    function _getBaseImageURIs(
        address tokenContract,
        TokenType tokenType
    ) internal view returns (BaseImageURIs memory baseImageURIs) {
        address pirateContract = _getSystem(PIRATE_NFT_ID);
        IGameGlobals gameGlobals = IGameGlobals(_getSystem(GAME_GLOBALS_ID));

        // NOTE: For now, only PirateNFT is supported.
        if (tokenContract != pirateContract) {
            return baseImageURIs;
        }

        string[] memory images;
        if (tokenContract == pirateContract) {
            if (tokenType == TokenType.ENHANCED) {
                images = gameGlobals.getStringArray(
                    PIRATE_NFT_IMAGES_ENHANCED_ID
                );
            } else {
                images = gameGlobals.getStringArray(
                    PIRATE_NFT_IMAGES_DEFAULT_ID
                );
            }
        }

        baseImageURIs.imageURI = images[0];
        baseImageURIs.animatedImageURI = images[1];
    }
    ...
    function _updateTokenURIs(
        ...
        traitsProvider.setTraitString(
            ...
            tokenId,
            IMAGE_TRAIT_ID,
            string.concat(baseImageURIs.imageURI, "/", tokenId.toString())
        );
        ...
        traitsProvider.setTraitString(
            ...
            string.concat(
                baseImageURIs.animatedImageURI,
                "/",
                tokenId.toString()
            )
        );
