...
contract SelfHostedPriceFeed {
    ...
    function getPrice(address _token) external view returns (uint256) {
        return priceUpdates[_token].price;
    }

    function setPrice(address _token, uint256 price) public onlyKeeper {
        uint256 timestamp = block.timestamp;
        require(isWhitelistedToken[_token], "FastPriceFeed::Unknown token");
        priceUpdates[_token] = PriceUpdate({
            price: uint128(price),
            lastUpdated: uint128(timestamp)
        });

        emit PriceSet(_token, price, timestamp);
    }
