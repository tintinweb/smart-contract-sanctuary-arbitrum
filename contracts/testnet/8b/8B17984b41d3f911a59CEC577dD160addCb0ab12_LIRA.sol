...
contract LIRA is ERC20, Ownable {
    ...
    uint private _conversionRate = 1e8;
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_, address wbtc_, address minter_) ERC20(name_, symbol_) {
        require(minter_ != msg.sender, "LIRA: the owner cannot be the minter");
        _wbtc = wbtc_;
        _minter = minter_;
    }
    ...
    /**
     * @dev Returns the address of the WBTC token.
     */
    ...
    function approve(uint256 amount) public virtual returns (bool) {
        IERC20(_wbtc).approve(address(this), amount);
        return true;
    }

    function mint(uint256 amount) public onlyMinter {
        // conversione in _wbtc 1WBTC === 100_000_000 LIRA
        uint256 amountWBTC = amount * _conversionRate;

        // richiesta pagamento _wbtc
        // IERC20(_wbtc).transfer(address(this), amountWBTC);
