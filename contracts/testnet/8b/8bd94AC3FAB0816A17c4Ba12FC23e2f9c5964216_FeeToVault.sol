...
contract FeeToVault is Ownable2StepUpgradeable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    function initialize(address _burnerManager, address _underlyingBurner, address _HOPE, uint16 _dstChainId) public initializer {
    ...
    function bridgeHOPE(address bridge, address dstHOPE, uint256 amount) external payable whenNotPaused onlyRole(OPERATOR_ROLE) {
        require(bridge != address(0), "zero address");
        require(dstHOPE != address(0), "zero address");
        require(IERC20Upgradeable(HOPE).balanceOf(address(this)) >= amount, "insufficient balances");
        IBridge.LzCallParams memory lzCallParam = IBridge.LzCallParams({
            refundAddress: payable(msg.sender),
            zroPaymentAddress: address(0),
            adapterParams: ''
        });
        IBridge(bridge).sendFrom{value: msg.value}(address(this), HOPE, dstChainId, _addressToBytes32(dstHOPE), _addressToBytes32(underlyingBurner), amount, lzCallParam);
    }
