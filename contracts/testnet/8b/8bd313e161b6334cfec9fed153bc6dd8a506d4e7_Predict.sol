...
contract Predict is ReentrancyGuard {
    ...
    function getUpPayout(uint256 _roundNumber) external view returns(uint256) {
        uint256 payout = rounds[_roundNumber].totalAmount / rounds[_roundNumber].bullAmount;
        if (payout != 0) {
            return payout;
        }
    }
    ...
    function getDownPayout(uint256 _roundNumber) external view returns(uint256) {
        uint256 payout = rounds[_roundNumber].totalAmount / rounds[_roundNumber].bearAmount;
        if (payout != 0) {
            return payout;
        }
    }
