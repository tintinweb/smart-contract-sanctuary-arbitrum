...
contract MarketPlaceConfig is Ownable {
    ...
    uint256[] public rateForMembershipLevel;
    uint256[] public rateForInputParameters;
    uint256 public conversionRate = 500; // default 5%
    uint256 public conversionRateMerchant = 500; // default 5%
    uint256 public hundredPercent = 10000;

    // config for membership
    uint256[] public pointNeedToUpgrade;
    uint256[] public purchaseNeedToUpgrade;
    uint256[] public loyaltyPointNeedForUpgrade;

    // dealing with percentages that lower than 1, minimum is  0.01%
    uint8 public precision = 2;
    // Loyaltypoint decimal
    uint8 public decimal = 18;

    // Min length for level
    uint8 public minLength = 5;
    ...
    event eventRateForMembershipLevelByIndex(uint256 _rate, uint256 _index);
    function configRateForMembershipLevelByIndex(uint256 _rate, uint256 _index) external onlyOwner{
        require(rateForMembershipLevel.length > _index, "Index must be lesser than length");
        rateForMembershipLevel[_index] = _rate;
        emit eventRateForMembershipLevelByIndex(_rate, _index);
    }
    ...
    event eventRateForInputParametersByIndex(uint256 _rate, uint256 _index);
    function configRateForInputParametersByIndex(uint256 _rate, uint256 _index) external onlyOwner{
        require(rateForInputParameters.length > _index, "Index must be lesser than length");
        rateForInputParameters[_index] = _rate;
        emit eventRateForInputParametersByIndex(_rate, _index);
    }
    ...
    event eventPointNeedToUpgradeByIndex(uint256 _rate, uint256 _index);
    function configPointNeedToUpgradeByIndex(uint256 _rate, uint256 _index) external onlyOwner{
        require(pointNeedToUpgrade.length > _index, "Index must be lesser than length");
        pointNeedToUpgrade[_index] = _rate;
        emit eventPointNeedToUpgradeByIndex(_rate, _index);
    }
    ...
    event eventPurchaseNeedToUpgradeByIndex(uint256 _rate, uint256 _index);
    function configPurchaseNeedToUpgradeByIndex(uint256 _rate, uint256 _index) external onlyOwner{
        require(purchaseNeedToUpgrade.length > _index, "Index must be lesser than length");
        purchaseNeedToUpgrade[_index] = _rate;
        emit eventPurchaseNeedToUpgradeByIndex(_rate, _index);
    }
    ...
    event eventLoyaltyPointNeedForUpgradeByIndex(uint256 _rate, uint256 _index);
    function configLoyaltyPointNeedForUpgradeByIndex(uint256 _rate, uint256 _index) external onlyOwner{
        require(loyaltyPointNeedForUpgrade.length > _index, "Index must be lesser than length");
        loyaltyPointNeedForUpgrade[_index] = _rate;
        emit eventLoyaltyPointNeedForUpgradeByIndex(_rate, _index);
    }
