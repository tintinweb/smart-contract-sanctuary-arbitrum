...
// calculate sdt award
contract SdtRewardFacet is ISdtReward {

    function initializeSdtRewardFacet(address _rewardsToken, uint256 _sdtPerBlock, uint256 _startBlock) external {
        ...
        LibSdtReward.initialize(_rewardsToken, _sdtPerBlock, _startBlock);
    }

    function updateSdtPerBlock(uint256 _sdtPerBlock) external override {
        LibAccessControlEnumerable.checkRole(Constants.STAKE_OPERATOR_ROLE);
        LibSdtReward.updateSdtPerBlock(_sdtPerBlock);
    }

    function addReserves(uint256 amount) external override {
        require(amount > 0, "SdtRewardFacet: amount must be greater than 0");
        LibSdtReward.addReserves(amount);
    }

    function sdtPoolInfo() external view returns (SdtPoolInfo memory) {
        return LibSdtReward.sdtPoolInfo();
    }

    function pendingSdt(address _account) external view override returns (uint256) {
        return LibSdtReward.pendingSdt(_account);
    }
}
