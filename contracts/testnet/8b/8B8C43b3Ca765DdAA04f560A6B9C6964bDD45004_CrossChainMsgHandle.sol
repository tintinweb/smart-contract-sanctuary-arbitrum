...
contract CrossChainMsgHandle is Ownable, ICrossChainMsgSend {
    ...
    mapping(uint256 => mapping(string => mapping(address => bool))) public signed;
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encode(
                        keccak256(
                            "ReceiveMsg(string fromHash,uint16 fromChainId,uint256 nonce,bytes destination,bytes payload)"
                        ),
                        keccak256(bytes(_fromHash)),
                        _chainId,
                        nonces[_chainId][_fromHash]++,
                        keccak256(_destination),
                        keccak256(_payload)
                    )
                )
            )
