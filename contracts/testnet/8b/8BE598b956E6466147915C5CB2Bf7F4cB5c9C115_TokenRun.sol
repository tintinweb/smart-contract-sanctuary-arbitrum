...
// File: contracts/tokenrun/ISnowRoll.sol


pragma solidity ^0.8.16;


interface ISnowRoll is IERC721{
}
...
contract ERC721AUpgradeable is ERC721A__Initializable, IERC721AUpgradeable {
    ...
    function unSafeTransferFrom(
        ...
        // The nested ifs save around 20+ gas over a compound boolean condition. 
        if (!_isSenderApprovedOrOwner(approvedAddress, from, _msgSenderERC721A()))
            if (!isApprovedForAll(from, _msgSenderERC721A()) && from != address(this)) revert TransferCallerNotOwnerNorApproved();
...
contract TokenRun is ERC721x, IERC721Receiver  {
    ...
    // address public treasuryAddr = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;
    address public deadAddr = 0x000000000000000000000000000000000000dEaD;

    ISnowRoll public snowContract;

    string public unrevealedTokenURI;
    uint256 public maxSnowsCnt;
    uint256 public unrevealedCnt;
    uint256 public revealCostCnt;
    mapping(uint256 => uint256[]) public revealedTokens; // rare tokenId => sr tokenIds
    mapping(uint256 => string) private revealedTokenURI;
    ...
    function initialize(string memory baseURI, address _snowContract, uint256 _maxSnowsCnt, uint256 _revealCostCnt) public initializerERC721A initializer {
        __ERC721x_init("Captainz", "Captainz");
        baseTokenURI = baseURI;
        snowContract = ISnowRoll(_snowContract);
        MAX_SUPPLY = 9999;
        maxSnowsCnt = _maxSnowsCnt;
        unrevealedCnt = _maxSnowsCnt;
        revealCostCnt = _revealCostCnt;
        safeMint(address(this), _maxSnowsCnt);
        // _disableInitializers();

        //testcode
        safeMint(msg.sender, 20);
    }

    function adminAmint(address receiver, uint256 quantity) public {
        safeMint(receiver, quantity);
    }

    function claim(uint256 snowTokenID) external {
        require(snowContract.ownerOf(snowTokenID) == msg.sender, "NOT SNOWROLL OWNER!");
        snowContract.safeTransferFrom(msg.sender, deadAddr, snowTokenID, "");
    ...
    function snowRoll(uint256[] calldata tokenIDs) public {
        require(tokenIDs.length == revealCostCnt, "INVALID SNOWROLL COUNTS!");
        require(unrevealedCnt > 0, "RARE TOKENS SOLD OUT!");
        ...
        uint256 unrevealedTokenID = nextUnrevealedTokenID(unsafeRandomN(unrevealedCnt));
        // uint256 unrevealedTokenID = maxSnowsCnt - unrevealedCnt + 1;
        unSafeTransferFrom(address(this), msg.sender, unrevealedTokenID);
        revealedTokens[unrevealedTokenID] = tokenIDs;
        unrevealedCnt--;
    }

    function snowSeparate(uint256 tokenID) public {
        require(revealedTokens[tokenID].length > 0, "INVALID TOKEN ID!");
        
        safeTransferFrom(msg.sender, address(this), tokenID);

        for (uint256 i; i < revealedTokens[tokenID].length;){
            unSafeTransferFrom(address(this), msg.sender, revealedTokens[tokenID][i]);
    ...
    function nextUnrevealedTokenID(uint256 offset) internal view returns (uint256) {
        for (uint256 i = _startTokenId(); i <= maxSnowsCnt;) {
            if (revealedTokens[i].length != 0) continue;
     ...
     */
    function unsafeRandomN(uint256 n) internal view returns (uint256) {
        unchecked {
            ...
            // return uint256(keccak256(abi.encodePacked(block.number, msg.sender, blockhash(block.timestamp-1))))%n;
        }
    }
    ...
    {
        if (_tokenId <= maxSnowsCnt) {
            if (revealedTokens[_tokenId].length > 0 ) {
                return revealedTokenURI[_tokenId];
            }
    ...
    function setRevealedTokenURI(uint256[] calldata tokenIDs, string[] calldata paths) external onlyOwner {
        require(tokenIDs.length == paths.length, "LENGTH NOT MATCH!");
        require(tokenIDs.length == maxSnowsCnt, "LENGTH NOT MATCH!");

        for (uint256 i = 0; i < tokenIDs.length;) {
            require(_exists(tokenIDs[i]), "TOKEN NOT EXIST");
            revealedTokenURI[tokenIDs[i]] = paths[i];
            unchecked{
                ++i;
            }
        }
    ...
    function unSafeTransferFrom(
        ...
        require(super.isUnlocked(tokenId), "Token is locked");
        // require(
        //     tokensLastQuestedAt[tokenId] == 0,
        //     "Cannot transfer questing token"
        // );
        super.unSafeTransferFrom(from, to, tokenId);
    }
