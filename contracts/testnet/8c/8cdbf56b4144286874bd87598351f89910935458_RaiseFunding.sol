...
// 创建不同的募资活动，用来募集加密货币
// 记录不同募资活动的详细信息（参与人数，地址列表、每个用户的集资数量、募集的加密货币总数）
// 业务逻辑（用户参与投资、创建新的募资活动、活动后的资金提取、募资失败情况下的退款活动）
...
contract CoFunding{
    ...
    struct CofundingCampaign{
        ...
        mapping(address => bool) isParticipated; // 某个地址是否参与本次募资活动的映射表 
    }
...
contract  RaiseFunding is CoFunding{
    ...
    function deposit(uint _campaingnIndex) external payable notNULL( _campaingnIndex) onlyonce(_campaingnIndex) returns(uint _index){
        ...
        cfc.funderList.push(Funder({  // 添加募资者信息
            addr: msg.sender,
            amount: msg.value
        }));
