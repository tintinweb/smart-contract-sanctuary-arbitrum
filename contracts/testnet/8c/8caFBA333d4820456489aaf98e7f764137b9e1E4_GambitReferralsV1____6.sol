...
abstract contract GambitReferralsV1 is IStableCoinDecimals, Initializable {
    ...
    // Params (adjustable)
    bool public enableUsdcReward; // if false, reward paid in CNG
    uint[10] public tierOpenFeeP; // % (of opening fee for each tier, eg. 10)

    bytes32[53] private _gap2; // storage slot gap (11 slots for above variables)

    // CUSTOM TYPES
    struct AllyDetails {
        address[] referrersReferred;
        uint volumeReferredUsdc; // 1e6 (USDC) or 1e18 (DAI)
        uint pendingRewardsToken; // 1e18
        uint totalRewardsToken; // 1e18
        uint totalRewardsValueUsdc; // 1e6 (USDC) or 1e18 (DAI)
        bool active;
    }
    struct ReferrerDetails {
        address ally; // ally assigned to referrer
        address[] tradersReferred; // referred traders
        uint volumeReferredUsdc; // 1e6 (USDC) or 1e18 (DAI), total trading volume incurred by tradersReferred
        uint pendingRewardsToken; // 1e18, pending referral rewards in CNG
        uint pendingRewardsUsdc; // 1e6 (USDC) or 1e18 (DAI), pendng referral rewards in USDC
        uint totalRewardsToken; // 1e18, total referral rewards in CNG
        uint totalRewardsValueUsdc; // 1e6 (USDC) or 1e18 (DAI), total referral rewards in USDC
        uint tier;
        bool active;
    }
    ...
    event UpdatedTargetVolumeUsdc(uint value);

    event UpdatedTierOpenFeeP(uint[10] arr);
    event UpdatedEnableUsdcReward(bool value);
    event UpdatedReferrerTier(address indexed referrer, uint tier);
    ...
    function initialize(
        ...
        uint[10] calldata _tierOpenFeeP,
        bool _enableUsdcReward
        ...
        validateTierOpenFeeP(_tierOpenFeeP);
        storageT = _storageT;
        tierOpenFeeP = _tierOpenFeeP;
        enableUsdcReward = _enableUsdcReward;
    }
    ...
    // later tier must be higher than previous tier or zero.
    function validateTierOpenFeeP(uint[10] memory arr) internal pure {
        uint i;
        // check increment order
        for (i = 1; i < 10; i++) {
            if (arr[i] == 0) break;
            if (arr[i] < arr[i - 1]) revert GambitErrorsV1.WrongOrder();
            if (arr[i] > 100) revert GambitErrorsV1.TooHigh();
        }

        // check tailing zeros
        for (; i < 10; i++) {
            if (arr[i] > 0) revert GambitErrorsV1.InvalidTailingZero();
        }
    }

    function updateTierOpenFeeP(uint[10] calldata arr) external onlyGov {
        validateTierOpenFeeP(arr);

        tierOpenFeeP = arr;
        emit UpdatedTierOpenFeeP(arr);
    }

    function updateEnableUsdcReward(bool value) external onlyGov {
        if (value == enableUsdcReward) revert GambitErrorsV1.SameValue();
        enableUsdcReward = value;

        emit UpdatedEnableUsdcReward(value);
    }

    function updateReferrerTier(address referrer, uint tier) external onlyGov {
        if (referrer == address(0)) revert GambitErrorsV1.ZeroAdress();
        ReferrerDetails storage r = referrerDetails[referrer];

        if (tier >= 10) revert GambitErrorsV1.TooHigh();
        r.tier = tier;

        emit UpdatedReferrerTier(referrer, tier);
    }

    // MANAGE ALLIES
    function whitelistAlly(address ally) external onlyGov {
        if (ally == address(0)) revert GambitErrorsV1.ZeroAdress();
    ...
    function whitelistReferrer(
        ...
        if (ally != address(0)) {
            ...
            if (r.ally == address(0)) {
                ...
                require(r.ally == ally, "ALLY_MISMATCH");
            }
        }
    ...
    function distributePotentialReward(
        address trader,
        uint volumeUsdc, // 1e6 (USDC) or 1e18 (DAI)
        uint pairOpenFeeP, // PRECISION
        uint tokenPriceUsdc // 1e10
    )
        external
        onlyCallbacks
        returns (
            uint referrerRewardValueUsdc, // 1e6 (USDC) or 1e18 (DAI)
            bool enabledUsdcReward
        )
    {
        address referrer = referrerByTrader[trader];
        ReferrerDetails storage r = referrerDetails[referrer];
        enabledUsdcReward = enableUsdcReward;

        if (!enabledUsdcReward && address(storageT.token()) == address(0))
            revert GambitErrorsV1.RewardUnavailable();

        if (!r.active) {
            return (0, enabledUsdcReward);
        }

        referrerRewardValueUsdc =
            (volumeUsdc * getReferrerFeeP(pairOpenFeeP, tierOpenFeeP[r.tier])) /
            PRECISION /
            100;

        // 1e18
        // USDC: 1e6  * 1e12 * 1e10 / 1e10 = 1e18
        // DAI:  1e18 * 1e0  * 1e10 / 1e10 = 1e18
        uint referrerRewardToken = (referrerRewardValueUsdc *
            (10 ** (18 - usdcDecimals())) *
            PRECISION) / tokenPriceUsdc;

        // NOTE: ally rewards are disabled
        // AllyDetails storage a = allyDetails[r.ally];

        // uint allyRewardValueUsdc;
        // uint allyRewardToken;

        // if (a.active) {
        //     allyRewardValueUsdc = (referrerRewardValueUsdc * allyFeeP) / 100;
        //     allyRewardToken = (referrerRewardToken * allyFeeP) / 100;

        //     a.volumeReferredUsdc += volumeUsdc;
        //     a.pendingRewardsToken += allyRewardToken;
        //     a.totalRewardsToken += allyRewardToken;
        //     a.totalRewardsValueUsdc += allyRewardValueUsdc;

        //     referrerRewardValueUsdc -= allyRewardValueUsdc;
        ...
        r.volumeReferredUsdc += volumeUsdc;
        if (enabledUsdcReward) {
            r.pendingRewardsUsdc += referrerRewardValueUsdc;
        } else {
            r.pendingRewardsToken += referrerRewardToken;
        }
        r.totalRewardsToken += referrerRewardToken;
        r.totalRewardsValueUsdc += referrerRewardValueUsdc;
        ...
        return (referrerRewardValueUsdc, enabledUsdcReward); // + allyRewardValueUsdc;
    }
    ...
    function claimAllyRewards() external {
        ...
        SafeERC20Upgradeable.safeTransfer(
            IERC20Upgradeable(address(storageT.token())),
            msg.sender,
            rewardsToken
        );
    ...
    function claimReferrerRewards() external {
        ...
        uint rewardsUsdc = r.pendingRewardsUsdc;

        require(rewardsToken > 0 || rewardsUsdc > 0, "NO_PENDING_REWARDS");
        ...
        if (rewardsToken > 0) {
            ...
            emit ReferrerRewardsClaimed(msg.sender, rewardsToken, false);
        }

        if (rewardsUsdc > 0) {
            r.pendingRewardsUsdc = 0;
            SafeERC20Upgradeable.safeTransfer(
                IERC20Upgradeable(address(storageT.usdc())),
                msg.sender,
                rewardsUsdc
            );
            emit ReferrerRewardsClaimed(msg.sender, rewardsUsdc, true);
        }
    }
    ...
    function getReferrerFeeP(
        ...
        uint referrerFeeP = ((pairOpenFeeP * 2) * tierFeeP) / 100; // 0.25% out of (PairOpenFeeP*2)
        return referrerFeeP;
    }
    ...
    function getPercentOfOpenFeeP(address trader) external view returns (uint) {
        return
            getPercentOfOpenFeeP_calc(
                tierOpenFeeP[referrerDetails[referrerByTrader[trader]].tier]
            );
    }

    function getPercentOfOpenFeeP_calc(
        uint tierFeeP
    ) public pure returns (uint resultP) {
        resultP = tierFeeP * PRECISION;
    }
...
/**
 * @dev GambitReferralsV1 with stablecoin decimals set to 18.
 */
contract GambitReferralsV1____6 is GambitReferralsV1 {
 ...
 * @dev GambitReferralsV1 with stablecoin decimals set to 18.
 */
contract GambitReferralsV1____18 is GambitReferralsV1 {
    function usdcDecimals() public pure override returns (uint8) {
        return 18;
    }
}
