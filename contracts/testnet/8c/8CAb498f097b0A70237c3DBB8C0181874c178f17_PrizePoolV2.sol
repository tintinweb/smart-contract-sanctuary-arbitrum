...
interface IPrizeBoxToken is IERC20 {

    function miningMint(address to, uint256 amount) external;

    function marketingMint(address to, uint256 amount) external;
...
interface IReferralStorage {
    ...
    function withdrawReward(address referrer, uint256 withdrawnAmount) external;

    function getEarnRewardMap(address referrer, address referee) external view returns (uint256);

    function getReferrerTotalEarnReward(address referrer) external view returns (uint256 totalEarnReward, uint256 withdrawnReward);


    function setEarnRewardMap(address referrer, address referee, uint256 earnReward) external;

    function setReferral(address referrer, address referee) external returns (bool);
...
import "@openzeppelin/contracts/utils/Counters.sol";
import "./interfaces/IPrizeNFT.sol";
import "./interfaces/IPrizeBoxToken.sol";
import "./interfaces/IReferralStorage.sol";

contract PrizePoolStorage {
    ...
    uint256 public constant FEE_TO_REFERRER = 2000; //20%

    uint256 public constant MARKETING_PER_REWARD = 1_000e18;
    ...
    // requestId => {roundId, poolType}
    mapping(uint256 => RoundInfo) public lotteryMap;
    ...
    //roundId => PoolType => PrizeInfo
    mapping(uint256 => mapping(PoolType => PoolInfo)) public allPrizePools;

    Counters.Counter internal _roundCounter;

    uint256 public lastRound;

    address public treasury;

    IPrizeBoxToken public token;
    ...
    struct PoolInfo {
        IPrizeNFT prizeNFT;
        uint256 singleBet; // 0.005 ETH
        uint256 betDuration; // 1 days
        uint256 startTime; //
        uint32 callbackNumwords;
        mapping(address => uint256) userLastMintTimestamp; // user => last mint timestamp
        uint32 step;
        uint32 totalNumWords;
        mapping(uint256 => RandomStruct) requestMap; // requestId => RandomStruct
        uint256[] winners;
        mapping(uint256 => uint256) winnersMap; // tokenId => weight
        mapping(uint256 => bool) claimMap; //indexId => claimed
        uint256 perNFTReward;
        uint256 totalReward; // eth
    }
...
import "./interfaces/IPrizeBoxToken.sol";
import "./PrizeNFT.sol";
import "./PrizePoolStorage.sol";

contract PrizePoolV2 is VRFConsumerBaseV2, Ownable, PrizePoolStorage {
    ...
    */
    ...
    function createNewRound(
        ...
        if (pool1Create) {
            IPrizeNFT prizeNFT = new PrizeNFT(
                lastRound,
                uint256(PoolType.ChosenOne)
            );
            PoolInfo storage poolInfo = allPrizePools[lastRound][
                PoolType.ChosenOne
            ];
            poolInfo.singleBet = _singleBet;
            poolInfo.betDuration = _betDuration;
            poolInfo.startTime = _startTime;
            poolInfo.prizeNFT = prizeNFT;
        }
        if (pool2Create) {
            IPrizeNFT prizeNFT = new PrizeNFT(
                lastRound,
                uint256(PoolType.TenPercent)
            );
            PoolInfo storage poolInfo = allPrizePools[lastRound][
                PoolType.TenPercent
            ];
        ...
        emit CreateNewRound(_singleBet, _betDuration, _startTime, pool1Create, pool2Create, pool3Create);
    }
    ...
    function mint(
        ...
        PoolInfo storage poolInfo = allPrizePools[_roundId][_poolType];
        IPrizeNFT prizeNFT = poolInfo.prizeNFT;
        uint256 singleBet = poolInfo.singleBet;
        uint256 betDuration = poolInfo.betDuration;
        uint256 startTime = poolInfo.startTime;
        uint32 callbackNumwords = poolInfo.callbackNumwords;
        mapping(address => uint256) storage userLastMintTimestamp = poolInfo
            .userLastMintTimestamp;
        ...
        if(refAddr != address(0x0)) {
            isFirstBinding = referralStorage.setReferral(refAddr, msg.sender);
        }

        if(isFirstBinding) {
            token.marketingMint(msg.sender, MARKETING_PER_REWARD);
            token.marketingMint(refAddr, MARKETING_PER_REWARD);
        }

        MintSlot memory mintSlot;


        uint256 lastMintTime = userLastMintTimestamp[msg.sender];
        mintSlot.isNormalMode = block.timestamp - lastMintTime >
            FREEZE_MODE_DURATION;
        if (mintSlot.isNormalMode) {
            require(_amount <= NORMAL_MODE_MAX_MINT, "normal amount exceed");
        } else {
            require(_amount <= FREEZE_MODE_MAX_MINT, "freeze amount exceed");
        }
        userLastMintTimestamp[msg.sender] = block.timestamp;

        require(msg.value >= singleBet * _amount, "send eth error");

        if(_amount == 10) {
            mintSlot.tokenMintAmount = 2500 * 1e18;
        } else {
            mintSlot.tokenMintAmount = _amount * 200 * 1e18;
        }
        token.miningMint(msg.sender, mintSlot.tokenMintAmount);

        // uint256 fee = 0;
        mintSlot.perDiscount = 20; //0.2%
        if (mintSlot.isNormalMode) {
            uint256 discount = _amount * mintSlot.perDiscount;
            mintSlot.fee = discount >= FEE_MAX_DISCOUNT
        ...
        mintSlot.feeResult = (msg.value * mintSlot.fee) / FEE_DECIMAL_PRECISION;
        if (mintSlot.feeResult > 0) {
            address referrer = referralStorage.getReferrer(msg.sender);
            //if he has referrer
            if(referrer != address(0x0)) {
                mintSlot.feeToReferrer = mintSlot.feeResult * FEE_TO_REFERRER / FEE_DECIMAL_PRECISION;
                if(mintSlot.feeToReferrer > 0) {
                    referralStorage.setEarnRewardMap(referrer, msg.sender, mintSlot.feeToReferrer);
                }
            }
            sendEther(payable(treasury), mintSlot.feeResult - mintSlot.feeToReferrer);
        }
        mintSlot.wager = msg.value - mintSlot.feeResult;
        poolInfo.totalReward += mintSlot.wager;

        for (uint256 i = 0; i < _amount; i++) {
            prizeNFT.safeMint(msg.sender);
        }
        emit Mint(_roundId, uint256(_poolType), msg.sender, _amount, mintSlot.fee, msg.value);
    }

    function withdrawAsReferrerReward() public {
        (uint256 totalEarnReward, uint256 withdrawnReward) = referralStorage.getReferrerTotalEarnReward(msg.sender);
        if(withdrawnReward >= totalEarnReward) {
            return;
        }
        uint256 withdrawnAmount = totalEarnReward - withdrawnReward;
        referralStorage.withdrawReward(msg.sender, withdrawnAmount);
        sendEther(payable(msg.sender), withdrawnAmount);
        emit WithdrawAsReferrerReward(msg.sender, totalEarnReward, withdrawnReward, withdrawnAmount);
    }

    function getTotalRandomNumber(
        uint256 _roundId,
        PoolType _poolType
    ) public view returns (uint32) {
        PoolInfo storage poolInfo = allPrizePools[_roundId][_poolType];
        IPrizeNFT prizeNFT = poolInfo.prizeNFT;
        uint256 totalSupply = IERC721Enumerable(prizeNFT).totalSupply();
        require(totalSupply > 0, "totalSupply is zero");

        uint32 tempNumWords = 0;
        if (_poolType == PoolType.ChosenOne) {
            tempNumWords = 1;
        } else if (_poolType == PoolType.TenPercent) {
            tempNumWords = uint32(uint256(totalSupply / 10));
        } else if (_poolType == PoolType.FiftyPercent) {
    ...
    function holdLottery(
        ...
        uint256 betDuration = poolInfo.betDuration;
        uint256 startTime = poolInfo.startTime;
        // uint32 step = poolInfo.step;
        // uint32 totalNumWords = poolInfo.totalNumWords;
        mapping(uint256 => RandomStruct) storage requestMap = poolInfo
            .requestMap;
        ...
        require(poolInfo.step == 0, "already fulfill");

        poolInfo.totalNumWords = getTotalRandomNumber(_roundId, _poolType);

        // poolInfo.perNFTReward = poolInfo.totalReward / poolInfo.totalNumWords;

        poolInfo.step = (poolInfo.totalNumWords + randomStep - 1) / randomStep;
        for (uint32 i = 0; i < poolInfo.step; i++) {
            uint32 numWords = 0;
            if (poolInfo.totalNumWords > (i + 1) * randomStep) {
                numWords = randomStep;
            } else {
                numWords = poolInfo.totalNumWords - i * randomStep;
            }
            ...
            requestMap[requestId] = RandomStruct({
                stepIndex: i,
                totalStep: poolInfo.step,
                numWords: numWords,
                fulfill: false
            });
            lotteryMap[requestId] = RoundInfo({
                roundId: _roundId,
                poolType: _poolType
            });
            emit HoldLottery222(msg.sender, _roundId, _poolType, poolInfo.totalNumWords, poolInfo.step, numWords, i);
        }
        emit HoldLottery(msg.sender, _roundId, _poolType, poolInfo.totalNumWords, poolInfo.step);
        return poolInfo.step;
    }
    ...
    function fulfillRandomWords(
        ...
        RoundInfo memory roundInfo = lotteryMap[_requestId];

        PoolInfo storage poolInfo = allPrizePools[roundInfo.roundId][
            roundInfo.poolType
        ];
        mapping(uint256 => RandomStruct) storage requestMap = poolInfo
            .requestMap;
        ...
        poolInfo.callbackNumwords += uint32(length);
        uint256 totalSupply = IERC721Enumerable(poolInfo.prizeNFT)
            .totalSupply();
        for (uint256 i = 0; i < length; i++) {
            uint256 random = _randomWords[i] % totalSupply;
            poolInfo.winners.push(random);
        }
        if (poolInfo.callbackNumwords == poolInfo.totalNumWords) {
            poolInfo.perNFTReward =
                poolInfo.totalReward /
                poolInfo.totalNumWords;
        }
    ...
    function claimReward(
        ...
        PoolInfo storage poolInfo = allPrizePools[_roundId][_poolType];
        IPrizeNFT prizeNFT = poolInfo.prizeNFT;
        uint256[] memory winners = poolInfo.winners;
        if (poolInfo.perNFTReward == 0) {
            return;
        }

        uint256 balanceOf = IERC721(prizeNFT).balanceOf(user);
        if (balanceOf == 0) {
            return;
        }
        if (poolInfo.callbackNumwords != poolInfo.totalNumWords) {
        ...
        if (poolInfo.winnersMap[winners[0]] == 0) {
            for (uint256 i = 0; i < winners.length; i++) {
                uint256 tokenId = winners[i];
                poolInfo.winnersMap[tokenId] = poolInfo.winnersMap[tokenId] + 1;
            }
        }

        uint claimAmount = 0;
        for (uint256 i = 0; i < balanceOf; i++) {
            uint256 tokenId = IERC721Enumerable(prizeNFT).tokenOfOwnerByIndex(
                user,
                i
            );
            uint256 weight = poolInfo.winnersMap[tokenId];
            if (weight > 0) {
                if (!poolInfo.claimMap[tokenId]) {
                    poolInfo.claimMap[tokenId] = true;
                    claimAmount += poolInfo.perNFTReward * weight;
                }
            }
        }
        if (claimAmount == 0) {
    ...
    function claimAll(uint256 _roundId) public {
        PoolInfo storage chosenOnePoolInfo = allPrizePools[_roundId][PoolType.ChosenOne];
        if(chosenOnePoolInfo.singleBet > 0) {
            claimReward(_roundId, PoolType.ChosenOne, msg.sender);
        }

        PoolInfo storage tenPercentoolInfo = allPrizePools[_roundId][PoolType.TenPercent];
        if(tenPercentoolInfo.singleBet > 0) {
            claimReward(_roundId, PoolType.TenPercent, msg.sender);
        }

        PoolInfo storage fiftyPercentPoolInfo = allPrizePools[_roundId][PoolType.FiftyPercent];
        if(fiftyPercentPoolInfo.singleBet > 0) {
            claimReward(_roundId, PoolType.FiftyPercent, msg.sender);
        }
    }
    ...
    function getWinnersLength(
        uint256 _roundId,
        PoolType _poolType
    ) public view returns (uint256) {
        PoolInfo storage poolInfo = allPrizePools[_roundId][_poolType];
        return poolInfo.winners.length;
    }
    ...
    function getClaimAmount(
        ...
        PoolInfo storage poolInfo = allPrizePools[_roundId][_poolType];
        IPrizeNFT prizeNFT = poolInfo.prizeNFT;
        uint256 perNFTReward = poolInfo.perNFTReward;
        uint256[] memory winners = poolInfo.winners;
        mapping(uint256 => bool) storage claimMap = poolInfo.claimMap;

        require(perNFTReward > 0, "reward error");

        uint256 balanceOf = IERC721(prizeNFT).balanceOf(msg.sender);
    ...
    function resetLotteryParams(
        ...
        for (uint256 i = 0; i < winners.length; i++) {
            delete poolInfo.winnersMap[winners[i]];
        }
        poolInfo.step = 0;
        poolInfo.callbackNumwords = 0;
        delete poolInfo.winners;
        poolInfo.perNFTReward = 0;
    }

    function getPoolInfoUserLastMintTimestamp(uint256 _roundId, PoolType _poolType, address user) public view returns(uint256) {
        PoolInfo storage poolInfo = allPrizePools[_roundId][_poolType];
        return poolInfo.userLastMintTimestamp[user];
    }

    function getPoolInfoRequestMap(uint256 _roundId, PoolType _poolType, uint256 _requestId) public view returns(RandomStruct memory) {
        PoolInfo storage poolInfo = allPrizePools[_roundId][_poolType];
        return poolInfo.requestMap[_requestId];
    }

    function getPoolInfoWinnersMap(uint256 _roundId, PoolType _poolType, uint256 _tokenId) public view returns(uint256) {
        PoolInfo storage poolInfo = allPrizePools[_roundId][_poolType];
        return poolInfo.winnersMap[_tokenId];
    }

    function getPoolInfoClaimMap(uint256 _roundId, PoolType _poolType, uint256 _indexId) public view returns(bool) {
        PoolInfo storage poolInfo = allPrizePools[_roundId][_poolType];
        return poolInfo.claimMap[_indexId];
    }

    function getPoolInfoWinners(uint256 _roundId, PoolType _poolType) public view returns(uint256[] memory) {
        PoolInfo storage poolInfo = allPrizePools[_roundId][_poolType];
        return poolInfo.winners;
    }
    ...
    event CreateNewRound(uint256 _singleBet,uint256 _betDuration,uint256 _startTime,bool pool1Create,bool pool2Create,bool pool3Create);

    event WithdrawAsReferrerReward(address indexed user, uint256 totalEarnReward, uint256 withdrawnReward, uint256 withdrawnAmount);

    event HoldLottery(address indexed user, uint256 _roundId, PoolType _poolType, uint256 totalNumWords, uint256 step);

    event HoldLottery222(address indexed user, uint256 _roundId, PoolType _poolType, uint256 totalNumWords, uint256 step, uint256 numWords, uint256 i);
}