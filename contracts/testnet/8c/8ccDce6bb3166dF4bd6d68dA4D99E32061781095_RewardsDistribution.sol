...
contract RewardsDistribution is Owned, IRewardsDistribution {
    ...
    /**
     * @notice Address of the SynthToken
     */
    ...
    /**
     * @dev _authority maybe the underlying synthr contract.
     * Remember to set the authority on a synthr upgrade
     */
    constructor(
        address _owner,
        address _authority,
    ...
    function setRewardEscrowV2(address _rewardEscrowV2) external onlyOwner {
        rewardEscrowV2 = _rewardEscrowV2;
    }
    ...
    function removeRewardDistribution(uint256 index) external onlyOwner {
        ...
        for (uint256 i = 0; i < distributions.length; i++) {
            if (i == index) {
                distributions[i] = distributions[distributions.length - 1];
                distributions.pop();
            }
        }
    }
    ...
    function distributeRewards(uint256 amount) external returns (bool) {
        ...
        require(rewardEscrowV2 != address(0), "RewardEscrowV2 is not set");
        require(synthToken != address(0), "SynthToken is not set");
        require(feePoolProxy != address(0), "FeePoolProxy is not set");
        require(
            IERC20(synthToken).balanceOf(address(this)) >= amount,
        ...
        for (uint256 i = 0; i < distributions.length; i++) {
            if (distributions[i].destination != address(0) || distributions[i].amount != 0) {
                ...
                // Transfer the SYNTH
                IERC20(synthToken).transfer(distributions[i].destination, distributions[i].amount);

                // If the contract implements RewardsDistributionRecipient.sol, inform it how many SYNTH its received.
        ...
        IERC20(synthToken).transfer(rewardEscrowV2, remainder);
