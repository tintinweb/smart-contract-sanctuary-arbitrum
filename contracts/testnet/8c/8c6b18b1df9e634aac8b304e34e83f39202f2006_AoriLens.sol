...
contract AoriPut is ERC20, ReentrancyGuard {
    ...
    function mintPut(uint256 quantityOfUSDC, address receiver, uint256 seatId) public nonReentrant returns (uint256) {
        ...
        if (receiver == AORISEATSADD.ownerOf(seatId)) {
            ...
            mintingFee = 0;
            feeToSeat = 0;
            optionsToMint = (quantityOfUSDC * 10**USDC_DECIMALS) / strikeInUSDC;
    ...
    function sellerSettlementITM() public nonReentrant returns (uint256) {
        ...
        uint256 USDCToReceive = ((optionsToSettle * settlementPrice) / decimalDiff) / (10**decimals()); //((1e18 / 1e6) * 1e6) / 1e6
        //store the settlement
        optionSellers[msg.sender] = 0;
...
contract AoriLens {
    ...
    function getActiveAsksAtRange(uint256 a, uint256 b, Orderbook orderbook) public view returns (uint256[] memory) {
        uint256[] memory sizeAtPrice = new uint256[]((a - b) / orderbook.askMinTick());
        uint256 requests = ((b - a) / orderbook.askMinTick());
        for(uint i; i < requests; i + orderbook.askMinTick()) {
            sizeAtPrice[a + i] = orderbook.asksAtPrice(a + i);
        }
        return sizeAtPrice;
    }
    
    function getActiveBidsAtRange(uint256 a, uint256 b, Orderbook orderbook) public view returns (uint256[] memory) {
        uint256[] memory sizeAtPrice = new uint256[]((a - b) / orderbook.bidMinTick());
        uint256 requests = ((b - a) / orderbook.bidMinTick());
        for(uint i; i < requests; i + orderbook.bidMinTick()) {
            sizeAtPrice[a + i] = orderbook.bidsAtPrice(a + i);
        }
        return sizeAtPrice;
    }
    ...
    function getActiveCallMarkets(CallFactory callFactory) public view returns (AoriCall[] memory) {
        ...
        for(uint i; i < allCalls.length; i++) {
            if(AoriCall(allCalls[i]).endingTime() >= block.timestamp && address(allCalls[i]) != address(0)) {
    ...
    function getActivePutMarkets(PutFactory putFactory) public view returns (AoriPut[] memory) {
        ...
        for(uint i; i < allPuts.length; i++) {
            if(AoriPut(allPuts[i]).endingTime() >= block.timestamp && address(allPuts[i]) != address(0)) {
    ...
    function getActiveOrderbooks(OrderbookFactory orderbookFactory) public view returns (Orderbook[] memory) {
        ...
        for(uint i; i < allBooks.length; i++) {
            if(Orderbook(allBooks[i]).endingTime() >= block.timestamp && address(allBooks[i]) != address(0)) {
    ...
    function getActiveOrderbooksAtEndingTime(OrderbookFactory orderbookFactory, uint256 endingTime) public view returns (Orderbook[] memory) {
        Orderbook[] memory allBooks = getActiveOrderbooks(orderbookFactory);
        Orderbook[] memory activeBooksAtSameTime = new Orderbook[](allBooks.length);

        uint256 activeCount;
        for(uint i; i < allBooks.length; i++) {
            if(Orderbook(allBooks[i]).endingTime() == endingTime && address(allBooks[i]) != address(0)) {
                activeBooksAtSameTime[activeCount++] = allBooks[i];
            }
        }
        return activeBooksAtSameTime;
    }
    
    function getAllOrderbooksAtEndingTime(OrderbookFactory orderbookFactory, uint256 endingTime) public view returns (Orderbook[] memory) {
        Orderbook[] memory allBooks = orderbookFactory.getAllOrderbooks();
        Orderbook[] memory activeBooksAtSameTime = new Orderbook[](allBooks.length);
...
contract MarginManager is Ownable, ReentrancyGuard {
    ...
    function settlePosition(address account, bytes32 key) public nonReentrant {
        ...
        if(position.isCall) {
            ...
            if(AoriCall(position.option).getITM() > 0) {
                ...
                uint256 usdcValOfCollateral = positionRouter.mulDiv(AoriCall(position.option).settlementPrice(), position.collateral, expScale);
                uint256 percentLoss = positionRouter.mulDiv(AoriCall(position.option).settlementPrice() - position.strikeInUSDC, USDCScale, usdcValOfCollateral);
                collateralMinusLoss = position.collateral - positionRouter.mulDiv(percentLoss, position.collateral, USDCScale);
                doTransferOut(underlying, account, collateralMinusLoss);
            } else {
                lpTokens[underlying].settleOption(position.option, true, false);
                doTransferOut(underlying, account, position.collateral);                
            }
        } else {
            require(block.timestamp >= AoriCall(position.option).endingTime(), "Option has not reached expiry");
            ...
            if(AoriPut(position.option).getITM() > 0) {
                ...
                collateralMinusLoss = position.collateral - mulDiv(position.strikeInUSDC - AoriPut(position.option).settlementPrice(), position.optionSize, expScale);
                doTransferOut(underlying, account, collateralMinusLoss);
            } else {
    ...
    /**
        Good
     */
    function accruePositionInterest(bytes32 key) public nonReentrant returns (bool) {
        Position storage position = positions[key];
        ERC20 underlying;
        //irm calc
        AoriCall call;
...
contract PositionRouter is Ownable {
    ...
    function executeOpenPosition(uint256 indexToExecute) public returns (bytes32) {
        ...
        if(positionRequest.isCall) {
            ...
            underlying = AoriPut(address(Orderbook(positionRequest.orderbook).USDC()));
            underlying.approve(address(manager), positionRequest.collateral);
        }
    ...
    */
    ...
    function getPosition(address _account, uint256 _index) public view returns (address, uint256, uint256, address, address, bool, uint256, uint256) {
...
contract Vault is Ownable, ReentrancyGuard, ERC4626 {
    ...
    function settleOption(address option, bool isCall, bool isITM) external returns (uint256) {
        if(isITM) {
            settleITMOption(option, isCall);
        } else {
            settleOTMOption(option, isCall);
        }
    }

    function settleITMOption(address option, bool isCall) internal nonReentrant returns (uint256) {
    ...
    function settleOTMOption(address option, bool isCall) internal nonReentrant returns (uint256) {
        Structs.settleVars memory vars;
        if(isSettled[option]) {
            return 0;
        } else {
...
contract Orderbook is Ownable, ReentrancyGuard {
    ...
    uint256 public askMinTick; //1e4
    uint256 public bidMinTick; //1e16
    ...
    mapping(address => uint256) public lockedOPTION;

    mapping(uint256 => uint256) public bidsAtPrice;
    mapping(uint256 => order[]) public bidSide;
    mapping(uint256 => uint256) public bidSideIndex;

    mapping(uint256 => uint256) public asksAtPrice;
    mapping(uint256 => order[]) public askSide;
    mapping(uint256 => uint256) askSideIndex;

    uint256[] activeBidPrices;
    uint256[] activeAskPrices;
    ...
    constructor(
        ...
        OPTION = _OPTION;
        USDC = _USDC;
        askMinTick = _askMinTick;
        bidMinTick = _bidMinTick;
    }

    event AskCreated(uint256 USDCPerOPTION, uint256 OPTIONSize);
    event BidCreated(uint256 OPTIONPerUSDC, uint256 _USDCSize);
    //createAsk(1*10**6, 1e18);
    function createAsk(uint256 _USDCPerOPTION, uint256 _OPTIONSize) public nonReentrant {
        require(askMinTick % _USDCPerOPTION == askMinTick);
        OPTION.transferFrom(msg.sender, address(this), _OPTIONSize);
        order memory userOrder;
        userOrder.account = msg.sender;
        userOrder.price = _USDCPerOPTION;
        userOrder.amount = _OPTIONSize;
        
        uint256 currentAsksHere = bidsAtPrice[_USDCPerOPTION];
        asksAtPrice[_USDCPerOPTION] = currentAsksHere + _OPTIONSize;

        uint256 userLockedOPTION = lockedOPTION[msg.sender];
        lockedOPTION[msg.sender] = userLockedOPTION + _OPTIONSize;
        askSide[_USDCPerOPTION].push(userOrder);
        emit AskCreated(_USDCPerOPTION, _OPTIONSize);
    }

    function createBid(uint256 _OPTIONPerUSDC, uint256 _USDCSize) public {
        require(bidMinTick % _OPTIONPerUSDC == bidMinTick);
        USDC.transferFrom(msg.sender, address(this), _USDCSize);
        order memory userOrder;
        userOrder.account = msg.sender;
        userOrder.price = _OPTIONPerUSDC;
        userOrder.amount = _USDCSize;

        uint256 currentBidsHere = bidsAtPrice[_OPTIONPerUSDC];
        bidsAtPrice[_OPTIONPerUSDC] = currentBidsHere + _USDCSize;

        uint256 userLockedOPTION = lockedOPTION[msg.sender];
        lockedOPTION[msg.sender] = userLockedOPTION + _USDCSize;
        bidSide[_OPTIONPerUSDC].push(userOrder);
        emit BidCreated(_OPTIONPerUSDC, _USDCSize);
    }
    //createAsk(1*10**6, 1e18);
    function fillAsk(uint256 USDCPerOPTION, uint256 OPTIONSize, uint256 seatId) public {
        require(askSide[USDCPerOPTION].length > 0, "No limit orders here");
        require(asksAtPrice[USDCPerOPTION] > 0, "No limit orders here");
        require(AORISEATSADD.confirmExists(seatId) && AORISEATSADD.ownerOf(seatId) != address(0x0), "Seat does not exist");
        
        USDC.transferFrom(msg.sender, address(this), mulDiv(USDCPerOPTION, OPTIONSize, expScale));
        uint256 filled;
        if(AORISEATSADD.ownerOf(seatId) == msg.sender) {
            uint256 localFee = getFee(msg.sender, seatId, mulDiv(USDCPerOPTION, OPTIONSize, expScale));
            uint256 seatShare = getSeatShare(seatId);
            doTransferOut(USDC, owner(), mulDiv(localFee, 100 - seatShare, 100));
            doTransferOut(USDC, AORISEATSADD.ownerOf(seatId), mulDiv(localFee, seatShare, 100));
            OPTIONSize -= localFee;
        }
        for(uint i = 0; i < askSide[USDCPerOPTION].length; i++) {
            order storage orderToFill;
            orderToFill = askSide[USDCPerOPTION][askSideIndex[USDCPerOPTION]];
            if(OPTIONSize >= orderToFill.amount) {
                doTransferOut(USDC, orderToFill.account, mulDiv(orderToFill.amount, USDCPerOPTION, expScale));
                OPTIONSize -= orderToFill.amount;
                filled += orderToFill.amount;
                asksAtPrice[USDCPerOPTION] -= orderToFill.amount;
                delete askSide[USDCPerOPTION][askSideIndex[USDCPerOPTION]];
                askSideIndex[USDCPerOPTION]++;
            } else {
                require(OPTIONSize < askSide[USDCPerOPTION][askSideIndex[USDCPerOPTION]].amount, "not enough to fill");
                uint256 OPTIONSizeLeft = askSide[USDCPerOPTION][askSideIndex[USDCPerOPTION]].amount - OPTIONSize;
                doTransferOut(USDC, orderToFill.account, mulDiv(askSide[USDCPerOPTION][askSideIndex[USDCPerOPTION]].amount, USDCPerOPTION, expScale));
                OPTIONSize -= orderToFill.amount;
                filled += orderToFill.amount;
                asksAtPrice[USDCPerOPTION] -= orderToFill.amount;
                askSide[USDCPerOPTION][askSideIndex[USDCPerOPTION]].amount = OPTIONSizeLeft;
                break;
            }
        }
        doTransferOut(OPTION, msg.sender, mulDiv(filled, USDCPerOPTION, USDCScale));
    }
  
    function fillBid(uint256 OPTIONPerUSDC, uint256 USDCSize, uint256 seatId) public {
        require(bidSide[OPTIONPerUSDC].length > 0, "No limit orders here");
        require(bidSide[OPTIONPerUSDC][bidSideIndex[OPTIONPerUSDC]].amount != 0, "order is non-existant");
        
        OPTION.transferFrom(msg.sender, address(this), mulDiv(OPTIONPerUSDC, USDCSize, USDCScale));
        ...
        uint256 localFee;
        uint256 filled;
        for(uint i = 0; i < bidSide[OPTIONPerUSDC].length; i++) {
            order storage orderToFill;
            orderToFill = bidSide[OPTIONPerUSDC][bidSideIndex[OPTIONPerUSDC]];
            if(USDCSize >= orderToFill.amount) {
                doTransferOut(OPTION, orderToFill.account, mulDiv(orderToFill.amount, OPTIONPerUSDC, USDCScale));
                USDCSize -= orderToFill.amount;
                bidsAtPrice[OPTIONPerUSDC] -= orderToFill.amount;
                filled += orderToFill.amount;
                delete bidSide[OPTIONPerUSDC][bidSideIndex[OPTIONPerUSDC]];
                bidSideIndex[OPTIONPerUSDC]++;
            } else {
                require(USDCSize < bidSide[OPTIONPerUSDC][bidSideIndex[OPTIONPerUSDC]].amount, "not enough to fill");
                uint256 usdcLeft = bidSide[OPTIONPerUSDC][bidSideIndex[OPTIONPerUSDC]].amount;
                doTransferOut(OPTION, orderToFill.account, mulDiv(usdcLeft, expScale, orderToFill.amount));
                USDCSize -= orderToFill.amount;
                bidsAtPrice[OPTIONPerUSDC] -= orderToFill.amount;
                filled += USDCSize;
                bidSide[OPTIONPerUSDC][bidSideIndex[OPTIONPerUSDC]].amount = usdcLeft - USDCSize;
                break;
            }
        }
        
        uint256 seatToReceive = mulDiv(localFee, getSeatShare(seatId), 100);
        doTransferOut(USDC, AORISEATSADD.ownerOf(seatId), seatToReceive);
        doTransferOut(USDC, owner(), localFee - seatToReceive);
        doTransferOut(USDC, msg.sender, mulDiv(USDCSize, OPTIONPerUSDC, expScale) - localFee);
    }
    ...
    function getFee(address receiver, uint256 seatId, uint256 size) internal view returns (uint256) {        
        if(AORISEATSADD.ownerOf(seatId) == receiver) {
            return 0;
        } else {
            return mulDiv(size, AORISEATSADD.tradingFee(), BPS_DIVISOR);
        }
    }

    function getSeatShare(uint256 seatId) internal view returns (uint256) {
        uint256 score = AORISEATSADD.seatScore(seatId);
        return ((score * 5) + 35);
    }
...
contract OrderbookFactory is Ownable {
    ...
    address public keeper;
    AoriSeats public AORISEATSADD;
    uint256 public askMinTick = 1e4; //1e4
    uint256 public bidMinTick = 1e6; //1e16
    ...
    */
    ...
    function setTicks(uint256 newAskMinTick, uint256 newBidMinTick) external onlyOwner returns (uint256, uint256) {
        askMinTick = newAskMinTick;
        bidMinTick = newBidMinTick;
        return (askMinTick, bidMinTick);
    }
    ...
    */
    function createOrderbook(
        ...
        Orderbook orderbook =  new Orderbook(OPTION_, USDC, AORISEATSADD, _duration , 1e4, 1e6); 
    ...
    function withdrawFees(ERC20 token, uint256 amount_) external onlyOwner returns(uint256) {
            ERC20(token).transfer(owner(), amount_);
            return amount_;
    }
...
contract PutFactory is Ownable {
    ...
    */
    ...
    function createPutMarket(
        ...
        emit AoriPutCreated(ERC20(address(putMarket)), strikeInUSDC, duration, USDC, oracle, name_, symbol_);
        return (putMarket);
    }
