...
contract RequestFLRarbitrumTestnet is OwnableUpgradeable {
    using SafeERC20Upgradeable for FlorinToken;

    mapping(address => bool) private minters;

    uint256 private flrTokenPerRequest;
    ...
    function initialize(FlorinToken _florinToken, uint256 _flrTokenPerRequest) external initializer {
        __Ownable_init_unchained();
        flrTokenPerRequest = _flrTokenPerRequest;
        florinToken = _florinToken;
    }

    function setFLRperRequest(uint256 _flrTokenPerRequest) external onlyOwner {
        flrTokenPerRequest = _flrTokenPerRequest;
    }

    function requestFreeFLR() external {
        require(minters[msg.sender] == false, "request only once per address");
        require(florinToken.balanceOf(address(this)) >= flrTokenPerRequest, "ran out of test FLR Token");
        florinToken.safeTransfer(_msgSender(), flrTokenPerRequest);
        minters[msg.sender] = true;
    }
    function transferAllFlrToOwner() external onlyOwner {
        florinToken.safeTransfer(_msgSender(), florinToken.balanceOf(address(this)));
    }
}