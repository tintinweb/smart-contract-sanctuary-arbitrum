...
contract KeeperArctic is AccessControlledAndUpgradeable, KeeperCompatibleInterface {
  ...
  function shouldUpdateMarketRaw()
    external
    view
    returns (
      bool canExec,
    ...
    )
  {
    ...
    if (shouldUpdate) {
      return (true, address(market), missedEpochsOracleRoundIds);
    }

    return (false, address(0), new uint80[](0));
  }
...
library OracleManagerUtils {
  // NOTE: this code isn't used currently, it is kept here as a reference for future work. It is the same as the `getOracleInfoForSystemStateUpdate` function in the `KeeperArctic` contract but doesn't take in the latest oracleId.
  ...
  function getChainlinkPriceAndTimestamp(
    ...
    returns (
      ...
      uint80 updatedOracleRoundId
    )
  {
    AggregatorV3Interface chainlinkOracle = oracleManager.chainlinkOracle();
    (, currentOraclePrice, currentOracleUpdateTimestamp, , ) = chainlinkOracle.getRoundData(oracleRoundId);
    updatedOracleRoundId = oracleRoundId;
    ...
    if (currentOracleUpdateTimestamp == 0 && previousOracleUpdateTimestamp != 0) {
      ...
      while (currentOracleUpdateTimestamp == 0) {
        updatedOracleRoundId = (((updatedOracleRoundId >> 64) + 1) << 64) + 1;
        (, currentOraclePrice, currentOracleUpdateTimestamp, , ) = chainlinkOracle.getRoundData(updatedOracleRoundId);
      }
    }

    return (currentOraclePrice, currentOracleUpdateTimestamp, updatedOracleRoundId);
  }

  function getNumberOfMissedEpochs(IOracleManager oracleManager, uint32 _latestExecutedEpochIndex)
    internal
    view
    returns (
      uint256 _numberOfMissedEpochs,
      uint256 relevantEpochStartTimestampWithMEWT,
      uint256 epochLength
    )
  {
    AggregatorV3Interface chainlinkOracle = oracleManager.chainlinkOracle();
    ...
    epochLength = oracleManager.EPOCH_LENGTH();
    uint256 mewt = oracleManager.MINIMUM_EXECUTION_WAIT_THRESHOLD();

    uint256 latestValidEpoch = ((previousOracleUpdateTimestamp - mewt - oracleManager.initialEpochStartTimestamp()) / epochLength);
    relevantEpochStartTimestampWithMEWT = ((_latestExecutedEpochIndex + 2) * epochLength) + mewt + oracleManager.initialEpochStartTimestamp();
    _numberOfMissedEpochs = latestValidEpoch - _latestExecutedEpochIndex - 1;

    // Check whether latestRoundData.startedAt is before end point of previous epoch
    // If met, then break
    if (oracleManager.getEpochStartTimestamp() - oracleManager.EPOCH_LENGTH() > previousOracleUpdateTimestamp) {
      return (0, relevantEpochStartTimestampWithMEWT, epochLength);
    }
  }

  function getMissedEpochOracleRoundIdsWithStep(
    uint80[] memory _missedEpochOracleRoundIds,
    uint256 _numberOfMissedEpochs,
    uint80 step,
    uint80 latestExecutedOracleRoundId,
    IOracleManager oracleManager,
    ...
    AggregatorV3Interface chainlinkOracle = oracleManager.chainlinkOracle();
    (, , uint256 previousOracleUpdateTimestamp, , ) = chainlinkOracle.getRoundData(latestExecutedOracleRoundId);
    (uint80 latestRoundId, , , , ) = chainlinkOracle.latestRoundData();

    while (currentUpdateIndex < _numberOfMissedEpochs) {
      uint80 previousOracleRoundId = latestExecutedOracleRoundId;
      latestExecutedOracleRoundId += step;

      (int256 currentOraclePrice, uint256 currentOracleUpdateTimestamp, uint80 updatedOracleRoundId) = getChainlinkPriceAndTimestamp(
        oracleManager,
        latestExecutedOracleRoundId,
        previousOracleUpdateTimestamp
      );

      if (latestExecutedOracleRoundId > latestRoundId || currentOracleUpdateTimestamp == 0) {
        for (uint80 stepIndex = 1; stepIndex <= step; stepIndex++) {
          (currentOraclePrice, currentOracleUpdateTimestamp, updatedOracleRoundId) = getChainlinkPriceAndTimestamp(
            oracleManager,
            previousOracleRoundId + stepIndex,
            previousOracleUpdateTimestamp
          );

          if (currentOracleUpdateTimestamp != 0) {
            latestExecutedOracleRoundId = updatedOracleRoundId;
            break;
          }
        }
      } else if (currentOracleUpdateTimestamp >= relevantEpochStartTimestampWithMEWT) {
        for (uint80 stepIndex = 1; stepIndex <= step; stepIndex++) {
          (currentOraclePrice, currentOracleUpdateTimestamp, updatedOracleRoundId) = getChainlinkPriceAndTimestamp(
            oracleManager,
            previousOracleRoundId + stepIndex,
  ...
  function getOracleInfoForSystemStateUpdate(
    ...
    uint80 latestExecutedOracleRoundId,
    uint80 step
  ) internal view returns (uint80[] memory _missedEpochOracleRoundIds) {
    (uint256 _numberOfMissedEpochs, uint256 relevantEpochStartTimestampWithMEWT, uint256 epochLength) = getNumberOfMissedEpochs(
      oracleManager,
    ...
    getMissedEpochOracleRoundIdsWithStep(
      _missedEpochOracleRoundIds,
      _numberOfMissedEpochs,
      step,
      latestExecutedOracleRoundId,
      oracleManager,
      relevantEpochStartTimestampWithMEWT,
      epochLength
    );
  }
  ...
  function getOracleInfoForSystemStateUpdate(
    ...
    return getOracleInfoForSystemStateUpdate(oracleManager, _latestExecutedEpochIndex, latestExecutedOracleRoundId, 1);
  }
  ...
  // TODO: delete and deprecate this function
  ...
  // TODO: delete and deprecate this function
  function getOracleInfoForSystemStateUpdateLegacy(
    IOracleManager oracleManager,
    uint32 latestExecutedEpochIndex,
    uint80 latestExecutedOracleRoundId
  ) internal view returns (uint80[] memory missedEpochOracleRoundIds) {
    console2.log("latestExecutedEpochIndex", latestExecutedEpochIndex);
    console2.log("latestExecutedOracleRoundId", latestExecutedOracleRoundId);
    ...
    console2.log("numberOfEpochsSinceLastEpoch", numberOfEpochsSinceLastEpoch);
    // If the oracle falls more than 6 epochs behind it will only return 6 of them (but catch up 6 at a time).
    //      And 30 for mumbai (because it can handle bigger transactions)
  ...
  // TODO: delete and deprecate this function
  function getEpochStartTimestamp(IOracleManager oracleManager, uint32 epochIndex) internal view returns (uint256) {
