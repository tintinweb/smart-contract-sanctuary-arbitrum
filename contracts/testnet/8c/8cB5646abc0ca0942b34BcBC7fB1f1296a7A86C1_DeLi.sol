...
import "./LPDistribute.sol";
import "./ICamelotFactory.sol";
import "./ICamelotRouter.sol";

contract DeLi is ERC20, Ownable {
    address public pair;
    ...
    DeLiDividendTracker public dividendTracker;
    ...
    Taxes public divTax = Taxes(25, 25);
    LPShare public lpShare = LPShare(95, 5); //95% - 5%

    uint256 public totalBuyTax = 50; //5%
    uint256 public totalSellTax = 50; //5%
    uint256 public taxDecimal = 1000; 
    ...
    event SendNFTStakingReward(uint256 tokensSwapped, uint256 amount);
    ...
    constructor(address _developerwallet,
        address _factory,
        address _swapRouter,
        address _weth) ERC20("xD", "xD") {
        dividendTracker = new DeLiDividendTracker();
        setDevWallet(_developerwallet);

        router= ICamelotRouter(_swapRouter);
    ...
    function initializePair() external onlyOwner {
        ...
        address _pair = factory.createPair(address(WETH), address(this));
        pair = _pair;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        DeLiDividendTracker newDividendTracker = DeLiDividendTracker(
            payable(newAddress)
        );
    ...
    function setMaxBuyAndSell(uint256 maxBuy, uint256 maxSell)
        ...
        onlyOwner
    {
        require(maxBuy >= 100000, "Cannot set maxbuy lower than 1% ");
    ...
    function updateRouter(address newRouter) external onlyOwner {
        router = ICamelotRouter(newRouter);
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            uint256 feeAmt;
            if (automatedMarketMakerPairs[to])
                feeAmt = (amount * totalSellTax) / taxDecimal;
            else if (automatedMarketMakerPairs[from])
                feeAmt = (amount * totalBuyTax) / taxDecimal;
    ...
    function swapAndLiquify(uint256 tokens) private {
        uint256 toSwapForLiq = (tokens * divTax.liquidity) / taxDecimal / 2;
        uint256 tokensToAddLiquidityWith = (tokens * divTax.liquidity)/taxDecimal / 2;
        uint256 toSwapForDev = (tokens * divTax.dev) /taxDecimal;
        ...
        uint256 dividends = lpBalance * lpShare.tokenHolder/100;
        //Send LP to nftStakeContract
        uint256 lpRewardForStaker = lpBalance - dividends;
        ...
        if (lpRewardForStaker > 0) {
            bool success = IERC20(pair).transfer(
                nftStakeContract,
                lpRewardForStaker
            );
            if (success) {
                emit SendNFTStakingReward(tokens, lpRewardForStaker);
            }
        }
    }
...
contract DeLiDividendTracker is Ownable, DividendPayingToken {
    struct AccountInfo {
        address account;
    ...
    constructor()
        DividendPayingToken("DeLi_Dividend_Tracker", "DeLi_Dividend_Tracker")
    {}
    ...
    function _transfer(
        ...
        require(false, "DeLi_Dividend_Tracker: No transfers allowed");
    }
