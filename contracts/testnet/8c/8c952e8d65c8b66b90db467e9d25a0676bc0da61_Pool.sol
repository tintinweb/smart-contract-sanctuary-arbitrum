...
contract Pool is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PoolStorage {
    ...
    function validateToken(address _indexToken, address _collateralToken, bool _isIncrease)
        external
        view
        returns (bool)
    {
        return _validateToken(_indexToken, _collateralToken, _isIncrease);
    }
    ...
    function increasePosition(
        ...
        _requireValidTokenPair(_indexToken, _collateralToken, true);
    ...
    function decreasePosition(
        ...
        _requireValidTokenPair(_indexToken, _collateralToken, false);
        uint256 borrowIndex = _accrueInterest(_collateralToken);
        bytes32 key = _getPositionKey(_owner, _indexToken, _collateralToken, _side);
        Position memory position = positions[key];
    ...
    function setMaintenanceMargin(uint256 _ratio) external onlyOwner {
    ...
    function _validateToken(address _indexToken, address _collateralToken, bool _isIncrease)
        internal
        view
        returns (bool)
    {
        ...
        returns (uint256 outAmount, uint256 outAmountAfterFee, uint256 daoFee, uint256 tokenPrice)
    {
        ...
        uint256 _fee = _calcFeeRate(_tokenOut, tokenPrice, (_lpAmount * poolValue) / totalSupply , addRemoveLiquidityFee, fee.taxBasisPoint, false);
    ...
    function _validatePosition(
        ...
        if (_liquidatePositionAllowed(_position, _side, _indexPrice, poolTokens[_collateralToken].borrowIndex)) {
            revert PoolErrors.UpdateCauseLiquidation();
        }
    }

    function _requireValidTokenPair(address _indexToken, address _collateralToken, bool _isIncrease)
        internal
        view
    {
        if (!_validateToken(_indexToken, _collateralToken, _isIncrease)) {
    ...
    function _accrueInterest(address _token) internal returns (uint256) {
        ...
        if (tokenInfo.lastAccrualTimestamp == 0 || asset.poolAmount == 0) {
            tokenInfo.lastAccrualTimestamp = (block.timestamp / accrualInterval) * accrualInterval;
        } else {
            uint256 nInterval = (block.timestamp - tokenInfo.lastAccrualTimestamp) / accrualInterval;
            if (nInterval == 0) {
    ...
    function _releasePoolAsset(
        ...
        if (_getPoolAsset(_collateralToken).reservedAmount < _vars.reserveReduced) {
        ...
        returns (uint256 feeValue)
    {
        ...
        feeValue = borrowFee + (_sizeChanged * fee.positionFee) / PRECISION;
    }
    ...
    function _getIndexPrice(address _token, Side _side, bool _isIncrease) internal view returns (uint256) {
        ...
        return oracle.getIndexPrice(_token, _isIncrease == (_side == Side.LONG));
    }

    function _getCollateralPrice(address _token, bool _isIncrease) internal view returns (uint256) {
            return  10 ** (USD_VALUE_DECIMAL - IERC20Metadata(_token).decimals()) ; 
    }
    ...
    function _updateGlobalShortPrice(
        ...
        uint256 shortPrice =
            PositionUtils.calcAveragePrice(Side.SHORT, lastSize, nextSize, averageShortPrices[_tranche][_indexToken], _indexPrice, _realizedPnl);
