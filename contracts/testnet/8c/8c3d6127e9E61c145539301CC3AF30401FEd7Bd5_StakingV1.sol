...
contract StakingV1 is IERC721Receiver, ReentrancyGuard, Pausable {
  ...
  uint256 public constant TIER_1 = 14;
  uint256 public constant TIER_2 = 7;
  ...
  // EVENTS
  //=======================================
  event StakedEvent(uint256 realmId, uint256 resourceId);
    ...
    whenNotPaused
  {
    ...
    emit StakedEvent(_realmId, _resourceId);
  }
  ...
  function _addData(
    ...
    uint256 resource = (_resource + _structure + _tier + _amount) * _multiplier;

    // Add data
    DATA.add(_realmId, _resourceId, resource);
  ...
  function _addCollectibles(uint256 _realmId, uint256 _resourceId) internal {
    ...
    if (_resourceId == 1) {
      id = foodIds[_rarity(_resourceId, foodProbability)];
