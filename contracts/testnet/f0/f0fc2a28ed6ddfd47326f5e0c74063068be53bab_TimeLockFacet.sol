...
library LibTimeLock {
    ...
    function queueTransaction(string calldata functionSignature, bytes calldata data) internal {
        ...
        uint256 executeTime = Chain.currentTimestamp() + Constants.TIME_LOCK_DELAY;
        ...
        tls.transactions[transactionHash] = Transaction(
            ...
            uint40(Chain.currentTimestamp() + Constants.TIME_LOCK_GRACE_PERIOD)
        );
    ...
    function executeTransaction(bytes32 transactionHash) internal returns (bytes memory) {
        TimeLockStorage storage tls = timeLockStorage();
        require(tls.transactions[transactionHash].executeTime > 0, "LibTimeLock: Transaction hasn't been queued");
        require(Chain.currentTimestamp() >= tls.transactions[transactionHash].executeTime, "LibTimeLock: Transaction hasn't surpassed time lock");
        require(Chain.currentTimestamp() <= tls.transactions[transactionHash].expiryTime, "LibTimeLock: Transaction is stale");
