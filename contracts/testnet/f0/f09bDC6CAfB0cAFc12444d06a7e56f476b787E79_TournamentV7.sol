...
contract TournamentV7 is ReentrancyGuard, ITournamentV7, Ownable {
    ...
    // quick match buy in token => buy in amount => waitingRoom
    mapping (address => mapping(uint256 => WaitingRoom)) public waitingRoom;
    ...
    function joinWaitingRoom(address _targetToken, address _buyInToken, uint256 _buyInAmount) external {
        WaitingRoom storage _waitingRoom = waitingRoom[_buyInToken][_buyInAmount];
        if( _waitingRoom.buyInToken == address(0)){
            ...
            waitingRoom[_buyInToken][_buyInAmount] = _tmpWaitingRoom;
            _waitingRoom = waitingRoom[_buyInToken][_buyInAmount];
        }
    ...
    function leave(address _buyInToken, uint256 _buyInAmount) external {
        WaitingRoom storage _waitingRoom = waitingRoom[_buyInToken][_buyInAmount];
