...
import {ID as TOKEN_TYPE_SYSTEM_ID, TokenType, TokenTypeSystem} from "../tokentype/TokenTypeSystem.sol";
import {MANAGER_ROLE} from "../../Constants.sol";
    ...
    ITokenURIHandler
{
    ...
    function getExtraTraits(
        ...
        string memory imageUri = _imageUri(tokenContract, tokenId);
        ...
        // Image
        // NOTE: this will use the enhanced URI if setTokenType has been called.
    ...
    function _imageUri(
        ...
        uint256 tokenEntity = EntityLibrary.tokenToEntity(
            tokenContract,
            tokenId
        );

        TokenTypeSystem tokenTypeSystem = TokenTypeSystem(
            _getSystem(TOKEN_TYPE_SYSTEM_ID)
        );
        TokenType tokenType = tokenTypeSystem.getTokenType(tokenEntity);
        string[] memory baseImageURIs = tokenTypeSystem.getBaseImageURIs(
            tokenEntity
        );
        string memory imageUri = ITraitsConsumer(tokenContract).imageURI(
            tokenId
        );
...
// Globals
uint256 constant PIRATE_NFT_IMAGES_DEFAULT_ID = uint256(
    keccak256("tokentype.piratenft.images.default")
);
uint256 constant PIRATE_NFT_IMAGES_ENHANCED_ID = uint256(
    keccak256("tokentype.piratenft.images.enhanced")
);

uint256 constant TOKEN_TYPE_TRAIT_ID = uint256(keccak256("token_type"));
...
contract TokenTypeSystem is GameRegistryConsumerUpgradeable {
    ...
    /**
     * @notice Returns unlocked status of an account
     * @param account Address to retrieve unlocked status for
     */
    function getAccountUnlocked(address account) external view returns (bool) {
        return
    ...
    /**
     * @notice Returns the TokenType for a given entity
     * @param tokenEntity Entity to get token type for
     */
    function getTokenType(
        uint256 tokenEntity
    ) external view returns (TokenType) {
        return _getTokenType(tokenEntity);
    }

    /**
     * @notice Returns base image URIs for the given token
     * @param tokenEntity Entity to get token type metadata for
     */
    function getBaseImageURIs(
        uint256 tokenEntity
    ) external view returns (string[] memory) {
        IGameGlobals gameGlobals = IGameGlobals(_getSystem(GAME_GLOBALS_ID));

        if (_getTokenType(tokenEntity) == TokenType.ENHANCED) {
            return gameGlobals.getStringArray(PIRATE_NFT_IMAGES_ENHANCED_ID);
        }

        return gameGlobals.getStringArray(PIRATE_NFT_IMAGES_DEFAULT_ID);
    }

    /**
     * @notice Grants a given account permission to change its tokens' types
     * @param account Address to enable or disable
     * @param isUnlocked Whether to unlock the account for making token type changes
     */
    ...
    /**
     * @notice Sets the tokenType and various metadata for valid token entities
     * @param tokenEntities Array of entities for tokens to update
     * @param tokenType The type to set tokens to
     */
    function setTokenType(
        uint256[] calldata tokenEntities,
        TokenType tokenType
    ) external whenNotPaused onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        address pirateContract = _getSystem(PIRATE_NFT_ID);
        ...
        for (uint256 i = 0; i < tokenEntities.length; i++) {
            ...
            // NOTE: extend getBaseImageURIs if supporting more contracts
            if (tokenContract != pirateContract) {
