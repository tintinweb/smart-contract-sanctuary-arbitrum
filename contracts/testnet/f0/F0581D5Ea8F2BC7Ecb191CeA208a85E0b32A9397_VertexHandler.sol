...
interface IFQuerier {
    ...
    function getSubaccountInfo(bytes32 subaccount) external view returns (SubaccountInfo memory);
...
contract VertexHandler is OwnableUpgradeable {
    ...
    function addressToBytes(address inputAddress) public pure returns (bytes memory) {
        bytes memory byteArray = new bytes(20);

        for (uint8 i = 0; i < 20; i++) {
            byteArray[i] = bytes1(uint8(uint160(inputAddress) / (2**(8*(19 - i)))));
        }

        return byteArray;
    }

    function getSubAccount(bytes32 subaccount) external view returns (IFQuerier.SubaccountInfo memory subaccountInfo) {
        IFQuerier querier = IFQuerier(vertexQuerier);
        subaccountInfo = querier.getSubaccountInfo(subaccount);
    }

    // https://github.com/vertex-protocol/vertex-typescript-sdk/blob/main/packages/contracts/src/utils/subaccountInfo.ts#L42
    function getVertexBalance(bytes32 subaccount) external view returns (int256 balance) {
        IFQuerier querier = IFQuerier(vertexQuerier);
        IFQuerier.SubaccountInfo memory subaccountInfo = querier.getSubaccountInfo(subaccount);

        balance = 0;

        for (uint i = 0; i < subaccountInfo.spotBalances.length; i++) {
            IFQuerier.SpotBalance memory spotBalance = subaccountInfo.spotBalances[i];
            IFQuerier.SpotProduct memory spotProduct = findSpotProduct(spotBalance.productId, subaccountInfo.spotProducts);
            balance += calcSpotBalanceValue(
                spotBalance,
                spotProduct
            );
            balance += calcLpBalanceValue(
                spotBalance,
                spotProduct
            );
        }
    ...
    function getTest(bytes32 subaccount) external view returns (int256 balance) {
        IFQuerier querier = IFQuerier(vertexQuerier);
        IFQuerier.SubaccountInfo memory subaccountInfo = querier.getSubaccountInfo(subaccount);

        balance = calcSpotBalanceValue(
                subaccountInfo.spotBalances[0],
                subaccountInfo.spotProducts[0]
                // findSpotProduct(spotBalance.productId, subaccountInfo.spotProducts[i])
            );
    ...
    function calcLpBalanceValue(
        ...
        if(spotBalance.lpBalance.amount == 0) {
            ...
            balance = calcLpTokenValue(spotProduct)*spotBalance.lpBalance.amount;
        }
    }

    function calcLpTokenValue(
        IFQuerier.SpotProduct memory spotProduct
    ...
    function findSpotProduct(uint32 productId, IFQuerier.SpotProduct[] memory spotProducts) internal pure returns(IFQuerier.SpotProduct memory spotProduct) {
        for (uint i=0; i< spotProducts.length; i++) {
            if(spotProducts[i].productId == productId) {
                spotProduct = spotProducts[i];
                break;
            }
        }
    }
