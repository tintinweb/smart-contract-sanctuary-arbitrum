    ...
    ReentrancyGuard
{
    ...
    mapping(uint8 => uint256) public currentMaxExposureAmount;
    mapping(uint8 => uint256) public currentBetAmount;
    mapping(uint8 => uint256) public futureBetAmount;

    mapping(uint8 => uint256) public epochForRecentExposure;
    ...
    function _executeRound(
        ...
        for (uint8 i = 0; i < length; i = i + 1) {
            ...
            if (genesisLockedOnce[timeframeId]) {
                ...
                _updateBettableAmount(timeframeId, timeframeIds, currentEpoch);
            }
        }
    }
    ...
    function _getBettableAmounts(
        ...
        if (maxMinuteExposure > 0) {
            ...
            maxMinuteExposure =
                (maxMinuteExposure * exposureBips) /
                config.FEE_BASE();

            // Total sum of weights
            uint256 weights;
            for (uint256 i; i < timeframeIds.length; i++) {
                weights += exposureBipsForTimeframe[timeframeIds[i]];
            }

            currentExposureAmount = weights > 0
                ? (maxMinuteExposure * exposureBipsForTimeframe[timeframeId]) /
                    weights
                : maxMinuteExposure;
            betAmount =
     ...
     */
    function _updateBettableAmount(
        uint8 timeframeId,
        uint8[] memory timeframeIds,
        ...
        (
            ...
            uint256 currentBet,
            uint256 futureBet
        ) = _getBettableAmounts(timeframeId, timeframeIds);
        // Calculate current exposure amount and bet amount, future bet amount for this timeframe.
        currentMaxExposureAmount[timeframeId] = currentExposure;
        currentBetAmount[timeframeId] = currentBet;
        futureBetAmount[timeframeId] = futureBet;

        epochForRecentExposure[timeframeId] = currentEpoch;
    }
    ...
    function _checkBetPool(uint8 timeframeId, uint256 epoch) private {
        ...
        if (currentMaxExposureAmount[timeframeId] < exposureAmount) {
            uint256 deltaExposure = exposureAmount -
                currentMaxExposureAmount[timeframeId];
    ...
    function openPosition(
        ...
        uint256 currentEpoch = getCurrentRoundId(timeframeId);
        if (currentEpoch > epochForRecentExposure[timeframeId]) {
            // this means current, future bet amount data are expired, and not up to date.
            uint8[] memory timeframeIds = getExecutableTimeframes();
            _updateBettableAmount(timeframeId, timeframeIds, currentEpoch);
        }

        uint256 maxBetAmount = epoch > currentEpoch
            ? futureBetAmount[timeframeId]
            : currentBetAmount[timeframeId];
    ...
    function _claim(address user, uint8 timeframeId, uint256 epoch) internal {
        // We use block time
        require(
    ...
    function getCurrentRoundId(
        uint8 timeframeId
    ) public view returns (uint256 roundFromBlockTime) {
    ...
    function setTimeframes(TimeFrame[] memory timeframes_) external onlyAdmin {
        ...
        for (uint256 i = 0; i < length; i = i + 1) {
            ...
            // In default, we spread exposure amount among all timeframes. Plain distribution
            exposureBipsForTimeframe[timeframes_[i].id] =
                config.FEE_BASE() /
                length;
        }
    }

    /// @dev set exposure bips for each timeframes
    function setExposureBips(uint256[] memory bips) external onlyAdmin {
        uint256 length = timeframes.length;
        require(bips.length == length, "INVALID_ARRAY_LENGTH");

        for (uint256 i = 0; i < length; i = i + 1) {
            require(bips[i] <= config.FEE_BASE(), "INVALID_BIPS");
            exposureBipsForTimeframe[timeframes[i].id] = bips[i];
        }
    }

    function getCurrentBettableAmount(
        uint8 timeframeId,
        uint256 epoch
    ) external view returns (uint256) {
        uint256 currentEpoch = getCurrentRoundId(timeframeId);

        if (epochForRecentExposure[timeframeId] == currentEpoch) {
            // data is recent
            if (epoch > currentEpoch) {
                return futureBetAmount[timeframeId];
            } else {
                return currentBetAmount[timeframeId];
            }
        } else {
            // data is expired, so we need to calculate new one
            uint8[] memory timeframeIds = getExecutableTimeframes();
