...
contract Pair {
    ...
    address internal constant PROTOCOL_ACCRUAL = address(0x1);
    ...
    event ClaimProtocol(address indexed recipient, uint amount0, uint amount1);
    ...
    function claimProtocolFees() external returns (uint claimed0, uint claimed1) {
        address recipient = PairFactory(factory).feeRecipient();
        require(msg.sender == recipient, "AccessControl");

        claimed0 = claimable0[PROTOCOL_ACCRUAL];
        claimed1 = claimable1[PROTOCOL_ACCRUAL];

        if (claimed0 > 0 || claimed1 > 0) {
            claimable0[PROTOCOL_ACCRUAL] = 0;
            claimable1[PROTOCOL_ACCRUAL] = 0;

            PairFees(fees).claimFeesFor(recipient, claimed0, claimed1);

            emit ClaimProtocol(recipient, claimed0, claimed1);
        }
    }
    ...
    function _update0(uint amount) internal {
        ...
        uint256 _ratio = (5 * amount * 1e18) / (6 * totalSupply); // 1e18 adjustment is removed during claim
        if (_ratio > 0) {
            index0 += _ratio;
        }
        claimable0[PROTOCOL_ACCRUAL] += amount / 6;
    ...
    function _update1(uint amount) internal {
        ...
        uint256 _ratio = (5 * amount * 1e18) / (6 * totalSupply);
        if (_ratio > 0) {
            index1 += _ratio;
        }
        claimable1[PROTOCOL_ACCRUAL] += amount / 6;
...
contract PairFactory {
    ...
    function acceptFeeRecipient() external {
        require(msg.sender == pendingFeeRecipient, 'not pending fee recipient');
