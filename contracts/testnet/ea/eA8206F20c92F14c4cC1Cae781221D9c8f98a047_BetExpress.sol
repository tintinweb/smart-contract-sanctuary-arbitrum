    ...
    IAffiliate
{
    ...
    function putBet(
        ...
        (SubBet[] memory subBets, uint64 minOdds) = abi.decode(
            betData.data,
            (SubBet[], uint64)
        );
        (
        ...
        if (expressOdds < minOdds) revert SmallOdds();
    ...
    /**
     * @notice Liquidity Pool: Resolve payout of the express bet with ID 'betId'.
     ...
     * @param subBets The SubBet array. See {IBetExpress-SubBet}.
     ...
     */
    function calcOdds(SubBet[] calldata subBets, uint128 amount)
        external
        view
     ...
     */
    function _calcOdds(SubBet[] memory subBets, uint128 amount)
        internal
        view
        returns (
            uint64[] memory conditionOdds,
