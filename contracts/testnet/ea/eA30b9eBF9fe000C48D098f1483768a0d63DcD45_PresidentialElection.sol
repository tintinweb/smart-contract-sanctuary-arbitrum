 ...
 */
contract PresidentialElection is ERC2771Context {
    // Errors Declaration
    ...
    IVotingStorage public VoterRegistration;

    // State Declaration
    uint private duration = block.timestamp + (24 * 3600);

    // Event Declaration
    event VoteCasted(string voterId, uint candidateId);
    event WinnerDeclared(uint winner);
    event RunOffRequired();

    mapping(uint => uint) public candidateTotalVotes;
     ...
     * @param i_voterRegistration Address of the Voter Registration contract.
     */
    constructor(
        address i_candidateRegistration,
        address i_voterRegistration,
        MinimalForwarder forwarder
    ) ERC2771Context(address(forwarder)) {
        CandidateRegistration = ICandidateRegistration(i_candidateRegistration);
        VoterRegistration = IVotingStorage(i_voterRegistration);
    }
    ...
    function add24Hours() external {
        ...
        duration = currentTime + (1 * 2000); // Adding 24 hours in seconds
    }
    ...
    function castVote(
        ...
        IVotingStorage.VoterInfo memory voterInfo = VoterRegistration
            .getVoterInfo();
        if (voterInfo.hasVoted) {
            revert PresidentialElection__AlreadyVoted();
        }

        VoterRegistration.updateVoterParam(_msgSender());
        CandidateRegistration.incrementVoteCount(candidateId, stateId);
    ...
    function searchVoter(string memory voterId) internal view returns (bool) {
        for (uint256 i = 0; i < VoterRegistration.getAllVoter().length; i++) {
            if (
                keccak256(bytes(VoterRegistration.getAllVoter()[i])) ==
                keccak256(bytes(voterId))
            ) {
                return true;
            }
        }
    ...
    function totalStateVotes(uint stateId) public view returns (uint) {
        require(stateId >= 1 && stateId <= 37, "Invalid state ID");

        uint totalVotes = 0;
        for (
            uint candidateId = 0;
            candidateId < CandidateRegistration.getApproveCandidateCount();
            candidateId++
        ) {
            totalVotes += CandidateRegistration
                .getCandidatePerState(stateId, candidateId)
                .voteCount;
        }
    ...
    function calculateWinner()
        external
        returns (uint highestVotes, uint candidateId)
    {
        if (block.timestamp < duration) {
        ...
        for (
            ...
            uint totalCandidateVotes = 0;
            for (uint stateId = 1; stateId <= 37; stateId++) {
                totalCandidateVotes += CandidateRegistration
                    .getCandidatePerState(stateId, candidateId)
                    .voteCount;
            }
        ...
        uint requiredStates = 25; // Two-thirds of 37 states

        uint statesMeetingQuota = 0;
        for (uint stateId = 1; stateId <= 37; stateId++) {
            uint winnerVotes = CandidateRegistration
                .getCandidatePerState(stateId, winnerCandidateId)
                .voteCount;
            ...
            if (winnerVotes >= totalStateVotes(stateId) / 4) {
                statesMeetingQuota++;
                console.log(
                    "Total Votes in %d is %d",
                    stateId,
                    totalStateVotes(stateId)
                );
            }
        }
        ...
        if (statesMeetingQuota >= requiredStates && highestVotes > 0) {
            ...
            console.log(
                "Highest Vote is %s, Candidate Id is %s",
                highestVotes,
                winnerCandidateId
            );
            console.log(
                CandidateRegistration
                    .getApprovedNomination(winnerCandidateId)
                    .candidate1
            );
        } else {
            // Conduct a run-off election within 21 days
            emit RunOffRequired();
            console.log("RunOff Required");
        }
    }
