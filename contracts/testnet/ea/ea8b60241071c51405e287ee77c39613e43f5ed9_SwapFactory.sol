...
import { SwapPair } from "./SwapPairUpgraded.sol";
...
import { SwapERC20Initializable } from "./SwapERC20Initializable.sol";
...
contract SwapPair is ISwapPair,Initializable, SwapERC20Initializable, ReentrancyGuardUpgradeable {
    ...
    function mint(address to) external override onlyRouter nonReentrant returns (uint liquidity) {
    ...
    // this low-level function should be called from a contract which performs important safety checks
    function burn(address to) external override onlyRouter nonReentrant returns (uint amount0, uint amount1) {
    ...
    // if fee is on, mint liquidity equivalent to 1/6th of the growth in sqrt(reserve0*reserve1)
    // lp tokens are minted to the feeTo address - this fee should only be a tax on liquidity deposited .
