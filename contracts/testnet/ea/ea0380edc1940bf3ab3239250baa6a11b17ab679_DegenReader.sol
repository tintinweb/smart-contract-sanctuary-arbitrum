...
contract DegenReader is IDegenReader {
  ...
  function netPnlOfPositionWithInterest(
    ...
    (pnlUsd_, isPnlPositive_) = _netPnlOfPosition(
      _positionKey,
      _assetPrice,
      _timestampAt
    );
  ...
  // function netPnlOfPositionWithInterestLastPrice(
  //   bytes32 _positionKey
  // ) external view returns (uint256 pnlUsd_, uint256 assetPrice_) {
  //   (assetPrice_, ) = priceManager.mostRecentPricePyth();
  //   pnlUsd_ = _netPnlOfPosition(_positionKey, assetPrice_, block.timestamp);
  // }
  ...
  function canPositionBeClosed(bytes32 _positionKey) external view returns (bool canBeClosed_) {
    (uint256 assetPrice_, ) = priceManager.getLastPriceUnsafe();
    PositionInfo memory positionInfo_ = degenMain.returnOpenPositionInfo(_positionKey);
    bool isUserPositionCloseAllowed_ = _isUserPositionCloseAllowed(positionInfo_.timestampOpened);
    (uint256 pnlUsd_, bool isPnlPositve_) = _netPnlOfPosition(
      _positionKey,
      assetPrice_,
      block.timestamp
    );
    bool isProfitLargeEnough;
    if (pnlUsd_ < 0) {
      isProfitLargeEnough = true;
      return (isUserPositionCloseAllowed_ && isProfitLargeEnough);
    } else {
      uint256 profitRatio_;
      // todo call fumction to return the profit percentage
      isProfitLargeEnough = _checkIfProfitIsSufficientToClose(profitRatio_);
      return (isUserPositionCloseAllowed_ && isProfitLargeEnough);
    }
  }

  function canPositionBeClosedUpdateData(
    bytes32 _positionKey,
    bytes memory _updateData
  ) external view returns (bool canBeClosed_) {
    uint256 assetPrice_ = _getPriceFromUpdateData(_updateData);
    PositionInfo memory positionInfo_ = degenMain.returnOpenPositionInfo(_positionKey);
    bool isUserPositionCloseAllowed_ = _isUserPositionCloseAllowed(positionInfo_.timestampOpened);
    (uint256 pnlUsd_, bool isPnlPositve_) = _netPnlOfPosition(
    ...
    if (pnlUsd_ < 0) {
      ...
      uint256 profitRatio_;
      // todo call function to return the profit percentage
      isProfitLargeEnough = _checkIfProfitIsSufficientToClose(profitRatio_);
      return (isUserPositionCloseAllowed_ && isProfitLargeEnough);
    }
  }

  function canPositionBeClosedWithAssetPrice(
    bytes32 _positionKey,
    uint256 _assetPrice
  ) external view returns (bool canBeClosed_) {
    PositionInfo memory positionInfo_ = degenMain.returnOpenPositionInfo(_positionKey);
  ...
  function _checkIfProfitIsSufficientToClose(
    uint256 _profitPercentage
  ) internal view returns (bool isProfitSufficient_) {
    isProfitSufficient_ = _profitPercentage >= poolManager.minProfitPercentageToClose();
  }
  ...
  function _netPnlOfPosition(
    ...
    uint256 _assetPrice,
    uint256 _timestampAt
  ) internal view returns (uint256 pnlUsd_, bool isPnlPositive_) {
    (pnlUsd_, isPnlPositive_) = degenMain.netPnlOfPosition(
      _positionKey,
      _assetPrice,
      _timestampAt
    );
