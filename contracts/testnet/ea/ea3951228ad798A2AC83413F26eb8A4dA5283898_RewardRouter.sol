...
contract RewardRouter is Initializable {
    ...
    event UserEvent(address indexed addr, string str, uint amount);
    ...
    function initialize(
        ...
        PEXStakingVar.unStakingLockDuration = 60 * 60 * 24 * 14;
        ...
        esPEXStakingVar.unStakingLockDuration = 60 * 60 * 24 * 14;
    }

    function requireGov(StakingLib.stakingVar storage self) internal view {
        require(msg.sender == self.storageT.gov(), "GOV_ONLY");
    }

    function getNewRewardRewardRouter(uint _newRewardAmount, uint _amount) internal view returns (uint){
        uint _allStaking = PEXStakingVar.allStaking + esPEXStakingVar.allStaking;
        require(_allStaking != 0, "allStaking equal 0");

        return _newRewardAmount.mulDiv(_amount, _allStaking);
    }
    ...
    function stakePEX(uint _amount) external {
        PEXStakingVar.stake(_amount);
        emit UserEvent(msg.sender, "stakePEX", _amount);
    }

    function unStakePEX(uint _amount) external {
        PEXStakingVar.unStake(_amount);
        emit UserEvent(msg.sender, "unStakePEX", _amount);
    }

    function withdrawPEX() external {
        PEXStakingVar.withdraw();
        emit UserEvent(msg.sender, "withdrawPEX", 0);
    }

    function claimPEX() external {
        PEXStakingVar.claim();
        emit UserEvent(msg.sender, "claimPEX", 0);
    }
    ...
    function stakeEsPEX(uint _amount) external {
        esPEXStakingVar.stake(_amount);
        emit UserEvent(msg.sender, "stakeEsPEX", _amount);
    }

    function unStakeEsPEX(uint _amount) external {
        esPEXStakingVar.unStake(_amount);
        emit UserEvent(msg.sender, "unStakeEsPEX", _amount);
    }

    function withdrawEsPEX() external {
        esPEXStakingVar.withdraw();
        emit UserEvent(msg.sender, "withdrawEsPEX", 0);
    }

    function claimEsPEX() external {
        esPEXStakingVar.claim();
        emit UserEvent(msg.sender, "claimEsPEX", 0);
    }
