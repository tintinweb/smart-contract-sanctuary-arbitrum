...
contract FundingRateArbitrage is ERC20, Ownable {
    ...
    function getIndex() public view returns (uint256) {
        if(this.totalSupply() == 0){
            ...
            return DecimalMath.decimalDiv(getNetValue().netValue, this.totalSupply());
        }
    }
    ...
    function requestWithdraw(
        ...
        _burn(msg.sender, earnUSDCAmount);
        WithdrawalRequests.push(
            WithdrawalRequest(earnUSDCAmount, msg.sender, false)
        );
    ...
    function convertToJUSD(uint256 earnUSDCAmount) external {
        uint256 JUSDAmount = earnUSDCAmount.decimalMul(getIndex());
        Converts[msg.sender].earnUSDCAmount += earnUSDCAmount;
        ...
        _transfer(msg.sender, address(this), earnUSDCAmount);
    }

    function convertToEarnUSDC(uint256 JUSDAmount) external {
        ...
        require(
            ...
            "do not have enough earnUSDC"
        );
        uint256 earnUSDCAmount = (convert.earnUSDCAmount * JUSDAmount) /
            convert.JUSDAmount;
        _transfer(address(this), msg.sender, earnUSDCAmount);
        Converts[msg.sender].earnUSDCAmount -= earnUSDCAmount;
        Converts[msg.sender].JUSDAmount -= JUSDAmount;
    }
}
