...
import {OkeytoTokenARB} from "./OkeytoTokenARB.sol";

contract ArbitrumExtension is OkeytoTokenARB, IArbToken {
    ...
    function initialize(
        ...
        __OkeytoToken_init(_name, _symbol, _decimals);
    }
...
// SPDX-License-Identifier: MIT
// Okeyto Game Token Contract

pragma solidity 0.8.18;

import {Initializable} from "./shared/Initializable.sol";
import {ERC20PermitUpgradeable} from "./shared/ERC20PermitUpgradeable.sol";
import {ERC20Upgradeable} from "./shared/ERC20Upgradeable.sol";
import {OwnableUpgradeable} from "./shared/OwnableUpgradeable.sol";
import {WithBlackList} from "./shared/WithBlackList.sol";

contract OkeytoTokenARB is
    Initializable,
    ERC20PermitUpgradeable,
    OwnableUpgradeable,
    WithBlackList
{
    uint8 private OKTTdecimals;
    ...
    function __OkeytoToken_init(
        ...
        __OkeytoToken_init_unchained(_decimals);
    }

    function __OkeytoToken_init_unchained(
        uint8 _decimals
    ) internal initializer {
        OKTTdecimals = _decimals;
    }

    /**
     * @dev return Diment decimals
     */
    ...
    /**
     * @dev transfer funds
     * @param recipient_ who will recive this tokens
    ...
    function transfer(
        ...
        require(!isBlocked[recipient_], "ERC20: recipient is in blacklist");

        return super.transfer(recipient_, amount_);
    }

    /**
     * @dev transfer funds via allowance
     * @param sender_ who will send funds
     * @param recipient_ who will recive this tokens
     * @param amount_ the amount to be transferred.
     */
    function transferFrom(
        ...
        require(!isBlocked[sender_], "ERC20: sender is in blacklist");
        require(!isBlocked[recipient_], "ERC20: recipient is in blacklist");
    ...
    /**
     * @dev is for easy multi transfer max 500 recipients allowed
     * @param recipients_ the recipients array to be transferred.
     * @param amounts_ the amount array to be transferred.
     */
    function multiTransfer(
        address[] memory recipients_,
        uint256[] memory amounts_
    ) external onlyNotBlacklisted {
        require(recipients_.length <= 500, "ERC20: max transfer limit 500");
        require(
            recipients_.length == amounts_.length,
            "ERC20: multiTransfer length mismatch"
        );
        ...
        require(
            _balance >= totalSend,
            "ERC20 : multiTransfer amount exceeds balance"
        );
    ...
    /**
     * @dev mint new tokens
     * @param destination_ who will recive this funds
     * @param amount_ The amount to be transferred.
     */
    function mint(address destination_, uint256 amount_) external onlyOwner {
        _mint(destination_, amount_);
        emit Mint(destination_, amount_);
    }

    /**
     * @dev burn owner wallet funds
     * @param amount_ amount for burn
     */
    ...
    /**
     * @dev burn blacklisted account funds
     * @param account_ user whom funds will burn
     */
    function destroyBlockedFunds(address account_) external onlyOwner {
        require(isBlocked[account_]);
...
import {ECDSAUpgradeable} from "../libs/ECDSAUpgradeable.sol";
import {Initializable} from "./Initializable.sol";
...
import {CountersUpgradeable} from "../libs/CountersUpgradeable.sol";
import {ECDSAUpgradeable} from "../libs/ECDSAUpgradeable.sol";
import {Initializable} from "./Initializable.sol";
...
contract WithBlackList is OwnableUpgradeable {
    /**
    ...
    function addToBlackList(address wallet) external onlyOwner {
        ...
        emit BlockWallet(wallet);
    }

    function removeFromBlackList(address wallet) external onlyOwner {
        isBlocked[wallet] = false;
        emit UnBlockWallet(wallet);
    }

    event BlockWallet(address indexed wallet);
    event UnBlockWallet(address indexed wallet);
}