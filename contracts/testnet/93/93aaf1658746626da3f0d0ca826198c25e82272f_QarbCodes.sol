...
contract QarbCodes is ERC721URIStorage, Ownable {
    ...
    uint public freeAmount   = 500;
    uint public freeCount    = 0;

    bytes private constant tokenDescription = "QaRb Codes on Arbitrum - Stored entirely on Arbitrum Blockchain";
    ...
    constructor(uint maxSupply_, string memory name_, string memory symbol_) ERC721(name_, symbol_) {
    ...
    function freeMint(string memory qrCodeText) external returns (uint256) {
        uint256 count = 1;

        require(freeMintAddresses[msg.sender] == false, "Free Mint already claimed");
...
//        require(msg.sender == tx.origin, "The minter is another contract");
    ...
    function mint(uint256 count, string memory qrCodeText) public payable  {
...
//        require(msg.sender == tx.origin, "The minter is another contract");
        ...
        for (uint256 i; i < count; ++i) { // Do this ++ unchecked?
...
//        bytes memory svg = bytes(tokenImage(tokenId));
        bytes memory svg = bytes(generateQRCode(_tokenQrCodeText[tokenId]));

//        return generateQRCode(_tokenQrCodeText[tokenId]);


    bytes memory title = abi.encodePacked("QaRbCode #", Strings.toString(tokenId));
...
//        return super.tokenURI(tokenId);
    ...
    function textAttributesAsJSON(uint256 tokenId) internal view returns (string memory json) {
    ...
    function generateQRCode(string memory handle) public pure returns (string memory) {
...
//        string memory QRCodeURI = generateQRURI(qrMatrix);
        return generateQRURIByteVersion(qrMatrix);
//        string memory QRCodeURI = generateQRURI(qrMatrix);
//        return QRCodeURI;
...
//        emit QRCodeURIGenerated(QRCodeURI);
    ...
    function generateQRURIByteVersion(QRMatrix memory _qrMatrix)
    public
    pure
    returns (string memory)
    {
        ...
        DynamicBuffer.appendSafe(svgBytes, '<svg viewBox="0 0 296 296" width="296" height="296" xmlns="http://www.w3.org/2000/svg"><style>.bg{fill:#FCF}.fg{fill:#000}</style><rect class="bg" x="0" y="0" width="296" height="296"></rect>');
