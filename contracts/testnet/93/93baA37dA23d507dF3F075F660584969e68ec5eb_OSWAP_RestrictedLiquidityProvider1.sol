...
contract OSWAP_RestrictedLiquidityProvider1 is IOSWAP_RestrictedLiquidityProvider1 {
    ...
    function addLiquidity(
        ...
        if (offerIndex == 0) {
            ...
            offerIndex = IOSWAP_RestrictedPair1(pair).createOrder(msg.sender, direction, allowAll, restrictedPrice, startDate, expire);
        } else {
        ...
        if (amountIn > 0) {
            ...
            IOSWAP_RestrictedPair1(pair).addLiquidity(direction, offerIndex);
        }
        ...
        returns (address pair, uint256 offerIndex) 
    {
        ...
        IOSWAP_RestrictedPair1(pair).setMultipleApprovedTraders(b, offerIndex, trader, allocation);
    }
    ...
    function removeLiquidity(
        ...
        IOSWAP_RestrictedPair1(pair).removeLiquidity(msg.sender, direction, offerIndex, amountOut, receivingOut);
    ...
    function removeAllLiquidity(
        ...
        address pair = pairFor(tokenA, tokenB, pairIndex);
        (uint256 amount0, uint256 amount1) = IOSWAP_RestrictedPair1(pair).removeAllLiquidity(msg.sender);
        // (uint256 amount0, uint256 amount1) = IOSWAP_RestrictedPair1(pair).removeAllLiquidity1D(msg.sender, false);
        // (uint256 amount2, uint256 amount3) = IOSWAP_RestrictedPair1(pair).removeAllLiquidity1D(msg.sender, true);
    ...
    function pairFor(address tokenA, address tokenB, uint256 index) internal view returns (address pair) {
        ...
        pair = address(uint256(keccak256(abi.encodePacked(
                ...
                /*restricted*/hex'c0d0e8b421201d3b1fb507267858ad5fc2f6fd3351546c6e4d87d48914106638' // restricted init code hash
            ))));
    }
