...
contract Mercenaries is WrappedCharacters {
    ...
    WrappedCharacters("Farmland Mercenaries", "MERCENARIES") {
        // Set the price to update traits
...
// ADMIN FUNCTIONS
        ...
        onlyAllowed
    {
        ...
        unchecked { wTokenID++; }
        // Store the wTokenID as a local variable
        ...
        underlyingToken[underlyingTokenHash].collectionAddress = address(this);
        // Add Token ID to the mapping
        underlyingToken[underlyingTokenHash].tokenID = tokenID;
        // Map the underlying token hash to the wrapped token id
        wrappedCharacter[tokenID] = underlyingTokenHash;
...
// VIEWS
        ...
        returns (string memory uri) 
    {
        ...
        if (collectionAddress == address(this)) {
            ...
            uint16 level = getLevel(wrappedTokenID);
        ...
        override(ERC721)
    {
        ...
        // Revert if any the mercenary is active
...
contract WrappedCharacters is CharacterActivity {

// CONSTRUCTOR

    constructor (string memory name_, string memory symbol_)
    CharacterActivity(name_, symbol_) {}
...
// FUNCTIONS
        ...
        isWrappable(collectionAddress)
    {
        ...
        unchecked { wTokenID++; }
        // Wrap the character
        _wrap(collectionAddress, tokenID, wTokenID);
        // Get characters stamina
        (uint256 stamina,,,,,,,,) = getStats(wTokenID);
        // Has this token been wrapped before
        if (stamina == 0) {
            // Then set the stats
           _storeStats(collectionAddress,tokenID);
        }
        // Mint a wrapped character
        _mint(_msgSender(),wTokenID);
    }
...
// CONSTRUCTOR
    ...
    constructor (string memory name_, string memory symbol_) 
    CharacterManager(name_, symbol_)
    {
        statsPerLevel = 3;
    }
...
// EXTERNAL FUNCTIONS
        ...
        onlyWrapped(wrappedTokenID)
    {
        ...
        bytes32 underlyingTokenHash = wrappedCharacter[wrappedTokenID];
        // Get current stat
        uint16 currentStat = stats[underlyingTokenHash][statIndex];
        // Set standard max stat
        uint16 maxStat = 99;
        if (statIndex == 5) {
            // Health has a different calculation for max stat
            maxStat = getMaxHealth(wrappedTokenID);                          
        } else if (statIndex == 6) {
            // Morale has a different calculation for max stat
            maxStat = getMaxMorale(wrappedTokenID);                          
        } else if (statIndex > 6) {
        ...
        if (currentStat + amount < maxStat + 1) {
            // Increase stat
            stats[underlyingTokenHash][statIndex] += amount;
        } else {
            // Set to max for the stat
            stats[underlyingTokenHash][statIndex] = maxStat;
        }
        // Write an event to the chain
        emit StatIncreased(_msgSender(), wrappedTokenID, amount, statIndex);
    }
        ...
        onlyWrapped(wrappedTokenID)
    {
        ...
        if (currentStat > amount) {
            ...
            stats[underlyingTokenHash][statIndex] = 1;
        }
        // Write an event to the chain
        emit StatDecreased(_msgSender(), wrappedTokenID, amount, statIndex);
    }
    ...
    /// @param amount to add
    function setStatTo(uint256 wrappedTokenID, uint16 amount, uint256 statIndex)
        external
        onlyAllowed
        onlyWrapped(wrappedTokenID)
    {
    ...
    function boostStat(uint256 wrappedTokenID, uint16 amount, uint256 statIndex)
        external
        onlyAllowed
        onlyWrapped(wrappedTokenID)
    {
        ...
        require(amount <= getStatBoostAvailable(wrappedTokenID), "This will exceed the available boosts for this character");
        // Increase the state variable tracking the boosts
        getStatBoosted[wrappedTokenID] += amount;
        // Increase requested Stat
        increaseStat(wrappedTokenID, amount ,statIndex);
    }
...
// INTERNAL FUNCTIONS
        ...
        isRegistered(collectionAddress)
    {
        ...
        if (characterCollections[collectionAddress].native) {
            ...
            randomNumbers = _getRandomNumbers(5, tokenID);
            // Set stat values
...
// ADMIN FUNCTIONS
    ...
    function updateStatsPerLevel(uint16 newStatsPerLevel)
        external
        onlyOwner
    {
        statsPerLevel = newStatsPerLevel;
    }
...
// VIEW FUNCTIONS
        ...
        )
    {
        ...
        if (total > 7){level = sqrt(experience).toUint16();}
    }  

    /// @dev Returns the wrapped characters level
    /// @param wrappedTokenID ID of the token
    function getLevel(uint256 wrappedTokenID)
        public
        view
        returns (
            uint16 level
        )
    {
        // Get the underlying token hash
        bytes32 underlyingTokenHash = wrappedCharacter[wrappedTokenID];
        // level is the square root of xp
        return sqrt(stats[underlyingTokenHash][7]).toUint16();
    }  
    ...
    {
        // Retrieve stats
        (uint16 stamina, uint16 strength,,,,,,,) = getStats(wrappedTokenID);
    ...
    {
        // Retrieve stats
        (,,,uint16 courage, uint16 intelligence,,,,) = getStats(wrappedTokenID);
        // Set Morale
        morale = (courage + intelligence) / 2 ;
...
contract CharacterManager is ERC721, ERC721Holder, ReentrancyGuard, Pausable, CollectionManager {
...
// FUNCTIONS
    ...
    {
        bytes32 underlyingTokenHash = hashUnderlyingToken(collectionAddress,tokenID);
        // If this character has been wrapped previously then
        if (!underlyingTokenExists(collectionAddress, tokenID)) {
            // Add Collection address to the mapping
            underlyingToken[underlyingTokenHash].collectionAddress = collectionAddress;
            // Add Token ID to the mapping
            underlyingToken[underlyingTokenHash].tokenID = tokenID;
        }
        // Map the underlying token hash to the wrapped token id
        wrappedCharacter[wrappedTokenID] = underlyingTokenHash;
        // Write an event
        emit Wrapped(_msgSender(), collectionAddress, block.number, tokenID, wrappedTokenID, underlyingTokenHash);                                     
    ...
    function _unwrap(uint256 wrappedTokenID)
        internal
    {
        // Get the underlying token details
        (address collectionAddress, uint256 tokenID) = getCharactersID(wrappedTokenID);
        // Check token underlyingTokenExists
        bool characterExists = underlyingTokenExists(collectionAddress, tokenID);
        require(characterExists, "There is no token to unwrap");
        // Delete mapping to underlying token hash
        delete wrappedCharacter[wrappedTokenID];
        // Write an event
        emit Unwrapped(_msgSender(), collectionAddress, block.number, tokenID, wrappedTokenID, hashUnderlyingToken(collectionAddress, tokenID));
        // Return Item to owner
        IERC721(collectionAddress).transferFrom(address(this), _msgSender(), tokenID);
    }
 // ADMIN FUNCTION
...
// VIEW FUNCTIONS
    ...
    {
        return (underlyingToken[wrappedCharacter[wrappedTokenID]].collectionAddress,
                underlyingToken[wrappedCharacter[wrappedTokenID]].tokenID);
    }
...
// EVENTS
    ...
    event ActivityStatusUpdated(address indexed account, uint256 wrappedTokenID, bool active);
    event ActivityStarted(address indexed account, uint256 wrappedTokenID, uint256 activityDuration, uint16 numberOfActivities, uint256 startBlock, uint256 endBlock); 
...
// EXTERNAL FUNCTIONS
    ...
    function updateActivityStatus(uint256 wrappedTokenID, bool active)
        external
        onlyAllowed
        onlyWrapped(wrappedTokenID)
    {
        // Write an event to the chain
        emit ActivityStatusUpdated(_msgSender(), wrappedTokenID, active);
        // Set active
        charactersActivity[wrappedTokenID].active = active;
        if (!active) {
            // Set block at which the activity completes
            charactersActivity[wrappedTokenID].completedBlock = block.number;
        }
    }
    ...
    function startActivity(uint256 wrappedTokenID, uint256 activityDuration, uint16 numberOfActivities, uint256 startBlock, uint256 endBlock)
        external 
        onlyAllowed
        onlyWrapped(wrappedTokenID)
    {
        ...
        // Write an event to the chain
        emit ActivityStarted(_msgSender(), wrappedTokenID, activityDuration, numberOfActivities, startBlock, endBlock);
        // Set active, duration, start & end block & number of activities
        charactersActivity[wrappedTokenID].active = true;                               
        charactersActivity[wrappedTokenID].activityDuration = activityDuration;
        charactersActivity[wrappedTokenID].startBlock = startBlock;
        charactersActivity[wrappedTokenID].endBlock = endBlock;
        charactersActivity[wrappedTokenID].numberOfActivities = numberOfActivities;
    }
...
//VIEWS
        ...
        )
    {
        ...
        Activity storage activity = charactersActivity[wrappedTokenID];
