...
import "../../interfaces/orders/IOrderDispatcher.sol";
...
abstract contract Collectible is ICollectible, ERC721, ERC721URIStorage, ERC721Enumerable, DAOAccessControlled {
    ...
    address daoToken; // ERC20 Token address

    // Area co-ordinates attached to this collectible
    Area area;
    ...
    constructor(
      ...
      DAOAccessControlled(IDAOAuthority(_authority)) {
        ...
        // Set the area attached to this collectible
        area = _area;
        ...
        _collectibleIds.increment(); // Start from 1 as id == 0 is a check for existence
    }

    /***************************************************************************
    // Mints a collectible NFT to patron when they meet the specified criteria
    // A mapping maps each Collectible NFT ID to its details
    *****************************************************************************/
    function _mint (
        address _patron,
        uint256 _expiry,
    ...
    ) internal virtual returns (uint256 newCollectibleId)
    {
        ...
        newCollectibleId = _collectibleIds.current();
        ...
        emit CollectibleMinted(newCollectibleId, _patron, _expiry, _transferable, dataURI);
    }

    // Activation/Deactivation of a Collectible NFT token
    ...
    /***************************************************************************************************
    // Called when the entity offering the collectible wishes to retire it.
    // The Collectible NFTs minted to patron stay with them as memorablia.
    // Any functionality on the collectible will be blocked forever
    // Retire collectibles are permanantly retired and can never be re-activated again
    ...
    ****************************************************************************************************/
    ...
    /*********************************************************************************************
    // Transfers reward tokens to the patrons wallet when they purchase drinks/products.
    // Internally also maintains and updates a tally around number of rewards held by a patron.
    // Should be called when the bartender serves an order and receives payment for the drink.
    **********************************************************************************************/
    ...
    /*********************************************************************************************
    // Burns reward tokens from patrons wallet when they redeem rewards for free drinks/products.
    // Internally also maintains and updates a tally around number of rewards held by a patron.
    // Should be called when the bartender serves an order in return for reward tokens as payment.
    **********************************************************************************************/
    ...
    /***************************************************************************************************
    // Credits visits to patrons passport
    // Used as a metric to determine eligibility for special NFT airdrops
    // Should be called by the mobile app whenever the patron visits the club
    // Only used for passport Collectible types
    ***************************************************************************************************/
    ...
    /***************************************************************************************************
    // Credits friend visits to patrons collectible
    // Used as a metric to determine eligibility for special NFT airdrops
    // Should be called by the mobile app whenever a friend of the patron 
    // holding collectible with given ID visits the club
    ***************************************************************************************************/
    ...
    function _updateDataURI(string memory _dataURI) internal virtual {
        ...
        emit DataURIUpdated(address(this), oldDataURI, dataURI);
    }

    function _calculateRewards(uint256 _price) internal virtual returns(uint256) {
        return (_price * (10**IDAOERC20(daoToken).decimals())) / 
                ITokenPriceCalculator(IOrderDispatcher(IDAOAuthority(authority).orderDispatcher()).priceCalculator()).pricePerMint();
    }
    ...
    // Returns collectible details for an nft tokenID
...
contract Offer is Collectible {
    ...
    event OrderRemoved(address, uint256);
    ...
    // Patron address => Offer NFT ID => boolean
    // Mapping to check if an NFT minted to a patron is served/redeemed
    ...
    constructor(
        ...
        address _daoToken
                    ...
                    _area, _authority, 
                    _offerDetails.dataURI,
                    _offerDetails.name,
                    _offerDetails.symbol,
                    _daoToken
                ) {
    ...
    // Called when a patron places an order using their mobile app
    function addPendingOrder(address _patron) external onlyForwarder {
        require((totalPurchases + pendingOrderCount + 1) <= offerDetails.maxPurchase, "MAX PURCHASE EXCEEDED");
        pendingOrderCount++;
        uint256 _offerId = this.mint(_patron, offerDetails.expiry, offerDetails.transferable);
        pendingOrders[_offerId] = true;
        emit OrderAdded(_patron, _offerId);
    }

    function completeOrder(uint256 _offerId) external onlyOrderDispatcher {
        pendingOrders[_offerId] = false;
        pendingOrderCount--;
        totalPurchases++;
        _setRedemption(_offerId);
        emit OrderRemoved(ownerOf(_offerId), _offerId);
    }

    function cancelOrder(address _patron, uint256 _offerId) external onlyBartender(entity) {
        pendingOrders[_offerId] = false;
        pendingOrderCount--;
        // TODO: Burn collectible
        emit OrderCancelled(_patron, _offerId);
    }
...
import "../collectibles/Offer.sol";
import "../access/DAOAccessControlled.sol";
import "../../interfaces/factories/IOfferFactory.sol";

contract OfferFactory is IOfferFactory, DAOAccessControlled {
    ...
    function removeOfferAddress(address _entity) external onlyGovernor  {        
        offerExists[_entity] =  false;
    }
}
...
interface IOrderDispatcher {
    ...
    event OrderDispatched(address, address, uint256);
