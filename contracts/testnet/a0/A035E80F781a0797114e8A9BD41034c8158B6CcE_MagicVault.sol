...
contract MagicVault is Initializable, ContextUpgradeable, ERC20Upgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function deposit(uint256 _amountUSDC) external nonReentrant {
        ...
        uint256 lp = provider.stake(address(this), _amountUSDC);

        // Mint MV-LP token
        _mint(msg.sender, lp);
        emit LPTokenMinted(msg.sender, lp);
    }
    ...
    function withdraw(uint256 _amountMVLpToken) external nonReentrant {
        ...
        uint256 usdcAmount = provider.unstake(msg.sender, _amountMVLpToken);
    ...
    function setProvider(address _providerAddress) external onlyOwner {
        ...
        if(totalSupply > 0){
            uint256 usdcAmount = oldProvider.migrate(_providerAddress, totalSupply);
