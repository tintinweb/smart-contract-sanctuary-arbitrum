...
contract DegenPoolManager is IDegenPoolManager, DegenPoolManagerSettings, ReentrancyGuard {
  ...
  function _processLiquidationClose(
    ...
    bool isInterestLiquidation_;

    if (_marginAmountUsd + _pnlUsd <= _interestFunding) {
  ...
  function processLiquidationClose(
    ...
    uint256 profitAmountInStable_ = (protocolProfitUsd_) /
      ((PRICE_PRECISION / 10 ** decimalsStableCoin));
    ...
    )
  {
    ...
    uint96 payoutMinusAllFees_ = _position.marginAmountUsd +
    ...
    uint256 payoutAmountInStable_ = (payoutMinusAllFees_) /
      ((PRICE_PRECISION / 10 ** decimalsStableCoin));
    ...
    )
  {
    ...
    if (_pnlUsd >= _marginAmountUsd) {
      ...
      unchecked {
        marginLeftUsd_ = 0;
        // note we don't add to totalTheoreticalBadDebtUsdPeriod here because this is not a real liquidation, so we do not know the value easily
      }
    } else {
      // the margin is not sufficient to cover the pnl and the funding fee, so the player is liquidated
    ...
    if (marginLeftUsd_ > 0) {
      uint256 payoutAmountInStable_ = (marginLeftUsd_) /
        ((PRICE_PRECISION / 10 ** decimalsStableCoin));
      uint256 marginAmountInStable_ = _marginAmountUsd /
        ((PRICE_PRECISION / 10 ** decimalsStableCoin));
      (marginAssetAmount_, feesPaid_) = _payout(
        _player,
        _marginAsset,
    ...
    closedPosition_.pnlUsd = uint96(_pnlUsd);
    closedPosition_.totalPayoutUsd = uint96(marginLeftUsd_);
  ...
  function _calculateProfitFee(
    ...
    uint256 priceMoveRange_ = maxPriceMove - minPriceMove;
    ...
    if (maxPriceMove > _priceMove && _priceMove >= minPriceMove) {
      ...
      uint256 feeDiff_;
      if (minFee > maxFeeForPositionSize_) {
        feeDiff_ = minFee - maxFeeForPositionSize_;
      } else {
        feeDiff_ = maxFeeForPositionSize_ - minFee;
      }

      // Calculate the fee using linear interpolation
      uint256 feePart1 = (_priceMove * feeDiff_) / priceMoveRange_;
      uint256 feePart2 = (minPriceMove * feeDiff_) / priceMoveRange_;

      // _priceMove >= minPriceMove so feePart1 >= feePart2 -> cannot underflow

      uint256 fee = maxFeeForPositionSize_ + shiftAmount_ - (feePart1 - feePart2);
      return fee;
    } else {
