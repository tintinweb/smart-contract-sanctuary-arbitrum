...
// Library imports
import { LibEmissionsUtils } from "../libraries/LibEmissionsUtils.sol";
...
interface IGameV2 {
    ...
    function coldStorageTreshold() external view returns (uint256);
}
...
import { IGameV2 } from "../interfaces/IGameV2.sol";
...
library LibEmissionsUtils {
    ...
    function claimEmissions(
        ...
        _verifyClaimProof(index, epoch, data, msg.sender, merkleProof);
     ...
     */
    function getClaimableMagicRNGEmissionsFor(address account, bytes calldata data) internal view returns (uint256) {
        uint256 cumulativeMagicRNGAmount = abi.decode(data, (uint256));
    ...
    function _verifyClaimProof(
        ...
        bytes32 node = keccak256(abi.encode(index, account, epoch, data));
        if (!MerkleProof.verify(merkleProof, gs().merkleRoot, node))
            revert Errors.InvalidProof(merkleProof, gs().merkleRoot, node);
    }
}
