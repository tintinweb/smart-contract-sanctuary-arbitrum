...
contract Traits is Ownable, ITraits {
    ...
    function setCZandSBF(string memory cz, string memory sbf) external onlyOwner {
        policeBody = cz;
        thiefBody = sbf;
    }
    ...
    function compileAttributes(uint256 tokenId) public view returns (string memory) {
        ...
        if (s.isChad) {
            traits = string(abi.encodePacked(
                    attributeForTypeAndValue("Type", "CZ"), ','
                ));
        } else {
            traits = string(abi.encodePacked(
                    attributeForTypeAndValue("Type", "SBF"), ',',
   ...
   */
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory metadata = string(abi.encodePacked(
                ...
                s.isChad ? 'CZ #' : 'SBF #',
                tokenId.toString(),
                '", "description": "CZ & SBF Game is a new generation play-to-earn NFT game on Arbitrum that incorporates probability-based derivatives alongside NFTs. Through a vast array of choices and decision-making possibilities, CZ & SBF Game promises to instil excitement and curiosity amongst the community as every individual adopts different strategies to do better than one another and to come out on top. The real question is, are you #TeamCZ or #TeamSBF? Choose wisely or watch the other get rich!", "image":"',
                s.isChad ? policeBody : thiefBody,
                '", "attributes":',
