...
error QualityWeightArgumentError();
error NoQualityWeight(uint256 term);
...
contract NFT is ERC721EnumerableUpgradeable, OwnableUpgradeable, IERC721ReceiverUpgradeable {
    ...
    uint256 public clientStartTokenId; //minttokenId
    ...
    {
        if(_msgSender() != clientAddress)
        {
            revert NotClient(_msgSender());
        }
        _;
    }
    ...
    function setClientInfo(uint256 _startTokenId, address _clientAddress) public onlyOwner
    {
        clientStartTokenId = _startTokenId;
    ...
    function mintByClient(address _to) external payable checkClient
    {
        _safeMint(_to, clientStartTokenId);
        clientStartTokenId++;
    }
