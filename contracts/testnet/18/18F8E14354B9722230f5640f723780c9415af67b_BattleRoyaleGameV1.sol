...
contract BattleRoyaleGameV1 is IBattleRoyaleGameV1, Pausable, Ownable, ERC721Holder, ReentrancyGuard  {
    ...
    // when one player is out of shrink circle, and does nothing for a while. anyone can forcibly remove the player and get token reward
    function forceRemovePlayers(address[] calldata playersAddress) external nonReentrant checkEndGame {
        require(playersAddress.length > 0);
        for (uint i = 0; i < playersAddress.length; i++){
            ...
            GameBasic.killPlayer(players, playersData, _tokensOnGround, tilemap, state, GameBasic.KillParams({
                nft: nft,
                mapSeed: mapSeed,
                round: state.round,
                config: config,
    ...
    function _afterPlayerAction(uint damage) internal checkEndGame {
        ...
        if (playerData != 0) {
            ...
            if (damage > 0) {
                GameBasic.applyDamage(players, playersData, _tokensOnGround, tilemap, state, GameBasic.ApplyDamageParams({
                    nft: nft,
                    mapSeed: mapSeed,
                    config: config,
                    player: player,
    ...
    function actionMove(uint[] calldata path) external playerAction {
        GameAction.move(playersData, tilemap, state,
    ...
    function actionMoveWithBoots(uint tokenIndex, uint[] calldata path) external playerAction {
        GameAction.moveWithBoots(playersData, tilemap, state, GameAction.BootsParams({
                config: config,
                state: state,
                nft: nft,
                mapSeed: mapSeed,
    ...
    function actionBomb(uint tokenIndex, uint targetPos, address[] memory targets, bool[] memory targetsIsPlayer) external playerAction {
        GameAction.bomb(players, playersData, tilemap, _tokensOnGround, state, GameAction.BombParams({
            mapSeed: mapSeed,
            config: config,
            nft: nft,
            player: signer2player[msg.sender],
    ...
    function actionShoot(uint tokenIndex, address target, bool targetIsPlayer) external playerAction {
        GameAction.shoot(players, playersData, _tokensOnGround, tilemap, state, GameAction.ShootParams({
            nft: nft,
            mapSeed: mapSeed,
            config: config,
            tokenIndex: tokenIndex,
    ...
    function actionEat(uint tokenIndex) external playerAction {
        GameAction.eat(playersData, state, GameAction.EatParams({
            round: state.round,
            nft: nft,
            player: signer2player[msg.sender],
            tokenIndex: tokenIndex
    ...
    function actionPick(uint[] calldata pos, uint[][] calldata tokensIdAndIndex) external playerAction {
        GameAction.pick(playersData, _tokensOnGround, tilemap, state,
    ...
    function actionDrop(uint pos, uint[] calldata tokenIndexes) external playerAction {
        GameAction.drop(playersData, _tokensOnGround, state, GameAction.DropParams({
            round: state.round,
            player: signer2player[msg.sender],
            pos: pos,
    ...
    function gameTick() public view returns(uint) {
        return state.status == GameConstants.GAME_STATUS_RUNNING ? GameView.gameTick(state.startTime, config.tickTime) : 0;
    }
    ...
    function isOutOfShrinkCircle(address playerAddress) public view returns(bool) {
        return state.status == GameConstants.GAME_STATUS_RUNNING && GameView.isOutOfShrinkCircle(playersData, tilemap, mapSeed, state, config, playerAddress);
    }

    function canForceRemovePlayer(address playerAddress) public view returns(bool) {
        return state.status == GameConstants.GAME_STATUS_RUNNING && isOutOfShrinkCircle(playerAddress) && gameTick() - PlayerDataHelper.getTick(playersData[playerAddress]) > config.forceRemoveTicks;
    }
