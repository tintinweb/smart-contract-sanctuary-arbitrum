...
contract ToadzMinter is Initializable, ToadzMinterContracts {
    ...
    whenNotPaused
    {
        ...
        require(
            !claimed[_proofHash].claimedAll,
            "ToadzMinter: Already claimed all Toadz"
        );

        if(claimed[_proofHash].leftToClaim == 0) {
            claimed[_proofHash].leftToClaim = _amount;
        }

        uint256 _leftToClaim = claimed[_proofHash].leftToClaim;
        ...
        uint256 _batchSize = _leftToClaim > maxBatchSize ? maxBatchSize : _leftToClaim;

        for(uint256 i = 0; i < _batchSize; i++) {
            toadz.mint(msg.sender);
        }

        claimed[_proofHash].leftToClaim -= _batchSize;

        if(claimed[_proofHash].leftToClaim == 0) {
            claimed[_proofHash].claimedAll = true;
        }
    }

    function leftToClaim(
        bytes32[] memory _proof,
        uint256 _amount)
    external
    view
    returns(uint256)
    {
        ...
        if(claimed[_proofHash].claimedAll) {
            return 0;
        }
        ...
        if(claimed[_proofHash].leftToClaim == 0) {
            ...
            return claimed[_proofHash].leftToClaim;
        }
    }

    function result(uint256 _amount) external view returns(bytes32) {
        return keccak256(abi.encodePacked(msg.sender, _amount));
    }
}
...
abstract contract ToadzMinterContracts is Initializable, ToadzMinterState {
    ...
    modifier contractsAreSet() {
        require(address(toadz) != address(0), "Contracts aren't set");
...
interface IToadz is IERC721EnumerableUpgradeable {
