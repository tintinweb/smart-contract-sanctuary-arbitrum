...
// File: contracts/Adrenalinka.sol
...
contract Adrenalin is ERC721Enumerable, Ownable, ERC2981 {
    ...
    uint256 private amountToReserve = 0;
    ...
    constructor() ERC721("Adrenalin", "ADF") {
       }
    ...
    function teamMint(uint256 amount, address to) external onlyOwner {
        ...
        for (uint256 i = 1; i <= amount; ) {
            uint256 tokenId;
            if (reservedMinted < amountToReserve) {
                reservedMinted += 1;
                tokenId = reservedMinted;
            } else {
    ...
    function mint(uint256 amount) external payable  {
        ...
        bool whitelisted = whitelist[_msgSender()] >= amount;
        if (whitelisted) {
            whitelist[_msgSender()] -= amount;
        }

        uint256 price_ = whitelisted ? whitelistPrice : price;

        uint256 totalPrice = price_ * amount;
        uint currentSupply = totalSupply();

        if (
            currentSupply + (amountToReserve - reservedMinted) + amount >
            maxSupply
        ) revert OverMint();
        if (currentSupply + amount > maxSupply) revert OverMint();
    ...
    function randomTokenId() internal returns (uint256) {
        uint256 maxIndex = maxSupply -
            totalSupply() -
            (amountToReserve - reservedMinted);
        ...
        return value + amountToReserve + 1;
    }
