...
contract BossGame {
    ...
    function finishGame(uint256 _randomNumber) external onlyInGameState(GameState.IN_PROGRESS) onlyFactory {
        require(block.timestamp > endTime, "not yet");

        if (_randomNumber <= calculateWinThreshold(totalDeposits, bossHp)) {
...
contract BossGameFactory is Ownable {
    ...
    function finishGame(uint256 _gameId) external {
        ...
        uint256 requestId = IRandomizer(randomizer).request(80000);
