...
contract IndexAnatomy is IIndexAnatomyClient, IIndexAnatomyViewer, IIndexAnatomyManager, IIndexAnatomyErrors, Owned {
    ...
    /// @inheritdoc IIndexAnatomyViewer
    uint16 public override mintFee;
    /// @inheritdoc IIndexAnatomyViewer
    uint16 public override burnFee;
    /// @inheritdoc IIndexAnatomyViewer
    uint256 public override scaledPerSecondeRateAUM;
    ...
    constructor(
        ...
        mintFee = _mintingFee;
        burnFee = _burningFee;
        scaledPerSecondeRateAUM = _perSecondRateAUM;
    ...
    function updateFees(uint16 _mintingFee, uint16 _burningFee, uint256 _perSecondRateAUM)
        external
        override
        onlyOwner
    {
    ...
    function redeem(
        ...
        uint128 feeAUM = AUMLib.calculateFee(
            scaledPerSecondeRateAUM, state.totalSupply, state.lastTransferTimestamp, block.timestamp
        );

        {
            uint16 _burnFee = burnFee;
            fee = ((indexShares * _burnFee) / (BP.DECIMAL_FACTOR + _burnFee)) + feeAUM;
        }

        uint256 indexBurnt;
        // TODO: uncomment next line
        // (indexBurnt, reserveBurnt) = _burnReserve(_convertToAssets(indexShares, state.totalSupply + feeAUM, _totalAssets(state.reserve, subIndexes)), state.reserve, indexShares, receiver);
        if (indexShares > indexBurnt) {
            uint256 sharesToBurn = indexShares - indexBurnt;
            uint256 reducedTotalSupply = state.totalSupply + feeAUM - indexBurnt;

            uint256 length = subIndexes.length;
            subIndexBurnBalances = new uint32[](length);

            for (uint256 i; i < length;) {
                subIndexBurnBalances[i] =
                    _subIndexBalances[i].mulDivDown(sharesToBurn, reducedTotalSupply).safeCastTo32();

                _subIndexBalances[i] -= subIndexBurnBalances[i];
    ...
    function deposit(
        ...
        uint16 _mintFee = mintFee;
        fee = (totalShares * _mintFee) / (BP.DECIMAL_FACTOR + _mintFee);
        ...
        returns (uint128 shares)
    {
        ...
        uint16 _mintFee = mintFee;
        uint128 fee = (totalShares * _mintFee) / (BP.DECIMAL_FACTOR + _mintFee);
        shares = totalShares - fee;
    }
        ...
        returns (IIndexViewer.RedeemInfo memory info)
    {
        info.feeAUM = AUMLib.calculateFee(
            scaledPerSecondeRateAUM, state.totalSupply, state.lastTransferTimestamp, executionTimestamp
        );
        uint16 _burnFee = burnFee;
        uint128 fee = (indexShares * _burnFee) / (BP.DECIMAL_FACTOR + _burnFee);
        uint256 assets =
...
interface IIndexAnatomyManager {
    ...
    /// @param _burningFee Burning fee
    /// @param _perSecondRateAUM AUM scaled per second rate
    function updateFees(uint16 _mintingFee, uint16 _burningFee, uint256 _perSecondRateAUM) external;
...
interface IIndexAnatomyViewer {
    ...
    function mintFee() external returns (uint16);

    /// @notice Burning fee in basis points
    function burnFee() external returns (uint16);

    /// @notice AUM fee in scaled per second rate
    function scaledPerSecondeRateAUM() external returns (uint256);
