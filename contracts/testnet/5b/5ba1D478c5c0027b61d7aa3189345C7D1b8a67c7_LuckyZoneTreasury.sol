...
import "./interfaces/ILuckyZoneTreasury.sol";
import "./interfaces/ILuckyGames.sol";
import "./interfaces/ISushiswapRouter.sol";

contract LuckyZoneTreasury is Ownable, ILuckyZoneTreasury, ReentrancyGuard {
    /// TOKEN VARIABLES
    ...
    modifier onlyGame() {
        require(
            isGame[msg.sender],
            "LuckyZoneTreasury: only games can call this function"
        );
        _;
    }
    ...
    modifier onlyPlayableToken(address token) {
        require(
            ...
            "LuckyZoneTreasury: token is not playable"
        );
        _;
    }
    ...
    function addGame(address game) external onlyOwner {
        require(!isGame[game], "LuckyZoneTreasury: game is already playable");
        isGame[game] = true;
        emit GameAdded(game);
    }
    ...
    function removeGame(address game) external onlyOwner {
        require(isGame[game], "LuckyZoneTreasury: game is not playable");
        isGame[game] = false;
    ...
    function gameDeposit(
        ...
        require(success, "LuckyZoneTreasury: transfer failed");
    ...
    function gameWithdraw(
        ...
        require(
            ...
            "LuckyZoneTreasury: not enough unclaimed rewards"
        );
    ...
    function gameResult(
        ...
        emit RewardsClaimed(to, token, amount);
    }
    ...
    function nonceUnlock(
        ...
        require(
            ...
            "LuckyZoneTreasury: nonce not locked"
        );
    ...
    function withdraw(
        ...
        require(amount > 0, "LuckyZoneTreasury: amount must be greater than 0");
        uint balance = IERC20(token).balanceOf(address(this));
        require(
            balance - unclaimedRewards[token] >= amount,
            "LuckyZoneTreasury: not enough balance"
        );
        bool success = IERC20(token).transfer(msg.sender, amount);
        require(success, "LuckyZoneTreasury: transfer failed");
