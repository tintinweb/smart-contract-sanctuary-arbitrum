...
interface ITokenTraitsProvider {
	...
	function getTraitBytesValue(address tokenAddress, uint256 tokenId, uint256 traitId) external view returns (bytes memory);

	function getTraitMetaId(
	...
	// API - MUTATE
	// ====================================================
	function setStringTrait(address tokenAddress, uint256 tokenId, uint256 traitId, string memory metaId, string memory value) external;
	...
	function setUintTrait(address tokenAddress, uint256 tokenId, uint256 traitId, string memory metaId, uint256 value) external;
	...
	function setIntTrait(address tokenAddress, uint256 tokenId, uint256 traitId, string memory metaId, int256 value) external;
	...
	function setBoolTrait(address tokenAddress, uint256 tokenId, uint256 traitId, string memory metaId, bool value) external;
 ...
 * */
abstract contract ERC2771RecipientUpgadeable is Initializable, IERC2771Recipient {
...
interface IImmortals
{
	...
	function lockTokens(uint256[] memory tokenIds) external;
	...
	function getTokenLockedState(uint256 tokenId) external view returns (bool isLocked, uint256 lockDuration);
...
import { ERC2771RecipientUpgadeable } from "../delegation-registry/ERC2771RecipientUpgradeable.sol";
...
contract QuestsManager is IQuestsManager, Initializable, AccessControlUpgradeable, UUPSUpgradeable, ERC2771RecipientUpgadeable {
	// ==================================================
	// =================== STATEV1 ======================
	// ==================================================
	...
	modifier onlyQuestingEnabled() {
		require(
			address(sideQuestsV1) != address(0) && address(campaignQuestsV1) != address(0),
			"Quest contracts not set"
		);
		require(address(backpackItems) != address(0), "BackpackItems contract not set");
		require(address(campaignProgression) != address(0), "CampaignProgression contract not set");
		require(address(immortal) != address(0), "Immortal contract not set");
		require(address(traitsProvider) != address(0), "TraitsProvider contract not set");
		_;
	}
	...
	function startQuest(
		...
		bool isCampaign,
		bool fromDelegate
	) external onlyQuestingEnabled {
		address user = _deligatedMsgSender(fromDelegate);
		if (user == address(0)) {
		...
		require(traits.traitType.length > 0, "Quest traits not set");
		...
		// check if the player owns the required hero tokens and they are not locked - scoped to reduce stack size
		{
			...
			for (uint256 i = 0; i < heroTokens.length; i++) {
				...
				(bool lockedState, ) = immortal.getTokenLockedState(heroTokens[i]);
		...
		// check if the heroes have the required traits - scoped to reduce stack size
		{
				...
				for (uint256 i = 0; i < heroTokens.length; i++) {
					...
					for (uint256 j = 0; j < traits.traitType[i].length; j++) {
						if (compoundTraits[traits.traitType[i][j]].length > 0) {
							...
							{
								ITokenTraitsProvider.Behavior behavior1;
								ITokenTraitsProvider.Behavior behavior2;
								...
								behavior1 = traitMetadata1.behavior;
								behavior2 = traitMetadata2.behavior;
								dataType1 = traitMetadata1.dataType;
								dataType2 = traitMetadata2.dataType;

								if (
									behavior1 == ITokenTraitsProvider.Behavior.NOT_SET ||
									behavior2 == ITokenTraitsProvider.Behavior.NOT_SET
								) {
									continue;
								}
							}
...

			{
				...
				immortal.lockTokens(heroTokens, quest.duration);
			}
			...
			unchecked {
				...
				for (uint256 i = 0; i < rewards.length; i++) {
					...
					uint256 percentChance =
						(((userTotalPoints - rewards[i].minThreshold) * BASE_DIVDER) * 100) /
	...
	function endQuest(string calldata questId, bool fromDelegate) external onlyQuestingEnabled {
		address user = _deligatedMsgSender(fromDelegate);

		require(inProgressQuests[user][questId].startTime != 0, "Quest not started");
		require(inProgressQuests[user][questId].endTime < block.timestamp, "Quest not ended");
		...
		// calculate the chance of getting a reward & mint the reward
		...
		// unlock the hero tokens
		for (uint256 i = 0; i < questState.heroTokens.length; i++) {
			immortal.unlockToken(questState.heroTokens[i]);
		}
		...
		// remove the quest from inProgress quests
	...
	function addCompoundTrait(string memory traitId, string[] memory traitTypes) public onlyRole(QUEST_MANAGER_ROLE) {
		require(bytes(traitId).length > 0, "traitId cannot be empty");
	...
	function _deligatedMsgSender(bool fromDelegate) internal view returns (address) {
		if (fromDelegate) {
	...
	function _msgSender() internal view override(ContextUpgradeable, ERC2771RecipientUpgadeable) returns (address ret) {
		if (msg.data.length >= 20 && isTrustedForwarder(msg.sender)) {
			assembly {
				ret := shr(96, calldataload(sub(calldatasize(), 20)))
			}
		...
		// get EOA for sender if any
		if (address(delegationRegistry) != address(0)) {
			address eoa = delegationRegistry.getEoaForDelegateWallet(ret);
			if (eoa != address(0)) {
				ret = eoa;
			}
		}
	...
	function _msgData()
		internal
		view
		override(ContextUpgradeable, ERC2771RecipientUpgadeable)
