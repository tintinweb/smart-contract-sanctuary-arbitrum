...
contract SaleERC1155 is Pausable, Initializable, ERC1155Holder {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public nftPerPayToken;  // round id => nft id => amount to give per one pay token
    uint256 public totalPayTokenCollected;
    uint256 public totalPayTokenWithdrawed;
    ...
    event EmergencyWithdrawed(address indexed user, uint256 indexed nftId, uint256 amount);
    event WithdrawedRaised(uint256 amount, uint256 actualAmount, uint256 fee);
    event EmergencyWithdrawedRaised(uint256 amount);
    ...
    function withdraw(uint256 amount, uint256 nftId) public onlySaleOwner {
        ...
        require(projectTokenBalance[nftId] >= amount, "Not enough service balance");
    ...
    function emergencyWithdraw(uint256 amount, uint256 nftId) public onlyRaiseAdmin ifProjectUnhealthy {
        require(projectTokenBalance[nftId] >= amount, "Not enough service balance");

        projectTokenBalance[nftId] -= amount;
        projectToken.safeTransferFrom(address(this), msg.sender, nftId, amount, "");

        emit EmergencyWithdrawed(msg.sender, nftId, amount);
    }
    ...
    function createRound (
        ...
        uint256[] memory nftAmountPerPayToken,  // How much *nft* to give for one *sale token*
        bool isFinal,
        ...
        public onlyRaiseAdmin 
    {
        ...
        require(nftIds.length == nftAmountPerPayToken.length, "Rate arrays sizes are different");
        ...
        for(uint256 i = 0; i < nftIds.length; i++) {
            nftPerPayToken[roundsLength][nftIds[i]] = nftAmountPerPayToken[i];
        }
    ...
    function buy(uint256 payTokenAmount, uint256 nftId) public whenNotPaused onlyHealthy {
        Round memory ongoingRound = getOngoingRound();

        uint256 projectTokenAmount = payTokenAmount * nftPerPayToken[ongoingRound.id][nftId];
    ...
    function claim(uint256 nftId) public whenNotPaused {
        ...
        uint256 withdrawedProjectToken = claimedProjectTokenByUser[msg.sender][nftId];

        require(boughtProjectToken > withdrawedProjectToken, "All the bought tokens claimed");

        uint256 percentsWithdrawn = withdrawedProjectToken * 100 / boughtProjectToken;
    ...
    function withdrawRaised() public onlySaleOwner onlyHealthy {
        uint256 tokenCollected = totalPayTokenCollected;
        uint256 tokenWithdrawed = totalPayTokenWithdrawed;
        ...
        totalPayTokenWithdrawed += amountToWithdrawNow;
        ...
        emit WithdrawedRaised(amountToWithdrawNow, amountToWithdrawNow - fee, fee);
    }

    function emergencyWithdrawRaised() public onlyRaiseAdmin ifProjectUnhealthy {        
        uint256 amountToWithdraw = totalPayTokenCollected - totalPayTokenWithdrawed;

        require(amountToWithdraw > 0, "Nothing to withdraw");

        totalPayTokenWithdrawed += amountToWithdraw;
        payToken.safeTransfer(msg.sender, amountToWithdraw);

        emit EmergencyWithdrawedRaised(amountToWithdraw);
    }
    ...
    function refund() public ifProjectUnhealthy {
        ...
        totalPayTokenWithdrawed += amountToRefund;
    ...
    function setIsUnhealthy(bool isUnhealthy_) public onlyRaiseAdmin {
        ...
        if (isUnhealthy)
            sumToRefundIfUnhealthy = totalPayTokenCollected - totalPayTokenWithdrawed;
    ...
    function canClaim(address user, uint256 nftId) public view returns (bool) {
        ...
        uint256 withdrawedProjectToken = claimedProjectTokenByUser[user][nftId];
