...
contract Dice is Common {
    ...
    // uint256 stopGain,
    // uint256 stopLoss
    function Dice_Play() external payable nonReentrant {
        uint256 wager = 100;
        uint32 multiplier = 10421;
        address tokenAddress = address(0x4ABFDf8636242086812A1CA085ceb027d20f2c86);
        bool isOver = true;
        uint32 numBets = 1;
        uint256 stopGain = 0;
        uint256 stopLoss = 0;
    ...
    event DebugFullfill(string message);
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal {
        ...
        emit DebugFullfill(
            string.concat(
                'randomWords: ',
                Strings.toString(randomWords[0]),
                ' number of bets ',
                Strings.toString(game.numBets)
            )
        );
        ...
        for (i = 0; i < game.numBets; i++) {
            ...
            emit DebugFullfill(
                string.concat(
                    'diceOutcomes: ',
                    Strings.toString(diceOutcomes[i]),
                    ' numberToRollOver ',
                    Strings.toString(numberToRollOver),
                    ' winChance ',
                    Strings.toString(winChance),
                    ' gamePayout ',
                    Strings.toString(gamePayout)
                )
            );
...
contract Common is ReentrancyGuard {
    ...
    function _transferWager(
        ...
        if (tokenAddress == address(0)) {
            ...
            if (msg.value < VRFfee) {
                revert('Not enough value to pay for VRF');
            }
    ...
    function _refundExcessValue(uint256 refund) internal {
        ...
        if (!success) {
            revert(
                string.concat(
                    "Couldn't refund excess value msg.value",
                    Strings.toString(msg.value),
                    ' and refund i need to send',
                    Strings.toString(refund),
                    ' and balance ',
