    ...
    UUPSUpgradeable
{
    ...
    modifier convertEnabled() {
        require(isConvertEnabled, "Convert disabled");
        _;
    }

    modifier claimEnabled() {
        require(isClaimEnabled, "Claim disabled");
        _;
    }
    ...
    function startConvert() public onlyOwner {
        require(!isConvertEnabled, "Cannot update start convert time");
    ...
    function stopConvert() public onlyOwner {
        require(isConvertEnabled, "Cannot update stop convert time");
    ...
    function startClaim() public onlyOwner {
        require(!isClaimEnabled, "Cannot update start claim time");
        isClaimEnabled = true;
    }
    ...
    function stopClaim() public onlyOwner {
        require(isClaimEnabled, "Cannot update stop claim time");
    ...
    function getClaimable(address claimAddress) public view returns (uint256) {
        ...
        uint progressTimestamp = block.timestamp -
            convertData[claimAddress].startClaimTimestamp;
    ...
    function claimRD() public claimEnabled nonReentrant {
        require(convertData[msg.sender].startClaimTimestamp > 0, "Cannot claim, convert first");
