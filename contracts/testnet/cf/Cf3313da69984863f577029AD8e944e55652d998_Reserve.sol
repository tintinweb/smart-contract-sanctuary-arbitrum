...
struct DappInfo {
    ...
    uint256 groupAmount;
    uint256 inviteCounts;
    uint256 validInviteCounts;
...
contract Reserve is Ownable {
    ...
    mapping (address => uint256) private _inviteCounts;
    mapping (address => uint256) private _validInviteCounts;
    ...
    function deposit(uint256 amount) external {
        ...
        _updateValidGroup(account, amount, 8);
    ...
    function _updateValidGroup(address account, uint256 amount, uint8 deep) internal {
        if (deep == 0) return;
        ...
        if (deep == 8) {
            if (!_userJoined[ref]) _validInviteCounts[ref] += 1;
            _inviteAmount[ref] += amount;
        }
        if (!_userJoined[ref]) _validGroupCounts[ref] += 1;
        _groupAmount[ref] += amount;

        _updateValidGroup(ref, amount, deep - 1);
    }
}