...
contract HelixDealTokens is IHelixDealTokens, ERC721PresetMinterPauserdUpgradeSafe {  
  ...
  */
  ...
  function updateDealPrincipalInvested(
    ...
    DealInfo storage deal = deals[dealAddress];
    deal.totalPrincipalInvested = deal.totalPrincipalInvested.add(principalAmount);

    emit DealPrincipaInvestedUpdated(dealAddress, principalAmount, deal.totalPrincipalInvested);
  }
  ...
  function bookRepayment(
    ...
    require(token.deal != address(0), "Invalid tokenId");
    ...
    require(
      ...
      "Cannot repay more than principal-deposited amount for token"
    );
  ...
  function updateDealPrincipalRepaid(
    ...
    DealInfo storage deal = deals[dealAddress];
    deal.totalPrincipalRepaid = deal.totalPrincipalRepaid.add(principalRepaid);
    
    require(deal.totalPrincipalRepaid <= deal.totalPrincipalInvested, "Cannot repay more than we minted");

    emit DealPrincipaRepaidUpdated(dealAddress, principalRepaid, deal.totalPrincipalRepaid);
  }

  /** @notice reduce a given deal token's principalAmount and principalRedeemed by a specified amount
   *  @dev this function is only intended for use as part of the v2.6.0 upgrade
   *    to rectify a bug that allowed users to create a Token that had a
   *    larger amount of principal than they actually made available to the
   *    borrower.  This bug is fixed in v2.6.0 but still requires past deal tokens
  ...
  function reducePrincipalAmount(uint256 tokenId, uint256 amount) external onlyAdmin {
    ...
    tokenInfo.principalInvested = tokenInfo.principalInvested.sub(amount);
    tokenInfo.principalRepaid = tokenInfo.principalRepaid.sub(amount);
  }
  ...
  function getDealInfo(address deal) external view override returns (DealInfo memory) {
    return deals[deal];
  }
  ...
  function dealTVL(address dealAddress) public view override returns(uint256) {
    DealInfo storage deal = deals[dealAddress];
    return deal.totalPrincipalInvested - deal.totalPrincipalRepaid; 
  }
