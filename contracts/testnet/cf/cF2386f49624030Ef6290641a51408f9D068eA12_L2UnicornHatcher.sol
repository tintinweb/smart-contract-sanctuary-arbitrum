...
contract L2UnicornHatcher is L2UnicornHatcherPermit, BaseGame, BaseL2, BaseL2Unicorn, BaseL2UnicornRule, BaseRecipient, Pausable {
    ...
    function hatchE0(uint256 gasPrice_, uint256 callbackGasLimit_, uint256 deadline, uint8 v, bytes32 r, bytes32 s) payable external whenNotPaused {
        ...
        _e0SeriesUserList[_msgSender()] = true;
    }
    ...
    function randomizerCallback(uint256 requestId_, bytes32 seed_) external {
        ...
        for (uint8 numberIndex_ = 0; numberIndex_ < hatchCount_;) {
            uint256 randomNumber_ = getHatchRandomNumber(eSeries_, seed_, numberIndex_);
            ...
            if (hatchRule.startTokenId != 0) {
                uint256 tokenId = IL2Unicorn(l2Unicorn).mintForLevel(user_, hatchRule.level, hatchRule.startTokenId);
                emit HatchCallback(user_, requestId_, seed_, eSeries_, numberIndex_, randomNumber_, hatchRule.level, tokenId, l2Unicorn);
            }
     ...
     * @notice 获取孵化随机数
     */
    function getHatchRandomNumber(uint8 eSeries_, bytes32 seed_, uint256 numberIndex_) public view returns (uint256){
        if (eSeries_ == 0) {
    ...
    function viewHatchResults(uint256 requestId_) external view returns (HatchResult[] memory hatchResults){
        ...
        uint256 arrayIndex_ = 0;
        for (uint8 numberIndex_ = 0; numberIndex_ < hatchCount_;) {
            uint256 randomNumber_ = getHatchRandomNumber(eSeries_, gameInfo_.seed, numberIndex_);
            IL2UnicornRule.HatchRule memory hatchRule = IL2UnicornRule(l2UnicornRule).getHatchRuleByESeriesRandomNum(eSeries_, randomNumber_);
