...
library SushiRouterWrapper {
    ...
    /// Sells the received tokens for the provided ammounts for the last token in the route
    /// Temporary solution until we implement accumulation policy.
    /// @param self the sushi router used to perform the sale.
    /// @param _assetAmmounts output amount from selling the tokens.
    ...
    function sellTokens(
        ...
        uint256[] memory _assetAmmounts,
        address[] memory _tokens,
        address _recepient,
        address[][] memory _routes
    ) public {
        uint256 ammountsLength = _assetAmmounts.length;
        uint256 tokensLength = _tokens.length;
        uint256 routesLength = _routes.length;

        require(ammountsLength == tokensLength, "E1");
        require(routesLength == tokensLength, "E1");
    ...
    /// Sells the received tokens for the provided ammounts for ETH
    /// Temporary solution until we implement accumulation policy.
    /// @param self the sushi router used to perform the sale.
    /// @param _assetAmmounts output amount from selling the tokens.
    /// @param _tokens tokens to sell.
    ...
    // ERROR MAPPING:
    // {
    //   "E1": "Rewards: token, ammount and routes lenght must match",
    // }
}
