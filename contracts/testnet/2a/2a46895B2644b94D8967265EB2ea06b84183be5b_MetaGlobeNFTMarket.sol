...
//二级市场合约，支持ERC1155资产的官方及C2C交易
contract MetaGlobeNFTMarket is AccessControlEnumerable, Ownable, ReentrancyGuard, Pausable{
  ...
  constructor(IERC20 _uruToken, IERC1155 _mgge) ReentrancyGuard() {
    uruToken = _uruToken;
    mgge = _mgge;
  ...
  function setUruFee(uint _fee) public nonReentrant onlyOwner {
      uru_fee = _fee;
  }
    ...
    returns(bool) 
  {
    ...
    require(mgge.balanceOf(_msgSender(), _id) > 0, "stakeNFT: ONLY_OWNERS_ALLOWED");
    ...
    //触发事件
    emit MarketItemCreated(itemId, _id, _amount, _msgSender(), _price); 
    ...
    returns(bool)
  {
    ...
    uint _price = idToMarketItem[_itemId].price;
    uint _id = idToMarketItem[_itemId].id;
    uint _amount = idToMarketItem[_itemId].amount;

    // transfer the price of the nft (sending money) from the buyer to the seller
    uint seller_earn_fee = 100 - uru_fee;
    uruToken.safeTransferFrom(_msgSender(), idToMarketItem[_itemId].seller, _price.mul(seller_earn_fee).div(100));

    //抽成
    uruToken.safeTransferFrom(_msgSender(), address(this), _price.mul(uru_fee).div(100));
    ...
    //触发事件
    emit OnMarketItemSold(_itemId, _id, _amount, idToMarketItem[_itemId].seller, _msgSender(), _price);    

    return true;
  }

  //卖家修改价格
 function updateOrder(uint256 itemId, uint256 price)
     public 
     nonReentrant 
     whenNotPaused
     onlyNonContract
     returns (bool) 
  {
    ...
    require(item.seller == _msgSender(), "not the owner of this item");

    item.price = price;
    
    //触发事件
    emit OnUpdateOrder(itemId, idToMarketItem[itemId].seller, item.id, price);

    return true;
  }

  //管理员强行下架
  function forceCancelOrderByoperator(uint256 itemId)
     public 
     onlyOperator
     returns (bool) 
  {
    ...
    //触发撤单事件
    emit OnCancelOrder(itemId, idToMarketItem[itemId].seller, item.id, item.amount);

    return true;
  }
  
  //卖家自行撤单
  function cancelOrder(uint256 itemId)
     public 
     nonReentrant 
     whenNotPaused
     onlyNonContract
    ...
    returns (MarketItem[] memory) 
  {
    ...
    for (uint256 index = 0; index < itemCount; index++) {
      //合约内
      if(idToMarketItem[index+1].onSell == true) {
  ...
  * @notice only owner can call this method
  */
  function withdrawURU() public onlyOwner nonReentrant returns(bool){
      //获取合约内剩余的代币余额
