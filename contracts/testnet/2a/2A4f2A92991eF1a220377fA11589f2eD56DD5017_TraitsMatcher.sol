...
bytes32 constant STARTER_CONTRACT_ROLE = keccak256("STARTER_CONTRACT_ROLE");
bytes32 constant ENDING_CONTRACT_ROLE = keccak256("ENDING_CONTRACT_ROLE");
...
contract TraitsMatcher is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
	// ==================================================
	// =================== STATEV1 ======================
	// ==================================================
	...
	mapping(string => string[]) public compoundTraits;
	...
	function removeCompoundTrait(string memory traitId) public onlyRole(QUEST_MANAGER_ROLE) {
		delete compoundTraits[traitId];
	}
	...
	function getRewardChances(
		uint256[] memory heroTokens,
		string memory questId,
		bool isCampaign
	) public view returns (uint256[4] memory rewardChances) {
		(IQuestsV1.Quest memory quest, , IQuestsV1.QuestTraits memory traits, ) = isCampaign
		...
		for (uint256 i = 0; i < heroTokens.length; i++) {
			...
			for (uint256 j = 0; j < traits.traitType[i].length; j++) {
				if (compoundTraits[traits.traitType[i][j]].length > 0) {
					...
					{
						ITokenTraitsProvider.TraitMetadata memory traitMetadata1 = traitsProvider.getTraitMetadata(
							address(immortal),
							_toUint256(compoundTraits[traits.traitType[i][j]][0])
						);
						ITokenTraitsProvider.TraitMetadata memory traitMetadata2 = traitsProvider.getTraitMetadata(
							address(immortal),
					...
					returns (bytes memory traitValueInBytes1) {
						...
						returns (bytes memory traitValueInBytes2) {
							...
							if (
								_compareValues(traitValue1, expectValue1) && _compareValues(traitValue2, expectedValue2)
							) {
		...
		rewardChances = [uint256(100), 0, 0, 0];
		unchecked {
			...
			for (uint256 i = 0; i < rewards.length; i++) {
				...
				rewardChances[i] =
					((rewards[i].minChance * BASE_DIVDER) +
						(((percentChance / 100) * (rewards[i].maxChance - rewards[i].minChance)) + (BASE_DIVDER / 2))) /
					BASE_DIVDER;
			}
		}
	}
