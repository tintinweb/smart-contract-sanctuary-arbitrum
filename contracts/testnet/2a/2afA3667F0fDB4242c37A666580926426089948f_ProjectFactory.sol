...
contract ProjectFactory is Ownable, BlackHolePrevention, AccessControl {
    ...
    event CollectionCreatedFromProject(uint256 collectionId, uint32 projectId);
    event CollectionCreatedByBridge(uint32 collectionId);
    constructor(
    ...
    function LaunchProjectFromBridge(
        ...
        CommunityRegistry thisCommunityRegistry = getCommunityRegistry(communityId);
        ...
        emit CollectionCreatedByBridge(collectionId);
    }

    function setProjectMapByCommunity(uint32 communityId,uint32 projectId, uint32 collectionId) external onlyRole(MAP_ACCESS) {
        require(projectMapByCommunity[communityId][projectId] == 0,"ProjectFactory: collection already registered");
        projectMapByCommunity[communityId][projectId] = collectionId;
    }
    ...
    function LaunchProject( // only on home chain
        ...
        require (
            projectMapByCommunity[communityId][projectId] == 0,
        ...
        emit CollectionCreatedFromProject(collectionId,projectId);
    }

    function getCommunityRegistry(uint32 communityId) internal view returns (CommunityRegistry) {
    ...
    function _LaunchProject(
        ...
        address _newTokenAddress = TokenFactory.deploy(tokenConfig, projectOwner);
        // add the new token contract address into the sale
        saleConfig.token = _newTokenAddress;
        // Launch new sale contract
        address _newSaleAddress = SaleFactory.deploy(saleConfig, projectOwner); 
        ...
        if (useCommunityRandom){
            address userRandom = thisCommunityRegistry.getRegistryAddress("USER_RANDOM");
            require(userRandom != address(0),"ProjectFactory : community random not configured");
            thisCommunityRegistry.grantRole(RANDOM_CONSUMER,_newTokenAddress);
        } else {
        ...
        thisCommunityRegistry.setRegistryAddress(tokenKey, _newTokenAddress);       
...
import "../interfaces/IRegistryConsumer.sol";
import "../interfaces/IRandomNumberProvider.sol";
import "../token/OperatorFilteredToken.sol";
import "../extras/recovery/BlackHolePrevention.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
...
contract TokenFactoryV1 is Ownable, BlackHolePrevention {
    ...
    function deploy(
        ...
        // Launch new token contract
        OperatorFilteredToken token = new OperatorFilteredToken();
...
contract LockableRevealERC721EnumerableToken is IToken, ERC721Enumerable, Ownable, BlackHolePrevention {
    ...
    IRegistryConsumer               public TheRegistry = IRegistryConsumer(0x1e8150050A7a4715aad42b905C08df76883f396F);
    string              constant    public REGISTRY_KEY_RANDOM_CONTRACT = "RANDOMV2_SSP";
    string              constant    public USER_RANDOM = "USER_RANDOM";
    bool                constant    public useCommunityRandom = true;
    ...
    function getRandomSource() internal view returns (IRandomNumberProvider) {
        console.log("*",useCommunityRandom,address(myCommunityRegistry),myCommunityRegistry.getRegistryAddress(USER_RANDOM));
        if (useCommunityRandom) {
            return IRandomNumberProvider(myCommunityRegistry.getRegistryAddress(USER_RANDOM));
        } else {
            return IRandomNumberProvider(TheRegistry.getRegistryAddress(REGISTRY_KEY_RANDOM_CONTRACT));
        }
    }
    ...
    function revealAtCurrentSupply() external onlyAllowed(TOKEN_CONTRACT_ACCESS_REVEAL) {
        ...
        currentReveal.REQUEST_ID = getRandomSource().requestRandomNumberWithCallback();
    ...
    function lastReveal() external onlyAllowed(TOKEN_CONTRACT_ACCESS_REVEAL) {
        ...
        if(VRFShifting) {
            ...
            require(currentReveal.RANGE_END - currentReveal.RANGE_START > 0, "Token: requires minted tokens for current range to be at least 1");
            console.log("241 ",address(getRandomSource()));
            currentReveal.REQUEST_ID = getRandomSource().requestRandomNumberWithCallback();
            requestToRevealId[currentReveal.REQUEST_ID] = currentRevealCount;
        } else {
            require(mintedSupply > 0, "Token: requires minted tokens for current range to be at least 1");
    ...
    */
    function tokenURI(uint256 _tokenId) public view override(IToken, ERC721) returns (string memory) {
        ...
        string memory slash = "/";
        return string.concat(tokenRevealURI, chain, slash, folder, slash, file);
    }
