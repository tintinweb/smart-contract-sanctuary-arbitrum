...
interface IMintManager {
    function mintXPachi(address to, uint256 amount) external;
}
...
import "./IERC20.sol";
import "./IMintManager.sol";

pragma solidity 0.8.19;

contract pachiSlotV4 is Ownable {
    /// VRF - SUPRA ORACLES ///
    address supraAddr;
    ...
    address public splitAddr = address(0);
    address public mintManager = address(0);

    // past requests Id.
    uint256[] public nonces;
    uint256 public lastNonce;
    uint8 public constant RTP = 93; // 93% RTP (accurately 92.32%)
    uint8 public constant HOUSE_EDGE = 100 - RTP; // 7% house edge
    uint8 public constant HOUSE_EDGE_DIVISOR = 100; // divisor
    ...
    constructor(address supraSC) {
        ...
        // 3.fee splitAddr to several addresses
    }
    ...
    function executePayout(
        ...
        // fee splitAddr should comes here (just send to splitAddr contract)
        if (splitAddr != address(0)) {
            bool success3 = IERC20(_tokenAddress).transfer(
                splitAddr,
                (_totalInput * HOUSE_EDGE) / HOUSE_EDGE_DIVISOR
            );
            require(success3, "Transfer failed for splitAddr");
        }

        if (mintManager != address(0)) {
            // need some security check
            IMintManager(mintManager).mintXPachi(
                _from,
                (_totalInput * RTP) / HOUSE_EDGE_DIVISOR
            );
        }
    ...
    function setSplit(address _splitAddr) external onlyOwner {
        splitAddr = _splitAddr;
    }
