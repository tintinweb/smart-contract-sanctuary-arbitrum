...
contract AoriLens {
    ...
    {
        return (ask.OPTION(), ask.USDCPerOPTION(), ask.getCurrentBalance());
    }
    //Get specific information about a certain ask
    ...
    {
        return (bid.USDC(), bid.OPTIONPerUSDC(), bid.getCurrentBalance());
    }
...
contract OrderbookFactory is Ownable {
    ...
    */
    function createOrderbook(
        ...
        Orderbook orderbook =  new Orderbook(getTradingFee(), OPTION_, USDC, AORISEATSADD); 
...
contract Orderbook is ReentrancyGuard {
    ...
    function createAsk(uint256 _USDCPerOPTION, uint256 _OPTIONSize) public returns (Ask) {
        Ask ask = new Ask(OPTION, USDC, AORISEATSADD, msg.sender, _USDCPerOPTION, fee_, AORISEATSADD.getFeeMultiplier(), _OPTIONSize);
        asks.push(ask);
    ...
    function createBid(uint256 _OPTIONPerUSDC, uint256 _USDCSize) public returns (Bid) {
        Bid bid = new Bid(USDC, OPTION, AORISEATSADD, msg.sender, _OPTIONPerUSDC, fee_, AORISEATSADD.getFeeMultiplier(), _USDCSize);
        bids.push(bid);
        //transfer before storing the results
    ...
    function cancelOrder(address order, bool isAsk) public {
        if(isAsk) {
...
contract Bid is ReentrancyGuard {
    ...
    uint256 public USDC_SCALE = 10**6;
    uint256 public immutable BPS_DIVISOR = 10000;
    ...
    */
    function fill(address receiver, uint256 amountOfOPTION, uint256 seatId) public nonReentrant {
        ...
        if(receiver == AORISEATSADD.ownerOf(seatId)) {
            ...
            USDCToReceive = mulDiv(OPTIONAfterFee, USDC_SCALE, OPTIONPerUSDC); //1eY = (1eX * 1eY) / 1eX
            //Transfers
            ...
            OPTIONAfterFee = amountOfOPTION;            
            USDCToReceive = mulDiv(amountOfOPTION, USDC_SCALE, OPTIONPerUSDC); //1eY = (1eX * 1eY) / 1eX
    ...
    */
    ...
    return (OPTIONPerUSDC * USDC.balanceOf(address(this))) / USDC_SCALE;
    }
}
...
contract Ask is ReentrancyGuard {
    ...
    function fill(address receiver, uint256 amountOfUSDC, uint256 seatId) public nonReentrant {
        ...
        if(receiver == AORISEATSADD.ownerOf(seatId)) {
            ...
            OPTIONToReceive = mulDiv(USDCAfterFee, 10**18, USDCPerOPTION); //1eY = (1eX * 1eY) / 1eX
