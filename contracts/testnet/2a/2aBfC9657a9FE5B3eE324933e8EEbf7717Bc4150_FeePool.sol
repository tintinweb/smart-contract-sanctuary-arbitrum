...
contract FeePool is Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
    ...
    struct FeePeriod {
        uint64 feePeriodId;
        uint64 startTime;
        uint256 allNetworksBackedDebt;
        uint256 allNetworksDebtSharesSupply;
    ...
    constructor(
        ...
        address _owner,
        address _resolver
    ) Proxyable(_owner, _proxy) LimitedSetup(3 weeks) MixinSystemSettings(_resolver) {
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        ...
        newAddresses[7] = CONTRACT_ISSUED_SYNTH_AGGREGATOR;
        newAddresses[8] = CONTRACT_FUTURES_MARKET_MANAGER;
        newAddresses[9] = CONTRACT_REWARDSDISTRIBUTION;
        newAddresses[10] = CONTRACT_SYNTHR_AGGREGATOR;
    ...
    function allNetworksBackedDebt() public view returns (uint256 debt, uint256 updatedAt) {
        uint256 rawData = issuedSynthAggregator().totalIssuedSynths();

        debt = uint256(rawData);
    ...
    function _recentFeePeriodsStorage(uint256 index) internal view returns (FeePeriod storage) {
    ...
    /**
     * @notice The RewardsDistribution contract informs us how many rewards are sent to RewardEscrow to be claimed.
     */
    function setRewardsToDistribute(uint256 amount) external optionalProxy {
        require(messageSender == address(rewardsDistribution()), "RewardsDistribution only");
        // Add the amount of rewards to distribute on top of any rolling unclaimed amount
    ...
    function closeCurrentFeePeriod() external issuanceActive {
        ...
        (uint256 backedDebt, ) = allNetworksBackedDebt();
        (uint256 debtSharesSupply, ) = allNetworksDebtSharesSupply();

        // close on this chain
        _closeSecondary(backedDebt, debtSharesSupply);
    }

    /**
     * @notice Close the current fee period and start a new one.
     */
    function _closeSecondary(uint256 _allNetworksBackedDebt, uint256 _allNetworksDebtSharesSupply) internal {
        // before closing the current fee period, set the recorded backedDebt and debtSharesSupply
        _recentFeePeriodsStorage(0).allNetworksDebtSharesSupply = _allNetworksDebtSharesSupply;
        _recentFeePeriodsStorage(0).allNetworksBackedDebt = _allNetworksBackedDebt;
        ...
        uint256 newFeePeriodId = block.timestamp;
     ...
     */
    function claimFees() external payable issuanceActive optionalProxy returns (bool) {
     ...
     */
    function _recordFeePayment(uint256 sUSDAmount) internal returns (uint256) {
        // Don't assign to the parameter
        uint256 remainingToAllocate = sUSDAmount;
        ...
        for (uint256 i = FEE_PERIOD_LENGTH - 1; i < FEE_PERIOD_LENGTH; i--) {
            uint256 feesAlreadyClaimed = _recentFeePeriodsStorage(i).feesClaimed;
            uint256 delta = _recentFeePeriodsStorage(i).feesToDistribute.sub(feesAlreadyClaimed);

            if (delta > 0) {
                // Take the smaller of the amount left to claim in the period and the amount we need to allocate
                uint256 amountInPeriod = delta < remainingToAllocate ? delta : remainingToAllocate;
     ...
     * @param synthTokenAmount The amount of SYNTH tokens.
     */
    function _recordRewardPayment(uint256 synthTokenAmount) internal returns (uint256) {
        // Don't assign to the parameter
        uint256 remainingToAllocate = synthTokenAmount;
        ...
        // The condition checks for overflow because we're going to 0 with an unsigned int.
        for (uint256 i = FEE_PERIOD_LENGTH - 1; i < FEE_PERIOD_LENGTH; i--) {
            uint256 toDistribute = _recentFeePeriodsStorage(i).rewardsToDistribute.sub(
                _recentFeePeriodsStorage(i).rewardsClaimed
            );

            if (toDistribute > 0) {
                // Take the smaller of the amount left to claim in the period and the amount we need to allocate
                uint256 amountInPeriod = toDistribute < remainingToAllocate ? toDistribute : remainingToAllocate;
     ...
     * @param sUSDAmount The amount of fees priced in sUSD.
     */
    function _payFees(address account, uint256 sUSDAmount) internal notFeeAddress(account) {
        // Grab the sUSD Synth
     ...
     * @param account The address to send the fees to.
     * @param synthTokenAmount The amount of SYNTH.
     */
    function _payRewards(address account, uint256 synthTokenAmount) internal notFeeAddress(account) {
        /* Escrow the tokens for 1 year. */
        uint256 escrowDuration = 52 weeks;

        // Record vesting entry for claiming address and amount
        // SYNTH already minted to rewardEscrow balance
        rewardEscrowV2().appendVestingEntry(account, synthTokenAmount, escrowDuration);
        synthrBridge().sendREAppend{value: msg.value}(account, synthTokenAmount, true);
    }

    function getSendAppendGasFee(address account) external view returns (uint256) {
        (, uint256 availableRewards) = feesAvailable(account);
        uint256 remainingToAllocate = availableRewards;
        ...
        if (rewardPaid > 0) {
            bytes memory lzPayload = abi.encode(PT_REWARD_ESCROW_APPEND, abi.encodePacked(account), rewardPaid, true);
            return synthrBridge().calcLZFee(lzPayload, PT_REWARD_ESCROW_APPEND);
        }
        return 0;
    }
    ...
    /**
     * @notice The total SYNTH rewards available in the system to be withdrawn
     */
     ...
     * @dev Returns two amounts, one for fees and one for SYNTH rewards
     */
    function feesAvailable(address account) public view returns (uint256, uint256) {
        // Add up the fees
        ...
        // Return totalRewards as is in SYNTH amount
    ...
    function _isFeesClaimableAndAnyRatesInvalid(address account) internal view returns (bool, bool) {
        ...
        (uint256 ratio, bool anyRateIsInvalid) = issuer().collateralisationRatioAndAnyRatesInvalid(account);
        ...
        uint256 ratio_threshold = targetRatio.multiplyDecimal(SafeDecimalMath.unit().add(getTargetThreshold()));
     ...
     */
    function feesByPeriod(address account) public view returns (uint256[2][FEE_PERIOD_LENGTH] memory results) {
        // What's the user's debt entry index and the debt they owe to the system at current feePeriod
        uint256 userOwnershipPercentage;
        userOwnershipPercentage = synthrAggregator().sharePercent(account);
        ...
        for (uint256 i = FEE_PERIOD_LENGTH - 1; i > 0; i--) {
            ...
            if (lastFeeWithdrawal < periodId) {
                userOwnershipPercentage = synthrAggregator().sharePercentOnPeriod(account, uint256(periodId));
     ...
     */
    function _feesAndRewardsFromPeriod(uint256 period, uint256 ownershipPercentage) internal view returns (uint256, uint256) {
        ...
        uint256 feesFromPeriod = fp.feesToDistribute.multiplyDecimal(ownershipPercentage);

        uint256 rewardsFromPeriod = fp.rewardsToDistribute.multiplyDecimal(ownershipPercentage);

        return (feesFromPeriod, rewardsFromPeriod);
    }

    function effectiveDebtRatioForPeriod(address account, uint256 period) external view returns (uint256) {
        ...
        return synthrAggregator().sharePercentOnPeriod(account, uint256(_recentFeePeriods[period].feePeriodId));
    }
     ...
     */
    function getLastFeeWithdrawal(address _claimingAddress) public view returns (uint256) {
     ...
     */
    function _setLastFeeWithdrawal(address _claimingAddress, uint256 _feePeriodID) internal {
    ...
    event FeesClaimed(address account, uint256 sUSDAmount, uint256 synthTokenRewards);
    bytes32 private constant FEESCLAIMED_SIG = keccak256("FeesClaimed(address,uint256,uint256)");

    function emitFeesClaimed(
        address account,
        uint256 sUSDAmount,
        uint256 synthTokenRewards
    ) internal {
        proxy._emit(abi.encode(account, sUSDAmount, synthTokenRewards), 1, FEESCLAIMED_SIG, 0, 0, 0);
    }
}
