...
import {IStrategyMS} from "../interfaces/IStrategyMS.sol";
import {LibAgent} from "../libraries/LibAgent.sol";
import {LibEIP712} from "../libraries/LibEIP712.sol";
import {LibCollateral} from "../libraries/LibCollateral.sol";
import {LibStrategyMS} from "../libraries/LibStrategyMS.sol";
...
/**
 * @title DEDERI Strategy Merge Spilt
 * @author dederi
 * @notice This contract is strategy Merge Spilt 涵盖策略合并，拆分.
 */
contract StrategyMSFacet is IStrategyMS, ReentrancyGuard {
    using LibStrategyMS for LibStrategyMS.Layout;
    using LibStrategyConfig for LibStrategyConfig.Layout;

    // market
    error SpiltMismatch();
    error StrategyDuplicate(uint256 strategyId);

    /**
     * @notice StrategySplitRequested
     * @param requestHashA 用户拆分后策略A的请求hash
     * @param requestHashB 用户拆分后策略B的请求hash
     * @param requestParam 用户拆分后原始策略的请求参数
     */
    event StrategySplitRequested(
        bytes32 requestHashA,
        bytes32 requestHashB,
        StrategyTypes.SpiltStrategyRequest requestParam
    );

    /**
     * @notice StrategySplitExecuted
     * @param requestHashA 用户拆分后A策略的请求hash
     * @param requestHashB 用户拆分后B策略的请求hash
     * @param strategyIdOld 用户拆分前的策略id
     * @param strategyIdA 用户拆分后的A策略的id
     * @param strategyIdB 用户拆分后的B策略的id
     */
    event StrategySplitExecuted(
        bytes32 requestHashA,
        bytes32 requestHashB,
        uint256 strategyIdOld,
        uint256 strategyIdA,
        uint256 strategyIdB
    );
    ...
    /**
     * @notice StrategySplitRequested
     * @param requestHash 用户策略合并的请求hash
     * @param mergeIdA 用户要合并的A策略的id
     * @param mergeIdB 用户要合并的B策略的id
     */
    event StrategyMergeRequested(bytes32 requestHash, uint256 mergeIdA, uint256 mergeIdB);

    /**
     * @notice StrategyMergeExecuted
     * @param requestHash 用户策略合并的请求hash
     * @param mergeIdA 用户要合并的A策略的id
     * @param mergeIdB 用户要合并的B策略的id
     * @param newStrategyId 合并后的策略id，如果相互抵消，则为0
     */
    event StrategyMergeExecuted(bytes32 requestHash, uint256 mergeIdA, uint256 mergeIdB, uint256 newStrategyId);

    /**
     * @notice Split strategy by user
     * @param strategyId The current strategy ID
     * @param originalCollateralToTopUpAmount Initial collateral for supplement
     * @param originalSplitCollateralAmount Collateral for original strategy
     * @param newlySplitCollateralAmount Collateral for newly split positions
     * @param positionIds Position ids to be split
     */
    function splitStrategy(
        uint256 strategyId,
        uint256 originalCollateralToTopUpAmount,
        ...
        // 验证仓位id数组参数是否有重复的(这里做了，执行时候可以不做，因为存起来了)
        LibStrategyConfig._checkPositionIdDuplicates(positionIds);

        // 构造请求hash 计算拆分后A的组合保证金
        bytes32 requestHashA = LibStrategyConfig._getRequestHashAndUpdateNonce(admin);

        LibStrategyMS.Layout storage l = LibStrategyMS.layout();

        // 请求赋值
        l.userSpiltRequest[requestHashA].owner = admin;
        l.userSpiltRequest[requestHashA].strategyId = strategyId;
        l.userSpiltRequest[requestHashA].positionIds = positionIds;
        l.userSpiltRequest[requestHashA].originalCollateralsToTopUpAmount = originalCollateralToTopUpAmount;
        l.userSpiltRequest[requestHashA].originalSplitCollateralAmount = originalSplitCollateralAmount;
        l.userSpiltRequest[requestHashA].newlySplitCollateralAmount = newlySplitCollateralAmount;

        // 计算拆分后B的组合保证金
        bytes32 requestHashB = LibStrategyConfig._getRequestHashAndUpdateNonce(admin);

        emit StrategySplitRequested(requestHashA, requestHashB, l.userSpiltRequest[requestHashA]);
    }

    /**
     * @notice 执行拆分
     * @param requestHashA 记录用户拆分请求参数，提供拆分后的A 组合保证金
     * @param requestHashB 记录用户拆分请求参数，提供拆分后的B 组合保证金
     */
    function executeSplitStrategy(bytes32 requestHashA, bytes32 requestHashB) external {
        LibAccessControlEnumerable.checkRole(Constants.KEEPER_ROLE);

        LibStrategyMS.Layout storage l = LibStrategyMS.layout();
        StrategyTypes.SpiltStrategyRequest memory requestParam = l.userSpiltRequest[requestHashA];
        delete l.userSpiltRequest[requestHashA];

        // 再次验证owner和是否激活(防止在此期间策略信息因为转移等原因发生变更)

        StrategyTypes.StrategyAllData memory strategyData = LibPositionCore._getStrategyAllData(
            requestParam.strategyId
        );
        //检查 owner 是否转移，策略是否有效
        LibStrategyConfig._ensureAdminAndActive(strategyData, strategyData.owner);

        uint256[] memory strategyPositionIds = LibPositionCore._getPositionIdsFromStrategy(
            strategyData.option,
            strategyData.future
        );
        uint256 originLen = strategyPositionIds.length;
        uint256 spiltLen = requestParam.positionIds.length;

        // 验证拆分个数不能为0或者为全部，原始策略中不能是1个
        // 验证新拆出的抵押品数量不能为0
        if (originLen == spiltLen || spiltLen == 0 || originLen == 1) {
            revert SpiltMismatch();
        }

        // 验证原始策略中的待添加抵押品，新拆出来的待添加抵押品是否有重复(请求中已验证，此处无需再次验证)
        // 验证用户请求的腿id是否重复(请求中已验证，此处无需再次验证)
        // 验证腿数量（因为是拆分，因此无需验证）

        // 验证仓位id数组是否全部包含在策略下面
        LibStrategyMS._validateSplitLegIds(strategyPositionIds, requestParam.positionIds);
        // 创建2个新的nft以及划转保证金，burn掉原来的，返回处理后的2个策略id或者抵押品
        (uint256 strategyIdA, uint256 strategyIdB) = LibPositionCore._handleSplitStrategy(requestParam);
        LibStrategyMS._checkSplitStrategy(requestParam, strategyIdA, strategyIdB, requestHashA, requestHashB);
        LibVault._marginIncrease(
            requestParam.owner,
            requestParam.originalSplitCollateralAmount + requestParam.newlySplitCollateralAmount
        );

        emit StrategySplitExecuted(requestHashA, requestHashB, requestParam.strategyId, strategyIdA, strategyIdB);
    }

    /**
     * @notice 用户发起合并策略请求
     * @param firstStrategyId 第一个策略
     * @param secondStrategyId 第二个策略
     * @param collateralAmount 待添加的抵押品
     * @custom:event emits a 策略合并请求 event
     */
    function mergeStrategies(
        ...
        LibStrategyMS.Layout storage l = LibStrategyMS.layout();
        address admin = LibAgent._getAdminAndUpdate(msg.sender);
        StrategyTypes.StrategyAllData memory firstStrategy = LibPositionCore._getStrategyAllData(firstStrategyId);
        StrategyTypes.StrategyAllData memory secondStrategy = LibPositionCore._getStrategyAllData(secondStrategyId);
        // 验证2个策略的owner和是否激活
        LibStrategyConfig._ensureAdminAndActive(firstStrategy, admin);
        LibStrategyConfig._ensureAdminAndActive(secondStrategy, admin);
        // 验证2个策略不是相同的
        if (firstStrategyId == secondStrategyId) {
            revert StrategyDuplicate(firstStrategyId);
        }
        // 构建requestHash
        bytes32 requestHash = LibStrategyConfig._getRequestHashAndUpdateNonce(admin);
        // 给requestHash赋值参数
        l.userMergeRequest[requestHash].owner = admin;
        l.userMergeRequest[requestHash].firstStrategyId = firstStrategyId;
        l.userMergeRequest[requestHash].secondStrategyId = secondStrategyId;
        l.userMergeRequest[requestHash].collateralAmount = collateralAmount;

        emit StrategyMergeRequested(requestHash, firstStrategyId, secondStrategyId);
    }

    /**
     * @notice keeper执行合并策略请求
     * @param requestHash 请求hash
     * @custom:event emits a 策略合并执行 event
     */
    function executeMergeStrategies(bytes32 requestHash) external nonReentrant {
        LibAccessControlEnumerable.checkRole(Constants.KEEPER_ROLE);

        LibStrategyMS.Layout storage l = LibStrategyMS.layout();
        StrategyTypes.MergeStrategyRequest memory requestParam = l.userMergeRequest[requestHash];
        delete l.userMergeRequest[requestHash];

        // 再次验证2个策略的owner和是否激活
        StrategyTypes.StrategyAllData memory firstStrategy = LibPositionCore._getStrategyAllData(
            requestParam.firstStrategyId
        );
        StrategyTypes.StrategyAllData memory secondStrategy = LibPositionCore._getStrategyAllData(
            requestParam.secondStrategyId
        );
        LibStrategyConfig._ensureAdminAndActive(firstStrategy, requestParam.owner);
        LibStrategyConfig._ensureAdminAndActive(secondStrategy, requestParam.owner);
        // 执行策略合并
        (bool isZero, uint256 mergeStrategyId) = LibPositionCore._handleMergeStrategies(requestParam);
        // 更新划转金额
        if (isZero) {
            // 根据返回值，如果完全抵消 2个策略抵押品划转到可用余额 vault，新加进来的抵押品因为没有更新划转，因此可以直接取消
            LibVault._marginIncrease(firstStrategy.owner, firstStrategy.collateralAmount);
            LibVault._marginIncrease(secondStrategy.owner, secondStrategy.collateralAmount);
        } else {
            LibVault._marginDecrease(requestParam.owner, requestParam.collateralAmount);
            // 如果不完成抵消，更新新加进来的抵押品，扣减可用余额，增加不可用余额
        }

        // 获取合并后的抵押品

        // 抵押品价值和保证金比较
        LibStrategyMS._checkMergeStrategy(requestHash, requestParam, firstStrategy, secondStrategy);

        // 发出事件
        emit StrategyMergeExecuted(
            requestHash,
            requestParam.firstStrategyId,
            requestParam.secondStrategyId,
            mergeStrategyId
...
interface IStrategyMS {
    ...
    function executeSplitStrategy(bytes32 requestHash, bytes32 requestHash2) external;

    function mergeStrategies(uint256 firstStrategyId, uint256 secondStrategyId, uint256 collateralAmount) external;

    function executeMergeStrategies(bytes32 requestHash) external;
}
...
library LibMarketPricer {
    ...
    function _getNewCashWithSellStrategy(
        ...
        return intMax(-availableBalance, int256(strategyEstimatedIM) - equity) + int256(fee);
    }
...
library LibPositionCore {
    ...
    function _handleSplitStrategy(
        ...
        for (uint256 i; i < originalLen; ) {
            ...
            for (uint256 j; j < newlySpiltLen; ) {
                uint256 newlyPositionId = requestParam.positionIds[j];
    ...
    function _getMakerFuturePnl(
        ...
        for (uint i; i < len; ) {
            futurePnl +=
                ((int256(makerFuture[i].entryPrice) - int256(takerFuture[i].entryPrice)) *
                    int256(makerFuture[i].size)) /
                1e18;
            unchecked {
                ++i;
            }
...
library LibStrategyConfig {
    ...
    function _validateMergeabilityOfStrategy(StrategyTypes.StrategyRequest memory strategy) internal pure {
        ...
        uint256 futureLen = strategy.future.length;
        address underlying;
        if (optionLen > 0) {
...
library LibStrategyMS {
    bytes32 internal constant STORAGE_SLOT = keccak256("dederi.contracts.storage.StrategyMS");

    error InvalidPositionId(uint256 positionId);

    struct Layout {
        /// @notice 拆分请求中需要的参数
        mapping(bytes32 => StrategyTypes.SpiltStrategyRequest) userSpiltRequest;
        /// @notice 合并策略中需要的参数
        mapping(bytes32 => StrategyTypes.MergeStrategyRequest) userMergeRequest;
    }
    ...
    /// @notice 要验证拆分出来的腿ID是否包含在原始腿ID中
    function _validateSplitLegIds(uint256[] memory originalLegIds, uint256[] memory splitLegIds) internal pure {
        uint256 spiltLen = splitLegIds.length;
        uint256 originalLegLen = originalLegIds.length;
        for (uint256 i; i < spiltLen; ) {
            bool found = false;
            for (uint256 j; j < originalLegLen; j++) {
                if (splitLegIds[i] == originalLegIds[j]) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                revert InvalidPositionId(splitLegIds[i]);
            }
            unchecked {
                ++i;
            }
    ...
    function _checkSplitStrategy(
        ...
        StrategyTypes.MarginItemWithHash memory strategyAMargin = LibMarginOracle._getPortfolioMarginInfoByHash(
            requestHashA
        );
        StrategyTypes.MarginItemWithHash memory strategyBmargin = LibMarginOracle._getPortfolioMarginInfoByHash(
            requestHashB
        );

        StrategyTypes.StrategyAllData memory strategyAData = LibPositionCore._getStrategyAllData(strategyIdA);
        StrategyTypes.StrategyAllData memory strategyBData = LibPositionCore._getStrategyAllData(strategyIdB);

        int256 strategyAUnrealizedPnl = strategyAMargin.futureUnrealizedPnl + strategyAMargin.optionValue;
        int256 strategyBUnrealizedPnl = strategyBmargin.futureUnrealizedPnl + strategyBmargin.optionValue;

        (int256 strategyANewCash, int256 strategyBNewCash) = LibMarketPricer._getNewCashWithSpliStrategy(
            strategyAMargin.im,
            strategyBmargin.im,
            strategyAUnrealizedPnl,
            strategyBUnrealizedPnl,
            strategyAData,
            strategyBData
        );

        LibMarketPricer._checkCollateralEnough(uint256(strategyANewCash), requestParam.originalSplitCollateralAmount);
        LibMarketPricer._checkCollateralEnough(
            uint256(strategyBNewCash),
            requestParam.originalCollateralsToTopUpAmount + requestParam.newlySplitCollateralAmount
        );
    }

    function _checkMergeStrategy(
        bytes32 requestHash,
        StrategyTypes.MergeStrategyRequest memory requestParam,
        StrategyTypes.StrategyAllData memory firstStrategy,
        StrategyTypes.StrategyAllData memory secondStrategy
    ) internal view {
        StrategyTypes.MarginItemWithId memory firstStrategyMargin = LibMarginOracle._getPortfolioMarginInfoByStrategyId(
            firstStrategy.strategyId
        );
        StrategyTypes.MarginItemWithId memory secondStrategyBMargin = LibMarginOracle
            ._getPortfolioMarginInfoByStrategyId(secondStrategy.strategyId);
        int256 firstStrategyUnrealizedPnl = firstStrategyMargin.futureUnrealizedPnl + firstStrategyMargin.optionValue;
        int256 secondStrategyUnrealizedPnl = secondStrategyBMargin.futureUnrealizedPnl +
            secondStrategyBMargin.optionValue;

        int256 firstStrategyEquity = LibMarketPricer._getEquity(firstStrategy, firstStrategyUnrealizedPnl);
        int256 secondStrategyEquity = LibMarketPricer._getEquity(secondStrategy, secondStrategyUnrealizedPnl);
        StrategyTypes.MarginItemWithHash memory strategyMargin = LibMarginOracle._getPortfolioMarginInfoByHash(
            requestHash
        );
        int256 equity = firstStrategyEquity + secondStrategyEquity;
        if (uint256(equity) < strategyMargin.im) {
            LibMarketPricer._checkCollateralEnough(requestParam.collateralAmount, strategyMargin.im - uint256(equity));
        }
    }
}
