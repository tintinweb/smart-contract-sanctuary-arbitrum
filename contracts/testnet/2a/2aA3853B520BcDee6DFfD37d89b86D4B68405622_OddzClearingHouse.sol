...
import "./interfaces/IOddzPositionOpeningHandler.sol";
    ...
    IUniswapV3SwapCallback
{
    ...
    /// @param orderType               0-marketOrder , 1- limit Order , 2- take Profit order , 3-stop Loss order
    /// @param triggeringPrice         Triggering price
    /// @param deadline                Time after which the transaction can no longer be executed
    ...
    /// @param positionId              Position Id in which user wants to merge new position
    ...
    address public oddzPositionOpeninghandler;
    ...
    function initialize(
        ...
        address _oddzPositionOpeninghandler
    ) public initializer {
        require(
        ...
        require(
            _oddzPositionOpeninghandler.isContract(),
           //oddz position opening handler not a contract
            "OCH :OPHNC"
        );
        ...
        oddzPositionOpeninghandler=_oddzPositionOpeninghandler;
    }

    /**
    * @notice to update the authorized address
    * @param _address address to update
    * @param _include true for adding
     */
    function updateAuthorizedAddresses(address _address,bool _include) external onlyOwner{
        require(
            _address.isContract(),
            // :authorized address is not a contract
            "OCH:ANC"
        );
        authorized[_address]=_include;
    }
        ...
        )
    {
        ...
        _checkLiquidityCollateralRequirement(trader, response.orderId);
    ...
    /**
    * @notice gives a new id when called from authorized addresses
    * @return _groupId new group Id
     */
    function updateGroupID() external onlyAuthorized returns(uint256 _groupId) {
     ...
     /**
    * @notice gives a new id when called from authorized addresses
    * @return _positionId new position Id
     */
    function updatePositionId() external onlyAuthorized returns(uint256 _positionId) {
       positionIDGenerator= positionIDGenerator+1;
       return positionIDGenerator;
    }
    ...
    *@return fundingGrowth funding growth variables
     */
    function settlePositionFunding(uint256 positionId, address baseToken) external onlyAuthorized returns(FundingGrowth memory){
        return _settlePositionFunding(positionId, baseToken);
    }

     /**@notice for calculating funding payment amount for position and settling it in realized Pnl
    *@param positionId position id
    *@param fundingGrowth updated funding growth variables
     */
    function settleFunding(uint256 positionId,FundingGrowth memory fundingGrowth )external onlyAuthorized{
    ...
    *@return  fundingGrowth funding growth variables
    ...
    /**
    * @notice called from authorized address and handles position generation
    * @param params takes all the paramter required for the position handler struct
    * @return baseAmount base token amount
    * @return quoteAmount quote token amount
     */
    function openPositionHandler(PositionHandlerParams calldata params)
        external
        onlyAuthorized
        returns(uint256 baseAmount,uint256 quoteAmount)
    {
        FundingGrowth memory fundingGrowth;
        fundingGrowth =_settlePositionFunding(0,params.baseToken);
        return _positionHandler(params,fundingGrowth);
    }

    /**
    * @notice close position handler.Can only be called by authorized addresses
    * @param params takes all the parameters of closePositionhandler
    * @param liquidator liquidator address otherwise zero address 
    * @param isLiquidation true if liquidating a position
    * @return _response response after swapping
     */
    function closePositionHandler(ClosePositionHandlerParams calldata params,address liquidator,bool isLiquidation)
        external 
        onlyAuthorized
        returns(ISwapManager.SwapResponseParams memory _response ){
        return _closePositionHandler(params,liquidator,isLiquidation);
    }
     ...
     * @param trader address of the trader who is removing liquidity
     ...
     * @param fundingGrowth funding growth
     */
        ...
        returns (uint256 _baseAmount, uint256 _quoteAmount)
    {
        ...
        IOddzPositionOpeningHandler.FundingGrowth memory funding=IOddzPositionOpeningHandler.FundingGrowth({
                twPremiumX96:fundingGrowth.twPremiumX96,
                twPremiumDivBySqrtPriceX96:fundingGrowth.twPremiumDivBySqrtPriceX96
        });

        IOddzPositionOpeningHandler.InternalPositionHandlerParams
            memory internalParams =IOddzPositionOpeningHandler.InternalPositionHandlerParams({
                trader: _params.trader,
                baseToken: _params.baseToken,
                ...
                fundingGrowth:funding
            });

        uint256 positionIdResponse;
        uint256 groupIdResponse;
        int256  realizedPnl;
        if (_params.isIsolate) {
            // to handle isolate position
         (positionIdResponse,realizedPnl)=IOddzPositionOpeningHandler(oddzPositionOpeninghandler).handleIsolatePositions(internalParams, _response);

        } else {
            // to handle positions inside a group
           (positionIdResponse,groupIdResponse,realizedPnl)=IOddzPositionOpeningHandler(oddzPositionOpeninghandler).handleGroupPositions(internalParams, _response);
        }
     ...
     * @param _isLiquidation true if liqudating position 
     * returns baseAmount, quoteAmount.
     */
    function _closePositionHandler(ClosePositionHandlerParams memory _params,address _liquidator,bool _isLiquidation)
        internal
        returns (ISwapManager.SwapResponseParams memory _response)
    {
        ...
        if (_response.pnlToBeRealized != 0) {
            ...
            if(info.groupID>0){
                collateralValue=IBalanceManager(balanceManager).getGroupCollateralValue(info.groupID);
            }
            else{
                collateralValue=IBalanceManager(balanceManager).getPositionCollateralValue(_params.positionID);
            }
            require(
                (_isLiquidation &&
                    IOddzConfig(oddzConfig).backstopLiquidityProvider(_liquidator)) ||
                   collateralValue >= 0,
                   //OddzClearingHouse : base debt
                "OCH:BD"
            );
        }
    ...
    function _settleLiquidityGeneratedPosition(
        ...
        if (!params.isClose) {
            ...
            emit PositionUpdated(
                internalParams.trader,
                internalParams.baseToken,
                groupIdResponse,
                positionIdResponse,
                _response.swappedBasePositionSize,
                _response.swappedQuotePositionSize,
    ...
    *@return  _fundingGrowth funding growth variables
     */
...
interface IOddzConfig {
    ...
    /// @return _maxPositionsPerDefaultGroup Max value of total positions per default group
    function maxPositionsPerDefaultGroup() external view returns (uint8 _maxPositionsPerDefaultGroup);
    ...
    ///@param _liquidator liquidator address
    ///@return _include true if address is included otherwise false
    function gelatoBotAddress(address _liquidator) external view returns(bool _include);

    ///@return _deleveragePercentage deleveraging percentage
    function deleveragePercentage() external view returns(uint24 _deleveragePercentage);

    ///@return _thresholdPercentage deleveraging threshold
    function reducingThresholdPercentage() external view returns(uint24 _thresholdPercentage);

    /// @return _reducePercentage reducing percentage
    function reducePercentage() external view returns(uint24 _reducePercentage);

    /// @return _deleveragingFees deleverage fees 
    function deleveragingFees() external view returns(uint24 _deleveragingFees);

    ///@param _baseToken base Token address
    ///@return _maxTick max ticks
    function maxTickCrossLimitWithinBlock(address _baseToken) external view returns(uint24 _maxTick); 

    ///@return threshold insurance manager threshold
    function insuranceManagerThreshold() external view returns(uint256 threshold);

    ///@param _address address
    ///@return _included true if address is backstop liquidity provider
    function backstopLiquidityProvider(address _address) external view returns(bool _included);
...
interface IOddzVault {
    ...
    /**
    * @notice returns settlement token address;
     */
    ...
    * @notice returns settlement token decimals
     ...
     * @param ratio    ratio (initial margin ratio or maintenance margin ratio)
     */
    function getLiquidityPositionCollateralByRatio(address trader,bytes32 orderID, uint24 ratio) external view returns (int256);
     ...
     * @param amount Amount of token that User Supplies; can be 0, do not require this
     */
     ...
     * @param trader The Address of the trader
     * @return balance of the trader for that token
     */
    function getAvailableCollateral(address trader)
        external
        view
...
interface IBalanceManager {
    struct PositionInfo {
        ...
        bool autoDeleverage; // auto deleverage the position if it;s true 
    ...
    struct GroupInfo {
        address trader; // address of the trader
        bool autoDeleverage;  // if for this group auto De leverage is enabled or not .
    ...
    *@param reduce true for reducing the position and false for adding
     */
    ...
    *@param groupId group id 
    *@param collateral collateral amount
    *@param reduce true for reducing and false for adding
     */
     ...
     /**
    * @notice updated the deleveraging status of the position
    * @param positionId position id
    * @param status  true for enabling deleverage
     */
    function updatePositionDeleverageStatus(
        uint256 positionId ,
        bool status
        )external ;

     /**
    * @notice updates the deleveraging status of the group
    * @param groupId group id
    * @param status  true for enabling deleverage
     */
    function updateGroupDeleverageStatus(
        uint256 groupId ,
        bool status
        )external ;
    ...
    * @return  liquidityOrderValue liquidity order collteral value 
     */
    function getLiquidityOrderCollateralValue(bytes32 orderId)
        external
        view 
    ...
    * @return impermanentPositionValue impermanent position value
     */
    function getImpermanentPositionValue(bytes32 orderId)
        external
        view
     ...
     */
    function getTotalOrderInfo(bytes32 orderId)
        external
        view
    ...
    * @return marginRequired margin required for the liquidity order
     */
     function getMarginRequirementForLiquidityOrderLiquidation(bytes32 orderId) 
        external
        view
        returns(int256 marginRequired);
     ...
     */
    function getLiquidityPositionUnrealisedPnL(bytes32 _orderId) external view returns(int256 _unrealizedPnL);
     /**
...
interface IOrderManager {
    ...
    *@return response Response of remove liquidity
    ...
    function removeLiquidity(RemoveLiquidityParams calldata params)
     ...
     /**
    *@notice updates the collateral amount of the liquidity order
    *@param orderId  order Id
    *@param collateral collateral amount
    *@param reduce true for reducing and false for adding
     */
    function updateLiquidityPositionCollateral(bytes32 orderId, uint256 collateral,bool reduce)
        external;
    ...
    */
    function getCurrentOrderIds(address trader)
        external
        view
...
import "./IOddzClearingHouse.sol";

interface IOddzPositionOpeningHandler{
    ...
    struct InternalPositionHandlerParams {
     ...
     function handleIsolatePositions(
        InternalPositionHandlerParams memory params,
        ISwapManager.SwapResponseParams memory response
    )external returns(uint256 positionId,int256 realizedPnl);

     function handleGroupPositions(
        InternalPositionHandlerParams memory params,
        ISwapManager.SwapResponseParams memory response
    )external returns(uint256 positionId,uint256 groupId,int256 realizedPnl);
}
...
library OddzSafeCast {
    ...
    function toUint128(uint256 value) internal pure returns (uint128 returnValue) {
        ...
        //Value not fit
        require(((returnValue = uint128(value)) == value), "SC:VNF128");
    }
    ...
    function toUint64(uint256 value) internal pure returns (uint64 returnValue) {
        //SafeCast: value doesn't fit in 64 bits
         //Value not fit
        require(((returnValue = uint64(value)) == value), "SC:VNF64");
    }
    ...
    function toUint32(uint256 value) internal pure returns (uint32 returnValue) {
         //Value not fit
        require(((returnValue = uint32(value)) == value), "SC:VNF32");
    }
    ...
    function toUint16(uint256 value) internal pure returns (uint16 returnValue) {
         //Value not fit
        require(((returnValue = uint16(value)) == value), "SC:VNF16");
    }
    ...
    function toUint8(uint256 value) internal pure returns (uint8 returnValue) {
         //Value not fit
        require(((returnValue = uint8(value)) == value), "SC:VNF8");
    }
    ...
    function toInt128(int256 value) internal pure returns (int128 returnValue) {
        //Value not fir
        require(((returnValue = int128(value)) == value), "SC:VNF128");
    }
    ...
    function toInt64(int256 value) internal pure returns (int64 returnValue) {
        //SafeCast: value doesn't fit in 64 bits
        require(((returnValue = int64(value)) == value), "SC:VNF64");
    }
    ...
    function toInt32(int256 value) internal pure returns (int32 returnValue) {
        ...
        require(((returnValue = int32(value)) == value), "SC:VNF32");
    }
    ...
    function toInt16(int256 value) internal pure returns (int16 returnValue) {
        ...
        require(((returnValue = int16(value)) == value), "SC:VNF16");
    }
    ...
    function toInt8(int256 value) internal pure returns (int8 returnValue) {
        ...
        require(((returnValue = int8(value)) == value), "SC:VNF8");
    }
    ...
    function toInt256(uint256 value) internal pure returns (int256) {
        ...
        require(value <= uint256(type(int256).max), "SC:VNF256");
        return int256(value);
    }
    ...
    function toInt24(int256 value) internal pure returns (int24 returnValue) {
        ...
        require(((returnValue = int24(value)) == value), "SC:VNF24");
    }
}
...
library OddzMathV2 {
    ...
    function neg256(int256 a) internal pure returns (int256) {
        //Oddz Math : Inversion over flow
        require(a > -2**255, "OM:IOF");
        return -a;
    }
...
interface IOddzClearingHouse{
    ...
    function updatePositionId() external returns(uint256 _positionId);

    function settlePositionFunding(uint256 positionId, address baseToken) external returns(FundingGrowth memory fundingGrowth) ;
    ...
    *@param fundingGrowth updated funding growth variables
     */
    function settleFunding(uint256 positionId,FundingGrowth memory fundingGrowth )external;
...
abstract contract OddzOwnable is ContextUpgradeable {
    ...
    modifier onlyOwner() {
        //caller not owner
        require(owner == _msgSender(), "OO:CNO");
        _;
    }
    ...
    function nominateNewOwner(address _newOwner) external onlyOwner {
        //Ownable: newOwner can not be zero addresss
        require(_newOwner != address(0), "OO:ZNO");
        //Same owner
        require(_newOwner != owner, "OO:SO");
        // same as candidate
        //already nominated
        require(_newOwner != nominatedOwner, "OO:AN");
    ...
    function AcceptOwnership() external {
        //Ownable: No one is nominated
        //None nominated
        require(nominatedOwner != address(0), "OO:NN");
        //caller not nominated
        require(nominatedOwner == _msgSender(), "OO:CNN");
