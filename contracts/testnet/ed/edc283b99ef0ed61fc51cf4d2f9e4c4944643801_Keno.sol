...
contract Keno is CommonSoloKeno, InternalRNG {
  ...
  uint256 internal immutable maxWagerValue;
  ...
  constructor(IRandomizerRouter _router, address _vaultManagerAddress) CommonSoloKeno(_router) {
    ...
    minWagerValue = vaultManager.getMinWager(address(this));
    maxWagerValue = vaultManager.getMaxWager();
    // minWagerValue = 1e2;
    // maxWagerValue = 1e8 * 1e30;
  }
  ...
  function checkWagerValue(uint256 _wager, address _token) internal view {
    ...
    require(dollarValue_ <= maxWagerValue, "Keno: wager is too big");
  }
  ...
  function encodeGameData(uint32[] calldata _choices) public pure returns (bytes memory) {
    return abi.encode(_choices);
  }

  function decodeGameData(bytes memory _gameData) public pure returns (uint32[] memory) {
  ...
  function calcReward(
    ...
    uint256 _correct,
    uint128 _wager
  ) public view returns (uint128 reward_) {
    reward_ = uint128((_wager * multipliers_[_picks][_correct]) / 1e2);
  }
...
abstract contract CommonSoloKeno is Core {
  ...
  function refundGame(uint256 _requestId) external nonReentrant whenNotCompleted(_requestId) {
    Game memory game = games_[_requestId];
  ...
  function play(
    ...
    uint128 _stopGain,
    uint128 _stopLoss
  ) internal virtual returns (uint256 payout_, uint32 playedGameCount_, uint256[] memory payouts_);

  function randomizerFulfill(
    uint256 _requestId,
    uint256[] calldata _randoms
  ) internal override whenNotCompleted(_requestId) nonReentrant {
  ...
  function _create(
    ...
    previousGameOfPlayer[player_] = requestId_;
