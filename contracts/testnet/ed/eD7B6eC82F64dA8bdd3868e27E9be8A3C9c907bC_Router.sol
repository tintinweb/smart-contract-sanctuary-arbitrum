...
contract Router {
    ...
    function increasePosition(address _indexToken, uint256 _amountIn, uint256 _sizeDelta, bool _isLong, uint256[] memory triggerPrices, address refer) external {
        _increasePosition(_indexToken, _amountIn, _sizeDelta, _isLong, triggerPrices, refer);
    }

    function decreasePosition(address _indexToken, uint256 _sizeDelta, bool _isLong) external {
        _decreasePosition(_indexToken, _sizeDelta, _isLong);
    }

    function _increasePosition(address _indexToken, uint256 _amountIn, uint256 _sizeDelta, bool _isLong, uint256[] memory triggerPrices, address refer) private {
        IVault(vault).increasePosition(_sender(), _indexToken, _amountIn, _sizeDelta, _isLong, triggerPrices, refer);
    }

    function _decreasePosition(address _indexToken, uint256 _sizeDelta, bool _isLong) private returns (uint256) {
        return IVault(vault).decreasePosition(_sender(), _indexToken, _sizeDelta, _isLong);
    }
