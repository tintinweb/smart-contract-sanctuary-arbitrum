...
uint256 constant ID = uint256(keccak256("game.piratenation.captaincomponent"));

contract CaptainComponent is BaseComponent {
    /** SETUP **/
    ...
    function setValue(
        ...
        setBytes(
            entity,
            abi.encode(tokenContract, tokenId, lastSetCaptainTime)
        );
    }
    ...
    {
        if (has(entity)) {
            (tokenContract, tokenId, lastSetCaptainTime) = abi.decode(
                getBytes(entity),
