...
contract BridgePortalZKF is ReentrancyGuard, Governable {
    using SafeERC20 for IERC20;
    using Address for address payable;

    //TOKEN_TYPE: ETH = 1, WETH = 2, MNT = 3, BNB = 4, USDC = 5;
    ...
    uint256 public minETHDepositValue = 100000000000000;
    uint256 public minERC20DepositValue = 1000000;
    ...
    event TransactionDeposited(
        ...
        address indexed to,
        bool isNativeToken,
        uint256 amount, //input must ETH/WETH/USDC
    ...
    function setERC20Addr(address _erc20Addr) external onlyGov {
        require(_erc20Addr != address(0x0), "OKOK: _erc20Addr cannot be 0x0");
        erc20Addr = _erc20Addr;
    }
    ...
    function setMinETHDepositValue(uint256 _minETHDepositValue) external onlyGov {
        minETHDepositValue = _minETHDepositValue;
    }

    function setMinERC20DepositValue(uint256 _minERC20DepositValue) external onlyGov {
        minERC20DepositValue = _minERC20DepositValue;
    }
    ...
    function depositNativeToken(address _to, uint256 _targetTokenType, uint256 _targetChainId) external payable nonReentrant whenNotPaused {
        ...
        require(msg.value >= minETHDepositValue, "OKOK: invalid _value");

        emit TransactionDeposited(msg.sender, _to, true, msg.value, _targetTokenType, _targetChainId);
    }

    function depositERC20(address _to, uint256 _amount, uint256 _targetTokenType, uint256 _targetChainId) external nonReentrant whenNotPaused {
        require(_to != address(0x0), "OKOK: _to cannot be 0x0");
        require(erc20Addr != address(0x0), "OKOK: erc20Addr cannot be 0x0");
        require(_targetTokenType != 0, "OKOK: _targetTokenType cannot be 0");
        require(_targetChainId != 0, "OKOK: _targetChainId cannot be 0");
        require(chainIdTokenTypeToFlag[_targetChainId][_targetTokenType], "OKOK: _targetTokenType not support in _targetChainId");
        require(_amount >= minERC20DepositValue, "OKOK: invalid _value");

        uint256 beforeAmount = IERC20(erc20Addr).balanceOf(address(this));
        _safeTransferFrom(erc20Addr, msg.sender, address(this), _amount);
        uint256 afterAmount = IERC20(erc20Addr).balanceOf(address(this));

        uint256 delta = afterAmount - beforeAmount;
        require(delta >= _amount, "OKOK: not receive enough token amount!");

        emit TransactionDeposited(msg.sender, _to, false, _amount, _targetTokenType, _targetChainId);
    }

    // to help users who accidentally send their tokens to this contract
    function withdrawToken(address _token, address _account, uint256 _amount) external onlyGov nonReentrant {
        require(_token != erc20Addr, "invalid _token");
    ...
    function sendERC20address(address payable _receiver, uint256 _outAmount, uint256 _depositId) external onlyOrderKeeper nonReentrant {
        require(depositIdToStatus[_depositId] == false, "depositId already processed");
        require(_outAmount > 0, "invalid _outAmount");
        require(_receiver != address(0), "invalid _receiver");
        require(_outAmount <= IERC20(erc20Addr).balanceOf(address(this)), "_outAmount large than contract balance");
        require(_receiver != address(this), "can not send eth to myself");

        depositIdToStatus[_depositId] = true;
        IERC20(erc20Addr).safeTransfer(_receiver, _outAmount);
