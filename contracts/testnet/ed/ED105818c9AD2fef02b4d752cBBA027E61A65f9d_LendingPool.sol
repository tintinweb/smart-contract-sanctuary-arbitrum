...
interface ICalculusAPRDataProvider {
...
library ValidationLogic {
  ...
  function validateBorrow(
    ...
    //Calculus: place where stETH is blocked from being borrowed
 ...
 * @dev Main point of interaction with an Calculus protocol's market
 * - Users can:
 *   # Deposit
 *   # Withdraw
 *   # Borrow
...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  function depositYield(address asset, uint256 amount) external override whenNotPaused {
    ...
    if (aprDataProvider != address(0)) {
      ICalculusAPRDataProvider(aprDataProvider).updateAPR(asset, amount, totalAmount);
    }
