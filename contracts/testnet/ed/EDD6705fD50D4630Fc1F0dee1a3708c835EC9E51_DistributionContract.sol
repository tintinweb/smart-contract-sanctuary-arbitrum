    ...
    function distributeRewardsToHolders(uint256 ethAmount) internal {
        ...
        for (uint256 i = 0; i < token.holdersCount(); i++) {
            ...
            if (isEligible(holder)) {
                ...
                uint256 holderShare = (holderBalance * remainingDistributionEth) / token.totalSupply();
                holders[holder].accumulatedArbot += holderShare;
                holders[holder].accumulatedEth += (remainingDistributionEth * holderShare) / remainingDistributionEth;
            }
        }
    }
