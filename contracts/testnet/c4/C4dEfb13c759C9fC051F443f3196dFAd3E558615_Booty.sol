...
contract Booty is ERC1155, Ownable {
    ...
    constructor(string memory _uri, address _toucans, uint256[NUM_CRATES] memory cratePrice, uint256[NUM_CRATES] memory maxSupply, uint256[NUM_CRATES] memory maxMint, uint256[NUM_CRATES] memory minToucan) ERC1155(_uri) {

        toucans = _toucans;
    
        for (uint tier = 0; tier < NUM_CRATES; tier++) {
            crates[tier] = Crate(cratePrice[tier], 0, maxSupply[tier], maxMint[tier], minToucan[tier], new uint256[](NUM_ARTIFACT));
        }
