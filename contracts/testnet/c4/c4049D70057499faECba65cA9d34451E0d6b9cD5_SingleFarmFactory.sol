...
contract SingleFarmFactory is
    ISingleFarmFactory, IHasPausable, IHasAdministrable, IHasTreasury, IDepositConfig,
    ...
    WhitelistedTokens
{
    ...
    // address used by the backend to create a new instance
    ...
    // min investment amount of manager per farm
    ...
    // fee from the manager to pay for operator transactions (default - 1e9)
    ...
    /// @param _admin address used by the bot to close/cancel farms
    ...
    /// @notice set the min manager investment of collateral a manager must invest per farm
    /// @dev can only be called by the `owner`
    /// @param _amount min manager investment of collateral a manager must invest per farm
    /* function setMinManagerInvestmentAmount(uint256 _amount) external override onlyOwner {
        if (_amount < 1) revert ZeroAmount();
        if (_amount < minInvestmentAmount) revert BelowMin(minInvestmentAmount, _amount);
    ...
    /// @dev can only be called by the `owner`
    /// @param _maker the admin address
    function setMaker(address _maker) external onlyOwner {
        if (_maker == address(0)) revert ZeroAddress();
        maker = _maker;
        emit MakerChanged(_maker);
    }
    ...
    function getProtocolFee() public view returns(uint256, uint256) {
