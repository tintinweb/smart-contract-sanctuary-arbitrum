...
contract NftMinning is OwnableUpgradeable, IERC721ReceiverUpgradeable {
    ...
    function deposit(uint unionsId_, uint[] calldata kunId_) public isStart {
        ...
        uint _totalP;
        uint _id;
        for (uint i = 0; i < kunId_.length; i++) {
            _id = kunId_[i];
            // update user this stake power
            uint _power = baseNftPower[getKunId(_id)];
            _power = getPower(_power);

            _totalP += _power;
            // update card
            kunInfo[_id] = DataTypes.KunInfo({
                owner: msg.sender,
                kunId: _id,
                kunPower: _power,
                depositTime: block.timestamp
            });
            kunIdList[msg.sender].push(_id);

            // kun
            kun.safeTransferFrom(msg.sender, address(this), _id);
            emit Deposit(msg.sender, unionsId_, _power);
        }
        // update global
        updatePool();
        TVL += _totalP;
        pool.totalPower += _totalP;
    ...
    function withdrawAll() public isStart {
        ...
        for (uint i = 0; i < _kunIdList.length; i++) {
    ...
    function claimGeTax() public isStart {
        ...
        uint _geTax = checkGeTax(uid_);
        if (_geTax != 0) {
            unionsInfo[uid_].geTax += _geTax;

            IERC20(MSW).transfer(msg.sender, _geTax);
            emit ClaimGeTax(msg.sender, unionOwner[msg.sender], _geTax);
        }
    }
