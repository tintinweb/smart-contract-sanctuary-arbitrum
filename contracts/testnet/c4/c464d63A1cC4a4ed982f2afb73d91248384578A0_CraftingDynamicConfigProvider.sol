...
contract ActionPermit is Pausable, IActionPermit, ManagerModifier {
  using Epoch for uint256;

  mapping(uint256 => uint128) public actionToEpochConfig;
  ILastActionMarkerStorage public immutable ACTION_STORAGE;
  ILockStorage public immutable LOCK_STORAGE;

  mapping(uint => bool) public actionLockMapping;

  constructor(
    address _manager,
    address _actionStorage,
    address _lockStorage
  ) ManagerModifier(_manager) {
    ACTION_STORAGE = ILastActionMarkerStorage(_actionStorage);
    LOCK_STORAGE = ILockStorage(_lockStorage);

    actionToEpochConfig[ACTION_ADVENTURER_HOMAGE] = Epoch.toConfig(
      DAILY_EPOCH_DURATION,
      DAILY_EPOCH_OFFSET
    );
    actionToEpochConfig[ACTION_ADVENTURER_BATTLE_V3] = Epoch.toConfig(
      DAILY_EPOCH_DURATION,
      DAILY_EPOCH_OFFSET
    );
    actionToEpochConfig[ACTION_ADVENTURER_VOID_CRAFTING] = Epoch.toConfig(
      DAILY_EPOCH_DURATION,
      NO_OFFSET
    );
    actionToEpochConfig[ACTION_ADVENTURER_REALM_CRAFTING] = Epoch.toConfig(
      DAILY_EPOCH_DURATION,
      NO_OFFSET
    );
  }
  ...
  function checkAndMarkActionComplete(
    ...
    uint256 _action,
    uint256 _minEpochs
  ) public whenNotPaused onlyManager returns (uint256 _epochsSinceLastAction, uint currentEpoch) {
    if (_minEpochs <= 0) {
      revert MinEpochsTooLow(_minEpochs);
    }

    checkPermissions(_sender, _tokenAddr, _tokenId, _action);
    (_epochsSinceLastAction, currentEpoch) = checkIfEnoughEpochsElapsed(
      _tokenAddr,
      _tokenId,
      _action,
      _minEpochs
    );
    ACTION_STORAGE.setActionMarker(_tokenAddr, _tokenId, _action, block.timestamp);
  }
  ...
  function checkAndMarkActionCompleteMany(
    ...
    uint256 _action,
    uint256 _minEpochs
  ) public whenNotPaused onlyManager returns (uint256 currentEpoch) {
    if (_minEpochs <= 0) {
      revert MinEpochsTooLow(_minEpochs);
    }

    require(_tokenAddrs.length > 0);

    checkPermissionsMany(_sender, _tokenAddrs, _tokenIds, _action);
    for (uint i = 0; i < _tokenAddrs.length; i++) {
      (, currentEpoch) = checkIfEnoughEpochsElapsed(
        _tokenAddrs[i],
        _tokenIds[i],
        _action,
        _minEpochs
    ...
    ACTION_STORAGE.setActionMarkerMany(_tokenAddrs, _tokenIds, _action, block.timestamp);
  }

  function forceMarkActionComplete(
    address _tokenAddr,
    uint256 _tokenId,
    uint256 _action
  ...
  function lock(address _tokenAddr, uint _tokenId, uint _lockDuration) external {
    _checkPermissionsInternal(msg.sender, _tokenAddr, _tokenId, ACTION_LOCK);
    LOCK_STORAGE.lockEntity(_tokenAddr, _tokenId, _lockDuration);
  }
  ...
  function lockBatch(
    ...
    checkPermissionsMany(msg.sender, _tokenAddrs, _tokenIds, ACTION_LOCK);
    LOCK_STORAGE.lockEntityBatch(_tokenAddrs, _tokenIds, _lockDuration);
  }
  ...
  function checkPermissions(
    ...
    if (actionLockMapping[_action]) {
      (bool isLocked, uint lockedUntil) = LOCK_STORAGE.isLocked(_tokenAddr, _tokenId);
      if (isLocked) {
        revert EntityLocked(_tokenAddr, _tokenId, lockedUntil);
      }
    }

    _checkPermissionsInternal(_sender, _tokenAddr, _tokenId, _action);
  }
  ...
  function checkPermissionsMany(
    ...
    for (uint i = 0; i < _tokenIds.length; i++) {
      checkPermissions(_sender, _tokenAddr, _tokenIds[i], _action);
    }
  }
  ...
  function checkPermissionsMany(
    ...
    require(_tokenAddrs.length == _tokenIds.length, "Invalid tokenId length");
    if (actionLockMapping[_action]) {
      (bool isLocked, address lockedAddr, uint lockedToken, uint lockedUntil) = LOCK_STORAGE
        .isLockedBatch(_tokenAddrs, _tokenIds);
      if (isLocked) {
        revert EntityLocked(lockedAddr, lockedToken, lockedUntil);
      }
    }

    for (uint i = 0; i < _tokenAddrs.length; i++) {
      _checkPermissionsInternal(_sender, _tokenAddrs[i], _tokenIds[i], _action);
    }
  }

  function _checkPermissionsInternal(
    address _sender,
    address _tokenAddr,
    uint256 _tokenId,
    uint256 _action
  ) internal view {
    if (ERC721(_tokenAddr).ownerOf(_tokenId) != _sender) {
      revert Unauthorized(_tokenAddr, _tokenId);
    }
  }
  ...
  function checkIfEnoughEpochsElapsed(
    ...
    uint256 _action,
    uint256 _minEpochs
  ) public view returns (uint256 result, uint256 currentEpoch) {
    uint128 epochConfig = actionToEpochConfig[_action];
    currentEpoch = block.timestamp.toEpochNumber(epochConfig);
    uint256 epochSpan = ACTION_STORAGE.getActionMarker(_tokenAddr, _tokenId, _action).toEpochNumber(
      epochConfig
    );

    if (currentEpoch >= epochSpan) {
      epochSpan = currentEpoch - epochSpan;
    } else {
      revert DuplicateActionAttempt(_tokenAddr, _tokenId);
    }

    if (epochSpan < _minEpochs) {
      revert InsufficientEpochSpan(_minEpochs, epochSpan, _tokenAddr, _tokenId);
    }

    result = currentEpoch - epochSpan;
  }

  function updateActionEpochConfig(
    uint256 _action,
    uint64 _duration,
    uint64 _offset
  ) external onlyAdmin {
    actionToEpochConfig[_action] = Epoch.toConfig(_duration, _offset);
  }

  function lockAction(uint action) external onlyAdmin {
    actionLockMapping[action] = true;
  }

  function unlockAction(uint action) external onlyAdmin {
    actionLockMapping[action] = false;
  }
}
...
import "../Action/ActionDemandAverageStorage.sol";
import "../Action/Actions.sol";
import "../Action/ActionPermit.sol";
import "../Utils/Epoch.sol";
...
contract CraftingDynamicConfigProvider is ManagerModifier {
  LabSupplyAverageStorage public immutable LAB_SUPPLY_STORAGE;
  ActionDemandAverageStorage public immutable ACTION_DEMAND_STORAGE;

  uint256 voidCostDynamicWeightFactor;
  uint256 baseVoidCostPercentage;
  mapping(uint256 => uint256) dailyProductivityPool;

  constructor(
    address _manager,
    address _labSupply,
    address _actionDemand
  ) ManagerModifier(_manager) {
    LAB_SUPPLY_STORAGE = LabSupplyAverageStorage(_labSupply);
    ACTION_DEMAND_STORAGE = ActionDemandAverageStorage(_actionDemand);
    voidCostDynamicWeightFactor = 4 * ONE_HUNDRED; // 4x demand
    baseVoidCostPercentage = (ONE_HUNDRED * 11) / 10; // 110%
  }

  function getConfigBatch(
    uint256[] calldata labIds
  ) external returns (DynamicLabCraftingConfig[] memory result) {
    result = new DynamicLabCraftingConfig[](labIds.length);
    uint256[] memory labSupply = LAB_SUPPLY_STORAGE.getSupplyBatch(labIds);
    uint256[] memory voidDemand = ACTION_DEMAND_STORAGE.getDemandBatch(
      ACTION_ADVENTURER_VOID_CRAFTING,
      labIds
    );
    uint256[] memory realmDemand = ACTION_DEMAND_STORAGE.getDemandBatch(
      ACTION_ADVENTURER_REALM_CRAFTING,
      labIds
    );

    for (uint i = 0; i < labIds.length; i++) {
      result[i] = _getConfig(labIds[i], labSupply[i], voidDemand[i], realmDemand[i]);
    }
  }
  ...
  function _getConfig(
    ...
    uint _voidDemand,
    uint _realmDemand
  ) internal view returns (DynamicLabCraftingConfig memory result) {
    _voidDemand = _voidDemand + 1;
    _realmDemand = _realmDemand + 1;
    uint totalDemand = (_voidDemand + _realmDemand - 1);

    result.realmCraftingCostPercentage = ONE_HUNDRED;
    result.voidCraftingCostPercentage =
      baseVoidCostPercentage +
      ((voidCostDynamicWeightFactor * _voidDemand) / (_realmDemand)); // base + (4 * void_demand) / _realmDemand
    result.productivityPerCraft = dailyProductivityPool[_labId] / totalDemand;
    result.durabilityLossPercentage = (_labSupply / totalDemand) + 1;

    if (result.durabilityLossPercentage < 1000) {
      result.durabilityLossPercentage = 1000;
    } else if (result.durabilityLossPercentage > ONE_HUNDRED) {
      result.productivityPerCraft =
        (ONE_HUNDRED * result.productivityPerCraft) /
        result.durabilityLossPercentage;
      result.durabilityLossPercentage = ONE_HUNDRED;
    }
  }

  function updateProductivityPools(
    uint256[] calldata _labIds,
    uint256[] calldata _productivityPools
  ) external onlyAdmin {
    require(_labIds.length == _productivityPools.length);

    for (uint i = 0; i < _labIds.length; i++) {
      dailyProductivityPool[_labIds[i]] = _productivityPools[i];
    }
  }
}
