    ...
    ICoverPoolMintCallback
{
    ...
    )
    {
        ...
        for (uint i = 0; i < pools.length;) {
            ...
            emit PoolQuoted(pools[i], results[i].amountIn.toUint256(), results[i].amountOut.toUint256(), results[i].priceAfter);
            unchecked {
                ++i;
            }
        }
    ...
    function sortQuoteResults(
        ...
        bool[] memory sortedFlags = new bool[](results.length);
        for (uint sorted = 0; sorted < results.length;) {
            ...
            if (sortIndex != type(uint256).max) {
                emit PoolQuoteSorted(results[sortIndex].pool, results[sortIndex].amountIn.toUint256(), results[sortIndex].amountOut.toUint256(), results[sortIndex].priceAfter, sortIndex, sorted);
