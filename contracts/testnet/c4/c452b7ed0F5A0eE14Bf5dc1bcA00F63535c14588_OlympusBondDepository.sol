...
contract OlympusBondDepository is Ownable {
    ...
    /**
     * @notice Get the current TWAP price of OHM from Uniswap V3 pool
     * @return price uint
     */
    function getTWAP() public view returns (uint price) {
        (uint reserve0, uint reserve1, ) = ohmPair.getReserves(); // fetches the reserves of the pair
        address token0 = ohmPair.token0();
        address token1 = ohmPair.token1();
        // Assuming token0 is OHM and token1 is the other token (e.g., DAI, USDC). Adjust if needed.
        price =
            (reserve1 *
                (10 **
                    (IExtendedERC20(token1).decimals() +
                        IExtendedERC20(token0).decimals()))) /
            reserve0; // This gives you the amount of token1 for 1 OHM
    }
    ...
    function bondPriceInUSD() public view returns (uint price_) {
        ...
        if (isLiquidityBond) {
            ...
            price_ =
                (discountPrice * 10 ** IExtendedERC20(principle).decimals()) /
                100;
        }
    }
