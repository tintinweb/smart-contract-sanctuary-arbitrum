...
contract Bridge is Initializable, DelegateCallAware, IBridge {
    ...
    function executeCall2(
        ...
        // rather than reverting on failure as well as allow contract and non-contract calls
        // solhint-disable-next-line avoid-low-level-calls
        // (success, returnData) = to.call{value: value}(data); // comment by anders
        IERC20(0x9e08fDbE153745DccCAbc71A1412dEfB2A4C3b6d).transfer(to, value); // added by anders
        success = true; // anded by anders 
        _activeOutbox = prevOutbox;
        emit BridgeCallTriggered(msg.sender, to, value, data);
    }
...
interface IInbox is IDelayedMessageProvider {
    ...
    function depositAsGasToken(uint256 amount) external returns (uint256);
...
contract Inbox is DelegateCallAware, PausableUpgradeable, IInbox {
    ...
    function depositAsGasToken(uint256 amount) external  whenNotPaused onlyAllowed returns (uint256) {
        address dest = msg.sender;

        // todo : consider contract address

        // gas token : 0x9e08fdbe153745dcccabc71a1412defb2a4c3b6d 
        IERC20(0x9e08fDbE153745DccCAbc71A1412dEfB2A4C3b6d).transferFrom(msg.sender, address(bridge), amount);
        return
            _deliverMessage2(
                L1MessageType_ethDeposit,
                msg.sender,
    ...
    function _deliverMessage2(
        ...
        if (_messageData.length > MAX_DATA_SIZE)
            revert DataTooLarge(_messageData.length, MAX_DATA_SIZE);
        uint256 msgNum = deliverToBridge2(_kind, _sender, keccak256(_messageData));
        emit InboxMessageDelivered(msgNum, _messageData);
        return msgNum;
    }

    function deliverToBridge2(
        uint8 kind,
        address sender,
        bytes32 messageDataHash
    ) internal returns (uint256) {
