...
contract MagicNFT is ERC721A, Ownable {
    ...
    uint256 public constant MAGIC_PRICE = 110 ether;

    uint256 public whitelistStart;
    uint256 public whitelistEnd;
    uint256 public reserveListStart;
    uint256 public reserveListEnd;
    ...
    function whitelistMint(bytes32[] calldata _merkleProof, uint256 quantity) external payable {
        ...
        require(_whitelistClaimed[account] == 0, "ALREADY_CLAIMED");
        require(verifyWhitelist(_merkleProof, account, quantity), "PRESALE_NOT_VERIFIED");
        require(totalSupply() + quantity <= COLLECTION_SIZE, "EXCEEDS_COLLECTION_SIZE");
    ...
    function reserveListMint(bytes32[] calldata _merkleProof, uint256 quantity) external payable {
        address account = _msgSender();
        require(isReserveListSaleActive(), "PRESALE_INACTIVE");
        require(_reserveListClaimed[account] == 0, "ALREADY_CLAIMED");
        require(verifyReservelist(_merkleProof, account, quantity), "PRESALE_NOT_VERIFIED");
        require(totalSupply() + quantity <= COLLECTION_SIZE, "EXCEEDS_COLLECTION_SIZE");
        uint256 cost = quantity * ETHER_PRICE;
        require(msg.value >= cost, "VALUE_TOO_LOW");
        _reserveListClaimed[account] = 1;
        _safeMint(account, quantity);
    }
