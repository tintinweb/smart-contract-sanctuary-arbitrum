...
struct DappInfo {
    ...
    address referrer;
    uint32 groupCounts;
    uint32 validGroupCounts;
    uint256 groupAmount;
    uint32 inviteCounts;
    uint32 validInviteCounts;
...
contract Reserve is Ownable {
    ...
    mapping (address => uint32) private _validGroupCounts;
    mapping (address => uint32) private _inviteCounts;
    mapping (address => uint32) private _validInviteCounts;
    ...
    event Join(address indexed account, address referrer);
    ...
    function dappInfo(address account) view external returns(DappInfo memory) {
        ...
        DappInfo memory info = DappInfo({
            ...
            validInviteCounts: _validInviteCounts[account],
            inviteAmount: _inviteAmount[account],
            isRisk: isRisk,
            isWhitelist: _whitelist[account]
        });
    ...
    function deposit(uint256 amount) external {
        ...
        _updateValidGroup(_userJoined[account], account, amount, 8);
    ...
    function withdraw() external {
        ...
        if (prizeAmount > 0) {
            _consensePool.withdraw(account, prizeAmount);
        ...
        emit Withdraw(account, amount + prizeAmount);
    }
    ...
    function _updateValidGroup(bool joined, address account, uint256 amount, uint8 deep) internal {
        if (deep == 0) return;
        ...
        if (deep == 8) {
            if (!joined) _validInviteCounts[ref] += 1;
            _inviteAmount[ref] += amount;
        }
        if (!joined) _validGroupCounts[ref] += 1;
        _groupAmount[ref] += amount;

        _updateValidGroup(joined, ref, amount, deep - 1);
    }
}