 ...
 * for minting and redeeming USG, as well as depositing and withdrawing collateral.
...
contract USGEngine is ReentrancyGuard, Ownable {
    ...
    address private WETH = 0xEe01c0CD76354C383B8c7B4e65EA88D00B06f36f;
    ...
    mapping(address user => uint256 amount) private s_USGMinted;
    ...
    function redeemCollateral(
        address tokenCollateralAddress,
        uint256 amountCollateral
    ) external payable moreThanZero(amountCollateral) nonReentrant {
        _redeemCollateral(tokenCollateralAddress, amountCollateral, msg.sender, msg.sender);
        revertIfHealthFactorIsBroken(msg.sender);
    }
     ...
     */
    function burnUsg(uint256 amount) external moreThanZero(amount) {
    ...
    function liquidate(
        ...
        address user,
        uint256 debtToCover
    ) external payable moreThanZero(debtToCover) nonReentrant {
     ...
     */
    function mintUsg(uint256 amountUsgToMint) public moreThanZero(amountUsgToMint) nonReentrant {
    ...
    function getMintableUSG(
        ...
        uint256 totalCollateralValueInUsdPreSupply;
        for (uint256 index = 0; index < s_collateralTokens.length; index++) {
            address token = s_collateralTokens[index];
            uint256 amount = s_collateralDeposited[user][token];
            if (tokenCollateralAddress == token) {
                totalCollateralValueInUsdPreSupply += _getUsdValue(
                    token,
                    amount + amountCollateral
                );
            } else {
                totalCollateralValueInUsdPreSupply += _getUsdValue(token, amount);
            }
        }

        (uint256 totalUsgMinted, ) = _getAccountInformation(user);

        uint256 totalUsgMintableAmount = (totalCollateralValueInUsdPreSupply *
    ...
    function getBurnableUSG(
        ...
        (uint256 totalUsgMinted, uint256 totalCollateralInUSD) = _getAccountInformation(user);
        uint256 totalUsgAfterBurn = 0;
        if (amountUSG < totalUsgMinted) {
            totalUsgAfterBurn = totalUsgMinted - amountUSG;
        }
        uint256 totalCollateralInUSDAfterBurn = (totalUsgAfterBurn * 100) / LIQUIDATION_THRESHOLD;
        if (totalCollateralInUSDAfterBurn >= totalCollateralInUSD) {
            return 0;
        } else {
            uint256 tokenCollateralInUSD = totalCollateralInUSD - totalCollateralInUSDAfterBurn;
            uint256 tokenAmount = getTokenAmountFromUsd(
                tokenCollateralAddress,
                tokenCollateralInUSD
            );
            return tokenAmount;
        }
    }
    ...
    function _burnUsg(uint256 amountUsgToBurn, address onBehalfOf, address usgFrom) private {
        if (s_USGMinted[onBehalfOf] >= amountUsgToBurn) {
            s_USGMinted[onBehalfOf] -= amountUsgToBurn;
        }
        s_USGMinted[onBehalfOf] = 0;
    ...
    function _getAccountInformation(
        address user
    ) private view returns (uint256 totalUsgMinted, uint256 collateralValueInUsd) {
        totalUsgMinted = s_USGMinted[user];
        collateralValueInUsd = getAccountCollateralValue(user);
    }

    function _healthFactor(address user) private view returns (uint256) {
        (uint256 totalUsgMinted, uint256 collateralValueInUsd) = _getAccountInformation(user);
    ...
    function getAccountInformation(
        address user
    ) external view returns (uint256 totalUsgMinted, uint256 collateralValueInUsd) {
