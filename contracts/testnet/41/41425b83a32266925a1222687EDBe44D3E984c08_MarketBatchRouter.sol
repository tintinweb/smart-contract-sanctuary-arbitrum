...
contract MarketBatchRouter is CloberMarketSwapCallbackReceiver, CloberBatchRouter {
    ...
    function batchLimitBid(LimitOrderParams[] calldata paramsList) external payable {
        for (uint256 i = 0; i < paramsList.length; ++i) {
            _limitOrder(paramsList[i], _BID);
        }
    }

    function batchLimitAsk(LimitOrderParams[] calldata paramsList) external payable {
        for (uint256 i = 0; i < paramsList.length; ++i) {
            _limitOrder(paramsList[i], _ASK);
        }
    }
    ...
    function batchLimitBidAfterClaim(
        ClaimOrderParams[] calldata claimParamsList,
        LimitOrderParams[] calldata limitOrderParamsList
    ) external payable {
        _claim(claimParamsList);
        for (uint256 i = 0; i < limitOrderParamsList.length; ++i) {
            _limitOrder(limitOrderParamsList[i], _BID);
        }
    }

    function batchLimitAskAfterClaim(
        ClaimOrderParams[] calldata claimParamsList,
        LimitOrderParams[] calldata limitOrderParamsList
    ) external payable {
        _claim(claimParamsList);
        for (uint256 i = 0; i < limitOrderParamsList.length; ++i) {
            _limitOrder(limitOrderParamsList[i], _ASK);
        }
    }
}
...
interface CloberBatchRouter {
     ...
     * @param paramsList The batch of limit order parameters.
     */
    function batchLimitBid(LimitOrderParams[] calldata paramsList) external payable;
     ...
     * @param limitOrderParamsList Array of limitOrderParams: The batch of parameters for the limit order.
     */
    function batchLimitBidAfterClaim(
        ClaimOrderParams[] calldata claimParamsList,
        LimitOrderParams[] calldata limitOrderParamsList
