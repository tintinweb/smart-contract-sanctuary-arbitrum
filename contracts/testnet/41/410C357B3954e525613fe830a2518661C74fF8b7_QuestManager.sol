...
import "../Boost/IBoostStorage.sol";
...
contract QuestManager is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  // Arrays
  //========================================
  uint256[] public bonusProbability = [70, 85, 95, 98, 100];
  ...
  function updateProbability(uint256[] calldata _value) external onlyAdmin {
    ...
    )
  {
    ...
    if (_boost == 1) {
      // Get anima bonus
      uint256 animaBonus = _getAnimaBonus(_adventurerId);
    ...
    emit Quested(_questId, cost, _addr, _adventurerId, xp, anima);
