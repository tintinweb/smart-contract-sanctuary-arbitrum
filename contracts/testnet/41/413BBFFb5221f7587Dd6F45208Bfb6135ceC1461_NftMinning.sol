...
contract NftMinning is OwnableUpgradeable, IERC721ReceiverUpgradeable {
    ...
    event ChangeTopUnion(uint indexed unionId, uint indexed topUnionId);
    ...
    function init() public initializer {
        ...
        // MSW = ;
        unions = IMSW(0x25F982641bD7111dF3f9e0166CE24515937EdE52);
    ...
    function setTopUnions(uint topUnionId_, uint unionId_) public onlyOwner {
        require(
            unionsInfo[unionId_].owner != address(0) &&
                unionsInfo[unionId_].topUnions == sytemUnions,
            "no good!"
        );
        require(
        ...
        emit ChangeTopUnion(unionId_, topUnionId_);
    }
    ...
    function checkPowerX() public view returns (uint _final) {
        uint _day = (block.timestamp - pool.startTime) / 1 days;
