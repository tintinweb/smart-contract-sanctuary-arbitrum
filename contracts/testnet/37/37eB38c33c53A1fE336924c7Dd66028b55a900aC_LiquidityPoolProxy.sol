...
abstract contract LiquidityPool is IOceanPrimitive {
        ...
        returns (uint256 inputAmount)
    {
        ...
        if (uint256(maximumInputAmount) > 0) {
            require(uint256(maximumInputAmount) >= inputAmount, "Slippage limit exceeded");
        }
    }
...
contract LiquidityPoolProxy is LiquidityPool, Ownable {
    ...
    {
        (uint256 xBalance, uint256 yBalance) = _getBalances();
        outputAmount = implementation.swapGivenInputAmount(xBalance, yBalance, inputAmount, _specifiedToken(inputToken));
    }
        ...
        returns (uint256 mintAmount)
    {
        ...
        mintAmount = implementation.depositGivenInputAmount(
            xBalance, yBalance, totalSupply, depositAmount, _specifiedToken(depositToken)
        );
    }
        ...
        returns (uint256 inputAmount)
    {
        (uint256 xBalance, uint256 yBalance) = _getBalances();
        inputAmount =
            implementation.swapGivenOutputAmount(xBalance, yBalance, outputAmount, _specifiedToken(outputToken));
    }
        ...
        returns (uint256 burnAmount)
    {
        ...
        burnAmount = implementation.withdrawGivenOutputAmount(
            xBalance, yBalance, totalSupply, withdrawnAmount, _specifiedToken(withdrawnToken)
        );
    }
