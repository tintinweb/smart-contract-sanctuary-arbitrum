...
contract ChildRewarder is IRewarder, Ownable, ReentrancyGuard {
    ...
    event AdminTokenRecovery(address _tokenAddress, uint256 _amt, address _adr);
    event LogInit();
    ...
    function onReward(
        ...
        uint256,
        uint256 _amt
    ) external override onlyParent nonReentrant {
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 reward = totalAllocPoint == 0 ? 0 : ((time * rewardPerSecond * pool.allocPoint) / totalAllocPoint);
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                uint256 time = block.timestamp - pool.lastRewardTime;
                uint256 reward = totalAllocPoint == 0
                    ? 0
...
/// @notice The (older) MasterChef contract gives out a constant number of ARBS tokens per second.
/// It is the only address with minting rights for ARBS.
...
contract MasterChefV2 is Ownable {
    ...
    /// @notice Info of each MCV2 user.
    /// `amount` LP token amount the user has provided.
    /// `rewardDebt` The amount of ARBS entitled to the user.
    struct UserInfo {
        uint256 amount;
        uint256 rewardDebt;
    }
    ...
    /// @notice Info of each MCV2 pool.
    /// `allocPoint` The amount of allocation points assigned to the pool.
    /// Also known as the amount of ARBS to distribute per second.
    struct PoolInfo {
        uint128 accArbsPerShare;
        uint64 lastRewardTime;
        uint64 allocPoint;
    }
    ...
    /// @notice Address of ARBS contract.
    IERC20 public ARBS;
    ...
    /// @notice Record whether LP was added.
    ...
    uint256 private constant ACC_ARBS_PRECISION = 1e12;
    ...
    event LogSetPool(uint256 indexed pid, uint256 allocPoint, IRewarder rewarder, bool overwrite, bool update);
    event LogUpdatePool(uint256 indexed pid, uint256 lastRewardTime, uint256 lpSupply, uint256 accArbsPerShare);
    event LogInit();
    event SetSupportV1Pool(bool supportV1Pool);
    event SetV1Pool(IMasterChef _MASTER_CHEF, IERC20 _arbs, uint256 _MASTER_PID);

    constructor() {}

    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting rights for ARBS.
    ...
    /// @notice View function to see pending ARBS on frontend.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _user Address of user.
    /// @return pending ARBS reward for a given user.
    function pendingArbs(uint256 _pid, address _user) external view validatePid(_pid) returns (uint256 pending) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 arbsReward = totalAllocPoint == 0
                ? 0
                : ((multiplier * arbsPerSecond() * pool.allocPoint) / totalAllocPoint);
            accArbsPerShare = accArbsPerShare + ((arbsReward * ACC_ARBS_PRECISION) / lpSupply);
        }
        pending = ((user.amount * accArbsPerShare) / ACC_ARBS_PRECISION) - user.rewardDebt;
    }
    ...
    /// @notice Calculates and returns the `amount` of ARBS per second allocated to this contract
    function arbsPerSecond() public view returns (uint256 amount) {
        amount = !supportV1Pool
            ? 0
            : (MASTER_CHEF.arbsPerSecond() * MASTER_CHEF.poolInfo(MASTER_PID).allocPoint) /
    ...
    function _updatePool(uint256 pid) internal validatePid(pid) returns (PoolInfo memory pool) {
        pool = poolInfo[pid];
        if (supportV1Pool && block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                ...
                pool.accArbsPerShare = uint128(pool.accArbsPerShare + ((arbsReward * ACC_ARBS_PRECISION) / lpSupply));
            }
            pool.lastRewardTime = uint64(block.timestamp);
            poolInfo[pid] = pool;
            emit LogUpdatePool(pid, pool.lastRewardTime, lpSupply, pool.accArbsPerShare);
        }
    }
    ...
    /// @notice Deposit LP tokens to MCV2 for ARBS allocation.
    /// @param pid The index of the pool. See `poolInfo`.
    ...
    function _deposit(
        ...
        // Effects
        uint256 _pendingArbs = ((user.amount * pool.accArbsPerShare) / ACC_ARBS_PRECISION) - user.rewardDebt;

        user.amount += amount;
        user.rewardDebt = (user.amount * pool.accArbsPerShare) / ACC_ARBS_PRECISION;

        // Interactions
        if (_pendingArbs != 0) {
            ARBS.safeTransfer(to, _pendingArbs);
        }

        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onReward(pid, to, to, _pendingArbs, user.amount);
        }

        lpToken[pid].safeTransferFrom(msg.sender, address(this), amount);

        emit Deposit(msg.sender, pid, amount, to);
        emit Harvest(msg.sender, pid, _pendingArbs);
    }
    ...
    /// @param to Receiver of the LP tokens and ARBS rewards.
    function _withdraw(
        uint256 pid,
        uint256 amount,
        address to
        ...
        IRewarder _rewarder = rewarder[pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onReward(pid, msg.sender, to, _pendingArbs, user.amount);
        }
    ...
    function harvestAll() external {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (user.amount > 0) {
                ...
                calc = (user.amount * pool.accArbsPerShare) / ACC_ARBS_PRECISION;
        ...
        if (totalPending > 0) {
            ARBS.safeTransfer(msg.sender, totalPending);
        }
    }
    ...
    /// @notice Harvests ARBS from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to this MCV2 contract.
    ...
    function queryHarvestFromMasterChef() public {
        if (supportV1Pool && block.timestamp - lastV1HarvestTimestamp > V1_HARVEST_QUERY_TIME) harvestFromMasterChef();
    }
    ...
    /// @notice Update the given pool's ARBS allocation point and `IRewarder` contract. Can only be called by the owner.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _allocPoint New AP of the pool.
    ...
    /// @notice Batch update the given pool's ARBS allocation point and `IRewarder` contract. Can only be called by the owner.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _allocPoint New AP of the pool.
    ...
    function setSupportV1Pool(bool _s) external onlyOwner {
        supportV1Pool = _s;
        emit SetSupportV1Pool(_s);
    }

    /// @param _MASTER_CHEF The SpookySwap MCV1 contract address.
    /// @param _arbs The ARBS token contract address.
    ...
    function setV1Pool(
        ...
        emit SetV1Pool(_MASTER_CHEF, _arbs, _MASTER_PID);
    }
}
...
interface IMasterChef {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ARBS to distribute per second.
        uint256 lastRewardBlock; // Last block number that SUSHI distribution occurs.
        uint256 accBooPerShare; // Accumulated ARBS per share, times 1e12. See below.
    }
    ...
    function arbsPerSecond() external view returns (uint256);
