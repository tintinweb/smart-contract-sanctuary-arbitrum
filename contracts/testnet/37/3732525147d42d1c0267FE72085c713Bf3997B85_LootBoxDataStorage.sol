  ...
  Pausable
{
  ...
  function name(uint256 _tokenId) external view returns (string memory) {
    uint16[16] memory chars = lootBoxes[_tokenId].characteristics;
    string memory typeName = characteristicDefinitions.types(
      chars[LOOTBOX_CHARACTERISTIC_TYPE]
    );
    return string.concat(typeName, " LootBox");
  }
