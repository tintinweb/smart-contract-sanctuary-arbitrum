...
contract Bank is AccessControlEnumerable, KeeperCompatibleInterface, Multicall {
    ...
    /// @dev Duplicate code with PvPGame <- Update in both contracts

    /// @notice emitted when starting a token's partner transfer
    event TokenPartnerTransferStarted(address indexed token, address newPartner);
    /// @notice emitted when accepting a token's partner transfer
    event TokenPartnerTransferAccepted(address indexed token, address newPartner);
