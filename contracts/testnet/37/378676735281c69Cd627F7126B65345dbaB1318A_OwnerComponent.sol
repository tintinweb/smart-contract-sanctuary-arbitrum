...
import "./IAddressComponent.sol";

contract AddressComponent is BaseComponent, IAddressComponent {
    /** SETUP **/
     ...
     */
    function setValue(uint256 entity, address value) external virtual {
    ...
    function getValue(
        uint256 entity
    ) external view virtual returns (address value) {
        if (has(entity)) {
            value = abi.decode(getBytes(entity), (address));
        }
    }
}
...
interface IAddressComponent is IComponent {
    /**
     * Sets the typed value for this component
     *
     * @param entity Entity to get value for
     */
    function setValue(uint256 entity, address value) external;
     ...
     * @param entity Entity to get value for
     */
    function getValue(uint256 entity) external view returns (address);
}
...
contract OwnerComponent is AddressComponent {
    ...
    constructor(
        address gameRegistryAddress
    ) AddressComponent(gameRegistryAddress, ID) {
        // Do nothing
    }
}