...
contract Wheel is Core {
  ...
  function bet(
    ...
    participants[currentGameId_][player_] = Bet(_color, _wager, _token, _referralReward, _vWINRAmount);
  ...
  function refundGame(uint256 _gameId) external nonReentrant {
    ...
    vaultManager.refund(bet_.token, _amount, bet_.mintedVWINR, sender_);
    vaultManager.removeReferralReward(bet_.token, sender_, bet_.referralReward);
  }
