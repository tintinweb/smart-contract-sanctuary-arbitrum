 ...
 * @author Helix
 */
contract PoolTokens is IPoolTokens, BaseUpgradeablePausable, ERC721PresetMinterPauserAutoIdUpgradeSafe {  
  ...
  */
  ...
  function initialize(address owner, IHelixConfig _config) external initializer {
    ...
    __ERC721_init_unchained("Helix V2 Pool Tokens", "HLX-V2-PT");
    __Pausable_init_unchained();
    ...
    // Now allow to transfer NFT between investors
    _pause();
  }
  ...
  function mint(
    ...
    pool.totalPrincipalInvestment = pool.totalPrincipalInvestment.add(params.principalAmount);
  ...
  function redeem(
    ...
    require(pool.totalPrincipalRedeemed <= pool.totalPrincipalInvestment, "Cannot redeem more than we minted");
...
//   }
  ...
  function calculateTokenId(address poolAddress, address investor) public override view returns(uint256) {
    return uint256(keccak256(abi.encodePacked(poolAddress, investor, address(this))));
  }
  ...
  /// @inheritdoc IPoolTokens
  function onLoanCreated(address newPool) external override onlyHelixFactory {
  ...
  function _createToken(
    ...
    tokenId = calculateTokenId(poolAddress, mintTo);

    TokenInfo storage userTokenInfo = tokens[tokenId];

    if (userTokenInfo.pool == address(0)) {
      userTokenInfo.pool = poolAddress;
  ...
  function poolTVL(address poolAddress) public view override returns(uint256) {
    PoolInfo storage pool = pools[poolAddress];
    return pool.totalPrincipalInvestment - pool.totalPrincipalRedeemed; 
  }
  ...
  function _beforeTokenTransfer(
    ...
    uint256 tokenId,
    uint256 amount
  ) internal virtual whenNotPaused override(ERC721PresetMinterPauserAutoIdUpgradeSafe) {
