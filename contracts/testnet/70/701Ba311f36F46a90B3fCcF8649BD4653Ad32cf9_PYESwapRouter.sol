...
library PYESwapLibrary {
    ...
    function amountsOut(uint amountIn, address[] memory path, uint totalFee, uint _adminFee) internal view returns (uint[] memory) {
        return getAmountsOut(amountIn, path, totalFee, _adminFee);
    }

    function amountsIn(uint amountOut, address[] memory path, uint totalFee, uint _adminFee) internal view returns (uint[] memory) {
    ...
    function getAmountsOut(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            uint adjustedIn = path.length > 2 && i > 0 && _feeToken == path[i] ? 
                amounts[i] : ((amounts[i] * (10000 - _adminFee)) / 10000);
    ...
    function getAmountsIn(
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            ...
            uint adjustedOut = path.length > 2 && i < amounts.length - 1 && _feeToken == path[i - 1] ? 
                amounts[i] : ((amounts[i] * 10000) / (10000 - _adminFee));
