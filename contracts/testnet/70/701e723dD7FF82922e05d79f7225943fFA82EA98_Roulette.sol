...
abstract contract Game is Ownable, Pausable, Multicall, VRFConsumerBaseV2, ReentrancyGuard {
    ...
    /// @notice Leftover VRF cost.
    uint256 private leftoverVRFCost;
    ...
    /// @notice Emitted after leftover VRF cost amount is transfered.
    /// @param user Address of the user.
    /// @param leftoverVRFCost Number of tokens refunded.
    event DistributeLeftoverVRFCost(address indexed user, uint256 leftoverVRFCost);

    /// @notice Emitted after leftover VRF cost amount is accounted.
    /// @param user Address of the user.
    /// @param leftoverVRFCost Number of leftover tokens.
    event LeftoverVRFCost(address indexed user, uint256 leftoverVRFCost);

    /// @notice No leftover Chainlink fee.
    error NoLeftoverVRFCost();
    ...
    /// @notice Calculates the leftover VRF cost based on the gas consumed.
    ...
    function _accountVRFCost(Bet storage bet, uint256 startGas) internal {
        ...
        if (actualVRFCost > bet.vrfCost) {
            ...
            uint256 remainedVRFCost = bet.vrfCost - actualVRFCost;
            leftoverVRFCost += remainedVRFCost;
            bet.vrfCost = actualVRFCost;
            emit LeftoverVRFCost(bet.user, remainedVRFCost);
        }
    ...
    /// @notice Withdraw leftover Chainlink fees.
    function withdrawLeftoverVRFCost() external onlyOwner {
        uint256 leftoverVRFCostToWithdraw = leftoverVRFCost;

        if (leftoverVRFCostToWithdraw == 0) {
            revert NoLeftoverVRFCost();
        }
        ...
        Address.sendValue(payable(address(bank)), leftoverVRFCostToWithdraw);
        emit DistributeLeftoverVRFCost(address(bank), leftoverVRFCostToWithdraw);
    }
    ...
    /// @notice Returns the amount of leftover VRF cost.
    /// @return The leftover VRF cost amount.
    function getLeftoverVRFCost() external view returns (uint256) {
        return leftoverVRFCost;
    }
}


// File contracts/games/Roulette.sol
...
contract Roulette is Game {
    ...
    function _getPayout(uint256 betAmount, uint40 numbers) private pure returns (uint256) {
        // betAmount * MODULO represents 37 number fields of roulette with precision of 10k (100%)
        // numbers * POPCNT_MULT spreads out the chosen number to uint256
        // & POPCNT_MASK is shredding the unnecessary values
        // % POPCNT_MODULO is dividing the final uint256 value in a way which counts up all the selected numbers
        // (((numbers * POPCNT_MULT) & POPCNT_MASK) % POPCNT_MODULO) returns the number of selected numbers
        // final value returned is number of roulette fields with precision divided by the number of selected fields
        return (betAmount * MODULO) / (((numbers * POPCNT_MULT) & POPCNT_MASK) % POPCNT_MODULO);
    }
    ...
    function wager(
        ...
        // Assert that last 3 bits of numners are empty bc there are 37 numbers to select in roulette including zero
        if (numbers == 0 || numbers >= 2**MODULO - 1) {
            revert NumbersNotInRange();
        }
        // 10k == 100% of the bet amount
        Bet memory bet = _newBet(token, tokenAmount, _getPayout(10000, numbers));
    ...
    function fulfillRandomWords(uint256 id, uint256[] memory randomWords) internal override {
        ...
        uint256 payout = _resolveBet(
            bet,
            (2**rolled) & rouletteBet.numbers != 0 ? _getPayout(bet.amount, rouletteBet.numbers) : 0
        );

        emit Roll(bet.id, bet.user, bet.token, bet.amount, rouletteBet.numbers, rolled, payout);
    ...
    function getLastUserBets(address user, uint256 dataLength) external view returns (FullRouletteBet[] memory) {
        Bet[] memory lastBets = _getLastUserBets(user, dataLength);
        FullRouletteBet[] memory lastRouletteBets = new FullRouletteBet[](lastBets.length);
        for (uint256 i; i < lastBets.length; i++) {
            lastRouletteBets[i] = FullRouletteBet(lastBets[i], rouletteBets[lastBets[i].id]);
        }
