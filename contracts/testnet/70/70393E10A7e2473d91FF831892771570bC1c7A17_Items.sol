...
// USER FUNCTIONS
        ...
        onlyWhenMintingActive(itemID)
    {
        ...
        if (address(item.paymentContract) != address(0)) {
...
// EVENTS
    ...
    event ItemAdded(address indexed account, uint256 itemID, string name, string imageUrl, uint256 price, uint256 totalSupply, IERC20 paymentContract, bool mintingActive, uint256 rarity, uint256 itemType, uint256 value1);
    ...
    /// @param price the starting price for the asset (if item is Payable) to be paid in ERC20 token at paymentAddress
    ...
    function addItem(
        ...
        onlyOwner 
    {
        if (address(paymentAddress) != address(0)) {
        ...
        ItemType memory item = ItemType(
                                    price, maxSupply, itemName, imageUrl,
                                    animationUrl, IERC20(paymentAddress), true,
                                    rarity, itemType, value1);                                // Create new item
        items[newItemId] = item;                                                              // Add item to the array
        totalItems++;                                                                         // Increment total items
        emit ItemAdded(_msgSender(), newItemId, itemName, 
                       imageUrl, price, maxSupply, IERC20(paymentAddress), true,
                       rarity, itemType, value1);                                             // Write event
    }
        ...
        onlyIfItemExists(itemID)
    {
        ...
        item.paymentContract = IERC20(paymentAddress);              // Set payment address (if payable)
        ...
        onlyOwner 
    {
        ...
        for (uint256 itemID = 0; itemID < total; itemID++) {                            // Loop through all items
            if (address(items[itemID].paymentContract) != address(0)) {
                amount = items[itemID].paymentContract.balanceOf(address(this));        // Retrieves the token balance
                if ( amount > 0 ) {                                                     // If there's a balance
                    items[itemID].paymentContract.safeTransfer(_msgSender(), amount);        // Send to the owner
                }
            }
        }
    }
