...
contract Bridge is AccessControl, IBridge {
    ...
    constructor(address layerZeroEndpoint, address router) {
        if (layerZeroEndpoint == address(0)) revert InvalidEndpoint();
        ...
        onlyRole(ROUTER_ROLE)
    {
        ...
        // to-do, _payInZero should be a parameter not hardcoded as false
        ...
        if (estimatedFee > msg.value) {
            revert InsufficientFee(estimatedFee);
        }
    ...
    function lzReceive(
        uint16 srcChainId,
        bytes calldata pathAddressBytes,
        uint64,
        bytes calldata payload
    )
        external
        override
    {
        // use this to imitate the try catch used in the lzReceive function above
        ...
        returns (ICrossRouter.SwapMessage memory swapMessage, uint256 srcActualKbp)
    {
        assembly ("memory-safe") {
            ...
            // We are preferring a dataload instead of copy here since it help us clean the upper bytes for each values
            ...
            // Call all the other bytes -> offset a 0x19, since we have 8 bytes remaining in the address, and since
