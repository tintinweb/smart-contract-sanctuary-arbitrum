...
interface ITradingClose is ITrading {
    ...
    event CloseTradeRejected(bytes32 indexed tradeHash);
...
library LibLimitOrder {
    ...
    bytes32 constant LIMIT_ORDER_POSITION = keccak256("shardex.limit.order.storage");
...
library LibTradingCore {
    ...
    bytes32 constant TRADING_CORE_POSITION = keccak256("shardex.trading.core.storage");
    ...
    function fundingFeeRate(
        ...
        if (ppi.longQty != ppi.shortQty) {
            fundingFeeR = int256((int256(ppi.longQty) - int256(ppi.shortQty)).abs() * pairConfig.fundingFeePerSecondP) / (int256(ppi.longQty).max(int256(ppi.shortQty)));
