...
abstract contract DataURIHandler is StatusHandler, ImageDataHandler, PersonalityHandler {
    ...
    function createPartsDataURI(uint256 characterData, bytes32 personalityData, uint256 status)
        external
        view
        returns (string memory)
    {
        //_createDataURI(uint256 slot, uint256 characterData, bytes32 personalityData, uint256 status)
        return _createDataURI(1, characterData, personalityData, status);
    }
