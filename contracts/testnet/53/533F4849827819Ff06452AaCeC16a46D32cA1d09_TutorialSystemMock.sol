...
struct TutorialCheckpointDefinition {
    ...
    // Checkpoint reward
    GameRegistryLibrary.TokenPointer[] checkpointReward;
}
...
contract TutorialSystem is GameRegistryConsumerUpgradeable {
    ...
    /// @notice Error when tutorial piece is already completed
    ...
    function setTutorialCheckpoint(
        ...
        if (definition.checkpointReward.length > 0) {
            ...
            ) = LootArrayComponentLibrary.convertLootInputToArrays(
...
uint256 constant ID = uint256(keccak256("game.piratenation.checkpointcomponent"));
 ...
 */
contract CheckpointComponent is BaseStorageComponent {
    /** SETUP **/
    ...
    function setValue(
        ...
        emitSetBytes(
            entity,
            abi.encode(value.numOfCheckpoints, value.checkpointPosition, value.groupId)
        );
    }
    ...
    function batchSetValue(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            encodedValues[i] = abi.encode(
                values[i].numOfCheckpoints, values[i].checkpointPosition, values[i].groupId
            );
        }
    ...
    function has(uint256 entity) public view virtual returns (bool) {
        Layout
           storage s = CheckpointComponentStorage
               .layout()
               .entityIdToStruct[entity];

        return s.numOfCheckpoints != 0 ||
           s.checkpointPosition != 0 ||
           s.groupId != 0;
    }
    ...
    function _setValueToStorage(
        ...
        Layout storage s = CheckpointComponentStorage
            .layout()
            .entityIdToStruct[entity];

        s.numOfCheckpoints = transform.numOfCheckpoints;
        s.checkpointPosition = transform.checkpointPosition;
        s.groupId = transform.groupId;
    }
}
...
uint256 constant ID = uint256(keccak256("game.piratenation.lootarraycomponent"));
 ...
 */
contract LootArrayComponent is BaseStorageComponent {
    /** SETUP **/
    ...
    function setValue(
        ...
        emitSetBytes(
            entity,
            abi.encode(value.lootType, value.tokenContract, value.lootId, value.amount)
        );
    }
    ...
    function append(
        ...
        Layout storage s = LootArrayComponentStorage
            .layout()
            .entityIdToStruct[entity];
   ...
   function removeValueAtIndexes(
       ...
       for (uint256 i = 0; i < indexes.length; i++) {
           ...
           if (indexToRemove != lastIndexInArray) {
               ...
               s.tokenContract[indexToRemove] =  s.tokenContract[lastIndexInArray];
    ...
    function batchSetValue(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            _setValueToStorage(entities[i], values[i]);
            encodedValues[i] = abi.encode(
                values[i].lootType, values[i].tokenContract, values[i].lootId, values[i].amount
            );
        }
    ...
    function _setValueToStorage(
        ...
        s.lootType = transform.lootType;
        s.tokenContract = transform.tokenContract;
        s.lootId = transform.lootId;
        s.amount = transform.amount;
    }
}
...
uint256 constant ID = uint256(keccak256("game.piratenation.tutorialcheckpointcomponent"));
 ...
 */
contract TutorialCheckpointComponent is BaseStorageComponent {
    /** SETUP **/
    ...
    function batchSetValue(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            _setValueToStorage(entities[i], values[i]);
            encodedValues[i] = abi.encode(
                values[i].currentCheckpoint, values[i].tutorialCompleted
            );
        }
    ...
    function getValue(
        ...
        // Get the struct from storage
        value = TutorialCheckpointComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function remove(uint256 entity) public virtual {
        // Remove the entity from the component
        delete TutorialCheckpointComponentStorage.layout().entityIdToStruct[entity];
        emitRemoveBytes(entity);
    }
    ...
    function has(uint256 entity) public view virtual returns (bool) {
        Layout
           storage s = TutorialCheckpointComponentStorage
               .layout()
               .entityIdToStruct[entity];

        return s.currentCheckpoint != 0;
    }
    ...
    function _setValueToStorage(
        ...
        s.currentCheckpoint = transform.currentCheckpoint;
        s.tutorialCompleted = transform.tutorialCompleted;
    }
}
