...
contract MemeOrdi is Ownable, ReentrancyGuard {
    ...
    event DeployEvent(address indexed deployer, string tickName, uint256 totalSupply, uint256 limitPerMint, uint256 ts);
    event MintEvent(uint256 eventIndex, uint256 ts, address trader, string tickName, uint256 amount,
        uint256 ethAmount, uint256 protocolEthAmount, uint256 subjectEthAmount, uint256 traderBalance, uint256 mintedAmount, uint256 holderCount);
    event Transfer(address indexed from, address indexed to, string tickName, uint256 value, uint256 holderCount);
    ...
    function deploy(string memory _name, uint256 _totalSupply, uint256 _limitPerMint) external payable nonReentrant {
      ...
      emit DeployEvent(msg.sender, _name, _totalSupply, _limitPerMint, block.timestamp);
    }
    ...
    function mintInscription(string memory _name, uint256 _amount) external payable nonReentrant {
      ...
      if (isp.balanceOf[msg.sender] == _amount) {
        isp.holderCount += 1;
      }
      uint256 ethAmount = protocolFee + subjectFee;
      emit MintEvent(mintIndex++, block.timestamp, msg.sender, _name, _amount, ethAmount, protocolFee, subjectFee, isp.balanceOf[msg.sender], isp.mintedAmount, isp.holderCount);
    }
    ...
    function transferInscription(string memory _name, uint256 _amount, address _recipient) external nonReentrant {
      ...
      if (isp.balanceOf[msg.sender] == 0) {
        isp.holderCount -= 1;
      }
      if (isp.balanceOf[_recipient] == _amount) {
        isp.holderCount += 1;
      }
      emit Transfer(msg.sender, _recipient, _name, _amount, isp.holderCount);
    }
    ...
    function getInscription(string memory _name) public view returns (uint256, uint256, uint256, uint256, uint256, string memory, address) {
      bytes32 key = getInscriptionKey(_name);
      Inscription storage isp = inscriptions[key];
      require(isp.deployer != address(0), "tick not deploy yet!");
    ...
    function isInscriptionExist(string memory _name) public view returns (bool) {
      bytes32 key = getInscriptionKey(_name);
      Inscription storage isp = inscriptions[key];
      if (isp.deployer != address(0)) {
        return true;
      }
      return false;
    }
