...
interface IRefundController {
    ...
    function updateUserEligibility(
        address _userAllocation,
        address _tokenAddress,
        uint256 _eligibility
    ) external;
    ...
    function changeTokenPortalWindow(
        address _tokenAddress,
        uint256 _windowOpenAt,
        uint256 _windowCloseUntil
    ) external;
}
...
contract Portal is Ownable, Pausable, ReentrancyGuard {
    ...
    event UpdateClaimableAt(address indexed tokenAddress, string portalName, uint256 newClaimableAt);
    event UpdatePortalMetadata(address indexed tokenAddress, string name, address[] fundingPool, uint256 tokenPrice, uint256 vestingAllocInBps);
    ...
    /**
     * @dev Update claimableAt
     *
     */
    function setClaimableAt(uint256 _claimableAt) external onlyOwner {
        uint256 _refundWindow = IRefundController(REFUND_CONTROLLER).refundWindow();

        claimableAt = _claimableAt;
        
        // adjust the refund window
        IRefundController(REFUND_CONTROLLER).changeTokenPortalWindow(tokenAddress, _claimableAt, _claimableAt + _refundWindow);

        emit UpdateClaimableAt(tokenAddress, portalName, _claimableAt);
    }

    /**
     * @dev Update Portal Metadata
     *
     */
    function setPortalMetadata(string memory _portalName, address[] memory _fundingPool, uint256 _tokenPrice, uint256 _vestingAllocInBps) external onlyOwner {
        portalName = _portalName;
        fundingPool = _fundingPool;
        tokenPrice = _tokenPrice;
        vestingAllocInBps = _vestingAllocInBps;

        emit UpdatePortalMetadata(tokenAddress, _portalName, _fundingPool, _tokenPrice, _vestingAllocInBps);
    }
