...
uint256 constant ID = uint256(
    keccak256("game.piratenation.vanityequipmentcomponent.v1")
);

struct Layout {
    uint256 equippedEntity;
    uint256 equippedTimestamp;
}

library EquippedVanityComponentStorage {
...
/**
 * @title EquippedVanityComponent
 * @dev This component indicates which vanity equippable is equipped to this entity
 */
contract EquippedVanityComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The vanity item entity that is equipped to this entity
        keys[0] = "equipped_entity";
        values[0] = TypesLibrary.SchemaValue.UINT256;

        // The timestamp of when the vanity item was equipped
        keys[1] = "equipped_timestamp";
     ...
     *
     * @param entity Entity to get value for
     * @param equippedEntity The vanity item entity that is equipped to this entity
     * @param equippedTimestamp The timestamp of when the vanity item was equipped
     */
    function setValue(
        uint256 entity,
        uint256 equippedEntity,
        ...
        _setValue(entity, Layout(equippedEntity, equippedTimestamp));
    }
    ...
    function getLayoutValue(
        ...
        value = EquippedVanityComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return equippedEntity The vanity item entity that is equipped to this entity
     * @return equippedTimestamp The timestamp of when the vanity item was equipped
     */
    function getValue(
        uint256 entity
    )
        ...
        view
        virtual
        returns (uint256 equippedEntity, uint256 equippedTimestamp)
    {
        if (has(entity)) {
            Layout memory s = EquippedVanityComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (equippedEntity, equippedTimestamp) = abi.decode(
                _getEncodedValues(s),
                (uint256, uint256)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = EquippedVanityComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.equippedEntity);
        values[1] = abi.encode(s.equippedTimestamp);
    }
    ...
    function setBytes(
        ...
        Layout memory s = EquippedVanityComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.equippedEntity, s.equippedTimestamp) = abi.decode(
            value,
            (uint256, uint256)
        );
    ...
    function remove(
        ...
        delete EquippedVanityComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete EquippedVanityComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.equippedEntity = value.equippedEntity;
        s.equippedTimestamp = value.equippedTimestamp;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        _emitSetBytes(
            entity,
            abi.encode(value.equippedEntity, value.equippedTimestamp)
        );
    }
    ...
    function _getEncodedValues(
        ...
        return abi.encode(value.equippedEntity, value.equippedTimestamp);
    }
}