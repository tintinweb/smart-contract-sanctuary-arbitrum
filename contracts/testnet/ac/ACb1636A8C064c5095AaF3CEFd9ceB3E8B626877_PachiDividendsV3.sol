...
/*
 * This contract is used to distribute dividends to users that allocated xPachi here
 *
 ...
 * This distribution slot is the source of the dividends distribution to xPachi allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */

contract PachiDividendsV3 is
    Ownable,
    ReentrancyGuard,
    IXPachiTokenUsage,
    IDividendsV2
{
    ...
    mapping(address => DividendsInfo) public dividendsInfo; // token => dividendsInfo
    mapping(address => mapping(address => UserInfo)) public users; // user => token => userInfo

    address public immutable xPachiToken; // xPachiToken contract

    mapping(address => uint256) public usersAllocation; // User's xPachi allocation
    uint256 public totalAllocation; // Contract's total xPachi allocation
    ...
    uint256 internal _cycleDurationSeconds = 1 days; // 7 days
    uint256 public currentCycleStartTime;

    constructor(address xPachiToken_, uint256 startTime_) {
        require(xPachiToken_ != address(0), "zero address");
        xPachiToken = xPachiToken_;
    ...
    /**
     * @dev Checks if caller is the xPachiToken contract
     */
    modifier xPachiTokenOnly() {
        require(
            msg.sender == xPachiToken,
            "xPachiTokenOnly: caller should be XPachiToken"
        );
        _;
    }
    ...
    /**
     * Allocates "userAddress" user's "amount" of xPachi to this dividends contract
     *
     * Can only be called by xPachiToken contract, which is trusted to verify amounts
     * "data" is only here for compatibility reasons (IxPachiTokenUsage)
     */
    function allocate(
        address userAddress,
        uint256 amount,
        bytes calldata /*data*/
    ) external override nonReentrant xPachiTokenOnly {
    ...
    /**
     * Deallocates "userAddress" user's "amount" of xPachi allocation from this dividends contract
     *
     * Can only be called by xPachiToken contract, which is trusted to verify amounts
     * "data" is only here for compatibility reasons (IxPachiTokenUsage)
     */
    ...
    function _updateDividendsInfo(address token) internal {
        ...
        // if no xPachi is allocated or initial distribution has not started yet
        if (
            totalAllocation == 0 ||
    ...
    function _harvestDividends(address token) internal {
        ...
        uint256 userXPachiAllocation = usersAllocation[msg.sender];
        uint256 pending = user.pendingDividends.add(
            userXPachiAllocation.mul(accDividendsPerShare).div(1e18).sub(
                user.rewardDebt
            )
        );

        user.pendingDividends = 0;
        user.rewardDebt = userXPachiAllocation.mul(accDividendsPerShare).div(
            1e18
        );
