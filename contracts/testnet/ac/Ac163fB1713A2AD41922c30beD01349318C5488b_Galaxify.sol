...
contract Galaxify is ERC20, Ownable {
    ...
    /// @notice initinal supply, 1 million $GXY on liquidity pool
    ...
    modifier onlyRewardManager() {
        require(msg.sender == address(nodeRewardManager), "Caller is not the node reward manager");
        _;
    }
    ...
    function setRewardManager(address payable nodeManagement) external onlyOwner {
        nodeRewardManager = NODERewardManager(nodeManagement);
    }

    /**
     * Block dumb bots from sniping, _blacklistMalicious can only be called by contract itself.
    **/
    function _blacklistMalicious(address account, bool value) internal {
    ...
    function _transfer(
        ...
        if (_isBlacklisted[from] || _isBlacklisted[to]) revert("Blacklisted address");

        if (!tradingOpen && from == address(uniswapV2Pair)) {
            if (from != owner() && to != owner()) _blacklistMalicious(to, true);
		}
	    
        if (from != address(nodeRewardManager)) {
            /// sell tax is burned
            if (to == address(uniswapV2Pair) && from != owner()) { 
                if (sellTax > 0) {
                    uint256 feeAmount = (amount * sellTax) / 100;
                    amount = amount - feeAmount;
                    super._transfer(from, deadAddress, feeAmount);

                    emit Burn(from, feeAmount);
                }
            }
        }
    ...
    function mintFromManager(address account, uint256 amount) external onlyRewardManager {
        _mint(account, amount);
    }

    function mintFromPresale(address to, uint256 amount) external onlyRewardManager {
        _mint(to, amount);
    }
}
