...
contract Baccarat is CommonSoloBaccarat, InternalRNG {
  ...
  function getResultNumbers(
    ...
    // console.log("BEFORE - 0 allNumbersDecks_ %s", allNumbersDecks_[0]);
    // console.log("BEFORE - 1 allNumbersDecks_ %s", allNumbersDecks_[1]);
    // console.log("BEFORE - 2 allNumbersDecks_ %s", allNumbersDecks_[2]);
    // console.log("BEFORE - 3 allNumbersDecks_ %s", allNumbersDecks_[3]);
    // console.log("BEFORE - 4 allNumbersDecks_ %s", allNumbersDecks_[4]);
    // console.log("BEFORE - 5 allNumbersDecks_ %s", allNumbersDecks_[5]);
    // console.log("BEFORE - 6 allNumbersDecks_ %s", allNumbersDecks_[6]);
    // console.log("BEFORE - 7 allNumbersDecks_ %s", allNumbersDecks_[7]);
    // console.log("BEFORE - 8 allNumbersDecks_ %s", allNumbersDecks_[8]);
    // console.log("BEFORE - 9 allNumbersDecks_ %s", allNumbersDecks_[9]);
    // console.log("BEFORE - 10 allNumbersDecks_ %s", allNumbersDecks_[10]);
    // console.log("BEFORE - 11 allNumbersDecks_ %s", allNumbersDecks_[11]);
    // console.log("BEFORE - 12 allNumbersDecks_ %s", allNumbersDecks_[12]);
    // console.log("BEFORE - 13 allNumbersDecks_ %s", allNumbersDecks_[13]);
    // console.log("BEFORE - 14 allNumbersDecks_ %s", allNumbersDecks_[14]);
    // console.log("BEFORE - 15 allNumbersDecks_ %s", allNumbersDecks_[15]);
    ...
    // console.log("AFTER - 0 allNumbersDecks_ %s", allNumbersDecks_[0]);
    // console.log("AFTER - 1 allNumbersDecks_ %s", allNumbersDecks_[1]);
    // console.log("AFTER - 2 allNumbersDecks_ %s", allNumbersDecks_[2]);
    // console.log("AFTER - 3 allNumbersDecks_ %s", allNumbersDecks_[3]);
    // console.log("AFTER - 4 allNumbersDecks_ %s", allNumbersDecks_[4]);
    // console.log("AFTER - 5 allNumbersDecks_ %s", allNumbersDecks_[5]);
    // console.log("AFTER - 6 allNumbersDecks_ %s", allNumbersDecks_[6]);
    // console.log("AFTER - 7 allNumbersDecks_ %s", allNumbersDecks_[7]);
    // console.log("AFTER - 8 allNumbersDecks_ %s", allNumbersDecks_[8]);
    // console.log("AFTER - 9 allNumbersDecks_ %s", allNumbersDecks_[9]);
    // console.log("AFTER - 10 allNumbersDecks_ %s", allNumbersDecks_[10]);
    // console.log("AFTER - 11 allNumbersDecks_ %s", allNumbersDecks_[11]);
    // console.log("AFTER - 12 allNumbersDecks_ %s", allNumbersDecks_[12]);
    // console.log("AFTER - 13 allNumbersDecks_ %s", allNumbersDecks_[13]);
    // console.log("AFTER - 14 allNumbersDecks_ %s", allNumbersDecks_[14]);
    ...
    // console.log("topCards_");
    // console.log("topCards_ %s", topCards_[0]);
    // console.log("topCards_ %s", topCards_[1]);
    // console.log("topCards_ %s", topCards_[2]);
    // console.log("topCards_ %s", topCards_[3]);
    // console.log("topCards_ %s", topCards_[4]);
    // console.log("topCards_ %s", topCards_[5]);
  ...
  //   unchecked {
  //     if (result_ == BaccaratResult.DRAW) {
  //       payout_ = (_bet.tieWins * multiplierTie) / 1e2;
  //     } else if (result_ == BaccaratResult.PLAYER_WINS) {
  //       payout_ = (_bet.playerWins * multiplierBanker) / 1e2;
  //     } else if (result_ == BaccaratResult.BANK_WINS) {
  //       payout_ = (_bet.bankWins * multiplierPlayer) / 1e2;
  //     } else {
  //       revert("Baccarat: invalid state");
  //       payout_ = 0;
  //     }
  ...
  function play(
    ...
    uint256 bankCount_;
    // unchecked {
    //   playerCount_ = topCards_[0] + topCards_[2];
    //   bankCount_ = topCards_[1] + topCards_[3];
    // }
    unchecked {
    ...
    // console.log("playerCount: %s", playerCount_);
    // console.log("player punto: %s", _puntoModulo(playerCount_));
    // console.log("bankCount: %s", bankCount_);
    // console.log("bank punto: %s", _puntoModulo(bankCount_));
    ...
    // note that in the Hand struct all 3 cards are dealt. However the hasThirdCard boolean 'decides' if the third dealt card is used in the calculation of the total punto of the hand

    // note that in the Hand struct all 3 cards are dealt. However the hasThirdCard boolean 'decides' if the third dealt card is used in the calculation of the total punto of the hand

    // note it is the case that in this design the player always gets first, third and fifth card, and the banker gets second, fourth and sixth card. This is not a problem as the cards are shuffled before each game. It does not add anything to 'deal from the top'. It can be changed to deal from the top, however this makes it more gas itensive and again it doesn't matter for the odds or player experience.

    // if either banker or players punto is 8 or 9, the game is over and no more cards are dealt (for either hands)
    if (
      // _checkImmediateWinner(_puntoModulo(playerCount_), _puntoModulo(bankCount_)) !=
      // BaccaratResult.UNDECIDED
      _checkImmediateWinner(playerCount_ % 10, bankCount_ % 10) != BaccaratResult.UNDECIDED
    ) {
      // console.log("game over - one of the two player has 8 or 9 points");
      // calculate potential payout for the game
    ...
    // player has 6 or 7 points, no third card for player
    if ((playerCount_ % 10) < 6) {
      // player gets card
      // console.log("player has less that 6 points, third card");
      ...
      // the player stands (no third card) so player has 6 or 7, check if the bank gets a third card
      if ((bankCount_ % 10) < 6) {
    ...
    // player has gotten third card, check if bank gets third card
    ...
    // // if player has 6 or 7 (and thus doesn't get third card) - the bank will get a third card if it has 5 or less
    // // first check if player has indeed only 2 cards (must follow, as for the bank)
    // if (!hands_.playerHand.hasThirdCard && !hands_.bankerHand.hasThirdCard) {
    //   // console.log("neither bank or player has third card");
    //   // bank has 5 or less, and player has 2 cards, bank gets third card
    //   if ((bankCount_ % 10) < 6) {
    //     // console.log("bank has less than 6 points, third card");
    //     unchecked {
    //       bankCount_ += _scaleValueToPunto(topCards_[5]);
    //     }
    //     hands_.bankerHand.hasThirdCard = true;
    //     // game is over, calculate payout, emit event
    //     emit HandFinalized(games_[_requestId].player, _requestId, hands_);
    //     payout_ = _calculateWinnings(_checkWinner(playerCount_ % 10, bankCount_ % 10), _bet);
    //     games_[_requestId].bankerHand = hands_.bankerHand;
    //     games_[_requestId].playerHand = hands_.playerHand;
    ...
    if (hands_.playerHand.hasThirdCard && !hands_.bankerHand.hasThirdCard) {
      // console.log("player has third card, bank doesn't");
      // check if bank gets third card
      // bool thirdCard_ = _doesBankerGetThirdCard(topCards_[4], _puntoModulo(bankCount_));
    ...
    // console.log("payout end of function: %s", payout_);
  ...
  function _doesBankerGetThirdCard(
    ...
    if (_puntoBanker < 3) {
      ...
      // console.log("_buntoBanker: %s", _puntoBanker);
      return bankerPuntoSix[_thirdCard];
    } else {
      // console.log("_puntoBanker: %s", _puntoBanker);
  ...
  function _checkWagerReturn(
    ...
    // uint128 totalWager_ = uint128(_tieWins + _bankWins + _playerWins);
    uint128 totalWager_ = uint128(
      chip2Token((_tieWins + _bankWins + _playerWins), _token, vaultManager.getPrice(_token))
    );
    // console.log("total bet in chips: %s", _tieWins + _bankWins + _playerWins);
    // console.log("totalWager_: %s", totalWager_);
    uint256 dollarValue_ = _computeDollarValue(_token, totalWager_);
    // console.log("dollarValue_: %s", dollarValue_ / 1e30);
    // console.log("maxWager: %s", vaultManager.getMaxWager() / 1e30);
    // require(dollarValue_ <= vaultManager.getMaxWager(), "Baccarat: Wager too high");
    require(dollarValue_ >= minWagerAmount, "Baccarat: Wager too low");
    return
      Bet({
        tokenPrice: uint128(vaultManager.getPrice(_token) / 1e18),
        totalWager: totalWager_,
        tieWins: _tieWins,
        bankWins: _bankWins,
        playerWins: _playerWins,
        decimals: uint16(IERC20Metadata(_token).decimals())
      });
  }
...
abstract contract CommonSoloBaccarat is Core, Test {
  /*==================================================== Events =============================================================*/
