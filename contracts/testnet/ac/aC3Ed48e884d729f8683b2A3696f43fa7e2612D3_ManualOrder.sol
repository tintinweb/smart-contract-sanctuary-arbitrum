...
contract ManualOrder is OrderBase {
    ...
    function createManualOrder(TccItemV2 calldata _tcc, uint256 _tokenAmount) external nonReentrant payable {
        ...
        _y.swapInToken = _tcc.swapPathConfig1._swapPathList[0];
        _y.swapOutToken = _tcc.swapPathConfig1._swapPathList[_tcc.swapPathConfig1._swapPathList.length - 1];
    ...
    function doManualOrder(address _user, bytes32 _md5) external nonReentrant {
        ...
        address _to = y.tcc.swapPathConfig2._swapPathList.length > 0 ? address(this) : _user;
        //如果是刷单模式
        if (y.tcc.swapPathConfig2._swapPathList.length > 0) {
            x.swapInAmount = MultiDexRouter(multiDexRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                y.tcc.swapPathConfig1._routerAddressListV2,
                x.swapInAmount,
                0,
                y.tcc.swapPathConfig1._swapPathList,
                _to,
                block.timestamp
            );
        } else {
            ...
            if (y.tcc.swapPathConfig1._swapPathList[y.tcc.swapPathConfig1._swapPathList.length - 1] == weth) {
            ...
            //成交在执行的价格区间
            require(x.swapInAmount.mul(10 ** 18) > y.tcc.swapPathConfig1._swapPriceZone[0].mul(_in) && x.swapInAmount.mul(10 ** 18) < y.tcc.swapPathConfig1._swapPriceZone[1].mul(_in), "P09");
        }
        //兼容单向兑换
        if (y.tcc.swapPathConfig2._swapPathList.length > 0) {
            //收取一部分代币B
            ...
            uint256 out = MultiDexRouter(multiDexRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                y.tcc.swapPathConfig2._routerAddressListV2,
                x.swapInAmount,
                0,
                y.tcc.swapPathConfig2._routerAddressListV2,
                address(this),
...
pragma solidity ^0.8.0;
    ...
    struct TccItemV2 {
        ...
        uint256 _maxFeePerTx; //每笔刷单消耗的GAS上限(刷单/限价单)
        OrderType _type;
        swapPathItem swapPathConfig1; //第一次兑换数据
        swapPathItem swapPathConfig2; //第二次兑换数据
    }