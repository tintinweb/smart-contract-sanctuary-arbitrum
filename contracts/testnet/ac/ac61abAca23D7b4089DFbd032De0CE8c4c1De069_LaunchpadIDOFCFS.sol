...
abstract contract AdminableUpgradeable is Initializable, OwnableUpgradeable, AccessControlUpgradeable {
    function initializeAdmin() public virtual initializer {
...
import "./Timed.sol";
import "../contracts/adminable/AdminableUpgradeable.sol";
import "hardhat/console.sol";

abstract contract GeneralIDO is Timed {
    ...
    function initializeGenral(uint256 _rate, uint256 _tokensForSale, address _priceFeed) public initializer {
        //rate in USD
        require(_rate > 0, "IDO: token rate can't be zero");
        require(_tokensForSale > 0, "IDO: token amount can't be zero");
        rate = _rate;
    ...
    function setTokensForSale(uint256 _tokensForSale) external onlyOwnerOrAdmin {
        require(_tokensForSale > 0, "IDO: Invalid Tokens Amount");
        require(block.timestamp < startTime, "IDO: Can't update after sale start");

        tokensForSale = _tokensForSale;
        emit CapChanged(tokensForSale);
    }
    //calculate the number of tokens can purchase with wei
    ...
    //calculate the purchase amount of tokens giving USD
    function calculatePurchaseAmountUSD(uint256 purchaseAmountUSDWei) public view returns (uint256) {
        uint256 price = getNativePriceUSD();
        return calculatePurchaseAmount(purchaseAmountUSDWei.mul(1e18).div(price));
    }
    ...
    // returns the Native currency price (Wei) of token
    function getRateNative() public view returns (uint256) {
        uint256 price = getNativePriceUSD();
        uint256 cPrice = (rate.mul(1e18)).div(price);
        return cPrice;
    }

    function getNativePriceUSD() public view returns (uint256) {
...
import "./WithClaimer.sol";
import "hardhat/console.sol";

contract LaunchpadIDOFCFS is
    ...
    WithClaimer
{
    ...
    function initialize (
        ...
        AdminableUpgradeable.initializeAdmin();
        GeneralIDO.initializeGenral(_rate, _tokensForSale, _priceFeed);
        Timed.initializeTime(_startTime, _endTime);
        WithClaimer.initializeClaimer(tokenAddress);
        Withdrawable.initializeWithdrawable(tokenAddress);
        id = _id;
    }
    ...
    function internalBuyTokens(uint256 value) private {
        ...
        require(amount >= minSell, "Sale: amount is too small");
        require(balances[account] + amount <= maxSell , "Sale: amount exceeds maxSale limit");
        require(tokensSold + amount <= tokensForSale, "Sale: cap reached");
...
abstract contract Timed is AdminableUpgradeable  {
    ...
    function initializeTime(uint256 _startTime, uint256 _endTime) public initializer {
        require(_startTime >= block.timestamp, "Time: Start time can't be in past");
        require(_endTime > _startTime, "Time: End time must be greater than start time");
    ...
    modifier ongoingSale() {
        require(isLive(), "Time: Sale not live");
        _;
    }
    ...
    /**
    @dev update the start time of sale before sale start
    @param newTime updated start time of the sale
     */
    function setStartTime(uint256 newTime) public onlyOwnerOrAdmin {
        require(block.timestamp < startTime, "Time: Can't update after sale start");
        require(newTime > block.timestamp, "Time: Time should be greater than current time");
        startTime = newTime;
        emit StartChanged(startTime);
    }
    /**
    @dev update the end time of sale before sale start
    @param newEndTime updated end time of the sale
     */
    function setEndTime(uint256 newEndTime) public onlyOwnerOrAdmin {
        require(block.timestamp < startTime, "Time: Can't update after sale start");
        require(newEndTime > startTime, "Time: Time should be greater than start time");
        endTime = newEndTime;
...
abstract contract WithClaimer is GeneralIDO {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function initializeClaimer(address _token) public initializer{
        isPaused = false;
        token = IERC20(_token);
    }

    function setClaimInfo(uint256[] memory times, uint256[] memory percents) public onlyOwnerOrAdmin {
        uint256 totalPercent;
        for (uint256 i = 0; i < times.length; i++) {
            require(percents[i] > 0, "Claimer: 0% is not allowed");
            require(times[i] > 0, "Claimer: time must specified");
            require(times[i] > getEndTime(), "Claimer: time must be after end time");
    ...
    function getRemainingAccountAmount(address account) external view returns (uint256) {
    ...
    function getClaimAmount(uint256 claimIdx, address account) internal view returns (uint256) {
    ...
    function delayClaim(uint256 claimIdx, uint256 newUnlockTime) external onlyOwner {
    ...
    function claim(address account, uint256 idx) external {
        ...
        require(claims[idx].unlockTime < block.timestamp, "Claimer: Not claimable");
    ...
    function getClaims(
        address account
    ) external view returns (uint256[] memory, uint256[] memory, uint256[] memory, bool[] memory, uint256[] memory) {
    ...
    function claimLength() public view returns(uint256) {
...
abstract contract Withdrawable is AdminableUpgradeable{
    ...
    function initializeWithdrawable(address _token) public initializer{
        sellToken = IERC20(_token);
    }
    /**
    ...
    function withdrawBalance(uint256 amount) external onlyOwnerOrAdmin {
        ...
        require(balance >= amount, "Withdrawable: Insufficient balance");
     ...
     */
    function withdrawToken(uint256 amount) external onlyOwnerOrAdmin {
        require(amount > 0, "Withdrawable: amount should be greater than zero");
...
abstract contract WithLimits is AdminableUpgradeable {
    ...
    function setMin(uint256 value) public onlyOwnerOrAdmin {
        require(maxSell == 0 || value <= maxSell, "Limit: Must be smaller than max");
        minSell = value;
        emit MinChanged(value);
    }

    function setMax(uint256 value) public onlyOwnerOrAdmin {
        require(minSell == 0 || value >= minSell, "Limit: Must be bigger than min");
        maxSell = value;
