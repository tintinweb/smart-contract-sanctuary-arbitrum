...
// MODIFIERS
    ...
    modifier onlyIfEnoughHealth(uint256 tokenID, uint256 numberOfQuests) {
        require (characterActivity.calculateHealth(tokenID) - numberOfQuests > 0, "Explorer needs more health");
        _; // Call the actual code
    }
...
// FUNCTIONS
    ...
    /// @param questID Quest ID
    /// @param tokenID Characters ID
    /// @param includeItem if you want to remove an item
    /// @param itemID of item to release from the quest
    function completeQuest(uint256 questID, uint256 tokenID, bool includeItem, uint256 itemID)
        external
        nonReentrant
        onlyIfCharacterAvailable(tokenID)
        onlyQuestExpired(tokenID)
    {
        ...
        (,uint256 numberOfActivities,,,) = characterActivity.charactersActivity(tokenID);              // Shortcut to characters activity
        ...
        for(uint256 i=0; i < numberOfActivities; i++) {                                                // Loop through the quest and mint items
            ...
            amountOfLandFound = getLandAmount(questID, tokenID, includeItem);                          // Calculate the Land
        ...
        if ( includeItem ) {
            _removeItem(tokenID,itemID);                                                               // Release an item TODO: Always remove item
        }
    }

    /// @dev End quest without collecting items
    /// @param tokenID Characters ID
    /// @param includeItem if you want to remove an item
    /// @param itemID of item to release from the quest
    function endQuest(uint256 tokenID, bool includeItem, uint256 itemID)
        external
        nonReentrant
        onlyIfCharacterAvailable(tokenID)
        onlyQuestExpired(tokenID)
    {
        ...
        characterActivity.setActive(tokenID, false, _msgSender());                                     // Set a variable to indicate that character is on a quest
        emit QuestCompleted(_msgSender(), latestQuest[tokenID], block.number, tokenID);                // Write an event
        // TODO: Automatically release items to ensure they don't get locked if a character is transferred
        if ( includeItem ) {
            _removeItem(tokenID,itemID);                                                               // Release an item TODO: Always remove item
        }
    }
...
// INTERNAL HELPER FUNCTIONS
        ...
        private
    {
        ...
        characterActivity.setActive(tokenID, true, _msgSender());                                          // Set a variable to indicate that character is on a quest
        characterActivity.setActivityDuration(tokenID, questDuration, _msgSender());                       // Set the speciic quest duration for this character
        characterActivity.setBeginActivity(tokenID, numberOfQuests, block.number, endBlock, _msgSender()); // Update the Quest details
    ...
    /// @param questID Quest ID
    /// @param tokenID Characters ID
    function getLandAmount(uint256 questID, uint256 tokenID, bool includesItem)
        private
        returns (
            uint256 amountOfLandFound
        )
    {
        ...
        if (includesItem) {                                                          // If an item is included then check the inventory
            uint256 questItemSet = quests[questID].itemSet;                          // Declare & set the item set to work from
            uint256 equipItemType = getItemType(questID, 'Equip');                   // Retrieve Item Types
        ...
        if (randomNumbers[0] % 100 < chanceOfFindingLand) {                         // Land is found if the random number is less than the chance of finding Land
            amountOfLandFound = ((randomNumbers[1] % 
                                getCharactersBravery(tokenID)) +1) * (10**18);      // You found between 1 and max 99 (capped at the characters of courage )
        }
    }
        ...
        )
    {
        ...
        uint256 rewardItemType = getItemType(questID, 'Reward');                                 // Retrieve Item Types
...
struct Quest { 
    ...
    string[] itemTypes;                             // Item types e.g., Reward = 0; Equip = 1; Health = 2;
...
// EVENTS
    ...
    event ItemFound(address indexed account, uint256 quest, uint256 indexed tokenID, uint256 indexed itemMinted, uint256 amountMinted, uint256 amountOfLandFound);
...
// ADMIN FUNCTIONS
    ...
    /// @param itemTypes an array of the item types available in the quest
    function createQuest(string calldata name, uint256 itemSet, uint256 questPrice, address paymentAddress, uint256 chanceOfFindingLand, uint256 questDuration, uint256 maxNumberOfQuestsBase, bool active, uint256[5] calldata rarityPercentages, string[] memory itemTypes)
        external
        onlyOwner
    {
        ...
        quest.itemTypes = itemTypes;                                 // add the item names to the array
    ...
    /// @param active set the status of the quest
    /// @param rarityPercentages an array of the rarity percentage for item distribution eg [400, 150, 50, 10, 0]
    /// @param itemTypes an array of the item types available in the quest eg ["Reward","Equip"]
    function updateQuest(uint256 questID, string calldata name, uint256 itemSet, uint256 questPrice, address paymentAddress, uint256 chanceOfFindingLand, uint256 questDuration, uint256 maxNumberOfQuestsBase, bool active, uint256[5] calldata rarityPercentages, string[] memory itemTypes)
        public
        onlyOwner
    {
        require(rarityPercentages.length == 5,                       "Requires 5 rarity values");
        ...
        quest.questDuration = questDuration;                         // Set base quest duration
        quest.active = active;                                       // Set quest as active/inactive
        delete quest.itemTypes;                                      // Remove the old item types
        quest.itemTypes = itemTypes;                                 // Add the item names to the array
...
// VIEWS
    ...
    /// @dev Returns a quests item types
    function getQuestsItemTypes(uint256 questID)
        external
        view
        returns (string[] memory itemTypes) 
    {
        itemTypes = quests[questID].itemTypes;
    }

    /// @dev Return the item types for a specific quest
    /// @param questID Quest ID
    /// @param itemTypeName Name of the item type
    function getItemType(uint256 questID, string memory itemTypeName)
        public
        view
        returns (
            uint256 itemType
    ...
    {
        uint256 total = quests[questID].itemTypes.length;
        for(uint256 i = 0; i < total; i++){                                    // Loop through the items
            if (keccak256(abi.encodePacked(itemTypeName)) ==
                keccak256(abi.encodePacked(quests[questID].itemTypes[i]))) {   // If item type name in the array return the in index
                return i;
            }
        }
    }
...
// TODO: Add support for different NFTs
...
// FUNCTIONS
        ...
        whenNotPaused
    {
        ...
        farmlandCharacters.safeTransferFrom(_msgSender(), address(this), tokenID, "");  // Transfer character to contract
    }
        ...
        onlyIfCharacterAvailable(tokenID)
    {
        ...
        ownerOfCharacter[_msgSender()][characterIndex] = 
                    ownerOfCharacter[_msgSender()][ownerOfCharacter[_msgSender()].length - 1];    // In the ownerOfCharacter items array swap the last item for the item being removed
        ...
        farmlandCharacters.safeTransferFrom(address(this), _msgSender(), tokenID, "");            // Return Item to owner
    }
