...
import "../characters/interfaces/ICheckForCharacter.sol";

contract CharacterOwner is Permissioned {

    constructor (
        address[2] memory farmlandAddresses       // Load key contract addresses
        ) Permissioned()
    {
        require(farmlandAddresses.length == 2,      "Invalid number of contract addresses");
...
// STATE VARIABLES
    ...
    /// @dev The Farmland Corn Contract
    ICornV2 public  immutable cornContract;
    
    /// @dev A list of other contracts where characters can live ... need to implement the getCharactersByAccount(account) function
...
// MODIFIERS
    ...
    modifier onlyCharacterOwner (uint256 tokenID) {
        ...
        require(isAccountOwnerOfCharacter(_msgSender(), tokenID),"You need to own or employ the Character");
        _;
    }

// ADMIN FUNCTIONS

    /// @dev Add a list of character contract that implement the getCharactersByAccount(account) function
    /// @param characterContracts array of addresses
    ...
    /// @dev Removes the address from the list
    /// @param characterContract address to remove from list
    function removeCharacterContract(address characterContract)
        external
        onlyOwner
    {
        (bool listed, uint256 index) = 
                checkCharacterContracts(characterContract);
        require(listed,                                     "Address not found on list");
        otherContracts[index] = 
                otherContracts[otherContracts.length - 1];  // Overwrite the item to delete with the last item in the array
        otherContracts.pop();                               // Delete the last item in the array
    }
...
// VIEW FUNCTIONS
    ...
    function isAccountOwnerOfCharacter(address account, uint256 tokenID)
        public
        view
        returns (
            bool ownsCharacter
    ...
    {
        if (farmlandCharacters.totalSupply() < tokenID) { 
            return false; 
            }
    ...
    /// @dev Check the list for an address
    /// @param characterContract address to check
    function checkCharacterContracts(address characterContract)
        public
        view
        returns (
            bool listed,
    ...
    {
        uint256 total = otherContracts.length;             // Get the total
        for(uint256 i=0; i < total; i++){                  // Loop through the addresses
            if ( characterContract == otherContracts[i]) 
            {
                addressIndex = i;                         // If we get a match on address in list return the index ID
                listed = true;                            // If we get a match on address in list return true
                break;
            }
        }
    }
...
interface ICheckForCharacter {
    function getCharactersByAccount(address account) external view returns (uint256[] memory characters);
}
