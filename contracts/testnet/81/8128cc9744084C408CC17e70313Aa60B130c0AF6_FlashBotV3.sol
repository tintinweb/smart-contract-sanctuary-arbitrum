...
contract FlashBotV3 is Ownable {
    ...
    function isbaseTokenSmaller(address pool0, address pool1, bool v3)
        internal
        view
        returns (
            bool baseSmaller,
        ...
        )
    {
        ...
        address pool1Token1;
        if (v3) {
            (pool0Token0, pool0Token1) = (IUniswapV3Pool(pool0).token0(), IUniswapV3Pool(pool0).token1());
            (pool1Token0, pool1Token1) = (IUniswapV3Pool(pool1).token0(), IUniswapV3Pool(pool1).token1());
        } else {
            (pool0Token0, pool0Token1) = (IUniswapV2Pair(pool0).token0(), IUniswapV2Pair(pool0).token1());
            (pool1Token0, pool1Token1) = (IUniswapV2Pair(pool1).token0(), IUniswapV2Pair(pool1).token1());            
        }
    ...
    function getReserves(address pool, bool v3)
        public 
        view 
        returns (
            uint256 reserve0, 
        ...
        ) 
    {
        ...
        (success, data) = token0.staticcall(abi.encodeWithSelector(IERC20.balanceOf.selector, pool));
        require(success && data.length >= 32);
        reserve0 = abi.decode(data, (uint256));
        
        (success, data) = token1.staticcall(abi.encodeWithSelector(IERC20.balanceOf.selector, pool));
        require(success && data.length >= 32);
        reserve1 = abi.decode(data, (uint256));
    }
    ...
    {
        (uint256 pool0Reserve0, uint256 pool0Reserve1) = getReserves(pool0, v3);
        (uint256 pool1Reserve0, uint256 pool1Reserve1) = getReserves(pool1, v3);
    ...
    function flashArbitrage(address pool0, address pool1, bool v3) external {
        ArbitrageInfo memory info;
        (info.baseTokenSmaller, info.baseToken, info.quoteToken) = isbaseTokenSmaller(pool0, pool1, v3);

        OrderedReserves memory orderedReserves;
        (info.lowerPool, info.higherPool, orderedReserves) = getOrderedReserves(pool0, pool1, v3, info.baseTokenSmaller);
        ...
        // avoid stack too deep error
        {
            ...
            swap.v3 = v3;
            swap.swap0to1 = info.baseTokenSmaller;
            swap.pool = info.lowerPool;
            ...
            if (v3) {
                ...
                (baseTokenOutAmount,,) = q.quoteOutV3(
                    router, 
                    info.lowerPool, 
                    borrowAmount, 
                    0,
    ...
    function _swap(SwapInfo memory info, bytes memory data) internal {
        if (info.v3) {
            int amount = info.swap0to1 ? int(info.amount0) : int(info.amount1);
            uint160 sqrtPriceLimit = info.swap0to1
            ? MIN_SQRT_RATIO + 1
            : MAX_SQRT_RATIO - 1;
            IUniswapV3Pool(info.pool).swap(
                address(this), 
                info.swap0to1, 
                amount, 
                sqrtPriceLimit, 
            ...
            IUniswapV2Pair(info.pool).swap(
                uint256(info.amount0), 
                uint256(info.amount1), 
                address(this), 
                data
                );
        }
    ...
    function uniswapV2Call(
        ...
        SwapInfo memory swap;
        swap.pool = info.targetPool;
        swap.v3 = false;
        swap.swap0to1 = info.debtTokenSmaller;
        (swap.amount0, swap.amount1) =
    ...
    function uniswapV3SwapCallback(
        ...
        (swap.amount0, swap.amount1) =
            info.debtTokenSmaller ? (info.debtTokenOutAmount, uint256(0)) : (uint256(0), info.debtTokenOutAmount);
        _swap(swap, new bytes(0));
        // IUniswapV3Pool(info.targetPool).swap(address(this), info.debtTokenSmaller, info.debtTokenOutAmount, new bytes(0));

        IERC20(info.debtToken).safeTransfer(info.debtPool, info.debtAmount);
    }
    
    /// @notice Calculate how much profit we can by arbitraging between two pools
    function getProfit(address pool0, address pool1, bool v3) external returns (uint256 profit, address baseToken) {
        (bool baseTokenSmaller, , ) = isbaseTokenSmaller(pool0, pool1, v3);
        baseToken = v3 ? (baseTokenSmaller ? IUniswapV3Pool(pool0).token0() : IUniswapV3Pool(pool0).token1()) :
        (baseTokenSmaller ? IUniswapV2Pair(pool0).token0() : IUniswapV2Pair(pool0).token1());

        (address lowerPool, 
        address higherPool, 
        OrderedReserves memory orderedReserves
        ) = getOrderedReserves(pool0, pool1, v3, baseTokenSmaller);
...
contract QuoterV3 {
    function quoteInV3(
        ...
        bool swap0to1
            ...
            (address tokenIn, address tokenOut) = swap0to1 ? 
            (IUniswapV3Pool(pool).token0(), IUniswapV3Pool(pool).token1()) :
            (IUniswapV3Pool(pool).token1(), IUniswapV3Pool(pool).token0());
