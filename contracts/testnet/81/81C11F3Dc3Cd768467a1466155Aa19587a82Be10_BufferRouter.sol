...
library Validator {
    ...
    function verifyUserTradeParams(
        ...
        bytes32 hashData = keccak256(
            abi.encode(
                keccak256(
                    "UserTradeSignatureWithSettlementFee(address user,uint256 totalFee,uint256 period,address targetContract,uint256 strike,uint256 slippage,bool allowPartialFill,string referralCode,uint256 timestamp,uint256 settlementFee,uint256 spread)"
                ),
                user,
                params.totalFee,
                params.period,
