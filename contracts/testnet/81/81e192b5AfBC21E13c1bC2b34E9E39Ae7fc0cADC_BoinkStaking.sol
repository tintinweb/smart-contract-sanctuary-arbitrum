...
contract BoinkStaking is ReentrancyGuard, Ownable {
    ...
    function depositNFT(address _nftContract, uint256[] calldata _tokenIds) external onlyBoosterNfts nonReentrant {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (IERC1155(_nftContract).supportsInterface(type(IERC721).interfaceId)) {
                // Handle ERC1155 supporting ERC721
                IERC1155(_nftContract).safeTransferFrom(msg.sender, address(this), _tokenIds[i], 1, "");
            } else {
                // Handle regular ERC721
    ...
    function withdrawNFT(address _nftContract, uint256[] calldata _tokenIds) external onlyBoosterNfts nonReentrant {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(IERC721(_nftContract).ownerOf(_tokenIds[i]) == address(this) || IERC1155(_nftContract).balanceOf(address(this), _tokenIds[i]) > 0, "Contract does not own this NFT");
            if (IERC1155(_nftContract).supportsInterface(type(IERC721).interfaceId)) {
                // Handle ERC1155 supporting ERC721
                IERC1155(_nftContract).safeTransferFrom(address(this), msg.sender, _tokenIds[i], 1, "");
            } else {
                // Handle regular ERC721
    ...
    function withdrawAllNFTs(address _nftContract) external onlyBoosterNfts nonReentrant {
        ...
        for (uint256 i = 0; i < userNFTs.length; i++) {
            ...
            require(IERC721(_nftContract).ownerOf(tokenId) == address(this) || IERC1155(_nftContract).balanceOf(address(this), tokenId) > 0, "Contract does not own this NFT");
            if (IERC1155(_nftContract).supportsInterface(type(IERC721).interfaceId)) {
                // Handle ERC1155 supporting ERC721
