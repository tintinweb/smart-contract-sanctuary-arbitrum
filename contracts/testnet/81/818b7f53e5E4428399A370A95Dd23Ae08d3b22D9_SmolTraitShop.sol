...
interface ISmolTraitShop {
    ...
    error InvalidTier();
    error AlreadyClaimedSpecialTraitFromThisTier(address _userAddress, uint256 _traitType, uint256 _traitId);
...
// -------------------------------------------------------------
    ...
    external contractsAreSet whenNotPaused {
        ...
        if (!traitToInfo[_traitType][_traitId].special) revert MustCallBuyExclusiveTrait(uint256(_traitType), _traitId);
        uint256 traitTier = checkTraitTier(_eventNumber, _traitType, _traitId);
        if (traitTier == 0) revert TraitNotAssignedToTierForSelectedEvent();
        ...
        for (uint256 i = 0; i < length; i++){
            ...
            if (userAllocationClaimed[msg.sender][_traitType][id]) revert AlreadyClaimedSpecialTraitFromThisTier(msg.sender, uint256(_traitType), id);
        }
        _buyMerkle(msg.sender, _proof, _smolId, _traitType,_traitId);
    }
...
// -------------------------------------------------------------
    ...
    function _checkTraitId (
        ...
        uint256 _traitId)
    internal view{
        if (_traitId <= 0 || traitTypeToLastId[_traitType] < _traitId) revert TraitIdDoesNotExist(uint(_traitType), _traitId);
    }
