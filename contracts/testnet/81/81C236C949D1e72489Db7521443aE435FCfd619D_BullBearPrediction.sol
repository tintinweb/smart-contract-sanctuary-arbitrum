...
contract BullBearPrediction is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public winBonus; // % of bonus given to winners on winning amount
    uint256 public lossBonus; // %of bonus given to losers on betting amount
    ...
    function claim(
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            ...
            if (rounds[epochs[i]].oracleCalled) {
                ...
                // add 30% extra to final reward
                addedReward = addedReward + ((addedReward * winBonus) / 10000);
            }
    ...
    /**
     * @notice Claim loss bonus if any for an array of epochs
     * @param epochs: array of epochs
     */
    function claimLossBonus(
        uint256[] calldata epochs
    ) external nonReentrant notContract {
        uint256 reward; // Initializes reward
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            ...
            if (rounds[epochs[i]].oracleCalled) {
                require(
                    lossBonusClaimable(epochs[i], msg.sender),
                    "Not eligible for claim loss bonus"
                );
                // Round memory round = rounds[epochs[i]];
                // add % of loss bonus to bet amount
                addedReward =
                    (ledger[epochs[i]][msg.sender].amount * lossBonus) /
                    10000;
            }
            // // Round invalid, refund bet amount
            // else {
