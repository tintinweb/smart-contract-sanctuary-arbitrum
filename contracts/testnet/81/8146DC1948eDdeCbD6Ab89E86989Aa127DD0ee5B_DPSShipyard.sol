...
contract DPSShipyard is Ownable, ReentrancyGuard {
    ...
    uint256 public claimFlagshipIdCounterV2 = 3001;

    mapping(DPSFlagshipI => bool) public flagshipsAllowed;
    ...
    event FlagshipMinted(address indexed _owner, uint256 _tokenId, uint256 _flagshipId);
    ...
    function claimFlagshipDPS(uint16 _dpsId) external nonReentrant {
        ...
        emit FlagshipMinted(msg.sender, _dpsId, _dpsId);
    }
     ...
     */
    function claimFlagshipV2(uint256 _claimerId, IERC721 _claimer) external nonReentrant {
        if (gameSettings.isPaused(10) == 1) revert Paused();
        if (_claimer.ownerOf(_claimerId) != msg.sender) revert WrongParams(1);
        if (_claimer == dps) revert WrongParams(2);
        if (!whitelistedClaimers[_claimer]) revert WrongParams(3);

        flagship.mint(msg.sender, claimFlagshipIdCounterV2);

        emit FlagshipMinted(msg.sender, _claimerId, claimFlagshipIdCounterV2);

        ++claimFlagshipIdCounterV2;
    }
    ...
    function repairFlagship(DPSFlagshipI _flagship, uint256 _flagshipId) public {
        ...
        if (_flagship.ownerOf(_flagshipId) != msg.sender) revert WrongParams(2);

        doubloonsMinter.burnDoubloons(msg.sender, gameSettings.repairFlagshipCost());
        _flagship.upgradePart(FLAGSHIP_PART.HEALTH, _flagshipId, 100);
     ...
     */
    function repairFlagships(DPSFlagshipI _flagship, uint256[] calldata _flagshipIds) external nonReentrant {
    ...
    function upgradeFlagship(
        ...
        uint256 _flagshipId,
        uint8[6] calldata _levels
    ) external nonReentrant {
        if (gameSettings.isPaused(12) == 1) revert Paused();
        if (!flagshipsAllowed[_flagship]) revert Unauthorized();

        // needs to be the owner of the flagship
        if (_flagship.ownerOf(_flagshipId) != msg.sender) revert WrongParams(3);

        uint8[7] memory currentLevels = _flagship.getPartsLevel(_flagshipId);
        ...
        for (uint256 i = 1; i < 7; ++i) {
            ...
            _flagship.upgradePart(part, _flagshipId, currentLevel);
        }
    ...
    function buySupportShips(SUPPORT_SHIP_TYPE _type, uint256 _quantity) external nonReentrant {
        ...
        doubloonsMinter.burnDoubloons(msg.sender, doubloonsPerShip * _quantity);
    ...
    function computeDoubloonsForUpgrade(uint256 _startLevel, uint256 _endLevel) private view returns (uint256) {
        ...
        uint256 doubloons;
        for (uint i = _startLevel + 1; i <= _endLevel; ++i) {
    ...
    function setContract(
        ...
        else if (_target == 4) doubloonsMinter = DPSDoubloonMinterI(_contract);
        ...
        else if (_target == 7) doubloon = IERC20MintableBurnable(_contract);
