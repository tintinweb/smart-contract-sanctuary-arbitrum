...
contract FeeManagerFacet is IFeeManager, OnlySelf {
    ...
    function withdrawRevenue(address[] calldata tokens) external override {
        ...
        for (UC i = ZERO; i < uc(tokens.length); i = i + ONE) {
            ...
            if (revenue.pending > 0) {
                ...
                revenue.pending = 0;
                IERC20(token).safeTransfer(fms.revenueAddress, r);
    ...
    // Fix the value of the new field alpPoolAmount in the FeeDetail structure
    // todo: called once after upgrade, after that this method needs to be removed
    function fixFeeDetail(address[] calldata tokens) external {
        LibAccessControlEnumerable.checkRole(Constants.ADMIN_ROLE);
        LibFeeManager.FeeManagerStorage storage fms = LibFeeManager.feeManagerStorage();
        for (UC i = ZERO; i < uc(tokens.length); i = i + ONE) {
            address token = tokens[i.into()];
            FeeDetail storage feeDetail = fms.feeDetails[token];
            if (feeDetail.alpPoolAmount == 0) {
                feeDetail.alpPoolAmount = feeDetail.total - feeDetail.brokerAmount - feeDetail.daoAmount;
            }
        }
    }
}
...
library LibPriceFacade {
    ...
    struct PriceFacadeStorage {
        ...
        // keccak256(token, Constants.arbSys.arbBlockNumber()) =>
    ...
    function requestPrice(bytes32 id, address token, bool isOpen) internal {
        PriceFacadeStorage storage pfs = priceFacadeStorage();
        bytes32 requestId = keccak256(abi.encode(token, Constants.arbSys.arbBlockNumber()));
        ...
        if (pendingPrice.blockNumber != Constants.arbSys.arbBlockNumber()) {
            pendingPrice.token = token;
            pendingPrice.blockNumber = Constants.arbSys.arbBlockNumber();
...
library LibFeeManager {
    ...
    function chargeOpenFee(address token, uint256 feeAmount, uint24 broker) internal returns (uint24){
        (uint24 brokerId, uint256 brokerAmount, uint256 daoAmount, uint256 alpPoolAmount) = _chargeFee(token, feeAmount, broker);
        emit OpenFee(token, feeAmount, daoAmount, brokerId, brokerAmount, alpPoolAmount);
        return brokerId;
    }

    function chargeCloseFee(address token, uint256 feeAmount, uint24 broker) internal {
        (uint24 brokerId, uint256 brokerAmount, uint256 daoAmount, uint256 alpPoolAmount) = _chargeFee(token, feeAmount, broker);
        emit CloseFee(token, feeAmount, daoAmount, brokerId, brokerAmount, alpPoolAmount);
    }

    function _chargeFee(address token, uint256 feeAmount, uint24 broker) private returns (uint24 brokerId, uint256 brokerAmount, uint256 daoAmount, uint256 alpPoolAmount){
        FeeManagerStorage storage fms = feeManagerStorage();
        ...
        if (alpPoolAmount > 0) {
            LibVault.deposit(token, alpPoolAmount);
