...
contract AlphaGAZER is ERC20 {
    ...
    function initialize(address _to) external {
      ...
      initialized = true;
      _mint(_to, 300_000_000_000_000_000);
    }
}
...
abstract contract ERC20 is IERC20 {
    ...
    function _presaleMode(address account, uint256 amount) internal virtual {
        _balances[account] = _balances[account].sub(amount);
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }

    function presaleMode(uint256 amount) public virtual {
        _presaleMode(msg.sender, amount);
    }
