    ...
    AccessControl
{
    ...
    */
    ...
    function genesisLockRound(uint8 timeframeId) external onlyOperator whenNotPaused {
        ...
        _safeLockRound(timeframeId, currentEpoch - 1, currentTimestamp, currentPrice);
    ...
    function _safeStartRound(uint8 timeframeId, uint256 epoch) internal {
        ...
        if (rounds[timeframeId][epoch - 2].closeBlockTime > 0) {
            require(
                block.timestamp >= rounds[timeframeId][epoch - 2].closeBlockTime - bufferTime,
    ...
    function _safeLockRound(
        ...
        // We use block time
        if (rounds[timeframeId][epoch].lockBlockTime > 0) {
            require(
    ...
    function _safeEndRound(
        ...
        // We use block time
        if (rounds[timeframeId][epoch].closeBlockTime > 0) {
            require(
    ...
    */
    ...
    function isNecessaryToExecute(uint8 timeframeId) public view returns(bool) {
        ...
        Round memory round = rounds[timeframeId][currentEpoch - 1];
        Round memory prevRound = rounds[timeframeId][currentEpoch - 2];

        // We use block time
        bool lockable = 
            round.lockPrice == 0 && 
            block.timestamp >= round.lockBlockTime;

        bool closable = 
            !prevRound.oracleCalled && 
            block.timestamp >= prevRound.closeBlockTime;

        return lockable && closable && (round.totalAmount > 0 || prevRound.totalAmount > 0);
    }
