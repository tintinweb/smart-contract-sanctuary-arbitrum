...
contract LiquidityLocker is ERC721,Ownable {
    ...
    function mintPosition(uint256 FUC_Amount, uint256 WETH_Amount, uint256 lockPeriod) public {
        require(lockPeriod == 1 || lockPeriod == 3 || lockPeriod == 6 || lockPeriod == 12, "Lock period must be 1, 3, 6, or 12 months");
    ...
    function withdraw(uint256 FUC_Amount, uint256 WETH_Amount) public {
    ...
    function withdrawFUC(uint256 FUC_Amount) public {
        require(msg.sender == fundManager, "Only the fund manager can withdraw tokens");
        uint256 contractFUCBalance = IERC20(FUC).balanceOf(address(this));
        require(FUC_Amount <= contractFUCBalance, "Not enough FUC tokens in the contract");
        require(IERC20(FUC).transfer(fundManager, FUC_Amount), "FUC transfer failed");
    ...
    function withdrawWETH(uint256 WETH_Amount) public {
        require(msg.sender == fundManager, "Only the fund manager can withdraw tokens");
        uint256 contractWETHBalance = IERC20(WETH).balanceOf(address(this));
        require(WETH_Amount <= contractWETHBalance, "Not enough WETH tokens in the contract");
        require(IERC20(WETH).transfer(fundManager, WETH_Amount), "WETH transfer failed");
    ...
    function redeem(uint256 tokenId, uint256 FUC_Amount, uint256 WETH_Amount) public {
        ...
        // Transfer the FUC and WETH
        require(IERC20(FUC).transfer(msg.sender, FUC_Amount), "FUC transfer failed");
        require(IERC20(WETH).transfer(msg.sender, WETH_Amount), "WETH transfer failed");

        emit Withdraw(msg.sender, FUC_Amount, WETH_Amount);
    }
    ...
    function redeem(uint256 tokenId) public {
        ...
        // Delete the NFTData for the token
