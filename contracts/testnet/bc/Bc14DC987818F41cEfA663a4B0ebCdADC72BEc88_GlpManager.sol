    ...
    IGlpManager
{
    ...
    function setShortsTrackerAveragePriceWeight(
        ...
        require(
            shortsTrackerAveragePriceWeight <= PercentageMath.PERCENTAGE_FACTOR,
            "GlpManager: invalid weight"
        );
    ...
    function addLiquidity(
        ...
        if (inPrivateMode) {
            revert("GlpManager: action not enabled");
        }
        return
            _addLiquidity(
                msg.sender,
    ...
    function getPrice(bool _maximise) external view override returns (uint256) {
        ...
        uint256 supply = IERC20Upgradeable(glp).totalSupply();
        return (aum * PercentageMath.GLP_PRECISION) / supply;
    }
    ...
    function getAumInEthg(
        ...
        return
            (aum * PercentageMath.ETHG_DECIMALS) /
    ...
    function getAum(bool maximise) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (_vault.stableTokens(token)) {
                ...
                // TODO need our oracle get bend oracle data offchain if we deploy protocol to L2
    ...
    function getGlobalShortAveragePrice(
        ...
        if (_shortsTrackerAveragePriceWeight == 0) {
            ...
            _shortsTrackerAveragePriceWeight == PercentageMath.PERCENTAGE_FACTOR
        ) {
    ...
    function _addLiquidity(
        ...
        require(ethgAmount >= _minEthg, "GlpManager: insufficient ETHG output");

        uint256 mintAmount = aumInEthg == 0
            ? ethgAmount
            : (ethgAmount * glpSupply) / aumInEthg;
    ...
    function _addLiquidityNFT(
        ...
        IVault.NftInfo memory nftInfo = vault.getNftInfo(_nft);
        ...
        vault.mintNToken(
            _nft,
            ((nftInfo.nftLtv * PercentageMath.NTOKEN_PRECISION) /
                PercentageMath.PERCENTAGE_FACTOR)
        );
    ...
    function _removeLiquidity(
        ...
        uint256 ethgAmount = (_glpAmount * aumInEthg) / glpSupply;
    ...
    function getLpAmountForRedeemNft(
        ...
        uint256 aumInEthg = getAumInEthg(false);
        uint256 glpSupply = IERC20Upgradeable(glp).totalSupply();
        uint256 glpPrice = aumInEthg / glpSupply;

        // nft floor price
        uint256 floorPrice = vault.getMaxPrice(_nft);
        uint256 floorPriceBend = vault.getBendDAOAssetPrice(_nft);
        floorPrice = floorPrice > floorPriceBend ? floorPrice : floorPriceBend;

        needGlpAmount =
            ((floorPrice * nftInfo.nftLtv) /
                PercentageMath.PERCENTAGE_FACTOR -
                _ethAmount) /
            glpPrice;

        // calculate fee
        uint256 ethgAmount = needGlpAmount * glpPrice;
        uint256 fee = vault.getFeeWhenRedeemNft(_nft, ethgAmount);

        uint256 feeGlp = fee / glpPrice;

        needGlpAmount += feeGlp;

        return needGlpAmount;
    }
    ...
    function _removeLiquidityNFT(
        ...
        require(
            _glpAmount > 0 || _ethAmount > 0,
            "GlpManager: invalid _amount"
        );
        require(
            lastAddedAt[_account] - cooldownDuration <= block.timestamp,
            "GlpManager: cooldown duration not yet passed"
        ...
        address owner = IERC721Upgradeable(nftInfo.certiNft).ownerOf(_tokenId);
        require(owner == _receiver, "GlpManager: no certiNft");
        ...
        IMintable(glp).burn(_account, _glpAmount);
        vault.burnCNft(nftInfo.certiNft, _tokenId);
        IERC20Upgradeable(ethg).safeTransfer(address(vault), ethgAmount);
