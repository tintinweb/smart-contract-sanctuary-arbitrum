...
interface IStakeReward {
    ...
    // LibFeeReward.claimFeeReward() & LibShxReward.claimShxReward()
...
library LibStakeReward {
    ...
    function checkOncePerBlock(address user) internal {
        StakeRewardStorage storage st = stakeRewardStorage();
        require(st.lastBlockNumberCalled[user] < Chain.currentBlockNumber(), "once per block");
        st.lastBlockNumberCalled[user] = Chain.currentBlockNumber();
    }
    ...
    function claimAllReward() internal {
        LibShxReward.claimShxReward(msg.sender);
    }
}
