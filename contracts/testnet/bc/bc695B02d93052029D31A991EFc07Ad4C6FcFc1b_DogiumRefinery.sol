...
contract DogiumRefinery is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public initialLPValue; // Initial number of LP tokens of ETH-AST
    uint256 public initialLPEth; // Initial number of ETH deposited in the LP
    ...
    function setStakeToken(IERC20 _stakeToken) external onlyOwner {
        require(!factoryStarted(), "Factory is running");
        lpToken = _stakeToken;
    }
    ...
    function getRewardPerSecond() public view returns (uint256) {
        ...
        return rewardPerSecondCalculation(lpSupply, initialLPValue, initialLPEth, rewardMultiplier, rewardMultiplierDivision, baseRate);
    }
    ...
    function factoryStarted() public view returns (bool) {
    ...
    function setInitialLPEth(uint256 _initialLPEth) external onlyOwner {
        initialLPEth = _initialLPEth;
    }
    ...
    function rewardPerSecondCalculation(uint256 _lpSupply, uint256 _initialLPValue, uint256 _initialLPEth, uint256 _rewardMultiplier, uint256 _rewardMultiplierDivision, uint256 _baseRate) public pure returns (uint256) {
        if(_lpSupply == 0){
            return 0;
        }
        
        if(_initialLPValue == 0 || _initialLPEth == 0 || _rewardMultiplier == 0 || _rewardMultiplierDivision == 0) {
            return _baseRate;
        }
        return _baseRate + _rewardMultiplier.mul(_lpSupply).mul(_initialLPEth).div(_initialLPValue.mul(_rewardMultiplierDivision).mul(86400));
    ...
    function deposit(uint256 _amount) public whenNotPaused nonReentrant {
        require(factoryStarted(), "You can't deposit yet");
