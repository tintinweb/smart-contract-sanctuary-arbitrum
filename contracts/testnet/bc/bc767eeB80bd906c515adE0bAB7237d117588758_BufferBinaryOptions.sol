    ...
    AccessControl
{
    ...
    function isExpirationValid(uint256 expiration) public view returns (bool) {
        bool dayend = (expiration - block.timestamp) < 86400 &&
            expiration % 86400 == 0;
        bool weekend = ((expiration - (1 * 86400) - (17 * 3600) - (30 * 60)) %
            604800 ==
            0) && (expiration - block.timestamp < 604800); // Weekends at friday 5:30 PM UTC
        ...
        )
    {
        ...
        if (optionParams.contracts > maxContracts) {
            require(optionParams.allowPartialFill, "O29");
        }
        revisedContracts = min(optionParams.contracts, maxContracts);
