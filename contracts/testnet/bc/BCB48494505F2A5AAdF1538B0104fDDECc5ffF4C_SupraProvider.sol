...
contract SupraProvider is Access, IRandomizerProvider {
  ...
  function request(uint256 _routerRequestId, uint8 _count, uint256 minConfirmations) external onlyRouter returns (uint256 requestId_) {
    requestId_ = randomizer.generateRequest("callback(uint256,uint256[])", _count, minConfirmations);
