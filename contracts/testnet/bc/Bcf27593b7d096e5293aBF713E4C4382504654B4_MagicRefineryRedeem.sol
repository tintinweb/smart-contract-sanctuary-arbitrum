...
contract MagicRefineryRedeem is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  // Events
  //=======================================
  event MagicRefineryRedeemed(address structureAddress, uint256 structureId);
  ...
  // External
  //=======================================
  function redeem(uint256 _id) external nonReentrant whenNotPaused {
    // Transfer
    ERC721A(address(REFINERY)).safeTransferFrom(
      msg.sender,
      address(VAULT),
      _id
    );

    emit MagicRefineryRedeemed(address(REFINERY), _id);
  }
