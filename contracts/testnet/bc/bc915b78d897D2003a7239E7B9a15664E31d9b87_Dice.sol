...
contract Dice is Common, AccessControl, IDiceGame {
    ...
    function setIncentivePool(address _incentivePool) external {
        IncentivePool = IIncentivePool(_incentivePool);
    }
        ...
        returns (uint256 id)
    {
        ...
        if (wager >= IncentivePool.minBetSizeForJackpot(tokenAddress)) {
            uint256 incentiveAmount = (wager *
                IncentivePool.incentivePercentForWager()) / 10000;
    ...
    function fulfillRandomWords(
        ...
        if (game.wager >= IncentivePool.minBetSizeForJackpot(tokenAddress)) {
            jackpotAmount = randomWords[game.numBets];
            IncentivePool.transferWinnings(playerAddress, jackpotAmount);
        }
