...
contract TokenIDOETH is Ownable, ReentrancyGuard {
    ...
    function addWhiteListsOne(address[] memory _buyers)
    external
    onlyOwner
    returns (bool)
    {
    ...
    function addWhiteListsTwo(address[] memory _buyers)
    external
    onlyOwner
    returns (bool)
    {
    ...
    function witheListOneSale(uint256 amount) internal {
        require(whiteListedOne[msg.sender], "Not in whiteListOne");
    ...
    function witheListTwoSale(uint256 amount) internal {
        require(whiteListedTwo[msg.sender], "Not in whiteListTwo");
    ...
    //set WitheListOne address
    function setWitheListOneAddress(address[] memory _whiteListOneAddress) external onlyOwner returns (bool) {
        for (uint256 i = 0; i < _whiteListOneAddress.length; i++) {
            whiteListedOne[_whiteListOneAddress[i]] = true;
        }
        return true;
    }

    //set WitheListTwo address
    function setWitheListTwoAddress(address[] memory _whiteListTwoAddress) external onlyOwner returns (bool) {
        for (uint256 i = 0; i < _whiteListTwoAddress.length; i++) {
            whiteListedTwo[_whiteListTwoAddress[i]] = true;
        }
        return true;
    }
}