    ...
    IParallaxStrategy
{
    ...
    modifier onlyWhitelistedToken(address token) {
        require(
            IParallax(PARALLAX).tokensWhitelist(address(this), token),
    ...
    ) external override onlyParallax {
        compoundMinAmount = newCompoundMinAmount;
    }

    function depositLPs(
        DepositLPs memory params
    ) external override nonReentrant onlyParallax returns (uint256) {
    ...
    function swapNativeTokenAndDeposit(
        SwapNativeTokenAndDeposit memory params
    ) external payable override nonReentrant onlyParallax returns (uint256) {
    ...
    function compound(
        uint256[] memory amountsOutMin
    ) public nonReentrant onlyParallax returns (uint256) {
