...
contract SaleERC20 is Pausable, Initializable {
    ...
    function withdraw(uint256 amount) public onlySaleOwner {
        ...
        require(projectTokenBalance >= amount, "Not enough service balance");
    ...
    function createRound (
        ...
        uint256 tokenPrice_,  // How much *token to sell* to give for one *sale token*
        bool isFinal,
    ...
    function buy(uint256 payTokenAmount) public whenNotPaused onlyHealthy {
        Round memory ongoingRound = getOngoingRound();
        uint256 projectTokenDecimals = IERC20Metadata(address(projectToken)).decimals();
        uint256 payTokenDecimals = IERC20Metadata(address(payToken)).decimals();

        uint256 projectTokenAmount = ongoingRound.tokenPrice * payTokenAmount * 10**projectTokenDecimals / 10**payTokenDecimals / 10**18;
