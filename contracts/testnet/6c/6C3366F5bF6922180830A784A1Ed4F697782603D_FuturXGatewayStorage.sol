...
contract FuturXGatewayStorage is IFuturXGatewayStorage, OwnableUpgradeable {
        ...
        returns (bytes32)
    {
        ...
        // TODO: Don't know why `storage` doesn't persist here
        PendingCollateral storage data = pendingCollaterals[key];
        _updatePendingCollateral(data, param.collateralToken, param.op);
        return key;
    }

    function _updatePendingCollateral(PendingCollateral storage _data, address _collateral, uint8 _op)
        internal
    {
        // Operation = 1 means increase count
        if (_op == 1) {
            if (_data.count > 0) {
                require(_data.collateral == _collateral);
            } else {
                _data.collateral = _collateral;
            }
            _data.count++;
        }
