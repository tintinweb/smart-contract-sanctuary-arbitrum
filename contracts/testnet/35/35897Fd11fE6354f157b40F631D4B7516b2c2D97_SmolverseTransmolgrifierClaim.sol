...
contract SmolverseTransmolgrifierClaim is Initializable, SmolverseTransmolgrifierClaimAdmin{
    ...
    function claim(uint256[] calldata _smolIds, uint256[] calldata _smolMultipliers, bytes32[][] calldata _merkleProofs) public {
        //Ensure sender owns smols
        //Ensure smol ids are less than x
        //Hash smol Id with amount
        //Verify hash with proof to root
        //Mark those smol ids as claimed for this current season
        //Mint them loots
        //Ensure smol is in school for a long enough time
        //TODO

        uint256 runningLootTotal = 0;

        TokenDetails[] memory _smolsSchoolDetails = ISchool(smolsSchoolAddress).getManyTokenDetails(smolsAddress, 0, _smolIds);
        ...
        for(uint256 i = 0; i < _smolIds.length; i++){
            ...
            require(_smolsSchoolDetails[i].timestampJoined > 0 && _smolsSchoolDetails[i].timestampJoined + timeStakedToAllowClaiming <= block.timestamp, "Smol has not been in school for enough time!");
            require(!seasonNumberToSmolIdToClaimed[currentSeasonNumber][_smolIds[i]], "Smol already claimed this season!");

            //Make sure supplied info (address and count) is correct.
            if(!MerkleProof.verify(
                _merkleProofs[i], 
                smolMultiplierMerkleRoot,  
                keccak256(abi.encodePacked(_smolIds[i], _smolMultipliers[i]))
            )) revert SmolIsNotInMerkleTree(_smolIds[i], _smolMultipliers[i]);

            seasonNumberToSmolIdToClaimed[currentSeasonNumber][_smolIds[i]] = true;

            //For both numbers, 1000 is 1
            //1300 is 1.3
            //25000 is 25x
            //multiply them is 1.3 * 25 = 32.5
            runningLootTotal += (_smolMultipliers[i] * seasonNumberToMultiplier[currentSeasonNumber]);
        }



        ISmolverseLoot(smolverseLootAddress).mintLootsAsAdmin(msg.sender, runningLootTotal / 1000000);
    }
}
...
contract SmolverseTransmolgrifierClaimAdmin is Initializable, SmolverseTransmolgrifierClaimContracts {
    ...
    function updateSeasonMultiplier(uint256 _seasonId, uint256 _multiplier) public requiresEitherRole(OWNER_ROLE, ADMIN_ROLE){
        seasonNumberToMultiplier[_seasonId] = _multiplier;
    }

    function updateCurrentSeasonNumber(uint256 _currentSeasonNumber) public requiresEitherRole(OWNER_ROLE, ADMIN_ROLE){
        currentSeasonNumber = _currentSeasonNumber;
    }

    function setSmolMultiplierMerkleRoot(bytes32 _smolMultiplierMerkleRoot) public requiresEitherRole(OWNER_ROLE, ADMIN_ROLE){
        smolMultiplierMerkleRoot = _smolMultiplierMerkleRoot;
    }

    function setTimeStakedToAllowClaiming(uint256 _timeStakedToAllowClaiming) public requiresEitherRole(OWNER_ROLE, ADMIN_ROLE){
        timeStakedToAllowClaiming = _timeStakedToAllowClaiming;
    }
}
...
abstract contract SmolverseTransmolgrifierClaimState is Initializable, UtilitiesV3Upgradeable {
    ...
    error SmolIsNotInMerkleTree(uint256 _smolId, uint256 _smolMultiplier);

    //State needs
    //Mapping to store what smol IDs have claimed so far for a season
    //All season data
    //Current season number


    //Store the multiplier for each season
    mapping(uint256 => uint256) public seasonNumberToMultiplier;

    //Mapping to store whether a smol ID has claimed for a season
    mapping(uint256 => mapping(uint256 => bool)) public seasonNumberToSmolIdToClaimed;

    //Store what the current season number is.
    uint256 public currentSeasonNumber;

    //Root for smol multiplier
    bytes32 public smolMultiplierMerkleRoot;
    ...
    //Smol school address
    address public smolsSchoolAddress;

    //Time they have to be staked to claim.
    uint256 public timeStakedToAllowClaiming;
