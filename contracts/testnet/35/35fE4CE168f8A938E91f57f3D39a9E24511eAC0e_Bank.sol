...
/**
 * @title Bank.sol, Contract responsible for keeping the bankroll and distribute payouts
 */
contract Bank {
    ...
    mapping(address => uint256) private _suspendedTime;
    mapping(address => bool) private _isPlayerSuspended;
     ...
     */
    event Bank_Game_State_Changed(address gameAddress, bool isValid);
    /**
     ...
     */
    event Bank_Token_State_Changed(address tokenAddress, bool isValid);
    /**
     * @dev event emitted when max payout percentage is changed
     * @param payout new payout percentage
     */
    event Bank_Max_Payout_Changed(uint256 payout);

    /**
     * @dev event emitted when max wrapped token address is changed
     * @param wrappedToken new wrappedToken address
     */
    event Bank_Max_Wrapped_Token_Changed(address wrappedToken);
    ...
    /// @notice Initialize the contract's owner role.
    ...
    function setGame(address game, bool isValid) external onlyOwner {
        _isGame[game] = isValid;
        emit Bank_Game_State_Changed(game, isValid);
    }
    ...
    function getIsValidWager(address game, address tokenAddress) external view returns (bool) {
        return (_isGame[game] && _isTokenAllowed[tokenAddress]);
    }
     ...
     * @param isValid state to set the address to
     */
    function setTokenAddress(address tokenAddress, bool isValid) external onlyOwner {
        _isTokenAllowed[tokenAddress] = isValid;
        emit Bank_Token_State_Changed(tokenAddress, isValid);
    }

    /**
     * @dev function to get if a given token can be wagered
     * @param tokenAddress address of the token to get info for
     */
    function getIsTokenAllowed(address tokenAddress) external view returns (bool) {
        return _isTokenAllowed[tokenAddress];
    }
    ...
    function setWrappedAddress(address wrapped) external onlyOwner {
        _wrappedToken = wrapped;
        emit Bank_Max_Wrapped_Token_Changed(wrapped);
    }
    ...
    function transferPayout(
        ...
        if (tokenAddress != address(0)) {
            ...
            if (!success) {
                (bool _success, ) = _wrappedToken.call{value: payout}(abi.encodeWithSignature("deposit()"));
                if (!_success) {
                    revert();
                }
                IERC20(_wrappedToken).safeTransfer(player, payout);
            }
        }
    }
    ...
    function suspend(uint256 suspensionTime) external {
        if (_suspendedTime[msg.sender] > block.timestamp) {
            revert AlreadySuspended(_suspendedTime[msg.sender]);
        }
        _suspendedTime[msg.sender] = block.timestamp + suspensionTime;
        _isPlayerSuspended[msg.sender] = true;
    }
    ...
    function increaseSuspensionTime(uint256 suspensionTime) external {
        _suspendedTime[msg.sender] += suspensionTime;
        _isPlayerSuspended[msg.sender] = true;
    }
    ...
    function permantlyBan() external {
        _suspendedTime[msg.sender] = 2**256 - 1;
        _isPlayerSuspended[msg.sender] = true;
    }
    ...
    function liftSuspension() external {
        if (_suspendedTime[msg.sender] > block.timestamp) {
            revert TimeRemaingOnSuspension(_suspendedTime[msg.sender]);
        }
        _isPlayerSuspended[msg.sender] = false;
    }
    ...
    function isPlayerSuspended(address player) external view returns (bool, uint256) {
        return (_isPlayerSuspended[player], _suspendedTime[player]);
    }
    ...
    function _setOwner(address owner_) private {
        // Check owner address
        if (owner_ == address(0) || owner_ == _owner) {
