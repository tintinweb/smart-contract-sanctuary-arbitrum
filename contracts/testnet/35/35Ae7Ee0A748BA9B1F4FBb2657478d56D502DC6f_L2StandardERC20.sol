...
contract L2StandardERC20 is ERC20Permit, Ownable2StepUpgradeable {
    event UpdateGateway(address _oldGateway, address _newGateway);
    ...
    function initialize(string memory name_, string memory symbol_, uint8 decimals_, address gateway_) external initializer {
        gateway = gateway_;
        _decimals = decimals_;
        __Ownable2Step_init();
    ...
    function updateGateway(address _gateway) external onlyOwner {
        ...
        gateway = _gateway;
        emit UpdateGateway(oldGateway, _gateway);
    }
