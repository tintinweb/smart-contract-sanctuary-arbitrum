...
contract CrossRouter is ICrossRouter, AccessControl, ReentrancyGuard {
    ...
    mapping(uint16 chainId => uint64 nonce) private _nonces;
    ...
    function swap(SwapParams calldata swapParams) external payable override nonReentrant returns (bytes32 swapId) {
        ...
        (crossMessage.amount, crossMessage.fee) = _feeHandler.applySlippage(
            swapParams.amount,
            (cp.actualKbp * cp.weight) / pool.totalWeight,
            cp.optimalDstBandwidth,
            (cp.actualBandwidth * cp.weight) / pool.totalWeight,
        ...
        uint256 swapAmount = swapParams.amount;

        // Avoiding stack too deep
        crossMessage.id = bytes32(
            abi.encodePacked(
                _bridgeVersion, _chainId, swapParams.dstChainId, MESSAGE_TYPE.SWAP, nextNonce(swapParams.dstChainId)
            )
        );
        ...
        returns (bytes32 messageId)
    {
        ...
        messageId = bytes32(
            abi.encodePacked(_bridgeVersion, _chainId, dstChainId, MESSAGE_TYPE.ADD_LIQUIDITY, nextNonce(dstChainId))
        );
        _bridge.dispatchMessage{ value: msg.value }(
            dstChainId,
            ...
            abi.encodePacked(
                MESSAGE_TYPE.ADD_LIQUIDITY, srcPoolId, dstPoolId, vouchers, optimalDstBandwidth, messageId, cp.actualKbp
            )
        );
        ...
        onlyRole(BRIDGE_ROLE)
    {
        ...
        if (srcActualKbp != cp.actualBandwidth) cp.actualBandwidth = srcActualKbp;
    ...
    function nextNonce(uint16 chainId) private returns (uint64) {
        uint64 next = _nonces[chainId];
        _nonces[chainId] += 1;
        return next;
    }
    ...
    function _sync(PoolObject storage pool) internal {
        ...
        if (poolTotalWeight > 0) {
            ...
            if (totalDeficit == 0 && poolUndistribVouchers > 0) {
                ...
                // temporary, will change soonest
                for (uint256 i; i < cpLength;) {
                    unchecked {
                        poolChainPaths[i].actualKbp += spentVouchers;
                        ++i;
                    }
                }
