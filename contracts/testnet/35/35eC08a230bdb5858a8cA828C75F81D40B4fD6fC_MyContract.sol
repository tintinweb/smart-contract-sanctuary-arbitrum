...
// Selector for the destination chain (in the CCIP protocol)
  ...
  // Function to get the current router
  function getRouter() public view returns (address) {
    return address(router);
  }

  // Function to get the current feeToken
  function getFeeToken() public view returns (address) {
    return address(s_feeToken);
  }

  // Function to check if a chain is enabled
  function isChainEnabled(uint64 chainSelector) public view returns (bool) {
    return s_chains[chainSelector].length > 0;
  }
