...
contract RecipeERC1155 is ERC1155, ReentrancyGuard, Ownable, Pausable {
...
contract CookIngredients is  RecipeERC1155{

     ERC1155 private ingredientsERC1155;
  
    uint256 stakeIdCount = 1;
    uint256 public _timeForReward = 2 hours;


    mapping(address => mapping(uint256 => uint256[])) public userIngredientStakes;
    mapping(address => mapping(uint256 => uint256)) public userIngredientStakesTime;
    mapping(address => mapping(uint256 => uint8)) public userRecipeClaimTokenId;
    

    constructor(address _ingredientsERC1155) {
        ingredientsERC1155 = ERC1155(_ingredientsERC1155);
    } 
    ...
    function stake(uint256[] memory tokenIds, uint8 cliamTokenId) external nonReentrant whenNotPaused{
        ...
        for (uint256 i = 0; i < tokenIds.length; i += 1) {
            ...
            ingredientsERC1155.safeTransferFrom(msg.sender, address(this), tokenIds[i], 1,'');
    ...
    function canAvailClaim(uint256 _stakeId) public  view returns (bool) {
        require(userIngredientStakesTime[msg.sender][_stakeId] != 0, "Claim: No claim  found for this stakeId");
        uint256 stakedTime = userIngredientStakesTime[msg.sender][_stakeId] +   _timeForReward;
     ...
     function claimReward(uint256 _stakeId) public {     
        ...
        for(uint i=0; i< tokenIds.length; i++){
            // ingredientsERC1155.burn(msg.sender, tokenIds[i], 1);
