...
contract StakingROLP is Ownable {
    ...
    function pendingToken(address _user, uint256  _indexRw) external view returns (uint256 pending) {
        ...
        return ((userAmount[_user].mul(accTokenPerShare).div(1e12)).sub(pendingReward.rewardDebt).add(pendingReward.rewardPending));
    }
    ...
    function compound(bool[] calldata _isClaim , bool[] calldata _isCompound) external {
        ...
        for(uint i=0; i<rewardInfo.length; i++) {
            ...
            uint256 pending = ((amountStaked).mul(rewardInfo[i].accTokenPerShare)).div(1e12).sub(pendingReward.rewardDebt);
        ...
        for(uint i=0; i<rewardInfo.length; i++) {
            PendingReward storage pendingReward = rewardPending[msg.sender][rewardInfo[i].rwToken];
            pendingReward.rewardDebt = ((userAmount[msg.sender]).mul(rewardInfo[i].accTokenPerShare)).div(1e12);
        }
    }
    ...
    function updatePool() internal  {
        ...
        for(uint j=0; j<rewardInfo.length; j++) {
            ...
            rewardInfo[j].accTokenPerShare = rewardInfo[j].accTokenPerShare.add((tokenReward.mul(1e12)).div(lpSupply));
        }
        poolInfo.lastTimeReward = block.timestamp;
    }
    ...
    function deposit(uint256 _amount) external {
        ...
        for(uint i=0; i<rewardInfo.length; i++) {
            ...
            pendingReward.rewardDebt = ((amountStaked.add(_amount)).mul(rewardInfo[i].accTokenPerShare)).div(1e12);
        }
    ...
    function withdraw(uint256 _amount) public {
        ...
        for(uint i=0; i<rewardInfo.length; i++) {
            ...
            pendingReward.rewardDebt = ((amountStaked.sub(_amount)).mul(rewardInfo[i].accTokenPerShare)).div(1e12);
        }
    ...
    function claim(bool[] calldata _isClaim) external {
        ...
        for(uint i=0; i<rewardInfo.length; i++) {
            ...
            pendingReward.rewardDebt = ((amountStaked).mul(rewardInfo[i].accTokenPerShare)).div(1e12);
        }
    }
