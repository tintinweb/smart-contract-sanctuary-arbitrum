...
/// [MIT License]
/// @title Base64
/// @notice Provides a function for encoding some bytes in base64, a library used for VotingEscrow contract
...
contract VotingEscrow is Initializable, IERC721Upgradeable, IERC721MetadataUpgradeable, IVotesUpgradeable {
     ...
     * @param token_addr `MVA` token address
     */
    function initialize(
        address token_addr
    ) public initializer {
    ...
    string constant public name = "veMINERVA";
    string constant public symbol = "veMVA";
    ...
    function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) internal pure returns (string memory output) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "lock #', toString(_tokenId), '", "description": "Minerva locks, can be used to boost gauge yields, vote on token emission, and receive bribes", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function _deposit_for(
        ...
        if (_value != 0 && deposit_type != DepositType.MERGE_TYPE) {
            assert(IERC20Upgradeable(token).transferFrom(from, address(this), _value));
        }
    ...
    function withdraw(uint _tokenId) external nonReentrant {
        ...
        assert(IERC20Upgradeable(token).transfer(msg.sender, value));
