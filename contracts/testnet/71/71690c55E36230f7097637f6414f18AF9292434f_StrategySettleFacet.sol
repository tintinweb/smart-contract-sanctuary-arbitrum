...
import {IStrategySettle} from "../interfaces/IStrategySettle.sol";
 ...
 * @author dederi
 * @notice This contract is strategy settle 涵盖到期结算，每日结算.
 */
contract StrategySettleFacet is IStrategySettle {
    event StrategySettled(uint256 strategyId, uint256[] expiringPositionId);
    event StrategyUnsettleBalanceReleased(uint256 strategyId, uint256 unsettled);
    event AccountUnsettleBalanceReleased(address user, uint256 unsettled);

    /// @notice 结算到期策略
    function settleStrategy(uint256 strategyId, uint256[] memory expiringPositionId) external {
        LibAccessControlEnumerable.checkRole(Constants.KEEPER_ROLE);
        //判断 id 是不是在策略下,判断是否在清算时间范围之内
        StrategyTypes.StrategyAllData memory strategy = LibPositionCore._getStrategyAllData(strategyId);
        LibStrategyConfig._ensureAdminAndActive(strategy, strategy.owner);
        LibStrategyConfig._checkPositionIdDuplicates(expiringPositionId);

        uint256 expiringLen = expiringPositionId.length;
        for (uint256 i; i < expiringLen; ) {
            unchecked {
                ++i;
            }
        }
        emit StrategySettled(strategyId, expiringPositionId);
    }

    /// @notice 释放策略下的UnsettleBalance
    function releaseStrategyUnsettleBalance(uint256 strategyId) external {
        LibAccessControlEnumerable.checkRole(Constants.KEEPER_ROLE);
        //判断是否在到期时间
        LibPositionCore.Layout storage l = LibPositionCore.layout();
        uint256 unsettled = l.strategies[strategyId].unsettled;
        l.strategies[strategyId].realisedPnl += int256(unsettled);
        l.strategies[strategyId].unsettled = 0;

        emit StrategyUnsettleBalanceReleased(strategyId, unsettled);
    }

    /// @notice 释放总账户下的UnsettleBalance
    function releaseAccountUnsettleBalance(address user) external {
        LibAccessControlEnumerable.checkRole(Constants.KEEPER_ROLE);
        //判断是否在到期时间
        LibVault.Layout storage l = LibVault.layout();
        uint256 unsettled = l.userBalance[user].unsettledBalance;
        l.userBalance[user].balance += unsettled;
        l.userBalance[user].unsettledBalance = 0;

        emit AccountUnsettleBalanceReleased(user, unsettled);
    }
}
