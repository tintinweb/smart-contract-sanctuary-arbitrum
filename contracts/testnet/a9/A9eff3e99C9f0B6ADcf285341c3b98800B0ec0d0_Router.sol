...
contract Router is AccessControl, IRouter {
    ...
    //TODO: Add modifiers and test with access control
    function updatePayout(uint256 slotId, uint256 payout) external override {
        uint256 queueId = slotIdToQueueId[slotId];
    ...
    function openBetForSlots(OpenTxn[] calldata params) external payable {
        _validateKeeper();
        for (uint32 index = 0; index < params.length; index++) {
            ...
            if (
                tokenX.allowance(user, currentParams.targetContract) <
                (currentParams.wager * currentParams.numBets) &&
            ...
            (bool isValid, string memory errorReason) = _verifyBet(
                currentParams
            );
    ...
    function _verifyBet(
        ...
        (address signer, ) = getAccountMapping(params.player);
        ...
        if (!_validate(hashData, params.signature, signer)) {
            return (false, "Router: User signature didn't match");
        }
        return (true, "");
    }

    function _openSlots(BetParams memory params) internal {
        ISlotsGame gameContract = ISlotsGame(params.targetContract);
        require(
            queuedBets[params.queueId].signatureTimestamp == 0,
            "Router: Bet exists"
        );
        uint256 slotId = gameContract.play{value: msg.value}(
            params.wager,
            params.tokenAddress,
            params.numBets,
            params.stopGain,
        ...
        queuedBets[params.queueId] = QueuedBet({
            queueId: params.queueId,
            tokenAddress: params.tokenAddress,
            numBets: params.numBets,
            stopGain: params.stopGain,
        ...
        slotIdToQueueId[slotId] = params.queueId;
        emit OpenBet(params.player, params.queueId, params.targetContract, slotId);
    }
}
