...
/// @notice Each purchaser will be allocated 1 share for each wei USDC committed. Presale purchasers will be allocated an additional % bonus on top of all presale shares purchased.
...
contract TokenOffering is Ownable {
    ...
    uint256 immutable maximumThreshold;
    uint256 immutable walletLimit;
    uint256 immutable presaleBoost;
    uint256 immutable largeTicketBoost;
    uint256 immutable largeTicketThreshold;

    uint256 private presaleShares;
    uint256 private publicShares;

    Phase public phase;

    IERC20 public usdc;

    mapping(address purchaser => uint256 shares) public shareTracking;
    ...
    event Purchase(address indexed purchaser, uint256 USDCvalue, uint256 shares);
    ...
    constructor(
        ...
        uint256 _presaleBoost,
        uint256 _largeTicketBoost,
        uint256 _largeTicketThreshold
    ) {
        require(_presaleBoost >= 100, "Presale boost must be greater than 100.");
        require(_largeTicketBoost >= 100, "Large ticket boost must be greater than 100.");
    ...
    function purchasePublic(uint256 amount) external {
        ...
        require(shareTracking[msg.sender] + amount <= walletLimit, "User has reached wallet limit.");
        usdc.transferFrom(msg.sender, address(this), amount);
        publicShares += amount;
        shareTracking[msg.sender] += amount;
    ...
    /// @param amount The amount of USDC the user wants to commit
    function purchasePresale(uint256 amount) external {
        require(phase == Phase.Presale, "Presale must be active.");
        require(allowList[msg.sender], "User must be on allowlist.");
        require(usdc.balanceOf(address(this)) + amount <= maximumThreshold, "Fundraise has reached maximum threshold.");

        uint256 currentUserShares = shareTracking[msg.sender];

        require(amount + currentUserShares <= walletLimit, "User has reached wallet limit.");

        usdc.transferFrom(msg.sender, address(this), amount);

        uint256 totalSharesToAdd = amount;

        if (currentUserShares + amount >= largeTicketThreshold) {
            totalSharesToAdd = (totalSharesToAdd * largeTicketBoost) / 100;
            if (currentUserShares < largeTicketThreshold) {
                totalSharesToAdd += _addBoostForPriorShares();
            }
        }

        presaleShares += totalSharesToAdd;
        shareTracking[msg.sender] += totalSharesToAdd;
        purchasers.push(msg.sender);

        emit Purchase(msg.sender, amount, (amount * presaleBoost) / 100);
    }
    ...
    function getTotalShares() external view returns (uint256) {
        return ((presaleShares * presaleBoost) / 100) + publicShares;
    }

    /// @notice Returns the number of shares owned by a user
    /// @param _address The address of the user
    /// @return The number of shares owned by the user
    function getUserShares(address _address) external view returns (uint256) {
        if (allowList[_address]) {
            return ((shareTracking[_address] * presaleBoost) / 100);
        } else {
            return shareTracking[_address];
        }
    }
    ...
    function _addBoostForPriorShares() internal view returns (uint256) {
        uint256 priorShares = shareTracking[msg.sender];
        uint256 boostToAdd = ((priorShares * largeTicketBoost) / 100) - priorShares;
        return boostToAdd;
    }
}
