...
contract UXSelfClaim {
    ...
    function claim(
        ...
        // check signature  
        bytes32 msgHash = toEthSignedMessageHash(msg.sender,amount,transId);
        require(vertify(msgHash,signature), "Signature is invalid");
    ...
    function toEthSignedMessageHash(address _account, uint256 amount, string memory transId) public pure returns (bytes32) {
        bytes32 hash = keccak256(abi.encodePacked(_account, amount, transId));
        return keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", hash));
    }

    function vertify(bytes32 hash, bytes memory signature) public view returns (bool){
        require(signer == ECDSA.recover(hash, signature), "signature vertify failed");
        return true;
    }
