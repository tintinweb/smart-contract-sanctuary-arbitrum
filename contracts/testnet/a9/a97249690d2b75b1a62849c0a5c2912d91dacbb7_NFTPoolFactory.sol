...
import "./interfaces/INeutroswapMaster.sol";
import "./interfaces/tokens/IXNeutroToken.sol";
import "./NFTPool.sol";

contract NFTPoolFactory {
    INeutroswapMaster public immutable master; // Address of the master
    IERC20 public immutable neutroToken;
    IXNeutroToken public immutable xNeutroToken;

    mapping(address => address) public getPool;
    address[] public pools;

    constructor(
        INeutroswapMaster master_,
        IERC20 neutroToken_,
        IXNeutroToken xNeutroToken_
    ) {
        master = master_;
        neutroToken = neutroToken_;
        xNeutroToken = xNeutroToken_;
    }
...
interface IXNeutroToken is IERC20 {
    ...
    function isTransferWhitelisted(address account)
        external
        view
        returns (bool);
}
...
// import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/solc-0.7/contracts/utils/EnumerableSet.sol";
 ...
 * spNFTs are yield-generating positions when the NFTPool contract has allocations from the Neutroswap Master
 */
contract NFTPool is ReentrancyGuard, INFTPool, ERC721("Neutroswap staking position NFT", "spNFT") {
  ...
  struct StakingPosition {
    ...
    uint256 boostPoints; // Allocated xNEUTRO from yieldboost contract (optional)
    uint256 totalMultiplier; // lockMultiplier + allocated xNEUTRO boostPoints multiplier
    uint256 pendingXNeutroRewards; // Not harvested xNeutro rewards
    uint256 pendingNeutroRewards; // Not harvested Neutro rewards
  }
  ...
  INeutroswapMaster public master; // Address of the master
  ...
  IERC20 private _neutroToken; // NeutroToken contract's address
  IXNeutroToken private _xNeutroToken; // XNeutroToken contract's address
  ...
  uint256 private _maxBoostMultiplier = 10000; // 100%, Max boost that can be earned from xNeutro yieldBooster

  uint256 private constant _TOTAL_REWARDS_SHARES = 10000; // 100%, high limit for xNeutroRewardsShare
  uint256 public xNeutroRewardsShare = 8000; // 80%, directly defines neutroShare with the remaining value to 100%
  ...
  function initialize(INeutroswapMaster master_, IERC20 neutroToken, IXNeutroToken xNeutroToken, IERC20 lpToken) external {
    require(msg.sender == factory && !initialized, "FORBIDDEN");
    _lpToken = lpToken;
    master = master_;
    _neutroToken = neutroToken;
    _xNeutroToken = xNeutroToken;
    initialized = true;

    // to convert NEUTRO to xNEUTRO
   _neutroToken.approve(address(_xNeutroToken), type(uint256).max);
  }
  ...
  event SetXNeutroRewardsShare(uint256 xNeutroRewardsShare);
  ...
  function getPoolInfo() external view override returns (
    address lpToken, address neutroToken, address xNeutroToken, uint256 lastRewardTime, uint256 accRewardsPerShare,
    ...
    return (
    address(_lpToken), address(_neutroToken), address(_xNeutroToken), lastRewardTime, _accRewardsPerShare,
  ...
  function pendingRewards(uint256 tokenId) external view returns (uint256) {
    ...
    return position.amountWithMultiplier.mul(accRewardsPerShare).div(1e18).sub(position.rewardDebt)
      .add(position.pendingXNeutroRewards).add(position.pendingNeutroRewards);
  }
  ...
  /**
   * @dev Set the share of xNEUTRO for the distributed rewards
   * The share of NEUTRO will incidently be 100% - xNeutroRewardsShare
   *
   * Must only be called by the owner
   */
  function setXNeutroRewardsShare(uint256 xNeutroRewardsShare_) external {
    _requireOnlyOwner();
    require(xNeutroRewardsShare_ <= _TOTAL_REWARDS_SHARES, "too high");

    xNeutroRewardsShare = xNeutroRewardsShare_;
    emit SetXNeutroRewardsShare(xNeutroRewardsShare_);
  }
  ...
  function createPosition(uint256 amount, uint256 lockDuration) external nonReentrant {
    ...
    _stakingPositions[currentTokenId] = StakingPosition({
      ...
      pendingNeutroRewards: 0,
      pendingXNeutroRewards: 0
    });
   ...
   * Users should still be able to deallocate xNEUTRO from the YieldBooster contract
   */
  ...
  function _harvestPosition(uint256 tokenId, address to) internal {
    ...
    // transfer rewards
    if (pending > 0 || position.pendingXNeutroRewards > 0 || position.pendingNeutroRewards > 0) {
      uint256 xNeutroRewards = pending.mul(xNeutroRewardsShare).div(_TOTAL_REWARDS_SHARES);
      uint256 neutroAmount = pending.add(position.pendingNeutroRewards).sub(xNeutroRewards);

      xNeutroRewards = xNeutroRewards.add(position.pendingXNeutroRewards);

      // Stack rewards in a buffer if to is equal to address(0)
      if (address(0) == to) {
        position.pendingXNeutroRewards = xNeutroRewards;
        position.pendingNeutroRewards = neutroAmount;
      }
      else {
        // convert and send xNEUTRO + NEUTRO rewards
        position.pendingXNeutroRewards = 0;
        position.pendingNeutroRewards = 0;

        if(xNeutroRewards > 0) xNeutroRewards = _safeConvertTo(to, xNeutroRewards);
        // send share of NEUTRO rewards
        neutroAmount= _safeRewardsTransfer(to, neutroAmount);

        // forbidden to harvest if contract has not explicitly confirmed it handle it
        _checkOnNFTHarvest(to, tokenId, neutroAmount, xNeutroRewards);
      }
    }
  ...
  */
  ...
  function _safeRewardsTransfer(address to, uint256 amount) internal returns (uint256) {
    uint256 balance = _neutroToken.balanceOf(address(this));
  ...
  /**
   * @dev Safe convert NEUTRO to xNEUTRO function, in case rounding error causes pool to not have enough tokens
   */
  function _safeConvertTo(address to, uint256 amount) internal returns (uint256) {
    ...
    if(amount > 0 ) _xNeutroToken.convertTo(amount, to);
    return amount;
  }
   ...
   */
  function _checkOnNFTHarvest(address to, uint256 tokenId, uint256 neutroAmount, uint256 xNeutroAmount) internal {
    address nftOwner = ERC721.ownerOf(tokenId);
    if (nftOwner.isContract()) {
      bytes memory returndata = nftOwner.functionCall(abi.encodeWithSelector(
          INFTHandler(nftOwner).onNFTHarvest.selector, msg.sender, to, tokenId, neutroAmount, xNeutroAmount), "non implemented");
...
// import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/solc-0.7/contracts/token/ERC721/IERC721Receiver.sol";
