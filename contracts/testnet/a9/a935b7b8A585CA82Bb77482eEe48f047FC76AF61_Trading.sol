...
contract Trading {
	...
	//Trading Rewards
	IERC20 public kool;
	///set reward to half of the trade size
	uint256 public rewardRate = 4; // divisor of trade size for rewards
	...
	constructor(address _koolAddress) {
		kool = IERC20(_koolAddress);
		owner = msg.sender;
	}
	...
	function setKool(address _koolAddress) external onlyOwner {
		kool = IERC20(_koolAddress);
	}
	...
	function settleOrder(
		...
		if (order.isClose) {
			...
			//transfer user trading reward in kool token based on order.margin
			uint256 reward = (order.size/rewardRate) * 10**(18-UNIT_DECIMALS);
			
			if(reward > kool.balanceOf(address(this))) {
				if(kool.balanceOf(address(this)) > 0) {
					kool.transfer(user, kool.balanceOf(address(this)));
				}
			}
			else{
				kool.transfer(user, reward);
			}
