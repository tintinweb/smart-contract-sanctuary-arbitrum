...
contract Stellarchy is ReentrancyGuard, Ownable, Compounds, Lab, Dockyard, Defences {
    ...
    function generatePlanet() external payable {
        ...
        numberOfPlanets += 1;
        _resourcesTimer[numberOfPlanets] = block.timestamp;
    ...
    function getLeaderBoard() external view returns (uint256, uint256, uint256) {
    ...
    function getEnergyForUpgrade(uint256 planetId) external view returns (S.EnergyCost memory) {
        ...
        required.tritiumMine = _baseMineConsumption(mines.tritiumMine + 1) - _baseMineConsumption(mines.tritiumMine);
        return required;
    }
