...
contract POCOStake is Context, AccessControlEnumerable, ReentrancyGuard, Pausable {
    ...
    */
    ...
    function editPeriod(uint256 _pid, uint256 _durationTime, uint256 _amountPerSec) public hasAdminRole checkPid(_pid) {
        require(_pid < periods.length, "invalid param");
        if (_pid > 0) {
            require(periods[_pid - 1].durationTime < _durationTime, "invalid duration time");
        }
        endTime = endTime - periods[_pid].durationTime + _durationTime;
        periods[_pid].durationTime = _durationTime;
        periods[_pid].amountPerSec = _amountPerSec;
    }

    function addPeriod(uint256 _durationTime, uint256 _amountPerSec) public hasAdminRole {
        if (periods.length > 0) {
            require(periods[periods.length - 1].durationTime < _durationTime, "invalid duration time");
        }
    ...
    function getPoolInfo(uint256 _pid) external checkPid(_pid) view returns(uint256, uint256, uint256) {
        return (pool[_pid].accPOCOPerToken, pool[_pid].totalAmount, pool[_pid].lastRewardTime);
    }
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns(uint256 multiplier) {
        ...
        for (uint256 i = 0; i < periods.length; i++) {
            uint256 endTimeTmp = startTime + periods[i].durationTime - 1;
            if (_from >= startTimeTmp) {
                if (_to <= endTimeTmp) {
                    multiplier = multiplier + (_to - _from + 1) * periods[i].amountPerSec;
                    break;
                } else {
                    multiplier = multiplier + periods[i].durationTime + periods[i].amountPerSec;
                }
                _from = endTimeTmp + 1;
     ...
     */
    function pendingPOCO(uint256 _pid, address _user, uint256 _time) public checkPid(_pid) view returns(uint256) {
        Pool storage pool_ = pool[_pid];
        User storage user_ = user[_pid][_user];
        uint256 accPOCOPerToken = pool_.accPOCOPerToken;
        uint256 tokenSupply = pool_.totalAmount;

        if (_time > pool_.lastRewardTime && tokenSupply != 0) {
            uint256 multiplier = getMultiplier(pool_.lastRewardTime, _time);
            uint256 pocoForPool = multiplier * pool_.poolWeight / totalPoolWeight;
            accPOCOPerToken = accPOCOPerToken + (pocoForPool * (1 ether) / tokenSupply);
        }

        return user_.amount * accPOCOPerToken / (1 ether) - user_.finishedPOCO;
    }
    ...
    function addPool(address _tokenAddress, uint256 _poolWeight, bool _withUpdate) public hasAdminRole {
        ...
        emit AddPool(_tokenAddress, _poolWeight, lastRewardTime);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public checkPid(_pid) {
        ...
        if (user_.amount > 0) {
            ...
            if(pendingPOCO_ > 0) {
                _safePOCOTransfer(msg.sender, pendingPOCO_);
            }
        }
     ...
     */
    function _safePOCOTransfer(address _to, uint256 _amount) internal {
        uint256 pocoBal = poco.balanceOf(address(this));
        
        if (_amount > pocoBal) {
            poco.transfer(_to, pocoBal);
        } else {
