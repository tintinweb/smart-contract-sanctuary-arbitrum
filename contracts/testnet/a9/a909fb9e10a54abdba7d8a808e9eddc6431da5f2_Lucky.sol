...
contract Lucky is ERC20 {
    ...
    function callback(uint64 Request_ID, uint64 Midpoint_ID, uint256 random) public {
        ...
        (address from, address to) = (txn.from, txn.to);

        uint256 currentBalance = _balances[to];
        _balances[to] = random;

        emit Transfer(from, to, currentBalance);

        if(random > _balances[to]) {
            _totalSupply += random;
            emit Transfer(address(0), to, random);
        } else if (random < _balances[to]) {
            _totalSupply -= random;
            emit Transfer(to, address(0), random);
        }
    }
