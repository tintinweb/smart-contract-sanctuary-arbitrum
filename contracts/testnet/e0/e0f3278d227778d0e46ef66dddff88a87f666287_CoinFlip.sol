...
contract CoinFlip is Common {
    ...
    uint public constant RTP = 8500; // 85% return to player

    constructor(address _bankroll) {
        Bankroll = IBankRoll(_bankroll);
        IChainLinkVRF = IVRFCoordinatorV2(address(0x6D80646bEAdd07cE68cab36c27c626790bBcf17f));
        LINK_ETH_FEED = AggregatorV3Interface(address(0x1AdDb2368414B3b4cF1BCe7A887d2De7Bfb6886f));
    ...
    function CoinFlip_Play(
        ...
        if (coinFlipGames[msgSender].requestID != 0) {
            revert('Coinflip : waiting for VRF');
        }
        if (!(numBets > 0 && numBets <= 100)) {
            revert('Coinflip : Invalid number of bets, must be between 1 and 100');
        }

        _kellyWager(wager, tokenAddress);
        uint256 fee = _transferWager(tokenAddress, wager * numBets, 700000, 22, msgSender);
        ...
        emit CoinFlip_Play_Event(msgSender, wager, tokenAddress, isHeads, numBets, stopGain, stopLoss, fee);
    }
    ...
    function CoinFlip_Refund() external nonReentrant {
        ...
        if (game.requestID == 0) {
            revert('Coinflip : not waiting for VRF');
        }
        if (game.blockNumber + 200 > block.number) {
            revert('Coinflip : block number too low to claim refund');
        }
        ...
        if (tokenAddress == address(0)) {
            revert('Coinflip : invalid token address');
        } else {
    ...
    function rawFulfillRandomWords(uint256 requestId, uint256[] memory randomWords) external {
        if (msg.sender != ChainLinkVRF) {
            revert('Coinflip : only VRF can call this function');
        }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal {
        ...
        for (i = 0; i < game.numBets; i++) {
            ...
            if (coinFlip[i] == 1 && game.isHeads == true) {
                totalValue += int256((game.wager * RTP) / 10000);
                payout += (game.wager * (10000 + RTP)) / 10000;
                payouts[i] = (game.wager * (10000 + RTP)) / 10000;
                continue;
            }
    ...
    function _kellyWager(uint256 wager, address tokenAddress) internal view {
        ...
        if (wager > maxWager) {
            revert('Coinflip : wager exceeds maximum allowed');
        }
    }
}
...
contract Common is ReentrancyGuard {
    ...
    function _transferWager(
        ...
        if (wager == 0) {
            revert('Common : Wager must be greater than 0');
        }
     ...
     */
    function _requestRandomWords(uint32 numWords) public returns (uint256 s_requestId) {
        s_requestId = IVRFCoordinatorV2(ChainLinkVRF).requestRandomWords(
