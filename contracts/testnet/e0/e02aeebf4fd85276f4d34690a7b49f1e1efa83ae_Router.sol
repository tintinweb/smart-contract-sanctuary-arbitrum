...
contract Router is BaseRouter, IRouter, ReentrancyGuard {
    ...
    */
    function triggerPosition(
        ...
        require(msg.sender == address(triggerOrderManager) || msg.sender == executor, "FBD"); //Forbidden
        _modifyPosition(
            bonds[_key].owner, 
            bonds[_key].isLong, 
            bonds[_key].posId,
    ...
    function _createPrepareTransaction(
        ...
        if (_txType == TRIGGER_POSITION && positionKeeper.getPositionSize(key) == 0) {
            require(transaction.status == TRANSACTION_STATUS_PENDING && 
            (
                txns[key].txType == CREATE_POSITION_LIMIT ||
                txns[key].txType == CREATE_POSITION_STOP_LIMIT ||
                txns[key].txType == CREATE_POSITION_STOP_MARKET
            ) , "IVLTGD"); //Invalid trigerData
        } else {
