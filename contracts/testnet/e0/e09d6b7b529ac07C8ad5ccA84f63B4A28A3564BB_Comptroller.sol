...
contract Comptroller is ComptrollerV10Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when a new borrow-side ARS speed is calculated for a market
    event ArsBorrowSpeedUpdated(AToken indexed aToken, uint newSpeed);

    /// @notice Emitted when a new supply-side ARS speed is calculated for a market
    event ArsSupplySpeedUpdated(AToken indexed aToken, uint newSpeed);

    /// @notice Emitted when a new ARS speed is set for a contributor
    event ContributorArsSpeedUpdated(address indexed contributor, uint newSpeed);

    /// @notice Emitted when ARS is distributed to a supplier
    event DistributedSupplierArs(AToken indexed aToken, address indexed supplier, uint arsDelta, uint arsSupplyIndex);

    /// @notice Emitted when ARS is distributed to a borrower
    event DistributedBorrowerArs(AToken indexed aToken, address indexed borrower, uint arsDelta, uint arsBorrowIndex);
    ...
    /// @notice Emitted when ARS is granted by admin
    event ArsGranted(address recipient, uint amount);

    /// @notice Emitted when ARS accrued for a user has been manually adjusted.
    event ArsAccruedAdjusted(address indexed user, uint oldArsAccrued, uint newArsAccrued);
    ...
    function mintAllowed(address aToken, address minter, uint mintAmount) external returns (uint) {
        ...
        distributeSupplierArs(aToken, minter);
    ...
    function redeemAllowed(address aToken, address redeemer, uint redeemTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateArsSupplyIndex(aToken);
        distributeSupplierArs(aToken, redeemer);
    ...
    function borrowAllowed(address aToken, address delegate, address borrower, uint borrowAmount) external returns (uint) {
        ...
        return borrowAllowed(aToken, borrower, borrowAmount);
    }
     ...
     * @return 0 if the borrow is allowed, otherwise a semi-opaque error code (See ErrorReporter.sol)
     */
    function borrowAllowed(address aToken, address borrower, uint borrowAmount) public returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        ...
        updateArsBorrowIndex(aToken, borrowIndex);
        distributeBorrowerArs(aToken, borrower, borrowIndex);
    ...
    function seizeAllowed(
        ...
        // Keep the flywheel moving
        updateArsSupplyIndex(aTokenCollateral);
        distributeSupplierArs(aTokenCollateral, borrower);
        distributeSupplierArs(aTokenCollateral, liquidator);
    ...
    function transferAllowed(address aToken, address src, address dst, uint transferTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateArsSupplyIndex(aToken);
        distributeSupplierArs(aToken, src);
        distributeSupplierArs(aToken, dst);
    ...
    function _initializeMarket(address aToken) internal {
        ...
        ArsMarketState storage supplyState = arsSupplyState[aToken];
        ArsMarketState storage borrowState = arsBorrowState[aToken];
        ...
        if (supplyState.index == 0) {
            ...
            supplyState.index = arsInitialIndex;
        }

        if (borrowState.index == 0) {
            // Initialize borrow state index with default value
            borrowState.index = arsInitialIndex;
        }
    ...
    /**
     * @notice Set ARS speed for a single market
     * @param aToken The market whose ARS speed to update
     * @param supplySpeed New supply-side ARS speed for market
     * @param borrowSpeed New borrow-side ARS speed for market
     */
    function setArsSpeedInternal(AToken aToken, uint supplySpeed, uint borrowSpeed) internal {
        Market storage market = markets[address(aToken)];
        require(market.isListed, "ars market is not listed");

        if (arsSupplySpeeds[address(aToken)] != supplySpeed) {
            // Supply speed updated so let's update supply state to ensure that
            //  1. ARS accrued properly for the old speed, and
            //  2. ARS accrued at the new speed starts after this block.
            updateArsSupplyIndex(address(aToken));

            // Update speed and emit event
            arsSupplySpeeds[address(aToken)] = supplySpeed;
            emit ArsSupplySpeedUpdated(aToken, supplySpeed);
        }

        if (arsBorrowSpeeds[address(aToken)] != borrowSpeed) {
            ...
            // Update speed and emit event
            arsBorrowSpeeds[address(aToken)] = borrowSpeed;
            emit ArsBorrowSpeedUpdated(aToken, borrowSpeed);
        }
    }

    /**
     * @notice Accrue ARS to the market by updating the supply index
     * @param aToken The market whose supply index to update
     * @dev Index is a cumulative sum of the ARS per aToken accrued.
     */
    function updateArsSupplyIndex(address aToken) internal {
        ArsMarketState storage supplyState = arsSupplyState[aToken];
        uint supplySpeed = arsSupplySpeeds[aToken];
        uint32 blockNumber = safe32(getBlockNumber(), "block number exceeds 32 bits");
        uint deltaBlocks = sub_(uint(blockNumber), uint(supplyState.block));
        if (deltaBlocks > 0 && supplySpeed > 0) {
            uint supplyTokens = AToken(aToken).totalSupply();
            uint arsAccrued = mul_(deltaBlocks, supplySpeed);
            Double memory ratio = supplyTokens > 0 ? fraction(arsAccrued, supplyTokens) : Double({mantissa: 0});
    ...
    /**
     * @notice Accrue ARS to the market by updating the borrow index
     * @param aToken The market whose borrow index to update
     * @dev Index is a cumulative sum of the ARS per aToken accrued.
     */
    function updateArsBorrowIndex(address aToken, Exp memory marketBorrowIndex) internal {
        ArsMarketState storage borrowState = arsBorrowState[aToken];
        uint borrowSpeed = arsBorrowSpeeds[aToken];
        uint32 blockNumber = safe32(getBlockNumber(), "block number exceeds 32 bits");
        uint deltaBlocks = sub_(uint(blockNumber), uint(borrowState.block));
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint borrowAmount = div_(AToken(aToken).totalBorrows(), marketBorrowIndex);
            uint arsAccrued = mul_(deltaBlocks, borrowSpeed);
            Double memory ratio = borrowAmount > 0 ? fraction(arsAccrued, borrowAmount) : Double({mantissa: 0});
    ...
    /**
     * @notice Calculate ARS accrued by a supplier and possibly transfer it to them
     * @param aToken The market in which the supplier is interacting
     * @param supplier The address of the supplier to distribute ARS to
     */
    function distributeSupplierArs(address aToken, address supplier) internal {
        // TODO: Don't distribute supplier ARS if the user is not in the supplier market.
        // This check should be as gas efficient as possible as distributeSupplierArs is called in many places.
        // - We really don't want to call an external contract as that's quite expensive.

        ArsMarketState storage supplyState = arsSupplyState[aToken];
        uint supplyIndex = supplyState.index;
        uint supplierIndex = arsSupplierIndex[aToken][supplier];

        // Update supplier's index to the current index since we are distributing accrued ARS
        arsSupplierIndex[aToken][supplier] = supplyIndex;

        if (supplierIndex == 0 && supplyIndex >= arsInitialIndex) {
            // Covers the case where users supplied tokens before the market's supply state index was set.
            // Rewards the user with ARS accrued from the start of when supplier rewards were first
            // set for the market.
            supplierIndex = arsInitialIndex;
        }

        // Calculate change in the cumulative sum of the ARS per aToken accrued
        Double memory deltaIndex = Double({mantissa: sub_(supplyIndex, supplierIndex)});

        uint supplierTokens = AToken(aToken).balanceOf(supplier);

        // Calculate ARS accrued: aTokenAmount * accruedPerAToken
        uint supplierDelta = mul_(supplierTokens, deltaIndex);

        uint supplierAccrued = add_(arsAccrued[supplier], supplierDelta);
        arsAccrued[supplier] = supplierAccrued;

        emit DistributedSupplierArs(AToken(aToken), supplier, supplierDelta, supplyIndex);
    }

    /**
     * @notice Calculate ARS accrued by a borrower and possibly transfer it to them
     * @dev Borrowers will not begin to accrue until after the first interaction with the protocol.
     * @param aToken The market in which the borrower is interacting
     * @param borrower The address of the borrower to distribute ARS to
     */
    function distributeBorrowerArs(address aToken, address borrower, Exp memory marketBorrowIndex) internal {
        // TODO: Don't distribute supplier ARS if the user is not in the borrower market.
        // This check should be as gas efficient as possible as distributeBorrowerArs is called in many places.
        // - We really don't want to call an external contract as that's quite expensive.

        ArsMarketState storage borrowState = arsBorrowState[aToken];
        uint borrowIndex = borrowState.index;
        uint borrowerIndex = arsBorrowerIndex[aToken][borrower];

        // Update borrowers's index to the current index since we are distributing accrued ARS
        arsBorrowerIndex[aToken][borrower] = borrowIndex;

        if (borrowerIndex == 0 && borrowIndex >= arsInitialIndex) {
            // Covers the case where users borrowed tokens before the market's borrow state index was set.
            // Rewards the user with ARS accrued from the start of when borrower rewards were first
            // set for the market.
            borrowerIndex = arsInitialIndex;
        }

        // Calculate change in the cumulative sum of the ARS per borrowed unit accrued
        Double memory deltaIndex = Double({mantissa: sub_(borrowIndex, borrowerIndex)});

        uint borrowerAmount = div_(AToken(aToken).borrowBalanceStored(borrower), marketBorrowIndex);
        
        // Calculate ARS accrued: aTokenAmount * accruedPerBorrowedUnit
        uint borrowerDelta = mul_(borrowerAmount, deltaIndex);

        uint borrowerAccrued = add_(arsAccrued[borrower], borrowerDelta);
        arsAccrued[borrower] = borrowerAccrued;

        emit DistributedBorrowerArs(AToken(aToken), borrower, borrowerDelta, borrowIndex);
    }

    /**
     * @notice Calculate additional accrued ARS for a contributor since last accrual
     * @param contributor The address to calculate contributor rewards for
     */
    function updateContributorRewards(address contributor) public {
        uint arsSpeed = arsContributorSpeeds[contributor];
        uint blockNumber = getBlockNumber();
        uint deltaBlocks = sub_(blockNumber, lastContributorBlock[contributor]);
        if (deltaBlocks > 0 && arsSpeed > 0) {
            uint newAccrued = mul_(deltaBlocks, arsSpeed);
            uint contributorAccrued = add_(arsAccrued[contributor], newAccrued);

            arsAccrued[contributor] = contributorAccrued;
            lastContributorBlock[contributor] = blockNumber;
        }
    }

    /**
     * @notice Claim all the ars accrued by holder in all markets
     * @param holder The address to claim ARS for
     */
    function claimArs(address holder) public {
        return claimArs(holder, allMarkets);
    }

    /**
     * @notice Claim all the ars accrued by holder in the specified markets
     * @param holder The address to claim ARS for
     * @param aTokens The list of markets to claim ARS in
     */
    function claimArs(address holder, AToken[] memory aTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimArs(holders, aTokens, true, true);
    }

    /**
     * @notice Claim all ars accrued by the holders
     * @param holders The addresses to claim ARS for
     * @param aTokens The list of markets to claim ARS in
     * @param borrowers Whether or not to claim ARS earned by borrowing
     * @param suppliers Whether or not to claim ARS earned by supplying
     */
    function claimArs(address[] memory holders, AToken[] memory aTokens, bool borrowers, bool suppliers) public {
        for (uint i = 0; i < aTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint j = 0; j < holders.length; j++) {
                    distributeBorrowerArs(address(aToken), holders[j], borrowIndex);
                }
            }
            if (suppliers == true) {
                updateArsSupplyIndex(address(aToken));
                for (uint j = 0; j < holders.length; j++) {
                    distributeSupplierArs(address(aToken), holders[j]);
                }
            }
        }
        for (uint j = 0; j < holders.length; j++) {
    ...
    /*** Ars Distribution Admin ***/

    /**
     * @notice Transfer ARS to the recipient
     * @dev Note: If there is not enough ARS, we do not perform the transfer all.
     * @param recipient The address of the recipient to transfer ARS to
     * @param amount The amount of ARS to (possibly) transfer
     */
    function _grantArs(address recipient, uint amount) public {
        require(adminOrInitializing(), "only admin can grant ars");
        uint amountLeft = grantArsInternal(recipient, amount);
        require(amountLeft == 0, "insufficient ars for grant");
        emit ArsGranted(recipient, amount);
    }

    /**
     * @notice Set ARS borrow and supply speeds for the specified markets.
     * @param aTokens The markets whose ARS speed to update.
     * @param supplySpeeds New supply-side ARS speed for the corresponding market.
     * @param borrowSpeeds New borrow-side ARS speed for the corresponding market.
     */
    function _setArsSpeeds(AToken[] memory aTokens, uint[] memory supplySpeeds, uint[] memory borrowSpeeds) public {
        require(adminOrInitializing(), "only admin can set ars speed");

        uint numTokens = aTokens.length;
        require(numTokens == supplySpeeds.length && numTokens == borrowSpeeds.length, "Comptroller::_setArsSpeeds invalid input");

        for (uint i = 0; i < numTokens; ++i) {
            setArsSpeedInternal(aTokens[i], supplySpeeds[i], borrowSpeeds[i]);
        }
    }

    /**
     * @notice Set ARS speed for a single contributor
     * @param contributor The contributor whose ARS speed to update
     * @param arsSpeed New ARS speed for contributor
     */
    function _setContributorArsSpeed(address contributor, uint arsSpeed) public {
        require(adminOrInitializing(), "only admin can set ars speed");

        // note that ARS speed could be set to 0 to hars liquidity rewards for a contributor
        ...
        arsContributorSpeeds[contributor] = arsSpeed;

        emit ContributorArsSpeedUpdated(contributor, arsSpeed);
    }
    ...
    /**
     * @notice Return the address of the ARS token
...
contract Ars {
    ...
    string public constant name = "Aquarius";
    ...
    /**
     * @notice Construct a new Ars token
    ...
    function approve(address spender, uint rawAmount) external returns (bool) {
        ...
        if (rawAmount == uint(-1)) {
            ...
            amount = safe96(rawAmount, "Ars::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint rawAmount) external returns (bool) {
        uint96 amount = safe96(rawAmount, "Ars::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external returns (bool) {
        ...
        uint96 amount = safe96(rawAmount, "Ars::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != uint96(-1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "Ars::transferFrom: transfer amount exceeds spender allowance");
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(signatory != address(0), "Ars::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Ars::delegateBySig: invalid nonce");
        require(now <= expiry, "Ars::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "Ars::getPriorVotes: not yet determined");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "Ars::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "Ars::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "Ars::_writeCheckpoint: block number exceeds 32 bits");
