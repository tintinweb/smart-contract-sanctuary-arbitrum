...
contract VibeFactory is BoringBatchable {
    ...
    function createAirdrop(
        ...
        factory.exec(proxy, abi.encodeCall(Airdrop.setMerkleRoot, (merkleInformation.merkleRoot_, merkleInformation.externalURI_)), 0);
        factory.transferOwnership(proxy, msg.sender);
...
contract Airdrop is Ownable {
    ...
    function setMerkleRoot(bytes32 merkleRoot_, string memory externalURI_) external onlyOwner {
        merkleRoot = merkleRoot_;
        externalURI = externalURI_;
        emit LogSetMerkleRoot(merkleRoot_, externalURI_);
    }

    function nftRedemption(address recipient, bytes32[] calldata proof, uint256 originalTokenId) external payable {
        // erc1155?
        bytes memory data;
        ...
        uint256 id = _mintNFT(recipient, originalTokenId); // the same token id
