...
// STATE VARIABLES
    ...
    /// @dev e.g., getItemsBySetAndType[itemSet]->[Item Struct Arrary]
    mapping (uint256 => Item[]) private getItemsBySet;
...
// ADMIN FUNCTIONS
        ...
        onlyOwner
    {
        ...
        // Write the items to various mappings
        getItemsBySetTypeAndRarity[itemSet][item.itemType][item.rarity].push(Item(itemID, item.itemType, itemSet, item.rarity, itemValue1, itemValue2));
        getItemsBySetAndRarity[itemSet][item.rarity].push(Item(itemID, item.itemType, itemSet, item.rarity, itemValue1, itemValue2));
        getItemsBySetAndType[itemSet][item.itemType].push(Item(itemID, item.itemType, itemSet, item.rarity, itemValue1, itemValue2));
        getItemsBySet[itemSet].push(Item(itemID, item.itemType, itemSet, item.rarity, itemValue1, itemValue2));
        sets.push(itemSet);
        // Write an event
    ...
    function deregisterItemBySet(uint256 itemSet, uint256 index)
        ...
        onlyOwner
    {
        Item[] storage items = getItemsBySet[itemSet];
        // Set the Item ID
        uint256 itemID = items[index].itemID;
        // In the items array swap the last item for the item being removed
        items[index] = items[items.length - 1];
...
// VIEWS
    ...
    /// @dev Retrieve list of Items sets
        ...
        external
        view
        returns (Item[] memory itemSets) // Define the array of items to be returned.
    {
        uint256 total = sets.length;
        for(uint256 i = 0; i < total;){
            // Return list of items
            itemSets = getItemsBySet[sets[i]];
            unchecked { i++; }
        }
    }
    ...
    {
        // Return list of items
        return getItemsBySetAndType[itemSet][itemType]; 
    }
    ...
    {
        // Return list of items
        return getItemsBySetTypeAndRarity[itemSet][itemType][itemRarity];
    }
    ...
    {
        // Return list of items
        return getItemsBySetAndRarity[itemSet][itemRarity];
    }
