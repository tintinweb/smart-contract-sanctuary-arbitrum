...
contract TokenERC is IERC20 , ReentrancyGuard {
      // TYPES DEFINITIONS
    ...
    */
    ...
    constructor(string memory _name, string memory _symbol, uint256 tottalsupply , uint64 _fee1 , uint64 _fee2 , uint8 _decimals) {
    ...
    */
    ...
    //internal
    function _getAddressByIndex(uint256 index) internal pure returns (address) {
        return address(uint160(uint256(keccak256(abi.encodePacked(index)))));
    }

    function _reflect(uint256 amount) internal returns(bool){
        
        uint256 reflectionPerToken = amount.div(_totalSupply);
        
        for (uint256 i = 0; i < _totalSupply; i++) {
        address tokenHolder = _getAddressByIndex(i);
        if (tokenHolder != address(0) && !_isExcludedFromReflection[tokenHolder]) {
        uint256 reflectionAmount = reflectionPerToken.mul(_balances[tokenHolder]);
        _balances[tokenHolder] = _balances[tokenHolder].add(reflectionAmount);
        }
        }
        return true;
    }
