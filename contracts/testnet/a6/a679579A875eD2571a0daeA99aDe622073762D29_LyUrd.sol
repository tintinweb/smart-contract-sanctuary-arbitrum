...
contract LyUrd is Initializable, OwnableUpgradeable, IERC20 {
    ...
    function claimable(uint256 _batchId, address _account) public view returns (uint256) {
        require(_batchId <= currentBatchId, "LyUr: program not exist");
        return (userBalance[_batchId][_account] * redeemPrograms[_batchId].rewardPerShare) / PRECISION - userClaimed[_batchId][_account];
    }
    ...
    function allocateReward(uint256 _totalAmount) external onlyOwner {
        ...
        require(totalUnclaimReward + _totalAmount <= rewardToken.balanceOf(address(this)), "LyUr: insufficient reward balance");
