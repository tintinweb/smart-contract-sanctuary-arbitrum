...
contract MiningStrategy is Access {
  ...
  uint256 public volumeRecordStartTime = block.timestamp - 30 minutes;
  ...
  function getVolumeDayIndex() public view returns (uint256 day_) {
    day_ = (block.timestamp - volumeRecordStartTime) / 30 minutes;
  }
  ...
  function computeDollarValue(
    ...
    _dollarValue = ((_amount * vault.getDollarValue(_token)) / 10 ** _decimals); // Calculate the dollar value by multiplying the amount by the current dollar value of the token on the vault and dividing by 10^decimals
  ...
  function increaseVolume(address _input, uint256 _amount) external {
    require(msg.sender == tokenManager, "Only Token Manager"); // Ensure that the caller is the Token Manager
   ...
   * @param MAX_MINT The maximum vWINR amount to mint
   */
  function _getMultiplier(uint256 _mintedByGames, uint256 MAX_MINT) internal returns (int256) {
  ...
  function calculate(
    ...
    _mintAmount =
      (_amount * ((uint256(_getMultiplier(_mintedByGames, MAX_MINT)) * PRECISION) / parity)) /
      PRECISION;
  }
