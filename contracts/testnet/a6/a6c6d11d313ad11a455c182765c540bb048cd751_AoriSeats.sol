...
contract AoriLens {
    ...
    function tableData(AoriLens lens, OrderbookFactory orderbookFactory) public view returns (string[] memory, Orderbook[] memory, address[] memory, bool[] memory) {
        ...
        for (uint256 i; i < orderbooks.length; i++) {
            ...
            names[i] = ERC20(address(orderbooks[i].OPTION())).name();
            isCalls[i] = AoriCall(address(options[i])).isCall();
        }
        return(names, orderbooks, options, isCalls);
    }
