...
contract AccruPresale is Initializable, OwnableUpgradeable, PausableUpgradeable
{
    bytes32 public privateSaleMerkleRoot;
    bytes32 public publicSale1MerkleRoot;
    bytes32 public publicSale2MerkleRoot;
    ...
    uint256 private constant MAX_PURCHASE = 5000000000;
     ...
     * @param _privateSaleMerkleRoot merkle root for private sale
     * @param _publicSale1MerkleRoot merkle root for public sale1
     * @param _publicSale2MerkleRoot merkle root for public sale2
     */
    function initialize(
        uint256[][3] memory _stages,
        address _usdc,
        address _paymentWallet,
        bytes32 _privateSaleMerkleRoot,
        bytes32 _publicSale1MerkleRoot,
        bytes32 _publicSale2MerkleRoot
        ...
        publicSale1MerkleRoot = _publicSale1MerkleRoot;
        publicSale2MerkleRoot = _publicSale2MerkleRoot;
    }
    ...
    function verifyPrivateSalePurchase(
        ...
        uint256 purchasedAmount = userDeposits[account];
        uint256 purchasedPrice = calculateTokenPrice(purchasedAmount);
        
        return ((price >= PRIVATE_SALE_MINPURCHASE || purchasedPrice >= PRIVATE_SALE_MINPURCHASE) && purchasedPrice + price <= MAX_PURCHASE);
    }
    ...
    function verifyPublicSale1Purchase(
        ...
        uint256 purchasedAmount = userDeposits[account] -
            privateSaleDeposits[account];
        uint256 purchasedPrice = calculateTokenPrice(purchasedAmount);

        return ((price >= PUBLIC_SALE1_MINPURCHASE || purchasedPrice >= PUBLIC_SALE1_MINPURCHASE) && purchasedPrice + price <= MAX_PURCHASE);
    }
    ...
    function verifyPublicSale2Purchase(
        ...
        uint256 purchasedAmount = userDeposits[account] -
            (privateSaleDeposits[account] + publicSale1Deposits[account]);
        uint256 purchasedPrice = calculateTokenPrice(purchasedAmount);

        return ((price >= PUBLIC_SALE2_MINPURCHASE || purchasedPrice >= PUBLIC_SALE2_MINPURCHASE) && purchasedPrice + price <= MAX_PURCHASE);
    }

    /**
     * @dev To buy token in a private sale using USDC
        ...
        returns (bool)
    {
        ...
        require(
            verifyPrivateSalePurchase(_msgSender(), usdPrice),
            "Invalid purchased"
        );
        ...
        returns (bool)
    {
        ...
        uint256 usdPrice = calculateTokenPrice(amount);
        require(
            verifyPublicSale1Purchase(_msgSender(), usdPrice),
            "Invalid purchased"
        );
        ...
        returns (bool)
    {
        ...
        uint256 usdPrice = calculateTokenPrice(amount);
        require(
            verifyPublicSale2Purchase(_msgSender(), usdPrice),
            "Invalid purchased"
        );
    ...
    // functions to set the roots of each individual Merkle Tree

    function setPrivateSaleMerkleRoot(bytes32 _root) external onlyOwner {
        privateSaleMerkleRoot = _root;
    }

    function setPublicSale1MerkleRoot(bytes32 _root) external onlyOwner {
        publicSale1MerkleRoot = _root;
    }

    function setPublicSale2MerkleRoot(bytes32 _root) external onlyOwner {
        publicSale2MerkleRoot = _root;
    }
}