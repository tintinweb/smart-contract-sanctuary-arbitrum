...
contract ARBTMachine is Ownable {
    ...
    constructor() {
        arb = 0xa7d719dc9141B5B53CE9aDB24838A1DBB9335513;
        arbc = 0xB43a684A135CC9bD31cCaC13530Fdc847ef8E34C;
        dao = 0x31c96D24aA757125aD6897583115b74f334651da;
        dividend = 0x31c96D24aA757125aD6897583115b74f334651da;
        weth = 0xe39Ab88f8A4777030A534146A9Ca3B52bd5D43A3;
        collect = 0xA6E3aAe73d2e80FC96DB5C5B435030954D665d3F;
    }
    ...
    function setArbc(address arbc_) public onlyOwner {
        arbc = arbc_;
    }

    function setArb(address arb_) public onlyOwner {
        arb = arb_;
    }

    function setCollect(address collect_) public onlyOwner {
        collect = collect_;
    }
    ...
    function convert(address fromToken, address toToken, uint256 amount) public {
        require(amount > 0, "Wrong amount");
        require(fromToken != toToken, "Wrong token");
        if (fromToken == arbt) {
            IERC20(arbt).transferFrom(_msgSender(), address(this), amount);
            IERC20(arbc).transfer(_msgSender(), amount);
        } else if (fromToken == arbc) {
            IERC20(arbc).transferFrom(_msgSender(), dead, amount);
            IERC20(arbt).transfer(_msgSender(), amount);
        } else {
            revert("Wrong token");
        }
    }
