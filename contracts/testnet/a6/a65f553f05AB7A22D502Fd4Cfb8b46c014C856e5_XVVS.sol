...
contract XVVS is ERC20(" VVSBar", "xVVS") {
    using SafeMath for uint256;
    IERC20 public vvs;

    // Define the Joe token contract
    constructor(IERC20 _vvs) public {
        vvs = _vvs;
    }
    ...
    function enterpool(uint256 _amount) public {
        ...
        uint256 totalJoe = vvs.balanceOf(address(this));
        ...
        vvs.transferFrom(msg.sender, address(this), _amount);
    }
    ...
    function leavepool(uint256 _share) public {
        ...
        uint256 what = _share.mul(vvs.balanceOf(address(this))).div(
            totalShares
        );
        _burn(msg.sender, _share);
        vvs.transfer(msg.sender, what);
    }
}