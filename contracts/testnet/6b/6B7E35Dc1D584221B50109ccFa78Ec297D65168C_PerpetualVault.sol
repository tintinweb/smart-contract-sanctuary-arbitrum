...
contract PerpetualVault is Context, IPositionRouterCallbackReceiver, IPersonalVault {
   ...
   */
  function deposit(uint256 amount) external lock override {
   ...
   */
  function withdraw(address recipient, uint256 amount, bool forced) public lock override {
  ...
  function run() external lock onlyKeeper payable override {
    ...
    if (isLong == _isLong) {
      (bool success, ) = address(this).call{value: fee}(
        abi.encodeWithSignature(
          "_createIncreasePosition(address,address,uint256,uint256,uint256,bool)",
          indexToken,
          _collateralToken,
          0,
          minOut,
      ...
      if (currentPositionKey != hex'') {   // if already have a position
        nextAction = abi.encodeWithSignature(
          '_createIncreasePosition(address,address,uint256,uint256,uint256,bool)',
          indexToken, _collateralToken, 0, minOut, 0, _isLong
        );
        (bool success, ) = address(this).call{value: fee}(
          abi.encodeWithSignature(
            "_createDecreasePosition(address,address,uint256,uint256,bool,uint256)",
            indexToken, _collateralToken, 0, 0, isLong, minOut
          )
        );
        require(success, "function call failed");
      } else {
  ...
  function gmxPositionCallback(bytes32 requestKey, bool isExecuted, bool isIncrease) override external {
    ...
    if (isExecuted) {
      ...
      // commented the below lines since we assume open only one position at the same time
      // bool exists = false;
  ...
  function _createIncreasePosition(
    ...
    // uint256 sizeDelta = gmxVault.tokenToUsdMin(address(hedgeToken), amountIn) * _leverage / BASIS_POINTS_DIVISOR;
