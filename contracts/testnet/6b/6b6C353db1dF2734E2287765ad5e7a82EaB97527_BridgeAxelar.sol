...
import { IAxelarBridgeCallback } from "../interfaces/IAxelarBridgeCallback.sol";
 ...
 */
contract BridgeAxelar is IAxelarExecutable, AccessControl, IBridgeAxelar {
    ...
    mapping(address addr => bool isAggregator) public _isAggregatorMapping;

    mapping(bytes32 commadnId => bytes32 payloadHash) private _failedMessages;
    ...
    constructor(address gateway_, address gasReceiver_, address router) {
        if (gateway_ == address(0)) revert InvalidAddress();
        if (gasReceiver_ == address(0)) revert InvalidGasReceiver();
    ...
    {
        string memory chainName = _chainLookup[chainId];
        gasService.payNativeGasForContractCall{ value: msg.value }(
            address(this), chainName, _bridgeLookup[chainId], payload, refundAddress
        );
        IAxelarGateway(gateway).callContract(chainName, _bridgeLookup[chainId], payload);
        ...
        internal
    {
        ...
        if (_bridgeLookup[chainId].toAddress() != sourceAddress.toAddress()) revert BridgeMismatch();

        try this.checkAndHandleReceive(commandId, chainId, payload) { }
        catch {
            _failedMessages[commandId] = keccak256(payload);
            emit MessageFailed(chainId, sourceAddress, commandId, payload);
        }
    }
    ...
    function setIsAggregator(address addr, bool isAggregator) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _isAggregatorMapping[addr] = isAggregator;
    }
    ...
    function checkAndHandleReceive(bytes32 commandId, uint16 srcChainId, bytes calldata payload) public {
        if (msg.sender != address(this) && !hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert InsufficientAccess();
        _handleReceive(commandId, srcChainId, payload);
    }

    /*//////////////////////////////////////////////////////////////
                          INTERNAL FUCNTIONS
    //////////////////////////////////////////////////////////////*/
    function _handleReceive(bytes32 commandId, uint16 srcChainId, bytes calldata payload) internal {
        uint8 functionType;
        ...
        if (functionType == 1) {
            ...
            bool isAggregator = _isAggregatorMapping[swapMessage.receiver];
            SwapPayloadData memory swapPayload;
            if (isAggregator) {
                {
                    bytes memory swapMessagePayload = swapMessage.payload;
                    assembly ("memory-safe") {
                        let data := add(swapMessagePayload, 0x20)
                        mstore(add(swapPayload, 0), shr(96, mload(data)))
                        mstore(add(swapPayload, 0x20), shr(96, mload(add(data, 0x14))))
                    }
                }
                IERC20 hgsToken = IERC20(IAsset(_router.getPool(swapMessage.dstPoolId).poolAddress).token());
                if (swapPayload.dstToken == hgsToken) {
                    swapMessage.receiver = swapPayload.receiver;
                    isAggregator = false;
                }
            }
            ...
            emit SwapMessageReceived(swapMessage, commandId);

            if (isAggregator) {
                IAxelarBridgeCallback(swapMessage.receiver).finishSwap(
                    srcChainId,
                    swapMessage.dstPoolId,
                    swapMessage.amount,
                    swapMessage.id,
                    swapPayload.receiver,
                    swapPayload.dstToken
                );
            }
            ...
            emit LiquidityMessageReceived(liquidityMessage, commandId);
        }
    }
    ...
    {
        // Safety: No need to update the free memory pointer in this block as ICrossRouter.SwapMessage memory
        // swapMessage
        // defined in
        // the return tuple above already allocates space for all the types in the struct (for dynamic types it
        // allocates 0x20 for an offset which points to mem[0x60:0x80] since it's unintialized)
        // we store only the fixed types in the assembly block so we do not yet exceed the free memory pointer
        assembly ("memory-safe") {
    ...
    {
        // SAFETY: see _decodeSwapMessage assembly safety
        assembly ("memory-safe") {
    ...
    function getFailedMessages(bytes32 commandId) external view returns (bytes32) {
        return _failedMessages[commandId];
    }
