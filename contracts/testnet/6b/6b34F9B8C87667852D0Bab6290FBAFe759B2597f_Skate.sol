...
import "./SkateMintable.sol";
import "./interfaces/ISkate.sol";

contract Skate is Initializable, ISkate, SkateMintable {
    using CountersUpgradeable for CountersUpgradeable.Counter;

    function initialize() external initializer {
        SkateMintable.__SkateMintable_init();
    }
    ...
    function tokenURI(
        ...
        require(_exists(_tokenId), "Skate: Token does not exist");
        return skateMetadata.tokenURI(_tokenId);
    }
...
import "./SkateContracts.sol";
import "../shared/RandomlyAssignedUpgradeable.sol";

abstract contract SkateMintable is
    Initializable,
    SkateContracts,
    RandomlyAssignedUpgradeable
{
    ...
    function __SkateMintable_init() internal initializer {
        SkateContracts.__SkateContracts_init();
        tokenIdCounter.increment();
        RandomlyAssignedUpgradeable.initialize(5, 1);
    }
    ...
    function mint(
        ...
        require(
            maxSupply >= currentSupply() + _amount,
            "SkateMintable: Max supply reached"
        );
    ...
    function currentSupply() public view returns (uint256) {
        return tokenIdCounter.current() - amountBurned - 1;
    }
}
...
import "./SkateState.sol";

abstract contract SkateContracts is Initializable, SkateState {
    function __SkateContracts_init() internal initializer {
        SkateState.__SkateState_init();
    }

    function setContracts(address _skateMetadata) external onlyAdminOrOwner {
        skateMetadata = ISkateMetadata(_skateMetadata);
    }

    modifier contractsAreSet() {
        require(areContractsSet(), "SkateContracts: Contracts aren't set");
        _;
    }

    function areContractsSet() public view returns (bool) {
        return address(skateMetadata) != address(0);
    }
}
...
import "./interfaces/ISkateMetadata.sol";

abstract contract SkateState is
    Initializable,
    ISkate,
    ERC721Upgradeable,
    AdminableUpgradeable
{
    ...
    ISkateMetadata public skateMetadata;

    uint256 public amountBurned;
    uint256 public maxSupply;
    uint256 public randomRequestLimit;

    function __SkateState_init() internal initializer {
        AdminableUpgradeable.__Adminable_init();
        ERC721Upgradeable.__ERC721_init("The Lost Skate", "LOST SKATE");
...
abstract contract WithLimitedSupplyUpgradeable is Initializable {
    ...
    function __WithLimitedSupplyUpgradeable_init(
        uint256 totalSupply_
    ) internal initializer {
