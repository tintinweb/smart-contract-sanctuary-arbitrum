...
contract PrizeManagerV3 is IPrizeManagerV3, ReentrancyGuard, Ownable {
    ...
    constructor (IGamePolicy _gamePolicy, address _treasury, address _voucher, address _warehouse) {
    ...
    function claimAll(uint256 _gameId) external nonReentrant {
        for (uint256 i = 0; i < prizeInfo[_gameId].winners.length; i++) {
            _claim(_gameId, prizeInfo[_gameId].winners[i]);
        }
    }
    ...
    function _claim(uint256 _gameId, address _winner) internal {
        ...
        if (isClaimable[_winner][_gameId] != 0) {
            (address _voucherToken, uint256 _voucherAmount) = voucher.info(vouchers[_winner][_gameId]);
            uint256 discount = 0;
            ...
            // transfer to treasury
                ...
                if (vouchers[_winner][_gameId] > 0) {
                    warehouse.recover(address(voucher), vouchers[_winner][_gameId]);
                    if( _prizeInfo.amount <= _voucherAmount){
                        discount = _prizeInfo.amount;
                    } else {
                        discount = _voucherAmount;
                    }       
                }
            }
            _transfer(_prizeInfo.token, _winner, _prizeInfo.amount - discount);
    ...
    function createPrize(uint256 _gameId, address[] memory _winners, uint256[] memory _voucherIds, address _token, uint256 _amount) external onlyGameRouter {
        prizeInfo[_gameId] = PrizeInfo(
            ...
            _voucherIds,
            msg.sender,
            block.timestamp + expiredTime
        );
        for (uint256 i = 0; i < _winners.length; i++) {
            isClaimable[_winners[i]][_gameId] = 1;
            if (_voucherIds[i] > 0) {
                vouchers[_winners[i]][_gameId] = _voucherIds[i];
            }
        }
    }
