...
/*
    Unwraps a uniV3 LP Position-NFT
*/
...
contract UniswapV3Helper is IUniswapV3Helper, ERC721Receivable {
    ...
    function removeLiquidity(
        ...
        (uint256 amount0, uint256 amount1) = _collectFees(_tokenId);
    ...
    function collectFees(uint256 _tokenId)
        external
        override
    ...
    function getPrice(uint256 _sqrtPriceX96)
        external
        view
    ...
    function getUserInterest(uint256 _tokenId)
        external
        view
        returns (uint256, uint256)
    {
    ...
    function getTickAtSqrtRatio(uint160 _sqrtPriceX96)
        public
        pure
        returns (int24)
    {
    ...
    function positionTokens(uint256 _tokenId)
        public
        view
        override
        returns (address, address)
    {
        (, , address tokenA, address tokenB, , , , , , , , ) = positionManager
            .positions(_tokenId);
        return (tokenA, tokenB);
    }

    function positionLiquidity(uint256 _tokenId) public view returns (uint256) {
        (, , , , , , , uint256 liquidity, , , , ) = positionManager.positions(
            _tokenId
        );
        return liquidity;
    }

    function getUserTokenAmount(uint256 _tokenId, int24 _tick)
        public
        view
        returns (uint256, uint256)
    {
    ...
    function getSqrtPriceX96(uint256 _amount0, uint256 _amount1)
        public
        pure
        returns (uint256)
    {
        uint256 ratioX192 = (_amount0 << 192) / _amount1;
        return _sqrt(ratioX192);
    }

    function getPositionGrowthInside(uint256 _tokenId)
        public
        view
        returns (uint256, uint256)
    {
    ...
    function _collectFees(uint256 _tokenId)
        internal
        returns (uint256, uint256)
    {
...
library PoolAddress {
        ...
        returns (address pool)
    {
        ...
        pool = address(
            bytes20( //Ported here from unit256 -> bytes20, explicit conversion
                keccak256(
                    abi.encodePacked(
                        hex"ff",
                        factory,
