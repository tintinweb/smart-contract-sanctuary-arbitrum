...
// note: only temp, pool query logic should be implemented in the oracle
...
contract PairFactory is IPairFactory, SafeOwnable {
    ...
    /// @notice Permissionlessly creates and initializes a new LendingPair
    /// @dev Verifies the existens and cardinality of a TWAP Oracle
    /// @param _baseToken This is the token whose Oracle is set in the lending Controller (base pair Asset)
    /// @param _userToken  This is the token whose Oracle is a uniswap TWAP oracle (user defined pair Asset)
    /// @return Address of the newly created LendingPair
    /// Todo: Implement this. Especially: Retrival and Verification of Uni Oracle.
    function createPairPermissionless(address _baseToken, address _userToken)
        external
        returns (address)
    {
        require(_baseToken != _userToken, "PairFactory: duplicate tokens");
        require(
            _baseToken != address(0) && _userToken != address(0),
            "PairFactory: zero address"
        );
        require(
            pairByTokens[_baseToken][_userToken] == address(0),
            "PairFactory: already exists"
        );
        require(
            lendingController.isBaseAsset(_baseToken),
        ...
        lendingController.preparePool(_baseToken, _userToken);

        (address tokenA, address tokenB) = _baseToken < _userToken
            ? (_baseToken, _userToken)
            : (_userToken, _baseToken);
        ...
        emit PairCreated(lendingPair, _baseToken, _userToken);
        ...
        returns (address)
    {
        ...
        // Checks if price Oracle has been set
        require(
            lendingController.tokenSupported(tokenA) &&
                lendingController.tokenSupported(tokenB),
...
contract BeaconProxyPayable is BeaconProxy {
    receive() external payable override {
        ...
        require(
            //Todo: this is the mainnet eth address
...
/// @dev todo: Implement a new oracle logic based on a native asset as well as an externally set TWAP Oracle

// Todo: implement a function retriving the oracle,
contract LendingPair is
    ...
    ERC721Receivable
{
    INonfungiblePositionManagerSimple internal constant positionManager =
        INonfungiblePositionManagerSimple(
            0x850936F011bA4aA94BD2dA13323a27a1d6053e18
        );
    IERC721 internal constant uniPositions =
        IERC721(0x850936F011bA4aA94BD2dA13323a27a1d6053e18);
    ...
    // Idea: I could route all oracle calls through the pair factory (gas inefficient)
    // I could implement a function which decides if the native oracle or the permissionless oracle is used
    // Alternatively could see if asset is registered in Controller and if not get the price from the oracle

    // Why not use two variable instead of a mapping?
    // Could be due to compatability reasons -> change later
    ...
    // Liquidate, accure interest, collect fees
    ...
    function accrue(address _token) public {
        if (lastBlockAccrued[_token] < block.number) {
            ...
            uint256 newSupply = (newDebt * _lpRate()) / 100e18;
    ...
    // Token Price, account health, debt
        ...
        returns (uint256)
    {
        ...
        return
            (_interestRatePerBlock(_token) *
                utilizationRate(_token) *
                _lpRate()) /
            100e18 /
            100e18;
    }
    ...
    //Todo change token proces fetching
    ...
    function _unwrapUniPosition(
        ...
        if (uniPosition[_account] > 0) {
            ...
            uint256 uniMinOutput = lendingController.uniMinOutputPct();
    ...
    {
        uint256 minRate = 0;
        uint256 lowRate = 8371385083713; // 20%
        uint256 highRate = 418569254185692; // 1,000%
    ...
    function _lpRate() internal pure returns (uint256) {
        return 50e18;
    }
}
...
// This contract will be clone for each token at new LendingPair creation,
// It enables ERC20 functionality around the postion tokens
// Note: This was probably implemented this way for having one contract manage two position tokens
// Todo: Etherscan can not show the newly issued tokens, look how aave deployed it, look at ERC-4626
...
contract TransferHelper {
    ...
    IWETH internal constant WETH =
        IWETH(0xCFb46152fa89d1eE91716961e7D8854df527d8EC); // Arbitrum-goerli
