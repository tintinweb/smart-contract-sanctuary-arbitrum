...
contract GuessOnChain is Ownable {
    ...
    constructor() {
        // token = IERC20(0x3576EFf2be8B1A31782b3E8dfD962a6ddb087890);
        // token = IERC20(0x179522635726710Dd7D2035a81d856de4Aa7836c);
        token = IERC20(0x9aDB5272143C7d365E0C06159cAb247154b108c3);
    ...
    function placeBet(
        ...
        if (userRoundBet[msg.sender][_round] > 0) {
            require(
                ...
                "You can not change your choice"
            );
        }
    ...
    function claimReward(uint256 _round) public isPausable {
        require(
            claimFreeze < block.timestamp,
            "Claiming is not allowed. Wait for some time"
        );
    ...
    function claimableReward(address _user, uint256 _round)
        public
        view
        returns (uint256 _amount)
    {
        ...
        _reward += _claimableAmount;

        return _reward;
    }

    // get recent n transactions
    function getRecentTransactions(uint256 n)
        public
        view
    ...
    function getRoundTransactions(uint256 _round)
        public
        view
        returns (Transaction[] memory)
    {
    ...
    function getUserTransactions(address _user, uint256 n)
        public
        view
        returns (Transaction[] memory)
    {
    ...
    function getRecentRoundTransactions(uint256 _round, uint256 n)
        public
        view
        returns (Transaction[] memory)
    {
    ...
    function getRecentGameResults(uint256 n)
        public
        view
    ...
    function getAllUserClaimedAmounts()
        public
        view
        returns (uint256[] memory, address[] memory)
    {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            for (uint256 _round = 1; _round <= totalGameRound; _round++) {
                ...
                uint256 _reward = claimableReward(users[i], _round);
                if (_reward > 0) {
    ...
    function getAllUserRoundClaimedAmounts(uint256 _round)
        public
        view
    ...
    function getAllUserBettedAmounts(uint256 _round)
        public
        view
        returns (
            address[] memory,
    ...
    function getRecentResult() public view returns (resultType[] memory) {
        uint256 count = 0;
        uint256 _start = roundResultOrder.length > 10
            ? roundResultOrder.length - 10
            : 0;
        uint256 _end = roundResultOrder.length > 0
            ? roundResultOrder.length - 1
            : 0;
    ...
    function getAllUserBettedAmountsList(uint256 _round)
        public
        view
        returns (uint256[] memory, uint256[] memory)
    {
    ...
    function getUserBetDetails(address _user, uint256 _round)
        public
        view
    ...
    function getAllQuestions() public view returns (questionType[] memory) {
        questionType[] memory _allQuestions = new questionType[](
            totalGameRound
        );

        for (
            uint256 _roundNumber = 1;
            _roundNumber <= totalGameRound;
            _roundNumber++
        ) {
    ...
    function getQuestion(uint256 _roundNumber)
        public
        view
        returns (questionType memory)
    {
        uint256 _option1Bet = roundOptionBet[_roundNumber][1];
        uint256 _option2Bet = roundOptionBet[_roundNumber][2];
    ...
    function isUserClaimedAmount(address _user, uint256 _round)
        public
        view
        returns (uint256)
    {
    ...
    // announce results
    function announceResult(uint256 _round, uint256 _result)
        external
        onlyAdmin
    {
    ...
    function overrideResult(uint256 _round, uint256 _result)
        external
        onlyAdmin
    {
        roundResult[_round] = _result;
    ...
    function addQuestion(
        ...
        require(
            keccak256(bytes(_category)) == keccak256(bytes("price-action")) ||
                ...
                keccak256(bytes("news&politics")) ||
                keccak256(bytes(_category)) == keccak256(bytes("events")) ||
                keccak256(bytes(_category)) == keccak256(bytes("market-cap")),
            "Invalid category. Category should be one of price-action, sports, news&politics, events, market-cap"
        );

        require(
            roundEndTime[_round] == 0 &&
                roundResultTime[_round] == 0 &&
                keccak256(bytes(roundQuestion[_round])) == keccak256(bytes("")),
    ...
    function setProjectFeeAddress(address _projectFeeAddress)
        external
        onlyOwner
        returns (bool)
    {
