...
contract TheFryingPan is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    function stake(uint256[] calldata tokenIds) external nonReentrant {
        ...
        if (
            ...
            revert(
                "TheFryingPan: max staked limit reached. Please upgrade staking level."
            );
        }
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(
                bacon.ownerOf(tokenId) == msg.sender,
    ...
    function _addBaconCopToPack(uint256 tokenId) internal {
        ...
        pack[alpha].push(
            Stake({owner: msg.sender, tokenId: tokenId, value: GreasePerAlpha})
        ); // Add the Bacon to the Pack
        ...
        nonReentrant
    {
        ...
        if (owed != 0) {
            ...
            generatedGREASEByAddress[msg.sender] =
                generatedGREASEByAddress[msg.sender] +
                owed;
        }
    }
     ...
     */
    function _claimBaconFromPan(uint256 tokenId, bool unstake)
        internal
        returns (uint256 owed)
    {
        ...
        generatedGREASEByTokenId[tokenId] =
            generatedGREASEByTokenId[tokenId] +
            owed;
     ...
     * @return owed - the amount of $GREASE earned
     */
    function _claimBaconCopFromPack(uint256 tokenId, bool unstake)
        internal
        returns (uint256 owed)
    {
        require(
            bacon.ownerOf(tokenId) == address(this),
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (!isCop(tokenId)) {
                ...
                require(
                    ...
                    "msg.sender is not stake.owner 1"
                );
                bacon.safeTransferFrom(address(this), msg.sender, tokenId, ""); // send back Bacon
                delete Pan[tokenId];
                stakedBaconCounterByAddress[msg.sender] -= 1;
                ...
                require(
                    _stake.owner == msg.sender,
                    "msg.sender is not stake.owner 2"
                );
                totalAlphaStaked -= alpha; // Remove alpha from total staked
                bacon.safeTransferFrom(address(this), msg.sender, tokenId, ""); // Send back Bacon
    ...
    function upgradeHideoutStakingLevel() external nonReentrant {
        ...
        StakingLevel memory currStakingLevel = stakingLevelPerAddress[
            msg.sender
        ];
    ...
    function getOwedGrease(uint256 tokenId) public view returns (uint256 owed) {
        ...
        if (!isCop(tokenId)) {
            owed =
                ((block.timestamp - _stake.value) * DAILY_GREASE_RATE) /
                1 days;
            return owed;
        }
