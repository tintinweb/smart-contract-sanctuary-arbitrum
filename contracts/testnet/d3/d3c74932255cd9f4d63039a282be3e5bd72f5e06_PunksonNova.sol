...
enum PunkAttributeType {SEX, HAIR, EYES, BEARD, EARS, LIPS, MOUTH, FACE, EMOTION, NECK, NOSE, CHEEKS, TEETH}
    ...
    ERC721r("Punks on Arbitrum - Nova Edition", "NPUNK", 100) {
    ...
    function mint(uint256 count) public payable  {
        ...
        require(totalSupply() + count <= maxSupply() - arbPunkReserve, "Sold Out!");
    ...
    function claim_punks() public {

        uint count = MintAmount[msg.sender];

        require(punkMintEnabled, "Mint not ready yet");
        require(PunkMinter[msg.sender] == true, "Free Mint already claimed");
        require(msg.sender == tx.origin, "The minter is another contract");
        require(totalSupply() + count <= arbPunkReserve, "Sold Out!");
    ...
    function set_reserve(uint _amount) public onlyOwner {
        arbPunkReserve  = _amount;
    }
    ...
    function team_mint(uint count) external onlyOwner nonReentrant {
        _mintRandom(msg.sender, count);
    }
