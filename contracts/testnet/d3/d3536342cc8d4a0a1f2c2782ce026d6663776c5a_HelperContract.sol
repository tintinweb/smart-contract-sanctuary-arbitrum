...
import "../trading/intf/IPerpetual.sol";
import "../trading/lib/Types.sol";
import "../trading/impl/JOJODealer.sol";
...
contract HelperContract {
    ...
    function getHedgingState(
        address perpetual
    ) public view returns (HedgingState memory hedgingState) {
        (int256 primaryCredit, uint256 secondaryCredit, , , ) = IDealer(
            jojoDealer
        ).getCreditOf(address(fundingRateArbitrage));
        hedgingState.USDCPerpBalance = primaryCredit;
        hedgingState.JUSDPerpBalance = secondaryCredit;
        (address USDC, , , , , , ) = jojoDealer.state();
        uint256 USDCWalletBalance = IERC20(USDC).balanceOf(
        ...
        (int256 PositionPerpAmount, int256 PositionCreditAmount) = IPerpetual(
            perpetual
        ).balanceOf(address(fundingRateArbitrage));
    ...
    function getAccountsStates(
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            accountStates[i].accountPerpState = new AccountPerpState[](
                perp.length
            );
    ...
    function getAccountJUSDStates(
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            accountJUSDState[i].collateralState = new CollateralState[](
                collaterals.length
            );
        ...
        )
    {
        ...
        */
        ...
        if (result.orderSenderFee < 0) {
            require(
                Liquidation._isSolidIMSafe(state, orderSender),
...
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import "@openzeppelin/contracts/utils/math/SafeCast.sol";
import "../../basicIntf/IMarkPriceSource.sol";
...
library Liquidation {
    ...
    function _isMMSafe(
        ...
        (int256 netValue, , , uint256 maintenanceMargin) = getTotalExposure(
            state,
            trader
        );
    ...
    function _isIMSafe(
        ...
        (int256 netValue, , uint256 initialMargin, ) = getTotalExposure(
            state,
            trader
        );
