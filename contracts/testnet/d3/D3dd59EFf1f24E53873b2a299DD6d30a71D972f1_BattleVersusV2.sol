...
contract BattleVersusV2 is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  uint256 public lootBoxTotalChance;
  uint256[] public lootBoxChances;
  uint256[] public lootBoxTokenIds;
  uint256[] public lootBoxMinimumLevels;
  ...
  function updateLootboxConfig(
    ...
    bool _awardLootBoxForLoss
  ) external onlyAdmin {
    lootBoxChances = _chances;
    lootBoxMinimumLevels = _minimumLevels;
    lootBoxTokenIds = _tokenIds;
    ...
    lootBoxTotalChance = totalChance;
  }
  ...
  function _tryDispenseLootBox(
    ...
    if (roll < lootBoxTotalChance && (awardLootBoxForLoss || _wins > 3)) {
      for (uint256 i = 0; i < lootBoxChances.length; i++) {
        if (roll >= lootBoxChances[i]) {
          roll -= lootBoxChances[i];
          continue;
        }

        if (_attackerLevel < lootBoxMinimumLevels[i]) {
          roll = 0;
          continue;
        }

        // Assign token
        tokenId = lootBoxTokenIds[i];

        // Dispense LootBox
        LOOTBOX_DISPENSER.dispense(msg.sender, lootBoxTokenIds[i], 1);
