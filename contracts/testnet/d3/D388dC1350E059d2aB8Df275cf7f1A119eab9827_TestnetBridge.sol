...
interface ArbgoerliBridge {
    function depositETH() external payable;
}

contract TestnetBridge is Proxied {
    using SafeERC20 for IERC20;
    using Address for address payable;
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    // solhint-disable var-name-mixedcase
    address public immutable ogoerliBridge;
    address public immutable arbgoerliBridge;
    address public immutable lineagoerliBridge;
    address public immutable zksgoerliBridge;
    ...
    constructor(
        address _ogoerliBridge,
        address _arbgoerliBridge,
        address _lineagoerliBridge,
        address _zksgoerliBridge
    ) {
        ogoerliBridge = _ogoerliBridge;
        arbgoerliBridge = _arbgoerliBridge;
        lineagoerliBridge = _lineagoerliBridge;
        zksgoerliBridge = _zksgoerliBridge;
        _managers.add(msg.sender);
    }

    function bridgeToOGoerli(uint256 _amount) external senderIsOwnerOrManager {
        require(
            address(ogoerliBridge) != address(0),
            "FeeCollector.bridgeToOGoerli: zero address"
        );

        OgoerliBridge(ogoerliBridge).depositETHTo{value: _amount}(
            address(this),
            200000,
            "0x00"
        );
    ...
    function bridgeToArbGoerli(
        uint256 _amount
    ) external senderIsOwnerOrManager {
        require(
            address(arbgoerliBridge) != address(0),
            "FeeCollector.bridgeToArbGoerli: zero address"
        );

        ArbgoerliBridge(arbgoerliBridge).depositETH{value: _amount}();
    }
