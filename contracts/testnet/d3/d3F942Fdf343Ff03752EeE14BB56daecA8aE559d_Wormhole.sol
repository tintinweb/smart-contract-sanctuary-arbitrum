...
contract Wormhole is Ownable, ReentrancyGuard {
    ...
    struct Receipt {
        ...
        // slot 1
        uint256 time;
        // slot 2 : 32 x 3 + 160 = 256
        bytes32 id; // TODO: put this and from, to in a uint256 bitmap
        uint32 from; // TODO: id of the source bridge
        uint32 to; // TODO: id of the dest bridge (to avoid signatures being used in the opposite side)
        address user;
    }
    ...
    // events
    event ReceiptCreated(address indexed depositer, uint256 amount, uint256 datetime, uint32 toBridge); // TODO: datetime needed ?
    event ReceiptUsed(address indexed withdrawer, uint256 amount, uint256 datetime, uint32 fromBridge); // TODO: datetime needed ?
    ...
    function deposit(uint256 amount, uint32 toBridge) external isLive validBridge(toBridge) nonReentrant {
        ...
        Receipt memory receipt = Receipt(amount, block.timestamp, receiptID, bridgeID, toBridge, _msgSender()); // TODO: receiptID, fromBridge necessary ?

        createdReceipts[receiptID] = receipt;
        receiptsPerAddress[_msgSender()].push(receiptID);
        // TODO: maybe we don't need to store them if we use events
