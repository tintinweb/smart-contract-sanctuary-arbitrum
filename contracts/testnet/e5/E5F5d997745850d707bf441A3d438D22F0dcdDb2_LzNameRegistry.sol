...
// File contracts/layerzero/token/onft/IONFT721Core.sol
...
// File contracts/layerzero/token/onft/IONFT721.sol
...
// File contracts/layerzero/token/onft/ONFT721Core.sol
...
// File contracts/layerzero/token/onft/ONFT721.sol
...
// File contracts/ILzNameRegistry.sol



pragma solidity ^0.8.0;

interface ILzNameRegistry {
    function setSubnode(string calldata _label, bytes32 _parentNode, address _owner, address _resolver, uint64 _expiry) external;
    function extendSubnode(string calldata _label, bytes32 _parentNode, uint64 _expiry) external;
    function ownerAndExpiryOf(bytes32 node) external view returns (address owner, uint64 expiry);
}


// File contracts/LzNameRegistry.sol
...
error NameNotExists(bytes32 node);

contract LzNameRegistry is Ownable, ReentrancyGuard, IERC2981, ILzNameRegistry, ONFT721 {
    using Address for address;
    ...
    bytes32 public constant ROOT = 0xd1b0d8d72a344815839b8ed3b62f7743913962fe82524d2e9c59674353a8fa3f;
    address public treasury;
    uint256 public fee;
    mapping(bytes32 => Record) records;
    mapping(bytes32 => address) controllers;
    mapping(bytes32 => uint16) dstChains;

    modifier onlyController(bytes32 node) {
        require(controllers[node] == msg.sender, "onlyController");
        _;
    }

    modifier onlyNameOwner(bytes32 node) {
        require(ownerOf(uint256(node)) == msg.sender, "onlyNameOwner");
        _;
    }

    constructor(address _treasury, uint256 _fee, uint256 _minGasToTransfer, address _lzEndpoint) 
            ONFT721("LzNameRegistry", "ZRO", _minGasToTransfer, _lzEndpoint) {
        treasury = _treasury;
        fee = _fee;
        _safeMint(msg.sender, uint256(ROOT));
        records[ROOT].expiry = 2^64 - 1;
    } 
    ...
    function setController(bytes32 _node, address _controller) external onlyNameOwner(_node){
        controllers[_node] = _controller;
    }

    function setSubnode(string calldata _label, bytes32 _parentNode, address _owner, address _resolver, uint64 _expiry) external onlyController(_parentNode){
        bytes32 node = _makeNode(_parentNode, _label);
        uint256 tokenId = uint256(node);
        bool exists = _exists(tokenId);

        if(exists && records[node].expiry >= block.timestamp){
            revert NameUnavailable(node);
        }

        if(exists){
            _transfer(_ownerOf(tokenId), _owner, tokenId);
        }
        else{
            _safeMint(_owner, tokenId);
        }

        records[node].label = _label;
        records[node].parentNode = _parentNode;
        records[node].resolver = _resolver;
        records[node].expiry = _expiry;
    }

    function extendSubnode(string calldata _label, bytes32 _parentNode, uint64 _expiry) external onlyController(_parentNode){
        bytes32 node = _makeNode(_parentNode, _label);
        if(!_exists(uint256(node))){
            revert NameNotExists(node);
        }
        require(_expiry > records[node].expiry, "Invalid expiry");
        records[node].expiry = _expiry;
    }

    function _makeNode(bytes32 parentNode, string memory label) private pure returns (bytes32){
        return keccak256(abi.encodePacked(parentNode, keccak256(bytes(label))));
    }

    function setResolver(bytes32 _node, address _resolver) external onlyNameOwner(_node) {
        records[_node].resolver = _resolver;
    }

    function resolverOf(bytes32 _node) public view returns (address) {
        return records[_node].resolver;
    }

    function labelOf(bytes32 _node) public view returns (string memory) {
        return records[_node].label;
    }

    function expiryOf(bytes32 _node) public view returns (uint64) {
        return records[_node].expiry;
    }

    function ownerAndExpiryOf(bytes32 _node) external view returns (address owner, uint64 expiry) {
        owner = _ownerOf(uint256(_node));
        expiry = records[_node].expiry;
    }

    function controllerOf(bytes32 node) external view returns (address) {
        return controllers[node];
    }

    function dstChainOf(bytes32 node) external view returns (uint16) {
        return dstChains[node];
    }
    ...
    function _send(address _from, uint16 _dstChainId, bytes memory _toAddress, uint[] memory _tokenIds, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams) internal override {
        ...
        Record[] memory infoList = new Record[](_tokenIds.length);
        for (uint i = 0; i < _tokenIds.length; i++) {
            _debitFrom(_from, _dstChainId, _toAddress, _tokenIds[i]);
            infoList[i] = records[bytes32(_tokenIds[i])];
            dstChains[bytes32(_tokenIds[i])] = _dstChainId;
        }

        bytes memory payload = abi.encode(_toAddress, _tokenIds, infoList);
    ...
    function _nonblockingLzReceive(
        ...
        // decode and load the toAddress
        (bytes memory toAddressBytes, uint[] memory tokenIds, Record[] memory recordList) = abi.decode(_payload, (bytes, uint[], Record[]));
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            records[bytes32(tokenIds[i])] = recordList[i];
        }
    ...
    function estimateSendBatchFee(uint16 _dstChainId, bytes memory _toAddress, uint[] memory _tokenIds, bool _useZro, bytes memory _adapterParams) public view override(ONFT721Core, IONFT721Core) returns (uint nativeFee, uint zroFee) {
        Record[] memory recordList = new Record[](_tokenIds.length);
        for (uint i = 0; i < _tokenIds.length; i++) {
            recordList[i] = records[bytes32(_tokenIds[i])];
        }
        bytes memory payload = abi.encode(_toAddress, _tokenIds, recordList);
        return lzEndpoint.estimateFees(_dstChainId, address(this), payload, _useZro, _adapterParams);
    }
}