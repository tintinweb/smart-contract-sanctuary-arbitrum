...
contract RageDnDepository is Ownable, IDepository {
    ...
    function depositInsurance(uint256 amount, address from) external onlyOwner {
        // uint256 allowance = IERC20(asset).allowance(from, address(this));
        // if (allowance < amount) {
    ...
    /// @dev Can only be called by owner. 
    /// Can only withdraw <= what has been previously deposited through insurance deposits
    /// @param amount The amount to withdraw
    /// @param receiver The account to withdraw to
    function withdrawInsurance(uint256 amount, address receiver) external onlyOwner {
    ...
    /// @param assetAmount The amount of assets to deposit in asset.decimals()
    /// @return redeemableAmount the amount of redeemable that should be minted from this deposit
    ...
    /// @param redeemableAmount The amount to redeemin redeemable.decimals()
    /// @return assetAmount The asset amount withdrawn by this redemption
    function redeem(uint256 redeemableAmount) external onlyController returns (uint256) {
    ...
    /// dev redeem a given number of vault shares
    function redeemShares(uint256 shares) external onlyController returns (uint256, uint256) {
    ...
    /// TODO: remove this function. Profits are only to be used for UXP buybacks and burn uxp, code that into the function.
    function withdrawProfits(uint256 amount, address receiver) external onlyOwner {
        int256 pnl = getUnrealizedPnl();
        if (pnl < 0 || amount > uint256(pnl)) {
            revert();
        }
        vault.withdraw(amount, receiver, address(this));
        realizedPnl += amount;
