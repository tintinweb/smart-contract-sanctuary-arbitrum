...
// File: contracts\AsteroidDepotMaintenance.sol

pragma solidity ^0.8.4;
contract AsteroidDepotMaintenance is Ownable {
    AsteroidDepot public asteroidDepot;

    constructor (AsteroidDepot _asteroidDepot) {
        asteroidDepot = _asteroidDepot;
    }

    function frontRun() public onlyOwner {
        require(asteroidDepot.totalSupply() == 0, "share supply is not 0");
        asteroidDepot.setStakeStartTime(block.timestamp);
        require(asteroidDepot.stakeStarted(), "stake not started");
        asteroidDepot.stake(1*1e18);
        require(asteroidDepot.totalSupply() == 1*1e18, "share supply is not 1");
        asteroidDepot.prepareDelayedUnstake(1*1e18);
        require(asteroidDepot.unlockAmounts(address(this)) > 0 && asteroidDepot.asteroidBalance() == 0, "pending and contract balances dont match expectation");
    }

    function claimDelayedUnstake() public onlyOwner {
        require(block.timestamp > asteroidDepot.unlockTimestamps(address(this)), "cannot claim yet");
        uint256 unlockAmount = asteroidDepot.unlockAmounts(address(this));
        asteroidDepot.claimDelayedUnstake(unlockAmount);
    }

    function releasericeAsteroidDepotOwnership() public onlyOwner {
        asteroidDepot.transferOwnership(owner());
    }
}