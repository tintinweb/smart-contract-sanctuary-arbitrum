...
contract Processor is Roles, ERC2771Context, IProcessor {
        ...
        override
    {
        ...
        _excutePostion(id, chainlinkPrice, true, closeType);
    }
        ...
        override
    {
        ...
        Types.Position memory position = positionStorage.getPosition(id);
        require(gasLess.owner == _msgSender(), "!owner-call");
        uint256 chainlinkPrice = chainlink.getPrice(position.pairId);
    ...
    function _excutePostion(
        ...
        IPosition.Master memory master = positionStorage.closePosition(
            id,
            price,
            pnl,
            closeType
        ...
        if (
            master.sharePercent == 0 || uint256(finalAmount) < position.amount
        ) {
            pool.settlePosition(position.owner, uint256(finalAmount), pnl);
            pool.creditFee(id, feeClose, isSelf ? 0 : executionFee);
        } else {
            uint256 amountShare = ((uint256(finalAmount) - position.amount) *
                master.sharePercent) / BPS;
            pool.settlePosition(
                position.owner,
                uint256(finalAmount) - amountShare,
                pnl
            );
            pool.transferOut(master.owner, amountShare);
            pool.creditFee(id, feeClose, isSelf ? 0 : executionFee);
        }
        position.amount = 0;
