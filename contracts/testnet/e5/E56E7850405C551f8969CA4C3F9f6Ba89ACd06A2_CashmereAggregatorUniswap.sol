...
contract CashmereAggregatorUniswap is AccessControl, ReentrancyGuard, EIP712 {
    ...
    bytes32 public constant CONTINUE_EXECUTOR_ROLE = keccak256("CONTINUE_EXECUTOR_ROLE");
    ...
    constructor() EIP712("Cashmere Swap", "0.0.2") {}
    ...
    function updateAssetRouter(IAssetRouter assetRouter_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        assetRouter = assetRouter_;
    }

    function updateBridge(IBridge bridge_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        bridge = bridge_;
    }
    ...
    function startSwap(SwapParams memory params) external payable {
        ...
        IERC20 lwsToken = IERC20(IAssetV2(assetRouter.getPool(params.lwsPoolId).poolAddress).token());
        require(
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
        ...
        if (address(params.srcToken) == NATIVE_TOKEN) {
            ...
            params.srcToken.safeTransferFrom(msg.sender, address(this), params.srcAmount);
        }
        uint256 returnAmount;
        if (params.srcToken != lwsToken /*&& params.router1Inch != address(0)*/) {
            uint256 srcBefore = params.srcToken.balanceOf(address(this)) - params.srcAmount;
            ...
            uint256 unspentAmount = params.srcToken.balanceOf(address(this)) - srcBefore;
    ...
    function continueSwap(ContinueSwapParams memory params) external onlyRole(CONTINUE_EXECUTOR_ROLE) {
        require(!continuedSwaps[params.srcChainId][params.id], "already continued");
        IAssetRouter.SwapMessage memory swapMsg = bridge.getReceivedSwaps(params.srcChainId, params.id);
        ...
        IERC20 hgsToken = IERC20(IAssetV2(assetRouter.getPool(payload.hgsPoolId).poolAddress).token());
        ...
        uint256 returnAmount;
        if (hgsToken != payload.dstToken /*&& params.router1Inch != address(0)*/) {
            uint256 hgsBefore = hgsToken.balanceOf(address(this)) - swapMsg.amount;
            ...
            returnAmount = payload.dstToken.balanceOf(address(this)) - dstBefore;
            if (unspentAmount > 0) {
    ...
    function uniswapSwap(IERC20 fromToken, IERC20 toToken, uint256 fromAmount) internal {
        address[] memory path = new address[](2);
        path[0] = address(fromToken);
        path[1] = address(toToken);
        uniswap.swapExactTokensForTokens(fromAmount, 0, path, address(this), block.timestamp + 1000);
    }
