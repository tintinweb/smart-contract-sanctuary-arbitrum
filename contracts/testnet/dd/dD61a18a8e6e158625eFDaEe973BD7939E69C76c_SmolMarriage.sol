...
contract SmolMarriage is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, ISmolMarriage {
    ...
    function initialize(
        ...
        marriageStakingPriceInWei = 100e18;
    ...
    function requestMarriage(
        ...
        require(!marriageRequest[creature2.ownerResolver][creature2.tokenId].valid ||
        !areCreaturesEqual(marriageRequest[creature2.ownerResolver][creature2.tokenId].partner, creature1), "SMOLMARRIAGE:MARRIAGE_REQUEST_ALREADY_EXISTS_FOR_SMOL");
    ...
    function requestDivorce(ICreatureOwnerResolverRegistry.Creature memory creature) public {
        ...
        require(!hasDivorceRequest(creature) && !hasPendingDivorceRequest(creature), "SMOLMARRIAGE:DIVORCE_REQUEST_ALREADY_EXISTS");
    ...
    function marry(
        ...
        smoloveActionsVault.stake(msg.sender, marriageStakingPriceInWei, IAtlasMine.Lock.twelveMonths);
        smoloveActionsVault.stake(partner, marriageStakingPriceInWei, IAtlasMine.Lock.twelveMonths);
        ...
        emit Married(creature1, creature2);
    }
    ...
    function divorce(
        ...
        redeemableDivorces[creature2.ownerResolver][creature2.tokenId].push(RedeemableDivorce(
            true,
            pendingRing,
            penaltyFee
        ));
    ...
    function redeemDivorcedRings(ICreatureOwnerResolverRegistry.Creature memory creature) public nonReentrant {
        require(isOwner(msg.sender, creature), "SMOLMARRIAGE:NOT_OWNER_OF_CREATURE");
        require(redeemableDivorces[creature.ownerResolver][creature.tokenId].length > 0, "SMOLMARRIAGE:NO_PENDING_RING_AFTER_DIVORCE");
        ...
        for(uint256 i = 0; i < redeemableDivorces[creature.ownerResolver][creature.tokenId].length; i++) {
            if(redeemableDivorces[creature.ownerResolver][creature.tokenId][i].valid) {
                ...
                staking.withdrawRing(redeemableDivorces[creature.ownerResolver][creature.tokenId][i].ring, creature, msg.sender);
                ...
                delete redeemableDivorces[creature.ownerResolver][creature.tokenId][i];
            }
        }
        if (penaltyFee > 0) {
            magic.transferFrom(
...
interface ISmoloveActionsVault {
    ...
    function internalClaimWithdrawAllAndRestake() external;
