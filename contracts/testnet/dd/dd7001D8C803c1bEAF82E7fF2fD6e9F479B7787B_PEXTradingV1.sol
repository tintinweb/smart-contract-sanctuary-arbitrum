...
contract PEXTradingV1 is Delegatable {
    ...
    function openTrade(
        ...
        uint maxTpDist = t.openPrice * MAX_GAIN_P / 100 / t.leverage;
        require(t.tp == 0 || (t.buy ? 
            t.tp <= t.openPrice + maxTpDist :
            ((t.tp >= t.openPrice - maxTpDist) && (maxTpDist <= t.openPrice))), "TP_TOO_BIG");
    ...
    function updateOpenLimitOrder(
        ...
        uint maxTpDist = price * MAX_GAIN_P / 100 / o.leverage;
        require(tp == 0 || (o.buy ? 
            tp <= price + maxTpDist :
            ((tp >= price - maxTpDist) && (maxTpDist <= price))), "TP_TOO_BIG");

        uint maxSlDist = price * MAX_SL_P / 100 / o.leverage;
        require(sl == 0 || (o.buy ? 
            sl >= price - maxSlDist :
            sl <= price + maxSlDist), "SL_TOO_BIG");
    ...
    function updateTp(
        ...
        require(newTp == 0 || (t.buy ?
                newTp > t.openPrice :
                newTp < t.openPrice), "WRONG_TP");

        uint maxTpDist = t.openPrice * MAX_GAIN_P / 100 / t.leverage;
        require(newTp == 0 || (t.buy ? 
            newTp <= t.openPrice + maxTpDist :
            ((newTp >= t.openPrice - maxTpDist) && (maxTpDist <= t.openPrice))), "TP_TOO_BIG");
