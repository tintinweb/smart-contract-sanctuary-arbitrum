  ...
  Pausable
{
  ...
  mapping(uint256 => uint256) characteristicsToTokenId;
  mapping(uint256 => RarityItemData) tokenIdToItemData;
  IRarityItemCharacteristicDefinitions characteristicDefinitions;
  ...
  function name(uint256 _tokenId) external view returns (string memory) {
    return generateName(tokenIdToItemData[_tokenId].characteristics);
  }

  //=============================================================================
  // This function packs up to 16 uint16 values from _characteristics array into
  // a single uint256. It iterates through the input array, shifts each value by
  // a multiple of 16 bits based on its position, and combines them using bitwise
  // OR operation.
  //=============================================================================
  function packCharacteristics(
    ...
    assembly {
      for {
        let i := 0
      } lt(i, mload(_characteristics)) {
        i := add(i, 1)
      } {
        let shiftedValue := shl(
          mul(i, 16),
          mload(add(add(_characteristics, 0x20), mul(i, 2)))
        )
        output := or(output, shiftedValue)
      }
    }
    return output;
  }
  ...
  function preGenerateIds(
    ...
    for (uint16 i = 0; i < _multipleCharacteristics.length; i++) {
      uint256 packed = packCharacteristics(_multipleCharacteristics[i]);

      if (characteristicsToTokenId[packed] == 0) {
        save(packed, _multipleCharacteristics[i]);
      }
    }
  }

  // Using this function will make this particular tokenId unmintable with the characteristics alone
  // It will be disconnected from the packedCharacteristics mapping forever and will only be mintable
  // With the updated characteristics. This means it's possible that multiple token ids will share packedCharacteristics
  // This might be required for migrations/balancing in the future.
  function updateCharacteristics(
    ...
    uint256 packedCharacteristics = packCharacteristics(_newCharacteristics);
