        ...
        )
    {
        ...
        for (uint256 i = 0; i < tournamentIds.length; i++) {
            ...
            bulkTournaments[i] = ITournamentReader.TournamentWithTokens(
                ...
                leaderboard.getLeaderboardConfig(tournamentIds[i]),
                ...
                tournamentManager.tournamentRewardPools(tournamentIds[i])
            );
        }
    }

    function bulkFetchUserTournaments(
        address user,
        uint256[] calldata tournamentIds
    ) external view returns (ITournamentReader.UserTournamentData memory) {
        bool[] memory hasUserParticipated = new bool[](tournamentIds.length);
        bool[] memory hasUserClaimed = new bool[](tournamentIds.length);
        uint256[] memory userTicketCount = new uint256[](tournamentIds.length);
        uint256[] memory userReward = new uint256[](tournamentIds.length);

        for (uint256 index = 0; index < tournamentIds.length; index++) {
            bytes32 id = keccak256(abi.encode(user, tournamentIds[index]));
            hasUserParticipated[index] = tournamentManager.tournamentUsers(
                tournamentIds[index],
                user
            );
            ITournamentLeaderboard.Stats memory stats = leaderboard
                .tournmanetRanks(id);
            hasUserClaimed[index] = stats.hasClaimed;

            userTicketCount[index] = leaderboard.tournamentUserTicketCount(
                tournamentIds[index],
                user
            );
            (userReward[index], ) = leaderboard.getUserReward(
                user,
                tournamentIds[index]
            );
        }
        return
            ITournamentReader.UserTournamentData({
