...
// uint256 constant IMAGE_PNG_256_ID = uint256(keccak256("image_png_256"));
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.tokentypecomponent")
);

/// @notice creates a type definition of the types available for use, DEFAULT and ENHANCED
enum TokenType {
    DEFAULT,
    ENHANCED
}
...
/**
 * @dev maps account address entity → boolean value representing permission to modify type
 */
contract TokenTypeComponent is BaseComponent {
    /** SETUP **/
     ...
     * @param entity Entity to get value for
     */
    function setValue(uint256 entity, TokenType value) external virtual {
        _set(entity, abi.encode(value));
    }
    ...
    function getValue(
        uint256 entity
    ) external view virtual returns (TokenType value) {
        value = abi.decode(getBytes(entity), (TokenType));
    }
}
...
import {ID as PIRATE_NFT_ID} from "../PirateNFTL2.sol";
import {ANIMATION_URL_TRAIT_ID, IMAGE_TRAIT_ID} from "../../Constants.sol";
...
import {IBoolComponent} from "../../core/components/IBoolComponent.sol";
import {EntityLibrary} from "../../core/EntityLibrary.sol";
import {GAME_LOGIC_CONTRACT_ROLE} from "../../Constants.sol";

import {ID as UNLOCKED_COMPONENT_ID} from "./TokenTypeUnlockedComponent.sol";
import {ID as TOKEN_TYPE_COMPONENT_ID, TokenType, TokenTypeComponent} from "./TokenTypeComponent.sol";
import "../../GameRegistryConsumerUpgradeable.sol";

uint256 constant ID = uint256(keccak256("game.piratenation.tokentypesystem"));

// Globals
uint256 constant PIRATE_NFT_ANIMATED_IMAGE_URI_DEFAULT_ID = uint256(
    keccak256("tokentype.piratenft.animated_image_uri.default")
);
uint256 constant PIRATE_NFT_ANIMATED_IMAGE_URI_ENHANCED_ID = uint256(
    keccak256("tokentype.piratenft.animated_image_uri.enhanced")
);
uint256 constant PIRATE_NFT_IMAGE_URI_DEFAULT_ID = uint256(
    keccak256("tokentype.piratenft.image_uri.default")
);
uint256 constant PIRATE_NFT_IMAGE_URI_ENHANCED_ID = uint256(
    keccak256("tokentype.piratenft.image_uri.enhanced")
);

// Cooldowns
uint256 constant SET_TYPE_COOLDOWN_ID = uint256(
    keccak256("tokentype.set_type_cooldown_id")
);
uint32 constant SET_TYPE_COOLDOWN_TIME = 1 hours;

struct BaseImageURIs {
    string defaultImageURI;
    string enhancedImageURI;
    string defaultAnimatedImageURI;
    string enhancedAnimatedImageURI;
}

contract TokenTypeSystem is GameRegistryConsumerUpgradeable {
    using Strings for uint256;

    /** MEMBERS **/

    /// @notice mapping from tokenId to TokenType, TODO: Turn this into a component?
    mapping(uint256 => TokenType) public tokenTypes;

    /** ERRORS **/
    /// @notice
    error AccountStillInCooldown(address account);

    /// @notice Invalid account permission to modify token type for token entity
    error InvalidAccountPermission(address account);

    /// @notice Invalid token permission to modify token type for token entity
    error InvalidTokenPermission(uint256 entityId);

    /// @notice Unsupported token contract
    error UnsupportedTokenContract(address tokenContract);
    ...
    /**
     * @notice Grants a given account permission to change its tokens' types
     * @param account    Address to enable/disable
     * @param isUnlocked  Whether to allow
     */
    function setAccountUnlocked(
        address account,
        bool isUnlocked
    ) external onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _getUnlockedAccount().setValue(
            EntityLibrary.addressToEntity(account),
            isUnlocked
        );
    }

    // set the type for the given tokenId
    function setTokenType(
        uint256[] calldata tokenEntities,
        TokenType tokenType
    ) external whenNotPaused {
        address account = _getPlayerAccount(_msgSender());
        ITraitsProvider traitsProvider = _traitsProvider();
        bool isGameLogicContract = _hasAccessRole(
            GAME_LOGIC_CONTRACT_ROLE,
            account
        );

        // Check for valid account permission.
        if (
            _getUnlockedAccount().getValue(
                EntityLibrary.addressToEntity(account)
            ) ==
            false &&
            isGameLogicContract == false
        ) {
            revert InvalidAccountPermission(account);
        }
        ...
        if (
            ICooldownSystem(_getSystem(COOLDOWN_SYSTEM_ID))
                .updateAndCheckCooldown(
                    EntityLibrary.addressToEntity(account),
                    SET_TYPE_COOLDOWN_ID,
                    SET_TYPE_COOLDOWN_TIME
                )
        ) {
            revert AccountStillInCooldown(account);
        }

        // Loop through tokens and set the token type.
        uint256 tokenId;
        address tokenContract;
        BaseImageURIs memory baseImageURIs = _getBaseImageURIs();
        for (uint256 i = 0; i < tokenEntities.length; i++) {
            // Verify that contract belongs to PirateNFTL2.
            // NOTE: extend _getBaseImageURIs if supporting more contracts
            if (_getSystem(PIRATE_NFT_ID) != tokenContract) {
                revert UnsupportedTokenContract(tokenContract);
            }
            ...
            // Check if the token is owned by the sender or if the sender is the game logic contract
            if (
                IERC721(tokenContract).ownerOf(tokenId) != account &&
                isGameLogicContract == false
            ) {
                revert InvalidTokenPermission(tokenEntities[i]);
            }

            // Set the token type for a given token entity.
            _getTokenType().setValue(tokenEntities[i], tokenType);

            // Update token's image url and animation url.
            _updateTokenURIs(
                tokenContract,
                tokenId,
                tokenType,
    ...
    function _getUnlockedAccount()
        internal
        view
        returns (IBoolComponent component)
    {
        component = IBoolComponent(
            _gameRegistry.getComponent(UNLOCKED_COMPONENT_ID)
        );
    }
    ...
    function _getTokenType()
        internal
        view
        returns (TokenTypeComponent component)
    {
        component = TokenTypeComponent(
            _gameRegistry.getComponent(TOKEN_TYPE_COMPONENT_ID)
        );
    }

    function _getBaseImageURIs()
        internal
        view
        returns (BaseImageURIs memory baseImageURIs)
    {
        IGameGlobals gameGlobals = IGameGlobals(_getSystem(GAME_GLOBALS_ID));

        // NOTE: If adding more supported contract types, extend this logic.
        baseImageURIs.defaultImageURI = gameGlobals.getString(
            PIRATE_NFT_IMAGE_URI_DEFAULT_ID
        );
        baseImageURIs.enhancedImageURI = gameGlobals.getString(
            PIRATE_NFT_IMAGE_URI_ENHANCED_ID
        );
        baseImageURIs.defaultAnimatedImageURI = gameGlobals.getString(
            PIRATE_NFT_ANIMATED_IMAGE_URI_DEFAULT_ID
        );
        baseImageURIs.enhancedAnimatedImageURI = gameGlobals.getString(
            PIRATE_NFT_ANIMATED_IMAGE_URI_ENHANCED_ID
        );
    }

    function _updateTokenURIs(
        address tokenContract,
        uint256 tokenId,
        TokenType tokenType,
        BaseImageURIs memory baseImageURIs,
        ITraitsProvider traitsProvider
    ) internal {
        // handle image trait setting
        string memory imageURI;
        if (tokenType == TokenType.ENHANCED) {
            imageURI = string.concat(
                baseImageURIs.enhancedImageURI,
                "/",
                tokenId.toString()
            );
        } else {
            imageURI = string.concat(
                baseImageURIs.defaultImageURI,
                "/",
                tokenId.toString()
            );
        }

        // handle animation url setting
        string memory animatedImageURI;
        if (tokenType == TokenType.ENHANCED) {
            animatedImageURI = string.concat(
                baseImageURIs.enhancedAnimatedImageURI,
                "/",
                tokenId.toString()
            );
        } else {
            animatedImageURI = string.concat(
                baseImageURIs.defaultAnimatedImageURI,
                "/",
                tokenId.toString()
            );
        }
...
import {BoolComponent} from "../../core/components/BoolComponent.sol";
import {TypesLibrary} from "../../core/TypesLibrary.sol";

uint256 constant ID = uint256(
    keccak256("game.piratenation.tokentypeunlockedcomponent")
);

/**
 * @dev maps account → boolean: true indicates account may change its token types.
 */
contract TokenTypeUnlockedComponent is BoolComponent {
    /** SETUP **/
