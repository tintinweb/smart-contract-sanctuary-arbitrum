...
contract DistributionContract {
    ...
    struct Holder {
        ...
        mapping(uint256 => uint256) accumulatedArbot; // Mapping from epoch to accumulated ARBOT
        mapping(uint256 => uint256) accumulatedEth; // Mapping from epoch to accumulated ETH
    ...
    function distributeRewardsToHolders(uint256 ethAmount) internal {
        ...
        for (uint256 i = 0; i < token.holdersCount(); i++) {
            ...
            if (isEligible(holder)) {
                ...
                holders[holder].accumulatedArbot[currentEpoch] += holderShareArbot;
                holders[holder].accumulatedEth[currentEpoch] += holderShareEth;
            }
        }
    }
    ...
    function claimArbot() external {
        ...
        uint256 totalArbot = 0;
        for (uint256 i = holders[msg.sender].lastClaimEpoch; i <= epoch.getCurrentEpoch(); i++) {
            totalArbot += holders[msg.sender].accumulatedArbot[i];
            holders[msg.sender].accumulatedArbot[i] = 0;
        }
        require(totalArbot > 0, "No available ARBOT to claim");
        holders[msg.sender].lastClaimEpoch = epoch.getCurrentEpoch();
        holders[msg.sender].lastClaimTime = block.timestamp;
        token.transfer(msg.sender, totalArbot);
    }
    ...
    function claimEth() external {
        ...
        uint256 totalEth = 0;
        for (uint256 i = holders[msg.sender].lastClaimEpoch; i <= epoch.getCurrentEpoch(); i++) {
            totalEth += holders[msg.sender].accumulatedEth[i];
            holders[msg.sender].accumulatedEth[i] = 0;
        }
        require(totalEth > 0, "No available ETH to claim");
        holders[msg.sender].lastClaimEpoch = epoch.getCurrentEpoch();
        holders[msg.sender].lastClaimTime = block.timestamp;
    ...
    function getAvailableArbotToClaim(address holder) public view returns (uint256) {
        uint256 totalArbot = 0;
        for (uint256 i = holders[holder].lastClaimEpoch; i <= epoch.getCurrentEpoch(); i++) {
            totalArbot += holders[holder].accumulatedArbot[i];
        }
        return totalArbot;
    }
    ...
    function getAvailableEthToClaim(address holder) public view returns (uint256) {
        uint256 totalEth = 0;
        for (uint256 i = holders[holder].lastClaimEpoch; i <= epoch.getCurrentEpoch(); i++) {
            totalEth += holders[holder].accumulatedEth[i];
        }
        return totalEth;
    }
    ...
    function updateEpoch() external {
        ...
        if (newEpoch > epoch.currentEpoch()) {
            ...
            // Reset total distributions for the new epoch
            totalDistributionArbot = 0;
            totalDistributionEth = 0;
        }
    }
}
...
contract EpochContract {
    uint256 public constant EPOCH_LENGTH = 1 hours; // Durée d'une epoch : 2 heures
