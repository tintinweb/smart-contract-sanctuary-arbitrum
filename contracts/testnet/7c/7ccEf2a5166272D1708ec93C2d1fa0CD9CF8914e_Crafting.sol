...
contract Crafting is Initializable, CraftingSettings {
    ...
    returns(uint256[] memory, uint256[] memory) {
        ...
        uint32 _corruptionBreakRateBoost = _calculateCorruptionBreakRateBoost();
    ...
    function _calculateCorruptionBreakRateBoost() private view returns(uint32) {
        uint256 _corruptionBalance = corruption.balanceOf(address(this));
        if(_corruptionBalance <= 200_000 ether) {
            return 0;
