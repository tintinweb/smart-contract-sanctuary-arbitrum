...
contract DexWeeklyUpdater is ArcBaseWithRainbowRoad
{
    ...
    function setAxelarSender(uint256 chainId, address sender) external onlyAdmins
    {
    ...
    function setChainlinkSender(uint256 chainId, address sender) external onlyAdmins
    {
        require(sender != address(0), 'Sender cannot be zero address');
        if(!this.chainIdExists(chainId)) {
            revert ChainIdDoesNotExist({chainId: chainId});
    ...
    function setLayerZeroSender(uint256 chainId, address sender) external onlyAdmins
    {
        require(sender != address(0), 'Sender cannot be zero address');
        if(!this.chainIdExists(chainId)) {
            revert ChainIdDoesNotExist({chainId: chainId});
