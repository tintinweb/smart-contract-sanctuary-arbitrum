...
contract EngramV2 is GapStorageV2, ERC721NonTransferrableUpgradeable, UUPSUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
    ...
    function mint(bytes memory _signature, bytes memory _encryptedEncryptedPrivateIdentifier, string memory _username) 
        ...
        whenNotPaused 
        hasValidSignature(_signature, abi.encodePacked(_msgSender()))
    {
        if(addressHasMinted[_msgSender()]) revert TokenAlreadyMinted();

        addressHasMinted[_msgSender()] = true;
        addressToTokenId[_msgSender()] = tokenCount;
        metadataContract.setTokenIdToEncryptedPrivateIdentifier(_msgSender(), tokenCount, _encryptedEncryptedPrivateIdentifier);
        if(bytes(_username).length > 0) {
            metadataContract.setUsername(_msgSender(), tokenCount, _username);
        }
    ...
    function claimUsername(bytes memory _signature,  uint256 _tokenId, string memory _username)
        ...
        onlyTrustedForwarder()
        whenNotPaused
        hasValidSignature(_signature, abi.encodePacked(_msgSender(), _tokenId, _username))
    {
        if(_msgSender() != ownerOf(_tokenId)) revert SenderDoesNotOwnToken();
        metadataContract.setUsername(_msgSender(), _tokenId, _username);
    }

    function testSignature(bytes memory _signature)
        public 
        onlyTrustedForwarder()
        hasValidSignature(_signature, abi.encodePacked(_msgSender()))
    {
   ...
   function _msgSender() internal view override(ContextUpgradeable, ERC2771ContextUpgradeable) returns (address sender) {
    ...
    function migrate(IERC721 _oldContract, uint256 totalSupply)
        external
        onlyAdmin
    { 
