...
contract PayWhatYouCanFacet is AppFacet {
    ...
    function updatePWYCStatus(
        bool payStatus_
    ) external onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        PayWhatYouCanStorage.layout()._pwycStatus = payStatus_;
    }

    function pwycStartEditionSale(
        string memory editionId_
    ) external onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        PayWhatYouCanStorage.Layout storage layout = PayWhatYouCanStorage
            .layout();
        if (layout._activeTokens[editionId_] == false) {
            layout._activeTokens[editionId_] = true;
            layout._pwycActiveTokensCount += 1;
        }

        if (layout._pwycSaleActive == false) {
            layout._pwycSaleActive = true;
        }
    }

    function pwycStopEditionSale(
        string memory editionId_
    ) external onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        PayWhatYouCanStorage.Layout storage layout = PayWhatYouCanStorage
            .layout();
        if (layout._activeTokens[editionId_] == true) {
            layout._pwycActiveTokensCount -= 1;
            layout._activeTokens[editionId_] = false;
        }
        if (layout._pwycActiveTokensCount == 0) {
            layout._pwycSaleActive = false;
        }
    }

    function pwycIsEditionActive(
        string memory editionId_
    ) external view returns (bool) {
        return PayWhatYouCanStorage.layout()._activeTokens[editionId_] == true;
    }

    function pwycActiveTokensCount() external view returns (uint256) {
        return PayWhatYouCanStorage.layout()._pwycActiveTokensCount;
    }
}
