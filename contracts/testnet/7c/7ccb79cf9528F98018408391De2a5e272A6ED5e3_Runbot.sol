...
contract Runbot is A_Base, A_Math, ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
	...
	function askMint(uint256 nb, uint256 nftType, uint256 nftSubType, uint256 borrowDuration, uint256 bidPrice) external payable returns (uint256){
		...
		require(nb <= _groupMintNbLimit, "Runbot: Cannot mint so many tokens at once");
		
		uint256 fees = getBaseFees(nftType, nftSubType, borrowDuration, true, nb);
	...
	function setBidPrice(uint256 tokenId, uint256 bidPrice) public {
	...
	function batchSetBidPrice(uint256[] memory tokenIds, uint256[] memory bidPrices) external {
		...
		for (uint256 i = 0; i < tokenIds.length; i++){
			setBidPrice(tokenIds[i], bidPrices[i]);
		}
	}
	...
	function getFees(uint256 tokenId) public view returns (uint256){
		...
		if (_isFirstBuy[tokenId] && _withFirstBuyRebate[tokenId]){
			rebateMintFees = getBaseFees(nftType, nftSubType, _borrowDuration[tokenId], true, 1);
		}
		
		uint256 baseFees = getBaseFees(nftType, nftSubType, _getCurrentBorrowDuration(tokenId), false, 1);
