...
uint256 constant ID = uint256(keccak256("game.piratenation.tokenidcomponent"));
 ...
 */
contract TokenIdComponent is BaseStorageComponent {
    /** SETUP **/
    ...
    function setValue(
        ...
        emitSetBytes(
            entity,
            abi.encode(value.tokenAddress, value.tokenId)
        );
    }
    ...
    function batchSetValue(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            encodedValues[i] = abi.encode(
                values[i].tokenAddress, values[i].tokenId
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = TokenIdComponentStorage
           .layout()
    ...
    function getBytes(
        ...
        Layout memory s = TokenIdComponentStorage.layout().entityIdToStruct[entity];
        value = abi.encode(s.tokenAddress, s.tokenId);
    }
    ...
    function _setValueToStorage(
        ...
        s.tokenAddress = transform.tokenAddress;
        s.tokenId = transform.tokenId;
    }
}