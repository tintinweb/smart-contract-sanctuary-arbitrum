...
contract LinearVesting {
    ...
    function claim() external {
        ...
        require(amount > 0, "LinearVesting: no tokens to claim");
        token.safeTransfer(msg.sender, amount);
        claimed[msg.sender] += amount;
    }

    function available(address address_) public view returns (uint) {
        return _available(address_);
    }

    function released(address address_) public view returns (uint) {
        return _released(address_);
    }

    function outstanding(address address_) public view returns (uint) {
    ...
    function _released(address address_) internal view returns (uint) {
        if (block.timestamp < startTime) {
            ...
            return (allocation[address_] * elapsedTime) / duration;
        }
    }
}
