 ...
 */
contract SmolOwnersRegistry is Ownable, ICreatureOwnerResolver {
    using EnumerableSet for EnumerableSet.AddressSet;

    ICreatureOwnerResolver public root;
    EnumerableSet.AddressSet private _resolvers;

    constructor(ICreatureOwnerResolver root_) {
        setRoot(root_);
    }
    ...
    function _isOwner(address account, uint256 tokenId) internal view returns (bool) {
        for (uint256 i = 0; i < EnumerableSet.length(_resolvers); i++) {
