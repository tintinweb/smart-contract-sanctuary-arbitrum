...
import "../ERC20/MintableBurnableERC20.sol";
import "@layerzerolabs/solidity-examples/contracts/token/oft/v2/BaseOFTV2.sol";


contract WrapperOFTV2 is BaseOFTV2 {
    using SafeERC20 for MintableBurnableERC20;
    MintableBurnableERC20 public immutable tokenMintableBurnable;
    ...
    constructor(
        ...
        tokenMintableBurnable = MintableBurnableERC20(_token);

        (bool success, bytes memory data) = _token.staticcall(
            abi.encodeWithSignature("decimals()")
        );
        require(success, "WrapperOFTWithFee: Failed to get token decimals");
        uint8 decimals = abi.decode(data, (uint8));

        require(
            _sharedDecimals <= decimals,
            "WrapperOFTWithFee: SharedDecimals must be <= decimals"
        );
     ...
     ************************************************************************/
    function circulatingSupply() public view virtual override returns (uint) {
        return tokenMintableBurnable.totalSupply();
    }

    function token() public view virtual override returns (address) {
        return address(tokenMintableBurnable);
    }
    ...
    function _debitFrom(
        ...
        require(_from == _msgSender(), "WrapperOFTWithFee: Owner is not send caller");

        tokenMintableBurnable.burnFrom(_from, _amount);
    ...
    function _creditTo(
        ...
        tokenMintableBurnable.mint(_toAddress, _amount);
    ...
    function _transferFrom(
        ...
        uint initialBalance = tokenMintableBurnable.balanceOf(_to);
        if (_from == address(this)) {
            tokenMintableBurnable.safeTransfer(_to, _amount);
        } else {
            tokenMintableBurnable.safeTransferFrom(_from, _to, _amount);
        }
        return tokenMintableBurnable.balanceOf(_to) - initialBalance;
    }
