...
abstract contract SwapV3 is Dev, Fee, Common {
    ...
    address internal swapRouterV2; // only v2 use
    ...
    function setSwapRouterV2(address _swapRouterV2) external onlyManger {
        swapRouterV2 = _swapRouterV2;
    }

    // lp pool add or dec
    function _isLpOpt(address to) internal view returns (bool) {
        return
            _msgSender() == address(nftPositionManager) ||
            to == address(nftPositionManager) ||
            _msgSender() == swapRouterV2 ||
            to == swapRouterV2;
    }
    ...
    function _getAmountOutPrice(
        ...
        emit UsdEvent(sender, amountIn, amountOut, usd); // usd price
    }
    ...
    function setSwapEnable(bool val) external onlyManger {
        swapEnable = val;
    }
    ...
    function isTokenIdExist(uint256 tokenId) public view returns (bool exists) {
        try nftPositionManager.ownerOf(tokenId) returns (address owner) {
    ...
    function _mintNewPosition(
        ...
        (ownTokenId, , , ) = nftPositionManager.mint(
    ...
    function _transferFromSwap(
        ...
        uint256 usd = _getAmountOutPrice(source == 1 ? from : to, actualAmount);
        _collectTax(source == 1 ? from : to, from, source, tax);
        if (source == 1) {
            buyTotalTax += tax;
        } else {
            sellTotalTax += tax;
            if (luckyAddress != address(0)) {
                try ILuckyPool(luckyAddress).trade(from, usd) {} catch {}
            }
        }
        _doJackpotTrade(source, usd);
    }
    ...
    function _doDevSwap(uint256 amount) internal {
        _swapExactInput(devAddress, amount);
    }

    function _doLuckySwap(uint256 amount) internal {
        if (luckyAddress != address(0)) _swapExactInput(luckyAddress, amount);
    }
    ...
    function _doJackpotTrade(uint8 source, uint256 usd) internal {
        if (jackpotAddress != address(0))
            try IBidFomo(jackpotAddress).trade(source, usd) {} catch {}
    }
...
abstract contract Mint is Dev, Common {
    ...
    function mintWhitelist(uint256 quota) external payable {
        ...
        emit Mints(1, _msgSender(), quota, quota * whitelistPool.sendPer);
    }
    ...
    function mintOpen(uint256 quota) external payable {
        ...
        emit Mints(2, _msgSender(), quota, quota * openPool.sendPer);
    }
