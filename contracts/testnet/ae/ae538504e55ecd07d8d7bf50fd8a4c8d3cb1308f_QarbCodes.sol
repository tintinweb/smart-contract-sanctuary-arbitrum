...
contract QarbCodes is QarbCodesInterface, QrCode, ERC721URIStorage, Ownable {
    ...
    uint256 public constant PHASE_1_WHITELIST_LIMIT = 100;
    uint256 public constant PHASE_2_LIMIT = 2000;
    ...
    mapping(uint256 => string) private _tokenQrCodeText;
    ...
    bytes private constant _TOKEN_DESCRIPTION =
        "QaRb Codes on Arbitrum - Stored entirely on Arbitrum Blockchain";
    ...
    /**
     * @dev The White List is active until everything is sold out, so even when
     *      the Public Sale is activated users can still claim their whitelist
     *      spot as long as the collection is not Sold Out yet.
     */
    function mintWhiteList(
        string memory qrCodeText
    ) external override returns (uint256) {
        require(1 <= _whiteList[msg.sender], "ALLOW_LIST_MAX_EXCEEDED");
        require(totalSupply() + 1 <= MAX_SUPPLY, "MAX_SUPPLY_EXCEEDED");
        ...
        _tokenQrCodeText[newTokenId] = qrCodeText;
    ...
    function mint(
        ...
        require(hasLaunched && isPublicSaleActive, "PUBLIC_MINT_INACTIVE");
        require(totalSupply() + 1 <= MAX_SUPPLY, "MAX_SUPPLY_EXCEEDED");
        require(msg.value >= cost, "INCORRECT_ETH_AMOUNT");
     ...
     *      enabled for a 2nd (or more) times.
     */
    ...
    function withdrawBalance(address payable to) external override onlyOwner {
    ...
    function tokenQrCodeText(
        ...
        return _tokenQrCodeText[tokenId];
    }
    ...
    function tokenURI(
        ...
        bytes memory svg = bytes(_generateQRCode(_tokenQrCodeText[tokenId]));
    ...
    function _textAttributesAsJSON(
        ...
        string memory attributeAsString = _tokenQrCodeText[tokenId];
        string memory attributeTypeAsString = "Value";
...
contract QrCode {
    function _generateQRCode(
        ...
        return _generateQRURIByteVersion(qrMatrix);
    }
    ...
    function _generateQRURIByteVersion(
        ...
        DynamicBuffer.appendSafe(
            svgBytes,
            '<svg viewBox="0 0 300 300" width="500" height="500" xmlns="http://www.w3.org/2000/svg"><style>.bg{fill:#FCF}.fg{fill:#000}</style><rect class="bg" x="0" y="0" width="500" height="500"></rect>' // solhint-disable-line
        );
...
interface QarbCodesInterface {
    ...
    function mintWhiteList(string memory qrCodeText) external returns (uint256);

    function mintFree(string memory qrCodeText) external returns (uint256);

    function mint(string memory qrCodeText) external payable returns (uint256);
