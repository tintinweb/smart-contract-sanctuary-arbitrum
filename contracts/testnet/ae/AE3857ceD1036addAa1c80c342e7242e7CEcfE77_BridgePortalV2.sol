...
contract BridgePortalV2 is ReentrancyGuard, Governable {
    ...
    uint256 public moveLiquidityTxId;
    mapping (uint256 => bool) public moveLiquidityTxIdToStatus;
    ...
    event moveLiquidityEvent(address indexed receiver, uint256 amount, uint256 amountOutMin, uint256 feeWei, uint256 moveLiquidityTxId);
    event MLReceivedOnDestination(uint16 chainId, address token, uint amountLD, uint256 balance, uint256 mlTxId);
    ...
    modifier onlyLiquidityKeeper() {
        require(isLiquidityKeeper[msg.sender], "BridgePortalV2: forbidden2");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "BridgePortalV2: already initialized");
        isInitialized = true;

        require(_targetPool != address(0), "invalid _targetPool");
    ...
    function setLiquidityKeeper(address _account, bool _isActive) external onlyGov {
    ...
    receive() external payable {
        ...
        //If receive ETH from stargate, no need to print event
        if (msg.sender == stargateEthVault) {
            return;
        } else {
    ...
    //move large liquidity between chains using stargate
    function moveLiquidity(uint256 _outAmount) external onlyLiquidityKeeper nonReentrant whenNotPaused {
        require(_outAmount > 0, "invalid _outAmount");

        bytes memory toAddress = abi.encodePacked(targetPool);
        bytes memory payload = abi.encodePacked(moveLiquidityTxId);

        (uint256 feeWei, uint256 otherRes) = IStargateRouter(stargateRouter).quoteLayerZeroFee(
                    dstChainId,                 // destination chainId
                    TYPE_SWAP_REMOTE,           // function type: see Bridge.sol for all types
                    toAddress,                  // destination of tokens
                    payload,                    // payload, using abi.encode()
                    IStargateRouter.lzTxObj(0, 0, "0x")
                    );
        uint256 totalNeedAmount = _outAmount + feeWei;
        require(address(this).balance >= totalNeedAmount, "balance not enough");

        uint256 amountOutMin = _outAmount.mul(9100).div(10000);
        IRouterETH(stargateRouterETH).swapETHAndCall{value:totalNeedAmount}(
            ...
            IRouterETH.SwapAmount(_outAmount, amountOutMin),    // the amount and the minimum swap amount
        ...
        emit moveLiquidityEvent(targetPool, _outAmount, amountOutMin, feeWei, moveLiquidityTxId);
        moveLiquidityTxId = moveLiquidityTxId + 1;
    }
    ...
    function sgReceive(
        ...
        (uint256 _mlTxId) = abi.decode(_payload, (uint256));
        moveLiquidityTxIdToStatus[_mlTxId] = true;

        uint256 balance = IStargateEthVault(stargateEthVault).balanceOf(address(this));
        IStargateEthVault(stargateEthVault).withdraw(balance);

        emit MLReceivedOnDestination(_chainId, _token, _amountLD, balance, _mlTxId);
    }
}
