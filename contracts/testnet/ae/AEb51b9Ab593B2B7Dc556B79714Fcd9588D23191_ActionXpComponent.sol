...
uint256 constant ID = uint256(keccak256("game.piratenation.actionxpcomponent"));

struct Layout {
    uint256 xpAmountToReward;
 ...
 */
contract ActionXpComponent is BaseStorageComponent {
    /** SETUP **/
    ...
    function batchSetValue(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            encodedValues[i] = abi.encode(
                values[i].xpAmountToReward, values[i].successAmount, values[i].failAmount, values[i].maxAmountAllowed
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ActionXpComponentStorage
           .layout()
    ...
    function getBytes(
        ...
        Layout memory s = ActionXpComponentStorage.layout().entityIdToStruct[entity];
        value = abi.encode(s.xpAmountToReward, s.successAmount, s.failAmount, s.maxAmountAllowed);
    }
    ...
    function setBytes(
        ...
        Layout memory s = ActionXpComponentStorage.layout().entityIdToStruct[entity];
        (s.xpAmountToReward, s.successAmount, s.failAmount, s.maxAmountAllowed) = abi.decode(value, (uint256, uint256, uint256, uint256));
        _setValue(entity, s);
    }
    ...
    function _setValueToStorage(
        ...
        s.xpAmountToReward = transform.xpAmountToReward;
        s.successAmount = transform.successAmount;
        s.failAmount = transform.failAmount;
        s.maxAmountAllowed = transform.maxAmountAllowed;
    }
    ...
    function _setValue(
        ...
        emitSetBytes(
            entity,
            abi.encode(value.xpAmountToReward, value.successAmount, value.failAmount, value.maxAmountAllowed)
        );
    }
}