...
// File: contracts/interfaces/IAntsArmyBonusPool.sol


pragma solidity >=0.5.0;

interface IAntsArmyBonusPool {
...
// File: contracts/token/Ants.sol


pragma solidity =0.8.19;

contract Ants is ERC20, Ownable {
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        return _antTransfer(_msgSender(), to, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        address spender = _msgSender();
        _spendAllowance(sender, spender, amount);
        return _antTransfer(sender, recipient, amount);
    }

    function _antTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (inSwap) {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountAntBurn = (taxAmount * burnFee) / (totalFee);
        uint256 amountAntLp = (taxAmount * liquidityFee) / (totalFee);
        uint256 amountAntBonus = (taxAmount * bonusFee) / (totalFee);
        taxAmount -= amountAntBurn;
        taxAmount -= amountAntLp;
        taxAmount -= amountAntBonus;
        ...
        _transfer(address(this), DEAD, amountAntBurn);
        _approve(address(this), address(bonusWallet), amountAntBonus);
        IAntsArmyBonusPool(bonusWallet).injectRewards(amountAntBonus);
        ...
        emit SwapBack(amountAntBurn, amountBackTokenGov1, amountBackTokenGov2, amountAntLp, amountBackTokenJackpot, amountAntBonus, amountBackTokenDev, block.timestamp);
    }
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'ANTS: ETH_TRANSFER_FAILED');
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "ANTS: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "ANTS: index out of bounds");
