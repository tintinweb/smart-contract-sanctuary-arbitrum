...
contract Transmolgrifier is AccessControlEnumerableV2, SmolsAddressRegistryConsumer {
	...
	function transmolgrify(uint256 _smolRecipeId, uint256 _smolIdToTransmogrify, uint256[] calldata _smolIdsToBurn) external {
		...
		for(uint256 i = 0; i < smolData.smolInputAmount; i++){
			require(ISmolsState(smolsStateAddress).getInitialSmol(_smolIdsToBurn[i]).gender == 2, "Not a female burn smol!");
		...
		require(ISmolsState(smolsStateAddress).getInitialSmol(_smolIdToTransmogrify).gender == 2, "Not a female transmol!");
