...
contract ScratchEmTreasury is Ownable, IScratchEmTreasury, ReentrancyGuard {
    ...
    function nonceUnlock(
        ...
        unclaimedRewards[token] -= amount;
        if (burnCut > 0) {
            uint burnAmount = _burnToken(token, burnCut);
            amount -= burnAmount;
        }
        if (swapType == 1) {
        ...
        if (afterTransferCut > 0) {
            uint afterTransferAmount = (amount * afterTransferCut) / 10000;
    ...
    function _burnToken(
        ...
        uint256 amount = (IERC20(token).balanceOf(address(this)) * burnCut) /
            10000;
    ...
    function claimRewards() external nonReentrant {
        ...
        for (uint256 i = 0; i < _games.length; i++) {
            ...
            delete hasRewardsInGame[msg.sender][_games[i]];
        }
        delete gamesWithRewards[msg.sender];
        for (uint256 i = 0; i < playableTokens.length; i++) {
