...
contract BGT_UFM {
    ...
    function isOrderEnded(uint256[] memory __order_ids) external view returns (bool[] memory) {
        bool[] memory list = new bool[](__order_ids.length);
        for (uint i=0; i<__order_ids.length; i++)
        {
            Order storage _order = _orders[__order_ids[i]];
            uint256 cycle = _fps[_order.fpId].day * (1 days);
            list[i] = bool(_order.time + cycle <= block.timestamp);
        }
        return list;
    }
