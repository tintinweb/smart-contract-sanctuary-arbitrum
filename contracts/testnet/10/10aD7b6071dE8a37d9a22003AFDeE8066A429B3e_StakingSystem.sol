...
interface ITombsToken is IERC20 {
    function mint(address to, uint256 amount) external;
}

contract StakingSystem is Ownable, ERC721Holder {
    ITombsToken public rewardsToken;
    IERC721 public nft;

    uint256 public stakedTotal;
    uint256 public stakingStartTime;
    uint256 public stakingTime = 60 seconds;
    uint256 public token = 1e18;
    ...
    constructor(address _nft, address _rewardsToken) {
        nft = IERC721(_nft);
        rewardsToken = ITombsToken(_rewardsToken);
    }
    ...
    function _stake(address _user, uint256 _tokenId) internal {
        ...
        nft.transferFrom(_user, address(this), _tokenId);
    ...
    function _unstake(address _user, uint256 _tokenId) internal {
        ...
        uint256[] memory tokenIds = new uint256[](staker.tokenIds.length - 1);
        uint256 count = 0;
        for(uint256 i = 0; i < staker.tokenIds.length; i++) {
            if(staker.tokenIds[i] != _tokenId) {
                tokenIds[count++] = staker.tokenIds[i];
            }
        }

        staker.tokenIds = tokenIds;
    ...
    function updateReward(address _user) public {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            if (
                staker.tokenStakingCoolDown[ids[i]] + stakingTime < block.timestamp &&
                ...
                uint256 amount = 0;
                if (ids[i] <= 4250) amount = 500;
                else if (ids[i] <= 4850) amount = 600;
                else amount = 800;                

                staker.balance += amount * token * stakedDays;
