...
contract AllocationFactory is Ownable {
    ...
    constructor(
        ...
        Vesting.AddressType[] memory _addressTypeList,
        address _usdcToken,
        address _chainlinkPriceFeed,
        address _chainlinkUptimeFeed
    ) {
        require(
            _startCliff > block.timestamp,
            "AllocationFactory/constructor: startCliff must be greater than block.timestamp"
        );
        require(
        ...
        uint256 totalSupplyCap = 10_000_000_000e18;
        uint256 vestingAmount = (totalSupplyCap * 15) / 100;
        uint256 crowdSaleAmount = (totalSupplyCap * 30) / 100;

        mghVesting = new Vesting(
            _startCliff,
            address(token),
            _addressList,
            _amountList,
        ...
        mghCrowdSale = new CrowdSale(
            10,
            address(token),
            _usdcToken,
            _chainlinkPriceFeed,
            _chainlinkUptimeFeed
        );
        ...
        emit DeployedCrowdSale(address(mghCrowdSale));
    }
...
contract MGHToken is ERC20, Ownable {
    ...
    uint256 public constant totalSupplyCap = 10_000_000_000e18;
    ...
    function inflationRemainder() public view returns (uint256 remainder) {
        ...
        uint256 supplyCap = totalSupplyCap;
