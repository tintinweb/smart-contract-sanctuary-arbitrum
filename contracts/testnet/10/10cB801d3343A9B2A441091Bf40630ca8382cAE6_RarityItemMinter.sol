  ...
  Pausable
{
  ...
  function updateSlots(uint16[] memory _slots) onlyAdmin external {
    slots = _slots;
  }

  function updateCategories(uint16[][] memory _categories) onlyAdmin external {
    categories = _categories;
  }

  function updateTypes(uint16[][][] memory _types) onlyAdmin external {
    types = _types;
  }

  function updatePrefixes(uint16[][] memory _prefixes) onlyAdmin external {
    prefixes = _prefixes;
  }

  function updateSuffixes(uint16[][] memory _suffixes) onlyAdmin external {
    suffixes = _suffixes;
  }
