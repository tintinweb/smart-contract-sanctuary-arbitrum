...
contract Uint256Component is BaseComponent, IUint256Component {
    ...
    function setValueFromToken(
        uint256 entity,
        address tokenAddress,
        uint256 tokenId
    ) external {
        setValue(entity, EntityLibrary.tokenToEntity(tokenAddress, tokenId));
    }

    function getValueAsToken(
        uint256 entity
    ) external view returns (address tokenAddress, uint256 tokenId) {
        return EntityLibrary.entityToToken(this.getValue(entity));
    }
}
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.placeablesceneobjectcomponent")
);

/**
 * @dev maps a PlaceableSceneObject to it's underlying GameItem token.
 */
contract PlaceableSceneObjectComponent is Uint256Component {
    /** SETUP **/
