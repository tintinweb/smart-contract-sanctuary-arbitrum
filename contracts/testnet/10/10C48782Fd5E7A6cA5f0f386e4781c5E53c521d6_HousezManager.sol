...
contract HousezManager is Initializable, HousezManagerContracts {
    ...
    function setFloorInfo(uint16 _floorNum, uint16 _numHousezForFloor, uint128 _bugzCost) external onlyAdminOrOwner {
        require(_floorNum > 0, "Bad floor number");

        require(_numHousezForFloor == 0 || _bugzCost == 0, "Can't set both bugz and housez required");
        ...
        floorNumToInfo[_floorNum].bugzCost = _bugzCost;

        emit FloorInfoUpdated(_floorNum, _numHousezForFloor, totalStakableHousez, _bugzCost);
    }

    function purchaseFloors(
        uint16[] calldata _floorNums)
    external
    onlyEOA
    whenNotPaused
    contractsAreSet
    {
        require(_floorNums.length > 0);
        _claimAllBugz(msg.sender);

        for(uint256 i = 0; i < _floorNums.length; i++) {
            uint16 _floorNum = _floorNums[i];
            uint128 _bugzCost = floorNumToInfo[_floorNum].bugzCost;
            require(_bugzCost > 0, "Cannot purchase this floor with bugz");
            require(!userToInfo[msg.sender].floorNumToHasPurchased[_floorNum], "Already purchased");
            userToInfo[msg.sender].floorNumToHasPurchased[_floorNum] = true;

            bugz.burn(msg.sender, _bugzCost);

            emit FloorPurchased(msg.sender, _floorNum);
        }
    }
    ...
    function numberOfFloorsUnlocked(address _user) external view returns(uint16) {
        uint16 _numberOfFloorsUnlocked = 0;
        uint16 _totalHousezStaked = userToInfo[_user].housezStaked;
        for(uint16 i = 1; i < 1000; i++) {
            FloorInfo storage _floorInfo = floorNumToInfo[i];
            if(_floorInfo.bugzCost > 0) {
                if(userToInfo[_user].floorNumToHasPurchased[i]) {
                    _numberOfFloorsUnlocked++;
                }
            } else if(_floorInfo.numHousezForFloor > 0) {
                if(_totalHousezStaked >= _floorInfo.numHousezForFloor) {
                    _totalHousezStaked -= _floorInfo.numHousezForFloor;
                    _numberOfFloorsUnlocked++;
                }
            } else {
                break;
        ...
        return _numberOfFloorsUnlocked;
    }

    // Claims all bugz for the user. This in case the boost is about to change.
    //
    function _claimAllBugz(address _owner) private {
        uint256[] memory _huntingToads = world.tokenIdsHunting(_owner);
        if(_huntingToads.length > 0) {
            huntingGrounds.adminClaimBugz(_huntingToads, _owner);
        }
    }
}
...
abstract contract HousezManagerState is Initializable, IHousezManager, ERC721HolderUpgradeable, AdminableUpgradeable {
    ...
    event FloorInfoUpdated(uint16 _floorNum, uint16 _numHousezForFloor, uint16 _totalHousezStakable, uint128 _bugzCost);
    ...
    event FloorPurchased(address _owner, uint16 _floorNum);
...
struct FloorInfo {
    ...
    // If 0, this floor cannot be built via staking.
    uint16 numHousezForFloor;
    // The bugz cost for purchasing this floor.
    // If 0, this floor cannot be achieved via purchasing.
    uint128 bugzCost;
    uint112 emptySpace;
}
...
struct UserInfo {
    ...
    // Slot 2
    // For floors purchasable via bugs, this indicates if the user has purchased the floor already.
    mapping(uint16 => bool) floorNumToHasPurchased;
}
