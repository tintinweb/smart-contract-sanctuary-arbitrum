...
import "./interfaces/ISmoloveRingVault.sol";
...
/**
 * @title  SmoloveRingVault contract
 * @author Archethect
 * @notice This contract contains all functionalities for staking Magic through the Battlefly Flywheel program for Smolove ring holders.
 */
contract SmoloveRingVault is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, ISmoloveRingVault {
    ...
    mapping(uint256 => uint256) public amountStakedPerRing;
    uint256 nextStakeId;
    ...
    function initialize(
        ...
        require(_admin != address(0), "SMOLOVE_RING_VAULT:INVALID_ADMIN_ADDRESS");
        require(_operator != address(0), "SMOLOVE_RING_VAULT:INVALID_OPERATOR_ADDRESS");
        require(_treasury != address(0), "SMOLOVE_RING_VAULT:INVALID_TREASURY_ADDRESS");
        require(_smolRings != address(0), "SMOLOVE_RING_VAULT:INVALID_RINGS_ADDRESS");
        require(_smolMarriage != address(0), "SMOLOVE_RING_VAULT:INVALID_MARRIAGE_ADDRESS");
        require(_magicAddress != address(0), "SMOLOVE_RING_VAULT:INVALID_MAGIC_ADDRESS");
        require(_BattleflyStakerAddress != address(0), "SMOLOVE_RING_VAULT:INVALID_BF_STAKER_ADDRESS");
        treasury = _treasury;
        smolRings = ISmolRings(_smolRings);
        smolMarriage = ISmolMarriage(_smolMarriage);
    ...
    function stake(uint256 amount, IAtlasMine.Lock lock) external {
        require(smolRings.balanceOf(msg.sender) > 0, "SMOLOVE_RING_VAULT:NOT_OWNER_OF_RING");
        magic.safeTransferFrom(msg.sender, address(this), amount);
        _stake(uint256(amount), lock);
    }
    ...
    function stake(
        ...
        require(
            isOwner(msg.sender, creature) && smolMarriage.isMarried(creature),
            "SMOLOVE_RING_VAULT:NOT_OWNER_OF_MARRIED_CREATURE"
        );
        magic.safeTransferFrom(msg.sender, address(this), amount);
        _stake(uint256(amount), lock);
    }
    ...
    function _stake(uint256 amount, IAtlasMine.Lock lock) internal returns (uint256) {
        require(amount > 0, "SMOLOVE_RING_VAULT:AMOUNT_SHOULD_BE_BIGGER_THAN_0");
        bool validLock = false;
        for (uint256 i = 0; i < allowedLocks.length; i++) {
            if (allowedLocks[i] == lock) validLock = true;
        }
        require(validLock, "SMOLOVE_RING_VAULT:LOCK_TIME_NOT_ALLOWED");
        uint256 battleflyStakerDepositId = BattleflyStaker.deposit(amount, lock);
        IBattleflyAtlasStaker.VaultStake memory vaultStake = BattleflyStaker.getVaultStake(battleflyStakerDepositId);
    ...
    function claimAllToTreasury() external {
        for (uint256 i = 0; i < nextStakeId; i++) {
            if (_getStakeClaimableAmount(i) > 0) {
                userStakes[i].nonTransferedClaims += _claim(i);
            }
        }
        uint256 amount = totalFee - totalFeeWithdrawn;
        require(amount > 0, "SMOLOVE_RING_VAULT:NO_FEE_TO_WIHDRAW");
    ...
    function claimToTreasury(uint256[] memory stakeIds) external {
        for (uint256 i = 0; i < stakeIds.length; i++) {
            if (_getStakeClaimableAmount(stakeIds[i]) > 0) {
                userStakes[stakeIds[i]].nonTransferedClaims += _claim(stakeIds[i]);
            }
        }
        uint256 amount = totalFee - totalFeeWithdrawn;
        require(amount > 0, "SMOLOVE_RING_VAULT:NO_FEE_TO_WITHDRAW");
        totalFeeWithdrawn += amount;
        magic.safeTransfer(treasury, amount);
        emit WithdrawFee(treasury, amount);
    }
    ...
    function claimAll() public nonReentrant {
        ...
        for (uint256 i = 0; i < stakesOfOwner[msg.sender].length(); i++) {
            ...
            if (_getStakeClaimableAmount(stakeId) > 0) {
                totalReward += _claim(stakeId) + userStakes[stakeId].nonTransferedClaims;
                userStakes[stakeId].nonTransferedClaims = 0;
            }
        }
        require(totalReward > 0, "SMOLOVE_RING_VAULT:NO_REWARDS_TO_CLAIM");
    ...
    function claim(uint256[] memory stakeIds) external nonReentrant {
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            ...
            require(s.owner == msg.sender, "SMOLOVE_RING_VAULT:ONLY_OWNER_CAN_CLAIM");
            if (_getStakeClaimableAmount(stakeIds[i]) > 0) {
                totalReward += _claim(stakeIds[i]) + userStakes[stakeIds[i]].nonTransferedClaims;
                userStakes[stakeIds[i]].nonTransferedClaims = 0;
            }
        }
        require(totalReward > 0, "SMOLOVE_RING_VAULT:NO_REWARDS_TO_CLAIM");
        magic.safeTransfer(msg.sender, totalReward);
    ...
    function _getStakeClaimableAmount(uint256 stakeId) internal view returns (uint256) {
        UserStake memory s = userStakes[stakeId];
        (uint256 claimAmount, ) = BattleflyStaker.getClaimableEmission(s.battleflyStakerDepositId);
    ...
    function getUserClaimableAmount(address user) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < stakesOfOwner[user].length(); i++) {
            uint256 stakeId = stakesOfOwner[user].at(i);
            total += _getStakeClaimableAmount(stakeId) + userStakes[stakeId].nonTransferedClaims;
        }
        return total;
    }
    ...
    function withdrawAll() external nonReentrant {
        require(stakesOfOwner[msg.sender].length() > 0, "SMOLOVE_RING_VAULT:NO_STAKES_TO_WITHDRAW");
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            ...
            if (userStakes[stakeId].unlockAt < block.timestamp) {
                receiveAmount += _withdraw(stakeId) + userStakes[stakeId].nonTransferedClaims;
                userStakes[stakeId].nonTransferedClaims = 0;
            }
        }
        require(receiveAmount > 0, "SMOLOVE_RING_VAULT:NO_STAKES_TO_WITHDRAW");
    ...
    function withdraw(uint256[] memory stakeIds) external nonReentrant {
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            UserStake storage s = userStakes[stakeIds[i]];
            require(s.owner == msg.sender, "SMOLOVE_RING_VAULT:ONLY_OWNER_CAN_WITHDRAW");
            receiveAmount += _withdraw(stakeIds[i]) + userStakes[stakeIds[i]].nonTransferedClaims;
            userStakes[stakeIds[i]].nonTransferedClaims = 0;
        }
        require(receiveAmount > 0, "SMOLOVE_RING_VAULT:NO_STAKES_TO_WITHDRAW");
        magic.safeTransfer(msg.sender, receiveAmount);
    ...
    function _withdraw(uint256 stakeId) internal returns (uint256) {
        ...
        require(s.unlockAt < block.timestamp, "SMOLOVE_RING_VAULT:CANNOT_WITHDRAW_BEFORE_LOCK_TIME");
    ...
    function setAllowedLock(IAtlasMine.Lock[] memory _allowedLocks) external onlyAdmin {
        allowedLocks = _allowedLocks;
    }

    modifier onlyAdmin() {
        require(hasRole(ADMIN_ROLE, msg.sender), "SMOLOVE_RING_VAULT:ACCESS_DENIED");
        _;
    }

    modifier onlyOperator() {
        require(hasRole(OPERATOR_ROLE, msg.sender), "SMOLOVE_RING_VAULT:ACCESS_DENIED");
        _;
    }
}
...
interface ISmoloveRingVault {
    ...
    function claim(uint256[] memory stakeIds) external;
