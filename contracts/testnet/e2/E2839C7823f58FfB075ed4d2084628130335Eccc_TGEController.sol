...
contract TGEController is IController {
  ...
  function addVault(address _vault) external onlyDeployerOrGovernance {
    vaults.push(TGEVault(_vault));
  }

  function deployVault(address _underlying, uint256 _allocation) external onlyDeployerOrGovernance returns (address) {
