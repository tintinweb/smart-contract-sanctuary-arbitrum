 ...
 */
interface I_Runbot is IERC721, IERC721Enumerable {
 ...
 *  @custom:security-contact [email protected]
 */
contract Runbot is A_Base, ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, I_Runbot {
	...
	function askTransfer(address receiver, uint256 tokenId) external payable returns (uint256){
		...
		payable(owner()).transfer(transferFees);
	...
	function buyTo(address to, uint256 tokenId) public payable {
		...
		feesModel.dispatchRoyaltyFees{value:fees}(_nftTypes[tokenId], _nftSubTypes[tokenId]);
