...
contract HelixConfig is BaseUpgradeablePausable {
  ...
  mapping(uint256 => bool) public booleanes;

  event BooleanUpdated(
    address owner,
    uint256 index,
    bool oldValue,
    bool newValue
  ...
  function setBoolean(uint256 index, bool newBoolean) public onlyAdmin {
    emit BooleanUpdated(msg.sender, index, booleanes[index], newBoolean);
    booleanes[index] = newBoolean;
  }

  function setTreasuryReserve(address newTreasuryReserve) public onlyAdmin {
    uint256 key = uint256(HelixConfigOptions.Addresses.TreasuryReserve);
    emit AddressUpdated(msg.sender, key, addresses[key], newTreasuryReserve);
    addresses[key] = newTreasuryReserve;
  }

  function setBorrowerImplementation(address newAddress) public onlyAdmin {
    uint256 key = uint256(HelixConfigOptions.Addresses.BorrowerImplementation);
  ...
  function setCreditLineImplementation(address newAddress) public onlyAdmin {
    uint256 key = uint256(
      HelixConfigOptions.Addresses.CreditLineImplementation
    );
    emit AddressUpdated(msg.sender, key, addresses[key], newAddress);
    addresses[key] = newAddress;
  }
  ...
  function setTranchedPoolImplementation(address newAddress) public onlyAdmin {
    uint256 key = uint256(
      HelixConfigOptions.Addresses.TranchedPoolImplementation
    );
    emit AddressUpdated(msg.sender, key, addresses[key], newAddress);
    addresses[key] = newAddress;
  }

  function setTranchedPoolShareImplementation(
    address newAddress
  ) public onlyAdmin {
    uint256 key = uint256(
      HelixConfigOptions.Addresses.TranchedPoolShareImplementation
    );
    emit AddressUpdated(msg.sender, key, addresses[key], newAddress);
    addresses[key] = newAddress;
  }

  function setMultiTranchedPoolImplementationRepository(
    address newAddress
  ) public onlyAdmin {
    uint256 key = uint256(
      HelixConfigOptions.Addresses.TranchedPoolImplementationRepository
    );
    emit AddressUpdated(msg.sender, key, addresses[key], newAddress);
    addresses[key] = newAddress;
  }

  function setUniTranchedPoolImplementationRepository(
    address newAddress
  ) public onlyAdmin {
    uint256 key = uint256(
      HelixConfigOptions.Addresses.UniTranchedPoolImplementationRepository
    );
    emit AddressUpdated(msg.sender, key, addresses[key], newAddress);
    addresses[key] = newAddress;
  }

  function setPoolShareImplementationRepository(
    address newAddress
  ) public onlyAdmin {
    uint256 key = uint256(
      HelixConfigOptions.Addresses.PoolSharesImplementationRepository
    );
    emit AddressUpdated(msg.sender, key, addresses[key], newAddress);
    addresses[key] = newAddress;
  }

  function setFundImplementationRepository(
    address newAddress
  ) public onlyAdmin {
    uint256 key = uint256(
      HelixConfigOptions.Addresses.FundImplementationRepository
    );
    emit AddressUpdated(msg.sender, key, addresses[key], newAddress);
    addresses[key] = newAddress;
  }
  ...
  function initializeFromOtherConfig(
    ...
    IHelixConfig initialConfig = IHelixConfig(_initialConfig);
  ...
  */
  ...
  function getBoolean(uint256 index) public view returns (bool) {
    return booleanes[index];
  }
