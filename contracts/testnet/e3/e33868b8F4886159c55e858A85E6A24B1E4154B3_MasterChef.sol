...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of ARBSs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accArbsPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accArbsPerShare` (and `lastRewardBlock`) gets updated.
    ...
    function add(
        ...
        poolInfo.push(
            PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardTime: lastRewardTime, accArbsPerShare: 0})
        );
    }
    ...
    function pendingARBS(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            accArbsPerShare += (arbsReward * 1e12) / lpSupply;
        }
        return (user.amount * accArbsPerShare) / 1e12 - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accArbsPerShare += (arbsReward * 1e12) / lpSupply;
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function safeArbsTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > arbsBal) {
            ARBS.transfer(_to, arbsBal);
        } else {
            ARBS.transfer(_to, _amount);
        }
    }
