...
contract RageDnDepository is IRageDnDepository {
    ...
    function approveAsset(uint256 amount) external {
    ...
    function withdraw(uint256 amount, address receiver) onlyController external returns (uint256) {
        uint256 shares = vault.withdraw(amount, receiver, address(this));
        netDeposits -= amount;
        emit Withdrawn(msg.sender, receiver, amount, shares);
        return shares;
    }

    function redeem(uint256 shares, address receiver) external returns (uint256) {
        uint256 amount = vault.redeem(shares, receiver, address(this));
        emit Redeemed(msg.sender, receiver, amount, shares);
        return amount;
    }
