...
contract CollectibleProducerWithProductivity is Pausable, ManagerModifier {
  ...
  function _produce(
    ...
    uint maxAmount = _adjustReactor(_calculateAmount(_capacity, _productivity, config), _realmId);
  ...
  function _adjustReactor(uint baseAmount, uint realmId) internal returns (uint) {
    if (STRUCTURE_STAKER.hasStaked(realmId, REACTOR, 0)) {
      return baseAmount * 2;
    }
    return baseAmount;
  }
