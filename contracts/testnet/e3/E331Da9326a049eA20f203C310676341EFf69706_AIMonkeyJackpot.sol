...
// File: sol/interfaces/IAIMonkeySwapTools.sol


pragma solidity =0.8.19;

interface IAIMonkeySwapTools {
   
    function getTokenUSDPrice(uint256 amount) external view returns (uint256 price);
    function getTokenUSDPrice12(uint256 amount) external view returns (uint256 price);
}
...
// File: sol/interfaces/SignatureChecker.sol
...
contract AIMonkeyJackpot is OwnableUpgradeable {
   using EnumerableSet for EnumerableSet.AddressSet;
    ...
    IAIMonkeySwapTools public swapTools;
    address public tradeToken;
    
    function initialize(IERC20 bonusToken_, address tradeToken_, address  swapTools_, uint256 minBonus_, uint256 maxBonus_) external initializer {
        __Ownable_init();
        bonusToken = bonusToken_;
        tradeToken = tradeToken_;
        swapTools = IAIMonkeySwapTools(swapTools_);
        minBonus = minBonus_;
        maxBonus = maxBonus_;
    ...
    function lottery() public onlyCaller {
        ...
        for (uint256 i = 0; i < ticks.length; i++) {
            ...
            if (randomValue < cumulativeProbability) {
                //计算奖金：当前池子大于1500个ARB，按1500个ARB发放，小于则按合约ARB余额发放
    ...
    function setSwapTools(address val) public onlyOwner {
        require(address(val) != address(0), "bad val");
        swapTools =IAIMonkeySwapTools(val);
    }
