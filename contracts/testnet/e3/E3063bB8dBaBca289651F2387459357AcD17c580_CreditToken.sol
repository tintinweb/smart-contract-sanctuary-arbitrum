...
contract CreditToken is Ownable, ERC20("Debt DAO Credit v2", "CREDIT-v2") {
    ...
    event MintCredit(uint256 indexed amount, address indexed user, address indexed minter);
    event BurnCredit(uint256 indexed amount, address indexed user, address indexed burner);

    constructor(address owner_, uint256 creditLimit) {
    ...
    function mint(address account, uint256 amount) external onlyMinters returns(bool) {
        ...
        emit MintCredit(amount, account, msg.sender);
    ...
    function burnFrom(address account, uint256 amount) external onlyMinters returns(bool) {
        ...
        emit BurnCredit(amount, account, msg.sender);
        return true;
    }

    function burn(uint256 amount) external returns(bool) {
        _burn(msg.sender, amount);
        emit BurnCredit(amount, msg.sender, msg.sender);
        return true;
    }
