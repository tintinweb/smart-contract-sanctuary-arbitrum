...
contract VertexHandler is OwnableUpgradeable {
    ...
    function getVertexBalance(bytes32 subaccount) external view returns (int128 balance) {        IFQuerier querier = IFQuerier(vertexQuerier);
    ...
    function findSpotProduct(uint32 productId, IFQuerier.SpotProduct[] memory spotProducts) internal pure returns(IFQuerier.SpotProduct memory) {
        for (uint i=0; i< spotProducts.length; i++) {
            if(spotProducts[i].productId == productId) {
                return spotProducts[i];
            }
        }
    }
