...
/**
 * @title WLPManager contract - manages the minting and burning of WLP
 * @author baldingghost
 * @notice this contract mints and redeems WLP. It is the only contract in the entire protocol that can mint WLP. The assets deposited into the WLP via this contract do not stay in this contract, they are always sent to the vault. The only asset this contract holds is USDW. 
 */
contract WLPManager is ReentrancyGuard, Context, AccessControlBase, IWLPManager {
   ...
   // if in private mode, both addLiquidity and removeLiquidity ar disabled
    ...
    // a handler is an address that is allowed to mint/redeem WLP on behalf of others
    mapping (address => bool) public isHandler;
    // when this config is enabled, whitelisted contracts are able to mint and redeem WLP on behalf of others. 
    bool public handlersEnabled = false;

    uint256 public override maxPercentageOfWagerFee = 2000; // 20%
    ...
    function setCollectFeesOnLiquidityEvent(bool _collectFeesOnLiquidityEvent) external onlyGovernance {
    ...
    /**
     * when private mode is enabled, minting and redemption of WLP is not possible (it is disabled) - only exception is that if handlersEnabled is true, whitelisted handlers are able to mint and redeem WLP on behalf of others.
     * @param _inPrivateMode xxx
     */
    ...
    /**
     * HandlerFlowExplanation: the handler flow is mainly useful for if the minting or redemption of WLP needs to be centrally coordinated. Note that the handerFlow doesn't allow a handler to forcefully remove liquidity of an WLP-lp - this because in order for an handler to successfully withdraw liquidity the WLP needs to give a token-approval. Same 
     */
    ...
    function setMaxPercentageOfWagerFee(uint256 _maxPercentageOfWagerFee) external onlyGovernance {
    ...
    function getPriceWLPInUsdw(bool _maximise) external view returns (uint256 tokenPrice_) {
        uint256 aum_ = getAumInUsdw(_maximise);
        uint256 supply_ = IERC20(wlp).totalSupply();
        if(supply_ == 0) { return 0; }
        tokenPrice_ = ((aum_ * WLP_PRECISION) / supply_);
    }
    ...
    function _addLiquidity(
        ...
        if(collectFeesOnLiquidityEvent) {
            ...
            // collect fees from vault, and distribute to WLP holders (to prevent frontrunning)
            _collectFees();
    ...
    function _removeLiquidity(
        ...
        // check if there is a cooldown period is 
        require(
