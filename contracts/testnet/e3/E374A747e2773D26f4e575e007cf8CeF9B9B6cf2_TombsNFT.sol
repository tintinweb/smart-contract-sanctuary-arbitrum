...
contract TombsNFT is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint64 public privateSalePeriod;
    mapping(address => bool) private freeMintBlackList;
    // Token Price
    // uint256 private constant PRICE_WHITELIST = 30 * 10**18; // 30 ARB Per Tombs
    // uint256 private constant PRICE_PUBLIC = 40 * 10**18; // 40 ARB Per Tombs

    uint256 private constant PRICE_WHITELIST = 4 * 10**14;
    uint256 private constant PRICE_PUBLIC = 8 * 10**14;

    uint256 private constant MAX_SUPPLY = 5000; // 5000 Crypto Athletes in CrowdSale
    uint256 private constant MAX_AMOUNT_PRIVATE = 1000; // 1000 Crypto Athletes in PreSale
    ...
    function mint(address payable _to, uint256 amount,  bytes32[] calldata merkleProof) public payable {
        ...
        require(total + amount <= MAX_SUPPLY, "MINT: Current count exceeds maximum element count.");
        require(total <= MAX_SUPPLY, "MINT: Please go to the Opensea to buy Crypto Athletes.");

        bytes32 node = keccak256(abi.encodePacked(msg.sender));

        if(MerkleProof.verify(merkleProof, mintWhitelistMerkleRoot, node)) {    
            require(amount <= MAX_MINT_PRESALE, "Invalid amount for a whitelist");
            ...
            require(block.timestamp >= goLiveDate + privateSalePeriod, "MINT: Invalid timestamp");
            require(msg.value >= amount * PRICE_PUBLIC, "public sale Insufficient balance 123");        
            for (uint256 i = 0; i < amount; i++) {
                _mintAnElement(_to);
            }
        }
    ...
    function _mintAnElement(address payable _to) private {
        ...
        emit CreateCryptoAthletes(_to, tokenId);
    }
