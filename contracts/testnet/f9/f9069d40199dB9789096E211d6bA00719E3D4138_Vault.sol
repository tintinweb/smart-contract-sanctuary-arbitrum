...
contract Vault is IVault, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function refreshPoolAmount(address _token) external {
        uint256 balance = IERC20Upgradeable(_token).balanceOf(address(this));
        VaultInfo.Data storage data = vaultInfo[_token];
        uint128 reservedAmounts = data.reservedAmounts - data.feeReserves;
        if (reservedAmounts < balance) {
            data.poolAmounts = uint128(balance) - reservedAmounts;
        } else {
            data.feeReserves = 0;
            data.poolAmounts = uint128(balance) - data.reservedAmounts;
        }
    }
}
