 ...
 */
contract OneInchLimitOrderProxy is AbstractLimitOrderProxy {
    using SafeERC20 for ERC20;

    address public limitOrderOperator;

    //slither-disable-next-line similar-names
    constructor(ERC20 _makerToken, ERC20 _takerToken, address _limitOrderOperator) AbstractLimitOrderProxy(_makerToken, _takerToken) {
        require(_limitOrderOperator != address(0), "!limitOrderOperator");
        limitOrderOperator = _limitOrderOperator;
    }

    /// @dev You need to send a transaction to write to a state variable.
    function setLimitOrderOperator(address limitOrderOperator_) external onlyOwner {
        require(limitOrderOperator_ != address(0), "!limitOrderOperator");
        limitOrderOperator = limitOrderOperator_;
    }
    ...
    function createOrder(uint256 optionID, uint256 makerAmount_, uint256 takerAmount_, LimitOrderType orderType_) external override nonReentrant {
        ...
        require(makerToken.transfer(limitOrderOperator, makerAmount_), "Could not transfer token to limit order operator");
        emit LimitOrderCreated(msg.sender, optionID, makerAmount_, takerAmount_);
    }

    /// @dev See {ILimitOrderProxy}
    function reinjectLiquidity(address payable optionContractAddress, uint256 optionID, uint256 amount) external override nonReentrant {
        require(msg.sender == limitOrderOperator, "Only limit order operator should call this function");
        takerToken.safeTransferFrom(msg.sender, address(this), amount);
