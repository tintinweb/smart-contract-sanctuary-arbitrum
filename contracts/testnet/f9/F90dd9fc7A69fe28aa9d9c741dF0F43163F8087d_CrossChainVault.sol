...
contract CrossChainVault is Ownable, Pausable, ICrossChainMsgReceive {
    address public crossChainMsgContract;
    ...
    constructor(
        ...
        address crossChainMsgContract_
    ) {
        ...
        for (uint256 i = 0; i < externalTokens_.length; i++) {
            ...
            tokenIndexes[ERC20(externalTokens_[i]).symbol()] = i;
        }
        crossChainMsgContract = crossChainMsgContract_;
    }
    ...
    function deposit(
        ...
        StructLibrary.CrossChainMsg memory chainMsg = StructLibrary
            .CrossChainMsg({
                fromChainId: block.chainid,
                toChainId: targetChain,
                fromContract: address(this),
                toContract: address(0),
                toUser: targetUser,
                btcToUser: targetBtcUser,
                tokenName: ERC20(token).symbol(),
                amount: amount,
                fromTxHash: "",
                fromUser: msg.sender,
                btcFromUser: "",
        ...
        ICrossChainMsgSend(crossChainMsgContract).send(chainMsg);
        emit Deposit(token, msg.sender, amount, targetChain, targetBtcUser);
    }

    function receiveMsg(
        StructLibrary.CrossChainMsg memory msg_
    ) external override {
        require(msg.sender == crossChainMsgContract, "invalid sender");
        require(supportedChains[msg_.fromChainId], "unsupported chain");
        require(supportedChains[msg_.toChainId], "unsupported chain");
        require(
            msg_.toContract == address(this),
            "invalid to contract address"
        );
        address tokenAddress = tokens[tokenIndexes[msg_.tokenName]]
            .tokenAddress;
        if (tokenAddress == address(0)) {
            tokenAddress = createBRC20Token(msg_.tokenName, msg_.tokenName);
            BRC20(tokenAddress).mint(msg_.toUser, msg_.amount);
        } else if (externalTokens[tokenAddress]) {
            TransferHelper.safeTransfer(tokenAddress, msg_.toUser, msg_.amount);
        } else {
            BRC20(tokenAddress).mint(msg_.toUser, msg_.amount);
        }
        emit Withdraw(tokenAddress, msg_.toUser, msg_.amount, msg_.fromTxHash);
    }
...
interface ICrossChainMsgReceive {
    ...
    //用户继承后接收处理消息
    function receiveMsg(StructLibrary.CrossChainMsg memory msg) external;
}
...
interface ICrossChainMsgSend {
    ...
    //用户发送消息
    function send(StructLibrary.CrossChainMsg memory msg) external;
}
...
library StructLibrary {
    struct CrossChainMsg {
        uint256 fromChainId; //源链id
        uint256 toChainId; // 目标链id
        address fromContract; // 原链合约地址
        address toContract; // 目标链合约地址
        string tokenName; // 代币名称
        uint256 amount; // 代币数量
        string fromTxHash; // 原链交易哈希
        address fromUser; // 原链用户地址
        string btcFromUser; // 原链用户地址
        address toUser; // 目标链用户地址
        string btcToUser; // 目标链用户地址
        bytes message; // 消息
    }
}
