...
contract Farming is Initializable, OwnableUpgradeable, PausableUpgradeable, IFarming {
    /**
    ...
    mapping (address => bool) public investorWallets;
    // Info of each user that stakes LP tokens.
    mapping (uint16 => mapping (address => UserInfo)) public userInfo;

    uint256 public investorsUnlockTimestamp;
    ...
    function initialize(        
        ...
        uint256 _investorsUnlockTimestamp
    ) public initializer {
        __Ownable_init();
        ...
        investorsUnlockTimestamp = _investorsUnlockTimestamp;
    }
    ...
    //************* MANAGER FUNCTIONS ********************************
    function setTokenPerBlock(uint256 _tokensFarmedPerBlock) public onlyOwner {
        tokensFarmedPerBlock = _tokensFarmedPerBlock;
        massUpdatePools();
    }

    function setInvestorsUnlockTimestamp(uint256 _investorsUnlockTimestamp) public onlyOwner {
        investorsUnlockTimestamp = _investorsUnlockTimestamp;
    }

    // Add a new lp to the pool. Can only be called by the owner.
    // XXX DO NOT add the same LP token more than once. Rewards will be messed up if you do.
    function addPool(uint64 _allocPoint, address _lpTokenAddress, uint64 _startBlock, uint64 _endBlock, bool _withUpdate) public onlyOwner {
    ...
    function addInvestorAddress(address _wallet) public onlyOwner {
        investorWallets[_wallet]=true;
    }

    function removeInvestorAddress(address _wallet) public onlyOwner {
        delete investorWallets[_wallet];
    }

    // Update the given pool's Token allocation point. Can only be called by the owner.
    function updatePoolData(uint16 _pid, uint64 _allocPoint, uint64 _startBlock, uint64 _endBlock, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    //************* ADMIN FUNCTIONS ********************************
    function withdrawRewardTokens(address _to, uint256 _amount) public onlyOwner {
    ...
    // Deposit LP tokens to farming contract for Token allocation.
    ...
    function withdraw(uint16 _pid, uint256 _amount) public override whenNotPaused {
        if (investorWallets[msg.sender]) {
            require(block.timestamp > investorsUnlockTimestamp, "LP tokens are not yet unlocked for private investors");
        }

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];

        require(user.amount >= _amount, "Withdrawal amount is greater than available");
        updatePool(_pid);
        user.pendingReward += user.amount * pool.accTokenPerShare / 1e12 - user.rewardDebt;
        if(_amount > 0) {
            user.amount -= _amount;
    ...
    // Claim rewarded tokens from farming.
    ...
    //************* VIEW FUNCTIONS ********************************
    function getContractData() external override view returns (uint256, uint256){
        return (tokensFarmedPerBlock, totalAllocPoint);
    }
    ...
    function getUserInfo(uint16 _pid, address _user) external override view returns (uint256, uint256, uint256, bool) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        return (
            user.amount,
            user.totalRewarded,
            user.pendingReward + user.amount * accTokenPerShare / 1e12 - user.rewardDebt,
            investorWallets[_user]
        );
    }
...
interface IFarming{
    ...
    function getContractData() external view returns (uint256, uint256);
    function getPoolLength() external view returns (uint256);
    function getPool(uint16 _index) external view returns (address, uint256, uint64, uint64, uint64, uint256, uint256);
    function getUserInfo(uint16 _pid, address _user) external view returns (uint256, uint256, uint256, bool);
}
