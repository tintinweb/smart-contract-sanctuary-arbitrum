...
contract ProfitShare is Roles, ERC2771Context, IProfitShare {
    ...
    uint256 private EPOCH = 20 minutes; //for test
    ...
    function _withdraw(
        ...
        uint256 currentEpoch = _currentEpoch();
        uint256 total;
        uint length = epoch.length;
        for (uint256 i; i < length; ) {
            if (profits[user][address(0x0)][epoch[i]] > 0) {
                require(!claimed[user][epoch[i]], "!claimed");
                require(epoch[i] < currentEpoch, "!epoch-done");
                total += uint256(profits[user][address(0x0)][epoch[i]]);
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encodePacked(
                        WITHDRAW_HASH,
                        keccak256(abi.encodePacked(epoch)),
                        gasLess.deadline,
                        nonce
                    )
                )
