...
contract PrimaryIndexTokenLiquidationV3 is
	Initializable,
	...
	ReentrancyGuardUpgradeable
{
	...
	modifier isProjectTokenListed(address _projectToken) {
		require(
			...
			"PITLiquidation: project token is not listed"
		);
		_;
	}
	...
	modifier isLendingTokenListed(address _lendingToken) {
		require(
			...
			"PITLiquidation: lending token is not listed"
		);
		_;
	}
	...
	modifier onlyRelatedContracts() {
		require(
			...
			"PITLiquidation: caller is not related Contract"
		);
		_;
	}
	...
	) public isProjectTokenListed(_projectToken) isLendingTokenListed(_lendingToken) nonReentrant {
		...
		returns (uint256)
	{
		...
		(uint minLA, uint maxLA) = getLimitLiquidationAmount(
			_account,
			_projectToken,
			_lendingToken
		);
		...
		require(
			_lendingTokenAmount >= minLA && _lendingTokenAmount <= maxLA,
			"PITLiquidation: invalid amount"
		);
		(uint256 healthFactorNumerator, uint256 healthFactorDenominator) = primaryIndexToken
			.estimatedHealthFactor(_account);
		...
		uint256 projectTokenLiquidatorReceived = _distributeReward(
			_account,
			_projectToken,
			projectTokenToSendToLiquidator,
			liquidator
		);
	...
	function getEstimatedRewardAmount(
		...
		uint256 projectTokenEvaluation = (getTokenPrice(_lendingToken, _lendingTokenAmount) *
			projectTokenMultiplier) / getTokenPrice(_projectToken, projectTokenMultiplier);
		uint256 depositedProjectTokenAmount = primaryIndexToken.depositedAmount(
			_projectToken,
			_account
		);
		...
		uint256 projectTokenToSendToLiquidator = (projectTokenEvaluation * lrfNumerator) /
			lrfDenominator;
	...
	function _getProjectTokenToSendToLiquidator(
		...
		uint256 projectTokenMultiplier = 10 ** ERC20Upgradeable(_projectToken).decimals();
		(uint256 lrfNumerator, uint256 lrfDenominator) = liquidatorRewardFactor(_account);
		uint256 repaid = primaryIndexToken.repayFromRelatedContract(
			_lendingToken,
			_lendingTokenAmount,
			liquidator,
			_account
		);
		uint256 projectTokenEvaluation = (getTokenPrice(_lendingToken, repaid) *
			projectTokenMultiplier) / getTokenPrice(_projectToken, projectTokenMultiplier);
	...
	function liquidatorRewardFactor(
		...
		(uint256 hfNumerator, uint256 hfDenominator) = primaryIndexToken.estimatedHealthFactor(
			_account
		);
		if (hfDenominator == 0) {
			lrfNumerator = 0;
	...
	function getEstimatedOutstandingInUSD(
		address user,
		address lendingToken
	) public view returns (uint256 outstanding, uint256 outstandingInUSD) {
		(uint256 loanBody, uint256 accrual, uint256 estimatedOutstandingInUSD) = primaryIndexToken
			.getEstimatedOutstandingInUSD(user, lendingToken);
	...
	function getMaxLiquidationAmount(
		...
		(
			uint256 estimatedOutstanding,
			uint256 estimatedOutstandingInUSD
		) = getEstimatedOutstandingInUSD(_account, _lendingToken);
		if (estimatedOutstanding == 0) return 0;

		(uint256 calNumeratorMaxLA, uint256 calDenominatorMaxLA) = _calMaxLA(
			_account,
			_projectToken,
			_lendingToken
		);
		...
		maxLAParams.calculatedMaxLA = maxLAParams.denominatorMaxLA > 0 &&
			maxLAParams.numeratorMaxLA > 0
			? (maxLAParams.numeratorMaxLA * 10 ** LIQUIDATOR_REWARD_FACTOR_DECIMAL) /
				maxLAParams.denominatorMaxLA
			: 0;

		uint estimatedOutstandingInUSDMul = estimatedOutstandingInUSD *
			10 ** LIQUIDATOR_REWARD_FACTOR_DECIMAL;
		maxLAParams.maxLACompare = maxLAParams.calculatedMaxLA > estimatedOutstandingInUSDMul
			? estimatedOutstandingInUSDMul
		...
		maxLA =
			(maxLAParams.maxLACompare * lendingTokenMultiplier) /
			getTokenPrice(_lendingToken, lendingTokenMultiplier);
	}
	...
	function _calMaxLA(
		...
		uint256 totalEstimatedWeightedLoanInUSD = primaryIndexToken.totalEstimatedWeightedLoanInUSD(
			_account
		);
		...
		IPrimaryIndexTokenV3.Ratio memory lvrProjectToken = primaryIndexToken
			...
			.loanToValueRatio;
		IPrimaryIndexTokenV3.Ratio memory lvrLendingToken = primaryIndexToken
			.lendingTokenInfo(_lendingToken)
			.loanToValueRatio;
	...
	function getLimitLiquidationAmount(
		...
		minLA = Math.min(
			maxLA,
			(minPartialLiquidationAmount * projectTokenMultiplier) /
				getTokenPrice(_lendingToken, projectTokenMultiplier)
		);
	}
}