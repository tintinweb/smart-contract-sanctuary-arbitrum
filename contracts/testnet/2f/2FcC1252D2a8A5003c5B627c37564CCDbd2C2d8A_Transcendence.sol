...
contract Transcendence is ManagerModifier, ReentrancyGuard, Pausable {
  ...
  function transcend(
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      // Transfer Anima
      ANIMA.safeTransferFrom(msg.sender, VAULT, temp);
      ...
      while(rewardIndex < rewardTranscendenceLevelCaps.length && temp > rewardTranscendenceLevelCaps[rewardIndex]) {
        rewardIndex++;
      }

      // Dispense rewards if eligible
      if (rewardIndex < rewardTokenIds.length) {
        LOOT_BOX_DISPENSER.dispense(msg.sender, rewardTokenIds[rewardIndex], 1);
      }
  ...
  function configureRewards(
    ...
    uint256[] calldata _rewardTokenIds) external onlyAdmin {
