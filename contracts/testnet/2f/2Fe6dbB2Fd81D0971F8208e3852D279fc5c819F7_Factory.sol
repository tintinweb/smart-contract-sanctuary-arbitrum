...
contract Exchange is Ownable {
    ...
    uint8 public tokenA_decimals;
    uint8 public tokenB_decimals;
    ...
    mapping(address => mapping(uint256 => LinkedListLib.LinkedList)) public orderBook;
    ...
    function getDeposits(address account, address tokenAddress) external view returns (uint256) {
        require(tokenAddress == tokenA || tokenAddress == tokenB, "Token is not in the pool");
        return deposits[account][tokenAddress];
    }

    // Sell
    function newSellOrder(uint32 price, uint256 sellAmount, uint256 priceIdx) external returns (bool) {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            uint256 buyAmount = orderBook[tokenB][price].nodes[head_].order.amount;

            if (sellAmount == 0) {
                return true;
            } else if ((price * sellAmount) / (10**uint256(tokenA_decimals)) >= buyAmount) {
                // sell amount >= buy amount
                LinkedListLib.Order memory o = orderBook[tokenB][price].nodes[head_].order;
                ...
                deposits[msg.sender][tokenA] -= (o.amount * 10**uint256(tokenA_decimals - tokenB_decimals)) / price;
                IERC20(tokenB).transfer(msg.sender, o.amount);
                IERC20(tokenA).transfer(
                    o.seller,
                    (o.amount * 10**uint256(tokenA_decimals - tokenB_decimals)) / price
                );
                sellAmount -= (o.amount * 10**uint256(tokenA_decimals - tokenB_decimals)) / price;
            } else if (buyAmount > (price * sellAmount) / (10**uint256(tokenA_decimals))) {
                LinkedListLib.Order memory o = orderBook[tokenB][price].nodes[head_].order;
                orderBook[tokenB][price].nodes[head_].order.amount -=
                    (price * sellAmount) / (10**uint256(tokenA_decimals));
                OPVSetLib._subVolume(_buyOrders, o.seller, head_, (price * sellAmount) / (10**uint256(tokenA_decimals)));
                PVNodeLib._subVolume(buyOB, priceIdx, (price * sellAmount) / (10**uint256(tokenA_decimals)));

                deposits[o.seller][tokenB] -= (price * sellAmount) / (10**uint256(tokenA_decimals));
                deposits[msg.sender][tokenA] -= sellAmount;
                IERC20(tokenB).transfer(msg.sender, (price * sellAmount) / (10**uint256(tokenA_decimals)));
        ...
        if (orderBook[tokenA][price].length == 0 && sellAmount > 0) {
            bytes32 orderId = LinkedListLib.initHead(orderBook[tokenA][price], msg.sender, sellAmount);
            OPVSetLib._add(_sellOrders, msg.sender, orderId, price, sellAmount);
            PVNodeLib._addVolume(sellOB, priceIdx, sellAmount);
        } else if (sellAmount > 0) {
            bytes32 orderId = LinkedListLib.addNode(orderBook[tokenA][price], msg.sender, sellAmount);
            OPVSetLib._add(_sellOrders, msg.sender, orderId, price, sellAmount);
            PVNodeLib._addVolume(sellOB, priceIdx, sellAmount);
        }
    ...
    function getAllSellOrders(uint32 price) external view returns (LinkedListLib.Order[] memory) {
    ...
    function activeSellOrders() external view returns (OPVSetLib.OPVnode[] memory) {
        OPVSetLib.OPVnode[] memory sellOrders = new OPVSetLib.OPVnode[](_sellOrders._orders[msg.sender].length);
    ...
    function deleteSellOrder(uint32 price, bytes32 orderId, uint256 priceIdx) external returns (bool) {
        require(
            buyOB[priceIdx].price == price && sellOB[priceIdx].price == price,
            "Price does not match the index"
        );

        LinkedListLib.Order memory o = orderBook[tokenA][price].nodes[orderId].order;
    ...
    // Buy
    function newBuyOrder(uint32 price, uint256 buyAmount, uint256 priceIdx) external returns (bool) {
        // get priceIdx using the FE
        require(
            buyOB[priceIdx].price == price && sellOB[priceIdx].price == price,
            "Price does not match the index"
        ...
        // no fee under 1000
        deposit(tokenB, (price * buyAmount) / (10**uint256(tokenB_decimals)));
        uint256 currentFee = (price * buyAmount * (1000 - feeRate)) / (10**uint256(tokenB_decimals)) / 1000;
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            if (buyAmount == 0) {
                ...
                deposits[msg.sender][tokenB] -= (price * o.amount) / (10**uint256(tokenA_decimals));
                IERC20(tokenA).transfer(msg.sender, o.amount);
                IERC20(tokenB).transfer(o.seller, (price * o.amount) / (10**uint256(tokenA_decimals)));
                buyAmount -= o.amount;
                ...
                deposits[msg.sender][tokenB] -= (price * buyAmount) / (10**uint256(tokenA_decimals));
                IERC20(tokenA).transfer(msg.sender, buyAmount);
                IERC20(tokenB).transfer(o.seller, (price * buyAmount) / (10**uint256(tokenA_decimals)));
                buyAmount = 0;
            }
        }
        // new buy order
        if (orderBook[tokenB][price].length == 0 && buyAmount > 0) {
            bytes32 orderId = LinkedListLib.initHead(orderBook[tokenB][price], msg.sender, (price * buyAmount) / (10**uint256(tokenA_decimals)));
            OPVSetLib._add(_buyOrders, msg.sender, orderId, price, (price * buyAmount) / (10**uint256(tokenA_decimals)));
            PVNodeLib._addVolume(buyOB, priceIdx, (price * buyAmount) / (10**uint256(tokenA_decimals)));
        } else if (buyAmount > 0) {
            bytes32 orderId = LinkedListLib.addNode(orderBook[tokenB][price], msg.sender, (price * buyAmount) / (10**uint256(tokenA_decimals)));
            OPVSetLib._add(_buyOrders, msg.sender, orderId, price, (price * buyAmount) / (10**uint256(tokenA_decimals)));
            PVNodeLib._addVolume(buyOB, priceIdx, (price * buyAmount) / (10**uint256(tokenA_decimals)));
        }
    ...
    function getAllBuyOrders(uint32 price) external view returns (LinkedListLib.Order[] memory) {
    ...
    function activeBuyOrders() external view returns (OPVSetLib.OPVnode[] memory) {
        OPVSetLib.OPVnode[] memory buyOrders = new OPVSetLib.OPVnode[](_buyOrders._orders[msg.sender].length);
    ...
    function getPVobs() external view returns (PVNodeLib.PVnode[] memory, PVNodeLib.PVnode[] memory) {
        return (sellOB, buyOB);
    }

    function initPVnode(uint32 price) external returns (uint256) {
        for (uint256 i = 0; i < sellOB.length; i++) {
            require(sellOB[i].price != price, "Price already exists in the order book");
        }
