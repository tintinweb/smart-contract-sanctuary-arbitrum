...
interface ISynthrSwap {
    function swapSynthToNative(uint256 _sourceAmount, bytes32 _sourceKey, bytes32 _destKey, address _target, bytes memory _data, uint16 _chainId) external payable;

    function destSwapSynthToNative(address _account, bytes32 _sourceKey, uint256 _sourceAmount, bytes32 _destKey, uint256 _destAmount, address _target, bytes memory _data) external;
}
...
contract SynthrIssuer is Owned, MixinSystemSettings {
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        ...
        newAddresses[11] = CONTRACT_SYNTHR_SWAP;
        newAddresses[12] = CONTRACT_SYNTHR_BRIDGE;
        newAddresses[13] = CONTRACT_SYNTHR_AGGREGATOR;
    ...
    function synthBurnFromSynthrSwap(
        ...
        require(synths[_synthKey].balanceOf(_account) >= _synthAmount, "Issuer: insufficient synth balance to burn.");
        synths[_synthKey].burn(_account, _synthAmount);
    }
    ...
    function destIssue(
        ...
        bytes32 _synthKey,
        uint256 _synthAmount
    ) external systemActive onlySynthrBridgeOrWrappedSynthr {
        synths[_synthKey].issue(_account, _synthAmount);
    ...
    modifier onlySynthrSwap() {
        require(msg.sender == address(synthrSwap()), "Issuer: Only the SynthrSwap contract can perform this action");
        _;
    }
