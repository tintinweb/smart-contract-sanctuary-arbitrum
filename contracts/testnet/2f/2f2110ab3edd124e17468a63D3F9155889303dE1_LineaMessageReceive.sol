...
contract LineaMessageReceive is Initializable, OwnableUpgradeable {

    IMessageService public messageService;

    address public remoteSender;

    IZKBridgeEntrypoint public zkBridge;

    uint16 public chainId;

    function initialize(address _messageService, address _remoteSender, uint16 _chainId, address _zkBridge) public initializer {
        ...
        chainId = _chainId;
        zkBridge = IZKBridgeEntrypoint(_zkBridge);
        __Ownable_init();
    }

    function receiveMessage(uint16 _srcChainId, address _srcAddress, uint16 _dstChainId, address _dstAddress, uint64 _sequence, bytes calldata _payload) external payable {
        //        require(msg.sender == address(messageService), "Not From MessageService");
        //        require(messageService.sender() == remoteSender, "invalid emitter");
        if (_dstChainId == chainId) {
            zkBridge.validateTransactionFromL2(_srcChainId, _srcAddress, _dstAddress, _sequence, _payload);
        } else {
            zkBridge.sendFromL2{value : msg.value}(_dstChainId, _dstAddress, _payload);
        }
    }
