...
contract TimeFlow is Ownable{
    ...
    //交易发起者是否发起timeFlow将token转交给接收者
    mapping(address=>mapping(uint256=>bool))public ifTimeFlow;
    //事务失败，发起者是否已提取违约金
    mapping(string=>mapping(address=>bool))public ifGetPenalSum;

    //设置timeContractAddress
    function setContractAddress(address timeAddress)external onlyOwner{
        thisTimeContract=timeAddress;
    }
   
    //交易事务发送者质押交易金额
    function senderStake(string calldata contentName)external{
        //得到事务id
    ...
    //交易事务接收者质押保证金
    function pointStake(string calldata contentName)external{
        //得到事务id
        uint256 getThisContentId=contentId(contentName);
        //得到需要质押的保证金
    ...
    function sendWithdraw(string calldata contentName)external{
        //得到事务id
        uint256 getThisContentId=contentId(contentName);
        //得到事务交易money
        uint256 amount=tradeMoney(contentName);
    ...
    function pointWithdraw(string calldata contentName)external{
        //得到事务id
        uint256 getThisContentId=contentId(contentName);
        //得到事务已经质押的money
        ...
        //得到事务交易token
        IERC20 thisToken=IERC20(tradeToken(contentName));
        require(ifTimeFlow[msg.sender][getThisContentId]==false);  //交易发起者是否已经doTimeFlow
    ...
    //执行相应token时间锁
    function doTimeFlow(string calldata contentName)external{
        //得到事务id
        uint256 getThisContentId=contentId(contentName);
        //得到事务交易token
        IERC20 thisToken=IERC20(tradeToken(contentName));

        require(msg.sender==getSource(contentName),"Sender error");  //交易发起者是否是该事务发送者
        require(getCheckResult(contentName),"Not enough votes, transaction fails!");//票数是否通过
        require(doThisTransfer[getSource(contentName)][contentName]==true,"Not send contract Token");//发起者是否向合约转入相应token的数量
        require(senderState(contentName)&&pointState(contentName),"The sender or receiver does not agree to the transaction!");//发起者确认与接收者确认
        require(getTime(contentName)==0,"time <= end time");  //当前时间是否>=结束时间
        //支付协议费用
        thisToken.safeTransfer(0xaE67336f06B10fbbb26F31d31AbEA897290109B9,calculate(contentName));
    ...
    function penalSum(string calldata contentName)external{
        //得到事务id
        uint256 getThisContentId=contentId(contentName);
        //得到事务已经质押的money
    ...
    function setBail(string calldata contentName,uint256 rate)external{
        uint256 getThisContentId=contentId(contentName);
    ...
    //得到事务Token
    function tradeToken(string calldata contentName)public view returns(address){
        return timeFlowNftsInter(thisTimeContract).getTradeToken(contentName);
    }

    //得到交易money
    function tradeMoney(string calldata contentName)public view returns(uint256){
    ...
    //得到剩余交易时间
    function getTime(string calldata contentName)public view returns(uint256){
