...
contract Woolf is IWoolf, ERC721Enumerable, Ownable, Pausable {
  ...
  // mint price
  uint256 public constant MINT_PRICE = .0006 ether;
  ...
  mapping(uint256 => uint256) public wolf_level;

  /** 
   * instantiates contract and rarity tables
   */
  constructor(address _wool, address _traits, uint256 _maxTokens) ERC721("Greedy Wolf", 'GREEDYWOLF') { 
    wool = WOOL(_wool);
    traits = ITraits(_traits);
    MAX_TOKENS = _maxTokens;
    //PAID_TOKENS = _maxTokens / 5;
    PAID_TOKENS = _maxTokens / 10;
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    /** 
   * the first 10% are paid in ETH
   * the next 10% are 20000 $WOOL
   * the next 20% are 40000 $WOOL
   * the next 20% are 80000 $WOOL
   * the final 40% are 160000 $WOOL
   * @param tokenId the ID to check the cost of to mint
   * @return the cost of the given token ID
   */
  function mintCost(uint256 tokenId) public view returns (uint256) {
    if (tokenId <= PAID_TOKENS) return 0;
    if (tokenId <= MAX_TOKENS * 1 / 5) return 20000 ether;
    if (tokenId <= MAX_TOKENS * 2 / 5) return 40000 ether;
    if (tokenId <= MAX_TOKENS * 3 / 5) return 80000 ether;
    return 160000 ether;
  }
  ...
  /**
   * the first 10% (ETH purchases) go to the minter
   * the remaining 90% have a 10% chance to be given to a random staked wolf
  ...
  function getLevel(uint256 tokenId) external returns (uint256) {
     return wolf_level[tokenId];
  }

  function setLevel(uint256 tokenId, uint256 level) external {
     wolf_level[tokenId] = level;
  }
