...
contract Exchange is Ownable {
    ...
    function newSellOrder(
        ...
        for (uint8 i = 0; i < len; i++) {
            ...
            if (sellAmount == 0) {
                ...
                deposits[o.seller][tokenB] -= (price * sellAmount)/ (10 ** uint256(TOKEN_diff_DECIMAL)) / 100;
                deposits[msg.sender][tokenA] -= sellAmount;
                IERC20(tokenB).transfer(msg.sender,(price * sellAmount) / (10 ** uint256(TOKEN_diff_DECIMAL)) / 100);
    ...
    function newBuyOrder(
        ...
        uint256 currentFee = (price * (buyAmount * (1000-feeRate)) / 1000)/ (10 ** uint256(TOKEN_diff_DECIMAL)) / 100;
        ...
        for (uint8 i = 0; i < len; i++) {
            ...
            if (buyAmount == 0) {
                ...
                deposits[msg.sender][tokenB] -= (price * o.amount)/ (10 ** uint256(TOKEN_diff_DECIMAL)) / 100;
                ...
                deposits[msg.sender][tokenB] -= (price * buyAmount)/ (10 ** uint256(TOKEN_diff_DECIMAL)) / 100;
