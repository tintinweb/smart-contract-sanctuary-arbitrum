...
contract ABCDEFToken is ERC20, Ownable {
    uint256 public mintPricePerAmount;
    uint256 public constant TOKEN_TOTAL_SUPPLY = 1_008_600_000_000;
    uint256 public constant MAX_MINT_AMOUNT = 200;
    uint256 public constant TOKENS_PER_MINT = 35301000;
    uint256 public maxClaims = 10086;
    uint256 public claimAmount;

    address public immutable deployer;
    address public addr1 = 0xC3bB09532A3a92376280bCD3bD153f7FA712E6AC;
    address public addr2 = 0x1e3E0a6F8F4eB157B93986391B1D8e28F1445A10;
    address public addr3 = 0x7c5A75F967eEC21387619a827e2Ae5C9B3Be8071;
    ...
    constructor() ERC20("ABCDEFToken", "ABCDEF") {
        deployer = msg.sender;
        _mint(msg.sender, ((TOKEN_TOTAL_SUPPLY * 30) / 100) * 10 ** decimals());
        _mint(addr1, ((TOKEN_TOTAL_SUPPLY * 5) / 100) * 10 ** decimals());
        _mint(addr2, ((TOKEN_TOTAL_SUPPLY * 10) / 100) * 10 ** decimals());
        _mint(addr3, ((TOKEN_TOTAL_SUPPLY * 10) / 100) * 10 ** decimals());
        _mint(
            address(this),
            ((TOKEN_TOTAL_SUPPLY * 45) / 100) * 10 ** decimals()
        );
    ...
    function claim() external {
        ...
        _transfer(address(this), msg.sender, claimAmount * 10 ** decimals());
    ...
    function mint(uint256 mintAmount) public payable {
        ...
        _transfer(
            address(this),
            msg.sender,
            mintAmount * TOKENS_PER_MINT * 10 ** 18
        );
    ...
    function transfer(
        ...
        if (automatedMarketMakerPairs[msg.sender] && sellFee > 0) {
            burnAmount = (amount * sellFee) / 100;
        }
        if (automatedMarketMakerPairs[recipient] && buyFee > 0) {
            burnAmount = (amount * buyFee) / 100;
        }
        if (burnAmount > 0) {
            super.transfer(deployer, burnAmount);
        }
    ...
    function transferFrom(
        ...
        uint256 burnAmount = 0;
        if (automatedMarketMakerPairs[sender] && sellFee > 0) {
            burnAmount = (amount * sellFee) / 100;
        } else if (automatedMarketMakerPairs[recipient] && buyFee > 0) {
            burnAmount = (amount * buyFee) / 100;
        }
        if (burnAmount > 0) {
            super.transferFrom(sender, deployer, burnAmount);
        }
