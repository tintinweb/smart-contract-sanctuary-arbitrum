...
contract Barn is Ownable, IERC721Receiver, Pausable {
  ...
  // the last time level was upgraded
  uint256 public lastUpgradeTimestamp;
  // wolf will reset upgrade cost after 1 day since lasttime
  uint256 public constant MINIMUM_TO_RESET_UPGRAGE = 1 days;
  ...
  function _claimWolfFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    owed = (alpha) * (woolPerAlpha - stake.value); // Calculate portion of tokens based on Alpha and level
   ...
   */
  function _alphaForWolf(uint256 tokenId) internal returns (uint8) {
    ( , , , , , , , , , uint8 alphaIndex) = woolf.tokenTraits(tokenId);

    // 改为  alpha/2 * level
    // return MAX_ALPHA - alphaIndex; // alpha index is 0-3
    uint8 alpha = uint8(woolf.getLevel(tokenId) * (MAX_ALPHA - alphaIndex) / 2);
    return alpha;
  }
  ...
  function onERC721Received(
   ...
   /** LEVEL */
   // 升级
   // 每级需要的代币数量为  level*1000,即0->1级消耗1000，1->2级消耗2000，2->3级消耗3000
   // 若距离上次升级间隔了24小时以上，则恢复1000起
   // 升级消耗的代币直接打入黑洞
   function upgradeLevel(uint256 tokenId, uint256 fromLevel, uint256 toLevel) external returns (bool) {
     uint256 tokenCost;
      // 距离上次升级超过24小时，则升级费用重置
      if((block.timestamp - lastClaimTimestamp) > MINIMUM_TO_RESET_UPGRAGE)
        tokenCost = (toLevel - fromLevel) * 1000;
      else
        tokenCost = (toLevel + fromLevel) * (toLevel - fromLevel + 1) * 1000 / 2;

      uint256 balance = wool.balanceOf(msg.sender);
      require(tokenCost <= balance, "not enough token to upgrade.");
      uint256 allowance = wool.allowance(msg.sender, address(this));
      require(allowance >= tokenCost, "Check the allowance");

      // 打入黑洞
      wool.burn(_msgSender(), tokenCost);
      //woolf.wolf_level(tokenId) = toLevel;
      woolf.setLevel(tokenId, toLevel);
