...
contract LiquidityProvider is ILiquidityProvider, BlockContext, ClearingHouseCallee, LiquidityProviderStorageV1 {
        ...
        )
    {
        ...
        if (openOrder.liquidity == 0) {
            ...
            openOrder.lastExchangeFeeIndex = _feeGrowthPerLiquidityUnitIndex;
        }
    ...
    function _getPendingFee(address baseToken, Account.LPInfo memory order) internal view returns (uint256) {
        if (order.liquidity == 0 || _feeGrowthPerLiquidityUnitIndex == 0) {
            return 0;
        }

        uint256 lpFeeRatioAccumulate;
        if (order.lastExchangeFeeIndex < _feeGrowthPerLiquidityUnitIndex - 1) {
            uint256 latestValue = _feeGrowthPerLiquidityUnitMap[baseToken][_feeGrowthPerLiquidityUnitIndex - 1];
    ...
    function updateLPCalcFeeRatio(address baseToken, uint256 lpCalcFeeRatio) external override {
        ...
        if (_feeGrowthPerLiquidityUnitIndex > 0) {
            previousValue = _feeGrowthPerLiquidityUnitMap[baseToken][_feeGrowthPerLiquidityUnitIndex - 1];
        }
        _feeGrowthPerLiquidityUnitMap[baseToken][_feeGrowthPerLiquidityUnitIndex] = previousValue + lpCalcFeeRatio;
        _feeGrowthPerLiquidityUnitIndex++;
    }
    ...
    function setLPCalcFeeRatio(
        address baseToken,
        uint256 startIndex,
        uint256 endIndex
    ) external {
        for (uint256 i = startIndex; i < endIndex; i++) {
            _feeGrowthPerLiquidityUnitMap[baseToken][i] =
                _feeGrowthPerLiquidityUnitMap[baseToken][i > 0 ? i - 1 : 0] +
                _totalLPCalcFeeRatioMap[baseToken][i];
            _feeGrowthPerLiquidityUnitIndex++;
        }
    }

    function getLPCalcFeeRatioLength(address baseToken) external view returns (uint256) {
        return _totalLPCalcFeeRatioMap[baseToken].length;
    }
}
