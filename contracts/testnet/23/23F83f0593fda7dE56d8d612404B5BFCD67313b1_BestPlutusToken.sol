  ...
  ICheckpointer
{
  ...
  function totalSupply() external view returns (uint256) {
    return _getTotalSupply() * getRate();
  }

  function getTotalSupplyWithMultiplier() external view returns (uint256) {
    return _getTotalSupply() * getMultiplier() * getRate();
  }

  function getPastTotalSupplyWithMultiplier(uint256 timepoint) external view returns (uint256) {
    return getPastTotalSupply(timepoint) * getPastMultiplier(timepoint) * getPastRate(timepoint);
  }

  function getVotesWithMultiplier(address account) external view returns (uint256) {
    return getVotes(account) * getMultiplier() * getRate();
  }

  function getPastVotesWithMultiplier(address account, uint256 timepoint) external view returns (uint256) {
    return getPastVotes(account, timepoint) * getPastMultiplier(timepoint) * getPastRate(timepoint);
  }
  ...
  function getRate() public view returns (uint256) {
    if (rateProvider == address(0)) return 1;

    return IRateProvider(rateProvider).getRate() / 1e4;
  }

  function getPastRate(uint256 timepoint) public view returns (uint256) {
    if (rateProvider == address(0)) return 1;

    return IRateProvider(rateProvider).getPastRate(timepoint) / 1e4;
  }
