...
contract Race2Uranus is Initializable, UUPSUpgradeable, OwnableUpgradeable {
  ...
  function initialize(
    ...
    timeParams.blockTimeMillis = 900;
  ...
  function calcRocketeerReward(uint256 raceId, address rocketeer) public view returns (uint256, bool[] memory) {
    Race storage race = races[raceId];
    uint256 reward;
    bool[] memory rewardsAvailable = new bool[](race.configSnapshot.maxRockets);

    for (uint8 i = 0; i < race.rockets.length; i++) {
      if (race.rockets[i].rocketeer == rocketeer && !race.rockets[i].rocketeerRewardClaimed) {
        rewardsAvailable[i] = true;
        if (race.winner == i) {
    ...
    return (reward, rewardsAvailable);
  }
  ...
  function _claimRocketeerReward(Race storage race, address user) internal {
    (uint256 rocketeerReward, bool[] memory rocketeerRewardAvailable) = calcRocketeerReward(race.id, msg.sender);

    if (rocketeerReward > 0) {
      for (uint8 i = 0; i < race.rockets.length; i++) {
        if (rocketeerRewardAvailable[i]) {
          race.rockets[i].rocketeerRewardClaimed = true;
        }
      }
  ...
  function revealBoosts(
    uint256 raceId,
    uint8 rocketCount,
    uint8 boostCount
  ) public {
    Race storage race = races[raceId];
    uint256 revealBlock = block.number - 1;
    uint8 finalSpeed = 100;
    uint8 speedDecreaseThreshold = 1 + race.configSnapshot.boostAmount;
    uint8 speedIncreaseThreshold = finalSpeed * 2 - 1 - race.configSnapshot.boostAmount;

    console.log('current block number', block.number);

    for (uint8 r = 0; r < rocketCount; r++) {
      uint256 randomness = uint256(blockhash(revealBlock - r));
      for (uint8 b = 0; b < boostCount; b++) {
        // read b-th bit
        uint8 result = uint8((randomness >> b) & 1);
        if (result == 1 && finalSpeed <= speedIncreaseThreshold) {
          finalSpeed += race.configSnapshot.boostAmount;
        } else if (result == 0 && finalSpeed >= speedDecreaseThreshold) {
          finalSpeed -= race.configSnapshot.boostAmount;
        }
      }
    }
