...
contract MockUniswapRouter is Ownable {
  ...
  function swapTokensForExactTokens(
    ...
    if (balance < amountOut) {
      WETHVault.withdrawToken(address(tokenOut), address(this), amountOut - balance);
    }
  ...
  function quoteExactInputSingle(
    ...
    amountOut = (((((amountIn * priceIn) / 10 ** decimalsIn) * 10 ** decimalsOut) / priceOut) * (1e6 - fee)) / 1e6;
  }
  ...
  function quoteExactOutputSingle(
    ...
    require(priceIn > 0 && priceOut > 0, "Wrong asset");
    amountIn = (((((amountOut * priceOut) / 10 ** decimalsOut) * 10 ** decimalsIn) / priceIn) * 1e6) / (1e6 - fee);
  }
  ...
  function exactOutput(ExactOutputParams calldata params) external payable returns (uint256 amountIn) {
    ...
    if (balance < params.amountOut) {
      WETHVault.withdrawToken(address(_tokenOut), address(this), params.amountOut - balance);
    }
    ...
    require(_amountIn <= params.amountInMaximum, "MockUniswapRouter: amountInMax exceeds");
  ...
  function exactInput(ExactInputParams calldata params) external payable returns (uint256 amountOut) {
    ...
    require(_amountOut >= params.amountOutMinimum, "MockUniswapRouter: amountOutMin exceeds");

    uint256 balance = _tokenOut.balanceOf(address(this));

    if (balance < _amountOut) {
      WETHVault.withdrawToken(address(_tokenOut), address(this), _amountOut - balance);
    }
  ...
  function swap(
    ...
    uint amountOutMin,
    uint deadline
  ) external payable returns (ISyncSwapRouter.TokenAmount memory amountOut) {
    ISyncSwapRouter.SwapPath memory pathFirst = paths[0];
    // ISyncSwapRouter.SwapStep memory stepFirst = pathFirst.steps[0];
    ISyncSwapRouter.SwapStep memory stepLast = pathFirst.steps[pathFirst.steps.length - 1];

    // for Testnet : use to as tokenOut
    (address tokenout, address to, ) = abi.decode(stepLast.data, (address, address, uint8));
    uint256 amountIn = pathFirst.amountIn;
    address[] memory path = new address[](2);
    path[0] = pathFirst.tokenIn;
    path[1] = tokenout;
    uint256[] memory amounts = swapExactTokensForTokens(amountIn, amountOutMin, path, msg.sender, deadline);
    return ISyncSwapRouter.TokenAmount(to, amounts[amounts.length - 1]);
  }
