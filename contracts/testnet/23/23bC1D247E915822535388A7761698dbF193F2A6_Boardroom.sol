...
contract Boardroom is ShareWrapper, ContractGuard {
  ...
  function stakeFor(address recipient, uint256 amount) public {
    require(amount > 0, "Boardroom: Cannot stake 0");
    super.stakefor(msg.sender, recipient, amount);
    members[msg.sender].epochTimerStart = treasury.epoch(); // reset timer
  ...
  function useWildcard(uint ID) external onlyOneBlock {
    require(IERC721(WildCard).balanceOf(msg.sender) > 0, "Not enough tokens");
    require(
      IERC721(WildCard).isApprovedForAll(msg.sender, address(this)) == true,
      "Approval Required"
    );
    require(
      IERC721(WildCard).ownerOf(ID) == msg.sender,
      "You do not own the token"
    );

    IERC721(WildCard).transferFrom(msg.sender, address(this), ID);

    members[msg.sender].epochTimerStart.sub(withdrawLockupEpochs);
  }

  function useMysteryBox() external {}
}
