...
contract MarginManager is Ownable {
    ...
    /**
        Good
     */
    function whitelistAsset(ERC20 token, AggregatorV3Interface oracle) public onlyOwner returns(ERC20) {
...
contract AoriLens {
    ...
    {
        return (ask.OPTION(), ask.USDCPerOPTION(), ask.endingTime(), ask.getCurrentBalance());
    }
    //Get specific information about a certain ask
    ...
    {
        return (bid.USDC(), bid.OPTIONPerUSDC(), bid.endingTime(), bid.getCurrentBalance());
    }
...
contract AoriPut is ERC20, ReentrancyGuard {
    ...
    function sellerSettlementITM() public nonReentrant returns (uint256) {
        ...
        uint256 USDCToReceive = (optionsToSettle * settlementPrice) / (10**decimals()); //((1e18 * 1e6) / 1e18
        //store the settlement
        optionSellers[msg.sender] = 0;
    ...
    function sellerSettlementOTM() public nonReentrant returns (uint256) {
        ...
        //settle
        uint256 USDCOwed = mulDiv(optionsSold, strikeInUSDC, decimals()); //((1e18 / 1e6) / 1e18
        USDC.transfer(msg.sender, USDCOwed);
...
contract Orderbook is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => Bid[]) bidsAtPrice;
    ...
    function createAsk(uint256 _USDCPerOPTION, uint256 _OPTIONSize) public returns (Ask) {
        Ask ask = new Ask(OPTION, USDC, AORISEATSADD, msg.sender, _USDCPerOPTION, fee_, AoriSeats(AORISEATSADD).getFeeMultiplier(), _OPTIONSize);
        asks.push(ask);
        //transfer before storing the results
        OPTION.transferFrom(msg.sender, address(ask), _OPTIONSize);
        //storage
        isAsk[address(ask)] = true;
        asksAtPrice[_USDCPerOPTION].push(ask); 
        ask.fundContract();
    ...
    function createBid(uint256 _OPTIONPerUSDC, uint256 _USDCSize) public returns (Bid) {
        Bid bid = new Bid(USDC, OPTION, AORISEATSADD, msg.sender, _OPTIONPerUSDC, fee_, AoriSeats(AORISEATSADD).getFeeMultiplier(), _USDCSize);
        bids.push(bid);
        //transfer before storing the results
    ...
    function fillAsks(address receiver, uint256 _USDCPerOPTION, uint256 USDCSize, uint256 seatId) external nonReentrant{
        require(USDCSize > 0);
        USDC.transferFrom(msg.sender, address(this), USDCSize);
        uint256 amountToFill = USDCSize;
        uint256 range = asks.length - filledAskIndex[_USDCPerOPTION];
        ...
        for (uint256 i; i < range; i++) {
            ...
            if(bal <= amountToFill) {
                ...
                ERC20(address(USDC)).decreaseAllowance(address(asks[filledAskIndex[_USDCPerOPTION]]), USDC.allowance(address(this), address(asks[filledAskIndex[_USDCPerOPTION]])));
                filledAskIndex[_USDCPerOPTION]++;
            } else {
                USDC.approve(address(asks[filledAskIndex[_USDCPerOPTION]]), bal);
    ...
    function fillBids(address receiver, uint256 _OPTIONPerUSDC, uint256 OPTIONSize, uint256 seatId) external nonReentrant {
        require(OPTIONSize > 0);
        OPTION.transferFrom(msg.sender, address(this), OPTIONSize);
        uint256 amountToFill = OPTIONSize;
        uint256 range = bids.length - filledBidIndex[_OPTIONPerUSDC];
        ...
        for (uint256 i; i < range; i++) {
            ...
            if(bal <= amountToFill) {
                ...
                bids[filledBidIndex[_OPTIONPerUSDC]].fill(receiver, bal, seatId);
                ERC20(address(OPTION)).decreaseAllowance(address(bids[filledBidIndex[_OPTIONPerUSDC]]), OPTION.allowance(address(this), address(bids[filledBidIndex[_OPTIONPerUSDC]])));
                filledBidIndex[_OPTIONPerUSDC]++;
            } else {
                OPTION.approve(address(bids[filledBidIndex[_OPTIONPerUSDC]]), bal);
                bids[filledBidIndex[_OPTIONPerUSDC]].fill(receiver, amountToFill, seatId);
                amountToFill = 0;
                ERC20(address(OPTION)).decreaseAllowance(address(bids[filledBidIndex[_OPTIONPerUSDC]]), OPTION.allowance(address(this), address(bids[filledBidIndex[_OPTIONPerUSDC]])));
            }
            if(amountToFill == 0) {
...
contract CallFactory is Ownable {
    ...
    */
    function createCallMarket(
        ...
        AoriCall callMarket = new AoriCall(address(manager), AoriSeats(AORISEATSADD).getFeeMultiplier(), strikeInUSDC, endingTime, UNDERLYING, USDC, oracle, AoriSeats(AORISEATSADD), name_, symbol_);
...
contract PutFactory is Ownable {
    ...
    */
    ...
    function createPutMarket(
        ...
        AoriPut putMarket = new AoriPut(address(manager), AoriSeats(AORISEATSADD).getFeeMultiplier(), strikeInUSDC, endingTime, USDC, UNDERLYING, oracle, AoriSeats(AORISEATSADD), name_, symbol_);

        isListed[address(putMarket)] = true;
        putMarkets.push(putMarket);

        emit AoriPutCreated(ERC20(address(putMarket)), strikeInUSDC, endingTime, USDC, oracle, name_, symbol_);
        return (putMarket);
    }
...
contract OrderbookFactory is Ownable {
    ...
    */
    function createOrderbook(
        ...
        Orderbook orderbook =  new Orderbook(getTradingFee(), OPTION_, USDC, AORISEATSADD, endingTime); 
...
contract Bid is ReentrancyGuard {
    ...
    event OfferFunded(address maker, uint256 USDCSize);
    ...
    function fundContract() public nonReentrant {
        ...
        hasBeenFunded = true;
        emit OfferFunded(maker, USDCSize);
    }
    /**
    ...
    */
    function fill(address receiver, uint256 amountOfOPTION, uint256 seatId) public nonReentrant {
        require(isFunded(), "no usdc balance");
        require(msg.sender != maker && receiver != maker, "Cannot take one's own order");
        ...
        if(receiver == AORISEATSADD.ownerOf(seatId)) {
            ...
            USDC.transfer(receiver, USDCToReceive);
        } else {
            ...
            USDC.transfer(receiver, USDCToReceive - (ownerTxFee + seatTxFee));
    ...
    */
    ...
    return (OPTIONPerUSDC * USDC.balanceOf(address(this))) / 10**USDCDecimals;
    }
}
...
contract Ask is ReentrancyGuard {
    ...
    event OfferFunded(address maker, uint256 OPTIONSize);
    ...
    function fundContract() public nonReentrant {
        ...
        hasBeenFunded = true;
        //officially begin the countdown
        emit OfferFunded(maker, OPTIONSize);
    }
    /**
        Partial or complete fill of the offer, with the requirement of trading through a seat
        regardless of whether the seat is owned or not.
        In the case of not owning the seat, a fee is charged in USDC.
     */
    function fill(address receiver, uint256 amountOfUSDC, uint256 seatId) public nonReentrant {
   ...
   function totalUSDCWanted() public view returns (uint256) {
    ...
    return (USDCPerOPTION * OPTION.balanceOf(address(this))) / 10**OPTIONDecimals;
    }
}
