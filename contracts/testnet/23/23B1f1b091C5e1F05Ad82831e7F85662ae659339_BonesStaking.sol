...
contract BonesStaking is BonesStakingState, ContractControl{
    ...
    modifier contractsAreSet() {
        require(address(neanderSmols) != address(0) && bones != address(0) && neanderStaking != address(0) && treasury != address(0),
    ...
    function stakeBones(uint256 _amount, uint256 _tokenId) external contractsAreSet whenNotPaused{
        ...
        require(IBones(bones).getBalanceOf(msg.sender) >= _amount, "Not enough balance");
        ...
        IBones(bones)._stake(msg.sender, address(this), _amount);
    }

    function unstakeBones(uint256 _startTime, uint256 _tokenId) external contractsAreSet whenNotPaused {
        ...
        IBones(bones)._unStake(address(this), msg.sender, amount);
    }
    
    function removeStake(uint256 _startTime, uint256 _tokenId) internal returns(uint256){
    ...
    function claimRewards(uint256 _tokenId) external contractsAreSet whenNotPaused {
    ...
    function _getRewards(StakeDetails storage current) internal returns(uint256){
    ...
    function getStakes(address _user) public view returns (StakeDetails[] memory) {
        return userToStakeDetails[_user];
    }

    function boostRewards(uint256 _amount, uint256 _tokenId) external contractsAreSet {
        require(IBones(bones).getBalanceOf(msg.sender) >= _amount, "Not enough balance");
        require(_amount == boostPayment, "Wrong amount sent");

        IBones(bones)._stake(msg.sender, treasury, _amount);
...
abstract contract BonesStakingState {
    ...
    uint256 public minStakeAmount = 2500 * 10**18;
    uint256 public reward = 250000000;

    uint256 public boostPayment = 5000 * 10**18;
    uint256 public boostAmount = 1 * 10**9;


    /*
     *  FRONTEND: Use the following to display a users stakes. 
     *  Make sure the staked are displayed in the same order they are in the array.
     */
...
interface IBones {
    ...
    function _stake(address from, address to, uint256 amount) external returns(bool);
    function _unStake(address from, address to, uint256 amount) external returns(bool);
}
...
abstract contract ContractControl is AccessControl {
    ...
    modifier onlyOwner() {
        require(isOwner(_msgSender()), "MinterControl: not a SMOLNEANDER_OWNER_ROLE");
        _;
    }

    modifier onlyStakingContract() {
        require(isStakingContract(_msgSender()), "MinterControl: not a SMOLNEANDER_CONTRACT_ROLE");
        _;
    }

    modifier onlyMinter {
        require(isMinter(_msgSender()), "MinterControl: not a SMOLNEANDER_MINTER_ROLE");
        _;
    }
