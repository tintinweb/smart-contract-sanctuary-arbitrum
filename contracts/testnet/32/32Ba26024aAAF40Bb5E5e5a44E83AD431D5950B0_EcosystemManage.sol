...
contract EcosystemManage is Initializable{
    ...
    event RandomTriggered(address a, uint rewardAmount, uint pexAmount, uint esPexAmount);
    ...
    function randomTrigger(uint _pexBuyMinAmount, uint24 _pexBuyFee) external onlyWhiteList {
        ...
        if(stakePlpReward > 0){
            ...
            EsPEX.convertEsPEX(address(this) ,pexAmount);
            ...
            emit RandomTriggered(msg.sender, stakePlpReward, pexAmount, esPexAmount);

            previousStakePlpReward = stakePlpReward;
            delete stakePlpReward;
        } else {
            emit RandomTriggered(msg.sender, 0, 0, 0);
        }
    }
