...
contract TekkenWager is Ownable {
    ...
    function createPendingGame(address _opponent, uint256 _wagerAmount) external isPlayerInGame checkPaused {
        gameIdCounter++;
        require(_wagerAmount >= minimumDMT && _wagerAmount <= maximumDMT, "Wager amount is not within the allowed range");
    ...
    function cancelGame() external {
        ...
        if (game.hasStarted == false) {
            ...
            require(block.timestamp > game.startTime + 1 hours, "Game must have been started for at least 1 hour");
    ...
    function decideGame(uint256 _gameId, address _winner) external onlyOwner {
        ...
        require(_winner == game.creator || _winner == game.opponent || _winner == address(0), "Winner must be one of the players or address(0)");
        ...
        if (_winner == address(0)) {
            ...
            // winner 
            uint256 payout = game.wagerAmount;
            if (feePercentage > 0) {
    ...
    function payWager(uint _wagerAmount) internal {
