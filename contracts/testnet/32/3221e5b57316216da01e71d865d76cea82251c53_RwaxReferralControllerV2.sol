...
import {IRwaxReferralRegistry} from "../interfaces/IRwaxReferralRegistry.sol";
import {IRWAXOracle} from "../interfaces/IRWAXOracle.sol";

contract RwaxReferralControllerV2 is Initializable, OwnableUpgradeable {
    ...
    IRwaxReferralRegistry public referralRegistry;
    ...
    function initialize(address _rwax, address _oracle, address _rwaxRegistry, uint256 _epochDuration)
        external
        initializer
    {
        require(_rwax != address(0), "RwaxReferralController::initialize: invalid address");
        require(_oracle != address(0), "RwaxReferralController::initialize: invalid address");
        require(_rwaxRegistry != address(0), "RwaxReferralController::initialize: invalid address");
        require(_epochDuration >= MIN_EPOCH_DURATION, "RwaxReferralController::initialize: must >= MIN_EPOCH_DURATION");
        __Ownable_init();
        RWAX = IERC20(_rwax);
        oracle = IRWAXOracle(_oracle);
        referralRegistry = IRwaxReferralRegistry(_rwaxRegistry);
    ...
    function updatePoint(address _trader, uint256 _point) external {
        require(msg.sender == updater, "RwaxReferralController::updatePoint: !updater");
        require(_trader != address(0), "RwaxReferralController::updatePoint: invalid address");
    ...
    function claim(uint256 _epoch, address _to) external {
        require(_epoch < currentEpoch, "RwaxReferralController::claim: !epoch");
        uint256 reward = claimable(_epoch, msg.sender);
        require(reward != 0, "RwaxReferralController::claim: !reward");

        UserInfo storage user = users[_epoch][msg.sender];
        user.claimed += reward;

        RWAX.safeTransfer(_to, reward);

        emit Claimed(_epoch, _to, reward);
    }

    function nextEpoch() external {
        require(enableNextEpoch, "RwaxReferralController::nextEpoch: !enableNextEpoch");
        (uint256 nextEpochTimestamp, uint256 _vestingDuration) = getNextEpoch();
        require(block.timestamp >= nextEpochTimestamp, "RwaxReferralController::nextEpoch: now < trigger time");
    ...
    function setDistributor(address _distributor) external onlyOwner {
        require(_distributor != address(0), "RwaxReferralController::setDistributor: invalid address");
        distributor = _distributor;
        emit DistributorSet(distributor);
    }

    function setUpdater(address _updater) external onlyOwner {
        require(_updater != address(0), "RwaxReferralController::setUpdater: invalid address");
        updater = _updater;
    ...
    function setOracle(address _oracle) external onlyOwner {
        require(
            _oracle != address(0) && _oracle != address(oracle), "RwaxReferralController::setOracle: invalid address"
        );
        oracle = IRWAXOracle(_oracle);
        oracle.update();
    ...
    function setEpochDuration(uint256 _epochDuration) public onlyOwner {
        require(
            ...
            "RwaxReferralController::setEpochDuration: must >= MIN_EPOCH_DURATION"
        );
        epochDuration = _epochDuration;
        emit EpochDurationSet(epochDuration);
    }

    function withdrawRWAX(address _to, uint256 _amount) external onlyOwner {
        require(_to != address(0), "RwaxReferralController::withdrawRWAX: invalid address");
        RWAX.safeTransfer(_to, _amount);

        emit RWAXWithdrawn(_to, _amount);
    }

    function setEnableNextEpoch(bool _enable) external {
        require(msg.sender == distributor, "RwaxReferralController::setEnableNextEpoch: !distributor");
    ...
    event RWAXWithdrawn(address indexed to, uint256 amount);
...
interface IRwaxReferralRegistry {
