...
contract Agent is IAgent, BlockContext, OwnerPausable, AgentStorage {
    ...
    function depositAndOpenPositionFor(
        ...
        require(deposit >= _txFee, "A_IANF");
        //deposit
        _deposit(nonce, account, deposit);
        _vaultDeposit(nonce, account, baseToken, deposit.sub(_txFee));
        //open position
        IUserAccount(account).openPosition(_clearingHouse, baseToken, isBaseToQuote, quote);
        // withdraw all
    ...
    function openPositionFor(
        ...
        _chargeFee(nonce, account, _txFee);
        //deposit
        _vaultDeposit(nonce, account, baseToken, collateral);
        //open position
        IUserAccount(account).openPosition(_clearingHouse, baseToken, isBaseToQuote, quote);
        // withdraw all
        _vaultWithdrawAll(nonce, account, baseToken);
