	...
	ReentrancyGuardUpgradeable
{
	...
	function initialize(
		...
		pitAtomicRepayment = IPrimaryIndexTokenAtomicRepaymentV3(_pitAtomicRepayment);
	...
	function setPrimaryIndexTokenAtomicAddress(
		...
		require(_newPrimaryIndexTokenAtomic != address(0), "PITLeverage: invalid address");
		emit SetPrimaryIndexTokenAtomic(address(pitAtomicRepayment), _newPrimaryIndexTokenAtomic);
		pitAtomicRepayment = IPrimaryIndexTokenAtomicRepaymentV3(_newPrimaryIndexTokenAtomic);
	}
	...
	function calculateMargin(
		...
		(uint256 lvrNumerator, uint256 lvrDenominator) = primaryIndexToken.getLoanToValueRatio(
			projectToken,
			lendingToken
		);
		uint margin = ((expAmount *
			(lvrDenominator *
				...
				lvrNumerator *
				safetyMarginDenominator)) / (lvrNumerator * safetyMarginDenominator));
		marginAmount =
	...
	function calculateSafetyMargin(
		...
		uint margin,
		uint exp
	) public view returns (uint safetyMarginNumerator, uint safetyMarginDenominator) {
		(uint256 lvrNumerator, uint256 lvrDenominator) = primaryIndexToken.getLoanToValueRatio(
			projectToken,
			lendingToken
		);
	...
	function _deferLiquidityCheck(
		...
		(uint totalPit, uint totalWeightedLoanInUSD) = primaryIndexToken.healthFactor(user);
		require(
			totalWeightedLoanInUSD <= totalPit,
			"PITLeverage: lendingTokenAmount exceeds pit remaining"
		);
		uint newTotalBorrowPerLendingToken = primaryIndexToken.getBorrowedPerLendingTokenInUSD(
			lendingToken
		);
		uint borrowLimitPerLendingToken = primaryIndexToken.borrowLimitPerLendingToken(
			lendingToken
		);
		uint256 newTotalDepositPerProjectToken = primaryIndexToken.getDepositedPerProjectTokenInUSD(
			projectToken
		);
		uint256 depositLimitPerProjectToken = primaryIndexToken.depositLimitPerProjectToken(
			projectToken
		);
		...
		require(
			...
			"PITLeverage: totalDeposit exceeded depositLimit per collateral asset"
		);
	}
	 ...
	 */
	function _nakedBorrow(address user, address lendingToken, uint lendingTokenAmount) internal {
		primaryIndexToken.updateInterestInAllBorrowPositions(user);

		primaryIndexToken.calcBorrowPosition(user, lendingToken, lendingTokenAmount);
	...
	function calculateAddingAmount(
		...
		uint depositedAmount = primaryIndexToken.depositedAmount(user, projectToken);
	...
	function _checkIsValidPosition(
		...
		IPrimaryIndexTokenV3.Ratio memory lvrProjectToken = primaryIndexToken
			...
			.loanToValueRatio;
		uint256 collaterallPIT = (notionalExposure * lvrProjectToken.numerator) /
	...
	/**
	 * @notice Reduce the amount of the long asset corresponding the new ratio position data when the deposited amount of the long asset is reduced.
	 * @param account The address of user wallet.
	...
	function reduceDepositedLongAsset(
		...
		uint256 remainingDepositedCount = primaryIndexToken.depositedAmount(account, projectToken);
		if (remainingDepositedCount == 0 && !isWithdraw) return;
		...
		if (remainingDepositedCount < totalLongAssetCountOfProjectToken) {
			...
			for (uint256 i = 0; i < _positionDatas.length; i++) {
				...
				if (_positionData.longAsset == projectToken) {
					if (remainingDepositedCount == 0) {
						deleteLeveragePosition(account, _positionData.shortAsset, i);
					} else {
	...
	/**
	 * @notice  Increase the amount of the long asset corresponding the new ratio position data when the deposited amount of the long asset is increase.
	 * But not exceed the max long asset amount of the initial position
	 * @param account The address of user wallet.
	 * @param projectToken The address of long asset.
	 */
	function addDepositedLongAsset(
		...
		uint256 maxLongAssetCountOfProjectToken = maxTotalLongAssetCount[account][projectToken];
	...
	/**
	 * @notice Reduce the amount of the short asset corresponding the new ratio position data when the loan body amount of the short asset is reduced.
	 * @param account The address of user wallet.
	 * @param lendingToken The address of short asset.
	 */
	function reduceShortAsset(address account, address lendingToken) external isPrimaryIndexToken {
		IPrimaryIndexTokenV3.BorrowPosition memory borrowPosition = primaryIndexToken
			.borrowPosition(account, lendingToken);
		uint256 totalBorrowCount = borrowPosition.loanBody + borrowPosition.accrual;
		uint256 totalShortAssetCountOfLendingToken = currentShortAssetCount[account][lendingToken];
			...
			for (uint256 i = 0; i < _positionDatas.length; i++) {
				...
				if (_positionData.shortAsset == lendingToken) {
					...
					if (totalBorrowCount == 0) {
						deleteLeveragePosition(account, lendingToken, i);
					}
				}
			}
		}
	...
	/**
	 * @notice Increase the amount of the short asset corresponding the new ratio position data when the loan body amount of the short asset is increase.
	 * But not exceed the max short asset amount of the initial positions
	 * @param account The address of user wallet.
	 * @param lendingToken The address of short asset.
	 */
	function addShortAsset(address account, address lendingToken) external isPrimaryIndexToken {
		IPrimaryIndexTokenV3.BorrowPosition memory borrowPosition = primaryIndexToken
			.borrowPosition(account, lendingToken);
		uint256 totalBorrowCount = borrowPosition.loanBody + borrowPosition.accrual;
		...
		uint256 maxShortAssetCountOfLendingToken = maxTotalShortAssetCount[account][lendingToken];

		if (currentShortAssetCountOfLendingToken == maxShortAssetCountOfLendingToken) return;
	...
	/**
	 * @notice Check whether a position is open with leverage borrow or not
	 * @param account The address of user wallet.
	 * @param projectToken The address of long asset.
	 * @param lendingToken The address of short asset.
	 */
	...
	/**
	 * @notice Deletes a leverage position for a user and project token at position index.
	 * @param user The address of the user.
	 * @param lendingToken The address of the short token.
	 * @param positionIndex The id of the opened position.
	 */
	function deleteLeveragePosition(
		...
		PositionData storage _positionData = positionData[user][positionIndex];
		if (
			_positionData.shortAsset == lendingToken &&
			(_positionData.shortCount == 0 || _positionData.longCount == 0)
		) {
			if (_positionData.shortCount == 0) {
				currentTotalLongAssetCount[msg.sender][_positionData.longAsset] -= _positionData
					.longCount;
				maxTotalLongAssetCount[msg.sender][
					_positionData.longAsset
				] = currentTotalLongAssetCount[msg.sender][_positionData.longAsset];
			} else {
				currentShortAssetCount[msg.sender][lendingToken] -= _positionData.shortCount;
				maxTotalShortAssetCount[msg.sender][lendingToken] = currentShortAssetCount[
					msg.sender
				][lendingToken];
			}
			PositionData[] storage positionDataOfUser = positionData[user];
			positionDataOfUser[positionIndex] = positionDataOfUser[positionDataOfUser.length - 1];
			positionDataOfUser.pop();
		}
	}
	 ...
	 * @param isFully If repay fully for the position, this value is true, else false.
	 */
	function closePositionByLongAsset(
		address projectToken, //any deposited project token
		address lendingToken,
		uint256 positionIndex,
		uint256 collateralAmount,
		bytes memory buyCalldata,
		...
		//get lending (loan body) USD convert to collateral amount to assign collateralAmount and generate buyCalldata
		PositionData[] storage _positionDataOfUser = positionData[msg.sender];
		uint256 shortAmount;
		if (_positionDataOfUser.length == 0) return;
		PositionData storage _positionData = _positionDataOfUser[positionIndex];
		if (_positionData.shortAsset == lendingToken) {
			currentShortAssetCount[msg.sender][lendingToken] -= _positionData.shortCount;
			maxTotalShortAssetCount[msg.sender][lendingToken] = currentShortAssetCount[msg.sender][
				lendingToken
			];
			shortAmount = _positionData.shortCount;
			_positionData.shortCount = 0;
		}
		uint256 amountReceivedLendingToken = pitAtomicRepayment.repayAtomicFromRelatedContract(
			msg.sender,
			lendingToken,
			projectToken,
			collateralAmount,
		...
		if (isFully) {
			require(
				amountReceivedLendingToken >= shortAmount,
				"Leverage: Remaining deposited of collateral not enough to close position"
			);
		}
		if (amountReceivedLendingToken >= shortAmount) {
			deleteLeveragePosition(msg.sender, lendingToken, positionIndex);
		} else {
			currentShortAssetCount[msg.sender][lendingToken] +=
				shortAmount -
				amountReceivedLendingToken;
			maxTotalShortAssetCount[msg.sender][lendingToken] = currentShortAssetCount[msg.sender][
				lendingToken
			];
			_positionData.shortCount = shortAmount - amountReceivedLendingToken;
		}
	}
	 ...
	 */
	function closePositionByShortAsset(
		uint256 positionIndex,
		address lendingToken,
		uint256 lendingTokenAmount
	) external {
		PositionData[] storage _positionDatas = positionData[msg.sender];
		uint256 shortAmount;
		if (_positionDatas.length == 0) return;
		PositionData storage _positionData = _positionDatas[positionIndex];

		if (_positionData.shortAsset == lendingToken) {
			require(
				lendingTokenAmount <= _positionData.shortCount,
				"Leverage: Exceed amoun to repay"
			);
			currentShortAssetCount[msg.sender][lendingToken] -= _positionData.shortCount;
			maxTotalShortAssetCount[msg.sender][lendingToken] = currentShortAssetCount[msg.sender][
				lendingToken
		...
		if (lendingTokenAmount == shortAmount) {
			deleteLeveragePosition(msg.sender, lendingToken, positionIndex);
		} else {
			currentShortAssetCount[msg.sender][lendingToken] += shortAmount - lendingTokenAmount;
			maxTotalShortAssetCount[msg.sender][lendingToken] = currentShortAssetCount[msg.sender][
				lendingToken
			];
			_positionData.shortCount = shortAmount - lendingTokenAmount;
		}
	}
}