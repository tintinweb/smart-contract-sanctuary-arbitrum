...
contract Roulette is Core {
  ...
  uint8[36] public chunkMultipliers;
  ...
  constructor(IRandomizerRouter _router) Core(_router) {
    ...
    chunkMaps[0] = [0,1];
    // for 1
    chunkMaps[1] = [0,2,24,27,30,31,35];
    // for 2
    chunkMaps[2] = [0,1,2,3,24,27,29,32,34];
    // for 3
    chunkMaps[3] = [1,3,24,27,30,31,33];
    // for 4
    chunkMaps[4] = [2,4,24,27,29,32,35];
    // for 5
    chunkMaps[5] = [2,3,4,5,24,27,30,31,34];
    // for 6
    chunkMaps[6] = [3,5,24,27,29,32,33];
    // for 7
    chunkMaps[7] = [4,6,24,27,30,31,35];
    // for 8
    chunkMaps[8] = [4,5,6,7,24,27,29,32,34];
    // for 9
    chunkMaps[9] = [5,7,24,27,30,33];
    // for 10
    chunkMaps[10] = [6,9,24,27,29,32,35];
    // for 11
    chunkMaps[11] = [6,7,8,9,24,27,30,32,34];
    // for 12
    chunkMaps[12] = [7,8,24,27,29,31,33];
    // for 13
    chunkMaps[13] = [9,11,25,27,30,32,35];
    // for 14
    chunkMaps[14] = [8,9,10,11,25,27,29,31,34];
    // for 15
    chunkMaps[15] = [8,10,25,27,30,32,33];
    // for 16
    chunkMaps[16] = [11,13,25,27,29,31,35];
    // for 17
    chunkMaps[17] = [10,11,12,13,25,27,30,32,34];
    // for 18
    chunkMaps[18] = [10,12,25,27,29,31,33];
    // for 19
    chunkMaps[19] = [13,15,25,28,30,31,35];
    // for 20
    chunkMaps[20] = [12,13,14,15,25,28,29,32,34];
    // for 21
    chunkMaps[21] = [12,14,25,28,30,31,33];
    // for 22
    chunkMaps[22] = [15,17,25,28,29,32,35];
    // for 23
    chunkMaps[23] = [14,16,17,25,28,30,31,34];
    // for 24
    chunkMaps[24] = [14,16,25,28,29,32,33];
    // for 25
    chunkMaps[25] = [17,19,26,28,30,31,35];
    // for 26
    chunkMaps[26] = [16,17,18,19,26,28,29,32,34];
    // for 27
    chunkMaps[27] = [16,18,26,28,30,31,33];
    // for 28
    chunkMaps[28] = [19,21,26,28,29,32,35];
    // for 29
    chunkMaps[29] = [18,19,20,21,26,28,30,32,34];
    // for 30
    chunkMaps[30] = [18,20,26,28,29,31,33];
    // for 31
    chunkMaps[31] = [21,23,26,28,30,32,35];
    // for 32
    chunkMaps[32] = [20,21,22,23,26,28,29,31,34];
    // for 33
    chunkMaps[33] = [20,22,26,28,30,32,33];
    // for 34
    chunkMaps[34] = [15,23,26,28,29,31,35];
    // for 35
    chunkMaps[35] = [22,23,26,28,30,32,34];
    // for 36
    chunkMaps[36] = [22,26,28,28,29,31,33];
  }
  ...
  function randomizerFulfill(
    ...
    uint8[36] memory chunkMultipliers_ = chunkMultipliers;
    uint32 payout_;
    ...
    vaultManager.setReferralReward(game_.token, game_.player, game_.totalWagerInToken, getHouseEdge());
  ...
  function bet(
    uint8[73] calldata _selections,
    uint8 _count,
    address _token
  ) external whenNotPaused nonReentrant {
    Configuration memory configuration_ = configuration;

    require(_count <= configuration_.maxCount, "GAME: Count is invalid!");
    ...
    uint128 totalWagerInToken_ = uint128(_count * chip2Token(totalWager_, _token, price_));
