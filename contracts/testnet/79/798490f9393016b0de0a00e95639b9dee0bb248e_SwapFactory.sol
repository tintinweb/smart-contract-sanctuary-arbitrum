...
contract SwapFactory is ISwapFactory, ReentrancyGuard {
    ...
    function createPair(
        ...
        ISwapPair(pair).initialize(token0, token1, supportsTokenFee, tokenB == token0);
    ...
    function setFeeToSetter(address _feeToSetter) external {
        require(msg.sender == feeToSetter, "Swap: FORBIDDEN");
        require(_feeToSetter != address(0), "Swap: INVALID_ADDRESS");
...
contract SwapPair is ISwapPair, SwapERC20, ReentrancyGuard {
    ...
    function initialize(address _token0, address _token1, bool _supportsTokenFee, bool _token0IsFee) external override {
        require(msg.sender == factory, "Swap: FORBIDDEN"); // sufficient check
        require(!pairInit, "Swap: INITIALIZED_ALREADY");
