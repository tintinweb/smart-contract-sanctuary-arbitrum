...
contract KotEGearBurn is Initializable, KotEGearBurnContracts {
    ...
    function mintPermit(uint256[] calldata _ids) public whenNotPaused contractsAreSet onlyEOA {
        require(amountBurnt + _ids.length <= maxBurns, "Max burns reached");
        require(_ids.length > 0, "Non-zero length required");

        amountBurnt += _ids.length;

        for(uint256 i = 0; i < _ids.length; i++) {
            _mintPermit(_ids[i]);
        }

        emit PermitsMinted(msg.sender, _ids.length);
    }

    function _mintPermit(uint256 _id) private {
        GearRarity _rarity = gearIdToRarity[_id];

        require(_rarity == GearRarity.Epic || _rarity == GearRarity.Legendary, "Only epic and legendary gear can be burnt");

        uint256 _burnAmount = rarityToBurnAmounts[_rarity];

        knightGear.burn(msg.sender, _id, _burnAmount);
