...
uint256 constant ID = uint256(
    keccak256("game.piratenation.tokenidcomponent.v2")
);
...
library TokenIdComponentStorage {
...
/**
 * @title TokenIdComponent
 * @dev An entity&#39;s contract address and token ID
 */
contract TokenIdComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Contract address of the entity token
        keys[0] = "token_address";
        values[0] = TypesLibrary.SchemaValue.ADDRESS;

        // ID of the entity token
        keys[1] = "token_id";
    ...
    function getValue(
        ...
        value = TokenIdComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = TokenIdComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.tokenAddress);
        values[1] = abi.encode(s.tokenId);
    }
    ...
    function getBytes(
        ...
        Layout memory s = TokenIdComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.tokenAddress, s.tokenId) = abi.decode(value, (address, uint256));
    ...
    function remove(
        ...
        delete TokenIdComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete TokenIdComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.tokenAddress = value.tokenAddress;
        s.tokenId = value.tokenId;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.tokenAddress, value.tokenId));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.tokenAddress, value.tokenId);
    }
}