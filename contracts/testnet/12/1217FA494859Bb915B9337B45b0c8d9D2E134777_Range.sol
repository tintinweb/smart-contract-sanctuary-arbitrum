...
contract Range is CommonSolo {
  ...
  function isWon(bool _over, uint32 _choice, uint256 _result) public view returns (bool won_) {
    won_ = _over ? (_result >= 100 && _result <= _choice) : (_result > _choice && _result <= range);
  }
  ...
  function play(
    ...
    for (uint8 i = 0; i < _game.count; ++i) {
      ...
      //_resultNumbers[i] > 100 ? _resultNumbers[i] += 1 : _resultNumbers[i] = 100;
