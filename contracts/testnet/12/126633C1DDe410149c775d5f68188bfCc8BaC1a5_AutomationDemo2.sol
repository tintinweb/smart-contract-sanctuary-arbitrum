...
contract AutomationDemo2 is ILogAutomation, StreamsLookupCompatibleInterface {
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        if(log.source == sourceAddress){
             ...
             int8 direction = abi.decode(p2, (int8));

             bytes memory p3 = abi.encodePacked(log.topics[3]);
             uint256 triggerPrice = abi.decode(p3, (uint256));
            ...
            revert StreamsLookup(
                ...
                log.timestamp,
                abi.encode(id, direction, triggerPrice)
            );
        }
    }
    ...
    {
        (uint256 id, int8 direction, uint256 triggerPrice) = abi.decode(extraData, (uint256, int8, uint256));

        BasicReport memory unverifiedReport = _getReportData(values[0]);
        uint256 tradePrice;
        int192  indexPrice = unverifiedReport.benchmark;
        if(indexPrice > 0){
        ...
        bool upkeepNeeded = direction > 0 ? tradePrice >= triggerPrice : tradePrice <= triggerPrice;
        return (upkeepNeeded, abi.encode(values, extraData));
    }
