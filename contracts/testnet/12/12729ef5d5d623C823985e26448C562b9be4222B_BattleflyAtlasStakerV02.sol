    ...
    ReentrancyGuardUpgradeable
{
        ...
        returns (uint256 amount)
    {
        ...
        emit WithdrawPosition(_msgSender(), amount, _depositId);
    }
    ...
    {
        amount = _claim(_depositId);
        emit ClaimEmission(_msgSender(), amount, _depositId);
    }
        ...
        returns (uint64)
    {
        ...
        emit RequestWithdrawal(_msgSender(), vaultStake.retentionUnlock, _depositId);
    ...
    function stakeTreasure(uint256 _tokenId, uint256 _amount) external onlySuperAdmin nonReentrant {
        ...
        getTREASURE().safeTransferFrom(_msgSender(), address(this), _tokenId, _amount, "");
        treasureStakers[_tokenId][_msgSender()] += _amount;

        // Token Approval is already done in constructor
        ATLAS_MINE.stakeTreasure(_tokenId, _amount);
        emit StakedTreasure(_msgSender(), _tokenId, _amount);
    }
    ...
    function unstakeTreasure(uint256 _tokenId, uint256 _amount) external onlySuperAdmin nonReentrant {
        require(_amount > 0, "BattleflyAtlasStaker: Invalid TREASURE amount");
        require(treasureStakers[_tokenId][_msgSender()] >= _amount, "BattleflyAtlasStaker: Invalid TREASURE amount");
        // Unstake TREASURE from AtlasMine
        ATLAS_MINE.unstakeTreasure(_tokenId, _amount);

        // Transfer TREASURE to the staker
        getTREASURE().safeTransferFrom(address(this), _msgSender(), _tokenId, _amount, "");
        treasureStakers[_tokenId][_msgSender()] -= _amount;
        emit UnstakedTreasure(_msgSender(), _tokenId, _amount);
    }
    ...
    function stakeLegion(uint256 _tokenId) external onlySuperAdmin nonReentrant {
        ...
        getLEGION().safeTransferFrom(_msgSender(), address(this), _tokenId, "");
        legionStakers[_tokenId] = _msgSender();

        // Token Approval is already done in constructor
        ATLAS_MINE.stakeLegion(_tokenId);
        emit StakedLegion(_msgSender(), _tokenId);
    }
    ...
    function unstakeLegion(uint256 _tokenId) external onlySuperAdmin nonReentrant {
        require(legionStakers[_tokenId] == _msgSender(), "BattleflyAtlasStaker: Invalid staker");
        // Unstake LEGION from AtlasMine
        ATLAS_MINE.unstakeLegion(_tokenId);

        // Transfer LEGION to the staker
        getLEGION().safeTransferFrom(address(this), _msgSender(), _tokenId, "");
        legionStakers[_tokenId] = address(0);
        emit UnstakedLegion(_msgSender(), _tokenId);
    }
    ...
    function _deposit(
        ...
        vaultStakes[_depositId] = VaultStake(lockAt, unlockAt, 0, _amount, 0, _msgSender(), _lock);
        VaultStake storage vaultStake = vaultStakes[_depositId];
    ...
    modifier onlySuperAdmin() {
        require(superAdmins[_msgSender()], "BattleflyAtlasStaker: caller is not a super admin");
        _;
    }

    modifier onlyWhitelistedVaults() {
        require(vaults[_msgSender()].enabled, "BattleflyAtlasStaker: caller is not whitelisted");
        _;
    }
    ...
    modifier onlyBattleflyBot() {
        require(_msgSender() == BATTLEFLY_BOT, "BattleflyAtlasStaker: caller is not a battlefly bot");
        _;
    }
