    ...
    ReentrancyGuard
{
        ...
        )
    {
        ...
        uint256 totalBetsForInterval;

        uint256 intervalForMaxBetAmount = config.intervalForExposureUpdate();
        uint256 multiplier = config.multiplier();

        for (uint256 i; i < timeframes.length; i++) {
            totalBetsForInterval += intervalForMaxBetAmount / timeframes[i].interval;
        }
        ...
        if (maxHourlyExposure > 0) {
            ...
            currentExposureAmount =
                (maxHourlyExposure * multiplier * intervalForMaxBetAmount) /
                100 /
                1 hours /
    ...
    function _claim(
        ...
        emit Claimed(marketName, user, timeframeId, epoch, claimedAmount, isRefund);
    }
