...
contract Skills {
    ...
    function finalizeOrder(uint256 orderId) internal virtual {
        ...
        _payouts[_orders[orderId].creator] += _orders[orderId].totalReturn;
        ...
        while(numberOfWinnersFound < numberOfWinners) {
            ...
            _payouts[_addressLookup[orderId][currentScore][currentAddressIndex]] += _winnerRewardAmount;
     ...
     */
    function claim(address receiver) external virtual {
        require(_payouts[receiver] > 0, "There are no funds to be claimed.");
        IWETH9Ext weth = IWETH9Ext(_wxpAddress);
        _allocatedFunds -= _payouts[receiver];
        uint256 tempAmount = _payouts[receiver]; // to avoid re-entrancy attacks, save to temp value then set to 0.
        _payouts[receiver] = 0;
        weth.withdrawTo(receiver, tempAmount);
    }

    /**
     * @dev Finds the amount of payouts for a particular address
     */
    function lookupRewards(address receiver) external view virtual returns(uint256) {
        return _payouts[receiver];
    }
