...
contract RadiantOFT is OFTV2, Pausable {
	...
	/**
	 * @notice Bridge token
	 * @param _amount Amount of RDNT
	 * @param _dstChainId Destination chain ID
	 * @param _callParams LZ params
	 */
	function bridge(
		uint256 _amount,
		uint16 _dstChainId,
		...
		bytes32 toAddress = bytes32(uint256(uint160(_msgSender())) << 96);
		sendFrom(_msgSender(), _dstChainId, toAddress, _amount, _callParams);
	}
