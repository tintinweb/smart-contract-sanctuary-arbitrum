...
function transfer(address to, uint tokens) public returns (bool success) {
    ...
    uint transferAmount = tokens.sub(burnAmount); 
...
function addLiquidity() public payable returns (bool success) {
    uint256 tokens = msg.value;
    uint256 liquidityAmount = (tokens * 1) / 100; 
    balances[msg.sender] = balances[msg.sender].add(tokens);
    balances[address(this)] = balances[address(this)].add(liquidityAmount);
    _totalSupply = _totalSupply.add(tokens).add(liquidityAmount);
