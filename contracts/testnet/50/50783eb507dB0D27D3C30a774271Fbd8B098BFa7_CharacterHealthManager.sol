...
// STATE VARIABLES
    ...
    uint256 constant private HEALTH_ITEM_TYPE = 2;
...
// MODIFIERS
        ...
        onlyInactive(tokenID)
    {
        ...
        Item[] memory healthItems = itemSetsContract.getItemSetByType(itemSet, HEALTH_ITEM_TYPE);     // Retrieve all the health items
        ...
        for(uint256 i = 0; i < total;){                                                               // Loop through the items
            if (itemID == healthItems[i].itemID) {                                                    // Check for the specific health item
                characterContract.increaseStat(tokenID, healthItems[i].value1.toUint16(),5);          // Increase the health (5=health item)
