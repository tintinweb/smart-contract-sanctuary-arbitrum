...
uint256 constant ID = uint256(
    keccak256("game.piratenation.shopmaxsupplycomponent.v1")
);

struct Layout {
    uint32 displayMaxSupply;
}

library ShopMaxSupplyComponentStorage {
...
/**
 * @title ShopMaxSupplyComponent
 * @dev Temporary component to display max supply until we implement tranches
 */
contract ShopMaxSupplyComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Display maximum number of SKU&#39;s we plan to sell ever
        keys[0] = "display_max_supply";
     ...
     *
     * @param entity Entity to get value for
     * @param displayMaxSupply Display maximum number of SKU&#39;s we plan to sell ever
     */
    function setValue(
        uint256 entity,
        uint32 displayMaxSupply
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(displayMaxSupply));
    }
    ...
    function getLayoutValue(
        ...
        value = ShopMaxSupplyComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return displayMaxSupply Display maximum number of SKU&#39;s we plan to sell ever
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint32 displayMaxSupply) {
        if (has(entity)) {
            Layout memory s = ShopMaxSupplyComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (displayMaxSupply) = abi.decode(_getEncodedValues(s), (uint32));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ShopMaxSupplyComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](1);
        values[0] = abi.encode(s.displayMaxSupply);
    }
    ...
    function setBytes(
        ...
        Layout memory s = ShopMaxSupplyComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.displayMaxSupply) = abi.decode(value, (uint32));
    ...
    function remove(
        ...
        delete ShopMaxSupplyComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete ShopMaxSupplyComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.displayMaxSupply = value.displayMaxSupply;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.displayMaxSupply));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.displayMaxSupply);
    }
}