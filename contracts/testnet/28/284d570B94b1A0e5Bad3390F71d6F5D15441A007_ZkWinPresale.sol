...
function claimTokens() public nonReentrant  {
    ...
    function distributeBonusTokens() public onlyOwner nonReentrant {
    require(isFinalized, "Presale not finalized yet");
...
function claimBonusTokens() public nonReentrant {
    require(isFinalized, "Presale not finalized yet");
    require(weiRaised >= SOFT_CAP, "Soft cap not reached, no bonus tokens to claim");
    require(timelocks[msg.sender] != TokenTimelock(address(0)), "No bonus tokens for this address");
    require(block.timestamp >= bonusUnlockTime, "Bonus tokens cannot be claimed before 1 days have passed");
    ...
    function refundExcessEther() public onlyOwner nonReentrant  {
    ...
    function refundAllEther() public onlyOwner nonReentrant  {
        for (uint256 i = 0; i < investors.length; i++) {
            address payable beneficiary = payable(investors[i]);
            uint256 contributedWei = contributions[beneficiary];
