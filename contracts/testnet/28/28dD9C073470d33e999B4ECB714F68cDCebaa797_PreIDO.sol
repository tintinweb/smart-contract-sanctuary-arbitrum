...
contract PreIDO is Ownable {
    ...
    address public popTokenAddress;
    ...
    uint256 public totalPreIDO;
    uint256 internal idoRatio = 1000000; // 1U => 1000
    uint256 internal inviterRewardFee = 5; //5%

    uint256 internal idoRequireMinAmount = 1 * 10 ** 17;
    uint256 internal idoRequireMaxAmount = 1 * 10 ** 18;

    mapping(address => uint256) accountIDOMaps;
    mapping(address => uint256) accountIDOAmount;
    mapping(address => uint256) public accountPOPTokenPaid;
    ...
    function setConfigAddress(
        address _popTokenAddress, 
        address _inviterAddress,
        address _managerAddress
    ) external onlyOwner {
        popTokenAddress = _popTokenAddress;
        managerAddress = _managerAddress;
        inviterAddress = _inviterAddress;
    }

    function applyIDO() public payable  {
        uint256 amount = msg.value;
        require(isIDO, "IDO end");
        require(accountIDOAmount[msg.sender] <= 0, "You had IDO");
        require(idoRequireMinAmount <= amount && amount <= idoRequireMaxAmount, "Require USDT amount error");
        uint256 popAmount = idoRatio.mul(amount);
        accountIDOMaps[msg.sender] = accountIDOMaps[msg.sender].add(popAmount);
        accountIDOAmount[msg.sender] = amount;
        addAccountLogs(msg.sender, 1, popAmount, "IDO Amount");
        totalPreIDO += popAmount;
        //inviter
    ...
    function invited(address inviter, uint256 amount) private {
        ...
        if (accountIDOMaps[inviter] > 0) {
            ...
            accountIDOMaps[inviter] = accountIDOMaps[inviter].add(inviterRewardAmount);
             totalPreIDO += inviterRewardAmount;
    ...
    function claim() public {
        ...
        require(accountPOPTokenPaid[msg.sender] <= 0, "You have received it");
        uint256 claimAmount= accountIDOMaps[msg.sender];
        accountPOPTokenPaid[msg.sender] = claimAmount;
        IERC20(popTokenAddress).approve(address(this), claimAmount);
        IERC20(popTokenAddress).transferFrom(address(this), msg.sender, claimAmount);
    ...
    returns(
        ...
        _idoAmount = accountIDOAmount[account];
    }
