...
contract LiquidityStaker is Pausable, ReentrancyGuard, ManagerModifier {
  ...
  function unstake(
    uint256 _stakedDepositIndex,
    uint256 _amount
  ) external nonReentrant whenNotPaused {
    LpDeposit storage deposit = deposits[msg.sender][_stakedDepositIndex];
    require(
    ...
    if (deposit.amount == 0) {
      ...
      if (_stakedDepositIndex == head[msg.sender]) {
      ...
      if (head[msg.sender] != 0) {
        ...
        while (current != _stakedDepositIndex) {
          previous = current;
        ...
        if (_stakedDepositIndex == tail[msg.sender]) {
  ...
  function listAllDeposits(
    address _staker
  ) external view returns (LpDepositListEntry[] memory) {
    ...
    LpDepositListEntry[] memory returnedDeposits = new LpDepositListEntry[](length);
    current = head[_staker];
    for (uint256 i = 0; i < length; i++) {
      returnedDeposits[i].index = i;
      returnedDeposits[i].timestamp = deposits[_staker][current].timestamp;
      returnedDeposits[i].amount = deposits[_staker][current].amount;
      current = deposits[_staker][current].next;
    }
    ...
    )
  {
    ...
    while (current != 0 && _cap - depositedAmount > 0) {
      LpDeposit storage deposit = deposits[_staker][current];
      uint256 depositDuration = block.timestamp - deposit.timestamp;
      countedAmount = (_cap - depositedAmount) >= deposit.amount
        ? deposit.amount
        : (_cap - depositedAmount);

      depositedAmount += countedAmount;
      if (depositDuration >= vestingPeriod) {
        fullyVestedAmount += countedAmount;
      } else {
        vestedAmount += (countedAmount * depositDuration) / vestingPeriod;
      }
    }
  }

  function canOptimize(address staker) public view returns (bool) {
    uint256 current = head[staker];
    uint256 totalAmount = 0;
    uint256 lastFullyVested = 0;

    while (current != 0) {
      LpDeposit storage deposit = deposits[staker][current];
    ...
    return totalAmount > 0 && lastFullyVested != head[staker];
  }
