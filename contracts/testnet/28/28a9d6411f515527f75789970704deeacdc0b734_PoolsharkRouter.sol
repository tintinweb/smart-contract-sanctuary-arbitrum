    ...
    function sortQuoteResults(
        ...
        for (uint sorted = 0; sorted < results.length;) {
            ...
            if (sortIndex != type(uint256).max) {
                emit PoolQuoteSorted(results[sortIndex].pool, results[sortIndex].amountIn.toUint256(), results[sortIndex].amountOut.toUint256(), results[sortIndex].priceAfter, sortIndex);
