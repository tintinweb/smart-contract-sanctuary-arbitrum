...
contract EditPosition is Roles, ERC2771Context, IEditPosition {
    ...
    function executeEditCopy(bytes32[] calldata copyIds)
        external
        virtual
        override
        hasRole(ORACLE)
    {
        for (uint256 i = 0; i < copyIds.length; ) {
    ...
    function editCopyPosition(bytes32 copyId) external {
        ...
        (
            ...
            int256 sizeChange,
            uint16 pairId
        ) = positionStorage.updatePositionWithMaster(copyId);
        require(user != address(0x0) && amount != 0, "!wrong-copy");
        uint256 oracleFee = _getFeeExecution(executionFee);
        amount += int256(oracleFee);
        IPair.Pair memory pair = pairs.get(pairId);
        int256 fees;
        if (sizeChange == 0) {
            pool.creditFee(copyId, 0, 0, oracleFee);
        } else {
            if (sizeChange < 0) {
                //TODO event closePosition for copy
                fees = (sizeChange * int32(pair.closeFee)) / int256(BPS**2);
                pool.creditFee(copyId, 2, uint256(-1 * fees), oracleFee);
            } else {
        ...
        if (amount < 0) {
            pool.transferOut(user, uint256(amount));
        }
    }
    ...
    function removeCollateral(bytes32 id, uint256 amount) external override {
        _removeCollateral(id, amount, _msgSender());
    }
    ...
    function removeCollateralGasLess(
        ...
        uint256 oracleFee = _getFeeExecution(executionFee);
        require(amount > oracleFee, "!fee-not-enough");
        _removeCollateral(id, amount - oracleFee, gasLess.owner);
        pool.creditFee(id, 0, 0, oracleFee);
    }
    ...
    function _incPosition(
        ...
        if (fundingFee < 0) {
            require(position.amount > uint256(fundingFee), "!funding-crash");
    ...
    function _decPosition(
        ...
        uint256 feeClose = (amount * position.leverage * pair.closeFee) /
            (BPS**2);
        uint256 price = chainlink.getPrice(position.pairId);
        (int256 pnl, ) = getPnL(
            position.pairId,
            position.isLong,
            price,
        ...
        (address masterAddress, uint256 amountShare) = positionStorage
            .closePosition(id, price, pnl, finalAmount, 5);
