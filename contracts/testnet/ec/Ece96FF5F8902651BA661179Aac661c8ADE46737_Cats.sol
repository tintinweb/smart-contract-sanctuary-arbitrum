...
contract Cats is ERC721Enumerable {
    ...
    uint256 public MAX_PER_CLONE = 10;
     ...
     */
    function mintInternal(uint256 _thisTokenId, string memory _thisTokenHash ) internal {
        require( ! _exists(_thisTokenId), "This token already has been cloned");

        tokenIdToHash[_thisTokenId] = _thisTokenHash;

        hashToMinted[tokenIdToHash[_thisTokenId]] = true;
    ...
    function matchAddresSigner(bytes32 signerHash, bytes memory signature)
        private
        view
        returns (bool)
    {
        return
            _signerAddress ==
            signerHash.toEthSignedMessageHash().recover(signature);
    }
    ...
    function cloneCats(
        ...
        string[] memory _tokenHashes
    ) public  {
        require(
            matchAddresSigner(signerHash, signature),
            "DIRECT_MINT_DISALLOWED"
        );
        require(!_usedNonces[nonce], "HASH_USED");
        require(
            hashTransaction(msg.sender, tokenQuantity, nonce) == signerHash,
            "HASH_FAIL"
        );

        require(tokenQuantity <= MAX_PER_CLONE, "MAX_PER_CLONE");

        for (uint256 i = 0; i < tokenQuantity; i++) {
            mintInternal(_tokenIds[i],_tokenHashes[i]);
        }
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    ...
                                    '", "description": "cMyCloneKatz is a collection of on-chain cloned cats. All the metadata and images stored 100% on-chain.", "image": "data:image/svg+xml;base64,',
                                    CatsLibrary.encode(
        ...
        returns (string memory)
    {
        ...
        if (ownerOf(_tokenId) == 0x000000000000000000000000000000000000dEaD) {
            tokenHash = string(
                abi.encodePacked("1", CatsLibrary.substring(tokenHash, 1, 9))
            );
        }
