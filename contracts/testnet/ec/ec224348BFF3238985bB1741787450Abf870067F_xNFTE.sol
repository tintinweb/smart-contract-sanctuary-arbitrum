...
/// @title Staking (modified from Origin Staking for nfte)
...
contract xNFTE is ERC20Votes, ReentrancyGuard {
    ...
    IERC20 public immutable nfte; // Must not allow reentrancy
    ...
    constructor(IERC20 nfte_) ERC20("Staked NFTE", "xNFTE") ERC20Permit("xNFTE") {
        nfte = nfte_;
    ...
    /// @notice Stake nfte to an address that may not be the same as the
    ...
    /// epoch.
    ///
    /// @param amount nfte to lockup in the stake
    ...
    function _stake(uint256 amount, uint256 duration, address to, bool doDelegation) internal nonReentrant {
        ...
        nfte.safeTransferFrom(msg.sender, address(this), amount); // Important that it's sender

        emit Stake(to, lockups[to].length - 1, amount, end, points);
    }

    /// @notice Collect staked nfte for a lockup.
    ...
    function unstake(uint256 lockupId) external nonReentrant {
        ...
        nfte.safeTransfer(msg.sender, amount);
    ...
    /// @param amount of additional nfte to lockup. amount can be zero
    /// @param doDelegation is bool; if true, delegate to sender
    function extend(uint256 lockupId, uint256 duration, uint256 amount, bool doDelegation) external nonReentrant {
        ...
        if (amount != 0) nfte.safeTransferFrom(msg.sender, address(this), amount); // Important that it's sender
    ...
    ///
    /// @param amount nfte to be staked
