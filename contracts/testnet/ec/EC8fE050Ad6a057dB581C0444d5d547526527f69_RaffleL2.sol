...
interface RaffleL1 {
    function setWinner(uint256 _tokenId, address _winner,bool taken) external;
}
...
contract RaffleL2 is VRFConsumerBaseV2, ConfirmedOwner, ReentrancyGuard {
    ...
    function setWinner(uint256 raffleId) external {
        ...
        if (totalEmitted < raffles[raffleId].minimumAmount) {
            ...
            setWinnerInL1(raffles[raffleId].prize, raffles[raffleId].winner,false);
        } else {
    ...
    function setWinnerInL1(uint256 _tokenId, address _winner,bool taken)
        public
        returns (uint256)
    {
