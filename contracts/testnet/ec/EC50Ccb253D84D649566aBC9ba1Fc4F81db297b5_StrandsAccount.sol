...
interface IStrandsPosition {
  ...
  function getOwnerTokenIds(address target) external view returns (uint256[] memory);
  ...
  function getPositionDetails(uint tokenId) external view returns (PositionDetails memory);
}
...
contract StrandsAccount is ERC721, DefaultOperatorFilterer, OwnableAdmins {
  ...
  function mint(
    ...
    address owner = getOwner(clearingFirm_, accountNumber_);
    require(owner == address(0), "NFT already exist");
    mintCounter += 1;
  ...
  function updateValues(
    ...
    uint tokenId = accountTree[clearingFirm_][accountNumber_];
    // Check the tokenId exist for clearingFirm_ and accountNumber_
    require(tokenId != 0, "NFT doesn't exist with clearingFirm and account number");

    accountDetails[tokenId].accountValue = accountValue_;
    accountDetails[tokenId].initialMargin = initialMargin_;
    accountDetails[tokenId].maintenanceMargin = maintenanceMargin_;
    accountDetails[tokenId].excessEquity = excessEquity_;
  }

  function transferAccount(string memory clearingFirm_, string memory accountNumber_, address to_) external onlyAdmins {
    require(to_ != address(0), "INVALID_RECIPIENT");

    uint tokenId_ = accountTree[clearingFirm_][accountNumber_];
    address owner = _ownerOf[tokenId_];

    /// Transfer StrandsAccount to new owner
    transferFrom(owner, to_, tokenId_);

    /// Transfer StrandsPosition to new owner
    uint256[] memory ownedTokenIds = IStrandsPosition(positionNFT).getOwnerTokenIds(owner);
    for (uint256 i = 0; i < ownedTokenIds.length; ) {
      // TODO: Think about gas saving
      IStrandsPosition.PositionDetails memory positionDetail = IStrandsPosition(positionNFT).getPositionDetails(
        ownedTokenIds[i]
      );
      if (
        ...
        IStrandsPosition(positionNFT).transferFrom(owner, to_, ownedTokenIds[i]);
      }
      unchecked {
        ++i;
      }
  ...
  function deleteAccount(string memory clearingFirm_, string memory accountNumber_) external onlyAdmins {
    uint tokenId_ = accountTree[clearingFirm_][accountNumber_];
    uint256[] memory ownedTokenIds = IStrandsPosition(positionNFT).getOwnerTokenIds(_ownerOf[tokenId_]);
  ...
  function transferFrom(address from, address to, uint256 id) public override onlyAdmins {
  ...
  function setPositionNFT(address positionNFT_) external onlyAdmins {
  ...
  function getTokenId(string memory clearingFirm_, string memory accountNumber_) public view returns (uint) {
    return accountTree[clearingFirm_][accountNumber_];
  }

  function getOwner(string memory clearingFirm_, string memory accountNumber_) public view returns (address) {
    uint tokenId = getTokenId(clearingFirm_, accountNumber_);
    // Check the tokenId exist for clearingFirm_ and accountNumber_
    if (tokenId > 0) {
  ...
  function getAccountDetails(uint tokenId_) public view returns (AccountDetails memory) {
    return accountDetails[tokenId_];
  }
