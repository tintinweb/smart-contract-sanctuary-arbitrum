...
contract L2UnicornHatcher is L2UnicornHatcherPermit, BaseGame, BaseL2, BaseL2Unicorn, BaseL2UnicornRule, BaseRecipient, Pausable {
    ...
    function randomizerCallback(uint256 requestId_, bytes32 seed_) external {
        ...
        for (uint8 numberIndex_ = 0; numberIndex_ < hatchCount_;) {
            (uint256 randomNumber_, IL2UnicornRule.HatchRule memory hatchRule_) = _getRandomNumberAndHatchRule(seed_, eSeries_, numberIndex_);
            if (hatchRule_.startTokenId != 0) {
                ...
                emit HatchCallback(user_, requestId_, seed_, eSeries_, numberIndex_, randomNumber_, hatchRule_.level, tokenId, l2Unicorn);
            }
    ...
    function viewHatchResults(uint256 requestId_) external view returns (HatchResult[] memory hatchResults){
        ...
        for (uint8 numberIndex_ = 0; numberIndex_ < hatchCount_;) {
            (uint256 randomNumber_, IL2UnicornRule.HatchRule memory hatchRule_) = _getRandomNumberAndHatchRule(gameInfo_.seed, eSeries_, numberIndex_);
    ...
    function _getRandomNumberAndHatchRule(bytes32 seed_, uint8 eSeries_, uint256 numberIndex_) private view returns (uint256 randomNumber_, IL2UnicornRule.HatchRule memory hatchRule_){
        randomNumber_ = getHatchRandomNumber(eSeries_, seed_, numberIndex_);
