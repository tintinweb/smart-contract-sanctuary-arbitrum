...
contract OrderbookFactory is Ownable {
    ...
    */
    ...
    function eventEmit(address makerOrTaker, uint256 price, uint256 size, bool isAsk, bool isMake, bool hasEnded, address order) external {
...
contract Orderbook {
    ...
    function createAsk(uint256 _USDCPerOPTION, uint256 _OPTIONSize) public returns (Ask) {
        ...
        ask.fundContract();
        factory.eventEmit(msg.sender, _USDCPerOPTION, _OPTIONSize, true, true, false, address(ask));
        return ask;
    }
    ...
    function createBid(uint256 _OPTIONPerUSDC, uint256 _USDCSize) public returns (Bid) {
        ...
        bid.fundContract();
        factory.eventEmit(msg.sender, _OPTIONPerUSDC, _USDCSize, false, true, false, address(bid));
        return bid;
    }
    ...
    function fillAsks(address receiver, uint256 _USDCPerOPTION, uint256 USDCsize, uint256 seatId, address[] memory asks) public returns(uint256) {
        ...
        for (uint256 i; i < asks.length; i++) {
            ...
            if(toFill > USDCsize) {
                ...
                factory.eventEmit(receiver, _USDCPerOPTION, USDCsize, true, false, false, address(asks[i]));
                break;
            }
            USDC.approve(asks[i], toFill);
            Ask(asks[i]).fill(receiver, toFill, seatId);
            factory.eventEmit(receiver, _USDCPerOPTION, USDCsize, true, false, true, asks[i]);
            USDCsize -= toFill;
    ...
    function fillBids(address receiver, uint256 _OPTIONPerUSDC, uint256 OPTIONsize, uint256 seatId, address[] memory bids) public returns(uint256) {
        ...
        for (uint256 i; i < bids.length; i++) {
            ...
            if(toFill > OPTIONsize) {
                ...
                factory.eventEmit(receiver, _OPTIONPerUSDC, OPTIONsize, false, false, false, address(bids[i]));
                break;
            }
            cumVol += OPTION.balanceOf(bids[i]);
            OPTION.approve(bids[i], toFill);
            Bid(bids[i]).fill(receiver, toFill, seatId);
            factory.eventEmit(receiver, _OPTIONPerUSDC, OPTIONsize, false, false, true, address(bids[i]));
            OPTIONsize -= toFill;
