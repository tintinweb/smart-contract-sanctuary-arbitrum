...
/**
 * @title RwaxStake
 * @notice Contract to stake RWAX token and earn VE_RWAX reward.
 * @author Rwax
 */
contract RwaxStake is Initializable, OwnableUpgradeable, IRwaxStake {
     ...
     */
    function initialize(address _rwax, address _veRwax, uint256 _rewardPerSecond) external initializer {
        __Ownable_init();
        require(_rewardPerSecond <= MAX_REWARD_PER_SECOND, "> MAX_REWARD_PER_SECOND");
        require(_rwax != address(0), "Invalid RWAX address");
        require(_veRwax != address(0), "Invalid VE_RWAX address");
        RWAX = IBurnableERC20(_rwax);
        VE_RWAX = IERC20(_veRwax);
    ...
    function pendingReward(address _to) external view returns (uint256) {
        ...
        uint256 rwaxSupply = RWAX.balanceOf(address(this));
        uint256 _accRewardPerShare = accRewardPerShare;
        if (block.timestamp > lastRewardTime && rwaxSupply != 0) {
            uint256 time = block.timestamp - lastRewardTime;
            uint256 reward = time * rewardPerSecond;
            uint256 boostedReward = getBoostedReward();
            _accRewardPerShare += (((reward + boostedReward) * ACC_REWARD_PRECISION) / rwaxSupply);
        }
    ...
    /**
     * @dev Staked RWAX tokens, and start earning rewards
    ...
    function stake(address _to, uint256 _amount) external override {
        ...
        if (user.amount != 0) {
            ...
            if (pending != 0) {
                _safeTransferVE_RWAX(_to, pending);
    ...
    function unstake(address _to, uint256 _amount) external override {
        ...
        IERC20(RWAX).safeTransfer(_to, amountToUnstake);
    ...
    /**
     * @dev Claims VE_RWAX rewards to the address `to`
    ...
    function claimRewards(address _to) external {
        ...
        if (_pendingReward != 0) {
            _safeTransferVE_RWAX(_to, _pendingReward);
    ...
    /// @param _rewardPerSecond The amount of VE_RWAX to be distributed per second.
    ...
    /// @param _boostedRewardPerSecond The boosted amount of VE_RWAX to be distributed per second.
    ...
    /// @notice Reserve an amount of VE_RWAX by sending it to auction treasury
    function reserveAuctionFund(uint256 amount) external onlyOwner {
        VE_RWAX.safeTransfer(auctionTreasury, amount);
    ...
    function update() public {
        if (block.timestamp > lastRewardTime) {
            ...
            if (rwaxSupply != 0) {
                ...
                uint256 reward = time * rewardPerSecond;
                uint256 boostedReward = getBoostedReward();
                accRewardPerShare = accRewardPerShare + (((reward + boostedReward) * ACC_REWARD_PRECISION) / rwaxSupply);
            }
            lastRewardTime = block.timestamp;
        }
    }

    // Safe VE_RWAX transfer function, just in case if rounding error causes pool to not have enough VE_RWAXs.
    function _safeTransferVE_RWAX(address _to, uint256 _amount) internal {
        require(VE_RWAX != IERC20(address(0)), "VE_RWAX not set");
        uint256 veRwaxBalance = VE_RWAX.balanceOf(address(this));
        if (_amount > veRwaxBalance) {
            VE_RWAX.safeTransfer(_to, veRwaxBalance);
        } else {
            VE_RWAX.safeTransfer(_to, _amount);
        }
    }
