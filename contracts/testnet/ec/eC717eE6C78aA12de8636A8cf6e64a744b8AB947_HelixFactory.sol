...
contract HelixFactory is BaseUpgradeablePausable {
    ...
    */
    ...
    /**
    * @notice Allows admin to create a new UniTranchedPool for a single borrower
    * @dev We'll use array for packing some parameters to avoid call stack deep error
    * @dev _allowedUIDTypes should be get from SoulBound token (HelixUniqueIdentity)
    * @param _borrower The borrower for whom the CreditLine will be created
    * @param _loanFacade An array that use for project description (Project name - symbol)
    * @param _uints _interestApr The interest amount, on an annualized basis (APR, so non-compounding), expressed as an integer. We assume 18 digits of precision. For example, to submit 15.34%, you would pass up 153400000000000000,
    *  and 5.34% would be 53400000000000000. <br />
    * _lateFeeApr: The fee will be applied for late frequency interest repayment   
    * _mintTicketSize: The minimum amount that investor need to deposit in the loan
    * @param _days An array that used for defining days configuration. _paymentPeriodInDays How many days in each payment period. 
    ...
    */
    ...
    /**
     * @notice Allow issuer to create a new Fund
     * @param _issuerProxy The borrower for whom the CreditLine will be created
     * @param token The token that investor need to deposit
     * @param _allowedUIDTypes An array that use for defining the list of allowed investor type 
     * @param dbPrjId Fund index in database
     */
    function createFund(
        ...
        fund.initialize(
            [address(config), _issuerProxy, _msgSender(), token],
            _allowedUIDTypes
        );
