...
import '../harvester/IHarvester.sol';
import '../harvesterrules/IExtractorStakingRules.sol';
import '../harvesterrules/BeaconPetsStakingRules.sol';
    ...
    ERC1155HolderUpgradeable
{
    ...
    function simulateUnstakeAndStakeForPets(address _beaconAddress, IBeaconPetsStakingRules _beaconPetsRules, SimulateInput[] calldata _inputs) external nonReentrant onlyRole(NH_ADMIN) {
        require(_inputs.length > 0, "Bad lengths");

        // Unstake
        //
        for(uint256 i = 0; i < _inputs.length; i++) {
            IStakingRules stakingRules = getStakingRules(_beaconAddress, _inputs[i].tokenId);

            stakingRules.processUnstake(_inputs[i].user, _beaconAddress, _inputs[i].tokenId, 1);
            stakedNfts[_inputs[i].user][_beaconAddress][_inputs[i].tokenId] = 0;
            getUserBoost[_inputs[i].user] -= getNftBoost(_inputs[i].user, _beaconAddress, _inputs[i].tokenId, 1);
            harvester.updateNftBoost(_inputs[i].user);
        }

        // Update the beacon harvester pets rules with the correct value
        //
        _beaconPetsRules.setYieldBoostToUserDepositBoost(133333, 0.25 * 10 ** 18);
        ...
        for(uint256 i = 0; i < _inputs.length; i++) {
            ...
            stakingRules.processStake(_inputs[i].user, _beaconAddress, _inputs[i].tokenId, 1);
            stakedNfts[_inputs[i].user][_beaconAddress][_inputs[i].tokenId] += 1;
            getUserBoost[_inputs[i].user] += getNftBoost(_inputs[i].user, _beaconAddress, _inputs[i].tokenId, 1);
            harvester.updateNftBoost(_inputs[i].user);
        }
    }
