...
contract Passport is Collectible {
    ...
    constructor(
        ...
        string memory _symbol,
        address _daoToken
    ) Collectible(_entity, CollectibleType.PASSPORT, _area, _authority, _dataURI, _name, _symbol, _daoToken)  {  

    }

    // Mints a passport NFT to the patron
    function mint(
        address _patron,
        uint256 _expiry,
        bool _transferable
    ) external onlyForwarder returns (uint256 _passportId)
    {

        // Check if the patron already holds this passport
        require(balanceOf(_patron) == 0, "Patron already holds the passport");
    ...
    function addVisit(uint256 _passportId) external onlyForwarder {
        _addVisit(_passportId);
    }

    function addFriendsVisit(uint256 _passportId) external onlyForwarder {
        _addFriendsVisit(_passportId);
    }

    function linkCollectible(address _collectible) external onlyForwarder {
        _linkCollectible(_collectible);
    }

    // Returns Passport ID for a patron
...
import "../collectibles/Passport.sol";
import "../access/DAOAccessControlled.sol";
import "../../interfaces/factories/IPassportFactory.sol";

contract PassportFactory is IPassportFactory, DAOAccessControlled {
    ...
    function pushPassportAddress(address _entity,address _passport) external onlyGovernor  {
        require(passportExists[_passport]==false,"Passport already exist");
        allPassports.push(_passport);
        passportExists[_passport] =  true;
        entityPassports[_entity].push(_passport);
        emit CreatedPassport(_entity, _passport);
