...
SOIL is an experimental self-marketmaking token using liquidity bins on Trader Joe v2.1
...
contract LADDER is ERC20, Ownable {
    ...
    address[] buyPairsList;
    bool flagBuyPair = false;
    ...
    function setFee(uint256 devFee_ , uint256 marketFee_ , uint256 lotusFee_ , uint256 stakeSellFee_ , uint256 stakeFee_) public onlyOwner{
        if(devFee_<25){
            devFee = devFee_;
        }
        if(marketFee_<25){
    ...
    function setWallet(address devWallet_ , address marketWallet_ , address lotusBuybackWallet_) public onlyOwner{
        devWallet = devWallet_;
        marketWallet = marketWallet_;
        lotusBuybackWallet = lotusBuybackWallet_;
    }

    function setBuyPairs(address[] calldata _onlyBuypairs) public onlyOwner{
        buyPairsList = _onlyBuypairs;
    }

    function isBuyPair(address _pair) internal {
        flagBuyPair = false;
        for(uint256 i=0; i< buyPairsList.length; i++) {
            if(address(buyPairsList[i]) == _pair) {
                flagBuyPair = true;
            }
        }
    }
            ...
            if ((flagDexPair && to != (address(this))) || (from == address(joeRouter) && to == address(pair))) {
                ...
                else
                {
                    if (seedLiquidityTimes >0 && getActiveBinId() == floorLiquidityBin) {
                        ...
                        balanceOf[lotusBuybackWallet] += sendToLotusVault;
                        balanceOf[from] -= sendToLotusVault;
                        emit Transfer(from, lotusBuybackWallet, sendToLotusVault);
                    }
                }
            }
