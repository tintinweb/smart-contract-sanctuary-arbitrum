...
contract Factory is Initializable, OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable {
    ...
    function mintTo(uint256 eventId, uint256 tokenId, address toAddress) external onlyOwner returns (bool result) {
        INFT(events[eventId].nftContract).mintNFT(tokenId, toAddress);
        return true;
    }
...
interface INFT {
    ...
    function mintNFT(uint256 tokenId, address receiver) external returns (bool);
