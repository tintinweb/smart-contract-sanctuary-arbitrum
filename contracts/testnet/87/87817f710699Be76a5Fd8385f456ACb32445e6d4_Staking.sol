...
contract Staking is Ownable, Pausable {
    ...
    function stake(uint256 poolId, uint256 amount, StakingTime time) public whenNotPaused {
        ...
        if (poolId == 0) {
            ...
            Tier currentTier = stakerInfo.tier;  // Tier based on all the staked sum

            if (tierToAcquire > currentTier) {
                stakerInfo.tier = tierToAcquire;
    ...
    function unstake(uint256 poolId, uint256 amount) public whenNotPaused {
        ...
        if (block.timestamp < poolStakerInfo.deadline && poolId == 0 && stakerInfo.tier >= Tier.Merchant) {
    ...
    function getUserTickets(address user) public view returns (uint256 userTickets) {
        ...
        if (userTier == Tier.Merchant || userTier == Tier.Dealer) {
            return poolStakerInfo.stake * TICKETS_PER_100_RAISE / (100 * RAISE_DECIMAL);
        }
        return 0;
    }
    ...
    function getAllTicketInfos() public view returns (TicketInfo[] memory) {
        ...
        for (uint i = 0; i < stakersNum; i++) {
            ...
            StakerInfo memory stakerInfo = stakers[stakerAddress];
            PoolStakerInfo memory poolStakerInfo = poolStakerInfos[0][stakerAddress];

            if (stakerInfo.tier == Tier.Dealer || stakerInfo.tier == Tier.Merchant) {
    ...
    function getUserAllocationBonusPercent(address user) public view returns (uint8) {
        return stakers[user].allocationBonusPercent;
    }
    ...
    function getTierByStakingAmount(uint256 amount) public pure returns (Tier tier) {
        if (amount >= 1_000_000 * RAISE_DECIMAL) return Tier.Tycoon;
        if (amount >= 50_000 * RAISE_DECIMAL) return Tier.Broker;
        if (amount >= 5_000 * RAISE_DECIMAL) return Tier.Dealer;
        if (amount >= 500 * RAISE_DECIMAL) return Tier.Merchant;
        if (amount >= 333 * RAISE_DECIMAL) return Tier.Fan;
        return Tier.None;
    }
