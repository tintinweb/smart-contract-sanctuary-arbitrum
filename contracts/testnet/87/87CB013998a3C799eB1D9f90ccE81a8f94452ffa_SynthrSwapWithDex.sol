...
interface IDexBridge {
    function swapSynthToNative(address _account, uint256 _sourceSynthAmount) external returns (uint256);

    function swapNativeToSynth(address _account, uint256 _sourceNativeAmount) external payable returns (uint256);
...
import "../interfaces/IIssuer.sol";
import "../interfaces/IDexBridge.sol";
import "../interfaces/ISynthrBridge.sol";
import "../libraries/TransferHelper.sol";

contract SynthrSwapWithDex is MixinResolver, Owned {
    bytes32 public constant CONTRACT_NAME = "SynthrSwapWithDex";
    ...
    bytes32 private constant CONTRACT_DEX_BRIDGE = "DexBridge";
    ...
    uint16 private constant PT_CROSS_SWAP_CASE_1 = 14;
    uint16 private constant PT_CROSS_SWAP_CASE_2 = 12;
    uint16 private constant PT_CROSS_SWAP_CASE_3 = 15;
    ...
    event SwapNativeToNativeFailed(address indexed _account, string reason);

    event SwapSourceSynthToNative(
        address indexed _account,
        bytes32 _sourceSynthKey,
        uint256 _sourceSynthAmount,
        bytes32 _destNativeKey,
    ...
    event SwapSourceNativeToNative(
        address indexed _account,
        bytes32 _sourceNativeKey,
        uint256 _sourceNativeAmount,
        bytes32 _sourceSynthKey,
    ...
    function resolverAddressesRequired() public pure override returns (bytes32[] memory addresses) {
        ...
        addresses[3] = CONTRACT_DEX_BRIDGE;
    ...
    function dexBridge() internal view returns (IDexBridge) {
        return IDexBridge(requireAndGetAddress(CONTRACT_DEX_BRIDGE));
    }
    ...
    function swapSynthToNative(
        ...
        require(issuer().synths(_sourceSynthKey).balanceOf(msg.sender) >= _sourceSynthAmount, "Insufficient synth balance.");
        require(equivalentSynthKey[_destNativeKey] != bytes32(0), "The key wasn't set yet.");
        ...
        (uint256 destAmount, uint256 fee) = offChainExchanger().exchangeForDexAggregation{value: priceUpdateFee}(
            msg.sender,
            _sourceSynthKey,
            equivalentSynthKey[_destNativeKey],
            _sourceSynthAmount,
        ...
        synthrBridge().sendCrossSwapSyAssetToNativeWithDex{value: msg.value - priceUpdateFee}(
            msg.sender,
            _sourceSynthKey,
            _sourceSynthAmount,
            equivalentSynthKey[_destNativeKey],
        ...
        emit SwapSourceSynthToNative(msg.sender, _sourceSynthKey, _sourceSynthAmount, _destNativeKey, _chainId);
    }
    ...
    function destSwapSynthToNative(
        ...
        issuer().synthIssueFromSynthrSwap(address(dexBridge()), _destSynthKey, _destSynthAmount);
        try dexBridge().swapSynthToNative(_account, _destSynthAmount) returns (uint256 result) {
            emit SwapSynthToNative(
                _account,
                _sourceSynthKey,
                _sourceSynthAmount,
                _destSynthKey,
            ...
            dexBridge().refundSynth(_account, _destSynthKey, _destSynthAmount);
    ...
    function swapNativeToSynth(
        ...
        require(_chainId != 0, "Not allowed swap on self chain");
        require(equivalentSynthKey[_sourceNativeKey] != bytes32(0), "The source key wasn't set yet.");

        uint256 priceUpdateFee = perpsV2ExchangeRate().getUpdateFee(_priceUpdateData);
        require(msg.value > _sourceNativeAmount + priceUpdateFee, "Inssuficient msg value in source native to swap.");

        uint256 swappedAmount = dexBridge().swapNativeToSynth{value: _sourceNativeAmount}(msg.sender, _sourceNativeAmount);
        require(swappedAmount > 0, "Swap action failed on DEX.");
        ...
        synthrBridge().sendCrossSwapNativeToSyAsset{value: msg.value - _sourceNativeAmount - priceUpdateFee}(
            msg.sender,
            equivalentSynthKey[_sourceNativeKey],
            swappedAmount,
            _destSynthKey,
    ...
    function destSwapNativeToSynth(
        ...
        emit SwapNativeToSynth(_account, _sourceKey, _sourceAmount, _destKey, _destAmount);
    }
    ...
    function swapNativeToNative(
        ...
        require(equivalentSynthKey[_sourceNativeKey] != bytes32(0), "The source native key was not set yet.");
        require(equivalentSynthKey[_destNativeKey] != bytes32(0), "The dest native key was not set yet.");
        ...
        uint256 swappedAmount = dexBridge().swapNativeToSynth{value: _sourceNativeAmount}(msg.sender, _sourceNativeAmount);
        require(swappedAmount > 0, "Native to Synth Swap action failed on DEX.");
        ...
        synthrBridge().sendCrossSwapNativeToNativeWithDex{value: msg.value - _sourceNativeAmount - priceUpdateFee}(
            msg.sender,
            equivalentSynthKey[_sourceNativeKey],
            swappedAmount,
            equivalentSynthKey[_destNativeKey],
    ...
    function destSwapNativeToNative(
        ...
        issuer().synthIssueFromSynthrSwap(address(dexBridge()), _destKey, _destAmount);

        try dexBridge().swapSynthToNative(_account, _destAmount) returns (uint256 result) {
            emit SwapNativeToNative(
                _account,
                _sourceKey,
                _sourceAmount,
                _destKey,
                _destAmount,
                equivalentCurrencyKey[_destKey],
                result
            );
            return true;
        } catch Error(string memory reason) {
            dexBridge().refundSynth(_account, _destKey, _destAmount);
            emit SwapNativeToNativeFailed(_account, reason);
            return false;
        }
    }
