...
contract TGEController is IController {
  ...
  function claimAll() external {
    require(tokenClaimEnabled == true, 'Soon');
    for (uint256 i = 0; i < vaults.length; i++) {
      if (vaults[i].claimed(msg.sender) == false && vaults[i].deposit(msg.sender) > 0) {
  ...
  modifier onlyDeployerOrGovernance() {
    require(msg.sender == governance || msg.sender == deployer, 'Unauthorized');
    _;
  }
  ...
  function deployVault(address _underlying, uint256 _allocation) external onlyDeployerOrGovernance {
    TGEVault vault = new TGEVault(address(this), pls, _underlying, _allocation);
    vaults.push(vault);

    emit VaultDeployed(address(vault), _underlying);
  }

  function setStarted(bool _started) external onlyGovernance {
    started = _started;
    emit Start();
  }
  ...
  function withdrawFromVault(TGEVault vault) external onlyGovernance {
    vault.withdrawFunds(governance);
  }

  function endTGE() external onlyGovernance {
    started = false;
    _withdrawToGovernance();

    //TODO: Start private rewards vesting
    //TODO: Start team vesting
    //TODO: Deposit rewards into rewards contracts
    // tokenClaimEnabled = true;
  ...
  /** PRIVATE FUNCTIONS */
  function _withdrawToGovernance() private {
    for (uint256 i = 0; i < vaults.length; i++) {
      vaults[i].withdrawFunds(governance);
    }
  }
...
contract TGEVault {
  ...
  function claim(address _user, address _to) external {
    ...
    require(deposit[_user] > 0, 'No deposits found');
  ...
  event Claim(address recipient, uint256 plsAmt);
}
