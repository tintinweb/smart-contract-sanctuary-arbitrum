...
contract TestERC20 is ERC20Burnable {
    ...
    event SetTokenSymbol(string newTokenSymbol);
    ...
    function burnAll() external {
        require(IERC20(address(this)).balanceOf(_msgSender()) >= 0, "Insufficient");
    ...
    function setDecimals(uint8 _decimals) external {
        ...
        address erc20Address = address(this);
        uint256 previousBalance = IERC20(erc20Address).balanceOf(msg.sender);

        if (previousBalance > 0) {
            ERC20Burnable(erc20Address).burn(previousBalance);
            _mint(msg.sender, previousBalance / (10**tokenDecimals) * (10**_decimals));
        }

        tokenDecimals = _decimals;
        emit SetTokenDecimals(tokenDecimals);
    }

    function setSymbol(string memory _symbol) external {
        tokenSymbol = _symbol;
        emit SetTokenSymbol(_symbol);
    }
}
