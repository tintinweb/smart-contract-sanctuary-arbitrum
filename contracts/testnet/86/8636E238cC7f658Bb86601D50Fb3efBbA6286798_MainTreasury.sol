...
library MiMC {
    ...
    function Encipher( uint256 in_x, uint256 in_k ) internal pure returns(uint256 out_x) {
        return MiMCpe5( in_x, in_k, uint256(keccak256("mimc")), 91 );
    }
    ...
    */
    function MiMCpe5( uint256 in_x, uint256 in_k, uint256 in_seed, uint256 round_count ) internal pure returns(uint256 out_x) {
        assembly {
    ...
    function MiMCpe5_mp( uint256[] memory in_x, uint256 in_k, uint256 in_seed, uint256 round_count ) internal pure returns (uint256) {
        uint256 r = in_k;
        uint256 localQ = 0x30644e72e131a029b85045b68181585d2833e84879b9709143e1f593f0000001;

        for( uint256 i = 0; i < in_x.length; i++ )
        {
            r = (r + in_x[i] + MiMCpe5(in_x[i], r, in_seed, round_count)) % localQ;
        }
