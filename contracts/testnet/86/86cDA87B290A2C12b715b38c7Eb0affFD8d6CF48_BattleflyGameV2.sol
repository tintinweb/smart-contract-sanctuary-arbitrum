...
import "./wastelands/interfaces/IWastelands.sol";
import "./game/interfaces/IBattleflyGame.sol";
...
contract BattleflyGameV2 is OwnableUpgradeable, ERC1155HolderUpgradeable, ERC721HolderUpgradeable {
    ...
    event PendingWithdrawalCreated(
        ...
        uint256 walletTransactionId,
        uint256 pendingId,
        uint256 timestamp
    );
    event PendingWithdrawalProcessed(
        address sender,
        address indexed user,
        uint256 amount,
    ...
    event MagicForRNGEmissionsWithdrawn(address indexed user, uint256 amount, uint256 timestamp);
    event MagicTransferredToColdStorage(uint256 amount);
    ...
    //Upgrade for RNG topups
    address public GAME_DIAMOND;

    //Upgrade for automatic Magic withdrawals to cold storage
    uint256 public coldStorageTreshold;
    address public BATTLEFLY_BOT;
    address public coldStorageAddress;
    ...
    function withdrawMagicForRNGEmissions(uint256 amount) external onlyGameDiamond {
        require(!paused, "Contract paused");
        require(MagicToken.balanceOf(address(this)) >= amount, "Not enough Magic available");
        MagicToken.safeTransfer(GAME_DIAMOND, amount);
        emit MagicForRNGEmissionsWithdrawn(msg.sender, amount, block.timestamp);
    }

    function setGameDiamondContract(address gameDiamond) external onlyOwner {
        require(gameDiamond != address(0), "Invalid address");
        GAME_DIAMOND = gameDiamond;
    }
    ...
    function processPendingWithdrawal(uint256 pendingId, bool authorize) external onlyOwner {
        ...
        emit PendingWithdrawalProcessed(
            msg.sender,
            user,
            amount,
            pendingWithdrawals[pendingId].transactionId,
            pendingId,
            authorize,
            block.timestamp
        );
    ...
    function transferFundsToColdStorage() external onlyBattleflyBot {
        IBattleflyGame(GAME_DIAMOND).transferMagicToGameContract();
        uint256 currentBalance = MagicToken.balanceOf(address(this));
        if(currentBalance > coldStorageTreshold) {
            MagicToken.transfer(coldStorageAddress, currentBalance - coldStorageTreshold);
        }
    }

    function setBattleflyBot(address battleflyBot) external onlyOwner {
        require(battleflyBot != address(0), "Invalid address");
        BATTLEFLY_BOT = battleflyBot;
    }

    function setColdStorageAddress(address coldStorageAddress_) external onlyOwner {
        require(coldStorageAddress_ != address(0), "Invalid address");
        coldStorageAddress = coldStorageAddress_;
    }

    function setColdStorageTreshold(uint256 coldStorageTreshold_) external onlyOwner {
        coldStorageTreshold = coldStorageTreshold_;
    }
    ...
    modifier onlyGameDiamond() {
        require(_msgSender() == GAME_DIAMOND, "Not Game Diamond");
        _;
    }

    modifier onlyBattleflyBot() {
        require(_msgSender() == BATTLEFLY_BOT, "Not Battlefly Bot");
        _;
    }
...
import { PaymentType } from "../libraries/LibStorage.sol";
