...
interface IBurner {
    function burn(address to, address token, uint amount, uint amountOutMin, address[] calldata path, uint24 v3PoolFee) external;
}
...
contract SwapBurnerUniV3 is IBurner, Ownable2Step {
    event SetRouter(ISwapRouter _router);
    ...
    /**
     * @notice Set router
     * @param _router router implment ISwapRouter
     */
    function setRouter(ISwapRouter _router) external onlyOwner {
        require(address(_router) != address(0), "Zero address not valid");
        router = _router;
     ...
     * @param token Swap in token address
     * @param amount Amount in
     * @param amountOutMin Amount out
     * @param path Swap path
     * @param v3PoolFee UniswapV3 pool fee
     */
    function burn(address to, address token, uint amount, uint amountOutMin, address[] calldata path, uint24 v3PoolFee) external override {
        ...
        if (!approved[router][token]) {
            TransferHelper.doApprove(token, address(router), type(uint).max);
            approved[router][token] = true;
        }
