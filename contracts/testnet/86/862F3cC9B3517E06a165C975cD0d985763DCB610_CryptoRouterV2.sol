...
abstract contract BaseRouter is EIP712, EndPoint, AccessControlEnumerable {
    ...
    ) EIP712("EYWA", "1") EndPoint(gateKeeper_, whitelist_, treasury_, addressBook_) {
    ...
    /**
     * @dev Registers complex operation.
     *
     * @param oooops complex operation hash;
     * @param registered flag.
     */
    function registerComplexOp(bytes32 oooops, bool registered) external onlyRole(DEFAULT_ADMIN_ROLE) {
        // TODO event
        ops[oooops] = registered;
    }
    ...
    function _execute(uint256 cPos, string[] calldata operations, bytes[] calldata params) internal virtual returns (
        bytes32 nextRequestId,
        uint64 chainIdTo,
        ExecutionResult result,
        uint8 lastOp
        ...
        for (uint256 i = cPos; i < operations.length; ++i) {
            (chainIdTo, router, amountOut, result) = _executeOp(operations[i], params[i], amountOut);
            ...
            if (result == ExecutionResult.Interrupted) {
                ...
                require(router != address(0), "BaseRouter: wrong receiver");
    ...
    ) internal virtual returns (uint64 chainIdTo, address router, uint256 amountOut, ExecutionResult result);
}
...
contract CryptoRouterV2 is RouterV2, ICryptoRouter {
    ...
    constructor(
        ...
        address treasury_,
        address addressBook_
    ) RouterV2(gateKeeper_, whitelist_, treasury_, addressBook_) {
    }

    function receiveValidatedData(bytes4 selector, address from, uint64 chainIdFrom) external virtual override onlyBridge returns (bool) {
        address router = IAddressBook(addressBook).router(chainIdFrom);
        require(from == router, "RouterV2: wrong sender");
        require(selector == RouterV2.resume.selector, "Synthesis: wrong selector");
        return true;
    }
    ...
    function _executeOp(
        ...
        bytes calldata params,
        uint256 prevAmountOut
    ) internal virtual override returns (uint64 chainIdTo, address router, uint256 amountOut, ExecutionResult result) {
        (chainIdTo, router, amountOut, result) = super._executeOp(op_, params, prevAmountOut);
        if (result == ExecutionResult.Failed) {
            result = ExecutionResult.Succeeded;
            bytes32 op = keccak256(bytes(op_));
            if (ADD_CRYPTO_CODE == op) {
                AddCryptoParams memory p = abi.decode(params, (AddCryptoParams));
                p.amountIn = p.amountIn == type(uint256).max ? amountOut : p.amountIn;
                amountOut = _addCrypto(p);
                if (amountOut == 0) {
    ...
    function _addCrypto(AddCryptoParams memory p) internal returns (uint256 amountOut) {
        ...
        amountOut = ICryptoPoolAdapter(poolAdapter).addLiquidity(
            p.pool,
            p.amountIn,
            p.i,
            p.to,
    ...
    function _swapCrypto(SwapCryptoParams memory p) internal returns (uint256 amountOut) {
        address poolAdapter = IAddressBook(addressBook).stablePoolAdapter(uint64(block.chainid));
        if (p.from != poolAdapter) {
            SafeERC20.safeTransferFrom(IERC20(p.tokenIn), p.from, poolAdapter, p.amountIn);
        }
        amountOut = ICryptoPoolAdapter(poolAdapter).swap(
            p.tokenIn,
            p.pool,
            p.i,
            p.j,
    ...
    function _removeCrypto(RemoveCryptoParams memory p) internal returns (uint256 amountOut) {
        address poolAdapter = IAddressBook(addressBook).stablePoolAdapter(uint64(block.chainid));
        if (p.from != poolAdapter) {
            SafeERC20.safeTransferFrom(IERC20(p.tokenIn), p.from, poolAdapter, p.amountIn);
        }
        amountOut = ICryptoPoolAdapter(poolAdapter).removeLiquidity(
            p.pool,
            p.i,
            p.to,
            p.tokenOut,
...
interface IRouterParams {
    ...
    /**
     * @dev ... can be set as prev op result by using uint256 max. 
     */
...
contract RouterV2 is BaseRouter, IRouter {
    ...
    mapping(bytes32 => bool) processedOps;
    ...
    constructor(
        ...
        address treasury_,
        address addressBook_
    ) BaseRouter(gateKeeper_, whitelist_, treasury_, addressBook_) {
    }
    ...
    function resume(
        ...
        string[] calldata operations,
        bytes[] calldata params
    ) external onlyBridge crosschainHandling(requestId) {
    ...
    function _executeOp(
        ...
        if (PERMIT_CODE == op) {
            ...
            _permit(p);
        } else if (LOCK_MINT_CODE == op) {
            SynthParams memory p = abi.decode(params, (SynthParams));
            if (currentRequestId == 0) {
                p.amountIn = p.amountIn == type(uint256).max ? prevAmountOut : p.amountIn;
                chainIdTo = _lock(p);
                router = IAddressBook(addressBook).router(chainIdTo);
            } else {
                processedOps[currentRequestId] = true;
                amountOut = _mint(p);
            }
        } else if (BURN_UNLOCK_CODE == op) {
            SynthParams memory p = abi.decode(params, (SynthParams));
            if (currentRequestId == 0) {
                p.amountIn = p.amountIn == type(uint256).max ? prevAmountOut : p.amountIn;
                chainIdTo = _burn(p);
            ...
            if (currentRequestId == 0) {
                require(processedOps[p.requestId] == false, "Router: op processed");
                ...
                if (EMERGENCY_UNLOCK_CODE == op) {
                    amountOut = _emergencyUnlock(eP);
                } else {
                    amountOut = _emergencyMint(eP);
                }
            }
        } else {
            result = ExecutionResult.Failed;
    ...
    function _lock(SynthParams memory p) internal returns (uint64 chainIdTo) {
    ...
    function _mint(SynthParams memory p) internal returns (uint256 amountOut) {
        ...
        amountOut = ISynthesisV2(synthesis).mint(p.tokenIn, p.amountIn, p.from, p.to);
    }
    ...
    function _burn(SynthParams memory p) internal returns (uint64 chainIdTo) {
        address synthesis = IAddressBook(addressBook).synthesis(uint64(block.chainid));
    ...
    function _wrap(WrapParams memory p) internal returns (uint256 amountOut) {
        ...
        (bool sent, ) = address(this).call{ value: p.amountIn }("");
        require(sent, "Router: failed to send Ether");
    ...
    function _permit(PermitParams memory p) internal {
