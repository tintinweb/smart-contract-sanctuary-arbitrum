...
contract Vault is Constants, ReentrancyGuard, Ownable, IVault {
    ...
    function takeAssetIn(
        ...
        _collectVaultFee(true, _token, _amount, _fee, _refer);
    ...
    function _accountDeltaAndFeeIntoTotalBalance(
        ...
        if (_adjustDelta != 0) {
            ...
            if (_hasProfit) {
                ...
                _setBalance(balances, _token, _feeRewardOnDelta, true);
            } else {
                require(totalUSD >= _feeRewardOnDelta, "Exceeded ROLP bottom");
                totalUSD -= _feeRewardOnDelta;
                _setBalance(balances, _token, _feeRewardOnDelta, false);
            }
        }
    ...
    function _collectVaultFee(bool _mint, address _token, uint256 _amount, uint256 _fee, address _refer) internal {
        ...
        if (_fee != 0 && _feeManager != ZERO_ADDRESS) {
            ...
            if (IERC20(_token).balanceOf(address(this)) >= feeMinusFeeReward) {
                IERC20(_token).safeTransfer(_feeManager, feeMinusFeeReward);
            } else {
                uint256 feeMinusFeeRewardInUSD = feeMinusFeeReward == 0 ? 0 : priceManager.fromTokenToUSD(_token, feeMinusFeeReward);
                IMintable(RUSD).mint(_feeManager, feeMinusFeeRewardInUSD);
            }
        ...
        if (_refer != ZERO_ADDRESS && settingsManager.referEnabled()) {
            ...
            if (IERC20(_token).balanceOf(address(this)) >= referFee) {
                IERC20(_token).safeTransfer(_refer, referFee);
            } else {
                uint256 referFeeInUSD = priceManager.fromTokenToUSD(_token, referFee);
                IMintable(RUSD).mint(_refer, referFeeInUSD);
            }
        ...
        if (_amount > 0) {
            ...
            if (_mint) {
                IMintable(RUSD).mint(address(this), amountInUSD);
            } else {
                IMintable(RUSD).burn(address(this), amountInUSD);
            }
        }
    }
