...
contract CommonUtil{
    ...
    // uint256 public periodInSecond = 2628000;  // prod 
    uint256 public periodInSecond = 900;  // dev
    ...
    event FeeGetting(address indexed _from, address indexed _receiver, address indexed _to, uint256 _feeSend, uint256 _feeReceive, uint256 _feeBuy);
...
contract MagicTokenV2 is ERC20, Ownable, CommonUtil {
  using SafeMath for uint256;
  // ------------------------ DECLARATION ------------------------
  ...
  uint8 private takeFeeSend = 0;
  uint256 private feeSend = 0;

  uint8 private takeFeeReceive=0;
  uint256 private feeReceive=0;



  // whitelist type(1 for send, 2 for receive, 3 for buy) ->
  // 1 if in whitelist, 2 if in blacklist`
  // ex: _specialList[1][address]: transfer of address in whitelist
  mapping(uint8 =>mapping(address => uint8)) internal _specialList;
  ...
  function roleAdd(address[] memory _addresses, uint8[] memory _roles) external onlyOwner{
      for (uint256 i = 0; i < _addresses.length; i++) {
          ...
          if(_roles[i] == 2){ //if address is admin => add to whilelist
             _specialList[1][_addresses[i]] = 1;
             _specialList[2][_addresses[i]] = 1;
             _specialList[3][_addresses[i]] = 1;
          }
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
        ...
        if(roles[_add] == 1) {
          --numOfSigners;
            ...
            _specialList[1][_addresses[i]] = 0;
            _specialList[2][_addresses[i]] = 0;
            _specialList[3][_addresses[i]] = 0;
        }
        roles[_add] = 0;
    }
  }
  /*@dev
    _addresses: address array: ex: [addr1, addr2]
    feeType: ex: [[1,1,2], [0,1,1]]  -- explain: [[send , receive , buy ~ receive]], limit 0(none),1 (whitelist),2 (blacklist)
  */
  function modiffySpecialListWithFeeType(address[] memory _addresses, uint8[][] memory feeType) external onlyOwner {
    for(uint256 i = 0; i < _addresses.length; i++) {
        for(uint8 j=1;j<4;j++){
            _specialList[j][_addresses[i]] = feeType[i][j-1];
        }
    }
  }
  ...
  // fee type = 1,2
  function setTransferFee(uint8 _type, uint8 _takeFee, uint256 _fee) external onlyOwner(){
        if(_type == 1){
            takeFeeSend = _takeFee;
            feeSend = _fee;
        }
        else if(_type == 2){
            takeFeeReceive = _takeFee;
            feeReceive = _fee;
        }
  }

  // fee type = 1,2
  function getTransferFee(uint8 _type) external view returns(uint8, uint256){
        if(_type == 1 )return (takeFeeSend, feeSend);
        return (takeFeeReceive, feeReceive);
  }
  ...
  function getSpecialList(address _address) external view onlyAdmin returns(uint8[3] memory){
      return [_specialList[1][_address], _specialList[2][_address], _specialList[3][_address]];
  }
...
// ------------------------MODIFIERs ------------------------
  ...
  modifier requiredTransfer(address _sender, address _from, address _receiver, uint256 _amount) {
      require(_specialList[1][_sender] != 2 , "sender in black list");
      require(_specialList[1][_from] != 2 , "from address in black list");
...
// ------------------------FOR SIGNER AND SYSTEM WALLET ------------------------
  ...
  requiredTransfer(_sender, _from, _receiver, _amount) returns (bool){
        require(_specialList[3][_sender] != 2, 'blacklist block');
        //calc taxFee // only supprt 0.0x percent
        uint256 _taxFeeSend = 0; 
        uint256 _taxFeeReceive = 0; 
        uint256 _taxFeeBuy = 0; 
        if(isPoolWallet(_from)){
            uint256 _feePercent = _getFeeFromPool(_from);
            _taxFeeBuy = _amount.mul(_feePercent).div(10**4); 
        }else if(takeFeeSend > 0 || takeFeeReceive >0){
            _taxFeeSend = _amount.mul(feeSend).div(10**4);
            _taxFeeReceive = _amount.mul(feeReceive).div(10**4);
        }
        // if in whitelist or sell case (pool) or no _takeFee 
        if((_taxFeeReceive == 0 && _taxFeeSend == 0 && _taxFeeBuy == 0) ||
            (_specialList[1][_from] == 1 && _specialList[2][_receiver] == 1) ||
            isPoolWallet(_receiver)
        ){
        ...
        if(isPoolWallet(_from)){
            tradingPools[_from].totalTax = tradingPools[_from].totalTax.add(_taxFeeBuy);
        }
        uint256 _totalTax = _taxFeeSend.add(_taxFeeReceive).add(_taxFeeBuy);
        _transfer(_from, _receiver, _amount.sub(_totalTax));
        
        _balances[taxWallet] = _balances[taxWallet].add(_totalTax);
        _balances[_from] =_balances[_from].sub(_totalTax);
        emit FeeGetting(_from,_receiver, taxWallet, _taxFeeSend, _taxFeeReceive, _taxFeeBuy);
        return true;
  }
