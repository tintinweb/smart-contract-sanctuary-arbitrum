...
contract MarketVault is ERC4626, ReentrancyGuard, Pausable, ERC2771Context {
    /////////////////////////////////////////////
    //              EVENTS
    /////////////////////////////////////////////
    ...
    /// @dev Emitted when Order Manager needs to withdraw assets from vault
    event WithdrawUserProfit(uint256 profitAmount);

    /// @dev Emitted when the OrderManager address is updated
    event OrderManagerUpdated(address newOrderManager);

    /// @dev Emitted when the Fee Manager contract address is updated
    ...
    ) ERC20(_name, _symbol) ERC4626(_asset) ERC2771Context(_trustedForwarder) {
    ...
    function deposit(uint256 assets, address receiver) public override nonReentrant whenNotPaused returns (uint256) {
        ...
        if (totalSupply() == 0) {
            _mint(0x000000000000000000000000000000000000dEaD, MINIMUM_SHARES); // permanently lock the first MINIMUM_SHARES
        ...
        returns (uint256)
    {
        ...
        uint256 withdrawalFeeShares = shares * withdrawalFee / MAX_WITHDRAWAL_FEE;
        uint256 withdrawalFeeAssets = convertToAssets(withdrawalFeeShares);

        _withdraw(_msgSender(), feeManager, owner, withdrawalFeeAssets, withdrawalFeeShares);
        _withdraw(_msgSender(), receiver, owner, assets - withdrawalFeeAssets, shares - withdrawalFeeShares);
        ...
        returns (uint256)
    {
        ...
        uint256 withdrawalFeeAssets = assets * withdrawalFee / MAX_WITHDRAWAL_FEE;
        uint256 withdrawalFeeShares = convertToShares(withdrawalFeeAssets);
    ...
    /// @notice Called by Order Manager to withdraw assets from vault when user has profits
    /// @param _profitAmount amount assets to withdraw
    function withdrawUserProfits(uint256 _profitAmount) external whenNotPaused onlyOrderManager {
        ...
        if (availableAssets < _profitAmount) {
            revert LibError.InsufficientAssets(_profitAmount, availableAssets);
        }
    ...
    /// @notice Update order manager for protocol upgrade
    /// @param _newOrderManager new orderManager address
    function updateOrderManager(address _newOrderManager) external onlyAdmin {
        if (_newOrderManager == address(0)) revert LibError.ZeroAddress();
        orderManager = _newOrderManager;
        emit OrderManagerUpdated(_newOrderManager);
    }
    ...
    /// @notice Rescues random funds stuck that the vault can't handle.
...
library LibError {
    ...
    // Forwarder
    error InvalidTransaction(string errorMsg);
    error InvalidToken();

    // PriceFeed
    error PythError(string errorMsg);
    error ChainlinkError(string errorMsg);

    // Order Manager
    error InvalidMarket(string errorMsg);
    error InvalidOrder(string errorMsg);
    error PriceMismatch(uint256 expected, uint256 current);
    error InvalidPosition(string errorMsg);
    error ExistingPosition();
    error InvalidLeverage(uint256 expected, uint256 current);
    error LiquidationError(string errorMsg);
...
interface IPriceFeed {
    function FEED_DECIMALS() external view returns (int32);

    function MAX_PRICE_DELAY_PRIMARY() external view returns (uint256);

    function MAX_PRICE_DELAY_SECONDARY() external view returns (uint256);

    function PRICE_FACTOR() external view returns (uint256);
    ...
    function getMarketPriceChainlink(bytes32 marketId)
        external
        view
        returns (uint256 marketPriceUsd, uint256 priceTimestamp);

    function getMarketPricePrimary(bytes32 marketId)
        external
        view
        returns (uint256 marketPriceUsd, uint256 priceTimestamp);

    function getMarketPricePyth(bytes32 marketId)
        external
        view
        returns (uint256 marketPriceUsd, uint256 priceTimestamp);

    function getMarketPriceSecondary(bytes32 marketId)
        external
        view
        returns (uint256 marketPriceUsd, uint256 priceTimestamp);

    function getTokenPriceChainlink(address tokenAddress)
        external
        view
        returns (uint256 tokenPriceUsd, uint256 priceTimestamp);

    function getTokenPricePrimary(address tokenAddress)
        external
        view
        returns (uint256 tokenPriceUsd, uint256 priceTimestamp);

    function getTokenPricePyth(address tokenAddress)
        external
        view
        returns (uint256 tokenPriceUsd, uint256 priceTimestamp);

    function getTokenPriceSecondary(address tokenAddress)
        external
        view
        returns (uint256 tokenPriceUsd, uint256 priceTimestamp);
    ...
    function marketToChainlinkFeed(bytes32) external view returns (address);
    ...
    function tokenToChainlinkFeed(address) external view returns (address);
    ...
    function updateMarketFeedChainlink(bytes32 marketId, address feedAddress) external;
    ...
    function updateTokenFeedChainlink(address tokenAddress, address feedAddress) external;
