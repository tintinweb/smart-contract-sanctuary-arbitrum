...
uint256 constant ID = uint256(keccak256("game.piratenation.loottablecomponent"));
 ...
 * @dev LootTable Component
 *
 * @dev Generated with component version 0.
 */
contract LootTableComponent is BaseStorageComponent {
    /** SETUP **/
    ...
    function batchSetValue(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            ...
            encodedValues[i] = abi.encode(
                values[i].lootIds, values[i].weights, values[i].totalWeight
            );
        }
    ...
    function has(uint256 entity) public view virtual returns (bool) {
        Layout
           storage s = LootTableComponentStorage
               .layout()
    ...
    function _setValueToStorage(
        ...
        Layout storage s = LootTableComponentStorage
            .layout()
            .entityIdToStruct[entity];

        s.lootIds = transform.lootIds;
        s.weights = transform.weights;
        s.totalWeight = transform.totalWeight;
    }
}
...
uint256 constant ID = uint256(keccak256("game.piratenation.mintedstarterpiratecomponentdev1"));

struct Layout {
    uint256 minted;
}

library MintedStarterPirateComponentStorage {
    bytes32 internal constant STORAGE_SLOT = bytes32(ID);
...
/**
 * @title MintedStarterPirateComponent
 * @dev Minted Starter Pirate Component
 *
 * @dev Generated with component version 1.
 */
contract MintedStarterPirateComponent is BaseStorageComponent {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Track whether wallet has minted a starter pirate and how many
        keys[0] = "minted";
    ...
    function getValue(
        ...
        // Get the struct from storage
        value = MintedStarterPirateComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = MintedStarterPirateComponentStorage
           .layout()
           .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](1);
        values[0] = abi.encode(s.minted);
    }
    ...
    function getBytes(
        ...
        Layout memory s = MintedStarterPirateComponentStorage.layout().entityIdToStruct[entity];
        value = abi.encode(s.minted);
    }
    ...
    function setBytes(
        ...
        Layout memory s = MintedStarterPirateComponentStorage.layout().entityIdToStruct[entity];
        (s.minted) = abi.decode(value, (uint256));
        _setValue(entity, s);
    }
    ...
    function remove(uint256 entity) public virtual {
        // Remove the entity from the component
        delete MintedStarterPirateComponentStorage.layout().entityIdToStruct[entity];
        emitRemoveBytes(entity);
    }
    ...
    function batchRemove(uint256[] calldata entities) public virtual {
        // Remove the entities from the component
        for (uint256 i = 0; i < entities.length; i++) {
            delete MintedStarterPirateComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
        emitBatchRemoveBytes(entities);
...
struct StarterPirateLootTable {
    // TokenTemplate Ids to use for starter pirates
    uint256[] templateIds;
    // Weights for each TokenTemplate Id
    uint256[] weights;
}
...
contract StarterPirateSystem is GameRegistryConsumerUpgradeable, ILootCallback {
    ...
    /**
     * Upload Starter Pirates LootTable
     * @param input      StarterPirateLootTable input
     */
    function uploadStarterPiratesLootTable(
        StarterPirateLootTable calldata input
    ) external onlyRole(MANAGER_ROLE) {
        // Check valid input params
        if (input.templateIds.length != input.weights.length) {
        ...
        // Get LootTableComponent
        LootTableComponent lootTableComponent = LootTableComponent(
            _gameRegistry.getComponent(LOOT_TABLE_COMPONENT_ID)
        );
        // Create LootTable
        lootTableComponent.setValue(
            EntityLibrary.addressToEntity(address(this)),
            LootTableComponentStruct(
                input.templateIds,
                input.weights,
                totalWeight
            )
    ...
    function grantStarterPirate(
        ...
        _grantLoot(account, tokenTemplateId, 1);
    }
    ...
    function grantLoot(
        ...
        _grantLoot(account, tokenTemplateId, amount);
    }
    ...
    function fulfillRandomWordsCallback(
        ...
        if (request.account != address(0)) {
            ...
            StarterPirateNFT(_getSystem(STARTER_PIRATE_NFT_ID)).mint(
                request.account,
                request.tokenId
            );
