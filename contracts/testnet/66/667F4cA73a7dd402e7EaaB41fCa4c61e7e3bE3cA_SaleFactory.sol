...
contract SaleFactory is Ownable {
    ...
    constructor(address erc20SaleContractAddr_, address erc1155SaleContractAddr_, address stakingContractAddr_) {
        ...
        stakingContractAddr = stakingContractAddr_;
    }

    function createSale(SaleType saleType, address saleTokenAddr, address tokenToSellAddr) public onlyOwner {
        address newSale = Clones.clone(saleContractAddresses[saleType]);

        require(newSale != address(0), "Failed to create sale");

        (bool success, ) = newSale.call(abi.encodeWithSignature("initialize(address,address,address,address)",  msg.sender, saleTokenAddr, tokenToSellAddr, stakingContractAddr));
    ...
    function updateStakingContract(address newStakingContractAddr) public onlyOwner {
        stakingContractAddr = newStakingContractAddr;
    }
