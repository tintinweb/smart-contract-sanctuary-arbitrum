...
contract Exchange is Ownable {
    ...
    uint8 private constant TOKEN_diff_DECIMAL = 7;
        ...
        for (uint8 i = 0; i < len; i++) {
            ...
            if (sellAmount == 0) {
                ...
                IERC20(tokenB).transfer(
                    msg.sender,
                    (price * sellAmount* (10 ** uint256(TOKEN_diff_DECIMAL)) ) / 100
                );
    ...
    function newBuyOrder(
        ...
        for (uint8 i = 0; i < len; i++) {
            ...
            if (buyAmount == 0) {
                ...
                IERC20(tokenB).transfer(o.seller, (price * o.amount* (10 ** uint256(TOKEN_diff_DECIMAL)) ) / 100);
                buyAmount -= o.amount;
                ...
                IERC20(tokenB).transfer(o.seller, (price * buyAmount* (10 ** uint256(TOKEN_diff_DECIMAL)) ) / 100);
                buyAmount = 0;
            }
        }
        // new buy order
    ...
    function collectFees() external onlyOwner returns (bool) {
        ...
        IERC20(tokenB).transfer(msg.sender, tokenBaccumulatedFee* (10 ** uint256(TOKEN_diff_DECIMAL)) );
