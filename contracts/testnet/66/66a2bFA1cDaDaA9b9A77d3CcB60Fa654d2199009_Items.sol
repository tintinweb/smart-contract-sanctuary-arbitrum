...
// STATE VARIABLES
    ...
    /// @dev A mapping to track items in use 
...
// EVENTS
    ...
    event ItemSetInUse(address indexed account, uint256 itemID, uint256 amount, bool inUse);
        ...
        onlyOwner
    {
        ...
        items[totalItems] = item;    // Set the quest details
        unchecked { totalItems++; }  // Increment the quest number
    }
    ...
    function updateItem(uint256 itemID, ItemType calldata item)
        ...
        onlyIfItemExists(itemID)
    {
        items[itemID] = item;   // Set the quest details
    }
    ...
    function setItemInUse(address account, uint256 itemID, uint256 amount, bool inUse)
        external
        onlyAllowed
    {
        if (inUse) {
            require(amount <= balanceOf(account,itemID), "Not enough items");
            getItemsInUse[account][itemID] = amount;     // Store the amount in use
        } else {
            delete getItemsInUse[account][itemID];       // Remove the amount in use
        }
        emit ItemSetInUse(account, itemID, amount, inUse);
    }
