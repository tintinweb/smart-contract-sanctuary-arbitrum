...
contract RockPaperScissors is Common {
    ...
    function RockPaperScissors_Play(
        ...
        rockPaperScissorsGames[msgSender] = RockPaperScissorsGame(
            ...
            numBets,
            action,
            tokenAddress == Bankroll.getveFlush()
        );
        rockPaperScissorsIDs[id] = msgSender;

        emit RockPaperScissors_Play_Event(msgSender, wager, tokenAddress, action, numBets, fee);
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal {
        ...
        for (i = 0; i < game.numBets; i++) {
            ...
            if (outcomes[i] == 1) {
                if (game.isveFlush) {
                    payout += ((game.wager * 297) / 100) - game.wager;
                    payouts[i] = ((game.wager * 297) / 100) - game.wager;
                } else {
                    payout += ((game.wager * 297) / 100);
                    payouts[i] = ((game.wager * 297) / 100);
                }
                winningBets++;
                continue;
            }
        ...
        Bankroll.increaseTotalBurned((game.numBets - winningBets) * game.wager);
    ...
    // 0 loss, 1-> win //0->Rock, 1-> Paper, 2->Scissors
    function _determineRPSResult(uint8 playerPick, uint8 rngPick) internal pure returns (uint8 result) {
