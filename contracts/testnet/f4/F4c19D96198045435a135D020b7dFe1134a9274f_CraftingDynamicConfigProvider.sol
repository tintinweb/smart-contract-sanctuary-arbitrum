...
contract ActionPermit is Pausable, IActionPermit, ManagerModifier {
  ...
  IBatchLastActionMarkerStorage public immutable ACTION_STORAGE;
  ...
  constructor(
    ...
    ACTION_STORAGE = IBatchLastActionMarkerStorage(_actionStorage);
  ...
  function checkAndMarkActionComplete(
    ...
    checkPermissions(_sender, _tokenAddr, _tokenId, _proof, _action);
    checkIfEnoughEpochsElapsed(_tokenAddr, _tokenId, _action, _minEpochs, _epochConfig);
  ...
  function checkAndMarkActionCompleteMany(
    ...
    checkPermissionsMany(_sender, _tokenAddrs, _tokenIds, _proofs, _action);
    checkIfEnoughEpochsElapsedBatch(_tokenAddrs, _tokenIds, _action, _minEpochs, _epochConfig);
  ...
  function lockBatch(
    ...
    checkPermissionsMany(msg.sender, _tokenAddrs, _tokenIds, _proofs, ACTION_LOCK);
  ...
  function checkPermissionsMany(
    ...
    if (actionLockMapping[_action]) {
      (bool isLocked, address lockedAddr, uint lockedToken, uint lockedUntil) = LOCK_STORAGE
        .isLockedBatch(_tokenAddr, _tokenIds);
      if (isLocked) {
    ...
    for (uint i = 0; i < _tokenIds.length; i++) {
      _checkPermissionsInternal(_sender, _tokenAddr, _tokenIds[i], _action);
    }
  }
  ...
  function checkIfEnoughEpochsElapsed(
    ...
    uint256 lastActionMarker = ACTION_STORAGE.getActionMarker(_tokenAddr, _tokenId, _action);
    _checkIfEnoughEpochsElapsed(
      _tokenAddr,
      _tokenId,
      _action,
      _minEpochs,
  ...
  function checkIfEnoughEpochsElapsedBatch(
    ...
    uint256[] memory lastMarkers = ACTION_STORAGE.getActionMarkerBatch(
      _tokenAddrs,
      _tokenIds,
      _action
    );
  ...
  function _checkIfEnoughEpochsElapsed(
    address _tokenAddr,
    uint256 _tokenId,
    uint256 _action,
    uint256 _minEpochs,
    ...
    uint currentEpoch = block.timestamp.toEpochNumber(_epochConfig);
    _lastActionTime = _lastActionTime.toEpochNumber(_epochConfig);

    if (currentEpoch >= _lastActionTime) {
      _lastActionTime = currentEpoch - _lastActionTime;
    } else {
      revert DuplicateActionAttempt(_tokenAddr, _tokenId);
    }

    if (_lastActionTime < _minEpochs) {
      revert InsufficientEpochSpan(_minEpochs, _lastActionTime, _tokenAddr, _tokenId);
    }
  }
}
...
contract CraftingDynamicConfigProvider is ICraftingDynamicConfigProvider, ManagerModifier {
  ...
  function _getConfig(
    ...
    if (result.durabilityLossPercentage < 100) {
      result.productivityPerCraft =
        (100 * result.productivityPerCraft) /
        result.durabilityLossPercentage;
      result.durabilityLossPercentage = 100;
