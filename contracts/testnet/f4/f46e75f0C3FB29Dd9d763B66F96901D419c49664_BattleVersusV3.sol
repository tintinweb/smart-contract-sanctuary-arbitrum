...
import "../Renown/IActivityRenown.sol";
...
contract BattleVersusV3 is ReentrancyGuard, Pausable, EpochConfigurable {
  ...
  IBattleProbabilityCalculator public BATTLE_PROBABILITY_CALCULATOR;
  IActionPermit public ACTION_PERMIT;
  IGlobalActionIdStorage public GLOBAL_ACTION_ID;
  IBatchAdventurerGateway public GATEWAY;
  IActivityRenown public ACTIVITY_RENOWN;
  IBattleV3RewardsDispenser public BATTLE_REWARDS;
  ...
  constructor(
    ...
    address _probabilityCalculator,
    address _actionPermit,
    address _activityRenown,
    ...
    BATTLE_PROBABILITY_CALCULATOR = IBattleProbabilityCalculator(_probabilityCalculator);
    ACTION_PERMIT = IActionPermit(_actionPermit);
    GATEWAY = IBatchAdventurerGateway(_gateway);
    ACTIVITY_RENOWN = IActivityRenown(_activityRenown);
    BATTLE_REWARDS = IBattleV3RewardsDispenser(_battleRewards);
  ...
  function fight(FightRequest calldata _request) external nonReentrant whenNotPaused {
    ...
    GATEWAY.checkAddressBatch(_request.oppAddresses, _request.oppProofs);
    ...
    for (uint i = 0; i < _request.advAddresses.length; i++) {
      (randomBase, levels[i]) = _fight(
        fightId + i,
        currentEpoch,
        _request.advAddresses[i],
        _request.advTokenIds[i],
    ...
    ACTIVITY_RENOWN.markActive(_request.advAddresses, _request.advTokenIds);
  }
  ...
  function _fight(
    ...
    if (ERC721(_opponentAddress).ownerOf(_opponentTokenId) == msg.sender) {
      revert InvalidOpponent(
        _adventurerAddress,
        _adventurerTokenId,
        _opponentAddress,
        _opponentTokenId,
        "Self-battling is not allowed"
      );
    }
    ...
    result.probability = BATTLE_PROBABILITY_CALCULATOR.calculateProbability(
      data.scan.attackerFinal,
      data.scan.opponentFinal
    );
  ...
  function updateProbabilityCalculator(address _probabilityCalculator) external onlyAdmin {
    BATTLE_PROBABILITY_CALCULATOR = IBattleProbabilityCalculator(_probabilityCalculator);
  }

  function updateRewards(address _rewardsContract) external onlyAdmin {
    BATTLE_REWARDS = IBattleV3RewardsDispenser(_rewardsContract);
  }
...
interface IPostBattleV3Hook {
  function postBattle(
    ...
    address[] calldata _opponentAddresses,
    uint[] calldata _opponentTokenIds,
    uint[][] calldata _peripheralTokenIds
  ) external;
}
