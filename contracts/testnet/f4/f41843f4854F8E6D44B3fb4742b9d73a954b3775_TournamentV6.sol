...
import "../../interfaces/ITournamentV6.sol";
import "../../interfaces/IJackpot.sol";
import "../../interfaces/IVoucher.sol";

contract TournamentV6 is ReentrancyGuard, ITournamentV6, Ownable {
    ...
    // gameId => index => useVoucher
    mapping(uint256 => mapping (uint256 => uint256)) public useVoucher;
    ...
    modifier onlyOperator {
        require(gamePolicy.isOperator(msg.sender), "PredictionV6: !operator");
        _;
    }

    /* ========== CONSTRUCTOR ========== */

    constructor (IGamePolicy _gamePolicy, IPrizeManagerV2 _prizeManager, IVoucher _voucher, address _feeReceiver, uint256 _currentGameId) {
    ...
    function create(
        ...
        require(_startTime > block.timestamp, "PredictionV6: !startTime");
        require(gamePolicy.isTargetToken(_targetToken), "PredictionV6: !target token");
        require(gamePolicy.isBuyInToken(_buyInToken), "PredictionV6: !buyIn token");
        require( _buyInAmount >= gamePolicy.getBuyInLimit(_buyInToken), "PredictionV6: !minBuyIn");
        require(_startTime <= block.timestamp + limitStart, "PredictionV6: Too early");
        address[] memory _new;
        uint256[] memory _newVoucher;
        currentGameId++;
    ...
    function join(uint256 _gameId, bool _useVoucher,uint256 _voucherId) external nonReentrant {
        GameInfo storage _gameInfo = gameInfo[_gameId];
        require(_gameInfo.status == GAME_STATUS.PENDING && _gameInfo.startTime > block.timestamp + delayTime, "PredictionV6: started");
        require(!isJoinedGame[msg.sender][_gameId], "PredictionV6: joined");
        require(_gameInfo.nPlayers < _gameInfo.maxPlayers, "PredictionV6: enough player");
        uint256 _discount = 0;
        if(_useVoucher){
            require(voucher.ownerOf(_voucherId) == msg.sender, "PredictionV6: !owner");
            (address _voucherToken, uint256 _voucherAmount) = voucher.info(_voucherId);
            require(_voucherToken == _gameInfo.buyInToken, "PredictionV6: voucher != buyIn Token");
            if (_voucherAmount > _gameInfo.buyInAmount) {
                _voucherAmount = _gameInfo.buyInAmount;
            }
            _discount =_voucherAmount;
            IERC721(voucher).safeTransferFrom(msg.sender, address(this), _voucherId);
            _gameInfo.vouchers.push(_voucherId);
            _gameInfo.totalPromotion += _discount;
        }
        IERC20(_gameInfo.buyInToken).safeTransferFrom(msg.sender, address(this), _gameInfo.buyInAmount - _discount);
        isJoinedGame[msg.sender][_gameId] = true;
        uint256 _index = _gameInfo.nPlayers;
        useVoucher[_gameId][_index] = _discount;
        _gameInfo.nPlayers++;
    ...
    function sponsor(uint256 _gameId, address _token, uint256 _amount) external {
        require(gameInfo[_gameId].status == GAME_STATUS.PENDING, "PredictionV6: game started");

        SponsorInfo storage _sponsorInfo = sponsorInfo[_gameId];
        uint256 _limit = gamePolicy.getSponsorLimit(_token);
        require(_limit > 0 && _amount >= _limit, "PredictionV6: !sponsor");
    ...
    function _cancelTournament(uint256 _gameId) internal {
        GameInfo storage _gameInfo = gameInfo[_gameId];
        uint256 _voucherId = 0;
        for (uint256 index = 0; index < _gameInfo.nPlayers; index++) {
            address _player =  _gameInfo.players[index];
            uint256 _discount = 0;
            if( useVoucher[_gameId][index] != 0){
                _discount = useVoucher[_gameId][index];
                IERC721(voucher).safeTransferFrom(address(this), _player, _voucherId++);
            }
            IERC20(_gameInfo.buyInToken).safeTransferFrom( address(this) , _player, _gameInfo.totalPromotion);
        }
    }
    ...
    function _finishGame(uint256 _gameId, address[] memory _winners, uint256 _option) internal {
        GameInfo storage _gameInfo = gameInfo[_gameId];
        require(_gameInfo.status == GAME_STATUS.AVAILABLE || _gameInfo.status == GAME_STATUS.PENDING, "PredictionV6: !available");
    ...
    function _createPrize(uint256 _gameId, address[] memory _winners, address _proposer, uint256 _option) internal {
        ...
        if (_option == 1) {
            _cancelTournament(_gameId);
        }
        if (_option == 2 || _option == 3) {
            IERC20(_gameInfo.buyInToken).safeTransferFrom( gamePolicy.getProducerAddress(),address(this), _gameInfo.totalPromotion);
    ...
    function record(uint256 _gameId, uint256 _roundId, uint256  _players, uint256 _predictions) external onlyOperator {
        GameInfo storage _gameInfo = gameInfo[_gameId];
        require(_gameInfo.status != GAME_STATUS.CLOSED && _gameInfo.status != GAME_STATUS.CANCELED, "PredictionV6: closed");
        require(block.timestamp >= _gameInfo.startTime && _gameInfo.startTime > 0, "PredictionV6: !started");
        require(!isHasData[_gameId][_roundId], "PredictionV6: have data");
    ...
    function update(uint256 _gameId, uint256 _roundId, uint256 _result) external onlyOperator {
        require(roundInfo[_gameId][_roundId].result == ROUND_RESULT(0), "PredictionV6: have result");
    ...
    function changeSponsoredGameId(uint256 _oldGameId, uint256 _newGameId) external onlyOperator {
        require(gameInfo[_oldGameId].status == GAME_STATUS.CANCELED, "PredictionV6: !canceled");
    ...
    function setWinningFee(uint256 _fee) external onlyOwner {
        require(_fee < ONE_HUNDRED_PERCENT, "PredictionV6: !fee");
        winingFeePercent = _fee;
    }

    function setCreationFee(address _token, uint256 _feeAmount) external onlyOwner {
        require(_feeAmount >= minCreationFeeAmount, "PredictionV6: !minCreationFee");
    ...
    function setSystemFeeRatio(uint256 _creatorRatio, uint256 _operationRatio) external onlyOwner {
        require(_creatorRatio + _operationRatio == BASE_RATIO, "PredictionV6: !data");
...
interface IGamePolicy {
    ...
    function getProducerAddress() external view returns (address);
...
interface ITournamentV6 {
    function join(uint256, bool, uint256) external;
