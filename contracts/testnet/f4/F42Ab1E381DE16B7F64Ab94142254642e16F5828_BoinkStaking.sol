...
contract BoinkStaking is ReentrancyGuard, Ownable {
    ...
    modifier onlyBoosterNfts(address _nftContract) {
        require(boosterNfts[_nftContract], "NFT contract not accepted");
        _;
    }
    ...
    function depositNFT(address _nftContract, uint256[] calldata _tokenIds) external onlyBoosterNfts(_nftContract) nonReentrant {
    ...
    function withdrawNFT(address _nftContract, uint256[] calldata _tokenIds) external onlyBoosterNfts(_nftContract) nonReentrant {
    ...
    function withdrawAllNFTs(address _nftContract) external onlyBoosterNfts(_nftContract) nonReentrant {
