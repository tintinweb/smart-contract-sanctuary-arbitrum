...
contract StargateFeeLibraryV07 is LzApp, IStargateFeeLibrary {
    ...
    ///@notice check if the token price is changed and return the new price and state    
    function isTokenPriceChanged(uint256 _srcPoolId, uint256 _dstPoolId) external view returns (bool, uint256, PriceDeviationState) {
        ...
        // account for if the src side of the price pair is higher (can always trade high to low)
        if(updatedSrcCurrentPriceSD > updatedDstCurrentPriceSD){
            updatedSrcCurrentPriceSD = updatedDstCurrentPriceSD;
        }

        (PriceDeviationState newState, uint256 _diff) = _getPriceDeviationStateAndDiff(updatedSrcCurrentPriceSD, updatedDstCurrentPriceSD);
        
        // compare two prices and check if the price and state are changed or not
        bool isChanged = _isPriceAndStateChanged(currentSrcToDstPricePair, newState, _diff);
        uint256 rtnPrice = isChanged? updatedSrcCurrentPriceSD : currentSrcToDstPricePair.srcCurrentPriceSD;
    ...
    function _priceUpdateRequired(PricePair storage currentSrcToDstPricePair, PriceDeviationState newState, uint256 _newSrcPrice, uint256 _newDstPrice)
        ...
        // find current abs diff between stored prices
        uint256 storedDiff = _getAbsoluteDiffAsBps(currentSrcToDstPricePair.srcCurrentPriceSD, currentSrcToDstPricePair.dstCurrentPriceSD);
        // find abs diff between stored prices and new prices
        ...
        // if either diff is greater than the threshold, return true
        uint256 maxNewAbsDiff = newSrcDiff > newDstDiff ? newSrcDiff : newDstDiff; // gets the greater of the diffs
        // is this diff greater than the threshold?
        bool gtThreshold = maxNewAbsDiff >= threshold ? true : false;
        if(!gtThreshold){
            return false;
        }
        // else return true if the diff is greater than the stored diff
        return maxNewAbsDiff > storedDiff ? true : false;
    }


    ///@notice examines both the price AND the state and returns true if either has changed notably against the price change threshold
    function _isPriceAndStateChanged(PricePair storage _currentPriceObj, PriceDeviationState _newState, uint256 diff) 
        internal view returns (bool) {
        
        // compare current deviation state to new state -- short circuit if they are different
        if (_newState != _currentPriceObj.state) {
            return true;
        }
    ...
    function _getPriceDeviationStateAndDiff(uint256 _srcCurrentPriceSD, uint256 _dstCurrentPriceSD) internal view returns (PriceDeviationState, uint256) {
