...
contract PairexTradingStorageV5 is Initializable{
    ...
    IERC20 public usdt;
    ITokenV5 public linkErc677; // link token
    ...
    function get000() external view returns(uint){
        return 0;
    }
    ...
    function getAllOpenTradesByTrader(address _trader) external view returns(AllTrades[] memory){
        ...
        for(uint i = 0; i < pairsCount; i++){
            for(uint j = 0; j < openTradesCount[_trader][i]; j++){
                result[index].trade = openTrades[_trader][i][j];
