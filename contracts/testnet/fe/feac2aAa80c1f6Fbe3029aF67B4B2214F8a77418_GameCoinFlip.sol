...
contract Game is IGame, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        address _rng,
        uint256 _id,
        uint256 _numbersPerRoll,
        uint256 _maxMultibets,
        bool _supportsMultibets
    ) {
    ...
    function play(
        ...
        {
            (bool _prepaidFulfillment, ) = payable(
                address(rng.getSponsorWallet())
        ...
        bytes32 _requestId = rng.makeRequestUint256Array(
            _rolls * numbersPerRoll
        );
    ...
    function getMaxBet(
        ...
        return
            (((usdt.balanceOf(ALP) * getEdge()) / 10000) * (10 ** 18)) /
            getMaxPayout(_bet, _data);
    }
    ...
    function validateBet(
        ...
        uint256[50] memory _data,
        uint256 _stake
    ) public virtual returns (uint256[50] memory) {}
...
contract GameCoinFlip is Game, ICaller {
    ...
    function validateBet(
        ...
        uint256[50] memory,
        uint256
    ) public pure override returns (uint256[50] memory) {
        if (_bet > 1) {
...
interface ICaller {
    function fulfillRNG(bytes32 _requestId, uint256[] memory _randomNumbers)
        external;
}
