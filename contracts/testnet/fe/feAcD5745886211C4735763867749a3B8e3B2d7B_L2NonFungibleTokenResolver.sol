...
contract L2NonFungibleTokenResolver is BaseL2Unicorn, BaseL2UnicornRule, BaseL2MagicCube, BaseL2MagicCubeRule, RewardsManager {
    ...
    function resolveL2Unicorn(uint256[] calldata tokenIdArr) external {
        ...
        for (uint256 i_ = 0; i_ < length;) {
            ...
            require(hatchRule.awardAmount > 0, "L2MagicCube: award amount is zero");
    ...
    function resolveL2MagicCube(uint256[] calldata tokenIdArr) external {
        ...
        uint256 totalAwardAmount;
        for (uint256 i_ = 0; i_ < length;) {
            require(_msgSender() == IERC721(l2MagicCube).ownerOf(tokenIdArr[i_]), "L2MagicCube: incorrect owner");
            IL2MagicCubeRule.TokenIdRule memory tokenIdRule = IL2MagicCubeRule(l2MagicCubeRule).getTokenIdRuleByTokenId(tokenIdArr[i_]);
            require(tokenIdRule.awardAmount > 0, "L2MagicCube: award amount is zero");
            totalAwardAmount += tokenIdRule.awardAmount;
            unchecked{++i_;}
        }
