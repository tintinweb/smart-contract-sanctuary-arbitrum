...
contract PriceFeed is Governable, IPriceFeed {
    ...
    IDomFiPerp public perp;

    event SetOwner(address owner);
    event SetOracle(IOracle newOracle, IOracle prevOracle);
    event SetPerp(IDomFiPerp newPerp, IDomFiPerp oldPerp);
    ...
    /**
     * @notice Cause the connected DomFiPerp, if registered, to emit a PriceUpdate
     * @dev no-op if no perp registered, so PriceFeeds can be set up before
     *      being passed into Perp constructor
    ...
    function notifyPerp(
        ...
        if (address(perp) != address(0)) {
            perp.updatePrice(productId, value, timestamp, address(oracle));
        }
    }
    ...
    function setOracle(IOracle newOracle) external onlyOwner {
        ...
        emit SetOracle(newOracle, oracle);

        oracle = newOracle;
    }

    function setPerp(IDomFiPerp newPerp) external onlyOwner {
        require(address(newPerp) != address(perp), '!same perp');
        emit SetPerp(newPerp, perp);
        perp = newPerp;
    }
