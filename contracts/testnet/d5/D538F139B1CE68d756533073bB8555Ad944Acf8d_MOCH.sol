...
contract MOCH is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        IUniswapV3Router _uniswapV3Router = IUniswapV3Router(0xab7664500b19a7a2362Ab26081e6DfB971B6F1B0);
        uniswapV3Router = _uniswapV3Router;

        _approve(address(this), address(uniswapV3Router), MAX);

        uniswapV3Pool = IUniswapV3Factory(_uniswapV3Router.factory()).createPool(address(this), _uniswapV3Router.WETH9(), 3000);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (from == uniswapV3Pool && to != address(uniswapV3Router) && ! _isExcludedFromFee[to] && cooldownEnabled) {
                // Cooldown
            ...
            if (to == uniswapV3Pool && from != address(uniswapV3Router) && ! _isExcludedFromFee[from]) {
                require(tradingOpen, "Trading not enabled!");
                _fee = sellFee;
            }
    ...
    function openTrading() external onlyOwner() {
        ...
        IERC20(uniswapV3Pool).approve(address(uniswapV3Router), type(uint).max);
    }
