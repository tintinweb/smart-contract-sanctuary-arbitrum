...
contract SwapBurnerUniV3 is IBurner, Ownable2Step {
    ...
    event SetPoolFee(uint24 _poolFee);

    address public immutable feeVault;
    address public router;
    uint24 public poolFee; 

    // mapping(address => mapping(address => bool)) public approved; // router=>token=>bool
    ...
    function setRouter(address _router) external onlyOwner {
        require(_router != address(0), "Zero address not valid");
        router = _router;
        emit SetRouter(_router);
    }

    function setPoolFee(uint24 _poolFee) external onlyOwner {
        require(_poolFee < 1e6, "wrong pool fee");
        poolFee = _poolFee;
        emit SetPoolFee(_poolFee);
    }

    function burn(address to, address token, uint amount, uint amountOutMin, address[] calldata path) external {
        require(msg.sender == feeVault, "Invalid caller");
        require(path.length == 2, "invalid path");
        require(router != address(0), "set Universal Router first");
        
        uint256 spendAmount = TransferHelper.doTransferFrom(token, msg.sender, router, amount);

        bytes memory command = new bytes(1);
        command[0] = 0;
        bytes memory _path = abi.encodePacked(path[0], poolFee, path[1]);
        bytes[] memory input = new bytes[](1);
        input[0] = abi.encode(to, spendAmount, amountOutMin, _path, false);
        
        IUniversalRouter(router).execute(command, input, block.timestamp);
    }
}
