...
contract Keno is CommonSoloKeno, InternalRNG {
   ...
   * @return playedGameCount_ count of played games
    ...
    returns (uint256 payout_, uint32 playedGameCount_, uint256[] memory payouts_)
  {
    ...
    for (uint256 i = 0; i < _game.count; ++i) {
      ...
      if (i == 0) {
        gameDraws[_requestId].firstDraw = [
        ...
        gameDraws[_requestId].secondDraw = [
          uint32(resultNumbers_[0]),
          uint32(resultNumbers_[1]),
          uint32(resultNumbers_[2]),
          uint32(resultNumbers_[3]),
        ...
        gameDraws[_requestId].thirdDraw = [
          uint32(resultNumbers_[0]),
          uint32(resultNumbers_[1]),
          uint32(resultNumbers_[2]),
          uint32(resultNumbers_[3]),
        ...
        revert("Keno: Can't play more than 3 games");
      }
  ...
  function previousPlayerChoicesRebet(
    ...
    uint256 length_ = games[requestId_].gameData.length;
    previousChoices_ = new uint32[](length_);
    previousChoices_ = games[requestId_].gameData;
  }
   ...
   * @param _count max amount of games rounds to play
...
abstract contract CommonSoloKeno is Core {
  ...
  mapping(uint256 => GameDraws) internal gameDraws;
  ...
  function getLatestResult(uint256 _requestId) external view returns (uint32[10] memory) {
    return gameDraws[_requestId].firstDraw;
  }

  function getLatestResult(
    uint256 _requestId,
    bool _thirdResult
  ) external view returns (uint32[10] memory) {
    if (_thirdResult) {
      return gameDraws[_requestId].thirdDraw;
    } else {
      return gameDraws[_requestId].secondDraw;
    }
  }
   ...
   * @return playedGameCount_ total amount of games played
   ...
   * @param _count max amount of games
   * @param _stopGain take profit amount in token
   * @param _stopLoss stop loss amount in token
  ...
  function _create(
    ...
    games[requestId_] = Game(_count, _wager, uint32(block.timestamp), _gameData, player_, _token);
