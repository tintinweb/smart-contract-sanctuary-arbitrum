...
contract IncentivePool is Ownable, AccessControl, IIncentivePool {
    ...
    // Variables
    uint256 public incentivePoolPercentForJackpot;
    uint256 public jackpotWinningPercentForUser;
    mapping(address => uint256) public override minBetSizeForJackpot;
    uint256 public override incentivePercentForWager;
    mapping(address => uint256) public minWinningsForTransfer;
    address jackpotToken;
    ...
    function _transferWinnings(
        ...
        if (tokenAddress != address(0)) {
            IERC20(tokenAddress).safeTransfer(to, jackpotWinAmount);
        } else {
    ...
    function transferWinnings(
        ...
        uint256 jackpotWinAmount = calculateJackpotWinAmount(randomNumber);

        // Check if winnings exceed the minimum transfer threshold
        if (jackpotWinAmount > minWinningsForTransfer[jackpotToken]) {
            _transferWinnings(to, jackpotWinAmount, jackpotToken);
            emit JackpotWinningsTransferred(to, jackpotWinAmount, jackpotToken);
        }
    }

    // Read functions

    function totalPoolBalance() public view returns (uint256 tokenBalance) {
        if (jackpotToken != address(0)) {
            tokenBalance = IERC20(jackpotToken).balanceOf(address(this));
        } else {
            tokenBalance = address(this).balance;
        }
    }

    // Internal function to calculate the jackpot win amount

    function calculateJackpotWinAmount(
        uint256 randomNumber
    ) internal view returns (uint256) {
        uint256 totalBalance = totalPoolBalance();
        uint256 jackpotPoolAmount = (incentivePoolPercentForJackpot *
            totalBalance) / 10000;
        uint256 jackpotWinAmount = randomNumber % jackpotPoolAmount;
        uint256 jackpotWinningForUser = (jackpotWinningPercentForUser *
            jackpotWinAmount) / 10000;
        return jackpotWinningForUser;
    }

    // Setter functions for configurable variables

    function setIncentivePoolPercentForJackpot(
        uint256 _incentivePoolPercentForJackpot
    ) external onlyRole(ADMIN_ROLE) {
        incentivePoolPercentForJackpot = _incentivePoolPercentForJackpot;
        emit UpdateIncentivePoolPercentForJackpot(
            _incentivePoolPercentForJackpot
        );
    }
    ...
    function setJackpotWinningPercentForUser(
        uint256 _jackpotWinningPercentForUser
    ) external onlyRole(ADMIN_ROLE) {
        jackpotWinningPercentForUser = _jackpotWinningPercentForUser;
        emit UpdateJackpotWinningPercentForUser(_jackpotWinningPercentForUser);
    }

    function setIncentivePercentForWager(
        uint256 _incentivePercentForWager
    ) external onlyRole(ADMIN_ROLE) {
        incentivePercentForWager = _incentivePercentForWager;
        emit UpdateIncentivePercentForWager(_incentivePercentForWager);
    }
