...
contract ThemisRaffleTicket is ERC1155, Ownable, IThemisRaffleTicket {
    ...
    function mint(address to, uint256 id, uint256 amount) external onlyExecutor{
        _mint(to, id, amount, "");
    }

    function burn(address from, uint256 id, uint256 amount) external onlyExecutor{
        _burn(from, id, amount);
    }

    function replace(address to, uint256 id, uint256 amount) external onlyExecutor{
        _burn(to, id + 1, amount);
        _mint(to, id, amount, "");
    }
    ...
    function setTokenUri(
        uint256 id,
        string calldata winUri,
        string calldata loseUri
    ) external onlyExecutor{
        uris[id] = winUri;
        uris[id + 1] = loseUri;
        emit UriSet(id, winUri, loseUri);
    }

    function modifyTokenUri(
        uint256 id,
        string calldata newWinUri,
        string calldata newLoseUri,
        bytes calldata signature
    ) external {
        address signer = keccak256(
            abi.encodePacked("MODIFY", id, _msgSender())
        ).toEthSignedMessageHash().recover(signature);

        require(isExecutor(signer), "ThemisRaffleTicket: signer is wrong.");

        uris[id] = newWinUri;
        uris[id + 1] = newLoseUri;
        emit UriModified(id, newWinUri, newLoseUri);
    }
    ...
    function setApprovalForAll(
        ...
        revert("ThemisRaffleTicket: Unable to authorize");
    }
    ...
    function safeTransferFrom(
        ...
        revert("ThemisRaffleTicket: untransferable");
    }
