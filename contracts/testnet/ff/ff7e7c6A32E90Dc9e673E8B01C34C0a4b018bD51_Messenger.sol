...
contract Messenger is Ownable {
  ...
  function sendChannelMessage(uint _senderTokenId, uint _channelTokenId, string calldata _content) public {
    ...
    require ((senderContract.balanceOf(msg.sender, _senderTokenId) > 0 && channelContract.balanceOf(msg.sender, _channelTokenId) > 0), "Error: must hold correct sender token");
