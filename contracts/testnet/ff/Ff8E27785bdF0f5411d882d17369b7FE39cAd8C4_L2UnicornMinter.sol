...
contract L2UnicornMinter is Ownable {
    ...
    event LotteryResult(address indexed user, uint256 indexed id, uint256 seed, uint256 result);
    ...
    function lottery() external {
        ...
        uint256 id = IRandomizer(_randomizer).request(1000000);
    ...
    function randomizerCallback(uint256 _id, bytes32 _value) external {
        ...
        //
        emit LotteryResult(lotteryInfo.user, _id, seed, result);
    }
    ...
    function withdrawFund() external onlyOwner {
        (uint256 balance_,) = balanceOfFund();
        require(balance_ > 0, "L2Unicorn: balance is zero");
        IRandomizer(_randomizer).clientWithdrawTo(msg.sender, balance_);
    }
