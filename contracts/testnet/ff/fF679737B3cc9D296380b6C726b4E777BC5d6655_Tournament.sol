...
// File: contracts/iNFT721-Ticket-Factory.sol
...
// File: contracts/NFT721-Ticket.sol
...
contract NFT721Ticket is ERC721Holder, ERC721Enumerable, Pausable, Ownable, ERC721Burnable
{
...
// File: contracts/NFT721-Ticket-Factory.sol
...
contract TicketCollectionFactory is AccessControl, ITicketCollectionFactory {
    ...
    function changeOwnershipNFTContract(address _newOwner , address ticketCollectionAddress) public onlyAdmin {
        NFT721Ticket(ticketCollectionAddress).transferOwnership(_newOwner);
    }

    
}
// File: contracts/tournament-maintain.sol
...
contract Tournament is AccessControl, ReentrancyGuard {
    ...
    address ticketCollectionFactory;
    ...
    struct TicketCollection {
        // Ticket collection to manage the imported tickets from users
        address ticketCollectionAddress; // change to ticketCollectionAddress;
        uint256 price;
        address tokenBuy;
    ...
    mapping(address => mapping(address => uint256[])) public depositTicketOfUser1;
    mapping(uint256 => RedeemRequest) public listRedeemRequests;
    mapping(address => bool) public whiteListSigner;
    address[] listTicketCollectionAddress;
    ...
    event CreateRedeemRequestEvent(uint256 requestId, address[] ticketCollectionAddress);
    ...
    // claim the rewards with NFT721 tickets?
    ...
    function setTicketCollectionFactory(address ticketCollectionFactoryAddress) public onlyAdmin {
        require(ticketCollectionFactoryAddress != address(0), "address 0");
    ...
    function createRedeemRequest(
        ...
        require(redeemRequest.status == RedeemRequestStatus.CLOSE, "redeem request already exist");

        address[] memory listCollectionAddress = new address[](rewardClaimTickets.length);
        ...
        for(uint256 i =0 ; i< rewardClaimTickets.length; i++) {
            ...
            redeemRequest.status = RedeemRequestStatus.PENDING;
            listCollectionAddress[i] = rewardClaimTickets[i].ticketCollectionAddress;
        } 

        emit CreateRedeemRequestEvent(requestId, listCollectionAddress);
    ...
    function claimTicket(uint256 requestId, bytes memory signature) external {
        ...
        RedeemRequest storage redeemRequest = listRedeemRequests[requestId];
        require(redeemRequest.status == RedeemRequestStatus.PENDING, "request dose not exist");
        ...
        redeemRequest.status = RedeemRequestStatus.COMPLETE;
    ...
    function withdrawTicket(TicketQuantity[] memory withdrawTickets) public {
        ...
        for(uint256 i=0 ; i< withdrawTickets.length; i++) {
            for(uint256 j = withdrawTickets[i].quantity ; j > 0 ; j--) {
                ...
                uint256 length = depositTicketOfUser1[msg.sender][ticketCollectionAddress].length;
                uint256 tokenId = depositTicketOfUser1[msg.sender][ticketCollectionAddress][length -1];
                IERC721(withdrawTickets[i].ticketCollectionAddress).safeTransferFrom(address(this), msg.sender, tokenId);
                depositTicketOfUser1[msg.sender][ticketCollectionAddress].pop();
            }
    ...
    function createTicketCollection(
        ...
        require(whiteListPaymentToken[tokenBuy] , "token buy is not whiteList payment token");
    ...
    function depositTicket(
       ...
       // DepositTicketDetail[] storage dpt = depositTicketOfUser[msg.sender];
        for (uint256 i =0 ; i < depositTicketDetail.length; i++) {

                for (uint256 x =0 ; x <  depositTicketDetail[i].tokenIds.length; x ++) {
                    depositTicketOfUser1[msg.sender][depositTicketDetail[i].ticketCollectionAddress].push(depositTicketDetail[i].tokenIds[x]);
    ...
    //     DepositTicketDetail[] storage dpt = depositTicketOfUser[msg.sender];
        ...
        require(tournament.ticketCollectionAddress == ticketCollectionAddress, "");
    ...
    function getNumberOfTicket(
        ...
        return depositTicketOfUser1[user][ticketCollectionAddress].length;
    ...
    function getListTicketOfUser(
        ...
        for(uint256 i =0 ; i < listTicketCollectionAddress.length; i++) {
            if(depositTicketOfUser1[user][listTicketCollectionAddress[i]].length > 0) {
                dpt[i].ticketCollectionAddress = listTicketCollectionAddress[i];
                dpt[i].tokenIds = depositTicketOfUser1[user][listTicketCollectionAddress[i]];
            }
