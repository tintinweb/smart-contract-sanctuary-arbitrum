...
contract VaultUtils is IVaultUtils, BasePositionConstants, Constants, Ownable {
    ...
    function validateConfirmDelay(
        ...
        PrepareTransaction memory transaction = IPositionRouter(positionRouter).getTransaction(_getPositionKey(_account, _indexToken, _isLong, _posId));
        bool validateFlag;
    ...
    function validateLiquidation(
        ...
        Position memory position = positionKeeper.getPosition(_getPositionKey(_account, _indexToken, _isLong, _posId));
    ...
    function _validateLiquidation(
        ...
        if (_position.averagePrice > 0) {
            ...
            uint256 migrateFeeUsd = _calculateMarginFee(
                _position.indexToken,
                _isLong,
                _position.size,
                _position
            ...
            return _checkMaxThreshold(remainingCollateral, _position.size, migrateFeeUsd, _position.indexToken, _raise);
        } else {
    ...
    function _validateAmountIn(
        ...
        require(amountInUSD > 0, "AmountIn is ZERO");
        return amountInUSD;
    }
    ...
    function _validateRemoveCollateral(
        ...
        require(_position.openedFee <= _position.collateral, "Insufficient position collateral, fee exceeded");
        _validateLiquidation(_isLong, true, _indexPrice, _position);
    }
    ...
    function _beforeDecreasePosition(
        ...
        if (!_isInternal) {
            require(vault.reservedAmounts(_position.indexToken) >= _sizeDelta, "Vault: reservedAmounts exceeded");
        }
        ...
        {
            //[usdOut, fee, collateralDelta, adjustedDelta]
            (hasProfit, posData) = _reduceCollateral(
                _indexToken, 
                _sizeDelta, 
                _isLong, 
                _indexPrice, 
        ...
        bool isPartialClose = _position.size != _sizeDelta;
        ...
        return (posData, hasProfit, isPartialClose, _position);
    }
    ...
    function _reduceCollateral(
        ...
        uint256 _indexPrice, 
        Position memory _position
    ) internal view returns (bool, uint256[4] memory) {
        bool hasProfit;
        ...
        return (hasProfit, [usdOut, fee, collateralDelta, adjustedDelta]);
    }
    ...
    function calculatePNL(
        ...
        _validateRouter();
        Position memory position = positionKeeper.getPosition(_key);
        require(position.owner != address(0), "Position not exist");
        require(position.averagePrice > 0 && position.size > 0, "Invalid position");
        int256 pnl;

        if (position.isLong) {
            if (_indexPrice >= position.averagePrice) {
                pnl = int256((position.size * (_indexPrice - position.averagePrice)) / position.averagePrice);
            } else {
                pnl = -1 * int256((position.size * (position.averagePrice - _indexPrice)) / position.averagePrice);
            }
        } else {
            if (_indexPrice <= position.averagePrice) {
                pnl = int256((position.size * (position.averagePrice - _indexPrice)) / position.averagePrice);
            } else {
                pnl = -1 * int256((position.size * (_indexPrice - position.averagePrice)) / position.averagePrice);
            }
        }

        uint totalFee = _calculateMarginFee(position.indexToken, position.isLong, position.size, position) + position.openedFee;
        pnl -= int256(totalFee);
        return pnl > 0 ? (true, uint256(pnl)) : (false, uint256(-1 * pnl));
    }
    ...
    function _validateRouter() internal view {
        require(Address.isContract(positionRouter), "Invalid positionRouter");
    }
}
