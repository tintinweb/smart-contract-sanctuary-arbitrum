	...
	ExplorationSettings
{
	...
	{
		require(
			minStakingTimeInSeconds > 0,
			"Minimum staking time is not defined"
		);
		for (uint256 i = 0; i < _tokenIds.length; i++) {
			require(
				ownerForStakedDonkey(_tokenIds[i]) == msg.sender,
				"Exploration: User does not own Donkey"
			);
			//User will only be allowed to claim if donkey is staked
			require(
				locationForStakedDonkey(_tokenIds[i]) == Location.EXPLORATION,
				"Exploration: Donkey is not exploring"
			);
			require(
				isBarnClaimEligible(_tokenIds[i]),
				"Donkey is not eligible for claim"
			);
	...
	function _claimBarn(uint256 _tokenId, address _to) private {
		...
		require(
			lastStakedTime > 0,
			"Exploration: Cannot have last staked time of 0"
		);
		...
		require(
			totalTimeStaked > minStakingTimeInSeconds,
			"Exploration: Donkey has not been staked long enough"
		);
	...
	function isBarnClaimedForToken(uint256 _tokenId)
		external
		view
		returns (bool)
	{
	...
	function transferDonkeysOutOfExploration(uint256[] calldata _tokenIds)
		...
		nonZeroLength(_tokenIds)
	{
		// Check to make sure flywheel invariant is maintained
		require(
			!magicPrinter.hasUserDeposits(msg.sender),
			"Exploration: user has deposits"
		);
	...
	function _requireValidDonkeyAndLocation(
		uint256 _tokenId,
		...
		if (_oldLocation != Location.NOT_STAKED) {
			require(
				ownerToStakedTokens[msg.sender].contains(_tokenId),
				"Exploration: Caller does not own Donkey"
			);
		}
		require(
			_oldLocation != _newLocation,
			"Exploration: Location must be different"
		);
	}
	...
	function ownerForStakedDonkey(uint256 _tokenId)
		public
		view
		override
		returns (address)
	...
	function totalStakedTimeForDonkeyInSec(uint256 _tokenId)
		internal
		view
		returns (uint256)
	{
		...
		return ((tokenIdToStakeTimeInSeconds[_tokenId]) +
			(block.timestamp - lastStakedTime));
	}

	function locationForStakedDonkey(uint256 _tokenId)
		public
		view
		override
		returns (Location)
	...
	function infoForDonkey(uint256 _tokenId)
		external
		view
	...
	function timeStakedForDonkey(uint256 _tokenId)
		public
		view
		returns (uint256)
	{
...
abstract contract ExplorationContracts is Initializable, ExplorationState {
	...
	function areContractsSet() public view returns (bool) {
		return
			...
			address(magicPrinter) != address(0);
	}
}
...
abstract contract ExplorationState is
	Initializable,
	IExploration,
	...
	AdminableUpgradeable
{
	...
	mapping(address => EnumerableSetUpgradeable.UintSet)
		internal ownerToStakedTokens;

	IMagicPrinter public magicPrinter;
