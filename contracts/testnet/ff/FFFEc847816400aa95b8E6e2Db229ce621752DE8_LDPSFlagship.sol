...
contract LDPSFlagship is AccessControlEnumerable, ERC721Enumerable {
    ...
    function mint(address _owner, uint256 _tokenId) external {
        ...
        uint8[7] memory parts = lender.getParts(_tokenId);
    ...
    function upgradePart(
        ...
        if (_part != FLAGSHIP_PART.HEALTH) {
            revert InvalidPartToUpgrade();
        }

        if (_level > 100) revert("You can't upgrade Health over 100");

        partsPerShip[_tokenId][partsOrder[FLAGSHIP_PART.HEALTH]] = _level;

        lender.upgradeFlagshipPartHook(_tokenId, FLAGSHIP_PART.HEALTH, _level);

        emit PartUpgraded(FLAGSHIP_PART.HEALTH, _tokenId, _level);
    }
