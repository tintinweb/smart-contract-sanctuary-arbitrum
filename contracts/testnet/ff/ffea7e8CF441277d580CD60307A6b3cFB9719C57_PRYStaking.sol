...
import "./PRYVesting.sol";

contract PRYStaking is PRYVesting {
    /*==================================================== State Variables ========================================================*/
    mapping(address => StakeDividend) public dividendPRYStakes;
    mapping(address => StakeDividend) public dividendVestedPRYStakes;

    /*==================================================== Constructor ===========================================================*/
    constructor(
        address _vaultRegistry,
        address _timelock
    ) PRYVesting(_vaultRegistry, _timelock) {}
    ...
    /**
     *
     * @dev Retrieves the staked amount of dividend PRY tokens for a specified account and stake type.
     * @param _account The address of the account to retrieve the staked amount for.
     * @param _isVested A boolean flag indicating whether to retrieve the vested PRY or PRY dividend stake.
     * @return _amount The staked amount of dividend PRY/vPRY tokens for the specified account and stake type.
     * @dev The function retrieves the staked amount of dividend PRY/vPRY tokens for the specified account and stake type from the dividendPRYStakes or dividenVestedPRYStakes mapping,
    ...
    function dividendStakedAmount(
        ...
        return
            _isVested
                ? dividendVestedPRYStakes[_account].amount
                : dividendPRYStakes[_account].amount;
    }
     ...
     * @dev The function retrieves the dividend stake for the specified account and stake type from the dividendPRYStakes or dividenVestedPRYdStakes mapping, depending on the value of the isVested parameter.
     */
    function getDividendStake(
        ...
        return
            _isVested
                ? dividendVestedPRYStakes[_account]
                : dividendPRYStakes[_account];
    }
     ...
     */
    function pendingDividendRewards(
        address _account
    ) external view returns (uint256 pending_) {
        // Calculate the pending reward based on the dividend vested PRY stake of the given account
        pending_ += _pendingByDividendStake(dividendVestedPRYStakes[_account]);
        // Calculate the pending reward based on the dividend PRY stake of the given account
        pending_ += _pendingByDividendStake(dividendPRYStakes[_account]);
    }
    ...
    function share(
        uint256 _amount
    ) external override isAmountNonZero(_amount) onlyProtocol {
        if (totalWeight > 0) {
            totalProfit += _amount;
            totalEarned += _amount;
    ...
    /**
     *
     * @dev Internal function to deposit PRY/vPRY as dividends.
     * @param _amount The amount of PRY/vPRY to be deposited.
     * @param _isVested Boolean flag indicating if tokens are vPRY.
    ...
    function depositDividend(
        ...
        if (_isVested) {
            tokenManager.takeVestedPRY(sender_, _amount);
            stake_ = dividendVestedPRYStakes[sender_];
            totalStakedVestedPRY += _amount;
        } else {
            tokenManager.takePRY(sender_, _amount);
            stake_ = dividendPRYStakes[sender_];
            totalStakedPRY += _amount;
        }
        ...
        uint256 weight_ = _calculateWeight(
     ...
     * @param _isVested Boolean flag indicating if stake is Vested PRY.
    ...
    function unstake(
        ...
        StakeDividend storage stake_ = _isVested
            ? dividendVestedPRYStakes[sender_]
            : dividendPRYStakes[sender_];
        ...
        // Burn the necessary amount of tokens and send the remaining unstaked tokens to the staker.
        if (_isVested) {
            tokenManager_.burnVestedPRY(burnAmount_);
            tokenManager_.sendVestedPRY(sender_, sendAmount_);
            unstakedWeight_ = _amount * weightMultipliers.vPry;
            totalStakedVestedPRY -= _amount;
        } else {
            tokenManager_.burnPRY(burnAmount_);
            tokenManager_.sendPRY(sender_, sendAmount_);
            unstakedWeight_ = _amount * weightMultipliers.pry;
            totalStakedPRY -= _amount;
        }
    ...
    function _claimDividend(
        ...
        // Send the rewards to the stake owner.
        tokenManager.sendUSDC(_account, reward_);
     ...
     * @return reward_ The amount of dividends claimed.
     */
    function _claimDividendBatch(
        address _account
    ) internal returns (uint256 reward_) {
        // Determine the stake details based on the boolean flag isVested.
        StakeDividend storage stakeVPRY_ = dividendVestedPRYStakes[_account];
        StakeDividend storage stakePRY_ = dividendPRYStakes[_account];

        // Calculate the pending rewards for the stake.
        reward_ = _pendingByDividendStake(stakeVPRY_);
        reward_ += _pendingByDividendStake(stakePRY_);
        ...
        // Update the profit debt for the stake.
        stakeVPRY_.profitDebt = _calcDebt(stakeVPRY_.weight);
        stakePRY_.profitDebt = _calcDebt(stakePRY_.weight);
    ...
    /**
     * @notice Computes the pending USDC amount of the stake.
     * @param _stake The stake for which to compute the pending amount.
     * @return holderProfit_ The pending USDC amount.
     */
...
abstract contract PRYVesting is
    IPRYStaking,
    Pausable,
    ReentrancyGuard,
    AccessControlBase
{
    ...
    //The total amount of PRY tokens staked in the contract
    uint256 public totalStakedPRY;
    //The total amount of vPRY tokens staked in the contract
    uint256 public totalStakedVestedPRY;
    ...
    WeightMultipliers public weightMultipliers = WeightMultipliers(1, 1, 2);
     ...
     * @return pending The amount of pending USDC tokens that the staker is eligible to receive.
     * @dev The function iterates over all active stakes for the given staker and calculates the pending rewards for each stake using the _pendingUSDCOfStake() internal function.
     * @dev The function is view-only and does not modify the state of the contract.
     */
    function pendingVestingRewards(
        address account
        ...
        for (uint256 i = 0; i < activeIndexes.length; i++) {
            ...
            pending += _pendingUSDCOfStake(stake);
        }
    }
     ...
     * @param account The address of the staker for whom to calculate the pending reward.
     * @return pending The amount of pending USDC tokens that the staker is eligible to receive.
     * @dev The function calculates the pending rewards for the stake using the _pendingUSDCOfStake() internal function.
     * @dev The function is view-only and does not modify the state of the contract.
     */
     ...
     * @return _withdrawable  withdrawable amount of PRY/vPRY
     */
    ...
    function getActiveIndexes(
        address staker
    ) public view returns (uint[] memory indexes) {
    ...
    /**
     * @return _totalStakedVPRY The total amount of vPRY tokens staked in the contract
     * @return _totalStakedPRY The total amount of PRY tokens staked in the contract
    ...
    function globalData()
        ...
        view
        returns (
            uint256 _totalStakedVPRY,
            uint256 _totalStakedPRY,
            uint256 _totalEarned
        )
    {
        _totalStakedVPRY = totalStakedVestedPRY;
        _totalStakedPRY = totalStakedPRY;
     ...
     */
    function getVestingStakeLength(
        address _account
    ...
    function _calculateWeight(
        ...
        return
            !vested ? amount * weightMultipliers.pry : vesting
                ? amount * weightMultipliers.vPryVesting
                : amount * weightMultipliers.vPry;
    }

    /**
     * @notice Computes the pending USDC amount of the stake.
     * @param stake The stake for which to compute the pending amount.
     * @return holderProfit The pending USDC amount.
     */
    function _pendingUSDCOfStake(
        StakeVesting memory stake
    ) internal view returns (uint256) {
        // Compute the holder's profit as the product of their stake's weight and the accumulated profit per weight.
        uint256 holderProfit = ((stake.weight * accumProfitPerWeight) /
            PRECISION);
        // If the holder's profit is less than their profit debt, return zero.
        return
    ...
    /**
     * @notice Calculates the amount of PRY/vPRY that should be burned upon unstaking.
     * @param amount The amount of PRY/vWINR being unstaked.
     * @return _burnAmount The amount of PRY/vPRY to be burned.
     */
    function _computeBurnAmount(
        uint256 amount
    ) internal view returns (uint256 _burnAmount) {
    ...
    /**
     * @notice Computes the withdrawable amount of PRY for the stake.
     * @param stake The stake for which to compute the withdrawable amount.
     * @return withdrawable_ The withdrawable amount of PRY.
     */
    function _withdrawableByVesting(
        ...
        uint256 totalStakedDuration_ = (block.timestamp - stake.startTime) /
            1 days;
        ...
        withdrawable_ =
            (stake.amount * (totalStakedDuration_ - _minDays)) /
    ...
    function cancel(uint256 _index) external {
        ...
        totalStakedVestedPRY -= amount_;
        totalWeight -= stake.weight;
        // claim rewards
        uint256 reward_ = _pendingUSDCOfStake(stake);
        ...
        tokenManager.sendVestedPRY(sender_, sendAmount_);

        // Burn the remaining vesting tokens
        tokenManager.burnVestedPRY(burnAmount_);
     ...
     */
    function setUnstakeBurnPercentage(
        uint256 _unstakeBurnPercentage
    ) external onlyGovernance {
    ...
    /**
     * @dev Deposit vPRY tokens into the contract and create a vesting stake with the specified parameters
     * @param amount The amount of vPRY tokens to deposit
    ...
    function depositVesting(
        ...
        // Transfer the vPRY tokens from the sender to the token manager contract
        tokenManager.takeVestedPRY(sender, amount);

        totalWeight += weight;
        totalStakedVestedPRY += amount;
    ...
    function withdrawVesting(
        ...
        // Redeemable PRY amount by stake
        uint256 redeemable_ = _withdrawableByVesting(stake_);

        if (redeemable_ > 0) {
            // Transfer redeemable PRY tokens
            tokenManager.sendPRY(sender_, redeemable_);
        }

        // Redeemable USDC amount by stake
        uint256 reward_ = _pendingUSDCOfStake(stake_);

        // Burn vested PRY tokens
        tokenManager.burnVestedPRY(stake_.amount);

        // Interactions
        if (reward_ > 0) {
            tokenManager.sendUSDC(sender_, reward_);
        ...
        totalStakedVestedPRY -= stake_.amount;
    ...
    function withdrawVestingBatch(
        ...
        for (uint256 i = 0; i < indexes.length; i++) {
            ...
            _amounts[2] += _pendingUSDCOfStake(stake);
        ...
        tokenManager.burnVestedPRY(_amounts[3]);

        // Update the total weight and total staked amount
        totalWeight -= _amounts[0];
        totalStakedVestedPRY -= _amounts[3];
    ...
    function _claim(uint256[] memory indexes, bool isClaim) internal {
        ...
        for (uint256 i = 0; i < indexes.length; i++) {
            ...
            uint256 fee_ = _pendingUSDCOfStake(stake_);
        ...
        // Interactions
        if (totalFee_ > 0) {
            tokenManager.sendUSDC(sender_, totalFee_);
        }
