...
contract BGPaymentFacet is WithModifiers, ReentrancyGuard {
    ...
    // Currencies: 0 = ETH, 1 = Magic, 2 = USDC
    function pay(
        uint256 currency,
        uint256 productType,
        uint256 pricePerProductTypeInMagic,
...
library LibPaymentUtils {
    ...
    function pay(
        ...
        // Currencies: 0 = ETH, 1 = Magic, 2 = USDC
        if (currency >= 3) revert Errors.InvalidCurrency();
        ...
        if (expectedAmount > 0) {
            if (currency == 0) {
                ...
                uint256 usdcAmountRequired = getAmountOfCurrencyForXMagic(2, expectedAmount);
    ...
    function processEthForWithdrawal(
        ...
        if (expectedEth <= gs().ethReserve && gs().ethReserve > 0) {
            ...
            if(gs().ethReserve > 0) {
                uint256 expectedMagic = IUniswapV2Router02(gs().sushiswapRouter).getAmountsOut(gs().ethReserve, path)[0];
                expectedMagic = (expectedMagic * (gs().bpsDenominator + gs().slippageInBPS)) / gs().bpsDenominator;
                transferAmount += IUniswapV2Router02(gs().sushiswapRouter).swapExactETHForTokens{ value: gs().ethReserve }(
                    expectedMagic,
                    path,
                    address(this),
                    block.timestamp
    ...
    function processUsdcForWithdrawal(
        ...
        if (expectedUsdc <= gs().usdcReserve && gs().usdcReserve > 0) {
            ...
            if(gs().usdcReserve > 0) {
                uint256 expectedMagic = IUniswapV2Router02(gs().sushiswapRouter).getAmountsOut(gs().usdcReserve, path)[1];
                expectedMagic = (expectedMagic * (gs().bpsDenominator - gs().slippageInBPS)) / gs().bpsDenominator;
