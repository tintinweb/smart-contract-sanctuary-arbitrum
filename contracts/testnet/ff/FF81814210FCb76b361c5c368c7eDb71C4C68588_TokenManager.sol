...
contract TokenManager is AccessControlBase, ReentrancyGuard {
    ...
    event TokensSet(IGEMB indexed GEMB, IGEMB indexed vGEMBAddress);
    event GEMBStakingSet(IGEMBStaking indexed GEMBStakingAddress);
    event ReferralStorageSet(IReferralStorage indexed referralStorageAddress);
    /*==================================================== State Variables =============================================================*/
    /// @notice GEMB address
    IGEMB public immutable GEMB;
    /// @notice vGEMB address
    IGEMB public immutable vGEMB;
    /// @notice GEMLP address
    IBasicFDT public immutable GEMLP;
    /// @notice GEMB staking address
    ...
    /// @notice total minted Vested GEMB by games
    uint256 public mintedByGames;
    /// @notice max mint amount by games
    uint256 public immutable MAX_MINT;
    /// @notice total coverted amount (GEMB => vGEMB)
    uint256 public totalConverted;
    /// @notice stores transferable GEMB amount
    uint256 public sendableAmount;
    /// @notice accumulative Vested GEMB fee amount
    uint256 public accumFee;
    /// @notice divider for minting vested GEMB fee
    ...
    /**
     *
     * @param _GEMBStaking GEMB staking address
     * @notice function to set GEMB Staking address
     * @notice grants GEMB_STAKING_ROLE to the address
     */
    function setGEMBStaking(IGEMBStaking _GEMBStaking) external onlyGovernance {
        require(address(_GEMBStaking) != address(0), "address can not be zero");
        GEMBStaking = _GEMBStaking;
        emit GEMBStakingSet(GEMBStaking);
    }
    ...
    /**
     *
     * @param _mintDivider new divider for minting Vested GEMB on gameCall
     */
    function setMintDivider(uint256 _mintDivider) external onlyGovernance {
        mintDivider = _mintDivider;
    }

    /*==================================================== GEMB Staking Functions =============================================================*/
    /**
     *
     * @param _from adress of the sender
     * @param _amount amount of Vested GEMB to take
     * @notice function to transfer Vested GEMB from sender to Token Manager
     */
    function takeVestedGEMB(
        address _from,
        uint256 _amount
    ) external nonReentrant onlyProtocol {
        vGEMB.safeTransferFrom(_from, address(this), _amount);
    }

    /**
     *
     * @param _from adress of the sender
     * @param _amount amount of GEMB to take
     * @notice function to transfer GEMB from sender to Token Manager
     */
    function takeGEMB(
        address _from,
        uint256 _amount
    ) external nonReentrant onlyProtocol {
        GEMB.safeTransferFrom(_from, address(this), _amount);
    }

    /**
     *
     * @param _to adress of the receiver
     * @param _amount amount of Vested GEMB to send
     * @notice function to transfer Vested GEMB from Token Manager to receiver
     */
    function sendVestedGEMB(
        address _to,
        uint256 _amount
    ) external nonReentrant onlyProtocol {
        vGEMB.safeTransfer(_to, _amount);
    }

    /**
     *
     * @param _to adress of the receiver
     * @param _amount amount of GEMB to send
     * @notice function to transfer GEMB from Token Manager to receiver
     */
    function sendGEMB(
        address _to,
        uint256 _amount
    ) external nonReentrant onlyProtocol {
        _sendGEMB(_to, _amount);
    }

    /**
     *
     * @param _to adress of the receiver
     * @param _amount amount of GEMB to mint
     * @notice function to mint GEMB to receiver
     */
    function mintGEMB(
        address _to,
        uint256 _amount
    ) external nonReentrant onlyProtocol {
        _mintGEMB(_to, _amount);
    }

    /**
     *
     * @param _amount amount of Vested GEMB to burn
     * @notice function to burn Vested GEMB from Token Manager
     */
    function burnVestedGEMB(
        uint256 _amount
    ) external nonReentrant onlyProtocol {
        vGEMB.burn(_amount);
    }
    ...
    /**
     *
     * @param _amount amount of GEMB to burn
     * @notice function to burn GEMB from Token Manager
     */
    function burnGEMB(uint256 _amount) external nonReentrant onlyProtocol {
        GEMB.burn(_amount);
    }

    /**
     *
     * @param _to GEMB receiver address
     * @param _amount amount of GEMB
     * @notice this function transfers GEMB to receiver
     * @notice if the sendable amount is insufficient it mints GEMB
     */
    ...
    /**
     *
     * @param _to GEMLP receiver address (claim on GEMB Staking)
     * @param _amount amount of GEMLP
     * @notice funtion to transfer GEMLP from Token Manager to receiver
     */
    function sendWLP(
        address _to,
        uint256 _amount
    ) external nonReentrant onlyProtocol {
        IERC20(GEMLP).safeTransfer(_to, _amount);
    }
     ...
     * @param _amount amount of GEMB
     * @notice internal function to mint GEMB
     */
    function _mintGEMB(address _to, uint256 _amount) internal {
        GEMB.mint(_to, _amount);
    }
     ...
     * @param _to address of receiver
     * @param _amount amount of GEMB
     * @notice internal function to transfer GEMB
     */
    function _sendGEMB(address _to, uint256 _amount) internal {
        GEMB.safeTransfer(_to, _amount);
    }

    /**
     * @notice function to share fees with GEMB Staking
     * @dev only callable by FEE_COLLECTOR_ROLE
     * @param amount amount of GEMB to share
     */
    function share(uint256 amount) external nonReentrant onlyProtocol {
        GEMBStaking.share(amount);
    }

    /*==================================================== Conversion =============================================================*/
    /**
     *
     * @param _amount amount of GEMB to convert
     * @notice function to convert GEMB to Vested GEMB
     * @notice takes GEMB, mints equivalent amount of Vested GEMB
     */
    function convertToken(uint256 _amount) external nonReentrant {
        // Transfer GEMB from sender to Token Manager
        GEMB.safeTransferFrom(msg.sender, address(this), _amount);
        // Mint equivalent amount of Vested GEMB
        vGEMB.mint(msg.sender, _amount);
     ...
     * @param _recipient Vested GEMB receiver
     * @notice function to mint Vested GEMB
    ...
    function mintVestedGEMB(
        ...
        // get referral rate
        uint256 _vGEMBRate = referralStorage.getPlayerVestedGEMBRate(
            _recipient
        );
        // add vested WINR rate to mint amount
        if (_vGEMBRate > 0) {
            _mintAmount += (_mintAmount * _vGEMBRate) / BASIS_POINTS;
        }
        // mint Vested WINR
        ...
        vGEMB.mint(_recipient, _mintAmount);
    ...
    function mintFee() external nonReentrant onlySupport {
        vGEMB.mint(address(GEMLP), accumFee);
        GEMLP.updateFundsReceived_VGEMB();
        accumFee = 0;
    }
...
interface IReferralStorage {
    struct Tier {
        uint256 GEMLPRate; // e.g. 2400 for 24%
        uint256 vGEMBRate; // 5000 for 50%
    }
    event SetWithdrawInterval(uint256 timeInterval);
    event SetHandler(address handler, bool isActive);
    event SetPlayerReferralCode(address account, bytes32 code);
    event SetTier(uint256 tierId, uint256 GEMLPRate, uint256 vGEMBRate);
    event SetReferrerTier(address referrer, uint256 tierId);
    event RegisterCode(address account, bytes32 code);
    event SetCodeOwner(address account, address newAccount, bytes32 code);
    event GovSetCodeOwner(bytes32 code, address newAccount);
    event Claim(address referrer, uint256 gemLpAmount);
    event Reward(
        address referrer,
        address player,
        address token,
    ...
    event SetVestedGEMBRate(uint256 vGEMBRate);
    ...
    function setTier(uint256 _tierId, uint256 _GEMLPRate) external;
    ...
    function getPlayerVestedGEMBRate(
        address _account
