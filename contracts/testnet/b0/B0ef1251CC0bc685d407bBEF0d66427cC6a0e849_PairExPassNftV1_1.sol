...
contract PairExPassNftV1_1 is Ownable, Pausable, ERC1155, ERC1155Supply {
    ...
    function mint(uint256 id, uint256 amount) public notContract whenNotPaused {
        ...
        require(amount <= userCanMintAmount, "EXCEED_MINT_AMOUNT");
        require(userAlreadyMintAmount[account][id] < userCanMintAmount, "USER_ALREADY_MINTED");
        require(usdt.balanceOf(account) > monthMintPrices[id], "USDT_NOT_ENOUGH");
        
        userAlreadyMintAmount[account][id] += 1;

        if(monthMintPrices[id] > 0) {
            usdt.safeTransferFrom(account, feeAddr, monthMintPrices[id]);
        }
    ...
    function isTokenIdExist(uint256 tokenId) public view returns (bool) {
        return (monthTimeStamps[tokenId] != 0);
    }
