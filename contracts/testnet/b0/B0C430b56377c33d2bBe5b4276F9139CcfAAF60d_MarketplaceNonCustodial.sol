    ...
    IMarketplaceNonCustodial
{
    ...
    function listItem(
        ...
        if (_standard == NFTStandard.E721) {
            ...
            if (_amount != 1) {
                revert InvalidAmountInput(_amount);
            }
            ...
            // ownership check
            if (_nft.balanceOf(msg.sender, _tokenId) < _amount) {
                revert NotEnoughItemsOwnedByCaller(_nftAddress, _tokenId);
            }
            // approval check
    ...
    function updateItemPrice(uint256 _orderId, uint256 _newPricePerItem)
        external
        override
    {
        ...
        if (isOrderSold(_orderId)) {
            revert ItemAlreadySold(_orderId);
        }
    ...
    function buyItem(uint256 _orderId) external payable override nonReentrant {
        ...
        require(
            msg.value == _order.pricePerItem * _order.amount,
        ...
        _markOrderSold(_orderId, msg.sender);
        emit ItemsBought(
            _orderId,
            _order.nftAddress,
            _order.standard,
        ...
        returns (bool)
    {
        ...
        Order memory _listing = listings[_orderId];
        if (_listing.standard == NFTStandard.E721) {
            IERC721 _nft = IERC721(_listing.nftAddress);
            //note don't need to make sure if owner is right because with any transfer approval will be cleared
            try _nft.getApproved(_listing.tokenId) returns (
            ...
            IERC1155 _nft = IERC1155(_listing.nftAddress);
            // check ownership and approval
            if (
                (_nft.balanceOf(_listing.seller, _listing.tokenId) >=
                    _listing.amount) &&
                (_nft.isApprovedForAll(_listing.seller, address(this)))
            ) {
                return true;
            } else {
                return false;
    ...
    function isOrderSold(uint256 _orderId) public view returns (bool sold) {
    ...
    function _markOrderSold(uint256 _orderId, address _buyer) internal {
...
interface IMarketplaceNonCustodial {
    ...
    error ItemAlreadySold(uint256 orderId);
    ...
    function isOrderSold(uint256 _orderId) external view returns (bool sold);
}
