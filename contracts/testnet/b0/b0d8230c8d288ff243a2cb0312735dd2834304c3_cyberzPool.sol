...
function pendingcyberz(uint256 _pid, address _user)
external
view
returns (uint256);
...
contract cyberzPool is Ownable, Pausable {
using SafeERC20 for IERC20;

struct UserInfo {
uint256 shares; // number of shares for a user.
uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
uint256 cyberzAtLastUserAction; // keep track of cyberz deposited at the last user action.
...
IERC20 public immutable token; // cyberz token.
...
/**
 * @notice Constructor
 * @param _token: cyberz token contract
 * @param _masterchefV2: MasterChefV2 contract
 * @param _admin: address of the admin
 * @param _treasury: address of the treasury (collects fees)
 * @param _operator: address of operator
 * @param _pid: cyberz pool ID in MasterChefV2
 */
constructor(
IERC20 _token,
...
masterchefV2.deposit(cyberzPoolPID, balance);
emit Init();
}
...
/**
 * @notice Checks if the msg.sender is either the cyberz owner address or the operator address.
 */
modifier onlyOperatorOrcyberzOwner(address _user) {
require(
msg.sender == _user || msg.sender == operator,
"Not operator or cyberz owner"
);
_;
}
...
IBoostContract(boostContract).oncyberzPoolUpdate(
_user,
user.lockedAmount,
lockDuration,
totalLockedAmount,
...
totalShares -= user.shares;
user.shares = 0;
uint256 earnAmount = totalAmount - user.cyberzAtLastUserAction;
...
/**
 * @notice Unlock user cyberz funds.
 * @dev Only possible when contract not paused.
 * @param _user: User address
 */
function unlock(address _user)
external
onlyOperatorOrcyberzOwner(_user)
whenNotPaused
{
UserInfo storage user = userInfo[_user];
require(
...
/**
 * @notice Deposit funds into the cyberz Pool.
 * @dev Only possible when contract not paused.
 * @param _amount: number of tokens to deposit (in cyberz)
...
/**
 * @notice Withdraw funds from the cyberz Pool.
...
/**
 * @notice Harvest pending cyberz tokens from MasterChef
 */
function harvest() internal {
uint256 pendingcyberz = masterchefV2.pendingcyberz(
cyberzPoolPID,
address(this)
);
if (pendingcyberz > 0) {
uint256 balBefore = available();
masterchefV2.withdraw(cyberzPoolPID, 0);
...
/**
 * @notice Withdraw unexpected tokens sent to the cyberz Pool
 */
...
uint256 pool = balanceOf() + calculateTotalPendingcyberzRewards();
 ...
 * @return Returns total pending cyberz rewards
 */
function calculateTotalPendingcyberzRewards() public view returns (uint256) {
uint256 amount = masterchefV2.pendingcyberz(cyberzPoolPID, address(this));
return amount;
}
...
totalShares == 0
? 1e18
: (((balanceOf() + calculateTotalPendingcyberzRewards()) *
(1e18)) / totalShares);
}
