...
contract Bet247L2 is Pausable {
    ...
    function openMatch(
        ...
        matches[key].result = MATCH_RESULT.NONE;
    ...
    function withdrawMatch(string calldata key) external returns (uint256) {
        ...
        require(
            matches[key]
            .bettors[msg.sender][matches[key].result].numberOfTicket > 0,
            "wrong prediction"
        );
        require(
            matches[key].bettors[msg.sender][matches[key].result].withdraw ==
                false,
            "withdrawn"
        );
