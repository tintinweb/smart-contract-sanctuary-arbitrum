...
contract Battle is IBattle, AccessControl {
	...
	function purchaseHireling(uint8 rarity, uint8 house, uint256 tokenIndex) external payable {
		...
		// TODO transfer payable amount to admin wallet
	...
	function getOwnedHirelingsIndexes() external view returns (uint256[][][] memory hirelingIndexes) {
		return HirelingLogic.getOwnedHirelingsIndexes(_contractData);
	}
...
//		require(!data.nextBattleConfig[msg.sender].ready, "9");
		...
		uint256 cardIndex0 = uint(blockhash(block.number - 1)) % totalSupplyRange;
		uint256 cardIndex1 = uint(blockhash(block.number - 2)) % totalSupplyRange;
		uint256 cardIndex2 = uint(blockhash(block.number - 3)) % totalSupplyRange;
		uint256 cardIndex3 = uint(blockhash(block.number - 4)) % totalSupplyRange;
		...
		// save the battle configuration for the requesting user
		data.nextBattleConfig[msg.sender] = Battle.BattlePrepConfig({
			enemyCards: FinalFormCards(finalFormCardsAddress).tokensByIndexes([cardIndex0, cardIndex1, cardIndex2, cardIndex3]),
...
//				uint8(uint(blockhash(block.number - 18)) % 4)
	...
	function getOwnedHirelingsIndexes(Battle.ContractData storage data) external view returns (uint256[][][] memory hirelingIndexes) {
