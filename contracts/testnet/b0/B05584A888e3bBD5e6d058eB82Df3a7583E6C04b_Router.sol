                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex"255d23c65a2183a297064951b10f961c0bfc879fb6141e632ed568004f180114" // init code hash
                        )
                    )
                )
            )
...
contract Router is IRouter {
    ...
    function _addLiquidity(
        ...
        require(
            IController(factory).pairs(tokenA, tokenB) != address(0),
            "Router: PAIR_NOT_EXIST"
        );
        (uint reserveA, uint reserveB) = Library.getReserves(
            factory,
            tokenA,
            tokenB
        );
    ...
    {
        address pair = Library.pairFor(factory, tokenA, tokenB);
        assert(ILiquidityToken(pair).transferFrom(msg.sender, pair, liquidity)); // send liquidity to pair
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            uint amountOutput;
            {
                ...
                amountInput =
                    ...
                    reserveInput;
                amountOutput = Library.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
...
interface IController {
    ...
    event AuthorizerUpdated(address indexed authorizer, address sender);
    event FeeManagerUpdated(address indexed fee, address sender);
    event RewardManagerUpdated(address indexed reward, address sender);
    event RewardWalletUpdated(address indexed rewardWallet, address sender);
    ...
    event StakingPoolCreated(address indexed pool);

    function allStakingPools(uint) external view returns (address pair);

    function allStakingPoolsLength() external view returns (uint256);
}