...
// HELPER FUNCTIONS
    ...
    /// @dev Add to an allowlist as a reward
        ...
        private 
    {
        ...
        if (random < packs[packID].allowlistThreshold) {                                // If the random number is less that the threshold then
            ...
            accounts[0] = _msgSender();                                                 // Add to the address of the waller that minted the pack
        ...
        )
    {
        ...
        uint256 packItemSet = packs[packID].itemSet;                                // Declare & set the pack item set to work from
        ...
        for (uint i = 0; i < 5;) {                                                  // Loop through the array of rarity %'s
            if (random > packs[packID].rarityPercentage[i] &&
...
// ADMIN FUNCTIONS
    ...
    /// @param allowlistThreshold Threshold for adding to an allowlist
    /// @param allowlistAddress Contract that supports addToAllowlist
    ...
    function createPack(string calldata name, uint256 itemSet, uint256 price, address paymentAddress, bytes32 packMerkleRoot, uint256 maxSupply, uint256 allowlistThreshold, address allowlistAddress, bool active, uint256[5] calldata rarityPercentages)
        external
        onlyOwner
    {
        ...
        pack.allowlistThreshold = allowlistThreshold;                 // Set Threshold for adding to an allowlist
        pack.allowlistAddress = allowlistAddress;                     // Set Contract that supports addToAllowlist
    ...
    /// @param active set the status of the pack
    /// @param rarityPercentages an array of the rarity percentage for item distribution eg [400, 150, 50, 10, 0]
    function updatePack(uint256 packID, string calldata name, uint256 itemSet, uint256 price, address paymentAddress, bytes32 packMerkleRoot, uint256 maxSupply, uint256 allowlistThreshold, address allowlistAddress, bool active, uint256[5] calldata rarityPercentages)
        public
        onlyOwner
    {
        require(rarityPercentages.length == 5,                        "Requires 5 rarity values");
