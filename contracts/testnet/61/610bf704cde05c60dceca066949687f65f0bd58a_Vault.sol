...
contract Vault is Constants, ReentrancyGuard, Ownable, IVault {
    ...
    mapping(address => EnumerableMap.AddressToUintMap) private bonds;
    ...
    function takeAssetIn(
        ...
        _increaseBalance(bonds[_account], _token, _amount);
        emit TakeAssetIn(_account, address(0), amountInUSD, 0, _token, _amount);
    }
    ...
    function takeAssetBack(
        ...
        _isRouter(msg.sender, true);
        require(_tryGet(bonds[_account], _token) >= _amount, "Insufficient bond to take back");
        IERC20(_token).safeTransfer(_account, _amount);
        _decreaseBalance(bonds[_account], _token, _amount);
        emit TakeAssetBack(_account, _amount, _token, _tokenPrice, _key, _txType);
    }
    ...
    function reduceBond(
        ...
        _isRouter(msg.sender, true);
        require(_tryGet(bonds[_account], _token) >= _amount, "Insufficient bond to reduce");
        _decreaseBalance(bonds[_account], _token, _amount);
        emit ReduceBond(_account, _amount, _token, _key, _txType);
    }
    ...
    function _takeAssetOut(
        ...
        uint256 tokenAmountOut = priceManager.fromTokenToUSD(_token, usdOutAfterFee, _tokenPrice);
    ...
    function _distributeFee(address _account, address _refer, uint256 _fee, address _token) internal {
        ...
        if (_fee > 0) {
            emit TakeAssetIn(_account, _refer, 0, _fee, _token, 0);
        }
    }
