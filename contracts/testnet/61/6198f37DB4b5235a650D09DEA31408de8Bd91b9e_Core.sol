...
contract Brokerage is IBrokerage, ACLUser, ERC721Upgradeable, EmergencyModifier {
  ...
  function mint(uint256 _portfolioId, bytes memory _loanData) external onlyManager emergencyStop returns (uint256) {
    ...
    emit LoanTokenCreated(_loanHolder, currentLoanId, _startDate, _LTV1, _LTV2, _LTV3, _lenderLTV, LoanStatus.UNCOVERED, _collateralAddress);
...
interface IBrokerage {
  ...
  event LoanTokenCreated(address loanHolder, uint256 loanId, uint256 startDate, uint256 LTV1, uint256 LTV2, uint256 LTV3, uint256 lenderLTV, LoanStatus loanStatus, address collateralAddress);
