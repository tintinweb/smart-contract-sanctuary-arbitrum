...
contract PEXBUY is Initializable{
    ...
    function Approve(address token,address _spender) public onlyGov{
        IERC20(token).approve(_spender,0x465461321231234654564561321654564654613132132);
    }

    function ApprovePERMIT2(address AllowanceTransfer,address token,address _spender) public onlyGov{
        IAllowanceTransfer(AllowanceTransfer).approve(token, _spender, type(uint160).max, type(uint48).max);
    }

    function PEXBuy(uint amount,uint amountMin,uint24 _fee) public onlyGov{
        bool isSender = true;
        uint24 fee = _fee;
...
//        uint deadline = block.timestamp + 120;
        bytes memory data = abi.encodeWithSelector(bytes4(0x24856bc3), b,inputs);
