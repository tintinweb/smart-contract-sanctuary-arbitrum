...
contract ClipFinanceNFT is ERC721Enumerable, AccessControl {
    ...
    function reserveNFT(
        ...
        for (uint256 i = 0; i < _amount; i++) {
            uint256 mintIndex = _team * teamMembers + teamMembersMinted[_team];
            mintIndex += 1;
            teamMembersMinted[_team] = mintIndex;
            require(
                mintIndex <= maxNFTs && mintIndex <= (_team + 1) * teamMembers,
                "Sold Out"
            );
    ...
    function mintNFTs(uint256 _numberOfTokens, uint256 _team) public payable {
        ...
        for (uint256 i = 0; i < _numberOfTokens; i++) {
            uint256 mintIndex = _team * teamMembers + teamMembersMinted[i];
            mintIndex += 1;
            teamMembersMinted[i] = mintIndex;
            require(
                mintIndex <= maxNFTs && mintIndex <= (_team + 1) * teamMembers,
                "Sold Out"
            );
