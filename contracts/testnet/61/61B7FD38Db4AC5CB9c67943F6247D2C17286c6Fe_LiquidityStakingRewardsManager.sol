...
contract LiquidityStakingRewards is ILiquidityStakingRewards, PeripheryPayments, ERC721Holder, ReentrancyGuard, Ownable {
    ...
    function decreaseLiquidityStake(uint128 liquidityAmount, uint256 amount0Min, uint256 amount1Min) public
        ...
        //
        INonfungiblePositionManager(positionManagerAddress).collect(
        ...
        (,,address token0, address token1,,,,,,,,) = INonfungiblePositionManager(positionManagerAddress).positions(liquidityTokenId);
        //
        uint256 totalAmount0 = amount0.add(collectAmount0);
        uint256 totalAmount1 = amount1.add(collectAmount1);
        // safeTransfer
        if(IERC20(token0).balanceOf(address(this)) > totalAmount0){
            IERC20(token0).safeTransfer(_msgSender(), totalAmount0);
        }
        if(IERC20(token1).balanceOf(address(this)) > totalAmount1){
            IERC20(token1).safeTransfer(_msgSender(), totalAmount1);
        }*/
    }
...
contract LiquidityStakingRewardsManager is RewardsManager {
    ...
    ) RewardsManager(rewardsToken_, 100e18) {
        positionManagerAddress = positionManagerAddress_;
        WETH9 = WETH9_;
    }

    function viewLiquidityStakingRewardsInfos(uint256 startIndex_, uint256 endIndex_) external view returns (LiquidityStakingRewardsInfo[] memory liquidityStakingRewardsInfos){
        if (startIndex_ >= 0 && endIndex_ >= startIndex_) {
            ...
            uint arrayIndex_ = 0;
            for (uint i_ = startIndex_; i_ < ((endIndex_ > total) ? total : endIndex_);) {
    ...
    function createLiquidityStakingRewards(uint256 liquidityTokenId_) external onlyOwner {
        LiquidityStakingRewards liquidityStakingRewards = new LiquidityStakingRewards(positionManagerAddress, rewardsToken, WETH9, liquidityTokenId_);
