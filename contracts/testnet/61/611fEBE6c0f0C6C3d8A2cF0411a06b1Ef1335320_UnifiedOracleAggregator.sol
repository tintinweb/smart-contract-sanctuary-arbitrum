...
/// @dev TODO outsource this to an interface contract
...
/// @dev double check hardcoded addresses
contract UnifiedOracleAggregator is IPriceOracle, SafeOwnable {
    ...
    constructor(
        ...
        emit SetOracle(WETH, address(_wethUsdLinkOracle));
    }

    /// @notice sets a Chainlink oracle, all prices for this token of all pairs will be retrieved from this oracle
    /// @dev set to address(0) for deleting the oracle
    /// @dev TODO could add oracle liveness check, e.g. require _linkOracle.latestTimestamp > (block.timestamp - X)
    ...
    /// @notice returns true if a given token has an oracle
    /// @notice if no linkOracle is set -> checks for uni TWAP oracle
    ...
    /// @dev if Chainlink is used, the price is converted from usd to ETH
    /// @dev ChainLink Oracles will have 8 decimals, unless it is an ETH-Pair, then it will return 18 decimals
    /// @dev StaticOracle uses the oracle library to get getWeightedArithmeticMeanTick
    ...
    function tokenPrice(address _token) public view override returns (uint256) {
        ...
        if (address(linkOracles[_token]) != address(0)) {
            ...
            if (IExternalOracle(linkOracles[_token]).decimals() == 8) {
                // This means we have an BaseAsset<->USD Pair
        ...
        require(
            ...
            "UnifiedOracleAggregator: No Uniswap orcacle found"
        );
