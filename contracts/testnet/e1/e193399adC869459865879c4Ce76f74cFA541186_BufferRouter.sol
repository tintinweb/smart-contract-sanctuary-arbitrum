...
contract BufferRouter is AccessControl, IBufferRouter {
    ...
    constructor(address _publisher, address _sfPublisher, IPyth _pyth) {
    ...
    function initiateTrade(
        ...
        uint256 amountToPay = (params.maxFeePerContract * params.contracts) +
            config.platformFee();
    ...
    function resolveQueuedTrades(
        ...
        for (uint32 index = 0; index < params.length; index++) {
            ResolveTradeParams memory currentParams = params[index];
            ...
            SignInfo memory publisherSignInfo = currentParams.publisherSignInfo;
            SignInfo memory sfSignInfo = currentParams.sfSignInfo;
            ...
            if (!isSfVerified) {
                emit FailResolve(
                    currentParams.queueId,
                    "Router: SF isn't valid"
                );
                continue;
            }
            if (
                !queuedTrade.isQueued ||
                publisherSignInfo.timestamp != queuedTrade.queuedTime
            ) {
            ...
            if (sfSignInfo.timestamp < block.timestamp) {
                // If the settlement fee has expired then cancel the trade
                _cancelQueuedTrade(currentParams.queueId);
                emit CancelTrade(
                    queuedTrade.user,
                    currentParams.queueId,
                    "Settlement fee has expired"
                );
                continue;
            }
            ...
            if (block.timestamp - queuedTrade.queuedTime <= MAX_WAIT_TIME) {
                _openQueuedTrade(currentParams, publisherSignInfo.timestamp);
            } else {
                _cancelQueuedTrade(currentParams.queueId);
                emit CancelTrade(
                    queuedTrade.user,
    ...
    function _validateSigner(
        ...
        bytes32 digest = ECDSA.toEthSignedMessageHash(
            keccak256(abi.encodePacked(assetPair, timestamp, ip))
        );
    ...
    function _openQueuedTrade(
        ResolveTradeParams memory params,
        uint256 timestamp
    ) internal {
        QueuedTrade storage queuedTrade = queuedTrades[params.queueId];
        IBufferBinaryOptions optionsContract = IBufferBinaryOptions(
            queuedTrade.targetContract
        );
        ...
        uint256 revisedSf;
        uint256 initialFee = queuedTrade.maxFeePerContract *
        ...
        try optionsContract.evaluateParams(optionParams) returns (
            uint256 _amount,
            uint256 _fee,
            uint256 _revisedContracts,
            uint256 _revisedSf
        ) {
            (amount, fee, revisedContracts, revisedSf) = (
                _amount,
                _fee,
                _revisedContracts,
                _revisedSf
            ...
            emit CancelTrade(queuedTrade.user, params.queueId, reason);
            return;
        }

        if (revisedContracts == 0) {
            _cancelQueuedTrade(params.queueId);
            emit CancelTrade(
                queuedTrade.user,
                params.queueId,
        ...
        tokenX.safeTransfer(queuedTrade.targetContract, fee);

        // Refund the user in case the trade amount was lesser
        if (fee < initialFee) {
            tokenX.safeTransfer(queuedTrade.user, initialFee - fee);
        }
        optionParams.amount = amount;
        optionParams.totalFee = fee;
        optionParams.contracts = revisedContracts;
        optionParams.sf = revisedSf;
        ...
        optionIdMapping[queuedTrade.targetContract][optionId] = params.queueId;
        emit OpenTrade(
            queuedTrade.user,
            params.queueId,
            optionId,
    ...
    function _cancelQueuedTrade(uint256 queueId) internal {
        ...
        ERC20(optionsContract.tokenX()).safeTransfer(
            queuedTrade.user,
            queuedTrade.maxFeePerContract * queuedTrade.contracts
        );
