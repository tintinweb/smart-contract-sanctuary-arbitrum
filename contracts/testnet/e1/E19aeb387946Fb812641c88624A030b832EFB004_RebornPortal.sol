...
interface IPiggyBankDefinition {
    ...
    event SetNewCountDownBlockLong(uint32 countDownBlockLong);
    ...
    AutomationCompatible
{
    ...
    function setNativeDropRoot(
        ...
        // update last drop timestamp, no back to specific hour, for frontend show
...
import {CommonError} from "src/lib/CommonError.sol";
import {IPiggyBank} from "src/interfaces/IPiggyBank.sol";
import {PiggyBankStorage} from "src/PiggyBankStorage.sol";
import {ReentrancyGuardUpgradeable} from "src/oz/contracts-upgradeable/security/ReentrancyGuardUpgradeable.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function stop(uint256 season) external override onlyPortal {
        ...
        emit SeasonStopped(season, block.number);
    }
    ...
    function _toNextRound(
        ...
        RoundInfo storage roundInfo = rounds[season];
        roundInfo.currentIndex++;
        roundInfo.startBlock = uint32(block.number);
    ...
    function checkIsSeasonEnd(uint256 season) public view returns (bool) {
        ...
        bool isAutoEnd = (rounds[season].totalAmount < rounds[season].target) &&
            block.number >= (countDownBlockLong + rounds[season].startBlock);
...
import {IPiggyBankDefinition} from "src/interfaces/IPiggyBank.sol";
...
import {SafeERC20Upgradeable} from "src/oz/contracts-upgradeable/token/ERC20/utils/SafeERC20Upgradeable.sol";
...
import {IDegenNFTDefinition} from "src/interfaces/nft/IDegenNFT.sol";
    ...
    PausableUpgradeable
{
    ...
    function merge(
        ...
        bool propertiesEq = checkPropertiesEq(tokenId1, tokenId2);
        if (!propertiesEq) {
            revert InvalidTokens();
        }

        // only shards can merge
        IDegenNFTDefinition.Property memory property = degenNFT.getProperty(
            tokenId1
        );
    ...
    function openMysteryBox(
        uint256[] calldata tokenIds,
        IDegenNFTDefinition.Property[] calldata metadataList
    ) external onlySigner {
    ...
    function burn(
        ...
        // level == 0 && not openMysteryBox
        if (level == 0) {
            if (tokenId <= SUPPORT_MAX_MINT_COUNT) {
                IDegenNFTDefinition.Property memory token1Property = degenNFT
                    .getProperty(tokenId);
        ...
        // refund NativeToken
        payable(msg.sender).sendValue(refundConfig.nativeToken);
    ...
    // set white list merkle tree root
    ...
    function checkPropertiesEq(
        ...
        IDegenNFTDefinition.Property memory token2Property = degenNFT
...
interface INFTManagerDefinition {
    enum StageType {
        Invalid,
        WhitelistMint,
        PublicMint,
...
interface INFTManager is INFTManagerDefinition {
    /**
...
import {BitMapsUpgradeable} from "@openzeppelin/contracts-upgradeable/utils/structs/BitMapsUpgradeable.sol";
import {INFTManagerDefinition} from "src/interfaces/nft/INFTManager.sol";
import {IDegenNFTDefinition} from "src/interfaces/nft/IDegenNFT.sol";

import {DegenNFT} from "src/nft/DegenNFT.sol";

contract NFTManagerStorage is INFTManagerDefinition {
    // degen nft address
