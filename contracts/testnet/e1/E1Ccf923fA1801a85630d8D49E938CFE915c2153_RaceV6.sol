...
abstract contract Game is Ownable, Pausable, Multicall, ReentrancyGuard {
    ...
    /**
        TODO
        IBank interface - harvestDividends() function
     */
    ...
    IBank public bank; // immutable

    function setBank(address _bankAddress) external onlyOwner {
        if (_bankAddress == address(0)) {
            revert InvalidAddress();
        }
        bank = IBank(_bankAddress);
...
import {INftManager} from "../NFTManager/INftManager.sol";

import {IERC20, SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

/// @title nekopachi slot
/// @author munji ([email protected])
contract RaceV6 is Game {
    using SafeERC20 for IERC20;
    ...
    /// @param _supraRouterAddress of the Supra VRF Coordinator.
    /// @param _houseEdge The house edge.
    constructor(
        address bankAddress,
        address _supraRouterAddress,
        uint16 _houseEdge,
    ...
    uint8 public constant NUM_OF_PLAYERS = 3;
    ...
    struct Race {
        ...
        address token;
        uint256 betUnit;
        uint256[] playerTokenIds; //tokenIds
        uint256 winnerTokenId;
        uint256 winnerSpeed;
    }

    uint256 public lastRaceId;
    mapping(uint256 => Race) public races;
    mapping(uint256 => uint256) public randIdToRaceId;
    uint256[] public openRaces;
    ...
    event PlayerEntered(uint256 raceId, uint256 tokenId);

    event RequestedRaceWinner(uint256 raceId, uint256 randId);
    ...
    function initRace(uint256 betUnit, address token) external whenNotPaused {
        /** 
            this function may only be called by predefined address (maybe automation address)
        */
        if (openRaces.length > 30) {
            revert("Too many ongoing races");
        }
        ...
        uint256 maxBetLimit = bank.getMaxBetAmount(
            token,
            10000 * NUM_OF_PLAYERS
        );
        if (betUnit * NUM_OF_PLAYERS > maxBetLimit) {
            revert("betUnit exceed maxBetAmount, please contact support");
        }
        uint256 minBetUnit = bank.getMinBetUnit(token);
        if (betUnit < minBetUnit * 10) {
            revert UnderMinBetUnit(betUnit);
        }

        uint256 maxBetUnit = bank.getMaxBetUnit(token);
        if (betUnit > maxBetUnit * 10) {
            revert ExceedMaxBetUnit(betUnit);
        }

        lastRaceId++;
        races[lastRaceId].startTime = block.timestamp;
        races[lastRaceId].isDone = false;
        races[lastRaceId].dueTime = block.timestamp + 1 days;
        races[lastRaceId].betUnit = betUnit;
        races[lastRaceId].token = token;
        openRaces.push(lastRaceId);

        emit RaceInitiated(
            lastRaceId,
            races[lastRaceId].startTime,
            races[lastRaceId].dueTime,
            token,
            betUnit
        );
    }
    ...
    function enterRace(
        ...
        require(nftManager.getOwnerOf(_tokenId) == msg.sender, "not owner");
        require(
            nftManager.isNftTransferable(_tokenId),
            "not in a transferable state"
        );
        require(!races[_raceId].isDone, "is already done");
        require(
            block.timestamp >= races[_raceId].startTime &&
                block.timestamp <= races[_raceId].dueTime,
            "not in a valid time"
        );

        uint256 numOfPlayers = races[_raceId].playerTokenIds.length;
        bool isGasToken = races[_raceId].token == address(0);
        if (isGasToken && msg.value < races[_raceId].betUnit) {
            revert("you sent less than betUnit");
        }

        if (numOfPlayers > NUM_OF_PLAYERS) {
            // call endRace function (if needed)
            revert("player limit exceeded");
        }
        ...
        if (!isGasToken) {
            IERC20(races[_raceId].token).safeTransferFrom(
                msg.sender,
                address(this),
                races[_raceId].betUnit
            );
        }

        nftManager.updatePendingStatus(_tokenId, true);
        races[_raceId].playerTokenIds.push(_tokenId);

        emit PlayerEntered(_raceId, _tokenId);
    }

    function endRace(uint256 _raceId) external {
        require(races[_raceId].isDone == false, "race is already done");

        (bool canExec, ) = checker();
        require(canExec, "Can't execute yet");

        uint8 rngCount = uint8(races[_raceId].playerTokenIds.length);
        ...
        randIdToRaceId[id] = _raceId;

        emit RequestedRaceWinner(_raceId, id);
    }

    /// @notice Resolves the bet using the Chainlink randomness.
    /// @param _id The bet ID.
    /// @param _randomNumbers Random numbers list. Contains only one for this game.
    function _callback(
        uint256 _id,
        ...
        address winnerAddress = nftManager.getOwnerOf(winnerTokenId);
        ...
        for (uint16 i = 0; i < uint16(numOfPlayers); i++) {
            ...
            if (i == winnerIndex) {
                if (isGasToken) {
                    ...
                    IERC20(_token).safeTransfer(address(bank), betAmountFee); // betAmountFee
                }

                // Game 컨트랙트의 기본적인 구조로, 다른 게임컨트랙과 규격을 맞춘거임... 딱히 나눌 필요는 없긴한듯

                // Transfer the payout from the bank, the bet amount fee to the bank, and account fees.
                bank.payout{value: isGasToken ? betAmountFee : 0}(
                    winnerAddress,
                    _token,
                    totalInput - betUnit - (betAmountFee * 2), // profit (게이머 입장에서 수익)
                    fee, // total fees (for allocation, 플랫폼 입장에서 수익)
                    betAmountFee
                );
                ...
                bank.cashIn(
                    nftManager.getOwnerOf(playerIds[i]),
                    _token,
                    betUnit, // cashIn
                    0,
                    betAmountFee
    ...
    function refundRace(uint256 _raceId) external {
        Race storage race = races[_raceId];
        if (race.isDone == true || race.startTime == 0) {
            revert NotPendingBet();
        }

        // disabled for test purpose
        // if (race.dueTime > block.timestamp + 15 minutes) {
        //     revert NotFulfilled();
        // }
        ...
        for (uint16 i = 0; i < uint16(numOfPlayers); i++) {
            uint256 tokenId = race.playerTokenIds[i];
            address owner = nftManager.getOwnerOf(tokenId);

            nftManager.updatePendingStatus(tokenId, false);
            if (race.token == address(0)) {
                (bool sent, ) = payable(owner).call{value: race.betUnit}("");
                require(sent, "Failed to send Ether");
            } else {
                IERC20(race.token).safeTransfer(owner, race.betUnit);
            }
        }
        ...
        emit RaceRefunded(_raceId, race.endTime, race.playerTokenIds);
    }
    ...
    {
        for (uint256 i; i < openRaces.length; i++) {
            uint256 __raceId = openRaces[i];

            if (races[__raceId].playerTokenIds.length >= NUM_OF_PLAYERS) {
                canExec = true;
            ...
            if (races[__raceId].dueTime < block.timestamp) {
                canExec = true;
                execPayload = abi.encodeWithSignature(
                    "endRace(uint256)",
                    __raceId
    ...
    function removeOpenRace(uint256 raceId) internal {
        uint256[] memory temp = new uint256[](openRaces.length - 1);
        uint256 index = 0;
        for (uint256 i; i < openRaces.length; i++) {
            if (openRaces[i] != raceId) {
                temp[index] = openRaces[i];
                index++;
            }
        }
    ...
    function getNumOfOpenRaces() external view returns (uint256) {
        return openRaces.length;
    }

    function getRaceDetail(
        uint256 _raceId
    ) external view returns (Race memory, bool isTimeOver) {
        Race memory race = races[_raceId];
        isTimeOver = races[_raceId].dueTime < block.timestamp;

        return (race, isTimeOver);
    }
}
...
interface INftManager {
    ...
    function updatePendingStatus(uint256 tokenId, bool isPending) external;

    function writeMatchResult(uint256 tokenId, bool matchResult) external;
    ...
    function getSupporterTokenId(address user) external view returns (uint256);
}