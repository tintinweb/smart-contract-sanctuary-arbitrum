...
contract BlpManager is ReentrancyGuard, Governable, IBlpManager {
    ...
    function _addLiquidity(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdg, uint256 _minGlp) private returns (uint256) {
        ...
        require(usdgAmount >= _minUsdg, "BlpManager: insufficient BXD output");

        uint256 mintAmount = aumInUsdg == 0 ? usdgAmount : usdgAmount.mul(glpSupply).div(aumInUsdg);
        require(mintAmount >= _minGlp, "BlpManager: insufficient BLP output");
    ...
    function _removeLiquidity(address _account, address _tokenOut, uint256 _glpAmount, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        if (usdgAmount > usdgBalance) {
            IBXD(usdg).mint(address(this), usdgAmount.sub(usdgBalance));
        }
...
contract GroupOrderBook is ReentrancyGuard, IGroupOrderBookk {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address payable;
    ...
    function getGroupOrder(
        ...
        for (uint256 i = 0; i < limit; i++) {
            GroupOrder memory item = listOrders[offset + i];
            items[i] = item;
        }
        return items;
    }
    ...
    function getBuyOrderByAddress(
        ...
        uint256 limit = _limit < listBuyOrdersIndex[msg.sender] ? _limit : listBuyOrdersIndex[msg.sender];
        BuyOrder[] memory items = new BuyOrder[](limit);
        GroupOrder[] memory groupItems = new GroupOrder[](limit);
        for (uint256 i = 0; i < limit; i++) {
            uint256 orderId = listBuyOrders[account][offset + i];
    ...
    function getSubGroupOrder(
        ...
        uint256 limit = _limit < subOrdersIndex[orderId] ? _limit : subOrdersIndex[orderId];
        SubOrder[] memory items = new SubOrder[](limit);
        for (uint256 i = 0; i < limit; i++) {
            uint256 subOrderId = subOrders[orderId][offset + i];
    ...
    function getAmountOut(uint256 _amountIn, address[] memory _path, uint256 discount) public view returns (uint256, uint256, uint256) {
        ...
        amountOut = IVault(vault).adjustForDecimals(amountOut, _tokenIn, _tokenOut);
        amountOut = amountOut.mul(BASIS_POINTS_DIVISOR).div(BASIS_POINTS_DIVISOR.sub(discount));
        return (priceIn, priceOut, amountOut);
    }

    function _wrapAndTransfer(uint256 _amountIn, address _path0, bool _shouldWrap) private {
        if (_shouldWrap) {
    ...
    function validatePair(
        ...
        require(_path.length == 2 || _path.length == 3, "path.length");
        require(_path[0] != _path[_path.length - 1], "path");
        require(_buyPath.length == 2 || _buyPath.length == 3, "_buyPath.length");
    ...
    function buyGroupOrder(
        ...
        require(item.groupAmount == 0 || _amountIn >= item.groupAmount, "_amountIn");
        ...
        listOrders[orderId].totalOut = listOrders[orderId].totalOut.add(amountOut);
        listOrders[orderId].totalIn = listOrders[orderId].totalIn.add(_amountIn);

        BuyOrder memory buyOrder;
        if(buyOrders[msg.sender][orderId].orderId > 0) {
            buyOrder = buyOrders[msg.sender][orderId];
            buyOrder.amountIn = buyOrder.amountIn.add(_amountIn);
            buyOrder.amountOut = buyOrder.amountOut.add(amountOut);
            buyOrder.times = buyOrder.times.add(1);
        } else {
            buyOrder = BuyOrder(
                orderId,
                1,
            ...
            listBuyOrdersIndex[msg.sender] = listBuyOrdersIndex[msg.sender].add(1);
            listBuyOrders[msg.sender][listBuyOrdersIndex[msg.sender]] = orderId;
        }
        buyOrders[msg.sender][orderId] = buyOrder;
    ...
    function createSubGroupOrder(
        ...
        listBuySubOrdersIndex[msg.sender] = listBuySubOrdersIndex[msg.sender].add(1);
        listBuySubOrders[msg.sender][listBuySubOrdersIndex[msg.sender]] = listSubOrdersIndex;
    ...
    function placeSubGroupOrder(
        ...
        require(item.groupAmount > 0 && subOrder.totalIn < item.groupAmount, "done");

        uint256 amountOut;
        (, , amountOut) = getAmountOut(_amountIn, _path, item.discount);    
        require(item.totalOut.add(amountOut) <= item.amountIn, "soldout");
        if(subOrder.totalOut.add(amountOut) >= item.groupAmount && item.groupAmount > 0 && subOrder.totalOut.add(amountOut) > item.amountIn) {
            revert("subOrder");
        }
        ...
        if(item.groupAmount > 0 && listSubOrders[subOrderId].totalIn >= item.groupAmount) {
            GroupOrder memory groupOrder = listOrders[orderId];
            groupOrder.totalOut = groupOrder.totalOut.add(subOrder.totalOut);
            groupOrder.totalIn = groupOrder.totalIn.add(subOrder.totalIn);
            listOrders[orderId] = groupOrder;
        }
        ...
        if(buySubOrders[msg.sender][subOrderId].subOrderId > 0) {
            ...
            listBuySubOrdersIndex[msg.sender] = listBuySubOrdersIndex[msg.sender].add(1);
            listBuySubOrders[msg.sender][listBuySubOrdersIndex[msg.sender]] = subOrderId;
        }
    ...
    function ownerClaimOrder(
            uint256 orderId
    ) external nonReentrant {
        GroupOrder memory item = listOrders[orderId];
        require(item.account == msg.sender, "owner");
        require(item.totalIn > 0, "totalIn");
        require(item.expired < block.timestamp, "expired");
        require(item.totalOut > item.amountIn.mul(item.threshold).div(BASIS_POINTS_DIVISOR), "threshold");
    ...
    function claimOrder(
        ...
        require(buyOrders[msg.sender][orderId].amountOut > 0, "amountOut");

        address _orderIn = item.path.length == 2 ? item.path[0] : item.path[1];
        uint256 amountOut = buyOrders[msg.sender][orderId].amountOut;
    ...
    function claimSubOrder(
        ...
        GroupOrder memory item = listOrders[orderId];
        require(item.expired < block.timestamp, "expired");
        require(item.totalOut > item.amountIn.mul(item.threshold).div(BASIS_POINTS_DIVISOR), "totalIn");
        ...
        require(buySubOrders[msg.sender][subOrderId].amountOut > 0, "amountOut");
...
contract GroupSellingOrderBook is ReentrancyGuard, IGroupOrderBookk {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address payable;
...
contract LeaderOrderBook is ReentrancyGuard, IGroupOrderBookk {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address payable;

    uint256 public constant DISCOUNT_PRECISION = 10000;
    ...
    function getAmountOut(uint256 _amountIn, address[] memory _path, uint256 discount) public view returns (uint256, uint256, uint256) {
        ...
        uint256 amountOut = _amountIn.mul(priceIn).div(priceOut);
        amountOut = IVault(vault).adjustForDecimals(amountOut, _tokenIn, _tokenOut);
        amountOut = amountOut.mul(DISCOUNT_PRECISION).div(DISCOUNT_PRECISION.sub(discount));
        return (priceIn, priceOut, amountOut);
    }
    ...
    function placeSubGroupOrder(
        ...
        listSubOrders[subOrderId].totalOut = subOrder.totalOut.add(amountOut);
        listSubOrders[subOrderId].totalIn = subOrder.totalIn.add(_amountIn);
        if(item.groupAmount > 0 && listSubOrders[subOrderId].totalIn >= item.groupAmount) {
            listOrders[orderId].totalOut = listOrders[orderId].totalOut.add(listSubOrders[subOrderId].totalOut);
            listOrders[orderId].totalIn = listOrders[orderId].totalIn.add(listSubOrders[subOrderId].totalIn);
        }
    ...
    function ownerClaimOrder(
        ...
        require(item.totalIn > 0, "totalIn");
        require(item.expired < block.timestamp, "expired");
        require(item.totalOut > item.amountIn.mul(item.threshold).div(DISCOUNT_PRECISION), "threshold");
    ...
    function claimSubOrder(
        ...
        GroupOrder memory item = listOrders[orderId];
        require(item.expired < block.timestamp, "expired");
        require(item.totalOut > item.amountIn.mul(item.threshold).div(DISCOUNT_PRECISION), "totalIn");
...
contract OrderBook is ReentrancyGuard, IOrderBook {
    ...
    function getUsdgMinPrice(address _otherToken) public view returns (uint256) {
        // USDG_PRECISION is the same as 1 BXD
...
contract Vault is ReentrancyGuard, IVault {
    ...
    // usdgAmounts tracks the amount of BXD debt for each whitelisted token
    mapping (address => uint256) public override usdgAmounts;

    // maxUsdgAmounts allows setting a max amount of BXD debt for a token
    ...
    function buyUSDG(address _token, address _receiver) external override nonReentrant returns (uint256) {
        ...
        IBXD(usdg).mint(_receiver, mintAmount);
    ...
    function sellUSDG(address _token, address _receiver) external override nonReentrant returns (uint256) {
        ...
        IBXD(usdg).burn(address(this), usdgAmount);
    ...
    function adjustForDecimals(uint256 _amount, address _tokenDiv, address _tokenMul) public view override returns (uint256) {
        uint256 decimalsDiv = _tokenDiv == usdg ? BXD_DECIMALS : tokenDecimals[_tokenDiv];
        uint256 decimalsMul = _tokenMul == usdg ? BXD_DECIMALS : tokenDecimals[_tokenMul];
...
contract MigrationHandler is ReentrancyGuard {
    ...
    function refund(
        ...
        uint256 iouTokenAmount = _usdgAmount.div(2); // each BXP is priced at $2
...
contract GmxTimelock is IGmxTimelock {
    ...
    function redeemUsdg(address _vault, address _token, uint256 _amount) external onlyAdmin {
        ...
        IBXD(usdg).addVault(address(this));

        IBXD(usdg).mint(address(this), _amount);
        ...
        IBXD(usdg).removeVault(address(this));
    }
...
contract Reader is Governable {
    ...
    function getAmountOut(IVault _vault, address _tokenIn, address _tokenOut, uint256 _amountIn) public view returns (uint256, uint256) {
        ...
        uint256 feeBasisPoints;
        {
            ...
            usdgAmount = usdgAmount.mul(10 ** BXD_DECIMALS).div(10 ** tokenInDecimals);
...
contract Timelock is ITimelock {
    ...
    function initGlpManager() external onlyAdmin {
        IBlpManager _glpManager = IBlpManager(glpManager);

        IMintable glp = IMintable(_glpManager.glp());
        glp.setMinter(glpManager, true);

        IBXD usdg = IBXD(_glpManager.usdg());
    ...
    function updateUsdgSupply(uint256 usdgAmount) external onlyKeeperAndAbove {
        address usdg = IBlpManager(glpManager).usdg();
        ...
        if (usdgAmount > balance) {
            ...
            IBXD(usdg).mint(glpManager, mintAmount);
        } else {
            uint256 burnAmount = balance.sub(usdgAmount);
            IBXD(usdg).burn(glpManager, burnAmount);
        }
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external onlyAdmin {
        IBlpManager(glpManager).setShortsTrackerAveragePriceWeight(_shortsTrackerAveragePriceWeight);
    }
    ...
    function setGlpCooldownDuration(uint256 _cooldownDuration) external onlyAdmin {
        ...
        IBlpManager(glpManager).setCooldownDuration(_cooldownDuration);
    }
...
contract RewardRouter is ReentrancyGuard, Governable {
    ...
    address public glp; // BXP Liquidity Provider token
    ...
    function mintAndStakeGlp(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minGlp) external nonReentrant returns (uint256) {
        ...
        address account = msg.sender;
        uint256 glpAmount = IBlpManager(glpManager).addLiquidityForAccount(account, account, _token, _amount, _minUsdg, _minGlp);
    ...
    function mintAndStakeGlpETH(uint256 _minUsdg, uint256 _minGlp) external payable nonReentrant returns (uint256) {
        ...
        address account = msg.sender;
        uint256 glpAmount = IBlpManager(glpManager).addLiquidityForAccount(address(this), account, weth, msg.value, _minUsdg, _minGlp);
    ...
    function unstakeAndRedeemGlp(address _tokenOut, uint256 _glpAmount, uint256 _minOut, address _receiver) external nonReentrant returns (uint256) {
        ...
        uint256 amountOut = IBlpManager(glpManager).removeLiquidityForAccount(account, _tokenOut, _glpAmount, _minOut, _receiver);
    ...
    function unstakeAndRedeemGlpETH(uint256 _glpAmount, uint256 _minOut, address payable _receiver) external nonReentrant returns (uint256) {
        ...
        IRewardTracker(stakedGlpTracker).unstakeForAccount(account, feeGlpTracker, _glpAmount, account);
        IRewardTracker(feeGlpTracker).unstakeForAccount(account, glp, _glpAmount, account);
        uint256 amountOut = IBlpManager(glpManager).removeLiquidityForAccount(account, weth, _glpAmount, _minOut, address(this));
...
contract RewardRouterV2 is IRewardRouterV2, ReentrancyGuard, Governable {
    ...
    function mintAndStakeGlp(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minGlp) external nonReentrant returns (uint256) {
        ...
        emit StakeBlp(account, glpAmount);

        return glpAmount;
    }

    function mintAndStakeBlp(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minGlp) external nonReentrant returns (uint256) {
        require(_amount > 0, "RewardRouter: invalid _amount");
        address account = msg.sender;
        uint256 glpAmount = IBlpManager(glpManager).addLiquidityForAccount(account, account, _token, _amount, _minUsdg, _minGlp);
        IRewardTracker(feeGlpTracker).stakeForAccount(account, account, glp, glpAmount);
    ...
    function mintAndStakeBlpETH(uint256 _minUsdg, uint256 _minGlp) external payable nonReentrant returns (uint256) {
        require(msg.value > 0, "RewardRouter: invalid msg.value");
        IWETH(weth).deposit{value: msg.value}();
        IERC20(weth).approve(glpManager, msg.value);
        address account = msg.sender;
    ...
    function unstakeAndRedeemGlp(address _tokenOut, uint256 _glpAmount, uint256 _minOut, address _receiver) external nonReentrant returns (uint256) {
        ...
        emit UnstakeBlp(account, _glpAmount);

        return amountOut;
    }

    function unstakeAndRedeemBlp(address _tokenOut, uint256 _glpAmount, uint256 _minOut, address _receiver) external nonReentrant returns (uint256) {
        require(_glpAmount > 0, "RewardRouter: invalid _glpAmount");
        address account = msg.sender;
        IRewardTracker(stakedGlpTracker).unstakeForAccount(account, feeGlpTracker, _glpAmount, account);
        IRewardTracker(feeGlpTracker).unstakeForAccount(account, glp, _glpAmount, account);
    ...
    function unstakeAndRedeemBlpETH(uint256 _glpAmount, uint256 _minOut, address payable _receiver) external nonReentrant returns (uint256) {
        require(_glpAmount > 0, "RewardRouter: invalid _glpAmount");
        address account = msg.sender;
        IRewardTracker(stakedGlpTracker).unstakeForAccount(account, feeGlpTracker, _glpAmount, account);
        IRewardTracker(feeGlpTracker).unstakeForAccount(account, glp, _glpAmount, account);
    ...
    function claimEsGmx() public nonReentrant {
        address account = msg.sender;
    ...
    // for BLP transfers it is also possible to transfer BLP into an account using the StakedGlp contract
