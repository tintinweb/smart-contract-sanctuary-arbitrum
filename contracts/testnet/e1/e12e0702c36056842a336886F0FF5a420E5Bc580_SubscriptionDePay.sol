...
contract SubscriptionDePay is Ownable, ReentrancyGuard, ERC2771Context {
    ...
    event UserCharged(address indexed user, address indexed token, uint256 fee, uint256 timestamp);
    event UserDeposit(address indexed user, address indexed token, uint256 deposit, uint256 timestamp);
    ...
    function userDeposit(address _token, uint _amount) external {
        ...
        emit UserDeposit(_msgSender(), _token, _amount, block.timestamp); 
    }
    /**
    ...
    function userWithdraw(address _token, uint _amount) external nonReentrant {
        ...
        emit UserWithdraw(_msgSender(), _token, _amount, block.timestamp); 
    }
    ...
    function chargeUser(
        ...
        emit UserCharged(u, t, underlying, block.timestamp);
    }
