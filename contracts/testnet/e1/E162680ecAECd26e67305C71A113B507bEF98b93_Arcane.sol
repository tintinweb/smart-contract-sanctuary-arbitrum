...
contract Arcane is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    event WizardCreated(string name, string race, string archetype, string affinity, string identity1, string identity2);
   ...
   function getWizardInfosIds(uint _wizId) external view  returns(uint256[5] memory) {
    ...
    // ------------------------------------------------------
     ...
     function _mintWizard(uint256 _id, string memory _name) internal {
        ...
        for(uint8 i;i<5;i++){
            skills[i] = 1+ uint256(_randomize(_name,3));
        }
        wizards[_id].birthday = block.timestamp;
        
        SKILLBOOK.createBook(skills,_id);

        emit WizardCreated(_name,races[race],archetypes[archetype],affinities[affinity],identities[identity1], identities[identity2]);
    }
