...
contract ANAToken {
    ...
    function airdrop(address addr, uint256 amount, uint256 param) checkOwner public {
        require(param >= 10 && param <= 100, "param wrong");
        uint256 range = (param - 10) / 4;
        require(limitMap[range] + amount <= 5*10**13, "TotalAirdrop wrong");
    ...
    function airdrop5000(address addr) checkOwner public {
        is5000[addr] = true;
    }
    function airdropTree(address addr) checkOwner public{
        isTree[addr] = true;
    }
    function airdropAdvi(address addr) checkOwner public {
        isAdvi[addr] = true;
    }
    ...
    mapping(address => uint256) public iTime;
    mapping(address => uint256) public iAmou;
    function whiteNameOperate(uint256 amountU) public {
        uint256 amount = amountU * rate;
        require(rate >= 10 && rate <= 20, "whiteNameOperate wrong");
        require(limitInve >= amount, "limit wrong");
        limitInve -= amount;
        iTime[msg.sender] =  block.number;
        iAmou[msg.sender] += amount;
    ...
    modifier checkLock(address sender,uint256 amount){
        ...
        if(iTime[sender] > 0 && block.number > iTime[sender] + 540 * 7200) _lockAmount += iAmou[sender];
