...
contract AORI is ERC20, Ownable {
    ...
    mapping(address => uint256) claimableAORI;
    ...
    function stakeLPTokens(ERC4626 vault, address receiver, uint256 shares, uint256 lockUpTime) public {
    ...
    function unstakeLPTokens(ERC4626 vault, address receiver, uint256 shares) public {
        ...
        //vault.transferFrom(address(this), receiver, shares);
    }

    function claimAORI(address receiver) public {
        claimAccrual(); 
        require(claimableAORI[receiver] >= 0, "Not allowed claimer");
        uint256 toReceive = claimableAORI[receiver];
        require(toReceive + liqMined <= maxLiqMining, "Liquidity mining is completed");
        claimableAORI[receiver] = 0;
        _mint(receiver, toReceive);
    }

    function claimAccrual() public {
        uint256 rewards;
        uint256 percentToBeMined = (block.timestamp - lastAccrueTime) * percentPerSecond; //1e18
        lastAccrueTime = block.timestamp;
        uint256 tokensMined = (percentToBeMined * expScale) / (maxLiqMining - liqMined); //1e18 / 1e18
        
        for(uint i; i < allVaults.length; i++) {
            if(stakerBal[stakers[i]][allVaults[i]] == 0){
                continue;
            } else {
                uint256 bal = stakerBal[stakers[i]][allVaults[i]]; //token scale
                uint256 preClaimable = claimableAORI[stakers[i]]; //1e18
                uint256 dollarVal = getPrice(AggregatorV3Interface(vaults[allVaults[i]].oracle)) * bal; 
                uint256 percentToUser = (dollarVal * 1e18) / totalAssetsValue(); //1e6 * 1e18 / 1e6
                uint256 tokensToUser = (percentToUser * tokensMined) / expScale; //1e18 * 1e18 / 1e18
                claimableAORI[stakers[i]] = preClaimable + ((tokensToUser * boostMultiplier[stakers[i]]) / 10000);
                rewards += tokensToUser;
            }
        }
        liqMined += rewards;
    }
    ...
    */
    ...
    function totalAssetsValue() internal view returns (uint256) {
        uint256 totalVal;
        for(uint i; i < allVaults.length; i++) {
            AggregatorV3Interface oracle = AggregatorV3Interface(vaults[allVaults[i]].oracle);
            totalVal += (getPrice(oracle) * ERC20((allVaults[i].asset())).balanceOf(address(this)));
        }
        return totalVal;
    }
