...
contract FeeCollector is ReentrancyGuard, AccessControlBase, IFeeCollector {
    ...
    function setSwapDistribution(
        ...
        // together all the ratios need to sum to 1e4 (100%)
        require(
            (_wlpHoldersRatio + _stakingRatio + _buybackAndBurnRatio + _coreRatio) == 1e4,
     ...
     */
    function getSwapDistribution() external view returns(SwapDistributionRatio memory swapDistributionConfig_) {
     ...
     */
    function getWagerDistribution() external view returns(WagerDistributionRatio memory wagerDistributionConfig_) {
     ...
     */
    function isWhitelistedDestination(address _address) external view returns (bool whitelisted_) {
    ...
    function _setAmountsForSwap(uint256 _amount) internal {
        ...
        reserves.buybackAndBurn += calculateDistribution(_amount, swapDistributionConfig.buybackAndBurn);
    ...
    function _setAmountsForWager(uint256 _amount) internal {
        ...
        reserves.buybackAndBurn += calculateDistribution(_amount, wagerDistributionConfig.buybackAndBurn);
