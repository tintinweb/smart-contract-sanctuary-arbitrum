...
// MasterChef is the master of PURPLE. He can make PURPLE and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once PURPLE is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.

contract MasterChefPurple is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of PURPLEs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPurplePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws tokens to a pool. Here's what happens:
        //   1. The pool's `accPurplePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime;   // Last timestamp PURPLEs distribution occurs.
        uint256 accPurplePerShare;  // Accumulated PURPLEs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The PURPLE TOKEN!
    PurpleToken public purple;
    // Dev address.
    address public devaddr;
    // Dev fee percentage.
    uint256 public devFee = 100;
    // PURPLE tokens created per second.
    uint256 public purplePerSecond;
    // Deposit Fee address
    ...
    // Max dev fee
    uint256 public constant MAX_DEV_FEE = 200;
    ...
    // The block number when PURPLE mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 purplePerSecond);
    ...
    event LogUpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 stakeSupply, uint256 accPurplePerShare);

    constructor(
        PurpleToken _purple,
        address _devaddr,
        address _feeAddress,
        uint256 _purplePerSecond,
        uint256 _startTime
    ) {
        purple = _purple;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        purplePerSecond = _purplePerSecond;
    ...
    // Update the given pool's PURPLE allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending PURPLEs on frontend.
    function pendingTokens(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accPurplePerShare = pool.accPurplePerShare;
        uint256 stakeSupply = pool.totalStaked;
        if (block.timestamp > pool.lastRewardTime && stakeSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 purpleReward = multiplier.mul(purplePerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accPurplePerShare = accPurplePerShare.add(purpleReward.mul(1e12).div(stakeSupply));
        }
        return user.amount.mul(accPurplePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
        uint256 totalPurple = multiplier.mul(purplePerSecond).mul(pool.allocPoint).div(totalAllocPoint);
        if (totalPurple == 0) return;

        if(purple.totalSupply() + totalPurple > purple.cap()) {
            totalPurple = purple.cap() - purple.totalSupply();
            if(totalPurple == 0 && purplePerSecond != 0)
                return _updateEmissionRate(0);
        }

        uint256 forDevs = totalPurple.mul(devFee).div(1000);
        uint256 purpleReward = totalPurple.sub(forDevs);
        purple.mint(devaddr, forDevs);
        purple.mint(address(this), purpleReward);
        pool.accPurplePerShare = pool.accPurplePerShare.add(purpleReward.mul(1e12).div(stakeSupply));
        pool.lastRewardTime = block.timestamp;
        emit LogUpdatePool(_pid, pool.lastRewardTime, stakeSupply, pool.accPurplePerShare);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            pending = user.amount.mul(pool.accPurplePerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safepurpleTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accPurplePerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accPurplePerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            safepurpleTransfer(msg.sender, pending);
        }
        if (_amount > 0) {
    ...
    // Safe purple transfer function, just in case if rounding error causes pool to not have enough purples.
    function safepurpleTransfer(address _to, uint256 _amount) internal {
        uint256 purpleBal = purple.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > purpleBal) {
            transferSuccess = purple.transfer(_to, purpleBal);
        } else {
            transferSuccess = purple.transfer(_to, _amount);
        }
        require(transferSuccess, "safepurpleTransfer: transfer failed");
    }
    ...
    function _updateEmissionRate(uint256 _purplePerSecond) internal {
        require(_purplePerSecond <= MAX_EMISSION_RATE, "Updated emissions are more than maximum rate");
        purplePerSecond = _purplePerSecond;
        emit UpdateEmissionRate(msg.sender, _purplePerSecond);
    }

    function updateEmissionRate(uint256 _purplePerSecond) public onlyOwner {
        _updateEmissionRate(_purplePerSecond);
        massUpdatePools();
    }    
