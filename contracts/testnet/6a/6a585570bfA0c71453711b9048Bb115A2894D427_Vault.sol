...
import "../tokens/interfaces/IDAI.sol";
...
contract Vault is ReentrancyGuard, IVault {
    ...
    // daiAmounts tracks the amount of DAI debt for each whitelisted token
    mapping (address => uint256) public override daiAmounts;

    // maxDaiAmounts allows setting a max amount of DAI debt for a token
    mapping (address => uint256) public override maxDaiAmounts;
    ...
    // this value is used to calculate the redemption values for selling of DAI
    ...
    event BuyDAI(address account, address token, uint256 tokenAmount, uint256 daiAmount, uint256 feeBasisPoints);
    event SellDAI(address account, address token, uint256 daiAmount, uint256 tokenAmount, uint256 feeBasisPoints);
    ...
    event IncreaseDaiAmount(address token, uint256 amount);
    event DecreaseDaiAmount(address token, uint256 amount);
    ...
    function setTokenConfig(
        ...
        maxDaiAmounts[_token] = _maxDaiAmount;
    ...
    function setDaiAmount(address _token, uint256 _amount) external {
        _onlyGov();

        uint256 daiAmount = daiAmounts[_token];
        if (_amount > daiAmount) {
            _increaseDaiAmount(_token, _amount.sub(daiAmount));
            return;
        }

        _decreaseDaiAmount(_token, daiAmount.sub(_amount));
    }
    ...
    // deposit into the pool without minting DAI tokens
    ...
    function buyDAI(address _token, address _receiver) external override nonReentrant returns (uint256) {
        _validateManager();
        ...
        uint256 daiAmount = tokenAmount.mul(price).div(PRICE_PRECISION);
        daiAmount = adjustForDecimals(daiAmount, _token, dai);
        _validate(daiAmount > 0, 18);

        uint256 feeBasisPoints = getFeeBasisPoints(_token, daiAmount, mintBurnFeeBasisPoints, taxBasisPoints, true);
        uint256 amountAfterFees = _collectSwapFees(_token, tokenAmount, feeBasisPoints);
        uint256 mintAmount = amountAfterFees.mul(price).div(PRICE_PRECISION);
        mintAmount = adjustForDecimals(mintAmount, _token, dai);

        _increaseDaiAmount(_token, mintAmount);
        _increasePoolAmount(_token, amountAfterFees);

        IDAI(dai).mint(_receiver, mintAmount);

        emit BuyDAI(_receiver, _token, tokenAmount, mintAmount, feeBasisPoints);

        useSwapPricing = false;
        return mintAmount;
    }

    function sellDAI(address _token, address _receiver) external override nonReentrant returns (uint256) {
        _validateManager();
        _validate(whitelistedTokens[_token], 19);
        useSwapPricing = true;

        uint256 daiAmount = _transferIn(dai);
        _validate(daiAmount > 0, 20);

        updateCumulativeFundingRate(_token);

        uint256 redemptionAmount = getRedemptionAmount(_token, daiAmount);
        _validate(redemptionAmount > 0, 21);

        _decreaseDaiAmount(_token, daiAmount);
        _decreasePoolAmount(_token, redemptionAmount);

        IDAI(dai).burn(address(this), daiAmount);

        // the _transferIn call increased the value of tokenBalances[dai]
        // usually decreases in token balances are synced by calling _transferOut
        // however, for dai, the tokens are burnt, so _updateTokenBalance should
        // be manually called to record the decrease in tokens
        _updateTokenBalance(dai);

        uint256 feeBasisPoints = getFeeBasisPoints(_token, daiAmount, mintBurnFeeBasisPoints, taxBasisPoints, false);
        uint256 amountOut = _collectSwapFees(_token, redemptionAmount, feeBasisPoints);
        _validate(amountOut > 0, 22);

        _transferOut(_token, amountOut, _receiver);

        emit SellDAI(_receiver, _token, daiAmount, amountOut, feeBasisPoints);
    ...
    function swap(address _tokenIn, address _tokenOut, address _receiver) external override nonReentrant returns (uint256) {
        ...
        // adjust daiAmounts by the same daiAmount as debt is shifted between the assets
        uint256 daiAmount = amountIn.mul(priceIn).div(PRICE_PRECISION);
        daiAmount = adjustForDecimals(daiAmount, _tokenIn, dai);
        ...
        uint256 feeBasisPoints;
        {
            ...
            uint256 feesBasisPoints0 = getFeeBasisPoints(_tokenIn, daiAmount, baseBps, taxBps, true);
            uint256 feesBasisPoints1 = getFeeBasisPoints(_tokenOut, daiAmount, baseBps, taxBps, false);
        ...
        _increaseDaiAmount(_tokenIn, daiAmount);
        _decreaseDaiAmount(_tokenOut, daiAmount);
    ...
    function getRedemptionAmount(address _token, uint256 _daiAmount) public override view returns (uint256) {
        uint256 price = getMaxPrice(_token);
        uint256 redemptionAmount = _daiAmount.mul(PRICE_PRECISION).div(price);
        return adjustForDecimals(redemptionAmount, dai, _token);
    }
    ...
    function adjustForDecimals(uint256 _amount, address _tokenDiv, address _tokenMul) public view returns (uint256) {
        uint256 decimalsDiv = _tokenDiv == dai ? DAI_DECIMALS : tokenDecimals[_tokenDiv];
        uint256 decimalsMul = _tokenMul == dai ? DAI_DECIMALS : tokenDecimals[_tokenMul];
    ...
    function getFeeBasisPoints(address _token, uint256 _daiDelta, uint256 _feeBasisPoints, uint256 _taxBasisPoints, bool _increment) public override view returns (uint256) {
        if (!hasDynamicFees) { return _feeBasisPoints; }

        uint256 initialAmount = daiAmounts[_token];
        uint256 nextAmount = initialAmount.add(_daiDelta);
        if (!_increment) {
            nextAmount = _daiDelta > initialAmount ? 0 : initialAmount.sub(_daiDelta);
        }

        uint256 targetAmount = getTargetDaiAmount(_token);
    ...
    function getTargetDaiAmount(address _token) public view returns (uint256) {
        uint256 supply = IERC20(dai).totalSupply();
    ...
    function _increaseDaiAmount(address _token, uint256 _amount) private {
        daiAmounts[_token] = daiAmounts[_token].add(_amount);
        uint256 maxDaiAmount = maxDaiAmounts[_token];
        if (maxDaiAmount != 0) {
            _validate(daiAmounts[_token] <= maxDaiAmount, 51);
        }
        emit IncreaseDaiAmount(_token, _amount);
    }

    function _decreaseDaiAmount(address _token, uint256 _amount) private {
        uint256 value = daiAmounts[_token];
        // since DAI can be minted using multiple assets
        // it is possible for the DAI debt for a single asset to be less than zero
        // the DAI debt is capped to zero for this case
        if (value <= _amount) {
            daiAmounts[_token] = 0;
            emit DecreaseDaiAmount(_token, value);
            return;
        }
        daiAmounts[_token] = value.sub(_amount);
        emit DecreaseDaiAmount(_token, _amount);
    }
