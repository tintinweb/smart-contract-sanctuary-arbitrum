...
import "./interface/IMToken.sol";
import "./interface/IERC20.sol";
import "./utils/ERC2771ContextUpgradeable.sol";

contract CoinStockMarket is OwnableUpgradeable, ERC2771ContextUpgradeable {
    ...
    function limitSell(uint price, uint amount) public returns (uint256) {
        ...
        Order memory order = newOrder(Type.LIMIT, Side.SELL, price, amount, 0);
    ...
    function marketSell(uint amount) public returns (uint256) {
        ...
        Order memory order = newOrder(Type.MARKET, Side.SELL, 0, amount, 0);
    ...
    function settleOut(
        ...
        require(
            order.status != Status.CANCELED && order.status != Status.FILLED
        );
        ...
        if (Side.BUY == order.side) {
            require(
                order.origQuoteOrderQty.sub(order.cummulativeQuoteQty).sub(
                    order.fee
                ) >= 0
            );
        ...
        if (Side.SELL == order.side) {
            uint256 toUserAmount = cummulativeQuoteQty.sub(
                curCummulativeQuoteQty
            );
    ...
    function clearOrder(uint orderId, Status status) internal {
        ...
        require(
            orderBook[orderId].status != Status.CANCELED &&
                orderBook[orderId].status != Status.FILLED &&
                orderBook[orderId].status != Status.EXPIRED
        );
        ...
        if (Side.BUY == order.side) {
            uint256 restQuoteAsset = order
                .origQuoteOrderQty
                .sub(order.cummulativeQuoteQty)
                .sub(order.fee);
    ...
    function excess(uint amount) public view returns (uint excessAmount) {
        uint _excess = amount.mul(excessMarginNumerator).div(
            excessMarginDenominator
        );
        return amount.add(_excess);
    }
    ...
    function updateExcessMarginDenominator(
        uint _updateExcessMarginDenominator
    ) public onlyOwner {
        excessMarginDenominator = _updateExcessMarginDenominator;
    }

    function updateExcessMarginNumerator(
        uint _updateExcessMarginNumerator
    ) public onlyOwner {
        excessMarginNumerator = _updateExcessMarginNumerator;
    }

    function updateLotShareNumber(uint _updateLotShareNumber) public onlyOwner {
        lotShareNumber = _updateLotShareNumber;
    }

    function updateBaseAsset(address _updateBaseAsset) public onlyOwner {
        baseAsset = IMToken(_updateBaseAsset);
    }

    function updateQuoteAsset(address _updateQuoteAsset) public onlyOwner {
        quoteAsset = IERC20(_updateQuoteAsset);
    }

    function updateSymbol(string memory _updateSymbol) public onlyOwner {
        symbol = _updateSymbol;
    }

    function setSettleMans(
        address account,
        bool _isSettleMans
    ) external onlyOwner {
    ...
    modifier onlySettleMans() {
        require(
            _settleMans[_msgSender()] == true,
            "Ownable: caller is not the _settleMans"
        );
        _;
    }

    function setWithdrawMans(
        address account,
        bool _isWithdrawMans
    ) external onlyOwner {
    ...
    modifier onlyWithdrawMans() {
        require(
            _withdrawMans[_msgSender()] == true,
            "Ownable: caller is not the _withdrawMans"
        );
        _;
    }

    function setWithdrawalTargetAddress(
    ...
    function withdrawQuoteAsset(uint _value) public onlyWithdrawMans {
        require(
            _withdrawalTargetAddress != address(0x0),
            "Ownable: _withdrawalTargetAddress is 0x0"
        );
