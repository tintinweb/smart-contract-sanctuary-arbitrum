...
contract LootTableComponent is BaseStorageComponentV2 {
     ...
     *
     * @param entity Entity to get value for
     * @param lootIds Id of the loot
     * @param weights Probability weight of each loot
     ...
     *
     * @param entity Entity to get value for
     * @return lootIds Id of the loot
     * @return weights Probability weight of each loot
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.mintedstarterpiratecomponent.v3")
);
 ...
 */
contract MintedStarterPirateComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param minted Track whether wallet has minted a starter pirate and how many
     */
    function setValue(
        uint256 entity,
        uint256 minted
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(minted));
    }
    ...
    function getLayoutValue(
        ...
        // Get the struct from storage
        value = MintedStarterPirateComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return minted Track whether wallet has minted a starter pirate and how many
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 minted) {
        if (has(entity)) {
            Layout memory s = MintedStarterPirateComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (minted) = abi.decode(_getEncodedValues(s), (uint256));
        }
    }
    ...
    function batchRemove(
        ...
        // Remove the entities from the component
        for (uint256 i = 0; i < entities.length; i++) {
            delete MintedStarterPirateComponentStorage
                .layout()
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.minted));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.minted);
    }
}
...
library RandomLibrary {
    ...
    function weightedCoinFlipBatch(
        ...
        uint256 successRate,
        uint8 amount
    ) internal pure returns (uint8 numSuccess, uint256 nextRandomWord) {
        if (successRate >= PERCENTAGE_RANGE) {
            numSuccess = amount;
            nextRandomWord = randomWord;
        } else {
...
contract StarterPirateSystem is GameRegistryConsumerUpgradeable, ILootCallback {
    ...
    function _grantLoot(
        ...
        if (
            mintedStarterPirateComponent.getLayoutValue(accountEntity).minted >
            0
        ) {
            revert AlreadyMintedStarterPirate(accountEntity);
        }
        // Mark off that this account has minted a starter pirate
        mintedStarterPirateComponent.setLayoutValue(
            accountEntity,
