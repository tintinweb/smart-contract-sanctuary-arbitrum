...
contract FarmingPoolV2 is Ownable, Pausable {
    ...
    mapping(address => uint256) public unstakeLocked;
    ...
    uint256 public constant CYCLE_TIME = 1 days;
    uint256 private constant MULTIPLICATOR = 10 ** 30;

    event Staked(address indexed user, uint256 amount);
    event ClaimedLpTokens(address indexed user, uint256 amount);
        ...
        if (updateTime < prevCycle) {
            ...
            for (uint i; i < length; ) {
                ...
                if (rewardAsset.nextReward != 0) {
                    // can't take lockedRate here because it could be updated not in the previous cycle
                    uint256 lastCycle = getCycleTime(
                        rewardAsset,
                        updateTime,
                        true
                    );
                    uint256 lastRate = getRateForDate(rewardAsset, lastCycle);

                    assetToReward[assets[i]] = Reward({
                        oldRate: lastRate,
                        startTime: lastCycle,
                        lockedPeriod: 0,
                        tokensPerDay: rewardAsset.nextReward,
                        virtualRewards: 0,
                        claimed: 0,
                        nextReward: 0,
                        lockedRate: rewardAsset.lockedRate
                    });
                }

                uint256 nextCycle = getCycleTime(
                    rewardAsset,
                    block.timestamp,
                    true
                );
    ...
    ) external onlyOwner updateState {
    ...
    function stakeTokens(uint256 amount) external whenNotPaused updateState {
        ...
        for (uint i; i < length; ) {
            ...
            uint256 prevCycle = getCycleTime(
                rewardAsset,
                block.timestamp,
                false
            );
            ...
            if (stake.rate < prevRate) {
                ...
                //then calculate new rate using previous rate to not lose rewards from current cycle
                stake.unclaimed +=
                    (stakedAmount * (prevRate - stake.rate)) /
                    MULTIPLICATOR;
                stake.rate =
                    nextRate -
                    (((nextRate - prevRate) * stakedAmount) / newStakedAmount);
            } else {
    ...
    function unstakeRequest(uint256 amount) external updateState {
        ...
        unstakeLocked[msg.sender] += amount;
    ...
    function claim() external whenNotPaused updateState {
        ...
        for (uint i; i < length; ) {
            ...
            if (prevCycle > lastUnstake[msg.sender]) {
                reward += stake.lockedRewards;
                stake.lockedRewards = 0;
            }

            if (prevRate > stake.rate) {
                reward +=
                    (totalStaked * (prevRate - stake.rate)) /
                    MULTIPLICATOR;
                stake.rate = prevRate;
            }
    ...
    function getClaimCycleTime() public view returns (uint256) {
        ...
        return protocolStartTime + currentCycle * claimPeriod;
    }
    ...
    function getGlobalCycleTime() public view returns (uint256) {
        ...
        return protocolStartTime + currentCycle * CYCLE_TIME;
    }

    function unstake(address user) private {
        uint256 amount = unstakeLocked[user];

        if (amount > 0) {
            stakingAsset.safeTransfer(user, amount);
            unstakeLocked[user] = 0;

            emit ClaimedLpTokens(user, amount);
        }
    }
    ...
    function getCycleTime(
        ...
        if (totalTime > rewardAsset.lockedPeriod) {
            uint256 currentCycle = (totalTime - rewardAsset.lockedPeriod) /
                CYCLE_TIME;
    ...
    function getRateForDate(
        ...
        uint256 totalDays = (totalTime - rewardAsset.lockedPeriod) / CYCLE_TIME;
    ...
    function getUpdatedRateForDate(
        ...
        if (updateTime < prevCycle) {
            ...
            if (rewardAsset.nextReward != 0) {
                uint256 lastCycle = getCycleTime(rewardAsset, updateTime, true);
                uint256 lastRate = getRateForDate(rewardAsset, lastCycle);

                rewardAsset.oldRate = lastRate;
                rewardAsset.startTime = lastCycle;
        ...
        return
            rewardAsset.oldRate +
            (MULTIPLICATOR *
                (rewards + totalDays * rewardAsset.tokensPerDay - claimed)) /
            amountLp;
    }
}
...
contract FarmingTreasury is Ownable {
    ...
    mapping(address => bool) poolToWhitelist;
    address[] pools;
    ...
    function addToWhitelist(address pool) external onlyOwner {
        ...
        for (uint i; i < length; ) {
            require(pools[i] != pool, "Pool is already whitelisted");
