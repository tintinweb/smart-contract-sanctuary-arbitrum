    ...
    IBinaryVault
{
    ...
    IERC20Upgradeable private underlyingToken;
    ...
    function initialize(
        ...
        underlyingToken = IERC20Upgradeable(underlyingTokenAddress);

        config = IBinaryConfig(config_);

        withdrawalDelayTime = 24 hours;
        // withdrawalDelayTime = 2 minutes; // For test
    }
    ...
    function addLiquidity(
        ...
        _withdrawManagementFee(0, _nextTokenId());
    ...
    function executeWithdrawalRequest(
        ...
        uint256 redeemAmount = (tokenValue * shareAmount) / shareBalance;
        // Send money to user
        underlyingToken.safeTransfer(user, redeemAmount - fee);
        ...
        if (shareAmount < shareBalance) {
            ...
            // TODO Check if this fomula is correct.
        ...
        // deduct
        totalDepositedAmount -= redeemAmount;
        watermark -= redeemAmount;
    ...
    function getInitialInvestExpression(
        ...
        string memory floatExpression = getFloatExpression(
            (_value * 10 ** 2) /
                10 ** IERC20Metadata(address(underlyingToken)).decimals()
        );
        return
            string(
                abi.encodePacked(
    ...
    function getImagePlainText(
        ...
        result = result.replaceString(
            "<!--VAULT_LOGO_IMAGE-->",
            config.tokenLogo(address(underlyingToken))
        );
        result = result.replaceString(
            "<!--VAULT_VALUE-->",
    ...
    function getManifestPlainText(
        uint256 _tokenId
    ) public view virtual returns (string memory _manifest) {
    ...
    function withdrawToken(address _token) external virtual onlyOwner {
        require(
            _token != address(underlyingToken),
            "INVALID_TOKEN"
        );
    ...
    */
    ...
    /**
    @dev This is function for withdraw management fee - Ryze Fee
    ...
    * @notice We set from and to parameter so that we can avoid falling in gas limitation issue
     */
    function withdrawManagmentFee(
        uint256 from,
        uint256 to
    ...
    function _withdrawManagementFee(uint256 from, uint256 to) internal virtual {
        ...
        for (uint tokenId = from; tokenId <= to; tokenId++) {
            ...
            if (fee > 0) {
              feeAmount += fee;
              uint256 feeShare = fee * totalShareSupply / totalDepositedAmount;
              if (shareBalances[tokenId] >= feeShare) {
                shareBalances[tokenId] = shareBalances[tokenId] - feeShare;
              }
              // We will set recent snapsot so that we will prevent to charge duplicated fee.
              recentSnapshots[tokenId] = netValue;
            }
        }
        if (feeAmount > 0) {
          uint256 feeShare = feeAmount * totalShareSupply / totalDepositedAmount;

          underlyingToken.safeTransfer(config.treasury(), feeAmount);
    ...
    function getManagementFee() public view returns (uint256 feeAmount) {
        ...
        for (uint tokenId = 0; tokenId < to; tokenId++) {
...
interface IBinaryVault {
    ...
    function claimBettingRewards(address to, uint256 amount, bool isRefund) external returns(uint256);
    ...
    function onPlaceBet(uint256 amount, address from, uint256 endTime, uint8 position) external;
