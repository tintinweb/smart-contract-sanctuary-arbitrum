...
contract Vault is Ownable {
    ...
    function withdrawToken(address _token, uint256 _amount) external onlyAdmin {
        ...
        require(_curAmount >= _amount, "Vault: Current balance is too low");
    ...
    function addDepositRequest(uint256 _amountLD, address _token) external {
        require(isAcceptingToken(_token), "Vault: Invalid token");
        ...
        _pendingBuffer.totalDepositAmount += _amountMD;
    ...
    function addWithdrawRequest(uint256 _amountMLP, address _token) external {
        ...
        _pendingBuffer.withdrawAmountPerUser[_withdrawer] += _amountMLP;
        _pendingBuffer.withdrawAmountPerToken[_token] = _pendingBuffer.withdrawAmountPerToken[_token] + _amountMLP;
        _pendingBuffer.totalWithdrawAmount += _amountMLP;
...
contract Controller is Ownable {
    ...
    function settleReport(uint16 _srcChainId, uint256 _updateNum) public onlyBridge {
    ...
    function _updateSnapshot(uint16 _srcChainId, MozBridge.Snapshot memory _snapshot, uint256 _updateNum) internal {
        // require(protocolStatus == ProtocolStatus.SNAPSHOTTING, "Protocol must be SNAPSHOTTING");
        // require(updateNum == _updateNum, "Controller: last snapshot report");
    ...
    function _settleReport(uint16 _srcchainId, uint256 _updateNum) internal {
        // require(protocolStatus == ProtocolStatus.SETTLING, "Protocol must be SETTLING");
        // require(updateNum == _updateNum, "Controller: last settle report");
        if(updateNum != _updateNum) return;
