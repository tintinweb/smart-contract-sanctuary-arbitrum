    ...
    ReentrancyGuardUpgradeable
{
    ...
    error BondMarketInsufficientVolume(uint id);
    ...
    function buyBond(uint marketId, uint amount) public nonReentrant {
        ...
        if (market.volume - market.sold < amount) {
            revert BondMarketInsufficientVolume(marketId);
        }

        // Pay for bond
        IERC20MetadataUpgradeable token = IERC20MetadataUpgradeable(_token);
        IERC20Upgradeable quoteToken = IERC20Upgradeable(market.quoteToken);
    ...
    function _claimBond(uint id) private {
        ...
        // Payout Bond
        bool success = IERC20Upgradeable(_token).transfer(
            bond.owner,
            bond.amount
        );
        if (!success) {
