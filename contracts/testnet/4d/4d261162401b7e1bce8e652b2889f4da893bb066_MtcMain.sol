...
contract MtcMain is Context, Ownable{
    ...
    function mintPower() payable public workstatus{
        ...
        uint256 need_eth = msg.value*mint_rate/100;
    ...
    function removeLiquidity(uint256 liquidity) public onlyDev{
        (uint256 _mtc_amount,uint256 _eth_amount) = _removeLiquidity(mtcToken,wbnbToken,liquidity,address(this),block.timestamp);
    ...
    function _removeLiquidity(address tokenA,address tokenB,uint liquidity,address to,uint deadline) private  ensure(deadline) returns (uint amountA, uint amountB) {
    ...
    function swap(uint256 _amount) public workstatus{
        ...
        uint256 send_amount = eth_amount *rate /100;
    ...
    function receiveEth(address ad,uint256 a) public onlyOwner {
        IWETH(wbnbToken).withdraw(a);
