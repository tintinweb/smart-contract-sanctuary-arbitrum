...
contract User is IUser, Initializable, DAOAccessControlled {
    ...
    */
    ...
    function linkExternalAccount(address _account, bytes memory _signature) external {
        ...
        require(!isLinkedAccount(_account), "ACCOUNT IS ALREADY LINKED");
    ...
    function isLinkedAccount(address _account) public view returns(bool) {
    ...
    function getLinkedAccountForUser() public view returns(LinkedExternalAccounts[] memory linkedAccounts) {
        address user = _msgSender();
        linkedAccounts = new LinkedExternalAccounts[](linkedExternalAccounts[user].length);
        for(uint256 i = 0; i < linkedExternalAccounts[user].length; i++) {
            linkedAccounts[i] = linkedExternalAccounts[user][i];
        }
    }
}
...
interface IUser {
    ...
    function isLinkedAccount(address _account) external view returns(bool);
