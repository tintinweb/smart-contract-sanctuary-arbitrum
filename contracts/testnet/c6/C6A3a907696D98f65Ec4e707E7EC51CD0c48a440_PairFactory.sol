...
import "contracts/interfaces/IBlue.sol";

contract Blue is IBlue {
    string public constant name = "BlueDex";
...
contract Minter is IMinter {
    ...
    IBlue public immutable _blue;
    IVoter public immutable _voter;
    IVotingEscrow public immutable _ve;
    IRewardsDistributor public immutable _rewards_distributor;
    uint256 public weekly = 1_000_000 * 1e18; // represents a starting weekly emission of 1M BLUE (BLUE has 18 decimals)
    ...
    constructor(
        ...
        _blue = IBlue(IVotingEscrow(__ve).token());
    ...
    function initialize(
        ...
        _blue.mint(address(this), max);
        _blue.approve(address(_ve), type(uint256).max);
    ...
    function circulating_supply() public view returns (uint256) {
        return _blue.totalSupply() - _ve.totalSupply();
    }
    ...
    function calculate_growth(uint256 _minted) public view returns (uint256) {
        ...
        uint256 _blueTotal = _blue.totalSupply();
        return
            (((((_minted * _veTotal) / _blueTotal) * _veTotal) / _blueTotal) *
                _veTotal) /
            _blueTotal /
            2;
    }
    ...
    function update_period() external returns (uint256) {
        ...
        if (block.timestamp >= _period + WEEK && initializer == address(0)) {
            ...
            uint256 _balanceOf = _blue.balanceOf(address(this));
            if (_balanceOf < _required) {
                _blue.mint(address(this), _required - _balanceOf);
            }

            require(_blue.transfer(team, _teamEmissions));
            require(_blue.transfer(address(_rewards_distributor), _growth));
            _rewards_distributor.checkpoint_token(); // checkpoint token balance that was just minted in rewards distributor
            _rewards_distributor.checkpoint_total_supply(); // checkpoint supply

            _blue.approve(address(_voter), weekly);
...
contract MerkleClaim {
    ...
    constructor(address _blue, bytes32 _merkleRoot) {
        BLUE = IBlue(_blue);
    ...
    function claim(
        ...
        require(BLUE.claim(to, amount), "CLAIM_FAILED");
...
/// @notice Part 2 of 2 in the WeVE (FTM) -> USDC + BLUE (OP) redemption process
/// This contract is responsible for receiving the LZ message and distributing USDC + BLUE
contract RedemptionReceiver is ILayerZeroReceiver {
    ...
    uint256 public redeemableBLUE;
    uint256 public leftoverBLUE;
    ...
    event Initialized(address fantomSender, uint256 redeemableUSDC, uint256 redeemableBLUE);

    function initializeReceiverWith(
        address _fantomSender,
        uint256 _redeemableUSDC,
        uint256 _redeemableBLUE
    ) external onlyTeam {
        ...
        redeemableBLUE = _redeemableBLUE;
        leftoverBLUE = _redeemableBLUE;

        emit Initialized(fantomSender, redeemableUSDC, redeemableBLUE);
    }
    ...
    function previewRedeem(uint256 amountWEVE)
        public
        view
        returns (uint256 shareOfUSDC, uint256 shareOfBLUE)
    {
        // pro rata USDC
        shareOfUSDC = (amountWEVE * redeemableUSDC) / ELIGIBLE_WEVE;
        // pro rata BLUE
        shareOfBLUE = (amountWEVE * redeemableBLUE) / ELIGIBLE_WEVE;
    }
    ...
    function lzReceive(
        ...
        (uint256 shareOfUSDC, uint256 shareOfBLUE) = previewRedeem(amountWEVE);
        ...
        leftoverBLUE -= shareOfBLUE; // this will revert if underflows
        require(
            BLUE.claim(redemptionAddress, shareOfBLUE),
            "CLAIM_FAILED"
        );
    }
...
/// @notice Part 1 of 2 in the WeVE (FTM) -> USDC + BLUE (OP) redemption process
...
contract VeArtProxy is IVeArtProxy {
    ...
    function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) external pure returns (string memory output) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "lock #', toString(_tokenId), '", "description": "BlueDex locks, can be used to boost gauge yields, vote on token emission, and receive bribes", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
