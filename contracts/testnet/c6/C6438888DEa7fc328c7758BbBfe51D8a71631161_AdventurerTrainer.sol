  ...
  function train(
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      for (uint256 i = 0; i < traitIds.length; i++) {
        ...
        // Get anima cost
        uint256 animaCost = _getAnimaCost(addr, adventurerId, traitAmounts[i]);
...
interface IAdventurerTrainerStorage {
  function setEpoch(address _addr, uint256 _adventurerId) external;
}
