...
contract TokenOffering is Ownable {
    struct UserShares {
        uint128 presaleShares;
        uint128 publicShares;
    }

    uint128 immutable maximumThreshold;
    uint128 immutable walletLimit;
    uint128 immutable presaleBoost;
    uint128 immutable largeTicketBoost;
    uint128 immutable largeTicketThreshold;

    uint128 private presaleShares;
    uint128 private publicShares;
    ...
    event Purchase(address indexed purchaser, uint128 USDCvalue, uint128 shares);
    ...
    constructor(
        address _usdc,
        uint128 _maximumThreshold,
        uint128 _walletLimit,
        uint128 _presaleBoost,
        uint128 _largeTicketBoost,
        uint128 _largeTicketThreshold
    ) {
    ...
    function purchasePublic(uint128 amount) external {
        ...
        require(
            shareTracking[msg.sender].presaleShares + shareTracking[msg.sender].publicShares + amount <= walletLimit,
            "User has reached wallet limit."
        );
    ...
    function getUserShares(address _address) external view returns (uint128) {
        if (allowList[_address]) {
            uint128 presaleSharesUser = (shareTracking[_address].presaleShares * presaleBoost) / 100;
    ...
    function _addBoostForPriorShares() internal view returns (uint128) {
        uint128 priorShares = shareTracking[msg.sender].presaleShares;
        uint128 boostToAdd = ((priorShares * largeTicketBoost) / 100) - priorShares;
        return boostToAdd;
    }
}
