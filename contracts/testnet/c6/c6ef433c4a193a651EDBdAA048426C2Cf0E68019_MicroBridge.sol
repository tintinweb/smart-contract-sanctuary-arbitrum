...
contract MicroBridge is NonblockingLzApp, MicroUtility, ReentrancyGuard {
    ...
    function init(bytes memory initPayload) external returns (bool) {
        ...
        (
            ...
            address _owner,
            address _manager
        ) = abi.decode(initPayload, (uint256, address, address));
        require(
    ...
    function estimateSendBatchFee(
        ...
        bytes memory payload = abi.encode(_nft, _toAddress, _tokenIds);
        return
    ...
    function _send(
        ...
        address nftAddress;
        assembly {
            nftAddress := mload(add(_nftAddress, 20))
        }

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            IMicroNFT(nftAddress).transferFrom(
                _from,
                address(this),
                _tokenIds[i]
            );
            IMicroNFT(nftAddress).bridgeOut(
                _from,
                uint64(_dstChainId),
                _tokenIds[i]
            );
        ...
        bytes memory payload = abi.encode(_nftAddress, _toAddress, _tokenIds);
        ...
        emit SendToChain(nftAddress, _dstChainId, _from, _toAddress, _tokenIds);
    }
    ...
    function _nonblockingLzReceive(
        ...
        ) = abi.decode(_payload, (bytes, bytes, uint256[]));
    ...
    function clearCredits(bytes memory _payload) external nonReentrant {
        bytes32 hashedPayload = keccak256(_payload);
        require(
        ...
        uint256 nextIndex = _creditTill(
            storedCredits[hashedPayload].nftAddress,
    ...
    function emergencyWithdraw() external onlyOwner {
        ...
        if (bnbBalance > 0) {
            (bool success, ) = payable(microManager.treasuryAddress()).call{
