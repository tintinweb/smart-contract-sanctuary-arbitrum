...
uint256 constant ID = uint256(
    keccak256("game.piratenation.mintcountercomponent.v1")
);
...
library MintCounterComponentStorage {
...
/**
 * @title MintCounterComponent
 * @dev An uneditable mint counter
 */
contract MintCounterComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param mints Counter tracking total mints
     */
    function setValue(
        uint256 entity,
        uint256 mints
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(mints));
    }
    ...
    function getLayoutValue(
        ...
        value = MintCounterComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return mints Counter tracking total mints
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 mints) {
        if (has(entity)) {
            Layout memory s = MintCounterComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (mints) = abi.decode(_getEncodedValues(s), (uint256));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = MintCounterComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](1);
        values[0] = abi.encode(s.mints);
    }
    ...
    function getBytes(
        ...
        Layout memory s = MintCounterComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.mints) = abi.decode(value, (uint256));
    ...
    function remove(
        ...
        delete MintCounterComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete MintCounterComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.mints));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.mints);
    }
}