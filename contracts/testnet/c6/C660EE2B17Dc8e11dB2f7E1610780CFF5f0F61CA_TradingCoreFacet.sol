...
contract TradingCoreFacet is ITradingCore, OnlySelf {
    ...
    function slippagePrice(
        ...
        if (isLong) {
            ...
            } else if (sc.slippageType == IPairsManager.SlippageType.NET_POSITION && pairQty.longQty + qty >= pairQty.shortQty) {
            ...
            } else if (sc.slippageType == IPairsManager.SlippageType.NET_POSITION && pairQty.shortQty + qty >= pairQty.longQty) {
...
library LibPairsManager {
    ...
    */
    ...
    function addSlippageConfig(
        ...
        if (slippageType != IPairsManager.SlippageType.FIXED) {
    ...
    function updateSlippageConfig(SlippageConfig memory sc) internal {
        ...
        require(config.enable, "LibPairsManager: Configuration not enabled");
        if (sc.slippageType != IPairsManager.SlippageType.FIXED) {
