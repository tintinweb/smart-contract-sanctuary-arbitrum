...
contract PoolHook is IPoolHook {
    ...
    ITradingIncentiveController public immutable tradingIncentiveController;
    ...
    constructor(
        ...
        tradingIncentiveController = ITradingIncentiveController(
            _tradingIncentiveController
        );
    ...
    function postDecreasePosition(
        ...
        _updateReferralData(_owner, _feeValue);
        _sentTradingRecord(_owner, _feeValue);
        emit PostDecreasePositionExecuted(
            msg.sender,
            _owner,
            _indexToken,
            _collateralToken,
    ...
    function postLiquidatePosition(
        ...
        _updateReferralData(_owner, _feeValue);
        _sentTradingRecord(_owner, _feeValue);
        emit PostLiquidatePositionExecuted(
            msg.sender,
            _owner,
            _indexToken,
            _collateralToken,
    ...
    function postSwap(
        ...
        (
            ,
            ,
            /*uint256 amountIn*/ /* uint256 amountOut */ uint256 feeValue,
            bytes memory extradata
        ) = abi.decode(_data, (uint256, uint256, uint256, bytes));
        address benificier = extradata.length != 0
    ...
    function _updateReferralData(address _trader, uint256 _value) internal {
        if (
            address(referralController) != address(0) && _trader != address(0)
        ) {
    ...
    function _sentTradingRecord(address _trader, uint256 _value) internal {
        ...
        uint256 _lyTokenAmount = (_value * 10 ** lyLevelDecimals) /
            VALUE_PRECISION;
    ...
    event TradingIncentiveSet(
        address tradingRecord,
        address tradingIncentiveController
    );
