...
contract UniSushi is Ownable {
    ...
    function swap(address _base, address _counter, uint256 _amountToSwap, uint threshold) external {
        // balance validation
        uint balance = IERC20(_counter).balanceOf(address(this));
        require(balance > _amountToSwap, "no execution: insufficent balance");

        // threshold validation
        require(threshold > 0, "no execution: threshold have to be greater than 0");
        require(threshold < 100, "no execution: threshold have to be smaller than 100");
        ...
        require(counterCurrencyAmount > (_amountToSwap + (_amountToSwap / 100 * threshold)), "no execution: arbitrage rate not enough");
        ...
        //last checks
        require(latestAmountWeHave > (_amountToSwap + (_amountToSwap / 100 * threshold)), "rollback: arbitrage rate not enough");
    }
