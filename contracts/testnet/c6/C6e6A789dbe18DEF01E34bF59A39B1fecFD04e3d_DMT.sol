...
import {DMTPoker} from "./poker/DMTPoker.sol";
...
contract DMT is ERC20 {
    ...
    constructor() ERC20("Sanko WSOP", "WSOP") {
        owner = msg.sender;
    ...
    function setPoker(address _poker) public {
        if (msg.sender != owner) {
            revert("Only owner can set poker");
        }
        poker = _poker;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        // The owner distributes test tokens to players;
        // the poker contract sends and receives tokens to/from players
        if (sender != owner && sender != poker && recipient != poker) {
            revert("Sender must be owner/poker or recipient must be poker");
        }
