...
interface IImbuedSoul is IERC721Upgradeable {
    ...
    function safeMint(
        ...
        SecondarySkill[] calldata secondarySkills) external;
...
interface ILandDeed is IERC721Upgradeable {
...
contract SeedEvolution is Initializable, SeedEvolutionSettings {
    ...
    function stakeSoLs(
        ...
        LifeformRealm[] calldata _firstRealms,
        LifeformRealm[] calldata _secondRealms)
    external
    whenNotPaused
    contractsAreSet
    onlyEOA
    {
        uint256 _totalCreated = _sol1Amount + _sol2Amount;

        require(_totalCreated > 0, "SeedEvolution: No SoL sent");
        require(_totalCreated == _paths.length, "SeedEvolution: Paths length does not match total created");
        require(_totalCreated == _firstRealms.length, "SeedEvolution: First realm length not equal to total");
        require(_totalCreated == _secondRealms.length, "SeedEvolution: Second realm length not equal to total");

        uint256 _totalMagicNeeded = 0;
        uint256 _totalBCNeeded = 0;

        for(uint256 i = 0; i < _totalCreated; i++) {
            Path _path = _paths[i];
            if(_path == Path.MAGIC) {
                _totalMagicNeeded += magicCost;
            } else if(_path == Path.MAGIC_AND_BC) {
                _totalMagicNeeded += magicCost;
                _totalBCNeeded += balancerCrystalStakeAmount;
            }

            _createLifeform(_path, _firstRealms[i], _secondRealms[i]);
        }

        if(_sol1Amount > 0) {
            seedOfLife.safeTransferFrom(msg.sender, address(this), seedOfLife1Id, _sol1Amount, "");
        }
        if(_sol2Amount > 0) {
            seedOfLife.safeTransferFrom(msg.sender, address(this), seedOfLife2Id, _sol2Amount, "");
        }
        if(_totalMagicNeeded > 0) {
            magic.safeTransferFrom(msg.sender, treasuryAddress, _totalMagicNeeded);
        }
        if(_totalBCNeeded > 0) {
            balancerCrystal.safeTransferFrom(msg.sender, address(this), balancerCrystalId, _totalBCNeeded, "");
        }
    }

    function _createLifeform(Path _path, LifeformRealm _firstRealm, LifeformRealm _secondRealm) private {
        require(_firstRealm != _secondRealm, "SeedEvolution: First and second realm must differ");
        ...
        emit LifeformCreated(lifeformIdToInfo[_lifeformId]);
    }

    function claimLandDeeds(
    ...
    nonZeroLength(_lifeformIds)
    {
        for(uint256 i = 0; i < _lifeformIds.length; i++) {
            _claimLandDeed(_lifeformIds[i]);
        }
    }

    function _claimLandDeed(uint256 _lifeformId) private {
        ...
        require(_info.path == Path.MAGIC_AND_BC, "SeedEvolution: Only those who staked a BC get a land deed");

        require(block.timestamp >= _info.startTime + timeUntilLandDeed, "SeedEvolution: Too early to claim a land deed");

        require(!_info.hasClaimedLand, "SeedEvolution: Already claimed land deed");
        _info.hasClaimedLand = true;

        landDeed.safeMint(msg.sender);

        emit LandDeedClaimed(_lifeformId);
    }

    function claimImbuedSouls(
        uint256[] calldata _lifeformIds)
    external
    whenNotPaused
    contractsAreSet
    ...
    {
        for(uint256 i = 0; i < _lifeformIds.length; i++) {
            _claimImbuedSoul(_lifeformIds[i]);
        }
    }

    function _claimImbuedSoul(uint256 _lifeformId) private {
        require(userToLifeformIds[msg.sender].contains(_lifeformId), "SeedEvolution: User does not own this lifeform");
        ...
        require(_info.path != Path.MAGIC_AND_BC || _info.hasClaimedLand, "SeedEvolution: Must claim land deed before imbued soul");

        require(block.timestamp >= _info.startTime + timeUntilDeath, "SeedEvolution: Too early to claim imbued soul");

        // Send back BC if needed.
        if(_info.path == Path.MAGIC_AND_BC) {
            balancerCrystal.safeTransferFrom(address(this), msg.sender, balancerCrystalId, 1, "");
        }

        // Mint the imbued soul from generation 0
        imbuedSoul.safeMint(msg.sender,
            0,
            classForLifeform(_lifeformId),
            offensiveSkillForLifeform(_lifeformId),
            secondarySkillsForLifeform(_lifeformId));

        emit ImbuedSoulClaimed(_lifeformId);
    }
    ...
    function offensiveSkillForLifeform(uint256 _lifeformId) public view returns(OffensiveSkill) {
        ...
        require(block.timestamp >= lifeformIdToInfo[_lifeformId].startTime + timeUntilOffensiveSkill, "SeedEvolution: Has not been long enough for offensive skill");
    ...
    function secondarySkillsForLifeform(uint256 _lifeformId) public view returns(SecondarySkill[] memory) {
        ...
        _randomNumber = _randomNumber = uint256(keccak256(abi.encode(_randomNumber, _randomNumber)));
...
abstract contract SeedEvolutionContracts is Initializable, SeedEvolutionState {
    ...
    onlyAdminOrOwner
    {
        ...
        landDeed = ILandDeed(_landDeedAddress);
...
abstract contract SeedEvolutionState is Initializable, ISeedEvolution, ERC1155HolderUpgradeable, AdminableUpgradeable {
    ...
    event LifeformCreated(LifeformInfo _evolutionInfo);
    event LandDeedClaimed(uint256 _lifeformId);
    event ImbuedSoulClaimed(uint256 _lifeformId);

    IRandomizer public randomizer;
    ISeedOfLife public seedOfLife;
    IBalancerCrystal public balancerCrystal;
    IMagic public magic;
    ILandDeed public landDeed;
    ...
    function __SeedEvolutionState_init() internal initializer {
		...
		realmToSecondarySkills[LifeformRealm.NETHERREALM].push(SecondarySkill.DEMONIC_BLAST);
		realmToSecondarySkills[LifeformRealm.NETHERREALM].push(SecondarySkill.DEMONIC_WAVE);
		realmToSecondarySkills[LifeformRealm.NETHERREALM].push(SecondarySkill.DEMONIC_NOVA);
