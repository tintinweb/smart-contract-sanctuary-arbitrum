...
interface ITroveClaimer {
    function claim(ClaimInfo calldata _claimInfo, bytes memory _verificationSig) external;
}
...
contract TroveClaimer is Initializable, EIP712Upgradeable, TroveClaimerAdmin {
    ...
    function claimInfoHash(ClaimInfo calldata _claimInfo) public view returns (bytes32) {
        return _hashTypedDataV4(
            keccak256(abi.encode(CLAIMINFO_TYPE_HASH, _claimInfo.claimer, _claimInfo.badgeAddress, _claimInfo.badgeId))
        );
    }
...
import { TroveClaimerSettings, ClaimInfo, Initializable, ITroveBadges } from "./TroveClaimerSettings.sol";

abstract contract TroveClaimerAdmin is Initializable, TroveClaimerSettings {
    function __TroveClaimerAdmin_init() internal onlyInitializing {
        __TroveClaimerSettings_init();
    }
    ...
    function setBadgeStatus(
        ...
        badgeToEnabledStatus[_badgeAddress][_badgeId] = _enabled;
    }
    ...
    function undoClaim(ClaimInfo calldata _claimInfo) external requiresEitherRole(ADMIN_ROLE, OWNER_ROLE) {
        ...
        require(
            _claimInfo.badgeAddress == address(troveBadgeCollection),
            "TroveClaimer: badgeAddress does not match the current troveBadgeCollection"
        );
        troveBadgeCollection.adminBurn(_claimInfo.claimer, _claimInfo.badgeId, 1);
    }

    function _undoIsClaimedStatus(ClaimInfo calldata _claimInfo) internal requiresEitherRole(ADMIN_ROLE, OWNER_ROLE) {
        require(isClaimed(_claimInfo), "TroveClaimer: Cannot undo claim status of an unclaimed badge");
...
abstract contract TroveClaimerSettings is Initializable, TroveClaimerState {
    function __TroveClaimerSettings_init() internal onlyInitializing {
...
import { ITroveClaimer, ClaimInfo } from "../interfaces/ITroveClaimer.sol";
import { ITroveBadges } from "../interfaces/ITroveBadges.sol";
...
abstract contract TroveClaimerState is Initializable, ITroveClaimer, UtilitiesUpgradeable {
    bytes32 public constant CLAIMINFO_TYPE_HASH =
        keccak256("ClaimInfo(address claimer,address badgeAddress,uint256 badgeId,uint256 nonce)");

    address public signingAuthority;
    ITroveBadges public troveBadgeCollection;

    // Maps badgeAddress -> badgeId -> enabled status.
    ...
    error NotRecipient();
    error InvalidBadge(address badgeCollection, uint256 badgeId);
    error InvalidSignature(address signer);
    error BadgeAlreadyClaimed(address claimer, address badgeCollection, uint256 badgeId, bytes32 nonce);

    function __TroveClaimerState_init() internal onlyInitializing {
        __Utilities_init();
    }
}