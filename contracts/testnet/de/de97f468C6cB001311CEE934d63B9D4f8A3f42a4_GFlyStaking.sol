...
contract GFlyStaking is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, IGFlyStaking {
    ...
    uint96[36] public emissionsAtMonth;

    IGFly public gFly;

    mapping(uint16 => uint96) public unlockableAtEpoch;
    mapping(uint16 => uint96) public miningPowerAtEpoch;
     ...
     */
    function stake(uint96 amount, uint16 lock)
        external
        override
        nonReentrant
        whenNotPaused
        onlyAvailableLock(lock)
        returns (uint256)
    {
     ...
     */
    function addToStake(uint96 amount, uint256 stakeId)
        external
        override
        nonReentrant
        whenNotPaused
    ...
    function unStakeAll() external override nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            ...
            if (currentEpoch >= unlockEpoch) {
                _claim(stakeId);
                uint96 stakeAmount = stakeById[stakeId].amount;
     ...
     */
    function extendLockPeriod(uint256 stakeId, uint16 lock)
        external
        override
        whenNotPaused
        onlyStakeOwner(stakeId)
     ...
     */
    function claimableById(uint256 stakeId) public view override returns (uint96 total) {
        ...
        for (uint16 i = stakeById[stakeId].lastProcessEpoch; i < currentEpoch; i++) {
            ...
            if (month < 37) {
                total += uint96((emissionsAtMonth[month - 1] * individualMiningPower) / totalMiningPower);
            }
        }
    }
     ...
     */
    function claimableByAddress(address account) external view override returns (uint96 total) {
    ...
    function distributeEmissions() external override whenNotPaused {
        ...
        uint96 totalMiningPower;
        for (uint16 i = currentEpoch; i <= currentEpoch + 365; i++) {
            totalMiningPower += (unlockableAtEpoch[i] * uint96(i - currentEpoch)) / 365;
        }
     ...
     */
    function setPause(bool state) external override {
    ...
    function _claim(uint256 stakeId) internal returns (uint96 claimable) {
    ...
    function _claimAndRestake(uint256 stakeId) internal returns (uint96 claimable) {
        claimable = claimableById(stakeId);
    ...
    function _setEmissions() internal {
        for (uint96 i = 0; i < 36; i++) {
            emissionsAtMonth[i] = uint96(-3238093535907360000 * int96(i + 1) + 768238028330952000000);
        }
    }
}
...
interface IGFlyStaking {
    ...
    event Staked(address indexed account, uint256 indexed stakeId, uint96 amount, uint16 lock, uint16 unlockEpoch);
    event UnStaked(address indexed account, uint256 indexed stakeId, uint96 amount);
    ...
    event Claimed(address indexed account, uint256 indexed stakeId, uint96 amount);
    event ClaimedAndRestaked(address indexed account, uint256 indexed stakeId, uint96 amount);
    event Paused(bool state);
    event EmissionsDistributed(uint96 totalMiningPower, uint16 currentEpoch);

    function distributeEmissions() external;

    function stake(uint96 amount, uint16 lock) external returns (uint256);

    function addToStake(uint96 amount, uint256 stakeId) external;
    ...
    function claimableById(uint256 stakeId) external view returns (uint96 total);

    function claimableByAddress(address account) external view returns (uint96 total);
