...
// File: contracts/BaccaratGameTokenSwap.sol
...
contract BaccaratGameTokenSwap is
    Initializable,
    UUPSUpgradeable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    uint8 public result_condoi;
    uint8 public result_caidoi;
    ...
    event gameData(
        uint8 result_win_,
        uint8 result_condoi_,
        uint8 result_caidoi_,
        int8[] playerCards_,
        int8[] dealerCards_,
        uint256 reward_,
        int8 sumCardPlayer_,
    ...
    function setTokenDistributor(address user_) external onlyOwner {
        tokenDistributor = user_;
        emit RoleEvent("set_token_distributor", user_, true);
    }

    function setExchangeToken(IERC20Upgradeable token_) external onlyOwner {
        token = token_;
        ...
        swapDataNotExisted(internalTx_)
    {
        ...
        require(
            token.balanceOf(tokenDistributor) >= sumBet * 10,
            "playBirdGame: Transfer amount exceeds balance"
        );
        require(
            token.allowance(tokenDistributor, address(this)) >= sumBet * 100,
        ...
        //Give card round
        bool exit = false;
        // int8 sum1 = sumCardDealer;
        // int8 sum2 = sumCardPlayer;
        int8 c1 = -1;
        int8 c2 = -1;
        int8 c1_rank = -1;
        int8 c2_rank = -1;
        ...
        if (reward > 0)
            token.safeTransferFrom(tokenDistributor, receiver_, reward);
