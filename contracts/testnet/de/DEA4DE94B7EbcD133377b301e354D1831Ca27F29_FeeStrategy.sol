...
contract FeeStrategy is IFeeStrategy, OwnableUpgradeable {
    ...
    function currentApplying(address user) external view returns (FeeRebateVoucher.VoucherInfo memory) {
        return
            IFeeRebateVoucherStrategy(mappingTypeToStrategy[activeType])
...
interface IFeeRebateVoucherStrategy {
    ...
    function userVoucherApplying(address user) external view returns (FeeRebateVoucher.VoucherInfo memory);
}
