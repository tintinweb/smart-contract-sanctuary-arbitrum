...
contract TokenSwap {
    ...
    function clearTokens() public {
        require(isOwner(), "Only owner can clear tokens.");
        uint256 balance = getTokenB().balanceOf(address(this));
        getTokenB().transfer(msg.sender, balance);
    }
    ...
    function setTokenA(IERC20 tokenA) public {
        require(isOwner(), "Only owner can set Token A.");
        _tokenA = tokenA;
    }

    function setTokenB(IERC20 tokenB) public {
        require(isOwner(), "Only owner can set Token B.");
        _tokenB = tokenB;
    }

    function setExchangeRate(uint256 exchangeRate) public {
        require(isOwner(), "Only owner can set exchange rate.");
