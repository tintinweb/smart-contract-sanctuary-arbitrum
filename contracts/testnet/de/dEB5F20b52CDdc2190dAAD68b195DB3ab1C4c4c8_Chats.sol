...
contract Chats is Ownable, ReentrancyGuard, Pausable {
    ...
    function getChatByUser(address _address) external view returns (Chat memory) {
        ...
        (IUsers.User memory user, bytes memory extraData)  = users.getUserByAddress(_address);
        bytes32 chatId = user.currentChatId;
    ...
    function _startChat(address _callee) private returns (bytes32) {
        ...
        IUsers.User memory user;
        bytes memory extraData;
        (user, extraData)  = users.getUserByAddress(msg.sender);
        bytes32 callerCurrentChatId = user.currentChatId;
        ...
        (user, extraData)  = users.getUserByAddress(_callee);
        bytes32 calleeCurrentChatId = user.currentChatId;
        require (
            calleeCurrentChatId == '' ||
            (
                calleeCurrentChatId != '' &&
    ...
    function startChat(address _callee) external nonReentrant whenNotPaused returns (bytes32) {
        return _startChat(_callee);
    }

    function confirmChat(bytes32 _id) external nonReentrant whenNotPaused {
    ...
    function cancelChat(bytes32 _id) external nonReentrant whenNotPaused {
        _cancelChat(_id);
    }

    function cancelCurrentStartNewChat(bytes32 _chatId, address _callee) external nonReentrant whenNotPaused returns (bytes32) {
        IUsers users = IUsers(usersContract);
        (IUsers.User memory user, bytes memory extraData)  = users.getUserByAddress(msg.sender);
        bytes32 currentChatId = user.currentChatId;
    ...
    function rejectChat(bytes32 _id) external nonReentrant whenNotPaused {
        IUsers users = IUsers(usersContract);
        Chat storage chat = chatsMapping[_id];
    ...
    function finishChat(bytes32 _id) external nonReentrant whenNotPaused {
        Chat storage chat = chatsMapping[_id];
        ...
        (IUsers.User memory caller, bytes memory extraData)  = users.getUserByAddress(chat.caller);
        (uint totalFee, uint duration) = getUnclaimedFee(chat.id);
        uint lockedAmount = caller.lockedAmount;
    ...
    function extendChat(bytes32 _id) external nonReentrant whenNotPaused {
        Chat storage chat = chatsMapping[_id];
        require(chat.status == Statuses.started, 'Chat status is not "started"');
    ...
    function getUnclaimedFee(bytes32 _id) public view returns (uint, uint) {
        Chat storage chat = chatsMapping[_id];
        uint start;
        uint end;
        ...
        uint fee = chat.fee * (end - start) / 3600;
