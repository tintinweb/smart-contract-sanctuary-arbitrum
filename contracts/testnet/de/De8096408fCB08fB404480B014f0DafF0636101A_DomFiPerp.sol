...
contract DomFiPerp is IDomFiPerp, ReentrancyGuard {
    ...
    // trading:
    mapping(bytes32 => Position) private productTotalPosition;
    ...
    function _increasePosition(
        ...
        _updateProductTotalPosition(params, funding, price, positionId);
    }
    ...
    function getTotalPnl() external override returns (FPSigned totalPnl) {
        for (uint i = 0; i < productIds.length; i++) {
            if (productTotalPosition[productIds[i]].productId == 0) {
                continue; // pnl for this product is 0
            }
            FPUnsigned currentPrice = oracle.getPrice(productIds[i]);
            (FPSigned pnl, ) = getPositionPnLAndFunding(
                productTotalPosition[productIds[i]],
                currentPrice
            );
    ...
    function _updateProductTotalPosition(
        IncreasePositionParams calldata params,
        FPSigned funding,
        FPUnsigned price,
        bytes32 positionId
    ) private {
        Position storage totalPosition = productTotalPosition[params.productId];
        if (totalPosition.margin == 0) {
            totalPosition.price = positions[positionId].price;
            totalPosition.funding = positions[positionId].funding;
            totalPosition.leverage = positions[positionId].leverage;
            totalPosition.margin = positions[positionId].margin;
            totalPosition.productId = positions[positionId].productId;
        } else {
            (
                FPUnsigned newPrice,
                FPSigned newFunding,
            ...
            totalPosition.price = newPrice;
            totalPosition.funding = newFunding;
            totalPosition.leverage = newLeverage;
            totalPosition.margin = newMargin;
        }
    }
