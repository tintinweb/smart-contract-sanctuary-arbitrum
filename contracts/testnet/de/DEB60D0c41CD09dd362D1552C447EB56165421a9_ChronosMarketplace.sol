    ...
    IChronosMarketPlace
{
    ...
    function initialize(uint16 _platformFee) public initializer {
        __Ownable_init();
    ...
    /// @inheritdoc IChronosMarketPlace
    function isChronosNft(address _nft) public view override returns (bool) {
        for (uint256 i = 0; i < chronosNftList.length; i++) {
            if (chronosNftList[i] == _nft) return true;
        }
        return false;
    }
...
interface IChronosMarketPlace {
    ...
    function isChronosNft(address nft) external view returns (bool);
