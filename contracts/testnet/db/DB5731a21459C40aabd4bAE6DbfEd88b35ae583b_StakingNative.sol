...
// File: contracts/stackingNative.sol

pragma solidity ^0.8.0;

contract StakingNative is Ownable {
    ...
    IERC20 public immutable tokenRw;

    uint256 public withdrawPeriod = 0 days;
    bool isEmergencyWithdraw = false;
    ...
    event EmergencyRewardWithdraw(address indexed owner, uint256  amount);
    event Withdrawal(uint256 balance);

    constructor() {
        tokenRw = IERC20(0x67dAa9ae4dF7630C9F789fEEa9420c77Ab21BAa9);
    }
    ...
    function pendingToken(address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastTimeReward && lpSupply != 0) {
            uint256 multiplier = _getMultiplier(pool.lastTimeReward, block.timestamp, pool.rewardEndTime);
            uint256 tokenReward = multiplier * (pool.tokenPerSecond);
    ...
    function withdraw(uint256 _amount) public {
        ...
        if (pending > 0) {
            tokenRw.transfer(msg.sender, pending);
        }

        if (_amount > 0) {
            require(block.timestamp >=  user.lastDepositedTime + withdrawPeriod, "withdraw: lock time is not enough");
    ...
    function updateRewardPerSecond(uint256 _rewardPerSecond) external onlyOwner {
        require(block.timestamp < poolInfo.startTime, "Pool has started");
        poolInfo.tokenPerSecond = _rewardPerSecond;
        emit NewRewardPerSecond(_rewardPerSecond);
    }

    /*
     * @notice Update limit time withdraw
     * @dev Only callable by owner.
     */
    function setWithdrawFeePeriod(uint256 _withdrawPeriod) external onlyOwner {
    ...
    function emergencyRewardWithdraw(uint256 _amount) external onlyOwner {
        tokenRw.transfer(address(msg.sender), _amount);
        emit EmergencyRewardWithdraw(msg.sender, _amount);
    }

    /*
     * @notice Update emergency withdraw
     * @dev Only callable by owner.
     */
    function setEmergencyWithdraw(bool _isEmergencyWithdraw) external onlyOwner {
        isEmergencyWithdraw = _isEmergencyWithdraw;
    }
    
    /*
     * @notice update rate stacking
     * @dev Only callable by owner.
     */
    function updateRateToken(uint256 _tokenPerSecond) external onlyOwner {
          updatePool();
          PoolInfo storage pool = poolInfo;
          pool.tokenPerSecond = _tokenPerSecond;
    }
    ...
    function emergencyWithdraw() public {
        ...
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[msg.sender];
        require(isEmergencyWithdraw , "withdraw error");
