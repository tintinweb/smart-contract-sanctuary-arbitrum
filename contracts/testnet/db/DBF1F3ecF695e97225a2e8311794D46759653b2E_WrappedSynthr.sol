...
contract BaseWrappedSynthr is Proxyable, MixinResolver {
    ...
    function collateralTransfer(
        ...
        if (extTokenState.collateralCurrency(_collateralKey) == NULL_ADDRESS) {
            ...
            uint256 synthInUSDC = exchangeAgent().getUSDCForSYNTH(synthAmountToLiquidate);
        } else {
            require(
...
contract WrappedSynthr is BaseWrappedSynthr {
    ...
    function fixCollateralAmount(
        ...
        if (_collateralKey != bytes32(0)) {
            bool isSucceed = _mint(_account, _collateralAmountToFix, _collateralKey);
            require(isSucceed, "Mint Synthr failed");
            emit StakeCollateral(_account, _account, _collateralKey, _collateralAmountToFix);
        }
        if (isSynthFix) {
