...
contract KEIManager is Owned {
    ...
    function depositAndMint(address _token, uint256 _amount) public {
        require(reserveController.isWhitelisted(_token) == true, "Stablecoin not supported.");
        ...
        reserveController.updateReserves(_token, mintAmount, true);
    }
    ...
    function depositUSDCReserves(uint256 _amount, address _token) public {
        ERC20(_token).approve(address(usdcHandler), _amount); // 0xd513E4537510C75E24f941f159B7CAFA74E7B3B9
        usdcHandler.depositUSDC(_amount);
        deployedTokens[_token] += _amount;
    }

    function withdrawUSDCReserves(uint256 _amount, address _token) public {
        usdcHandler.withdrawUSDC(_amount);
        deployedTokens[_token] -= _amount;
    }
    ...
    function setUSDCHandler(address _contract) public {
        usdcHandler = USDCHandler(_contract);
    }

    function setDAIHandler(address _contract) public {
        daiHandler = DAIHandler(_contract);
    }
...
contract KEI is Owned, ERC20 {
    ...
    string private constant NAME = "PEI Stablecoin";
    string private constant SYMBOL = "PEI";
...
contract ReserveController is Owned{
    ...
    uint8 public allowedDeviation;
    uint256 private constant SCALE_FACTOR = 10000; // To handle decimal percentages
    ...
    function calculatePenalty(address _stablecoin, uint256 depositAmount) public view returns (uint256) {
        ...
        uint256 currentReserves = stablecoinReserves[_stablecoin];
        uint256 newReserves = currentReserves + depositAmount;
        uint256 totalReserves = calculateTotalReserves();

        // Calculate current and new weight percentages
        uint256 currentWeight = currentReserves * SCALE_FACTOR / totalReserves;
        uint256 newWeight = newReserves * SCALE_FACTOR / totalReserves;
        uint256 deviation;
        ...
        if (deviation <= 500) { // 5%, scaled up
            return 0;
        } else {
            // Polynomial fee calculation
            uint256 penalty = a * deviation * deviation / SCALE_FACTOR 
                          + b * deviation / SCALE_FACTOR;

            if (penalty > 9000) { // Cap at 90%, scaled up
                return 9000;
            } else {
                return penalty;
            }
    ...
    function maxDepositWithoutPenalty(address _stablecoin, uint256 totalReserves) public view returns (uint256) {
        uint256 reserves = stablecoinReserves[_stablecoin];
    ...
    // todo remove from whitelistedStablecoins 
    ...
    function calculateTotalReserves() public view returns (uint256) {
        uint256 total = 0;
        for (uint i = 0; i < supportedStablecoins.length; i++) {
            address stablecoin = supportedStablecoins[i];
            total += stablecoinReserves[stablecoin];
        }
        return total;
    }
    ...
    function calculatePercentage(address _stablecoin) public view returns (uint256) {
        ...
        return (stablecoinReserves[_stablecoin] * 100) / totalReserves;
    }

    function updateReserves(address _stablecoin, uint256 _amount, bool _value) public {
        if (_value == true) {
