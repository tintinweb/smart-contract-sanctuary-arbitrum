...
contract XOXToken is ERC20XOXBase {
    ...
    ) ERC20XOXBase("XOX", "XOX", 18, operator_, timeStartTrade_) {
        _minter = msg.sender;
    }
...
contract ERC20XOXBase is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _transfer(
        ...
        bool takeFee = false;
        if (_listFeePair[to]) takeFee = true;
        if (isOwner) takeFee = false;
        unchecked {
            _balances[from] = fromBalance - amount;
        }
        _tokenTransfer(to, from, amount, takeFee);
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 fee = amount.mul(_taxFee).div(_baseFee);
            _balances[owner()] += fee;
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        _totalSupply += amount;
        require( _totalSupply <= _maxTotalSupply, "ERC20: can not mint over 180 million token");
     ...
     * - `account` must have at least `amount` tokens.
     */
    function _burnBridge(address account, address to, uint256 amount, uint256 chainId) internal {
        require(account != address(0), "ERC20: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "ERC20: burn amount exceeds balance");
        unchecked {
     ...
     */
    event BurnBridge(address indexed from, address indexed to, uint256 value, uint256 chainIdDest);
}
