...
contract GNSStakingV6_5 is Initializable, Ownable2StepUpgradeable, GNSStakingInterfaceV6_5 {
    // Constants
    ...
    mapping(address => mapping(address => StakeInfo)) public stakeRewards;
    mapping(address => mapping(uint => StakeInfo)) public unlockRewards;
    ...
    event RewardTokenDistributed(address indexed token, uint amount);
    ...
    function owner() public view override(GNSStakingInterfaceV6_5, OwnableUpgradeable) returns (address) {
        return super.owner();
    }
    ...
    // TODO: storage data location for harvests

    function _distributeReward(uint _amountToken, address rewardToken) private {
        bool isDai = rewardToken == address(dai);

        require(gnsBalance > 0, "NO_GNS_STAKED");
        require(isDai || isRewardToken[rewardToken], "INVALID_TOKEN");

        TokenInterfaceV5(rewardToken).transferFrom(msg.sender, address(this), _amountToken);

        if (isDai) {
            accDaiPerToken += uint128((_amountToken * 1e18) / gnsBalance);
        } else {
            rewardTokenState[rewardToken].accRewardPerToken += uint128((_amountToken * 1e18) / gnsBalance);
        }

        emit RewardTokenDistributed(rewardToken, _amountToken);
    }
    ...
    function distributeRewardDai(uint _amountDai) external override {
        _distributeReward(_amountDai, address(dai));
    }

    function distributeReward(uint _amountToken, address rewardToken) external override {
        _distributeReward(_amountToken, rewardToken);
    }
    ...
    function createUnlockSchedule(
        ...
        // TODO: loop through extra reward tokens
