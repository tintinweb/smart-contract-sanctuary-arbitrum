    ...
    function liquidatePosition(uint256 _posId) external nonReentrant {
        ...
        if (_pnl < position.collateral) {
            ...
            if (msg.sender == liquidateRegistrant[_posId] || liquidateRegistrant[_posId] == address(0)) {
                ...
                uint256 bountyCaller = (uint256(remainingCollateral) * uint256(firstCallerPercent + resolverPercent)) /
                    BASIS_POINTS_DIVISOR;
                vault.takeVUSDOut(msg.sender, bountyCaller);

                vault.takeVUSDOut(settingsManager.feeManager(), uint256(remainingCollateral) - bountyCaller);
            } else {
                uint256 bountyCaller = (uint256(remainingCollateral) * uint256(firstCallerPercent)) /
                    BASIS_POINTS_DIVISOR;
                vault.takeVUSDOut(liquidateRegistrant[_posId], bountyCaller);
                uint256 bountyResolver = (uint256(remainingCollateral) * uint256(resolverPercent)) /
                    BASIS_POINTS_DIVISOR;
                vault.takeVUSDOut(msg.sender, bountyResolver);

                vault.takeVUSDOut(
                    settingsManager.feeManager(),
                    uint256(remainingCollateral) - bountyCaller - bountyResolver
                );
            }

            vault.accountDeltaIntoTotalUSD(true, _pnl);
        } else {
            vault.accountDeltaIntoTotalUSD(true, position.collateral);
        }
