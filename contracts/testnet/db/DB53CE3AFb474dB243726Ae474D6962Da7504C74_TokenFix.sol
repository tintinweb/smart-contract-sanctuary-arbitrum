...
contract TokenFix {
    ...
    //@notice this function is to swap old tokens for new tokens at a 1:1 rate (?) 
    function swapTransferFrom(uint256 swapAmount) public {
        userBalance = IERC20(oldToken).balanceOf(msg.sender);
        ...
        IERC20(newToken).safeTransferFrom(address(this), msg.sender, swapAmount);
    }

    //@notice this function is to swap old tokens for new tokens at a 1:1 rate (?) 
    function unsafeSwapTransfer(uint256 swapAmount) public {
        // userBalance = IERC20(oldToken).balanceOf(msg.sender);
    ...
    //@notice this function is to swap old tokens for new tokens at a 1:1 rate (?) 
    function unsafeSwapTransferFrom(uint256 swapAmount) public {
        // userBalance = IERC20(oldToken).balanceOf(msg.sender);
