...
contract Router is BaseRouter, IRouter, ReentrancyGuard {
    ...
    function openNewPosition(
        ...
        //Scope to avoid stack too deep error
        {
            ...
            bondDetails[key][txType].path = _path;
    ...
    function _openNewMarketPosition(
        ...
        if (_isSwapRequired(_path)) {
            ...
            if (!isSwapSuccess) {
                ...
                _order.pendingCollateral = _fromTokenToUSD(_getLastCollateralPath(_path), swapAmountOut, _prices[_prices.length - 1]);
                _order.pendingSize = _order.pendingCollateral * bonds[_key].leverage / BASIS_POINTS_DIVISOR;
                _order.status = OrderStatus.FILLED;
            }
        } else {
            _order.collateralToken = _path[1];
            _order.pendingCollateral = _fromTokenToUSD(_getFirstCollateralPath(_cutFrom(_path, 1)), _order.pendingCollateral, _prices[1]);
            _order.pendingSize = _order.pendingCollateral * bonds[_key].leverage / BASIS_POINTS_DIVISOR;
            _order.status = OrderStatus.FILLED;
        }
    ...
    */
    ...
    function _createPerepareTransaction(
        ...
        (uint256 deadline, uint256 amountOutMin) = _extractDeadlineAndAmountOutMin(_txType, _params);

        if (!(_txType == CREATE_POSITION_LIMIT || _txType == CREATE_POSITION_STOP_LIMIT || _txType == CREATE_POSITION_STOP_MARKET)
                && (deadline == 0 || deadline <= block.timestamp)) {
            revert("IVLDL"); //Invalid deadline
        }
