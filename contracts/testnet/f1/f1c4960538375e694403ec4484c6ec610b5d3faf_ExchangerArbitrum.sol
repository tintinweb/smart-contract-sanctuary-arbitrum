...
interface ArbSys {
    ...
    function arbBlockHash(uint64 blocknum) external view returns (bytes32);
}

address constant ARBSYS_ADDR = address(100);

contract ExchangerArbitrum is IExchanger, TypeAndVersionInterface, ConfirmedOwner {
    ...
    function commitTrade(bytes32 commitment) external override {
        ...
        s_commitmentReceived[commitment] = ArbSys(ARBSYS_ADDR).arbBlockNumber() - 1;
    ...
    function resolveTradeWithReport(
        ...
        // if (ArbSys(ARBSYS_ADDR).arbBlockNumber() > s_commitmentReceived[commitmentHash] + s_maxDelay)
        //     revert TradeExceedsWindow(
        //         ArbSys(ARBSYS_ADDR).arbBlockNumber(),
        ...
        // if (report.upperBlockhash != ArbSys(ARBSYS_ADDR).arbBlockHash(report.blocknumberUpperBound))
        //     revert BlockhashMismatch(report.upperBlockhash, ArbSys(ARBSYS_ADDR).arbBlockHash(report.blocknumberUpperBound));
