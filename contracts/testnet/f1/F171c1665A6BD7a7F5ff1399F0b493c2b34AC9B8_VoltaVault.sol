...
contract Vault is ReentrancyGuard, VoltaVaultNFT, Ownable {
    ...
    mapping(uint256 => bool) premiumVault;
    ...
    event PremiumPaid(uint256 vaultID, address recipient, address token, uint256 amount);
    ...
    modifier checkPremium(uint256 vaultId) {
        if(premiumVault[vaultId]) {
            require(block.timestamp > unlockTime[vaultId], "Vault is locked");
        }
        _;
    }
    ...
    function depositCollateral(uint256 vaultID, uint256 amount) external checkPremium(vaultID) {
        require(_exists(vaultID), "Vault does not exist");
    ...
    function payBackToken(uint256 vaultID, uint256 amount) external checkPremium(vaultID) {
    ...
    function payPremium(uint256 vaultId, address recipient, address token, uint256 amount) external onlyOwner {
        require(((IERC20(token).balanceOf(address(this)))) >  amount, "Contract does not have enough balance to transfer");

        premiumVault[vaultId] = true;
        unlockTime[vaultId] = block.timestamp + 7 days;

        require(IERC20(token).transfer(recipient, amount), "Transfer failed");

        emit PremiumPaid(vaultId, recipient, token, amount);
    }
