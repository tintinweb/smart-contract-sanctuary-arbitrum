...
contract UniforgeCollection is Ownable, ERC721A, ERC2981, DefaultOperatorFilterer{
     ...
     * The caller has to send `mintFee`*`quantity` ether and the sale should be open.
    ...
    /**
     * @dev Allows the contract owner to set the maximum number of tokens minted at once.
     * @param quantity The new mint limit.
     */
     ...
     * creators with revenue strategies while giving the option to remain fully decentralized.
     *
     * When `_royaltyEnforced` is set to true, approval and transfer operations are overriden
     * reverting operations when the operator is contained in the CORI subscription blacklist. 
     * Therefore, the NFTs of this contract are only traded in ERC2981 compliant marketplaces.
     *
     * When `_royaltyEnforced` is set to false, this contract does not interact with the
     * operator filter for approvals and transfers, saving gas in every market transaction.
     * It keeps signaling the royalty information from the ERC2981 standard, but it does not
     * enforce any filters for operators that limit the performance of certain marketplaces.
     */ 
    ...
    /**
     * @notice If `_royaltyEnforced` is set to true, inherits from Opensea's Operator Filter.
     */
