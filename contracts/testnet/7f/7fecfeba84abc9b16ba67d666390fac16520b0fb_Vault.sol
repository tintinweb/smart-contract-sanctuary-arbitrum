...
contract Vault is ReentrancyGuard, AccessControlBase, IVault {
    ...
    mapping (address => uint256) public circuitBreakerAmounts;
    ...
    function payout(
         ...
         */
        if (_tokens[0] == _tokens[1]) { // wagerAsset in, is the same as winAsset
            ...
            if(totalAmountOut_ == 0) { // the player has effectively won nothing converted into _tokens[1]
            ...
            if (totalAmountOut_ >= amountOutAfterSwap_) { // vault has made a loss
                ...
                if (feesPaidInOut_ >= totalAmountOut_) {
                    ...
                    // WP-H3: register the feesPaidInOut_ as profit for the vault/WLPs
                    _increasePoolAmount(_tokens[1], totalAmountOut_); // !!! this is wew
                    unchecked {
     ...
     */
    function directPoolDeposit(address _tokenIn) external override protocolNotPaused onlyManager {
        require(
    ...
    function _swap(
...

        {
            ...
            if (decimalsOut_ >= decimalsIn_) {
                amountOut_ = Math.mulDiv(_amountIn * 10**(decimalsOut_ - decimalsIn_), priceIn_, priceOut_);
            } else {
                amountOut_ = Math.mulDiv(_amountIn, priceIn_, priceOut_ * 10**(decimalsIn_ - decimalsOut_));
            }

            if (USDW_DECIMALS >= decimalsIn_) {
                usdwAmount_ = Math.mulDiv(_amountIn * 10**(USDW_DECIMALS - decimalsIn_), priceIn_, 1e30);
            } else {
                usdwAmount_ = Math.mulDiv(_amountIn, priceIn_, 1e30 * 10**(decimalsIn_ - USDW_DECIMALS));
            }
        }
    ...
    function _collectWagerFees(
        ...
        unchecked { wagerFeeReserves[_tokenEscrowIn] += wagerFeeCharged_; }
    ...
    function _collectSwapFees(
        ...
        unchecked { swapFeeReserves[_tokenAddress] += feesPaidInOut_; }
        return (amountOutAfterFees_, feesPaidInOut_);
    }
   ...
   /**
    ...
    function _increasePoolAmount(
        ...
        unchecked { poolAmounts[_tokenIn] += _amountToIncrease; }
    ...
    function _decreasePoolAmount(
        ...
        unchecked { poolAmounts[_tokenOut] -= _amountToDecrease; }
        // check if circuit breaker is enable in vault config
        if(circuitBreakerEnabled) {
            // if enbaled, check if the poolAmounts are below the circuitBreakerAmounts
            if(poolAmounts[_tokenOut] < circuitBreakerAmounts[_tokenOut]) {
                // if so, call the WLP manager (whom will enforce a policy or not)
    ...
    function _increaseUsdwAmount(
        address _token, 
        uint256 _amountToIncrease) private {
        unchecked { usdwAmounts[_token] += _amountToIncrease; }
    ...
    function _adjustForDecimals(
        ...
        if (_decimalsTokenOut >= _decimalsTokenIn) {
            scaledAmount_ = Math.mulDiv(_amountA * 10**(_decimalsTokenOut - _decimalsTokenIn), _amountB, _amountDiv);
        } else {
            scaledAmount_ = Math.mulDiv(_amountA, _amountB, _amountDiv * 10**(_decimalsTokenIn - _decimalsTokenOut));
        }
    }
    ...
    function tokenToUsdMin(
        ...
        uint256 _tokenAmount) external override view returns (uint256 usdAmount_) {
        // using the lower price bound of the asset
        usdAmount_ = (_tokenAmount * getMinPrice(_tokenToPrice)) / (10 ** tokenDecimals[_tokenToPrice]);
    }
    ...
    function usdToTokenMax(
        address _tokenToPrice, 
        uint256 _usdAmount) external view returns (uint256 tokenAmountMax_) {
        // using the lower price bound of the asset
    ...
    function usdToTokenMin(
        address _tokenToPrice, 
        uint256 _usdAmount) external view returns (uint256 tokenAmountMin_) {
    ...
    function getReserve() external override view returns(uint256 totalReserveValue_) {
        totalReserveValue_ = (IWLPManager(wlpManagerAddress).getAum(false) - IWLPManager(wlpManagerAddress).reserveDeduction());
    }
    ...
    function _amountOfTokenForToken(
    ...
    ) internal view returns(uint256 amountTokenOut_) {
        {
            ...
            if (decimalsOut_ >= decimalsIn_) {
                amountTokenOut_ = Math.mulDiv(_amountIn * 10**(decimalsOut_ - decimalsIn_), priceIn_, priceOut_);
            } else {
                amountTokenOut_ = Math.mulDiv(_amountIn, priceIn_, priceOut_ * 10**(decimalsIn_ - decimalsOut_));
            }
        }
    }
     ...
     * @param _token address of the token
     */
    function getTargetUsdwAmount(address _token) external override view returns (uint256 usdwAmount_) {
