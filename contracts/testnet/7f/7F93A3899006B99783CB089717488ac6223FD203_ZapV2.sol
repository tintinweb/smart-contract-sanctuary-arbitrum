...
interface IRewardPool {
  ...
  function userInfo(
    uint _pid,
    address _user
  ) external view returns (uint amount, uint rewardDebt);
}
  ...
  function _swapETHToLP(
    ...
    if (tokenType[_lpToken] == TokenType.LP) {
      ...
      if (token0 == WETH || token1 == WETH) {
        ...
        }(altToken, altTokenAmount, 0, 0, _recipient, block.timestamp + 40);
        emit AddLiquidity(
          WETH,
          altToken,
...
contract ZapV2 is ZapBase {
  ...
  function zapIntoACWithToken(
    ...
    IStrategyManager(_strategyManager).depositFor(_pid, amountOut, msg.sender);
