...
contract Router is AccessControl, IRouter {
    ...
    bytes32 public constant ROUTER_WRAPPER_ROLE =
        keccak256("ROUTER_WRAPPER_ROLE");
    ...
    function refund(address user, address targetContract) external override onlyRole(ROUTER_WRAPPER_ROLE) {
    ...
    function runParameterChecks(
        ...
        if (params.game == IRouterWrapper.Game.SLOTS) {
            require(
                Validator.verifyUserBetParams(params, signer),
        ...
        } else if (params.game == IRouterWrapper.Game.DICE) {
            require(
                Validator.verifyDiceBetParams(params, signer),
                "Router: User signature didn't match"
            );
    ...
    function initiatePlay(
        IRouterWrapper.BetParams calldata params
    ) external payable override onlyRole(ROUTER_WRAPPER_ROLE) {
        if (params.game == IRouterWrapper.Game.SLOTS) {
            ISlotsGame gameContract = ISlotsGame(params.targetContract);
...
library Validator {
    ...
    function getUserBetHash(
        IRouterWrapper.BetParams memory params
