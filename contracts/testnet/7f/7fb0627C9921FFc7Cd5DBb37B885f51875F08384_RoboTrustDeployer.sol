...
contract RoboTrustDeployer {
    ...
    function createTrust(uint256 _period, uint _maxPayouts, uint[] memory _paymentAmounts, address _trustee, address _beneficiary) public payable {
        RoboTrust newTrust = (new RoboTrust){value: msg.value}(msg.sender, _trustee, _beneficiary, _period, _maxPayouts, _paymentAmounts, priceFeedAddress);
...
contract RoboTrust is FinancialMathHelper {
    ...
    constructor(address _owner, address _trustee, address _beneficiary, uint256 _period, uint _maxPayouts, uint[] memory _paymentAmounts, address _priceFeedAddress) FinancialMathHelper(_priceFeedAddress) payable {
