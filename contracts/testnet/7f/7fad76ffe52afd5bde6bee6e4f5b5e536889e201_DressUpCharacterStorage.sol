...
contract DressUpCharacterStorage is Ownable, StatusHandler, ImageDataHandler {
    ...
    function setTransformDatas(uint256 index, bytes memory data) external onlyOwner {
        _setTransformDatas(index, data, 0);
    }
    ...
    function setStyleDatas(uint256 index, bytes memory data) external onlyOwner {
        _setTransformDatas(index, data, 1);
    }
        ...
        returns (string memory result)
    {
        assembly {
            ...
            // mstore(mc, ',{"trait_type":"traitA","value":"valueA"}')
            mstore(mc, "%2C%7B%22trait_type%22%3A%22trai")
            mc := add(mc, 32)
...
library Base64 {
    ...
    /// See: https://datatracker.ietf.org/doc/html/rfc4648
    /// @param fileSafe  Whether to replace '+' with '-' and '/' with '_'.
    function uriEncode(bytes memory data, bool fileSafe) internal pure returns (string memory result) {
        /// @solidity memory-safe-assembly
        assembly {
            let dataLength := mload(data)
            ...
            if dataLength {
                ...
                for {} 1 {} {
                    ...
                    for {
                        ...
                        let temp := mload(and(shr(cc, input), 0x3F))

                        switch lt(temp, 0x5f)
                        // if "-" or "_" , translate "%2B" or "%2F"
                        case 1 {
                            mstore(i, add(0x253242, mul(4, sub(temp, 0x5f))))
                            i := add(i, 3)
                        }
                        default {
                            mstore8(i, temp)
                ...
                // Add 31 and mask with `not(31)` to round the
                // free memory pointer up the next multiple of 32.
                mstore(0x40, and(add(end, 35), not(31)))

                // Equivalent to `o = [0, 2, 1][dataLength % 3]`.
                let o := div(2, mod(dataLength, 3))

                // Offset `ptr` and pad with '%3D'. We can simply write over the end.
                mstore(sub(ptr, o), shl(208, 0x253344253344))
                // Write the length of the string.
                mstore(result, add(encodedLength, mul(o, 2)))
            }
        }
    }
    ...
    /// @dev Encodes `data` using the base64 encoding described in RFC 4648.
    /// Equivalent to `encode(data, false, false)`.
    function uriEncode(bytes memory data) internal pure returns (string memory result) {
        result = uriEncode(data, false);
    }
}
...
abstract contract ImageDataHandler {
    ...
    // // _IMAGE_DATAS / _MAIN_DATAS / _SUB_DATAS / _TRANSFORM_DATAS
    ...
    // _IMAGE_DATAS / _MAIN / _TRANSFORM_DATAS / _STYLE_DATAS_SEED
    // _IMAGE_DATAS / _SUB / _TRANSFORM_DATAS / _STYLE_DATAS_SEED
    // fbbce30e / e466491e / 8e2a671f / dfa6c9a2
    //  16 / 12 / 8 / 4
    uint256 private constant _M_DATAS_SEED = 0xfbbce30ee466491e8e2a671fdfa6c9a2;
    uint256 private constant _P_DATAS_SEED = 0xfbbce30e05e534fb8e2a671fdfa6c9a2;
    ...
    uint256 private constant _MAIN_TRANSFORM_DATAS_SEED = 0x8e2a671f;
    ...
    /* /////////////////////////////////////////////////////////////////////////////
    MAIN_DATAS [slot = 0] / SUB_DATAS [slot = 1]
    ///////////////////////////////////////////////////////////////////////////// */
    function _setMainDatas(uint256[] memory imageIds, uint256 characterId) internal {
        _setCharacterDatas(imageIds, characterId, 0);
    }

    function _setSubDatas(uint256[] memory imageIds, uint256 characterId) internal {
        _setCharacterDatas(imageIds, characterId, 1);
    }
    ...
    function _setCharacterDatas(uint256[] memory imageIds, uint256 characterId, uint256 slot) internal {
        assembly {
            ...
            // write to storage
            mstore(0x12, add(_M_DATAS_SEED, slot))
    ...
    function _markCharacterRender(uint256 characterId, uint256 slot) internal view returns (string memory) {
        string memory symbol = _createBg(characterId);
        string memory useContent = '<use href="#0" x="0" y="0" clip-path="url(#mark)" />';
    ...
    function _setCharacterTransformDatas(uint256 characterId, uint256 characterTransformData) internal {
        assembly {
            // write to storage
            mstore(0x04, _MAIN_TRANSFORM_DATAS_SEED)
            mstore(0x00, characterId)
    ...
    function _createUseContent(uint256 categoryId, uint256 transformId) internal view returns (string memory) {
        return string.concat(
            ...
            categoryId.toString(),
            '" x="0" y="0" transform="',
            _getTransformDatas(transformId, 0),
            '" />'
        );
    }
    ...
    function _setTransformDatas(uint256 index, bytes memory data, uint256 slot) internal {
        assembly {
            slot := mul(0x04, add(iszero(slot), 1))
            mstore(slot, _M_DATAS_SEED)
            mstore(0x00, index)
    ...
    function _getTransformDatas(uint256 index, uint256 slot) internal view returns (string memory data) {
        assembly {
            // free memory pointer
            data := mload(0x40)
    ...
    // function createDataURI(uint256 tokenId, uint256 slot, uint256 markCheck) public view returns (string memory) {
    //     string memory imageURI = Base64.encode(bytes(_characterRenderAssembly(tokenId, slot)));
    ...
    function _characterRender(uint256 characterId, uint256 slot) internal view returns (string memory result) {
        assembly {
            function toStr(p, x) -> rp {
                // use scratch space
                let ss := 0x20
            ...
            // _prefix
            mstore(mc, '<svg xmlns="http://www.w3.org/20')
            mc := add(mc, 32)

            mstore(mc, '00/svg" viewBox="0 0 1000 1000">')
            mc := add(mc, 32)
            ...
            for {
                ...
                switch i
                case 0 {
                    mstore(mc, '<rect width="1000" height="1000"')
                    mc := add(mc, 32)
    ...
    function _firstConcatCharacterRender(uint256 characterId, uint256 slot)
        internal
        view
        returns (string memory result)
    {
    ...
    // for special character
    function _secondConcatCharacterRender(uint256 characterId, uint256 slot)
        internal
        view
        returns (string memory result)
    {
    ...
    function createImageURI(uint256 tokenId, uint256 slot) public view returns (string memory) {
        return Base64.encode(bytes(_characterRender(tokenId, slot)));
        // return Base64.encode(bytes(_characterRender(tokenId, slot)), true);
        // return Base64.uriEncode(bytes(_characterRender(tokenId, slot)));
    }

    // function createImageURItoUrIencode(uint256 tokenId, uint256 slot) public view returns (string memory) {
    //     return Base64.uriEncode(bytes(_characterRender(tokenId, slot)));
    // }
    ...
    // function createDataURIAssembly(uint256 tokenId, uint256 slot, uint256 markCheck)
    //     public
    //     view
    //     returns (string memory result)
    // {
    ...
    //         // imageURI
    //         let len := mload(imageURI)

    //         for { let cc } 1 {} {
    //             cc := add(cc, 0x20)
    //             mstore(mc, mload(add(imageURI, cc)))
    //             mc := add(mc, 0x20)

    //             if iszero(lt(cc, len)) { break }
    //         }

    //         mc := sub(mc, sub(0x20, mod(len, 32)))
