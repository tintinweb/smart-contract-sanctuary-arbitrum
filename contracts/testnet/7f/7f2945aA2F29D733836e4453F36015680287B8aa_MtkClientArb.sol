...
contract MtkClientArb is CrossChainEnabledArbitrumL2 {
    ArbSys constant arbsys = ArbSys(address(100));

    address public mtk;

    event BalanceRetrieved(address indexed caller, address indexed account, uint256 indexed balance);
    event RewardRetrieved(address indexed caller, address indexed account, uint256 indexed reward);
    event TokensListed(address indexed caller, address indexed account, uint256 indexed balance);

    constructor(address mtkAddress) {
        mtk = mtkAddress;
    }

    function balanceOfMtk(address account) public view returns (uint256) {
        uint256 balance = IERC20(mtk).balanceOf(account);
    ...
    function getBlockNumberL2() external view returns (uint256) {
