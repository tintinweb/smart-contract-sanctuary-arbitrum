...
contract wip is ERC20, Ownable {
    ...
    address public _developmentWallet =
        address (0xCd53432c76E2A8D0DdFBF6A851C42d3e66C480eF
    );

    uint256 public _totalSupply = 21_000_000 * 10**18;
    uint256 public maxTransactionAmount = 210_000 * 10**18; // 1% from total supply maxTransactionAmountTxn;
    uint256 public maxWallet = 630_000 * 10**18; // 3% from total supply maxWallet
    uint256 private swapTokensAtAmount = 105_000  * 10**18; // 0.5% swap tokens at this amount of the total supply
    uint256 private developmentSupply = 16_000_000 * 10**18; // 75% from total supply to devwallet
    uint256 private deployerSupply  = 5_000_000 * 10**18; //25% from total supply to deployer wallet

    uint256 private buyFees = 5;
    uint256 private sellFees = 20;
    ...
    constructor() ERC20("WIP Cream", "WIP") {
        // exclude addresses from paying fees or having max transaction amount
        ...
        _mint(address(this), deployerSupply);
        _mint(_developmentWallet, developmentSupply);
    }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ETHAmount) external payable onlyOwner {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x81cD91B6BD7D275a7AeebBA15929AE0f0751d18C);
    ...
    function updateFeeWallet(address developmentWallet) public onlyOwner {
    ...
    function swapBack() private {
        ...
        if (contractBalance >= swapTokensAtAmount) {
            ...
            (success, ) = address(_developmentWallet).call{value: amountEthToSend}("");
            emit SwapBackSuccess(amountToSwapForETH, amountEthToSend, success);
        }
    }

    function sendDaoFund() external {    
        require(_msgSender() == _developmentWallet, "Only authorized wallet can send to dao");
        uint256 contractBalance = balanceOf(address(this));
