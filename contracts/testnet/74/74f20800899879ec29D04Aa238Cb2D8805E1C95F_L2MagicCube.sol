...
contract L2MagicCube is IL2MagicCube, ERC721EnumerableUpgradeable, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    function initialize() initializer public {
        ...
        __ERC721_init("L2MagicCube", "L2MagicCube");
    ...
    function batchBurn(uint256[] calldata tokenIdArr) external {
        ...
        for (uint256 i_ = 0; i_ < length;) {
            ...
            require(_isApprovedOrOwner(_msgSender(), tokenId), "L2MagicCube: caller is not token owner or approved");
            _burn(tokenId);
            unchecked {i_++;}
        }
    }
    ...
    function mintForLevel(address to_, uint8 level_, uint256 levelStartTokenId_) external returns (uint256) {
        require(hasRole(_ROLE_ADMIN, _msgSender()) || hasRole(_ROLE_MINTER, _msgSender()), "L2MagicCube: caller access denied");
        require(levelStartTokenId_ != 0, "L2MagicCube: level start tokenId is zero");
