...
contract Market is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    event AskNew(address indexed collection, address indexed seller, uint256 indexed tokenId, uint256 askPrice, address tokenBuy);

    // Ask order is updated
    event AskUpdate(address indexed collection, address indexed seller, uint256 indexed tokenId, uint256 askPrice, address tokenBuy);
    ...
    function createAskOrder(
        ...
        // Emit event
        emit AskNew(_collection, msg.sender, _tokenId, _askPrice, _tokenBuy);
    }
    ...
    function _buyToken(
        ...
        if (creatorFee != 0) {
            pendingRevenue[_collections[_collection].creatorAddress][askOrder.tokenBuy] += creatorFee;
        }
        ...
        // Emit event
        emit Trade(_collection, _tokenId, askOrder.seller, msg.sender, _price, netPrice, askOrder.tokenBuy);
    }
    ...
    function batchTransfer(TransferDetails[] memory transferDetail, address reciver) external
    {
        for(uint256 i = 0; i < transferDetail.length; i++) {
            ...
            IERC721(collection).safeTransferFrom(address(msg.sender), reciver, tokenId);
            }
        }
        emit BatchTransferEvent(transferDetail, reciver);
    }
    ...
    function buyTokens(BuyTokensDetails[] memory buyTokensDetail) external {
         for(uint256 i = 0; i < buyTokensDetail.length ; i++) {
            ...
            buyToken(collection, tokenId, price);  
        }
...
contract MintingStation is AccessControl {
     ...
     */
    function mintCollectible(address _tokenReceiver, uint256 _level, uint256 _type) external onlyMinter returns (uint256) {
        uint256 tokenId = nft.safeMint(_tokenReceiver, _level, _type);
        return tokenId;
    }
    ...
    function recoverNonFungibleToken(address _token, uint256 _tokenId) external onlyOwner {
        nft.recoverNonFungibleToken(_token, _tokenId);
    }

    function recoverToken(address _token) external onlyOwner {
        nft.recoverToken(_token);
    }
...
contract NFT is ERC721Holder, ERC721Enumerable, Pausable, Ownable, ERC721Burnable
{
    ...
    function safeMint(address to, uint256 _level, uint256 _type) public onlyOwner returns (uint256) {
...
contract Token is ERC20, ERC20Burnable, Ownable {
    constructor() ERC20("BOXTOKEN", "BOXTOKEN") {}
...
contract TokenERC1155 is ERC1155, Ownable, Pausable, ERC1155Burnable {
