...
contract BattleVersusV2 is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  function fight(
    ...
    if (address(BATTLE_BOOST) != address(0)) {
      playerBaseLpBonus = BATTLE_BOOST.getAnimaBase(msg.sender);
    }
  ...
  function lpAnimaBase(address addr) external view returns (uint256) {
    ...
    return BATTLE_BOOST.getAnimaBase(addr);
  }
  ...
  function _traitBattle(
    ...
    if (traitBattleData.roll < rewards.traitUpgradeThreshold) {
      ...
      traitBattleData.traitUpgrade = (uint16)(rewards.traitUpgradePoints / ONE_HUNDRED);
