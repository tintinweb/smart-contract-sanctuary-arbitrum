    ...
    ISmoloveActionsVault
{
    ...
    function _stake(
        ...
        s.amount = amount;
        if(originalUnlock == 0) {
            (,uint256 timeLock) = atlasMine.getLockBoost(lock);
            uint256 vestingTime = atlasMine.getVestingTime(lock);
            s.unlockAt = block.timestamp + timeLock + vestingTime + 1 days;
        } else {
            s.unlockAt = originalUnlock;
        }
        s.internalUnlockAt = vaultStake.unlockAt;
        s.lock = lock;
    ...
    function internalClaimWithdrawAllAndRestake() external nonReentrant {
        uint256 claimAmount;
        (,uint256 timeLock) = atlasMine.getLockBoost(battleflyLock);
        uint256 vestingTime = atlasMine.getVestingTime(battleflyLock);
        uint256[] memory stakeIds = activeStakes.values();
        for (uint256 i = 0; i < stakeIds.length; i++) {
            if (userStakes[stakeIds[i]].internalUnlockAt < block.timestamp &&
                userStakes[stakeIds[i]].unlockAt > (block.timestamp + timeLock + vestingTime + 1 days)) {
                address owner = userStakes[stakeIds[i]].owner;
                uint256 unlockTime = userStakes[stakeIds[i]].unlockAt;
                IAtlasMine.Lock lock = userStakes[stakeIds[i]].lock;
                (uint256 received, uint256 claimed) = _withdraw(stakeIds[i], true);
                claimAmount += claimed;
                _stake(owner,received,lock,unlockTime);
            } else {
                if (_getStakeClaimableAmount(stakeIds[i]) > 0) {
                    claimAmount += _claim(stakeIds[i]);
                }
            }
        }
        if (claimAmount > 0) {
    ...
    function withdrawAll() external nonReentrant {
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            ...
            if (userStakes[stakeId].unlockAt < block.timestamp) {
                (uint256 received, uint256 claimed) = _withdraw(stakeId, false);
    ...
    function withdraw(uint256[] memory stakeIds) external nonReentrant {
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            ...
            (uint256 received, uint256 claimed) = _withdraw(stakeIds[i], false);
            receiveAmount += received;
            claimAmount += claimed;
        }
        require(receiveAmount > 0 || claimAmount > 0, "SMOLOVE_ACTIONS_VAULT:NO_STAKES_TO_WITHDRAW");
    ...
    function _withdraw(uint256 stakeId, bool internalWithdraw) internal returns (uint256, uint256) {
        UserStake storage s = userStakes[stakeId];
        uint256 rewardAmount;
        if(internalWithdraw) {
            require(s.internalUnlockAt < block.timestamp, "SMOLOVE_ACTIONS_VAULT:CANNOT_WITHRAW_BEFORE_LOCKTIME");
        } else {
    ...
    function setActiveBattleflyLock(IAtlasMine.Lock _battleflyLock) external onlyAdmin {
        require(_battleflyLock < allowedLocks[0], "SMOLOVE_ACTIONS_VAULT:BATTLEFLY_LOCK_MUST_NOT_BE_BIGGER_THAN_SMALLEST_ALLOWED_LOCK");
