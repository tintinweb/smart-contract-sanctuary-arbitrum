...
contract DistributionContract {
    ...
    struct Holder {
        ...
        uint256 lastClaimTime; // Ajout de lastClaimTime dans la structure
    }
    ...
    function distributeRewardsToHolders(uint256 ethAmount) internal {
        ...
        // Calcul du total de l'ARBOT à distribuer
        totalDistributionArbot += ethAmount;

        for (uint256 i = 0; i < token.holdersCount(); i++) {
            address holder = token.holderAtIndex(i);
            if (isEligible(holder)) {
                uint256 holderBalance = holders[holder].balance;
                uint256 holderShareArbot = (totalDistributionArbot * holderBalance) / totalSupply;
                uint256 holderShareEth = (totalDistributionEth * holderBalance) / totalSupply;
                holders[holder].accumulatedArbot += holderShareArbot;
                holders[holder].accumulatedEth += holderShareEth;
            }
        }
    }
    ...
    function isEligible(address holder) public view returns (bool) {
        return epoch.getCurrentEpoch() >= holders[holder].lastClaimEpoch + ELIGIBILITY_EPOCHS && block.timestamp >= holders[holder].lastClaimTime + 1 days;
    }
    ...
    function updateEpoch() external {
        ...
        if (newEpoch > epoch.currentEpoch()) {
            ...
            distributeRewardsToHolders(totalDistributionEth);
        }
    }
}
