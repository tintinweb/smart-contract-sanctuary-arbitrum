...
contract ArbMessageSend is Initializable, OwnableUpgradeable, IL2MessageSend {
    ...
    function initialize(address _arbSys,address _zkBridge) public initializer {
        __Ownable_init();
        zkBridge = _zkBridge;
    ...
    function sendMessage(
        ...
        arbSys.sendTxToL1(messageReceive, message);
    ...
    function sendMessage(
        ...
        bytes memory message = abi.encodeWithSignature("receiveMessage(string)", message);
        arbSys.sendTxToL1(destination, message);
    }
    ...
    function setArbSys(address _arbSys) external onlyOwner {
        require(_arbSys != address(0), "Zero address");
        arbSys = ArbSys(_arbSys);
    }
