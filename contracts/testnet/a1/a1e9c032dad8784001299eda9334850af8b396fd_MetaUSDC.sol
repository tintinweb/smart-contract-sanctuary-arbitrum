...
contract MetaUSDC is Ownable, ReentrancyGuard {
   ...
   using SafeERC20 for IERC20;
    ...
    event StartRound(uint256 indexed idGame, uint256 indexed idSi);
    event RewardsCalculated(uint256 indexed idGame, uint256 oddsAmount, uint256 treasuryAmount);
    event TokenRecovery(address indexed token, uint256 amount);
    event TreasuryClaim(uint256 amount);
    event SetBettableStatus(uint256 indexed idGame, bool status);
    ...
    function betBlue(uint256 roundID, uint256 amount) external nonReentrant  {
        ...
        emit BetBlue(msg.sender, roundID, updateAmount, amount);
    }
    ...
    function betRed(uint256 roundID, uint256 amount) external nonReentrant {
        ...
        emit BetRed(msg.sender, roundID, updateAmount, amount);
    }
     ...
     * @param _gameResult: result
     * @param _idSI: next round #
    ...
    function executeRound(Position _gameResult, uint256 _idSI) external onlyAgent {
        ...
        if(round.totalAmountBet == 0 || 
            ...
            _safeStartRound(currentRound, _idSI);
        }
        else{
            round.gameResult = _gameResult;
            round.idSI = _idSI;
     ...
     * @dev Callable by Agent
     */
    function _setBettableStatus(bool _status) external onlyAgent{
       rounds[currentRound].gameStatus = _status;

       emit SetBettableStatus(currentRound, _status);
    }
    
    /**
     * @notice Set new platform fee and baseRate
     * @dev Callable by SetBettableStatudcontract owner
     */
     ...
     * @dev Callable by contract owner
     */
    function recoverETH(address payable _newadd,uint256 amount) external onlyOwner {
     ...
     * @param roundID: roundID
     */
    function _safeStartRound(uint256 roundID, uint256 idSi) internal {
        require(genesisStartOnce, "Can only run after genesisStartRound is triggered");
        _startRound(roundID, idSi);
    }
     ...
     * @param roundID: roundID
     */
    function _startRound(uint256 roundID, uint256 idSi) internal {
        Round storage round = rounds[roundID];
        round.idGame = roundID;
        round.gameStatus = true;
        round.totalAmountBet = 0;

        emit StartRound(roundID, idSi);
    }
