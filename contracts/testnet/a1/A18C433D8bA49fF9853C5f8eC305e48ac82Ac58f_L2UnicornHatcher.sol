 ...
 * @notice 孵化
 */
contract L2UnicornHatcher is L2UnicornHatcherPermit, BaseL2, BaseL2Unicorn, BaseL2UnicornRule, BaseRecipient, BaseGame {
     ...
     * @notice 孵化 EO
     */
    function hatchE0(uint256 gasPrice_, uint256 callbackGasLimit_, uint256 deadline, uint8 v, bytes32 r, bytes32 s) payable external {
        uint8 eSeries = 0;
     ...
     * @notice 孵化 E1 E2
     */
    function hatchE1E2(uint256 gasPrice_, uint256 callbackGasLimit_, uint8 hatchCount_, uint8 eSeries_) payable external {
    ...
    function randomizerCallback(uint256 requestId_, bytes32 seed_) external {
        ...
        for (uint8 index_ = 0; index_ < hatchCount_;) {
            ...
            emit HatchCallback(user_, requestId_, seed_, eSeries_, index_, randomNumber_, tokenId);
            unchecked{++index_;}
        }
    }
...
abstract contract L2UnicornHatcherPermit is EIP712, Ownable {
    ...
    function permit(address user, uint8 eSeries, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public {
        require(block.timestamp <= deadline, "L2UnicornHatcherPermit: expired deadline");
        bytes32 structHash = keccak256(abi.encode(_PERMIT_TYPEHASH, user, eSeries, _nonces[authorizer]++, deadline));
        bytes32 hash = _hashTypedDataV4(structHash);
        address signer = ECDSA.recover(hash, v, r, s);
        require(signer == authorizer, "L2UnicornHatcherPermit: invalid signature");
    }

    function nonces() public view returns (uint256) {
        return _nonces[authorizer];
    }
