...
contract User is IUser, DAOAccessControlled {
     ...
     * @return  _           User ID
     */
    function register(string memory _name, string memory _avatarURI, string memory _dataURI) external onlyForwarder returns (uint256) {
        return _createUser(_name, _avatarURI, _dataURI, _msgSender());
    }
    ...
    // Update DataURI from mobile app.
...
interface IUser {
    ...
    function register(string memory _name, string memory _avatarURI, string memory _dataURI) external returns (uint256);
