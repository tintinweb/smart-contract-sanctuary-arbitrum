...
// FUNCTIONS
        ...
        onlyWhenPackEnabled(packID)
    {
        ...
        for (uint256 i = 0; i < amount;) {
            ...
            // Write an event
            emit PackClaimed(_msgSender(), packID, index, itemToMint, totalToMint);
            // Mint reward items
        ...
        onlyWhenPackEnabled(packID)
    {
        ...
        for (uint256 i = 0; i < amount;) {
            ...
            (uint256 itemToMint, uint256 totalToMint) = getRewardItem(packID, i);
            // Write an event
            emit PackMinted(_msgSender(), packID, itemToMint, totalToMint);
            // Mint reward items
            itemsContract.mintItem(itemToMint, totalToMint, _msgSender());
            unchecked { i++; }
        }
        ...
        if (pack.paymentAddress != address(0)) {
            ...
            uint256 priceToPay = pack.price * amount;
...
// ADMIN FUNCTIONS
        ...
        onlyOwner 
    {
        ...
        for (uint256 packID=0; packID < total; packID++) {
            // Setup the payment contract
            IERC20 paymentContract = IERC20(packs[packID].paymentAddress);
