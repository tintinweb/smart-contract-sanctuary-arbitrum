...
contract TimeContent is Ownable,ReentrancyGuard{
    ...
    function createThing(string calldata _contentName,string memory _willThing,uint256 _tradeMoney,uint256 _endTime,address _pointAddress,address token)public{   
        ...
        //违约事件
        _punishment.push(punishment(_contentId,0,"",""));
        _contentId++;
    }
    ...
    //审核者对事务投票
    function voteContent(string calldata contentName,bool judge)external{
        //根据输入的name查找相应id
    ...
    //审查者和部署者对违约事件投票
    function votePunish(string calldata contentName,bool _punishJudge)external {
    ...
    //接收者是否确定进行该事务
    function judgeReceiverContent(bool _thisContent,string calldata contentName)external{
