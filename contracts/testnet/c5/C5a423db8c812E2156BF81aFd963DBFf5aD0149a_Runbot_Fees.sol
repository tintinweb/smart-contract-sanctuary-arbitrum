...
contract Runbot_Fees is A_Base, A_Math, Ownable, A_Dust, ReentrancyGuard, I_Runbot_Fees {
	...
	constructor(){
		...
		_borrowFees[2] = BorrowFees(0.004 ether, 0.02 ether, 30 days, 365 days);
	...
	function dispatchRoyaltyTransferFees(uint256 nftType, uint256 nftSubType) external payable nonReentrant{
		payable(owner()).transfer(msg.value);
	}
	
	function dispatchRoyaltyMintFees(uint256 nftType, uint256 nftSubType) external payable nonReentrant{
		payable(owner()).transfer(msg.value);
	}
	
	function dispatchRoyaltySellFees(uint256 nftType, uint256 nftSubType) external payable nonReentrant{
