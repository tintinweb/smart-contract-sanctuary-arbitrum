...
contract OptionPositionNFT is ERC721,DefaultOperatorFilterer,OwnableAdmins
  {
    ...
    function setSize(uint tokenId, uint size) public onlyAdmins {
        positionDetails[tokenId].size=size;
    }
    ...
    function mint(address to,string memory symbol,string memory exchange,uint purchasePrice,
        uint openSpotPrice,uint tradeTime, uint multiplier, uint size) public onlyAdmins {
        mintCounter += 1;
        PositionDetails memory po=PositionDetails({
           symbol:symbol,
            exchange:exchange,
            ...
            size:size});
        positionDetails[mintCounter]=po;
    ...
    function transferFrom(
        ...
        require(isAdmin(msg.sender) || msg.sender == from || isApprovedForAll[from][msg.sender] ||
             msg.sender == getApproved[id],"NOT AUTHORIZED");
    ...
    function getOwnerPositions(address target) external view returns (PositionDetails[] memory) {
        uint balance = balanceOf(target);
        PositionDetails[] memory result = new PositionDetails[](balance);
        uint j=0;
