...
contract SSTExchange is Ownable {
    IERC20 public SST;
    IERC20 public USDT;

    uint public rRate;
    uint public minUSDTEmission = 10000000;
    uint public minSST = 1000000000;
    bool public statusAdditionEmission;

    //In bps, example 300 = 1%
    uint public fee = 100;
    //10000 = 1, 3800 = 0.38
    ...
    function swap( uint _amountSTT) public {
        require(_amountSTT >= minSST);

        //Distribution fee
        uint _fee = calculate(_amountSTT, fee);
        SST.burnFrom(msg.sender, _amountSTT - _fee);
        SST.feeDistribution(_fee, msg.sender);
    ...
    function emmisionSST(uint _usdt) private {
        uint _amountEm = calculate(_usdt / rRate, ratio) ;
        SST.emission(_amountEm);
    }

    function countingRate() private {
       rRate = (USDT.balanceOf(address(this)) * 10 ** 9)  / SST.totalSupply();
    }
    ...
    //ADMIN FUNCTIONS
    function setContracts(IERC20 _sst, IERC20 _usdt) public onlyOwner {
        SST = _sst;
        USDT = _usdt;
    }
    ...
    function setMin(uint _minUSDTEmission, uint _minSST) public onlyOwner {
