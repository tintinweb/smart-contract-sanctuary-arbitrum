...
interface IARB is IERC20Upgradeable {
...
contract Crafting is Initializable, CraftingSettings {
    ...
    using SafeERC20Upgradeable for IARB;
    ...
    function _endCraftingPostValidation(uint256 _craftingId, uint256 _randomNumber, address _owner) private {
        ...
        (uint256 _magicRewarded, uint256 _arbRewarded, uint256 _toadIdRewarded) = _rewardAdditionalsRewards(_craftingRecipe.dropAdditionalRewards, _owner, _randomNumber);

        emit CraftingEnded(_craftingId, _bugzRewarded, _itemOutcomes, _magicRewarded, _arbRewarded, _toadIdRewarded);
    }

    function _rewardAdditionalsRewards(
        bool _dropAdditionalRewards,
        address _owner,
        uint256 _randomNumber)
    private
    returns(uint256 _magicRewarded, uint256 _arbRewarded, uint256 _toadIdRewarded)
    {
        if(!_dropAdditionalRewards) {
            return (0,0,0);
        }

        _randomNumber = uint256(keccak256(abi.encodePacked(_randomNumber, "ADDITIONAL")));

        uint256 _multiSigArbBalance = arb.balanceOf(multiSigAddress);
        uint256 _multiSigMagicBalance = magic.balanceOf(multiSigAddress);

        uint256 _result = _randomNumber % 100000;
        if(_result < TOADZ_ODDS) { // Toad
            // Reached the end of toadz owned by the multisig.
            if(startingToadId == 0 || toadz.ownerOf(startingToadId) != multiSigAddress) {
                return (0,0,0);
            }
            _toadIdRewarded = startingToadId;
            startingToadId--;
            toadz.adminSafeTransferFrom(multiSigAddress, _owner, _toadIdRewarded);

        } else if(_result < (TOADZ_ODDS + ARB_ODDS) && _multiSigArbBalance >= ARB_MAGIC_REWARD_AMOUNT) {

            arb.safeTransferFrom(multiSigAddress, _owner, ARB_MAGIC_REWARD_AMOUNT);
            _arbRewarded = ARB_MAGIC_REWARD_AMOUNT;
        } else if(_result < (TOADZ_ODDS + ARB_ODDS + MAGIC_ODDS) && _multiSigMagicBalance >= ARB_MAGIC_REWARD_AMOUNT) {
            magic.safeTransferFrom(multiSigAddress, _owner, ARB_MAGIC_REWARD_AMOUNT);
            _magicRewarded = ARB_MAGIC_REWARD_AMOUNT;
        }
    }
...
abstract contract CraftingSettings is Initializable, CraftingContracts {
    ...
    onlyAdminOrOwner
    {
        ...
        for(uint256 i = 0; i < _craftingRecipe.inputs.length; i++) {
            ...
            require(_input.inputOptions.length > 0);
        ...
        // Output validation.
        require(_craftingRecipe.outputs.length > 0);
    ...
    onlyAdminOrOwner
    {
        require(_isValidRecipeId(_recipeId));
    ...
    function setStartingToadId(uint256 _startingToadId) external onlyAdminOrOwner {
        startingToadId = _startingToadId;
    }
...
abstract contract CraftingState is ICrafting, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, AdminableUpgradeable {
    ...
    event CraftingEnded(
        ...
        uint256 _magicRewarded,
        uint256 _arbRewarded,
        uint256 _toadIdRewarded
    );

    uint256 constant ARB_MAGIC_REWARD_AMOUNT = 30 ether;
    uint256 constant TOADZ_ODDS = 2500;
    uint256 constant ARB_ODDS = 2500;
    uint256 constant MAGIC_ODDS = 7500;
    ...
    mapping(string => uint64) recipeNameToRecipeId;
    ...
    // The address for the multisig.
    address public multiSigAddress;
    IMagic magic;
    IARB arb;
    // The starting toad id to give out Toadz. Works backwards
    uint256 public startingToadId;
...
struct CraftingRecipe {
    ...
    // Recipe will drop additional magic, arb, and toad rewards
    //
    bool dropAdditionalRewards;
}
