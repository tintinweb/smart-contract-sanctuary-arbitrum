...
contract TournamentLeaderboard is Ownable, AccessControl {
    ...
    mapping(bytes32 => ITournamentLeaderboard.Rank) public tournamentUserRank;
    ...
    function getTournamentLeaderboard(
        ...
        while (cursor != bytes32(0) && index < offset) {
            _leaderboard[index] = tournamentUserRank[cursor];
            cursor = tournamentUserRank[cursor].next;
            index++;
        }
        return _leaderboard;
    }
    ...
    function updateUserRank(
        ...
        if (previousIndex == 0) {
            ...
            ITournamentLeaderboard.Rank
                storage previousRank = tournamentUserRank[previousIndex];
            newRank = ITournamentLeaderboard.Rank(
                previousRank.next,
                previousIndex,
                user,
    ...
    function claimReward(uint256 tournamentId) external {
        ...
        require(!tournamentUserRank[rankIndex].hasClaimed, "Already claimed");
        if (reward > 0) {
            ...
            tournamentUserRank[rankIndex].hasClaimed = true;
