...
contract MissionOne is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  // Uints
  //=======================================
  uint256 public animaBaseRate;

  //=======================================
  // Mappings
  //=======================================
  mapping(uint256 => uint256) public particleCosts;

  //=======================================
  // Events
  //=======================================
  event StartedMissionOne(
    address addr,
    uint256 adventurerId,
    uint256 particleCost
  );
  event ClaimedMissionOne(address addr, uint256 adventurerId, uint256 anima);
  ...
  constructor(
    ...
    particleCosts[1] = 0.07 ether;
    particleCosts[2] = 0.14 ether;

    animaBaseRate = 0.1 ether;
  }
  ...
  function start(
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      uint256 time = _time[j];

      // Check time is below 3
      require(time < 3, "MissionOne: Time must be below 3");
      ...
      STORAGE.set(addr, adventurerId, _getTime(time), animaBaseRate);

      // Transfer particle if above 0
      if (time > 0) {
        PARTICLE.safeTransferFrom(msg.sender, VAULT, particleCosts[time]);
      }

      emit StartedMissionOne(addr, adventurerId, particleCosts[time]);
    }
  }

  function claim(address[] calldata _addrs, uint256[] calldata _adventurerIds)
    external
    nonReentrant
    whenNotPaused
  {
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      address addr = _addrs[j];
      uint256 adventurerId = _adventurerIds[j];
      address owner = ERC721A(addr).ownerOf(adventurerId);

      // Check sender owns adventurer
      require(owner == msg.sender, "MissionOne: You do not own Adventurer");

      uint256 anima = _getAnima(addr, adventurerId);

      // Mint Anima
      ANIMA.mintFor(owner, anima);

      // Mark rewards as claimed
      STORAGE.claim(addr, adventurerId);

      emit ClaimedMissionOne(addr, adventurerId, anima);
    }
  }
  ...
  function updateParticleCosts(uint256[] calldata _values) external onlyAdmin {
    particleCosts[1] = _values[0];
    particleCosts[2] = _values[1];
  }

  function updateAnimaBaseRates(uint256 _value) external onlyAdmin {
    animaBaseRate = _value;
  }

  //=======================================
  // Internal
  //=======================================
  function _getTime(uint256 _time) internal pure returns (uint256) {
    if (_time == 0) {
      return 7 days;
    } else if (_time == 1) {
      return 15 days;
    ...
    returns (uint256)
  {
    ...
    uint256 anima = ADVENTURER_DATA.aov(_addr, _adventurerId, 0) *
      STORAGE.rates(_addr, _adventurerId);
