...
contract MessageRouter is NonblockingLzApp {
    ...
    function send(uint16 chainId, address destination, bytes calldata payload, bytes calldata params) public payable {
        bytes memory _destination = abi.encodePacked(destination, address(this));
        lzEndpoint.send{value:msg.value}(chainId, _destination, payload, payable(msg.sender), address(this), params);
    }
