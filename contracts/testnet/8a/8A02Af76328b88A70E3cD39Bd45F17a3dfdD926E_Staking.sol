...
contract Staking is Ownable, Pausable {
    ...
    function getStakersInfoForLottery(Tier tier) public view returns (UserLotteryInfo[] memory userInfos) {
        address[] memory users = getStakersByTier(tier);
        userInfos = new UserLotteryInfo[](users.length);

        for (uint i = 0; i < users.length; i++) {
            (uint256 userTickets, , , , ) = getUserInfo(users[i]);
            userInfos[i] = UserLotteryInfo(users[i], userTickets);
        }
    }
