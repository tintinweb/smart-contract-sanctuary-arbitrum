...
contract Minter is IMinter {
    ...
    uint public weekly = 15_000_000 * 1e18; // represents a starting weekly emission of 15M BERO (BERO has 18 decimals)
    ...
    function calculate_growth(uint _minted) public view returns (uint) {
        ...
        uint _BEROTotal = _BERO.totalSupply();
        return
    ...
    function update_period() external returns (uint) {
        ...
        if (block.timestamp >= _period + WEEK && initializer == address(0)) { // only trigger if new week
            ...
            uint _balanceOf = _BERO.balanceOf(address(this));
...
import {IBERO} from "contracts/interfaces/IBERO.sol";
...
contract MerkleClaim {
    ...
    /// ============ Immutable storage ============
    uint internal constant LOCK = 86400 * 7 * 52 * 4;
    /// @notice BERO token to claim
    IBERO public immutable BERO;
    IVotingEscrow public immutable hiBERO;
    /// @notice ERC20-claimee inclusion root
    bytes32 public immutable merkleRoot;
    bytes32 public immutable merkleRootHiBero;

    /// ============ Mutable storage ============

    /// @notice Mapping of addresses who have claimed tokens
    mapping(address => bool) public hasClaimed;
    mapping(address => bool) public hasClaimedHiBero;
    ...
    constructor(address _BERO, address _hiBERO, bytes32 _merkleRoot, bytes32 _merkleRootHiBero) {
        BERO = IBERO(_BERO);
        hiBERO = IVotingEscrow(_hiBERO);
        merkleRoot = _merkleRoot;
        merkleRootHiBero = _merkleRootHiBero;
    }
    ...
    /// @param to recipient of claim
    /// @param amount of tokens claimed
    event ClaimHiBero(address indexed to, uint256 amount);
    /// ============ Functions ============
    ...
    function claim(
        ...
        require(BERO.claim(to, amount), "CLAIM_FAILED");
    ...
    /// @param amount of tokens owed to claimee
    /// @param proof merkle proof to prove address and amount are in tree
    function claimHiBERO(
        address to,
        uint256 amount,
        bytes32[] calldata proof
    ) external {
        // Throw if address has already claimed tokens
        require(!hasClaimedHiBero[to], "ALREADY_CLAIMED");

        // Verify merkle proof, or revert if not in tree
        bytes32 leaf = keccak256(abi.encodePacked(to, amount));
        bool isValidLeaf = MerkleProof.verify(proof, merkleRootHiBero, leaf);
        require(isValidLeaf, "NOT_IN_MERKLE");

        // Set address to claimed
        hasClaimedHiBero[to] = true;

        // Claim tokens for address
        require(BERO.claim(address(this), amount), "CLAIM_FAILED");
        BERO.approve(address(hiBERO), amount);
        hiBERO.create_lock_for(amount, LOCK, to);

        // Emit claim event
        emit ClaimHiBero(to, amount);
    }
}
...
contract VeArtProxy is IVeArtProxy {
    ...
    function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) external pure returns (string memory output) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "lock #', toString(_tokenId), '", "description": "Beradrome locks, can be used to boost gauge yields, vote on token emission, and receive bribes", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
...
contract Voter is IVoter {
    ...
    function createGauge(address _pool) external returns (address) {
        ...
        if (msg.sender != governor) { // gov can create for any pool, even non-Beradrome pairs
...
contract VotingEscrow is IERC721, IERC721Metadata, IVotes {
    ...
    /// @param token_addr `BERO` token address
