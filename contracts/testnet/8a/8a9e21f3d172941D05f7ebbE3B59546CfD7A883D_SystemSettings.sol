...
contract SystemSettings is Owned, MixinSystemSettings, ISystemSettings {
    ...
    // penalty taken away from target of force liquidation (with 18 decimals). E.g. 30% is 0.3e18
    function forceLiquidationPenalty() external view returns (uint256) {
        return getForceLiquidationPenalty();
    }
    ...
    function setForceLiquidationPenalty(uint256 penalty) external onlyOwner {
        flexibleStorage().setForceLiquidationPenalty(SETTING_FORCE_LIQUIDATION_PENALTY, penalty);
        emit ForceLiquidationPenaltyUpdated(penalty);
    }
    ...
    event ForceLiquidationPenaltyUpdated(uint256 newPenalty);
...
library SystemSettingsLib {
    ...
    function setLiquidationRatio(
        ...
        uint256 getForceLiquidationPenalty,
        uint256 getIssuanceRatio
    ) external {
        require(
            _liquidationRatio <= MAX_LIQUIDATION_RATIO.divideDecimal(SafeDecimalMath.unit().add(getForceLiquidationPenalty)),
    ...
    function setForceLiquidationPenalty(
        IFlexibleStorage flexibleStorage,
        bytes32 settingName,
        uint256 penalty
    ) external {
        // MAX_LIQUIDATION_PENALTY is enforced on both Collateral and force liquidations
