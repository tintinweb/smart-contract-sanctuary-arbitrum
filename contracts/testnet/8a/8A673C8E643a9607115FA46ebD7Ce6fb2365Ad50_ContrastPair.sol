...
import "./interfaces/IContrastPair.sol";
import "./ContrastPairBase.sol";

contract ContrastPair is ContrastPairBase {
    /// -----------------------------------------------------------------------
    /// Dependencies
    /// -----------------------------------------------------------------------
    ...
    function quotes(
        ...
        return _quotes[marketId];
    }

    /// @dev This function is constrained by gas limits and may revert in the future.
    function totalMarketsOpen()
        external
        view
        override
        returns (uint256 openMarkets)
    {
        uint256 totalMarkets = totalMarkets();

        unchecked {
            for (uint256 i; i < totalMarkets; ++i) {
                if (block.timestamp < uint256(_markets[i].maturity))
                    ++openMarkets;
            }
        }
    }
    ...
    /// @dev This function is constrained by gas limits and may revert in the future.
    function totalMarketsOpenForAccount(
        address account
    ) external view override returns (uint256 openMarkets) {
        uint256 totalMarkets = totalMarkets();

        unchecked {
            for (uint256 i; i < totalMarkets; ++i) {
                (uint256 idBelow, uint256 idAbove) = idsForMarket(i);

                if (
                    block.timestamp < uint256(_markets[i].maturity) &&
                    (balanceOf(account, idBelow) > 0 ||
                        balanceOf(account, idAbove) > 0)
                ) {
                    ++openMarkets;
                }
            }
    ...
    function canEmergencyExit(uint256 marketId) external view returns (bool) {
    ...
    /// @notice Computes the market's implied probability for its true outcome, expressed using 18 decimal places.
    function computeImpliedProbability(
        uint256 marketId
    ) external view override returns (uint256) {
        // Sum of outcome token supplys cannot exceed `ASSET` totalSupply.
        unchecked {
            (uint256 idBelow, uint256 idAbove) = idsForMarket(marketId);

            uint256 supplyBelow = totalSupply[idBelow];
            uint256 supplyAbove = totalSupply[idAbove];
            uint256 sum = supplyBelow + supplyAbove;

            return supplyBelow.divWad(sum);
        }
    }

    /// @notice Computes the potential ROI for a markets outcome, expressed using 18 decimal places.
    function computeOutcomeROI(
        uint256 marketId,
        bool outcome
    ) external view override returns (uint256 roi) {
        (uint256 id0, uint256 id1) = idsForMarket(marketId);

        uint256 supplyBelow = totalSupply[id0];
        uint256 supplyAbove = totalSupply[id1];

        roi = outcome
            ? supplyAbove.divWad(supplyBelow).mulDiv(95, 100)
            : supplyBelow.divWad(supplyAbove).mulDiv(95, 100);
    }

    /// -----------------------------------------------------------------------
    /// Read-only ERC1155 Metadata Logic
    /// -----------------------------------------------------------------------
...
import "./mixins/ERC1155Supply.sol";

import "./interfaces/IContrastPair.sol";
import "./interfaces/IChainlinkAggregatorV3.sol";

abstract contract ContrastPairBase is IContrastPair, Clone, ERC1155Supply {
    /// -----------------------------------------------------------------------
    /// Dependencies
    /// -----------------------------------------------------------------------
    ...
    /// @notice A mapping of underlying assets, users can bet with
    ...
    function ASSET() internal pure returns (address) {
        return _getArgAddress(0x0);
    }

    /// @dev The amount of the bond currency required to create a quote.
    function QUOTE_BOND_AMOUNT() internal pure returns (uint256) {
        return uint256(_getArgUint96(0x14));
    }

    /// @dev The duration of the dispute window for a quote in seconds.
    function QUOTE_DISPUTE_WINDOW() internal pure returns (uint256) {
    ...
    function EMERGENCY_WITHDRAWAL_WINDOW() internal pure returns (uint256) {
    ...
    function idsForMarket(
        uint256 marketId
    ) public pure returns (uint256 idBelow, uint256 idAbove) {
        idBelow = marketId << 1;
        idAbove = idBelow + 1;
    }
    ...
    /// @notice Adds a new underlying asset.
    /// @param asset The address of the underlying asset.
    function addAsset(
        address asset,
        uint256 quoteBondAmount,
        uint256 qouteDisputeWindow,
        uint256 emergencyWithdrawalWindow
    ) external {
        if (assets[asset].isActive) {
            revert ASSET_ALREADY_ACTIVE();
        }

        assets[asset] = Asset({
            quoteBondAmount: quoteBondAmount,
            qouteDisputeWindow: qouteDisputeWindow,
            emergencyWithdrawalWindow: emergencyWithdrawalWindow,
            isActive: true
        });
    }

    /// @notice Removes an existing underlying asset.
    /// @param asset The address of the underlying asset.
    ...
    function createMarket(
        ...
        _markets.push(
            Market({
                asset: asset,
                priceFeed: IChainlinkAggregatorV3(priceFeed),
                exchangeRate: 0,
                maturity: maturity,
    ...
    /// @param amountStrikeBelow The amount of below outcome tokens the user wishes to receive.
    /// @param amountStrikeAbove The amount of above outcome tokens the user wishes to receive.
    ...
    function enter(
        uint256 marketId,
        uint256 amountStrikeBelow,
        uint256 amountStrikeAbove
    ) external virtual {
        // Checks:
        ...
        market.asset.safeTransferFrom(
            msg.sender,
            address(this),
            amountStrikeBelow + amountStrikeAbove
        );

        uint256 fee0 = amountStrikeBelow.mulDiv(TRANSFER_FEE_BIPS, 10_000);
        uint256 fee1 = amountStrikeAbove.mulDiv(TRANSFER_FEE_BIPS, 10_000);

        uint256 adjusted0 = amountStrikeBelow - fee0;
        uint256 adjusted1 = amountStrikeAbove - fee1;

        accumulatedFees[market.asset] += fee0 + fee1;

        (uint256 idBelow, uint256 idAbove) = idsForMarket(marketId);

        if (amountStrikeBelow > 0) _mint(msg.sender, idBelow, adjusted0, "");
        if (amountStrikeAbove > 0) _mint(msg.sender, idAbove, adjusted1, "");
    ...
    /// @param amountStrikeBelow The amount of below outcome tokens the user wishes to burn.
    /// @param amountStrikeAbove The amount of above outcome tokens the user wishes to burn.
    /// @dev The function also collects a transfer fee, which is a percentage of the specified amount
    /// of outcome tokens being exchanged. The fee is defined by the TRANSFER_FEE_BIPS constant.
    function exit(
        uint256 marketId,
        ...
        if (amountStrikeBelow > 0)
            _burn(msg.sender, idBelow, amountStrikeBelow);
        if (amountStrikeAbove > 0)
            _burn(msg.sender, idAbove, amountStrikeAbove);

        uint256 fee = amountStrikeBelow.mulDiv(TRANSFER_FEE_BIPS, 10_000) +
            amountStrikeAbove.mulDiv(TRANSFER_FEE_BIPS, 10_000);

        uint256 adjusted = amountStrikeBelow + amountStrikeAbove - fee;

        accumulatedFees[market.asset] += fee;

        market.asset.safeTransfer(msg.sender, adjusted);
    ...
    function redeem(uint256 marketId, uint256 amountIn) external virtual {
        ...
        if (market.outcome == Outcome.BELOW) {
            _burn(msg.sender, idBelow, amountIn);
        } else {
            _burn(msg.sender, idAbove, amountIn);
        }

        uint256 winnings = amountIn.mulWad(market.exchangeRate);

        uint256 fee = winnings.mulDiv(REDEEM_FEE_BIPS, 10_000);

        uint256 amountOut = winnings - fee + amountIn;

        accumulatedFees[market.asset] += fee;

        market.asset.safeTransfer(msg.sender, amountOut);
    ...
    function emergencyExit(
        ...
        Market storage market = _markets[marketId];
        Asset memory asset = assets[market.asset];

        if (amountStrikeBelow == 0 && amountStrikeAbove == 0) {
        ...
        if (
            block.timestamp < market.maturity + asset.emergencyWithdrawalWindow
        ) {
        ...
        uint256 sum = amountStrikeBelow + amountStrikeAbove;

        market.asset.safeTransfer(msg.sender, sum);
    ...
    function price(uint256 marketId, uint80 roundId) external virtual {
        ...
        Market memory market = _markets[marketId];
        Quote storage quote = _quotes[marketId];
        Asset memory asset = assets[market.asset];
        ...
        (, int256 answer, , uint256 updatedAt, ) = market
            .priceFeed
        ...
        if (quote.quoter == address(0)) {
            ...
            if (block.timestamp > quote.creation + asset.qouteDisputeWindow) {
    ...
    function settle(uint256 marketId) external virtual {
        ...
        Market storage market = _markets[marketId];
        Quote memory quote = _quotes[marketId];
        Asset memory asset = assets[market.asset];
        ...
        if (answer > strike) {
            market.outcome = Outcome.ABOVE;
            market.exchangeRate = totalSupply[idBelow]
                .divWad(totalSupply[idAbove])
                .toUint96();
        } else if (answer <= strike) {
            market.outcome = Outcome.BELOW;
            market.exchangeRate = totalSupply[idAbove]
                .divWad(totalSupply[idBelow])
                .toUint96();
        }

        market.roundId = quote.roundId;

        market.asset.safeTransfer(quote.quoter, asset.quoteBondAmount);
    ...
    function withdrawFees(
        ...
        amountOut = accumulatedFees[asset];

        delete accumulatedFees[asset];
...
interface IContrastPair {
    ...
    /// @title Asset
    /// @dev A struct representing an asset supported by the protocol.
    /// @notice `quoteBondAmount` The amount of the bond currency required to create a quote.
    /// @notice `qouteDisputeWindow` The duration of the dispute window for a quote in seconds.
    /// @notice `emergencyWithdrawalWindow` The duration of time in seconds after which emergency withdrawals can be made for a quote.
    /// @notice `isActive` Status of the asset.
    struct Asset {
        uint256 quoteBondAmount;
        uint256 qouteDisputeWindow;
        uint256 emergencyWithdrawalWindow;
    ...
    /// @param amountStrikeBelow The amount of the false outcome token minted.
    /// @param amountStrikeAbove The amount of the true outcome token minted.
    event MarketEntered(
        uint256 indexed marketId,
    ...
    /// @param amountStrikeBelow The amount of the false outcome token burned.
    /// @param amountStrikeAbove The amount of the true outcome token burned.
    event MarketExited(
        uint256 indexed marketId,
        address indexed caller,
        uint256 amountStrikeBelow,
    ...
    error CALLER_NOT_FACTORY();

    error ASSET_ALREADY_ACTIVE();
    ...
    function quotes(uint256) external view returns (Quote memory);

    /// @dev This function is constrained by gas limits and may revert in the future.
    function totalMarketsOpen() external view returns (uint256);
    ...
    /// @dev Computes the market's implied probability for its true outcome, expressed using 18 decimal places.
    function computeImpliedProbability(
        uint256 marketId
    ) external view returns (uint256);

    /// @dev Computes the potential ROI for a markets outcome, expressed using 18 decimal places.
    function computeOutcomeROI(
        uint256 marketId,
        bool outcome
    ) external view returns (uint256);

    function accumulatedFees(address asset) external view returns (uint256);
    ...
    function redeem(uint256 marketId, uint256 amountIn) external;
    ...
    function price(uint256 marketId, uint80 roundId) external;
