...
contract Transmolgrifier is AccessControlEnumerableV2, SmolsAddressRegistryConsumer {
	...
	bool transmogActive = false;

	event SmolTransmogrified(uint256 smolIdToTransmogrify, uint256 smolRecipeId, uint256[] smolIdsToBurn);
	event SmolRecipeDeleted(uint256 smolRecipeId);
	event SmolRecipeAdded(uint256 smolRecipeId, SmolData smolData, string smolImage);
	event SmolRecipeAdjusted(uint256 smolRecipeId, SmolData smolData);

	function transmolgrify(uint256 _smolRecipeId, uint256 _smolIdToTransmogrify, uint256[] calldata _smolIdsToBurn) external {

		require(transmogActive, "Transmog isn't currently active!");
		...
		for(uint256 i = 0; i < smolData.smolInputAmount; i++){
			require(ISmolsState(smolsStateAddress).getInitialSmol(_smolIdsToBurn[i]).gender == 2, "Not a female smol!");

			//Pull the iq of the to-be-burned smol from the school
			uint128 _iqOfSmol = ISchool(schoolAddress).tokenDetails(smolsAddress, 0, _smolIdsToBurn[i]).statAccrued;

			//Remove all the iq of the smol
			ISchool(schoolAddress).removeStatAsAllowedAdjuster(smolsAddress, 0, _smolIdsToBurn[i], _iqOfSmol);

			//Add it to the aggregate amount of iq to grant.
			_iqToGrant += _iqOfSmol;

			//Burn the smol
			IERC721(smolsAddress).transferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, _smolIdsToBurn[i]);
		}

		//Give the aggregate IQ to the to-be-adjusted smol
		ISchool(schoolAddress).addStatAsAllowedAdjuster(smolsAddress, 0, _smolIdToTransmogrify, _iqToGrant);

		//Ensure they own the to-be-adjusted smol
		require(IERC721(smolsAddress).ownerOf(_smolIdToTransmogrify) == msg.sender, "You don't own the subject smol!");

		//Set the initial smol to the recipe chosen
		ISmolsState(smolsStateAddress).setInitialSmol(_smolIdToTransmogrify, smolData.smol);
		
		emit SmolTransmogrified(_smolIdToTransmogrify, _smolRecipeId, _smolIdsToBurn);
	}

	function setTransmogActive(bool _transmogActive) external requiresRole(OWNER_ROLE) {
		transmogActive = _transmogActive;
	}
	...
	function addSmolRecipes(SmolData[] calldata _smolsData) external requiresRole(OWNER_ROLE) {
		for(uint256 i = 0; i <_smolsData.length; i++){
			...
			address rendererAddress = smolsAddressRegistry.getAddress(SmolAddressEnum.SMOLSRENDERERADDRESS);
			...
			emit SmolRecipeAdded(thisSmolRecipeId, _smolData, string(ISmolsRenderer(rendererAddress).generateSVG(_smolsData[i].smol)));
		}
...
interface ISmolsRenderer{
    ...
    function generateSVG(Smol memory) external view returns(bytes memory);
}
