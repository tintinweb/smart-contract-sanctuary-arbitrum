...
contract LiquidityPoolProxy is LiquidityPool, Ownable {
    ...
    function withdrawGivenInputAmount(
        ...
        uint256 burnAmount
    ) public view override returns (uint256 withdrawnAmount) {
        (uint256 xBalance, uint256 yBalance) = _getBalances();
        uint256 totalSupply = _getTotalSupply();
    ...
    ) public view override returns (uint256 burnAmount) {
        (uint256 xBalance, uint256 yBalance) = _getBalances();
        uint256 totalSupply = _getTotalSupply();
    ...
    function _specifiedToken(uint256 tokenId) private view returns (uint256) {
        if (tokenId == xToken) {
            return 0;
        } else {
