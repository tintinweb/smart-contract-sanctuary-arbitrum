...
contract MToken is ERC20, Ownable {
    ...
    function settleMint(address account, uint256 amount) external onlySettlemans {
        _mint(account, amount);
    }

    function settleBurn(uint256 amount) external onlySettlemans {
    ...
    modifier onlySettlemans(){
        require(_settlemans[_msgSender()] == true, "Ownable: caller is not the settlemans");
        _;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (totalSupply() > 0)
        {
            require(_isPauseTransfer == false, "ERC20: transfer paused");
