...
uint256 constant ID = uint256(
    keccak256("game.piratenation.energyprovidedcomponent.v1")
);
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.equipmenttypecomponent.v1")
);
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.imageurlcomponent.v1")
);
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.isplaceablecomponent.v1")
);
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.placeableobjectcomponent.v1")
);
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.raritycomponent.v1")
);
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.shiprankcomponent.v1")
);
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.size3dcomponent.v1")
);
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.soulboundcomponent.v1")
);
...
contract TraitsMigrationSystem is GameRegistryConsumerUpgradeable {
    ...
    function setTraitBytes(
        ...
        if (traitId == ANIMATION_URL_TRAIT_ID) {
            ...
            placeableObject.setValue(entity, layout);
            return true;
    ...
    function getTraitBool(
        ...
        if (traitId == IS_PLACEABLE_TRAIT_ID) {
            ...
            value = PlaceableObjectComponent(
                _gameRegistry.getComponent(PlaceableObjectComponentId)
            ).getValue(entity).isBottomStackable;
        } else if (traitId == PLACEABLE_IS_TOP_STACKABLE_TRAIT_ID) {
            isComponent = true;
            value = PlaceableObjectComponent(
                _gameRegistry.getComponent(PlaceableObjectComponentId)
            ).getValue(entity).isTopStackable;
        } else if (traitId == SOULBOUND_TRAIT_ID) {
            isComponent = true;
    ...
    function _setCombatModifier(
        ...
        CombatModifiersComponentLayout memory layout = component.getValue(
            entity
        );
