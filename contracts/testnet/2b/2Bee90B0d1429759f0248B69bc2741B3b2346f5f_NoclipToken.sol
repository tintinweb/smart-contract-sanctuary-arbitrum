...
contract NoclipToken is Context, IERC20, Ownable {
  ...
  uint private _finalBuyTax = 300; // 3.00%
  uint private _finalSellTax = 300;
  ...
  string private constant _symbol = "NOCLIP";
  uint8 private constant _decimals = 18;
  uint private _totalSupply = 10_000_000 * 10**_decimals;

  uint public _maxTxAmount = 300; // 3.0%
  uint public _maxWalletSize = 300; // 3.0%
  uint public _swapThreshold = 1; // 0.01%
  ...
  function distributeETH(uint amount) private {
    uint half = amount.div(2);
    Address.sendValue(_marketingWallet, half);
    Address.sendValue(_rewardWallet, amount.sub(half));
  }

  function initPair() external onlyOwner {
    require (uniswapV2Pair == address(0), "Pair is already initialized");
    // ARBITRUM GOERLI: 0x454206AD825cAfaE03c9581014AF6b74f7D53713
    // ARBITRUM MAINNET: 0xbeE5c10Cf6E4F68f831E11C1D9E59B43560B3642
    uniswapV2Router = 0x454206AD825cAfaE03c9581014AF6b74f7D53713;
