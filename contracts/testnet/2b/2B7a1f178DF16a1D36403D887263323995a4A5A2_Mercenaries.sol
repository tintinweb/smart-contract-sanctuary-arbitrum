...
// ADMIN FUNCTIONS
        ...
        onlyAllowed
    {
        ...
        uint256 startingTokenID = wTokenID + 1;                                         // Set starting ID ... increment wTokenID outside of the loop
        for(uint256 i = 0; i < amount;){                                                // Loop through the amount to mint
        ...
        wTokenID += amount;                                                             // Increment wrapped token id
    }
...
// VIEWS
        ...
        returns (string memory uri) 
    {
        ...
        if (collectionAddress == address(this)) {
            ...
            string memory mercenaryName = string(abi.encodePacked("Mercenary #",Strings.toString(wrappedTokenID)));
            ...
            string memory output = Base64.encode(abi.encodePacked(json1, encodeTraits(wrappedTokenID), encodeStats(underlyingTokenHash)));
    ...
    function bytes16ToString(bytes16 toConvert) private pure returns (string memory) {
        uint8 i = 0;
        ...
        override(ERC721)
    {
        ...
        require(!charactersActivity[tokenId].active, "Mercenary is active"); // Revert if any the mercenary is active
    }
...
// CONSTRUCTOR
    ...
    WrappedCharacterActivity(name_, symbol_) {}
...
// FUNCTIONS
    ...
    {
        unchecked { wTokenID++; }                       // Increment wrapped token id
...
// CONSTRUCTOR
    ...
    constructor (string memory name_, string memory symbol_)
    WrappedCharacterManager(name_, symbol_) {}
...
// EXTERNAL FUNCTIONS
        ...
        onlyWrapped(wrappedTokenID)
    {
        ...
        if (statIndex == 6) {
            maxStat = getMaxMorale(wrappedTokenID);                          // Morale has a different calculation for max stat
        }
        if (statIndex > 6) {
    ...
    /// @param amount to add
    function setStatTo(uint256 wrappedTokenID, uint16 amount, uint256 statIndex) external onlyAllowed {
        bytes32 underlyingTokenHash = wrappedCharacter[wrappedTokenID];      // Get the underlying token hash
        uint16 currentStat = stats[underlyingTokenHash][statIndex];          // Get current stat
        if (amount > currentStat) {
            increaseStat(wrappedTokenID, amount - currentStat, statIndex);
        } else {
            decreaseStat(wrappedTokenID, currentStat - amount, statIndex);
        }
    }
...
// INTERNAL FUNCTIONS
        ...
        isRegistered(collectionAddress)
    {
        ...
        if (courage > 95 || intelligence > 95) // Give bonus for a Genius or Hero
        {
            morale += morale / 2;
        }
...
// VIEW FUNCTIONS
    ...
    function getStats(uint256 wrappedTokenID)
        ...
        view
        returns (
            uint16 stamina, uint16 strength, uint16 speed, uint16 courage, uint16 intelligence, uint16 health, uint16 morale, uint16 experience, uint16 level
        )
    {
        bytes32 underlyingTokenHash = wrappedCharacter[wrappedTokenID]; // Get the underlying token hash
        ...
        if (total > 6){morale = stats[underlyingTokenHash][6];}
        if (total > 7){experience = stats[underlyingTokenHash][7];}
        if (total > 8){level = stats[underlyingTokenHash][8];}
    }  
    ...
    /// @param wrappedTokenID Characters ID
    function getMaxMorale(uint256 wrappedTokenID)
        public
        view
        returns (
            uint16 morale
    ...
    {
        (,,,uint16 courage, uint16 intelligence,,,,) = getStats(wrappedTokenID); // Retrieve stats
        morale = (courage + intelligence) / 2 ;
...
// CONSTRUCTOR
    ...
    constructor (string memory name_, string memory symbol_)
    WrappedCharacterStats(name_, symbol_) {}
