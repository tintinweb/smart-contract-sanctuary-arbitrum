...
contract Vault is Constants, ReentrancyGuard, Ownable, IVault {
    ...
    function _collectVaultFee(bool _mint, address _token, uint256 _amount, uint256 _fee, address _refer) internal {
        ...
        if (_fee != 0 && _feeManager != ZERO_ADDRESS) {
            ...
            uint256 feeMinusFeeRewardInToken = feeMinusFeeReward == 0 ? 0 : 
                (isRUSD ? feeMinusFeeReward : priceManager.fromUSDToToken(_token, feeMinusFeeReward));
                
            if (!isRUSD && IERC20(_token).balanceOf(address(this)) >= feeMinusFeeRewardInToken && feeMinusFeeRewardInToken > 0) {
        ...
        if (_refer != ZERO_ADDRESS && settingsManager.referEnabled()) {
            ...
            uint256 referFeeInToken = referFee == 0 ? 0 : 
                (isRUSD ? referFee : priceManager.fromUSDToToken(_token, referFee));

            if (!isRUSD && IERC20(_token).balanceOf(address(this)) >= referFeeInToken && referFeeInToken > 0) {
    ...
    function updateTotalBalances() external {
        require(_isInternal() || msg.sender == owner(), "Forbidden");
        uint256 sum;
    ...
    function stake(address _account, address _token, uint256 _amount) external nonReentrant {
        ...
        require(mintAmount > 0, "MinAmount too low");
    ...
    function unstake(address _tokenOut, uint256 _rolpAmount, address _receiver) external nonReentrant {
        ...
        _setBalance(balances, _tokenOut, amountOutInToken, false);
    ...
    function rescueERC20(address _recipient, address _token, uint256 _amount) external onlyOwner {
        ...
        if (isVaultBalance) {
            ...
            _setBalance(balances, _token, _amount, false);
        }
