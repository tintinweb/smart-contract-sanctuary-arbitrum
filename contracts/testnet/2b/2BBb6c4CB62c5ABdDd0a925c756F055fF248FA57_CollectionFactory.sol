...
contract CollectionFactory is NonblockingLzApp {
    ...
    function create(string memory chSym, string memory tName, string memory tSym, string memory collName, string memory collURI, string memory fileURI, uint256 price, uint256 from, uint256 to, address destAddr, string[] memory URIs, uint gas) public payable {
        ...
        if (chainSymbolToId[chSym] == chainId) {
            IOmniERC721 collection = _add(tName, tSym, collName, collURI, tokenOwner, price);
            _setDetails(collection, fileURI, URIs, from, to);
            emit Created(collection.getAddress(), tokenOwner);
            return;
        }
        bytes memory payload = _getPayload(tName, tSym, collName, collURI, fileURI, price, from, to, tokenOwner, URIs);
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64, bytes memory _payload) internal override {
        ...
        (string memory tokenName, string memory tokenSymbol, string memory collName, string memory collectionURI, string memory fileURI, address creator, uint256 mintPrice, uint256 dropFrom, uint256 dropTo, string[] memory metadataURIs)
        = abi.decode(_payload, (string, string, string, string, string, address, uint256, uint256, uint256, string[]));
        IOmniERC721 collection = _add(tokenName, tokenSymbol, collName, collectionURI, creator, mintPrice);
    ...
    function estimateFees(string memory chSym, string memory tName, string memory tSym, string memory URI, uint256 price, uint256 from, uint256 to, address tokenOwner, string[] memory URIs, uint gas) external view returns (uint) {
        bytes memory payload = _getPayload(tName, tSym, "", URI, "", price, from, to, tokenOwner, URIs);
        uint16 chId = uint16(chainSymbolToId[chSym]);
        return _getLzFee(chId, payload, _getAdapter(gas));
    }

    function _getPayload(string memory tName, string memory tSym, string memory collName, string memory URI, string memory fileURI, uint256 price, uint256 from, uint256 to, address tokenOwner, string[] memory URIs) private pure returns(bytes memory) {
        return abi.encode(tName, tSym, collName, URI, fileURI, tokenOwner, price, from, to, URIs);
    }
    ...
    function _add(string memory tName, string memory tSym, string memory collName, string memory collURI, address tokenOwner, uint256 price) internal returns (IOmniERC721) {
        return _collectionsRepository.addCollection(tName, tSym, collName, collURI, tokenOwner, price);
    }
