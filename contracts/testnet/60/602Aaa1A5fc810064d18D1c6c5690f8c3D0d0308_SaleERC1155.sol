...
contract SaleERC1155 is Pausable, Initializable, ERC1155Holder {
    ...
    IERC1155 public tokenToSell;
    Round[] public rounds;
    mapping(address => mapping(uint256 => uint256)) public boughtTokenToSellByUser; // User => nft id => amount
    mapping(address => mapping(uint256 => uint256)) public claimedTokenToSellByUser; // User => nft id => amount
    mapping(address => uint256) public donatedSaleTokenByUser; // User => amount
    mapping(uint256 => uint256) public totalTokenToSellSold;  // nft id => amount
    mapping(uint256 => uint256) public serviceBalance;   // nft id => amount
    mapping(uint256 => mapping(address => uint8)) public whitelists;  // round id => whitelist infos
    ...
    uint8 constant public WHITELISTED_MASK = 128;
    ...
    event Withdrawed(address indexed user, uint256 indexed nftId, uint256 amount);
    event WithdrawedProfit(uint256 amount, uint256 actualAmount, uint256 fee);
    event EmergencyWithdrawedProfit(uint256 amount);
    ...
    function initialize(
        ...
        public initializer  
    {
        require(minimumAmountToFund_ == 0, "Minimum amount is not supported");

        saleToken = IERC20(saleTokenAddr);
        tokenToSell = IERC1155(tokenToSellAddr);
        claimPercents = [100];
    ...
    function fund(uint256 amount, uint256 nftId) public {
        serviceBalance[nftId] += amount;
        tokenToSell.safeTransferFrom(msg.sender, address(this), nftId, amount, "");

        emit Funded(msg.sender, nftId, amount);
    }

    function withdraw(uint256 amount, uint256 nftId) public onlySaleOwner {
        require(isSaleFinished(), "Not available before sale end");
        require(serviceBalance[nftId] >= amount, "Not enough service balance");

        serviceBalance[nftId] -= amount;
        tokenToSell.safeTransferFrom(address(this), msg.sender, nftId, amount, "");

        emit Withdrawed(msg.sender, nftId, amount);
    }
    ...
    function createRound (
        ...
        uint8[] calldata allocationBonuses
    )
        public onlyRaiseAdmin 
    {
        ...
        require(whitelist.length == allocationBonuses.length, "Arays must have the same size");
        ...
        for (uint256 i = 0; i < whitelist.length; i++) {
            require(allocationBonuses[i] <= 127, "Too large percent");

            whitelists[roundsLength][whitelist[i]] = allocationBonuses[i] | WHITELISTED_MASK;
        }
    ...
    function buy(uint256 saleTokenAmount, uint256 nftId) public whenNotPaused onlyHealthy {
        ...
        uint8 whitelistUserInfo = whitelists[ongoingRound.id][msg.sender];

        require(whitelistUserInfo & WHITELISTED_MASK != 0, "User can't participate");

        uint8 allocationBonusPercent = whitelistUserInfo ^ WHITELISTED_MASK;
        uint256 userMaxAllocation = maxAllocationPerUser + maxAllocationPerUser * allocationBonusPercent / 100;
        uint256 alreadyBought = boughtTokenToSellByUser[msg.sender][nftId];

        require(tokenToSellAmount <= userMaxAllocation - alreadyBought, "Allocation per user exceeded");
        require(tokenToSellAmount <= ongoingRound.maxAllocation - ongoingRound.boughtTokenToSell, "Round allocation exceeded");

        boughtTokenToSellByUser[msg.sender][nftId] += tokenToSellAmount; 
        totalTokenToSellSold[nftId] += tokenToSellAmount;
        rounds[ongoingRound.id].boughtTokenToSell += tokenToSellAmount;

        uint256 initialSaleTokenBalance = saleToken.balanceOf(address(this));

        saleToken.safeTransferFrom(msg.sender, address(this), saleTokenAmount);

        uint256 finalSaleTokenBalance = saleToken.balanceOf(address(this));
        uint256 saleTokenReceived = finalSaleTokenBalance - initialSaleTokenBalance;

        totalSaleTokenCollected += saleTokenReceived;
        donatedSaleTokenByUser[msg.sender] += saleTokenReceived;

        emit Bought(msg.sender, nftId, tokenToSellAmount);
    }

    function claim(uint256 nftId) public whenNotPaused {
        uint256 boughtTokenToSell = boughtTokenToSellByUser[msg.sender][nftId];
        uint256 withdrawedTokenToSell = claimedTokenToSellByUser[msg.sender][nftId];

        require(boughtTokenToSell > withdrawedTokenToSell, "All the bought tokens claimed");
        ...
        require(sumToGive <= serviceBalance[nftId], "Not enough service balance");

        serviceBalance[nftId] -= sumToGive;
        claimedTokenToSellByUser[msg.sender][nftId] += sumToGive;
        tokenToSell.safeTransferFrom(address(this), msg.sender, nftId, sumToGive, "");

        emit Claimed(msg.sender, nftId, sumToGive);
    }

    function withdrawProfit() public onlySaleOwner onlyHealthy {
        ...
        if (isWithdrawVestingEnabled) {
            uint256 percentsWithdrawn = tokenWithdrawed * 100 / tokenCollected;
            uint8 accPercentsToGive;
        ...
        totalSaleTokenWithdrawed += amountToWithdrawNow;

        uint256 fee = amountToWithdrawNow * serviceFeePercent / 100;
        saleToken.safeTransfer(msg.sender, amountToWithdrawNow - fee);
        saleToken.safeTransfer(raiseAdmin, fee);

        emit WithdrawedProfit(amountToWithdrawNow, amountToWithdrawNow - fee, fee);
    }

    function emergencyWithdrawProfit() public onlyRaiseAdmin ifProjectUnhealthy {        
        uint256 amountToWithdraw = totalSaleTokenCollected - totalSaleTokenWithdrawed;

        require(amountToWithdraw > 0, "Nothing to withdraw");

        totalSaleTokenWithdrawed += amountToWithdraw;
        saleToken.safeTransfer(msg.sender, amountToWithdraw);

        emit EmergencyWithdrawedProfit(amountToWithdraw);
    }

    function refund() public ifProjectUnhealthy {
        uint256 amountToRefund = sumToRefundIfUnhealthy * donatedSaleTokenByUser[msg.sender] / totalSaleTokenCollected;

        require(amountToRefund > 0, "Nothing to refund");

        totalSaleTokenWithdrawed += amountToRefund;
        donatedSaleTokenByUser[msg.sender] = 0;
        saleToken.safeTransfer(msg.sender, amountToRefund);
    ...
    function setIsUnhealthy(bool isUnhealthy_) public onlyRaiseAdmin {
        ...
        if (isUnhealthy)
            sumToRefundIfUnhealthy = totalSaleTokenCollected - totalSaleTokenWithdrawed;
    ...
    function canParticipate(address user) public view returns (bool) {
        ...
        return whitelists[round.id][user] & WHITELISTED_MASK != 0;
    }

    function getUserAllocationBonus(address user) public view returns (uint8 allocationBonusPercent) {
        Round memory round = getOngoingRound();
        uint8 whitelistUserInfo = whitelists[round.id][user];

        require(whitelistUserInfo & WHITELISTED_MASK != 0, "User can't participate");

        allocationBonusPercent = whitelistUserInfo ^ WHITELISTED_MASK;
    }
    
    function canClaim(address user, uint256 nftId) public view returns (bool) {
        uint256 boughtTokenToSell = boughtTokenToSellByUser[user][nftId];
        uint256 withdrawedTokenToSell = claimedTokenToSellByUser[user][nftId];
    ...
    function isSaleFinished() public view returns (bool) {
        ...
        return rounds[roundsLength - 1].isFinal && rounds[roundsLength - 1].deadline >= block.timestamp;
    }
}
