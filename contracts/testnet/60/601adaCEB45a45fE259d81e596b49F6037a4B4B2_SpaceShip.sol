...
contract SpaceShip is Ownable, Pausable {
    ...
    */
    function _claimHumanFromShip(uint256 tokenId, bool unstake) internal returns (uint256) {
        ...
        claimableAmount = human.level * (((block.timestamp - human.lastClaim) * DAILY_ASTRO_EARN) / 1 days);
        if (unstake) {
   ...
   */
    ...
    function _levelForHuman(uint256 tokenId) internal view returns (uint8) {
        return arbiGobblers.getTokenTraits(tokenId).levelIndex;
    }

    function randomAlienOwner(uint256 rand) external view returns (address) {
        if (totalArbiGobblerStaked == 0) return address(0x0);
        uint256 randomIndex = (
            uint256 (
                keccak256(
                    abi.encodePacked(
        ...
        return getArbiGobblerData[randomIndex].owner;
    }
