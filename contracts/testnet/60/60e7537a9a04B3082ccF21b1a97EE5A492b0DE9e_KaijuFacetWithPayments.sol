...
contract KaijuFacet is SolidStateERC1155, IERC2981, WithStorage, WithModifiers {
    ...
    function getPioneerPackCost(
        ...
        return _constants().packTypeToUsdPrice[packType]; // TODO
    }
    ...
    function setPackAmount(
        LibToken.PioneerPackType packType,
        uint8 amount
    ) public ownerOnly {
        _shop().packQuantities[packType] = amount;
    }
    ...
    function hasClaimedGen1(uint256 tokenId) external view returns (bool) {
        return _token().hasClaimedFreeKaiju[tokenId];
    }
...
contract KaijuFacetWithPayments is IPaymentsReceiver, KaijuFacet {
    ...
    function acceptGasToken(
        ...
        if (
            msg.value <
            _constants().minPriceByPaymentType[PaymentType.ETH_IN_USD]
        ) {
            revert LibMeta.IncorrectPaymentAmount(
                msg.value,
                _paymentAmountInPricedToken
    ...
    function getMinPriceByPaymentType(
        PaymentType paymentType
    ) public view returns (uint256) {
        return _constants().minPriceByPaymentType[paymentType];
    }

    function setMinPriceByPaymentType(
        PaymentType paymentType,
        uint256 minPrice
    ) public ownerOnly {
        _constants().minPriceByPaymentType[paymentType] = minPrice;
    }


    function getUsdPriceForPackType(
        LibToken.PioneerPackType packType
    ) public view returns (uint256) {
        return _constants().packTypeToUsdPrice[packType];
    }

    function setUsdPriceForPackType(
        LibToken.PioneerPackType packType,
        uint256 price
    ) public ownerOnly {
        uint256 originalPrice = _constants().packTypeToUsdPrice[packType];
        _constants().usdPriceToPackType[originalPrice] = LibToken
            .PioneerPackType
            .NONE;
        _constants().usdPriceToPackType[price] = packType;
        _constants().packTypeToUsdPrice[packType] = price;
    }
}
