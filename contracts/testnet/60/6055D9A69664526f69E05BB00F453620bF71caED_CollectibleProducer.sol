...
contract CollectibleProducer is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  function collect(
    ...
    require(
      _realmIds.length <= maxCollect,
      "CollectibleProducer: Above max Realms"
    );
    ...
    returns (uint256)
  {
    ...
    return elapsedTime - ((elapsedTime / cooldown) * cooldown);
  }
  ...
  function updateMaxCollect(uint256 _maxCollect) external onlyAdmin {
    maxCollect = _maxCollect;
  }
