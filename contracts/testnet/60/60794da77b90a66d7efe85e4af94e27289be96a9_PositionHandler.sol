...
contract PositionHandler is PositionConstants, IPositionHandler, Ownable {
    ...
    function modifyPosition(
        ...
        bool isTriggerDelayPosition = false;
        ...
        if (_txType == ADD_COLLATERAL || _txType == REMOVE_COLLATERAL) {
            ...
            (Position memory position, OrderInfo memory order) = abi.decode(_data, ((Position), (OrderInfo)));
            isTriggerDelayPosition = position.size == 0;
            _triggerPosition(
                _account,
                key,
                _isLong,
        ...
        if (_txType == CREATE_POSITION_MARKET ||
                ...
                (_txType == TRIGGER_POSITION && isTriggerDelayPosition)) {
            vault.decreaseBond(key, _account, _txType);
        }
    }
    ...
    function _triggerPosition(
        ...
        require(
            (statusFlag == ORDER_FILLED || hitTrigger) &&
                (_position.owner == _account || msg.sender == router),
