...
contract SettingsManager is ISettingsManager, Ownable, Constants {
    ...
    function updateCumulativeFundingRate(address _token, bool _isLong) external override hasPermission {
        ...
        cumulativeFundingRates[_token][_isLong] += _getNextFundingRate(_token, _isLong);
        lastFundingTimes[_token][_isLong] = uint256(block.timestamp / fundingInterval) * fundingInterval;
    ...
    function collectMarginFees(
        ...
        if (referralSystem != address(0)) {
            ...
            if (discountPercentage > 0) {
                feeUsd -= feeUsd * discountPercentage / BASIS_POINTS_DIVISOR;
            }
        }
    ...
    function getNextFundingRate(address _token, bool _isLong) external view returns (uint256) {
        return _getNextFundingRate(_token, _isLong);
    }

    function _getNextFundingRate(address _token, bool _isLong) internal view returns (uint256) {
        uint256 intervals = (block.timestamp - lastFundingTimes[_token][_isLong]) / fundingInterval;

        if (vault.poolAmounts(_token) == 0) {
            return 0;
        }
        ...
        return
            ((
                ...
                intervals) / vault.poolAmounts(_token);
    }
    ...
    */
    function validateCollateralPathAndCheckSwap(address[] memory _collateralPath) external override view returns (bool) {
        if (isEnableNonStableCollateral) {
            ...
            _isApprovalCollateralToken(_collateralPath[0], true);
            return false;
        } else {
