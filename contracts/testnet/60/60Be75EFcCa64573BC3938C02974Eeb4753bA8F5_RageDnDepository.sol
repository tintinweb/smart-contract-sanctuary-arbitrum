...
contract RageDnDepository is IRageDnDepository {
    ...
    IERC20Metadata public redeemable;
    ...
    constructor(address _vault, address _controller, address _redeemable) {
        vault = IDnGmxSeniorVault(_vault);
        controller = IUXDController(_controller);
        asset = IERC20Metadata(vault.asset());
        redeemable = IERC20Metadata(_redeemable);
        // redeemable = IERC20Metadata(controller.redeemable());
    }
    ...
    function deposit(uint256 amount) external onlyController returns (uint256) {
        ...
        uint256 redeemableAmount = vault.deposit(
            amount, address(this)
        ).fromDecimalToDecimal(asset.decimals(), 18 /* redeemable.decimals()*/);
        totalRedeemable += redeemableAmount;
        emit Deposited(msg.sender, amount, redeemableAmount);
        return redeemableAmount;
    }

    /// @dev withdraw a specific amount of asset
    function withdraw(uint256 amount) onlyController external returns (uint256) {
        netDeposits -= amount;
        uint256 redeemableAmount = vault.withdraw(
            amount, address(controller), address(this)
        ).fromDecimalToDecimal(asset.decimals(), 18 /* redeemable.decimals()*/);
        totalRedeemable -= redeemableAmount;
        emit Withdrawn(msg.sender, amount, redeemableAmount);
        return redeemableAmount;
    }

    /// dev redeem a specific amount of redeemable
    function redeem(uint256 redeemableAmount) external returns (uint256) {
        uint256 shareAmount = redeemableAmount.fromDecimalToDecimal(/*redeemable.decimals()*/ 18, asset.decimals());
        uint256 assetAmount = vault.redeem(shareAmount, address(controller), address(this));
        netDeposits -= assetAmount;
        totalRedeemable -= redeemableAmount;
        emit Redeemed(msg.sender, redeemableAmount, assetAmount);
        return assetAmount;
    }
