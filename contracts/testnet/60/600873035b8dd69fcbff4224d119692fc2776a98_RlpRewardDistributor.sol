...
contract RlpRewardDistributor is Initializable, OwnableUpgradeable {
    ...
    function initialize(address _pool, address _rewardToken, address _weth, address _ethUnwrapper)
        ...
        initializer
    {
        require(_rewardToken != address(0), "RlpRewardDistributor::initialize: Invalid address");
        require(_pool != address(0), "RlpRewardDistributor::initialize: Invalid address");
        require(_weth != address(0), "RlpRewardDistributor::initialize: Invalid address");
        require(_ethUnwrapper != address(0), "RlpRewardDistributor::initialize: Invalid address");
        __Ownable_init();
        pool = IPool(_pool);
    ...
    function transferRewards(address _to, uint256 _amount) external onlyRequester {
        require(_to != address(0), "RlpRewardDistributor::transferRewards: Invalid address");
    ...
    function transferRewardsToSingleToken(address _to, uint256 _amount, address _tokenOut, uint256 _minAmountOut)
        ...
        onlyRequester
    {
        require(_to != address(0), "RlpRewardDistributor::transferRewardsToSingleToken:invalidAddress");
        require(withdrawableTokens[_tokenOut], "RlpRewardDistributor::transferRewardsToSingleToken:notWithdrawable");
        if (_amount > 0) {
            ...
            require(_amountOut >= _minAmountOut, "RlpRewardDistributor::transferRewardsToSingleToken:slippage");
    ...
    function convertToRlp(address _token, uint256 _amount, uint256 _minRlpAmount) external onlyController {
        require(_token != address(0), "RlpRewardDistributor::convertToRlp: Invalid address");
        if (_amount > 0) {
            uint256 _balanceBefore = rewardToken.balanceOf(address(this));
            IERC20(_token).safeIncreaseAllowance(address(pool), _amount);
            pool.addLiquidity(address(rewardToken), _token, _amount, _minRlpAmount, address(this));
            uint256 _amountOut = rewardToken.balanceOf(address(this)) - _balanceBefore;
            require(_amountOut >= _minRlpAmount, "RlpRewardDistributor::convertToRlp: slippage");
    ...
    function setRequester(address _requester) external onlyOwner {
        require(_requester != address(0), "RlpRewardDistributor::setRequester: Invalid address");
        requester = _requester;
        emit RequesterSet(_requester);
    }

    function recoverFund(address _token, address _to, uint256 _amount) external onlyOwner {
        require(_to != address(0), "RlpRewardDistributor::recoverFund: Invalid address");
        require(_token != address(0), "RlpRewardDistributor::recoverFund: Invalid address");
    ...
    function recoverETH(address _to, uint256 _amount) external onlyOwner {
        require(_to != address(0), "RlpRewardDistributor::recoverETH: Invalid address");
        (bool success,) = _to.call{value: _amount}(new bytes(0));
        require(success, "RlpRewardDistributor::recoverETH: ETH transfer failed");
        emit EthRecovered(_to, _amount);
    }

    function setTokenWithdrawable(address _token, bool _allowed) external onlyOwner {
        require(_token != address(0), "RlpRewardDistributor::setTokenWithdrawable: Invalid address");
    ...
    function setController(address _controller) external onlyOwner {
        require(_controller != address(0), "RlpRewardDistributor::setController: Invalid address");
