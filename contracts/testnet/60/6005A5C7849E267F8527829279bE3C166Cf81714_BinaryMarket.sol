    ...
    AccessControl
{
    ...
    */
    ...
    function genesisStartRound() external onlyOperator whenNotPaused {
        ...
        genesisStartBlockNumber = getBlockNumber();
    ...
    function executeRound(
        ...
        for (uint8 i = 0; i < timeframeIds.length; i = i + 1) {
            ...
            if(genesisLockOnces[timeframeId]) {
                ...
                if (currentEpoch == currenetLockedEpochs[timeframeId]) {
                    currentEpoch += 1;
                }
    ...
    function _safeStartRound(uint8 timeframeId, uint256 epoch) internal {
        ...
        require(
            getBlockNumber() >= rounds[timeframeId][currenetLockedEpochs[timeframeId]].closeBlock,
    ...
    function _startRound(uint8 timeframeId, uint256 epoch) internal {
        ...
        round.startBlock = getBlockNumber();
        round.lockBlock = getBlockNumber() + timeframes[timeframeId].intervalBlocks;
        round.closeBlock = getBlockNumber() + timeframes[timeframeId].intervalBlocks * 2;
        round.epoch = epoch;
        round.totalAmount = 0;
    ...
    function _safeLockRound(
        ...
        require(
            getBlockNumber() >= rounds[timeframeId][epoch].lockBlock,
    ...
    function _safeEndRound(
        ...
        require(
            getBlockNumber() >= rounds[timeframeId][epoch].closeBlock,
    ...
    */
        ...
        returns (bool)
    {
        ...
        if (rounds[timeframeId][epoch].startBlock != 0) {
            ...
            getBlockNumber() > rounds[timeframeId][epoch].startBlock;
        } else {
            return timestamp <= block.timestamp + futureBettingTimeUpTo &&  getBlockNumber() <= blockNumber && block.timestamp <= timestamp;
        }
    ...
    function refundable(
        ...
        return
            !round.oracleCalled &&
            getBlockNumber() > round.closeBlock + timeframes[timeframeId].bufferBlocks &&
            betInfo.amount != 0;
    }
    ...
    */
    ...
    function isNecessaryToExecute(uint8 timeframeId) public view returns(bool) {
        ...
        bool lockable = currentActiveRound.startBlock != 0 && currentActiveRound.lockPrice == 0 && getBlockNumber() >= currentActiveRound.lockBlock;
        bool closable = currentLockedRound.lockBlock !=0 && currentLockedRound.closePrice == 0 && !currentLockedRound.oracleCalled && getBlockNumber() >= currentLockedRound.closeBlock;
    ...
    */
    ...
    {
        return getRoundIdAt(timeframeId, block.timestamp, getBlockNumber());
    }
    ...
    */
    ...
    function getBlockNumber() public view returns(uint256 blockNumber){
        console.log("blockNumber",  ArbSys(address(100)).arbBlockNumber());
        blockNumber = ArbSys(address(100)).arbBlockNumber(); // arbitrum mainnet
    }
