...
import {ICalculusIncentivesController} from './ICalculusIncentivesController.sol';
 ...
 * @notice Distributor contract for rewards to the Calculus protocol, using a staked token as rewards asset.
 * The contract stakes the rewards before redistributing them to the Calculus protocol participants.
 * @author Calculus
 **/
contract StakedTokenIncentivesController is
  ICalculusIncentivesController,
  ...
  DistributionManager
{
  ...
  address private constant Calculus_TOKEN = 0x4a7412dff5BfaE87867FcCe7010A69d8b2842a6A;
  ...
  /// @inheritdoc ICalculusIncentivesController
  ...
  function DISTRIBUTION_END()
    external
    view
    override(DistributionManager, ICalculusIncentivesController)
    returns (uint256)
  {
  ...
  function getUserAssetData(
    address user,
    address asset
  ) public view override(DistributionManager, ICalculusIncentivesController) returns (uint256) {
  ...
  function _claimRewards(
    ...
    if (IERC20(Calculus_TOKEN).balanceOf(address(this)) >= amountToClaim) {
      IERC20(Calculus_TOKEN).safeTransfer(to, amountToClaim);
    }
