...
contract Evm721StarterUSDC is ERC721, Ownable {
    ...
    uint256 public usdcPrice = 1 * 10 ** 6; // 1200 USDC (because usdc is a 6 decimal ERC20 token)
    ...
    function mintUsdc(address _to, uint256 _quantity) external {
        uint256 requiredAmount = usdcPrice * _quantity;

        // Check if the sender has enough balance to cover the cost.
        require(usdc.balanceOf(msg.sender) >= requiredAmount, "Insufficient USDC balance");

        // Check if the contract is allowed to transfer the required amount of USDC.
        require(usdc.allowance(msg.sender, address(this)) >= requiredAmount, "Insufficient allowance");
    ...
    function setusdcPrice(uint256 _newPrice) external onlyOwner {
        usdcPrice = _newPrice;
    }
