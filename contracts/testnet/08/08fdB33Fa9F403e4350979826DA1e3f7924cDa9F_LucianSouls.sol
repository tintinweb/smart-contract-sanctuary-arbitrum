...
contract LucianSouls is ERC1155LazyMint, PermissionsEnumerable {
    ...
    // 1. Is the Gem NFT Collection
    ERC1155LazyMint public immutable gem;
    // 2. Is the Soul NFT Collection
    ERC1155LazyMint public immutable soul;
    ...
    constructor(
        ...
        gem = ERC1155LazyMint(address(this));
        soul = ERC1155LazyMint(address(this));
    ...
    function verifyClaim(
        ...
        if(_tokenId == 0 || _tokenId > 10){
            ...
            //Gem
            require(soul.balanceOf(_claimer, 0) >= _quantity * 50, "You don't own enough Soul NFTs");
        }else{
            //Monster
            string memory _gemCaution = "You don't own enough Summoning Gem NFTs";
            uint256 _multiplier = 2;

            if (_tokenId <= 4) {
                require(gem.balanceOf(_claimer, 0) >= _quantity, _gemCaution);
            }else if (_tokenId <= 7) {
                require(this.balanceOf(_claimer, _tokenId - 3) >= _quantity * 2, _gemCaution);
                _multiplier = 100;
            }else if (_tokenId <= 10) {
                require(this.balanceOf(_claimer, _tokenId - 6) >= _quantity * 2, _gemCaution);
                _multiplier = 1000;
            }

            require(soul.balanceOf(_claimer, 0) >= _quantity * _multiplier, "You don't own enough Soul NFTs");
    ...
    function openChests(
        ...
        for (uint256 i = 0; i < _chestIds.length; i++) {
            this.openChest(_chestIds[i], _amountsToOpen[i]);
        }
    }
    ...
    function openChest(
        ...
        require(chest.balanceOf(opener, _chestId) >= _amountToOpen, "You don't own enough Chest NFTs");
        (uint256[] memory tokenIds, uint256[] memory amounts)  = getRewardUnits(_chestId, _amountToOpen);
    ...
    function getRewardUnits(
        uint256 _chestId,
        uint256 _amountToOpen
    ) internal view returns (uint256[] memory tokenIds, uint256[] memory amounts) {   
        ...
        if (_chestId == 0) {
            _lotteryWeightMaterial = [uint256(45), 35, 9, 9, 2];
            _amountsMaterial = [uint256(5), 3, 1, 1, 1];
        }else if (_chestId == 1) {
            _lotteryWeightMaterial = [uint256(45), 9, 35, 9, 2];
            _amountsMaterial = [uint256(5), 3, 1, 1, 1];
        }else if (_chestId == 2) {
            _lotteryWeightMaterial = [uint256(45), 9, 9, 35, 2];
            _amountsMaterial = [uint256(5), 3, 1, 1, 1];
        }else if (_chestId == 3) { // MANEKI
            _lotteryWeightMaterial = [uint256(60), 12, 12, 12, 4];
