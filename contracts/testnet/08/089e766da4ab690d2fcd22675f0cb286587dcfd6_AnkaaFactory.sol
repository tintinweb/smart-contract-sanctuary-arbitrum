...
import './IAnkaaFactory.sol';
import './AnkaaPair.sol';

contract AnkaaFactory is IAnkaaFactory {
     ...
     */
    modifier onlyOwner() {
        require(owner == msg.sender, "AnkaaFactory: caller is not the owner");
        _;
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'AnkaaFactory: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'AnkaaFactory: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'AnkaaFactory: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(AnkaaPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        require(pair != address(0), "AnkaaFactory: FAILED");
        AnkaaPair(pair).initialize(token0, token1);
    ...
    function setOwner(address _owner) external onlyOwner {
        require(_owner != address(0), "AnkaaFactory: zero address");
        emit OwnershipTransferred(owner, _owner);
        owner = _owner;
    }

    function setFeePercentOwner(address _feePercentOwner) external onlyOwner {
        require(_feePercentOwner != address(0), "AnkaaFactory: zero address");
    ...
    function setSetStableOwner(address _setStableOwner) external {
        require(msg.sender == setStableOwner, "AnkaaFactory: not setStableOwner");
        require(_setStableOwner != address(0), "AnkaaFactory: zero address");
    ...
    function setOwnerFeeShare(uint newOwnerFeeShare) external onlyOwner {
        require(newOwnerFeeShare > 0, "AnkaaFactory: ownerFeeShare mustn't exceed minimum");
        require(newOwnerFeeShare <= OWNER_FEE_SHARE_MAX, "AnkaaFactory: ownerFeeShare mustn't exceed maximum");
    ...
    function setReferrerFeeShare(address referrer, uint referrerFeeShare) external onlyOwner {
        require(referrer != address(0), "AnkaaFactory: zero address");
        require(referrerFeeShare <= REFERER_FEE_SHARE_MAX, "AnkaaFactory: referrerFeeShare mustn't exceed maximum");
