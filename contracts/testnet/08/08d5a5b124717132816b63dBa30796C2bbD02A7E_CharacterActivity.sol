...
//VIEWS
    ...
    {
        health = getCharactersHealth(tokenID);                                           // Create local variable
        Activity storage activity = charactersActivity[tokenID];                         // Shortcut to characters activity
        if (activity.active) {
                uint256 blockSinceStartOfActivity = block.number - activity.startBlock;  // Calculate blocks since activity started
                health -= blockSinceStartOfActivity / activity.activityDuration;         // Subtract health used is # of blocks since start of activity / # of Blocks to consume One Health stat
        } else {                                                                         // If not active
            uint256 maxHealth = getCharactersMaxHealth(tokenID);                         // Get characters max health
            if (block.number > activity.endBlock && activity.endBlock > 0) {             // Ensure block numbers higher than end of the activity to avoid divide by zero issues
                uint256 blockSinceLastActivity = block.number - activity.endBlock;       // Calculate blocks since last activity finished
                health += blockSinceLastActivity / activity.activityDuration;            // Store health + health regenerated is # of blocks since last activity / # of Blocks To Regenerate One Health stat
            }
            if (health > maxHealth) {                                                    // Ensure new enrgy amount doesn;t exceed max hhealth
                return maxHealth;                                                        // otherwise return the max health
            }
        }
    }
