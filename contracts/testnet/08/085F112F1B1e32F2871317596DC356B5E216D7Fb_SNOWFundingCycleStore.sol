...
import './abstract/SNOWControllerUtility.sol';
...
/** 
  ...
  @dev
  Adheres to -
  ISNOWTokenStore: General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.

  @dev
  Inherits from -
  SNOWControllerUtility: Includes convenience functionality for checking if the message sender is the current controller of the project whose data is being manipulated.
*/
contract SNOWFundingCycleStore is SNOWControllerUtility, ISNOWFundingCycleStore {
  //*********************************************************************//
  // --------------------------- custom errors ------------------------- //
  //*********************************************************************//
  ...
  */
  function get(uint256 _projectId, uint256 _configuration)
    ...
    view
    override
    returns (SNOWFundingCycle memory fundingCycle)
  {
  ...
  */
    ...
    returns (SNOWFundingCycle memory fundingCycle)
  {
    ...
    SNOWFundingCycle memory _fundingCycle;
  ...
  */
  function currentBallotStateOf(uint256 _projectId) external view override returns (SNOWBallotState) {
    // Get a reference to the latest funding cycle configuration.
    uint256 _fundingCycleConfiguration = latestConfigurationOf[_projectId];

    // Resolve the funding cycle for the latest configuration.
    SNOWFundingCycle memory _fundingCycle = _getStructFor(_projectId, _fundingCycleConfiguration);
  ...
  */
  ...
  constructor(ISNOWDirectory _directory) SNOWControllerUtility(_directory) {}
  ...
  */
  function configureFor(
  ...
  ) external override onlyController(_projectId) returns (SNOWFundingCycle memory) {
    ...
    if (_data.discountRate > SNOWConstants.MAX_DISCOUNT_RATE) revert INVALID_DISCOUNT_RATE();
    ...
    if(_data.ballot != ISNOWFundingCycleBallot(address(0))) {
      ...
      try _data.ballot.supportsInterface(type(ISNOWFundingCycleBallot).interfaceId) returns (bool _supports) {
    ...
    if (
      _data.ballot != ISNOWFundingCycleBallot(address(0)) ||
      _data.duration > 0 ||
  ...
  */
  function _configureIntrinsicPropertiesFor(
    ...
    SNOWFundingCycle memory _baseFundingCycle = _getStructFor(_projectId, _currentConfiguration);
    ...
    uint256 _timestampAfterBallot = _baseFundingCycle.ballot == ISNOWFundingCycleBallot(address(0))
      ? 0
  ...
  */
  function _initFor(
    uint256 _projectId,
    SNOWFundingCycle memory _baseFundingCycle,
    uint256 _configuration,
    uint256 _mustStartAtOrAfter,
    uint256 _weight
  ) private {
  ...
  */
  function _standbyOf(uint256 _projectId) private view returns (uint256 configuration) {
    ...
    SNOWFundingCycle memory _fundingCycle = _getStructFor(_projectId, configuration);
    ...
    SNOWFundingCycle memory _baseFundingCycle = _getStructFor(_projectId, _fundingCycle.basedOn);
  ...
  */
  function _mockFundingCycleBasedOn(SNOWFundingCycle memory _baseFundingCycle, bool _allowMidCycle)
    private
    view
  ...
  */
  function _deriveStartFrom(SNOWFundingCycle memory _baseFundingCycle, uint256 _mustStartAtOrAfter)
    private
    pure
  ...
  */
  function _deriveWeightFrom(SNOWFundingCycle memory _baseFundingCycle, uint256 _start)
    private
    pure
    ...
    if (_baseFundingCycle.duration == 0)
      return
        PRBMath.mulDiv(
          ...
          SNOWConstants.MAX_DISCOUNT_RATE - _baseFundingCycle.discountRate,
          SNOWConstants.MAX_DISCOUNT_RATE
        );
  ...
  */
  function _deriveNumberFrom(SNOWFundingCycle memory _baseFundingCycle, uint256 _start)
    private
    pure
    returns (uint256)
  {
  ...
  */
  function _isApproved(uint256 _projectId, SNOWFundingCycle memory _fundingCycle)
    private
    view
    returns (bool)
  {
  ...
  */
  ...
  ) private view returns (SNOWBallotState) {
    // If there is no ballot funding cycle, implicitly approve.
    if (_ballotFundingCycleConfiguration == 0) return SNOWBallotState.Approved;

    // Get the ballot funding cycle.
    SNOWFundingCycle memory _ballotFundingCycle = _getStructFor(
      _projectId,
  ...
  */
    ...
    returns (SNOWFundingCycle memory fundingCycle)
  {
    ...
    fundingCycle.ballot = ISNOWFundingCycleBallot(address(uint160(_packedUserProperties)));
