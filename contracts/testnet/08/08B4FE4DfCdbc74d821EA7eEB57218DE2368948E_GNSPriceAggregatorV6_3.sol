...
contract GNSPriceAggregatorV6_3 is ChainlinkClient, TWAPPriceGetter {
    ...
    constructor(
        address _linkToken,
        IUniswapV3Pool _tokenDaiLp, // 目前推測是用作經濟模型 staking 相關
        uint32 _twapInterval,
        StorageInterfaceV5 _storageT,
        PairsStorageInterfaceV6 _pairsStorage,
        ChainlinkFeedInterfaceV5 _linkPriceFeed, // 獲取 link price 價格，用作計算 link 費用
        uint _minAnswers,
    ...
    // temp set tokenPriceDaiValue
    function updateTokenPriceDaiValue(uint value) external onlyGov {
        _updateTokenPriceDaiValue(value);
    }
    ...
    function fulfill(
        ...
        || (price >= feedPrice ?
            ...
            if(answers.length == minAnswers){
                CallbacksInterfaceV6.AggregatorAnswer memory a;
...
abstract contract TWAPPriceGetter {
    ...
    // temp dai price for test
    uint public tokenPriceDaiValue;
    ...
    // temp set tokenPriceDai function
    function _updateTokenPriceDaiValue(uint value) internal {
    tokenPriceDaiValue = value;
    }
