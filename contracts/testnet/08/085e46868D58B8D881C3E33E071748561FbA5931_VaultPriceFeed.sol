...
contract VaultPriceFeed is IVaultPriceFeed, BaseConstants, BaseAccess {
    ...
    mapping(address => bool) public isSupportFastPrice;

    event SetTokenConfig(address indexed token, address fastPriceFeed, uint256 priceDecimals);
    event SetSupportFastPrice(address token, bool isSupportFastPrice);
    event SetTokenAggregator(address indexed token, address aggreagator);

    function setSupportFastPrice(address _token, bool _isSupport) external onlyOwner {
        isSupportFastPrice[_token] = _isSupport;
        emit SetSupportFastPrice(_token, _isSupport);
    }
    ...
    */
    ...
    function _isSupportFastPrice(address _token) internal view returns (bool) {
        return isSupportFastPrice[_token] && fastPriceFeeds[_token] != address(0);
    }
}
