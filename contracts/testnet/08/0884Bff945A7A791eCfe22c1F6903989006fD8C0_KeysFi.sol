...
contract KeysFi is IKeysFiforFriend {
    ...
    function addKeys(address subject, address from, uint256 amount) private {
        ...
        keysBalance[subject][from] += amount;
    }

    function removeKeys(address subject, uint256 amount) private {
        uint ownAmout = keysBalance[subject][msg.sender];
        require(
    ...
    ***return 
    ...
    function getSubjectData(
        uint id, //user id
        address subject //user address
    ) external view returns (User[] memory, uint[] memory, uint, uint, uint) {
        if (id > 0) {
            subject = frindeContract.getAddressByCode(id);
        }
        User[] memory _holders = new User[](51);
        uint[] memory _keys = new uint[](51);
        for (uint i = 0; i < holders_length[subject]; i++) {
            _holders[i] = frindeContract.getUserFrom(holders[subject][i]);
            _keys[i] = keysBalance[subject][holders[subject][i]];
            if (i == 50) break;
        }
        ...
        return (
            _holders,
            _keys,
            holders_length[subject],
            holding_length[subject],
            keysBalance[subject][subject]
        );
    }

    function getHolding() external view returns (User[] memory, uint[] memory) {
        User[] memory _holding = new User[](51);
        uint[] memory _keys = new uint[](51);
        for (uint i = 0; i < holding_length[msg.sender]; i++) {
            _holding[i] = frindeContract.getUserFrom(holding[msg.sender][i]);
            _keys[i] = keysBalance[holding[msg.sender][i]][msg.sender];
            if (i == 50) break;
        }
        return (_holding, _keys);
    }
    ...
    **returns
    ...
    function getTowPrice(
        ...
        return (getBuyPrice(subject, 1), getSellPrice(subject, 1));
    }
