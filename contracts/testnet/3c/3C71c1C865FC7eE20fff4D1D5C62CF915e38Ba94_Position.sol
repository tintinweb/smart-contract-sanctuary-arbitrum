...
contract Position is Roles, IPosition {
    bytes32 private constant EDIT_POSITION = keccak256("EDIT_POSITION");
    ...
    function addPositionWithMaster(
        ...
        uint256 percentTp,
        uint256 percentSl
    ) external override hasRole(EDIT_POSITION) returns (bytes32) {
        require(
    ...
    function updatePosition(
        bytes32 id,
        Types.Position calldata position
    ) external override hasRole(EDIT_POSITION) {
    ...
    function updatePositionWithMaster(
        ...
        external
        override
        hasRole(EDIT_POSITION)
        returns (address user, address master, int256 amount, int256 sizeChange)
    {
        ...
        int256 amountChange;
        require(
            position.leverage != masterPosition.leverage ||
                position.entryPrice != masterPosition.entryPrice,
            "!updated"
        );
        if (masterPosition.entryPrice == position.entryPrice) {
            ...
            amountChange =
                ((int256(position.amount) *
                    (int256(position.entryPrice) - currentPrice)) /
                    (int256(masterPosition.entryPrice) - currentPrice)) -
                int256(position.amount);
            sizeChange = amountChange * int32(position.leverage);
            position.entryPrice = masterPosition.entryPrice;
        }
        position.amount = uint256(int256(position.amount) + amountChange);
        _updatePosition(id, position);
        return (position.owner, masterPosition.owner, amountChange, sizeChange);
    }
    ...
    function closePosition(
        ...
        override
        hasRole(EDIT_POSITION)
        returns (address masterAddress, int256 amountShare)
    {
        ...
        if (master.sharePercent > 0) {
            amountShare =
                ((finalAmount - int256(positions[id].amount)) *
                    int256(master.sharePercent)) /
                int256(BPS);
        }
    ...
    function getPosition(
        bytes32 id
    ) external view override returns (Types.Position memory) {
        return positions[id];
    }

    function getMasterInfo(
        bytes32 id
    ) external view override returns (Master memory) {
        return masterInfo[id];
    }

    function getPositions(
        bytes32[] calldata id
    ) external view override returns (Types.Position[] memory _positions) {
    ...
    function getIOs(
        uint16 pairId
    ) external view override returns (IPosition.OI memory) {
        return OIs[pairId];
    }
