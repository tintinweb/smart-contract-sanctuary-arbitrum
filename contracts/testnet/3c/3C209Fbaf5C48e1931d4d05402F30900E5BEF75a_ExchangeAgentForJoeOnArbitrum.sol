...
contract ExchangeAgentForJoeOnArbitrum is Owned {
    ...
    constructor(address _weth, address _usdc, address _synth, IJoeRouter02 _dexRouter, address _owner) Owned(_owner) {
        WETH = _weth;
        USDC = _usdc;
        SYNTH = _synth;
    ...
    function swapTokenToSYNTH(address _tokenIn, uint256 _amountIn, address _to) external returns (uint256) {
        require(_to != address(0), "ExchangeAgent: zero recipient address");
    ...
    function _swapETHToToken(uint256 _amountIn, address _tokenOut, address _to) internal returns (bool, uint256) {
