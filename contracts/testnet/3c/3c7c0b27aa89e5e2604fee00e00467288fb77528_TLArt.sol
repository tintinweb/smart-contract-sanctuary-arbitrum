...
import "../shared/SSTORE2.sol";
import "../interfaces/ITLArt.sol";
import "./TLArtState.sol";
import "../interfaces/ITLDescriptor.sol";

contract TLArt is ITLArt, TLArtState {

	function initialize() external initializer {
		TLArtState.__TLArtState_init();
	}
	...
	function putTrait(string memory _traitType, 
					  ...
					  uint256 _traitId,
					  string memory _encodedBytes)
		override
		external
	{
		address pointer = SSTORE2.write(bytes(_encodedBytes));
		string memory key = string(abi.encodePacked(_traitType, "#", _traitName));
		traitToPointerMap[key] = pointer;
		traits[_traitType].push(TraitInfo(_traitId, _traitName));
	}

	function getTrait(string memory key)
		override
		external
		view
		returns (SVGUtils.SVGPart memory res)
	{
		// string memory key = string(abi.encodePacked(_traitType, "#", _traitName));
		address pointer = traitToPointerMap[key];
		bytes memory data = SSTORE2.read(pointer);
		// string memory result = bytesToString(data);

		SVGUtils.SVGPart memory part = SVGUtils.SVGPart(data,data);
		return part;
	}

	function getTrait(string memory _traitType,
					  uint256 _traitId)
		override
		external
		view
		returns (SVGUtils.SVGPart memory res)
	{
		TraitInfo[] memory arr = traits[_traitType];
		string memory name;
		for (uint i = 0; i < arr.length; i++) {
			if (arr[i].id == _traitId) {
				name = arr[i].name;
				break;
			}
		}

		string memory key = string(abi.encodePacked(_traitType, "#", name));
		address pointer = traitToPointerMap[key];
		bytes memory data = SSTORE2.read(pointer);
		SVGUtils.SVGPart memory part = SVGUtils.SVGPart(data,data);
		return part;
	...
	function putBackground(string memory _name, 
						   string memory _bytes)
		override
		external 
	{
		address pointer = SSTORE2.write(bytes(_bytes));
		// bytes memory svgAsBytes = bytes(svg);
		// string memory key = string(abi.encodePacked(_traitType, "#", _traitName));
		backgrounds[_name] = pointer;
	}

	function getBackground(string memory _name)
		override
		external 
		view 
		returns (string memory result)
	{
		address pointer = backgrounds[_name];
		bytes memory data = SSTORE2.read(pointer);

		return string(data);
	}

	function getTraitsFromType(string memory _type)
		external
		view
		returns (TraitInfo[] memory)
	{
...
abstract contract TLArtState is Initializable, AdminableUpgradeable {
	//Events 
	...
	//Errors
		...
		Store
		Trait -> Bytes
		"background#blue, pointer/address"
	*/

	mapping(string => address) internal traitToPointerMap;
	mapping(string => address) internal backgrounds;

	//trying this
	mapping(string => TraitInfo[]) internal traits;


	function __TLArtState_init() internal initializer {
