    ...
    WhitelistedTokens
{
    ...
    bytes32 private constant CREATE_FARM_HASH = keccak256("CreateFarm");
    /*//////////////////////////////////////////////////////////////
                            STATE VARIABLES
    //////////////////////////////////////////////////////////////*/
    ...
    // max amount which can be fundraised by the manager per Single Farm
    ...
    // percentage of fees from the profits of the df to the protocol (default - 5e18 (5%))
    uint256 private protocolFee;
    // fee from the manager to pay for operator transaction (default - 1e9)
    ...
    /// @param _capacityPerFarm max amount which can be fundraised by the manager per df
    /// @param _minInvestmentAmount min investment amount per investor per df
    /// @param _maxInvestmentAmount max investment amount per investor per df
    /// @param _maxLeverage max leverage which can be used by the manager when creating an df
    /// @param _usdc USDC contract address
    /// @param _admin address used by the bot to close/cancel Dfs
    ...
    function createFarm(
        ...
        require(!operators[_operator], "the operator already exists");
        operators[_operator] = true;

        // checking the signature is correct
        address _signer = getCreateFarmDigest(_operator, msg.sender)
            .toEthSignedMessageHash().recover(_signature);
        if(_signer != maker) revert InvalidSignature(maker, _signer);
