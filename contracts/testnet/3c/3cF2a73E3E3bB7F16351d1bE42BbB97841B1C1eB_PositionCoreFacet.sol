...
import {IPositionCore} from "../interfaces/IPositionCore.sol";
...
import {InvalidStrategy, InvalidParam} from "../errors/GenericErrors.sol";
import {LibAccessControlEnumerable} from "../libraries/LibAccessControlEnumerable.sol";

/**
 * @title DEDERI PositionCore
 * @author dederi
 * @notice This contract is PositionCore 涵盖创建和修改仓位数据.
 */
contract PositionCoreFacet is IPositionCore, ReentrancyGuard {
    using LibPositionCore for LibPositionCore.Layout;
    using LibStrategyConfig for LibStrategyConfig.Layout;

    function setStrategyNft(address _strategyNft) external {
        LibAccessControlEnumerable.checkRole(Constants.ADMIN_ROLE);
        LibPositionCore.Layout storage l = LibPositionCore.layout();
        if (_strategyNft == address(0)) {
            revert InvalidParam();
        }
        l.strategyNFT = IStrategyNFT(_strategyNft);
    }

    /// @dev Returns an Future Position struct of a given positionId
    function getFuturePosition(uint256 positionId) external view returns (StrategyTypes.Future memory) {
        return LibPositionCore.layout().futurePositions[positionId];
    }

    /// @dev Returns an Option Position struct of a given positionId
    function getOptionPosition(uint256 positionId) external view returns (StrategyTypes.Option memory) {
        return LibPositionCore.layout().optionPositions[positionId];
    }

    /// @dev Returns an Strategy struct of a given strategyId
    function getStrategy(uint256 strategyId) external view returns (StrategyTypes.StrategyData memory) {
        return LibPositionCore._getStrategy(strategyId);
    }

    //    /// @dev Returns an array of Strategy structs given an array of strategyIds
    //    function getStrategies(uint256[] memory strategyIds) external view returns (StrategyTypes.StrategyData[] memory) {
    //        uint256 strategyLen = strategyIds.length;
    //
    //        StrategyTypes.StrategyData[] memory result = new StrategyTypes.StrategyData[](strategyLen);
    //        for (uint256 i; i < strategyLen; ) {
    //            result[i] = LibPositionCore.layout().strategies[strategyIds[i]];
    //            unchecked {
    //                ++i;
    //            }
    //        }
    ...
    /// @dev Returns an Position struct of a given positionId
    function getPosition(uint256 positionId) external view returns (StrategyTypes.PositionData memory) {
        return LibPositionCore.layout().positions[positionId];
    }

    function getStrategyWithOwner(
        uint256 strategyId
    ) external view returns (StrategyTypes.StrategyDataWithOwner memory) {
        return LibPositionCore._getStrategyWithOwner(strategyId);
    }

    function getStrategyAllData(
        uint256 strategyId
    ) external view returns (StrategyTypes.StrategyAllData memory strategyData) {
        return LibPositionCore._getStrategyAllData(strategyId);
    }

    // @notice 使用StrategyAllData结构体验证策略有效性
    function verifyStrategyById(uint256 strategyId, bytes32 strategyHash) external view {
        StrategyTypes.StrategyAllData memory data = this.getStrategyAllData(strategyId);
        bytes32 structHash = keccak256(abi.encode(data));
        if (strategyHash != structHash) {
...
interface IPositionCore {
    ...
    //   function getPositions(uint256[] memory positionIds) external view returns (StrategyTypes.PositionData[] memory);

    function getFuturePosition(uint256 positionId) external view returns (StrategyTypes.Future memory);

    //    function getFuturePositions(uint256[] memory positionIds) external view returns (StrategyTypes.Future[] memory);

    function getOptionPosition(uint256 positionId) external view returns (StrategyTypes.Option memory);

    //    function getOptionPositions(uint256[] memory positionIds) external view returns (StrategyTypes.Option[] memory);
    ...
    //    function getStrategiesWithOwner(
    ...
    //    //////////
    //
    //    function createStrategy(StrategyTypes.StrategyRequest memory strategy, address receipt) external;
    //
    //    function createAndMerge(StrategyTypes.Strategy memory requestParam) external returns (bool);
    //
    //    function increaseStrategyCollateral(uint256 tokenId, StrategyTypes.CollateralInfo[] memory newCollaterals) external;
    //
    //    function decreaseStrategyCollateral(
    //        uint256 tokenId,
    //        StrategyTypes.CollateralInfo[] memory newCollaterals
    //    ) external returns (StrategyTypes.CollateralInfo[] memory);
    //
    //    function splitStrategy(StrategyTypes.SpiltStrategyRequest memory requestParam) external;
    //
    //    function mergeStrategies(StrategyTypes.MergeStrategyRequest memory requestParam) external returns (bool);
    //
    //    function increasePositions(uint256 tokenId, StrategyTypes.Strategy memory strategy) external;
    //
    //    function decreasePositions(uint256 tokenId, uint256[] memory positionIds) external;
    //
    //    function settleExpiring(uint256 tokenId) external;
    //
    //    function settleExpirings(uint256[] memory tokenIds) external;
}
