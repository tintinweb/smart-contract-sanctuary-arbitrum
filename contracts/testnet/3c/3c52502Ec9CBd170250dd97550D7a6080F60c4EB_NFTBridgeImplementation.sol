...
contract NFTBridge is NFTBridgeGovernance {
    ...
    // 订单号,Token地址,TokenId,接收链Id，发送人，接收人
    ...
    // 订单号,源链Token地址，新token地址，tokenId，源链Id，发送链Id，接收者地址
    ...
    function logTransfer(NFTBridgeStructs.Transfer memory transfer, uint256 callValue) internal returns (uint64 sequence) {
        ...
        sequence = zkBridge().publishMessage{
        value : callValue
        }(encoded);
    }
    ...
    function completeTransferBatch(bytes memory proofBlob, bytes32[] memory merkleProof) public {
        _completeTransferBatch(proofBlob, merkleProof);
    }

    // Execute a Transfer message
    function _completeTransfer(bytes memory proofBlob, bytes32[] memory merkleProof) internal {
        (IZKBridge.VM memory vm, bool valid, string memory reason) = zkBridge().parseAndVerifyProof(proofBlob, merkleProof);
        ...
        emit ReceiveNFT(vm.sequence, address(uint160(uint256(transfer.tokenAddress))), address(transferToken), transfer.tokenID, transfer.tokenChain, vm.emitterChainId, transferRecipient);
    }

    function _completeTransferBatch(bytes memory proofBlob, bytes32[] memory merkleProof) internal {
        (IZKBridge.VM memory vm, bool valid, string memory reason) = zkBridge().parseAndVerifyProof(proofBlob, merkleProof);
        ...
        for (uint256 i = 0; i < transfer.tokenAddress.length; i++) {
            ...
            if (transfer.tokenChain[i] != chainId()) {
                if (transfer.tokenChain[i] == 1) {
                    // Cache SPL token info which otherwise would get lost
                    setSplCache(transfer.tokenID[i], NFTBridgeStorage.SPLCache({
                    name : transfer.name[i],
                    symbol : transfer.symbol[i]
                    }));
                }
            ...
            emit ReceiveNFT(vm.sequence, address(uint160(uint256(transfer.tokenAddress[i]))), address(transferToken), transfer.tokenID[i], transfer.tokenChain[i], vm.emitterChainId, transferRecipient);
        }
    ...
    function verifyBridgeVM(IZKBridge.VM memory vm) internal view returns (bool){
    ...
    function parseTransferBatch(bytes memory encoded) public pure returns (NFTBridgeStructs.TransferBatch memory transfer) {
        //        (bytes32[] memory tokenAddress,
        //        uint16[] memory tokenChain,
        //        bytes32[] memory symbol,
        //        bytes32[] memory name,
        //        uint256[] memory tokenID,
        //        string[] memory uri,
        //        bytes32  to,
        //        uint16  toChain) = abi.decode(encoded,(bytes32[],uint16[] ,bytes32[] ,bytes32[] ,uint256[] ,string[] ,bytes32 ,uint16));
