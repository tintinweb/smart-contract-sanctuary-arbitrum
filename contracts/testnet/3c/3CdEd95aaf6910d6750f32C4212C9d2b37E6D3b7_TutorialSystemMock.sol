...
contract CheckpointComponent is BaseStorageComponent {
    ...
    function getBytes(
        ...
        Layout memory s = CheckpointComponentStorage.layout().entityIdToStruct[entity];
        value = abi.encode(s.numOfCheckpoints, s.checkpointPosition, s.groupId);
    }
    ...
    function setBytes(
        ...
        Layout memory s = CheckpointComponentStorage.layout().entityIdToStruct[entity];
        (s.numOfCheckpoints, s.checkpointPosition, s.groupId) = abi.decode(value, (uint64, uint64, uint256));
