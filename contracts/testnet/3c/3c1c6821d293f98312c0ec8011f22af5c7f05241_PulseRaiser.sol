...
contract PulseRaiser is IPulseRaiser, Normalizer, ClaimTracker {
    ...
    uint256 public constant PERIOD_SECONDS = 720;
    ...
    function modifyPriceBase(uint8 dayIndex_, uint16 priceBase_) external {
        ...
        encodedpp[encodedppIndex] = _encodePriceBasesMemory(priceBases);
    ...
    //     }
    //     uint8 encodedppIndex = (dayIndex_ < 25)? 0 : 1;
    //     encodedpp[encodedppIndex] = _encodePriceBases(priceBases);
    ...
    function _splitPriceBases(uint8 encodedppIndex) private view returns (uint16[] memory) {
        uint16[] memory split = new uint16[](PERIODS);
        for (uint8 dayIndex = 0; dayIndex < PERIODS; dayIndex++) {
            split[dayIndex] = uint16(
                (encodedpp[encodedppIndex] >> (dayIndex * 10)) & LOWEST_10_BITS_MASK
            );
        }
        return split;
    }
