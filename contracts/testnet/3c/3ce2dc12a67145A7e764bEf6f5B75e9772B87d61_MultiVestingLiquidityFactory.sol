...
import "./lib/SafeERC20.sol";
import "./bond/MultiVestingLiquidity.sol";
import "./bond/LiquidityTreasury.sol";

contract MultiVestingLiquidityFactory is Ownable {
    /* ======== STATE VARIABLS ======== */
    ...
    /**
     * @notice deploys liquidity treasury and bond contracts and returns address of both
     ...
     */
    function createLiquidityBondAndTreasury(
        address _payoutToken,
        address _payinToken,
        address _initialOwner,
        address _affiliate,
        ...
        LiquidityTreasury treasury = new LiquidityTreasury(_payoutToken, _initialOwner);
        MultiVestingLiquidity bond =
        new MultiVestingLiquidity(address(treasury), _payoutToken, _payinToken, _initialOwner, MizuRegistry, _affiliate, _quoteToken, _router, _receiver);

        return IRegistry(MizuRegistry).pushBond(
            _payoutToken, _payinToken, address(treasury), address(bond), _initialOwner, address(this)
        );
    }

    /**
     * @notice deploys liquidity bond contract for an existing treasury and returns address of both
     *     @dev bond and treasury need to have the same payoutToken
     *     @param _payoutToken address the token paid out from the bond
     *     @param _payinToken address the token used to open a bond position and add liquidity
     *     @param _customTreasury address of the already deployed treasury
     *     @param _initialOwner address address of the admin of the bond and treasury
     *     @param _affiliate address need to be whitelisted by Mizu
     *     @param _quoteToken address used to calculate payin/payout ratio (I.E payin > WETH > payout)
     *     @param _router the router to get token prices from, used to calculate bond price
    ...
    function createLiquidityBond(
        ...
        MultiVestingLiquidity bond =
        new MultiVestingLiquidity(_customTreasury, _payoutToken, _payinToken, _initialOwner, MizuRegistry, _affiliate, _quoteToken, _router, _receiver);

        return IRegistry(MizuRegistry).pushBond(
            _payoutToken, _payinToken, _customTreasury, address(bond), _initialOwner, address(this)
        );
    }
