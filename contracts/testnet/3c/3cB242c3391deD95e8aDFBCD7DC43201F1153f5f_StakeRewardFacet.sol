...
library LibStakeReward {
    ...
    function checkOncePerBlock(address user) internal {
        StakeRewardStorage storage st = stakeRewardStorage();
        require(st.lastBlockNumberCalled[user] < Constants.arbSys.arbBlockNumber(), "once per block");
        st.lastBlockNumberCalled[user] = Constants.arbSys.arbBlockNumber();
    }
...
library LibApxReward {
    ...
    function apxPoolInfo() internal view returns (IApxReward.ApxPoolInfo memory poolInfo) {
        ...
        uint256 apxReward;
        if (Constants.arbSys.arbBlockNumber() > pool.lastRewardBlock && pool.totalStaked != 0) {
            uint256 blockGap = Constants.arbSys.arbBlockNumber() - pool.lastRewardBlock;
    ...
    function pendingApx(address _user) internal view returns (uint256) {
        ...
        if (Constants.arbSys.arbBlockNumber() > pool.lastRewardBlock && lpSupply != 0) {
            uint256 blockGap = Constants.arbSys.arbBlockNumber() - pool.lastRewardBlock;
    ...
    function stake(uint256 _amount) internal {
        ApxRewardStorage storage st = apxRewardStorage();
        require(_amount > 0, 'Invalid amount');
        require(Constants.arbSys.arbBlockNumber() >= st.startBlock, "Mining not started yet");
        IApxReward.ApxPoolInfo storage pool = st.poolInfo;
    ...
    function updatePool() internal {
        ApxRewardStorage storage st = apxRewardStorage();
        IApxReward.ApxPoolInfo storage pool = st.poolInfo;
        if (Constants.arbSys.arbBlockNumber() <= pool.lastRewardBlock) {
            return;
        }
        uint256 lpSupply = pool.totalStaked;
        if (lpSupply == 0) {
            pool.lastRewardBlock = Constants.arbSys.arbBlockNumber();
            return;
        }
        uint256 blockGap = Constants.arbSys.arbBlockNumber() - pool.lastRewardBlock;
        uint256 apxReward = blockGap * pool.apxPerBlock;
