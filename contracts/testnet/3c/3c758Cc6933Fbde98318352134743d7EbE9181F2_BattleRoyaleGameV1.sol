...
contract BattleRoyaleGameV1 is IBattleRoyaleGameV1, Pausable, Ownable, ERC721Holder, ReentrancyGuard  {
    ...
    function forceRemovePlayers(address[] calldata playersAddress) external nonReentrant checkEndGame {
        ...
        battleToken.mint(msg.sender, uint(config.forceRemoveRewardToken) * playersAddress.length * (1 ether));
    ...
    function playerSpawnPos(address playerAddress) public view returns(uint playerPos) {
        if (state.status != GameConstants.GAME_STATUS_RUNNING) {
            return 0;
        }
