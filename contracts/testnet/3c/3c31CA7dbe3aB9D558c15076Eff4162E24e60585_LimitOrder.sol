...
import {Module, OrderType, ModuleData, TxItem, UserInfoItem, TokenInfoItem, TokenInfoItem2, LimitItem, TccItemV2, TcdItem, TaskConfig, BalanceItem, TokenItem, FeeItem, FeeItem2, SwapTokenItem, SwapEventItem, OrderEventItem, CreateItem, GasItem, GasItemList, ExactInputParams} from "./StructList.sol";
import {MultiDexRouter} from "./IMultiDexRouter.sol";
import {OrderBase} from "./OrderBase.sol";
import {IWETH} from "./IWETH.sol";

contract LimitOrder is OrderBase {
    ...
    function createLimitTask(TccItemV2[] calldata _tccList) external nonReentrant payable {
        uint256 _ethAmount = 0;
        address _user = msg.sender;
        uint256 _num = _tccList.length;
        for (uint256 i = 0; i < _num; i++) {
            TccItemV2 memory _tcc = _tccList[i];
            CreateItem memory _y = new CreateItem[](1)[0];
            require(_tcc._type == OrderType.LimitOrder, "L01");
            require(!taskNameList[_tcc._taskName], "L02");
            taskNameList[_tcc._taskName] = true;
            _y.md5 = keccak256(abi.encodePacked(_tcc._taskName));
            require(!md5List[_y.md5], "L03");
            md5List[_y.md5] = true;
            _y.execData = abi.encodeCall(this.limitOrder, (_user, _y.md5));
            ...
            require(!taskIdStatusList[_y.taskId], "L04");
            ...
            if (_y.swapInToken != weth) {
                uint256 _amountB = IERC20(_y.swapInToken).balanceOf(address(this));
                IERC20(_y.swapInToken).transferFrom(_user, address(this), _tcc._maxSpendTokenAmount);
                uint256 _amountA = IERC20(_y.swapInToken).balanceOf(address(this));
                _maxSpendTokenAmount = _amountA.sub(_amountB);
                //                _y._taskConfig.tcc._limitItem._swapInAmount = _amountA.sub(_amountB);
                //                _y._taskConfig.tcc._limitItem._swapInAmountOld = _amountA.sub(_amountB);
            } else {
                IWETH(weth).deposit{value: _tcc._maxSpendTokenAmount}();
                _maxSpendTokenAmount = _tcc._maxSpendTokenAmount;
                _ethAmount = _ethAmount.add(_tcc._maxSpendTokenAmount);
                //                _y._taskConfig.tcc._limitItem._swapInAmount = _tcc._limitItem._swapInAmount;
                //                _y._taskConfig.tcc._limitItem._swapInAmountOld = _tcc._limitItem._swapInAmount;
                //_ethAmount = _ethAmount.add(_tcc._limitItem._swapInAmount);
            }
            //_ethAmount = _ethAmount.add(_tcc._maxFeePerTx);
            //userInfoList[_user].ethDepositAmount = userInfoList[_user].ethDepositAmount.add(_tcc._maxFeePerTx);
            //userInfoList[_user].ethAmount = userInfoList[_user].ethAmount.add(_tcc._maxFeePerTx);
            //_y._taskConfig.tcc._limitItem._minswapOutAmount = _tcc._limitItem._minswapOutAmount;
            //_y._taskConfig.tcc._limitItem._swapOutStandardAmount = _tcc._limitItem._swapOutStandardAmount;
            ...
            _y._taskConfig.tcd = TcdItem({
                ...
                _maxSpendTokenAmount: _maxSpendTokenAmount, //限价单用到
                _completed: false //限价单用到
            });
            taskConfigList[_y.taskId] = _y._taskConfig;
            userAllLimitOrderList[_user].add(_y.taskId);
            userActiveLimitOrderList[_user].add(_y.taskId);
        ...
        uint256 gasAmount = msg.value.sub(_ethAmount);
        userInfoList[_user].ethDepositAmount = userInfoList[_user].ethDepositAmount.add(gasAmount);
        userInfoList[_user].ethAmount = userInfoList[_user].ethAmount.add(gasAmount);
        // require(msg.value == _ethAmount, "L05");
    }


    struct limitOrderItem {
        bytes32 _taskId;
        address swapInToken;
    ...
    function limitOrder(address _user, bytes32 _md5) external nonReentrant {
        require(tx.gasprice <= gasPriceLimitForAutomate, "L06");
        require(msg.sender == dedicatedMsgSender || dedicatedMsgSenderList[msg.sender], "L07");
        limitOrderItem memory x = new limitOrderItem[](1)[0];
        x._taskId = md5TaskList[_md5];
        TaskConfig storage y = taskConfigList[x._taskId];
        require(y.tcc._type == OrderType.LimitOrder, "L08");
        require(y.tcd._status, "L09");
        require(block.timestamp >= y.tcc._start_end_Time[0] && block.timestamp <= y.tcc._start_end_Time[1], "L10");
        y.tcd._taskExTimes = y.tcd._taskExTimes.add(1);
        x.swapInToken = y.tcd._swapInToken;
        x.swapOutToken = y.tcd._swapOutToken;
        if (!y.tcc.isV3) {
            if (IERC20(x.swapInToken).allowance(address(this), multiDexRouterAddress) < y.tcd._maxSpendTokenAmount) {
                IERC20(x.swapInToken).approve(multiDexRouterAddress, approveAmount);
            }
        } else {
            if (IERC20(x.swapInToken).allowance(address(this), V3Dex) < y.tcd._maxSpendTokenAmount) {
                IERC20(x.swapInToken).approve(V3Dex, approveAmount);
            }
        }

        x._in = y.tcd._maxSpendTokenAmount;
        //        uint256 swapOutAmount;
        if (y.tcc._swapPathConfig1._swapPathList[y.tcc._swapPathConfig1._swapPathList.length - 1] == weth) {
            if (!y.tcc.isV3) {
                x.swapOutAmount = MultiDexRouter(multiDexRouterAddress).swapExactTokensForETHSupportingFeeOnTransferTokens(
                    y.tcc._swapPathConfig1._routerAddressListV2,
                    y.tcd._maxSpendTokenAmount,
                    0,
                ...
                x.swapOutAmount = MultiDexRouter(V3Dex).swapExactTokensForToken(
                    ExactInputParams({
                        ...
                        recipient: _user,
                        amountIn: y.tcd._maxSpendTokenAmount,
                        amountOutMinimum: 0,
                        sqrtPriceLimitX96: 0,
                        forceUseWETH: false
                    })
            ...
            if (!y.tcc.isV3) {
                x.swapOutAmount = MultiDexRouter(multiDexRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                    y.tcc._swapPathConfig1._routerAddressListV2,
                    y.tcd._maxSpendTokenAmount,
                    0,
                    y.tcc._swapPathConfig1._swapPathList,
        ...
        //成交在执行的价格区间
        require(x.swapOutAmount.mul(10 ** 18) > y.tcc._swapPathConfig1._swapPriceZone[0].mul(x._in) && x.swapOutAmount.mul(10 ** 18) < y.tcc._swapPathConfig1._swapPriceZone[1].mul(x._in), "P09");
        //        if (swapInToken == weth) {
        //            y.tcc._limitItem._swapOutAmount = MultiDexRouter(multiDexRouterAddress).swapExactETHForTokensSupportingFeeOnTransferTokens{value: y.tcc._limitItem._swapInAmount}(
        //                y.tcc._routerAddressList,
        //                y.tcc._limitItem._minswapOutAmount,
        //                y.tcc._swapRouter,
        //                _user,
        //                block.timestamp
        //            );
        ...
        //                y.tcc._limitItem._swapOutAmount = MultiDexRouter(multiDexRouterAddress).swapExactTokensForETHSupportingFeeOnTransferTokens(
        //                    y.tcc._routerAddressList,
        //                    y.tcc._limitItem._swapInAmount,
        //                    y.tcc._limitItem._minswapOutAmount,
        //                    y.tcc._swapRouter,
        //                    _user,
        //                    block.timestamp
        //                );
        //            } else {
        //                y.tcc._limitItem._swapOutAmount = MultiDexRouter(multiDexRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(
        //                    y.tcc._routerAddressList,
        //                    y.tcc._limitItem._swapInAmount,
        //                    y.tcc._limitItem._minswapOutAmount,
        //                    y.tcc._swapRouter,
        ...
        //            }
        //        }
        (x._fee, x._feeToken) = _getFeeDetails();
        x._poolFee = x._fee.mul(swapRate).div(swapAllRate);
        x._totalFee = x._fee.add(x._poolFee);
        require(x._totalFee <= y.tcc._maxFeePerTx, "L11");
        require(userInfoList[_user].ethAmount >= x._totalFee, "L12");
        _transfer(x._fee, x._feeToken);
        payable(usdtPoolAddress).transfer(x._poolFee);
        userInfoList[_user].ethAmount = userInfoList[_user].ethAmount.sub(x._totalFee);
        userInfoList[_user].ethUsedAmount = userInfoList[_user].ethUsedAmount.add(x._totalFee);
        //平台总费用累计
        totalFee = totalFee.add(x._totalFee);
        _cancelTask(x._taskId);
        SwapEventItem memory t = SwapEventItem(x.swapInToken, x.swapOutToken, IERC20(x.swapInToken).decimals(), IERC20(x.swapOutToken).decimals(), 0, 0, 0, y.tcd._maxSpendTokenAmount, 0, x.swapOutAmount);
        emit OrderEvent(block.number, block.timestamp, y.tcc._type, _user, x._taskId, tx.origin, x._totalFee, t);
        //        if (devToken != address(0) && devFee > 0) {
        //            require(userInfoList[_user].devAmount >= devFee, "L13");
        ...
        //     userInfoList[_user].devUsedAmount = userInfoList[_user].devUsedAmount.add(devFee);
        // }
        userActiveLimitOrderList[_user].remove(x._taskId);
        //orderInfoList[txAmount] = OrderEventItem(block.number, block.timestamp, y.tcc._type, _user, _taskId, tx.origin, _fee.add(_poolFee), t);
