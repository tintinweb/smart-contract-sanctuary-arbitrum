...
contract DungeonSystem is IDungeonSystem, GameRegistryConsumerUpgradeable {
    error DungeonNotAvailable(uint256 dungeon);
    error DungeonExpired(uint256 dungeon);
    error DungeonMapNotFound(uint256 dungeon);
    error DungeonAlreadyCompleted(uint256 dungeon);
    error DungeonNodeAlreadyCompleted(uint256 dungeon, uint256 node);
    error DungeonNodeOutOfOrder(uint256 dungeon, uint256 node);
    error DungeonNodePreviousNotCompleted(uint256 dungeon, uint256 node);
    error DungeonNodeNotStarted(uint256 dungeon, uint256 node);
    ...
    function getDungeonNode(
        uint256 dungeon,
        uint256 node
    ) public view override returns (DungeonNode memory) {
        return
    ...
    function endDungeonBattle(EndDungeonBattleParams calldata params) external {
        address account = _getPlayerAccount(_msgSender());
        // _validateEndDungeonBattle(account, params);
        ...
        DungeonNode memory node = getDungeonNode(
            params.dungeonEntity,
            params.node
        );
        ILootSystem(_getSystem(LOOT_SYSTEM_ID)).grantLoot(account, node.loots);
    }

    function _getDungeon(
    ...
    function _validateStartDungeonBattle(
        ...
        DungeonMap memory dungeon = _getDungeon(params.dungeonEntity);

        // Check that the dungeon exists.
        if (dungeon.nodes.length == 0) {
            revert DungeonMapNotFound(params.dungeonEntity);
        }

        // Check that the dungeon has already started.
        if (dungeon.startAt > block.timestamp) {
            revert DungeonNotAvailable(params.dungeonEntity);
        }

        // Check that the dungeon has not yet finished.
        // If they have already started the dungeon, they get a margin to finish.
        if (params.node == 0) {
            if (dungeon.endAt < block.timestamp) {
                revert DungeonExpired(params.dungeonEntity);
            }
        } else {
            if (
                (dungeon.endAt + getExtraTimeForDungeonCompletion() <
                    block.timestamp)
            ) {
                revert DungeonExpired(params.dungeonEntity);
            }
        ...
        if (nextNode != 0) {
            ...
            if (currentNode != nextNode - 1) {
                revert DungeonNodeOutOfOrder(params.dungeonEntity, nextNode);
            }
        ...
        if (
            ...
            revert DungeonNodeAlreadyCompleted(params.dungeonEntity, nextNode);
        }
    }
    ...
    function _validateEndDungeonBattle(
        ...
        if (
            dungeon.startAt > block.timestamp || // Start is in the future
            (dungeon.endAt + getExtraTimeForDungeonCompletion() <
        ...
        if (
            progressSystem.getCurrentNode(account, params.dungeonEntity) !=
            params.node
        ) {
            revert DungeonNodeOutOfOrder(params.dungeonEntity, params.node);
        }
        ...
        if (
            ...
            revert DungeonNodeNotStarted(params.dungeonEntity, params.node);
        }
...
struct DungeonMap {
    uint256 endAt; // epoch timestamp in seconds
    uint256 startAt; // epoch timestamp in seconds
    DungeonNode[] nodes;
}
...
interface IDungeonMapSystem {
    /**
     * @dev Set the dungeon map for a specified dungeon entity.
     * @param dungeon Entity Id of the dungeon to set.
    ...
    /**
     * @dev Get the dungeon map for a specified dungeon entity.
     * @param dungeon Entity Id of the dungeon to get.
     * @return DungeonMap
     */
    function getDungeonMap(
        uint256 dungeon
    ...
    /**
     * @dev Get the dungeon node for a specified dungeon
     * @param dungeon Entity Id of the dungeon to get.
    ...
    /**
     *
     * @param dungeon Entity id of daily dungeon
     ...
     * @param dungeon Entity id of daily dungeon
     * @param node Index of the dungeon node to get the enemy for
     * @return Loot[] Entity id of the enemy for the dungeon node
     */
...
interface IDungeonSystem {
     ...
     * @param dungeonEntity Id of the dungeon to preview.
