...
contract EllerianHeroUpgradeable is Ownable {
  ...
  address _signatureAddr, address _feesAddress, address _vrfAddress, address _signerAddr) external onlyOwner {
  ...
  */
  ...
  function SynchronizeHero (bytes memory _signature, uint256 _tokenId, uint256 _level, uint256 _exp) external {
    require (IERC721(minterAddress).ownerOf(_tokenId) == msg.sender, "22");
