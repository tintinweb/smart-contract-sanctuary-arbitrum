...
import "./AbstractOptions.sol";
import "../pools/ERCERCPhySettlPool.sol";

/**
 * @title an call option contract which underlier is a token
 * @notice attached pool need to have the same liquidity token
 */
contract PutOptionsERCERC is AbstractOptions {
    using SafeERC20 for ERC20;

    constructor(MGHPriceProvider pp, ILimitOrderProxy _limitOrderProxy, address _underlyingToken, address _settlementToken, SettlerProxy _settlerProxy, PricerProxy _pricerProxy, address payable _settlementFeeRecipient)
    AbstractOptions(pp, OptionType.Put, _limitOrderProxy, _underlyingToken, _settlementToken, new ERCERCPhySettlPool(_settlementToken, _underlyingToken), _settlerProxy, _pricerProxy, _settlementFeeRecipient) {}

    /**
     * @param premium as the raw premium price after calculation, before eventual rate conversion in currency token
     * @dev in that case msg.sender is the buyer of the option
     */
    function _transferPremium(uint256 premium) internal override {
        //        despite create is payable, in this cas we don't accept ether
        require(msg.value == 0, "option can't receive eth");
        //        require(pool.liquidityToken().allowance(msg.sender, address(this)) >= premium, 'allowance for the pool of liquidity token is not enough');
        require(settlementAsset.allowance(msg.sender, address(this)) >= premium, 'allowance for the pool of liquidity token is not enough');

        //        pool.liquidityToken().transferFrom(msg.sender, address(this), premium);
        settlementAsset.safeTransferFrom(msg.sender, address(this), premium);
    }
    
    /// @dev See {AbstractOptions}
    function _sendProtocolFees(uint256 protocolFee) internal override {
        pool.liquidityToken().safeTransfer(settlementFeeRecipient, protocolFee);
    }

    /// @dev See {AbstractOptions}
    function _isExerciseSwap(Option storage option) internal override returns (bool) {
        bool holderTransfered = CommonUtils.transferFromWithReturn(pool.settlementToken(), option.holder, address(pool), option.quantityWad);
        if (!holderTransfered) return false;

        bool poolTransfered = CommonUtils.transferFromWithReturn(pool.liquidityToken(), address(pool), option.holder, option.strikeWad * option.quantityWad / 10 ** 18);
        require(poolTransfered, "Pool transfer liquidity token failed");
        return true;
    }

    /// @dev See {AbstractOptions}
    function _isStrikeOk(Option storage option) internal override view returns (bool) {
        return option.strikeWad * (10 ** (18 - settlementAsset.decimals())) 
            >= underlyingAssetPriceIndex.latestAnswer();
    }

    //slither-disable-start similar-names
    /// @dev See {AbstractOptions}
    function _computeLimitOrderArgs(Option storage option) internal override view returns (uint256 makerAmount_, uint256 takerAmount_, LimitOrderType type_) {
        makerAmount_ = option.quantityWad;
        takerAmount_ = option.strikeWad * option.quantityWad / 1e18;
        type_ = LimitOrderType.Put;
    }
    //slither-disable-end similar-names

    /// @dev See {AbstractOptions}
    function _createOption(uint256 optionID, uint256 strikeWad, uint256 quantityWad, uint256 poolPremium, uint256 maturity) internal override {
        options[optionID].state = State.Active;
        options[optionID].holder = payable(msg.sender);
        options[optionID].strikeWad = strikeWad;
        options[optionID].quantityWad = quantityWad;
        options[optionID].premiumWad = poolPremium;
        options[optionID].maturity = block.timestamp + maturity;
        
        uint256 toProvide = strikeWad * quantityWad / 1e18;
        options[optionID].liquidityAmount = toProvide;
    }
}
...
contract ERCERCPhySettlPool is AbstractLiquidityPool {
    ...
    constructor(address _liquidityToken, address _settlementToken) AbstractLiquidityPool(_liquidityToken, _settlementToken){}
    ...
    function _provideLiquidity(uint256 quantity) override internal {
        ...
        _raiseLiquidityShares(quantity);
    }

    function _withdrawLiquidity(address provider, uint256 quantity) override internal {
        ...
        require(_lockedBalanceOf(provider) + quantity <= balanceOf(provider), "Not enough unlocked tokens");
