...
contract Common is ReentrancyGuard {
  ...
  function _transferToBankroll(address tokenAddress, uint256 amount) internal {
    uint256 amountToMarketing = (amount * marketingFee) / totalFee;
    uint256 amountToDev = (amount * devFee) / totalFee;
    uint256 amountToYield = (amount * yieldFee) / totalFee;
    uint256 amountToJackpot = amount - amountToMarketing - amountToDev - amountToYield;
    if (tokenAddress == address(0)) {
      (bool success, ) = payable(address(marketingWallet)).call{value: amountToMarketing}("");
      if (!success) {
        revert RefundFailed();
      }
      (success, ) = payable(address(devWallet)).call{value: amountToDev}("");
      if (!success) {
        revert RefundFailed();
      }
      (success, ) = payable(address(yieldWallet)).call{value: amountToYield}("");
      if (!success) {
        revert RefundFailed();
      }
      (success, ) = payable(address(Bankroll)).call{value: amountToJackpot}("");
      if (!success) {
        revert RefundFailed();
      }
    } else {
      ...
      IERC20(tokenAddress).safeTransfer(address(Bankroll), amountToJackpot);
    }
  }
  ...
  function getVRFFee() public view returns (uint256 fee) {
    fee = (IRandomizer(randomizer).estimateFee(2500000, 2) * 125) / 100;
  }
  ...
  function setFees(uint256 _marketingFee, uint256 _devFee, uint256 _yieldFee) external {
    if (msg.sender != Bankroll.owner()) {
      revert NotOwner(Bankroll.owner(), msg.sender);
    }
    ...
    devFee = _devFee;
    yieldFee = _yieldFee;
    jackpotFee = totalFee - marketingFee - devFee - yieldFee;
  }
 ...
 */
contract JackpotGame is Common {
  using SafeERC20 for IERC20;
  ...
  mapping(uint256 => address) vrfIDs;
  ...
  function jackpotPlay(address _tokenAddress, uint8[] memory _playedNumbers) external payable nonReentrant {
    ...
    if (!(_playedNumbers.length > 0 && _playedNumbers.length <= 5)) {
    ...
    if (fees[0] > fees[1] && data.feePayed > fees[0] - fees[1]) {
      IRandomizer(randomizer).clientWithdrawTo(msg.sender, ((data.feePayed - (fees[0] - fees[1])) * 90) / 100);
    }
    uint256 betPrice = Bankroll.getBetPrice(_tokenAddress);
    uint256 wager = betPrice * _playedNumbers.length;
    uint256 feePayed = _transferWager(_tokenAddress, wager);
