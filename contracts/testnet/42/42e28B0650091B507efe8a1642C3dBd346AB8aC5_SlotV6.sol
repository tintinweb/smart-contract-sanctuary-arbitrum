...
contract SlotV6 is Game {
    ...
    function _getPayout(uint256 betAmount) private pure returns (uint256) {
        return betAmount * 2; // 최대 10000배 (10000)
    }
    ...
    function spin(
        ...
        uint256 _totalInput = betUnit * uint256(rngCount);
    ...
    function _callback(
        ...
        uint8[3][] memory _reels = new uint8[3][](_randomNumbers.length);
        uint256[] memory _payouts = new uint256[](_randomNumbers.length);
        uint256 _totalPayout = 0;

        for (uint8 i = 0; i < _randomNumbers.length; i++) {
            uint8 r1 = reel[(_randomNumbers[i] % 100) % 36];
            uint8 r2 = reel[((_randomNumbers[i] % 10000) / 100) % 36];
            uint8 r3 = reel[((_randomNumbers[i] % 1000000) / 10000) % 36];
        ...
        emit RequestPaid({
            ...
            _reels: _reels,
            _payouts: _payouts,
            _totalPayout: resolvedPayout, //resolvedPayout
