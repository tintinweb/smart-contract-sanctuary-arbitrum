...
contract Vault is ReentrancyGuard, IVault {
    ...
    mapping(address => mapping( address =>uint256)) public override fundingRates;    
    ...
    // cumulativeFundingRates tracks the funding rates based on utilization (coll, idx) => rate
    ...
    function initialize(
        ...
        //must set funding factors after init        
    }
    ...
    function setError(uint256 _errorCode, string calldata _error) external override {
        require(msg.sender == errorController, "Vault: :(");
    ...
    function setFundingInterval(uint256 _fundingInterval) external override {
        _onlyGov();
        _validate(_fundingInterval >= MIN_FUNDING_RATE_INTERVAL, 10);    
        fundingInterval = _fundingInterval;
    }
    
    function setFundingRate(address _collateralToken, address _indexToken, uint256 _fundingRate) external override {
        //set Timelock and set as keeper role to Timelock
        _onlyGov();
        _validate(_fundingRate <= MAX_FUNDING_RATE_FACTOR, 11);
        updateCumulativeFundingRate(_collateralToken, _indexToken); //should update funding rate before set funding rate
        fundingRates[_collateralToken][_indexToken] = _fundingRate;
    }
    ...
    function getMaxPrice(address _token) public view override returns (uint256) {
        ...
        return vaultUtils.getMaxPrice(_token, priceFeed, includeAmmPrice, useSwapPricing);
    }
    ...
    function getMinPrice(address _token) public view override returns (uint256) {
        ...
        return vaultUtils.getMinPrice(_token, priceFeed, includeAmmPrice, useSwapPricing);
    }
    ...
    function getRedemptionCollateral(address _token) public view returns (uint256) {
        ...
        return vaultUtils.getRedemptionCollateral(_token);
    }

    function getRedemptionCollateralUsd(address _token) public view returns (uint256) {
        return vaultUtils.tokenToUsdMin(_token, getRedemptionCollateral(_token));
    }
    ...
    function usdToTokenMax(address _token, uint256 _usdAmount) public view returns (uint256) {
        ...
        return vaultUtils.usdToTokenMax(_token, _usdAmount);
    }
    ...
    function usdToTokenMin(address _token, uint256 _usdAmount) public view returns (uint256) {
        ...
        return vaultUtils.usdToTokenMin(_token, _usdAmount);
    }
    ...
    function usdToToken(address _token, uint256 _usdAmount, uint256 _price) public view returns (uint256) {
        ...
        return vaultUtils.usdToToken(_token, _usdAmount, _price);
    }
    ...
    function updateCumulativeFundingRate(address _collateralToken, address _indexToken) public {
        ...
        if (lastFundingTimes[_collateralToken][_indexToken] == 0) {
            lastFundingTimes[_collateralToken][_indexToken] =
                (block.timestamp / (fundingInterval)) *
                (fundingInterval);
            return;
        }

        if (lastFundingTimes[_collateralToken][_indexToken] + (fundingInterval) > block.timestamp) {
            return;
        }

        uint256 _fundingRate = getNextFundingRate(_collateralToken, _indexToken);
        cumulativeFundingRates[_collateralToken][_indexToken] = cumulativeFundingRates[_collateralToken][_indexToken]+(_fundingRate);
        lastFundingTimes[_collateralToken][_indexToken] = block.timestamp/(fundingInterval)*(fundingInterval);

        emit UpdateFundingRate(_collateralToken, cumulativeFundingRates[_collateralToken][_indexToken]);
    ...
    function getNextFundingRate(
        ...
        // if (
        //     lastFundingTimes[_collateralToken][_indexToken]+(fundingInterval) > block.timestamp
        // ) {
        //     return 0;
        // }
        return vaultUtils.getNextFundingRate(_collateralToken, _indexToken);
    }
    ...
    //     return
    //         vaultUtils.getNextAveragePrice(
    //             _indexToken,
    //             _size,
    //             _averagePrice,
    //             _isLong,
    ...
    // ) public view returns (uint256) {
    //     return
    //         vaultUtils.getPositionFee(_account, _collateralToken, _indexToken, _isLong, _sizeDelta);
    // }
    ...
    function _collectMarginFees(address _account, address _collateralToken, address _indexToken, bool _isLong, uint256 _sizeDelta, uint256 _size, uint256 _entryFundingRate) private returns (uint256) {
        uint256 feeUsd = vaultUtils.getPositionFee(_account, _collateralToken, _indexToken, _isLong, _sizeDelta);
    ...
    function _decreasePoolAmount(address _token, uint256 _amount) private {
        ...
        // poolAmounts[_token] = poolAmounts[_token]-(_amount, "Vault: poolAmount exceeded");
        poolAmounts[_token] = poolAmounts[_token] - (_amount);
    ...
    function _increaseGlobalShortSize(address _token, uint256 _amount) internal {
        ...
        if (maxSize != 0) {
            ...
            _validate(globalShortSizes[_token] <= maxSize, 56);
        }
    }
