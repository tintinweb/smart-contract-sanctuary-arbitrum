...
contract GToken is ERC20Upgradeable, ERC4626Upgradeable, OwnableUpgradeable, IGToken {
    ...
    mapping(address => Lock[])  AddressLocks;
    ...
    function convertToPlp(uint usdtAmount,uint nPnl) public view returns (uint) {
        if (usdtAmount == type(uint).max && nPnl >= PRECISION) {
            return usdtAmount;
        }

        return usdtAmount.mulDiv((totalAssets() - nPnl)*PRECISION, totalSupply(), MathUpgradeable.Rounding.Down);
    }

    function convertToUsdt(uint lplAmount,uint nPnl) public view returns (uint) {
        if (lplAmount == type(uint).max && nPnl >= PRECISION) {
            return lplAmount;
        }

        return lplAmount.mulDiv(totalSupply()*PRECISION, totalAssets() - nPnl, MathUpgradeable.Rounding.Down);
    }
    ...
    function runDeposit(bytes32 requestId,uint nPnl) public checks() returns (uint){
        require(_msgSender() == FeednPnlAddress,"not feed address");
        ...
        uint lplAmount = convertToPlp(applyDepositData.usdtAmount,nPnl);
    ...
    function runWithdraw(bytes32 requestId,uint nPnl) public checks() returns (uint){
        require(_msgSender() == FeednPnlAddress,"not feed address");
        ...
        uint usdtAmount = convertToUsdt(applyWithdrawData.lplAmount,nPnl);
