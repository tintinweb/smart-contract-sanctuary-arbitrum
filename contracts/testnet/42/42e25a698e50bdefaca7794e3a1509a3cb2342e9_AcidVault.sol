...
contract AcidVault is ERC4626, Ownable {
    ...
    function depositWithRewards(uint256 amount, address to, uint256 minAmountRewards) public returns (uint256) {
        require(amount > 0, "AcidVault: deposit amount must be greater than 0");
        require(minAmountRewards > 0, "AcidVault: deposit reward amount expected  must be greater than 0");

        _collectRewards(minAmountRewards);
    ...
    function redeemWithRewards(uint256 shares, address receiver, address owner, uint256 minAmountRewards)
        public
        returns (uint256)
    {
        require(shares > 0, "AcidVault: redeem shares must be greater than 0");
        require(minAmountRewards > 0, "AcidVault: redeem reward amount expected must be greater than 0");
    ...
    function withdrawWithRewards(uint256 assets, address receiver, address owner, uint256 minAmountRewards)
        public
        returns (uint256)
    {
        require(assets > 0, "AcidVault: withdraw assets must be greater than 0");
        require(minAmountRewards > 0, "AcidVault: withdraw reward amount expected must be greater than 0");
    ...
    function collectRewards(uint256 minAmountRewards) public {
        _collectRewards(minAmountRewards);
        _depositRewards();
    }
    ...
    function _collectRewards(uint256 minAmountRewards) internal {
        ...
        if (reward > minRewardToSwap) {
            ...
            camelotDex.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                reward, minAmountRewards, path, address(this), address(0), block.timestamp
            );
