...
contract DPSQRNGMock is AccessControlEnumerable {
    ...
    /// @notice requestId =>abi.encodePacked(owner,target,index)
    ...
    uint256 private returningResult;
    uint256[] private returningArr;
    ...
    function makeRequestUint256(bytes calldata _uniqueId) external onlyRole(REQUEST_ROLE) {
        resultsByUserSingle[_uniqueId] = uint256(keccak256(abi.encode(msg.sender, block.timestamp)));
    }
    ...
    function fulfillUint256(bytes32 _requestId, bytes calldata _data) external {
    ...
    function makeRequestUint256Array(uint256 _size, bytes calldata _uniqueId) external onlyRole(REQUEST_ROLE) {
        ...
        for (uint256 i; i < _size; i++) {
            arr[i] = uint256(keccak256(abi.encode(msg.sender, block.timestamp, i)));
        }
        resultsByUserArray[_uniqueId] = arr;
    }
    ...
    /// @param _requestId Request ID
    /// @param _data ABI-encoded response
    function fulfillUint256Array(bytes32 _requestId, bytes calldata _data) external {
        require(expectingRequestWithIdToBeFulfilled[_requestId], "Request ID not known");
        expectingRequestWithIdToBeFulfilled[_requestId] = false;
    ...
    function getRandomNumber(
        ...
        unchecked {
            return
                (uint256(keccak256(abi.encodePacked(_randomNumber, _blockNumber, _entropy, _min, _max))) %
                    (_max - _min + 1)) + _min;
        }
    }

    function setReturningResult(uint256 val) external {
        returningResult = val;
    }

    function setReturningArray(uint256[] memory val) external {
        returningArr = val;
    }
}