    ...
    ERC2771Context
{
    ...
    uint256 private constant MAX_FEE = 10000; //1%
    bytes32 private constant TRADING = keccak256("TRADING");
    bytes32 private constant GOV = keccak256("GOV");

    uint256 public executionFee = 1e5; //0.1$
    uint256 public permitFee = 5e4; //0.05$
    uint256 public withdrawFee = 1000; //0.1%
    uint256 private _poolBalances;
    uint256 public liquidityShare = 500000; //50%
    ...
    constructor(
        ...
        Roles(_roles)
        ERC20("Token Liquidity Provider", "TLP")
        ERC20Permit("TLP")
    ...
    function editParams(uint256 _executionFee, uint256 _permitFee)
        external
        hasRole(GOV)
    {
    ...
    function changeLiquidityShare(uint256 percent)
        external
        virtual
        override
        hasRole(GOV)
    {
        require(liquidityShare <= BPS, "!valid-percent");
        liquidityShare = percent;
    }

    function changeWithdrawFee(uint256 amount)
        external
        virtual
        override
        hasRole(GOV)
    {
        require(amount <= MAX_FEE, "!max-fee");
        withdrawFee = amount;
    }
    ...
    function deposit(uint256 amount)
        ...
        returns (uint256)
    {
        return _deposit(_msgSender(), amount, 0);
    }
    ...
    function depositGasLess(uint256 amount, Types.GasLess calldata gasLess)
        ...
        returns (uint256)
    {
        VerifyPool._verifyDeposit(
            DOMAIN_HASH,
            nonces(gasLess.owner),
            amount,
            gasLess
        );
        _useNonce(gasLess.owner);
        uint256 fee = _getFeeExecution(executionFee);
        oracleFee += fee;
        emit FeePaid(bytes32(uint256(0)), 0, 0, fee);
        return _deposit(gasLess.owner, amount, fee);
    }
    ...
    function depositGasLessWithPermit(
        ...
        _useNonce(gasLess.owner);
        uint256 fee = _getFeeExecution(executionFee + permitFee);
        oracleFee += fee;
        emit FeePaid(bytes32(uint256(0)), 0, 0, fee);
        return _deposit(gasLess.owner, amount, fee);
    }
    ...
    function _deposit(
        ...
        amount -= fee;
        oracleFee += fee;
        uint256 amountLP = (totalSupply() == 0 || _poolBalances == 0)
            ? amount * 1e12
            : (amount * totalSupply()) / _poolBalances;
        _poolBalances += amount;
        lastDeposit[owner] = block.timestamp;
        _mint(owner, amountLP);
        emit Deposit(owner, amount, amountLP);
        return amountLP;
    }

    function withdraw(uint256 _amountLP)
        external
        virtual
        override
        nonReentrant
        returns (uint256)
    {
        return _withdraw(_msgSender(), _amountLP, 0);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 fee = _getFeeExecution(executionFee);
        oracleFee += fee;
        emit FeePaid(bytes32(uint256(1)), 0, 0, fee);
        return _withdraw(gasLess.owner, amountLp, fee);
    }
    ...
    function _withdraw(
        ...
        require(amountLP > 0, "!amount");
        require(_poolBalances > 0 && totalSupply() > 0, "!empty");
        require(
            lastDeposit[owner] + LOCK_PERIOD < block.timestamp,
            "!lock-deposit"
        );
        uint256 amountUSDC = ((amountLP * _poolBalances) *
            (BPS - withdrawFee)) / (totalSupply() * BPS);
        _burn(owner, amountLP);
        usdc.safeTransfer(owner, amountUSDC - fee);
        oracleFee += fee;
        _poolBalances -= amountUSDC;
        emit Withdraw(owner, amountUSDC, amountLP);
    ...
    function creditFee(
        ...
        if (fee > 0) {
            uint256 liq = (fee * liquidityShare) / BPS;
            _poolBalances += liq;
            protocolFee += fee - liq;
        }
        if (oracle > 0) {
        ...
        emit FeePaid(id, feeType, fee, oracle);
    }
    ...
    function transferOut(address to, uint256 amount)
        ...
        hasRole(TRADING)
    {
        if (to == address(0x0) || amount == 0) return;
    ...
    function settlePosition(
        ...
        usdc.safeTransfer(to, amount);
        if (pnl < 0) {
            _poolBalances += uint256(-1 * pnl);
        } else {
            require(_poolBalances >= uint256(pnl), "!pool-balance");
            _poolBalances -= uint256(pnl);
        }
    }

    function protocolWithdraw(uint256 amount, address wallet)
        external
        override
        hasRole(GOV)
    {
        require(amount <= protocolFee, "!not-have");
        usdc.safeTransfer(wallet, amount);
        protocolFee -= amount;
        emit ProtocolWithdraw(wallet, amount);
    }

    function oracleWithdraw(uint256 amount, address wallet)
        external
        override
        hasRole(GOV)
    {
        require(amount <= oracleFee, "!not-have");
        usdc.safeTransfer(wallet, amount);
        oracleFee -= amount;
        emit OracleWithdraw(wallet, amount);
    }
...
library VerifyPool {
    ...
    bytes32 private constant DEPOSIT_HASH =
        keccak256(
            bytes("Deposit(uint256 amount,uint256 deadline,uint256 nonce)")
        );

    bytes32 private constant WITHDRAW_HASH =
        keccak256(
            bytes("Withdraw(uint256 amountLp,uint256 deadline,uint256 nonce)")
        );
    ...
    function _verifyDeposit(
        ...
        //  "Deposit(uint256 amount,uint256 deadline,uint256 nonce)"
