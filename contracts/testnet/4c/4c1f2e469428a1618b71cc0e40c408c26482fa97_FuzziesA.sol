...
contract FuzziesA is ERC721A, Ownable {
    ...
    constructor() ERC721A("fuzzies!", "FUZZ") {
        _currentBaseURI = "ipfs://bafybeigokuhqjrnz53bfl2itiikefm7p6ca3q5hnutx4to2fnnne3ptohq/";
    ...
    function mint(uint configIndex) public payable {
        ...
        _mint(msg.sender, mintConfigs[configIndex].quantity);
    ...
    function setSplitConfigAddress(uint configIndex, address _address) public onlyOwner {
        require(configIndex < 7, "Invalid index");

        uint256 balance = splitConfigs[configIndex].balance;
        uint256 weight = splitConfigs[configIndex].weight;

        splitConfigs[configIndex] = SplitConfig({
            _address: payable(_address),
            weight: weight,
            balance: balance
        });
    }
