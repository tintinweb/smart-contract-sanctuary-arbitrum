  ...
  ReentrancyGuard
{
  ...
  IAdventurerGateway public GATEWAY;
  Anima public ANIMA;
  IRealm public REALM;
  IResource public RESOURCE;
  IBatchBurnableStaker public STRUCTURE_STAKER;
  ILastActionMarkerStorage public ACTION_STORAGE;
  ...
  // Rewards
  //=======================================
  IRewardsPool public REWARDS_POOL;
  ...
  constructor(
    ...
    STRUCTURE_STAKER = IBatchBurnableStaker(_structureStaker);
  ...
  function payHomage(
    ...
    for (i = 0; i < _requests.length; i++) {
      ...
      for (uint256 j = 0; j < request.destinations.length; j++) {
        ...
        // Check the adventurer's eligibility to pay homage
        epochsSinceLastHomage -=
          destination.structureAmount *
          homageConfig.epochCost;
        ...
        require(
          ...
          "MonumentHomage: Adventurer can't pay homage to this monument, ascendance level too low"
        );

        // Ensure the adventurer has enough homage points (epochs since last homage)
        require(
          epochsSinceLastHomage > 0,
          "MonumentHomage: Adventurer can't pay homage yet"
        );
  ...
  function getEligibleStructureAmounts(
    ...
    for (uint256 i = 0; i < _realmIds.length; i++) {
      result[i] = STRUCTURE_STAKER.stakerBalance(
        _realmIds[i],
        address(MONUMENT),
        _tokenIds[i]
      );
...
library Epoch {
  ...
  function toEpoch(uint256 _timestamp, uint256 _duration, uint256 _offset) internal pure returns (uint256) {
