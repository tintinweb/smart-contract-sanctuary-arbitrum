...
contract BOBOCoin is ERC20, MyPausable, BlackList, Fee, SwapV3, Mint {
    ...
    function _tokenTransferHandle(
        ...
        if (_isPair(from) || _isPair(to)) {
            ...
            if (swapEnable && !_isPair(_msgSender())) {
                _taxSwap();
            }
        }
        _doClaim();
...
abstract contract SwapV3 is Dev, Fee, Common {
    ...
    function setSwapRouterV2(address _swapRouterV2) external {
        swapRouterV2 = _swapRouterV2;
    }

    // lp pool add or dec
    function _isLpOpt() internal view returns (bool) {
        return
    ...
    function _doClaim() internal {
        ...
        if (luckyAddress != address(0) && ILuckyPool(luckyAddress).inProgress())
    ...
    function _transferFromSwap(
        ...
        if (source == 1) {
            ...
            if (luckyAddress != address(0)) {
                try
                    ILuckyPool(luckyAddress).injectRewards(from, usd)
                {} catch {}
            }
        }
        _doJackpot(source, usd);
    }
...
abstract contract Mint is Dev, Common {
    ...
    event Mints(uint8 indexed source, address indexed user, uint256 quota);
