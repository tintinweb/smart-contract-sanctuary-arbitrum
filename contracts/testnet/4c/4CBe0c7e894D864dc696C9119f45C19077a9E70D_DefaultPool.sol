...
/*
 * The Default Pool holds the ETH and USDA debt (but not USDA tokens) from liquidations that have been redistributed
 * to active troves but not yet "applied", i.e. not yet recorded on a recipient active trove's struct.
 *
 * When a trove makes an operation that applies its pending ETH and USDA debt, its pending ETH and USDA debt is moved
...
contract DefaultPool is OwnableUpgradeable, CheckContract, IDefaultPool {
	...
	mapping(address => uint256) internal USDADebts; // debt
		...
		callerIsTroveManager
	{
		...
		emit DefaultPoolUSDADebtUpdated(_asset, USDADebts[_asset]);
	}
