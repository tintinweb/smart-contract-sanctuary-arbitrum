...
contract TLDescriptor is TLDescriptorUtils, ITLDescriptor  {
		...
		returns (string memory)
	{
		...
		parts[0] = art.getTrait("BG", _seed.background);
		parts[1] = art.getTrait("B", _seed.body);
		parts[2] = art.getTrait("E", _seed.eyes);
		parts[3] = art.getTrait("L", _seed.look);
		parts[4] = art.getTrait("M", _seed.mouth);
		parts[5] = art.getTrait("G", _seed.gender);
		parts[6] = art.getTrait("A", _seed.accessory);
		parts[7] = art.getTrait("O", _seed.outfit);
		parts[8] = art.getTrait("F", _seed.food);
		parts[9] = art.getTrait("FH", _seed.facialHair);
	...
	function createKey(uint256 _id, string memory _traitType)
		...
		returns (string memory)
	{
		return string(abi.encodePacked(_traitType, "#", _id));
	}
}
...
interface ITLDescriptor {
	...
	function getTraitDescription(uint256 _id, 
								 string memory _traitType) 
		external 
		returns (string memory);
...
interface ITLArt {
	...
	// 	external 
	// 	view 
	// 	returns (SVGUtils.SVGPart memory);
