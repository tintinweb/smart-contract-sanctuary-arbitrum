...
contract Arbitrage{
    ...
    function useCalcul(uint256 minSwapIn, uint256 maxSwapIn, uint256 stepSwapIn, uint256 minSwapOut, uint256 maxSwapOut, uint256 stepSwapout) external {
        InfoResult memory infor = calculComplex(minSwapIn, maxSwapIn, stepSwapIn, minSwapOut, maxSwapOut, stepSwapout);
    ...
    function calculComplexB(uint256 minSwapIn, uint256 maxSwapIn, uint256 stepSwapInFirst, uint256 stepSwapInSecond, uint256 stepSwapIn, uint256 stepSwapout) public view returns (InfoResult memory){
		...
		uint256 amountShibGain = calculSHIBXForPrice(trySub(reserveArbitrumUSDT, swapArbitrumUSDTOut), reserveArbitrumSHIBX + swapArbitrumSHIBXIn, trySub(swapArbitrumUSDTOut, swapAvalancheSHIBXBrut));
