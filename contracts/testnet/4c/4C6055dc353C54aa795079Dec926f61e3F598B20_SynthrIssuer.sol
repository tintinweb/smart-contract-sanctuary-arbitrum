...
contract SynthrIssuer is Owned, MixinSystemSettings, IIssuer {
    ...
    function getSendBurnGasFee(
        ...
        bytes memory lzPayload = abi.encode(PT_SYNTH_BURN, abi.encodePacked(account), synthKey, synthAmount, debtShare, burnToTarget);
    ...
    function fixCoveredDebtShare(address _account, uint256 _amountToFix) external onlySynthetix {
        synthetixDebtShare().mintShare(_account, _amountToFix);
    }
