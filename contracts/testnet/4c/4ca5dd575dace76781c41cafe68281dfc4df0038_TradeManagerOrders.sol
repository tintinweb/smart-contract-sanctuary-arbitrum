...
contract TradeManagerOrders is ITradeManagerOrders, TradeSignature, TradeManager {
    ...
    function extendPositionToLeverage(
        ...
        _verifyConstraints(
            params_.tradePair,
            constraints_,
            ITradePair(params_.tradePair).detailsOfPosition(params_.positionId).isShort ? UsePrice.MAX : UsePrice.MIN
        );
...
interface ITradeManagerOrders is ITradeManager, ITradeSignature {
    ...
    function openPosition(
        OpenPositionParams calldata,
        Constraints calldata, 
        UpdateData[] calldata)
        external
