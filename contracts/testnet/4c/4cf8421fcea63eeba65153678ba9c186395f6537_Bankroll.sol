...
/**
 * @title Bankroll, contract responsible for keeping the bankroll and distribute payouts
 */
...
contract Bankroll is Ownable {
    ...
    mapping(address => uint256) public suspendedTime;
    mapping(address => bool) public isPlayerSuspended;
    mapping(address => uint256) public totalBurned;

    IERC20 public veFlush;
    IERC20 public Flush;

    address public immutable teamWallet;

    address public immutable holderWallet;
     ...
     * @param name veFlush or Flush
     */
    event Bankroll_Token_State_Changed(address tokenAddress, string name);
    /**
    ...
    constructor(address _teamWallet, address _holderWallet) {
    ...
    function withdrawNativeFunds(address to, uint256 amount) external onlyOwner {
        ...
        if (!success) {
            revert('Bankroll: cannot withdraw native funds');
        }
    }
    ...
    function getIsValidWager(address game, address tokenAddress) external view returns (bool) {
        return (isGame[game] && (address(veFlush) == tokenAddress || address(Flush) == tokenAddress));
    }

    function setveFlush(address _address) public onlyOwner {
        veFlush = IERC20(_address);
        emit Bankroll_Token_State_Changed(_address, 'veFlush');
    }

    function setFlush(address _address) public onlyOwner {
        Flush = IERC20(_address);
        emit Bankroll_Token_State_Changed(_address, 'Flush');
    }
    ...
    function transferPayout(address player, uint256 payout) external {
        if (!isGame[msg.sender]) {
            revert('Bankroll: not a valid game');
        }
        IFlush(address(Flush)).mint(player, payout);
    }

    /**
     * @dev function that games call increase the total amount of token they burn (loss)
    ...
    function getTotalBurned(address[] memory games) external view returns (uint256[] memory) {
        uint256[] memory totalBurnedArray = new uint256[](games.length);
        for (uint256 i = 0; i < games.length; i++) {
            totalBurnedArray[i] = totalBurned[games[i]];
        }
        return totalBurnedArray;
    }
    ...
    function suspend(uint256 suspensionTime) external {
        if (suspendedTime[msg.sender] > block.timestamp) {
            revert('Bankroll: player already suspended');
        }
    ...
    function liftSuspension() external {
        if (suspendedTime[msg.sender] > block.timestamp) {
            revert('Bankroll: player still suspended');
        }
