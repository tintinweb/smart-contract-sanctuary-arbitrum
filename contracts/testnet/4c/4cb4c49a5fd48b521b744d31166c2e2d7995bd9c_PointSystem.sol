...
contract PointSystem is AccessControl {
        ...
        Counters.Counter private _subjectCounter;
        ...
        function addSubject(string memory name) external onlyPKT {
            uint256 subjectId = _subjectCounter.current();
            Subject storage subject = subjects[subjectId];
            subject.subjectId = subjectId;
            subject.subjectName = name;

            listSubject.push(subjectId);
            _subjectCounter.increment();
        }
        ...
        function addPoint(PointDetail[] memory pointDetail , string memory studentId , string memory name) public onlyPKT {
            ...
            for (uint256 i =0 ; i < pointDetail.length; i++) {
                ...
                if(pointSubject[studentId][pointDetail[i].subjectId] == 0) {
        ...
        function getPointOfStudent(string memory studentId) public view returns( string memory, uint256, PointOfStudentDetail[] memory, string memory) {
            ...
            for (uint256 i = 0; i < listSubject.length; i++) {
                ...
                if(getPointWithSubject(studentId,listSubject[i]) > 0) {
        ...
        function getListSubject() public view returns(string[] memory) {
            string[] memory names = new string[](listSubject.length);

            for (uint256 i =0 ; i< listSubject.length; i++) {
                names[i] = subjects[listSubject[i]].subjectName;
            }
