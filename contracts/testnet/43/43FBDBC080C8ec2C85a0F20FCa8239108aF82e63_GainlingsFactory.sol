...
// File: Gainlings/GainlingsFactoryCheaper.sol
...
contract GainlingsFactory is ERC721Enumerable, RrpRequesterV0, ReentrancyGuard, Ownable
{
    ...
    uint256 public _dustingTime = 14400; //10 days
    uint256 public _buffStartFactor = 60;

    string private _imageBaseUri = "";
    string private _animationBaseUri = "";
    bytes32 public merkleRoot; /// @notice bytes32 hash of the merkle root
    ...
    mapping(bytes32 => AttackRequest) private requestIdToAttackRequest; // mint = 0 , fight = 1

    mapping(uint256 => uint256)   private _tokenLevels;
    mapping(uint256 => uint256)   private _tokenBounties;
    mapping(uint256 => uint256)   private _tokenStacks;
    mapping(uint256 => uint256)   private _tokenLabn; //last attack block number
    ...
    function initContract() internal {
        ...
        _founderWallets[0] = 0xB4E40EF781d10e8f15Df89731aDF0CdD6204DCe4;
        _founderWallets[1] = 0x961C7fB450846c0476CCb7728DD00290558FE4C4;
        _founderWallets[2] = 0x8d5e056fAB617eF9Bf423B34daa29eFfF55A7134;
        _founderWallets[3] = 0x0D36B481280a1189d7E3bc9D66048dB7a0E9B91A;
    }
    ...
    function fulfillRandomnessMint(bytes32 requestId, bytes calldata data) external onlyAirnodeRrp{
        ...
        _preppedWarriors[tokenId] = getPreppedWarrior(tokenId); 
         emit GainlingsEvent(4, getMintResultOutput(tokenId,qrngUint256));
    }
    ...
    function seed(uint256 tokenId) private view returns (uint256 seeed){
    ...
    ///// game functions \\\\\
    function attack(uint256 attackerTokenId, uint256 defenderTokenId) public   {
        ...
        uint256 _tokenLastAttack = _tokenLabn[attacker.id];// > _seasonBattleStartBlockTime ? _tokenLabn[attacker.id] : _seasonBattleStartBlockTime; TODO TO MAINNET!

        require((_tokenLastAttack + attacker.weight)  <= block.timestamp, "please cool down");
    ...
    function attackWarrior(AttackRequest memory attackRequest,uint256 randomness)  internal  {
        ...
        uint256 attRand = randoms[0];
        uint256 attRandMin =  ((_buffStartFactor + (_buffStartFactor - (_tokenLevels[attackRequest.attacker.id] * 2))) / 2) * _tokenLevels[attackRequest.attacker.id];
        attRand = attRand < attRandMin ? attRandMin : attRand;
        attackerAttack += attRand;

        uint256 defRand = randoms[1];
        uint256 defRandMin =  ((_buffStartFactor + (_buffStartFactor - (_tokenLevels[attackRequest.defender.id] * 2))) / 2) * _tokenLevels[attackRequest.defender.id];
        defRand = defRand < defRandMin ? defRandMin : defRand;
        defenderAttack += defRand;
        ...
        {
            defenderAttack = defenderAttack - attackRequest.attacker.defense;
        }
        else if(catNumber == -1 || catNumber == 14)// defender bonus
        {
            attackerAttack = attackerAttack - attackRequest.defender.defense;
        }
        else
        {
            attackerAttack = attackerAttack - attackRequest.defender.defense;
        ...
        if(tokenDusted(attackRequest.attacker.id)){
            winnerToken = attackRequest.defender.id;
            loserToken = attackRequest.attacker.id;
        }
        if(tokenDusted(attackRequest.defender.id)){
            winnerToken = attackRequest.attacker.id;
            loserToken = attackRequest.defender.id;
        }
        ...
        _tokenLevels[winnerToken] = _tokenLevels[winnerToken] + 1 > 25 ? 25 : _tokenLevels[winnerToken] + 1;
        ...
        _warriorLocks[attackRequest.defender.id] = false;
         ...
         emit GainlingsEvent(2,  getFightResultOutput(attackRequest.attacker.id, attackRequest.defender.id, attRand, defRand,winnerToken));
    }
    function tokenDusted(uint256 tokenId) internal view returns (bool dusted){
         uint256 lastAttack = _tokenLabn[tokenId] > _seasonBattleStartBlockTime ? _tokenLabn[tokenId] : _seasonBattleStartBlockTime;
         return lastAttack + _dustingTime < block.timestamp;
    }
    ...
    function getPreppedWarrior(uint256 tokenId) internal view returns (Warrior memory warrior){
        string [6] memory stringTraits = buildTraitsFromSeed(seed(tokenId));
    ...
    function buildTraitsFromSeed(uint256 numbers) internal view returns (string[6] memory traits){
        uint256[6] memory randoms = splitRandom(numbers); 
    ...
    function withdrawRake()internal {
        uint256 balancePerMember = address(this).balance / 5 / 5;
        
        payable(_founderWallets[0]).transfer(balancePerMember); //THE DRAWER
        payable(_founderWallets[1]).transfer(balancePerMember); //THE TALKER
        payable(_founderWallets[2]).transfer(balancePerMember); //THE CODER
        payable(_founderWallets[3]).transfer(balancePerMember); //THE GENERALIST
        payable(owner()).transfer(balancePerMember); //THE PROJECT
    }
    function setPrice(uint256 price) external onlyOwner {
        _publicPrice = price;
    }
    function setFounderWallet(uint256 nr, address founderAddress) external{
        require(msg.sender == _founderWallets[nr], "no permissions");
        _founderWallets[nr] = founderAddress;
    }
    ...
    function getFightResultOutput(uint256 attacker, uint256 defender, uint256 aRandom, uint256 bRandom, uint256 winner) internal view returns (string memory output){
    ...
    function getMintResultOutput(uint256 tokenId, uint256 tokenSeed) internal pure returns (string memory output){
        //_tokenBounties[tokenId]
         bytes memory dataURI = abi.encodePacked(
            '{',
                '"id": "',tokenId.toString(),'",',
    ...
    function splitRandom(uint256 numbers) internal pure returns (uint256[6] memory randoms){
        uint256 a = 999;
         return [
            numbers % a,
            (numbers / a) % a,
    ...
    function splitRandomInTwo(uint256 numbers) internal pure returns (uint256[2] memory randoms){
        uint256 a = 999;
         return [
            numbers % a,
            (numbers / a) % a
    ...
    function getTokenUri(uint256 tokenId,bool encode)public view returns (string memory){
        ...
        uint256 seeed = seed(tokenId);
        if(seeed != 0){
            return getRevealedToken(tokenId,encode,seeed);
        }
        else{
    ...
    function getTokenURI(uint256 tokenId,bool encode)public view returns (string memory){
        ...
        bytes memory dataURI = abi.encodePacked(
            '{',
                ...
                '"animation_url":"',getAnimationURI(tokenId),'",', //webgl url
                '"external_url":"',getAnimationURI(tokenId),'",', //webgl url
                '"image":"',getImageURI(tokenId),'",', //title image url (example image before mint, real image after mint)
                '"attributes":',
                '[');
        dataURI = abi.encodePacked( dataURI,'{"trait_type":"Alive","value": "',aliveString,'"}');
    ...
    function getCooldown(uint256 tokenId) public view returns (uint256 cooldown){
        uint256 lastAttack =_tokenLabn[tokenId]; // > _seasonBattleStartBlockTime ? _tokenLabn[tokenId] : _seasonBattleStartBlockTime; TODO TO MAINNET! 
        Warrior memory tokenAttributes = getPreppedWarrior(tokenId);

        uint256 nextAttackBlock = lastAttack + tokenAttributes.weight;
    ...
    function getBlock() public view returns (uint256 blocknr){
        return block.number;
    } 
