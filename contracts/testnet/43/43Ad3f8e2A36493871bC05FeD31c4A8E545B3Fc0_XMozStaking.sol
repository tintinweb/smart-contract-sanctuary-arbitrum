...
contract XMozStaking is Ownable {
    ...
    function setRewardConfig(address[] calldata _rewardTokens, uint256[] calldata _rewardAmountsPerWeek) external onlyOwner {
        require(_rewardTokens.length == _rewardAmountsPerWeek.length && rewardTokens.length == 0, "XMozStaking: Invalid length");
        update();
    ...
    function update() internal {
        ...
        uint256 durationInPeriods = timeSinceLastUpdate / (5 * 60);
        uint256 remainingTimeInCurrentPeriod = timeSinceLastUpdate % (5 * 60);
    ...
    function initialize(uint256 _startTime) internal {
        uint256 diff1 = _startTime % (5 * 60);
        uint256 diff2 = block.timestamp % (5 * 60);
