...
contract EscrowERC20 {
    ...
    event WithdrawnAll(address indexed payee, address indexed token,uint256 amount);
     ...
     */
    function deposit(address payee,address token,uint amount) public  {
        IERC20(token).safeTransferFrom(msg.sender,address(this),amount);
        _deposits[token][payee] += amount;
     ...
     */
    function withdrawAll(address payable payee,address token) public {
        uint256 payment = _deposits[token][payee];
        _deposits[token][payee] = 0;
        IERC20(token).safeTransfer(payee,payment);
        emit WithdrawnAll(payee,token, payment);
    }

    function withdraw(address payable payee,address token,uint amount) public {
        require(amount<=_deposits[token][payee],"amount too large");
        unchecked{
            _deposits[token][payee]-=amount;
        }
