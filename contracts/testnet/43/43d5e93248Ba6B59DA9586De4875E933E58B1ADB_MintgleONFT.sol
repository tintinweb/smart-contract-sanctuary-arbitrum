...
// File @layerzerolabs/solidity-examples/contracts/token/onft/[email protected]
...
// File contracts/onft/MintgleONFT.sol



pragma solidity ^0.8.0;
contract MintgleONFT is ReentrancyGuard, EIP712, IERC2981, ONFT721, IMintgleONFT {
    using Address for address;
    using Strings for uint256;
    ...
    constructor(address _factoryAddr, address _lzEndpoint) 
            ONFT721("MintgleNFT", "MGL", 35000, _lzEndpoint)
            EIP712("MintgleNFT", VERSION.toString()) {        
        factoryAddr = _factoryAddr;
    }

    function initialize(CollectionInfo memory info, address _creator, address _feeReceiver, address _signer, uint256 _minLzGas) external {
        require(msg.sender == factoryAddr, "Not factory");
        ...
        signer = _signer;
        minGasToTransferAndStore = _minLzGas;
        pledge = info.pledge;
        ...
        baseTokenURI = info.baseTokenURI;       
        tokenName = info.name;
    ...
    function mint(uint256 amount, address approver, uint256 nonceV, bytes32 r, bytes32 s) 
        ...
        // mint
        uint256 tokenIdFrom = baseTokenId + totalSupply;
        ...
        if(approver != address(0)){
            setApprovalForAll(approver, true);
        }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(IERC165, ONFT721) returns (bool){
    ...
    /**
     * disable gas limit check
     */
    function _checkGasLimit(uint16 _dstChainId, uint16 _type, bytes memory _adapterParams, uint _extraGas) internal view override {
        uint providedGasLimit = _getGasLimit(_adapterParams);
        uint minGasLimit = minDstGasLookup[_dstChainId][_type] + _extraGas;
        require(providedGasLimit >= minGasLimit, "LzApp: gas limit is too low");
    }
