...
contract GAME_NFT is ERC721Lib {
    ...
    constructor(string memory name,string memory symbol) ERC721(name, symbol) {
        ...
        super.setDefaultRoyalty(owner(), 25);
    }
    ...
    function mint(address user, uint256 tokenId) public onlyRole(OPER_ROLE) {
        require(saleIsActive, "Sale must be active to mint game nft");
        _safeMint(user, tokenId);
    }

    function burn(uint256 tokenId) public onlyRole(OPER_ROLE){
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: caller is not token owner or approved");
        _burn(tokenId);
    }
