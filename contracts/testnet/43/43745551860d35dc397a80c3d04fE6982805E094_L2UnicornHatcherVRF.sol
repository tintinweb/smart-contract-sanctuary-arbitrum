...
abstract contract BaseVRFConsumer is VRFConsumerBaseV2, Ownable {
    ...
    mapping(address => uint256) internal _userRequestCount;

    mapping(address => mapping(uint256 => uint256)) internal _userRequestIds;
 ...
 * @notice 独角兽孵化(VRF)
 */
contract L2UnicornHatcherVRF is L2UnicornHatcherPermit, BaseVRFConsumer, BaseMaximum, BaseL2, BaseL2Unicorn, BaseL2UnicornRule, BaseRecipient, Pausable {
    ...
    constructor(address l2Unicorn_, address l2UnicornRule_, address l2_, address recipient_, address authorizer_, address vrfCoordinator_, bytes32 keyHash_, uint64 vrfSubscriptionId_)
    ...
    function hatchE0(uint256 deadline, uint8 v, bytes32 r, bytes32 s) payable external whenNotPaused {
        ...
        require(_nonces[_msgSender()] <= _userMaxNonces[_msgSender()], "L2UnicornHatcher: user already hatch E0 series");
        //
     ...
     * @notice 孵化 E1 E2
     */
    function hatchE1E2(uint32 hatchCount_, uint8 eSeries_) payable external whenNotPaused {
    ...
    function userFulfill(uint256[] memory requestIds_) external {
        require(requestIds_.length > 0, "L2UnicornHatcherVRF: request id is empty");
    ...
    function _fulfillByRequestId(uint256 requestId_, bool immediately_) private {
        ...
        if (immediately_) {
            uint8 eSeries_ = _requestIdESeries[requestId_];
            _mint(requestInfo_.user, requestId_, requestInfo_.randomWord, eSeries_, hatchCount_);
    ...
    function _mint(address user_, uint256 requestId_, uint256 randomWord_, uint8 eSeries_, uint32 hatchCount_) private {
        for (uint32 numberIndex_ = 0; numberIndex_ < hatchCount_;) {
