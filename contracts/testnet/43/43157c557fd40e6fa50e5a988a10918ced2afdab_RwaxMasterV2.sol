...
import {IRwaxStake} from "../interfaces/IRwaxStake.sol";
import "../interfaces/IRewarder.sol";

/// @title RwaxMaster V2
contract RwaxMasterV2 is Ownable, ReentrancyGuard {
    ...
    IPool public immutable rwaxPool;
    IRwaxStake public immutable rwaxStake;
    ...
    constructor(address _rwaxPool, address _rwaxStake, address _weth, address _rewardToken) {
        rwaxPool = IPool(_rwaxPool);
        rwaxStake = IRwaxStake(_rwaxStake);
        weth = IWETH(_weth);
    ...
    /// @param _rewardPerSecond The amount of RWAX to be distributed per second.
    ...
    /// @return pending RWAX reward for a given user.
        ...
        nonReentrant
    {
        ...
        IERC20(assetToken).safeIncreaseAllowance(address(rwaxPool), assetAmount);
        rwaxPool.addLiquidity(tranche, assetToken, assetAmount, minLpAmount, address(this));
    ...
    function addLiquidityETH(uint256 pid, uint256 minLpAmount, address to) external payable nonReentrant {
        ...
        weth.safeIncreaseAllowance(address(rwaxPool), _amountIn);
        rwaxPool.addLiquidity(tranche, address(weth), _amountIn, minLpAmount, address(this));
        uint256 lpAmount = ILPToken(tranche).balanceOf(address(this)) - balanceLpTokenBefore;
        ...
        nonReentrant
    {
        ...
        tranche.safeIncreaseAllowance(address(rwaxPool), lpAmount);
        rwaxPool.removeLiquidity(address(tranche), toToken, lpAmount, minOut, to);
    }
    ...
    function removeLiquidityETH(uint256 pid, uint256 lpAmount, uint256 minOut, address to) external nonReentrant {
        ...
        rwaxPool.removeLiquidity(address(tranche), address(weth), lpAmount, minOut, address(this));
    ...
    function _transferReward(address _to, uint256 _amount, bool _staking) internal {
        if (_staking) {
            rewardToken.safeIncreaseAllowance(address(rwaxStake), _amount);
            rwaxStake.stake(_to, _amount);
        } else {
...
interface IRwaxStake {
    ...
    function RWAX() external view returns (IBurnableERC20);

    function VE_RWAX() external view returns (IERC20);
