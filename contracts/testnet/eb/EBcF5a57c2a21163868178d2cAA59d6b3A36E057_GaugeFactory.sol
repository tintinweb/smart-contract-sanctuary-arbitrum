...
import "contracts/interfaces/IExpd.sol";

contract Expd is IExpd {
    string public constant name = "ExpandFinance";
    string public constant symbol = "EXPD";
    ...
    function initialMint(address _recipient) external {
        ...
        initialMinted = true;
        _mint(_recipient, 500 * 1e6 * 1e18); //#settings
    }
    ...
    L2GovernorVotesQuorumFraction
{
    ...
    constructor(IVotes _ve)
        L2Governor("expandFinance Governor")
        L2GovernorVotes(_ve)
...
contract ExternalBribe is IBribe {
    ...
    uint256 internal constant DURATION = 1 hours; // rewards are released over the voting period
    ...
    function getEpochStart(uint256 timestamp) public pure returns (uint256) {
        ...
        return timestamp < bribeEnd ? bribeStart : bribeStart + 1 hours;
    }
...
contract Gauge is IGauge {
    ...
    uint256 internal constant DURATION = 1 hours; // rewards are released over 1 days
    uint256 internal constant PRECISION = 10**18;
    uint256 internal constant MAX_REWARD_TOKENS = 16;
...
contract InternalBribe is IBribe {
    ...
    uint256 public constant DURATION = 1 days; // rewards are released over 1 days
...
contract Minter is IMinter {
    uint256 internal constant WEEK = 3600; // allows minting once per week (reset every Thursday 00:00 UTC)
    uint256 internal constant EMISSION = 990;
    uint256 internal constant TAIL_EMISSION = 2;
    uint256 internal constant PRECISION = 1000;
    IExpd public immutable _expd;
    ...
    uint256 internal constant LOCK = 3600 * 7 * 52 * 4;
    ...
    constructor(
        ...
        _expd = IExpd(IVotingEscrow(__ve).token());
    ...
    function initialize(
        ...
        _expd.mint(address(this), max);
        _expd.approve(address(_ve), type(uint256).max);
    ...
    function circulating_supply() public view returns (uint256) {
        return _expd.totalSupply() - _ve.totalSupply();
    }
    ...
    function calculate_growth(uint256 _minted) public view returns (uint256) {
        ...
        uint256 _expdTotal = _expd.totalSupply();
        return
            (((((_minted * _veTotal) / _expdTotal) * _veTotal) / _expdTotal) *
                _veTotal) /
            _expdTotal /
            2;
    }
    ...
    function update_period() external returns (uint256) {
        ...
        if (block.timestamp >= _period + WEEK && initializer == address(0)) {
            ...
            uint256 _balanceOf = _expd.balanceOf(address(this));
            if (_balanceOf < _required) {
                _expd.mint(address(this), _required - _balanceOf);
            }

            require(_expd.transfer(team, _teamEmissions));
            require(_expd.transfer(address(_rewards_distributor), _growth));
            _rewards_distributor.checkpoint_token(); // checkpoint token balance that was just minted in rewards distributor
            _rewards_distributor.checkpoint_total_supply(); // checkpoint supply

            _expd.approve(address(_voter), weekly);
...
import {IExpd} from "contracts/interfaces/IExpd.sol";
...
/// @notice Part 2 of 2 in the WeVE (FTM) -> USDC + EXPD (OP) redemption process
/// This contract is responsible for receiving the LZ message and distributing USDC + EXPD
contract RedemptionReceiver is ILayerZeroReceiver {
    ...
    uint256 public redeemableEXPD;
    uint256 public leftoverEXPD;
    ...
    event Initialized(address fantomSender, uint256 redeemableUSDC, uint256 redeemableEXPD);

    function initializeReceiverWith(
        address _fantomSender,
        uint256 _redeemableUSDC,
        uint256 _redeemableEXPD
    ) external onlyTeam {
        ...
        redeemableEXPD = _redeemableEXPD;
        leftoverEXPD = _redeemableEXPD;

        emit Initialized(fantomSender, redeemableUSDC, redeemableEXPD);
    }
    ...
    function previewRedeem(uint256 amountWEVE)
        public
        view
        returns (uint256 shareOfUSDC, uint256 shareOfEXPD)
    {
        // pro rata USDC
        shareOfUSDC = (amountWEVE * redeemableUSDC) / ELIGIBLE_WEVE;
        // pro rata EXPD
        shareOfEXPD = (amountWEVE * redeemableEXPD) / ELIGIBLE_WEVE;
    }
    ...
    function lzReceive(
        ...
        (uint256 shareOfUSDC, uint256 shareOfEXPD) = previewRedeem(amountWEVE);
        ...
        leftoverEXPD -= shareOfEXPD; // this will revert if underflows
        require(
            EXPD.claim(redemptionAddress, shareOfEXPD),
            "CLAIM_FAILED"
        );
    }
...
contract VeArtProxy is IVeArtProxy {
    ...
    function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) external pure returns (string memory output) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "lock #', toString(_tokenId), '", "description": "HiroFinance locks, can be used to boost gauge yields, vote on token emission, and receive bribes", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
...
contract Voter is IVoter {
    ...
    uint internal constant DURATION = 1 hours; // rewards are released over 1 days
    address public minter;
    ...
    function createGauge(address _pool) external returns (address) {
        ...
        if (msg.sender != governor) { // gov can create for any pool, even non-Sterling pairs
...
contract VotingEscrow is IERC721, IERC721Metadata, IVotes {
    ...
    /// @param token_addr `EXPD` token address
    ...
    uint internal constant MAXTIME = 4 * 365 * 3600;
    int128 internal constant iMAXTIME = 4 * 365 * 3600;
