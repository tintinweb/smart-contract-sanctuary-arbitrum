    ...
    ReentrancyGuard
{
    ...
    constructor() ERC20("PYESwap", "PYES") {
...
abstract contract ERC20Snapshot is ERC20 {
    ...
    function balanceOfAt(address account, uint256 snapshotId) public view virtual returns (uint256) {
        ...
        return snapshotted ? value : (balanceOf(account) + IRewardToken(address(this)).staked(account));
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            ...
            _updateTotalSupplySnapshot();
        } else if (IRewardToken(address(this)).isStakingContract(to)) { 
            // user is staking
            _updateAccountSnapshot(from);
        } else if (IRewardToken(address(this)).isStakingContract(from)) {
            // user is unstaking
            _updateAccountSnapshot(to);
        } else {
            // transfer
    ...
    function _updateAccountSnapshot(address account) private {
        _updateSnapshot(
            ...
            (balanceOf(account) + IRewardToken(address(this)).staked(account))
        );
    }
