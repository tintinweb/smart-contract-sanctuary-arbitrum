...
// FUNCTIONS
        ...
        onlyWhenPackEnabled(packID)
    {
        ...
        for (uint256 i = 0; i < amount;) {                                                   // Mint the right amount of packs
        ...
        onlyWhenPackEnabled(packID)
    {
        ...
        for (uint256 i = 0; i < amount;) {                                                   // Mint an amount of packs
            (uint256 itemToMint, uint256 totalToMint) = getRewardItem(packID);               // Calculate which item to mint
            emit PackClaimed(_msgSender(), packID, index, itemToMint, totalToMint);          // Write an event
            itemsContract.mintItem(itemToMint, totalToMint, _msgSender());                   // Mint reward items
            unchecked { i++; }
        ...
        onlyWhenPackEnabled(packID)
    {
        ...
        if (pack.allowlistAddress != address(0)){                                        // If there is an allow list configured
            rewardAddToAllowList(packID);                                                // Add to the allowlist as a reward if threshold met
        }
    }
...
// HELPER FUNCTIONS
        ...
        private 
    {
        ...
        if (random < packs[packID].allowlistThreshold) {                                // If the random number is less that the threshold then
            address[] memory accounts = new address[](1);                               // Instantiate an array
            accounts[0] = _msgSender();                                                 // Add to the address of the wallet that minted the pack
            IAddToAllowList(packs[packID].allowlistAddress).addToAllowlist(accounts);   // Add to the allowlist of the contract configured
        }
    }
        ...
        )
    {
        ...
        uint256 itemSet = packs[packID].itemSet;                                // Declare & set the pack item set to work from
        randomNumbers = getRandomNumbers(3);                                    // Return some random numbers
        uint256 random = randomNumbers[0] % 1000;                               // Choose a random number between 0-999
        uint256 rarityBucket = 0;                                               // Initialise local variable
        for (uint i = 0; i < 5;) {                                              // Loop through the array of drop rates
            if (random > packs[packID].dropRate[i] &&
                itemSetsContract.getItemSetByRarity(itemSet,i).length > 0) {    // Choose rarity & ensure an item is registered
                // itemSetsContract.countsBySetAndRarity(itemSet,i) > 0) {    // Choose rarity & ensure an item is registered
                rarityBucket = i;                                               // Set the rarity bucket
                break;                                                          // Move on
            }
            unchecked { i++; }
        } 
        Item[] memory items = itemSetsContract.getItemSetByRarity(itemSet, rarityBucket); // Retrieve the list of items
        require(items.length > 0,                        "ADMIN: Not enough items registered");
        uint256 index = randomNumbers[1] % items.length; // Randomly choose item to mint
        itemToMint = items[index].itemID;                // Finds the items ID
        uint256 cap = items[index].value1;               // Calculate cap
...
// ADMIN FUNCTIONS
    ...
    function addPack(Pack calldata pack)
        ...
        onlyOwner
    {
        packs[lastPackID] = pack;    // Set the pack details
        unchecked { lastPackID++; }  // Increment the total items
    }
    ...
    {
        packs[packID] = pack;  // Set the pack details
    }
    ...
    function deletePack(uint256 packID)
        ...
        onlyIfPackExists(packID)
    {
        delete packs[packID];   // delete the pack mapping
    }
...
// INTERNAL FUNCTIONS
        ...
        returns (string[] memory allPacks) 
    {
        ...
        if ( total == 0 ) {
            ...
            for(uint256 i = 0; i < total;){    // Loop through the packs
            ...
            uint256 index)
    {
        ...
        if (total == 0 ) {
            ...
            for(uint256 i=0; i < total;){                               // Loop through the addresses
                if ( packs[packID].allowlist[i].account == account &&   // if account matches
                    !packs[packID].allowlist[i].claimed )               // and unclaimed
                {
                    return (true,i);
                }
                unchecked { i++; }
...
abstract contract IItems is IERC1155, IERC1155Burnable, IERC1155Supply {
    ...
    function getItemsByTokenID(uint256 tokenID) external view virtual returns (uint256[] memory itemsByToken);
...
interface IItemSets {
    function getItemSetByType(uint256 itemSet, uint256 itemType) external view returns (Item[] memory itemsBySetAndType);
    function getItemSetByTypeAndRarity(uint256 itemSet, uint256 itemType, uint256 itemRarity) external view returns (Item[] memory itemsBySetTypeAndRarity);
