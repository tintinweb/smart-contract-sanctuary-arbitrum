...
contract MiningStrategy is Access {
  /*==================================================== Events =============================================================*/
  ...
  /// @notice Last indexed period volume
  uint256 public lastIndexedVolume;
  ...
  uint256 public volumeRecordStartTime = block.timestamp - 5 minutes;
  ...
  constructor(
    ...
    updateAddresses(_WINR, _vault, _pool, _pairToken);
    updateHalvings(_percentages, _configs);
  ...
  function updateHalvings(
    ...
    for (uint256 i = 0; i < _percentages.length; i++) {
      ...
      require(_configs[i].minMultiplier < _configs[i].maxMultiplier, "Min greater than max");
  ...
  function setTokenManager(address _tokenManager) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function getVolumeDayIndex() public view returns (uint256 day_) {
    day_ = (block.timestamp - volumeRecordStartTime) / 5 minutes;
  }

  function getVolumeOfDay(uint256 _dayIndex) public view returns (uint256 volume_) {
    volume_ = dailyVolumes[_dayIndex];
  }

  function _computeDollarValue(
    address _token,
    uint256 _amount
  ) internal view returns (uint256 _dollarValue) {
  ...
  function increaseVolume(address _input, uint256 _amount) external {
    require(msg.sender == tokenManager, "Only Token Manager");
    uint256 _dayIndex = getVolumeDayIndex();
    uint256 _dollarValue = _computeDollarValue(_input, _amount);

    dailyVolumes[_dayIndex] += _dollarValue;

    emit VolumeIncreased(_dollarValue, dailyVolumes[_dayIndex]);
  }
   ...
   function decreaseVolume(address _input, uint256 _amount) external {
    ...
    emit VolumeDecreased(_dollarValue, dailyVolumes[_dayIndex]);
  }
  ...
  function getMultiplier(uint256 _mintedByGames, uint256 MAX_MINT) public returns (int256) {
    ...
    Config memory config = getCurrentConfig(_mintedByGames, MAX_MINT);
    ...
    if (currentDayVolume < prevDayVolume) {
      currentMultiplier = (currentMultiplier * (1e18 + 2 * periodChangeRate)) / int256(PRECISION);
    } else {
      int256 decrease = (currentMultiplier * periodChangeRate) / int256(PRECISION);
  ...
  function calculate(
    ...
    _mintAmount =
      (_amount * ((uint256(getMultiplier(_mintedByGames, MAX_MINT)) * PRECISION) / parity)) /
      PRECISION;
  }
