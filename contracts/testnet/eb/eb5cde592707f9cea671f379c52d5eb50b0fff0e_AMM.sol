...
contract AMM is Message, ReentrancyGuard, ILayerZeroReceiver {
    ...
    constructor(
        ...
        trustedRemoteHash = keccak256(abi.encodePacked(address(this), _L1Target));
    }
    ...
    /// @dev Dependent on SG.
    /// @param destChainId the SG chain id of the destination chain.
    /// @param srcPoolId0 The id of the src pool for token0.
    /// @param dstPoolId0 The id of the src pool for token1.
    /// @param srcPoolId1 The id of the src pool for token1.
    /// @param dstPoolId1 The id of the src pool for token1.
    function syncToL1(
        uint16 destChainId,
        uint256 srcPoolId0,
        uint256 dstPoolId0,
