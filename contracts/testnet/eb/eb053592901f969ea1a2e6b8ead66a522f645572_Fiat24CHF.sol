...
contract Fiat24Token is ERC20PausableUpgradeable, AccessControlUpgradeable {
    ...
    event CashDeposit(uint256 indexed tokenIn, address indexed addressIn, uint256 indexed toToken, string depositDetails);
    event CashPayout(uint256 indexed tokenOut, address indexed addressOut, uint256 indexed toToken, string payoutDetails);
    event ClientPayout(uint256 indexed tokenId, address indexed sender, uint256 payoutAccount, uint256 amount, string contactId, string uuid);
  ...
  function cashDepositOK(uint256 recipientAccountId, uint256 amount, string memory depositDetails) external {
    require(_msgSender() == fiat24account.ownerOf(9101), "Fiat24Token: Sender not Cash Deposit 9101");
    transferByAccountId(recipientAccountId, amount);
    emit CashDeposit(recipientAccountId, fiat24account.ownerOf(recipientAccountId), recipientAccountId, depositDetails);
  }

  function cashDepositNOK(uint256 recipientAccountId, uint256 amount, string memory depositDetails) external {
    require(_msgSender() == fiat24account.ownerOf(9101), "Fiat24Token: Sender not Cash Deposit 9101");
    transferByAccountId(9103, amount);
    emit CashDeposit(recipientAccountId, fiat24account.ownerOf(recipientAccountId), 9103, depositDetails);
  }

  function cashPayoutOK(uint256 senderAccountId, uint256 amount, string memory payoutDetails) external {
    require(_msgSender() == fiat24account.ownerOf(9102), "Fiat24Token: Sender not Cash Payout 9102");
    transferByAccountId(9104, amount);
    emit CashPayout(senderAccountId, fiat24account.ownerOf(senderAccountId), 9104, payoutDetails);
  }

  function cashPayoutNOK(uint256 senderAccountId, uint256 amount, string memory payoutDetails) external {
    require(_msgSender() == fiat24account.ownerOf(9102), "Fiat24Token: Sender not Cash Payout 9102");
    transferByAccountId(9103, amount);
    emit CashPayout(senderAccountId, fiat24account.ownerOf(senderAccountId), 9103, payoutDetails);
  }
  ...
  function clientPayout(uint256 amount, string memory contactId) external {
    ...
    emit ClientPayout(tokenId, msg.sender, 9102, amount, contactId, uuid);
  }
...
function generateUUID() public view returns (string memory) {
  ...
  for (uint8 i = 0; i < 32; i += 4) {
      ...
      uuid[index++] = hexChars[(value >> (124 - i)) & 0x0F];
      uuid[index++] = hexChars[(value >> (120 - i)) & 0x0F];
      uuid[index++] = hexChars[(value >> (116 - i)) & 0x0F];
      uuid[index++] = hexChars[(value >> (112 - i)) & 0x0F];
  }
...
contract Fiat24Account is ERC721EnumerableUpgradeable, ERC721PausableUpgradeable, AccessControlUpgradeable {
    ...
    function mintByClient(uint256 _tokenId) external {
        ...
        require(numDigits <= minDigitForSale, "Fiat24Account: Number of digits of accountId > max. digits");
        require(numDigits > maxDigitForSale && !merchantAccountId, "Fiat24Account: Premium accountId cannot be mint by client");
    ...
    function mintByClientWithF24(uint256 _tokenId) external {
        ...
        uint256 numDigits = _tokenId.numDigits();
        bool merchantAccountId = _tokenId.hasFirstDigit(MERCHANTDIGIT);
        require((numDigits <= maxDigitForSale) || (numDigits > maxDigitForSale && numDigits <= minDigitForSale && merchantAccountId), "Fiat24Account: AccountId not available for sale. Use mintByClient()");
        require(!_tokenId.hasFirstDigit(INTERNALDIGIT), "Fiat24Account: Internal accountId cannot be mint by client");
        require(_mintAllowed(_msgSender(), _tokenId),
        "Fiat24Account: Not allowed. The target address has an account or once had another account.");
    ...
    function mintByWallet(address to, uint256 _tokenId) external {
        require(this.balanceOf(_msgSender()) > 0, "Fiat24Account: Minting address has no account");
        uint256 minterTokenId = this.tokenOfOwnerByIndex(_msgSender(), 0);
        require(minterTokenId.hasFirstDigit(MERCHANTDIGIT) && (minterTokenId >= 8 && minterTokenId <= 8999), "Fiat24Account: Incorrect account id for wallet");
        require(walletProviderMap[minterTokenId].isAvailable, "Fiat24Account: Account not wallet provider");
        require(_tokenId.numDigits() >= 5, "Fiat24Account: mintByWallet only for 5+ digits tokens");
        require(!_tokenId.hasFirstDigit(MERCHANTDIGIT),"Fiat24Account: Merchant account cannot be minted by wallet");
        walletProvider[_tokenId] = minterTokenId;
        if(minterTokenId >= 8000 && minterTokenId <= 8999) {
            f24.burnFrom(_msgSender(), 100);
        } else if(minterTokenId >= 800 && minterTokenId <= 899) {
            f24.burnFrom(_msgSender(), 50);
        } else if(minterTokenId >= 80 && minterTokenId <= 89) {
            f24.burnFrom(_msgSender(), 10);
        } else if(minterTokenId == 8) {
            f24.burnFrom(_msgSender(), 5);
        }
        status[_tokenId] = Status.Tourist;
        _mint(to, _tokenId);
    }
    ...
    function changeClientStatus(uint256 tokenId, Status _status) external {
        require(hasRole(OPERATOR_ROLE, msg.sender) || hasRole(CLIENTSTATUSCHANGE_ROLE, msg.sender), "Fiat24Account: Not an operator/clientstatuschange");
    ...
    function setClientLimit(uint256 tokenId, uint256 clientLimit) external {
        require(hasRole(CLIENTSTATUSCHANGE_ROLE, msg.sender) || hasRole(OPERATOR_ROLE, msg.sender), "Fiat24Account: Not an operator");
    ...
    function setNftAvatar(string memory url) external {
        require(this.balanceOf(_msgSender()) > 0, "Fiat24Account: Address has no account");
