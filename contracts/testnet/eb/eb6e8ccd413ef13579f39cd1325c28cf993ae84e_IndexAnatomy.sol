...
contract IndexAnatomy is IIndexAnatomyClient, IIndexAnatomyViewer, IIndexAnatomyManager, IIndexAnatomyErrors, Owned {
    ...
    function redeem(
        ...
        uint128 feeAUM = AUMLib.calculateFee(
            AUM_SCALED_PER_SECOND_RATE, state.totalSupply, state.lastTransferTimestamp, block.timestamp
        );
        ...
        if (indexShares > indexBurnt) {
            if (reserveCached == state.reserve) {
                ...
                _burnSubIndexes(
                    ...
                    state.totalSupply + feeAUM - indexBurnt,
                    receiver,
                    subIndexes,
    ...
    {
        info.feeAUM = AUMLib.calculateFee(
            AUM_SCALED_PER_SECOND_RATE, state.totalSupply, state.lastTransferTimestamp, executionTimestamp
        );
        uint128 fee = (indexShares * BURN_FEE) / (BURN_FEE_DENOMINATOR);
        uint256 assets =
            _convertToAssets(indexShares - fee, state.totalSupply + info.feeAUM, totalAssets(state.reserve));
        info.reserve = Math.min(assets, state.reserve);

        uint256 indexBurnt = info.reserve > 0 ? indexShares.mulDivUp(info.reserve, assets) : 0;

        if (indexBurnt < indexShares) {
            info.assets = new IIndexViewer.RedeemAssetInfo[](_subIndexes.length);

            for (uint256 i; i < _subIndexes.length; i++) {
                uint32 burnBalance = _subIndexBalances[i].mulDivDown(
                    indexShares - indexBurnt, state.totalSupply + info.feeAUM - indexBurnt
                ).safeCastTo32();

                info.assets[i].chainId = _subIndexes[i].chainId;
                info.assets[i].assets = new address[](_subIndexes[i].assets.length);
                info.assets[i].amounts = new uint256[](_subIndexes[i].assets.length);

                for (uint256 j; j < _subIndexes[i].assets.length; j++) {
                    info.assets[i].assets[j] = _subIndexes[i].assets[j];
                    info.assets[i].amounts[j] =
                        _subIndexes[i].balances[j].mulDivDown(burnBalance, SubIndexLib.TOTAL_SUPPLY);
                }
            }
        }
    }
...
library AUMLib {
    ...
    ) internal view returns (uint128 fee) {
        if (scaledPerSecondRate >= 0) {
            uint256 timePassed;
            unchecked {
                timePassed = uint32(currentTimestamp % type(uint32).max) - lastTransferTimestamp;
            }
