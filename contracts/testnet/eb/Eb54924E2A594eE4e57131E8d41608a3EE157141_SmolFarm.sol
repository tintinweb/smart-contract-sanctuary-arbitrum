...
contract SmolFarm is Initializable, ISmolFarm, SmolFarmContracts {
    ...
    nonZeroLength(_tokenIds)
    {
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            _stakeSmolBrain(_tokenIds[i]);
        }
    }

    function _stakeSmolBrain(uint256 _tokenId) private {
        userToTokensStaked[msg.sender].add(_tokenId);

        tokenIdToStakeStartTime[_tokenId] = block.timestamp;

        emit SmolStaked(msg.sender, _tokenId, block.timestamp);

        smolBrains.safeTransferFrom(msg.sender, address(this), _tokenId);
    }

    function unstakeSmolBrains(
        uint256[] calldata _tokenIds)
    external
    onlyEOA
    contractsAreSet
    ...
    {
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            require(userToTokensStaked[msg.sender].contains(_tokenIds[i]), "Not owned by user");

            _unstakeSmolBrain(_tokenIds[i]);
        }
    }

    function _unstakeSmolBrain(uint256 _tokenId) private {
        ...
        emit SmolUnstaked(msg.sender, _tokenId);
    ...
    nonZeroLength(_tokenIds)
    {
        for(uint256 i = 0; i < _tokenIds.length; i++) {
           ...
           _finishClaimingReward(_tokenIds[i]);
        }
    }
    ...
    function numberOfRewardsToClaim(uint256 _tokenId) public view returns(uint256) {
        if(tokenIdToStakeStartTime[_tokenId] == 0) {
            return 0;
        }

        uint256 _timeForCalculation = tokenIdToStakeStartTime[_tokenId] + (tokenIdToRewardsClaimed[_tokenId] * _timeForReward);
...
abstract contract SmolFarmContracts is Initializable, SmolFarmState {
    ...
    modifier contractsAreSet() {
        require(address(treasures) != address(0)
            ...
            && address(smolBrains) != address(0), "Contracts aren't set");
    ...
    ERC721HolderUpgradeable
{
    ...
    event SmolUnstaked(address indexed _owner, uint256 indexed _tokenId);
