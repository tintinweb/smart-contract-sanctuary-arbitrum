...
contract BarqAwards is BarqAwardsV0 {
     ...
     */
    uint256 public constant TOTAL_POINTS_CAP = TOTAL_TOKEN_SUPPLY;

    /**
     * Maximum number of points allowed to be awarded per user per day.
     */
    uint256 public constant MAXIMUM_POINTS_PER_DAY = 1000 * POINTS_PER_AWARD;
     ...
     */
    function initializeAfterUpgrade(address newPresenter) public reinitializer(2) {       
    ...
    function awardTo(address user, uint256 amount) public onlyPresenter {
        if (willExceedeDailyQuota(user, amount)) {
            emit DailyAwardPointsLimitReached(user);
            return;
        }

        if (willExceedeTotalCap(amount)) {
            emit TotalAwardPointsLimitReached();
            return;
        }

        updateDailyAmount(user, amount);
        updateTotalAmount(user, amount);
    }
    ...
    function updateDailyAmount(
        ...
        if (currentDay != lastAwardDay) {
            for (uint256 i = 0; i < lastAwardedUsersList.length; ++i) {
    ...
    function willExceedeDailyQuota(
        address user,
        uint256 amount
    ...
    function willExceedeTotalCap(uint256 amount) private view returns (bool) {
...
uint256 constant TOTAL_TOKEN_SUPPLY = 4737000000;
...
import "./v0/TransactionControllerV0.sol";
...
contract TransactionController is TransactionControllerV0{
     ...
     */
    function initializeAfterUpgrade(TrustNetwork trustNetwork, BarqAwards barqAwards) public reinitializer(2) {       
...
contract TrustNetwork is TrustNetworkV0 {
     ...
     * established.
     */
    function areConnected(address party1, address party2)
        public
        view
        returns (bool)
    {
     ...
     */
    function getConnections(address party)
        public
        view
        ...
        for (uint256 i = 0; i < userConnections.length; ++i) {
            if (userConnections[i] == counterparty) {
                if (userConnections.length > 1) {
                    userConnections[i] = userConnections[userConnections.length - 1];
                }
                userConnections.pop();
            }
        }
...
contract BarqAwardsV0 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize() reinitializer(1) public {
        __Ownable_init();
 ...
 * A smart contract aggregating all transactions happening in the barq universe.
 */
contract TransactionControllerV0 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() {
        _disableInitializers();
    }
 ...
 * each other.
 */
contract TrustNetworkV0 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() {
        _disableInitializers();
    }
