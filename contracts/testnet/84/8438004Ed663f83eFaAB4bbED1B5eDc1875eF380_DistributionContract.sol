...
contract DistributionContract {
    ...
    uint256 public constant ELIGIBILITY_EPOCHS = 12; // Nombre d'epochs avant que les détenteurs soient éligibles à la distribution
    uint256 public currentEpoch; // Numéro d'epoch actuel
    ...
    constructor(address _token, address _teamAddress) {
        ...
        epoch = new EpochContract(block.timestamp); // Instancier le contrat EpochContract avec le timestamp actuel
        teamAddress = _teamAddress;
        currentEpoch = epoch.getCurrentEpoch();
    }

    function distributeRewards(uint256 arbotAmount, uint256 ethAmount) external {
        totalDistribution += arbotAmount;

        uint256 eligibleDistribution = (ethAmount * 50) / 100; // 50% pour la team
        uint256 remainingDistribution = (ethAmount * 50) / 100; // 50% pour les détenteurs

        // Transférer les ARBOT et ETH destinés à l'équipe
        token.transfer(teamAddress, (arbotAmount * 50) / 100);
        payable(teamAddress).transfer(eligibleDistribution);

        distributeToHolders(arbotAmount, ethAmount, remainingDistribution);
    }

    function distributeToHolders(uint256 arbotAmount, uint256 ethAmount, uint256 remainingDistribution) internal {
        for (uint256 i = 0; i < token.holdersCount(); i++) {
            address holder = token.holderAtIndex(i);
            if (isEligible(holder)) {
                uint256 holderShare = (holders[holder].balance * remainingDistribution) / token.totalSupply();
                holders[holder].accumulatedArbot += (arbotAmount * holderShare) / remainingDistribution;
                holders[holder].accumulatedEth += (ethAmount * holderShare) / remainingDistribution;
            }
        }
    }
    ...
    function claimArbot() external {
        ...
        holders[msg.sender].lastClaimEpoch = currentEpoch;
    ...
    function isEligible(address holder) public view returns (bool) {
        return currentEpoch >= holders[holder].lastClaimEpoch + ELIGIBILITY_EPOCHS;
    }
    ...
    function getNextDistributionTotal() public view returns (uint256) {
    ...
    function updateEpoch() external {
        ...
        if (newEpoch > currentEpoch) {
            ...
            distributeToHolders(0, 0, 0); // Effectuer la distribution des récompenses pour le nouvel epoch
        }
    }
}
