...
import "./TLDMetadataContracts.sol";
...
contract TLDMetadata is Initializable, TLDMetadataContracts, ITLDMetadata {
    using StringsUpgradeable for uint256;
    // using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    function initialize() external initializer {
        TLDMetadataContracts.__TLDMetadataContracts_init();
    }

    function tokenURI(uint256 _tokenURI) public view override returns (string memory) {
        uint256 _tokenIndex = _tokenURI;

        SVGUtils.Seed memory seed = seeder.generateSeed(_tokenIndex);

        return descriptor.describe(_tokenURI, seed, false);
    }
}
...
abstract contract TLDMetadataContracts is Initializable, TLDMetadataState {
	
	function __TLDMetadataContracts_init() internal initializer {
		TLDMetadataState.__TLDMetadataState_init();
	}

	function setContracts(address _seederAddress, 
						  address _descriptorAddress) external onlyAdminOrOwner {
		seeder = ITLDSeeder(_seederAddress);
		descriptor = ITLDescriptor(_descriptorAddress);
	}
	...
	function areContractsSet() public view returns (bool) {
		return address(seeder) != address(0)
...
abstract contract TLDMetadataState is Initializable, AdminableUpgradeable {
    ...
    ITLDSeeder public seeder;
    ITLDescriptor public descriptor;
}
