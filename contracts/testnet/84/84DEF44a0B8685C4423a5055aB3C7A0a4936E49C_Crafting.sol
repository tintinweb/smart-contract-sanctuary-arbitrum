...
import "./CraftingDynamicConfigProvider.sol";
import "../Renown/ActivityRenown.sol";
import "../RandomPicker/RandomPicker.sol";
import "../Armory/IDurabilityEnabledArmory.sol";
import "../Utils/ArrayUtils.sol";
import "../Productivity/IProductivityStorage.sol";
import "../Productivity/ProductivityConstants.sol";
...
contract Crafting is ICrafting, ManagerModifier, ReentrancyGuard, Pausable {
  ...
  IProductivityStorage public immutable PRODUCTIVITY_STORAGE;
  ...
  CraftingDynamicConfigProvider public immutable CRAFTING_CONFIG_PROVIDER;
  ...
  constructor(CraftingContractCreationRequest memory _request) ManagerModifier(_request.MANAGER) {
    ...
    ACTION_DEMAND_STORAGE = ActionDemandAverageStorage(_request.ACTION_DEMAND_STORAGE);
    ...
    CRAFTING_CONFIG_PROVIDER = CraftingDynamicConfigProvider(_request.CRAFTING_CONFIG_PROVIDER);
    PRODUCTIVITY_STORAGE = IProductivityStorage(_request.PRODUCTIVITY_STORAGE);
  ...
  function _handleCrafting(
    ...
    totalCrafts = _verifyMaterials(_requests, levels);
    uint craftsCount;
    (totalAnima, craftsCount) = _totals(totalCrafts, animaMultipliers);
  ...
  function _totals(
    ...
    uint32[] storage animaMultipliers
  ) internal view returns (uint256 totalAnima, uint256 craftsCount) {
    for (uint i = 0; i < supportedLabIds.length; i++) {
      LabCraftingConfig storage config = labConfig[supportedLabIds[i]];
      totalAnima +=
        (totalCrafts[i] * config.baseAnimaCost * uint(animaMultipliers[i])) /
        ONE_HUNDRED;
...
contract CraftingDynamicConfigProvider is ManagerModifier {
  ...
  function _getConfig(
    ...
    uint totalDemand = _voidDemand + _realmDemand;
    ...
    result.durabilityLossPercentage = _labSupply / totalDemand;

    if (result.durabilityLossPercentage > ONE_HUNDRED) {
