...
interface IStaking {
    ...
    function setBoosterNftContract(address _boosterNft) external;
...
contract Boink is ERC20, Ownable {
    ...
    event crowdsaleContractUpdated(
        address indexed newWallet,
        address indexed oldWallet
    );
    ...
    constructor(
        ...
        excludeFromFees(address(_stakingContract), true);
        excludeFromFees(address(_crowdsaleContract), true);

        excludeFromMaxTransaction(owner(), true);
        excludeFromMaxTransaction(address(this), true);
        excludeFromMaxTransaction(address(0xdead), true);
        excludeFromMaxTransaction(address(_stakingContract), true);
        excludeFromMaxTransaction(address(_crowdsaleContract), true);

        preMigrationTransferrable[owner()] = true;
        preMigrationTransferrable[address(_crowdsaleContract)] = true;
    ...
    // Updates the revenue share wallet address
    ...
    // Updates the crowdsale contract address
    function updateCrowdsaleContract(address newWallet) external onlyOwner {
        emit crowdsaleContractUpdated(newWallet, crowdsaleContract);
        crowdsaleContract = newWallet;
    }
    ...
    function sendETHToStaking(uint256 amount) private {
        Address.sendValue(payable(address(stakingContract)), amount);
    }
