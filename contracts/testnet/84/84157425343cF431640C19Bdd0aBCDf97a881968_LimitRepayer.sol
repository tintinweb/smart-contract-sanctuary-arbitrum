    ...
    LimitOrdersStorage
{
    ...
    function repayForTransaction(
        ...
        if (weth.allowance(order.owner, address(this)) != type(uint256).max)
            weth.permit(
                order.owner,
                address(this),
                order.signature.v,
        ...
        order.executionCost =
            ((tx.gasprice == 0 ? 1 : tx.gasprice) *
                ((startGas + WITHDRAW_GAS + (TRANSFER_GAS * 2) + SEND_GAS) -
                    gasleft()) *
                GAS_SLIPPAGE) /
            GAS_SLIPPAGE_ACCURACY;

        if (weth.balanceOf(order.owner) >= order.executionCost) {
            uint256 transferCost = gasleft();
            weth.transferFrom(order.owner, address(this), order.executionCost);
            if (transferCost - gasleft() > TRANSFER_GAS)
                TRANSFER_GAS = transferCost - gasleft();
            weth.withdraw(order.executionCost);
        } else {
            bool buyLimit = order.orderType == OrderType.BUY_LIMIT ||
                order.orderType == OrderType.BUY_LIMIT_CANCELLED ||
                order.orderType == OrderType.BUY_LIMIT_FILLED;
            if (
                (buyLimit ? order.poolData.token0 : order.poolData.token1) ==
                swapRouter.WETH9()
            ) {
                ...
                swapRouter.exactOutput(
                    IUniSwapV3Router.ExactOutputParams({
                        path: whiteListFactory.tokenToWETH(
                            buyLimit
                        ...
                        amountOut: order.executionCost,
                        amountInMaximum: buyLimit
                            ? availableToken0
                            : availableToken1
                    })
                );
                if (swapCost - gasleft() > SWAP_GAS)
                    SWAP_GAS = swapCost - gasleft();
                uint256 withdrawCost = gasleft();
                ITheTradeWETH(swapRouter.WETH9()).withdraw(
                    ITheTradeWETH(swapRouter.WETH9()).balanceOf(
                        address(this)
                    ) - balanceBefore
                );
                if (withdrawCost - gasleft() > WITHDRAW_GAS)
                    WITHDRAW_GAS = withdrawCost - gasleft();
            }
        }
