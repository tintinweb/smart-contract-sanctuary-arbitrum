...
contract SurgeUI {
    ...
    function _getActiveOptions(
        ...
        while (index < totalActiveTokens) {
            uint256 tokenId = optionTokenInstance.tokenByIndex(index);
            OptionPosition memory position = optionTokenInstance.optionPosition(
                tokenId
            );
            if (
    ...
    function _getVault(
        ...
        address[] memory markets = vaultInstance.markets();
        for (uint256 i = 0; i < markets.length; i++) {
            totalActiveOptions += _getActiveOptions(
                vaultAddress,
                vaultInstance.marketConfiguration(markets[i]).optionToken
            ).length;
        }
        vault_.totalActiveOptions = totalActiveOptions;
