...
contract CoinToss is Game {
    ...
    function _callback(
        ...
        _resolveBet(bets[_id], bets[_id].amount);
        // CoinTossBet storage coinTossBet = coinTossBets[_id];
        // Bet storage bet = bets[_id];
        // // rolled faces
        // bool[] memory _rolled = new bool[](bet.rngCount);
        // uint256[] memory _payouts = new uint256[](bet.rngCount);
        // uint256[] memory _payoutsToDisplay = new uint256[](bet.rngCount);
        // uint256 totalPayOut = 0;
        // coinTossBet.numOfToss = bet.rngCount;
        // for (uint256 i = 0; i < bet.rngCount; i++) {
...
abstract contract Game is Ownable, Pausable, Multicall, ReentrancyGuard {
    ...
    uint16 public houseEdge; // differ by games, divisor = 10000
    ...
    function setHouseEdge(uint16 _houseEdge) external onlyOwner {
    ...
    event SetHouseEdge(uint16 houseEdge);
