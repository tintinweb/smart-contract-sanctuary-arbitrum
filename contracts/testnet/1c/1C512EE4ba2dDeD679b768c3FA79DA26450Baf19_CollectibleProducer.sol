...
contract CollectibleProducer is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  // Arrays
  //=======================================
  uint256[10] public cooldownAdditions;
  uint256[10] public costs;
  ...
  event Produced(uint256 realmId, uint256 collectibleId, uint256 cost);
  ...
  constructor(
    ...
    cooldownAdditions = [12, 12, 24, 24, 36, 36, 48];
    costs = [
      100000000000000000,
      150000000000000000,
      300000000000000000,
  ...
  function collect(
    ...
    for (; h < realmIdsLength; h++) {
      ...
      require(
        PRODUCTION.isActived(realmId),
        "CollectibleProducer: You must start production first"
      );

      uint256 j = 0;
      uint256 length = collectibleIds.length;
  ...
  function getHoursElapsed(uint256 _realmId) external view returns (uint256) {
    uint256 startedAt = PRODUCTION.getStartedAt(_realmId);

    return _hoursElapsed(startedAt);
  }
    ...
    returns (uint256)
  {
    ...
    uint256 startedAt = PRODUCTION.getStartedAt(_realmId);
    uint256 cooldown = cooldownAdditions[rarity];

    return _hoursElapsed(startedAt) / cooldown;
  }
  ...
  function _collect(
    ...
    // Get quantity
    uint256 quantity = _hoursElapsed(_startedAt) / cooldown;

    // Check if quantity is greater than 0
    require(quantity > 0, "CollectibleProducer: Quantity is below 0");

    // Check if desired quantity is allowed
    require(
      quantity >= _desiredQuantity,
      "CollectibleProducer: Desired quantity is too high"
    );
    ...
    // Transfer to vault
    TOKEN.safeTransferFrom(msg.sender, VAULT_OC, cost);

    // Mint
    COLLECTIBLE.mintFor(msg.sender, _collectibleId, _desiredQuantity);

    // TODO: make sure this is what we need
    emit Produced(_collectibleId, _desiredQuantity, cost);
  }
  ...
  function _hoursElapsed(uint256 _time) internal view returns (uint256) {
    ...
    return (block.timestamp - _time) / 3600;
  }
...
interface IProduction {
  ...
  function isActived(uint256 _realmId) external view returns (bool);
