...
// Selector for the destination chain (in the CCIP protocol)
  ...
  function transferToken(
    ...
    Client.EVMTokenAmount memory tokenAmount = Client.EVMTokenAmount(token, amount);
        Client.EVMTokenAmount[] memory tokenAmounts = new Client.EVMTokenAmount[](1);
        tokenAmounts[0] = tokenAmount;

        Client.EVM2AnyMessage memory message = Client.EVM2AnyMessage(abi.encode(receiver), "", tokenAmounts, address(0), "");
    emit AnyMessage(message);
    IRouterClient i_router = IRouterClient(routerAddress);
