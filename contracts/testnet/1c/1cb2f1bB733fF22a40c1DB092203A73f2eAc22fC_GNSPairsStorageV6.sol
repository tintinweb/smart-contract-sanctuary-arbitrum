...
contract GNSPairsStorageV6 {
    ...
    struct Feed{
        address feed1; //两个喂价地址
        address feed2;
        FeedCalculation feedCalculation;    //聚合策略？
        uint maxDeviationP; //最大偏差？？
    } // PRECISION (%)
    ...
    struct Pair{
        ...
        string to;
        Feed feed;  //预言机
        uint spreadP;               // PRECISION    //交易对也有点差？
        uint groupIndex;    //那种类型的交易对？
        uint feeIndex;  //费索引？意思是很多交易对都采用相同的费率？
    }
    struct Group{
        string name;
        bytes32 job;
    ...
    //貌似fee有类型，这个类型的币对都是一样的fee;
    struct Fee{
        string name;
        uint openFeeP;              // PRECISION (% of leveraged pos)   //开仓费用点数；
        uint closeFeeP;             // PRECISION (% of leveraged pos)   //平仓
        uint oracleFeeP;            // PRECISION (% of leveraged pos)   //预言机费？？？？
        uint nftLimitOrderFeeP;     // PRECISION (% of leveraged pos)   //nft限价单下单费点数？
        uint referralFeeP;          // PRECISION (% of leveraged pos)   //推荐人费用点数
        uint minLevPosDai;          // 1e18 (collateral x leverage, useful for min fee) //最少杠杆仓位DAI，测试网是1500DAI
    }
    ...
    function pairJob(uint _pairIndex) external returns(string memory, string memory, bytes32, uint){
        ...
        //属于同一个group的，job都是一样；
        //每次都对currentOrderId++；每个请求都有独一无二的orderId;
