...
interface IZap {
    // function addLiquidity(address tokenIn, uint256 tokenInAmount, address tokenA, address tokenB, bool stable) external returns (uint amountA, uint amountB, uint liquidity);
}
...
contract Zap is ReentrancyGuardUpgradeable, OwnableUpgradeable, IZap {
    ...
    function callContract(address contractAddress, bytes memory data, uint256 gasLimit, uint256 gasPrice) external {
        uint256 size = data.length;
        assembly {
            if iszero(call(gasLimit, contractAddress, gasPrice, data, size, 0, 0)) {
