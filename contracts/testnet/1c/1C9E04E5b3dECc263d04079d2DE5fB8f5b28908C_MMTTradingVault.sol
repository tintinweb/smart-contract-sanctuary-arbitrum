    ...
    IVault
{
    ...
    uint256 constant GNS_PRECISION = 1e10; // 10 decimals (gns/asset oracle)
    ...
    uint256 constant MAX_GNS_SUPPLY_MINT_DAILY_P = PRECISION / 20; // 0.05% / day (18.25% / yr max)
    ...
    uint256 public maxGnsSupplyMintDailyP; // PRECISION (% of gns supply)
    ...
    function updateMaxGnsSupplyMintDailyP(uint256 newValue)
        external
        onlyManager
    {
    ...
    function deplete(uint256 assets) external {
        ...
        address sender = _msgSender();
        IGnsToken(mttToken).burn(sender, amountGns);
    ...
    function refill(uint256 assets) external {
        ...
        uint256 amountGns = (assets * GNS_PRECISION) / mttTokenToAssetsPrice();
        ...
        IGnsToken(mttToken).mint(sender, amountGns);
