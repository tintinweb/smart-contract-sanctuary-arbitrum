...
contract MiningStrategy is AccessControlBase {
	...
	uint256 public volumeRecordStartTime = block.timestamp - 40 minutes;
	...
	function getVolumeDayIndex() public view returns (uint256 day_) {
		day_ = (block.timestamp - volumeRecordStartTime) / 20 minutes;
	}
    ...
    */
	...
	function increaseVolume(address _input, uint256 _amount) external onlyProtocol {
		...
		emit VolumeIncreased(_dollarValue, dailyVolumes[_dayIndex], _dayIndex); // Emit a VolumeIncreased event with the updated volume
	}
	...
	function decreaseVolume(address _input, uint256 _amount) external onlyProtocol {
		...
		emit VolumeDecreased(_dollarValue, dailyVolumes[_dayIndex], _dayIndex); // Emit a VolumeDecreased event with the updated volume
	}
	...
	function _getMultiplier(uint256 _mintedByGames) internal returns (uint256) {
		...
		uint256 prevDayVolume = getVolumeOfDay(index - 2);
		uint256 currentDayVolume = getVolumeOfDay(index - 1);
		...
		uint256 periodChangeRate = ((diff * 1e36) / prevDayVolume) / PRECISION;
		...
		if (currentDayVolume < prevDayVolume) {
			...
			uint256 decrease = (currentMultiplier * periodChangeRate) / PRECISION;
			newMultiplier = decrease > currentMultiplier
				? config.minMultiplier
				: currentMultiplier - decrease;
		}
		newMultiplier = newMultiplier > config.maxMultiplier
			? config.maxMultiplier
			: newMultiplier;
		newMultiplier = newMultiplier < config.minMultiplier
			? config.minMultiplier
			: newMultiplier;
	...
	function calculate(
		...
		_mintAmount =
			(_amount * ((_getMultiplier(_mintedByGames) * PRECISION) / parity)) /
			PRECISION;
	}
