...
contract PairexTradingStorageV1 is Initializable{
   ...
   function temp() external {
    ...
    function approveEcoSystem() external onlyGov {
        usdt.safeApprove(vault, type(uint256).max);
        usdt.safeApprove(address(ecosystemManage), type(uint256).max);
    }
    ...
    function handlePlatformFee(uint _amount) external onlyTrading{
        platformFee += _amount;
    }
    ...
    function claimGovFee() external onlyGov{
        require(govFeesUsdt > 0, "GOV_FEE_0");
        require(usdt.balanceOf(address(this)) >= govFeesUsdt, "UADT_NOT_ENOUGH");
