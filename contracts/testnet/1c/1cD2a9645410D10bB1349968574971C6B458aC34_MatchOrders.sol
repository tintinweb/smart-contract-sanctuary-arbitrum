...
contract MatchOrders is OwnableUpgradeable {
    ...
    function getLinkedSigner(address externalAccount) external view returns (address) {
        bytes32 contractSubaccount = bytes32(uint256(uint160(externalAccount) << 96));

        return IEndpoint(vertexEndPoint).getLinkedSigner(contractSubaccount);
    }
