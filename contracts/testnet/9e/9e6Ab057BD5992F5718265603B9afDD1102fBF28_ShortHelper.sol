...
interface IController {
    function baseTokenQuoteCurrencyPool() external view returns (address);
    ...
    ) external returns (uint256 vaultId, uint256 wPowerPerpAmount);
     ...
     */
    function deposit(uint256 _vaultId,uint256 _baseTokenAmount) external;
...
/**
 * @notice contract simplifies opening a short wPowerPerp position by selling wPowerPerp on uniswap v3 and returning baseToken to user
 */
contract ShortHelper is IERC721Receiver, ReentrancyGuard, Ownable {
    using SafeMath for uint256;
     ...
     * @param _baseTokenAddr baseToken address
     */
    constructor(
        ...
        require(_baseTokenAddr != address(0), "Invalid baseToken address");
        ...
        IERC20 _baseToken = IERC20(_baseTokenAddr);
        _wPowerPerp.approve(_swapRouter, type(uint256).max);
        _baseToken.approve(_swapRouter, type(uint256).max);
    ...
    /**
     * @notice mint power perp, trade with uniswap v3 and send back premium in baseToken
    ...
    function openShort(
        ...
        require(
            _exactInputParams.tokenOut == address(baseToken) && _exactInputParams.tokenIn == wPowerPerp,
            "Wrong swap tokens"
        );
        baseToken.transferFrom(msg.sender,address(this), _baseTokenAmount);        
        (uint256 vaultId, uint256 wPowerPerpAmount) = controller.mintPowerPerpAmount(
            _vaultId,
            _powerPerpAmount,
            _uniNftId,
            _baseTokenAmount
        ...
        // if the recipient is this address: unwrap baseToken and send back to msg.sender
        if (_exactInputParams.recipient == address(this)) {
            // baseToken.withdraw(amountOut);
    ...
    /**
     * @notice buy back wPowerPerp with baseToken on uniswap v3 and close position
     * @param _vaultId short wPowerPerp vault id
    ...
    function closeShort(
        ...
        require(
            _exactOutputParams.tokenOut == wPowerPerp && _exactOutputParams.tokenIn == address(baseToken),
            "Wrong swap tokens"
        );
        ...
        // pay baseToken and get wPowerPerp in return.
    ...
    /**
     * @dev only receive balance from baseToken contract and controller.
     */
    receive() external payable {
        require(msg.sender == address(baseToken) || msg.sender == address(controller), "can't receive balance");
    }
    ...
    function Approve(address _controller, uint256 _amount) external onlyOwner{
       IERC20(baseToken).approve(_controller, _amount);
    }
}