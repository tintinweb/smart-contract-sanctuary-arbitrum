    ...
    ReentrancyGuard
{
    ...
    function getCurrentBettableAmount(
        ...
        if (direction == 0) {
            if (exposureAmount >= maxMinuteExposure) {
                bullAmount = 0;
                bearAmount = maxMinuteExposure;
            } else {
                if (epoch > currentEpoch) {
                    // ahead betting
                    bullAmount = (maxMinuteExposure - exposureAmount) / 2;
                    bearAmount = maxMinuteExposure / 2;
                } else {
                    bullAmount = (maxMinuteExposure - exposureAmount);
                    bearAmount = maxMinuteExposure;
                }
            }
        } else {
            ...
            if (exposureAmount >= maxMinuteExposure) {
                bearAmount = 0;
                bullAmount = maxMinuteExposure;
            } else {
                if (epoch > currentEpoch) {
                    // ahead betting
                    bearAmount = (maxMinuteExposure - exposureAmount) / 2;
                    bullAmount = maxMinuteExposure / 2;
                } else {
                    bearAmount = (maxMinuteExposure - exposureAmount);
                    bullAmount = maxMinuteExposure;
                }
            }
        }
    ...
    function openPosition(
        ...
        vault.placeBet(amount, msg.sender, endTime, uint8(position));
