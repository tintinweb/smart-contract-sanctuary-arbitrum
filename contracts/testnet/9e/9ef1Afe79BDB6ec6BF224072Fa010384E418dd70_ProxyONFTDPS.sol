...
contract ProxyONFTDPS is ONFT1155Core, IERC1155Receiver {
    using ERC165Checker for address;

    IERC1155MintableBurnable public immutable token;
    ...
    constructor(address _lzEndpoint, address _proxyToken) ONFT1155Core(_lzEndpoint) {
        ...
        token = IERC1155MintableBurnable(_proxyToken);
    }
    ...
    function _debitFrom(address _from, uint16, bytes memory, uint[] memory _tokenIds, uint[] memory _amounts) internal virtual override {
        ...
        require(spender == _from || token.isApprovedForAll(_from, spender), "ONFT1155: send caller is not owner nor approved");
        
        token.burnBatch(_from, _tokenIds, _amounts);
    }

    function _creditTo(uint16, address _toAddress, uint[] memory _tokenIds, uint[] memory _amounts) internal virtual override {
        token.mintBatch(_toAddress, _tokenIds, _amounts, "");
    }
...
/**
 * @dev Interface of the IERC1155 expanded to include mintBatch & burnBatch functionality
 * @dev
 */
interface IERC1155MintableBurnable is IERC1155 {

    function mintBatch(address _receiver, uint256[] memory _ids, uint256[] memory _amounts, bytes memory data) external;
