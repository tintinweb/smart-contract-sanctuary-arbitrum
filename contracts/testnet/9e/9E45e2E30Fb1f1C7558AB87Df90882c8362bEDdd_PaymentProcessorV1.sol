...
/// @title Memefi Payment Processor Contract
contract PaymentProcessorV1 is Ownable, EIP712 {
    ...
    /// @param receiver Address which receives payment on backend
    ...
    constructor(address signer) EIP712("Memefi", "1") {
        signerRole = signer;
    }
    ...
    function checkSignature(
        ...
        bytes32 typedHash = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "ExecData(address receiver,address ptFromAccount,address[] ptFromAccountReceivers,uint256[] fromAccountAmounts,address executor,uint256 nonce,uint256 deadline)"
                    ),
                    receiver,
                    ptFromAccount,
    ...
    function execute(
        ...
        require(receiver != address(0), "PPV1: Receiver address zero");
        require(!isNonceUsed[nonce], "PPV1: Nonce already used");
        require(block.timestamp <= deadline, "PPV1: Transaction overdue");
        require(
        ...
        if ((ptFromAccountReceivers.length >= 1)) {
            ...
            if (ptFromAccount == address(0)) {
                ...
                for (uint256 i = 0; i < ptFromAccountReceivers.length; i++) {
                    uint256 balanceERC20 = IERC20(ptFromAccount).balanceOf(
                        msg.sender
                    );
                    uint256 allowanceERC20 = IERC20(ptFromAccount).balanceOf(
                        msg.sender
                    );
                    require(
                        balanceERC20 >= sum && allowanceERC20 >= sum,
                        "PPV1: Check allowance and balance"
                    );
