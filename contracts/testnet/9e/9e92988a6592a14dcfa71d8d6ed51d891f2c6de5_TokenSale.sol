...
contract TokenSale is Ownable, ReentrancyGuard {
    ...
    function buyTokens(address _referrer) external payable nonReentrant {
        ...
        require(participantCount > 0, "Maximum participant count reached");
        
        uint256 tokenAmount = msg.value / tokenPrice;
        require(token.balanceOf(address(this)) >= tokenAmount, "Insufficient token balance");

        if (_referrer != address(0) && _referrer != msg.sender) {
            uint256 referralReward = (msg.value * referralRewardPercentage) / 100;
            referrals[_referrer] += referralReward;
        }
    ...
    function withdrawReferralReward() external nonReentrant {
        uint256 referralReward = referrals[msg.sender];
        require(referralReward > 0, "No referral reward available");
