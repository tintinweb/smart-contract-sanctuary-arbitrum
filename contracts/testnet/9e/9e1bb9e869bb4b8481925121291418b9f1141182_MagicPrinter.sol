    ...
    MagicPrinterSettings
{
	...
	*/
        ...
        returns (uint256 depositId)
    {
        ...
        // );
        // MAGIC.approve(address(this), _amount);
        MAGIC.increaseAllowance(address(this), _amount);
        uint256 allowance = MAGIC.allowance(msg.sender, address(this));
        require(allowance > 0, "Allowance low");

        // uint256 balance = MAGIC.balanceOf(msg.sender);
        // require(balance > _amount, "Not enough dinero");
        
        MAGIC.transferFrom(msg.sender, address(this), _amount);
        // MAGIC.transfer(address(this), _amount);
        // MAGIC.approve(address(this), _amount + 1);
        // 
    ...
    //     return flywheelVault.currentEpoch();
    // }
    ...
    //             withdrawAmount += flywheelVault.withdraw(_depositIds[i]);
    //             // clean up depositId
    //         }
    //     }
    //     // TODO
    //     // transfer withdrawAmount to user
    ...
    //         "MagicPrinter: User does not have stakes"
    //     );
    ...
    // /**
    //  * @dev Request a withdrawal from Flywheel. This works with a retention period of 14 epochs.
    //  * Once the retention period has passed, the stake can be withdrawn.
    //  */
    // function requestWithdrawal(uint256[] calldata _depositIds) public {
    //     for (uint256 i = 0; i < _depositIds.length; i++) {
    //         require(
    //             canRequestWithdrawal(_depositIds[i]),
    //             "Deposit is not ready to request for withdrawal"
    //         );
    ...
    // {
    //     return flywheelVault.canRequestWithdrawal(_depositId);
    // }
    ...
    //     MAGIC.safeTransfer(TREASURY_VAULT, fee);
    //     // emit a message
    // }
    ...
    // {
    //     return flywheelVault.canWithdraw(_depositId);
    // }

    // // function retentionLock(uint256 _depositId) public view returns(bool) {
    // // 	return atlasStaker.getVaultStake(_depositId).retentionUnlock;
    // // }
    ...
    //     return flywheelVault.getAllowedLocks();
    // }
    ...
    //  function requireFlywheelUnstakeInvariants(address _owner)
    //     external
    //     view
    //     override
    // {
    //     require(depositsByUser[_owner].length() == 0, "MagicPrinter: Cannot have stakes");
    // }
}
...
abstract contract MagicPrinterContracts is Initializable, MagicPrinterState {
    ...
    //     return address(flywheelVault) != address(0);
    // }

    // function setFlywheelVault(address _flywheelVault)
    //     external
    //     onlyAdminOrOwner
    // {
    //     flywheelVault = IBattleflyAtlasStakerV02(_flywheelVault);
    ...
    AdminableUpgradeable
{
    ...
    //     LockPeriod _lock
    // );
    // event ExplorationWithdraw(
    //     address _user,
    //     uint256 _amount,
    //     uint256 _depositId
    // );
    // event ExplorationClaim(
    //     address _user,
    //     uint256 _amount,
    //     uint256 _depositId,
    //     LockPeriod _lock
    // );
    // event ExplorationEmission(
    //     uint64 _day,
    //     uint256 _timeStamp,
    //     uint256 _twoWeeks,
    //     uint256 _oneMonth,
    //     uint256 _threeMonths,
    //     uint256 _sixMonths,
    //     uint256 _twelveMonths
    // );
    // event AtlasDepositAmounts(
    //     uint64 _day,
    //     uint256 _timeStamp,
    //     uint256 _twoWeeks,
    //     uint256 _oneMonth,
...
//     uint64 day;
//     uint256 twoWeeksAmount;
// }

// struct DailyWithdrawStakes {
//     uint64 day;
//     EnumerableSetUpgradeable.UintSet twoWeeks;
// }
...
interface IExploration {
	...
	function numberOfDonkeysStakedForUser(address _owner)
        external
        view
        returns(uint256);
}
...
interface IMagicPrinter {
    ...
    function deposit(uint256 _amount) external returns (uint256 depositId);
