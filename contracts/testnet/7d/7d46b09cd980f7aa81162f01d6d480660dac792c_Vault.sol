...
contract Vault is ReentrancyGuard, AccessControlBase, IVault {
    ...
    uint16 private constant MAX_WAGER_FEE = 1500; // 15%
    ...
    function payout(
        ...
        // note: the wagerFeeBasisPoints stays in the Vault  (for now) however it is owned by the WLP tokens anymore (same for swapFees and referralFees!
        ...
        if (_tokens[0] == _tokens[1]) { // token in, is the same as token out
             ...
             * The WLP has received the _escrowAmount (from vaultmanager). The wager fee is not part of the WLP profit so we will not take it into account -> _amountNetDifference
             */
            uint256 _amountNetDifference;
            /**
             * amountAfterWagerFee_ value represents what is added to the vault and belongs to the WLP holders.
             * _totalAmount value represents what the vault will need to pay to the recipient
             * if amountAfterWagerFee_ is smaller than _totalAmount the vault made a loss
             * if amountAfterWagerFee_ is larger the vault made a profit
             */
            if (amountAfterWagerFee_ <= _totalAmount) { // the vault made a loss
                // the wagerFeeBasisPoints is not fully for the WLP, so we deduct amountAfterWagerFee_ instead, _amountNetDifference is the value that on net is lost by the WLP holders
                _amountNetDifference = (_totalAmount - amountAfterWagerFee_);
                ...
                // if the amountAfterWagerFee_ is larger as _totalAmount, the vault is making a profit and hence we do not need to deduct it from totalAmount
                // vice versa as the if statement, we register the net proft
                _amountNetDifference = (amountAfterWagerFee_ - _totalAmount);
                ...
                if (wagerFeeCharged_ >= _totalAmount) {
                    // if the wagerFeeCharged_ is larger as the _totalAmount, the player is not receiving anything 
                    // so we need to update the tokenBalances to reflect that tokens have entered the vault
            ...
            // transfer the _toPayOnNet amount tokens to the recipient and do accounting of the internal balances
            _payoutPlayer(
                _tokens[0], // _addressTokenOut (same as _tokens[1])
                (_totalAmount - wagerFeeCharged_), // wagerFeeCharged_ cannot be larger as totalAmount, because if that would be the case the function would have returned already (see previous if/else-statement) - amountAfterWagerFee_ cannot be larger as wagerFeeCharged_by definition
            ...
            if (wagerFeeCharged_ < _totalAmount) { 
                totalAmountOut_ = _amountOfTokenForToken(
                                _tokens[0],
                                _tokens[1], 
                                (_totalAmount - wagerFeeCharged_)
                            );
            } else {
                // if wagerFees are larger then the totalAmount, the players winnings are completely consumed by the wagerFees
                totalAmountOut_ = 0;
            }
            ...
            if(totalAmountOut_ == 0) { // the player has effectively won nothing expressed in _tokens[1]
                /**
                 * note this occurs when the player has won so little that it cannot be expressed in _tokens[1]. This can particulairly occur when the player has chosen to win a token with less decimals as the wagerAsset (_tokens[0]). But it can also happen if the player has bet in 1e17 DAI (and has won 2 * 1e17 DAI) but wants the winnings in ETH. Since ETH is 1e18 ETH is 1600 more valueable as 1e18 DAI, 2 * 1e17 DAI cannot be 'expressed' in ETH - hence totalAmountOut_ will be 0. 
                 * In this situation no tokens will be transferred to the player! 
                 */
                _increasePoolAmount(_tokens[0], (_escrowAmount - wagerFeeCharged_));
                // since on net tokens have entered the vault, we need to update the totalIn_
                totalIn_[_tokens[0]] += (_escrowAmount - wagerFeeCharged_);
            ...
            _updateTokenBalance(_tokens[0]);
            /**
             * totalAmountOut_ represents the amount of tokens[1] that the recipient has 'won' (with the wagerfee already deducted in conversion!)
             * amountOutAfterSwap_ represents the amount of the wager (coverted to token[1]) that is now owned by the vault (with the swapFee already deducted from it)
             * If totalAmountOut_ is larger as amountOutAfterSwap_ than it means the vault made a loss in this tx.
             * If totalAmountOut_ is smaller, then the vault has made a profit.
             * _amountNetDifference value the net difference in either way (profit or loss)
             */
            uint256 _amountNetDifference;
            /**
            ...
            if (totalAmountOut_ >= amountOutAfterSwap_) { // vault has made a loss
                // Calculate how much the WLP needs to pay the player on net. we deduct the feesPaid in the swap since these stay in the contract. We do not need to deduct that wagerFeeBasisPoints since we already deducted that in the step where we calculated  totalAmountOut_. totalAmountOut_ cannot be higher as feesPaidInOut_ because the swap fee is a percentage (smaller as 100%).  
                ...
                // in this case, the wlp pool actually made a profit (quite unusual, but we still want to handle this situation) -> UNUSUAL SITUATION
     ...
     * @return correctedSwapFees_ the amount of swap fees that are available to be withdrawn
    ...
    function withdrawAllFees(address _tokenToWithdraw) external override protocolNotPaused returns(
            ...
            uint256 registeredReferralKickback_) {
                ...
                correctedSwapFees_ = swapFeeReserves[_tokenToWithdraw];
                ...
                if(registeredReferralKickback_ >= maxAmountReferral_) {
                    ...
                    // option 1: since we suspect a exploit/problem in referral system, we null the referral rewards, with this no funds can be lost
                    registeredReferralKickback_ = 0;
                    // in option 1 we do not need to do _decreasePoolAmount because no vault funds are paid out for referral

                    // // option 2: we cap the referrarewards to the maximum amount that is allowed, this will result in a loss of funds
                    // registeredReferralKickback_ = maxAmountReferral_;
                    // _decreasePoolAmount(_tokenToWithdraw, registeredReferralKickback_);

                    // if favour option 1. The reason being that if we do option 1 or option 2, we will in both cases have to do manual transaction in the FeeCollector and/or ReferalRegistry to restore the correct referral rewards (since in both option 1 and 2 there will be too little referral rewards in the contracts than is stored in the mappings in ReferralStorage). Since both options require manual intervention, we might as well do option 1, which is less likely to result in a loss of funds.
                ...
                _transferOut(
                    _tokenToWithdraw, 
                    (correctedSwapFees_ + totalWagerFees_ + registeredReferralKickback_), 
                    feeCollector_
                );

                // todo emit event?

                return (correctedSwapFees_, totalWagerFees_, registeredReferralKickback_);
    }
     ...
     * note wagerfees are now owned by the WLP token holders directly!       
     */
    function _collectWagerFees(
        ...
        uint256 wagerFee_ = wagerFeeBasisPoints; // note wagerFee_ only ussed once, this sload gas saving doesn't save gas!
        amountAfterWagerFee_ = (_amountEscrow * (BASIS_POINTS_DIVISOR - wagerFee_)) / BASIS_POINTS_DIVISOR;
