...
contract SourceMinter is Withdraw {
    ...
    function sendNFT(
        ...
        // The caller has own the nft before sending it 
        ...
        // require(nft.ownerOf(tokenId) == 0x000000000000000000000000000000000000dEaD, "The NFT has not been burned, check user approvals to this contract");
        nft.safeTransferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, tokenId);

        Client.EVM2AnyMessage memory message = Client.EVM2AnyMessage({
            receiver: abi.encode(receiver),
            data: abi.encodeWithSignature("mintTokenId(address, tokenId)", msg.sender, tokenId),
            tokenAmounts: new Client.EVMTokenAmount[](0),
            extraArgs: "",
            feeToken: payFeesIn == PayFeesIn.LINK ? i_link : address(0)
        });
    ...
    function burnNFT(uint256 tokenId) external {
        // The caller owns the nft before sending it 
        require(nft.ownerOf(tokenId) == msg.sender, "sendNFT: You must own the NFT to send it.");
        //burn the NFT, requires a user to set approval or approvalForAll
        // require(nft.ownerOf(tokenId) == 0x000000000000000000000000000000000000dEaD, "The NFT has not been burned, check user approvals to this contract");
        nft.safeTransferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, tokenId);
