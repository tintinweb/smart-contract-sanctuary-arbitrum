...
contract L2NonFungibleTokenResolver is BaseL2Unicorn, BaseL2UnicornRule, BaseL2MagicCube, BaseL2MagicCubeRule, RewardsManager {
    ...
    constructor(address l2Unicorn_, address l2UnicornRule_, address l2MagicCube_, address l2MagicCubeRule_, address rewardsToken_)
    ...
    function resolveL2Unicorn(uint256[] calldata tokenIdArr) external {
        ...
        for (uint256 i_ = 0; i_ < length;) {
            ...
            emit Resolve(_msgSender(), l2MagicCube, hatchRule.level, tokenId_, awardAmount_);
            unchecked{++i_;}
        }
    ...
    function resolveL2MagicCube(uint256[] calldata tokenIdArr) external {
        uint256 length = tokenIdArr.length;
        uint256 totalAwardAmount;
        for (uint256 i_ = 0; i_ < length;) {
            uint256 tokenId_ = tokenIdArr[i_];
            require(_msgSender() == IERC721(l2MagicCube).ownerOf(tokenId_), "L2NonFungibleTokenResolver: incorrect owner");
            IL2MagicCubeRule.TokenIdRule memory tokenIdRule = IL2MagicCubeRule(l2MagicCubeRule).getTokenIdRuleByTokenId(tokenId_);
            uint256 awardAmount_ = tokenIdRule.awardAmount;
            require(awardAmount_ > 0, "L2NonFungibleTokenResolver: award amount is zero");
            totalAwardAmount += awardAmount_;
            emit Resolve(_msgSender(), l2MagicCube, tokenIdRule.level, tokenId_, awardAmount_);
            unchecked{++i_;}
        }
        IL2MagicCube(l2MagicCube).batchBurn(tokenIdArr);
        if (totalAwardAmount > 0) {
            IERC20(rewardsToken).transfer(_msgSender(), totalAwardAmount * 1e18);
        }
    }
