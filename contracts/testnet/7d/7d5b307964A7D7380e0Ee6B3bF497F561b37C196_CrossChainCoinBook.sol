    ...
    ReentrancyGuardUpgradeable
{
    ...
    * @param r The round number for the listing fee
    ...
    function insertActiveOrder(uint256 order, bytes32 key) internal {
        ...
        uint256 length = activeOrders[key].length;
        if (length == 0) {
            activeOrders[key].push(order);
            return;
        }
        uint256 ratio = ((orderId[order].sellAmount * 10**18) / orderId[order].buyAmount);
        if (ratio < ((orderId[activeOrders[key][length - 1]].sellAmount * 10**18) / orderId[activeOrders[key][length - 1]].buyAmount)) {
            activeOrders[key].push(order);
            return;
        }
        for (uint i = 0; i < activeOrders[key].length; i++) {
            if (ratio > ((orderId[activeOrders[key][i]].sellAmount * 10**18) / orderId[activeOrders[key][i]].buyAmount)) {
