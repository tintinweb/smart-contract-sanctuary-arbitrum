...
contract TestnetOracle is Ownable, IOracle {
    ...
    mapping(bytes32 => PriceFeed) feeds;
    ...
    function setValue(
        ...
        require(timestamp > feeds[productId].timestamp, '!timestamp');
        feeds[productId] = PriceFeed(value, timestamp);
        notifyPriceFeed(productId, value, timestamp);
    }

    function setValues(
        bytes32[] calldata productIds,
        FPUnsigned[] calldata values,
        uint64[] calldata timestamps
    ) external onlyOracle {
        for (uint256 i=0; i < productIds.length; i++) {
            require(timestamps[i] > feeds[productIds[i]].timestamp, '!timestamp');
            feeds[productIds[i]] = PriceFeed(values[i], timestamps[i]);
            notifyPriceFeed(productIds[i], values[i], timestamps[i]);
        }
    }

    /**
     * @notice Notify PriceFeed, if registered, of all price updates
     * @dev No-op if unregistered, so oracles can set up before PriceFeed or be
     *      used for other things
     * @param productId product being updated
     * @param value new value
     * @param timestamp time new value was generated
     */
    function notifyPriceFeed(
        bytes32 productId, FPUnsigned value, uint64 timestamp
    ) internal onlyOracle {
        if (address(priceFeed) != address(0)) {
            priceFeed.notifyPerp(productId, value, timestamp);
        }
    }
    ...
    function getValue(bytes32 productId) external view returns (
        ...
        require(timestamp != 0, "oracle value unset");
    }
    ...
    modifier onlyOracle() {
        require(isOracle[msg.sender], '!whitelist');
        _;
    }
}