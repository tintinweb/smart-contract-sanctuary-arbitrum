...
// STATE VARIABLES
    ...
    /// @dev Store the items in a set with a specific type & rarity [common, uncommon, rare, epic, legendary]
    /// @dev e.g., getItemsBySetTypeAndRarity(ItemSet,ITEM_TYPE,rarity)
    /// @dev e.g., getItemsBySetTypeAndRarity[itemSet][itemType][rarity]->[Item Struct]
    mapping (uint256 => mapping (uint256 => mapping (uint256 => Item[]))) private getItemsBySetTypeAndRarity;
    
    /// @dev Store the items in a set with a specific rarity [common, uncommon, rare, epic, legendary]
    /// @dev e.g., getItemsBySetAndRarity[itemSet][rarity]->[Item Struct]
    mapping (uint256 => mapping (uint256 => Item[])) private getItemsBySetAndRarity;

    /// @dev Store the items in a set with a specific type
    /// @dev e.g., getItemsBySetAndType[itemSet][itemType]->[Item Struct Arrary]
    mapping (uint256 => mapping (uint256 => Item[])) private getItemsBySetAndType;

// EVENTS

event ItemRegistered (address indexed account, uint256 itemID, uint256 itemType, uint256 itemRarity, uint256 itemSet);
...
// ADMIN FUNCTIONS
    ...
    function registerItem(uint256 itemID, uint256 itemSet, uint256 itemValue1, uint256 itemValue2)
        ...
        onlyOwner
    {
        (,,,,,,,uint256 itemRarity,uint256 itemType,,) = itemsContract.items(itemID);   // Store the item rarity & type in local variable
        getItemsBySetTypeAndRarity[itemSet][itemType][itemRarity].push(Item(itemID, itemType, itemSet, itemRarity, itemValue1, itemValue2));
        getItemsBySetAndRarity[itemSet][itemRarity].push(Item(itemID, itemType, itemSet, itemRarity, itemValue1, itemValue2));
        getItemsBySetAndType[itemSet][itemType].push(Item(itemID, itemType, itemSet, itemRarity, itemValue1, itemValue2));
        emit ItemRegistered (_msgSender(), itemID, itemType, itemRarity, itemSet); // Write an event
    }

    /// @dev Deregister items for minting as rewards
    /// @param index index of item in array
    function deregisterItemBySetTypeAndRarity(uint256 itemSet, uint256 itemType, uint256 itemRarity, uint256 index)
        external
        onlyOwner
    {
        Item[] storage items = getItemsBySetTypeAndRarity[itemSet][itemType][itemRarity];
        uint256 itemID = items[index].itemID;                   // Set the Item ID
        ...
        emit ItemDeregistered(_msgSender(), itemID, itemSet);   // Write an event
    }
    ...
    function deregisterItemBySetAndRarity(uint256 itemSet, uint256 itemRarity, uint256 index)
        ...
        onlyOwner
    {
        Item[] storage items = getItemsBySetAndRarity[itemSet][itemRarity];
        uint256 itemID = items[index].itemID;                   // Set the Item ID
        items[index] = items[items.length - 1];                 // In the items array swap the last item for the item being removed
        items.pop();                                            // Delete the final item in the items array
        emit ItemDeregistered(_msgSender(), itemID, itemSet);   // Write an event
    ...
    /// @dev Deregister items for minting as rewards
    /// @param index index of item in array
    function deregisterItemBySetAndType(uint256 itemSet, uint256 itemType, uint256 index)
        external
        onlyOwner
    {
        Item[] storage items = getItemsBySetAndType[itemSet][itemType];
        uint256 itemID = items[index].itemID;                 // Set the Item ID
        items[index] = items[items.length - 1];               // In the items array swap the last item for the item being removed
        items.pop();                                          // Delete the final item in the items array
        emit ItemDeregistered(_msgSender(), itemID, itemSet); // Write an event
...
// VIEWS
    ...
    // TODO:USED BY HEALTH MANAGER TO IDENTIFY HEALTH ITEMS IN A SET
    /// @dev Retrieve list Items by set
    /// @param itemSet Item Set e.g., a grouping of items for selection
    function getItemSetByType(uint256 itemSet, uint256 itemType)
        external
        view
        returns (Item[] memory itemsBySetAndType)       // Define the array of items to be returned.
    {
        return getItemsBySetAndType[itemSet][itemType]; // Return list of items
    }

    // TODO: USED BY QUESTS TO IDENTIFY REWARD ITEMS IN A SET, TYPE & RARITY
    ...
    function getItemSetByTypeAndRarity(uint256 itemSet, uint256 itemType, uint256 itemRarity)
        external
        view
        returns (Item[] memory itemsBySetTypeAndRarity)                   // Define the array of items to be returned.
    {
        return getItemsBySetTypeAndRarity[itemSet][itemType][itemRarity]; // Return list of items
    }

    // TODO: USED BY PACKS TO IDENTIFY REWARD ITEMS IN A SET & RARITY
    ...
    function getItemSetByRarity(uint256 itemSet, uint256 itemRarity)
        external
        view
        returns (Item[] memory itemsBySetAndRarity)         // Define the array of items to be returned.
    {
        return getItemsBySetAndRarity[itemSet][itemRarity]; // Return list of items
    }
