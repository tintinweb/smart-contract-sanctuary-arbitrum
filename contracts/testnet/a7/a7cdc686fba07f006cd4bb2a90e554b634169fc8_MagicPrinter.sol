...
import "./MagicPrinterSettings.sol";
import "../interfaces/IAtlasMine.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";

contract MagicPrinter is
	Initializable,
	MagicPrinterSettings
{
	...
	function initialize() external initializer {
		MagicPrinterSettings.__MagicPrinterSettings_init();
	}
	...
	*/
	...
	{
		approved = MAGIC.approve(address(this), MAGIC_NUMBER);
		approved = MAGIC.approve(address(flywheelVault), MAGIC_NUMBER);
	}
	...
	{
		require(_amount > 0,
			"MagicPrinter: Deposit amount should be greater than 0"
		);
		require(
			MAGIC.allowance(msg.sender, address(this)) >= _amount, 
			"MagicPrinter: Not enough approved"
		);
		uint256 balance = MAGIC.balanceOf(msg.sender);
		require(balance >= _amount, 
			"Magic Printer: Not enough dinero"
		);
		MAGIC.transferFrom(msg.sender, address(this), _amount);
		depositId = flywheelVault.deposit(_amount, _lock);
		// depositId = totalDeposits++; // for testing
		...
		depositsByUser[msg.sender].add(depositId);
		deposits[depositId] = userDeposit;
		emit ExplorationDeposit(
				msg.sender,
				_amount,
				depositId,
				_lock
		...
		returns (uint256 withdrawAmount)
	{
		for (uint256 i = 0; i < _depositIds.length; i++) {
			if (canWithdraw(_depositIds[i])) {
				// claim reward for deposit. _claim handles fee calculation and transfers
				_claim(_depositIds[i]);
	...
	{
		require(
			depositsByUser[msg.sender].length() != 0,
			"MagicPrinter: caller does not have deposits"
		);
		EnumerableSetUpgradeable.UintSet storage userDeposits = depositsByUser[
			msg.sender
		];

		for (uint256 i = 0; i < userDeposits.length(); i++) {
			emissionAmount += _claim(userDeposits.at(i));
		}
	}
	...
	{
		UserDeposit memory userDeposit = deposits[_depositId];
		require(
			userDeposit.user == msg.sender,
			"MagicPrinter: caller is not owner"
		);
		require(
			canWithdraw(_depositId),
			"MagicPrinter: deposit still locked"
		);

		amount = flywheelVault.withdraw(_depositId);
		MAGIC.transfer(msg.sender, amount);
		depositsByUser[msg.sender].remove(_depositId);
		delete deposits[_depositId];
		emit PrinterWithdrawal(
			msg.sender,
			amount,
			_depositId
		);
	...
	// Internal claim function to interface with FW and claim a fee
	function _claim(uint256 _depositId)
		internal
		returns (uint256 emissionAmount)
	{
		UserDeposit memory userDeposit = deposits[_depositId];
		require(
			msg.sender == userDeposit.user,
			"MagicPrinter: User did not make this deposit"
		);

		emissionAmount = flywheelVault.claim(userDeposit.depositId);

		// calculate fee from emissionAmount.
		uint256 fee = (emissionAmount * FEE) / FEE_DENOMINATOR;
		emissionAmount -= fee;

		// transfer fee to treasury and remainder to user
		MAGIC.transfer(msg.sender, emissionAmount);
		MAGIC.transfer(TREASURY_VAULT, fee);

		emit PrinterClaimEmission(
			msg.sender,
			emissionAmount,
			_depositId,
			initialUnlock(_depositId)
		);
	}

	function hasUserDeposits(address _user) public 
		view
		override
		returns (bool hasDeposits) 
	{
		hasDeposits = depositsByUser[_user].length() > 0;
	}

	function initialUnlock(uint256 _depositId) public 
		view  
		returns (uint64 epoch) {
		return flywheelVault.getVaultStake(_depositId).unlockAt;
	}

	function canWithdraw(uint256 _depositId) public
		view
		returns (bool withdrawable)
	{
		withdrawable = flywheelVault.canWithdraw(_depositId);
	}

	function getClaimableEmission(uint256 _depositId)
		public
		view
		returns (uint256 emissionAmount, uint256 fee)
	{
		(emissionAmount, ) = flywheelVault.getClaimableEmission(
			_depositId
		);
		fee = (emissionAmount * FEE) / FEE_DENOMINATOR;
		emissionAmount -= fee;
	}
	 ...
	 */
	function requestWithdrawal(uint256[] memory _depositIds) 
		public
		contractsAreSet
		treasuryIsSet
		printerValuesSet
	...
	{
		for (uint256 i = 0; i < _depositIds.length; i++) {
			UserDeposit memory userDeposit = deposits[_depositIds[i]];
			require(
				userDeposit.user == msg.sender, 
				"MagicPrinter: caller is not correct user");
			flywheelVault.requestWithdrawal(_depositIds[i]);
			emit PrinerRequestWithdrawal(
				userDeposit.user,
...
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "./MagicPrinterContracts.sol";

abstract contract MagicPrinterSettings is Initializable, MagicPrinterContracts {
	function __MagicPrinterSettings_init() internal initializer {
		MagicPrinterContracts.__MagicPrinterContracts_init();
	}
}
...
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "./MagicPrinterState.sol";
import "../interfaces/IBattleflyAtlasStakerV02.sol";

import "@openzeppelin/contracts-upgradeable/token/ERC20/utils/SafeERC20Upgradeable.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";

abstract contract MagicPrinterContracts is Initializable, MagicPrinterState {
	function __MagicPrinterContracts_init() internal initializer {
		MagicPrinterState.__MagicPrinterState_init();
	}
	using SafeERC20Upgradeable for IERC20Upgradeable;

	function setContracts(address _flywheelVault, address _exploration, address _magic)
		external
		onlyAdminOrOwner
	{
		flywheelVault = IBattleflyAtlasStakerV02(_flywheelVault);
		exploration = IExploration(_exploration);
		MAGIC = IERC20Upgradeable(_magic);
	}

	modifier contractsAreSet() {
		require(areContractsSet(), "MagicPrinter: Contracts aren't set");
		_;
	}

	function areContractsSet() public view returns (bool) {
		return address(MAGIC) != address(0)
			&& address(flywheelVault) != address(0)
			&& address(exploration) != address(0);
	}

	function setTreasury(address _treasury) public onlyOwner {
		require(
			_treasury != address(0),
			"MagicPrinter: invalid treasury address"
		);
		TREASURY_VAULT = _treasury;
		emit SetTreasury(block.timestamp, msg.sender);
	}

	modifier treasuryIsSet() {
		require(
			address(TREASURY_VAULT) != address(0),
			"MagicPrinter: Treasury address is not set"
		);
		_;
	}

	/**
	 * @dev Set FEE amount. NOTE: 5% FEE is FEE_DENOM * 0.05 = 500
	 */
	function setPrinterValues(uint256 _fee, uint256 _magicNumber) public
		onlyOwner
	{
		FEE = _fee;
	...
	modifier printerValuesSet() {
		require(
			FEE != 0 
			&& MAGIC_NUMBER != 0,
			"MagicPrinter: Values not set"  
		);
		_;
	}
	...
	modifier onlyDonkeyHolders() {
		require(
			...
			"MagicPrinter: User is not a donkey holder"
		);
		_;
	}

	function isAllowListed(address _user) public view returns (bool) {
		return exploration.numberOfDonkeysStakedForUser(msg.sender) > 0;
	}
}
...
import "../interfaces/IMagicPrinter.sol";
import "../interfaces/IAtlasMine.sol";
import "../interfaces/IBattleflyFlywheelVaultV02.sol";
import "../interfaces/IBattleflyAtlasStakerV02.sol";

abstract contract MagicPrinterState is
	Initializable,
	IMagicPrinter,
	...
	ReentrancyGuardUpgradeable
{
	...
	// 5%
	uint256 public FEE;
	uint256 public MAGIC_NUMBER;

	IBattleflyAtlasStakerV02 public flywheelVault;
	IExploration public exploration;
	address public TREASURY_VAULT;

	IERC20Upgradeable public MAGIC;
	mapping(address => EnumerableSetUpgradeable.UintSet) internal depositsByUser;
	mapping(uint256 => UserDeposit) internal deposits;
	uint256 public totalDeposits;

	event ExplorationDeposit(
		address _user,
		uint256 _amount,
		uint256 _depositId,
		IAtlasMine.Lock _lock
	);
	event PrinterWithdrawal(
		address _user,
		uint256 _amount,
		uint256 _depositId
	);
	event PrinterClaimEmission(
		address _user,
		uint256 _amount,
		uint256 _depositId,
		uint64 _lock
	);
	event PrinerRequestWithdrawal(
		address _user,
		uint256 _depositId,
		uint256 _amount,
		IAtlasMine.Lock _lock
	...
	function __MagicPrinterState_init() internal initializer {
...
interface IExploration {
	...
	function numberOfDonkeysStakedForUser(address _owner) external
		view
		returns (uint256);
}
...
interface IMagicPrinter {
	function approveDonkeyMagic() external 
		returns (bool approved);

	function deposit(uint256 _amount, IAtlasMine.Lock _lock) external 
		returns (uint256 depositId);

	function withdraw(uint256[] memory _depositIds) external 
		returns (uint256 withdrawAmount); 
	...
	function hasUserDeposits(address _user) external
		returns (bool hasDeposits);
...
interface IBattleflyFlywheelVaultV02 {
    ...
    function withdraw(uint256[] calldata _depositIds)
        external
        returns (uint256);
    ...
    function canRequestWithdrawal(uint256 _depositId)
        external
        view
        returns (bool requestable);

    function canWithdraw(uint256 _depositId)
        external
        view
        returns (bool withdrawable);

    function initialUnlock(uint256 _depositId)
        external
        view
        returns (uint64 epoch);

    function retentionUnlock(uint256 _depositId)
        external
        view
        returns (uint64 epoch);
    ...
    function depositIdsOfUser(address user)
        external
        view
