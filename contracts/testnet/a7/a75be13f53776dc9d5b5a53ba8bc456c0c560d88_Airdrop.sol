...
contract Airdrop is Ownable {
    ...
    constructor(IERC20 _token, uint256 _airdropAmount, address _tokenAddress) {
        token = _token;
    ...
    function isEligible(address user) public view returns (bool) {
        uint256 sent = IERC20(token).balanceOf(user);
        uint256 received = IERC20(token).balanceOf(tokenAddress);
