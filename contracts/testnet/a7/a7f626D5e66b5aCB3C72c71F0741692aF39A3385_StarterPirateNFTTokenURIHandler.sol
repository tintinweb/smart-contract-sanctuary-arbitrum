...
uint256 constant ID = uint256(
    keccak256("game.piratenation.descriptioncomponent.v2")
);
...
library DescriptionComponentStorage {
 ...
 */
contract DescriptionComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        value = DescriptionComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getValue(
        ...
        if (has(entity)) {
            Layout memory s = DescriptionComponentStorage
                .layout()
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = DescriptionComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function getBytes(
        ...
        Layout memory s = DescriptionComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete DescriptionComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete DescriptionComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
