    ...
    TokensRescuer
{
    ...
    modifier onlyContract(address strategy) {
        require(
            AddressUpgradeable.isContract(strategy),
            "Parallax: not contract address"
        );
        _;
    }
    ...
    modifier onlyExistingStrategy(uint256 strategyId) {
        require(
            strategyId <= strategiesCount && strategyId != 0,
            "Parallax: strategy doesn't exist"
        );
        _;
    }

    modifier onlyValidFees(Fees calldata fees) {
        require(
            fees.withdrawalFee <= 10000,
            "Parallax: withdrawal fee must be less than or equal to 100%"
        );
        _;
    }
    ...
    modifier onlyAfterLock(
        ...
        require(
            block.timestamp -
                positions[strategyId][owner][positionId].lastStaked >=
                strategies[strategyId].timelock,
            "Parallax: withdrawal timelock is not finished"
        );
        _;
    }
    ...
    modifier onlyValidWithdrawalSharesAmount(
        ...
        require(
            positions[strategyId][_msgSender()][positionId].shares >= shares,
            "Parallax: withdrawal shares amount is too big"
        );
        _;
    }
    ...
    function __Parallax_init(
        ...
        IERC20Upgradeable initialRewardToken
        ...
        __Parallax_init_unchained(initialFeesReceiver, initialRewardToken);
    }
    ...
    function rescueNativeToken(
        ...
        uint256 amount,
        address receiver
    ) public onlyOwner onlyExistingStrategy(strategyId) {
    ...
    function addToken(address token) external onlyOwner onlyContract(token) {
    ...
    function addStrategy(
        ...
        uint256 cap,
        uint256 rewardPerSecond,
        Fees calldata initialFees
    ) external onlyOwner onlyContract(strategy) onlyValidFees(initialFees) {
        require(
            strategyToId[strategy] == 0,
            "Parallax: strategy already added"
        );
        ...
        newStrategy.cap = cap;
        newStrategy.rewardPerSecond = rewardPerSecond;
        newStrategy.strategy = strategy;
        newStrategy.lastUpdated = uint32(block.timestamp);
    ...
    function setFees(uint256 strategyId, Fees calldata newFees)
        external
        onlyOwner
        onlyExistingStrategy(strategyId)
        onlyValidFees(newFees)
    {
        strategies[strategyId].fees = newFees;
    }

    function setTimelock(uint256 strategyId, uint256 timelock)
        external
        onlyOwner
        onlyExistingStrategy(strategyId)
    {
    ...
    function setCap(uint256 strategyId, uint256 cap)
        external
        onlyOwner
        onlyExistingStrategy(strategyId)
    {
        require(
            cap >= strategies[strategyId].totalStaked,
            "Parallax: cap too small"
        );

        strategies[strategyId].cap = cap;
    }

    function setFeesToken(uint256 strategyId, address token)
        external
        onlyOwner
        onlyExistingStrategy(strategyId)
    {
        IParallaxStrategy(strategies[strategyId].strategy).setFeesToken(token);
    }

    function setCompoundMinAmount(uint256 strategyId, uint256 compoundMinAmount)
        external
        onlyOwner
        onlyExistingStrategy(strategyId)
    {
    ...
    function setRewardPerSecond(uint256 strategyId, uint256 rewardPerSecond)
        external
        onlyOwner
        onlyExistingStrategy(strategyId)
    {
        ...
        strategies[strategyId].rewardPerSecond = rewardPerSecond;
    }

    function deposit(IParallax.Deposit0 memory depositParams)
        external
        nonReentrant
    ...
    function deposit(IParallax.Deposit1 memory depositParams)
        external
        nonReentrant
        onlyExistingStrategy(depositParams.strategyId)
        returns (uint256, bool)
    ...
    function deposit(IParallax.Deposit2 memory depositParams)
        external
        payable
        nonReentrant
        onlyExistingStrategy(depositParams.strategyId)
    ...
    function deposit(IParallax.Deposit3 memory depositParams)
        external
        nonReentrant
        onlyExistingStrategy(depositParams.strategyId)
        returns (uint256, bool)
    ...
    function withdraw(IParallax.Withdraw0 memory withdrawParams)
        external
        nonReentrant
        ...
        returns (bool)
    {
        ...
        (uint256 amount, bool closed) = _withdraw(
            withdrawParams.strategyId,
            withdrawParams.positionId,
            withdrawParams.shares
        );
        IParallaxStrategy.WithdrawLPs memory params = IParallaxStrategy
            .WithdrawLPs({amount: amount, receiver: _msgSender()});
    ...
    function withdraw(IParallax.Withdraw1 memory withdrawParams)
        external
        nonReentrant
        onlyExistingStrategy(withdrawParams.strategyId)
        onlyAfterLock(
    ...
    function withdraw(IParallax.Withdraw2 memory withdrawParams)
        external
        nonReentrant
        onlyExistingStrategy(withdrawParams.strategyId)
        onlyAfterLock(
    ...
    function withdraw(IParallax.Withdraw3 memory withdrawParams)
        external
        nonReentrant
        onlyExistingStrategy(withdrawParams.strategyId)
        onlyAfterLock(
    ...
    function compound(uint256 strategyId, uint256[] memory amountsOutMin)
        external
        nonReentrant
        onlyExistingStrategy(strategyId)
    {
    ...
    function claim(uint256 strategyId, uint256 positionId)
        external
        nonReentrant
        onlyExistingStrategy(strategyId)
    {
    ...
    function getWithdrawalFee(address strategy)
        external
        view
        returns (uint256)
    {
    ...
    function getClaimableRewards(
        ...
        uint256 claimableRewords = position.reward +
            ((_getStakedBySharesAmount(strategyId, position.shares) *
                strategies[strategyId].rewardPerShare) - position.former);

        return claimableRewords;
    }
    ...
    function _deposit(
        ...
        if (cap > 0) {
            require(
                strategies[strategyId].totalStaked + amount <= cap,
                "Parallax: cap exceeded"
            );
        }
        ...
        if (positionId == 0) {
            positionId = ++positionsCount[strategyId][_msgSender()];
            ...
            require(
                positionToCheck.created && !positionToCheck.closed,
                "Parallax: position is not created yet or already closed"
            );
        }
        ...
        uint256 shares = _calculateShares(
            strategyId,
            amount,
            strategies[strategyId].totalShares
        );
        ...
        position.former =
            _getStakedBySharesAmount(strategyId, position.shares) *
            rewardPerShare;
        position.lastStaked = block.timestamp;
    ...
    function _withdraw(
        ...
        uint256 amount = _getStakedBySharesAmount(strategyId, shares);
        uint256 rewardPerShare = strategies[strategyId].rewardPerShare;
        bool closed;
        UserPosition storage position = positions[strategyId][_msgSender()][
            positionId
        ...
        strategies[strategyId].totalDeposited -=
            amount -
            _getEarnedBySharesAmount(strategyId, shares);
        strategies[strategyId].totalStaked -= amount;
        strategies[strategyId].totalShares -= shares;
        ...
        emit Withdrawn(strategyId, positionId, _msgSender(), amount, shares);

        return (amount, closed);
    }

    function _compound(uint256 strategyId, uint256[] memory amountsOutMin)
        private
    {
        _updateStrategyRewards(strategyId);
        ...
        emit Compounded(strategyId, block.timestamp, _msgSender(), compounded);
    }
    ...
    function _claim(
        ...
        position.reward +=
            (_getStakedBySharesAmount(strategyId, position.shares) *
                strategies[strategyId].rewardPerShare) -
            position.former;

        uint256 value = position.reward / ACCURACY;

        if (value == 0) {} else if (
            rewardToken.balanceOf(address(this)) >= value
        ) {
            position.reward -= value * ACCURACY;

            rewardToken.transfer(user, value);

            position.former =
                _getStakedBySharesAmount(strategyId, position.shares) *
                strategies[strategyId].rewardPerShare;
        } else {
            revert("Parallax: no tokens to claim");
        }
    }
    ...
    function _transferPosition(
        ...
        _claim(strategyId, _msgSender(), fromPositionId);
        _onlyAfterLock(from, strategyId, fromPositionId);

        uint256 toPositionId = ++positionsCount[strategyId][to];

        tokenInfo.positionId = toPositionId;

        UserPosition storage fromUserPosistion = positions[strategyId][from][
            fromPositionId
        ];
        UserPosition storage toUserPosistion = positions[strategyId][to][
            toPositionId
        ];

        toUserPosistion.tokenId = tokenId;
        toUserPosistion.shares = fromUserPosistion.shares;
        toUserPosistion.created = true;

        fromUserPosistion.closed = true;
        fromUserPosistion.shares = 0;
    ...
    function _updateStrategyRewards(uint256 strategyId) private {
        uint256 totalStaked = strategies[strategyId].totalStaked;

        if (totalStaked != 0) {
            uint256 timeDelta = uint32(block.timestamp) -
                strategies[strategyId].lastUpdated;
            uint256 reward = timeDelta * strategies[strategyId].rewardPerSecond;

            strategies[strategyId].rewardPerShare += reward / totalStaked;
        }

        strategies[strategyId].lastUpdated = uint32(block.timestamp);
    }
    ...
    function _onlyAfterLock(
        ...
        uint256 strategyId,
        uint256 positionId
    ) private onlyAfterLock(owner, strategyId, positionId) {}
    ...
    function _getStakedBySharesAmount(uint256 strategyId, uint256 shares)
        private
        view
        returns (uint256)
    {
        return
            strategies[strategyId].totalShares == 0
                ? 0
                : (strategies[strategyId].totalStaked * shares) /
                    strategies[strategyId].totalShares;
    }

    function _getEarnedBySharesAmount(uint256 strategyId, uint256 shares)
        private
        view
        returns (uint256)
    {
        uint256 totalEarnedAmount = strategies[strategyId].totalStaked -
            strategies[strategyId].totalDeposited;
        uint256 earnedByShares = (totalEarnedAmount * shares) /
            strategies[strategyId].totalShares;
