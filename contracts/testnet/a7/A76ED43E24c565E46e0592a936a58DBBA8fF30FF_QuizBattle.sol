...
contract QuizBattle is Ownable
{
    ...
    mapping(bytes32 => bool) public CheckedSignature;
    ...
    */
    ...
    constructor(IERC20 addressTokenReward) 
    {
    ...
    /*
        ...
        ...
        120s => 1440s ==> x1 reward: set indexTime = 4, effectiveTime = 1440, coefficient = 1000 (1 * 1000);
    */
        ...
        )
    {
        ...
        isOpenBattle = (battleEndTime > timestamp) ? true : false;
        isStarted = (battleEndTime == battleDataOfUser.timestampEnd) ? true : false;
        isSubmited = battleDataOfUser.checkSubmit;

        data = new Question[](TotalQuestionOnBattle);

        if(battleDataOfUser.checkSubmit == true)
        {
            for(uint256 indexQuestion = 0; indexQuestion < TotalQuestionOnBattle; indexQuestion++)
            {
        ...
        else
        {
            for(uint256 indexQuestion = 0; indexQuestion < TotalQuestionOnBattle; indexQuestion++)
            {
                uint256 index = battleDataOfUser.questionListForBattle[indexQuestion];
                data[indexQuestion].Question = QuestionList[index].Question;
                data[indexQuestion].Answer0 = QuestionList[index].Answer0;
                data[indexQuestion].Answer1 = QuestionList[index].Answer1;
                data[indexQuestion].Answer2 = QuestionList[index].Answer2;
                data[indexQuestion].Answer3 = QuestionList[index].Answer3;
            }
        }
    }

    function GetListIndexQuestion(address user) public view returns(uint256[] memory listIndexQuestion)
    {
        listIndexQuestion = new uint256[](TotalQuestionOnBattle);
        UserBattleData storage battleDataOfUser = UserBattle[user];
        for(uint256 indexQuestion = 0; indexQuestion < TotalQuestionOnBattle; indexQuestion++)
        {
            listIndexQuestion[indexQuestion] = (block.timestamp < BattleEndTime && battleDataOfUser.timestampEnd == BattleEndTime) ? 
                                                    battleDataOfUser.questionListForBattle[indexQuestion] : 0;
        }
    }
