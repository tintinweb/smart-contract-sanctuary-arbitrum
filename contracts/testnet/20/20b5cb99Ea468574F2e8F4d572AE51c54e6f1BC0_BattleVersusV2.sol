...
contract BattleVersusV2 is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  mapping(uint16 => LootBoxRewardConfig) public lootBoxRewardConfigs;
  bool public awardLootBoxForLoss;
  ...
  constructor(
    ...
    bonusMultiplier = 6;
    awardLootBoxForLoss = false;
  }

  struct LootBoxRewardConfig {
  ...
  function fight(
    ...
    fightData.lpBonusMultiplier = _getLpBonusMultiplier(msg.sender);
  ...
  function updateLootboxForLoss(bool _awardLootBoxForLoss) external onlyAdmin {
    awardLootBoxForLoss = _awardLootBoxForLoss;
  }
  ...
  function updateLootboxConfig(
    ...
    lootBoxRewardConfigs[_upsetWins] = LootBoxRewardConfig(
      totalChance,
      _chances,
      _tokenIds,
      _minimumLevels
  ...
  function _beginVersusBattle(
    ...
    battleData.lpBonusMultiplier = _lpBonusMultiplier;
    ...
    for (uint8 traitId = 2; traitId < 8; traitId++) {
      ...
      if (traitBattleData.won) {
        battleData.wins++;
        if (traitBattleData.hasBonus) {
          battleData.upsetWins++;
        }
      }
    }
  ...
  function _tryDispenseLootBox(
    ...
    uint256 roll;
    LootBoxRewardConfig storage rewardsConfig = lootBoxRewardConfigs[
      _upsetWins
    ];
    // Get roll and random base
    (roll, randomBase) = Random.getNextRandom(_randomBase, 100000);
    ...
    if (
      roll < rewardsConfig.lootBoxTotalChance &&
      (awardLootBoxForLoss || _wins > 3)
    ) {
      for (uint256 i = 0; i < rewardsConfig.lootBoxChances.length; i++) {
        if (roll >= rewardsConfig.lootBoxChances[i]) {
          roll -= rewardsConfig.lootBoxChances[i];
          continue;
        }

        if (_attackerLevel < rewardsConfig.lootBoxMinimumLevels[i]) {
          roll = 0;
          continue;
        }

        // Assign token
        tokenId = rewardsConfig.lootBoxTokenIds[i];

        // Dispense LootBox
        LOOTBOX_DISPENSER.dispense(
          msg.sender,
          rewardsConfig.lootBoxTokenIds[i],
          1
        );
  ...
  function _traitBattle(
    ...
    if (traitBattleData.attackerTrait < traitBattleData.opponentTrait) {
      ...
      if (traitBattleData.won) {
        ...
        traitBattleData.hasBonus = false;
  ...
  function _calculateAnima(
    ...
    bool _withinRange,
    uint256 bonusLpMultiplier
  ) internal returns (uint256) {
    // Check if bonus should be given
    uint256 animaBonus = data.hasBonus
      ? ((animaReward *
        (_getAnimaMultiplier(MID - data.probability) + bonusLpMultiplier)) /
        PRECISION)
      : 0;
    ...
    emit ContenderRoll(
      ...
      _winnerId,
      data.traitId,
      data.hasBonus ? data.probability : UPPER - data.probability,
      anima,
      animaBonus,
      _attackerReward
    );
