...
contract RouterWrapper is AccessControl, IRouterWrapper {
    ...
    function _validateKeeper() private view {
        require(isKeeper[msg.sender], "RouterWrapper: Forbidden");
    }
        ...
        for (uint32 index = 0; index < params.length; index++) {
            ...
            if (tokenX.balanceOf(user) < amountToPay) {
                emit FailResolve(
                    ...
                    "RouterWrapper: Insufficient balance"
                );
                continue;
            }
            ...
            if (
                ...
                emit FailResolve(
                    currentParams.queueId,
                    "RouterWrapper: Incorrect allowance"
                );
                continue;
            ...
            if (params[index].register.shouldRegister) {
                ...
                {} catch Error(string memory reason) {
                    emit FailResolve(
                        currentParams.queueId,
                        "RouterWrapper: Registration failed"
                    );
                    continue;
                }
            }
            ...
            IRouter routerContract = IRouter(routerAddress);

            try
                routerContract.runParameterChecks(currentParams, signer)
            {} catch Error(string memory reasonForCheckFailure) {
                emit FailResolve(currentParams.queueId, reasonForCheckFailure);
                continue;
            }

            // Transfer the fee to the admin
            IERC20(currentParams.tokenAddress).safeTransferFrom(
            ...
            IERC20(currentParams.tokenAddress).safeTransferFrom(
                currentParams.player,
                address(currentParams.targetContract),
                currentParams.wager * currentParams.numBets
            );

            try routerContract.initiatePlay(currentParams) {} catch Error(
                string memory reasonForOpenFailure
            ) {
                emit FailResolve(currentParams.queueId, reasonForOpenFailure);
                continue;
            }
    ...
    function refund(
        ...
        IRouter router = IRouter(routerContract);
        for (uint32 index = 0; index < refundParams.length; index++) {
            try
                router.refund(
                    refundParams[index].user,
                    refundParams[index].targetContract
                )
            {} catch Error(string memory reason) {
                emit FailRefund(
                    refundParams[index].user,
                    refundParams[index].targetContract,
                    "RouterWrapper: Refund failed"
                );
                continue;
            }
            emit Refund(
        ...
        for (uint256 index = 0; index < revokeParams.length; index++) {
            ...
            if (nonceAfter != nonceBefore + 1) {
                emit FailRevoke(
                    params.user,
                    params.tokenX,
                    "RouterWrapper: Permit did not succeed"
                );
            }
            emit RevokeRouter(
                params.user,
    ...
    function _routerValidations(
        ...
        if (!contractRegistry[params.targetContract]) {
            return (false, "RouterWrapper: Unauthorized contract");
        }

        if (queuedBets[params.queueId].player != address(0)) {
            return (false, "RouterWrapper: Bet exists");
        }

        // Check if the signature has already been used
        if (prevSignature[params.signature]) {
            return (false, "RouterWrapper: Signature already used");
        }
    ...
    function _approveViaSignature(
        ...
        if (nonceAfter != nonceBefore + 1) {
            emit FailResolve(queueId, "RouterWrapper: Permit did not succeed");
            return false;
        }
        emit ApproveRouter(user, nonceBefore, permit.value, permit.deadline, tokenX);
        return true;
    }
}
