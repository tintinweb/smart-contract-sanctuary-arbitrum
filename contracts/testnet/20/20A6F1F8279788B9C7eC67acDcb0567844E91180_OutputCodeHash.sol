...
interface IPancakeV3Factory {
    ...
    function setInnerBribe(address pool, address innerBribe) external;
...
interface IPancakeV3PoolOwnerActions {
    ...
    /// @notice Set the InnerBribe 
    function setInnerBribe(address _innerBribe) external;
}
...
contract PancakeV3Pool is IPancakeV3Pool {
    ...
    Oracle.Observation[65535] public override observations;
     ...
     /// innerBribe
    IBribe public innerBribe;

    event SetInnerBribeEvent(address _innerBribe);
    ...
    function swap(
        ...
        _collectProtocol(protocolFees.token0,protocolFees.token1);
        slot0.unlocked = true;
    }
    ...
    function setFeeProtocol(uint32 feeProtocol0, uint32 feeProtocol1) external override lock onlyFactoryOrFactoryOwner {
        require(
            (feeProtocol0 == 0 || (feeProtocol0 >= 1000 && feeProtocol0 <= 99000)) &&
            (feeProtocol1 == 0 || (feeProtocol1 >= 1000 && feeProtocol1 <= 99000))
        );
    ...
    function collectProtocol(
       ...
       return _collectProtocol(amount0Requested,amount1Requested);
    }
    function _collectProtocol(
        ...
        if (amount0 > 0) {
            ...
            innerBribe.notifyRewardAmount(token0,amount0);
        }
        if (amount1 > 0) {
            if (amount1 == protocolFees.token1) amount1--; // ensure that the slot is not cleared, for gas savings
            protocolFees.token1 -= amount1;
            innerBribe.notifyRewardAmount(token1,amount1);
        }

        emit CollectProtocol(msg.sender, address(innerBribe), amount0, amount1);
    }

    function setInnerBribe(address _innerBribe) external override onlyFactoryOrFactoryOwner {
      innerBribe = IBribe(_innerBribe);
      emit SetInnerBribeEvent(address(_innerBribe));
    }
}
