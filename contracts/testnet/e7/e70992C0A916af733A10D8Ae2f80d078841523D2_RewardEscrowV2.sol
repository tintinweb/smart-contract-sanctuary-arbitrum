...
contract BaseRewardEscrowV2 is Owned, IRewardEscrowV2, LimitedSetup(8 weeks), MixinResolver {
    ...
    /* An account's total escrowed synthr balance to save recomputing this for fee extraction purposes. */
    mapping(address => uint256) public totalEscrowedAccountBalance;

    /* An account's total vested reward synthr. */
    mapping(address => uint256) public totalVestedAccountBalance;

    /* Mapping of nominated address to recieve account merging */
    mapping(address => address) public nominatedReceiver;

    /* The total remaining escrowed balance, for verifying the actual synthr balance of this contract against. */
    ...
    function resolverAddressesRequired() public view virtual override returns (bytes32[] memory addresses) {
        ...
        addresses[0] = CONTRACT_WRAPPED_SYNTHR;
     ...
     * @dev A call to this should accompany a previous successful call to synthr.transfer(rewardEscrow, amount),
