...
contract AntiBotBuybackBabyToken is IERC20Extended, OwnableUpgradeable, BaseToken {
    using SafeMath for uint256;
...
contract AntiBotBuybackBabyTokenFactory is TokenFactoryBase {
    using Address for address payable;
    AntiBotBuybackBabyToken[] public antiBotBuybackBabyToken;
    ...
    function create(
        ...
        AntiBotBuybackBabyToken tokenAddress = new AntiBotBuybackBabyToken();
        token = address(tokenAddress);
        AntiBotBuybackBabyToken(payable(token)).initialize(
            msg.sender,
            name,
            symbol,
            totalSupply,
        ...
        antiBotBuybackBabyToken.push(AntiBotBuybackBabyToken(payable(token)));
        assignTokenToOwner(msg.sender, token, 6);
        emit TokenCreated(msg.sender, token, TokenType.buybackBaby, 1);
    }

    function getTokens() public view returns(AntiBotBuybackBabyToken[] memory) {
        return antiBotBuybackBabyToken;
    }
}