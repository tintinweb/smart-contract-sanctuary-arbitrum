...
contract AMM is Message, ReentrancyGuard, ILayerZeroReceiver {
    ...
    function burnVouchers(uint16 destChainId, uint256 amount0, uint256 amount1) external payable nonReentrant {
        ...
        if (amount0 > 0) {
            ...
            bytes memory payload = abi.encode(MessageType.BurnVoucher, L1Token0, msg.sender, amount0);
        ...
        if (amount1 > 0) {
            ...
            bytes memory payload = abi.encode(MessageType.BurnVoucher, L1Token1, msg.sender, amount1);
            lzEndpoint.send{value: fee}(
