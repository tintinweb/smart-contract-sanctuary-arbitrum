...
contract ElleriaBidManager is Ownable, ReentrancyGuard {
  ...
  function ReduceUsdcCost(uint256 _usdcCostInWEI) external onlyOwner {
    ...
    for (uint256 i = 0; i < bidCounter; i += 1) {
      if (bids[i].remainingBidQuantity > 0) {
        ...
        usdcAbi.transferFrom(address(this), bids[i].owner, refundAmount);
      }
    }
  }
  ...
  function ConsumeBid(uint256 _bidId, uint256 quantity, uint256 _variant) external onlyOwner onlyIfBidValid(_bidId) {
    ...
    uint256 elmPrice = usdcCostInWEI * quantity;
  ...
  function CreateBid(uint256 _elmAmountInWEI, uint256 _usdcAmountInWEI, uint256 quantity) external nonReentrant {
    elleriumAbi.transferFrom(msg.sender, address(this), _elmAmountInWEI * quantity);
    usdcAbi.transferFrom(msg.sender, address(this), _usdcAmountInWEI * quantity);

    bids[bidCounter] = Bid(
      msg.sender,
      _elmAmountInWEI * quantity,
      _usdcAmountInWEI * quantity,
      _elmAmountInWEI,
      quantity,
      false
    );

    emit BidCreated(msg.sender, _elmAmountInWEI, _usdcAmountInWEI, quantity, bidCounter++);
  }
  ...
  function refundBid(uint256 _bidId) internal {
    ...
    elleriumAbi.transferFrom(address(this), bids[_bidId].owner, bids[_bidId].elmBalance);
    usdcAbi.transferFrom(address(this), bids[_bidId].owner, bids[_bidId].usdcBalance);
