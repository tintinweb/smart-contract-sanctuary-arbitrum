...
contract PositionRouter is BaseRouter, IPositionRouter, ReentrancyGuard {
    ...
    */
    ...
    function _modifyPosition(
        ...
        //Position struct has 13 val, min length will be 13 * 64
        if (_isOpenPosition(_txType) && _data.length > 13 * 64) {
        ...
        if (_isOpenPosition(_txType)) {
            ...
            data = abi.encode(position.isLong, params, order);
        } else if (_txType == UPDATE_TRAILING_STOP) {
            data = abi.encode(position.isLong, order);
