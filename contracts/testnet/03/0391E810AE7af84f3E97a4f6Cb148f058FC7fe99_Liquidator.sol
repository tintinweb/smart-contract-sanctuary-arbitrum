...
contract Liquidator is Owned, MixinSystemSettings, ILiquidator {
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        ...
        newAddresses[1] = CONTRACT_WRAPPED_SYNTHR;
    ...
    /// @dev An account with no SYNTH collateral will not be open for liquidation since the ratio is 0
    function isLiquidationOpen(address account, bool isSelfLiquidation) external view returns (bool) {
        ...
        if (!isSelfLiquidation) {
            ...
            // Open for liquidation if the deadline has passed and the user has enough SYNTH collateral.
            if (_deadlinePassed(liquidation.deadline) && _hasEnoughSYNTHR(account)) {
                return true;
            }
            return false;
        }
    ...
    /// @notice Checks if an account has enough SYNTHR balance to be considered open for forced liquidation.
    function _hasEnoughSYNTHR(address account) internal view returns (bool) {
    ...
    function calculateAmountToFixCollateral(
        ...
        uint256 dividend;
        if (debtBalance > collateral.multiplyDecimal(ratio)) {
            dividend = debtBalance.sub(collateral.multiplyDecimal(ratio));
        }
    ...
    // check synthr rate is not stale
    function flagAccountForLiquidation(address account) external {
    ...
    /// @dev This function checks if the account's c-ratio is OK and that the rate of SYNTH is not stale
    function checkAndRemoveAccountInLiquidation(address account) external {
        systemStatus().requireSystemActive();
