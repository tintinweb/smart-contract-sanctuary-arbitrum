...
contract Transcendence is ManagerModifier, ReentrancyGuard, Pausable {
  ...
  constructor(
    ...
    uint256 _minArchetypeId,
    uint256 _maxArchetypeId,
    uint256 _maxProfession,
    bytes32 _merkleRoot
    ...
    minArchetypeId = _minArchetypeId;
    maxArchetypeId = _maxArchetypeId;
  ...
  function transcend(
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      // Check archetypeId is valid
      require(
        archetypeId >= minArchetypeId && archetypeId <= maxArchetypeId,
        "Transcendence: Invalid archetypeId"
      );
      ...
      require(
        isValidLeaf,
        "Transcendence: Variant class does not match Adventurer class"
      );
  ...
  {
    return
      animaBaseCost + (animaIncrement * _adventurerLevel(_addr, _adventurerId));
  }
  ...
  function _adventurerLevel(address _addr, uint256 _adventurerId)
    ...
    returns (uint256)
  {
    // Returns Transcendence level
    return ADVENTURER_DATA.aovProperties(_addr, _adventurerId, 0, 1)[0];
  }
}
...
interface IAoV {
  ...
  function burnFor(uint256 _tokenId) external;
}
