...
contract Market is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    event NonFungibleTokenRecovery(address indexed token, uint256 indexed tokenId, uint256 collectionId);
    ...
    function modifyCollection(
        ...
        //require(_collectionAddressSet.contains(_collection), "Operations: Collection not listed");
        require(_collections[_collection][_collectionId].creatorAddress != address(0),"Operations: Collection not listed");
     ...
     * @dev Callable by owner
     */
    function recoverNonFungibleToken(address _token, uint256 _tokenId, uint256 _collectionId) external onlyOwner nonReentrant {
        require(!_askTokenIds[_token][_collectionId].contains(_tokenId), "Operations: NFT not recoverable");
    ...
    function buyTokens(BuyTokensDetails[] memory buyTokensDetail) external {
         for(uint256 i = 0; i < buyTokensDetail.length ; i++) {
            ...
            buyToken(collection, tokenId, price, collectionid);  
        }
...
//        listCollection.push(collectionId);
    ...
    function buyTicket (
       ...
       // listCollection[msg.sender] = collectionId;
 ...
 //   function getOwnerOfCollectionOFnumberTicket() public view returns(uint256[])
