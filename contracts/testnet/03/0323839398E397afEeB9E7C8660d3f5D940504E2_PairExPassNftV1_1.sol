...
contract PairExPassNftV1_1 is Ownable, Pausable, ERC1155, ERC1155Supply {
    ...
    mapping(uint => mapping(address => bool)) public whiteListForMint;
    ...
    function updateWhiteListForMint(uint256 tokenId, address addr, bool status) public onlyManager {
        require(isValidTokenId(tokenId), "INVALID_TOKENID");
        require(addr != address(0), "ADDRESS IS NIL");

        whiteListForMint[tokenId][addr] = status;
    }

    function batchUpdateWhiteListForMint(uint256 tokenId, address[] calldata addr, bool status) external onlyManager {
        for(uint i = 0; i < addr.length; i++){
            updateWhiteListForMint(tokenId, addr[i], status);
        }
    }
    ...
    function isInWhiteList(uint256 tokenId, address addr) public view returns(bool){
        return whiteListForMint[tokenId][addr];
    }
    ...
    function mint(uint256 id, uint256 amount) public notContract whenNotPaused {
        ...
        if(monthMintPrices[id] > 0 && !isInWhiteList(id, account)) {
