...
contract MercuryRegistry is ConfirmedOwner, AutomationCompatibleInterface, StreamsLookupCompatibleInterface {
  ...
  function packPriceDeviationAndStaleness(int192 price, uint32 stalenessSeconds, uint32 deviationPercentagePPM) 
    public pure returns (bytes32) {
      return bytes32(((uint256(uint192(price) << 32) | stalenessSeconds) << 32) | deviationPercentagePPM);
  }
