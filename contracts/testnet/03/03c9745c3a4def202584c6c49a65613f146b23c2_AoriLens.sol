...
contract AoriCall is ERC20, ReentrancyGuard {
    ...
    IERC20 public immutable USDC = IERC20(0x428152F54328B1adb787fb0B9Caf16f2C881C57A);
    uint256 public settlementPrice;
    uint256 public immutable feeMultiplier;
    uint256 public immutable decimalDiff;
    bool public hasEnded = false;
    AoriSeats public immutable AORISEATSADD; //joke test address
    ...
    function mintCall(uint256 quantityOfUNDERLYING, uint256 seatId) public nonReentrant returns (uint256) {
        ...
        require(UNDERLYING.decimals() == 18 && block.timestamp < endingTime); //safety check
        ...
        if (msg.sender == AORISEATSADD.ownerOf(seatId)) {
            ...
            SafeERC20.safeTransferFrom(UNDERLYING, msg.sender, address(this), optionsToMint);
    ...
    function buyerSettlementITM(uint256 optionsToSettle) public nonReentrant returns (uint256) {
        _setSettlementPrice();
        require(block.timestamp >= endingTime && balanceOf[msg.sender] >= optionsToSettle);
    ...
    function sellerSettlementITM() public nonReentrant returns (uint256) {
        ...
        //settle
        SafeERC20.safeTransfer(UNDERLYING, msg.sender, UNDERLYINGToReceive / 10**USDC.decimals());
    ...
    */
    function getPrice() public view returns (uint256) {
        ...
        if(price == 0) {
            ...
            return (uint256(price) / (10**(8 - USDC.decimals()))); 
        }
    }
    /** 
...
contract AoriPut is ERC20, ReentrancyGuard {
    ...
    uint256 public immutable decimalDiff;
    bool public hasEnded = false;
    IERC20 public USDC = IERC20(0x428152F54328B1adb787fb0B9Caf16f2C881C57A);
    AoriSeats public immutable AORISEATSADD; //joke test address
    ...
    function buyerSettlementITM(uint256 optionsToSettle) public nonReentrant returns (uint256) {
        _setSettlementPrice();
        require(block.timestamp >= endingTime && balanceOf[msg.sender] >= 0);
...
contract Ask is ReentrancyGuard {
    ...
    AoriSeats public immutable AORISEATSADD = AoriSeats(0x70a40baBDb3e2e9C6f5c76ADf8Cdb43D1Ac5C97B);
...
contract Bid is ReentrancyGuard {
    ...
    */
    function fill(uint256 amountOfOPTION, uint256 seatId) public nonReentrant {
        ...
        require(msg.sender != maker);
        require(!hasEnded, "offer has been previously been cancelled");
        require(endingTime <= endingTime, "This offer has expired");
...
contract CallFactory is Ownable {
    ...
    uint256 public fee;
    address public AORISEATSADD;

    constructor(address _AORISEATSADD) {
        AORISEATSADD = _AORISEATSADD;
    }
    ...
    */
    ...
    function setAORISEATSADD(address newAORISEATSADD) external onlyOwner returns(address) {
    ...
    */
    function createCallMarket(
        ...
        AoriCall callMarket = new AoriCall(AoriSeats(AORISEATSADD).getFeeMultiplier(), strikeInUSDC, duration, UNDERLYING, oracle, AoriSeats(AORISEATSADD), name_, symbol_);
    ...
    function getAllCallMarkets() external view returns(address[] memory) {
        return callMarkets;
    }
}
...
contract AoriLens {
    ...
    {
        return (ask.getOPTION(), ask.USDCPerOPTION(), ask.endingTime(), ask.getCurrentBalance());
    }
    //Get specific information about a certain ask
    ...
    {
        return (bid.getUSDC(), bid.OPTIONPerUSDC(), bid.endingTime(), bid.getCurrentBalance());
    }
    ...
    function getActiveCallMarkets(CallFactory callFactory) public view returns (address[] memory) {
        address[] memory allCalls = callFactory.getAllCallMarkets();
        address[] memory activeCalls = new address[](allCalls.length);

        uint256 activeCount;
        for(uint i; i < allCalls.length; i++) {
            if(AoriCall(allCalls[i]).endingTime() >= block.number) {
    ...
    function getActivePutMarkets(PutFactory putFactory) public view returns (address[] memory) {
        address[] memory allPuts = putFactory.getAllPutMarkets();
        address[] memory activePuts = new address[](allPuts.length);

        uint256 activeCount;
        for(uint i; i < allPuts.length; i++) {
            if(AoriCall(allPuts[i]).endingTime() >= block.number) {
...
contract Orderbook is Ownable {
    ...
    function createAsk(uint256 _USDCPerOPTION, uint256 _duration, uint256 _OPTIONSize) public returns (Ask) {
        Ask ask = new Ask(OPTION, USDC, msg.sender, _USDCPerOPTION, fee_, AoriSeats(AORISEATSADD).getFeeMultiplier() , _duration, _OPTIONSize);
        asks.push(ask);
    ...
    function createBid(uint256 _OPTIONPerUSDC, uint256 _duration, uint256 _USDCSize) public returns (Bid) {
        Bid bid = new Bid(USDC, OPTION, msg.sender, _OPTIONPerUSDC, fee_, AoriSeats(AORISEATSADD).getFeeMultiplier() , _duration, _USDCSize);
        bids.push(bid);
...
contract OrderbookFactory is Ownable {
    ...
    */
     ...
     */
    function setTradingFee(uint256 newFee) external onlyOwner returns(uint256) {
        fee = newFee;
        return fee;
    }
    ...
    */
    function createOrderbook(
        ...
        Orderbook orderbook =  new Orderbook(fee, OPTION_, AORISEATSADD, _duration); 
...
contract PutFactory is Ownable {
    ...
    */
    function createPutMarket(
        ...
        AoriPut putMarket = new AoriPut(AoriSeats(AORISEATSADD).getFeeMultiplier(), strikeInUSDC, duration, oracle, AoriSeats(AORISEATSADD), name_, symbol_);
    ...
    function getAllPutMarkets() external view returns(address[] memory) {
        return putMarkets;
    }
}