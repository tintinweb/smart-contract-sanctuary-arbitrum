...
contract BidFomo is Caller, ReentrancyGuard {
    ...
    function conqueringReward() external view returns (uint256) {
        if (pool.period == 0) {
            return 0;
        }
        uint256 startAt = pool.bitAt == 0 ? pool.startTime : pool.bitAt;
        return (block.timestamp - startAt) * rewardPerSecond;
    }
    ...
    function withdraw(bool all) external {
        ...
        emit Withdrawal(_msgSender(), availableAmount);
        if (all) {
            ...
            for (uint256 i = 0; i < userBids[_msgSender()].length; i++) {
                if (
                    (block.timestamp - userBids[_msgSender()][i].startAt) <
                    3 days
                ) {
                    len++;
                }
            }
            UserBid[] memory newBids = new UserBid[](len);
            uint256 index = 0;
            for (uint256 i = 0; i < userBids[_msgSender()].length; i++) {
                if (
                    (block.timestamp - userBids[_msgSender()][i].startAt) <
                    ...
                    newBids[index] = userBids[_msgSender()][i];
                    index++;
                }
            }
            delete userBids[_msgSender()];
            for (uint256 i = 0; i < newBids.length; i++) {
                userBids[_msgSender()].push(newBids[i]);
            }
        }
    }
    ...
    function bid(string memory message) external {
        ...
        if (pool.bidder != address(0)) {
            uint256 reward = (block.timestamp - pool.bitAt) * rewardPerSecond;
            if (reward > 0 && boboToken.balanceOf(address(this)) >= reward) {
                boboToken.transfer(pool.bidder, reward);
            }
