...
contract Vault is ReentrancyGuard, IVault {
    ...
    constructor(address _dlp) public {
        gov = msg.sender;
        dlp = _dlp;
    }
    ...
    function setFees(
        ...
        require(_swapFeeBasisPoints <= MAX_FEE_BASIS_POINTS, "swapFeeBasisPoints should be smaller than MAX");
        require(_stableSwapFeeBasisPoints <= MAX_FEE_BASIS_POINTS, "stableSwapFeeBasisPoints should be smaller than MAX");
    ...
    function directPoolDeposit(address _token) external override nonReentrant {
        require(whitelistedTokens[_token], "This toke is not whitelisted");
    ...
    function liquidatePosition(address _account, address _collateralToken, address _indexToken, bool _isLong, address _feeReceiver) external override nonReentrant {
        if (inPrivateLiquidationMode) {
            require(isLiquidator[msg.sender], "This user is not Liquidator");
        }
        ...
        require(liquidationState != 0, "liquidationState is not zero");
    ...
    function addLiquidity(address _fundingAccount, address _account, address _token, uint256 _amount) external {
        require(_amount > 0, "Vault: invalid _amount");
        IERC20(_token).safeTransferFrom(_fundingAccount, address(this), _amount);
        uint256 price = getMinPrice(_token);
        uint256 afterFeeAmount = _amount.mul(BASIS_POINTS_DIVISOR.sub(liquidityFeeBasisPoints)).div(BASIS_POINTS_DIVISOR);
        uint256 mintAmount = afterFeeAmount.mul(price).div(PRICE_PRECISION);
        _increasePoolAmount(_token, afterFeeAmount);
        liquidityAmount[_token][_account] = liquidityAmount[_token][_account].add(afterFeeAmount);
        IMintable(dlp).mint(_account, mintAmount);
        emit AddLiquidity(_account, _token, afterFeeAmount, mintAmount);
    }

    function removeLiquidity(address _account, address _tokenOut, uint256 _dlpAmount, address _receiver) external {
        require(_dlpAmount > 0, "Vault: invalid _dlpAmount");
        uint256 price = getMaxPrice(_tokenOut);
        require(price > 0, "Price not Exist");
        IMintable(dlp).burn(_account, _dlpAmount);
        uint256 amountOut = _dlpAmount.mul(PRICE_PRECISION).div(price);
        uint256 afterFeeAmount = amountOut.mul(BASIS_POINTS_DIVISOR.sub(liquidityFeeBasisPoints)).div(BASIS_POINTS_DIVISOR);
        _decreasePoolAmount(_tokenOut, afterFeeAmount);
        liquidityAmount[_tokenOut][_account] = liquidityAmount[_tokenOut][_account].sub(amountOut);
        IERC20(_tokenOut).transfer(_receiver, afterFeeAmount);
        emit RemoveLiquidity(_account, _tokenOut, _dlpAmount, amountOut);
    }
}
