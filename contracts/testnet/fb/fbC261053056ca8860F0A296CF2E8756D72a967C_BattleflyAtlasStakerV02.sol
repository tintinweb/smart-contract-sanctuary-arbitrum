    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function requestWithdrawal(uint256 _depositId)
        public
        override
        onlyWhitelistedVaults
        nonReentrant
        ...
        returns (uint64)
    {
        ...
        // We set the retention period before the withdrawal can happen to 14 days (lock period) + 2 extra days
        // to take timing issues into account due to the cronjob executed transactions.
        vaultStake.retentionUnlock = currentEpoch + 14;
    ...
    function getClaimableEmission(uint256 _depositId) public view override returns (uint256 emission, uint256 fee) {
        ...
        if (currentEpoch > 0) {
            ...
            uint64 x = vaultStake.retentionUnlock == 0 ? 0 : 1;
            emission =
    ...
    modifier whenNotPaused() {
        require(!paused, "BattleflyAtlasStaker: contract paused");
        _;
    }
}
...
interface IBattleflyAtlasStakerV02 {
    ...
    event AddedVault(address vault, uint16 fee, uint16);
    event RemovedVault(address vault);
    ...
    event NewDeposit(address vault, uint256 amount, uint256 unlockedAt);
    event WithdrawPosition(address vault, uint256 amount, uint256 depositId);
    event ClaimEmission(address vault, uint256 amount, uint256 depositId);
    event RequestWithdrawal(address vault, uint64 withdrawalEpoch, uint256 depositId);
...
library BattleflyAtlasStakerUtils {
    ...
    function getLockPeriod(IAtlasMine.Lock _lock, IAtlasMine ATLAS_MINE) external pure returns (uint64) {
        ...
        if (_lock == IAtlasMine.Lock.threeMonths) {
            return 13 weeks + 1 days + uint64(ATLAS_MINE.getVestingTime(_lock));
        }
        if (_lock == IAtlasMine.Lock.sixMonths) {
            return 23 weeks + 1 days + uint64(ATLAS_MINE.getVestingTime(_lock));
        }
