...
abstract contract AccountRoleVerifier {
    /// @dev Address of the account NFT contract.
    ...
    AccessControlRolesUpgradeable
{
    ...
    modifier onlyAccountRole(
        ...
        consumeRoleSignature(accountId, accountUser, authorisedRole, signature);
        _;
    }
     ...
     */
    function validateRoleSignature(
        uint256 id,
        address accountUser,
        AccountRole accountUserRole,
        bytes calldata signature
    ) public view {
        validateRolePermission(id, accountUser, accountUserRole);
     ...
     * @param signature The user signature, with payload: `{id}{nonce}{role}`.
     */
    function consumeRoleSignature(
        uint256 id,
        address accountUser,
        AccountRole accountUserRole,
        bytes calldata signature
    ) public {
        validateRoleSignature(id, accountUser, accountUserRole, signature);
    ...
    function grantAccountUserRole(
        ...
        validateRoleManagement(id, user, role, owner, ownerSignature);
        require(
    ...
    function validateRoleManagement(
        uint256 id,
        address user,
        AccountRole role,
        address owner,
...
import "@openzeppelin/contracts-upgradeable/security/ReentrancyGuardUpgradeable.sol";
import "./abstract/AccessControlRolesUpgradeable.sol";
import "./abstract/AccountRoleVerifier.sol";
import "./types/AccountRoles.sol";
import "./Account.sol";
