...
uint256 constant ID = uint256(
    keccak256("game.piratenation.trustedvalidatorcomponent.v2")
);
...
struct Layout {
    ...
    bool hasSsl;
    address serverPublicAddress;
}

library TrustedValidatorComponentStorage {
...
/**
 * @title TrustedValidatorComponent
 * @dev A public trusted validators
 */
contract TrustedValidatorComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Domain for a trusted validators
        keys[0] = "host";
        values[0] = TypesLibrary.SchemaValue.STRING;

        // Port for a trusted validator
        keys[1] = "port";
        values[1] = TypesLibrary.SchemaValue.UINT16;

        // Test flag for experimental multiplayer features for validators
        keys[2] = "has_multiplayer_pirates";
        values[2] = TypesLibrary.SchemaValue.BOOL;

        // Flag for WSS protocol for trusted validators
        keys[3] = "has_ssl";
        values[3] = TypesLibrary.SchemaValue.BOOL;

        // Public addresses for a trusted validator
        keys[4] = "server_public_address";
     ...
     *
     * @param entity Entity to get value for
     * @param host Domain for a trusted validators
     * @param port Port for a trusted validator
     * @param hasMultiplayerPirates Test flag for experimental multiplayer features for validators
     * @param hasSsl Flag for WSS protocol for trusted validators
     * @param serverPublicAddress Public addresses for a trusted validator
     */
    function setValue(
        uint256 entity,
    ...
    function getLayoutValue(
        ...
        value = TrustedValidatorComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return host Domain for a trusted validators
     * @return port Port for a trusted validator
     * @return hasMultiplayerPirates Test flag for experimental multiplayer features for validators
     * @return hasSsl Flag for WSS protocol for trusted validators
     * @return serverPublicAddress Public addresses for a trusted validator
     */
    function getValue(
        uint256 entity
    )
    ...
    {
        if (has(entity)) {
            Layout memory s = TrustedValidatorComponentStorage
                .layout()
            ...
            (
                ...
                _getEncodedValues(s),
                (string, uint16, bool, bool, address)
            );
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = TrustedValidatorComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](5);
        values[0] = abi.encode(s.host);
        values[1] = abi.encode(s.port);
        values[2] = abi.encode(s.hasMultiplayerPirates);
        values[3] = abi.encode(s.hasSsl);
        values[4] = abi.encode(s.serverPublicAddress);
    }
    ...
    function setBytes(
        ...
        (
            ...
            s.hasSsl,
            s.serverPublicAddress
        ) = abi.decode(value, (string, uint16, bool, bool, address));
    ...
    function remove(
        ...
        delete TrustedValidatorComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.serverPublicAddress = value.serverPublicAddress;
    }
