...
contract FeeToVault is Ownable2StepUpgradeable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    function bridgeHOPE(address bridge, address L1HopeToken, uint amount) external payable whenNotPaused onlyRole(OPERATOR_ROLE) {
        require(bridge != address(0), "zero address");
        require(L1HopeToken != address(0), "zero address");
        ...
        IBridge(bridge).sendFrom(address(this), HOPE, 1, bytes32(abi.encode(L1HopeToken)), bytes32(abi.encode(underlyingBurner)), amount, lzCallParam);
    }
