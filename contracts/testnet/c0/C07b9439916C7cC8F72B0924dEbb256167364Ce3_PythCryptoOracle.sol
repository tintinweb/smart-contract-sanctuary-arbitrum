...
contract PythCryptoOracle is OracleConnector {
    ...
    event CoreUtilsUpdated(address indexed coreUtils);
    event PriceUpdated(bytes[] data, uint256 price, uint256 timestamp);
    ...
    constructor(
        ...
        pyth = IPyth(pyth_);
        priceId = priceId_;
        _updateCoreUtils(coreUtils_);
    }

    function updateCoreUtils(address coreUtils_) external onlyOwner returns (bool) {
        _updateCoreUtils(coreUtils_);
        return true;
    }
    ...
    function updatePrice(
        ...
        require(msg.sender == coreUtils, "PythCryptoOracle: Caller is not core utils");
        ...
        price = convertToUint(currentPrice, decimals);
        timestamp = currentPrice.publishTime;
        emit PriceUpdated(updateData, price, timestamp);
    }
    ...
    function _updateCoreUtils(address coreUtils_) private {
        require(coreUtils_ != address(0), "PythCryptoOracle: Core utils is zero address");
        coreUtils = coreUtils_;
        emit CoreUtilsUpdated(coreUtils_);
    }
}