...
contract RewardRouter is Initializable {
    ...
    function updatePexMinNewRewardAmount(uint _minNewRewardAmount) external{
        PEXStakingVar.minNewRewardAmount = _minNewRewardAmount;
    }

    function updatePexUnStakingLockDuration(uint _unStakingLockDuration) external{
        PEXStakingVar.unStakingLockDuration = _unStakingLockDuration;
    }

    function rewardSettlementPEX(uint _newRewardAmount) external {
        StakingLib.stakingVar storage self = PEXStakingVar;
        requireGov(self);
        ...
        for (uint i = 0; i < self.addressStaking.length; i++) {
            self.userStaking[self.addressStaking[i]].rewardAmount += getNewRewardRewardRouter(
                _newRewardAmount, self.userStaking[self.addressStaking[i]].stakingAmount
            );
        }
    ...
    function updateEsPexMinNewRewardAmount(uint _minNewRewardAmount) external{
        esPEXStakingVar.minNewRewardAmount = _minNewRewardAmount;
    }

    function updateEsPexUnStakingLockDuration(uint _unStakingLockDuration) external{
        esPEXStakingVar.unStakingLockDuration = _unStakingLockDuration;
    }

    function rewardSettlementEsPEX(uint _newRewardAmount) external {
        StakingLib.stakingVar storage self = esPEXStakingVar;
        requireGov(self);
