...
contract DeriLens is NameVersion {
    ...
    function getDepositTokenInfo(address pool_, address account_) public view returns (DepositTokenInfo [] memory infos) {
        ILensPool pool = ILensPool(pool_);
        ISymbolsLens.PriceAndVolatility[] memory pvs;
        ISymbolsLens.SymbolInfo[] memory symbolsInfo = symbolsLens.getSymbolsInfo(pool_, pvs);
        for (uint256 i = 0; i<symbolsInfo.length; i++){
            bytes32 vaultId = keccak256(abi.encodePacked(account_, symbolsInfo[i].symbol));
            infos[i].info = ILensVault(pool.userVault(vaultId)).getVaultTokenVolume();
            infos[i].symbol = symbolsInfo[i].symbol;
        }
    }
    ...
    function getTdInfo(address pool_, address account_) public view returns (TdInfo memory info) {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            info.positions[i].traderPnl = notional - p.cost;

            info.positions[i].margin = info.positions[i].vaultLiquidity.utoi() + info.amountB0 + info.positions[i].traderPnl;
            info.positions[i].marginUsed = info.positions[i].traderPnl < 0 ? info.positions[i].traderPnl.abs() : int256(0);
    ...
    function estimateAlpOutputFromAmountUsd(address pool_, address Alp_, uint256 amountUsd) public view returns (uint256 amountAlp) {
        ILensPool pool = ILensPool(pool_);
        IERC20 Alp = IERC20(Alp_);
        uint256 totalSupply = Alp.totalSupply();
        int256 availableLiquidity = pool.getLiquidity().utoi() + pool.lpsPnl();
        uint256 feeAmount = amountUsd * pool.mintFeeBasisPoints() / pool.BASIS_POINTS_DIVISOR();
        amountAlp = availableLiquidity == 0 ? amountUsd-feeAmount : (amountUsd-feeAmount) * totalSupply / availableLiquidity.itou();
        // return pool.getLiquidity();
    }

    function estimateAmountUsdOutputFromAlp(address pool_, address Alp_, uint256 amountAlp) public view returns (uint256 amountUsd) {
        ILensPool pool = ILensPool(pool_);
        IERC20 Alp = IERC20(Alp_);
        uint256 totalSupply = Alp.totalSupply();
        int256 availableLiquidity = pool.getLiquidity().utoi() + pool.lpsPnl();
        // return pool.liquidity().itou();
        uint256 amountOutUsd = amountAlp * availableLiquidity.itou() / totalSupply;
        uint256 feeAmount = amountOutUsd * pool.burnFeeBasisPoints() / pool.BASIS_POINTS_DIVISOR();
        amountUsd = amountOutUsd - feeAmount;
    }
}
