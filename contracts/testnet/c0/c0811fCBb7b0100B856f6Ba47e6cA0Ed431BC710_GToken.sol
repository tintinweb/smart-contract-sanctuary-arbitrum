...
contract GToken is ERC20Upgradeable, ERC4626Upgradeable, OwnableUpgradeable, IGToken {
    ...
    uint public currentEpoch;                // global id // 1
    ...
    function initialize(
        ...
        && _contractAddresses.feednPnlAddress != address(0)
        ...
        FeednPnlAddress = _contractAddresses.feednPnlAddress;
    ...
    function convertToPlp(uint usdtAmount,int uPnl) public view returns (uint) {
        ...
        return usdtAmount.mulDiv(assetSub*PRECISION, totalSupply(), MathUpgradeable.Rounding.Down);
    }
    ...
    function convertToUsdt(uint lplAmount,int uPnl) public view returns (uint) {
        ...
        uint assetSub = int(totalAssets()) - uPnl < 0?1:uint(int(totalAssets()) - uPnl);
        return lplAmount.mulDiv(totalSupply()*PRECISION, assetSub, MathUpgradeable.Rounding.Down);
    }
    ...
    function applyDeposit(
        ...
        require(assets <= _assetIERC20().balanceOf(receiver),"assets amount not enough");
        require(assets >= MixDeposit,"assets amount to small");
        bytes32 requestId;
    ...
    function runDeposit(bytes32 requestId,int uPnl) public checks() returns (uint){
        ...
        require(applyDepositData.usdtAmount <= _assetIERC20().balanceOf(applyDepositData.sender),"assets amount not enough");
        uint lplAmount = convertToPlp(applyDepositData.usdtAmount,uPnl);

        scaleVariables(lplAmount, applyDepositData.usdtAmount, true);
    ...
    function applyWithdraw(
        ...
        require(lplAmount <= _assetIERC20().balanceOf(receiver),"lpl amount not enough");
    ...
    function runWithdraw(bytes32 requestId,int uPnl) public checks() returns (uint){
        console.log("runWithdraw");
        require(_msgSender() == FeednPnlAddress,"not feed address");
        RequestApplyWithdraw memory  applyWithdrawData = RequestIdWithdraw[requestId];
        require(applyWithdrawData.lplAmount <= _assetIERC20().balanceOf(applyWithdrawData.sender),"assets amount not enough");
        uint usdtAmount = convertToUsdt(applyWithdrawData.lplAmount,uPnl);
        console.log("runWithdraw1");
        scaleVariables(applyWithdrawData.lplAmount, usdtAmount, false);
        console.log("runWithdraw2");
...
//        }
//        console.log("runWithdraw4 --- %d",i);
//        if(sum == applyWithdrawData.lplAmount && i<AddressLocks[applyWithdrawData.receiver].length){
//        }else{
//            AddressLocks[applyWithdrawData.receiver][i-1].assets-= sum - applyWithdrawData.lplAmount;
//        }
//        console.log("runWithdraw5 --- %d",i);
//        for(uint j=0;j<i;j++){
//            AddressLocks[applyWithdrawData.receiver].pop;
//        }
        return usdtAmount;
    }
    ...
    function withdraw(
        ...
        address receiver,
        address owner
    ) public override checks() returns (uint) {
        require(assets <= maxWithdraw(owner), "ERC4626: withdraw more than max");
    ...
    function makeWithdrawRequest(uint shares, address owner) external {
        ...
        uint unlockEpoch = currentEpoch + 3;
