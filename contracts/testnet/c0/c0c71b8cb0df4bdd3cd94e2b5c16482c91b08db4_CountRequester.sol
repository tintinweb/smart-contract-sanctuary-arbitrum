...
/// @notice Example of a sending a StateQuery request for a Counter's 'count()' value, and
...
contract CountRequester {
    ...
    event CountBatchReceived(uint64 blockNumber, uint256 count, address requestSender);
    ...
    {
        return IStateQueryGateway(stateQueryGateway).requestStateQuery{value: msg.value}(
            StateQuery({
            ...
            CountRequester.handleResponse.selector,
            abi.encode(msg.sender)
        );
    }

    function requestCountBatch(
        ...
        return IStateQueryGateway(stateQueryGateway).requestStateQueryBatch{value: msg.value}(
            stateQueries,
            CountRequester.handleBatchResponse.selector,
            abi.encode(msg.sender, _blockNumbers)
        );
    }

    function handleResponse(bytes calldata _responseResult, bytes calldata _callbackExtraData)
        external
    {
        if (msg.sender != stateQueryGateway) {
            revert NotFromStateQueryGateway(msg.sender);
        }
        uint256 count = abi.decode(_responseResult, (uint256));
    ...
    function handleBatchResponse(
        bytes[] calldata _responseResults,
        ...
        for (uint256 i = 0; i < _responseResults.length; i++) {
            uint256 count = abi.decode(_responseResults[i], (uint256));
            lastReceivedCount = count;

            emit CountBatchReceived(blockNumbers[i], count, requestSender);
        }
    }
}
