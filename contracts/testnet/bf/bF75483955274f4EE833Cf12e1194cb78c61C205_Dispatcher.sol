...
contract Dispatcher is IDispatcher, Initializable, DAOAccessControlled {
    ...
    */
    function _mintAvailableCollectibles(address _patron) internal {
        ...
        IExternalCollectionManager.ContractDetails[] memory allLoot8Collections = ICollectionManager(authority.getAuthorities().collectionManager).getAllCollectionsWithChainId(CollectionType.ANY, true);
        ...
        for(uint256 i = 0; i < allLoot8Collections.length; i++) {
            ...
            if( 
                ...
                (,,,,CollectionDataAdditional memory _additionCollectionData,,,,,) = ICollectionManager(authority.getAuthorities().collectionManager).getCollectionInfo(allLoot8Collections[i].source);
