...
contract GachaSystem is IGachaSystem, GameRegistryConsumerUpgradeable {
    ...
    function dispense(
        ...
        // Handle entry fee
        LootArrayComponentLibrary.handleBurningInputs(
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.activegachascomponent.v2")
);

struct Layout {
    uint256[] activeGachaEntities;
}

library ActiveGachasComponentStorage {
...
/**
 * @title ActiveGachasComponent
 * @dev Holds an array of active gachas
 */
contract ActiveGachasComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // The entity IDs of gachas
        keys[0] = "active_gacha_entities";
     ...
     *
     * @param entity Entity to get value for
     * @param activeGachaEntities The entity IDs of gachas
     */
    function setValue(
        uint256 entity,
        uint256[] memory activeGachaEntities
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(activeGachaEntities));
    }
    ...
    function append(
        ...
        Layout storage s = ActiveGachasComponentStorage
            .layout()
            .entityIdToStruct[entity];
        for (uint256 i = 0; i < values.activeGachaEntities.length; i++) {
            s.activeGachaEntities.push(values.activeGachaEntities[i]);
        }

        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(s.activeGachaEntities));
    }
    ...
    function removeValueAtIndexes(
        ...
        for (uint256 i = 0; i < indexes.length; i++) {
            ...
            // Get the last index
            uint256 lastIndexInArray = s.activeGachaEntities.length - 1;
            // Move the last value to the index to pop
            if (indexToRemove != lastIndexInArray) {
                s.activeGachaEntities[indexToRemove] = s.activeGachaEntities[
                    lastIndexInArray
                ];
            }
            // Pop the last value
            s.activeGachaEntities.pop();
        }
    ...
    function getLayoutValue(
        ...
        value = ActiveGachasComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return activeGachaEntities The entity IDs of gachas
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256[] memory activeGachaEntities) {
        if (has(entity)) {
            Layout memory s = ActiveGachasComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (activeGachaEntities) = abi.decode(
                _getEncodedValues(s),
                (uint256[])
            );
        }
    ...
    function getByteValues(
        ...
        values = new bytes[](1);
        values[0] = abi.encode(s.activeGachaEntities);
    }
    ...
    function setBytes(
        ...
        Layout memory s = ActiveGachasComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.activeGachaEntities) = abi.decode(value, (uint256[]));
    ...
    function remove(
        ...
        delete ActiveGachasComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete ActiveGachasComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.activeGachaEntities = value.activeGachaEntities;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.activeGachaEntities));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.activeGachaEntities);
    }
}
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.countercomponent.v1")
);
...
contract CounterComponent is BaseStorageComponentV2 {
    ...
    function getValue(
        ...
        if (has(entity)) {
            Layout memory s = CounterComponentStorage.layout().entityIdToStruct[
                entity
            ];
            (counts) = abi.decode(_getEncodedValues(s), (uint256));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = CounterComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.counts));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.counts);
    }
}
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.lootindexcomponent.v1")
);
...
library LootIndexComponentStorage {
    bytes32 internal constant STORAGE_SLOT = bytes32(ID);
...
/**
 * @title LootIndexComponent
 * @dev Holds an array of loot indexes
 */
contract LootIndexComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param lootIndexes Loot indexes
     */
    function setValue(
        uint256 entity,
        uint256[] memory lootIndexes
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(lootIndexes));
    }
    ...
    function append(
        ...
        Layout storage s = LootIndexComponentStorage.layout().entityIdToStruct[
            entity
        ];
        for (uint256 i = 0; i < values.lootIndexes.length; i++) {
            s.lootIndexes.push(values.lootIndexes[i]);
        }

        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(s.lootIndexes));
    }
    ...
    function removeValueAtIndexes(
        ...
        for (uint256 i = 0; i < indexes.length; i++) {
            uint256 indexToRemove = indexes[i];
            // Get the last index
            uint256 lastIndexInArray = s.lootIndexes.length - 1;
            // Move the last value to the index to pop
            if (indexToRemove != lastIndexInArray) {
                s.lootIndexes[indexToRemove] = s.lootIndexes[lastIndexInArray];
            }
            // Pop the last value
            s.lootIndexes.pop();
        }
    ...
    function getLayoutValue(
        ...
        // Get the struct from storage
        value = LootIndexComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return lootIndexes Loot indexes
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256[] memory lootIndexes) {
        if (has(entity)) {
            Layout memory s = LootIndexComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (lootIndexes) = abi.decode(_getEncodedValues(s), (uint256[]));
        }
    }
    ...
    function getByteValues(
        ...
        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](1);
        values[0] = abi.encode(s.lootIndexes);
    }
    ...
    function getBytes(
        ...
        Layout memory s = LootIndexComponentStorage.layout().entityIdToStruct[
            entity
        ];
        value = _getEncodedValues(s);
    }
    ...
    function setBytes(
        ...
        (s.lootIndexes) = abi.decode(value, (uint256[]));
        _setValueToStorage(entity, s);
    ...
    function remove(
        ...
        // Remove the entity from the component
        delete LootIndexComponentStorage.layout().entityIdToStruct[entity];
        _emitRemoveBytes(entity);
    }
    ...
    function batchRemove(
        ...
        // Remove the entities from the component
        for (uint256 i = 0; i < entities.length; i++) {
            delete LootIndexComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
        _emitBatchRemoveBytes(entities);
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.lootIndexes = value.lootIndexes;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.lootIndexes));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.lootIndexes);
    }
}
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.timerangecomponent")
);
...
library TimeRangeComponentStorage {
    bytes32 internal constant STORAGE_SLOT = bytes32(ID);
...
/**
 * @title TimeRangeComponent
 * @dev Component to track start and end times
 */
contract TimeRangeComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function setValue(
        ...
        _setValue(entity, Layout(startTime, endTime));
    }
    ...
    function getLayoutValue(
        ...
        // Get the struct from storage
        value = TimeRangeComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return startTime Start time of the range
     * @return endTime End time of the range
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint32 startTime, uint32 endTime) {
        if (has(entity)) {
            Layout memory s = TimeRangeComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (startTime, endTime) = abi.decode(
                _getEncodedValues(s),
                (uint32, uint32)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = TimeRangeComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.startTime);
        values[1] = abi.encode(s.endTime);
    }
    ...
    function getBytes(
        ...
        Layout memory s = TimeRangeComponentStorage.layout().entityIdToStruct[
            entity
        ];
        value = _getEncodedValues(s);
    }
    ...
    function setBytes(
        ...
        (s.startTime, s.endTime) = abi.decode(value, (uint32, uint32));
        _setValueToStorage(entity, s);
    ...
    function remove(
        ...
        // Remove the entity from the component
        delete TimeRangeComponentStorage.layout().entityIdToStruct[entity];
        _emitRemoveBytes(entity);
    }
    ...
    function batchRemove(
        ...
        // Remove the entities from the component
        for (uint256 i = 0; i < entities.length; i++) {
            delete TimeRangeComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
        _emitBatchRemoveBytes(entities);
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.startTime, value.endTime));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.startTime, value.endTime);
    }
}
