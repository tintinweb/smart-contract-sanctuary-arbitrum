...
contract VaultAccessControlRegistry is IVaultAccessControlRegistry, AccessControl, Pausable {
    ...
    /**
     * WINR Protocol wants the protocol to be as trustless as possible. However in the initial state of the protocol some configuration functions need to be quickly callable and tweaked. In order to reduce the amount of functions/modifiers that can change important configs at a whim forever, we have a deadman switch flow that can be called once. 
     * Before the flip is flipped (so it is false) all the onlyTimelockGovernance modifiers are controlled by the 'governance contract' (or the controller contract)
     * After the flip is flipped (so it is true) the  onlyTimelockGovernance will be only callable by the timelock contract (which is immutably infigured at deployment)
     */
    bool public timelockActivated = false; // this is the deadman switch bool
    ...
    function flipTimelockDeadmanSwitch() external onlyRole(GOVERANCE_ROLE) {
        // the switch cannot be unflipped - so this function can only be called once
        require(
            !timelockActivated,
     ...
     * @dev even though parent AccessControl provides revoke and grant functions as well, we only want one governanceAddress active at the same time!
     * @param _governanceAddress the new to be configured governance address
     * note: technically it is still possible for the protocol to have mutlipe goverance roles, we allow this on purpose as the situation might require it.
     */
    ...
    function isCallerGovernance(address _account) external view whenNotPaused returns (bool isGovernance_) {
        isGovernance_ = hasRole(GOVERANCE_ROLE, _account);
    }

    function isCallerManager(address _account) external view  whenNotPaused returns (bool isManager_) {
        isManager_ = hasRole(MANAGER_ROLE, _account);
    }

    function isCallerEmergency(address _account) external view whenNotPaused returns (bool isEmergency_) {
