...
contract _ChronoMages is ERC721A, ERC721ABurnable, AccessControl, ReentrancyGuard {
  ...
  event DeadMage(address _from, uint256 tokenId);
  ...
  uint256 constant publicPrice = 2 * 10**14;
  ...
  function isRitualReady(address _to) public view returns (bool) {
    if (_ritual[_to].requiredBlock <= block.number) {
      return true;
    }
    return false;
  }
  ...
  function isTransformationReady(address _to) public view returns (bool) {
    if (_transformation[_to].requiredBlock <= block.number) {
      return true;
    }
    return false;
  }
  ...
  function beginTransformation(address _from, uint256 tokenId) external callerIsUser notPaused {
    ...
    safeTransferFrom(_from, address(this), tokenId);
  }
