...
import {MGHPriceProvider, IPriceProviderObservable, IPriceProviderForceable} from "../../../src/priceProviders/PriceProviderInterfaces.sol";
import {ILimitOrderProxy} from "../../../src/limitOrders/ILimitOrderProxy.sol";

contract TestPriceProvider is MGHPriceProvider, Ownable, IPriceProviderObservable, IPriceProviderForceable {
    using ERC165Checker for address;

    uint256 private price; //WAD
    uint256 private _startedAt;
    uint256 private _updatedAt;
    address[] public subscriber = [address(0)];
    ...
    /*
    price can be changed by anyone!
    */
    function setPrice(uint256 _price) public override {
        price = _price;
        _updatedAt = block.timestamp;
        emit PriceChange(price);
        uint256 length = subscriber.length;
        for (uint256 i = 0; i < length; i++) {
            if (subscriber[i] != address(0)) {
                // ILimitOrderProxy(subscriber[i]).priceEvent();
            }
        }
    }
    ...
    function description() external pure returns (string memory){
        return "MGH Test Price Provider";
    }
    ...
    function setSubscriber(address _to) public override onlyOwner {
        if (_to.supportsInterface(type(ILimitOrderProxy).interfaceId)) {
            subscriber.push(_to);
        } else {
            revert("unable to add subscriber not implementing requested interface");
        }
    }

    function removeSubscriber(uint subscriberID) public override onlyOwner {
        require(subscriberID < subscriber.length);
        subscriber[subscriberID] = address(0);
    }
