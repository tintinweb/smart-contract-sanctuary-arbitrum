...
contract pachiCharacterNFTV1 is ERC721Base {
    struct Character {
        string name;
        // string description;
        // string image;
        uint256 level;
        uint256 lp; // love point
        // uint256 hp;
        // uint256 attack;
        // uint256 defense;
        // uint256 speed;
        // uint256 special;
        uint256 jackpotPoint;
        uint256 divdendsPoint;
        // uint256 xpBoostPoint;
        // uint256 ap;
    }
    ...
    mapping(address => uint256) public ownerToTokenId; // address => tokenId
    mapping(address => bool) public ownerToIsClaimed; // address => isClaimed
    mapping(uint256 => string) private fullURI;
    ...
    function mintTo(address _to, string memory _tokenURI) public override {
        require(_canMint(), "Not authorized to mint.");
        // require(!ownerToIsClaimed[_to], "Address already claimed");

        _setTokenURI(nextTokenIdToMint(), _tokenURI);
        _safeMint(_to, 1, "");
        ownerToIsClaimed[_to] = true;
        ownerToTokenId[_to] = nextTokenIdToMint();
    }

    function _canMint() internal view override returns (bool) {
        return msg.sender == owner() || msg.sender == nftManager;
    }
    ...
    function _setTokenURI(
        ...
        require(
            msg.sender == owner() || msg.sender == nftManager,
            "Not authorized to set TokenURI"
        );
        // require(bytes(fullURI[_tokenId]).length == 0, "URI already set");
        fullURI[_tokenId] = _tokenURI;
    }
    ...
    // should limit transfer to only owner
    function _beforeTokenTransfers(
        address,
        address,
        uint256,
    ...
    function setTransferPaused(bool _transferPaused) public onlyOwner {
