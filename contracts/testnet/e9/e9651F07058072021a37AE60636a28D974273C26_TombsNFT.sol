...
contract TombsNFT is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 private constant PRICE_WHITELIST = 3 * 10**15; // 0.003 ARB Per Tombs
    uint256 private constant PRICE_PUBLIC = 4 * 10**15; // 0.004 ARB Per Tombs
    ...
    function mint(address payable _to, uint256 amount,  bytes32[] calldata merkleProof) public payable {
        ...
        require(total <= MAX_SUPPLY, "MINT: Please go to the Opensea to buy Tombs.");
        ...
        if(MerkleProof.verify(merkleProof, mintWhitelistMerkleRoot, node)) {                
            ...
            require(block.timestamp >= goLiveDate + preSalePeriod, "MINT: Invalid timestamp");
