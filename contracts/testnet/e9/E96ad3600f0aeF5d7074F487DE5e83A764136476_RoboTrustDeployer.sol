...
contract RoboTrustDeployer {
    ...
    function createTrust(uint256 _period, uint _maxPayouts, uint[] memory _paymentAmounts) public payable {
        RoboTrust newTrust = (new RoboTrust){value: msg.value}(msg.sender, _period, _maxPayouts, _paymentAmounts, priceFeedAddress);
        uint index = trustList.length;
        trustList.push(newTrust);
        trustAddresses.push(address(newTrust));
...
contract RoboTrust is FinancialMathHelper {
    ...
    constructor(address _owner, uint256 _period, uint _maxPayouts, uint[] memory _paymentAmounts, address _priceFeedAddress) FinancialMathHelper(_priceFeedAddress) payable {
