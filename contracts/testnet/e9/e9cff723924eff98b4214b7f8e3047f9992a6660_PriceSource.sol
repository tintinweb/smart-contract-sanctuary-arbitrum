...
contract PriceSource is IPriceSource, IPriceSourceErrors {
    ...
    /// @inheritdoc IPriceSource
    function price(ConstituentLib.Constituent calldata constituent) external view override returns (UFixed128 result) {
        return _price(
            abi.decode(PriceSourceLib.priceSourceOf(constituent.asset, constituent.chainId), (PriceSourceLib.Source))
        );
    }
