...
contract SettingsManager is ISettingsManager, Ownable, Constants {
    ...
    function updateCumulativeFundingRate(address _indexToken, address _collateralToken, bool _isLong) external override hasPermission {
        if (lastFundingTimes[_indexToken][_isLong] == 0) {
            lastFundingTimes[_indexToken][_isLong] = uint256(block.timestamp / fundingInterval) * fundingInterval;
            return;
        }

        if (lastFundingTimes[_indexToken][_isLong] + fundingInterval > block.timestamp) {
            return;
        }

        cumulativeFundingRates[_indexToken][_isLong] += _getNextFundingRate(_indexToken, _collateralToken, _isLong);
        lastFundingTimes[_indexToken][_isLong] = uint256(block.timestamp / fundingInterval) * fundingInterval;
        emit UpdateFundingRate(
            _indexToken,
            _isLong,
            cumulativeFundingRates[_indexToken][_isLong],
            lastFundingTimes[_indexToken][_isLong]
        );
    }
    ...
    function getNextFundingRate(address _indexToken, address _collateralToken, bool _isLong) external view returns (uint256) {
        return _getNextFundingRate(_indexToken, _collateralToken, _isLong);
    }

    function _getNextFundingRate(address _indexToken, address _collateralToken, bool _isLong) internal view returns (uint256) {
        uint256 intervals = (block.timestamp - lastFundingTimes[_indexToken][_isLong]) / fundingInterval;

        if (vault.poolAmounts(_collateralToken) == 0) {
            return 0;
        }

        return vault.poolAmounts(_collateralToken) == 0 ? 0 
            : (((
                    fundingRateFactor[_indexToken][_isLong] > 0
                        ? fundingRateFactor[_indexToken][_isLong]
                        : DEFAULT_FUNDING_RATE_FACTOR
                ) *
                    vault.reservedAmounts(_collateralToken) *
                    intervals) / vault.poolAmounts(_collateralToken));
    }
