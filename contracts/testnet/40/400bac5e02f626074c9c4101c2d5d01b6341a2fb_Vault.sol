...
contract Vault is Ownable, ERC4626 {
    ...
    function withdrawAssets(uint256 assets, address receiver) public {
        ...
        AORI(aori).unstakeLPTokens(Vault(address(this)), receiver, convertToShares(assets));
    ...
    function settleITMOption(address option, bool isCall) internal returns (uint256) {
    ...
    function settleOTMOption(address option, bool isCall) internal returns (uint256) {
        Structs.settleVars memory vars;
        if(isSettled[option]) {
            return 0;
        } else {
...
contract AORI is ERC20, Ownable {
    ...
    mapping(address => uint256) totalMinedAtLastClaim;
    ...
    uint256 maxLiqMining = (30000000 * 1e18); //30m or 30% of total supply
    uint256 liqMined;
    uint256 percentPerSecond = 57870370370; //1e18 Half a percent per day in seconds
...
contract MarginManager is Ownable {
    ...
    function settlePosition(address account, bytes32 key) public {
        ...
        lpTokens[underlying].settleOption(position.option, position.isCall, isITM);
        if (isITM) {
    ...
    /**
        Good
     */
    function liquidatePosition(bytes32 key, uint256 fairValueOfOption, address liquidator) public returns (Structs.Position memory) {
        ...
        Structs.doTransferOut(underlying, address(lpTokens[underlying]), position.collateral - localVars.profit);
        Structs.doTransferOut(underlying, liquidator, localVars.profit);
    ...
    function accruePositionInterest(bytes32 key) public returns (bool) {
        ...
        uint256 currentCollat = position.collateral;
        require(block.timestamp - position.lastAccrueTime > 0, "cannot accrue");
        if(position.isCall) {
            underlying = ERC20(address(AoriCall(position.option).UNDERLYING()));
        } else {
            underlying = ERC20(address(AoriPut(position.option).USDC()));
        }
        interestOwed = positionRouter.getInterestOwed(false, underlying, position.strikeInUSDC, position.optionSize, position.entryMarginRate, position.lastAccrueTime);
...
interface IPositionRouter is IOwnable {
    ...
    function getInterestOwed(bool isCall, ERC20 underlying, uint256 strikeInUSDC, uint256 optionSize, uint256 entryMarginRate, uint256 lastAccrueTime) external view returns (uint256);
...
contract Orderbook {
    ...
    function createAsk(uint256 _USDCPerOPTION, uint256 _OPTIONSize) public returns (Ask) {
        ...
        address askAddress = address(ask);
        OPTION.transferFrom(msg.sender, askAddress, _OPTIONSize);
        asks.push(ask);
        isAsk[askAddress] = true;
        ask.fundContract();
        emit AskCreated(msg.sender, askAddress, _USDCPerOPTION, _OPTIONSize);
        return ask;
    }
    ...
    function createBid(uint256 _OPTIONPerUSDC, uint256 _USDCSize) public returns (Bid) {
        ...
        address bidAddress = address(bid);
        USDC.transferFrom(msg.sender, bidAddress, _USDCSize);
        bids.push(bid);
        isBid[bidAddress] = true;
        bid.fundContract();
        emit BidCreated(msg.sender, bidAddress, _OPTIONPerUSDC, _USDCSize);
        return bid;
    }


    function fillAsks(address receiver, uint256 _USDCPerOPTION, uint256 USDCsize, uint256 seatId) public returns(uint256) {
        Ask[] memory activeAsks = getActiveAsksAtPrice(_USDCPerOPTION);
        require(activeAsks.length > 0, "No asks at this price");
        USDC.transferFrom(msg.sender, address(this), USDCsize);
        cumVol += USDCsize;
        for (uint i = 0; i < activeAsks.length && USDCsize > 0; i++) {
            uint256 amountLeft = Structs.mulDiv(OPTION.balanceOf(address(activeAsks[i])), activeAsks[i].USDCPerOPTION(), 10**OPTION.decimals());
            address askAddress = address(activeAsks[i]);
            if (amountLeft <= USDCsize) {
                USDC.approve(askAddress, amountLeft);
                activeAsks[i].fill(receiver, amountLeft, seatId);
                USDCsize -= amountLeft;
            } else {
                USDC.approve(askAddress, USDCsize);
                activeAsks[i].fill(receiver, USDCsize, seatId);
                USDCsize = 0;
            }
            if(USDC.allowance(address(this), askAddress) > 0) {
                USDC.decreaseAllowance(receiver, USDC.allowance(address(this), askAddress));
            }
        }
        emit Filled(receiver, _USDCPerOPTION, USDCsize, true);
        return USDCsize;
    }


    
    function fillBids(address receiver, uint256 _OPTIONPerUSDC, uint256 OPTIONsize, uint256 seatId) public returns(uint256) {
        Bid[] memory activeBids = getActiveBidsAtPrice(_OPTIONPerUSDC);
        require(activeBids.length > 0, "No bids at this price");
        OPTION.transferFrom(msg.sender, address(this), OPTIONsize);
        for (uint i = 0; i < activeBids.length && OPTIONsize > 0; i++) {
            uint256 amountLeft = Structs.mulDiv(USDC.balanceOf(address(activeBids[i])), activeBids[i].OPTIONPerUSDC(), 10**USDC.decimals());
            address bidAddress = address(activeBids[i]);
            if (amountLeft <= OPTIONsize) {
                OPTION.approve(bidAddress, amountLeft);
                activeBids[i].fill(receiver, amountLeft, seatId);
                OPTIONsize -= amountLeft;
                cumVol += USDC.balanceOf(bidAddress);
            } else {
                OPTION.approve(bidAddress, OPTIONsize);
                uint256 USDCToReceive = activeBids[i].fill(receiver, OPTIONsize, seatId);
                cumVol += USDCToReceive;
                OPTIONsize = 0;
            }
            if(OPTION.allowance(address(this), bidAddress) > 0) {
                OPTION.decreaseAllowance(receiver, OPTION.allowance(address(this), bidAddress));
            }
        }
        emit Filled(receiver, _OPTIONPerUSDC, OPTIONsize, false);
        return OPTIONsize;
    }
    ...
    function getActiveAsksAtPrice(uint256 _USDCPerOPTION) public view returns (Ask[] memory) {
        ...
        for (uint256 i = 0; i < asks.length; i++) {
            Ask ask = Ask(asks[i]);
            if (ask.isFunded() && !ask.hasEnded() && ask.USDCPerOPTION() == _USDCPerOPTION) {
                activeAsks[count++] = ask;
            }
        }
        Ask[] memory activeAsksAtPrice = new Ask[](count);
        for (uint256 i = 0; i < count; i++) {
            activeAsksAtPrice[i] = activeAsks[i];
        }
    ...
    function getActiveBidsAtPrice(uint256 _OPTIONPerUSDC) public view returns (Bid[] memory) {
        ...
        Bid[] memory activeBidsAtPrice = new Bid[](count);
        for (uint256 i = 0; i < count; i++) {
            activeBidsAtPrice[i] = activeBids[i];
        }
        return activeBidsAtPrice;
    }
