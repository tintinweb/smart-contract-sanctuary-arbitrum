...
contract DutchAuction is Initializable, PausableUpgradeable, AccessControlUpgradeable, Versioned {
     ...
     */
    function currentStage(uint256 timestamp) public view returns (bool, uint256, AuctionInfo memory) {
        ...
        return (false, MAX_UINT, AuctionInfo(0, 0, 0, 0, 0, 0, 0, 0, 0, ""));
    }
    ...
    function getPrice(uint256 stage, uint256 timestamp) public view validStage(stage) returns (uint256) {
        ...
        if (timestamp <= startTimestamp) {
            ...
            if (startPrice > drop) {
                return Math.max(startPrice.sub(drop), endPrice);
            } else {
                return endPrice;
            }
        }
    ...
    function mintNFT(uint256 stage, bytes32[] calldata whitelistProof) public payable 
        ...
        if (merkleRoot != 0) {
            ...
            require(MerkleProof.verify(whitelistProof, merkleRoot, leaf), "not in WL");
        }
        
        uint256 currentPrice = getPrice(stage, block.timestamp);
        require(msg.value >= currentPrice, "px too low");
...
contract Versioned {
    string public constant version = "cc8095297b933e423494bad6d053ea098bd017e9";
}
