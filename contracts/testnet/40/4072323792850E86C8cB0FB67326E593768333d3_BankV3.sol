...
import {IMintManager} from "./IMintManager.sol";
...
contract BankV3 is AccessControlEnumerable, Multicall {
    using SafeERC20 for IERC20;
    /**
        TODO: rewrite automation related functions
     */
    ...
    /// @notice Emitted after the max bet unit is set for a token.
    /// @param token Address of the token.
    /// @param minBetUnit Minimum bet amount.
    event SetTokenMaxBetUnit(address indexed token, uint256 minBetUnit);

    /// @notice Sets the max bet unit for a specific token.
    /// @param token Address of the token.
    /// @param tokenMaxBetUnit Minimum bet amount.
    function setTokenMaxBetUnit(
        address token,
        uint256 tokenMaxBetUnit
    ) external onlyTokenOwner(DEFAULT_ADMIN_ROLE, token) {
        tokens[token].maxBetUnit = tokenMaxBetUnit;
        emit SetTokenMinBetUnit(token, tokenMaxBetUnit);
    }

    /// @notice Gets the token's max bet unit
    /// @param token Address of the token.
    /// @return maxBetUnit Min bet amount.
    /// @dev The min bet amount should be at least 10000 cause of the `getMaxBetAmount` calculation.
    function getMaxBetUnit(
        address token
    ) external view returns (uint256 maxBetUnit) {
        maxBetUnit = tokens[token].maxBetUnit;
        // if (maxBetUnit < 10000) {
        //     maxBetUnit = 10000;
        // }
    }
    ...
    function _setDefaultTokenValues(address token) internal {
        uint8 decimals;
        if (token == address(0)) {
            decimals = 18;
        } else {
        ...
        tokens[token].balanceRisk = 8000; //80%
        tokens[token].allowed = true;
        tokens[token].paused = false;
        tokens[token].minHouseEdgeWithdrawAmount = 1 * 10 ** decimals;
        tokens[token].maxBetUnit = 100 * (10 ** decimals);
        tokens[token].minBetUnit = 1 * 10 ** decimals;

        HouseEdgeSplit storage tokenHouseEdge = tokens[token].houseEdgeSplit;
        tokenHouseEdge.bank = 1000; // 10%
        tokenHouseEdge.dividend = 9000; // 90%
        tokenHouseEdge.partner = 0;
        tokenHouseEdge.treasury = 0;
        tokenHouseEdge.team = 0;

        emit SetTokenMaxBetUnit(token, 100 * 10 ** decimals);
        emit SetTokenMinBetUnit(token, 1 * 10 ** decimals);
        emit SetBalanceRisk(token, 5000); // 50%
        emit SetTokenHouseEdgeSplit(token, 1000, 9000, 0, 0, 0);
        emit SetAllowedToken(token, true);
        emit SetPausedToken(token, false);
        emit SetMinHouseEdgeWithdrawAmount(token, 0);
    }

    /**
        TODO
        - set mintManager (in order to dropXPachi to gamer)
        - set nftManager (in order to give xp to gamer's NFT)      
    ...
    /// @param _mintManager Address of the mint manager.
    constructor(
    ...
    address[] STABLE_COINS = [0x8192dA45b932EFDc56dee24aC205d6Bcf209AA73];

    function setStableCoins(
        address[] memory _stableCoins
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        STABLE_COINS = _stableCoins;
    }

    /// @notice reward xPachi to user. only reward if token is stable coin
    /// @param token betting token
    /// @param to user address
    /// @param amount usdc amount that user pay for house edge
    function _reward(address token, address to, uint256 amount) internal {
        for (uint256 i = 0; i < STABLE_COINS.length; i++) {
            if (token == STABLE_COINS[i]) {
                if (address(mintManager) != address(0)) {
                    mintManager.allocateXPachi(to, amount);
                    break;
                }
            }

            uint256 tokenId = nftManager.getSupporterTokenId(to);
            // get token decimals
            uint8 decimals = IERC20Metadata(token).decimals();
            uint8 exp = uint8(amount / 10 ** decimals);
            uint8 lovePoint = 0;

            if (address(nftManager) != address(0) || tokenId != 0) {
                nftManager.updateNFTStatus(tokenId, exp, lovePoint);
            }
        }
    }
    ...
    function grantGameRole(
        ...
        grantRole(GAME_ROLE, gameAddress);
    }
    ...
    function payout(
        ...
        // should add logic for xp & xpachi drop
        _reward(token, user, betAmountFees);
    }
    ...
    function cashIn(
        ...
        // should add logic for xp & xpachi drop
        _reward(tokenAddress, user, betAmountFees);
    }
    ...
    /// @param token Address of the token.
    /// @param minBetUnit Minimum bet amount.
    event SetTokenMinBetUnit(address indexed token, uint256 minBetUnit);
    ...
    function deposit(
        address token,
        uint256 amount
    ) external payable onlyTokenOwner(DEFAULT_ADMIN_ROLE, token) {
        if (_isGasToken(token)) {
            amount = msg.value;
        } else {
    ...
    function withdraw(
        address token,
        uint256 amount
    ) public onlyTokenOwner(DEFAULT_ADMIN_ROLE, token) {
    ...
    /// @param token Address of the token.
    /// @param tokenMinBetUnit Minimum bet amount.
    function setTokenMinBetUnit(
        address token,
        uint256 tokenMinBetUnit
    ) external onlyTokenOwner(DEFAULT_ADMIN_ROLE, token) {
        tokens[token].minBetUnit = tokenMinBetUnit;
        emit SetTokenMinBetUnit(token, tokenMinBetUnit);
    }

    // /// @notice Sets the Chainlink VRF subscription ID for a specific token.
    // /// @param token Address of the token.
    // /// @param subId Subscription ID.
    // function setTokenVRFSubId(
    //     address token,
    //     uint64 subId
    // ) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    /// @param token Address of the token.
    /// @return minBetUnit Min bet amount.
    /// @dev The min bet amount should be at least 10000 cause of the `getMaxBetAmount` calculation.
    function getMinBetUnit(
        address token
    ) external view returns (uint256 minBetUnit) {
        minBetUnit = tokens[token].minBetUnit;
        if (minBetUnit < 10000) {
            minBetUnit = 10000;
        }
    }
    ...
    /// @notice Runs by Gelato.Network at every block to determine if `performUpkeep` should be called.
    /// @return canExec Boolean that when True will trigger the on-chain performUpkeep call.
    /// @return execPayload Bytes that will be used as input parameter when calling performUpkeep.
    /// @dev `checkData` and `execPayload` are encoded with types (uint8, address).
    function checker(
        address _token
    ...
    /// @notice Executed by Gelato.Network when `canExec` is true.
    /// @param _token token Address
    function regularWithdraw(address _token) external {
        Token memory token = tokens[_token];
        ...
        if (
            ...
            withdrawPartnerAmount(_token);
        } else if (
            token.houseEdgeSplit.treasuryAmount +
                token.houseEdgeSplit.teamAmount >
            token.minHouseEdgeWithdrawAmount
        ) {
            withdrawHouseEdgeAmount(_token);
        }
    }
...
interface IMintManager {
    function allocateXPachi(address to, uint256 amount) external;

    function claimXPachi() external;
}
