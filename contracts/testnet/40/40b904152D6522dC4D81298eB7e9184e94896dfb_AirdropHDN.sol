...
import "../libraries/ContractBase.sol";
import "../libraries/Roles.sol";

/// @custom:security-contact [email protected]
contract AirdropHDN is BaseUpgradableContract, RoleExecutor, RoleProposer {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.UintSet;

    // STATE
    // IERC20Upgradeable _treasury;
    ...
    function initialize(address admin, address treasury) public initializer {
        ...
        _ensureRole(EXECUTOR_ROLE, admin);
        _ensureRole(PROPOSER_ROLE, admin);

        // _treasury = IERC20Upgradeable(treasury);
    }
    ...
    ////// AIRDROP - 0: DATA / HELPER //////
    address public constant TOKEN_HDX =
        0xF4fe727C855c2D395852ca43F645caB4b504Af23;
    address public constant TOKEN_SHDX =
        0xb7F5ca475D7F62ab9A6729d8118b0E65E666f005;
    address public constant TOKEN_GHDX =
        0xE33050122e346CAAb4505BEaDc514C024722a38E;
    error AirdropWrongJob(uint256 want, uint256 have);
    struct AirdropData {
        address[][] holdersInitial;
        address[] holdersFiltered;
    ...
    AirdropData internal _store;
    AirdropStatus internal _status;
    ...
    ////// AIRDROP - 1: Load Token Holders //////
    function airdropLoadHolders(address[] memory csv) public {
        if (_status.job > 1) {
            revert AirdropWrongJob(1, _status.job);
        }
        _store.holdersInitial.push(csv);
    }

    function airdropLoadHoldersMarkCompleted() public {
        _status.job = 2;
    }

    ////// AIRDROP - 2: Filter Empty Balances //////
    function airdropFilterHolders(uint BATCH_SIZE) public {
        if (_status.job != 2) {
            revert AirdropWrongJob(2, _status.job);
        }
        IERC20Upgradeable _hdx = IERC20Upgradeable(TOKEN_HDX);
        IERC20Upgradeable _shdx = IERC20Upgradeable(TOKEN_SHDX);
        IERC20Upgradeable _ghdx = IERC20Upgradeable(TOKEN_GHDX);

        uint ix2;
        for (uint i = 0; i < _store.holdersInitial.length; i++) {
            uint j = 0;
            uint dslen = _store.holdersInitial[i].length;
            if (_status.ix - ix2 >= dslen) {
                ix2 += dslen;
                continue;
            }
            j = _status.ix - ix2;
            ...
            for (; j < dslen; j++) {
                address holder = _store.holdersInitial[i][j];
                if (
                    _hdx.balanceOf(holder) > 0 ||
                    _shdx.balanceOf(holder) > 0 ||
                    _ghdx.balanceOf(holder) > 0
                ) {
                    _store.holdersFiltered.push(holder);
                }

                _status.ix++;
                if (BATCH_SIZE > 0 && _status.ix >= BATCH_SIZE) {
                    _status.ix = _status.ix;
                    return;
                }
            }
        }
    ...
    ////// AIRDROP - 3.5: Determine Actual Balances //////
    function airdropSnapBalances(uint BATCH_SIZE) public {
        if (_status.job != 3) {
            revert AirdropWrongJob(3, _status.job);
        }
        IERC20Upgradeable _hdx = IERC20Upgradeable(TOKEN_HDX);
        HdxIndex _shdx = HdxIndex(TOKEN_SHDX);
        IERC20Upgradeable _ghdx = IERC20Upgradeable(TOKEN_GHDX);

        uint hdxRebaseIndex = _shdx.index();
        ...
        for (; i < _store.holdersFiltered.length; i++) {
            address holder = _store.holdersFiltered[i];
            // TODO !!! MULTIPLY GHDX * index
            _store.balances[holder] =
                _hdx.balanceOf(holder) +
                _shdx.balanceOf(holder) +
                _ghdx.balanceOf(holder) *
                hdxRebaseIndex;

            ix2++;
            if (BATCH_SIZE > 0 && ix2 >= BATCH_SIZE) {
                _status.ix = ix2;
                return;
            }
        }
    ...
    ////// AIRDROP - 3.5: Determine Actual Balances //////
    function airdropTransformCombineBalances(uint BATCH_SIZE) public {
        if (_status.job != 3) {
            revert AirdropWrongJob(3, _status.job);
        }
        IERC20Upgradeable _hdx = IERC20Upgradeable(TOKEN_HDX);
        ...
        ////// AIRDROP - 4: Transform and combine balances //////

        ////// AIRDROP - 5: Process diffsets until airdrop time //////
...
interface HdxIndex is IERC20Upgradeable {
