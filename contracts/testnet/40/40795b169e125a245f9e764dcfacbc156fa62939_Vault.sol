...
contract Vault is Constants, ReentrancyGuard, Ownable, IVault {
    ...
    function increasePoolAmount(address _indexToken, uint256 _amount) public override {
        _isPositionHandler(msg.sender, true);

        if (!collateralTokens.contains(_indexToken)) {
            collateralTokens.add(_indexToken);
        }

        _updatePoolAmount(_indexToken, _amount, true);
    }

    function decreasePoolAmount(address _indexToken, uint256 _amount) public override {
        _isPositionHandler(msg.sender, true);
        _updatePoolAmount(_indexToken, _amount, false);
    }

    function _updatePoolAmount(address _indexToken, uint256 _amount, bool _isPlus) internal {
        if (_isPlus) {
            poolAmounts[_indexToken] += _amount;
        } else {
            require(poolAmounts[_indexToken] >= _amount, "Vault: poolAmount exceeded");
            poolAmounts[_indexToken] -= _amount;
        }

        emit UpdatePoolAmount(_indexToken, _amount, poolAmounts[_indexToken], _isPlus);
    }
    ...
    function _takeAssetOut(
        ...
        _collectFee(_fee, _refer, _rebatePercentage);
    ...
    function distributeFee(bytes32 _key, address _account, uint256 _fee) external override {
        _isPositionHandler(msg.sender, true);
        _collectFee(_fee, address(0), 0);

        if (_fee > 0) {
            emit DistributeFee(_key, _account, settingsManager.feeManager(), _fee);
        }
    }

    function _collectFee(uint256 _fee, address _refer, uint256 _rebatePercentage) internal {
        //Pay rebate first
        ...
        if (_fee != 0 && feeManager != ZERO_ADDRESS) {
            ...
            _fee -= feeMinusFeeReward;

            if (feeMinusFeeReward > 0) {
                IMintable(RUSD).mint(feeManager, feeMinusFeeReward);
                totalRUSD += feeMinusFeeReward;
            }
        }
    ...
    function stake(address _account, address _token, uint256 _amount) external nonReentrant {
        ...
        _accountDeltaAndFee(true, 0, usdAmountFee);
        _collectFee(usdAmountFee, ZERO_ADDRESS, 0);
        ...
        _increaseTokenBalances(_token, _amount);
        increasePoolAmount(_token, usdAmountAfterFee);
    ...
    function unstake(address _tokenOut, uint256 _rolpAmount, address _receiver) external nonReentrant {
        ...
        decreasePoolAmount(_tokenOut, usdAmountAfterFee);
        _accountDeltaAndFee(true, 0, usdAmountFee);
        _collectFee(usdAmountFee, ZERO_ADDRESS, 0);
    ...
    function getTraingTokens() external view returns (address[] memory) {
