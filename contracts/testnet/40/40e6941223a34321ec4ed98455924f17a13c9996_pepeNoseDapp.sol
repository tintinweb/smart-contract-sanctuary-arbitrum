...
contract pepeNoseDapp {
    ...
    ERC20 private pepeNoseToken;
    
    uint256 private constant winningTouchers = 18;
    uint256 private constant explosionDelay = 3600;
    address[winningTouchers] private touchers;
    uint256 private touchPrice;
    uint256 private touchIndex;
    uint256 private explosionTime;
    uint256 private totalPot;
    uint256 private teamFund;
    uint256 private cooldown = 1800; //cooldown period for price increment
    uint256 private lastTouchTime;
    address private defaultReferee; //Variable for a default referal link when someone plays for the first time
    
    mapping(address=>uint256) private playerTouchCount;
    mapping(address=>uint256) private playerSecToTimeout;
    uint256 private totalTouches;
    ...
    constructor() {
        ...
        defaultReferee = address(0xDe6D3A6C1fB9B7d8f65736e36F083188B4d74E6c); //default Referee is set to sewyash.eth
        pepeNoseToken = ERC20(0x8AAcEBF743D37C1ca989F84daeACA57a180A971a);
    ...
    function GetWinners() external view returns (address[winningTouchers] memory _addr) {
    ...
    function NoseTouch(address referee) external {
        require(explosionTime >= block.timestamp, "Game has ended.");
        require(msg.sender == owner || referee != msg.sender, "Cannot refer yourself!");
        ...
        totalPot = totalPot + ((touchPrice * 8) / 10);
            
        uint256 fee = touchPrice / 10;

        pepeNoseToken.transfer(refAddr, fee); // Referral paid out in $NOSE tokens - 10% txn cut

        //transfer the remaining fee (after referral and team Fund
        pepeNoseToken.transferFrom(msg.sender, address(this), fee);

        //then burn the tokens from the contract
        PEPENOSE(address(pepeNoseToken)).burn(fee);
    ...
    function DistributePot() external {
        require(explosionTime < block.timestamp, "Game not expired yet");
        require(totalPot > 0, "No shekels in the pot");
        
        uint256 reward = (totalPot - teamFund) / winningTouchers;

        for(uint256 i = 0; i < winningTouchers; ++i) {
            pepeNoseToken.transfer(touchers[i], reward); //rewards paid out in $NOSE
        }

        // Add 10% of totalPot to teamFund
        teamFund += totalPot / 10;

        totalPot = 0;
    }

    function WithdrawTeamFunds() external {
        require(msg.sender == owner, "Only owner/deployer can withdraw team funds");
        pepeNoseToken.transfer(owner, teamFund);
        teamFund = 0;
    }
}