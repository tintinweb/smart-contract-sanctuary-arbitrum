...
contract GachaComponent is BaseStorageComponentV2 {
    ...
    {
        if (has(entity)) {
            Layout memory s = GachaComponentStorage.layout().entityIdToStruct[entity];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = GachaComponentStorage
           .layout()
    ...
    function setBytes(
        ...
        Layout memory s = GachaComponentStorage.layout().entityIdToStruct[entity];
        (s.gachaId, s.gachaLength, s.inputLootEntity) = abi.decode(value, (uint256, uint32, uint256));
    ...
    function batchRemove(uint256[] calldata entities) public virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete GachaComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _getEncodedValues(
        ...
        return
            abi.encode(
                value.gachaId, value.gachaLength, value.inputLootEntity
            );
    }
}