...
contract SwissTreasury is Ownable {
    ...
    ITIMEERC20 public immutable Swiss;
    ...
    constructor (
        ...
        require( _Swiss != address(0) );
        Swiss = ITIMEERC20(_Swiss);
    ...
    /**
        @notice allow approved address to deposit an asset for Swiss
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Swiss needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        limitRequirements(msg.sender, send_);
        Swiss.mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Swiss for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        Swiss.burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = sSwiss.balanceOf( msg.sender ); // Can only borrow against sOHM held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Swiss
        @param _amount uint
     */
    function repayDebtWithSwiss( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved as debtor" );
        require( isReserveSpender[ msg.sender ], "Not approved as spender" );

        Swiss.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(Swiss), _amount, _amount );
    }
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( Swiss.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Swiss valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        ...
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Swiss decimals
            value_ = _amount.mul( 10 ** Swiss.decimals() ).div( 10 ** IERC20( _token ).decimals() );
