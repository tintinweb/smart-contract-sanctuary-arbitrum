...
import "./interfaces/IHappinessBooster.sol";
import "../interfaces/ICreatureOwnerResolverRegistry.sol";
import "../interfaces/ICreatureOwnerResolver.sol";

/**
 * @title  HappinessBooster contract
 * @author Archethect
 * @notice This contract contains all functionalities for boosting happiness of Smols
 */
contract HappinessBooster is ReentrancyGuard, AccessControl, IHappinessBooster {
    ...
    mapping(uint256 => BoostType) public boosters;
    ...
    constructor(
        ...
        require(operator_ != address(0), "HAPPINESSBOOSTER:ILLEGAL_ADDRESS");
        require(admin_ != address(0), "HAPPINESSBOOSTER:ILLEGAL_ADDRESS");
        require(smolMarriage_ != address(0), "HAPPINESSBOOSTER:ILLEGAL_ADDRESS");
        require(smolHappiness_ != address(0), "HAPPINESSBOOSTER:ILLEGAL_ADDRESS");
        require(treasury_ != address(0), "HAPPINESSBOOSTER:ILLEGAL_ADDRESS");
    ...
    modifier onlyAdmin() {
        require(hasRole(ADMIN_ROLE, msg.sender), "HAPPINESSBOOSTER:ACCESS_DENIED");
        _;
    }

    modifier onlyOperator() {
        require(hasRole(OPERATOR_ROLE, msg.sender), "HAPPINESSBOOSTER:ACCESS_DENIED");
        _;
    }

    function boostHappiness(ICreatureOwnerResolverRegistry.Creature memory creature, uint256 boostType)
        public
        nonReentrant
    {
        require(isOwner(msg.sender, creature), "HAPPINESSBOOSTER:NOT_OWNER_OF_CREATURE");
        require(smolMarriage.isMarried(creature), "HAPPINESSBOOSTER:SMOL_NOT_MARRIED");
        require(boosters[boostType].valid, "HAPPINESSBOOSTER:NON_VALID_BOOST_TYPE");
        ICreatureOwnerResolverRegistry.Creature memory partner = areCreaturesEqual(
            ...
            : smolMarriage.getMarriage(creature).creature1;
        smolHappiness.increaseHappiness(creature, boosters[boostType].happinessBoost);
        smolHappiness.increaseHappiness(partner, boosters[boostType].happinessBoost);
        if (boosters[boostType].tokenType == 0) {
            IERC1155(boosters[boostType].contractAddress).safeTransferFrom(
                msg.sender,
                0x000000000000000000000000000000000000dEaD,
                boosters[boostType].id,
                boosters[boostType].requiredAmount,
                "0x0"
            );
        } else {
            IERC20(boosters[boostType].contractAddress).transferFrom(
                msg.sender,
                treasury,
                boosters[boostType].requiredAmount
            );
        }
        emit HappinessBoosted(creature, partner, boosters[boostType].happinessBoost);
    }

    function setAllowedBoosters(uint256[] calldata boostTypes, BoostType[] calldata boosters_) public onlyOperator {
        require(boostTypes.length == boosters_.length, "HAPPINESSBOOSTER:INVALID_ARRAY_LENGTH");
        for (uint256 i = 0; i < boosters_.length; i++) {
            boosters[boostTypes[i]] = boosters_[i];
        }
    }

    function removeAllowedBoosters(uint256[] calldata boostTypes) public onlyOperator {
        for (uint256 i = 0; i < boostTypes.length; i++) {
            delete boosters[boostTypes[i]];
        }
    }
...
/**
 * @title  IHappinessBooster contract
 * @author Archethect
 * @notice This contract contains all functionalities for boosting happiness of Smols
 */
interface IHappinessBooster {
    struct BoostType {
        bool valid;
    ...
    function boostHappiness(ICreatureOwnerResolverRegistry.Creature memory creature, uint256 boostType) external;

    function setAllowedBoosters(uint256[] calldata boostTypes, BoostType[] calldata boosters_) external;

    function removeAllowedBoosters(uint256[] calldata boostTypes) external;
}
