...
contract PositionHandler is PositionConstants, IPositionHandler, Ownable {
    ...
    function _increaseMarketPosition(
        ...
        uint256 collateralPrice = _getCollateralPrice(_prices);
    ...
    function modifyPosition(
        ...
        if (_txType == ADD_COLLATERAL || _txType == REMOVE_COLLATERAL) {
            ...
            } else if (_isDelayPosition(originalTxType)) {
    ...
    */
    ...
    function _addOrRemoveCollateral(
        ...
        if (_txType == ADD_COLLATERAL) {
            amountInUSD = priceManager.fromTokenToUSD(_getCollateralToken(_path), _amountIn, _getCollateralPrice(_prices));
            ...
            require(_amountIn <= _position.collateral 
                && _amountIn <= _position.reserveAmount, "IVLPRC/EXD"); //Invalid position reserved collateral, exceeded
    ...
    function _confirmDelayTransaction(
        ...
        {
            uint256 collateralDecimals = priceManager.getTokenDecimals(_getCollateralToken(_path));
            uint256 collateralPrice = _getCollateralPrice(_prices);
    ...
    function _decreasePosition(
        ...
        require(decreaseReserveAmount <= _position.reserveAmount, "ISFPRA"); //Insufficient position reserve amount
        ...
        {
            (usdOut, usdOutFee, collateralDelta) = _reduceCollateral(
                key, 
                _indexToken, 
                _sizeDelta, 
                _isLong, 
    ...
    function _reduceCollateral(
        ...
        if (_position.size == _sizeDelta) {
            ...
            // Reduce the _position's collateral by collateralDelta
            // transfer collateralDelta out
        ...
        {
            indexPrice = _getIndexPrice(_prices);
            _validateDecreasePosition(_indexToken, _isLong, indexPrice, _position);
        }

        return (usdOut, fee, collateralDelta);
    }
