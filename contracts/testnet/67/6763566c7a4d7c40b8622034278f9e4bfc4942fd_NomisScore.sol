...
contract NomisScore is ERC721, ERC721Burnable, Ownable, Pausable, EIP712 {
    ...
    /**
     * @dev A mapping of addresses to scores.
     */
    ...
    /**
     * @dev Emitted when a new score is set.
     * @param to The address to which the score is being set.
     * @param score The score being set.
     */
    event ScoreSet(address indexed to, uint16 score);

    /**
     * @dev Emitted when an existed score is updated.
     * @param tokenId The updated token id.
     * @param owner The address to which the score is being updated.
     * @param score The score being updated.
     */
    event ScoreUpdated(uint256 indexed tokenId, address indexed owner, uint16 score);

    /**
    * @dev Emitted when a token is burned.
    * @param owner The address of the owner of the token being burned.
    * @param tokenId The ID of the token being burned.
    */
    event TokenBurned(address indexed owner, uint256 indexed tokenId);

    /**
     * @dev The constructor sets the name and symbol of the ERC721 token, as well as the domain separator for EIP-712.
     */
    constructor() ERC721("NomisScore", "NMSS") EIP712("NMSS", "0.2") {
        _tokenIds.reset();
    }
     ...
     * @param score The score being set.
     * @param to The address to set the score for.
     */
    function _setScore(
        uint16 score, 
        address to
        ...
        Score storage scoreStruct = scores[to];
        scoreStruct.score = score;
        scoreStruct.updated = block.timestamp;
        scoreStruct.tokenId = tokenId;
        _safeMint(to, tokenId);
        _tokenIds.increment();

        emit ScoreSet(to, score);
    }

    /**
     * @dev Updates the score for the calling address and token id.
     * @param score The score being set.
     * @param to The address to set the score for.
     */
    function _updateScore(
        ...
        emit ScoreUpdated(scoreStruct.tokenId, to, score);
    }
    ...
    function setScore(
        ...
        require(score > 0, "setScore: Score cannot be zero");
        ...
        if (scores[to].score != 0) {
            _updateScore(score, to);
        }
        else {
            _setScore(score, to);
        }
    ...
    */
    function getScore(address addr) external view returns(uint16 score, uint256 updated, uint256 tokenId) {
        require(scores[addr].score != 0, "getScore: No score found for the specified address");

        Score storage _score = scores[addr];
