...
contract SaleERC20 is Pausable, Initializable {
    ...
    mapping(address => uint256) public boughtTokenToSellByUser; // User => amount
    mapping(address => uint256) public claimedTokenToSellByUser; // User => amount
    mapping(uint256 => mapping(address => bool)) public whitelists;  // round id => whitelist
    uint256 public totalTokenToSellSold;
    uint256 public totalSaleTokenCollected;
    uint256 public totalSaleTokenWithdrawed;
    ...
    function fund(uint256 amount) public {
        ...
        tokenToSell.safeTransferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount) public onlyOwner {
        require(serviceBalance >= amount, "Not enough service balance");
        serviceBalance -= amount;
        tokenToSell.safeTransfer(msg.sender, amount);
    }
    ...
    function createRound (
        ...
        uint256 tokenPrice,  // How much *token to sell* to give for one *sale token*
        ...
        public onlyOwner 
    {
        ...
        require(roundsLength == 0 || rounds[roundsLength-1].isActive == false || rounds[roundsLength-1].deadline < block.timestamp, "First stop ongoing round");
        
        for(uint256 i = 0; i < whitelist.length; i++) {
            whitelists[roundsLength][whitelist[i]] = true;
        }
    ...
    function stopRound(uint256 roundId) public onlyOwner {
        rounds[roundId].isActive = false;
    }

    function buy(uint256 saleTokenAmount) public whenNotPaused {
        Round memory ongoingRound = getOngoingRound();

        (, Tier userTier, , , uint8 allocationBonusPercent) = staking.getUserInfo(msg.sender);

        require(canParticipate_(msg.sender, ongoingRound, userTier), "User can't participate");

        uint256 tokenToSellAmount = saleTokenAmount * ongoingRound.tokenPrice;
        uint8 userAllocationBonus = allocationBonusPercent;
        uint256 userMaxAllocation = ongoingRound.maxAllocationPerUser + ongoingRound.maxAllocationPerUser * userAllocationBonus / 100;
        uint256 alreadyBought = boughtTokenToSellByUser[msg.sender];

        require(tokenToSellAmount <= userMaxAllocation - alreadyBought, "Allocation per user exceeded");
        require(tokenToSellAmount <= ongoingRound.maxAllocation - alreadyBought, "Round allocation exceeded");

        saleToken.safeTransferFrom(msg.sender, address(this), saleTokenAmount);

        totalSaleTokenCollected += saleTokenAmount;
        boughtTokenToSellByUser[msg.sender] += tokenToSellAmount; 
        totalTokenToSellSold += tokenToSellAmount;
        emit Bought(msg.sender, tokenToSellAmount);
    }

    function claim() public whenNotPaused {
        uint256 boughtTokenToSell = boughtTokenToSellByUser[msg.sender];
        uint256 withdrawedTokenToSell = claimedTokenToSellByUser[msg.sender];

        require(boughtTokenToSell > 0, "No tokens bought");

        uint256 percentsWithdrawn = withdrawedTokenToSell * 100 / boughtTokenToSell;
        uint8 accPercentsToGive;
        ...
        require(accPercentsToGive > percentsWithdrawn, "Nothing to claim");
        uint256 sumToGive = (accPercentsToGive - percentsWithdrawn) * boughtTokenToSell / 100;

        require(sumToGive <= serviceBalance, "Not enough service balance");
        tokenToSell.safeTransfer(msg.sender, sumToGive);
        serviceBalance -= sumToGive;
        claimedTokenToSellByUser[msg.sender] += sumToGive;
        emit Claimed(msg.sender, sumToGive);
    }

    function withdrawProfit() public onlyOwner {
        uint256 tokenCollected = totalSaleTokenCollected;
        uint256 tokenWithdrawed = totalSaleTokenWithdrawed;
        uint256 amountToWithdraw = tokenCollected - tokenWithdrawed;

        require(amountToWithdraw > 0, "Nothing to withdraw");

        totalSaleTokenWithdrawed = tokenCollected;
        saleToken.safeTransfer(msg.sender, amountToWithdraw);
    }

    function setVestingSchedule(uint256[] calldata claimTimes_, uint8[] calldata claimPercents_) public onlyOwner {
        require(claimTimes_.length == claimPercents_.length, "Array sizes must be the same");

        uint256 totalPercent = 0;
        for(uint256 i = 0; i < claimTimes_.length; i++) totalPercent += claimPercents_[i];
    ...
    function shiftVestingSchedule(uint256 secondsToShift) public onlyOwner {
        for(uint256 i = 0; i < claimTimes.length; i++) claimTimes[i] += secondsToShift;
    }
    ...
    function initialize(address owner_, address saleTokenAddr, address tokenToSellAddr, address stakingAddress) public initializer  {
        saleToken = IERC20(saleTokenAddr);
        tokenToSell = IERC20(tokenToSellAddr);
    ...
    function getOngoingRound() public view returns (Round memory) {
        ...
        require(round.isActive, "No active rounds");
        return round;
    }
    ...
    function canParticipate(address user) public view returns (bool) {
        ...
        (, Tier userTier, , ,) = staking.getUserInfo(user);
        return canParticipate_(user, round, userTier);
    }
    
    function canParticipate_(address user, Round memory round, Tier userTier) internal view returns (bool) {  
        return round.requiredTier == userTier && whitelists[round.id][user];
    }
    
    function canClaim(address user) public view returns (bool) {
        uint256 boughtTokenToSell = boughtTokenToSellByUser[user];
        uint256 withdrawedTokenToSell = claimedTokenToSellByUser[user];
        ...
        return accPercentsToGive > percentsWithdrawn;
    }

    function totalRaised() public view returns (uint256) {
        return totalSaleTokenCollected;
    }
}
...
contract Staking is Ownable, Pausable {
    ...
    function stake(uint256 poolId, uint256 amount, StakingTime time) public whenNotPaused {
        ...
        if (poolStakerInfo.stake > 0) {
            ...
            if (pending > 0) {
                ...
                raiseToken.transfer(msg.sender, pending);
            }
        }

        Tier currentTier = getTierByStakingAmount(poolStakerInfo.stake);  // Tier based on all the staked sum
    ...
    function unstake(uint256 poolId, uint256 amount) public whenNotPaused {
        ...
        poolStakerInfo.stake -= amount;
        poolStakerInfo.rewardDebt = poolStakerInfo.stake * poolInfo.accRaisePerShare / 1e12;
        poolInfo.token.transfer(msg.sender, amountToUnstake);
    ...
    function emergencyUnstake(uint256 poolId) public whenNotPaused {
        ...
        require(poolStakerInfo.stake >= 0, "Not enough balance");
    ...
    function withdraw(uint256 amount) public onlyOwner {
        ...
        serviceBalance -= amount;
        raiseToken.transfer(msg.sender, amount);
    }
    ...
    function getAllTicketInfos() public view returns (TicketInfo[] memory) {
        uint256 stakersNum = stakerAddresses.length;
        uint256 currentQueriedTicketInfosIndex;
        RequiredTierStakeInfo memory requiredTierStakeInfo_ = requiredTierStakeInfo;

        TicketInfo[] memory queriedTicketInfos = new TicketInfo[](stakersNum);

        for (uint i = 0; i < stakersNum; i++) {
            address stakerAddress = stakerAddresses[i];
            PoolStakerInfo memory poolStakerInfo = poolStakerInfos[0][stakerAddress];

            Tier userTier = getTierByStakingAmount_(requiredTierStakeInfo_, poolStakerInfo.stake);

            if (userTier == Tier.Dealer || userTier == Tier.Merchant) {
                queriedTicketInfos[currentQueriedTicketInfosIndex] = TicketInfo({
                    user: stakerAddress,
                    amount: poolStakerInfo.stake * TICKETS_PER_100_RAISE / (100 * RAISE_DECIMAL)
                });
                currentQueriedTicketInfosIndex++;
            }
        }

        TicketInfo[] memory queriedTicketInfosActualSize = new TicketInfo[](currentQueriedTicketInfosIndex);

        for (uint i = 0; i < currentQueriedTicketInfosIndex; i++) 
            queriedTicketInfosActualSize[i] = queriedTicketInfos[i];
        
        return queriedTicketInfosActualSize;
    }

    function getStakersByTier(Tier tier) public view returns(address[] memory) {
        uint256 stakersNum = stakerAddresses.length;

        uint256 currentQueriedStakersIndex;
        address[] memory queriedStakers = new address[](stakersNum);
        RequiredTierStakeInfo memory requiredTierStakeInfo_ = requiredTierStakeInfo;
        ...
        for (uint i = 0; i < stakersNum; i++) {
            ...
            if (userTier == tier) {
                queriedStakers[currentQueriedStakersIndex] = stakerAddress;
                currentQueriedStakersIndex++;
            }
        }

        address[] memory queriedStakersActualSize = new address[](currentQueriedStakersIndex);

        for (uint i = 0; i < currentQueriedStakersIndex; i++) 
            queriedStakersActualSize[i] = queriedStakers[i];
        
        return queriedStakersActualSize;
    }
    ...
    function getTierByStakingAmount(uint256 amount) public view returns (Tier tier) {
        return getTierByStakingAmount_(requiredTierStakeInfo, amount);
    }
    
    function getTierByStakingAmount_(RequiredTierStakeInfo memory requiredTierStakeInfo_, uint256 amount) internal pure returns (Tier tier) {
