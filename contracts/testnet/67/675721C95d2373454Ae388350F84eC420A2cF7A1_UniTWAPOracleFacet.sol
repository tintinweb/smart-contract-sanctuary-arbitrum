...
contract UniTWAPOracleFacet is IUniTWAPOracle {
    ////////////
    // Errors //
    ////////////

    error TWAPOracle__InvalidParam();
    error TWAPOracle__ConfigsDecimalsIsZero();
    error TWAPOracle__ConfigsLengthIsZero();
    ...
    event TokenTWAPConfigUpdated(
        address indexed underlying,
        LibUniTWAPOracle.PriceSource priceSource,
    ...
    function initUniTWAPOracle(uint32 _anchorPeriod) external {
        LibAccessControlEnumerable.checkRole(Constants.DEPLOYER_ROLE);
        if (_anchorPeriod == 0) {
            revert TWAPOracle__InvalidParam();
        }
        LibUniTWAPOracle.initialize(_anchorPeriod);
    }

    /// @notice 获取uni v3的价格，保留18位小数，以usdc计价
    function getTWAPUniV3(address asset) external view returns (uint256) {
        return LibUniTWAPOracle._getTWAPUniV3(asset);
    }

    /// @notice 获取现货预言机锚定期
    function getSpotOracleAnchorPeriod() external view returns (uint256) {
        LibUniTWAPOracle.Layout storage l = LibUniTWAPOracle.layout();
        return (l.anchorPeriod);
    }
     ...
     */
    function getUniswapTwap(LibUniTWAPOracle.TokenTWAPConfig memory _config) external view returns (uint256) {
        return LibUniTWAPOracle._getUniswapTwap(_config);
    }

    /// @notice 获取UNI TWAP预言机配置
    function getUniTWAPConfig(address _underlying) external view returns (LibUniTWAPOracle.TokenTWAPConfig memory) {
        LibUniTWAPOracle.Layout storage l = LibUniTWAPOracle.layout();
        return l.tokenTWAPConfigs[_underlying];
    }
     ...
     * @param tokenTWAPConfigs_ config array
     * @custom:access Only Owner
     * @custom:error Zero length error thrown, if length of the array in parameter is 0
     */
    function setUniTWAPConfigs(LibUniTWAPOracle.TokenTWAPConfig[] memory tokenTWAPConfigs_) external {
        if (tokenTWAPConfigs_.length == 0) revert TWAPOracle__ConfigsLengthIsZero();
        uint256 numTokenTWAPConfigs = tokenTWAPConfigs_.length;
        for (uint256 i; i < numTokenTWAPConfigs; ) {
            setUniTWAPConfig(tokenTWAPConfigs_[i]);
            unchecked {
                ++i;
            }
        }
    ...
    /**
     * @notice The static token configurations which define what prices are supported and how
     * @param tokenTWAPConfig Token twap config struct
     * @custom:access Only Owner
     * @custom:error Range error is thrown if maxStale period of token is not greater than zero
     * @custom:event Emits TokenTWAPConfigUpdated event on successfully setting of the token config
     */
    function setUniTWAPConfig(
        LibUniTWAPOracle.TokenTWAPConfig memory tokenTWAPConfig
    ) public notNullAddress(tokenTWAPConfig.underlying) notNullAddress(tokenTWAPConfig.uniswapMarket) {
        LibAccessControlEnumerable.checkRole(Constants.ADMIN_ROLE);

        LibUniTWAPOracle.Layout storage l = LibUniTWAPOracle.layout();
        if (tokenTWAPConfig.baseDecimals == 0 || tokenTWAPConfig.quoteDecimals == 0) {
            revert TWAPOracle__ConfigsDecimalsIsZero();
        }
        l.tokenTWAPConfigs[tokenTWAPConfig.underlying] = tokenTWAPConfig;
        emit TokenTWAPConfigUpdated(
            tokenTWAPConfig.underlying,
            tokenTWAPConfig.priceSource,
            tokenTWAPConfig.baseDecimals,
            tokenTWAPConfig.quoteDecimals,
            tokenTWAPConfig.isUniswapReversed,
            tokenTWAPConfig.uniswapMarket
        );
    }
}
