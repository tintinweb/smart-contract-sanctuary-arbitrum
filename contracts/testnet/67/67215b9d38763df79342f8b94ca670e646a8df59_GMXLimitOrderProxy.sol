...
import "./AbstractLimitOrderProxy.sol";
import "../options/AbstractOptions.sol";
import "../options/IOptions.sol";
import "../dex/gmx/interfaces/IOrderBook.sol";

/**
 * @title Proxy contract to GMX swap
 */
contract GMXLimitOrderProxy is AbstractLimitOrderProxy {
    event GMXLimitOrderCanceled(uint256 orderIndex);
    event GMXSetMinOutPerc(uint256 minOutPerc);

    /// @notice OrderBook contract of GMX
    IOrderBook immutable public orderBook;

    /// @notice Minimum amount by percentage of expected takerAmount to be received
    uint256 public minOutPerc;

    //slither-disable-next-line similar-names
    constructor(ERC20 _makerToken, ERC20 _takerToken, address _orderBook) AbstractLimitOrderProxy(_makerToken, _takerToken) {
      orderBook = IOrderBook(_orderBook);
      minOutPerc = 50; // 50%
    }

    /**
     * @notice set {minOutPerc}
     */
    function setMinOutPerc(uint256 minOutPerc_) external onlyOwner {
        minOutPerc = minOutPerc_;
        emit GMXSetMinOutPerc(minOutPerc_);
    }

    //slither-disable-next-line similar-names
    function createOrder(uint256 optionID, uint256 makerAmount_, uint256 takerAmount_, LimitOrderType orderType_) external override nonReentrant {
        require(makerAmount_ > 0 && takerAmount_ > 0, 'AbstractLimitOrderProxy: Invalid option params');
        OrderStruct memory limitOrder = OrderStruct({
            optionContractAddress: payable(msg.sender),
            optionID: optionID,
            makerAmount: makerAmount_,
            takerAmount: takerAmount_,
            maturityDate: 300 days,
            timestamp: block.timestamp,
            state: LimitOrderState.Active,
            orderType: orderType_
        });
        limitOrders[msg.sender][optionID] = limitOrder;
    

        require(IERC20(makerToken).approve(address(orderBook), makerAmount_), "Approve Maker token for Order book failed");
        uint256 minExecutionFee = orderBook.minExecutionFee();

        address[] memory path = new address[](2);
        path[0] = address(makerToken);
        path[1] = address(takerToken);

        require(payable(address(this)).balance >= minExecutionFee, "[GMXLimitOrderProxy] Insufficient ETH Balance");

        orderBook.createSwapOrder{value: minExecutionFee} (
            path,
            makerAmount_,
            takerAmount_ * minOutPerc / 100,
            takerAmount_ / makerAmount_,
            true,
            minExecutionFee,
            false,
            false
        );
        emit LimitOrderCreated(msg.sender, optionID, makerAmount_, takerAmount_);
    }

    /// @dev See {ILimitOrderProxy}
    function reinjectLiquidity(address payable optionContractAddress, uint256 optionID, uint256 amount) external override onlyOwner nonReentrant {
        require(takerToken.balanceOf(address(this)) >= amount, "Insufficient proxy balance");
        require(takerToken.approve(optionContractAddress, amount), "Cannot approve taker token for Option contract");
        _reinjectLiquidity(optionContractAddress, optionID, amount);
    }

    /**
     * @notice Cancel placed order on GMX
     * @param orderIndex Order index on GMX
     */
    function cancelSwapOrder(uint256 orderIndex) public onlyOwner nonReentrant {
        orderBook.cancelSwapOrder(orderIndex);
        emit GMXLimitOrderCanceled(orderIndex);
    }
...
import "../options/AbstractOptions.sol";
import "./ILimitOrderProxy.sol";

/**
 * @title Abstract class for Limit Order proxy
 */
abstract contract AbstractLimitOrderProxy is ILimitOrderProxy, Ownable, ReentrancyGuard {
    using SafeERC20 for ERC20;

    /// Maker & taker tokens
    ERC20 immutable public takerToken;
    ERC20 immutable public makerToken;

    /// @dev Mapping limit orders by Option contract address & option ID
    mapping(address => mapping(uint256 => OrderStruct)) public limitOrders;

    //slither-disable-next-line similar-names
    constructor(ERC20 _makerToken, ERC20 _takerToken) {
        makerToken = ERC20(_makerToken);
        takerToken = ERC20(_takerToken);
    }

    /**
     * @dev See {ILimitOrderProxy}
     */
    function reinjectLiquidity(address payable optionContractAddress, uint256 optionID, uint256 amount) external virtual;

    /**
     * @notice Execute re-inject to Option contract, and then to Liquidity Pool
     */
    function _reinjectLiquidity(address payable optionContractAddress, uint256 optionID, uint256 amount) internal {
        AbstractOptions optionContract = AbstractOptions(optionContractAddress);
        optionContract.reinjectLiquidity(optionID, amount);
    }
 ...
 */
abstract contract AbstractOptions is IOptions, Ownable, ReentrancyGuard {
    using SafeERC20 for ERC20;

    bytes32 public constant PRICE_INFO_HASH =
        keccak256(
            "PriceInfo(uint8 optionType,uint256 price,uint256 strike,uint256 maturity,uint256 nonce,uint256 expiration,address buyer)"
        );
    ...
    ERC20 immutable public underlyingAsset;
    ERC20 immutable public settlementAsset;
    string public underlyingAssetSymbol;
    string public settlementAssetSymbol;

    string public strikeCurrencySymbol;
    ...
    mapping(bytes32 => uint256) private _claimedARPAmount;
    ...
    string private name = "MGH protocol";
     ...
     */
    constructor(MGHPriceProvider pp, OptionType _type, ILimitOrderProxy _limitOrderProxy, address _underlyingToken, address _settlementToken, AbstractLiquidityPool _pool, SettlerProxy _settlerProxy, PricerProxy _pricerProxy, address payable _settlementFeeRecipient) {
        ...
        underlyingAsset = ERC20(_underlyingToken);
        settlementAsset = ERC20(_settlementToken);

        if (address(underlyingAsset) != address(0)) {
            underlyingAssetSymbol = underlyingAsset.symbol();
        } else {
            underlyingAssetSymbol = "ETH";
        }

        if (address(settlementAsset) != address(0)) {
            settlementAssetSymbol = settlementAsset.symbol();
        } else {
            settlementAssetSymbol = "ETH";
        }

        // TODO set this property dynamically - hardcoded so far since our options are all priced with USDC
        strikeCurrencySymbol = "USDC";
    }
    ...
    function nonce(
        address buyer
    ) public view returns (uint256 buyerPriceInfoNonce) {
    ...
    function _verifySignature(
        ...
        require(priceInfo.nonce == priceInfoNonce[msg.sender]);
        ...
        if (optionType == OptionType.Put) {
            require(priceInfo.optionType == 1, "Not for put option");
        } else if (optionType == OptionType.Call) {
            require(priceInfo.optionType == 2, "Not for call option");
        } else {
            revert NotCallOrPut();
        }
    ...
    function _getClaimedARP(uint256 optionID, address provider) internal view returns(uint256) {
        return _claimedARPAmount[CommonUtils.keyOf(optionID, provider)];
    }

    function _setClaimedARP(uint256 optionID, address provider, uint256 amount) internal {
        _claimedARPAmount[CommonUtils.keyOf(optionID, provider)] = amount;
    }
    ...
    function create(
        ...
        uint256 snapshotId = pool.currentSnapshotId() + 1;
        options[optionID].snapshotId = snapshotId;

        _transferPremium(premium);
        _sendProtocolFees(protocolFee);
        _optionsSize++;

        settlementAsset.safeTransfer(address(pool), poolPremium);
        require(pool.snapshot(option) == snapshotId, "Invalid snapshot id creatation");
        emit Create(optionID, msg.sender, quantityWad, strikeWad, priceInfo.price, premium, protocolFee, poolPremium);
    }
    ...
    function _transferPremium(uint256 premium) internal virtual;

    function _sendProtocolFees(uint256 protocolFee) internal virtual;

    function _isExerciseSwap(Option storage option) internal virtual returns (bool);

    /// @dev See {IOptions}
    function exercise(uint256 optionID) external payable override nonReentrant {
    ...
    function _exercisedOption(uint256 optionID) internal {
        ...
        _placeLimitOrder(optionID);
    }

    /**
     * @notice Place a limit order to DEX of {limitOrderProxy}
     */
    function _placeLimitOrder(uint256 optionID) internal {
        require(optionID < _optionsSize, '!optionID');
        Option storage option = options[optionID];
        require(option.state == State.Exercised, '!Exercised');
        //slither-disable-next-line similar-names
     ...
     */
    function batchPostExercised(uint256[] calldata optionIDs, address[] calldata providers) external onlyOwner nonReentrant {
     ...
     */
    function batchDisbursePremium(uint256 optionID, address[] calldata providers) external onlyOwner nonReentrant {
     ...
     * @dev See {IOptions}
     */
    function reinjectLiquidity(uint256 optionID, uint256 amount) external nonReentrant {
        _reinjectLiquidity(optionID, amount);
        emit ReinjectLiquidity(msg.sender, optionID, amount);
    }

    function _reinjectLiquidity(uint256 optionID, uint256 amount) internal {
        ...
        require(settlementAsset.approve(address(pool), amount), "Approve Settlement Asset for pool failed");
        pool.reinjectLiquidity(amount);
    }

    /**
     * @notice Provider claims premium shares of an option
     */
    function claimPremium(uint256 optionID) external nonReentrant returns (uint256) {
        return pool.claimPremiumFor(options[optionID].snapshotId, msg.sender);
    }

    /**
     * @notice Provider claims premium shares of multiple options
     */
    function claimPremiums(uint256[] calldata optionIDs) external nonReentrant returns (uint256 claimed) {
        // slither-disable-start calls-loop
        for (uint i = 0; i < optionIDs.length; i++) {
     ...
     */
    function batchClaimARP(uint256[] calldata optionIDs, address[] calldata providers) external onlyOwner nonReentrant {
        for (uint i = 0; i < optionIDs.length; i++) {
            uint256 optionID = optionIDs[i];
    ...
    /**
     * @notice Claim ARP profit of an option
     */
    function claimARP(uint256 optionID) external nonReentrant returns (uint256) {
        return _claimARP(optionID, msg.sender);
    }

    /**
     * @notice Claim ARP profit of multiple options
     */
    function claimARPs(uint256[] calldata optionIDs) external nonReentrant returns (uint256 claimed) {
        for (uint i = 0; i < optionIDs.length; i++) {
    ...
    function _claimARP(uint256 optionID, address provider) internal returns (uint256) {
        ...
        uint256 amountClaimed = _getClaimedARP(optionID, provider);
        ...
        _setClaimedARP(optionID, provider, filledShares);
     ...
     */
    function resolvePending(uint256 optionID) external nonReentrant {
        require(optionID < _optionsSize, "!optionID");
        Option storage option = options[optionID];
...
/**
 * @title Interface for GMX OrderBook smart contract
 * @dev https://github.com/gmx-io/gmx-contracts
 */
interface IOrderBook {
    // Minimum execution fee of MGX protocol
    function minExecutionFee() external returns (uint256);

    /**
     * @notice Create a swap order by given token pair 
     * @param _path Array of tokenIn (to sell) & tokenOut (expected to receive)
     * @param _amountIn Amount of tokenIn
     * @param _minOut Minimum amount of tokenOut to execute swap
     * @param _triggerRatio Expected ratio tokenIn/tokenOut to swap
     * @param _triggerAboveThreshold If maker want to swap above trigger ratio
     * @param _executionFee Fee for protocol, must be greater than or equal {minExecutionFee}
     * @param _shouldWrap true if wrap {_executionFee} in {_amountIn}
     * @param _shouldUnwrap Unknown
     */
    ...
    /**
     * @notice Maker cancel a swap order placed by his/her-self
...
interface ILiquidityPool{
    event Withdraw(address indexed account, uint256 quantity);
    event Provide(address indexed account, uint256 quantity);
    ...
    function totalLiquidityBalance() external view returns (uint256 balance);
 ...
 */
abstract contract AbstractLiquidityPool is ILiquidityPool, Ownable, PoolToken, ReentrancyGuard {
    using SafeERC20 for ERC20;
    ...
    mapping(bytes32 => bool) private _reducedProviders;
    ...
    mapping(bytes32 => bool) private _claimedPremiumProviders;
    ...
    constructor(address _liquidityToken, address _settlementToken) {
        ...
        require(liquidityToken.approve(owner(), _MAX_INT), "Cannot approve liquidity tokenfor owner");
        require(settlementToken.approve(owner(), _MAX_INT), "Cannot approve settlement token for owner");
    }
    ...
    /** @notice Returns the total liquidity token available in the pool (locked or not) */
    function totalLiquidityBalance() public view override returns (uint256) {
        return _totalLiquidityBalance();
    }
    ...
    function provideLiquidity(uint256 quantity) payable override external {
        ...
        emit Provide(msg.sender, quantity);
    }
    ...
    function _raiseLiquidityShares(uint256 quantity) internal {
     ...
     * @dev Usually amount = maxBurn for convenience.
     */
    function withdrawLiquidity(uint256 quantity) override public nonReentrant {
        _withdrawLiquidity(msg.sender, quantity);
        emit Withdraw(msg.sender, quantity);
    }

    /// @dev See {ILiquidityPool}
    function withdrawAll() override public nonReentrant {
        address provider = msg.sender;
        uint256 locked = _lockedBalanceOf(provider);
        require(locked <= balanceOf(provider), "Not enough unlocked tokens");
        uint256 quantity = balanceOf(provider) - locked;

        _withdrawLiquidity(provider, quantity);
        emit Withdraw(provider, quantity);
    }

    function _withdrawLiquidity(address provider, uint256 quantity) virtual internal;

    function _reduceLiquiditySharesOf(address receiver, uint256 quantity) internal {
        _burn(receiver, quantity);
    }

    function _lockLiquidity(uint256 quantity) internal {
        require(quantity > 0, "!quantity");
        require(_totalLiquidityBalance() != 0, "Pool Error : minimum liquidity available in the pool is required to be locked");
        require(totalSupply() >= quantity + totalLockedQuantity, "Not enough unlocked balance");
     ...
     * @param spender the token spender
     */
    function liquidityTokenAllowance(address owner, address spender) override external view returns (uint256 allowance){
        allowance = liquidityToken.allowance(owner, spender);
    }
     ...
     * @param spender the token spender
     */
    function settlementTokenAllowance(address owner, address spender) override external view returns (uint256 allowance){
        allowance = settlementToken.allowance(owner, spender);
    }

    /**
     * @notice set allowance of settlement token, used for limit order to be able to transfer liquidity tokens
     */
    function setSettlementTokenAllowance(uint quantity, address spender) override external onlyOwner {
        settlementToken.safeApprove(spender, quantity);
    }

    /// @dev Liquidity token decimals
    function decimals() public view override returns (uint8) {
        return liquidityToken.decimals();
    }

    /**
     * @notice Take a snapshot for a given option
     */
    function snapshot(IOptions.Option calldata option) public onlyRole(SNAPSHOT_ROLE) returns (uint256 snapshotId) {
        snapshotId = super._snapshot();
        activeSnapshotIds.push(snapshotId);

        uint256 totalAmount = option.liquidityAmount;
        uint256 premium = option.premiumWad;
        _lockLiquidity(totalAmount);
     ...
     */
    function deactivate(uint256 snapshotId, bool needUnlock) public onlyRole(SNAPSHOT_ROLE) {
    ...
    function _lockedBalanceOf(address provider) internal view returns(uint256) {
        ...
        uint256 foundIdx = _MAX_INT; // Init with Max index
        ...
        for (uint256 i = len - 1; ; i--) {
            ...
            if (foundIdx > snapshotId) {
                (foundIdx, balanceAt) = balanceOfAt(provider, snapshotId);
            }
            (,totalAt) = totalSupplyAt(snapshotId);
            lockedAmount = _isReducedLiquidity(snapshotId, provider) ? 0 : totalLockedAt[snapshotId] * balanceAt / totalAt;
            // TODO: Maybe have to add 1 to locked amount due to surplus of division
            // totalLocked += (lockedAmount == 0 ? 0 : Math.min(balanceAt, lockedAmount + 1)); 
    ...
    function _getSharesOfAt(uint256 amount, address provider, uint256 snapshotId) internal view returns(uint256 _shares, uint256 _balanceAt, uint256 totalSupplyAt_, uint256 foundIdx) {
        (foundIdx,_balanceAt) = balanceOfAt(provider, snapshotId);
        (, totalSupplyAt_) = totalSupplyAt(snapshotId);
        _shares = (amount * _balanceAt)/totalSupplyAt_;
    }

    function _isReducedLiquidity(uint256 snapshotId, address provider) internal view returns(bool) {
        return _reducedProviders[CommonUtils.keyOf(snapshotId, provider)];
    }

    function _setReducedLiquidity(uint256 snapshotId, address provider) internal {
        _reducedProviders[CommonUtils.keyOf(snapshotId, provider)] = true;
    }

    function _removeReducedLiquidity(uint256 snapshotId, address provider) internal {
        _reducedProviders[CommonUtils.keyOf(snapshotId, provider)] = true;
    }
     ...
     */
    function liquidate(uint256 snapshotId, address[] calldata providers) public onlyRole(SNAPSHOT_ROLE) {
        require(_isSnapshotIdActive(snapshotId), "Snapshot is not active");
        // totalSupplyAt is same to all providers at snapshotId
        (,uint256 totalSupplyAt_) = totalSupplyAt(snapshotId);
        for (uint i = 0; i < providers.length; i++) {
            _liquidateFor(providers[i], snapshotId, totalSupplyAt_);
        }
    }
     ...
     */
    function _liquidateFor(address provider, uint256 snapshotId, uint256 totalSupplyAt_) internal {
        if (_isReducedLiquidity(snapshotId, provider)) return;

        (,uint256 balanceAt) = balanceOfAt(provider, snapshotId);
        uint256 totalLockedAt_ = totalLockedAt[snapshotId];
        uint256 lockedAt_ = lockedAt[snapshotId];
        uint256 reduceShares = (balanceAt * totalLockedAt_) / totalSupplyAt_;

        if (reduceShares > 0 && reduceShares <= lockedAt_) {
            uint256 scanned = _liquidityTotalScanned[snapshotId];
            // If, Scanned all providers for liquidate this option
            if (scanned + balanceAt == totalSupplyAt_) {
                // But, There are few small amount is not allocated, allocate to last provider
                if (lockedAt_ > reduceShares) {
                    reduceShares = lockedAt_;
                }
                _removeReducedLiquidity(snapshotId, provider);
                _deactivate(snapshotId, false);
            } else {
                lockedAt[snapshotId] = lockedAt_ - reduceShares;
                _liquidityTotalScanned[snapshotId] = scanned + balanceAt;
            }
            _unlockLiquidity(reduceShares);
    ...
    function _isClaimedPremium(uint256 snapshotId, address provider) internal view returns(bool) {
        return _claimedPremiumProviders[CommonUtils.keyOf(snapshotId, provider)];
    }

    function _setClaimedPremium(uint256 snapshotId, address provider) internal {
        _claimedPremiumProviders[CommonUtils.keyOf(snapshotId, provider)] = true;
    }

    //slither-disable-start costly-loop
    function _removeClaimedPremium(uint256 snapshotId, address provider) internal {
        delete _claimedPremiumProviders[CommonUtils.keyOf(snapshotId, provider)];
    }
    //slither-disable-end costly-loop
     ...
     */
    function disbursePremiums(uint256 snapshotId, address[] calldata providers) public onlyRole(SNAPSHOT_ROLE) {
        require(_isSnapshotIdActive(snapshotId), "Snapshot is not active");
        bool allPremiumClaimed = false;
        // totalSupplyAt is same to all providers at snapshotId
        (,uint256 totalSupplyAt_) = totalSupplyAt(snapshotId);
        for (uint i = 0; i < providers.length; i++) {
            (,allPremiumClaimed) = _disbursePremiumFor(snapshotId, providers[i], totalSupplyAt_);
        }
     ...
     */
    function claimPremiumFor(uint256 snapshotId, address provider) public onlyRole(SNAPSHOT_ROLE) returns(uint256 premiumShares) {
        (,uint256 totalSupplyAt_) = totalSupplyAt(snapshotId);
        (premiumShares,) = _disbursePremiumFor(snapshotId, provider, totalSupplyAt_);
    }

    /**
     * @notice Disburse Premium shares of an option (at snapshotId) to provider
     */
    function _disbursePremiumFor(uint256 snapshotId, address provider, uint256 totalSupplyAt_) internal returns (uint256 premiumShares, bool allPremiumClaimed) {
        require(!_isClaimedPremium(snapshotId, provider), "Provider already claimed premium");
        ...
        if (premiumShares > 0 && premiumShares <= remainPremiumAt_) {
            uint256 scanned = premiumTotalScanned[snapshotId];
            // If, Scanned all providers for premium of this option
            if (scanned + balanceAt == totalSupplyAt_) {
                // Now ew expect remaining premium to be claimed equals to premiumShares of provider
                // If, There is a small remain premium is not claimed after all providers claimed
                if (remainPremiumAt_ > premiumShares) {
                    premiumShares = remainPremiumAt_;
                }
                allPremiumClaimed = true;
                _removeClaimedPremium(snapshotId, provider);
            } else {
                remainPremiumAt[snapshotId] = remainPremiumAt_ - premiumShares;
                premiumTotalScanned[snapshotId] = scanned + balanceAt;
            }

            _raiseLiquiditySharesOf(provider, premiumShares);
            _setClaimedPremium(snapshotId, provider);
            emit DisbursePremiumFor(snapshotId, provider, premiumShares);
        }
    }

    /**
     * @notice return true if snapshot id (created for an EXERCISED option) is still active. This means all liquidity of the option are NOT liquidated yet.
     */
    function isSnapshotIdActive(uint256 snapshotId) external view returns(bool) {
        return _isSnapshotIdActive(snapshotId);
    }
    ...
    function _removeActiveSnapshot(uint256 snapshotId) internal {
        ...
        if (idx < activeSnapshotIds.length) {
            ...
            for (uint i = idx; i < activeSnapshotIds.length - 1; i++){
