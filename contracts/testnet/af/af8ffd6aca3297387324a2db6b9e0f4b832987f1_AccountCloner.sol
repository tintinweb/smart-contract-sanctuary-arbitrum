...
contract AccountCloner {
    ...
    // Set accountImplementation, only by deployer of previous accountImplementation
    function setAccountImplementation(address accountImplementation_) public {
        require(IFace(accountImplementation).owner() == msg.sender, "AccountCloner: Only deployer");
        accountImplementation = accountImplementation_;
    }

    // Get init code used to call createAccount for owner with salt
    ...
    // Create account if needed, and return its address
    function createAccount(address owner, uint256 salt) external returns (address account) {
        ...
        if (account.code.length == 0) {
            address clone = Clones.cloneDeterministic(accountImplementation, _ownerSalt(owner, salt));

            // Assert cloned to predicted address
            assert(clone == account);

            IFace(account).initialize(owner);
        }
    }
    ...
    // Add owner to salt, to get one account per owner
