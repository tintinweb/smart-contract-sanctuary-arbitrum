...
contract GFarmTradingStorageV5 {
    ...
    //TokenInterfaceV5 public dai; // dai token
    IERC20 public dai;
    TokenInterfaceV5 public linkErc677; // link token

    // Trading variables
    uint public maxTradesPerPair = 10;
    uint public maxTradesPerBlock = 10;
    ...
    uint public nftSuccessTimelock = 0;                 // 50 blocks
    ...
    constructor(address govAddr, address daiAddr, address linkAddr) {
        require(govAddr != address(0), "GOV ADDRESS IS NIL");
        require(daiAddr != address(0), "DAI ADDRESS IS NIL");
        gov = govAddr;
        dai = IERC20(daiAddr);
        linkErc677 = TokenInterfaceV5(linkAddr);
    }
    ...
    function updateLinkToken(address _token) external onlyGov{
        require(_token != address(0));
        require(trading.isPaused() && callbacks.isPaused(), "NOT_PAUSED");
        linkErc677 = TokenInterfaceV5(_token);
    }
    ...
    function claimFees() external onlyGov{
        dai.safeTransfer(gov, govFeesDai);
        govFeesDai = 0;
    }
