...
contract Card {
    ...
    address[] internal players; // all players
    ...
    function commit(uint256[] calldata selectedCards, bytes32 commitmentHash) external {
    ...
    // reveal cards
    function reveal(uint256[] calldata revealedCards) external {
        require(status == 2, "Game not started");
        require(playerInfo[msg.sender].health > 0, "You have lost the game!");
    ...
    function autoCommit() external {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            this.commit(playerInfo[account].candidateCards, hash);
        }
    }

    function autoReveal() external {
        uint256 len = players.length;
        for (uint256 i = 0; i < len; i++) {
            address account = players[i];
            this.reveal(playerInfo[account].candidateCards);
        }
    }
    ...
    function _exist(uint256 card, uint256[] memory cards) internal view returns (bool) {
    ...
    function _isWinPK(uint256 card1, uint256 card2) internal view returns (bool) {
        // 1 kill 2
        // 2 kill 3
        // 3 kill 1
        unchecked {
