...
contract Treasury is ContractGuard {
  ...
  uint256 private constant ROLL_IN_PROGRESS = 10;
  ...
  IRandomizer public randomizer =
    IRandomizer(0x5b8bB80f2d72D0C85caB8fB169e8170A05C94bAF); // 0x923096Da90a3b60eb7E12723fA2E1547BA9236Bc goerli
  // exclusions from total supply
  uint256[] public epochArray = [6, 7, 8, 9];
  uint256[] public epochArray2 = [3, 4, 5];
  uint256 public randomSelector = 5;
  uint256 callbackGasLimit = 100000000;
  ...
  function initialize(
    ...
    nativePriceOne = 180000000000000; // Set the peg to 0.00018
  ...
  function requestRandomWords(uint256 _epoch) internal returns (uint256) {
    ...
    if (randomSwitch) {
     ...
     //IOracle(nativeOracle).setPeriod(epochPeriods[epoch-1]); carefull here, some more testig to be done!!!!
  ...
  function setWhitelistNative(address _address) external onlyOperator {
    IERC20Taxable(native).setWhiteList(_address);
  }

  function setWhitelistShare(address _address) external onlyOperator {
    IERC20Taxable(share).setWhiteList(_address);
  }
