...
contract Vault is ReentrancyGuard, AccessControlBase, IVault {
    ...
    function rebalanceDeposit(
        ...
        // todo we would want to add non-re-entrant maybe?
        _isRebalancer();
    ...
    function _collectWagerFees(
        ...
        if (wagerFee_ == 0) return(_amountEscrow, 0);
        ...
        emit WagerFeesCollected(_tokenEscrowIn, tokenToUsdMin(_tokenEscrowIn, feeAmountCharged_), feeAmountCharged_);
     ...
     * @return afterFeeAmount_  todo note add description
     * @return feeAmount_ todo note add description
     */
   ...
   /**
    ...
    function _payout(
        ...
        _validateBufferAmount(_addressTokenOut);
        _transferOut(
            _addressTokenOut,
            _toPayOnNet, 
            _recipient
    ...
    function getMinPrice(address _token) public override view returns (uint256 priceLowerBound_) {
        ...
        priceLowerBound_ = IVaultPriceFeedGMX(priceFeed).getPrice(
            ...
            false, /** previously includeAmmPrice */
            true /** previously useSwapPricing */
        );
    }

    /**
     * @notice returns the amount of a specitic tokens can be redeemed for a certain amount of USDC 
     ...
     */
    function tokenToUsdMin(address _tokenToPrice, uint256 _tokenAmount) public override view returns (uint256 usdAmount_) {
    ...
    /**
     * @notice returns the amount of fees (in basis points) that will be charged for a swap
     * @param _tokenInAddress address of the asset being sold (going into the WLP)
     * @param _tokenOutAddress address of the tokenOut (so the asset being bought)
     * @param _amountIn amount of the tokenIn (so the asset being sold)
     * @return feeBasisPoint_ the fee that will be charged 
     */
    function getFeeBasisPoint(
        address _tokenInAddress,
        address _tokenOutAddress,
        uint256 _amountIn
    ) internal view returns (uint256 feeBasisPoint_) {
        feeBasisPoint_ = _getFeeBasisPoints(_tokenInAddress, _tokenOutAddress, _amountIn);
    }

    /**
     * @notice returns the USD value of 1 WLP token
     * WATCH OUT WHAT DO WE SET THE MAXIMUMUM OR MINUTMM?
     */
    function getWlpValue() external view returns (uint256 wlpValue_) {
        // todo: we need to think about if we want to use the lower price bound of the oracle (false) or the upper price bound (true). If we use the lower bound, i think the value will be more favourable 
        // I think true is more favourable, since it will use the higher bound price to value the assets in the WLP and make WLP more expensive 
        wlpValue_ = IWLPManager(managerAddress).getPrice(false);
    }
     ...
     * todo: probably should be the timelock modifier since this is so impactful (this could rug if you implement a malicious vaultMaanger)
     */
