...
contract CollectionManager is ICollectionManager, Initializable, DAOAccessControlled {
    ...
    function passedPreMintingChecks(address _patron, address _collection) public view returns(bool) {
        ...
        if(collectionChainId[_collection] == block.chainid) {
            if(
                ...
                (collectionDataAdditional[_collection].maxBalance == 0 && IERC721(_collection).balanceOf(_patron) >= 1) ||
                (collectionDataAdditional[_collection].maxBalance >= 1 && IERC721(_collection).balanceOf(_patron) >= collectionDataAdditional[_collection].maxBalance) ||
    ...
    */
    ...
    function getAllCollections(CollectionType _collectionType, bool _onlyActive) public view 
