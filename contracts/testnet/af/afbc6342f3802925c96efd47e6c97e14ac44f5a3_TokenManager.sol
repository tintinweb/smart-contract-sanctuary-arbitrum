...
contract TokenManager is Access, ReentrancyGuard {
  ...
  /// @notice access role for minting vWINR Fees
  bytes32 public constant FEE_MINTER_ROLE = keccak256("FEE_MINTER_ROLE");
  ...
  function takeVestedWINR(
    address _from,
    uint256 _amount
  ) external nonReentrant onlyRole(WINR_STAKING_ROLE) {
   ...
   */
  function burnVestedWINR(uint256 _amount) external nonReentrant onlyRole(WINR_STAKING_ROLE) {
    vWINR.burn(_amount);
  }
   ...
   */
  function burnWINR(uint256 _amount) external nonReentrant onlyRole(WINR_STAKING_ROLE) {
    WINR.burn(_amount);
  }
   ...
   */
  function sendWLP(address _to, uint256 _amount) external nonReentrant onlyRole(WINR_STAKING_ROLE) {
   ...
   */
  function share(uint256 amount) external nonReentrant onlyRole(FEE_COLLECTOR_ROLE) {
   ...
   */
  function mintFee() external nonReentrant onlyRole(FEE_MINTER_ROLE) {
   ...
   */
  function increaseVolume(address _input, uint256 _amount) external nonReentrant {
    require(msg.sender == vaultManager, "Only Vault Manager");
   ...
   */
  function decreaseVolume(address _input, uint256 _amount) external nonReentrant {
    require(msg.sender == vaultManager, "Only Vault Manager");
...
interface IMiningStrategy {
  function calculate(uint256 _amount, uint256 _mintedByGames) external returns (uint256 amount_);
