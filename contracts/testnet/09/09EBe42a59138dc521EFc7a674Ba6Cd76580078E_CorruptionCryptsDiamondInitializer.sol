...
contract CorruptionCryptsDiamondInitializer is CorruptionCryptsDiamondState {
        ...
        returns (bool)
    {
        ...
        for (uint16 i = 0; i < _temples.length; i++) {
            uint256 distance = FixedPointMathLib.sqrt(
                uint256(
                    (int256(int8(_temples[i].coordinate.x)) -
                        int256(int8(_coordinate.x)))**2
                ) +
                    uint256(
                        (int256(int8(_temples[i].coordinate.y)) -
                            int256(int8(_coordinate.y)))**2
                    )
            );
    ...
    function generateTemplePositions() external view returns (Temple[] memory) {
        ...
        for (uint256 i = 0; i < _activeTemplesLength; i++) {
            ...
            while (!generated) {
                Coordinate memory thisCoordinate = ICorruptionCryptsInternal(
                    address(this)
                ).generateTempleCoordinate(i);
