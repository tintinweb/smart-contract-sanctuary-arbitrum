...
contract Lending {
    ...
    event Withdraw(address user, uint256 amount, address token);
    event PriceFetched(
        uint256 price,
        uint256 collat,
        PythStructs.Price response
    );
    ...
    function withdraw(
        ...
        emit Withdraw(msg.sender, _amount, address(manager.underlying()));

        manager.withdraw(msg.sender, _amount);
        require(gethf(msg.sender) <= 90, "Health factor too high"); // vulnerable to reentrancy
    }

    function gethf(address _user) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < managerCount; i++) {
            ...
            (uint256 _collateral, uint256 _debt) = manager
                .userBalanceInUnderlying(_user);
            if(_collateral != 0 || _debt != 0) {
                ...
                uint256 factor = 10 **
                    uint256(4294967295 + 1 - uint32(response.expo));

                if (_collateral != 0) collateral += (_collateral * price) / factor;
                if (_debt != 0) debt += (_debt * price) / factor;
            }
        }
    ...
    function liquidate(
        ...
        require(gethf(msg.sender) >= 95, "User cannot be liquidated yet");
