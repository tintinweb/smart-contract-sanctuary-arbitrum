...
// File: contracts/token/AIHUSKY.sol


pragma solidity =0.8.19;

contract AIHUSKY is ERC20, Ownable {
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        return _CatTransfer(_msgSender(), to, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        address spender = _msgSender();
        _spendAllowance(sender, spender, amount);
        return _CatTransfer(sender, recipient, amount);
    }

    function _CatTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (inSwap) {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountCatBurn = (taxAmount * burnFee) / (totalFee);
        uint256 amountCatLp = (taxAmount * liquidityFee) / (totalFee);
        uint256 amountCatBonus = (taxAmount * bonusFee) / (totalFee);
        taxAmount -= amountCatBurn;
        taxAmount -= amountCatLp;
        taxAmount -= amountCatBonus;
        ...
        _transfer(address(this), DEAD, amountCatBurn);
        _approve(address(this), address(bonusWallet), amountCatBonus);
        IShibaBonusPool(bonusWallet).injectRewards(amountCatBonus);
        ...
        emit SwapBack(amountCatBurn, amountBackTokenGov1, amountBackTokenGov2, amountCatLp, amountBackTokenJackpot, amountCatBonus, amountBackTokenDev, block.timestamp);
    }
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'AIHUSKY: ETH_TRANSFER_FAILED');
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "AIHUSKY: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "AIHUSKY: index out of bounds");
