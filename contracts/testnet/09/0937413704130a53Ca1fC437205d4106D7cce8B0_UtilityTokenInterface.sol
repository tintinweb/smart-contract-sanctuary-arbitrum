...
contract UtilityTokenInterface is AccessControl {

  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
  bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");

  IERC20UtilityToken public token;
  ...
  constructor(address _tokenAddress) {
    token = IERC20UtilityToken(_tokenAddress);
  ...
  function mintUniversal(
    ...
    require(tokenId == 1, "token id must be 1");
    require(maxSupply == 0 || token.totalSupply() + amount > maxSupply, "maxSupply reached");
    ...
    require(currentDayMinted + amount <= maxMintPerDay, "max mint per day reached");
  ...
  function setMaxMintPerDay(uint256 _maxMintPerDay) external onlyOwner {
