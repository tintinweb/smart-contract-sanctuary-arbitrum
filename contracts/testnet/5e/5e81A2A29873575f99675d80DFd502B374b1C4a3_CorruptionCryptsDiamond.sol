...
contract CorruptionCryptsDiamond is CorruptionCryptsDiamondState {
    ...
    function claimMapTiles(address _user) internal {
        ...
        uint256 numPendingMapTiles = ICorruptionCryptsInternal(address(this)).calculateNumPendingMapTiles(_user);
        ...
        addressToUserData[_user].roundIdToEpochLastClaimedMapTiles[
            currentRoundId
        ] = ICorruptionCryptsInternal(address(this)).currentEpoch() - epochsToReimburse;

        //Generate an array randomly of map tiles to add.
        MapTile[] memory mapTilesToAdd = ICorruptionCryptsInternal(address(this)).generateMapTiles(numToClaim, _user);
    ...
    function takeTurn(Move[] calldata _moves) public {
        ...
        for (uint256 moveIndex = 0; moveIndex < _moves.length; moveIndex++) {
            ...
            if (move.moveTypeId == MoveType.CreateLegionSquad) {
                ...
                (uint32[] memory _legionIds, uint8 _targetTemple) = abi.decode(
                    moveDataBytes,
                    (uint32[], uint8)
                );

                createLegionSquad(msg.sender, _legionIds, _targetTemple);
                continue;
            }
            ...
            if (move.moveTypeId == MoveType.BlowUpMapTile) {
                ...
                Coordinate memory _coordinate = abi.decode(moveDataBytes, (Coordinate));
