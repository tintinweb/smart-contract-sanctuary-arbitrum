...
import {IGEMB, IERC20} from "../../interfaces/tokens/IGEMB.sol";
...
contract Vesting is Access {
    ...
    constructor(IGEMB _Gemb, IGEMB _vGemb, address _admin) Access(_admin) {
        GEMB = _Gemb;
        vGEMB = _vGemb;

        categoryDetails[Category.GEMB_LABS] = CategoryDetail(
            180,
            1080,
            150 * 10 ** 24
        );
        categoryDetails[Category.MARKETING] = CategoryDetail(
            0,
            720,
            70 * 10 ** 24
        );
        categoryDetails[Category.ADVISORS] = CategoryDetail(
            0,
            1080,
            15 * 10 ** 24
        );
        categoryDetails[Category.PREV_HOLDERS] = CategoryDetail(
            0,
            720,
            10 * 10 ** 24
        );
        categoryDetails[Category.CORE_CONTRIBUTORS] = CategoryDetail(
            0,
            720,
            175 * 10 ** 24
        );
    ...
    function setInitialTimestamp(
        ...
        require(
            ...
            "Initial timestamp must be in the future"
        );
        isInitialized = true;
    ...
    function investorsLength()
        external
        view
        returns (uint256 _investorsLenght)
    {
        return investors.length;
    }

    function addInvestorBatch(
        address[] calldata _gembWallets,
        address[] calldata _vGembWallets,
        ...
        require(
            _categories.length == _gembWallets.length,
            "addInvestorBatch: array lengths should be the same"
        );
        require(
            _gembWallets.length == _vGembWallets.length,
            "addInvestorBatch: array lengths should be the same"
        );
    ...
    function addInvestor(
        address _gembWallet,
        address _vGembWallet,
        uint256 _tokensAllotment,
        Category _category
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _addInvestor(_gembWallet, _vGembWallet, _tokensAllotment, _category);
    }
    ...
    /// @param _gembWallet The addresses of new investors.
    /// @param _vGembWallet The addresses of new investors.
    ...
    /// @dev gembWallet or vGembWallet can be zero.
    function _addInvestor(
        ...
        require(
            _gembWallet != address(0) || _vGembWallet != address(0),
            "addInvestor: invalid address"
        );
        address _investor = _gembWallet == address(0)
            ? _vGembWallet
            : _gembWallet;
        require(
            !investorsInfo[_investor].exists,
            "addInvestor: investor exists"
        );
        require(
            _tokensAllotment > 0,
        ...
        require(
            totalAllotments[_category] + _tokensAllotment <=
                categoryDetails[_category].tokenSupply,
        ...
        investor.gembWallet = _gembWallet;
        investor.vGembWallet = _vGembWallet;
    ...
    ///@dev If gembWallet = zero, sends withdrawable amount to vGembWallet as Vested Gemb
    ///@dev If vGembWallet = zero, sends withdrawable amount to gembWallet as Gemb
    ...
    function withdrawTokens() external onlyInvestor initialized {
        ...
        if (
            investor.gembWallet != address(0) &&
            investor.vGembWallet != address(0)
        ) {
            GEMB.mint(investor.gembWallet, tokensAvailable / 2);
            vGEMB.mint(investor.vGembWallet, tokensAvailable / 2);
            ...
            if (investor.gembWallet != address(0)) {
                GEMB.mint(investor.gembWallet, tokensAvailable);
                emit Withdrawn(msg.sender, tokensAvailable, 0);
            } else {
                vGEMB.mint(investor.vGembWallet, tokensAvailable);
    ...
    function _calculateUnlockedTokens(
        ...
        CategoryDetail memory categoryDetail = categoryDetails[
            investor.category
        ];
        uint256 cliffTimestamp = initialTimestamp +
            categoryDetail.cliffDays *
            1 days;
        ...
        if (currentTimeStamp > initialTimestamp) {
            if (currentTimeStamp <= cliffTimestamp) {
                ...
                uint256 vestingUnlockedAmount = (occurence *
                    vestingDistroAmount) / categoryDetail.recurrence;
