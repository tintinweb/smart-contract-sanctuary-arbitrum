...
// File: contracts/NametagSocialFi.sol
...
contract NametagSocialFi is Ownable {
    ...
    // @dev Marker if the supply of shares subject has been sold at least once
    ...
    modifier checkSignature(bytes memory data, bytes memory signature) {
        require(signerRole == keccak256(data).toEthSignedMessageHash().recover(signature), "Invalid signature");
        _;
    }

    constructor(address signer) {
        signerRole = signer;
        emit SignerRoleChanged(signer);
    }
    ...
    function setSignerRole(address newSignerRole) external onlyOwner {
    ...
    // @dev create relation between sharesId and address
    // @param sharesId - ID of shares subject
    // @param finalTimestamp - signature expiration date
    // @param signature
    function addSharesSubject(uint256 sharesId, uint256 finalTimestamp, bytes calldata signature) external checkSignature(abi.encode(sharesId, msg.sender, finalTimestamp), signature) {
    require(block.timestamp < finalTimestamp, "Signer transaction expired");

        sharesSubjects[sharesId] = _msgSender();
        ...
        if (!isSupplyMinted[sharesId]) {
            ...
            if (sharesSupply[sharesId] == 0) {
                ...
                sharesBalance[sharesId][_msgSender()]++;
            }

            emit BuyShares(msg.sender, sharesId, 1, 0, 0, 0, sharesSupply[sharesId]);
        }

        emit NewSharesSubject(sharesId, _msgSender());
    }
    ...
    // @dev Sell supplies
    // @param sharesId - ID of shares subject from which the supply is bought
    // @pram amount - amount of supply
    function sellShares(uint256 sharesId, uint256 amount) public payable {
        uint256 supply = sharesSupply[sharesId];
    ...
    function withdrawFee(uint256 sharesId) public {
        require(_msgSender() == sharesSubjects[sharesId], "Caller is not shares subject");
        ...
        emit WithdrawFee(sharesId, _msgSender(), feeBalance);

        (bool success,) = _msgSender().call{value: feeBalance}("");
