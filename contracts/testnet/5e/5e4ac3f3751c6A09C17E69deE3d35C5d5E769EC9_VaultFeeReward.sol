...
/** @title VaultFeeReward
    @notice DomFiPerp vault LPs can claim part of platform fee via this contract
    adapted from https://github.com/Synthetixio/synthetix/edit/develop/contracts/StakingRewards.sol
 */
contract VaultFeeReward is IVaultReward, Governable, ReentrancyGuard, Pausable {
    using FixedPoint for FixedPoint.Unsigned;
    ...
    FixedPoint.Unsigned public cumulativeRewardPerTokenStored;

    mapping(address => FixedPoint.Unsigned) private claimableReward;
    mapping(address => FixedPoint.Unsigned) private previousRewardPerToken;

    event SetOwner(address owner);
    event SetDomFiPerp(IDomFiPerp domFiPerp);
    event ClaimedReward(address user, IERC20Metadata rewardToken, uint256 amount);
    event Reinvested(address user, IERC20Metadata rewardToken, uint256 amount);
    event Staked(address indexed user, uint256 amount);
    event Withdrawn(address indexed user, uint256 amount);

    constructor(IDomFiPerp _domFiPerp, IERC20Metadata _rewardToken) {
        owner = msg.sender;
    ...
    function updateReward(address account) public override {
        ...
        FixedPoint.Unsigned vaultReward = FixedPoint.fromUnscaledUint(
            domFiPerp.distributeVaultReward()
        );

        FixedPoint.Unsigned supply = FixedPoint.Unsigned.wrap(domFiPerp.totalSupply());

        if (supply.isGreaterThan(0)) {
            cumulativeRewardPerTokenStored = cumulativeRewardPerTokenStored.add(vaultReward.div(supply));
        }
        if (cumulativeRewardPerTokenStored.isEqual(0)) return;

        claimableReward[account] = claimableReward[account].add(
            FixedPoint.Unsigned.wrap(domFiPerp.balanceOf(account)).mul(
                cumulativeRewardPerTokenStored.sub(previousRewardPerToken[account])
            )
        );
    ...
    function claimReward() public returns (uint256 rewardToSend) {
        ...
        FixedPoint.Unsigned prevReward = claimableReward[msg.sender];
        rewardToSend = prevReward.trunc();
        claimableReward[msg.sender] = prevReward.sub(rewardToSend);

        if (rewardToSend > 0) {
            rewardToken.safeTransfer(msg.sender, rewardToSend);
    ...
    function reinvest() external returns (uint256 reinvestAmount) {
        ...
        FixedPoint.Unsigned prevReward = claimableReward[msg.sender];
        reinvestAmount = prevReward.trunc();
        claimableReward[msg.sender] = prevReward.sub(reinvestAmount);

        if (reinvestAmount > 0) {
            rewardToken.safeApprove(address(domFiPerp), reinvestAmount);
            domFiPerp.deposit(reinvestAmount, msg.sender);

            emit Reinvested(msg.sender, rewardToken, reinvestAmount);
        }
    }

    function getClaimableReward(address account) external view returns (uint256) {
        FixedPoint.Unsigned currentClaimableReward = claimableReward[account];
        FixedPoint.Unsigned supply = FixedPoint.Unsigned.wrap(domFiPerp.totalSupply());
        if (supply.isEqual(0)) return currentClaimableReward.trunc();

        FixedPoint.Unsigned _pendingReward = FixedPoint.fromUnscaledUint(
            domFiPerp.getPendingVaultReward()
        );
        FixedPoint.Unsigned _rewardPerTokenStored = cumulativeRewardPerTokenStored.add(
            _pendingReward.div(supply)
        );
        if (_rewardPerTokenStored.isEqual(0)) return currentClaimableReward.trunc();

        return
            currentClaimableReward
                .add(
                    FixedPoint.Unsigned.wrap(domFiPerp.balanceOf(account)).mul(
                        _rewardPerTokenStored.sub(previousRewardPerToken[account])
                    )
                )
                .trunc();
    }
