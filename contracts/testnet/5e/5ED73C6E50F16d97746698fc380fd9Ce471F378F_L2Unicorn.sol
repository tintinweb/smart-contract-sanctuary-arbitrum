...
contract L2Unicorn is IL2Unicorn, ERC721EnumerableUpgradeable, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    function pause() public onlyRole(ROLE_ADMIN) {
        _pause();
    }
    ...
    function setBaseURI(string calldata baseURI) external onlyRole(ROLE_ADMIN) {
        _baseURI_ = baseURI;
    }
    ...
    function mintForLevel(address to_, uint8 level_, uint256 levelStartTokenId_) external {
        require(hasRole(ROLE_ADMIN, _msgSender()) || hasRole(ROLE_MINTER, _msgSender()), "L2Unicorn: caller access denied");
    ...
    function viewTokenIds(address account_, uint8 startIndex_, uint8 endIndex_) external view returns (uint256[] memory tokenIdArr_){
        if (startIndex_ >= 0 && endIndex_ >= startIndex_) {
            ...
            uint8 arrayIndex_ = 0;
            for (uint8 index_ = startIndex_; index_ < ((endIndex_ > total) ? total : endIndex_);) {
