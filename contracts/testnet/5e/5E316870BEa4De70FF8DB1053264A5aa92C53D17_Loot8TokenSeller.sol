...
import "../../contracts/access/DAOAccessControlled.sol";
import "../../interfaces/finance/ILoot8TokenSeller.sol";
...
contract Loot8TokenSeller is ILoot8TokenSeller, Pausable, DAOAccessControlled {
    ...
    constructor(address _loot8Token, address _receiver, address _daoAuthority) {
        loot8Token = _loot8Token;
        receiver = _receiver;
        _setAuthority(_daoAuthority);
    }

    function setPrice(address _token, uint256 _price) external onlyGovernor {
        uint256 _oldPrice = price[_token];
        price[_token] = _price;
        emit PriceUpdated(_token, _oldPrice, _price);
    }

    function buy(address _token, uint256 _amount) external whenNotPaused {
        require(price[_token] > 0, "Token not configured for sale");
        uint256 decimals = IERC20Metadata(_token).decimals();
        IERC20 token = IERC20(_token);

        uint256 _price = price[_token];
        uint256 _totalTokens = _amount * 10 ** decimals / _price;
        _totalTokens = _totalTokens * 10 ** (18 - decimals);
        address buyer = _msgSender();

        require(token.allowance(buyer, address(this)) >= _amount, "ERC20: Insufficient Allowance");

        token.safeTransferFrom(buyer, receiver, _amount);
        ILoot8Token(loot8Token).mint(buyer, _totalTokens);

        emit Bought(_msgSender(), _token, _totalTokens);
    }
    ...
    view
    virtual
    override(Context, DAOAccessControlled)
    returns (address sender)
    {
...
interface ILoot8TokenSeller {

    event Bought(address _buyer, address _token, uint256 _amount);
    event PriceUpdated(address _token, uint256 _oldPrice, uint256 _newPrice);
