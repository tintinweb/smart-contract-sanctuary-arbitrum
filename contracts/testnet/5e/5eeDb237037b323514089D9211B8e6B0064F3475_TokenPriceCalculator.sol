...
contract TokenPriceCalculator is ITokenPriceCalculator, Initializable, DAOAccessControlled {
    ...
    function initialize(address _loot8Token, address _authority) public initializer{
    ...
    /**
     * @notice  Calculate amount of LOOT8 tokens eligible _per receiver_ based on amount paid
     * @param   _amountPaid Amount paid in USD, 6 decimals
     */
    function getTokensEligible(uint256 _amountPaid) external view returns (uint256) {
        return _amountPaid * (10**ILoot8Token(loot8Token).decimals()) / pricePerMint;
    }
