...
contract Exploration is Initializable, ExplorationSettings {
    ...
    // function ownerForUnstakedDonkey(uint256 _tokenId) external view override returns(address) {
    //     // address _owner = 
    //     return tld.ownerOf(_tokenId);
    // }

    //TODO: Update to get curr running time added to this 
    function totalStakedTimeForToken(uint256 _tokenId) external view override returns (uint256) {
        uint256 lastStakedTime = tokenToStakeInfo[_tokenId].lastStakedTime;
        require(lastStakedTime > 0, "Exploration: Donkey is not staked");
        return (tokenIdToRunningStakeTime[_tokenId]) + (block.timestamp - lastStakedTime);
    }
    ...
    // function ownerForStakedDonkey(uint256 _tokenId) public view returns(address) {
    //     uint256 lastStakedTime = tokenToStakeInfo[_tokenId].lastStakedTime;
    //     require(lastStakedTime > 0, "Exploration: Donkey is not staked");
    //     return tokenToStakeInfo[_tokenId].owner;
    // }
...
interface IExploration {
	...
	function startExploring(uint256 _tokenId, address owner) external;

	function stopExploring(uint256 _tokenId, address owner) external;
	...
	// function ownerForUnstakedDonkey(uint256 _tokenId) external view returns(address);

	function locationForStakedDonkey(uint256 _tokenId) external view returns(Location);

	function balanceOf(address _owner) external view returns (uint256);

	function totalStakedTimeForToken(uint256 _tokenId) external view returns (uint256);
}
