...
contract DexSwapFactory is IDexSwapFactory, Ownable {
    ...
    function fee(bool _stable) external view returns (uint256) {
        return _stable ? _fee.stableFee : _fee.volatileFee;
    }
    ...
    function updateFee(Fee memory fee_) external onlyOwner returns (bool) {
        require(fee_.stableFee <= MAX_FEE && fee_.volatileFee <= MAX_FEE, "DexSwapFactory: Fee overflow");
        _fee = fee_;
        emit FeeUpdated(fee_);
        return true;
    }

    function updateStablePair(address token0, address token1, bool stable) external onlyOwner returns (bool) {
        IDexSwapPair(getPair[token0][token1]).updateStable(stable);
        emit StablePairUpdated(token0, token1, stable);
        return true;
    }
...
contract DexSwapPair is IDexSwapPair, DexSwapERC20, Constants {
    ...
    function getAmountOut(uint256 amountIn, address tokenIn, address caller) public view returns (uint256 amountOut) {
        IDexSwapFactory _factory = IDexSwapFactory(factory);
        uint256 fee = _factory.feeWhitelistContains(caller) ? 0 : _factory.fee(stable);
        amountIn -= (amountIn * fee) / DIVIDER;
        amountOut = _getAmountOut(amountIn, tokenIn);
    }
    ...
    function getAmountIn(uint256 amountOut, address tokenIn, address caller) public view returns (uint256 amountIn) {
        ...
        IDexSwapFactory _factory = IDexSwapFactory(factory);
        require(!stable, "DexSwapPair: Pair is stable");
        uint256 fee = _factory.feeWhitelistContains(caller) ? 0 : _factory.fee(stable);
        uint256 numerator = _reserve0 * amountOut * DIVIDER;
        uint256 denominator = (_reserve1 - amountOut) * (DIVIDER - fee);
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "DexSwapPair: FORBIDDEN");
        token0 = _token0;
        token1 = _token1;
    ...
    function updateStable(bool stable_) external returns (bool) {
        require(msg.sender == factory, "DexSwapFactory: Caller is not factory");
        require(totalSupply == 0, "DexSwapFactory: Liquidity gt 0");
        stable = stable_;
        emit StableUpdated(stable_);
        return true;
    }
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, address caller, bytes calldata data) external {
        require(
    ...
    function _swap(
        ...
        {
            IDexSwapFactory _factory = IDexSwapFactory(factory);
            uint256 feeValue = _factory.fee(stable);
            if (caller != address(0) && _factory.feeWhitelistContains(caller)) feeValue = 0;
            uint256 balance0Adjusted = (balance0 * DIVIDER) - (amount0In * feeValue);
            uint256 balance1Adjusted = (balance1 * DIVIDER) - (amount1In * feeValue);
            require(
                _k(balance0Adjusted, balance1Adjusted) >= _k(uint256(_reserve0), uint256(_reserve1)),
                "DexSwapPair: K"
            );
        }
        _update(balance0, balance1, _reserve0, _reserve1);
    ...
    function _getAmountOut(uint256 amountIn, address tokenIn) internal view returns (uint256) {
        (uint256 _reserve0, uint256 _reserve1, ) = getReserves();
        require(amountIn > 0, "DexSwapPair: INSUFFICIENT_INPUT_AMOUNT");
        require(_reserve0 > 0 && _reserve1 > 0, "DexSwapPair: INSUFFICIENT_LIQUIDITY");
        if (tokenIn == token1) (_reserve0, _reserve1) = (_reserve1, _reserve0);
        if (stable) {
            ...
            uint256 y = _reserve1 - _get_y(amountIn + _reserve0, xy, _reserve1);
            return (y * (tokenIn == token0 ? decimals1 : decimals0)) / 1e18;
        } else {
            return (amountIn * _reserve1) / (_reserve0 + amountIn);
        }
    }
...
interface IDexSwapFactory {
    ...
    event StablePairUpdated(address indexed token0, address indexed token1, bool stable);
    event PairCreated(address indexed token0, address indexed token1, address pair, uint256);
    event PeripheryWhitelistAdded(address[] periphery);
    event PeripheryWhitelistRemoved(address[] periphery);

    function fee() external view returns (Fee memory fee);
    ...
    function updateStablePair(address token0, address token1, bool stable) external returns (bool);

    function updateFee(Fee memory fee_) external returns (bool);
...
interface IDexSwapPair is IDexSwapERC20 {
    ...
    event StableUpdated(bool stable);
    event Swap(
