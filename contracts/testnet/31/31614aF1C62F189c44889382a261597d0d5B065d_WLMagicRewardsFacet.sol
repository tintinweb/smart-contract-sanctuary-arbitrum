...
// Library imports
import { LibMagicRewardsUtils } from "../libraries/LibMagicRewardsUtils.sol";
...
contract WLMagicRewardsFacet is WithModifiers, ERC721BaseInternal, ERC721EnumerableInternal, ReentrancyGuard {
    ...
    function topupMagicRewards(uint256 amount) external {
        ...
        ws().magicRewardsPerLand += amount / wastelandConstants().MAX_CAP_LAND;
    ...
    function claimMagicReward(uint256 landId) external notPaused nonReentrant {
        ...
        uint256 claimable = LibMagicRewardsUtils.claimableMagicReward(landId);
        if (claimable > 0) {
            ws().rewardsWithdrawnPerLand[landId] += claimable;
            IERC20(ws().magic).safeTransfer(msg.sender, claimable);
            emit MagicRewardsClaimed(msg.sender, landId, claimable);
        }
    }
    ...
    function totalClaimable(address account) external view returns(uint256 claimable) {
...
// Storage imports
import { LibStorage, WastelandStorage, WastelandConstants } from "./LibStorage.sol";
...
library LibMagicRewardsUtils {
    ...
    function wastelandConstants() internal pure returns (WastelandConstants storage) {
        return LibStorage.wastelandConstants();
    }
...
struct WastelandConstants {
    uint256 MAX_CAP_LAND;
}
...
library LibStorage {
    ...
    // Constants are structs where the data gets configured on Wasteland initialization
    bytes32 constant WASTELAND_CONSTANTS_POSITION = keccak256("battlefly.constants.wasteland");
    ...
    function wastelandConstants() internal pure returns (WastelandConstants storage gc) {
        bytes32 position = WASTELAND_CONSTANTS_POSITION;
        assembly {
            gc.slot := position
        }
    }
