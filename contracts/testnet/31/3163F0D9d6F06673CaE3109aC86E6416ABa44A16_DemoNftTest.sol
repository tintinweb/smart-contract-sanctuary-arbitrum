...
contract DemoNftTest is ERC721, Ownable, ReentrancyGuard {
  using Strings for uint;
  using Counters for Counters.Counter;

  struct Sale {
      uint state; //state = 0, Not yet put on sale ,// state =1, whitelisted is available, // state = 2, public sale is availabe
  ...
  uint public maxPublicSupply = 7777; // mintable supply 
  uint public maxSupply = 8888;
  uint public _reserveTokenId = maxPublicSupply;
  Sale public sale; //whiteListSale = 0.015 ether & publicSale = 0.025 ether
  ...
  function whitelistWallet(address[] calldata _users) public onlyOwner {
      for (uint i = 0; i < _users.length; i++) {
          isWhitelisted[_users[i]] = true;
      }
  }
  
  function unWhitelistWallet(address[] calldata _users) public onlyOwner {
     for(uint i = 0; i < _users.length; i++) {
