...
/**
 * @title HelixConfig
 * @notice This contract represents information of loan, which are related to investors; 
 * includes the time to invest, total invested amount or status of loan (cancel or not). 
 * A UniTranchedPool belongs to a loan. Investors will interact with it to deposit, withdraw or redeem.
 * @author Helix
 */
...
contract HelixUniTranchedPool is BaseUpgradeablePausable, IHelixUniTranchedPool, IRequiresUID, IVersioned {
  ...
  /**
   * @notice Initialize a new MultiTranchedPool 
   * @param _loanFacade An array that use for project description (Project name - symbol)
   * @param _addresses config: The HelixConfig contract's address
   * borrower: The borrower proxy contract's address
   * _interestModel: The interest model library contract's address
   * @param _uints _interestApr The interest amount, on an annualized basis (APR, so non-compounding), expressed as an integer.
   ...
   * _mintTicketSize: The minimum amount that investor need to deposit in the loan
  ...
  /// @param ids An array that use for defining the list of allowed investor type 
  ...
  /// @notice An array that use for defining the list of allowed investor type 
  /// @return allowedUIDTypes An array that use for defining the list of allowed investor type 
  ...
  /**
   * @notice Deposit a USDC amount into the pool for a tranche. Mints an NFT to the caller representing the position without executing approve transaction
   * @param tranche The number representing the tranche to deposit into
   * @param amount The USDC amount to tranfer from the caller to the pool
  ...
  /// @notice Cancel a loan before funding date
  ...
  /// @notice Set open funding time which allows junior/senior investor to invest.
...
//   }
  ...
  /// @notice Set value of document's hash
  /// @param fileHash Hash of documents, that is related to a loan
  function setFileHash(bytes32 fileHash) external override onlyLocker{
    return creditLine.setFileHash(fileHash);
  }

  /// @notice Set interest model to calculate interest amount for the loan
  /// @param _interestModel Address of interest model contract
  function setInterestModel(address _interestModel) external onlyAdmin {
    return creditLine.setInterestModel(_interestModel);
  }

  /// @notice Set limit value of loan amount
  /// @param newAmount New limit loan amount
  function setLimit(uint256 newAmount) external onlyAdmin {
    return creditLine.setLimit(newAmount);
  }

  /// @notice Set max limit of loan amount
  /// @param newAmount New max limit of loan amount
  function setMaxLimit(uint256 newAmount) external onlyAdmin {
    return creditLine.setMaxLimit(newAmount);
  }

  /// @notice Get info of tranche in pool
  /// @param tranche Index of tranche in pool
  ...
  /// @notice Check if a pool is matching condition so borrower can drawdown from the pool
  ...
  /// @notice Check if amount in pool is enough and ratio of 2 tranches in pool is valid
