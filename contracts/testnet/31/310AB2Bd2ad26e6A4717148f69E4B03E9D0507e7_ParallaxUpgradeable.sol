...
interface IParallax is IFees {
    ...
    ///@notice view method for getting current feesReceiver
    function feesReceiver() external view returns (address);

    /** @notice view method for getting current withdrawal fee by strategy
     *   @param strategy address of strategy
     *   @return withdrawal fee
     **/
    function getWithdrawalFee(address strategy) external view returns (uint256);

    /** @notice view method for check if the token is in the whitelist.
     *   @param strategy address of strategy
    ...
    Timelock
{
     ...
     * @param initialFeesReceiver - commissions recipient.
     * @param initialERC721 address of erc721 contract for positions.
     */
     ...
     *      Can only be called by the current owner.
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
     * @param token - address of a new token to add.
     */
    function addToken(
        uint256 strategyId,
        address token
    ...
    /**
     * @dev removes a token from a whitelist of tokens that can be accepted
     *      as the tokens for deposits and withdraws.
     *      Can only be called by the current owner.
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
    ...
    /**
     * @dev registers a new earning strategy on this contract.
     *      An earning strategy must be deployed
     *      before the calling of this method.
     *      Can only be called by the current owner.
     * @param strategy - an address of a new earning strategy
     *                   that should be added.
     * @param timelock - number of seconds during which users can't withdraw
     *                   their deposits after last deposit.
     *                   Applies only for earning strategy that is adding.
     *                   Can be updated later.
     * @param cap - a cap for the amount of deposited LP tokens.
     * @param rewardPerBlock - a reward amount that will be distributed
     *                         between all users in a strategy every block.
     *                         Can be updated later.
     * @param initialFees - fees that will be applied for earning strategy
     *                      that is adding.
     *                      Currently only withdrawal fee is supported.
     *                      Applies only for earning strategy that is adding.
     *                      Can be updated later.
     *                      Each fee should contain 2 decimals:
     *                      5 = 0.05%, 10 = 0.1%, 100 = 1%, 1000 = 10%.
     *  @param rewardToken - a reward token in which rewards will be paid.
     *                       Can be updated later.
     */
    ...
    /**
     * @dev sets a new receiver for fees from all earning strategies.
     *      Can only be called by the current owner.
     * @param newFeesReceiver - a wallet that will receive fees from all
     *                          earning strategies.
     */
    ...
    /**
     * @dev  sets a new fees for an earning strategy.
     *       Can only be called by the current owner.
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
     * @param newFees - fees that will be applied for earning strategy.
     *                  Currently only withdrawal fee is supported.
     *                  Each fee should contain 2 decimals:
     *                  5 = 0.05%, 10 = 0.1%, 100 = 1%, 1000 = 10%.
     */
    ...
    /**
     * @dev sets a timelock for withdrawals (in seconds).
     *      Timelock - period during which user is not able to make a withdrawal
     *      after last successful deposit.
     *      Can only be called by the current owner.
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
     ...
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
     * @param newRewardToken - new reward token in which rewards will be paid.
     */
    ...
    /**
     * @dev sets a new cap for the amount of deposited LP tokens.
     *      A new cap must be more or equal to the amount of staked LP tokens.
     *      Can only be called by the current owner.
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
     * @param cap - a new cap for the amount of deposited LP tokens which
     *              will be applied for earning strategy.
     */
    function setCap(
        uint256 strategyId,
        uint256 cap
    ...
    /**
     * @dev sets a value for an earning strategy (in reward token)
     *      after which compound must be executed.
     *      The compound operation is performed
     *      during every deposit and withdrawal.
     *      And sometimes there may not be enough reward tokens
     *      to complete all the exchanges and liquidity additions.
     *      As a result, deposit and withdrawal transactions may fail.
     *      To avoid such a problem, this value is provided.
     *      And if the number of rewards is even less than it,
     *      compound does not occur.
     *      As soon as there are more of them,
     *      a compound immediately occurs in time of first deposit or withdrawal
     *      Can only be called by the current owner.
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
     * @param compoundMinAmount - a value in reward token
     *                            after which compound must be executed.
     */
    ...
    /**
     * @notice setups a reward amount that will be distributed
     *         between all users in a strategy every block.
     *         Can only be called by the current owner.
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
    ...
    /**
     * @notice setups a strategy status.
     *         Sets permission or prohibition for depositing funds on the strategy.
     *         Can only be called by the current owner.
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
     * @param flag - strategy status. False - not active, true - active.
     */
    ...
    /**
     * @notice accepts deposits from users.
     *         This method accepts ERC-20 LP tokens
     *         that will be used in earning strategy.
     *         Appropriate amount of ERC-20 LP tokens must be approved
     *         for earning strategy in which it will be deposited.
     *         Can be called by anyone.
     * @param depositParams parameters for deposit
     *                      (for more details see a specific earning strategy).
     * @return PositionId and true if the position has just been created
     */
    function deposit(
    ...
    /**
     * @notice  accepts deposits from users.
     *          This method accepts a group of different ERC-20 tokens
     *          in equal part that will be used in earning strategy
     *          (for more detail
     *          s see the specific earning strategy documentation).
     *          Appropriate amount of all ERC-20 tokens must be approved
     *          for earning strategy in which it will be deposited.
     *          Can be called by anyone.
     * @param depositParams parameters for deposit
     *                      (for more details see a specific earning strategy).
    ...
    /**
     * @notice  accepts deposits from users.
     *          This method accepts ETH tokens
     *          that will be used in earning strategy.
     *          ETH tokens must be attached to the transaction.
     *          Can be called by anyone.
     * @param depositParams parameters for deposit
     *                      (for more details see a specific earning strategy).
     * @return PositionId and true if the position has just been created
    ...
    /**
     * @notice accepts deposits from users.
     *         This method accepts any whitelisted ERC-20 tokens
     *         that will be used in earning strategy.
     *         Appropriate amount of ERC-20 tokens must be approved
     *         for earning strategy in which it will be deposited.
     *         Can be called by anyone.
     * @param depositParams parameters for deposit
     *                      (for more details see a specific earning strategy).
    ...
    /**
     * @notice a withdraws users' deposits + reinvested yield.
     *         This method allows to withdraw ERC-20 LP tokens
     *         that were used in earning strategy
     *         Can be called by anyone.
     * @param withdrawParams for withdraw
     *                       (for more details see a specific earning strategy).
    ...
    /**
     * @notice a withdraws users' deposits without reinvested yield.
     *         This method allows to withdraw ERC-20 LP tokens
     *         that were used in earning strategy
     *         Can be called by anyone.
     * @param withdrawParams for withdraw
    ...
    /**
     * @notice withdraws users' deposits + reinvested yield.
     *         This method allows to withdraw a group of ERC-20 tokens
     *         in equal parts that were used in earning strategy
     *         (for more details see
     *         the specific earning strategy documentation).
     *         Can be called by anyone.
     * @param withdrawParams for withdraw
     *                       (for more details see a specific earning strategy).
     * @return true if the position was closed
    ...
    /**
     * @notice  withdraws users' deposits + reinvested yield.
     *          This method allows to withdraw ETH tokens
     *          that were used in earning strategy.
     *          Can be called by anyone.
     * @param withdrawParams for withdraw
     *                       (for more details see a specific earning strategy).
    ...
    /**
     * @notice  withdraws users' deposits + reinvested yield.
     *          This method allows to withdraw any whitelisted ERC-20 tokens
     *          that were used in earning strategy.
     *          Can be called by anyone.
     * @param withdrawParams for withdraw
     *                       (for more details see a specific earning strategy).
    ...
    /**
     * @notice claims all rewards from earning strategy
     *         and reinvests them to increase future rewards.
     *         Can be called by anyone.
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
     * @param amountsOutMin - an array of minimum values
     ...
     *                        unique for each earning strategy.
     *                        See the specific earning strategy documentation
     *                        for more details.
     */
    function compound(
        uint256 strategyId,
    ...
    /**
     * @notice  claims tokens that were distributed on users
     *          deposit and earned by a specific position of a user.
     *          Can be called by anyone.
     *
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
     *
     * @param positionId - an ID of a position.
     *                     Must be an existing position ID.
     */
    function claim(
        uint256 strategyId,
        uint256 positionId
    ...
    /**
     * @notice add a new transaction to the execution queue.
     *         Can only be called by the current owner.
     * @param transaction struct of:
     *                    dest - the address on which the method will be called.
     *                    value - the value of Wei to send.
     *                    signature - method signature.
     *                    data - method call payload
     *                    exTime - the time from which
     *                    the transaction can be executed.
     *                    exTime must be less than
     *                    the current block.timestamp + DELAY
    ...
    /**
     * @notice remove a transaction from the execution queue.
     *         Can only be called by the current owner.
     * @param transaction struct of:
     *                    dest - the address on which the method will be called.
     *                    value - the value of Wei to send.
     ...
     *                    data - method call payload
     *                    exTime - the time
     *                    from which the transaction can be executed.
     *                    exTime must be less
     *                    than the current block.timestamp + DELAY
     */
    ...
    /**
     * @notice executes a transaction from the queue.
     *         Can only be called by the current owner.
     * @param transaction struct of:
     *                    dest - the address on which the method will be called.
     *                    value - the value of Wei to send.
     ...
     *         Can be called by anyone.
     * @param strategy - address of an earning strategy
     *                   to retrieve a withdrawal fee.
     *
     * @return withdrawal fee
     */
    ...
    /**
     * @notice Returns an amount of tokens that are staked under a shares
     *         amount.
     * @dev Staked == deposited + earned.
     * @param strategyId An ID of an earning strategy. Must be an existing
     *                   earning strategy ID.
     * @param shares An amount of shares for which to calculate the staked
     *               amount of tokens.
    ...
    /**
     * @notice Returns an amount of earned by shares tokens in a specified
     *         earning strategy. Can be called by anyone.
     * @param strategyId An ID of an earning strategy. Must be an existing
     *                   earning strategy ID.
     * @param shares An amount of shares for which to calculate the earned
     *               amount of tokens.
    ...
    /**
     * @notice Returns an amount of earned by user tokens in a specified earning
    ...
    /**
     * @notice returns claimable by the user amount in the position.
     *         Can be called by anyone.
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
     * @param user - a user to check  earned tokens amount.
     ...
     * @return address of a user by him unique ID
     */
    ...
    /**
     * @notice  returns a unique ID for a user in an earning strategy.
     *          Can be called by anyone.
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
    ...
    /**
     * @dev withdraws an ETH token that accidentally ended up
     *      on an earning strategy contract and cannot be used in any way.
     *      Can only be called by the current owner.
     * @param strategyId - an ID of an earning strategy.
     *                     Must be an existing earning strategy ID.
     * @param amount - a number of tokens to withdraw from this contract.
    ...
    /**
     * @notice safely transfers ERC-721 token (user position),
     *         checking first that contract recipients are aware of the
     *         ERC-721 protocol to prevent tokens from being forever locked.
     *         Can be called by anyone.
     * @param from - a wallet from which token (user position)
     *               will be transferred.
     * @param to - a wallet to which token (user position) will be transferred.
     * @param tokenId - an ID of token to transfer.
     * @param data - additional encoded data that can be used
     *               somehow in time of tokens (users' positions) transfer.
     */
    function safeTransferFrom(
        address from,
        address to,
    ...
    function _addNewUserIfNeeded(uint256 strategyId, address owner) private {
        if (positionsCount[strategyId][owner] == 1) {
            Strategy storage strategy = strategies[strategyId];

            ++strategy.usersCount;
            ++userAmountStrategies[owner];

            strategy.users[strategy.usersCount] = owner;
            strategy.usersToId[owner] = strategy.usersCount;

            if (userAmountStrategies[owner] == 1) {
                ++usersCount;

                users[usersCount] = owner;
                usersToId[owner] = usersCount;
            }
        }
    }

    function _deleteUserIfNeeded(uint256 strategyId, address owner) private {
        if (positionsCount[strategyId][owner] == 0) {
            Strategy storage strategy = strategies[strategyId];
            uint256 userId = strategy.usersToId[owner];
            ...
            delete strategy.usersToId[owner];

            --strategies[strategyId].usersCount;
            --userAmountStrategies[owner];

            if (userAmountStrategies[owner] == 0) {
                uint256 globalUserId = usersToId[owner];
    ...
    function _onlyValidWithdrawalSharesAmount(
        ...
        if (positions[strategyId][_msgSender()][positionId].shares < shares) {
