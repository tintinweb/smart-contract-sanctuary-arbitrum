...
interface IReferralManager {
  function calculateTaxOfficeReferralWeights(
    ...
    external
    returns (address _upline, uint256 _referrerWeight, uint256 _referreeWeight);
}
...
interface IERC20Taxable is IERC20 {
  ...
  function isWhitelistedRecipient(
    address _account
  ) external view returns (bool isWhitelisted);
...
contract TaxOffice is MultipleOperator {
  ...
  event HandledMainTokenTax(
    address indexed _origin,
    address indexed _account,
    uint256 _amount,
    uint256 _timestamp
  );
  event HandledShareTokenTax(
    address indexed _origin,
    address indexed _account,
    uint256 _amount,
    uint256 _timestamp
  ...
  function searchSorted(
    uint256[] memory _monotonicArray,
    uint256 _value
  ...
  function calculateMainTokenTax()
    external
    view
    virtual
  ...
  function calculateShareTokenTax()
    external
    view
    virtual
    returns (uint256 taxRate)
  ...
  function setMainTokenEnableDynamicTax(
    bool _enableDynamicTax
  ) external onlyOperator {
    mainToken.setEnableDynamicTax(_enableDynamicTax);
  }

  function setMainTokenWhitelistType(
    address _account,
    uint8 _type
  ) external onlyOperator {
  ...
  function setMainTokenWhitelistType(
    ...
    require(
      _accounts.length == _types.length,
      "Error: Account and Types lengths not equal"
    );
    for (uint8 idx = 0; idx < _accounts.length; ++idx) {
  ...
  function setShareTokenEnableDynamicTax(
    bool _enableDynamicTax
  ) external onlyOperator {
...
// File: TaxOfficeV5.sol

contract TaxOfficeV5 is TaxOffice {
  using SafeERC20 for IERC20;

  //*================ CUSTOM USER TYPES ================*//

  struct UserInfo {
    uint256 taxMultiplier; //Tax multiplier.
    bool taxMultiplierIncrease; //Determines if the individual tax amount should increase / decrease.
    uint256 mainTokenReferralAmount; //Tax Referrals earnt in LEVEL.
    uint256 shareTokenReferralAmount; //Tax Referrals earnt in LODGE.
    uint256 daiReferralAmount; //Tax Referrals earnt in dai.
  }
  ...
  address[] public mainTokenTaxPath;
  IERC20 public mainTokenTaxPathEndpoint;
  bool public mainTokenReferralFollowsSwap;

  address[] public shareTokenTaxPath;
  IERC20 public shareTokenTaxPathEndpoint;
  bool public shareTokenReferralFollowsSwap;

  mapping(address => UserInfo) public userInfo;

  bool public enableStaticTaxRates;
  uint256 public staticMainTokenTaxRate;
  uint256 public staticShareTokenTaxRate;

  bool public taxChoice = true;
  ...
  constructor(
    ...
    router = _router;
    mainTokenTaxPathEndpoint = IERC20(_mainToken);
    shareTokenTaxPathEndpoint = IERC20(_shareToken);
  }
  ...
  function calculateMainTokenTax()
    ...
    returns (uint256 taxRate)
  {
    if (enableStaticTaxRates == true) {
      taxRate = staticMainTokenTaxRate;
    } else {
      uint256 mainTokenPrice = _getMainTokenPrice();
      uint8 taxTierIdx = searchSorted(mainTokenTaxTwapTiers, mainTokenPrice);
      taxRate = mainTokenTaxRateTiers[taxTierIdx];
    ...
    if (userInfo[tx.origin].taxMultiplierIncrease == true) {
      taxRate = taxRate
        .mul(BASIS_POINTS_DENOM.add(userInfo[tx.origin].taxMultiplier))
        .div(BASIS_POINTS_DENOM);
    } else {
      taxRate = taxRate
        .mul(BASIS_POINTS_DENOM.sub(userInfo[tx.origin].taxMultiplier))
        .div(BASIS_POINTS_DENOM);
    }
  }
  ...
  //*================ TAX HANDLING FUNCTIONS ================*//

  function handleMainTokenTax(uint256 _amount) external override onlyOperator {
    ...
    if (taxChoice) {
      ...
      IERC20(mainToken).approve(address(router), _amount);
      ...
      IERC20(mainToken).approve(address(router), _amount / 2);
      uint256 tokenAmount = IERC20(mainToken).balanceOf(address(this));
      uint256 newBalance = address(this).balance.sub(initialBalance);
      // add the liquidity
      IUniswapV2Router(router).addLiquidityETH{ value: newBalance }(
        address(mainToken),
        tokenAmount,
  ...
  function handleShareTokenTax(uint256 _amount) external override onlyOperator {
    ...
    if (taxChoice) {
      ...
      if (_getMainTokenPrice() > 2.20e18) {
        address[] memory path = new address[](2);
        path[0] = address(shareToken);
        path[1] = IUniswapV2Router(router).WETH();

        IERC20(shareToken).approve(address(router), _amount);
        uint256 initialBalance = address(this).balance;
        // make the swap
        IUniswapV2Router(router)
          .swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        uint256 tokenAmount = IERC20(shareToken).balanceOf(address(this));
        uint256 newBalance = address(this).balance.sub(initialBalance);
        // add the liquidity
        IUniswapV2Router(router).addLiquidityETH{ value: newBalance / 2 }(
          address(shareToken),
          tokenAmount / 2,
          0, // slippage is unavoidable
          0, // slippage is unavoidable
        ...
        path1[0] = address(shareToken);
        path1[1] = IUniswapV2Router(router).WETH();
        path1[2] = address(mainToken);

        IERC20(shareToken).approve(address(router), _amount);
        uint256 initialBalance1 = address(this).balance;
        // make the swap
        IUniswapV2Router(router)
          .swapExactTokensForETHSupportingFeeOnTransferTokens(
            _amount / 4,
        ...
        uint256 tokenAmount1 = IERC20(mainToken).balanceOf(address(this));
        IERC20(mainToken).approve(address(router), tokenAmount1);
        uint256 newBalance1 = address(this).balance.sub(initialBalance1);
        // add the liquidity
        IUniswapV2Router(router).addLiquidityETH{ value: newBalance1 }(
          address(mainToken),
          tokenAmount1,
          0, // slippage is unavoidable
          0, // slippage is unavoidable
        ...
        shareToken.transfer(treasury, _amount);
      }
    }
    //Emit event.
    emit HandledShareTokenTax(tx.origin, msg.sender, _amount, block.timestamp);
  ...
  function setTaxPaths(
    address[] memory _mainTokenTaxPath,
    address[] memory _shareTokenTaxPath
  ) public onlyOperator {
    mainTokenTaxPath = _mainTokenTaxPath;
    mainTokenTaxPathEndpoint = _mainTokenTaxPath.length > 1
      ? IERC20(_mainTokenTaxPath[_mainTokenTaxPath.length - 1])
      : mainToken;
    shareTokenTaxPath = _shareTokenTaxPath;
    shareTokenTaxPathEndpoint = _shareTokenTaxPath.length > 1
      ? IERC20(_shareTokenTaxPath[_shareTokenTaxPath.length - 1])
      : mainToken;
  }

  function setReferralsFollowsSwaps(
    bool _mainTokenReferralFollowsSwap,
    bool _shareTokenReferralFollowsSwap
  ) external onlyOperator {
    mainTokenReferralFollowsSwap = _mainTokenReferralFollowsSwap;
    shareTokenReferralFollowsSwap = _shareTokenReferralFollowsSwap;
  }

  function setTokenTaxRates(
    uint256 _staticMainTokenTaxRate,
    uint256 _staticShareTokenTaxRate
  ) external onlyOperator {
    staticMainTokenTaxRate = _staticMainTokenTaxRate;
    staticShareTokenTaxRate = _staticShareTokenTaxRate;
  }

  function setUserInfo(
    address _user,
    uint256 _taxMultiplier,
    bool _taxMultiplierIncrease,
    uint256 _mainTokenReferralAmount,
    uint256 _shareTokenReferralAmount,
    uint256 _daiReferralAmount
  ) external onlyOperator {
    userInfo[_user] = UserInfo({
      taxMultiplier: _taxMultiplier,
      taxMultiplierIncrease: _taxMultiplierIncrease,
      mainTokenReferralAmount: _mainTokenReferralAmount,
      shareTokenReferralAmount: _shareTokenReferralAmount,
      daiReferralAmount: _daiReferralAmount
    });
  }
