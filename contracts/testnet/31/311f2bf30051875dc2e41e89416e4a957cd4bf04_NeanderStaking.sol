...
contract NeanderStaking is NeanderStakingState, ContractControl { 
    ...
    function getUserInfo(address user) external view returns(userInfo[] memory){
        ...
        for(uint i=0; i<tokens.length; i++) {
            ...
            info[i] = userInfo(tokens[i], tokenIdToStakeStartTime[tokens[i]], endTime, getRewardAmount(tokens[i]));
        }
        return info;
    }
...
abstract contract NeanderStakingState is ERC721Holder {
    ...
    mapping(address => EnumerableSetUpgradeable.UintSet) internal userToTokensStaked;
