...
contract LiquidityProvider is ILiquidityProvider, BlockContext, ClearingHouseCallee, LiquidityProviderStorageV1 {
    ...
    function addLiquidity(AddLiquidityParams calldata params) external override returns (AddLiquidityResponse memory) {
        ...
        return AddLiquidityResponse({ base: baseAmount, quote: quoteAmount, fee: fee, liquidity: liquidity });
    }
        ...
        returns (RemoveLiquidityResponse memory)
    {
        ...
        // LP_LOF: remove too much liquidity
        require(params.liquidity <= openOrder.liquidity, "LP_RTML");
        ...
        )
    {
        ...
        if (openOrder.liquidity == 0) {
            ...
            openOrder.lastExchangeFeeIndex = _feeGrowthPerLiquidityUnitIndexMap[params.baseToken];
        }
    ...
    ///        2. quote/base liquidity does NOT include amm fees since
    ...
    function _getPendingFee(address baseToken, Account.LPInfo memory order) internal view returns (uint256) {
        if (order.liquidity == 0 || _feeGrowthPerLiquidityUnitIndexMap[baseToken] == 0) {
            return 0;
        }

        uint256 lpFeeRatioAccumulate;
        if (order.lastExchangeFeeIndex < _feeGrowthPerLiquidityUnitIndexMap[baseToken] - 1) {
            uint256 latestValue = _feeGrowthPerLiquidityUnitMap[baseToken][
                _feeGrowthPerLiquidityUnitIndexMap[baseToken] - 1
            ];
            uint256 startValue = _feeGrowthPerLiquidityUnitMap[baseToken][order.lastExchangeFeeIndex];
            lpFeeRatioAccumulate = latestValue - startValue;
        }
    ...
    function updateLPCalcFeeRatio(address baseToken, uint256 lpCalcFeeRatio) external override {
        ...
        if (_feeGrowthPerLiquidityUnitIndexMap[baseToken] > 0) {
            previousValue = _feeGrowthPerLiquidityUnitMap[baseToken][_feeGrowthPerLiquidityUnitIndexMap[baseToken] - 1];
        }
        _feeGrowthPerLiquidityUnitMap[baseToken][_feeGrowthPerLiquidityUnitIndexMap[baseToken]] =
            previousValue +
            lpCalcFeeRatio;
        _feeGrowthPerLiquidityUnitIndexMap[baseToken]++;
    }

    function getLPCalcFeeRatio(address baseToken, uint256 index) external view returns (uint256) {
        return _feeGrowthPerLiquidityUnitMap[baseToken][index];
    }

    function getLPCalcFeeRatioIndex(address baseToken) external view returns (uint256) {
        return _feeGrowthPerLiquidityUnitIndexMap[baseToken];
    }
}
...
abstract contract LiquidityProviderStorageV1 {
    ...
    // TODO: ******remove below mapping before deploy to prod.********
    // address: baseToken, value: fee/_totalLiquidity
    mapping(address => uint256[]) public _totalLPCalcFeeRatioMap;

    uint256 internal _feeGrowthPerLiquidityUnitIndex;

    // address: baseToken
    // uint256: the global accumulator of trade fee for liquidity unit
    mapping(address => mapping(uint256 => uint256)) internal _feeGrowthPerLiquidityUnitMap;
    // address: baseToken
    // uint256: index
    mapping(address => uint256) internal _feeGrowthPerLiquidityUnitIndexMap;
}