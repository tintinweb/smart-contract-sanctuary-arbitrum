...
contract PixelMushrohmERC721 is IPixelMushrohmERC721, ERC721Enumerable, PixelMushrohmAccessControlled {
    ...
    function maxOutSporePower(uint256 _tokenId) external override onlyOwner {
        ...
        uint256 _newSporePower = sporePowerCost.mul(maxLevel);
        uint256 _sporePowerDiff = _newSporePower.sub(sporePowerMap[_tokenId]);
        totalSporePower = totalSporePower.add(_sporePowerDiff);
        sporePowerMap[_tokenId] = _newSporePower;
    }
    ...
    function redeemSporePower(uint256 _tokenId, uint256 _amount) external override onlyRedeemer {
        require(!staking.isStaked(_tokenId), "PixelMushrohm: unstake before redeeming spore power");
        require(_amount > 0, "PixelMushrohm: amount must be greater than 0");
        require(_amount <= sporePowerMap[_tokenId], "PixelMushrohm: cannot redeem more spore power than is available");

        emit RedeemSporePower(_tokenId, _amount);
        totalSporePower = totalSporePower.sub(_amount);
        sporePowerMap[_tokenId] = sporePowerMap[_tokenId].sub(_amount);
    }
...
interface IPixelMushrohmStaking {
    ...
    function setStakingCost(uint256 _cost) external;
