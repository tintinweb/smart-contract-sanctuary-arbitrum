...
contract xmmm  is ERC721Enumerable, ERC721Royalty, MintCounter, ReentrancyGuard, AccessControl, Ownable {
    ...
    constructor(address payable shareholderAddress_) ERC721("xmm", "MMX") {
        ...
        ableToMint(numberOfTokens)
    {
        ...
        uint256 ts = totalSupply();
        uint256 startingIndex = ts + reserveSupply;
        uint256 tokenId = startingIndex;   
        
        require(startingIndex + recipients.length * numberOfTokens <= MAX_SUPPLY, "Exceeds maximum supply");
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenIdStr)) : "";
    }
