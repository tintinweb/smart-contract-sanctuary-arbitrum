...
struct ToadHouseTraits {
    ...
    HouseBase base;
    WoodType front;
    WoodVariation frontVariation;
    WoodType side;
    WoodVariation sideVariation;
    WoodType door;
    WoodVariation doorVariation;
    WoodType beams;
    WoodVariation beamsVariation;
    WoodType roof;
    WoodVariation roofVariation;
}
...
struct ToadHouseTraitStrings {
    ...
    string beams;
    string roof;
    uint8 frontVariation;
    uint8 sideVariation;
    uint8 doorVariation;
    uint8 beamsVariation;
    uint8 roofVariation;
}
...
string constant FRONT = "Front";
string constant SIDE = "Side";
string constant DOOR = "Door";
string constant BEAMS = "Beams";
string constant ROOF = "Roof";
...
contract ToadHousezMinter is Initializable, ToadHousezMinterContracts {
    ...
    returns(ToadHouseTraits memory _toadHouseTraits)
    {
        ...
        _toadHouseTraits.base = HouseBase(
            _pickTrait(uint16(_randomNumber & 0xFFFF),
            traitTypeToRarities[BASE],
            traitTypeToAliases[BASE]));
        _randomNumber >>= 16;
        ...
        _toadHouseTraits.front = _woodTypeOrder[0];
        _toadHouseTraits.side = _woodTypeOrder[1];
        _toadHouseTraits.door = _woodTypeOrder[2];
        _toadHouseTraits.beams = _woodTypeOrder[3];
        _toadHouseTraits.roof = _woodTypeOrder[4];

        _toadHouseTraits.frontVariation = WoodVariation(
            _pickTrait(uint16(_randomNumber & 0xFFFF),
            traitTypeToRarities[VARIATION],
            traitTypeToAliases[VARIATION]));
        _randomNumber >>= 16;

        _toadHouseTraits.sideVariation = WoodVariation(
            _pickTrait(uint16(_randomNumber & 0xFFFF),
            traitTypeToRarities[VARIATION],
            traitTypeToAliases[VARIATION]));
        _randomNumber >>= 16;

        _toadHouseTraits.doorVariation = WoodVariation(
            _pickTrait(uint16(_randomNumber & 0xFFFF),
            traitTypeToRarities[VARIATION],
            traitTypeToAliases[VARIATION]));
        _randomNumber >>= 16;

        _toadHouseTraits.beamsVariation = WoodVariation(
            _pickTrait(uint16(_randomNumber & 0xFFFF),
            traitTypeToRarities[VARIATION],
            traitTypeToAliases[VARIATION]));
        _randomNumber >>= 16;

        _toadHouseTraits.roofVariation = WoodVariation(
            _pickTrait(uint16(_randomNumber & 0xFFFF),
            traitTypeToRarities[VARIATION],
            traitTypeToAliases[VARIATION]));
    }
...
abstract contract ToadHousezMinterState is Initializable, IToadHousezMinter, AdminableUpgradeable {
    ...
    function __ToadHousezMinterState_init() internal initializer {
        ...
        traitTypeToRarities[BACKGROUND] = [255];
        traitTypeToAliases[BACKGROUND] = [0];

        traitTypeToRarities[BASE] = [255];
        traitTypeToAliases[BASE] = [0];

        traitTypeToRarities[VARIATION] = [255, 224, 80];
