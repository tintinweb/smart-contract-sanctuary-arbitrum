...
contract TradingPortalFacet is ITradingPortal, OnlySelf {
    ...
    function openMarketTrade(OpenDataInput calldata data) external override {
        ...
        ITrading.PendingTrade memory pt = ITrading.PendingTrade(
            ...
            data.tokenIn, data.qty, data.stopLoss, data.takeProfit, uint128(Chain.currentBlockNumber())
        );
        bytes32 tradeHash = keccak256(abi.encode(pt, ts.salt, "trade", Chain.currentBlockNumber(), Chain.currentTimestamp()));
        ts.salt++;
    ...
    /*
       ...
       totalBalanceUsd = 80012 + 59946 = 149958

       points
       USDC = 59946 * 10000 / 149958 = 3997
