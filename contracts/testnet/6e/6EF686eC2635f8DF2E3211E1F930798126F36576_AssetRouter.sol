...
contract AssetRouter is IAssetRouter, AccessControl, ReentrancyGuard {
    ...
    event CrossChainSwapPerformed(SwapMessage _message);
    ...
    function swap(SwapParams calldata swapParams) external payable nonReentrant returns (bytes32) {
        ...
        require(swapMessage.amount >= swapParams.minAmount, "Slippage too high");

        require(cp.bandwidth >= swapParams.amount, "Src bandwidth too low");
        require(cp.kbp + cp.vouchers >= swapMessage.amount, "Dst bandwidth too low");
        require(cp.bandwidth >= swapMessage.amount, "Dst bandwidth too low");
        ...
        (swapMessage.vouchers, swapMessage.optimalDstBandwidth) = _sendVouchers(
            swapParams.srcPoolId,
        ...
        IERC20(IAssetV2(pool.poolAddress).token()).safeTransferFrom(msg.sender, pool.poolAddress, swapParams.amount);
        {
            swapMessage.id = keccak256(
                abi.encode(bridge, bridge.nextNonce(swapParams.dstChainId), block.timestamp, MESSAGE_TYPE.SWAP)
            );

            // console.log("src Pool ", swapParams.srcPoolId);
            // console.log("dstPoolId Pool ", swapParams.dstPoolId);
            // console.log("to  ", swapParams.to);
            console.logBytes(swapMessage.payload);
    ...
    function swapRemote(SwapMessage memory receivedMessage)
        external
        nonReentrant
        ...
        returns (SwapMessage memory)
    {
        ChainPath storage cp = getChainPath(
            receivedMessage.dstPoolId,
            receivedMessage.srcChainId,
            receivedMessage.srcPoolId
        );
        PoolObject storage pool = poolLookup[receivedMessage.dstPoolId];
        cp.kbp -= receivedMessage.amount;
        cp.actualKbp -= receivedMessage.amount;
        cp.actualBandwidth += receivedMessage.amount;
        IAssetV2 asset = IAssetV2(pool.poolAddress);
        asset.release(receivedMessage.receiver, receivedMessage.amount);
        if (receivedMessage.fee > 0 && address(feeCollector) != address(0)) {
            emit FeeCollected(receivedMessage.fee);
            pool.totalLiquidity += receivedMessage.fee;
            IERC20(asset.token()).safeApprove(address(feeCollector), receivedMessage.fee);
            feeCollector.collectFees(asset, receivedMessage.fee);
        }
        emit CrossChainSwapPerformed(receivedMessage);
    ...
    function sendVouchers(
        ...
        (uint256 vouchers, uint256 optimalDstBandwidth) = _sendVouchers(_srcPoolId, _dstChainId, _dstPoolId, false);
        id = keccak256(abi.encode(bridge, bridge.nextNonce(_dstChainId), block.timestamp, MESSAGE_TYPE.ADD_LIQUIDITY));
        bridge.dispatchMessage{ value: msg.value }(
            _dstChainId,
    ...
    function getChainPath(
        ...
        require(cp.dstChainId == _dstChainId && cp.dstPoolId == _dstPoolId, "chainPath does not exist");
        return cp;
    }
     ...
     */
    function getEffectivePath(uint16 _dstChainId, uint256 _amountToSimulate)
        external
        view
        returns (uint16[2] memory effectivePath)
    {
        unchecked {
            ...
            for (uint256 i; i < poolsSrc.length; i++) {
                PoolObject memory poolSrc = poolLookup[poolsSrc[i]];

                for (uint256 j; j < poolsDst.length; j++) {
