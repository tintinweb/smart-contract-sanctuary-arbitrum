...
contract LaunchPadPublic is Ownable {
    ...
    function vestedAmount(
        ...
        uint256 totalVestedAmount = info.upfrontAmountClaimed
            ? info.released
            : info.released.add(info.upfrontAmount);
        ...
        if (block.timestamp >= tgeDate) {
            multiplyIntervals = (block.timestamp - tgeDate) / vestInterval;
        }

        if (multiplyIntervals > 0) {
            uint256 newVestedAmount = info
                .leftOverAmount
                .mul(multiplyIntervals)
                .div(vestDuration);
