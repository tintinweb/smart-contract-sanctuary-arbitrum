...
contract FaucetLBFR is Ownable {
    ...
    uint256 timeOffset = 0;

    event Claim(address account, uint256 claimedTokens);
    ...
    function _getWeekID() internal view returns (uint256 weekID) {
    ...
    function _validateSigner(
        ...
        bytes32 digest = ECDSA.toEthSignedMessageHash(
            keccak256(abi.encodePacked(tokens, weekID, sender))
        );
    ...
    function claim(
        ...
        require(weekID == _getWeekID(), "Claiming only allowed for current week");
        require(
        ...
        uint256 claimableTokens = totalTokensAllocated -
            totalTokensAllocatedPerUserPerWeek[msg.sender][weekID];
