...
contract PrimaryIndexTokenAtomicRepayment is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getRemainingDeposit(address user, address projectToken) public view returns(uint remainingDeposit) {
        remainingDeposit = primaryIndexToken.getDepositedAmount(projectToken, user);
    }
   ...
   /**
    ...
    function _deferLiquidityCheck(address user, address projectToken, address lendingToken) internal view {
        (uint256 healthFactorNumerator, uint256 healthFactorDenominator) = primaryIndexToken.healthFactor(user, projectToken, lendingToken);
    ...
    function repayAtomic(address prjToken, uint collateralAmount, bytes memory buyCalldata, bool isRepayFully) public nonReentrant{
        ...
        uint256 depositedProjectTokenAmount = primaryIndexToken.getDepositedAmount(prjToken, msg.sender);
