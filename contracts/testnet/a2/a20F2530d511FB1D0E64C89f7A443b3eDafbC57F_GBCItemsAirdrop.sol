...
import "./GBCLabsItems.sol";

contract GBCItemsAirdrop is ERC1155Receiver {

    GBCLabsItems constant ITEMS = GBCLabsItems(0x88f68f52F43bB17138a8d367650575D52c413069);
    IERC721 constant GBC = IERC721(0xa1C6bf3D592221081f26903477c5967649516343);
    uint constant TOKEN_ID = 20;
    uint constant gap = 3600;

    mapping(uint => uint) private _lastClaim;

    function claim(uint[] memory tokens) external {
        for (uint256 i = 0; i < tokens.length; i++) {
            uint token = tokens[i];
            require(GBC.ownerOf(token) == msg.sender, "Not owner");
            require(_lastClaim[token] + gap <= block.timestamp, "Claim too early");
            _lastClaim[token] = block.timestamp;
        }
        ITEMS.mint(msg.sender, TOKEN_ID, tokens.length);
    }
    ...
    function onERC1155Received(
        ...
        if(msg.sender == address(ITEMS) && id == TOKEN_ID) {
    ...
    function onERC1155BatchReceived(
        ...
        if(msg.sender == address(ITEMS) && ids[0] == TOKEN_ID && ids.length == 1) {
            return this.onERC1155Received.selector;
        } else {
            return bytes4(0);
        }
