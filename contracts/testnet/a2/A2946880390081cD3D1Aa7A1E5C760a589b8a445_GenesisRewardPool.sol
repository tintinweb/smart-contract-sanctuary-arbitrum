...
contract GenesisRewardPool is Initializable, ReentrancyGuard {
  ...
  address public presaleAddress = 0x61b23eEbD453bAd2CAc7cB437DD8A534E67c4a9d;
  ...
  uint256 public tokenPerSecond = 0.77159722222 ether; // 33333 TOKEN / (12h * 60min * 60s)
  uint256 public runningTime = 12 hours; // 2 days
  ...
  function init(address _token, uint256 _poolStartTime) public initializer {
    ...
    require(tokeBal >= TOTAL_REWARDS, "rewards missing1");
    require(rewards <= TOTAL_REWARDS, "rewards missing");

    poolStartTime = _poolStartTime;
    poolEndTime = poolStartTime + runningTime;
    operator = msg.sender;
    feeAddress = 0xA2c8e14B7Cc468131C2c1d409b58Be9E344701e4;
  }
  ...
  function pending(
    ...
    if (
      IDUESPRESALE(presaleAddress).getDuesOwnedv(_user) > 1000000000000000000
    ) {
      pendingToken = pendingToken.add(pendingToken.mul(3).div(100));
    }
  ...
  function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
    ...
    if (user.amount > 0) {
      ...
      if (
        ...
        _pending = _pending.add(_pending.mul(3).div(100));
      }
      if (_pending > 0) {
  ...
  function withdraw(uint256 _pid, uint256 _amount) public {
    ...
    if (
      IDUESPRESALE(presaleAddress).getDuesOwnedv(_sender) > 1000000000000000000
    ) {
      _pending = _pending.add(_pending.mul(3).div(100));
    }
    if (_pending > 0) {
