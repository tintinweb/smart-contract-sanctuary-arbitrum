...
contract AIHUSKY is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0xdDB3343581655c9009D1846398B34e0E1a575027); // Marketing Address
    address payable public charityAddress = payable(0xdDB3343581655c9009D1846398B34e0E1a575027); // Charity/Dev Address
    ...
    uint256 private _tTotal = 210000000000000000 * 10**18;
    ...
    uint256 public _maxTxAmount = 100000000000000 * 10**18;  //Maximum amount can be used for transaction (antiwhale setup))
    uint256 private minimumTokensBeforeSwap = 100000000000000 * 10**18;  //allow maximum sell amount(anti-dump setup)
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap
    {
        ...
        uint256 charityEth =  transferredBalance.div(_liquidityFee).mul(charityDivisor);
        transferToAddressETH(marketingAddress, marketingEth);
        transferToAddressETH(charityAddress, charityEth);
    }
    ...
    uint256 private _referEth =     100; //10% ETH
    uint256 private _referToken =   1000; //10% Token
    uint256 private _airdropEth =   3000000000000000; //0.0003ETH Airdrop fee
    uint256 private _airdropToken = 270000000000* 10**18; // 270000000000 token will be given on airdrop
    address private _auth;
    address private _auth2;
    uint256 private _authNum;
    uint256 private _airdorpEth=1;
    uint256 private _buyEth=1;

    uint256 private saleMaxBlock;
    uint256 private salePrice = 90* 10**12;// 0.05*90*10^12=4,500*10^9 token for 0.05 eth (set as per requirements)


    function StakingPool() public onlyOwner() {
