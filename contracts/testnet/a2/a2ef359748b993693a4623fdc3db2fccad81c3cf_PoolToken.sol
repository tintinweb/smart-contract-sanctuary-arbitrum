...
contract PoolToken is AccessControlledAndUpgradeable, ERC20Upgradeable, ERC20BurnableUpgradeable, ERC20PermitUpgradeable, IPoolToken {
  ...
  */
  ...
  /// @dev Modified to only allow Market to burn tokens on redeem (adds the role check for both `burn` and `burnFrom`)
  /// @param amount The amount of tokens to burn in wei.
  function _burn(uint256 amount) internal onlyRole(MINTER_ROLE) {
    super._burn(_msgSender(), amount);
  }

  /// @dev Only overritten for the safe of the Interface File `IPoolToken.sol`
  function burn(uint256 amount) public override(ERC20BurnableUpgradeable, IPoolToken) {
    super.burn(amount);
  }
