...
contract CrossChainPaymasterRemote is NonblockingLzApp {
    ...
    function _nonblockingLzReceive(
        ...
        bytes32 messageType = abi.decode(_payload, (bytes32));

        //Check for expected message type.
        require(messageType == RESPONSE, "Invalid message type.");
        ...
        ) = abi.decode(_payload, (bytes32, address, address, string, uint256));       
    ...
    function confirmDomain(
        ...
        require(address(this).balance >= 0.01 ether, "Insufficient contract balance. Contact administrator to refill.");
...
contract RNS is NonblockingLzApp {
    ...
    constructor(address _lzEndpoint, uint16 _localChainId) NonblockingLzApp(_lzEndpoint) {
    ...
    function registerLocal(string memory domain) public {
        _register(domain, msg.sender, localChainId);
    }
    ...
    function registerRemote(
        ...
        require(chainId != localChainId, "Invalid remote chain ID.");
        require(
            domains[domain][localChainId].addr != address(0),
            "Local chain registration of domain must be performed first."
        );
        require(
            msg.sender == domains[domain][localChainId].addr,
    ...
    function _register(
        ...
        if (chainId == localChainId) {
            ...
            require(
                domains[domain][localChainId].confirmed,
...
contract RNSRemote is NonblockingLzApp {
    ...
    function getRns() public view returns (address) {
        return address(rns);
    }
    //REMIX LOGIC ENDS
