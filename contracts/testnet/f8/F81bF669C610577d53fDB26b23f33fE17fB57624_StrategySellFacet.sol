 ...
 * @author dederi
 * @notice This contract is strategy Open 涵盖平仓策略，平仓合并.
 */
contract StrategySellFacet is IStrategySell {
    ...
    function sellStrategy(
        ...
        sellerRequest.admin = takerAddr;

        LibEIP712._checkSignatureExists(makerStrategy.admin, signature);

        StrategyTypes.StrategyDataWithOwner memory makerData = LibPositionCore._getStrategyWithOwner(
            sellerRequest.strategyId
        );
        LibStrategyConfig._ensureAdminAndActive(makerData, makerStrategy.admin);
        if (!LibStrategySell.validatePosition(makerData.positionIds, sellerRequest.positionIds)) {
        ...
        LibStrategyConfig._updateUserSellRequestStrategy(sellerRequestHash, sellerRequest);

        emit StrategySellRequested(makerRequestHash, sellerRequestHash, makerStrategy, sellerRequest);
    }

    //改成传递 requestid，防止链下出问题
    function executeSellStrategy(bytes32 takerRequestHash, bytes32 makerRequestHash) external {
        ...
        StrategyTypes.SellStrategyRequest memory takerRequest = l.userSellRequest[takerRequestHash];
        LibMarketPricer._checkCollateralSufficiency(makerStrategy.collaterals, makerRequestHash, true);

        StrategyTypes.StrategyDataWithOwner memory takerStrategy = LibPositionCore._getStrategyWithOwner(
            takerRequest.strategyId
        );
        //检查 owner 是否转移，策略是否有效
        LibStrategyConfig._ensureAdminAndActive(takerStrategy, takerRequest.admin);

        uint256 makerStrategyId = LibPositionCore._handleCreateStrategy(makerStrategy, takerRequest.receiver);
        // LibPositionCore._handleDecreasePositions(takerRequest.strategyId, takerRequest.positionIds);

        // 要把平仓之后盈利的部分要放在策略账户里面，更改 collateral ，pnl
        // _transferPremium(
        //     makerStrategy,
        //     makerStrategy.admin,
        //     takerStrategy.owner
        // );
        //减少保证金
        // LibVault.marginDecrease(makerStrategy.admin, makerStrategy.collaterals);
        //TODO 前置判断，判断提前平仓是否有盈利 有的话需要把盈利放在 unsettled 里面
        // LibVault.vault.lockBalanceUpdate(takerStrategy.owner,LibStrategyConfig.usdcToken,0);

        delete l.userRequestStrategy[makerRequestHash];
        delete l.userSellRequest[takerRequestHash];
...
library LibStrategySell {
    ...
    struct Layout {
        /// @notice 支持的抵押品
        mapping(address => bool) userSellRequest;
    }

    /// @dev 验证2个仓位id数组中是否有重复
    function validatePosition(
        ...
        for (uint256 i = 0; i < validatePositionIds.length; ) {
            for (uint256 j = 0; j < strategyPositionIds.length; ) {
                ...
                if (j == strategyPositionIds.length - 1) {
                    return false;
                }
                unchecked {
                    ++j;
