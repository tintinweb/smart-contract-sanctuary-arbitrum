...
interface ISmolChopShop {
    ...
    error MustEquipSkinToUpgradeVehicle();
    ...
    /// @notice Equip sets of unlocked upgrades for vehicles.
    /// @param _vehicleAddress Array of addresses for collections that vehicle tokens are from.
    /// @param _vehicleId Array of id numbers for selected vehicle tokens.
    ...
    /// @notice Set new base URI to be concatenated with upgrade Id + suffix.
    /// @param _upgradeType Enum(0-6) representing which type of upgrade is being referenced.
    ...
    /// @notice Set new URI suffix to be added to the end of baseURI + upgrade Id.
    /// @param _upgradeType Enum(0-6) representing which type of upgrade is being referenced.
...
contract SmolChopShop is Initializable, SmolChopShopInternal {
    ...
    // Equip sets of unlocked upgrades for vehicles.
    /// @inheritdoc ISmolChopShop
    function equipUpgrades(
        address[] calldata _vehicleAddress,
        uint256[] calldata _vehicleId,
...
abstract contract SmolChopShopAdmin is Initializable, SmolChopShopView {
    ...
    // Set new base URI to be concatenated with upgrade Id + suffix.
    /// @inheritdoc ISmolChopShop
    function changeBaseURI(UpgradeType _upgradeType, string calldata _newBaseURI) external requiresEitherRole(ADMIN_ROLE, OWNER_ROLE){
        baseURI[_upgradeType] = _newBaseURI;
    }

    // Set new URI suffix to be added to the end of baseURI + upgrade Id.
    /// @inheritdoc ISmolChopShop
    function changeSuffixURI(UpgradeType _upgradeType, string calldata _newSuffixURI) external requiresEitherRole(ADMIN_ROLE, OWNER_ROLE){
        suffixURI[_upgradeType] = _newSuffixURI;
    }
...
abstract contract SmolChopShopInternal is Initializable, SmolChopShopAdmin {
    ...
    function _equip(
        ...
        uint32 skinId = vehicleToEquippedUpgrades[_vehicleAddress][_vehicleId].skin;
        if (_upgradesToEquip.skin == 0 && skinId == 0) revert MustEquipSkinToUpgradeVehicle();
        Vehicle memory vehicle = _upgradesToEquip;
        if (_upgradesToEquip.skin == 0) {
            vehicle.skin = skinId;
        }
        else {
            skinId = _upgradesToEquip.skin;
            _checkBeforeEquip(_vehicleAddress, _vehicleId, _upgradesToEquip.skin, 0, UpgradeType.Skin);
        }
        _checkBeforeEquip(_vehicleAddress, _vehicleId, vehicle.background, skinId, UpgradeType.Background);
        _checkBeforeEquip(_vehicleAddress, _vehicleId, vehicle.color, skinId, UpgradeType.Color);
        _checkBeforeEquip(_vehicleAddress, _vehicleId, vehicle.topMod, skinId, UpgradeType.TopMod);
        _checkBeforeEquip(_vehicleAddress, _vehicleId, vehicle.frontMod, skinId, UpgradeType.FrontMod);
        _checkBeforeEquip(_vehicleAddress, _vehicleId, vehicle.sideMod, skinId, UpgradeType.SideMod);
        _checkBeforeEquip(_vehicleAddress, _vehicleId, vehicle.backMod, skinId, UpgradeType.BackMod);
...
abstract contract SmolChopShopState is Initializable, UtilitiesV2Upgradeable, ISmolChopShop {
    ...
    /// @notice Base URI for a specific Upgrade type to be concatenated with Upgrade ID + suffix
    /// @dev could also hold individual URIs or SVGs in upgrade struct but having IPFS folders seperated by type is cheaper. dev has option of using either method
    mapping (UpgradeType => string)
        public baseURI;

    /// @notice Suffix URI for a specific Upgrade type to be concatenated with base + Upgrade ID
    /// @dev ex: ".png" or ".json"
    mapping (UpgradeType => string)
        public suffixURI;
...
abstract contract SmolChopShopView is Initializable, SmolChopShopState {
    ...
    function upgradeURI(uint256 _upgradeId) public view returns (string memory uri_) {
        ...
        uint256 id = _upgradeId - (uint256(_upgradeType) * UPGRADE_TYPE_OFFSET); 
        string memory URI = baseURI[_upgradeType];
        string memory suffix = suffixURI[_upgradeType];
        uri_ = bytes(URI).length > 0 ? string(abi.encodePacked(URI, id.toString(), suffix)) : "";
    } 
    ...
    // ex: _upgradeType == _upgradeType.Background, skip when the id is < the first id in Backgrounds (1 * UPGRADE_TYPE_OFFSET) or >= UpgradeType.Color
