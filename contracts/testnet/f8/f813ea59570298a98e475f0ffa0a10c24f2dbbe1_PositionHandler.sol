...
contract PositionHandler is PositionConstants, IPositionHandler, Ownable {
    ...
    function trggerTest(        
        ...
        try vaultUtils.validateTrigger(_isLong, _getIndexPrice(_prices), order) returns (uint8 res) {
            statusFlag = res;
        ...
        bool hitTrigger;
        try triggerOrderManager.executeTriggerOrders(
            position.owner,
            _path[0],
            _isLong,
            _posId,
            _getIndexPrice(_prices)
        ) returns (bool res, uint256 triggerAmountPercent) {
            hitTrigger = res;
        } catch (bytes memory err) {
            errMsg = string(err);
        }

        emit TestValidateTrigger(_key, statusFlag == ORDER_FILLED, hitTrigger, errMsg);
    }

    event TestValidateTrigger(bytes32 key, bool, bool, string errMsg);
