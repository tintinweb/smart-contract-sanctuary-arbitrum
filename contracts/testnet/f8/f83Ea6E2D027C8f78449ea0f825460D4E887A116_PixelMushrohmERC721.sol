...
contract PixelMushrohmERC721 is IPixelMushrohmERC721, ERC721Enumerable, PixelMushrohmAccessControlled, ReentrancyGuard {
    ...
    uint256 public lastFirstGenId = 0;
    ...
    function mintAllFirstGen(uint256 _amount) external onlyOwner {
        uint256 last = 0;
        for (uint256 i = lastFirstGenId.add(1); i < _amount.add(lastFirstGenId); i++) {
            _mint(msg.sender, i, MintType.BRIDGE);
            last = i;
        }
        lastFirstGenId = last;
    }

    function mintAllNextGen(uint256 _amount) external onlyOwner {
