...
/**************************************************************************************************************
// This contract will allow an entity to configure and push events for passports.
// A new version of this contract should be created and deployed by a factory each time an org/entity
// offers a new event to its patrons who hold a particular passport.
// The mobile app will interact with this contract to fetch information 
// about various events available for a passport.
**************************************************************************************************************/
// SPDX-License-Identifier: AGPL-3.0
pragma solidity ^0.8.0;
...
contract Event is Collectible {
    ...
    struct EventDetails {
        uint256 startTime;
        uint256 endTime;
        uint256 price;  // entry fee. 6 decimals precision 24494022 = 24.494022 USD
        uint256 maxPurchase;
        bool transferable;
    }
    ...
    constructor(
        ...
        collectibleData.rewards = _calculateRewards(_eventDetails.price);  

        IDispatcher(authority.getAuthorities().dispatcher).addOfferWithContext(address(this), eventDetails.maxPurchase, eventDetails.endTime, eventDetails.transferable);  
    }
    ...
    function mint (
        ...
        uint256 _expiry,
        bool _transferable
    ) external override onlyDispatcher returns (uint256 _nftId) {
        _nftId = Collectible._mint(_patron, _expiry, _transferable);
    }

    function toggle(uint256 _nftId) external onlyBartender(collectibleData.entity) returns(bool _status) {
        _status = _toggle(_nftId);
    }
    ...
    function creditRewards(address _patron, uint256 _amount) external pure {
        // Do nothing as business doesn't need this feature on events
       return;
    }
