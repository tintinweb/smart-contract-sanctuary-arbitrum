...
interface IZKBridgeEntrypoint {
    ...
    function sendFromL2(uint16 dstChainId, address dstAddress, bytes memory payload) external payable returns (uint64 sequence);
...
contract ZKBridge is Governance, IZKBridgeEntrypoint {
    ...
    function validateTransactionFromL2(uint16 srcChainId, address srcAddress, address dstAddress, uint64 sequence, bytes calldata payload) external{
        IZKBridgeReceiver(dstAddress).zkReceive(srcChainId, srcAddress, sequence, payload);
    }
    ...
    function _parsePayload(bytes memory payload) internal pure returns (Payload memory txPayload) {
        uint index = 0;
