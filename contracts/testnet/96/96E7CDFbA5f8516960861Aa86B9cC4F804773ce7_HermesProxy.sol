 ...
 */
interface IConfigurableRightsPool is
...
library KassandraSafeMath {
    ...
    function bpow(uint base, uint exp) internal pure returns (uint) {
        require(
            base >= KassandraConstants.MIN_BPOW_BASE,
            "ERR_BPOW_BASE_TOO_LOW"
        );
        require(
            base <= KassandraConstants.MAX_BPOW_BASE,
        ...
        uint fractionPartPow = bpowApprox(
            base,
            fractionPart,
            KassandraConstants.BPOW_PRECISION
        );
    ...
    function bpowApprox(
        ...
        for (uint i = 1; term >= precision; i++) {
            ...
            (uint c, bool cneg) = bsubSign(
                exp,
                (bigK - KassandraConstants.ONE)
            );
...
// File contracts/utils/HermesProxy.sol
...
contract HermesProxy is Ownable {
    ...
    function joinPool(
        ...
        {
            IKassandraCommunityStore.PoolInfo memory poolInfo = communityStore
                .getPoolInfo(crpPool);
            require(
                !poolInfo.isPrivate ||
        ...
        returns (uint256 tokenAmountOut, uint256 spotPriceAfter)
    {
        ...
        maxPrice = KassandraSafeMath.bdiv(
            KassandraSafeMath.bmul(maxPrice, tokenInExchange),
            tokenOutExchange
        );
