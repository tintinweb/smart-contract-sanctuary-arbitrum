...
contract FbDaoMasterChef is OwnableUpgradeable, ReentrancyGuard {
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 _shareReward = (_seconds *
                rewardPerSecond *
                pool.allocPoint) / totalAllocPoint_;
    ...
    function pendingAllRewards(address _user)
        external
        view
        returns (uint256 _totalPendingRewards)
    {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            _totalPendingRewards =
                _totalPendingRewards +
                pendingReward(pid, _user);
        }
    }
    ...
    function setRewardPerSecond(uint256 _rewardPerSecond) external onlyOwner {
        require(_rewardPerSecond <= 0.01 ether, "too high rate");
        massUpdatePools();
        rewardPerSecond = _rewardPerSecond;
    }
    ...
    function _harvestReward(
        ...
        PoolInfo memory pool = poolInfo[_pid];
        uint256 _pending = ((user.amount * pool.accSharePerShare) / 1e18) -
            user.rewardDebt;
        if (_pending > 0) {
            if (_burnReward) {
                emit RewardSacrificed(_account, _pid, _pending);
            } else {
                _transferReward(_account, _pending);
                emit RewardPaid(_account, _pid, _pending);
            }
    ...
    function _transferReward(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            IERC20(reward).transfer(_to, _amount);
        }
    }
