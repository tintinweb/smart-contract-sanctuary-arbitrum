...
contract Farm is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 currentDeposit; // Current deposited, fix from sushiswap to allow single staking of TOKEN
    }
    ...
    // Starts as the multisig but will pass to veTOKEN voter
        ...
        returns (uint256)
    {
        ...
        return
            _calculatePending(user.rewardDebt, accTokenPerShare, user.amount);
    }

    // ============================== User Functions ==============================

    /**
     * Deposit LP tokens to Millener for TOKEN allocation.
     * @param _pid pool Id
    ...
    /**
     * Withdraw LP tokens from Millener.
     * @param _pid pool Id
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        user.rewardDebt =
            user.rewardDebt -
            int256(_amount.mul(pool.accTokenPerShare).div(1e12));
    ...
    function compound(uint256 _pid) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        if (address(pool.lpToken) != address(token)) revert E1();
        updatePool(_pid);
        uint256 pending = _calculatePending(
            user.rewardDebt,
            pool.accTokenPerShare,
    ...
    /**
     * Updates the reward rate per block from now on.
     * @param _tokenPerBlock new rate
     */
    ...
    function _deposit(
        ...
        if (_amount > 0) {
            ...
            _user.rewardDebt =
                _user.rewardDebt +
                int256(_amount.mul(_pool.accTokenPerShare).div(1e12));
     ...
     * @param _accTokenPerShare accomulated token per share
    ...
    function _calculatePending(
        ...
        return
            _rewardDebt < 0
                ? _amount.mul(_accTokenPerShare).div(1e12).add(
                    uint256(-_rewardDebt)
                )
                : _amount.mul(_accTokenPerShare).div(1e12).sub(
                    uint256(_rewardDebt)
                );
    }
    ...
    error E2(); // Reward transaction failed (add more TOKEN to Millener)
