...
contract FeeCalculator is Ownable, Governable, IFeeCalculator {
    ...
    mapping(address => FixedPoint.Unsigned) public accountFeeDiscount;
    mapping(address => FixedPoint.Unsigned) public senderFeeDiscount;

    FixedPoint.Unsigned public MAX_ACCOUNT_DISCOUNT = FixedPoint.fromUnscaledUint(50).div(100); // 50%
    FixedPoint.Unsigned public MAX_SENDER_DISCOUNT = FixedPoint.fromUnscaledUint(90).div(100); // 90%

    event SetIsDiscountEnabled(bool isDiscountEnabled);
    event SetDiscountForAccount(address account, FixedPoint.Unsigned discount);
    event SetDiscountForSender(address sender, FixedPoint.Unsigned discount);

    constructor() Ownable() Governable() {}

    /**
     * @notice Get fee for a particular account, sender, and product.
    ...
    function getFee(
        ...
        address account,
        address sender
    ) external view override returns (FixedPoint.Unsigned fee) {
        fee = productFee;
        if (isDiscountEnabled) {
            FixedPoint.Unsigned discount = account == sender
                ? accountFeeDiscount[account]
                : accountFeeDiscount[account].add(senderFeeDiscount[sender]);

            fee = fee.mul(FixedPoint.fromUnscaledUint(1).sub(discount));
        }
    ...
    function setDiscountForAccount(address _account, FixedPoint.Unsigned _discount) external onlyOwner {
        require(_discount.isLessThanOrEqual(MAX_ACCOUNT_DISCOUNT));
        accountFeeDiscount[_account] = _discount;
        emit SetDiscountForAccount(_account, _discount);
    }

    function setDiscountForSender(address _sender, FixedPoint.Unsigned _discount) external onlyOwner {
        require(_discount.isLessThanOrEqual(MAX_SENDER_DISCOUNT));
