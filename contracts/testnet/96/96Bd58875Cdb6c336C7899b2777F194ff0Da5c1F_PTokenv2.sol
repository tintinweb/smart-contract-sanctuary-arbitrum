...
contract PTokenv2 is ERC20Upgradeable, IPToken {
    ...
    function applyDeposit(
        ...
        require(usdtAmount <= usdt.balanceOf(receiver), "usdt amount not enough");
        ...
        bytes32 requestId = keccak256(abi.encodePacked("deposit", _msgSender(), usdtAmount, receiver, applyId++));
        RequestIdDeposit[requestId] = RequestApplyDeposit(_msgSender(), usdtAmount, receiver);

        emit ApplyDeposit(requestId);
    }

    function proxyTest() public returns(uint){
        return convertToUsdt(10000,0) * 10;
    }

    function runDeposit(bytes32 requestId, int uPnl) public isUpnlFeeder returns (uint){
        RequestApplyDeposit memory applyDepositData = RequestIdDeposit[requestId];

        require(applyDepositData.sender != address(0) && applyDepositData.usdtAmount != 0, "request id not gound");
        ...
        AddressLocks[applyDepositData.sender].push(Lock(applyDepositData.sender, plpAmount, block.timestamp));
    ...
    function applyWithdraw(
        ...
        require(plpAmount <= usdt.balanceOf(receiver), "plp amount not enough");
        require(plpAmount >= MinWithdraw, "plpAmount amount to small");

        uint sum = 0;
        for (uint i = 0; i < AddressLocks[_msgSender()].length; i++) {
            if (AddressLocks[_msgSender()][i].timestamp + LockDuration > block.timestamp || sum >= plpAmount) {
                break;
            }
        ...
        bytes32 requestId = keccak256(abi.encodePacked("withdraw", _msgSender(), plpAmount, receiver, applyId++));
        RequestIdWithdraw[requestId] = RequestApplyWithdraw(_msgSender(),plpAmount,receiver);

        emit ApplyWithdraw(requestId);
    }

    function runWithdraw(bytes32 requestId, int uPnl) public isUpnlFeeder returns (uint){
        RequestApplyWithdraw memory applyWithdrawData = RequestIdWithdraw[requestId];

        require(applyWithdrawData.sender != address(0) && applyWithdrawData.plpAmount != 0, "request id not gound");
        require(applyWithdrawData.plpAmount <= usdt.balanceOf(applyWithdrawData.sender), "assets amount not enough");
        ...
        for (i = 0; i < AddressLocks[applyWithdrawData.receiver].length; i++) {
            if (AddressLocks[applyWithdrawData.receiver][i].timestamp + LockDuration > block.timestamp || sum >= applyWithdrawData.plpAmount) {
                break;
            }
        ...
        if (sum != applyWithdrawData.plpAmount) {
            ...
            AddressLocks[applyWithdrawData.receiver][i - 1].assets = sum - applyWithdrawData.plpAmount;
        } else {
    ...
    // To be compatible with old pairs storage contract v6 (to be used only with gUSDT vault)
