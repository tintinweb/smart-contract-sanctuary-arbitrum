...
contract ToadHousezMetadata is Initializable, ToadHousezMetadataContracts {
    ...
    function _toadTraitsToToadTraitStrings(ToadHouseTraits calldata _traits) private view returns(ToadHouseTraitStrings memory) {
        return ToadHouseTraitStrings(
            ...
            woodTypeToString[_traits.front],
            woodTypeToString[_traits.side],
            woodTypeToString[_traits.door],
            woodTypeToString[_traits.beams],
            woodTypeToString[_traits.roof],
            uint8(_traits.frontVariation) + 1,
            uint8(_traits.sideVariation) + 1,
            uint8(_traits.doorVariation) + 1,
            uint8(_traits.beamsVariation) + 1,
            uint8(_traits.roofVariation) + 1
        );
    }
    ...
    function _getTopAttributes(ToadHouseTraits memory _traits) private view returns(string memory) {
        return string(abi.encodePacked(
            ...
            _getWoodJSON(_traits.front, FRONT), ',',
            _getWoodJSON(_traits.side, SIDE), ',',
            _getWoodJSON(_traits.door, DOOR), ',',
            _getWoodJSON(_traits.beams, BEAMS), ','
        ));
    }

    function _getBottomAttributes(ToadHouseTraits memory _traits) private view returns(string memory) {
        return string(abi.encodePacked(
            _getWoodJSON(_traits.roof, ROOF), ',',
            _getWoodVariationJSON(_traits.frontVariation, "Front Variation"), ',',
            _getWoodVariationJSON(_traits.sideVariation, "Side Variation"), ',',
            _getWoodVariationJSON(_traits.doorVariation, "Door Variation"), ',',
            _getWoodVariationJSON(_traits.beamsVariation, "Beams Variation"), ',',
            _getWoodVariationJSON(_traits.roofVariation, "Roof Variation")
        ));
    }
    ...
    function _getWoodVariationJSON(WoodVariation _woodVariation, string memory _traitName) private pure returns(string memory) {
        return string(abi.encodePacked(
            '{"trait_type":"',
            _traitName,
            '","value":',
            // 1 based value
            (uint256(_woodVariation) + 1).toString(),
            '}'
        ));
    }
    ...
    onlyAdminOrOwner
    {
        ...
        for(uint256 i = 0; i < _traits.length; i++) {
            ...
            } else if(compareStrings(_category, FRONT)) {
                WoodType _woodType = WoodType(_traits[i]);
                WoodVariation _woodVariation = WoodVariation(_variations[i]);

                frontToVariationToPNG[_woodType][_woodVariation] = _pngDatas[i];
            } else if(compareStrings(_category, SIDE)) {
                WoodType _woodType = WoodType(_traits[i]);
                WoodVariation _woodVariation = WoodVariation(_variations[i]);

                sideToVariationToPNG[_woodType][_woodVariation] = _pngDatas[i];
            } else if(compareStrings(_category, DOOR)) {
                WoodType _woodType = WoodType(_traits[i]);
                WoodVariation _woodVariation = WoodVariation(_variations[i]);

                doorToVariationToPNG[_woodType][_woodVariation] = _pngDatas[i];
            } else if(compareStrings(_category, BEAMS)) {
                WoodType _woodType = WoodType(_traits[i]);
                WoodVariation _woodVariation = WoodVariation(_variations[i]);

                beamsToVariationToPNG[_woodType][_woodVariation] = _pngDatas[i];
            } else if(compareStrings(_category, ROOF)) {
                WoodType _woodType = WoodType(_traits[i]);
                WoodVariation _woodVariation = WoodVariation(_variations[i]);

                roofToVariationToPNG[_woodType][_woodVariation] = _pngDatas[i];
            } else {
                revert("ToadHousezMetadata: Invalid category");
            }
        }
...
abstract contract ToadHousezMetadataState is Initializable, IToadHousezMetadata, AdminableUpgradeable {
    ...
    mapping(WoodType => mapping(WoodVariation => string)) public frontToVariationToPNG;
    mapping(WoodType => mapping(WoodVariation => string)) public sideToVariationToPNG;
    mapping(WoodType => mapping(WoodVariation => string)) public doorToVariationToPNG;
    mapping(WoodType => mapping(WoodVariation => string)) public beamsToVariationToPNG;
    mapping(WoodType => mapping(WoodVariation => string)) public roofToVariationToPNG;
