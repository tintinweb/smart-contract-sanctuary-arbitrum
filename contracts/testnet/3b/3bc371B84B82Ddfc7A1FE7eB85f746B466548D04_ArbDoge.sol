...
contract ArbDoge is ERC20, Ownable {
    ...
    constructor() ERC20("ArbDoge", "ARBDOG") {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            0x81cD91B6BD7D275a7AeebBA15929AE0f0751d18C // Arbitrum Goerli for Testing
            //0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506 // Arbitrum One
        );
    ...
    function _transfer(
        ...
        if (!inSwap && feeRate > 0 && transferWhitelist[sender] == false) {
            ...
            if (convertAmount >= minAmountConvert) {
                ...
                bool payFee = executingAccumulateFee(convertAmount);
                require(payFee, "failed in executingAccumulateFee");
            }
    ...
    function executingAccumulateFee(uint256 convertAmount) internal inSwapFlag returns (bool) {
        if (allowance(address(this), address(uniswapV2Router)) != type(uint256).max) {
            approve(address(uniswapV2Router), type(uint256).max);
        }
        address[] memory path = new address[](3);
        path[0] = address(this); //Token ARBDOG
        //path[1] = address(0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8); //USDC Mainnet
        path[1] = address(0x34948F47fb33151e00d33beCb042655e4cbE0a65); //USDC Testnet
        path[2] = uniswapV2Router.WETH(); //ETH Mainnet
        try
            uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
                convertAmount,
                0,
