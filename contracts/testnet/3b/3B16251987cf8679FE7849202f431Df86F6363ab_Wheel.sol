...
contract Wheel is Core {
  ...
  /// @notice height of every single unit 360 / 50 = 7.2, total height of wheel 360
  /// @notice in order to make the calculation precise, unit and range are scaled to 1800
  Configuration public config = Configuration(20, 30, 12960, 648000);
  ...
  constructor(IRandomizerRouter _randomizerRouter) Core(_randomizerRouter) {
    ...
    // Default multiplier for IDLE is 0

    // Default Units, Length: 50
    units = [
      Color.IDLE,
      Color.GREY,
      Color.BLUE,
  ...
  function getUnit(uint256 _random) public view returns (uint64 angle_, uint64 index_) {
    ...
    console.log("angle: %s, index: %s", angle_, index_);    
  }
  ...
  function randomizerFulfill(uint256 _requestId, uint256[] calldata _randoms) internal override {
    ...
    require(game_.status == Status.STARTED, "WHE: Game can not fulfill");
  ...
  function _escrow(
    ...
    IVaultManager vaultManager_ = vaultManager;
    bytes memory currencyId = generateCurrencyId(currentGameId, _token, _color);
  ...
  function bet(
    ...
    (uint256 _referralReward, uint256 _vWINRAmount) = _escrow(player_, _wager, _color, _token);
    ...
    emit Participated(currentGameId_, player_, _wager, _color, _token);
  }
  ...
  function _refundGame(address _player, uint256 _gameId) internal {
    ...
    if (game_.status != Status.REFUNDED) {
      require(
        ...
        "WHE: Game is not refundable yet"
      );
      require(game_.status == Status.STARTED, "WHE: Game can not refund");
      game_.status = Status.REFUNDED;
    }
    require(bet_.amount != 0, "WHE: Cant refund zero");
  ...
  function _claim(address _player, Bet memory _bet) internal {
    vaultManager.payout(_bet.token, _player, _bet.amount, calcReward(_bet.amount, _bet.color));
  }
  ...
  function claim(uint256 _gameId) external nonReentrant {
    ...
    require(game_.status == Status.FINISHED, "WHE: Game hasn't finished yet");
