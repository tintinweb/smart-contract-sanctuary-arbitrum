...
contract PureWallet is ReentrancyGuard {
    using SwapManager for ISwapV2Router;
    ...
    ISwapV2Router internal immutable SWAP;
    ...
    constructor(
        ...
        SWAP = ISwapV2Router(SWAP_ROUTER);
        feeType = _feeChoice;
    ...
    modifier walletShouldActive(address user) {
        if (userStatus[user].isActivated) {
            revert WalletNonActive();
        }
        _;
    }

    modifier walletShouldNonactive(address user) {
        if (userStatus[user].isActivated) {
            revert WalletActive();
        }
        _;
    ...
    modifier isCodeActive(string memory code) {
        if (rewardRecipients[Converter.stringToBytes32(code)].recipient != address(0)) {
            if (rewardRecipients[Converter.stringToBytes32(code)].expiredTime < block.timestamp) {
                revert CodeExpired();
            }
        }
        _;
    ...
    mapping(bytes32 => RewardRecipient) public rewardRecipients;
    ...
    function deactivatePureWallet() external walletShouldActive(msg.sender) {
        userStatus[msg.sender].isActivated = false;
    }

    function activatePureWalletByOwner(address user) external ownerOnly walletShouldNonactive(user) {
        userStatus[user].isActivated = true;
    }

    function addActivateCode(string calldata code, address rewardRcv, uint256 timeInSecond) external ownerOnly {
        require(rewardRecipients[Converter.stringToBytes32(code)].recipient == address(0), "code already exists");

        RewardRecipient memory newRewardRecipient = RewardRecipient(rewardRcv, block.timestamp + timeInSecond);
        rewardRecipients[Converter.stringToBytes32(code)] = newRewardRecipient;
    }

    //deposit, only for ETH//
    function depositETH() public payable nonReentrant walletShouldActive(msg.sender) {
    ...
    function depositERC20(address token, uint256 amountWithFee) external nonReentrant walletShouldActive(msg.sender) {
    ...
    //redeem ETH
    function redeemETH(bytes32 offlineToken) external nonReentrant walletShouldActive(msg.sender) {
    ...
    //redeem ERC-20
    function redeemERC20(address token, bytes32 offlineToken) external nonReentrant walletShouldActive(msg.sender) {
