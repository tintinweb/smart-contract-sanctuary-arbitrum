...
contract AnchorContract is AccessControl, ERC2771Context, Multicall {
    ...
    bytes32 private constant MEMBER_ROLE = keccak256("MEMBER_ROLE");

    /// @dev list of anchored did
    EnumerableSet.Bytes32Set private didHashSet;

    /// @dev mapping fron didHash to didDocHash
    ...
    function anchorDID(
        ...
        uint256 operation;
        if (!didHashSet.contains(_didHash)) operation = 0; //New did anchored
        if (didHashSet.contains(_didHash)) operation = 1; //update existing did
