...
contract BOOStaking is Ownable {
    ...
    function distributeRewards() public {
        ...
        require(timeSinceLastDistribution >= 600, "Less than 1h distribution");
        require(rewardsPool > 0 , "Rewards pool is empty");

        uint256 hoursPassed = timeSinceLastDistribution / 600;
        uint256 rewardsPerToken = stakingRewardRate / totalStaked * DECIMALS * hoursPassed;

        for (uint256 i = 0; i < stakers.length; i++) {
            address staker = stakers[i];
            uint256 stakerReward = stakedBalances[staker] * rewardsPerToken / DECIMALS;
            pendingRewards[staker] += stakerReward;
            rewardsPool -= stakerReward;
        }
