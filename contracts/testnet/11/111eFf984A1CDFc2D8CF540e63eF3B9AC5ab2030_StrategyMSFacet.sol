...
contract StrategyMSFacet is IStrategyMS, ReentrancyGuard {
     ...
     */
    event StrategySplitRequested(bytes32 requestHashA, bytes32 requestHashB);
     ...
     */
    event StrategySplitExecuted(bytes32 requestHashA, bytes32 requestHashB, uint256 strategyIdA, uint256 strategyIdB);
     ...
     * @param originalCollateralsToTopUp Initial collateral for supplement
     * @param newlySplitCollaterals Collateral for newly split positions
     */
    function splitStrategy(
        uint256 strategyId,
        uint256[] memory positionIds,
        StrategyTypes.CollateralInfo[] memory originalCollateralsToTopUp,
        StrategyTypes.CollateralInfo[] memory newlySplitCollaterals
    ) external {
        // 验证owner和是否激活
        address admin = LibAgent._getAdminAndUpdate(msg.sender);
        StrategyTypes.StrategyDataWithOwner memory strategy = LibPositionCore._getStrategyWithOwner(strategyId);
        ...
        // 验证抵押品是否有重复的(这里做了，执行时候可以不做，因为存起来了)
        LibCollateral._checkCollateralDuplicates(originalCollateralsToTopUp);
        LibCollateral._checkCollateralDuplicates(newlySplitCollaterals);
        ...
        // 请求赋值
        l.userSpiltRequest[requestHashA].admin = admin;
        l.userSpiltRequest[requestHashA].strategyId = strategyId;
        l.userSpiltRequest[requestHashA].positionIds = positionIds;
        // 赋值原始待添加的抵押品
        LibCollateral._collateralsMemoryToStorage(
            l.userSpiltRequest[requestHashA].originalCollateralsToTopUp,
            originalCollateralsToTopUp
        );
        // 赋值拆分处理的
        LibCollateral._collateralsMemoryToStorage(
            l.userSpiltRequest[requestHashA].newlySplitCollaterals,
            newlySplitCollaterals
        );

        // 计算拆分后B的组合保证金
        bytes32 requestHashB = LibStrategyConfig._getRequestHashAndUpdateNonce(admin);

        emit StrategySplitRequested(requestHashA, requestHashB);
    }
    ...
    function executeSplitStrategy(bytes32 requestHashA, bytes32 requestHashB) external {
        ...
        StrategyTypes.StrategyDataWithOwner memory strategy = LibPositionCore._getStrategyWithOwner(
            requestParam.strategyId
        );
        LibStrategyConfig._ensureAdminAndActive(strategy, requestParam.admin);

        uint256[] memory strategyPositionIds = strategy.positionIds;
        uint256 originLen = strategyPositionIds.length;
        uint256 spiltLen = requestParam.positionIds.length;
        //        uint256 originalCollateralsToTopUpLen = requestParam.originalCollateralsToTopUp.length;
        //        uint256 newlySplitCollateralsLen = requestParam.newlySplitCollaterals.length;
        ...
        // 验证仓位id数组是否全部包含在策略下面
        LibStrategyMS._validateSplitLegIds(strategy.positionIds, requestParam.positionIds);
        ...
        // 校验信息：拆分后的2个策略的保证金是否充足
        LibMarketPricer._checkCollateralSufficiency(requestParam.newlySplitCollaterals, requestHashB, true);
        //marketPricer.checkCollateralSufficiency(requestParam.newlySplitCollaterals,requestHash,true);

        emit StrategySplitExecuted(requestHashA, requestHashB, strategyIdA, strategyIdB);
    }
    ...
    function mergeStrategies(
        ...
        LibStrategyMS.Layout storage l = LibStrategyMS.layout();
        address admin = LibAgent._getAdminAndUpdate(msg.sender);
        StrategyTypes.StrategyDataWithOwner memory firstStrategy = LibPositionCore._getStrategyWithOwner(
            firstStrategyId
        );
        StrategyTypes.StrategyDataWithOwner memory secondStrategy = LibPositionCore._getStrategyWithOwner(
            secondStrategyId
        );
        // 验证2个策略的owner和是否激活
        ...
        // 给requestHash赋值参数
        l.userMergeRequest[requestHash].admin = admin;
        ...
        LibCollateral._collateralsMemoryToStorage(l.userMergeRequest[requestHash].newCollaterals, newCollaterals);
    ...
    function executeMergeStrategies(bytes32 requestHash) external nonReentrant {
        ...
        LibStrategyConfig._ensureAdminAndActive(firstStrategy, requestParam.admin);
        LibStrategyConfig._ensureAdminAndActive(secondStrategy, requestParam.admin);
        // 执行策略合并
        ...
        if (isZero) {
            ...
            StrategyTypes.StrategyDataWithOwner memory firstStrategy2 = LibPositionCore._getStrategyWithOwner(
                requestParam.firstStrategyId
            );

            StrategyTypes.StrategyDataWithOwner memory secondStrategy2 = LibPositionCore._getStrategyWithOwner(
                requestParam.secondStrategyId
            );

            firstStrategy2;
            secondStrategy2;
            // for 抵押品，循环增加可用余额，减少不可用余额
        } else {
