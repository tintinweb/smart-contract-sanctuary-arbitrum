...
contract NFTBridge is NFTBridgeData {
    ...
    error   InvalidSender();
    error   NoBaseURI();
    error   NotYourToken(address claimant ,address owner);
    

    error InsufficientValueSent(uint256 value , uint256 fee);
    error TokenAlreadyBridged(address _nft);

    event CantFindThisNFT(uint256 homeChain, address nft);

    event CreatedIndividual(address token);
    event CreatedBased(address token);
    event UnrecognisedSelector(uint256 modeswitch);
    event TokenMintedWithURI(address nft,uint256 TokenId,string URI);
    event TokenRestored(address nft,uint256 TokenId,address owner);
    ...
    function createBaseURIToken(
        ...
        if(ds().localTokenInfo[_nft].owner != address(0)) revert TokenAlreadyBridged(_nft);
        if (msg.sender != Ownable(_nft).owner())
            revert NotYourToken(msg.sender,Ownable(_nft).owner());
        ...
        if (msg.value < fee) revert InsufficientValueSent(msg.value , fee);
    ...
    function createIndividualURIBasedToken(address _nft, uint256 destinationChain) external payable returns (bytes32 messageId){
        if (ds().localTokenInfo[_nft].owner != address(0)) revert TokenAlreadyBridged(_nft);
        if(msg.sender != Ownable(_nft).owner()) revert NotYourToken(msg.sender,Ownable(_nft).owner());
        (
            uint64 destinationChainSelector,
            Client.EVM2AnyMessage memory message,
            uint256 fee,
    ...
    function transferMyToken(address _nft, uint256 tokenId, uint256 destinationChain) external payable returns (bytes32 messageId){
        ...
        uint256 fee;
        uint64 destinationChainSelector;
        if(IERC721(_nft).ownerOf(tokenId) != msg.sender) revert NotYourToken(msg.sender,Ownable(_nft).owner());
    ...
    function ccipReceive(
        ...
        uint256 modeswitch = uint256(bytes32(message.data[32:64]));
        if ( (modeswitch == 1) || (modeswitch == 2)){
            ...
            if (block.chainid != td.homeChain) {                 
                if (tsd.nft == address(0)) {
                    emit CantFindThisNFT(td.homeChain, tsd.nft);
                    return;
                }
                NamedNFT n = NamedNFT(tsd.localNFT);
                n.mintWithURI(td.owner,td.TokenId,td.URI);
                emit TokenMintedWithURI(tsd.nft,td.TokenId,td.URI);
            } else {
                IERC721(tsd.nft).transferFrom(address(this),td.owner,td.TokenId);
                emit TokenRestored(tsd.nft,td.TokenId,td.owner);
            }
            ...
            emit UnrecognisedSelector(modeswitch);
        }
    }
