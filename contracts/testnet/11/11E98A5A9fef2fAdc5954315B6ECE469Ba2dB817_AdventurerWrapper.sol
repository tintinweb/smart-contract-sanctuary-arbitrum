...
contract AdventurerWrapper is ManagerModifier, ReentrancyGuard, Pausable {
  ...
  // Mappings
  //=======================================
  mapping(address => uint256) public archetypes;
  ...
  function wrap(
    ...
    for (uint256 index = 0; index < _ids.length; index++) {
      ...
      // TODO: Could give a class based on the collection
      // i.e. Smol - Mischief
      ADVENTURER_DATA.updateAov(
        addr,
        id,
        traits.ADVENTURER_TRAIT_ARCHETYPE,
        archetypes[addr]
      );
  ...
  function configureArchetypes(
    address[] calldata _addresses,
    uint256[] calldata _archetypes
  ) external onlyAdmin {
    require(
      _addresses.length == _archetypes.length,
      "AdventurerWrapper: Invalid input"
    );

    for (uint256 index; index < _addresses.length; index++) {
      archetypes[_addresses[index]] = _archetypes[index];
    }
  }
}
