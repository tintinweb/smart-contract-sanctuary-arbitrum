 ...
 * @author  
 *
 * @notice Contract for settling trades for every account. 
 *
 */
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < words.length; i++) {
            ...
            if (sm.state == 0) {
                ...
                sm.symbolsInaddr = uint256(uint16(uint256(word)));
                sm.symbolCounter = 0;
                marginCache = getMargin(td.account);
                sm.state = 1;
            } else if (sm.state == 1) {
                td.symbol = bytes32(bytes30(word)); //first 31 bytes
                sm.txsInsymbol = uint256(uint16(uint256(word)));
                sm.txCounter = 0;
