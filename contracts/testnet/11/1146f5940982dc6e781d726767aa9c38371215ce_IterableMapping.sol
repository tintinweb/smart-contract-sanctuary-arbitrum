...
contract QFSToken is ERC20, Ownable {
    ...
    event Senddevelopment(uint256 ethSend);
    ...
    constructor() payable ERC20("QFSToken", "QFS") {
        ...
        totalBuyFee         = liquidityFeeOnBuy + developmentFeeOnBuy + marketingFeeOnBuy + rewardsFeeOnBuy + burnFeeOnBuy;
        ...
        totalSellFee        = liquidityFeeOnSell + developmentFeeOnSell + marketingFeeOnSell + rewardsFeeOnSell + burnFeeOnSell;
    ...
    function updateBuyFees(uint256 _liquidityFeeOnBuy, uint256 _developmentFeeOnBuy, uint256 _marketingFeeOnBuy, uint256 _rewardsFeeOnBuy, uint256 _burnFeeOnBuy) external onlyOwner {
    ...
    function updateSellFees(uint256 _liquidityFeeOnSell, uint256 _developmentFeeOnSell, uint256 _marketingFeeOnSell ,uint256 _rewardsFeeOnSell, uint256 _burnFeeOnSell) external onlyOwner {
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 ethShare = (developmentFeeOnBuy + developmentFeeOnSell) + (rewardsFeeOnBuy + rewardsFeeOnSell) + (marketingFeeOnBuy + marketingFeeOnSell);
            ...
            if(contractTokenBalance > 0 && ethShare > 0) {
                ...
                if((developmentFeeOnBuy + developmentFeeOnSell) > 0) {
                    uint256 developmentETH = newBalance * (developmentFeeOnBuy + developmentFeeOnSell) / ethShare;
                    sendETH(payable(developmentWallet), developmentETH);
                    emit Senddevelopment(developmentETH);
                }
    ...
    function updateRewardToken(address _rewardToken) external onlyOwner {
        ...
        require(isContract(_rewardToken), "Reward token must be a contract");
        require(_rewardToken != DEAD, "Reward tokencannot be the zero address");
        rewardToken = _rewardToken;
