...
/**
 * @title Aquarius's AErc20 Contract
...
contract AErc20 is AToken, AErc20Interface {
    ...
    /**
    * @notice Admin call to delegate the votes of the ARS-like underlying
    * @param arsLikeDelegatee The address to delegate votes to
    * @dev ATokens whose underlying are not ArsLike should revert here
    */
    function _delegateArsLikeTo(address arsLikeDelegatee) external {
        require(msg.sender == admin, "only the admin may set the ars-like delegate");
        ArsLike(underlying).delegate(arsLikeDelegatee);
    }
}
...
// File: contracts/AquariusAggregatorPriceOracle.sol
...
contract AquariusAggregatorPriceOracle is PriceOracle {
