...
import {IFundingRateModel} from "../interfaces/IFundingManager.sol";
import {Vault} from "./vault/Vault.sol";
import {IMarketFactory, Addresses} from "../interfaces/IMarketFactory.sol";
...
contract MarketFactory is IMarketFactory {
    ...
    constructor(ITradingEngine _engine) {
        engine = _engine;
    }
    ...
    function deployMarket(
        ...
        if (params.marketType == MarketType.Standard) {
            indexVault = address(
                new Vault{salt: keccak256(abi.encode(addresses.indexToken, params.name))}(
            ...
            quoteVault = address(
                new Vault{salt: keccak256(abi.encode(addresses.quoteToken, params.name))}(
            ...
            emit VaultCreated(indexVault, msg.sender);
            emit VaultCreated(quoteVault, msg.sender);
        } else {
            quoteVault = address(
                new Vault{salt: keccak256(abi.encode(addresses.quoteToken))}(
                    addresses.quoteToken,
                    address(engine),
                    params.name
                )
        ...
        return
            engine.addMarket(
                MarketAddresses({
                    ...
                    indexToken: addresses.indexToken,
                    vault: indexVault,
                    quoteToken: addresses.quoteToken,
                    quoteVault: quoteVault,
                    priceFeed: addresses.priceFeed,
                    fundingRateModel: addresses.fundingRateModel
                }),
                params
            );
    }
...
contract Vault is ERC20, ReentrancyGuard, Pausable, Ownable {
    ...
    event VaultUpdated(bool hasProfit, uint256 pnl, uint256 fee);
    ...
    constructor(
        ...
        ERC20(
            ...
            string(abi.encodePacked("v", IERC20Metadata(_asset).symbol(), "-", _marketName))
        )
    {
        asset = _asset;
    ...
    function updateVault(bool hasProfit, uint256 pnl, uint256 fee) external onlyTradingEngine {
        if (!hasProfit) {
            realizedLoss += pnl;
        } else {
            realizedProfit += pnl;
        ...
        // realizedProfit += fee;
        // emit VaultUpdated(hasProfit, pnl, fee);

        if (!hasProfit) {
            // check surpass threshold
            distributePnl();
        }
    }
    ...
    function deposit(
        ...
        tradingEngine.updateVaultBalance(address(this), _amount, true);

        emit Deposit(_receiver, _amount);
        emit BalanceUpdated(vaultBalance);
    ...
    function withdraw(
        ...
        tradingEngine.updateVaultBalance(address(this), amount, false);
    ...
    function _redeem(uint256 _shares, address _receiver) internal returns (uint256) {
        ...
        // TODO: why liquidity is not updated after withdraw
        // this line of code was just added
        liquidity -= assets;
        vaultBalance -= assets;
        // transfer assets
        emit Withdraw(_receiver, assets);
        emit BalanceUpdated(vaultBalance);
        return assets;
    }
    ...
    function distributePnl() public {
        ...
        if (pnl > 0) {
            if (hasProfit) {
                // mint new shares to lock the profit
                // asset amount increases, shares amount should increase to keep pps the same
                newLockedShares = _issueSharesForAmount(pnl, address(this));
            } else {
                // if there is no profit, we need to burn shares so that the pps is reflected immediately
                sharesToBurn = _convertToShares(pnl, Math.Rounding.Up);
            }
        }

        uint256 prevLockedShares = balanceOf(address(this)) - newLockedShares;

        // Now that price per share has updated, we can burn the shares we intended to burn as a result of losses/fees.
        ...
        if (sharesToBurn > 0) {
            // can only burn maximum number of shares this contract has
            sharesToBurn = Math.min(sharesToBurn, prevLockedShares + newLockedShares);
            _burnShares(sharesToBurn, address(this));

            // deduct the shares that have been burned from the locked shares
            // if the shares to burn is greater than the new locked shares, the difference will be taken from the prev locked shares
            uint256 deducted = Math.min(sharesToBurn, newLockedShares);
            newLockedShares -= deducted;
            prevLockedShares -= (sharesToBurn - deducted);
        }

        uint256 remainLockedShares = prevLockedShares + newLockedShares;

        if (remainLockedShares > 0) {
            uint256 remainingTime;
            if (_fullProfitUnlockDate > block.timestamp) {
                remainingTime = _fullProfitUnlockDate - block.timestamp;
            }

            uint256 newUnlockPeriod = (prevLockedShares *
                remainingTime +
                newLockedShares *
                profitMaxUnlockTime) / remainLockedShares;

            profitUnlockingRate = (remainLockedShares * MAX_BPS_EXTENDED) / newUnlockPeriod;
    ...
    function getAmountInAndUpdateVaultBalance() external onlyTradingEngine returns (uint256) {
        ...
        if (amountIn > 0) {
            tradingEngine.updateVaultBalance(address(this), amountIn, true);
    ...
    function payout(
        ...
        address _receiver
    ) external nonReentrant onlyTradingEngine {
...
import {MarketParams} from "../interfaces/IMarket.sol";
