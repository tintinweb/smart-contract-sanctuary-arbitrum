...
contract Treasury is Ownable, Pausable
{
    ...
    {
        uint256 amountTokenIn = PacketsVeGRB[indexPacket];
        require(amountTokenIn != 0, "Error BuyPacket: Invalid packet");
        address user = _msgSender();
        require(CyberCreditToken.balanceOf(user) >= amountTokenIn, "Error BuyPacket: Invalid Balance CyberCredit");

        CyberCreditToken.transferFrom(user, address(this), amountTokenIn);

        if(veGRBToken.balanceOf(address(this)) <= amountTokenIn)
        {
            veGRBToken.transfer(user, veGRBToken.balanceOf(address(this)));
            return;
        }
        veGRBToken.transfer(user, amountTokenIn);
    }
