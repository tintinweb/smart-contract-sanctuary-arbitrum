...
import "../Renown/ActivityRenown.sol";
...
struct CraftingContractCreationRequest {
  ...
  address PRODUCTIVITY_STORAGE;
  address RANDOM_PICKER;
  address AOV;
}
...
struct Recipe {
  bool enabled;
  RecipeRequirement[] requirements;
...
contract Crafting is ICrafting, ManagerModifier, ReentrancyGuard, Pausable {
  ...
  error InvalidMaterial(uint recipeId, address materialAddress, uint materialId);
  ...
  // Immutables
  //=======================================
  IActivityRenown public immutable ACTIVITY_RENOWN;
  ...
  IProductivity public immutable PRODUCTIVITY_STORAGE;
  ...
  DynamicCraftingConfigCache dynamicConfigCache;
  ...
  mapping(address => mapping(uint256 => uint256)) public materialGroupConfig;
  ...
  // Constructor
  //=======================================
  constructor(CraftingContractCreationRequest memory _request) ManagerModifier(_request.MANAGER) {
    ACTION_ID_STORAGE = IGlobalActionIdStorage(_request.ACTION_ID_STORAGE);
    ACTION_PERMIT = IActionPermit(_request.ACTION_PERMIT);
    ACTIVITY_RENOWN = IActivityRenown(_request.ACTIVITY_RENOWN);
    ACTION_DEMAND_STORAGE = IActionDemandAverageStorage(_request.ACTION_DEMAND_STORAGE);
    ...
    ADVENTURER_DATA = IAdventurerData(_request.ADVENTURER_DATA);
    ADVENTURER_ARMORY = IArmory(_request.ADVENTURER_ARMORY);
    ANIMA = Anima(_request.ANIMA);
    RESOURCE = IResource(_request.RESOURCE);
    CRAFTING_CONFIG_PROVIDER = ICraftingDynamicConfigProvider(_request.CRAFTING_CONFIG_PROVIDER);
    PRODUCTIVITY_STORAGE = IProductivity(_request.PRODUCTIVITY_STORAGE);
    RANDOM_PICKER = IRandomPicker(_request.RANDOM_PICKER);
    AOV = _request.AOV;
  ...
  function craft(
    ...
    require(msg.sender == tx.origin, "Crafting is not allowed through another contract");
    if (gasleft() < (_realmRequests.totalCrafts + _voidRequests.totalCrafts) * minimumGasPerCraft) {
      revert GasTooLow(
        gasleft(),
        (_realmRequests.totalCrafts + _voidRequests.totalCrafts) * minimumGasPerCraft
      );
    }
  ...
  function maximumRealmCraftsAvailable() public view returns (uint[] memory) {
    return _maximumCraftAmounts(dynamicConfigCache);
  }

  function craftingCostsView() external view returns (CraftingCost[] memory result) {
    result = new CraftingCost[](supportedLabIds.length);

    for (uint i = 0; i < result.length; i++) {
      result[i].realm = labConfig[supportedLabIds[i]].baseAnimaCost;
      result[i].void =
        (result[i].realm * uint(dynamicConfigCache.voidCraftingCostPercentage[i])) /
        ONE_HUNDRED;
    }
  }
  ...
  function craftingCosts() external returns (CraftingCost[] memory result) {
    ...
    for (uint i = 0; i < result.length; i++) {
      result[i].realm = labConfig[supportedLabIds[i]].baseAnimaCost;
      result[i].void =
        (result[i].realm * uint(_getDynamicConfig().voidCraftingCostPercentage[i])) /
        ONE_HUNDRED;
    }
  }
  ...
  function _affectRealms(uint[] memory realmCrafts, uint randomBase) internal {
    ...
    (uint[][] memory realmIds, ) = RANDOM_PICKER.useRandomizerBatch(
      uint256(uint160(LAB_ADDRESS)),
      supportedLabIds,
      realmCrafts,
      randomBase
    );

    address[] memory realmAddresses;
    uint[][] memory labIds;
    for (uint i = 0; i < supportedLabIds.length; i++) {
      if (realmCrafts[i] == 0) {
        continue;
      }
      realmAddresses = ArrayUtils.toMemoryArray(REALM_ADDRESS, realmCrafts[i]);
      labIds = new uint[][](realmCrafts[i]);
      for (uint j = 0; j < realmIds[i].length; j++) {
        labIds[i] = ArrayUtils.toMemoryArray(supportedLabIds[i], 1);
      }

      REALM_ARMORY.reduceDurabilityBatch(
        realmAddresses,
        realmIds[i],
        LAB_ADDRESS,
        labIds,
        dynamicConfigCache.durabilityLossPercentage[i],
        true,
        true
      );

      PRODUCTIVITY_STORAGE.changeBatch(
        realmIds[i],
        int(uint(dynamicConfigCache.productivityPerCraft[i])),
        true
      );

      uint animaCapacity = labConfig[supportedLabIds[i]].animaCapacity;
      for (uint j = 0; j < realmIds[i].length; j++) {
        RESOURCE.add(realmIds[i][j], resources.ANIMA_CAPACITY, animaCapacity);
      }
    }
  }
  ...
  function _handleCrafting(
    ...
    uint _action
  ) internal returns (uint256 totalAnima, uint[] memory totalCrafts, uint randomBase) {
    ...
    uint[] memory levels = new uint[](_requests.adventurerAddresses.length);
    for (uint i = 0; i < levels.length; i++) {
      levels[i] = ADVENTURER_DATA.aov(
    ...
    (totalCrafts, craftingCostAdjustments) = _verifyMaterials(_requests, levels);
    uint craftsCount;
    ...
    ACTION_DEMAND_STORAGE.increaseDemandBatch(_action, supportedLabIds, totalCrafts);
  }

  function _dispense(uint _action, CraftingRequests calldata _requests) internal returns (uint) {
    uint craftId = ACTION_ID_STORAGE.getAndIncrementNextId(_action, _requests.totalCrafts);
  ...
  function _totals(
    ...
    for (uint i = 0; i < supportedLabIds.length; i++) {
      ...
      animaCosts =
        (config.baseAnimaCost * (craftingCostAdjustments[i]) * uint(animaMultipliers[i])) /
        ONE_HUNDRED;
  ...
  function _verifyLabRequirementsAndIncrementCounts(
    ...
    if (labConfig[labId].minimumLevel > adventurerLevel) {
      revert AdventurerLevelTooLow(labConfig[labId].minimumLevel, adventurerLevel);
    }
  ...
  function _verifyMaterialsAreCorrectTypeAndAmount(
    ...
    for (uint i = 0; i < _materialIdIndexes.length; i++) {
      ...
      mem.materialId = _materialIds[_materialAddressIndexes[i]][_adventurerIndex][
      ...
      if (_requirement.requirementType == RecipeRequirementType.EXACT_TOKEN) {
        if (
          ...
          revert InvalidMaterial(_recipeId, mem.materialAddress, mem.materialId);
        }
      } else if (_requirement.requirementType == RecipeRequirementType.MATERIAL_GROUP) {
        if (
          _requirement.requirementArgument !=
          materialGroupConfig[mem.materialAddress][mem.materialId]
        ) {
          revert InvalidMaterial(_recipeId, mem.materialAddress, mem.materialId);
        }
      } else {
      ...
      mem.submittedMaterialsAmount += _materialAmounts[i];
      _totalAmounts[_materialAddressIndexes[i]][_adventurerIndex][
        _materialIdIndexes[i]
      ] += _materialAmounts[i];
    }

    if (_requirement.tokenAmount * _recipeAmount != mem.submittedMaterialsAmount) {
  ...
  function _maximumCraftAmounts(
    ...
    uint[] memory availableLabs = RANDOM_PICKER.getQueueSizes(
      uint256(uint160(LAB_ADDRESS)),
      supportedLabIds
    );
    result = new uint[](availableLabs.length);
    for (uint i = 0; i < supportedLabIds.length; i++) {
      uint durabilityLoss = config.durabilityLossPercentage[i];
      if (durabilityLoss < 1) {
        durabilityLoss = 1;
      }
      if (durabilityLoss > ONE_HUNDRED) {
        durabilityLoss = ONE_HUNDRED;
      }
      result[i] = (ONE_HUNDRED * availableLabs[i]) / durabilityLoss;
    }
  }

  function _getDynamicConfig() internal returns (DynamicCraftingConfigCache storage) {
    uint256 currentEpoch = Epoch.packTimestampToEpoch(block.timestamp, cacheEpochConfig);
    DynamicCraftingConfigCache storage cache = dynamicConfigCache;
    ...
    DynamicLabCraftingConfig[] memory newConfigs = CRAFTING_CONFIG_PROVIDER.getConfigBatch(
      supportedLabIds
    );
    ...
    for (uint i = 0; i < newConfigs.length; i++) {
      cache.durabilityLossPercentage[i] = uint32(newConfigs[i].durabilityLossPercentage);
      cache.productivityPerCraft[i] = uint32(newConfigs[i].productivityPerCraft);
      cache.voidCraftingCostPercentage[i] = uint32(newConfigs[i].voidCraftingCostPercentage);
      cache.realmCraftingCostPercentage[i] = uint32(newConfigs[i].realmCraftingCostPercentage);
    }
  ...
  function _calculateCraftingCost(
    ...
    return
      ((((baseCost * curveCostConfig.labSoftCap) / (curveCostConfig.labSoftCap + labCount - 1)) *
        curveCostConfig.range) + curveCostConfig.min) / ONE_HUNDRED;
  }
  ...
  function clearCache() external onlyAdmin {
    dynamicConfigCache.epoch = 0;
  }

  function configureMaterialGroup(
    address _materialAddress,
    uint[] calldata _materialTokenIds,
    uint[] calldata _materialGroup
  ) external onlyAdmin {
    for (uint i = 0; i < _materialTokenIds.length; i++) {
      materialGroupConfig[_materialAddress][_materialTokenIds[i]] = _materialGroup[i];
    }
  }
  ...
  function updateRecipes(
    ...
    bool disableExistingRecipes
  ) external onlyAdmin {
    if (disableExistingRecipes) {
