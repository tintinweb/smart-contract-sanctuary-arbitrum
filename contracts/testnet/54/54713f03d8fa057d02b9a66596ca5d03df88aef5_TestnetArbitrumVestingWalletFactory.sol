...
import "../test/testnet-contracts/TestnetArbitrumVestingWallet.sol";

contract TestnetArbitrumVestingWalletFactory {
    ...
    function createWallets(
        ...
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            ArbitrumVestingWallet wallet = new TestnetArbitrumVestingWallet(
                _beneficiaries[i],
                _startTimestamp,
                _durationSeconds
            );
...
contract TestnetArbitrumVestingWallet is ArbitrumVestingWallet {
    uint256 constant SECONDS_PER_THREE_MINUTES = 60 * 3;

      constructor(address _beneficiaryAddress, uint64 _startTimestamp, uint64 _durationSeconds)
        ArbitrumVestingWallet(_beneficiaryAddress, _startTimestamp, _durationSeconds)
    {}
        ...
        returns (uint256)
    {
        ...
        if (timestamp < start()) {
            ...
            uint256 vestedTimeSeconds = timestamp - start();
            uint256 vestedTimeSecondsMonthFloored =
                vestedTimeSeconds - (vestedTimeSeconds % SECONDS_PER_THREE_MINUTES);
            uint256 remaining =
                ((totalAllocation - cliff) * (vestedTimeSecondsMonthFloored)) / duration();
