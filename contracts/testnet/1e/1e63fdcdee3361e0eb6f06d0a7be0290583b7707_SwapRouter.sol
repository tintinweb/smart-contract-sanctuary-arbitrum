...
import { SwapLibrary, ISwapPair } from "./libraries/SwapLibrary.sol";
...
abstract contract SupportingSwap is FeeStore, ISwapRouter {
    ...
    function swapExactTokensForTokens(
        ...
        address pair = SwapLibrary.pairFor(path[0], path[1]);

        address feeTaker = ISwapPair(pair).feeTaker();
        ...
        amounts = SwapLibrary.amountsOut(amountIn, path, totalFee, adminFee);
    ...
    function swapTokensForExactTokens(
        ...
        if(path[0] == pairFeeAddress[pair]) {
            ...
            amounts = SwapLibrary.amountsIn(amountOut, path, totalFee, adminFee);
            require(amounts[0] <= amountInMax, "SwapRouter: EXCESSIVE_INPUT_AMOUNT");
            (, adminFeeDeduct) = SwapLibrary.adminFeeCalculation(amounts[0], adminFee);
            TransferHelper.safeTransferFrom(
                path[0], msg.sender, adminFeeAddress, adminFeeDeduct
            );
    ...
    returns (uint[] memory amounts)
    {
        ...
        if(path[path.length - 1] == pairFeeAddress[pair]){
            (amountETHOut, adminFeeDeduct) = SwapLibrary.adminFeeCalculation(amountETHOut, adminFee);
        }
        if(totalFee > 0) {
    ...
    returns (uint[] memory amounts)
    {
        ...
        if(path[0] == pairFeeAddress[pair]){
            ...
            assert(IWETH(WETH).transfer(SwapLibrary.pairFor(path[0], path[1]), amounts[0]));
        }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if(path[path.length - 1] == pairFeeAddress[pair]){
            (amountOutMin,adminFeeDeduct) = SwapLibrary.adminFeeCalculation(amountOutMin, adminFee);
        }
        require(
    ...
    function _swap(address _feeCheck, uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (uint amount0Fee, uint amount1Fee, uint _amount0Out, uint _amount1Out) = SwapLibrary._calculateFees(_feeCheck, input, output, amountsI, input == token0 ? uint(0) : amountOut, input != token0 ? amountOut : uint(0));
            address to = i < _path.length - 2 ? SwapLibrary.pairFor(output, finalPath) : _to;
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            (uint amountInput, uint amountOutput) = 
                SwapLibrary._calculateAmounts(_feeCheck, input, output, token0);
            (uint amount0Out, uint amount1Out) = input == token0 ? 
                (uint(0), amountOutput) : (amountOutput, uint(0));

            (uint amount0Fee, uint amount1Fee, uint _amount0Out, uint _amount1Out) = 
                SwapLibrary._calculateFees(_feeCheck, input, output, amountInput, amount0Out, amount1Out);
...
library SwapLibrary {
    ...
    function pairFor(address tokenA, address tokenB) internal view returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                abi.encodePacked(IFactory(factory).INIT_CODE_PAIR_HASH(), abi.encode(IFactory(factory).PAIR_LOGIC(), factory, token0, token1))
            )))));
    }
