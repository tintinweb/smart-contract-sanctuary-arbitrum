...
contract RRToken is BEP20('RR', 'RR'), Pausable, IRR, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier onlyAuthorizedUnlockedMintCaller() {
        require(_msgSender() == owner() || _authorizedUnlockedMintCaller[_msgSender()],"RRP: UNLOCKED_MINT_CALLER_NOT_AUTHORIZED");
        _;
    }
    
    modifier onlyAuthorizedLockedMintCaller() {
        require(_msgSender() == owner() || _authorizedUnlockedMintCaller[_msgSender()] || _authorizedLockedMintCaller[_msgSender()],"RRP: LOCKED_MINT_CALLER_NOT_AUTHORIZED");
        _;
    }
    ...
    function stake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(_unlockFactor[token] > 0, "RR: FACTOR_NOT_SET");
        require(_unlockBlockGap[token] > 0, "RR: BLOCK_GAP_NOT_SET");
    ...
    function unstake(address token, uint256 amount) external override nonReentrant returns (bool) {
        require(amount > 0, "RR: ZERO_UNSTAKE_AMOUNT");
        LpStakeInfo storage info = _stakingRecords[_msgSender()][token];
        require(amount <= info.amountStaked, "RR: UNSTAKE_AMOUNT_EXCEEDED");
        uint256 unlockedAmount = _settleUnlockAmount(_msgSender(), token, info.amountStaked, info.blockNumber);
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "RR::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "RR::delegateBySig: invalid nonce");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "RR::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "RR::_writeCheckpoint: block number exceeds 32 bits");
