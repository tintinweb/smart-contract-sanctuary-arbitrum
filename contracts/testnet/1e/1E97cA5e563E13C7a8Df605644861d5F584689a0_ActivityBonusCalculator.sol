...
contract ActivityBonusCalculator is IAdventurerRenownBonusCalculator, ManagerModifier {
  ...
  function calculateBonus(
    ...
    uint boostMulitplier = _boostMultiplier(_sender);
    return _renownBase(boostMulitplier) * int(level);
  }
  ...
  function calculateBonusBatch(
    ...
    int renownPerLevel = _renownBase(_boostMultiplier(_sender));
  ...
  function _boostMultiplier(address _sender) internal view returns (uint) {
    return (ONE_HUNDRED * BATTLE_BOOST.getAnimaBase(_sender)) / BASE_AMOUNT;
  }
