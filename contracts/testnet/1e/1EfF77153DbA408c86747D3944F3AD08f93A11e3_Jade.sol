...
// File @openzeppelin/contracts/token/ERC20/ERC20[email protected]
...
contract Jade is ERC20Burnable, Ownable {
    using SafeMath for uint256;
    ...
    ) public ERC20("JadeSwap", "JADE") {
        require(
            _initialSupply <= _maxSupply,
            "Jade: The _initialSupply should not exceed the _maxSupply"
        );
    ...
    modifier onlyDev() {
        require(
            ...
            "Jade: You don't have the permission!"
        );
        _;
    }

    modifier hasMinterRole() {
        require(isMinter[_msgSender()], "Jade: You don't have the permission!");
        _;
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyDev {
        require(
            ...
            "Jade: The router already has that address"
        );
    ...
    function mint(address _user, uint256 _amount) external hasMinterRole {
        ...
        require(
            _totalSupply.add(_amount) <= MAX_SUPPLY,
            "Jade: No more minting allowed!"
        );
    ...
    function updateFee(uint256 feeType, uint256 fee) external onlyDev {
        require(fee <= 900, "Jade: The tax Fee cannot exceed 9%");
    ...
    function recoverToken(address _token) external onlyDev {
        ...
        require(
            tokenBalance > 0,
            "Jade: The contract doen't have tokens to be recovered!"
        );
