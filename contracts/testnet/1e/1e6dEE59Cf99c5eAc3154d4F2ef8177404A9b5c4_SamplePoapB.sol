    ...
    ContractVersion
{
    ...
    /// Storage
    mapping (uint256 => uint256) public poapId;
    mapping (uint256 => string) public poapURI;
    ...
    function initialize(
        ...
        // Init contracts
        __ERC721A_init("Sample POAP B", "SPOAPB");
        __Context_init();
    ...
    /// Mint
    function mint(address _to, uint256 _poapId) external virtual onlyRole(MINTER_ROLE) whenNotPaused {
        if (bytes(poapURI[_poapId]).length == 0) revert PoapURIDoesNotExist();

        uint256 tokenId = _nextTokenId();
        poapId[tokenId] = _poapId;
        _mint(_to, 1);

        emit PoapMinted(_to, _poapId, tokenId);
    }

    /// Airdrop
    function airdrop(address[] calldata _receivers, uint256 _poapId) external virtual onlyRole(MINTER_ROLE) whenNotPaused {
        uint256 _startFromTokenId = _nextTokenId();
        uint256 receiversLength = _receivers.length;

        for (uint256 i = 0; i < receiversLength;) {
            address receivers_ = _receivers[i];
        ...
        emit PoapDropped(_receivers, _poapId, _startFromTokenId, receiversLength);
    }

    /// Burn
    function burn(address _tokenOwner, uint256 _tokenId) external virtual onlyRole(BURNER_ROLE) whenNotPaused {
       if (ownerOf(_tokenId) != _tokenOwner) revert InvalidTokenOwner();

        uint256 _poapId = poapId[_tokenId];
        delete poapId[_tokenId];
        
        _burn(_tokenId);

        emit PoapBurned(_tokenOwner, _poapId, _tokenId);
    }

    /// Set poap URI
    function setPoapURI(uint256 _poapId, string memory _poapURI) external virtual onlyRole(ADMIN_ROLE) whenNotPaused {
        if (bytes(_poapURI).length == 0) revert InvalidPoapURI();
        poapURI[_poapId] = _poapURI;
        
        emit PoapURIUpdated (_poapId, _poapURI);
    }

    /// Set royalty info
    function setRoyaltyInfo(address _royaltyReceiver, uint96 _feeBasisPoints) external virtual onlyRole(ADMIN_ROLE) whenNotPaused {
        _setDefaultRoyalty(_royaltyReceiver, _feeBasisPoints);
        emit RoyaltyInfoUpdated(_royaltyReceiver, _feeBasisPoints);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        uint256 _poapId = poapId[tokenId];
        string memory _poapURI = poapURI[_poapId];
    ...
    /// See {ERC2981Upgradeable-_feeDenominator}
