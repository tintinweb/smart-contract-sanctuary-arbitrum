...
contract ReceiverToken is ERC1155, Ownable {
  ...
  function receiverMint(uint _tokenId, uint _amount) external {
    require(_exists[_tokenId], "token does not exist, mint sender token first");

    _mint(msg.sender, _tokenId, _amount, new bytes(0));
    emit ReceiverMint(msg.sender, _tokenId, _amount, block.timestamp);

    tokenSupply[_tokenId] = tokenSupply[_tokenId] + _amount;
  }

  function receiverChannelMint(uint _tokenId, uint _amount) external {
    require(_exists[_tokenId], "token does not exist, mint sender token first");
    ...
    if (!channelToken._gated) {
      _mint(msg.sender, _tokenId, _amount, new bytes(0));
      emit ReceiverChannelMint(msg.sender, _tokenId, _amount, block.timestamp);
    }
  ...
  function receiverInitialMint(address to, uint tokenId, uint amount) external onlySenderorChannelContract {
  ...
  function receiverChannelInitialMint(address _to, uint _tokenId, uint _amount) external onlySenderorChannelContract {
    ...
    emit ReceiverMint(_to, _tokenId, 1, block.timestamp);
  }
  ...
  //       // encode the payload with the receiving tokenId and contract addr
  //       bytes memory payload = abi.encode(msg.sender, _tokenId, _gated_contract);
