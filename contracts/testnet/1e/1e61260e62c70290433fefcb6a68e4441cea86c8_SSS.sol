...
interface IERC721 is IERC165 {
    /**
* @dev Emitted when `tokenId` token is transferred from `from` to
`to`.
*/
    event Transfer(
...
first that contract recipients
* are aware of the ERC721 protocol to prevent tokens from being
forever locked.
** Requirements:
...
interface IERC721Enumerable is IERC721 {
    ...
    /**
* @dev Returns a token ID owned by `owner` at a given `index` of its
token list.
...
** Contracts that want to implement ERC165 should inherit from this
contract and override {supportsInterface} to check
* for the additional interface id that will be supported. For example:
** ```solidity
* function supportsInterface(bytes4 interfaceId) public view virtual
override returns (bool) { * return interfaceId == type(MyInterface).interfaceId ||
...
*/
    function toString(uint256 value) internal pure returns (string memory) {
        ...
        //
        // https://github.com/oraclize/ethereum-api/blob/b42146b063c7d6ee1358846c1982
        // 46239e9360e8/oraclizeAPI_0.4.25.sol
        if (value == 0) {
            return "0";
        }
        uint256 temp = value;
...
**
https://diligence.consensys.net/posts/2019/09/stop-using-soliditys-transfe
r-now/[Learn more].
...
*
https://solidity.readthedocs.io/en/v0.5.11/security-considerations.html#us
e-the-checks-effects-interactions-pattern[checks-effects-interactions
pattern].
*/
...
*/
    ...
    /**
* @dev Same as
{xref-Address-functionCallWithValue-address-bytes-uint256-}[`functionCallW
ithValue`], but
...
* gas optimizations e.g. when performing a transfer operation
(avoiding double writes).
* This has O(1) time complexity, but alters the order of the
_ownedTokens array.
* @param from address representing the previous owner of the given
token ID
* @param tokenId uint256 ID of the token to be removed from the
tokens list of the given address
*/
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId)
        private
    {
        // To prevent a gap in from's tokens array, we store the last
        // token in the index of the token to delete, and
...
contract SSS is ERC721Enumerable, Ownable {
    ...
    uint256 public maxMintAmount = 163;
    ...
    function withdraw() public payable onlyOwner {
        // This will payout the owner of the contract the balance.
