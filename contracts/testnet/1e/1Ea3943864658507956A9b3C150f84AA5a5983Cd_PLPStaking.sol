...
contract PLPStaking is Initializable {
    ...
    function stake(uint _amount) external {
        ...
        emit UserEvent(msg.sender, "stakePLP", _amount);
    }

    function unStake(uint _amount) external {
        PLPStakingVar.unStake(_amount);
        emit UserEvent(msg.sender, "unStakePLP", _amount);
    }

    function withdraw() external {
        PLPStakingVar.withdraw();
        emit UserEvent(msg.sender, "withdraw", 0);
    }

    function claim() external {
        PLPStakingVar.claim();
        emit UserEvent(msg.sender, "claim", 0);
    }
}