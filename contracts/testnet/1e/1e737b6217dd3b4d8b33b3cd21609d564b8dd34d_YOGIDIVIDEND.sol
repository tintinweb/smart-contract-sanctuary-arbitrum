...
contract YOGIDIVIDEND is IREWARD {   
    ...
    constructor() {
        router = IRouter(0x4648a43B2C14Da09FdF82B161150d3F634f40491);
        _owner = msg.sender;
    }
    ...
    function rescueToken(address tokenAddress,address _receiver, uint256 tokens) external onlyOwner {
        S_IERC20(tokenAddress).transfer(_receiver, tokens);
    }
    ...
    function claimChoice(address ctoken) external {
        ...
        if(subtotal > 0) {
            ...
            router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: subtotal}(
                0,
                path,
                address(user),
                block.timestamp
