...
import { IHelixLoan } from '../interfaces/IHelixLoan.sol';
...
/**
 * @title HelixLoan
 * @notice Helix Loan contract is the main contract for most borrower/investor interactions such as investment, update repayment Data, TVL.
 * @author Helix
 */

contract HelixLoan is HelixBaseUpgradeablePausable, EIP712Upgradeable, IHelixLoan, IVersioned {
  ...
  event RepaymentReceiptsConfirmed(
    ...
    uint256 redeemedInterest,
    uint256 previousTVL,
    uint256 updatedTVL
  );

  event LoanStatusUpdated(bool status);
  ...
  string public constant name = 'Helix Loan';
  string public constant version = '1';

  // bytes32 public constant INDICATIVE_INTEREST_TYPEHASH = keccak256("IndicativeInterest(address investor,address loan,uint256 investmentDate,uint256 amount,uint256 salt,uint256 expirationTime)");
  bytes32 public constant INDICATIVE_INTEREST_TYPEHASH =
    0xbf57fad2f73584a45ce800520b036ee4d9bdedb4ec65585c954d2c8db2b3a6fd;

  /// @notice Get HelixConfig contract's address
  IHelixConfig public config;
  /// @notice Loan Manager - Gnosis 
  ...
  /// @notice Check whether the loan is open or not
  bool public closed;
   ...
   */
  function initialize(
    // config - manager wallet - loan wallet - token - borrower wallet
    ...
    loanManager = _addresses[1];
    loanWallet = _addresses[2];
    ...
    __BaseUpgradeablePausable__init(loanManager);
    __EIP712_init(name, version);
  }

  /// @notice Open or close loan
  /// @dev nly be called by loan manager
  /// @param status True if loan manager want close loan and vice versa
  function toggleLoan(bool status) public onlyManager {
    require(closed != status, HelixErrors.UPDATED_LOAN_STATUS_IS_THE_SAME);
    closed = status;
    emit LoanStatusUpdated(status);
  }

  /// @notice Change loan wallet address
  /// @dev Only be called by loan manager
  /// @param _loanWallet New loan wallet address
  function changeLoanWallet(
    address _loanWallet
  ) external onlyManager {
    require(_loanWallet != address(0), HelixErrors.NOT_ALLOWED_ZERO_ADDRESS);
    address previousLoanWallet = loanWallet;
    loanWallet = _loanWallet;
    emit LoanWalletUpdated(previousLoanWallet, _loanWallet);
  }

  /**
   * @notice Update verified repayment receipts to update TVL and log emitting for more transparency
   * @dev This function will update the TVL, update Helix Loan tokens NFT details. And the most import thing
   * is emitting log for more transparency.
   * This function can only be called Protocol Admin (Not loan manager)
   * @param txs An array of Repayment Transaction Objects where each object includes: 
   *  RepaymentTransactionType txType; -> 0: Principal, 1: Interest
      bytes32 txHash; -> Repayment Transaction Hash
      address investor; -> Investor address
      uint256 principal; -> Repaid principal amount
      uint256 interest; -> Repaid interest amount
   */
  function updateRepaymentReceipts(RepaymentTransaction[] calldata txs) public onlyProtocolAdmin {
    IHelixLoanTokens poolTokens = config.getHelixLoanTokens();
    for (uint i = 0; i < txs.length; ) {
      RepaymentTransaction memory repaymentTx = txs[i];

      uint principalAmount = repaymentTx.principal;
      uint interestAmount = repaymentTx.interest;
      ...
      // Get TVL must happened before calling redeem function as it will got updated
      uint256 oldTVL = getTVL();

      uint tokenId = poolTokens.calculateTokenId(address(this), investor);
      poolTokens.redeem(tokenId, principalAmount, interestAmount);
  ...
  function invest(
    ...
    require(hasAllowedUID(msg.sender), HelixErrors.NOT_SATISFY_KYC_CONDITION_FOR_LOAN);
    ...
    InvestmentStatus status = investments[_msgSender()][investmentHash].status;

    bool investmentApprovedByManager = false;

    if (status == InvestmentStatus.NotExisted) {
      ...
      investmentApprovedByManager = true;
    }

    if (!investmentApprovedByManager) {
      require(
        investments[_msgSender()][investmentHash].status == InvestmentStatus.Pending,
        HelixErrors.INVALID_INVESTMENT_PLACEHOLDER
      );
    }
    
    // Mint pool token if not exists and update principal amount
    IHelixLoanTokens loanTokens = config.getHelixLoanTokens();
    loanTokens.mint(
      IHelixLoanTokens.MintParams({
        principalAmount: amount
      }),
      _msgSender()
    );

    token.safeTransferFrom(_msgSender(), loanWallet, amount);
    ...
    emit Invested(_msgSender(), address(this), investmentDate, amount, salt, expirationTime, loanWallet, investmentHash);
  }

  /// @notice Investor invests token in the loan without executing approve transaction
  /// @param investmentDate The subscription date of the investments
  /// @param amount The amount that investor will need to deposit
  /// @param investSignature The signature of Manager to allow sender to invest
  ...
  /// @notice Manager cancel an investments of an investor
  ...
  function getTVL() public view returns(uint256) {
    return config.getHelixLoanTokens().loanTVL(address(this));
  }
  ...
  function getInvestmentHash(
   ...
   function encodeInvestmentData(
    ...
    return abi.encodePacked(bytes1(0x19), bytes1(0x01), IGnosisSafe(loanManager).domainSeparator(), investmentHash);
  }
  ...
  function _allowInvestByOnlyManager(
    ...
    IGnosisSafe(loanManager).checkSignatures(
      getInvestmentHash(
        investor, 
        investmentDate, 
        amount, 
  ...
  modifier isNotClosed() {
    require(!closed, HelixErrors.LOAN_NOT_CLOSED);
    _;
  }
}
