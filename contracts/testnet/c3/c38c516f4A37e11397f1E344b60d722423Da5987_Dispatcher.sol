...
contract Dispatcher is IDispatcher, Initializable, DAOAccessControlled {
    ...
    function setCollectionManager(address _collectionManager) external onlyGovernor {        
        collectionManager = _collectionManager;
        emit Loot8CollectionMangerSet(_collectionManager);        
    }

    function addOfferWithContext(address _offer, uint256 _maxPurchase, uint256 _expiry) external {
        
        require(msg.sender == collectionManager, 'UNAUTHORIZED');
    ...
    */
    ...
    returns(uint256 newReservationId) {
        ...
        require(ICollectionManager(collectionManager).checkCollectionActive(_offer), "OFFER IS NOT ACTIVE");
        ...
        address _entity = ICollectionManager(collectionManager).getCollectionData(_offer).entity;
    ...
    */
    function _fulfillReservation(uint256 _reservationId) internal {
        ...
        ICollectionManager(collectionManager).setCollectibleRedemption(offer, _offerId);

        uint256 rewards = ICollectionManager(collectionManager).calculateRewards(offer, 1);

        _mintRewards(_reservationId, rewards);

        _creditRewardsToPassport(_reservationId, rewards);

        address _entity = ICollectionManager(collectionManager).getCollectionData(offer).entity;
    ...
    function _getOfferPassport(address _offer) internal view returns(address) {
        ...
        address[] memory _collections = ICollectionManager(collectionManager).getAllLinkedCollections(_offer);

        for(uint256 i = 0; i < _collections.length; i++) {
            if(
                ICollectionManager(collectionManager).getCollectionType(_collections[i]) == 
    ...
    function _mintRewards(uint256 _reservationId, uint256 _rewards) internal {
        address offer = reservations[_reservationId].offer;
        address entity = ICollectionManager(collectionManager).getCollectionData(offer).entity;
    ...
    function _creditRewardsToPassport(uint256 _reservationId, uint256 _rewards) internal {
        ...
        if(_passport != address(0)) {
            ICollectionManager(collectionManager).creditRewards(_passport, reservations[_reservationId].patron, _rewards);
        }
    }
    ...
    function _mintAvailableCollectibles(address _patron) internal {
        ...
        address[] memory allLoot8Collections = ICollectionManager(collectionManager).getAllCollections(CollectionType.ANY, true);

        for(uint256 i = 0; i < allLoot8Collections.length; i++) {

            (string[] memory points, uint256 radius) = ICollectionManager(collectionManager).getLocationDetails(allLoot8Collections[i]);

            bool isActive = ICollectionManager(collectionManager).checkCollectionActive(allLoot8Collections[i]);
            ...
            if( 
                ...
                uint256 balance = IERC721(allLoot8Collections[i]).balanceOf(_patron);
                if(balance == 0) {
                    _mintCollectibles(allLoot8Collections[i], _patron);
                }
            }
        }
    }
    ...
    function mintLinkedCollectionsTokensForHolders(address _collection) external onlyForwarder {
        ...
        require(ICollectionManager(collectionManager).checkCollectionActive(_collection), "Collection is retired");
    ...
    function _mintCollectibles(address _collection, address _patron) internal returns (uint256 collectibleId) {
        collectibleId = ICollectionManager(collectionManager).mintCollectible(_patron, _collection);

        if (ICollectionManager(collectionManager).getCollectionType(_collection) == CollectionType.PASSPORT) {
    ...
    function mintLinked(address _collection, address _patron) public virtual override {
        ...
        require(
            ...
            _msgSender() == collectionManager || 
            isTrustedForwarder(msg.sender), "UNAUTHORIZED"
        );
        
        // Check if collection is active
        require(ICollectionManager(collectionManager).checkCollectionActive(_collection), "Collectible is retired");
        
        address[] memory linkedCollections = ICollectionManager(collectionManager).getAllLinkedCollections(_collection);

        for (uint256 i = 0; i < linkedCollections.length; i++) {
            if(
                IERC721(_collection).balanceOf(_patron) > 0 &&
                ICollectionManager(collectionManager).checkCollectionActive(linkedCollections[i]) &&
                ICollectionManager(collectionManager).getCollectionData(linkedCollections[i]).mintWithLinked &&
    ...
    function getActiveReservationsForEntity(address _entity) external view returns(Reservation[] memory _entityActiveReservations) {
        ...
        for(uint256 i = 0; i < offerList.length; i++) {
            ...
            if(ICollectionManager(collectionManager).getCollectionData(offer).entity == _entity) {
