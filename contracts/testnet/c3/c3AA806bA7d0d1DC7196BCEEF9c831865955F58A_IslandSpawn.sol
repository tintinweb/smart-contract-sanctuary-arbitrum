...
abstract contract Int256Component is BaseComponent {
    /** SETUP **/
     ...
     * @param entity Entity to get value for
     */
    function setValue(uint256 entity, int256 value) external virtual {
    ...
    function getValue(
        uint256 entity
    ) external view virtual returns (int256 value) {
        value = abi.decode(getBytes(entity), (int256));
    }
}
...
import {Int256Component} from "./Int256Component.sol";

uint256 constant ID = uint256(
    keccak256("game.piratenation.rotation1dcomponent")
);

/**
 * Stores the orientation of an entity (representing a physical object of some kind) in the scene.
 */
contract Rotation1DComponent is Int256Component {
    /** SETUP **/

    /** Sets the GameRegistry contract address for this contract  */
    constructor(
        address gameRegistryAddress
    ) Int256Component(gameRegistryAddress, ID) {
        // Do nothing
    }
}
...
uint256 constant ID = uint256(keccak256("game.piratenation.islandspawn"));
 ...
 */
contract IslandSpawn is OwnerSystem {
...
contract IslandSystem is SceneSystem {
    ...
    function _canAddOrUpdateSceneObject(
        ...
        if (objectParams.instanceEntity != 0) {
            ...
            uint256 instanceOwner = OwnerComponent(
                _gameRegistry.getComponent(OWNER_COMPONENT_ID)
            ).getValue(objectParams.instanceEntity);

            if (instanceOwner != sceneEntity) {
                return false;
            }
        } else {
    ...
    function _canRemoveSceneObject(
        ...
        return instanceOwner != sceneEntity;
    }
}
...
import {IS_PLACEABLE_TRAIT_ID, ITEM_SIZE_TRAIT_ID, GAME_LOGIC_CONTRACT_ROLE, GAME_ITEMS_CONTRACT_ROLE, GAME_NFT_CONTRACT_ROLE} from "../Constants.sol";
import {OwnerComponent, ID as OWNER_COMPONENT_ID} from "../core/components/OwnerComponent.sol";
import {Position3DComponent, ID as POSITION_3D_COMPONENT_ID} from "../core/components/Position3DComponent.sol";
import {Rotation1DComponent, ID as ROTATION_1D_COMPONENT_ID} from "../core/components/Rotation1DComponent.sol";
import {EntityLibrary} from "../core/EntityLibrary.sol";
import {GameRegistryConsumerUpgradeable} from "../GameRegistryConsumerUpgradeable.sol";
import {PlaceableSceneObjectComponent, ID as PLACEABLE_SCENE_OBJECT_COMPONENT_ID} from "./PlaceableSceneObjectComponent.sol";
...
struct PlaceableSceneObjectData {
    int256 posX;
    int256 posY;
    int256 posZ;
    int256 rotation;
...
contract PlaceableSceneObjectSystem is GameRegistryConsumerUpgradeable {
    ...
    {
        return _create(itemEntity, x, y, z, rotation);
    }
    ...
    function readPlacement(
        uint256 instanceEntity
    ) external view returns (PlaceableSceneObjectData memory data) {
        // Get Item Instance Specific Data
        (data.posX, data.posY, data.posZ) = Position3DComponent(
            _gameRegistry.getComponent(POSITION_3D_COMPONENT_ID)
        ).getValue(instanceEntity);

        data.rotation = Rotation1DComponent(
            _gameRegistry.getComponent(ROTATION_1D_COMPONENT_ID)
        ).getValue(instanceEntity);
    }
    ...
    function update(
        ...
        Position3DComponent(
            _gameRegistry.getComponent(POSITION_3D_COMPONENT_ID)
        ).setValue(instanceEntity, x, y, z);
        Rotation1DComponent(
            _gameRegistry.getComponent(ROTATION_1D_COMPONENT_ID)
        ).setValue(instanceEntity, rotation);
    }
    ...
    function remove(
        ...
        // TODO(Neil): Remove from owner?
        PlaceableSceneObjectComponent(
            _gameRegistry.getComponent(PLACEABLE_SCENE_OBJECT_COMPONENT_ID)
    ...
    function _create(
        ...
        PlaceableSceneObjectComponent(
            _gameRegistry.getComponent(PLACEABLE_SCENE_OBJECT_COMPONENT_ID)
        ).setValueFromToken(instanceEntity, tokenContract, tokenId);
        Position3DComponent(
            _gameRegistry.getComponent(POSITION_3D_COMPONENT_ID)
        ).setValue(instanceEntity, x, y, z);
        Rotation1DComponent(
    ...
    /**
     * @dev Returns true if the item is placeable, false otherwise.
     * @param tokenContract The contract address of the item.
     * @param tokenId The token id of the item.
     * @return x The size of the item along the x axis.
     * @return y The size of the item along the y axis.
     * @return z The size of the item along the z axis.
    ...
    //     ITraitsProvider traitsProvider = _traitsProvider();
    //     uint256[] memory values = traitsProvider.getTraitUint256Array(
    //         tokenContract,
    //         tokenId,
    //         ITEM_SIZE_TRAIT_ID
    //     );
...
abstract contract SceneSystem is OwnerSystem, ISceneSystem {
    ...
    function _removeSceneObject(
        ...
        _mintSceneObject(account, instanceEntity);
    }
