...
abstract contract BaseRouter is Pausable, EIP712, EndPoint, AccessControlEnumerable {
     ...
     */
    function registerComplexOp(ComplexOp[] memory complexOps_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 length = complexOps_.length;
        for (uint256 i; i < length; ++i) {
            uint256 operationsLength = complexOps_[i].operations.length;
            string memory complexOp;
            for (uint256 j; j < operationsLength; ++j) {
                complexOp = string.concat(complexOp, complexOps_[i].operations[j]);
            }
            bytes32 oop = keccak256(bytes(complexOp));
            ops[oop] = complexOps_[i].registered;
            emit ComplexOpSet(complexOp, oop, complexOps_[i].registered);
        }
    }
...
contract CryptoRouterV2 is RouterV2, ICryptoRouter {
    ...
    function _executeOp(
        ...
        if (result == ExecutionResult.Failed) {
            ...
            if (ADD_CRYPTO_CODE == op) {
                ...
                p.to = _checkTo(p.to, uint64(block.chainid), nextOp);
                maskedParams.amountOut = _addCrypto(p);
                ...
                (p.amountIn, p.from, p.emergencyTo) = _checkMaskedParams(p.amountIn, p.from, p.emergencyTo, maskedParams);
                p.to = _checkTo(p.to, uint64(block.chainid), nextOp);
                maskedParams.amountOut = _removeCrypto(p);
                maskedParams.to = p.to;
                maskedParams.emergencyTo = p.emergencyTo;
                if (maskedParams.amountOut == 0) {
                    result = ExecutionResult.Interrupted;
                ...
                (p.amountIn, p.from, p.emergencyTo) = _checkMaskedParams(p.amountIn, p.from, p.emergencyTo, maskedParams);
                p.to = _checkTo(p.to, uint64(block.chainid), nextOp);
                maskedParams.amountOut = _swapCrypto(p);
                maskedParams.to = p.to;
                maskedParams.emergencyTo = p.emergencyTo;
                if (maskedParams.amountOut == 0) {
                    result = ExecutionResult.Interrupted;
    ...
    function _checkTo(address to, uint64 chainId, bytes32 nextOp) internal view virtual override returns (address correctTo) {
        correctTo = super._checkTo(to, chainId, nextOp);
...
interface ISynthesisV2 {
    ...
    function synthByOriginal(address otoken) external view returns (address stoken);
...
contract RouterV2 is BaseRouter, IRouter {
    ...
    function _executeOp(
        ...
        if (PERMIT_CODE == op) {
            ...
            if (isOpHalfDone == false) {
                ...
                p.to = _checkTo(p.to, p.chainIdTo, nextOp);
                LOCK_MINT_CODE == op ? _lock(p) : _burn(p);
                chainIdTo = p.chainIdTo;
                if (LOCK_MINT_CODE != op) {
                    ISynthAdapter synthImpl = ISynthAdapter(p.tokenIn);
    ...
    function _emergencyMint(SynthParams memory p) internal returns (uint256 amountOut) {
        require(currentChainIdFrom == p.chainIdTo, "Router: wrong emergency init");
        address synthesis = IAddressBook(addressBook).synthesis(uint64(block.chainid));
        p.tokenIn = ISynthesisV2(synthesis).synthByOriginal(p.tokenIn);
        amountOut = ISynthesisV2(synthesis).emergencyMint(p.tokenIn, p.amountIn, p.from, p.emergencyTo);
    }

    function _burn(SynthParams memory p) internal {
        address synthesis = IAddressBook(addressBook).synthesis(uint64(block.chainid));
    ...
    function _checkTo(address to, uint64 chainId, bytes32 nextOp) internal view virtual returns (address correctTo) {
        if (nextOp == bytes32(0)) {
            correctTo = to;
