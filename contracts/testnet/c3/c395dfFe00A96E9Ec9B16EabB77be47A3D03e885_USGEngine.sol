...
interface IUSGEngine {
    ...
    // Errors
    ///////////////////
    error USGEngine__TokenAddressesAndPriceFeedAddressesAmountsDontMatch();
    error USGEngine__NeedsMoreThanZero();
    error USGEngine__TokenNotAllowed(address token);
    error USGEngine__TransferFailed();
    error USGEngine__BreaksHealthFactor(uint256 healthFactorValue);
 ...
 * Collateral: Crypto
 *
 * This contract is the core of usg.money. It handles the USG 'mint
 * and redeem' logic and is based on the MakerDAO DSS system.
 */
contract USGEngine is USGEngineAbstract {
    ///////////////////
    // Functions
    ///////////////////
    ...
    constructor(
        ...
        address usgAddress
    ) USGEngineAbstract(tokenAddresses, priceFeedAddresses, liquidationThresholds, collateralDecimals, usgAddress) {}
    ...
    ) external payable override(USGEngineAbstract) {
        depositCollateral(tokenCollateralAddress, amountCollateral);
        mintUsg(amountUsgToMint, tokenCollateralAddress);
    }
    ...
    ) external payable override(USGEngineAbstract) moreThanZero(amountCollateral) {
        _burnUsg(amountUsgToBurn, msg.sender, msg.sender, tokenCollateralAddress);
    ...
    function redeemCollateral(
        address tokenCollateralAddress,
        uint256 amountCollateral
    ) external payable override(USGEngineAbstract) moreThanZero(amountCollateral) nonReentrant {
        _redeemCollateral(tokenCollateralAddress, amountCollateral, msg.sender, msg.sender);
        revertIfHealthFactorIsBroken(msg.sender, tokenCollateralAddress);
    }
     ...
     */
    function burnUsg(uint256 amount, address collateral) external override(USGEngineAbstract) moreThanZero(amount) {
        _burnUsg(amount, msg.sender, msg.sender, collateral);
     ...
     * In return, you have to burn your USG to pay off their debt, but you don't pay off your own.
     * @param user: The user who is insolvent. They have to have a _healthFactor below MIN_HEALTH_FACTOR
     * @param debtToCover: The amount of USG you want to burn to cover the user's debt.
     *
    ...
    function liquidate(
        ...
        address user,
        uint256 debtToCover
    ) external payable override(USGEngineAbstract) moreThanZero(debtToCover) nonReentrant {
        uint256 startingUserHealthFactor = _healthFactor(user, collateral);
        if (startingUserHealthFactor >= MIN_HEALTH_FACTOR) {
            revert USGEngine__HealthFactorOk();
        }
        // If covering 100 USG, we need to $100 of collateral
        uint256 tokenAmountFromDebtCovered = getTokenAmountFromUsd(collateral, debtToCover);
        // And give them a 10% bonus
        // So we are giving the liquidator $110 of WETH for 100 USG
        // We should implement a feature to liquidate in the event the protocol is insolvent
        // And sweep extra amounts into a treasury
        uint256 bonusCollateral = (tokenAmountFromDebtCovered * LIQUIDATION_BONUS) / 100;
        // Burn USG equal to debtToCover
        // Figure out how much collateral to recover based on how much burnt
        _redeemCollateral(collateral, tokenAmountFromDebtCovered + bonusCollateral, user, msg.sender);
        _burnUsg(debtToCover, user, msg.sender, collateral);
     ...
     */
    function mintUsg(uint256 amountUsgToMint, address collateral) public override(USGEngineAbstract) moreThanZero(amountUsgToMint) nonReentrant {
        s_USGMinted[msg.sender][collateral] += amountUsgToMint;
        revertIfHealthFactorIsBroken(msg.sender, collateral);
        bool minted = i_usg.mint(msg.sender, amountUsgToMint);
    ...
    function getMintableUSG(
        ...
        address user,
        uint256 amountCollateral
    ) public view override(USGEngineAbstract) returns (uint256, bool) {
        ...
        uint256 totalUsgMintableAmount = (_getUsdValue(tokenCollateralAddress, amount + normalizedAmount) *
            liquidationThreshold[tokenCollateralAddress]) / 100;

        (uint256 totalUsgMinted, ) = _getAccountInformation(user, tokenCollateralAddress);

        if (totalUsgMintableAmount <= totalUsgMinted) {
            uint256 debtUsgAmount = totalUsgMinted - totalUsgMintableAmount;
            return (debtUsgAmount, false); // cannot mint usg anymore
        } else {
            uint256 mintableUsgAmount = totalUsgMintableAmount - totalUsgMinted;
            return (mintableUsgAmount, true);
        }
    }
    ...
    function getBurnableUSG(
        ...
        address user,
        uint256 amountUSG
    ) public view override(USGEngineAbstract) returns (uint256) {
        (uint256 totalUsgMinted, uint256 totalCollateralInUSD) = _getAccountInformation(user, tokenCollateralAddress);
        uint256 totalUsgAfterBurn = 0;
        uint256 tokenAmountInUSD = 0;
        if (amountUSG < totalUsgMinted) {
            totalUsgAfterBurn = totalUsgMinted - amountUSG;
        }
        uint256 inneedUSGAmount = 0;
        inneedUSGAmount += (totalCollateralInUSD * liquidationThreshold[tokenCollateralAddress]) / 100;

        if (inneedUSGAmount >= totalUsgAfterBurn) {
            tokenAmountInUSD = totalCollateralInUSD;
        } else {
            uint256 backupTokenInUSG = ((totalUsgAfterBurn - inneedUSGAmount) * 100) / liquidationThreshold[tokenCollateralAddress];
            tokenAmountInUSD = totalCollateralInUSD >= backupTokenInUSG ? totalCollateralInUSD - backupTokenInUSG : 0;
        }
    ...
    function _getAccountInformation(
        address user,
        address collateral
    ) internal view override(USGEngineAbstract) returns (uint256 totalUsgMinted, uint256 collateralValueInUsd) {
        totalUsgMinted = s_USGMinted[user][collateral];
        collateralValueInUsd = getAccountCollateralValue(user, collateral);
    }

    function _healthFactor(address user, address collateral) internal view override(USGEngineAbstract) returns (uint256) {
        (uint256 totalUsgMinted, uint256 collateralValueInUsd) = _getAccountInformation(user, collateral);
        return _calculateHealthFactor(totalUsgMinted, collateralValueInUsd, collateral);
    }
    ...
    function getAccountCollateralValue(
        address user,
        address collateral
    ) public view override(USGEngineAbstract) returns (uint256 totalCollateralValueInUsd) {
        uint256 amount = s_collateralDeposited[user][collateral];
        totalCollateralValueInUsd = _getUsdValue(collateral, amount);
        return totalCollateralValueInUsd;
    }

    function getTokenAmountFromUsd(address token, uint256 usdAmountInWei) public view override(USGEngineAbstract) returns (uint256) {
        uint256 tokenAmount;
        ...
        return (tokenAmount * 10 ** s_collateralDecimal[token]) / 10 ** 18;
    }
}
...
import "./interfaces/IUSGEngine.sol";

abstract contract USGEngineAbstract is ReentrancyGuard, Ownable, IUSGEngine {
    ///////////////////
    // State Variables
    ///////////////////
    USG internal immutable i_usg;
    ...
    mapping(address user => mapping(address token => uint256 amount)) internal s_USGMinted;
    ...
    constructor(
        ...
        if (tokenAddresses.length != priceFeedAddresses.length && tokenAddresses.length != collateralDecimals.length) {
            revert USGEngine__TokenAddressesAndPriceFeedAddressesAmountsDontMatch();
        }
    ...
    function normalizeTokenAmount(uint256 amount, address collateral) internal view returns (uint256) {
    ...
    function burnUsg(uint256 amount, address collateral) external virtual moreThanZero(amount) {}
    ...
    function mintUsg(uint256 amountUsgToMint, address collateral) public virtual moreThanZero(amountUsgToMint) nonReentrant {}
    ...
    function depositCollateral(
        ...
        if (tokenCollateralAddress == WETH) {
            require(msg.value == amountCollateral, "USG: Not enough balance");
        } else {
            bool success = IERC20(tokenCollateralAddress).transferFrom(msg.sender, address(this), amountCollateral);
            if (!success) {
                revert USGEngine__TransferFailed();
            }
        }
        uint256 normalizedAmount = normalizeTokenAmount(amountCollateral, tokenCollateralAddress);
    ...
    function getMintableUSG(address tokenCollateralAddress, address user, uint256 amountCollateral) public view virtual returns (uint256, bool) {}

    function getBurnableUSG(address tokenCollateralAddress, address user, uint256 amountUSG) public view virtual returns (uint256) {}
    ...
    function _redeemCollateral(address tokenCollateralAddress, uint256 amountCollateral, address from, address to) internal {
        ...
        if (tokenCollateralAddress == WETH) {
            ...
            require(success, "USG: Transfer ETH failed");
        } else {
    ...
    function _burnUsg(uint256 amountUsgToBurn, address collateral, address onBehalfOf, address usgFrom) internal {
        if (s_USGMinted[onBehalfOf][collateral] >= amountUsgToBurn) {
            s_USGMinted[onBehalfOf][collateral] -= amountUsgToBurn;
        } else {
            s_USGMinted[onBehalfOf][collateral] = 0;
        }

        bool success = i_usg.transferFrom(usgFrom, address(this), amountUsgToBurn);
        // This conditional is hypothetically unreachable
        if (!success) {
            revert USGEngine__TransferFailed();
        }
        i_usg.burn(amountUsgToBurn);
    }
    ...
    function _getAccountInformation(
        address user,
        address collateral
    ) internal view virtual returns (uint256 totalUsgMinted, uint256 collateralValueInUsd) {}
    ...
    function _calculateHealthFactor(uint256 totalUsgMinted, uint256 collateralValueInUsd, address collateral) internal view returns (uint256) {
        if (totalUsgMinted == 0) return type(uint256).max;
        return (collateralValueInUsd * liquidationThreshold[collateral] * 1e18) / (totalUsgMinted * 100);
    }
    ...
    function revertIfHealthFactorIsBroken(address user, address collateral) internal view {
        ...
        if (userHealthFactor < MIN_HEALTH_FACTOR) {
            revert USGEngine__BreaksHealthFactor(userHealthFactor);
        }
    }
    ...
    ////////////////////////////////////////////////////////////////////////////
    ////////////////////////////////////////////////////////////////////////////
    function calculateHealthFactor(uint256 totalUsgMinted, uint256 collateralValueInUsd, address collateral) external view returns (uint256) {
        return _calculateHealthFactor(totalUsgMinted, collateralValueInUsd, collateral);
    }

    function getAccountInformation(address user, address collateral) external view returns (uint256 totalUsgMinted, uint256 collateralValueInUsd) {
