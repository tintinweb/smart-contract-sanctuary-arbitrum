...
contract VePost is Ownable, IERC721, IERC721Metadata {
    ...
    function _transferRevenueTracking(
        ...
        if (!whiteListContractPostTech[_to]) {
    ...
    function withdraw(uint _tokenId) external nonreentrant {
        ...
        _decrease_tracking_revenue(msg.sender, _tokenId);
        ...
        uint256 tokenNftLpId = vePostToLPNft[_tokenId];

        if (tokenNftLpId == 0) {
            assert(IERC20(token).transfer(msg.sender, value));
        } else {
            lpNft.transferFrom(address(this), msg.sender, tokenNftLpId);
        }

        // Burn the NFT
        _burn(_tokenId);

        emit Withdraw(msg.sender, _tokenId, value, _blockTimeStamp());
    ...
    function averageWeightInTime(
        ...
        uint256 eliminateStart = endTime - lock.start;
        uint256 eliminateEnd = startTime - lock.start;

        uint256 integralWeigh = uint128(lock.amount) *
            (eliminateStart - eliminateEnd) -
            (uint128(lock.amount) / (2 * (lock.end - lock.start))) *
            (eliminateStart ** 2 - eliminateEnd ** 2);

        return integralWeigh / (endTime - startTime);
    }
    ...
    function averageMultiWeightInTime(
        ...
        uint256 eliminateStart;
        uint256 eliminateEnd;
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (startTime < endTime) {
                ...
                //                uint a = _balanceOfNFT(_tokenId, startTime);
                //                uint b = _balanceOfNFT(_tokenId, _endTime);
                //                balanceOfN
                //                eliminateStart = endTime - lock.start;
                //                eliminateEnd = startTime - lock.start;
                //
                //                uint128 buffer = uint128(lock.amount) * lock.boostMultiplier / basePercent;
                //                integralWeigh =
                //                    buffer *
                //                    (eliminateStart - eliminateEnd) -
                //                    (buffer / (2 * (lock.end - lock.start))) *
                //                    (eliminateStart ** 2 - eliminateEnd ** 2);
            }
        ...
        return totalIntegralWeigh / countToken;
    }
    ...
    function tokenURI(uint _tokenId) external view returns (string memory) {
        ...
        return
            vePostNFTSVG.buildVePost(
                _tokenId,
                lockType[_tokenId] == LockType.TOKEN_POST ? 0 : 1,
                _locked.start,
                _locked.end,
                _blockTimeStamp(),
                _locked.boostMultiplier,
...
contract VePostNFTSVG {
    ...
    function buildBackground(
        ...
        string memory color1 = lockType == 1 ? "#3088EB" : "#1565FE";
        string memory color2 = lockType == 1 ? "#6F49DC" : "#4473FF";
        string memory color3 = lockType == 1 ? "#8241D8" : "#567BFF";
