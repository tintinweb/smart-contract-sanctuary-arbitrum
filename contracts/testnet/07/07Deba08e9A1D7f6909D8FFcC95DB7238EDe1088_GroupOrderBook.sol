...
contract GroupOrderBook is ReentrancyGuard, IGroupOrderBookk {
    ...
    function getAmountOut(uint256 _amountIn, address[] memory _path, uint256 discount) internal view returns (uint256, uint256, uint256) {
        address _tokenIn = _path.length == 2 ? _path[0] : _path[1];
        address _tokenOut = _path.length == 1 ? _path[1] : _path[2];
        uint256 priceIn = IVault(vault).getMinPrice(_tokenIn);
        uint256 priceOut = IVault(vault).getMaxPrice(_tokenOut);
    ...
    function _wrapAndTransfer(uint256 _amountIn, address _path0, bool _shouldWrap) internal {
        if (_shouldWrap) {
    ...
    function buyGroupOrder(
        ...
        require(_path.length == 2 || _path.length == 3, "_path.length");
        require(_path[0] != _path[_path.length - 1], "_path");
        GroupOrder memory item = listOrders[orderId];
        require(_amountIn >= item.groupAmount, "_amountIn");
        validatePair(item.path, _path);

        uint256 amountOut;
        (, , amountOut) = getAmountOut(_amountIn, _path, item.discount);
        require(item.totalOut.add(amountOut) <= item.amountIn, "amountOut");

        _transferInETH();
        _wrapAndTransfer(_amountIn, _path[0], _shouldWrap);

        listOrders[orderId].totalOut = item.totalOut.add(amountOut);
        listOrders[orderId].totalIn = item.totalIn.add(_amountIn);
    ...
    function createSubGroupOrder(
        ...
        require(_path.length == 2 || _path.length == 3, "invalid _path.length");
        require(_path[0] != _path[_path.length - 1], "invalid _path");
        GroupOrder memory item = listOrders[orderId];
        require(_amountIn < item.groupAmount && item.groupAmount > 0, "invalid _amountIn");
        validatePair(item.path, _path);
    ...
    function placeSubGroupOrder(
        ...
        require(subOrder.totalOut > item.groupAmount && item.groupAmount > 0, "done");
        validatePair(item.path, _path);
