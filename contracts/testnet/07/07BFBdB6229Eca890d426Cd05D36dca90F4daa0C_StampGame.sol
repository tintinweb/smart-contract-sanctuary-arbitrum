...
/*
 * @title Stamp!
 * @dev This smart contract facilitates an on-chain PvP (Player versus Player) game called "Stamp". 
 * In this competitive environment, players strive to be the last Stamper before the timer concludes. 
 * The game operates in perpetual rounds, where each Stamp action post-timer initiates a new round.
 ...
 *
 * Telegram: https://t.me/stampscroll_official
 * Website/app: https://stampscroll.xyz
 * Twitter: https://twitter.com/stamp_scroll
 */
...
contract StampGame is ReentrancyGuard, Ownable {
    IERC20 public paymentToken;

    address public lastStamper;
    ...
    uint256 public lastStampTime;
    uint256 public currentRoundPrizePool;
    uint256 public nextRoundPrizePool;
    uint256 public stampCost;
    uint256 public nextRoundStampCost;
    ...
    event Stamped(address indexed stamper, uint256 time);
    ...
    constructor(uint256 _stampCost) {
    ...
    receive() external payable {
        if (isActive && (lastStampTime + gameDuration > block.timestamp)) {
    ...
    function timeUntilEnd() external view returns (uint256) {
        ...
        uint256 timeElapsed = block.timestamp - lastStampTime;
    ...
    function _checkGameStatus() internal {
        if (
            lastStampTime != 0 &&
            block.timestamp > lastStampTime + gameDuration &&
            isActive
        ) {
            uint256 prizeAmount = currentRoundPrizePool; // Use the segregated prize pool
            payable(lastStamper).transfer(prizeAmount);
            totalRewardsWon += prizeAmount; // Increment the total rewards sent

            emit PrizeClaimed(lastStamper, prizeAmount);
            emit GameEnded(lastStamper, prizeAmount);

            // Store the winner of this round and their reward
            roundWinners.push(
                RoundWinner({winner: lastStamper, reward: prizeAmount})
            );
            ...
            // Update stampCost for the next round
            if (nextRoundStampCost > 0) {
                stampCost = nextRoundStampCost;
                nextRoundStampCost = 0; // Reset nextRoundStampCost
            }
            ...
            lastStamper = address(0);
            lastStampTime = 0;
            isActive = true;
        }
    }

    // Handles the transfer and burn of tokens for a Stamp
    function _handleTokenTransferAndBurn() internal {
        require(
            paymentToken.transferFrom(msg.sender, address(this), stampCost),
    ...
    // Validates the conditions for a Stamp
    function _validateStampConditions() internal view {
        ...
        require(balance >= stampCost, "Not enough $TOKEN tokens");
    ...
    // Updates the game state post a Stamp
    function _updateGameState() internal {
        lastStamper = msg.sender;
        lastStampTime = block.timestamp;
        stampCount[msg.sender]++;
        totalStamps++;
        totalTokensUsed += stampCost;
        emit Stamped(msg.sender, block.timestamp);
    }
    ...
    // Set the cost to stamp, takes effect on the next round
    function setStampCost(uint256 _stampCost) external onlyOwner {
        nextRoundStampCost = _stampCost;
    }
    ...
    // Main function for players to perform a Stamp
    function stampIt() external nonReentrant gameActive notBlacklisted {
        _checkGameStatus();
        _validateStampConditions();
