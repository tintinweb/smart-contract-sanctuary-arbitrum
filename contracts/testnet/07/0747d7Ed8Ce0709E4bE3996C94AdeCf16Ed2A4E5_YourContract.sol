/**
 *Submitted for verification at arbiscan.io on 2022-02-16
*/
...
contract YourContract is ERC721A, ERC721ABurnable, AccessControl, ReentrancyGuard {
  ...
  // stores information about an individual mage's traits
  struct Traits {
    // power of the wind and air
    uint256 gale;
    // power of the flame and change
    uint256 blaze;
    // power of mystery and the unknown
    uint256 moon;
    // power of the waves and water
    uint256 torrent;
    // power of connection and love
    uint256 revelry;
  }

  // returns the traits of a particular tokenID
  mapping(uint256 => Traits) _tokenTraits;

  // returns a ritual struct for a given wallet address
  mapping(address => Ritual) _ritual;

  // emits Begin Ritual success for front-end consumption
  event BeginRitual(uint256 indexed _to, uint256 indexed _amount);

  // emits Finish Ritual success for front-end consumption
  event FinishRitual(uint256 indexed _to, uint256 indexed _amount, bytes indexed);

  /*************************
     STATE VARIABLES
     *************************/

  // number of attributes in the Traits struct
  uint256 traitsLength = 5;
  ...
  function finishRitual(address _to) external callerIsUser notPaused {
    ...
    for (uint256 i; i < _amount; i++) {
      _blockHash = abi.encodePacked(keccak256(_blockHash));
      toTokenTraits(_tokenId, _blockHash);
      _tokenId++;
    }
  ...
  // helper function to map _tokenTraits for each minted NFT
  function toTokenTraits(uint256 _tokenId, bytes memory _blockHash) internal {
    Traits memory traits;
    traits.blaze = uint256(keccak256(abi.encodePacked(_blockHash, uint8(1)))) % 100;
    traits.gale = uint256(keccak256(abi.encodePacked(_blockHash, uint8(2)))) % 100;
    traits.moon = uint256(keccak256(abi.encodePacked(_blockHash, uint8(3)))) % 100;
    traits.revelry = uint256(keccak256(abi.encodePacked(_blockHash, uint8(4)))) % 100;
    traits.torrent = uint256(keccak256(abi.encodePacked(_blockHash, uint8(5)))) % 100;
    _tokenTraits[_tokenId] = traits;
  }

  // verifies correct amount of ETH sent to contract and refunds if overg
