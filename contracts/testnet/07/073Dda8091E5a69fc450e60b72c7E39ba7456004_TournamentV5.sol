...
contract TournamentV5 is ReentrancyGuard, ITournamentV5, Ownable {
    ...
    function record(uint256 _gameId, uint256 _roundId, uint256[] memory  _players, uint256[] memory _predictions) external onlyOperator {
        GameInfo storage _gameInfo = gameInfo[_gameId];
    ...
    function update(uint256 _gameId, uint256 _roundId, uint256 _result) external onlyOperator {
        require(roundInfo[_gameId][_roundId].result == ROUND_RESULT(_result), "PredictionV5: have result");
...
interface ITournamentV5 {
    ...
    function record(uint256, uint256, uint256[] memory, uint256[] memory) external;
