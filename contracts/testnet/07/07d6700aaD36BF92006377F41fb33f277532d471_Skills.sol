...
contract Skills {
    ...
    function stake(uint endDate, uint256 submissionsAllotted) payable virtual external returns(uint256) {
        ...
        require(fundsNeeded > unallocatedFunds, "There is not enough funds to distribute the rewards of this order.");
    ...
    function lookupReward(uint256 orderId, address receiver) public view virtual returns(uint256) {
        ...
        require(_submissions[orderId][receiver] > 0, "This address has not submitted for the order and cannot get any rewards from the order.");
