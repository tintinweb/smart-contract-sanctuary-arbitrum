...
contract SynthrBridge is BaseBridge {
    ...
    event SendPacket(uint16 indexed dstChainId, uint16 indexed packetType, uint64 nonce);
    event ReceivePacket(uint16 indexed srcChainId, uint16 indexed packetType, uint64 nonce);
    ...
    function _nonblockingLzReceive(
        ...
        emit ReceivePacket(_srcChainId, packetType, nonce);
    }
    ...
    function _broadcast(bytes memory lzPayload, uint16 packetType) internal {
        ...
        for (uint ii = 0; ii < length; ii++) {
            ...
            (lzFee, ) = ILayerZeroEndpoint(lzEndpoint).estimateFees(dstChainId, thisAddress, lzPayload, false, _adapterParam);
            require(userFund >= lzFee, "SB: Insufficient LZ fee");
            userFund -= lzFee;
            _lzSend(dstChainId, lzPayload, payable(thisAddress), address(0x0), _adapterParam, lzFee);
            
            nonce = ILayerZeroEndpoint(lzEndpoint).getOutboundNonce(dstChainId, thisAddress);
            emit SendPacket(dstChainId, packetType, nonce);
        }
    }
