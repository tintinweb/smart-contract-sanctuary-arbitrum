...
contract POCOStake is Context, AccessControlEnumerable, ReentrancyGuard, Pausable {
    ...
    */
    ...
    function resetPeriod(uint256 _pid) public hasAdminRole checkPid(_pid) {
        for (uint256 i = 0; i < periods[_pid].length; i++) {
            periods[_pid].pop();
        }
    }

    function setStartTime(uint256 _pid, uint256 _time) public hasAdminRole checkPid(_pid) {
        pool[_pid].startTime = _time;
        pool[_pid].endTime = pool[_pid].startTime;
        for (uint256 i = 0; i < periods[_pid].length; i++) {
            pool[_pid].endTime = pool[_pid].endTime + periods[_pid][i].durationTime;
        }
    }

    function editPeriod(uint256 _pid, uint256 _idx, uint256 _durationTime, uint256 _amountPerSec) public hasAdminRole checkPid(_pid) {
        require(_idx < periods[_pid].length, "invalid param");
        if (_idx > 0) {
            require(periods[_pid][_idx - 1].durationTime < _durationTime, "invalid duration time");
        }
        pool[_pid].endTime = pool[_pid].endTime - periods[_pid][_idx].durationTime + _durationTime;
        periods[_pid][_idx].durationTime = _durationTime;
        periods[_pid][_idx].amountPerSec = _amountPerSec;
    }

    function addPeriod(uint256 _pid, uint256 _durationTime, uint256 _amountPerSec) public hasAdminRole checkPid(_pid) {
        if (periods[_pid].length > 0) {
            require(periods[_pid][periods[_pid].length - 1].durationTime < _durationTime, "invalid duration time");
        }
        periods[_pid].push( Period({durationTime: _durationTime, amountPerSec: _amountPerSec}) );
        pool[_pid].endTime = pool[_pid].endTime + _durationTime;
    }
     ...
     */
    function getMultiplier(uint256 _pid, uint256 _from, uint256 _to) public view returns(uint256 multiplier) {
        if (_from < pool[_pid].startTime) {
            _from = pool[_pid].startTime;
        }
        if (_to > pool[_pid].endTime) {
            _to = pool[_pid].endTime;
        }

        uint256 startTimeTmp = pool[_pid].startTime;
        for (uint256 i = 0; i < periods[_pid].length; i++) {
            uint256 endTimeTmp = pool[_pid].startTime + periods[_pid][i].durationTime - 1;
            if (_from >= startTimeTmp) {
                if (_to <= endTimeTmp) {
                    multiplier = multiplier + (_to - _from + 1) * periods[_pid][i].amountPerSec;
                    break;
                } else {
                    multiplier = multiplier + periods[_pid][i].durationTime * periods[_pid][i].amountPerSec;
                }
                _from = endTimeTmp + 1;
                startTimeTmp = endTimeTmp + 1;
            }
        }
    }
    ...
    function pendingPOCO(uint256 _pid, address _user) public checkPid(_pid) view returns(uint256) {
        ...
        if (block.timestamp > pool_.lastRewardTime) {
            uint256 totalPOCO = getMultiplier(_pid, pool_.lastRewardTime, block.timestamp) * pool_.poolWeight / totalPoolWeight;
        ...
        uint256 pendingPOCO_ = user_.amount * accPOCOPerToken_ / (1 ether) - user_.finishedPOCO;
     ...
     */
    function addPool(address _tokenAddress, uint256 _poolWeight, uint256 _startTime, bool _withUpdate) public hasAdminRole {
        ...
        emit AddPool(_tokenAddress, _poolWeight, _startTime);
    }
    ...
    function depositReward(uint256 _pid, uint256 _amount) external checkPid(_pid) {
        Pool storage pool_ = pool[_pid];
        bool success = IERC20(pool_.tokenAddress).transferFrom(address(msg.sender), address(this), _amount);
        require(success, "transfer error");
        pool_.leftRewardAmount = pool_.leftRewardAmount + _amount;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public checkPid(_pid) {
        require(block.timestamp < pool[_pid].endTime, "Staking already ended");
        ...
        if (user_.amount > 0) {
            uint256 pendingPOCO_ = user_.amount * pool_.accPOCOPerToken / (1 ether) - user_.finishedPOCO;
            if(pendingPOCO_ > 0) {
                _safePOCORewardTransfer(_pid, msg.sender, pendingPOCO_);
            }
        }
     ...
     */
    function _safePOCORewardTransfer(uint256 _pid, address _to, uint256 _amount) internal {
        _amount = _amount > pool[_pid].leftRewardAmount ? pool[_pid].leftRewardAmount : _amount;
        poco.transfer(_to, _amount);
        pool[_pid].leftRewardAmount = pool[_pid].leftRewardAmount - _amount;
    }
}