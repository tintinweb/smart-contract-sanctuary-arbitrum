...
contract ZkBridgeOracle is ILayerZeroOracleV2, Initializable, OwnableUpgradeable {
    ...
    // proofType=>chainId=>blockUpdater
    mapping(uint16 => mapping(uint16 => IBlockUpdater)) public blockUpdaters;
    ...
    function updateHash(uint16 _sourceChainId, uint16 _proofType, bytes32 _blockHash, bytes32 _blockData, address _userApplication) external {
        _updateHash(_sourceChainId, _proofType, _blockHash, _blockData, _userApplication);
    }

    function batchUpdateHash(uint16[] calldata _sourceChainIds, uint16[] calldata _proofType, bytes32[] calldata _blockHashes, bytes32[] calldata _blockDatas, address[] calldata _userApplication) external {
        require(_sourceChainIds.length == _proofType.length, "Parameter lengths must be the same");
        require(_sourceChainIds.length == _blockHashes.length, "Parameter lengths must be the same");
        require(_sourceChainIds.length == _blockDatas.length, "Parameter lengths must be the same");
        ...
        for (uint256 i = 0; i < _sourceChainIds.length; i++) {
            _updateHash(_sourceChainIds[i], _proofType[i], _blockHashes[i], _blockDatas[i], _userApplication[i]);
        }
    }
    ...
    function _updateHash(uint16 _sourceChainId, uint16 _proofType, bytes32 _blockHash, bytes32 _blockData, address _userApplication) internal {
        IBlockUpdater blockUpdater = blockUpdaters[_proofType][_sourceChainId];
        require(address(blockUpdater) != address(0), "ZkBridgeOracle:Unsupported source chain");
        (bool exist,uint256 blockConfirmation) = blockUpdater.checkBlockConfirmation(_blockHash, _blockData);
        ...
        ILayerZeroUltraLightNodeV2(uln).updateHash(_sourceChainId, _blockHash, blockConfirmation, _blockData);
    }
    ...
    function setBlockUpdater(uint16 proofType, uint16 _sourceChainId, address _blockUpdater) external onlyOwner {
        require(_blockUpdater != address(0), "ZkBridgeOracle:Zero address");
        emit ModBlockUpdater(_sourceChainId, address(blockUpdaters[proofType][_sourceChainId]), _blockUpdater);
        blockUpdaters[proofType][_sourceChainId] = IBlockUpdater(_blockUpdater);
    }
