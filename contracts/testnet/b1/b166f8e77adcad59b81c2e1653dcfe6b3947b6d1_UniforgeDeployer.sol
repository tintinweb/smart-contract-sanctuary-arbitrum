 ...
 * @notice UniforgeDeployer is an optimized smart contract 
 * factory that enables the creation of UniforgeCollection 
 * contracts and contains methods to interact with Uniforge.
 * @notice UniforgeCollection is an universal token contract 
 * that extends ERC721A with enforced royalty capabilities.
 */
contract UniforgeDeployer is Ownable {
    ...
    /**
     * @notice Allows the caller to deploy a new Uniforge Collection.
     * @param owner The address of the new owner of the new UniforgeCollection contract.
     * @param name The name of the ERC721 token.
     * @param symbol The symbol of the ERC721 token.
     * @param baseURI The base Uniform Resource Identifier (URI) of the ERC721 token metadata.
     * @param mintFee The fee for minting a single token while the public sale is open.
     * @param mintLimit The maximum number of tokens that can be minted in a single transaction.
     ...
     * @param royaltyPercentage The percentage of the royalties for the ERC2981 standard.
     * @param royaltyEnforced The boolean to enable or disable the Operator Filter standard.
     */
    function deployNewCollection(
        ...
        // Deploy a new UniforgeCollection contract.
    ...
    /**
     * @notice Allows the contract owner to provide a discount to a specific creator.
     * @param creator The address of the creator who gets the discount.
    ...
    function setCreatorDiscount(address creator, uint256 percentage) public onlyOwner {
        ...
        // Verify that the percentage is between the bounds.
 ...
 * @author Dapponics
 * @notice An optimized and universal token contract that
 * extends ERC721A with enforced royalty capabilities.
 */
contract UniforgeCollection is Ownable, ERC721A, ERC2981, DefaultOperatorFilterer{
    ...
    event MintLimitUpdated(uint256 indexed _mintLimit);
    event SaleStartUpdated(uint256 indexed _saleStart);

    /**
     * @notice Transfers ownership to the creator right at deployment and declares all the variables.
     ...
     * @param baseURI_ The base Uniform Resource Identifier (URI) of the ERC721 token metadata.
     * @param mintFee_ The fee for minting a single token while the public sale is open.
     * @param mintLimit_ The maximum number of tokens that can be minted in a single transaction.
     ...
     * @param royaltyEnforced_ The boolean to enable or disable the Operator Filter capabilities.
     */
    constructor(
        address owner_,
        string memory name_,
     ...
     * The caller has to send `mintFee`*`quantity` ether and the sale should be open to mint.
     * The `quantity` has to be greater than 0 and less than or equal to `mintLimit`.
    ...
    function mintNft(uint256 quantity) public payable {
        ...
        // Verify that the supply won't be exceeded
    ...
    /**
     * @notice Allows the contract owner to mint tokens without constraints for marketing purposes.
     * @param receiver The address to receive the minted tokens.
     * @param quantity The number of tokens to mint.
     */
    function creatorMint(address receiver, uint256 quantity) public onlyOwner {
        // Verify that the supply won't be exceeded
        if (_totalMinted() + quantity > _maxSupply) revert UniforgeCollection__MaxSupplyExceeded();
    ...
    /**
     * @dev Allows the contract owner to set the mint limit.
     * @param quantity The new maximum number of tokens minteable at once.
     */
    function setMintLimit(uint256 quantity) public onlyOwner {
        _mintLimit = quantity;
    ...
    function setBaseURI(string memory newBaseURI) public onlyOwner {
        // Verify that the baseURI is not locked
     ...
     * @param receiver The recipient of the royalty fees.
     * @param feeNumerator The percentage in two decimal places (e.g. 100 = 1%).
     */
    function setDefaultRoyalty(address receiver, uint96 feeNumerator) public onlyOwner {
        _royaltyReceiver = receiver;
        _royaltyPercentage = feeNumerator;
        _setDefaultRoyalty(_royaltyReceiver, _royaltyPercentage);
        emit RoyaltyUpdated(_royaltyReceiver, _royaltyPercentage);
    }

    /**
     * @notice Allows the contract owner to set the royalty receiver and percentage to zero.
     */
    ...
    /**
     * @notice Enables or disables the operator filter for marketplaces. This toggle empowers
     * creators and communities with new revenue strategies while giving the option to remain 
     * fully independent and decentralized.
     *
     * When `_royaltyEnforced` is set to true, the operator filter is turned on, meaning
     * the NFTs of this contract can only be traded in ERC2981 compliant marketplaces.
     *
     * When `_royaltyEnforced` is set to false, this contract acts as a conventional ERC721
     * contract, signaling the royalty information through the ERC2981 standard but not
     * enforcing filters for operators that limit the performance of certain marketplaces.
     *
     * Approval and transfer operations are overriden conditionally depending on this toggle.
     */ 
    function toggleRoyaltyEnforcement() public onlyOwner {
        _royaltyEnforced ? _royaltyEnforced = false : _royaltyEnforced = true;
        emit RoyaltyEnforced(_royaltyEnforced);
    }

    /**
     * @notice If royaltyEnforced is set to true, inherits from Opensea's Operator Filter
     */
    function setApprovalForAll(address operator, bool approved) public override {
        if(_royaltyEnforced){
            // Acts as modifier {onlyAllowedOperatorApproval(address operator)}
    ...
    function transferFrom(address from, address to, uint256 tokenId) public payable override {
        if(_royaltyEnforced){
            // Acts as modifier {onlyAllowedOperator(address from)}
    ...
    */
     ...
     */
    function lockedBaseURI() public view returns (bool) {
    ...
    /**
     * @notice Returns the maximum number of tokens that can be minted in a single transaction.
     */
    ...
    /**
     * @notice Returns the current state of the royalty enforcement toggle.
     */
    function royaltyEnforced() public view returns (bool) {
