...
import {DelegatedCalls} from "../../utils/DelegatedCalls.sol";
...
library BinaryVaultFacetStorage {
    struct Layout {
        ...
        mapping(address => bool) whitelistedMarkets;
...
contract BinaryVaultFacet is ReentrancyGuard, IBinaryVaultFacet, IBinaryVaultPluginImpl {
    ...
    modifier onlyMarket() {
        ...
        require(s.whitelistedMarkets[msg.sender], "ONLY_MARKET");
        _;
    }

    modifier onlyOwner {
        require(IVaultDiamond(address(this)).owner() == msg.sender, "Ownable: caller is not the owner");
        _;
    }
    ...
    function setWhitelistMarket(
        ...
        s.whitelistedMarkets[market] = whitelist;
    ...
    function addLiquidity(
        ...
        if (!isNew) {
            require(IBinaryVaultNFTFacet(address(this)).ownerOf(tokenId) == msg.sender, "NOT_OWNER");

            BinaryVaultDataType.WithdrawalRequest memory withdrawalRequest = s.withdrawalRequests[
                tokenId
            ];
        ...
        if (isNew) {
            ...
            __burn(tokenId);
            // Mint New position.
            uint256 newTokenId = IBinaryVaultNFTFacet(address(this)).nextTokenId();

            s.shareBalances[newTokenId] = currentShares + newShares;
            s.initialInvestments[newTokenId] = currentInitialInvestments + amount;
    ...
    function mergePositions(
        ...
        for (uint256 i; i < tokenIds.length; i = i + 1) {
            ...
            BinaryVaultDataType.WithdrawalRequest memory request = s.withdrawalRequests[tokenId];
    ...
    function requestWithdrawal(
        ...
        require(IBinaryVaultNFTFacet(address(this)).ownerOf(tokenId) == msg.sender, "NOT_OWNER");
        BinaryVaultDataType.WithdrawalRequest memory r = s.withdrawalRequests[tokenId];
        ...
        uint256 minExpectAmount = underlyingTokenAmount > pendingRisk
            ...
            : 0;
        BinaryVaultDataType.WithdrawalRequest memory _request = BinaryVaultDataType.WithdrawalRequest(
            tokenId,
            shareAmount,
    ...
    function executeWithdrawalRequest(
        ...
        require(user == IBinaryVaultNFTFacet(address(this)).ownerOf(tokenId), "NOT_REQUEST_OWNER");
        BinaryVaultFacetStorage.Layout storage s = BinaryVaultFacetStorage.layout();

        BinaryVaultDataType.WithdrawalRequest memory _request = s.withdrawalRequests[tokenId];
        ...
        if (fee > 0) {
            ...
            IERC20(s.underlyingTokenAddress).safeTransfer(s.config.treasury(), fee);
        }
    ...
    function cancelWithdrawalRequest(uint256 tokenId) external virtual {
        require(msg.sender == IBinaryVaultNFTFacet(address(this)).ownerOf(tokenId), "NOT_REQUEST_OWNER");
    ...
    {
        uint256[] memory tokenIds = IBinaryVaultNFTFacet(address(this)).tokensOfOwner(user);
    ...
    function onRoundExecuted(
        ...
        emit VaultChangedFromMarket(prevTvl, s.totalDepositedAmount, s.watermark);
    }
    ...
    function getShareBipsExpression(
        ...
        uint256 percent = (s.shareBalances[tokenId] * 10_000) / s.totalShareSupply;
    ...
    function getWithdrawalExpression(
        ...
        if (withdrawalRequest.timestamp == 0) {
            ...
            withdrawalRequest.timestamp +s. withdrawalDelayTime <= block.timestamp
        ) {
            return "Executable";
        } else {
            return "Pending";
    ...
    function getManifestPlainText(
        uint256 _tokenId
    ) internal view virtual returns (string memory _manifest) {
        BinaryVaultFacetStorage.Layout storage s = BinaryVaultFacetStorage.layout();
        ...
        for (uint256 tokenId = from; tokenId <= to; tokenId++) {
            ...
            if (fee > 0) {
                ...
                if (s.shareBalances[tokenId] >= feeShare) {
                    s.shareBalances[tokenId] = s.shareBalances[tokenId] - feeShare;
                }
    ...
    function whitelistMarkets(address market) external view returns(bool) {
        return BinaryVaultFacetStorage.layout().whitelistedMarkets[market];
    }
    ...
    function initialInvestments(uint256 tokenId) external view returns (uint256) {
    ...
    function withdrawalRequests(uint256 tokenId) external view returns (BinaryVaultDataType.WithdrawalRequest memory) {
    ...
    function pluginSelectors() private pure returns (bytes4[] memory s) {
        ...
        s[23] = IBinaryVaultFacet.whitelistMarkets.selector;
...
interface IBinaryVault {
    ...
    function getExposureAmountAt(
        uint256 endTime
    ) external view returns (uint256 exposureAmount, uint8 direction);
}
...
interface IBinaryVaultFacet is IBinaryVault{
    function setWhitelistMarket(address market, bool whitelist) external;
    ...
    function config() external view returns(address);
    function underlyingTokenAddress() external view returns(address);
    function whitelistMarkets(address) external view returns(bool);
    ...
    function withdrawalRequests(uint256) external view returns(BinaryVaultDataType.WithdrawalRequest memory);
    function totalShareSupply() external view returns(uint256);
    function totalDepositedAmount() external view returns(uint256);
    function watermark() external view returns(uint256);
    function pendingWithdrawalTokenAmount() external view returns(uint256);
...
library DelegatedCallsStorage {
    ...
    bytes32 internal constant STORAGE_SLOT = keccak256('balancecapital.storage.DelegatedCalls');
