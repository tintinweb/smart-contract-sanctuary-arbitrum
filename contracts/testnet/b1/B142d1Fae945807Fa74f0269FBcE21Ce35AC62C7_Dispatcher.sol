    ...
    */
    ...
    function _mintAvailableCollectibles(address _patron) internal {
        ...
        for(uint256 i = 0; i < allLoot8Collections.length; i++) {
            ...
            (string[] memory points, uint256 radius) = ICollectionManager(authority.getAuthorities().collectionManager).getLocationDetails(allLoot8Collections[i].source);
    ...
    function _mintCollectibles(address _collection, address _patron) internal returns (uint256 collectibleId) {
        collectibleId = ICollectionManager(authority.getAuthorities().collectionManager).mintCollectible(_patron, _collection);

        if (ICollectionManager(authority.getAuthorities().collectionManager).getCollectionType(_collection) == CollectionType.PASSPORT) {
    ...
    function mintLinked(address _collection, address _patron) public virtual override {
        ...
        require(ICollectionManager(authority.getAuthorities().collectionManager).checkCollectionActive(_collection), "Collectible is retired");
        
        address[] memory linkedCollections = ICollectionHelper(authority.collectionHelper()).getAllLinkedCollections(_collection);

        for (uint256 i = 0; i < linkedCollections.length; i++) {
            if(
                IERC721(_collection).balanceOf(_patron) > 0 &&
                ICollectionManager(authority.getAuthorities().collectionManager).checkCollectionActive(linkedCollections[i]) &&
                ICollectionManager(authority.getAuthorities().collectionManager).getCollectionData(linkedCollections[i]).mintWithLinked &&
                ICollectionManager(authority.getAuthorities().collectionManager).getCollectionChainId(linkedCollections[i]) == block.chainid &&
    ...
    /* function getAllActiveReservations() external view returns(Reservation[] memory _activeReservations) {
        ...
        uint256 j;
        _activeReservations = new Reservation[](count);
    ...
    function getPatronReservations(address _patron, bool _checkActive) external view returns(Reservation[] memory _patronReservations) {
        ...
        uint256 j;
        _patronReservations = new Reservation[](count);
    ...
    function patronReservationActiveForOffer(address _patron, address _offer) external view returns(bool) {
        ...
        uint256 patronOfferRecentReservation = offerContext[_offer].patronRecentReservation[_patron];
    ...
    function getActiveReservationsForEntity(address _entity) external view returns(Reservation[] memory _entityActiveReservations) {
        ...
        address[] memory offerList = allOffers;
        ...
        for(uint256 i = 0; i < offerList.length; i++) {
            ...
            if(ICollectionManager(authority.getAuthorities().collectionManager).getCollectionData(offer).entity == _entity) {
                ...
                uint256[] memory reservationIdsList = offerContext[offer].reservations;
                
                for(uint256 j = 0; j < reservationIdsList.length; j++) {
                    if(
                        reservations[reservationIdsList[j]].expiry > block.timestamp &&
                        !reservations[reservationIdsList[j]].fulfilled &&
                        !reservations[reservationIdsList[j]].cancelled
                    ) {
                        count++;
                    }
                }
        ...
        uint256 k;
        _entityActiveReservations = new Reservation[](count);
            ...
            if(ICollectionManager(authority.getAuthorities().collectionManager).getCollectionData(offer).entity == _entity) {
                ...
                for(uint256 j = 0; j < reservationIdsList.length; j++) {
                    if(
                        ...
                        _entityActiveReservations[k] = reservations[reservationIdsList[j]];
                        k++;
