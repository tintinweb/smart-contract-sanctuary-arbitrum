...
contract BaseWrappedSynthr is MixinResolver, Owned {
    ...
    function exchange(
        ...
        (amountReceived) = exchanger().exchange{value: msg.value}(msg.sender, bytes32(0), args);
    }
    ...
    function burnSynths(uint256 amount, bytes32 synthKey) external payable issuanceActive systemActive {
        (uint256 synthAmount, uint256 debtShare, uint256 reclaimed, uint256 refunded) = issuer().burnSynths(
            msg.sender,
            synthKey,
            amount
        );
        synthrBridge().sendBurn{value: msg.value}(msg.sender, synthKey, synthAmount, debtShare, reclaimed, refunded, false);
        liquidatorRewards().updateEntry(msg.sender);
    }

    function burnSynthsToTarget(bytes32 synthKey) external payable issuanceActive systemActive {
        (uint256 synthAmount, uint256 debtShare, uint256 reclaimed, uint256 refunded) = issuer().burnSynthsToTarget(
            msg.sender,
            synthKey
        );
        synthrBridge().sendBurn{value: msg.value}(msg.sender, synthKey, synthAmount, debtShare, reclaimed, refunded, true);
        liquidatorRewards().updateEntry(msg.sender);
    }
    ...
    function collateralTransfer(
        ...
        bytes32 _collateralKey,
        uint256 _usdAmount
    ) public systemActive returns (bool, uint256) {
    ...
    function liquidateDelinquentAccount(address account, bytes32 collateralKey) external payable systemActive returns (bool) {
    ...
    function liquidateSelf(bytes32 collateralKey) external payable systemActive returns (bool) {
...
contract WrappedSynthr is BaseWrappedSynthr {
    ...
    function issueSynths(
        ...
        if (_synthToMint > 0) {
            (synthAmount, debtShare) = issuer().issueSynths(msg.sender, _synthToMint, _destChainId);
            // require(synthAmount > 0 && debtShare > 0, "System is suspended due to variance of debt ratio");
        }

        if (_collateralAmount > 0 || synthAmount > 0) {
    ...
    function withdrawCollateral(bytes32 _collateralKey, uint256 _collateralAmount) external payable issuanceActive systemActive {
