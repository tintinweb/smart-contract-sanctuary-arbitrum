...
contract Mines is MinesBase {
  ...
  function setMultipliers(uint256 _numMines) external onlyGovernance {
    for (uint256 g = 1; g <= 25 - _numMines; g++) {
      ...
      minesMultipliers[_numMines][g] = (9900 * (10 ** 9)) / ((multiplier * (10 ** 9)) / divisor);
    }
  }
   ...
   */
  function getState(address _player) external view returns (Game memory minesState_) {
  ...
  function bet(
    ...
    require(game_.requestID == 0, "Mines: Awaiting VRF");
  ...
  function endGame() external nonReentrant {
    ...
    uint256 payout_ = (multiplier_ * wager_) / 10000;
    ...
    delete (games[_msgSender()]);

    // Emit an event to indicate the end of the game and provide payout details
    emit EndGame(_msgSender(), wager_, payout_, token_, multiplier_);
  }
   ...
   */
  function revealCells(bool[25] calldata _cells, bool _isCashout) external payable nonReentrant {
    // Retrieve the game information for the current player
    Game storage game = games[_msgSender()];

    // Ensure that the player is in a game
    require(game.numMines > 0, "Mines: Not in game");
    require(game.requestID == 0, "Mines: Awaiting VRF");
  ...
  function randomizerFulfill(uint256 _requestId, uint256[] calldata _randoms) internal override {
    ...
    console.log("numberOfMinesLeft_ %s", numberOfMinesLeft_);
    console.log("numberOfRevealedCells_ %s", numberOfRevealedCells_);
    uint256 multiplier_ = minesMultipliers[numberOfMinesLeft_][numberOfRevealedCells_];
    console.log("multiplier_ %s", multiplier_);
    ...
    if (game_.isCashout == false) {
      ...
      emit RevealAndCashout(
        player_,
        wager_,
        (multiplier_ * wager_) / 10000,
        token_,
        mines_,
        game_.revealedCells,
        multiplier_
      );
      shareEscrow(game_, wager_, (multiplier_ * wager_) / 10000);
  ...
  function _pickCell(
    ...
    uint256 winChance = 10000 - (_numberOfMinesLeft * 10000) / _numberCellsLeft;
    
    bool won_ = false;
    if (_random % 10000 <= winChance) {
      won_ = true;
    }
...
abstract contract MinesBase is Core {
  ...
  mapping(uint256 => address) gameIds;
