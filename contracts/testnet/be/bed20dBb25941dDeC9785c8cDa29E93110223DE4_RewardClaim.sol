...
contract RewardClaim is ReentrancyGuard {
  ...
  constructor() {
    ...
    function isWhitelisted(uint rewardId, address addr, bytes32[] memory proof) external view returns (bool) {
        return verify(rewardId, keccak256(abi.encode(addr)), proof);
    }
    ...
    function ClaimRoyalty(uint rewardId, uint256 amount, bytes memory signature, bytes32[] memory proof) external nonReentrant {
        ...
        require (IERC20(rewards[rewardId].royaltyAddress).balanceOf(address(this)) >= rewards[rewardId].royaltyAmount, "RewardClaim: Insufficient ERC20 Token.");
        require (amount == rewards[rewardId].royaltyAmount, "RewardClaim: Invalid Amount");
        ...
        IERC20(rewards[rewardId].royaltyAddress).transfer(msg.sender, rewards[rewardId].royaltyAmount);
        rewards[rewardId].isAddressClaimed[msg.sender] = true;

        emit RewardClaimed(rewardId, msg.sender, rewards[rewardId].royaltyAddress, rewards[rewardId].royaltyAmount);
    }
