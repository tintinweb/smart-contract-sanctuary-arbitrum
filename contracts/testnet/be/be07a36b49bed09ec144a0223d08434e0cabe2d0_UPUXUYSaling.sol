 ...
 * UPUXUY Proxy
 */
contract UPUXUYSaling is Manager, Round {
    ...
    mapping(uint8 => SalingConf) public salingConfs;
    mapping(uint8 => uint8) public statusOfRound; // 0:preparing, 1:saling, 2:claimable
    ...
    constructor(address _vipPass, address _svipPass, address _usdt, address _upuxuy) {
        ...
        unchecked {
            capOfSaling = Upuxuy.maxSupply() * 15 / 100;
        }
    }
    ...
    function setSalingConf(uint8 round, SalingConf calldata conf) public onlyManager {
        require(conf.PricePublic > 0, "price for public is zero");
        salingConfs[round] = conf;
    }
    ...
    function setCapOfSaling(uint256 cap) public onlyManager {
        capOfSaling = cap;
    }

    function buy(uint256 numOfUsdt) public returns(bool) {
        require(statusOfRound[currRound] == 1, "saling is not opened");
    ...
    function _allocationRatio(uint8 round) internal view returns (uint256) {
        uint256 targetUsdt = _usdtOfUpuxuy(salingConfs[round].UpuxuySupply, salingConfs[round].PricePublic);
    ...
    function beClaimed(uint8 round, address account) public view returns(
        ...
        uint256 usdtAcountBack
    ) {
        usdtAmount = usdtAmountOf(account, round);
        if (usdtAmount == 0) {
            return (isVip, price, ratio, usdtAmount, upuxuyAmount, usdtAcountBack);
        }
        isVip = (VipPass.balanceOf(account) > 0 || SVipPass.balanceOf(account) > 0);
        price = (isVip ? salingConfs[round].PriceVip : salingConfs[round].PricePublic);
        if (isVip && price == 0) {
            price = salingConfs[round].PricePublic;
        }
        ratio = _allocationRatio(round);
        upuxuyAmount = _upuxuyOfUsdt(usdtAmount, price, ratio);
        if (ratio < denominatorOfRatio) {
            usdtAcountBack = usdtAmount - _usdtOfUpuxuy(upuxuyAmount, price);
        }
        if (salingConfs[round].UpuxuySupply > 0 && totalSoldOfRound[round] + upuxuyAmount > salingConfs[round].UpuxuySupply) {
            usdtAcountBack += _usdtOfUpuxuy(upuxuyAmount - (salingConfs[round].UpuxuySupply - totalSoldOfRound[round]), price);
            upuxuyAmount = salingConfs[round].UpuxuySupply - totalSoldOfRound[round];
        }
        if (capOfSaling > 0 && totalSold + upuxuyAmount > capOfSaling) {
            usdtAcountBack += _usdtOfUpuxuy(upuxuyAmount - (capOfSaling - totalSold), price);
            upuxuyAmount = capOfSaling - totalSold;
        }
        return (isVip, price, ratio, usdtAmount, upuxuyAmount, usdtAcountBack);
    }
    ...
    function claim(uint8 round) public {
        ...
        (bool isVip, uint256 price, uint256 ratio, uint256 usdtAmount, uint256 upuxuyAmount, uint256 usdtAcountBack) = beClaimed(round, _msgSender());
        if (usdtAmount == 0) {
        ...
        if (usdtAcountBack > 0 && !Usdt.transfer(_msgSender(), usdtAcountBack)) {
            revert("fail to transfer usdt");
        }
        emit Claimed(_msgSender(), round, usdtAmount, upuxuyAmount, isVip, price, usdtAcountBack, ratio); 
    }
}