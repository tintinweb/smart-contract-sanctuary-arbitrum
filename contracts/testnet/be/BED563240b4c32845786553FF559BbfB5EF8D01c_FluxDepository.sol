...
contract FluxDepository is UUPSUpgradeable, OwnableUpgradeable, FluxDepositoryStorage {
    ...
    function redeem(address token, uint256 amountToRedeem) external /* onlyOwner */returns (uint256) {
        ...
        return fToken.redeem(amountToRedeem);
    }

    function withdraw(address token, uint256 redeemAmount) external /* onlyOwner */ returns (uint256) {
        if (token != assetToken()) {
            revert InvalidAsset(token);
        }
        return fToken.redeemUnderlying(redeemAmount);
    }
