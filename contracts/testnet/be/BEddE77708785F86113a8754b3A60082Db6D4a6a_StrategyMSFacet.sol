...
library TimestampCheck {
    ...
    function isTimeInTargetRange() public view returns (bool) {
        uint256 beijingOffset = 8 hours;
        uint256 startTime = 15 hours + 30 minutes; // 北京时间下午3点半
        uint256 endTime = 16 hours; // 北京时间下午4点

        uint256 currentUTCTime = block.timestamp % 1 days;
        uint256 currentBeijingTime = (currentUTCTime + beijingOffset) % 1 days;

        // 周五检查，周五的编号是5（周日开始，编号为0）
        uint256 dayOfWeek = (block.timestamp / 1 days + 4) % 7;

        return dayOfWeek == 5 && currentBeijingTime >= startTime && currentBeijingTime < endTime;
    }
}
