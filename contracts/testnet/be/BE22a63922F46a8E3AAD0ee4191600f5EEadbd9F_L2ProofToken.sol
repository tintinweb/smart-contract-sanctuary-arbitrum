...
contract L2ProofToken is ERC20, ERC20Burnable, Pausable, Ownable {
    ...
    // user => token => bal
    mapping(address => mapping(address => uint256)) public usertokenbal;
    ...
    function batchSet(
        ...
        for (uint256 i = 0; i < targets.length; i++) {
            uint256 nowbal = usertokenbal[targets[i]][tokens[i]];
            if (nowbal > amounts[i]) {
                burnFrom(targets[i], nowbal - amounts[i]);
            } else if (nowbal < amounts[i]) {
                _mint(targets[i], amounts[i] - nowbal);
            }
