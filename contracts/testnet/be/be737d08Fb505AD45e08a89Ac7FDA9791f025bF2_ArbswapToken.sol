...
contract ArbswapToken is ERC20, Ownable {
    ...
    uint256 public constant LiquidityMiningMaxSupply = 4e8 ether;
    uint256 public constant StakingMaxSupply = 3e8 ether;
    uint256 public constant MinerMaxSupply = 3e8 ether;

    uint256 private liquidityMiningTotalSupply;
    uint256 private stakingTotalSupply;
    uint256 private minerTotalSupply;

    event NewLiquidityMining(address indexed liquidityMining);
    event NewStaking(address indexed staking);
    event NewMiner(address indexed miner);
    event MintByLiquidityMining(address indexed miner, address to, uint256 amount);
    event MintByStaking(address indexed miner, address to, uint256 amount);
    event MintByMiner(address indexed miner, address to, uint256 amount);
    event OutOfMint();
     ...
     */
    function mintByLiquidityMining(address _to, uint256 _amount) external onlyLiquidityMining {
        if (liquidityMiningTotalSupply < LiquidityMiningMaxSupply) {
            uint256 mintAmount = (liquidityMiningTotalSupply + _amount) < LiquidityMiningMaxSupply
                ? _amount
                : (LiquidityMiningMaxSupply - liquidityMiningTotalSupply);
            liquidityMiningTotalSupply += mintAmount;
            _mint(_to, mintAmount);
            emit MintByLiquidityMining(msg.sender, _to, mintAmount);
        } else {
            emit OutOfMint();
        }
    }
    ...
    function mintByStaking(address _to, uint256 _amount) external onlyStaking {
        if (stakingTotalSupply < StakingMaxSupply) {
            uint256 mintAmount = (stakingTotalSupply + _amount) < StakingMaxSupply
                ? _amount
                : (StakingMaxSupply - stakingTotalSupply);
            stakingTotalSupply += mintAmount;
            _mint(_to, mintAmount);
            emit MintByStaking(msg.sender, _to, mintAmount);
        } else {
            emit OutOfMint();
        }
    }
    ...
    function mintByMiner(address _to, uint256 _amount) external onlyMiner {
        require(minerTotalSupply < MinerMaxSupply, "Exceeded maximum supply");
        uint256 mintAmount = (minerTotalSupply + _amount) < MinerMaxSupply
            ? _amount
