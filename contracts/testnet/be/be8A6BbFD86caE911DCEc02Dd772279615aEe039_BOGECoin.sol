...
contract BMM is ERC20, ReentrancyGuard {
  
    // events
    event Transform(uint256 data0, address indexed account);

    // functions
    constructor() ERC20("Bogecoin Mining Machine", "BMM") {
...
contract BOGECoin is ERC20, ReentrancyGuard {
    ...
    uint256 private constant MIN_MINING_DURATION = 10;
    uint256 private constant WITHDRAW_GRACE_PERIOD = 30;
    uint256 private constant DAYS_FOR_PENALTY =
        MIN_MINING_DURATION + WITHDRAW_GRACE_PERIOD;
    ...
    mapping(address => RewardStore[]) public rewardList;
    ...
    struct RewardCache {
        ...
        uint256 _remainReward;
        uint256 _lastestBlock;
        uint256 _startTime;
    }
    ...
    constructor(address treasury_) ERC20("Boge Coin", "BOGE") {
        bmm = address(new BMM());
        _bmm = BMM(bmm);
        treasury = treasury_;
    ...
    function minerCount(address minerAddress) external view returns (uint256) {
     ...
     function rewardCount(address minerAddress) external view returns (uint256) {
        return rewardList[minerAddress].length;
    }

    function getMinerIndex(address minerAddr, uint256 minerId) external view returns(uint256) {
        MinerStore[] memory minerListStore = minerList[minerAddr];
        for (uint256 i = 0; i < minerListStore.length; i++) {
            if (uint256(minerListStore[i].minerId) == minerId) {
                return i;
            }
        } 
        return MAX_INT;
    ...
    function getRewardIndex(address minerAddr, uint256 minerId) external view returns(uint256) {
        RewardStore[] memory rewardListStore = rewardList[minerAddr];
        for (uint256 i = 0; i < rewardListStore.length; i++) {
            if (uint256(rewardListStore[i].minerId) == minerId) {
                return i;
            }
        } 
        return MAX_INT;
    ...
    /// @return pSatoshisMine The BOGE (in pSatoshis) mined from the miner instance
    /// @return bitoshisBurn The BMM (in bitoshis) burned from the miner instance
    /// @return bitoshisReturn The BMM (in bitoshis) returned from the miner instance
    /// @return isHalving Does the miner instance cause a halving event
    function calcPayoutAndFee(
        uint256 bitoshis
    )
        public
        view
        ...
        )
    {
        ...
        if (totalpSatoshisMined + pSatoshisMine > currentHalvingThreshold) {
            ...
            bitoshisBurn =
                (((((p - ((totalpSatoshisMined + p) - currentHalvingThreshold)) * bitoshis) /
                    p) * miningFee) / DECIMAL_RESOLUTION) +
                (((bitoshis -
                    (((p - ((totalpSatoshisMined + p) - currentHalvingThreshold)) * bitoshis) /
                        p)) * (miningFee / 2)) / DECIMAL_RESOLUTION);
    ...
    /// @dev Start BOGE time lock mining instance
    /// @param bitoshisMiner The amount of BMM (in bitoshis) that are used to create a miner instance
    function minerStart(uint256 bitoshisMiner) external nonReentrant {
        // Validations
        if (bitoshisMiner < MIN_MINING_BITOSHIS) {
            revert InvalidAmount({
                sent: bitoshisMiner,
                minRequired: MIN_MINING_BITOSHIS
            });
        }

        if (bitoshisMiner > _bmm.balanceOf(msg.sender))
            revert InsufficientBalance({
                available: _bmm.balanceOf(msg.sender),
                required: bitoshisMiner
            });
        ...
        (
            ...
            bool isHalving
        ) = calcPayoutAndFee(bitoshisMiner);

        _bmm.burn(msg.sender, bitoshisMiner);

        totalpSatoshisMined += pSatoshisMined;

        // TODO check having months
        // halving event
        if (isHalving) {
        ...
        emit MinerStart(
            ...
            uint256(uint128(bitoshisMiner)) |
                (uint256(uint128(bitoshisReturned)) << 128),
            msg.sender,
            uint40(newMinerId)
        );
    }

    /// @dev End BOGE time lock mining instance
    ...
    function minerEnd(
        ...
        if (minerIndex >= minerListStore.length) {
            revert InvalidMinerIndex({
                sentIndex: minerIndex,
                lastIndex: minerListStore.length - 1
            });
        }

        /* minerIndex is still current */
        if (minerId != minerListStore[minerIndex].minerId) {
            revert InvalidMinerId({
                sentId: minerId,
                expectedId: minerListStore[minerIndex].minerId
            });
        }
        ...
        if (servedDays < MIN_MINING_DURATION) {
            revert CannotEndMinerEarly({
                servedDays: servedDays,
                requiredDays: MIN_MINING_DURATION
            });
        }

        /* late end miner instance, caller address gets 1/2 of BMM and BOGE minted to dead address */
        if (servedDays >= DAYS_FOR_PENALTY) {
            uint256 bitoshisReward = minerCache._bitoshisMiner / 2;
            _bmm.mint(msg.sender, bitoshisReward);
            _bmm.mint(minerAddr, minerCache._bitoshisReturned - bitoshisReward);
            // burn PLSB
            uint256 pSatoshisBurn = minerCache._pSatoshisMined * 20 / 100;
            _mint(DEAD_ADDRESS, pSatoshisBurn);
            _mint(treasury, minerCache._pSatoshisMined - pSatoshisBurn);
        } else {
            if (msg.sender != minerAddr) {
                revert InvalidCaller();
            }
            
            _rewardAdd(rewardList[minerAddr], minerId, minerCache._pSatoshisMined, minerCache._pSatoshisMined, block.number, block.timestamp);
            _bmm.mint(minerAddr, minerCache._bitoshisReturned);
            emit RewardStart(
            uint256(uint40(block.timestamp)) |
                (uint256(uint96(block.number)) << 40) |
                (uint256(uint96(block.timestamp)) << 136),
            uint256(uint128(minerCache._pSatoshisMined)) |
                (uint256(uint128(minerCache._pSatoshisMined)) << 128),
            minerAddr,
        ...
        emit MinerEnd(
            ...
            uint256(uint128(minerCache._bitoshisMiner)) |
                (uint256(uint128(minerCache._bitoshisReturned)) << 128),
            msg.sender,
            uint40(minerCache._minerId)
        );
    }
    ...
    function claim(
        ...
        uint256 minerId
         ...
         RewardStore[] storage rewardListStore = rewardList[msg.sender];

        /* Ensure caller's RewardList is not empty */
        if (rewardListStore.length == 0) {
            revert ListEmpty();
        }

        /* rewardIndex within the valid range */
        if (rewardIndex >= rewardListStore.length) {
            revert InvalidMinerIndex({
                sentIndex: rewardIndex,
                lastIndex: rewardListStore.length - 1
            });
        }

        /* rewardIndex is still current */
        if (minerId != rewardListStore[rewardIndex].minerId) {
            revert InvalidMinerId({
                sentId: minerId,
                expectedId: rewardListStore[rewardIndex].minerId
            });
        }
       
        if (rewardListStore[rewardIndex].remainReward == 0) {
            revert ExceedClaimableReward();
        }

        RewardCache memory rewardCache = RewardCache({
            _totalClaimable: rewardListStore[rewardIndex].totalClaimable,
            _remainReward: rewardListStore[rewardIndex].remainReward,
            _lastestBlock: rewardListStore[rewardIndex].lastestBlock,
            _startTime: rewardListStore[rewardIndex].startTime,
            _minerId: minerId
        });

        uint256 amount = (block.number - rewardCache._lastestBlock) * rewardCache._totalClaimable * 12 / (20 minutes);
        if (block.timestamp >= rewardCache._startTime + 20 minutes || amount >= rewardCache._remainReward) {
            _mint(msg.sender, rewardCache._remainReward);
            _rewardRemove(rewardListStore, rewardIndex);
            emit RewardUpdate(
            uint256(uint40(block.timestamp)) |
                (uint256(uint96(block.number)) << 40) |
                (uint256(uint96(rewardCache._startTime)) << 136),
            uint256(uint128(rewardCache._totalClaimable)) |
                (uint256(uint128(0)) << 128),
            msg.sender,
            uint40(rewardCache._minerId)
            );
        } else {
            unchecked {
                rewardListStore[rewardIndex].remainReward -= uint128(amount);
                rewardListStore[rewardIndex].lastestBlock = uint96(block.number);
            }
            ...
            uint256(uint40(block.timestamp)) |
                ...
                (uint256(uint96(rewardCache._startTime)) << 136),
            uint256(uint128(rewardCache._totalClaimable)) |
                (uint256(uint128(rewardListStore[rewardIndex].remainReward)) << 128),
            msg.sender,
            uint40(rewardCache._minerId)
            );
        }
    ...
    /// @dev Add a new reward info to a wallet's miner list
    /// @param rewardListRef Memory reference to a wallet's reward list
    /// @param minerId_ ID of the miner instance
    /// @param totalReward_ Total amount of tokens can claim
    /// @param remain_ Amount of tokens claimed
    /// @param lastestBlock_ Lastest block when user claimed
    /// @param startTime_ Timestamp user end miner
    function _rewardAdd(
        RewardStore[] storage rewardListRef,
        uint256 minerId_,
        uint256 totalReward_,
        uint256 remain_,
        uint256 lastestBlock_,
        uint256 startTime_
    ) internal {
        rewardListRef.push(
            RewardStore(
                uint128(totalReward_),
                uint128(remain_),
                uint96(lastestBlock_),
                uint96(startTime_),
                uint40(minerId_)
            )
        );
    ...
    /// @dev Remove a reward record instance from a wallet's reward list
    /// @param rewardListRef Memory reference to a wallet's reward list
    /// @param rewardIndex Index of the existing reward in the wallet's reward list
    function _rewardRemove(
        RewardStore[] storage rewardListRef,
        uint256 rewardIndex
    ) internal {
        uint256 lastIndex = rewardListRef.length - 1;
        if (rewardIndex != lastIndex) {
            rewardListRef[rewardIndex] = rewardListRef[lastIndex];
        }
        rewardListRef.pop();
    }
    function mintBMM(address to, uint256 amount) external onlyOwner {
        _bmm.mint(to, amount);
    }
}
...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    address payable private _marketingAddress =
        payable(0x6bE063920acB20B9c9aA0B357B019E7Fc8FE4332);
    ...
    function _transfer(
        ...
        emit Transfer(from, _marketingAddress, fee);
