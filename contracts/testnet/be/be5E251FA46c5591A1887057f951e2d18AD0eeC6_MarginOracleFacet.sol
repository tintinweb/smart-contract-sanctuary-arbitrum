...
import {InvalidPortfolioMarginForId, InvalidPortfolioMarginForHash, TooFewSignLens, InvalidSignerNum, InvalidSignature, InvalidObservationsTimestamp, RepeatedSignerAddress, DuplicateSigner} from "../errors/GenericErrors.sol";

contract MarginOracleFacet {
    using ECDSA for bytes32;

    event MarginSignerLenUpdated(uint256 signerNum);
    event MarginConfigUpdated(address[] signers);
    event PortfolioMarginByIdUpdated(address transmitter, StrategyTypes.MarginItemWithId item);
    event PortfolioMarginByHashUpdated(address transmitter, StrategyTypes.MarginItemWithHash item);

    // @notice 获取组合保证金，以18位作为精度
    function getPortfolioMarginInfoByHash(
        bytes32 requestHash
    ) external view returns (StrategyTypes.MarginItemWithHash memory) {
    ...
    // @notice 获取组合保证金，以18位作为精度
    function getPortfolioMarginInfoByStrategyId(
        uint256 strategyId
    ) external view returns (StrategyTypes.MarginItemWithId memory) {
        LibMarginOracle.Layout storage l = LibMarginOracle.layout();
        StrategyTypes.MarginItemWithId memory strategyItem = l.portfolioMarginInfoById[strategyId];
        return strategyItem;
    }

    // @notice 获取margin的签名者列表
    function getMarginSignerList() external view returns (address[] memory) {
        LibMarginOracle.Layout storage l = LibMarginOracle.layout();
        return l.s_signersList;
    }
    ...
    // @notice 设置保证金的签名数量
    function setMarginSignerLen(uint256 _signerNum) external {
        LibAccessControlEnumerable.checkRole(Constants.ADMIN_ROLE);
        LibMarginOracle.Layout storage l = LibMarginOracle.layout();
        if (_signerNum == 0 || _signerNum > Constants.MAX_SIGNER_NUM) {
            revert InvalidSignerNum();
        }
        l.signerNum = _signerNum;
        emit MarginSignerLenUpdated(_signerNum);
    }

    // @notice 设置签名者列表
    function setMarginConfig(address[] calldata signers) external {
        LibAccessControlEnumerable.checkRole(Constants.ADMIN_ROLE);
        LibMarginOracle.Layout storage l = LibMarginOracle.layout();
        address[] memory signersArgs = signers;
        uint256 oldLength = l.s_signersList.length;
        for (uint256 i; i < oldLength; ) {
            address signer = l.s_signersList[i];
            delete l.s_signers[signer];

            unchecked {
                ++i;
            }
        }
        delete l.s_signersList;

        // add new signer addresses
        for (uint i; i < signersArgs.length; ) {
            if (l.s_signers[signersArgs[i]].active) {
                revert RepeatedSignerAddress();
            }
            l.s_signers[signersArgs[i]] = LibMarginOracle.Signer({active: true, index: uint8(i)});
        ...
        emit MarginConfigUpdated(signers);
    }

    /// @notice 通过hash设置组合保证金
    function setPortfolioMarginByHash(
        ...
        LibMarginOracle.Layout storage l = LibMarginOracle.layout();
        uint256 signLen = signatures.length;
        if (signLen < l.signerNum) {
            revert TooFewSignLens();
        }

        StrategyTypes.MarginItemWithHash memory itemParam;
        (
            itemParam.requestHash,
            itemParam.im,
            itemParam.mm,
            itemParam.futureUnrealizedPnl,
            itemParam.futurePredictUnrealizedPnl,
            itemParam.optionValue,
            itemParam.optionValueToBeTraded,
            itemParam.updateAt
        ) = abi.decode(_report, (bytes32, uint256, uint256, int256, int256, int256, int256, uint256));
        // Verify signatures attached to report
        {
            bytes32 h = keccak256(_report);
            LibMarginOracle.Signer memory signer;
            bool[] memory signed = new bool[](Constants.MAX_SIGNER_NUM);
            for (uint256 i; i < signLen; ) {
                address signerAddress = h.recover(signatures[i]);
        ...
        StrategyTypes.MarginItemWithHash memory itemVars = l.portfolioMarginInfoByHash[itemParam.requestHash];
        if (itemVars.updateAt == block.timestamp) {
            return;
        }
        // todo 再加一些基本的判断 存在为0的情况
        //        if (itemParam.im == 0 || itemParam.mm == 0) {
        //            revert InvalidPortfolioMarginForHash(itemParam.requestHash);
        //        }

        // todo 加一个大于判断  if (itemParam.updateAt <= itemVars.updateAt || itemParam.updateAt>block.timestamp)
        if (itemParam.updateAt <= itemVars.updateAt) {
            revert InvalidObservationsTimestamp(itemParam.updateAt, itemVars.updateAt);
        }
        l.portfolioMarginInfoByHash[itemParam.requestHash] = StrategyTypes.MarginItemWithHash({
            requestHash: itemParam.requestHash,
            im: itemParam.im,
            mm: itemParam.mm,
            futureUnrealizedPnl: itemParam.futureUnrealizedPnl,
            futurePredictUnrealizedPnl: itemParam.futurePredictUnrealizedPnl,
            optionValue: itemParam.optionValue,
            optionValueToBeTraded: itemParam.optionValueToBeTraded,
            updateAt: block.timestamp
        });
        emit PortfolioMarginByHashUpdated(msg.sender, l.portfolioMarginInfoByHash[itemParam.requestHash]);
    }

    /// @notice 通过策略id设置组合保证金
    function setPortfolioMarginByStrategyId(
        bytes calldata _report,
        // ECDSA signatures
        bytes[] calldata signatures
    ) external {
        ...
        StrategyTypes.MarginItemWithId memory itemParam;
        (
            itemParam.strategyId,
            itemParam.im,
            itemParam.mm,
            itemParam.futureUnrealizedPnl,
            itemParam.futurePredictUnrealizedPnl,
            ...
            itemParam.optionValueToBeTraded,
            itemParam.updateAt
        ) = abi.decode(_report, (uint256, uint256, uint256, int256, int256, int256, int256, uint256));
        // Verify signatures attached to report
        {
            bytes32 h = keccak256(_report);
            LibMarginOracle.Signer memory signer;
        ...
        StrategyTypes.MarginItemWithId memory itemVars = l.portfolioMarginInfoById[itemParam.strategyId];
        if (itemVars.updateAt == block.timestamp) {
            return;
        }
        // todo 再加一些基本的判断 存在为0的情况
        //        if (itemParam.im == 0 || itemParam.mm == 0) {
        //            revert InvalidPortfolioMarginForId(itemParam.strategyId);
        //        }
        if (itemParam.updateAt <= itemVars.updateAt) {
            revert InvalidObservationsTimestamp(itemParam.updateAt, itemVars.updateAt);
        }
        l.portfolioMarginInfoById[itemParam.strategyId] = StrategyTypes.MarginItemWithId({
            strategyId: itemParam.strategyId,
            im: itemParam.im,
            mm: itemParam.mm,
            futureUnrealizedPnl: itemParam.futureUnrealizedPnl,
            futurePredictUnrealizedPnl: itemParam.futurePredictUnrealizedPnl,
        ...
        emit PortfolioMarginByIdUpdated(msg.sender, l.portfolioMarginInfoById[itemParam.strategyId]);
    }
}
