...
contract UXSwapV1 {
    ...
    // Commission in percentage (e.g., 1 for 1%)
    ...
    function pairInfo(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB, uint totalSupply) {
        IUniswapV2Pair pair = IUniswapV2Pair(UniswapV2Library.pairFor(routerFactory, tokenA, tokenB));
    ...
    function computeLiquidityShareValue(uint liquidity, address tokenA, address tokenB) external view returns (uint tokenAAmount, uint tokenBAmount) {
        ...
        // Calculate the share of reserves based on the liquidity provided
    ...
    function trade(
        ...
        // Collect commission
        if (commission > 0) {
            SafeERC20.safeTransfer(IERC20(tokenIn), revCommissionWallet, commission);
        }

        emit TradeSuccess(msg.sender,tokenIn,tokenOut,amountIn,amountOutMin,to,commission);
    }
    ...
    function tradeETH(
        ...
        // Perform the swap
        uint256 deadline = block.timestamp + deadlineDelayTime;
        uniswapRouter.swapExactETHForTokens{value: amountInAfterCommission}(
            amountOutMin,
            path,
            to,
            deadline
        ...
        emit TradeSuccess(msg.sender,uniswapRouter.WETH(),tokenOut,amountIn,amountOutMin,to,commission);
    }
    ...
    function addToBlacklist(address _user) public isSuperOperator {
        ...
        require(
            _user != address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D), 
    ...
    function setRouterFactory(address _factory) external isSuperOperator {
        routerFactory = _factory;
    }
