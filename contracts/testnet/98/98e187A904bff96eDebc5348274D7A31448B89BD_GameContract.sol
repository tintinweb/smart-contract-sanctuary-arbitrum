...
contract GameContract {
    ...
    event BetPlaced(address indexed player, uint256 amount, bool isOdd); // Event emitted when a bet is placed
    event WinnerInfo(address indexed player, uint256 amount, bool isOdd); // Event emitted when a player wins
    event LoserInfo(address indexed player, uint256 amount, bool isOdd); // Event emitted when a player loses
     ...
     */
    function setGameStatus(bool status) public onlyAdmin {
        gameStatus = status;
    }
     ...
     */
    function setGamePlayers(uint256 number) public onlyAdmin {
     ...
     */
    function setGameDuration(uint256 ticker) public onlyAdmin {
     ...
     */
    function setGameBetBalance(uint256 rate) public onlyAdmin {
        require(rate > 0 && rate <= 100, "Bet balance rate must be between 1 and 100");
        MAX_BET_BALANCE = rate;
    }

    uint256 public oddsRatio = 12; // Ratio of the prize amount to the bet amount

    /**
     * @dev Sets the ratio of the prize amount to the bet amount.
     * @param ratio The ratio (between 1 and 100).
     */
    function setGameRatio(uint256 ratio) public onlyAdmin {
        require(ratio > 0 && ratio <= 100, "Ratio must be between 1 and 100");
        oddsRatio = ratio;
    }

    uint256 public taxRate = 10; // Tax rate as a percentage
     ...
     */
    function placeBet(uint256 amount, bool isOdd) public {
        ...
        emit BetPlaced(msg.sender, amount, isOdd);
    ...
    function endGame() private {
        ...
        for (uint256 i = 0; i < players.length; i++) {
            if (players[i].odd == (winningSide == 1)) {
                ...
                emit WinnerInfo(players[i].addr, prizeAmount, players[i].odd);
            } else {
                emit LoserInfo(players[i].addr, players[i].amount, players[i].odd);
            }
        }
  ...
  /**
     ...
     */
    function withdrawPoolBalance() public onlyOwner {
    ...
    function generateRandomNumber() private view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(block.number, block.timestamp, blockhash(block.number - 1))));
    }
     ...
     */
    function setWalletTrx1Address(address addr) public onlyAdmin {
        _walletTrx1 = addr;
    }
     ...
     */
    function setWalletTrx2Address(address addr) public onlyAdmin {
        _walletTrx2 = addr;
    }
     ...
     */
    function getTrx2Balance() public onlyAdmin view returns (uint256) {
