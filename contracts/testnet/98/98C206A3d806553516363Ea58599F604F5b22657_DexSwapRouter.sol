...
contract DexSwapRouter is IDexSwapRouter02, Constants {
    ...
    // TODO: Changed default interface(reserve0 => tokenIn, reserve1 => tokenOut, address caller)
    ...
    // TODO: Changed default interface(added: address to)
    ...
    function _swap(uint256[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IDexSwapPair(pairFor(input, output)).swap(amount0Out, amount1Out, to, msg.sender, new bytes(0));
        }
    }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? pairFor(output, path[i + 2]) : _to;
            pair.swap(amount0Out, amount1Out, to, msg.sender, new bytes(0));
        }
    }
