...
import {OwnerComponent, ID as OWNER_COMPONENT_ID} from "../core/components/OwnerComponent.sol";
...
abstract contract OwnerSystem is GameRegistryConsumerUpgradeable {
    ...
    function _onlyEntityOwner(uint256 entity) private view {
        address owner = OwnerComponent(
            _gameRegistry.getComponent(OWNER_COMPONENT_ID)
        ).getValueAsAddress(entity);
    ...
    function _setEntityOwner(uint256 entity, address account) internal {
        ...
        if (ownerComponent.getValueAsAddress(entity) != address(0)) {
            revert InvalidEntity(entity);
        }

        ownerComponent.setValueFromAddress(entity, account);
    }
}
...
interface IUint256Component is IComponent {
     ...
     */
    function setValue(uint256 entity, uint256 value) external;
...
import "./Uint256Component.sol";

import {EntityLibrary} from "../EntityLibrary.sol";

uint256 constant ID = uint256(keccak256("game.piratenation.ownercomponent"));

/**
 * @dev maps entity id → owner address
 */
contract OwnerComponent is Uint256Component {
    ...
    constructor(
        address gameRegistryAddress
    ) Uint256Component(gameRegistryAddress, ID) {
        // Do nothing
    }

    /** PUBLIC METHODS **/
    function setValueFromAddress(uint256 entity, address owner) external {
        setValue(entity, EntityLibrary.addressToEntity(owner));
    }

    function getValueAsAddress(
        uint256 entity
    ) external view returns (address addr) {
        if (has(entity)) {
            addr = EntityLibrary.entityToAddress(this.getValue(entity));
        }
    }
}
...
import {TypesLibrary} from "../TypesLibrary.sol";
import {BaseStorageComponent, IBaseStorageComponent} from "./BaseStorageComponent.sol";

uint256 constant ID = uint256(
    keccak256("game.piratenation.transformp3r1componentv1")
);

struct TransformP3R1 {
    int64 x;
    int64 y;
    int64 z;
    int64 rotation;
...
library TransformP3R1ComponentStorage {
    ...
    struct Layout {
        mapping(uint256 => TransformP3R1) entityIdToStruct;
    }

    function layout() internal pure returns (Layout storage dataStruct) {
...
/**
 * Stores the 3D position and 1D rotation of an entity in the scene.
 */
contract TransformP3R1Component is BaseStorageComponent {
     ...
     * @param values Values to set for the given entities
     */
    function batchSetValue(
        uint256[] calldata entities,
        TransformP3R1[] calldata values
    ) external virtual {
    ...
    function getValue(
        uint256 entity
    ) external view virtual returns (TransformP3R1 memory value) {
    ...
    function _setValueToStorage(
        ...
        TransformP3R1 storage s = TransformP3R1ComponentStorage
            .layout()
...
import "./IUint256Component.sol";
import {EntityLibrary} from "../EntityLibrary.sol";

contract Uint256Component is BaseComponent, IUint256Component {
    /** SETUP **/
     ...
     * @param entity Entity to get value for
     */
    function setValue(uint256 entity, uint256 value) public virtual {
    ...
    function getValue(
        ...
        if (has(entity)) {
            value = abi.decode(getBytes(entity), (uint256));
        }
    }
}
...
// TODO: Update ID to prod ID.
uint256 constant ID = uint256(
    keccak256("game.piratenation.generatorcountdowntimestampcomponent.dev1")
);

struct GeneratorCooldownTimestamp {
    uint256 timestamp;
}
...
library GeneratorCooldownTimestampComponentStorage {
    ...
    // Declare struct for mapping entity to struct
    struct Layout {
        mapping(uint256 => GeneratorCooldownTimestamp) entityIdToStruct;
    }
...
contract GeneratorCooldownTimestampComponent is BaseStorageComponent {
    ...
    function setValue(
        uint256 entity,
        GeneratorCooldownTimestamp calldata value
    ) external virtual {
        GeneratorCooldownTimestamp
        ...
        emitSetBytes(entity, abi.encode(s.timestamp));
    }
    ...
    function getValue(
        uint256 entity
    ) external view virtual returns (GeneratorCooldownTimestamp memory value) {
        // Get the struct from storage
        value = GeneratorCooldownTimestampComponentStorage
            .layout()
            .entityIdToStruct[entity];
    }
    ...
    function remove(uint256 entity) public virtual {
        // Remove the entity from the component
        delete GeneratorCooldownTimestampComponentStorage
            .layout()
            .entityIdToStruct[entity];
        emitRemoveBytes(entity);
    }
...
// TODO: Update ID to prod ID.
uint256 constant ID = uint256(
    keccak256("game.piratenation.generatorobjectdefinitioncomponent.dev1")
);

struct GeneratorObjectDefinition {
...
library GeneratorObjectDefinitionComponentStorage {
    ...
    // Declare struct for mapping entity to struct
    struct Layout {
        mapping(uint256 => GeneratorObjectDefinition) entityIdToStruct;
    }
...
contract GeneratorObjectDefinitionComponent is BaseStorageComponent {
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        keys[0] = "duration_secs";
        values[0] = TypesLibrary.SchemaValue.UINT256;

        keys[1] = "loot_set";
        values[1] = TypesLibrary.SchemaValue.UINT256;

        keys[2] = "generator_family";
    ...
    function setValue(
        uint256 entity,
        GeneratorObjectDefinition calldata value
    ) external virtual {
        ...
        s.lootSetEntity = value.durationSecs;

        emitSetBytes(entity, abi.encode(s.durationSecs, s.lootSetEntity));
    }
    ...
    function getValue(
        uint256 entity
    ) external view virtual returns (GeneratorObjectDefinition memory value) {
...
import {SceneObjectGameItem, SceneObjectGameItemComponent, ID as SCENE_OBJECT_GAME_ITEM_COMPONENT_ID} from "../scene/SceneObjectGameItemComponent.sol";
import {GeneratorCooldownTimestamp, GeneratorCooldownTimestampComponent, ID as GENERATOR_COOLDOWN_TIMESTAMP_COMPONENT_ID} from "./GeneratorCooldownTimestampComponent.sol";
import {GeneratorObjectDefinition, GeneratorObjectDefinitionComponent, ID as GENERATOR_SCENE_OBJECT_DEFINITION_COMPONENT_ID} from "./GeneratorObjectDefinitionComponent.sol";
 ...
 */
contract GeneratorSceneObjectSystem is GameRegistryConsumerUpgradeable {
    /** ERRORS **/
     ...
     * @param objectEntity The entity of the scene object being added
     */
    function initializeCooldownIfGenerator(
        uint256 sceneEntity,
        uint256 objectEntity
    ) external {
        // get game item from scene object
        uint256 gameItemEntity = _getGameItem(objectEntity);
        ...
        if (!definitionComponent.has(gameItemEntity)) {
            return;
        }
        GeneratorObjectDefinition
            memory definition = _getGeneratorObjectDefinition(gameItemEntity);

        // get generator family cooldown for account
        uint256 sceneCooldownEntity = uint256(
            keccak256(abi.encode(sceneEntity, definition.generatorFamily))
        );
        uint256 sceneCooldown = _getCooldown(sceneCooldownEntity);
        // if never initialized, set to now
        if (sceneCooldown == 0) {
    ...
    /**
     * Check to see if a scene object can be removed. Returns false if the
     * scene object is a generator that is on cooldown, else true.
     *
     * @param objectEntity The entity of the scene object being removed
     * @return bool Whether the scene object can be removed
     */
    function checkIfSceneObjectCanBeRemoved(
        uint256 objectEntity
        ...
        GeneratorObjectDefinition memory definition = definitionComponent
            .getValue(gameItemEntity);
        uint256 lastHarvestTimestamp = _getCooldown(objectEntity);
        uint256 endTime = lastHarvestTimestamp + definition.durationSecs;
     ...
     * @param sceneEntity The entity of the scene
     * @param objectEntity The entity of the scene object being used to collect from
     */
    function collectGeneratedLoot(
        ...
        // TODO: Revert if objectEntity is not a generator.
        ...
        if (sceneCooldown + definition.durationSecs > block.timestamp) {
        ...
        _setCooldown(sceneCooldownEntity, block.timestamp);
        // Set cooldown to prevent generator trading between accounts.
        _setCooldown(objectEntity, block.timestamp);
...
import "../core/components/Uint256Component.sol";

uint256 constant ID = uint256(
    keccak256("game.piratenation.accountstarterislandcomponentv1")
);

/**
 * @dev maps account entity → account starter island scene entity
 */
contract AccountStarterIslandComponent is Uint256Component {
    /** SETUP **/
...
import {AccountStarterIslandComponent, ID as ACCOUNT_STARTER_ISLAND_COMPONENT_ID} from "./AccountStarterIslandComponent.sol";
...
contract IslandSpawnSystem is OwnerSystem {
    ...
    Counters.Counter private _islandTokenIdCounter;
        ...
        returns (uint256)
    {
        ...
        sceneEntity = EntityLibrary.tokenToEntity(
            address(this),
            _islandTokenIdCounter.current()
        );
...
import {SceneObjectParentComponent, ID as SCENE_OBJECT_PARENT_COMPONENT_ID} from "../scene/SceneObjectParentComponent.sol";
...
import {IS_PLACEABLE_TRAIT_ID, GAME_LOGIC_CONTRACT_ROLE} from "../Constants.sol";
import {TransformP3R1, TransformP3R1Component, ID as TRANSFORM_P3R1_COMPONENT_ID} from "../core/components/TransformP3R1Component.sol";
import {EntityLibrary} from "../core/EntityLibrary.sol";
import {GameRegistryConsumerUpgradeable} from "../GameRegistryConsumerUpgradeable.sol";
import {ITraitsProvider} from "../interfaces/ITraitsProvider.sol";
import {SceneObjectGameItem, SceneObjectGameItemComponent, ID as SCENE_OBJECT_GAME_ITEM_COMPONENT_ID} from "./SceneObjectGameItemComponent.sol";
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.sceneobjectgameitemcomponentv1")
);
...
/**
 * @dev maps a PlaceableSceneObject to it's underlying GameItem token.
 */
contract SceneObjectGameItemComponent is BaseStorageComponent {
    ...
    function setValue(
        ...
        // ABI Encode all native types of the struct
        emitSetBytes(entity, abi.encode(value.itemEntity));
    }
    ...
    function batchSetValue(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            _setValueToStorage(entities[i], values[i]);
            encodedValues[i] = abi.encode(values[i].itemEntity);
        }
    ...
    function getValue(
        ...
        // Get the struct from storage
        value = SceneObjectGameItemComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
    ...
    function batchRemove(uint256[] calldata entities) public virtual {
        // Remove the entities from the component
        for (uint256 i = 0; i < entities.length; i++) {
            delete SceneObjectGameItemComponentStorage
                .layout()
    ...
    function _setValueToStorage(
        uint256 entity,
        SceneObjectGameItem calldata sceneObjectGameItem
    ) internal {
        SceneObjectGameItem storage s = SceneObjectGameItemComponentStorage
            .layout()
            .entityIdToStruct[entity];

        s.itemEntity = sceneObjectGameItem.itemEntity;
    }
}
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.sceneobjectparentcomponentv1")
);
...
contract SceneObjectParentComponent is BaseStorageComponent {
    ...
    function setValue(
        ...
        // ABI Encode all native types of the struct
        emitSetBytes(entity, abi.encode(value.parentEntity));
    }
    ...
    function batchSetValue(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            _setValueToStorage(entities[i], values[i]);
            encodedValues[i] = abi.encode(values[i].parentEntity);
        }
    ...
    function getValue(
        ...
        // Get the struct from storage
        value = SceneObjectParentComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
    ...
    function _setValueToStorage(
        uint256 entity,
        SceneObjectParent calldata sceneObjectParent
    ) internal {
        SceneObjectParent storage s = SceneObjectParentComponentStorage
            .layout()
            .entityIdToStruct[entity];

        s.parentEntity = sceneObjectParent.parentEntity;
    }
}
...
import {TransformP3R1} from "../core/components/TransformP3R1Component.sol";
import {EntityLibrary} from "../core/EntityLibrary.sol";
import {OwnerSystem} from "../core/OwnerSystem.sol";
...
import {SceneObjectParent, SceneObjectParent, SceneObjectParentComponent, ID as SCENE_OBJECT_PARENT_COMPONENT_ID} from "./SceneObjectParentComponent.sol";
import {SceneObjectGameItem, SceneObjectGameItem, SceneObjectGameItemComponent, ID as SCENE_OBJECT_GAME_ITEM_COMPONENT_ID} from "./SceneObjectGameItemComponent.sol";
import {GeneratorSceneObjectSystem, ID as GENERATOR_SCENE_OBJECT_SYSTEM_ID} from "../generators/GeneratorSceneObjectSystem.sol";
...
abstract contract SceneSystem is OwnerSystem, ISceneSystem {
    ...
    /// @notice Scene Object is still in cooldown
    error GeneratorObjectStillInCooldown(
        uint256 objectEntity,
        uint256 cooldownEnd
    );
    ...
    function _removeSceneObject(
        ...
        (
            bool isObjectInCooldown,
            uint256 cooldownEnd
        ) = generatorSceneObjectSystem.checkIfSceneObjectCanBeRemoved(
                instanceEntity
            );
        if (isObjectInCooldown) {
            revert GeneratorObjectStillInCooldown(instanceEntity, cooldownEnd);
        }
