...
contract Orderbook {
    ...
    function fillAsks(address receiver, uint256 _USDCPerOPTION, uint256 USDCsize, uint256 seatId) public returns(uint256) {
        ...
        while(askIndex < activeAsks.length) {
            uint256 i = askIndex;
            ...
            if (amountLeft <= USDCsize) {
                ...
                if(OPTION.balanceOf(address(activeAsks[i])) == 0) {
                    askIndex++;
                }
                USDCsize = 0;
                break;
    ...
    function fillBids(address receiver, uint256 _OPTIONPerUSDC, uint256 OPTIONsize, uint256 seatId) public returns(uint256) {
        ...
        while(bidIndex < activeBids.length) {
            uint256 i = bidIndex;
            ...
            if (amountLeft <= OPTIONsize) {
                ...
                if(USDC.balanceOf(address(activeBids[i])) == 0) {
                    bidIndex++;
                }
                OPTIONsize = 0;
                break;
    ...
    //     Ask[] memory activeAsks = new Ask[](asksAtPrice[_USDCPerOPTION].length);
    //     uint256 count;
    ...
    //     Bid[] memory activeBids = new Bid[](bidsAtPrice[_OPTIONPerUSDC].length);
    //     uint256 count;
    ...
    function getActiveAsks() public view returns (Ask[] memory) {
        uint256 length = asks.length - askIndex;
        Ask[] memory activeAsks = new Ask[](length);
        uint256 count;
        for (uint256 i; i < length; i++) {
            Ask ask = Ask(asks[askIndex + i]);
            if (ask.isFunded() && !ask.hasEnded()) {
                activeAsks[count] = ask;
                count++;
            }
    ...
    function getActiveBids() public view returns (Bid[] memory) {
        uint256 length = bids.length - bidIndex;
        Bid[] memory activeBids = new Bid[](length);
        uint256 count;
        for (uint256 i; i < bids.length; i++) {
            Bid bid = Bid(bids[bidIndex + i]);
