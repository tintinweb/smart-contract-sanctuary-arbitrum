...
library Validator {
    ...
    function verifyPublisher(
        ...
        bytes32 hashData = keccak256(
            abi.encode(
                keccak256(
                    "PublisherSignature(string memory assetPair,uint256 timestamp,uint256 price)"
                ),
    ...
    function verifyCloseAnytime(
        ...
        bytes32 hashData = keccak256(
            abi.encode(
                keccak256(
                    "CloseAnytimeSignature(string memory assetPair,uint256 timestamp,uint256 optionId)"
                ),
                keccak256(bytes(assetPair)),
                timestamp,
                optionId
