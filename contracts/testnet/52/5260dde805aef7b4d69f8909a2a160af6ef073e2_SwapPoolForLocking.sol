...
contract SwapPoolForLocking is Ownable {
    using SafeMath for uint256;
    using Address for address;
    using SafeERC20 for IERC20;
    bool public canSwap = false;
    bool public canClaim = false;
    address public blackHoleAddress = address(1);
    ...
    function SwapToken(uint256 _tokenId) external {
        ...
        SwapNFT.transferFrom(msg.sender, blackHoleAddress, _tokenId);
    ...
    function SwapTokenList(uint256[] memory _tokenIdList) external {
        ...
        for (uint256 i=0;i<_tokenIdList.length;i++) {
            ...
            SwapNFT.transferFrom(msg.sender, blackHoleAddress, _tokenIdList[i]);
