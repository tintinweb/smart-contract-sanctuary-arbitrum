...
contract horde is ERC721Omni {

    uint256 public constant usdcPrice = 4000000;
    uint256 public totalSupply;
    string private _baseURI = "ipfs://QmS84uLAUvGLverNnvyU8YhsHKJi6E3WnfvuD7qmRmBos2/";
    uint256 public gasForLzReceive = 900000;
    IUSDC public usdc = IUSDC(0x01BE23585060835E02B77ef475b0Cc51aA1e0709);

    constructor(address _lzEndpoint) ERC721Omni("Cyber", "Hunters") { 
        endpoint = ILayerZeroEndpoint(_lzEndpoint);//0x4D747149A57923Beb89f22E6B7B97f7D8c087A00
    }

    function mintToBSC(uint16 _chainId, uint256 _amount) public payable {
        require(trustedRemoteLookup[_chainId].length > 0, "This chain is currently unavailable for travel");
        uint256 totalPrice = usdcPrice*_amount;
        require(usdc.balanceOf(msg.sender)>=totalPrice, "Not enough USDC Owned");
        require(usdc.allowance(msg.sender, address(this)) >= totalPrice, "Insufficient allowed USDC");

        usdc.transferFrom(msg.sender, address(this), totalPrice);

        bytes memory payload = abi.encode(msg.sender, _amount);
        uint16 pack = 1;
        bytes memory adapterParams = abi.encodePacked(pack, gasForLzReceive);
