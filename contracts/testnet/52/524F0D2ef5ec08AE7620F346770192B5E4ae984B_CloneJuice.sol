...
contract CloneJuice is Context, ERC721A, Ownable, Pausable, ReentrancyGuard {
	using Strings for uint256;

	string public _contractBaseURI = "ipfs://bbbbbbbbbbbbbb/";
	string public _unrevealedURI = "ipfs/uuuuuuuuuuuuuuuuuu";
	string private _contractURI = "ipfs://xxxxxxxxxxxxxx";

	address public cloneMintingContract; //the future clone contract

	// Public sale params
	uint256 public publicSaleDuration = 6 days; //TODO: update me
	uint256 public publicSaleStartTime = block.timestamp; //TODO: update me
	uint256 public publicSaleJuiceStartingPrice = 0.1 ether; //TODO: update me

	// auction not less than 0.01 ETH
	uint256 public constant AUCTION_ENDING_PRICE = 0.01 ether;

	address private airdropAccount;

	//increased on the next juices
	uint256 public maxSupply = 12000; //TODO: update me

	constructor() ERC721A("Clone Juice", "JUICE") {
	 ...
	 */
	function buyJuice(uint256 qty) external payable whenNotPaused nonReentrant {
	...
	function getMintPrice() public view returns (uint256) {
		...
		if (elapsed >= publicSaleDuration) {
			return AUCTION_ENDING_PRICE;
		} else {
			uint256 currentPrice = ((publicSaleDuration - elapsed) * publicSaleJuiceStartingPrice) /
				publicSaleDuration;
			return currentPrice > AUCTION_ENDING_PRICE ? currentPrice : AUCTION_ENDING_PRICE;
		}
	}
	...
	* @param saleStartPrice - price of the sale
	 */
	function startPublicSale(uint256 saleDuration, uint256 saleStartPrice) external onlyOwner {
