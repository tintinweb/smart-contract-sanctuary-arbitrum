...
contract StakingRewards is Ownable, Initializable {
    ...
    function getAccountTotalSharePercentage(address account) external view returns (uint256 totalSharePercentage) {
        ...
        uint256 lodeAdjustedAmount = stakeInfo.lodeAmount * _getLockMultiplier(stakeInfo.lodeLockTime, stakeInfo.relockCount);
...
// File: contracts/VotingPower.sol
...
contract VotingPower {
    ...
    enum TokenType { USDC, USDT, FRAX, DAI, WBTC, ETH, MAGIC, DPX, ARB, PLVGLP }
    enum OperationType { SUPPLY, BORROW }
    ...
    mapping(TokenType => mapping(OperationType => uint256)) public totalVotes;

    event VoteCast(address indexed user, TokenType token, OperationType operation, uint256 shares);
    ...
    function vote(TokenType[] calldata tokens, OperationType[] calldata operations, uint256[] calldata shares) external {
        require(tokens.length == operations.length && tokens.length == shares.length, "Arrays must have the same length");

        uint256 userVotingPower = stakingRewards.getAccountTotalSharePercentage(msg.sender);
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            // Validate that the user can only vote for supply emissions for PLVGLP
            if (token == TokenType.PLVGLP) {
                require(operation == OperationType.SUPPLY, "Only supply emissions are allowed for PLVGLP");
            }
    ...
    function getResults() external view returns (TokenType[] memory, OperationType[] memory, uint256[] memory) {
        TokenType[] memory tokens = new TokenType[](10);
        OperationType[] memory operations = new OperationType[](2);
        uint256[] memory results = new uint256[](20);
