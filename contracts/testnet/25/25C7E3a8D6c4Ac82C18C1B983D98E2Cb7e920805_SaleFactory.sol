...
contract SaleFactory is Ownable {
    event SaleCreated(address newSaleAddr);

    address public saleContractAddr;
    address public stakingContractAddr;
    mapping(address => bool) public isCreatedByFactory;  // Sale address => is created by factory
    address[] public sales;

    constructor(address saleContractAddr_, address stakingContractAddr_) {
        saleContractAddr = saleContractAddr_;
        stakingContractAddr = stakingContractAddr_;
    }

    function createClone() public onlyOwner {
        address newSale = Clones.clone(saleContractAddr);

        require(newSale != address(0), "Failed to create sale");

        (bool success, ) = newSale.call(abi.encodeWithSignature("initialize(address,address,address)",  msg.sender, saleContractAddr, stakingContractAddr));

        require(success, "Initialization failed.");

        isCreatedByFactory[newSale] = true;
        sales.push(newSale);
        
        emit SaleCreated(newSale);
    }

    function updateSaleContract(address newSaleContractAddr) public onlyOwner {
        saleContractAddr = newSaleContractAddr;
    }
    ...
    function totalSalesNum() public view returns (uint256) {
        return sales.length;
    }
}
...
contract Staking is Ownable, Pausable {
    ...
    function stake(uint256 poolId, uint256 amount, StakingTime time) public whenNotPaused {
        ...
        if (poolId == 0) {
            ...
            if (uint8(tierToAcquire) > uint8(currentTier)) {
    ...
    function debit(uint256 amount) public onlyOwner {
        require(serviceBalance >= amount, "Not enough money");
        serviceBalance -= amount;
        raiseToken.safeTransferFrom(address(this), msg.sender, amount);
    }
