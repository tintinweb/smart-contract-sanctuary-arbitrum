...
contract GitUPayments is ERC721SignatureMint {
    mapping(address => string) private invoiceUrls;
    ...
    function sendFunds(
        ...
        string calldata invoiceUrl,
        uint256 invoiceAmount
    ) external payable {
        require(recipient != address(0), "Invalid recipient address");
        require(bytes(invoiceUrl).length > 0, "Invalid invoice URL");
        require(invoiceAmount > 0, "Invoice amount must be greater than zero");
        require(
            msg.value >= invoiceAmount,
            "Insufficient funds for the invoice"
        );

        (bool success, ) = recipient.call{value: invoiceAmount}("");
        require(success, "Transfer failed");

        invoiceUrls[msg.sender] = invoiceUrl;

        emit FundsSent(msg.sender, recipient, invoiceAmount, invoiceUrl);
    }

    function getInvoiceUrl(
        address account
    ) external view returns (string memory) {
        return invoiceUrls[account];
    }
