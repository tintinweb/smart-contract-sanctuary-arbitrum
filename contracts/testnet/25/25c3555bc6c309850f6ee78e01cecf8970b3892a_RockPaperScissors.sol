...
contract RockPaperScissors is Common {
    ...
    function RockPaperScissors_Play(
        ...
        if (action >= 3) {
            revert('RPS : Invalid action');
        }
        if (rockPaperScissorsGames[msgSender].requestID != 0) {
            revert('RPS : waiting for VRF');
        }
        if (!(numBets > 0 && numBets <= 100)) {
            revert('RPS : Invalid number of bets, must be between 1 and 100');
        }

        _kellyWager(wager, tokenAddress);
        uint256 fee = _transferWager(tokenAddress, wager * numBets, 800000, 22, msgSender);
        ...
        emit RockPaperScissors_Play_Event(msgSender, wager, tokenAddress, action, numBets, stopGain, stopLoss, fee);
    }
    ...
    function RockPaperScissors_Refund() external nonReentrant {
        ...
        if (game.requestID == 0) {
            revert('RPS : Not awaiting VRF');
        }
        if (game.blockNumber + 200 > block.number) {
            revert('RPS : block number too low to claim refund');
        }
        ...
        if (tokenAddress == address(0)) {
            revert('RPS : invalid token address');
        } else {
    ...
    function rawFulfillRandomWords(uint256 requestId, uint256[] memory randomWords) external {
        if (msg.sender != ChainLinkVRF) {
            revert('RPS : only coordinator can fulfill');
        }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal {
        ...
        for (i = 0; i < game.numBets; i++) {
            ...
            if (outcomes[i] == 2) {
                payout += (game.wager * 85) / 100;
                totalValue -= int256((game.wager) / 100);
                payouts[i] = (game.wager * 85) / 100;
                continue;
            }

            if (outcomes[i] == 1) {
                payout += (game.wager * 185) / 100;
                totalValue += int256((game.wager * 85) / 100);
                payouts[i] = (game.wager * 185) / 100;
                continue;
            }
    ...
    function _kellyWager(uint256 wager, address tokenAddress) internal view {
        ...
        if (wager > maxWager) {
            revert('RPS : wager exceeds maximum allowed');
        }
    }
}
