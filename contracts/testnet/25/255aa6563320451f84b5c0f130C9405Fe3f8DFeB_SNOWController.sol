...
import './abstract/SNOWOperatable.sol';
import './interfaces/ISNOWController.sol';
import './interfaces/ISNOWMigratable.sol';
import './interfaces/ISNOWOperatorStore.sol';
import './interfaces/ISNOWPaymentTerminal.sol';
import './interfaces/ISNOWProjects.sol';
import './libraries/SNOWConstants.sol';
import './libraries/SNOWFundingCycleMetadataResolver.sol';
import './libraries/SNOWOperations.sol';
import './libraries/SNOWSplitsGroups.sol';
...
/**
  ...
  @dev
  Adheres to -
  ISNOWController: General interface for the generic controller methods in this contract that interacts with funding cycles and tokens according to the protocol's rules.
  ISNOWMigratable: Allows migrating to this contract, with a hook called to prepare for the migration.

  @dev
  Inherits from -
  SNOWOperatable: Several functions in this contract can only be accessed by a project owner, or an address that has been preconfifigured to be an operator of the project.
  ERC165: Introspection on interface adherance. 
*/
contract SNOWController is SNOWOperatable, ERC165, ISNOWController, ISNOWMigratable {
  // A library that parses the packed funding cycle metadata into a more friendly format.
  using SNOWFundingCycleMetadataResolver for SNOWFundingCycle;
  ...
  */
  mapping(uint256 => mapping(uint256 => mapping(ISNOWPaymentTerminal => mapping(address => uint256))))
  ...
  */
  ISNOWProjects public immutable override projects;
  ...
  */
  ISNOWTokenStore public immutable override tokenStore;

  /**
    @notice
    The contract that stores splits for each project.
  */
  ISNOWSplitsStore public immutable override splitsStore;
  ...
  */
  ...
  {
    return
      _interfaceId == type(ISNOWController).interfaceId ||
      _interfaceId == type(ISNOWMigratable).interfaceId ||
      _interfaceId == type(ISNOWOperatable).interfaceId ||
  ...
  */
  constructor(
    ISNOWOperatorStore _operatorStore,
    ISNOWProjects _projects,
    ISNOWDirectory _directory,
    ISNOWFundingCycleStore _fundingCycleStore,
    ISNOWTokenStore _tokenStore,
  ...
  */
  function launchProjectFor(
    ...
    SNOWGroupedSplits[] calldata _groupedSplits,
    SNOWFundAccessConstraints[] calldata _fundAccessConstraints,
  ...
  */
  function launchFundingCyclesFor(
    ...
    virtual
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SNOWOperations.RECONFIGURE)
  ...
  */
  ...
  /**
    @notice
    Issues an owner's ERC20 SNOWTokens that'll be used when claiming tokens.

    @dev
    Deploys a project's ERC20 SNOWToken contract.
  ...
  */
  function issueTokenFor(
    ...
    virtual
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SNOWOperations.ISSUE)
    returns (ISNOWToken token)
  {
  ...
  */
  function changeTokenOf(
    ...
    virtual
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SNOWOperations.CHANGE_TOKEN)
  {
    // Get a reference to the project's current funding cycle.
    SNOWFundingCycle memory _fundingCycle = fundingCycleStore.currentOf(_projectId);
  ...
  function mintTokensOf(
    ...
    // Scoped section prevents stack too deep. `_fundingCycle` only used within scope.
    {
      ...
      _requirePermissionAllowingOverride(
        ...
        _projectId,
        SNOWOperations.MINT,
        directory.isTerminalOf(_projectId, ISNOWPaymentTerminal(msg.sender)) ||
    ...
    if (_reservedRate == SNOWConstants.MAX_RESERVED_RATE)
      ...
      beneficiaryTokenCount = PRBMath.mulDiv(
        _tokenCount,
        SNOWConstants.MAX_RESERVED_RATE - _reservedRate,
        SNOWConstants.MAX_RESERVED_RATE
      );
  ...
  */
  function migrate(uint256 _projectId, ISNOWMigratable _to)
    external
    virtual
    override
    requirePermission(projects.ownerOf(_projectId), _projectId, SNOWOperations.MIGRATE_CONTROLLER)
  {
    ...
    returns (uint256 tokenCount)
  {
    ...
    uint256 _leftoverTokenCount = tokenCount == 0
      ...
      : _distributeToReservedTokenSplitsOf(
        ...
        SNOWSplitsGroups.RESERVED_TOKENS,
        tokenCount
      );
  ...
  */
  function _distributeToReservedTokenSplitsOf(
    ...
    SNOWSplit[] memory _splits = splitsStore.splitsOf(_projectId, _domain, _group);

    //Transfer between all splits.
    for (uint256 _i = 0; _i < _splits.length; _i++) {
      // Get a reference to the split being iterated on.
      SNOWSplit memory _split = _splits[_i];

      // The amount to send towards the split.
      uint256 _tokenCount = PRBMath.mulDiv(
        _amount,
        _split.percent,
        SNOWConstants.SPLITS_TOTAL_PERCENT
      );
      ...
      if (_tokenCount > 0) {
        tokenStore.mintFor(
          ...
          _split.allocator != ISNOWSplitAllocator(address(0))
        ...
        if (_split.allocator != ISNOWSplitAllocator(address(0)))
  ...
  */
  function _configure(
    ...
    if (_metadata.reservedRate > SNOWConstants.MAX_RESERVED_RATE) revert INVALID_RESERVED_RATE();

    // Make sure the provided redemption rate is valid.
    if (_metadata.redemptionRate > SNOWConstants.MAX_REDEMPTION_RATE)
      revert INVALID_REDEMPTION_RATE();

    // Make sure the provided ballot redemption rate is valid.
    if (_metadata.ballotRedemptionRate > SNOWConstants.MAX_REDEMPTION_RATE)
      revert INVALID_BALLOT_REDEMPTION_RATE();

    // Configure the funding cycle's properties.
    SNOWFundingCycle memory _fundingCycle = fundingCycleStore.configureFor(
      _projectId,
      _data,
      SNOWFundingCycleMetadataResolver.packFundingCycleMetadata(_metadata),
      _mustStartAtOrAfter
    );
    ...
    for (uint256 _i; _i < _fundAccessConstraints.length; _i++) {
      SNOWFundAccessConstraints memory _constraints = _fundAccessConstraints[_i];
  ...
  */
  function _reservedTokenAmountFrom(
    ...
    if (_reservedRate == SNOWConstants.MAX_RESERVED_RATE) return _unprocessedTokenBalanceOf;
...
import './../interfaces/ISNOWOperatable.sol';
...
/** 
  ...
  @dev
  Adheres to -
  ISNOWOperatable: General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.
*/
abstract contract SNOWOperatable is ISNOWOperatable {
  //*********************************************************************//
  // --------------------------- custom errors -------------------------- //
  //*********************************************************************//
  error UNAUTHORIZED();
  ...
  */
  ISNOWOperatorStore public immutable override operatorStore;
...
interface ISNOWOperatorStore {
  ...
  function setOperator(SNOWOperatorData calldata _operatorData) external;

  function setOperators(SNOWOperatorData[] calldata _operatorData) external;
}
...
interface ISNOWPaymentTerminal is IERC165 {
...
*/
struct SNOWFundAccessConstraints {
...
import './SNOWGlobalFundingCycleMetadata.sol';
import './../interfaces/ISNOWFundingCycleDataSource.sol';

/** 
  @member global Data used globally in non-migratable ecosystem contracts.
  @member reservedRate The reserved rate of the funding cycle. This number is a percentage calculated out of `SNOWConstants.MAX_RESERVED_RATE`.
  @member redemptionRate The redemption rate of the funding cycle. This number is a percentage calculated out of `SNOWConstants.MAX_REDEMPTION_RATE`.
  @member ballotRedemptionRate The redemption rate to use during an active ballot of the funding cycle. This number is a percentage calculated out of `SNOWConstants.MAX_REDEMPTION_RATE`.
...
*/
struct SNOWFundingCycleMetadata {
  SNOWGlobalFundingCycleMetadata global;
  uint256 reservedRate;
...
interface ISNOWTokenStore {
  event Issue(
    uint256 indexed projectId,
    ISNOWToken indexed token,
    string name,
    string symbol,
    address caller
  );
  ...
  event Change(
    uint256 indexed projectId,
    ISNOWToken indexed newToken,
    ISNOWToken indexed oldToken,
    address owner,
    address caller
  );
  ...
  function tokenOf(uint256 _projectId) external view returns (ISNOWToken);

  function projectOf(ISNOWToken _token) external view returns (uint256);
  ...
  function changeFor(
    ...
    ISNOWToken _token,
    address _newOwner
  ) external returns (ISNOWToken oldToken);
...
import './../interfaces/ISNOWSplitAllocator.sol';
...
/** 
  ...
  @member percent The percent of the whole group that this split occupies. This number is out of `SNOWConstants.SPLITS_TOTAL_PERCENT`.
...
import '@openzeppelin/contracts/utils/introspection/IERC165.sol';
import '../structs/SNOWSplitAllocationData.sol';
...
*/
interface ISNOWSplitAllocator is IERC165 {
  /**
    @notice
    This function is called by SNOWPaymentTerminal.distributePayoutOf(..), during the processing of the split including it
    ...
    @param _data the data passed by the terminal, as a SNOWSplitAllocationData struct:
                  address token;
                  uint256 amount;
                  uint256 decimals;
                  uint256 projectId;
                  ...
                  SNOWSplit split;
  */
  function allocate(SNOWSplitAllocationData calldata _data) external payable;
}
...
*/
struct SNOWSplitAllocationData {
  address token;
  uint256 amount;
  uint256 decimals;
  uint256 projectId;
