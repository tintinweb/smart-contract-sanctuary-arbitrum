...
contract DistributionContract {
    ...
    mapping(address => mapping(uint256 => uint256)) private unclaimedRewardsArbot;
    mapping(address => mapping(uint256 => uint256)) private unclaimedRewardsEth;
    ...
    function distributeRewardsToHolders(uint256 ethAmount) internal {
        ...
        totalDistributionArbot += token.balanceOf(address(this));
        ...
        for (uint256 i = 0; i < token.holdersCount(); i++) {
            ...
            if (isEligible(holder)) {
                ...
                unclaimedRewardsArbot[holder][epoch.getCurrentEpoch()] += holderShareArbot;
                unclaimedRewardsEth[holder][epoch.getCurrentEpoch()] += holderShareEth;
            }
        }
    }
    ...
    function claimArbot() external {
        ...
        unclaimedRewardsArbot[msg.sender][epoch.getCurrentEpoch()] = 0;
    }
    ...
    function claimEth() external {
        ...
        unclaimedRewardsEth[msg.sender][epoch.getCurrentEpoch()] = 0;
    }

    function isEligible(address holder) public view returns (bool) {
        uint256 lastClaimEpoch = holders[holder].lastClaimEpoch;
        uint256 currentEpoch = epoch.getCurrentEpoch();

        if (currentEpoch >= lastClaimEpoch + ELIGIBILITY_EPOCHS) {
            if (unclaimedRewardsArbot[holder][currentEpoch] > 0 || unclaimedRewardsEth[holder][currentEpoch] > 0) {
                return true;
            }
        }
