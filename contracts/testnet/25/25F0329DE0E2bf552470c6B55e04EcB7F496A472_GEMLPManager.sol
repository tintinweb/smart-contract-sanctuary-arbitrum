...
contract GEMLPManager is ReentrancyGuard, AccessControlBase, IGEMLPManager {
    ...
    address public immutable override gemLp;
    ...
    /**
     * @notice when private mode is enabled, minting and redemption of GEMLP is not possible (it is disabled) - only exception is that if handlersEnabled is true, whitelisted handlers are able to mint and redeem GEMLP on behalf of others.
     ...
     * @param _handler address of the handler that will be allowed to handle the GEMLPs gemLp on their behalf
    ...
    function setCooldownDuration(
        ...
        require(
            ...
            "GEMLPManager: invalid _cooldownDuration"
        );
        cooldownDuration = _cooldownDuration;
    ...
    /**
     * @notice the function that can mint GEMLP/ add liquidity to the vault
     * @dev this function mints GEMLP to the msg sender, also this will mint USDW to this contract
     ...
     * @param _minGemLp the minimum amount of GEMLP the callers wants to receive
     * @return gemLpAmount_ returns the amount of GEMLP that was minted to the _account
     */
    function addLiquidity(
        ...
        uint256 _minUsdw,
        uint256 _minGemLp
    ) external override nonReentrant returns (uint256 gemLpAmount_) {
        if (inPrivateMode) {
            revert("GEMLPManager: action not enabled");
        }
        gemLpAmount_ = _addLiquidity(
            _msgSender(),
            _msgSender(),
            _token,
            _amount,
    ...
    /**
     * @notice the function that can mint GEMLP/ add liquidity to the vault (for a handler)
     * @param _fundingAccount the address that will source the tokens to de deposited
     * @param _account the address that will receive the GEMLP
     * @param _token the address of the token being deposited as LP
     * @param _amount the amount of the token being deposited
     * @param _minUsdw the minimum USDW the callers wants his deposit to be valued at
     * @param _minGemLp the minimum amount of GEMLP the callers wants to receive
     ...
     * @param _gemLpAmount  amount of gemLp tokens to be redeemed for _tokenOut
     ...
     * @return tokenOutAmount_ uint256 amount of the tokenOut the caller receives (for their burned GEMLP)
     */
    ...
    /**
     * @notice handler remove liquidity function - redeems GEMLP for selected asset
     * @param _account  the address that will source the GEMLP  tokens
     * @param _tokenOut address of the token the redeemer wants to receive
     * @param _gemLpAmount  amount of gemLp tokens to be redeemed for _tokenOut
     * @param _minOut minimum amount of _tokenOut the redemeer wants to receive
     * @param _receiver  address that will reive the _tokenOut assets
    ...
    function circuitBreakerTrigger(address _token) external {
        ...
        require(
            ...
            "GEMLPManager: only vault can trigger circuit break"
        );
        ...
        if (reserveDeductionOnCB != 0) {
            ...
            // with the deduction of the circuit breaker we will not lower the GEMLP, but we will lower the getReserves() function in the vault
    ...
    /*==================== View functions GEMB/JB *====================*/

    /**
     * @notice returns the value of 1 gemLp token in USD (scaled 1e30)
     * @param _maximise when true, the assets maxPrice will be used (upper bound), when false lower bound will be used
     * @return tokenPrice_ returns price of a single GEMLP token
     */
    function getPriceGemLp(
        bool _maximise
    ) external view returns (uint256 tokenPrice_) {
        uint256 supply_ = IERC20(gemLp).totalSupply();
        if (supply_ == 0) {
            return 0;
        }
        tokenPrice_ = ((getAum(_maximise) * GEMLP_PRECISION) / supply_);
    }

    /**
     * @notice returns the GEMLP price of 1 GEMLP token denominated in USDW (so in 1e18, $1 = 1e18)
     * @param _maximise when true, the assets maxPrice will be used (upper bound), when false lower bound will be used
     */
    function getPriceGEMLPInUsdw(
        ...
        tokenPrice_ = ((getAumInUsdw(_maximise) * GEMLP_PRECISION) / supply_);
    }
     ...
     */
    function getAumInUsdw(
        bool _maximise
    ) public view override returns (uint256 aumUSDW_) {
        aumUSDW_ =
            (getAum(_maximise) * (10 ** USDW_DECIMALS)) /
            PRICE_PRECISION;
    }

    /**
     * @notice returns the total value of all the assets in the GEMLP/Vault
    ...
    function getAums() external view returns (uint256[] memory aumAmountsUSD_) {
        ...
        aumAmountsUSD_[0] = getAum(
            true /** use upper bound oracle price for assets */
        );
        aumAmountsUSD_[1] = getAum(
            false /** use lower bound oracle price for assets */
        );
    }
    ...
    /*==================== Internal functions GEMB/JB *====================*/

    /**
     * @notice function used by feecollector to mint GEMLP tokens
     * @dev this function is only active when
     * @param _token address of the token the GEMLP will be minted for
     * @param _amount amount of tokens to be added to the vault pool
     * @param _minUsdw minimum amount of USDW tokens to be received
     * @param _minGemLp minimum amount of GEMLP tokens to be received
     */
    function addLiquidityFeeCollector(
        ...
        require(
            ...
            "GEMLP: only fee collector can call this function"
        );
        gemLpAmount_ = _addLiquidity(
            _msgSender(),
            _msgSender(),
    ...
    /**
     * @notice config function to enable or disable the collection of gemLp fees on liquidity events (mint and burning)
     * @dev this mechnism is in place to the sandwiching of the distribution of gemLp fees
    ...
    function _collectFees() internal {
        // note: in the process of collecting fees and converting it into gemLp the protocol 'by design' re-enters the GEMLPManager contract
     ...
     * @param _account address that will receive the newly minted GEMLP tokens
     ...
     * @param _minGemLp minimum amount of GEMLP the caller wants to receive
     * @return mintAmountGEMLP_ amount of GEMLP tokens minted
     */
    function _addLiquidity(
        ...
        uint256 _minGemLp,
        bool _swapLess
    ) private returns (uint256 mintAmountGEMLP_) {
        require(_amountDeposit != 0, "GEMLPManager: invalid _amount");
        if (collectFeesOnLiquidityEvent) {
            // prevent reentrancy looping if the gemLp collection on mint/deposit is enabled
            collectFeesOnLiquidityEvent = false;
            // collect fees from vault and distribute to GEMLP holders (to prevent frontrunning of GEMLP feecollection)
            _collectFees();
            // set the configuration back to true, so that it
            collectFeesOnLiquidityEvent = true;
        }
        // cache address to save on SLOADs
        address gemLp_ = gemLp;
        // calculate aum before buyUSDW
        uint256 aumInUsdw_ = getAumInUsdw(
            true /**  get AUM using upper bound prices */
        );
        uint256 gemLpSupply_ = IERC20(gemLp_).totalSupply();

        // mechanism in place to prevent manipulation of gemLp price by the first gemLp minter
        bool firstMint_;
        if (gemLpSupply_ == 0) {
            firstMint_ = true;
            // first mint must issue more than 10 GEMLP to ensure GEMLP pricing precision
            require(
                (_minGemLp >= 1e18),
                "GEMLPManager: too low GEMLP amount for first mint"
            );
        }
        ...
        uint256 usdwAmount_ = vault.deposit(
            _tokenDeposit, // the token that is being deposited into the vault for GEMLP
            address(this), // the address that will receive the USDW tokens (minted by the vault)
            _swapLess
        );
        // the vault has minted USDW to this contract (GEMLP Manager), the amount of USDW minted is equivalent to the value of the deposited tokens (in USD, scaled 1e18) now this GEMLP Manager contract has received usdw, 1e18 usdw is 1 USD 'debt'. If the caller has provided tokens worth $10k, then about 1e5 * 1e18 USDW will be minted. This ratio of value deposited vs amount of USDW minted will remain the same.

        // check if the amount of usdwAmount_ fits the expectation of the caller
        require(
            usdwAmount_ >= _minUsdw,
            "GEMLPManager: insufficient USDW output"
        );
        /**
         * Initially depositing 1 USD will result in 1 GEMLP, however as the value of the GEMLP grows (so historically the GEMLP LPs are in profit), a 1 USD deposit will result in less GEMLP, this because new LPs do not have the right to 'cash in' on the GEMLP profits that where earned bedore the LP entered the vault. The calculation below determines how much GEMLP will be minted for the amount of USDW deposited.
         */
        mintAmountGEMLP_ = aumInUsdw_ == 0
            ? usdwAmount_
            : ((usdwAmount_ * gemLpSupply_) / aumInUsdw_);
        require(
            mintAmountGEMLP_ >= _minGemLp,
            "GEMLPManager: insufficient GEMLP output"
        );

        // only on the first mint 1 GEMLP will be sent to the timelock
        if (firstMint_) {
            mintAmountGEMLP_ -= 1e18;
            // mint 1 GEMLP to the timelock address to prevent any attack possible
            IMintable(gemLp_).mint(timelockAddressImmutable, 1e18);
        }

        // gemLp is minted to the _account address
        IMintable(gemLp_).mint(_account, mintAmountGEMLP_);
        ...
        return mintAmountGEMLP_;
    }

    /**
     * @notice internal function that withdraws assets from the vault
     * @dev burns GEMLP, burns usdw, transfers tokenOut from the vault to the caller
     * @param _account the addresss that wants to redeem its GEMLP from
     * @param _tokenOut address of the token that the redeemer wants to receive for their gemLp
     * @param _gemLpAmount the amount of GEMLP that is being redeemed
    ...
    function _removeLiquidity(
        ...
        require(_gemLpAmount != 0, "GEMLPManager: invalid _gemLpAmount");
        // check if there is a cooldown period
        require(
            (lastAddedAt[_account] + cooldownDuration) <= block.timestamp,
            "GEMLPManager: cooldown duration not yet passed"
        );
        // calculate how much the lower bound priced value is of all the assets in the GEMLP
        uint256 aumInUsdw_ = getAumInUsdw(false);
        // cache gemLp address to save on SLOAD
        address gemLp_ = gemLp;
        // fetch how much GEMLP tokens are minted/outstanding
        uint256 gemLpSupply_ = IERC20(gemLp_).totalSupply();
        ...
        uint256 usdwAmountToBurn_ = (_gemLpAmount * aumInUsdw_) / gemLpSupply_;
        ...
        if (usdwAmountToBurn_ > usdwBalance_) {
            // auditor note: this situation, where usdw token need to be minted without actual tokens being deposited, can only occur when there are almost no GEMLPs left and the vault in general. Another requirement for this to occur is that the prices of assets in the vault are (far) lower at the time of withdrawal relative ti the time they where originally added to the vault.
            IUSDW(usdw_).mint(address(this), usdwAmountToBurn_ - usdwBalance_);
        }
        // burn the GEMLP token in the wallet of the LP remover, will fail if the _account doesn't have the GEMLP tokens in their wallet
        IMintable(gemLp_).burn(_account, _gemLpAmount);
        ...
        require(
            ...
            "GEMLPManager: insufficient output"
        );
    ...
    function _validateHandler() private view {
        require(handlersEnabled, "GEMLPManager: handlers not enabled");
        require(isHandler[_msgSender()], "GEMLPManager: forbidden");
    }
}
