...
import './utils/NativeOwnerApproval.sol';

// CoinFlip Custodial
contract CoinFlipCustodial is NativeOwnerApproval, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    /* ========== EVENTS ========== */
    event Withdraw(uint8 indexed withdrawType, address indexed playerAddress, uint256 amount);
    event EtherDeposited(address indexed from, uint256 amount);

    function initialize() public virtual initializer {
        _initializeEIP712('CoinFlipCustodial', '1');
        __Ownable_init();
    ...
    function withdrawEtherByPlayer(
        uint256 amount,
        uint256 deadline,
        bytes32 sigR,
        bytes32 sigS,
        ...
        _withdrawEther(0, msg.sender, amount);
    }

    function moveEtherToInHouseWallets(
        address payable[] calldata inHouseWallets,
        uint256[] calldata amounts
    ) external onlyOwner {
        require(
            inHouseWallets.length == amounts.length,
            'Custodial: Mismatch in No of Elements of InHouseWallets and Amounts'
        );
        for (uint256 i = 0; i < inHouseWallets.length; i++) {
            _withdrawEther(1, inHouseWallets[i], amounts[i]);
        }
    }
    ...
    function _withdrawEther(uint8 withdrawType, address player, uint256 amount) internal {
        (bool sent, bytes memory data) = payable(player).call{value: amount}('');
        string memory response = string(data);
        require(sent, response);
        emit Withdraw(withdrawType, player, amount);
    }
...
contract NativeOwnerApproval is EIP712Base, OwnableUpgradeable {
    bytes32 private constant OWNER_APPROVAL_TYPEHASH =
        keccak256(bytes('OwnerApproval(uint256 nonce,address from,bytes playerData)'));
    ...
    function checkOwnerApproval(bytes memory playerData, bytes32 sigR, bytes32 sigS, uint8 sigV) internal {
        ...
        return
            keccak256(
                abi.encode(
                    ...
                    keccak256(ownerApprovalTx.playerData)
                )
            );
    }
