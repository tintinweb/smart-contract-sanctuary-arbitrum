...
contract GroupOrderBook is ReentrancyGuard, IGroupOrderBookk {
    ...
    function buyGroupOrder(
        ...
        require(
            item.groupAmount == 0 || amountOut >= item.groupAmount,
            "_amountIn"
        );
        ...
        listOrders[orderId].totalOut = listOrders[orderId].totalOut.add(
            amountOut
        );
        listOrders[orderId].totalIn = listOrders[orderId].totalIn.add(
            _amountIn
        );
        ...
        if (buyOrders[msg.sender][orderId].orderId > 0) {
            ...
            listBuyOrdersIndex[msg.sender] = listBuyOrdersIndex[msg.sender].add(
                1
            );
    ...
    function placeSubGroupOrder(
        ...
        require(
            item.groupAmount > 0 && subOrder.totalOut < item.groupAmount,
            "done"
        );
        ...
        //     revert("subOrder");
        // }
        uint256 subTotalOut = subOrder.totalOut.add(amountOut);
        if (item.groupAmount > 0 && subTotalOut >= item.groupAmount) {
            if (item.totalOut.add(subTotalOut) > item.amountIn) {
                revert("soldout");
            }
        }
        ...
        if (
            item.groupAmount > 0 &&
            listSubOrders[subOrderId].totalIn >= item.groupAmount
        ) {
