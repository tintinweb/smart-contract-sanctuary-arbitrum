...
interface IMercenaries {
        function mint(address to, uint256 amount, bytes16[][] calldata traits) external;
}
...
// EVENTS
    ...
    event MercenaryClaimed(address indexed account, uint256 index, uint256 amount);
    event MercenaryMinted(address indexed account, uint256 amount);
...
// FUNCTIONS
    ...
    /// @param amount of mercenaries to receive
    /// @param traits a 2 dimensional array representing the mercenaries traits e.g., [["blue hat","brown eyes],["green hat","blue eyes"]]
    function claimAirDrop(uint256 index, bytes32[] calldata merkleProof, uint256 amount, bytes16[][] calldata traits)
        external
        nonReentrant
        whenNotPaused
        onlyUnclaimed(index)
    {
        ...
        _setClaimed(index);                                                      // Flag mercenaries as claimed
        emit MercenaryClaimed(_msgSender(), index, amount);                      // Write an event
        mercenaries.mint(_msgSender(), amount, traits);                          // Mint Mercenaries
    }
    ...
    function claim(uint256 index, bytes16[][] calldata traits)
        ...
        whenNotPaused
    {
        require( traits.length == 1,                                "Only provide a single traits array");
        ...
        require (allowlist[index].claimed == false,                 "Mercenary Already Claimed");
        allowlist[index].claimed = true;                            // Set claimed
        emit MercenaryClaimed(_msgSender(), index, 1);              // Write an event
        mercenaries.mint(_msgSender(), 1, traits);                  // Mint Mercenaries
    }

    /// @dev Mint a mercenary
    /// @param amount of mercenarys to buy
    /// @param traits a 2 dimensional array representing the mercenaries traits e.g., [["blue hat","brown eyes],["green hat","blue eyes"]]
    function mint(uint256 amount, bytes16[][] calldata traits)
        external
        payable
        nonReentrant
        whenNotPaused
    {
        ...
        emit MercenaryMinted(_msgSender(), amount);         // Write an event
        mercenaries.mint(_msgSender(), amount, traits);     // Mint Mercenaries
    }
...
// ADMIN FUNCTIONS
        ...
        onlyAllowed
    {
        ...
        for (uint i = 0; i < total ; i++) {
            allowlist.push(Allowlist(accounts[i], false)); // Add to the allowlist
            emit AccountAllowlisted(_msgSender(), accounts[i]);  // Write an event
        }
    }

    /// @dev Allow change in the prices
    /// @param _mercenaryPrice new price in ETH
    function setPrice(uint256 _mercenaryPrice) 
        external
        onlyOwner
    {
        mercenaryPrice = _mercenaryPrice;
...
//VIEWS
    ...
    function getAllowlistIndex(address account)
        ...
        returns (
            ...
            uint256 index)
    {
        uint256 total = allowlist.length;                // Get the current allowlist array index
        if (total == 0 ) {
            return (false, 0);
        } else {
            for(uint256 i=0; i < total; i++){            // Loop through the addresses
                if ( allowlist[i].account == account &&  // if account matches
                     allowlist[i].claimed == false )     // and unclaimed
                {
                    return (true,i);
                }
            }
