...
import {IHelixUniTranchedPool} from "../interfaces/IHelixUniTranchedPool.sol";
...
contract HelixUniTranchedPool is BaseUpgradeablePausable, IHelixUniTranchedPool, IRequiresUID, IVersioned {
  ...
  uint8 internal constant PATCH_VERSION = 2;
  ...
  function initialize(
    ...
    _createAndSetCreditLine(
      _addresses[2], // _interestModel
      _addresses[1], // borrower
      _targets[0], // _maxTarget
      _targets[1], // _minTarget
      _uints[0], // interest APR
      _days[0], // _paymentPeriodInDays
      _days[1], // _termInDays
      _uints[1], // late Fee APR
      _days[2], // _principalGracePeriodInDays
      _days[4] // fundingDate
    );

    createdAt = block.timestamp;
    minTicketSize = _uints[2];
  ...
  function setAllowedUIDTypes(uint256[] calldata ids) external onlyLocker {
    require(
      _poolSlices[0].uniTranche.totalSupply() == 0,
  ...
  function drawdown() external override onlyLocker notCancelled whenNotPaused returns(uint256) {
    ...
    require(closed, "Pool is not closed yet");
    ...
    TrancheInfo memory uniTrancheInfo = _getTrancheInfoByAddress(currentSlice.uniTranche);    

    uint256 amountAvailable = HelixTranchingLogic.sharePriceToUsdc(
      uniTrancheInfo.principalSharePrice,
      uniTrancheInfo.principalDeposited
    );
    ...
    uint256 oldUniPrincipalSharePrice = uniTrancheInfo.principalSharePrice;
    uniTrancheInfo.principalSharePrice = 0;
    // uniTrancheInfo.principalSharePrice = currentSlice
    //   .uniTranche
    ...
    emit SharePriceUpdated(
      ...
      uniTrancheInfo.id,
      uniTrancheInfo.principalSharePrice,
      int256(oldUniPrincipalSharePrice - uniTrancheInfo.principalSharePrice) * -1,
      uniTrancheInfo.interestSharePrice,
      0
    );
...
//   }
   ...
   */
  function assess(uint256 interestPayment, uint256 principalPayment) external override notCancelled whenNotPaused {
    _assess(interestPayment, principalPayment);
  }
  ...
  //  */
  // function pay(uint256 amount) internal override notCancelled whenNotPaused {
  //   /// @dev  IA: cannot pay 0
  //   if(amount == 0) {
  //     revert InvalidAmount();
  //   }
...
//   }
  ...
  function poolRequirementsFulfilled() public view returns(bool) {
    ...
    uint256 totalPrincipalDeposited = uniTrancheInfo.principalDeposited;
  ...
  function _lockPool() internal {
    ...
    TrancheInfo memory uniTrancheInfo = _getTrancheInfoByAddress(slice.uniTranche);    

    uint256 currentTotal = uniTrancheInfo.principalDeposited;

    creditLine.setLimit(Math.min(creditLine.limit() + currentTotal, creditLine.maxLimit()));

    IHelixPoolShares(uniTrancheInfo.tranche).lockPool();
  ...
  function _initializeNextSlice(uint256 _newFundableAt, string[2] memory _loanFacade) internal returns(address uniTranche) {
    /// @dev SL: slice limit
    require(numSlices < 5, "SL");
    (uniTranche) = HelixTranchingLogic.initializeNextSlice(_poolSlices, numSlices, _loanFacade, config);
    numSlices = numSlices + 1;
    fundableAt = _newFundableAt;

    emit TranchesCreated(uniTranche);
  }
  ...
  function _getTrancheInfoByAddress(IHelixPoolShares _tranche) internal view returns(IHelixUniTranchedPool.TrancheInfo memory){

      uint maximumInvestment = creditLine.maxLimit();
      uint minimumInvestment = creditLine.minLimit();

      return IHelixUniTranchedPool.TrancheInfo({
  ...
  function _getTrancheInfoById(uint256 _trancheId) internal view returns(IHelixUniTranchedPool.TrancheInfo memory){
      uint256 sliceId = HelixTranchingLogic.trancheIdToSliceIndex(_trancheId);
      PoolSlice storage slice = _poolSlices[sliceId];

      IHelixPoolShares tranche = slice.uniTranche;
  ...
  function _assess(uint256 interestPayment, uint256 principalPayment) internal {
  ...
  event TranchesCreated(address indexed uni);
  // // Modifiers /////////////////////////////////////////////////////////////////
