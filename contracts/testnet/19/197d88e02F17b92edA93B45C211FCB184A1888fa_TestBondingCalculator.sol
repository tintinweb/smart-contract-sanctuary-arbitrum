...
contract TestBondingCalculator is IBondingCalculator {
    ...
    function getReserves(address _pair) public view returns (uint256 reserve0,uint256 reserve1) {
        (reserve0, reserve1, ) = IUniswapV2Pair(_pair).getReserves();
    }

    function getPairTotalSupply(address _pair) public view returns (uint256 totalValue) {
        totalValue = IUniswapV2Pair(_pair).totalSupply();
    }
