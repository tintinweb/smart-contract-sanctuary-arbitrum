...
contract Airdrop is Ownable {
    ...
    function dropTokens(address[] memory _recipients, uint256[] memory _amount) public onlyOwner returns (bool) {
        require(_recipients.length == _amount.length, "invalid params");

        for (uint256 i = 0; i < _recipients.length; i++) {
            require(_recipients[i] != address(0), "recipients address is 0");
    ...
    function dropEther(address[] memory _recipients, uint256[] memory _amount) public payable onlyOwner returns (bool) {
        ...
        require(total <= msg.value, "invalid amount params");
        require(_recipients.length == _amount.length, "invalid params");

        for (uint256 i = 0; i < _recipients.length; i++) {
            require(_recipients[i] != address(0));

            (bool sent, ) = payable(_recipients[i]).call{ value: _amount[i] }("");
    ...
    function withdrawTokens(address beneficiary) public onlyOwner {
        ...
        IERC20(tokenAddr).safeTransfer(beneficiary, IERC20(tokenAddr).balanceOf(address(this)));
    }
