...
contract Router is BaseRouter, IRouter, ReentrancyGuard {
    ...
    */
    ...
    function setPriceAndExecute(bytes32 _key, uint256 _txType, uint256[] memory _prices) external {
        ...
        , "FBD"); //Forbidden
        address[] memory path = txnDetails[_key][_txType].path;
        require(path.length > 0, "NE/EXCTD"); //Transaction not exist or has already executed
        require(txnDetails[_key][_txType].path.length == _prices.length, "IVLARL"); //Invalid array length
        _setPriceAndExecute(_key, _getTxTypeForExecuting(_txType), path, _prices);
    }
    ...
    function _createPrepareTransaction(
        ...
        if (!_isDirectExecuted) {
            PrepareTransaction storage transaction = txns[key];
            require(transaction.status == TRANSACTION_STATUS_NONE 
                || transaction.status == TRANSACTION_STATUS_EXECUTED, "IVLPTS/PRCS"); //Invalid prepare transaction status, processing
            txnDetails[key][_txType].path = _path;
            transaction.txType = _txType;
            transaction.startTime = block.timestamp;
            transaction.status = _isDirectExecuted ? TRANSACTION_STATUS_NONE : TRANSACTION_STATUS_PENDING;
        }
    ...
    function getBondDetail(bytes32 _key, uint256 _txType) external view returns (BondDetail memory) {
        return bondDetails[_key][_txType];
    }
