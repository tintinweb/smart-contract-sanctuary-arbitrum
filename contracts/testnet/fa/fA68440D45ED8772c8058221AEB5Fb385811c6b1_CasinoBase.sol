...
contract CasinoBase is LiquidityProvider, Defense, CasinoGameContracts, Sports, ReentrancyGuard {
    ...
    uint8 public houseEdge; // House edge of the casino (% value)
    uint8 public maxBetPercentage; // Maximum bet amount (% of casino LP funds)
    ...
    constructor(
        IERC20 _usdtToken,
        uint8 _maxBetPercentage,
        uint8 _houseEdge,
        address _transpareWallet
    ) {
        require(address(_usdtToken) != address(0), "Invalid USDT token address");
        require(_houseEdge >= 0 && _houseEdge <= 100, "House edge must be 0 <= x <= 100");
        require(_maxBetPercentage >= 0 && _maxBetPercentage <= 100, "Max bet must be 0 <= x <= 100");
    ...
    function placeCasinoBet(
        ...
        require(_betAmount <= (casinoUSDTLiquidityBalance * maxBetPercentage) / 100, "Bet amount too high");
    ...
    function notifyGameResult(
        ...
        if (_multiplier == 0) {
            casinoUSDTLiquidityBalance += (_betAmount * 99) / 100;
            userBalance[transpareWallet] += (_betAmount * 1) / 100;
        } else {
            uint256 _adjustedMultiplier = _multiplier * (100 - houseEdge);
            uint256 _winAmount = (_betAmount * _adjustedMultiplier) / 100;
     ...
     */
    function updateHouseEdge(uint8 _newHouseEdge) external onlyOwner {
        require(_newHouseEdge >= 0 && _newHouseEdge <= 100, "House edge must be 0 <= x <= 100");
     ...
     */
    function updateMaxBet(uint8 _newMaxBetPercentage) external onlyOwner {
        require(_newMaxBetPercentage >= 0 && _newMaxBetPercentage <= 100, "Max bet must be 0 <= x <= 100");
    ...
    /**
     * @notice Resets the casino's protector tool based on the given window type.
     * @dev Renews the sliding window with the latest balance and current timestamp.
     ...
     * @param _windowType The type of window to renew (either LiquidityPool or Casino).
     */
    function enableSlidingWindow(WindowType _windowType, uint256 _windowSize, uint256 _windowShiftSize, uint8 _thresholdPercentage) external onlyOwner {
        require(!isSlidingWindowActive[_windowType], "Sliding window is already active");
...
contract CasinoGameContracts is IOwnable {
    ...
    address[] public listOfAllCasinoGameContracts;
    ...
    function _checkSlidingWindow(WindowType _windowType, address _user, uint256 _proposedBalance) internal {
        if (highRiskMode) {
            ...
            if (_proposedBalance <= _allowedBalance) {
                ...
                enableHighRiskMode();
                disableSlidingWindow(_windowType);
...
contract SlidingWindow is IOwnable {
    ...
    mapping(WindowType =>  bool) internal isSlidingWindowActive;
     ...
     * @param _windowType Type of the sliding window (LiquidityPool or Casino).
     */
    function disableSlidingWindow(WindowType _windowType) public onlyOwner {
