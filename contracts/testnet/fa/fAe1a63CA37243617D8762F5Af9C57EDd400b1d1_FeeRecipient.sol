...
/// @title Accures the protocol fees
/// @notice can push the tokens to the fee converter
contract FeeRecipient is IFeeRecipient, TransferHelper, SafeOwnable {
    address public feeConverter;

    modifier onlyConverter() {
        require(
            msg.sender == feeConverter,
            "FeeRecipient: caller is not the converter"
        );
        _;
    }
    ...
    /// @notice pushes the accumulated tokens to the fee converter
    /// @param _token token to push
    /// @param _amount to push
    /// @dev can only be called by the fee converter
    function pushToken(address _token, uint256 _amount)
        external
        override
        onlyConverter
    {
    ...
    /// @notice sets a new fee converter
    /// @param _feeConverter address of the new fee converter
