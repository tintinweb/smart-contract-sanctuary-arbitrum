...
contract ItemsInitFacet is UsingDiamondOwner, WithStorage {
    ...
    function init(address minter) external onlyOwner {
        ...
        _constants().chestOpeningPrice = 500000000000000;
...
struct TokensConstants {
    ...
    uint256 chestOpeningPrice;
    uint256 nftConversionPrice;
}
...
struct TokensStorage {
    ...
    uint256 itemsIndex;
    mapping(uint256 => address) requestedChestOpening; // mapping from tokenID to requestor
    mapping(address => uint16) requestedNftConversions; // mapping from requestor address to how many NFT conversions they have requested/paid for (ie. there's a flat fee for all)
}
