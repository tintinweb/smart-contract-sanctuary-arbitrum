...
interface ILiquidityPool{
    ...
    event DisbursePremiumFor(uint256 snapshotId, address indexed provider, uint256 premiumShares, uint256 timestamp);
...
abstract contract AbstractLiquidityPool is ILiquidityPool, Ownable, ReentrancyGuard {
    ...
    function _disbursePremiumFor(
        ...
        if (premiumShares > 0) {
            ...
            emit DisbursePremiumFor(snapshotId, provider, premiumShares, block.timestamp);
        }
    }
    //slither-disable-end calls-loop
    //slither-disable-end reentrancy-events
