  ...
  LiquidityPoolManagerPointer
{
  ...
  modifier onlyPyramidOwner() {
    address sender = _msgSender();
    require(sender != address(0), "Pyramids: Cannot be from the zero address");
    require(
      isOwnerOfPyramids(sender),
      "Pyramids: No Pyramid owned by this account"
    );
    _;
  }
  ...
  modifier checkPermissions(uint256 _pyramidId) {
    address sender = _msgSender();
    require(pyramidExists(_pyramidId), "Pyramids: This pyramid doesn't exist");
    require(
      isApprovedOrOwnerOfPyramid(sender, _pyramidId),
      "Pyramids: You do not control this Pyramid"
    );
    _;
  }
  ...
  modifier checkPermissionsMultiple(uint256[] memory _pyramidIds) {
    ...
    for (uint256 i = 0; i < _pyramidIds.length; i++) {
      require(
        pyramidExists(_pyramidIds[i]),
        "Pyramids: This pyramid doesn't exist"
      );
      require(
        isApprovedOrOwnerOfPyramid(sender, _pyramidIds[i]),
        "Pyramids: You do not control this Pyramid"
      );
    }
    _;
  ...
  modifier onlyWhitelist() {
    ...
    require(
      sender == whitelist,
      "Pyramids: Only the Whitelist contract can call this function"
    );
    _;
  }
  ...
  constructor(
  ...
  ) ERC721("Pyramid Ecosystem", "PRMDNFT") {
    require(
      address(_pyramid) != address(0),
      "PyramidsManager: Pyramid is not set"
    );
    pyramid = _pyramid;
    ...
    string memory ipfsBaseURI = "ipfs://QmRixq9jye3NY6EgBuEPZuBmyVBV4wT7jfKrmKnSWHoEkh/";
  ...
  function createPyramidWithTokens(
    ...
    returns (uint256)
  {
    return _createPyramidWithTokens(_msgSender(), pyramidName, pyramidValue);
  }
    ...
    returns (uint256)
  {
    ...
    _pyramids[pyramidId].rewardMult = tiers[tierLevel].level;
    return pyramidId;
  }
  ...
  function _createPyramidWithTokens(
    ...
    logTier(tiers[0].level, int256(pyramidValue));
    ...
    )
  {
    ...
    if (pyramid.rewardMult != tiers[0].level) {
      logTier(pyramid.rewardMult, -int256(pyramid.pyramidValue));
      logTier(tiers[0].level, int256(pyramid.pyramidValue));
    }
    ...
    pyramid.rewardMult = tiers[0].level;
