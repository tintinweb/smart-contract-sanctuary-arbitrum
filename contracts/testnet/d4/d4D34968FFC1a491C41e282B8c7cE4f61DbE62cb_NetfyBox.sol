...
contract NetfyBox is Ownable {
    ...
    constructor(NetfyBoxDB db, IERC20 net, address dev) {
        _db = db;
        _net = net;
        _dev = dev;
    }
    ...
    function createBlockchain() external {
        ...
        IERC20(_net).transferFrom(msg.sender, address(this), 1000 * 1e18);
    ...
    function claimLiquidity(uint blockchainId) internal {
        ...
        //dev reclaim 1% o
        IERC20(_net).transfer(_dev, devclaim);
        // send to user
        IERC20(_net).transfer(msg.sender, pending);
    }
