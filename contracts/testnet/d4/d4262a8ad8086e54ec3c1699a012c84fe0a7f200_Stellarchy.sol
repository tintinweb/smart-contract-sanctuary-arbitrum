...
contract Stellarchy is Compounds, Lab, Dockyard, Defences {
    ...
    function getSpendableResources(
        ...
        amounts.steel = interfaces.steel.balanceOf(planetOwner);
        amounts.quartz = interfaces.quartz.balanceOf(planetOwner);
        amounts.tritium = interfaces.tritium.balanceOf(planetOwner);
        return amounts;
    }

    function getCollectibleResources() public view returns (S.ERC20s memory) {
    ...
    function getEnergyAvailable() external view returns (uint256 energy) {
        S.Interfaces memory interfaces = _getInterfaces();
        uint256 planetId = interfaces.erc721.tokenOf(msg.sender);
        ...
        uint256 energyRequired = _calculateEnergyConsumption(mines);
        if (grossProduction < energyRequired) {
            return 0;
        }
