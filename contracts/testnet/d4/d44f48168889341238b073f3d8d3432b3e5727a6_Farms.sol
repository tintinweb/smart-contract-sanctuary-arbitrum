...
contract Farms is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime; // Last timestamp that Reward distribution occurs.
    ...
    // RewardSwapToken tokens created per second.
    uint256 public rewardPerSecond;

    // RewardToken tokens created per day.
    uint256 public rewardPerDay;

    // Bonus mulipliers for early reward makers.
    ...
    // The timestamp when RewardSwapToken mining starts.
    uint256 public startTime;

    // The timestamp when RewardSwapToken claiming starts.
    ...
    // Update the given pool's RewardSwapToken allocation point. Can only be called by the owner.
    function set(
        uint256 _pid, 
        uint256 _allocPoint, 
        uint256 _lockTime, 
    ...
    // Deposit LP tokens to MasterChef for rewardSwapToken allocation.
    ...
    function updateRewardRate(uint256 _rewardPerDay) external onlyOwner {
        uint256 _rewardPerSecond = _rewardPerDay / 86400;
        rewardPerDay = _rewardPerDay;
        rewardPerSecond = _rewardPerSecond;
        emit RewardRateUpdated(_rewardPerSecond, _rewardPerDay);
    }
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 totalReward = (multiplier * rewardPerSecond * pool.allocPoint) / totalAllocPoint;
            accRewardPerShare += (totalReward * 1e12) / lpSupply;
        }
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBalance = reward.balanceOf(address(this));
        reward.safeTransfer(_to, _amount > tokenBalance ? tokenBalance : _amount);
    }
    ...
    function updatePool(uint256 _pid) internal {
        ...
        uint256 multiplier = getMultiplier(_pid, pool.lastRewardTime, block.timestamp);
        uint256 totalReward = (multiplier * rewardPerSecond * pool.allocPoint) / totalAllocPoint;
        if (devFee > 0) { reward.mint(devAddr, ((totalReward * devFee) / 10000)); }
        reward.mint(address(this), totalReward);
        pool.accRewardPerShare += (totalReward * 1e12) / lpSupply;
        pool.lastRewardTime = block.timestamp;
    }
