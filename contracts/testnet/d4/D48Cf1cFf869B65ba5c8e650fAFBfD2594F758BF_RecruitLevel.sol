...
contract RecruitLevel is Initializable, RecruitLevelSettings {
    ...
    function oddsOfAscending() public view returns(uint32) {
        uint256 _numRecruitsAscending = auxAscensionInfo.numRecruitsAscending;
        if(_numRecruitsAscending == 0) {
            _numRecruitsAscending = 1;
        }

        uint256 _currentCap = currentAscensionCap();

        return uint32((10**25 / (10**20 + (((_numRecruitsAscending * 10**28) / _currentCap) * ((uint256(auxAscensionInfo.sModifier) * 10**5) / 100000))**2)) / uint256(auxAscensionInfo.dModifier));
    }

    function currentAscensionCap() public view returns(uint256) {
        return ((block.timestamp - uint256(auxAscensionInfo.timeCapLastChanged)) * uint256(auxAscensionInfo.capRatePerSecond)) + uint256(auxAscensionInfo.capAtLastRateChange) - (uint256(auxAscensionInfo.numRecruitsAscended) * 1 ether);
    }
