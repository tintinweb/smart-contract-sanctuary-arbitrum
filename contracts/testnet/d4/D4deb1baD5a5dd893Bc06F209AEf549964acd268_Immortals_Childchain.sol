 ...
 */
abstract contract LockableUpgradeable is IERC5192, ILockable, Initializable, ERC165Upgradeable {
	// ====================================================
	// STATE
	// ====================================================
	...
	function lockTokenState(uint256 tokenId) internal {
	...
	function getTokenLockedState(uint256 tokenId) public view returns (bool lockedState, LockInfo memory lockInfo) {
		...
		if(!LockableStorage.layout().tokenLockingEnabled) {
			// unlocked if token locking is disabled
			lockedState = false;
		}
		else {
			if (LockableStorage.layout().overrideLocked) {
				// handle override locking
				lockedState = true;
			}
			else {
				// else return tokens locked state
				lockedState = LockableStorage.layout().tokenLockedStates[tokenId];
			}
		}
	}
	...
	/// @dev Must be overriden for role-gated locking
	function lockToken(uint256 tokenId) public virtual {
		lockTokenState(tokenId);

		// Reset info state (since the duration is indefinite)
		LockableStorage.layout().tokenLockedInfo[tokenId] = LockInfo({ lockTime: 0, lockDuration: 0 });
	}
	...
	function lockTokens(uint256[] calldata tokenIds) public virtual {
		for (uint i = 0; i < tokenIds.length; i++) {
			lockToken(tokenIds[i]);
		}
	}

	function lockTokens(uint256[] calldata tokenIds, uint256 lockDuration) public virtual{
		for (uint i = 0; i < tokenIds.length; i++) {
			lockToken(tokenIds[i], lockDuration);
		}
	}
	...
	function unlockTokens(uint256[] calldata tokenIds) public virtual {
		for (uint i = 0; i < tokenIds.length; i++) {
			unlockToken(tokenIds[i]);
		}
	}
...
import "../../delegation-registry/IWalletDelegation.sol";
import { ERC2771RecipientUpgadeable } from "../../delegation-registry/ERC2771RecipientUpgradeable.sol";

import { COLLECTION_ADMIN_ROLE, IMMORTALS_CHILDCHAIN_MINTER_ROLE, QUESTING_CONTRACT_ROLE } from "../../WLRoleConstants.sol";

contract Immortals_Childchain is
	Initializable,
	ERC721Upgradeable,
	...
	ERC2771RecipientUpgadeable
{
	...
	function initialize(
		...
		IWalletDelegation delegationRegistry_
	) public initializer {
		...
		_setupRole(IMMORTALS_CHILDCHAIN_MINTER_ROLE, _msgSender());
		_setupRole(QUESTING_CONTRACT_ROLE, _msgSender());
		_setupRole(COLLECTION_ADMIN_ROLE, _msgSender());

		_setRoleAdmin(COLLECTION_ADMIN_ROLE, DEFAULT_ADMIN_ROLE);
		_setRoleAdmin(QUESTING_CONTRACT_ROLE, DEFAULT_ADMIN_ROLE);
		_setRoleAdmin(IMMORTALS_CHILDCHAIN_MINTER_ROLE, DEFAULT_ADMIN_ROLE);

		delegationRegistry = delegationRegistry_;

		_tokenIdCounter.increment(); // Token ids start at 1
	}
	...
	function supportsInterface(
		...
		public
		view
		override(ERC721Upgradeable, ERC721EnumerableUpgradeable, LockableUpgradeable, AccessControlUpgradeable)
		returns (bool)
	{
		return super.supportsInterface(interfaceId);
	}

	function _authorizeUpgrade(address newImplementation) internal override onlyRole(COLLECTION_ADMIN_ROLE) {}
	...
	function _beforeTokenTransfer(
		...
		uint256 tokenId,
		uint256 batchSize
	) internal override(ERC721Upgradeable, ERC721EnumerableUpgradeable) onlyWhenUnlocked(tokenId) {
		// We're either burning or minting
		// if (from != address(0) || to != address(0)) revert("transfers are only allowed on mainnet");
	...
	function lockToken(uint256 tokenId) public override onlyRole(QUESTING_CONTRACT_ROLE) {
		super.lockToken(tokenId);
	}

	function lockToken(uint256 tokenId, uint256 lockDuration) public override onlyRole(QUESTING_CONTRACT_ROLE) {
		super.lockToken(tokenId, lockDuration);
	}

	function unlockToken(uint256 tokenId) public override onlyRole(QUESTING_CONTRACT_ROLE) {
	...
	function mintOrTransfer(
		address to,
		uint256[] calldata tokenIds
	) external onlyRole(IMMORTALS_CHILDCHAIN_MINTER_ROLE) {
		for (uint256 i = 0; i < tokenIds.length; i++) {
			if (_exists(tokenIds[i])) {
				super.unlockToken(tokenIds[i]);
				_transfer(_ownerOf(tokenIds[i]), to, tokenIds[i]);
			} else _safeMint(to, tokenIds[i]);
		}
	}

	function burnTokens(uint256[] calldata tokenIds) external onlyRole(IMMORTALS_CHILDCHAIN_MINTER_ROLE) {
		for (uint256 i = 0; i < tokenIds.length; i++) {
			super.unlockToken(tokenIds[i]);
			if (_exists(tokenIds[i])) _burn(tokenIds[i]);
		}
	}
	...
	function setTokenTraitsProvider(ITokenTraitsProvider traitsProvider) public onlyRole(COLLECTION_ADMIN_ROLE) {
		tokenTraitsProvider = traitsProvider;
	}
	...
	function setRandomOffset(uint256 r) public onlyRole(COLLECTION_ADMIN_ROLE) {
		randomOffset = r;
	}
	...
	/// @notice Convenience function for retrieving all tokens help by provided wallet.
	/// @dev Replaces need to call balanceOf, tokenOfOwnerByIndex and tokenURI calls.
	/// @param address_ Wallet address.
	/// @return indicies Array of wallet's owned token indicies.
	function getTokensForAddress(
		address address_
	) public view returns (uint256[] memory indicies, bool[] memory lockedStates, uint256[] memory lockedUntil) {
		uint256 balance = balanceOf(address_);
		indicies = new uint256[](balance);
		lockedStates = new bool[](balance);
		lockedUntil = new uint256[](balance);

		for (uint256 i = 0; i < balance; i++) {
			uint256 index = tokenOfOwnerByIndex(address_, i);
			indicies[i] = index;
			LockInfo memory info;
			(lockedStates[i], info) = getTokenLockedState(index);
			if (lockedStates[i]) lockedUntil[i] = (info.lockTime + info.lockDuration);
		}
	}
	...
	// ====================================================
	function setTrustedForwarder(address forwarder) external onlyRole(COLLECTION_ADMIN_ROLE) {
	...
	function setWalletDelegationRegistry(
		IWalletDelegation walletDelegationRegistry_
	) external onlyRole(COLLECTION_ADMIN_ROLE) {
		delegationRegistry = walletDelegationRegistry_;
	}
	...
	// ====================================================
	function devMint(address to, uint256[] calldata tokenIds) public {
		for (uint256 i = 0; i < tokenIds.length; i++) {
			if (!_exists(tokenIds[i])) _safeMint(to, tokenIds[i]);
		}
	}

	function devBurn(uint256[] calldata tokenIds) public {
		for (uint256 i = 0; i < tokenIds.length; i++) {
			super.unlockToken(tokenIds[i]);
			if (_exists(tokenIds[i])) _burn(tokenIds[i]);
		}
	...
	function burnAll() public {
		...
		uint256[] memory tokensToBurn = new uint256[](supply);
