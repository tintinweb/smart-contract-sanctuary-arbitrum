...
import "./interfaces/IPachiTokenV2.sol";
import "./interfaces/IXPachiToken.sol";
import "./interfaces/IXPachiTokenUsage.sol";

/*
 * xPACHI is Camelot's escrowed governance token obtainable by converting PACHI to it
 * It's non-transferable, except from/to whitelisted addresses
 * It can be converted back to PACHI through a vesting process
 * This contract is made to receive xPACHI deposits from users in order to allocate them to Usages (plugins) contracts
 */
contract XPachiTokenV3 is
    Ownable,
    ReentrancyGuard,
    ERC20("Pachi escrowed token V3", "xPACHI"),
    IXPachiToken
{
    using Address for address;
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IPachiTokenV2;

    struct XPachiBalance {
        uint256 allocatedAmount; // Amount of xPACHI allocated to a Usage
        uint256 redeemingAmount; // Total amount of xPACHI currently being redeemed
    }

    struct RedeemInfo {
        uint256 pachiAmount; // PACHI amount to receive when vesting has ended
        uint256 xPachiAmount; // xPACHI amount to redeem
        uint256 endTime;
        IXPachiTokenUsage dividendsAddress;
        uint256 dividendsAllocation; // Share of redeeming xPACHI to allocate to the Dividends Usage contract
    }

    IPachiTokenV2 public immutable pachiToken; // PACHI token to convert to/from
    IXPachiTokenUsage public dividendsAddress; // Camelot dividends contract

    EnumerableSet.AddressSet private _transferWhitelist; // addresses allowed to send/receive xPACHI

    mapping(address => mapping(address => uint256)) public usageApprovals; // Usage approvals to allocate xPACHI
    mapping(address => mapping(address => uint256))
        public
        override usageAllocations; // Active xPACHI allocations to usages

    uint256 public constant MAX_DEALLOCATION_FEE = 200; // 2%
    mapping(address => uint256) public usagesDeallocationFee; // Fee paid when deallocating xPACHI
    ...
    uint256 public minRedeemDuration = 7 days; // 7 days // 60
    uint256 public maxRedeemDuration = 90 days; // 3 months // 180
    // Adjusted dividends rewards for redeeming xPACHI
    uint256 public redeemDividendsAdjustment = 50; // 50%

    mapping(address => XPachiBalance) public xPachiBalances; // User's xPACHI balances
    mapping(address => RedeemInfo[]) public userRedeems; // User's redeeming instances

    constructor(IPachiTokenV2 pachiToken_) {
        pachiToken = pachiToken_;
    ...
    event CancelRedeem(address indexed userAddress, uint256 xPachiAmount);
    ...
    /*
     * @dev Returns user's xPACHI balances
     */
    function getXPachiBalance(
        address userAddress
    ) external view returns (uint256 allocatedAmount, uint256 redeemingAmount) {
        XPachiBalance storage balance = xPachiBalances[userAddress];
        return (balance.allocatedAmount, balance.redeemingAmount);
    }

    /*
     * @dev returns redeemable PACHI for "amount" of xPACHI vested for "duration" seconds
     */
    function getPachiByVestingDuration(
        uint256 amount,
        uint256 duration
    ...
    /**
     * @dev returns approved xPachi to allocate from "userAddress" to "usageAddress"
     */
    ...
    /**
     * @dev returns allocated xPachi from "userAddress" to "usageAddress"
     */
    ...
    /**
     * @dev returns if "account" is allowed to send/receive xPACHI
     */
    ...
    function updateDividendsAddress(
        IXPachiTokenUsage dividendsAddress_
    ) external onlyOwner {
    ...
    function updateTransferWhitelist(
        ...
        require(
            ...
            "updateTransferWhitelist: Cannot remove xPachi from whitelist"
        );
    ...
    /**
     * @dev Approves "usage" address to get allocations up to "amount" of xPACHI from msg.sender
     */
    function approveUsage(
        IXPachiTokenUsage usage,
        uint256 amount
    ...
    /**
     * @dev Convert caller's "amount" of PACHI to xPACHI
     */
    ...
    /**
     * @dev Convert caller's "amount" of PACHI to xPACHI to "to" address
     */
    function convertTo(
        uint256 amount,
        address to
    ...
    /**
     * @dev Initiates redeem process (xPACHI to PACHI)
     *
    ...
    function redeem(
        ...
        require(xPachiAmount > 0, "redeem: xPachiAmount cannot be null");
        require(duration >= minRedeemDuration, "redeem: duration too low");

        _transfer(msg.sender, address(this), xPachiAmount);
        XPachiBalance storage balance = xPachiBalances[msg.sender];

        // get corresponding PACHI amount
        uint256 pachiAmount = getPachiByVestingDuration(xPachiAmount, duration);
        emit Redeem(msg.sender, xPachiAmount, pachiAmount, duration);

        // if redeeming is not immediate, go through vesting process
        if (duration > 0) {
            // add to SBT total
            balance.redeemingAmount = balance.redeemingAmount.add(xPachiAmount);

            // handle dividends during the vesting process
            uint256 dividendsAllocation = xPachiAmount
            ...
            // immediately redeem for PACHI
            _finalizeRedeem(msg.sender, xPachiAmount, pachiAmount);
        }
    }
    ...
    function finalizeRedeem(
        ...
        _finalizeRedeem(msg.sender, _redeem.xPachiAmount, _redeem.pachiAmount);

        // handle dividends compensation if any was active
        if (_redeem.dividendsAllocation > 0) {
            // deallocate from dividends
            IXPachiTokenUsage(_redeem.dividendsAddress).deallocate(
                msg.sender,
                _redeem.dividendsAllocation,
                new bytes(0)
            );
    ...
    function cancelRedeem(
        ...
        // make redeeming xPACHI available again
        balance.redeemingAmount = balance.redeemingAmount.sub(
            _redeem.xPachiAmount
        );
        _transfer(address(this), msg.sender, _redeem.xPachiAmount);
        ...
        emit CancelRedeem(msg.sender, _redeem.xPachiAmount);
    ...
    /**
     * @dev Allocates caller's "amount" of available xPACHI to "usageAddress" contract
     *
    ...
    function allocate(
        ...
        // allocates xPACHI to usageContract
        IXPachiTokenUsage(usageAddress).allocate(msg.sender, amount, usageData);
    }

    /**
     * @dev Allocates "amount" of available xPACHI from "userAddress" to caller (ie usage contract)
     *
     * Caller must have an allocation approval for the required xPachi xPACHI from "userAddress"
     */
    ...
    /**
     * @dev Deallocates caller's "amount" of available xPACHI from "usageAddress" contract
     *
     * args specific to usage contract must be passed into "usageData"
     */
    function deallocate(
        ...
        // deallocate xPACHI into usageContract
        IXPachiTokenUsage(usageAddress).deallocate(
            msg.sender,
            amount,
            usageData
        );
    ...
    /**
     * @dev Deallocates "amount" of allocated xPACHI belonging to "userAddress" from caller (ie usage contract)
     *
     * Caller can only deallocate xPACHI from itself
     */
    ...
    /**
     * @dev Convert caller's "amount" of PACHI into xPACHI to "to"
     */
    function _convert(uint256 amount, address to) internal {
        ...
        pachiToken.safeTransferFrom(msg.sender, address(this), amount);
    }

    /**
     * @dev Finalizes the redeeming process for "userAddress" by transferring him "pachiAmount" and removing "xPachiAmount" from supply
     *
     * Any vesting check should be ran before calling this
     * PACHI excess is automatically burnt
     */
    function _finalizeRedeem(
        ...
        uint256 pachiExcess = xPachiAmount.sub(pachiAmount);

        // sends due PACHI tokens
        pachiToken.safeTransfer(userAddress, pachiAmount);

        // burns PACHI excess if any
        pachiToken.burn(pachiExcess);
        _burn(address(this), xPachiAmount);

        emit FinalizeRedeem(userAddress, xPachiAmount, pachiAmount);
    }

    /**
     * @dev Allocates "userAddress" user's "amount" of available xPACHI to "usageAddress" contract
     *
     */
    function _allocate(
        address userAddress,
        ...
        uint256 approvedXPachi = usageApprovals[userAddress][usageAddress];
        require(approvedXPachi >= amount, "allocate: non authorized amount");

        // remove allocated amount from usage's approved amount
        usageApprovals[userAddress][usageAddress] = approvedXPachi.sub(amount);
    ...
    /**
     * @dev Deallocates "amount" of available xPACHI to "usageAddress" contract
     *
     * args specific to usage contract must be passed into "usageData"
     */
    function _deallocate(
        ...
        // check if there is enough allocated xPACHI to this usage to deallocate
        ...
        // burn corresponding PACHI and XPACHI
        pachiToken.burn(deallocationFeeAmount);
    ...
    modifier onlyMintManagerOrOwner() {
        require(
            msg.sender == mintManager || msg.sender == owner(),
            "OnlyMintManager: caller is not the mintManager nor owner"
        );
        _;
    }
    ...
    function dropXPachi(
        address to,
        uint256 amount
    ) external onlyMintManagerOrOwner {
        IPachiTokenV2(pachiToken).mintTo(address(this), amount);
        _mint(to, amount);
    }
...
interface IPachiTokenV2 is IERC20 {
...
interface IXPachiToken is IERC20 {
