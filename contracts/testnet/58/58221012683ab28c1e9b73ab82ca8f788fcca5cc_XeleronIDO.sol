...
contract XeleronIDO is Ownable, ReentrancyGuard {
    ...
    //amount
    uint256 public MaxIDOPayInAmount = 100000000000000000000; // 100 ETH
    ...
    function initialize(
        ...
        uint256 _PrivSaleStartTime,
        uint256 _PrivSaleEndTime,
        uint256 _PubSaleStartTime,
        uint256 _PubSaleEndTime
    ) external onlyOwner returns (bool) {
        require(_PrivSaleStartTime >= block.timestamp, "_PrivSaleStartTime error");
        require(_PrivSaleEndTime >= _PrivSaleStartTime, "_PrivSaleEndTime error");
        require(_PubSaleStartTime >= _PrivSaleEndTime, "_PubSaleStartTime error");
        require(_PubSaleEndTime >= _PubSaleStartTime, "_PubSaleEndTime error");

        weth = _weth;
        PayOutToken = _PayOutToken;
        PrivateSaleStartTime = _PrivSaleStartTime;
        PrivateSaleEndTime = _PrivSaleEndTime;
        PublicSaleStartTime = _PubSaleStartTime;
        PublicSaleEndTime = _PubSaleEndTime;
    ...
    function privateSale(uint256 amount) internal {
    ...
    function publicSale(uint256 amount) internal {
    ...
    function buy() nonReentrant payable public {
        ...
        require(block.timestamp > PrivateSaleStartTime, "IDO round1 is upcoming");
    ...
    //set Public Sale limit
    function setPublicSaleLimit(uint256 _MinPublicSaleSinglePayInAmount, uint256 _MaxPublicSaleSinglePayInAmount) external onlyOwner returns (bool) {
        require(_MinPublicSaleSinglePayInAmount > 0, "_MaxPublicSaleSinglePayInAmount error");
        require(_MaxPublicSaleSinglePayInAmount > _MinPublicSaleSinglePayInAmount, "_MaxPublicSaleSinglePayInAmount error");
