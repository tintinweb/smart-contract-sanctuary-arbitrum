...
contract DegenPool {
  ...
  modifier onlyPosOwnerAndNotClosed(bytes32 id) {
  ...
  function hashOrderToEthSigned(Order calldata order) public view returns (bytes32) {
    require(order.validUntil > block.timestamp, "ValidUntil too early");
    return ECDSA.toEthSignedMessageHash(abi.encode(order));
  }

  function hashOrder(Order calldata order) public view returns (bytes memory) {
    require(order.validUntil > block.timestamp, "ValidUntil too early");
  ...
  function verify(Order calldata order, bytes calldata signature_) public view returns(bool) {
    ...
    return ECDSA.recover(ECDSA.toEthSignedMessageHash(abi.encode(order)), signature_) == order.player;
  }
  ...
  function _share(uint96 amount, address player) internal returns (uint96) {
    console.log("amount", amount, player);
  ...
  function _closeInLoss(bytes32 id, Position memory pos, uint64 price, uint96 size) internal onlyPosOwnerAndNotClosed(id) {
  ...
  function _closeInProfit(bytes32 id, Position memory pos, uint64 price, uint96 size) internal onlyPosOwnerAndNotClosed(id) {
  ...
  function closePosition(bytes[] calldata priceData, bytes32 id) external {
    ...
    if (pos.long) {
        if (price <= pos.bustPrice) {
            ...
            _closeInLoss(id, pos, price, size);
        } else {
            _closeInProfit(id, pos, price, size);
        }
    } else if (!pos.long) {
        if (price >= pos.bustPrice) {
            _liquidate(id, pos, price);
  ...
  function emergencyClose(bytes[] calldata priceUpdateData, bytes32 id) external onlyPosOwnerAndNotClosed(id) {
