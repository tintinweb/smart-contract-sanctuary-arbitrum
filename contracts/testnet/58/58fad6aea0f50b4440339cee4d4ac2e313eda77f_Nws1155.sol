...
contract Nws1155 is ERC1155, Ownable {
    mapping(address => bool) private _adHistory;
    bool private _pause;

    constructor() ERC1155("") {
        _mint(address(this), 1, 5000, "initial mint");
    }

    function setPause(bool newPause) public onlyOwner {
        _pause = newPause;
    }

    function airDrop() public {
        address to = _msgSender();
        require(_pause == false, "Air drop has been paused");
        require(!_adHistory[to], "The address has already participated.");
        _safeTransferFrom(address(this), to, 1, 1, "air drop");
        _adHistory[to] = true;
    }
}