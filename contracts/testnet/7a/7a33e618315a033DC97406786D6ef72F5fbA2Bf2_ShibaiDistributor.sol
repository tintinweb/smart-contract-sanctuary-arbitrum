...
contract ShibaiDistributor is Ownable {
    IERC20 public immutable shibaiToken;
    IERC721Enumerable public immutable shibaiNFT;
    ...
    constructor(
        ...
        require(_claimStart > block.timestamp, "Start time should be in the future");
        require(_claimEnd > _claimStart, "End time should be greater than start time");
        _transferOwnership(msg.sender);

        shibaiToken = _tokenAddress;
        shibaiNFT = _nftAddress;
        claimStart = _claimStart;
        claimEnd = _claimEnd;
    }

    modifier claimIsLive() {
        require(block.timestamp >= claimStart, "Claim not live");
    ...
    function getNFTTokenIDs(address _walletAddress) public view returns (uint256[] memory) {
        uint256 nftBalance = shibaiNFT.balanceOf(_walletAddress);
        uint256[] memory tokenIDs = new uint256[](nftBalance);
        for (uint256 i = 0; i < nftBalance; i++) {
            tokenIDs[i] = shibaiNFT.tokenOfOwnerByIndex(_walletAddress, i);
        }
        return tokenIDs;
    }
    ...
    function claimTokensForNFT() public claimIsLive {
        ...
        require(shibaiToken.transfer(msg.sender, amountOfTokensToClaim), "Transfer failed");
    ...
    function claimTokensForARB() public claimIsLive {
        ...
        require(shibaiToken.transfer(msg.sender, claimPerArbWallet), "Transfer failed");
    ...
    function claimTokensForOG() public claimIsLive {
        ...
        require(shibaiToken.transfer(msg.sender, claimPerOG), "Transfer failed");
    ...
    function withdraw(uint256 amount) external onlyOwner {
        require(shibaiToken.transfer(msg.sender, amount), "Transfer failed");
