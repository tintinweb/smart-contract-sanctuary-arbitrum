...
// File @openzeppelin/contracts-upgradeable/utils/math/SignedMathUpg[email protected]
...
// File contracts/interfaces/IOperatorFilter.sol
...
contract Box is ERC1155SupplyUpgradeable, ERC1155PresetMinterPauserUpgradeable {
    ...
    mapping(uint256 => uint256) public specUri;

    function initialize(string memory __uri) override public initializer {
        super.initialize("");
        _uri = __uri;
    }
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        if (specUri[tokenId] > 0) {
            return
                string.concat(
                    _uri,
                    string.concat(specUri[tokenId].toString(), ".json")
                );
        }
    ...
    function _mayTransfer(
        ...
        // if (balanceOf(operator,tokenId)>0) return true;
        ...
        override(ERC1155PresetMinterPauserUpgradeable, ERC1155SupplyUpgradeable)
    {
        if (from != address(0) && to != address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                uint256 id = ids[i];
                if (!_mayTransfer(msg.sender, id)) {
                    revert("ERC1155OperatorFilter: illegal operator");
                }
            }
        }
