  ...
  ManagerModifier
{
  ...
  // Constructor
  //=======================================
  constructor(address _manager, uint256 _maxBatch) ManagerModifier(_manager) {
    maxBatch = _maxBatch;
  }
  ...
  function stakeBatchFor(
    ...
    require(
      addrslength <= maxBatch,
      "StructureStaker: Only allowed up to maxBatch"
    );

    for (; j < addrslength; j++) {
      _stake(_staker, _realmId, _addrs[j], _structureIds[j]);
    }
  }
  ...
  function unstakeBatchFor(
    ...
    for (; j < addrslength; j++) {
      _unstake(_staker, _realmId, _addrs[j], _structureIds[j]);
    }
  }
  ...
  function getStaker(
    ...
    return (staker.staker, staker.stakedAt);
  }
  ...
  function _unstake(
    ...
    // Only staker can unstake
    _onlyStaker(_realmId, _staker, _addr, _structureId);
  ...
  function _onlyStaker(
    uint256 _realmId,
    address _staker,
    address _structureAddr,
    uint256 _structureId
  ) internal view {
    require(
      stakers[_realmId][_structureAddr][_structureId].staker == _staker,
      "StructureStaker: You did not stake this Structure"
    );
  }
}
