...
import {PayWhatYouCanStorage} from "./PayWhatYouCanStorage.sol";

contract PayWhatYouCanFacet is AppFacet {
    mapping(string => bool) _activeTokens;

    function pwycAppEnabled() external view returns (bool) {
        return PayWhatYouCanStorage.layout()._pwycStatus;
    }

    function pwycStartSale(
    ) external onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        PayWhatYouCanStorage.layout()._pwycSaleActive = true;
    }

    function pwycStopSale() external onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        PayWhatYouCanStorage.layout()._pwycSaleActive = false;
    }

    function pwycSaleActive() external view returns(bool) {
        return PayWhatYouCanStorage.layout()._pwycSaleActive;
    }

    function updatePWYCStatus(bool payStatus_) external onlyRolesOrOwner(BaseStorage.ADMIN_ROLE) {
        PayWhatYouCanStorage.layout()._pwycStatus = payStatus_;
    }

    function pwycStartEditionSale(string memory editionId_
    ) external onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        if (_activeTokens[editionId_] == false) {
            PayWhatYouCanStorage.layout()._pwycTokenIds.push(editionId_);
            _activeTokens[editionId_] = true;
        }

        if (PayWhatYouCanStorage.layout()._pwycSaleActive == false) {
            PayWhatYouCanStorage.layout()._pwycSaleActive = true;
        }
    }

    function pwycStopEditionSale(string memory editionId_
    ) external onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        if (_activeTokens[editionId_] == true) {
            removeActiveToken(editionId_);
            _activeTokens[editionId_] = false;
        }
        if (PayWhatYouCanStorage.layout()._pwycTokenIds.length == 0) {
            PayWhatYouCanStorage.layout()._pwycSaleActive = false;
        }
    }

    function removeActiveToken(string memory editionId_) internal {
        uint activeTokensLength = PayWhatYouCanStorage.layout()._pwycTokenIds.length;
        for(uint i = 0; i < activeTokensLength; i++) {
            if(keccak256(abi.encodePacked(PayWhatYouCanStorage.layout()._pwycTokenIds[i])) == keccak256(abi.encodePacked(editionId_))) {
                PayWhatYouCanStorage.layout()._pwycTokenIds[i] = PayWhatYouCanStorage.layout()._pwycTokenIds[activeTokensLength - 1];
                PayWhatYouCanStorage.layout()._pwycTokenIds.pop();
                break;
            }
        }
    }

    function pwycTokenIds() external view returns(string[] memory) {
        return PayWhatYouCanStorage.layout()._pwycTokenIds;
    }
}
...
library PayWhatYouCanStorage {
    // This slot should not be changed
    bytes32 private constant STORAGE_SLOT =
        keccak256("niftykit.apps.payWhatYouCan.storage");

    struct Layout {
        bool _pwycStatus;
        bool _pwycSaleActive;
        string[] _pwycTokenIds;
    }
