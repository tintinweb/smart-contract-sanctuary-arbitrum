...
contract LandDistributor is Permissioned, ERC777Holder {
    ...
    /// @param _recipient amount to send
    /// @param _amount amount to add
    function issueLand(address _recipient, uint256 _amount)
        external
        onlyAllowed
    {
        // Send to the recipient
        IERC777(landContract).operatorSend(address(this), _recipient, _amount , "", "");
    }

    /// @dev Add an amount of a token to the contract
    /// @param _amount amount to add
    function addLand(uint256 _amount)
        external
        onlyOwner
    {
        // Send Land to the contract
        IERC777(landContract).operatorSend(_msgSender(), address(this), _amount , "", "");
    }
  ...
  function _tokensReceived(IERC777 token, uint256 amount, bytes memory) internal view override {
    ...
    require(_msgSender() == address(token),"The contract can only recieve Land tokens");
  }
