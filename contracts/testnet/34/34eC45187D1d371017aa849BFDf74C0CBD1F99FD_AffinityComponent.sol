...
uint256 constant ID = uint256(
    keccak256("game.piratenation.affinitycomponent.dev1")
);
...
library AffinityComponentStorage {
...
/**
 * @title AffinityComponent
 * @dev The affinity of a unit; useful in combat
 */
contract AffinityComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param affinity The affinity of the unit
     */
    function setValue(
        uint256 entity,
        uint8 affinity
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(affinity));
    }
    ...
    function getLayoutValue(
        ...
        value = AffinityComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return affinity The affinity of the unit
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint8 affinity) {
        if (has(entity)) {
            Layout memory s = AffinityComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (affinity) = abi.decode(_getEncodedValues(s), (uint8));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = AffinityComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](1);
        values[0] = abi.encode(s.affinity);
    }
    ...
    function getBytes(
        ...
        Layout memory s = AffinityComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (s.affinity) = abi.decode(value, (uint8));
    ...
    function remove(
        ...
        delete AffinityComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete AffinityComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.affinity));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.affinity);
    }
}