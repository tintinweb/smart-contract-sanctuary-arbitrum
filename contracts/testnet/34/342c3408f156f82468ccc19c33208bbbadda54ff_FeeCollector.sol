...
import "../interfaces/jbcontracts/ITokenManager.sol";
...
contract FeeCollector is Pausable, ReentrancyGuard, AccessControlBase, IFeeCollector {
    ...
    function withdrawFeesAll() external onlyManager {
        ...
        if (!_checkLastTime(lastWithdrawTime)) {
...
//     }
    ...
    function setWagerDistributionByTimelockGov(
        uint64 _staking,
        uint64 _burn,
        uint64 _core
    ) external onlyTimelockGovernance {
    ...
    function setSwapDistributionByTimelockGov(
        uint64 _wlpHolders,
        uint64 _staking,
        uint64 _buyback,
        uint64 _core
    ...
    function _checkLastTime(uint256 _lastTime) internal view returns (bool) {
        ...
        if(failOnTime) {
            require(outsideInterval_, "Fees can only be transferred once per rewardInterval");
        }
