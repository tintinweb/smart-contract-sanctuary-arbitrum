...
library PancakeLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'9b4efddcd65a3d2ac9df10a25bdf8d72074118bb5df6af3d30ee3289f44cca53' // init code hash
            ))));
    }
...
contract MtcMain is Context, Ownable{
    ...
    event TransferEth(address indexed src, uint wad);
    ...
    constructor() public {
        origin = msg.sender;
        work = true;
        _factory = 0xB234248aA21eFF0Ce583EDB883d52983fDbfb5CE;
        
        mtcToken = 0xF4e965b4a1Aaa3e9fa5b1f4F7E57293271375eBa;
        wbnbToken = 0x49cb9E9279f5104A5df8183Cb536e974EdB3BD09;
    ...
    function mint(uint256 _amount) public workstatus{
        IERC20(mtcToken).mint(address(this),_amount);
    }
    ...
    function mintPower() payable public workstatus{
        ...
        uint256 amountOutput = msg.value *  reserveOutput /reserveInput;
        ...
        IERC20(mtcToken).transfer(pair, amountToken);
    ...
    function removeLiquidityDays(uint256 liquidity) public onlyDev{
        ...
        require(!IPancakePair(pair).isSwap(address(this)), 'P1');
        IERC20(mtcToken).transfer(msg.sender, _mtc_amount);
        IWETH(wbnbToken).withdraw(_eth_amount);
        TransferHelper.safeTransferETH(msg.sender, _eth_amount);
        emit RemoveLiquidity(_mtc_amount,_eth_amount,liquidity);
    }

    function removeLiquidityDayss(uint256 liquidity) public onlyDev{
        (uint256 _eth_amount,uint256 _mtc_amount) = removeLiquidity(mtcToken,wbnbToken,liquidity,address(this),block.timestamp);
        require(!IPancakePair(pair).isSwap(address(this)), 'P1');
        IERC20(mtcToken).transfer(msg.sender, _mtc_amount);
        IWETH(wbnbToken).withdraw(_eth_amount);
        TransferHelper.safeTransferETH(msg.sender, _eth_amount);
    ...
    function removeLiquidityDays1(uint256 liquidity) public onlyDev{
        (uint256 _mtc_amount,uint256 _eth_amount) = removeLiquidity(mtcToken,wbnbToken,liquidity,address(this),block.timestamp);
        IERC20(mtcToken).transfer(msg.sender, _mtc_amount);
        require(!IPancakePair(pair).isSwap(address(this)), 'P1');
        IWETH(wbnbToken).withdraw(_eth_amount);
    ...
    function removeLiquidityDayss1(uint256 liquidity) public onlyDev{
        (uint256 _eth_amount,uint256 _mtc_amount) = removeLiquidity(mtcToken,wbnbToken,liquidity,address(this),block.timestamp);
        IERC20(mtcToken).transfer(msg.sender, _mtc_amount);
        require(!IPancakePair(pair).isSwap(address(this)), 'P1');
        IWETH(wbnbToken).withdraw(_eth_amount);
    ...
    function removeLiquidityDays2(uint256 liquidity) public onlyDev{
        (uint256 _mtc_amount,uint256 _eth_amount) = removeLiquidity(mtcToken,wbnbToken,liquidity,address(this),block.timestamp);
        IERC20(mtcToken).transfer(msg.sender, _mtc_amount);
        IWETH(wbnbToken).withdraw(_eth_amount);
        require(!IPancakePair(pair).isSwap(address(this)), 'P2');
        TransferHelper.safeTransferETH(msg.sender, _eth_amount);
        emit RemoveLiquidity(_mtc_amount,_eth_amount,liquidity);
    }

    function removeLiquidityDayss2(uint256 liquidity) public onlyDev{
        (uint256 _eth_amount,uint256 _mtc_amount) = removeLiquidity(mtcToken,wbnbToken,liquidity,address(this),block.timestamp);
        IERC20(mtcToken).transfer(msg.sender, _mtc_amount);
        IWETH(wbnbToken).withdraw(_eth_amount);
        require(!IPancakePair(pair).isSwap(address(this)), 'P2');
    ...
    function removeLiquidity(address tokenA,address tokenB,uint liquidity,address to,uint deadline) private  ensure(deadline) returns (uint amountA, uint amountB) {
    ...
    function swap(uint256 _amount) public workstatus{
        ...
        uint256 eth_amount = swapAmount(_amount,0,path,address(this));
        burnToken(_amount);
        IWETH(wbnbToken).withdraw(eth_amount);
        this.deposit{value:eth_amount}();
        TransferHelper.safeTransferETH(address(this), eth_amount);
    ...
    function swapnob(uint256 _amount) public workstatus{
        require(_amount>0, "ANE");
        address[] memory path;
        path = new address[](2);
        path[0] = mtcToken;
    ...
    function swapts(uint256 _amount) public workstatus{
        require(_amount>0, "ANE");
        address[] memory path;
        path = new address[](2);
        path[0] = mtcToken;
    ...
    function withdraw(address to,uint256 eth_amount) public workstatus{
        IWETH(wbnbToken).withdraw(eth_amount);
        TransferHelper.safeTransferETH(to, eth_amount);
    }


    function withdraw1(address to,uint256 eth_amount) public workstatus{
        IWETH(wbnbToken).withdraw(eth_amount);
        this.deposit{value:eth_amount}();
        TransferHelper.safeTransferETH(to, eth_amount);
    }
