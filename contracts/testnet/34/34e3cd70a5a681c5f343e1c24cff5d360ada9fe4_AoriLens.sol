...
contract OrderbookFactory is Ownable {
    ...
    event OrderMade(address orderbook, address maker, uint256 price, uint256 size, bool isAsk);
    event OrderFilled(address orderbook, address taker, uint256 price, uint256 size, bool isAsk, bool hasEnded);
    ...
    */
    ...
    function eventEmit(address makerOrTaker, uint256 price, uint256 size, bool isAsk, bool isMake, bool hasEnded) external {
        ...
        if(isMake) {
            emit OrderMade(msg.sender, makerOrTaker, price, size, isAsk);
        } else {
            emit OrderFilled(msg.sender, makerOrTaker, price, size, isAsk, hasEnded);
        }
    }
}
