...
contract PeerToPeer is ReentrancyGuard, Ownable {
    ...
    function cancelPosition(uint256 _positionId) external nonReentrant {
        ...
        delete positionIdsByAddress[msg.sender][_positionId - 1];
    ...
    function takePosition(uint256 _positionId) external nonReentrant {
        if (positions[_positionId].open != true) {
            revert Peer__PositionIsClosed();
        }
    ...
    function withdrawPosition(uint256 _positionId) external nonReentrant {
        if (positions[_positionId].sellerAddress != msg.sender) {
            revert Peer__NotAutorized();
        }
        if (positions[_positionId].open == true) {
        ...
        delete positions[_positionId];
        delete positionIdsByAddress[msg.sender];
