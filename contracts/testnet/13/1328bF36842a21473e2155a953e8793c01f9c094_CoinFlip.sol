...
contract CoinFlip is Ownable, ReentrancyGuard {
    ...
    event Lose(address loser, uint256 amount);
    ...
    function maxBet() public view returns (uint256) {
        return rewardPool / 1000; //0.1% from reward pool
    }
    ...
    function flip(uint256 userBet, bool isTail) external payable returns (uint256) {
        ...
        flipToAddress[id] = UserInfo(msg.sender, userBet, isTail);
    ...
    function randomizerCallback(uint256 _id, bytes32 _value) external {
        ...
        if (player.isTail == result) {
            ...
            emit Win(player.user, player.userBet*2);
        } else {
            rewardPool += player.userBet;
            emit Lose(player.user, player.userBet);
        }
    }
