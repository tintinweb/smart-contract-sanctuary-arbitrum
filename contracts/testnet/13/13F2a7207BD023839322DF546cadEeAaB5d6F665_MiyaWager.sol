...
contract MiyaWager is Wager {
    constructor(IERC20 _DMT, address _oracle) Wager(_DMT, _oracle) {}
}
...
import {WagerSettings} from "./WagerSettings.sol";
import {FeeCollection, Fee} from "../FeeCollection.sol";
// import {SwapAndSend} from "./SwapAndSend.sol";

abstract contract Wager is FeeCollection, WagerSettings, OwnableOracle, Pausable {
    struct WagerGame {
        uint256 wagerAmount;
        address[] players;
    }

    mapping(uint256 gameId => WagerGame game) public games;
        ...
        onlyOracle
    {
        ...
        require(numPlayers >= 2 && numPlayers <= maxPlayers, "Need between 2 and maxPlayers");
        require(
        ...
        games[gameIdCounter] = WagerGame({players: _players, wagerAmount: _wagerAmount});
    ...
    function decideGame(uint256 _gameId, address _winner) external onlyOracle {
        WagerGame memory game = games[_gameId];
    ...
    function refundPlayers(WagerGame memory game) private {
        for (uint256 i = 0; i < game.players.length; i++) {
            require(DMT.transfer(game.players[i], game.wagerAmount), "Token transfer failed");
        }
    }

    function releasePlayers(WagerGame memory game) private {
        for (uint256 i = 0; i < game.players.length; i++) {
...
abstract contract WagerSettings is FeeCollection, Ownable {
    ...
    function updateMaxPlayers(uint256 _maxPlayers) external onlyOwner {
