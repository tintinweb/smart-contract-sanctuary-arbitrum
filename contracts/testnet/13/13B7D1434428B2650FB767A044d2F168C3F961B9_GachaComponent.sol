...
uint256 constant ID = uint256(keccak256("game.piratenation.gachacomponent"));

/**
 * @title GachaComponent
 * @dev Gacha Component
 */
contract GachaComponent is BaseComponent {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Entity ID of the loot set to output
        keys[0] = "output_loot_set_entity";
     ...
     * @param outputLootSetEntity uint256 Entity ID of the loot set to output    
     */
    function setValue(
        uint256 entity,
        uint256 outputLootSetEntity
    ) external virtual {
        setBytes(entity, abi.encode(outputLootSetEntity));
    }
    ...
    {
        if (has(entity)) {
            (outputLootSetEntity) = abi.decode(
                getBytes(entity),
                (uint256)
            );
        }
