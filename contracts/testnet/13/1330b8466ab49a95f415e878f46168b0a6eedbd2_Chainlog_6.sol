...
import {Chainlog} from "src/Chainlog.sol";

contract Chainlog_6 is Chainlog {
    // @todo      ^ Adjust name of Chainlog instance.
    constructor(address initialAuthed) Chainlog(initialAuthed) {}
}

/**
 * @notice Chainlog Management Script
 */
contract ChainlogScript is Script {
    /// @dev Deploys a new Chainlog instance via Greenhouse instance
        ...
        public
    {
        ...
        bytes memory creationCode = abi.encodePacked(
            type(Chainlog_6).creationCode, abi.encode(initialAuthed)
            // @todo      ^ Adjust name of Chainlog instance.
        );
    ...
    /// @dev Sets address `addr` to key `key`.
    function setAddress(address self, bytes32 key, address addr) public {
        vm.startBroadcast();
        IChainlog(self).setAddress(key, addr);
        vm.stopBroadcast();

        console2.log("Address set");
        console2.log("Key:");
    ...
    /// @dev Sets version to `version`.
    function setVersion(address self, string memory version) public {
        vm.startBroadcast();
        IChainlog(self).setVersion(version);
        vm.stopBroadcast();

        console2.log("Version set to", version);
    }

    /// @dev Sets sha256sum to `sha256sum`.
    function setSha256sum(address self, string memory sha256sum) public {
        vm.startBroadcast();
        IChainlog(self).setSha256sum(sha256sum);
        vm.stopBroadcast();

        console2.log("Sha256sum set to", sha256sum);
    }

    /// @dev Sets IPFS to `ipfs`.
    function setIPFS(address self, string memory ipfs) public {
        vm.startBroadcast();
        IChainlog(self).setIPFS(ipfs);
        vm.stopBroadcast();

        console2.log("IPFS set to", ipfs);
    }
