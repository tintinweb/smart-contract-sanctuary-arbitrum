...
import "./layerzero/util/ExcessivelySafeCall.sol";
...
contract BridgeProxy is Initializable, NonblockingLzAppUpgradeable {
    ...
    function initialize(address _token, address _router, address _endpoint) external initializer {
    ...
    function getRecieveQueueInfo(uint16 _srcChainId, bytes calldata _srcAddress, uint64 _nonce)
        external
        view
        returns (address _to, uint256 _amount)
    {
        ReceiveInfo memory _info = receiveQueue[_srcChainId][_srcAddress][_nonce];
        _to = _info.to;
    ...
    function sendTokens(
        ...
        bytes memory _payload = abi.encode(_to, _amount);
        _lzSend(_dstChainId, _payload, _refundAddress, _zroPaymentAddress, _adapterParam, msg.value);
        uint64 nonce = lzEndpoint.getOutboundNonce(_dstChainId, address(this));
        emit SendToChain(_dstChainId, _to, _amount, nonce);
    }

    function deQueue(uint16 _srcChainId, bytes calldata _srcAddress, uint64 _nonce) external onlyRouter {
        ReceiveInfo memory _info = receiveQueue[_srcChainId][_srcAddress][_nonce];
        if (_info.to != address(0)) {
            _info.amount = 0;
            _info.nonce = 0;
            _info.srcChainId = 0;
            _info.to = address(0);
            receiveQueue[_srcChainId][_srcAddress][_nonce] = _info;
            emit Dequeue(_srcChainId, _srcAddress, _nonce);
        }
    }

    // For test case handle fail, remove on mainnet
    function nonblockingLzReceive(
        ...
        require(_msgSender() == address(this), "Caller must be Bridge Proxy");

        require(!IBridgeRouter(router).paused(), "Paused");
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload)
        ...
        override
    {
        (bytes memory _to, uint256 _amount) = abi.decode(_payload, (bytes, uint256));
        address _toAddress;
        assembly {
        ...
        receiveQueue[_srcChainId][_srcAddress][_nonce] =
            ReceiveInfo({to: _toAddress, amount: _amount, srcChainId: _srcChainId, nonce: _nonce});
    ...
    event Dequeue(uint16 _srcChainId, bytes _srcAddress, uint64 _nonce);
}
