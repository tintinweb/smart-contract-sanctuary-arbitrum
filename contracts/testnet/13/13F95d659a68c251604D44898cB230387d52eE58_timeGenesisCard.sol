...
contract timeGenesisCard is ERC721URIStorage, Ownable{
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    uint256 private immutable totalSupply=10000;   //total 10000
    uint256 private whiteUserNum;  //whitelist amount
    ...
    receive() external payable {
        emit Received(msg.sender, price(msg.sender));
    }

    //public mint
    function mintTimeWhale(address userAddress)public payable{
        //limit total 10000
        require(_tokenIdCounter.current()<=totalSupply,"There are no NFTS available for mint anymore"); 
        uint256 tokenId = _tokenIdCounter.current();
        uint256 contractFee=price(userAddress);
        userIfWhitelist[userAddress] = false;
    ...
    function price(address userAddress)public view returns(uint256){
        if(userIfWhitelist[userAddress]){
            ...
            //Not whitelist 0.02 fee
            return 0.02 ether;
        }
    }

    //set uri
    function setWhaleUri(string calldata whaleUri)public onlyOwner{
        uri=whaleUri;
    }
