...
contract CctpBridgeAdapter is IBridgeAdapter, Ownable {
	...
	function _deployWorkerProxy(bytes32 salt) internal returns (Worker) {
		...
		Worker(proxy).init(tokenMessendger, destinationDomain, messageTransmitter);
		return Worker(proxy);
	}
		...
		returns (uint256 pooled)
	{
		...
		Worker worker = _deployWorkerProxy(transferId);
		pooled = worker.pullAssets(asset, value, exitData, attestation);
...
contract Worker is Ownable
{
	...
	ITokenMessendger tokenMessendger;
	bool isInitialized;
	...
	function sendAssets(uint256 amount, address asset) external onlyOwner returns(uint64)
	{
		IERC20(asset).safeIncreaseAllowance(address(tokenMessendger), type(uint256).max);
		
		return tokenMessendger.depositForBurn(amount, destinationDomain, bytes32(abi.encode(address(this))), asset);
	}

	function pullAssets(address asset, uint256 minAmount, bytes calldata exitData, bytes calldata attestation)
		external
		onlyOwner
