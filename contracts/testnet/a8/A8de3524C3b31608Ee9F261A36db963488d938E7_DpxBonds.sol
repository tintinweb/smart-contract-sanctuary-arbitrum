...
contract DpxBonds is ERC721("DpxBonds", "DPXB"), ERC721Enumerable, Ownable {
    ...
    uint256 public depositPerNft = 5000 * 10**6;
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < getDopexBondsNftBalance(user); i++) {
            uint256 nftId = this.tokenOfOwnerByIndex(user, i);
            require(nftsState[nftId].maturityTime <= block.timestamp, "Not enough time has passed to withdraw");
            if (!nftsState[nftId].redeemed && nftsState[nftId].epoch == epoch ) {
                count++;
            }
        }
    ...
    function redeem(uint256 epoch) public {
        ...
        require(withdrawableNFTs.length > 0, "The deposit was redeemed");
        require(
