...
contract UXDToken is Ownable, IRedeemable, ERC20("UXD", "UXD Stablecoin") {
    ...
    function setSupplyCap(uint256 maxSupply) external onlyOwner {
        require(maxSupply > 0, "Zero Amount");
        supplyCap = maxSupply;
...
interface IRageDnDepository {
    ...
    function redeemShares(uint256 shareAmount) external returns (uint256, uint256);
}
...
import {IDnGmxSeniorVault} from "../../external/rage/IDnGmxSeniorVault.sol";
import {IRageDnDepository} from "./IRageDnDepository.sol";
import {IUXDController} from "../../core/IUXDController.sol";
import {MathLib} from "../../libraries/MathLib.sol";

contract RageDnDepository is IRageDnDepository {
    ...
    function deposit(uint256 assetAmount) external onlyController returns (uint256) {
        netAssetDeposits += assetAmount;
        IERC20(asset).approve(address(vault), assetAmount);
        uint256 shares = vault.deposit(
            assetAmount, address(this)
        );
    ...
    /// @dev withdraw a specific amount of asset
    function redeem(uint256 redeemableAmount) onlyController external returns (uint256) {
        uint256 assetAmount = _redeemableToAssets(redeemableAmount);
        totalRedeemable -= redeemableAmount;
        netAssetDeposits -= assetAmount;
        uint256 shares = vault.withdraw(
            assetAmount, address(controller), address(this)
        );
        emit Withdrawn(msg.sender, assetAmount, redeemableAmount, shares);
        return assetAmount;
    }

    /// dev redeem a specific amount of redeemable
    function redeemShares(uint256 shares) external returns (uint256, uint256) {
        uint256 assetAmount = vault.redeem(shares, address(controller), address(this));
        uint256 redeemableAmount = _assetsToRedeemable(assetAmount);
        netAssetDeposits -= assetAmount;
        totalRedeemable -= redeemableAmount;
        emit Redeemed(msg.sender, assetAmount, redeemableAmount, shares);
        return (assetAmount, redeemableAmount);
    }
    ...
    function getDepositoryAssets() external view returns (uint256) {
        return vault.convertToAssets(vault.balanceOf(address(this)) );
    }

    function _assetsToRedeemable(uint256 assetAmount) internal view returns (uint256) {
        return assetAmount.fromDecimalToDecimal(IERC20Metadata(asset).decimals(), 18);
    }

    function _redeemableToAssets(uint256 redeemableAmount) internal view returns (uint256) {
        return redeemableAmount.fromDecimalToDecimal(18, IERC20Metadata(asset).decimals());
    }
}
...
library MathLib {
    ...
    /// @dev Converts a number from one decimal to another
    /// @param amount the number to convert
    /// @param inDecimals the number decimals of the input number.
    /// @param outDecimals the number decimals of the output number.
