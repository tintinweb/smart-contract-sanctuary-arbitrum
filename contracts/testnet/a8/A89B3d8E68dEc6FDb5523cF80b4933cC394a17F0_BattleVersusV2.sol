...
contract BattleVersusV2 is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  function fight(
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      require(
        BATTLE_ENTRY.isEligible(addr, adventurerId, oppAddr, oppAdventurerId),
        "BattleVersusV2: Opponent is not eligible to battle"
      );

      // Set entry
      BATTLE_ENTRY.set(addr, adventurerId, entryDays);
  ...
  function updateEntryDays(uint256 _value) external onlyAdmin {
    entryDays = _value;
  }
  ...
  function _traitBattle(
    ...
    if (traitBattleData.attackerTrait < traitBattleData.opponentTrait) {
      ...
      if (traitBattleData.won) {
        traitBattleData.attackerAnima = _rewardWinner(
          traitBattleData,
        ...
        traitBattleData.opponentAnima = _rewardWinner(
          traitBattleData,
...
interface IBattleEntry {
  ...
  // External
  //=======================================
  function set(address _addr, uint256 _id, uint256 _seconds) external;
