...
contract DistributionContract {
    ...
    for (uint256 i = 0; i < token.holdersCount(); i++) {
        ...
        if (isEligible(holder)) {
            ...
            uint256 holderPercentage = (holderBalance * 100) / totalTokenSupply;
            uint256 holderShareArbot = (holderBalance * remainingDistributionEth) / totalTokenSupply;
            uint256 holderShareEth = (remainingDistributionEth * holderPercentage) / 100;
    ...
    function claim() external {
        ...
        require(availableArbot > 0 || availableEth > 0, "No available rewards to claim");

        holders[msg.sender].accumulatedArbot = 0;
        holders[msg.sender].accumulatedEth = 0;
        holders[msg.sender].lastClaimTimestamp = block.timestamp;
    ...
    function isEligible(address holder) public view returns (bool) {
        return (block.timestamp >= holders[holder].lastClaimTimestamp + CLAIM_COOLDOWN);
    }
