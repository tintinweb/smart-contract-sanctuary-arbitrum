...
import "../interfaces/IAquariusAggregatorPriceOracle.sol";
...
contract Compounder is OwnableUpgradeable, PausableUpgradeable {
	...
	event RewardTokensUpdated(address[] _tokens);
	...
	IMintableToken public arsToken;
	address public baseToken; //token that ars is paired with in LP
	address public priceProvider;
	address[] public rewardTokens;
	address[] public wethToArs;
	address public uniRouter;
	address public multiFeeDistribution;
	address public lockZap;
	address public bountyManager;
	IAquariusAggregatorPriceOracle public oracle;
	mapping(address => uint256) public lastAutocompound;
	mapping(address => address[]) public rewardToBaseRoute;

	address public aCore;
	...
	function initialize(
		...
		require(_oracle != address(0), "0x0 address");
		require(_compoundFee > 0 && _compoundFee <= 2000, "Invalid compound fee");
		require(_slippageLimit >= 0 && _slippageLimit < PERCENT_DIVISOR, "Invalid slippage limit");

		uniRouter = _uniRouter;
		multiFeeDistribution = _mfd;
		baseToken = _baseToken;
		lockZap = _lockZap;
		oracle = IAquariusAggregatorPriceOracle(_oracle);
		arsToken = IMultiFeeDistribution(multiFeeDistribution).arsToken();
		priceProvider = IMultiFeeDistribution(multiFeeDistribution).getPriceProvider();
		wethToArs = [baseToken, address(arsToken)];
	...
	function addRewardTokens(address[] memory _tokens) external onlyOwner {
		rewardTokens = _tokens;
		emit RewardTokensUpdated(_tokens);
	}
	...
	function setAcore(address _aCore) external onlyOwner {
		aCore = _aCore;
	}

	function setWcore(address _Wcore) external onlyOwner {
		Wcore = _Wcore;
	}

	function setATokens(address[] memory _tokens, address[] memory _aTokens) external onlyOwner {
		require(_tokens.length == _aTokens.length, "Invalid param");

		uint256 length = _aTokens.length;
	...
	function _claimAndSwapToBase(address _user) internal returns (uint256) {
		...
		for (uint256 i = 0; i < rewardTokens.length; i++) {
			...
			if (rewardTokens[i] != baseToken) {
				IERC20(rewardTokens[i]).safeApprove(uniRouter, 0);
				IERC20(rewardTokens[i]).safeApprove(uniRouter, balance);
				try
					IUniswapV2Router01(uniRouter).swapExactTokensForTokens(
						balance,
						0,
						rewardToBaseRoute[rewardTokens[i]],
						address(this),
						block.timestamp + 600
					)
				{} catch {}
	...
	function _convertBaseToLPandStake(address _user) internal returns (uint256 liquidity) {
		...
		if (baseBal != 0) {
			...
			liquidity = ILockZap(lockZap).zapOnBehalf(baseBal, 0, _user);
		}
	}
	...
	function claimCompound(address _user, bool _execute) public returns (uint256 fee) {
		...
		if (!_execute) {
			uint256 pendingInArs = _wethToArs(noSlippagePendingEth, _execute);
			fee = (pendingInArs * compoundFee) / PERCENT_DIVISOR;
			return fee;
		}
		...
		if (isAutoCompound) {
			fee = _wethToArs(((actualWethAfterSwap * compoundFee) / PERCENT_DIVISOR), _execute);
		}
		_convertBaseToLPandStake(_user);
		if (isAutoCompound) {
			arsToken.approve(bountyManager, fee);
	...
	function viewPendingRewards(address _user) public view returns (address[] memory tokens, uint256[] memory amts) {
		...
		for (uint256 i = 0; i < pending.length; i++) {
			if (pending[i].token != address(arsToken)) {
	...
	function _estimateTokensOut(address _in, address _out, uint256 _amtIn) internal view returns (uint256 tokensOut) {
		uint256 priceInAsset = oracle.getUnderlyingPrice(aTokens[_in]);
		uint256 priceOutAsset = oracle.getUnderlyingPrice(aTokens[_out]);
		tokensOut = _amtIn * priceInAsset / priceOutAsset;
	}
	...
	function _wethToArs(uint256 _wethIn, bool _execute) internal returns (uint256 arsOut) {
		uint256 arsPrice = IPriceProvider(priceProvider).getTokenPrice();
		if (_wethIn != 0) {
			if (_execute) {
				...
				arsOut = amounts[amounts.length - 1];
			} else {
		...
		uint256 ethValueOfARS = arsPrice * arsOut;
		require(ethValueOfARS / 10 ** 8 >= (_wethIn * slippageLimit) / 10000, "too much slippage");
	}
...
interface IAquariusAggregatorPriceOracle {
...
interface IMultiFeeDistribution is IFeeDistribution {
	...
	function arsToken() external view returns (IMintableToken);
...
struct Balances {
	uint256 total; // sum of earnings and lockings; no use when LP and ARS is different
	uint256 unlocked; // ARS token
	uint256 locked; // LP token or ARS token
	uint256 lockedWithMultiplier; // Multiplied locked amount
	uint256 earned; // ARS token
}