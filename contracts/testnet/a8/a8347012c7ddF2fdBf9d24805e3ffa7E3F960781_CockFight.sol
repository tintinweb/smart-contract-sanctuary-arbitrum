...
contract CockFight is Ownable, ReentrancyGuard {
    ...
    uint256 private betFee; //opposite of platform fee
    ...
    address public distributorContract; // contract address of fee receiver
    address public distributor; // wallet address for fee distributor
    ...
    constructor(
        ...
        require(
            _fee <= BASERATE,
            "Platform fee should be greater less than or equal to 10,000"
        );
        tokenAddress = _tokenAddress;
        minimumBet = _minAmountBet;
        fee = _fee;
        betFee = BASERATE - _fee;

        Agent[msg.sender] = true;
        distributor = _distributor;
        distributorContract = _distribute;
    }
    ...
    function betWhite(uint256 roundID, uint256 amount) external nonReentrant {
        ...
        require(
            ledger[roundID][msg.sender].amountBet == 0 ||
        ...
        //compute
        uint256 feeBet = ((amount * betFee) / BASERATE);

        if (ledger[roundID][msg.sender].amountBet == 0) {
            // Update round data
            ...
            betInfo.amountFinal = amount - feeBet;
            betInfo.amountBet = amount;
        ...
        uint256 updateAmount = betInfo.amountBet;
    ...
    function claim(uint256[] calldata roundIDs) external nonReentrant {
        ...
        for (uint256 i = 0; i < roundIDs.length; i++) {
            ...
            if (round.gameResult == Position.White) {
                ...
                addedReward =
                    (ledger[roundIDs[i]][msg.sender].amountBet *
                        round.oddsAmount) /
                    BASERATE;
            ...
            // Round Draw, refund bet final
            else if (round.gameResult == Position.Draw) {
                require(
                    refundable(roundIDs[i], msg.sender),
                    "Not eligible for refund"
                );
                addedReward = ledger[roundIDs[i]][msg.sender].amountFinal;
            }
            // Round Cancelled, refund bet amount
            else if (round.gameResult == Position.Cancelled) {
                require(
                    refundable(roundIDs[i], msg.sender),
                    "Not eligible for refund"
                );
                addedReward = ledger[roundIDs[i]][msg.sender].amountBet;
            }
        ...
        onlyAgent
    {
        ...
        if (_gameResult == Position.Cancelled) {
    ...
    function setFee(uint256 newFee) external onlyOwner {
        require(newFee >= 0 && newFee <= BASERATE, "Fee invalid!");
        fee = newFee;
        betFee = BASERATE - newFee;
     ...
     * @dev Callable by contract owner
     */
    function setNewDistribute(address _distributorContract) external onlyOwner {
     ...
     */
    function claimTreasury() external nonReentrant onlyDistributer {
        require(
            distributorContract != address(0),
            "The distributor address is zero address"
        );
        ...
        returns (bool)
    {
        ...
        return !round.gameStatus && !betInfo.claimed && betInfo.amountBet != 0;
    }
    ...
    function _calculateRewards(uint256 roundID) internal {
        Round storage round = rounds[roundID];
        require(round.oddsAmount == 0, "Rewards calculated");

        uint256 totalBets = round.totalAmountBet;
        uint256 ads = 0;
        ...
        if (
            ...
            if (round.WhiteAmount != 0 && round.RedAmount == 0) {
                // White wins and Red has no bet
                if (round.gameResult == Position.White) {
                    // White wins
                    rewardAmount = fee;
                    uint256 deduction = (totalBets * betFee) / BASERATE;
                    treasuryAmt = deduction; // Only 10% of bet amount goes to treasuryAmount
                } else {
                    // Red wins
                    treasuryAmt = totalBets; // All the bet amount goes to treasuryAmount
                }
            } else if (round.WhiteAmount == 0 && round.RedAmount != 0) {
                // Red wins and White has no bet
            ...
            } else if (round.RedAmount != 0 && round.WhiteAmount != 0) {
                // Bets on both sides
                ads =
                    (totalBets * fee) /
                    (
            ...
            ads = (totalBets * betFee) / BASERATE;
            treasuryAmt = ads;
        }

        round.oddsAmount = rewardAmount;
        round.totalFee = treasuryAmt;

        // Add to treasury
        treasuryAmount += treasuryAmt;

        emit RewardsCalculated(roundID, rewardAmount, treasuryAmount);
    }
