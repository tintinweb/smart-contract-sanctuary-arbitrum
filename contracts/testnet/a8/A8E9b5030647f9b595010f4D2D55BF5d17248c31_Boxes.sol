...
/// @dev Farmland - Starter Box Smart Contract
...
// FUNCTIONS
    ...
    /// @dev Mint a Starter Box
    function mintBoxes(uint256 _boxID, uint256 _amount)
        external
        payable
        nonReentrant
    {
        ...
        uint256 totalPrice = box.boxPrice * _amount;
        uint256 totalLand = box.amountOfLand * _amount;        

        // Check the correct ETH is sent
        require( msg.value >= totalPrice,"Starter Box: ETH sent is not correct" );
    ...
    function singletonAddress(address _value)
        internal
        pure
    ...
    function singletonUint256(uint256 _value)
        internal
        pure
        ...
        onlyOwner
    {
        ...
        require(sent, "Starer Box: Failed to send Ether");
    }
...
struct Box { 
    ...
    string name;
    /// @dev This is the amount of ETH required for the Starter box
    uint256 boxPrice;
    /// @dev This is the amount of ETH required for the Citizen
    uint256 citizenPrice;
    /// @dev This is the box ID to include in the starter box
    uint256 packID;
    /// @dev This is the total number of boxes to include in the starter box
    uint256 numberOfPacks;
    /// @dev This is the total number of items in a box to include in the starter box (3 = small, 5= medium, 10 = large)
    uint256 numberOfItemsInPack;
    /// @dev This is the total number of mercenaries to include in the starter box
    uint256 numberOfMercenaries;
    /// @dev This is the amount of Land to include in the starter box
...
// MODIFIERS
    ...
    modifier onlyWhenBoxEnabled(uint256 boxID) {
        require (boxes[boxID].active, "Box inactive");
        _;
    }
    ...
    modifier onlyIfBoxExists(uint256 boxID) {
        require(boxID < lastBoxID,"Box does not exist");
        _;
    }
...
// ADMIN FUNCTIONS
    ...
    /// @param _box Box struct
    function addBox(Box calldata _box)
        external
        onlyOwner
    {
        // Set the box details
        boxes[lastBoxID] = _box;
    ...
    /// @param _box box  struct
    function updateBox(uint256 _boxID, Box calldata _box)
        external 
        onlyOwner
        onlyIfBoxExists(_boxID)
    {
        // Set the box details
        boxes[_boxID] = _box;
    }
    ...
    /// @param _boxID ID of the box
    function deleteBox(uint256 _boxID)
        external 
        onlyOwner
        onlyIfBoxExists(_boxID)
    {
    ...
    /// @param _value to start or stop the contract
    function isPaused(bool _value)
        external
        onlyOwner
    {
        if ( !_value ) {
