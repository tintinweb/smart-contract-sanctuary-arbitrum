...
contract Executor is Ownable {
    ...
    modifier onlyOperator {
        require(this.isOperator(msg.sender), "Executor: !operator");
        _;
    }

    function aggregate(Call[] memory calls) external onlyOperator returns (uint256 blockNumber, bytes[] memory returnData)  {
    ...
    function tryAggregate(bool requireSuccess, Call[] memory calls) public returns (Result[] memory returnData) {
        ...
        for(uint256 i = 0; i < calls.length; i++) {
            ...
            if (requireSuccess) {
                require(success, "Executor aggregate: call failed");
            }
