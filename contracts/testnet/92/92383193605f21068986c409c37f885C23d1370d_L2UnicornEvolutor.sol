 ...
 * @notice 进化
 */
contract L2UnicornEvolutor is BaseGame, BaseL2Unicorn, BaseL2UnicornRule, BaseRecipient, Pausable {
     ...
     * @notice 进化
     */
    function evolve(uint256 gasPrice_, uint256 callbackGasLimit_, uint256[] calldata tokenIdArr) payable external whenNotPaused {
        //验证
        ...
        require(!Pausable(l2Unicorn).paused(), "L2UnicornEvolutor: contract already paused");
        //存入随机数手续费
    ...
    function randomizerCallback(uint256 requestId_, bytes32 seed_) external {
        ...
        //
        uint256 randomNumber_ = getEvolveRandomNumber(seed_);
        //进化级别集合
        ...
        uint256 evolveTokenIdCount_ = evolveTokenIdArr_.length;
        //
        for (uint256 index_ = 0; index_ < evolveTokenIdCount_;) {
            //根据进化TokenId级别和随机数，取进化到的TokenId
            IL2UnicornRule.HatchRule memory hatchRule = IL2UnicornRule(l2UnicornRule).getHatchRuleByEvolveTokenIdLevelRandomNum(evolveLevelArr_[index_], randomNumber_);
            //进化铸造
            uint256 newTokenId = IL2Unicorn(l2Unicorn).mintForLevel(user_, hatchRule.level, hatchRule.startTokenId);
            //
            emit EvolveCallback(user_, requestId_, seed_, evolveTokenIdArr_[index_], evolveLevelArr_[index_], newTokenId, hatchRule.level);
            //
            unchecked{++index_;}
        }
    }
     ...
     * @notice 获取进化随机数
     */
    function getEvolveRandomNumber(bytes32 seed_) public view returns (uint256){
     ...
     * @notice 查看进化结果
     */
    function viewEvolveResults(uint256 requestId_, uint8 startIndex_, uint8 endIndex_) external view returns (EvolveResult[] memory evolveResults){
        //进化级别集合
        uint256[] memory evolveTokenIdArr_ = _requestIdEvolveTokenIdArr[requestId_];
        uint8[] memory evolveLevelArr_ = _requestIdEvolveLevelArr[requestId_];
        uint256 evolveTokenIdCount_ = evolveTokenIdArr_.length;
        ...
        //
        uint256 randomNumber_ = getEvolveRandomNumber(gameInfo_.seed);
        //
        if (startIndex_ >= 0 && endIndex_ >= startIndex_) {
            uint256 len = endIndex_ + 1 - startIndex_;
            uint256 arrayLen = len > evolveTokenIdCount_ ? evolveTokenIdCount_ : len;
            evolveResults = new EvolveResult[](arrayLen);
            uint256 arrayIndex_ = 0;
            for (uint8 index_ = startIndex_; index_ < ((endIndex_ > evolveTokenIdCount_) ? evolveTokenIdCount_ : endIndex_);) {
                evolveResults[arrayIndex_] = EvolveResult(
                    requestId_,
                    gameInfo_.user,
                    gameInfo_.seed,
                    index_,
                    evolveTokenIdArr_[index_],
                    evolveLevelArr_[index_],
                    randomNumber_
                );
                unchecked{++index_; ++arrayIndex_;}
            }
