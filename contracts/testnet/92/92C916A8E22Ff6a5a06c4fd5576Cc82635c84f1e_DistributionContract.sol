...
contract DistributionContract {
    ...
    struct Holder {
        uint256 balance;
        uint256 lastClaimEpochArbot;
        uint256 lastClaimEpochEth;
    ...
    for (uint256 i = 0; i < token.holdersCount(); i++) {
        ...
        if (isEligible(holder)) {
            ...
            holders[holder].accumulatedEth += holderShare;  // Modifier cette ligne pour ajouter la récompense à l'accumulation existante
        }
    }
}
    ...
    function claimArbot() external {
        require(isEligible(msg.sender), "Not eligible to claim ARBOT");
        ...
        holders[msg.sender].lastClaimEpochArbot = epoch.getCurrentEpoch();
        token.transfer(msg.sender, availableArbot);
    }

    function claimEth() external {
        require(isEligible(msg.sender), "Not eligible to claim ETH");
        ...
        holders[msg.sender].lastClaimEpochEth = epoch.getCurrentEpoch();
        payable(msg.sender).transfer(availableEth);
    }

    function isEligible(address holder) public view returns (bool) {
        uint256 currentEpoch = epoch.getCurrentEpoch();
        uint256 lastClaimEpochArbot = holders[holder].lastClaimEpochArbot;
        uint256 lastClaimEpochEth = holders[holder].lastClaimEpochEth;

        return (currentEpoch >= lastClaimEpochArbot + ELIGIBILITY_EPOCHS) && (currentEpoch >= lastClaimEpochEth + ELIGIBILITY_EPOCHS);
    }
    ...
    function excludeFromRewards(address _account) external {
        require(msg.sender == teamAddress, "Only the team address can exclude addresses from rewards.");
        excludedFromRewards[_account] = true;
    }

    function includeInRewards(address _account) external {
        require(msg.sender == teamAddress, "Only the team address can include addresses in rewards.");
