...
contract Skills {
    ...
    uint256 private _initialMaximumStake;
    uint256 private _initialStakeMultiple;
    ...
    uint256 private _fundsUsedFromSupply;

    event OrderCreated(uint256 indexed id);

    // Need to use a struct because of stack too deep error
    // https://soliditydeveloper.com/stacktoodeep
    struct ConstructorArguments {
        address payable wxpAddress;
        uint256 minimumStake;
        uint256 initialMaximumStake;
        uint256 initialStakeMultiple;
        ...
        uint256 initialTotalSupply; // likely this will get removed one day when scope changes
    }

    constructor(ConstructorArguments memory args) {
        _wxpAddress = args.wxpAddress;
        _minimumStake = args.minimumStake;
        _initialMaximumStake = args.initialMaximumStake;
        _initialStakeMultiple = args.initialStakeMultiple;
        _minimumNumberOfSubmissionsPerOrder = args.minimumNumberOfSubmissionsPerOrder;
        _maximumNumberOfSubmissionsPerOrder = args.maximumNumberOfSubmissionsPerOrder;
        _eventHorizon = args.eventHorizon;
        _minimumOrderDuration = args.minimumOrderDuration;
        _maximumOrderDuration = args.maximumOrderDuration;
        _creatorCutPercentage = args.creatorCutPercentage;
        _firstPlaceCutPercentage = args.firstPlaceCutPercentage;
        _initialTotalSupply = args.initialTotalSupply;
    }
    ...
    function stake(uint endDate, uint256 submissionsAllotted) payable virtual external returns (uint256) {
        ...
        require(msg.value <= getMaximumStake(), "This amount is beyond the maximum stake.");
        ...
        require(endDate - block.timestamp >= _minimumOrderDuration, "The order duration is too short.");
        require(endDate - block.timestamp <= _maximumOrderDuration, "The order duration is too long.");
        ...
        uint256 stakeMultiple = getStakeMultiple();
        uint256 totalReturn = msg.value * stakeMultiple / 100;
        ...
        _fundsUsedFromSupply += fundsNeeded;
        ...
        _orders[currentIndex].stakeMultiple = stakeMultiple;
     ...
     */
    function getStakeMultiple() public view virtual returns (uint256) {
        uint256 accuracy = 1000000;
        uint256 percentageOfSupplyLeft = (_initialTotalSupply - _fundsUsedFromSupply) * accuracy / _initialTotalSupply;
        uint256 stakeMultiple = percentageOfSupplyLeft * _initialStakeMultiple / accuracy;
        return stakeMultiple;
    }
  ...
  * @dev Returns the current minimum stake.
     */
    function getMaximumStake() public view virtual returns (uint256) {
        uint256 accuracy = 1000000;
        uint256 percentageOfSupplyLeft = (_initialTotalSupply - _fundsUsedFromSupply) * accuracy / _initialTotalSupply;
        uint256 maximumStake = percentageOfSupplyLeft * _initialMaximumStake / accuracy;
        return maximumStake;
    }
