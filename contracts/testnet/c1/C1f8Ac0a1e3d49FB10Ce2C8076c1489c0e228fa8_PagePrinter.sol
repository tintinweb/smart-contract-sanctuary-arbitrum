...
import { IONFT721Core } from "@layerzerolabs/solidity-examples/contracts/token/onft/IONFT721Core.sol";
...
error PagePrinter_InsufficientValue();
...
contract PagePrinter is ONFT721Core, ERC721, IPagePrinter {
    ...
    mapping(uint256 => uint256) public pagesPrinted;
    ...
    function mint(uint256 _amount) public payable {
        ...
        if (!mintNft && msg.value != _amount * BASE_PRICE)
            revert PagePrinter_InsufficientValue();

        for (uint256 i; i < _amount;) {
            _safeMint(_msgSender(), printerId);
            unchecked {
                ++printerId;
                ++i;
            }
        ...
        if (mintNft) NFT.mint(_msgSender(), NFT_ID, _amount);
    }
     ...
     * @param _pages Amount of the next pages in the series to print
     */
    function print(uint256 _printerId, uint256 _pages) public {
        if (_msgSender() != ownerOf(_printerId)) revert PagePrinter_SenderNotAllowed();

        uint256 nextPage = pagesPrinted[_printerId];
        pagesPrinted[_printerId] += _pages;

        for (uint256 i; i < _pages;) {
            PAGE.print(_msgSender(), nextPage + i, 1);
            unchecked { ++i; }
        }
    }
    ...
    function _debitFrom(
        ...
        override
    {
        if (!_isApprovedOrOwner(_msgSender(), _tokenId)) revert PagePrinter_SenderNotAllowed();
    ...
    /**
     * @notice Override `ONFT721Core` function to send a token from the current chain to a destination chain
    ...
    function sendFrom(
        ...
        override
    {
        if (!PAGE.mintClosed() && _msgSender() != address(bytes20((_toAddress))))
    ...
    /**
     * @notice Override `ONFT721Core` function to send a batch of tokens from the current chain to a destination chain
