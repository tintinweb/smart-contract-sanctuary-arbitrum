...
contract Questing is Initializable, QuestingTimeKeeper {
    ...
    // Used for AdvancedQuesting since questing XP is stored in the Questing contract.
    function processQPGainAndLevelUp(
        uint256 _tokenId,
        uint8 _currentQuestLevel)
    external
    override
    whenNotPaused
    contractsAreSet
    onlyAdminOrOwner
    {
        _processQPGainAndLevelUp(_tokenId, _currentQuestLevel);
    }
    ...
    returns(uint256[] memory)
    {
        return userToQuestsInProgress[_user].values();
    }
}
...
abstract contract QuestingContracts is Initializable, QuestingState {
    ...
    modifier contractsAreSet() {
        require(areContractsSet(), "Questing: Contracts aren't set");
