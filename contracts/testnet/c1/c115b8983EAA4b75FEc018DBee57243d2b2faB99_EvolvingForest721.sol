...
contract EvolvingForest721 is ERC721, IERC2981, AccessControl, DefaultOperatorFilterer {
  ...
  modifier onlyBurner() {
    require(hasRole(MINTER_ROLE, _msgSender()), "not a burner");
    _;
  }

  constructor(string memory _name, string memory _symbol, string memory _baseUri, string memory _contractURIString, address _contractOwner) 
  ...
  function mintNext(address _to, uint256 _amount) public onlyMinter {
  ...
  function mintUniversal(address _to, uint256 _tokenId, uint256 _amount) public onlyMinter {
    if (_tokenId == 0) {
      mintNext(_to, _amount);
    } else {
      require(_amount == 1, "bad amount");
      for (uint256 i; i < _amount; i++) {
  ...
  function mintBatch(address[] memory _to, uint256[] memory _tokenIds, uint256[] memory _amount) public onlyMinter {
    for (uint256 i; i < _to.length; i++) {
      mintUniversal(_to[i], _tokenIds[i], _amount[i]);
    }
  }
  ...
  function burnUniversal(address _from, uint256 _tokenId, uint256 _amount) external onlyBurner {
    require(ownerOf(_tokenId) == _from, "not owner");
    require(_amount == 1, "bad amount");
    _burn(_tokenId);
  }

  function burnBatch(uint256[] memory _tokenIds) external onlyBurner {
  ...
  function supportsInterface(bytes4 interfaceId) 
    public virtual override(ERC721, IERC165, AccessControl) view returns (bool) {
        return 
