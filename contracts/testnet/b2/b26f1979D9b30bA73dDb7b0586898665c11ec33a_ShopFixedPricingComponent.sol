...
uint256 constant ID = uint256(
    keccak256("game.piratenation.shopfixedpricingcomponent.v1")
);
...
library ShopFixedPricingComponentStorage {
...
/**
 * @title ShopFixedPricingComponent
 * @dev Pricing data for a shop SKU
 */
contract ShopFixedPricingComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Purchase price in USDC (6 decimal token)
        keys[0] = "price";
     ...
     *
     * @param entity Entity to get value for
     * @param price Purchase price in USDC (6 decimal token)
     * @param ethPrice Purchase price in ETH
     */
    function setValue(
        uint256 entity,
        uint256 price,
        ...
        _setValue(entity, Layout(price, ethPrice));
    }
    ...
    function getLayoutValue(
        ...
        value = ShopFixedPricingComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return price Purchase price in USDC (6 decimal token)
     * @return ethPrice Purchase price in ETH
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 price, uint256 ethPrice) {
        if (has(entity)) {
            Layout memory s = ShopFixedPricingComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (price, ethPrice) = abi.decode(
                _getEncodedValues(s),
                (uint256, uint256)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ShopFixedPricingComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.price);
        values[1] = abi.encode(s.ethPrice);
    }
    ...
    function setBytes(
        ...
        Layout memory s = ShopFixedPricingComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.price, s.ethPrice) = abi.decode(value, (uint256, uint256));
    ...
    function remove(
        ...
        delete ShopFixedPricingComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.price, value.ethPrice));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.price, value.ethPrice);
    }
}