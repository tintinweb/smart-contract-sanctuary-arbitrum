...
import "./LostLandContracts.sol";

contract LostLand is Initializable, LostLandContracts {
	using EnumerableSetUpgradeable for EnumerableSetUpgradeable.UintSet;

    function initialize() external initializer {
    	LostLandContracts.__LostLandContracts_init();
    }
    	...
    	nonZeroLength(_tokenIds)
    {
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            // To
            exploration.startExploring(_tokenId);
    ...
    function transferDonkeysToLostLand(uint256[] calldata _tokenIds)
    	external 
    	whenNotPaused
    	contractsAreSet
    	onlyEOA
    	nonZeroLength(_tokenIds)
    {
        ...
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            _requireValidDonkeyAndLocation(_tokenId, Location.LOST_LAND);

            // From
            _transferFromLocation(_tokenId);

            // To
            // Nothing needed for LostLand

            // Finalize
            tokenIdToInfo[_tokenId].location = Location.LOST_LAND;
        }

        emit DonkeyLocationChanged(_tokenIds, msg.sender, Location.LOST_LAND);
    }

    function transferDonkeysOutOfLostLand(uint256[] calldata _tokenIds)
    	external
	    whenNotPaused
	    contractsAreSet
	    onlyEOA
	...
	function _transferFromLocation(uint256 _tokenId) private {
        ...
        if(_oldLocation == Location.LOST_LAND) {
            // Nothing to do here.
        } else if(_oldLocation == Location.EXPLORATION) {

            exploration.stopExploring(_tokenId);
            ...
            revert("LostLand: Unknown from location");
        }
	}
	...
	function _requireValidDonkeyAndLocation(uint256 _tokenId, Location _newLocation) private view {
	    ...
	    if(_oldLocation != Location.NOT_STAKED) {
            require(ownerToStakedTokens[msg.sender].contains(_tokenId), "LostLand: No donkeys owned");
        }

        require(_oldLocation != _newLocation, "LostLand: Location must be different");
	}
    ...
    function ownerForStakedDonkey(uint256 _tokenId) public view override returns(address) {
        ...
        require(_owner != address(0), "LostLand: Donkey is not staked");
        return _owner;
    }
    ...
    function infoForDonkey(uint256 _tokenId) external view returns(TokenInfo memory) {
        require(isDonkeyStaked(_tokenId), "LostLand: Donkey is not staked");
...
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "./LostLandState.sol";

abstract contract LostLandContracts is Initializable, LostLandState {

	function __LostLandContracts_init() internal initializer {
		LostLandState.__LostLandState_init();
	}

	function setContracts(
		address _tldAddress,
		address _explorationAddress)
	external onlyAdminOrOwner 
	{
		tld = ITLD(_tldAddress);
		exploration = IExploration(_explorationAddress);
	}

    modifier contractsAreSet() {
        require(areContractsSet(), "LostLand: Contracts aren't set");
        _;
    }
...
abstract contract LostLandState is Initializable, ILostLand, ERC721HolderUpgradeable, AdminableUpgradeable {
	...
	function __LostLandState_init() internal initializer {
