...
contract RFQExtractor is BaseExtractor, IExtractor, IPlug {
    ...
    constructor(
        address _SocketProtocol,
        address _socket
    ) BaseExtractor(_socket) {
...
contract SocketProtocol {
        ...
        for (uint i = 0; i < submitOrder.subOrders.length; i++) {
            ...
            bytes32 subOrderHash = extractors[
                submitOrder.subOrders[i].extractor
                    ...
                    submitOrder.order.info,
                    submitOrder.subOrders[i].extractorPayload
                );
    ...
    function fullFillBridgeOrder(SetttleSubOrder calldata settleOrder) payable public {
        extractors[settleOrder.suborder.extractor].fullFill{value: msg.value}(
