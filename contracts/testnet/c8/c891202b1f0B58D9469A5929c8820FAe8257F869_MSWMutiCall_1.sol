...
interface INftMinning {
    ...
    function kunIdList(address) external view returns (uint[] memory);
...
import {INftMinning} from "contracts/interface/INftMinning.sol";
...
contract MSWMutiCall_1 is OwnableUpgradeable {
    ...
    function frontEndUnions(
        address user
    ) external view returns (uint _unions, string memory _url) {
        _unions = minning.unionOwner(user);
        if (_unions != 0) {
            _url = mswUnions.tokenURI(_unions);
        }
    ...
    function frontEndDeposited(
        ...
        external
        view
        returns (uint[] memory ids, uint[] memory power, uint[] memory time)
    {
        ids = minning.kunIdList(user);
    ...
    function frontEndKun(
        ...
        uint len = kun.tokensOfOwner(user).length;
