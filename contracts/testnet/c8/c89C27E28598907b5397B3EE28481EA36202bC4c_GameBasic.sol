...
library GameBasic {
    ...
    event ActionMove(uint indexed round, address indexed player, uint eventIndex, uint startPos, uint[] path);
    event ActionShoot(uint indexed round, address indexed player, uint eventIndex, uint fromPos, uint targetPos, bool hitSucceed);
    event ActionPick(uint indexed round, address indexed player, uint eventIndex);
    event ActionDrop(uint indexed round, address indexed player, uint eventIndex);
    event ActionBomb(uint indexed round, address indexed player, uint eventIndex, uint fromPos, uint targetPos, uint explosionRange);
    event ActionEat(uint indexed round, address indexed player, uint eventIndex, uint heal);

    event ActionDefend(uint indexed round, address indexed player, uint eventIndex, uint defense);
    event ActionDodge(uint indexed round, address indexed player, uint eventIndex, bool succeed);

    event PlayerHurt(uint indexed round, address indexed player, uint eventIndex, uint damage);
    event PlayerKilled(uint indexed round, address indexed player, uint eventIndex);
    event PlayerWin(uint indexed round, address indexed player, uint eventIndex, uint tokenReward, uint nftReward);

    event MapChanged(uint indexed round, uint eventIndex);

    event GameStart(uint indexed round, uint eventIndex);
    event GameEnd(uint indexed round, uint eventIndex);
    ...
    function startGame(uint mapSeed, IBattleRoyaleGameV1.GameState storage state, uint mapSize, address player) private returns(uint) {
        ...
        state.shrinkTick = 1;
        emit GameStart(state.round, state.eventCount++);
        return mapSeed;
    }
    ...
    function endGame(
        ...
        state.status = GameConstants.GAME_STATUS_REGISTER;
        if (leftPlayerCount == 1) {
            ...
            emit PlayerWin(state.round, winerAddr, state.eventCount++, config.winnerRewardToken + config.winnerBonusRewardToken, config.winnerRewardNFT + config.winnerBonusRewardNFT);
        }
        //burn left tokens
        ...
        emit GameEnd(state.round, state.eventCount++);
        state.eventCount = 0;
        state.round += 1;
    ...
    function destroyTile(mapping(uint => uint) storage tilemap, mapping(uint => uint) storage tokensOnGround, IBattleRoyaleGameV1.GameState storage state, uint mapSize, uint pos, uint mapSeed, uint round, uint tick, IBattleRoyaleNFT nft) internal {
        ...
        if (t == IBattleRoyaleGameV1.TileType.Wall || t == IBattleRoyaleGameV1.TileType.ChestBox) {
            ...
            emit MapChanged(round, state.eventCount++);
    ...
    function applyRing(mapping(address => uint) storage playersData, IBattleRoyaleGameV1.GameState storage state, RingParams memory p) private returns(bool dodge) {
        dodge = false;
        ...
        for (uint i = tokenIds.length - 1; i > 0; i--) {
            ...
            if (Property.decodeType(token) == Property.NFT_TYPE_RING) {
                ...
                if(dodgeCount > 0) {
                    ...
                    emit ActionDodge(p.round, p.player, state.eventCount++, dodge);
                    dodgeCount -= 1;
    ...
    function applyArmor(mapping(address => uint) storage playersData, IBattleRoyaleGameV1.GameState storage state, ArmorParams memory p) private returns(uint) {
        uint playerData = playersData[p.player];
        ...
        for (uint i = tokenIds.length - 1; i > 0; i--) {
            ...
            if (Property.decodeType(token) == Property.NFT_TYPE_ARMOR) {
                ...
                emit ActionDefend(p.round, p.player, state.eventCount++, defense - leftDefense);
            }
            if (p.damage == 0) {
                break;
            }
    ...
    function applyDamage(
        ...
        mapping(uint => uint) storage tilemap, 
        IBattleRoyaleGameV1.GameState storage state, 
        ApplyDamageParams memory p
    ) public {
        if (p.damage == 0) return;

        uint characterId = PlayerDataHelper.firstTokenId(playersData[p.player]);
        uint tick = GameView.gameTick(state.startTime, p.config.tickTime);
        if (p.canDodge && applyRing(playersData, state, RingParams({
            round: state.round,
            player: p.player,
            mapSeed: p.mapSeed,
            tick: tick,
        ...
        p.damage = applyArmor(playersData, state, ArmorParams({
            round: state.round,
            player: p.player,
            damage: p.damage,
            nft: p.nft
        ...
        emit PlayerHurt(state.round, p.player, state.eventCount++, p.damage);
        if (hp == 0) {
            killPlayer(players, playersData, tokensOnGround, tilemap, state,
                KillParams({
                    nft: p.nft,
                    mapSeed: p.mapSeed,
                    round: state.round,
    ...
    function killPlayer(
        ...
        for (uint i = 1; i < tokenIds.length; i++) {
            data = TokenOnGroundHelper.addToken(data, p.round, tokenIds[i]);
        }
        tokensOnGround[pos] = data;
        _removePlayer(players, playersData, p.player);
        emit PlayerKilled(p.round, p.player, state.eventCount++);
    }
    ...
    function newChestBox(mapping(uint => uint) storage tilemap, IBattleRoyaleGameV1.GameState storage state, IBattleRoyaleGameV1.GameConfig memory config, uint mapSeed, address player) external {
        (uint left, uint top, uint right, uint bottom) = GameView.shrinkRect(state, config);
        uint mapSize = config.mapSize;
        if (left + right < mapSize && top + bottom < mapSize) {
            for (uint i = 0; i < 3; i++) {
                ...
                if (GameView.getTileType(tilemap, x, y, mapSize) == IBattleRoyaleGameV1.TileType.None) {
                    ...
                    emit MapChanged(state.round, state.eventCount++);
                    break;
                }
            }
        }
...
interface IBattleRoyaleGameV1 {
    ...
    function mapSeed() external view returns (uint);
