...
contract LyRwax is Initializable, OwnableUpgradeable, IERC20 {
    ...
    string public constant name = "Rwax Loyalty Token";
    string public constant symbol = "lyRWAX";
    ...
    function upgrade_useRwaxStake(address _rwaxStake, bool _enableStaking) external reinitializer(3) {
        require(_rwaxStake != address(0), "Invalid staking contract");
    ...
    function mint(address _to, uint256 _amount) external {
        require(_msgSender() == minter, "LyRwax: !minter");
        _mint(_to, _amount);
    }
    ...
    function claimRewards(uint256 _batchId, address _receiver) external {
        ...
        require(amount > 0, "LyRwax: nothing to claim");
        _rewards[_batchId][sender] += amount;
        if (enableStaking) {
            rewardToken.safeIncreaseAllowance(address(rwaxStake), amount);
            rwaxStake.stake(_receiver, amount);
        } else {
    ...
    function setMinter(address _minter) external onlyOwner {
        require(_minter != address(0), "LyRwax: zero address");
        minter = _minter;
