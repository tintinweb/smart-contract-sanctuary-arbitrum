...
interface ISggcNFT {
    ...
    event DeliveredCollection(uint256 indexed tokenId, string bill_no);
    event BatchMetadataUpdate(uint256 _fromTokenId, uint256 _toTokenId);
    event ContractURIUpdated(string newContractURI);
    event MaxSupplyUpdated(uint256 newMaxSupply);
    event ProvenanceHashUpdated(bytes32 previousHash, bytes32 newHash);

    function deliverCollection(uint256 _tokenId, string memory _bill_no) external;

    function mintSggc(address _to, uint256 _tokenId) external;
...
contract SggcNFT is ISggcNFT, ERC721Enumerable, Ownable {
    ...
    function mintSggc(address _to, uint256 _tokenId) external override onlyOwner {
    ...
    /// @param _bill_no 提货单号
    function deliverCollection(uint256 _tokenId, string memory _bill_no) external override {
        require(_tokenId < maxSupply(), "SggcNFT: tokenId must be less than max supply");
        require(_ownerOf(_tokenId) == msg.sender, 'SggcNFT: You dont own the tokenId');
        ...
        emit DeliveredCollection(_tokenId, _bill_no);
    }
