  ...
  function dispenseRewards(
    ...
    for (uint256 i = 0; i < dist.itemChances.length * 2; i++) {
      ...
      if (roll < dist.itemChances[tokenIndex]) {
        ...
        IERC721A collection = IERC721A(dist.externalCollections[i]);

        // Check if the token is still in the vault, if not - either stop here or continue to the next reward
        if (collection.ownerOf(tokenId) != vaultAddress) {
          if (
            dist.fallbackType == DepletedFallbackType.DISPENSE_NEXT_AVAILABLE
          ) {
            continue;
        ...
        collection.safeTransferFrom(vaultAddress, _receiver, tokenId, "");
