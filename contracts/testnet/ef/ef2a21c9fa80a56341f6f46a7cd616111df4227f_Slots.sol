...
contract Slots is Common {
    ...
    constructor(
        ...
        address _linkFeed,
        uint8[343] memory _multipliers,
        uint16[343] memory _outcomeNum
    ) {
        Bankroll = IBankRoll(_bankroll);
        IChainLinkVRF = IVRFCoordinatorV2(_vrfCoordinator);
        LINK_ETH_FEED = AggregatorV3Interface(_linkFeed);
        ChainLinkVRF = _vrfCoordinator;
        _setSlotsMultipliers(_multipliers, _outcomeNum, 343);
    }
    ...
    function Slots_Play(uint256 wager, address tokenAddress, uint32 numBets) external payable nonReentrant {
        address msgSender = _msgSender();

        if (slotsGames[msgSender].requestID != 0) {
            revert('Slot : waiting for VRF');
        }
        if (!(numBets > 0 && numBets <= 100)) {
            revert('Slot : Invalid number of bets, must be between 1 and 100');
        }

        uint256 fee = _transferWager(tokenAddress, wager * numBets, 800000, 24, msgSender);
        ...
        emit Slots_Play_Event(msgSender, wager, tokenAddress, numBets, fee);
    }
    ...
    function Slots_Refund() external nonReentrant {
        ...
        if (game.requestID == 0) {
            revert('Slot : not waiting for VRF');
        }
        if (game.blockNumber + 200 > block.number) {
            revert('Slot : block number too low to claim refund');
        }
        ...
        if (tokenAddress == address(0)) {
            revert('Slot : invalid token address');
        } else {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal {
        ...
        for (i = 0; i < game.numBets; i++) {
            ...
            if (multipliers[i] != 0) {
                if (game.isveFlush) {
                    payout += (game.wager * multipliers[i]) - game.wager;
                    payouts[i] = (game.wager * multipliers[i]) - game.wager;
                } else {
