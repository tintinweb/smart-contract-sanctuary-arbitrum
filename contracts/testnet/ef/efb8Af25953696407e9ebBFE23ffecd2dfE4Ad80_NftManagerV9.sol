...
/// @author munji - yimunj93[email protected]
...
contract NftManagerV9 is PermissionsEnumerable {
    /** 
    ...
    function registerItem(
        ...
        require(
            itemEffects[_itemName].exp == 0 &&
                ...
                itemEffects[_itemName].dividendsPoint == 0 &&
    ...
    function purchaseItem(
        ...
        // pachiToken.burn(itemEffects[_itemName].price);

        nftStatus[tokenId].items.push(_itemName);
        ...
        if (itemEffects[_itemName].dividendsPoint > 0) {
            nftStatus[tokenId].dividendsPoint += itemEffects[_itemName]
                .dividendsPoint;
        }
    ...
    function _updateExpAndLevel(
        ...
        if (nftStatus[tokenId].exp >= 100) {
            ...
            nftStatus[tokenId].dividendsPoint += 1;
        }
    }
    ...
    function setAsSupporter(uint256 tokenId) external nftExists(tokenId) {
        ...
        // both tokenId should be in a transferable state
        ...
        addressToSupporter[msg.sender] = tokenId;
    }

    function feedSnack(uint256 tokenId) external nftExists(tokenId) {
        require(pachi721.ownerOf(tokenId) == msg.sender, "not owner");
        require(nftStatus[tokenId].lovePoint < 100, "already full");

        // should instead use burn
        bool sent = pachiToken.transferFrom(
            msg.sender,
            address(this),
            100 * 10 ** 18
    ...
    function initiateNFTStatus(
        ...
        nftStatus[tokenId].jackpotBoost = jackpotBoost;
        nftStatus[tokenId].dividendsPoint = dividendsPoint;
    ...
    function updatePendingStatus(
        ...
        // emit PendingStatusUpdated(tokenId, isPending, block.timestamp);
    }
    ...
    function writeMatchResult(
        ...
        // emit MatchResultWritten(tokenId, matchResult, block.timestamp);
    }
    ...
    function getOwnerOf(uint256 tokenId) external view returns (address) {
        return pachi721.ownerOf(tokenId);
    }

    function getNFTStatus(
        uint256 tokenId
    ) external view returns (NFTStatus memory) {
        NFTStatus memory status = nftStatus[tokenId];
        return (status);
    }

    function getSupporterTokenId(address user) external view returns (uint256) {
        return addressToSupporter[user];
    }

    function getAvailableSkins(
        uint256 tokenId
    ) external view returns (string[] memory) {
        return nftStatus[tokenId].availableSkins;
    }

    function checkIfSkinIsAvailable(
        uint256 tokenId,
        address user,
        string memory skin
    ) external view returns (bool) {
        if (nftStatus[tokenId].availableSkins.length == 0) return false;
        if (pachi721.ownerOf(tokenId) != user) return false;

        for (uint256 i = 0; i < nftStatus[tokenId].availableSkins.length; i++) {
            if (
                keccak256(
                    abi.encodePacked(nftStatus[tokenId].availableSkins[i])
    ...
    function getItems(uint256 tokenId) external view returns (string[] memory) {
        return nftStatus[tokenId].items;
    }
    ...
    function initiateMating(
        ...
        require(
            pachi721.ownerOf(tokenIds[0]) == msg.sender &&
                pachi721.ownerOf(tokenIds[1]) == msg.sender,
            "not owner"
        );
        require(
            this.isNftPlayable(tokenIds[0]) && this.isNftPlayable(tokenIds[1]),
            "already mating or in a pending status"
        );
        require(
            IERC20(token).allowance(msg.sender, address(this)) >= birthPrice,
            "not enough allowance"
        );
        require(
            IERC20(token).balanceOf(msg.sender) >= birthPrice,
            "not enough balance"
        );
        ...
        require(sent, "Failed to send Token");
        // IERC20(token).burn(birthPrice);

        nftStatus[tokenIds[0]].isMating = true;
        nftStatus[tokenIds[1]].isMating = true;
        ...
        addressToMatingIds[msg.sender].push(matingId);

        emit MatingStatusUpdated(
            matingId,
            matingStatus[matingId],
            block.timestamp
        );
    }

    function finalizeMating(uint256 _matingId) external onlyRole(NFT_ROLE) {
        ...
        deleteMatingIdInAddress(_matingId);

        emit MatingStatusUpdated(
            _matingId,
            matingStatus[_matingId],
            block.timestamp
        );
    }

    function checkIfMatingDone(uint256 _matingId) external view returns (bool) {
        return matingStatus[_matingId].birthed;
    }

    function getIsFinalizable(uint256 _matingId) external view returns (bool) {
        return
            matingStatus[_matingId].endTime >= block.timestamp &&
            matingStatus[_matingId].birthed == false;
    }

    function getMatingIds(
        address user
    ) external view returns (uint256[] memory) {
        return addressToMatingIds[user];
    }

    function getMatingStatus(
        uint256 _matingId
    ) external view returns (MatingStatus memory) {
        return matingStatus[_matingId];
    }

    function deleteMatingIdInAddress(uint256 _matingId) internal {
        uint256[] storage matingIds = addressToMatingIds[msg.sender];
        for (uint256 i = 0; i < matingIds.length; i++) {
            if (matingIds[i] == _matingId) {
                matingIds[i] = matingIds[matingIds.length - 1];
                matingIds.pop();
                break;
            }
        }
