...
contract AutomationDemo2 is ILogAutomation, StreamsLookupCompatibleInterface {
    struct BasicReport {
        // v0.3 Basic
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        if(log.source == sourceAddress){
             bytes memory p1 = abi.encodePacked(log.topics[1]);
             uint256 id = abi.decode(p1, (uint256));
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        uint256 tradePrice;
        int192  indexPrice = verifiedReport.benchmark;
        if(indexPrice > 0){
            tradePrice = uint256(int256(indexPrice));
        }

        IOrder(sourceAddress).executeOrder(id, tradePrice);
    }
