    ...
    TokensRescuer
{
    ...
    struct Strategy {
        ...
        uint256 totalStaked;
        uint256 totalShares;
        uint256 lastCompoundBlockNumber;
        uint256 lock;
        uint256 cap;
    ...
    modifier onlyAfterLock(
        ...
        require(
            block.number -
                ...
                strategies[strategyId].lock,
            "Parallax: withdrawal lock is not finished"
        );
        _;
    }
    ...
    function addToken(
        uint256 strategyId,
        address token
    ) external onlyOwner onlyExistingStrategy(strategyId) onlyContract(token) {
    ...
    function setLock(
        ...
        strategies[strategyId].lock = lock;
    }
    ...
    function setRewardToken(
        ...
        onlyOwner
        onlyExistingStrategy(strategyId)
        onlyContract(address(rewardToken))
    {
        strategies[strategyId].rewardToken = rewardToken;
    }
    ...
    function getClaimableRewards(
        ...
        return claimableRewards / ACCURACY;
    }
    ...
    function getUsers(
        ...
        require(cursor <= usersCount, "Parallax: cursor out of bounds");
    ...
    function getUsersByStrategy(
        ...
        require(
            cursor <= strategy.usersCount,
            "Parallax: cursor out of bounds"
        );
    ...
    function _deposit(
        ...
        if (positionId == 0) {
            ...
            if (positionId == 1) {
                ...
                ++strategy.usersCount;
                ++userAmountStrategies[_msgSender()];

                strategy.users[strategy.usersCount] = _msgSender();
                strategy.usersToId[_msgSender()] = strategy.usersCount;

                if (userAmountStrategies[_msgSender()] == 1) {
                    ++usersCount;

                    users[usersCount] = _msgSender();
                    usersToId[_msgSender()] = usersCount;
                }
            }
        ...
        position.former =
            _getStakedBySharesAmount(strategyId, position.shares) *
            rewardPerShare;
        position.lastStaked = block.number;
    ...
    function _withdraw(
        ...
        if (position.shares == 0) {
            ...
            if (positionsCount[strategyId][_msgSender()] == 0) {
                Strategy storage strategy = strategies[strategyId];
                uint256 userId = strategy.usersToId[_msgSender()];
                ...
                delete strategy.usersToId[_msgSender()];

                --strategies[strategyId].usersCount;
                --userAmountStrategies[_msgSender()];

                if (userAmountStrategies[_msgSender()] == 0) {
                    uint256 globalUserId = usersToId[_msgSender()];
    ...
    function _compound(
        ...
        strategies[strategyId].lastCompoundBlockNumber = block.number;
    ...
    function _updateStrategyRewards(uint256 strategyId) private {
        strategies[strategyId].rewardPerShare = _getUpdatedRewardPerShare(
            strategyId
        );
        strategies[strategyId].lastUpdated = block.number;
    }

    function _getUpdatedRewardPerShare(
        uint256 strategyId
    ) private view returns (uint256) {
        uint256 _rewardPerShare = strategies[strategyId].rewardPerShare;
        uint256 _totalStaked = strategies[strategyId].totalStaked;

        if (_totalStaked != 0) {
            uint256 _blockDelta = block.number -
                strategies[strategyId].lastUpdated;
            uint256 _reward = _blockDelta *
                strategies[strategyId].rewardPerBlock;

            _rewardPerShare += (_reward * ACCURACY) / _totalStaked;
        }
