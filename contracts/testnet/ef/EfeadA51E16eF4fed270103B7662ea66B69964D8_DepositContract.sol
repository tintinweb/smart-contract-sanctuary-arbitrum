...
contract DepositContract is FarmCoinAccessControlled {
    ...
    function withdrawETH(address account, uint256 index) external {
        ...
        if (
            ...
            keccak256(bytes(deposit.currency)) == keccak256(bytes("ETH"))
        ) {
            uint256 refundRate = 100;
            uint256 refundAmount = deposit.depositAmount.mul(refundRate).div(
                100
            );

            if (block.timestamp < deposit.expire) {
    ...
    function withdrawUSDC(address account, uint256 index) external {
        ...
        require(
            keccak256(bytes(deposit.currency)) == keccak256(bytes("USDC")),
            "Withdraw: Invalid currency"
        );

        if (deposit.depositAmount > 0 && !deposit.isWithdrawn) {
            uint256 refundRate = 100;
            uint256 refundAmount = deposit.depositAmount.mul(refundRate).div(
                100
            );
    ...
    function claimAndDeleteAllRewardsETH(
        ...
        uint256 availableReward = calculateRewards(deposit);
    ...
    function getDepositData() public view returns (Deposit[] memory) {
