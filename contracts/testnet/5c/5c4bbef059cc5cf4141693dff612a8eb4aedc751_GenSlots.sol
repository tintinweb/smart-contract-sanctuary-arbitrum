...
interface IRandomizer {
    ...
    function clientBalanceOf(address _contract)
        external view
...
contract GenSlots {
    ...
    uint256 callbackGasLimit = 50000; //100k for mainnet
    uint256 callbackRequestConfirmations = 2;
    ...
    */
    ...
    constructor(address _token, address payable _teamAddress) {
        owner = msg.sender;
        tokenAddress = _token;
        potFee = 5000;
        teamFee = 5000;
    ...
    function tokenSpin() public {
        ...
        if (freeSpin[msg.sender]) {
            ...
            for (uint256 i; i < stakingAddresses.length; i++) {
                IERC20(tokenAddress).transfer(
                    stakingAddresses[i],
                    (tokenSpinPrice * stakingFees[i]) / 10000
                );
            }
    ...
    function ethSpin() public payable {
        ...
        for (uint256 i; i < stakingAddresses.length; i++) {
            IERC20(tokenAddress).transfer(
                stakingAddresses[i],
                (swappedTokens * stakingFees[i]) / (10000 - potFee)
            );
        }
    ...
    function spin(
        ...
        uint256 id = randomizer.request(
            callbackGasLimit,
            callbackRequestConfirmations
        );
    ...
    function randomizerCallback(uint256 _id, bytes32 _value) external {
        require(idToRoll[_id].finished == false, "Request ID not known");
        ...
        idToRoll[_id].symbols[0] = symbol;
        if (
            (uint256(keccak256(abi.encode(randomNumber, 1))) % 10000) + 1 <=
            sameSymbolOdds
        ) {
            idToRoll[_id].symbols[1] = symbol;
        } else {
            idToRoll[_id].symbols[1] = wheel[
                uint256(keccak256(abi.encode(randomNumber, 2))) % wheel.length
            ];
        }

        if (
            (uint256(keccak256(abi.encode(randomNumber, 3))) % 10000) + 1 <=
            sameSymbolOdds
        ) {
            idToRoll[_id].symbols[2] = symbol;
        } else {
            idToRoll[_id].symbols[2] = wheel[
    ...
    function randomizerDeposit() external payable {
        randomizer.clientDeposit{value: msg.value}(address(this));
    }

    function randomizerBalance()
        public view
        returns (uint256 deposit, uint256 reserved)
    {
    ...
    */
    ...
    function setCallbackRequestConfirmations(
        uint256 _callbackRequestConfirmations
    ) public onlyOwner {
        callbackRequestConfirmations = _callbackRequestConfirmations;
    }
    ...
    function setAllFees(
        ...
        require(
            _potFee + _teamFee + stakingFeesSum == 10000,
