...
interface ICreatorHeroesStorage {
    function createBattleCharacterDataURI(uint256 characterData, bytes32 personalityData, uint256 status)
        external
        view
        returns (string memory);

    function createSpecialBattleCharacterDataURI(
...
interface ISpecialCharacterContract {
    function getStructureData(uint256 tokenId) external view returns (uint256);
}

contract CreatorHeroes is Ownable, ERC721A, ImageDataHandler, PersonalityHandler {
    ...
    address public specialCharacterContractAddress;

    mapping(uint256 => uint256) public icon;

    constructor() ERC721A("CreatorHeroes", "CH") {}
        ...
        payable
    {
        ...
        _checkCategolyId(imageIds, 1);
        _setStructureData(tokenId, imageIds, 0);

        // imageIds[1] != 0 or imageIds[2] != 0
    ...
    function setSpecialCharacterContractAddress(address newSpecialCharacterContractAddress) public onlyOwner {
        specialCharacterContractAddress = newSpecialCharacterContractAddress;
    }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721A) returns (string memory) {
        ...
        uint256 check = (characterData >> 32) & _MASK_UINT8;

        if (check == 0) {
            return ICreatorHeroesStorage(storageContractAddress).createBattleCharacterDataURI(
                characterData, personalityData, status
            );
        } else {
            (address pointer, uint256 categoryId, uint256 specialCharacterId, uint256 styleId) =
                ICreatorHeroesStorage(storageContractAddress).getImageData(check);

            uint256 specialCharacterData =
                ISpecialCharacterContract(specialCharacterContractAddress).getStructureData(specialCharacterId);

            return ICreatorHeroesStorage(storageContractAddress).createSpecialBattleCharacterDataURI(
                characterData, personalityData, status, pointer, specialCharacterData
            );
        }
    }
