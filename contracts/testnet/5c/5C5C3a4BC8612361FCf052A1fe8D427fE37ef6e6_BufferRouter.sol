...
contract BufferRouter is AccessControl, IBufferRouter {
    ...
    function getSafeStrike(
        ...
        uint256 minSafeStrikePercent = config.minSafeStrikePercent();
        uint256 delta = (minSafeStrikePercent * 1e2) +
            ((optionsContract.totalMarketOI() *
                (config.maxSafeStrikePercent() - minSafeStrikePercent) *
                1e2) / optionsContract.getMaxOI());
        return
            isAbove
                ? (strike * (1e4 + delta)) / 1e4
                : (strike * (1e4 - delta)) / 1e4;
    }
    ...
    function _openTrade(
        ...
        optionParams.strike = getSafeStrike(
            params.strike,
            params.isAbove,
            optionsContract,
            config
