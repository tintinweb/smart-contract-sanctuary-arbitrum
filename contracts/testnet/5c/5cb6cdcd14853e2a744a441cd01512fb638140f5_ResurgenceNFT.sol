...
contract ResurgenceNFT is ERC1155, Ownable {
    ...
    constructor(address _owner) ERC1155("https://yourtoken.com/api/token/{id}.json") Ownable(_owner) {
        phase = MintingPhase.PAUSED;

    }

    function setPhase(MintingPhase _phase) public onlyOwner {
        phase = _phase;
    }
    ...
    function mint(uint256 id) public {
        require(allowlist[_msgSender()], "Not allowed to mint");
        require(id < 6, "Invalid token id");
        require(uint256(phase) == id, "Not the correct phase");
        _mint(_msgSender(), id, 1, "");
    }

    function batchBurnAndMintNew() public {
        require(phase == MintingPhase.PHASE_7, "Not in phase 7");
        _burnBatch(_msgSender(), tokenIds, amounts);
        _mint(_msgSender(), 7, 1, "");
    }
