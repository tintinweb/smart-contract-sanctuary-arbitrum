...
contract ZkBridgeOracle is ILayerZeroOracleV2, Initializable, OwnableUpgradeable {
    ...
    function initialize(address _layerZeroEndpoint, address _zkMptValidator) public initializer {
    ...
    function batchUpdateMptHash(uint16[] calldata _sourceChainIds, bytes32[] calldata _blockHashes, bytes32[] calldata __receiptHashes, address[] calldata _userApplications) external {
        require(_sourceChainIds.length == _blockHashes.length, "ZkBridgeOracle:Parameter lengths must be the same");
        require(_sourceChainIds.length == __receiptHashes.length, "ZkBridgeOracle:Parameter lengths must be the same");
        ...
        for (uint256 i = 0; i < _sourceChainIds.length; i++) {
            _updateHash(_sourceChainIds[i], _blockHashes[i], __receiptHashes[i], _blockHashes[i], __receiptHashes[i], _userApplications[i]);
        }
    }
