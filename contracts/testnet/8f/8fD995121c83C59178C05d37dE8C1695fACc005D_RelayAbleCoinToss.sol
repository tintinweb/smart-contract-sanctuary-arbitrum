...
import {RelayAbleGame} from "./Game.sol";
...
contract RelayAbleCoinToss is RelayAbleGame {
...
abstract contract RelayAbleGame is
    Pausable,
    VRFConsumerBaseV2,
    ReentrancyGuard,
    ERC2771Context,
    Ownable
{
    ...
    function _newBet(
        ...
        address unwrappedToken = isWrappedGasToken ? address(0) : tokenAddress;

        Token storage token = tokens[unwrappedToken];
        ...
        uint64 VRFSubId;
        {
            ...
            ) = bank.getBetRequirements(unwrappedToken, multiplier);
        ...
        uint256 chainlinkVRFFee = getChainlinkVRFCost(unwrappedToken);
        ...
        _userBets[_msgSender()].push(id);
        bets[id] = newBet;

        token.pendingCount++;

        // Transfer the tokens
        IERC20(isWrappedGasToken ? address(wrapped) : tokenAddress)
            .safeTransferFrom(
                _msgSender(),
                address(this),
    ...
    function _accountVRFCost(Bet storage bet, uint256 startGas) internal {
        ...
        tokens[address(wrapped) == bet.token ? address(0) : bet.token]
            .VRFFees += actualVRFCost;
    }
    ...
    function _resolveBet(
        ...
        bool isWrappedGasToken = address(wrapped) == token;
        address unwrappedToken = isWrappedGasToken ? address(0) : token;

        tokens[unwrappedToken].pendingCount--;
        ...
        if (payout > betAmount) {
            ...
            uint256 betAmountFee = _getFees(unwrappedToken, betAmount);
            uint256 profitFee = _getFees(unwrappedToken, profit);
            ...
            if (isWrappedGasToken) {
                // Unwrap the bet amount to be send to the bank.
                wrapped.withdraw(betAmountFee);
            } else {
            ...
            bank.payout{value: isWrappedGasToken ? betAmountFee : 0}(
                isWrappedGasToken ? address(this) : user,
                unwrappedToken,
                profitPayout,
                fee
            );
            ...
            if (isWrappedGasToken) {
                wrapped.deposit{value: profitPayout}();
                wrapped.transfer(user, betAmountPayout + profitPayout);
            } else {
            ...
            uint256 fee = _getFees(unwrappedToken, payout);
            payout -= fee;
            ...
            if (isWrappedGasToken) {
                // Unwrap the cash in amount to be send to the bank.
                wrapped.withdraw(bankCashIn);
            } else {
                // Transfer the lost bet amount and fee to the bank
                IERC20(token).safeTransfer(address(bank), bankCashIn);
            }

            bank.cashIn{value: isWrappedGasToken ? bankCashIn : 0}(
                unwrappedToken,
                bankCashIn,
                fee
            );
            ...
            if (isWrappedGasToken) {
                // Unwrap the lost bet amount to be send to the bank.
                wrapped.withdraw(betAmount);
            } else {
                IERC20(token).safeTransfer(address(bank), betAmount);
            }
            bank.cashIn{value: isWrappedGasToken ? betAmount : 0}(
                unwrappedToken,
                betAmount,
                0
            );
    ...
    function withdrawTokensVRFFees(address token) external {
        ...
        if (tokenChainlinkFees != 0) {
            ...
            wrapped.transfer(bank.getTokenOwner(token), tokenChainlinkFees);
    ...
    function withdrawOverchargedVRFCost(address user) external {
        ...
        wrapped.transfer(user, overchargedVRFCost);
    ...
    function refundBet(uint256 id) external {
        ...
        tokens[address(wrapped) == tokenAddress ? address(0) : tokenAddress]
            .pendingCount--;

        bet.resolved = true;
        bet.payout = bet.amount;

        IERC20(tokenAddress).safeTransfer(bet.user, bet.amount);

        uint256 chainlinkVRFCost = bet.vrfCost;
        wrapped.transfer(bet.user, chainlinkVRFCost);
