...
contract Vault is Constants, ReentrancyGuard, Ownable, IVault {
    ...
    function takeAssetOut(
        ...
        emit TakeAssetOut(_account, _refer, _usdOut, _fee, _usdOut, _token, _tokenPrice);
    }
    ...
    function _decreaseBond(bytes32 _key, address _account, uint256 _txType) internal {
        require(bonds[_key][_txType].owner == _account, "Invalid bond owner");

        if (bonds[_key][_txType].amount > 0) {
            bonds[_key][_txType].amount = 0;
            bonds[_key][_txType].token = address(0);
        }
    }
    ...
    function _takeAssetOut(
        ...
        uint256 tokenAmountOut = priceManager.fromUSDToToken(_token, usdOutAfterFee, _tokenPrice);
