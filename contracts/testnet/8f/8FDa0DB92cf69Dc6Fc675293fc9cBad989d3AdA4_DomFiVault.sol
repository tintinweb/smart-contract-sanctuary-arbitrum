...
contract DomFiVault is Ownable, ReentrancyGuard, IVault, ERC4626, IRewardRecipient {
    ...
    uint256 constant MAX_WITHDRAW_DELAY = 10 days; // worst-case wait to withdraw collateral
    uint256 constant TOTAL_DELAY_PER_UTILIZATION_RATIO = 100 days; // withdrawing 10% of the vault would take 1 day for each 10% utilization ratio above the minimum
    ...
    function previewRedeemRequest(Shares shares, address account) external returns (uint64 length, int64 offset) {
        ...
        if (instantRedeemAvailable()) {
            return (0, int64(uint64(block.timestamp)));
        }
