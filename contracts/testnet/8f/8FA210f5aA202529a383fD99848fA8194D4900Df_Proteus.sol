...
contract Proteus is ILiquidityPoolImplementation, Slices {
    ...
    uint256 public constant BASE_FEE = 2000;
     ...
     * @dev isolating u in the equation above gives us the equation:
     *   u = (bx + ay +- sqrt((bx)^2 + 4xy - 2aybx + (ay)^2 )) / k(2 - 2ab)
     *   Because of the +- we have two solutions. We always want the
     *   larger solution.
     */
    function _getUtility(
        ...
        int256 ay = a.muli(y);
        int256 bx = b.muli(x);
        int128 denominatorInv = (k.mul(ABDK_TWO.sub(ABDK_TWO.mul(a.mul(b)))))
            .inv();
        int256 discriminant = int256(
            Math.sqrt(uint256((bx**2) + (4 * x * y) + (ay**2) - (2 * ay * bx)))
        );
        int256 r0 = denominatorInv.muli(discriminant + ay + bx);
        int256 r1 = denominatorInv.muli((-discriminant) + ay + bx);
