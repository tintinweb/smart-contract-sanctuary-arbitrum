...
contract EllerianEquipment is ERC721 {
    ...
    uint256 private onchainSupply;  // Keeps track of the current supply.
    uint256 private onchainCounter; // Onchain mints start from 100000000.
    uint256 constant onchainStartTokenId = 100000000;
    ...
    */
    function mintUsingToken(address _recipient, uint256 _amount, uint256 _equipId) public {
        ...
        for (uint256 a = 0; a < _amount; a++) {
            uint256 tokenId = onchainStartTokenId + onchainCounter++;
    ...
    */
    function RetrieveFromGame(bytes memory _signature, uint256[] memory _tokenIds) external {
        ...
        require(signatureAbi.verify(signerAddr, msg.sender, _tokenIds.length, "withdrawal", tokenSum, _signature), "Invalid Unstake");
    }
