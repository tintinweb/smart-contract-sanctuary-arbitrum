...
contract CATERC721Proxy is Context, IERC721Receiver, CATERC721Governance, CATERC721Events, ERC165, INexa {
    using Strings for uint256;
    ...
    function bridgeOut(
        ...
        ICATERC721Proxy.CATPayload memory transfer = ICATERC721Proxy.CATPayload({
            uri: uriString,
            tokenId: tokenId,
            ...
            destTokenAddress: tokenContracts(destChain) == bytes32(0) ? addressToBytes(address(this)) : tokenContracts(destChain),
    ...
    function bridgeIn(ICATERC721Proxy.CATPayload memory transfer) internal {
    ...
    function nexaReceive(bytes32 payloadSender, uint256 srcChain, bytes memory payload, bytes32) external override {
        ICATERC721Proxy.CATPayload memory transfer = abi.decode(payload, (ICATERC721Proxy.CATPayload));
...
contract CATERC721Getters is CATERC721State {
    ...
    function encodeTransferCATNFT(
        ICATERC721.CATPayload memory transfer
    ...
    function decodeTransferCATNFT(
        bytes memory encoded
    ) public pure returns (ICATERC721.CATPayload memory transfer) {
        uint index = 0;
    ...
    function encodeTransferCATNFTProxy(
        ICATERC721Proxy.CATPayload memory transfer
    ) public pure returns (bytes memory encoded) {
        encoded = abi.encodePacked(
            transfer.uri,
            transfer.tokenId,
    ...
    function decodeTransferCATNFTProxy(
        bytes memory encoded
    ) public pure returns (ICATERC721Proxy.CATPayload memory transfer) {
        uint index = 0;
...
contract CATERC721Governance is CATERC721Getters, CATERC721Setters, Ownable {
    ...
    modifier onlyOwnerOrOwnerSignature(ICATERC721.SignatureVerification memory signatureArguments) {
        if (_msgSender() == owner()) {
            _;
        } else {
    ...
    modifier onlyRoutingEngine() {
        require(_msgSender() == address(routingEngine()) || _msgSender() == address(this), "CAT:Invalid Routing Engine");
        _;
    }
    ...
    function registerChain(
        ...
        ICATERC721.SignatureVerification memory signatureArguments
...
interface ICATERC721 {
    struct CATPayload {
        // URI of the CAT NFT token
        string uri;
        // Token Id of the NFT token
        uint256 tokenId;
    ...
    function bridgeIn(CATPayload memory catPayload) external;
...
import "../../cat/ERC721/CATERC721Proxy.sol";
...
contract CATProxyNFTFacet is Context {
	...
	function handleDeployProxyNFT(
		...
		address projectedTokenAddress = computeAddressNFTProxy(salt);
		require(!isContract(projectedTokenAddress) , "already address deployed");
		tokenAddress = address(new CATERC721Proxy{ salt: salt }());
		CATERC721Proxy(tokenAddress).initialize(diamondStorage.routingEngine, protocolPriority, diamondStorage.chainId, existingToken);
	 ...
	 * @param salt token salt which is unique and store in relayer backend system.
	 */
	function computeAddressNFTProxy(bytes32 salt) public view returns (address addr) {
