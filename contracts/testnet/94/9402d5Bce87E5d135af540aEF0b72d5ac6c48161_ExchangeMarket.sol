...
import "./interface/IMarketProxy.sol";
import "@openzeppelin/contracts/access/AccessControl.sol";
import "@openzeppelin/contracts/interfaces/IERC2981.sol";

contract ExchangeMarket is AccessControl{
    ...
    // receiver who will receive the platformFee.
    address public treasuryHolder;

    IMarketProxy public MarketProxy;
    ...
    */
    ...
    /**
        @notice - this function executes the BUY and SELL orders.
        @param orderdetails - see {OrderDetails}.
        @param sign - see{Sign}.
    */
    ...
    function excuteOrder(
        ...
        if(
            ...
            require(orderdetails.buyer == msg.sender,'Buyer must be the caller');
        ...
        if(
            ...
            require(orderdetails.seller == msg.sender,'Seller must be the caller');
            Fee memory fee = getFees(
                                orderdetails
                            );
            transferAsset(
    ...
    //@notice the function calculates platform Fee, assetPrice, royaltyFee from the NFT token SalePrice.
    ...
    /**
        @notice this function transfers NFTs and tokens.
        @param order - see {OrderDetails}.
        @param fee - see {Fee}.
     */
    ...
    function transferAsset(
        ...
        Fee memory fee
                ...
                MarketProxy.erc721safeTransferFrom(
        ...
        if (fee.platformFee > 0) {
            MarketProxy.erc20safeTransferFrom(
    ...
    //@param signer address of user,who's selling the NFT.
    //@param nftAddress address of NFT contract where the NFT token is created/Minted.
    //@param paymentAssetAddress address of the token, which is used as payment token(WETH/WBNB/WMATIC...).
    //@param tokenId unique id  of NFT.
    //@param paymentAmount the price of NFT(assetFee + buyerFee).
    ...
    function verifySign(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(this, nftAddress, signer, paymentAssetAddress, paymentAmount, tokenId,sign.nonce)
        );
...
//@title ITransferProxy is used for communicating with NFT, token.
...
interface IMarketProxy {

    //@notice the function transfers Camly721 NFTs to users.
    //@param token, ERC721 address @dev see {IERC721A}.
