...
// File contracts/interfaces/ITokenGatewayCommon.sol
...
// File contracts/interfaces/IL2TokenGateway.sol
...
// File contracts/gateways/GatewayManager.sol

// SPDX-FileCopyrightText: 2022 Lido <[email protected]>

pragma solidity ^0.8.13;

contract GatewayManager is AccessControl {
    struct State {
...
// File contracts/gateways/InterchainTokensGateway.sol
    ...
    ITokenGatewayCommon
{
    ...
    function finalizeInboundTransfer(
        ...
        uint256 amount,
        bytes calldata
    ) external onlyL1Token(token) onlyCounterpartGateway {
        _onFinalizeInboundTransfer(from, to, amount);
    }

    function _onFinalizeInboundTransfer(
        address from,
        address to,
        uint256 amount
    ) internal virtual;
    ...
    error ErrorNotCounterpartGateway();
}


// File contracts/interfaces/IArbSys.sol
...
// File contracts/libraries/L2CrossDomain.sol

// SPDX-FileCopyrightText: 2022 Lido <[email protected]>

pragma solidity ^0.8.13;

library L2CrossDomain {
    struct CrossDomainTxOptions {
        uint256 maxGas;
        uint256 callValue;
        uint256 gasPriceBid;
    ...
    function isCrossDomainSender(address account) internal view returns (bool) {
        uint160 offset = uint160(0x1111000000000000000000000000000000001111);
        address aliasedAccount = address(uint160(account) + offset);
        return msg.sender == aliasedAccount;
    }
}
...
// File contracts/interfaces/IERC20Ownable.sol
...
// File contracts/libraries/L2OutboundDataParser.sol
...
// File contracts/gateways/L2TokensGateway.sol
...
contract L2TokensGateway is InterchainTokensGateway, IL2TokenGateway {
        ...
        returns (bytes memory res)
    {
        ...
        uint256 id = L2CrossDomain.sendCrossDomainTx(
            arbSys,
            counterpartGateway,
    ...
    function _onFinalizeInboundTransfer(
        ...
        address to,
        uint256 amount
    ) internal override whenDepositsEnabled {
        IERC20Ownable(l2Token).mint(to, amount);
        emit DepositFinalized(l1Token, from, to, amount);
    }

    modifier onlyCounterpartGateway() override {
        if (!L2CrossDomain.isCrossDomainSender(counterpartGateway)) {
            revert ErrorNotCounterpartGateway();
        }
        _;
    }
}