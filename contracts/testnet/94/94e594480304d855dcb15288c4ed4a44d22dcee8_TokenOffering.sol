...
contract TokenOffering is Ownable {
    struct UserShares {
    ...
    constructor(
        ...
        require(_presaleBoost >= 1000, "Presale boost must be greater than 1000.");
        require(_largeTicketBoost >= 1000, "Large ticket boost must be greater than 1000.");
    ...
    function purchasePresale(uint64 amount) external {
        ...
        if (currentUserDeposit + amount >= largeTicketThreshold) {
            totalSharesToAdd = (totalSharesToAdd * largeTicketBoost) / 1000;
    ...
    function getTotalShares() external view returns (uint64) {
        return ((presaleShares * presaleBoost) / 1000) + publicShares;
    }
    ...
    function getUserShares(address _address) external view returns (uint64) {
        if (allowList[_address]) {
            uint64 presaleSharesUser = (shareTracking[_address].presaleShares * presaleBoost) / 1000;
    ...
    function _addBoostForPriorShares() internal view returns (uint64) {
        ...
        uint64 boostToAdd = ((priorShares * largeTicketBoost) / 1000) - priorShares;
        return boostToAdd;
    }
}