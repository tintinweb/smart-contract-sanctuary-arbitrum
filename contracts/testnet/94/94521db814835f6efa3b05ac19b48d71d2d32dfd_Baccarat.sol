...
contract Baccarat is CommonSoloBaccarat, InternalRNG {
  ...
  function previousPlayerChoicesRebet(
    ...
    tieWins_ = game_.bet.tieWinsInChips;
    bankWins_ = game_.bet.bankWinsInChips;
    playerWins_ = game_.bet.playerWinsInChips;
  }
  ...
  function rebet() external {
    ...
    BaccaratGame memory game_ = games_[requestId_];
    _bet(
      game_.bet.tieWinsInChips,
      game_.bet.bankWinsInChips,
      game_.bet.playerWinsInChips,
      game_.token
    );
  }
...
abstract contract CommonSoloBaccarat is Core {
  ...
  // old event (with a struct within a struct)
  // event HandFinalized(address indexed player, uint256 requestId, BothHands hands);

  // new event (no struct within a struct, emits 2 structs)
  ...
  // struct BothHands {
  //   bool hasThirdCardPlayer;
  //   bool hasThirdCardBanker;
  //   uint8 firstCardPlayer;
  //   uint8 secondCardPlayer;
  //   uint8 thirdCardPlayer;
  //   uint8 firstCardBanker;
  //   uint8 secondCardBanker;
  //   uint8 thirdCardBanker;
  // }
  ...
  function _refundGame(uint256 _requestId) internal {
    ...
    // vaultManager.refund(_game.token, _game.bet.totalWagerInChips, 0, _game.player);
