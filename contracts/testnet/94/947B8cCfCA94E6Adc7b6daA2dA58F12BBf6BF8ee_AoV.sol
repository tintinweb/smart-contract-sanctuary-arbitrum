...
contract AoV is IAoV, ERC721A, ReentrancyGuard, ManagerModifier {
  ...
  function burnFor(uint256 _tokenId) external override onlyMinter {
    // Burn
    _burn(_tokenId);
  }
  ...
  {
    return
      IAovMetadata(_metadataAddress(address(this), _tokenId)).uri(
        address(this),
        _tokenId
      );
  }
  ...
  // Internal
  //=======================================
  function _metadataAddress(address _addr, uint256 _tokenId)
    internal
    view
    returns (address)
  {
    return ADVENTURER_DATA.metadata(_addr, _tokenId);
  }
