 ...
 */
contract ERC1155TokenDistributor is AccessControl, ReentrancyGuard, ITokenDistributor {
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN");
    bytes32 public constant STAKING_ROLE = keccak256("STAKING");

    ITestERC1155 public erc1155;
    ...
    constructor(
        ...
        require(erc1155_ != address(0), "ERC1155TOKENDISTRIBUTOR:ILLEGAL_ADDRESS");
        require(admin_ != address(0), "ERC1155TOKENDISTRIBUTOR:ILLEGAL_ADDRESS");
        require(staking_ != address(0), "ERC1155TOKENDISTRIBUTOR:ILLEGAL_ADDRESS");
        erc1155 = ITestERC1155(erc1155_);
    ...
    modifier onlyAdmin() {
        require(hasRole(ADMIN_ROLE, msg.sender), "ERC1155TOKENDISTRIBUTOR:ACCESS_DENIED");
        _;
    }

    modifier onlyStaking() {
        require(hasRole(STAKING_ROLE, msg.sender), "ERC1155TOKENDISTRIBUTOR:ACCESS_DENIED");
        _;
    }

    function payout(address payee, uint256 amount) public nonReentrant onlyStaking {
        uint256 totalPending = totalPendingRewards[payee] + amount;
        uint256 realAmount = totalPending / (1 ether / 2);
        totalPendingRewards[payee] = totalPending % (1 ether / 2);
        if (realAmount > 0) {
            erc1155.mint(realAmount, 1155, payee);
        }
    }
}
