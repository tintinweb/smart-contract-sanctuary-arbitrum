...
contract DropFacet is AppFacet {
    ...
    function batchAirdrop(
    ...
    ) external payable onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        uint256 length = recipients.length;
        require(quantities.length == length, "Invalid Arguments");

        uint64 totalMinted = 0;
        ...
        if (niftyKit.getOwnerPerksRate(IERC173(address(this)).owner()) > 0) {
            return;
        }

        (uint256 mintFees, ) = niftyKit.getFeesByQuantity(totalMinted);
        require(mintFees <= msg.value, "Value incorrect");
    ...
    function airdropFee() external view returns (uint256) {
        INiftyKitV3 niftyKit = BaseStorage.layout()._niftyKit;
        (uint256 mintFees, uint256 ownerPerks) = niftyKit.getFeesByQuantity(1);
        return ownerPerks > 0 ? 0 : mintFees;
    }
    ...
    function _purchaseMint(uint64 quantity, address to) internal {
        ...
        require(
            mintPrice + mintFees + ownerPerks <= msg.value,
            "Value incorrect"
        );
