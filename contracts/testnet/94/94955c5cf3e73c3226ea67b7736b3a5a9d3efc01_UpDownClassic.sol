...
contract UpDownClassic is IClassic, SupraOracle, Ownable, Pausable, ReentrancyGuard {
    ...
    function claim(address[][] calldata betTokens, uint256[] calldata epochs) external nonReentrant notContract {
        ...
        for (uint256 i; i < epochs.length; i++) {
            ...
            if (round.oracleCalled) {
                for (uint256 j; j < betTokens[i].length; j++) {
                    ...
                    roundReward += ledger[epochs[i]][msg.sender][betTokens[i][j]].amount * round.rewardAmount
            ...
            // Round invalid, refund bet amount
                ...
                for (uint256 j; j < betTokens[i].length; j++) {
                    ...
                    roundReward += ledger[epochs[i]][msg.sender][betTokens[i][j]].amount;
                    ledger[epochs[i]][msg.sender][betTokens[i][j]].claimed = true;
                }
            }
        ...
        emit Claim(msg.sender, epochs, betTokens, reward);
        if (reward > 0) {
...
interface IClassic {
    ...
    event Claim(address indexed sender, uint256[] indexed epoch, address[][] indexed tokens, uint256 amount);
