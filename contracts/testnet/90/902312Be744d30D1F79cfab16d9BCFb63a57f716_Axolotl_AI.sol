...
// File: contracts/token/Axolotl_AI.sol


pragma solidity =0.8.19;


contract Axolotl_AI is ERC20, Ownable { //IsIBEP20 ?
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;

    event SwapBack(uint256 holder, uint256 liquidity, uint256 flex, uint256 marketing, uint256 dev,  uint timestamp);
    ...
    uint256 public flexFee = 100;
    uint256 public marketingFee = 100;
    uint256 public devFee = 100;
    uint256 public totalFee = holderFee + liquidityFee + flexFee + marketingFee + devFee;
    uint256 public feeDenominator = 10000;


    // Fees receivers
    address private flexWallet;
    ...
    address routerAddress = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
    uint256 _targettotalSupply = 10_000_000_000 * 1e6; //Todo: Check if supply is 10 billion and if its 6 decimals

    EnumerableSet.AddressSet private _pairs;


    // 0x912ce59144191c1204e64559fe8253a0e49e6548, 0x6eccab422d763ac031210895c81787e87b43a652, 0xc873fecbd354f5a56e00e710b90ef4201db2448d, 0x82af49447d8a07e3bd95bd0d56f35241523fbab1
    // 0x000C0de00009B0036758Ff24b7210146A0929010,0x000C0de00009B0036758Ff24b7210146A0929010,0x000C0de00009B0036758Ff24b7210146A0929010,0x000C0de00009B0036758Ff24b7210146A0929010
    constructor(
        IERC20 _backToken,  // 0x912ce59144191c1204e64559fe8253a0e49e6548
        address _factory, //0x6eccab422d763ac031210895c81787e87b43a652
        address _swapRouter, //0xc873fecbd354f5a56e00e710b90ef4201db2448d
        address _weth // 0x82af49447d8a07e3bd95bd0d56f35241523fbab1
    ) ERC20("Axolotl AI", "ARBAX") { //Todo: Check wether it shows properly
    ...
    function _axoTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!canAddLiquidityBeforeLaunch[sender]) { //If trading isnt opened yet, and you are not specially authorized, you cant transfer or trade tokens
        ...
        uint side = 0; //Side = 0 -> Just a transfer between wallets. Side:1 -> Someone just bought Axo. Side:2 -> Someone just sold Axo.
        address user_ = sender;
        address pair_ = recipient;
        // Set Fees
        if (isPair(sender)) { //Why isPair? Maybe only on trades you pay fee, but not on simple transfers between 2 wallets?
            //overallFees(); //Pull the most current fees after they have been set
            side = 1;
            //user_ = recipient;
            //pair_ = sender;
            //try jackpotWallet.tradeEvent(sender, amount) {} catch {} //Todo: delete? Whole block or only this line?
        } else if (isPair(recipient)) {
            //sellFees();
            side = 2;
        } else {
           // shouldTakeFee = false; //Todo: Comment in! This is commented out for testing. This enables fees on simple transfers!
        }

        if (shouldSwapBack()) { //When all is well and sender is not a pair, taxes get taken and sent: Burn, LP, bonus
            swapBack();
        }

        uint256 amountReceived = shouldTakeFee ? takeFee(sender, amount) : amount; //Calculat after fee amount to send; maybe refactor
    ...
    //Swap all of Axo for arb while paying fees
    ...
    function swapBack() internal swapping {
        ...
        try dividendDistributor.deposit{value: amountAxoHolder}() {} catch {} //Safe holders reflection to Distributor

        taxAmount -= amountAxoLp; //Todo: This never gets used anywhere. Where does it go? Is it supposed to be added to lp?
        taxAmount -= amountAxoHolder;
        ...
        uint256 amountBackToken = backToken.balanceOf(address(this)) - balanceBefore; //This is the arb he is getting back from the swap
        uint256 backTokenTotalFee = totalFee - holderFee - liquidityFee;
        uint256 amountBackTokenFlex = (amountBackToken * flexFee) / (backTokenTotalFee);
        ...
        if (addLiquidityEnabled) { //Automatically converts balance of this contract to LP
            _doAddLp(); //Todo: Are we able to pull LP out at some point, or is it stuck forever?
        }


        emit SwapBack(amountAxoHolder, amountAxoLp, amountBackTokenFlex, amountBackTokenMarketing, amountBackTokenDev, block.timestamp);
    }
    ...
    function _doAddLp() internal {
        ...
        if(half < 1000) return; //Todo: Swap this value to something that reflects our total supply
    ...
    function rescueToken(address tokenAddress) external onlyOwner {
       // IERC20(tokenAddress).safeTransfer(msg.sender,IERC20(tokenAddress).balanceOf(address(this))); //Todo: This doesnt work for the newly deployed token
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'Axolotl AI: ETH_TRANSFER_FAILED');
    }
    ...
    function launch() public onlyOwner {
        ...
        //Tokenomics
        _mint(address(this),  (_targettotalSupply * 20) / 100); //Minting so it can be burnt    //Todo Note: Does mint and instaly burn work? If yes, delete belo wextra burn function. Set fees before launching or this will fail
        _burn(address(this), (_targettotalSupply * 20) / 100);  //Burning the 20%
        _mint(presaleWallet,  (_targettotalSupply * 30) / 100); //Presale 30%
        _mint(liqWallet,  (_targettotalSupply * 20) / 100); // Liquidity 20%
        _mint(devWallet,  (_targettotalSupply * 5) / 100); // Development 5%
        _mint(marketingWallet,  (_targettotalSupply * 5) / 100); // Marketing/Partnerships 5%
        _mint(flexWallet,  (_targettotalSupply * 4) / 100); // Cex Listings 4%
        _mint(flexWallet,  (_targettotalSupply * 3) / 100); // Advisors 3%
        _mint(owner(),  (_targettotalSupply * 9) / 100); // Unlocked 9%
        _mint(airdropWallet,  (_targettotalSupply * 4) / 100); // Airdrop 4%

        //Leave 9 % unlocked for trading
    }
    ...
    function setOverallFees(
        ...
        totalFee = _holderFee + _liquidityFee + _flexFee + _marketingFee + devFee;
    }

    function setFeeReceivers(
        address _flexWallet, //Todo: Launch: Set receivers before trading is allowed
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "Axolotl AI: pair is the zero address");
        isDividendExempt[pair] = true;
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "Axolotl AI: index out of bounds");
    ...
    function changeIsDividendExempt(address holder, bool exempt) external onlyOwner {
        require(holder != address(this) && holder != getPair(0)); //Pair is supposed to be dividenexempt by default and suppoed to stay that way. Other pairs could be made pay divident, which is a mistake. Is the pair supposed to be exempt or not?
        isDividendExempt[holder] = exempt;                          //I think yes, because they are not supposed to be earning dividens for holding
