    ...
    Ownable
{
    ...
    uint256 public protocolFeePercent = 0.02e18; // 2%
    uint256 public bubbleOwnerFeePercent = 0.02e18; // 2%
    ...
    function getScalingFactor(address bubbleAddress) public view returns (uint256) {
        uint256 xf = bubbles[bubbleAddress].supply-1;
        uint256 originalValue = getPrice(1, xf <= 0 ? 0 : xf);
        uint256 scalingFactor = bubbles[bubbleAddress].value / originalValue;
    ...
    function createBubble() public payable {
        ...
        emit BubbleCreated(msg.sender);
        buyShares(msg.sender, 1);
    }

    function buyShares(address bubbleAddress, uint256 amount) public payable canTrade {
    ...
    function sellShares(address bubbleAddress, uint256 amount) public payable canTrade {
        uint256 supply = bubbles[bubbleAddress].supply;
