...
contract MarketplaceARB is Ownable {
    ...
    event AddOrderAuction(uint256 _auctionIdCount, address nftAddress, uint256 tokenId, address paymentToken, uint256 startBlock, uint256 endBlock, address seller);
    ...
    function addAuction(address nftAddress_, uint256 tokenId_, address paymentToken_, uint256 minPrice_, uint256 startBlock_, uint256 endBlock_) public onlySupportedPaymentToken(paymentToken_) {
        ...
        IERC721(nftAddress_).transferFrom(_msgSender(), address(this), tokenId_);
        emit AddOrderAuction( _auctionId, nftAddress_,  tokenId_, paymentToken_, startBlock_,  endBlock_, msg.sender);
    }
    ...
    function bid(uint256 auctionId_, uint256 bidPrice_) public {
        ...
        require(bidPrice_ > _orderAuction.highestBid, "Bid must be greater than current highest bid");
        if(_orderAuction.highestBid > 0) {
            _orderAuction.bids[_orderAuction.highestBidder] += _orderAuction.highestBid;
        }
        IERC20(_orderAuction.paymentToken).transferFrom(msg.sender, address(this), bidPrice_);
        _orderAuction.highestBid = bidPrice_;
        _orderAuction.highestBidder = msg.sender;
        emit BidPlaced(auctionId_, msg.sender, bidPrice_);
    }

    function addBid(uint256 auctionId_, uint256 price_) public {
        OrderAuction storage _orderAuction = orderAuction[auctionId_];
        require(msg.sender != _orderAuction.seller, "Owner can't bid on own NFT");
        require(block.timestamp >= _orderAuction.startBlock && block.timestamp <= _orderAuction.endBlock, "Auction is not currently active");
        require(_orderAuction.bids[msg.sender] > 0, "you have not added bid");
        uint256 amount = _orderAuction.bids[msg.sender] + price_;
        IERC20(_orderAuction.paymentToken).transferFrom(msg.sender, address(this), price_);
        if(amount > _orderAuction.highestBid) {
            _orderAuction.highestBid = amount;
            _orderAuction.highestBidder = msg.sender;
            _orderAuction.bids[_orderAuction.highestBidder] = _orderAuction.highestBid;
        } else {
            _orderAuction.bids[msg.sender] = amount;
        }
        emit BidPlaced(auctionId_, msg.sender, amount);
    }
