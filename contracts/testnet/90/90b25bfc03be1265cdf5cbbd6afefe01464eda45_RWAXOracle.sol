...
contract RWAXOracle {
    ...
    PairOracle public rwaxEthPair;
    PairOracle public ethBusdPair;

    constructor(address _rwax, address _weth, address _rwaxEthPair, address _ethBusdPair, address _updater) {
        require(_rwax != address(0), "RWAXOracle::initialize: invalid address");
        require(_weth != address(0), "RWAXOracle::initialize: invalid address");
        require(_rwaxEthPair != address(0), "RWAXOracle::initialize: invalid address");
        require(_ethBusdPair != address(0), "RWAXOracle::initialize: invalid address");
        require(_updater != address(0), "RWAXOracle::initialize: invalid address");
        rwaxEthPair = PairOracle({
            pair: IUniswapV2Pair(_rwaxEthPair),
            token: _rwax,
    ...
    function getCurrentTWAP() public view returns (uint256) {
        // round to 1e12
        return rwaxEthPair.currentTWAP() * ethBusdPair.currentTWAP() / PairOracleTWAP.PRECISION / PRECISION;
    }

    // =============== USER FUNCTIONS ===============

    function update() external {
        require(msg.sender == updater, "RWAXOracle::updatePrice: !updater");
        rwaxEthPair.update();
        ethBusdPair.update();
        lastTWAP = rwaxEthPair.lastTWAP * ethBusdPair.lastTWAP / PairOracleTWAP.PRECISION / PRECISION;
