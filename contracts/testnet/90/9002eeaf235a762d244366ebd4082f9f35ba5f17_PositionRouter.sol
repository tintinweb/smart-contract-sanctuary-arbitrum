...
contract PositionRouter is BaseRouter, IPositionRouter, ReentrancyGuard {
    ...
    function openNewPosition(
        ...
        //Scope to avoid stack too deep error
        {
            ...
            require(position.owner == address(0) || position.owner == msg.sender, "IVLPO"); //Invalid positionOwner
    ...
    */
    ...
    function _modifyPosition(
        ...
        //Min encode data length for position 13 and order 9 is 22 * 32 = 704
        if (_isOpenPosition(_txType) && _data.length >= 704) {
            (position, order) = abi.decode(_data, ((Position), (OrderInfo)));
            params = txnDetails[_key][_txType].params;
            amountIn = order.pendingCollateral;
            //data = abi.encode(_isFastExecute, _isNewPosition, params, position, order);
        } else {
        ...
        if (!_isFastExecute || _isDelayPosition(_txType)) {
        ...
        if (_txType == ADD_POSITION || 
            ...
            require(position.size > 0, "IVLPS/NI"); //Invalid position, not initialized
        }
        ...
        if (_isOpenPosition(_txType)) {
            bool isNewPosition = order.status == OrderStatus.PENDING;
            ...
            data = abi.encode(amountIn, _isFastExecute, isNewPosition, params, position, order);
        ...
        positionHandler.modifyPosition(
            _key,
            _isDelayPosition(_txType) ? TRIGGER_POSITION : _txType,
            _path,
            _prices,
            data
        );
    ...
    function _createPrepareTransaction(
        ...
        require(amountOutMin > 0, "IVLAOM"); //Invalid amountOutMin
