...
contract PriceFeed is IPriceFeed, Initializable, EndpointGatedUpgradeable {
    ...
    // =============== USER FUNCTIONS ===============
    function updatePrice(bytes[] memory _pythData, bytes[] memory _chainlinkData) external payable {
        if (_pythData.length != 0) {
            _updatePythPrice(_pythData);
        }
        if (_chainlinkData.length != 0) {
