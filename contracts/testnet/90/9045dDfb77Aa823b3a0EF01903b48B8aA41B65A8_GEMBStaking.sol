...
import "./GEMBVesting.sol";

contract GEMBStaking is GEMBVesting {
    /*==================================================== State Variables ========================================================*/
    mapping(address => StakeDividend) public dividendGEMBStakes;
    mapping(address => StakeDividend) public dividendVestedGEMBStakes;

    /*==================================================== Constructor ===========================================================*/
    constructor(
        address _vaultRegistry,
        address _timelock
    ) GEMBVesting(_vaultRegistry, _timelock) {}
    ...
    /**
     *
     * @dev Retrieves the staked amount of dividend GEMB tokens for a specified account and stake type.
     * @param _account The address of the account to retrieve the staked amount for.
     * @param _isVested A boolean flag indicating whether to retrieve the vested GEMB or GEMB dividend stake.
     * @return _amount The staked amount of dividend GEMB/vGEMB tokens for the specified account and stake type.
     * @dev The function retrieves the staked amount of dividend GEMB/vGEMB tokens for the specified account and stake type from the dividendGEMBStakes or dividenVestedGEMBdStakes mapping,
    ...
    function dividendStakedAmount(
        ...
        return
            _isVested
                ? dividendVestedGEMBStakes[_account].amount
                : dividendGEMBStakes[_account].amount;
    }
     ...
     * @dev The function retrieves the dividend stake for the specified account and stake type from the dividendGEMBStakes or dividenVestedGEMBdStakes mapping, depending on the value of the isVested parameter.
     */
    function getDividendStake(
        ...
        return
            _isVested
                ? dividendVestedGEMBStakes[_account]
                : dividendGEMBStakes[_account];
    }
    ...
    function pendingDividendRewards(
        ...
        // Calculate the pending reward based on the dividend vested GEMB stake of the given account
        pending_ += _pendingByDividendStake(dividendVestedGEMBStakes[_account]);
        // Calculate the pending reward based on the dividend GEMB stake of the given account
        pending_ += _pendingByDividendStake(dividendGEMBStakes[_account]);
    }
    ...
    /**
     *
     * @dev Internal function to deposit GEMB/vGEMB as dividends.
     * @param _amount The amount of GEMB/vGEMB to be deposited.
     * @param _isVested Boolean flag indicating if tokens are vGEMB.
    ...
    function depositDividend(
        ...
        if (_isVested) {
            tokenManager.takeVestedGEMB(sender_, _amount);
            stake_ = dividendVestedGEMBStakes[sender_];
            totalStakedVestedGEMB += _amount;
        } else {
            tokenManager.takeGEMB(sender_, _amount);
            stake_ = dividendGEMBStakes[sender_];
            totalStakedGEMB += _amount;
        }
     ...
     * @param _isVested Boolean flag indicating if stake is Vested GEMB.
    ...
    function unstake(
        ...
        StakeDividend storage stake_ = _isVested
            ? dividendVestedGEMBStakes[sender_]
            : dividendGEMBStakes[sender_];
        ...
        // Burn the necessary amount of tokens and send the remaining unstaked tokens to the staker.
        if (_isVested) {
            tokenManager_.burnVestedGEMB(burnAmount_);
            tokenManager_.sendVestedGEMB(sender_, sendAmount_);
            unstakedWeight_ = _amount * weightMultipliers.vGemb;
            totalStakedVestedGEMB -= _amount;
        } else {
            tokenManager_.burnGEMB(burnAmount_);
            tokenManager_.sendGEMB(sender_, sendAmount_);
            unstakedWeight_ = _amount * weightMultipliers.gemb;
            totalStakedGEMB -= _amount;
        }
    ...
    function _claimDividend(
        ...
        // Send the rewards to the stake owner.
        tokenManager.sendGEMLP(_account, reward_);
    ...
    function _claimDividendBatch(
        ...
        // Determine the stake details based on the boolean flag isVested.
        StakeDividend storage stakeVGEMB_ = dividendVestedGEMBStakes[_account];
        StakeDividend storage stakeGEMB_ = dividendGEMBStakes[_account];

        // Calculate the pending rewards for the stake.
        reward_ = _pendingByDividendStake(stakeVGEMB_);
        reward_ += _pendingByDividendStake(stakeGEMB_);
        ...
        // Update the profit debt for the stake.
        stakeVGEMB_.profitDebt = _calcDebt(stakeVGEMB_.weight);
        stakeGEMB_.profitDebt = _calcDebt(stakeGEMB_.weight);
    ...
    /**
     * @notice Computes the pending GEMLP amount of the stake.
     * @param _stake The stake for which to compute the pending amount.
     * @return holderProfit_ The pending GEMLP amount.
     */
...
abstract contract GEMBVesting is
    IGEMBStaking,
    Pausable,
    ReentrancyGuard,
    AccessControlBase
{
    ...
    //The total amount of GEMB tokens staked in the contract
    uint256 public totalStakedGEMB;
    //The total amount of vGEMB tokens staked in the contract
    uint256 public totalStakedVestedGEMB;
     ...
     * @return pending The amount of pending GEMLP tokens that the staker is eligible to receive.
     * @dev The function iterates over all active stakes for the given staker and calculates the pending rewards for each stake using the _pendingGEMLPOfStake() internal function.
    ...
    function pendingVestingRewards(
        ...
        for (uint256 i = 0; i < activeIndexes.length; i++) {
            ...
            pending += _pendingGEMLPOfStake(stake);
        }
    }
     ...
     * @param account The address of the staker for whom to calculate the pending reward.
     * @return pending The amount of pending GEMLP tokens that the staker is eligible to receive.
     * @dev The function calculates the pending rewards for the stake using the _pendingGEMLPOfStake() internal function.
     * @dev The function is view-only and does not modify the state of the contract.
     */
     ...
     * @return _withdrawable  withdrawable amount of GEMB/vGEMB
     */
    ...
    /**
     * @return _totalStakedVGEMB The total amount of vGEMB tokens staked in the contract
     * @return _totalStakedGEMB The total amount of GEMB tokens staked in the contract
    ...
    function globalData()
        ...
        view
        returns (
            uint256 _totalStakedVGEMB,
            uint256 _totalStakedGEMB,
            uint256 _totalEarned
        )
    {
        _totalStakedVGEMB = totalStakedVestedGEMB;
        _totalStakedGEMB = totalStakedGEMB;
    ...
    function _calculateWeight(
        ...
        return
            !vested ? amount * weightMultipliers.gemb : vesting
                ? amount * weightMultipliers.vGembVesting
                : amount * weightMultipliers.vGemb;
    }

    /**
     * @notice Computes the pending GEMLP amount of the stake.
     * @param stake The stake for which to compute the pending amount.
     * @return holderProfit The pending GEMLP amount.
     */
    function _pendingGEMLPOfStake(
    ...
    /**
     * @notice Calculates the amount of GEMB/vGEMB that should be burned upon unstaking.
     * @param amount The amount of GEMB/vGEMB being unstaked.
     * @return _burnAmount The amount of GEMB/vGEMB to be burned.
     */
    ...
    /**
     * @notice Computes the withdrawable amount of GEMB for the stake.
     * @param stake The stake for which to compute the withdrawable amount.
     * @return withdrawable_ The withdrawable amount of GEMB.
     */
    ...
    function cancel(uint256 _index) external {
        ...
        totalStakedVestedGEMB -= amount_;
        totalWeight -= stake.weight;
        // claim rewards
        uint256 reward_ = _pendingGEMLPOfStake(stake);
        ...
        tokenManager.sendVestedGEMB(sender_, sendAmount_);

        // Burn the remaining vesting tokens
        tokenManager.burnVestedGEMB(burnAmount_);
    ...
    /**
     * @dev Deposit vGEMB tokens into the contract and create a vesting stake with the specified parameters
     * @param amount The amount of vGEMB tokens to deposit
    ...
    function depositVesting(
        ...
        uint256 accTokenPerDay = (amount - accTokenFirstDay) /
            period.claimDuration;

        // Transfer the vGEMB tokens from the sender to the token manager contract
        tokenManager.takeVestedGEMB(sender, amount);

        totalWeight += weight;
        totalStakedVestedGEMB += amount;
    ...
    function withdrawVesting(
        ...
        // Redeemable GEMB amount by stake
        uint256 redeemable_ = _withdrawableByVesting(stake_);
        // Redeemable GEMLP amount by stake
        uint256 reward_ = _pendingGEMLPOfStake(stake_);
        ...
        // Mint GEMB tokens to decrease MAX_SUPPLY
        if (amountToBurn > 0) {
            // this code piece is used to decrease burn amount from GEMB total supply
            tokenManager.mintGEMB(address(tokenManager), amountToBurn);
            tokenManager.burnGEMB(amountToBurn);
        }

        // Burn vested GEMB tokens
        tokenManager.burnVestedGEMB(stake_.amount);

        // Interactions
        if (reward_ > 0) {
            tokenManager.sendGEMLP(sender_, reward_);
        ...
        totalStakedVestedGEMB -= stake_.amount;
    ...
    function withdrawVestingBatch(
        ...
        for (uint256 i = 0; i < indexes.length; i++) {
            ...
            _amounts[2] += _pendingGEMLPOfStake(stake);
        ...
        // Mint GEMB tokens to decrease total supply
        if (amountToBurn > 0) {
            // this code piece is used to decrease burn amount from GEMB total supply
            // Mint GEMB tokens to the tokenManager contract
            tokenManager.mintGEMB(address(tokenManager), amountToBurn);
            tokenManager.burnGEMB(amountToBurn);
        }

        // Burn total vested GEMB tokens
        tokenManager.burnVestedGEMB(_amounts[3]);

        // Update the total weight and total staked amount
        totalWeight -= _amounts[0];
        totalStakedVestedGEMB -= _amounts[3];
    ...
    function _claim(uint256[] memory indexes, bool isClaim) internal {
        ...
        for (uint256 i = 0; i < indexes.length; i++) {
            ...
            uint256 fee_ = _pendingGEMLPOfStake(stake_);
        ...
        // Interactions
        if (totalFee_ > 0) {
            tokenManager.sendGEMLP(sender_, totalFee_);
        }
