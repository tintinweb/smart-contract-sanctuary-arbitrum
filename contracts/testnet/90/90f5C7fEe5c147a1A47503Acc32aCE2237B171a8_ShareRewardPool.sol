...
contract ShareRewardPool is Initializable, ReentrancyGuard {
    ...
    AggregatorV3Interface priceFeed =
        AggregatorV3Interface(0x62CAe0FA2da220f43a51F86Db2EDb36DcA9A5A08); // goerli
    // AggregatorV3Interface(0x639Fe6ab55C921f74e7fac1ee960C0B6293ba612); // mainnet

    IUniswapV2Factory constant v2Factory =
        IUniswapV2Factory(address(0x7B2cde10ebd24D0C5399359Ffc708B44BBc1854e)); // goerli
    // IUniswapV2Factory(address(0xc35DADB65012eC5796536bD9864eD8773aBc74C4)); // mainnet
    // Info of each user.
    struct UserInfo {
    ...
    uint256[] public percentageTiers = [
        100,
        50,
        44,
        41,
    ...
    bool public whaleSlide = true;
    uint256 public constant TOTAL_REWARDS = 30000 ether;
    uint256 public runningTime = 370 days; // 370 days
    // 0.0009384384 ether; 30000 share / (370 days * 24h * 60min * 60s)
    uint256 public sharesPerSecond = TOTAL_REWARDS / runningTime;
    ...
    function pendingShare(
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _sharesReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint
            );
            accTokensPerShare = accTokensPerShare.add(
                _sharesReward.mul(1e18).div(tokenSupply)
            );
        }
        return
    ...
    function depositfor(
        ...
        if (user.amount > 0) {
            uint256 _pending = user
                ...
                .sub(user.rewardDebt);
            if (_pending > 0) {
                safeShareTransfer(recipient, _pending);
                emit RewardPaid(recipient, _pending);
            }
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        UserInfo storage user = userInfo[_pid][_sender];
        if (whaleSlide) {
            if (isUniswapV2Pair(address(pool.token))) {
                require(
                    block.timestamp > user.lastWithdrawTime + 24 hours,
                    "withdraw: user is in cooldown"
                );
                uint256 value = calculateUserLPValue(
                    address(pool.token),
                    user.amount
                );
                uint256 cursor = 0;
                ...
                if (cursor > 1) {
                    require(
                        _amount <=
                            user.amount.mul(percentageTiers[cursor]).div(100),
                        "withdraw: try with less money"
                    );
                }
            }
        }
    ...
    function calculateUserLPValue(
        address _lpToken,
        uint256 amount
    ) public view returns (uint256) {
        IUniswapV2Pair pair = IUniswapV2Pair(_lpToken);
        (, uint256 token1Reserve, ) = pair.getReserves(); // wETH amount
        uint256 totalSupply = pair.totalSupply();
        (, int256 price, , , ) = priceFeed.latestRoundData();

        uint256 lpTokenPrice = (uint256(price).mul(token1Reserve).mul(2))
            .div(totalSupply)
            .div(1e8);
        return lpTokenPrice.mul(amount);
    }

    function setWhaleSlide(bool _whaleSlide) external onlyOperator {
        whaleSlide = _whaleSlide;
    }
}