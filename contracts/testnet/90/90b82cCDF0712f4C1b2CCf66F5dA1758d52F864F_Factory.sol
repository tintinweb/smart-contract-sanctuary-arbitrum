...
contract Exchange is Ownable {
    ...
    function newSellOrder(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            if (sellAmount == 0) {
                return true;
            } else if ((price * sellAmount) / (10**tokenA_decimals) >= buyAmount) {
                ...
                deposits[msg.sender][tokenA] -= (o.amount / price) * (10**tokenA_decimals - tokenB_decimals);
                IERC20(tokenB).transfer(msg.sender, o.amount);
                IERC20(tokenA).transfer(o.seller, (o.amount / price) * (10**tokenA_decimals - tokenB_decimals));
                sellAmount -= (o.amount / price) * (10**tokenA_decimals - tokenB_decimals);
            } else if (buyAmount > (price * sellAmount) / (10**tokenA_decimals)) {
                LinkedListLib.Order memory o = orderBook[tokenB][price]
                    .nodes[head_]
                    .order;
                orderBook[tokenB][price].nodes[head_].order.amount -=
                    (price * sellAmount) / (10**tokenA_decimals);
                ...
                PVNodeLib._subVolume(buyOB, priceIdx, (price * sellAmount) / (10**tokenA_decimals));

                deposits[o.seller][tokenB] -= (price * sellAmount) / (10**tokenA_decimals);
                deposits[msg.sender][tokenA] -= sellAmount;
                IERC20(tokenB).transfer(msg.sender, (price * sellAmount) / (10**tokenA_decimals));
    ...
    function newBuyOrder(
        ...
        // no fee under 1000
        deposit(tokenB, (price * buyAmount) / (10**tokenB_decimals));
        uint256 currentFee = (price * buyAmount * (1000 - feeRate)) / (10**tokenB_decimals) / 1000;
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            if (buyAmount == 0) {
                ...
                deposits[msg.sender][tokenB] -= (price * o.amount) / (10**tokenA_decimals);
                IERC20(tokenA).transfer(msg.sender, o.amount);
                IERC20(tokenB).transfer(o.seller, (price * o.amount) / (10**tokenA_decimals));
                buyAmount -= o.amount;
                ...
                deposits[msg.sender][tokenB] -= (price * buyAmount) / (10**tokenA_decimals);
                IERC20(tokenA).transfer(msg.sender, buyAmount);
                IERC20(tokenB).transfer(o.seller, (price * buyAmount) / (10**tokenA_decimals));
                buyAmount = 0;
            }
        }
        // new buy order
        if (orderBook[tokenB][price].length == 0 && buyAmount > 0) {
            bytes32 orderId = LinkedListLib.initHead(
                ...
                msg.sender,
                (price * buyAmount) / (10**tokenA_decimals)
            );
            OPVSetLib._add(
                _buyOrders,
                msg.sender,
            ...
            PVNodeLib._addVolume(buyOB, priceIdx, (price * buyAmount) / (10**tokenA_decimals));
        } else if (buyAmount > 0) {
            bytes32 orderId = LinkedListLib.addNode(
                orderBook[tokenB][price],
                msg.sender,
