...
import {LibBaseConfig} from "../libraries/LibBaseConfig.sol";
...
contract BaseConfigFacet {
    event FeeSet(uint256 minTakerFee, uint256 minMakerFee);
    event FeeRateSet(
        uint256 takerFeeRate,
        uint256 makerFeeRate,
        uint256 futureSettlementFeeRate,
    ...
    event IMSet(uint256 imRFQFactor, uint256 minIM, uint256 imTradeFactor, uint256 transferIMFactor);
    ...
    function setFee(uint256 _minTakerFee, uint256 _minMakerFee) external {
        ...
        LibBaseConfig.Layout storage l = LibBaseConfig.layout();
    ...
    function setFeeRate(
        ...
        uint256 _futureSettlementFeeRate,
        uint256 _optionSettlementFeeRate,
        uint256 _optionSettleAmountMaxFeeRate
    ) external {
        LibAccessControlEnumerable.checkRole(Constants.ADMIN_ROLE);
        LibBaseConfig.Layout storage l = LibBaseConfig.layout();
        l.takerFeeRate = _takerFeeRate;
        l.makerFeeRate = _makerFeeRate;
        l.futureSettlementFeeRate = _futureSettlementFeeRate;
        l.optionSettlementFeeRate = _optionSettlementFeeRate;
        l.optionSettleAmountMaxFeeRate = _optionSettleAmountMaxFeeRate;
        emit FeeRateSet(
            _takerFeeRate,
            _makerFeeRate,
            _futureSettlementFeeRate,
            _optionSettlementFeeRate,
            _optionSettleAmountMaxFeeRate
        );
    }

    function setIM(uint256 _imRFQFactor, uint256 _minIM, uint256 _imTradeFactor, uint256 _transferIMFactor) external {
        LibAccessControlEnumerable.checkRole(Constants.ADMIN_ROLE);
        LibBaseConfig.Layout storage l = LibBaseConfig.layout();
        l.imRFQFactor = _imRFQFactor;
        l.minIM = _minIM;
        l.imTradeFactor = _imTradeFactor;
        l.transferIMFactor = _transferIMFactor;
        emit IMSet(_imRFQFactor, _minIM, _imTradeFactor, _transferIMFactor);
    }
}
...
import {BaseConfigFacet} from "./BaseConfigFacet.sol";
...
contract StrategyOpenFacet is IStrategyOpen, ReentrancyGuard {
    ...
    /**
     * @notice 用户开仓+合并
     * @param receiver maker 签名地址
     * @param signature maker 签名字节
     * @param makerStrategy maker 签名结构体
     * @param takerStrategy taker 结构体
     */
    function createStrategy(
        ...
        // 校验2个角色策略是否完全相反方向
        bool isOpposite = LibStrategyConfig._validateOppositeStrategies(makerStrategy, takerStrategy);
        if (!isOpposite) {
            revert InvalidStrategy();
        }
        // 校验2个角色对应策略的仓位是否有可以合并的，如果有直接revert; 结合上面的完全相反验证，是否仅验证其中一个的可合并性就可以
        LibStrategyConfig._validateMergeabilityOfStrategy(makerStrategy);
        ...
        if (makerStrategy.mergeId != 0) {
            ...
            LibStrategyConfig._ensureAdminAndActive(makerMergeStrategy, makerMergeStrategy.owner);
        }

        if (takerStrategy.mergeId != 0) {
            StrategyTypes.StrategyAllData memory takerMergeStrategy = LibPositionCore._getStrategyAllData(
                takerStrategy.mergeId
            );
            LibStrategyConfig._ensureAdminAndActive(takerMergeStrategy, takerMergeStrategy.owner);
        }
    ...
    /**
     * @notice 执行开仓+合并
     * @param makerRequestHash maker 执行key，用于传输开仓请求的组合保证金
     * @param takerRequestHash taker 执行key，用于传输开仓请求的组合保证金
     */
    function executeCreateStrategy(bytes32 makerRequestHash, bytes32 takerRequestHash) external nonReentrant {
        ...
        uint256 takerFee = LibPositionCore._checkStrategy(takerStrategy, takerRequestHash, true);

        //将保证金从用户账户划转到策略账户
        LibVault._marginDecrease(makerStrategy.owner, makerStrategy.collateralAmount);
        LibVault._marginDecrease(takerStrategy.owner, takerStrategy.collateralAmount);
        // LibVault._marginIncrease(vl.feeOwner,makerFee + takerFee);
        vl.protocolFee += makerFee + takerFee;

        int256 makerPremium = LibPositionCore._getPremium(makerStrategy.option);
        int256 takerPremium = -makerPremium;

        uint256 makerStrategyId;
        uint256 takerStrategyId;
        if (makerPremium > 0) {
            makerStrategyId = LibPositionCore._controlCreateStrategy(makerStrategy, uint256(makerPremium), 0);
            takerStrategyId = LibPositionCore._controlCreateStrategy(takerStrategy, 0, takerPremium);
        } else {
            makerStrategyId = LibPositionCore._controlCreateStrategy(makerStrategy, 0, makerPremium);
            takerStrategyId = LibPositionCore._controlCreateStrategy(takerStrategy, uint256(takerPremium), 0);
        }
        //缺少将 takerFee 和 makerFee 转移到我们平台的地址
