...
import "../lib/int/ILiquidityTreasury.sol";
import "../lib/int/IRegistry.sol";
import "../utils/PricerLib.sol";

contract MultiVestingLiquidity is OwnableAdmin {
    ...
    event BondRedeemed(address recipient, uint256 payout, uint256 remaining, uint256 vestingID);

    /* ======== STATE VARIABLES ======== */

    IERC20 public immutable payoutToken; // token paid for payinToken
    IERC20 public immutable payinToken; // inflow token
    ILiquidityTreasury public immutable customTreasury; // pays for and receives payinToken
    ...
    uint256 public totalPayinBonded; // tracks payin deposited in the bond
    uint256 public totalPayoutGiven; // tracks payout withdrawn from treasury
    Terms public terms; // stores terms for new bonds
    mapping(address => Bond) public bondInfo; // stores bond information for depositors
    uint256[10] public totalDebts; // total debt for each vesting
    ...
    struct Terms {
        uint256[10] vestingTerms; // in blocks
        uint256[10] bonus; // 1000/100000 = 1%
        uint256 maxPayout; // in thousandths of a %. i.e. 500 = 0.5%
        uint256[10] maxDebt; // payout token decimal debt ratio, max % total supply created as debt
    }

    // Info for bond holder
    struct Bond {
        uint256[10] payout; // payout token remaining to be paid
        uint256[10] vesting; // Blocks left to vest
        uint256[10] lastBlocks; // Last interaction for each vestingID
        uint256 nonce; // Last interaction
    }
    ...
    constructor(
        ...
        customTreasury = ILiquidityTreasury(_customTreasury);
        ...
        // if you want to have a the bond price accounting the real price impact
    ...
    function initializeBond(
        ...
        for (uint256 i = 0; i < _vestingTerms.length; i++) {
            if (_bonus[i] > 0 && _maxDebt[i] > 0) {
                //check vesting only for active vestings
                require(_vestingTerms[i] >= 100, "Vesting need to be higher than 100 blocks");
            } else if (_vestingTerms[i] == 0) {
                //set the vesting to 1 if set to 0 to avoid division by 0
                _vestingTerms[i] = 1;
            }
            require(_maxDebt[i] <= 100_000_000, "MaxDebt > 100% totalSupply");
        }
        require(currentDebt() == 0, "Debt must be 0 for initialization");
        require(_maxPayout <= 1000, "Max payout > 1% totalSupply");

        terms = Terms({vestingTerms: _vestingTerms, bonus: _bonus, maxPayout: _maxPayout, maxDebt: _maxDebt});
     ...
     *  @param _maxPayout uint % of total supply of payoutToken in thousandths of a %. i.e. 500 = 0.5%
     *  @param _maxDebt uint[10] max number of tokens to be in debt for (wei)
     *  @param _isVariable bool if true the bond discount decrease as debt ratio increase
     *  until it reaches the minBonus % of bonus
     */
    function setBondTerms(
        uint256[10] memory _vestings,
        uint256[10] calldata _bonuses,
        uint256 _minBonus,
        uint256 _maxPayout,
        uint256[10] calldata _maxDebt,
        bool _isVariable
    ) external onlyAdmin {
        //unactive vestings must be set 0 or 1
        for (uint256 i = 0; i < _vestings.length; i++) {
            if (_bonuses[i] > 0 && _maxDebt[i] > 0) {
                //check vesting only for active vestings
                require(_vestings[i] >= 100, "Vesting need to be higher than 100 blocks");
            } else if (_vestings[i] == 0) {
                //set the vesting to 1 if set to 0 to avoid division by 0
                _vestings[i] = 1;
            }
            require(_maxDebt[i] <= 100_000_000, "MaxDebt > 100% totalSupply");
     ...
     */
    function setBondTermsByVestingId(uint256 _vestingID, uint256 _vesting, uint256 _bonus, uint256 _maxDebt)
        external
        onlyAdmin
    {
        //unactive vestings must be set 0 or 1
        uint256 i = _vestingID;
        if (_bonus > 0 && _maxDebt > 0) {
            //check vesting only for active vestings
     ...
     *  @param _vestings uint[10] how many blocks the vesting last
     */
    function setBondVestings(uint256[10] memory _vestings) external onlyAdmin {
        for (uint256 i = 0; i < _vestings.length; i++) {
            if (terms.bonus[i] > 0 && terms.maxDebt[i] > 0) {
                //check vesting only for active vestings
     ...
     *  @param _bonuses uint[10] the bonus given to bonder in thousandths of a %. i.e. 500 = 0.5%
     */
    function setBondBonuses(uint256[10] calldata _bonuses) external onlyAdmin {
        terms.bonus = _bonuses;
    }
     ...
     *  @param _minBonus uint minimum % of bonus given to bonders in thousandths of a %. i.e. 500 = 0.5%
     */
    function setBondMinBonus(uint256 _minBonus) external onlyAdmin {
        require(_minBonus <= 100000, "over 100%: underflow protection");
        minBonus = _minBonus;
    }
     ...
     *  @param _maxPayout uint % of total supply of payoutToken in thousandths of a %. i.e. 500 = 0.5%
     */
    function setBondMaxPayout(uint256 _maxPayout) external onlyAdmin {
        terms.maxPayout = _maxPayout;
    }
     ...
     *  @param _maxDebt uint[10] max number of tokens to be in debt for (wei)
     */
    function setBondMaxDebt(uint256[10] calldata _maxDebt) external onlyAdmin {
        for (uint256 i = 0; i < _maxDebt.length; i++) {
            require(_maxDebt[i] <= 100_000_000, "MaxDebt > 100% totalSupply");
        }
        terms.maxDebt = _maxDebt;
    }
    ...
    function setSlippageRatio(uint256 _slippageRatio) external onlyAdmin {
        //reduce _amountIn to allow larger bonds
        require(_slippageRatio <= 100000, "Over 100%"); // 100% = 100000
     ...
     *  @dev be sure that both tokens have a pair with quoteToken
     *  @dev otherwise just set one one of the tokens as quoteToken to avoid using it
    ...
    function updateRouter(address _newRouter) external onlyAdmin {
        ...
        //approve the new router for max in order to add liquidity
        IERC20(address(payinToken)).safeApprove(_newRouter, 2 ** 256 - 1);
        IERC20(address(payoutToken)).safeApprove(_newRouter, 2 ** 256 - 1);
    }
    ...
    function recoverTokens(address _token) external onlyAdmin {
        require(_token != address(payoutToken)); //protect users redeemable tokens
        ...
        returns (uint256)
    {
        ...
        decayDebtAll();
        uint256 payout = getPayout(_amount, _vestingID); // payout to bonder is computed
        ...
        // fees are calculated
        (uint256 fee, uint256 refFee, uint256 affFee) = actualFee(_amount, _referrer);
        ...
        uint256 amtLiquidity = getLpQuote(depositAmount, IUniswapV2Router01(Router).factory());
        ...
        require(totalDebts[_vestingID] <= maxDebt(_vestingID), "Max capacity reached");
        ...
        customTreasury.deposit(payout, amtLiquidity);
        (uint256 amountLiqPayin, uint256 amountLiqPayout, uint256 amountLP) = addLiquidity(depositAmount, amtLiquidity);
        ...
        emit BondCreated(
            _amount,
            payout,
            block.number + (terms.vestingTerms[_vestingID]),
            _vestingID,
            amountLiqPayin,
            amountLiqPayout,
            amountLP
    ...
    function redeem(address _depositor, uint256 _vestingID) external returns (uint256) {
        ...
        uint256 percentVested = percentVestedFor(_depositor, _vestingID); // (blocks since last interaction / vesting term remaining)

        if (percentVested >= 10000) {
            // if fully vested delete user info
            delete bondInfo[ _depositor ].payout[_vestingID];
            delete bondInfo[ _depositor ].vesting[_vestingID];
            delete bondInfo[ _depositor ].lastBlocks[_vestingID];
            ...
            uint256 payout = info.payout[_vestingID] * (percentVested) / (10000);

            bondInfo[_depositor].payout[_vestingID] = bondInfo[_depositor].payout[_vestingID] - (payout);
            bondInfo[_depositor].vesting[_vestingID] =
                info.vesting[_vestingID] - (block.number - (info.lastBlocks[_vestingID]));
            bondInfo[_depositor].lastBlocks[_vestingID] = block.number;
     ...
     */
    function decayDebt(uint256 _vestingID) internal {
     ...
     */
    function actualFee(uint256 _amount, address _referrer)
        internal
        view
     ...
     */
    function getLpQuote(uint256 _payinAmount, address _factory) internal view returns (uint256) {
        (uint256 reservePayin, uint256 reservePayout) =
            UniswapV2Library.getReserves(_factory, address(payinToken), address(payoutToken));
        return UniswapV2Library.quote(_payinAmount, reservePayin, reservePayout);
    }

    /**
     *  @notice performs addLiquidity operations using router
     *  @dev if your token have tax fees, exclude bond, treasury and router from fees
     */
    function addLiquidity(uint256 amountPayin, uint256 amountPayout)
        private
        returns (uint256 amountPayin_, uint256 amountPayout_, uint256 amountLP_)
    {
        (amountPayin_, amountPayout_, amountLP_) = IUniswapV2Router01(Router).addLiquidity(
            address(payinToken),
            address(payoutToken),
            amountPayin,
            amountPayout,
    ...
    /**
     *  @notice return the struct terms, useful for frontend
     */
    function getTerms()
        external
        view
    ...
    {
        vestings_ = terms.vestingTerms;
        bonuses_ = terms.bonus;
        maxPayout_ = terms.maxPayout;
        maxDebts_ = terms.maxDebt;
    }

    /**
     *  @notice calculates payout with bonus using the actual payin/payout ratio
     */
    function getPayout(uint256 _amountIn, uint256 _vestingID) public view returns (uint256 payout_) {
        require(_amountIn >= 100000, "size too small");

        uint256 amountReduced = _amountIn * (slippageRatio) / (100000);
        uint256 bonus = getBonus(_vestingID);

        //get the ratio between tokens, calculate amount and apply bonus
        uint256 ratio =
            PricerLib.getTokensBondRatio(address(payinToken), amountReduced, address(payoutToken), quoteToken, Router);
        payout_ = mulDiv(_amountIn, (ratio), (1e18));

        //add bonus to all cases
        payout_ += mulDiv(payout_, bonus, 100_000);
    }
     ...
     */
    function getBonus(uint256 _ID) public view returns (uint256 bonus_) {
        if (isPriceVariable) {
     ...
     */
    function totalDebt() public view returns (uint256 totalDebt_) {
        for (uint256 i = 0; i < totalDebts.length; i++) {
    ...
    /**
     *  @notice determine maximum bond capacity based on payoutToken totalSupply
     *  @dev set the maxDebt as a % of payoutToken tot supply where 100_000_000= 100%
     *  @return uint
     */
    function maxDebt(uint256 _vestingID) public view returns (uint256) {
        return payoutToken.totalSupply() * (terms.maxDebt[_vestingID]) / (100_000_000);
    }
     ...
     *
     */
    function debtRatio(uint256 _vestingID) public view returns (uint256 debtRatio_) {
        // 18 decimals, 1ether = 100%
        debtRatio_ = simpleRatio(_vestingID) * 100000 / 1e18;
    }

    function simpleRatio(uint256 _vestingID) internal view returns (uint256 simple) {
        simple = PricerLib.FixedFraction(currentDebtID(_vestingID), maxDebt(_vestingID));
    }
     ...
     */
    function currentDebt() public view returns (uint256 currentDebt_) {
        uint256 total = totalDebt();
        for (uint256 i = 0; i < terms.vestingTerms.length; i++) {
     ...
     *  @return currentDebt_ uint
     */
    function currentDebtID(uint256 _vestingID) public view returns (uint256 currentDebt_) {
     ...
     *  @return decay_ uint
     */
    function debtDecayID(uint256 _vestingID) public view returns (uint256 decay_) {
     ...
     */
    function debtDecayAll() public view returns (uint256 totalDecay_) {
        for (uint256 i = 0; i < totalDebts.length; i++) {
     ...
     */
    function percentVestedFor(address _depositor, uint256 _vestingID) public view returns (uint256 percentVested_) {
        Bond memory bond = bondInfo[_depositor];
        uint256 blocksSinceLast = block.number - (bond.lastBlocks[_vestingID]);
        uint256 vesting = bond.vesting[_vestingID];
    ...
    function pendingPayoutFor(address _depositor, uint256 _vestingID) external view returns (uint256 pendingPayout_) {
        uint256 percentVested = percentVestedFor(_depositor, _vestingID);
        uint256 payout = bondInfo[_depositor].payout[_vestingID];
...
interface ILiquidityTreasury {
    ...
    function deposit(uint256 _amountPayoutToken, uint256 _tokensForLiquidity) external;
