...
contract CoinFlip {
    ...
    function flip(uint256 betAmount, CoinFlipValues _coinFlipValues) external payable {
        ...
        require(msg.value >= betAmount + estimateFee + houseCutFee, 'Insufficient ETH being Sent');
        uint256 id = randomizer.request(callbackGasLimit);
        userToGames[msg.sender].push(id);
        coinFlipGames[id] = CoinFlipGame(msg.sender, betAmount, _coinFlipValues, defaultChoice, 0);
    ...
    function randomizerCallback(uint256 _id, bytes32 _value) external {
        ...
        address betAmountReceiver;
        uint256 amountToBeSent;
        if (game.prediction == result) {
            betAmountReceiver = game.player;
            amountToBeSent = game.betAmount * 2;
        } else {
            betAmountReceiver = bankRoll;
            amountToBeSent = game.betAmount;
        }
        (bool sent, bytes memory data) = payable(betAmountReceiver).call{value: amountToBeSent}('');
        string memory response = string(data);
        require(sent, response);
