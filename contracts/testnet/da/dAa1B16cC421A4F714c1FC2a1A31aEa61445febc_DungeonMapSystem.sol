...
/**
 * @title DungeonMapSystem
 */
contract DungeonMapSystem is
    IDungeonMapSystem,
    GameRegistryConsumerUpgradeable
{
    /** MEMBERS **/

    /// @notice DungeonMapEntity ➞ DungeonMap
    mapping(uint256 => DungeonMap) private dungeonMaps;

    /** EVENTS */

    /// @notice Emitted when a dungeon map has been updated
    event DungeonMapUpdated(uint256 indexed dungeonMapEntity);

    /// @notice Emitted when a dungeon map has been deleted
    event DungeonMapDeleted(uint256 indexed dungeonMapEntity);

    /// @notice Emitted when a dungeon node has been updated
    event DungeonNodeUpdated(
    ...
    /// @notice Emitted when a dungeon node has been deleted
    event DungeonNodeDeleted(
        uint256 indexed dungeonMapEntity,
        uint256 indexed nodeId
    );
    ...
    /// @notice Error when parameters for setting a new dungeon map are invalid
    error InvalidSetDungeonMapsParams();

    /// @notice Error when dungeon map does not exist
    error DungeonMapDoesNotExist();
    ...
    /**
     * @inheritdoc IDungeonMapSystem
     */
    function setDungeonMap(
        uint256 dungeonMapEntity,
        DungeonMap calldata dungeonMap
    ) external onlyRole(MANAGER_ROLE) {
        // Generate a dungeon entity from the dungeon token id
        if (dungeonMap.nodes.length == 0) {
            revert InvalidSetDungeonMapsParams();
        }
        _setDungeonMap(dungeonMapEntity, dungeonMap);
    }
    ...
    function addDungeonNode(
        uint256 dungeonMapEntity,
        DungeonNode calldata dungeonNode
    ) external onlyRole(MANAGER_ROLE) {
        _addDungeonNode(dungeonMapEntity, dungeonNode);
    }

    /**
     * @inheritdoc IDungeonMapSystem
     */
    function removeDungeonNode(
        uint256,
        uint256
    ) external view onlyRole(MANAGER_ROLE) {
        // Not implementing yet because it's unclear how the storage nodes
        // should behave when a node is removed. Infact, for now we simply
        // delete all nodes and rebuild them when updating with new map data.
    ...
    function getDungeonMap(
        ...
        return dungeonMaps[dungeonMapEntity];
    }
    ...
    function getDungeonNode(
        ...
        return dungeonMaps[dungeonMapEntity].nodes[node];
    }
    ...
    function getDungeonNodeEnemy(
        ...
        return dungeonMaps[dungeonMapEntity].nodes[node].enemies[enemyIndex];
    }
    ...
    function getDungeonNodeLoots(
        ...
        return dungeonMaps[dungeonMapEntity].nodes[node].loots;
    }
    ...
    function hasDungeonNodes(
        ...
        return dungeonMaps[dungeonMapEntity].nodes.length > 0;
    }

    /** INTERNAL **/

    function _setDungeonMap(
        uint256 dungeonMapEntity,
        DungeonMap calldata dungeonMap
    ) internal {
        // If dungeon map exists, delete it first.
        if (dungeonMaps[dungeonMapEntity].nodes.length > 0) {
            _deleteDungeonMap(dungeonMapEntity);
        }

        // Emit dungeon created event before nodes are created.
        emit DungeonMapUpdated(dungeonMapEntity);

        // Add each node in the dungeon map.
        DungeonNode calldata node;
        for (uint256 i = 0; i < dungeonMap.nodes.length; i++) {
            node = dungeonMap.nodes[i];
            _addDungeonNode(dungeonMapEntity, node);
        }
    }

    function _deleteDungeonMap(uint256 dungeonMapEntity) internal {
        // Error if dungeon map does not exist.
        if (dungeonMaps[dungeonMapEntity].nodes.length == 0) {
            revert DungeonMapDoesNotExist();
        }

        // Emit a node deleted event for each node in the dungeon map.
        // NOTE: node ID's are assumed to be sequential node indeces.
        for (
            uint256 i = 0;
            i < dungeonMaps[dungeonMapEntity].nodes.length;
            i++
        ) {
            emit DungeonNodeDeleted(dungeonMapEntity, i);
        }

        // Delete the entire dungeon map.
        delete dungeonMaps[dungeonMapEntity];

        // Emit dungeon deleted event after nodes are deleted.
        emit DungeonMapDeleted(dungeonMapEntity);
    }

    function _addDungeonNode(
        uint256 dungeonMapEntity,
        DungeonNode calldata dungeonNode
    ) internal {
        // Validate loots.
        _lootSystem().validateLoots(dungeonNode.loots);

        // Add node to dungeon map.
        dungeonMaps[dungeonMapEntity].nodes.push(dungeonNode);

        emit DungeonNodeUpdated(
            dungeonMapEntity,
            dungeonNode.nodeId,
            dungeonNode.enemies,
            dungeonNode.loots
