...
contract VaultUtils is IVaultUtils, Constants, Ownable {
    ...
    function validateConfirmDelay(
        ...
        if (transaction.status == 0) {
            if (
                block.timestamp >= (transaction.startTime + settingsManager.delayDeltaTime()) &&
                transaction.params[0] > 0
            ) validateFlag = true;
    ...
    function validateDecreasePosition(
        ...
        bool validateFlag;
        (bool hasProfit, ) = priceManager.getDelta(_indexToken, position.size, position.averagePrice, _isLong, _indexPrice);
    ...
    //     uint256 _indexPrice
    // ) external view override returns (bool) {
    //     OrderInfo memory order = positionKeeper.getOrder(_getPositionKey(_account, _indexToken, _isLong, _posId));
    //     uint256 price = _indexPrice == 0 ? priceManager.getLastPrice(_indexToken) : _indexPrice;
    //     uint256 stopPrice;
