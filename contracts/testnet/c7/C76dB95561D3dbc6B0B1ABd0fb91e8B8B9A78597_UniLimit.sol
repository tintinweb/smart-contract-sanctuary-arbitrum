...
contract UniLimit is IERC721Receiver {
    ...
    /// @dev Mint params for a unilimit position.
    struct MintParams {
        address token0; 
        address token1;
    ...
    /// Modifier which checks if new depositis are currently allowed
    ...
    /// @dev currently disabled functionality of opening a position with an existing uniswap liquidity NFT
    // function openPosition(uint256 tokenId, int24 sellTarget, bool sellAboveTarget, uint16 protocolFee) onlyDepositsActive external {
    //     require(protocolFee >= currentMinFee, 'Protocol fee set too low');
    //     uniNft.safeTransferFrom(msg.sender, address(this), tokenId);
    //     _storePositionInfo(tokenId, sellTarget, sellAboveTarget, msg.sender, protocolFee);
    //     emit AddPosition(tokenId, msg.sender);
    // }
    ...
    function _payOutPosition(uint256 tokenId, bool stoppedByProtocol) internal returns (bool success) {
        ...
        require(position.owner != address(0), 'Uniswap position with given token id is not managed by unilimit (no position found)');
        {
    ...
    // function getSellTarget(uint256 tokenId) external view returns(int24){
    //     return positionInfos[tokenId].sellTarget;
    // }
}
