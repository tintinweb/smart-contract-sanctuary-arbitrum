...
contract IntellCreatorNFTContract is
    ERC721Enumerable,
    ReentrancyGuard,
    ICreatorNFT
{
    ...
    mapping(uint256 => uint256) private _creatorNFTMintedHistory;
    ...
    function creatorNFTMintedHistory(uint256 _modelId)
        external
        view
        override
        returns (uint256)
    {
        return _creatorNFTMintedHistory[_modelId];
    }
    ...
    function adopt(bytes calldata message, bytes calldata signature)
        external
        nonReentrant
    {
        ...
        require(_VERIFIED_AS_CREATOR
            && address(this) == _CREATOR_NFT_CONTRACT_ADDR
            && !_USER_SUSPENDED
            && _MODEL_APPROVED
            && _MODEL_UPLOADED
            && !_MODEL_SUSPENDED
            && _MODEL_PROGRESS_STATUS > 3, "THIS IS REQUIRED VALID");
        require(
        ...
        _tokenIdByModelId[_MODEL_ID] = supply + 1;
        _modelIdByTokenId[supply + 1] = _MODEL_ID;

        _creatorNFTMintedHistory[_MODEL_ID] = block.number;
    ...
    function mintPrice() public view returns (uint256) {
        return _intellSetting.creatorNFTMintPrice();
    }
