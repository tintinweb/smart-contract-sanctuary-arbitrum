...
/**
 * @notice contract simplifies opening a short wPowerPerp position by selling wPowerPerp on uniswap v3 and returning wbtc to user
 */
contract ShortHelper is IERC721Receiver, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        require(_wbtcAddr != address(0), "Invalid wbtc address");
        ...
        IERC20 _wbtc = IERC20(_wbtcAddr);
        _wPowerPerp.approve(_swapRouter, type(uint256).max);
        _wbtc.approve(_swapRouter, type(uint256).max);
    ...
    /**
     * @notice mint power perp, trade with uniswap v3 and send back premium in wbtc
    ...
    function openShort(
        ...
        require(
            _exactInputParams.tokenOut == address(wbtc) && _exactInputParams.tokenIn == wPowerPerp,
            "Wrong swap tokens"
        );
        wbtc.transferFrom(msg.sender,address(this), _wbtcAmount);        
        ...
        // if the recipient is this address: unwrap wbtc and send back to msg.sender
        if (_exactInputParams.recipient == address(this)) {
            // wbtc.withdraw(amountOut);
            // payable(msg.sender).sendValue(amountOut);
            wbtc.transfer(msg.sender, amountOut);
        }
    ...
    /**
     * @notice buy back wPowerPerp with wbtc on uniswap v3 and close position
     * @param _vaultId short wPowerPerp vault id
    ...
    function closeShort(
        ...
        require(
            _exactOutputParams.tokenOut == wPowerPerp && _exactOutputParams.tokenIn == address(wbtc),
            "Wrong swap tokens"
        );
        ...
        // pay wbtc and get wPowerPerp in return.
    ...
    /**
     * @dev only receive balance from wbtc contract and controller.
     */
    receive() external payable {
        require(msg.sender == address(wbtc) || msg.sender == address(controller), "can't receive balance");
    }
    ...
    function Approve(address _controller, uint256 _amount) external onlyOwner{
       IERC20(wbtc).approve(_controller, _amount);
    }
}