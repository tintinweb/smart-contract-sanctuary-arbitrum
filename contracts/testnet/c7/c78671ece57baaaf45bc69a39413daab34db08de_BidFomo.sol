...
contract BidFomo is Caller, ReentrancyGuard {
    ...
    function start(uint256 _initialPrice) external onlyManger {
        paused = false;
    ...
    function trade(uint8 source, uint256 usd) external onlyCaller {
    ...
    function _claimReward() internal nonReentrant {
        ...
        if (reward > 0) {
            // send eth
            SafeTransfer.unwrapWETH9(WETH, pool.bidder, reward);
        }
    }
