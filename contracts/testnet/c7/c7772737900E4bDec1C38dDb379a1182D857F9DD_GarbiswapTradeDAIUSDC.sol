...
contract GarbiswapTradeDAIUSDC is ERC20Burnable, Ownable {
    ...
    function convertDecimal6to18(uint256 number) public view returns (uint256) { 
        number = number.mul(1e18).div(6);
        return number;
    }

    function convertDecimal18to6(uint256 number) public view returns (uint256) { 
        number = number.mul(6).div(1e18);
        return number;
    }
}
