...
interface ICrowdsale {
    ...
    function preSaleInfoReturn(address account) external view returns (LaunchpadStructs.ReturnInfo memory result);
...
contract CrowdSaleAYAToken is ICrowdsale, Ownable, Pausable, ReentrancyGuard {
     ...
     */
    function getVestingWallets(address beneficiary, address[] memory vestingManagers) public view virtual override returns (address[] memory) {
     ...
     *  返回合约信息
     */
    function preSaleInfoReturn(address account) external view virtual override returns (LaunchpadStructs.ReturnInfo memory result) {
        result.price = price;
        result.raise = raise;
        ...
        result.userVestingWallets = getVestingWallets(account, result.vestingManagers);
