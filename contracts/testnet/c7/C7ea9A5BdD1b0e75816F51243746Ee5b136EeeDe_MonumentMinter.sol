...
contract MonumentMinter is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  // Uintss
  //=======================================
  uint256 public collectibleCost = 10;
  uint256 public maxEntities = 7;
  uint256 public buildHours = 24;
  ...
  function mint(
    ...
    // TODO: check if realm has earthen geo features
    ...
    for (uint256 j = 0; j < _entityIds.length; j++) {
      ...
      uint256 collectibleAmount = collectibleCost * desiredQuantity;
    ...
    // Build
    TIMER.build(_realmId, totalQuantity * buildHours);
  }
  ...
  function updateCollectibleCost(uint256 _collectibleCost) external onlyAdmin {
    collectibleCost = _collectibleCost;
  }
  ...
  function updateBuildHours(uint256 _buildHours) external onlyAdmin {
    buildHours = _buildHours;
  }
