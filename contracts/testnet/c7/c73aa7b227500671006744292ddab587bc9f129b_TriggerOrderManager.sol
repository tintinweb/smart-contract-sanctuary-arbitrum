...
contract TriggerOrderManager is ITriggerOrderManager, BasePosition, ReentrancyGuard {
    ...
    function triggerPosition(
        ...
        if (position.size == 0) {
            ...
            bool isValidTxType = txType == CREATE_POSITION_LIMIT || txType == CREATE_POSITION_STOP_MARKET
            ...
            require(triggerOrders[key].key == key, "Trigger not exist");
            path = positionKeeper.getPositionFinalPath(key);
            txType = TRIGGER_POSITION;
        }

        require(path.length > 0, "Invalid pathLength");
        (isFastExecute, prices) = _getPricesAndCheckFastExecute(path);
        require(path.length == prices.length, "Invalid params/prices length");
