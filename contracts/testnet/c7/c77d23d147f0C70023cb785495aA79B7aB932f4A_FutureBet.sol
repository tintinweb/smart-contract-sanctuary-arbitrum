/**
 *Submitted for verification at Arbiscan.io on 2023-10-03
*/
...
contract FutureBet is Ownable {
    ...
    constructor() {
        token = IERC20(0x826153C6F9c2c2EeF5dcCbfE5E48d95d914E4892);
        projectFeeAddress = 0xf01ACd54082dD7A0180a0bF74e38179b7012DF7c;
    ...
    function placeBet(uint256 _round,uint256 _quest_no,uint256 _amount,uint256 _betOption) public isPausable {
        ...
        require(subQuestionLocked[_round][_quest_no] == false,"Question is locked");
    ...
    function announceResult(uint256 _round,uint256 _quest_no,uint256 _result) external onlyAdmin {
        ...
        require(subQuestionLocked[_round][_quest_no] == true,"Question is not locked");
    ...
    function addQuestion(
        ...
        roundStartTime[totalGameRound] = (_roundStartTime * 1 hours) + block.timestamp;
        roundPoolSize[totalGameRound] = 0;

        
    }

    function addSubQuestion(uint256 _round,string memory _question, string [] memory _subQuestionOptionNames) public onlyAdmin {
   ...
   function updateQuestion(
        ...
        if (_roundStartTime >0) {
            roundStartTime[_round] = (_roundStartTime * 1 hours) + block.timestamp;
        }
        
    }

    // update question
    function updateSubQuestion(uint256 _round,uint256 _quest_no, string memory _question,string[] memory _subQuestionOptionNames ) public onlyAdmin {
        require(_round <= totalGameRound,"Round does not exist");
        require(_quest_no <= subQuestionCount[_round],"Question does not exist");
        require(roundCanceled[_round] == false,"Round is canceled");
        require(_subQuestionOptionNames.length >= 2 && _subQuestionOptionNames.length <=3,"Invalid option names");
