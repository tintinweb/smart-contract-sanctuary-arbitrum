...
abstract contract GambitTradingV1Facet3 is GambitTradingV1Base {
    ...
    function removeCollateral(
        ...
        uint fee = storageT.getDevGovFees(
            pairIndex,
            (t.positionSizeUsdc * t.leverage) / 1e18,
            false
        );
        if (newPositionSizeUsdc <= fee) revert GambitErrorsV1.BelowFee();
        newPositionSizeUsdc -= fee;
    ...
    function executeNftOrder(
        ...
        uint nftId,
        uint nftType,
        bytes[] calldata priceUpdataData
        ...
        if (
            ...
            (uint256 price, , bool success) = storageT
                .priceAggregator()
                .fulfill{value: address(this).balance}( // transfer all ETH
                orderId,
                priceUpdataData
            );
