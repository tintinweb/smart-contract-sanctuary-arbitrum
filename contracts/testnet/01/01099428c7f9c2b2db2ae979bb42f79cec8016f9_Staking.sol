...
interface IRaidHandler {
    function raidPool(uint256 tokenId, address raider, uint256 numOfStakers, uint256 localBuds, uint256 globalBuds) external returns (uint256 requestId);
}
...
contract Staking is  UUPSUpgradeable, AutomationCompatible, IERC721Receiver, CCIPReceiverUpgradable, OwnableUpgradeable{
    ...
    function initialize(
        address _router,
        address[5] memory tokenAddreses,
        uint256 _baseAPR
    ) public initializer{
        _budsToken = IBudsToken(tokenAddreses[0]);
        _farmerToken = IFarmer(tokenAddreses[1]);
        _stonerToken = IBoosters(tokenAddreses[2]);
        _informantToken = IBoosters(tokenAddreses[3]);
        _narcToken = IFarmer(tokenAddreses[4]);
        router = IRouterClient(_router);
        CCIP_RECEIVER_INIT(_router);
        __Ownable_init();
        CROSS_CHAIN_RAID_MESSAGE = keccak256("CROSS_CHAIN_RAID");
        CROSS_CHAIN_STAKE_MESSAGE = keccak256("CROSS_CHAIN_STAKE_MESSAGE");
        baseAPR = _baseAPR;
        noOfChains=4;
        globalStakedBudsCount=42000000*4;
    }
...
//--------------------------------------------------STAKING FUNCTION-----------------------------------------------//
    ...
    function distributeRaidingRewards(address to, uint256 rewardAmount) internal{
        ...
        _budsToken.burn((rewardAmount / 1000)/1000);
        _budsToken.transfer(to, rewardAmount - ((rewardAmount / 1000)/1000));
    }

    function raid(uint256 tokenId) public {
        require(_narcToken.balanceOf(msg.sender) != 0, "For narcs");
        uint256 res = IRaidHandler(raidHandler).raidPool(tokenId, msg.sender, stakerAddresses.length, localStakedBudsCount, globalStakedBudsCount);
        if(tokenId != 0){
    ...
    function finalizeRaid(address raider, bool isSuccess, bool isboosted, uint256 _boosts) external{
        ...
        if(isSuccess){
            distributeRaidingRewards(raider,_budsToken.balanceOf(address(this)));
    ...
    function crossChainStake(uint256 _budsAmount, uint256 _farmerTokenId, uint64 _destinationChainSelector) external payable{
        ...
        override
    {
        ...
        if(messageType == CROSS_CHAIN_STAKE_MESSAGE){
            ...
            uint256 res = IRaidHandler(raidHandler).raidPool(tokenId, sender, stakerAddresses.length, localStakedBudsCount,globalStakedBudsCount);
        }else{
            revert("Invalid msg");
        }
    }
        ...
        return
            Client.EVM2AnyMessage({
                ...
                extraArgs: Client._argsToBytes(
                    Client.EVMExtraArgsV1({gasLimit: 1500_000, strict: false})
                ),
...
abstract contract CCIPReceiverUpgradable is IAny2EVMMessageReceiver, IERC165, Initializable {
  address internal i_router;


  function CCIP_RECEIVER_INIT(address router) internal onlyInitializing{
