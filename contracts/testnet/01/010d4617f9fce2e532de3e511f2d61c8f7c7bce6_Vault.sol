...
contract Vault is ReentrancyGuard, AccessControlBase, IVault {
	...
	function payout(
		...
		(, uint256 wagerFeeCharged_) = _collectWagerFees(
			_wagerAsset,
			_escrowAmount
		);
	 ...
	 */
	function setCircuitBreakerAmount(address _token, uint256 _amount) external onlyTeam {
	 ...
	 */
	function setCircuitBreakerEnabled(bool _setting) external onlyTeam {
