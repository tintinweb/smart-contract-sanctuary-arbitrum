...
contract SynthrIssuer is Owned, MixinSystemSettings, IIssuer {
    ...
    function _debtBalanceOfAndTotalDebt(
        ...
        bytes32 currencyKey
    ) internal view returns (uint256 debtBalance, uint256 totalSystemValue, bool anyRateIsInvalid) {
    ...
    function _remainingIssuableSynths(
        address _issuer
    ) internal view returns (uint256 maxIssuable, uint256 alreadyIssued, uint256 totalSystemDebt, bool anyRateIsInvalid) {
    ...
    ) external view returns (uint256 totalRedeemed, uint256 amountToLiquidate, bool removeFlag) {
    ...
    function liquidateAccount(
        ...
        external
        onlySynthrBridgeOrWrappedSynthr
        returns (uint256 totalRedeemed, uint256 amountToLiquidate, uint256 sharesToRemove)
    {
    ...
    function destIssue(address _account, bytes32 _synthKey, uint256 _synthAmount) external onlySynthrBridgeOrWrappedSynthr {
    ...
    function _burnSynths(
        ...
        bytes32 synthKey,
        uint256 amount
    ) internal returns (uint256 amountBurnt, uint256 debtShareBurnt, uint256 amountBurntInUSD) {
    ...
    function _voluntaryBurnSynths(
        ...
        uint256 amount,
        bool burnToTarget
    ) internal returns (uint256 synthAmount, uint256 debtShare, uint256 reclaimed, uint256 refunded) {
        // check breaker
        if (!_verifyCircuitBreaker()) {
            return (0, 0, 0, 0);
        }
