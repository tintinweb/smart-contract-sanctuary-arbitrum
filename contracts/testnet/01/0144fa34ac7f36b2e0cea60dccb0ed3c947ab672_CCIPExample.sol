...
import { IERC165 } from './interfaces/ccip/IERC165.sol';

contract CCIPExample is
  ...
  OwnableUpgradeable
{
  ...
  modifier validChain(uint64 chainId) {
    require(supportedChains[chainId].length > 0, 'VCHAIN');
    _;
  }

  modifier onlyRouter() {
    require(msg.sender != address(ccipRouter), 'ONLYROUTER');
    _;
  }

  function initialize(IRouterClient _router, IERC20 _feeToken)
    public
    initializer
  {
    __Ownable_init();
    ccipRouter = _router;
    feeToken = _feeToken;
    feeToken.approve(address(ccipRouter), 2**256 - 1);
  }

  function enableChain(uint64 chainId, uint256 gasLimit) external onlyOwner {
    require(gasLimit > 0, 'INVALID');
    supportedChains[chainId] = getExtraArgs(gasLimit);
  }
    ...
    validChain(message.sourceChainId)
  {
    ...
    emit MessageReceived(message.messageId, sender);
  }

  function getExtraArgs(uint256 gasLimit) public pure returns (bytes memory) {
  ...
  function getMessageAndFee(
    ...
    Client.EVM2AnyMessage memory message = Client.EVM2AnyMessage({
      ...
      extraArgs: supportedChains[destChainId],
      feeToken: address(0) // We leave the feeToken empty indicating we'll pay raw native.
    });
    return (message, ccipRouter.getFee(destChainId, message));
  }
  ...
  function sendDataPayNative(
    ...
    (Client.EVM2AnyMessage memory message, uint256 fee) = getMessageAndFee(
      destChainId,
      receiver,
      data
    );
    bytes32 messageId = ccipRouter.ccipSend{ value: fee }(destChainId, message);
    uint256 _bal = address(this).balance;
    if (_bal > 0) {
      payable(_msgSender()).call{ value: _bal }('');
    }
...
interface IRouterClient {
  ...
  function isChainSupported(uint64 chainId)
    external
    view
  ...
  function getSupportedTokens(uint64 chainId)
    external
    view
