...
contract UnipDistributor is ReentrancyGuard, Ownable {
    ...
    uint256 public constant MAX_ADDRESSES = 80_000;
    ...
    uint256 public lpClaimedAmount = 0;
    uint256 public rushClaimedAmount = 0;
    ...
    function calcClaimableAmount() public view returns(uint256) {
        if (claimableTokens[_msgSender()] > 0) {
            return claimableTokens[_msgSender()];
        }
    ...
    function claim(uint128 nonce, bytes calldata signature, address referrer) public nonReentrant {
        ...
        if (claimableTokens[_msgSender()] > 0) {
            ...
            require(amount >= ULP_PRECISION, "Unip: airdrop has ended");
    ...
    function isValidSignature(uint128 nonce, bytes memory signature) view internal returns (bool) {
