...
contract MSWNFT is Ownable, ERC721Enumerable {
    using Address for address;
    using Strings for uint256;
    ...
    // constructor
    constructor() ERC721("MSWNFT", "MSW-NFT") {
    ...
    // mint
    function mint(address to_, uint cardId_, uint amount_) public {
        require(cardInfoes[cardId_].cardId != 0, "MSW: wrong cardId");
        // require(
        //     cardInfoes[cardId_].currentAmount + amount_ <=
        //         cardInfoes[cardId_].maxAmount,
        //     "MSW: amount out of max"
        // );
        require(
    ...
    // upgrade
    function upgrade(uint tokenId_, uint toCardId_) public {
        ...
        require(cardInfoes[_oldCardId].cardId < toCardId_, "MSW: wrong lv");

        uint diff = cardInfoes[toCardId_].price - cardInfoes[_oldCardId].price;
        cardIdMap[tokenId_] = toCardId_;

        cardInfoes[_oldCardId].currentAmount--;
        cardInfoes[toCardId_].currentAmount++;
