...
contract RankedRewardsClaim is IClaimable, GameRegistryConsumerUpgradeable {
        ...
        onlyRole(GAME_LOGIC_CONTRACT_ROLE)
    {
        ...
        ILootSystem.Loot[] memory loots = rewardsForPlacement[claim][placement];
        ILootSystem lootSystem = ILootSystem(_getSystem(LOOT_SYSTEM_ID));
        lootSystem.grantLoot(account, loots);
    }

    function getLoot(uint256 claim, uint256 placement)
        external
        view
        returns (ILootSystem.Loot[] memory)
    {
        return rewardsForPlacement[claim][placement];
    }

    function getPlacement(address account, uint256 claim)
        external
        view
        returns (uint256)
    {
    ...
    function setLoots(uint256 claim, ILootSystem.Loot[][] memory newLoots)
        external
    ...
    function setWinners(address[] calldata newOrderedWinners, uint256 claim)
        external
        onlyRole(MANAGER_ROLE)
    {
        ...
        for (uint256 idx; idx < localHighestPlacement; ++idx) {
            ...
            placementForWinner[claim][winner] = idx;
        }
    }
}
