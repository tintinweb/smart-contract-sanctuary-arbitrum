...
import "./library/TWAddress.sol";
import "./library/Context.sol";
import "./library/TWStrings.sol";
...
import {ERC721A} from "./ERC721AVirtualApprove.sol";

import "./library/ContractMetadata.sol";
import "./library/Multicall.sol";
import "./library/Ownable.sol";
import "./library/Royalty.sol";
import "./library/BatchMintMetadata.sol";
import "./library/DefaultOperatorFilterer.sol";
...
import "./library/PrimarySale.sol";
import "./library/PermissionsEnumerable.sol";
...
/// @author thirdweb & munji - [email protected]
...
interface ISignatureMintERC721 {
    ...
    struct UpdateUriRequest {
        uint256 tokenId;
        string uri;
        uint256 pricePerToken; //price per update
        address currency;
    ...
    event UriUpdatedWithSignature(
        address indexed signer,
        uint256 indexed tokenId,
        UpdateUriRequest updateRequest
    );
    ...
    struct BirthRequest {
        ...
        uint256 dividendsPoint;
        uint256 matingId;
        bytes32 uid;
    }

    event BirthWithSignature(
        address indexed signer,
        address indexed mintedTo,
        uint256 indexed tokenIdMinted,
        BirthRequest birthRequest
    );
...
import {NFTStatus, ItemEffects, MatingStatus} from "../NFTManager/StatusStructs.sol";
import {INftManager} from "../NFTManager/INftManager.sol";

/// @author munji - [email protected]
contract Pachi721V9 is ERC721SignatureMint {
    /** =================================================================================================
        Setup
        =================================================================================================
     */
    ...
    modifier onlyNFTManagerOrOwner() {
        require(
            msg.sender == address(nftManager) || msg.sender == owner(),
            "only NFT manager"
        );
        _;
    }
    ...
    function updateUriWithSignature(
        UpdateUriRequest calldata _req,
        bytes calldata _signature
    ) external {
        address signer = processUpdateRequest(_req, _signature);
        ...
        // IERC20(_req.currency).burn(_req.pricePerToken);

        require(bytes(_req.uri).length > 0, "empty uri.");
        _setTokenURI(_req.tokenId, _req.uri);
        emit UriUpdatedWithSignature(signer, _req.tokenId, _req);
    }

    /** =================================================================================================
        Get Birth (Mating Logic)
        =================================================================================================
     */

    function birthWithSignature(
        BirthRequest calldata _req,
        bytes calldata _signature
    ) external onlyNFTManagerOrOwner {
        uint256 tokenIdToMint = nextTokenIdToMint();

        // Verify and process payload.
        address signer = processBirthRequest(_req, _signature);

        address receiver = _req.to;

        // Collect price
        _collectPriceOnClaim(owner(), 1, _req.currency, _req.pricePerToken);
        ...
        // Finalize Mating Process
        nftManager.finalizeMating(_req.matingId);

        // Mint tokens
        _setTokenURI(tokenIdToMint, _req.uri);
        _safeMint(receiver, 1);

        nftManager.initiateNFTStatus(
            tokenIdToMint,
            0,
            false,
            [_req.parent1, _req.parent2],
            _req.jackpotBoost,
            _req.dividendsPoint
        );

        emit BirthWithSignature(signer, receiver, tokenIdToMint, _req);
    }
    ...
    function _beforeTokenTransfers(
        ...
        // isMating, isSupporter should be false
        for (uint256 i = 0; i < quantity; i++) {
            uint256 tokenId = startTokenId + i;
            require(
                nftManager.isNftTransferable(tokenId),
                "is mating or supporter or pending"
            );
        }
    }
...
/// @author thirdweb & @munj1 - [email protected]

import "./ISignatureMintERC721.sol";
...
abstract contract SignatureMintERC721 is EIP712, ISignatureMintERC721 {
    ...
    bytes32 private constant UPDATE_TYPEHASH =
        keccak256(
            "UpdateUriRequest(uint256 tokenId,string uri,uint256 pricePerToken,address currency,uint128 validityStartTimestamp,uint128 validityEndTimestamp,bytes32 uid)"
        );
    mapping(bytes32 => bool) private updated;

    function verifyUpdate(
        UpdateUriRequest calldata _req,
        bytes calldata _signature
    ) public view returns (bool success, address signer) {
        signer = recoverAddressForUpdate(_req, _signature);
        success = !updated[_req.uid] && _canSignMintRequest(signer); // & hasRole(MINTER_ROLE, signer), signer)
    }
    ...
    function processUpdateRequest(
        ...
        bool success;
        (success, signer) = verifyUpdate(_req, _signature);
        ...
        updated[_req.uid] = true;
    }

    /// @dev Returns the address of the signer of the mint request.
    function recoverAddressForUpdate(
        UpdateUriRequest calldata _req,
        bytes calldata _signature
    ) internal view returns (address) {
        return
            _hashTypedDataV4(keccak256(encodeRequestForUpdate(_req))).recover(
                _signature
            );
    }

    function encodeRequestForUpdate(
        UpdateUriRequest calldata _req
    ...
    /**
        Give Birth Logic
     */
    bytes32 private constant BIRTH_TYPEHASH =
        keccak256(
            "BirthRequest(address to,string uri,uint256 pricePerToken,address currency,uint128 validityStartTimestamp,uint128 validityEndTimestamp,uint256 parent1,uint256 parent2,uint256 generation,uint256 jackpotBoost,uint256 dividendsPoint,uint256 matingId,bytes32 uid)"
        );

    mapping(bytes32 => bool) private birthed;

    function verifyBirth(
        BirthRequest calldata _req,
        bytes calldata _signature
    ) public view returns (bool success, address signer) {
        signer = recoverAddressForBirth(_req, _signature);
        success = !birthed[_req.uid] && _canSignMintRequest(signer); // & hasRole(MINTER_ROLE, signer), signer)
    }

    /// @dev Verifies a mint request and marks the request as minted.
    function processBirthRequest(
        BirthRequest calldata _req,
        bytes calldata _signature
    ) internal returns (address signer) {
        bool success;
        (success, signer) = verifyBirth(_req, _signature);
        require(success, "invalid signature");
        require(
            _req.validityStartTimestamp <= block.timestamp &&
                _req.validityEndTimestamp >= block.timestamp,
        ...
        birthed[_req.uid] = true;
    }

    /// @dev Returns the address of the signer of the mint request.
    function recoverAddressForBirth(
        BirthRequest calldata _req,
        bytes calldata _signature
    ) internal view returns (address) {
        return
            _hashTypedDataV4(keccak256(encodeRequestForBirth(_req))).recover(
                _signature
            );
    }

    function encodeRequestForBirth(
        BirthRequest calldata _req
    ) internal pure returns (bytes memory) {
        return
            abi.encode(
...
abstract contract OperatorFilterToggle is IOperatorFilterToggle {
    ...
    function setOperatorRestriction(bool _restriction) external {
        require(
            _canSetOperatorRestriction(),
            "Not authorized to set operator restriction."
        );
...
interface INftManager {
    ...
    function isNftPlayable(uint256 tokenId) external view returns (bool);
