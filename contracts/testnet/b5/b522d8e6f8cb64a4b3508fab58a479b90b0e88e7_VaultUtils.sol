...
contract VaultUtils is IVaultUtils, BasePositionConstants, Constants, Ownable {
    ...
    function validateLiquidation(
        ...
        if (_position.averagePrice > 0) {
            ...
            int256 previousFee = _position.previousFee <= 0 ? int256(0) : _position.previousFee;
            _position.previousFee = int256(0);
            //Remove the bonus fee (if existed) due to liquidation
            (bool hasProfit, uint256 delta, int256 fee) = calculatePnl(
                _indexPrice,
                true,
                false,
                _position
            ...
            fee += previousFee;
            require(fee >= 0, "Fee conflicted"); //Should not happen
            ...
            //Charge previous fee if previous fee is positive
            if (_position.previousFee > 0) {
                fee += _position.previousFee;
            }

            return _checkMaxThreshold(remainingCollateral, _position.size, uint256(fee), _position.indexToken, _raise);
        } else {
    ...
    function _validateAddCollateral(
        ...
        uint256 borrowFee = settingsManager.getBorrowFee(_indexToken, _positionSize - _positionCollateral, _lastIncreasedTime);
        _validateSizeCollateralAmount(_positionSize, _positionCollateral + amountInUSD);
        return (amountInUSD, borrowFee);
    }
    ...
    function validateRemoveCollateral(
        ...
        _isValidContract(address(positionKeeper));
        return validateRemoveCollateral(
            _amountIn, 
            _indexPrice,
    ...
    function validateRemoveCollateral(
        ...
        require(_amountIn <= _position.reserveAmount && _amountIn <= _position.collateral, "Insufficient reserved collateral");
        //Calculate borrow fee for this loanDelta
        ...
        if (_position.previousFee > 0) {
            require(_position.collateral >= uint256(_position.previousFee), "Fee exceeded positionCollateral");
        }
    ...
    function beforeDecreasePosition(
        ...
        (hasProfit, encodedData) = beforeDecreasePosition(
            _collateralToken,
            _sizeDelta,
            _indexPrice,
            position
        ...
        int256 fee;
        uint256[3] memory posData;
        (fee, posData, position) = abi.decode(encodedData, ((int256), (uint256[3]), (Position)));
        return (hasProfit, fee, posData, position);
    }
    ...
    function beforeDecreasePosition(
...

        {
            ...
            (hasProfit, fee, posData) = _reduceCollateral(
                _sizeDelta, 
                _indexPrice, 
                _position
            );
        ...
        return (hasProfit, abi.encode(fee, posData, _position));
    }
    ...
    function _reduceCollateral(
        ...
        uint256 _indexPrice, 
        Position memory _position
    ) internal view returns (bool, int256, uint256[3] memory) {
        bool hasProfit;
        ...
        //Scope to avoid stack too deep error
        {
            ...
            fee = _calculateFees(_sizeDelta, 0, true, true, _position);
        ...
        if (fee > 0 && usdOut < uint256(fee)) {
            require(uint256(fee) <= _position.collateral, "Insufficient position collateral to deduct fee");
            _position.collateral -= uint256(fee);
        }

        _validateDecreasePosition(true, _indexPrice, _position);
        return (hasProfit, fee, [usdOut, collateralDelta, adjustedDelta]);
    }
    ...
    function calculatePnl(
        ...
        _isValidPosition(_position);
        bool isPriceUpper = _indexPrice >= _position.averagePrice;
        uint256 priceDiff = isPriceUpper ? _indexPrice - _position.averagePrice 
            : _position.averagePrice - _indexPrice;
        int256 multiplier = _position.isLong ? (isPriceUpper ? int256(1) : int256(-1)) 
            : (isPriceUpper ? int256(-1) : int256(1));
        int pnl = multiplier * int256((_position.size * priceDiff) / _position.averagePrice);
        int previousFee = _position.previousFee;
        _position.previousFee = 0;
        int fee = previousFee + _calculateFees(0, 0, _isApplyBorrowFee, _isApplyFundingFee, _position);
        pnl -= int256(fee);
        return pnl > 0 ? (true, uint256(pnl), fee) 
            : (false, uint256(-1 * pnl), fee);
    }
    ...
    function _calculateFees(
        ...
        return settingsManager.getFees(
            _sizeDelta == 0 ? _position.size : _sizeDelta,
            _loanDelta == 0 ? _position.size - _position.collateral : _loanDelta,
            _isApplyBorrowFee, 
            _isApplyFundingFee,
            _position
        );
    ...
    function _isValidPosition(Position memory _position) internal pure {
        require(_position.owner != address(0) && _position.size > 0 && _position.averagePrice > 0, "Position notExist");
    }
}
