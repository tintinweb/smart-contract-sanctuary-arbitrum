...
contract StakedVEIN is ERC4626 {
    ...
    address public vein;
    address private controller; //for admin functions
    ...
    constructor(
        address asset_,
        address _controller
    ) ERC4626(ERC20(asset_), "Staked VEIN", "sVEIN", _controller) {
        vein = asset_;
        controller = _controller;
    }
    ...
    function transfer(
        ...
        if (msg.sender != vein && recipient_ != vein && recipient_ != controller)
            revert OnlyTransfersWithVein();
    ...
    function transferFrom(
        ...
        if (sender_ != vein && recipient_ != vein && recipient_ != controller)
            revert OnlyTransfersWithVein();
    ...
    function afterDeposit(uint256 assets_, uint256 shares_) internal override {
        ...
        if (msg.sender != vein) VEIN(vein).rebalanceFloor();
    }
    ...
    function beforeWithdraw(
        ...
        VEIN(vein).rebalanceFloor();
    }

    function totalAssets() public view override returns (uint256) {
        return VEIN(vein).balanceOf(address(this));
    }
}
...
contract VEIN is FloorToken, TransferTripleTaxToken{
    ...
    function _deployFloorLiquidity(uint256 amountEth_) internal {
        ...
        deltaIds[0] = int256(int24(floorId) - int24(activeBin));
        distributionX[0] = 0;
    ...
    //address(this).approve(address(_sVault), type(uint256).max);
}