...
contract WartlocksHallow is Initializable, WartlocksHallowSettings {
    ...
    contractsAreSet
    {
        ...
        require(bugz.balanceOf(msg.sender) > minimumBugzToStake, "WartlocksHallow: Not enough bugz in wallet");
    ...
    contractsAreSet
    {
        ...
        for(uint256 i = 0; i < _burnItemParams.length; i++) {
            ...
            require(collectionToItemIdToIsBurnable[_params.collection][_params.itemId], "Not burnable");
            require(_params.amount > 0, "Bad amount");

            if(_params.collection == BurnableCollection.ITEMZ) {
                itemz.burn(msg.sender, _params.itemId, _params.amount);
            } else { // Consumables. Can't burn since not admin.
                consumables.safeTransferFrom(msg.sender, address(this), _params.itemId, _params.amount, "");
            }
...
abstract contract WartlocksHallowContracts is Initializable, WartlocksHallowState {
    ...
    onlyAdminOrOwner
    {
        ...
        consumables = IERC1155Upgradeable(_consumablesAddress);
...
abstract contract WartlocksHallowState is Initializable, IWartlocksHallow, ERC1155HolderUpgradeable, AdminableUpgradeable {
    ...
    event BurnableItemsAdded(BurnableCollection burnableCollection, uint128[] itemIds);
    ...
    IItemz public itemz;
    IBugz public bugz;
    IERC1155Upgradeable public consumables;
    ...
    mapping(BurnableCollection => mapping(uint128 => bool)) public collectionToItemIdToIsBurnable;
    ...
    function __WartlocksHallowState_init() internal initializer {
        ...
        uint128[] memory _burnableItemz = new uint128[](8);
        _burnableItemz[0] = 3;
        _burnableItemz[1] = 4;
        _burnableItemz[2] = 5;
        _burnableItemz[3] = 6;
        ...
        for(uint256 i = 0; i < _burnableItemz.length; i++) {
            collectionToItemIdToIsBurnable[BurnableCollection.ITEMZ][_burnableItemz[i]] = true;
        }
        emit BurnableItemsAdded(BurnableCollection.ITEMZ, _burnableItemz);

        uint128[] memory _burnableConsumables = new uint128[](2);
        _burnableConsumables[0] = 8;
        _burnableConsumables[1] = 9;

         for(uint256 i = 0; i < _burnableConsumables.length; i++) {
            collectionToItemIdToIsBurnable[BurnableCollection.CONSUMABLES][_burnableConsumables[i]] = true;
        }
        emit BurnableItemsAdded(BurnableCollection.CONSUMABLES, _burnableConsumables);
    }
}

enum BurnableCollection {
    ITEMZ,
    CONSUMABLES
}
