...
// Interface for Signing & Verification
contract ISignatureVerifier {
  function verify( address _signer, uint256 _nonce, bytes memory signature) public pure returns (bool) {}
} 
...
contract EllerianHero is ERC721 {
  ...
  bool areTransfersAllowed;                         // Restricts transfer of heroes to only authorized addresses.
  ...
  */
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    return uriAbi.GetTokenUri(upgradeableAbi.GetHeroClass(tokenId), upgradeableAbi.GetAttributeRarity(tokenId));
  }
  ...
  /*
   * Allows the owner to set if transfers are allowed.
   */
  function SetTransfersAllowed(bool _allow) external onlyOwner {
    areTransfersAllowed = _allow;
  }
  ...
  */ 
  ...
  function safeTransferFrom (address _from, address _to, uint256 _tokenId, bytes memory _data) public override {
    ...
    if (_approvedAddresses[_from] || _approvedAddresses[_to]) {
      ...
      upgradeableAbi.ResetHeroExperience(_tokenId);

      if (!areTransfersAllowed) {
        _to = ownerAddress; // Jail illegal transfers.
      }
    }
