...
abstract contract BarnMintable is Initializable, BarnContracts {
    ...
    function getRanNumberDis() external view returns (uint256[] memory){
        uint256[] memory ret = new uint256[](ranNumCount);
        for (uint i = 0; i < ranNumCount; i++) {
            ret[i] = randomNums[i];
        }
        return ret;
    }

    function generateRandomNumber(uint256 _tokenId) private returns (uint256) {
        // uint256 requestId = randomizer.requestRandomNumber();
        uint256 requestId = committed;
        committed++;
        //TODO: Probably store tokenId => requestId mapping
        return randomizer.isRandomReady(requestId) ? randomizer.revealRandomNumber(requestId) : generateLocalRandomNumber(requestId);
    }

    function generateLocalRandomNumber(uint256 _requestId) private returns (uint256) {
        //TODO: Write fallback method. Defaulting to 99999 for least-rare barn
        return 99999;
    }
	...
	AdminableUpgradeable
{
    ...
    mapping(uint256 => uint256) public randomNums;
    uint256 ranNumCount;
    // uint256 public existingGenNums;
