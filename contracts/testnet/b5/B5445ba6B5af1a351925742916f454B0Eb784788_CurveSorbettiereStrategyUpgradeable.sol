    ...
    IParallaxStrategy
{
     ...
     *                    PARALLAX - address of the main contract
     *                    that controls all strategies in the system.
     *                    SORBETTIERE - address of the
     *                    Sorbettiere's staking smart-contract.
     ...
     *                    smart-contract used in the strategy for exchanges.
     ...
     *                    (MIM3CRV-f, Curve.fi Factory USD Metapool: MIM)
     ...
     *                    all exchange transactions in this strategy are valid.
     *                    If time elapsed, exchange and transaction will fail.
     ...
     *          Can only be called by the Parallax contact.
     *  @param newCompoundMinAmount (uint256) - a value in SPELL token after which compound must be executed.
     */
    ...
    function _deposit(DepositParams memory params) private returns (uint256) {
        // Add liquidity to the Curve's USDC/USDT
        // and MIM/USDC-USDT liquidity pools
    ...
    function _withdraw(
        ...
        // Withdraw (unstake) Curve's MIM/USDC-USDT LP tokens
        // from the Sorbettiere staking pool
        _sorbettiereWithdraw(params.amount);

        // Remove liquidity from the Curve's MIM/USDC-USDT
        // and USDC/USDT liquidity pools
    ...
    function _getAmountOut(
        uint256 amountIn,
        address[] memory path
    ) private view onlyCorrectPathLength(path) returns (uint256 amount) {
        uint256[] memory amounts = IUniswapV2Router02(SUSHI_SWAP_ROUTER)
