...
import { ILockableChildchain } from "./interfaces/ILockableChildchain.sol";
...
library LockableStorageChildchain {
	...
	struct Layout {
		...
		mapping(uint256 => ILockableChildchain.LockInfo) tokenLockedInfo;
	}
 ...
 */
abstract contract LockableUpgradeableChildchain is IERC5192, ILockableChildchain, Initializable, ERC165Upgradeable {
	// ====================================================
	// STATE
	// ====================================================
	using LockableStorageChildchain for LockableStorageChildchain.Layout;
	...
	function _lockToken(uint256 tokenId) private {
		if (!LockableStorageChildchain.layout().tokenLockingEnabled) {
			revert TokenLockingDisabled();
		}
		LockableStorageChildchain.layout().tokenLockedStates[tokenId] = true;
		emit Locked(tokenId);
	}
	...
	function lockToken(uint256 tokenId) internal {
		...
		LockableStorageChildchain.layout().tokenLockedInfo[tokenId] = LockInfo({ lockTime: 0, lockDuration: 0 });
	}

	function lockToken(uint256 tokenId, uint256 lockDuration) internal {
		_lockToken(tokenId);

		LockableStorageChildchain.layout().tokenLockedInfo[tokenId] = LockInfo({
	...
	function unlockToken(uint256 tokenId) internal {
		...
		LockableStorageChildchain.layout().tokenLockedStates[tokenId] = false;
		emit Unlocked(tokenId);

		delete LockableStorageChildchain.layout().tokenLockedInfo[tokenId];
	}
	...
	function getTokenLockingEnabled() public view returns (bool) {
		return LockableStorageChildchain.layout().tokenLockingEnabled;
	}

	function getTokenLockedState(uint256 tokenId) public view returns (bool lockedState, LockInfo memory lockInfo) {
		lockInfo = LockableStorageChildchain.layout().tokenLockedInfo[tokenId];
		lockedState = LockableStorageChildchain.layout().tokenLockedStates[tokenId];
	}
	...
	function enableTokenLocking() public virtual {
		LockableStorageChildchain.layout().tokenLockingEnabled = true;
		emit LockingEnabledToggle(true);
	}

	function disableTokenLocking() public virtual {
		LockableStorageChildchain.layout().tokenLockingEnabled = false;
...
interface ILockableChildchain is IERC5192 {
	// ====================================================
	// EVENTS
	// ====================================================
...
import "@opengsn/contracts/src/interfaces/IERC2771Recipient.sol";
import "../../../thirdparty/falkor-contracts/tokens/utils/LockableUpgradeableChildchain.sol";
	...
	ERC2771RecipientUpgadeable
{
	...
	// EVENTS
	// ====================================================
	event BatchLocked(uint256[] tokenIds, uint256 duration);
	...
	function supportsInterface(
		...
		public
		view
		override(ERC721EnumerableUpgradeable, LockableUpgradeableChildchain, AccessControlUpgradeable)
		returns (bool)
	{
		return
			AccessControlUpgradeable.supportsInterface(interfaceId) ||
			ERC721EnumerableUpgradeable.supportsInterface(interfaceId) ||
			LockableUpgradeableChildchain.supportsInterface(interfaceId) ||
			super.supportsInterface(interfaceId);
	}
	...
	function _beforeTokenTransfer(
		...
		uint256 tokenId,
		uint256 batchSize
	) internal override(ERC721EnumerableUpgradeable) onlyWhenUnlocked(tokenId) {
	...
	function lockTokens(uint256[] calldata tokenIds, uint256 lockDuration) public {
		...
		for (uint i = 0; i < tokenIds.length; i++) {
			if (sender != ownerOf(tokenIds[i]) && !hasRole(QUESTING_CONTRACT_ROLE, sender)) {
				revert Unauthorized();
			}
			super.lockToken(tokenIds[i], lockDuration);
		}
		emit BatchLocked(tokenIds, lockDuration);
	}
	...
	/**
	 * @notice Helper for QuestManager
	 */
	function lockSingleToken(uint256 tokenId, uint256 lockDuration) public {
		address sender = _msgSender();
		if (sender != ownerOf(tokenId) && !hasRole(QUESTING_CONTRACT_ROLE, sender)) {
			revert Unauthorized();
		}
	 ...
	 * @notice Helper for QuestManager
	 */
	function unlockSingleToken(uint256 tokenId) public {
		address sender = _msgSender();
		if (sender != ownerOf(tokenId) && !hasRole(QUESTING_CONTRACT_ROLE, sender)) {
			revert Unauthorized();
		}
	...
	function tokenURI(uint256 tokenId) public view override returns (string memory) {
		...
		uint256 id = tokenId; //((tokenId + randomOffset) % maxSupply()) + _startTokenId();
