...
// MODIFIERS
    ...
    modifier onlyIfEnoughHealth(uint256 tokenID, uint256 numberOfQuests) {
        ...
        farmlandCharacters.setHealthTo(tokenID, health);
...
// FUNCTIONS
        ...
        onlyQuestActive(tokenID)
    {
        ...
        (,uint256 numberOfActivities,,,) = farmlandCharacters.charactersActivity(tokenID);  // Get the number of activities
        farmlandCharacters.reduceHealth(tokenID, numberOfActivities);                       // Update the characters health
...
// INTERNAL HELPER FUNCTIONS
        ...
        returns (bool hazardAvoided)
    {
        ...
        if (!hazardsEnabled) {
           ...
           uint256[] memory randomNumbers = new uint256[](1);                 // If Hazards are enabled
           randomNumbers = getRandomNumbers(1);                               // Return a random number
           if (getCharactersBravery(tokenID) > randomNumbers[0] % 100 ) {     // Bravery has to be greater than a random number between 0-100 for the character to avoid the hazard
...
// VIEWS
        ...
        )
    {
        ...
        if (courage > 95 || intelligence > 95)                                                // Founders, Genius or Hero
        {
            bravery += bravery / 2;
        }
    }
...
import "@openzeppelin/contracts/access/Ownable.sol";
import "../WrappedCharacter/interfaces/IWrappedCharacter.sol";
...
contract CharacterManager is ERC721Holder, ReentrancyGuard, Pausable, Ownable {
    ...
    {
        require(farmlandAddresses.length == 5, "Invalid number of contract addresses");
        farmlandCharacters = IWrappedCharacter(farmlandAddresses[1]);
    }

// STATE VARIABLES

    /// @dev The Farmland Character Contract
    IWrappedCharacter internal immutable farmlandCharacters;

    /// @dev A mapping to track characters and there owners
...
// MODIFIERS
    ...
    modifier onlyCharacterOwner (uint256 tokenID) {
        require(characterOwner[tokenID] == _msgSender(),"You don't own this Character or it's not added to the contract");
        _;
    }
...
// FUNCTIONS
    ...
    function _addCharacter(uint256 tokenID)
        ...
        whenNotPaused
    {
        characterOwner[tokenID] = _msgSender();                                 // Map the owner to the character token id
    ...
    function _releaseCharacter(uint256 tokenID)
        ...
        onlyCharacterOwner(tokenID)
    {
        delete characterOwner[tokenID];                                        // Delete the mapping between owner and token id
...
struct Stats {uint256 stamina; uint256 strength; uint256 speed; uint256 courage; uint256 intelligence;}
struct ExtendedStats {uint256 health; uint256 experience; uint256 level;}
...
abstract contract IWrappedCharacter is IERC721 {
    mapping(bytes32 => uint256[]) internal stats;
    mapping(bytes32 => Stats) internal nonNativeStats;
    mapping (bytes32 => ExtendedStats) public extendedStats;
    ...
    function setBeginActivity(uint256 wrappedTokenID, uint256 activityDuration, uint256 NumberOfActivities, uint256 startBlock, uint256 endBlock) external virtual;
    function setHealthTo(uint256 wrappedTokenID, uint256 amount) external virtual;
    function increaseHealth(uint256 wrappedTokenID, uint256 amount) external virtual;
    function reduceHealth(uint256 wrappedTokenID, uint256 amount) external virtual;
    function calculateHealth(uint256 wrappedTokenID) external virtual view returns (uint256 health);
    ...
    function getMaxHealth(uint256 wrappedTokenID) external virtual view returns (uint256 health);
    function getStats(uint256 wrappedTokenID) external virtual view returns (uint256 stamina, uint256 strength, uint256 speed, uint256 courage, uint256 intelligence, uint256 health, uint256 experience, uint256 level);
