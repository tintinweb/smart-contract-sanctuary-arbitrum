...
contract WINRBlackJack is IWINRBlackJack, CoreLight {
    ...
    uint64 internal constant minWagerAmount = 1e17 * 9; // 0.90 usd
    ...
    // index of the game (also index of the dealers hand)
    uint256 internal gameIndex = 1e12; // game index starts at 1e12

    // handindexes of the playes hands
    ...
    // @dev if a handIndex in this array is part of the game depends on the amountHands number in the game struct
    // @dev if a handIndex is active or not depends on the activeHandsInGame mapping
    ...
    // @note contract overwrites the array, so it can store handIndexes of a previous game, but these will not be accessed by the contract since array is parsed with amountHands
    // @dev considering 5 amount hands if array is [true, true, false, true, false] it means that handIndex 0, 1 and 3 are active and 2 and 4 are inactive
    // @dev if amount hands is 2 and array is [true, true, false, true, false] it means that handIndex 0 and 1 are active and 2, 3 and 4 are inactive (because they are not part of the game)
    ...
    /**
     * handIndex -> for the players cards
     * gameIndex -> for the dealers cards
     */
    ...
    // array with the drawn cards count for a game. The index of the array is the card number, the value is the amount of times the card has been , so if [2, 0, 1 ..] it means that 2 aces have been drawn and 1 3./
    // gameIndex => drawnCardsOfGame -> note drawn cards are not overwritten when a new game is started
    ...
    function randomizerFulfill(uint256 _requestId, uint256[] calldata _randoms) internal override {
        ...
        require(game_.awaitingRandomness, "Blackjack: Game is not awaiting randomness");
        game_.awaitingRandomness = false; // Set the awaitingRandomness flag to false
        game_.randomness = uint64(_randoms[0]); // Store the randomness in the contract's storage
        ...
        if (game_.status == GameStatus.TABLE_DEAL) {
            unchecked {
                ...
                for (uint256 i = 1; i < amountCards_; i += 2) {
                    ...
                    _dealFirstCardsToHand(handIndex_, topCards_[i], topCards_[i + 1]);
                    index_++;
                }
            ...
            if (_checkAllHandsInactive(_player)) {
                ...
                _processDealerTurn(_player, _randoms[0]); // Process the dealer's turn
                return;
            } else {
            ...
            // revert("Blackjack: Game is in invalid state - DEALER_TURN");
            ...
            uint256[] memory topCards_ =
                HelperBlackJack.drawCardsFromStack(_randoms[0], 12, drawnCards[activePlayerMapping[_player]]);
    ...
    function _processDealerTurn(address _player, uint256 _randomness) internal {
        unchecked {
            // If the game is in the DEALER_TURN status, draw cards for the dealer
            // if we have 1 deck the max amount of cards a dealer can ever get is 6; Ace, Ace, Ace, Ace, 2, 2 = 18 (stand)
            uint256 _gameIndex = activePlayerMapping[_player];

            uint256[] memory topCards_ = HelperBlackJack.drawCardsFromStack(_randomness + 1, 12, drawnCards[_gameIndex]);
    ...
    function _checkAllHandsInactive(address _player) internal view returns (bool) {
        unchecked {
            ...
            for (uint256 i = 0; i < games[_player].amountHands; i++) {
    ...
    function _getActiveHandIndex(address player) internal view returns (uint32) {
        unchecked {
            ...
            for (uint32 i = 0; i < games[player].amountHands; i++) {
                if (activeHandsInGame[player][i]) {
                    return handIndexesGame[player][i];
                }
            }
            // revert("Blackjack: No active hands found");
        }
    }
    ...
    function standOff(uint256 _handIndex) external {
        unchecked {
            ...
            if (_checkAllHandsInactive(msg.sender)) {
                ...
                games[msg.sender].activeHandIndex = 0;
    ...
    function _initHand(
        ...
        unchecked {
            ...
            hand_.status = HandStatus.AWAITING_HIT; // Set the status of the hand to AWAITING_HIT
    ...
    function _checkWagerReturn(uint256 _betAmount, address _token) internal view returns (uint256) {
        unchecked {
            ...
            require((dollarValue_ / 1e12) > minWagerAmount, "Blackjack: Wager too low");
            return tokenAmount_;
        }
    }
     ...
     */
    function _dealFirstCardsToHand(uint256 _handIndex, uint256 _firstCard, uint256 _secondCard) internal {
        unchecked {
            // check first card and second card not 0
            ...
            cards[_handIndex] = HelperBlackJack.dealFirstCardsToHand(_firstCard, _secondCard);

            uint256 totalCount_ = cards[_handIndex].totalCount;
            ...
            if (totalCount_ == 21) {
                ...
                hands[_handIndex].status = HandStatus.BLACKJACK;
                _completeActiveHand(hands[_handIndex].player, _handIndex);
            } else {
                // If the total count of the cards is not 21, the hand is still playing
                // Update the hand's status to PLAYING
                hands[_handIndex].status = HandStatus.PLAYING;
            }

            // Emit an event to log the dealt cards and their total count
            emit HandDealt(_handIndex, hands[_handIndex].gameIndex, _firstCard, _secondCard, totalCount_);
        }
    }
    ...
    function _dealCardToHand(uint256 _handIndex, uint256 _card) internal {
        unchecked {
            ...
            // Emit an event to log the hit action
            emit HandHit(_handIndex, _card, cards_.totalCount, hand_.isDouble);
        }
    }
    ...
    function _processHandCompleted(address _player, uint256 _handIndex) internal {
        unchecked {
            ...
            if (_checkAllHandsInactive(_player)) {
                ...
                _processDealerTurn(_player, games[_player].randomness);
    ...
    function _processDealerBlackjack(address _playerAddress) internal {
        unchecked {
            uint256 length_ = games[_playerAddress].amountHands;
        ...
        internal
    {
        unchecked {
            ...
            uint256 _random = games[_hand.player].randomness + _handIndex;
    ...
    function _checkState(uint256 _handIndex) internal view {
        unchecked {
            ...
            require(games[msg.sender].status == GameStatus.PLAYER_TURN, "Blackjack: Game is not in player turn"); // note this is a logical check but technically redundant
