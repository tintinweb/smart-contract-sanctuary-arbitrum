...
//Modified from: https://github.com/abdelhamidbakhta/token-vesting-contracts/blob/main/contracts/TokenVesting.sol
...
contract VestingManager is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) private vestingSchedulesTotalAmount;
    mapping (address => uint256) private vestingSchedulesReleased;
    mapping(address => uint256) private contractTotalBalance;

    mapping(address => uint256) private holdersVestingCount;
    mapping(address => bytes32[]) private holdersVestingIds;
    ...
    function getVestingSchedulesCountByRecipient(address _recipient) external view returns(uint256){
        return holdersVestingCount[_recipient];
    }

    function getVestingIdsForRecipient(address _recipient) external view returns(bytes32[] memory){
        return holdersVestingIds[_recipient];
    }
    ...
    function getVestingScheduleByAddressAndIndex(address _holder, uint256 _index) external view
    returns(VestingSchedule memory){
        return getVestingSchedule(computeVestingScheduleIdForAddressAndIndex(_holder, _index));
    }

    function getVestingSchedulesTotalAmount(address _token) external view returns(uint256){
        return vestingSchedulesTotalAmount[_token];
    }
    ...
    * @param _cliffPeriodInSeconds Cliff period of the vest in seconds.
    ...
    */
    function createVestingSchedule(
        ...
        require( this.getWithdrawableAmount(_token) >= _amountTotal, "TokenVesting: insufficient tokens for vesting schedule");
        require(_totalVestingDuration > 0, "TokenVesting: totalVestingDuration must be > 0");
        require(_amountTotal > 0, "TokenVesting: amount must be > 0");
        require(_unlockInterval >= 1, "TokenVesting: unlockInterval must be >= 1");

        bytes32 vestingScheduleId = this.computeNextVestingScheduleIdForHolder(_recipient);
        uint256 cliffPeriodInSeconds = _startTime + _cliffPeriodInSeconds;
        ...
        vestingSchedulesTotalAmount[_token] = vestingSchedulesTotalAmount[_token] + _amountTotal;
        vestingSchedulesIds.push(vestingScheduleId);
        holdersVestingCount[_recipient] = holdersVestingCount[_recipient] + 1;
        holdersVestingIds[_recipient].push(vestingScheduleId);

        deposit(_amountTotal, _token);
        emit VestingScheduleCreated(_recipient, _token, _startTime, _cliffPeriodInSeconds, _totalVestingDuration, _unlockInterval, _amountTotal, _revocable, vestingScheduleId);
    }
    ...
    function revoke(bytes32 vestingScheduleId) public onlyOwner onlyIfVestingScheduleNotRevoked (vestingScheduleId){
        ...
        vestingSchedulesTotalAmount[vestingSchedule.erc20Token] = vestingSchedulesTotalAmount[vestingSchedule.erc20Token] - unreleased;
        vestingSchedule.revoked = true;

        emit Revoked(vestingScheduleId);
    }

    function withdraw(uint256 _amount, address _token) public nonReentrant onlyOwner {
        require(this.getWithdrawableAmount(_token) >= _amount, "TokenVesting: not enough withdrawable funds");
        require((this.getWithdrawableAmount(_token) - vestingSchedulesTotalAmount[_token]) >= _amount, "TokenVesting: cannot withdraw funds scheduled to be vested");
    ...
    */
    function release(
        ...
        require(
            isRecipient || isOwner,
            "TokenVesting: only recipient and owner can release vested tokens"
        );
        uint256 vestedAmount = _computeReleasableAmount(vestingSchedule);
        ...
        vestingSchedulesTotalAmount[vestingSchedule.erc20Token] = vestingSchedulesTotalAmount[vestingSchedule.erc20Token] - _amount;
        vestingSchedulesReleased[vestingSchedule.erc20Token] = vestingSchedulesReleased[vestingSchedule.erc20Token] + _amount;
    ...
    function getWithdrawableAmount(address _token) public view returns(uint256){
        return contractTotalBalance[_token] - vestingSchedulesTotalAmount[_token];
    }
    ...
    function deposit(uint256 _amountInWEI, address _token) public {
        IERC20(_token).transferFrom(msg.sender, address(this), _amountInWEI);
        contractTotalBalance[_token] = contractTotalBalance[_token] + _amountInWEI;
    ...
    function _computeReleasableAmount(VestingSchedule memory vestingSchedule) internal view returns(uint256){
        ...
        if ((currentTime < vestingSchedule.cliffPeriodInSeconds) || vestingSchedule.revoked == true) {
            return 0;
    ...
    function amountExcludedFromCirculatingSupply(address _token) external view returns(uint256) {
        return vestingSchedulesTotalAmount[_token] - vestingSchedulesReleased[_token];
    }
