...
// Interface for the setCollectibleSlot function within Farmland Characters
interface ICharacterSlots {
    function setCollectibleSlot(uint256 id, uint256 slotIndex, uint256 slot) external;
}

/// @dev Farmland - CharacterSlotManager Smart Contract
contract CharacterSlotManager is Permissioned {

    /// @dev This is the Farmland Character contract
    ICharacterSlots characterContract;

    constructor (
        address payable charactersAddress
        ) Permissioned()
    {
        characterContract = ICharacterSlots(charactersAddress);
    }

    /// @dev Update the info held in a characters slot
    /// @param tokenID character to update
    /// @param slotIndex is the slot index to store data (1-8)
    /// @param value is the information to store (uint256)
    function updateSlot(uint256 tokenID, uint256 slotIndex, uint256 value)
        external
        onlyAllowed
    {
        characterContract.setCollectibleSlot(tokenID, slotIndex, value);
    }
