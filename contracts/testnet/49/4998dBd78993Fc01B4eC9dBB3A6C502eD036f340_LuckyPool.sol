...
contract LuckyPool is Caller {
    ...
    event ClaimReward(uint256 period, address winner, uint256 rewards);
    event TicketEvent(uint256 period, address player, uint256 amount);
    ...
    function injectRewards(address user, uint256 usd) external {
        if (!_isCaller(_msgSender()) || !_inProgress() || usd < 100 * 1e6) {
            return;
        }
    ...
    function _claimReward() internal {
        ...
        if (pool.totalWeight > 0) {
            ...
            if (reward > 0) {
                TransferHelper.safeTransfer(address(WETH), user, reward);
            }
        }
