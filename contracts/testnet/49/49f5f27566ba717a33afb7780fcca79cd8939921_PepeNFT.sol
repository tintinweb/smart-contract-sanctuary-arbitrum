...
contract PepeNFT is ERC1155, Ownable {
    ...
    event updateSigner(address indexed _newSigner);

    constructor(
        address _signer
    ) ERC1155("https://nft.pepeclub.xyz/api/nft/{id}.json") {
        serverSigner = _signer;

        tokensSupply[PLATINUM] = 50;
        tokensSupply[GOLD] = 200;
        tokensSupply[SILVER] = 500;
        tokensSupply[BRONZE] = 1000;

        tokensAmount[PLATINUM] = 0;
        tokensAmount[GOLD] = 0;
        tokensAmount[SILVER] = 0;
        tokensAmount[BRONZE] = 0;
    }
    ...
    function claimNFT(
        ...
        require(
            recoverSigner(message, signature) == serverSigner,
            "Invalid signature"
        );

        require(
            tokensAmount[tokenId] < tokensSupply[tokenId],
            "Unable to mint more due to limited supply"
        );

        // after verify, mint NFT
        _mint(msg.sender, tokenId, 1, "");
        tokensAmount[tokenId]++;
    ...
    function generateMessage(
        ...
        return
            prefixed(
                keccak256(
                    abi.encodePacked(_receiver, _tokenId, _nonce, _contract)
                )
            );
    }
}