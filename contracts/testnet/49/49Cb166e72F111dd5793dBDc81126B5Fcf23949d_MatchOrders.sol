...
contract MatchOrders is AccessControl {
    ...
    constructor(address _vertexEndPoint) {
    ...
    function linkVertexSigner(address externalAccount, uint64 nonce) internal {
        ...
        bytes memory linkerTx = abi.encodePacked("\x19", abi.encode(linkSigner));
        IEndpoint(vertexEndPoint).submitSlowModeTransaction(linkerTx);
    }

    function setLinkerSigner(address externalAccount, uint64 nonce) external onlyAdmin {
