...
contract Users is IUsers, Ownable, ReentrancyGuard{
    ...
    event UserMediaChange(address indexed _address, string[] _mediaArray);
    ...
    function updateProfile(
            string memory _userName, uint _birthYear, Genders _gender, uint _fee, AreasOfInterest[] memory _interests,
            string memory _bio, uint _latitude, uint _longitude, Zodiac _sign) external {
        string[] memory mediaArray;
        ...
        if (users[msg.sender].status == Statuses.notRegistered) {
            ...
            users[msg.sender] = User(
                _userName, _birthYear, _gender, Statuses.available, _fee,
                0, 0, 0, _interests, _bio, _latitude, _longitude, _sign, 0, 0,
                0x0, 0x0, mediaArray, regDateTime, 0, 0, 0);
        }
        else {
            ...
            regDateTime = user.registrationDateTime;
        }
        emit UserProfileChange(_userName, _birthYear, _gender, Statuses.available, _fee, _interests, _bio, _latitude, _longitude, _sign, regDateTime);
    }

    function updateStats(address _caller, address _callee, uint _chatStartDateTime, uint _chatEndDateTime) onlyBy(chatsContract) external {
    ...
    function unlockDeposit(address _address, uint _exclude) onlyBy (chatsContract) external returns (uint) {
    ...
    function pay(address _address, uint _amount, address _depositor) onlyBy(chatsContract) external {
    ...
    function addRating(address _address, uint _rating) ratingsEnabled external {
    ...
    function removeRating(address _address) ratingsEnabled external {
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, _address));
    ...
    function blockUser(address _address) blocksEnabled external returns (bytes32) {
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, _address));
    ...
    function unblockUser(address _address) blocksEnabled external returns (bool) {
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, _address));
    ...
    function updateMedia(string[] memory _mediaArray) external {
        require(users[msg.sender].status != Statuses.notRegistered, 'Register first');
        User storage user = users[msg.sender];
        user.media = _mediaArray;
        emit UserMediaChange(msg.sender, _mediaArray);
    }
}
