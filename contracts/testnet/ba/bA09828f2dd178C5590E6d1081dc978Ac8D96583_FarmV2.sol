/**
 *Submitted for verification at Arbiscan on 2022-05-02
*/
...
/* Reponsible for the distribution of hats produced by the Farm */
contract FarmDistributionCenter is Ownable {
    ...
    function sendRewards(uint256 _amount, address _user) public onlyFarm {
...
contract FarmV2 is Ownable, ReentrancyGuard {
    ...
    // Rewards contract
    FarmDistributionCenter private hatDistributor;
    ...
    constructor(
        ...
        hatDistributor = FarmDistributionCenter(_hatDistributor);
    ...
    function harvest(uint256 _pid) public {
        ...
        user.rewardDebt = (user.amount.mul(pool.accAssetPerShare).div(1e12))
            .toInt256();
    ...
    function compound(uint256 _pid) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        if (address(pool.lpToken) != address(asset))
    ...
    function updateEmissionRate(uint256 _assetPerSecond) public onlyOwner {
        ...
        assetPerSecond = _assetPerSecond;
        massUpdatePools();
        emit UpdateEmissionRate(_assetPerSecond);
    }
    ...
    event UpdateEmissionRate(uint256 assetPerSecond);
