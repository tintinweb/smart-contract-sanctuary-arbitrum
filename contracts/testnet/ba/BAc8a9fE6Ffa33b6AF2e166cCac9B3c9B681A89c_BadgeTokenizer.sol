...
import "./BadgifyRegistrar.sol";

/**
 * @title Badgify Tokenizer
 * @author raldblox.eth (github.com/raldblox)
 * @dev Tokenize Digital Badges with dynamic on-chain metadata
 * and on-chain visual graphics
 */
contract BadgeTokenizer is ERC721, Pausable, Ownable2Step, ERC721Burnable {
    ...
    mapping(uint256 => DesignInfo) internal designInfo;

    mapping(uint256 => address) internal designMaker;
    mapping(uint256 => uint256) internal tokenDesigners;

    mapping(uint256 => mapping(address => address)) private issuers;
    mapping(uint256 => bool) private soulBoundTokens;
    mapping(uint256 => bool) private distributedTokens;
    mapping(uint256 => string) private addAttributes;
    mapping(address => uint256) private issuedTokens;
    mapping(address => uint256) private recievedTokens;

    IBadgeThemer private themer;
    IBadgeVisualizer private visualizer;
    IBadgifyRegistrar private registrar;
    ...
    function setRegistrar(address _new) external onlyOwner {
        registrar = IBadgifyRegistrar(_new);
    }
    ...
    function versionThemer() external view returns (string memory) {
        return themer.version();
    }

    function versionRegistrar() external view returns (string memory) {
        return registrar.version();
    }

    function versionVisualizer() external view returns (string memory) {
    ...
    function tokenizerFee(uint256 _price) external onlyOwner {
        tFees = _price;
    }

    function price() public view returns (uint256) {
        return tFees * decimals();
    }
    ...
    function setBadgeInfo(
        ...
        require(
            !registrar.blacklisted(msg.sender),
    ...
    function setDesignInfo(
        ...
        designInfo[designId] = DesignInfo({
            designer: msg.sender,
            header: headers,
            title: title,
            category: category,
        ...
        designMaker[designId] = msg.sender;
    }
    ...
    function singleMint(
        ...
        require(msg.value >= price(), "BADGIFY: Tokenizer fee not met");
        require(
            !registrar.blacklisted(msg.sender),
            "BADGIFY: Sender is blacklisted"
        );
        ...
        setBadgeInfo(tokenId, headers, title, category, fontcolor, bgcolor);
    }

    function nameAddress(string memory _name, address _addr) external {
        if (msg.sender == admin) {
            registrar.setNamespace(_name, _addr);
        } else {
            registrar.setNamespace(_name, msg.sender);
        }
    }
    ...
    function multiMint(
        ...
        require(
            msg.value >= price().mul(addr.length),
        ...
        if (msg.sender != admin) {
            for (uint256 i = 0; i < addr.length; i++) {
                mint(addr[i], msg.sender, _isNonTransferable, true, _designIds);
            }
        } else {
            for (uint256 i = 0; i < addr.length; i++) {
                mint(
    ...
    function mint(
        ...
        address issuer,
        bool _isNonTransferable,
        bool _distributedToken,
        uint256 _designIds
    ) internal returns (uint256) {
        uint256 tokenId_ = totalSupply;
        _safeMint(reciever, tokenId_);
        soulBoundTokens[tokenId_] = _isNonTransferable;
        issuers[tokenId_][reciever] = issuer;
        issuedTokens[issuer] += 1;
        recievedTokens[reciever] += 1;
        distributedTokens[tokenId_] = _distributedToken;
        tokenDesigners[tokenId_] = _designIds;
        totalSupply++;
        return tokenId_;
    }
    ...
    function viewIssuer(
        ...
        return issuers[tokenId][reciever];
    }
            ...
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"Digital Badges","description":"","image":"","external_link":"","fee_recipient":""}'
                            )
                        )
                    )
                )
    ...
    function tokenURI(
        ...
        string memory description_ = "Digital Badge on the Blockchain";
    ...
    function addAttribute(uint256 tokenId, string memory _attributes) external {
        if (msg.sender != admin) {
            require(ownerOf(tokenId) == msg.sender, "BADGIFY: Not Authorized");
        }
        addAttributes[tokenId] = _attributes;
    }

    function generateAttribute(
        uint256 tokenId
    ) internal view returns (string memory) {
        bool withThemer = distributedTokens[tokenId];
        BadgeInfo memory info = badgeInfo[tokenId];
        DesignInfo memory design = designInfo[tokenDesigners[tokenId]];
        return
            string(
                abi.encodePacked(
                    ...
                    withThemer ? design.bgcolor : info.bgcolor,
                    '"},{"trait_type": "font color", "value": "',
                    withThemer ? design.fontcolor : info.fontcolor,
                    '"},{"trait_type": "badge issuer", "value": "',
                    registrar.viewNamespace(ownerOf(tokenId)),
                    '"},{"trait_type": "category", "value": "',
                    withThemer ? design.category : info.category,
                    '"},{"trait_type": "token type", "value": "',
                    soulBoundTokens[tokenId]
                        ? "non-transferable token"
                        : "transferable token",
                    '"}',
                    addAttributes[tokenId]
                )
            );
    }
    ...
    function generateImage(
        ...
        BadgeInfo memory info = badgeInfo[tokenId];
        bool withThemer = distributedTokens[tokenId];
        uint256 designer = withThemer ? tokenDesigners[tokenId] : 0;
        DesignInfo memory design = designInfo[designer];
        ...
        string memory encodedBytes = Base64.encode(
            bytes(
                abi.encodePacked(
                    ...
                    themer.header(
                        withThemer ? design.header : info.header,
                        withThemer ? design.fontcolor : info.fontcolor
                    ),
                    themer.lowerBound("black"),
                    themer.title(withThemer ? design.title : info.title),
                    themer.category(
                        withThemer ? design.category : info.category
                    ),
                    themer.format()
    ...
    function _transfer(
        ...
        require(!soulBoundTokens[tokenId], "BADGIFY: Not Transferrable Token");
...
interface IBadgeThemer {
    ...
    function title(string memory) external pure returns (string memory);
