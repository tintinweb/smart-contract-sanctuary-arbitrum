...
abstract contract BaseFacet is AccessControl {
}
...
contract ERC20Facet is BaseFacet, ERC20 {
    ...
    modifier maxWallet(
        ...
        if (!hasRole(LibDiamond.EXCLUDED_FROM_MAX_WALLET_ROLE, recipient)) {
            require(
                balanceOf(recipient) + amount <=
                    LibDiamond.MAX_TOKEN_PER_WALLET,
                "Wallet exceeds max"
            );
        }
        _;
    ...
    constructor() ERC20("ArbiFarmer", "FARM") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);

        // LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();

        // // pre approve contract
        // _approve(address(this), _uniswapRouter, 2**256 - 1);
        ...
        // _ds.lpPools[address(swapPair)] = true;
        ...
        // _grantRole(LibDiamond.EXCLUDED_FROM_FEE_ROLE, _ds.dividendContract);
        // _grantRole(LibDiamond.EXCLUDED_FROM_FEE_ROLE, _ds.teamWallet);
        // _grantRole(LibDiamond.EXCLUDED_FROM_FEE_ROLE, _ds.liquidityWallet);
        ...
        // _grantRole(LibDiamond.EXCLUDED_FROM_MAX_WALLET_ROLE, LibDiamond.BURN_ADDRESS);
        // _grantRole(LibDiamond.EXCLUDED_FROM_MAX_WALLET_ROLE, swapPair);
        // _grantRole(LibDiamond.EXCLUDED_FROM_MAX_WALLET_ROLE, _ds.dividendContract);
        // _grantRole(LibDiamond.EXCLUDED_FROM_MAX_WALLET_ROLE, _ds.teamWallet);
        // _grantRole(LibDiamond.EXCLUDED_FROM_MAX_WALLET_ROLE, _ds.liquidityWallet);
    ...
    function setLiquidityFee(uint256 _liquidityFeeBasisPoints)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        _ds.liquidityFeeBasisPoints = _liquidityFeeBasisPoints;
    }

    function setTeamFee(uint256 _teamFeeBasisPoints)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        _ds.teamFeeBasisPoints = _teamFeeBasisPoints;
    }

    function setDividendFee(uint256 _dividendFeeBasisPoints)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        _ds.dividendFeeBasisPoints = _dividendFeeBasisPoints;
    }

    function sendToTeam(uint256 _amount) private {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        payable(_ds.teamWallet).transfer(_amount);
    }

    function sendToDividend(uint256 _amount) private {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        payable(_ds.dividendContract).transfer(_amount);
    }
    ...
    function teamWallet() external view returns (address) {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        return _ds.teamWallet;
    }

    function liquidityFee() external view returns (uint256) {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        return _ds.liquidityFeeBasisPoints;
    }

    function teamFee() external view returns (uint256) {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        return _ds.teamFeeBasisPoints;
    }

    function dividendFee() external view returns (uint256) {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        return _ds.dividendFeeBasisPoints;
    }

    function totalFees() public view returns (uint256) {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        return
            _ds.dividendFeeBasisPoints +
            _ds.teamFeeBasisPoints +
            _ds.liquidityFeeBasisPoints;
    }
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) internal override maxWallet(from, to, amount) {
        ...
        if (sendFee) {
            ...
            if (contractTokenBalance >= _ds.numTokensToSwap) {
                _processFees(_ds.numTokensToSwap, 0);
            }
        }
        ...
        // IDividendTracker(_ds.dividendContract).setBalance(
        //     payable(from),
        //     balanceOf(from)
        // );
        // IDividendTracker(_ds.dividendContract).setBalance(
        ...
        //     balanceOf(to)
        // );
        // if (!_ds.processingFees && _ds.processingGas > 0) {
        //     IDividendTracker(_ds.dividendContract).process(_ds.processingGas);
        // }
    }

    function _processFees(uint256 tokenAmount, uint256 minAmountOut)
        private
        lockTheSwap
    {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        ...
        if (contractTokenBalance >= tokenAmount && totalTax > 0) {
            uint256 liquidityAmount = (tokenAmount *
                _ds.liquidityFeeBasisPoints) / totalTax;
            uint256 dividendAmount = (tokenAmount *
                _ds.dividendFeeBasisPoints) / totalTax;
            uint256 liquidityTokens = liquidityAmount / 2;
            uint256 teamAmount = tokenAmount -
                (liquidityAmount + dividendAmount);
            uint256 liquifyAmount = liquidityAmount -
                (liquidityTokens + teamAmount + dividendAmount);
            ...
            // send team ETH
            sendToTeam(
                (address(this).balance * _ds.teamFeeBasisPoints) /
                    (_ds.teamFeeBasisPoints + _ds.dividendFeeBasisPoints)
            );
            sendToDividend(address(this).balance);
        }
    }

    // =========== UNISWAP ===========

    function swapTokensForEth(uint256 tokenAmount, uint256 minAmountOut)
        private
    {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        ...
        path[0] = address(this);
        path[1] = IUniswapV2Router02(_ds.uniswapRouter).WETH();

        // make the swap
        IUniswapV2Router02(_ds.uniswapRouter)
    ...
    function addLiquidity(uint256 tokenAmount, uint256 _value) private {
        LibDiamond.DiamondStorage storage _ds = LibDiamond.diamondStorage();
        IUniswapV2Router02(_ds.uniswapRouter).addLiquidityETH{value: _value}(
            address(this),
            tokenAmount,
            0,
            0,
...
library LibDiamond {
    ...
    bytes32 public constant EXCLUDED_FROM_DIVIDEND_ROLE =
        keccak256("EXCLUDED_FROM_DIVIDEND_ROLE");

    // With `magnitude`, we can properly distribute dividends even if the amount of received ether is small.
    uint256 internal constant MAGNITUDE = 2**128;
    uint256 public constant PERCENTAGE_DENOMINATOR = 10000;
    uint256 public constant MAX_TOKEN_PER_WALLET = 10000000 * 10**18; // Max holding limit, 1.00% of supply
    ...
    struct DiamondStorage {
        ...
        mapping(address => uint256) magnifiedDividend;
        mapping(address => uint256) withdrawnDividend;
