...
contract pachiSlotV5T1 is Ownable {
    /// VRF - SUPRA ORACLES ///
    ...
    function getEstimatedLoss(
        uint256 _wager,
        uint256 _numSpins
    ) public pure returns (uint256) {
        return (_wager * _numSpins * HOUSE_EDGE) / HOUSE_EDGE_DIVISOR;
    }

    function getEstimatedLossFromTotalInput(
        uint256 _totalInput
    ) public pure returns (uint256) {
        return (_totalInput * HOUSE_EDGE) / HOUSE_EDGE_DIVISOR;
    }
    ...
    function executePayout(
        ...
        uint expectedRevenue = getEstimatedLossFromTotalInput(_totalInput);
        ...
        if (mintManager != address(0)) {
            ...
            IMintManager(mintManager).mintXPachi(_from, expectedRevenue);
        }
