...
contract Fiat24USDCTopUp is AccessControl, Pausable {
    ...
    function topUpUsdc(uint256 usdcAmount_) external {
        ...
        require(USD24.transferFrom(cryptoDeskAddress, msg.sender, topUpAmount), "Fiat24USDCTopUp: USD24 token transfer failed");
    ...
    function getUsedQuota(uint256 tokenId_) public view returns(uint256) {
        ...
        if(tokenEligibleForUsdcTopUp(tokenId_)) {
            if(quotas[tokenId_].isAvailable) {
                DateTime._DateTime memory tsLast = DateTime.parseTimestamp(quotas[tokenId_].quotaMonth);
                DateTime._DateTime memory tsNow = DateTime.parseTimestamp(block.timestamp);
                if(quotas[tokenId_].quotaMonth == 0 || tsLast.year < tsNow.year || (tsLast.year == tsNow.year && tsLast.month < tsNow.month)) {
                    quota = 0;
                } else {
