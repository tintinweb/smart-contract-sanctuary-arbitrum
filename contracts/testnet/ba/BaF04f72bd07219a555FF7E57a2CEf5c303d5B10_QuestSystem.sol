...
import {IEnergySystem, ID as ENERGY_SYSTEM_ID} from "../energy/IEnergySystem.sol";
    ...
    function startQuest(
        ...
        for (uint8 idx; idx < questDef.inputs.length; ++idx) {
            ...
            if (tokenType == GameRegistryLibrary.TokenType.ERC20) {
                ...
                // Spend NFT energy if needed
                if (inputDef.energyRequired > 0) {
                    IEnergySystem(_getSystem(ENERGY_SYSTEM_ID)).spendEnergy(
                        input.tokenContract,
                        input.tokenId,
