...
contract DexTrader is Ownable {
    ...
    event Trade(address indexed token0, address indexed token1, uint24 fee, uint amountIn, uint amountOut);
    ...
    function trade(address token0, address token1, uint24 poolFee, uint256 amountIn) external onlyExecutor returns (uint256 amountOut) {
        ...
        emit Trade(token0, token1, poolFee, amountIn, amountOut);
        return amountOut;
    }
