...
abstract contract BaseVRFConsumer is VRFConsumerBaseV2, Ownable {
    ...
    function beforeRequest(address user_, uint8 randomCount_) internal returns (uint256 requestId_) {
        ...
        _requestIdRequestInfo[requestId_] = RequestInfo(user_, new uint256[](0));
    ...
    function beforeFulfillRandomWords(uint256 requestId_, uint256[] memory randomWords_) internal returns (address) {
        RequestInfo storage requestInfo_ = _requestIdRequestInfo[requestId_];
        require(requestInfo_.user != address(0), "request not found");
        requestInfo_.randomWords = randomWords_;
        ...
        if (startIndex_ >= 0 && endIndex_ >= startIndex_) {
            ...
            for (uint256 index_ = startIndex_; index_ < ((endIndex_ > total) ? total : endIndex_);) {
                ...
                requestResultArr[arrayIndex_] = RequestResult(
                    ...
                    _requestIdRequestInfo[requestId_].randomWords
                );
...
contract L2UnicornHatcherVRF is L2UnicornHatcherPermit, BaseVRFConsumer, BaseMaximum, BaseL2, BaseL2Unicorn, BaseL2UnicornRule, BaseRecipient, Pausable {
    ...
    function hatchE0(uint256 deadline, uint8 v, bytes32 r, bytes32 s) payable external whenNotPaused {
        ...
        //
        uint256 requestId_ = beforeRequest(_msgSender(), 1);
    ...
    function hatchE1E2(uint8 hatchCount_, uint8 eSeries_) payable external whenNotPaused {
        ...
        //
        uint256 requestId_ = beforeRequest(_msgSender(), hatchCount_);
    ...
    function fulfillRandomWords(uint256 requestId_, uint256[] memory randomWords_) override internal {
        ...
        address user_ = beforeFulfillRandomWords(requestId_, randomWords_);
        uint8 hatchCount_ = _requestIdHatchCount[requestId_];
        require(randomWords_.length == hatchCount_, "L2UnicornHatcherVRF: random words length, hatch count exception");
        uint8 eSeries_ = _requestIdESeries[requestId_];
        for (uint8 numberIndex_ = 0; numberIndex_ < hatchCount_;) {
            uint256 randomWord_ = randomWords_[numberIndex_];
            ...
            (uint256 randomNumber_, IL2UnicornRule.HatchRule memory hatchRule_) = _getRandomNumberAndHatchRule(randomWord_, eSeries_);
    ...
    /*function getHatchRandomNumber(uint8 eSeries_, uint256 randomWord_, uint256 numberIndex_) public view returns (uint256){
        if (eSeries_ == 0) {
            ...
            bytes32 newRandomWord_ = keccak256(abi.encodePacked(bytes32(randomWord_), bytes32(numberIndex_)));
            return uint256(newRandomWord_) % IL2UnicornRule(l2UnicornRule).modNumber();
        }
    }*/

    function getHatchRandomNumber(uint256 randomWord_) public view returns (uint256){
        return randomWord_ % IL2UnicornRule(l2UnicornRule).modNumber();
    }
    ...
    function viewHatchResults(uint256 requestId_) external view returns (HatchResult[] memory hatchResults){
        ...
        for (uint8 numberIndex_ = 0; numberIndex_ < hatchCount_;) {
            ...
            (uint256 randomNumber_, IL2UnicornRule.HatchRule memory hatchRule_) = _getRandomNumberAndHatchRule(requestInfo_.randomWords[numberIndex_], eSeries_);
            hatchResults[numberIndex_] = HatchResult(
                requestInfo_.user,
                requestId_,
                requestInfo_.randomWords[numberIndex_],
                eSeries_,
                numberIndex_,
                randomNumber_,
                hatchRule_.level
    ...
    function _getRandomNumberAndHatchRule(uint256 randomWord_, uint8 eSeries_) private view returns (uint256 randomNumber_, IL2UnicornRule.HatchRule memory hatchRule_){
        randomNumber_ = getHatchRandomNumber(randomWord_);
        hatchRule_ = IL2UnicornRule(l2UnicornRule).getHatchRuleByESeriesRandomNum(eSeries_, randomNumber_);
    }
