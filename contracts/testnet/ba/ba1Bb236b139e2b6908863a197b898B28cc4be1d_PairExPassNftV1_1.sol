...
contract PairExPassNftV1_1 is Ownable, Pausable, ERC1155, ERC1155Supply {
    ...
    // nft params
    uint256[12] public monthTimeStamps;
    uint256[12] public monthMintPrices;
    uint256[12] public monthSupplyAmounts;
    ...
    event TimeStampsArrayUpdated(uint[] value);
    event MonthPriceUpdated(uint id, uint value);
    event MonthPricesArrayUpdated(uint[] value);
    event MonthAmountUpdated(uint id, uint value);
    event MonthAmountsArrayUpdated(uint[] value);
    ...
    function setMonthTimeStamp(uint256 tokenId, uint timeStamp) external onlyOwner {
        require(tokenId < 12, "WRONG_TOKENID");
        monthTimeStamps[tokenId] = timeStamp;

        emit TimeStampUpdated(tokenId, timeStamp);
    }

    function setMonthTimeStampsArray(uint256[] calldata timeStamps) external onlyOwner {
        require(timeStamps.length == 12, "MONTH_MUST12");

        for(uint i = 0; i < timeStamps.length; i++){
            monthTimeStamps[i] = timeStamps[i];
        }

        emit TimeStampsArrayUpdated(timeStamps);
    }

    function setMonthPrice(uint256 tokenId, uint256 price) external onlyOwner {
        require(tokenId < 12, "WRONG_TOKENID");
    ...
    function setMonthPricesArray(uint[] calldata prices) external onlyOwner {
        require(prices.length == 12, "MONTH_MUST12");

        for(uint i = 0; i < prices.length; i++){
            monthMintPrices[i] = prices[i];
        }

        emit MonthPricesArrayUpdated(prices);
    }

    function setMonthAmount(uint256 tokenId, uint256 amount) external onlyOwner {
        require(tokenId < 12, "WRONG_TOKENID");
    ...
    function setMonthAmountsArrary(uint[] calldata amounts) external onlyOwner {
        require(amounts.length == 12, "MONTH_MUST12");

        for(uint i = 0; i < amounts.length; i++){
            monthSupplyAmounts[i] = amounts[i];
        }

        emit MonthAmountsArrayUpdated(amounts);
    }
    ...
    function isMintable(uint256 tokenId) public view returns(bool){
        ...
        if(tokenId == 11) {
            ...
            require(monthTimeStamps[1] > monthTimeStamps[11], "GOV_NOT_UPDATE_TIMESTAMP");
            endTime = monthTimeStamps[1];
        } else if (tokenId == 10) {

            require(monthTimeStamps[0] > monthTimeStamps[10], "GOV_NOT_UPDATE_TIMESTAMP");
            endTime = monthTimeStamps[0];
        } else {

            endTime = monthTimeStamps[tokenId + 2];
        }
    ...
    function isExpired(uint256 tokenId) public view returns(bool){
        ...
        if (tokenId == 11) {
            ...
            require(monthTimeStamps[0] > monthTimeStamps[11], "GOV_NOT_UPDATE_TIMESTAMP");
            endTime = monthTimeStamps[0];
        } else {

            endTime = monthTimeStamps[tokenId + 1];
        }
    ...
    function burnExpired(address account, uint256 tokenId) private {
        require(isExpired(tokenId), "NOT_EXPIRED");
        uint256 amountToBurn = balanceOf(account, tokenId);
        _burn(account, tokenId, amountToBurn);
    }
