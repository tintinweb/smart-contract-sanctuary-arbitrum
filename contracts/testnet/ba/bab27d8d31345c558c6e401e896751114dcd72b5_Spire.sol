...
abstract contract AccessControl is Context, IAccessControl {
    ...
    function _setAdminContractAccess(bool access) internal {
        ...
        _revokedAdminAccess = !_revokedAdminAccess ? (!access ? true : false) : true;
    ...
    function _setTeamContractAccess(bool access) internal {
        ...
        _revokedTeamAccess = !_revokedTeamAccess ? (!access ? true : false) : true;
...
import {
    Echoes,
    AlreadyIntializedEchoes,
    InitialEchoContestsDoNotHaveWinners,
    InvalidAdditionalEchoContestCount
} from "./Echoes.sol";
...
contract GenesisText is Echoes {
    ...
    function _createNextChapterContest(uint256 genesisTextId) internal {
        ...
        // has set a winner.
            ...
            if (!_initialEchoContestsHaveWinners(genesisTextId, nextChapterId[genesisTextId])) {
...
contract Spire is ISpire, GenesisText, ERC1155Holder, AccessControl, Ownable, ReentrancyGuard, Multicall {
    ...
    function setAdminContractAccess(bool access) external onlyOwner nonReentrant {
        _setAdminContractAccess(access);
    }

    // allows the super admin to revoke team access
    function setTeamContractAccess(bool access) external onlyOwner nonReentrant {
        _setTeamContractAccess(access);
    }

    function checkContractRole(address account) public nonReentrant {
        ...
        returns (bool canAdvance, uint256 newCurrentFlowId, uint256 newPreviousFlowId)
    {
        ...
        if (latestContestsHaveClosed && previousContestHasWinner) {
            canAdvance = true;
            (newCurrentFlowId, newPreviousFlowId) = _incrementFlowCounter(currFlowId, prevFlowId, Flow.Chapter);
        } else {
            canAdvance = false;
        }
    }
    ...
    function _advanceChapterFlowIfPossible() private returns (bool) {
        ...
        if (canAdvance) {
            ...
            if (_nextChapterId == 0) {
                if (nextEchoChapterId[newCurrentFlowId] == 0 || !_initialEchoContestsHaveWinners(newCurrentFlowId, 0)) {
                    return false;
                }
            } else {
                if (
                    !IContest(chapterContests[newCurrentFlowId][_nextChapterId - 1]).hasWinner()
                        || !_initialEchoContestsHaveWinners(newCurrentFlowId, _nextChapterId)
                ) {
                    return false;
                }
            }
    ...
    function _shouldSkipGenesisText(uint256 genesisTextId, Flow flow) internal view returns (bool) {
        ...
        return !IToggleGovernance(genesisGovernors[genesisTextId]).hasEnoughOpenGovernToggles(nextChapterId[genesisTextId] + 1);
    }
        ...
        returns (uint256 newCurrentFlowId, uint256 newPreviousFlowId)
    {
        ...
        do {
            ...
            if (previousFlowId == currentFlowId) {
                if (_shouldSkipGenesisText(currentFlowId, flow)) revert AllGenesisTextsSkipped();
            }
    ...
    function _latestEchoContestsHaveWinners(uint256 genesisTextId) internal view returns (bool) {
        return _initialEchoContestsHaveWinners(genesisTextId, _latestEchoChapterId(genesisTextId));
    }
    ...
    function getChapterContests(uint256 genesisTextId, uint256 chapterId) public view returns (address) {
