...
contract Lending {
    ...
    event PriceFetched(uint256 price, uint256 collat, PythStructs.Price response);
    ...
    function withdraw(
        ...
        PythStructs.Price memory response = pyth.getPrice(manager.priceId());

        uint256 price = uint256(uint64(response.price));
        uint256 factor = 10 ** uint256(4294967295 + 1 - uint32(response.expo));

        uint256 collateral = 0;
        uint256 debt = 0;

        uint256 _collateral = 1000 * 1e18;
        uint256 _debt = 1000 * 1e18;
        ...
        emit PriceFetched(factor, collateral, response);
    ...
    function gethf() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < managerCount; i++) {
            ...
            (uint256 _collateral, uint256 _debt) = manager
                .userBalanceInUnderlying(msg.sender);
