...
// File contracts/GamblinoLottery.sol
...
/** @title Gamblino Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract GamblinoLottery is ReentrancyGuard, ILottery, Ownable {
    ...
    uint256 public maxPriceTicketInTether = 50 ether;
    uint256 public minPriceTicketInTether = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInTether;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] tetherPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInTether;
        uint32 finalNumber;
    }
     ...
     * @param _tetherTokenAddress: address of the USDT token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _tetherTokenAddress, address _randomGeneratorAddress) {
        tetherToken = IERC20(_tetherTokenAddress);
        ...
        nonReentrant
    {
        ...
        uint256 amountTetherToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInTether,
            _ticketNumbers.length
        );

        // Transfer tether tokens to this contract
        tetherToken.safeTransferFrom(address(msg.sender), address(this), amountTetherToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInTether += amountTetherToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInTetherToTransfer
        uint256 rewardInTetherToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInTetherToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        tetherToken.safeTransfer(msg.sender, rewardInTetherToTransfer);

        emit TicketsClaim(msg.sender, rewardInTetherToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInTether) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;
        ...
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].tetherPerBracket[j] =
                ...
                _lotteries[_lotteryId].tetherPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInTether - amountToShareToWinners);

        // Transfer USDT to treasury address
        tetherToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        tetherToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInTether += _amount;
     ...
     * @dev Callable by operator
     * @param _endTime: endTime of the lottery
     * @param _priceTicketInTether: price of a ticket in USDT
     * @param _discountDivisor: the divisor to calculate the discount magnitude for bulks
     * @param _rewardsBreakdown: breakdown of rewards per bracket (must sum to 10,000)
     * @param _treasuryFee: treasury fee (10,000 = 100%, 100 = 1%)
     */
    function startLottery(
        ...
        require(
            (_priceTicketInTether >= minPriceTicketInTether) && (_priceTicketInTether <= maxPriceTicketInTether),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInTether: _priceTicketInTether,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            tetherPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInTether: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(tetherToken), "Cannot be USDT token");
     ...
     * @dev Only callable by owner
     * @param _minPriceTicketInTether: minimum price of a ticket in USDT
     * @param _maxPriceTicketInTether: maximum price of a ticket in USDT
     */
    function setMinAndMaxTicketPriceInTether(uint256 _minPriceTicketInTether, uint256 _maxPriceTicketInTether)
        external
        onlyOwner
    {
        require(_minPriceTicketInTether <= _maxPriceTicketInTether, "minPrice must be < maxPrice");

        minPriceTicketInTether = _minPriceTicketInTether;
        maxPriceTicketInTether = _maxPriceTicketInTether;
    }
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].tetherPerBracket[_bracket];
        } else {
            return 0;
        }
    }
