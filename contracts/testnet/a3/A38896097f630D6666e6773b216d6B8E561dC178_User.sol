...
abstract contract DAOAccessControlled is Context {
    ...
    modifier onlySupervisor() {
        require(authority.getAuthorities().supervisor == _msgSender(), "UNAUTHORIZED");
        _;
    }
    ...
    function setAccessNumber(uint256 _no) external onlyGovernor returns(uint256){
        accessnumber = _no;
        return accessnumber;
    }
...
contract User is IUser,Initializable, DAOAccessControlled {
    ...
    */
    ...
    function setTestUSer(uint256 _no) external onlySupervisor {
        testuserids.push(_no);
    }
}
