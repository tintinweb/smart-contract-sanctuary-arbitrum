    ...
    ReentrancyGuard
{
    ...
    uint256 public ticketPrice = 400 * 10 ** DECIMALS; // 10 ERC20
    uint256 public totalTicketCount = 179620;
    uint256 private emptyTickets = 129515;
    enum CardResult {
        LOSS,
        WIN400,
        WIN700,
    ...
    uint internal variantCount = 100;

    mapping(CardResult => uint) generationCounts;
    ...
    string internal baseURI =
        "ipfs://bafybeihvtwgfvrwcktep2kp3qbrf7lqqfe4byrkpxrehsab2vftltugmhm/";

    string internal unscratchedURI =
        "ipfs://bafybeihvtwgfvrwcktep2kp3qbrf7lqqfe4byrkpxrehsab2vftltugmhm/";

    address public stakingPoolAddress =
        0x835161723FC2C1788382eBd05B38c03Ec63acb14;

    uint public stakingPoolCut = 15;

    uint public burnCut = 7;
    ...
    event SupraRouterChanged(ISupraRouter newSupraRouter);
    ...
    constructor(
         ...
         */
        generationCounts[CardResult.WIN400] = 17620;
        generationCounts[CardResult.WIN700] = 17620;
        generationCounts[CardResult.WIN800] = 5380;
        generationCounts[CardResult.WIN1300] = 8255;
        generationCounts[CardResult.WIN2700] = 745;
        generationCounts[CardResult.WIN4000] = 100;
        generationCounts[CardResult.WIN7000] = 75;
        generationCounts[CardResult.WIN40000] = 310;

        rewardAmounts[CardResult.WIN400] = 400;
        rewardAmounts[CardResult.WIN700] = 700;
        rewardAmounts[CardResult.WIN800] = 800;
        rewardAmounts[CardResult.WIN1300] = 1300;
        rewardAmounts[CardResult.WIN2700] = 2700;
        rewardAmounts[CardResult.WIN4000] = 4000;
        rewardAmounts[CardResult.WIN7000] = 7000;
        rewardAmounts[CardResult.WIN40000] = 40000;
    ...
    function setSupraRouter(ISupraRouter _supraRouter) external onlyOwner {
        supraRouter = _supraRouter;
    ...
    function startMint(uint8 ticketCount) external nonReentrant {
         ...
         */
        require(ticketCount > 0, "Can't mint 0 token");
        require(ticketCount <= 20, "Can't mint more than 20 token");
    ...
    function _calculateResult(
        ...
        uint index = (randomNo % 10e6) % variantCount;
        uri = string(
            abi.encodePacked(
    ...
    function scratchAndClaimAllCardsTreasury(address user) external {
        require(msg.sender == address(treasury), "Only treasury can call this");
        userScratchedAllAt[user] = block.timestamp;
        userClaimedAllAt[user] = block.timestamp;
    }
    ...
    function getCardScratched(uint256 tokenId) internal view returns (bool) {
        if (cardMintedAt[tokenId] > userScratchedAllAt[msg.sender]) {
    ...
    function getAllScratchedCards() external view returns (uint256[] memory) {
        uint256[] memory tokenIds = new uint256[](balanceOf(msg.sender));
        uint256 counter = 0;
        for (uint256 i = 0; i < totalSupply(); i++) {
            if (ownerOf(i) == msg.sender) {
                if (getCardScratched(i)) {
                    tokenIds[counter] = i;
                    counter++;
                }
            }
    ...
    function getCardClaimed(uint256 tokenId) internal view returns (bool) {
        if (cardMintedAt[tokenId] > userClaimedAllAt[msg.sender]) {
    ...
    function getAllClaimedCards() external view returns (uint256[] memory) {
        uint256[] memory tokenIds = new uint256[](balanceOf(msg.sender));
        uint256 counter = 0;
        for (uint256 i = 0; i < totalSupply(); i++) {
            if (ownerOf(i) == msg.sender) {
    ...
    function getAllCards() external view returns (uint256[] memory) {
        uint256[] memory tokenIds = new uint256[](balanceOf(msg.sender));
        uint256 counter = 0;
        for (uint256 i = 0; i < totalSupply(); i++) {
            if (ownerOf(i) == msg.sender) {
