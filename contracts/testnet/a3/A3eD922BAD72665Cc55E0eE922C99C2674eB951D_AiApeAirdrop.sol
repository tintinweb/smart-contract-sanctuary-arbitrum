...
contract AiApeAirdrop is Ownable, ReentrancyGuard {
    ...
    function getAllNft(
        address myAddress
    ) public view returns (uint256[] memory nftAllItem) {
        ERC721A nft = ERC721A(nftAddress);
        uint256 nftAmount = nft.totalSupply();
        nftAllItem = new uint256[](nftAmount);
        uint256 count = 0;
        ...
        // 调整数组长度为实际符合条件的 NFT 数量
        assembly {
            mstore(nftAllItem, count)
        }
        return nftAllItem;
    }
