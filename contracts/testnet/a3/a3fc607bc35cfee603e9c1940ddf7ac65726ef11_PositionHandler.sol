...
contract PositionHandler is PositionConstants, IPositionHandler, BaseExecutor {
    ...
    */
    ...
    function forceClosePosition(bytes32 _key, uint256[] memory _prices) external {
        ...
        (bool hasProfit, uint256 pnl) = vaultUtils.calculatePNL( _key, _getFirstParams(_prices));
        require(
    ...
    function _triggerPosition(
        ...
        settingsManager.updateCumulativeFundingRate(_position.indexToken, _collateralToken, _isLong);
        uint8 statusFlag = vaultUtils.validateTrigger(_isLong, _indexPrice, _order);
    ...
    function _confirmDelayTransaction(
        ...
        bytes32 key = _getPositionKey(_position.owner, _position.indexToken, _isLong, _posId);
        vaultUtils.validateConfirmDelay(_position.owner, _position.indexToken, _isLong, _posId, true);
    ...
    function _liquidatePosition(
        ...
        settingsManager.updateCumulativeFundingRate(_position.indexToken, _collateralToken, _isLong);
        bytes32 key = _getPositionKey(_position.owner, _position.indexToken, _isLong, _posId);
        uint256 previousFee = _position.openedFee;
        _position.openedFee = 0;
        ...
        positionKeeper.updateGlobalShortData(key, _position.size, _indexPrice, false);
        ...
        if (!_isLong && fee < _position.collateral) {
        ...
        settingsManager.decreaseOpenInterest(_position.indexToken, _position.owner, _isLong, _position.size);
        vault.decreaseReservedAmount(_collateralToken, _position.reserveAmount);
        vault.decreasePoolAmount(_collateralToken, fee);
        positionKeeper.emitLiquidatePositionEvent(key, _indexPrice, fee);
    }
    ...
    function _decreasePosition(
        ...
        settingsManager.updateCumulativeFundingRate(_position.indexToken, _collateralToken, _position.isLong);
        ...
        //Scope to avoid stack too deep error
        {
            key = _getPositionKey(_position.owner, _position.indexToken, _position.isLong, _position.posId);
            positionKeeper.updateGlobalShortData(key, _sizeDelta, _indexPrice, false);
            collateral = _position.collateral;
            (posData, hasProfit, isParitalDecrease, _position) = _beforeDecreasePosition(
                _collateralToken, 
                _sizeDelta, 
                _indexPrice, 
                _position
        ...
        if (!hasProfit && posData[3] > 0 && !_position.isLong) {
        ...
        if (posData[0] > 0) {
            //Decrease pool amount if usdOut > 0
        ...
        if (posData[1] <= posData[0]) {
            ...
            //Distribute fee
            vault.distributeFee(key, _position.owner, posData[1]);
        }
    }
    ...
    function _beforeDecreasePosition(
        ...
        uint256 _indexPrice, 
        Position memory _position
    ) internal view returns (uint256[4] memory posData, bool hasProfit, bool isParitalDecrease, Position memory) {
       //posData: [usdOut, fee, collateralDelta, adjustedDelta]
        ...
        return (posData, hasProfit, isParitalDecrease, _position);
    }
    ...
    function _increasePosition(
        ...
        _position.openedFee += fee;
        _position.collateral += _amountIn;
        _position.reserveAmount += _amountIn;
        _position.entryFundingRate = settingsManager.cumulativeFundingRates(_position.indexToken, _isLong);
        _position.size += _sizeDelta;
        _position.lastIncreasedTime = block.timestamp;
        _position.lastPrice = _indexPrice;
        vault.accountDeltaAndFee(true, 0, fee);
        ...
        settingsManager.increaseOpenInterest(_collateralToken, _position.owner, _isLong, _sizeDelta);
        positionKeeper.unpackAndStorage(_key, abi.encode(_position), DataType.POSITION);
        ...
        if (_isLong) {
            ...
            vault.increaseGuaranteedAmount(_collateralToken, _sizeDelta + _position.openedFee - _amountIn);
            vault.decreasePoolAmount(_collateralToken, fee);
        } 
...
interface ISettingsManager {
    ...
    function updateCumulativeFundingRate(address _indexToken, address _collateralToken, bool _isLong) external;
