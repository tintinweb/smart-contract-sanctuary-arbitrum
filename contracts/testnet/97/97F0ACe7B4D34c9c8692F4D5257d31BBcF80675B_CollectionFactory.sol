...
contract CollectionFactory is NonblockingLzApp {
    ...
    function estimateFees(string memory chSym, string memory URI, string[] memory URIs, uint gas) external view returns (uint) {
        bytes memory payload = _getPayload("", URI, "", 5, 0, 0, msg.sender, URIs);
        uint16 chId = uint16(chainSymbolToId[chSym]);
