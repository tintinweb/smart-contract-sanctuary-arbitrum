...
contract AntiBotStandardBabyToken is ERC20Upgradeable, OwnableUpgradeable, BaseToken {
    using SafeMath for uint256;
...
contract AntiBotBabyTokenFactory is TokenFactoryBase {
    AntiBotStandardBabyToken[] public babyTokens;
    ...
    function create(
        ...
        address antiBot_
            ...
            AntiBotStandardBabyToken tokenAddress = new AntiBotStandardBabyToken();
            token = address(tokenAddress);
            AntiBotStandardBabyToken(payable(token)).initialize(
            owner,
            name,
            symbol,
            totalSupply,
        ...
        babyTokens.push(AntiBotStandardBabyToken(payable(token)));
        assignTokenToOwner(owner, token, 4);
        emit TokenCreated(owner, token, TokenType.baby, 1);
    }

    function getTokens() public view returns(AntiBotStandardBabyToken[] memory) {
        return babyTokens;
    }
}