...
contract GainlingsLibrary is Ownable {
    ...
    function splitRandomInThree(uint256 numbers) public pure returns (uint256[3] memory randoms){
        uint256 a = 999;
         return [
            numbers % a,
            (numbers / a) % a,
...
// File: Gainlings/GainlingsTrophyFactory.sol
...
contract GainlingsTrophyFactory is ERC721Enumerable, RrpRequesterV0, ReentrancyGuard, Ownable
{
    ///// API3 \\\\
    ...
    ///// interfaces \\\\
    GainlingsTrophyStorage _traitStorage;
    ...
    uint256 private _trophyPrice = 10;
    ...
    mapping(address => uint256)   public _attacks;
    mapping(address => uint256)   public _claims;

    string[] private _traitTypeNames = ["Body","Blood","Helmet"]; 
    
    
    constructor( address _airnodeRrp) 
    ERC721("The Gainlings Trophies", "TGT") 
    ...
    function makeRequestToAirnode() internal returns (bytes32 requestId) {
    ...
    function mintQRNG(uint256 amount)  external nonReentrant{
        uint256 attacks =  _attacks[msg.sender]; // 20
        uint256 claims =  _claims[msg.sender]; // 11 
        require(totalSupply() + amount <= _totalMint, "supply");
        require(claims + _trophyPrice <= attacks, "not enough trophies");
        _claims[msg.sender] = claims + _trophyPrice;

         mint();
    }
    function increaseKillCount(address receiver, bytes32[] calldata _proof) external nonReentrant{
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(MerkleProof.verify(_proof, merkleRoot, leaf), "list");
        _attacks[receiver] = _attacks[receiver]  + 1;
    }
    function mint() internal {
        uint256 supply = totalSupply();
        bytes32 requestId = makeRequestToAirnode();

        requestToWaitingState[requestId] = true;
        requestToTokenId[requestId] = supply + 1;

        _safeMint(msg.sender, supply + 1);
        
        emit TrophyMinted(msg.sender,supply+1);
    }
    ...
    function fulfillRandomnessMint(bytes32 requestId, bytes calldata data) external onlyAirnodeRrp{
        ...
        _tokenSeeds[tokenId] = qrngUint256;
         ...
         emit TrophySeeded(tokenId,qrngUint256);
    ...
    function buildTraitsFromSeed(uint256 numbers) internal view returns (string[3] memory traits){
        uint256[3] memory randoms = _gainlingsLibrary.splitRandomInThree(numbers); 
    ...
    function getRandomTrait(uint256 random, uint256 traitTypeNr) internal  view  returns (string memory trait) {
        ...
        for (uint256 i = 0; i < possibleTraits.length; i++) {
            if (random < possibilities[i]) {
                trait = possibleTraits[i];
            }
        }
        return trait;
    }
    function setContractParams(address traitStorageAddress,address libraryAddress) external onlyOwner {
        _traitStorageAddress = traitStorageAddress;
        _traitStorage = GainlingsTrophyStorage(_traitStorageAddress);
    ...
    function getRevealedToken(uint256 tokenId,bool encode,uint256 seeed)internal view returns (string memory){
        ...
        string [3] memory stringTraits = buildTraitsFromSeed(seeed);
        string memory svg = _traitStorage.getCompleteImage(stringTraits);
        bytes memory dataURI = getTokenUriStart(tokenId,svg);
    ...
    function getTokenUriStart(uint256 tokenId,string memory svg)internal view returns (bytes memory){
        string memory animationUri = string(abi.encodePacked(_animationBaseUri,"?tokenId=", tokenId.toString()));
        return  abi.encodePacked(
            '{',
                '"name": "Gainlings Trophy #',tokenId.toString(),'",', //name of the token
                '"description":"A skin for scalps",', //description
                '"animation_url":"',animationUri,'",', //webgl url
                '"external_url":"',animationUri,'",', //webgl url
