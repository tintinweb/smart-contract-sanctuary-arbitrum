...
import "./interfaces/IUSDTArbitrumExtension.sol";

contract InfiniteContinent is OwnableUpgradeable, PausableUpgradeable, ERC721EnumerableUpgradeable {
    ...
    address _tetherArbitrumExtension;
    address _cashier;
    ...
    event Present(address purchaser, Community community, uint[5] tokenIds);
    ...
    function __InfiniteContinent_init(
        ...
        address cashier,
        address tetherArbitrumExtensionAddress,
        ...
        _pause();
        _tetherArbitrumExtension = tetherArbitrumExtensionAddress;
        _cashier = cashier;
    ...
    function purchase(Community community, uint timestamp, uint[5] calldata tokenIds, bytes calldata signature) external whenNotPaused {
        ...
        IUSDTArbitrumExtension(_tetherArbitrumExtension).transferFrom(purchaser, address(this), price);
        ...
        if (community == Community.A || community == Community.B) {
            ...
            for (uint i = 0; i < 5; ++i) {
                giftToken.transferFrom(officialAddress, purchaser, tokenIds[i]);
            }

            _records[community] += 1;
            emit Present(purchaser, community, tokenIds);
        }
    ...
    function setTetherArbitrumExtension(address newTetherArbitrumExtension) external onlyOwner {
        _tetherArbitrumExtension = newTetherArbitrumExtension;
    }

    function getTetherArbitrumExtension() external view returns (address) {
        return _tetherArbitrumExtension;
    }
    ...
    function withdrawPayment(uint amount) external {
        ...
        IUSDTArbitrumExtension(_tetherArbitrumExtension).transfer(cashier, amount);
    }
...
interface IUSDTArbitrumExtension {
...
library Pricing {
    function tokenPricing(uint tokenId) internal pure returns (uint){
        ...
        return (tokenId - 301) / 100 * 2000e6 * 3 / 100 + 2000e6;
    }
}
...
import "../InfiniteContinent.sol";

// NOTE: deployed on testnet for test
contract InfiniteContinentTest is InfiniteContinent {
    function __InfiniteContinentTest_init(
        string memory name,
        string memory symbol,
        address cashier,
        address tetherArbitrumExtensionAddress,
        ...
        __InfiniteContinent_init(
            name,
            symbol,
            cashier,
            tetherArbitrumExtensionAddress,
            giftTokenAddress,
            officialAddress,
            verifierAddress
        );
    ...
    function setNextTokenId_ONLY_FOR_TEST(uint tokenId) external onlyManager {
        require(tokenId > 200 && tokenId <= 10000, "invalid token id");
        _idCounterPurchase._value = tokenId;
    }
