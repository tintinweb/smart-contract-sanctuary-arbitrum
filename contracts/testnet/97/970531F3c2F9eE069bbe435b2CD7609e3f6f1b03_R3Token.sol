    ...
    OwnableUpgradeable
{
    ...
    //     0x0aA363C0C8EfAde9690a3C769B6238BC28de552a;
        ...
        initializer
    {
        ...
        // addPair(_factory.createPair(address(this), '0x82af49447d8a07e3bd95bd0d56f35241523fbab1')); // Arbitrum MainNet - WETH
        //// addPair(_factory.createPair(address(this), '0xdd69db25f6d620a7bad3023c5d32761d353d3de9')); // Arbitrum MainNet - GETH
        ...
        _mobilityAddress = 0x70997970C51812dc3A010C7d01b50e0d17dc79C8;
        ...
        transfer(_mobilityAddress, (_totalSupply * 15) / 100);

        // 30%（早期投资人钱包，数量30 0000 0000）
        transfer(_earlyInvestorsAddress, (_totalSupply * 30) / 100);

        // 10%（战略储备钱包，数量10 0000 0000）
        transfer(_strategicReserveAddress, (_totalSupply * 10) / 100);

        // 2%（捐赠钱包，数量2 0000 0000）
        transfer(_donateAddress, (_totalSupply * 2) / 100);

        // 8%（空投余额钱包，数量8 0000 0000；每次空投转出至：0xcE199E5A7f8832D77DE0730C3E1d111Bca61EBc8)
        transfer(_airdropAddress, (_totalSupply * 8) / 100);

        // 35%（质押钱包，数量35 0000 0000）
        transfer(_pledgeAddress, (_totalSupply * 35) / 100);
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        unchecked {
            _supplyOwned[account] = accountBalance - amount;
    ...
    function totalTaxes() public view returns (uint256) {
        return _assetManagementTax + _marketingTax + _pledgePoolTax;
    }
    ...
    function _transfer(
        ...
        unchecked {
            _supplyOwned[sender] -= afterTaxAmount;
    ...
    function transferTaxes() public onlyOwner {
        ...
        // 资管可兑换数量
        uint256 assetManagementAmount = (contractBalance *
            10 *
            _assetManagementTax) / 100;
        // 营销可兑换数量
        uint256 marketingAmount = (contractBalance * 10 * _marketingTax) / 100;

        // 质押池可兑换数量
        uint256 pledgePoolAmount = (contractBalance * 10 * _pledgePoolTax) /
            100;
        ...
        // 对3个地址余额累加
        _supplyOwned[_assetManagementTaxAddress] += assetManagementAmount;
        _supplyOwned[_marketingTaxAddress] += marketingAmount;
        _supplyOwned[_pledgePoolTaxAddress] += pledgePoolAmount;
    }
    ...
    function _takeTaxes(uint256 amount) internal returns (uint256) {
        // 计算资管部分数量
        uint256 assetManagementTaxAmount = (amount * _assetManagementTax) / 100;

        // 计算营销部分数量
        uint256 marketingTaxAmount = (amount * _marketingTax) / 100;

        // 计算质押部分数量
        uint256 pledgePoolTaxAmount = (amount * _pledgePoolTax) / 100;
