...
import {
    ...
    IUniswapV2Router02,
    IWrappedNativeToken
} from "../interfaces/ICashmereAggregator.sol";

contract CashmereAggregatorUniswap is ICashmereAggregator, AccessControl, ReentrancyGuard, EIP712 {
    ...
    mapping(bytes32 swapId => PendingSwap swaps) private _pendingSwaps;

    mapping(uint16 chainId => mapping(bytes32 swapId => bool continued)) private _continuedSwaps;
    ...
    function startSwap(SwapParams memory params) external payable {
        // Replicate our param in memory (direct replication has minor impact : 450813 vs 450746)
        // TODO : Only replicate essential part

        // Verify base param's
        if (params.lwsPoolId == 0 || params.hgsPoolId == 0) revert InvalidLwsGws();

        // Verify the signer who called this method
        bytes32 hashedData = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
        ...
        if (recover(hashedData, params.signature) != msg.sender) revert InvalidSignature();
        ...
        if (address(params.srcToken) == NATIVE_TOKEN) {
            if (params.srcAmount > value) revert InsuficiantValue();
        ...
        bytes memory payload = abi.encodePacked(
            params.lwsPoolId, params.hgsPoolId, params.dstToken, params.minHgsAmount, msg.sender, params.signature
        );
    ...
    function continueSwap(ContinueSwapParams calldata params) external onlyRole(CONTINUE_EXECUTOR_ROLE) {
        require(!_continuedSwaps[params.srcChainId][params.id], "already continued");
        ICrossRouter.SwapMessage memory swapMsg = _bridge.getReceivedSwaps(params.srcChainId, params.id);
...

        {
            ...
            uint256 s;
            uint8 v;
            uint256 srcChainId = params.srcChainId;

            // TODO: check for dirty bits
            assembly ("memory-safe") {
                mstore(payload, srcChainId)
                mstore(add(payload, 0x20), 0x00)

                let currentOffset := add(0x20, data)
                let firstChunk := mload(currentOffset)
                mstore(add(payload, 0x40), shr(240, firstChunk)) // lwsPoolId
                mstore(add(payload, 0x60), shr(224, firstChunk)) // hgsPoolId
                mstore(add(payload, 0x80), shr(64, firstChunk)) // dstToken

                currentOffset := add(currentOffset, 0x18)
                mstore(add(payload, 0xa0), mload(currentOffset)) // minHgsAmount
                mstore(add(payload, 0xc0), shr(96, mload(add(0x20, currentOffset)))) // receiver

                currentOffset := add(currentOffset, 0x34)
                r := mload(currentOffset)
                s := mload(add(currentOffset, 0x20))
                v := shr(248, mload(add(currentOffset, 0x40)))

                mstore(0x40, add(payload, 0x160))
            }
            payload.signature = abi.encodePacked(r, s, v);
        }

        // Verify the signer who called this method
        AggregatorInfo storage aggregatorInfo = _aggregatorInfos[swapMsg.srcChainId];
        bytes32 hashedData = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
            ...
            aggregatorInfo.chainId,
            aggregatorInfo.srcAggregatorAddress
        );
        if (recover(hashedData, payload.signature) != payload.receiver) revert InvalidSignature();

        // hgs -> dst
        bool isDstNative = address(payload.dstToken) == NATIVE_TOKEN;
        if (isDstNative) {
            payload.dstToken = IERC20(_wrappedNativeToken);
        }

        IERC20 hgsToken = IERC20(IAsset(_assetRouter.getPool(payload.hgsPoolId).poolAddress).token());
        uint256 returnAmount;
        if (hgsToken != payload.dstToken /*&& params.router1Inch != address(0)*/ ) {
            ...
            _approve(hgsToken, address(_uniswap), swapMsg.amount);
            //            (bool success, ) = params.router1Inch.call{ value: 0 }(params.data);
            //            require(success, "!inSuccess");
            _uniswapSwap(hgsToken, payload.dstToken, swapMsg.amount);
        ...
        _continuedSwaps[params.srcChainId][params.id] = true;
    ...
    function updateBridge(IBridge bridge) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _bridge = bridge;
    }
    ...
    function getPendingSwaps(bytes32 swapId) external view returns (PendingSwap memory) {
        return _pendingSwaps[swapId];
    }

    function getContinuedSwaps(uint16 chainId, bytes32 swapId) external view returns (bool) {
        return _continuedSwaps[chainId][swapId];
    }
...
interface ICashmereAggregator {
    ...
    struct PendingSwap {
        ...
        // Fourth storage slot
        bool processed; // 2 bytes -> uint8
        address receiver; // 20 bytes -> uint160 -> Remain 10 bytes free
        // Fifth storage slot
        uint256 minHgsAmount;
        // Sixth storage slot
        bytes signature; // Slot pointer, since dynamic size
    }
    ...
    function continueSwap(ContinueSwapParams calldata params) external;
    ...
    function updateBridge(IBridge bridge) external;
...
abstract contract EIP712 {
    ...
    mapping(address addressThis => mapping(uint256 chainId => bytes32 domainSeparator)) private _cachedDomainSeparators;
    ...
    constructor(string memory name, string memory version) {
        ...
        _cachedDomainSeparators[address(this)][block.chainid] =
            _buildDomainSeparator(typeHash, hashedName, hashedVersion, block.chainid, address(this));
     ...
     */
    function _domainSeparatorV4(uint256 chainid, address srcAddress) internal returns (bytes32 separator) {
        separator = _cachedDomainSeparators[srcAddress][chainid];
        // If it wasn't build before we build it now
        if (separator == bytes32(0)) {
            separator = _buildDomainSeparator(_TYPE_HASH, _HASHED_NAME, _HASHED_VERSION, chainid, srcAddress);
            _cachedDomainSeparators[srcAddress][chainid] = separator;
        }
    }
    ...
    {
        bytes32 separator = _domainSeparatorV4(chainid, srcAddress);
        assembly ("memory-safe") {
            // Compute the digest.
            mstore(0x00, 0x1901000000000000) // Store "\x19\x01".
            mstore(0x1a, separator) // Store the domain separator.
            mstore(0x3a, structHash) // Store the message hash.
    ...
    /// and the `signature`.
    /// From: https://github.com/Vectorized/solady/blob/main/src/utils/ECDSA.sol
    ///
