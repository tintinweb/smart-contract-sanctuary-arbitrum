...
contract AdventurerEquipment is IAdventurerEquipment, ManagerModifier {
  ...
  mapping(address => mapping(uint => mapping(uint => uint32))) public currentEquippedIds;

  mapping(uint256 => bool) slotState;

  uint16[] public currentEnabledSlots;
  ...
  constructor(
    ...
    currentEnabledSlots = new uint16[](4);
  ...
  function equip(
    ...
    for (uint j = 0; j < _slots.length; j++) {
      require(slotState[_slots[j]], "Slot disabled");
      require(uint(currentEnabledSlots[j]) == _slots[j]);
    }
    ...
    mapping(uint => uint32) storage equippedIds = currentEquippedIds[_adventurerAddr][
      _adventurerId
    ];
    uint slot;
    uint itemId;
  ...
  function equipBatch(
    ...
    for (uint i = 0; i < _adventurerAddresses.length; i++) {
      ...
      mapping(uint => uint32) storage equippedIds = currentEquippedIds[adventurerAddress][
        adventurerTokenId
      ];
  ...
  function getEquippedBatch(
    ...
    uint[] memory slots = _getCurrentSlots();

    for (uint i = 0; i < _adventurerAddresses.length; i++) {
      result[i] = new uint[](slots.length);
      for (uint j = 0; j < slots.length; j++) {
        result[i][j] = uint(
          currentEquippedIds[_adventurerAddresses[i]][_adventurerTokenIds[i]][slots[j]]
        );
      }
    }
  }
  ...
  function getEquipped(
    ...
    result = new uint[](slots.length);
    for (uint j = 0; j < slots.length; j++) {
      result[j] = uint(currentEquippedIds[_adventurerAddress][_adventurerTokenId][slots[j]]);
    }
  }
  ...
  function _getCurrentSlots() internal view returns (uint[] memory slots) {
    slots = new uint[](currentEnabledSlots.length);
    for (uint i = 0; i < currentEnabledSlots.length; i++) {
      slots[i] = currentEnabledSlots[i];
    }
  }
  ...
  function updateSlots(uint16[] calldata _slots) external onlyAdmin {
    for (uint i = 0; i < currentEnabledSlots.length; i++) {
      slotState[currentEnabledSlots[i]] = false;
    }
    if (currentEnabledSlots.length != _slots.length) {
      currentEnabledSlots = new uint16[](_slots.length);
    }

    for (uint i = 0; i < _slots.length; i++) {
      slotState[_slots[i]] = true;
