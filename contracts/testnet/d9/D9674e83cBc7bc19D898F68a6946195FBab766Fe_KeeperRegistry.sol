  ...
  ERC677ReceiverInterface
{
  ...
  //bytes17 private constant OPTIMISM_L1_FEE_DATA_PADDING = 0xffffffffffffffffffffffffffffffffff;
  ...
  PaymentModel public s_paymentModel;
  uint256 public s_l1;
  uint256 public s_l2;
  ...
  constructor(
    ...
    s_paymentModel = PaymentModel(paymentModel);
  ...
  function registerUpkeep(
    ...
    _createUpkeep(id, target, gasLimit, admin, 1000000000000000000000, checkData);
    s_storage.nonce++;
  ...
  function setDataDigits(uint256 digits) public onlyOwner {
    s_data_digits = digits;
  }

  function setPaymentPremiumPPB(uint32 paymentPremiumPPB) public onlyOwner {
    s_storage.paymentPremiumPPB = paymentPremiumPPB;
  }

  function setFallbackGasPrice(uint256 fallbackGasPrice) public onlyOwner {
    s_fallbackGasPrice = fallbackGasPrice;
  }

  function setFallbackLinkPrice(uint256 fallbackLinkPrice) public onlyOwner {
  ...
  function getMaxPaymentForGas(uint256 gasLimit) public returns (uint96 maxPayment) {
    ...
    if (s_paymentModel == PaymentModel.Optimism) {
      //data = bytes.concat(msg.data, s_data_padding);
      data = new bytes(s_data_digits);
    }
  ...
  function _calculatePaymentAmount(
    ...
    uint256 linkEth,
    bytes memory data
  ) private returns (uint96 payment) {
    uint256 weiForGas = gasWei * (gasLimit + REGISTRY_GAS_OVERHEAD);
    uint256 premium = PPB_BASE + s_storage.paymentPremiumPPB; // 1 700 000 000
    uint256 l1CostWei = 0;
    if (s_paymentModel == PaymentModel.Arbitrum) {
      // probably need some hacks to estimate the max payment
      l1CostWei = ARBITRUM_ORACLE.getCurrentTxL1GasFees();
    } else if (s_paymentModel == PaymentModel.Optimism) {
      l1CostWei = OPTIMISM_ORACLE.getL1Fee(data);
    }
    //    uint96 total = ((weiForGas + l1CostWei) * 1e9 * premium) /
    //      (linkEth) +
    //      (uint256(s_storage.flatFeeMicroLink) * (1e12));
    uint256 l1 = (l1CostWei * 1e9 * premium) / linkEth;
    uint256 l2 = (weiForGas * 1e9 * premium) / linkEth;
    uint256 keeperProfit = s_storage.flatFeeMicroLink * 1e12;
    uint96 total = uint96(l1 + l2 + keeperProfit);
    s_l1 = l1;
    s_l2 = l2;
    s_gasWei = gasWei;
  ...
  function _performUpkeepWithParams(PerformParams memory params) private nonReentrant returns (bool success) {
    ...
    success = true; //_callWithExactGas(params.gasLimit, upkeep.target, callData);
