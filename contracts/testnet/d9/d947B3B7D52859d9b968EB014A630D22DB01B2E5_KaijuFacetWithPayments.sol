...
contract KaijuFacet is SolidStateERC1155, IERC2981, WithStorage, WithModifiers {
    ...
    function _purchasePioneerKaiju(
        ...
        if (_paymentType == PaymentType.ETH_IN_USD && msg.value != _paymentAmountInToken) {
            revert LibMeta.IncorrectPaymentAmount(msg.value, _paymentAmountInToken);
        }

        LibToken.PioneerPackType expectedPackType = _constants().usdPriceToPackType[_paymentAmountInUSD];
...
contract KaijuFacetWithPayments is IPaymentsReceiver, KaijuFacet {
    ...
    function getMagicTokenAddress() internal view returns (address) {
        return _constants().magicTokenAddress;
    }

    function getArbTokenAddress() internal view returns (address) {
