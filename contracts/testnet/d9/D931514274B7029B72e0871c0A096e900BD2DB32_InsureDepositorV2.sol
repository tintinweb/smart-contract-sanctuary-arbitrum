...
contract InsureDepositorV2 is InsureDepositorV1, IRewardForwarder {
    ...
    /// @notice reward contract for token distribution
    ...
    event RewardContractSet(address _rewardContract);
    ...
    /**
     * @notice set reward contract to distribute a reserve pool token
     */
     ...
     */
    function setDedicatedMsgSender(
        ...
        returns (uint256 _fee)
    {
        ...
        IERC20Upgradeable(reservePoolLP).safeTransfer(rewardContract, _fee);
        // update reward information on the reward contract
        IStakingRewards(rewardContract).notifyRewardAmount(_fee);

        lastForwardTime = block.timestamp;

        emit FeeForwarded(rewardContract, _fee);
    }
