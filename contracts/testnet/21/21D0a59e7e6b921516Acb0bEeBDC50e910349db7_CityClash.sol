...
contract CityClash is CityClashScore {
    ...
    function uploadMetadata(CityClashTypes.CityWithId[] memory _cities) external onlyOwner {
        ...
        for(uint i = 0; i < _cities.length; i++) {
            ...
            if(idToCities[_cities[i].id].points == 0 && _exists(_cities[i].id)) {
                idToCities[_cities[i].id] = CityClashTypes.City(_cities[i].city, _cities[i].country, _cities[i].points, _cities[i].cityFaction, 0);
    ...
    function upgradeCity(uint _tokenId, uint _towerTokenId) external {
        require(ownerOf(_tokenId) == msg.sender, "You must be the owner of the city to upgrade it");
        CityClashTowersInterface cityClashTowers = CityClashTowersInterface(cityClashTowersAddress);
        require(cityClashTowers.ownerOf(_towerTokenId) == msg.sender, "You ust be the owner of the tower to upgrade your city");

        uint amountToUpgrade = cityClashTowers.idToNumStories(_towerTokenId) / 2;
        CityClashTypes.City storage city = idToCities[_tokenId];
        // city.points += amountToUpgrade;

        //TODO: update country score
...
contract CityClashNFT is ERC721, Ownable, ReentrancyGuard {
    ...
    string baseUrl = "";
    address cityClashTowersAddress;
    ...
    constructor(/*address _cityClashTowersAddress*/) ERC721("CityClash", "City Clash is a P2E, social strategy game where 3 factions compete by buying and selling NFTs based on real world cities. The game is built entirely on-chain, and is fully functional at mint time!") {
        // cityClashTowersAddress = _cityClashTowersAddress;
