...
contract SynthrDebtShare is Owned, MixinSystemSettings {
    using SafeMath for uint256;
    ...
    bytes32 public constant CONTRACT_NAME = "SynthrDebtShare";
    ...
    function balanceOfOnPeriod(address account, uint256 periodId) public view returns (uint256) {
        ...
        require(i < 0, "SynthrDebtShare: not found in recent history");
        return 0;
    }
    ...
    function debtRatio() public view returns (uint256) {
        ...
        return
            totalSupplyOnPeriod[currentPeriodId] == 0
                ? SafeDecimalMath.preciseUnit()
                : totalIssuedSynthr.divideDecimalRoundPrecise(totalSupplyOnPeriod[currentPeriodId]);
    }
    ...
    function mintShare(address account, uint256 amount) external onlyIssuer {
        ...
        totalSupplyOnPeriod[currentPeriodId] = totalSupplyOnPeriod[currentPeriodId] + amount;
    ...
    function _increaseBalance(address account, uint256 amount) internal {
        ...
        if (accountBalanceCount == 0) {
            ...
            uint128 newAmount = uint128(uint256(balances[account][accountBalanceCount - 1].amount) + amount);
    ...
    function _deductBalance(address account, uint256 amount) internal {
        ...
        require(accountBalanceCount != 0, "SynthrDebtShare: account has no share to deduct");
    ...
    modifier onlyIssuer() {
        require(
            ...
            "SynthrDebtShare: only issuer and synthr bridge can mint/burn"
        );
        _;
    }
    ...
    modifier onlyAuthorizedToSnapshot() {
        require(
            ...
            "SynthrDebtShare: not authorized to snapshot"
        );
        _;
    }
