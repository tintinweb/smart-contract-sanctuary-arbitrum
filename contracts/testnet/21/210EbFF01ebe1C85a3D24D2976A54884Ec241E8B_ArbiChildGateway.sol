/**
 *Submitted for verification at Arbiscan on 2022-12-13
*/
...
// File: arbos-precompiles/arbos/builtin/ArbSys.sol
...
// File: contracts/arbitrum/adamArbitrum.sol
...
contract adamArbitrum is ERC721 {
    ...
    function bridgeMintWithURI(address to, uint256 tokenId, string memory tokenURI) external onlyGateway {
        _mint(to, tokenId);
...
// File: contracts/arbitrum/ArbiChildGateway.sol
...
contract ArbiChildGateway is IERC721Receiver,  Ownable{
    ...
    function applyL1ToL2Alias(address l1Address) external pure returns (address l2Address) {
        l2Address = AddressAliasHelper.applyL1ToL2Alias(l1Address);
    }
    ...
    /// @param l2Address L2 address as viewed in msg.sender
    /// @return l1Address the address in the L1 that triggered the tx to L2
    function undoL1ToL2Alias(address l2Address) external pure returns (address l1Address) {
        l1Address = AddressAliasHelper.undoL1ToL2Alias(l2Address);
    }
    ...
    function withdraw(
        ...
        // TODO: necessary?
        require(
            adamArbitrum(l2Token).l1Address() == l1Token,
            "INVALID_TOKEN"
        );
        adamArbitrum(l2Token).bridgeBurn(msg.sender, tokenId);

        bytes memory _l1MessageCallData = abi.encodeWithSelector(
            IGateway.finalizeWithdraw.selector,
            l1Token,
            tokenId,
            to
        );
    ...
    function finalizeDeposit(
        ...
        adamArbitrum(l2Token).bridgeMint(to, tokenId);
    }
    ...
    function finalizeDepositWithURI(
        ...
        require(l1ToL2Token[l1Token] == l2Token, "WRONG_ADDRESS_PAIRING");
        adamArbitrum(l2Token).bridgeMintWithURI(to, tokenId,tokenURI);
    }

    function finalizeRegistrationByOwner(
        address l1Address, address l2Address
    ) external onlyOwner {
