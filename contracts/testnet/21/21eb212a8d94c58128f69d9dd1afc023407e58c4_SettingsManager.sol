    ...
    */
    function validateCollateralPathAndCheckSwap(address[] memory _collateralPath) external override view returns (bool) {
        if (isEnableNonStableCollateral) {
            require(_collateralPath.length == 1, "IVLCPL/1"); //Invalid collateral path length, must be 1

            if (!_isApprovalCollateralToken(_collateralPath[0])) { 
                revert("IVLACT"); //Invalid approval collateral token
            }

            return false;
        } else {
            require(_collateralPath.length == 1 || _collateralPath.length == 2, "IVLCPTL/12"); //Invalid collateral path length, must be 1 or 2
            require(isStable[_collateralPath[_collateralPath.length - 1]], "IVLCPT/LNST"); //Invalid collateral path, last must be stable token

            if (_collateralPath.length == 2 && !isCollateral[_collateralPath[0]]) {
                revert("IVLCPT/FNCT"); //Invalid collateral path, first must be collateral token
            }
