...
contract DomFiVault is Ownable, ReentrancyGuard, IVault, ERC4626, IRewardRecipient {
    ...
    FPUnsigned public HEALTHY_UTILIZATION_RATIO = FixedPoint.fromUnscaledUint(8).div(10); // utilization ratio below which it's safe to let users withdraw immediately
    uint256 public MAX_WITHDRAW_DELAY = 10 days; // worst-case wait to withdraw collateral
    uint256 public TOTAL_DELAY_PER_UTILIZATION_RATIO = 100 days; // withdrawing 10% of the vault would take 1 day for each 10% utilization ratio above the minimum
    ...
    // kill switches
    bool public delaysDisabled = false;
    ...
    function withdraw(uint256 assets, address receiver, address account) public virtual override onlyManager(account) returns (Shares shares) {
        ...
        if (instantRedeemAvailable()) {
            (shares, , fee) = _convertInput(assets, Shares.wrap(0), totalAssets(), Shares.wrap(totalSupply()));
        } else {
        ...
        if (delaysDisabled) { return shares;}
    ...
    function redeem(Shares shares, address receiver, address account) public virtual override onlyManager(account) returns (uint256 assets) {
        ...
        uint256 fee;
        if (instantRedeemAvailable()) {
            (, assets, fee) = _convertInput(0, shares, totalAssets(), Shares.wrap(totalSupply()));
        } else {
        ...
        if (delaysDisabled) { return assets;}
     ...
     *  @return assets max assets that a request for `shares` could be redeemed for
     */
    ...
    function instantRedeemAvailable() public returns (bool available) {
        if (delaysDisabled) {
            return true;
        }
        if (balance == 0) {
        ...
        (available, ) = PerpLib.getUtilizationRatio(
    ...
    function getUtilizationRatio() public returns (FPSigned utilizationRatio) {
        (, utilizationRatio) = PerpLib.getUtilizationRatio(
            domFiPerp.totalOpenInterest(),
            balance,
            domFiPerp.getTotalPnl(),
            0, // pendingWithdraw,
    ...
    function previewRedeemRequest(Shares shares, address account) external returns (uint64 length, int64 offset) {
        ...
        if (previousRequest.beginsAt > 0 && uint64(block.timestamp) > previousRequest.beginsAt) {
        ...
        uint64 penalty;
        (length, penalty) = PerpLib._getDelayForFullWithdraw(
    ...
    function makeRedeemRequest(Shares shares, address account, address receiver) external onlyManager(account) returns (uint64 length, uint64 beginsAt) {
        ...
        require(shares <= Shares.wrap(balanceOf(account)), 'exceeds balance');
    ...
    function maxRedeem(address owner) public virtual override returns (Shares maxShares) {
        ...
        maxShares = pendingWithdrawRequest[owner].shares.mul(available);
    ...
    */
    ...
    ///@dev testnet kill switch for disabling withdraw delays. makeRedeemRequest will be instant and ERC4626 methods should work
    function disableDelays(bool delaysDisabled_) external {
        delaysDisabled = delaysDisabled_;
    }
