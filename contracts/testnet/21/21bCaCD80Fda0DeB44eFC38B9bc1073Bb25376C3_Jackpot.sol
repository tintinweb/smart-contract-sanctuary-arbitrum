...
interface IBankRoll {
  function totalJackpotPrize() external view returns (uint256);

  function totalLastbuyPrize() external view returns (uint256);

  function totalWeeklyPrize() external view returns (uint256);
  ...
  function deposit(uint256 _amountToJackpot, uint256 _amountToLastbuy, uint256 _amountToWeekly) external payable;

  function transferPayout(address player, uint256 payout, address tokenAddress, uint8 prize) external;
...
contract Common is ReentrancyGuard {
  ...
  function _transferToBankroll(address tokenAddress, uint256 amount) internal {
    ...
    if (tokenAddress == address(0)) {
      ...
      if (amountToBankroll != 0) {
        ...
        //   revert RefundFailed();
        // }
        uint256 amountToJackpot = (amountToBankroll * jackpotPercent) / 100;
        uint256 amountToLastbuy = (amountToBankroll * lastBuyPercent) / 100;
        uint256 amountToWeekly = amountToBankroll - amountToJackpot - amountToLastbuy;

        Bankroll.deposit{value: amountToBankroll}(amountToJackpot, amountToLastbuy, amountToWeekly);
      }
    } else {
      if (amountToMarketing != 0) {
  ...
  function getCurrentJackpot() public view returns (uint256) {
    return Bankroll.totalJackpotPrize() / 2;
  }
  ...
  function getLastbuyPrize() public view returns (uint256) {
    return Bankroll.totalLastbuyPrize();
  }
  ...
  function getWeeklyPrize() public view returns (uint256) {
    return Bankroll.totalWeeklyPrize();
  }
   ...
   * @param prize 0 JackpotPrice / 1 Lastbuy / 2 Weekly

   */
  function _transferPayout(address player, uint256 payout, address tokenAddress, uint8 prize) internal {
    Bankroll.transferPayout(player, payout, tokenAddress, prize);
  }
...
contract Jackpot is Common {
  ...
  mapping(uint256 => address[]) private ticketsLastbuyLotteryAtRound;
  LotteryState public lastbuyLotteryState;
  uint256 public lastbuyLimit = 100;

  uint256 public weeklyLotteryRound = 0;
  mapping(uint256 => address[]) private ticketsWeeklyLotteryAtRound;
  ...
  function randomizerCallback(uint256 _id, bytes32 _value) external {
    ...
    if (playerAddress != address(0)) {
      ...
      for (uint32 i = 0; i < game.playedNumbers.length; i++) {
        ...
        if (weeklyLotteryState == LotteryState.OPEN) {
          ticketsWeeklyLotteryAtRound[weeklyLotteryRound].push(playerAddress);
        }
      }
      ...
      if (payout != 0) {
        _transferPayout(playerAddress, payout, game.tokenAddress, 0);
      }
    ...
    // LastBuy Lottery
    playerAddress = vrfLastBuyIDs[_id];
    if (playerAddress != address(0) && ticketsLastbuyLotteryAtRound[lastbuyLotteryRound].length > 0) {
      ...
      if (payout != 0) {
        _transferPayout(lastBuyWinner, payout, address(0), 1);
      }
    ...
    // Weekly Lottery
    playerAddress = vrfWeeklyIDs[_id];
    if (playerAddress != address(0) && ticketsWeeklyLotteryAtRound[weeklyLotteryRound].length > 0) {
      ...
      if (payout != 0) {
        _transferPayout(weeklyWinner, payout, address(0), 2);
      }
  ...
  function getNumberOfWeeklyLotteryTickets(uint256 _round) public view returns (uint256) {
    return ticketsWeeklyLotteryAtRound[_round].length;
  }

  function getWeeklyLotteryTickets(uint256 _round) public view returns (address[] memory) {
    return ticketsWeeklyLotteryAtRound[_round];
  }

  function getNumberOfLastbuyLotteryTickets(uint256 _round) public view returns (uint256) {
    return ticketsLastbuyLotteryAtRound[_round].length;
  }

  function getLastBuyLotteryTickets(uint256 _round) public view returns (address[] memory) {
