...
contract DegenMain is IDegenMain, DegenBase {
  ...
  function submitOrder(
    ...
    _orderIndex_ = _submitOrder(_order, _currentPrice);
  }
  ...
  function executeOrder(
    ...
    positionKey_ = _executeOrder(_orderIndex_, _caller, _assetPrice, _marginAmountUsdc);
  }
  ...
  function liquidatePosition(
    ...
    // uint256 threshold_ = ;
    // console.log("isPnlPositive_", isPnlPositive_);
    // console.log("marginMinusInterest_", marginMinusInterest_);

    if (pnlUsd_ >= _calculateEffectiveMargin(marginMinusInterest_) || marginMinusInterest_ == 0) {
      ...
      _decreaseOpenInterestUsd(position_.isLong, position_.positionSizeUsd);
  ...
  function closePosition(
    ...
    // console.log("is long", position_.isLong);
    // console.log("pnlUsd_", pnlUsd_);
    // console.log("interestFunding_", interestFunding_);
    // console.log("isPnlPositive_", isPnlPositive_);
    // console.log("assetPrice ", _assetPrice);
    // console.log("position_.priceOpened", position_.priceOpened);
    // console.log("position_.fundingRateOpen", position_.fundingRateOpen);
    // console.log("position_.timestampOpened", position_.timestampOpened);
    // console.log("block.timestamp", block.timestamp);
    // console.log("fundingRateTimeBuffer", fundingRateTimeBuffer);
  ...
  function getAllLiquidatablePositions(
    ...
    for (uint256 i = 0; i < openPositionsKeys_.length(); i++) {
      ...
      // console.log("_assetPrice", _assetPrice);
      // console.log("_timestampAt", _timestampAt);
      // revert("stop");
  ...
  ) internal pure returns (uint256 interestAccruedUsd_) {
    if (_currentTimeStamp >= _timeStampOpened + _fundingRateTimeBuffer) {
      interestAccruedUsd_ =
        (_positionSizeUsd * _fundingRate * (_currentTimeStamp - _timeStampOpened)) /
  ...
  function _isPositionLiquidatableByKey(
    ...
    PositionInfo memory position_ = positions[_positionKey];
    // console.log("position_.positionSizeUsd", position_.positionSizeUsd);
    // console.log("position_.priceOpened", position_.priceOpened);
    // console.log("_assetPrice", _assetPrice);
    // console.log("position_.fundingRateOpen", position_.fundingRateOpen);
    // console.log("position_.timestampOpened", position_.timestampOpened);
    // console.log("_timestampAt", _timestampAt);
    // console.log("fundingRateTimeBuffer", fundingRateTimeBuffer);
    // console.log("position_.marginAmountUsd", position_.marginAmountUsd);
    // console.log("position_.isLong", position_.isLong);
    // revert("stop");
   ...
   * @param _maginAmountOpenUsd margin amount when the position was opened in usd, scaled 1e18
  ...
  function _isPositionLiquidable(
    ...
    uint256 _fundingRateTimeBuffer,
    uint256 _maginAmountOpenUsd,
    bool _isLong
  ) internal view returns (bool isLiquidatble_) {

    (uint256 pnlUsd_, uint256 marginMinusInterest_,uint256 interestFunding_ , bool isPnlPositive_) = _calculatePnlAndInterestUsd(
      _marginAmount,
      _positionSizeUsd,
    ...
    if (isPnlPositive_) {
      ...
      // console.log("     ");
      // console.log("     ");
      // console.log("isNegative_", isNegative_);
      return false;
    } else {
      uint256 threshold_ = _calculateEffectiveMargin(marginMinusInterest_);
   ...
   * @return marginMinusInterest_ margin minus the interest that has accrued
  ...
  function _calculatePnlAndInterestUsd(
    ...
    internal
    pure
    returns (uint256 pnlUsd_, uint256 marginMinusInterest_, uint256 interestAccruedUsd_, bool _isPnlPositive)
  {
    interestAccruedUsd_ = _calculateAmountOfFundingRateInterestUsd(
      _timeOpened,
      _timeCurrently,
    ...
    if(_marginAmount <= interestAccruedUsd_) {
      // margin is smaller than interest, so the margin becomes 0 or negative, so the pnl becomes negative
      marginMinusInterest_ = 0;
      _isPnlPositive = false;
    } 
    if(_marginAmount > interestAccruedUsd_) {
      // margin is bigger than interest, so the margin becomes smaller
      marginMinusInterest_ = _marginAmount - interestAccruedUsd_;
    }
   ...
   * @return _orderIndex_ index of the order that is submitted
   */
  function _submitOrder(OrderInfo memory _order, uint256) internal returns (uint256 _orderIndex_) {
    _checkOpenOrderAllowed();
    // check if leverage is within bounds
    require(_order.positionLeverage >= minLeverage, "Degen: leverage too low"); // 1e18 = 1x leverage, 2e18 = 2x leverage
    require(_order.positionLeverage <= maxLeverage, "Degen: leverage too high");

    // if (_order.marginInStables) {
    //   // wager is held in usdc, check if the wager is within bounds
    //   _checkPositionSizeWagerInUsdc(_order.positionLeverage, _order.wagerAmount);
    // } else {
    //   // wager is held in asset, check if the wager is within bounds
    //   _checkPositionSizeAsset(_order.positionLeverage, _order.wagerAmount, _currentPrice);
    // }
  ...
  function _cancelOrder(
    ...
    // todo maybe emit wagerReturned_?
  ...
  function _executeOrder(
    ...
    // } else {
    //   // wager is held in asset, check if the wager is within bounds
    //   (positionSizeUsd_, marginValueUsd_) = _checkPositionSizeAsset(
    //     order_.positionLeverage,
    //     _marginAmountUsdc,
    //     _assetPrice
    //   );
    ...
    // console.log("marginAmountUsd:", _marginAmountUsd);
    _increaseOpenInterestUsd(order_.isLong, positionSizeUsd_);
    // revert("Degen: position not opened");
...
contract DegenBase is IDegenBase {
  ...
  //
  uint256 public immutable differenceStableDecimals;
  ...
  uint256 public minMarginAmountUsd;
  ...
  uint256 public totalLongExposureUsd;

  // total amount of short positions open (so total short exposure)
  uint256 public totalShortExposureUsd;
  ...
  constructor(
    ...
    differenceStableDecimals = 18 - _stableDecimals;
  ...
  /**
   * @notice set the max position size in the usd
   * @dev 1 usd is scaled 1e18, so 1e18 is 1 usd
   * @param _maxPositionSizeUsd the new max position size
   */
  function setMaxPositionSizeUsd(uint256 _maxPositionSizeUsd) external onlyPoolManagerController {
    maxPositionSizeUsd = _maxPositionSizeUsd;
    emit SetMaxPostionSizeUsd(_maxPositionSizeUsd);
  }

  /**
   * @notice set the min position size in the usd
   * @dev 1 usd is scaled 1e18, so 1e18 is 1 usd
   * @param _minPositionSizeUsd the new min position size
   */
  function setMinMarginAmountUsd(uint256 _minPositionSizeUsd) external onlyPoolManagerController {
    minMarginAmountUsd = _minPositionSizeUsd;
    emit SetMinMarginAmountUsd(_minPositionSizeUsd);
  }
  ...
  ) internal view returns (uint256 positionSizeUsd_, uint256 marginValueUsd_) {
    unchecked {
   ...
   * @param _positionSizeUsd the position size in usd
   */
  function _increaseOpenInterestUsd(bool _isLong, uint256 _positionSizeUsd) internal {
    unchecked {
      if (_isLong) {
        // increase the total long exposure
        totalLongExposureUsd += _positionSizeUsd;
        require(totalLongExposureUsd <= maxExposureForAsset, "Degen: max exposure reached");
      } else {
        // increase the total short exposure
        totalShortExposureUsd += _positionSizeUsd;
        require(totalShortExposureUsd <= maxExposureForAsset, "Degen: max exposure reached");
      }
    }
  }
   ...
   * @param _positionSizeUsd the position size in usd that is being closed
   */
  function _decreaseOpenInterestUsd(bool _isLong, uint256 _positionSizeUsd) internal {
    unchecked {
      if (_isLong) {
  ...
  function _updateFundingRate(bool _isLong) internal view returns (uint256 _fundingRate) {
    ...
    int256 totalShort_ = int256(totalShortExposureUsd);
    int256 totalLong_ = int256(totalLongExposureUsd);
    int256 skewness_ = ((totalLong_ - totalShort_) * 1e6) / (totalLong_ + totalShort_);
    // int256 skewness_ = (int256(totalLongExposureUsd) - int256(totalShortExposureUsd));
