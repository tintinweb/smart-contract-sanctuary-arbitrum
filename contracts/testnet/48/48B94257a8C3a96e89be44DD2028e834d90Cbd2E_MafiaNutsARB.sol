...
abstract contract LzApp is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function setTrustedRemote(uint32 _remoteChainId, bytes calldata _path) external onlyOwner {
...
// File layerzero/contracts/token/onft/ONFT721.sol
...
import { ONFT721, IERC721 } from "./layerzero/ONFT721.sol";
...
contract MafiaNutsARB is ONFT721 {
    ...
    event Upgrade(uint indexed tokenId, uint indexed level);
    ...
    function upgradeToken(UpgradeInfo memory _upgradeInfo) external {
        // validation
        require(_exists(_upgradeInfo.tokenId), "!exist");
        require(_upgradeInfo.expiry <= block.timestamp, "expired");
        require(getLevel(_upgradeInfo.level) != 5, "max level");
        ...
        require(signer == authorizedSigner, "!authorized");

        // upgrade
        _levelMap[_upgradeInfo.tokenId] = _upgradeInfo.level;
        emit Upgrade(_upgradeInfo.tokenId, _upgradeInfo.level);
    }
