...
import "./interfaces/IContrast.sol";
import "./ContrastBase.sol";

contract Contrast is ContrastBase {
    /// -----------------------------------------------------------------------
    /// Dependencies
    /// -----------------------------------------------------------------------
    ...
    function markets(uint256 marketId) external view override returns (Market memory) {
        return _markets[marketId];
    }

    function quotes(uint256 marketId) external view override returns (Quote memory) {
        return _quotes[marketId];
    }

    /// @dev This function is constrained by gas limits and may revert in the future.
    function totalMarketsOpen() external view override returns (uint256 openMarkets) {
            ...
            for (uint256 i; i < totalMarkets; ++i) {
                ...
                if (
                    block.timestamp < uint256(_markets[i].maturity)
                        && (balanceOf(account, idBelow) > 0 || balanceOf(account, idAbove) > 0)
                ) {
                    ++openMarkets;
                }
            }
    ...
    function canEmergencyExit(uint256 marketId) external view returns (bool) {
        ...
        if (block.timestamp < market.maturity + asset.emergencyWithdrawalWindow) {
            return false;
        }
    ...
    function computeImpliedProbability(uint256 marketId) external view override returns (uint256) {
...
abstract contract ContrastBase is IContrast, ERC1155Supply {
    /// -----------------------------------------------------------------------
    /// Dependencies
    /// -----------------------------------------------------------------------
    ...
    function idsForMarket(uint256 marketId)
        public
        pure
        returns (uint256 idBelow, uint256 idAbove)
    {
        idBelow = marketId << 1;
        idAbove = idBelow + 1;
    }
    ...
    function enter(uint256 marketId, uint256 amountStrikeBelow, uint256 amountStrikeAbove)
        external
        virtual
    {
        // Checks:
        ...
        market.asset.safeTransferFrom(
            msg.sender, address(this), amountStrikeBelow + amountStrikeAbove
        );
    ...
    /// @dev The function also collects a transfer fee, which is a percentage of the specified amount
    /// of outcome tokens being exchanged. The fee is defined by the TRANSFER_FEE_BIPS constant.
    function exit(uint256 marketId, uint256 amountStrikeBelow, uint256 amountStrikeAbove)
        external
        virtual
    {
        // Checks:
        ...
        emit MarketExited(marketId, msg.sender, amountStrikeBelow, amountStrikeAbove);
    }
    ...
    /// @param amountStrikeBelow The amount of below outcome tokens the user wishes to burn.
    /// @param amountStrikeAbove The amount of above outcome tokens the user wishes to burn.
    function emergencyExit(uint256 marketId, uint256 amountStrikeBelow, uint256 amountStrikeAbove)
        external
        virtual
    {
        // Checks:
    ...
    function price(uint256 marketId, uint80 roundId) external virtual {
        ...
        // - If second quote comes, the quoter is not charged bond amount and quoter in quote does not change.
        ...
        if (quote.quoter == address(0)) {
            market.asset.safeTransferFrom(msg.sender, address(this), asset.quoteBondAmount);
    ...
    function settle(uint256 marketId) external virtual {
        ...
        if (answer > strike) {
            ...
            if (totalSupply[idAbove] == 0) {
                market.exchangeRate = 0;
            } else {
                market.exchangeRate = totalSupply[idBelow].divWad(totalSupply[idAbove]).toUint96();
            }
        } else if (answer <= strike) {
            market.outcome = Outcome.BELOW;
            if (totalSupply[idBelow] == 0) {
                market.exchangeRate = 0;
            } else {
                market.exchangeRate = totalSupply[idAbove].divWad(totalSupply[idBelow]).toUint96();
            }
        }
    ...
    function withdrawFees(address asset, address to) external returns (uint256 amountOut) {
