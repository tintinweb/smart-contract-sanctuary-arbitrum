...
contract ARBRegistrarControllerV2 is Ownable, IARBRegistrarController, IERC165 {
    ...
    function registerWithConfig(
        ...
        auction.consumeQuota(msg.sender);

        (bool isAvaiable, uint256 freeDuration) = winnerOrAvailable(name);
        if (!isAvaiable) {
...
contract Auction is Ownable {
    ...
    //controller address for modify quota
    mapping(address => bool) public controllers;
    ...
    function consumeQuota(address user) public onlyController {
        if (userStatus[user].quota < 1) {
            revert NotEnoughQuota();
        }
        UserStatus storage us = userStatus[user];
