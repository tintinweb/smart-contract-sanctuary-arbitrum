...
contract CityMinter is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  uint256 public hoursPerCity = 15;
  ...
  constructor(
    ...
    collectibleAmount = _collectibleCost;
  ...
  function mint(
    ...
    for (uint256 j = 0; j < _cityIds.length; j++) {
      ...
      // TODO: make sure this is what you need
      emit CollectiblesUsed(_realmId, collectibleId, collectibleAmount);
    }

    // Check if quantity is below max cities
    require(
      quantity <= maxCities,
    ...
    // Build
    CITY_STORAGE.build(_realmId, quantity * hoursPerCity);
  }
  ...
  function updateCollectibleAmount(uint256 _collectibleAmount)
    external
    onlyAdmin
  {
  ...
  function _mint(
    ...
    // TODO: check event has everything you need
