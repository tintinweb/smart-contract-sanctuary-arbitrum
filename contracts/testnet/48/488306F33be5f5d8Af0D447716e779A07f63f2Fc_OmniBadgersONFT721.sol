...
// File: contracts/token/onft/OmniBadgersONFT721.sol
...
contract OmniBadgersONFT721 is ONFT721 {
    uint256 public constant MAX_SUPPLY = 8888;
    uint256 public constant MAX_PER_MINT = 10;
    uint256 public constant MAX_PER_ADDRESS = 50;
    uint256 public constant PRICE_PER_MINT = 0.075 ether;
    ...
    constructor(
        ...
        require(_endMintId <= MAX_SUPPLY, "OmniBadgersONFT721: _endMintId overreaching max supply");
        nextMintId = _startMintId;
        maxMintId = _endMintId;
    }

    modifier callerIsEOA() {
        require(tx.origin == msg.sender, "OmniBadgersONFT721: caller is not EOA");
        _;
    }

    function mint(uint8 qty) external callerIsEOA  {
        require(
            nextMintId + qty <= maxMintId,
            "OmniBadgersONFT721: max mint limit reached"
        );
        require(
            qty <= MAX_PER_MINT,
            "OmniBadgerONFT721: only max 5 qty per mint allowed"  
        );
    ...
    function withdrawAll() external onlyOwner {
        ...
        require(balance > 0, "Contract has zero balance");
