...
contract VertexHandler is OwnableUpgradeable {
    ...
    function getVertexBalance(bytes32 subaccount) external view returns (int128 balance) {
        IFQuerier querier = IFQuerier(vertexQuerier);
        ...
        for (uint i = 0; i < subaccountInfo.spotBalances.length; i++) {
            ...
            balance += calcSpotBalanceValue(
                spotBalance,
                findSpotProduct(spotBalance.productId, subaccountInfo.spotProducts)
            );
        }
    }

    function getTest(bytes32 subaccount) external view returns (int128 balance) {
        IFQuerier querier = IFQuerier(vertexQuerier);
        IFQuerier.SubaccountInfo memory subaccountInfo = querier.getSubaccountInfo(subaccount);
    ...
    ) internal pure returns(int128 balance) {
        balance = spotBalance.balance.amount*spotProduct.oraclePriceX18;
    }
