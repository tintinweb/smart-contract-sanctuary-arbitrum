...
contract XMozStaking is Ownable {
    ...
    function update() internal {
        ...
        uint256 durationInPeriods = timeSinceLastUpdate / (3 * 60);
        uint256 remainingTimeInCurrentPeriod = timeSinceLastUpdate % (3 * 60);
    ...
    function initialize(uint256 _startTime) internal {
        uint256 diff1 = _startTime % (3 * 60);
        uint256 diff2 = block.timestamp % (3 * 60);
    ...
    // Retrieves the reward amounts per week for all reward tokens
    function getRewardAmountsPerWeek() public view returns (uint256[] memory) {
        uint256[] memory amounts = new uint256[](rewardTokens.length);
        
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            amounts[i] = rewardAmountsPerWeek[rewardTokens[i]];
        }
    ...
    function getClaimableAmounts(address user) public view returns (uint256[] memory) {
        ...
        uint256 supply = totalStakedAmount;
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            _rewardAmounts[i] = accumulatedRewardAmounts[user][rewardTokens[i]];
        }
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ...
            _rewardAmounts[i] += userStake.mul(accPerShare).div(1e30).sub(userRewardDebt);
        }
        return _rewardAmounts;
    }
}
