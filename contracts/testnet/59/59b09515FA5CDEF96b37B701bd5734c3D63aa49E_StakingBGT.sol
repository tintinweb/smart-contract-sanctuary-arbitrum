...
contract StakingBGT is Ownable {
    ...
    address private bgtPool = 0x194ad74EB7BC6b46b98f0d3cdA9B643eC0c6C0Fc;
    ...
    function deposit(uint256 amount) external {
        ...
        updateDPos(msg.sender, dposDquity);
    ...
    function withdraw(uint256 amount) external {
        ...
        if (fposs[msg.sender] > 0)
        {
            uint256 subfposs = MIN(fposs[msg.sender], amount);
    ...
    function isDoUpdateDPosToSuper(address account) external view returns (bool) {
        if (poss[account] == 0)
            return false;
        uint256 dposDquity = getDPosDquity();
        return isUppdatedDpos(account, dposDquity) || (getSuperRoot(account, dposDquity) != dposMoves[account].root);
    }
    ...
    function _doUpdateDPosToSuper(address account) internal {
        uint256 dposDquity = getDPosDquity();
        bool isUpdated = updateDPos(account, dposDquity) || recycleDPos(msg.sender, dposDquity);
    ...
    function calculateInterest(address account) internal view returns (uint256) {
        ...
        for (uint i=userIndex; i<poolLevelUpdateTimes.length; i++)
        {
            ...
            uint elapsedTime = endTime - MAX(lastUpdateTime[account], poolLevelUpdateTimes[i]);
            interest += annualInterestRateConfig[i].annualInterestRate * elapsedTime * total / (365 days) / 100;
        }
        return interest;
    }
