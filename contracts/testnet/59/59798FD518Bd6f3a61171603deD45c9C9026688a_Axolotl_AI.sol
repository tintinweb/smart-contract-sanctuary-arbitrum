...
//Creator: Luigixb#2867 on Discord

contract Axolotl_AI is ERC20, Ownable {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;

    event SwapBack( uint256 liquidity, uint256 flex, uint256 marketing, uint256 dev,  uint timestamp);
    ...
    address routerAddress = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
    uint256 constant _targettotalSupply = 10_000_000_000 * 1e6;
    ...
    function _axoTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (isPair(sender)) {
            ...
            //shouldTakeFee = false; //Don't take fees on simple transfers. Todo:
        }
    ...
    function swapBack() internal swapping {
        ...
        taxAmount -= amountAxoLp; //Keep these tokens to add them to LP automatically, Swap the Rest into ETH
        ...
        uint256 amountBackToken = backToken.balanceOf(address(this)) - balanceBefore; //This is the ARB he/she is getting back from the swap

        uint256 holderReflectionInEth = ((amountBackToken * holderFee) / totalFee ); //holderfee in % gets calculated from the ETH amount and deposited
        try dividendDistributor.deposit{value: holderReflectionInEth}() {} catch {} //Safe holders reflection to Distributor, in ETH/ARB
        amountBackToken = amountBackToken - holderReflectionInEth;
    ...
    function distributeTokens() public onlyOwner {
        ...
        //Tokenomics
        _mint(address(this),  (_targettotalSupply * 20) / 100); //Minting so it can be burnt
    ...
    function setOverallFees(
        ...
        require((totalFee * 100 / feeDenominator)  < 25, "Overall fee is over 25%; that's to much."); //Prevents abusive fees
    }
