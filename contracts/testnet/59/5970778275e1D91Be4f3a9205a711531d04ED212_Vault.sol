...
contract Vault is Constants, Initializable, ReentrancyGuardUpgradeable, IVault {
    ...
    // add collateral to reduce leverage / remove collateral to increase leverage
    function addOrRemoveCollateral(
        uint256 _posId,
        bool isPlus,
        uint256 _amount
    ) public nonReentrant preventBanners(msg.sender) {
    ...
    function addOrRemoveCollateralPacked(uint256 a) external {
        uint256 posId = a / 2 ** 128;
        bool isPlus = (a / 2 ** 127) % 2 == 1;
        uint256 amount = a % 2 ** 127;
        return addOrRemoveCollateral(posId, isPlus, amount);
    }
