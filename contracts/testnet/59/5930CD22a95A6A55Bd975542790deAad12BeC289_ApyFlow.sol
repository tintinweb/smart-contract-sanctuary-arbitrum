...
contract ApyFlow is ERC20("Yielder", "YLDR"), Ownable {
    ...
    function _redeem(uint256 shares) internal returns (uint256 assets) {
        ...
        for (uint256 i = 0; i < vaults.length(); i++) {
            ...
            assets += assetConverter.safeSwap(token, address(asset), amount);
        }

        uint256 valueInAsset = pricesOracle.convertFromUSD(
            valueInUSD,
            address(asset)
        );
        uint256 processedPricePerToken = (valueInAsset * (10 ** decimals())) /
            shares;
        ...
        if (processedPricePerToken > lastPricePerToken) {
            adjustedPricePerToken =
                lastPricePerToken +
                ((processedPricePerToken - lastPricePerToken) *
                    (1000 - feeInPpm)) /
                1000;
        }

        uint256 fee = (assets *
            (processedPricePerToken - adjustedPricePerToken)) /
            processedPricePerToken;
        if (fee > 0) {
