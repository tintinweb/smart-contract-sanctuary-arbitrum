...
contract L2UnicornMinter {
    ...
    function play(uint256 gasPrice_, uint256 callbackGasLimit_) payable external {
        ...
        _requestIdGameInfo[requestId_] = GameInfo(msg.sender, 0, 0, false);
        //
    ...
    function randomizerCallback(uint256 id, bytes32 value) external {
        ...
        //IL2Unicorn(l2Unicorn).mintWithMinter(gameInfo_.user, gameInfo_.tokenId);
        emit PlayGameResult(gameInfo_.user, id, seed_, tokenId_);
    }
    ...
    function viewGameInfos(address user_, uint256 startIndex_, uint256 endIndex_) external view returns (GameInfo[] memory gameInfos){
        if (startIndex_ >= 0 && endIndex_ >= startIndex_) {
            ...
            for (uint index_ = startIndex_; index_ < ((endIndex_ > total) ? total : endIndex_);) {
                ...
                //gameInfos[arrayIndex_] = GameInfo(gameInfo_.user, gameInfo_.seed, gameInfo_.tokenId, gameInfo_.mint);
