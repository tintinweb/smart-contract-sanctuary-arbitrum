...
contract BridgePortalV3 is ReentrancyGuard, Governable {
    ...
    bool public isNativeInput;
    address notNativeTokenAddr;
    ...
    constructor(uint256 _targetChainId, bool _isNativeInput, address _notNativeTokenAddr) {
        targetChainId = _targetChainId;
        isNativeInput = _isNativeInput;
        notNativeTokenAddr = _notNativeTokenAddr;
    }

    function setNotNativeTokenAddr(address _notNativeTokenAddr) external onlyGov {
        require(_notNativeTokenAddr != address(0x0), "OKOK: _notNativeTokenAddr cannot be 0x0");
        notNativeTokenAddr = _notNativeTokenAddr;
    }
    ...
    receive() external payable {
        require(paused == false, "paused");
        require(isNativeInput == true, "native token not support!");
    ...
    function _depositTransaction(
        ...
        require(_value >= minDepositValue, "OKOK: invalid _value");
    ...
    function depositERC20(address _to, uint256 _amount) external nonReentrant {
        require(_to != address(0x0), "OKOK: _to cannot be 0x0");
        require(isNativeInput == false, "OKOK: you should deposit native token!");
        require(notNativeTokenAddr != address(0x0), "OKOK: notNativeTokenAddr cannot be 0x0");
        require(_amount >= minDepositValue, "OKOK: invalid _value");

        uint256 beforeAmount = IERC20(notNativeTokenAddr).balanceOf(address(this));
        _safeTransferFrom(notNativeTokenAddr, msg.sender, address(this), _amount);
        uint256 afterAmount = IERC20(notNativeTokenAddr).balanceOf(address(this));

        uint256 delta = afterAmount - beforeAmount;
        require(delta >= _amount, "OKOK: not receive enough erc20 token amount!");

        // Emit a TransactionDeposited event so that the rollup node can derive a deposit
        // transaction for this deposit.
        emit TransactionDeposited(msg.sender, _to, _amount, targetChainId);
    }
    ...
    function sendToken(address payable _receiver, uint256 _outAmount, uint256 _depositId) external onlyOrderKeeper nonReentrant {
        require(isNativeInput == true, "send native token not support!");
    ...
    function sendERC20(address payable _receiver, uint256 _outAmount, uint256 _depositId) external onlyOrderKeeper nonReentrant {
        require(isNativeInput == false, "send erc20 token not support!");
        require(depositIdToStatus[_depositId] == false, "depositId already processed");
        require(_outAmount > 0, "invalid _outAmount");
        require(_receiver != address(0), "invalid _receiver");
        require(_outAmount <=IERC20(notNativeTokenAddr).balanceOf(address(this)), "_outAmount large than contract balance");
        require(_receiver != address(this), "can not send eth to myself");

        depositIdToStatus[_depositId] = true;
        IERC20(notNativeTokenAddr).safeTransfer(_receiver, _outAmount);
    ...
    function balanceOfERC20() external view returns (uint256) {
        return IERC20(notNativeTokenAddr).balanceOf(address(this));
    }
}
