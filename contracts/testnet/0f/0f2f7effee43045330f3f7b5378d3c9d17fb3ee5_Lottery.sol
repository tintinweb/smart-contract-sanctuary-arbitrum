...
contract Lottery is RrpRequesterV0 {
    uint256 public constant FEE_TAKE = 30; // 3 basis points
    uint256 public constant FEE_MULTIPLE = 10_000;
    uint256 public roundNumber;
    uint256 public totalPayouts;
    uint256 public roundTotalEth;
    uint256 public queuedWager;
    uint256 public queuedRoundMax;
    uint256 public wager = 0.25 ether;
    uint256 public roundMax = 4 ether;
    ...
    //    constructor(address _airnodeRrp, address _airnode, bytes32 _endpointIdUint256, address _sponsorWallet)
    ...
    function queueResolve() external {
        ...
        // Give msg.sender a tip to incentivize this function getting called ASAP
    ...
    function queueLotteryParams(uint256 _wager, uint256 _roundMax) external onlyOwner {
        if (_roundMax == 0 || _wager == 0 || _wager == _roundMax || _roundMax % _wager != 0)
        {
    ...
    function setRequestParameters(address _airnode, bytes32 _endpointIdUint256, address _sponsorWallet)
        ...
        onlyOwner
    {
        if (_airnode == address(0) || _sponsorWallet == address(0) || _endpointIdUint256 == 0) {
