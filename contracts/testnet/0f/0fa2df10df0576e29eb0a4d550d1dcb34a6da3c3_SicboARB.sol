  ...
  PausableUpgradeable
{
  ...
  function buyNft(uint256 amount) external payable {
    ...
    IERC20Upgradeable(tokenAddress).transferFrom(msg.sender, address(this), nftPrice * amount);
  ...
  function play(
    ...
    require(priceMark < prices.length, "out of prices");
    uint256 price = prices[priceMark];
    require(IERC20Upgradeable(tokenAddress).balanceOf(msg.sender) >= price, "pay not enough");
    require(house.balance >= price,"house balance not enough");
    ...
    if (result) {
      reward = (price * (1000 - feePercent)) / 1000;
      IERC20Upgradeable(tokenAddress).safeTransfer(msg.sender, reward + price);
      rounds[currentRoundIndex].totalNetLoss += reward;
      house.totalNetLoss += reward;
    }else {
        rounds[currentRoundIndex].totalNetProfit += price;
        house.totalNetProfit += price;
        IERC20Upgradeable(tokenAddress).transferFrom(msg.sender, address(this), price);
    }    
  ...
  function endRound(uint256 holderReceivedPercent) external onlyOwner {
    ...
    if (
      rounds[currentRoundIndex].totalNetProfit <
      rounds[currentRoundIndex].totalNetLoss || sicboContract.nextTokenId() == 1
    ) {
  ...
  function deposit(uint256 amount) external  onlyOwner {
    require(IERC20Upgradeable(tokenAddress).balanceOf(msg.sender) == amount, "Not enough native coin");
