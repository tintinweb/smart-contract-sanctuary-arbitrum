...
contract FeeToVault is Ownable2StepUpgradeable, PausableUpgradeable, AccessControlUpgradeable {
    event UpdateBridgeGateway(address _oldBridgeGateway, address _newBridgeGateway);
    ...
    function initialize(address _burnerManager, address _underlyingBurner, address _bridgeGateway, address _HOPE, uint16 _dstChainId) public initializer {
    ...
    function bridgeHOPE(address dstHOPE, uint256 amount) external payable whenNotPaused onlyRole(OPERATOR_ROLE) {
        ...
        IBridge(bridgeGateway).sendFrom{value: msg.value}(address(this), HOPE, dstChainId, _addressToBytes32(dstHOPE), _addressToBytes32(underlyingBurner), amount, lzCallParam);
    }
    ...
    function updateBridgeGateway(address _bridgeGateway) external onlyOwner {
        emit UpdateBridgeGateway(bridgeGateway, _bridgeGateway);
        bridgeGateway = _bridgeGateway;
    }
