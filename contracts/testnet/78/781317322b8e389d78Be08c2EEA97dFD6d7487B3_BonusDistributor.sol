...
contract BonusDistributor is IBonusDistributor, Ownable2Step {
  ...
  function tokensPerSecond() external view override returns (uint128) {
    ...
    unchecked {
      return uint128((supply * bonusMultiplierBasisPoints) / BASIS_POINTS_DIVISOR / BONUS_DURATION);
    }
  }
  ...
  function pendingRewards() public view override returns (uint) {
    ...
    uint supply = IERC20(rewardTracker).totalSupply() * getRate();

    unchecked {
      return
        ((block.timestamp - lastDistributionTime) * supply * bonusMultiplierBasisPoints) /
