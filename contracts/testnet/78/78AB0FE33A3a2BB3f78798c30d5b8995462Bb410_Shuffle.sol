...
contract Shuffle is IShuffle, Ownable {
    ...
    function batchSearch(uint256[] calldata cardIndexes, uint256[] calldata gameIds) external view returns (uint256[] memory cards) {
        require(cardIndexes.length == gameIds.length, "invalid length");
        cards = new uint256[](gameIds.length);
        for (uint256 i = 0; i < gameIds.length; ++i) {
            cards[i] = search(cardIndexes[i], gameIds[i]);
        }
    }
