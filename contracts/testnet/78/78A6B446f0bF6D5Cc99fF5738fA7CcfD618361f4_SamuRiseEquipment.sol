...
contract SamuRiseEquipment is ERC1155Upgradeable, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    using ECDSAUpgradeable for bytes32;
    using StringsUpgradeable for uint256;

    string public constant name = "SamuRiseEquipment"; 
    string public constant symbol = "SAMURISE_EQUIPMENT";
    ...
    modifier doesNotExceedMaximum(uint256 _collectionId,uint256  _numberToMint)  {
        require(tokenSupply[_collectionId] + _numberToMint < tokenMaxSupply[_collectionId], "claiming would exceed maximum supply for this token");
        _;
    }

    function isValidTokenId(uint256 _id) internal view returns (bool) {
        return _id >= 0 && _id <= collectionIndex;
    }
    ...
    function mint(bytes memory _signature, uint256 _collectionId, uint256 _numberToClaim, uint256 _nonce) 
        public
        whenNotPaused 
        doesNotExceedMaximum(_collectionId, _numberToClaim) 
        hasValidSignature(_signature, abi.encodePacked(msg.sender, uint256(EquipmentAction.MINT), _collectionId, _numberToClaim, _nonce)) 
    {
    ...
    function ownerMint(uint256 _collectionId, uint256 _numberToMint)  
        external 
        onlyOwner
        doesNotExceedMaximum(_collectionId, _numberToMint)  
    {
