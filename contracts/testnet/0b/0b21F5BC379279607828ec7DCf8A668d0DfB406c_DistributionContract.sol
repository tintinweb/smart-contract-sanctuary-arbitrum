...
contract DistributionContract {
    ...
    constructor(address _token, address _teamAddress) {
        ...
        distributeRewardsToHolders(0, 0); // Effectuer la distribution initiale des récompenses
    }
    ...
    receive() external payable {
       ...
       function distributeRewards(uint256 arbotAmount, uint256 ethAmount) public {
    ...
    function distributeRewardsToHolders(uint256 arbotAmount, uint256 ethAmount) internal {
        uint256 remainingDistributionArbot = (arbotAmount * 50) / 100; // 50% pour les détenteurs
        uint256 remainingDistributionEth = (ethAmount * 50) / 100; // 50% pour les détenteurs
    ...
    function getCurrentEpoch() public view returns (uint256) {
     ...
     function updateEpoch() external {
        ...
        if (newEpoch > epoch.currentEpoch()) {
            epoch.updateEpoch();
            distributeRewardsToHolders(totalDistributionArbot, totalDistributionEth); // Effectuer la distribution des récompenses pour le nouvel epoch
        }
    }
}
