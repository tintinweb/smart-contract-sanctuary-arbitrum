...
import { ERC20Upgradeable, IERC20Upgradeable as IERC20 } 
    from "@openzeppelin/contracts-upgradeable/token/ERC20/ERC20Upgradeable.sol";
import { ERC20SnapshotUpgradeable } from "./extensions/ERC20SnapshotUpgradeable.sol";
import { OwnableUpgradeable } from "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import { ReentrancyGuardUpgradeable } 
    from "@openzeppelin/contracts-upgradeable/security/ReentrancyGuardUpgradeable.sol"; 
import { EnumerableSetUpgradeable as EnumerableSet } 
    from "@openzeppelin/contracts-upgradeable/utils/structs/EnumerableSetUpgradeable.sol";
import { PYESwapTokenVesting } from "./TEST_PYESwapTokenVesting.sol";
...
contract PYESwapTokenVesting is Ownable, ReentrancyGuard {
    ...
    function claimTokens() external nonReentrant {
        ...
        if (withdrawablePYES > 0) {   
            IPYESwapToken(book).mintVestedTokens(withdrawablePYES, msg.sender);
    ...
    function _calculateWithdrawableAmounts(
        ...
        if (block.timestamp > startTime) { 
            ...
            uint256 vestedTime = (getElapsedTime() / 2 minutes) + 1;
 ...
 *
 * copied from source: openzeppelin/contracts-upgradeable/token/ERC20/extensions/ERC20SnapshotUpgradeable.sol
 */
...
abstract contract ERC20SnapshotUpgradeable is Initializable, ERC20Upgradeable {
    ...
    /**
     * @dev Retrieves the balanceOf + staked for `account` at the time `snapshotId` was created.
     */
    function balanceOfAt(address account, uint256 snapshotId) public view virtual returns (uint256) {
        (bool snapshotted, uint256 value) = _valueAt(snapshotId, _accountBalanceSnapshots[account]);

        return snapshotted ? value : (balanceOf(account) + IPYESwapToken(address(this)).staked(account));
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            ...
            _updateTotalSupplySnapshot();
        } else if (IPYESwapToken(address(this)).isStakingContract(to)) { 
            // user is staking
            _updateAccountSnapshot(from);
        } else if (IPYESwapToken(address(this)).isStakingContract(from)) {
            // user is unstaking
            _updateAccountSnapshot(to);
        } else {
            // transfer
    ...
    function _updateAccountSnapshot(address account) private {
        _updateSnapshot(
            ...
            (balanceOf(account) + IPYESwapToken(address(this)).staked(account))
        );
    }
