...
contract Vault is Constants, ReentrancyGuard, Ownable, IVault {
    ...
    mapping(bytes32 => ConvertOrder) convertOrders;
    ...
    modifier hasAccess() {
        require(msg.sender == address(positionHandler) 
            || msg.sender == address(stakingRouter)
            || msg.sender == address(swapRouter)
            || msg.sender == positionRouter
            || msg.sender == owner() //Test
            , "Forbidden"
        );
        _;
    }
    ...
    function setStakingRouter(address _stakingRouter) external onlyOwner {
        require(Address.isContract(_stakingRouter), "Invalid stakingRouter");
        stakingRouter = IStaking(_stakingRouter);
    }
    ...
    // function cancelPendingOrder(address _token, bool _isLong, uint256 _posId) external nonReentrant {
    //     positionHandler.cancelPendingOrder(msg.sender, _token, _isLong, _posId);
    // }
    ...
    function setVaultSettings(
        ...
        require(Address.isContract(address(_positionHandler)), "Invalid PositionHandler");
    ...
    function takeAssetIn(
        ...
        _collectVaultFee(true, _token, amountInUSD, feeInUSD, _refer);
    ...
    function _takeAssetOut(
        ...
        if (isDebt || _token == RUSD) {
            ...
            if (_isTransferBounty) {
                ...
                _collectVaultFee(false, RUSD, usdOutAfterFee, _fee, _refer);
            }
        } else {
            _transferTo(_token, exactAmountOut, _account);

            if (_isTransferBounty) {
                _setBalance(balances, _token, exactAmountOut, false);
                totalUSD -= usdOutAfterFee;
            } else {
                _collectVaultFee(false, _token, exactAmountOut, _fee, _refer);
            }
        }
    ...
    function _accountDeltaAndFeeIntoTotalBalance(
        ...
        if (_adjustDelta != 0) {
            ...
            uint256 feeRewardOnDeltaInToken = priceManager.fromUSDToToken(_token, feeRewardOnDelta) / PRICE_PRECISION;
            
            if (_hasProfit) {
                totalUSD += feeRewardOnDelta;
                _setBalance(balances, _token, feeRewardOnDeltaInToken, true);
            } else {
                bool isVaultUSDExceeded = totalUSD < feeRewardOnDelta;

                if (isVaultUSDExceeded && !settingsManager.isOnBeta()) {
                    revert("Vault USD exceeded");
                } else if (!isVaultUSDExceeded) {
                    totalUSD -= feeRewardOnDelta;
                }

                _setBalance(balances, _token, feeRewardOnDeltaInToken, false);
            }
        }

        totalUSD += (_fee * settingsManager.feeRewardBasisPoints()) / BASIS_POINTS_DIVISOR;
        _setBalance(balances, _token, (_fee * settingsManager.feeRewardBasisPoints()) / BASIS_POINTS_DIVISOR, true);
    }

    function _distributeFee(address _account, address _refer, uint256 _fee, address _token) internal {
        _collectVaultFee(true, _token, _fee, _fee, _refer);
        emit TakeAssetIn(_account, _refer, 0, _fee, _token);
    }

    function _collectVaultFee(bool _mint, address _token, uint256 _amount, uint256 _fee, address _refer) internal {
        ...
        if (_fee != 0 && _feeManager != ZERO_ADDRESS) {
            ...
            uint256 feeMinusFeeRewardInToken = feeMinusFeeReward == 0 ? 0 : priceManager.fromUSDToToken(_token, feeMinusFeeReward) / PRICE_PRECISION;

            if (IERC20(_token).balanceOf(address(this)) >= feeMinusFeeRewardInToken && feeMinusFeeRewardInToken > 0) {
                IERC20(_token).safeTransfer(_feeManager, feeMinusFeeRewardInToken);
            } else if (feeMinusFeeReward > 0) {
        ...
        if (_refer != ZERO_ADDRESS && settingsManager.referEnabled()) {
            ...
            uint256 referFeeInToken = referFee == 0 ? 0 : priceManager.fromUSDToToken(_token, referFee) / PRICE_PRECISION;

            if (IERC20(_token).balanceOf(address(this)) >= referFeeInToken && referFeeInToken > 0) {
                IERC20(_token).safeTransfer(_refer, referFeeInToken);
            } else if (referFee > 0) {
    ...
    function updateTotalROLP() external hasAccess {
        totalROLP = IERC20(ROLP).balanceOf(address(this));
    }

    function updateBalance(address _token) external hasAccess {
        balances.set(_token, IERC20(_token).balanceOf(address(this)));
    }

    function updateBalances() external hasAccess {
        uint256 sum;
    ...
    function stake(address _account, address _token, uint256 _amount) external nonReentrant {
        ...
        _setBalance(balances, _token, _amount, true);
    ...
    function unstake(address _tokenOut, uint256 _rolpAmount, address _receiver) external nonReentrant {
        require(settingsManager.isStaking(_tokenOut), "This token not allowed for instaking");
        ...
        uint256 usdAmountAfterFee = usdAmount - usdAmountFee;
        totalUSD -= usdAmount;
        uint256 amountOut = _tokenOut == address(ROLP) ? totalUSD * PRICE_PRECISION / totalROLP / PRICE_PRECISION
            :  priceManager.fromUSDToToken(_tokenOut, usdAmountAfterFee);
        _setBalance(balances, _tokenOut, amountOut, false);
        ...
        _transferTo(_tokenOut, amountOut, _receiver);
        emit Unstake(msg.sender, _tokenOut, _rolpAmount, amountOut);
    }
    ...
    function emergencyDeposit(address _token, uint256 _amount) external {
        require(settingsManager.isCollateral(_token) || settingsManager.isStable(_token), "Invalid deposit token");
    ...
    //     require(isEnableConvertRUSD, "Convert RUSD temporarily disabled");
    //     uint256 _orderIndex = lastIndex[msg.sender];
    //     require(IERC20(RUSD).balanceOf(msg.sender) >= _amount, "Insufficient RUSD to convert");
    //     IMintable(RUSD).burn(_recipient, _amount);
    //     bytes32 orderKey = _getConvertOrderKey(_token, msg.sender, _orderIndex);
    //     ConvertOrder storage order = convertOrders[orderKey];
    //     order.index = _orderIndex;
    //     order.indexToken = _token;
    //     order.sender = msg.sender;
    //     order.recipient = _recipient;
    //     order.amountIn = _amount;
    //     order.state = ConvertOrderState.PENDING;
    //     emit ExecuteConvertOrder(msg.sender, _token, _recipient, _amount, 0, _orderIndex, ConvertOrderState.PENDING);
    //     lastIndex[msg.sender] += 1;
    // }

    // function executeConvertRUSD(
    //     address _token, 
    //     address _account, 
    //     uint256 _orderIndex,
    //     uint256 _convertPrice
    // ) external onlyExecutor returns(uint256) {
    //     ConvertOrder storage order = convertOrders[_getConvertOrderKey(_token, _account, _orderIndex)];
    //     require(order.state == ConvertOrderState.PENDING, "Not in pending state");
    //     uint256 _amountIn = order.amountIn;
    //     uint256 _amountOut = (_amountIn * PRICE_PRECISION / _convertPrice) / PRICE_PRECISION;
    //     require(_amountOut > 0, "Amount out should be greater than ZERO");

    //     if (IERC20(_token).balanceOf(address(this)) >= _amountOut) {
    //         order.amountOut = _amountOut;
    //         IERC20(_token).safeTransferFrom(address(this), order.recipient, _amountOut);
    //         emit ExecuteConvertOrder(_account, _token, order.recipient, _amountIn, _amountOut, _orderIndex, ConvertOrderState.EXECUTED); 
    //         return _amountOut;
    //     } else {
    //         isEnableConvertRUSD = false;
    //         IMintable(RUSD).mint(_account, _amountIn);
    //         emit ExecuteConvertOrder(_account, _token, order.recipient, _amountIn, 0, _orderIndex, ConvertOrderState.ROLLBACKED);
    //         return uint256(0);
    //     }
    // }

    function _getConvertOrderKey(address _token, address _sender, uint256 _orderIndex) 
        internal pure returns(bytes32) {
        return keccak256(abi.encodePacked(_token, _sender, _orderIndex));
    }
    ...
    function _setBalance(EnumerableMap.AddressToUintMap storage _map, address _token, uint256 _amount, bool _isAdd) internal {
        uint256 prevBalance = _map.contains(_token) ? _map.get(_token) : 0;
        _map.set(_token, _isAdd ? prevBalance + _amount : prevBalance - _amount);
    }
}
