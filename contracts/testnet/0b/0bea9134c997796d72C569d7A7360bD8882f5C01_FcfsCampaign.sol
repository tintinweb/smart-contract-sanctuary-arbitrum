...
abstract contract SaleBase is Base {
    ...
    function exportAll() external onlyReporter view returns (uint, address[] memory, uint[] memory) {
    ...
    function export(uint from, uint to) external onlyReporter view returns (uint, address[] memory, uint[] memory) {
        uint len =  _sales.buyerList.length;
    ...
    function getClaimStatus(address user) public view returns (bool purchaseClaimed, bool certsEnabled, bool certClaimed, bool certsClaimable) {
        purchaseClaimed = _sales.userPurchaseMap[user].claimed;
        (certsEnabled, certClaimed) = _isCertsClaimable(user);
        if (isFinished() && !certClaimed && _sales.userPurchaseMap[user].totalFund > 0) {
            certsClaimable = true;
        }
    }
    ...
    // Used to claim superCerts & referral rewards
    function claimCerts(address referer, bytes memory signature) external override nonReentrant {
        _requireFinish(true);
  
        _require(!_sales.userPurchaseMap[msg.sender].claimed, "Already claimed");
        _sales.userPurchaseMap[msg.sender].claimed = true;
        ...
        // Claim cert if enable and claimable
        _claimCerts(msg.sender, totalEntitlement);
...
library Types {
    ...
    struct Purchases {
        ...
        bool claimed;     // Claimed Referral reward and/or certs (if enabled).
    }
