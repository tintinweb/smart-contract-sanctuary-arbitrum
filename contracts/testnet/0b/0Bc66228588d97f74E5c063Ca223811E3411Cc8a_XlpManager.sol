...
contract XlpManager is ReentrancyGuard, Governable, IXlpManager {
    ...
    uint256 public constant XLP_PRECISION = 10 ** 18;
    ...
    constructor(address _vault, address _usdg, address _xlp, address _shortsTracker, uint256 _cooldownDuration) public {
        gov = msg.sender;
        vault = IVault(_vault);
        usdg = _usdg;
        xlp = _xlp;
    ...
    function setShortsTrackerAveragePriceWeight(uint256 _shortsTrackerAveragePriceWeight) external override onlyGov {
        require(shortsTrackerAveragePriceWeight <= BASIS_POINTS_DIVISOR, "XlpManager: invalid weight");
    ...
    function addLiquidity(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minXlp) external override nonReentrant returns (uint256) {
        if (inPrivateMode) { revert("XlpManager: action not enabled"); }
        return _addLiquidity(msg.sender, msg.sender, _token, _amount, _minUsdg, _minXlp);
    }

    function addLiquidityForAccount(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdg, uint256 _minXlp) external override nonReentrant returns (uint256) {
        _validateHandler();
        return _addLiquidity(_fundingAccount, _account, _token, _amount, _minUsdg, _minXlp);
    }
    ...
    function removeLiquidity(address _tokenOut, uint256 _xlpAmount, uint256 _minOut, address _receiver) external override nonReentrant returns (uint256) {
        if (inPrivateMode) { revert("XlpManager: action not enabled"); }
        return _removeLiquidity(msg.sender, _tokenOut, _xlpAmount, _minOut, _receiver);
    }

    function removeLiquidityForAccount(address _account, address _tokenOut, uint256 _xlpAmount, uint256 _minOut, address _receiver) external override nonReentrant returns (uint256) {
        _validateHandler();
        return _removeLiquidity(_account, _tokenOut, _xlpAmount, _minOut, _receiver);
    }
    ...
    function _addLiquidity(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdg, uint256 _minXlp) private returns (uint256) {
        ...
        require(usdgAmount >= _minUsdg, "XlpManager: insufficient USDG output");

        uint256 mintAmount = aumInUsdg == 0 ? usdgAmount : usdgAmount.mul(xlpSupply).div(aumInUsdg);
        ...
        emit AddLiquidity(_account, _token, _amount, aumInUsdg, xlpSupply, usdgAmount, mintAmount);
    ...
    function _removeLiquidity(address _account, address _tokenOut, uint256 _xlpAmount, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        uint256 usdgAmount = _xlpAmount.mul(aumInUsdg).div(xlpSupply);
        ...
        emit RemoveLiquidity(_account, _tokenOut, _xlpAmount, aumInUsdg, xlpSupply, usdgAmount, amountOut);
