...
library StakingLib {
    ...
    function getNewReward(stakingVar storage self, address _address, uint _amount) internal view returns (uint){
        require(self.allStaking != 0, "allStaking equal 0");

        uint stakeErc20 = self.userStaking[_address].stakingAmount;
        return _amount.mulDiv(stakeErc20, self.allStaking);
    }

    function rewardSettlement(stakingVar storage self, uint _newRewardAmount) internal {
        requireGov(self);
        ...
        require(self.addressStaking.length > 0, "addressStaking is 0");

        for (uint i = 0; i < self.addressStaking.length; i++) {
            self.userStaking[self.addressStaking[i]].rewardAmount += getNewReward(self, self.addressStaking[i], _newRewardAmount);
        }
    ...
    function withdraw(stakingVar storage self) internal {
        ...
        self.userStaking[msg.sender].unStakingTimestamp = 0;
        self.userStaking[msg.sender].unStakingAmount = 0;
    ...
    function claim(stakingVar storage self) internal {
        ...
        self.userStaking[msg.sender].rewardAmount = 0;
...
contract PLPStaking is Initializable {
    ...
    function initialize(
        ...
        PLPStakingVar.minNewRewardAmount = 10 ** 18;
    ...
    function rewardSettlement(uint _newRewardAmount) external {
        PLPStakingVar.rewardSettlement(_newRewardAmount);
    }

    function updateMinNewRewardAmount(uint _minNewRewardAmount) external{
        PLPStakingVar.minNewRewardAmount = _minNewRewardAmount;
    }

    function updateUnStakingLockDuration(uint _unStakingLockDuration) external{
        PLPStakingVar.unStakingLockDuration = _unStakingLockDuration;
    }
