...
contract Vault is Ownable, ERC20, ReentrancyGuardUpgradeable {
    ...
    /// @notice Add the token address to the list of deposit allowed token addresses.
    ...
    /// @notice Remove the token address from the list of deposit allowed token addresses.
    ...
    /// @notice Add the token address to the list of withdrawal token addresses.
    ...
    /// @notice Remove the token address from the list of withdrawal token addresses.
    ...
    /// @notice Add the plugin address and plugin id.
    ...
    /// @notice Remove the plugin by plugin id.
    ...
    function addDepositRequest(address _token, uint256 _tokenAmount) external {
        ...
        // Convert the USD value to LP tokens
    ...
    function addWithdrawRequest(address _token, uint256 _amountLP) external nonReentrant {
        ...
        // Add the withdrawal request to the list
    ...
    function settleWithdrawRequest() external onlyMaster nonReentrant {
        ...
        // Clear the withdrawal requests array
    ...
    function getTotalAsset() public view returns(uint256 _totalAsset) {
    ...
    function getTokensByPluginAndPoolId(uint256 _pluginId, uint256 _poolId) public view returns (address[] memory) {
        ...
        if(pluginIdToIndex[_pluginId] == 0) return poolAllowedTokens;
        Plugin memory plugin = plugins[pluginIdToIndex[_pluginId] - 1];
    ...
    function convertAssetToLP(uint256 _amount) public view returns (uint256) {
        ...
        return _amount * totalSupply() / getTotalAsset();
    }
    ...
    function convertLPToAsset(uint256 _amount) public view returns (uint256) {
        ...
        return _amount * getTotalAsset() / totalSupply();
    }
