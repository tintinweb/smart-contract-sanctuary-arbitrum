...
interface IRaidHandler {
    function raidPool(uint256 tokenId, address raider) external returns (uint256 requestId);
}
...
contract Staking is  UUPSUpgradeable, AutomationCompatible, IERC721Receiver, CCIPReceiverUpgradable, OwnableUpgradeable{
    ...
    function initialize(
        ...
        baseAPR = _baseAPR;
        noOfChains=3;
        globalStakedBudsCount=42000000*3;
    }
...
//--------------------------------------------------STAKING FUNCTION-----------------------------------------------//
    ...
    function raid(uint256 tokenId) public {
        ...
        uint256 res = IRaidHandler(raidHandler).raidPool(tokenId, msg.sender);
        if(tokenId != 0){
    ...
    function crossChainRaid(uint16 chainSelector, uint256 tokenId) external payable {
        require(_narcToken.balanceOf(msg.sender) != 0, "For narcs");
        ...
        //add ccip cross chain messaging   
        ...
        override
    {
        ...
        if(messageType == CROSS_CHAIN_STAKE_MESSAGE){
            ...
            uint256 res = IRaidHandler(raidHandler).raidPool(tokenId, sender);
        }else{
            revert("Invalid msg");
        }
    }
