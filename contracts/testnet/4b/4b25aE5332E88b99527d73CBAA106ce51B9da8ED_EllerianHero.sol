...
contract EllerianHero is ERC721 {
  ...
  address private uncWalletAddress;         // Reference to the unclaimed wallet contract. 
   ...
   */
  function SetAddresses(address _uncAddr, address _upgradeableAddr, 
  address _tokenMinterAddr, address _vrfAddr, address _verifierAddr) external onlyOwner {
    uncWalletAddress = _uncAddr;
  ...
  */
  function mintPresales (address _owner, uint256 _amount, uint256 _variant, bytes32[] memory _proof) public payable {
      ...
      if (_owner != ownerAddress) {  
        require (msg.value == mintCostInWEI * _amount);
      }
  ...
  */
  function mintUsingToken(address _recipient, uint256 _variant) public returns (uint256) {
    require( currentSupply < maximumMintable, "8");
