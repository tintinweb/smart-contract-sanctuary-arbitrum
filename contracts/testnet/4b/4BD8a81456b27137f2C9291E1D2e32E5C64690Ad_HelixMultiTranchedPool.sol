...
contract HelixMultiTranchedPool is BaseUpgradeablePausable, IHelixMultiTranchedPool, IRequiresUID, IVersioned {
  ...
  uint8 internal constant PATCH_VERSION = 9;
  ...
  function test() view public returns(uint256, uint256,uint256,uint256,uint256,uint256 ){
  ...
  return (HelixTranchingLogic.sharePriceToUsdc(
      juniorTrancheInfo.principalSharePrice,
      juniorTrancheInfo.principalDeposited
    ), juniorTrancheInfo.principalSharePrice, juniorTrancheInfo.principalDeposited, 
    HelixTranchingLogic.sharePriceToUsdc(
        seniorTrancheInfo.principalSharePrice,
        seniorTrancheInfo.principalDeposited
      ), seniorTrancheInfo.principalSharePrice, seniorTrancheInfo.principalDeposited);
  }

  function drawdown() external view override onlyLocker notCancelled whenNotPaused returns(uint256,uint256,uint256,uint256) {
    ...
    uint hh = HelixTranchingLogic.sharePriceToUsdc(
        seniorTrancheInfo.principalSharePrice,
        seniorTrancheInfo.principalDeposited
      );

    uint tt = amountAvailable;
    ...
    return (amountAvailable, gg, hh, tt);
    // return 0;
  }
...
abstract contract IHelixMultiTranchedPool is IHelixTranchedPool {
    ...
    function drawdown() external virtual returns(uint256,uint256,uint256,uint256);
