...
contract ElleriaBidManager is Ownable, ReentrancyGuard {
 ...
 function SupplementBid(uint256 _bidId, uint256 _newElmBidPrice) external nonReentrant onlyIfBidValid(_bidId) {
    require(bids[bidCounter].owner == msg.sender, "AuctionBridge: you are not owner");
    require(bids[bidCounter].elmBidPrice < _newElmBidPrice, "AuctionBridge: bids can only be raised");

    uint256 valueDifference = (_newElmBidPrice - bids[bidCounter].elmBidPrice) * bids[bidCounter].remainingBidQuantity;
    elleriumAbi.transferFrom(msg.sender, address(this), valueDifference);
  
    bids[bidCounter].elmBalance = bids[bidCounter].elmBalance + valueDifference;
    bids[bidCounter].elmBidPrice = _newElmBidPrice;

    emit BidUpdated(
      msg.sender, 
      bids[bidCounter].elmBalance, 
      bids[bidCounter].usdcBalance, 
      bids[bidCounter].remainingBidQuantity, 
      bidCounter++
      );
  }
