...
contract SmolMarriage is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, ISmolMarriage {
    ...
    mapping(address => mapping(uint256 => RedeemableDivorce)) public redeemableDivorce;
    ...
    function divorce(
        ...
        redeemableDivorce[creature2.ownerResolver][creature2.tokenId] = RedeemableDivorce(
            true,
            pendingRing,
            penaltyFee
        );
    ...
    /**
     * @notice Redeem the ring of the partner of the divorce initiator
     * @param creature creature object of creature to redeem the ring from
     */
    function redeemDivorcedRing(ICreatureOwnerResolverRegistry.Creature memory creature) public nonReentrant {
        require(isOwner(msg.sender, creature), "SMOLMARRIAGE:NOT_OWNER_OF_CREATURE");
        require(
            redeemableDivorce[creature.ownerResolver][creature.tokenId].valid,
            "SMOLMARRIAGE:NO_PENDING_RING_AFTER_DIVORCE"
        );
        if (redeemableDivorce[creature.ownerResolver][creature.tokenId].penaltyFee > 0) {
            magic.transferFrom(
                address(this),
                msg.sender,
                redeemableDivorce[creature.ownerResolver][creature.tokenId].penaltyFee
            );
        }
        staking.withdrawRing(redeemableDivorce[creature.ownerResolver][creature.tokenId].ring, creature, msg.sender);
        emit RedeemedDivorcedRing(
            creature,
            redeemableDivorce[creature.ownerResolver][creature.tokenId].ring,
            redeemableDivorce[creature.ownerResolver][creature.tokenId].penaltyFee
        );
        delete redeemableDivorce[creature.ownerResolver][creature.tokenId];
    }
