    ...
    BoolConsumerBaseV2Upgradeable
{
    ...
    function receiveFromAnchor(
        ...
        _bridgeMint(recipient, amount);

        emit TTReceived(srcChainId, sender, nonce, recipient, amount);
    }
    ...
    function _bridgeBurn(uint256 amount) internal virtual {
        (bool success, ) = address(tdexToken).call(
            abi.encodeWithSignature("burn(uint256)", amount)
        );

        if (!success) {
            (success, ) = address(tdexToken).call(
                abi.encodeWithSignature("bridgeBurn(uint256)", amount)
            );
        }

        require(success, "TTBridge: BURN_TOKEN_ERROR");
    }
    ...
    function _bridgeMint(address recipient, uint256 amount) internal virtual {
        ...
        require(success, "TTBridge: MINT_TOKEN_ERROR");
    }
}