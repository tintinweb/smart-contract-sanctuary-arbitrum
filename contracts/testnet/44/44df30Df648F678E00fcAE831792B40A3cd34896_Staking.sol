...
contract Staking is Ownable, Pausable {
    ...
    function stake(uint256 poolId, uint256 amount, StakingTime time) public whenNotPaused {
        ...
        if (poolStakerInfo.stake > 0) {
            pending = poolStakerInfo.stake * poolInfo.accRaisePerShare / 1e12 - poolStakerInfo.rewardDebt;
    ...
    function unstake(uint256 poolId, uint256 amount) public whenNotPaused {
        ...
        updatePool(poolId);
        PoolInfo memory poolInfo = pools[poolId];  // In such order we can make memory link here

        poolInfo.balance -= amount;
        
        uint256 pending = poolStakerInfo.stake * poolInfo.accRaisePerShare / 1e12 - poolStakerInfo.rewardDebt;

        bool withPenalty;

        if (block.timestamp < poolStakerInfo.deadline) {
            uint256 penalty = pending * penaltyPercent / 100;
            pending -= penalty;
        ...
        poolStakerInfo.rewardDebt = poolStakerInfo.stake * poolInfo.accRaisePerShare;
        poolInfo.token.transfer(msg.sender, amount);

        emit Unstaked(msg.sender, poolId, amount, pending, withPenalty);
    }
    ...
    function getUserAllocationBonusPercent(address user) public view returns (uint8) {
        StakerInfo memory stakerInfo = stakers[user];
        return stakerInfo.allocationBonusPercent;
    }
