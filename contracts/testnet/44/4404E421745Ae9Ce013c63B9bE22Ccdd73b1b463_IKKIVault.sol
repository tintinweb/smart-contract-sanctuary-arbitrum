...
contract IKKIVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 IKKIAtLastUserAction; // keeps track of IKKI deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // IKKI token
    ...
    /**
     * @notice Constructor
     * @param _token: IKKI token contract
    ...
    /**
     * @notice Deposits funds into the IKKI Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in IKKI)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.IKKIAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Reinvests IKKI tokens into Craftsman
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the IKKI Vault
     */
     ...
     * @return Expected reward to collect in IKKI
     */
    function calculateHarvestIKKIRewards() external view returns (uint256) {
        uint256 amount = ICraftsman(craftsman).pendingIKKI(0, address(this));
     ...
     * @return Returns total pending IKKI rewards
     */
    function calculateTotalPendingIKKIRewards() external view returns (uint256) {
        uint256 amount = ICraftsman(craftsman).pendingIKKI(0, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the IKKI Vault
