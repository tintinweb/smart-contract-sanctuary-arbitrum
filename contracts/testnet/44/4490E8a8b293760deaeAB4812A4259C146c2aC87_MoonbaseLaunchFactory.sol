...
import "./MoonbaseLaunchpadBeacon.sol";
import "../LaunchpadIDOFCFS.sol";

contract MoonbaseLaunchFactory is Ownable {
    using SafeERC20 for IERC20;

    address[] public launchpads;

    MoonbaseLaunchpadBeacon immutable beacon;

    event Launchpadceated(
        string id,
        uint256 startTime,
        uint256 endTime,
        uint256 rate,
    ...
    constructor(address _initBlueprint) {
        beacon = new MoonbaseLaunchpadBeacon(_initBlueprint, _msgSender());
    }
    ...
    function createLaunchpad(
        ...
        BeaconProxy launchpad = new BeaconProxy(
            address(beacon),
            abi.encodeWithSelector(
                LaunchpadIDOFCFS(payable(address(0))).initialize.selector,
                id,
                startTime,
                endTime,
                rate,
        ...
        IERC20(token).transferFrom(msg.sender, address(launchpad), amount);

        emit Launchpadceated(id, startTime, endTime, rate, tokensForSale, priceFeed, token, amount);
    }
...
contract MoonbaseLaunchpadBeacon is Ownable {
