...
contract ReferralStorage is ReentrancyGuard, Pausable, AccessControlBase, IReferralStorage {
   ...
   */
  function getPlayerReferralInfo(address _account) public view override returns (bytes32 code_, address referrer_) {
   ...
   */
  function returnPlayerRefferalAddress(address _player) public view returns (address referrer_) {
  ...
  function setReward(address _player, address _token, uint256 _amount) external onlyManager {
    ...
    if (referrer_ != address(0)) {
      ...
      uint256 amountRebate_ = calculateRebate(_amount, tiers[referrerTiers[referrer_]].WLPRate);
   ...
   */
  function getPendingWLPRewards(address _referrer) public view returns (uint256 totalWlpAmount_) {
    address[] memory wlTokens_ = allWhitelistedTokens;
    ...
    for (uint256 i = 0; i < wlTokens_.length; ++i) {
      ...
      if (amount_ != 0) {
        ...
        usdwAmount_ = (usdwAmount_ * 1e18) / (10 ** vault.tokenDecimals(token_));
        ...
        uint256 feeBasisPoints_ = vaultUtils.getBuyUsdwFeeBasisPoints(token_, usdwAmount_);

        // Calculate the amount of WLP that can be claimed for the current token
        totalWlpAmount_ += (amountWithFee_ * (BASIS_POINTS_DIVISOR - feeBasisPoints_)) / BASIS_POINTS_DIVISOR;
      }
    }
