...
uint256 constant ID = uint256(
    keccak256("game.piratenation.generatorcountdowntimestampcomponent.v2")
);
 ...
 */
contract GeneratorCooldownTimestampComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param timestamp Timestamp of the last time the generator was used
     */
    function setValue(
        uint256 entity,
        uint256 timestamp
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(timestamp));
    }
     ...
     *
     * @param entity Entity to get value for
     * @return timestamp Timestamp of the last time the generator was used
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 timestamp) {
        if (has(entity)) {
            Layout memory s = GeneratorCooldownTimestampComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (timestamp) = abi.decode(_getEncodedValues(s), (uint256));
        }
    }
    ...
    function getByteValues(
        ...
        values = new bytes[](1);
        values[0] = abi.encode(s.timestamp);
    }
    ...
    function setBytes(
        ...
        Layout memory s = GeneratorCooldownTimestampComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.timestamp) = abi.decode(value, (uint256));
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.timestamp));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.timestamp);
    }
}
