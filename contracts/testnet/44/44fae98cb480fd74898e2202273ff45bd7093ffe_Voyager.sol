...
interface VoyagerMessageHandler {
...
contract Voyager is AccessControl, ReentrancyGuard, Pausable, IVoyager {
    ...
    bytes4 public constant HANDLE_VOYAGER_MESSAGE_SELECTOR = 0xccdde8d0;
    ...
    function iDeposit(
        ...
        if (amount > MAX_TRANSFER_SIZE || destAmount > amount)
            revert AmountTooLarge();

        if (srcToken == address(wrappedNativeToken)) {
    ...
    function iRelay(
        ...
        if (relayData.destToken == address(wrappedNativeToken)) {
            ...
            _unwrapwrappedNativeTokenTo(
                payable(relayData.recipient),
                relayData.amount
            );
        } else {
            IERC20(relayData.destToken).safeTransferFrom(
        ...
        if (isContract(relayData.recipient) && relayData.message.length > 0) {
            (execFlag, execData) = relayData.recipient.call(
                abi.encodeWithSelector(
                    HANDLE_VOYAGER_MESSAGE_SELECTOR,
                    relayData.destToken,
                    relayData.amount,
                    relayData.message
                )
