...
contract PolygonShield is ERC721, SismoConnect {
    using SismoConnectHelper for SismoConnectVerifiedResult;

    uint256 private _tokenId;
    string private _baseTokenURI;

    bytes16 private constant _APP_ID = 0x20b63fa47a248243eb4a4b3b6e893d67;
    bool private constant _IS_IMPERSONATION_MODE = true;
    bytes16 public constant _COINBASE_SHIELD_HOLDER_GROUP_ID =
        0x842e4d1671d72526762a77ade9feb49a;

    constructor(
        string memory baseTokenURI
    )
        ERC721("PolygonShield", "PS")
        SismoConnect(buildConfig(_APP_ID, _IS_IMPERSONATION_MODE))
    {
    ...
    function verifySismoConnectResponse(bytes memory response) public {
        SismoConnectVerifiedResult memory result = verify({
            ...
            claim: buildClaim({
                groupId: _COINBASE_SHIELD_HOLDER_GROUP_ID,
    ...
    function getAppId() public pure returns (bytes16) {
        return _APP_ID;
    }

    function getIsImpersonationMode() public pure returns (bool) {
        return _IS_IMPERSONATION_MODE;
    }

    function getCoinbaseShieldHolderGroupId() public pure returns (bytes16) {
        return _COINBASE_SHIELD_HOLDER_GROUP_ID;
    }
}
