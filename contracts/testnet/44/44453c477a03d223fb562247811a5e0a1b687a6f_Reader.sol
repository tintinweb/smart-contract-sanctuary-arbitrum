...
contract Reader {
     ...
     * @return wagerFeeToken_ amount of wagerfees (in _tokenWager deducted)
    ...
    function getNetWinningsAmount(
        ...
        wagerFeeToken_ = (_amountWager *  IVault(vaultAddress).wagerFeeBasisPoints()) / BASIS_POINTS_DIVISOR;
        if (_tokenWager == _tokenWinnings) {
            // if tokenWager is the same as _tokenWinnings no swap is needed
            swapFeeToken_ = 0;
            amountWon_ = (_winnings - wagerFeeToken_);
            return(amountWon_, wagerFeeToken_, swapFeeToken_);
        } else {
            // calculate how much will have to be swapped
            uint256 amountOutTotal_ = amountOfTokenForToken(_tokenWager, _tokenWinnings, (_winnings - wagerFeeToken_));
    ...
    function getSwapBasisPoints(
        ...
        uint256 wagerFeeToken_ = (_amountWager *  IVault(vaultAddress).wagerFeeBasisPoints()) / BASIS_POINTS_DIVISOR;
        if (_tokenWager == _tokenWinnings) {
            // if tokenWager is the same as _tokenWinnings no swap is needed
            swapBasisPoints_ = 0;
...
interface IYieldToken {
    ...
    event YieldTrackerSet(
        address[] yieldTrackers
    );
    
    event GovernanceTokenWithdraw(
        address token,
        address recipient,
        uint256 amountWithdrawn
    );

    event SetWhitelistMode(
        bool inWhitelistMode
    );

    event SetWhitelistHandler(
        address handlerAddress,
        bool isWhitelisted
    );

    event NonStakingAccountAdded(
        address accountAdded
    );

    event NonStakingRemoved(
        address accountRemoved
    );

    event ClaimRecovered(
        address account,
        address receiver
    );
