...
interface IXAegisToken is IERC20 {
...
import "../interfaces/IXAegisTokenUsage.sol";
import "../interfaces/IYieldBooster.sol";
import "../interfaces/tokens/IXAegisToken.sol";

/*
 * This contract is a xAEG Usage (plugin) that can boost spNFTs' yield (staking positions on NFTPools) when it
 * receives allocations from the XAegisToken contract
 */
contract YieldBooster is Ownable, ReentrancyGuard, IXAegisTokenUsage, IYieldBooster {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.UintSet;

    IXAegisToken public immutable xAegisToken; // XAegisToken contract
    ...
    // User's position xAEG total allocation
    ...
    mapping(address => uint256) private _usersTotalAllocation; // User's xAEG total allocation
    mapping(address => uint256) private _poolsTotalAllocation; // Pool's total allocation
    uint256 public totalAllocation; // Contract's total xAEG allocation

    bool public forcedDeallocationStatus; // Authorize users to forcibly deallocate everything

    constructor(IXAegisToken xAegisToken_) {
        xAegisToken = xAegisToken_;
    }
    ...
    /**
     * @dev Checks if caller is the XAegisToken contract
     */
    modifier xAegisTokenOnly() {
        require(msg.sender == address(xAegisToken), "xAegisTokenOnly: caller should be xAegisToken");
        _;
    }
    ...
    /**
     * @dev Returns total xAEG allocated to this contract by "userAddress"
     */
    ...
    /**
     * @dev Returns total xAEG allocated to this contract by "poolAddress"
     */
    ...
    /**
     * @dev Returns allocated xAEG to "tokenId" spNFT from "poolAddress" NFTPool by "userAddress"
     */
    ...
    /**
     * Allocates "userAddress" user's "amount" of xAEG to this usage contract
     * "data" should contain tokenId and poolAddress
     *
     * Can only be called by XAegisToken contract, which is trusted to verify amounts
     */
    function allocate(
        address userAddress,
        uint256 amount,
        bytes calldata data
    ) external override nonReentrant xAegisTokenOnly {
        (address poolAddress, uint256 tokenId) = abi.decode(data, (address, uint256));
        _allocate(userAddress, poolAddress, tokenId, amount);

        // allocated xAEG is added (as boost points) to spNFT
        INFTPool(poolAddress).boost(tokenId, amount);
    }

    /**
     * Deallocates "userAddress" user's "amount" of xAEG from this usage contract
     * "data" should contain tokenId and poolAddress
     *
     * Can only be called by XAegisToken contract, which is trusted to verify amounts
     */
    function deallocate(
        ...
        // should only be called if spNFT has not been burned, to avoid having stuck xAEG on it
        if (INFTPool(poolAddress).exists(tokenId)) {
            // allocated xAEG is removed (as boost points) from the spNFT
     ...
     * The pool should remove the allocated xAEG (boost points) from its own spNFT when calling this function
     */
    function deallocateAllFromPool(address userAddress, uint256 tokenId) external override nonReentrant {
        ...
        // update user's xAEG allocations balance
        xAegisToken.deallocateFromUsage(userAddress, amount);
    }

    /**
     * Deallocates msg.sender's "amount" of xAEG from XAegisToken, without adjusting this contract allocations balances
     *
    ...
    function forceDeallocate() external nonReentrant {
        ...
        uint256 amount = xAegisToken.usageAllocations(msg.sender, address(this));

        // update user's xAEG allocations balance
        xAegisToken.deallocateFromUsage(msg.sender, amount);
    }
    ...
    /**
     * @dev Returns multiplier that should be applied to a spNFT based on its boost points (allocated xAEG)
     *
     ...
     * 1% of the pool's allocated xAEG
     *
    ...
    /**
     * @dev Allocates "userAddress" user's "amount" of xAEG to "tokenId" spNFT of "poolAddress" pool
     */
    ...
    /**
     * @dev Deallocates "userAddress" user's "amount" of xAEG allocated to "tokenId" spNFT of "poolAddress" pool
     */
    function _deallocate(address userAddress, address poolAddress, uint256 tokenId, uint256 amount) internal {
        ...
        require(userPositionAllocation >= amount, "deallocate: not enough allocated xAegis");
