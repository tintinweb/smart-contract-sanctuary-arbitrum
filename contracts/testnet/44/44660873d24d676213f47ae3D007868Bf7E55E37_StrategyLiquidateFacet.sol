...
contract StrategyLiquidateFacet is IStrategyLiquidate {
    ...
    // todo 接口参数要改
    function liquidateStrategy(uint256 strategyId) external {
        ...
        l.userLiquidateRequest[requestHash].strategyId = strategyId;
        l.userLiquidateRequest[requestHash].admin = admin;

        uint256 mergeId;
        emit StrategyLiquidationRequested(requestHash, strategyId, mergeId);
    }
    ...
    function executeLiquidateStrategy(bytes32 requestHash) external {
        ...
        if (requestParam.strategyId == 0) {
            revert InvalidStrategyId(requestParam.strategyId);
        }
        ...
        // 校验其保证金是否大于维持保证金
        LibMarketPricer._checkCollateralSufficiency(strategy.collaterals, requestHash, false);

        // 验证新用户金额并划转（更新余额）

        // 新的用户接盘 转移nft

        // todo 如果是清算+合并，给待合并的策略id赋值
        uint256 mergeId;
        emit StrategyLiquidationExecuted(requestHash, requestParam.strategyId, mergeId);
    }
}
