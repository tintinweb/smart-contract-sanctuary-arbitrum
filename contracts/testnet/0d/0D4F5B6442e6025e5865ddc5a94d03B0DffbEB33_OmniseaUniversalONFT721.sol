...
contract OmniseaUniversalONFT721 is NonblockingLzApp, ERC165, ReentrancyGuard, IOmniseaUniversalONFT721 {
    ...
    constructor(address _lzEndpoint, uint256 _minGasToTransferAndStore) NonblockingLzApp(_lzEndpoint) {
    ...
    function sendFrom(address _from, uint16 _dstChainId, bytes memory _toAddress, uint _tokenId, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams, BasicCollectionParams memory _collectionParams) public payable virtual override {
    ...
    function sendBatchFrom(address _from, uint16 _dstChainId, bytes memory _toAddress, uint[] memory _tokenIds, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams, BasicCollectionParams memory _collectionParams) public payable virtual override {
    ...
    function _send(
        ...
        if (collectionId == bytes32(0)) {
            // Original collection must be registered in the repository
//            require(_factory.isDrop(msg.sender));
...
contract OmniseaERC721Proxy {
    fallback() external payable {
        _delegate(address(0x6ef0871ed810f323eA516A77B0988353b667dfa4));
    }
