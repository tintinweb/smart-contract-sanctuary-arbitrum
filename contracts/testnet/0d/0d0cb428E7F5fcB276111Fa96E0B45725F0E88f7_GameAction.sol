...
library GameAction {
    ...
    function _move(
        ...
        emit ActionMove(p.round, p.player, state.eventCount++, pos, p.path);
    }
    ...
    function move(
        ...
        _move(playersData, tilemap, state, p);
    }
    ...
    function moveWithBoots(
        mapping(address => uint) storage playersData, 
        mapping(uint => uint) storage tilemap, 
        IBattleRoyaleGameV1.GameState storage state,
        BootsParams memory p
    ) external {
        ...
        _move(playersData, tilemap, state, MoveParams({
            config: p.config,
            round: state.round,
            mapSeed: p.mapSeed,
            player: p.player,
    ...
    function _calcDamage(ShootParams2 memory p, IBattleRoyaleGameV1.GameState memory state) private view returns(uint) {
        ...
        uint seed = uint(keccak256(abi.encodePacked(p.mapSeed, p.player, p.tokenId, GameView.gameTick(state.startTime, p.config.tickTime))));
    ...
    function _shoot(
        ...
        uint damage = _calcDamage(p, state);
        emit ActionShoot(state.round, p.player, state.eventCount++, p.fromPos, p.toPos, damage > 0);
        if (damage > 0) {
            if (p.targetIsPlayer) {
                GameBasic.applyDamage(players, playersData, tokensOnGround, tilemap, state, GameBasic.ApplyDamageParams({
                    nft: p.nft,
                    mapSeed: p.mapSeed,
                    config: p.config,
                    player: p.target,
                ...
                GameBasic.destroyTile(tilemap, tokensOnGround, state, p.config.mapSize, p.toPos, p.mapSeed,state.round, GameView.gameTick(state.startTime, p.config.tickTime), p.nft);
            }
        }
    }
    ...
    function shoot(
        ...
        _shoot(
            players, playersData, tokensOnGround, tilemap, state,
            ShootParams2({
            fromPos: fromPos,
            toPos: toPos,
            target: p.target,
    ...
    function pick(
        ...
        require(p.pos.length == p.tokensIdAndIndex.length && p.pos.length > 0, "Pick1");
        ...
        emit ActionPick(state.round, p.player, state.eventCount++);
    ...
    function drop(
        ...
        emit ActionDrop(p.round, p.player, state.eventCount++);
        emit MapChanged(state.round, state.eventCount++);
    }
    ...
    function _bombOne(
        ...
        if (p.targetIsPlayer) {
            ...
            GameBasic.applyDamage(players, playersData, tokensOnGround, tilemap, state,
            ...
            GameBasic.destroyTile(tilemap, tokensOnGround, state, p.config.mapSize, uint(uint160(p.target)), p.mapSeed, state.round, GameView.gameTick(state.startTime, p.config.tickTime), p.nft);
        }
    }
    ...
    function bomb(
        ...
        for(uint i = 0; i < p.targets.length; i++) {
            _bombOne(players, playersData, tilemap, tokensOnGround, state, BombParams2({
                throwRange: throwRange,
                explosionRange: explosionRange,
                damage: damage,
                bombPos: p.bombPos,
        ...
        emit ActionBomb(state.round, p.player, state.eventCount++, playerPos, p.bombPos, explosionRange);
    }
    ...
    function eat(
        ...
        emit ActionEat(p.round, p.player, state.eventCount++, hp2 - hp);
    }
