...
import "./ERC1155TokenSetCorruptionHandlerContracts.sol";

contract ERC1155TokenSetCorruptionHandler is Initializable, ERC1155TokenSetCorruptionHandlerContracts {

    function initialize() external initializer {
        ERC1155TokenSetCorruptionHandlerContracts.__ERC1155TokenSetCorruptionHandlerContracts_init();
    }
    ...
    onlyCorruptionRemoval
    {
        ...
        uint256[] memory _suppliedTokenIds = abi.decode(_userData, (uint256[]));

        require(_requirement.amount > 0, "Requirement amount must be greater than 0");
        require(_suppliedTokenIds.length == _requirement.amount, "Must give the same number of tokens as required amount");

        for(uint256 i = 0; i < _suppliedTokenIds.length; i++) {
            uint256 _suppliedTokenId = _suppliedTokenIds[i];

            bool _isTokenIdAcceptable = false;
            for(uint256 j = 0; j < _requirement.tokenIdOptions.length; j++) {
                if(_suppliedTokenId == _requirement.tokenIdOptions[j]) {
                    _isTokenIdAcceptable = true;
                    break;
                }
            }

            require(_isTokenIdAcceptable, "Bad token id");

            IERC1155Upgradeable(_requirement.collection).safeTransferFrom(_user, treasuryAddress, _suppliedTokenId, 1, "");
        }
    }
...
import "./ERC1155TokenSetCorruptionHandlerState.sol";

abstract contract ERC1155TokenSetCorruptionHandlerContracts is Initializable, ERC1155TokenSetCorruptionHandlerState {

    function __ERC1155TokenSetCorruptionHandlerContracts_init() internal initializer {
        ERC1155TokenSetCorruptionHandlerState.__ERC1155TokenSetCorruptionHandlerState_init();
    }
    ...
    modifier contractsAreSet() {
        require(areContractsSet(), "ERC1155TokenSetCorruptionHandler: Contracts aren't set");
        _;
    }
...
import "./IERC1155TokenSetCorruptionHandler.sol";
import "../../shared/AdminableUpgradeable.sol";
import "../external/ITreasure.sol";
import "../treasuremetadatastore/ITreasureMetadataStore.sol";
import "../corruptionremoval/ICorruptionRemoval.sol";
import "../corruptionremoval/ICustomRemovalHandler.sol";
import "../consumable/IConsumable.sol";

abstract contract ERC1155TokenSetCorruptionHandlerState is Initializable, ICustomRemovalHandler, IERC1155TokenSetCorruptionHandler, AdminableUpgradeable {

    ICorruptionRemoval public corruptionRemoval;
    address public treasuryAddress;

    function __ERC1155TokenSetCorruptionHandlerState_init() internal initializer {
...
interface IERC1155TokenSetCorruptionHandler {
