 ...
 * contract written by Tim Clancy. It removes circumstances
 */
abstract contract Managerial is Ownable {
  ...
  /**
   * @dev Emitted when a permit is updated.
   * @param updator The address that updated the permit.
   * @param updatee The address that had its permit updated.
 ...
 * 8 March, 2023
 */
contract November721a is ERC721A, Managerial {
  using Strings for uint256;

  /// Constant identifiers for manager rights in contract
   ...
   * @param _burner The address that burned the token
   */
  event TokensSudoBurned(uint256[] _tokenIds, address _burner);

  /**
   * @dev Emitted when an owner burns their tokens
   * @param _tokenIds The token IDs
   * @param _burner The address that burned the token
   */
   ...
   * @param _withdrawer The address that withdrew the contract
   */
   ...
   * @param _operator The address that acting on behalf of the owner
   * @return _tokenURI The URI for the token
   */
   ...
   * @param _receivers Array of addresses to mint to
   */
   ...
   * @dev `_burn(..., true)` verifies caller is owner or approved operator for
   * each token being burned
   */
