...
contract Portal is Ownable, Pausable, ReentrancyGuard {
    ...
    event UpdatePortalMetadata(string name, address[] fundingPool, address tokenAddress, uint256 tokenPrice, uint256 vestingAllocInBps);
    ...
    function setClaimableAt(uint256 _claimableAt) external onlyOwner {
        ...
        emit UpdateClaimableAt(tokenAddress, name, _claimableAt);
    }
     ...
     *
     */
    function setPortalMetadata(string memory _name, address[] memory _fundingPool, address _tokenAddress, uint256 _tokenPrice, uint256 _vestingAllocInBps) external onlyOwner {
        name = _name;
        fundingPool = _fundingPool;
        tokenAddress = _tokenAddress;
        tokenPrice = _tokenPrice;
        ...
        emit UpdatePortalMetadata(_name, _fundingPool, _tokenAddress, _tokenPrice, _vestingAllocInBps);
    }
