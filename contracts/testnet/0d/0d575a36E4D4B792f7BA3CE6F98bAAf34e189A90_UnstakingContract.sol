...
constructor(address _token, address _distributionContract) {
    token = ArbotToken(_token);
...
function stake(uint256 amount) external {
    ...
    if (stakes[msg.sender].amount == 0) {
        stakes[msg.sender] = Stake(amount, getCurrentEpoch());
    } else {
...
function distributeRewards(uint256 amount) external {
    require(msg.sender == distributionContract, "Not authorized");

    require(totalStaked > 0, "No tokens staked");
    require(token.transfer(distributionContract, amount), "Transfer failed");
}

function getCurrentEpoch() public view returns (uint256) {
    // Add the logic to calculate the current epoch based on block.timestamp and epoch duration here
}
}
...
contract UnstakingContract {
    ...
    function unstake(uint256 amount) external {
        ...
        if (penalty > 0) {
            token.transfer(address(distribution), penalty);
        }
