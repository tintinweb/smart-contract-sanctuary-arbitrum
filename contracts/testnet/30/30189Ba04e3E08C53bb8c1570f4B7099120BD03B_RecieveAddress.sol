...
contract RecieveAddress is IStargateReceiver{

    address public stargateRouter;
    address public dealer;
    address public mockSwap;
    address public USDC;
    address public USDCJOJO;
    uint256 public gasUsed;

    using SafeERC20 for IERC20;

    event ReceivedOnDestination(address token, uint);

    constructor(address _stargateRouter, address _Dealer, address _mockSwap, address _USDC, address _USDCJOJO){
    ...
    function sgReceive(
        ...
        require(_token == USDC, "is not usdc");
        (address _toAddr) = abi.decode(_payload, (address));
        IERC20(USDC).approve(mockSwap, amountLD);
        SupportsSwap(mockSwap).swap(amountLD, _token);

        IERC20(USDCJOJO).approve(dealer, amountLD);
        IDealer(dealer).deposit(amountLD, 0, _toAddr);
