 ...
 *  @author [@epiqueras, @shotaronowhere] - <[email protected]>
...
contract ExposedDualSortitionSumTreeFactory {
     ...
     */
    function _sortitionSumTrees(bytes32 _key) public view returns(uint K_draw, uint[] memory stackDraw, uint[] memory nodesDraw,uint K_set, uint[] memory stackSet, uint[] memory nodesSet, uint _threshold, uint _lastThresholdUpdate) {
        return (
     ...
     */
    function _queryLeafs(bytes32 _key, uint _cursor, uint _count, bool _isSetTree) public view returns(uint startIndex, uint[] memory values, bool hasMore) {
