...
interface ISmolChopShop {
    ...
    /// @notice Burns amount of trophies in exchange for equal value in shop trophies. One way exchange. No converting back to racingTrophies from shopTrophies. ShopTrophies are only used to buy vehicle upgrades. 
    /// @param _trophyId Token Id of trophy type to be burned.
    /// @param _amountToBurn Amount of trophies to be exchanged.
    function exchangeTrophies(
        uint256 _trophyId, 
    ...
    /// @notice Set other trophy values in shopTrophies for calculating exchange rate.
    /// @param _trophyId Array of trophy id numbers from the racing trophies contract.
    /// @param _trophyExchangeValue Array of trophy values (denominated in shopTrophies) to be assigned to each given _trophyId.
    ...
    /// @param _trophyId Id number of trophy from the racing trophies contract.
    function setShopTrophyId(
        uint256 _trophyId
    ) external;
...
contract SmolChopShop is Initializable, SmolChopShopInternal {
    ...
    // Burns amount of trophies in exchange for equal value in shop trophies. Shop trophies are only used to buy vehicle upgrades. 
    // One way exchange. No converting back to racingTrophies from shopTrophies.
    /// @inheritdoc ISmolChopShop
    function exchangeTrophies(
        uint256 _trophyId, 
        uint256 _amountToBurn
    ) external contractsAreSet whenNotPaused {
        require (_amountToBurn > 0);
        if (trophyExchangeValue[_trophyId] == 0) revert TrophyExchangeValueNotSet();
        _checkTrophyBalance(msg.sender, _trophyId, _amountToBurn);
        uint256 amountToRecieve = _amountToBurn * trophyExchangeValue[_trophyId];
        racingTrophies.burn(msg.sender, _trophyId, _amountToBurn);
        racingTrophies.mint(msg.sender, shopTrophyId, amountToRecieve);
    }
...
abstract contract SmolChopShopAdmin is Initializable, SmolChopShopView {
    ...
    // Set exchange rate for 1155 trophy ids from racing trophy contract denominated in shopTrophies.
    /// @inheritdoc ISmolChopShop
    function setExchangeRates(
        uint256[] calldata _trophyId,
        uint256[] calldata _trophyExchangeValue
...
abstract contract SmolChopShopInternal is Initializable, SmolChopShopAdmin {
    ...
    function _unlockUpgrade(
        ...
        if (_price != 0){
            racingTrophies.burn(_userAddress, shopTrophyId, _price);
        }
...
abstract contract SmolChopShopState is Initializable, UtilitiesV2Upgradeable, ISmolChopShop {
    ...
    /// @notice Id number of ERC1155 token from racing trophys contract that is used for upgrade payment and exchange
    /// @dev used to set values for other trophies and must be set for contract to function
    uint256 public shopTrophyId;

    /// @notice Value associated with each trophy id denominated in shop trophies.
    /// @dev must be set before trophies can be exchanged
    mapping (uint256 => uint256) 
        public trophyExchangeValue;

    /// @notice Mapping for keeping track of how many shop trophies user spent in total.
...
abstract contract SmolChopShopView is Initializable, SmolChopShopState {
    ...
    function _checkBeforePurchase(
        ...
        if (price_ != 0){
            _checkTrophyBalance(_userAddress, shopTrophyId, price_);
        }
    }
    ...
    function _checkTrophyBalance(
        ...
        if (shopTrophyId == 0) revert ShopTrophyIdNotSet();
