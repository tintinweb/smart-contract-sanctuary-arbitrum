    ...
    Ownable
{
    ...
    event LiquidityRemovedFromPosition(
        address indexed user,
        uint256 tokenId,
        uint256 amount,
        uint256 shares,
    ...
    // FIXME do we really need to pause withdrawals? i am always writing it as we cannot limit our users to withdraw, only pause new deposits
    // Got it.
    /**
    ...
    function addLiquidityPosition(
        ...
        require(
            user == msg.sender || isApprovedForAll(user, msg.sender),
        ...
        internal
    {
        ...
        require(
            msg.sender == user || isApprovedForAll(user, msg.sender),
            "NOT_APPROVED"
        );
    ...
    function _removeLiquidityPosition(
        ...
        emit LiquidityRemovedFromPosition(
            user,
            tokenId,
            _underlyingTokenAmount,
            shareAmount,
        ...
        nonReentrant
    {
        ...
        require(
            block.timestamp >= _request.timestamp + WITHDRAWAL_DELAY_TIME,
            "TOO_EARLY"
        );
        // Check if request is already executed
        require(
            _request.status == WithdrawalRequestStatus.NONE,
            "ALREADY_EXECUTED_OR_CANCELLED"
        );
    ...
    function _cutTradingFee(uint256 amount) internal returns (uint256) {
        ...
        uint256 treasury_fee = (fee * config.treasury_bips()) /
            config.FEE_BASE();
     ...
     */
    function increaseDepositedAmount(uint256 amount)
        external
        override
        onlyMarket
    {
     ...
     * @dev This function is used to update total deposited amount from user betting
     */
    function decreaseDepositedAmount(uint256 amount)
        external
        override
        onlyMarket
    {
