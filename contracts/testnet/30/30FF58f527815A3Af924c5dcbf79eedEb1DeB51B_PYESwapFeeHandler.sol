...
import { OpsReady, IOps } from "./external/gelato/OpsReady.sol";
import { IPYESwapRouter } from "./interfaces/IPYESwapRouter.sol";
import { ISmartChefPYE } from "./interfaces/ISmartChefPYE.sol";
import { IPYESwapToken } from "./interfaces/IPYESwapToken.sol";

contract PYESwapFeeHandler is Ownable, OpsReady {
    ...
    event FeeDestinationsSet(address _developmentDest, address _stakingDest, bool _isStakingContract);

    event VestingParamsSet(address _vestingDest, bool _distributeToVesting);
    ...
    constructor (
        ...
        router = IPYESwapRouter(0xBF70b27466c0cE7330E603Cd6F56A73D9bc208d2);
        weth = IERC20(_weth);
        pyes = 0xf5a8141FD66C07df73568e35caD5D8507F9193fd;
        swapPath.push(_weth);
        swapPath.push(0xf5a8141FD66C07df73568e35caD5D8507F9193fd);
        minimumDistribution = 10**14;
        development = 2000;
        staking = 3500;
        buyBack = 4500;
        developmentDest = 0x6FcC9408b0931F9671F24A463e8ed23A9e9A80e9;
        stakingDest = 0x4ddE29173fF4696e2dB55798FFB145841d165dA5;
        isStakingContract = true;
        gasReserve = 10**12;
    }
    ...
    function updateFeeDest(address _developmentDest, address _stakingDest, bool _isStakingContract) external onlyOwner {
        developmentDest = _developmentDest;
        stakingDest = _stakingDest;
        isStakingContract = _isStakingContract;

        emit FeeDestinationsSet(_developmentDest, _stakingDest, _isStakingContract);
    }

    function updateVestingDest(address _vestingDest, bool _distributeToVesting) external onlyOwner {
        vestingDest = _vestingDest;
        distributeToVesting = _distributeToVesting;

        emit VestingParamsSet(_vestingDest, _distributeToVesting);
    }
    ...
    function startDistributeTask() external onlyOwner {
        distributeTaskId = IOps(ops).createTaskNoPrepayment(
            address(this), 
            this.autoDistributeRewards.selector,
            address(this),
            abi.encodeWithSelector(this.canAutoDistributeRewards.selector),
            address(weth)
        );
    ...
    function startSwapATask() external onlyOwner {
        swapATaskId = IOps(ops).createTaskNoPrepayment(
            address(this), 
            this.autoSwapA.selector,
            address(this),
            abi.encodeWithSelector(this.canAutoSwapA.selector),
            address(weth)
        );
    ...
    function stopSwapATask() external {
        _stopTask(swapATaskId);
        emit SwapATaskStopped(swapATaskId, block.timestamp);
        delete swapATaskId;
    }

    function autoSwapA() external onlyOps {
        (uint256 fee, address feeToken) = IOps(ops).getFeeDetails();
        _transfer(fee, feeToken);
    ...
    function startSwapBTask() external onlyOwner {
        swapBTaskId = IOps(ops).createTaskNoPrepayment(
            address(this), 
            this.autoSwapB.selector,
            address(this),
            abi.encodeWithSelector(this.canAutoSwapB.selector),
            address(weth)
        );
    ...
    function stopSwapBTask() external {
        _stopTask(swapBTaskId);
        emit SwapBTaskStopped(swapBTaskId, block.timestamp);
        delete swapBTaskId;
    }

    function autoSwapB() external onlyOps {
        (uint256 fee, address feeToken) = IOps(ops).getFeeDetails();
        _transfer(fee, feeToken);
    ...
    function canAutoDistributeRewards() external view returns (bool canExec, bytes memory execPayload) {
        ...
        execPayload = abi.encodeWithSelector(this.autoDistributeRewards.selector);
    }
    ...
    function canAutoSwapA() external view returns (bool canExec, bytes memory execPayload) {
        ...
        execPayload = abi.encodeWithSelector(this.autoSwapA.selector);
    }
    ...
    function canAutoSwapB() external view returns (bool canExec, bytes memory execPayload) {
        ...
        execPayload = abi.encodeWithSelector(this.autoSwapB.selector);
    }

    function _stopTask(bytes32 taskId) internal {
    ...
    function _swapAB() internal {
        uint256 lengthA = tokensToSwapA.length();
        address tkn;
        uint256 bal;
        address[] memory path = new address[](2);
        path[1] = address(weth);
        for (uint a = 0; a < lengthA; a++) {
            tkn = tokensToSwapA.at(a);
            bal = IERC20(tkn).balanceOf(address(this));
            if (bal > minimumSwapAmt[tkn]) {
                path[0] = tkn;
                IERC20(tkn).approve(address(router), bal);
        ...
        uint256 lengthB = tokensToSwapA.length();
        for (uint b = 0; b < lengthB; b++) {
            tkn = tokensToSwapB.at(b);
            bal = IERC20(tkn).balanceOf(address(this));
            if (bal > minimumSwapAmt[tkn]) {
                path[0] = tkn;
                IERC20(tkn).approve(address(router), bal);
    ...
    function _distributeRewards() internal {
        ...
        if (buybackShare > 0) {
            ...
            if (!useDead) { IPYESwapToken(pyes).burn(amounts[1]); }
