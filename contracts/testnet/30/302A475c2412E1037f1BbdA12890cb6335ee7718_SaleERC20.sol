...
contract SaleERC20 is Pausable, Initializable {
    ...
    event EmergencyWithdrawed(address indexed user, uint256 amount);
    ...
    function withdraw(uint256 amount) public onlySaleOwner {
    ...
    function emergencyWithdraw(uint256 amount) public onlyRaiseAdmin ifProjectUnhealthy {
        require(serviceBalance >= amount, "Not enough service balance");

        serviceBalance -= amount;
        tokenToSell.safeTransfer(msg.sender, amount);

        emit EmergencyWithdrawed(msg.sender, amount);
    }
        ...
        public onlyRaiseAdmin
    {
        ...
        if (roundsLength == 0)
            require(serviceBalance >= minimumAmountToFund, "Sale is not funded");
    ...
    function buy(uint256 saleTokenAmount) public whenNotPaused onlyHealthy {
    ...
    function isSaleFinished() public view returns (bool) {
        ...
        return rounds[roundsLength - 1].isFinal && block.timestamp > rounds[roundsLength - 1].deadline;
    }
}
