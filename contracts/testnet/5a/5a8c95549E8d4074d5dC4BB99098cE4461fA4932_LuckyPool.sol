...
contract LuckyPool is Caller, ReentrancyGuard {
    ...
    function trade(address user, uint256 usd) external onlyCaller {
        require(inProgress(), "not progress");
        require(usd >= 100, "Insufficient USD");
        tickets.push(Ticket({user: user, weight: usd}));
    ...
    function claimReward() external nonReentrant {
        ...
        emit ClaimReward(pool.period, user, reward);
        historyPool.push(
            HistoryPool({
                period: pool.period,
                winner: user,
        ...
        if (reward > 0) {
            // send eth
            SafeTransfer.unwrapWETH9(WETH, user, reward);
        }
    }
