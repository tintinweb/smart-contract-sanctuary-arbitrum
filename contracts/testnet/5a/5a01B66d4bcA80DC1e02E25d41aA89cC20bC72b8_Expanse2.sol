...
import "./interfaces/IExpanse2.sol";
...
contract Expanse2 is IExpanse2, IManager, Ownable {
  ...
  mapping(address => PositionInfo) private clientPositions;
  mapping(address => PositionInfo[]) private clientPositionsHistory;
  ...
  function newPosition(
    ...
    PositionInfo memory newClientPosition = PositionInfo(
      State.Purchased,
      wbtcAmount,
      usdceAmount,
      0,
  ...
  function getPosition(address clientUUID) public view override returns (PositionInfo memory) {
    require(clientUUID != address(0), "Invalid clientUUID");
    return clientPositions[clientUUID];
  }
  ...
  function swapUSDCEtoWBTC(
    ...
    for (uint i = 0; i < clientUUIDs.length; i++) {
      ...
      if (
        ...
        clientPositions[clientUUIDs[i]].entryAmount * 125 <=
  ...
  function borrowUSDCE(
    ...
    IPool(AAVE_POOL).borrow(USDCE, totalAmount, 1, 0, address(this));
  ...
  function repayUSDCE(
    ...
    IPool(AAVE_POOL).repay(USDCE, totalAmount, 1, address(this));
    _safeSubTokens(USDCE, totalAmount);
...
abstract contract IManager {
  ...
  modifier onlyManager() {
    require(managers.contains(msg.sender), "Not manager");
    _;
  }
  ...
  function _removeManager(address _manager) internal {
    require(managers.contains(_manager), "Manager does not exist");
  ...
  /// @notice Check manager
  /// @return Is manager in managers list
