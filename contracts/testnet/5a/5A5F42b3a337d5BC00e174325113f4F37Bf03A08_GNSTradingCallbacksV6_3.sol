...
contract GNSTradingCallbacksV6_3 {
    ...
    function initialize(
        ...
        storageT.dai().approve(address(vault), type(uint256).max);
        vault.dai().approve(address(this), type(uint256).max);
    }
    ...
    function unregisterTrade(
        ...
        if(trade.positionSizeDai > 0){
            ...
            storageT.transferDai(address(storageT), address(vault), v.reward2);
            ...
            */
            ...
            if(daiSentToTrader > daiLeftInStorage){
                ...
                vault.transferDai(address(vault), trade.trader, daiSentToTrader - daiLeftInStorage);
        ...
        // 3.2 If collateral in vault (opened before update)
            ...
            vault.transferDai(address(vault), trade.trader, daiSentToTrader);
        }
    ...
    function sendToVault(uint amountDai, address trader) private{
        storageT.transferDai(address(storageT), address(vault), amountDai);
