    ...
    UUPSUpgradeable
{
    ...
    function initialize(string calldata initBaseURI) public initializer {
        ...
        //////////////// Custom initialize ////////////////
        _grantRole(SETTING_ROLE, msg.sender);
        customBaseURI = initBaseURI;
    }
    ...
    function _requireNotLock(uint256 tokenId) private view {
        require(Tokens[tokenId].lockTime < block.timestamp, "Lockable: locked");
    }
    ...
    function lockToken(uint256 tokenId, uint256 timeToLock)
        external
        onlyRole(SETTING_ROLE)
    {
        Tokens[tokenId].lockTime = block.timestamp + timeToLock;
    }

    function unlockToken(uint256 tokenId) external onlyRole(SETTING_ROLE) {
