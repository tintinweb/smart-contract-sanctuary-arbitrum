...
abstract contract SwapV3 is Dev, Fee, Common {
    ...
    function _transferFromSwap(
        ...
        uint256 buyTax = _calcPercent(
            amount,
            buyLiquidityFee + buyJackpotFee + buyDevFee
        );
        uint256 sellTax = _calcPercent(
            amount,
            sellBlackFee +
                sellLiquidityFee +
                sellJackpotFee +
        ...
        uint256 usd = _getAmountOutPrice(user, actualAmount);
        _collectTax(user, from, source, buyTax + sellTax);
        buyTotalTax += buyTax;
        sellTotalTax += sellTax;
        if (source == 1) {
            if (luckyAddress != address(0)) {
                try ILuckyPool(luckyAddress).trade(user, usd) {} catch {}
            }
        }
