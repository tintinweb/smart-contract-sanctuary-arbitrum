...
contract ParticleMinter is ManagerModifier, ReentrancyGuard, Pausable {
  ...
  // TODO: Need to test new requires
  ...
  function init(
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      (, bool isRealmSet) = PARTICLE_TRACKER.currentRealm(addr, adventurerId);

      // Check current realm has not been set
      require(!isRealmSet, "ParticleMinter: You are already exploring");
      ...
      // Add explorer
      PARTICLE_TRACKER.addExplorer(realmId, addr, adventurerId);
  ...
  function capture(
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      // Capture
      uint256 currentRealm = _capture(addr, adventurerId);

      // Check if realm has enough population
      _checkPopulation(realmId);

      // Remove explorer
      PARTICLE_TRACKER.removeExplorer(currentRealm, addr, adventurerId);
  ...
  // Internal
  //=======================================
  function _capture(address _addr, uint256 _adventurerId)
    internal
    returns (uint256)
  {
    // Check sender owns adventurer
    ...
    // Get current realm exploring
    (uint256 currentRealm, bool isRealmSet) = PARTICLE_TRACKER.currentRealm(
      _addr,
      _adventurerId
    );
    ...
    PARTICLE.mintFor(_realmOwner(currentRealm), realmerParticles);
  ...
  function _totalParticlesForAdventurer(address _addr, uint256 _adventurerId)
    ...
    returns (uint256)
  {
    uint256 particles = adventurerBaseParticle;

    // Retrieve adventurer data
    uint256[] memory properties = _getAdventurerData(_addr, _adventurerId);
  ...
  function _checkAddress(address _addr, bytes32[] calldata proof)
    internal
    view
  {
    // Verify address
    ...
    require(isValidLeaf, "ParticleMinter: Address not supported");
  }
}
...
interface IData {
  function data(uint256 _realmId, uint256 _type)
    external
    view
    returns (uint256);
