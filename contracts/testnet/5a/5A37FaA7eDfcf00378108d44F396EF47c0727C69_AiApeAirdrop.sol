...
contract AiApeAirdrop is Ownable, ReentrancyGuard {
    address public defaultInvestmentToken;
    uint256 public startTime = block.timestamp + 10;
    uint256 public endTime = block.timestamp + 3600 * 24 * 7;
    uint256 public airdropAmount =
        3000000000000;
    uint256 public airdropRecord = 60000000000;
    uint256 public airdropNftAmount =
        1869000000000;
    uint256 public totalAirdorps =
        21_000_000_000_000_000;
    uint256 public totalAirdorpsFromNFT =
        186_900_000_000_000_000;
    address[] public airdropAddress;
    uint256 public maxQualifiedAddresses = 50000;
    address public nftAddress;
    uint256 public airdropRecordsAll;
    uint256 public airdropRecordsFormNFTAll;
    mapping(uint => bool) public exists1;
    mapping(address => address) public referrers;
    mapping(address => address[]) public inviteRecords;
    mapping(address => uint256) public inviteNumRecords;
    ...
    function getInviteNum(address myAddress) external view returns (uint256) {
        return inviteNumRecords[myAddress];
    }

    function claimReferralRewards() external nonReentrant {
        uint256 token = referralRewards[msg.sender];
        uint256 tokenAmount = token * 1e18;
        require(token > 0, "reward is zero");
        require(
            IERC20(defaultInvestmentToken).transfer(msg.sender, tokenAmount),
            "Transfer failed"
        );
    ...
    function getAirdropNumber(
        address referrer
    ) public view returns (uint256 number) {
        if (referrer != msg.sender && msg.sender.balance > 0 && airdropRecords[msg.sender] == 0) {
            number = airdropAmount + airdropRecord;
        } else if (msg.sender.balance > 0 && airdropRecords[msg.sender] == 0) {
    ...
    function getAirdrop(address referrer) external nonReentrant {
        ...
        uint256 _airdropRecordsAll;
        if ((referrer != address(0) && referrer != msg.sender)) {
            inviteRecords[referrer].push(msg.sender);
            referrers[msg.sender] = referrer;
            inviteNumRecords[referrer]++;
            referralRewards[referrer] += airdropRecord;
            totalReferralRewards[referrer] += airdropRecord;
            airdropAmountOne += airdropRecord;
            _airdropRecordsAll += airdropRecord;
        }
        _airdropRecordsAll += airdropAmountOne;
        airdropRecordsAll += _airdropRecordsAll;
        airdropRecords[msg.sender] = airdropAmountOne;
        airdropAddress.push(msg.sender);
        maxQualifiedAddresses -= 1;
        uint256 tokenAmount = airdropAmountOne * 1e18;
        require(
            IERC20(defaultInvestmentToken).transfer(
                msg.sender,
                tokenAmount
    ...
    function getAirdropByNft(uint256[] memory tokenIds) external nonReentrant {
        uint256 airdropCountBefore = nftReceivedTimes[msg.sender];
        require(airdropCountBefore <= 10, "No more than 10");
        require(tokenIds.length > 0,"No have NFT");
        uint256[] storage nftOwenList = nftOwenMap[msg.sender];
        uint256 amount = 0;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (amount + airdropCountBefore >= 10) {
                break;
            }
            if (
                !exists1[tokenIds[i]] && getNftOwner(tokenIds[i]) == msg.sender
            ) {
                nftOwenList.push(tokenIds[i]);
                exists1[tokenIds[i]] = true;
                amount++;
            }
        }
        nftReceivedTimes[msg.sender] += amount;
        uint256 airdropAmountItem = airdropNftAmount * amount;
        uint256 tokenAmount = airdropAmountItem * 1e18;
        require(
            IERC20(defaultInvestmentToken).transfer(
                msg.sender,
                tokenAmount
        ...
        airdropRecordsFormNFTAll += airdropAmountItem;
