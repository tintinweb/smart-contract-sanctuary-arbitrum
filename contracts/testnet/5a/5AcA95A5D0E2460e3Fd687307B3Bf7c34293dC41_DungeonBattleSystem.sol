...
import {AFFINITY_PRECISION_FACTOR, AffinitySystem, ID as AFFINITY_SYSTEM_ID} from "../affinity/AffinitySystem.sol";
import {CoreMoveSystem, ID as CORE_MOVE_SYSTEM_ID} from "../combat/CoreMoveSystem.sol";
import {DAMAGE_TRAIT_ID, ELEMENTAL_AFFINITY_TRAIT_ID} from "../Constants.sol";
...
struct ValidateVersusResultParams {
    ...
    IEquippable attackerEquipmentSystem;
 ...
 */
library BattleValidationLibrary {
    /**
    ...
    function canBeAttacked(
        ...
        if (
            !ITokenTemplateSystem(_getSystem(TOKEN_TEMPLATE_SYSTEM_ID))
                .hasTrait(nftContract, nftTokenId, IS_MOB_TRAIT_ID)
        ) {
...
import {IS_MOB_TRAIT_ID, CURRENT_HEALTH_TRAIT_ID, HEALTH_TRAIT_ID, MINTER_ROLE} from "../Constants.sol";
import {GameRegistryConsumerUpgradeable} from "../GameRegistryConsumerUpgradeable.sol";
import {ITokenTemplateSystem, ID as TOKEN_TEMPLATE_SYSTEM_ID} from "../tokens/ITokenTemplateSystem.sol";
...
contract MobSpawn is GameRegistryConsumerUpgradeable {
     ...
     */
    function initializeMob(uint256 templateId) external onlyRole(MINTER_ROLE) {
        ITokenTemplateSystem tokenTemplateSystem = ITokenTemplateSystem(
            _getSystem(TOKEN_TEMPLATE_SYSTEM_ID)
        );
...
import {BattleValidationLibrary, ValidateVersusResultParams} from "../combat/BattleValidationLibrary.sol";
...
import {EndBattleParams, IDungeonBattleSystem, ID} from "./IDungeonBattleSystem.sol";
import {DungeonNode, IDungeonMapSystem, ID as DUNGEON_MAP_SYSTEM_ID} from "./IDungeonMapSystem.sol";
import {StartDungeonBattleParams} from "./IDungeonSystem.sol";
import {DungeonTrigger, DungeonTriggerSystem, ID as DUNGEON_TRIGGER_SYSTEM_ID} from "./DungeonTriggerSystem.sol";
...
uint256 constant DUNGEON_BATTLE_MAX_MOVE_COUNT = uint256(
    keccak256("dungeon_battle.max_move_count")
);

struct EndBattleState {
    uint256 shipStartingHealth;
    uint256 mobStartingHealth;
    VersusResultType resultType;
}

/**
 * @title DungeonBattleSystem
 */
contract DungeonBattleSystem is CoreBattleSystem, IDungeonBattleSystem {
    /** MEMBERS */
    ...
    /// @notice Invalid success param provided
    error InvalidSuccessParam(bool success, VersusResultType resultType);
    ...
    /**
     * @inheritdoc IDungeonBattleSystem
     */
    function startBattle(
        ...
        // Get dungeon node
        DungeonTrigger memory trigger = DungeonTriggerSystem(
            _getSystem(DUNGEON_TRIGGER_SYSTEM_ID)
        ).getDungeonTrigger(params.dungeonTriggerEntity);

        DungeonNode memory node = IDungeonMapSystem(
            _getSystem(DUNGEON_MAP_SYSTEM_ID)
        ).getDungeonNode(trigger.dungeonMapEntity, params.node);
        ...
        if (!mobCombatable.canBeAttacked(node.enemies[0], new uint256[](0))) {
            revert InvalidEntity();
        }
    ...
    function endBattle(
        ...
        EndBattleState memory state;

        // Get ship + mob starting health
        state.shipStartingHealth = battle.attackerCombatable.getCurrentHealth(
            battle.attackerEntity,
            traitsProvider
        );
        state.mobStartingHealth = battle.defenderCombatable.getCurrentHealth(
            battle.defenderEntity,
            traitsProvider
        );

        // Determine result type
        if (params.totalDamageDealt >= state.mobStartingHealth) {
            state.resultType = VersusResultType.ATTACKER_WIN;
        } else if (params.totalDamageTaken >= state.shipStartingHealth) {
            state.resultType = VersusResultType.DEFENDER_WIN;
        } else {
            state.resultType = VersusResultType.DRAW;
        }

        // Make battle validation checks
        _validateEndBattle(params, state, battle, traitsProvider);

        // For now do not emit results, we should store in mapping instead
        // BattleEvents(_getSystem(BATTLE_EVENTS_ID)).emitVersusBattleResult(
        //     params.account,
        //     battle.attackerEntity,
        //     battle.defenderEntity,
        //     params.battleEntity,
        //     state.shipStartingHealth, // Damage does not persist yet
        //     state.mobStartingHealth, // Damage does not persist for mobs
    ...
    function _validateEndBattle(
        ...
        if (
            ...
            revert InvalidSuccessParam(params.success, state.resultType);
        }
        if (
        ...
        if (
            !BattleValidationLibrary.validateVersusResult(
...
import {ITraitsConsumer, TraitsConsumer, GameRegistryConsumer} from "../../TraitsConsumer.sol";
