...
contract XMozStaking is Ownable {
    ...
    mapping(address => mapping(address => uint256)) public accRewardAmounts;
    ...
    function setFee(uint256 _treasuryFeeBP) external onlyOwner {
        require(_treasuryFeeBP <= MAX_FEE, "StargatePlugin: fees > limit");
    ...
    function claimReward() external {
        // StakingInfo storage user = stakingInfo[msg.sender];
        require(stakingInfo[msg.sender] > 0, "XMozStaking: Users must stake XMoz to claim rewards");
        synchronizeXMozBalance(msg.sender);
        update();
        accumulateReward();
        distribiteReward();
    ...
    function accumulateReward() internal {
        // StakingInfo memory user = stakingInfo[msg.sender]; 
        for(uint256 i = 0; i < rewardTokens.length; i++) {
            uint256 rewardAmount = stakingInfo[msg.sender].mul(accUnitPerShare[rewardTokens[i]]).div(1e30).sub(rewardDebts[msg.sender][rewardTokens[i]]);
            accRewardAmounts[msg.sender][rewardTokens[i]] += rewardAmount;
        }
    }

    function distribiteReward() internal {
        for(uint256 i = 0; i < rewardTokens.length; i++) {
            safeRewardTransfer(rewardTokens[i], msg.sender, accRewardAmounts[msg.sender][rewardTokens[i]]);
            accRewardAmounts[msg.sender][rewardTokens[i]] = 0;
        }
    }
    ...
    function update() internal {
        uint256 duration = (block.timestamp - lastUpdateTime) / (60 * 3);
        uint256 diff = (block.timestamp - lastUpdateTime) % (60 * 3);

        uint256 supply = totalStakedAmount;
        if(supply == 0) {
            lastUpdateTime = block.timestamp - diff;
            return;
        }
        for(uint256 i = 0; i < rewardTokens.length; i++) {            
            accUnitPerShare[rewardTokens[i]] = accUnitPerShare[rewardTokens[i]].add(duration.mul(rewardAmountsPerWeek[rewardTokens[i]]).mul(1e30).div(supply));
        }
        lastUpdateTime = block.timestamp - diff;
    }
    ...
    function initialize(uint256 _startTime) internal {
        uint256 diff1 = _startTime % (60 * 3);
        uint256 diff2 = block.timestamp % (60 * 3);
