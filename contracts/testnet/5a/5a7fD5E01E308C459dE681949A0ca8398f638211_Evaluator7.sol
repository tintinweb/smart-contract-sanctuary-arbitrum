...
contract Evaluator7 {
    ...
    function handRank(uint a, uint b, uint c, uint d, uint e, uint f, uint g) public view returns (uint8) {
        uint val = evaluateV1(a,b,c,d,e,f,g);
    ...
    function evaluate(uint8[] calldata cards) public view returns (uint) {
        require(cards.length == 7, "invalid cards length");
        return evaluateV1(cards[0], cards[1], cards[2], cards[3], cards[4], cards[5], cards[6]);
    }

    function evaluateV1(uint a, uint b, uint c , uint d, uint e, uint f, uint g) public view returns (uint) {
        uint suit_hash = 0;
