...
contract GmxPlugin is Ownable, IPlugin, ReentrancyGuardUpgradeable {
    ...
    function setRouterConfig(
        ...
        require(_exchangeRouter != address(0) && _router != address(0) && _depositVault != address(0) && _withdrawVault != address(0) && _orderVault != address(0) && _reader != address(0), "GMX: Invalid Address");
    ...
    function addPool(uint256 _poolId,  address _indexToken, address _longToken, address _shortToken, address _marketToken) external onlyOwner {
    ...
    function removePool(uint256 _poolId) external onlyOwner {
        ...
        uint256 indexToRemove = findPoolIndexById(_poolId);
    ...
    function findPoolIndexById(uint256 _poolId) public view returns (uint256 poolIndex) {
    ...
    function stake(bytes calldata _payload) internal {
        ...
        uint256 index = findPoolIndexById(_poolId);
    ...
    function buyGMToken(uint8 _poolId , uint256 _longTokenAmount, uint256 _shortTokenAmount) internal {
        ...
        PoolConfig memory pool = pools[findPoolIndexById(_poolId)];
        ...
        address[] memory longTokenSwapPath; // Define swap paths if needed
    ...
    function getMarketTokenPrice(uint256 _poolId, bool _maximize) public view returns (int256) {
        require(poolExistsMap[_poolId], "GMX: Pool with this poolId does not exist");
        // Retrieve pool configuration
        PoolConfig memory _pool = pools[findPoolIndexById(_poolId)];
