    ...
    ReentrancyGuardUpgradeable
{
    ...
    function divest(address asset, bytes memory _info)
        external onlyManager whenNotPaused nonReentrant
    {
        IERC20Upgradeable assetContract = IERC20Upgradeable(asset);
        uint256 allowanceAmount = assetContract.allowance(operator, address(this));
        if(allowanceAmount == 0) revert ZeroAmount();
        assetContract.transferFrom(operator, address(this), allowanceAmount);

        emit Divested(address(this), msg.sender, asset, allowanceAmount, _info);
    }

    function liquidate(uint256 _orderId)
        external whenNotPaused nonReentrant
    {
        ...
        if(order.isOpenning) {
            order.isOpenning = false;

            emit Liquidated(address(this), msg.sender, _orderId);
        }
    }
