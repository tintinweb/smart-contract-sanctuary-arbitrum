...
contract PYESwapRouter is SupportingSwap {
    ...
    function initialize(address _WETH, address[] calldata _stables) external {
        require(msg.sender == initializer && WETH == address(0), "Invalid Initialize call");
...
import { PYESwapLibrary, IPYESwapPair, IPYESwapFactory } from "./libraries/PYESwapLibrary.sol";
...
abstract contract SupportingSwap is FeeStore, IPYESwapRouter {
    ...
    returns (uint[] memory amounts)
    {
        ...
        if(path[0] == pairFeeAddress[pair]){
            ...
            if(address(this) != adminFeeAddress){
                // IWETH(WETH).deposit{value: adminFeeDeduct}();
    ...
    function setStableToken(address _token, bool _flag) external {
        require(msg.sender == adminFeeSetter, "PYESwap: FORBIDDEN");
...
library PYESwapLibrary {
    ...
    // address private constant factory = 0x04a80eB6a3eb1e8E0FAf8CA1564659168c45CAD8;
    ...
    function getAmountsOut(
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            uint lpFee = IFeeStore(address(this)).pairFeeAddress(address(pair)) == path[i] ? 17 : 0;
    ...
    function pairFor(address tokenA, address tokenB) internal view returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                hex'bcbf7d100873b74be5f124cf0a03553c05da70c838207bc0f9c39e56be394c1e' // init code hash
            )))));
    }
