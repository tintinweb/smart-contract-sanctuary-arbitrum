...
library Address {
     ...
     * like Gnosis Safe, and does not provide security since it can be cirWagmivented by calling from a contract
     * constructor.
     * ====
     */
...
// File: contracts/interfaces/IWagmieBonusPool.sol


pragma solidity >=0.5.0;

interface IWagmieBonusPool {
...
// File: contracts/token/AIWagmie.sol


pragma solidity =0.8.18;

contract AIW is ERC20, Ownable {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;

    event SwapBack(uint256 burn, uint256 liquidity, uint256 dev, uint timestamp);
    ...
    uint256 public bonusFeeSell = 100;
    ...
    constructor(
        ...
        address _swapRouter,
        address _weth
    ) ERC20("AI WAGMI", "WAGMI") {
        uint256 _totalSupply = 120_000_000_000_000 * 1e6;
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        return _wagmiTransfer(_msgSender(), to, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        address spender = _msgSender();
        _spendAllowance(sender, spender, amount);
        return _wagmiTransfer(sender, recipient, amount);
    }

    function _wagmiTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (inSwap) {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountWagmiBurn = (taxAmount * burnFee) / (totalFee);
        uint256 amountWagmiLp = (taxAmount * liquidityFee) / (totalFee);
        uint256 amountWagmiBonus = (taxAmount * bonusFee) / (totalFee);
        taxAmount -= amountWagmiBurn;
        taxAmount -= amountWagmiLp;
        taxAmount -= amountWagmiBonus;
        ...
        _transfer(address(this), DEAD, amountWagmiBurn);
        _approve(address(this), address(bonusWallet), amountWagmiBonus);
        IWagmieBonusPool(bonusWallet).injectRewards(amountWagmiBonus);
        ...
        emit SwapBack(amountWagmiBurn, amountWagmiLp, amountWagmiBonus, block.timestamp);
    }
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'AIWagmiE: ETH_TRANSFER_FAILED');
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "AIWagmiE: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "AIWagmiE: index out of bounds");
