...
import "../characters/interfaces/ICitizen.sol";
import "../mercenaries/interfaces/IAddToAllowList.sol";
import "../packs/interfaces/IAddToPacksAllowList.sol";
import "../utils/interfaces/ILandDistributor.sol";
import "../utils/Permissioned.sol";

/// @dev Farmland - Starter Kit Smart Contract
contract StarterKit is ReentrancyGuard, ERC721Holder, Permissioned {
...
// STATE VARIABLES
    ...
    /// @dev This is the starterKitPrice of then starter kit
    uint256 public starterKitPrice = 0.1 ether;

    /// @dev This is the starterKitPrice of then starter kit
    uint256 public citizenPrice = 0.05 ether;

    /// @dev This is the pack to include in the starter kit
    uint256 public packId = 0;

    /// @dev This is the amount of Land to include in the starter kit
    uint256 public amountOfLand = 5000 ether;

// EVENTS

    event StartKitMinted(address indexed account, uint256 starterKitPrice, uint256 amountOfLand, uint256 packId);
...
// FUNCTIONS
    ...
    /// @dev Mint a Starter Kit
    function mintStarterKit(uint256 amount)
        external
        payable
        nonReentrant
    {
        // Check the correct ETH is sent
        uint256 totalPrice = starterKitPrice * amount;
        require( msg.value >= totalPrice,"Starter Kit: ETH sent is not correct" );
        // Write an event
        emit StartKitMinted(_msgSender(), totalPrice, amountOfLand, packId);
        for(uint256 i=0; i < amount;){
            // Add to the packs & mercenary allowlists
            packs.addToAllowlist(singletonUint256(packId), singletonAddress(_msgSender()), singletonUint256(1));
            mintMercenaries.addToAllowlist(singletonAddress(_msgSender()));
            // Mint a Citizen & send to new owner
            citizens.mintCollectible{value: citizenPrice}(1);
            citizens.safeTransferFrom(address(this), _msgSender(), citizens.totalSupply()-1);
            unchecked { ++i; }
        }
        // Send the Land
        landDistributor.issueLand(_msgSender(), amountOfLand * amount);
    }
    ...
    /// @dev Allow the owner to change the starter kit price
    /// @param newPrice new Starter Kit Price in ETH
    function setStarterKitPrice(uint256 newPrice) 
        external
        onlyOwner
    {
        starterKitPrice = newPrice;
        emit SetPrice(_msgSender(), newPrice, "Starter Kit Price");
    }

    /// @dev Allow the owner to change the Citizen price
    /// @param newPrice new Citizen in ETH
    function setCitizenPrice(uint256 newPrice) 
        external
        onlyOwner
    {
        citizenPrice = newPrice;
        emit SetPrice(_msgSender(), newPrice, "Citizen Price");
    }

    /// @dev Allow the owner to change the amount of Land in a starter kit
    /// @param newLandAmount new starterKitPrice in ETH
    function setLandAmount(uint256 newLandAmount) 
        external
        onlyOwner
    {
        amountOfLand = newLandAmount;
        emit SetLandAmount(_msgSender(), newLandAmount);
    }

    /// @dev Allow the owner to change the packId for the starter kit
    /// @param newPackId new pack Id
    function setPackId(uint256 newPackId)
        external
        onlyOwner
    {
        packId = newPackId;
        emit SetPackId(_msgSender(), newPackId);
    }
        ...
        onlyOwner
    {
        ...
        require(sent, "Starer Kit: Failed to send Ether");
    }
