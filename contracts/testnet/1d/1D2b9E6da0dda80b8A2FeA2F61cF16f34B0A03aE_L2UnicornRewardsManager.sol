...
contract L2UnicornRewardsManager is BaseL2Unicorn, BaseL2UnicornRule, RewardsManager {

    constructor(address l2Unicorn_, address l2UnicornRule_, address rewardsToken_)
    ...
    */
    function resolveL2Unicorn(uint256[] calldata tokenIdArr) external {
        ...
        for (uint256 i_ = 0; i_ < length;) {
            require(_msgSender() == IERC721(l2Unicorn).ownerOf(tokenIdArr[i_]), "L2Unicorn: incorrect owner");
            IL2UnicornRule.HatchRule memory hatchRule = IL2UnicornRule(l2UnicornRule).getHatchRuleByTokenId(tokenIdArr[i_]);
            if (hatchRule.awardAmount > 0) {
                totalAwardAmount += hatchRule.awardAmount;
            }
            unchecked{++i_;}
        }
