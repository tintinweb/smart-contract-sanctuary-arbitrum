...
import "../Realm/IRealmActivityTracker.sol";
...
contract QuestManager is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  IRealmActivityTracker public immutable TRACKER;
  ...
  constructor(
    ...
    TRACKER = IRealmActivityTracker(_tracker);
    VAULT = _vault;

    quests = IQuest(_questsAddress);

    particleCost = _particleCost;
  ...
  function quest(
    ...
    // TODO: Should we check for population here as well?
    // Gas is already at 334k...
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      require(realmIds.length > 0, "QuestManager: No realms provided");

      // Multiply particle cost by quest count
      uint256 totalParticleCost = particleCost * questCount;
      ...
      // Set quest tracker
      QUEST_TIMER.set(addr, adventurerId, questCount, 24);

      // Transfer Particle to vault
      PARTICLE.safeTransferFrom(msg.sender, VAULT, totalParticleCost);

      // Go on quests
      _quests(addr, adventurerId, questIds, realmIds);

      // TODO: this use to have total xp and anima, do we need it?
      emit QuestedTotal(addr, adventurerId, questCount, totalParticleCost);
    }
  }
  ...
  function updateParticleCost(uint256 _particleCost) external onlyAdmin {
    particleCost = _particleCost;
  }
  ...
  function _quests(
    ...
    for (uint256 j = 0; j < _questIds.length; j++) {
      ...
      (uint256 a, uint256 b) = _quest(_addr, _adventurerId, questId, realmId);
  ...
  function _quest(
    ...
    // Go on quest
    (uint256 xp, uint256 traitId, uint256 traitAmount, uint256 anima) = quests
      .go(_addr, _adventurerId, _questId, _realmId);

    // Add to trait
    ADVENTURER_DATA.addToBase(_addr, _adventurerId, traitId, traitAmount);

    // Track Quest activity
    TRACKER.track(_realmId, 0, 1);
...
interface IRealmActivityTracker {
  function track(
    uint256 _realmId,
    uint256 _prop,
    uint256 _val
