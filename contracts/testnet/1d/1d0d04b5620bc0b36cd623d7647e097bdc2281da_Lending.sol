...
contract Lending {
    ...
    event Withdraw(address user, uint256 amount, address token, uint256 price, PythStructs.Price response);
    ...
    function withdraw(
        ...
        uint256 price = uint256(uint64(response.price)) * (10 ** uint256(18 - (4294967295 + 1 - uint32(response.expo))));

        emit Withdraw(msg.sender, _amount, address(manager.underlying()), price, response);
