...
import { ISmartChef } from "./interfaces/ISmartChef.sol";
...
contract SwapFeeHandler is Ownable, AutomateTaskCreator {
    ...
    event BurnTokenUpdated(address _reward);
    ...
    function updateBurnToken(address _reward) external onlyOwner {
        buyBackToken = _reward;
        buyBackPath[1] = _reward;
        emit BurnTokenUpdated(_reward);
    }
    ...
    function canAutoDistributeRewards() external view returns (bool canExec, bytes memory execPayload) {
        uint256 rewardBalance = address(this).balance + weth.balanceOf(address(this)) - gasReserve;
        canExec = (
    ...
    function _distributeRewards() internal {
        ...
        if (stakingShare > 0) {
            if (isStakingContract) {
                if (distributeToVesting) {
                    if (ISmartChef(vestingDest).endTime() < block.timestamp) {
                        distributeToVesting = false;
                    }
                    uint256 _stakingShares = ISmartChef(stakingDest).totalShares();
                    uint256 _vestingShares = ISmartChef(vestingDest).totalShares();
                    ...
                    ISmartChef(stakingDest).addWETHDonation(_stakingShare);

                    weth.approve(vestingDest, _vestingShare);
                    ISmartChef(vestingDest).addWETHDonation(_vestingShare);
                } else {
                    weth.approve(stakingDest, stakingShare);
                    ISmartChef(stakingDest).addWETHDonation(stakingShare);
                }
            } else {
        ...
        if (buybackShare > 0) {
            ...
            if (!useDead) { ISwapToken(buyBackToken).burn(amounts[1]); }
