...
contract ReactorMinter is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  // External
  //=======================================
  function mint(uint256[] calldata _kinds, uint256[] calldata _quantities)
    external
    nonReentrant
    whenNotPaused
  {
    ...
    for (; j < _kinds.length; j++) {
      ...
      if (kind == 0) {
        startTokenId = FOOD_REACTOR.mintFor(msg.sender, quantity);
        addr = address(FOOD_REACTOR);
      } else if (kind == 1) {
        startTokenId = AQUATIC_REACTOR.mintFor(msg.sender, quantity);
        addr = address(AQUATIC_REACTOR);
      } else if (kind == 2) {
        startTokenId = TECH_REACTOR.mintFor(msg.sender, quantity);
        addr = address(TECH_REACTOR);
      } else if (kind == 3) {
        startTokenId = EARTHEN_REACTOR.mintFor(msg.sender, quantity);
        addr = address(EARTHEN_REACTOR);
      }

      emit Minted(startTokenId, address(addr), kind, quantity, cost, totalCost);
    }
  }
