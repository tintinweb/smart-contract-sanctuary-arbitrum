...
contract GFlyLPStaking is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, IGFlyLPStaking {
    ...
    function unlockBatchStakes(uint256[] memory stakeIds) external override whenNotPaused whenUnlockable {
        for (uint256 i = 0; i < stakeIds.length; i++) {
            require(stakeById[stakeIds[i]].owner == msg.sender, "GFlyLPStaking:NOT_STAKE_OWNER");
            require(isStakeUnlockable(stakeIds[i]), "GFlyLPStaking:STAKE_IS_NOT_UNLOCKABLE");
    ...
    function swapMagicPairToETHPair(
        ...
        if (stakeById[stakeId].autoIncreaseLock) {
            autoIncreasedForLocks[stakeById[stakeId].lock] -= originalAmount;
            autoIncreasedForLocksForAdditionalPairs[3][stakeById[stakeId].lock] += lpTokenAmount;
        } else {
            unlockableAtEpoch[stakeById[stakeId].unlockEpoch] -= originalAmount;
            unlockableAtEpochForAdditionalPairs[3][stakeById[stakeId].unlockEpoch] += lpTokenAmount;
        }
    ...
    function distributeEmissions() external override whenNotPaused {
        ...
        for (uint16 i = currentEpoch; i <= currentEpoch + 365; i++) {
            totalMiningPower += (((((unlockableAtEpoch[i] + autoIncreasedForLocks[i - currentEpoch]) *
    ...
    function _unlockStake(uint256 stakeId) internal {
        require(isStakeUnlockable(stakeId), "GFlyLPStaking:STAKE_IS_NOT_UNLOCKABLE");
        if (stakeById[stakeId].autoIncreaseLock) {
...
interface IGFlyLPStaking {
    ...
    event MagicPairSwappedForETHPair(
        address indexed account,
        uint256 indexed stakeId,
