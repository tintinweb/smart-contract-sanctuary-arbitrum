...
contract NeanderSmol is ContractControl, ERC721Enumerable {
    using Strings for uint256;
    using Counters for Counters.Counter;

    uint256 constant LAST_MALE = 2839;
    uint256 constant LAST_FEMALE = 5678;
    ...
    Counters.Counter public _maleTokenIdTracker;
    Counters.Counter public _femaleTokenIdTracker;

    string public baseURI;

    uint256 public decimals = 9;
    uint256 public commonSenseMaxLevel = 100 * (10**decimals);
    ...
    constructor() ERC721("Smol Age", "SmolAge") {
    ...
    function mintFemale(bytes32[] calldata _merkleProof) external {
        require(mintActive, "SmolNeander: Mint not active");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(_merkleProof, merkleRoot, leaf),
        ...
        require(minted[msg.sender] != true, "SmolNeander: Already minted");
        minted[msg.sender] = true;
        _mint(msg.sender, Gender.Female);
    }
    ...
    function _mint(address _to, Gender _gender) internal {
        ...
        if (_gender == Gender.Male) {
            ...
            require(
                _tokenId <= LAST_MALE,
                "SmolNeander: exceeded tokenId for male"
            );
        } else {
            _tokenId = _femaleTokenIdTracker.current();
            _femaleTokenIdTracker.increment();
            require(
                _tokenId <= LAST_FEMALE,
                "SmolNeander: exceeded tokenId for female"
            );
        }

        emit SmolNeanderMint(_to, _tokenId, _gender);
        _safeMint(_to, _tokenId);
    }

    function updateCommonSense(uint256 _tokenId, uint256 amount)
        external
        onlyStakingContract
    {
    ...
    /*
     * Staking Logic to prevent transfers when staked
     * Implementation courtesy of samurisenft
     */
    ...
    // function setStaked(uint256 _tokenId) external onlyStakingContract {
    //     tokenIdToStaked[_tokenId] = true;
    // }
    // function isStaked(uint256 _tokenId) public view returns (bool)  {
    //     return tokenIdToStaked[_tokenId];
    // }
...
abstract contract ContractControl is AccessControl {
    ...
    bytes32 public constant SMOLNEANDER_PAUSER_ROLE = keccak256("SMOLNEANDER_PAUSER_ROLE");
    ...
    modifier onlyPauser {
        require(isPauser(_msgSender()), "MinterControl: not a SMOLNEANDER_PAUSER_ROLE");
        _;
    }
    ...
    constructor() {
        ...
        _setRoleAdmin(SMOLNEANDER_PAUSER_ROLE, SMOLNEANDER_OWNER_ROLE);
        ...
        _setupRole(SMOLNEANDER_PAUSER_ROLE, _msgSender());
    }
    ...
    function grantPauser(address _pauser) external {
        grantRole(SMOLNEANDER_PAUSER_ROLE, _pauser);
    }

    function isPauser(address _pauser) public view returns (bool) {
        return hasRole(SMOLNEANDER_PAUSER_ROLE, _pauser);
    }
