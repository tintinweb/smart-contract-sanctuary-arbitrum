...
contract KaijuInitFacet is UsingDiamondOwner, WithStorage {
    ...
    function init(address spellcasterPayments) external onlyOwner {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        uint256 SINGLE_USD_PRICE = 25 * 10**5;
        uint256 HERO_USD_PRICE = 60 * 10**5;
        uint256 LEGENDARY_USD_PRICE = 100 * 10**5;
        // add check for is initialized?

        // maybe move to params
        _constants().contractUri = 'https://assets.kaijucards.io/metadata/pioneerKaiju/pioneerKaijuMetadata.json';
        _constants().baseUri = 'https://assets.kaijucards.io/metadata/pioneerKaiju/';
        _constants().NUM_TRADES_ALLOWED_AFTER_TRANSMOG = 8;

        //TODO Update
        _constants().spellcasterPayments = IPayments(spellcasterPayments);

        // _constants().minPriceByPaymentType[PaymentType.ETH_IN_USD] = 25 * 10**14; // 0.0025 ETH, $25 USD if ETH/USD == $10K
        // _constants().minPriceByPaymentType[PaymentType.MAGIC_IN_USD] = 25 * 10**16; // 0.25 MAGIC, $25 USD if MAGIC/USD == $100
        // _constants().minPriceByPaymentType[PaymentType.ARB_IN_USD] = 25 * 10**15; // 0.25 ARB, $25 USD if ARB/USD == $100

        _constants().minPriceByPaymentType[PaymentType.ETH_IN_USD] = 25 * 10**10; // TMP
        _constants().minPriceByPaymentType[PaymentType.MAGIC_IN_USD] = 25 * 10**10; // TMP
        _constants().minPriceByPaymentType[PaymentType.ARB_IN_USD] = 25 * 10**10; // TMP

        // TODO: Update to actual prices
        ...
        // _constants().packTypeToUsdPrice[LibToken.PioneerPackType.SINGLE] = 25 * 10**8;
        // _constants().packTypeToUsdPrice[LibToken.PioneerPackType.HERO] = 60 * 10**8;
        // _constants().packTypeToUsdPrice[LibToken.PioneerPackType.LEGENDARY] = 100 * 10**8;

        _constants().usdPriceToPackType[SINGLE_USD_PRICE] = LibToken.PioneerPackType.SINGLE;
        _constants().usdPriceToPackType[HERO_USD_PRICE] = LibToken.PioneerPackType.HERO;
        _constants().usdPriceToPackType[LEGENDARY_USD_PRICE] = LibToken.PioneerPackType.LEGENDARY;
        _constants().packTypeToUsdPrice[LibToken.PioneerPackType.SINGLE] = SINGLE_USD_PRICE;
        _constants().packTypeToUsdPrice[LibToken.PioneerPackType.HERO] = HERO_USD_PRICE;
        _constants().packTypeToUsdPrice[LibToken.PioneerPackType.LEGENDARY] = LEGENDARY_USD_PRICE;

        _shop().packQuantities[LibToken.PioneerPackType.NONE] = 0;
        _shop().packQuantities[LibToken.PioneerPackType.SINGLE] = 1;
        _shop().packQuantities[LibToken.PioneerPackType.HERO] = 3;
        _shop().packQuantities[LibToken.PioneerPackType.LEGENDARY] = 12;

        _token().pioneerTradingIsEnabled = false;

        //TODO add remaining interfaces and update supportsInterface function to take the data from here
...
struct TokensConstants {
    ...
    mapping (LibToken.PioneerPackType => uint256) packTypeToUsdPrice;
