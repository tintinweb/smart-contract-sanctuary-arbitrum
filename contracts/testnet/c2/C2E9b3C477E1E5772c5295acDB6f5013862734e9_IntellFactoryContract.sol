...
contract IntellFactoryContract is
    IFactory,
    Ownable
{
    ...
    constructor(
        ...
        address _intellTokenAddr_
    ) {
        _intellSetting = _intellSetting_;
        enablePaymentTokenAddrs[_intellTokenAddr_] = true;
    }

    function setEnablePaymentTokenAddrs(address _newPaymentToken, bool enable) external onlyOwner {
    ...
    function checkStatus(bytes calldata _data) internal view returns (bool) {
        
        (
            uint256 _CREATOR_NFT_TOKEN_ID,
            uint256 _MODEL_PROGRESS_STATUS,
            address _USER_ADDR,
            address _CREATOR_NFT_CONTRACT_ADDR,
            address _PAYMENT_TOKEN_ADDR,
            address _FACTORY_CONTRACT_ADDR,
            bool _VERIFIED_AS_CREATOR,
            bool _USER_SUSPENDED,
            bool _MODEL_UPLOADED,
            bool _HAS_SHARE,
            bool _MODEL_APPROVED
        ) = abi.decode(
                _data,
                (
                    uint256,
        ...
        require(
                ...
                _HAS_SHARE &&
                _MODEL_APPROVED &&
                _MODEL_PROGRESS_STATUS > 3 &&
                _CREATOR_NFT_TOKEN_ID > 0 &&
                _VERIFIED_AS_CREATOR &&
                address(this) == _FACTORY_CONTRACT_ADDR,
            "The launch status is invalid"
        );

        require(
            IERC721Enumerable(_intellSetting.creatorNFTContractAddr()).ownerOf(
                _CREATOR_NFT_TOKEN_ID
            ) == msg.sender,
            "THE CALLDER MUST BE HOLDER OF NFT TOKEN ID"
        );
        ...
        require(
            _CREATOR_NFT_CONTRACT_ADDR ==
                _intellSetting.creatorNFTContractAddr(),
            "NEXT INTELIGGENCE EXCHANGE: INVALID SHARE NFT ADDRESS"
        );
    ...
    function createShareNFTContract(
    ...
    ) external override returns (address investorNFT) {
        require(
            verifyMessage(_modelStatusMessasge, _modelStatusSignature) &&
                verifyMessage(_launchMessage, _launchSignature),
            "ONLY ACCEPT TRUTHHOLDER SIGNED MESSAGE"
        );

        require(
            checkStatus(_modelStatusMessasge),
            "THE CONDITION IS NOT ENOUGH TO LAUNCH"
        );

        uint256 _CREATOR_NFT_TOKEN_ID = abi.decode(
            _modelStatusMessasge,
            (uint256)
        );

        require(
            paymentToken().balanceOf(msg.sender) >= shareLaunchPrice(),
        ...
        bytes32 salt = keccak256(_launchMessage);
        assembly {
            investorNFT := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        IShareERC721(investorNFT).launch(
            _launchMessage,
            _intellSetting
        );
...
import "./ERC165.sol";
import "./IERC721.sol";
import "./IERC721ShareMetadata.sol";
...
contract ShareERC721 is Context, ERC165, IERC721, IERC721ShareMetadata {
        ...
        returns (bool)
    {
        return
            ...
            interfaceId == type(IERC721ShareMetadata).interfaceId ||
...
interface IERC721ShareMetadata is IERC721 {
    ...
    IShareERC721
{
    ...
    modifier onlyAdmin() {
        require(intellSetting.admin() == _msgSender(), "Ownable: caller is not the owner");
        _;
    }

    function owner() public view returns (address) {
        return
            IERC721Enumerable(CREATOR_NFT_CONTRACT_ADDR).ownerOf(
    ...
    function launch(bytes calldata _data, IIntellSetting _intellSetting_)
        external
        override
    {
        require(msg.sender == factory(), "The caller must be factory contract");
        ...
        (
            ...
            require(
                block.timestamp < _DEADLINE && _DEADLINE > 0,
                "PASSED THE DEADLINE"
            );
        ...
        CREATOR_NFT_CONTRACT_ADDR = _CREATOR_NFT_CONTRACT_ADDR;
    ...
    function adopt(bytes calldata message, bytes calldata signature) external nonReentrant {
        ...
        (
            ...
            bool _USER_SUSPENDED,
            bool _FROM_US,
            bool _MODEL_SUSPENDED,
            bool _MODEL_APPROVED,
            bool _STOP_SELLING_SHARE,
            uint256 _NUM,
            uint256 _CREATOR_NFT_TOKEN_ID
        ) = abi.decode(message, (address, address, bool, bool, bool, bool, bool, bool, uint256, uint256));

        require((FOR_ONLY_US_INVESTOR && _FROM_US && _KYC_VERIFICATION_AS_INVESTOR) || (!FOR_ONLY_US_INVESTOR && !_FROM_US), "KYC is required");

        require(!_USER_SUSPENDED 
            && !_FROM_US 
            && !_MODEL_SUSPENDED 
            && _MODEL_APPROVED 
            && !_STOP_SELLING_SHARE
            && _CREATOR_NFT_TOKEN_ID == CREATOR_NFT_TOKEN_ID 
            && _SHARE_NFT_ADDR == address(this), "THE USER PROFILE IS INVALID");
        ...
        require(
            tx.origin == msg.sender && msg.sender == _USER_ADDR,
            "NO BOT"
        );
        require(
    ...
    function withdraw(bytes calldata message, bytes calldata signature)
        public
        onlyOwner
    {
        ...
        (bool _canWithdraw, address _owner) = abi.decode(message, (bool, address));
