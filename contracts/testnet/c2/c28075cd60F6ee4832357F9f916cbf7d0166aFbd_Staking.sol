...
contract StakingStorage {
    ...
    string public constant name = 'Mayfair Staking';

    address public mayfairAddress;

    IERC20 public mayfair;
    ...
    bytes32 public constant DELEGATION_TYPEHASH =
        keccak256('Delegation(address delegatee,uint256 nonce,uint256 expiry,uint256 pid)');
...
contract Staking is StakingGov, Pausable, ReentrancyGuard, Ownable {
    ...
    function availableWithdraw(uint256 pid, address account) public view returns (uint256) {
        ...
        if (!withdrawable(pid, account)) {
            ...
            return
                ((user.amount + user.withdrawn) * (block.timestamp - user.depositTime - pool.lockPeriod)) /
                (pool.vestingPeriod) -
                user.withdrawn;
        }
    }
    ...
    function getReward(uint256 pid) public nonReentrant updateReward(pid, msg.sender) {
        ...
        if (reward > 0) {
            ...
            mayfair.safeTransfer(msg.sender, reward);
    ...
    function addPool(
        ...
        require(mayfairAddress != address(0), 'ERR_MAYFAIR_NOT_SET');

        // Pools that gives voting power must have mayfair as staking token
        // and must have a volting multiplier higher than the base 1
        if (_votingMultiplier != 0) {
            require(_stakingToken == mayfairAddress, 'ERR_NOT_VOTING_STAKING_TOKEN');
    ...
    function addReward(uint256 pid, uint256 reward) external onlyOwner updateReward(pid, address(0)) {
        ...
        mayfair.safeTransferFrom(msg.sender, address(this), reward);
    ...
    function setMayfair(address _mayfair) external onlyOwner {
        require(_mayfair != address(0), 'ERR_ZERO_ADDRESS');
        require(mayfairAddress == address(0), 'ERR_MAYFAIR_ALREADY_SET');
        bool returnValue = IERC20(_mayfair).transfer(msg.sender, 0);
        require(returnValue, 'ERR_NONCONFORMING_TOKEN');
        mayfair = IERC20(_mayfair);
        mayfairAddress = _mayfair;
    }
