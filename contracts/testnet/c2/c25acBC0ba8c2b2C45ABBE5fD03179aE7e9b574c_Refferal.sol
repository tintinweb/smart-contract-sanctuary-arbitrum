...
contract Refferal is Ownable {
    ...
    constructor(address root, address _treasury, IERC20 _feeToken) {
        _register(_msgSender(), '0cfdf7');
        _addTree(address(0), root);

        treasury = _treasury;
        feeToken = _feeToken;
    }

    function setTreasury(address _treasury, uint _fee, IERC20 _feeToken) external {
        treasury = _treasury;
        fee = _fee;
        feeToken = _feeToken;
    }
    ...
    function _addTree(address upline, address root) internal {
        ...
        if(nextTo[upline].child.length == 3) nextTo[root].level += 1;
    ...
    function register(address _refferrer, string memory code, address upline, address root) external {
        require(feeToken.transferFrom(_msgSender(), treasury, fee), "Refferal::register: fee fail");
