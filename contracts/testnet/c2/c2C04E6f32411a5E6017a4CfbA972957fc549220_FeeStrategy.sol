...
contract FeeStrategy is Access {
  /*==================================================== Events =============================================================*/
  ...
  //Config public config = Config(5_000_000_000_000_000, 1_000_000_000_000_000);
  Config public config = Config(5_000_000_000_000_000, 1_000_000_000_000_000);

  /// @notice Last calculated multipliers index id
  uint256 public lastCalculatedIndex = 0;
  /// @notice Start time of periods
  uint256 public periodStartTime = block.timestamp - 20 minutes;
  ...
  //  Last calculated multiplier in 1e4
  ...
  constructor(IVault _vault, address _admin) Access(_admin) {
  ...
  /**
   *
   * @dev Function to update the maximum and minimum multipliers in the contract's configuration.
   * @param config_ The configuration struct containing the new maximum and minimum multipliers.
   * @notice Only the contract's default admin role is allowed to call this function.
   * @notice The new maximum and minimum multipliers must not be equal to zero.
   * @notice The new minimum multiplier must be less than the new maximum multiplier.
   */
  function updateConfig(Config memory config_) public onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  /**
   *
   * @dev Function to set the address of the vault that will hold the funds managed by the contract.
   * @param _vault The address of the vault contract.
   * @notice Only the contract's default admin role is allowed to call this function.
   * @notice The new vault address must not be zero.
   */
  function setVault(IVault _vault) public onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  /**
   *
   * @dev Function to calculate the dollar value of a given amount of a whitelisted token in the vault.
   * @param _token The address of the whitelisted token on the vault.
   * @param _amount The amount of the token to calculate the dollar value for.
   * @return _dollarValue The calculated dollar value for the given amount of the token.
   */
  function computeDollarValue(
    ...
    _dollarValue = ((_amount * vault.getDollarValue(_token)) / 10 ** _decimals);
    _dollarValue = _dollarValue / 1e12;
  }

  /**
   *
   * @dev Internal function to read the total profit and loss of a whitelisted token in the vault.
   * @param _token The address of the whitelisted token.
   * @return _totalLoss The total loss for the given token in the vault.
   * @return _totalProfit The total profit for the given token in the vault.
   */
  function _getProfitLoss(
    address _token
  ) internal view returns (uint256 _totalLoss, uint256 _totalProfit) {
    (_totalLoss, _totalProfit) = vault.returnTotalInAndOut(_token);
  }

  /**
   *
   * @dev Internal function to calculate the dollar value of the total profit and loss of all whitelisted tokens in the vault.
   * @return _totalLoss The total loss in USD for all whitelisted tokens in the vault.
   * @return _totalProfit The total profit in USD for all whitelisted tokens in the vault.
   */
  function _computeProfitLoss() internal view returns (uint256 _totalLoss, uint256 _totalProfit) {
    uint256 _allWhitelistedTokensLength = vault.allWhitelistedTokensLength();

    for (uint256 i = 0; i < _allWhitelistedTokensLength; i++) {
      address _token = vault.allWhitelistedTokens(i);
      (uint256 _loss, uint256 _profit) = _getProfitLoss(_token);
      uint256 _lossInDollar = computeDollarValue(_token, _loss);
      uint256 _profitInDollar = computeDollarValue(_token, _profit);
      _totalLoss += _lossInDollar;
      _totalProfit += _profitInDollar;
    }
  }

  /**
   *
   * @dev Internal function to set the wager fee for a given day index in the periodReserves array and in the vault.
   * @param _index The day index.
   * @param _wagerFee The wager fee percentage in 1e18.
   */
  function _setWagerFee(uint256 _index, uint256 _wagerFee) internal {
    currentMultiplier = _wagerFee;
    _wagerFee = _wagerFee / 1e14;
    multiplier = _wagerFee * 1e14;
    periodReserves[_index].currentMultiplier = currentMultiplier;
   ...
   * @notice function to set period reserve with profit loss calculation
   */
  function setReserve(uint256 index) internal {
    ...
    if (_totalProfit >= _totalLoss) {
      amount_ = _totalProfit - _totalLoss;
    } else {
      amount_ = _totalLoss - _totalProfit;
    }
    ...
    if (index > 1) {
      PeriodReserve memory prevReserve_ = periodReserves[index - 1];

      _totalProfit - prevReserve_.profit;
    ...
    periodReserves[index] = PeriodReserve(amount_, _totalProfit, _totalLoss, changeType_, 0);
  }

  /**
   *
   * @param prevIndex prev period index
   * @param currentIndex current period index
   * @return prevPeriod_ prev period reserve
   * @return diffReserve_ difference of prev and current periods
   */
  function getDifference(
    uint256 prevIndex,
    uint256 currentIndex
  ) public view returns (PeriodReserve memory prevPeriod_, PeriodReserve memory diffReserve_) {
    ...
    if (prevPeriod_.totalAmount >= currentPeriod_.totalAmount) {
      diffReserve_.totalAmount = prevPeriod_.totalAmount - currentPeriod_.totalAmount;
    } else {
      diffReserve_.totalAmount = currentPeriod_.totalAmount - prevPeriod_.totalAmount;
    }
  ...
  /**
   * @notice Calculates the current multiplier based on the period reserve and change rate
  ...
  function getMultiplier() public returns (uint256) {
    ...
    // If the multiplier has already been calculated for this period, return it
    if (lastCalculatedIndex == index) {
      return currentMultiplier;
    }

    // Calculate the reserve for the current period
    setReserve(index);

    // If the current period is the first period, set the multiplier to the minimum multiplier
    if (index < 1) {
      currentMultiplier = config.minMultiplier;
    } else {
      // Get the reserve and difference between the current period and the previous period
      (PeriodReserve memory prevPeriodReserve_, PeriodReserve memory diffReserve_) = getDifference(
        index - 1,
        index
      );

      // Calculate the change rate between the previous period and the current period
      uint256 diff = diffReserve_.totalAmount;
      uint256 periodChangeRate;

      if (prevPeriodReserve_.totalAmount != 0 && diff != 0) {
        periodChangeRate = (diff * PRECISION) / prevPeriodReserve_.totalAmount;
      }

      // Calculate the new multiplier based on the change rate and change type
      if (diffReserve_.changeType == ReserveChangeType.LOSS) {
        // If the reserve decreased, decrease the multiplier
        uint256 decrease = (2 * (currentMultiplier * periodChangeRate)) / PRECISION;
        currentMultiplier = currentMultiplier > decrease
          ? currentMultiplier - decrease
          : config.minMultiplier;
      } else if (periodChangeRate != 0) {
        // If the reserve increased, increase the multiplier
        currentMultiplier = (currentMultiplier * (1e18 + periodChangeRate)) / PRECISION;
      }

      // Enforce the minimum and maximum multipliers
    ...
    // Set the wager fee based on the current multiplier
  ...
  function calculate(address _token, uint256 _amount) external returns (uint256 amount_) {
    ...
    amount_ = (_value * getMultiplier()) / PRECISION;
  }
}
