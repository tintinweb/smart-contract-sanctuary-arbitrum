...
contract CollectionContract is Ownable {
    ...
    mapping (address => CollectionInfo) socials; 
    ...
    function deployCollection(DeploymentParams memory dp, CollectionInfo memory colInfo) public {
        IFactory(factory).deployGumBall(dp.name, dp.symbol, dp.uris, dp.supplyBASE, dp.supplyGBT, dp.base, dp.artist, dp.delay, dp.bFee);
        (address gbt, , , ) = IFactory(factory).deployInfo(IFactory(factory).totalDeployed() - 1);
        _setCollectionInfo(colInfo, gbt);
    }

    function _setCollectionInfo(CollectionInfo memory colInfo, address gbtAddress) internal {
        socials[gbtAddress] = colInfo;
    }

    function setCollectionInfo(CollectionInfo memory colInfo, address gbtAddress) public {
        require(msg.sender == IGBT(gbtAddress).artist(), "!Authorized");

        socials[gbtAddress] = colInfo;
    }

    function readCollectionInfo(address gbtAddress) public view returns (CollectionInfo memory returnedInfo) {
        return socials[gbtAddress];
    }
    ...
    function collectionPage(address[] memory gbts) public view returns (CollectionPage[] memory _cp) {
        ...
        for (uint256 i = 0; i < gbts.length; i++) {
            ...
            CollectionInfo memory col = socials[gbts[i]];
            ...
            cp[i].collection.fee = IGNFT(nft).bFee();
    ...
    function individualCollection(address gbts) external view returns (CollectionPage memory _cp) {

        CollectionPage memory cp;

        (uint256 index, address gbt, address nft, address xgbt, bool allowed) = findCollectionByAddress(gbts);
        
        CollectionInfo memory col = socials[gbt];
        ...
        cp.collection.fee = IGNFT(nft).bFee();
...
struct DynamicCollectionData {
    uint256 currentPrice;
    uint256 mintableNFTLength;  // remaining nfts
    uint256 availableSupply; // remaining gbt
}
