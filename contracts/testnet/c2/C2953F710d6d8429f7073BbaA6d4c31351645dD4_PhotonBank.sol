...
// Photon Proxy Token
// Requires handler permissions on the underlying token for streamlined transfers
// Does not require mint/burn permissions on the underlying token
// For use on primary chain only

contract PhotonBank is ProxyOFTV2, Ownable2Step {
    using SafeERC20 for IERC20;
    ...
    function exit(address _newAddress) external onlyOwner {
        ...
        // reset outboundAmount
        outboundAmount = 0;
    }

    // withdraw a specific amount to a new address
    function withdraw(address _newAddress, uint256 _amount) external onlyOwner {
        innerToken.safeTransfer(_newAddress, _amount);
        // reset outboundAmount
        outboundAmount -= _amount;
    }
