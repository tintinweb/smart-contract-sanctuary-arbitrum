...
contract JackpotBankroll is Ownable {
   ...
   * @param betPrice betPrice of token address
   */
  event Bankroll_Token_State_Changed(address tokenAddress, bool isValid, uint256 betPrice);
  /**
  ...
  function getBetPrice(address tokenAddress) external view returns (uint256) {
    if (!isTokenAllowed[tokenAddress] || betPrice[tokenAddress] == 0) {
      revert InvalidTokenAddress();
    }
    return (betPrice[tokenAddress]);
  }

  /**
   * @dev function to get the current Jackpot Price
   */
  ...
  /**
   * @dev function to get the next Jackpot Price
   */
  function getNextJackpot() external view returns (uint256) {
    return address(this).balance / 4;
  }

  /**
   * @dev function to set if a given token can be wagered
   * @param _tokenAddress address of the token to set address
   * @param _isValid state to set the address to
   * @param _betPrice betPrice to set the address to
   */
  function setTokenAddress(address _tokenAddress, bool _isValid, uint256 _betPrice) external onlyOwner {
    isTokenAllowed[_tokenAddress] = _isValid;
    betPrice[_tokenAddress] = _betPrice;
    emit Bankroll_Token_State_Changed(_tokenAddress, _isValid, _betPrice);
  }
  ...
  function transferPayout(address player, uint256 payout, address tokenAddress) external {
    ...
    if (tokenAddress != address(0)) {
      ...
      if (!success) {
        (bool _success, ) = wrappedToken.call{value: payout}(abi.encodeWithSignature("deposit()"));
        if (!_success) {
          revert();
        }
  ...
  function suspend(uint256 suspensionTime) external {
    if (suspendedTime[msg.sender] > block.timestamp) {
      revert AlreadySuspended(suspendedTime[msg.sender]);
    }
    suspendedTime[msg.sender] = block.timestamp + suspensionTime;
    playerSuspended[msg.sender] = true;
  }
  ...
  function permantlyBan() external {
    suspendedTime[msg.sender] = 2 ** 256 - 1;
    playerSuspended[msg.sender] = true;
  }
  ...
  function liftSuspension() external {
    if (suspendedTime[msg.sender] > block.timestamp) {
      revert TimeRemaingOnSuspension(suspendedTime[msg.sender]);
    }
    playerSuspended[msg.sender] = false;
  }
  ...
  function isPlayerSuspended(address player) external view returns (bool, uint256) {
    return (playerSuspended[player], suspendedTime[player]);
  }
