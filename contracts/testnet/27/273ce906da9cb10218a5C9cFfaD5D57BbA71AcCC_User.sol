...
contract User is IUser, DAOAccessControlled {
    ...
    ****************************************************************************************/
    ...
    function friendList(address _walletAddress) public view returns(UserAttributes[] memory _friends) {
        require(userAttributes[_walletAddress].id != 0, "NON EXISTENT");

        for(uint256 i = 0; i < allUsers.length; i++) {
            if(areFriends[_walletAddress][allUsers[i]]) {
                _friends[i] = userAttributes[allUsers[i]];
            }
        }
    }
...
interface IUser {
    ...
    function friendList(address _walletAddress) external view returns(UserAttributes[] memory _friends);
