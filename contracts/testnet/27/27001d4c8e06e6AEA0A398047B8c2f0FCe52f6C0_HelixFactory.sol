...
contract HelixFactory is BaseUpgradeablePausable {
    ...
    event PoolCreated(IHelixTranchedPool indexed pool, address indexed borrower);
    event CreditLineCreated(address indexed creditLine);
    event FundCreated(IHelixFund indexed fund, address indexed issuer, uint256 dbPrjId);

    function initialize(address owner, IHelixConfig _config, uint256[] calldata _allowedUIDTypes)
        public
        initializer
    {
        require(
    ...
    function createBorrower(address owner) external returns (address) {
        ...
        IHelixBorrower borrower = IHelixBorrower(_borrower);
    ...
    /**
    * @notice Allows admin to create a new MultiTranchedPool for a single borrower
    * @dev We'll use array for packing some parameters to avoid call stack deep error
    * @dev _allowedUIDTypes should be get from SoulBound token (HelixUniqueIdentity)
    * @param _borrower The borrower for whom the CreditLine will be created
    * @param _loanFacade An array that use for project description (Project name - symbol)
    * @param _borrower The borrower for whom the CreditLine will be created
    * @param _uints _juniorFeePercent: The percent of senior interest allocated to junior investors, expressed as
    * integer percents. eg. 20% is simply 20. <br />
    * _interestApr The interest amount, on an annualized basis (APR, so non-compounding), expressed as an integer.
    *  We assume 18 digits of precision. For example, to submit 15.34%, you would pass up 153400000000000000,
    *  and 5.34% would be 53400000000000000. <br />
    * _lateFeeApr: The fee will be applied for late frequency interest repayment   
    * @param _days An array that used for defining days configuration. _paymentPeriodInDays How many days in each payment period. 
     `_paymentPeriodInDays` How many days in each payment period. <br />
     `_termInDays` Number of days in the credit term. It is used to set the `termEndTime` upon first drawdown. <br />
     `_principalGracePeriodInDays` Number of days for late principal payment before liquidation. <br />
     `_fundableAt` Open funding time which allows junior/senior investor to invest. <br />
     `_fundingDate` Close funding time which not allows investor to invest more.
    * @param _targets An array that use for defining maximum and minimum raising target, and the junior/senior ratio (expected) as well
    * @param _allowedUIDTypes An array that use for defining the list of allowed investor type 
    * Requirements:
    * You are the admin
    */
    function createMultiTranchesPool(
        // 
    ...
    ) external onlyAdmin returns (IHelixMultiTranchedPool) {
        IHelixMultiTranchedPool pool;
        ...
        // need to enclose in a scope to avoid overflowing stack
        {
            ...
            pool = IHelixMultiTranchedPool(address(poolProxy));
        }
   
        pool.initialize(
            _loanFacade,
            [address(config), _borrower, _interestModel],
            _uints,
            _days,  
            _targets,
            _allowedUIDTypes
        ...
        // TODO: Must be replace by Issuing ERC20 Pool shares
        // config.getJuniorLP().onPoolCreated(pool);
        return pool;
    }

    function createUniTranchesPool(
        // name - symbol
        string[2] calldata _loanFacade,
        address _borrower,
        address _interestModel,
        ...
        uint256[] calldata _allowedUIDTypes
    ) external onlyAdmin returns (IHelixUniTranchedPool) {
        IHelixUniTranchedPool pool;

        // need to enclose in a scope to avoid overflowing stack
        {
            ImplementationRepository repo = config.getUniTranchedPoolImplementationRepository();
            UcuProxy poolProxy = new UcuProxy(repo, _borrower);
            pool = IHelixUniTranchedPool(address(poolProxy));
        }
    ...
    function createFund(
        ...
        uint256[] calldata _allowedUIDTypes,
        uint256 dbPrjId
    ) external returns (IHelixFund) {
        require(!hasAllowedUID(_issuer), "not authorized");
        IHelixFund fund;
        // need to enclose in a scope to avoid overflowing stack
        {
            ImplementationRepository repo = config.getFundImplementationRepository();
            UcuProxy fundProxy = new UcuProxy(repo, _issuerProxy);
            fund = IHelixFund(address(fundProxy));
        }

        fund.initialize(
            [address(config), _issuerProxy, _issuer, token],
            _allowedUIDTypes
        );
        emit FundCreated(fund, _issuerProxy, dbPrjId);
        // TODO: Must be replace by Issuing ERC20 Pool shares
        // config.getJuniorLP().onPoolCreated(pool);
        return fund;
    }
    ...
    function setAllowedUIDTypes(uint256[] calldata _allowedUIDTypes) public onlyAdmin {
        allowedUIDTypes = _allowedUIDTypes;
    }
