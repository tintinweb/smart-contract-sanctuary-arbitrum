    ...
    AccessControl
{
    ...
    // We will use block.time instead of block.number
    struct Round {
        uint256 epoch;
    ...
    // @dev timeframeId => block number => round. Rounds that will be closed at certain block time.
    mapping(uint8 => mapping(uint256 => Round)) public roundsEndAt;

    /// @dev
    mapping(uint8 => uint256) public currentEpochs;
    mapping(uint8 => uint256) public currentLockedEpochs;
    mapping(uint8 => uint256) public currentClosedEpochs;
    ...
    event StartRound(uint8 indexed timeframeId, uint256 indexed epoch, uint256 startTime);
    event LockRound(
    ...
    modifier onlyAdminOrOperator() {
        require(isOperator(msg.sender) && isAdmin(msg.sender), "ONLY_ADMIN_OR_OPERATOR");
        _;
    }
    ...
    constructor(
        ...
        require(timeframes_.length != 0, "INVALID_ARRAY_LENGTH");
    ...
    */
    ...
    function _getPriceFromOracle() internal returns (uint256, uint256) {
        ...
        require(
            timestamp >= oracleLatestTimestamp,
    ...
    function _writeOraclePrice(uint256 timestamp, uint256 price) internal {
        if (oracle.isWritable()) {
            oracle.writePrice(timestamp, price);
        }
    }
    ...
    function genesisStartRound() external onlyOperator whenNotPaused {
        ...
        uint256 junk;
        if (block.timestamp % timeframes[0].interval > 0) {
            junk = timeframes[0].interval - block.timestamp % timeframes[0].interval;
        }

        genesisStartBlockTimestamp = block.timestamp + junk;
        for (uint256 i = 0; i < timeframes.length; i = i + 1) {
            _startRound(timeframes[i].id, 1);
            currentEpochs[timeframes[i].id] = 1;
        }
    ...
    function genesisLockRound(uint8 timeframeId) external onlyOperator whenNotPaused {
        ...
        (uint256 currentTimestamp, uint256 currentPrice) = _getPriceFromOracle();
        uint256 currentEpoch = getCurrentRoundId(timeframeId);

        if (currentEpoch == currentEpochs[timeframeId]) {
            currentEpoch += 1;
        }
        
        _safeLockRound(timeframeId, currentEpochs[timeframeId], currentTimestamp, currentPrice);
        _startRound(timeframeId, currentEpoch);
        genesisLockOnces[timeframeId] = true;
        
        currentLockedEpochs[timeframeId] = currentEpochs[timeframeId];
    ...
    function executeRound(
        ...
        for (uint8 i = 0; i < timeframeIds.length; i = i + 1) {
            ...
            if(genesisLockOnces[timeframeId]) {
                ...
                _safeLockRound(
                    timeframeId,
                    currentEpochs[timeframeId],
                    currentTimestamp,
                    currentPrice
                );
                _safeEndRound(
                    timeframeId,
                    currentLockedEpochs[timeframeId],
                    currentTimestamp,
                    currentPrice
                );
                ...
                currentClosedEpochs[timeframeId] = currentLockedEpochs[timeframeId];
                currentLockedEpochs[timeframeId] = currentEpochs[timeframeId];
                currentEpochs[timeframeId] = currentEpoch;
            }
    ...
    function _safeStartRound(uint8 timeframeId, uint256 epoch) internal {
        ...
        require(
            block.timestamp >= rounds[timeframeId][currentLockedEpochs[timeframeId]].closeBlockTime - bufferTime,
    ...
    function _startRound(uint8 timeframeId, uint256 epoch) internal {
        ...
        // TODO
        roundsEndAt[timeframeId][round.closeBlockTime] = round;
    ...
    function _safeLockRound(
        ...
        // We use block time
        require(
            rounds[timeframeId][epoch].startBlockTime != 0,
            "Can only lock round after round has started"
        );
        require(
            block.timestamp >= rounds[timeframeId][epoch].lockBlockTime - bufferTime,
    ...
    function _safeEndRound(
        ...
        // We use block time
        require(
            rounds[timeframeId][epoch].lockBlockTime != 0,
            "Can only end round after round has locked"
        );
        require(
            block.timestamp >= rounds[timeframeId][epoch].closeBlockTime - bufferTime,
    ...
    function _endRound(
        ...
        if (willDepositAmount > willClaimAmount) {
            vault.increaseDepositedAmount(willDepositAmount - willClaimAmount);
        }

        if (willDepositAmount < willClaimAmount) {
            vault.decreaseDepositedAmount(willClaimAmount - willDepositAmount);
        }
    ...
    */
    function getCurrentBettableAmount(uint8 timeframeId, uint256 epoch) public view returns (uint256) {
        ...
        if (rounds[timeframeId][epoch].startBlockTime != 0) {
            // Current epoch
            uint256 totalRisk;
            for (uint256 i; i < timeframes.length; i ++) {
                uint8 _timeframeId = timeframes[i].id;
                // TODO
                Round memory round = roundsEndAt[_timeframeId][rounds[timeframeId][epoch].closeBlockTime];
                uint256 _risk;
                if (round.bearAmount > round.bullAmount) {
                    _risk = round.bearAmount - round.bullAmount;
                } else {
                    _risk = round.bullAmount - round.bearAmount;
                }
                totalRisk += _risk;
            }

            if (totalRisk > maxMinuteExposure) {
                return 0;
            } else {
                return maxMinuteExposure - totalRisk;
            }
        } else {
            // Future betting
            ...
            for (uint256 i; i < timeframes.length; i ++) {
                // We use block time
                uint256 _epoch = getRoundIdAt(timeframes[i].id, timestamp);
                Round memory round = rounds[timeframeId][_epoch];
                uint256 _risk;
            ...
            if (totalRisk > maxMinuteExposure) {
                ...
                // Use 50% of total available allocation in case of future betting.
                return (maxMinuteExposure - totalRisk) / 2;
            }
        }
    }
    ...
    function openPosition(
        ...
        // Start round if round is not started.
        if (rounds[timeframeId][epoch].startBlockTime == 0) {
            _startRound(timeframeId, epoch);
        }
        ...
        uint256 currentBettableAmount = getCurrentBettableAmount(timeframeId, epoch);
        require(currentBettableAmount > 0, "Bet is not available");
    ...
    function claimBatch(uint8[] memory timeframeIds, uint256[][] memory epochs) external {
        require(timeframeIds.length == epochs.length, "INVALID_ARRAY_LENGTH");
        ...
        returns (bool)
    {
        ...
        if (block.timestamp >= timestamp + timeframes[timeframeId].interval - bufferTime) {
            return false;
        }
        ...
        if (rounds[timeframeId][epoch].startBlockTime != 0) {
            ...
            return timestamp <= block.timestamp + futureBettingTimeUpTo;
        }
    }
    ...
    function refundable(
        ...
        return
            !round.oracleCalled &&
            block.timestamp > round.closeBlockTime &&
            betInfo.amount != 0;
    }
    ...
    */
    ...
    function isNecessaryToExecute(uint8 timeframeId) public view returns(bool) {
        ...
        Round memory currentActiveRound = rounds[timeframeId][currentEpochs[timeframeId]];
        Round memory currentLockedRound = rounds[timeframeId][currentLockedEpochs[timeframeId]];

        // We use block time
        bool lockable = 
            currentActiveRound.startBlockTime != 0 && 
            currentActiveRound.lockPrice == 0 && 
            block.timestamp >= currentActiveRound.lockBlockTime - bufferTime;

        bool closable = 
            currentLockedRound.lockBlockTime !=0 && 
            currentLockedRound.closePrice == 0 && 
            !currentLockedRound.oracleCalled && 
            block.timestamp >= currentLockedRound.closeBlockTime - bufferTime;

        return lockable && closable && (currentLockedRound.totalAmount > 0 || currentActiveRound.totalAmount > 0 || round.totalAmount > 0 || prevRound.totalAmount > 0);
    }
    ...
    function getExecutableTimeframes() external view returns(uint8[] memory result) {
        ...
        uint256 toDrop = timeframes.length - count;
        if (toDrop > 0) {
            assembly {
    ...
    */
    function getRoundIdAt(uint8 timeframeId, uint256 timestamp)
        ...
        returns(uint256 roundFromBlockTime)
    {
        roundFromBlockTime = (timestamp - genesisStartBlockTimestamp) / timeframes[timeframeId].interval;
    }

    /**
    * @dev Get block from epoch
    */
    function getBlockTimeForEpoch(uint8 timeframeId, uint256 epoch) public view returns(uint256 timestamp) {
        timestamp = genesisStartBlockTimestamp + epoch * timeframes[timeframeId].interval;
    }
    ...
    */
    ...
    /// @dev Set buffer time
    function setBufferTime(uint256 time) external onlyOperator {
        bufferTime = time;                
    }
}
