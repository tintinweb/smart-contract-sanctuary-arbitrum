...
contract NutStaking is OwnableUpgradeable, PausableUpgradeable {
     ...
     */
    function initialize(address _mafiaNuts) external initializer {
        __Ownable_init();
        __Pausable_init();
        MAFIA_NUTS = _mafiaNuts;
    ...
    function _stakeNut(address _user, uint _tokenId) internal {
        ...
        IERC721(MAFIA_NUTS).transferFrom(_user, address(this), _tokenId);
        emit Stake(_user, _tokenId);
    }

    function _unstakeNut(address _user, uint _tokenId) internal {
        delete stakerMap[_user];
        IERC721(MAFIA_NUTS).transferFrom(address(this), _user, _tokenId);
