...
function openShortPositionRequest(
    ...
    address token = isCall ? address(AoriCall(option).UNDERLYING()) : address(AoriPut(option).USDC());
    require(isCall ? lens._checkCallIsListed(callFactory, option) : lens._checkPutIsListed(putFactory, option), "Not a valid market");
    require((isCall ? AoriCall(option).endingTime() : AoriPut(option).endingTime()) != 0, "Invalid maturity");
    ...
    function executeOpenPosition(uint256 indexToExecute) public returns (bytes32) {
        ...
        ERC20 underlying = positionRequest.isCall ? AoriCall(positionRequest.option).UNDERLYING() : AoriPut(positionRequest.option).USDC();
    ...
    function getInterestOwed(bool isCall, ERC20 underlying, uint256 strikeInUSDC, uint256 optionSize, uint256 entryMarginRate, uint256 lastAccrueTime) public view returns (uint256) {
        ...
        interestFactor = ((borrowRate + entryMarginRate) / 2) * (block.timestamp - lastAccrueTime);
        if (isCall) {
    ...
    function getInitialMargin(ERC20 token, uint256 fairValueInUSDCScale, uint256 optionSize, bool isCall) public view returns(uint256) {
        uint256 positionVal;
        positionVal = Structs.mulDiv(fairValueInUSDCScale, optionSize, 10**token.decimals()); //1e6 * 1e18 / 1e6
        return Structs.mulDiv(positionVal, IERC20(token).decimals(), getPrice(manager.oracles(address(token)))) + Structs.mulDiv(optionSize, initialMarginRatio, BPS_DIVISOR); 
    }
