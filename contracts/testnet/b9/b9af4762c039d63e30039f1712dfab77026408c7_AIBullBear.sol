...
contract AIBullBear is ERC20, Ownable {
    ...
    // Buy Fees
    uint256 public burnFeeBuy = 10000;

    uint256 public treasureBuy = 50000;
    // Sell Fees
    uint256 public burnFeeSell = 10000;

    uint256 public treasureSell = 50000;
    ...
    constructor(address _weth) ERC20("BullBear AI", "AIBB") {
    ...
    function takeFee(
        ...
        uint256 feeAmount = 0;
        if (side == 1) {
            feeAmount = treasureBuy + burnFeeBuy;
            _transfer(sender, treasureWallet, treasureBuy);
            _burn(sender, burnFeeBuy);
        } else {//=2 only
            feeAmount = treasureSell + burnFeeSell;
            _transfer(sender, treasureWallet, treasureSell);
            _burn(sender, burnFeeSell);
        }
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, "AIBB: ETH_TRANSFER_FAILED");
    }
    ...
    function setFeeReceivers(address _treasureWallet) external onlyOwner {
      ...
      function setAllowLiquidityAddressBeforeLaunch(address _allow) external onlyOwner {
          canAddLiquidityBeforeLaunch[_allow] = true;
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "AIBB: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "AIBB: index out of bounds");
