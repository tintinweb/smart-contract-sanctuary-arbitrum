...
contract PositionKeeper is PositionConstants, ReentrancyGuard, IPositionKeeper, Ownable {
    ...
    function decreaseReservedAmount(address _token, bool _isLong, uint256 _amount) external override nonReentrant onlyPositionHandler {
        if (ISettingsManager(settingsManager).isOnBeta()) {
            require(reservedAmounts[_token][_isLong] >= _amount, "ReservedAmounts exceeded");
        }   

        uint256 newAmount = reservedAmounts[_token][_isLong] < _amount ? 0 : reservedAmounts[_token][_isLong] - _amount;
        reservedAmounts[_token][_isLong] = newAmount;

        if (reservedAmounts[_token][_isLong] > 0) {
    ...
    function decreasePoolAmount(address _indexToken, bool _isLong, uint256 _amount) external override nonReentrant onlyPositionHandler {
        if (ISettingsManager(settingsManager).isOnBeta()) {
            require(poolAmounts[_indexToken][_isLong] >= _amount, "PoolAmount exceeded");
        }

        uint256 newAmount = poolAmounts[_indexToken][_isLong] < _amount ? 0 : poolAmounts[_indexToken][_isLong] - _amount;
        poolAmounts[_indexToken][_isLong] = newAmount;

        if (poolAmounts[_indexToken][_isLong] > 0) {
    ...
    function getPositionCollateralToken(bytes32 _key) external override view returns (address) {
        return orders[_key].collateralToken;
    }
