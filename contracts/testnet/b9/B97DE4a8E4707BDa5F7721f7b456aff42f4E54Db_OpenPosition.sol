...
interface IOpenPosition {
    ...
    function excuteCopy(bytes32 id, CopyRequest[] calldata copyRequest)
        external;
...
contract OpenPosition is Roles, ERC2771Context, IOpenPosition {
    ...
    uint256 constant BPS = 1e6; //Base Percent
    ...
    uint256 public maxCopyExpire = 60; //60s
        ...
        override
    {
        ...
        uint256 length = copyRequest.length;
    ...
    function openCopyPosition(
        ...
        uint256 amount = copyRequest.fixedAmount != 0
            ...
            : ((position.amount * copyRequest.percentAmount) / BPS);
        require(amount * position.leverage >= minSize, "!min-size");
        pool.transferIn(copyRequest.owner, amount);
        uint256 feeOpen = (amount * position.leverage * openFee) / BPS;
        require(amount > feeOpen + executionFee, "!fee-not-enough");
        Types.Position memory cpPosition = Types.Position(
            ...
            position.entryPrice,
            amount - feeOpen - executionFee,
            position.fundingTracker
        );
        bytes32 id = positionStorage.addPositionWithMaster(
            cpPosition,
            IPosition.Master(copyRequest.master, copyRequest.sharePercent),
            masterId
        );
        //TODO fix tp percent
        // if (copyRequest.percentTp > 0) {
        //     positionStorage.setTp(id, copyRequest.percentTp);
        // }
        // if (copyRequest.percentSl > 0)
        //     positionStorage.setSl(id, copyRequest.percentTp);
        pool.creditFee(id, feeOpen, executionFee);
        return id;
    }
