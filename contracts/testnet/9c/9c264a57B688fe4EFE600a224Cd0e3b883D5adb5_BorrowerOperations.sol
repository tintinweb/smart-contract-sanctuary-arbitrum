...
contract BorrowerOperations is AGLBase, CheckContract, IBorrowerOperations {
	...
	function _closeTrove(
		...
		IUSDAScriptTest USDATokenCached = usdaToken;
		...
		if (debt > userUSDA) {
			uint256 amountNeeded = debt - userUSDA;
			...
			require(
				USDATokenCached.balanceOf(msg.sender) >= debt,
		...
		_requireSufficientUSDABalance(
			USDATokenCached,
			msg.sender,
			debt.sub(aglParams.USDA_GAS_COMPENSATION(_asset))
		);
		...
		_repayUSDA(_asset, activePoolCached, USDATokenCached, msg.sender, debt);
...
interface IAgilelyDexTrader {
	/**
	 * @param traderSelector the Selector of the Dex you want to use. If not sure, you can find them in AgilelyDexTrader.sol
	...
	/**
	 * exchange uses Agilely's traders but with your own routing.
...
abstract contract IUSDAScriptTest {
    // --- Events ---
