...
import {ITorAddressRegister} from "src/ITorAddressRegister.sol";
import {TorAddressRegister} from "src/TorAddressRegister.sol";

contract TorAddressRegister_1 is TorAddressRegister {
    // @todo                ^ Adjust name of TorAddressRegister instance.
    constructor(address initialAuthed) TorAddressRegister(initialAuthed) {}
}

/**
 * @notice TorAddressRegister Management Script
 */
contract TorAddressRegisterScript is Script {
    /// @dev Deploys a new TorAddressRegister instance via Greenhouse instance
        ...
        public
    {
        ...
        bytes memory creationCode = abi.encodePacked(
            type(TorAddressRegister_1).creationCode, abi.encode(initialAuthed)
        );
        // @todo                ^ Adjust name of TorAddressRegister instance.
    ...
    // -- ITorAddressRegister Functions --

    /// @dev Adds tor address `torAddress`.
    function add(address self, string memory torAddress) public {
        vm.startBroadcast();
        ITorAddressRegister(self).add(torAddress);
        vm.stopBroadcast();

        console2.log("Tor Address added", torAddress);
    }

    /// @dev Adds list of tor addresses `torAddresses`.
    function add(address self, string[] memory torAddresses) public {
        vm.startBroadcast();
        ITorAddressRegister(self).add(torAddresses);
        vm.stopBroadcast();

        console2.log("Tor Addresses added");
        for (uint i; i < torAddresses.length; i++) {
            console2.log("  ", torAddresses[i]);
        }
    }

    /// @dev Adds tor address with index `index`.
    function remove(address self, uint index) public {
        vm.startBroadcast();
        ITorAddressRegister(self).remove(index);
        vm.stopBroadcast();

        console2.log(
            string.concat(
                "Tor Address with index=", vm.toString(index), " removed"
            )
        );
    }
