...
library GlobalConstants {
    ...
    uint256 public constant DEFAULT_CONTEST_MINIMUM_TIME = 10 minutes;
    uint256 public constant DEFAULT_CONTEST_MINIMUM_APPROVED_ENTRIES = 3;
...
contract Contest is IContest, HasConfigStore, Ownable, ReentrancyGuard, Multicall {
        ...
        nonReentrant
    {
        ...
        entries[entryId] = Entry(false, entryURI, msg.sender, stakedContract, stakedTokenId);
        IContestStaker(address(_getContestFactory())).freezeStake(stakedContract, stakedTokenId, msg.sender, 1);
        emit SubmittedEntry(stakedContract, stakedTokenId, entryId, msg.sender, entryURI);
    }
    ...
    function reclaimEntry(uint256 entryId) external nonReentrant {
        ...
        if (entries[entryId].entrant == address(0)) revert NotEntrant();
        address stakedContract = entries[entryId].stakedContract;
        uint256 stakedTokenId = entries[entryId].stakedTokenId;
    ...
    function getEntryURI(uint256 entryId) external view returns (string memory) {
...
contract ContestFactory is IContestFactory, ContestStaker, Ownable {
    ...
    function removeStakedContract(address stakedContract) public onlyContract nonReentrant {
        _removeStakedContract(stakedContract);
    }

    function removeStakeableTokenId(address stakedContract, uint256 tokenId) public onlyContract nonReentrant {
...
contract ContestStaker is IContestStaker, ERC1155Holder, ReentrancyGuard, Multicall {
     ...
     */
    function stake(address stakedContract, uint256 tokenId, uint256 amount) public nonReentrant {
        if (!stakedContracts[stakedContract]) revert InvalidStakedContract();
        if (!stakeableTokenIds[stakedContract].contains(tokenId)) revert InvalidTokenId();
     ...
     */
    function unstake(address stakedContract, uint256 tokenId, uint256 amount) public nonReentrant {
    ...
    function getStakeAmountsForUser(
        address stakedContract,
        address user
    )
        public
    ...
    function canUseStake(address stakedContract, uint256 tokenId, address staker) public view override returns (bool) {
        return getUsableStake(stakedContract, tokenId, staker) > 0;
    }

    function isStakedContract(address stakedContract) public view returns (bool) {
...
contract ToggleGovernance is ERC1155Holder, IToggleGovernance, ReentrancyGuard, Multicall {
    ...
    function hasEnoughOpenGovernToggles(uint256 requiredOpenToggles) public view override returns (bool) {
