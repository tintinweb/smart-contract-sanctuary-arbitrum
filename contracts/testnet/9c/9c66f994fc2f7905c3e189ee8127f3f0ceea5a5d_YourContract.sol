...
contract YourContract is ERC721A, ERC721ABurnable, AccessControl, ReentrancyGuard {
  ...
  event BeginRitual(address indexed _to, uint256 indexed _amount);

  // emits Finish Ritual success for front-end consumption
  event FinishRitual(uint256 indexed _startTokenId, uint256 indexed _endTokenId, bytes indexed _batchHash);
  ...
  function finishRitual(address _to) external callerIsUser notPaused {
    ...
    emit FinishRitual(_startTokenId, _endTokenId, _blockHash);
