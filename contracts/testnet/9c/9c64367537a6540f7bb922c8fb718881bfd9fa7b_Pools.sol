...
contract Pools is Ownable, ReentrancyGuard, ERC20 {
    ...
    function deposit(uint256 _amount) external whenInitialized nonReentrant {
        ...
        if (_amount > 0) {
            ...
            uint256 currentBaseBalance = IERC20(base).balanceOf(msg.sender);

            if (currentStakedBalance == 0 && currentBaseBalance == 0) {
                _beforeTokenTransfer(msg.sender, address(this), _amount);
                stakedToken.safeTransferFrom(msg.sender, address(this), _amount);
                user.amount += _amount;
                IBase(base).mintBase(msg.sender, 1);
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (currentStakedBalance - _amount == 0 && currentBaseBalance > 0) {
                user.amount -= _amount;
                _beforeTokenTransfer(address(this), msg.sender, _amount);
                stakedToken.safeTransfer(msg.sender, _amount);
                IBase(base).burnBase(msg.sender, currentBaseBalance);
                totalStakedTokens -= _amount;
                if(!isRewardExempt[msg.sender]){ setShare(msg.sender, user.amount); }
...
interface IBase {
    function burnBase(address _staker, uint256 _amount) external;
    function mintBase(address _depositor, uint256 amount) external;
}
...
contract Base is ERC20 {
    ...
    event BaseBurned(address _account, address destination, uint256 _amount);
    event BaseMinted(address _account, address destination, uint256 _amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("Your Base is non-transferrable");
    }
    ...
    function burnBase(address _staker, uint256 _amount) external onlyStakingContract() {
    ...
    function _burn(address account, uint256 amount) internal override {
        ...
        emit BaseBurned(account, address(0), amount);
        _afterTokenTransfer(account, address(0), amount);
    }

    // --------- Modified Mint Fxns ----------
   
    function mintBase(address _depositor, uint256 amount) external onlyStakingContract() {
       _mint(_depositor, amount);
    }

    function _mint(address account, uint256 amount) internal override {
        require(account != address(0), "ERC20: cannot mint to the zero address");
        require(Balances[account] == 0, "Already has a Base"); 
        _beforeTokenTransfer(address(0), account, amount);

        _tTotal += amount;
        Balances[account] += amount;
        emit Transfer(address(0), account, amount);
        emit BaseMinted(address(0), account, amount);
