...
import "./IElleriumMedalERC20.sol";
...
contract ElleriaERC20Bridge is Ownable {
  ...
  IElleriumMedalERC20 private medalAbi;
  address private medalsAddr;
  ...
  */
   ...
   */
  function SetReferences(address _signatureAddr, address _signerAddr, address _medalsAddr, address _elmAddr) external onlyOwner {
      signatureAbi = ISignature(_signatureAddr);
      signerAddr = _signerAddr;

      medalAbi = IElleriumMedalERC20(_medalsAddr);
      medalsAddr = _medalsAddr;

      elleriumAddr = _elmAddr;
  }

   /**
   * Allows someone to bridge an ERC20 token
   * into Elleria for in-game usage.
   */
  ...
  /**
   * Allows someone to withdraw $MEDALS from Elleria (minted from contract).
   */
  function RetrieveMedalsFromGame(bytes memory _signature, uint256 _amountInWEI, uint256 _txnCount) external {
    require(!_isProcessed[_txnCount], "Duplicate TXN Count!");

    medalAbi.mint(msg.sender, _amountInWEI);
    _isProcessed[_txnCount] = true;

    emit ERC20Withdraw(msg.sender, medalsAddr, _amountInWEI, ++withdrawCounter);
    require(signatureAbi.verify(signerAddr, msg.sender, _amountInWEI, "medals withdrawal", _txnCount, _signature), "Invalid withdraw");
  }
  ...
  function RetrieveElleriumFromGame(bytes memory _signature, uint256 _amountInWEI, uint256 _txnCount) external {
    ...
    IERC20(elleriumAddr).transfer(msg.sender, _amountInWEI);
    _isProcessed[_txnCount] = true;
