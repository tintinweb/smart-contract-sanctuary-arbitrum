...
import "../Liquidity/ILiquidityStaker.sol";

contract BattleBoost is Pausable, ManagerModifier {
  ...
  function getAnimaBase(
    ...
    if (address(liquidityToken) == address(0) || address(liquidityStaker) == address(0)) {
      return baseAnima;
    }
    ...
    ) = liquidityStaker.currentStatusTotal(staker, cap);
    uint256 unvestedDeposit = totalDeposit - fullyVested;
    for (uint256 i = 0; i < THRESHOLDS.length; i++) {
      uint256 thresholdAmount = (THRESHOLDS[i] * totalSupply) / ONE_HUNDRED;
      if (totalDeposit < thresholdAmount) {
        return baseAnima;
      }
      ...
      if (fullyVested >= thresholdAmount) {
        ...
        baseAnima +=
          (ANIMA_BASE_PER_THRESHOLD * partiallyVested) /
          unvestedDeposit;
      }
    }
    return baseAnima;
  ...
  function setContracts(address _lpToken, address _lpStaker) public onlyAdmin {
...
interface ILiquidityStaker {
  ...
  function unstake(uint256 _stakedDepositIndex, uint256 _amount) external;
