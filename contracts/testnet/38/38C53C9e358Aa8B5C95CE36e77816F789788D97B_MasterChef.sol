...
contract MasterChef is Ownable {
    ...
    // set a max arbs per second, which can never be higher than 1 per second
    uint256 public constant maxArbsPerSecond = 1e18;
    ...
    function pendingARBS(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 arbsReward = multiplier.mul(arbsPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accArbsPerShare = accArbsPerShare.add(arbsReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accArbsPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        ARBS.mintByLiquidityMining(devaddr, arbsReward.div(10));
        ARBS.mintByLiquidityMining(address(this), arbsReward);

        pool.accArbsPerShare = pool.accArbsPerShare.add(arbsReward.mul(1e12).div(lpSupply));
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 pending = user.amount.mul(pool.accArbsPerShare).div(1e12).sub(user.rewardDebt);

        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accArbsPerShare).div(1e12);
...
interface IArbswapToken is IERC20 {
    ...
    function mintByCommunity(address _to, uint256 _amount) external;

    function mintByPartnership(address _to, uint256 _amount) external;

    function mintByInitialLiquidity(address _to, uint256 _amount) external;

    function mintByTeam(address _to, uint256 _amount) external;
}
