...
contract Coinflip is ReentrancyGuard, Ownable {
    ...
    uint256 public minFlipAmount = .01 ether; //min flip amounts
    uint256 public callbackGasLimit = 5_000_000; //min flip amounts
    ...
    event FlippCallback(uint256 indexed flipId, CoinSide headsOrTails);
    ...
    event NewMinFlipAmount(uint256 minFlipAmount);
    ...
    function flip(uint256 _flipAmountAndFees, CoinSide _headsOrTails) external notContract returns (uint256, uint256) {
        ...
        require(flipAmount >= minFlipAmount, "amount must >= minFlipAmount");
    ...
    function randomizerCallback(uint256 _id, bytes32 _value) external {
        ...
        CoinSide flipResult = uint256(_value) % 2 == 0 ? CoinSide.HEADS : CoinSide.TAILS;
    ...
    function setFlipMultiple(uint256 _multiple) external onlyOwner {
        require(_multiple > 0, "multiple must > 0");
        flipMultiple = _multiple;

        emit NewFlipMultiple(_multiple);
    }

    function setMinFlipAmount(uint256 _minFlipAmount) external onlyOwner {
        require(_minFlipAmount > 0, "_minFlipAmount must > 0");
        minFlipAmount = _minFlipAmount;

        emit NewMinFlipAmount(_minFlipAmount);
    }
