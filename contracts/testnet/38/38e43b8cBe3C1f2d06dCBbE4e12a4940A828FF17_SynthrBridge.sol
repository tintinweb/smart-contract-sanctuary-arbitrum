...
interface ISynthrAggregator {
    ...
    function liquidate(
        ...
        uint debtShare,
        uint increasedAccumulatedShare
    ) external;
...
contract SynthrBridge is BaseBridge {
    ...
    event Liquidate(address indexed account, bytes32 indexed collateralKey, uint collateralAmount, uint debtShare, uint16 srcChainId);
    ...
    modifier onlySynthr() {
        require(
            msg.sender == getAddressFromResolver("WrappedSynthr") ||
                ...
                msg.sender == getAddressFromResolver("RewardEscrowV2") ||
    ...
    function calcLZFee(bytes memory lzPayload, uint16 packetType) external view returns (uint totalFee) {
        ...
        for (uint ii = 0; ii < length; ii++) {
            dstChainId = _supportedChains.at(ii);
            _adapterParam = _lzAdapterParam(dstChainId, packetType);
    ...
    function _nonblockingLzReceive(
        ...
        if (packetType == PT_SYNTH_MINT) {
            ...
            if (_destChainId == _selfLZChainId) {
                ISynthrIssuer(getAddressFromResolver("Issuer")).destIssue(_accountForCollateral, _synthKey, _synthAmount, _debtShare);
            }
            ...
            (, bytes memory accountBytes, bytes32 synthKey, uint burnAmount, uint debtShare, bool burnToTarget) = abi.decode(_payload, (uint16, bytes, bytes32, uint, uint, bool));
            address accountForSynth = accountBytes.toAddress(0);
            if (!burnToTarget) {
                ISynthrIssuer(getAddressFromResolver("Issuer")).destBurn(accountForSynth, synthKey, burnAmount, debtShare);
            }
            _sendBurn(accountForSynth, synthKey, burnAmount, debtShare);
            emit SynthBurn(accountForSynth, synthKey, burnAmount, debtShare, _srcChainId);
            ...
            if (destChainId == _selfLZChainId) {
                IExchanger(getAddressFromResolver("Exchanger")).updateDestinationForExchange(destAccount, destKey, destAmount);
            }
            ...
            (, bytes memory accountBytes, bytes32 collateralKey, uint collateralAmount, uint debtShare, uint increasedAccumulatedShare) = abi.decode(
                _payload,
                (uint16, bytes, bytes32, uint, uint, uint)
            );

            address account = accountBytes.toAddress(0);
            _sendLiquidate(account, collateralKey, collateralAmount, debtShare, increasedAccumulatedShare);

            uint16 __srcChainId = _srcChainId;
            emit Liquidate(account, collateralKey, collateralAmount, debtShare, __srcChainId);
    ...
    function sendBurn(
        ...
        // synthAmount is always greater than refund according to core contract logic
        uint burnAmount = synthAmount + reclaimed - refund;
        _sendBurn(accountForSynth, synthKey, burnAmount, debtShare);
        emit SynthBurn(accountForSynth, synthKey, burnAmount, debtShare, _selfLZChainId);

        bytes memory lzPayload = abi.encode(PT_SYNTH_BURN, abi.encodePacked(accountForSynth), synthKey, burnAmount, debtShare, burnToTarget);
    ...
    function _sendBurn(
        ...
        ISynthrAggregator(synthrAggregator).burnSynth(accountForSynth, synthKey, burnAmount, debtShare);
    }
    ...
    function sendLiquidate(
        ...
        _sendLiquidate(account, collateralKey, collateralAmount, debtShare, increasedAccumulatedShare);
        emit Liquidate(account, collateralKey, collateralAmount, debtShare, _selfLZChainId);

        bytes memory lzPayload = abi.encode(PT_LIQUIDATE, abi.encodePacked(account), collateralKey, collateralAmount, debtShare, increasedAccumulatedShare);
    ...
    function _sendLiquidate(
        ...
        ISynthrAggregator(synthrAggregator).liquidate(account, collateralKey, collateralAmount, debtShare, increasedAccumulatedShare);
    }
    ...
    function sendVest(address account, uint amount) external payable {
        _sendVest(account, amount);
    ...
    function _sendVest(address account, uint amount) private {
    ...
    function _broadcast(bytes memory lzPayload, uint16 packetType) internal {
        ...
        for (uint ii = 0; ii < length; ii++) {
            dstChainId = _supportedChains.at(ii);
            _adapterParam = _lzAdapterParam(dstChainId, packetType);
            (lzFee, ) = iEndpoint.estimateFees(dstChainId, address(this), lzPayload, false, _adapterParam);
            require(userFund >= lzFee, "SB: Insufficient LZ fee");
            userFund -= lzFee;
            _lzSend(dstChainId, lzPayload, payable(address(this)), address(0x0), _adapterParam, lzFee);

            outBoundNonce = iEndpoint.getOutboundNonce(dstChainId, address(this));
