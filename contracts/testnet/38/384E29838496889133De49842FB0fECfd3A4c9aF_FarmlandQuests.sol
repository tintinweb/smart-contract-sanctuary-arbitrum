...
// STATE VARIABLES
    ...
    /// @dev PUBLIC: Create an mapping to track a characters current quest
...
// SETTERS
        ...
        nonReentrant
    {
        ...
        ownerOfExplorers[_msgSender()].push(Character(tokenID, traitTotal, block.number));                 // Add details to Character Struct
        ...
        explorerAdded(tokenID)
    {
        ...
        uint256 nextQuest = getCharactersCurrentQuest(tokenID) + 1;          // Increment quest number
        latestQuest[tokenID] = nextQuest;                                    // Update quest number
        ...
        emit QuestStarted(_msgSender(), nextQuest, endBlock, tokenID);       // Write an event
        ...
        explorerAdded(tokenID)
    {
        ...
        uint256 currentQuest = getCharactersCurrentQuest(tokenID);
        (,uint256 characterIndex) = getCharactersIndex(_msgSender(),tokenID);                     // Find the characters index
        ...
        for(uint256 i=0; i < questLength; i++) {                                                  // Loop through the quest and mint items
            ...
            uint256 totalToMint = (getRandomNumber() % rewardItems[itemToMint].scarcity)+1;       // Random number capped @ item scarcity
            itemsReceived.push(ItemReceived(currentQuest, tokenID, itemToMint, totalToMint));     // Add the items received
        ...
        if (getRandomNumber() % 100 < 5) {                                                        // Land is found if the random number is less 5 .. ie. 5% chance of finding Land           
            amountOfLandFound = (getRandomNumber() % traitTotal) * (10**18);                      // You found between 0 and max 98 (capped at the average of courage & intelligence)
...
// GETTERS
    ...
    /// @dev Return a characters current quest
    /// @param tokenID Characters ID
    function getCharactersCurrentQuest(uint256 tokenID)
        public
        view
        returns (
            uint256 charactersCurrentQuest)
    {
       return latestQuest[tokenID];
    }
    ...
    /// @param tokenID Characters ID
    function getCharactersItemsReceived(uint256 tokenID)
        external
        view
        ...
        if ( total == 0 ) {
            ...
            for(uint256 i = 0; i < total; i++){                                     // Loop through the draws
                if(itemsReceived[i].tokenID == tokenID)
                {
...
struct ItemReceived {uint256 quest; uint256 tokenID; uint256 itemID; uint256 amount;}
struct Character {uint256 tokenID; uint256 traitTotal; uint256 addedBlockNumber;}
