...
import {DUNGEON_MAP_TRAIT_ID, DUNGEON_START_TIME_TRAIT_ID, DUNGEON_END_TIME_TRAIT_ID, MANAGER_ROLE, GAME_LOGIC_CONTRACT_ROLE} from "../Constants.sol";
...
contract DungeonSystem is IDungeonSystem, GameRegistryConsumerUpgradeable {
    ...
    function endDungeonBattle(
        ...
        DungeonNode memory node = _getDungeonNode(
            _getDungeonMapEntity(params.dungeonTriggerEntity),
            params.node
        );
    ...
    function _validateStartDungeonBattle(
        ...
        uint256 startAt = tokenTemplateSystem.getTraitUint256(
            tokenContract,
            tokenId,
            DUNGEON_START_TIME_TRAIT_ID
        );

        uint256 endAt = tokenTemplateSystem.getTraitUint256(
            tokenContract,
            tokenId,
        ...
        // } else {
        //     if (
        //         (endAt + getExtraTimeForDungeonCompletion() < block.timestamp)
        //     ) {
        //         revert DungeonExpired(params.dungeonTriggerEntity);
        //     }
        // }
