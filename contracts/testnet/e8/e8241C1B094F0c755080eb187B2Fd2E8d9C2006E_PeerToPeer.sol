...
contract PeerToPeer is ReentrancyGuard, Ownable {
    struct Token {
        uint256 tokenId;
        string name;
        string symbol;
    ...
    function addPosition(
        ...
        positions[currentPositionId] = Position(
            ...
            block.timestamp,
            msg.sender,
            tokens[_sellToken].name,
            tokens[_buyToken].name,
            _sellTokenQuantity,
            _exchangeRate,
            _calculateBuyTokenQuantity(_sellTokenQuantity, _exchangeRate),
            true,
            false
        );
        ...
        emit PositionAdded(msg.sender, _sellToken, _sellTokenQuantity);
    }
    ...
    //Internal function
    function _calculateBuyTokenQuantity(
        uint256 _sellTokenQuantity,
        uint256 _exchangeRate
    ) internal pure returns (uint256) {
        return _sellTokenQuantity * _exchangeRate;
    }

    function cancelPosition(uint256 _positionId) external {
    ...
    function takePosition(uint256 _positionId) external {
        if (positions[_positionId].open != true) {
            revert Peer__PositionIsClosed();
        }
