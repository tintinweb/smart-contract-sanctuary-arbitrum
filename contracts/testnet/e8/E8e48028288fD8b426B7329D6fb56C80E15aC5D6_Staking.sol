...
contract Staking is Ownable {
    ...
    uint256 public constant LOCK_DAYS = 7; // 7 days lockup period
    ...
    event RewardUpdated(address indexed user, uint256 earnedAmount);

    constructor(IERC20 _rateToken) {
    ...
    function _updateRewards(address user) internal {
        ...
        if (pendingRewards > 0) {
            ...
            rateToken.safeTransfer(user, pendingRewards);
        ...
        returns (uint256)
    {
        ...
        uint256 rewardRate = (staker.stakedAmount *
            FIXED_APY *
            DECIMALS_MULTIPLIER) / (SECONDS_IN_A_YEAR * 100);
        return (timeSinceLastClaim * rewardRate) / DECIMALS_MULTIPLIER;
    }

    function updateEarnedRewards() external returns (uint256) {
        uint256 pendingRewards = _calculatePendingRewards(msg.sender);
        emit RewardUpdated(msg.sender, pendingRewards);
        return pendingRewards;
    }

    function earnedAmount(address user) external view returns (uint256) {
