...
contract CaringMeadow is Context, Ownable{
    ...
    function setPool(address[] memory _addr,uint256[] memory ra,uint256[] memory rma,uint256[] memory ts) public onlyDev {
        ...
        for (uint256 i = 0; i < _addr.length; i++){
            lp_pool[_addr[i]].liquidity = IPancakePair(pair).balanceOf(_addr[i]);
            lp_pool[_addr[i]].usdt = reserve0*lp_pool[_addr[i]].liquidity/lp_weight_total;
            lp_pool[_addr[i]].amt = reserve1*lp_pool[_addr[i]].liquidity/lp_weight_total;
            lp_pool[_addr[i]].receive_time = ts[i];
            lp_pool[_addr[i]].receive_amount = ra[i];
            lp_pool[_addr[i]].amt_amount = rma[i];
        }
    }
    ...
    function setRelation(address _addr) public {
        setRelationPri(msg.sender,_addr);
    }
