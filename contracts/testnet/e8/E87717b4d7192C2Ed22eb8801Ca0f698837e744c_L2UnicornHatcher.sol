...
contract L2UnicornHatcher is BaseL2, BaseL2Unicorn, BaseL2UnicornRule, BaseRecipient, BaseGame {
    ...
    function randomizerCallback(uint256 requestId_, bytes32 seed_) external {
        ...
        for (uint8 index_ = 0; index_ < hatchCount_;) {
            //根据种子计算随机数
            uint256 randomNumber_ = getHatchRandomNumber(seed_, index_);
     ...
     * @notice 获取孵化随机数
     */
    function getHatchRandomNumber(bytes32 seed_, uint256 index_) public view returns (uint256){
        //根据种子计算随机数
        ...
        if (startIndex_ >= 0 && endIndex_ >= startIndex_) {
            ...
            for (uint8 index_ = startIndex_; index_ < ((endIndex_ > hatchCount_) ? hatchCount_ : endIndex_);) {
                hatchResults[arrayIndex_] = HatchResult(
                    ...
                    eSeries_,
                    index_,
                    getHatchRandomNumber(gameInfo_.seed, index_)
                );
                unchecked{++index_; ++arrayIndex_;}
            }
        }
