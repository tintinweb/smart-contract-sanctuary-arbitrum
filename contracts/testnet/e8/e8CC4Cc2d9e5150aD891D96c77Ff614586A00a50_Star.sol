...
contract Star is StarState {
    ...
    function tokenURI(
        ...
        string memory baseURI = _baseURI();
        string memory timestamp = getStarStoryTimeStamp(tokenId).toString();

        // Id baseURI is set, redirect to unique json file per token
        if (bytes(baseURI).length > 0 && bytes(timestamp).length > 0) {
    ...
    AdminableUpgradeable
{
    ...
    function setStarStoryTimeStamp(uint256 _tokenId) external {
