...
contract ExtractorStakingRules is IExtractorStakingRules, ERC165Upgradeable, StakingRulesBase {
    ...
    // If non-zero value is not defined here, the lifetime variable is used.
    mapping(uint256 => uint256) extractorIdToLifetime;
    ...
    event ExtractorLifetime(uint256 lifetime, uint256 tokenId);
    // Deprecated and replaced by ExtractorLifetime
    ...
    function init(
        ...
        // Set lifetime for backwards compatibility
        //
        lifetime = _lifetime;
    ...
    function isExtractorActive(uint256 _spotId) public view returns (bool) {
        return block.timestamp <= stakedExtractor[_spotId].stakedTimestamp + extractorLifetime(stakedExtractor[_spotId].tokenId);
    }

    function extractorLifetime(uint256 _tokenId) public view returns (uint256) {
        if(extractorIdToLifetime[_tokenId] > 0) {
            return extractorIdToLifetime[_tokenId];
        } else {
            return lifetime;
        }
    }

    function extractorLifetimes(uint256[] calldata _tokenIds) public view returns (uint256[] memory) {
        uint256[] memory _lifetimes = new uint256[](_tokenIds.length);
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            _lifetimes[i] = extractorLifetime(_tokenIds[i]);
        }
        return _lifetimes;
    }
    ...
    function setExtractorLifetime(uint256 _tokenId, uint256 _lifetime) external onlyRole(SR_ADMIN) {
        nftHandler.harvester().callUpdateRewards();

        _setExtractorLifetime(_tokenId, _lifetime);
    }
    ...
    function _setExtractorLifetime(uint256 _tokenId, uint256 _lifetime) internal {
        extractorIdToLifetime[_tokenId] = _lifetime;
        emit ExtractorLifetime(_lifetime, _tokenId);
    }
}
