...
contract OrderBook is ReentrancyGuard, IOrderBook {
    ...
    uint256 public constant DAI_PRECISION = 1e18;
    ...
    function initialize(
        ...
        emit Initialize(_router, _vault, _weth, _dai, _minExecutionFee, _minPurchaseTokenAmountUsd);
    }
    ...
    function getDaiMinPrice(address _otherToken) public view returns (uint256) {
        // DAI_PRECISION is the same as 1 DAI
        uint256 redemptionAmount = IVault(vault).getRedemptionAmount(_otherToken, DAI_PRECISION);
    ...
    function validateSwapOrderPriceWithTriggerAboveThreshold(
        ...
        // 1. DAI doesn't have a price feed so we need to calculate it based on redepmtion amount of a specific token
        // That's why DAI price in USD can vary depending on the redepmtion token
        // 2. In complex scenarios with path=[DAI, BNB, BTC] we need to know how much BNB we'll get for provided DAI
        // to know how much BTC will be received
        // That's why in such scenario BNB should be used to determine price of DAI
        if (tokenA == dai) {
            // with both _path.length == 2 or 3 we need dai price against _path[1]
            tokenAPrice = getDaiMinPrice(_path[1]);
        } else {
    ...
    function _vaultSwap(address _tokenIn, address _tokenOut, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        if (_tokenOut == dai) { // buyDAI
            amountOut = IVault(vault).buyDAI(_tokenIn, _receiver);
        } else if (_tokenIn == dai) { // sellDAI
            amountOut = IVault(vault).sellDAI(_tokenOut, _receiver);
        } else { // swap
...
interface IVault {
    ...
    function buyDAI(address _token, address _receiver) external returns (uint256);
    function sellDAI(address _token, address _receiver) external returns (uint256);
    ...
    function getFeeBasisPoints(address _token, uint256 _daiDelta, uint256 _feeBasisPoints, uint256 _taxBasisPoints, bool _increment) external view returns (uint256);
    ...
    function daiAmounts(address _token) external view returns (uint256);
    function maxDaiAmounts(address _token) external view returns (uint256);
    function getRedemptionAmount(address _token, uint256 _daiAmount) external view returns (uint256);
