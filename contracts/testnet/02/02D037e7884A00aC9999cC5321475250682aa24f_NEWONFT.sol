...
contract NEWONFT is ONFT721Core, IONFT721, ERC721URIStorage {
    ...
    uint private constant ZK_SYNC = 324;
    ...
    function sendOnft(uint16 destChainId, uint tokenId) external payable nonReentrant {
        ...
        bytes memory payload = abi.encode(_tokenURIBytes, _tokenOwnerBytes, tokenId);

        _debitFrom(_tokenOwner, 0, bytes(""), tokenId);
        // lockToken(tokenId, _tokenOwner);
    ...
    function _nonblockingLzReceive(
        ...
        // Decode the _payload
        (bytes memory _tokenURIBytes, bytes20 _tokenOwnerBytes, uint _tokenId) = abi.decode(_payload, (bytes, bytes20, uint));

        // Convert bytes back to string and bytes20 back to address if needed
        string memory _tokenURI = string(_tokenURIBytes);
        address _tokenOwner = address(_tokenOwnerBytes);

        if (lockedOwnerToken[_tokenId] == _tokenOwner) {
            _creditTo(0, _tokenOwner, _tokenId);
        } else {
            _tokenIds.increment();
            uint newTokenId = _tokenIds.current();
            _safeMint(_tokenOwner, newTokenId);
            _setTokenURI(newTokenId, _tokenURI);
            idToMarketItem[newTokenId].owner = payable(_tokenOwner);
        }
    ...
    function createMarketSale(uint tokenId) external payable nonReentrant {
    ...
    function fetchSellerNFTs(address _seller) external view returns (MarketItem[] memory) {
        uint totalItemCount = _tokenIds.current();
        uint itemCount = 0;
        uint currentIndex = 0;
