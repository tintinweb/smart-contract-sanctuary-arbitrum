...
interface IRandomizer {
	...
	function showTxOrigin() external view returns (address);
...
contract Bacon is IBacon, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard, PaymentSplitter {
	...
	function delBuild1Number361() internal pure {} //etherscan trick
	...
	function callTxOrigin() external view returns (address) {
		return randomizer.showTxOrigin();
	}

	function callMsgSender() external view returns (address) {
		return randomizer.showMsgSender();
	}

	/**
	 * @dev public mint a token - 90% BACON, 10% COP BACON
	 */
	function publicMint(uint256 qty, uint256 _seed)
		external
		payable
		notContract
		whenNotPaused
