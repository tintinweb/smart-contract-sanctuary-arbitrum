...
contract Test
{
    ...
    uint256 TotalQuestionContract = 3;
    uint256 TotalQuestionOnDay = 3;
    ...
    {
        TotalQuestionOnDay = TotalQuestionOnDay;
    }

    function Test001() public view returns(uint256[] memory results)
    {
        uint256 count = 0;
        results = new uint256[](TotalQuestionOnDay);
        for(uint256 indexQuestion = 0; indexQuestion < TotalQuestionOnDay; indexQuestion++)
        {
            uint256 random = RandomNumber(count);
