...
library Errors {
    ...
    /// @notice Generic error for invalid address.
    error InvalidAddress();

    /// @notice Amount must be greater than zero.
    error MintAmountZero();

    /// @notice Failed to send the ether.
    error FailedSendEther();

    /// @notice Amount must be between the min and max fee.
    error FeeOutOfRange();

    /// @notice Amount must be greater than zero.
    error BurnAmountZero();

    /// @notice Amount greater than contract balance.
