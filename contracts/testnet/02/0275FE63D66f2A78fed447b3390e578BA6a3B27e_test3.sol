    ...
    ReentrancyGuardUpgradeable
{
    ...
    function verifyPublicSale2Purchase(
        ...
        return (amount >= publicSale2MinPurchase ||
            purchasedAmount + amount >= publicSale2MinPurchase);
    }

    function verifyPublicSale2BPurchase(
        address account,
        uint256 amount
    ) public view returns (bool) {
        uint256 purchasedAmount = publicSale2BDeposits[account] / baseDecimals;
        return (amount >= publicSale2MinPurchase || // change
            purchasedAmount + amount >= publicSale2MinPurchase);
    }
        ...
        returns (bool)
    {
        ...
        if (isPurchaseLimitEnable) {
            require(
                verifyPublicSale2BPurchase(_msgSender(), amount),
                "Invalid purchased"
            );
        }
