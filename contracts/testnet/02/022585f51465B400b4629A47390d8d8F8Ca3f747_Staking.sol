...
contract Staking is Ownable {
    ...
    IERC20 public immutable tokenStake;
    ...
    constructor(IERC20 _tokenStake) {
        require(address(_tokenStake) != address(0), "zeroAddr");
    ...
    function pendingToken(address _user, uint _indexRw) external view returns (uint256, uint256) {
        PoolInfo memory pool = poolInfo;
        UserInfo memory user = userInfo[_user];
        uint256 lpSupply = pool.totalStake + pool.totalPoint;
        ...
        if (block.timestamp > pool.lastTimeReward && lpSupply != 0) {
            ...
            uint256 tokenReward = multiplier * (rewardInfo[_indexRw].tokenPerSecond);
        ...
        return (((user.amount + user.point)* (accTokenPerShare)) / (1e12) - (pendingReward.rewardDebt) + pendingReward.rewardPending,  block.timestamp);
    }
    ...
    function updatePool() public {
        ...
        for(uint i=0; i<rewardInfo.length; i++) {
            uint256 tokenReward = multiplier * (rewardInfo[i].tokenPerSecond);
            rewardInfo[i].accTokenPerShare = rewardInfo[i].accTokenPerShare + ((tokenReward * (1e12)) / (lpSupply));
        }
        pool.lastTimeReward = block.timestamp;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        if (user.amount > 0) {
            for(uint i=0; i<rewardInfo.length; i++) {
                ...
                uint256 pending = ((user.amount + user.point) * rewardInfo[i].accTokenPerShare) / 1e12 - pendingReward.rewardDebt;
                if (pendingReward.rewardPending +  pending > 0) {
                    rewardInfo[i].rwToken.transfer(msg.sender, pendingReward.rewardPending +  pending);
                }
                pendingReward.rewardDebt = ((user.amount + user.point + _amount) * (rewardInfo[i].accTokenPerShare)) / (1e12);
            }
        }
        if (_amount > 0) {
            user.lastDepositedTime = block.timestamp;
            tokenStake.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function withdraw(uint256 _amount) public {
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount >= user.lock + _amount, "withdraw: amount not enough");
        ...
        if (_amount > 0) {
            tokenStake.safeTransfer(address(msg.sender), _amount);
            ...
            onlyPermission 
    {
        ...
        for(uint i=0; i<_users.length; i++) {
            for(uint j=0; j<rewardInfo.length; j++) {
                ...
                uint256 pending = ((user.amount + user.point) * rewardInfo[j].accTokenPerShare) / 1e12 - pendingReward.rewardDebt;
                if (pending > 0) {
                    pendingReward.rewardPending =   pendingReward.rewardPending + pending;
                }
                pendingReward.rewardDebt = ((user.amount + user.point + _points[i]) * (rewardInfo[j].accTokenPerShare)) / (1e12);
    ...
    function lock(address _addr, uint256 _amount) external onlyPermission returns (bool) {
        ...
        if(user.lock + _amount <= user.amount) {
             user.lock += _amount;
            return true;
        }
        return false;
    ...
    function emergencyWithdraw() public {
        ...
        tokenStake.safeTransfer(address(msg.sender), (user.amount -user.lock));
        ITracker(stakeTracker).burn(address(msg.sender), user.amount);
        
        pool.totalStake -= (user.amount - user.lock);
        pool.totalPoint -= user.point;
        user.amount = user.lock;
        user.point = 0;
        for(uint i=0; i<rewardInfo.length; i++) {
            PendingReward storage pendingReward = rewardPending[msg.sender][rewardInfo[i].rwToken];
