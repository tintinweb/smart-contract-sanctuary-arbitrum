...
abstract contract ContrastPairBase is
    IContrastPair,
    Ownable,
    Clone,
    ERC1155Supply
{
    ...
    function idsForMarket(
        uint256 marketId
    ) public pure returns (uint256 id0, uint256 id1) {
        id0 = marketId << 1;
        id1 = id0 + 1;
    }
    ...
    function exit(
        ...
        uint256 fee = amount0.mulDiv(TRANSFER_FEE_BIPS, 10_000) +
            amount1.mulDiv(TRANSFER_FEE_BIPS, 10_000);
    ...
    function settle(uint256 marketId) external virtual {
        ...
        if ((isCall && answer > strike) || (!isCall && answer <= strike)) {
            ...
            market.exchangeRate = totalSupply[id0]
                .divWad(totalSupply[id1])
                .toUint96();
        } else {
            market.outcome = Outcome.FALSE;
            market.exchangeRate = totalSupply[id1]
                .divWad(totalSupply[id0])
                .toUint96();
        }
