...
contract PEXAdlCallbacksV1_1 is Initializable {
    ...
    function executeAdlCloseOrderCallback(
        ...
        for(uint i = 0; i < adlConditionInfo.totalAdlOrderLength; i++) {
            ...
            adlTradersUsdtFlows[t.trader].adlType = adlConditionInfo.o[i].adlType;
        ...
        // transfer usdt
        callbacks.adlSendToVault(adlConditionInfo.totalSendToVault, adlConditionInfo.o[0].nftHolder); // 1. total loss position collateral from storageT to vault
        ...
        for(uint i = 0; i < adlBatchTraders.length; i++) {
            ...
            if(adlTradersUsdtFlows[adlBatchTraders[i]].adlType == IStorageT.AdlOrder.ADLSL) {

                // 3.1 adl loss postion storageT remain collateral to traders
                ...
                // 3.2 adl profit position storageT remain collateral to traders
                storageT.transferUsdt(address(storageT), adlBatchTraders[i], adlTradersUsdtFlows[adlBatchTraders[i]].storageTSentToTrader);

                // 4. adl profit positions vault to traders
                callbacks.adlVaultSendToTrader(adlTradersUsdtFlows[adlBatchTraders[i]].vaultSentToTrader, adlBatchTraders[i]);
            }
