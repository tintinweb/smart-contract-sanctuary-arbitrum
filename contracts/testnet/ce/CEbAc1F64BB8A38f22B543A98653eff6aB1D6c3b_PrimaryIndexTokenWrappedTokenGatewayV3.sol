...
contract PrimaryIndexTokenWrappedTokenGatewayV3 is
	Initializable,
	...
	ReentrancyGuardUpgradeable
{
	...
	function initialize(
		address pit,
		address weth,
		address _pitLiquidation,
		address _pitLeverage
	) public initializer {
	...
	modifier isProjectTokenListed(address _projectToken) {
		require(
			...
			"WTG: project token is not listed"
		);
		_;
	}
	...
	modifier isLendingTokenListed(address _lendingToken) {
		require(
			...
			"WTG: lending token is not listed"
		);
		_;
	}
	...
	function withdraw(uint256 projectTokenAmount) public nonReentrant {
		uint256 receivedProjectTokenAmount = primaryIndexToken.withdrawFromRelatedContracts(
			address(WETH),
			projectTokenAmount,
			msg.sender,
			address(this)
	...
	function redeemUnderlying(uint256 lendingTokenAmount) public nonReentrant {
		primaryIndexToken.redeemUnderlyingFromRelatedContract(
			address(WETH),
			lendingTokenAmount,
			msg.sender
		);
	...
	function repay(uint lendingTokenAmount) public payable nonReentrant {
		...
		uint paybackAmount = lendingTokenAmount >= totalOutStanding
			? totalOutStanding
			: lendingTokenAmount;
	...
	function borrow(address projectToken, uint256 lendingTokenAmount) public nonReentrant {
		primaryIndexToken.borrowFromRelatedContract(
			projectToken,
			address(WETH),
			lendingTokenAmount,
			msg.sender
	...
	function liquidateWithLendingETH(
		...
		pitLiquidation.liquidateFromModerator(
			_account,
			_projectToken,
			address(WETH),
			_lendingTokenAmount,
	...
	function liquidateWithProjectETH(
		...
		uint256 receivedWETH = pitLiquidation.liquidateFromModerator(
			_account,
			address(WETH),
			_lendingToken,
			_lendingTokenAmount,
	...
	function leveragedBorrowWithProjectETH(
		...
		uint256 addingAmount = pitLeverage.calculateAddingAmount(
			msg.sender,
			address(WETH),
