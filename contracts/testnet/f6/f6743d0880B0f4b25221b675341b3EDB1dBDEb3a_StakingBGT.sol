...
contract StakingBGT is Ownable {
    ...
    function doSettlementInterest() external {
        ...
        uint256 total = poss[account] >= getDPosDquity() ? (poss[account] + dposs[account] + fposs[account]) : poss[account];
    ...
    function calculateInterest(address account) public view returns (uint256) {
        ...
        uint256 total = poss[account] >= getDPosDquity() ? (poss[account] + dposs[account] + fposs[account]) : (poss[account] + fposs[account]);
