...
// 19. adminDenominatorFor2Abilities must be greater than 0.
// 20. The adminDenominatorFor3Abilities must be > adminDenominatorFor2Abilities
// 21. The value must be > 0 and < 10000
...
contract Battle is IBattle, AccessControl {
	...
	struct ContractData {
		...
		mapping(address => uint256) userWinStreaks; // user win streaks
		uint256 adminDenominatorFor2Abilities;
		uint256 adminDenominatorFor3Abilities;
		mapping(uint8 => uint16) hpToValue;
		mapping(uint8 => uint16) attackToValue;
	}
	...
	function initialize(
		...
		uint256 adminDenominatorFor2Abilities_,
		uint256 adminDenominatorFor3Abilities_
	) external {
		...
		_contractData.adminDenominatorFor2Abilities = adminDenominatorFor2Abilities_;
		_contractData.adminDenominatorFor3Abilities = adminDenominatorFor3Abilities_;
	}
	...
	function prepareBattle() external {
		...
		PrepareBattleLogic.prepareBattle(_contractData, _finalFormCardsAddress, _mintInfoOracleAddress);
	...
	function setAbilityDenominators(uint256 adminDenominatorFor2Abilities, uint256 adminDenominatorFor3Abilities) external onlyRole(ADMIN_AUTH) {
		require(adminDenominatorFor2Abilities > 0, "19");
		require(adminDenominatorFor3Abilities > adminDenominatorFor2Abilities, "20");
		_contractData.adminDenominatorFor2Abilities = adminDenominatorFor2Abilities;
		_contractData.adminDenominatorFor3Abilities = adminDenominatorFor3Abilities;
	}

	function getAbilityDenominators() external view returns (uint256, uint256) {
		return (_contractData.adminDenominatorFor2Abilities, _contractData.adminDenominatorFor3Abilities);
	}

	function getWinStreak(address user) external view returns (uint256) {
		return _contractData.userWinStreaks[user];
	}

	function updateHpValue(uint8 hpId, uint16 value) external onlyRole(ADMIN_AUTH) {
		require(value > 0 && value < 10000, "21");
		_contractData.hpToValue[hpId] = value;
	}

	function updateAttackValue(uint8 attackId, uint16 value) external onlyRole(ADMIN_AUTH) {
		require(value > 0 && value < 10000, "21");
		_contractData.attackToValue[attackId] = value;
	}

	function getHpValue(uint8 hpId) external view returns (uint16) {
		return _contractData.hpToValue[hpId];
	}

	function getAttackValue(uint8 attackId) external view returns (uint16) {
		return _contractData.attackToValue[attackId];
	}

	//--------- Just For Testing ---------// TODO remove functions in this block before prod

	function getAllHirelingsOfType(uint8 rarity, uint8 house) external view returns (Hireling[] memory) {
		return _contractData.hirelingPool[rarity][house];
	}
...
//	}
	...
	function battle(CardOrHireling[4] calldata deck) external {
...
//  Battle Log Format
// The first 3 digits (0-999) are the type of log (div 100/1e2)
//
// 001: Game End
//      1e3 Digit: 0 = Win, 1 = Lose, 2 = Tie
//
// 002: Base Attack
//      1e3- > 1e5: Index of You
//      1e6 -> 1e7 : Index of Enemy
//      1e8 -> 1e11: Your Damage
//      1e12 -> 1e15: Enemy Damage
//
// 003: Death
//      We left lots of space for cards to spawn and have large indexes, it goes up to 26 cards on each side
//      1e3 -> 1e28: Your Dead Cards (0000000000000000000000101 --> your 1st and 3rd card died)
//      1e29 -> 1e54: Enemy Card Deaths (0000000000000000000000101 --> enemy 1st and 3rd card died)
//
// 004: Ability
//      1e3 -> 1e6: Ability Code
//      1e7 -> 1e77: This is custom data space to be used by the ability, please see table below.
...
library BattleLogic {
    ...
    struct CardData {
        ...
        uint16 health;
        uint16 attack;
        bool deathNotProcessed;
        bool shieldActive;
    }

    struct LocalBattleVariables {
        // we need to store if the death had been logged yet, because we do not remove the 0 health from the deck data
        // if we did not keep track of "loggedDeath", then it would emit battle logs for a death more than once.
        CardData[4] deckData;
        CardData[4] enemyDeckData;
        uint8 i;
        uint8 count;
        uint8 j;
        uint256 currentLogIndex;
        uint256[128] logs;
        uint8 deckIndex;
    ...
    function battle(Battle.CardOrHireling[4] calldata deck, uint256 battleId, address pledgingAddress, address finalFormCardsAddress, address mintInfoOracleAddress, Battle.ContractData storage data) external returns (uint8 result) {
        // check that user has a active battle config
        require(data.nextBattleConfig[msg.sender].ready, "No battle config set");

        LocalBattleVariables memory localVariables;

        for (; localVariables.i < 4; localVariables.i++) {
            if (deck[localVariables.i].exists) {

                // get the card metadata and check that the sender owns the card
                if (deck[localVariables.i].isCard) {
                    // check that user has this card pledged
                    require(IPledging(pledgingAddress).checkOwnerOfPledgedCard(finalFormCardsAddress, deck[localVariables.i].id) == msg.sender, "Could not verify card ownership");

                    // get card metadata
                    // TODO we could abstract out to the mint info oracle
                    (uint256 health, uint256 attack) = MintInfoOracle(mintInfoOracleAddress).getBattleInfo(finalFormCardsAddress, deck[localVariables.i].id);
                    localVariables.deckData[localVariables.i].originalIndex = localVariables.i;
                    localVariables.deckData[localVariables.i].health = data.hpToValue[uint8(health)];
                    localVariables.deckData[localVariables.i].attack = data.attackToValue[uint8(attack)];
                    localVariables.deckData[localVariables.i].deathNotProcessed = true;
                }

                // check you own the hireling and get the hireling metadata
                else {
                    // count how many times this hireling is used (we can ignore cards with earlier index)
                    localVariables.count = 1;
                    for (localVariables.j = localVariables.i + 1; localVariables.j < 4; localVariables.j++) {
                        if (!deck[localVariables.j].isCard && deck[localVariables.j].id == deck[localVariables.i].id) {
                            localVariables.count++;
                        }
                    }

                    // check that user owns ${count} amount of this hireling
                    require(data.userHirelingsOwnershipLookup[msg.sender][uint8(deck[localVariables.i].id % 100)][uint8(deck[localVariables.i].id / 100 % 100)][deck[localVariables.i].id / 10000] >= localVariables.count, "Could not verify hireling ownership");

                    localVariables.deckData[localVariables.i].originalIndex = localVariables.i;
                    localVariables.deckData[localVariables.i].health = uint16(data.hpToValue[uint8(data.hirelingPool[uint8(deck[localVariables.i].id % 100)][uint8(deck[localVariables.i].id / 100 % 100)][deck[localVariables.i].id / 10000].hp)]);
                    localVariables.deckData[localVariables.i].attack = uint16(data.attackToValue[uint8(data.hirelingPool[uint8(deck[localVariables.i].id % 100)][uint8(deck[localVariables.i].id / 100 % 100)][deck[localVariables.i].id / 10000].attack)]);
                    localVariables.deckData[localVariables.i].deathNotProcessed = true;
                }
            }

            // set the card slot as empty
            // TODO we might not need this, as this should be initted to 0, 0, 0 anyways. We should try a test with removing this.
//            else {
//                localVariables.deckData[localVariables.i] = [uint(0), uint(0), uint(0), uint(0)];
//            }

            if (data.nextBattleConfig[msg.sender].enemyCards[localVariables.i].exists) {
                if (data.nextBattleConfig[msg.sender].enemyCards[localVariables.i].isCard) {
                    (uint256 health, uint256 attack) = MintInfoOracle(mintInfoOracleAddress).getBattleInfo(finalFormCardsAddress, data.nextBattleConfig[msg.sender].enemyCards[localVariables.i].id);
                    localVariables.enemyDeckData[localVariables.i].originalIndex = localVariables.i;
                    localVariables.enemyDeckData[localVariables.i].health = data.hpToValue[uint8(health)];
                    localVariables.enemyDeckData[localVariables.i].attack = data.attackToValue[uint8(attack)];
                    localVariables.enemyDeckData[localVariables.i].deathNotProcessed = true;
                } else {

                    localVariables.enemyDeckData[localVariables.i].originalIndex = localVariables.i;
                    localVariables.enemyDeckData[localVariables.i].health = uint16(data.hpToValue[uint8(data.hirelingPool[uint8(data.nextBattleConfig[msg.sender].enemyCards[localVariables.i].id % 100)][uint8(data.nextBattleConfig[msg.sender].enemyCards[localVariables.i].id / 100 % 100)][data.nextBattleConfig[msg.sender].enemyCards[localVariables.i].id / 10000].hp)]);
                    localVariables.enemyDeckData[localVariables.i].attack = uint16(data.attackToValue[uint8(data.hirelingPool[uint8(data.nextBattleConfig[msg.sender].enemyCards[localVariables.i].id % 100)][uint8(data.nextBattleConfig[msg.sender].enemyCards[localVariables.i].id / 100 % 100)][data.nextBattleConfig[msg.sender].enemyCards[localVariables.i].id / 10000].attack)]);
                    localVariables.enemyDeckData[localVariables.i].deathNotProcessed = true;
                }
            }
//            else {
//                localVariables.enemyDeckData[localVariables.i] = [uint(0), uint(0), uint(0), uint(0)];
//            }
        }

        // run any abilities that proc at start of battle


        // battle loop
        while (isDeckAlive(localVariables.deckData) && isDeckAlive(localVariables.enemyDeckData)) {
            // move up deckData
            localVariables.deckIndex = 0;
            if (localVariables.deckData[0].health != 0) {
                localVariables.deckIndex++;
            }

            if (localVariables.deckData[1].health != 0) {
                localVariables.deckData[localVariables.deckIndex] = localVariables.deckData[1];
                localVariables.deckIndex++;
            }

            if (localVariables.deckData[2].health != 0) {
                localVariables.deckData[localVariables.deckIndex] = localVariables.deckData[2];
                localVariables.deckIndex++;
            }

            if (localVariables.deckData[3].health != 0) {
                localVariables.deckData[localVariables.deckIndex] = localVariables.deckData[3];
                localVariables.deckIndex++;
            }

            while (localVariables.deckIndex < 4) {
                delete localVariables.deckData[localVariables.deckIndex];
                localVariables.deckIndex++;
            }

            // move up enemy deck
            localVariables.enemyDeckIndex = 0;
            if (localVariables.enemyDeckData[0].health != 0) {
                localVariables.enemyDeckIndex++;
            }

            if (localVariables.enemyDeckData[1].health != 0) {
                localVariables.enemyDeckData[localVariables.enemyDeckIndex] = localVariables.enemyDeckData[1];
                localVariables.enemyDeckIndex++;
            }

            if (localVariables.enemyDeckData[2].health != 0) {
                localVariables.enemyDeckData[localVariables.enemyDeckIndex] = localVariables.enemyDeckData[2];
                localVariables.enemyDeckIndex++;
            }

            if (localVariables.enemyDeckData[3].health != 0) {
                localVariables.enemyDeckData[localVariables.enemyDeckIndex] = localVariables.enemyDeckData[3];
                localVariables.enemyDeckIndex++;
            }

            while (localVariables.enemyDeckIndex < 4) {
                delete localVariables.enemyDeckData[localVariables.enemyDeckIndex];
                localVariables.enemyDeckIndex++;
            }

            // basic attack
            if (localVariables.enemyDeckData[0].health < localVariables.deckData[0].attack) {
                localVariables.enemyDeckData[0].health = 0;
            } else {
                localVariables.enemyDeckData[0].health = localVariables.enemyDeckData[0].health - localVariables.deckData[0].attack;
            }

            if (localVariables.deckData[0].health < localVariables.enemyDeckData[0].attack) {
                localVariables.deckData[0].health = 0;
            } else {
                localVariables.deckData[0].health = localVariables.deckData[0].health - localVariables.enemyDeckData[0].attack;
            }

            // log basic attack
            localVariables.logs[localVariables.currentLogIndex] =
                2 + // event 003
                1e3 * localVariables.deckData[0].originalIndex + // index of you
                1e6 * localVariables.enemyDeckData[0].originalIndex + // index of enemy
                1e8 * localVariables.deckData[0].attack + // your damage
                1e12 * localVariables.enemyDeckData[0].attack; // enemy damage
            localVariables.currentLogIndex++;

            // iterate over each card and see if their health is 0
            uint256 deathLog = 3; // set event

            // log any deaths to your deck
            // TODO abtract this into a function
            for (localVariables.i = 0; localVariables.i < localVariables.deckData.length; localVariables.i++) {
                if (localVariables.deckData[localVariables.i].health == 0 && localVariables.deckData[localVariables.i].deathNotProcessed == true) {
                    localVariables.deckData[localVariables.i].deathNotProcessed = false;
                    deathLog += 1e3 * 10 ** localVariables.i; // marks the index as 1 indicating a death at this position
                }
            }

            // log any deaths to your enemy deck
            for (localVariables.i = 0; localVariables.i < localVariables.enemyDeckData.length; localVariables.i++) {
                if (localVariables.enemyDeckData[localVariables.i].health == 0 && localVariables.enemyDeckData[localVariables.i].deathNotProcessed == true) {
                    localVariables.enemyDeckData[localVariables.i].deathNotProcessed = false;
                    deathLog += 1e29 * 10 ** localVariables.i; // marks the index as 1 indicating a death at this position
                }
            }

            // add the death log if anything died, by checking nothing was added to the original death log
            if (deathLog != 3) {
                localVariables.logs[localVariables.currentLogIndex] = deathLog;
                localVariables.currentLogIndex++;

            }

            // early exit if dead
            // TODO this could potentially move into the if (deathLog != 2) block
            if (!isDeckAlive(localVariables.deckData) || !isDeckAlive(localVariables.enemyDeckData)) {
                break;
            }
        ...
        // check the player is the winner
        if (isDeckAlive(localVariables.deckData)) {
            localVariables.result = 0;
            localVariables.logs[localVariables.currentLogIndex] = 1; // game end and a win
            data.userWinStreaks[msg.sender]++;
        }

        // check the enemy is the winner
        else if (isDeckAlive(localVariables.enemyDeckData)) {
            localVariables.result = 1;
            localVariables.logs[localVariables.currentLogIndex] = 1001; // game end and a loss
            delete data.userWinStreaks[msg.sender];
            for (uint8 i = 0; i < 4; i++) {
                for (uint8 j = 0; j < 4; j++) {
                    for (uint256 k = 0; k < data.userHirelingsLists[msg.sender][i][j].length; k++) {
                        data.userHirelingsOwnershipLookup[msg.sender][i][j][data.userHirelingsLists[msg.sender][i][j][k]] = 0;
                    }
                    delete data.userHirelingsLists[msg.sender][i][j];
                }
            }
//            delete data.userHirelingsOwnershipLookup[msg.sender];
        }

        // event for a draw
        else {
            localVariables.result = 2;
            localVariables.logs[localVariables.currentLogIndex] = 2001; // game end and a draw
        }
        ...
        // create BattleData
        data.battleDataMap[battleId] = Battle.BattleData(true, localVariables.result, localVariables.logs);

        delete data.nextBattleConfig[msg.sender];

        return localVariables.result;
    }

    function isDeckAlive(CardData[4] memory deck) internal pure returns (bool) {
        return deck[0].health > 0 || deck[1].health > 0 || deck[2].health > 0 || deck[3].health > 0;
    }
}
...
library PrepareBattleLogic {

    function random(uint add) internal view returns (uint256) {
        // sha3 and now have been deprecated
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, add)));
        // convert hash to integer
        // players is an array of entrants
    }

    function determineAbilityQuantity(uint256 adminDenominatorFor2Abilities, uint256 adminDenominatorFor3Abilities, uint256 winStreak, uint256 rngNumber) internal view returns (uint8) {

        if (winStreak == 0) {
            return 1;
        }

        uint256 roll = rngNumber % 99;
        uint256 ability2Threshold = winStreak * 100 / adminDenominatorFor2Abilities;
        uint256 ability3Threshold = winStreak * 100 / adminDenominatorFor3Abilities;

        if (roll < ability3Threshold) {
            return 3;
        }

        if (roll < ability2Threshold) {
            return 2;
        }
    ...
    function prepareBattle(Battle.ContractData storage data, address finalFormCardsAddress, address mintInfoOracleAddress) external {
        ...
        uint256[4] memory abilityQuantities = MintInfoOracle(mintInfoOracleAddress).getAbilityQuantities();

        // determine the amount of abilities each card has
        uint256 winStreak = data.userWinStreaks[msg.sender];
        uint8[4] memory cardAbilityQuantities = [
            determineAbilityQuantity(data.adminDenominatorFor2Abilities, data.adminDenominatorFor3Abilities, winStreak, random(29)),
            determineAbilityQuantity(data.adminDenominatorFor2Abilities, data.adminDenominatorFor3Abilities, winStreak, random(30)),
            determineAbilityQuantity(data.adminDenominatorFor2Abilities, data.adminDenominatorFor3Abilities, winStreak, random(31)),
            determineAbilityQuantity(data.adminDenominatorFor2Abilities, data.adminDenominatorFor3Abilities, winStreak, random(32))
        ];

        // rely on win streak AND co-efficient
        uint256[4] memory cardIndexes = [
            random(1) % abilityQuantities[cardAbilityQuantities[0]],
            random(2) % abilityQuantities[cardAbilityQuantities[1]],
            random(3) % abilityQuantities[cardAbilityQuantities[2]],
            random(4) % abilityQuantities[cardAbilityQuantities[3]]
        ];

        uint256[4] memory tokenIds = MintInfoOracle(mintInfoOracleAddress).getCardIdByAbilities(
            cardAbilityQuantities,
            cardIndexes
        );

        data.nextBattleConfig[msg.sender].enemyCards[0].isCard = true;
        data.nextBattleConfig[msg.sender].enemyCards[0].id = tokenIds[0];
        data.nextBattleConfig[msg.sender].enemyCards[0].exists = true;
        data.nextBattleConfig[msg.sender].enemyCards[1].exists = true;
        data.nextBattleConfig[msg.sender].enemyCards[2].exists = true;
        data.nextBattleConfig[msg.sender].enemyCards[3].exists = true;

        // Determine how many cards the adversary will have
        if (random(23) % 100 == 0) {
            // remove cards 1-3
            data.nextBattleConfig[msg.sender].enemyCards[1].exists = false;
            data.nextBattleConfig[msg.sender].enemyCards[2].exists = false;
            data.nextBattleConfig[msg.sender].enemyCards[3].exists = false;
        }

        if (data.nextBattleConfig[msg.sender].enemyCards[1].exists && (random(24) % 100 < 5)) {
            data.nextBattleConfig[msg.sender].enemyCards[2].exists = false;
            data.nextBattleConfig[msg.sender].enemyCards[3].exists = false;
        }

        if (data.nextBattleConfig[msg.sender].enemyCards[2].exists && (random(25) % 100 < 10)) {
            data.nextBattleConfig[msg.sender].enemyCards[3].exists = false;
        }

        // Assign values for the cards that exist in adversary deck
        if (data.nextBattleConfig[msg.sender].enemyCards[1].exists) {
            data.nextBattleConfig[msg.sender].enemyCards[1].isCard = true;
            data.nextBattleConfig[msg.sender].enemyCards[1].id = tokenIds[1];
        }

        if (data.nextBattleConfig[msg.sender].enemyCards[2].exists) {
            data.nextBattleConfig[msg.sender].enemyCards[2].isCard = true;
            data.nextBattleConfig[msg.sender].enemyCards[2].id = tokenIds[2];
        }

        if (data.nextBattleConfig[msg.sender].enemyCards[3].exists) {
        ...
        uint8[] memory hirelingRarities = new uint8[](3);
        uint8[] memory hirelingHouses = new uint8[](3);

        // for each card that exists above slot 0 (1-3), if it is a duplicate OR if we roll a 10% change on rng, use a hireling for that card index
        if (
            data.nextBattleConfig[msg.sender].enemyCards[1].exists
            && (
                cardIndexes[0] == cardIndexes[1]
                || random(26) % 10 == 0
            )
        ) {
            hirelingRarities[0] = uint8(random(5) % 4); // 0 - 3
            hirelingHouses[0] = uint8(random(6) % 4); // 0 - 3
            data.nextBattleConfig[msg.sender].enemyCards[1].isCard = false;
            data.nextBattleConfig[msg.sender].enemyCards[1].id = ((random(7) % data.hirelingPool[hirelingRarities[0]][hirelingHouses[0]].length) * 10000) + (uint256(hirelingHouses[0]) * 100) + uint256(hirelingRarities[0]);
        }

        if (
            data.nextBattleConfig[msg.sender].enemyCards[2].exists
            && (
                cardIndexes[0] == cardIndexes[2]
                || cardIndexes[1] == cardIndexes[2]
                || random(27) % 10 == 0
            )
        ) {
            hirelingRarities[1] = uint8(random(8) % 4); // 0 - 3
            hirelingHouses[1] = uint8(random(9) % 4); // 0 - 3
            data.nextBattleConfig[msg.sender].enemyCards[2].isCard = false;
            data.nextBattleConfig[msg.sender].enemyCards[2].id = ((random(10) % data.hirelingPool[hirelingRarities[1]][hirelingHouses[1]].length) * 10000) + (uint256(hirelingHouses[1]) * 100) + uint256(hirelingRarities[1]);
        }

        if (
            data.nextBattleConfig[msg.sender].enemyCards[3].exists
            && (
                cardIndexes[0] == cardIndexes[3]
                || cardIndexes[1] == cardIndexes[3]
                || cardIndexes[2] == cardIndexes[3]
                || random(28) % 10 == 0
            )
        ) {
            hirelingRarities[2] = uint8(random(11) % 4); // 0 - 3
            hirelingHouses[2] = uint8(random(12) % 4); // 0 - 3
            data.nextBattleConfig[msg.sender].enemyCards[3].isCard = false;
            data.nextBattleConfig[msg.sender].enemyCards[3].id = ((random(13) % data.hirelingPool[hirelingRarities[2]][hirelingHouses[2]].length) * 10000) + (uint256(hirelingHouses[2]) * 100) + uint256(hirelingRarities[2]);
        }
...
//            )
//        ) {
//            hirelingRarities[3] = uint8(random(14) % 4); // 0 - 3
//            hirelingHouses[3] = uint8(random(15) % 4); // 0 - 3
//            data.nextBattleConfig[msg.sender].enemyCards[2].isCard = false;
//            data.nextBattleConfig[msg.sender].enemyCards[2].id = ((random(16) % data.hirelingPool[hirelingRarities[3]][hirelingHouses[3]].length) * 10000) + (uint256(hirelingHouses[3]) * 100) + uint256(hirelingRarities[3]);
//        }
...
//            )
//        ) {
//            hirelingRarities[4] = uint8(random(17) % 4); // 0 - 3
//            hirelingHouses[4] = uint8(random(18) % 4); // 0 - 3
//            data.nextBattleConfig[msg.sender].enemyCards[3].isCard = false;
//            data.nextBattleConfig[msg.sender].enemyCards[3].id = ((random(19) % data.hirelingPool[hirelingRarities[4]][hirelingHouses[4]].length) * 10000) + (uint256(hirelingHouses[4]) * 100) + uint256(hirelingRarities[4]);
//        }
...
//            )
//        ) {
//            hirelingRarities[5] = uint8(random(20) % 4); // 0 - 3
//            hirelingHouses[5] = uint8(random(21) % 4); // 0 - 3
//            data.nextBattleConfig[msg.sender].enemyCards[3].isCard = false;
//            data.nextBattleConfig[msg.sender].enemyCards[3].id = ((random(22) % data.hirelingPool[hirelingRarities[5]][hirelingHouses[5]].length) * 10000) + (uint256(hirelingHouses[5]) * 100) + uint256(hirelingRarities[5]);
//        }

        // get the available hirelings and free common hireling houses
        (uint256[4][4] memory availableHirelings, uint8[2] memory freeCommonHouses) = HirelingLogic.generateHirelings(data);
        //		bool[4][4] memory purchasedHirelings;// todo remove probably
