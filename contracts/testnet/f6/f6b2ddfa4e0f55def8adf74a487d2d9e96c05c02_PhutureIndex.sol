...
contract PhutureIndex is PhutureERC20, Owned, ReentrancyGuard, IIndex, IPhutureIndexErrors {
    ...
    function redeem(
        ...
        if (msg.sender != owner) {
            ...
            if (allowed != type(uint256).max) {
                allowance[owner][msg.sender] = allowed - indexShares;
            }
        }

        (uint256 reserveBurnt, uint128 fee, uint256 reserveAssets) =
            _indexAnatomy.redeem{value: msg.value}(_indexShares, receiver, reserveCached, redeemData, state, subIndexes);

        _burn(_indexShares, owner, fee, feePool, reserveBurnt.safeCastTo96());

        // emit Withdraw(msg.sender, receiver, owner, reserveAssets, indexShares);
    ...
    /// @inheritdoc IIndexViewer
    function previewRedeem(uint256 indexShares) external view override returns (uint256) {
        return _indexAnatomy.previewRedeem(uint128(indexShares), state);
    }

    /// @inheritdoc IIndexViewer
    function estimateRedeemFee(bytes calldata data) external view returns (uint256) {
        return _indexAnatomy.estimateRedeemFee(data);
    }
...
interface IIndexAnatomyClient {
    ...
    function redeem(
        ...
        SubIndexLib.SubIndex[] calldata subIndexes
    ) external payable returns (uint256 reserveBurnt, uint128 fee, uint256 reserveAssets);
    ...
    function previewRedeem(uint128 indexShares, IIndexState.State calldata state)
        external
        view
    ...
    /// @notice Returns fee for remote redeem
    function estimateRedeemFee(bytes calldata batches) external view returns (uint256);
}
