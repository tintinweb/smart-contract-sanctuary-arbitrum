    ...
    ERC2771Context
{
    ...
    bytes32 private constant FUND_CHANGE = keccak256("FUND_CHANGE");
    bytes32 private constant FEE_ADDED = keccak256("FEE_ADDED");
    bytes32 private constant SETTLE = keccak256("SETTLE");
    ...
    function changeLiquidityShare(
        uint256 percent
    ) external virtual override hasRole(GOV) {
    ...
    function depositGasLess(
        ...
        uint256 fee = _getFeeExecution(150000, 1);
        oracleFee += fee;
    ...
    function depositGasLessWithPermit(
        ...
        _useNonce(gasLess.owner);
        uint256 fee = _getFeeExecution(200000, 1);
        oracleFee += fee;
        emit FeePaid(bytes32(uint256(0)), 0, 0, fee);
        return _deposit(gasLess.owner, amount, fee);
    }
    ...
    function creditFee(
        ...
        uint256 fee,
        uint256 oracle
    ) external override hasRole(FEE_ADDED) {
        if (fee > 0) {
    ...
    function transferIn(
        address from,
        uint256 amount
    ) external override hasRole(FUND_CHANGE) {
        if (amount == 0) return;
    ...
    function settlePosition(
        ...
        uint256 amount,
        int256 pnl
    ) external override hasRole(SETTLE) {
        if (to == address(0x0) || amount == 0) return;
        usdc.safeTransfer(to, amount);
        if (pnl < 0) {
