...
contract TokenFactory is NonblockingLzApp {
    ...
    mapping(address => mapping(address => mapping(address => uint256))) public payments;
    ...
    function mintToken(
        ...
        if (_chToId[chSym] == _contractChainId) {
            ...
            if (price > 0) {
                processMintPayment(price, msg.sender, asset, coll);
            }
        ...
        if (mintPrice > 0) {
            processMintPayment(mintPrice, msg.sender, asset, coll);
        }

        bytes memory payload = _getPayload(coll, URI, tokenOwner, mintPrice, creator, asset);
    ...
    function lzMint(address coll, string memory URI, address rec, uint256 paid, address cre) external {
    ...
    function _nonblockingLzReceive(
        ...
        (address coll, string memory URI, address rec, uint256 paid, address cre, address asset) = abi.decode(_payload, (address, string, address, uint256, address, address));

        try this.lzMint(coll, URI, rec, paid, cre) {
        } catch {
            IOmniERC721 nft = IOmniERC721(coll);
            refunds[rec][asset] += nft.getMintPrice();
            emit NewRefund(coll, rec);
        }
    }

    function processMintPayment(uint256 price, address spender, address asset, address coll) internal {
        bool isSupported;
        ...
        baseAsset.transferFrom(spender, address(this), price);
        payments[coll][spender][asset] += price;
        earnings[coll][asset] += price;
    }

    function payCreator(address coll) external {
        IOmniERC721 nft = IOmniERC721(coll);
        require(nft.getCreator() == msg.sender);
        require(nft.isClaimable());

        for (uint256 i = 0; i < assets.length; i++) {
            uint256 earn = earnings[coll][assets[i]];
            if (earn > 0) {
                IERC20 erc = IERC20(assets[i]);
                erc.transfer(msg.sender, earn * 98 / 100);
                erc.transfer(_treasury, earn * 2 / 100);
            }
        }
        nft.setClaimed();
    }

    function refund(address coll, address rec, address asset) onlyOwner external {
        IOmniERC721 nft = IOmniERC721(coll);
        require(nft.isClaimable());
        uint256 sum = payments[coll][rec][asset];
        require(sum > 0);
        IERC20(asset).transfer(rec, sum);
        payments[coll][rec][asset] = 0;
        earnings[coll][asset] -= sum;
    }
