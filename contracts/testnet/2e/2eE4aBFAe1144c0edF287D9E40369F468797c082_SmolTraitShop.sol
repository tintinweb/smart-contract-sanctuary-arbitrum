...
// -------------------------------------------------------------
    ...
    external contractsAreSet whenNotPaused {
        ...
        _checkWhitelistStatus(msg.sender, _traitType, _traitId, _proof);
        uint256 price = _checkBeforePurchase(msg.sender, _smolId, _traitType, _traitId);
        userAllocationClaimed[msg.sender][_traitType][_traitId] = true;
        _unlockTrait(msg.sender, price, _smolId, _traitType, _traitId);
    }
...
// -------------------------------------------------------------
    ...
    external contractsAreSet whenNotPaused {
        _equipSet(msg.sender, _smolId, _traitsToEquip);
    }
    ...
    function putOnCostume(
        uint256 _smolId) 
    external contractsAreSet whenNotPaused {
        _checkSmolOwnership(msg.sender, _smolId);
...
// -------------------------------------------------------------
    ...
    // base URI to be concatenated with trait ID + suffix
    mapping (TraitType => string)
        public baseURI;

    // suffix URI to be concatenated with base + trait ID
    mapping (TraitType => string)
        public suffixURI;
...
// -------------------------------------------------------------
    ...
    /// @return background Arrays containing trait id numbers that can be bought for each trait type.
    function getAllTraitsForSale() 
    external view returns (uint256[] memory background, uint256[] memory body, uint256[] memory hair, uint256[] memory clothes, uint256[] memory glasses, uint256[] memory hat, uint256[] memory mouth, uint256[] memory costume) {
        background = traitIdsForSale[TraitType.Background].values();
        body = traitIdsForSale[TraitType.Body].values();
        hair = traitIdsForSale[TraitType.Hair].values();
        clothes = traitIdsForSale[TraitType.Clothes].values();
        glasses = traitIdsForSale[TraitType.Glasses].values();
        hat = traitIdsForSale[TraitType.Hat].values();
        mouth = traitIdsForSale[TraitType.Mouth].values();
        costume = traitIdsForSale[TraitType.Costume].values();
    }
    ...
    /// @notice Get all id numbers for one trait type that is currently owned by selected smol.
    /// @param _smolId Id number of selected Smol Brain token.
    /// @param _traitType Enum(0-7) representing which type of trait is being referenced.
    /// @return Array of trait id numbers that can be bought for a trait type.
    ...
    /// @param _smolId Id number of selected Smol Brain token.
    /// @return background Arrays containing trait id numbers that have been unlocked for smol.
    function getAllTraitsOwnedBySmol(
        uint256 _smolId) 
    external view returns (uint256[] memory background, uint256[] memory body, uint256[] memory hair, uint256[] memory clothes, uint256[] memory glasses, uint256[] memory hat, uint256[] memory mouth, uint256[] memory costume) {
        background = traitIdsOwnedBySmol[_smolId][TraitType.Background].values();
        body = traitIdsOwnedBySmol[_smolId][TraitType.Body].values();
        hair = traitIdsOwnedBySmol[_smolId][TraitType.Hair].values();
        clothes = traitIdsOwnedBySmol[_smolId][TraitType.Clothes].values();
        glasses = traitIdsOwnedBySmol[_smolId][TraitType.Glasses].values();
        hat = traitIdsOwnedBySmol[_smolId][TraitType.Hat].values();
        mouth = traitIdsOwnedBySmol[_smolId][TraitType.Mouth].values();
        costume = traitIdsOwnedBySmol[_smolId][TraitType.Costume].values();
    }
...
// -------------------------------------------------------------
    ...
    function _checkWhitelistStatus(
        ...
        bytes32[] calldata _proof) 
    internal view {
        bytes32 leaf = keccak256(abi.encodePacked(_userAddress, _traitType, _traitId));
