...
import './interfaces/ISNOWSingleTokenPaymentTerminalStore.sol';
...
/**
  ...
  @dev
  Adheres to:
  ISNOWSingleTokenPaymentTerminalStore: General interface for the methods in this contract that interact with the blockchain's state according to the protocol's rules.

  @dev
  Inherits from -
  ReentrancyGuard: Contract module that helps prevent reentrant calls to a function.
*/
contract SNOWSingleTokenPaymentTerminalStore is ReentrancyGuard, ISNOWSingleTokenPaymentTerminalStore {
  ...
  */
  ISNOWPrices public immutable override prices;
  ...
  */
  mapping(ISNOWSingleTokenPaymentTerminal => mapping(uint256 => uint256)) public override balanceOf;
  ...
  */
  mapping(ISNOWSingleTokenPaymentTerminal => mapping(uint256 => mapping(uint256 => uint256)))
    public
  ...
  */
  function currentReclaimableOverflowOf(
    ...
    uint256 _totalSupply = ISNOWController(directory.controllerOf(_projectId))
  ...
  */
  ...
  /**
    ...
    @dev
    The msg.sender must be an ISNOWSingleTokenPaymentTerminal. The amount specified in the params is in terms of the msg.sender's tokens.
  ...
  */
  function recordPaymentFrom(
    address _payer,
    SNOWTokenAmount calldata _amount,
    uint256 _projectId,
    ...
    )
  {
    ...
    if (fundingCycle.useDataSourceForPay()) {
      ...
      SNOWPayParamsData memory _data = SNOWPayParamsData(
        ISNOWSingleTokenPaymentTerminal(msg.sender),
        _payer,
        _amount,
        _projectId,
      ...
      (_weight, memo, delegate) = ISNOWFundingCycleDataSource(fundingCycle.dataSource()).payParams(
        _data
      );
    }
    ...
    balanceOf[ISNOWSingleTokenPaymentTerminal(msg.sender)][_projectId] =
      balanceOf[ISNOWSingleTokenPaymentTerminal(msg.sender)][_projectId] +
      _amount.value;
  ...
  /**
    ...
    @dev
    The msg.sender must be an ISNOWSingleTokenPaymentTerminal. The amount specified in the params is in terms of the msg.senders tokens.
  ...
  */
    ...
    )
  {
    ...
    // Scoped section prevents stack too deep. `_reclaimedTokenAmount`, `_currentOverflow`, and `_totalSupply` only used within scope.
    {
      ...
      SNOWTokenAmount memory _reclaimedTokenAmount;
      ...
      // Another scoped section prevents stack too deep. `_token`, `_decimals`, and `_currency` only used within scope.
      {
        ...
        address _token = ISNOWSingleTokenPaymentTerminal(msg.sender).token();

        // Get a reference to the terminal's decimals.
        uint256 _decimals = ISNOWSingleTokenPaymentTerminal(msg.sender).decimals();

        // Get areference to the terminal's currency.
        uint256 _currency = ISNOWSingleTokenPaymentTerminal(msg.sender).currency();
        ...
        // Get the number of outstanding tokens the project has.
        _totalSupply = ISNOWController(directory.controllerOf(_projectId)).totalOutstandingTokensOf(
          _projectId,
          fundingCycle.reservedRate()
        );
        ...
        _reclaimedTokenAmount = SNOWTokenAmount(_token, reclaimAmount, _decimals, _currency);
      }

      // If the funding cycle has configured a data source, use it to derive a claim amount and memo.
      if (fundingCycle.useDataSourceForRedeem()) {
        // Create the params that'll be sent to the data source.
        SNOWRedeemParamsData memory _data = SNOWRedeemParamsData(
          ISNOWSingleTokenPaymentTerminal(msg.sender),
          _holder,
          _projectId,
          fundingCycle.configuration,
        ...
        (reclaimAmount, memo, delegate) = ISNOWFundingCycleDataSource(fundingCycle.dataSource())
          .redeemParams(_data);
      } else {
        memo = _memo;
      }
    ...
    if (reclaimAmount > balanceOf[ISNOWSingleTokenPaymentTerminal(msg.sender)][_projectId])
  ...
  /**
    ...
    @dev
    The msg.sender must be an ISNOWSingleTokenPaymentTerminal.
  ...
  */
  function recordDistributionFor(
    ...
    override
    nonReentrant
    returns (SNOWFundingCycle memory fundingCycle, uint256 distributedAmount)
  {
    // Get a reference to the project's current funding cycle.
    fundingCycle = fundingCycleStore.currentOf(_projectId);
    ...
    (uint256 _distributionLimitOf, uint256 _distributionLimitCurrencyOf) = ISNOWController(
      directory.controllerOf(_projectId)
    ).distributionLimitOf(
        _projectId,
        fundingCycle.configuration,
        ISNOWSingleTokenPaymentTerminal(msg.sender),
        ISNOWSingleTokenPaymentTerminal(msg.sender).token()
      );
    ...
    uint256 _balanceCurrency = ISNOWSingleTokenPaymentTerminal(msg.sender).currency();
    ...
    if (distributedAmount > balanceOf[ISNOWSingleTokenPaymentTerminal(msg.sender)][_projectId])
      revert INADEQUATE_PAYMENT_TERMINAL_STORE_BALANCE();

    // Store the new amount.
    usedDistributionLimitOf[ISNOWSingleTokenPaymentTerminal(msg.sender)][_projectId][
      fundingCycle.number
  ...
  */
  function recordUsedAllowanceOf(
    ...
    override
    nonReentrant
    returns (SNOWFundingCycle memory fundingCycle, uint256 usedAmount)
  {
    // Get a reference to the project's current funding cycle.
    fundingCycle = fundingCycleStore.currentOf(_projectId);
    ...
    (uint256 _overflowAllowanceOf, uint256 _overflowAllowanceCurrency) = ISNOWController(
      directory.controllerOf(_projectId)
    ).overflowAllowanceOf(
        _projectId,
        fundingCycle.configuration,
    ...
    usedOverflowAllowanceOf[ISNOWSingleTokenPaymentTerminal(msg.sender)][_projectId][
      fundingCycle.configuration
  ...
  */
  ...
  /**
    ...
    @dev
    The msg.sender must be an ISNOWSingleTokenPaymentTerminal. The amount returned is in terms of the msg.senders tokens.
  ...
  */
    ...
    returns (uint256 balance)
  {
    ...
    balance = balanceOf[ISNOWSingleTokenPaymentTerminal(msg.sender)][_projectId];

    // Set the balance to 0.
    balanceOf[ISNOWSingleTokenPaymentTerminal(msg.sender)][_projectId] = 0;
  }
  ...
  */
  function _reclaimableOverflowDuring(
    ...
    if (_redemptionRate == SNOWConstants.MAX_REDEMPTION_RATE) return _base;
    ...
    return
      PRBMath.mulDiv(
        ...
        _redemptionRate +
          PRBMath.mulDiv(
            _tokenCount,
            SNOWConstants.MAX_REDEMPTION_RATE - _redemptionRate,
            _totalSupply
          ),
        SNOWConstants.MAX_REDEMPTION_RATE
      );
  }
  ...
  */
  function _overflowDuring(
    ...
    (uint256 _distributionLimit, uint256 _distributionLimitCurrency) = ISNOWController(
      directory.controllerOf(_projectId)
  ...
  function _currentTotalOverflowOf(
    ...
    return
      (_decimals == 18)
        ...
        : SNOWFixedPointNumber.adjustDecimals(_totalOverflow18Decimal, 18, _decimals);
  }
}
