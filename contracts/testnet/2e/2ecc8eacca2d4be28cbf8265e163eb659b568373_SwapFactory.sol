...
import { SwapPairProxy } from "./SwapPairProxy.sol";
import { SwapPairUpgradeable } from "./SwapPairUpgradeable.sol";
import { ProxyAdmin } from "@openzeppelin/contracts/proxy/transparent/ProxyAdmin.sol";
import { IToken } from "./interfaces/IToken.sol";


contract SwapFactory is ISwapFactory, ReentrancyGuard, ProxyAdmin {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(SwapPairProxy).creationCode));
    address PAIR_LOGIC;
    ...
    constructor(address _feeTo) {
        feeTo = _feeTo;
        PAIR_LOGIC = address(new SwapPairUpgradeable());
    }
    ...
    function createPair(
        ...
        bytes memory bytecode = abi.encodePacked(
            type(SwapPairProxy).creationCode,
            abi.encode(PAIR_LOGIC, address(this), token0, token1, supportsTokenFee, tokenB == token0)
        );
...
import { SwapERC20Upgradeable } from "./SwapERC20Upgradeable.sol";
...
contract SwapPairUpgradeable is ISwapPair,Initializable, ContextUpgradeable, SwapERC20Upgradeable, ReentrancyGuardUpgradeable {
    ...
    event Initialize(address token0, address token1, ISwapRouter router, address caller);
    ...
    function initialize(address _token0, address _token1, bool _supportsTokenFee, bool _token0IsFee) internal onlyInitializing() {
    ...
    function skim(address to) external override lock {
        address _token0 = token0; // gas savings
        address _token1 = token1; // gas savings
        _safeTransfer(_token0, to, IERC20(_token0).balanceOf(address(this)) - reserve0, false);
        _safeTransfer(_token1, to, IERC20(_token1).balanceOf(address(this)) - reserve1, false);
    }
    ...
    function setBaseToken(address _baseToken) external override {
        require(msg.sender == factory, "Swap: NOT_FACTORY");
...
contract SwapPairProxy is TransparentUpgradeableProxy {
    constructor (address logic, address admin, address _token0, address _token1, bool _supportsTokenFee, bool _token0IsFee) TransparentUpgradeableProxy(logic, admin, generateData(_token0, _token1, _supportsTokenFee, _token0IsFee)) {}
        function generateData(address _token0, address _token1, bool _supportsTokenFee, bool _token0IsFee) internal pure returns (bytes memory data) {
        data = abi.encodeWithSignature("initialize(address,address,bool,bool)", _token0, _token1, _supportsTokenFee, _token0IsFee);
    }
}
...
import { ISwapERC20 } from "./interfaces/ISwapERC20.sol";
import { ECDSA } from "./libraries/ECDSA.sol";
import {Initializable} from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
contract SwapERC20Upgradeable is ISwapERC20, Initializable {
