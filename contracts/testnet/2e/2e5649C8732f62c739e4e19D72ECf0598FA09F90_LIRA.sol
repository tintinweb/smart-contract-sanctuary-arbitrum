...
contract LIRA is ERC20("Satoshi LIRA", "LIRA"), Ownable {
    ...
    function mint(uint256 amount) public onlyOwner {
        if (totalSupply() == 0) {
            _mint(_msgSender(), amount * 10 ** 8);
        } else {
            uint256 quoteAmount = uint(quote(amount, totalSupply(), wbtc.balanceOf(address(this))));

            _mint(_msgSender(), quoteAmount * 10 ** 8);
        }

        wbtc.safeTransferFrom(_msgSender(), address(this), amount);
    }
    ...
    function burn(uint256 amount) public onlyOwner {
        ...
        wbtc.safeTransferFrom(address(this), _msgSender(), uint(quote(amount, wbtc.balanceOf(address(this)), totalSupply())));
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) public pure returns (int amountB) {
        require(amountA > 0, 'LIRA: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'LIRA: INSUFFICIENT_LIQUIDITY');

        //  ax(amountLiraIN) : ay(liraTotalSupply) = bx(amountWbtcOUT) : (wbtcBalanceOfLiraContract)
        amountB = (int(amountA) * int(reserveB)) / int(reserveA);
    }
