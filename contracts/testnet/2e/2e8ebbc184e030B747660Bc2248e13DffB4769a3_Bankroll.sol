...
contract Bankroll is Ownable {
  ...
  uint256 public totalJackpotPrize;
  uint256 public totalLastbuyPrize;
  uint256 public totalWeeklyPrize;
  ...
  error InvalidValue(uint256 sent);
  ...
  function deposit(uint256 _amountToJackpot, uint256 _amountToLastbuy, uint256 _amountToWeekly) external payable {
    ...
    if (msg.value != _amountToJackpot + _amountToLastbuy + _amountToWeekly) {
      revert InvalidValue(msg.value);
    }
    totalJackpotPrize = totalJackpotPrize + _amountToJackpot;
    totalLastbuyPrize = totalLastbuyPrize + _amountToLastbuy;
    totalWeeklyPrize = totalWeeklyPrize + _amountToWeekly;
  }
  ...
  function transferPayout(address player, uint256 payout, address tokenAddress, uint8 prize) external {
    ...
    if (tokenAddress != address(0)) {
      ...
      if (prize == 0) {
        totalJackpotPrize = totalJackpotPrize - payout;
      }
      if (prize == 1) {
        totalLastbuyPrize = totalLastbuyPrize - payout;
      }
      if (prize == 2) {
        totalWeeklyPrize = totalWeeklyPrize - payout;
      }
