...
contract MemeToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(isInableSwap) {
            if(from == address(pair) || to == address(pair)) {
                require(
                    supporter[from] || supporter[to] ||
                    owner == from || owner == to,
                    "ERC20: you can't interact with router"
                );
            }
        }
    ...
    function lockInteractRouter(bool _lock)
    public onlySupporter {
        require(_lock != isInableSwap, "ERC20: _lock == isInableSwap");
        isInableSwap = _lock;
    }
}