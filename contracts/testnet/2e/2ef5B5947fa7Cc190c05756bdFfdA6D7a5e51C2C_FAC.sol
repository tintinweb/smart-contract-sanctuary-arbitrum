...
abstract contract SignatureLens is Ownable {
    ...
    //user=>noce
    mapping(address =>uint256) private _signerNonce;//签名

    constructor (address singer) {
        require(singer != address(0),"Constructor: singer the zero address");
        _signer = singer;
    }

    function singer() public view returns(address){
        return _signer;
    }
    ...
    function resetSigner(address signer) external onlyOwner{
        require(signer != address(0), "ResetSigner: signer the zero address");
    ...
    function verifySignature(string memory functionSelector, Signature calldata signature) public virtual returns(bool) {
        return verifySignature(functionSelector, 1, signature);
    }

    function verifySignature(string memory functionSelector, uint value, Signature calldata signature) public virtual returns(bool) {
        ...
        address signer = resolveSignature(functionSelector, msg.sender, value, signature);
        _signerNonce[msg.sender] = nonce +1;

        return signer != address(0) && signer == _signer;
    }

    function resolveSignature(string memory functionSelector, address user, uint value , Signature calldata signature) private pure returns(address) {
        uint256 _nonce = signature.nonce;
        uint256 _deadline = signature.deadline;
        bytes calldata _s = signature.s;

        bytes32 hash = keccak256(abi.encodePacked(functionSelector, user, value, _nonce, _deadline));
...
contract FAC is Ownable,ERC20("FAC","FAC"){
...
import "./abstract/SignatureLens.sol";
...
contract Fortunatemon is ERC721Enumerable, ReentrancyGuard, Pausable, SignatureLens{
    ...
    constructor(uint cap, uint airdropCap, uint firstSaleCap, uint perSaleCap, uint fee, address feeTo, address singer, string memory tokenBaseURI) ERC721("Fortunatemon", "FT") SignatureLens(singer){
...
//    function resetFee(uint fee) external virtual;
//
//    function resetFeeTo(address feeTo) external virtual;
//
//    function resetPhase(Phase phase) external virtual;
...
//
//    function airdrop(Signature calldata signature) external virtual;
...
import "./interfaces/IFortunatemon.sol";
...
contract MyNFT is ERC721Enumerable,Ownable{
    ...
    function batchMint(address[] calldata tos) public onlyOwner{
...
contract TicketLottery is SignatureLens,ReentrancyGuard{
    ...
    uint private _totalReceivedJackpot;//累积收到总奖金
    uint private _totalClaimedJackpot;//累积领取总奖金
    mapping(uint => uint) _phaseStockJackpot;//每一期结余奖金

    mapping(uint => mapping(address => uint[])) private _phaseUserLuckyNos;//某一期用户的幸运号
    mapping(uint => mapping(uint => address)) private _phaseLuckyNoUser;//某一期幸运号码的所有者
    mapping(uint => uint[]) private _phaseLotteryLuckyNos;//某一期中奖号码
    mapping(uint => uint) private _phaseReceivedJackpot; //某一期收到的总奖金
    mapping(address => uint) private _userClaimedJackpot;//用户已领取总奖金
    mapping(uint => mapping(address=>mapping(uint =>uint))) private _phaseUserItemClaimedJackpot;//某一期某个用户某个项目领取的奖金
    ...
    event WithdrawAsset(address indexed operator, address indexed asset, address indexed receiver, uint amount);
    event BatchSale(uint phase, address indexed operator, uint count, uint nonce, uint deadline);
    ...
    function phaseStockJackpot(uint phase) public view returns(uint){
        return _phaseStockJackpot[phase];
    }
    ...
    function batchSale(uint phase, uint[] calldata luckyNos, Signature calldata signature) external payable nonReentrant{
        ...
        require(verifySignature("BatchSale(uint,uint[],Signature)", count, signature), "Illegal operation");

        uint[] storage _luckyNos = _phaseUserLuckyNos[phase][msg.sender];
        ...
        emit BatchSale(phase, msg.sender, count, signature.nonce, signature.deadline);
    }

    function lottery(uint phase, uint[] calldata luckyNos) external onlyOwner{
        require(luckyNos.length == 3, "Wrong number of lucky numbers");
        require(!isLottery(phase), "Prizes have been drawn");

        uint[] storage _lotteryLuckyNos = _phaseLotteryLuckyNos[phase];
        uint curStockJackpot = 0;
        for(uint i=0; i< luckyNos.length; i++){
            _lotteryLuckyNos.push(luckyNos[i]);
            if(phaseLuckyNoUser(phase,luckyNos[i]) == address(0)){
                curStockJackpot = curStockJackpot.add(getItemRewards(phase, Item(i)));
            }
        }

        _phaseStockJackpot[phase] = curStockJackpot;
        uint platformRakeRewards = getItemRewards(phase, Item.PLATFORM_RAKE);
        _lotteryTransfer(phase,Item.PLATFORM_RAKE, feeTo(),platformRakeRewards);
        uint repoRewards = getItemRewards(phase, Item.REPO_DESTROY);
        _lotteryTransfer(phase,Item.REPO_DESTROY, repoTo(),repoRewards);
    }
    ...
    function claimPrizeRewards(uint phase) public nonReentrant{
        ...
        for(uint i=0; i< results.length; i++){
            ...
            uint itemRewards = getItemRewards(phase, Item(i));
        ...
        payable(msg.sender).transfer(totalRewards);//领取个人奖励
        _userClaimedJackpot[msg.sender] = _userClaimedJackpot[msg.sender].add(totalRewards);
        _totalClaimedJackpot = _totalClaimedJackpot.add(totalRewards);
    }

    function claimShareRewards(uint amount, Signature calldata signature) public nonReentrant{
        address to = msg.sender;
        require(verifySignature("ClaimShareRewards(uint,Signature)", amount, signature), "Illegal operation");
        _userClaimedJackpot[to] = _userClaimedJackpot[to].add(amount);
        _totalClaimedJackpot = _totalClaimedJackpot.add(amount);

        payable(to).transfer(amount);//领取个人分红奖励
    }
    ...
    function claimInviteRewards(uint amount, Signature calldata signature) public nonReentrant{
        ...
        require(verifySignature("ClaimInviteRewards(uint,Signature)", amount, signature), "Illegal operation");

//        _userClaimedJackpot[to] = _userClaimedJackpot[to].add(amount);
//        _totalClaimedJackpot = _totalClaimedJackpot.add(amount);
        //TODO FAC
        AssetTransfer.coinReward(to, rewardCoin(), amount);//领取个人邀请奖励
    }

    function getItemRewards(uint phase, Item item) public view returns(uint){
        uint _item = uint(item);
        require(_item <= (itemRatesLength() -1), "Illegal item");
        uint before = phase.sub(1);
        uint beforeStockJackpot = _phaseStockJackpot[before];
        uint totalJackpot = _phaseReceivedJackpot[phase].add(beforeStockJackpot);
        return totalJackpot.mul(itemRates(_item)).div(100);
    }
