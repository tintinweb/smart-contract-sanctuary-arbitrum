...
contract OperatorFilteredToken is LockableRevealERC721EnumerableToken {
    ...
    function init(TokenConstructorConfig memory config, address _actualOwner) public virtual override  {

        if (address(OPERATOR_FILTER_REGISTRY).code.length > 0) {
            OPERATOR_FILTER_REGISTRY.registerAndSubscribe(address(this), DEFAULT_SUBSCRIPTION);
        }
        super.init(config,_actualOwner);
    }
...
contract LockableRevealERC721EnumerableToken is IToken, ERC721Enumerable, Ownable, BlackHolePrevention {
    ...
    constructor() ERC721("GOLDEN_TOKEN_CONTRACT","GT"){
        //_initialized = true;
    }


    IRegistryConsumer   constant    public TheRegistry = IRegistryConsumer(0x1e8150050A7a4715aad42b905C08df76883f396F);
    ...
    function init(TokenConstructorConfig memory config, address _actualOwner ) public virtual  {
        require(!_initialized, "Token: Contract already initialized");
        _name = config.erc721name;
        _symbol = config.erc721symbol;
        ...
        chain = id.toString();
        _transferOwnership(_actualOwner);
        _initialized = true;
    }
    ...
    function revealAtCurrentSupply() external onlyAllowed(TOKEN_CONTRACT_ACCESS_REVEAL) {
        ...
        //
        console.log(mintedSupply , reservedSupply , maxSupply);
    ...
    function process(uint256 _random, uint256 _requestId) external {
        require(VRFShifting, "Token: VRF Shifting must be enabled");
        require(
            msg.sender == address(getRandomSource()), 
            "Token: process() Unauthorised caller"
        );
