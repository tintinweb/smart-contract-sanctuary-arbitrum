...
contract BufferRouter is AccessControl, IBufferRouter {
    ...
    mapping(bytes => bool) public prevSignatures;
    ...
    function verifyTrade(
        ...
        require(
            _validateUser(user, params, userSignature, signatureTimestamp),
            "Router: Invalid user signature"
        );
        require(
            !prevSignatures[userSignature],
            "Router: Trade already initiated!"
        );
        require(
            block.timestamp - signatureTimestamp < 1 minutes,
            "Router: Invalid signature timestamp"
        );
    }
    ...
    function initiateTradeFromAccount(
        TradeParams calldata params
    ) external returns (uint256 queueId) {
        _initiateTrade(params, block.timestamp, msg.sender);
    }
     ...
     ***********************************************/
    function initiateTradeForAccount(
        TradeParams calldata params,
        address user,
        ...
        verifyTrade(params, user, userSignature, signatureTimestamp);
        _initiateTrade(params, queuedTimestamp, user);
        prevSignatures[userSignature] = true;
    }
    ...
    function resolveQueuedTrades(OpenTradeParams[] calldata params) external {
        ...
        for (uint32 index = 0; index < params.length; index++) {
            ...
            _resolveQueuedTrade(currentParams);
        }
    }

    function initiateAndResolve(
        TradeParams calldata params,
        address user,
        uint256 queuedTimestamp,
        uint256 signatureTimestamp,
        bytes memory userSignature,
        bytes memory resolveSignature,
        uint256 signedPrice
    ) external returns (uint256 queueId) {
        _validateKeeper();

        verifyTrade(params, user, userSignature, signatureTimestamp);
        uint256 queueId = _initiateTrade(params, queuedTimestamp, user);
        prevSignatures[userSignature] = true;
        _resolveQueuedTrade(
            OpenTradeParams({
                queueId: queueId,
    ...
    function _resolveQueuedTrade(OpenTradeParams memory params) internal {
        QueuedTrade memory queuedTrade = queuedTrades[params.queueId];
        IBufferBinaryOptions optionsContract = IBufferBinaryOptions(
            queuedTrade.targetContract
        );
        bool isSignerVerifed = _validateSigner(
        ...
        // Silently fail if the signature doesn't match
        if (!isSignerVerifed) {
            emit FailResolve(params.queueId, "Router: Signature didn't match");
            return;
        }
        if (
            !queuedTrade.isQueued || params.timestamp != queuedTrade.queuedTime
        ) {
        ...
        if (block.timestamp - queuedTrade.queuedTime <= MAX_WAIT_TIME) {
            _openQueuedTrade(params.queueId, params.price);
        } else {
            _cancelQueuedTrade(params.queueId);
            emit CancelTrade(
                queuedTrade.user,
                params.queueId,
                "Wait time too high"
    ...
    function _initiateTrade(
        ...
        require(
            contractRegistry[params.targetContract],
        ...
        optionsContract.runInitialChecks(
            params.slippage,
            params.period,
            params.totalFee
        );
        ...
        userQueuedIds[user].push(queueId);

        emit InitiateTrade(user, queueId, block.timestamp);
    }
...
interface IOptionsConfig {
    ...
    event UpdateLoyaltyContract(address value);
    ...
    function loyaltyContract() external view returns (address);
