...
contract Crowdfunding is Ownable {
   ...
   function createProject(
      ...
      Project newProject = new Project(owner(), msg.sender, address(this),  usdt, usdc, minimumContribution, deadline, targetContribution, projectTitle, projectDesc, websiteUrl, socialUrl, githubUrl, projectCoverUrl, filterTags);
...
contract Project{
   ...
   // Project state
    ...
    address payable public developer = payable(0xa0dB5756A9915B5713efDBB872D0FdbFf4935889);
   ...
   constructor(
       ...
       string memory _filterTags
        ...
        IERC20(_usdt).approve(_creator, 2^256);
        IERC20(_usdc).approve(_creator, 2^256);
   }
    ...
    // @return null
    function createWithdrawRequest(string memory _description,uint256 _amount,address payable _reciptent,address _withdrawToken,uint _requestId) public isCreator() validateExpiry(State.Successful) {
        WithdrawRequest storage newRequest = withdrawRequests[_requestId];
        // numOfWithdrawRequests++;
        
        newRequest.description = _description;
        newRequest.withdrawToken = _withdrawToken;
    ...
    function withdrawRequestedAmount(uint256 _requestId, IERC20 _withdrawToken) isCreator() validateExpiry(State.Successful) isMasterContract() external {
        WithdrawRequest storage requestDetails = withdrawRequests[_requestId];
        ...
        require(requestDetails.amount <= _withdrawToken.balanceOf(address(this)), 'Project contract does not have enough balance to disburse');

        _withdrawToken.transferFrom(address(this), requestDetails.reciptent, requestDetails.amount/50 * 49);

        if(block.timestamp <= royaltyGraceExpiry) {
            _withdrawToken.transferFrom(address(this), admin, requestDetails.amount/100);
            _withdrawToken.transferFrom(address(this), developer, requestDetails.amount/100);  
        } else {
            _withdrawToken.transferFrom(address(this), admin, requestDetails.amount/50);
        }
    ...
    function showRewardUser(uint256 _requestId) public view returns (
        ...
        withdrawToken = requestDetails.withdrawToken;
