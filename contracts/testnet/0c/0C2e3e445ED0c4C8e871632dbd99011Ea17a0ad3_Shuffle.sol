...
contract Shuffle is IShuffle, Ownable {
    ...
    uint256 public constant ALLOWED_PROOF_SUBMISSION_DELAY = 40;
    ...
    // Queries card deal records.
    function batchQueryCardDealRecord(uint256[] memory indexes, uint256 gameId)
        external
        view
        inDealingPhase(gameId)
        returns (uint256[] memory results)
    {
        results = new uint256[](indexes.length);
    ...
    function search(uint256 cardIndex, uint256 gameId)
        ...
        returns (uint256)
    {
        if (cardDeals[gameId].record[cardIndex] != (1 << numPlayers[gameId]) - 1) {
    ...
    function batchSearch(uint256[] calldata cardIndexes, uint256 gameId) external view returns (uint256[] memory cards) {
