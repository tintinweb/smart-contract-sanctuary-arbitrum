...
contract Exchange is Ownable {
    ...
    function newSellOrder(
        ...
        currentFee = adjustDecimals(currentFee, tokenA_decimals);
        ...
        for (uint8 i = 0; i < len; i++) {
            ...
            if (sellAmount == 0) {
                ...
                IERC20(tokenA).transfer(o.seller, (o.amount / price) * 100);
                ...
                IERC20(tokenB).transfer(msg.sender, (price * sellAmount) / 100);
    ...
    function newBuyOrder(
        ...
        // no fee under 1000
        deposit(tokenB, adjustDecimals((price * buyAmount) / 100, tokenB_decimals));
        uint256 currentFee = (price * (buyAmount * (1000-feeRate)) / 1000) / 100;
        currentFee = adjustDecimals(currentFee, tokenB_decimals);
        ...
        for (uint8 i = 0; i < len; i++) {
            ...
            if (buyAmount == 0) {
                ...
                IERC20(tokenB).transfer(o.seller, (price * o.amount) / 100);
                buyAmount -= o.amount;
                ...
                IERC20(tokenB).transfer(o.seller, (price * buyAmount) / 100);
                buyAmount = 0;
            }
        }
        // new buy order
        if (orderBook[tokenB][price].length == 0 && buyAmount > 0) {
            bytes32 orderId = LinkedListLib.initHead(
                ...
                msg.sender,
                adjustDecimals((price * buyAmount) / 100, tokenB_decimals)
            );
            OPVSetLib._add(
                _buyOrders,
                msg.sender,
            ...
            PVNodeLib._addVolume(buyOB, priceIdx, adjustDecimals((price * buyAmount) / 100, tokenB_decimals));
        } else if (buyAmount > 0) {
            bytes32 orderId = LinkedListLib.addNode(
                orderBook[tokenB][price],
                msg.sender,
    ...
    function adjustDecimals(uint256 amount, uint8 decimals) private pure returns (uint256) {
        if (decimals > 18) {
