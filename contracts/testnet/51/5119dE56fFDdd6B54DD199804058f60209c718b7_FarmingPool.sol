...
/**
 * @title FarmingPool
 * @dev A smart contract for staking and claiming rewards from multiple assets.
 */
contract FarmingPool is Ownable {
    ...
    modifier updateState(uint256 amount, bool staking) {
        ...
        for (uint i; i < length;) {
            ...
            if (staking) {
                ...
                //to prevent infinity increasing of claimed variable
    ...
    function changeRewards(address _asset, uint256 _tokensPerDay) external onlyOwner {
    ...
    function stakeTokens(uint256 amount) external updateState(amount, true) {
    ...
    function unstakeTokens(uint256 amount) external updateState(amount, false) {
        ...
        for (uint i; i < length;) {
            ...
            uint256 earnedReward = (amount * (rate - stake.rate)) /
            MULTIPLICATOR;
            stake.unclaimed += earnedReward;
    ...
    /**
     * @dev Claim rewards for the user from all staked assets.
     */
    function claim() external {
        uint256 totalStaked = userTotalStaked[msg.sender];
        uint256 length = assets.length;
        require(
            lastClaim[msg.sender] < getClaimCycleTime(),
    ...
    /**
     * @dev Get the timestamp when the next claim cycle starts.
     * @return The timestamp of the next claim cycle.
     */
     ...
     */
    function getCurrentRate(address asset) public view returns (uint256) {
