...
contract ARBI_EBorrow  is UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    uint constant DECIMAL_OFFSET = 1e12;
...
// End test
    ...
    function borrow(uint borrowAmount) public payable nonReentrant(){
        (uint mintable, bool canMint) = IUsgEngine(engine).getMintableUSG(baseAsset, msg.sender, borrowAmount);
        ...
        require(ERC20(baseAsset).transfer(msg.sender, borrowAmount), "transfer token fail");
    }

    function borrow2(uint borrowAmount) public payable nonReentrant(){
        (uint mintable, bool canMint) = IUsgEngine(engine).getMintableUSG(baseAsset, msg.sender, borrowAmount);
        require(canMint, 'user cant mint more usg');
        ...
        uint convertedBorrowAmount = borrowAmount.mul(DECIMAL_OFFSET);
        if(mintable > convertedBorrowAmount) {
            mintable = convertedBorrowAmount;
        } else {
            mintable = mintable.mul(99).div(100);
        }
        IUsgEngine(engine).depositCollateralAndMintUsg{value:0}(baseAsset, borrowAmount, mintable);
    }

    function borrow3(uint borrowAmount) public payable nonReentrant(){
        (uint mintable, bool canMint) = IUsgEngine(engine).getMintableUSG(baseAsset, msg.sender, borrowAmount);
        require(canMint, 'user cant mint more usg');
        ...
        IUsgEngine(engine).depositCollateralAndMintUsg(baseAsset, borrowAmount, mintable);
        require(ERC20(usg).transfer(msg.sender, mintable), "transfer token fail");
    }
    function getMintable(uint borrowAmount) public view returns (uint) {
         (uint mintable, bool canMint) = IUsgEngine(engine).getMintableUSG(baseAsset, msg.sender, borrowAmount);
          uint convertedBorrowAmount = borrowAmount.mul(DECIMAL_OFFSET);
        if(mintable > convertedBorrowAmount) {
            mintable = convertedBorrowAmount;
        } else {
            mintable = mintable*99/100;
        }
        return mintable;
    }
    function test(uint borrowAmount) public{
         (uint mintable, bool canMint) = IUsgEngine(engine).getMintableUSG(baseAsset, msg.sender, borrowAmount);
          uint convertedBorrowAmount = borrowAmount.mul(DECIMAL_OFFSET);
        if(mintable > convertedBorrowAmount) {
            mintable = convertedBorrowAmount;
    ...
    function repay(uint borrowAmount) public nonReentrant(){
        uint burnable = IUsgEngine(engine).getBurnableUSG(baseAsset, msg.sender, borrowAmount.mul(DECIMAL_OFFSET));
