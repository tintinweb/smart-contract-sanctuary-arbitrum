...
contract MetaVerseDAOStaking is Ownable, ERC1155Receiver {
    ...
    function deposit(uint256 collectionId, uint256 _amount) internal {
        ...
        if (previousDeposit > 0) {
            ...
            //_userInfo[msg.sender][collectionId] = UserInfo(_amount,block.timestamp,0,block.timestamp+)
    ...
    function checkReward(uint256 collectionId, uint256 amount) public {}

    function checkPercentage(uint256 fatiqueTime, uint256 stakedTime) public view returns(uint256) {
        if (block.timestamp < stakedTime) {
            uint256 percentage = (stakedTime - fatiqueTime) / FATIQUE_TIME;
            return percentage;
        }
        return 0;
    }
