...
contract DistributionContract {
    ...
    receive() external payable {
      ...
      function distributeETH() private {
        ...
        payable(teamAddress).transfer(teamShare); // Directly transfer to team
        totalEthDistributed += teamShare; // Update total eth distributed
    ...
    uint256 value = address(this).balance; // Réactualiser la balance après avoir envoyé à l'équipe
    for (uint i = 0; i < holdersCount; i++) {
        address holder = arbotToken.holderAtIndex(i);
        if (!excludedAddresses[holder]) {
...
function distributeArbotTokens() private {
 ...
 // The claim function will also change, as we no longer need to claim for team address
    function claim() external {
        require(msg.sender != teamAddress, "Team cannot claim"); // The team is not allowed to claim
        ...
        if (ethAmount > 0) {
            ...
            totalEthDistributed += ethAmount; // Update total eth distributed
        }
        if (arbotAmount > 0) {
            arbotClaimable[msg.sender] = 0;
            arbotToken.transfer(msg.sender, arbotAmount);
            totalArbotDistributed += arbotAmount; // Update total arbot distributed
        }
    }
