...
contract OrderManager is IUniswapV3MintCallback, IOrderManager, OddzOwnable {
    ...
    /// @param baseAmount         number of base token added
    /// @param quoteAmount        number of quote token added
    /// @param collateralForOrder collateral allocated for this order
    /// @param globalFundingGrowth Globakl funding variables 
    ...
    address internal oddzClearingHouse;
    address internal oddzMarket;
    address internal swapManager;
    address internal oddzConfig;
    address internal oddzVault;
    address internal oddzClearingHouseExtended;
    ...
    mapping(address => uint256) internal feeGrowthGlobal;

    // first key: base token, second key: tick index
    // value: the accumulator of **Tick.GrowthInfo** outside each tick of each pool
    mapping(address => mapping(int24 => Tick.GrowthInfo)) internal growthOutsideTick;
    ...
    modifier onlyClearingHouse() {
        require(
            ...
            //OddzOrderManager: Only clearhouse & clearHouse extended allowed
            "OM:OCA"
        );
        _;
    }
    ...
    function initialize(address _oddzMarket, address _oddzConfig,address _oddzVault) external initializer {
        require(
            _oddzMarket.isContract(),
            //OddzOrderManager:market address not a contract
            //market not contract
            "OM:MNC"
        );
        require(
            _oddzConfig.isContract(),
            //OddzOrderManager:oddz config address not a contract
            //config not contract
            "OM:CNC"
        );
        require(
            _oddzVault.isContract(),
            //OddzOrderManager: oddz vault should be a contract
            //vault not contract
            "OM:VNC"
        );
        __OddzOwnable_init();
    ...
    /**
     * @notice Used to update clearing house extended contract address
    ...
    function updateClearingHouseExtended(address _clearingHouseExtended) external onlyOwner {
        require(
            ...
            //OddzOrderManager: clearing house should be a contract
            //clearing house extended not contract
            "OM: CNEC"
        );
     ...
     * returns base, quote, liquidity, fee, orderId
     */
    function addLiquidity(AddLiquidityParams calldata _params)
        external
        override
        onlyClearingHouse
        returns (
    ...
    {
        address pool = IOddzMarket(oddzMarket).fetchPool(_params.baseToken);
        uint256 feeGrowthGlobalX128 = feeGrowthGlobal[_params.baseToken];
         mapping(int24 => Tick.GrowthInfo) storage tickMap = growthOutsideTick[_params.baseToken];
        
        bool initializedBeforeLower = UniswapInteractor.getIsTickInitialized(pool, _params.lowerTickOfOrder);
        bool initializedBeforeUpper = UniswapInteractor.getIsTickInitialized(pool, _params.upperTickOfOrder);
        ...
        UniswapInteractor.AddLiquidityResponse
            memory response = UniswapInteractor.addLiquidity(
                UniswapInteractor.AddLiquidityParams(
                    pool,
                    _params.lowerTickOfOrder,
                    _params.upperTickOfOrder,
                    _params.baseAmount,
                    _params.quoteAmount,
                    abi.encode(MintCallbackData(_params.trader, pool))
                )
            );
        
        (, int24 currentTick, , , , , ) = UniswapInteractor.fetchSlot0(pool);

        // initialize tick info
        if (!initializedBeforeLower && UniswapInteractor.getIsTickInitialized(pool, _params.lowerTickOfOrder)) {
            tickMap.initialize(
                _params.lowerTickOfOrder,
                currentTick,
                Tick.GrowthInfo(
                    feeGrowthGlobalX128,
                    _params.twPremiumX96,
                    _params.twPremiumDivBySqrtPriceX96
                )
            );
        }
        if (!initializedBeforeUpper && UniswapInteractor.getIsTickInitialized(pool, _params.upperTickOfOrder)) {
            tickMap.initialize(
                _params.upperTickOfOrder,
                currentTick,
                Tick.GrowthInfo(
        ...
        Funding.Growth memory growth=Funding.Growth({
            twPremiumX96:_params.twPremiumX96,
            twPremiumDivBySqrtPriceX96:_params.twPremiumDivBySqrtPriceX96
        });
        ...
        return _addLiquidity(
            InternalAddLiquidityParams({
                    ...
                    poolAddress: pool,
                    lowerTickOfOrder: _params.lowerTickOfOrder,
                    upperTickOfOrder: _params.upperTickOfOrder,
                    liquidity: response.liquidity,
                    baseAmount: response.base,
                    quoteAmount: response.quote,
                    collateralForOrder:_params.collateralForOrder,
                    globalFundingGrowth: growth
                })
        );
     ...
     * @dev Called by the clearing house contract
     * Also update the order details for the trader
     * returns base, quote, takerBase, takerQuote, orderId
     */
    function removeLiquidity(RemoveLiquidityParams calldata _params)
        external
        override
        onlyClearingHouse
        returns (RemoveLiquidityResponse memory _response)
    {
        ...
        (uint256 fee, uint256 feeGrowthInside) =
            _fetchPendingFeeAndFeeGrowthInsideByOrder(
                _params.baseToken,
                openOrder
            );
        ...
        if (_params.liquidityAmount != 0) {
            ...
            openOrder.liquidity = openOrder
                .liquidity
                .sub(_params.liquidityAmount)
                .toUint128();
        }

        openOrder.owedRealizedPnl =openOrder.owedRealizedPnl.add(fee.toInt256());
        ...
        if (!UniswapInteractor.getIsTickInitialized(pool, _params.lowerTickOfOrder)) {
            growthOutsideTick[_params.baseToken].clear(_params.lowerTickOfOrder);
        }
        if (!UniswapInteractor.getIsTickInitialized(pool, _params.upperTickOfOrder)) {
            growthOutsideTick[_params.baseToken].clear(_params.upperTickOfOrder);
        }

        //update Order Information if the liquidity is zero
        if (openOrder.liquidity == 0) {
            totalCollateralForOrders[_params.trader] = totalCollateralForOrders[
                _params.trader
            ].sub(openOrder.collateralForOrder);
            bytes32[] storage orderIds = currentOrderIdsMap[_params.trader][
                _params.baseToken
            ];

            IOddzVault(oddzVault).updateCollateralBalance(
                         _params.trader,
                        openOrder.owedRealizedPnl
                    );
           ...
           openOrder.lastFeeGrowthInside = feeGrowthInside;  
        }

        // consider this code when we complete funding payment implementation
        // if flipped from initialized to uninitialized, clear the tick info
        if (!UniswapInteractor.getIsTickInitialized(pool, _params.lowerTickOfOrder)) {
            growthOutsideTick[_params.baseToken].clear(_params.lowerTickOfOrder);
        }
    ...
    */
    function updateLiquidityPositionOwedRealizedPnl(bytes32 _orderId, int256 _amount)
        external
        override
        onlyClearingHouse{
            if(_amount!=0){
                  currentOrderMap[_orderId].owedRealizedPnl =  currentOrderMap[_orderId].owedRealizedPnl.add(_amount);
                  emit PnlRealized(_orderId, _amount);
            }
    ...
    * @param _orderId order id
    * @param _twPremiumX96 updated time weighted premium
    * @param _twPremiumDivBySqrtPriceX96 updated time weighted premium div by sqrt price
    * @return _liquidityCoefficientInFundingPayment liquidity coefficient value in funding payment
     */
    function updateFundingGrowthAndLiquidityCoefficientInFundingPayment(
    ...
    )external override onlySwapManager returns (int256 _liquidityCoefficientInFundingPayment){
        mapping(int24 => Tick.GrowthInfo) storage tickMap = growthOutsideTick[_baseToken];
        address pool = IOddzMarket(oddzMarket).fetchPool(_baseToken);

        // funding of liquidity coefficient
        (, int24 tick, , , , , ) = UniswapInteractor.fetchSlot0(pool);
        OrderInfo storage order = currentOrderMap[_orderId];
        Tick.FundingGrowthRangeInfo memory fundingGrowthRangeInfo =
                tickMap.getAllFundingGrowth(
                    ...
                    tick,
                    _twPremiumX96,
                    _twPremiumDivBySqrtPriceX96
        );

        // the calculation here is based on cached values
        _liquidityCoefficientInFundingPayment = _liquidityCoefficientInFundingPayment.add(
    ...
    */
    function getLiquidityCoefficientInFundingPayment(
        ...
        int256 _twPremiumX96,
        int256 _twPremiumDivBySqrtPriceX96
    ) external view override returns (int256 _liquidityCoefficientInFundingPayment) {
        ...
        OrderInfo memory order = currentOrderMap[_orderId];
        Tick.FundingGrowthRangeInfo memory fundingGrowthRangeInfo =
            tickMap.getAllFundingGrowth(
                order.lowerTick,
                order.upperTick,
        ...
        // the calculation here is based on cached values
        _liquidityCoefficientInFundingPayment = Funding.calcLiquidityCoefficientInFundingPaymentByOrder(order, fundingGrowthRangeInfo);
            
    }

    /** @notice used to udpate the time weighted premium value of a liquidity position
    * @param _orderId order id
    * @param _twPremiumGrowthGlobalX96 new time weighted premium growth
     */
    function updateTwPremiumGrowthGlobal(
        bytes32 _orderId,
        int256 _twPremiumGrowthGlobalX96
    ) external override onlyClearingHouse {
        currentOrderMap[_orderId].lastTwPremiumGrowthGlobalX96 = _twPremiumGrowthGlobalX96;
    }
    ...
    {
        return currentOrderIdsMap[_trader][_baseToken];
    }
    ...
    function getTotalOrdersAmountInPool(
        ...
        bytes32[] memory orderIds = currentOrderIdsMap[_trader][_baseToken];
    ...
    /**@notice internal add liquidity function.Used to update liquidity order detials
    * @param _params all the paramter of internalAddLiquidity structure
    */
    function _addLiquidity(
      InternalAddLiquidityParams memory _params
    ) internal returns(AddLiquidityResponse memory ){

        (, int24 currentTick, , , , , ) = UniswapInteractor.fetchSlot0(_params.poolAddress);
        mapping(int24 => Tick.GrowthInfo) storage tickMap = growthOutsideTick[_params.baseToken];
        //caluclate order ID
        bytes32 orderId = calcOrderID(
            _params.trader,
        ...
        if (openOrder.liquidity == 0) {
            ...
            openOrder.trader    = _params.trader;
            openOrder.lowerTick = _params.lowerTickOfOrder;
            openOrder.upperTick = _params.upperTickOfOrder;
            Tick.FundingGrowthRangeInfo memory fundingGrowthRangeInfo =
                tickMap.getAllFundingGrowth(
                    openOrder.lowerTick,
                    openOrder.upperTick,
                    currentTick,
                    _params.globalFundingGrowth.twPremiumX96,
                    _params.globalFundingGrowth.twPremiumDivBySqrtPriceX96
                );
        ...
        openOrder.collateralForOrder = openOrder.collateralForOrder.add(
            _params.collateralForOrder
        );
        totalCollateralForOrders[_params.trader] = totalCollateralForOrders[
            _params.trader
        ].add(_params.collateralForOrder);
        ...
        return AddLiquidityResponse({
            ...
            liquidityAmount:_params.liquidity,
            feeAmount:fee,
            orderId:orderId
        });
    }
    ...
    function _fetchPendingFeeAndFeeGrowthInsideByOrder(address _baseToken, OrderInfo memory _order)
        internal
        view
        returns (uint256 _pendingFee, uint256 _feeGrowthInside)
    {
        address pool = IOddzMarket(oddzMarket).fetchPool(_baseToken);
        (, int24 tick, , , , , ) = UniswapInteractor.fetchSlot0(pool);
        mapping(int24 => Tick.GrowthInfo) storage tickMap = growthOutsideTick[_baseToken];
        _feeGrowthInside = tickMap.getFeeGrowthInsideX128(
            _order.lowerTick,
            _order.upperTick,
            tick,
            feeGrowthGlobal[_baseToken]
        );
        _pendingFee = FullMath.mulDiv(
            _feeGrowthInside - _order.lastFeeGrowthInside,
            _order.liquidity,
            FixedPoint128.Q128
        );

        return (_pendingFee, _feeGrowthInside);
    }

    function rSwap(rSwapParams memory _params) 
         external 
         override 
         onlySwapManager
         returns (rSwapResponse memory)
    {
        address pool = IOddzMarket(oddzMarket).fetchPool(_params.baseToken);
        ...
        int24 tickSpacing = UniswapInteractor.fetchTickSpacing(pool);

        _params.sqrtPriceLimitX96 = _params.sqrtPriceLimitX96 == 0
            ? (_params.isShort ? TickMath.MIN_SQRT_RATIO + 1 : TickMath.MAX_SQRT_RATIO - 1)
            : _params.sqrtPriceLimitX96;

        UniswapInteractor.SwapState memory swapState =
            UniswapInteractor.fetchSwapState(pool, _params.specifiedAmount, feeGrowthGlobal[_params.baseToken]);
        
        uint256 fee;
        uint256 insuranceFundFee; 
        // insuranceFundFee = fee * insuranceFundFeeRatio
        uint24 insuranceFundFeeRatio = IOddzConfig(oddzConfig).insuranceFundFeeRatio(_params.baseToken);

        bool isExactInput = _params.specifiedAmount > 0;
        while (swapState.amountSpecifiedRemaining != 0 && swapState.sqrtPriceX96 != _params.sqrtPriceLimitX96) {
            InternalSwapStep memory step;
            step.initialSqrtPriceX96 = swapState.sqrtPriceX96;

             // find next tick
             // note the search is bounded in one word
            (step.nextTick, step.isNextTickInitialized) = UniswapInteractor.fetchNextInitializedTickWithinOneWord(
                pool,
                swapState.tick,
                tickSpacing,
                _params.isShort
            ...
            (swapState.sqrtPriceX96, step.amountIn, step.amountOut, step.fee) = SwapMath.computeSwapStep(
                ...
                (
                    _params.isShort
                        ? step.nextSqrtPriceX96 < _params.sqrtPriceLimitX96
                        : step.nextSqrtPriceX96 > _params.sqrtPriceLimitX96
                )
                    ? _params.sqrtPriceLimitX96
                ...
                // isShort: fee is charged in base token in uniswap pool; thus, use uniswapFee to replay
                // !isShort: fee is charged in quote token in clearing house; thus, use swapFeesRatio
                _params.isShort ? _params.uniswapFee : _params.swapFees
            );
            ...
            // update oddzCH global fee growth if there is liquidity in this range
            // note oddzCH only collects quote fee when swapping base -> quote
            if (swapState.liquidity > 0) {
                if (_params.isShort) {
                    step.fee = FullMath.mulDivRoundingUp(step.amountOut, _params.swapFees, 1e6);
                }
            ...
            if (swapState.sqrtPriceX96 == step.nextSqrtPriceX96) {
                ...
                if (step.isNextTickInitialized) {
                    if (_params.shouldUpdateState) {
                        // update the tick if it has been initialized
                        mapping(int24 => Tick.GrowthInfo) storage tickMap = growthOutsideTick[_params.baseToken];
                        // according to the above updating logic,
                        // if isShort, state.feeGrowthGlobalX128 will be updated; else, will never be updated
                        tickMap.cross(
                            step.nextTick,
                            Tick.GrowthInfo({
                    ...
                    int128 liquidityNet = UniswapInteractor.fetchTickLiquidityNet(pool, step.nextTick);
                    if (_params.isShort){
                        liquidityNet = liquidityNet.neg128();
                    }
                    swapState.liquidity = LiquidityMath.addDelta(swapState.liquidity, liquidityNet);
                }

                swapState.tick = _params.isShort ? step.nextTick - 1 : step.nextTick;
        ...
        if (_params.shouldUpdateState) {
            ...
            feeGrowthGlobal[_params.baseToken] = swapState.feeGrowthGlobalX128;
        }

        return rSwapResponse({ 
            tick: swapState.tick, 
            fee: fee, 
            insuranceFee: insuranceFundFee
        });
    }


    function fetchPendingFee(bytes32 _orderId, address _baseToken)
        external
        view
        override
        returns (uint256 totalPendingFee)
    {
        
        OrderInfo memory order = currentOrderMap[_orderId];

        ( totalPendingFee, ) = _fetchPendingFeeAndFeeGrowthInsideByOrder(_baseToken, order);
    
    }
    

    function getTotalCollateralForOrders(address _trader)
        external
        view
        override
        returns (uint256 _collateral)
    {
        return totalCollateralForOrders[_trader];
    }

    /**
     *@notice  to get the info of the order
     *@param _orderId order is of the liquidity position
     *@return _info order info
     */
    function getCurrentOrderMap(bytes32 _orderId)
        external
        view
        override
        returns (OrderInfo memory _info)
    {
        return currentOrderMap[_orderId];
    }
}
