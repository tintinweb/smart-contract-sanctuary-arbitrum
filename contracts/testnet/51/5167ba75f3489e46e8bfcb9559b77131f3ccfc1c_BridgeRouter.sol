...
contract BridgeRouter is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function estimateSendTokensFee(
        ...
        return bridgeProxy.estimateSendTokensFee(_srcChainId, _toAddress, _amount, _useZro, _adapterParams);
    }
    ...
    function bridge(
        ...
        bridgeProxy.sendTokens{value: msg.value}(
            _srcChainId, _to, _amount, payable(msg.sender), _zroPaymentAddress, _adapterParam
        );
        emit Bridge(_to, _amount, msg.sender, _zroPaymentAddress, _adapterParam);
    }
    ...
    function approveTransfer(uint16 _srcChainId, bytes calldata _srcAddress, uint64 _nonce) external nonReentrant {
        ...
        (address _to, uint256 _amount) = bridgeProxy.getRecieveQueueInfo(_srcChainId, _srcAddress, _nonce);
        if (_amount > 0 && _to != address(0)) {
            bridgeProxy.deQueue(_srcChainId, _srcAddress, _nonce);
            IERC20(token).safeTransfer(_to, _amount);
            emit Approved(_srcChainId, _srcAddress, _nonce);
        }
    }
    ...
    event Approved(uint16 _srcChainId, bytes _srcAddress, uint64 _nonce);
    event Bridge(bytes _to, uint256 _amount, address _refundAddress, address _zroPaymentAddress, bytes _adapterParam);
}
...
interface IBridgeProxy {
    ...
    function deQueue(uint16 _srcChainId, bytes calldata _srcAddress, uint64 _nonce) external;
}