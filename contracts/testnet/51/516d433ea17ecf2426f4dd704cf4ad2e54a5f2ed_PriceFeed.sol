...
contract PriceFeed is IPriceFeed, Initializable, EndpointGatedUpgradeable {
    ...
    /// @dev This price feed returns price in precision of 10 ^ (30 - base token decimals)
    uint256 private constant VALUE_PRECISION = 1e30;

    IPyth public pyth;
    IFundingRateManager public fundingRateManager;

    FundingRateConfig private fundingRateConfig;

    mapping(address => MarketConfig) public marketConfig;

    function initialize(address _initialOwner, address _pyth, address _fundingRateManager) external initializer {
    ...
    function configMarket(
        ...
        if (_priceSource == PriceSource.Chainlink && address(_chainlinkPriceFeed) == address(0)) revert ZeroAddress();
        if (_priceSource == PriceSource.Pyth && _pythId == bytes32(0)) revert InvalidPythId();
        marketConfig[_market] = MarketConfig({
            baseUnits: 10 ** IERC20Metadata(_market).decimals(),
            priceSource: _priceSource,
    ...
    function updatePrice(bytes[] calldata _data) external payable {
        for (uint256 i; i < _data.length;) {
            (PriceSource _priceSource, bytes[] memory _priceData) = abi.decode(_data[i], (PriceSource, bytes[]));
            _updatePrice(_priceSource, _priceData);
            unchecked {
                ++i;
            }
        }
    ...
    // =============== INTERNAL FUNCTIONS ===============
    function _setFundingRateManager(address _fundingRateManager) internal {
        if (_fundingRateManager == address(0)) revert ZeroAddress();
        fundingRateManager = IFundingRateManager(_fundingRateManager);

        fundingRateConfig = FundingRateConfig({
            precision: fundingRateManager.PRECISION(),
            fundingInterval: fundingRateManager.FUNDING_INTERVAL()
        });

        emit FundingRateManagerSet(_fundingRateManager);
    }

    function _updatePrice(PriceSource _priceSource, bytes[] memory _priceData) internal {
        if (_priceSource == PriceSource.Pyth) {
            return _updatePythPrice(_priceData);
        } else if (_priceSource == PriceSource.Chainlink) {
            /// @dev todo implement chainlink low legacy price feed
        }
    }

    function _updatePythPrice(bytes[] memory _priceData) internal {
        try pyth.updatePriceFeeds{value: msg.value}(_priceData) {} catch {}
    }

    function _getMarkPrice(address _market) internal view returns (uint256) {
        int256 _lastFundingRate = fundingRateManager.lastFundingRate(_market);
        uint256 _nextFundingTime = fundingRateManager.nextFundingTime(_market);
        uint256 _durationTillNextUpdate = MathUtils.zeroCapSub(_nextFundingTime, block.timestamp);
        return _getChainlinkPrice(_market)
            * (
                fundingRateConfig.precision.toInt256()
                    + (_lastFundingRate * _durationTillNextUpdate.toInt256() / fundingRateConfig.fundingInterval.toInt256())
            ).toUint256() / fundingRateConfig.precision;
    }

    function _getIndexPrice(address _market) internal view returns (uint256) {
        MarketConfig memory _marketConfig = marketConfig[_market];
        if (_marketConfig.priceSource == PriceSource.Pyth) {
            return _getPythPrice(_market);
        } else if (_marketConfig.priceSource == PriceSource.Chainlink) {
            return _getChainlinkPrice(_market);
        }
        revert UnknownMarket();
    }

    function _getChainlinkPrice(address _market) internal view returns (uint256) {
        MarketConfig memory _marketConfig = marketConfig[_market];
        (, int256 _guardPrice,,,) = _marketConfig.chainlinkPriceFeed.latestRoundData();
        uint256 _priceDecimals = _marketConfig.chainlinkPriceFeed.decimals();
        return _guardPrice.toUint256() * VALUE_PRECISION / _marketConfig.baseUnits / (10 ** _priceDecimals);
    }

    function _getPythPrice(address _market) internal view returns (uint256) {
        MarketConfig memory _marketConfig = marketConfig[_market];
        PythStructs.Price memory price = pyth.getPriceUnsafe(_marketConfig.pythId);
        return uint256(int256(price.price)) * VALUE_PRECISION / _marketConfig.baseUnits
