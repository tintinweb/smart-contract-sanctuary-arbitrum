...
contract OracleAggregator is IOracleAggregator, Ownable {
    ...
    uint256 public constant CANCELATION_PERIOD = 15 minutes;

    EnumerableSet.AddressSet private _keepers;
    mapping(address => mapping(uint256 => uint256)) private _dataCache;
    mapping(address => mapping(uint256 => bool)) private _dataExist;

    address public updater;

    function getData(address oracleId, uint256 timestamp) external view returns (uint256) {
        require(hasData(oracleId, timestamp), "OracleAggregator: Data is not exist");
        return _dataCache[oracleId][timestamp];
    }

    function hasData(address oracleId, uint256 timestamp) public view returns (bool) {
        return _dataExist[oracleId][timestamp];
    }
    ...
    function keepersList(uint256 offset, uint256 limit) external view returns (address[] memory output) {
        uint256 kepersLength = _keepers.length();
        if (offset >= kepersLength) return new address[](0);
        uint256 to = offset + limit;
        if (kepersLength < to) to = kepersLength;
        output = new address[](to - offset);
        for (uint256 i = 0; i < output.length; i++) output[i] = _keepers.at(offset + i);
    }

    function __callback(uint256 timestamp, uint256 data) external returns (bool) {
        require(!hasData(msg.sender, timestamp), "OracleAggregator: Data already exist");
        _dataCache[msg.sender][timestamp] = data;
        _dataExist[msg.sender][timestamp] = true;
    ...
    function addKeepers(address[] memory keepers_) external onlyOwner returns (bool) {
        for (uint256 i = 0; i < keepers_.length; i++) {
            ...
            require(keeper_ != address(0), "OracleAggregator: Keeper is zero address");
    ...
    function updateUpdater(address updater_) external onlyOwner returns (bool) {
        require(updater_ != address(0), "OracleAggregator: Updater is zero address");
        updater = updater_;
