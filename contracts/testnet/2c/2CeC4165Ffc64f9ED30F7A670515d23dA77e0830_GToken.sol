...
contract GToken is ERC20Upgradeable, ERC4626Upgradeable, OwnableUpgradeable, IGToken {
    ...
    function runWithdraw(bytes32 requestId,int uPnl) public checks() returns (uint){
        ...
        if(sum != applyWithdrawData.lplAmount){
            i-=1;
            AddressLocks[applyWithdrawData.receiver][i].assets=sum - applyWithdrawData.lplAmount;
        }

        for(uint j=0;j<AddressLocks[applyWithdrawData.receiver].length;j++){
            if(j+i+1 > AddressLocks[applyWithdrawData.receiver].length){
                delete AddressLocks[applyWithdrawData.receiver][j];
            }else{
                AddressLocks[applyWithdrawData.receiver][j]=AddressLocks[applyWithdrawData.receiver][j+i+1];
            }
        }
