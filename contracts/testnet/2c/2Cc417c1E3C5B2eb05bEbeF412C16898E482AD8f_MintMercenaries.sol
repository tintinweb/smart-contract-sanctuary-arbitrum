...
//TODO: add interface minting mercs import "";

interface IMercenaries {
        function mint(address to, uint256 amount, uint16[][] calldata traits) external;
}
...
// FUNCTIONS
    ...
    function claimMercenaryAirDrop(uint256 index, bytes32[] calldata merkleProof, uint256 amount, uint16[][] calldata traits)
        external
        nonReentrant
        whenNotPaused
        onlyUnclaimed(index)
    ...
    /// @param traits a 2 dimensional array representing the mercenaries traits e.g., [[7,2,5,1,1],[6,3,1,4,5]]
    function claimMercenary(uint256 index, uint16[][] calldata traits)
        external
        nonReentrant
        whenNotPaused
    {
    ...
    /// @param traits a 2 dimensional array representing the mercenaries traits e.g., [[7,2,5,1,1],[6,3,1,4,5]]
    function mintMercenary(uint256 amount, uint16[][] calldata traits)
        external
        payable
        nonReentrant
        whenNotPaused
