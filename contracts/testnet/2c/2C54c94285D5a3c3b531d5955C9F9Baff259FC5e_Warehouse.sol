...
contract Warehouse is ERC721Holder{
    /* ========== STATE VARIABLES ========== */

    mapping (address => mapping(uint256 => address)) public prevOwners;
    ...
    constructor(address _gamePolicy){
        gamePolicy = IGamePolicy(_gamePolicy);
    }

    /* ========== VIEWS ========== */

    /* ========== PUBLIC FUNCTIONS ========== */
    function add(address _token, uint256 _id, address _owner) external {
        require(msg.sender == _token, "Warehouse: !token");
        totalItems++;
        prevOwners[_token][_id] = _owner;
    }

    function recover(address _token, uint256 _id) external {
        require(gamePolicy.isPrizeManager(msg.sender) || gamePolicy.isTournamentRouter(msg.sender), "Warehouse: !tournament router");
        require(prevOwners[_token][_id] != address(0), "Warehouse: !exist");
        address _owner = prevOwners[_token][_id];
        prevOwners[_token][_id] = address(0);
        IERC721(_token).safeTransferFrom(address(this), _owner, _id);
    }
