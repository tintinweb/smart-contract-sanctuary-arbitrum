...
contract TriggerOrderManager is ITriggerOrderManager, BasePosition, ReentrancyGuard {
    ...
    function triggerPosition(
        ...
        require(txn.status == TRANSACTION_STATUS_NONE, "Invalid txStatus");
        uint256 txType = txn.txType;
        bool isValidTxType;
        address[] memory path;
        bool isDirectExecuted;
        uint256[] memory prices;

        if (txType == ADD_TRAILING_STOP) {
            //Trigger for adding trailing stop
            path = new address[](1);
            path[0] = _indexToken;
            (, prices) = _getPricesAndCheckDirectExecute(path);
            IRouter(router).setPriceAndExecute(
                key,
                ADD_TRAILING_STOP,
                prices
            );
            ...
            isValidTxType = txType == CREATE_POSITION_LIMIT || txType == CREATE_POSITION_STOP_MARKET
                || txType == CREATE_POSITION_STOP_LIMIT;
            require(isValidTxType, "Invalid txType");
            path = IRouter(router).getPath(key, txType);
            require(path.length > 0, "Invalid pathLength");
            require(_indexToken == path[0], "Invalid indexToken");
            (isDirectExecuted, prices) = _getPricesAndCheckDirectExecute(path);
            ...
            //Trigger for updating trigger orders
            require(triggerOrders[key].key == key, "Not found triggerData");
            (isDirectExecuted, prices) = _getPricesAndCheckDirectExecute(path);
            IRouter(router).triggerPosition(
                key,
                isDirectExecuted,
    ...
    function updateTriggerOrders(
        ...
        (bool isDirectExecute, uint256 indexPrice) = _getPriceAndCheckDirectExecute(_indexToken);
    ...
    function executeUpdateTriggerOrdersInBatch(
        bytes32[] memory _keys,
        uint256[] memory _prices
    ) external {
        require(msg.sender == executor, "Forbidden: Not executor");
        require(_keys.length == _prices.length && _keys.length > 0, "Invalid array length");

        for (uint256 i = 0; i < _keys.length; i++) {
            try ITriggerOrderManager(address(this)).executeUpdateTriggerOrders(_keys[i], _prices[i]) {
            } catch {}
        }
    }

    function executeUpdateTriggerOrders(
        bytes32 _key,
        uint256 _indexPrice
    ) external {
        require(msg.sender == executor || msg.sender == address(this), "Forbidden");
        _executeUpdateTriggerOrders(
            _key,
            _indexPrice,
            triggerOrders[_key].isLong,
            triggerOrders[_key].tpPrices,
            triggerOrders[_key].slPrices,
            triggerOrders[_key].tpAmountPercents,
            triggerOrders[_key].slAmountPercents,
            triggerOrders[_key].tpTriggeredAmounts,
            triggerOrders[_key].slTriggeredAmounts
        );
    }

    function _executeUpdateTriggerOrders(
        bytes32 _key,
        uint256 _indexPrice,
        bool _isLong,
        uint256[] memory _tpPrices,
        ...
        bool validateTriggerData = _validateTriggerOrdersData(
            _isLong,
            _indexPrice,
            _tpPrices,
            _slPrices,
        ...
        if (!validateTriggerData) {
            delete triggerOrders[_key];
        } else {
            if (triggerOrders[_key].tpPrices.length + triggerOrders[_key].slPrices.length < _tpPrices.length + _slPrices.length) {
            require(msg.value == settingsManager.triggerGasFee(), "Invalid triggerGasFee");
        }
            triggerOrders[_key].status = TriggerStatus.OPEN;
    ...
    function _validateRouter() internal view {
        require(router != address(0), "IVLR/NI"); //Invalid router, not initialized
    }
}
...
contract BasePositionConstants {
    ...
    uint256 public constant TRANSACTION_STATUS_EXECUTE_FAILED = 3;
    //End constant params
