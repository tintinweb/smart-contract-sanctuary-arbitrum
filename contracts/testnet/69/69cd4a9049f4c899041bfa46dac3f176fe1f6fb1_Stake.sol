...
/**
 * @title RwaxStake
 * @notice Contract to stake UTOKEN token and earn GOVTOKEN reward.
 * @author Rwax
 */
contract Stake is Initializable, OwnableUpgradeable {
     ...
     */
    function initialize(address _utoken, address govtoken, uint256 _rewardPerSecond) external initializer {
        __Ownable_init();
        ...
        UTOKEN = IBurnableERC20(_utoken);
        GOVTOKEN = IERC20(govtoken);
     ...
     * @dev Called by the proxy contract
     */
    function reInit(address _utoken, address govtoken, uint256 _rewardPerSecond) external reinitializer(VERSION) {
        require(_rewardPerSecond <= MAX_REWARD_PER_SECOND, "> MAX_REWARD_PER_SECOND");
        require(_utoken != address(0), "Invalid UTOKEN address");
        require(govtoken != address(0), "Invalid GOVTOKEN address");
        UTOKEN = IBurnableERC20(_utoken);
    ...
    function pendingReward(address _to) external view returns (uint256) {
        ...
        uint256 utokenSupply = UTOKEN.balanceOf(address(this));
        uint256 _accRewardPerShare = accRewardPerShare;
        if (block.timestamp > lastRewardTime && utokenSupply != 0) {
            uint256 time = block.timestamp - lastRewardTime;
            uint256 reward = time * rewardPerSecond;
            uint256 boostedReward = getBoostedReward();
            _accRewardPerShare += (((reward + boostedReward) * ACC_REWARD_PRECISION) / utokenSupply);
        }
    ...
    /**
     * @dev Staked UTOKEN tokens, and start earning rewards
    ...
    function stake(address _to, uint256 _amount) external  {
        ...
        if (user.amount != 0) {
            ...
            if (pending != 0) {
                _safeTransferGOVTOKEN(_to, pending);
    ...
    function unstake(address _to, uint256 _amount) external  {
        ...
        IERC20(UTOKEN).safeTransfer(_to, amountToUnstake);
    ...
    /**
     * @dev Claims GOVTOKEN rewards to the address `to`
    ...
    function claimRewards(address _to) external {
        ...
        if (_pendingReward != 0) {
            _safeTransferGOVTOKEN(_to, _pendingReward);
    ...
    /// @param _rewardPerSecond The amount of GOVTOKEN to be distributed per second.
    ...
    /// @param _boostedRewardPerSecond The boosted amount of GOVTOKEN to be distributed per second.
    ...
    /// @notice Reserve an amount of GOVTOKEN by sending it to auction treasury
    function reserveAuctionFund(uint256 amount) external onlyOwner {
        GOVTOKEN.safeTransfer(auctionTreasury, amount);
    ...
    function update() public {
        if (block.timestamp > lastRewardTime) {
            ...
            if (utokenSupply != 0) {
                ...
                uint256 reward = time * rewardPerSecond;
                uint256 boostedReward = getBoostedReward();
                accRewardPerShare = accRewardPerShare + (((reward + boostedReward) * ACC_REWARD_PRECISION) / utokenSupply);
            }
    ...
    function _safeTransferGOVTOKEN(address _to, uint256 _amount) internal {
        require(GOVTOKEN != IERC20(address(0)), "GOVTOKEN not set");
        uint256 veRwaxBalance = GOVTOKEN.balanceOf(address(this));
        if (_amount > veRwaxBalance) {
            GOVTOKEN.safeTransfer(_to, veRwaxBalance);
        } else {
            GOVTOKEN.safeTransfer(_to, _amount);
        }
    }
