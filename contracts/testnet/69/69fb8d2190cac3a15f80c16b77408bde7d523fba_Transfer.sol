...
contract Transfer {
    ...
    modifier onlyOwner {
        require(msg.sender == owner, "Transfer: caller is not the owner");
        _;
    }

    modifier onlyVerifiedToken(address _token) {
        require(verifiedTokens[_token] == true, "Transfer: token is not verified");
        _;
    }
    ...
    function removeVerifiedToken(address _token) public onlyOwner {
        require(verifiedTokens[_token] == true, "Transfer: token is not in the list");
    ...
    function transfer(IERC20Interface token, address to, uint256 amount, string memory message) public onlyVerifiedToken(address(token)) returns (bool) {
        ...
        require(senderBalance >= amount, "Transfer: insufficient balance");
