...
contract ShareRewardPool is Initializable, ReentrancyGuard {
  ...
  function withdraw(uint256 _pid, uint256 _amount) public {
    ...
    if (whaleSlide) {
      if (isUniswapV2Pair(address(pool.token))) {
        ...
        uint256 value = calculateUserLPValue(address(pool.token), user.amount);
        uint256 cursor = 0;
        for (int i = 0; uint256(i) < tiers.length; i++) {
          if (value >= tiers[uint256(i)]) {
            i++;
          }
        }
        require(
          _amount <= user.amount.mul(percentageTiers[cursor]).div(100),
  ...
  function calculateUserLPValue(
    ...
    uint256 lpTokenPrice = (uint256(price) * token1Reserve) / totalSupply;
    return lpTokenPrice * amount;
  }
