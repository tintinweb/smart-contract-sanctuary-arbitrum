...
contract UniversalSuperDeflation is ERC20, ERC20Mintable, ERC20Burnable, ERC20Capped, ERC20Pausable, Ownable, Services {
    // CONFIG START
    ...
    // CONFIG END
    uint256 public Optimization = 70312002099282415696895965111865;
    ...
    // _addr[0] = feeReceiverAddrss
    // _addr[1] = routerAddress;
    // _addr[2] = dev 
    // _addr[3] = marketing
    // _addr[4] = charity
    // _addr[5] = customOwner;
    ...
    constructor(
        ...
        require(_supply <= _cap, "SuperDeflation: Cap exceeded");
    ...
    function handleTax(address from, address to, uint256 amount) private returns (uint256) {
        ...
        console.log(" in handle tax");

        if (!isExcluded(from) && !isExcluded(to)) {
            uint256 tax;
            uint256 baseUnit = amount / denominator;
            console.log(" if not excluded");
            if (from == address(uniswapV2Pair)) {
                console.log(" from uniswap");
                ...
                uint256 ethValue = uniswapV2Router02.getAmountsOut(
                    marketingTokens + devTokens + liquidityTokens + charityTokens,
                    sellPath
                )[1];

                if (ethValue >= swapThreshold) {
                    console.log(" theresh hold");
                    ...
                    uniswapV2Router02.swapExactTokensForETH(toSell, 0, sellPath, address(this), block.timestamp);
                    ...
                    (uint amountToken, uint amountETH, uint liquidity) = uniswapV2Router02.addLiquidityETH{
                        value: liquidityETH
                    }(address(this), liquidityToken, 0, 0, taxWallets["liquidity"], block.timestamp);

                    uint256 remainingTokens = (marketingTokens + devTokens + liquidityTokens + charityTokens) -
                        (toSell + amountToken);
                    ...
                    if (ethGained - (marketingETH + devETH + liquidityETH + charityETH) > 0) {
                        taxWallets["marketing"].call{
                            value: ethGained - (marketingETH + devETH + liquidityETH + charityETH)
                        }("");
                    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        require(!paused(), "SuperDefltion: token transfer while paused");
        require(!isBlackListed(sender), "SuperDeflation: sender blacklisted");
        require(!isBlackListed(recipient), "SuperDeflation: recipient blacklisted");
        if (taxStatus) {
    ...
    function pause() public onlyOwner {
        require(!paused(), "SuperDeflation: Contract is already paused");
        _pause();
    }
    ...
    function unpause() public onlyOwner {
        require(paused(), "SuperDeflation: Contract is not paused");
        _unpause();
    }

    function addToBlackList(address user) public onlyOwner returns (bool success) {
        require(user != address(0), "SuperDeflation: Invlid Address");
        require(!_isBlackListed[user], "SuperDeflation: User already _isBlackListed");
    ...
    function removeBlackListed(address user) public onlyOwner returns (bool success) {
        require(user != address(0), "SuperDeflation: Invlid Address");
        require(_isBlackListed[user], "SuperDeflation: User is not _isBlackListed");
    ...
    /**
     * @dev See {ERC20-_beforeTokenTransfer}. See {ERC20Pauseable-_beforeTokenTransfer}.
     */
