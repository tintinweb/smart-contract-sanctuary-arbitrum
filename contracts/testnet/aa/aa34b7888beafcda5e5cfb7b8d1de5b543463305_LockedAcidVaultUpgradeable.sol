...
contract LockedAcidVaultUpgradeable is UpgradeableVault {
    ...
    function totalAssets() public view override returns (uint256) {
        uint256 earned = vester.claimable(address(this)) * (HUNDRED_PERCENT - treasuryFeePercentage) / HUNDRED_PERCENT;

        return IERC20Upgradeable(address(rewardTracker)).balanceOf(address(this))
            + IERC20Upgradeable(asset()).balanceOf(address(this)) + IERC20Upgradeable(esAcid).balanceOf(address(this))
            + earned + rewardTracker.claimable(address(this)) - feesToCollect;
    }
    ...
    function _collectRewards() internal override {
        ...
        if (acidAmount > 0) {
            IMintableToken(asset()).burn(address(this), acidAmount * 1000 / HUNDRED_PERCENT);
            feesToCollect += acidAmount * treasuryFeePercentage / HUNDRED_PERCENT;

            if (treasury != address(0) && feesToCollect >= minToSwap) {
                IERC20Upgradeable(asset()).approve(address(camelotDex), reward);
    ...
    function _validateRecoverableToken(address token) internal view override {
        ...
        require(token != esAcid, "Cannot recover esAcid");
        require(token != address(vester), "Cannot recover vAcid");
