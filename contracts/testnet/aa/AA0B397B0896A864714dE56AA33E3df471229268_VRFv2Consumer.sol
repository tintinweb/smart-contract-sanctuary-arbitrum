...
contract VRFv2Consumer is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function generateRandomNumberAndFindWinner(address[] memory _users, uint256[][] memory _ticketNumbers)
        external
        onlyOwner
        returns (uint256 requestId)
    {
        require(_users.length == _ticketNumbers.length, "Mismatch in user and ticket numbers count");
        ...
        lastUsers = _users;
        lastTicketNumbers = _ticketNumbers;
    ...
    function fulfillRandomWords(
        ...
        emit InputData(lastUsers, lastTicketNumbers);
    ...
    function getResult(uint256 index) external view returns (LotteryResult memory) {
