...
interface IExchangeAgent {
    ...
    function getSYNTHFromUSDC(uint256 amountIn) external view returns (uint256);
...
interface ISYNTHManager {
    function transferSynth(address _to, uint256 _amount) external;
}
...
import "../interfaces/ISYNTHManager.sol";
...
contract BaseWrappedSynthr is MixinResolver, Owned {
    ...
    bytes32 private constant CONTRACT_EXAGENT = "ExchangeAgent";
    ...
    bytes32 private constant CONTRACT_SYNTH_MANAGER = "SynthManager";
    ...
    function resolverAddressesRequired() public pure override returns (bytes32[] memory addresses) {
        ...
        addresses[8] = CONTRACT_COLLATERAL_MANAGER;
        addresses[9] = CONTRACT_SYNTH_MANAGER;
        addresses[10] = CONTRACT_EXAGENT;
        addresses[11] = CONTRACT_OFF_CHAIN_EXCHANGER;
    }
    ...
    function exchangeAgent() internal view returns (IExchangeAgent) {
        return IExchangeAgent(requireAndGetAddress(CONTRACT_EXAGENT));
    }
    ...
    function synthManager() internal view returns (ISYNTHManager) {
        return ISYNTHManager(requireAndGetAddress(CONTRACT_SYNTH_MANAGER));
    }

    function isWaitingPeriod(bytes32 currencyKey) external view returns (bool) {
        return exchanger().maxSecsLeftInWaitingPeriod(msg.sender, currencyKey) > 0;
    }
    ...
    function burnSynths(uint256 amount, bytes32 synthKey) external issuanceActive systemActive {
        (uint256 synthAmount, , uint256 reclaimed, uint256 refunded) = issuer().burnSynths(msg.sender, synthKey, amount);
        synthrBridge().sendBurn(msg.sender, synthKey, synthAmount, reclaimed, refunded);
        liquidatorRewards().updateEntry(msg.sender);
    }

    function burnSynthsToTarget(bytes32 synthKey) external issuanceActive systemActive {
        (uint256 synthAmount, , uint256 reclaimed, uint256 refunded) = issuer().burnSynthsToTarget(msg.sender, synthKey);
        synthrBridge().sendBurn(msg.sender, synthKey, synthAmount, reclaimed, refunded);
        liquidatorRewards().updateEntry(msg.sender);
    }
    ...
    function destBurn(
        ...
        bytes32 _synthKey,
        uint256 _synthAmount
    ) external onlySynthrBridge returns (bool) {
        try issuer().destBurn(_account, _synthKey, _synthAmount) returns (uint256 result) {
            emit DestBurn(_account, _synthKey, _synthAmount, result);
            return true;
        } catch Error(string memory reason) {
            issuer().destIssue(_account, _synthKey, _synthAmount);
            emit DestBurnFailed(_account, reason);
            return false;
        }
    }
    ...
    function collateralTransfer(
        ...
        uint256 _usdAmount,
        uint16 _destChainId
    ) internal systemActive returns (bool, uint256) {
        require(extTokenState.collateralCurrency(_collateralKey) != address(0), "No exist collateral currency");
        ...
        if (_destChainId == 0) {
            ...
            _burn(_from, collateralAmount, _collateralKey);
            if (extTokenState.collateralCurrency(_collateralKey) == NULL_ADDRESS) {
                collateralManager().depositeCollateral{value: collateralAmount}(_from, NULL_ADDRESS, collateralAmount);
            } else {
                if (
    ...
    function synthRewardsDistribution(address _to, uint256 _usdAmount) internal returns (bool, uint256) {
        uint256 synthAmount = exchangeAgent().getSYNTHFromUSDC(_usdAmount);
        synthManager().transferSynth(_to, synthAmount);
        return (true, _usdAmount);
    }
    ...
    function liquidateDelinquentAccount(
        ...
        (uint256 totalRedeemed, uint256 amountLiquidated, ) = issuer().liquidateAccount(
            account,
            collateralKey,
            destChainId,
            false
        ...
        if (totalRedeemed > 0) {
            ...
            if (totalRedeemed > liquidator().flagReward().add(liquidator().liquidateReward())) {
                ...
                (bool flagRewardTransferSucceeded, ) = synthRewardsDistribution(flagger, liquidator().flagReward());
                require(flagRewardTransferSucceeded, "Flag reward Synth transfer did not succeed.");
                ...
                (bool liquidateRewardTransferSucceeded, ) = synthRewardsDistribution(msg.sender, liquidator().liquidateReward());
            ...
            (bool liquidatorRewardTransferSucceeded, uint256 collateralAmount) = collateralTransfer(
                account,
                collateralKey,
                totalRedeemed,
                destChainId
            ...
            (bool synthrRewarddistributeSucceeded, uint256 synthAmountLiquidated) = synthRewardsDistribution(
                address(liquidatorRewards()),
                stakerRewards
            );
            require(synthrRewarddistributeSucceeded, "Transfer to LiquidatorRewards failed.");

            liquidatorRewards().updateEntry(account);
            synthrBridge().sendLiquidate{value: msg.value}(account, collateralKey, collateralAmount, destChainId);
            liquidatorRewards().notifyRewardAmount(synthAmountLiquidated);
            emit AccountLiquidated(account, totalRedeemed, amountLiquidated, msg.sender);
            return true;
        } else {
    ...
    function liquidateSelf(bytes32 collateralKey, uint16 destChainId) external payable systemActive returns (bool) {
        ...
        (bool success, uint256 collateralAmount) = collateralTransfer(msg.sender, collateralKey, totalRedeemed, destChainId);
        ...
        liquidatorRewards().updateEntry(msg.sender);
        synthrBridge().sendLiquidate{value: msg.value}(msg.sender, collateralKey, collateralAmount, destChainId);
        liquidatorRewards().notifyRewardAmount(synthAmountLiquidated);
        emit AccountLiquidated(msg.sender, totalRedeemed, amountLiquidated, msg.sender);
        return success;
    }
    ...
    event DestBurnFailed(address indexed account, string message);
...
contract WrappedSynthr is BaseWrappedSynthr {
    ...
    function chainBalanceOf(address _account, uint16 _destChainId) external view returns (uint256) {
        ...
        for (uint256 ii = 0; ii < availableCollateralCurrencies.length; ii++) {
            ...
            if (synthrAggregator().collateralByIssuerAggregation(_collateralCurrencyKey, _destChainId, _account) > 0) {
                (uint256 collateralRate, ) = exchangeRates().rateAndInvalid(_collateralCurrencyKey);
                synthrBalance = synthrBalance.add(
                    synthrAggregator()
                        .collateralByIssuerAggregation(_collateralCurrencyKey, _destChainId, _account)
    ...
    function chainBalanceOfPerKey(
        ...
        (uint256 collateralRate, ) = exchangeRates().rateAndInvalid(_collateralKey);
        return
            synthrAggregator().collateralByIssuerAggregation(_collateralKey, _destChainId, _account).multiplyDecimal(
                collateralRate
            );
    }
    ...
    function getSendWithdrawGasFee(
        ...
        return synthrBridge().calcLZFee(lzPayload, PT_WITHDRAW_COLLATERAL, _destChainId);
    }
    ...
    function balanceOf(address account) external view returns (uint256) {
        ...
        for (uint256 ii = 0; ii < availableCollateralCurrencies.length; ii++) {
            bytes32 _collateralCurrencyKey = availableCollateralCurrencies[ii];
            if (synthrAggregator().collateralByIssuer(_collateralCurrencyKey, account) > 0) {
                (uint256 collateralRate, ) = exchangeRates().rateAndInvalid(_collateralCurrencyKey);
                synthrBalance = synthrBalance.add(
                    synthrAggregator().collateralByIssuer(_collateralCurrencyKey, account).multiplyDecimal(collateralRate)
                );
            }
        }
        return synthrBalance;
    ...
    function offChainExchange(
        ...
        return offChainExchanger().exchange{value: msg.value}(args, priceUpdateData);
    }
    ...
    function issueSynths(
        ...
        if (_collateralAmount > 0) {
            _issueSynths(msg.sender, msg.sender, _collateralKey, _collateralAmount);
            synthrBridge().sendDepositCollateral(msg.sender, _collateralKey, _collateralAmount);
            if (extTokenState.collateralCurrency(_collateralKey) == NULL_ADDRESS) {
        ...
        if (_synthToMint > 0) {
            if (_destChainId != 0) {
                require(msgValueForGas > 0, "LZ gas fee must not be zero");
            }
            (synthAmount, ) = issuer().issueSynths(msg.sender, _synthToMint, _destChainId);
            synthrBridge().sendMint{value: msgValueForGas}(msg.sender, sUSD, synthAmount, _destChainId);
            liquidatorRewards().updateEntry(msg.sender);
        }
    }

    function issueMaxSynths(uint16 _destChainId) external payable issuanceActive systemActive {
        uint256 synthAmount;
        uint256 debtShare;
        (synthAmount, debtShare) = issuer().issueMaxSynths(msg.sender, _destChainId);
        synthrBridge().sendMint{value: msg.value}(msg.sender, sUSD, synthAmount, _destChainId);
        liquidatorRewards().updateEntry(msg.sender);
    }
    ...
    function withdrawCollateral(
        ...
        require(extTokenState.collateralCurrency(_collateralKey) != address(0), "No Collateral Currency exists.");
        require(
            synthrAggregator().collateralByIssuerAggregation(_collateralKey, _destChainId, msg.sender) >= _collateralAmount,
            "Insufficient Collateral Balance to withdraw."
        );
        require(
            _collateralAmount <= issuer().checkFreeCollateral(msg.sender, _collateralKey, _destChainId),
            "Try again with lower amount."
        );
        if (_destChainId == 0) {
            extTokenState.withdrawCollateral(msg.sender, _collateralKey, _collateralAmount);

            bool isSucceed = _burn(msg.sender, _collateralAmount, _collateralKey);
            require(isSucceed, "Burn Synthr failed.");
        }
        synthrBridge().sendWithdraw{value: msg.value}(msg.sender, _collateralKey, _collateralAmount, _destChainId);
