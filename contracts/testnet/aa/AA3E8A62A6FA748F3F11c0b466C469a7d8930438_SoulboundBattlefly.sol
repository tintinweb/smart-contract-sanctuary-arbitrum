...
interface ISoulboundBattlefly is IERC721EnumerableUpgradeable {
    event Minted(address indexed receiver, uint256 indexed tokenId, uint256 battleflyType, uint256 transactionId, bytes signature, bool stake);
    event MintedWhitelist(address indexed receiver, uint256 indexed tokenId, uint256 battleflyType, bytes32 merkleroot, uint256 index, bool stake);
...
import "../interfaces/soulbound/ISoulboundBattlefly.sol";
import "../game/interfaces/IBattleflyGame.sol";
    ...
    ISoulboundBattlefly
{
    ...
    function mint(bool, bytes calldata data) external nonReentrant whenNotPaused {
        ...
        for (uint256 i = currentId; i < (currentId + amount); ) {
            ...
            emit Minted(msg.sender, i, battleflyType, transactionId, signature, true);
        }
        currentId += amount;
    }
    ...
    function mintWhitelist(
        ...
        for (uint256 i = currentId; i < (currentId + toMint); ) {
            ...
            game.stakeSoulbound(msg.sender, i);
            {
                emit MintedWhitelist(msg.sender, i, battleflyType, merkleroot, index, true);
            }
        }
        currentId += toMint;
    }
    ...
    function totalMintedForTypeByAddress(address account, uint256 battleflyType) external view returns (uint256) {
        return mintedForType[battleflyType][account];
    }
