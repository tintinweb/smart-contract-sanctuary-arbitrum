...
contract AccruPresale_ is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public privateSaleMinPurchase;
    uint256 public publicSale1MinPurchase;
    uint256 public publicSale2MinPurchase;
    uint256 public reserveTotalSold;
    uint256 public publicSale2BTotalSold;
    mapping(address => uint256) public publicSale2BDeposits;
    ...
    modifier checkPurchaseAmount(uint256 amount) {
        uint256 totalAmount = stages[0][2] +
            stages[1][2] +
            stages[2][2] +
            stages[2][4];
    ...
    function calculatePrice(
        ...
        uint256 price = _amount * stages[_sale][_saleIndex];
        return price;
    }
    ...
    function verifyPublicSale1Purchase(
        ...
        return (amount >= publicSale1MinPurchase ||
            purchasedAmount + amount >= publicSale1MinPurchase);
    }
        ...
        returns (bool)
    {
        ...
        uint256 usdPrice = calculatePrice(amount, uint8(saleStage.privateSale), 3);
        ...
        returns (bool)
    {
        ...
        uint256 usdPrice = calculatePrice(amount, uint8(saleStage.publicSale1), 3);
        uint256 ethAmount = (usdPrice * baseDecimals) / getLatestPrice();
        require(msg.value >= ethAmount, "Less payment");
        uint256 excess = msg.value - ethAmount;
        if (isPurchaseLimitEnable) {
    ...
    /**
     * @dev To buy token in a public sale2A using ETH
     * @param amount No of tokens to buy
     */
    function publicSale2ATokenBuy(
        uint256 amount,
        bytes calldata sig
    )
        external
        ...
        returns (bool)
    {
        ...
        uint256 usdPrice = calculatePrice(amount, uint8(saleStage.publicSale2), 3);
        uint256 ethAmount = (usdPrice * baseDecimals) / getLatestPrice();
        require(msg.value >= ethAmount, "Less payment");
        uint256 excess = msg.value - ethAmount;
    ...
    /**
     * @dev To buy token in a public sale2B using ETH
     * @param amount No of tokens to buy
     */
    function publicSale2BTokenBuy(
        uint256 amount,
        bytes calldata sig
    )
        external
        ...
        returns (bool)
    {
        ...
        publicSale2BTotalSold += amount;
        require(
            publicSale2BTotalSold <= stages[2][4],
            "Amount exceeds max token to buy"
        );

        uint256 usdPrice = calculatePrice(amount, uint8(saleStage.publicSale2), 5);
        uint256 ethAmount = (usdPrice * baseDecimals) / getLatestPrice();
        require(msg.value >= ethAmount, "Less payment");
        uint256 excess = msg.value - ethAmount;

        nonces[_msgSender()]++;
        publicSale2BDeposits[_msgSender()] += (amount * baseDecimals);
        userDeposits[_msgSender()] += (amount * baseDecimals);
        usdRaised += usdPrice;
        sendValue(payable(paymentWallet), ethAmount);
        if (excess > 0) sendValue(payable(_msgSender()), excess);
    ...
    function ethBuyHelper(
        ...
        uint256 usdPrice = calculatePrice(amount, sale, saleIndex);
