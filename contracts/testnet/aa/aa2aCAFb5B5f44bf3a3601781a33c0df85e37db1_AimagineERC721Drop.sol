...
contract AimagineERC721Drop is ERC721A, IERC2981, ReentrancyGuard, AccessControl, IAimagineERC721Drop, Ownable {
    ...
    /**
     * @dev Number of normal mints required to trigger the royalty mint.
     * If `royaltyMintSchedule` is set to 0, the royalty mint feature is disabled.
     */
    ...
    /**
     * @dev Gas limit for sending funds during withdrawals.
     * This is used to prevent gas exhaustion during fund withdrawals.
     */
    ...
    /**
     * @dev Constant to indicate that royalty will be supplied for every mint.
     */
    ...
    /**
     * @dev The current index for token IDs (the next token ID to be minted).
     * The first token ID will be 1, as the contract starts with _startTokenId() = 1.
     */
    ...
    function _payoutAimagineFee(uint256 quantity) internal {
        ...
        (bool success,) = AIMAGINE_MINT_FEE_RECIPIENT.call{value: aimagineFee, gas: FUNDS_SEND_GAS_LIMIT}("");
    ...
    /**
     * @notice Internal function to mint supply royalties for a given mint quantity.
     * @param mintQuantity The quantity of tokens to mint the supply royalties for.
     */
    function _mintSupplyRoyalty(uint256 mintQuantity) internal {
        ...
        totalRoyaltyMints = Math.min(totalRoyaltyMints, config.editionSize - (mintQuantity + _totalMinted()));
    ...
    /**
     * @notice Allows the admin to update the royalty mint schedule.
     * @param newSchedule The new royalty mint schedule to be set.
     */
