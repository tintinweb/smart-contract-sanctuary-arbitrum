...
contract WrappedCharacter is WrappedCharacterActivity {
    using Strings for string;

// STATE VARIABLES

    // @dev Track the wrapped token id
    uint256 private wTokenID;
...
// STATE VARIABLES
    ...
    mapping(bytes32 => uint256[]) internal stats;
...
// EVENTS
    ...
    event StatIncreased(address indexed account, uint256 wrappedTokenID, uint256 amount, uint256 statIndex);
    event StatReduced(address indexed account, uint256 wrappedTokenID, uint256 amount, uint256 statIndex);
...
// EXTERNAL FUNCTIONS
    ...
    function increaseStamina(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
        _increaseStat(wrappedTokenID, amount, 0);
    }

    /// @dev Reduce a characters health but can't go below one
    /// @param wrappedTokenID Characters ID
    /// @param amount to reduce to the characters health
    function reduceStamina(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
    ...
    /// @param wrappedTokenID Characters ID
    /// @param amount to add to the characters health
    function increaseStrength(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
    ...
    /// @param wrappedTokenID Characters ID
    /// @param amount to reduce to the characters health
    function reduceStrength(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
    ...
    /// @param wrappedTokenID Characters ID
    /// @param amount to add to the characters health
    function increaseSpeed(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
    ...
    /// @param wrappedTokenID Characters ID
    /// @param amount to reduce to the characters health
    function reduceSpeed(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
    ...
    /// @param wrappedTokenID Characters ID
    /// @param amount to add to the characters health
    function increaseCourage(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
    ...
    /// @param wrappedTokenID Characters ID
    /// @param amount to reduce to the characters health
    function reduceCourage(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
    ...
    /// @param wrappedTokenID Characters ID
    /// @param amount to add to the characters health
    function increaseIntelligence(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
    ...
    /// @param wrappedTokenID Characters ID
    /// @param amount to reduce to the characters health
    function reduceIntelligence(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
    ...
    /// @param wrappedTokenID Characters ID
    /// @param amount to add to the characters health
    function setHealthTo(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
        (,,,,,uint256 health,,) = getStats(wrappedTokenID);
        if (amount > health) {
            _increaseStat(wrappedTokenID, amount - health, 5);
        } else {
            _decreaseStat(wrappedTokenID, health - amount, 5);
        }
    }
    ...
    /// @param wrappedTokenID Characters ID
    /// @param amount to add to the characters health
    function increaseHealth(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
    ...
    /// @param wrappedTokenID Characters ID
    /// @param amount to reduce to the characters health
    function reduceHealth(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
    ...
    function increaseExperience(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
        _increaseStat(wrappedTokenID, amount, 6);
    }

    /// @dev Reduce a characters experience
    /// @param wrappedTokenID Characters ID
    /// @param amount to reduce the characters experience
    function reduceExperience(uint256 wrappedTokenID, uint256 amount) external onlyAllowed onlyWrapped(wrappedTokenID) {
        _decreaseStat(wrappedTokenID, amount, 6);
    }

    /// @dev Increase a characters level by 1
    /// @param wrappedTokenID Characters ID
    function increaseLevel(uint256 wrappedTokenID) external onlyAllowed onlyWrapped(wrappedTokenID) {
        _increaseStat(wrappedTokenID, 1, 7);
    }

    /// @dev Increase a characters level by 1
    /// @param wrappedTokenID Characters ID
    function reduceLevel(uint256 wrappedTokenID) external onlyAllowed onlyWrapped(wrappedTokenID) {
        _decreaseStat(wrappedTokenID, 1, 7);
    }

// INTERNAL FUNCTIONS

    /// @dev INTERNAL: Import or generate character stats
        ...
        internal
    {
        ...
        (,,bool native) = getCollection(collectionAddress);
        if (native) {
            ...
            stamina = (randomNumbers[0] % 80) + 15;
            strength = (randomNumbers[1] % 80) + 15;
            speed = (randomNumbers[2] % 80) + 15;
            courage = (randomNumbers[3] % 80) + 15;
            intelligence = (randomNumbers[4] % 80) + 15;
        }
        // Calculate health
        ...
        stats[underlyingTokenHash].push(stamina);
        stats[underlyingTokenHash].push(strength);
        stats[underlyingTokenHash].push(speed);
        stats[underlyingTokenHash].push(courage);
        stats[underlyingTokenHash].push(intelligence);
        stats[underlyingTokenHash].push(health);
        stats[underlyingTokenHash].push(0); // experience
        stats[underlyingTokenHash].push(0); // level
    }

    /// @dev Increases a stat
    /// @param wrappedTokenID ID of the token
    function _increaseStat(uint256 wrappedTokenID, uint256 amount, uint256 statIndex)
        internal
    {
        bytes32 underlyingTokenHash = wrappedCharacter[wrappedTokenID];      // Get the underlying token hash
        uint256 currentStat = stats[underlyingTokenHash][statIndex];         // Get current stat
        uint256 maxStat = 99;                                                // Set standard max stat
        if (statIndex == 5) {
            maxStat = getMaxHealth(wrappedTokenID);                          // Health has a different calculation for max stat
        }
        if (statIndex > 5) {
            maxStat = 10000000;                                              // Experience & Levels have a higher max stat
        }
    ...
    /// @param wrappedTokenID ID of the token
    function _decreaseStat(uint256 wrappedTokenID, uint256 amount, uint256 statIndex)
        internal
    {
        bytes32 underlyingTokenHash = wrappedCharacter[wrappedTokenID];    // Get the underlying token hash
        uint256 currentStat = stats[underlyingTokenHash][statIndex];       // Get current stat
        ...
        if (currentStat > amount) {                                        // Check to see if we'll go below the minimum stat of 1
            stats[underlyingTokenHash][statIndex] -= amount;               // Reduce stat
        } else {
            stats[underlyingTokenHash][statIndex] = 1;                     // Set to minimum of 1
        }
        emit StatReduced(_msgSender(), wrappedTokenID, amount, statIndex); // Write an event to the chain
    }
...
// VIEW FUNCTIONS
    ...
    function getStats(uint256 wrappedTokenID)
        ...
        view
        returns (
            uint256 stamina, uint256 strength, uint256 speed, uint256 courage, uint256 intelligence, uint256 health, uint256 experience, uint256 level
        )
    {
        bytes32 underlyingTokenHash = wrappedCharacter[wrappedTokenID]; // Get the underlying token hash
    ...
    {
        (uint256 stamina, uint256 strength,,,,,,) = getStats(wrappedTokenID); // Get Stats
...
// CONSTRUCTOR
    ...
    constructor ()
    ERC721A("Wrapped Characters", "wCHARACTER") {}
...
// EVENTS
    ...
    event Wrapped(address indexed account, address indexed collection, uint256 blockNumber, uint256 tokenID, uint256 wrappedTokenID);
    event Unwrapped(address indexed account, address indexed collection, uint256 blockNumber, uint256 tokenID, uint256 wrappedTokenID);
...
// FUNCTIONS
        ...
        isWrappable(collectionAddress)
    {
        ...
        emit Wrapped(_msgSender(), collectionAddress, block.number, tokenID, wrappedTokenID);  // Write an event
        ...
        nonReentrant
    {
        ...
        emit Unwrapped(_msgSender(), collectionAddress, block.number, tokenID, wrappedTokenID); // Write an event
...
// EVENTS
    ...
    event BeginActivitySet(address indexed account, uint256 wrappedTokenID, uint256 activityDuration, uint256 numberOfActivities, uint256 startBlock, uint256 endBlock); 

// EXTERNAL FUNCTIONS

    /// @dev Sets the character active on an activity such as a quest
    /// @param wrappedTokenID ID of the token
    /// @param active / inactive (true/false)
    function setActive(uint256 wrappedTokenID, bool active) external onlyAllowed onlyWrapped(wrappedTokenID) {
        _setActive(wrappedTokenID, active);
    }

    /// @dev Begin a characters activity
    /// @param wrappedTokenID Characters ID
    ...
    function setBeginActivity(uint256 wrappedTokenID, uint256 activityDuration, uint256 numberOfActivities, uint256 startBlock, uint256 endBlock) external onlyAllowed onlyWrapped(wrappedTokenID) {
        _setBeginActivity(wrappedTokenID, activityDuration, numberOfActivities, startBlock, endBlock);
    }
...
// INTERNAL FUNCTIONS
    ...
    function _setActive(uint256 wrappedTokenID, bool active)
        internal
    {
    ...
    /// @param startBlock the duration of the activity
    /// @param endBlock the duration of the activity
    function _setBeginActivity(uint256 wrappedTokenID, uint256 activityDuration, uint256 numberOfActivities, uint256 startBlock, uint256 endBlock)
        internal
    {
...
//VIEWS
        ...
        )
    {
        ...
        uint256 maxHealth = getMaxHealth(wrappedTokenID);                                // Get characters max health
...
// STATE VARIABLES
    ...
    Collection[] public characterCollections;
...
// MODIFIERS
    ...
    modifier isWrappable (address collectionAddress) {
        (bool enabled,,) = getCollection(collectionAddress);
        require(enabled,"Not enabled for wrapping");
        _;
    }
...
// FUNCTIONS
    ...
    /// @param collectionAddress address of the NFT collection
    function enableCollection(address collectionAddress, bool native)
        external
        onlyOwner
    {
        characterCollections.push(Collection(collectionAddress,native));
    }
    ...
    function disableCollection(address collectionAddress)
        ...
        onlyOwner
    {
        (bool enabled, uint256 index,) = getCollection(collectionAddress);
        require(enabled,                                                    "Collection not found");
        characterCollections[index] = 
            characterCollections[characterCollections.length - 1];          // Overwrite the item to delete with the last item in the array
        characterCollections.pop();                                         // Delete the last item in the array
    }

// VIEWS

    /// @dev Returns a list of all collections
    function getCollections()
        external
        view
        returns (Collection[] memory allCollections) 
    {
        return characterCollections; // Return the array
    }
    ...
    {
        uint256 total = characterCollections.length;                                    // Store total number of collections into a local variable
        if ( total == 0 ) {                                                             // If no collections added, return disabled
            return (false, 0, false);
        } else {
            for(uint256 i = 0; i < total; i++){                                         // Loop through the collections
                if (characterCollections[i].collectionAddress == collectionAddress) {   // Until the address is found
                    return (true, i, characterCollections[i].native);                 // if found return true & CollectionID & if stats are passthrough
                }
            }
        }
    }
