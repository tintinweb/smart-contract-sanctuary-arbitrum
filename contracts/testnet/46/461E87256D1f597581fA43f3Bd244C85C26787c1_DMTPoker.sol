...
contract DMTPoker is OwnableOracle, DMTPokerChips {
    IERC20 public dmt;
    ...
    function convertToChips(uint256 amount) public {
    ...
    function payouts(address[] memory recipients, uint256[] memory amounts) public onlyOracle {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            // Try-catch statement so 1 failed payout doesn't stop the rest
            try this.individualPayout(recipient, amount) {
                successfulPayouts++;
            } catch {
