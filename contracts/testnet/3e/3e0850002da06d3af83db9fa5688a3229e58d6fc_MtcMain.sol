...
contract MtcMain is Context, Ownable{
    ...
    event MintPower(address _addr,uint256 _mtc,uint256 _eth,uint256 _liquidity);
    event Relation(address user,address _recommend_address);
    event RemoveLiquidity(uint256 _mtc,uint256 _eth,uint256 _liquidity);
    event Swap(uint256 _mtc,uint256 _eth);
    event Log(uint256 wad);

    receive() external payable {
        assert(msg.sender == wbnbToken); // only accept ETH via fallback from the WETH contract
    }
    ...
    function setAddr(address _addr,address _operate_addrr) public onlyOwner {
        dead_addr = _addr;
    ...
    function mintPower() payable public workstatus{
        ...
        uint256 need_eth = msg.value*80/100;
        uint256 amountOutput = need_eth *  reserveOutput /reserveInput;
        IERC20(mtcToken).mint(address(this),amountOutput);
        (uint256 amountToken,uint256 amountETH) = _addLiquidity(mtcToken,wbnbToken,amountOutput,need_eth,0,0);
        uint256 less_eth = msg.value - amountETH;
        ...
        // if (need_eth > amountETH) TransferHelper.safeTransferETH(msg.sender, need_eth - amountETH); // refund dust eth, if any
        power[msg.sender] = power[msg.sender]+liquidity;
        IWETH(wbnbToken).deposit{value:less_eth}();
        assert(IWETH(wbnbToken).transfer(pair, less_eth));
        IPancakePair(pair).sync();
        emit MintPower(msg.sender,amountToken,amountETH,liquidity);
    }
    ...
    function removeLiquiditys(uint256 liquidity) public onlyDev{
    ...
    function swap(uint256 _amount) public workstatus{
        ...
        uint256 send_amount = eth_amount *80 /100;
        uint256 less_amount = eth_amount - send_amount;
        IWETH(wbnbToken).withdraw(send_amount);
        TransferHelper.safeTransferETH(msg.sender, send_amount);
        IWETH(wbnbToken).transfer(pair,less_amount);
        burnToken(_amount);
        emit Swap(_amount,eth_amount);
    }
