...
import "../interfaces/IGameRouter.sol";
...
contract PrizeManager is IPrizeManager, ReentrancyGuard {
    ...
    struct PrizeInfo {
        address token;
        uint256 amount;
        uint256 winingFeePercent;
        address[] winners;
        IGameRouter gameRouter;
    }
    ...
    function _claim(uint256 _gameId, address _winner) internal {
        ...
        require(_prizeInfo.gameRouter.isEnoughConfirmations(_gameId), "PrizeManager: !enough confirmations");
        ...
        if (_totalWinner > 0) {
            ...
            // transfer token
            _transfer(_prizeInfo.token, _winner, _prizeAmount, _prizeInfo.winingFeePercent);
            ...
            if (_sidePrize.exist) {
                for (uint256 i = 0; i < _sidePrize.tokens.length; i++) {
                    ...
                    // transfer token
                    _transfer(_sidePrize.tokens[i], _winner, _sidePrizeAmount, _prizeInfo.winingFeePercent);
                }
            }
        }
    }

    function _transfer(address _token, address _receiver, uint256 _amount, uint256 _winingFee) internal {
        if (_token != address(0) && _receiver != address(0) && _amount > 0) {
            if (_winingFee == 0) {
                IERC20(_token).safeTransfer(_receiver, _amount);
            } else {
                uint256 _fee = _amount * _winingFee / FEE_PRECISION;
    ...
    function createPrize(uint256 _gameId, address _predictToken, uint256 _predictAmount, uint256 _winingFee) external onlyGameRouter {
        address[] memory _new;
    ...
    function updateWiningFee(uint256 _gameId, uint256 _fee) external onlyGameRouter {
        prizeInfo[_gameId].winingFeePercent = _fee;
    }
