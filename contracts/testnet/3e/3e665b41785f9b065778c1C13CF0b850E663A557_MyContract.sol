...
contract MyContract is
  ...
  event AnyMessage(Client.EVM2AnyMessage message);
...
// Selector for the destination chain (in the CCIP protocol)
  ...
  // Initializer - sets up the contract ready for use
  function initialize() public initializer {
    __Ownable_init();
    __UUPSUpgradeable_init();
  }

  // Function to authorize upgrades to the contract
  ...
  // Function to set the router for cross-chain interactions
  function setRoute(address _router) public onlyOwner {
     router = IRouterClient(_router);
  }

  // Function to set the router for cross-chain interactions
  function setFeeToken(address feeToken) public onlyOwner {
    s_feeToken = IERC20Upgradeable(feeToken);
    s_feeToken.approve(address(router), 2 ** 256 - 1);
  }

  // TODO: permissions on enableChain/disableChain
  function enableChain(uint64 chainSelector) external {
    Client.EVMExtraArgsV1 memory extrargsV1 = Client.EVMExtraArgsV1({
        gasLimit: 3000000,
        strict: false
    });
    s_chains[chainSelector] = Client._argsToBytes(extrargsV1);
  }
  ...
  // Function to transfer tokens from the current chain to another chain
  function transferToken(
    ...
    // Check balance for msg.sender
    IERC20Upgradeable sourceTokenContract = IERC20Upgradeable(token);
    uint256 balance = sourceTokenContract.balanceOf(msg.sender);
    require(balance >= amount, 'Not enough tokens');

    sourceTokenContract.transferFrom(msg.sender, address(this), amount);
    sourceTokenContract.approve(address(router), amount);
    Client.EVMTokenAmount[] memory tokenAmounts = new Client.EVMTokenAmount[](1);
    tokenAmounts[0] = Client.EVMTokenAmount(token, amount);
