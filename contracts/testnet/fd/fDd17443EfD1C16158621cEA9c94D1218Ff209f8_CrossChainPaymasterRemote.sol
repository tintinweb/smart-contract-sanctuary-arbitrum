...
contract CrossChainPaymasterRemote is NonblockingLzApp {
    ...
    function _nonblockingLzReceive(
        ...
        ) = abi.decode(_payload, (bytes, address, address, string));       

        //1. Credit the remoteAddress the locked up amount.
        credits[remoteAddress] = Credit(senderAddress, domain, msg.value, msg.value); //TODO: confirm how this works; msg.value is not likely equal to the msg.value of the initiating TX on the remote chain.
    ...
    function confirmDomain(
        ...
        emit Gas("Start of confirmDomain", gasleft());
        
        //TODO: how to pay for transaction using GSN style paymaster

        //Call "confirmDomain" for provided remoteAddress
        //TODO: how to get msg.sender to equal remoteAddress in RNS call? Is this even possible or does it require a different verification? and therefore logic change in RNSRemote contract
        rnsRemote.confirmDomain(LOCAL_CHAIN_ID, domain);

        emit Gas("After rns.confirmDomain", gasleft());

        //TODO: send message to remote Paymaster with final TX cost
        //Encode the sender's address, remote address, and the domain into the LZ payload.
        uint256 gasUsed = 1; //TODO: calculate gas used.
        bytes memory payload = abi.encode(
            RESPONSE,
            msg.sender,
            localAddress,
            domain,
            gasleft() //TODO: account for cost of _lzsend
        );

        emit Gas("After abi.encode", gasleft());
        ...
        _lzSend(
            ...
            address(0x0),
            bytes(""),
            credits[localAddress].gasLeft //TODO: calculate initial_lockup_amount - confirm_domain_cost - lz_response_cost
        );

        emit Gas("End of confirmDomain", gasleft());

        //TODO: IDEA: no need to report back the gas left, let the "client" query this contract to read from storage the amount left; set this amount after _lzSend is invoked
        credits[msg.sender].gasLeft = gasleft();
    }
}
