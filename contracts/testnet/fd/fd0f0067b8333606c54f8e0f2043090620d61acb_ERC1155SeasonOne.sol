...
abstract contract CoreRef is Pausable {
    ...
    function pause() external hasAnyOfThreeRoles(Roles.ADMIN, Roles.GOVERNOR_DAO_PROTOCOL_ROLE, Roles.GUARDIAN) {
        _pause();
    }

    /// @notice set pausable methods to unpaused
    /// callable by admin or token governance
    function unpause() external hasAnyOfThreeRoles(Roles.ADMIN, Roles.GOVERNOR_DAO_PROTOCOL_ROLE, Roles.GUARDIAN) {
        _unpause();
    }
...
interface IGlobalReentrancyLock {
    ...
    function adminEmergencyRecover() external;
