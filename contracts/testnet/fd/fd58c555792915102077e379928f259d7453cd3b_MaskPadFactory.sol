...
contract MaskPadERC20 is IPancakeERC20 {
    using SafeMath for uint;

    string public constant name = 'MaskPad LPs';
...
contract MaskPadPair is IPancakePair, MaskPadERC20 {
    using SafeMath  for uint;
...
contract MaskPadFactory is IPancakeFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(MaskPadPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        bytes memory bytecode = type(MaskPadPair).creationCode;
