...
contract SynthrAggregator is OwnableUpgradeable {
    ...
    event ToggleBridge(address indexed bridge, bool added);

    modifier onlyBridge() {
        require(_bridges[msg.sender], "Caller is not SynthrBridge");
        _;
    }

    function bridges(address _bridge) external view returns (bool) {
        return _bridges[_bridge];
    }
    ...
    function config(uint16 __selfLZChainId) external onlyOwner {
        _selfLZChainId = __selfLZChainId;
    }

    function toggleBridge(address _bridge) external onlyOwner {
        bool newStatus = !_bridges[_bridge];
        _bridges[_bridge] = newStatus;

        emit ToggleBridge(_bridge, newStatus);
    }
    ...
    function withdrawCollateral(
        ...
        amount = preAmount > amount ? amount : preAmount;
        _collateralByIssuerAggregation[collateralKey][chainId][account] -= amount;
        _collateralByIssuer[collateralKey][account] -= amount;
    }
    ...
    function burnSynth(
        ...
        synthAmount = preAmount > synthAmount ? synthAmount : preAmount;
        _chainSynthTotalSupply[synthKey][chainId] -= synthAmount;
        _synthTotalSupply[synthKey] -= synthAmount;
    }
    ...
    function exchangeSynth(
        ...
        sourceAmount = preAmount > sourceAmount ? sourceAmount : preAmount;
        _chainSynthTotalSupply[sourceKey][srcChainId] -= sourceAmount;
        _synthTotalSupply[sourceKey] -= sourceAmount;
        ...
        _synthTotalSupply[bytes32("sUSD")] += fee;
    }
    ...
    function liquidate(
        ...
        uint preAmount = _collateralByIssuerAggregation[collateralKey][destChainId][account];
        amount = preAmount > amount ? amount : preAmount;
        _collateralByIssuerAggregation[collateralKey][destChainId][account] -= amount;
        _collateralByIssuer[collateralKey][account] -= amount;
    }
