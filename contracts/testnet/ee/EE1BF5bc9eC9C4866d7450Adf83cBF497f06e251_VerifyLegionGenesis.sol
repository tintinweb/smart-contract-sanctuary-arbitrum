...
contract VerifyLegionGenesis is OwnableUpgradeable {
    ...
    function balanceOf(address owner_) external view returns (uint256 balance) {
        balance += tokensToBalance(tokensInAtlasMine(owner_));
        balance += tokensToBalance(tokensInBridgeworld(crafting, owner_));
        balance += tokensToBalance(tokensInBridgeworld(questing, owner_));
        balance += tokensToBalance(tokensInBridgeworld(summoning, owner_));
    }
    ...
    function tokensInAtlasMine(address owner_)
        internal
        view
        returns (uint256[] memory)
    {
        return atlas.getStakedLegions(owner_);
    }

    function tokensInBridgeworld(IBridgeworldLegion bridgeworld, address owner_)
        internal
        view
        returns (uint256[] memory)
    {
        return bridgeworld.getStakedLegions(owner_);
    }
}
