...
contract PancakeV3Factory is IPancakeV3Factory {
    ...
    modifier onlyOwnerOrLmPoolDeployer() {
        require(msg.sender == owner || msg.sender == lmPoolDeployer, "owner or LM pool deployer");
        _;
    }
    ...
    function collectProtocol(
        ...
        return IPancakeV3Pool(pool).collectProtocol( amount0Requested, amount1Requested);
    }

    function setInnerBribe(address pool,address innerBribe) external override onlyOwner {
      IPancakeV3Pool(pool).setInnerBribe(innerBribe);
    }
}