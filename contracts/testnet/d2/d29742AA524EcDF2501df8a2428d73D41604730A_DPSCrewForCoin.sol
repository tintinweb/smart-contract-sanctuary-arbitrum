...
contract DPSCrewForCoin is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    /// @notice a general flag that all assets borrowed before a certain block to be automatically expired
    uint256 public generalExpireTimestmap;
    ...
    function redeemDPS(uint256 _tokenId) public nonReentrant {
        ...
        Asset storage asset = assets[dps][_tokenId];
        if (asset.borrower == address(0) || !asset.borrowed) revert NotListed();
        if (!isDPSExpired(_tokenId)) revert NotExpired();
    ...
    function redeemFlagship(uint256 _tokenId) public nonReentrant {
        ...
        Asset storage asset = assets[flagship][_tokenId];
        if (asset.borrower == address(0) || !asset.borrowed) revert NotListed();
        if (!isFlagshipExpired(_tokenId)) revert NotExpired();
    ...
    function beforeTransferHookDPS(
        ...
        // we let a token to be transferred only to the borrower or burned if expired
        if (isDPSExpired(_tokenId) && (_to != asset.borrower && _to != address(0))) revert Expired();
    }
    ...
    function afterTransferHookDPS(
        ...
        if (isDPSExpired(_tokenId) && _to == asset.borrower) {
            redeemDPS(_tokenId);
        }
    }
    ...
    function beforeTransferHookFlagship(
        ...
        // we let a token to be transferred only to the borrower if expired
        if (isFlagshipExpired(_tokenId) && (_to != asset.borrower && _to != address(0))) revert Expired();
    }
    ...
    function afterTransferHookFlagship(
        ...
        // we burn the token once it's expired
        if (isFlagshipExpired(_tokenId) && _to == asset.borrower) {
    ...
    function isDPSExpired(uint256 _assetId) public view returns (bool) {
        Asset memory asset = assets[dps][_assetId];
        return asset.endTime < block.timestamp || asset.startTime <= generalExpireTimestmap;
    }

    function isFlagshipExpired(uint256 _assetId) public view returns (bool) {
        Asset memory asset = assets[flagship][_assetId];
        return asset.endTime < block.timestamp || asset.startTime <= generalExpireTimestmap;
    }
    ...
    function setGeneralExpireTimestmap(uint256 newGeneralExpireTimestmap) external onlyOwner {
        generalExpireTimestmap = newGeneralExpireTimestmap;
        emit SetGeneralExpireTimestmap(newGeneralExpireTimestmap);
    }
    ...
    event SetGeneralExpireTimestmap(uint256 _newGeneralExpireTimestmap);
}
