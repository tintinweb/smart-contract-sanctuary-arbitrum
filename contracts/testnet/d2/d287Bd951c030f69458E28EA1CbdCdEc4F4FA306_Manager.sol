...
contract Manager is CredentialVerifier, IManager2, AccessControl {
    ...
    address public immutable credentialVerifier;
    ...
    function airdropOvercap(uint256 round, uint256 size)
        external
    ...
    function setRoundData(uint256 round, ILaunchpad.Launch memory data)
        external
        onlyRole(LP_ADMIN_ROLE)
    {
        require(rounds.length > round, 'bad round');
    ...
    function setStableToken(uint256 round, address router)
        external
        onlyRole(LP_ADMIN_ROLE)
    {
        require(rounds.length > round, 'bad round');
