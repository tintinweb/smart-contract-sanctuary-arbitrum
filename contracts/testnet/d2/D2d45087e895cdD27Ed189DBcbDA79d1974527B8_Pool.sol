...
// File src/vault/ConcordVault.sol

pragma solidity ^0.8.17;

/// @title ConcordVault
/// @notice Vault to hold one asset and represent as collateral in Concord system
contract ConcordVault is ERC4626, Pausable, Ownable {
    /// -----------------------------------------------------------------------
    /// Mutable Storage
    /// -----------------------------------------------------------------------
    ...
    constructor(
        ...
        uint8 decimalOffset_
    ) ERC4626(IERC20(asset_)) ERC20(name_, symbol_) ERC20Permit(name_) {
        _decimalOffset = decimalOffset_;
    }
    ...
    function previewRedeem(
        ...
        return super.previewRedeem(assets);
    }
    ...
    function depositWithPermit(
        ...
        IERC20Permit(super.asset()).permit(
            _msgSender(),
            address(this),
            assets,
            deadline,
    ...
    {
        return IChainlinkAggregatorV3(asset).latestRoundData();
    }
}
...
/// @title Pool
/// @notice create, manage, interact with vault
contract Pool is IPool, Ownable {
    /// -----------------------------------------------------------------------
    /// Mutable Storage
    /// -----------------------------------------------------------------------

    address[] internal _cVaultList;

    mapping(address => address) internal _assetToCVaultMap; // this map is just for quick retrieval

    mapping(address => IChainlinkAggregatorV3) internal _priceFeed; // chainlinkUSDC, chainlinkWETH, chainlinkWBTC,...

    /// -----------------------------------------------------------------------
    /// Modifier
    /// -----------------------------------------------------------------------

    modifier verifyAssetExistence(address underlyingAsset_) {
        address cVault = _assetToCVaultMap[underlyingAsset_];
        if (cVault == address(0)) revert CVaultNotExist();
        if (IERC4626(cVault).asset() != underlyingAsset_)
            revert GivenAssetNotMatchUnderlyingAsset();
        _;
    }
    ...
    function isAssetAllowed(address cVault) public view returns (bool) {
        for (uint256 i = 0; i < _cVaultList.length; i++) {
            if (_cVaultList[i] == cVault) {
                return true;
            }
        }
        return false;
    ...
    function getVaultDecimals() external pure override returns (uint8) {
        return 27;
    }

    function getPriceFeed(
        address cVault
    ) public view override returns (IChainlinkAggregatorV3) {
        return _priceFeed[cVault];
    }
    ...
    function verifyDecimals(
        ...
        uint8 vaultDecimals = IERC20Metadata(asset_).decimals() +
            decimalOffset_;
        if (vaultDecimals != 27) revert InvalidVaultDeicmals(vaultDecimals);
        return true;
    }

    function getCVaultList() public view override returns (address[] memory) {
        return _cVaultList;
    }

    function getCVaultByAsset(address asset_) public view returns (address) {
        return _assetToCVaultMap[asset_];
    }
    ...
    function createVault(
        ...
        // Verify decimals vault decimal must be express in Ray
        verifyDecimals(asset_, decimalOffset_);

        // Verify given asset
        if (_assetToCVaultMap[asset_] != address(0))
            revert UnderlyingAssetExisted(asset_);

        bytes memory encodedArguments = abi.encodePacked(
            type(ConcordVault).creationCode,
            abi.encode(asset_, name_, symbol_, decimalOffset_)
        );

        bytes32 salt_ = keccak256(
            abi.encodePacked(msg.sender, salt, block.chainid)
        );
        address vault;

        // Create new vault using create2
        assembly {
            vault := create2(
                0,
                add(encodedArguments, 32),
                mload(encodedArguments),
                salt_
            )
        }

        ConcordVault(vault).transferOwnership(msg.sender);

        _cVaultList.push(vault);
        _assetToCVaultMap[asset_] = vault;
        setPriceFeed(vault, priceFeed_);

        emit VaultCreated(asset_, vault, msg.sender, _cVaultList.length - 1);

        return vault;
    }

    function setPriceFeed(address vault, address priceFeed_) public onlyOwner {
        _priceFeed[vault] = IChainlinkAggregatorV3(priceFeed_);
    }

    /// @dev supply help user supply underlyingAsset like WETH, DAI, WBTC into concord system
    /// @dev Concord Pool will convert underlyingAsset into CVault - a kind of ERC4626 Vault
    function supply(
        address asset,
        uint256 amount,
        address receiver
    ) public override verifyAssetExistence(asset) {
        _executeSupply(asset, amount, receiver);
    }
    ...
    function _executeSupply(
        ...
        address cVault = _assetToCVaultMap[asset];

        // Approve vault to transfer asset from pool to vault
        SafeERC20.safeApprove(IERC20(asset), cVault, amount);

        // Deposit asset to vault by sending asset from pool to vault, then receive shares at receiver address
        IERC4626(cVault).deposit(amount, receiver);

        emit AssetSupplied(asset, msg.sender, receiver, amount);
    }

    function previewWithdraw(
        address asset,
        uint256 amount
    ) external view verifyAssetExistence(asset) returns (uint256) {
        address cVault = _assetToCVaultMap[asset];
        return ConcordVault(cVault).previewWithdraw(amount);
    }
    ...
    function withdraw(
        ...
        _executeWithdraw(asset, amount, receiver);
    }
    ...
    function withdrawWithPermit(
        ...
        address cVault = _assetToCVaultMap[asset];
        uint256 shares = ConcordVault(cVault).previewWithdraw(amount);
        IERC20Permit(cVault).permit(
            msg.sender,
            address(this),
            shares,
            dealine,
    ...
    function _executeWithdraw(
        ...
        ConcordVault(cVault).withdraw(amount, receiver, address(this));

        emit AssetWithdrawed(asset, msg.sender, receiver, amount);
    }

    function balanceOf(
        address asset,
        address owner
    ) external view override verifyAssetExistence(asset) returns (uint256) {
        return ConcordVault(_assetToCVaultMap[asset]).balanceOf(owner);
    }
}