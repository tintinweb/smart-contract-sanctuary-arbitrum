...
contract PNFT is ERC721, ERC2981, ERC721Enumerable, Ownable {
    ...
    function setName(uint256 tokenId, string calldata name) external onlyOwner {
        ...
        emit MetadataChanged(tokenId, "name", oldValue, name);
    }

    function setExternalUrl(uint256 tokenId, string calldata externalUrl) external onlyOwner {
        require(_exists(tokenId), "ERC721: invalid token ID");
        string memory oldValue = _tokenMetadata[tokenId].externalUrl;
        _tokenMetadata[tokenId].externalUrl = externalUrl;
        emit MetadataChanged(tokenId, "externalUrl", oldValue, externalUrl);
    }
    ...
    function setImageUrl(uint256 tokenId, string calldata imageUrl) external {
        ...
        _tokenMetadata[tokenId].imageUrl = imageUrl;
        emit MetadataChanged(tokenId, "imageUrl", oldValue, imageUrl);
    }
    ...
    function setDescription(uint256 tokenId, string calldata description) external {
        ...
        emit MetadataChanged(tokenId, "description", oldValue, description);
    }

    /**
     * really wish otherImageUrls could be calldata
     */
    function setOtherImageUrls(uint256 tokenId, string[] memory otherImageUrls) external {
        require(_exists(tokenId), "ERC721: invalid token ID");
        require(_isApprovedOrOwner(_msgSender(), tokenId), "Caller is not NFT owner nor approved");
        string[] memory oldValue = _tokenMetadata[tokenId].otherImageUrls;
        _tokenMetadata[tokenId].otherImageUrls = otherImageUrls;
        emit MetadataChanged(tokenId, "otherImageUrls", oldValue, otherImageUrls);
    }
