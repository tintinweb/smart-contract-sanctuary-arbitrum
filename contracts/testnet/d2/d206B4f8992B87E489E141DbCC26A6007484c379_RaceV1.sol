...
import {IPachi721} from "../NFTManager/IPachi721.sol";

import {IERC20, SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

/// @title nekopachi slot
/// @author munji ([email protected])
contract RaceV1 is Game {
    using SafeERC20 for IERC20;
    ...
    function setPachi721(address _pachi721) external onlyOwner {
        pachi721 = IPachi721(_pachi721);
    }
    ...
    mapping(uint256 => uint256) randIdToRaceId;
    uint256[] openRaces;
    ...
    function initRace(uint256 betUnit, address token) external whenNotPaused {
        ...
        */
        ...
        if (betUnit < minBetUnit * 100) {
            revert UnderMinBetUnit(betUnit);
        }

        uint256 maxBetUnit = bank.getMaxBetUnit(token);
        if (betUnit > maxBetUnit * 100) {
            revert ExceedMaxBetUnit(betUnit);
        }
    ...
    function enterRace(
        ...
        require(pachi721.ownerOf(_tokenId) == msg.sender, "not owner");
        require(
        ...
        for (uint16 i = 0; i < uint16(numOfPlayers); i++) {
            ...
            if (i == winnerIndex) {
                ...
                bank.payout{value: isGasToken ? betAmountFee : 0}(
                    winnerAddress,
                    _token,
                    totalInput - betUnit, // profit (게이머 입장에서 수익)
    ...
    function refundRace(uint256 _raceId) external {
        ...
        for (uint16 i = 0; i < uint16(numOfPlayers); i++) {
            uint256 tokenId = race.playerTokenIds[i];
            address owner = pachi721.ownerOf(tokenId);
