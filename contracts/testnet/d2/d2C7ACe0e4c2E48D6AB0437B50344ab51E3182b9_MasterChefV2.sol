...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///     FAIR rewards each pool gets.
    /// `accTokenPerShare` Accumulated FAIRs per share, times 1e12.
    ...
    ///     the FAIR rewards to all the PantokenSwap products.
    ...
    /// @notice Address of FAIR contract.
    IERC20 public immutable FAIR;
    ...
    uint256 public MASTERCHEF_FAIR_PER_BLOCK = 40 * 1e18;
    uint256 public constant ACC_FAIR_PRECISION = 1e18;

    /// @notice Basic boost factor, none boosted user's boost factor
    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
    uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
    /// @notice total token rate = toRegular + toSpecial
    uint256 public constant FAIR_RATE_TOTAL_PRECISION = 1e12;
    /// @notice FAIR distribute % for regular farm pool
    uint256 public tokenRateToRegularFarm = 1e12;
    /// @notice FAIR distribute % for special pools
    uint256 public tokenRateToSpecialFarm =
        FAIR_RATE_TOTAL_PRECISION - tokenRateToRegularFarm;
    ...
    event UpdateTokenRate(uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// @param _FAIR The FAIR token contract address.
    constructor(IERC20 _FAIR) {
        FAIR = _FAIR;
    }
    ...
    /// only for FAIR distributions within PantokenSwap products.
    function add(
        uint256 _allocPoint,
        IERC20 _lpToken,
        bool _isRegular,
        ...
        // stake FAIR token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != FAIR, "FAIR token can't be added to farm pools");
        require(!isPool[_lpToken], "pool is exist");
    ...
    /// @notice Update the given pool's FAIR allocation point. Can only be called by the owner.
    ...
    function set(
        ...
        require(isPool[lpToken[_pid]], "pool is not exist");
    ...
    /// @notice View function for checking pending FAIR rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    ...
    function pendingToken(
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 tokenReward = multiplier
                .mul(tokenPerBlock(pool.isRegular))
                .mul(pool.allocPoint)
                .div(
                    (
                        pool.isRegular
            ...
            accTokenPerShare = accTokenPerShare.add(
                tokenReward.mul(ACC_FAIR_PRECISION).div(lpSupply)
            );
        }
        ...
        uint256 boostedAmount = user
            ...
            .div(BOOST_PRECISION);
        return
            boostedAmount.mul(accTokenPerShare).div(ACC_FAIR_PRECISION).sub(
                user.rewardDebt
            );
    }

    function updatePerBlock(uint256 _amount) external onlyOwner {
        massUpdatePools();
        MASTERCHEF_FAIR_PER_BLOCK = _amount;
    }
    ...
    /// @notice Calculates and returns the `amount` of FAIR per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function tokenPerBlock(
        bool _isRegular
    ) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_FAIR_PER_BLOCK.mul(tokenRateToRegularFarm).div(
                FAIR_RATE_TOTAL_PRECISION
            );
        } else {
            amount = MASTERCHEF_FAIR_PER_BLOCK.mul(tokenRateToSpecialFarm).div(
                FAIR_RATE_TOTAL_PRECISION
            );
        }
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        pool = poolInfo[_pid];
        if (block.number > pool.lastRewardBlock && isPool[pool.lpToken]) {
            uint256 lpSupply = pool.totalBoostedShare;
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            poolInfo[_pid].totalDeposit = poolInfo[_pid].totalDeposit.sub(
                _amount
            );
    ...
    /// @notice Update the % of FAIR distributions for regular pools and special pools.
    /// @param _regularFarmRate The % of FAIR to regular pools each block.
    /// @param _specialFarmRate The % of FAIR to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateTokenRate(
        ...
        require(
            _regularFarmRate.add(_specialFarmRate) == FAIR_RATE_TOTAL_PRECISION,
        ...
        emit UpdateTokenRate(_regularFarmRate, _specialFarmRate);
    }
    ...
    /// @notice Settles, distribute the pending FAIR rewards for given user.
    ...
    function settlePendingToken(
        ...
        uint256 accToken = boostedAmount
            .mul(poolInfo[_pid].accTokenPerShare)
            .div(ACC_FAIR_PRECISION);
    ...
    /// @notice Safe Transfer FAIR.
    /// @param _to The FAIR receiver address.
    /// @param _amount transfer FAIR amounts.
    function _safeTransfer(
        address _to,
        uint256 _amount
    ) internal returns (uint256) {
        if (_amount > 0) {
            uint256 balance = FAIR.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }

            FAIR.safeTransfer(_to, _amount);
            return _amount;
        }
        return 0;
    }
