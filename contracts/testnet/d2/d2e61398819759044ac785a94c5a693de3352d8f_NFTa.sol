    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(uint16 => DataWraper.Data) public totalIssueNumber;
    mapping(uint16 => DataWraper.Data) public leftNumber;
    mapping(uint256 => uint256) public tokenInfo;

    IOperatorFilter private operatorFilter_;

    function setOperatorFilter(
        IOperatorFilter filter
    ...
    function __NFTa_init_unchained(
        string memory,
        string memory
    ...
    function initialize(
        ...
        __ERC721AQueryable_init_unchained();
        __ERC721ABurnable_init_unchained();
        __NFTa_init_unchained(_name, _symbol);
        _grantRole(MINTER_ROLE, _boxRouter);
    ...
    function mintNFT(
        ...
        address user,
        uint8 class
    ) public override nonReentrant onlyRole(MINTER_ROLE) {
        DataWraper.Data memory oldLeftNumber = leftNumber[_productId];
        (uint32[6] memory oldNumbers, , ) = oldLeftNumber.getData();
        uint256 _index = uint256(class - 1);
        require(0 < oldNumbers[_index], "mn err");
        __mint(user, oldNumbers[_index], _productId, class);
        oldNumbers[_index] = oldNumbers[_index] - 1;
        oldLeftNumber.updateNums(oldNumbers);
        leftNumber[_productId] = oldLeftNumber;
    }
    ...
    function leftInfo(
        ...
        returns (DataWraper.Data memory)
    {
        return leftNumber[_productId];
    }
    ...
    function mintBatch(
        ...
        uint total = 0;
        (uint32[6] memory _newLeftNumbers, , ) = _newData.getData();
        DataWraper.Data memory olNumber = leftNumber[_productId];
        (uint32[6] memory oldNumbers, , ) = olNumber.getData();
        uint256 _startTokenId = _nextTokenId();
        for (uint8 _class = 1; _class <= 6; _class++) {
            uint256 _index = _class - 1;
            require(_newLeftNumbers[_index] <= oldNumbers[_index], "mb err");
            uint32 toMint = oldNumbers[_index] - _newLeftNumbers[_index];
            if (toMint == 0) {
                continue;
            }
            for (uint32 j = 0; j < toMint; j++) {
                uint256 info = (uint256(_productId) << 8) | _class;
                uint32 mintIndex = oldNumbers[_index] - j - 1;
                info = (info << 32) | mintIndex;
                tokenInfo[_startTokenId] = info;
                _startTokenId++;
            }
            total += toMint;
        }
        if (total > 0) {
            _safeMint(user, total);
        }
        leftNumber[_productId] = _newData;
    }
    ...
    function leftByClass(
        ...
        (uint32[6] memory oldNumbers, , ) = leftNumber[_productId].getData();
        return oldNumbers[class - 1];
    }
    ...
    function getMetaId(
        ...
        DataWraper.Data memory tlNumber = totalIssueNumber[_productId];
        (uint32[6] memory tlNumbers, , ) = tlNumber.getData();
        uint256 batchNum = tlNumbers[class - 1];
    ...
    function issueNFT(
        ...
        totalIssueNumber[_productId] = _newData;
        leftNumber[_productId] = _newData;
    }
    ...
    function getClass(uint256 tokenId) external view returns (uint8 class) {
        uint256 info = tokenInfo[tokenId];
        class = uint8(info >> 32);
    }
}