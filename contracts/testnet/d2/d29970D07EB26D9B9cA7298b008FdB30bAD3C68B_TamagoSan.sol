...
contract TamagoSan is ERC721A,Ownable,AccessControl,ERC1155Holder{
    ...
    constructor() ERC721A("Tamagosan NFT","TSN"){
    ...
    function randomMintPack(address to, uint256[] memory tokenIDs,bytes32 messageHash,bytes calldata signature, uint quantity, uint amountTsan, uint price) external payable {
        ...
        require(totalSupply() + 3 <= maxSupply, "All the NFTs were sold");
    ...
    function mintShop(address to, uint256[] memory tokenIDs, uint prices, bytes32 messageHash,bytes calldata signature) public payable {
    ...
    function mintShopEth(address to, uint256[] memory tokenIDs, uint prices, bytes32 messageHash,bytes calldata signature) public payable {    
        require(prices > 0, "Price is 0");
        verifyHash(to, tokenIDs, messageHash);
        verifySigner(messageHash,signature);
        for(uint256 i = 0; i < tokenIDs.length; i++) {
    ...
    function recycleParts(uint256[] memory tokenIDs, uint prices, bytes32 messageHash,bytes calldata signature) public {
        require(prices > 0, "Price is 0");
        verifyHash(msg.sender, tokenIDs, messageHash);
        verifySigner(messageHash,signature);
        for(uint256 i = 0; i < tokenIDs.length; i++) {
    ...
    function upgrade(address proprietary, uint[] memory tokenIDs) public {
        for (uint i = 0; i < tokenIDs.length; i++) {
            ...
            if (tokenID >= 1 && tokenID <= 1999) {
                require(traits.balanceOf(proprietary, 2000) > 0, "You don't have enough BOOSTER CLASSIC Card");
                traits.burn(proprietary, 2000, 1);
            } else if (tokenID >= 2001 && tokenID <= 3999) {
                require(traits.balanceOf(proprietary, 4000) > 0, "You don't have enough BOOSTER ADVANCED Card");
                traits.burn(proprietary, 4000, 1);
            } else if (tokenID >= 4001 && tokenID <= 5999) {
                require(traits.balanceOf(proprietary, 6000) > 0, "You don't have enough BOOSTER LEGENDARY Card");
                traits.burn(proprietary, 6000, 1);
            } else if (tokenID >= 6001 && tokenID <= 7999) {
                require(traits.balanceOf(proprietary, 8000) > 0, "You don't have Upgrade Card");
                traits.burn(proprietary, 8000, 1);
            } else if (tokenID >= 8001 && tokenID <= 9999) {
                require(traits.balanceOf(proprietary, 10000) > 0, "You don't have Upgrade Card");
                traits.burn(proprietary, 10000, 1);
            } else {
                revert("Invalid association of token ID and Upgrade Card");
            }
            traits.burn(proprietary, tokenID, 1);
            traits.mint(proprietary, (tokenID + 2000));
        }
    }
...
contract Traits is ERC1155, Ownable, AccessControlEnumerable, ERC1155Supply{
    ...
    function externalMint(address to, uint256 tokenID) external {
        require(msg.sender == allowedContract, "Only allowed contract can mint");
        _mint(to, tokenID, 1,'');
    }
