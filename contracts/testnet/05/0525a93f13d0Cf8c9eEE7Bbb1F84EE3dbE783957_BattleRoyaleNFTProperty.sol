...
contract BattleRoyaleNFTProperty is IBattleRoyaleNFTRenderer {
    ...
    function _renderImage(bytes memory data) private pure returns (bytes memory r) {
        ...
        for (uint i = 26; i < data.length; i += 3) {
            ...
            for (uint j = 0; j < 8; j++) {
                ...
                if (y != lastY || d != lastD) {
                    if (lastD > 0) {
                        r = abi.encodePacked(r, '<rect fill="#', colors[lastD], '" x="', (lastX + offsetX).toString(), '" y="', (lastY + offsetY).toString(), '" width="',(x - lastX).toString(), '" height="1" />');
                    }
                    lastX = x;
                    lastY = y;
                    lastD = d;
        ...
        if (lastD > 0) {
            r = abi.encodePacked(r, '<rect fill="#', colors[lastD], '" x="', (lastX + offsetX).toString(), '" y="', (lastY + offsetY).toString(), '" width="',(width - lastX), '" height="1" />');
        }
    }
