...
uint256 constant ID = uint256(
    keccak256("game.piratenation.tokentypeaccountpermissioncomponent")
);

/**
 * @dev maps account address entity → boolean value representing permission to modify type
 */
contract TokenTypeAccountPermissionComponent is BoolComponent {
    /** SETUP **/
...
import {ID as ACCOUNT_PERMISSION_COMPONENT_ID} from "./TokenTypeAccountPermissionComponent.sol";
...
contract TokenTypeSystem is GameRegistryConsumerUpgradeable {
    ...
    /**
     * @notice Enables or disables if a given address is allowed to toggle
     * @param account    Address to enable/disable
     * @param isAllowed  Whether to allow
     */
    function setAllowedAccount(
        address account,
        bool isAllowed
    ) external onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _getAccountPermission().setValue(
    ...
    function setTokenType(
        ...
        if (
            _getAccountPermission().getValue(
                EntityLibrary.addressToEntity(account)
            ) ==
            false &&
    ...
    function _getAccountPermission()
        internal
        view
        returns (IBoolComponent component)
    {
        component = IBoolComponent(
            _gameRegistry.getComponent(ACCOUNT_PERMISSION_COMPONENT_ID)
        );
    }
