...
contract AdvancedQuesting is Initializable, AdvancedQuestingSettings {
    ...
    function _startAdvancedQuesting(StartQuestParams memory _startQuestParams, bool _isRestarting) private {
        ...
        if(!_isRestarting && _startQuestParams.treasureIds.length > 0) {
    ...
    function _saveStakedTreasures(
        StartQuestParams memory _startQuestParams)
    private
    {
    ...
    function _continueAdvancedQuesting(uint256 _legionId) private {
        ...
        if(_usingOldSchema) {
            ...
            delete legionIdToLegionQuestingInfoV1[_legionId].triadOutcome;
    ...
    onlyAdminOrOwner
    {
        for(uint256 i = 0; i < _endParams.length; i++) {
            EmergencyEndQuestingParams calldata _params = _endParams[i];

            uint256 _randomNumber;
            if(legionIdToLegionQuestingInfoV1[_params.legionId].requestId == 0) {
                _randomNumber = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, block.number)));
            } else {
                _randomNumber = randomizer.revealRandomNumber(legionIdToLegionQuestingInfoV1[_params.legionId].requestId);
            }

            LegionMetadata memory _legionMetadata = legionMetadataStore.metadataForLegion(_params.legionId);

            questing.processQPGainAndLevelUp(_params.legionId, _legionMetadata.questLevel);

            ZoneInfo storage _zoneInfo = zoneNameToInfo[_params.zone];

            AdvancedQuestReward[] memory _earnedRewards = _distributeRewards(
                _zoneInfo.parts[2], // Give them the rewards for the last part
                _legionMetadata,
                _params.owner,
                _randomNumber,
                3
            ...
            // Send back legion and treasure. Ensure to send it to the owner, not the msg.sender.
            legion.adminSafeTransferFrom(address(this), _params.owner, _params.legionId);

            if(_params.treasureIds.length > 0) {
                treasure.safeBatchTransferFrom(
                    address(this),
                    _params.owner,
                    _params.treasureIds,
            ...
            delete legionIdToLegionQuestingInfoV1[_params.legionId].startTime;
            delete legionIdToLegionQuestingInfoV2[_params.legionId].startTime;

            // This will fix Subgraphs state, instead of being stuck forever.
            emit AdvancedQuestEnded(_params.owner, _params.legionId, _earnedRewards);
        }
    }
    ...
    private
    {
        ...
        (uint256[] memory _treasureIds, uint256[] memory _treasureAmounts) = _unstakeTreasures(_legionId, _usingOldSchema, _isRestarting);

        emit AdvancedQuestEnded(msg.sender, _legionId, _earnedRewards);

        if(_isRestarting) {
            _startAdvancedQuesting(StartQuestParams(_legionId, _activeZoneForLegion(_usingOldSchema, _legionId), _endingPart, _treasureIds, _treasureAmounts), _isRestarting);
        }
    }
    ...
    returns(AdvancedQuestReward[] memory)
    {
        ...
        // Add one to the array length for the universal lock check.
        //
        // AdvancedQuestReward[] memory _earnedRewards = new AdvancedQuestReward[](_endingPart.rewards.length + 1);
        ...
        for(uint256 i = 0; i < _endingPart.rewards.length; i++) {
            ...
            // uint256 _odds = uint256(_reward.baseRateRewardOdds)
                // + uint256(_reward.generationToRarityToBoost[_legionGeneration][_legionRarity])
                // + (uint256(_reward.boostPerFlippedCard) * uint256(_cardsFlipped));
                // + _reward.boostForQuestLevel[_legionMetadata.questLevel];
            ...
            if(_hitReward) {
                ...
                _earnedRewards[i] = _mintHitReward(_zoneRewardOption, _randomNumber, _owner);
    ...
    function _playTreasureTriad(uint256 _legionId, UserMove[] calldata _playerMoves, bool _restartQuestingIfPossible) private {
        ...
        if(_outcome.numberOfCorruptedCardsLeft == 0 || !generationToCanHaveStasis[_legionMetadata.legionGeneration]) {
    ...
    returns(uint256 _endTime, uint8 _stasisHitCount)
    {
        ...
        if(zoneNameToInfo[_zoneName].parts[_advanceToPart - 1].playTreasureTriad
            ...
            && generationToCanHaveStasis[_legionMetadata.legionGeneration])
        {
...
abstract contract AdvancedQuestingSettings is Initializable, AdvancedQuestingContracts {
    ...
    onlyAdminOrOwner
    {
        ...
        _zonePart.rewards[_rewardIndex].boostForQuestLevel = _questLevelBoosts;
    }
...
struct ZoneReward {
    ...
    // For the given quest level, what boost the legion gets for this reward.
    uint8[7] boostForQuestLevel;
}
