...
contract T3 is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    uint256 private sale1;
    uint256 private sale2;
    uint256 private sale3;
    ...
    function verifyPrivateSalePurchase(
        ...
        if (
            purchasedAmount >= sale1 && purchasedAmount + amount <= maxPurchase
        ) {
            return true;
        }
        return (amount >= sale1 && amount <= maxPurchase);
    }
    ...
    function verifyPublicSale1Purchase(
        ...
        return (amount >= sale2 || purchasedAmount + amount >= sale2);
    }
    ...
    function verifyPublicSale2Purchase(
        ...
        return (amount >= sale3 || purchasedAmount + amount >= sale3);
    }
     ...
     * @param account Address for which to buy tokens
     * @param amount No of tokens to buy
    ...
    function setSale1(uint256 _minAmount) external onlyOwner {
        sale1 = _minAmount;
    }

    function setSale2(uint256 _minAmount) external onlyOwner {
        sale2 = _minAmount;
    }

    function setSale3(uint256 _minAmount) external onlyOwner {
        sale3 = _minAmount;
    }
