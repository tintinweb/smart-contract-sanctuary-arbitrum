...
contract FinalFormCards is IFinalFormCards, ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable {
    ...
    address private _evolutionAddress;
...
//        _evolutionAddress = evolutionAddress_;
    ...
    function mint(address to, string calldata tokenUri, uint8 house, uint8 rarity, uint8 set, uint32 power) external virtual returns(uint256) {
        ...
        // add to the oracle
        IMintInfoOracle(_mintOracleAddress).setMintInfo(
            address(this),
            currentTokenId,
            house,
            rarity,
...
//            && msg.sender == _evolutionAddress,
            "You don't have authority to burn this card"
        );
    ...
    function burnLegendary(uint256 tokenId) external {
        require(_ownerOf(tokenId) == msg.sender, "Can't burn a card you don't own");

        (uint8 rarity, , ,) = IMintInfoOracle(_mintOracleAddress).getMintInfo(address(this), tokenId);
        require(rarity == 3, "Card is not legendary");

        _burn(tokenId);
        IChromos(_chromosAddress).mint(56250000000000000000, msg.sender);
    }
