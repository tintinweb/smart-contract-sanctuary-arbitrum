...
contract L2UnicornMinter {
    ...
    mapping(address => mapping(uint256 => uint256)) private _playerRequestIds;
    ...
    function play(uint256 gasPrice_, uint256 callbackGasLimit_) payable external {
        //randomizerFee
        //uint256 callbackGasLimit_ = 30000;
        ...
        _requestIdGameInfo[requestId_] = GameInfo(msg.sender, 0);
        //
    ...
    function _addRequestIdToUser(address player_, uint256 requestId_) private {
        uint256 length = balanceOf(player_);
        _playerRequestIds[player_][length] = requestId_;
    }

    function balanceOf(address player_) public view returns (uint256) {
        return _balances[player_];
    }
    ...
    function viewGameInfos(address player_, uint256 startIndex_, uint256 endIndex_) external view returns (GameInfo[] memory gameInfos){
        if (startIndex_ >= 0 && endIndex_ >= startIndex_) {
            uint len = endIndex_ + 1 - startIndex_;
            uint total = balanceOf(player_);
            ...
            for (uint index_ = startIndex_; index_ < ((endIndex_ > total) ? total : endIndex_);) {
                uint256 requestId_ = _playerRequestIds[player_][index_];
                //GameInfo memory gameInfo_ = getGameInfoById(requestId_);
                //gameInfos[arrayIndex_] = GameInfo(gameInfo_.player, gameInfo_.seed, gameInfo_.tokenId, gameInfo_.mint);
                gameInfos[arrayIndex_] = getGameInfoById(requestId_);
    ...
    function mintForMinter(address to_, uint256 tokenId_) external {
        require(msg.sender == owner, "L2UnicornMinter: caller is not the owner");
        IL2Unicorn(l2Unicorn).mintWithMinter(to_, tokenId_);
    }
