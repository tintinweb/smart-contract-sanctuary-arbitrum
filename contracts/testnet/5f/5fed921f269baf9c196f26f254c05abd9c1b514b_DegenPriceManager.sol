...
contract DegenPriceManager is IDegenPriceManager, AccessControl {
  ...
  bytes32 public constant CONTRACTS = bytes32(keccak256("CONTRACTS"));
  bytes32 public constant ORACLES = bytes32(keccak256("ORACLES"));
  ...
  uint256 public timestampLatestPriceUpdate;
  ...
  // price information configuation
  uint256 public allowedDeviation;

  uint256 public maxDurationOfNonPythPriceUpdate;

  bool public checkFreshness;
  ...
  modifier onlyContracts() {
    require(hasRole(CONTRACTS, msg.sender), "DegenPriceManager: not contracts");
    _;
  }

  modifier onlyOracles() {
    require(hasRole(ORACLES, msg.sender), "DegenPriceManager: not oracles");
    _;
  }

  // configuration functions

  function setAllowedDeviation(uint256 _allowedDeviation) external onlyAdmin {
    allowedDeviation = _allowedDeviation;
  }

  function setMaxDurationOfNonPythPriceUpdate(
    uint256 _maxDurationOfNonPythPriceUpdate
  ) external onlyAdmin {
    maxDurationOfNonPythPriceUpdate = _maxDurationOfNonPythPriceUpdate;
  }

  function setCheckFreshness(bool _checkFreshness) external onlyAdmin {
    checkFreshness = _checkFreshness;
  }
   ...
   * @notice function that is called by the router to get the latest price of the asset and update the price if the price is more recent as the price currently set in this contract
   * @param _priceUpdateData updateData is a encode pyth PriceFeed struct
   * @param _forceSync bool for force syncing with pyth before getting the latest price
   * @return assetPrice_ the price of the asset in USD with 18 decimals
   * @return secondsSincePublish_ the number of seconds since the returned price was published
   */
  function getLatestAssetPriceAndUpdate(
  ...
  function _checkFreshnessOfUpdate(bytes calldata _priceUpdateData) internal view returns (bool) {
    PythStructs.PriceFeed memory updateInfo_ = abi.decode(
      _priceUpdateData,
      (PythStructs.PriceFeed)
    );
    require(updateInfo_.id == pythAssetId, "DegenReader: invalid price feed id");
    return timestampLatestPriceUpdate >= updateInfo_.price.publishTime;
  }
  ...
  function refreshPrice(
    ...
    // todo remove onlyContracts
    if (checkFreshness) {
      bool isFresh_ = _checkFreshnessOfUpdate(_priceUpdateData);
      if (isFresh_) {
        return (priceOfAssetUint, _returnFreshnessOfOnChainPrice());
      }
    }
    ...
    timestampLatestPriceUpdate = mostRecentPrice_.publishTime;
    return (assetPrice_, block.timestamp - mostRecentPrice_.publishTime);
  }

  /**
   * @notice this function is called by keepers of WINR to update the price of the asset without using pyth
   * @dev using this function is not manditory, but it can be used to update the price of the asset without using pyth (potentially saving costs of paying for price updates)
   * @dev to prevent oracles from being malicious and setting the price to 0 or a different outragious value, the price can only be set to a value that is within the allowed deviation of the on-chain price.
   * @param _assetPrice price of the asset in USD with 18 decimals
   */
  function setPrice(uint256 _assetPrice) external onlyOracles {
    // get the onchain price and its staleness
    (uint256 onChainAssetPrice_, uint256 secondsSincePublish_) = getLastPriceUnsafe();
    // check if the on-chain price is within stateness bounds
    require(
      maxDurationOfNonPythPriceUpdate <= secondsSincePublish_,
      "DegenPriceManager: On chain price is stale"
    );
    // this is done to prevent that the oracle can set the price to 0 or a different outragious value
    // if the on-chain price is stale, the oracles need to update the price via pyth (and not via this function), this prevents manipulation since the oracle can only set the price to a value that is within the allowed deviation of the on-chain price.
    uint256 deviation = _assetPrice > onChainAssetPrice_
      ? ((_assetPrice - onChainAssetPrice_) * BASIS_POINTS) / onChainAssetPrice_
      : ((onChainAssetPrice_ - _assetPrice) * BASIS_POINTS) / onChainAssetPrice_;
    // if a price set via the oracle deviates too much from the on-chain pyth price, the oracle is not allowed to set the price, this since we consider the pyth oracle to be more reliable than the oracle that sets the price via this function.
    require(deviation <= allowedDeviation, "need update pyth price");
    priceOfAssetUint = _assetPrice;
    timestampLatestPriceUpdate = block.timestamp;
    mostRecentSyncTimestamp = block.timestamp;
    emit OraclePriceUpdated(_assetPrice);
  }
  ...
  {
    return (priceOfAssetUint, timestampLatestPriceUpdate);
  }
  ...
  function _convertPriceToUint(
    ...
    require(price > 0, "DegenPriceManager: invalid price");
    unchecked {
  ...
  function _syncPriceWithPyth() internal returns (uint256 priceOfAssetUint_, bool isUpdated_) {
    PythStructs.Price memory priceInfo_ = pyth.getPriceUnsafe(pythAssetId);
    if (priceInfo_.publishTime > timestampLatestPriceUpdate) {
      mostRecentPricePyth = priceInfo_;
      priceOfAssetUint = _convertPriceToUint(priceInfo_);
      timestampLatestPriceUpdate = priceInfo_.publishTime;
      mostRecentSyncTimestamp = block.timestamp;
  ...
  function _returnFreshnessOfOnChainPrice() internal view returns (uint256 secondsSincePublish_) {
    secondsSincePublish_ = block.timestamp - timestampLatestPriceUpdate;
  }
  ...
  function addOrRemoveContract(address _contract, bool _add) external onlyAdmin {
    if (_add) {
      grantRole(CONTRACTS, _contract);
    } else {
      revokeRole(CONTRACTS, _contract);
    }
  }

  function addOrRemoveOracle(address _oracle, bool _add) external onlyAdmin {
    if (_add) {
