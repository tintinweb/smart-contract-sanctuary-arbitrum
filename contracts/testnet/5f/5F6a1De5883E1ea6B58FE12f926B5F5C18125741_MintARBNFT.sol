...
contract MintARBNFT is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) public quantityForRares;
    mapping(uint256 => uint256) public currentQuantityForRares;
    ...
    function changeMaxNFT(uint256 _max, uint256 _rare) external onlyOwner {
        require(_max > 0, "not a zero");
        quantityForRares[_rare] = _max;
    }
        ...
        nonReentrant
    {
        ...
        require(currentQuantityForRares[_rare].add(quantity) <= quantityForRares[_rare], "nft too much");
        ...
        currentQuantityForRares[_rare] = currentQuantityForRares[_rare].add(quantity);
