...
/// @title Interface for the LiquidationManager contract
interface ILiquidationManager {
    ...
    /// @notice event emitted when collateral is moved from liquidated holding to the liquidator
    ...
    /// @notice sets the liquidation rewards percentage
    /// @param _val the new value
    function setUserReward(uint256 _val) external;

    /// @notice returns the address of the manager container contract
    function managerContainer() external view returns (IManagerContainer);

    /// @notice method used to pay stablecoin debt by using own collateral
    /// @param _user user address to self liquidate for
    /// @param _collateral token to be used as collateral
    /// @param _pUsdAmount the amount of stablecoin to repay
    /// @param _data extra data mainly for strategies to retrieve collateral from in case the Holding balance is not enough
    /// @return collateralUsed the amount of collateral used for self-liquidation
    /// @return pUsdAmountRepayed the amount of pUsd repayed
    function selfLiquidate(
        address _user,
        address _collateral,
        uint256 _pUsdAmount,
        LiquidateCalldata calldata _data
    ) external returns (uint256 collateralUsed, uint256 pUsdAmountRepayed);
}
...
contract LiquidationManager is ILiquidationManager, ReentrancyGuard, Ownable {
    ...
    uint256 public userRewardPercentage;
    ...
    constructor(address _managerContainer) {
        ...
        userRewardPercentage = 1e4; //10% goes to the liquidator
    }

    // -- Owner specific methods --
    function setUserReward(uint256 _val) external override onlyOwner {
        require(_val < LIQUIDATION_PRECISION, "2001");
        userRewardPercentage = _val;
    }
    ...
    function selfLiquidate(
        ...
        validAmount(_pUsdAmount)
        returns (uint256 collateralUsed, uint256 pUsdAmountRepayed)
    {
        //steps:
        //check holding
        //check solvency
        //check _pUsdAmount to be less or equal to borrowed pUsd
        //check & retrieve collateral if needed
        //remove debt for amount
        //remove collateral from user
        //transfer collateral to feeAddress

        SelfLiquidateTempData memory tempData;

        tempData.holdingManager = _getHoldingManager();
        tempData.holding = tempData.holdingManager.userHolding(_user);

        // user should have a holding account in the system
        require(tempData.holdingManager.isHolding(tempData.holding), "3002");

        tempData.stablesManager = _getStablesManager();

        // collateral registry should be active
        (tempData.isRegistryActive, tempData.registryAddress) = tempData
            .stablesManager
            .shareRegistryInfo(_collateral);
        tempData.totalBorrowed = ISharesRegistry(tempData.registryAddress)
            .borrowed(tempData.holding);
        require(tempData.isRegistryActive, "1200");

        require(
            !tempData.stablesManager.isSolvent(_collateral, tempData.holding),
            "3073"
        );

        require(_pUsdAmount <= tempData.totalBorrowed, "2003");

        tempData.pUsdAmountToBurn = _pUsdAmount;

        //--
        // calculate needed collateral
        //--
        //
        tempData.exchangeRate = ISharesRegistry(tempData.registryAddress)
            .getExchangeRate();
        tempData.totalRequiredCollateral = _getCollateralAmountForUSDValue(
            _collateral,
            tempData.pUsdAmountToBurn,
            tempData.exchangeRate
        );

        tempData.totalAvailableCollateral = ISharesRegistry(
            tempData.registryAddress
        ).collateral(tempData.holding);

        tempData.totalRequiredCollateral = tempData.totalRequiredCollateral >
            tempData.totalAvailableCollateral
            ? tempData.totalAvailableCollateral
            : tempData.totalRequiredCollateral;

        tempData.totalLiquidatorCollateral = _user != msg.sender
            ? (tempData.totalRequiredCollateral * userRewardPercentage) /
                LIQUIDATION_PRECISION
            : 0;

        //--
        // if needed retrieve collateral from strategies
        //--
        //
        tempData.totalRequiredCollateral += tempData.totalLiquidatorCollateral;
        uint256 finalAvailableCollateralAmount = _retrieveCollateral(
            _collateral,
            tempData.holding,
            tempData.totalRequiredCollateral,
            _data._strategies,
        ...
        //--
        // re-calculate needed collateral in case total obtained from strategies is lower than the required amount
        //--
        //
        tempData.totalRequiredCollateral = tempData.totalRequiredCollateral >
            finalAvailableCollateralAmount
            ? finalAvailableCollateralAmount
            : tempData.totalRequiredCollateral;
        require(
        ...
        uint256 collateralWithPUsdDecimals = tempData.totalRequiredCollateral -
            tempData.totalLiquidatorCollateral;
        uint256 collateralDecimals = IERC20Metadata(_collateral).decimals();
        if (collateralDecimals > 18) {
            collateralWithPUsdDecimals =
                collateralWithPUsdDecimals /
        ...
        tempData.pUsdAmountToBurn =
            (collateralWithPUsdDecimals * tempData.exchangeRate) /
            _getManager().EXCHANGE_RATE_PRECISION();

        //--
        // repay & remove collateral
        //--
        //
        ...
        pUsdAmountRepayed = tempData.pUsdAmountToBurn;
        collateralUsed =
            tempData.totalRequiredCollateral +
            tempData.totalLiquidatorCollateral;
    }
    ...
    struct SelfLiquidateTempData {
        ...
        uint256 totalLiquidatorCollateral;
        uint256 pUsdAmountToBurn;
        uint256 exchangeRate;
    }

    /// @dev calculate collateral based on the USD value
    function _getCollateralAmountForUSDValue(
        ...
        //
        totalCollateral =
            (1e18 * _pUsdAmount * _getManager().EXCHANGE_RATE_PRECISION()) /
            (_exchangeRate * 1e18);
        require(totalCollateral > 0, "3014");
    ...
    function _retrieveCollateral(
        ...
        for (uint256 i = 0; i < _strategies.length; i++) {
            ...
            //if we reach the necessary balance, no need to withdraw from all strategies
            if (IERC20(_token).balanceOf(_holding) >= _amount) {
                break;
            }
        }

        return IERC20(_token).balanceOf(_holding);
    }
