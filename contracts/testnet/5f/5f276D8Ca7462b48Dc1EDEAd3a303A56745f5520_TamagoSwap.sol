...
contract TamagoSwap is Ownable, ERC721Holder, ERC1155Holder {
  ...
  uint64 private _swapsHistoryCounter;
  ...
  function getSwapHistoryIDs() public view returns(uint64[] memory){ 
    return swapHistoryIDs;
  }
  ...
  function getSwapHistory(uint64 swapId) public  view returns (SwapHistory memory)  {
    return _swapsHistory[swapId];     
  }
  ...
  */
  ...
  function recordSwapHistory(uint64 swapId) internal {
    ...
    uint256[] memory initiatorNftIds = new uint256[]( _swaps[swapId].initiatorNftIds.length);
    for(uint i = 0; i <  _swaps[swapId].initiatorNftIds.length; i++) {
        initiatorNftIds[i] =  _swaps[swapId].initiatorNftIds[i];
    }
    uint256[] memory secondUserNftIds = new uint256[](_swaps[swapId].secondUserNftIds.length);
    for(uint i = 0; i < _swaps[swapId].secondUserNftIds.length; i++) {
        secondUserNftIds[i] = _swaps[swapId].secondUserNftIds[i];
    }
    SwapHistory storage swapHistory = _swapsHistory[swapsHistoryCounter];
        ...
        swapHistory.initiatorNftIds = initiatorNftIds;
