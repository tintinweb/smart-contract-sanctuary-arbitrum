...
contract timeFlow is Ownable,ReentrancyGuard{
    ...
    mapping(string=>bool)private contentNameToBool;  //事务name是否已经存在
    //发起者对某个事务的转入合约token数量
    mapping(address=>mapping(uint256=>uint256))private userTradeBalance;

    //交易发起者是否向合约转入相应token数量
    mapping(address=>mapping(string=>bool)) private doThisTransfer;
    ...
    //发起者是否向合约转入相应token
    modifier senderStatus(string calldata _thisName){
        require(doThisTransfer[msg.sender][_thisName]==true,"Not send contract Token");//发起者是否向合约转入相应token的数量
        _;
    }
    ...
    //验证
    function verifySignature(uint256 nonce, bytes calldata signature) public view{
    ...
    function createThing(string calldata _contentName,string memory _willThing,uint256 _tradeMoney,uint256 _endTime,address _pointAddress)internal{
        ...
        userTradeBalance[msg.sender][_contentId]=transferMoney;
        //将事务信息推送
    ...
    //检查用户是否是审核者
    function checkUserRole(address userAddress)public view returns(bool){
        for(uint256 i=0;i<_supervisor.length;i++){
            if(_supervisor[i].supervisorAddress==userAddress){
                return true;
            }
        }
        revert("Supervisor not found!");
    }
    ...
    //发起者是否确定要发起该事务
    function judgeSendContent(bool _thisContent,string calldata contentName)external{
    ...
    //接收者是否确定进行该事务
    function judgeReceiverContent(bool _thisContent,string calldata contentName)external senderStatus(contentName){
    ...
    function contractTransfer(IERC20 token, string calldata contentName)public lockState() nonReentrant() payable{
        ...
        uint256 amount=userTradeBalance[msg.sender][getThisContentId];
        //向合约存入交易金额
    ...
    function sendWithdraw(IERC20 token, string calldata contentName)public lockState() traderStatus(contentName) senderStatus(contentName) nonReentrant(){
    ...
    //执行相应token时间锁
    function doWork(IERC20 token,string calldata contentName)public lockState() senderStatus(contentName) nonReentrant() payable{
        uint256 getThisContentId=contentNameToContentId(contentName);
    ...
    //社区与事务者对响应事件投票
    function deResponseVote(uint256 getResponseId)external{
    ...
    function withdraw(uint256 getResponseId,IERC20 tokenAddress)external payable onlyOwner{
