    ...
    OwnableUpgradeable
{
    ...
    function initialize() public initializer {
        __ERC721_init("Smols", "SMOLS");
        __Context_init_unchained();
        __Ownable_init_unchained();
    }

    /// @dev Sets a privileged minter.
    /// @param _minter The address to make privileged.
    /// @param _privileged Whether this user is privileged.
    ...
    function privilegedMint(address _to, uint256 _tokenId) external {
        require(
            msg.sender == owner() || privilegedMinters[msg.sender],
            "Not a privileged minter!"
        );
        _mint(_to, _tokenId);
        totalSupply++;
    }
