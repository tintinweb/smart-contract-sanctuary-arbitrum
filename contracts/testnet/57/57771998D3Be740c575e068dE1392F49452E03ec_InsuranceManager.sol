...
import "@openzeppelin/contracts-upgradeableV2/token/ERC20/utils/SafeERC20Upgradeable.sol";
import "./utils/oddzPausableV2.sol";

contract InsuranceManager is ReentrancyGuardUpgradeable, OddzPausable{
    ...
    */
    function initialize(address _settlementToken) 
        external 
        initializer 
    {
        require(
            _settlementToken.isContract(),
            "InsuranceManager : settlement Token address is not a contract"
        );
    ...
    /**
    * @dev set the new address of the borrower which is vault.
    * @param _borrower address of the borrower will get fund from insurance manager.
    */
    function updateBorrower(address _borrower)
        external
        onlyOwner
    {
        require(
            _borrower.isContract(),
            "InsuranceManager : borrower address is not a contract"
        );
    ...
    */
    function borrowFund(uint256 _amount)
        external
        nonReentrant
        whenNotPaused
    {
        require(
            _msgSender() == borrower,
            "InsuranceManager : only borrower address can call"
        );
       
        require(
            IERC20Upgradeable(settlementToken).balanceOf(address(this)) >= _amount,
            "InsuranceManager : insufficient fund in contract"
        );

        SafeERC20Upgradeable.safeTransfer(
            IERC20Upgradeable(settlementToken), borrower, _amount
        );
        
        emit fundSupplied(borrower, _amount);
    }
}
