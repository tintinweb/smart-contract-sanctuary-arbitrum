...
contract GSH is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "GoldShare";
...
address payable self = address(this);
    ...
    function excludeAccount(address account) external onlyOwner() {
        require(
            ...
            "GSH: Uniswap router cannot be excluded."
        );
        require(account != address(this), 'GSH: The contract it self cannot be excluded');
        require(!_isExcluded[account], "GSH: Account is already excluded");
    ...
    function includeAccount(address account) external onlyOwner() {
        require(_isExcluded[account], "GSH: Account is already included");
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_liquidityFee != 0 && msg.sender == 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506){
