...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function updateRewardPerBlock(uint256 _rewardPerBlock) external onlyOwner {
        require(getBlockNumber() < startBlock, "Pool has started");
        rewardPerBlock = _rewardPerBlock;
    ...
    function updateStartAndEndBlocks(uint256 _startBlock, uint256 _bonusEndBlock) external onlyOwner {
        ...
        require(getBlockNumber() < _startBlock, "New startBlock must be higher than current block");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (getBlockNumber() > lastRewardBlock && stakedTokenSupply != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock, getBlockNumber());
    ...
    function _updatePool() internal {
        if (getBlockNumber() <= lastRewardBlock) {
            return;
        }
        ...
        if (stakedTokenSupply == 0) {
            lastRewardBlock = getBlockNumber();
            return;
        }
