    ...
    PausableUpgradeable
{
    ...
    function initialize(address _magic, address _atlasStaker) external initializer {
        __Ownable_init();
        ...
        returns (uint256 atlasStakerDepositId)
    {
        ...
        emit NewUserStake(atlasStakerDepositId, _amount, vaultStake.unlockAt, _msgSender(), _lock);
    }
    ...
    function requestWithdrawal(uint256[] memory _depositIds) public override {
        for (uint256 i = 0; i < _depositIds.length; i++) {
            ...
            require(userStake.owner == _msgSender(), "BattleflyFlywheelVault: caller is not the owner");
    ...
    modifier onlyMembers() {
        require(whitelistedUsers[_msgSender()], "BattleflyWheelVault: caller is not a whitelisted member");
        _;
    }
