...
contract Lending {
    ...
    event PlaceOrder(uint order_sn, address lender, uint amount, uint rate);
    event CancelOrder(uint order_sn);
    event BorrowSuccess(uint order_sn, uint order_balance, address lender, uint receipt_sn, address borrower, uint chainid, uint c_sn, uint amount, uint time, uint rate);
    event BorrowIncomplete(address user, uint chainid, uint c_sn, uint amount);
    event RepaySuccess(uint chainid, uint c_sn, address source, uint c_frozen, address lender, uint receipt_sn, uint income, uint changed_order, uint order_balance);
    event StartLiquidation(uint receipt_sn, address borrower, uint chainid, uint c_sn, address source, uint c_frozen, address receiver);
    ...
    function borrow(address borrower, uint chainid, uint c_sn, address source, address token, uint frozen, uint amount, uint order_sn, uint[] calldata candidate_orders) external onlyOwner {
        ...
        if (orders[order_sn].balance >= amount) {
            ...
            if (final_order_sn == 0) {
                emit BorrowIncomplete(borrower, chainid, c_sn, amount);
                return;
            }
        }
        ...
        require(success);
        emit BorrowSuccess(final_order_sn, order_balance, lender, curr_receipt_sn, borrower, chainid, c_sn, amount, block.timestamp, rate);
    }

    function placeOrder(uint amount, uint rate) external returns (uint, uint, uint) {
        ...
        emit PlaceOrder(curr_order_sn, msg.sender, amount, rate);
        return (curr_order_sn, amount, rate);
    }

    function cancelOrder(uint order_sn) external returns (uint, uint) {
        ...
        emit CancelOrder(order_sn);
        return (order_sn, amount);   
    }

    function repay(uint receipt_sn) external payable returns (uint, uint, uint, uint) {
        ...
        emit RepaySuccess(receipt.chainid, receipt.c_sn, collateral.source, collateral.frozen, receipt.lender, receipt_sn, income, latest_order, order_balance);
        return (receipt_sn, msg.value, receipt.chainid, receipt.c_sn);   
    }

    function liquidate(uint receipt_sn, address receiver) external returns (uint) {
        LoanReceipt memory receipt = loanReceipts[receipt_sn];
        ...
        emit StartLiquidation(receipt_sn, receipt.borrower, receipt.chainid, receipt.c_sn, collateral.source, collateral.frozen, receiver);
        return receipt_sn;
    }
    ...
    function reborrow(uint order_sn, uint amount, uint chainid, uint c_sn) external returns (uint, uint, uint, uint, uint, uint) {
        require(amount > 0);
        ...
        (bool success, ) = msg.sender.call{value: amount}("");
        require(success);
        emit BorrowSuccess(order_sn, balance, lender, curr_receipt_sn, msg.sender, chainid, c_sn, amount, block.timestamp, rate);
        return (chainid, c_sn, curr_receipt_sn, amount, block.timestamp, rate);
    }
}