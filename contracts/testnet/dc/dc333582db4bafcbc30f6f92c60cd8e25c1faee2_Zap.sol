...
contract Zap is IZap {
    ...
    constructor(address _swapper, address _registry) {
    ...
    function swapAndStake(SwapParams memory _swapParams, address _pool) external {
        ...
        uint256 amountSwapped = abi.decode(returnedData, (uint256));
...
interface IZap {
     ...
     *  @param _swapParams A struct containining all necessary params allowing a token swap
     *  @param _pool The pool address
     */
    function swapAndStake(SwapParams memory _swapParams, address _pool) external;
}
