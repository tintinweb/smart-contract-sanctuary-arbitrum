...
contract SubscriptionManager is ISubscriptionManager, Initializable, DAOAccessControlled {
    ...
    function isValidPassport(address _passport) internal view returns (bool) {
        address _collectionManager = authority.getAuthorities().collectionManager;
        ICollectionManager cm = ICollectionManager(_collectionManager);
        bool isPassport = cm.getCollectionType(_passport) == ICollectionData.CollectionType.PASSPORT;
        // if (isPassport) {
    ...
    function hasValidSubscriptionConfig(address _passport) internal view returns (bool) {
        return subscriptionConfig[_passport].tradingEnabled &&
        subscriptionConfig[_passport].platformFeePercent > 0 &&
        subscriptionConfig[_passport].peopleFeePercent > 0 &&
        subscriptionConfig[_passport].peopleFeeReceiver != address(0);
    }
    ...
    function getPrice(uint256 _floorPrice, uint256 supply, uint256 amount) public pure returns (uint256) {
        ...
        uint256 summation = _floorPrice + (sum2 - sum1);
        return summation * 1 ether / 16000;
    }

    function getBuyPrice(address passport, uint256 amount) public view returns (uint256, uint256, uint256) {
        require(subscriptionConfig[passport].tradingEnabled, "Trading disabled");
        uint256 price = getPrice(subscriptionConfig[passport].floorPrice, passportsSupply[passport], amount);
        uint256 platformFee = price * subscriptionConfig[passport].platformFeePercent / 1 ether;
        uint256 peopleFee = price * subscriptionConfig[passport].peopleFeePercent / 1 ether;
    ...
    function getSellPrice(address passport, uint256 amount) public view returns (uint256, uint256, uint256) {
        require(subscriptionConfig[passport].tradingEnabled, "Trading disabled");
        uint256 price = getPrice(subscriptionConfig[passport].floorPrice, passportsSupply[passport] - amount, amount);
        uint256 platformFee = price * subscriptionConfig[passport].platformFeePercent / 1 ether;
        uint256 peopleFee = price * subscriptionConfig[passport].peopleFeePercent / 1 ether;
        return (price, platformFee, peopleFee);
    }

    function subscribe(address passport, uint256 amount) public {
        ...
        address passportOwner = subscriptionConfig[passport].peopleFeeReceiver;
        (uint256 price, uint256 platformFee, uint256 peopleFee) = getBuyPrice(passport, amount);
        ...
        loot8Token.safeTransferFrom(_msgSender(), passportOwner, peopleFee);
    ...
    function unsubscribe(address passport, uint256[] memory _passportIds) public {
        ...
        uint256 supply = passportsSupply[passport];
        address owner = subscriptionConfig[passport].peopleFeeReceiver;
        (uint256 price, uint256 platformFee, uint256 peopleFee) = getSellPrice(passport, amount);
        passportsSupply[passport] = supply - amount;
        loot8Token.safeTransfer(_msgSender(), price);
        loot8Token.safeTransfer(loot8Wallet, platformFee);
        loot8Token.safeTransfer(owner, peopleFee);
