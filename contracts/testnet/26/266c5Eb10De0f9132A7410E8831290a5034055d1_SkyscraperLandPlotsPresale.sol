...
contract SkyscraperLandPlotsPresale is
    Ownable,
    Pausable,
    ContextMixin,
    ReentrancyGuard,
    ...
    DefaultOperatorFilterer
{
    ...
    function mint(
        ...
        uint256 currenySupply = totalSupply();
        require(_mintAmount > 0);
        require(_mintAmount <= maxMintAmount);
        require(currenySupply + _mintAmount <= maxSupply);
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            uint256 randomIndex = random().mod(_randomNumbers.length);
    ...
    // Restricts NFT sales to only marketplaces that enforce creator fees
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public override (ERC721, IERC721) whenNotPaused nonReentrant onlyAllowedOperator(from) {
    ...
    function burn(uint256 tokenId) public nonReentrant onlyOwner  {
        _burn(tokenId);
    }
