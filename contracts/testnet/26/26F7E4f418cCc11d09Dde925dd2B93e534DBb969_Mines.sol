...
contract Mines is Common {
     ...
     */
    function Mines_GetMultipliers(uint256 numMines, uint256 numRevealed) public view returns (uint256 multiplier) {
     ...
     */
    function Mines_GetMaxReveal() external view returns (uint256[24] memory maxReveal) {
     ...
     */
    function Mines_GetState(address player) external view returns (MinesGame memory minesState) {
    ...
    function Mines_Start(
        ...
        uint256 fee = _transferWager(tokenAddress, wager, 400000, 22, msgSender);
        ...
        emit Mines_Play_Event(msgSender, wager, tokenAddress, numMines, isCashout, fee);
    }
     ...
     */
    function Mines_Reveal(bool[25] calldata tiles, bool isCashout) external payable nonReentrant {
        address msgSender = _msgSender();
    ...
    function Mines_End() external nonReentrant {
        ...
        _transferToBank(tokenAddress, wager);
        delete (minesGames[msgSender]);
        _transferPayout(msgSender, payout, tokenAddress);
        emit Mines_End_Event(msgSender, wager, payout, tokenAddress, numMines, revealedTiles, multiplier);
    }
    ...
    function Mines_Refund() external nonReentrant {
        ...
        if (minesGames[msgSender].blockNumber + 200 > block.number) {
            revert BlockNumberTooLow(block.number, minesGames[msgSender].blockNumber + 200);
        }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal {
        ...
        for (uint256 i = 0; i < game.tilesPicked.length; i++) {
            if (game.numMines == 0 || 25 - numberOfRevealedTiles == game.numMines) {
            ...
            if (game.tilesPicked[i]) {
                bool gem = _pickTile(player, i, 25 - numberOfRevealedTiles, game.numMines, randomWords[randomCounter]);
                if (gem == false) {
                    game.numMines -= 1;
                    mines[i] = true;
                    won = false;
        ...
        if (!won) {
            ...
            _transferToBank(game.tokenAddress, game.wager);
            delete (minesGames[player]);

            return;
        }

        uint256 multiplier = minesMultipliers[game.numMines][numberOfRevealedTiles];
        ...
        if (game.isCashout == false) {
            ...
            _transferToBank(tokenAddress, game.wager);
            delete (minesGames[player]);
    ...
    function _kellyWager(
        ...
        uint256 balance;
        if (tokenAddress == address(0)) {
            balance = address(Bank).balance;
        } else {
            balance = IERC20(tokenAddress).balanceOf(address(Bank));
        }
...
contract Common is ReentrancyGuard {
    ...
    function _transferWager(
        ...
        if (!Bank.getIsValidWager(address(this), tokenAddress)) {
            revert NotApprovedBank();
        }
        if (wager == 0) {
            revert ZeroWager();
        }
     ...
     */
    function _transferToBank(address tokenAddress, uint256 amount) internal {
        if (tokenAddress == address(0)) {
            ...
            IERC20(tokenAddress).safeTransfer(address(Bank), amount);
        }
    }
    ...
    /**
     * @dev function to transfer VRF fees acumulated in the contract to the Bank
     * Can only be called by owner
     */
    function transferFees(address to) external nonReentrant {
        if (msg.sender != Bank.getOwner()) {
            revert NotOwner(Bank.getOwner(), msg.sender);
        }
        uint256 fee = VRFFees;
        VRFFees = 0;
    ...
    function _transferPayout(
        ...
        Bank.transferPayout(player, payout, tokenAddress);
    }
