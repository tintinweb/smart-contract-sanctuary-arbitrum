...
interface StorageInterfaceV5{
    ...
    function dai() external view returns(USDTInterface);
...
contract GNSPairsStorageV6 {
    ...
    constructor(uint _currentOrderId, address storageAddr) {
        require(storageAddr != address(0), "STORAGE ADDRESS IS NIL");
        require(_currentOrderId > 0, "ORDER_ID_0");
        storageT = StorageInterfaceV5(storageAddr);
    ...
    modifier feeOk(Fee calldata _fee){
        require(_fee.openFeeP > 0 && _fee.closeFeeP > 0 && _fee.oracleFeeP > 0 && _fee.minLevPosDai > 0, "WRONG_FEES");
        _;
    }
