...
contract HelixFund is BaseUpgradeablePausable, IHelixFund, IVersioned {
  ...
  event Cancel(address indexed account, uint256 nonce);
  ...
  // bytes32 public constant INDICATIVE_INTEREST_TYPEHASH = keccak256("IndicativeInterest(address investor,address fund,uint256 subscriptionDate,uint256 amount,uint256 nonce)");
  bytes32 public constant INDICATIVE_INTEREST_TYPEHASH =
    0x3d95e89ed19b897e9f5fe9ef199db075c1145c4164d09669a9763b25549d36f7;
  ...
  function initialize(
    ...
    _setupRole(ISSUER_ROLE, _addresses[2]);
  ...
  function approveInterest(
    ...
    uint256 amount,
    address account
  ) public override onlyIssuer isNotCanceled(account) isNotClosed{
    if (!hasAllowedUID(account)) {
      revert NotAuthorized();
    }
    investment[account][nonces[account]] = InvestmentInfo({
      subscriptionDate: subscriptionDate,
      amount: amount,
      status: InvestmentStatus.PendingPayment
    });

    emit ApproveInterest(account, address(this), amount, issuer);
  }
  ...
  function invest(
  ...
  ) public override isNotCanceled(msg.sender) isNotClosed nonReentrant{
    ...
    require(
      investment[msg.sender][nonce].status == InvestmentStatus.PendingPayment ||
    ...
    token.safeTransferFrom(msg.sender, issuer, amount);
    if (
      investment[msg.sender][nonce].status == InvestmentStatus.PendingPayment
    ) {
      investment[msg.sender][nonce].status = InvestmentStatus.Invested;
    } else {
      investment[msg.sender][nonce] = InvestmentInfo({
        subscriptionDate: subscriptionDate,
        amount: amount,
    ...
    emit Invested(msg.sender, address(this), amount, issuer);
  }
  ...
  function investWithPermit(
    ...
    invest(subscriptionDate, amount, investSignature);
  }
  ...
  function cancelInvestment(
    ...
    require(
      investment[account][nonce].status == InvestmentStatus.NotInterest ||
        investment[account][nonce].status == InvestmentStatus.PendingPayment,
      'Can not cancel'
    );
    investment[account][nonce].status == InvestmentStatus.Canceled;
    _increaseNonce(account);

    emit Cancel(account, nonce);
  }
  ...
  function _allowInvestByOnlyIssuer(
    ...
    return (issuerProxy.hasRole(
      OWNER_ROLE,
      ECDSAUpgradeable.recover(digest, signature)
    ) || hasRole(ISSUER_ROLE, ECDSAUpgradeable.recover(digest, signature)));
  }

  function _increaseNonce(address investor) internal {
    nonces[investor]++;
  }

  modifier isNotCanceled(address account) {
    require(
      investment[account][nonces[account]].status != InvestmentStatus.Canceled,
      'Not allow to invest'
    );
    _;
  }
