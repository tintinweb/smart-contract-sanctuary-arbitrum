...
contract ShopFacet is WithStorage, WithModifiers {
    event FoundersPackPurchased(address indexed owner);

    function purchaseFoundersPack() external payable {
        require(
            !_ss().purchasedFoundersPackByAddress[msg.sender],
            "Already purchased"
        );
        ...
        emit FoundersPackPurchased(msg.sender);
    }
    ...
    function getCharacterTokenCost() public view returns (uint256) {
        return
            _gc().priceOracle.getPerDollarTokenPrice(
                _gc().nativeTokenSymbol,
    ...
    function getFoundersPackFullCost() public view returns (uint256) {
        return getCharacterTokenCost() + getFoundersPackPurchaseGasOffset();
    }
...
import {PriceOracle} from "../standalone/PriceOracle.sol";
...
struct GameConstants {
    // The symbol of the native token of the chain we are in
    string nativeTokenSymbol;
    // TODO: Can we integrate these as facets?
...
struct TokensConstants {
    string contractUri;
    string rareGen0EggUri;
    string epicGen0EggUri;
    string legendaryGen0EggUri;
    uint16 totalGen0Eggs;
    uint64 totalGen0RareEggs;
    uint64 totalGen0EpicEggs;
    uint64 totalGen0LegendaryEggs;
    uint64 gen0RareEggsBaseId;
    uint64 gen0EpicEggsBaseId;
    uint64 gen0LegendaryEggsBaseId;
}
...
library LibStorage {
    ...
    bytes32 public constant _GAME_CONSTANTS_POSITION =
        keccak256("thebeacon.constants.game");
    ...
    function gameConstants() internal pure returns (GameConstants storage gc) {
        bytes32 position = _GAME_CONSTANTS_POSITION;
        assembly {
            gc.slot := position
        }
    }
...
contract WithStorage {
    ...
    function _gc() internal pure returns (GameConstants storage) {
    ...
    WithModifiers
{
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        return (
            ...
            (salePrice * _ts().royaltiesPercentage * 100) / 10000
        );
    }
        ...
        returns (bool)
    {
        ...
        // TODO: Set on initializer as ENV VAR when we know if it works
    ...
    function unlockNft(uint256 id, address owner)
        external
    ...
    function mintEggs(uint256[] calldata ids, uint256[] calldata amounts)
        external
        ownerOnly
        pausable
    {
        _ts().nftsIndex = _ts().nftsIndex + ids.length;
        require(
            _ts().totalMintedEggs + ids.length <= _tc().totalGen0Eggs,
            "All eggs have been already minted"
        );
        _mintBatch(msg.sender, ids, amounts, "");

        for (uint256 i; i < ids.length; i++) {
            _lockNftInternal(ids[i]);
        }

        _ts().totalMintedEggs = _ts().totalMintedEggs + uint16(ids.length);
    }
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        ...
        if (
            (tokenId <
                (_tc().gen0LegendaryEggsBaseId +
                    _tc().totalGen0LegendaryEggs)) &&
            (tokenId >= _tc().gen0RareEggsBaseId)
        ) {
            if (tokenId >= _tc().gen0LegendaryEggsBaseId) {
                return _tc().legendaryGen0EggUri;
            } else if (tokenId >= _tc().gen0EpicEggsBaseId) {
                return _tc().epicGen0EggUri;
            } else {
                return _tc().rareGen0EggUri;
            }
        }
        ...
        )
    {
        return (
            ...
            _tc().gen0RareEggsBaseId,
            _tc().gen0EpicEggsBaseId,
            _tc().gen0LegendaryEggsBaseId
        );
    }

    function getTotalGen0EggQuantities()
        public
        view
        returns (
            uint16 totalGen0Eggs,
        ...
        )
    {
        return (
            _tc().totalGen0Eggs,
            _tc().totalGen0RareEggs,
            _tc().totalGen0EpicEggs,
            _tc().totalGen0LegendaryEggs
        );
    }
...
contract PriceOracle is AccessControl {
    ...
    FeedRegistryInterface private _chainlinkRegistry;

    mapping(string => int256) public tokenUsdPrice;
    mapping(string => address) public tokenFeedAddress;

    constructor(address registry_) {
        _setupRole(ADMIN_ROLE, msg.sender);
        _chainlinkRegistry = FeedRegistryInterface(registry_);
    }

    function setUsdPrice(string memory tokenName_, int256 usdPrice)
        external
        onlyRole(ADMIN_ROLE)
    {
        tokenUsdPrice[tokenName_] = usdPrice;
    }

    function setTokenAddress(string memory tokenName_, address _tokenAddress)
        external
        onlyRole(ADMIN_ROLE)
    {
        tokenFeedAddress[tokenName_] = _tokenAddress;
    }

    function getUsdPrice(string memory tokenName_)
        public
        view
        returns (int256)
    {
        if (tokenFeedAddress[tokenName_] != address(0)) {
            (, int256 price, , , ) = _chainlinkRegistry.latestRoundData(
                tokenFeedAddress[tokenName_],
                Denominations.USD
            );
            return price;
        }

        require(tokenUsdPrice[tokenName_] > 0, "Token price not available");

        return tokenUsdPrice[tokenName_];
    }

    // Dollars are x100 for precision
    function getPerDollarTokenPrice(string memory tokenName_)
        public
        view
        returns (uint256)
    {
        return getPerDollarTokenPrice(tokenName_, 1);
    }

    function getPerDollarTokenPrice(string memory tokenName_, uint256 amount)
        public
        view
        returns (uint256)
    {
        int256 usdCost = getUsdPrice(tokenName_);
        return ((amount * 1 ether) / uint256(usdCost)) / 100;
    }
}
...
library LibTokens {
    ...
    uint64 public constant NFTS_BASE_ID = 100_000; // "infinite" slots
