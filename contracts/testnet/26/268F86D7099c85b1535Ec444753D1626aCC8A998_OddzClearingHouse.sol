    ...
    IUniswapV3SwapCallback
{
        ...
        returns (IOrderManager.RemoveLiquidityResponse memory response )
    {
        ...
        _settleLiquidityGeneratedPosition(trader, params, response);
        ...
        returns (uint256 baseAmount, uint256 quoteAmount)
    {
        ...
        require(info.trader==trader, "OddzClearingHouse:Invalid position id for the trader");
        ...
        returns (uint256 _baseAmount, uint256 _quoteAmount)
    {
        ...
        if (_params.isIsolate) {
            ...
            _handleIsolatePositions(internalParams, _response);
        } else {
            // to handle positions inside a group
            _handleGroupPositions(internalParams, _response);
        }
        ...
        returns (uint256 _baseAmount, uint256 _quoteAmount)
    {
        ...
        //_updateParticipatingMarkets(_params.trader,_params.baseToken,false);
    ...
    function _handleGroupPositions(
        ...
        if (_params.isNewGroup) {
            ...
            if(_params.isDefaultGroup){
                ...
                require(groupInfo.trader==_params.trader, "OddzClearingHouse : Invalid Group ID");
                groupId = _params.groupId;
            }
        ...
        if (!marketPresent) {
            ...
            if(IBalanceManager(balanceManager).getTakerBasePositionSize(positionId) == 0){
                ...
                require(pnlToBeRealized.abs() <= positionInfo.takerQuoteSize.abs(), "BalanceManager:Inconsistent quote balance and realizedPnL");
        ...
        if (!params.isClose) {
            ...
            if (response.takerBaseAmount.abs() > _DUST) {
                ...
                if (params.isIsolate && params.positionID==0) {
                    ...
                    require(
                        info.baseToken == params.baseToken , "OddzClearingHouse :Different markets"
                    );
            ...
            if (response.takerBaseAmount.abs() > _DUST) {
                ...
                // swaps
                (, int256 swappedQuoteSize) = ISwapManager(swapManager)
                ...
                // emit an event on closing a position just like normal position and closing.But position id in this case will be 0
                emit PositionUpdated(
                    maker,
                    params.baseToken,
                    0,
...
interface ISwapManager{
    ...
    * @dev can only be called from Oddz ClearingHouse contract
    * @param params The parameters of the swap
    * @return swappedBasePositionSize The result of the swap (base token amount)
    * @return swappedQuotePositionSize quote token amount
     */
    ...
    * @return pnlToBeRealized pnl to be realized
     */ 
    function getPnLToBeRealized(uint256 positionId,int256 baseAmount,int256 quoteAmount) external view returns(int256 pnlToBeRealized);
}
