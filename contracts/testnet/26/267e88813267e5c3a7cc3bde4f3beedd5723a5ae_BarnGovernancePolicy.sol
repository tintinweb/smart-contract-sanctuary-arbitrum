...
contract BarnGovernancePolicy is Policy, Owned {
    ...
    uint256 public constant WARMUP_PERIOD = 1 minutes;//2 days;

    /// @notice Amount of time a submitted proposal must exist before triggering activation.
    uint256 public constant ACTIVATION_TIMELOCK = 5 minutes;//2 days;

    /// @notice Amount of time a submitted proposal must exist before triggering activation.
    uint256 public constant ACTIVATION_DEADLINE = 15 minutes;//3 days;
    ...
    uint256 public constant VOTING_PERIOD = 10 minutes;// 3 days;
    ...
    /// @notice Amount of time after the proposal is activated (NOT AFTER PASSED) when it can be executed (otherwise proposal will go stale).
    /// @dev    This is inclusive of the voting period (so the deadline is really ~4 days, assuming a 3 day voting window).
    uint256 public constant EXECUTION_DEADLINE = 30 minutes; //2 weeks;

    /// @notice Amount of time a non-executed proposal must wait for the proposal to go through.
    /// @dev    This is inclusive of the voting period (so the deadline is really ~4 days, assuming a 3 day voting window).
    uint256 public constant COLLATERAL_DURATION = 10 minutes;//16 weeks;
    ...
    uint256 public constant PROPOSAL_TIMELOCK = 0; //2 weeks;
