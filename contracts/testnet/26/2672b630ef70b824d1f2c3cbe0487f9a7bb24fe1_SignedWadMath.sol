...
library SignedWadMath{
    ...
    function toDaysWadUnsafe(uint256 x)public pure returns (int256 r) {
        /// @solidity memory-safe-assembly
        assembly {
            // Multiply x by 1e18 and then divide it by 7200.
            r := div(mul(x, 1000000000000000000), 7200)
        }
    }
    ...
    function fromDaysWadUnsafe(int256 x)public pure returns (uint256 r) {
        /// @solidity memory-safe-assembly
        assembly {
            // Multiply x by 7200 and then divide it by 1e18.
            r := div(mul(x, 7200), 1000000000000000000)
        }
    }
...
abstract contract VRGDA {
    ...
    constructor(int256 _targetPrice, int256 _priceDecayPercent) {
        ...
        decayConstant = SignedWadMath.wadLn(1e18 - _priceDecayPercent);
    ...
    function getVRGDAPrice(int256 timeSinceStart, uint256 sold) public view virtual returns (uint256) {
        unchecked {
            // prettier-ignore
            return uint256(SignedWadMath.wadMul(targetPrice, SignedWadMath.wadExp(SignedWadMath.unsafeWadMul(decayConstant,
                // Theoretically calling toWadUnsafe with sold can silently overflow but under
                // any reasonable circumstance it will never be large enough. We use sold + 1 as
                // the VRGDA formula's n param represents the nth token and sold is the n-1th token.
                timeSinceStart - getTargetSaleTime(SignedWadMath.toWadUnsafe(sold + 1))
            ))));
        }
    }
...
abstract contract LogisticVRGDA is VRGDA {
    ...
    function getTargetSaleTime(int256 sold) public view virtual override returns (int256) {
        unchecked {
            return -SignedWadMath.unsafeWadDiv(SignedWadMath.wadLn(SignedWadMath.unsafeDiv(logisticLimitDoubled, sold + logisticLimit) - 1e18), timeScale);
        }
    }
}

contract EBBLogisticVRGDA is LogisticVRGDA{
    
    constructor() LogisticVRGDA(
            299e18, // Target price.
            0.99e18, // Price decay percent.
            // Max gobblers mintable via VRGDA.
            SignedWadMath.toWadUnsafe(990000 *10),
            0.004057e18 // Time scale.
        ){}

    function getPrice(uint256 blockSinceStart, uint256 sold) external view returns (uint256){
        unchecked{
            return getVRGDAPrice(SignedWadMath.toDaysWadUnsafe(blockSinceStart), sold);
        }
    }
}