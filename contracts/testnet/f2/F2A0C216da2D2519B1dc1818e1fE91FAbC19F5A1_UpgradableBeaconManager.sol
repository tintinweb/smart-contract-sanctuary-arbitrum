...
/**
 * @title UpgradableBeaconManager
 * @author Josh-Jack
 * @notice The Upgradable Beacon Manager is responsible for holding the current logic contracts and upgrading the logic contracts.
 */
contract UpgradableBeaconManager is Initializable, IUpgradableBeaconManager, ACLUser, EmergencyModifier {
  ...
  event UpgradedImplementation(address concreteManager, address newImplementation);
  /** @notice Fires an event when an upgradable beaon is deployed. */
  event UpgradableBeaconDeployed(address deployer, address beacon, address initalImplementation);

  ///@dev checks to ensure that only the concrete manager can upgrade the implementation
  modifier isValidImplementation(address implementation_) {
    require(implementation_ != address(0), "Must specify a implementation address.");
    _;
  }

  ///@dev checks to ensure that the name in the mapping key has not been used
  modifier isValidName(string memory name_) {
    require(implementations[name_] == address(0), "Name has already been created");
    _;
  }

  /**
   * @notice The constructor will iniate this contract and the ACL contract.
   * @param aclAddress_ is the address of the ACL contract.
   * @dev this initializes the upgradeable beacon that will hold the implementation address.
   */
  constructor(address aclAddress_, address _emsAddress) {
    aclContract = ACL(aclAddress_);
   ...
   */
  function deployUpgradeableBeacon(string memory name_, address implementation_) public emergencyStop isCoreOrManager isValidImplementation(implementation_) isValidName(name_) {
    ...
    emit UpgradableBeaconDeployed(msg.sender, beacons[name_], implementation_);
  }

  /**
   * @notice Sets a new implementation address on an upgrade beacon.
   * have the index store the beacon?
   * This function may only be called by the concrete manager.
   * @param newImplementation_ The address of the new implementation.
   * @param name_ The Name of which the implementation is stored.
   */
  function upgrade(address newImplementation_, string memory name_) public emergencyStop onlyManager isValidImplementation(newImplementation_) {
  ...
  function getBeacon(string memory name_) external view override(IUpgradableBeaconManager) returns (address) {
