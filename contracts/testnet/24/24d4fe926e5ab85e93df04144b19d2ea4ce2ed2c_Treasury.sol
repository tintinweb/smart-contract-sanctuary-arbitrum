...
contract Treasury is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address public veRwaxRedemptionPool;
    address public RLP;
    mapping(address => bool) public withdrawableTokens;
    // ===== end: V3 Storage ====

    // ===== begin: V4 Storage ====
    address public rwaxUsdtLP;
    // ===== end: V4 Storage ====

    modifier onlyVeRwaxRedemptionPool() {
        require(msg.sender == veRwaxRedemptionPool, "Treasury::only VE_RWAX redemption pool");
        _;
    }
    ...
    function reinit_v3(address _weth, address _ethUnwrapper, address _rlp) external reinitializer(3) {
        require(_weth != address(0), "Treasury::invalid address");
        require(_ethUnwrapper != address(0), "Treasury::invalid address");
        require(_rlp != address(0), "Treasury::invalid address");
        weth = IWETH(_weth);
        ethUnwrapper = IETHUnwrapper(_ethUnwrapper);
        RLP = _rlp;
    }

    function reinit_v4(address _rwaxUsdtLP) external reinitializer(VERSION) {
        require(_rwaxUsdtLP != address(0), "Treasury::invalid address");
        rwaxUsdtLP = _rwaxUsdtLP;
    }

    /* ========== PUBLIC FUNCTIONS ========== */

    function distribute(address _token, address _receiver, uint256 _amount) external onlyVeRwaxRedemptionPool {
        require(_token != address(0), "Treasury::invalid token address");
        require(_token == address(RLP) || _token == address(rwaxUsdtLP), "Treasury::token not allowed");
        require(_receiver != address(0), "Treasury::invalid receiver address");
        IERC20(_token).safeTransfer(_receiver, _amount);
        emit TokenDistributed(veRwaxRedemptionPool, _token, _receiver, _amount);
    }

    function convertRLPToToken(address _receiver, address _tokenOut, uint256 _lpAmount, uint256 _minAmountOut)
        external
        onlyVeRwaxRedemptionPool
    {
        require(RLP != address(0), "Treasury::rlp not set");
        require(_receiver != address(0), "Treasury::invalid receiver address");
        uint256 actualAmountOut;
        if (_tokenOut == address(weth)) {
            actualAmountOut = _removeLiquidity(RLP, _tokenOut, _lpAmount, _minAmountOut, address(this));
            _safeUnwrapETH(actualAmountOut, _receiver);
        } else {
            actualAmountOut = _removeLiquidity(RLP, _tokenOut, _lpAmount, _minAmountOut, _receiver);
        }
        emit TokenDistributed(veRwaxRedemptionPool, _tokenOut, _receiver, actualAmountOut);
    }

    function convertToRLP(address _token, uint256 _amount, uint256 _minAmountOut)
        external
        nonReentrant
        onlyRole(CONTROLLER_ROLE)
    {
        uint256 amountOut = _addLiquidity(_token, _amount, RLP, _minAmountOut, address(this));
        emit RLPConverted(_token, address(RLP), _amount, amountOut);
    }
    ...
    function setVeRwaxRedemptionPool(address _veRwaxRedemptionPool) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_veRwaxRedemptionPool != address(0), "Treasury::invalid token address");
        veRwaxRedemptionPool = _veRwaxRedemptionPool;
        emit VeRwaxRedemptionPoolSet(_veRwaxRedemptionPool);
    }

    function setRLPToken(address _rlp) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_rlp != address(0), "Treasury::invalid token address");
        RLP = _rlp;
        emit RLPTokenSet(_rlp);
    }
    ...
    function _addLiquidity(address _token, uint256 _amount, address _rlp, uint256 _minAmountOut, address _to)
        internal
        returns (uint256 actualLPAmountOut)
    {
        uint256 lpBalanceBefore = IERC20(_rlp).balanceOf(_to);
        IERC20(_token).safeIncreaseAllowance(address(pool), _amount);
        pool.addLiquidity(_rlp, _token, _amount, _minAmountOut, _to);
        actualLPAmountOut = IERC20(_rlp).balanceOf(_to) - lpBalanceBefore;
    }
    ...
    function _removeLiquidity(
        ...
        IERC20(_rlp).safeIncreaseAllowance(address(pool), _lpAmount);
        // we will calculate amount out ourself, so skip slippage check on pool
        pool.removeLiquidity(RLP, _tokenOut, _lpAmount, 0, _receiver);
    ...
    /* ========== EVENTS ========== */
    event RLPConverted(address indexed token, address indexed rlp, uint256 amount, uint256 lpAmountOut);
    event TokenDistributed(address indexed spender, address token, address receiver, uint256 amount);
    event RLPTokenSet(address indexed token);
    event VeRwaxRedemptionPoolSet(address indexed token);
