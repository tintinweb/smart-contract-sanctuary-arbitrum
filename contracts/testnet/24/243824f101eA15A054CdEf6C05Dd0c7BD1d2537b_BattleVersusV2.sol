...
contract BattleVersusV2 is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  uint256 public increaseDivider;
  uint256 public decreaseDivider;
  ...
  function updateIncreaseDivider(uint256 _value) external onlyAdmin {
    increaseDivider = _value;
  }

  function updateDecreaseDivider(uint256 _value) external onlyAdmin {
    decreaseDivider = _value;
  }
  ...
  function _results(
    ...
    if (_contender1Wins > _contender2Wins) {
      ...
      // Increase rate of attacker
      MISSION_ONE_STORAGE.adjustRate(
        _attackerAddr,
        _attackerId,
        _animaIncrease(_opponentAddr, _opponentId)
      );

      // Decrease rate of opponent
      MISSION_ONE_STORAGE.adjustRate(
        _opponentAddr,
        _opponentId,
        _animaDecrease(_opponentAddr, _opponentId)
      );
  ...
  function _animaDecrease(address _addr, uint256 _adventurerId)
    internal
    view
    returns (uint256)
  {
    return MISSION_ONE_STORAGE.rates(_addr, _adventurerId) / decreaseDivider;
  }

  function _animaIncrease(address _addr, uint256 _adventurerId)
    internal
    view
    returns (uint256)
  {
    return MISSION_ONE_STORAGE.rates(_addr, _adventurerId) / increaseDivider;
  }
...
interface IMissionOneStorage {
  ...
  function claim(address _addr, uint256 _id) external;
  ...
  function timers(address _addr, uint256 _id) external view returns (uint256);

  function rates(address _addr, uint256 _id) external view returns (uint256);
}
