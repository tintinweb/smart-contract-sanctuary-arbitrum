...
// BurniLaunchpadV1 v1.2
contract BurniLaunchpadV1 is Ownable, ReentrancyGuard {
  ...
  function whitelistAddresses(address[] memory _addresses, uint256 _allocation) external onlyOwner returns (bool) {
    for (uint256 i = 0; i < _addresses.length; i++) {
      require(_whitelistAddress(_addresses[i], _allocation));
    }
    return true;
  }
