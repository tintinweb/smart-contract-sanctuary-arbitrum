    ...
    ReentrancyGuard
{
    ...
    uint256 public currentMaxExposureAmount;
    uint256 public currentBetAmount;
    uint256 public futureBetAmount;
    ...
    uint256 public intervalForMaxBetAmount = 1 hours;
    uint256 public lastTimestampForMaxBetAmount;
    uint256 public totalBetsForInterval;
    ...
    function _executeRound(
        ...
        for (uint8 i = 0; i < length; i = i + 1) {
            ...
            if (genesisLockedOnce[timeframeId]) {
                ...
                if (
                    block.timestamp >=
                    lastTimestampForMaxBetAmount + intervalForMaxBetAmount
                ) {
                    _updateBettableAmount();
                }
            }
        }
    }
     ...
     */
    function _updateBettableAmount() private {
        (
        ...
        currentMaxExposureAmount = currentExposure;
        currentBetAmount = currentBet;
        futureBetAmount = futureBet;

        lastTimestampForMaxBetAmount = block.timestamp;
    }
    ...
    function openPosition(
        ...
        if (
            ...
            // this means current and future bet amount data are expired, and not up to date. So we need to update it.
            _updateBettableAmount();
        }
    ...
    function setTimeframes(TimeFrame[] memory timeframes_) external onlyAdmin {
        ...
        for (uint256 i = 0; i < length; i = i + 1) {
            ...
            totalBets += intervalForMaxBetAmount / timeframes_[i].interval;
        }
        totalBetsForInterval = totalBets;
    }
    ...
    function setIntervalForMaxAmount(uint256 _time) external onlyAdmin {
        intervalForMaxBetAmount = _time;
        uint256 totalBets;
        for (uint256 i; i < timeframes.length; i++) {
            totalBets += intervalForMaxBetAmount / timeframes[i].interval;
        }

        totalBetsForInterval = totalBets;
    }

    function setMultiplier(uint256 _value) external onlyAdmin {
