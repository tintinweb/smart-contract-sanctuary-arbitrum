...
contract AntiBotStandardTokenTest is IERC20Upgradeable, OwnableUpgradeable, BaseToken {
    using SafeMath for uint256;
    ...
    function initialize(
        ...
        address antiBotContract_
    ) external initializer {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
        _mint(owner_, totalSupply_);
        antiBotContract = IAntiBot(antiBotContract_);
        antiBotContract.setTokenOwner(owner_);
    ...
    function _transfer(
        ...
        if (enableAntiBot) {
            antiBotContract.onPreTransferCheck(sender, recipient, amount);
        }
...
contract AntiBotStandardTokenFactory is TokenFactoryBase {
    using Address for address payable;
    using SafeMath for uint256;
    AntiBotStandardTokenTest[] public tokens;
    ...
    function create(
        ...
        uint8 decimals, 
        uint256 totalSupply,
        address antiBotContract
    ) external payable enoughFee nonReentrant returns (address token) {
        refundExcessiveFee();
        payable(feeTo).sendValue(flatFee);
        token = Clones.clone(implementation);
        AntiBotStandardTokenTest(token).initialize(msg.sender, name, symbol, decimals, totalSupply, antiBotContract);
        tokens.push(AntiBotStandardTokenTest(token));
        assignTokenToOwner(msg.sender, token, 0);
        emit TokenCreated(msg.sender, token, TokenType.standard, 1);
    }

    function getTokens() external view returns(AntiBotStandardTokenTest[] memory){
        return tokens;
    }
}