...
contract FlorinTestFaucet is OwnableUpgradeable {
    using SafeERC20Upgradeable for FlorinToken;

    FlorinToken public florinToken;

    uint256 public florinPerUser;

    mapping(address => bool) private alreadyReceivedFlorin;

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() initializer {} // solhint-disable-line

    function initialize(FlorinToken _florinToken, uint256 _florinPerUser) external initializer {
        __Ownable_init_unchained();
        florinPerUser = _florinPerUser;
        florinToken = _florinToken;
    }

    function setFlorinPerUser(uint256 _florinPerUser) external onlyOwner {
        florinPerUser = _florinPerUser;
    }

    function getFlorin() external {
        require(!alreadyReceivedFlorin[_msgSender()], "already received florin");
        require(florinToken.balanceOf(address(this)) >= florinPerUser, "faucet is empty");
        alreadyReceivedFlorin[_msgSender()] = true;
        florinToken.safeTransfer(_msgSender(), florinPerUser);
    }    
}