...
contract OptionsConfig is Ownable, IOptionsConfig {
    ...
    address public override loyaltyContract;
    ...
    function setLoyaltyContract(address _loyaltyContract) external onlyOwner {
        ...
        emit UpdateLoyaltyContract(_loyaltyContract);
    }

    function setOptionFeePerTxnLimitPercent(
        uint16 _optionFeePerTxnLimitPercent
    ) external onlyOwner {
        optionFeePerTxnLimitPercent = _optionFeePerTxnLimitPercent;
        emit UpdateOptionFeePerTxnLimitPercent(_optionFeePerTxnLimitPercent);
    }

    function setOverallPoolUtilizationLimit(
        uint16 _overallPoolUtilizationLimit
    ) external onlyOwner {
        require(
            _overallPoolUtilizationLimit <= 100e2 &&
                _overallPoolUtilizationLimit > 0,
            "Wrong utilization value"
        );
        overallPoolUtilizationLimit = _overallPoolUtilizationLimit;
        emit UpdateOverallPoolUtilizationLimit(_overallPoolUtilizationLimit);
    }

    function setAssetUtilizationLimit(
        uint16 _assetUtilizationLimit
    ) external onlyOwner {
        require(
            _assetUtilizationLimit <= 100e2 && _assetUtilizationLimit > 0,
            "Wrong utilization value"
        );
        assetUtilizationLimit = _assetUtilizationLimit;
        emit UpdateAssetUtilizationLimit(_assetUtilizationLimit);
    }
...
interface IBufferRouter {
    ...
    struct TradeData {
        ...
        IBufferBinaryOptions.Option option;
        bool isCancelled;
    }
    struct QueueIdMapping {
        uint256 optionId;
        bool isSet;
    }
...
interface IBufferBinaryOptions {
    ...
    event CreateOptionsContract(
        ...
        address pool,
        address tokenX,
        uint256 baseSettlementFeeForAbove,
        uint256 baseSettlementFeeForBelow,
