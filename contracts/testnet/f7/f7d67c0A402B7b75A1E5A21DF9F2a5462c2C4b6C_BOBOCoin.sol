...
import "./swap3/ISwapRouter02.sol";
import "./SwapV3.sol";
...
contract BOBOCoin is ERC20, MyPausable, BlackList, Fee, SwapV3, Mint {
    constructor(
        address devAddress_,
    ...
    function _transfer(
        ...
        _requireNotPaused();
        require(!_getBlackListStatus(sender), "operator in blacklist");
        require(!_getBlackListStatus(recipient), "recipient in blacklist");
    ...
    function _tokenTransferHandle(
        ...
        if (_isPair(from) || _isPair(to)) {
            if (!_isExcludeFromFee(from) && !_isExcludeFromFee(to)) {
                require(swapEnable, "Swap not started");
...
abstract contract SwapV3 is Dev, Fee, Common {
    ...
    bool internal inSwapHandle = false;
    uint256 public buyTotalTax;
    uint256 public sellTotalTax;

    event TradeTax(address sender, uint8 source, uint256 amount);
    event UsdEvent(uint256 amountIn, uint256 amountOut, uint256 usd);

    modifier inSwapHandleLock() {
        inSwapHandle = true;
        _;
        inSwapHandle = false;
    }
    ...
    function _getAmountOutPrice(
        ...
        usd = (_getETHPrice() * amountOut) / 10 ** (18 + 8);
        emit UsdEvent(amountIn, amountOut, usd);
    }
    ...
    function _isPair(address pool) internal view returns (bool) {
    ...
    function removePair(address pair) external returns (bool) {
        _onlyManger();
        return swapPairs.remove(pair);
    }

    function setSwapEnable(bool val) external {
        _onlyManger();
        swapEnable = val;
    }

    function setAddLpEnabled(bool _enabled) external {
        _onlyManger();
    ...
    function launch(
        ...
        swapPairs.add(pool500);
        swapPairs.add(pool3000);
        swapPairs.add(pool10000);
    }
    ...
    function _swapExactInput(
        ...
        amountOut = _swapExactInputRouter2(recipient, _amountIn, poolFee);
        if (amountOut == 0) {
            amountOut = _swapExactInputRouter2(recipient, _amountIn, 10000);
        }
        if (amountOut == 0) {
            amountOut = _swapExactInputRouter2(recipient, _amountIn, 500);
        }
    }
    ...
    function isTokenIdExist(uint256 tokenId) public view returns (bool exists) {
        try positionManager.ownerOf(tokenId) returns (address owner) {
            exists = address(this) == owner;
        } catch {
            exists = false;
        }
    }
    ...
    function _doAddLp(uint256 amount0, uint256 amount1) internal {
        if (!addLpEnabled || amount0 == 0 || amount1 == 0) {
            return;
        }
        TransferHelper.safeApprove(swapToken0, address(swapRouter), amount0);
        TransferHelper.safeApprove(swapToken1, address(swapRouter), amount1);

        if (ownTokenId == 0 || !isTokenIdExist(ownTokenId)) {
            mintNewPosition(swapToken0, swapToken1, amount0, amount1);
        } else {
            _increaseLp(ownTokenId, amount0, amount1);
        }
    }
    ...
    function mintNewPosition(
        ...
        (ownTokenId, , , ) = positionManager.mint(
    ...
    function _doClaim() internal {
        if (jackpotAddress != address(0))
            try IBidFomo(jackpotAddress).claimReward() {} catch {}
        if (luckyAddress != address(0))
            try ILuckyPool(luckyAddress).claimReward() {} catch {}
    }
    ...
    function _transferFromSwap(
        ...
        uint256 tax = _calcPercent(
            amount,
            source == 1
                ? (buyLiquidityFee + buyJackpotFee + buyDevFee)
                : (sellBlackFee +
                    sellLiquidityFee +
                    sellJackpotFee +
                    sellBonusFee +
        ...
        actualAmount = amount - tax;
        _collectTax(from, source, tax);
        uint256 usd = _getAmountOutPrice(actualAmount);
        if (source == 1) {
            buyTotalTax += tax;
        } else {
            sellTotalTax += tax;
            if (luckyAddress != address(0)) {
                try
                    ILuckyPool(luckyAddress).injectRewards(_msgSender(), usd)
                {} catch {}
            }
        }
        _doJackpot(2, usd);
    ...
    function _collectTax(address sender, uint8 source, uint256 tax) internal {
        _innerTransfer(sender, address(this), tax);
        emit TradeTax(sender, source, tax);
    }

    function _taxSwap() internal inSwapHandleLock {
        _taxSwapBuy();
        _taxSwapSell();
    }
    ...
    function _taxSwapBuy() internal {
        ...
        uint256 totalFee = buyDevFee + buyJackpotFee + buyLiquidityFee;
        _swapExactInput(
            devAddress,
            _calcPercent2(buyTotalTax, buyDevFee, totalFee)
        );
    ...
    function _taxSwapSell() internal {
        ...
        _swapExactInput(
            devAddress,
            _calcPercent2(sellTotalTax, sellDevFee, totalFee)
        );
        _innerTransfer(
            address(this),
            blackholdAddress,
        ...
        if (stakeEarnAddress != address(0)) {
            _innerTransfer(
                address(this),
                stakeEarnAddress,
                _calcPercent2(sellTotalTax, sellBonusFee, totalFee)
            );
            try
                IBonusPool(stakeEarnAddress).injectRewards(
                    _calcPercent2(sellTotalTax, sellBonusFee, totalFee)
                )
            {} catch {}
        }
    ...
    function _doJackpotSwap(uint256 jackpot) internal {
        if (jackpotAddress != address(0)) {
            _swapExactInput(jackpotAddress, jackpot);
        }
    }

    function _doJackpot(uint8 source, uint256 usd) internal {
        if (jackpotAddress != address(0)) {
            try IBidFomo(jackpotAddress).injectRewards(source, usd) {} catch {}
        }
    }
    ...
    function _addLp(uint256 liquidity) internal {
        ...
        uint256 wethAmount = _swapExactInput(address(this), half);
        _doAddLp(half, wethAmount);
    }
}
...
abstract contract MyPausable is Pausable, Dev {
    function setPaused(bool pause) external virtual {
        _onlyManger();
        if (pause) {
            _pause();
        } else {
...
abstract contract Mint is Dev, Common {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    event Mints(address user, uint256 quota);
    ...
    function mintWhitelist(uint256 quota) external payable {
        ...
        require(_isWhitelistUser(_msgSender()), "WL: not whitelistUser");
        require(
    ...
    function mintOpen(uint256 quota) external payable {
        ...
        require(
            msg.value >= quota * openPool.unitPrice,
            "OP: Not enough ETH sent"
        );
        openPool.soldQuota += quota;
        openMints[_msgSender()] += quota;
        payable(owner()).transfer(msg.value);
        _innerTransfer(address(this), _msgSender(), quota * openPool.sendPer);
        emit Mints(_msgSender(), quota);
    }
    ...
    function isWhitelistUser(address account) external view returns (bool) {
        return _isWhitelistUser(account);
    }

    function addWhitelistUser(address[] memory accounts) external {
        _onlyManger();
    ...
    function removeWhitelistUser(address[] memory accounts) external {
        _onlyManger();
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function _isWhitelistUser(address account) internal view returns (bool) {
...
interface ILuckyPool {
    function injectRewards(address user, uint256 usd) external;
...
interface IBidFomo {
    function injectRewards(uint8 source, uint256 usd) external;
...
abstract contract Fee is Dev {
    ...
    address public stakeEarnAddress = address(0);
    ...
    uint256 public buyJackpotFee = 25;

    // sell fees
    uint256 public sellBlackFee = 10;
    uint256 public sellDevFee = 10;
    uint256 public sellLiquidityFee = 10;
    uint256 public sellJackpotFee = 30;
    uint256 public sellBonusFee = 25;
    uint256 public sellLuckyFee = 15;
    ...
    function setFeeAddress(
        address _stakeEarnAddress,
        address _jackpotAddress,
        address _luckyAddress
    ) external {
        _onlyManger();
        stakeEarnAddress = _stakeEarnAddress;
    ...
    function addExcludeFromFee(address account) external {
        _onlyManger();
        _addExcludeFromFee(account);
    }
    ...
    function removeExcludeFromFee(address[] memory accounts) external {
        _onlyManger();
        for (uint256 i = 0; i < accounts.length; i++) {
    ...
    function _isExcludeFromFee(address account) internal view returns (bool) {
...
abstract contract Dev is Ownable {
    ...
    function rescue721(address tokenAddress, uint256 tokenId) external {
        _onlyManger();
...
abstract contract BlackList is Dev {
    ...
    function setBlackList(address[] memory _evilUsers, bool _is) external {
        _onlyManger();
