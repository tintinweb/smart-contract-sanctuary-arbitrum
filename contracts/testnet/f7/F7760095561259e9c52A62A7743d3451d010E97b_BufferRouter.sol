    ...
    ERC1155Holder
{
    ...
    uint256 public override nextQueueId = 0;

    bool public isInPrivateKeeperMode = true;
    uint64 public maximumPriceDelayForResolving = 2 minutes;
    ...
    mapping(uint256 => QueuedTrade) public override queuedTrades;
    mapping(address => bool) public contractRegistry;
    mapping(address => bool) public isKeeper;
    IPyth public pyth;

    constructor(address _tournamentManager, IPyth _pyth) {
        pyth = _pyth;
    ...
    function getPrice(
        ...
        IPyth.PriceFeed[] memory prices = pyth.parsePriceFeedUpdates{
            ...
            priceId,
            uint64(timestamp),
            uint64(timestamp + maximumPriceDelayForResolving)
        );
    ...
    function resolveQueuedTrades(
        OpenTradeParams[] calldata params
    ) external payable {
        _validateKeeper();
        for (uint32 index = 0; index < params.length; index++) {
            ...
            if (block.timestamp - queuedTrade.queuedTime <= MAX_WAIT_TIME) {
                _openQueuedTrade(currentParams.queueId, currentPrice);
            } else {
