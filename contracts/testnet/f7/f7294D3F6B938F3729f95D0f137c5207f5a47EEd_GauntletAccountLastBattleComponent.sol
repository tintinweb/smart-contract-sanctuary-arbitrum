...
uint256 constant ID = uint256(
    keccak256("game.piratenation.gauntletaccountlastbattlecomponent.dev1")
);
...
library GauntletAccountLastBattleComponentStorage {
...
/**
 * @title GauntletAccountLastBattleComponent
 * @dev A player&#39;s most recent battle entity
 */
contract GauntletAccountLastBattleComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Entity for player&#39;s most recent battle
        keys[0] = "battle_entity";
     ...
     *
     * @param entity Entity to get value for
     * @param battleEntity Entity for player&#39;s most recent battle
     */
    function setValue(
        uint256 entity,
        uint256 battleEntity
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(battleEntity));
    }
    ...
    function getLayoutValue(
        ...
        value = GauntletAccountLastBattleComponentStorage
            .layout()
     ...
     *
     * @param entity Entity to get value for
     * @return battleEntity Entity for player&#39;s most recent battle
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256 battleEntity) {
        if (has(entity)) {
            Layout memory s = GauntletAccountLastBattleComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (battleEntity) = abi.decode(_getEncodedValues(s), (uint256));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = GauntletAccountLastBattleComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](1);
        values[0] = abi.encode(s.battleEntity);
    }
    ...
    function setBytes(
        ...
        Layout memory s = GauntletAccountLastBattleComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.battleEntity) = abi.decode(value, (uint256));
    ...
    function remove(
        ...
        delete GauntletAccountLastBattleComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.battleEntity = value.battleEntity;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.battleEntity));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.battleEntity);
    }
}