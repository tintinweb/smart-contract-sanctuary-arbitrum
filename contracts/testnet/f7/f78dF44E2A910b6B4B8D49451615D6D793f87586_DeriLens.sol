...
contract DeriLens is NameVersion {
    ...
    function getAlpInfo(address pool_) public view returns (AlpInfo [] memory infos) {
        ILensPool pool = ILensPool(pool_);
        uint256 length = pool.allWhitelistedTokensLength();
        IVault vault = IVault(pool.lpVault());
        infos = new AlpInfo[](length);
        for (uint256 i = 0; i<length; i++){
            address token = pool.allWhitelistedTokens(i);
            int256 tokenVolume = vault.getVaultLiquidityTokenVolume(token).utoi();
            infos[i].symbol = IERC20(token).symbol();
            infos[i].symbolAddress = token;
            if (tokenVolume != 0){
                infos[i].volume = tokenVolume;
            }
        }
    }
