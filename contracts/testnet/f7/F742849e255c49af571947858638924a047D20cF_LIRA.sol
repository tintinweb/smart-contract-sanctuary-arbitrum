...
contract LIRA is ERC20("Satoshi LIRA", "LIRA"), Ownable {
    ...
    function mint(uint256 amount) public onlyOwner {
        require(wbtc.balanceOf(address(this)) >= _lockedAmount + amount, 'LIRA: ISSUFFICENT_WBTC_BALANCE');

        uint mintAmount = _lockedAmount != 0
            ? quote(amount, totalSupply(), _lockedAmount)
            : amount * 10 ** 8;
        ...
        _mint(_msgSender(), mintAmount.div(100).mul(95));
    }
    ...
    function burn(address to, uint256 amount) public {
        require(amount <= totalSupply(), 'LIRA: INSUFFICENT_SUPPLY');

        uint feeAmount = amount.div(1000).mul(5);
        ...
        uint wbtcFee = quoteAmount.div(1000).mul(5);
    ...
    function quoteWbtc(uint amount) public view returns (uint) {
        return quote(amount, _lockedAmount, totalSupply());
    }

    function quoteLira(uint amount) public view returns (uint) {
        return quote(amount, totalSupply(), _lockedAmount);
    }
