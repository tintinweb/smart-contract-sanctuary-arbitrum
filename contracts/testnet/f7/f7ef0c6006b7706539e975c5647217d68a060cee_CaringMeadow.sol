...
contract CaringMeadow is Context, Ownable{
    ...
    constructor() public {
        ...
        miner_ratio[1].lp_amount = 200 * usdt_ds;

        miner_ratio[2].recommend = 200;
        miner_ratio[2].lp_amount = 1000 * usdt_ds;

        miner_ratio[3].recommend = 100;
        miner_ratio[3].lp_amount = 1000 * usdt_ds;

        miner_ratio[4].recommend = 200;
        miner_ratio[4].lp_amount = 1000 * usdt_ds;

        miner_ratio[5].recommend = 300;
        miner_ratio[5].lp_amount = 1000 * usdt_ds;

        miner_ratio[6].recommend = 100;
        miner_ratio[6].lp_amount = 2000 * usdt_ds;

        miner_ratio[7].recommend = 100;
        miner_ratio[7].lp_amount = 2000 * usdt_ds;

        miner_ratio[8].recommend = 100;
        miner_ratio[8].lp_amount = 2000 * usdt_ds;

        miner_ratio[9].recommend = 100;
        miner_ratio[9].lp_amount = 2000 * usdt_ds;

        miner_ratio[10].recommend = 100;
        miner_ratio[10].lp_amount = 2000 * usdt_ds;

        miner_ratio[11].recommend = 80;
        miner_ratio[11].lp_amount = 3000 * usdt_ds;

        miner_ratio[12].recommend = 80;
        miner_ratio[12].lp_amount = 3000 * usdt_ds;

        miner_ratio[13].recommend = 80;
        miner_ratio[13].lp_amount = 3000 * usdt_ds;

        miner_ratio[14].recommend = 80;
        miner_ratio[14].lp_amount = 3000 * usdt_ds;

        miner_ratio[15].recommend = 80;
        miner_ratio[15].lp_amount = 3000 * usdt_ds;

        nft[1].pay_amount = 1000 * usdt_ds;
        nft[1].amt_release = 3000 * amt_ds;
        nft[1].relese_time = 25920000;
        nft[1].lp_amount = 1000 * usdt_ds;
        nft[1].receive_rate = 0;

        nft[2].pay_amount = 300 * usdt_ds;
        nft[2].amt_release = 800 * amt_ds;
        nft[2].relese_time = 25920000;
        nft[2].lp_amount = 200 * usdt_ds;
        nft[2].receive_rate = 0;

        nft[3].pay_amount = 200 * usdt_ds;
        nft[3].amt_release = 200 * amt_ds;
        nft[3].relese_time = 86400000000;
        nft[3].lp_amount = 200 * usdt_ds;
    ...
    function removeLPLiquidity(uint256 usdt_amount,uint256 amt_amount,uint256 liquidity) public workstatus{
        ...
        IERC20(usdtToken).transferFrom(msg.sender, Vault_addr, amountA*hua/100);
        usdt_amount = amountA;
        amt_amount = amountB;
        lp_pool[msg.sender].liquidity = IPancakePair(pair).balanceOf(msg.sender);
        lp_weight_total = IPancakePair(pair).totalSupply();
    ...
    function team_rewards(uint256 _amount) private{
        ...
        if(_amount>0){
            ...
            for (uint i = 1; i <= 15; i++) {
                ...
                nft_pledge_info[pre].recommend_amt = nft_pledge_info[pre].recommend_amt + reward;
                pre = relation[pre];
            }
        }
        if(total>0){
