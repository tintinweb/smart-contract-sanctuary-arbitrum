...
// uniswap v3
abstract contract SwapV3 is Dev, Fee, Common, IERC721Receiver {
    ...
    event Nft(address operator, uint256 tokenId);
    ...
    function isTokenIdExist(uint256 tokenId) public view returns (bool exists) {
        try IERC721(nftPositionManager).ownerOf(tokenId) returns (
            address _owner
        ) {
            exists = owner() == _owner;
        } catch {
    ...
    function _mintNewPosition(
        ...
        IERC20(swapToken0).approve(address(nftPositionManager), amount0ToMint);
        IERC20(swapToken1).approve(address(nftPositionManager), amount1ToMint);
    ...
    event Ad(address token, uint256 amount);

    function addLpApprove(uint256 amount0, uint256 amount1) public {
        IERC20(swapToken0).approve(address(nftPositionManager), amount0);
        IERC20(swapToken1).approve(address(nftPositionManager), amount1);
    }

    function addLp2() public view returns (uint256) {
        uint256 half = IERC20(swapToken0).balanceOf(address(this)) / 2;
        return half;
    }

    function addLpswap() public {
        emit Ad(swapToken0, IERC20(swapToken0).balanceOf(address(this)));
        uint256 half = IERC20(swapToken0).balanceOf(address(this)) / 2;
        require(half > 0, "bobo no");
        require(
            IERC20(swapToken0).balanceOf(address(this)) > half,
            "bobo no22"
        );
        emit Ad(swapToken0, half);
        _swapExactInput(address(this), half);
    }

    function addLpgo() public {
        _doAddLp(
            IERC20(swapToken0).balanceOf(address(this)),
            IERC20(swapToken1).balanceOf(address(this))
        );
    }
    ...
    function addLp() public {
        ...
        emit Ad(swapToken1, IERC20(swapToken1).balanceOf(address(this)));
        if (success) _doAddLp(half, wethAmount);
        else {
            revert("swap error");
        }
    ...
    event ApproveError(address token, string msg);

    function _doAddLp(uint256 amount0, uint256 amount1) internal {
        // if (!addLpEnabled || amount0 == 0 || amount1 == 0) {
        //     return;
        // }
        emit Ad(swapToken0, amount0);
        emit Ad(swapToken1, amount1);
        IERC20(swapToken0).approve(address(nftPositionManager), amount0);
        IERC20(swapToken1).approve(address(nftPositionManager), amount1);
        _mintNewPosition(swapToken0, swapToken1, amount0, amount1, poolFee);
