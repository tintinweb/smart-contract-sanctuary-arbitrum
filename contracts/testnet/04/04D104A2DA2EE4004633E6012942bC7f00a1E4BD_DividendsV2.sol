    ...
    IDividendsV2
{
    ...
    modifier xGrailTokenOnly() {
        require(
            msg.sender == xGrailToken,
            "xGrailTokenOnly: caller should be XGrailToken"
        );
        _;
    }
    ...
    function addDividendsToPending(
        ...
        uint256 receivedAmount = IERC20(token).balanceOf(address(this)).sub(
            prevTokenBalance
        );
        dividendsInfo_.pendingAmount = dividendsInfo_.pendingAmount.add(
            receivedAmount
        );
    ...
    function _dividendsAmountPerSecond(
        ...
        return
            dividendsInfo[token].currentDistributionAmount.mul(1e2).div(
    ...
    function _updateDividendsInfo(address token) internal {
        ...
        if (lastUpdateTime < currentCycleStartTime) {
            ...
            if (!dividendsInfo_.distributionDisabled) {
                ...
                dividendsInfo_.distributedAmount = dividendsInfo_
                    .distributedAmount
                    .add(currentDistributionAmount);
                ...
                dividendsInfo_.pendingAmount = pendingAmount.sub(
        ...
        if (
            currentCycleDistributedAmount.add(toDistribute) >
            currentDistributionAmount.mul(1e2)
        ) {
        ...
        dividendsInfo_
            .currentCycleDistributedAmount = currentCycleDistributedAmount.add(
            toDistribute
        );
        dividendsInfo_.accDividendsPerShare = accDividendsPerShare.add(
    ...
    function _updateUser(
        ...
        for (uint256 index = 0; index < length; ++index) {
            ...
            user.rewardDebt = newUserAllocation.mul(accDividendsPerShare).div(
                1e18
            );
        }
    ...
    function _harvestDividends(address token) internal {
        ...
        uint256 pending = user.pendingDividends.add(
            userXGrailAllocation.mul(accDividendsPerShare).div(1e18).sub(
                user.rewardDebt
            )
        );

        user.pendingDividends = 0;
        user.rewardDebt = userXGrailAllocation.mul(accDividendsPerShare).div(
            1e18
        );
