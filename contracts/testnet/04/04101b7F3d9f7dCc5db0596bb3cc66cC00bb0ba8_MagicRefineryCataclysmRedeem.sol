...
uint256 constant MAGIC_REFINERY_PROPERTY_LEVEL = 0;
uint256 constant MAGIC_REFINERY_PROPERTY_AMOUNT_SPENT = 1;
uint256 constant MAGIC_REFINERY_PROPERTY_CREATED_AT = 2;
...
import "./IMagicRefineryDataConstants.sol";
import "./IMagicRefineryData.sol";

contract MagicRefineryCataclysmRedeem is
  ReentrancyGuard,
  Pausable,
  ManagerModifier
{
  ...
  // Events
  //=======================================
  event Redeemed(address structureAddress, uint256 structureId);
  ...
  constructor(
    ...
    redeemAmounts[3] = 1000 ether;
    redeemAmounts[4] = 3500 ether;
    redeemAmounts[5] = 10000 ether;
  }
  ...
  function redeem(uint256[] calldata _ids) external nonReentrant whenNotPaused {
    ...
    for (uint256 j = 0; j < _ids.length; j++) {
      ...
      // Check if level is above 2
      if (level > 2) {
        // Check sender owns refinery
        require(
          REFINERY.ownerOf(structureId) == msg.sender,
          "MagicRefineryCataclysmRedeem: You do not own the Refinery"
        );
        ...
        // Collect amount redeemed from each refinery for final transfer
        totalMagic += redeemAmounts[level];

        emit Redeemed(address(REFINERY), structureId);
      }
    }
    ...
    if (totalMagic > 0) {
      // Transfer $MAGIC
      TOKEN.safeTransferFrom(VAULT, msg.sender, totalMagic);
    }
  }
