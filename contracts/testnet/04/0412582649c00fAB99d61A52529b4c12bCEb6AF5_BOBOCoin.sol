...
contract BOBOCoin is ERC20, MyPausable, BlackList, Fee, SwapV3 {
    uint256 private buyTotalTax;
    uint256 private sellTotalTax;
    ...
    function _taxSwapBuy() internal {
        if (buyTotalTax == 0 || buyTotalTax > balanceOf(address(this))) {
            return;
        }
        swapV3Single(
            swapToken0,
            swapToken1,
            devAddress,
            calcPercent(buyTotalTax, buyDevFee)
        );
        _doJackpotSwap(calcPercent(buyTotalTax, buyJackpotFee));
        _addLp(calcPercent(buyTotalTax, buyLiquidityFee));
        buyTotalTax = 0;
    }

    function _taxSwapSell() internal {
        if (sellTotalTax == 0 || sellTotalTax > balanceOf(address(this))) {
            return;
        }
        swapV3Single(
            swapToken0,
            swapToken1,
            devAddress,
            calcPercent(sellTotalTax, sellDevFee)
        );
        _doJackpotSwap(calcPercent(sellTotalTax, sellJackpotFee));
        _doLuckySwap(calcPercent(sellTotalTax, sellLuckyFee));
        _addLp(calcPercent(sellTotalTax, sellLiquidityFee));
        super._transfer(
            address(this),
            blackholdAddress,
            calcPercent(sellTotalTax, sellBlackFee)
        );
        if (stakeEarnAddress != address(0)) {
            super._transfer(
                address(this),
                stakeEarnAddress,
                calcPercent(sellTotalTax, sellBonusFee)
            );
    ...
    function _doLuckySwap(uint256 amount) internal {
        if (
            luckyAddress != address(0) && ILuckyPool(luckyAddress).inProgress()
        ) {
            swapV3Single(swapToken0, swapToken1, luckyAddress, amount);
        }
    }
    ...
    function _doJackpotSwap(uint256 jackpot) internal {
        if (
            ...
            IBidFomo(jackpotAddress).inProgress()
        ) {
            swapV3Single(swapToken0, swapToken1, jackpotAddress, jackpot);
        }
    }
    ...
    function _doJackpot(uint8 source, uint256 usd) internal {
        if (
            ...
            IBidFomo(jackpotAddress).injectRewards(source, usd);
        }
    }
    ...
    function _addLp(uint256 liquidity) internal {
        ...
        uint256 wethAmount = swapV3Single(
            swapToken0,
            swapToken1,
            address(this),
            half
        );
        addLp(half, wethAmount);
    }
}
...
// uniswap v3
abstract contract SwapV3 is Dev, Fee, ERC721Holder {
    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private swapPairs;
    ...
    function _poolExists() internal view returns (bool) {
    ...
    function addLp(uint256 amount0, uint256 amount1) internal {
        ...
        if (ownTokenId == 0 || !isTokenIdExist(ownTokenId)) {
            ...
            increaseLp(ownTokenId, amount0, amount1);
        }
    }
    ...
    function _getAmountOut(uint256 amountIn) internal view returns (uint256) {
        ...
        uint256 amountOut = (amountIn * priceX96) >> 96;
        return amountOut;
    }
    ...
    function _getAmountOutPrice(
        ...
        uint256 eth = _getETHPrice();
        usd = (eth * amountOut) / 10 ** 6; // token decimal
        emit UsdEvent(amountIn, amountOut, eth, usd);
    }
}
