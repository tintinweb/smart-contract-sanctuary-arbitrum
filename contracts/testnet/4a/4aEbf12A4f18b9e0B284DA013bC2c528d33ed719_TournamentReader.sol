...
contract TournamentReader {
    ...
    constructor(address _tournamentManager) {
        ...
        leaderboard = ITournamentLeaderboard(tournamentManager.leaderboard());
    }
    ...
    function bulkFetchTournaments(
        ...
        view
        returns (
            ITournamentReader.TournamentWithTokens[] memory bulkTournaments
        )
    {
        bulkTournaments = new ITournamentReader.TournamentWithTokens[](
            tournamentIds.length
        );
        for (uint256 i = 0; i < tournamentIds.length; i++) {
            (
                ITournamentManager.TournamentMeta memory tournamentMeta,
                ITournamentManager.TournamentConditions
                    memory tournamentConditions
            ) = tournamentManager.tournaments(tournamentIds[i]);
            bulkTournaments[i] = ITournamentReader.TournamentWithTokens(
                tournamentMeta,
                tournamentConditions,
                tournamentMeta.buyinToken.symbol(),
                tournamentMeta.buyinToken.decimals(),
                tournamentMeta.rewardToken.symbol(),
                tournamentMeta.rewardToken.decimals()
            );
        }
    }
    ...
    function isTradingAllowed(
        ...
        require(
            tournamentManager.tournamentUnderlyingAssets(tournamentId, asset),
            "T17"
        );
        (
            ITournamentManager.TournamentMeta memory tournamentMeta,

        ) = tournamentManager.tournaments(tournamentId);
        require(tournamentMeta.tradingStarted, "T18");
        require(expiration < tournamentMeta.close, "T19");
    }
    ...
    function getTournamentConditions(
        ...
        (
            ,
            ITournamentManager.TournamentConditions memory tournamentConditions
        ) = tournamentManager.tournaments(tournamentId);
        return tournamentConditions;
    }
    ...
    function checkParams(
        ...
        ITournamentManager.TournamentType tournamentType = tournamentMeta
            .tournamentType;
        if (
            tournamentType == ITournamentManager.TournamentType.Type3 &&
            ...
            (tournamentType == ITournamentManager.TournamentType.Type1 &&
        ...
        for (
            uint256 i = 0;
            i < tournamentLeaderboard.rewardPercentages.length;
            i++
        ) {
            totalRewardPercentage += tournamentLeaderboard.rewardPercentages[i];
        }
        if (totalRewardPercentage != 100e2) {
            revert("T14");
        }
    }
    ...
    function checkBuyConditions(
        ...
        ERC20 buyinToken = tournament.tournamentMeta.buyinToken;

        require(block.timestamp < tournament.tournamentMeta.close, "T5");
        require(tournament.tournamentMeta.isVerified, "T0");
        require(
        ...
        uint256 tournamentUserTicketCount = leaderboard
            .tournamentUserTicketCount(tournamentId, user);
        require(
            tournamentUserTicketCount == 0 ||
                tournament.tournamentMeta.tradingStarted,
            "T4"
        );
        require(
            tournamentUserTicketCount <
