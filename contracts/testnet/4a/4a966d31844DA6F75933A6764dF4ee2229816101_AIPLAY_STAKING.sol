...
contract AIPLAY_STAKING is ReentrancyGuard, Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using SafeMath for uint;
    
    uint256[5] public Duration = [14,90,180,365];
    uint256[5] public Bonus = [1200,6000,12000,16000];
    ...
    uint256 public emergencyWithdrawFees; // 25% ~ 2500
    ...
    address public feesRecevierAddress;
    ...
    constructor(
        ...
        address _emergencyWithdrawfeesReceiver
    ) {
        token = IERC20(_token);
        started = _started;
        emergencyWithdrawFees = _emergencyWithdrawFees;
        feesRecevierAddress = _emergencyWithdrawfeesReceiver;
    }
    ...
    function getAllPoolInfo() public view returns(uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256){
        return(Duration[0],Duration[1],Duration[2],Duration[3],Bonus[0],Bonus[1],Bonus[2],Bonus[3]);
    }
   ...
   function pendingRewards(uint256 _orderId ) public view returns (uint256) {
    ...
    function claimReward(uint256 _orderId ) public nonReentrant {
        OrderInfo storage orderInfo = orders[_orderId];
        require(_orderId <= latestOrderId && orderInfo.amount > 0 && !orderInfo.claimed , "Order Already claimed!");
        uint256 reward;
        uint256 claimAvailable;
        if(block.timestamp >= orderInfo.endtime){
            reward = (orderInfo.amount.mul(orderInfo.Bonus).mul(orderInfo.Duration)).div(10000*365);
            ...
            token.transfer(address(msg.sender) , claimAvailable);
        }
        else{
            uint256 stakeTime = block.timestamp.sub(orderInfo.starttime);
            reward = (orderInfo.amount.mul(orderInfo.Bonus).mul(stakeTime)).div(10000*365*86400);
    ...
    function withdrawARB(address payable _reciever, uint256 _amount) public onlyOwner {
