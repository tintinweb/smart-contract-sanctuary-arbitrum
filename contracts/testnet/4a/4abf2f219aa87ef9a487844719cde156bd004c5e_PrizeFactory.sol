...
contract PrizeFactory is Ownable {
    ...
    string public constant NFT_BASE_URI = "https://google.com/";
    ...
    function createPrizePool(
        ...
        //create NFT
        PrizeNFT prizeNFT = new PrizeNFT(round, NFT_BASE_URI);
...
contract PrizeNFT is ERC721, ERC721Enumerable, Ownable, IPrizeNFT {
    ...
    constructor(
        ...
        ERC721(
            string(abi.encodePacked("PrizeNFT-Round-", Strings.toString(round))),
            "PBox"
        )
    {
        baseURI = _baseURI;
...
contract PrizePool is VRFConsumerBaseV2 {
    ...
    mapping(address => uint256) userLastMintTimestamp;
    ...
    //indexId => claimed
    mapping(uint256 => bool) claimMap;

    PrizeFactory.PoolType public poolType;


    event Mint(address indexed user, uint8 amount, uint256 fee, uint256 ethValue);

    event HoldLottery(address indexed user, PrizeFactory.PoolType poolType, uint256 requestId, uint32 numWords);

    event RequestFulfilled(uint256 indexed _requestId, uint256[] _randomWords);

    event ClaimReward(address indexed user, uint256 balanceOf, uint256 claimAmount);
    ...
    function fulfillRandomWords(
        ...
        perNFTReward = address(this).balance / _randomWords.length;
    ...
    function claimReward() public {
        ...
        for (uint256 i = 0; i < balanceOf; i++) {
            ...
            for (uint256 j = 0; j < winners.length; j++) {
                if (tokenId == winners[j]) {
                    if (!claimMap[tokenId]) {
                        claimMap[tokenId] = true;
                        claimAmount += perNFTReward;
                    }
                }
            }
        }
        require(claimAmount > 0, "rewards to be claim are 0");
        sendEther(payable(msg.sender), claimAmount);
        emit ClaimReward(msg.sender, balanceOf, claimAmount);
    }
    ...
    function getClaimAmount() public view returns(uint256 claimAmount) {
        require(perNFTReward > 0, "reward error");
