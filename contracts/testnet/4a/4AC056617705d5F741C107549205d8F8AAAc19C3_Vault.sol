...
contract Vault is ReentrancyGuard, IVault {
    ...
    function _addLiquidity(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdg) external returns (uint256) {
        require(_amount > 0, "DlpManager: invalid _amount");
        IERC20(_token).safeTransferFrom(_fundingAccount, address(this), _amount);
        uint256 price = getMinPrice(_token);
        uint256 mintAmount = _amount.mul(price).div(PRICE_PRECISION);
        require(mintAmount >= _minUsdg, "DlpManager: insufficient USDG output");
        _increaseGuaranteedUsd(_token, mintAmount);
        IMintable(dlp).mint(_account, mintAmount);
        emit AddLiquidity(_account, _token, _amount, mintAmount);
        return mintAmount;
    }

    function _removeLiquidity(address _account, address _tokenOut, uint256 _dlpAmount, uint256 _minOut, address _receiver) external returns (uint256) {
        ...
        _decreaseGuaranteedUsd(_tokenOut, _dlpAmount);
        IERC20(_tokenOut).transfer(_receiver, amountOut);
