...
contract GameLogic {
    ...
    uint public PERCENT_BASE = 10000;

    address private _balanceReceiver;
    uint private _winRate;
    uint private _feeRate;

    constructor(address gameRegistry_, uint bulletPrice_, address balanceReceiver_, uint winRate_, uint feeRate_) {
    ...
    function buyBullets(uint amount_) external payable {
        ...
        uint feeAmount = (msg.value * _feeRate) / PERCENT_BASE;
    ...
    function hitRegister(
        ...
        if (bullets_.length == etherIds_.length) {
            ...
            uint winAmount = (bulletPrice / _winRate) * PERCENT_BASE;
