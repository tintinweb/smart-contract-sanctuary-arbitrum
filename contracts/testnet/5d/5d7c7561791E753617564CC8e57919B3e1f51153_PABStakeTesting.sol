...
contract PABStakeTesting is PABStake {
  ...
  ) PABStake(_peekaboo, _BOO, _daily_boo_reward_rate) {}

  function claimPeekABoo(uint256 tokenId, bool unstake) internal override returns (uint256 toBePaid) {
    ...
    require(currentTimestamp > 0, "no days have passed");
    require(currentTimestamp - peekabooStaked.value >= MINIMUM_TO_EXIT, "Must have atleast 1 day worth of $BOO");
    if (totalBooEarned < MAXIMUM_GLOBAL_BOO) {
      toBePaid =
        ((currentTimestamp - peekabooStaked.value) * DAILY_BOO_RATE[peekabooRef.getTokenTraits(tokenId).tier]) /
        1 days;
    } else if (peekabooStaked.value > lastClaimTimestamp) {
      toBePaid = 0; // $BOO production stopped already
    } else {
      toBePaid =
        ((lastClaimTimestamp - peekabooStaked.value) * DAILY_BOO_RATE[peekabooRef.getTokenTraits(tokenId).tier]) /
    ...
    if (unstake) {
      ...
      pabstake[tokenId] = PeekABooNormalStaked({ owner: _msgSender(), tokenId: tokenId, value: currentTimestamp }); // reset pabstake
    }
    emit PeekABooClaimed(tokenId, toBePaid, unstake);
  }

  function setCurrentTimestamp(uint256 numberOfDays) external onlyOwner {
    if (currentTimestamp == 0) {
      currentTimestamp = block.timestamp + (numberOfDays * 60 * 60 * 24) + 60;
    } else {
      currentTimestamp = currentTimestamp + (numberOfDays * 60 * 60 * 24) + 60;
    }
  }

  function getPeekABooValue(uint256[] calldata tokenIds) public view override returns (uint256[] memory) {
    PeekABoo peekabooRef = peekaboo;
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      if (totalBooEarned < MAXIMUM_GLOBAL_BOO) {
        toBePaid =
          ((currentTimestamp - timestamps[i]) * DAILY_BOO_RATE[peekabooRef.getTokenTraits(tokenIds[i]).tier]) /
          1 days;
      } else if (timestamps[i] > lastClaimTimestamp) {
        toBePaid = 0; // $BOO production stopped already
      } else {
        toBePaid =
          ((lastClaimTimestamp - timestamps[i]) * DAILY_BOO_RATE[peekabooRef.getTokenTraits(tokenIds[i]).tier]) /
          1 days; // stop earning additional $BOO if it's all been earned
      }
      values[i] = toBePaid;
    }
