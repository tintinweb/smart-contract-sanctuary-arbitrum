...
contract TransformP3R1Component is BaseStorageComponent {
    ...
    function getValue(
        uint256 entity
    ) external view virtual returns (TransformP3R1 memory transform) {
        // Get the struct from storage
        transform = TransformP3R1ComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
...
contract IslandSystem is SceneSystem {
    ...
    function _canRemoveSceneObject(
        ...
        return instanceParent == sceneEntity;
    }
}
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.sceneobjectparentcomponent")
);

/**
 * @dev maps entity id → owner address
 */
contract SceneObjectParentComponent is Uint256Component {
    /** SETUP **/
...
import {SceneObjectParentComponent, ID as SCENE_OBJECT_PARENT_COMPONENT_ID} from "./SceneObjectParentComponent.sol";
import {SceneObjectGameItemComponent, ID as SCENE_OBJECT_GAME_ITEM_COMPONENT_ID} from "./SceneObjectGameItemComponent.sol";
...
abstract contract SceneSystem is OwnerSystem, ISceneSystem {
    ...
    function _removeSceneObject(
        ...
        uint256 objectEntity = SceneObjectGameItemComponent(
            _gameRegistry.getComponent(SCENE_OBJECT_GAME_ITEM_COMPONENT_ID)
        ).getValue(instanceEntity);
        ...
        _mintSceneObject(account, objectEntity);
