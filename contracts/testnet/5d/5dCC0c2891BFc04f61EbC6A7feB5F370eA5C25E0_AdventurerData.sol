  ...
  Pausable
{
  ...
  event CreatedAoVProfession(
    address addr,
    uint256 id,
    uint256 archetype,
    uint256 profession
  );
  event CreatedAoVPoints(
    address addr,
    uint256 id,
    uint256 archetype,
    uint256 profession,
  ...
  function createFor(address _addr, uint256 _id) external override onlyManager {
    // Create traits
  ...
  // For AoV With profession
  function createFor(
    address _addr,
    uint256 _id,
    uint256 _archetype,
    uint256 _profession
  ) external override onlyManager checkProps(_archetype, _profession) {
    // Create traits
    _createTraits(_addr, _id);
    ...
    aov[_addr][_id][1] = _archetype; // Archetype ID
    aov[_addr][_id][2] = classes[_archetype]; // Class
    aov[_addr][_id][3] = _profession; // Profession

    emit CreatedAoVProfession(_addr, _id, _archetype, _profession);
  }

  // For AoV With profession and points
  function createFor(
    address _addr,
    uint256 _id,
    uint256 _archetype,
    ...
    emit CreatedAoVPoints(_addr, _id, _archetype, _profession, _points);
  }
  ...
  function initData(
    ...
    // Check if profession has been set before
    require(aov[_addr][_id][3] == 0, "AdventurerData: Already set profession");

    aov[_addr][_id][3] = _profession; // Profession

    _setDefaults(_addr, _id, _points);

    emit InitiatedData(_addr, _id, _profession, _points);
  }

  // TODO:
  // function batchProfession(
  ...
  //     aov[addr][id][3] = profession; // Profession

  //     base[addr][id][2] = 5; // Strength
  //     base[addr][id][3] = 5; // Dexterity
  //     base[addr][id][4] = 5; // Constitution
  //     base[addr][id][5] = 5; // Intelligence
  //     base[addr][id][6] = 5; // Wisdom
  //     base[addr][id][7] = 5; // Charisma

  //     emit BatchedProfession(addr, id, profession);
  //   }
  // }
  ...
  function moveFor(
    ...
    for (uint256 j = 0; j < baseCount; j++) {
      base[_addr][_id][j] = base[_existingAddr][_existingId][j];
      base[_existingAddr][_existingId][j] = 0;
    }

    // Move Aov
    for (uint256 j = 0; j < aovCount; j++) {
      aov[_addr][_id][j] = aov[_existingAddr][_existingId][j];
      aov[_existingAddr][_existingId][j] = 0;
    }

    // Move Extension
    for (uint256 j = 0; j < extensionCount; j++) {
      extension[_addr][_id][j] = extension[_existingAddr][_existingId][j];
      extension[_existingAddr][_existingId][j] = 0;
    }

    emit Moved(_existingAddr, _existingId, _addr, _id);
  }
