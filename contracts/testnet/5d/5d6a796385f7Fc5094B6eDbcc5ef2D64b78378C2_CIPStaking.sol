...
contract CIPStaking is Ownable{
    ...
    function getSystemSetting() public view returns (uint256,uint256,uint256,uint256,uint256,uint256,uint256) {  
        return (MIN_STAKE_AMOUNT,MIN_WITHDRAWAL_AMOUNT,ROI_PERCENT_ANNUALY,ROI_PERCENT_PERDAY,ROI_PERCENT_HOUR,ROI_PERCENT_MINUTE,ROI_PERCENT_SECOND);
    }

    //Level income get data:-
    function getLevelSetting() public view returns (uint[20] memory, uint[20] memory, uint[20] memory) {  
        return (ref_bonuses,ref_bonuses_direct_require,self_business_required);
    }

    //Rank income get data:-
    function getRankSetting() public view returns (uint[4] memory, uint[4] memory, uint[4] memory,uint[4] memory,uint[4] memory) { 
      return (rank_venus,rank_earth,rank_mars,rank_jupiter,rank_sun);
    }

    //universal income get data:-
    function getUniversalSetting() public view returns (uint[5] memory, uint[5] memory, uint[5] memory,uint[5] memory) { 
      return (up_gold,up_diamond,up_platinum,up_kohinoor);
    }

    //Update System Settings OnlyOwner:-
    function systemSetting(uint256 _MIN_STAKE_AMOUNT,uint256 _MIN_WITHDRAWAL_AMOUNT,uint _ROI_PERCENT_ANNUALY) onlyOwner public {
