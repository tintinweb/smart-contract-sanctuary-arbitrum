...
contract BoinkStaking is IBoinkStaking, ReentrancyGuard, Ownable {
    ...
    function depositNFT(uint256[] calldata _tokenIds) external nonReentrant override {
        // require(balanceOf[_msgSender()] > 0, "Must stake tokens first");
    ...
    function claimReward() public nonReentrant override {
        // Update last claimed amount to current total ETH received
        lastClaimedETHAmount[_msgSender()] = totalETHReceived;

        // Then calculate the reward based on this updated state
        uint256 ethReward = calculateReward(_msgSender());

        if (ethReward > 0) {
            payable(_msgSender()).transfer(ethReward);
            emit RewardClaimed(_msgSender(), ethReward);
        }
    }
    ...
    function stake(uint256 _amount) external nonReentrant override {
        ...
        require(balanceOf[_msgSender()] + _amount <= stakingToken.getMaxWallet(), "Exceeds maxWallet limit");
