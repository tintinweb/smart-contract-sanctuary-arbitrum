...
contract LaunchPadPublic is Ownable {
    ...
    uint256 tokenAmount;
    uint256 maxStableCoinAmountPerUser;
    ...
    function setTokenParams(
        ...
        uint256 _maxStableCoinAmountPerUser,
        ...
        maxStableCoinAmountPerUser = _maxStableCoinAmountPerUser;
    ...
    function buyTokens(uint256 _paymentAmount) public {
        ...
        require(
            ...
            "Token sale: your balance is not enough to buy tokens."
        );

        require(
            paymentToken.allowance(_msgSender(), address(this)) >=
                _paymentAmount,
            "Token sale: invalid allowance to buy Token sale tokens."
        );
        ...
        require(
            paymentAmount <= maxStableCoinAmountPerUser,
            "Token sale: buy exceed the allowed amount."
        );
    ...
    function vestedAmount(
        ...
        require(
            info.amount > 0,
            "Token sale: The beneficiary's address cannot be found"
        );
        ...
        uint256 multiplyInterval;
        if (block.timestamp >= tgeDate) {
            multiplyInterval = (block.timestamp - tgeDate) / vestInterval;
        }
        uint256 maxInterval = vestDuration / vestInterval;
        if (multiplyInterval > maxInterval) {
            multiplyInterval = maxInterval;
        }

        if (maxInterval > 0) {
            uint256 newVestedAmount = info
                .leftOverAmount
                .mul(multiplyInterval)
                .div(maxInterval);
