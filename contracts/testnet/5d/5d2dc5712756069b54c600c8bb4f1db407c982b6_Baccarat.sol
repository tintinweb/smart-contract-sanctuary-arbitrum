...
contract Baccarat is CommonSoloBaccarat, InternalRNG {
  ...
  function _puntoModulo(uint256 _total) internal pure returns (uint256) {
    return _total % 10;
  }
  ...
  function _puntoModulo64(uint64 _total) internal pure returns (uint64) {
    return _total % 10;
  }
  ...
  function calculatePlayerTotal(BaccaratGame memory _game) public pure returns (uint64 total_) {
    if (_game.playerHand.hasThirdCard) {
      total_ = _puntoModulo64(
        _game.playerHand.firstCard + _game.playerHand.secondCard + _game.playerHand.thirdCard
      );
    } else {
      total_ = _puntoModulo64(_game.playerHand.firstCard + _game.playerHand.secondCard);
    }
  }

  function calculateBankerTotal(BaccaratGame memory _game) public pure returns (uint64 total_) {
    if (_game.bankerHand.hasThirdCard) {
      total_ = _puntoModulo64(
        _game.bankerHand.firstCard + _game.bankerHand.secondCard + _game.bankerHand.thirdCard
      );
    } else {
      total_ = _puntoModulo64(_game.bankerHand.firstCard + _game.bankerHand.secondCard);
    }
  }
  ...
  function whoIsWinner(uint256 _requestId) public view returns (BaccaratResult result_) {
    ...
    uint64 playerTotal_ = calculatePlayerTotal(game_);
    uint64 bankTotal_ = calculateBankerTotal(game_);
    return _checkWinner64(playerTotal_, bankTotal_);
  }

  function getPlayerPunto(uint256 _requestId) public view returns (uint64) {
    BaccaratGame memory game_ = games_[_requestId];
    return calculatePlayerTotal(game_);
  }

  function getBankerPunto(uint256 _requestId) public view returns (uint64) {
    BaccaratGame memory game_ = games_[_requestId];
    return calculateBankerTotal(game_);
  }
   ...
   */
  function _checkWinner64(
    uint64 _playerTotal,
    uint64 _bankTotal
  ) internal pure returns (BaccaratResult result_) {
    // both bank and player have 8 or 9
  ...
  function _checkImmediateWinner(
    ...
    if (_playerTotal > 7 || _bankTotal > 7) {
      // both bank or player have 8 or 9
      if (_playerTotal == _bankTotal) {
        return BaccaratResult.DRAW;
      } else if (_playerTotal > _bankTotal) {
        return BaccaratResult.PLAYER_WINS;
  ...
  // /**
  //  * @param result_ baccarat result struct
  //  * @param _tieWins amount bet on the game tie
  //  * @param _bankWins amount of bet on the banker win
  //  * @param _playerWins amount of bet on the player win
  //  * @return payout_ amount of payout for the game
  //  */
  ...
  // /**
  //  * @notice baccarat play function
  //  * @param _tieWins amount bet on tie
  //  * @param _bankWins amount bet on banker win
  //  * @param _playerWins amount bet on player win
  //  * @param _random vrf random value
  //  */
  function play(
    uint256 _requestId,
    Bet memory _bet,
    ...
    BothHands memory hands_ = BothHands(
      Hand(false, uint8(topCards_[0]), uint8(topCards_[2]), uint8(topCards_[4])), // player hand
      Hand(false, uint8(topCards_[1]), uint8(topCards_[3]), uint8(topCards_[5])) // banker hand
    );
    ...
    if (
      ...
      // payout_ = _calculateWinnings(
      //   _checkWinner(_puntoModulo(playerCount_), _puntoModulo(bankCount_)),
      //   _bet
      // );
      payout_ = _calculateWinnings(_checkWinner(playerCount_ % 10, bankCount_ % 10), _bet);
      emit HandFinalized(_requestId, hands_);
    ...
    if (hands_.playerHand.hasThirdCard && !hands_.bankerHand.hasThirdCard) {
      ...
      bool thirdCard_ = _doesBankerGetThirdCard(topCards_[4], bankCount_ % 10);
      if (thirdCard_) {
   ...
   * @param _tieWins amount wagered on tie
   * @param _bankWins amount wagered on banker win
   * @param _playerWins amount wagered on player win
...
abstract contract CommonSoloBaccarat is Core, Test {
  ...
  event HandFinalized(uint256 requestId, BothHands hands);
  ...
  function _create(Bet memory _bet, address _token) internal whenNotPaused nonReentrant {
    // isWagerAcceptable(_token, _totalWager) uncommented!
