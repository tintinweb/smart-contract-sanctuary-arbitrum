...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///
    ///   pending reward = (user share * pool.accCyberzPerShare) - user.rewardDebt
    ///
    ...
    ///     CYBERZ rewards each pool gets.
    /// `accCyberzPerShare` Accumulated CYBERZs per share, times 1e12.
    ...
    ///     the CYBERZ rewards to all the cyberzfi products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accCyberzPerShare;
...
/// @notice Address of CYBERZ contract.
IBEP20 public immutable CYBERZ;

/// @notice The only address can withdraw all the burn CYBERZ.
...
///  @notice 0.4 cyberzs per block
uint256 public constant MASTERCHEF_CYBERZ_PER_BLOCK = 4 * 1e17;
uint256 public constant ACC_CYBERZ_PRECISION = 1e18;

/// @notice Basic boost factor, none boosted user's boost factor
uint256 public constant BOOST_PRECISION = 100 * 1e10;
/// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
/// @notice total cyberz rate = toBurn + toRegular + toSpecial
uint256 public constant CYBERZ_RATE_TOTAL_PRECISION = 1e12;
/// @notice The last block number of CYBERZ burn action being executed.
/// @notice CYBERZ distribute % for burn
uint256 public cyberzRateToBurn = 0;
/// @notice CYBERZ distribute % for regular farm pool
uint256 public cyberzRateToRegularFarm = 1e12;
/// @notice CYBERZ distribute % for special pools
uint256 public cyberzRateToSpecialFarm = 0;
...
uint256 public totalcyberzStaked;

// event Init();
event AddPool(uint256 indexed pid, uint256 allocPoint, IBEP20 indexed lpToken, bool isRegular);
event SetPool(uint256 indexed pid, uint256 allocPoint);
event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accCyberzPerShare);
event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

event UpdateCyberzRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
...
/// @param _CYBERZ The CYBERZ token contract address.
...
/// only for CYBERZ distributions within CyberzFi products.
function add(
uint256 _allocPoint,
IBEP20 _lpToken,
bool _isRegular,
...
// stake CYBERZ token will cause staked token and reward token mixed up,
// may cause staked tokens withdraw as reward token,never do it.
// require(_lpToken != CYBERZ, "CYBERZ token can't be added to farm pools");
...
/// @notice Update the given pool's CYBERZ allocation point. Can only be called by the owner.
...
/// @notice View function for checking pending CYBERZ rewards.
/// @param _pid The id of the pool. See `poolInfo`.
...
uint256 cyberzReward = multiplier.mul(cyberzPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
(pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
);
accCyberzPerShare = accCyberzPerShare.add(cyberzReward.mul(ACC_CYBERZ_PRECISION).div(lpSupply));
}

uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
return boostedAmount.mul(accCyberzPerShare).div(ACC_CYBERZ_PRECISION).sub(user.rewardDebt);
}

/// @notice Update cyberz reward for all the active pools. Be careful of gas spending!
...
/// @notice Calculates and returns the `amount` of CYBERZ per block.
/// @param _isRegular If the pool belongs to regular or special.
function cyberzPerBlock(bool _isRegular) public view returns (uint256 amount) {
if (_isRegular) {
amount = MASTERCHEF_CYBERZ_PER_BLOCK.mul(cyberzRateToRegularFarm).div(CYBERZ_RATE_TOTAL_PRECISION);
} else {
amount = MASTERCHEF_CYBERZ_PER_BLOCK.mul(cyberzRateToSpecialFarm).div(CYBERZ_RATE_TOTAL_PRECISION);
}
}

/// @notice Calculates and returns the `amount` of CYBERZ per block to burn.
function cyberzPerBlockToBurn() public view returns (uint256 amount) {
amount = MASTERCHEF_CYBERZ_PER_BLOCK.mul(cyberzRateToBurn).div(CYBERZ_RATE_TOTAL_PRECISION);
}
...
pool.accCyberzPerShare = pool.accCyberzPerShare.add((cyberzReward.mul(ACC_CYBERZ_PRECISION).div(lpSupply)));

// mint cyberzs for devAdd
CYBERZ.mint(devaddr, cyberzReward.div(10));
// mint cyberzs for pool
CYBERZ.mint(address(this), cyberzReward);
}

pool.lastRewardBlock = block.number;
poolInfo[_pid] = pool;
emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accCyberzPerShare);
}
}
...
if (user.amount > 0) {
settlePendingCyberz(msg.sender, _pid, multiplier);
}
...
if (_pid == 0) {
totalcyberzStaked = totalcyberzStaked + _amount;
}
...
user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accCyberzPerShare).div(
ACC_CYBERZ_PRECISION
);
poolInfo[_pid] = pool;
...
// /// @notice Harvests CYBERZ from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
...
if (_pid == 0) {
totalcyberzStaked = totalcyberzStaked - amount;
}
...
/// @notice Send CYBERZ pending for burn to `burnAdmin`.
/// @param _withUpdate Whether call "massUpdatePools" operation.
function burnCyberz(bool _withUpdate) public onlyOwner {
if (_withUpdate) {
massUpdatePools();
}

uint256 multiplier = block.number.sub(lastBurnedBlock);
uint256 pendingCyberzToBurn = multiplier.mul(cyberzPerBlockToBurn());

// SafeTransfer CYBERZ
_safeTransfer(burnAdmin, pendingCyberzToBurn);
lastBurnedBlock = block.number;
}

/// @notice Update the % of CYBERZ distributions for burn, regular pools and special pools.
/// @param _burnRate The % of CYBERZ to burn each block.
/// @param _regularFarmRate The % of CYBERZ to regular pools each block.
/// @param _specialFarmRate The % of CYBERZ to special pools each block.
/// @param _withUpdate Whether call "massUpdatePools" operation.
function updateCyberzRate(
uint256 _burnRate,
...
"MasterChefV2: Cyberz rate must be greater than 0"
);
require(
_burnRate.add(_regularFarmRate).add(_specialFarmRate) == CYBERZ_RATE_TOTAL_PRECISION,
"MasterChefV2: Total rate must be 1e12"
);
if (_withUpdate) {
massUpdatePools();
}
// burn cyberz base on old burn cyberz rate
burnCyberz(false);

cyberzRateToBurn = _burnRate;
cyberzRateToRegularFarm = _regularFarmRate;
cyberzRateToSpecialFarm = _specialFarmRate;

emit UpdateCyberzRate(_burnRate, _regularFarmRate, _specialFarmRate);
}
...
settlePendingCyberz(_user, _pid, prevMultiplier);

user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accCyberzPerShare).div(
ACC_CYBERZ_PRECISION
);
...
/// @notice Settles, distribute the pending CYBERZ rewards for given user.
/// @param _user The user address for settling rewards.
/// @param _pid The pool id.
/// @param _boostMultiplier The user boost multiplier in specific pool id.
function settlePendingCyberz(
address _user,
uint256 _pid,
...
uint256 accCyberz = boostedAmount.mul(poolInfo[_pid].accCyberzPerShare).div(ACC_CYBERZ_PRECISION);
uint256 pending = accCyberz.sub(user.rewardDebt);
// SafeTransfer CYBERZ
_safeTransfer(_user, pending);
}

/// @notice Safe Transfer CYBERZ.
/// @param _to The CYBERZ receiver address.
/// @param _amount transfer CYBERZ amounts.
function _safeTransfer(address _to, uint256 _amount) internal {
if (_amount > 0) {
uint256 balance = CYBERZ.balanceOf(address(this)) - totalcyberzStaked;
if (balance < _amount) {
_amount = balance;
}

CYBERZ.safeTransfer(_to, _amount);
}
}
}