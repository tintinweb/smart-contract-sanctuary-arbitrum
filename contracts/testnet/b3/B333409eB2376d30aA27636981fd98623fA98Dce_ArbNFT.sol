...
contract ArbNFT is ERC721, Ownable {
    ...
    string private bgEyes='"/><path fill-rule="evenodd" clip-rule="evenodd" d="M9 7H10H11V8V9H10H9V8V7ZM13 9V8V7H14H15V8V9H14H13Z" fill="white"/>';
    ...
    function setZFrame(bytes memory encodec) public onlyOwner {
        z = abi.decode(encodec, (string[]));
    }

    function setColorFrame(bytes memory encodec) public onlyOwner {
    ...
    function genEyes(uint256 h) internal view returns(string memory) {
        string memory eye = '';
        if(h%4 == 0) {
            eye = string(abi.encodePacked(eye, bgEyes, cEyes, "M10 7H9V8H10V7ZM14 7H13V8H14V7Z", eEyes));
        }
        else if(h%4 == 1) {
            eye = string(abi.encodePacked(eye, bgEyes, cEyes, "M11 7H10V8H11V7ZM15 7H14V8H15V7Z", eEyes));
        }
        else if(h%4 == 2) {
            eye = string(abi.encodePacked(eye, bgEyes, cEyes, "M11 8H10V9H11V8ZM15 8H14V9H15V8Z", eEyes));
        }
        else {
            eye = string(abi.encodePacked(eye, bgEyes, cEyes, "M10 8H9V9H10V8ZM14 8H13V9H14V8Z", eEyes));
        }
        return eye;
    }
    ...
    function genImgSVG(uint256 tokenId) public view returns (string memory) {
        ...
        output = string(abi.encodePacked(output, z[6], z[7], z[8], z[9], z[10]));
        output = string(abi.encodePacked(output, z[11], color[arb.hatBg]));
        output = string(abi.encodePacked(output, z[12], color[arb.hatLine], genEyes(random(string(abi.encodePacked(re1,tokenId.toString())))), color[arb.eyes]));
        output = string(abi.encodePacked(output, z[13], color[arb.clothes], z[14], color[arb.logoClothes]));
        output = string(abi.encodePacked(output, z[15], color[arb.trousers], z[16], z[17]));
        return output;
    }
    ...
    function setWhiteListUser(address user) public onlyOwner {
    ...
    function mint() public {
        ...
        require(isWhiteList[msg.sender], "You aren't allow mint");
        _safeMint(_msgSender(), numClaimed);
        numClaimed += 1;
        isWhiteList[msg.sender] = false;
        emit Mint(msg.sender, numClaimed);
    }

    function ownerMint(uint256 tokenId) public onlyOwner {
        require(tokenId > 95000 && tokenId < 100001, "invalid claim");
