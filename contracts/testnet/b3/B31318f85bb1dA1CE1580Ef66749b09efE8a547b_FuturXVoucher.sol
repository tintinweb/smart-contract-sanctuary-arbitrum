...
contract FuturXVoucher is ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    modifier onlyHandler() {
        require(msg.sender == getHandler(), "Handler: 403");
        _;
    }
    ...
    function setHandler(address handler) external onlyOwner {
        StorageSlot.getAddressSlot(bytes32("futurX.voucher.handler")).value = handler;
    }
    ...
    function batchMint(ClaimRequest[] memory requests) external onlyHandler {
        for (uint256 i = 0; i < requests.length; i++) {
            ClaimRequest memory request = requests[i];
            _claim(
                request.voucherId,
    ...
    function getHandler() public view returns (address) {
        return StorageSlot.getAddressSlot(bytes32("futurX.voucher.handler")).value;
    }
