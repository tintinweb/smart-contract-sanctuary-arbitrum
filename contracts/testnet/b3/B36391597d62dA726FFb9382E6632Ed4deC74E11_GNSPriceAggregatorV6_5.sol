...
contract GNSPriceAggregatorV6_5 is ChainlinkClient, TWAPPriceGetter {
    ...
    struct ChainlinkFeed {
        ChainlinkFeedInterfaceV5 priceFeed;
        uint8 decimals; // 2k gas cheaper to read from here
    }
    ...
    constructor(
        ...
        ChainlinkFeedInterfaceV5 _collateralPriceFeed,
        uint _minAnswers,
        ...
        storageT = _storageT;
        collateralPrecision = 10 ** _storageT.dai().decimals();
    ...
    function updateCollateralPriceFeed(ChainlinkFeedInterfaceV5 value) external onlyGov {
        require(address(value) != address(0), "VALUE_0");

        collateralPriceFeed = value;
    ...
    function linkFee(uint pairIndex, uint leveragedPosDai) public view returns (uint) {
        ...
        (, int collateralPriceUsd, , , ) = collateralPriceFeed.latestRoundData();

        // NOTE: all [token / USD] feeds are 8 decimals
        return
            (pairsStorage.pairOracleFeeP(pairIndex) * leveragedPosDai * uint(collateralPriceUsd)) /
            uint(linkPriceUsd) /
            PRECISION /
            100;
    }
