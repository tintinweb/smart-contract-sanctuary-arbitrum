...
contract Tournament is AccessControl, ReentrancyGuard {
    ...
    function createRedeemRequest(
        ...
        address[] memory listTicketCollectionAddress = new address[](rewardClaimTickets.length);
        ...
        for(uint256 i =0 ; i< rewardClaimTickets.length; i++) {
            ...
            listTicketCollectionAddress[i] = rewardClaimTickets[i].ticketCollectionAddress;
        } 

        emit CreateRedeemRequestEvent(requestId, listTicketCollectionAddress);
    ...
    function withdrawTicket(TicketQuantity[] memory withdrawTickets) public {
        ...
        for(uint256 i = 0; i < dpt.length; i++) {
            for(uint256 j =0 ; j < withdrawTickets.length ; j ++) {
                if(dpt[i].ticketCollectionAddress == withdrawTickets[j].ticketCollectionAddress) {
                    for(uint256 x =0 ; x < withdrawTickets[j].quantity; x++) {
                        IERC721(dpt[i].ticketCollectionAddress).safeTransferFrom(address(this), msg.sender, dpt[i].tokenIds[i]);
                        for (uint y = 0; y < dpt[i].tokenIds.length - 1; y++) {
                            dpt[i].tokenIds[y] = dpt[i].tokenIds[y + 1];
                        }
                        // Rút ngắn mảng đi một phần tử
                        dpt[i].tokenIds.pop();
                    }
                }
            }
    ...
    function depositTicket(
        ...
        for (uint256 i =0 ; i < depositTicketDetail.length; i++) {
            DepositTicketDetail[] storage dpt = depositTicketOfUser[msg.sender];
            for (uint256 j =0 ; j < dpt.length ; j ++) {
                dpt[j].ticketCollectionAddress = depositTicketDetail[i].ticketCollectionAddress;
                for (uint256 x =0 ; x <  depositTicketDetail[i].tokenIds.length; x ++) {
                    dpt[i].tokenIds.push(depositTicketDetail[i].tokenIds[x]);
                }
            }
        }
    }
    ...
    function useTicket(
       ...
       // check tournament co phu hop voi tichket collection khong.
       for(uint256 i =0 ; i< dpt.length ; i ++) {
        if(dpt[i].ticketCollectionAddress == ticketCollectionAddress) {
            NFT721Ticket(ticketCollectionAddress).burn(dpt[i].tokenIds[0]);
            // Di chuyển toàn bộ phần tử sang trái một vị trí
            for (uint j = 0; j < dpt[i].tokenIds.length - 1; j++) {
                dpt[i].tokenIds[j] = dpt[i].tokenIds[j + 1];
            }
            // Rút ngắn mảng đi một phần tử
            dpt[i].tokenIds.pop();
    ...
    function getNumberOfTicket(
        ...
        DepositTicketDetail[] memory dpt = depositTicketOfUser[user];
        for(uint256 i =0 ; i < dpt.length ; i++) {
            if(dpt[i].ticketCollectionAddress == ticketCollectionAddress) {
               return dpt[i].tokenIds.length;
            }
        }
    ...
    function getListAddressOfCollection(
        ...
        //     ticketCollection.listTicketOwners.length
        // );
        // uint256[] memory numberOfTickets = new uint256[](
        //     ticketCollection.listTicketOwners.length
        // );

        // for (uint256 i = 0; i < ticketCollection.listTicketOwners.length; i++) {
        //     address owner = ticketCollection.listTicketOwners[i];
