    ...
    function getEstimatedFee(
        ...
        for (uint256 i; i < priorityProtocolIds.length; i++) {
            ...
            if (priorityProtocolIds[i] == 2) {
                fee = LibGasEstimation.lzEstimatedFee(sendPayload, protocol);
                if(fee != 0) break;
                else continue;
            } 
            if (priorityProtocolIds[i] == 3) {
                fee = LibGasEstimation.ccipEstimatedFee(sendPayload, protocol);
                if(fee != 0) break;
                else continue;
            }
            if (priorityProtocolIds[i] == 4) {
                fee = LibGasEstimation.wormholeEstimatedFee(sendPayload, protocol);
                if(fee != 0) break;
                else continue;
            }
        }

        if(fee == 0) fee = 0.05 ether;
...
library LibGasEstimation {
    ...
    */
    function lzEstimatedFee(
        ...
        (nativeFee, ) = ILayerZeroEndpoint(LibInternalGetters.bytesToAddress(protocol.endPoint)).estimateFees(
            uint16(destChainProtocol.protocolChainId),
            address(this),
            abi.encode(sendPayload),
            false,
    ...
    */
    function ccipEstimatedFee(
        ...
        fee = IRouterClient(LibInternalGetters.bytesToAddress(protocol.endPoint)).getFee(
            uint64(destChainProtocol.protocolChainId),
            message2Send
        );
    }
    ...
    */
    function wormholeEstimatedFee(
        ...
        (fee, ) = IWormholeRelayer(LibInternalGetters.bytesToAddress(protocol.endPoint))
            .quoteEVMDeliveryPrice(uint16(destChainProtocol.protocolChainId), 0, 300000);
    }
}
