...
contract BGT_UFM {
    ...
    event buy(address indexed sender, uint256 indexed fp_id, address indexed token_contract, uint256 orderId, uint256 amount);
    event update_fp(uint256 indexed fp_id, address indexed token_contract, uint256 fp_quantity, uint256 fp_quantity_sold);
    event order_expired(uint256 indexed orderId, address indexed sender, address indexed token_contract, uint256 amount);

    constructor (address bgt) {
        _owner = msg.sender;
        _bgt = IERC20(bgt);
    }
    ...
    function registeredFp(uint256 __fp_id, uint256 __fp_quantity) external onlyAdmin {
        require(fp_token_contract[__fp_id] == address(0), "Financial package has been registered");

        address __token_contract = address(_bgt);
    ...
    function create(uint256 __fp_id, uint256 __amount) external {
        ...
        TransferHelper.safeTransferFrom(fp_token_contract[__fp_id], msg.sender, address(this), __amount);
        _bgt.approve(address(_staking), __amount);
        _staking.deposit(__amount);

        _autoOrderIds++;
        _orders[_autoOrderIds] = Order(_autoOrderIds, __amount, msg.sender, block.timestamp, 0);

        emit buy(msg.sender, __fp_id, fp_token_contract[__fp_id], _autoOrderIds, __amount);
        emit update_fp(__fp_id, fp_token_contract[__fp_id], fp_quantity[__fp_id], fp_quantity_sold[__fp_id]);
    }

    function expired(uint256 __order_id, uint256 __income) external onlyAdmin {
        Order storage _order = _orders[__order_id];
        require(_order.status == 0, "order error");
        _staking.deposit(_order.amount);
        _bgt.transfer(_order.sender,  _order.amount);
        _bgt.transferFrom(msg.sender, _order.sender,  __income);
        _order.status = 1;
        emit order_expired(__order_id, _order.sender, address(_bgt), _order.amount);
    }

    function claimInterest() external onlyAdmin {
