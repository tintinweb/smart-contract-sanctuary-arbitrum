...
contract RedeemRouter {
    ...
    function redeem(
        ...
        (uint256 reserveBefore, uint32[] memory subIndexBurnBalances) =
            IIndexClient(index).redeem(indexShares, msg.sender, owner, receiver, subIndexes);

        if (subIndexBurnBalances.length != 0) {
            if (reserveCached == reserveBefore) {
                _burnSubIndexes(receiver, subIndexBurnBalances, subIndexes, redeemData.localData, redeemData.remoteData);
            } else {
    ...
    function _burnSubIndexes(
        address receiver,
        uint32[] memory subIndexBurnBalances,
        SubIndexLib.SubIndex[] calldata subIndexes,
        ...
        for (uint256 i; i < subIndexes.length;) {
            if (subIndexes[i].chainId == block.chainid) {
                _validate(subIndexBurnBalances[i], subIndexes[i], localQuotes[localIndex].quotes);
        ...
        if (batches.length != 0) {
            _handleRemoteSwap(messageRouter, receiver, batches);
        }
    }
    ...
    function _handleRemoteSwap(address messageRouter, address receiver, Batch[] memory batches) internal {
