...
contract Sleuth {
    ...
    function query(bytes calldata q) external payable returns (bytes memory) {
        return queryInternal(q, abi.encodeWithSignature("query()"));
    }

    function query(bytes calldata q, bytes memory c) external payable returns (bytes memory) {
