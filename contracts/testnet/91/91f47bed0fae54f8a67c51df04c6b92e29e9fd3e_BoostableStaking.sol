...
import {
  BoostableRewards,
  Reward,
  RewardSystemReadable
} from "./model/BoostableStakingModel.sol";
...
contract BoostableStaking is BaseVesta, TokenTransferrer, IBoostableStaking {
  bytes1 public constant GAUGE_MANAGER = 0x01;
  uint256 public constant PERCENTAGE_DENOMINATOR = 100;
  uint256 public constant BOOST_DECIMALS = 1e18;

  address public depositToken;
  IGaugesManager public gaugesManager;

  uint256 public rewardsDuration;

  uint256 public totalStaked;
  uint256 public totalBoostWeight;
  uint256 public override totalBoosts;

  mapping(address => uint256) private userDepositBalances;
  mapping(address => uint256) private userBoosts;
  mapping(address => uint256) private userBoostWeights;

  mapping(address => BoostableRewards) private rewardSystems;
  ...
  function setUp(
    ...
    address _gaugesManager
  ) external initializer {
    __BASE_VESTA_INIT();

    _addRewardSystem(_rewardToken, PERCENTAGE_DENOMINATOR);

    depositToken = _depositToken;
    rewardsDuration = _rewardsDuration;

    gaugesManager = IGaugesManager(_gaugesManager);
    _setPermission(_gaugesManager, GAUGE_MANAGER);
    _transferOwnership(_owner);
  }

  function _addRewardSystem(address _token, uint256 _boostedWeightPercentage) private {
    if (_boostedWeightPercentage > PERCENTAGE_DENOMINATOR) {
      revert BoostWeightPercentagesMustAddTo100();
    }

    BoostableRewards storage system = rewardSystems[_token];
    system.boostedWeightPercentage = _boostedWeightPercentage;
    system.baseWeightPercentage = PERCENTAGE_DENOMINATOR - _boostedWeightPercentage;

    isRewardToken[_token] = true;
    rewardTokens.push(_token);
  }

  function _updateRewards(address _account, bool _claim) internal {
    uint256 length = rewardTokens.length;

    address rewardToken;
    BoostableRewards storage system;

    for (uint256 i = 0; i < length;) {
      rewardToken = rewardTokens[i];
      system = rewardSystems[rewardToken];

      uint256 stakingRewardPerToken = _rewardPerToken(true, system);
      uint256 boostingRewardPerToken = _rewardPerToken(false, system);

      system.stakingReward.rewardPerTokenStored = stakingRewardPerToken;
      system.boostingReward.rewardPerTokenStored = boostingRewardPerToken;

      system.lastUpdate = getLastTimeRewardApplicable(system.endPeriod);

      if (_account != address(0)) {
        uint256 newRewardAmount = _earned(_account, system);
        system.userRewardPerTokenPaid[_account] = stakingRewardPerToken;
        system.userRewardPerBoost[_account] = boostingRewardPerToken;

        if (_claim && newRewardAmount != 0) {
          system.rewards[_account] = 0;
          _performTokenTransfer(rewardToken, _account, newRewardAmount);

          emit RewardPaid(_account, newRewardAmount);
        } else {
          system.rewards[_account] = newRewardAmount;
        }
      }
  ...
  function earned(address _token, address _account)
    ...
    returns (uint256)
  {
    return _earned(_account, rewardSystems[_token]);
  }
  ...
  {
    tokens_ = rewardTokens;

    uint256 length = tokens_.length;
    rewards_ = new uint256[](length);

    for (uint256 i = 0; i < length;) {
      rewards_[i] = _earned(_account, rewardSystems[tokens_[i]]);
    ...
    return (tokens_, rewards_);
  }

  function _earned(address _account, BoostableRewards storage system)
    private
    view
    returns (uint256)
  {
    uint256 userPerToken =
      _rewardPerToken(true, system) - system.userRewardPerTokenPaid[_account];

    uint256 userPerBoost =
      _rewardPerToken(false, system) - system.userRewardPerBoost[_account];

    uint256 rewardFromStaking =
      Math.mulDiv(userDepositBalances[_account], userPerToken, Math.RAY);
    uint256 rewardFromBoosting =
      Math.mulDiv(userBoostWeights[_account], userPerBoost, Math.RAY);

    return rewardFromStaking + rewardFromBoosting + system.rewards[_account];
  }

  function stake(address _user, uint256 _stakingAmount, uint256 _setBoost)
    external
    override
    hasPermission(GAUGE_MANAGER)
  {
    _updateRewards(_user, false);

    userDepositBalances[_user] += _stakingAmount;
    totalStaked += _stakingAmount;

    _performTokenTransferFrom(depositToken, _user, address(this), _stakingAmount);
    _setUserBoost(_user, _setBoost);

    emit Stake(_user, _stakingAmount);
  }

  function withdrawViaGaugeManager(address _user, uint256 _amount)
    external
    override
    hasPermission(GAUGE_MANAGER)
  {
  ...
  function _withdraw(address _user, uint256 _amount) private {
    ...
    uint256 depositBalance = userDepositBalances[_user];

    if (_amount >= depositBalance) revert UseExitFunction();

    depositBalance -= _amount;

    userDepositBalances[_user] = depositBalance;
    totalStaked -= _amount;
    _setUserBoost(_user, userBoosts[_user]);
  ...
  function setUserBoost(address _user, uint256 _boostValue)
    external
    override
    hasPermission(GAUGE_MANAGER)
  {
    _updateRewards(_user, false);
    _setUserBoost(_user, _boostValue);
  }

  function _setUserBoost(address _user, uint256 _boostValue) private {
    uint256 prevBoost = userBoosts[_user];

    totalBoosts = totalBoosts - prevBoost + _boostValue;
    userBoosts[_user] = _boostValue;

    uint256 lastUserBoostWeight = userBoostWeights[_user];
    uint256 newUserBoostWeight =
      Math.mulDivRoundingUp(userDepositBalances[_user], _boostValue, BOOST_DECIMALS);

    userBoostWeights[_user] = newUserBoostWeight;
    totalBoostWeight = totalBoostWeight - lastUserBoostWeight + newUserBoostWeight;

    emit AdjustBoost(_user, _boostValue);
  }

  function exit(address _user) external override hasPermission(GAUGE_MANAGER) {
    _updateRewards(_user, true);

    uint256 depositBalance = userDepositBalances[_user];
    uint256 boostBalance = userBoosts[_user];
    uint256 boostWeight = userBoostWeights[_user];

    if (depositBalance == 0) revert StakeBalanceEmpty();

    userDepositBalances[_user] = 0;
    totalStaked -= depositBalance;

    userBoosts[_user] = 0;
    totalBoosts -= boostBalance;

    userBoostWeights[_user] = 0;
    totalBoostWeight -= boostWeight;

    _performTokenTransfer(depositToken, _user, depositBalance);

    emit Withdrawn(_user, depositBalance);
  }
  ...
  function notifyRewardAmount(address _rewardToken, uint128 _reward)
    external
    override
    hasPermissionOrOwner(GAUGE_MANAGER)
  {
    _updateRewards(address(0), false);

    if (!isRewardToken[_rewardToken]) {
      _addRewardSystem(_rewardToken, PERCENTAGE_DENOMINATOR);
    }

    BoostableRewards storage system = rewardSystems[_rewardToken];
    Reward storage stakingRewardCache = system.stakingReward;
    Reward storage boostRewardCache = system.boostingReward;

    uint128 cachedEndPeriod = system.endPeriod;

    uint256 rewardToStaking =
      Math.mulDiv(_reward, system.baseWeightPercentage, PERCENTAGE_DENOMINATOR);
    uint256 rewardToBoosting = _reward - rewardToStaking;

    stakingRewardCache.rewardPerTokenStored = _rewardPerToken(true, system);
    boostRewardCache.rewardPerTokenStored = _rewardPerToken(false, system);

    if (block.timestamp >= cachedEndPeriod) {
      stakingRewardCache.rewardRatePerSecond =
        Math.mulDiv(rewardToStaking, Math.RAY, rewardsDuration);

      boostRewardCache.rewardRatePerSecond =
        Math.mulDiv(rewardToBoosting, Math.RAY, rewardsDuration);
    } else {
      uint256 remainingTime = cachedEndPeriod - block.timestamp;

      uint256 leftoverStaking =
        Math.mulDiv(remainingTime, stakingRewardCache.rewardRatePerSecond, Math.RAY);
      uint256 leftoverBoosting =
        Math.mulDiv(remainingTime, boostRewardCache.rewardRatePerSecond, Math.RAY);

      stakingRewardCache.rewardRatePerSecond =
        Math.mulDiv((rewardToStaking + leftoverStaking), Math.RAY, rewardsDuration);
      boostRewardCache.rewardRatePerSecond =
        Math.mulDiv((rewardToBoosting + leftoverBoosting), Math.RAY, rewardsDuration);
    }

    _performTokenTransferFrom(_rewardToken, msg.sender, address(this), _reward);
    emit RewardAdded(_rewardToken, _reward);

    system.lastUpdate = uint128(block.timestamp);
    system.endPeriod = uint128(block.timestamp + rewardsDuration);
  }

  function setBoostWeights(address _token, uint256 _boostWeightPercentage)
    external
    onlyOwner
  {
    if (_boostWeightPercentage > PERCENTAGE_DENOMINATOR) {
      revert BoostWeightPercentagesMustAddTo100();
    }

    if (!isRewardToken[_token]) {
      _addRewardSystem(_token, _boostWeightPercentage);
      return;
    }

    BoostableRewards storage system = rewardSystems[_token];
    system.boostedWeightPercentage = _boostWeightPercentage;
    system.baseWeightPercentage = PERCENTAGE_DENOMINATOR - _boostWeightPercentage;
  }

  function getUserDeposit(address _user) external view override returns (uint256) {
    return userDepositBalances[_user];
  }
    ...
    returns (uint256 rewardPerStaking_, uint256 rewardPerBoostingWeight_)
  {
    ...
    return (_rewardPerToken(true, system), _rewardPerToken(false, system));
  }

  function _rewardPerToken(bool _stakingStream, BoostableRewards storage _system)
    internal
    view
    returns (uint256)
  {
    uint256 totalWeight = _stakingStream ? totalStaked : totalBoostWeight;
    Reward memory reward = _stakingStream ? _system.stakingReward : _system.boostingReward;
    ...
    return reward.rewardPerTokenStored
      + Math.mulDiv(
        (getLastTimeRewardApplicable(_system.endPeriod) - _system.lastUpdate),
        reward.rewardRatePerSecond,
        totalWeight
      );
  }

  function getLastTimeRewardApplicable(uint128 _endPeriod) private view returns (uint64) {
    return uint64(Math.min(block.timestamp, _endPeriod));
  }

  function getBoostAmount(address _account) external view override returns (uint256) {
    return userBoosts[_account];
  }

  function getBoostWeight(address _user) external view override returns (uint256) {
    return userBoostWeights[_user];
  }
  ...
  {
    BoostableRewards storage system = rewardSystems[_token];
    return (system.stakingReward, system.boostingReward);
  }
  ...
  function getRewardTokenConfiguration(address _token)
    external
    view
    returns (RewardSystemReadable memory system_)
  {
    BoostableRewards storage systemData = rewardSystems[_token];

    system_.baseWeightPercentage = systemData.baseWeightPercentage;
    system_.boostedWeightPercentage = systemData.boostedWeightPercentage;
    system_.endPeriod = systemData.endPeriod;
    system_.lastUpdate = systemData.lastUpdate;
    system_.stakingReward = systemData.stakingReward;
    system_.boostingReward = systemData.boostingReward;
