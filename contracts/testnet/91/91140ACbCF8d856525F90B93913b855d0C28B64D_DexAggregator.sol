...
contract DexAggregator is MixinResolver, Owned {
    ...
    event DexSwapSynthToNativeOn0x(
    ...
    event DexSwapNativeToSynthOn0x(
        address indexed _account,
        bytes32 _sourceNativeKey,
        uint256 _sourceNativeAmount,
        bytes32 _destSynthKey,
    ...
    function swapSynthToNativeOn0X(
        ...
        address _target,
        bytes memory _data
    ) external systemActive onlySynthrSwap returns (uint256) {
        require(
            issuer().synths(_synthKey).balanceOf(address(this)) >= _synthAmount,
            "Insufficient synth balance on DEX aggregator."
        );
        if (IERC20(address(issuer().synths(_synthKey))).allowance(address(this), _target) <= _synthAmount) {
            require(IERC20(address(issuer().synths(_synthKey))).approve(_target, type(uint256).max), "Synth approve failed.");
        }
        ...
        emit DexSwapSynthToNativeOn0x(_account, _synthKey, _synthAmount, nativeKey, swappedAmount);
    ...
    function swapNativeToSynthOn0X(
        ...
        address _target,
        bytes memory _data
    ) external payable systemActive onlySynthrSwap returns (uint256) {
        require(msg.value >= _nativeAmount, "Insufficient native currency value on DEX aggregator.");
        uint256 beforeSynthBalance = issuer().synths(_destSynthKey).balanceOf(address(this));

        (bool success, bytes memory res) = _target.call{value: msg.value}(_data);
        require(success, string(bytes("SWAP_CALL_FAILED: ").concat(bytes(res.getRevertMsg()))));

        uint256 afterSynthBalance = issuer().synths(_destSynthKey).balanceOf(address(this));

        uint256 swappedAmount = afterSynthBalance >= beforeSynthBalance ? afterSynthBalance - beforeSynthBalance : 0;
        require(swappedAmount > 0, "Invalid buy currency amount.");
        TransferHelper.safeTransfer(address(issuer().synths(_destSynthKey)), _account, swappedAmount);
        emit DexSwapNativeToSynthOn0x(_account, _nativeKey, _nativeAmount, _destSynthKey, swappedAmount);
