...
contract StarNFTV4 is Ownable, ERC165, IERC721, IERC721Metadata, IStarNFT {
    ...
    //id => uri id
    mapping(uint256 => uint256) public uriMap;
    ...
    returns (string memory)
    {
        ...
        uint256 uriId = uriMap[tokenId];
        return
        bytes(_baseURI).length > 0
        ? string(abi.encodePacked(_baseURI, uriId.toString(), ".json"))
        : "";
    }
    ...
    function mintUri(address account, uint256 cid, uint256 uriId)
    external
    onlyMinter
    returns (uint256)
    {
    ...
    function mintBatchUri(
        address account,
        uint256 amount,
        uint256[] calldata cidArr,
        uint256[] calldata uriArr
    ...
    function setUriMap(uint256 tokenId, uint256 uriId) external onlyOwner {
        uriMap[tokenId] = uriId;
    }
