...
// This is the sender which lives on L2, it sends payloads to checkout an NFT on L1
contract CCS is OwnableWithTransferCallback {
    ...
    mapping(uint256 => uint256) public lastAttemptedSwapTimeForId;

    constructor(address _endpoint) {
        endpoint = _endpoint;
    }

    // This can be called by anyone to send a message to L0, which then bridges it to L1
    function send(uint16 _dstChainId, bytes memory _payload, uint256 _nativeFee, uint256 _ethPayment)
        external
        payable
    {
        bytes memory trustedRemote = trustedRemoteLookup[_dstChainId];
        require(trustedRemote.length != 0, "Configure remote");

        // Ensure that multiple people aren't all trying to buy the same NFT
        (, , uint256[] memory ids, ) =
            abi.decode(_payload, (address, address, uint256[], uint256));

        require(ids.length == 1, "1 at a time");

        uint256 lastAttemptedSwapTime = lastAttemptedSwapTimeForId[ids[0]];

        require((block.timestamp - lastAttemptedSwapTime) > 1 days, "Too soon");
        lastAttemptedSwapTimeForId[ids[0]] = block.timestamp;

        // Send message to L0
        ILayerZeroEndpoint(endpoint).send{value: _nativeFee}(
            _dstChainId, trustedRemote, _payload, payable(msg.sender), address(0x0), bytes("")
        );

        // Optimistically cover the ETH amount needed
        payable(owner()).safeTransferETH(_ethPayment);
    }
