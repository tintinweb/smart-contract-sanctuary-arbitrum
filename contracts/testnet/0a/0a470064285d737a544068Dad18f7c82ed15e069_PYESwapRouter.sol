...
// !!!!!!!!!
    ...
    function swapExactTokensForTokens(
        ...
        uint transferAmount = amountIn;
        uint adminFeeDeduct;
        if(path[0] == pairFeeAddress[pair]){
            (amountIn, transferAmount, adminFeeDeduct) = PYESwapLibrary.adminFeeCalculation(amountIn, adminFee, swapFee);
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, pair, transferAmount
        );
    ...
    function swapTokensForExactTokens(
        ...
        if(path[0] == pairFeeAddress[pair]) {
            ...
            totalFee = feeTaker != address(0) ? IToken(feeTaker).getTotalFee(to) + sFee : sFee;
            ...
            (, adminFeeDeduct,) = PYESwapLibrary.adminFeeCalculation(amounts[0], adminFee, swapFee);
            TransferHelper.safeTransferFrom(
                path[0], msg.sender, adminFeeAddress, adminFeeDeduct
            );
...
// !!!!!!!!!
    ...
    returns (uint[] memory amounts)
    {
        ...
        uint adminFeeDeduct;
        if(path[0] == pairFeeAddress[pair]){
            (amountIn, adminFeeDeduct,) = PYESwapLibrary.adminFeeCalculation(amountIn, adminFee, swapFee);
    ...
    returns (uint[] memory amounts)
    {
        ...
        if(path[1] == pairFeeAddress[pair]){
            (amountETHOut, adminFeeDeduct,) = PYESwapLibrary.adminFeeCalculation(amountETHOut, adminFee, swapFee);
        }
        if(totalFee > 0) {
    ...
    returns (uint[] memory amounts)
    {
        ...
        if(path[0] == pairFeeAddress[pair]){
            ...
            ( ,adminFeeDeduct, swapFeeDeduct) = PYESwapLibrary.adminFeeCalculation(amounts[0], adminFee, swapFee);
            if(address(this) != adminFeeAddress){
                payable(adminFeeAddress).transfer(adminFeeDeduct);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if(path[1] == pairFeeAddress[pair]){
            (amountOutMin,adminFeeDeduct,) = PYESwapLibrary.adminFeeCalculation(amountOutMin, adminFee, swapFee);
        }
        require(
...
library PYESwapLibrary {
    ...
    address private constant factory = 0xCef79C43fa6fb4fC93911ea65c26a33286802993;
    ...
    function pairFor(address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                hex'd4e130d626aa7b7c64fdee7a1e89b37ce329dc586915edbdaab78d8454436737' // init code hash
            )))));
    }
    ...
    function adminFeeCalculation(
        ...
        uint swapFeeDeduct = (_amounts * _swapFee) / 10000;
        uint swapAmount = _amounts - swapFeeDeduct;
        uint transferAmount = _amounts - adminFeeDeduct;

        return (swapAmount, transferAmount, adminFeeDeduct);
    }
}
