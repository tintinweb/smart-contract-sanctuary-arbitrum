...
contract DistributionContract {
    ...
    function distributeRewards(uint256 arbotAmount, uint256 ethAmount) external {
        ...
        totalDistributionEth += ethAmount;

        // Transférer les ARBOT destinés à l'équipe
        token.transfer(teamAddress, (arbotAmount * 50) / 100);

        // Transférer les ETH destinés à l'équipe
        payable(teamAddress).transfer((ethAmount * 50) / 100);

        distributeRewardsToHolders(arbotAmount, ethAmount);
    }
    ...
    function distributeRewardsToHolders(uint256 arbotAmount, uint256 ethAmount) internal {
        ...
        for (uint256 i = 0; i < token.holdersCount(); i++) {
            ...
            if (isEligible(holder)) {
                ...
                holders[holder].accumulatedEth += (eligibleDistribution * holderShare) / remainingDistribution;
            }
        }
    }
    ...
    function getAvailableArbotToClaim(address holder) public view returns (uint256) {
        uint256 availableArbot = holders[holder].accumulatedArbot;
        return availableArbot;
    }

    function getAvailableEthToClaim(address holder) public view returns (uint256) {
        uint256 availableEth = holders[holder].accumulatedEth;
        return availableEth;
    }
    ...
    function updateEpoch() external {
        ...
        if (newEpoch > epoch.currentEpoch()) {
            epoch.updateEpoch();
            distributeRewardsToHolders(0, 0); // Effectuer la distribution des récompenses pour le nouvel epoch
        }
    }
}
