    ...
    ERC1155Burnable
{
    ...
    uint256 public defaultPercentage;
    ...
    function mint(
        ...
        if (_percentage == 0) {
            _percentage = defaultPercentage;
        }
        uint256 _toTreasuryAmount = (msg.value * _percentage) / 100;
        uint256 _toRevenueAmount = msg.value - _toTreasuryAmount;

        address _revenueAddress = revenueAddress[_id];
        if (_revenueAddress == address(0)) {
            _revenueAddress = treasuryAddress;
        }
