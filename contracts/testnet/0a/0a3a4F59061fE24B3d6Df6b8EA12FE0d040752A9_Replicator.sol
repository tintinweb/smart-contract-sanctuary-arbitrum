...
interface ICassette is IERC1155 {
    function currentMaxChapter() external view returns (uint);
...
interface IReplicator is IERC721 {}
...
import { IReplicator } from "./interfaces/IReplicator.sol";

error Replicator_FunctionLocked();
error Replicator_IncorrectOwner();
error Replicator_IncorrectValue();
error Replicator_InvalidValue();
error Replicator_InvalidMintState();
error Replicator_MintNotClosed();
error Replicator_SenderNotAllowed();
error Replicator_TokenAlreadyExists();
error Replicator_WithdrawFailed();

contract Replicator is ONFT721Core, ERC721Enumerable, IReplicator {
    ICassette public immutable CASSETTE;
    uint256 public immutable PRICE;
    uint256 public immutable ROOT_CHAIN_ID;

    string public baseURI;
    mapping(uint256 => uint256) public cassettesMinted;
    ...
    event Replicate(
        ...
        uint256 indexed startChapter,
        uint256 indexed numOfCassettes
    );
    ...
    constructor(
        ...
        ERC721("KAIJU ORIGINS: The Journals of Stod - Replicator", "REPLICATOR")
     ...
     */
    modifier lockable() {
        if (functionLocked[msg.sig]) revert Replicator_FunctionLocked();
        _;
    }

    /**
     * @notice Modifier requiring cassette minting to be closed
     */
    modifier onlyAfterMintClosed() {
        if (!CASSETTE.mintClosed()) revert Replicator_InvalidMintState();
        _;
    }

    /**
     * @notice Modifier validating the replicate transaction
     * @param _replicatorId Replicator used to mint the next cassettes in the series from
     * @param _numOfCassettes Amount of next cassettes in the series to mint
     */
    modifier validateReplicate(uint256 _replicatorId, uint256 _numOfCassettes) {
        if (msg.sender != ownerOf(_replicatorId)) revert Replicator_SenderNotAllowed();
        if (
            cassettesMinted[_replicatorId] + _numOfCassettes + 1 > CASSETTE.currentMaxChapter()
            || _numOfCassettes == 0
        ) revert Replicator_InvalidValue();
        _;
    }
    ...
    /**
     * @notice Mint a specified number of replicators
     * @param _amount Amount of replicators to mint
     */
    function mint(uint256 _amount) public payable {
        if (CASSETTE.mintClosed()) revert Replicator_InvalidMintState();
        if (msg.value != _amount * PRICE) revert Replicator_IncorrectValue();
    ...
    /**
     * @notice Mint a specified amount of the next cassettes in the series using a replicator
     * @param _receiver Address to receive the cassettes
     * @param _startChapter Starting chapter to mint from
     * @param _numOfCassettes Amount of next cassettes in the series to mint
     */
    function _replicate(address _receiver, uint256 _startChapter, uint256 _numOfCassettes) internal {
        for (uint256 i; i < _numOfCassettes;) {
            uint256 nextChapter = _startChapter + i;

            CASSETTE.replicatorMint(_receiver, nextChapter, 1);
            emit Replicate(_receiver, nextChapter, 1);
    ...
    /**
     * @notice Mint a specified amount of next cassettes in the series using a replicator
     * @param _replicatorId Replicator to use to mint the next cassettes in the series
     * @param _numOfCassettes Amount of next cassettes in the series to mint
     */
    function replicate(
        uint256 _replicatorId,
        uint256 _numOfCassettes
    )
        public
        payable
        validateReplicate(_replicatorId, _numOfCassettes)
    {
        uint256 nextChapter = cassettesMinted[_replicatorId] + 1;
        cassettesMinted[_replicatorId] += _numOfCassettes;

        _replicate(msg.sender, nextChapter, _numOfCassettes);
    }

    /**
     * @notice Mint a specified amount of the next cassettes in the series to a destination chain using a replicator
     * @dev For more details see https://layerzero.gitbook.io/docs/evm-guides/master
     * @param _replicatorId Replicator to use to mint the next cassettes in the series from
     * @param _numOfCassettes Amount of next cassettes in the series to mint
        ...
        validateReplicate(_replicatorId, _numOfCassettes)
    {
        ...
        bytes memory payload = abi.encode(
            ...
            _toAddress,
            _toSingletonArray(nextChapter),
            _toSingletonArray(_numOfCassettes)
        );
    ...
    function withdraw(address _receiver) public onlyOwner {
        ...
        if (!success) revert Replicator_WithdrawFailed();
    }
        ...
        returns (uint256 nativeFee, uint256 zroFee)
    {
        return estimateBatchFee(
            ...
            _toSingletonArray(cassettesMinted[_tokenId]),
            _useZro,
            _adapterParams
        );
    }
    ...
    function estimateSendBatchFee(
        ...
        returns (uint256 nativeFee, uint256 zroFee)
    {
        uint256[] memory _cassettesMinted = new uint256[](_tokenIds.length);
        for (uint256 i; i < _tokenIds.length;) {
            _cassettesMinted[i] = cassettesMinted[_tokenIds[i]];
            unchecked { ++i; }
        }
     ...
     * @param _cassettesMinted Cassettes minted by each replicator
     * @param _useZro Flag indicating whether to use $ZRO for payment
     * @param _adapterParams Parameters for custom functionality
     */
    function estimateBatchFee(
        ...
        bytes memory _toAddress,
        uint256[] memory _tokenIds,
        uint256[] memory _cassettesMinted,
        bool _useZro,
        bytes memory _adapterParams
    ) public view returns (uint256 nativeFee, uint256 zroFee) {
        bytes memory payload = abi.encode(_functionType, _toAddress, _tokenIds, _cassettesMinted);
        return lzEndpoint.estimateFees(_dstChainId, address(this), payload, _useZro, _adapterParams);
    }
    ...
    function _debitFrom(
        ...
        override
    {
        if (!_isApprovedOrOwner(msg.sender, _tokenId)) revert Replicator_SenderNotAllowed();
        if (ERC721.ownerOf(_tokenId) != _from) revert Replicator_IncorrectOwner();
    ...
    function _creditTo(
        ...
        override
    {
        if (_exists(_tokenId) && ERC721.ownerOf(_tokenId) != address(this)) revert Replicator_TokenAlreadyExists();
    ...
    function _send(
        ...
        override
    {
        if (!CASSETTE.mintClosed() && msg.sender != address(bytes20((_toAddress))))
            revert Replicator_InvalidMintState();
        ...
        bytes memory payload = abi.encode(FUNCTION_TYPE_SEND, _toAddress, _tokenIds, _cassettesMinted);
        ...
        override
    {
        (
            ...
            uint256[] memory cassettesMintedArr
        ...
        if (functionType == FUNCTION_TYPE_REPLICATE) {
            ...
            _replicate(
                toAddress,
                tokenIds[0], // alias for _startChapter
                cassettesMintedArr[0] // alias for _numOfCassettes
            );
        } else {
            for (uint256 i; i < tokenIds.length;) {
                cassettesMinted[tokenIds[i]] = cassettesMintedArr[i];
                unchecked { ++i; }
            }
    ...
    /**
     * @dev Replicators are soulbound until after the mint is closed
     * @inheritdoc ERC721
     */
