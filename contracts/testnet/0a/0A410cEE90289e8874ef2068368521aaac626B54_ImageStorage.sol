...
contract ImageStorage is Permissioned {
    ...
    function addImage(Image calldata _image) external onlyOwner {
        ...
        images[_image.imageName] = _image;
        imageNames.push(_image.imageName);
    }

    function deleteImage(string calldata _imageName) external onlyOwner {
        require(bytes(images[_imageName].imageName).length > 0, "Image name must not be empty");
        delete images[_imageName];
    }

    function getImageAsBase64(string memory _imageName) external view returns (string memory) {
        Image storage image = images[_imageName];
    ...
    function getImageNames() external view returns (string[] memory) {
        return imageNames;
    }
}
