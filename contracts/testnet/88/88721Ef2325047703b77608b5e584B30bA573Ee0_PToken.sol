...
contract PToken is ERC20Upgradeable, IPToken {
    ...
    function runDeposit(uint requestId, int uPnl) public isUpnlFeeder returns (uint){
        ...
        LockInfo[LockByte32]=Lock(applyDepositData.sender, plpAmount, block.timestamp);
        DoubleEndedQueue.pushBack(AddressLocks[applyDepositData.sender],LockByte32);
    ...
    function runWithdraw(uint requestId, int uPnl) public isUpnlFeeder returns (uint){
        ...
        if (sum != applyWithdrawData.plpAmount) {
            ...
            LockInfo[DoubleEndedQueue.at(AddressLocks[_msgSender()],i-1)].assets = sum - applyWithdrawData.plpAmount;
        } else {
        ...
        if (needDelIndex >= 0) {
            ...
            for (uint j = 0; j < index; j++) {
                delete LockInfo[DoubleEndedQueue.popFront(AddressLocks[_msgSender()])];
            }
        }
