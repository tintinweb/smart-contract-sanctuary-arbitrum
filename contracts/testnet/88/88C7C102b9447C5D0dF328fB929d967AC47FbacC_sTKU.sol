...
contract sTKU is FeeToken {
    error InsufficientTimePassed(uint256 currentTime, uint256 unlockTime);
    ...
    constructor(
        ...
        address _TKU,
        uint256 _lockTimer
    ) FeeToken(_USDT, _TKU, "Staked TKU", "sTKU", 0) {
    ...
    function unstake(uint256 _toUnstake) external nonReentrant {
        if (lastDepositTime[msg.sender] + lockTimer > block.timestamp) {
            revert InsufficientTimePassed(
                block.timestamp,
                lastDepositTime[msg.sender] + lockTimer
            );
        }
        if (_toUnstake > _balances[msg.sender]) {
            revert InsufficientTKUBalance(_toUnstake, _balances[msg.sender]);
        }
        if (_toUnstake > _allowances[msg.sender][address(this)]) {
            revert InsufficientTKUAllowance(
                _toUnstake,
                _allowances[msg.sender][address(this)]
            );
        }
    ...
    function setLockTimer(uint256 newLockTimer) external onlyOwner {
