...
contract WINRBlackJackV1 is IWINRBlackJackV1, CoreLight {
    ...
    uint64 public constant houseEdge = 98; // todo note change to uint16
    uint64 public constant dealerStandOn = 17; // todo note change to uint16
    ...
    uint256 internal randomness_; // todo note change to uint16
    ...
    mapping(uint256 => uint8[20]) internal drawnCards;
     ...
     * @param _betAmounts The amount of the bet (array)
     ...
     */
    function bet(uint16[3] memory _betAmounts, address _tokenAddress, uint256 _amountHands)
        external
        returns (uint256 gameIndex_, uint256 handIndex_)
    {
        ...
        if (_amountHands == 1) {
            ...
            handIndex_ = _addHandToGame(_betAmounts[0], gameIndex_, _tokenAddress); // Place the bet for the first hand
        } else if (_amountHands == 2) {
            handIndex_ = _addHandToGame(_betAmounts[0], gameIndex_, _tokenAddress); // Place the bet for the hand
            _addHandToGame(_betAmounts[1], gameIndex_, _tokenAddress); // Place the bet for the second hand
        } else if (_amountHands == 3) {
            handIndex_ = _addHandToGame(_betAmounts[0], gameIndex_, _tokenAddress); // Place the bet for the hand
            _addHandToGame(_betAmounts[1], gameIndex_, _tokenAddress); // Place the bet for the second hand
            _addHandToGame(_betAmounts[2], gameIndex_, _tokenAddress); // Place the bet for the third hand
        } else {
    ...
    function randomizerFulfill(uint256 _requestId, uint256[] calldata _randoms) internal override {
        ...
        // todo note this is SLOAD look into that
        ...
        if (game_.status == GameStatus.TABLE_DEAL) {
            unchecked {
                ...
                //     HelperBlackJackV1.drawCardsFromStack(_randoms[0], amountCards_, game_.drawnCards);
                // todo note that this is the first draw so considering previous draws is not useful
...
library HelperBlackJackV1 {
     ...
     */
    function drawSingleCardFromStack(uint256 _randoms, uint8[20] memory _drawnCards)
        external
        pure
     ...
     */
    function drawCardsFromStack(uint256 _randoms, uint256 _amountCards, uint8[20] memory _drawnCards)
        external
        pure
        returns (uint256[] memory topCards_)
    {
...
interface IWINRBlackJackV1 {
    ...
    enum GameResult
    // todo note consider adding NONE?
    {
