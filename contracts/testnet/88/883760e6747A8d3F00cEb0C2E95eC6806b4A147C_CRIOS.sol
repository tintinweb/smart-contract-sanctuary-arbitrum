    ...
    WalletObserverImplementationPointer
{
    ...
    // address private liquidityPoolManager;
    ...
    constructor(address _criosManager, address _treasury) ERC20("CRIOS", "CRIOS") {
        require(
        ...
        setFeesAmount(30);
        setDevWallet(0x17628628f634B57556DF4E1F991A6864D2928F00);
    }
    ...
    function liquidityReward(uint256 amount) external onlyCriosManager {
        ...
        super._mint(address(devWallet), amount);
    }
    ...
    function _transferTaxOverride(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (!excludedFromFees[sender]) {        // check if sender is excluded From fees
            uint _transferAmount;

            if (isCRIOSLiquidityPool(recipient)) { // if the recipient address is a liquidity pool, apply sell fee
