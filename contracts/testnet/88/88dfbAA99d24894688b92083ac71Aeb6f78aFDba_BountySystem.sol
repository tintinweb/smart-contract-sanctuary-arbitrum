...
contract BountySystem is IBountySystem, GameRegistryConsumerUpgradeable {
    ...
    function fulfillRandomWordsCallback(
        ...
        if (request.account != address(0)) {
            ...
            // Grant successXp
            _handleXp(request.account, successXp);
    ...
    /**
     * Handles the granting of XP, awarded to user captain
     * @param account The account that is receiving XP reward to their captain
     * @param successXp The amount of XP to grant
     */
    function _handleXp(address account, uint256 successXp) internal {
        // Grant XP if any
        if (successXp > 0) {
            // Get user captain
            (address tokenContract, uint256 tokenId) = ICaptainSystem(
                _getSystem(CAPTAIN_SYSTEM_ID)
            ).getCaptainNFT(account);
            // Grant XP to user captain
