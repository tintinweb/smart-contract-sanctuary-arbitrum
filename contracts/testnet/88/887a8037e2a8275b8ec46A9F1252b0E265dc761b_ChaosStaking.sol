...
// File: contracts/interfaces/IChaosStaking.sol

interface IChaosStaking {
    event RewardsSet(
...
// File: contracts/ChaosStaking.sol

contract ChaosStaking is
    IChaosStaking,
    OwnableUpgradeable,
    ...
    PausableUpgradeable
{
    ...
    function setRewards(
        ...
        uint256 rewardBalance = address(stakingToken) != address(rewardsToken)
            ...
            : stakingToken.balanceOf(address(this)).sub(totalStaked);
    ...
    function recoverNonLockedRewardTokens() external override onlyOwner {
        uint256 nonLockedTokens = address(stakingToken) != address(rewardsToken)
            ...
            : rewardsToken.balanceOf(address(this)).sub(rewardTokensLocked).sub(totalStaked);
    ...
    function withdraw(uint256 _amount) public override nonReentrant updateReward(msg.sender) {
        ...
        require(
            block.timestamp >= lastStakedTime[msg.sender] + lockingPeriod,
            "Tokens are locked for now"
        );
