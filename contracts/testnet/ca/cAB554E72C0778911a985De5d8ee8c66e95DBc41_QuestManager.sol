...
contract QuestManager is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  function quest(
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      // TODO: Check quest count
      require(questCount <= 7, "QuestManager: Quest count is above max");
      ...
      // Go on quests
      _quests(addr, adventurerId, questIds);
  ...
  function _quests(
    ...
    for (uint256 j = 0; j < _questIds.length; j++) {
      ...
      (uint256 a, uint256 b) = _quest(_addr, _adventurerId, questId);
  ...
  function _quest(
    ...
    (uint256 xp, uint256 traitId, uint256 traitAmount, uint256 anima) = quests
      .go(_addr, _adventurerId, _questId);
