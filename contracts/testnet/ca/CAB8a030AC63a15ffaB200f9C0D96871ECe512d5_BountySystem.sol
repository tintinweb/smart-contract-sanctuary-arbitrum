...
struct SetBountyInputParam {
    ...
    // Bounty Base loot
    BountyLootInput baseLoot;
    // Bounty Bonus loot
    BountyLootInput bonusLoot;
}
...
contract BountySystem is IBountySystem, GameRegistryConsumerUpgradeable {
    ...
    function setBountyDefinition(
        ...
        BountyComponent(_gameRegistry.getComponent(BOUNTY_COMPONENT_ID))
            .setValue(
                ...
                definition.baseLoot.lootEntity,
                definition.bonusLoot.lootEntity
            );
        ...
        LootSetComponent(_gameRegistry.getComponent(LOOT_SET_COMPONENT_ID))
            .setValue(
                definition.baseLoot.lootEntity,
                definition.baseLoot.lootType,
                definition.baseLoot.tokenContract,
                definition.baseLoot.lootId,
                definition.baseLoot.amount
            );
        // If Bonusloot exists set Bounty Bonus reward Loot component with unique Bounty Bonus reward Loot GUID
        if (definition.bonusLoot.lootEntity > 0) {
            LootSetComponent(_gameRegistry.getComponent(LOOT_SET_COMPONENT_ID))
                .setValue(
                    definition.bonusLoot.lootEntity,
                    definition.bonusLoot.lootType,
                    definition.bonusLoot.tokenContract,
                    definition.bonusLoot.lootId,
                    definition.bonusLoot.amount
                );
        }
    ...
    function endBounty(
        ...
        if (failedBounty) {
            ...
            _setActiveBountyCompleted(account, activeBountyId, false);
        } else {
    ...
    function fulfillRandomWordsCallback(
        ...
        if (request.account != address(0)) {
            ...
            // If bonusloot exists, award it
            if (bonusLootId > 0) {
                // Convert bonusloot to ILootSystem.Loot
                ILootSystem.Loot[] memory bonusRewardLoot = _convertLootSet(
                    bonusLootId
                );
                // Grant bonusloot
                lootSystem.grantLootWithRandomWord(
    ...
    function _handleLoot(address account, uint256 activeBountyId) internal {
        ...
        if (lootSystem.validateLoots(baseRewardLoot) || bonusLootId > 0) {
            // Request VRF
        ...
        // Set ActiveBounty status to COMPLETED
        _setActiveBountyCompleted(account, activeBountyId, true);
    }
     ...
     * @param activeBountyId ID of the active bounty to end
     * @param success Whether or not the bounty was successful
     */
    ...
    function _validateSetBountyInput(
        ...
        // Check Bounty successXp, bountyTimeLock, lowerBound, upperBound is present
        if (
            definition.successXp == 0 ||
        ...
        if (
            ...
            definition.baseLoot.lootEntity == 0
        ) {
            revert MissingInputs();
        }
        ...
        lootSystem.validateLoots(_convertBountyLootInput(definition.baseLoot));
        // If Bounty bonusloot exists check Bonus loot
        if (definition.bonusLoot.lootEntity > 0) {
            lootSystem.validateLoots(
                _convertBountyLootInput(definition.bonusLoot)
            );
        }
    }
}
