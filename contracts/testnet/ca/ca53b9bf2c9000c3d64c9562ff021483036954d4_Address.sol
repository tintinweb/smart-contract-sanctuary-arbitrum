 ...
 */
/* New Solidity version requires change here:
 * The reason for this error is that starting from Solidity version 0.8.0, the default location for functions and state variables has changed. The new default location for state variables is memory, and for functions, it's calldata. This change was introduced to make the codebase more explicit and prevent unintentional storage usage.
 */
abstract contract Context {
    function _msgSender() internal view virtual returns (address payable) {
        return payable(msg.sender); // Specify the return type as address payable
    }
...
library AddressArrayUtils {
     ...
     */
    function indexOf(address[] memory A, address a) internal pure returns (int256, bool) {
        ...
        return (-1, false); // Return -1 directly as an int256
    }
    ...
    */
    ...
    {
        (int256 index, bool isIn) = indexOf(A, a);
        if (!isIn) {
            revert("Address not in array.");
        } else {
            (address[] memory _A,) = pop(A, uint256(index));
            return _A;
        }
    }
