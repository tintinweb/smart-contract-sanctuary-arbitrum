...
// File contracts/intf/IDealer.sol
...
// File contracts/intf/IPerpetual.sol
...
// File contracts/testSupport/ContractHelper.sol
...
contract ContractHelper {
    ...
    constructor(address _JOJODealer) {
    ...
    function getAccountsStates(address[] calldata accounts) public view returns (AccountState[] memory accountStates) {
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            (int256 netValue, uint256 exposure, uint256 maintenanceMargin) =
            ...
            for (uint256 j = 0; i < perp.length; j++) {
                (int256 paper, int256 credit) = Perpetual(perp[j]).balanceOf(accounts[i]);
    ...
    function getPerpsStates(address[] calldata perps) public view returns (PerpState[] memory perpStates) {
        for (uint256 i = 0; i < perps.length; i++) {
            ...
            perpStates[i].fundingRate = Perpetual(perps[i]).getFundingRate();
