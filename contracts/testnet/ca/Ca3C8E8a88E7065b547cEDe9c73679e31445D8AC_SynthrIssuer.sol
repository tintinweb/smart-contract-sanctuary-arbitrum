...
interface ISynthrAggregator {
    /* ========== VIEW FUNCTIONS ========== */
    function collateralByIssuerAggregation(bytes32 currencyKey, uint16 chainId, address account) external view returns (uint256);
...
contract SynthrIssuer is Owned, MixinSystemSettings {
    ...
    function issuableSynthExpected(
        ...
        if (alreadyIssued >= maxIssuable) {
            ...
            maxIssuable = maxIssuable - alreadyIssued;
        }
    }
    ...
    function getSendMintGasFee(
        ...
        if (_destChainId != 0) {
            ...
            return synthrBridge(_bridgeName).calcFee(lzPayload, PT_MINT_SYNTH, _destChainId);
        }
        return 0;
    }
    ...
    function getSendLiquidateGasFee(
        ...
        return synthrBridge(_bridgeName).calcFee(lzPayload, PT_LIQUIDATE, _chainId);
    }
    ...
    function burnSynthsWithoutDebt(
        ...
        address from,
        uint amount
    ) external onlyAllowedCaller returns (uint256 burnAmount) {
        require(address(synths[currencyKey]) != address(0), "synth doesn't exist");
        require(amount > 0, "cannot issue 0 synths");

        (uint256 reclaimed, uint256 rebated, ) = exchanger().settle(from, currencyKey);

        // Burn some synths
        synths[currencyKey].burn(from, amount);

        burnAmount = amount + reclaimed - rebated;
    }
    ...
    function bridgeSynth(
        ...
        require(synths[_synthKey].balanceOf(_account) >= _amount, "Insufficient synth amount to bridge");
        synths[_synthKey].burn(_account, _amount);
        synthrBridge(_bridgeName).sendBridgeSyToken{value: msg.value}(_account, _synthKey, _amount, _destChainId, _erc20Payment);
    ...
    modifier onlySynthrBridgeOrWrappedSynthr() {
        require(
            msg.sender == address(wrappedSynthr()) || resolver.isBridge(msg.sender),
    ...
    modifier onlyAllowedCaller() {
        require(msg.sender == address(feePool()), "Issuer: Not allowed caller");
        _;
    }
