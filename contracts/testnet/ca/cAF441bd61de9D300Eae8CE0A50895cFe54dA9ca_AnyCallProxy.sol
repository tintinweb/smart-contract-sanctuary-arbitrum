...
contract AnyCallProxy is NonblockingLzApp {
    ...
    * @param _paymentAddress Payment address
    ...
    */
    function anyCall(address _paymentAddress, address _to, bytes calldata _data, uint16 _toChainID) external payable {
        ...
        //(uint nativeFee,) = lzEndpoint.estimateFees(_toChainID, address(this), payload, false, bytes(""));
        //console.log("nativeFee : %s", nativeFee);

        _lzSend( // {value: messageFee} will be paid out of this contract!
            _toChainID, // destination chainId
            payload, // abi.encode()'ed bytes
            payable(_paymentAddress), // (msg.sender will be this contract) refund address (LayerZero will refund any extra gas back to caller of send()
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64, bytes memory _payload) internal override {
        (address _contract, bytes memory _data) = abi.decode(_payload, (address, bytes));
