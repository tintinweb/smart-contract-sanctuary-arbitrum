...
contract FeePool is Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        ...
        newAddresses[1] = CONTRACT_SYNTHRDEBTSHARE;
        ...
        newAddresses[6] = CONTRACT_ISSUED_SYNTH_AGGREGATOR;
        newAddresses[7] = CONTRACT_FUTURES_MARKET_MANAGER;
    ...
    function _claimFees(address claimingAddress) internal returns (bool) {
        ...
        require(!anyRateIsInvalid, "Invalid SYNTH rate.");
    ...
    function feesByPeriod(address account) public view returns (uint256[2][FEE_PERIOD_LENGTH] memory results) {
        ...
        userOwnershipPercentage = synthrDebtShare().sharePercent(account);
        ...
        for (uint256 i = FEE_PERIOD_LENGTH - 1; i > 0; i--) {
            ...
            if (lastFeeWithdrawal < periodId) {
                userOwnershipPercentage = synthrDebtShare().sharePercentOnPeriod(account, uint256(periodId));
    ...
    function effectiveDebtRatioForPeriod(address account, uint256 period) external view returns (uint256) {
        ...
        return synthrDebtShare().sharePercentOnPeriod(account, uint256(_recentFeePeriods[period].feePeriodId));
    }
