...
contract Focus is IFocus, Pausable, EpochConfigurable {
  // adventurer address => adventurer id => timestamp
  mapping(address => mapping(uint => uint)) public LAST_ACTIVITY_TIMESTAMP;
  // adventurer address => adventurer id => remaining focus
  mapping(address => mapping(uint => uint)) public CURRENT_FOCUS;

  uint256 public FOCUS_PER_ACTIVITY;
  uint256 public MAX_FOCUS;

  constructor(address _manager) EpochConfigurable(_manager, 24 hours, 0) {
    FOCUS_PER_ACTIVITY = 10000;
    MAX_FOCUS = 100000;
  }
  ...
  function isFocusedThisEpoch(
    ...
    for (uint i = 0; i < _adventurerAddresses.length; i++) {
      result[i] = (epochAtTimestamp(
        LAST_ACTIVITY_TIMESTAMP[_adventurerAddresses[i]][_adventurerIds[i]]
      ) >= currentEpoch);
    }
  }
  ...
  function markFocusedBatch(
    ...
    for (uint i = 0; i < _adventurerAddresses.length; i++) {
      _markFocused(_adventurerAddresses[i], _adventurerIds[i]);
    }
  }

  function markFocused(address _adventurerAddress, uint _adventurerId) external onlyManager {
    _markFocused(_adventurerAddress, _adventurerId);
  }
  ...
  function spendFocus(
    ...
    _spendFocus(_adventurerAddress, _adventurerId, _spentFocus);
  }
  ...
  function spendFocusBatch(
    ...
    for (uint i = 0; i < _adventurerAddresses.length; i++) {
      _spendFocus(_adventurerAddresses[i], _adventurerIds[i], _spendings[i]);
    }
  }
  ...
  function _currentFocus(
    ...
    return CURRENT_FOCUS[_adventurerAddress][_adventurerId];
  }

  function _spendFocus(address _adventurerAddress, uint _adventurerId, uint _spentFocus) internal {
    uint focus = _currentFocus(_adventurerAddress, _adventurerId);
    if (focus < _spentFocus) {
      revert InsufficientFocus(_adventurerAddress, _adventurerId, _spentFocus, focus);
    }

    CURRENT_FOCUS[_adventurerAddress][_adventurerId] = focus - _spentFocus;
  }

  function _markFocused(address _adventurerAddress, uint _adventurerId) internal {
    uint lastActivityEpoch = epochAtTimestamp(
      LAST_ACTIVITY_TIMESTAMP[_adventurerAddress][_adventurerId]
    );
    if (lastActivityEpoch >= currentEpoch()) {
      return;
    }

    LAST_ACTIVITY_TIMESTAMP[_adventurerAddress][_adventurerId] = block.timestamp;
    uint focus = CURRENT_FOCUS[_adventurerAddress][_adventurerId];
    if (focus >= MAX_FOCUS) {
      return;
    }

    focus += FOCUS_PER_ACTIVITY;
    if (focus > MAX_FOCUS) {
      focus = MAX_FOCUS;
    }
    CURRENT_FOCUS[_adventurerAddress][_adventurerId] = focus;
  }

  function updateFocusConfig(uint256 focusPerActivity, uint256 maxFocus) external onlyAdmin {
    FOCUS_PER_ACTIVITY = focusPerActivity;
    MAX_FOCUS = maxFocus;
  }
}
