...
contract CockFight is ERC721A, Ownable, ReentrancyGuard {

    uint256 mintPricePublic = 0.002 ether;
    uint256 mintPriceWhitelist = 0.001 ether;
    uint256 public MAX_SUPPLY = 5555;
    uint256 MAX_PER_TX_WHITELIST = 3;
    uint256 MAX_PER_WALLET_WHITELIST = 20;
    uint256 MAX_PER_TX_PUBLIC = 3;
    uint256 MAX_PER_WALLET_PUBLIC = 20;

    mapping(address => uint256) whitelistMints;
    mapping(address => uint256) publicMints;

    address signingAddress = 0x339fb5c406Ecb3D2384d06ECe2CfB3d3BB8271FF;
    ...
    constructor() ERC721A("CockFight", "CKF") {}

    function setMintingPhase(uint256 newMintPhase) external onlyOwner {
        mintPhase = newMintPhase;
    }

    function publicMintCock(uint256 quantity) external payable nonReentrant isPerson {
        ...
        require(msg.value == quantity*mintPricePublic, "Invalid ether amount sent.");
    ...
    function whitelistMintCock(uint256 quantity, bytes memory signature) external payable nonReentrant isPerson {
        require(mintPhase >= 1, "Whitelist mint unopened.");
        require(quantity > 0, "Quantity must be non-zero.");
        require(quantity <= MAX_PER_TX_WHITELIST, "Mint quantity higher than max.");
        require(msg.value == quantity*mintPriceWhitelist, "Invalid ether amount sent.");
        require(_totalMinted() + quantity <= MAX_SUPPLY, "Max supply reached.");
    ...
    */
    ...
    function revealTheCocks(string memory newBaseURI) external onlyOwner {
    ...
    */
    ...
    function setMintPricePublic(uint256 newPrice) external onlyOwner {
        mintPricePublic = newPrice;
    }

    function setMintPriceWhitelist(uint256 newPrice) external onlyOwner {
        mintPriceWhitelist = newPrice;
    }
    ...
    //signature is generated offchain
    function verifyAddressSigner(bytes memory signature) private view returns (address) {
        bytes32 hash = keccak256(abi.encodePacked(msg.sender));
        bytes32 message = ECDSA.toEthSignedMessageHash(hash);
        address receivedAddress = ECDSA.recover(message, signature);
