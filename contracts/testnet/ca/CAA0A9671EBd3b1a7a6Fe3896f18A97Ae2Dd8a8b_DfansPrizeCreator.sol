...
import "./DfansPrize.sol";

contract DfansPrizeCreator is Ownable {
  event ContractDeployed(address indexed owner, address indexed prizeAddress);
  ...
  function createDfansPrize(
      ...
      DfansPrize dp = DfansPrize(clone);
      dp.initialize(name, symbol, collectionId, royaltyFraction, beneficiary, royaltyBeneficiary, owner, initialPublish, fixedPrice);
...
contract DfansPrize is ERC721, ERC721Royalty, ERC721Pausable, Ownable, Initializable, DefaultOperatorFilterer {
    ...
    address payable private _royalTyBeneficiaryAddress;

    // all prize member set
    EnumerableSet.AddressSet internal prizeMemberSet;
    // last user address before release
    address payable private lastMintUser;
    ...
    function royalTyBeneficiary() public view returns (address payable) {
        return _royalTyBeneficiaryAddress;
    }

    // 返回本期奖池包含的钱包地址
    function getPrizeMemberSet() public view returns (address[] memory) {
        uint256 size = prizeMemberSet.length();
        address[] memory values = new address[](size);
        for (uint256 i = 0; i < size; i++) {
            values[i] = prizeMemberSet.at(i);
        }
        return values;
    }

    // 返回本期奖池最后一个mint的钱包地址
    function getLastMintUser() public view returns (address payable) {
        return lastMintUser;
    }

    // 返回本期奖池开奖用户数量
    function getPrizeUserNum() public view returns (uint256) {
        return prizeMemberSet.length();
    }


    // initial prize instance
    function initialize(
        string memory name_,
        string memory symbol_,
        string memory collectionId,
        ...
        _royalTyBeneficiaryAddress = _royaltyBeneficiary;
        _setDefaultRoyalty(_royaltyBeneficiary, royaltyFraction);
    ...
    /// @notice set the royalty beneficiary
    function setRoyaltyBeneficiary(address payable _beneficiary, uint96 royaltyFraction) public onlyOwner {
        // mint beneficiary and royalty receiver are same
        _royalTyBeneficiaryAddress = _beneficiary;
    ...
    function safeMint(address to, uint256 tokenId) public onlyOwner {
        ...
        prizeMemberSet.add(to);
        lastMintUser = payable(to);
    }
    ...
    function safePublish(uint256 quantity, uint256 price) public whenNotPaused onlyOwner {
        ...
        require(price >= 0, "quantity can not less than 0");
    ...
    function release() public onlyOwner { 
        ...
        require(balance != 0, "insufficient balance to release");

        uint256 prizeUserNum = prizeMemberSet.length();
        require(prizeUserNum != 0, "no mint user to prize");

        // 8 digital ether value at least
        uint256 halfPrizePool = balance / 2;
        uint256 unitPayment = halfPrizePool / 1e10 / prizeUserNum * 1e10;
        require(unitPayment != 0, "insufficient balance to release");

        
        // holders will get unitPayment for every nft
        for (uint256 i = 0; i < prizeUserNum; i++) {
            address prizeUser = prizeMemberSet.at(i);
            if (prizeUser != address(0)) {
                unchecked {
                    _transfer(prizeUser, unitPayment);
                    _released[prizeUser] += unitPayment;
                    totalReleased += unitPayment;
                }
            }
        }
        
        // transfer halfPrizePool to last mint user
        _transfer(lastMintUser, halfPrizePool);
        totalReleased += halfPrizePool;
        _released[lastMintUser] += halfPrizePool;

        delete prizeMemberSet;
    }
