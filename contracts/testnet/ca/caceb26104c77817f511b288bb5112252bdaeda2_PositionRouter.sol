...
contract PositionRouter is BaseRouter, IPositionRouter, ReentrancyGuard {
    ...
    function openNewPosition(
        ...
        //Scope to avoid stack too deep error
        {
            ...
            require(position.owner == address(0) || position.owner == msg.sender, "FBD: Invalid owner");
    ...
    */
    ...
    function _modifyPosition(
        ...
        if (!_isFastExecute) {
            ...
            if (_txType == ADD_COLLATERAL || _txType == REMOVE_COLLATERAL) {
                ...
                data = abi.encode(_params, position);
            } else if (_txType == UPDATE_TRAILING_STOP) {
                data = abi.encode(positionKeeper.getOrder(_key));
                ...
                data = abi.encode(position, positionKeeper.getOrder(_key));
            } else {
    ...
    function _createPrepareTransaction(
        ...
        bytes32 key = _getPositionKeyV2(_account, _getFirstPath(_path), _isLong, _posId);
