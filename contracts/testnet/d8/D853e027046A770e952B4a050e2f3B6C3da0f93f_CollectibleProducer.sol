...
contract CollectibleProducer is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  // TODO: might make this uint256[] calldata _realmIds to allow
  // batch collect
  function collect(
    uint256 _realmId,
    ...
    require(
      _collectibleIds.length <= maxCollectibles,
      "CollectibleProducer: Above max Collectibles"
    );

    // Check if reactor is productive
    require(
      STRUCTURE_STAKER.hasStaked(_realmId, address(REACTOR), 0),
      "CollectibleProducer: Not productive"
    );

    // Check if product is initialized
    require(
      PRODUCTION.isActived(_realmId),
      "CollectibleProducer: You must start production first"
    );

    uint256 j = 0;
    uint256 length = _collectibleIds.length;
    ...
    for (; j < length; j++) {
      ...
      // Collect
      _collect(_realmId, collectibleId, desiredQuantity, startedAt);
    }
  }
