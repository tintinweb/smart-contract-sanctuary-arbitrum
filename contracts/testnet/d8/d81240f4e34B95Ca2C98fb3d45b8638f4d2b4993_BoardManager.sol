...
contract BoardManager is Ownable, IBoardManager {
    ...
    function ensureValidStage(GameStage stage, uint256 boardId) internal {
    ...
    function canRaise(
        ...
        return
            ...
            boards[boardId].nextPlayer == playerIndex &&
            playerStatuses[boardId][permanentAccount].folded &&
            amount >= amountToCall(permanentAccount, boardId) + boards[boardId].minRaise &&
            boards[boardId].chips[playerIndex] >= amount &&
    ...
    function _performBetWithAmount(
        ...
        if (betType == BetType.Call) {
            ...
            // @todo: The minimal raise size should be larger than delta betting size
            // uint256 deltaBetSize = boards[boardId].bets[playerIndex] - highestBet(boardId);
