...
contract Exploration is Initializable, ExplorationSettings {
    ...
    contractsAreSet
    {
    	...
    	emit StartedExploring(_tokenId, block.timestamp);
    }
    ...
    contractsAreSet
    {
        ...
        emit StoppedExploring(_tokenId);
    }
}
...
abstract contract ExplorationContracts is Initializable, ExplorationState {
	...
	function setContracts(
        address _lostLandAddress,
        address _barnAddress)
    external
    onlyAdminOrOwner
    {
        lostLand = ILostLand(_lostLandAddress);
    ...
    function areContractsSet() public view returns(bool) {
        return address(lostLand) != address(0)
...
import "../interfaces/ILostLand.sol";
import "../interfaces/IBarn.sol";

abstract contract ExplorationState is Initializable, IExploration, AdminableUpgradeable {
	event StartedExploring(uint256 _tokenId, uint256 _timestamp);
	event ClaimedBarn(uint256 _tokenId, uint256 _barnId, uint256 _timestamp);
	event StoppedExploring(uint256 _tokenId);

	ILostLand public lostLand;
	IBarn public barn;

	mapping(uint256 => uint256) tokenIdToLastClaimedTime;
	mapping(address => mapping(uint256 => uint256)) ownerToTokenIdToTotalBarnsClaimed;
...
interface IExploration {
	...
	function startExploring(uint256 _tokenId) external;

	function stopExploring(uint256 _tokenId) external;
}
