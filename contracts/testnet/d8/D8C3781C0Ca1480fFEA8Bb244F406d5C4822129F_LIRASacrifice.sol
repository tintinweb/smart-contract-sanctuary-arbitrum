...
contract LIRASacrifice is Context {
    ...
    // reward tokens reedemed
    uint public redeemed = 0;
    ...
    function round() public view canSacrifice returns (SacrificeRound memory round_) {
        for (uint i = 0; i < _rounds.length; i++) {
            if (block.timestamp >= _rounds[i].start && block.timestamp <= _rounds[i].end) {
                ...
                console.log('_rounds[i]', _rounds[i].end);
                return _rounds[i];
            }
        }
    }
    ...
    function sacrificable() public view canSacrifice returns (uint) {
        return round().sacrificable;
    }

    function bonus() public view canSacrifice returns (uint) {
        return round().bonus;
    }
    ...
    function sacrifice(uint amount_) public canSacrifice {
        ...
        require(round().sacrificable - amount_ >= 0, 'LIRA_SACRIFICE_ROUND_SOLD_OUT');

        IERC20(sacrifiedToken).transferFrom(_msgSender(), address(this), amount_);

        _sacrifices.push(Sacrifice(_msgSender(), amount_.add(amount_.div(100).mul(bonus())), false));
    ...
    function redeem(address to) public canRedeem {
        ...
        uint liraBalance = IERC20(redeemToken).balanceOf(address(this));
        uint liraSupply = redeemed.add(liraBalance);
        ...
        IERC20(redeemToken).transferFrom(address(this), to, _sacrifices[sacrificeIndex].amount);
    ...
    function start(address sacrifiedToken_, address redeemToken_, uint[] memory bonuses_, uint duration_, uint limit_) public onlyShaman isNotStarted {
        ...
        for (uint i = 0; i < bonuses_.length; i++) {
            ...
            // console.log('push %s %s - %s', i, startTime, endTime);
            _rounds.push(SacrificeRound(bonuses_[i], limit_, startTime, endTime));
        }
    }
}
