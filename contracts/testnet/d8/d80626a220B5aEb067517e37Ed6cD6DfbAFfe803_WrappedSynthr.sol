...
contract BaseWrappedSynthr is MixinResolver, Owned {
    ...
    function destBurn(address _account, bytes32 _synthKey, uint256 _synthAmount) external onlySynthrBridge returns (bool) {
    ...
    function collateralTransfer(
        ...
        uint256 _usdAmount,
        uint16 _destChainId
    ) internal systemActive returns (bool) {
        if (_destChainId == 0) {
    ...
    function liquidateDelinquentAccount(
        ...
        if (totalRedeemed > 0) {
            ...
            bool liquidatorRewardTransferSucceeded = collateralTransfer(account, collateralKey, totalRedeemed, destChainId);
    ...
    function liquidateSelf(bytes32 collateralKey, uint16 destChainId) external payable systemActive returns (bool) {
        ...
        bool success = collateralTransfer(msg.sender, collateralKey, totalRedeemed, destChainId);
        ...
        _burn(msg.sender, collateralAmount, collateralKey);
...
contract WrappedSynthr is BaseWrappedSynthr {
    ...
    function chainBalanceOfPerKey(address _account, bytes32 _collateralKey, uint16 _destChainId) external view returns (uint256) {
        (uint256 collateralRate, ) = exchangeRates().rateAndInvalid(_collateralKey);
        return
