...
contract CoinFlip is Ownable, Pausable {
    ...
    uint256 private requiredConfirmations = 2; // Required block confirmations before VRF request is considered final.
    address public supraRouterAddress; // SupraOracle's router address for VRF.
    address public clientWalletAddress; // Transpare's wallet address that pays for VRF service.
    ...
    mapping(uint256 => uint8) public activeGames;
    mapping(uint256 => bool) public isGameActive; // Maps the VRF nonce to bool if game is active.

    // Events to log key contract actions
    event CoinFlipGameStarted(uint256 nonce, uint8 userGuess);
    event CoinFlipGameResult(
        uint256 nonce,
        uint8 userGuess,
        uint8 result,
        uint256 multiplier
    ...
    function play(
        ...
        uint8 userGuess = uint8(_betParameters[0]);
        require(
            userGuess == HEADS || userGuess == TAILS,
            "Must be HEADS (0) or TAILS (1)"
        );

        // No need to check for duplicated nonces. SupraRouter generated nonces are always unique
        uint256 nonce = ISupraRouter(supraRouterAddress).generateRequest(
        ...
        activeGames[nonce] = userGuess;
        isGameActive[nonce] = true;

        emit CoinFlipGameStarted(nonce, userGuess);
     ...
     * @param nonce Unique identifier for the game round provided by the SupraRouter.
     * @param rngList Random numbers from VRF.
     */
    function handleVRFResult(
        uint256 nonce,
        uint256[] calldata rngList
    ) external onlySupraRouter whenNotPaused {
        require(rngList.length == 1, "Expecting a single random number");
        require(isGameActive[nonce], "Game does not exist");

        uint8 userGuess = activeGames[nonce];
        uint8 coinResult = uint8(rngList[0] % 2);
        uint256 multiplier = (userGuess == coinResult) ? WIN_MULTIPLIER : 0;

        emit CoinFlipGameResult(nonce, userGuess, coinResult, multiplier);

        delete activeGames[nonce];
        delete isGameActive[nonce];

        ICasinoBase(casino).notifyGameResult(nonce, multiplier);
    }
