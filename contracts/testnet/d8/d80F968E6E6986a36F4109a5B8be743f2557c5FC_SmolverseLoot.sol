...
contract UtilitiesV3Upgradeable is Initializable, AccessControlEnumerableUpgradeable, PausableUpgradeable {
...
import {SmolsLibrary} from "../../shared/SmolsLibrary.sol";

//TODO
//We are going to want to limit transferability of this collection ONLY through marketplace listings - done
//Create script to generate merkle trees/proofs - done
//Test merkle features - donish
//Add script to upload all loots - done
...
contract SmolverseLoot is Initializable, SmolverseLootAdmin, ERC721Upgradeable {
    ...
    function craftRainbowTreasures(
        ...
        for (uint256 index = 0; index < _rainbowTreasureCraftInputs.length; index++) {
            craftRainbowTreasures(_rainbowTreasureCraftInputs[index]);
        }
    }

    function craftRainbowTreasures(RainbowTreasureCraftInput calldata _rainbowTreasureCraftInputs) internal {
        uint256[] memory tokenIds = _rainbowTreasureCraftInputs.tokenIds;
        if (tokenIds.length != 15) revert InvalidCraft(_rainbowTreasureCraftInputs);
        bool isByShape = _rainbowTreasureCraftInputs.craftType == CraftType.BY_SHAPE;
        ...
        for (uint256 index = 0; index < tokenIds.length; index++) {
            ...
            if (current != 0 && next != current) revert InvalidCraft(_rainbowTreasureCraftInputs);
            current = next;
            _burn(tokenId);
        }
        ISmolTreasures(treasuresAddress).mint(msg.sender, RAINBOW_TREASURE_ID, 1);
    }

    function rerollLoots(uint256[] calldata _tokenIds) external {
    ...
    function convertToLoot(LootConversionInput calldata _lootConversionInput) external {
    ...
    function mintLootsAsAdmin(address _receiver, uint256 _count) external onlyRole(SMOL_LOOT_MINTER_ROLE) {
...
abstract contract SmolverseLootAdmin is Initializable, SmolverseLootContracts {
    ...
    function setTroveAddress(address _troveAddress) external requiresEitherRole(OWNER_ROLE, ADMIN_ROLE){
...
abstract contract SmolverseLootState is Initializable, ISmolverseLoot, UtilitiesV3Upgradeable {
    ...
    bytes32 internal SMOL_LOOT_MINTER_ROLE = keccak256("SMOL_LOOT_MINTER");

    string internal baseURI;
    string internal collectionDescription;
