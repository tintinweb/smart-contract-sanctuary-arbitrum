...
/**
 * @title Contract for MetaTope Auction V4
 * Copyright 2022 MetaTope
 */
contract MetaTopeAuctionV4 is MetaTopeIERC1155Receiver, Ownable {
        ...
        notContract
    {
        ...
        if (oldPrice == 0) {
            ...
            if (bidders[msg.sender].depositToken == _depositToken) {
                int256 priceBalance = 0;
                priceBalance = int256(bidders[msg.sender].totalPrice - _totalPrice);
                if (priceBalance > 0) {
                    // User needs more token, user needs to pay addtional
                    require(_depositToken.transferFrom(msg.sender, address(this), uint256(priceBalance)), "Token not transferred from user");
    ...
    function claim() external onlyEnded {
        ...
        if (wBid.tokenAmount > 0) {
            ...
            uint256 balance = wBid.totalPrice - price;

            if (balance > 0) {
                require(wBid.depositToken.transfer(msg.sender, balance), "Remaining deposit token was not transferred");
            }
        } else {
