...
// File: openzeppelin-contracts\contracts\token\ERC20\extensions\IERC20Metadata.sol
...
// File: contracts\AsteroidDepot.sol

pragma solidity ^0.8.4;
contract AsteroidDepot is ERC20("Staked Asteroid", "sAsteroid"), Ownable, Pausable {
    using SafeERC20 for Asteroid;
    ...
    uint256 public cookedAsteroid; // AST pending withdrawal
    ...
    constructor(Asteroid _asteroid) {
        asteroid = _asteroid;
    }

    // Views

    function asteroidBalance() public view returns (uint256 balance) {
        return asteroid.balanceOf(address(this)) - cookedAsteroid;
    }

    function _unstakeOutput(uint256 _share) internal view returns (uint256 output) {
        uint256 totalShares = totalSupply();
        return _share.mul(asteroidBalance()).div(totalShares);
    }
    ...
    function stake(uint256 _amount) external whenNotPaused {
        ...
        uint256 totalShares = totalSupply();
        // If no sAST exists, mint it 1:1 to the amount put in
        if (totalShares == 0 || asteroidBalance() == 0) {
            _mint(_msgSender(), _amount);
        } else {
            uint256 share = _amount.mul(totalShares).div(asteroidBalance());
            _mint(_msgSender(), share);
        }

        asteroid.transferToAsteroidDepot(_msgSender(), _amount);
    }

    function quickUnstake(uint256 _share) external whenNotPaused {
        // QUICK_UNSTAKE_CONTRIBUTION_PERCENT of the claimable AST will remain in the depot
        ...
        // QUICK_UNSTAKE_BURN_PERCENT of the claimable AST is burned
        uint256 amountSpoiled = unstakeOutput.mul(QUICK_UNSTAKE_BURN_PERCENT).div(100);

        // burn staker's share
        _burn(_msgSender(), _share);

        asteroid.burn(address(this), amountSpoiled);
        asteroid.safeTransfer(_msgSender(), output);
    }

    /**
     * @dev _share argument specified in sAST
     */
    function prepareDelayedUnstake(uint256 _share) external whenNotPaused {
        // calculate output and burn staker's share
        uint256 output = _unstakeOutput(_share); //converts sAST values to AST
        _burn(_msgSender(), _share);
        ...
        cookedAsteroid += output;

        asteroid.burn(address(this), amountSpoiled);
    }

    /**
     * @dev argument specified in AST, not sAST
     */
    function claimDelayedUnstake(uint256 _amount) external whenNotPaused {
        require(block.timestamp >= unlockTimestamps[_msgSender()], "AST not yet unlocked");
        require(_amount <= unlockAmounts[_msgSender()], "insufficient locked balance");

        // deduct from unlocked
        unlockAmounts[_msgSender()] -= _amount;

        cookedAsteroid -= _amount;

        // transfer claim
        asteroid.safeTransfer(_msgSender(), _amount);
    }
