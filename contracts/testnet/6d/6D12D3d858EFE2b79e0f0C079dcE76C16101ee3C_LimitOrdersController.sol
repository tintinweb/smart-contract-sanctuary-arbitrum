    ...
    LimitOrdersStorage
{
    ...
    function setGasParameters(
        uint256 _withdrawGas,
        uint256 _transferGas,
        uint256 _swapGas,
        uint256 _sendGas
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        WITHDRAW_GAS = _withdrawGas;
        TRANSFER_GAS = _transferGas;
        SWAP_GAS = _swapGas;
        SEND_GAS = _sendGas;
    }
    ...
    function sweepTokens(
        ...
        require(_token != address(nftPositionManager), "SWET1");
    ...
    function setSwapFeePercentage(
        ...
        uint256 newSwapFeePercentageAccuracy
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            newSwapFeePercentageAccuracy >= newSwapFeePercentage,
            "STSF1"
        );
        require(
            newSwapFeePercentageAccuracy % 10 == 0,
            "STSF2"
        );
        SWAP_FEE_PERCENTAGE = newSwapFeePercentage;
        SWAP_FEE_PERCENTAGE_ACCURACY = newSwapFeePercentageAccuracy;
        emit SetNewSwapFeePercentage(
            newSwapFeePercentage,
            newSwapFeePercentageAccuracy
        );
    }
    ...
    //     emit SetNewFeeCollector(newFeeCollector);
    // }
