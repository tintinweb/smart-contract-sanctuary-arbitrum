...
library DateTime {
  ...
  function diffDays(
    ...
    require(fromTimestamp <= toTimestamp, "toTimestamp must be or gt eq to fromTimeStamp");
...
contract VestingMock is Access {
  ...
  constructor(IWINR _Winr, IWINR _vWinr, address _admin) Access(_admin) {
    ...
    categoryDetails[Category.WINR_LABS] = CategoryDetail(5, 1080, 150 * 10 ** 24); // 5 min cliff for test
    categoryDetails[Category.MARKETING] = CategoryDetail(0, 720, 70 * 10 ** 24);
    categoryDetails[Category.ADVISORS] = CategoryDetail(0, 1080, 15 * 10 ** 24);
    categoryDetails[Category.PREV_HOLDERS] = CategoryDetail(0, 720, 10 * 10 ** 24);
    categoryDetails[Category.CORE_CONTRIBUTORS] = CategoryDetail(0, 720, 175 * 10 ** 24);
  }
  ...
  function addInvestorBatch(
    ...
    for (uint256 i = 0; i < _tokensAllotments.length; i++) {
      _addInvestor(_winrWallets[i], _vWinrWallets[i], _tokensAllotments[i], _categories[i]);
    }
  }
  ...
  function _addInvestor(
    ...
    require(_tokensAllotment > 0, "addInvestor: the investor allocation must be more than 0");
    require(
      totalAllotments[_category] + _tokensAllotment <= categoryDetails[_category].tokenSupply,
      "addInvestor: allotment greater than available supply"
    );
  ...
  function withdrawTokens() external onlyInvestor initialized {
    ...
    if (investor.winrWallet != address(0) && investor.vWinrWallet != address(0)) {
      WINR.mint(investor.winrWallet, tokensAvailable / 2);
      vWINR.mint(investor.vWinrWallet, tokensAvailable / 2);

      emit Withdrawn(msg.sender, tokensAvailable / 2, tokensAvailable / 2);
    } else {
      if (investor.winrWallet != address(0)) {
        WINR.mint(investor.winrWallet, tokensAvailable);
        emit Withdrawn(msg.sender, tokensAvailable, 0);
      } else {
        vWINR.mint(investor.vWinrWallet, tokensAvailable);
  ...
  function _calculateUnlockedTokens(
    ...
    uint256 cliffTimestamp = initialTimestamp + categoryDetail.cliffDays * 1 minutes;
    uint256 vestingTimestamp = cliffTimestamp + categoryDetail.recurrence * 1 minutes;
    ...
    if (currentTimeStamp > initialTimestamp) {
      if (currentTimeStamp <= cliffTimestamp) {
        ...
        uint256 occurence = DateTime.diffMins(cliffTimestamp, currentTimeStamp);
