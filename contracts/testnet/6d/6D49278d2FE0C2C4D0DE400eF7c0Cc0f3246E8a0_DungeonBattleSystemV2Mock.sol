...
uint256 constant ID = uint256(
    keccak256("game.piratenation.isenemycomponent.dev1")
);
...
library IsEnemyComponentStorage {
    bytes32 internal constant STORAGE_SLOT = bytes32(ID);
...
/**
 * @title IsEnemyComponent
 * @dev Is this entity an enemy?
 */
contract IsEnemyComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Whether or not this entity is an enemy
        keys[0] = "value";
     ...
     *
     * @param entity Entity to get value for
     * @param value Whether or not this entity is an enemy
     */
    function setValue(
        uint256 entity,
        bool value
    ...
    function getLayoutValue(
        ...
        // Get the struct from storage
        value = IsEnemyComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return value Whether or not this entity is an enemy
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (bool value) {
        if (has(entity)) {
            Layout memory s = IsEnemyComponentStorage.layout().entityIdToStruct[
                entity
            ];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = IsEnemyComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        // Remove the entity from the component
        delete IsEnemyComponentStorage.layout().entityIdToStruct[entity];
        _emitRemoveBytes(entity);
    }
    ...
    function batchRemove(
        ...
        // Remove the entities from the component
        for (uint256 i = 0; i < entities.length; i++) {
            delete IsEnemyComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
        _emitBatchRemoveBytes(entities);
