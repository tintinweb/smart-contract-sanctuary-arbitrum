...
error Cassette_ExceedsMaxChapter();
...
contract Cassette is ONFT1155Core, ERC1155, IONFT1155, ICassette {
    ...
    {
        if (_chainOffset >= _totalChains) revert Cassette_InvalidConfiguration();

        MAX_CHAPTER = _maxChapter;
    ...
    /**
     * @notice Modifier requiring the sender to be the replicator contract
     */
    modifier onlyReplicator() {
        if (msg.sender != address(replicator)) revert Cassette_NotAllowed();
        _;
    }
    ...
    /**
     * @notice Calculate chapter number of a specified token ID
     * @param _tokenId Token ID to return the chapter number of
     * @return uint256 A token's corresponding chapter number
     */
    function chapter(uint256 _tokenId) public view returns (uint256) {
        if (_tokenId > MAX_CHAPTER * TOTAL_CHAINS) revert Cassette_ValueOutOfRange();

        return _tokenId == 0 ? 0 : (_tokenId + TOTAL_CHAINS - 1) / TOTAL_CHAINS;
    }

    /**
     * @notice Calculate token ID of a specified chapter
     * @dev This will vary depending on which chain this contract is deployed to
     * @param _chapter Chapter to return the token ID of
     * @return uint256 A chapter's corresponding token ID
     */
    function tokenId(uint256 _chapter) public view returns (uint256) {
        if (_chapter > MAX_CHAPTER) revert Cassette_ValueOutOfRange();

        return _chapter == 0 ? 0 : ((_chapter - 1) * TOTAL_CHAINS) + CHAIN_OFFSET + 1;
    }
     ...
     */
    function setReplicator(IReplicator _replicator) public payable lockable onlyOwner {
    ...
    /**
     * @notice Set the current maximum chapter
     * @param _currentMaxChapter Current maximum chapter
     */
    function setCurrentMaxChapter(uint256 _currentMaxChapter) public payable lockable onlyOwner {
        if (_currentMaxChapter > MAX_CHAPTER) revert Cassette_ExceedsMaxChapter();

        currentMaxChapter = _currentMaxChapter;
    }

    /**
     * @notice Close cassette burning
     */
    function closeBurn() public payable lockable onlyOwner {
        burnClosed = true;
    }

    /**
     * @notice Close cassette minting
     */
    ...
    /**
     * @notice Mint a specified number of a specific cassette using a replicator
     * @param _to Address receiving the cassettes
     * @param _chapter Cassette chapter to mint
     * @param _amount Amount of cassettes to mint
     */
    function replicatorMint(address _to, uint256 _chapter, uint256 _amount) public payable onlyReplicator {
        _mint(_to, _chapter, _amount);
    }
    ...
    function mint(uint256 _amount) public payable {
        if (mintClosed) revert Cassette_MintClosed();
     ...
     * @param _to Address receiving the cassettes
     * @param _chapter Chapter to mint
     * @param _amount Amount of cassettes to mint
     */
    function _mint(address _to, uint256 _chapter, uint256 _amount) internal {
        if (_chapter > MAX_CHAPTER) revert Cassette_ExceedsMaxChapter();

        _mint(_to, tokenId(_chapter), _amount, "");
    }

    /**
     * @notice Burn a token to receive the next chapter in the series
     * @param _tokenId Token to burn
     * @param _amount Amount of tokens to burn
     */
    function burn(uint256 _tokenId, uint256 _amount) public payable {
        if (burnClosed) revert Cassette_BurnClosed();

        uint256 _chapter = chapter(_tokenId);

        if (_chapter + 1 > currentMaxChapter) revert Cassette_ExceedsMaxChapter();

        _burn(msg.sender, _tokenId, _amount);
        _mint(msg.sender, _chapter + 1, _amount);
    }

    /**
     * @notice Burn a batch of tokens to receive the next chapter in the series
        ...
        payable
    {
        ...
        for (uint256 i; i < _tokenIds.length;) {
            uint256 _chapter = chapter(_tokenIds[i]);

            if (_chapter + 1 > currentMaxChapter) revert Cassette_ExceedsMaxChapter();

            _burn(msg.sender, _tokenIds[i], _amounts[i]);
            _mint(msg.sender, _chapter + 1, _amounts[i]);
    ...
    /**
     * @notice Burn a token on the current chain to receive the next chapter in the series on a destination chain
     ...
     * @param _toAddress Address to receive the next chapter in the series
     * @param _tokenId Token to burn
     * @param _amount Amount of tokens to burn
    ...
    {
        if (burnClosed) revert Cassette_BurnClosed();
        if (chapter(_tokenId) + 1 > currentMaxChapter) revert Cassette_ExceedsMaxChapter();
    ...
    /**
     * @notice Burn a batch of tokens on the current chain to receive the next chapters in the series on a destination chain
     * @dev For more details see https://layerzero.gitbook.io/docs/evm-guides/master
     * @param _from Address to burn tokens from
     * @param _dstChainId Destination chain's LayerZero ID
     * @param _toAddress Address to receive the next chapters in the series
     * @param _tokenIds Tokens to burn
     * @param _amounts Amounts of tokens to burn
     * @param _refundAddress Address to send refund to if transaction is cheaper than expected
     * @param _zroPaymentAddress Address of $ZRO token holder that will pay for the transaction
        ...
        payable
    {
        ...
        for (uint256 i; i < _tokenIds.length;) {
            if (chapter(_tokenIds[i]) + 1 > currentMaxChapter) revert Cassette_ExceedsMaxChapter();
            unchecked { ++i; }
        }
        ...
        internal
    {
        ...
        if (_functionType == FUNCTION_TYPE_SEND || _functionType == FUNCTION_TYPE_SEND_BATCH) {
            for (uint256 i; i < _tokenIds.length;) {
                if (chapter(_tokenIds[i]) > 0) revert Cassette_InvalidSendData();
                unchecked { ++i; }
            }
        }
    ...
    /**
     * @notice Bump token IDs to those corresponding to the next chapter in the series
     * @param _tokenIds Tokens to bump to the next chapter
     * @return uint256[] Tokens corresponding to the next chapter in the series
     */
    function _bumpTokenIds(uint256[] memory _tokenIds) internal view returns (uint256[] memory) {
        for (uint256 i; i < _tokenIds.length;) {
            unchecked {
                _tokenIds[i] = tokenId(chapter(_tokenIds[i]) + 1);
                ++i;
            }
        }
