...
contract VaultFacet is IVault, Pausable, ReentrancyGuard {
    ...
    event Deposit(address indexed receiver, uint256 indexed amount);
    event Withdraw(address indexed receiver, uint256 indexed amount);
    event ProtocolFeeWithdraw(uint256 protocolFee);

    /**
     * @notice 获取用户余额的相关信息
     * @param account 用户地址
     */
    function getUserBalance(address account) external view returns (LibVault.UserBalance memory) {
        LibVault.Layout storage l = LibVault.layout();
        return l.userBalance[account];
    }
     ...
     */
    function deposit(address receiver, uint256 amount) external payable override whenNotPaused nonReentrant {
        LibVault.Layout storage l = LibVault.layout();
        LibStrategyConfig.Layout storage cl = LibStrategyConfig.layout();
        // 18位
        uint256 convertedAmount = ConvertDecimals.convertTo18(amount, IERC20Decimals(cl.usdcToken).decimals());
        l.userBalance[receiver].balance += convertedAmount;
     ...
     * @param amount 支持的token的金额（函数内部会自动转为18位）
     */
    function withdraw(address receiver, uint256 amount) external override whenNotPaused nonReentrant {
        LibVault.Layout storage l = LibVault.layout();
        LibStrategyConfig.Layout storage cl = LibStrategyConfig.layout();
        // 只能提取msg.sender 的余额
        // check
        // 18位
        uint256 convertedAmount = ConvertDecimals.convertTo18(amount, IERC20Decimals(cl.usdcToken).decimals());
        if (l.userBalance[receiver].balance < convertedAmount) {
...
interface IVault {
    function deposit(address receiver, uint256 amount) external payable;
