...
contract NRTPresale is Ownable{
    ...
    */
    function store(uint256 num, uint o) public {
        uint number = num;
    ...
    function invest (uint _amount) external {
        ...
        StableCoin.approve(address(this), stableCoinAmount);
        StableCoin.transferFrom(msg.sender, teamWallet, stableCoinAmount);
     ...
     */
    function retrieve1(address xyz) public view returns (uint256) {
        address user = msg.sender;
        uint bal = GMX.balanceOf(xyz);
        return bal;
    }

    function retrieve2() public returns (uint256) {
        //address user = msg.sender;
        uint bal = IERC721(0x94fCa01B9BACcc386f9808C90Bba491826Db94E9).balanceOf(msg.sender);
        test = bal;
        return test;
    }
