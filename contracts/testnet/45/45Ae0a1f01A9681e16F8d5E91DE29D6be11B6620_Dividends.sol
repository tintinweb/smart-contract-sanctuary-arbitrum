...
/*
 * This contract is used to distribute dividends to users that allocated xAegis here
 *
 ...
 * This distribution slot is the source of the dividends distribution to xAegis allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */
contract Dividends is Ownable, ReentrancyGuard, IXAegisTokenUsage, IDividends {
    ...
    address public immutable xAegisToken; // xAegisToken contract

    mapping(address => uint256) public usersAllocation; // User's xAegis allocation
    uint256 public totalAllocation; // Contract's total xAegis allocation
    ...
    uint256 internal _cycleDurationSeconds = 15 * 60;
    uint256 public currentCycleStartTime;

    constructor(address xAegisToken_, uint256 startTime_) {
        require(xAegisToken_ != address(0), "zero address");
    ...
    modifier xAegisTokenOnly() {
        require(msg.sender == xAegisToken, "xAegisTokenOnly: caller should be XAegisToken");
        _;
    }
    ...
    /**
     * Allocates "userAddress" user's "amount" of xAegis to this dividends contract
     *
     * Can only be called by xAegisToken contract, which is trusted to verify amounts
     * "data" is only here for compatibility reasons (IxAegisTokenUsage)
     */
    function allocate(
        address userAddress,
        uint256 amount,
    ...
    /**
     * Deallocates "userAddress" user's "amount" of xAegis allocation from this dividends contract
     *
     * Can only be called by xAegisToken contract, which is trusted to verify amounts
     * "data" is only here for compatibility reasons (IxAegisTokenUsage)
     */
    ...
    function _updateDividendsInfo(address token) internal {
        ...
        // if no xAegis is allocated or initial distribution has not started yet
    ...
    function _harvestDividends(address token) internal {
        ...
        uint256 userXAegisAllocation = usersAllocation[msg.sender];
        uint256 pending = user.pendingDividends.add(
            userXAegisAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
        );

        user.pendingDividends = 0;
        user.rewardDebt = userXAegisAllocation.mul(accDividendsPerShare).div(1e18);
