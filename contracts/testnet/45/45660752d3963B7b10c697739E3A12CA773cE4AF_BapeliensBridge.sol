...
contract BapeliensBridge is Pausable, Ownable, ReentrancyGuard {
    /**
     * Events
     */
    ...
    address public bapelienAddress;
    ...
    constructor(address bapelienAddress_) {
        bapelienAddress = bapelienAddress_;
        _pause();
    }
    ...
    function bridge(uint256[] memory tokenIds) public whenNotPaused nonReentrant {
        require(bapelienAddress != address(0), "INVALID_ADDRESS");

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(IERC721(bapelienAddress).ownerOf(tokenIds[i]) == msg.sender, "NOT_OWNER");

            bridgedOwner[tokenIds[i]] = msg.sender;
            IERC721(bapelienAddress).safeTransferFrom(msg.sender, address(this), tokenIds[i]);
        }
    ...
    function setBapeliensAddress(address newBapeliensAddress) public onlyOwner {
        bapelienAddress = newBapeliensAddress;
    }
    ...
    function emergencyReturn(uint256 tokenId) public onlyOwner {
        require(bapelienAddress != address(0), "INVALID_ADDRESS");
        require(IERC721(bapelienAddress).ownerOf(tokenId) == address(this), "NOT_IN_CONTRACT");
        require(bridgedOwner[tokenId] != address(0), "NO_OWNER");
        IERC721(bapelienAddress).safeTransferFrom(address(this), bridgedOwner[tokenId], tokenId);
    }
