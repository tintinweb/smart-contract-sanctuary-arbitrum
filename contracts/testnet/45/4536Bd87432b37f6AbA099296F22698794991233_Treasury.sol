...
contract Treasury is OwnableUpgradeable {
    ...
    uint256 public mintRewardsAmount = 10000;
    ...
    function distributionRewards(address rewardPool_, uint256 ratio_) external onlyOwner {
        require(rewardPool_ != address(0), "Treasury: reward pool is zero address");
        require(ratio_ > 0, "Treasury: ratio is zero");
        uint256 rewardAmount_ = rewardsAmount.div(100).mul(ratio_);
        if (rewardAmount_ > 0) {
            IERC20Metadata(rewardsToken).transfer(rewardPool_, rewardAmount_);
        }
    }
