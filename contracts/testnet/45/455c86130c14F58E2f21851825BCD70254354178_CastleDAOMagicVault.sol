...
import "./interfaces/ICastleDAOMagicVault.sol";
import "./interfaces/IAtlasMine.sol";

contract CastleDAOMagicVault is
    ICastleDAOMagicVault,
    Initializable,
    OwnableUpgradeable,
    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        require(_magic != address(0), "CastleDAOVault: invalid address");
        require(_atlasStaker != address(0), "CastleDAOVault: invalid address");
        require(_treasury != address(0), "CastleDAOVault: invalid address");
    ...
    {
        require(_amount > 0, "CastleDAOVault:AMOUNT_MUST_BE_GREATER_THAN_0");
        ...
        IBattleflyAtlasStakerV02.VaultStake memory vaultStake = ATLAS_STAKER
            .getVaultStake(atlasStakerDepositId);

        UserStake storage userStake = userStakes[atlasStakerDepositId];
        userStake.amount = _amount;
        userStake.lockAt = vaultStake.lockAt;
        userStake.owner = _msgSender();
        userStake.lock = _lock;

        depositIdByUser[_msgSender()].add(atlasStakerDepositId);

        emit NewUserStake(
            atlasStakerDepositId,
            _amount,
            vaultStake.unlockAt,
            _msgSender(),
            _lock
    ...
    function withdrawAll()
        ...
        returns (uint256 amount)
    {
        uint256[] memory depositIds = depositIdByUser[_msgSender()].values();
        require(depositIds.length > 0, "CastleDAOVault: No deposited funds");
    ...
    function requestWithdrawal(uint256[] memory _depositIds) public override {
        for (uint256 i = 0; i < _depositIds.length; i++) {
            ...
            require(
                userStake.owner == _msgSender(),
                "CastleDAOVault: caller is not the owner"
            );
    ...
    function getAllDepositIds(address _to)
        public
        view
        override
        returns (uint256[] memory deposits)
    {
        uint256[] memory depositIds = depositIdByUser[_to].values();
        return depositIds;
    }
    ...
    function _withdraw(uint256 _depositId) internal returns (uint256 amount) {
        ...
        require(
            ...
            "CastleDAOVault: stake not yet unlocked"
        );

        amount = ATLAS_STAKER.withdraw(_depositId);
        MAGIC.safeTransfer(_msgSender(), amount);

        depositIdByUser[_msgSender()].remove(_depositId);
    ...
    function _claim(uint256 _depositId) internal returns (uint256 emission) {
        ...
        uint256 treasuryEmission = (emission * 1000) / 10000;
        uint256 userEmission = (emission * 9000) / 10000;

        MAGIC.safeTransfer(_msgSender(), userEmission);
        MAGIC.safeTransfer(TREASURY, treasuryEmission);
    ...
    /**
     * @dev Remove funds sent to the contract
     */
    function ownerWithdraw() external onlyOwner {
        MAGIC.safeTransfer(owner(), MAGIC.balanceOf(address(this)));
    ...
    function getAllowedLocks()
        ...
        view
        override
        returns (IAtlasMine.Lock[] memory)
    {
    ...
    function getUserStake(uint256 _depositId)
        public
        view
        override
        returns (UserStake memory)
    {
        return userStakes[_depositId];
    }
...
interface ICastleDAOMagicVault {
    struct UserStake {
        uint64 lockAt;
        uint256 amount;
        address owner;
    ...
    function getAllDepositIds(address) external view returns (uint256[] memory);

    function getUserStake(uint256) external view returns (UserStake memory);
