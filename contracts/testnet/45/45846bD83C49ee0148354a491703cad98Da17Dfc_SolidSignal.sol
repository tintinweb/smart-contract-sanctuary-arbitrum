...
/**
 * @title SolidSignal
 * @dev Presale fair launch contract for SolidSignal token
 */
contract SolidSignal is Ownable {
    // Maximum number of tokens to be sold in presale
    uint256 public immutable presaleCap;

    uint256 minimumPurchaseAmount; //6 decimal places
    ...
    // Flag to indicate if token allocation is allowed
    bool public allowAllocation;
    ...
    constructor(
        ...
        require(
            _treasury != address(0),
            "SolidSignal: Requires a non zero address"
        );
     ...
     */
    function buyFor(uint256 amount, address account) external {
        _buy(amount, account);
    }

    ///@ooglec remove this function as it's no longer needed after declaring balances mapping as public
    /**
    ...
    function _buy(uint256 amount, address account) internal {
        ...
        require(isOpen, "SolidSignal: Presale is not open");
        require(!isClosed, "SolidSignal: Presale is closed");
        require(amount >= minimumPurchaseAmount, "SolidSignal: Amount is low");
        ...
        require(
            ...
            "SolidSignal: Not enough tokens left for sale"
        );
        // Check if buyer has sufficient usdcToken balance
        require(
            usdcToken.balanceOf(account) >= amount,
            "SolidSignal: Insufficient funds"
        ); ///This is a redundant check. You already check the boolen response of transferFrom hence remove this
    ...
    function start() external onlyOwner {
        ...
        require(!isOpen, "SolidSignal: sale is already open");
    ...
    function endSale() external onlyOwner {
        ...
        require(!isClosed, "SolidSignal: sale is already over");
    ...
    function withdraw() external onlyOwner {
        ...
        require(balance > 0, "SolidSignal: balance withdrawn");
        ...
        bool transferedToTreasury = usdcToken.transfer(
            treasury,
            treasuryAmount
        );
        bool transferedToAdmin = usdcToken.transfer(msg.sender, ownerAmount);
        require(transferedToAdmin);
        require(transferedToTreasury);
    }
}