...
contract BidFomo is Caller, ReentrancyGuard {
    ...
    address public uniswapPool =
        address(0x13211bA19a57F80fcCBD1Ae1eB6FCAAA20CFbd96);
    ...
    function bid(string memory message) public {
        ...
        (uint256 curBiddingAmount, uint256 bidPrice) = getCurrentBidAmount();
        require(
    ...
    function getTickConsult(
        uint32 secondsAgo
    ) public view returns (int24 arithmeticMeanTick) {
        (arithmeticMeanTick, ) = OracleLibrary.consult(uniswapPool, secondsAgo);
    }
    ...
    {
        secondsAgo = OracleLibrary.getOldestObservationSecondsAgo(uniswapPool);
    }

    function getTick(uint256 amount) public view returns (uint256) {
    ...
    function getTick2(uint256 amount) public view returns (uint256) {
        (, int24 tick, , , , , ) = IUniswapV3Pool(uniswapPool).slot0();
        uint256 amountOut = OracleLibrary.getQuoteAtTick(
            tick,
            uint128(amount),
    ...
    {
        bidPrice = (pool.bidding * (1000 + bidPriceGrowthRate)) / 1000;
        amount = (bidPrice * (10 ** 8)) / getBOBOPrice();
