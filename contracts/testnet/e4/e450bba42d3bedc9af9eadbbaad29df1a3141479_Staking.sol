...
contract Staking is Ownable, ReentrancyGuard, Referral {
    ...
    uint256 private latestOrderId = 0; // A
    uint256 public totalStake = 0; // B // keep on increasing    (totalStake-totalWithdrawal) = stake left
    uint256 public totalWithdrawal = 0; // C // keep on icreasing
    uint256 public totalRewardPending = 0; // D // increase/decrease
    uint256 public totalRewardsDistribution = 0; // E

    mapping(uint256 => PoolInfo) public pooldata; // F
    mapping(address => uint256) public balanceOf; // G
    mapping(address => uint256) public totalRewardEarn; // H
    mapping(uint256 => OrderInfo) public orders; // I
    mapping(address => uint256[]) private orderIds; // J
    ...
    function deposit(uint256 _amount, uint256 _lockupDuration, address _referrer) external { // A B D F G I J
        if (_referrer != address(0) && !userInfo[_msgSender()].referred) {
            addReferee(_referrer);
        }

        PoolInfo storage pool = pooldata[_lockupDuration]; // F
        ...
        uint256 requiredToken = (totalStake - totalWithdrawal) + totalRewardPending + userReward; // B // D // (totalStake-totalWithdrawal) = stake left
        require(requiredToken <= token.balanceOf(address(this)), "TokenStakingCIP: insufficient contract balance to return rewards"); // G
        require(token.transferFrom(_msgSender(), address(this), _amount), "TokenStakingCIP: token transferFrom via deposit not succeeded");

        orders[++latestOrderId] = OrderInfo( // A // I
            _msgSender(), // beneficiary
            _amount,
            pool.lockupDuration,
            pool.returnPer,
            block.timestamp, // starttime
            block.timestamp + pool.lockupDuration, // endtime
            0, // claimedReward
            false // .claimed
        );

        totalStake += _amount; // B
        totalRewardPending += userReward; // D
        balanceOf[_msgSender()] += _amount; // G
        orderIds[_msgSender()].push(latestOrderId); // A // J
        emit Deposit(_msgSender(), pool.lockupDuration, _amount, pool.returnPer);
    }

    function withdraw(uint256 orderId) external nonReentrant { // A C D E G H I
        require(orderId <= latestOrderId, "TokenStakingCIP: INVALID orderId, orderId greater than latestOrderId"); // A

        OrderInfo storage orderInfo = orders[orderId]; // I
        ...
        totalRewardEarn[_msgSender()] += claimAvailable; // H
        totalRewardsDistribution += claimAvailable; // E
        totalRewardPending -= claimAvailable; // D
        orderInfo.claimedReward += claimAvailable;

        balanceOf[_msgSender()] -= orderInfo.amount; // G
        totalWithdrawal += orderInfo.amount; // C
    ...
    function emergencyWithdraw(uint256 orderId) external nonReentrant {
        ...
        uint256 claimAvailable = pendingRewards(orderId);
        uint256 fees = (orderInfo.amount * emergencyWithdrawFees) / 100; // double check
        orderInfo.amount -= fees; // double check
        uint256 total = orderInfo.amount + claimAvailable;
        ...
        balanceOf[_msgSender()] -= (orderInfo.amount + fees); // G // double check
        totalWithdrawal += orderInfo.amount; // C
        orderInfo.claimed = true;
    ...
    function claimRewards(uint256 orderId) external nonReentrant { // A C E H I
        require(orderId <= latestOrderId, "TokenStakingCIP: INVALID orderId, orderId greater than latestOrderId"); // A
    ...
    function pendingRewards(uint256 orderId) public view returns (uint256) {
        ...
        if (!orderInfo.claimed) {
            if (block.timestamp >= orderInfo.endtime) {
                ...
                uint256 APY = (orderInfo.amount * orderInfo.returnPer) / 100;
                uint256 reward = (APY * stakeTime) / _days365; // uint256 totalReward = (orderInfo.amount*orderInfo.returnPer*stakeTime)/(10000*86400*365);
    ...
    function claimRefRewards() external nonReentrant {
        Referral_rewards memory ref_rewards = _calculateRefRewards(_msgSender()); // remaining rewards
        ...
        uint256 requiredToken = (totalStake - totalWithdrawal) + totalRewardPending + availableRewards; // B // D // (totalStake-totalWithdrawal) = stake left
    ...
    function _totalRewards(address ref) private view returns (uint256) {
        ...
        for (uint256 i = 0; i < arr.length; i++) {
            ...
            rewards += (order.claimedReward + pendingRewards(arr[i])); // as per current time the total released reward for stake holder
        }
        return rewards;
    }
