...
interface IXlpManager {
    ...
    function addLiquidity(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minXlp) external returns (uint256);
    function addLiquidityForAccount(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdg, uint256 _minXlp) external returns (uint256);
...
contract RewardRouterV2 is IRewardRouterV2, ReentrancyGuard, Governable {
    ...
    function batchStakeLexForAccount(address[] memory _accounts, uint256[] memory _amounts) external nonReentrant onlyGov {
        address _lex = lex;
    ...
    function mintAndStakeXlp(address _token, uint256 _amount, uint256 _minUsdg, uint256 _minXlp) external nonReentrant returns (uint256) {
        require(_amount > 0, "RewardRouter: invalid _amount");

        address account = msg.sender;
        uint256 xlpAmount = IXlpManager(xlpManager).addLiquidityForAccount(account, account, _token, _amount, _minUsdg, _minXlp);
        IRewardTracker(feeXlpTracker).stakeForAccount(account, account, xlp, xlpAmount);
        IRewardTracker(stakedXlpTracker).stakeForAccount(account, account, feeXlpTracker, xlpAmount);
    ...
    function mintAndStakeXlpETH(uint256 _minUsdg, uint256 _minXlp) external payable nonReentrant returns (uint256) {
        ...
        address account = msg.sender;
        uint256 xlpAmount = IXlpManager(xlpManager).addLiquidityForAccount(address(this), account, weth, msg.value, _minUsdg, _minXlp);
    ...
    function unstakeAndRedeemXlp(address _tokenOut, uint256 _xlpAmount, uint256 _minOut, address _receiver) external nonReentrant returns (uint256) {
        ...
        address account = msg.sender;
        IRewardTracker(stakedXlpTracker).unstakeForAccount(account, feeXlpTracker, _xlpAmount, account);
        IRewardTracker(feeXlpTracker).unstakeForAccount(account, xlp, _xlpAmount, account);
        uint256 amountOut = IXlpManager(xlpManager).removeLiquidityForAccount(account, _tokenOut, _xlpAmount, _minOut, _receiver);
    ...
    function unstakeAndRedeemXlpETH(uint256 _xlpAmount, uint256 _minOut, address payable _receiver) external nonReentrant returns (uint256) {
        require(_xlpAmount > 0, "RewardRouter: invalid _xlpAmount");

        address account = msg.sender;
        IRewardTracker(stakedXlpTracker).unstakeForAccount(account, feeXlpTracker, _xlpAmount, account);
        IRewardTracker(feeXlpTracker).unstakeForAccount(account, xlp, _xlpAmount, account);
        uint256 amountOut = IXlpManager(xlpManager).removeLiquidityForAccount(account, weth, _xlpAmount, _minOut, address(this));
    ...
    function handleRewards(
        ...
        if (_shouldClaimEsLex) {
            uint256 esLexAmount0 = IRewardTracker(stakedLexTracker).claimForAccount(account, account);
            uint256 esLexAmount1 = IRewardTracker(stakedXlpTracker).claimForAccount(account, account);
        ...
        if (_shouldClaimWeth) {
            if (_shouldConvertWethToEth) {
                uint256 weth0 = IRewardTracker(feeLexTracker).claimForAccount(account, address(this));
    ...
    function signalTransfer(address _receiver) external nonReentrant {
        require(IERC20(lexVester).balanceOf(msg.sender) == 0, "RewardRouter: sender has vested tokens");
    ...
    function acceptTransfer(address _sender) external nonReentrant {
        require(IERC20(lexVester).balanceOf(_sender) == 0, "RewardRouter: sender has vested tokens");
    ...
    function _validateReceiver(address _receiver) private view {
        require(IRewardTracker(stakedLexTracker).averageStakedAmounts(_receiver) == 0, "RewardRouter: stakedLexTracker.averageStakedAmounts > 0");
        require(IRewardTracker(stakedLexTracker).cumulativeRewards(_receiver) == 0, "RewardRouter: stakedLexTracker.cumulativeRewards > 0");

        require(IRewardTracker(bonusLexTracker).averageStakedAmounts(_receiver) == 0, "RewardRouter: bonusLexTracker.averageStakedAmounts > 0");
        require(IRewardTracker(bonusLexTracker).cumulativeRewards(_receiver) == 0, "RewardRouter: bonusLexTracker.cumulativeRewards > 0");

        require(IRewardTracker(feeLexTracker).averageStakedAmounts(_receiver) == 0, "RewardRouter: feeLexTracker.averageStakedAmounts > 0");
        require(IRewardTracker(feeLexTracker).cumulativeRewards(_receiver) == 0, "RewardRouter: feeLexTracker.cumulativeRewards > 0");

        require(IVester(lexVester).transferredAverageStakedAmounts(_receiver) == 0, "RewardRouter: lexVester.transferredAverageStakedAmounts > 0");
        require(IVester(lexVester).transferredCumulativeRewards(_receiver) == 0, "RewardRouter: lexVester.transferredCumulativeRewards > 0");
        ...
        require(IVester(xlpVester).transferredAverageStakedAmounts(_receiver) == 0, "RewardRouter: lexVester.transferredAverageStakedAmounts > 0");
        require(IVester(xlpVester).transferredCumulativeRewards(_receiver) == 0, "RewardRouter: lexVester.transferredCumulativeRewards > 0");
    ...
    function _stakeLex(address _fundingAccount, address _account, address _token, uint256 _amount) private {
        require(_amount > 0, "RewardRouter: invalid _amount");

        IRewardTracker(stakedLexTracker).stakeForAccount(_fundingAccount, _account, _token, _amount);
        IRewardTracker(bonusLexTracker).stakeForAccount(_account, _account, stakedLexTracker, _amount);
    ...
    function _unstakeLex(address _account, address _token, uint256 _amount, bool _shouldReduceBnLex) private {
        require(_amount > 0, "RewardRouter: invalid _amount");

        uint256 balance = IRewardTracker(stakedLexTracker).stakedAmounts(_account);

        IRewardTracker(feeLexTracker).unstakeForAccount(_account, bonusLexTracker, _amount, _account);
        IRewardTracker(bonusLexTracker).unstakeForAccount(_account, stakedLexTracker, _amount, _account);
        ...
        if (_shouldReduceBnLex) {
            ...
            if (stakedBnLex > 0) {
                ...
                IRewardTracker(feeLexTracker).unstakeForAccount(_account, bnLex, reductionAmount, _account);
