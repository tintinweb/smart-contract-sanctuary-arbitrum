...
contract IssuedSynthAggregator is Owned, MixinSystemSettings, IIssuedSynthAggregator {
    ...
    bytes32 private constant CONTRACT_FUTURESMARKET_MANAGER = "FuturesMarketManager";
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        ...
        newAddresses[2] = CONTRACT_FUTURESMARKET_MANAGER;
    ...
    function futuresMarketManager() internal view returns (IFuturesMarketManager) {
        return IFuturesMarketManager(requireAndGetAddress(CONTRACT_FUTURESMARKET_MANAGER));
    }

    function totalIssuedSynths() external view returns (uint256 _issuedSynths) {
        uint256 _synthAmount = _totalIssuedSynths();
        (uint256 _totalMarketDebt, ) = futuresMarketManager().totalDebt();
        _issuedSynths = _synthAmount + _totalMarketDebt;
    }

    function totalIssuedSynthPerAsset(bytes32 currencyKey) external view returns (uint256 _issuedSynth) {
        if (currencyKey == sUSD) {
            _issuedSynth = synthrAggregator().synthTotalSupply(currencyKey);
        } else {
            (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(currencyKey);
            uint256 totalSynths = synthrAggregator().synthTotalSupply(currencyKey);
            _issuedSynth = totalSynths.multiplyDecimal(currencyRate);
        }
    }

    function totalIssueSynthWithoutMarketDebt() external view returns (uint256) {
        return _totalIssuedSynths();
    }

    function _totalIssuedSynths() internal view returns (uint256 _issuedSynths) {
        bytes32[] memory availableSynths = issuer().availableCurrencyKeys();
        for (uint256 ii = 0; ii < availableSynths.length; ii++) {
            bytes32 currencyKey = availableSynths[ii];
            if (currencyKey == sUSD) {
                uint256 totalSynths = synthrAggregator().synthTotalSupply(currencyKey);
