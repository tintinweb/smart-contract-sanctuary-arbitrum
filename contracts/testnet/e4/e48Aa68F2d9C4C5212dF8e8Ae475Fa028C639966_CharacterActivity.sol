...
// EVENTS
    ...
    event BeginActivitySet(address indexed account, uint256 tokenID, uint256 activityDuration, uint256 numberOfActivities, uint256 startBlock, uint256 endBlock); 
...
// FUNCTIONS
        ...
        onlyCharacterOwner(tokenID, ownerOfCharacter)
    {
        ...
        charactersActivity[tokenID].active = active;                     // Set active
        if (!active) {
            charactersActivity[tokenID].completedBlock = block.number;   // Set block at which the activity completes
        }
    }
    ...
    /// @param ownerOfCharacter address of character owner
    function setBeginActivity(uint256 tokenID, uint256 activityDuration, uint256 numberOfActivities, uint256 startBlock, uint256 endBlock, address ownerOfCharacter)
        external
        onlyAllowed
        onlyCharacterOwner(tokenID, ownerOfCharacter)
    {
        require(endBlock > startBlock,                                           "End block should be higher than start");
        emit BeginActivitySet(_msgSender(), tokenID, activityDuration, 
                                    numberOfActivities, startBlock, endBlock);   // Write an event to the chain
        charactersActivity[tokenID].active = true;                               // Set active
        charactersActivity[tokenID].activityDuration = activityDuration;         // Set activity duration
        charactersActivity[tokenID].startBlock = startBlock;                     // Set start block
        charactersActivity[tokenID].endBlock = endBlock;                         // Set end block
        charactersActivity[tokenID].numberOfActivities = numberOfActivities;     // Set number of activies
    }
...
//VIEWS
        ...
        )
    {
        ...
        if (!activity.active) {                                                        // Character not on a activity
            uint256 blocksToMaxHealth = activity.completedBlock + (activity.activityDuration * // activity.numberOfActivities); // Calculate blocks until health is restored                            
                                  (getCharactersMaxHealth(tokenID) - getCharactersHealth(tokenID)));
