...
contract L1SocketDepositHelper is Owned {
    ...
    /// @param vault The Socket vault address
    function updateVault(address collateral, address vault) external onlyOwner {
        vaults[collateral] = VaultInterface(vault);

        emit VaultUpdated(collateral, vault);
    }
    ...
    function depositETHToAppChain(address _receiver, uint256 _msgGasLimit, address _connector, bytes calldata _data)
        external
        payable
    {
        vaults[weth].depositToAppChain{value: msg.value}(_receiver, msg.value, _msgGasLimit, _connector);
        emit TokensDeposited(_connector, msg.sender, _receiver, msg.value, _data);
    }
    ...
    function _depositToAppChain(
        ...
        vaults[_asset].depositToAppChain(_receiver, _amount, _msgGasLimit, _connector);
