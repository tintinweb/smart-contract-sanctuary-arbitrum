...
import "../../interfaces/IFreeTournamentV2.sol";
import "../../interfaces/IFreeTicket.sol";

contract FreeTournamentV2 is ReentrancyGuard, IFreeTournament, Ownable {
    ...
    constructor (IGamePolicy _gamePolicy, IPrizeManagerV2 _prizeManager, IFreeTicket _freeTicket, uint256 _currentGameId) {
    ...
    function join(uint256 _gameId, uint256 _ticketId) external nonReentrant {
        GameInfo storage _gameInfo = gameInfo[_gameId];
        require(_gameInfo.status == GAME_STATUS.PENDING && _gameInfo.startTime - delayTime > block.timestamp, "FreeTournament: started");
        ...
        require(freeTicket.ownerOf(_ticketId) == msg.sender, "FreeTournament: !owner");
    ...
    function create(
        ...
        require(_startTime >= block.timestamp + delayTime, "FreeTournament: Too late");
        require(_startTime <= block.timestamp + limitStart, "FreeTournament: Too early");
    ...
    function record(uint256 _gameId, uint256 _roundId, uint256  _players, uint256 _predictions) external onlyOperator {
        ...
        require(block.timestamp >= _gameInfo.startTime, "FreeTournament: !started");
    ...
    function update(uint256 _gameId, uint256 _roundId, uint256 _result) external onlyOperator {
        require(roundInfo[_gameId][_roundId].result == ROUND_RESULT(0), "FreeTournament: have result");
