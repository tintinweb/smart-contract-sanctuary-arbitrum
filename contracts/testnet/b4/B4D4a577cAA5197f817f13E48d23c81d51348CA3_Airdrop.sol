...
contract Airdrop is Ownable {
    ...
    function init(bytes calldata data) external {
        (
            ...
            address owner_,
            bool physical
        ) = abi.decode(data, (address, address, uint32, uint32, IERC20, uint96, address, bool));
        require(beginTime_ < endTime_, "Airdrop: Invalid time range.");
    ...
    function setMerkleRoot(bytes32 merkleRoot_, string memory externalURI_) public onlyOwner {
        merkleRoot = merkleRoot_;
        externalURI = externalURI_;
        emit LogSetMerkleRoot(merkleRoot_, externalURI_);
    }

    function nftRedemption(address recipient, bytes32[] calldata proof, uint256 originalTokenId) external {
        bytes memory data;
        _redemptionPreCheck(data, originalTokenId);
        require(!isPhysicalRedemption, "Airdrop: Non-NFT redemption.");
        _merkleProofVerify(proof, originalTokenId);
        uint256 id = _mintNFT(recipient);
        getPayment(redemptionFee, fees.mintingFee);
        emit LogNFTRedemption(recipient, originalTokenId, id);
    }

    function physicalRedemption(bytes32[] calldata proof, bytes calldata data, uint256 originalTokenId) external payable{
        _redemptionPreCheck(data, originalTokenId);
    ...
    function _merkleProofVerify(bytes32[] calldata proof, uint256 originalTokenId) internal view {
        if (merkleRoot != bytes32(0)) {
            require(
                MerkleProof.verify(
                    proof,
                    merkleRoot,
                    keccak256(abi.encodePacked(msg.sender, address(originalNFT), originalTokenId))
                ),
    ...
    function claimEarnings(address recipient) public onlyOwner {
