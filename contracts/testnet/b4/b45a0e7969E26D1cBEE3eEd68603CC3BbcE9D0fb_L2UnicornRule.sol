...
contract L2UnicornRule is IL2UnicornRule {
    ...
    function getAllHatchRule() public pure returns (HatchRule[] memory) {
        HatchRule[] memory hatchRuleArr = new HatchRule[](9);
        for (uint256 level_ = 0; level_ <= 9;) {
            hatchRuleArr[level_] = getHatchRuleByLevel(level_);
            unchecked{++level_;}
        }
        return hatchRuleArr;
    }

    function getHatchRuleByLevel(uint256 level_) public pure returns (HatchRule memory) {
        ...
        if (level_ == 0) {
            ...
            return HatchRule(9, 999999, 999999, 999999, 999999, 1000, 3999, 3000, 1000000);
        } else {
    ...
    */
    function getHatchRuleByCurrentLevelRandomNum(uint256 currentLevel_, uint256 randomNum_) public pure returns (HatchRule memory) {
        for (uint256 levelIndex_ = 0; levelIndex_ <= 9;) {
            EvolveRule memory evolveRule = getEvolveRuleByCurrentLevelIndex(currentLevel_, levelIndex_);
    ...
    function getEvolveRuleByCurrentLevelIndex(uint256 currentLevel_, uint256 index_) public pure returns (EvolveRule memory) {
        if (currentLevel_ == 1) {
            if (index_ == 0) {
