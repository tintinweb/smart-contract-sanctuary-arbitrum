...
contract ArbswapToken is ERC20, Ownable {
    ...
    event MintByCommunity(address indexed miner, address to, uint256 amount);
    event MintByPartnership(address indexed miner, address to, uint256 amount);
    event MintByInitialLiquidity(address indexed miner, address to, uint256 amount);
    event MintByTeam(address indexed miner, address to, uint256 amount);
    event OutOfMint();
     ...
     */
    function mintByCommunity(address _to, uint256 _amount) external onlyCommunity {
        require(communityTotalSupply < CommunityMaxSupply, "Exceeded maximum supply");
        uint256 mintAmount = (communityTotalSupply + _amount) < CommunityMaxSupply
            ? _amount
            : (CommunityMaxSupply - communityTotalSupply);
        communityTotalSupply += mintAmount;
        _mint(_to, mintAmount);
        emit MintByCommunity(msg.sender, _to, mintAmount);
    }
     ...
     */
    function mintByPartnership(address _to, uint256 _amount) external onlyPartnership {
        require(partnershipTotalSupply < PartnershipMaxSupply, "Exceeded maximum supply");
        uint256 mintAmount = (partnershipTotalSupply + _amount) < PartnershipMaxSupply
            ? _amount
            : (PartnershipMaxSupply - partnershipTotalSupply);
        partnershipTotalSupply += mintAmount;
        _mint(_to, mintAmount);
        emit MintByPartnership(msg.sender, _to, mintAmount);
    }
     ...
     */
    function mintByInitialLiquidity(address _to, uint256 _amount) external onlyInitialLiquidity {
        require(initialLiquidityTotalSupply < InitialLiquidityMaxSupply, "Exceeded maximum supply");
        uint256 mintAmount = (initialLiquidityTotalSupply + _amount) < InitialLiquidityMaxSupply
            ? _amount
            : (InitialLiquidityMaxSupply - initialLiquidityTotalSupply);
        initialLiquidityTotalSupply += mintAmount;
        _mint(_to, mintAmount);
        emit MintByInitialLiquidity(msg.sender, _to, mintAmount);
    }
    ...
    function mintByTeam(address _to, uint256 _amount) external onlyTeam {
        ...
        uint256 mintAmount = (teamTotalSupply + _amount) < TeamMaxSupply ? _amount : (TeamMaxSupply - teamTotalSupply);
        teamTotalSupply += mintAmount;
        _mint(_to, mintAmount);
        emit MintByTeam(msg.sender, _to, mintAmount);
    }
