...
contract FixedRateMarket is Initializable, ReentrancyGuard, ERC20Upgradeable, IFixedRateMarket {
  ...
  modifier onlyAdmin() {
    require(_qAdmin.hasRole(_qAdmin.ADMIN_ROLE(), msg.sender), "FRM0 only admin");
    _;
  }
  ...
  function borrow(uint amountPV, uint64 maxAPR) external nonReentrant {
  ...
  /// @param maxAPR Only accept `Quote`s up to specified APR. You may think of
  /// this as a maximum slippage tolerance variable
  function borrowETH(uint amountPV, uint64 maxAPR) external nonReentrant {
  ...
  /// this as a maximum slippage tolerance variable
  function lend(uint amountPV, uint64 minAPR) external nonReentrant {
  ...
  /// @param minAPR Only accept `Quote`s up to specified APR. You may think of
  /// this as a maximum slippage tolerance variable
  function lendETH(uint64 minAPR) external payable nonReentrant {
    require(address(_underlying) == _qAdmin.WETH(), "FRM24 ETH operation not permitted in this market");
    ...
    // If user sends more ETH than is actually being executed, the excessive amount should
    // be refunded to the user
    uint feeIncurred = proratedProtocolFee(executedPV);
    if (executedPV + feeIncurred < msg.value) {
      (bool success,) = msg.sender.call{value: msg.value - executedPV - feeIncurred}("");
  ...
  function repayBorrow(uint amount) external nonReentrant returns(uint){
    return _repayBorrow(amount, false);
  }
  
  /// @notice Borrower will make repayments to the smart contract using ETH, which
  /// holds the value in escrow until maturity to release to lenders.
  /// @return uint Remaining account borrow amount
  function repayBorrowWithETH() external payable nonReentrant returns(uint) {
    require(address(_underlying) == _qAdmin.WETH(), "FRM24 ETH operation not permitted in this market");
    uint balanceBefore = _accountBorrows[msg.sender];
    uint balanceAfter = _repayBorrow(msg.value, true);

    // If user sends more ETH than is actually being executed, the excessive amount should
    // be refunded to the user
    if (balanceBefore - balanceAfter < msg.value) {
      (bool success,) = msg.sender.call{value: msg.value - (balanceBefore - balanceAfter)}("");
      require(success, "FRM25 unsuccessful ETH transfer");
    }
  ...
  function redeemQTokensByRatio(uint amount) external nonReentrant returns(uint) {
  ...
  /// balance.
  /// @return uint Amount of qTokens redeemed
  function redeemAllQTokensByRatio() external nonReentrant returns(uint) {
  ...
  /// @param amount Amount of qTokens to redeem
  /// @return uint Amount of qTokens redeemed
  function redeemQTokensByRatioWithETH(uint amount) external nonReentrant returns(uint) {
    require(address(_underlying) == _qAdmin.WETH(), "FRM24 ETH operation not permitted in this market");
  ...
  /// balance.
  /// @return uint Amount of qTokens redeemed
  function redeemAllQTokensByRatioWithETH() external nonReentrant returns(uint) {
    require(address(_underlying) == _qAdmin.WETH(), "FRM24 ETH operation not permitted in this market");
  ...
  function liquidateBorrow(address borrower, uint amount, IERC20 collateralToken) external nonReentrant {
  ...
  function hypotheticalMaxLendPV(uint amount) public view returns (uint) {
    uint mantissa = 1e8;
    return (amount * mantissa) / (mantissa + proratedProtocolFee(mantissa));
  }
  ...
  /// @param isPaidInETH Is amount being paid in ETH
  /// @return uint Remaining account borrow amount
  function _repayBorrow(uint amount, bool isPaidInETH) internal returns(uint){
