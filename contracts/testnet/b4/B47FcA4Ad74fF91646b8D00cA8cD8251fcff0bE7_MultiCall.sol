...
/**
 * @title DEDERI MultiCall
 * @author dederi
 * @notice This contract is used to multi call func
 */
contract MultiCall is AccessControl {
    ...
    error MultiCall__ValueMismatch();
    error MultiCall__CallFailed(uint256 i);
    ...
    function aggregate(
        Call[] calldata calls
    ) public payable onlyRole(Constants.KEEPER_ROLE) returns (uint256 blockNumber, bytes[] memory returnData) {
        ...
        for (uint256 i; i < length; ) {
            ...
            if (!success) {
                revert MultiCall__CallFailed(i);
            }
            unchecked {
                ++i;
            }
    ...
    ) public payable onlyRole(Constants.KEEPER_ROLE) returns (Result[] memory returnData) {
        uint256 length = calls.length;
    ...
    function aggregate3Value(
        ...
        if (msg.value != valAccumulator) {
            revert MultiCall__ValueMismatch();
        }
    }
}
