...
contract SwapFactory is ISwapFactory, ReentrancyGuard, ProxyAdmin {
    ...
    function createPair(
        ...
        bytes memory bytecode = abi.encodePacked(
            ...
            abi.encode(PAIR_LOGIC, address(this), token0, token1)
        );
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        ISwapPair(pair).setFeeData(supportsTokenFee, tokenB == token0);
...
contract SwapPairUpgradeable is ISwapPair,Initializable, ContextUpgradeable, SwapERC20Upgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _token0, address _token1) internal onlyInitializing() {
    ...
    function setFeeData(bool _supportsTokenFee, bool _token0IsFee) external {
        require(msg.sender == factory, "Swap: NOT_FACTORY");
