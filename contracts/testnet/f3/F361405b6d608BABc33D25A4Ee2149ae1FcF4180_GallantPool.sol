...
contract GallantPool is OwnableUpgradeable, LPToken {
  ...
  function lockAssets(
    ...
    uint256 amountAfterFee = normalizedAmount - normalizeAmount(liquidityFee, _decimals);
  ...
  function setEmitterImplementation(uint16 _chainId, bytes32 _emitter) external onlyOwner {
    require(_emitter != bytes32(0), 'Invalid emitter address');
  ...
  function unlockAssets(bytes memory encodedVM) external {
    ...
    token.safeTransfer(transferRecipient, unlockAmount);

    emit UnlockedAmount(transferRecipient, unlockAmount, chainId, t.toChain);
  }
