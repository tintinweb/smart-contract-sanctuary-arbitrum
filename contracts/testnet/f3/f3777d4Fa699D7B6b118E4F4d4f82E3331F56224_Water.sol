...
//@TODO: implemnetation of redeem / mint as users can mint and redeem without fee?
...
contract Water is IWater, ERC4626, Ownable, Pausable, Constant {
    ...
    uint96 private feeBPS = 0; // 50 / 0.5%
    ...
    function settingManagerValue(bytes32 params, uint256 value) external onlyOwner {
        if (value == 0) revert ThrowZeroAmount();
        else if (params == "feeBPS") feeBPS = uint96(value);
    ...
    function leverageVault(uint256 _amount) external onlyBartender {
        ...
        _totalDebt += _amount;
        IERC20(usdcToken).safeTransfer(address(bartender), _amount);

        emit LeverageBartender(_amount, block.timestamp);
    }

    /// @dev See {IWater-repayDebt}
    function repayDebt(uint256 _amount) external onlyBartender {
        _totalDebt -= _amount;
        IERC20(usdcToken).safeTransferFrom(address(bartender), address(this), _amount);
    ...
    function withdraw(
        ...
        uint256 fees = _transferRecipientFee(bytes32("withdraw"), _assets, _owner);

        uint256 assetsAfterFee = _assets - fees;

        shares = previewWithdraw(_assets);

        _withdraw(_msgSender(), _receiver, _owner, assetsAfterFee, shares);
    ...
    function deposit(uint256 _assets, address _receiver) public override whenNotPaused returns (uint256 shares) {
        ...
        // @TODO unit tests required to if the protocol fee is correct
        uint256 fees = _transferRecipientFee(bytes32("deposit"), _assets, _receiver);
    ...
    function totalAssets() public view override returns (uint256 _totalAssets) {
        return IERC20(usdcToken).balanceOf(address(this)) + _totalDebt;
    }

    /// @return _feeBPS gets the protocol fee
    function getFeeBPS() public view returns (uint256 _feeBPS) {
        return _feeBPS;
    }

    /// @return _feeRecipient gets the fee recipient address
    function getFeeRecipient() public view returns (address _feeRecipient) {
    ...
    function previewWithdraw(uint256 _assets) public view override returns (uint256) {
        uint256 fees = (_assets * feeBPS) / MAX_BPS;
    ...
    // @TODO add previewDeposit + protocol fee
    // @TODO add previewWithdraw + protocol fee
    // @TODO add maxDeposit + protocol fee + asset cap
    // @TODO add maxWithdraw + protocol fee + total debt
    ...
    /// @param _owner owner of the Water token
    function _transferRecipientFee(bytes32 _params, uint256 _assets, address _owner) internal returns (uint256) {
        uint256 fees = (_assets * feeBPS) / MAX_BPS;
        if (_params == "deposit") {
        ...
        if (_params == "withdraw") {
            //bypassing the preivewWithdraw function
            uint256 shares = _convertToShares(fees, Math.Rounding.Up);

            _withdraw(_msgSender(), _feeRecipient, _owner, fees, shares);
        }
        return fees;
    }

    function getWaterPrice() public view returns (uint256) {
        return totalAssets() == 0 ? 1_000_000 : (totalAssets() * (10 ** 6)) / totalSupply();
    }
