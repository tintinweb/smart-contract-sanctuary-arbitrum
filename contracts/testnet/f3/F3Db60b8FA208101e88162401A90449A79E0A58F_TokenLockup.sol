...
/**
    ...
    @dev When deployed the contract acts as a proxy for a single token that it creates release schedules for
...
contract TokenLockup is ReentrancyGuard {
    ...
    error TokenLockup__FirstReleaseGreaterThanMax();
    error TokenLockup__ZeroReleaseCount(); // "< 1 release"
    error TokenLockup__ReleaseGreaterThanBIPSCap(); // "release > 100%"
    error TokenLockup__ReleaseLowerCap(); // "release < 100%"
    error TokenLockup__ZeroPeriod(); // "period = 0"
    error TokenLockup__ExceedsMaxCancelers(uint256 numCancelableBy); // // "max 10 cancelableBy addressees"
    error TokenLockup__MaxTimelocksExceeded(uint256 numTimelocks); // // "Max timelocks exceeded"
    error TokenLockup__AmountLessThanMinTimelock(uint256 minTimelockAmount); // // "amount < min funding"
    error TokenLockup__ZeroAddress(); // "to 0 address"
    error TokenLockup__InvalidSchedule(uint256 scheduleID); // "bad scheduleId"
    error TokenLockup__InvalidTimelock(uint256 timelockIndex); // "invalid timelock"
    error TokenLockup__InvalidTimestamp(uint256 commencementTimestamp); // Expected commencementTimestamp does not match
    error TokenLockup__AtLeastOneTokenPerRelease(
        uint256 amount,
        uint256 releaseCount
    ); // "< 1 token per release"
    error TokenLockup__DeflationTokenDeclined(); // "deflation token declined"
    error TokenLockup__InitialReleaseOutOfRange(); // "initial release out of range"
    error TokenLockup__DenyCancelTimelock(); // "You are not allowed to cancel this timelock"
    error TokenLockup__TotalAmountDoesNotMatch(uint256 totalAmount); // Expected totalAmount does not match
    error TokenLockup__EmptyTimelock(); // "Timelock has no value left"
    error TokenLockup__MismatchedArrayLength(
        uint256 expectedLength,
        uint256 actualLength
    ); // "mismatched array length"
    error TokenLockup__InvalidAllowance(uint256 allowance, uint256 tryDecrease); // "decrease > allowance"
    error TokenLockup__InsufficientUnlockedAmount(uint256 unlockedAmount); // "amount > unlocked"
    error TokenLockup__InvalidTransfer(); // "bad transfer"
    error TokenLockup__ValueGreaterThanAllowance(); // "bad transfer"
    error TokenLockup__InvalidFutureReleases();
    ...
    /**
        ...
        @param _minTimelockAmount The minimum amount of tokens that can be locked up. Should be set to some non-zero value. Defaults to 100 tokens. Ex. 100 * (10 ** token.decimals())
        @param _maxReleaseDelay The maximum delay in seconds that can be set for the first release.
    ...
    */
    constructor(
        ...
        if (_minTimelockAmount == 0) {
            _minTimelockAmount =
                100 *
                (10 ** IERC20Metadata(_token).decimals()); // 100 tokens
        }
    ...
    */
    function createReleaseSchedule(
        ...
        // maxReleaseDelay only applies to delayUntilFirstReleaseInSeconds, not overall periodBetweenReleasesInSeconds
        if (delayUntilFirstReleaseInSeconds > maxReleaseDelay) {
            revert TokenLockup__FirstReleaseGreaterThanMax();
        }

        if (releaseCount == 0) {
            revert TokenLockup__ZeroReleaseCount();
        }
        if (initialReleasePortionInBips > BIPS_PRECISION) {
            revert TokenLockup__ReleaseGreaterThanBIPSCap();
        }

        if (releaseCount > 1) {
            if (periodBetweenReleasesInSeconds == 0) {
                revert TokenLockup__ZeroPeriod();
            }
        } else if (releaseCount == 1) {
            if (initialReleasePortionInBips != BIPS_PRECISION) {
                revert TokenLockup__ReleaseLowerCap();
            }
        }

        if (initialReleasePortionInBips == BIPS_PRECISION) {
            if (releaseCount > 1) {
                revert TokenLockup__InvalidFutureReleases();
            }
        }
    ...
    */
    function fundReleaseSchedule(
        ...
        if (cancelableBy.length > 10) {
            revert TokenLockup__ExceedsMaxCancelers(cancelableBy.length);
        }
    ...
    function _fund(
        ...
        if (timelocks[to].length > MAX_TIMELOCKS) {
            revert TokenLockup__MaxTimelocksExceeded(timelocks[to].length);
        }
        if (amount < minTimelockAmount) {
            revert TokenLockup__AmountLessThanMinTimelock(minTimelockAmount);
        }
        if (to == address(0)) {
            revert TokenLockup__ZeroAddress();
        }
        if (scheduleId >= releaseSchedules.length) {
            revert TokenLockup__InvalidSchedule(scheduleId);
        }
        if (amount < releaseSchedules[scheduleId].releaseCount) {
            revert TokenLockup__AtLeastOneTokenPerRelease(
                amount,
                releaseSchedules[scheduleId].releaseCount
            );
        }
        ...
        if (token.balanceOf(address(this)) - tokensBalance != amount) {
            revert TokenLockup__DeflationTokenDeclined();
        }
        ...
        if (
            ...
            revert TokenLockup__InitialReleaseOutOfRange();
        }
    ...
    /**
        @notice Cancel a cancelable timelock created by the fundReleaseSchedule function.
            WARNING: this function can cancel a release schedule created by fundReleaseSchedule.
            If canceled the tokens that are locked at the time of the cancellation will be returned to the funder
            and unlocked tokens will be transferred to the recipient.
    ...
    */
    function cancelTimelock(
        ...
        if (timelockCountOf(target) <= timelockIndex) {
            revert TokenLockup__InvalidTimelock(timelockIndex);
        }

        if (reclaimTokenTo == address(0)) {
            revert TokenLockup__ZeroAddress();
        }

        Timelock memory timelock = timelocks[target][timelockIndex];

        if (!_canBeCanceled(timelock)) {
            revert TokenLockup__DenyCancelTimelock();
        }

        if (timelock.scheduleId != scheduleId) {
            revert TokenLockup__InvalidSchedule(scheduleId);
        }

        if (timelock.commencementTimestamp != commencementTimestamp) {
            revert TokenLockup__InvalidTimestamp(commencementTimestamp);
        }

        if (timelock.totalAmount != totalAmount) {
            revert TokenLockup__TotalAmountDoesNotMatch(totalAmount);
        }

        uint256 canceledAmount = lockedBalanceOfTimelock(target, timelockIndex);

        if (canceledAmount == 0) {
            revert TokenLockup__EmptyTimelock();
        }
        ...
        timelocks[target][timelockIndex].tokensTransferred = timelock
            .totalAmount;
        return true;
    }
    ...
    function _canBeCanceled(
        ...
        uint256 _length = timelock.cancelableBy.length;
    ...
    function batchFundReleaseSchedule(
        ...
        uint256 _toLen = to.length;
        if (_toLen != amounts.length) {
            revert TokenLockup__MismatchedArrayLength(_toLen, amounts.length);
        }
        if (_toLen != commencementTimestamps.length) {
            revert TokenLockup__MismatchedArrayLength(
                _toLen,
                commencementTimestamps.length
            );
        }
        if (_toLen != scheduleIds.length) {
            revert TokenLockup__MismatchedArrayLength(
                _toLen,
                scheduleIds.length
            );
    ...
    */
    ...
    /**
        ...
        @param who the address to check
        @param timelockIndex  Specific timelock belonging to the who address
        @return balance total remaining balance of a timelock
     */
    function balanceOfTimelock(
        ...
        if (timelock.totalAmount <= timelock.tokensTransferred) {
            ...
            balance = timelock.totalAmount - timelock.tokensTransferred;
        }
    }

    /**
        @notice Gets the total unlocked balance of a specific address's timelocks
        @param who The address to check
        @param timelockIndex The index of the timelock for the who address
        @return total unlocked amount for the specified timelock
    */
    ...
    */
    function transferFrom(
        ...
        if (_allowances[from][msg.sender] < value) {
            revert TokenLockup__ValueGreaterThanAllowance();
        }
    ...
    */
    function decreaseAllowance(
        ...
        if (currentAllowance < subtractedValue) {
            revert TokenLockup__InvalidAllowance(
                currentAllowance,
                subtractedValue
            );
        }
    ...
    */
    ...
    function _transfer(
        ...
        uint256 _unlockedBalanceOf = unlockedBalanceOf(from);
        if (_unlockedBalanceOf < value) {
            revert TokenLockup__InsufficientUnlockedAmount(_unlockedBalanceOf);
        }

        uint256 _remainingTransfer = value;
        uint256 _timelockCountOf = timelockCountOf(from);

        // transfer from unlocked tokens
        for (uint256 i; i < _timelockCountOf; ++i) {
            Timelock memory _timelock = timelocks[from][i];
            // if the timelock has no value left
            if (_timelock.tokensTransferred == _timelock.totalAmount) {
                continue;
            } else {
                ...
                if (_remainingTransfer > _unlockedBalanceOfTimelock) {
                    // if the remainingTransfer is more than the unlocked balance use it all
                    _remainingTransfer -= _unlockedBalanceOfTimelock;
                    timelocks[from][i]
                    ...
                    timelocks[from][i].tokensTransferred += _remainingTransfer;
                    _remainingTransfer = 0;
                    break;
                }
            }
        }
        ...
        if (_remainingTransfer != 0) {
            revert TokenLockup__InvalidTransfer();
        }
    ...
    */
    ...
    function transferTimelock(
        ...
        if (unlockedBalanceOfTimelock(msg.sender, timelockId) < value) {
            revert TokenLockup__InsufficientUnlockedAmount(
                unlockedBalanceOfTimelock(msg.sender, timelockId)
            );
        }
    ...
    /**
        ...
        @param commencedTimestamp the commencement time to use in the calculation for the schedule.
