...
contract Corruption is Initializable, CorruptionState {
    ...
    function setCorruptionStreamInfo(address _account, uint128 _newRatePerSecond, uint256 _generatedCorruptionCap) external onlyAdminOrOwner {
        // Mint any tokens at the old rate. This also updates the timeLastMinted.
        //
    ...
    function _tokensAccumulated(address _account) private view returns(uint256) {
        uint256 _tokenBalance = super.balanceOf(_account);
        ...
        uint256 _rawAccumulated = (block.timestamp - _streamInfo.timeLastMinted) * _streamInfo.ratePerSecond;

        if(_tokenBalance + _rawAccumulated > _streamInfo.generatedCorruptionCap) {
            return _streamInfo.generatedCorruptionCap - _tokenBalance;
        } else {
            return _rawAccumulated;
        }
    }

    function _mintAccumulatedTokens(address _account) private {
        uint256 _amount = _tokensAccumulated(_account);
...
abstract contract CorruptionState is
    Initializable,
    ICorruption,
    ERC20Upgradeable,
    AdminableUpgradeable
{
    event CorruptionStreamModified(
        address _account,
        uint128 _ratePerSecond,
        uint256 _generatedCorruptionCap
    );
