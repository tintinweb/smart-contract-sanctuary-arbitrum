...
contract DomFiPerp is IDomFiPerp, ReentrancyGuard {
    ...
    // kill switches:
    bool public aggregationDisabled = false;
    ...
    function _increasePosition(
        ...
        positions[positionId] = Position({
            ...
            price: newPrice,
            oraclePrice: priceFeed.getPrice(product.productId),
        ...
        if (isLiquidatable) {
            ...
            if (
                ...
                !PerpLib._canTakeProfit(
                    ...
                    priceFeed.getPrice(product.productId),
        ...
        )
    {
        ...
        for (uint256 i = 0; i < positionIds.length; i++) {
            ...
            FPUnsigned price = priceFeed.getPrice(position.productId);
            (
    ...
    function liquidatePosition(bytes32 positionId) private returns (uint256 liquidatorReward) {
        ...
        FPUnsigned oraclePrice = priceFeed.getPrice(product.productId);
        (FPSigned pnl, , bool isLiquidatable) = _canLiquidate(
            position,
            oraclePrice
        );
    ...
    function _getMaxSafeWithdrawableMargin(
        ...
        FPUnsigned oraclePrice = priceFeed.getPrice(position.productId);
    ...
    function _calculatePrice(
        ...
        return priceFeed.getPrice(productId) * slippage;
    }
    ...
    function updatePrice(
        ...
        require(msg.sender == address(priceFeed), "PriceFeed only");
        emit PriceUpdate(
            productId,
            value,
            timestamp,
    ...
    function setAddresses(
        ...
        fundingManager = _fundingManager;
        feeDistributor = _feeDistributor;
        emit AddressesSet(_priceFeed, _feeCalculator, _fundingManager, _feeDistributor);
    }
    ...
    ///@dev testnet kill switch for getTotalPnl; disables position aggregation and reporting
    function disableTotalPnl(bool disabled) external {
        onlyOwner();
        aggregationDisabled = disabled;
    }
