...
contract USDOExchange is IUSDOExchange, Ownable {
    ...
    //        max amount
    uint256 public maxExchangeAmountPer;
    ...
    //    ---------------event-----------------
    event UpdateMaxExchangeAmountPer(uint256 oldMaxExchangeAmountPer, uint256 newMaxExchangeAmountPer);
    event BuyUSDO(uint256 amount, address indexed to, address indexed payer);

    //    --------------------operator-----------------------

    constructor(uint256 _maxExchangeAmountPer, address _USDC, address _USDO) {
        maxExchangeAmountPer = _maxExchangeAmountPer;
        primaryAsset = _USDC;
        USDO = _USDO;
        isExchangeOpen = true;
    }

    function updateMaxExchangeAmountPer(uint256 newMaxExchangeAmountPer) external onlyOwner {
        emit UpdateMaxExchangeAmountPer(maxExchangeAmountPer, newMaxExchangeAmountPer);
        maxExchangeAmountPer = newMaxExchangeAmountPer;
    }
    ...
    function buyUSDO(uint256 amount, address to) external {
        ...
        require(amount <= maxExchangeAmountPer, USDOErrors.EXCHANGE_AMOUNT_TOO_BIG);
