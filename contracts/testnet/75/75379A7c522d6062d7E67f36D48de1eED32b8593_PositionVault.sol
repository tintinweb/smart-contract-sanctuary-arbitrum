...
contract PositionVault is Constants, Initializable, ReentrancyGuardUpgradeable, IPositionVault {
    ...
    function addOrRemoveCollateral(
        ...
        if (isPlus) {
            ...
            vault.takeVUSDOut(_account, _amount);
        }
    ...
    function createAddPositionOrder(
        ...
        orderVault.createAddPositionOrder(_posId, _collateralDelta, _sizeDelta, _allowedPrice, fee);
    ...
    function triggerForOpenOrders(uint256 _posId) external nonReentrant onlyOperator(1) {
        Position memory position = positions[_posId];
        address _account = position.owner;
        Order memory order = orderVault.getOrder(_posId);
        ...
        if (order.positionType == POSITION_LIMIT) {
            ...
            orderVault.updateOrder(_posId, order.positionType, 0, 0, OrderStatus.FILLED);
            _removeUserOpenOrder(_account, _posId);
            ...
            orderVault.updateOrder(_posId, POSITION_LIMIT, order.collateral, order.size, order.status);
            ...
            _decreasePosition(_posId, price, order.size);
    ...
    function triggerForTPSL(uint256 _posId) external nonReentrant onlyOperator(1) {
        (uint256 triggeredAmount, uint256 triggerPrice) = orderVault.executeTriggerOrders(_posId);
        _decreasePosition(_posId, triggerPrice, triggeredAmount);
    }
    ...
    // for other vaults to directly close user's position
