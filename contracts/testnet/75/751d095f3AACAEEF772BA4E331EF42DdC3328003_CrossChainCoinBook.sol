    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(uint32 => mapping(IERC20 => bool)) public restrictedTokens;
    ...
    function initialize(
        ...
        initialId = currentId;
        maxPriceAge = 1800;
        thisChain = uint32(block.chainid);
    ...
    * @param r The round number for the listing fee
    ...
    function createOrder(
     ...
     returns (uint256 received, uint256 sold) {
        ...
        if (_buyTokenChain == thisChain && activeOrders[buyHash].length > 0) {            
    ...
    function fulfillOrders(uint256[] calldata _orderIds, uint256[] calldata buyAmounts, uint256[] calldata sellAmounts, address _buyer)  external override onlyMatcher nonReentrant {
        ...
        uint32 buyTokenChain = orderId[_orderIds[0]].buyTokenChain;
     ...
     */
    function getActiveOrders(IERC20 _sellToken, IERC20 _buyToken, uint32 _buyTokenChain)
        external
        view
        override
        returns (uint256[] memory)
    {
        bytes32 pair = keccak256(abi.encodePacked(_sellToken, thisChain, _buyToken, _buyTokenChain));
        bytes32 inverse = keccak256(abi.encodePacked(_buyToken, _buyTokenChain, _sellToken, thisChain));
...
interface ICrossChainCoinBook {
    ...
    function getActiveOrders(IERC20 _sellToken, IERC20 _buyToken, uint32 _buyTokenChain) external view returns (uint256[] memory);
    ...
    function setRestrictedToken(IERC20 token, uint32 chain, bool flag) external;
