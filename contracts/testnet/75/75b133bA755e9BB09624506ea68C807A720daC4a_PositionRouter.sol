...
interface IOrderBook {
    ...
    function executeSwapOrder(address, uint256, address payable, bytes[] calldata) external payable;
    function executeDecreaseOrder(address, uint256, address payable, bytes[] calldata) external payable;
    function executeIncreaseOrder(address, uint256, address payable, bytes[] calldata) external payable;
}
...
interface IRouter {
    ...
    function swap(address[] memory _path, uint256 _amountIn, uint256 _minOut, address _receiver, bytes[] calldata _priceUpdateData) external payable ;
}
...
interface IVault {
    ...
    function lastFundingTimes(address _collateralToken, address _indexToken) external view returns (uint256);
    ...
    function fundingRates(address _collateralToken, address _indexToken) external view returns (uint256);
    function setFundingInterval(uint256 _fundingInterval)  external;
    function cumulativeFundingRates(address _collateralToken, address _indexToken) external view returns (uint256);
    function setFundingRate(address _collateralToken, address _indexToken, uint256 _fundingRate) external;
    // function setFundingRates(address[] memory _collateralToken, address[] memory _indexToken, uint256[] memory _fundingRateFactor) external;
    function getNextFundingRate(address _collateralToken, address _indexToken) external view returns (uint256);
...
library Address {
    ...
    function isContract(address account) internal view returns (bool) {
        ...
        uint256 size;
        // solhint-disable-next-line no-inline-assembly ("memory-safe") 
    ...
    function _verifyCallResult(bool success, bytes memory returndata, string memory errorMessage) private pure returns(bytes memory) {
        if (success) {
            ...
            if (returndata.length > 0) {
                // The easiest way to bubble the revert reason is using memory via assembly ("memory-safe") 
