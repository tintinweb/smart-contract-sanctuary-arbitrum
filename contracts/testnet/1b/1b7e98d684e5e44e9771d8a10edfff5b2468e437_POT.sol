...
contract POT is Ownable{
    ...
    address public ByBitsAddress = 0x11668139AC2569b7bA34ee43Ccb13f3CEb55b098;

    address private signerAddress = 0x960A4406d23Cb0cced0584B769bde13de60F27c5;

    event UserDeposit(address indexed _address, string indexed  _userid, uint indexed _amount);
    event UserTradeType(string indexed _userid, uint indexed _amount, uint indexed _tradeType);
    ...
    function Deposit(uint amount, uint tradetype , string memory id) public{
        ...
        // USDT.transfer(ByBitsAddress, amount);
        Insert(msg.sender, amount, tradetype, id);
    }

    function Insert(address userAddress, uint amount, uint tradetype, string memory id) internal{
        UserIdCounter++;
    ...
    function SetByBitsAddress(address _address) external onlyOwner{
        ByBitsAddress = _address;
    }
    ...
    function WithdrawRequest(
        ...
        require(Pause == false, "Contract is paused");
        string memory data = Strings.strConcat(_rawdata, _uint2str(_amount));
  ...
  function OwnerWithdraw(address _token, uint _amount) external onlyOwner{
