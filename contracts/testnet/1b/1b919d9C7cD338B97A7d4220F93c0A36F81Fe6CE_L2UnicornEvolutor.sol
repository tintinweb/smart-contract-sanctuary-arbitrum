...
contract L2UnicornEvolutor is BaseGame, BaseL2Unicorn, BaseL2UnicornRule, BaseRecipient, Pausable {
    ...
    function randomizerCallback(uint256 requestId_, bytes32 seed_) external {
        ...
        for (uint256 numberIndex_ = 0; numberIndex_ < length_;) {
            (uint256 randomNumber_, IL2UnicornRule.HatchRule memory hatchRule_) = _getRandomNumberAndHatchRule(seed_, evolveLevelArr_[numberIndex_], numberIndex_);
    ...
    function viewEvolveResults(uint256 requestId_) external view returns (EvolveResult[] memory evolveResults){
        ...
        for (uint256 numberIndex_ = 0; numberIndex_ < length_;) {
            (uint256 randomNumber_, IL2UnicornRule.HatchRule memory hatchRule_) = _getRandomNumberAndHatchRule(gameInfo_.seed, evolveLevelArr_[numberIndex_], numberIndex_);
    ...
    function _getRandomNumberAndHatchRule(bytes32 seed_, uint8 evolveLevel_, uint256 numberIndex_) private view returns (uint256 randomNumber_, IL2UnicornRule.HatchRule memory hatchRule_){
        randomNumber_ = getEvolveRandomNumber(seed_, numberIndex_);
