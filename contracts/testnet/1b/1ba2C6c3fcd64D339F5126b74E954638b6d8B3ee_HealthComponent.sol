...
uint256 constant ID = uint256(
    keccak256("game.piratenation.healthcomponent.dev1")
);
...
library HealthComponentStorage {
 ...
 * @dev Contains the health data for a unit
 */
contract HealthComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function setValue(
        ...
        _setValue(entity, Layout(maxHealth, currentHealth));
    }
    ...
    function getLayoutValue(
        ...
        value = HealthComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return maxHealth The maximum health of the unit
     * @return currentHealth The current health of the unit
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint128 maxHealth, uint128 currentHealth) {
        if (has(entity)) {
            Layout memory s = HealthComponentStorage.layout().entityIdToStruct[
                entity
            ];
            (maxHealth, currentHealth) = abi.decode(
                _getEncodedValues(s),
                (uint128, uint128)
            );
        }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = HealthComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.maxHealth);
        values[1] = abi.encode(s.currentHealth);
    }
    ...
    function setBytes(
        ...
        (s.maxHealth, s.currentHealth) = abi.decode(value, (uint128, uint128));
    ...
    function remove(
        ...
        delete HealthComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete HealthComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.currentHealth = value.currentHealth;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.maxHealth, value.currentHealth));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.maxHealth, value.currentHealth);
    }
}