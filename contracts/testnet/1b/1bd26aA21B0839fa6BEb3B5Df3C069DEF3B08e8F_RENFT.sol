...
contract RENFT is ERC721, ERC721Burnable, ERC721Pausable, Ownable {
    ...
    function transferAllOwnerNFTs(address _from, address _to, uint[] memory _tokenIds) external { 
        require(vaultFactory != address(0), "NFT: vault factory cannot be zero address");
        require(_to != address(0), "NFT: cant not transfer to zero address");

        for (uint i = 0; i < _tokenIds.length; i++ ) {
            safeTransferFrom(_from, _to, _tokenIds[i]);
        }

        IVaultFactory(vaultFactory).updateAdmin(_to);
