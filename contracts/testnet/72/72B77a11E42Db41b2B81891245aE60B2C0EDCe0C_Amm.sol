...
contract Amm is IAmm, BlockContext, OwnerPausable, ReentrancyGuardUpgradeable, AmmStorageV1 {
    ...
    function initialize(InitializeParams memory params) external initializer {
        ...
        // Pack A and gamma:
        // shifted A + gamma
        uint256 arrayAGamma = SigmaMath.shift(params.A, 128);
        arrayAGamma = SigmaMath.bitwiseOr(arrayAGamma, params.gamma);
    ...
    function _resetRepegging() internal {
        ...
        _coinBalancesSinceLastRepegging = SigmaMath.copy(_coinBalances);
    }
    ...
    function rampAGamma(
        ...
        uint256[2] memory arrayAGamma = _getAGamma();
        uint256 initialAGammaTemp = SigmaMath.shift(arrayAGamma[0], 128);
        initialAGammaTemp = SigmaMath.bitwiseOr(initialAGammaTemp, arrayAGamma[1]);
    ...
    function stopRampAGamma() external onlyOwner {
        uint256[2] memory arrayAGamma = _getAGamma();
        uint256 currentAGamma = SigmaMath.shift(arrayAGamma[0], 128);
        currentAGamma = SigmaMath.bitwiseOr(currentAGamma, arrayAGamma[1]);
