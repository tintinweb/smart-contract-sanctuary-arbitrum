...
contract CCIPProtocolFacet is IAny2EVMMessageReceiver, Events, Setters {
    ...
    function ccipSend(
        ...
        Structs.MessagingProtocol memory destChainProtocol = LibInternalGetters.getChainProtocolInfo(sendPayload.destChain, protocol.id);
...
contract RoutingEngineFacet is Setters {
    ...
    function sendMessage(
        ...
        if(gasLimit < 450000) gasLimit = 450000;
        ...
        for (uint256 i; i < priorityProtocolIds.length; i++) {
            ...
            if (priorityProtocolIds[i] == 1) {
                ...
                try
                    LZProtocolFacet(address(this)).lzSend{value: msg.value}(sendPayload, protocol, gasLimit)
                returns (bool protocolSent) {
                    sent = protocolSent;
                    break;
                } catch {
                ...
                try
                    CCIPProtocolFacet(address(this)).ccipSend{value: msg.value}(sendPayload, protocol, gasLimit)
                returns (bool protocolSent) {
                    sent = protocolSent;
                    break;
                } catch {
                ...
                try
                    WormholeProtocolFacet(address(this)).wormholeSend{value: msg.value}(sendPayload, protocol, gasLimit)
                returns (bool protocolSent) {
                    sent = protocolSent;
                    break;
                } catch {
    ...
    function getEstimatedFee(
        ...
        for (uint256 i; i < priorityProtocolIds.length; i++) {
            ...
            if (priorityProtocolIds[i] == 2) {
                currentFee = LibGasEstimation.lzEstimatedFee(sendPayload, protocol, gasLimit);
                if (currentFee > highestFee) highestFee = currentFee;
            }
            if (priorityProtocolIds[i] == 3) {
                currentFee = LibGasEstimation.ccipEstimatedFee(sendPayload, protocol, gasLimit);
                if (currentFee > highestFee) highestFee = currentFee;
            }
            if (priorityProtocolIds[i] == 4) {
                currentFee = LibGasEstimation.wormholeEstimatedFee(sendPayload, protocol, gasLimit);
                if (currentFee > highestFee) highestFee = currentFee;
            }
        }
    }
...
contract WormholeProtocolFacet is IWormholeReceiver, Events, Setters {
    ...
    function wormholeSend(
        ...
        ).quoteEVMDeliveryPrice(uint16(destChainProtocol.protocolChainId), 0, gasLimit);
