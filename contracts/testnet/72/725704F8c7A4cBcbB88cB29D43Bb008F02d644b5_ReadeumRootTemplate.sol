...
import "ERC2981.sol";
import "Strings.sol";
import "ONFT1155.sol"; // Import the ONFT1155 base contract

contract ReadeumRootTemplate is ONFT1155, ERC2981 {
    uint256 public MAX_TOKEN_ID;
    string public _baseUri;
    string private _keyHash; // Encryption key hash
    ...
    constructor(
        ...
        uint96 royaltyValue,
        address _layerZeroEndpoint, // LayerZero endpoint address
        //uint16 _chainId, // Chain ID for LayerZero
        uint256 _maxTokenId, // Maximum token ID
        uint256[] memory ids, // Array of token IDs for initial batch mint
        uint256[] memory amounts, // Corresponding amounts for each token ID
        bytes memory data, // Additional data (can be empty)
        string memory keyHash // Encryption key hash
    ) ONFT1155(baseUri, _layerZeroEndpoint) {
        _baseUri = baseUri;
        _setRoyalties(royaltyRecipient, royaltyValue);
        MAX_TOKEN_ID = _maxTokenId;
        keyHash = _keyHash;

        // Mint the initial batch of tokens
        _mintBatch(msg.sender, ids, amounts, data);
    }

    // Set royalties information
    function _setRoyalties(address recipient, uint96 feeBasisPoints) internal {
        _setDefaultRoyalty(recipient, feeBasisPoints);
    }

    // Override uri for each token id
    function uri(
        uint256 tokenId
    ...
    // Check if a wallet holds any of the tokens
    function hasTokenBalance(address wallet) internal view returns (bool) {
    ...
    // Modifier to restrict function access to token holders
    modifier onlyHolder() {
        require(
            hasTokenBalance(msg.sender),
            "You must be holder of the token to view it"
        );
        _;
    }

    // View encryption key hash
    function viewHash() public view onlyHolder returns (string memory) {
        return _keyHash;
    }

    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ONFT1155, ERC2981) returns (bool) {
