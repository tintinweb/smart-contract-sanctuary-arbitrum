  ...
  function reveal(
    ...
    for (uint256 i = 0; i < _lootBoxTokenIds.length; i++) {
      ...
      for (uint256 j = 0; j < _lootBoxAmount[i]; j++) {
        ...
        for (uint r = 0; r < holder.rewardTokenTypes.length; r++) {
          DispensedReward memory reward = result.rewards[i];
          holder.rewardTokenTypes[i] = (uint256)(reward.tokenType);
          holder.rewardTokenAddresses[i] = reward.token;
          holder.rewardTokenIds[i] = reward.tokenId;
          holder.rewardAmounts[i] = reward.amount;
        }
