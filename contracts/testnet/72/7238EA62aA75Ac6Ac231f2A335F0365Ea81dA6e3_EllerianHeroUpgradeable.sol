...
contract EllerianHeroUpgradeable is Ownable {
  ...
  address private eventAttributeChangeAddress;
  IVRFHelper vrfAbi;

  /**
   * Allows the owner to register a event smart contract
   * at will be allowed to change attributes.
   * (this way all attributes changes, if any, will be made transparent)
   */
  function SetEventAddress (address _eventAttributeAddress) external onlyOwner {
    eventAttributeChangeAddress = _eventAttributeAddress;
  }
   ...
   */
  function UpdateAttributesThroughEvent(uint256 _tokenId, uint256 _str, uint256 _agi, uint256 _vit, uint256 _end, uint256 _intel, uint256 _will) external {
    require(msg.sender == eventAttributeChangeAddress, "21");
    ...
    emit AttributeChange(_tokenId, _str, _agi, _vit, _end, _intel, _will);
  }
   ...
   */
  function ResetHeroExperience(uint256 _tokenId) external {
      require(_approvedAddresses[msg.sender], "18");
      heroExperience[_tokenId] = experience_needed[heroLevel[_tokenId] - 1];
  }
  ...
  */
  ...
  function GetAttributeRarity(uint256 _tokenId) external view returns (uint256) {
    ...
    for (uint i = 0; i < attributesRarity.length; i++) {
      if (_totalAttributes < attributesRarity[i]) {
        return i;
      }
    }
  ...
  function SetHeroLevel (uint256 _tokenId, uint256 _level) external {
    ...
    emit LevelChange(_tokenId, _level, 0, 0);
  }
  ...
  address _feesAddress, address _vrfAddress) external onlyOwner {
  ...
  /**
    ...
    * Must be called directly from a wallet.
    */  
  function AttemptHeroUpgrade(uint256 tokenId, uint256 goldAmountInEther, uint256 tokenAmountInEther) public {
      require (IERC721(minterAddress).ownerOf(tokenId) == tx.origin, "22");
      require(msg.sender == tx.origin, "9");

      // Transfer $MEDALS
      uint256 correctAmount = goldAmountInEther * 1e18;
      IERC20(goldAddress).transferFrom(tx.origin, feesAddress, correctAmount);

      // Transfer $ELLERIUM
      correctAmount = tokenAmountInEther * 1e18;
      if (correctAmount > 0) {
      IERC20(tokenAddress).transferFrom(tx.origin, feesAddress, correctAmount);
      }

      // Attempts the upgrade.
      uint256 level = updateLevel(tokenId, goldAmountInEther, tokenAmountInEther);

      emit Upgrade(tx.origin, tokenId, level);
  }
  ...
  function updateLevel(uint256 _tokenId, uint256 _goldAmountInEther, uint256 _tokenAmountInEther) internal returns (uint256) {
      ...
      if (randomNumber >= fail_probability[heroLevel[_tokenId]]) {
        ...
        emit LevelChange(_tokenId, heroLevel[_tokenId], _goldAmountInEther, _tokenAmountInEther);
      }
  ...
  // add upgrade event for emit, check this emit in the website to see if it'll be called
  event Upgrade(address _from, uint256 _tokenid, uint256 _level);
  event LevelChange(uint256 _tokenid, uint256 _level, uint256 _gold, uint256 _token);
  event AttributeChange(uint256 _tokenId, uint256 _str, uint256 _agi, uint256 _vit, uint256 _end, uint256 _intel, uint256 _will);
}
