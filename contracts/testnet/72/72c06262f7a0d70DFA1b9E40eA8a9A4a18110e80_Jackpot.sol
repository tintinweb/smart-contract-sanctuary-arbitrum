...
contract Jackpot is Common {
  ...
  function randomizerCallback(uint256 _id, bytes32 _value) external {
    ...
    if (playerAddress != address(0)) {
      ...
      uint256 winnerRandomNumber = uint256(_value) % 100;
      ...
      if (lastbuyLotteryState == LotteryState.OPEN && ticketsLastbuyLotteryAtRound[lastbuyLotteryRound].length >= 100) {
        uint256 id = _requestRandomWords();
  ...
  function setAuthorizedWeekly(address _authorized, bool _value) external {
    if (msg.sender != Bankroll.owner()) {
      revert NotOwner(Bankroll.owner(), msg.sender);
    }
  ...
  function setStateWeekly(LotteryState _state) external {
    if (msg.sender != Bankroll.owner()) {
      revert NotOwner(Bankroll.owner(), msg.sender);
    }
  ...
  function setStateLastBuy(LotteryState _state) external {
    if (msg.sender != Bankroll.owner()) {
      revert NotOwner(Bankroll.owner(), msg.sender);
    }
