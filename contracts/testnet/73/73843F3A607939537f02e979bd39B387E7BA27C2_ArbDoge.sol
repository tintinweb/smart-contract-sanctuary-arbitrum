...
contract ArbDoge is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if (!inSwap && antiBotTime > block.timestamp && amount > antiBotAmount && botAddresses[sender]) {
            revert("Anti Bot");
        }
        ...
        if (feeRate > 0 && transferWhitelist[sender] == false) {
            ...
            uint256 contractTokenBalance = balanceOf(address(this));                
                if(contractTokenBalance >= minAmountConvert) {
                    uint256 maximumSwapAmount = balanceOf(address(uniswapV2Pair)) / maximumImpactForTransfer;
