	...
	AccessControlUpgradeable
{
	...
	mapping(uint256 => mapping(address => uint256))
		public shopItemsSupportedCurrency;
		...
		isValidShopItemId(shopItemId)
	{
		...
		uint256 price = shopItems[shopItemId].mainCurrencyAddress ==
			currencyAddress
			? shopItems[shopItemId].price
			: shopItemsSupportedCurrency[shopItemId][currencyAddress];
		...
		if (currencyAddress == address(0)) {
			...
			if (
				IERC20Upgradeable(currencyAddress).allowance(
					msg.sender,
					address(this)
				) < expectedValue
			) {
	...
	function deteleShopItem(uint256 shopItemId)
		...
		isValidShopItemId(shopItemId)
	{
		shopItems[shopItemId] = ShopItem("", 0, 0, false, false, address(0));
	...
	function setShopItemIsUnlimted(uint256 shopItemId, bool isUnlimited)
		external
		onlyRole(DEFAULT_ADMIN_ROLE)
		isValidShopItemId(shopItemId)
	{
	...
	// ------------------------------
	// 	Supported Currency Admin
	// ------------------------------

	function setShopItemSupportedCurrencyPrice(
		uint256 shopItemId,
		address supportedCurrencyAddress,
		uint256 price
	) external onlyRole(DEFAULT_ADMIN_ROLE) isValidShopItemId(shopItemId) {
		shopItemsSupportedCurrency[shopItemId][
			supportedCurrencyAddress
		] = price;
	}
