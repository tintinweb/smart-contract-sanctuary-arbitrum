...
contract Mased is ERC20 {
    using SafeMath for uint256;
    uint256 public constant MAX_TOTAL_SUPPLY = 334993669991137048755964415475;

    constructor(uint256 _initialSupply) ERC20("Mased Token", "MASED") {
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(
            ...
            "MasedToken: Max total supply exceeded"
        );
        _mint(_to, _amount);
    }

    // Safe Mased transfer function
    function safeOmoTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 masedBal = balanceOf(address(this));
        if (_amount > masedBal) {
            _transfer(address(this), _to, masedBal);
        } else {
