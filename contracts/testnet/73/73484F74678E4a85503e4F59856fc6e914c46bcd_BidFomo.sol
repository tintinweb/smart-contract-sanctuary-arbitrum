...
contract BidFomo is Caller {
    ...
    event ClaimReward(uint256 indexed period, address indexed winner, uint256 rewards);
    event Bid(uint256 indexed period, address indexed player, uint256 amount);

    address public immutable WETH;
    uint256 public capTime = 1 days;
    uint256 public constant lowerTime = 10 minutes;
    uint256 public rewardRateInPool = 60; //60%
    uint256 public bidGrowthTime = 60; // second
    uint256 public tradeTime = 30; // second
    uint256 public bidPriceGrowthRate = 3; // 3%
    uint256 public initialPrice;

    Counters.Counter private _tokenIds;
    IERC20 public boboToken;
    IERC20 public veboboToken;
    Pool public pool;
    ...
    constructor(
        address _weth,
        address _bobo,
        address _veboboToken,
        address _devAddress
    ) {
        WETH = _weth;
        boboToken = IERC20(_bobo);
        veboboToken = IERC20(_veboboToken);
        devAddress = _devAddress;
        _addCaller(_msgSender());
        _addCaller(devAddress);
        _addCaller(_bobo);
        _addCaller(_veboboToken);
    }

    function start(uint256 _initialPrice) external {
        _onlyManger();
        paused = false;
    ...
    function setCapTime(uint256 _capTime) external {
        _onlyManger();
        capTime = _capTime;
    }
    ...
    function _inProgress() internal view returns (bool) {
    ...
    function previous() external view returns (HistoryPool memory) {
        return _previous();
    }

    function _previous() internal view returns (HistoryPool memory pre) {
    ...
    function balance(
        address user
    ) external view returns (uint256 totalAmount, uint256 availableAmount) {
        return _balance(user);
    }

    function _balance(
        address user
    ) internal view returns (uint256 totalAmount, uint256 availableAmount) {
    ...
    function withdraw() external {
        (uint256 totalAmount, uint256 availableAmount) = _balance(_msgSender());
        ...
        IBonusPool(address(veboboToken)).burn(_msgSender(), totalAmount);
        delete userBids[_msgSender()];
    }

    function bid(string memory message) external {
        require(_inProgress(), "This period not start");
        require(
            bytes(message).length > 0 && bytes(message).length <= 26,
            "Message: max 26 characters"
        );
        uint256 curBidding = (pool.bidding * (100 + bidPriceGrowthRate)) / 100;
        require(
            boboToken.balanceOf(_msgSender()) > curBidding,
        ...
        emit Bid(pool.period, _msgSender(), curBidding);

        pool.bidding = curBidding;
        pool.bidder = _msgSender();
        pool.message = message;

        pool.endTime += (pool.endTime - pool.startTime + bidGrowthTime) >
            pool.capTime
            ? 0
            : bidGrowthTime;

        IBonusPool(address(veboboToken)).mint(_msgSender(), curBidding);
        userBids[_msgSender()].push(
            UserBid({amount: curBidding, startAt: block.timestamp})
        );
    }

    // trade amount must be greater than $100
    function injectRewards(uint8 source, uint256 usd) external {
        require(_isCaller(_msgSender()), "not caller");
        require(_inProgress(), "not progress");
    ...
    function _claimReward() internal {
        require(
            !_inProgress() && _previous().period != pool.period,
            "not ended"
        );
        uint256 reward = pool.bidder != address(0)
            ? ((_wethBalance(address(this)) * rewardRateInPool) / 100)
            : 0;
        if (reward > 0) {
            TransferHelper.safeTransfer(address(WETH), pool.bidder, reward);
        }
        emit ClaimReward(pool.period, pool.bidder, reward);
        historyPool.push(
            HistoryPool({
                period: pool.period,
                winner: pool.bidder,
...
abstract contract Caller is Dev {
    ...
    modifier onlyCaller() {
        require(_isCaller(_msgSender()), "caller is not the owner or dev");
        _;
    }

    function isCaller(address addr) public view returns (bool) {
        return _isCaller(addr);
    }
    ...
    function addCaller(address addr) external returns (bool) {
        _onlyManger();
    ...
    function removeCaller(address addr) external returns (bool) {
        _onlyManger();
