...
// File: contracts/uyux/Soulbound.sol
...
contract SoulBound is ERC721, ERC721URIStorage {
    ...
    constructor() ERC721("UXUY SoulBound", "UXSBT") {}

    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, ERC721URIStorage) returns (bool) {
        return super.supportsInterface(interfaceId);
    }

    function register(address owner, string calldata name) external {
        _mint(owner,name);
    }

    function bulkRegister(address[] memory owners, string[] calldata names ) external {
        require(owners.length == names.length, "address.len must equal name.len ");
        for (uint256 i = 0; i < names.length; i++) {
           _mint(owners[i],names[i]);
        }
    }

    function nameOfId(uint256 tokenId) external view returns (string memory){
        require(_exists(tokenId), "ERC721Metadata: NAME query for nonexistent token");
        return bytes32ToString(bytes32(tokenId));
    }
    ...
    function _mint(address to, string calldata name) internal {
        bytes32 label = keccak256(bytes(name));
        uint256 tokenId = uint256(label);
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        ownerToIds[to].push(tokenId);
        emit NameRegistered(name, tokenId, to);
    }
    ...
    function isOwnerExists(address key) public view returns (bool) {
         if(ownerToIds[key].length > 0){
             return true;
         }
         return false;
    }
    ...
    function ownerOfName(string calldata name) public view returns (address) {
        bytes32 label = keccak256(bytes(name));
        uint256 tokenId = uint256(label);
