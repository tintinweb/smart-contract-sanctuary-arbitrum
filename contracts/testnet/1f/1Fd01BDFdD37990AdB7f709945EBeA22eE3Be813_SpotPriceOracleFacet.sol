...
import {InvalidAddress, InvalidObservationsTimestamp, PriceOutOfRange, LengthMismatch} from "../errors/GenericErrors.sol";

contract SpotPriceOracleFacet is ISpotPriceOracle {
    struct SpotOracleParam {
        address asset; // Asset address
        uint256 price; // Asset price in 18 decimals
        uint256 observationsTimestamp; // when were observations made offchain
    }
    error MaxStalePeriodIsZero();
    error TokenFeedConfigsLengthIsZero();
    error AnchorRatioMismatch(uint256 min, uint256 lower, uint256 upper, uint256 max);
    ...
    event SpotPricePosted(address indexed asset, uint256 indexed price, uint256 startedAt, uint256 updatedAt);

    /// @notice Emit when a token config is updated
    event TokenFeedConfigUpdated(address indexed asset, address feed, uint256 maxStalePeriod);

    /// @notice Emit when a anchor ratio is updated
    event SpotPriceAnchorRatioUpdated(uint256 lowerBoundAnchorRatio, uint256 upperBoundAnchorRatio);
    ...
    /**
     * @notice Get the underlying price of a listed underlying token asset
    ...
    function getUnderlyingPrice(address asset) external view returns (uint256) {
        ...
        LibSpotPriceOracle.PriceDataItem memory priceData = l.priceData[asset];
        return priceData.price;
    }

    /**
     * @notice Get the chainlink price of a listed underlying token asset
     * @param asset Address of the asset
     * @return price Chainlink price in USDC, with 18 decimals of precision
     */
    function getChainlinkPriceInUsdc(address asset) external view returns (uint256) {
        return _getChainlinkPriceInUsdc(asset);
    }

    /// @notice 获取现货预言机配置
    function getTokenFeedConfig(address _underlying) external view returns (LibSpotPriceOracle.TokenFeedConfig memory) {
        LibSpotPriceOracle.Layout storage l = LibSpotPriceOracle.layout();
        return l.tokenFeedConfigs[_underlying];
    }

    /// @notice 获取现货预言机锚定比例
    function getSpotOracleAnchorRatio() external view returns (uint256, uint256) {
        LibSpotPriceOracle.Layout storage l = LibSpotPriceOracle.layout();
     ...
     * @param tokenFeedConfigs_ config array
     * @custom:access Only ADMIN_ROLE
     * @custom:error Zero length error thrown, if length of the array in parameter is 0
     */
    function setTokenFeedConfigs(LibSpotPriceOracle.TokenFeedConfig[] memory tokenFeedConfigs_) external {
        if (tokenFeedConfigs_.length == 0) revert TokenFeedConfigsLengthIsZero();
        uint256 numTokenFeedConfigs = tokenFeedConfigs_.length;
        for (uint256 i; i < numTokenFeedConfigs; ) {
            setTokenFeedConfig(tokenFeedConfigs_[i]);
            unchecked {
                ++i;
            }
        }
     ...
     * @param tokenFeedConfig_ Token config struct
     * @custom:access Only ADMIN_ROLE
     ...
     * @custom:event Emits TokenFeedConfigUpdated event on successfully setting of the token config
     */
    function setTokenFeedConfig(
        LibSpotPriceOracle.TokenFeedConfig memory tokenFeedConfig_
    ) public notNullAddress(tokenFeedConfig_.asset) notNullAddress(tokenFeedConfig_.feed) {
        LibAccessControlEnumerable.checkRole(Constants.ADMIN_ROLE);
        LibSpotPriceOracle.Layout storage l = LibSpotPriceOracle.layout();
        /* Fail if maxStalePeriod = 0 */
        if (tokenFeedConfig_.maxStalePeriod == 0) revert MaxStalePeriodIsZero();
        l.tokenFeedConfigs[tokenFeedConfig_.asset] = tokenFeedConfig_;
        emit TokenFeedConfigUpdated(tokenFeedConfig_.asset, tokenFeedConfig_.feed, tokenFeedConfig_.maxStalePeriod);
    }

    function setSpotPriceAnchorRatio(uint256 _lowerBoundAnchorRatio, uint256 _upperBoundAnchorRatio) external {
        LibAccessControlEnumerable.checkRole(Constants.ADMIN_ROLE);
        LibSpotPriceOracle.Layout storage l = LibSpotPriceOracle.layout();
        if (
        ...
        emit SpotPriceAnchorRatioUpdated(l.lowerBoundAnchorRatio, l.upperBoundAnchorRatio);
    }

    /**
     * @notice Manually set the price of a given asset
     * @param params 现货设置价格结构体数组
     */
    function setUnderlyingPrices(SpotOracleParam[] calldata params) external {
        uint256 numAssets = params.length;
        for (uint256 i; i < numAssets; ) {
            setUnderlyingPrice(params[i]);
            unchecked {
                ++i;
            }
        }
    ...
    /**
     * @notice Manually set the price of a given asset
     * @param param 现货设置价格结构体
     */
    function setUnderlyingPrice(SpotOracleParam calldata param) public notNullAddress(param.asset) {
        LibAccessControlEnumerable.checkRole(Constants.KEEPER_ROLE);
        LibSpotPriceOracle.Layout storage l = LibSpotPriceOracle.layout();
        uint256 latestTransmissionTimestamp = l.priceData[param.asset].transmissionTimestamp;
        ...
        // todo 上线时候去掉这个
        if (l.isEnable) {
            // check diff with chainlink price and univ3 twap price
            uint256 clPriceInUSDC = _getChainlinkPriceInUsdc(param.asset);
            if (!_isWithinAnchor(param.price, clPriceInUSDC)) {
                if (l.isEnable) {
                    ...
                    revert PriceOutOfRange(param.price, clPriceInUSDC);
                }
            }
        }

        l.priceData[param.asset] = LibSpotPriceOracle.PriceDataItem(
            param.price,
            param.observationsTimestamp,
            uint256(block.timestamp)
        );
        emit SpotPricePosted(param.asset, param.price, param.observationsTimestamp, block.timestamp);
    }
    ...
    function _getChainlinkPrice(
        ...
        LibSpotPriceOracle.Layout storage l = LibSpotPriceOracle.layout();
        LibSpotPriceOracle.TokenFeedConfig memory tokenConfig = l.tokenFeedConfigs[asset];
