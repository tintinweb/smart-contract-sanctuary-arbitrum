...
contract Account is IAccount, ReentrancyGuard {
    ...
    function tradeETH(string memory protocolName, bytes calldata data) external onlyAccountOwner nonReentrant {
        _trade(protocolName, data);
    }
    ...
    function _trade(string memory name, bytes calldata data) private {
        ...
        (bool success, bytes memory result) = protocol.call(data);
        if (!success) {
