...
import {IVRFCoordinatorV2, VRFCoordinatorV2Interface} from "../interfaces/IVRFCoordinatorV2.sol";
...
contract GameCore is ReentrancyGuard {
    ...
    uint16 public numConfirmations;
    ...
    modifier bankrollApproved(address tokenAddress) {
    ...
    function _setNumConfirmations(uint16 _numConfirmations) internal {
        numConfirmations = _numConfirmations;
    }

    function _setCallbackGasLimit(uint32 _value) internal {
    ...
    function _transferWager(
        ...
        uint256 wager,
        uint256 gasAmount
    ) internal bankrollApproved(tokenAddress) {
        if (wager == 0) {
            revert ZeroWager();
        }
        (bool suspended, uint256 suspendedTime) = Bankroll
            .isPlayerSuspendedInfo(msg.sender);
        if (suspended) {
    ...
    /**
     * @dev calculates in form of native token the premium fee charged by chainlink VRF
     * @return fee amount of premium fee
     */
    function getChainlinkPremiumFee() public view returns (uint256 fee) {
...
interface IBankRoll {
    ...
    function isPlayerSuspendedInfo(
        address player
...
import {GameCore, SafeERC20, IERC20, IBankRoll, IVRFCoordinatorV2, AggregatorV3Interface} from "./common/GameCore.sol";

/**
 * @title rock paper scissors game, players select an action and play against the VRF actions
 */

contract RockPaperScissors is GameCore {
    using SafeERC20 for IERC20;

    /**
     * @dev constructs RockPaperScissors instance
     * @param _bankroll bankroll address
     * @param _vrf vrf coordinator address
     * @param link_eth_feed link to native token price feed
     * @param _keyHash subscription account key hash value
     * @param _subscriptionId subscription id
     * @param _numConfirmations number of confirmations
     * @param _callbackGasLimit callback gas limit value
     */
    constructor(
        address _bankroll,
        address _vrf,
        ...
        notZeroAddress(_bankroll)
        notZeroAddress(_vrf)
        notZeroAddress(link_eth_feed)
    {
    ...
    /**
     * @dev set new subscription account keyHash value
     */
    ...
    /**
     * @dev set new subscription id value
     */
    function setSubscriptionId(uint64 value) external onlyOwner {
        _setSubscriptionId(value);
    }

    /**
     * @dev set new number of confirmations
     */
    function setNumConfirmations(uint16 value) external onlyOwner {
        _setNumConfirmations(value);
    }

    /**
     * @dev set new callback gas limit value
     */
    function setCallbackGasLimit(uint32 value) external onlyOwner {
        _setCallbackGasLimit(value);
    }
    ...
    mapping(address => RockPaperScissorsGame) internal rockPaperScissorsGames;
    mapping(uint256 => address) internal rockPaperScissorsIDs;
