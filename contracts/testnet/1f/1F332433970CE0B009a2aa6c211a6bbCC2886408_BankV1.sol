...
contract BankV1 is AccessControlEnumerable, Multicall {
    ...
    function grantHarvesterRole(
        ...
        grantRole(HARVESTER_ROLE, harvester);
    }

    function revokeHarvesterRole(
        address harvester
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        revokeRole(HARVESTER_ROLE, harvester);
    }
    ...
    /**
        ...
        - migrate exectuePayout() functionality in pachiSlot.sol to Bank.sol         
    ...
    /// @param execPayload Data which was passed back from `checker`.
    function performUpkeep(bytes calldata execPayload) external {
    ...
    function checker(
        ...
        if (
            ...
            canExec = true;
            execPayload = abi.encode(upkeepAction, tokenAddressData);
        }
    }
