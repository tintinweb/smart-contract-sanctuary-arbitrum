...
contract ContrastFactory is Owned {
    /// -----------------------------------------------------------------------
    /// Dependencies
    /// -----------------------------------------------------------------------
    ...
    event Deployment(address instance);

    /// -----------------------------------------------------------------------
    /// Immutable Storage
    /// -----------------------------------------------------------------------

    address internal immutable CONTRAST_ERC1155;

    /// -----------------------------------------------------------------------
    /// Construction
    /// -----------------------------------------------------------------------

    constructor(address _owner, address _contrastERC1155) Owned(_owner) {
        CONTRAST_ERC1155 = _contrastERC1155;
    }
    ...
    function spawn(
        ...
        bytes32 salt
    ) external virtual onlyOwner returns (address instance) {
        instance = CONTRAST_ERC1155.cloneDeterministic({
        ...
        ContrastERC1155(instance).initialize(owner);

        emit Deployment(instance);
    }

    /// -----------------------------------------------------------------------
    /// Read-only Market Helper Logic
    /// -----------------------------------------------------------------------

    /// @dev This function is constrained by gas limits and may revert in the future.
    function totalMarketsOpen(address erc1155)
        external
        view
        returns (uint256 openMarkets)
    {
        uint256 totalMarkets = ContrastERC1155(erc1155).totalMarkets();

        unchecked {
            for (uint256 i; i < totalMarkets; ++i) {
                (,, uint64 maturity,,,,,,,) =
                    ContrastERC1155(erc1155).markets(i);

                if (block.timestamp < uint256(maturity)) ++openMarkets;
            }
        }
    }

    /// @dev This function is constrained by gas limits and may revert in the future.
    function totalMarketsOpenForAccount(address erc1155, address account)
        external
        view
        returns (uint256 openMarkets)
    {
        ...
        unchecked {
            for (uint256 i; i < totalMarkets; ++i) {
                ...
                (uint256 id0, uint256 id1) =
                    ContrastERC1155(erc1155).idsForMarket(i);

                if (
                    block.timestamp < uint256(maturity)
                        && (
                            ContrastERC1155(erc1155).balanceOf(account, id0) > 0
                                || ContrastERC1155(erc1155).balanceOf(account, id1) > 0
                        )
                ) {
                    ++openMarkets;
                }
    ...
    function computeImpliedProbability(address erc1155, uint256 marketId)
        external
        view
        returns (uint256)
    {
        ...
        unchecked {
            (uint256 id0, uint256 id1) =
                ContrastERC1155(erc1155).idsForMarket(marketId);

            uint256 supply0 = ContrastERC1155(erc1155).totalSupply(id0);
            uint256 supply1 = ContrastERC1155(erc1155).totalSupply(id1);
            uint256 sum = supply0 + supply1;

            return supply0.divWadDown(sum);
        }
    }

    /// @notice Computes the potential ROI for a markets outcome, expressed using 18 decimal places.
    function computeOutcomeROI(address erc1155, uint256 marketId, bool outcome)
        external
        view
        returns (uint256 roi)
    {
        (, uint96 exchangeRate, uint64 maturity,,,,,,,) =
            ContrastERC1155(erc1155).markets(marketId);
        ...
        roi = outcome
            ? supply1.divWadDown(supply0).mulDivDown(95, 100)
            : supply0.divWadDown(supply1).mulDivDown(95, 100);
    }
    ...
    function predictDeterministicAddress(
        ...
        return CONTRAST_ERC1155.predictDeterministicAddress({
            data: abi.encodePacked(
                underlyingAsset,
                quoteBondAmount,
                quoteDisputeWindow,
    ...
    function predictInitCodeHash(
        ...
        return CONTRAST_ERC1155.initCodeHash({
            data: abi.encodePacked(
                underlyingAsset,
                quoteBondAmount,
                quoteDisputeWindow,
