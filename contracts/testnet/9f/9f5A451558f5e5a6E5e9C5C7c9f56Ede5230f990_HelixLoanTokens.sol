...
library HelixConfigOptions {
  enum Addresses {
    ...
    HelixConfig, // 6
    Go, // 7
    LoanImplementationRepository // 8
  }
}
...
import "../upgradeable/HelixBaseUpgradeablePausable.sol";

import {IHelixLoanTokens} from "../interfaces/IHelixLoanTokens.sol";
import {ERC721PresetMinterPauserdUpgradeSafe} from "../external/ERC721PresetMinterPauserdUpgradeSafe.sol";
...
/**
 * @title HelixLoanTokens
 * @notice HelixLoanTokens is an ERC721 compliant contract, which can represent
 *  investment shares of any of the borrower loan.
 * @author Helix
 */
contract HelixLoanTokens is IHelixLoanTokens, HelixBaseUpgradeablePausable, ERC721PresetMinterPauserdUpgradeSafe {  
  ...
  */
  ...
  function initialize(address owner, IHelixConfig _config) external initializer {
    require(
    ...
    __ERC721_init_unchained("Helix V2 loan Tokens", "HLX-V2-PT");
    __Pausable_init_unchained();
  ...
  function halt() external onlyRole(OWNER_ROLE) {
    paused() ? _unpause() : _pause();
  }

  function toggleAllowToTransfer() external onlyRole(OWNER_ROLE) {
    allowToTransfer = allowToTransfer ? false : true;
  }

  /// @inheritdoc IHelixLoanTokens
  function mint(
    MintParams calldata params,
    address to
  ) external virtual override onlyLoan whenNotPaused returns (uint256 tokenId) {
    address loanAddress = _msgSender();

    LoanInfo storage loan = loans[loanAddress];
    loan.totalPrincipalInvestment = loan.totalPrincipalInvestment.add(params.principalAmount);
  ...
  ) external virtual override onlyLoan whenNotPaused {
    TokenInfo storage token = tokens[tokenId];
    address loanAddr = token.loan;
    require(token.loan != address(0), "Invalid tokenId");
    require(_msgSender() == loanAddr, "Only the token's loan can redeem");

    LoanInfo storage loan = loans[loanAddr];
    loan.totalPrincipalRedeemed = loan.totalPrincipalRedeemed.add(principalRedeemed);
    require(loan.totalPrincipalRedeemed <= loan.totalPrincipalInvestment, "Cannot redeem more than we minted");
  ...
  /** @notice reduce a given loan token's principalAmount and principalRedeemed by a specified amount
   *  @dev this function is only intended for use as part of the v2.6.0 upgrade
   *    to rectify a bug that allowed users to create a loanToken that had a
   *    larger amount of principal than they actually made available to the
   *    borrower.  This bug is fixed in v2.6.0 but still requires past loan tokens
  ...
  function burn(uint256 tokenId) external virtual override whenNotPaused {
    ...
    bool fromTokenloan = _validLoan(_msgSender()) && token.loan == _msgSender();
    address owner = ownerOf(tokenId);
    require(canBurn || fromTokenloan, "ERC721Burnable: caller cannot burn this token");
    require(
      token.principalRedeemed == token.principalAmount,
      "Can only burn fully redeemed tokens"
    );
    _destroyAndBurn(owner, address(token.loan), tokenId);
  }
  ...
  function calculateTokenId(address loanAddress, address investor) public override view returns(uint256) {
    return uint256(keccak256(abi.encodePacked(loanAddress, investor, address(this))));
  }
  ...
  function getLoanInfo(address loan) external view override returns (LoanInfo memory) {
    return loans[loan];
  }

  /// @inheritdoc IHelixLoanTokens
  function onLoanCreated(address newloan) external override onlyHelixFactory {
    loans[newloan].created = true;
  }
  ...
  /// @inheritdoc IHelixLoanTokens
  function validLoan(address sender) public view virtual override returns (bool) {
    return _validLoan(sender);
  }
   ...
   *  0 for tokens created from a deposit, and could be non-zero for tokens created from a split
   * @param loanAddress loan to which the token belongs
  ...
  function _createToken(
    ...
    tokenId = calculateTokenId(loanAddress, mintTo);

    TokenInfo storage userTokenInfo = tokens[tokenId];

    if (userTokenInfo.loan == address(0)) {
      userTokenInfo.loan = loanAddress;
      userTokenInfo.principalAmount = 0;
      userTokenInfo.principalRedeemed = principalRedeemed;
      userTokenInfo.interestRedeemed = interestRedeemed;
    ...
    userTokenInfo.principalAmount +=  principalAmount;

    emit TokenPrincipalUpdated({
      owner: mintTo,
        loan: loanAddress,
        tokenId: tokenId,
        amount: userTokenInfo.principalAmount
    });
  }

  function _destroyAndBurn(address owner, address loan, uint256 tokenId) internal {
    delete tokens[tokenId];
    _burn(tokenId);
    emit TokenBurned(owner, loan, tokenId);
  }
  ...
  function _validLoan(address loanAddress) internal view virtual returns (bool) {
    return loans[loanAddress].created;
  }
  ...
  function loanTVL(address loanAddress) public view override returns(uint256) {
    LoanInfo storage loan = loans[loanAddress];
    return loan.totalPrincipalInvestment - loan.totalPrincipalRedeemed; 
  }
  ...
  function supportsInterface(
    ...
    override(
      ...
      IERC165Upgradeable,
      ERC721PresetMinterPauserdUpgradeSafe
    )
    returns (bool)
  {
  ...
  function _beforeTokenTransfer(
    ...
    uint256 tokenId,
    uint256 amount
  ) internal virtual override(ERC721PresetMinterPauserdUpgradeSafe) {
    if (from != address(0)) {
      require(allowToTransfer, "Not allow to transfer between investors!");
    }
    
    super._beforeTokenTransfer(from, to, tokenId, amount);
  }

  modifier onlyHelixFactory() {
    require(_msgSender() == config.helixFactoryAddress(), "Only Helix factory is allowed");
    _;
  }

  modifier onlyLoan() {
    require(_validLoan(_msgSender()), "Invalid loan!");
    _;
  }
}
 ...
 */
contract ERC721PresetMinterPauserdUpgradeSafe is
  Initializable,
  AccessControlUpgradeable,
...
interface IHelixLoanTokens is IERC721Upgradeable {
  struct TokenInfo {
    address loan;
    uint256 principalAmount;
    uint256 principalRedeemed;
  ...
  struct LoanInfo {
    uint256 totalPrincipalInvestment;
    uint256 totalPrincipalRedeemed;
    bool created;
  }

  function calculateTokenId(address loanAddress, address investor) external view returns(uint256);


  function loanTVL(address loanAddress) external view returns(uint256);

  /**
   * @notice Called by loan to create a debt position in a particular tranche and amount
   * @param params Struct containing the tranche and the amount
   * @param to The address that should own the position
   * @return tokenId The token ID (auto-incrementing integer across all loans)
   */
  function mint(MintParams calldata params, address to) external returns (uint256);

  /**
   * @notice Redeem principal and interest on a loan token. Called by valid loans as part of their redemption
   *  flow
   ...
   *  the redemption cannot cause the loan's total principal redeemed to exceed the loan's total minted
   *  principal
  ...
  /**
   * @notice Burns a specific ERC721 token and removes deletes the token metadata for loanTokens, BackerReards,
  ...
  /**
   * @notice Called by the HelixFactory to register the loan as a valid loan. Only valid loans can mint/redeem
   * tokens
   * @param newloan The address of the newly created loan
   */
  function onLoanCreated(address newloan) external;

  function getTokenInfo(uint256 tokenId) external view returns (TokenInfo memory);

  function getLoanInfo(address loan) external view returns (LoanInfo memory);

  /// @notice Query if `loan` is a valid loan. A loan is valid if it was created by the Helix Factory
  function validLoan(address loan) external view returns (bool);
  ...
  /**
   * @notice Mint event emitted for a new Tranchedloan deposit or when an existing loan token is
   *  split
   * @param owner address to which the token was minted
   * @param loan tranched loan that the deposit was in
  ...
  /**
   * @notice Redeem event emitted when interest and/or principal is redeemed in the token's loan
   * @param owner owner of the loan token
   * @param loan tranched loan that the token belongs to
   * @param principalRedeemed amount of principal redeemed from the loan
   * @param interestRedeemed amount of interest redeemed from the loan
   */
  event TokenRedeemed(
    address indexed owner,
    address indexed loan,
   ...
   * @param loan tranched loan that the token belongs to
   */
  event TokenBurned(address indexed owner, address indexed loan, uint256 indexed tokenId);
}
...
library HelixConfigHelper {
  ...
  function getHelixLoanTokens(IHelixConfig config) internal view returns (IHelixLoanTokens) {
    return IHelixLoanTokens(helixLoanTokensAddress(config));
  }
  ...
  function getLoanImplementationRepository(
    IHelixConfig config
  ) internal view returns (ImplementationRepository) {
    return
      ImplementationRepository(
        config.getAddress(uint256(HelixConfigOptions.Addresses.LoanImplementationRepository))
      );
  }
  ...
  function helixLoanTokensAddress(IHelixConfig config) internal view returns (address) {
    return config.getAddress(uint256(HelixConfigOptions.Addresses.HelixLoanTokens));
  }
...
library HelixErrors {
    ...
    string public constant LOAN_NOT_CLOSED = "2";
    string public constant UPDATED_LOAN_STATUS_IS_THE_SAME = "3";
    ...
    string public constant NOT_SATISFY_KYC_CONDITION_FOR_LOAN = "10";
...
import {HelixBaseUpgradeablePausable} from "../upgradeable/HelixBaseUpgradeablePausable.sol";
...
contract ImplementationRepository is HelixBaseUpgradeablePausable {
