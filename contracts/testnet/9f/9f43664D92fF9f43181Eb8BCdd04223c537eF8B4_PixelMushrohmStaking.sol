...
contract PixelMushrohmStaking is IPixelMushrohmStaking, PixelMushrohmAccessControlled {
    /* ========== DEPENDENCIES ========== */
    ...
    function setSporePowerPerWeek(uint256 _sporePowerPerWeek) external override onlyPolicy {
    ...
    function setStakingCost(uint256 _cost) external override onlyOwner {
        stakingCost = _cost;
    }
    ...
    {
        if (paymentToken != address(0) && stakingCost != 0) {
            IERC20(paymentToken).safeTransferFrom(msg.sender, address(this), stakingCost);
        }
