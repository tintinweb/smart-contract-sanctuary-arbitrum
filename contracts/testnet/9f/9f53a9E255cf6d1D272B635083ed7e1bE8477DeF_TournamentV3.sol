...
import "../../interfaces/IPrizeManagerV2.sol";
import "../../interfaces/ITournamentV3.sol";
import "../../interfaces/IJackpot.sol";

contract TournamentV3 is ReentrancyGuard, ITournamentV3, Ownable {
    ...
    modifier onlyOperator {
        require(gamePolicy.isOperator(msg.sender), "PredictionV3: !operator");
        _;
    }

    /* ========== CONSTRUCTOR ========== */

    constructor (IGamePolicy _gamePolicy, IFastPriceFeed _fastPriceFeed, IPrizeManagerV2 _prizeManager, address _feeReceiver, uint256 _currentGameId) {
    ...
    function join(uint256 _gameId) external nonReentrant {
        ...
        require(_gameInfo.status == GAME_STATUS.PENDING && _gameInfo.startTime > block.timestamp, "PredictionV3: started");
        require(!isJoinedGame[msg.sender][_gameId], "PredictionV3: joined");
        require(_gameInfo.nPlayers < _gameInfo.maxPlayers, "PredictionV3: enough player");
    ...
    function sponsor(uint256 _gameId, address _token, uint256 _amount) external {
        require(gameInfo[_gameId].status == GAME_STATUS.PENDING, "PredictionV3: game started");

        SponsorInfo storage _sponsorInfo = sponsorInfo[_gameId];
        uint256 _limit = gamePolicy.getSponsorLimit(_token);
        require(_limit > 0 && _amount >= _limit, "PredictionV3: !sponsor");
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        if (!_sponsorInfo.flag) {
            _sponsorInfo.flag = true;
        }
    ...
    function _executeGame(uint256 _gameId, uint256 _roundId, uint256 _startTime, address[] memory _players, uint256[] memory _predictions) internal {
        ...
        require(_gameInfo.currentRoundNumber + 1 == _roundId, "PredictionV3: !roundId");

        if (_roundId > _gameInfo.maxRounds && _roundId > 1) {
            _finishGame(_gameId, _players, 2);
            return;
        }
        _gameInfo.currentRoundNumber++;

        require(_gameInfo.startTime <= block.timestamp, "PredictionV3: !start");

        require(_validateInput(_gameId, _previousRoundId, _players), "PredictionV3: invalid data");

        if (_previousRoundId > 0) {
            require(_updateRound(_gameId, _previousRoundId), "PredictionV3: !update round");
            // start new round
            if (_startTime == 0) {
        ...
        if (_gameInfo.nPlayers >= _gameInfo.minPlayers || _previousRoundId > 0) {
            ...
            // not start
            _finishGame(_gameId, _players, 1);
        }
    }
    ...
    function _finishGame(uint256 _gameId, address[] memory _winners, uint256 _option) internal {
        GameInfo storage _gameInfo = gameInfo[_gameId];
        require(_gameInfo.status == GAME_STATUS.AVAILABLE || _gameInfo.status == GAME_STATUS.PENDING, "PredictionV3: !available");
    ...
    function _createPrize(uint256 _gameId, address[] memory _winners, address _proposer, uint256 _option) internal {
        ...
        if (_option == 2 || _option == 3) {
            ...
            if (sponsorInfo[_gameId].flag == true) {
                uint256[] memory _sizePrizeAmounts;
                SponsorInfo memory _sponsorInfo = sponsorInfo[_gameId];
                for (uint256 i = 0; i < _sponsorInfo.tokens.length; i++) {
    ...
    function createGame(
        ...
        require(_startTime > block.timestamp, "PredictionV3: !startTime");
        require(gamePolicy.isTargetToken(_targetToken), "PredictionV3: !target token");
        require(gamePolicy.isBuyInToken(_buyInToken), "PredictionV3: !buyIn token");
        address[] memory _new;
        currentGameId++;
    ...
    function executeGame(uint256 _gameId, uint256 _roundId, uint256 _startTime, address[] memory _players, uint256[] memory _predictions) external onlyOperator {
        require(!isHasData[_gameId][_roundId], "PredictionV3: have data");
        GameInfo storage _gameInfo = gameInfo[_gameId];
        require (_gameInfo.status != GAME_STATUS.CLOSED && _gameInfo.status != GAME_STATUS.CANCELED, "PredictionV3: closed");
    ...
    function updateGameData(uint256 _gameId, uint256 _roundId, address[] memory _players, uint256[] memory _predictions) external onlyOperator {
        require(isHasData[_gameId][_roundId], "PredictionV3: !have data");
        require (gameInfo[_gameId].status != GAME_STATUS.CLOSED && gameInfo[_gameId].status != GAME_STATUS.CANCELED, "PredictionV3: closed");
    ...
    function finishGame(uint256 _gameId, address[] memory _winners, bool _isConsoled) external onlyOperator {
        uint256 _roundId = gameInfo[_gameId].currentRoundNumber;
        if (_isConsoled) {
            bool _flag = false;
            ...
            require(!_flag, "PredictionV3: winners not matching");
            if (_roundId > 0) {
                _roundId -= 1;
            }
        }
        require(_validateInput(_gameId, _roundId, _winners), "PredictionV3: invalid winners");
        if (_isConsoled) {
            _finishGame(_gameId, _winners, 3);
        } else {
            _finishGame(_gameId, _winners, 2);
        }
    }

    function changeSponsoredGameId(uint256 _oldGameId, uint256 _newGameId) external onlyOperator {
        require(gameInfo[_oldGameId].status == GAME_STATUS.CANCELED, "PredictionV3: !canceled");
        sponsorInfo[_newGameId] = sponsorInfo[_oldGameId];
        sponsorInfo[_oldGameId].flag = false;
        emit SponsorChanged(_oldGameId, _newGameId, msg.sender);
    }


    function setWinningFee(uint256 _fee) external onlyOwner {
        require(_fee < ONE_HUNDRED_PERCENT, "PredictionV3: !fee");
    ...
    function setSystemFeeRatio(uint256 _creatorRatio, uint256 _operationRatio) external onlyOwner {
        require(_creatorRatio + _operationRatio == BASE_RATIO, "PredictionV3: !data");
...
interface IPrizeManagerV2 {
    function createPrize(uint256, address[] memory, address, uint256) external;
