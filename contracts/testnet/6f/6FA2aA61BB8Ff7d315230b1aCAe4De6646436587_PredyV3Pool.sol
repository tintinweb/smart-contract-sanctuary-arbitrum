...
contract ProductVerifier is IProductVerifier, Ownable {
    ...
    function openPosition(
        ...
        if (!_params.isLiquidationRequired) {
            PositionVerifier.verifyPosition(pool.getPosition(_vaultId), _params.proofs, true, threshold);
        }
...
contract PredyV3Pool is IPredyV3Pool, Ownable, Constants {
    ...
    function openPosition(
        ...
        extraVaultParams[vaultId].isLiquidationRequired = _params.isLiquidationRequired;
        ...
        (uint256 amount0, uint256 amount1) = productVerifier.openPosition(vaultId, _params);
    ...
    function createOrGetVault(uint256 _vaultId) internal returns (uint256 vaultId, VaultLib.Vault storage) {
        if (_vaultId == 0) {
            ...
            vaultId = _vaultId;
            require(extraVaultParams[vaultId].owner == msg.sender, "P4");
        }
...
import "lib/v3-core/contracts/interfaces/IUniswapV3Pool.sol";
import "lib/v3-core/contracts/libraries/Tick.sol";
import "lib/openzeppelin-contracts/contracts/access/Ownable.sol";
import "./Constants.sol";
