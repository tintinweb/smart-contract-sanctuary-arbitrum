...
// MODIFIERS
    ...
    modifier onlyInactive(uint256 tokenID) {
        require (characterContract.getBlocksUntilActivityEnds(tokenID) == 0,"Character still active");
    ...
    modifier onlyOwnerOfToken(uint256 tokenID) {
        require (characterContract.ownerOf(tokenID) == msg.sender,"Only the owner of the token can perform this action");
        _; // Call the actual code
    }  
    ...
    function restoreHealth(uint256 tokenID, uint256 itemSet, uint256 itemID, uint256 total)
        external
        nonReentrant
        onlyOwnerOfToken(tokenID)
        onlyInactive(tokenID)
    {
        require(itemsContract.balanceOf(msg.sender, itemID) >= total, "You don't have that item in your wallet");
        // Return the Item Modifiers values
        (uint256 amountToIncrease, uint256 amountToBurn) = getItemModifier(itemSet, itemID);
        ...
        characterContract.increaseStat(tokenID, (amountToIncrease*total).toUint16() ,5);  
        // Burn the health item(s)                     
        itemsContract.burn(msg.sender, itemID, amountToBurn * total);
    }
    ...
    /// @param itemID item ID
    /// @param total number of items to use
    function restoreMorale(uint256 tokenID, uint256 itemSet, uint256 itemID, uint256 total)
        external
        nonReentrant
        onlyOwnerOfToken(tokenID)
        onlyInactive(tokenID)
    {
        ...
        characterContract.increaseStat(tokenID, (amountToIncrease*total).toUint16() ,6);
        ...
        onlyInactive(tokenID)
    {
        ...
        for (uint256 i = 0; i < total;) {
            ...
            characterContract.boostStat(tokenID, amount[i] ,statIndex[i]);
            // Increment counter
            unchecked { i++; }
        }
    }
...
// VIEWS
    ...
    {
        require(itemSetsContract.getItemSet(itemSet).length > 0,"No items found in this set");
