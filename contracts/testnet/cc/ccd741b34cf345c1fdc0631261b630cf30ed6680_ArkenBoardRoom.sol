    ...
    PausableUpgradeable
{
    ...
    function setPenaltyFeeWallet(
        address _penaltyFeeWallet
    ) external onlyOwner {}
    ...
    function earned(address _member) public view returns (uint256) {
        ...
        return _reward.balanceOf(address(this));
    }
    ...
    function _claimReward(address _to) internal {
        IERC20 _reward = IERC20(reward);
        uint256 _earned = _reward.balanceOf(address(this));
    ...
    function _increaseStake(address _to, uint256 _amount) internal {
        uint256 lockDuration = EPOCH_DURATION * currentEpoch;
        _claimReward(_to);
        emit Deposited(_to, _amount, lockDuration, block.timestamp);
    }
    ...
    function emergencyWithdraw() external {
        uint256 _amount = IERC20(reward).balanceOf(address(this));
        _claimReward(msg.sender);
        emit EmergencyWithdrawn(msg.sender, _amount, block.timestamp);
    }
