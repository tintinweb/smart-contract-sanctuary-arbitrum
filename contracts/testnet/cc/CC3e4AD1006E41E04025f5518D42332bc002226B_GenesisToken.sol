...
import "./IERC721ATransfersCallback.sol";

contract GenesisToken is ERC721Drop {
    address public transfersCallback;
    ...
    {
        transfersCallback = address(0);
    }

    function getLastTransferTimestamp(
        uint256 _tokenId
    ) public view virtual returns (uint64 timestamp) {
        return _ownershipOf(_tokenId).startTimestamp;
    }

    function setTransfersCallback(
        address _transferCallback
    ) external onlyOwner {
        transfersCallback = _transferCallback;
    }
    ...
    function _afterTokenTransfers(
        ...
        if (transfersCallback != address(0)) {
            IERC721ATransfersCallback(transfersCallback).afterTokenTransfers(
                from,
                to,
                startTokenId,
                quantity
...
interface IERC721ATransfersCallback {
    function estimateFees(
        ...
        uint256 startTokenId,
        uint256 quantity
    ) external view returns (uint, uint, bytes memory, bytes memory);
