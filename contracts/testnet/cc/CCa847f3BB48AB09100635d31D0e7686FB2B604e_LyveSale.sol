...
contract LyveSale is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using ECDSA for bytes32;
    IERC20 public immutable LYVE;
    address public immutable esLYVE;
    uint256 public immutable lyveTotalSaleAmount;
    ...
    constructor(
        ...
        LYVE = _LYVE;
        lyveTotalSaleAmount = _lyveTotalSaleAmount;
    ...
    modifier onlywhitelist() {
        require( whitelist[msg.sender] == true,"not white list");
        _;
    }
   ...
   function addWhiteList(address[] memory addressList) external onlyOwner {
    ...
    function start() external onlyOwner {
        ...
        started = true;
        LYVE.safeTransferFrom(msg.sender, address(this), lyveTotalSaleAmount + lyveTotalSaleAmount*20/100);
    }

    function commit() external payable onlywhitelist nonReentrant {
        require(
            started &&
        ...
        require(
            totalCommitments + msg.value <= ethersToRaise,
            "Can only deposit Ether more than ethersToRaise."
        );
        require(
            minCommit <= userCommitments[msg.sender] + msg.value &&
                userCommitments[msg.sender] + msg.value <= maxCommit,
            "Commitment amount is outside the allowed range."
        );
        userCommitments[msg.sender] += msg.value;
        totalCommitments += msg.value;
        emit Commit(msg.sender, msg.value);
    }

    function simulateClaim(address account) external view returns (uint256, uint256) {

        if (userCommitments[account] == 0) return (0, 0);

        uint256 ethersToSpend = Math.min(
            userCommitments[account],
            (userCommitments[account] * ethersToRaise) / totalCommitments
        );
        uint256 ethersToRefund = userCommitments[account] - ethersToSpend;
        uint256 tokensToReceive = (lyveTotalSaleAmount * ethersToSpend) /
            ethersToRaise;
    ...
    function claim() external nonReentrant returns (uint256, uint256) {
        ...
        uint256 ethersToSpend = Math.min(
            userCommitments[msg.sender],
            (userCommitments[msg.sender] * ethersToRaise) / totalCommitments
        );
        uint256 ethersToRefund = userCommitments[msg.sender] - ethersToSpend;
        uint256 tokensToReceive = (lyveTotalSaleAmount * ethersToSpend) /
            ethersToRaise;
        userClaimed[msg.sender] = true;

        LYVE.safeTransfer(msg.sender, tokensToReceive);
        uint256 receiveEsLyveAmount = tokensToReceive*20/100;
        LYVE.safeApprove(esLYVE,receiveEsLyveAmount);
        EsLYVE(esLYVE).depositEsLYVE(receiveEsLyveAmount, msg.sender);
    ...
    function finish() external onlyOwner {
        ...
        if (ethersToRaise > totalCommitments) {
            uint256 tokensToBurn = (lyveTotalSaleAmount *
                (ethersToRaise - totalCommitments)) / ethersToRaise;
            uint256 esLyveToBurn = tokensToBurn*20/100;
            uint256 totalToBurn = tokensToBurn + esLyveToBurn;
            uint256 balance = LYVE.balanceOf(address(this));
            if(balance < totalToBurn){
                LYVE.safeTransfer(burnAddress, balance );
            }else{
                LYVE.safeTransfer(burnAddress, totalToBurn );
            }
        }
