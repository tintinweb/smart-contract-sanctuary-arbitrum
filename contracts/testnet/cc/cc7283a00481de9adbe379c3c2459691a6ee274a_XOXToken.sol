...
contract XOXToken is ERC20XOXBase {
    ...
    function burnBridge(
        ...
        _burnBridge(account, amount, chainId);
    }
...
contract ERC20XOXBase is Context, IERC20, IERC20Metadata, Ownable {
     ...
     * - `account` must have at least `amount` tokens.
     */
    function _burnBridge(address account, uint256 amount, uint256 chainId) internal {
        require(account != address(0), "ERC20: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "ERC20: burn amount exceeds balance");
        unchecked {
        ...
        _totalSupply -= amount;
        emit BurnBridge(account, address(0), amount, chainId);
    }
     ...
     */
    event BurnBridge(address indexed from, address indexed to, uint256 value, uint256 chainId);
}
