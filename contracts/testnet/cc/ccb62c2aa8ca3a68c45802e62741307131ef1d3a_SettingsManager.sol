...
contract SettingsManager is ISettingsManager, Ownable, Constants {
    ...
    function getFees(
        ...
        if (_isApplyBorrowFee) {
            fee += getBorrowFee(_position.indexToken, _loanDelta, _position.lastIncreasedTime);
        }

        fee = getDiscountFee(_position.owner, fee);

        if (!_isApplyFundingFee) {
            return int256(fee);
        }

        int256 fundingFee = getFundingFee(_position.indexToken, _position.isLong, _position.size, _position.entryFunding);

        if (fundingFee == 0) {
            return int256(fee);
        }

        return int256(fee) + fundingFee;
    }
    ...
    function getFundingRate(address _indexToken, address _collateralToken) public view override returns (int256) {
        ...
        uint256 fundingRate = (diff * fundingRateFactor[_indexToken] * basisFundingRateFactor) 
            / vaultPoolAmount;
    ...
    function updateFunding(address _indexToken, address _collateralToken) external override {
        ...
        if (lastFundingTimes[_indexToken] != 0) {
            int256 fundingRate = getFundingRate(_indexToken, _collateralToken);
