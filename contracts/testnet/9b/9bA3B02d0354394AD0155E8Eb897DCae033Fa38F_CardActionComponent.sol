...
uint256 constant ID = uint256(
    keccak256("game.piratenation.cardactioncomponent.v1")
);
 ...
 * @dev Card Action Component is a component that defines data for a specific action associated with card.
 */
contract CardActionComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getValue(
        ...
        value = CardActionComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = CardActionComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete CardActionComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete CardActionComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
