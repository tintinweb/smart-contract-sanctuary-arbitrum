...
contract GNSPriceAggregatorV6_3 is ChainlinkClient, TWAPPriceGetter {
    ...
    // State
    address[] public nodes; //预言机节点合约？

    mapping(uint => Order) public orders; //只有pair等信息，没有addr信息；
    mapping(bytes32 => uint) public orderIdByRequest; //key就是requestId
    ...
    function getPrice(
        ...
        Chainlink.Request memory linkRequest = buildChainlinkRequest(
            job,
            address(this),
            this.fulfill.selector //回调函数是固定的，并且没有输入参数，说明回调会执行所有的订单；
        );

        linkRequest.add("from", from); //价格对的from, to;
	...
	//当喂价为0，或者喂价与chainlink的绝对差值小于maxDeviationP；
        if(price == 0
        || (price >= feedPrice ?
            price - feedPrice :
            feedPrice - price
            ...
            answers.push(price);
            //够3个预言机报了合格的价格，就开始执行交易；//后续再来的预言机喂价，都不会再执行了，只会更新价格；
            if(answers.length == minAnswers){
                CallbacksInterfaceV6.AggregatorAnswer memory a;
                //平仓需要的主要信息；
                a.orderId = orderId;
                a.price = median(answers);  //获取价格中位数；
                a.spreadP = pairsStorage.pairSpreadP(r.pairIndex);  //gov设置的每个交易对的点差；
                ...
                }else if(r.orderType == OrderType.LIMIT_OPEN){  //限价单开仓委托；
                    c.executeNftOpenOrderCallback(a);

                }else if(r.orderType == OrderType.LIMIT_CLOSE){   //限价单平仓委托，强平，sl, tp；
                    c.executeNftCloseOrderCallback(a);

                }else{
                    c.updateSlCallback(a);  //更新sl, tp;
                }
            ...
            emit PriceReceived( //这个事件不是在每次喂价都会发生，只有有效喂价的时候，才会发生；
                requestId,
                orderId,
                msg.sender,
                r.pairIndex,
