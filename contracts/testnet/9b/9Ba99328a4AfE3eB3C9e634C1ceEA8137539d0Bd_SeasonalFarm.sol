    ...
    ReentrancyGuardUpgradeable
{
    ...
    event MaxDepositChanged(uint256 maxDeposit);
    ...
    uint256 public endFundraisingTime;
    ...
    uint256 tokenPriceAtLastPerformanceFeeMint;
    uint256 latestManagementFeeMintAt;
    ...
    function initialize(
        ...
        endFundraisingTime = block.timestamp + 3 days;
    ...
    function setFarmManagement(address _farmManagement) external override returns (bool) {
        ...
        emit FarmManagementSet(_farmManagement, msg.sender);
    ...
    function setMaxDeposit(uint256 _amount) external onlyManager {
        ...
        maxDeposit = _amount;
        emit MaxDepositChanged(_amount);
    }
        ...
        returns (uint256)
    {
        ...
        if(entranceFee > 0) {
            ...
            emit EntranceFeeMinted(msg.sender, to, entranceFee, managerFee, adminFee);
        }

        _mint(msg.sender, liquidityMinted);

        emit Deposited(msg.sender, _asset, _amount, usdAmount, liquidityMinted);

        return liquidityMinted;
    }

    function withdraw(address _asset, uint256 _shareAmount)
        external whenNotPaused nonReentrant
    {
        ...
        if(exitFee > 0) {
            _shareAmount = _shareAmount.sub(exitFee);
            address to = manager();
            (uint256 managerFee, uint256 adminFee) = _transferFeeToManager(msg.sender, to, exitFee);
            emit ExitFeeTransferred(msg.sender, to, exitFee, managerFee, adminFee);

            // Penalty fee
            uint256 _penaltyFee = _availablePenaltyFee(exitFee);
            if(_penaltyFee > 0) {
                IHasProtocolInfo protocolInfo = IHasProtocolInfo(factory);
                require(transferFrom(msg.sender, protocolInfo.treasury(), _penaltyFee), "token transfer failed");
                emit PenaltyFeeTransferred(msg.sender, _penaltyFee);
        ...
        // TODO: transfer the asset

        emit Withdrawal(msg.sender, _asset, _shareAmount, portion, assetAmount);
    }

    /// @notice transfer asset to the operator
    function invest(address _asset, uint256 _amount)
        external onlyManager whenNotPaused nonReentrant
    {
        if (block.timestamp <= endFundraisingTime) revert BelowMin(endFundraisingTime, block.timestamp);
        ...
        uint256 _newOrder = orderCounter.current();
        orderCounter.increment();

        orders[_newOrder] = Order(_asset, _amount, true);

        emit Invested(msg.sender, _newOrder, _asset, _amount);
    }

    function divest(uint256 _orderId)
        external onlyManager whenNotPaused nonReentrant
    {
        ...
        if(order.isOpenning) {
            ...
            IERC20Upgradeable asset = IERC20Upgradeable(order.asset);
            uint256 allowanceAmount = asset.allowance(operator, address(this));
            if(allowanceAmount == 0) revert ZeroAmount();
            asset.transferFrom(operator, address(this), allowanceAmount);

            emit Divested(msg.sender, _orderId, order.asset, allowanceAmount);
        }
    }
        ...
        external onlyManager whenNotPaused nonReentrant
    {
        ...
        if(order.isOpenning) {
            ...
            emit Liquidated(msg.sender, _orderId);
        }
    }
