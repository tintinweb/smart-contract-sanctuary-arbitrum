...
import "../interfaces/IHelixLoan.sol";
...
/**
 * @title HelixFactory
 * @notice Contract that allows us to create other contracts, such as CreditLine
 * @author Helix
 */

contract HelixFactory is HelixBaseUpgradeablePausable {
    using HelixConfigHelper for IHelixConfig;
    
    IHelixConfig public config;

    event LoanCreated(IHelixLoan indexed loan, address indexed loanWallet, address indexed loanManager, address borrower, uint256 dbPrjId);
    ...
    /**
     * @notice Allow issuer to create a new loan
     * @param _borrower The borrower for whom the CreditLine will be created
     * @param _loanWallet The Loan wallet for Loan contract
     * @param _token The token that investor need to deposit
     ...
     * @param dbPrjId Loan index in database
     */
    function createLoan(
        address _borrower,
        address _loanWallet,
        ...
        uint256 dbPrjId
    ) external onlyProtocolAdmin returns (IHelixLoan) {
        address[] memory owners = _getOwnersForLoanDeployment(_borrower);
        address loanManager = IGnosisSafeProxyFactory(config.getGnosisSafeFactory()).createProxyWithNonce(
        ...
        {
            ImplementationRepository repo = config.getLoanImplementationRepository();
            UcuProxy loanProxy = new UcuProxy(repo, loanManager);
            loan = IHelixLoan(address(loanProxy));
        }
    
        {
            loan.initialize(
                [address(config), loanManager, _loanWallet, _token, _borrower],
                _allowedUIDTypes
            );
            emit LoanCreated(loan, _loanWallet, loanManager, _borrower, dbPrjId);
            config.getHelixLoanTokens().onLoanCreated(address(loan));
        }
    ...
    function _getOwnersForLoanDeployment(address _borrower) internal view returns(address[] memory) {
