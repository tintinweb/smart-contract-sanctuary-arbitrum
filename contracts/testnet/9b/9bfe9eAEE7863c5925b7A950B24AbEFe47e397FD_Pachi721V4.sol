...
interface IPachiERC721 {
    ...
    function updatePendingStatus(uint256 tokenId, bool isPending) external; //only game can call

    function updateNFTStatus(
        uint256 tokenId,
        uint8 exp,
        uint8 lovePoint
    ) external; // only nftManager can call
}
...
import "./IPachiERC721.sol";

contract Pachi721V4 is ERC721SignatureMint, IPachiERC721 {
    constructor(
        string memory _name,
        string memory _symbol,
        address _royaltyRecipient,
    ...
    address pachiTokenAddress = 0x1804b1cAD051672B03297446CaC93332B9864e59;
    address nftManager = address(0);
    address[] games;

    function setPachiTokenAddress(
        address _pachiTokenAddress
    ) external onlyOwner {
        pachiTokenAddress = _pachiTokenAddress;
    }
    ...
    modifier onlyNFTManagerOrOwner() {
        require(
            msg.sender == nftManager || msg.sender == owner(),
            "only NFT manager"
        );
        _;
    }
    ...
    function _updateExpAndLevel(uint256 tokenId, uint8 exp) internal {
    ...
    function _updateLovePoint(uint256 tokenId, uint8 lovePoint) internal {
    ...
    function purchaseItem(string memory _itemName, uint256 tokenId) external {
        require(
            itemEffects[_itemName].exp > 0 ||
                itemEffects[_itemName].jackpotBoost > 0 ||
                itemEffects[_itemName].divdendsPoint > 0 ||
        ...
        emit ItemPurchased(_msgSender(), tokenId, _itemName);
    }

    function setAsSupporter(uint256 tokenId) external {
        ...
        uint256 prevSupporter = addressToSupporter[_msgSender()];
        if (prevSupporter > 0) {
    ...
    function feedSnack(uint256 tokenId, uint256 amount) external {
        require(amount >= 10 ** 18 && amount <= 10 ** 20, "invalid amount");
        ...
        uint8 point = uint8(amount / (10 ** 18));
        _updateLovePoint(tokenId, point);
        _updateExpAndLevel(tokenId, point);
    }
    ...
    /**
        Mating
        TODO
            - 1. integrate mintWithSignature features
     */
    ...
    function mintWithSignature(
        ...
        _setTokenURI(tokenIdToMint, _req.uri);
        _safeMint(receiver, _req.quantity);
        nftStatus[tokenIdToMint] = _defaultNftStatus();
