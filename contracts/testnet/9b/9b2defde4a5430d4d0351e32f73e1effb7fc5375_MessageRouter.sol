...
contract MessageRouter is NonblockingLzApp {
    ...
    function send(uint16 chainId, bytes memory destination, bytes calldata payload, bytes calldata params) public payable {
        lzEndpoint.send{value:msg.value}(chainId, destination, payload, payable(msg.sender), address(this), params);
    }

    function estimateFee(uint16 chainId, address destination, bytes calldata payload, bytes calldata params) external view returns (uint nativeFee, uint zroFee) {
        return lzEndpoint.estimateFees(chainId, destination, payload, false, params);
    }
