...
contract OmniseaUniversalONFT721Proxy is NonblockingLzApp, ERC165, IOmniseaUniversalONFT721Proxy, ReentrancyGuard {
    ...
    function estimateSendFee(address _collection, uint16 _dstChainId, bytes memory _toAddress, uint _tokenId, bool _useZro, bytes memory _adapterParams) public view virtual override returns (uint nativeFee, uint zroFee) {
        ...
        BasicCollectionParams memory collectionParams = BasicCollectionParams({
            tokenId: _tokenId,
            name: IOmniseaONFT721(_collection).name(),
            symbol: IOmniseaONFT721(_collection).symbol(),
            uri: IOmniseaONFT721(_collection).tokenURI(_tokenId),
            tokenURI: IOmniseaONFT721(_collection).tokenURI(_tokenId),
            owner: msg.sender
        });
        bytes memory payload = abi.encode(_toAddress, _tokenId, collectionParams, collectionId);
    ...
    function _tokenExistsAndOwnerOf(IOmniseaONFT721 _collection, uint _tokenId) internal view returns (bool, address) {
...
contract OmniseaONFT721Proxy {
    fallback() external payable {
        _delegate(address(0x6Df0ED63A5Ee407d64441A33ee6b3274FeF18C15));
    }
...
interface IOmniseaUniversalONFT721Proxy is IERC165 {
     ...
     */
    function estimateSendFee(address _collection, uint16 _dstChainId, bytes calldata _toAddress, uint _tokenId, bool _useZro, bytes calldata _adapterParams) external view returns (uint nativeFee, uint zroFee);
