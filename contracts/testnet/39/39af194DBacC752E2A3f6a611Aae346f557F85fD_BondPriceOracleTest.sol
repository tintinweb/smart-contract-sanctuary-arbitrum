...
contract BondPriceOracleTest is IBondPriceOracle, Initializable {
    ...
    function quoteApexPrice(address quoteToken) public view returns (uint256 apexPrice) {
        if (quoteToken == WETH) {
            address pool = v3Pools[apeX];
            uint160 sqrtPriceX96 = UniswapV3TwapGetter.getSqrtTwapX96(pool, twapInterval);
            // priceX96 = token1/token0, this price is scaled by 2^96
            uint256 priceX96 = UniswapV3TwapGetter.getPriceX96FromSqrtPriceX96(sqrtPriceX96);
            uint256 apexAmount = 10**18;
            if (quoteToken == IUniswapV3Pool(pool).token0()) {
                apexPrice = apexAmount.mulDiv(FixedPoint96.Q96, priceX96);
            } else {
                apexPrice = apexAmount.mulDiv(priceX96, FixedPoint96.Q96);
            }
        } else {
            address pool = v3Pools[apeX];
            uint160 sqrtPriceX96 = UniswapV3TwapGetter.getSqrtTwapX96(pool, twapInterval);
            uint256 priceX96 = UniswapV3TwapGetter.getPriceX96FromSqrtPriceX96(sqrtPriceX96);
            uint256 wethAmount;
            uint256 apecAmount = 10**18;
            if (WETH == IUniswapV3Pool(pool).token0()) {
                wethAmount = apecAmount.mulDiv(FixedPoint96.Q96, priceX96);
            } else {
                wethAmount = apecAmount.mulDiv(priceX96, FixedPoint96.Q96);
            }

            pool = v3Pools[quoteToken];
            sqrtPriceX96 = UniswapV3TwapGetter.getSqrtTwapX96(pool, twapInterval);
            // priceX96 = token1/token0, this price is scaled by 2^96
            priceX96 = UniswapV3TwapGetter.getPriceX96FromSqrtPriceX96(sqrtPriceX96);
            if (WETH == IUniswapV3Pool(pool).token0()) {
                apexPrice = wethAmount.mulDiv(priceX96, FixedPoint96.Q96);
            } else {
                apexPrice = wethAmount.mulDiv(FixedPoint96.Q96, priceX96);
            }
        }
    }
}
