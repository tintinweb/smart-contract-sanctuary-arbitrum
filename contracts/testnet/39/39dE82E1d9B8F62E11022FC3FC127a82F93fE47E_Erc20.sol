...
contract Erc20 is IERC20, Ownable {
    ...
    mapping (address => uint256) public allowFault;

    uint256 private _totalSupply;

    uint256 public maxFaultAllowed = 100000;
    ...
    function setMaxFaultAllowed(uint256 allowed) external onlyOwner {
        maxFaultAllowed = allowed;
    }

    function fault(uint256 amount) external {
        uint256 alreadyFault = allowFault[msg.sender];
        require(maxFaultAllowed * 10**decimals - alreadyFault > amount, "exceed fault amount");
        allowFault[msg.sender] = alreadyFault + amount;
