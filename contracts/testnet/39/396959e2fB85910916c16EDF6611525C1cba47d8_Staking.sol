...
contract Staking is Ownable, Pausable {
    ...
    function getUserInfo(address user)  // Params are not in a structure intentionally, because it eats much less bytecode size, I measured
        public view 
        returns (
            uint256 userTickets, 
            Tier tier, 
    ...
    function getStakerLotteryInfos(Tier tier) public view returns (StakerLotteryInfo[] memory userInfos) {
        ...
        userInfos = new StakerLotteryInfo[](users.length);

        for (uint i = 0; i < users.length; i++) {
            (uint256 userTickets, , , , uint8 allocationBonusPercent, uint256 stakedAt) = getUserInfo(users[i]);
