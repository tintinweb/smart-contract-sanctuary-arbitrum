 ...
 * bugs confusing "shares" and asset numbers. Same bytecode as OZ 6b17b3.
 *
 * For more information, please see @ openzeppelin/contracts/token/ERC20/extensions/ERC4626.sol
 */
abstract contract ERC4626 is ERC20, IERC4626 {
    ...
    function convertToShares(uint256 assets) public view virtual override returns (Shares shares) {
        return _convertToShares(assets, Math.Rounding.Down);
    }

    /** @dev See {IERC4626-convertToAssets}. */
    function convertToAssets(Shares shares) public view virtual override returns (uint256 assets) {
    ...
    function maxMint(address) public view virtual override returns (Shares) {
        return Shares.wrap(type(uint256).max);
    }

    /** @dev See {IERC4626-maxWithdraw}. */
    function maxWithdraw(address owner) public virtual override returns (uint256) {
        return _convertToAssets(Shares.wrap(balanceOf(owner)), Math.Rounding.Down);
    }

    /** @dev See {IERC4626-maxRedeem}. */
    function maxRedeem(address owner) public virtual override returns (Shares) {
        return Shares.wrap(balanceOf(owner));
    }

    /** @dev See {IERC4626-previewDeposit}. */
    function previewDeposit(uint256 assets) public view virtual override returns (Shares) {
        return _convertToShares(assets, Math.Rounding.Down);
    }

    /** @dev See {IERC4626-previewMint}. */
    function previewMint(Shares shares) public view virtual override returns (uint256) {
        return _convertToAssets(shares, Math.Rounding.Up);
    }

    /** @dev See {IERC4626-previewWithdraw}. */
    function previewWithdraw(uint256 assets) public view virtual override returns (Shares) {
        return _convertToShares(assets, Math.Rounding.Up);
    }

    /** @dev See {IERC4626-previewRedeem}. */
    function previewRedeem(Shares shares) public view virtual override returns (uint256) {
        return _convertToAssets(shares, Math.Rounding.Down);
    }

    /** @dev See {IERC4626-deposit}. */
    function deposit(uint256 assets, address receiver) public virtual override returns (Shares) {
        require(assets <= maxDeposit(receiver), "ERC4626: deposit more than max");

        Shares shares = previewDeposit(assets);
    ...
    function mint(Shares shares, address receiver) public virtual override returns (uint256) {
    ...
    function withdraw(
        ...
        address receiver,
        address owner
    ) public virtual override returns (Shares) {
     ...
     */
    function _convertToShares(uint256 assets, Math.Rounding rounding) internal view virtual returns (Shares shares) {
        uint256 supply = totalSupply();
        return
            (assets == 0 || supply == 0)
                ? _initialConvertToShares(assets, rounding)
                : Shares.wrap(assets.mulDiv(supply, totalAssets(), rounding));
    }
    ...
    ) internal view virtual returns (Shares shares) {
        return Shares.wrap(assets);
    }

    /**
     * @dev Internal conversion function (from shares to assets) with support for rounding direction.
     */
    function _convertToAssets(Shares shares, Math.Rounding rounding) internal view virtual returns (uint256 assets) {
        uint256 supply = totalSupply();
        return
            (supply == 0)
            ? _initialConvertToAssets(shares, rounding)
            : Shares.unwrap(shares).mulDiv(totalAssets(), supply, rounding);
    }
    ...
    function _deposit(
        ...
        _mint(receiver, Shares.unwrap(shares));
    ...
    function _withdraw(
        ...
        if (caller != owner) {
            _spendAllowance(owner, caller, Shares.unwrap(shares));
        }
        ...
        _burn(owner, Shares.unwrap(shares));
...
library PerpLib {
    ...
    function _getWithdrawDelay(
        ...
        FPSigned sumPnL,
        FPSigned healthyUtilizationRatio,
        ...
        {
            FPSigned divider = FixedPoint.fromUnscaledInt(int256(balance - pendingWithdraw)) - sumPnL;
            if (divider != FixedPoint.ZERO.fromUnsigned()) {
                utilizationRatio = totalOpenInterest.fromUnsigned() / divider;
            } else if (totalOpenInterest != FixedPoint.ZERO) {
                return uint64(maxWithdrawDelay);
            }
        }

        int256 rawDelay = (utilizationRatio - healthyUtilizationRatio).mul(delayPerUtilizationRatio).mul(requestedShareFraction).trunc();
        return uint64(
            rawDelay < 0
                ? 0
...
import '../interfaces/staking/IRewardDistributor.sol';
...
/** @title DomFiVault for Domination Finance perps
 */
contract DomFiVault is Ownable, ReentrancyGuard, IVault, ERC4626, IRewardRecipient {
    using FixedPoint for FPUnsigned;
    using FixedPoint for FPSigned;
    ...
    IRewardDistributor public rewardDistributor; /// @notice if set, distributes external token reward to vault LPs
    IFeeDistributor public feeDistributor; /// @notice distributes trade/liquidity fees to vault LPs
    IDomFiPerp public domFiPerp;

    FPUnsigned HEALTHY_UTILIZATION_RATIO = FixedPoint.fromUnscaledUint(8).div(10); // utilization ratio below which it's safe to let users withdraw immediately
    uint256 MAX_WITHDRAW_DELAY = 10 days; // worst-case wait to withdraw collateral
    uint256 TOTAL_DELAY_PER_UTILIZATION_RATIO = 100 days; // withdrawing 10% of the vault would take 1 day for each 10% utilization ratio above the minimum
    uint64 public constant WITHDRAW_WINDOW = 1 days; // time until an available WithdrawRequest expires
    uint256 public pendingWithdraw = 0; // sum of outstanding withdraw requests
    Shares public pendingRedeem = Shares.wrap(0); // sum of outstanding withdraw requests

    mapping(address => WithdrawRequest) public pendingWithdrawRequest;
    ...
    function deposit(uint256 amount, address user) public override returns (Shares) {
        return deposit(amount, user, 0, Signature(0, 0, 0));
    }
    ...
    function deposit(
    ...
    ) public nonReentrant returns (Shares shares) {
        require((canUserDeposit || msg.sender == owner()) && (msg.sender == user || domFiPerp.validateManager(msg.sender, user)), '!stake');
        require(amount <= maxDeposit(user), '!maxCollateral');

        uint256 fee;
        (shares, , fee) = _convertInput(amount, Shares.wrap(0), totalAssets(), Shares.wrap(totalSupply()));

        if (signature.v != 0 && signature.r != 0 && signature.s!= 0) {
            IERC20Permit(asset()).safePermit(
                msg.sender,
                address(this),
                amount,
                deadline,
        ...
        if (fee > 0) {
            _distributePendingFees(fee, address(0));
        }
        balance += amount - fee;
    }

    function mint(Shares shares, address receiver) public override returns (uint256) {
        return mint(shares, receiver, 0, Signature(0, 0, 0));
    }
    ...
    function mint(
        ...
        require(
            (canUserDeposit || msg.sender == owner()) &&
                (msg.sender == receiver || domFiPerp.validateManager(msg.sender, receiver)),
            '!stake'
        );
        require(shares <= maxMint(receiver), '!maxCollateral');

        uint256 fee;
        (, assets, fee) = _convertOutput(0, shares, totalAssets(), Shares.wrap(totalSupply()));
    ...
    /** @notice withdraw `assets` to `receiver`, burning `shares` from `account`
     *  @return shares shares burnt to make the withdrawal
     */
    function withdraw(uint256 assets, address receiver, address account) public virtual override onlyManager(account) returns (Shares shares) {
        require(assets <= maxWithdraw(account), 'ERC4626: withdraw more than max');

        WithdrawRequest storage request = pendingWithdrawRequest[account];
        uint256 fee;
        (shares, , fee) = _convertOutput(assets, Shares.wrap(0), request.assets, request.shares);

        balance -= assets;
        pendingWithdraw -= assets;

        if (fee > 0) {
            _distributePendingFees(fee, address(0));
        }

        _withdraw(msg.sender, receiver, account, assets, shares);

        if (assets >= maxWithdraw(account)) {
            delete pendingWithdrawRequest[account];
        } else {
            request.sharesRedeemed = request.sharesRedeemed + shares;
        }
    }

    /** @notice redeem `shares` from `account`. send collateral to `receiver`
     *  @return assets collateral sent to receiver
     */
    function redeem(Shares shares, address receiver, address account) public virtual override onlyManager(account) returns (uint256 assets) {
        require(shares <= maxRedeem(account), 'ERC4626: redeem more than max');

        WithdrawRequest storage request = pendingWithdrawRequest[account];
        
        uint256 fee;
        (, assets, fee) = _convertInput(0, shares, request.assets, request.shares);
    ...
    /** @notice get max shares that can be requested
     *  @return shares max shares that can be requested
     *  @return assets max assets that can be requested
     */ 
    function requestable(address owner) public view returns (Shares shares, uint256 assets) {
        shares = Shares.wrap(balanceOf(owner));
        if (activeWithdrawRequest(owner)) {
            shares = shares - (pendingWithdrawRequest[owner].shares - pendingWithdrawRequest[owner].sharesRedeemed);
        }
        assets = _convertToAssets(shares, Math.Rounding.Down);
    }

    function instantRedeemAvailable() public returns (bool) {
        if (balance == 0) {
            return false;
        }
        FPSigned utilizationRatio = domFiPerp.totalOpenInterest().fromUnsigned() /
            (FixedPoint.fromUnscaledInt(int256(balance)) - domFiPerp.getTotalPnl())
        ;
        return utilizationRatio <= HEALTHY_UTILIZATION_RATIO.fromUnsigned();
    }

    function subtractPendingWithdraw(address account) private {
        WithdrawRequest storage request = pendingWithdrawRequest[account];
        if (request.sharesRedeemed == Shares.wrap(0)) {
            pendingRedeem = pendingRedeem - request.shares;
            pendingWithdraw -= request.assets;
        } else {
            pendingRedeem = pendingRedeem - (request.shares - request.sharesRedeemed);
            (, uint256 requestRemainingAssets, ) = _convertInput(0, request.shares - request.sharesRedeemed, request.assets, request.shares);
            pendingWithdraw -= requestRemainingAssets;
        }
    }

    /**
     * @notice Preview a withdraw request
     * @param shares Shares that would be requested
     * @param account Account to preview the request
     * @return length Length of the resulting WithdrawRequest
     * @return offset Time between request and when withdrawals will begin.
     */
    function previewRedeemRequest(Shares shares, address account) external returns (uint64 length, int64 offset) {
        require(shares > Shares.wrap(0), '!amount');
        ...
        require(assets > 0, 'no withdrawable assets');

        FPSigned sumPnL = domFiPerp.getTotalPnl();

        WithdrawRequest storage previousRequest = pendingWithdrawRequest[account];

        uint64 previousWait = 0;
        if (previousRequest.beginsAt > 0) {
            previousWait = uint64(block.timestamp) - previousRequest.beginsAt;
        }
        uint64 windowElapsed = 0;
        if (previousWait > previousRequest.length) {
            windowElapsed = uint64(Math.min(previousWait - previousRequest.length, WITHDRAW_WINDOW));
            // for purposes of penalty calculation, don't "count" the
            // withdraw window as time served
            previousWait -= windowElapsed;
        }

        (uint64 length, uint64 penalty) = PerpLib._getDelayForFullWithdraw(
            shares.div(Shares.wrap(totalSupply())),
            sumPnL,
            HEALTHY_UTILIZATION_RATIO.fromUnsigned(),
            domFiPerp.totalOpenInterest(),
            balance,
            pendingWithdraw,
            MAX_WITHDRAW_DELAY,
            TOTAL_DELAY_PER_UTILIZATION_RATIO,
            previousRequest.length,
            previousWait
        );

        if (activeWithdrawRequest(account)) {
            offset = int64(previousRequest.beginsAt + windowElapsed + penalty) - int64(uint64(block.timestamp));
        } else {
            offset = int64(penalty);
        }
    }

    /**
     * @notice update or create a request to redeem `shares`
     * @param shares request permission to redeem `shares`, or redeem immediately if possible
     * @param receiver instantly redeem to this address if possible. if 0x0, do not instantly redeem
     */
    function makeRedeemRequest(Shares shares, address account, address receiver) external onlyManager(account) returns (uint64 length, uint64 beginsAt) {
        require(shares > Shares.wrap(0), '!amount');

        if (instantRedeemAvailable() && receiver != address(0)) { // TODO check this behaves properly when receiver = 0
            require(shares <= Shares.wrap(balanceOf(account)), 'ERC4626: redeem more than max');

            (, uint256 assets, uint256 fee) = _convertInput(0, shares, totalAssets(), Shares.wrap(totalSupply()));
            ...
            _withdraw(msg.sender, receiver, account, assets, shares);
            subtractPendingWithdraw(account);
            emit WithdrawCleared(msg.sender, account, pendingWithdrawRequest[account]);
            delete pendingWithdrawRequest[account];
        ...
        {
            (Shares maxShares, ) = requestable(account);
            require(shares <= maxShares, 'exceeds balance');
        }
        ...
        subtractPendingWithdraw(account);
        if (activeWithdrawRequest(account)) {
            beginsAt = previousRequest.beginsAt + windowElapsed + penalty;
        } else {
            beginsAt = uint64(block.timestamp) + penalty;
        }
        pendingWithdrawRequest[account] = WithdrawRequest({
            assets: assets,
            shares: shares,
            sharesRedeemed: Shares.wrap(0),
            beginsAt: beginsAt,
            length: length
        });

        emit WithdrawRequested(msg.sender, account, previousRequest);
    }

    /// @notice fraction of a request that can be redeemed or withdrawn. [0,1]
    function availableFraction(address owner) internal view returns (FPUnsigned redeemFraction) {
        WithdrawRequest storage request = pendingWithdrawRequest[owner];
        if (!activeWithdrawRequest(owner) || uint64(block.timestamp) <= request.beginsAt) {
            return FixedPoint.ZERO;
        }
        FPUnsigned alreadyRedeemed = request.sharesRedeemed.div(request.shares);
        if (uint64(block.timestamp) >= request.beginsAt + request.length) {
            // full withdraw available after delay and during window
            return FixedPoint.ONE - alreadyRedeemed;
        }
        // during the withdraw period, linearly release funds
        FPUnsigned linearRelease = FixedPoint.fromUnscaledUint(block.timestamp - request.beginsAt)
            .div(request.length);

        if (linearRelease >= alreadyRedeemed) {
            return linearRelease - alreadyRedeemed;
        } else {
            return FixedPoint.ZERO;
        }
    }
    ...
    function maxWithdraw(address owner) public virtual override returns (uint256 maxAssets) {
        if (instantRedeemAvailable()) {
            return _convertToAssets(Shares.wrap(balanceOf(owner)), Math.Rounding.Down);
        }

        WithdrawRequest storage request = pendingWithdrawRequest[owner];
        FPUnsigned available = availableFraction(owner);
        Shares availableShares = request.shares.mul(available);

        uint256 oldWorth = _convertToAssets(availableShares, Math.Rounding.Down, request.shares, request.assets);
        uint256 currentWorth = _convertToAssets(availableShares, Math.Rounding.Down);

        // user cannot benefit from an increase in vault balance subsequent to the request
        return Math.min(oldWorth, currentWorth);
    }

    /// @notice get max redeemable shares amount
    function maxRedeem(address owner) public virtual override returns (Shares maxShares) {
        if (instantRedeemAvailable()) {
            return Shares.wrap(balanceOf(owner));
        }
        maxShares = pendingWithdrawRequest[owner].shares.mul(availableFraction(owner));
        // do we actually have that many shares?
        if (balanceOf(owner) < Shares.unwrap(maxShares)) {
            maxShares = Shares.wrap(balanceOf(owner));
        }
    }

    /// @notice whether `owner` has an active withdraw request
    /// @return status `false` if request is nonexistent or expired, `true` otherwise
    function activeWithdrawRequest(address owner) public view returns (bool status) {
        WithdrawRequest storage request = pendingWithdrawRequest[owner];
        return request.beginsAt > 0
            && uint64(block.timestamp) <= request.beginsAt + request.length + WITHDRAW_WINDOW;
    }

    /**
     * @notice deposit referral fee from FeeDistributor
     * @dev only can be called by feeDistributor
     */
    function addReferralFee(uint256 amount, address user) external override returns (Shares shares) {
        require(msg.sender == address(feeDistributor), '!feeDistributor');
    ...
    /**
     * @notice donate collateral to the vault
     * @dev receives fees, rewards, etc for distribution to depositors
     * @param rewardAmount collateral tokens to take from sender
     */
    function transferRewardAmount(uint256 rewardAmount) external override {
        require(rewardAmount + balance <= maxCollateral, '!maxCollateral');
        IERC20(asset()).safeTransferFrom(msg.sender, address(this), rewardAmount);
        balance += rewardAmount;
    }

    function previewDeposit(uint256 assets) public view virtual override returns (Shares shares) {
        (shares, , ) = _convertInput(assets, Shares.wrap(0), totalAssets(), Shares.wrap(totalSupply()));
    }

    /**
     * @notice Compute the assets/shares that can be exchanged for the provided assets/shares
     * @param assets Assets to be converted to shares (deposit)
     * @param shares Shares to be converted to assets (redeem)
     */
    function _convertInput(
        ...
        Shares sharesOut, uint256 assetsOut, uint256 fee
    ) {
        FPUnsigned feeRate;
        if (Shares.unwrap(shares) != 0) { // shares in; redeem
            require(assets == 0, 'overspecified');

            feeRate = feeCalculator.getWithdrawFee(msg.sender);
            assets = _convertToAssets(shares, Math.Rounding.Down, rateShares, rateAssets);
        } else { // assets in; deposit
            feeRate = feeCalculator.getDepositFee(msg.sender);
        }
        fee = feeRate.mul(assets).roundUp();
        assetsOut = assets - fee;
        sharesOut = _convertToShares(assetsOut, Math.Rounding.Down, rateShares, rateAssets);
    }

    /**
     * @notice Compute the assets/shares required to obtain the requested assets/shares
     * @param assets Requested asset output (withdraw)
     * @param shares Requested share output (mint)
     */
    function _convertOutput(
        ...
        Shares sharesIn, uint256 assetsIn, uint256 fee
    ) {
        FPUnsigned feeRate;
        if (Shares.unwrap(shares) != 0) { // shares out; mint
            require(assets == 0, 'overspecified');

            feeRate = feeCalculator.getDepositFee(msg.sender);
            assets = _convertToAssets(shares, Math.Rounding.Down, rateShares, rateAssets);
        } else { // assets out, withdraw
            feeRate = feeCalculator.getWithdrawFee(msg.sender);
        }
        assetsIn = (FixedPoint.fromUnscaledUint(assets) / (FixedPoint.ONE - feeRate)).roundUp();
        fee = assetsIn - assets;
        sharesIn = _convertToShares(assetsIn, Math.Rounding.Up, rateShares, rateAssets);
    }

    function _convertToShares(uint256 assets, Math.Rounding rounding) internal view override returns (Shares shares) {
        return _convertToShares(assets, rounding, Shares.wrap(totalSupply()), totalAssets());
    }
    /// @dev ERC4626 reference impl modified to explicitly take a rate
    function _convertToShares(
        uint256 assets,
        Math.Rounding rounding,
        Shares rateShares,
        uint256 rateAssets
    ) internal view returns (Shares shares) {
        uint256 supply = totalSupply();
        return
            (assets == 0 || supply == 0)
                ? _initialConvertToShares(assets, rounding)
                : Shares.wrap(assets.mulDiv(Shares.unwrap(rateShares), rateAssets, rounding));
    }

    function _convertToAssets(Shares shares, Math.Rounding rounding) internal view override returns (uint256 assets) {
        return _convertToAssets(shares, rounding, Shares.wrap(totalSupply()), totalAssets());
    }
    /// @dev ERC4626 reference impl modified to explicitly take a rate
    function _convertToAssets(
        ...
        return
            (supply == 0)
            ? _initialConvertToAssets(shares, rounding)
            : Shares.unwrap(shares).mulDiv(rateAssets, Shares.unwrap(rateShares), rounding);
    }

    function previewWithdraw(uint256 assets) public view virtual override returns (Shares shares) {
        (shares, , ) = _convertOutput(assets, Shares.wrap(0), totalAssets(), Shares.wrap(totalSupply()));
    }

    function previewRedeem(Shares shares) public view virtual override returns (uint256 assets) {
        (, assets, ) = _convertInput(0, shares, totalAssets(), Shares.wrap(totalSupply()));
    }
    ...
    function maxMint(address receiver) public view override returns (Shares maxShares) {
        if (!canUserDeposit && receiver != owner()) {
            return Shares.wrap(0);
        }
        maxShares = _convertToShares(maxCollateral - balance, Math.Rounding.Down);
    }

    function previewMint(Shares shares) public view override returns (uint256 assets) {
        (, assets, ) = _convertOutput(0, shares, totalAssets(), Shares.wrap(totalSupply()));
    }

    function _distributePendingFees(uint256 reward, address caller) internal {
        IERC20(asset()).safeTransfer(address(feeDistributor), reward);
        feeDistributor.receiveFee(caller);
    }
    ...
    function setHealthyUtilizationRatio(FPUnsigned healthyUtilizationRatio) external onlyOwner {
        HEALTHY_UTILIZATION_RATIO = healthyUtilizationRatio;
    }

    function setMaxWithdrawDelay(uint256 maxWithdrawDelay) external onlyOwner {
        MAX_WITHDRAW_DELAY = maxWithdrawDelay;
    }

    function setTotalDelayPerUtilizationRatio(uint256 totalDelayPerUtilizationRatio) external onlyOwner {
        TOTAL_DELAY_PER_UTILIZATION_RATIO = totalDelayPerUtilizationRatio;
    }
    ...
    function _initialConvertToShares(
        uint256 assets,
        Math.Rounding /*rounding*/
    ) internal pure override returns (Shares shares) {
        return Shares.wrap(assets * 10**18);
    }
    ...
    function _initialConvertToAssets(
        ...
        return Shares.unwrap(shares) / 10**18;
    }
    ...
    function setAddresses(
        ...
        if (address(domFiPerp) != address(0)) {
            require(domFiPerp == _domFiPerp, 'domFiPerp can not be updated');
        }
    ...
    /** @notice Cut TradeFee from PnL, distribute the TradeFee, and update the Vault State
    */
    function withdrawAndGetFee(
        address caller,
        uint256 margin,
        FPSigned pnl,
        FPUnsigned fee,
        IDomFiPerp.Position memory position
    ) external override onlyDomFiPerp returns (
        uint256 totalFee,
        int256 netPnl
    ) {
        ...
        FPSigned pnlAfterFee = pnl.sub(totalFee).roundTraderPnl();
        netPnl = pnlAfterFee.trunc();

        // Update vault
        uint256 _pnlAfterFee = pnlAfterFee.abs().trunc();
        if (pnlAfterFee < FixedPoint.fromUnscaledInt(0)) {
        ...
        _distributePendingFees(totalFee, caller);
        balance -= totalFee;

        return (totalFee, netPnl);
    }

    function withdrawWithoutFee(address positionOwner, uint256 amount) external override onlyDomFiPerp {
        IERC20(asset()).safeTransfer(positionOwner, amount);
    }

    /** @notice Allow DomFiPerp to withdraw liquidator reward.
     *          Distribute remaining fee and update the vault state.
    */
    function approveAndUpdateLiquidatorReward(
        ...
        address liquidator
    ) external override onlyDomFiPerp {
        if (remainingReward > 0) {
            _distributePendingFees(remainingReward, liquidator);
        }
        balance += increaseBalance;
        IERC20Extended(asset()).increaseAllowance(address(domFiPerp), liquidatorReward);
    }

    modifier onlyDomFiPerp() {
        require(msg.sender == address(domFiPerp), '!domFiPerp');
        _;
    }

    modifier onlyManager(address account) {
        require(account == msg.sender || domFiPerp.validateManager(msg.sender, account), '!manager');
        _;
    }
}