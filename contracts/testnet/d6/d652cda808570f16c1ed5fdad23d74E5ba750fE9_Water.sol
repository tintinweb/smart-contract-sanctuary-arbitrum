...
import {IBartender} from "../interfaces/IBartender.sol";

// import "hardhat/console.sol";

contract Water is OwnableUpgradeable, ERC4626Upgradeable, PausableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using Math for uint256;

    /** @dev Water related variables */
    IERC20Upgradeable public usdcToken;
    //@todo check if we need a default cap
    uint256 public depositCap;
    uint256 public totalDebt;
    uint128 public depositFeeBPS;
    uint128 public withdrawFeeBPS;
    ...
    /// @param amount total USDC amount to be safeTransferred (6 decimals)
    ...
    event FeesUpdated(uint128 _depositFeeBPS, uint128 _withdrawalFeeBPS);

    event AdminParamsUpdated(address newBartender, address feeRecipient, address liquor);
    ...
    function initialize(address _usdcToken) external initializer {
        ...
        depositCap = 200000 * 1e6;
        usdcToken = IERC20Upgradeable(_usdcToken);

        WATER_DEFAULT_PRICE = 1_000_000;
        MAX_BPS = 100_000;
        RATE_PRECISION = 1e30;

        __Ownable_init();
        __Pausable_init();
        __ERC4626_init(IERC20Upgradeable(_usdcToken));
        __ERC20_init("Water", "S-WATER");
    }
    ...
    /* ##################################################################
                                OWNER FUNCTIONS
    ################################################################## */
    function setAdminParams(address _newBartender, address _feeRecipient, address _liquor) external onlyOwner {
        bartender = IBartender(_newBartender);
        feeRecipient = _feeRecipient;
        liquor = _liquor;
        emit AdminParamsUpdated(_newBartender, feeRecipient, _liquor);
    }
    ...
    /// @notice sets the fee for deposit and withdrawal
    /// @param _depositFeeBPS: updated deposit fee
    /// @param _withdrawalFeeBPS: updated withdrawal fee
    ///@dev feeBPS = fee basis points
    function updateFees(uint128 _depositFeeBPS, uint128 _withdrawalFeeBPS) external onlyOwner {
        depositFeeBPS = _depositFeeBPS;
        withdrawFeeBPS = _withdrawalFeeBPS;
    ...
    function leverageVault(uint256 _amount) external onlyBartenderOrLiquor {
        ...
        require(
            _amount <= usdcBalance,
            string(abi.encodePacked("ThrowUnavailableSupply: totalSupply=", usdcBalance, ", withdrawAmount=", _amount))
        );

        totalDebt += _amount;
        usdcToken.transfer(address(bartender), _amount);
    ...
    function repayDebt(uint256 _amount) external onlyBartenderOrLiquor {
        if (_amount > totalDebt) totalDebt = 0;
        else {
            totalDebt -= _amount;
        }

        if (_msgSender() == address(bartender)) {
            usdcToken.transferFrom(address(bartender), address(this), _amount);
        } else {
            usdcToken.transferFrom(liquor, address(this), _amount);
        }

        emit LeverageBartenderDebt(_amount, totalDebt, block.timestamp);
    }

    /// @notice Allows the owner to withdraw the collected fees
    function withdrawFees() public onlyOwner {
        require(collectedFees > 0, "Zero Amount");

        uint256 feesToWithdraw = collectedFees;
        collectedFees = 0;

        usdcToken.transfer(owner(), feesToWithdraw);
    }
    ...
    function deposit(uint256 _assets, address _receiver) public override whenNotPaused returns (uint256 shares) {
        require(_receiver != address(0), "Water: receiver is zero address");
        require(_assets > 0, "Water: deposit amount is zero");
    ...
    function withdraw(
        ...
        require(_receiver != address(0), "Water: receiver is zero address");
        uint256 fee = _assets.mulDiv(withdrawFeeBPS, MAX_BPS) / 1000;
        collectedFees += fee;
        _assets -= fee;
    ...
    function totalAssets() public view override returns (uint256 _totalAssets) {
        return usdcToken.balanceOf(address(this)) + totalDebt - collectedFees;
    }
    ...
    /// @dev checks the max amount user balance and assets of vault
    /// @param _owner address whose maximum withdrawable assets needs to be computed
    /// @return max no. of assets
    function maxWithdraw(address _owner) public view override returns (uint256) {
        uint256 maxAvailable = usdcToken.balanceOf(address(this));
        // checks the balance of the user
        uint256 maxOfUser = convertToAssets(balanceOf(_owner));
    ...
    /// @param _owner address whose maximum redeemable shares needs to be computed
    ...
    function getFeeBPS() public view returns (uint256, uint256) {
        return (withdrawFeeBPS, depositFeeBPS);
    }
    ...
    /// @return depositCap gets the underlying asset cap
    function getAssetCap() public view returns (uint256) {
        return depositCap;
    }
    ...
    function previewMint(uint256 assets) public view override returns (uint256) {
        ...
        return supply == 0 ? assets : assets.mulDiv(supply * MAX_BPS, totalAssets() * (MAX_BPS - depositFeeBPS));
    }

    function previewDeposit(uint256 shares) public view override returns (uint256) {
        uint256 supply = totalSupply(); // Saves an extra SLOAD if totalSupply is non-zero.

        return supply == 0 ? shares : shares.mulDiv(totalAssets() * (MAX_BPS - depositFeeBPS), supply * MAX_BPS);
    }
    ...
    function previewWithdraw(uint256 assets) public view override returns (uint256) {
        ...
        return supply == 0 ? assets : assets.mulDiv(supply * MAX_BPS, totalAssets() * (MAX_BPS - withdrawFeeBPS));
    }
    ...
    function previewRedeem(uint256 shares) public view override returns (uint256) {
        ...
        return supply == 0 ? shares : shares.mulDiv(totalAssets() * (MAX_BPS - withdrawFeeBPS), supply * MAX_BPS);
    }
    ...
    function getUtilizationRatio() public view returns (uint256) {
        return totalDebt == 0 ? 1 * RATE_PRECISION : totalDebt.mulDiv(RATE_PRECISION, totalAssets());
    }
    ...
    function updateTotalDebt(uint256 profit) external onlyBartenderOrLiquor returns (uint256) {
        totalDebt += profit;
        return totalDebt;
    }
