...
import {Module, OrderType, ModuleData, TccItemV2, TcdItem, TaskConfig, TokenItem, FeeItem, FeeItem2, SwapTokenItem, SwapEventItem, OrderEventItem, CreateItem, GasItem, GasItemList, ExactInputParams} from "./StructList.sol";
...
contract ManualOrder is OrderBase {
    ...
    function doManualOrder(address _user, bytes32 _md5) external nonReentrant {
        ...
        //或者当天日期
        x.day = IUsdtPool(usdtPoolAddress).getYearMonthDay(block.timestamp);
        //得到taskId
        ...
        if (!y.tcc.isV3) {
            ...
            //            }
            approveDex(IERC20(x._TokenItem.swapInToken), multiDexRouterAddress, x.swapInAmount);
        } else {
            ...
            //            }
            approveDex(IERC20(x._TokenItem.swapInToken), V3Dex, x.swapInAmount);
        }
        ...
        if (y.tcc._swapPathConfig2._swapPathList.length > 0) {
            if (!y.tcc.isV3) {
                ...
                //                    })
                //                );
                x.swapInAmount = _dov3swap(y.tcc._swapPathConfig1._factoryV3[0], y.tcc._swapPathConfig1._swapPathList, y.tcc._swapPathConfig1._feeListV3, _to, x.swapInAmount, true);
            }
        } else {
            ...
            if (y.tcc._swapPathConfig1._swapPathList[y.tcc._swapPathConfig1._swapPathList.length - 1] == weth) {
                if (!y.tcc.isV3) {
                    ...
                    //                        })
                    //                    );
                    x.swapInAmount = _dov3swap(y.tcc._swapPathConfig1._factoryV3[0], y.tcc._swapPathConfig1._swapPathList, y.tcc._swapPathConfig1._feeListV3, _to, x.swapInAmount, false);
                }
            } else {
                if (!y.tcc.isV3) {
                    x.swapInAmount = MultiDexRouter(multiDexRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if (y.tcc._swapPathConfig2._swapPathList.length > 0) {
            ...
            //判断是否有授权额度,如果没有则授权
            if (!y.tcc.isV3) {
                approveDex(IERC20(x._TokenItem.swapOutToken), multiDexRouterAddress, x.swapInAmount);
                ...
                //                }
                approveDex(IERC20(x._TokenItem.swapOutToken), V3Dex, x.swapInAmount);
            }
            //完成第二次交易,得到代币A的数量
            uint256 out;
            if (!y.tcc.isV3) {
                ...
                //                    })
                //                );
                out = _dov3swap(y.tcc._swapPathConfig2._factoryV3[0], y.tcc._swapPathConfig2._swapPathList, y.tcc._swapPathConfig2._feeListV3, _to, x.swapInAmount, true);
            }
            //得到消耗的代币数量
