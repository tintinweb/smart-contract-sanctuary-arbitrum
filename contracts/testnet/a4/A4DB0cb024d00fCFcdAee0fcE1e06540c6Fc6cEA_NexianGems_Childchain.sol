...
import "../../../thirdparty/falkor-contracts/tokens/erc721/IERC721TokenUriProvider.sol";

import "../interfaces/IERC721ASafeMintable.sol";

import { MINTER_ROLE, COLLECTION_ADMIN_ROLE } from "../../WLRoleConstants.sol";

contract NexianGems_Childchain is
	Initializable,
	ERC721ContractMetadataUpgradeable,
	...
	UUPSUpgradeable
{
	...
	IERC721TokenUriProvider public tokenUriProvider;
	...
	function supportsInterface(
		bytes4 interfaceId
	) public view override(ERC721ContractMetadataUpgradeable, AccessControlEnumerableUpgradeable) returns (bool) {
	...
	/**
	 * @dev Sets the collection reveal timestamp
	 * @param timestamp reveal timestamp
	 */
	function setRevealTimestamp(uint256 timestamp) public onlyRole(DEFAULT_ADMIN_ROLE) {
		revealTimestamp = timestamp;
	}

	/**
	 * @dev Sets the unrevealed URI
	 * @param _notRevealedURI URI to set
	 */
	...
	/**
	 * @dev to get the token URI
	 * @param tokenId token id to get the URI for
	 */
	function tokenURI(uint256 tokenId) public view override returns (string memory) {
		if (block.timestamp < revealTimestamp && revealTimestamp != 0) {
			return notRevealedURI;
		} else {
			// If a tokenUriProvider is set, use it to get the token URI
