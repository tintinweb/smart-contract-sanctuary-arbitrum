...
interface IMetaversepad {
    function userInfo(address _address) external view returns (UserInPool.User memory);
}
...
import "../interface/IMetaversepad.sol";
import "../interface/FullMath.sol";

contract ClaimPortal is Ownable, Pausable {
    using SafeERC20 for IERC20;

    IERC20 _ERC20Interface;
    // IMetaversepad _MetaversePoolInterface;
    ...
    constructor(
        ...
        // _MetaversePoolInterface = IMetaversepad(_fundingPool);
    }

    modifier isEligible() {
        require(!_claimed[msg.sender], "Claimed!");
        uint256 _tokenBalance = _ERC20Interface.balanceOf(address(this));
        require(_tokenBalance > 0, "Insufficient Claimable Token Amount!");
        _;
    }

    /**
     * @dev Update config for this portal.
     * @param _fundingPool for pool address, _tokenAddress for token address, and _tokenPrice for token price.
     */
    function updatePortalConfig(
        address[] memory _fundingPool,
        address _tokenAddress,
        uint256 _tokenPrice
     ...
     */
    function claimToken() external isEligible whenNotPaused {
        uint256 _tokenAmount = _calculateClaimableToken(msg.sender);
        require(_tokenAmount > 0, "Zero Allocation!");
        _ERC20Interface.safeTransferFrom(address(this), msg.sender, _tokenAmount);
    ...
    function emergencyWithdraw() external onlyOwner whenPaused {
        uint256 _tokenBalance = _ERC20Interface.balanceOf(address(this));
        _ERC20Interface.safeTransferFrom(address(this), msg.sender, _tokenBalance);
    }
     ...
     */
    function userAllocation(address _address) public view returns (uint256) {
        uint256 totalAllocation;
        for (uint256 i; i < fundingPool.length; i++) {
            IMetaversepad _MetaversePoolInterface = IMetaversepad(fundingPool[i]);
            totalAllocation += _MetaversePoolInterface.userInfo(_address).totalAllocation;
        }
    ...
    /**
     * @dev Calculate amount of token that can be claimed, based on the pool allocation.
     * @return Claimable token
     */
    function _calculateClaimableToken(address _address) private view returns (uint256) {
        uint256 _totalAllocation = userAllocation(_address);
        return FullMath.mulDiv(_totalAllocation, 10**18, tokenPrice);
    }
}
...
contract ClaimPortalFactory is Ownable {
    ...
    function launchPortal(
        ...
        portal = new ClaimPortal(owner(), _name, _tokenAddress, _fundingPool, _tokenPrice);

        emit PortalCreation(portal, _tokenAddress, _fundingPool, block.timestamp, _name, _tokenPrice);
    }
}