...
import "./interface/IStargateLPFarming.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";
import "@openzeppelin/contracts/interfaces/IERC20.sol";
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

contract MagicVault is ERC20, ReentrancyGuard {
    ...
    function deposit(uint256 _amountUSDC) public nonReentrant {
		...
		uint256 sgLPTokenReceived = afterBalance - prevBalance;
    ...
    function claim(uint256 _amountMAGIC, bytes memory signature) public nonReentrant {
        ...
        // Verify signature
        if(Signature.verify(_amountMAGIC, signature) != signer) revert InvalidSignature();
    ...
    function withdraw(uint256 _amountMVLpToken) public nonReentrant {
        ...
        stargateRouter.instantRedeemLocal(
            stargatePoolID, 
            sgLPToken.balanceOf(address(this)),
            address(this)
        );

        // Transfer $USDC to user
        emit USDCwithdraw(msg.sender, _amountMVLpToken, usdc.balanceOf(address(this)));
...
library Signature {
    function verify(uint amount, bytes memory signature) public pure returns (address) {
        bytes32 payloadHash = keccak256(abi.encode(amount));
