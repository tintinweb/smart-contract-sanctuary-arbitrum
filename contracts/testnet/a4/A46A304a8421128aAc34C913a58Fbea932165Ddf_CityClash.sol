...
contract CityClash is CityClashScore {
    ...
    function updateCityImage(uint _tokenId, string memory imageBaseUrl) external onlyOwner {
        CityClashTypes.City storage city = idToCities[_tokenId];
        require(keccak256(abi.encodePacked(city.baseImageUrl)) == keccak256(abi.encodePacked("upgraded")), "The city must have been upgraded");
        city.baseImageUrl = imageBaseUrl;
...
contract CityClashNFT is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 public citiesWaveMax = 318; //local max that can be updated
    ...
    function mintPublicSale(uint256 _count) external payable nonReentrant whenPublicSaleActive returns (uint256, uint256) {
        ...
        require(_tokenIds.current() + _count <= citiesWaveMax, "Current mint wave has been minted");
