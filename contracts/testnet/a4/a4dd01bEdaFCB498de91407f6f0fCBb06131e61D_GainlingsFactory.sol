...
contract GainlingsFactory is ERC721Enumerable, RrpRequesterV0, ReentrancyGuard, Ownable
{
    ...
    struct ContractParams{
        ...
        uint256 _buffStartFactor;
        uint256 _trophySubtrahend;
        uint256 _cooldownBuffPerTrophy;
        uint256 _weightMultiplier;
        uint256 _cooldownBuff;
        uint256 _randomCap;
        uint256 _mintsPerWallet;
        bool _startCooldownActive;
    }
    ...
    uint256 public _cooldownBuffPerTrophy = 360;
    uint256 public _weightMultiplier = 1;
    uint256 public _cooldownBuff = 200;
    uint256 public _randomCap = 990;
    uint256 public _mintsPerWallet = 25;
    uint256 public _rushPrice = 2;
    uint256 public _rugPrice = 5;
    uint256 public _snatchPrice = 10;  
    ...
    modifier canAttack(uint256 attackerTokenId,uint256 defenderTokenId) {
        require(gasleft() > 600000, "0");
        require(_phase == 4, "1");
        require(attackerTokenId != defenderTokenId, "2");
        require(_exists(attackerTokenId) , "3");
        require(_exists(defenderTokenId) , "4");
        require(ownerOf(attackerTokenId) == msg.sender, "5");
        require(_warriorLocks[attackerTokenId] == false, "6");
        require(_warriorLocks[defenderTokenId] == false, "7");
        _;
    }
    ...
    function makeRequestToAirnode(bool isMint) internal returns (bytes32 requestId) {
        bytes4 functionId = isMint ? this.fulfillRandomnessMint.selector : this.fulfillRandomnessAttack.selector; 
    ...
    function whitelistMint(uint256 amount, bytes32[] calldata _proof) external payable nonReentrant{
        ...
        require(mints + amount <= _mintsPerWallet, "limit");
        require(MerkleProof.verify(_proof, merkleRoot, leaf), "list");
        require(_phase == 1, "stage");
        require(totalSupply() + amount <= _totalMint, "supply");
        require(msg.value >= amount * _publicPrice, "value");
    ...
    function mint(uint256 amount) internal {
        ...
        for (uint256 i; i < amount; i++) {
            ...
            bytes32 requestId = makeRequestToAirnode(true);
    ...
    function isTokenInCooldown(uint256 id, uint256 weight) public view returns (bool isInCooldown){
        ...
        uint256 lvlCapped = _tokenLevels[id] > _maxLevel ? _maxLevel : _tokenLevels[id];
        isInCooldown =  _defenderLastAttack + (weight - (lvlCapped * _cooldownBuffPerTrophy) *_weightMultiplier)  >= block.timestamp;
        return isInCooldown;
    }

    ///// game functions \\\\\
    function attackSnatch(uint256 attackerTokenId, uint256 defenderTokenId) external canAttack(attackerTokenId,defenderTokenId) nonReentrant {
        require(_tokenLevels[attackerTokenId] >= _maxLevel + _snatchPrice, "8");
        _tokenLevels[attackerTokenId] = _tokenLevels[attackerTokenId] - _snatchPrice;
        attackInternal(attackerTokenId,defenderTokenId,2);
    }
    function attackRug(uint256 attackerTokenId, uint256 defenderTokenId) external canAttack(attackerTokenId,defenderTokenId) nonReentrant {
        require(_tokenLevels[attackerTokenId] >= _maxLevel + _rugPrice, "8");
        _tokenLevels[attackerTokenId] = _tokenLevels[attackerTokenId] - _rugPrice;
        attackInternal(attackerTokenId,defenderTokenId,1);
    }
    function attackRush(uint256 attackerTokenId, uint256 defenderTokenId) external canAttack(attackerTokenId,defenderTokenId) nonReentrant {
        require(_tokenLevels[attackerTokenId] >= _maxLevel + _rushPrice, "8");
        _tokenLevels[attackerTokenId] = _tokenLevels[attackerTokenId] - _rushPrice;
        attackInternal(attackerTokenId,defenderTokenId,0);
    }
    function attack(uint256 attackerTokenId, uint256 defenderTokenId) external canAttack(attackerTokenId,defenderTokenId) nonReentrant {
        uint256 _tokenLastAttack = _tokenLabn[attackerTokenId]; 
        if(_startCooldownActive){
            _tokenLastAttack =  _tokenLabn[attackerTokenId] > _seasonBattleStartBlockTime ? _tokenLabn[attackerTokenId] : _seasonBattleStartBlockTime; 
        }

        uint256 attLvlCapped = _tokenLevels[attackerTokenId] > _maxLevel ? _maxLevel : _tokenLevels[attackerTokenId];
        require((_tokenLastAttack + (_preppedWarriors[attackerTokenId].weight - (attLvlCapped * _cooldownBuffPerTrophy)) *_weightMultiplier)  <= block.timestamp, "please cool down"); 

        attackInternal(attackerTokenId,defenderTokenId,0);
    }
    function attackInternal(uint256 attackerTokenId, uint256 defenderTokenId,uint256 attackType) internal nonReentrant canAttack(attackerTokenId,defenderTokenId)  {

        _warriorLocks[attackerTokenId] = true;
        _warriorLocks[defenderTokenId] = true;


        bytes32 requestId = makeRequestToAirnode(false);
        requestToWaitingState[requestId] = true;
        requestIdToAttackRequest[requestId] = AttackRequest(_preppedWarriors[attackerTokenId],_preppedWarriors[defenderTokenId],attackType);
    ...
    function attackWarrior(AttackRequest memory attackRequest,uint256 randomness)  internal  {
        ...
        //uint256[2] memory randoms = splitRandomInTwo(randomness);
        uint256 attackerAttack = attackRequest.attacker.attack;
        uint256 defenderAttack = attackRequest.defender.attack;

        uint256 attLvlCapped = _tokenLevels[attackRequest.attacker.id] > _maxLevel ? _maxLevel : _tokenLevels[attackRequest.attacker.id];
        uint256 devLvlCapped = _tokenLevels[attackRequest.defender.id] > _maxLevel ? _maxLevel : _tokenLevels[attackRequest.defender.id];

        uint256 attRandMin = _buffStartFactor * attLvlCapped - ((attLvlCapped+1) * attLvlCapped * _trophySubtrahend / 2);
        uint256 defRandMin = _buffStartFactor * devLvlCapped - ((devLvlCapped+1) * devLvlCapped * _trophySubtrahend / 2);
        ...
        if(attackRequest.attackType == 0){
            _burn(loserToken);
        }
        else if(attackRequest.attackType == 2){
            safeTransferFrom(ownerOf(loserToken), ownerOf(winnerToken), loserToken);
        }

        _tokenLevels[winnerToken] = _tokenLevels[winnerToken] + 1;
    ...
    function reseedAttack(uint256 attackerTokenId, uint256 defenderTokenId)public onlyOwner {
        bytes32 requestId = makeRequestToAirnode(false);
        requestToWaitingState[requestId] = true;
        requestIdToAttackRequest[requestId] = AttackRequest(_preppedWarriors[attackerTokenId],_preppedWarriors[defenderTokenId],0);
    }
    ...
    function setContractParams(address traitStorageAddress,address libraryAddress,string memory uri,uint256 price) external onlyOwner {
    ...
    function getTokenStats(uint256 tokenId) public view returns (WarriorLiveStats memory stats){
        uint256 cooldown = 0;
        uint256 lastAttack = _tokenLabn[tokenId] > _seasonBattleStartBlockTime ? _tokenLabn[tokenId] : _seasonBattleStartBlockTime; 
        Warrior memory tokenAttributes = getPreppedWarrior(tokenId);

        uint256 lvlCapped = _tokenLevels[tokenId] > _maxLevel ? _maxLevel : _tokenLevels[tokenId];
        uint256 nextAttackBlock = lastAttack + (tokenAttributes.weight  - (lvlCapped * _cooldownBuffPerTrophy) * _weightMultiplier); 

        cooldown = block.timestamp >= nextAttackBlock ? 0 : nextAttackBlock - block.timestamp;
        stats = WarriorLiveStats(tokenId,  _tokenBounties[tokenId],_tokenStacks[tokenId],cooldown);
        return stats;
    }
    function getContractParams() public view returns (ContractParams memory params){
        params = ContractParams(
            _totalMint,  
            _publicPrice,
            _buffStartFactor,
            _trophySubtrahend,
            _cooldownBuffPerTrophy,
            _weightMultiplier,
            _cooldownBuff,
            _randomCap,
            _mintsPerWallet,
            _startCooldownActive);
