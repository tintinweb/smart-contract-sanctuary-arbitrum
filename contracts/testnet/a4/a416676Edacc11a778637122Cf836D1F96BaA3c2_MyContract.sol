...
// Selector for the destination chain (in the CCIP protocol)
  ...
  function transferToken(
    ...
    require(s_feeToken.balanceOf(msg.sender) >= amount, 'Insufficient balance');
    s_feeToken.transferFrom(msg.sender, address(this), amount);
    s_feeToken.approve(address(router), amount);

    Client.EVMTokenAmount[] memory tokenAmounts = new Client.EVMTokenAmount[](1);
    tokenAmounts[0] = Client.EVMTokenAmount(address(s_feeToken), amount);
