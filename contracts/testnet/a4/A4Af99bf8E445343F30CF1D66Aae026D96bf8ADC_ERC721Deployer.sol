...
contract MyERC721 is ERC721Enumerable, Ownable {
    ...
    mapping (uint256 => string) private tokenUriList;

    function mint(string memory tokenUri) external onlyOwner {
        uint256 ts = totalSupply()+1;
        _safeMint(msg.sender,ts);
        tokenUriList[ts] = tokenUri;
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        _requireMinted(tokenId);
        return tokenUriList[tokenId];
    }
}

contract ERC721Deployer is Ownable {
    ...
    function deployERC721(string memory name, string memory symbol) external {
        MyERC721 newContract = new MyERC721(name, symbol);
