...
contract Router is AccessControl, IRouter {
    ...
    mapping(uint256 => QueuedBet) public queuedBets;
    mapping(uint256 => uint256) public requestIdToQueueId;
    mapping(address => bool) public contractRegistry;
    mapping(address => bool) public isKeeper;
    mapping(bytes => bool) public prevSignature;
    mapping(address => address) public tokenToPriceProvider;
    ...
    constructor(address _accountRegistrar, address _admin) {
    ...
    /**
     * @dev Set the contract registry for a target contract.
     * @param targetContract The address of the target contract.
     * @param register A boolean indicating whether to register or unregister the contract.
     */
    ...
    /**
     * @dev Set the status of a keeper.
     * @param _keeper The address of the keeper.
     * @param _isActive A boolean indicatings whether the keeper is active or not.
     */
    function setKeeper(
        address _keeper,
        bool _isActive
    ...
    /**
     * @dev Set the price provider of the token.
     * @param _tokenAddress The address of the token.
     * @param _priceProvider The price provider of the token.
     */
    function setPriceProvider(
        address _tokenAddress,
        address _priceProvider
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        tokenToPriceProvider[_tokenAddress] = _priceProvider;
    }
    ...
    /**
     * @dev Validates if the caller is a keeper.
     */
    function _validateKeeper() private view {
        require(isKeeper[msg.sender], "Router: Forbidden");
    }

    // PUBLIC / EXTERNAL FUNCTIONS

    /**
     * @dev Returns the account mapping for a user.
    ...
    /**
     * @dev Refunds the user by calling the refund function of the game contract.
     * @param user The address of the user.
     * @param targetContract The address of the target game contract.
     */
    function refund(address user, address targetContract) external {
        IGame game = IGame(targetContract);
        game.refund(user);
    }

    /**
     * @dev Updates the payout of a queued bet.
     * @param queueId The ID of the request.
    ...
    function updatePayout(
        ...
        QueuedBet memory queuedBet = queuedBets[queueId];
        queuedBet.payout = payout;
        queuedBet.isProcessed = true;
        queuedBets[queueId] = queuedBet;
    }

    // KEEPER ONLY FUNCTIONS

    /**
     * @dev Opens a bet for slots.
     * @param params The array of OpenTxn parameters.
     */
    function openBets(OpenTxn[] calldata params) external payable {
        _validateKeeper();
        for (uint32 index = 0; index < params.length; index++) {
            BetParams memory currentParams = params[index].betParams;
            Permit memory permit = params[index].permit;
            address user = params[index].user;

            ERC20 tokenX = ERC20(currentParams.tokenAddress);
            uint256 amountToPay = currentParams.wager *
                currentParams.numBets +
                platformFee;
            ...
            // Check if user has correct allowance
            if (
                ...
                bool success = _approveViaSignature(
                    address(tokenX),
                    user,
                    currentParams.queueId,
                    permit
            ...
            (bool isValid, string memory errorReason) = _routerValidations(
                currentParams
            );
            if (!isValid) {
            ...
            (address signer, ) = getAccountMapping(user);

            if (currentParams.game == Game.SLOTS) {
                if (!Validator.verifyUserBetParams(currentParams, signer)) {
                    emit FailResolve(
                        currentParams.queueId,
                ...
                (isValid, errorReason) = _openSlots(currentParams);
                if (!isValid) {
                    emit FailResolve(currentParams.queueId, errorReason);
                    continue;
                }
            } else if (currentParams.game == Game.DICE) {
                if (!Validator.verifyDiceBetParams(currentParams, signer)) {
                    emit FailResolve(
                        currentParams.queueId,
                        "Router: User signature didn't match"
                    );
                ...
                (isValid, errorReason) = _openDice(currentParams);
                if (!isValid) {
                    emit FailResolve(currentParams.queueId, errorReason);
                    continue;
                }
            ...
            // Update the queued bet information
            queuedBets[currentParams.queueId] = QueuedBet({
                queueId: currentParams.queueId,
                tokenAddress: currentParams.tokenAddress,
                numBets: currentParams.numBets,
                stopGain: currentParams.stopGain,
                stopLoss: currentParams.stopLoss,
                player: currentParams.player,
                signature: currentParams.signature,
                signatureTimestamp: currentParams.signatureTimestamp,
                wager: currentParams.wager,
            ...
            prevSignature[currentParams.signature] = true;

            // Emit the OpenBet event
            emit OpenBet(
    ...
    /**
     * @dev Revokes approvals for multiple tokens.
     * @param revokeParams The array of RevokeParams.
     */
    ...
    /**
     * @dev Verifies the bet parameters.
     * @param params The bet parameters.
     * @return isValid A boolean indicating if the bet is valid.
     * @return errorReason The reason for the invalid bet, if any.
     */
    function _routerValidations(
        BetParams memory params
    ) internal view returns (bool isValid, string memory errorReason) {
        // Check if the target contract is authorized
        if (!contractRegistry[params.targetContract]) {
            return (false, "Router: Unauthorized contract");
        }

        if (queuedBets[params.queueId].player != address(0)) {
            return (false, "Router: Bet exists");
        }
    ...
    /**
     * @dev Approves the transfer of tokens via a signature.
     * @param tokenX The address of the token.
     * @param user The address of the user.
     * @param queueId The ID of the queue.
     * @param permit The permit parameters.
     * @return success A boolean indicating if the approval was successful.
     */
    function _approveViaSignature(
        address tokenX,
        address user,
        uint256 queueId,
    ...
    function _openSlots(
        ...
        ISlotsGame gameContract = ISlotsGame(params.targetContract);
        try
            gameContract.runInitialChecks(
                params.player,
                params.tokenAddress,
                params.wager,
                params.numBets,
        ...
        // Transfer the wagered tokens to the game contract
        IERC20(params.tokenAddress).safeTransferFrom(
            params.player,
            address(params.targetContract),
            params.wager * params.numBets
        );

        try
            gameContract.play{value: msg.value}(
                params.wager,
                params.tokenAddress,
                params.numBets,
                params.stopGain,
    ...
    function _openDice(
        ...
        // Get the game contract
        IDiceGame gameContract = IDiceGame(params.targetContract);
...
interface IRouter {
    ...
    event CancelBet(address indexed account, uint256 queueId, string reason);
    event OpenBet(
    ...
    event FailTrigger(address targetContract, string reason);
    event RevokeRouter(
        address user,
        uint256 nonce,
        uint256 value,
...
library Validator {
    ...
    function getUserBetHash(
        IRouter.BetParams memory params
    ) internal view returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    keccak256(
                        "UserBetSignature(address user,address tokenAddress,uint256 numBets,uint256 stopGain,uint256 stopLoss,uint256 wager,uint256 signatureTimestamp,address targetContract)"
                    ),
                    params.player,
                    params.tokenAddress,
                    params.numBets,
    ...
    function getDiceBetHash(
        ...
        return
            keccak256(
                abi.encode(
                    keccak256(
                        "UserBetSignature(address user,address tokenAddress,uint256 numBets,uint256 stopGain,uint256 stopLoss,uint256 multiplier,bool isOver,uint256 wager,uint256 signatureTimestamp,address targetContract)"
                    ),
                    params.player,
                    params.tokenAddress,
                    params.numBets,
    ...
    function verifyUserBetParams(
        IRouter.BetParams memory params,
        address signer
    ) internal view returns (bool) {
        bytes32 hashData = getUserBetHash(params);
        return _validate(hashData, params.signature, signer);
    }

    function verifyDiceBetParams(
        IRouter.BetParams memory params,
        address signer
    ) internal view returns (bool) {
        bytes32 hashData = getDiceBetHash(params);
        return _validate(hashData, params.signature, signer);
    }
