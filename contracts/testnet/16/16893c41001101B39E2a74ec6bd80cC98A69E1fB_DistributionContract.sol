...
contract DistributionContract {
    ...
    for (uint256 i = 0; i < token.holdersCount(); i++) {
        ...
        if (!excludedFromRewards[holder]) {
            eligibleSupply += holders[holder].balance;
        }
    }
...
function isEligible(address holder) public view returns (bool) {
    ...
    function claimArbot() external {
        require(isEligiblearbot(msg.sender), "Not eligible to claim ARBOT");
    ...
    function claimEth() external {
        require(isEligibleeth(msg.sender), "Not eligible to claim ETH");
    ...
    function isEligiblearbot(address holder) public view returns (bool) {
        uint256 currentEpoch = epoch.getCurrentEpoch();
        uint256 lastClaimEpochArbot = holders[holder].lastClaimEpochArbot;
        uint256 lastClaimEpochEth = holders[holder].lastClaimEpochEth;

        return (currentEpoch >= lastClaimEpochArbot + ELIGIBILITY_EPOCHS);

    }

    function isEligibleeth(address holder) public view returns (bool) {
        uint256 currentEpoch = epoch.getCurrentEpoch();
        uint256 lastClaimEpochArbot = holders[holder].lastClaimEpochArbot;
        uint256 lastClaimEpochEth = holders[holder].lastClaimEpochEth;

        return (currentEpoch >= lastClaimEpochEth + ELIGIBILITY_EPOCHS);
    ...
    uint256 currentEpoch = epoch.currentEpoch();
    
    require(newEpoch > currentEpoch, "Epoch has not changed yet.");
    require(block.timestamp >= epoch.getNextEpochTimestamp(), "Not eligible to update epoch yet.");
...
contract EpochContract {
    ...
    uint256 public nextEpochTimestamp; // Timestamp du prochain epoch
    ...
    constructor(uint256 _startTimestamp) {
        ...
        currentEpoch = 1;
        nextEpochTimestamp = _startTimestamp + EPOCH_LENGTH;
    }
    ...
    function getNextEpochTimestamp() public view returns (uint256) {
    ...
    function updateEpoch() external {
        ...
        if (newEpoch > currentEpoch) {
            ...
            nextEpochTimestamp = block.timestamp + EPOCH_LENGTH;
