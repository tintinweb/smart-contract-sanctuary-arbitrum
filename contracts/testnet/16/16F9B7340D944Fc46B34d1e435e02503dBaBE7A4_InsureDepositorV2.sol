...
import {IOwnership} from "../interfaces/external/IOwnership.sol";
import {ILiquidityGauge} from "../interfaces/external/ILiquidityGauge.sol";
import {IInsureVoteEscrow} from "../interfaces/external/IInsureVoteEscrow.sol";

contract InsureDepositorV1 is Initializable, UUPSUpgradeable, ERC20Upgradeable {
    ...
    function initialLock() external onlyOwner {
        ...
        if (veinsure == 0) {
            ...
            //create new lock
            uint256 insureBalanceStaker = IERC20Upgradeable(insure).balanceOf(
                address(this)
            );
    ...
    function deposit(
        ...
        IERC20Upgradeable(insure).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
    ...
    function _lockInsure() internal {
        uint256 insureBalance = IERC20Upgradeable(insure).balanceOf(
            address(this)
        );
    ...
    function _createLock(uint256 _value, uint256 _unlockTime)
        internal
        returns (bool)
    {
...
import {InsureDepositorV1} from "./InsureDepositorV1.sol";
import {IGovFeeDistributor} from "../interfaces/external/IGovFeeDistributor.sol";
import {ILiquidityGauge} from "../interfaces/external/ILiquidityGauge.sol";
import {IRewardForwarder} from "../interfaces/IRewardForwarder.sol";

error DistributorNotSet();
error ReservePoolLPNotSet();
error GaugeNotSet();
error AllowanceNotChanged();
error RewardNotForwardable();
error NotDedicatedMsgSender();

/**
 * @title InsureDepositorV2
 * @author InsureDAO
 * SPDX-License-Identifier: MIT
 * @notice This version enables to claim Reserve Pool's iToken reward,
 *         and also send it to Liquidity Gauge.
 */
contract InsureDepositorV2 is InsureDepositorV1, IRewardForwarder {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    /// @notice GovFeeDistributor which distributes governance fee in a vault.
    address public distributor;
    /// @notice InsureDAO Reserve Pool's LP token
    address public reservePoolLP;
    /// @notice decicated msg.sender determined by gelato ops
    address public dedicatedMsgSender;
    /// @notice last timestamp when fee forwarding executed
    uint256 public lastForwardTime;

    event FeeForwarded(address _to, uint _amount);
    event DistributorSet(address _distributor);
    event ReservePoolLPSet(address _lp);
    event ApprovalChanged(address _gauge, address _token, uint256 _approval);
    ...
    modifier reservePoolLPSet() {
        if (reservePoolLP == address(0)) revert ReservePoolLPNotSet();
        _;
    }

    modifier forwardReady() {
        if (!isRewardForwardable(gauge, reservePoolLP))
            revert RewardNotForwardable();
        _;
    }
    ...
    /**
     * @notice set distributor address, owner can call this function
    ...
    /**
     * @notice set LP token address, owner can call this function
     * @param _reservePoolLP new LP token address
     */
    function setReservePoolLP(address _reservePoolLP) external onlyOwner {
        reservePoolLP = _reservePoolLP;

        emit ReservePoolLPSet(_reservePoolLP);
    }

    /**
     * @notice set dedicated msg.sender issued by gelato ops
     */
    ...
    /**
     * @inheritdoc IRewardForwarder
     * @notice this function should be automated by gelato using ForwardResolver.sol
     *         you can also call this manually setting owner for dedicated sender
     */
    ...
    {
        _fee = IGovFeeDistributor(distributor).claim();

        ILiquidityGauge(gauge).deposit_reward_token(reservePoolLP, _fee);

        lastForwardTime = block.timestamp;

        emit FeeForwarded(gauge, _fee);
    }
    ...
    function toggleForwardApproval(
        ...
        _toggleApproval(_gauge, _reward, _approve);
    }
    ...
    function isRewardForwardable(address _gauge, address _reward)
        ...
        returns (bool)
    {
        uint256 _allowance = IERC20Upgradeable(_reward).allowance(
            address(this),
            _gauge
        );
    ...
    /**
     * @dev toggle approval for spender. if approve, max uint256(2**256 - 1) is set to the allowance.
     *      otherwise, 0 is set.
     * @param _spender token spender
     * @param _token token to spend
     * @param _approve whether allow to spend token
     */
    function _toggleApproval(
        address _spender,
        address _token,
        bool _approve
    ) internal {
        uint256 _newAllowance = _approve ? type(uint256).max : 0;
        uint256 _curretAllowance = IERC20Upgradeable(_token).allowance(
            address(this),
            _spender
        );

        if (_curretAllowance == _newAllowance) revert AllowanceNotChanged();

        IERC20Upgradeable(_token).safeApprove(_spender, _newAllowance);

        emit ApprovalChanged(_spender, _token, _newAllowance);
    }
}
...
/**
 * @title Reward compatible LiquidityGauge interface
 * @author Curve finance
 * @notice this is minimal API for Curve Liquidity Gauge V5 implementation,
 *         which can have multiple reward token to claim.
 */
interface ILiquidityGauge {
    /**
     * @notice deposit reward to gauge. reward must be added to gauge before deposit.
     * @param _reward_token reward token address to deposit
