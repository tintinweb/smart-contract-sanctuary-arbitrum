...
import {IStrategyLiquidate} from "../interfaces/IStrategyLiquidate.sol";
...
import {LibStrategyLiquidate} from "../libraries/LibStrategyLiquidate.sol";
import {LibAccessControlEnumerable} from "../libraries/LibAccessControlEnumerable.sol";
import {LibVault} from "../libraries/LibVault.sol";

/**
 * @title DEDERI Strategy Liquidate
 * @author dederi
 * @notice This contract is strategy Open 涵盖策略清算.
 */
contract StrategyLiquidateFacet is IStrategyLiquidate {
    using LibStrategyLiquidate for LibStrategyLiquidate.Layout;
    using LibStrategyConfig for LibStrategyConfig.Layout;

    /**
     * @notice StrategyLiquidationRequested
     * @param requestHash 清算人 request Hash
     * @param strategyId 要清算的策略ID
     * @param mergeId 清算人要合并的策略ID
     */
    event StrategyLiquidationRequested(bytes32 requestHash, uint256 strategyId, uint256 mergeId);

    /**
     * @notice StrategyLiquidationExecuted
     * @param requestHash 清算人 request Hash
     * @param strategyId 要清算的策略ID
     * @param mergeId 清算人要合并的策略ID
     */
    event StrategyLiquidationExecuted(bytes32 requestHash, uint256 strategyId, uint256 mergeId);

    error InvalidStrategyId(uint256 strategyId);

    // todo 接口参数要改
    function liquidateStrategy(StrategyTypes.LiquidateStrategyRequest memory liquidationStrategy) external {
        // 验证其清算的策略是否存在
        ...
        StrategyTypes.StrategyAllData memory liquidationAllData = LibPositionCore._getStrategyAllData(
            liquidationStrategy.strategyId
        );
        LibStrategyConfig._ensureAdminAndActive(liquidationAllData, liquidationAllData.owner);
        if (liquidationStrategy.mergeId != 0) {
            StrategyTypes.StrategyAllData memory liquidationMergeStrategy = LibPositionCore._getStrategyAllData(
                liquidationStrategy.mergeId
            );
            LibStrategyConfig._ensureAdminAndActive(liquidationMergeStrategy, liquidationStrategy.owner);
        }
        LibStrategyLiquidate._updateLiquidateRequestStrategy(requestHash, liquidationStrategy);

        emit StrategyLiquidationRequested(requestHash, liquidationStrategy.strategyId, liquidationStrategy.mergeId);
    }

    function executeLiquidateStrategy(bytes32 requestHash) external {
        LibAccessControlEnumerable.checkRole(Constants.KEEPER_ROLE);
        LibStrategyLiquidate.Layout storage l = LibStrategyLiquidate.layout();
        LibPositionCore.Layout storage pl = LibPositionCore.layout();
        // 通过请求获取用户策略ID
        StrategyTypes.LiquidateStrategyRequest memory requestParam = l.userLiquidateRequest[requestHash];
        StrategyTypes.StrategyData memory strategy = pl.strategies[requestParam.strategyId];

        //校验保证金是否充足，被清算策略是否还在，校验清算价格是不是在报价之内（这里需要看看是传一个总的报价还是传策略里面单个仓位的清算报价）
        LibStrategyLiquidate._checkLiquidateStrategy(requestParam, requestHash);

        int256 makerPremium = LibPositionCore._getPremium(requestParam.option);
        int256 makerFuturePnl = LibPositionCore._getMakerFuturePnl(requestParam.future, requestParam.future);
        int256 makerPnl = makerPremium + makerFuturePnl;
        // revert Test(makerPnl);
        LibVault._marginIncrease(
            pl.strategyNFT.ownerOf(requestParam.strategyId),
            strategy.collateralAmount - uint256(-makerPnl)
        );
        LibVault._marginDecrease(requestParam.owner, requestParam.collateralAmount);

        if (requestParam.mergeId != 0) {
            StrategyTypes.MergeStrategyRequest memory mergeParam;
            mergeParam.owner = requestParam.owner;
            mergeParam.firstStrategyId = requestParam.strategyId;
            mergeParam.secondStrategyId = requestParam.mergeId;
            mergeParam.collateralAmount = requestParam.collateralAmount;
            LibPositionCore._handleMergeStrategies(mergeParam);
            pl.strategyNFT.burn(requestParam.strategyId);
        } else {
            pl.strategyNFT.transferByStrategyManager(
                pl.strategyNFT.ownerOf(requestParam.strategyId),
                requestParam.owner,
        ...
        delete l.userLiquidateRequest[requestHash];

        emit StrategyLiquidationExecuted(requestHash, requestParam.strategyId, requestParam.mergeId);
    }
}
...
library LibPositionCore {
    ...
    function _getMakerFuturePnl(
        ...
        for (uint i; i < len; ) {
            futurePnl += (int256(makerFuture[i].entryPrice) - int256(takerFuture[i].entryPrice)) * int256(makerFuture[i].size) / 1e18;
            unchecked {
                ++i;
            }
        }
...
library LibStrategyLiquidate {
    bytes32 internal constant STORAGE_SLOT = keccak256("dederi.contracts.storage.StrategyLiquidate");

    struct Layout {
        /// @notice 清算请求中需要的参数
        mapping(bytes32 => StrategyTypes.LiquidateStrategyRequest) userLiquidateRequest;
    }
    ...
    function _updateLiquidateRequestStrategy(
        bytes32 requestHash,
        StrategyTypes.LiquidateStrategyRequest memory liquidationStrategy
    ) internal {
        LibStrategyLiquidate.Layout storage l = LibStrategyLiquidate.layout();
        l.userLiquidateRequest[requestHash].strategyId = liquidationStrategy.strategyId;
        l.userLiquidateRequest[requestHash].owner = liquidationStrategy.owner;
        l.userLiquidateRequest[requestHash].mergeId = liquidationStrategy.mergeId;
        l.userLiquidateRequest[requestHash].collateralAmount = liquidationStrategy.collateralAmount;

        uint256 optionLen = liquidationStrategy.option.length;
        for (uint256 i; i < optionLen; ) {
            StrategyTypes.Option memory option_ = liquidationStrategy.option[i];
            l.userLiquidateRequest[requestHash].option.push(option_);
        ...
        uint256 futureLen = liquidationStrategy.future.length;
        for (uint256 i; i < futureLen; ) {
            StrategyTypes.Future memory future_ = liquidationStrategy.future[i];
            l.userLiquidateRequest[requestHash].future.push(future_);
    ...
    function _checkLiquidateStrategy(
        StrategyTypes.LiquidateStrategyRequest memory liquidationStrategy,
        bytes32 requestHash
    ) internal view {
        StrategyTypes.MarginItemWithHash memory marginByHash = LibMarginOracle._getPortfolioMarginInfoByHash(
        ...
        // bool isLiquidation = LibMarketPricer._isLiquidation(marginByHash.mm,marginByHash.futureUnrealizedPnl + marginByHash.optionValue, liquidationAllData);
        // if(!isLiquidation) {
        //     revert
        // }
        LibStrategyConfig._ensureAdminAndActive(liquidationAllData, liquidationAllData.owner);
        ...
        int256 newCashIn;
        if (liquidationStrategy.mergeId > 0) {
            StrategyTypes.StrategyAllData memory mergeAllData = LibPositionCore._getStrategyAllData(
                liquidationStrategy.mergeId
            );
            LibStrategyConfig._ensureAdminAndActive(mergeAllData, liquidationStrategy.owner);
            StrategyTypes.MarginItemWithId memory marginById = LibMarginOracle._getPortfolioMarginInfoByStrategyId(
                liquidationStrategy.mergeId
            );

            int256 unrealizedPnl = marginById.futureUnrealizedPnl + marginById.optionValue;
            newCashIn = LibMarketPricer._getNewCashWithLiquidationAndMerge(
                marginByHash.im,
                marginById.im,
                predictUnrealizedPnl,
                unrealizedPnl,
            ...
            newCashIn = LibMarketPricer._getNewCashWithLiquidation(
                marginByHash.im,
                predictUnrealizedPnl,
                liquidationStrategy
            );
        ...
        LibMarketPricer._checkCollateralEnough(uint256(newCashIn), liquidationStrategy.collateralAmount);
    }

    // function _getUnrealizedPnl(StrategyTypes.LiquidateStrategyRequest memory liquidationStrategy, StrategyTypes.StrategyAllData memory liquidationAllData) internal pure returns(int256 predictPnl){
    //     uint256 opLen = liquidationStrategy.option.length;
    //     uint256 fuLen = liquidationStrategy.future.length;
    //     int256 pnl;
    //     for(uint256 i; i < fuLen;) {
    //         pnl += ((int256(liquidationStrategy.future[i].entryPrice) - int256(liquidationAllData.future[i].entryPrice)) * int256(liquidationAllData.future[i].size)) / 1e18;
    //         unchecked {
    //             ++i;
    //         }
    //     }
    ...
    //         pnl += (int256(liquidationStrategy.option[i].premium) - int256())
    //     }
