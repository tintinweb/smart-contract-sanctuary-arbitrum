...
contract SynthrIssuer is Owned, MixinSystemSettings {
    using SafeMath for uint256;
    ...
    function issueSynths(
        ...
        uint256 amount,
        uint256 destChainId
    ) external onlySynthrBridgeOrWrappedSynthr returns (uint256 synthAmount, uint256 debtShare) {
    ...
    function issueMaxSynths(address from, uint256 destChainId)
        external
        onlySynthrBridgeOrWrappedSynthr
    ...
    function destIssue(
        ...
        bytes32 _synthKey,
        uint256 _synthAmount
    ) external onlySynthrBridgeOrWrappedSynthr {
        synths[_synthKey].issue(_account, _synthAmount);
        if (_account == feePool().FEE_ADDRESS()) {
            feePool().recordFeePaid(_synthAmount);
        } else {
    ...
    function destBurn(
        ...
        bytes32 _synthKey,
        uint256 _synthAmount
    ) external onlySynthrBridgeOrWrappedSynthr returns (uint256) {
        synths[_synthKey].issue(_account, _synthAmount);
        (uint256 actualDebtRemoved, , , ) = _voluntaryBurnSynths(_account, _synthKey, _synthAmount, false);
        liquidatorRewards().updateEntry(_account);
        return actualDebtRemoved;
    ...
    function _burnSynths(
        ...
        returns (
            uint256 amountBurnt,
            uint256 debtShareBurnt,
            uint256 amountBurntInUSD
        )
    {
        ...
        )
    {
        ...
        uint256 synthAmountBurntInUSD;
        (synthAmount, debtShare, synthAmountBurntInUSD) = _burnSynths(from, from, synthKey, amount);
    ...
    /* ========== MODIFIERS ========== */
    modifier onlySynthrBridgeOrWrappedSynthr() {
        require(
            msg.sender == address(wrappedSynthr()) || msg.sender == address(synthrBridge()),
