...
contract RewardTracker is IRewardTracker, BaseToken, ReentrancyGuard {
  ...
  function tokensPerSecond() external view returns (uint128) {
    return IRewardDistributor(distributor).tokensPerSecond();
  }
  ...
  function claimable(address _account) external view returns (uint) {
    ...
    unchecked {
      ...
      uint _pendingRewards = IRewardDistributor(distributor).pendingRewards() * PRECISION;
  ...
  function _updateRewards(address _account) private {
    uint _blockReward = IRewardDistributor(distributor).distribute();
    uint _supply = totalSupply(); // total supply = sum of all DepositTokens.totalSupply() aka shares
    uint _cumulativeRewardPerToken = cumulativeRewardPerToken; // aka accRewardPerShare
    ...
    if (_account != address(0)) {
      ...
      if (_claimableReward > 0 && _stakedAmount > 0) {
        ...
        // tracking averaged staked amounts used to accrue account rewards; for vesting
