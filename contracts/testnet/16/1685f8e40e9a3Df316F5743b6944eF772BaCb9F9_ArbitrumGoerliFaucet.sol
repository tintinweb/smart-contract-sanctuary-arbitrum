...
import "./interfaces/IAaveFaucet.sol";

contract ArbitrumGoerliFaucet is Ownable, Initializable, ReentrancyGuard {
    IAaveFaucet private immutable _aaveFaucet;
    ...
    constructor(address aaveFaucet_) {
        _aaveFaucet = IAaveFaucet(aaveFaucet_);
    }
    ...
    function mintTokens(address token, address to, uint256 amount) external onlyAdmin {
        uint256 maxAmount = _aaveFaucet.MAX_MINT_AMOUNT() * 10 ** IERC20Metadata(token).decimals();
        while (amount > 0) {
            uint256 mintAmount = amount > maxAmount ? maxAmount : amount;
            _aaveFaucet.mint(token, to, mintAmount);
            amount -= mintAmount;
        }
    }
