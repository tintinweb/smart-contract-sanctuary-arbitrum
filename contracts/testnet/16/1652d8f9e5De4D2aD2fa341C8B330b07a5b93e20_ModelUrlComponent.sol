...
uint256 constant ID = uint256(
    keccak256("game.piratenation.modelurlcomponent.v1")
);
...
library ModelUrlComponentStorage {
...
/**
 * @title ModelUrlComponent
 * @dev The default model gltf url for an entity
 */
contract ModelUrlComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getValue(
        ...
        value = ModelUrlComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ModelUrlComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getBytes(
        ...
        Layout memory s = ModelUrlComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete ModelUrlComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete ModelUrlComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
