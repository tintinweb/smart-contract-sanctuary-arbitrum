...
contract FairtradeReferrer is SafeOwnable, BlockContext {
    enum UpsertAction {
        ADD,
        REMOVE,
        UPDATE
    ...
    event OnReferralCodeCreated(address createdBy, address createdFor, uint256 timestamp, string referralCode);
    ...
    event OnPartnerTierSetted(address addr, string tier);

    event OnPartnerTierRemoved(address addr);

    mapping(address => Partner) public partners;
    mapping(address => Trader) public traders;
    mapping(string => address) public referralCodeToPartnerMap;
    mapping(address => string) public partnerTiers;

    bool private isInitialised;

    function init() public initializer {
        require(!isInitialised, "Contract has already been initialized");
        isInitialised = true;
        __SafeOwnable_init();
    }

    function _createReferralCode(address createdFor, string calldata referralCode) internal {
        ...
        address existingReferralCodePartner = referralCodeToPartnerMap[referralCode];
        ...
        require(bytes(existingPartner.referralCode).length == 0, "This address already has a referral code assigned.");
    ...
    function getReferralCodeByPartnerAddress(address partnerAddress) external view returns (string memory) {
        Partner memory partner = partners[partnerAddress];
        require(bytes(partner.referralCode).length > 0, "Referral code doesn't exist");
    ...
    function getMyRefereeCode() public view returns (string memory) {
        ...
        require(bytes(trader.referralCode).length > 0, "You do not have a referral code");
        return (trader.referralCode);
    }

    function _setReferralCode(string calldata referralCode, address sender) internal {
        address partner = referralCodeToPartnerMap[referralCode];
        uint256 timestamp = _blockTimestamp();
        UpsertAction action;
        string memory oldReferralCode = referralCode;

        require(partner != sender, "You cannot be a referee of a referral code you own");

        // the trader in we are performing the upserts for
        Trader storage trader = traders[sender];

        if (bytes(referralCode).length == 0 && bytes(trader.referralCode).length == 0) {
    ...
    function setPartnerTier(address partnerAddress, string calldata tier) public onlyOwner {
        require(partnerAddress != address(0x0), "Provide an address");
        require(bytes(tier).length > 0, "Provide a tier");

        Partner memory partner = partners[partnerAddress];
        require(bytes(partner.referralCode).length > 0, "Partner does not exist");

        partnerTiers[partnerAddress] = tier;
        emit OnPartnerTierSetted(partnerAddress, tier);
    }

    function removePartnerTier(address partnerAddress) public onlyOwner {
        require(partnerAddress != address(0x0), "Provide an address");

        Partner memory partner = partners[partnerAddress];
        require(bytes(partner.referralCode).length > 0, "Partner does not exist");

        string memory tier = partnerTiers[partnerAddress];
        require(bytes(tier).length > 0, "Partner is not uncapped");

        delete partnerTiers[partnerAddress];
        emit OnPartnerTierRemoved(partnerAddress);
    }

    function batchUpdatePartnerTier(address[] calldata partnerAddresses, string[] calldata tiers) public onlyOwner {
        require(partnerAddresses.length == tiers.length, "Partner list and tier list are mistmatched");
        for (uint256 i = 0; i < partnerAddresses.length; i++) {
            if (keccak256(bytes(tiers[i])) == keccak256("0")) {
                removePartnerTier(partnerAddresses[i]);
            } else {
                setPartnerTier(partnerAddresses[i], tiers[i]);
            }
        }
    }
}