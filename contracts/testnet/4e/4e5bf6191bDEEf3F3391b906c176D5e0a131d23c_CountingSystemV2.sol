...
contract Uint256Component is BaseComponent, IUint256Component {
     ...
     * @param entity Entity to get value for
     */
    function setValue(uint256 entity, uint256 value) external virtual {
     ...
     * @param entity Entity to get value for
     */
    function getValue(uint256 entity) external view virtual returns (uint256) {
        uint256 value = abi.decode(getBytes(entity), (uint256));
        return value;
    }
}
...
import {ICountingSystem} from "./ICountingSystem.sol";
import {IUint256Component} from "../core/components/IUint256Component.sol";
import {Uint256Component} from "../core/components/Uint256Component.sol";

uint256 constant ID = uint256(keccak256("game.piratenation.countingsystemv2"));

/// @notice Be sure to list your counting key in `packages/shared/src/countingIds.ts`.
contract CountingSystemV2 is ICountingSystem, GameRegistryConsumerUpgradeable {
    // key => entity => count
    mapping(uint256 => mapping(uint256 => uint256)) _counts;
    ...
    function getCount(
        ...
        IUint256Component countComponent = _getCounter(key);
        if (address(countComponent) != address(0)) {
            return countComponent.getValue(entity);
        }
    ...
    function setCount(
        ...
        IUint256Component countComponent = _getOrCreateCounter(key);
        countComponent.setValue(entity, value);
    }
    ...
    function incrementCount(
        ...
        IUint256Component countComponent = _getOrCreateCounter(key);
        uint256 newValue = countComponent.has(entity)
            ? countComponent.getValue(entity) + amount
            : amount;
        countComponent.setValue(entity, newValue);
    }

    /** INTERNAL **/

    function _getOrCreateCounter(
        uint256 key
    ) internal returns (IUint256Component) {
        IUint256Component countComponent = _getCounter(key);

        if (address(countComponent) == address(0)) {
            countComponent = new Uint256Component(address(_gameRegistry), key);
            _gameRegistry.registerComponent(key, address(countComponent));
        }
    ...
    function _getCounter(
        uint256 key
    ) internal view returns (IUint256Component) {
        // Just use the counting key as the component id
        IUint256Component countComponent = IUint256Component(
            _gameRegistry.getComponent(key)
        );
