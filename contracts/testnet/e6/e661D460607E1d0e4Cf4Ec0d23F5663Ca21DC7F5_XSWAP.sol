...
contract XSWAP {
    ...
    function depositTo(uint256 amount_, address to) external {
        ...
        balances[to] += amount_;
        TransferHelper.safeTransfer(tokenAddress, to, amount_);
    }
    ...
    function withdraw(uint256 amount_, uint256 duration) external {
        ...
        uint256 fairAmount = amount_ - (amount_ * ratio) / 1000;
    ...
    function confirmWithdraw(uint256 redeemId) external {
        ...
        balances[msg.sender] -= info.xFairAmount;
        userRedeemAmount[msg.sender] -= info.xFairAmount;
        _deleteRedeemEntry(msg.sender, redeemId);
