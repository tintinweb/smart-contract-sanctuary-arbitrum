...
abstract contract WithClaimer is GeneralIDO {
    ...
    function initializeClaimer() public initializer{
        isPaused = false;
    }

    function setClaimInfo(address _token, uint256[] memory times, uint256[] memory percents) public onlyOwnerOrAdmin {
        token = IERC20(_token);
    ...
    function setToken(address _token) external onlyOwnerOrAdmin {
        require(claims[0].unlockTime > block.timestamp, "Claimer: Claim started");
        token = IERC20(_token);
    }
