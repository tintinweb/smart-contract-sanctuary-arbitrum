...
import "./DynamicNFTWithChainlink.sol";

// Interface for AAVE V3 lending protocol
...
contract InsuranceContract is DynamicNFTWithChainlink, ReentrancyGuard {
    address public lendingProtocol;
    address public oracle;
    address public fakeUSDCAddress;
    ...
    constructor(
        ...
        address _oracle,
        address _fakeUSDCAddress,
        uint256 _insuranceFee,
        uint256 _claimFee
    ) {
        lendingProtocol = _lendingProtocol;
        oracle = _oracle;
        fakeUSDCAddress = _fakeUSDCAddress;
        insuranceFee = _insuranceFee;
        claimFee = _claimFee;
    }

    function purchaseInsurance() external nonReentrant {
        require(!userPositions[msg.sender].isInsured, "Already insured");

        // Get user's position from the lending protocol
        (
            uint256 totalCollateral,
            uint256 totalDebt,
            uint256 availableBorrows,
        ...
        ) = IAaveV3LendingPool(lendingProtocol).getUserAccountData(msg.sender);

        require(totalCollateral > 0, "No active position");

        // Calculate the insured amount, simulate risk
        uint256 totalInsured;
        if (totalDebt > availableBorrows) {
            totalInsured = totalDebt / 2;
        } else {
            totalInsured = totalDebt;
        }

        // Transfer fake USDC as insurance fee
        require(
            IERC20(fakeUSDCAddress).transferFrom(
                msg.sender,
                address(this),
                insuranceFee
            ),
    ...
    function claimInsurance(uint256 tokenId) external nonReentrant {
        require(ownerOf(tokenId) == msg.sender, "Not the owner");
        require(userPositions[msg.sender].isInsured, "Not insured");

        // Check position health from the oracle
        bool isHealthy = userPositions[msg.sender].positionHealth > 0;

        if (!isHealthy) {
            // Burn the NFT
            _burn(tokenId);

            // Transfer insured amount as a claim fee
            require(
                IERC20(fakeUSDCAddress).transfer(msg.sender, claimFee),
                "Transfer failed"
            );
        }

        userPositions[msg.sender].isInsured = false;
    }
}
...
contract DynamicNFTWithChainlink is ERC721, Ownable {
    // Mapping to store dynamic data for each NFT
    mapping(address => PolicyNFT) public userPositions;

    // Mapping to store request ID for randomness
    ...
    struct PolicyNFT {
        uint256 lentSize; // Collateral
        uint256 borrowSize; // Debt
        uint256 totalInsured;
    ...
    // Mint a soul-bound dynamic NFT with initial dynamic data
    function mintSoulBoundDynamicNFT(
        uint256 _lentSize,
        uint256 _borrowSize,
        uint256 _totalInsured,
        address _clientAddress,
        uint256 _positionHealth
    ) external {
        uint256 tokenId = tokenIdCounter;
        tokenIdCounter++;

        _mint(_clientAddress, tokenId);

        // Initial dynamic data
        userPositions[_clientAddress] = PolicyNFT({
            lentSize: _lentSize,
            borrowSize: _borrowSize,
            totalInsured: _totalInsured,
            clientAddress: _clientAddress,
            isInsured: true,
            positionHealth: _positionHealth // Initial health value (to be updated by Chainlink)
        });
    }
