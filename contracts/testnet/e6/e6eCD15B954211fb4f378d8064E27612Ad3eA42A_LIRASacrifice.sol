...
contract LIRASacrifice is Context {
    ...
    uint public sacrificableLimit = 0;
    ...
    function sacrifices() public view returns (Sacrifice[] memory sacrifices_) {
        for (uint i = 0; i < _sacrifices.length; i++) {
            if (_sacrifices[i].owner == _msgSender()) {
                sacrifices_[sacrifices_.length] = _sacrifices[i];
            }
        }
    }
    ...
    function sacrificable() public view canSacrifice returns (uint) {
        return sacrificableLimit - round().sacrified;
    }
    ...
    function sacrifice(uint amount_) public canSacrifice {
        ...
        require(round().sacrified + amount_ <= sacrificableLimit, 'LIRA_SACRIFICE_ROUND_SOLD_OUT');

        IERC20(sacrifiedToken).transferFrom(_msgSender(), address(this), amount_);

        _sacrifices.push(Sacrifice(_msgSender(), amount_.add(amount_.div(100).mul(bonus())).mul(10 ** 8), false));
    ...
    function redeem(address to) public canRedeem {
        ...
        uint liraSupply = redeemed.add(IERC20(redeemToken).balanceOf(address(this)));
