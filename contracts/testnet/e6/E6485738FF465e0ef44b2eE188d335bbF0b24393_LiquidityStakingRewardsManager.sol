...
contract LiquidityStakingRewardsManager is Ownable {
    ...
    function viewLiquidityStakingRewardsInfos(uint256 startIndex_, uint256 endIndex_) external view returns (LiquidityStakingRewardsInfo[] memory liquidityStakingRewardsInfos){
        if (startIndex_ >= 0 && endIndex_ >= startIndex_) {
            ...
            uint index = 0;
            for (uint _i = startIndex_; _i < ((endIndex_ < total) ? endIndex_ : total);) {
    ...
    function createLiquidityStakingRewards(uint256 liquidityTokenId_) external onlyOwner {
        ...
        //
        liquidityTokenIdLiquidityStakingRewards[liquidityTokenId_] = address(liquidityStakingRewards);
        //
    ...
    function withdrawRewards() external onlyOwner {
        IERC20(rewardsToken).transfer(_msgSender(), IERC20(rewardsToken).balanceOf(address(this)));
    }
    ...
    function distributionRewards(uint256 liquidityTokenId_, uint256 ratio_, uint256 rewardsDuration_) external onlyOwner {
        address liquidityStakingRewards = liquidityTokenIdLiquidityStakingRewards[liquidityTokenId_];
        require(liquidityStakingRewards != address(0), "LiquidityStakingRewardsManager: LiquidityStakingRewards is zero address");
        require(ratio_ > 0, "LiquidityStakingRewardsManager: ratio is zero");
        //
        uint256 rewardAmount_ = rewardsAmount * 1e18 * ratio_ / 100;
        if (rewardAmount_ > 0) {
            IERC20(rewardsToken).transfer(liquidityStakingRewards, rewardAmount_);
            ILiquidityStakingRewards(liquidityStakingRewards).notifyRewardAmount(rewardAmount_, rewardsDuration_);
        }
    }
    ...
    function removeLiquidityTokenId(uint256 tokenId_) private {
        ...
        delete liquidityTokenIdLiquidityStakingRewards[tokenId_];
        delete liquidityRewardsRatio[tokenId_];
