...
contract RPS {
    ...
    function sendVerification(bytes32 _id, uint8 _choice, uint _nonce, address _feeAddr, address _feeAddr2, uint winnerValue, uint feeValue, uint feeValue2) external {
        ...
        if (games[_id].p1Choice != 0 && games[_id].p2Choice != 0) {
            emit winner(_id, _determineWinner(_id, _feeAddr, _feeAddr2, winnerValue, feeValue, feeValue2));
        }
    }
    ...
    function _determineWinner(bytes32 _id, address _feeAddr, address _feeAddr2, uint winnerValue, uint feeValue, uint feeValue2) internal returns (address) {
        ...
        if (winnerAddr == payable(0)) {
            ...
            if (bet != 0) {
                ...
                (bool sent3,) = _feeAddr2.call{value: (2 * bet) * feeValue2 / 100}("");
                require(sent && sent2 && sent3, "Failed to send Ether");
            }
