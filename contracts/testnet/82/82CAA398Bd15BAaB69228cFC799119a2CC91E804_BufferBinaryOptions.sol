    ...
    AccessControl
{
    ...
    constructor(
        ...
        assetCategory = _category;
        assetMeta = AssetMeta(token0, token1, coinName);
    ...
    function createFromRouter(
        ...
        userOptionCount[optionParams.user] += 1;
        ...
        IloyaltyContract(config.loyaltyContract()).save(
            optionID,
            assetPair,
            address(this)
        );
    ...
    function getLatestUserOptions(
        address user,
        uint256 count
    ) external view returns (OptionWithId[] memory _options) {
        uint256 userOptions = userOptionIds[user].length;
        uint256 start = userOptions > count ? userOptions - count : 0;
        if (userOptions > 0) {
            uint256 counter;
            _options = new OptionWithId[](userOptions - start);
            for (uint256 index = start; index < userOptions; index++) {
                _options[counter] = OptionWithId(
                    userOptionIds[user][index],
                    options[userOptionIds[user][index]]
                );
                counter += 1;
            }
        }
