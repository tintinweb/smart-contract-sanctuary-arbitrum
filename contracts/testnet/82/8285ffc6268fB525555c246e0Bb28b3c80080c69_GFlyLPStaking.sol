...
import "../interfaces/gFly/IGFlyLPStaking.sol";
...
contract GFlyLPStaking is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, IGFlyLPStaking {
    ...
    //extension for unlocking stakes
    bool public override unlockable;
    mapping(uint256 => bool) public permanentlyLockedStake;

    //extension for gFLY/ETH LP
    IERC20Upgradeable public gFlyLPETH;
    IStakingUtils public stakingUtils;
    ...
    modifier whenUnlockable() {
        require(unlockable, "GFlyLPStaking:NOT_UNLOCKABLE");
        _;
    }
    ...
    // 0 = gFLY/Magic, 1 = gFLY/USDC, 2 = gFLY/PSI, 3 = gFLY/ETH
    modifier onlyAvailableStakeType(uint16 stakeType) {
        require(stakeType == 0 || stakeType == 3, "GFlyLPStaking:UNALLOWED_STAKE_TYPE");
        _;
    }
    ...
    function stake(
        ...
        permanentlyLockedStake[currentStakeId] = true;
    ...
    function unStakeAll() external override nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            ...
            if (!stakeById[stakeId].autoIncreaseLock && currentEpoch >= unlockEpoch) {
                ...
                totalAmounts[stakeType] += stakeAmount;
                if (stakeType == 0) {
     ...
     * @dev Unlock a stake
     */
    function unlockStake(uint256 stakeId) external override whenNotPaused whenUnlockable onlyStakeOwner(stakeId) {
        _unlockStake(stakeId);
    }

    /**
     * @dev Unlock all stakes of the sender
     */
    function unlockAllStakes() external override whenNotPaused whenUnlockable {
        for (uint256 i = 0; i < stakeIdsByAddress[msg.sender].values().length; i++) {
            if(isStakeUnlockable(stakeIdsByAddress[msg.sender].at(i))) {
                _unlockStake(stakeIdsByAddress[msg.sender].at(i));
            }
        }
    }

    /**
     * @dev Unlock a batch of stakes of the sender
     */
    function unlockBatchStakes(uint256[] memory stakeIds) external override whenNotPaused whenUnlockable {
        for (uint256 i = 0; i < stakeIds.length; i++) {
            require(stakeById[stakeIds[i]].owner == msg.sender, "GFlyLPStaking:NOT_STAKE_OWNER");
            require(
                isStakeUnlockable(stakeIds[i]),
                "GFlyLPStaking:STAKE_IS_NOT_UNLOCKABLE"
            );
            _unlockStake(stakeIds[i]);
        }
    }

    function swapMagicPairToETHPair(uint256 stakeId) external override nonReentrant whenNotPaused onlyStakeOwner(stakeId) {
        require(stakeById[stakeId].stakeType == 0, "GFlyLPStaking:INCORRECT_STAKE_TYPE");
        _claim(stakeId);
        uint256 originalAmount = stakeById[stakeId].amount;
        uint256 ethLPBefore = gFlyLPETH.balanceOf(address(this));
        gFlyLP.approve(address(stakingUtils),originalAmount);
        stakingUtils.swapMagicPairToETHPair(originalAmount, stakeById[stakeId].owner);
        uint256 lpTokenAmount = gFlyLPETH.balanceOf(address(this)) - ethLPBefore;
        stakeById[stakeId].stakeType = 3;
        stakeById[stakeId].amount = lpTokenAmount;
        emit MagicPairSwappedForETHPair(msg.sender, stakeId, originalAmount, lpTokenAmount);
    }
    ...
    /**
    * @dev Returns if a stake is unlockable or not
     */
    function isStakeUnlockable(uint256 stakeId) public view override returns(bool) {
        return !permanentlyLockedStake[stakeId];
    }
    ...
    function distributeEmissions() external override whenNotPaused {
        ...
        for (uint16 i = currentEpoch; i <= currentEpoch + 365; i++) {
            totalMiningPower += (((((unlockableAtEpoch[i] + autoIncreasedForLocks[i - currentEpoch]) * miningPowerCoefficientsInWei[0]) / 1e18) * uint256(i - currentEpoch)) / 365);
            uint256 miningPowerOfAdditionalPairs;
            for(uint16 j = 1; j <= 3; j++) {
                miningPowerOfAdditionalPairs += (((((unlockableAtEpochForAdditionalPairs[j][i] +
                autoIncreasedForLocksForAdditionalPairs[j][i - currentEpoch]) * miningPowerCoefficientsInWei[j]) /
                1e18) * uint256(i - currentEpoch)) / 365)   ;
            }
            totalMiningPower += miningPowerOfAdditionalPairs;
        }
    ...
    /**
     * @dev Set the unlockable state for stakes
     */
    function setUnlockable(bool state) external override onlyAdmin {
        unlockable = state;
        emit Unlockable(state);
    }

    /**
     * @dev Set the GFlyLPETH pair
     */
    function setGFlyLPETH(address gFlyLPETH_) external {
        require(hasRole(ADMIN_ROLE, msg.sender), "GFlyStaking:ACCESS_DENIED");
        require(gFlyLPETH_ != address(0), "GFlyLPStaking:INVALID_ADDRESS");
        gFlyLPETH = IERC20Upgradeable(gFlyLPETH_);
    }

    /**
     * @dev Set staking utils
     */
    function setStakingUtils(address stakingUtils_) external {
        require(hasRole(ADMIN_ROLE, msg.sender), "GFlyStaking:ACCESS_DENIED");
        require(stakingUtils_ != address(0), "GFlyLPStaking:INVALID_ADDRESS");
        stakingUtils = IStakingUtils(stakingUtils_);
    }
    ...
    function _unlockStake(uint256 stakeId) internal {
        require(
            isStakeUnlockable(stakeId),
            "GFlyLPStaking:STAKE_IS_NOT_UNLOCKABLE"
        );
        if (stakeById[stakeId].autoIncreaseLock) {
            _autoIncreaseLock(stakeId, false);
        }
        stakeById[stakeId].pendingRewards = claimableById(stakeId);
        stakeById[stakeId].lastProcessEpoch = currentEpoch;
        if (stakeById[stakeId].stakeType == 0) {
            unlockableAtEpoch[currentEpoch] += stakeById[stakeId].amount;
            unlockableAtEpoch[stakeById[stakeId].unlockEpoch] -= stakeById[stakeId].amount;
        } else {
            unlockableAtEpochForAdditionalPairs[stakeById[stakeId].stakeType][currentEpoch] += stakeById[stakeId]
                .amount;
            unlockableAtEpochForAdditionalPairs[stakeById[stakeId].stakeType][
                stakeById[stakeId].unlockEpoch
            ] -= stakeById[stakeId].amount;
        }
        stakeById[stakeId].unlockEpoch = currentEpoch;
        stakeById[stakeId].lock = 0;
        emit StakeUnlocked(msg.sender, stakeId, 0, currentEpoch);
    }
    ...
    function _transferStakeOfTypeToContract(
        ...
        if (stakeType == 0) {
            ...
            gFlyLPETH.safeTransferFrom(account, address(this), amount);
        }
    }
    ...
    function _transferStakeOfTypeFromContract(
        ...
        if (stakeType == 0) {
            ...
            gFlyLPETH.safeTransfer(account, amount);
        }
    }
...
interface IGFlyLPStaking {
    ...
    event MagicPairSwappedForETHPair(address indexed account, uint256 indexed stakeId, uint256 originalAmount, uint256 lpTokenAmount);
    ...
    function swapMagicPairToETHPair(uint256 stakeId) external;
}
