...
uint256 constant ID = uint256(
    keccak256("game.piratenation.tradelicensecomponent.v3")
);

struct Layout {
    bool hasTradeLicense;
}

library TradeLicenseComponentStorage {
...
/**
 * @title TradeLicenseComponent
 * @dev Trade License Component
 */
contract TradeLicenseComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Flag tracking if the player wallet has trade license
        keys[0] = "has_trade_license";
     ...
     *
     * @param entity Entity to get value for
     * @param hasTradeLicense Flag tracking if the player wallet has trade license
     */
    function setValue(
        uint256 entity,
        bool hasTradeLicense
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(hasTradeLicense));
    }
    ...
    function getLayoutValue(
        ...
        value = TradeLicenseComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return hasTradeLicense Flag tracking if the player wallet has trade license
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (bool hasTradeLicense) {
        if (has(entity)) {
            Layout memory s = TradeLicenseComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (hasTradeLicense) = abi.decode(_getEncodedValues(s), (bool));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = TradeLicenseComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](1);
        values[0] = abi.encode(s.hasTradeLicense);
    }
    ...
    function setBytes(
        ...
        Layout memory s = TradeLicenseComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.hasTradeLicense) = abi.decode(value, (bool));
    ...
    function remove(
        ...
        delete TradeLicenseComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete TradeLicenseComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.hasTradeLicense = value.hasTradeLicense;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.hasTradeLicense));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.hasTradeLicense);
    }
}
...
import "../../traits/TraitsConsumer.sol";
    ...
    TradeLicenseChecks
{
    ...
    function emitTransferEvent(
        ...
        // sender ineligible
        if (from != address(0) && !_hasTradeLicense(from)) {
            return;
        }
    ...
    function balanceOfBatch(
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            if (!_hasTradeLicense(accounts[i])) {
                balances[i] = 0;
            }
        }
