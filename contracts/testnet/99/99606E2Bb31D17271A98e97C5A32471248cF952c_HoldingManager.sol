...
import "./interfaces/vyper/IMinter.sol";
...
contract Holding is IHolding {
    ...
    function mint(address _minter, address _gauge)
        external
        override
        onlyAllowed
    {
    ...
    modifier onlyAllowed() {
        ...
        require(
            msg.sender == manager.strategyManager() ||
                ...
                msg.sender == manager.liquidationManager() ||
...
contract HoldingManager is IHoldingManager, ReentrancyGuard, Ownable {
    ...
    address public holdingImplementationReference;
        ...
        returns (address)
    {
        ...
        // instead of deploying the contract, it is cloned
        address newHoldingAddress = Clones.clone(
            holdingImplementationReference
        );
    ...
    {
        _wrap();
        _deposit(address(this), _getManager().WETH(), msg.value);
    }
    ...
    function withdraw(address _token, uint256 _amount)
        ...
        nonReentrant
    {
        (uint256 userAmount, uint256 feeAmount) = _withdraw(_token, _amount);
        ...
        nonReentrant
    {
        ...
        _unwrap(_amount);
        (uint256 userAmount, uint256 feeAmount) = _withdraw(
            wethAddress,
            _amount
        );
        ...
        if (feeAmount > 0) {
            // solhint-disable-next-line avoid-low-level-calls
            (bool feeSuccess, ) = payable(_getManager().feeAddress()).call{
                value: feeAmount
            }("");
    ...
    /// @param _minter IMinter address
    /// @param _gauge gauge to mint for
    function mint(address _minter, address _gauge) external override {
        IHolding(userHolding[msg.sender]).mint(_minter, _gauge);
    }
        ...
        returns (uint256)
    {
        ...
        OperationsLib.safeApprove(_tokenIn, address(_dex), _amountIn);

        uint256 amountSwapped = _dex.swap(
            _tokenIn,
            _tokenOut,
            _amountIn,
            address(this),
        ...
        validHolding(userHolding[msg.sender])
    {
        ...
        emit Borrowed(holding, _token, _amount, _mintDirectlyToUser);
    }
        ...
        notPaused
    {
        ...
        for (uint256 i = 0; i < _data.length; i++) {
            _getStablesManager().repay(
                ...
                _data[i].token,
                _data[i].amount,
                _repayFromUser ? msg.sender : userHolding[msg.sender]
            );
            emit Repayed(
                userHolding[msg.sender],
                _data[i].token,
        ...
        validHolding(userHolding[msg.sender])
    {
        ...
        emit Repayed(userHolding[msg.sender], _token, _amount, _repayFromUser);
    }

    /// @notice user wraps native coin
    /// @dev this function must receive ETH in the transaction
    function wrap() external payable override nonReentrant notPaused {
        _wrap();
        IERC20(_getManager().WETH()).transfer(msg.sender, msg.value);
    }

    /// @notice user unwraps wrapped native coin
    /// @param _amount the amount to unwrap
    function unwrap(uint256 _amount) external override nonReentrant notPaused {
        IERC20(_getManager().WETH()).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
    ...
    function _wrap() private {
        ...
        emit NativeCoinWrapped(msg.sender, msg.value);
    }

    function _unwrap(uint256 _amount) private {
        IWETH(_getManager().WETH()).withdraw(_amount);
        emit NativeCoinUnwrapped(msg.sender, _amount);
    }
        ...
        returns (uint256, uint256)
    {
        ...
        if (
            ...
            require(
                _getStablesManager().isSolvent(_token, userHolding[msg.sender]),
                "3009"
            );
        }
        ...
        if (withdrawalFee > 0) {
            withdrawalFeeAmount = OperationsLib.getFeeAbsolute(
                _amount,
                withdrawalFee
            );
        }
