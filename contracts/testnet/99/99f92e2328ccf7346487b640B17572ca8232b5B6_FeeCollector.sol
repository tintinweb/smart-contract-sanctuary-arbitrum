...
contract FeeCollector is IFeeCollectorV2, AccessControl {
    ...
    event FeesCollected(IAsset asset, uint256 amount);

    constructor() {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function collectFees(IAsset asset_, uint256 amount_) external onlyRole(COLLECTOR_ROLE) {
        IERC20(asset_.token()).safeTransferFrom(msg.sender, address(this), amount_);
        emit FeesCollected(asset_, amount_);
    }

    function withdraw(IERC20 token_, uint256 amount_) public onlyRole(DEFAULT_ADMIN_ROLE) {
