...
contract MoonPepe is ERC20, Ownable {
    ...
    uint256 private immutable i_limitPerWalletBeforeTime =
        12620700000 * 10 ** 6; // 3 Percent of total supply
    uint256 private immutable i_limitPerWalletAfterTime = 42069000000 * 10 ** 6; // 10 Percent of total supply
    ...
    function _transfer(
        ...
        if (block.timestamp <= s_endTime) {
            require(
                ...
                "Owned amount exceeds the maximum limit"
            );
        } else {
            require(
