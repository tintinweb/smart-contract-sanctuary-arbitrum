...
contract DungeonSystem is IDungeonSystem, GameRegistryConsumerUpgradeable {
    ...
    function startDungeonBattle(
        ...
        // Validate
        _validateStartDungeonBattle(account, params, progressSystem);
        ...
        // Record progress
        _markStartDungeonBattle(account, battleEntity, params, progressSystem);
    ...
    function endDungeonBattle(
        ...
        // Setup
        address account = _getPlayerAccount(_msgSender());
        IDungeonMapSystem mapSystem = IDungeonMapSystem(
        ...
        _markEndDungeonBattle(account, params, progressSystem);

        // Grant loot if the battle was successful.
        DungeonNode memory node = mapSystem.getDungeonNode(
    ...
    /**
     * @dev Validates the parameters for the startDungeonBattle function.
     */
    function _validateStartDungeonBattle(
        address account,
        StartDungeonBattleParams calldata params,
        IDungeonProgressSystem progressSystem
    ) internal view {
        ...
        if (
            IDungeonMapSystem(_getSystem(DUNGEON_MAP_SYSTEM_ID))
                .getDungeonMap(trigger.dungeonMapEntity)
                .nodes
                .length == 0
        ) {
    ...
    function _validateEndDungeonBattle(
        ...
        EndDungeonBattleParams calldata params,
        IDungeonMapSystem mapSystem,
        IDungeonProgressSystem progressSystem
    ) internal {
        // Check that the dungeon exists.
        if (
            mapSystem.getDungeonMap(trigger.dungeonMapEntity).nodes.length == 0
        ) {
            revert DungeonMapNotFound(params.dungeonTriggerEntity);
        }
