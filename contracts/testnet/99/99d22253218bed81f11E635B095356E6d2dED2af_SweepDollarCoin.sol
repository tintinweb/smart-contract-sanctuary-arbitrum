    ...
    PausableUpgradeable
{
    ...
    function _debitFrom(address _from, uint16 _dstChainId, bytes memory _toAddress, uint _amount) internal override returns(uint) {
        if (
            transfer_approver_address != address(0) &&
            !transferApprover.checkTransfer(_from, address(uint160(bytes20(keccak256(_toAddress)))))
        ) revert TransferNotAllowed();
    ...
    function _creditTo(uint16 _srcChainId, address _toAddress, uint _amount) internal override returns(uint) {
        if (
            transfer_approver_address != address(0) &&
