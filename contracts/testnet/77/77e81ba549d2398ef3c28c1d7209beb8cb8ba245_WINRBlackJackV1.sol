...
contract WINRBlackJackV1 is IWINRBlackJackV1, CoreLight {
        ...
        returns (uint256 gameIndex_, uint32 handIndex_)
    {
        unchecked {
            ...
            if(game_.status != GameStatus.NONE || game_.status != GameStatus.FINISHED){
                revert("Blackjack: Game is not finished");
            }
            ...
            if (_amountHands == 1) {
                ...
                handIndex_ = _addHandToGame(_chipAmounts[0], gameIndex_, _tokenAddress); // Place the bet for the first hand
                handIndexesGame[msg.sender][0] = handIndex_;
                handIndexesGame[msg.sender][1] = _addHandToGame(_chipAmounts[0], gameIndex_, _tokenAddress); // Place the bet for the second hand
                activeHandsInGame[msg.sender][0] = true;
                ...
                handIndexesGame[msg.sender][0] = handIndex_;
                handIndexesGame[msg.sender][1] = _addHandToGame(_chipAmounts[0], gameIndex_, _tokenAddress); // Place the bet for the second hand
                handIndexesGame[msg.sender][2] = _addHandToGame(_chipAmounts[0], gameIndex_, _tokenAddress); // Place the bet for the third hand
                activeHandsInGame[msg.sender][0] = true;
                activeHandsInGame[msg.sender][1] = true;
    ...
    // function _returnCompletedHandsInGame(address _playerAddress)
    //     internal
    //     view
    //     returns (uint256[] memory completedHands_, uint256 length_)
    // {
    //     Game memory game = games[_playerAddress];
    //     uint32 amountHands = game.amountHands;

    //     unchecked {
    //         uint256 count = 0;
    //         for (uint32 i = 0; i < amountHands; ++i) {
    //             if (!activeHandsInGame[_playerAddress][i]) {
    //                 count++;
    //             }
    //         }
    ...
    //         for (uint32 i = 0; i < amountHands; ++i) {
    //             if (!activeHandsInGame[_playerAddress][i]) {
    //                 completedHands_[index] = handIndexesGame[_playerAddress][i];
    //                 index++;
    //             }
    //         }
    ...
    //     for (uint32 i = 0; i < amountHands; ++i) {
    //         allHands_[i] = handIndexesGame[_playerAddress][i];
    //     }
    ...
    //         for (uint32 i = 0; i < amountHands; ++i) {
    //             if (activeHandsInGame[_playerAddress][i]) {
    //                 activeHands_[index] = handIndexesGame[_playerAddress][i];
    //                 index++;
    //             }
    //         }
    ...
    // function returnGameAddress()

    // function returnGame(address _player) external view returns (Game memory game_) {
    //     game_ = games[_player];
    //     // if game is finished, return an empty game_ struct
    //     if (game_.status == GameStatus.FINISHED) {
    //         // return empty game sruct
    //         Game memory emptyGame_;
    //         return emptyGame_;
    //     } else if (game_.status == GameStatus.NONE) {
    //         revert("Blackjack: Game does not exist");
    //     } else {
    //         return game_;
    //     }
    // }
...
interface IWINRBlackJackV1 {
    ...
    struct Game {
        ...
        uint32 amountHands;
        // uint32[5] handIndexesGame;
        // address playerAddress;
        // address wagerAsset;
        bool canInsure;
        bool awaitingRandomness;
        // bool[5] activeHandsInGame;
        GameStatus status;
    }
