...
contract OrderBook is Governable, ReentrancyGuard {
    ...
    function executeOrders(
        ...
        require(_addresses.length == _orderIndexs.length, 'OrderBook: not same length');
        isKeeperCall = isKeeper[msg.sender];

        for (uint256 i = 0; i < _addresses.length; i++) {
            try this.executeOrder(_addresses[i], _orderIndexs[i], _feeReceiver) {}
            catch Error(string memory executionError) {
                emit ExecuteOrderError(_addresses[i], _orderIndexs[i], executionError);
            }
            catch (bytes memory lowLevelData) {
                emit ExecuteOrderFailure(_addresses[i], _orderIndexs[i], lowLevelData);
            }
        }
    }
    ...
    function cancelMultiple(
        ...
        for (uint256 i = 0; i < _orderIndexs.length; i++) {
            cancelOrder(_address, _orderIndexs[i]);
        }
    }
