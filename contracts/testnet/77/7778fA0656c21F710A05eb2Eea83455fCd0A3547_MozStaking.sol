...
contract MozStaking is Ownable {
    ...
    mapping(address => uint256) public xMozBalances; // User's xMOZ balances
    mapping(address => RedeemInfo[]) public userRedeems; // User's redeeming instances
    uint256 public totalRedeemingMozAmount;
    ...
    */
    function getXMozBalance(address userAddress) external view returns (uint256 redeemingAmount) {
        uint256 balance = xMozBalances[userAddress];
        return balance;
    }
    ...
    */
    function redeem(uint256 xMozAmount, uint256 duration) external  {
        ...
        totalRedeemingMozAmount += mozAmount;
        require(mozaicToken.balanceOf(address(this)) > totalRedeemingMozAmount, "Insufficient balance");
    ...
    */
    function finalizeRedeem(uint256 redeemIndex) external  validateRedeem(msg.sender, redeemIndex) {
        ...
        xMozBalances[msg.sender] = redeemingAmount - _redeem.xMozAmount;
        _finalizeRedeem(msg.sender, _redeem.xMozAmount, _redeem.mozAmount);
        totalRedeemingMozAmount -= _redeem.mozAmount;
        // remove redeem entry
    ...
    */
    function cancelRedeem(uint256 redeemIndex) external  validateRedeem(msg.sender, redeemIndex) {
        ...
        xMozBalances[msg.sender] = redeemingAmount - _redeem.xMozAmount;
        totalRedeemingMozAmount -= _redeem.mozAmount;
        xMozToken.transfer(msg.sender, _redeem.xMozAmount);
    ...
    */
    function _finalizeRedeem(address userAddress, uint256 xMozAmount, uint256 mozAmount) internal {
        ...
        require(mozaicToken.balanceOf(address(this)) >= mozAmount, "Insufficient Moz Balance");
        ...
        xMozToken.burn(xMozAmount, address(this));
