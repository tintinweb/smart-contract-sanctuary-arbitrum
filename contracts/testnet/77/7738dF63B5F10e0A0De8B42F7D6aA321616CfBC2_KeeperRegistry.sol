...
event GasDebug(uint256 attmpt, uint256 gas);
 ...
 emit GasDebug(0, gasAmount);
    uint256 startGas = gasleft();
    emit GasDebug(1, startGas);
    startGas = startGas-PERFORM_GAS_CUSHION;
    startGas = startGas - startGas/64;
      emit GasDebug(2, startGas);
    assembly {
      let g := gas()
    ...
    returns (bool success)
  {
    ...
    emit UpkeepPerformed(tx.gasprice, success, params.from, 0, params.performData);
    return success;
  }
