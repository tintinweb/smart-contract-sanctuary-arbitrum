...
contract DEVIL is Context, IERC20, Ownable {
    ...
    function configureBuy(uint256 buyTax) external onlyOwner(){
        _totalBuyTax = buyTax;
        require(_totalBuyTax <= 9 , "Max buy fee under 9%");
    }

    function configureSell(uint256 sellTax) external onlyOwner(){
        _totalSellTax = sellTax;
        require(_totalSellTax <= 9 , "Max sell fee under 9%");
    }

    function configureMarketAddr(address payable account) external onlyOwner{
        marketingWallet = account;
    }

    function setTradeStatusOpen() external onlyOwner{
        tradeOpen = true;
    }
