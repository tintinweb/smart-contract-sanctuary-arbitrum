...
// USER FUNCTIONS
    ...
    {
        if (items[itemID].maxSupply != 0) {                                  // If a max supply is defined
    ...
    function mintItems(uint256[] calldata itemIDs, uint256[] calldata amounts, address recipient)
        ...
        onlyAllowed
    {
        uint256 total = itemIDs.length;                                                // Store the total items IDs passed
        require (total == amounts.length,                                              "Items arrays length's don't match");
        uint256 itemID;
        for(uint256 i = 0; i < total; i++){                                            // Loop through the contracts
            itemID = itemIDs[i];                                                       // Store the itemID in local variable
        ...
        _mintBatch(recipient, itemIDs, amounts, "");                                   // Mint items
    }
...
// ADMIN FUNCTIONS
        ...
        onlyIfItemExists(itemID)
    {
        ...
        _mint(msg.sender, itemID, amount, "");                               // Mint item
    }
...
abstract contract ItemTypes is ERC1155Supply, ERC1155Burnable, ReentrancyGuard, Permissioned {
...
// STATE VARIABLES
    ...
    /// @dev Item types e.g., itemTypes[0] == 'Reward',  itemTypes[1] == 'Equip', itemTypes[2] == 'Health',
    string[] itemTypes;
...
// EVENTS
    ...
    event ItemAdded(address indexed account, uint256 itemID, string name, string description, string imageUrl, uint256 totalSupply, bool mintingActive, uint256 rarity, uint256 itemType, uint256 value1);

    /// @dev Adds a new item to the game
    /// @param itemName name of the item asset
    /// @param itemDescription description of the item asset
    /// @param imageUrl URI of the image
    /// @param animationUrl URI of the animation
    /// @param maxSupply the cap on the assets
    /// @param rarity stores the item rarity
    /// @param itemType stores the items type
    /// @param value1 stores an undefined value
    function addItem(
        ...
        onlyOwner 
    {
        ...
        ItemType memory item = ItemType(maxSupply, itemName, itemDescription, imageUrl, 
                                    animationUrl, true, rarity, itemType, value1);            // Create new item
        items[newItemId] = item;                                                              // Add item to the array
        totalItems++;                                                                         // Increment total items
        emit ItemAdded(_msgSender(), newItemId, itemName, itemDescription, imageUrl,
                       maxSupply, true, rarity, itemType, value1);                            // Write event
    }
    ...
    /// @param animationUrl URI of the animation
    /// @param maxSupply The cap on the assets
    /// @param mintingActive start or stop minting for a specific item
    /// @param rarity stores the item rarity
    /// @param itemType stores the items type
    /// @param value1 stores an undefined value
    function updateItem(
        ...
        onlyIfItemExists(itemID)
    {
        ItemType storage item = items[itemID];  // Shortcut accessor
        item.name = itemName;                   // Set name
        item.description = itemDescription;     // Set description
        ...
        item.mintingActive = mintingActive;     // Flip minting active
        item.rarity = rarity;                   // Set rarity
        item.itemType = itemType;               // Set type
        item.value1 = value1;                   // Set value1
    }
    ...
    /// @dev Add / Update the item types array
    /// @param newItemTypes an array of the item types available eg ["Reward","Equip"]
    function replaceItemTypes(string[] memory newItemTypes)
        public
        onlyOwner
    {
        if (itemTypes.length > 0) {
            delete itemTypes;       // Remove the old item types
        }
        itemTypes = newItemTypes;   // Add the item names to the array
    }
...
// GETTERS
    ...
    {
        uint256 total = totalItems;                                // Store total number of items into a local variable
        if ( total == 0 ) {
            return new ItemType[](0);                              // if no items added, return an empty array
        } else {
            ItemType[] memory _allItems = new ItemType[](total);
            for(uint256 i = 0; i < total; i++){                    // Loop through the items
                _allItems[i] = items[i];                           // Add item to array
            }
            return _allItems;                                      // Return the array
        }
    }

    /// @dev Returns a list of all item types
    function getItemTypes()
        external
        view
        returns (string[] memory allItemTypes) 
    {
    ...
    function getItemType(string memory itemTypeName)
        public
        view
        returns (
            uint256 itemType
    ...
    {
        uint256 total = itemTypes.length;
        for(uint256 i = 0; i < total; i++){                     // Loop through the items
            if (keccak256(abi.encodePacked(itemTypeName)) ==
                keccak256(abi.encodePacked(itemTypes[i]))) {   // If item type name in the array return the index
                return i;
            }
        }
    }
...
// ADMIN FUNCTIONS
        ...
        returns (string memory output) 
    {
        ...
        ItemType memory item = items[itemID];                                     // Shortcut accessor
        string memory json1 = Base64.encode(bytes(string(abi.encodePacked(        // Encode the metadata
            '{',
            '"name": "',            item.name, '",',
        ...
        string memory json2 = Base64.encode(bytes(string(abi.encodePacked(        // Encode the metadata
            '"attributes": [',
                '{ "id": 0, "trait_type": "Rarity", "value": "' ,Strings.toString(item.rarity), '" },',
                '{ "id": 1, "trait_type": "Type", "value": "' ,Strings.toString(item.itemType), '" },',
                '{ "id": 2, "trait_type": "Value 1", "value": "',Strings.toString(item.value1), '" }',
                ']',
            '}'
        ))));
        output = string(abi.encodePacked('data:application/json;base64,', json1, json2));   // Return the result
    }
}
