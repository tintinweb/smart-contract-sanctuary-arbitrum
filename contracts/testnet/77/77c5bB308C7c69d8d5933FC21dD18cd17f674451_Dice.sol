...
// File @chainlink/contracts/src/v0.8/VRFConsume[email protected]
...
abstract contract Game is Ownable, Pausable, Multicall, VRFConsumerBaseV2, ReentrancyGuard {
    ...
    constructor(
        ...
        if (
            LINK_ETH_feedAddress == address(0) || chainlinkCoordinatorAddress == address(0) || bankAddress == address(0)
        ) {
            revert InvalidAddress();
        }
        require(numRandomWords != 0 && numRandomWords <= 500, "Wrong Chainlink NumRandomWords");

        bank = IBank(bankAddress);
        _chainlinkConfig.chainlinkCoordinator = IVRFCoordinatorV2(chainlinkCoordinatorAddress);
    ...
    function _newBet(
        ...
        if (bank.isAllowedToken(tokenAddress) == false || token.houseEdge == 0) {
        ...
        Bet memory newBet = Bet(false, user, tokenAddress, id, betAmount, block.number, 0, fee);
        ...
        if (!isGasToken) {
            IERC20(tokenAddress).safeTransferFrom(user, address(this), betAmount);
        }
    ...
    function _accountVRFCost(Bet storage bet, uint256 startGas) internal {
        ...
        (uint32 fulfillmentFlatFeeLinkPPMTier1, , , , , , , , ) = _chainlinkConfig.chainlinkCoordinator.getFeeConfig();
        // Calculates the VRF premium fee in ETH
        uint256 chainlinkPremium = ((1e12 * uint256(fulfillmentFlatFeeLinkPPMTier1) * uint256(weiPerUnitLink)) / 1e18);

        // Calculate the gas fee (adding the estimated gas spent after this calculation) + premium
        uint256 actualVRFCost = (tx.gasprice * (startGas - gasleft() + _chainlinkConfig.gasAfterCalculation)) +
            chainlinkPremium;
    ...
    function _resolveBet(Bet storage bet, uint256 payout) internal returns (uint256) {
        ...
        if (payout > betAmount) {
            ...
            bank.cashIn{value: isGasToken ? bankCashIn : 0}(token, bankCashIn, fee);
        } else {
    ...
    function setChainlinkConfig(
        ...
        emit SetChainlinkConfig(requestConfirmations, keyHash, gasAfterCalculation);
    }

    /// @notice Sets the Chainlink VRF V2 configuration.
    /// @param callbackGasLimit How much gas is needed in the Chainlink VRF callback.
    function setVRFCallbackGasLimit(address token, uint32 callbackGasLimit) external onlyOwner {
    ...
    function withdrawTokensVRFFees(address token) external {
        ...
        if (tokenChainlinkFees != 0) {
            ...
            Address.sendValue(payable(bank.getTokenOwner(token)), tokenChainlinkFees);
    ...
    function getChainlinkVRFCost(address token) public view returns (uint256) {
        ...
        return
            ...
            ((1e12 * uint256(fulfillmentFlatFeeLinkPPMTier1) * uint256(weiPerUnitLink)) / 1e18);
    }
}


// File contracts/games/Dice.sol
...
contract Dice is Game {
    ...
    function _getPayout(uint256 betAmount, uint8 cap) private pure returns (uint256) {
    ...
    function fulfillRandomWords(uint256 id, uint256[] memory randomWords) internal override {
        ...
        uint256 payout = _resolveBet(bet, rolled > diceBet.cap ? _getPayout(bet.amount, diceBet.cap) : 0);

        emit Roll(bet.id, bet.user, bet.token, bet.amount, diceBet.cap, rolled, payout);
    ...
    function getLastUserBets(address user, uint256 dataLength) external view returns (FullDiceBet[] memory) {
        ...
        for (uint256 i; i < lastBets.length; i++) {
            lastDiceBets[i] = FullDiceBet(lastBets[i], diceBets[lastBets[i].id]);
        }
        return lastDiceBets;
    }
}