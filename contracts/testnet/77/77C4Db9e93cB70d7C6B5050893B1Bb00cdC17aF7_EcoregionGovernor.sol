...
import { GovernorUpgradeable as Governor } from "@openzeppelin/contracts-upgradeable/governance/GovernorUpgradeable.sol";
import { GovernorSettingsUpgradeable as GovernorSettings } from "@openzeppelin/contracts-upgradeable/governance/extensions/GovernorSettingsUpgradeable.sol";
import { GovernorCountingSimpleUpgradeable as GovernorCountingSimple } from "@openzeppelin/contracts-upgradeable/governance/extensions/GovernorCountingSimpleUpgradeable.sol";
import { GovernorVotesUpgradeable as GovernorVotes } from "@openzeppelin/contracts-upgradeable/governance/extensions/GovernorVotesUpgradeable.sol";
import { GovernorVotesQuorumFractionUpgradeable as GovernorVotesQuorumFraction } from "@openzeppelin/contracts-upgradeable/governance/extensions/GovernorVotesQuorumFractionUpgradeable.sol";
import { GovernorPreventLateQuorumUpgradeable as GovernorPreventLateQuorum } from "@openzeppelin/contracts-upgradeable/governance/extensions/GovernorPreventLateQuorumUpgradeable.sol";
import { GovernorTimelockControlUpgradeable as GovernorTimelockControl } from "@openzeppelin/contracts-upgradeable/governance/extensions/GovernorTimelockControlUpgradeable.sol";
...
import { IGovernorUpgradeable as IGovernor } from "@openzeppelin/contracts-upgradeable/governance/IGovernorUpgradeable.sol";
import { IVotesUpgradeable as IVotes } from "@openzeppelin/contracts-upgradeable/governance/utils/IVotesUpgradeable.sol";

contract EcoregionGovernor is Initializable, GovernorSettings, GovernorCountingSimple, GovernorVotesQuorumFraction, GovernorPreventLateQuorum, GovernorTimelockControl
{
	constructor(address _token, address _timelock, uint64 _blockTime)
	{
		initialize(_token, _timelock, _blockTime);
	}

	function initialize(address _token, address _timelock, uint64 _blockTime) public initializer
	{
		__Governor_init("EcoregionGovernor");
		__GovernorSettings_init(15 minutes * _blockTime / 1e3, 2 hours * _blockTime / 1e3, 1000000e18); // voting delay, voting period, proposal threshold
		__GovernorCountingSimple_init();
		__GovernorVotes_init(IVotes(_token));
		__GovernorVotesQuorumFraction_init(0.05e18); // 5% quorum
		__GovernorPreventLateQuorum_init(10 minutes * _blockTime / 1e3);
		__GovernorTimelockControl_init(TimelockController(payable(_timelock)));
	}

	function quorumDenominator() public pure override returns (uint256 _quorumDenominator)
	{
		return 1e18; // 100%
	}

	function proposalThreshold() public view override(Governor, GovernorSettings) returns (uint256 _proposalThreshold)
	{
		return GovernorSettings.proposalThreshold();
	}

	function proposalDeadline(uint256 _proposalId) public view override(IGovernor, Governor, GovernorPreventLateQuorum) returns (uint256 _proposalDeadline)
	{
		return GovernorPreventLateQuorum.proposalDeadline(_proposalId);
	}

	function state(uint256 _proposalId) public view override(Governor, GovernorTimelockControl) returns (ProposalState _state)
	{
		return GovernorTimelockControl.state(_proposalId);
	}

	function _castVote(uint256 _proposalId, address _account, uint8 _support, string memory _reason, bytes memory _params) internal override(Governor, GovernorPreventLateQuorum) returns (uint256)
	{
		return GovernorPreventLateQuorum._castVote(_proposalId, _account, _support, _reason, _params);
	}

	function _cancel(address[] memory _targets, uint256[] memory _values, bytes[] memory _calldatas, bytes32 _descriptionHash) internal override(Governor, GovernorTimelockControl) returns (uint256 _proposalId)
	{
		return GovernorTimelockControl._cancel(_targets, _values, _calldatas, _descriptionHash);
	}

	function _execute(uint256 _proposalId, address[] memory _targets, uint256[] memory _values, bytes[] memory _calldatas, bytes32 _descriptionHash) internal override(Governor, GovernorTimelockControl)
	{
		GovernorTimelockControl._execute(_proposalId, _targets, _values, _calldatas, _descriptionHash);
	}

	function _executor() internal view override(Governor, GovernorTimelockControl) returns (address __executor)
	{
		return GovernorTimelockControl._executor();
	}

	function supportsInterface(bytes4 _interfaceId) public view override(Governor, GovernorTimelockControl) returns (bool _supportsInterface)
	{
