...
contract BaseUpgradeable is Initializable, AccessControlEnumerableUpgradeable, PausableUpgradeable, IERC173 {
...
import { Initializable, TreasureZoneAdmin } from "./TreasureZoneAdmin.sol";

contract TreasureZone is Initializable, EIP712Upgradeable, ZoneInterface, TreasureZoneAdmin {
    // Type for signing.
    struct AuthsigArgs {
        bytes32 orderHash;
        address fulfiller;
        address offerer;
        bytes32 zoneHash;
        bytes offererSignature;
        uint256 numerator;
        uint256 authsigDeadline;
    }

    bytes32 public constant AUTHSIG_ARGS_TYPE_HASH = keccak256(
        "AuthsigArgs(bytes32 orderHash,address fulfiller,address offerer,bytes32 zoneHash,bytes offererSignature,uint256 numerator,uint256 authsigDeadline)"
    );

    // Raised when the authority signature has expired.
    error AuthsigExpired(uint256 currentTime, uint256 authsigDeadline);
    // Raised when the authority signature is invalid.
    error InvalidAuthsig(address recoveredSigner, bytes32 argsHash, bytes authsig);

    function initialize(address _signingAuthority) external initializer {
        __TreasureZoneAdmin_init();
        __EIP712_init("TreasureZone", "1.0");
        signingAuthority = _signingAuthority;
    }

    function validateOrder(ZoneParameters calldata _zoneParameters)
        external
        view
        whenNotPaused
        returns (bytes4 validOrderMagicValue)
    {
        (bytes memory offererSignature, uint256 numerator, uint256 authsigDeadline, bytes memory authsig) =
            abi.decode(_zoneParameters.extraData, (bytes, uint256, uint256, bytes));

        // Ensure the signature deadline has not passed.
        if (block.timestamp > authsigDeadline) {
            revert AuthsigExpired(block.timestamp, authsigDeadline);
        }

        // Ensure the signature is valid.
        AuthsigArgs memory args = AuthsigArgs(
            _zoneParameters.orderHash,
            _zoneParameters.fulfiller,
            _zoneParameters.offerer,
            _zoneParameters.zoneHash,
            offererSignature,
            numerator,
            authsigDeadline
        );
        bytes32 argsHash = this.getAuthsigArgsHash(args);
        address signer = ECDSAUpgradeable.recover(argsHash, authsig);

        if (signer != signingAuthority) {
            revert InvalidAuthsig(signer, argsHash, authsig);
        }

        // All conditions satisfied. Return magic value, which is this function's selector.
    ...
    function supportsInterface(bytes4 _interfaceId)
        public
        view
        override(ZoneInterface, BaseUpgradeable)
        returns (bool)
    {
        return _interfaceId == type(ZoneInterface).interfaceId || super.supportsInterface(_interfaceId);
    }
    ...
    function getAuthsigArgsHash(AuthsigArgs calldata _args) public view returns (bytes32) {
        return _hashTypedDataV4(
            keccak256(
                abi.encode(
                    AUTHSIG_ARGS_TYPE_HASH,
                    _args.orderHash,
                    _args.fulfiller,
                    _args.offerer,
                    _args.zoneHash,
                    keccak256(_args.offererSignature),
                    _args.numerator,
                    _args.authsigDeadline
                )
            )
        );
    }
...
import { Initializable, TreasureZoneState } from "./TreasureZoneState.sol";

contract TreasureZoneAdmin is Initializable, TreasureZoneState {
    function __TreasureZoneAdmin_init() internal onlyInitializing {
        __TreasureZoneState_init();
    }
...
import { BaseUpgradeable, Initializable } from "./BaseUpgradeable.sol";

contract TreasureZoneState is Initializable, BaseUpgradeable {
    address public signingAuthority;

    function __TreasureZoneState_init() internal onlyInitializing {
        __Base_init();
    }
}