...
contract GameRegistry is IGameRegistry, AccessControl {
    ...
    function removeBullets(address player_, uint amount_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_playersStats[player_].bullets >= amount_, "No more bullets");

        _playersStats[player_].bullets -= amount_;
    }

    function removeEther(address player_, uint[] calldata ids_) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function addBullets(address player_, uint amount_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _playersStats[player_].bullets += amount_;
    }
