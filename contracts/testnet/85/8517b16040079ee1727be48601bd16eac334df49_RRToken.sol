...
interface IRR is IBEP20 {
    ...
    function getUnlockSpeed(
        address staker,
        address token
    ) external view returns (uint256);

    function claimableUnlocked(
        address token,
        address account
    ) external view returns (uint256);
...
contract RRToken is BEP20("RR", "RR"), Pausable, IRR, ReentrancyGuard {
    ...
    event LOG_CLAIM_UNLOCKED(address indexed staker, uint256 claimedAmount);

    event LOG_SET_UNLOCK_FACTOR(address indexed token, uint256 factor);
    ...
    function setPair(address router) public onlyOwner {
    ...
    function getUnlockFactor(
        address token
    ) external view override returns (uint256) {}

    function getUnlockBlockGap(
        address token
    ) external view override returns (uint256) {}
    ...
    function setUnlockFactor(
        address token,
        uint256 _factor
    ) external override {}

    function setUnlockBlockGap(
        address token,
        uint256 _blockGap
    ) external override {}

    function setAuthorizedUnlockedMintCaller(
        address caller
    ) external override {}

    function removeAuthorizedUnlockedMintCaller(
        address caller
    ) external override {}

    function setAuthorizedLockedMintCaller(address caller) external override {}

    function removeAuthorizedLockedMintCaller(
        address caller
    ) external override {}

    function mintUnlockedToken(address to, uint256 amount) external override {}

    function mintLockedToken(address to, uint256 amount) external override {}
}