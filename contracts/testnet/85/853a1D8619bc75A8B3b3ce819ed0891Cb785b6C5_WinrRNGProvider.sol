...
import "../../interfaces/randomizer/providers/winr-rng/IVRFCoordinator.sol";
import "../../interfaces/randomizer/providers/winr-rng/IWinrRNG.sol";
import "../../interfaces/randomizer/providers/IRandomizerProvider.sol";
import "../Access.sol";

contract WinrRNGProvider is Access, IRandomizerProvider {
  /*==================================================== State Variables ====================================================*/
  /// @notice The oracle identifier used for validating the VRF proof came from the oracle.
  bytes32 public ORACLE_ID = 0xb153b806e789d725c09c822a0267c6611d2bf853bc4372dcb43ece0a54fdc582;
  /// @notice The maximum callback gas limit.
  uint32 public constant MAXIMUM_CALLBACK_GAS_LIMIT = 10000000;

  IRandomizerRouter public router;
  IVRFCoordinator public randomizer;
  address public walletAddress;
  mapping(bytes32 => uint256) public routerRequestIds;

  constructor(IVRFCoordinator _randomizer) {
  ...
  function changeRandomizer(IVRFCoordinator _randomizer) public onlyGovernance {
    revokeRole(RANDOMIZER_ROLE, address(randomizer));
  ...
  function callback(bytes32 _requestId, uint256[] calldata _rngList) external onlyRandomizer {
  ...
  function request(uint256 _routerRequestId, uint32 _count, uint256 minConfirmations) external onlyRouter returns (uint256) {
    bytes32 requestId_ = randomizer.requestRandomWords(
        ORACLE_ID,
        _count,
        uint16(minConfirmations),
        MAXIMUM_CALLBACK_GAS_LIMIT,
        address(this),
        IWinrRNG.callback.selector
    );
    console.logBytes32(requestId_);
    routerRequestIds[requestId_] = _routerRequestId;

    return uint256(requestId_);
  }
}
...
interface IWinrRNG {
    function callback(bytes32, uint256[] calldata) external;
}
