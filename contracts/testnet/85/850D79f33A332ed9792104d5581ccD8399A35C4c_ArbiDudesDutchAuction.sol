...
import { ArbiDudesGenOne } from "./Arbidudes.sol";
...
contract ArbiDudesDutchAuction is Pausable, Ownable, ReentrancyGuard {
  ArbiDudesGenOne public dudesContract;
  ...
  constructor(ArbiDudesGenOne arbiDudes) {
    setDudesContract(arbiDudes);
    setMinDudesMintableMultiple(5);
    setMintableMultiplePrice(70000000000000000); //0.07 ETH
    _auctionStartedAt = block.timestamp;
  }

  function setDudesContract(ArbiDudesGenOne arbiDudes) public onlyOwner {
  ...
  function renounceOwnershipDudes() public virtual onlyOwner {
    dudesContract.renounceOwnership();
  }

  function transferOwnershipDudes(address newOwner) public virtual onlyOwner {
    dudesContract.transferOwnership(newOwner);
  }

  // Allow the owner to claim any amount of NFTs and direct them to another address.
  function ownerClaimMultipleDudes(uint256 amount, address to)
    public
    nonReentrant
    onlyOwner
  {
  ...
  function mintPrice() public view returns (uint256) {
    return mintPriceSince(secondsSinceAuctionStart());
  }

  function mintPriceSince(uint256 secondsAuction)
    public
    pure
    returns (uint256)
  {
    uint256 exponent = 5;
    if (secondsAuction < 300) {
    ...
    if (secondsAuction < 900) {
      // 50m - 45m // from 0,5 to 0,1
      // price = 1,3 - (4/300)secondsAuction
      return
        ((13 * 10**exponent) - ((secondsAuction * 40 * 10**exponent) / 3000)) *
        10**(18 - exponent - 1);
    }

    if (secondsAuction < 1200) {
    ...
    if (secondsAuction < 2100) {
      // 40m - 25m // from 0,05 to 0,02
      // price = 0,09 - (0,01/300)secondsAuction
      return
        ((9 * 10**(exponent - 2)) -
          ((secondsAuction * 10**(exponent - 2)) / 300)) * 10**(18 - exponent);
    }

    if (secondsAuction < 2400) {
