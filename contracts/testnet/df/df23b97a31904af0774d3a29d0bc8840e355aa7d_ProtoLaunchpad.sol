...
contract ProtoLaunchpad is Ownable, Initializable {
    ...
    uint256 public baseMinAllocation;
    uint256 public baseMaxAllocation;
    uint256 public totalFundRaisedByPool;
    ...
    function initialize(
        ...
        string memory _name,
        uint256 _poolMaxCap,
        uint256[2] memory baseAllocation, // index 0 baseMinAllocation, index 1 baseMaxAllocation
        ...
        if (_noOfTiers == 0) {
            baseMinAllocation = baseAllocation[0];
            baseMaxAllocation = baseAllocation[1];
        } else {
     ...
     */
    function updateBaseAllocation(uint256 _minAllocation, uint256 _maxAllocation) public onlyOwner {
        baseMinAllocation = _minAllocation;
        baseMaxAllocation = _maxAllocation;
    }
     ...
     */
    function updateUsersTier(address[] memory _users, uint256[] memory _tiers) external onlyOwner {
    ...
    function buyTokens(uint256 amount) external _hasAllowance(msg.sender, amount) returns (bool) {
        ...
        require(totalFundRaisedByPool.add(amount) <= poolMaxCap, "Exceeds pool max cap");
        ...
        if (noOfTiers != 0) {
            ...
            totalFundRaisedByPool = totalFundRaisedByPool.add(amount);
            ...
            require(expectedAmount >= baseMinAllocation, "Amount less than base min allocation");
            require(expectedAmount <= baseMaxAllocation, "Amount greater than base max allocation");
            totalFundRaisedByPool = totalFundRaisedByPool.add(amount);
            userInfo[msg.sender].totalAllocation = expectedAmount;
            ERC20Interface.safeTransferFrom(msg.sender, projectOwner, amount);
