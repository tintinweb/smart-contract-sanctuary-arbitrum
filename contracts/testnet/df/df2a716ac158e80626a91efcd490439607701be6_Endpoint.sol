...
contract Endpoint is IEndpoint, Initializable, EIP712Upgradeable, Ownable2StepUpgradeable {
    ...
    mapping(address user => uint64 nonce) private nonces;

    string constant WITHDRAW_COLLATERAL_SIGNATURE =
        "WithdrawCollateral(address _account,uint64 _nonce, address _token,uint256 _amount)";
    string constant MATCH_ORDER_SIGNATURE =
        "Order(address _account,uint64 _nonce, int256 _price, int256 _amount, bool _reduceOnly)";
        ...
        )
    {
        ...
        _markets = new IOffchainBook.Market[](_length);
        _fees = new IOffchainBook.FeeStore[](_length);
        _risks = new IOffchainBook.RiskStore[](_length);
        for (uint256 i = 0; i < _length; i++) {
            address _market = _marketIds[i];
            address _offchainBook = perpEngine.getOffchainBook(_market);
            _markets[i] = IOffchainBook(_offchainBook).getMarket();
            _fees[i] = IOffchainBook(_offchainBook).getFeeStore();
            _risks[i] = IOffchainBook(_offchainBook).getRiskStore();
        }
    }
    ...
    function depositCollateral(address _account, uint256 _amount) external {
        ...
        marginBank.handleDepositTransfer(_account, _amount);
    }
    ...
    function _processTransaction(bytes calldata _params) internal {
        ...
        if (_type == TransactionType.ExecuteSlowMode) {
            return;
        } else if (_type == TransactionType.WithdrawCollateral) {
        ...
        } else if (_type == TransactionType.SettlePnl) {
            SettlePnl memory _tx = abi.decode(_params[1:], (SettlePnl));
            perpEngine.settlePnl(_tx.markets, _tx.account);
        } else if (_type == TransactionType.ClaimExecutionFees) {
            address[] memory _markets = perpEngine.getMarketIds();
    ...
    function _withdrawCollateral(WithdrawCollateral memory _params, bytes memory _signature) internal {
        _validateNonce(_params.account, _params.nonce);
        ...
        _validateSignature(_params.account, _digest, _signature);
    ...
    function _matchOrder(SignedMatchOrders memory _params) internal {
        ...
        address _offchainBook = perpEngine.getOffchainBook(_params.market);
    ...
    function _validateOrder(SignedOrder memory _signedOrder) internal {
        ...
        _validateNonce(_order.account, _order.nonce);
        bytes32 _digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(bytes(MATCH_ORDER_SIGNATURE)),
        ...
        _validateSignature(_order.account, _digest, _signedOrder.signature);
    }

    function _validateSignature(address _account, bytes32 _digest, bytes memory _signature) internal pure {
        address _recovered = ECDSA.recover(_digest, _signature);
        if (_recovered == address(0) || _recovered != _account) revert InvalidSignature();
    }
    ...
    function _validateNonce(address _account, uint64 _nonce) internal {
        if (_nonce != nonces[_account]++) revert InvalidNonce();
    }
}
