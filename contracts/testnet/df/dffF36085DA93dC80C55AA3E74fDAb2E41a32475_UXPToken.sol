...
import {OFTV2} from "../external/layer-zero/token/oft/v2/OFTV2.sol";

/// @title UXPToken
/// @notice UXP public governance token
contract UXPToken is Ownable, OFTV2, ERC20Permit, ERC20Votes {

    error NotApproved(uint256 allowance, uint256 spendAmount);

    constructor(address custodian, uint256 maxTotalSupply, address lzEndpoint) OFTV2("UXD Governance Token", "UXP", 18, lzEndpoint) ERC20Permit("UXP") {
        if (maxTotalSupply != 0) {
    ...
    function xChainSend(uint16 dstChainId, bytes32 toAddress, uint256 amount, address payable refundAddress, address zeroPaymentAddress, bytes memory adapterParams) external {
