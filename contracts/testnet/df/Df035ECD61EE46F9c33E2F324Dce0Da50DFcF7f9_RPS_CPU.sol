...
contract RPS_CPU {
    ...
    function _checkResult(Hands player, Hands computer)
        internal
        pure
        returns (Results)
    {
        ...
        // win
        if (player == Hands.Rock && computer == Hands.Scissors) {
            return Results.Win;
        }
        if (player == Hands.Paper && computer == Hands.Rock) {
            return Results.Win;
        }
        if (player == Hands.Scissors && computer == Hands.Paper) {
            return Results.Win;
        }
    ...
    function addDeposit() public payable {
        ...
        scoreOfOwner[msg.sender].deposit =
            scoreOfOwner[msg.sender].deposit +
            msg.value;
    }

    // send reward if player win
    function _sendRewardToken(uint256 token) internal returns (uint256) {
        token = token * 2;
    ...
    // start game
    function doGame(Hands playerHand, uint256 bet) external {
        console.log("bet: '%d / wallet: '%d'", uint256(bet));
        require(bet > 0, "bet is under 0, must be set over 0");
        require(
            scoreOfOwner[msg.sender].deposit > 0,
            "your deposit is under 0, deposit must be over 0"
        );
        require(
            bet <= scoreOfOwner[msg.sender].deposit,
        ...
        console.log(
            "player hand: '%d / computer hand: '%d' / result: '%d'",
            uint256(playerHand),
            uint256(cpuHand),
            uint256(result)
        );

        emit ResultNotification(
            result,
            playerHand,
            cpuHand,
            scoreOfOwner[msg.sender]
        );
    }
}