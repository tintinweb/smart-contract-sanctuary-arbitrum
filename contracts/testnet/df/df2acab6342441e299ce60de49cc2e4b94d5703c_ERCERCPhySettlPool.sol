...
abstract contract AbstractLiquidityPool is ILiquidityPool, Ownable, PoolToken, ReentrancyGuard {
    ...
    function setOptionContract(address optionContract) external onlyOwner {
        ...
        grantRole(SNAPSHOT_ROLE, optionContract);
        // set unlimited allowance to the option contract (the owner)
        require(liquidityToken.approve(optionContract, _MAX_INT), "Cannot approve liquidity tokenfor owner");
        require(settlementToken.approve(optionContract, _MAX_INT), "Cannot approve settlement token for owner");
    }
    ...
    function withdrawLiquidity(uint256 quantity) override public nonReentrant {
        ...
        _withdrawLiquidity(msg.sender, quantity, balanceOf(msg.sender), _lockedBalanceOf(msg.sender));
    ...
    /// @notice Lock a {quantity} liquidity for entire pool
    ...
    //    return _getSharesOfAt(amount, provider, snapshotId);
       (foundIdx,_balanceAt) = balanceOfAt(provider, snapshotId);
        (, _totalSupplyAt) = totalSupplyAt(snapshotId);
    ...
    function _liquidateFor(
        ...
        (,balanceAt) = balanceOfAt(provider, snapshotId);
    ...
    function _isClaimedPremium(uint256 snapshotId, address provider) internal view returns(bool) {
        return _claimedPremiumProviders[snapshotId][provider];
    }
