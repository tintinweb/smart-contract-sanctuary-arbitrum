...
contract Oracle is ChainlinkRequestInterface, OracleInterface, Ownable, LinkTokenReceiver, WithdrawalInterface {
  ...
  function fulfillOracleRequest(
    bytes32 _requestId,
    uint256 _payment, //请求支付的金额？
    address _callbackAddress,
    bytes4 _callbackFunctionId,
    uint256 _expiration,
    bytes32 _data //这个应该是预言机喂价数据？
  )
    external
    onlyAuthorizedNode
    override
    ...
    returns (bool)
  {
    ...
    require(commitments[_requestId] == paramsHash, "Params do not match request ID"); //要求请求一定要存在；
    withdrawableTokens = withdrawableTokens.add(_payment);  //这个随便自己增加吗？//由于是自己部署的合约，所以无所谓？
    delete commitments[_requestId];
    require(gasleft() >= MINIMUM_CONSUMER_GAS_LIMIT, "Must provide consumer enough gas"); //对用户gas有要求？？？防止执行一半而失败？？
