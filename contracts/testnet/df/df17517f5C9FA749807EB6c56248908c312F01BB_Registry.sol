...
contract Registry is ERC2345{
    ...
    event Created_Unique_Digital(uint256 version, string unique_digital_id, string owner_id, string payload, uint256 time);
    ...
    event Transfer_of_Ownership(uint256 version, string unique_digital_id, string old_owner_id, string new_owner_id, string payload, uint256 time, uint256 units);
    ...
    function createUniqueDigital(address _receiver, uint256 _version, string memory _unique_digital_id, string memory _owner_id, string memory _name, string memory _description, string memory _location, string memory _payload, uint256 _time) public {
        ...
        emit Created_Unique_Digital(_version, _unique_digital_id, _owner_id, _payload, _time);
    }
    ...
    function transferToken(address _from, address _to, string memory _old, string memory _new, uint256 _version, string memory _unique_digital_id, uint256 _amount, bool _all, uint256 _units, string memory _payload, uint256 _time) public {
        ...
        if(_all){
            ...
            emit Transfer_of_Ownership(_version, _unique_digital_id, _old, _new, _payload, _time, balance);
        }
        else{
            safeTransferFrom(_from, _to, match_nfts[_unique_digital_id], _amount, _all, _units, "");
            emit Transfer_of_Ownership(_version, _unique_digital_id, _old, _new, _payload, _time, _units);
        }
...
contract ERC2345{
     ...
     */
    event TransferSingle(address indexed operator, address indexed from, address indexed to, uint256 id, uint256 value, uint256 units);
    ...
    mapping(uint256 => mapping(address => uint256)) private _balances_units;
    ...
    function balanceOfUnits(address account, uint256 id) public view virtual returns (uint256) {
        require(account != address(0), "ERC1155: balance query for the zero address");
        return _balances_units[id][account];
    }
    /**
        ...
        virtual
    {
        ...
        if(units > 0){
            require(amount == 1, "ERC1155: For divisible, amount must be 1.");
        }
        require(
        ...
        if(all){
            ...
            _balances[id][to] = 1;
            uint256 fromBalanceUnits = _balances_units[id][from];
            _balances_units[id][from] = 0;
            _balances_units[id][to] += fromBalanceUnits;
            units = fromBalanceUnits;
        }
        else if(units > 0){
            uint256 fromBalanceUnits = _balances_units[id][from];
            require(fromBalanceUnits >= units, "ERC1155: insufficient units for transfer");

            _balances_units[id][from] = fromBalanceUnits - units;
            _balances_units[id][to] += units;
            if(_balances[id][to] != 1){
                _balances[id][to] = 1;
            }
            if(_balances_units[id][from] == 0){
                _balances[id][from] = 0;
            }
        }
        else{
        ...
        emit TransferSingle(operator, from, to, id, amount, units);
     ...
     */
    function _mint(address account, uint256 id, uint256 amount, uint256 units,bytes memory data) internal virtual {
        ...
        _balances_units[id][account] += units;
        emit TransferSingle(operator, address(0), account, id, amount, units);
     ...
     */
    function _burn(address account, uint256 id, uint256 amount, uint256 units, bool all) internal virtual {
        require(account != address(0), "ERC1155: burn from the zero address");
        uint256 unitBalance = 0;
        if(units > 0){
            require(amount == 1, "ERC1155: For divisible, amount must be 1.");
            unitBalance = _balances_units[id][account];
            require(unitBalance >= units, "ERC1155: burn amount units exceeds balance");
        }
        address operator = msg.sender;
        ...
        uint256 fromBalanceUnits = _balances_units[id][account];
        if(all){
            units = fromBalanceUnits;
            _balances_units[id][account] = 0;
            _balances[id][account] = 0;
        }
        else if(units > 0){
            _balances_units[id][account] = unitBalance - units;
            if(units == 0){
        ...
        emit TransferSingle(operator, account, address(0), id, amount, units);
    }
    ...
    {
        if (isContract(to)) {
            try IERC2345Receiver(to).onERC1155Received(operator, from, id, amount, data) returns (bytes4 response) {
                if (response != IERC2345Receiver(to).onERC1155Received.selector) {
    ...
    {
        if (isContract(to)) {
            try IERC2345Receiver(to).onERC1155BatchReceived(operator, from, ids, amounts, data) returns (bytes4 response) {
                if (response != IERC2345Receiver(to).onERC1155BatchReceived.selector) {
                    revert("ERC1155: ERC1155Receiver rejected tokens");
                }
            } catch Error(string memory reason) {
                revert(reason);
