...
contract Dice is Ownable {
  ...
  function flip(uint256 _guess, uint256 _amount) public {
    ...
    uint256 dice2 = uint256(keccak256(abi.encodePacked(msg.sender, block.timestamp, block.number, block.difficulty))).mod(6).add(1);
    ...
    Bet memory data = Bet({result: (result == _guess), amount: _amount, guess : _guess, dice1: dice1, dice2: dice2, claimed: false});
