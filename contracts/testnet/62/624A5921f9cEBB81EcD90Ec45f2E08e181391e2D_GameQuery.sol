...
contract GameQuery {
    ...
    function allGames(IBattleRoyaleNFT nft, address player) public view returns (GameData[] memory games) {
    ...
    function allPlayers(IBattleRoyaleGameV1 game) public view returns (PlayerData[] memory players) {
        uint count = game.playerCount();
    ...
    function allTiles(IBattleRoyaleGameV1 game) public view returns(uint[] memory tiles) {
    ...
    struct GameSnapshot{
        ...
        uint mapSeed;
        uint[][] allTokenIds;
        uint[][] allTokenProperties;
    }

    function gameSnapshot(IBattleRoyaleGameV1 game) public view returns(GameSnapshot memory snapshot) {
        IBattleRoyaleGameV1.GameConfig memory config = game.gameConfig();
        IBattleRoyaleGameV1.GameState memory state = game.gameState();
        (uint[][] memory allTokenIds, uint[][] memory allTokenProperties) = game.allTokensOnGround(0, 0, config.mapSize, config.mapSize);
        snapshot = GameSnapshot({
            ...
            allTokenProperties: allTokenProperties
        });
    }
    ...
    function allTokens(IBattleRoyaleNFT nft, address owner) external view returns(uint[] memory data) {
        uint count = nft.balanceOf(owner);
        data = new uint[](count * 2);
        for (uint i = 0; i < count; i++) {
            uint id = nft.tokenOfOwnerByIndex(owner, i);
            uint property = nft.tokenProperty(id);
            data[i * 2] = id;
            data[i * 2 + 1] = property;
        }
    }
