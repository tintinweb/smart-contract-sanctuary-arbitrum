...
contract OpenPosition is Roles, ERC2771Context, IOpenPosition {
    ...
    bytes32 private constant CHANGE_COPY = keccak256("CHANGE_COPY");
    ...
    function executeCopy(
        ...
        _permitsCall(permits);
        Types.Position memory position = positionStorage.getPosition(id);
        require(
            position.timestamp + maxCopyExpire > block.timestamp,
            "!expire-copy"
        );
        uint length = copyRequest.length;
        Fee memory fee = Fee(
            pairs.getOpenFee(position.pairId),
            _getFeeExecution(350000, length)
        );
    ...
    function _permit(Types.Permit calldata permit) internal {
    ...
    function _permitsCall(Types.Permit[] calldata permits) internal {
        uint length = permits.length;
        for (uint256 i; i < length; ) {
            Types.Permit memory permit = permits[i];
            (bool success, bytes memory ret) = usdc.call(
                abi.encodeWithSelector(
                    IERC20Permit.permit.selector,
                    permit.owner,
                    permit.spender,
            ...
            if (!success) {
                emit PermitFailed(permit.owner, ret);
            }
            unchecked {
                ++i;
            }
    ...
    function openCopyPosition(
        ...
        uint256 amount = copyRequest.fixedAmount != 0
            ...
            : (position.amount * BPS * uint256(copyRequest.percentAmount)) /
                (BPS * BPS - uint256(position.leverage) * uint256(fee.openFee));
        uint256 feeOpen = (amount * position.leverage * fee.openFee) /
            (BPS ** 2);
        amount += fee.oracleFee;
        require(
        ...
        require(amount > feeOpen + fee.oracleFee, "!fee-not-enough");
        Types.Position memory cpPosition = Types.Position(
            ...
            position.entryPrice,
            amount - feeOpen - fee.oracleFee,
            position.fundingTracker
        );
        require(risks.checkRisk(cpPosition, 0), "!risk");
        ...
        pool.creditFee(id, 1, feeOpen, fee.oracleFee);
    ...
    function openPositionGasLess(
        ...
        ++nonces[gasLess.owner];
        return _openPosition(gasLess.owner, order, _getFeeExecution(350000, 1));
    }
    ...
    function openPositionGasLessWithPermit(
        ...
        ++nonces[gasLess.owner];
        _permit(permit);
        return _openPosition(gasLess.owner, order, _getFeeExecution(380000, 1));
    }
    ...
    function _openPosition(
        ...
        require(risks.checkRisk(position, 0), "!risk");
    ...
    function updateLimit(
        ...
        positionStorage.setTp(id, tp);
        positionStorage.setSl(id, sl);
    }
    ...
    function updateLimitGasLess(
        ...
        positionStorage.setTp(id, tp);
        positionStorage.setSl(id, sl);
        uint256 feeOracle = _getFeeExecution(150000, 1);
    ...
    function changeCopyAmount(
        ...
        address master,
        address copier
    ) external override hasRole(CHANGE_COPY) {
        copiedAmount[copier][master] += amount;
    }

    function resetCopyAmount(address master) external {
        copiedAmount[_msgSender()][master] = 0;
    }
