...
interface IStringComponent is IComponent {
    /**
     * Sets the typed value for this component
     *
     * @param entity Entity to get value for
     */
    function setValue(uint256 entity, string calldata value) external;
     ...
     * @param entity Entity to get value for
     */
    function getValue(uint256 entity) external view returns (string memory);
}

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.9;

import "./StringComponent.sol";

uint256 constant ID = uint256(keccak256("game.piratenation.namecomponent"));

/**
 * @dev maps entity id → name
 */
contract NameComponent is StringComponent {
    ...
    ) StringComponent(gameRegistryAddress, ID) {
        // Do nothing
    }
}
...
import "./IStringComponent.sol";

contract StringComponent is BaseComponent, IStringComponent {
    /** SETUP **/
     ...
     * @param entity Entity to get value for
     */
    function setValue(uint256 entity, string calldata value) external virtual {
    ...
    function getValue(
        ...
        if (has(entity)) {
            value = abi.decode(getBytes(entity), (string));
        }
    }
}
