...
contract XMozToken is Ownable, OFTV2 {
    ...
    modifier onlyMaster() {
        require(msg.sender == mozStaking || msg.sender == multiSigAdmin, "Invalid caller");
        _;
    }

    function setMultiSigAdmin(address _address) public onlyOwner {
        require(_address != address(0x0), "Invalid addr");
        multiSigAdmin = _address;
    }

    function setMozStakingAddress(address _address) public onlyOwner {
        require(_address != address(0x0), "Invalid addr");
        mozStaking = _address;
    }
    ...
    */
    ...
    function mint(uint256 _amount, address _to) external onlyMaster {
        _mint(_to, _amount);
    }

    function burn(uint256 _amount, address _from) external onlyMaster {
        _burn(_from, _amount);
    }
}
