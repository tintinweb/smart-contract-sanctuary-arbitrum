...
contract RandomizerRouter is Access, IRandomizerRouter {
  ...
  function request(uint32 _count, uint256 _minConfirmations) external onlyConsumer returns (uint256 requestId_) {
    ...
    provider_.request(requestIds, uint8(_count), _minConfirmations);
  ...
  function trigger(
    ...
    IRandomizerProvider(request_.provider).request(_requestId, uint8(request_.count), 0);
  ...
  {
    Request memory request_ = requests[_requestId];
    IRandomizerProvider(request_.provider).request(_requestId, uint8(request_.count), request_.minConfirmations);
  ...
  ) external onlyTeam onlyNotFilled(_requestId) 
  {
    ...
    provider_.request(_requestId, uint8(request_.count), request_.minConfirmations);
...
interface IRandomizerProvider {
  function request(uint256 _requestId, uint8 _count, uint256 minConfirmations) external returns (uint256);
}
