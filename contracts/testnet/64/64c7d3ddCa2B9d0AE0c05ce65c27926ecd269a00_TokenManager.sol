...
import "../interfaces/core/strategies/IMiningStrategy.sol";
import "../interfaces/core/strategies/IFeeStrategy.sol";
import "../interfaces/Stakings/IWINRStaking.sol";
import "../interfaces/referrals/IReferralStorage.sol";

contract TokenManager is Access {
  using SafeERC20 for IWINR;
  using SafeERC20 for IERC20;

  event Converted(address indexed account, uint256 amount);
  event StrategiesSet(IMiningStrategy indexed miningStrategy, IFeeStrategy indexed feeStrategy);
  event FeeCollectorSet(address indexed feeCollector);
  event VaultManagerSet(address indexed vaultManager);
  event TokensSet(IWINR indexed WINR, IWINR indexed vWINR);
  /*==================================================== State Variables =============================================================*/
  /// @notice WINR address
  IWINR public WINR;
  ...
  uint256 private sendableAmount;
  ...
  /// @notice access role for WINR staking
  bytes32 public constant WINR_STAKING_ROLE = keccak256("WINR_STAKING_ROLE");
  /// @notice access role for Fee collector
  ...
  /**
   *
   * @param  _WINR WINR address
   * @param _vWINR Vested WINR address
   * @notice function to set WINR and Vested WINR tokens
   */
  function setTokens(IWINR _WINR, IWINR _vWINR) external onlyGovernance {
    WINR = _WINR;
    vWINR = _vWINR;

    emit TokensSet(_WINR, _vWINR);
  }

  /**
   *
   * @param _feeCollector fee collector address
   * @notice fucntion to set fee collector
   */
  ...
  function setWINRStaking(IWINRStaking _WINRStaking) external onlyGovernance {
    ...
    _grantRole(WINR_STAKING_ROLE, address(WINRStaking));
  }
   ...
   */
  function takeVestedWINR(address _from, uint256 _amount) external onlyRole(WINR_STAKING_ROLE) {
   ...
   */
  function takeWINR(address _from, uint256 _amount) external onlyRole(WINR_STAKING_ROLE) {
   ...
   */
  function sendVestedWINR(address _to, uint256 _amount) external onlyRole(WINR_STAKING_ROLE) {
   ...
   */
  function sendWINR(address _to, uint256 _amount) external onlyRole(WINR_STAKING_ROLE) {
   ...
   */
  function mintWINR(address _to, uint256 _amount) external onlyRole(WINR_STAKING_ROLE) {
   ...
   */
  function burnVestedWINR(uint256 _amount) external onlyRole(WINR_STAKING_ROLE) {
    vWINR.burn(_amount);
  }
   ...
   */
  function burnWINR(uint256 _amount) external onlyRole(WINR_STAKING_ROLE) {
    WINR.burn(_amount);
  }
   ...
   */
  function mintIfNecessary(address _to, uint256 _amount) external onlyRole(WINR_STAKING_ROLE) {
   ...
   */
  function sendWLP(address _to, uint256 _amount) external onlyRole(WINR_STAKING_ROLE) {
    WLP.safeTransfer(_to, _amount);
  }
  ...
  function share(uint256 amount) external onlyRole(FEE_COLLECTOR_ROLE) {
  ...
  function mintVestedWINR(address _input, uint256 _amount, address _recipient) external {
    ...
    uint256 _mintAmount = miningStrategy.calculate(_feeAmount, mintedByGames, MAX_MINT);
    // uint256 _tier = referralStorage.referrerTiers(_recipient);
    // if (_tier > 0) {
    //   _mintAmount = (_mintAmount * _tier) / BASIS_POINTS;
    // }
  ...
  /**
   * @notice function to mint Vested WINR
   * @notice only fee collector can mint
  ...
  function mintFee() external {
    ...
    vWINR.mint(feeCollector, accumFee);
    accumFee = 0;
  }
