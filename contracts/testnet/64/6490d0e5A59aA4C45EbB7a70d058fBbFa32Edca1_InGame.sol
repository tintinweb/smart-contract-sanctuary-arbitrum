...
contract StakeManager is Ownable {
  ...
  modifier onlyPeekABoo() {
    require(
      ...
      "Only the PeekABoo contract can call this function"
    );
    _;
  }
  ...
  function isService(address service) public view returns (bool) {
    return (keccak256(abi.encodePacked(stakeServiceToServiceName[service])) !=
...
contract PeekABoo is IPeekABoo, ERC721Enumerable, Ownable, Pausable, Developers {
  ...
  function _transfer(
    ...
    tokenTraits[tokenId].owner = to;
    for (uint256 i = 0; i < addressToTokens[from].length; i++) {
      if (addressToTokens[from][i] == tokenId) {
        addressToTokens[from][i] = addressToTokens[from][addressToTokens[from].length - 1];
        addressToTokens[from].pop();
      }
    }
    addressToTokens[to].push(tokenId);
  }
}
