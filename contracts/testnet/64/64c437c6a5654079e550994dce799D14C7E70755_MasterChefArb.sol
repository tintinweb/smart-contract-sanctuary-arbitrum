...
contract MasterChefArb is Ownable {
    ...
    function setArbsPerSecond(uint256 _arbsPerSecond, bool _withUpdate)
        external
        onlyOwner
    {
        require(
            _arbsPerSecond <= maxArbsPerSecond,
            "setArbsPerSecond: too many arbs!"
        );
    ...
    function add(
        ...
        if (_lock) {
            require(
                _boostWeight >= MIN_BOOST_WEIGHT &&
                    _boostWeight <= MAX_BOOST_WEIGHT,
    ...
    function pendingARBS(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (pool.lock) {
            return
                (user.boostAmount * accArbsPerShare) /
                PRECISION -
                user.rewardDebt;
        } else {
            return
                (user.amount * accArbsPerShare) / PRECISION - user.rewardDebt;
        }
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 arbsReward = (multiplier * arbsPerSecond * pool.allocPoint) /
            totalAllocPoint;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 pending = (user.amount * pool.accArbsPerShare) /
            PRECISION -
            user.rewardDebt;
    ...
    function deposit(
        ...
        if (user.amount > 0) {
            require(
                ...
                "_amount and _duration can not be zero at the same time"
            );
        } else {
            require(
                _amount > 0 && _duration > 0,
                "_amount and _duration can not be zero"
            );
        }
        ...
        uint256 pending = (user.boostAmount * pool.accArbsPerShare) /
            PRECISION -
            user.rewardDebt;
        pool.boostAmount -= user.boostAmount;
        user.amount += _amount;
        user.boostAmount =
            (user.amount * totalLockDuration * pool.boostWeight) /
            pool.durationFactor /
            PRECISION;
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if (pool.lock) {
            pending =
                (user.boostAmount * pool.accArbsPerShare) /
                PRECISION -
                user.rewardDebt;
            pool.boostAmount -= user.boostAmount;
        } else {
            pending =
                (user.amount * pool.accArbsPerShare) /
                PRECISION -
                user.rewardDebt;
        }
