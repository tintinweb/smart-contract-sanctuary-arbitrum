    ...
    IBinaryVault
{
    ...
    function __burn(uint256 tokenId) internal virtual {
        ...
        if (withdrawalRequests[tokenId].timestamp > 0) {
        ...
        nonReentrant
    {
        ...
        if (withdrawalTokenAmount > 0) {
            pendingWithdrawalShareAmount -= withdrawalShareAmount;
            pendingWithdrawalTokenAmount -= withdrawalTokenAmount;
        }
    ...
    function claimBettingRewards(
        ...
        uint256 claimAmount = isRefund
            ? amount
            : (2 * amount * (config.FEE_BASE() - config.tradingFee()) /
                config.FEE_BASE());
        IERC20Upgradeable(underlyingTokenAddress).safeTransfer(
            user,
            claimAmount
    ...
    function onRoundExecuted(uint256 wonAmount, uint256 loseAmount)
        ...
        onlyMarket
    {
        uint256 tradingFeeBips = config.tradingFee();
        uint256 fee1 = (wonAmount * 2 * tradingFeeBips) / config.FEE_BASE();
        uint256 fee2 = loseAmount * tradingFeeBips / config.FEE_BASE();
        ...
        if (loseAmount - fee2 > wonAmount) {
            totalDepositedAmount += (loseAmount - wonAmount) - fee2;
        ...
        returns (string memory)
    {
        ...
        string memory floatExpression = ((_value * 10**2) /
                10**IERC20MetadataUpgradeable(underlyingTokenAddress).decimals()).getFloatExpression();
        return
            string(
                abi.encodePacked(
                    floatExpression,
                    " ",
                    IERC20MetadataUpgradeable(underlyingTokenAddress).symbol()
                )
            );
    }
