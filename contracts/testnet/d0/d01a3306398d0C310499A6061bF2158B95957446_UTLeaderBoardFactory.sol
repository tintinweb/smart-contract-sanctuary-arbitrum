...
*/
contract UTLeaderBoardFactory is GenericTraitFactory {
    ...
    function deploy(
        ...
        TheProxy trait_proxy = new TheProxy(GOLDEN_KEY(), LOOKUPAddr);
        GenericTraitInterface trait = GenericTraitInterface(address(trait_proxy));
        // init proxy contract
        trait.setup(traitRegistryAddress, traitId, traitDefaults);
        trait.init();
        // CommunityRegistry myCommunityRegistry = GTRegistry(traitRegistryAddress).myCommunityRegistry();
        // myCommunityRegistry.grantRole(
        //     TOKEN_CONTRACT_ACCESS_SALE,
        //     address(trait_proxy)
        // );
        return(address(0), address(trait));
    }
...
interface GenericTraitInterface {
    function setup( address _registry, uint16 _traitId, bytes[] memory _defaultPropValues ) external;
...
* @dev This factory is used to create new traits (perks) in a community
*/
abstract contract GenericTraitFactory is Ownable, BlackHolePrevention {
     ...
     */
    function addTrait(inputTraitStruct calldata _inputTrait) external returns (uint16 traitId) {
        ...
        // Get trait registry
        GTRegistry traitRegistry = GTRegistry(thisCommunityRegistry.getRegistryAddress(string(abi.encodePacked("TRAIT_REGISTRY_", _inputTrait.tokenNum.toString()))));
        ...
        GTRegistry.traitStruct[] memory traits = new GTRegistry.traitStruct[](1);
        traits[0] = GTRegistry.traitStruct(traitId, TRAIT_TYPE(), _inputTrait.start, _inputTrait.end, _inputTrait.enabled, storageCtrAddress, _inputTrait.ipfsHash, _inputTrait.name);
        traitRegistry.addTrait(traits);

        // Grant access to the Controller for this trait
        if (controllerCtrAddress != address(0)){
            traitRegistry.setTraitControllerAccess(controllerCtrAddress, traitId, true);
        }
        address modifierConsumerAddress = GalaxisRegistry.getRegistryAddress("MODIFIER_CONSUMER");
        traitRegistry.setTraitControllerAccess(modifierConsumerAddress, traitId, true);
    }
    ...
    function deploy(
        ...
        uint16 traitId,
        bytes[] memory traitDefaults
    ) internal virtual returns(address controllerCtrAddress, address storageCtrAddress);
