...
contract BinaryMarket is Pausable, IBinaryMarket, AccessControl {
    ...
    function genesisStartRound() external onlyOperator whenNotPaused {
        ...
        genesisStartBlockTimestamp = block.timestamp - block.timestamp % timeframes[2].interval;
        ...
        whenNotPaused
    {
        ...
        // FIXME check if somebody who will bet first round will not place UP and win
        // Answer Before genesisStarted and locked, no one can place bet
        // FIXME so next round will be UP always?
        // Answer Nobody can bet this locked round, They can bet on next round only. This next round is not locked yet (lockprice and close price is 0 now). So we don't need to worry.
    ...
    function _executeRound(uint8[] memory timeframeIds, uint256[] memory roundIds, uint256 price) internal {
        require(
            genesisStartOnce,
            "Can only run after genesisStartRound is triggered"
        );
    ...
    * @dev Execute round
    */
    function executeRound(uint8[] memory timeframeIds, uint256[] memory roundIds, uint256 price)
        external
        onlyOperator
        whenNotPaused
        {
    ...
    function executeCurrentRound(uint8[] memory timeframeIds, uint256 price)
        ...
        whenNotPaused
    {
        uint256[] memory roundIds = new uint256[](timeframeIds.length);

        for (uint8 i = 0; i < timeframeIds.length; i ++) {
    ...
    function _safeLockRound(
        ...
        if (rounds[timeframeId][epoch].lockBlockTime > 0) {
            require(
                block.timestamp >=
                    rounds[timeframeId][epoch].lockBlockTime - bufferTime,
        ...
        require(rounds[timeframeId][epoch].lockOracleTimestamp == 0, "Already locked.");
        /// @dev We allow to write price after lockblock time + 1m only.
        if (timestamp <= startBlockTime + timeframes[timeframeId].interval + 60) {
    ...
    function _safeEndRound(
        ...
        if (rounds[timeframeId][epoch].closeBlockTime > 0) {
            require(
                block.timestamp >=
                    rounds[timeframeId][epoch].closeBlockTime - bufferTime,
        ...
        uint256 startBlockTime = getBlockTimeForEpoch(timeframeId, epoch);
        /// @dev We allow to write price after close block time + 1m only.
        if (timestamp <= startBlockTime + 2 * timeframes[timeframeId].interval + 60 && rounds[timeframeId][epoch].lockOracleTimestamp > 0) {
    ...
    function openPosition(
        ...
        require(
            currentBettableAmount > 0 && amount <= currentBettableAmount,
        ...
        returns (bool)
    {
        ...
        // We use block time
        bool lockable = currentRound.lockPrice == 0 &&
            block.timestamp >= currentRound.lockBlockTime;
        ...
        returns (uint8[] memory result)
    {
        ...
        // FIXME why to write it with assembly? just create new array with correct size and foreach copy
        // Answer I thought this is more gas friendly. What do you think? If you want to change it, no problem.
