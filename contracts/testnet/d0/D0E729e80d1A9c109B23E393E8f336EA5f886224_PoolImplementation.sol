...
WAP: PI: wrong Aave pool
NPT: PI: nonexistent pTokenId
NP:  PI: no position
CNL: PI: cannot liquidate
USM: PI: unsupported market
IVT: PI: invalid vToken
WC:  PI: wrong comptroller
NSS: PI: no swapper support
RNA: PI: replace not allowed
CNS: PI: collector not set
IB0: PI: insufficient B0
RLE: PI: removedLiquidity > total liquidity
NPN: PI: negative penalty
IL:  PI: pool insufficient liquidity
UC:  PI: unauthorized call
IM:  PI: insufficient margin
OR:  PI: only router
NLP: PI: not LP
SEF: PI: send ETH fail
NT:  PI: not trader
RE:  PI: reentry
*/
...
contract PoolImplementation is PoolStorage, NameVersion {
    ...
    function collectProtocolFee() external {
        require(protocolFeeCollector != address(0), 'CNS');
    ...
    function removeLiquidity(address asset, uint256 amount, OracleSignature[] memory oracleSignatures) external _reentryLock_
    {
        ...
        require(data.liquidity + data.lpsPnl > removedLiquidity, 'RLE');
        ...
        require(s.removeLiquidityPenalty >= 0, 'NPN');
    ...
    function trade(
        ...
        require(isRouter[msg.sender], 'OR');
