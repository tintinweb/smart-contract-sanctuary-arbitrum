...
import "./IAutomationContract.sol";
import "../interfaces/IMarket.sol";
import "../interfaces/IMarketCommon.sol";
import "../interfaces/IOracleManager.sol";
import "../abstract/AccessControlledAndUpgradeable.sol";

contract AutomationContract is IAutomationContract, AccessControlledAndUpgradeableModifiers {
  event MarketAdded(address indexed market);
  event OrderCreated(uint256 indexed orderId, address market, OrderAction order);
  event OrderCancelled(uint256 indexed orderId);
  event OrderExecuted(uint256 indexed orderId);
  event OrderLost(uint256 indexed orderId);

  using SafeERC20 for IERC20;

  mapping(address => MarketOrderQueue) internal marketQueue;
  mapping(address => LostOrdersInfo) public lostOrders;

  // These indexes are unique accross the markets.
  uint256 public latestActionIndex;
  ...
  function registerAndApprovePaymentTokenForMarket(address market) external adminOnly {
    address paymentToken = IMarket(market).get_paymentToken();
    IERC20(paymentToken).approve(market, type(uint256).max);

    emit MarketAdded(market);
  }

  function getLostOrdersInfo(address market) external view returns (LostOrdersInfo memory) {
    return lostOrders[market];
  }

  function getActiveOrdersAmountForUser(address market, address user) external view returns (uint256) {
    return marketQueue[market].activeOrdersCounter[user];
  }
  ...
  function getActiveOrdersAmountInList(
    ...
    uint256 headIndex = marketQueue[market].headItemForOrder[actionType][poolType];
    uint256 amount = 0;

    while (headIndex > 0) {
      amount++;
      headIndex = marketQueue[market].orderActions[headIndex].nextHeadID;
    }
  ...
  function getActiveOrderInListByIndex(
    ...
    require(index < getActiveOrdersAmountInList(market, poolType, actionType), "Index out of bounds");
    ...
    return marketQueue[market].orderActions[headIndex];
  }
  ...
  function getHeadOrderForMarket(
    ...
    return marketQueue[market].headItemForOrder[actionType][poolType];
  }

  function getOrderActionForMarket(address market, uint256 index) external view returns (OrderAction memory) {
    return marketQueue[market].orderActions[index];
  }

  function getUserOrdersForMarket(address market, address user) external view returns (OrderAction[] memory) {
    uint256 ordersAmount = marketQueue[market].activeOrdersCounter[user];
    OrderAction[] memory actions = new OrderAction[](ordersAmount);
    uint256 currentIndex = 0;

    for (uint8 poolTypeInt = 0; poolTypeInt <= uint8(IMarketCommon.PoolType.LONG); poolTypeInt++) {
      IMarketCommon.PoolType poolType = IMarketCommon.PoolType(poolTypeInt);

      for (uint8 actionTypeInt = 0; actionTypeInt <= uint8(ActionType.AboveStopBuy); actionTypeInt++) {
        ActionType currentActionType = ActionType(actionTypeInt);
        uint256 queueIndex = marketQueue[address(market)].headItemForOrder[currentActionType][poolType];

        while (queueIndex > 0) {
          OrderAction memory action = marketQueue[address(market)].orderActions[queueIndex];
          if (action.user == user) {
            actions[currentIndex] = action;
            currentIndex++;
          }
          queueIndex = marketQueue[address(market)].orderActions[queueIndex].nextHeadID;
        }
      }
    }
  ...
  function cancelUserOrder(address market, uint256 index) external {
    OrderAction storage orderAction = marketQueue[address(market)].orderActions[index];

    require(!orderAction.isCancelled, "Order already cancelled");
    require(orderAction.amount > 0, "Invalid order being cancelled");
    require(orderAction.user == msg.sender, "Only order creator can cancel it");

    ActionType actionType = orderAction.actionType;
    IMarketCommon.PoolType poolType = orderAction.poolType;

    orderAction.isCancelled = true;

    uint256 queueIndex = marketQueue[market].headItemForOrder[actionType][poolType];
    while (queueIndex > 0) {
      bool isCancelled = marketQueue[market].orderActions[queueIndex].isCancelled;
      uint256 nextHeadID = marketQueue[market].orderActions[queueIndex].nextHeadID;

      if (isCancelled) {
        marketQueue[market].headItemForOrder[actionType][poolType] = nextHeadID;
        queueIndex = nextHeadID;
      } else {
        break;
      }
    }

    marketQueue[address(market)].activeOrdersCounter[orderAction.user]--;
    _handleFundsReturn(IMarket(market), orderAction);
  ...
  function _activateOrder(
    ...
    uint256 index,
    bool isNewOrder,
    OrderAction memory orderAction
  ) internal {
    if (isNewOrder) {
      _handleFundsTransfer(market, orderAction);
      emit OrderCreated(index, address(market), orderAction);
    }

    marketQueue[address(market)].activeOrdersCounter[orderAction.user]++;
  }

  function _markOrderAsLost(
    IMarket market,
    uint256 index,
    bool isNewOrder,
    OrderAction memory orderAction
  ) internal {
    // We need to transfer the funds even if it is a lost order so that it isn't possible for users to make 'unfunded' orders that get lost forever..
    // We create the order even if it is lost (but only if it is a new order). This is just for better UX in the Backend.
    if (isNewOrder) {
      _handleFundsTransfer(market, orderAction);
      emit OrderCreated(index, address(market), orderAction);
    }

    lostOrders[address(market)].orderIndices.push(index);
    emit OrderLost(index);
  }

  function _handleFundsReturn(IMarket market, OrderAction memory orderAction) internal {
    if (orderAction.actionType == ActionType.AboveStopBuy || orderAction.actionType == ActionType.BelowStopBuy) {
      address paymentToken = market.get_paymentToken();
      IERC20(paymentToken).safeTransfer(orderAction.user, orderAction.amount);
    }
  }

  function _handleFundsTransfer(IMarket market, OrderAction memory orderAction) internal {
    if (orderAction.actionType == ActionType.AboveStopBuy || orderAction.actionType == ActionType.BelowStopBuy) {
      address paymentToken = market.get_paymentToken();
      IERC20(paymentToken).safeTransferFrom(orderAction.user, address(this), orderAction.amount);
    } else {
      address poolToken = market.get_pool_token(orderAction.poolType, orderAction.poolTier);
      require(IERC20(poolToken).allowance(orderAction.user, address(this)) >= orderAction.amount, "Not sufficient allowance");
    }
  }
  ...
  function submitStopLossOrder(
    ...
    submitOrderAction(ActionType.StopLoss, price, amount, poolTier, poolType, market, previousNode);
  }
  ...
  function submitTakeProfitOrder(
    ...
    submitOrderAction(ActionType.TakeProfit, price, amount, poolTier, poolType, market, previousNode);
  }
  ...
  function submitAboveStopBuyOrder(
    ...
    submitOrderAction(ActionType.AboveStopBuy, price, amount, poolTier, poolType, market, previousNode);
  }
  ...
  function submitBelowStopBuyOrder(
    ...
    submitOrderAction(ActionType.BelowStopBuy, price, amount, poolTier, poolType, market, previousNode);
  }
  ...
  function submitStopLossAndTakeProfitOrder(
    ...
    submitOrderAction(ActionType.StopLoss, stopLossPrice, amount, poolTier, poolType, market, previousNodeStopLoss);
    submitOrderAction(ActionType.TakeProfit, takeProfitPrice, amount, poolTier, poolType, market, previousNodeTakeProfit);
  }
  ...
  function submitOrderAction(
    ...
    _submitOrderAction(actionType, price, amount, poolTier, poolType, msg.sender, market, 0, previousNode);
  }

  function _resubmitLostOrderAction(
    IMarket market,
    OrderAction memory order,
    uint256 orderIndex,
    uint256 previousNode
  ) internal {
    _submitOrderAction(order.actionType, order.price, order.amount, order.poolTier, order.poolType, order.user, market, orderIndex, previousNode);
  }

  function _submitOrderAction(
    ActionType actionType,
    uint256 price,
    uint112 amount,
    uint8 poolTier,
    ...
    address user,
    IMarket market,
    uint256 nextActionIndex,
    uint256 previousNode // If this is zero, this is the head
  ) internal {
    require(price > 0, "Price must be greater than 0");
    require(amount > 0, "Amount must be greater than 0"); // TODO: We probably want a higher minimum than this.

    bool isNewOrder = false;
    if (nextActionIndex == 0) {
      nextActionIndex = ++latestActionIndex;
      isNewOrder = true;
    }
    OrderAction storage orderAction = marketQueue[address(market)].orderActions[nextActionIndex];

    orderAction.actionType = actionType;
    orderAction.price = price;
    orderAction.amount = amount;
    orderAction.poolTier = poolTier;
    orderAction.poolType = poolType;
    orderAction.user = user;

    if (previousNode == 0) {
      uint256 currentOrderHeadId = marketQueue[address(market)].headItemForOrder[orderAction.actionType][orderAction.poolType];
      OrderAction storage currentHead = marketQueue[address(market)].orderActions[currentOrderHeadId];
      if (currentHead.amount > 0) {
        bool validOrderToSubmit = (AutomationCommon.isDecreasingPriceInvariant(actionType, poolType) && orderAction.price >= currentHead.price) ||
          (!AutomationCommon.isDecreasingPriceInvariant(actionType, poolType) && orderAction.price <= currentHead.price);

        if (validOrderToSubmit) {
          marketQueue[address(market)].headItemForOrder[orderAction.actionType][orderAction.poolType] = nextActionIndex;
          orderAction.nextHeadID = currentOrderHeadId;

          _activateOrder(market, nextActionIndex, isNewOrder, orderAction);
        } else {
          _markOrderAsLost(market, nextActionIndex, isNewOrder, orderAction);
        }
      } else {
        marketQueue[address(market)].headItemForOrder[orderAction.actionType][orderAction.poolType] = nextActionIndex;
      ...
      OrderAction storage previousNodeAction = marketQueue[address(market)].orderActions[previousNode];
      require(previousNodeAction.actionType == orderAction.actionType, "Incorrect list");

      OrderAction storage nextNode = marketQueue[address(market)].orderActions[previousNodeAction.nextHeadID];

      bool validOrderToSubmit = (AutomationCommon.isDecreasingPriceInvariant(actionType, poolType) &&
        previousNodeAction.price >= orderAction.price &&
        (nextNode.price == 0 || orderAction.price >= nextNode.price));

      validOrderToSubmit =
        validOrderToSubmit ||
        (!AutomationCommon.isDecreasingPriceInvariant(actionType, poolType) &&
          previousNodeAction.price <= orderAction.price &&
          (nextNode.price == 0 || orderAction.price <= nextNode.price));

      if (validOrderToSubmit) {
        orderAction.nextHeadID = previousNodeAction.nextHeadID;
        previousNodeAction.nextHeadID = nextActionIndex;
  ...
  // TODO: Think about making this code return multiple orders rather than just one?
  function shouldExecuteStopOrderCore(IMarket market) public view returns (bool canExec, uint256 orderIndex) {
    ...
    for (uint8 poolTypeInt = 0; poolTypeInt <= uint8(IMarketCommon.PoolType.LONG); poolTypeInt++) {
      ...
      for (uint8 actionTypeInt = 0; actionTypeInt <= uint8(ActionType.AboveStopBuy); actionTypeInt++) {
        ActionType currentActionType = ActionType(actionTypeInt);
        orderIndex = marketQueue[address(market)].headItemForOrder[currentActionType][poolType];
        OrderAction memory currentOrder = marketQueue[address(market)].orderActions[orderIndex];
        if (currentOrder.price != 0) {
          if (
            (AutomationCommon.isDecreasingPriceInvariant(currentActionType, currentOrder.poolType) && uint256(price) <= currentOrder.price) ||
            (!AutomationCommon.isDecreasingPriceInvariant(currentActionType, currentOrder.poolType) && uint256(price) >= currentOrder.price)
          ) {
            return (true, orderIndex);
          }
        }
      }
    }
  ...
  function _validPriceAfterIndex(
    IMarket market,
    uint256 currentIndex,
    uint256 checkPrice,
    bool decreasingInvariant
  ) internal view returns (bool valid, uint256 nextHeadID) {
    uint256 currentOrderPrice = marketQueue[address(market)].orderActions[currentIndex].price;
    nextHeadID = marketQueue[address(market)].orderActions[currentIndex].nextHeadID;
    uint256 nextOrderPrice = marketQueue[address(market)].orderActions[nextHeadID].price;

    if (decreasingInvariant && currentOrderPrice >= checkPrice && (nextOrderPrice == 0 || checkPrice >= nextOrderPrice)) {
      valid = true;
    }
    if (!decreasingInvariant && currentOrderPrice <= checkPrice && (nextOrderPrice == 0 || checkPrice <= nextOrderPrice)) {
      valid = true;
  ...
  function _shouldReinsertLostOrder(IMarket market)
    internal
    view
    returns (
      bool canExec,
  ...
  {
    LostOrdersInfo memory lostOrderInfo = lostOrders[address(market)];

    if (lostOrderInfo.currentIndex < lostOrderInfo.orderIndices.length) {
      uint256 lostOrderIndex = lostOrderInfo.orderIndices[lostOrderInfo.currentIndex];
      OrderAction memory lostOrder = marketQueue[address(market)].orderActions[lostOrderIndex];
      ActionType actionType = lostOrder.actionType;
      IMarketCommon.PoolType poolType = lostOrder.poolType;
      uint256 headIndex = marketQueue[address(market)].headItemForOrder[actionType][poolType];
      bool isDecreasingPriceInvariant = AutomationCommon.isDecreasingPriceInvariant(actionType, poolType);

      if (headIndex == 0) {
        return (true, lostOrderIndex, 0);
      }

      uint256 headOrderPrice = marketQueue[address(market)].orderActions[headIndex].price;
      if (isDecreasingPriceInvariant && lostOrder.price >= headOrderPrice) {
        return (true, lostOrderIndex, 0);
      }
      if (!isDecreasingPriceInvariant && lostOrder.price <= headOrderPrice) {
        return (true, lostOrderIndex, 0);
      ...
      uint256 currentIndex = headIndex;
      while (currentIndex >= 0) {
        (bool validPrice, uint256 nextHeadID) = _validPriceAfterIndex(market, currentIndex, lostOrder.price, isDecreasingPriceInvariant);
        if (validPrice) {
          return (true, lostOrderIndex, currentIndex);
        }
        currentIndex = nextHeadID;
      }
    }
  }

  function _deleteHeadOrderFromQueue(
    IMarket market,
    ActionType actionType,
    IMarketCommon.PoolType poolType,
    address user
  ) internal {
    uint256 headItem = marketQueue[address(market)].headItemForOrder[actionType][poolType];
    uint256 nextHeadID = marketQueue[address(market)].orderActions[headItem].nextHeadID;

    marketQueue[address(market)].headItemForOrder[actionType][poolType] = nextHeadID;
    marketQueue[address(market)].activeOrdersCounter[user]--;
  }

  function _validOrderToExecute(IMarket market, uint256 orderIndex) internal view returns (bool validOrder, OrderAction memory order) {
    order = marketQueue[address(market)].orderActions[orderIndex];

    require(order.price > 0, "Invalid order index provided");
    require(!order.isCancelled, "Order already cancelled");

    uint256 headIndex = marketQueue[address(market)].headItemForOrder[order.actionType][order.poolType];
    require(headIndex == orderIndex, "Only head order can be executed");
    ...
    if (
      (AutomationCommon.isDecreasingPriceInvariant(order.actionType, order.poolType) && uint256(price) <= order.price) ||
      (!AutomationCommon.isDecreasingPriceInvariant(order.actionType, order.poolType) && uint256(price) >= order.price)
    ) {
      return (true, order);
    }
  ...
  function mintFor(IMarket market, uint256 orderIndex) public {
    (bool validOrder, OrderAction memory order) = _validOrderToExecute(market, orderIndex);

    require(validOrder, "Order price conditions are not met");
    require(order.actionType == ActionType.BelowStopBuy || order.actionType == ActionType.AboveStopBuy, "Invalid order type for minting");

    _mintFor(market, orderIndex);
    emit OrderExecuted(orderIndex);
  }

  function _mintFor(IMarket market, uint256 orderIndex) internal {
    OrderAction storage orderAction = marketQueue[address(market)].orderActions[orderIndex];

    if (orderAction.poolType == IMarketCommon.PoolType.LONG) {
      IMarket(market).mintLongFor(orderAction.poolTier, orderAction.amount, orderAction.user);
    } else {
      IMarket(market).mintShortFor(orderAction.poolTier, orderAction.amount, orderAction.user);
    }

    orderAction.executed = true;

    _deleteHeadOrderFromQueue(market, orderAction.actionType, orderAction.poolType, orderAction.user);
  }

  function redeemFor(IMarket market, uint256 orderIndex) public {
    (bool validOrder, OrderAction memory order) = _validOrderToExecute(market, orderIndex);

    require(validOrder, "Order can not be executed");
    require(order.actionType == ActionType.StopLoss || order.actionType == ActionType.TakeProfit, "Invalid order type for redeeming");

    _redeemFor(market, orderIndex);
    emit OrderExecuted(orderIndex);
  }

  function _redeemFor(IMarket market, uint256 orderIndex) internal {
    OrderAction storage orderAction = marketQueue[address(market)].orderActions[orderIndex];
    bool successfulRedeem = false;

    address poolToken = market.get_pool_token(orderAction.poolType, orderAction.poolTier);

    try IERC20(poolToken).transferFrom(orderAction.user, address(this), orderAction.amount) {
      if (orderAction.poolType == IMarketCommon.PoolType.LONG) {
        IMarket(market).redeemLongFor(orderAction.poolTier, orderAction.amount, orderAction.user);
      } else {
        IMarket(market).redeemShortFor(orderAction.poolTier, orderAction.amount, orderAction.user);
      }
  ...
  function reinsertLostOrder(
    IMarket market,
    uint256 orderIndex,
    uint256 behindNode
  ) public {
    LostOrdersInfo memory lostOrderInfo = lostOrders[address(market)];
    require(lostOrderInfo.currentIndex < lostOrderInfo.orderIndices.length, "No lost orders to reinsert");

    uint256 lostOrderIndex = lostOrderInfo.orderIndices[lostOrderInfo.currentIndex];
    require(lostOrderIndex == orderIndex, "Order is not reinsertable");

    OrderAction memory order = marketQueue[address(market)].orderActions[orderIndex];
    _resubmitLostOrderAction(market, order, orderIndex, behindNode);
    lostOrders[address(market)].currentIndex++;
  }
  ...
  function executeUserAction(
    ...
    if (shouldMint) {
      mintFor(market, orderIndex);
    } else {
      redeemFor(market, orderIndex);
    }
  }

  function shouldExecuteStopOrder(IMarket market) external view returns (bool _canExec, bytes memory execPayload) {
    (bool canExecStopOrder, uint256 orderIndex) = shouldExecuteStopOrderCore(market);
    if (canExecStopOrder) {
      OrderAction memory order = marketQueue[address(market)].orderActions[orderIndex];
      ...
      return (
        ...
        abi.encodeCall(
          ...
          (market, orderIndex, order.actionType == ActionType.AboveStopBuy || order.actionType == ActionType.BelowStopBuy)
        )
      );
    }
  }

  function shouldReinsertLostOrder(IMarket market) external view returns (bool _canExec, bytes memory execPayload) {
    (bool canReinsertLostOrder, uint256 index, uint256 behindNode) = _shouldReinsertLostOrder(market);
    if (canReinsertLostOrder) {
      return (canReinsertLostOrder, abi.encodeCall(this.reinsertLostOrder, (market, index, behindNode)));
    }
  }
}
