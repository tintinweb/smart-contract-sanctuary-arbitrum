...
contract Vault is ReentrancyGuard, AccessControlBase, IVault {
    ...
    address public gemLpManagerAddress;
    ...
    /*==================== Operational functions Custom GEMB/JB *====================*/
    ...
    function _payout(
        ...
        if (_escrowAmount <= _totalForVault) {
            unchecked {
                ...
                // decrease the net amount the GEMLP has to pay from the pool amounts
    ...
    /**
     * @notice function called by the vault manager to add assets to the GEMLP (profit)
    ...
    function payin(
        ...
        // add the tokens to the GEMLP, this will incrase the value of the gemLp
        ...
        emit PayinGEMLP(_inputToken, _escrowAmount);
    }

    /*==================== Operational functions GEMB/JB *====================*/

    /**
     * @notice function that adds a whitelisted asset to the pool, without issuance of GEMLP or USDW!
     * @param _tokenIn address of the token to directly deposit into the pool
     * @dev take note that depositing LP by this means will NOT mint GEMLP to the caller. This function would only make sense to use if called by the GEMB DAO. If you call this function you will receive nothing in return, it is effectively gifting liquidity to the pool without getting anything back.
     */
    ...
    /**
     * @notice function that adds a whitelisted asset to the pool, and adds the amount to the GEMLP funds
     * @dev the tokens need to be transferred to the vault before calling this function
     * @param _tokenIn address of the token to directly deposit into the pool
     */
        ...
        )
    {
        ...
        // calculate the maximum amount of referral kickback that can be paid out (a fraction of wagerFeeReserves, the size of fraction is configured in GEMLPManager)
        uint256 maxAmountReferral_ = (totalWagerFees_ *
            IGEMLPManager(gemLpManagerAddress).maxPercentageOfWagerFee()) /
        ...
        if (registeredReferralKickback_ > maxAmountReferral_) {
            ...
            // note: the vault/GEMLPs 'pay for' the referral kickback! to reflect this we need to decrease pool amout and also update the totalOut
     ...
     * @dev when ManagerMode is enabled, this function can only be called by a GEMLPManager contract
     * @param _tokenIn the token used to purchase/mint the GEMLP
     * @param _receiverUSDW the address the caller, this is generally the GEMLPManager contract, this address will receive the USDW (not the GEMLP)
     * @return mintAmountUsdw_ the amount of usdw that is minted to the GEMLPManager contract
     */
    function deposit(
        address _tokenIn,
        ...
        protocolNotPaused
        nonReentrant
        returns (uint256 mintAmountUsdw_)
    {
        _validateManager();
        _validate(_whitelistedTokens[_tokenIn], 9);
        uint256 tokenAmount_ = _transferIn(_tokenIn);
        _validate(tokenAmount_ != 0, 10);
        // fetch the price of the incoming token, the vault always prices an incoming asset by its lower bound price (so in the benefit of the GEMLPs)
        ...
        // mint usdw to the _receiverUSDW contract (generally gemLpManager if ManagerMode is enabled)
    ...
    /**
     * @notice redeem gemLp for asset of choice (burn gemLp, withdraw asset) -> sellUSDW/sellUSDW
     * @param _tokenOut the address of the token the seller wants to redeem his USDW for
     * @param _receiverTokenOut the address that will receive the _tokenOut (so the asset the withdrawer is redeeming their GEMLP for)
     * @return amountOut_ the amount of _tokenOut that the receiver has redeemed
     * @dev when ManagerMode is enabled, this function can only be called by the gemLpManager contract!
     */
    function withdraw(
        address _tokenOut,
    ...
    /**
     * @notice function allowing a purchaser to buy a GEMLP asset with another GEMLP asset
    ...
    function swap(
        ...
        nonReentrant
        protocolNotPaused
        returns (uint256 amountOutAfterFees_)
    {
    ...
    function _collectWagerFees(
        ...
        amountAfterWagerFee_ = ((_amountEscrow * (1e18 - wagerFeeBasisPoints)) /
            1e18);
        unchecked {
     ...
     * @param _toPayOnNet amount of _addressTokenOut the GEMLP will transfer to the
    ...
    function _transferIn(
        address _tokenIn
    ) private returns (uint256 amountTokenIn_) {
    ...
    function _updateTokenBalance(address _tokenToUpdate) private {
        tokenBalances[_tokenToUpdate] = IERC20(_tokenToUpdate).balanceOf(
            address(this)
        );
    }

    /**
     * @notice accounting function that increases the registered/realized GEMLP assets
     * @dev this is a very important function to understand! this function increases the value of GEMLP
     * @dev note that this is different from the tokenBalances! poolAmounts belong to the GEMLPs
    ...
    function _increasePoolAmount(
        ...
        // if the registered pool amounts are larger than the actual balance of the token, something went wrong in the accounting because this is technically a impossability - by definition the poolAmounts registered to GEMLPs will always be lower as the balance, even if the GEMLPs are in a net loss (historically). So this check is in place to essentially check if the vault isn't broken/exploited.
        _validate(
    ...
    /**
     * @notice accounting function that decreases the registered/realized GEMLP assets
     * @dev this is a very important function to understand! this function decreases the value of GEMLP
     * @dev note that this is different from the tokenBalances! poolAmounts belong to the GEMLPs
    ...
    function _decreasePoolAmount(
        ...
        if (circuitBreakerEnabled) {
            ...
            if (poolAmounts[_tokenOut] < circuitBreakerAmounts[_tokenOut]) {
                // if so, call the GEMLP manager (whom will enforce a CB policy onto the vault)
                IGEMLPManager(gemLpManagerAddress).circuitBreakerTrigger(
                    _tokenOut
                );
            }
        }
    ...
    /**
     * @notice returns the lowerbound/minimum price of the gemLp asset
     * @dev the return value is scaled 1e30 (so $1 = 1e30)
     * @param _token address of the token/asset
     ...
     * @dev this function uses the lower bound price, so the price/value for outgoing assets this is at the benefit of the GEMLPs
     * @param _tokenToPrice address of the token to price/value
    ...
    function usdToTokenMax(
        address _tokenToPrice,
        uint256 _usdAmount
    ) external view override returns (uint256 tokenAmountMax_) {
        // using the lower price bound of the asset
     ...
     * @dev this function uses the upper bound price, so the price/value is for incoming assets at the benefit of the GEMLPs
    ...
    function usdToTokenMin(
        address _tokenToPrice,
        uint256 _usdAmount
    ) external view override returns (uint256 tokenAmountMin_) {
    ...
    function returnTotalOutAndIn(
        ...
        view
        override
        returns (uint256 totalOutAllTime_, uint256 totalInAllTime_)
    {
    ...
    /**
     * @notice returns the usd value of all the assets in the gemLp combined (only the realized ones)
    ...
    function getReserve()
        ...
        view
        override
        returns (uint256 totalReserveValue_)
    {
        totalReserveValue_ = (IGEMLPManager(gemLpManagerAddress).getAum(false) -
            IGEMLPManager(gemLpManagerAddress).reserveDeduction());
    }

    /**
     * @notice returns the USD(scaled 1e30) value of 1 GEMLP token
     */
    function getGemLpValue()
        external
        view
        override
        returns (uint256 gemLpValue_)
    {
        gemLpValue_ = IGEMLPManager(gemLpManagerAddress).getPriceGemLp(false);
    }
     ...
     * @dev this is a timelocked feature since it moves GEMLP owned vaul assets to a different address
     ...
     */
    function setCircuitBreakerAmount(
        address _token,
        uint256 _amount
    ...
    /**
     * @notice enables managed mode - when enabled only addresses configured as mananager can mint usdw (so gemLpManager for example)
     */
     ...
     * @dev take note that the GEMLPManager mints the GEMLP, the vault mints USDW
     ...
     * @dev note that this function can drascitally change the GEMLP value
     * @param _token address of the token
...
interface IVaultManager {
    ...
    /// @notice transfers GEMLP tokens from this contract to Fee Collector and triggers Fee Collector
    /// @param _fee the amount of GEMLP sends to Fee Controller
    function transferGEMLPFee(uint256 _fee) external;
