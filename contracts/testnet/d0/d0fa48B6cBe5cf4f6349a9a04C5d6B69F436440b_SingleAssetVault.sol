...
contract SingleAssetVault is ApyFlowVault {
    ...
    constructor(
        PortfolioScoreOracle _oracle,
        IERC20Metadata _asset,
        string memory name,
        string memory symbol
    ) ApyFlowVault(_asset) ERC20(name, symbol) {
        ...
        returns (int256)
    {
        ...
        uint256 assetsInVault = balanceAtVault > 0
            ? vault.convertToAssets(balanceAtVault)
            : 0;
	 ...
	 */
    function rebalance(
        IERC4626Minimal sourceVault,
        IERC4626Minimal destinationVault,
        uint256 shares
    ) external {
        ...
        uint256 assets = sourceVault.redeem(
            shares,
            address(this),
            address(this)
        );
        ...
        returns (uint256 shares)
    {
        ...
        for (uint256 i = 0; i < vaults.length(); i++) {
            IERC4626Minimal vault = IERC4626Minimal(vaults.at(i));
            uint256 amount = (assets * oracle.portfolioScore(address(vault))) /
                _totalScore;
            if (amount == 0) continue;
            deposited += vault.convertToAssets(
        ...
        returns (uint256 assets)
    {
        for (uint256 i = 0; i < vaults.length(); i++) {
            IERC4626Minimal vault = IERC4626Minimal(vaults.at(i));
            uint256 amountToRedeem = (shares * vault.balanceOf(address(this))) /
                totalSupply();
