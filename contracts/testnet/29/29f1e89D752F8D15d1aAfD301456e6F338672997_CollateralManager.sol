...
contract CollateralManager is ICollateralManager, Owned, Pausable, MixinResolver {
    ...
    // The maximum amount of debt in sUSD that can be issued by non collateral.
    ...
    function getBorrowRate() public view returns (uint256 borrowRate, bool anyRateIsInvalid) {
        // get the synth backed debt.
        uint256 synthDebt = _issuer().totalIssuedSynths(sUSD);

        // now get the non synth backed debt.
        (uint256 nonSynthDebt, bool ratesInvalid) = totalLong();

        // the total.
        uint256 totalDebt = synthDebt.add(nonSynthDebt);

        // now work out the utilisation ratio, and divide through to get a per second value.
        uint256 utilisation = nonSynthDebt.divideDecimal(totalDebt).divideDecimal(SECONDS_IN_A_YEAR);
