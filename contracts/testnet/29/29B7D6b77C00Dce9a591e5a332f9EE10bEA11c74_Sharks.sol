...
contract Sharks is SharksAccessControl, ERC721Enumerable {
    ...
    modifier onlySharkOwner(uint256 tokenId_) {
        require(ownerOf(tokenId_) == msg.sender, "You do not own this shark.");
        _;
    }

    event Minted(address to, uint256 tokenId);
    event Revealed(uint256 tokenId, uint256 rarity);
    event XpIncreased(uint256 tokenId, uint256 xp);
    ...
    function mint(address to_, uint256 mintsCount_) external onlyMinter {
        ...
        for (uint i = 0; i < _actualMintsCount; i++) {
    ...
    function _beforeTokenTransfer(address from_, address to_, uint256 tokenId_) internal override {
        if (address(sharksTransferControl) != address(0)) {
            require(sharksTransferControl.sharkCanBeTransferred(tokenId_) == true, "transfer not allowed");
        }

        super._beforeTokenTransfer(from_, to_, tokenId_);
    }

    function size(uint256 tokenId_) public view returns (uint256) {
        require(address(sharksSizeControl)  != address(0), "SharksSizeControl not set");

        return sharksSizeControl.sharkSize(tokenId_);
    }

    function increaseXp(uint tokenId_, uint xp_) public onlyXpManager {
        _increaseXp(tokenId_, xp_);
    }

    function _increaseXp(uint tokenId_, uint xp_) internal {
        totalSharksSize += xp_;
        xp[tokenId_] += xp_;
        emit XpIncreased(tokenId_, xp_);
    }
    ...
    function setSharksSizeControl(address sharksSizeControl_) external onlyOwner {
        sharksSizeControl = SharksSizeControl(sharksSizeControl_);
        emit SharksSizeControlChanged(sharksSizeControl_);
    }

    function setSharksTransferControl(address sharksTransferControl_) external onlyOwner {
...
contract SharksStaking is Ownable {
    ...
    event Stake(uint256 tokenId);
    event Restake(uint256 tokenId);
    event Unstake(uint256 tokenId);
    event SetXpPerDay(uint256 xpPerDay);
    event SetMaxStakeDaysCount(uint256 maxStakeDaysCount);
