...
contract Quest
{
    ...
    function GetQuestion(address user) public view returns(Question[] memory data)
    {
        ...
        for(uint256 indexQuestion = 0; indexQuestion < TotalQuestionOnDay; indexQuestion++)
        {
            ...
            data[indexQuestion].Answer3,) = QuestionDataContract.ListQuestionsContract(user, questionNumber);
        }
    }
    ...
    function SubmidQuestions(address user, uint256[] calldata results) public
    {
        ...
        {
            uint256 questionNumber = ListQuestionsUser[user][indexQuestion];
            (,,,,,uint256 resultAnswerQuestionInContract) = QuestionDataContract.ListQuestionsContract(user, questionNumber);
...
{
    function ListQuestionsContract(address user, uint256 indexQuest) external pure returns(
