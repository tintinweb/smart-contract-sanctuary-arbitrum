...
contract Vault is Constants, ReentrancyGuard, Ownable, IVault {
    ...
    function distributeFee(bytes32 _key, address _account, uint256 _fee) external override {
        _isPositionHandler(msg.sender, true);
        _distributeFee(_key, _account, _fee);
    }

    function _distributeFee(bytes32 _key, address _account, uint256 _fee) internal {
        address _refer = address(0); //Temporarily set referral as address(0)
        _collectFee(_fee, _refer);

        if (_fee > 0) {
            emit DistributeFee(_key, _account, _refer, _fee);
        }
    }

    function _collectFee(uint256 _fee, address _refer) internal {
    ...
    function stake(address _account, address _token, uint256 _amount) external nonReentrant {
        ...
        _collectFee(usdAmountFee, ZERO_ADDRESS);
...
interface IReferralSystem {
    function getDiscountable(address _account) external view returns(address[] memory, uint256[] memory);
}
