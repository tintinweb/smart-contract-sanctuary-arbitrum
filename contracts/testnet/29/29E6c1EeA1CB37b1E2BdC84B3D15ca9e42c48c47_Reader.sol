...
contract Reader {
    IOptionRouterRead public routerContract;

    constructor(IOptionRouterRead _routerContract) {
    ...
    function getQueuedOptions(
        uint256 lastSyncedQueueId,
        uint256 page,
        uint256 limit
    ) external view returns (IOptionRouterRead.QueuedTrade[] memory queueIds) {
        uint256 maxId = min(
            routerContract.nextQueueId(),
            lastSyncedQueueId + ((page + 1) * limit)
        );

        queueIds = new IOptionRouterRead.QueuedTrade[](
            min(limit, maxId - (lastSyncedQueueId + (page * limit)))
        );

        uint256 counter;
        for (
            uint256 queueId = lastSyncedQueueId + (page * limit);
            queueId < maxId;
            queueId++
        ) {
            IOptionRouterRead.QueuedTrade memory queuedTrade = routerContract
                .queuedTrades(queueId);

            if (queuedTrade.isQueued == true) {
                queueIds[counter] = queuedTrade;
                counter += 1;
            }
        }
    }

    function test(uint256 id)
        external
        view
        returns (IOptionRouterRead.QueuedTrade memory)
    {
        return routerContract.queuedTrades(id);
    }
}
