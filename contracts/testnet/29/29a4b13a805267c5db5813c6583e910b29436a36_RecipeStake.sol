/**
 *Submitted for verification at Arbiscan on 2022-07-30
*/
...
contract RecipeStake is  IngredientsERC11155, ERC721Holder{
    uint nonce = 1;
    ...
    mapping(string => mapping(uint256 => uint256))  tokenIdToRewardsClaimed;
    ...
    function unStack(uint256 _stakeId, string memory _genType) public nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint _numberToClaim =  numberOfRewardsToClaim(tokenIds[i], staker.time, _genType);
    ...
    function printUserClaims() public  view returns (uint256[] memory, uint[] memory) {
        ...
        for(uint256 i =0; i < stakers.length; i++ ){
            ...
            for(uint256 j =0;j<stakers[i].tokenIds.length;j++){
                claims[i] = claims[i] +  numberOfRewardsToClaim(stakers[i].tokenIds[j], stakeTime, "gen0");
            }
        }
    ...
    function anyClaimInProgress() public  view returns (bool) {
        ...
        RecipeStaker[] memory stakers = recipeStakers[msg.sender];
        for(uint256 i =0; i < stakers.length; i++ ){
            uint256 tokenId =  stakers[i].tokenIds[0];
            uint256 stakeTime =  stakers[i].time;
    ...
    function claimReward(uint256 _stakeId, string memory _genType) public {
        ...
        for(uint i=0; i< tokenIds.length; i++){
            ...
            _claimReward(_numberToClaim, tokenIds[i],_genType, staker.claimNum, i);
            tokenIdToRewardsClaimed[_genType][tokenIds[i]] += _numberToClaim;
        }
    }
  
    function _claimReward(uint _numClaim, uint _tokenId,string memory _genType, uint number, uint _claimedRewardId) private {
