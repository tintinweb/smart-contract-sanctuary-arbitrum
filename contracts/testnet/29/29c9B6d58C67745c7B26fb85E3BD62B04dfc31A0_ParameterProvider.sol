...
contract ParameterProvider is IParameterProvider, Controllable {

    // Value Map
    mapping(ParamKey => uint) public valueMap;
    mapping(ParamKey => uint) private _snapshot; // This is temporary for validation

    event SetValue(ParamKey key, uint value);

    function setValue(ParamKey key, uint value) public override onlyController {
        valueMap[key] = value;
        emit SetValue(key, value);
    }

    function setValues(ParamKey[] memory keys, uint[] memory values) public override onlyController {
    ...
    function getValue(ParamKey key) external override view returns (uint) {
        return valueMap[key];
    }

    function getValues(ParamKey[] memory keys) external override view returns (uint[] memory output) {

        uint len = keys.length;
        output = new uint[](len);

        for (uint n=0; n<len; n++) {
            output[n] = valueMap[keys[n]];
        }
    }

    // To validate, a snapshot has be be created first.
    function validateChanges(ParamKey[] calldata keys, uint[] calldata values) external override returns (bool success) {
        
        // Create a temp snapshot with all values
        _createTempSnapshot();

        // Make changes to temp snapshot
        uint len = keys.length;
        for (uint n=0; n<len; n++) {
            _snapshot[keys[n]] = values[n];
        }

        // Validate the changed data
        if (!_is100Pcnt(ParamKey.RevShareReferralPcnt, ParamKey.RevShareTreasuryPcnt, ParamKey.RevShareDealsTeamPcnt)) {
            return false;
        }

        if (!_isEachLessOrEqual100Pcnt(ParamKey.ProposalQuorumPcnt, ParamKey.ReferralUplineSplitPcnt)) {
            return false;
        }

        if (!_isEachLessOrEqual100Pcnt(ParamKey.StakerDiscountMinPnct, ParamKey.StakerDiscountCapPnct)) {
            return false;
        }

        if (!_isEachLessOrEqual100Pcnt(ParamKey.SaleUserMaxFeePcnt, ParamKey.SaleUserCurrentFeePcnt, ParamKey.SaleMaxPurchasePcntByFund)) {
            return false;
        }

        if (_isLess(ParamKey.SaleUserMaxFeePcnt, ParamKey.SaleUserCurrentFeePcnt)) { 
            return false;
        }
        return true;
    }
    // Helper functions to validate against the tempory snapshot

    function _createTempSnapshot() private  {
        uint len = uint(ParamKey.lastIndex);
        for (uint n=0; n<len; n++) {
            _snapshot[ParamKey(n)] = valueMap[ParamKey(n)];
        }
    }

    // Is value of k1 less than k2
    function _isLess(ParamKey k1, ParamKey k2) private view returns (bool) {
        return _snapshot[k1] < _snapshot[k2];
    }

    function _is100Pcnt(ParamKey k1, ParamKey k2, ParamKey k3) private view returns (bool) {
        return (_snapshot[k1] + _snapshot[k2] + _snapshot[k3]) == Constant.PCNT_100;
    }

    function _isEachLessOrEqual100Pcnt(ParamKey k1, ParamKey k2) private view returns (bool) {
        return (_snapshot[k1] <= Constant.PCNT_100 && _snapshot[k2] <= Constant.PCNT_100);
    }

    function _isEachLessOrEqual100Pcnt(ParamKey k1, ParamKey k2, ParamKey k3) private view returns (bool) {
        return (_snapshot[k1] <= Constant.PCNT_100 && _snapshot[k2] <= Constant.PCNT_100 && _snapshot[k3] <= Constant.PCNT_100);
    }
}
