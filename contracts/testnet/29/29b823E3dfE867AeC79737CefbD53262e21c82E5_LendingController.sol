...
contract LendingController is ILendingController, SafeOwnable {
    ...
    uint256 public override uniMinOutputPct; // 99e18 = 99%
    ...
    constructor(
        ...
        observationCardinalityNext = 1800; //Arbitrum
    }
    ...
    /// @notice new Oracle logic, combining ling and uni oracles
    /// @notice all Oracle logic is executed by the unifiedOracleAggregator
    ...
    /// @notice enables or disables a token to be used as base asset. New pairs can only be created against those.
    /// @notice the base asset must be supported by the oracle
    /// @param _token ERC20
    /// @param _isBaseAsset true / false
    function setBaseAsset(address _token, bool _isBaseAsset) public {
        //Todo: Add Logic such that link oracle is set within the aggregator
        require(
    ...
    /// @notice Will call the OracleAggregators prepare Pool function with specified cardinality
    ...
    /// @notice Sets the default col factor with will be used for subsequent pairs
    /// @notice sets the col factor to a default value (e.g. 0.5) -> can later be changed by governance
    /// Need to have a var defaultColFactor and set colfactor[token] = defaultColFactor or implement a logic: if colFactor[token] === 0 return defaultColFactor
    ...
    /// @notice Set min obserations for the uni oracle
    /// @notice Added for permissionless pair creation
    function setObservationCardinalityNext(uint16 _observationCardinalityNext)
        public
        onlyOwner
    {
