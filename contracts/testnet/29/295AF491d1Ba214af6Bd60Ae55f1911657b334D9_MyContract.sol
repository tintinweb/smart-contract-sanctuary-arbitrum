...
// Selector for the destination chain (in the CCIP protocol)
  ...
  /// @notice sends data to receiver on dest chain. Assumes address(this) has sufficient native gas token.
    ...
    bytes32 messageId = router.ccipSend{value: router.getFee(destChainSelector, message)}(destChainSelector, message);
  ...
  function sendDataAndTokens(
    ...
    for (uint256 i = 0; i < tokenAmounts.length; i++) {
      IERC20Upgradeable(tokenAmounts[i].token).transferFrom(msg.sender, address(this), tokenAmounts[i].amount);
      IERC20Upgradeable(tokenAmounts[i].token).approve(address(router), tokenAmounts[i].amount);
    }
    Client.EVM2AnyMessage memory message = Client.EVM2AnyMessage({
      receiver: receiver,
      data: data,
