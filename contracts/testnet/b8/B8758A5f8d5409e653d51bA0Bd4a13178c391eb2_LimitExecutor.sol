    ...
    LimitOrdersStorage
{
        ...
        allowedOrder(_tokenId, _poolAddress)
    {
        ...
        // revert(toString(toTransferToken1));
        ...
        returns (uint256 token0Amount, uint256 token1Amount)
    {
        ...
        (uint256 feeAmount0, uint256 feeAmount1) = INonfungiblePositionManager(nftPositionManager).collect(
        ...
        if(feeAmount0 > 0)
            feeAmount0*SWAP_FEE_PERCENTAGE/SWAP_FEE_PERCENTAGE_ACCURACY;

        if (feeAmount1 > 0)
            feeAmount1*SWAP_FEE_PERCENTAGE/SWAP_FEE_PERCENTAGE_ACCURACY;
        ...
        uint256 balanceOfToken0After = IERC20Upgradeable(
            _order.poolData.token0
        ).balanceOf(address(this)) - feeAmount0;
        uint256 balanceOfToken1After = IERC20Upgradeable(
            _order.poolData.token1
        ).balanceOf(address(this)) - feeAmount1;
