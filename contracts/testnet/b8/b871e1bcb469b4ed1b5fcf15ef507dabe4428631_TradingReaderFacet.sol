...
library LibLimitOrder {
    ...
    function openLimitOrder(IBook.OpenDataInput calldata data) internal {
        ...
        ILimitOrder.LimitOrder memory order = ILimitOrder.LimitOrder(
            ...
            data.tokenIn, data.isLong, data.broker, data.stopLoss, data.qty, data.takeProfit, uint40(Chain.currentTimestamp())
        );
        bytes32 orderHash = keccak256(abi.encode(order, los.salt, "order", Chain.currentBlockNumber(), Chain.currentTimestamp()));
        los.salt++;
...
library LibPriceFacade {
    ...
    struct PriceFacadeStorage {
        ...
        // keccak256(token, Chain.currentBlockNumber()) =>
    ...
    function requestPrice(bytes32 id, address token, bool isOpen) internal {
        PriceFacadeStorage storage pfs = priceFacadeStorage();
        bytes32 requestId = keccak256(abi.encode(token, Chain.currentBlockNumber()));
        ...
        if (pendingPrice.blockNumber != Chain.currentBlockNumber()) {
            pendingPrice.token = token;
            pendingPrice.blockNumber = Chain.currentBlockNumber();
    ...
    function requestPriceCallback(bytes32 requestId, uint64 price) internal {
        ...
        cachePrice.timestamp = uint40(Chain.currentTimestamp());
        ...
        if (Chain.currentTimestamp() > updatedAt + pfs.maxDelay) {
            emit PriceRejected(msg.sender, requestId, pendingPrice.token, price, beforePrice, updatedAt);
            return;
        }
