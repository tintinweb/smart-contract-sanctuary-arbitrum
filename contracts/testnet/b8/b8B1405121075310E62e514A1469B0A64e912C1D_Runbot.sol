 ...
 */
contract Runbot is A_Base, A_Math, ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
	
	////
	////
	////
	//////////////// Structs & Events & Public variables ////////////////
	...
	mapping(uint256 => bool) private _isBuying;
	...
	constructor() ERC721("Runbot", "RUNBOT") {
		_baseURL = "https://runbot.io/nft-metadata/";
	...
	function askTransfer(address receiver, uint256 tokenId) external payable returns (uint256){
		...
		require(msg.value >= _transferFees, "Runbot: Not enough transfer fees");
	...
	function getFirstTransferAsk() external view returns (TransferAsk memory){
		require(_transferAsksStart < _transferAsksEnd, "Runbot: No transfer ask");
		
		return _transferAsks[_transferAsksStart];
	}
	...
	function askMint(uint256 nftType, uint256 nftSubType, uint256 borrowDuration, uint256 bidPrice) external payable returns (uint256){
		uint256 fees = getBaseFees(nftType, nftSubType, borrowDuration, true);
	
		require(msg.value >= fees, "Runbot: Not enough mint fees");
		
		_mintAsks[_mintAsksEnd] = MintAsk(_msgSender(), nftType, nftSubType, borrowDuration, bidPrice);
		_mintAsksEnd += 1;
	...
	function getFirstMintAsk() external view returns (MintAsk memory){
		require(_mintAsksStart < _mintAsksEnd, "Runbot: No mint ask");
		
		return _mintAsks[_mintAsksStart];
	}
	
	function completeAskMint(uint256 mintId, uint256 lockTimeout, bool withFirstBidRebate, bool isApproved) external onlyOwner returns (uint256){
		...
		if(isApproved){
			...
			result = safeMint(mintAsk.asker, mintAsk.nftType, mintAsk.nftSubType, bidPrice, mintAsk.borrowDuration, lockTimeout, withFirstBidRebate);
		}
	...
	function batchSafeMint(uint256 nb, address to, uint256 nftType, uint256 nftSubType, uint256 bidPrice, uint256 borrowDuration, uint256 lockTimeout, bool withFirstBidRebate) external onlyOwner returns (uint256, uint256) {
		uint256 firstTokenId = _lastTokenId;
		
		for (uint256 i = 0; i < nb; i++){
			safeMint(to, nftType, nftSubType, bidPrice, borrowDuration, lockTimeout, withFirstBidRebate);
		}
		return (firstTokenId, _lastTokenId-1);
	}
	
	function batchSafeMintLinear(uint256 nb, address to, uint256 nftType, uint256 nftSubTypeStart, uint256 nftSubTypeAdd, uint256 bidPriceStart, uint256 bidPriceAdd, uint256 borrowDurationStart, uint256 borrowDurationAdd, uint256 lockTimeoutStart, uint256 lockTimeoutAdd, bool withFirstBidRebate) external onlyOwner returns (uint256, uint256) {
		for (uint256 i = 0; i < nb; i++){
		...
		return (_lastTokenId-nb, _lastTokenId-1);
	}
	
	function safeMint(address to, uint256 nftType, uint256 nftSubType, uint256 bidPrice, uint256 borrowDuration, uint256 lockTimeout, bool withFirstBidRebate) public onlyOwner returns (uint256) {
	...
	function getNbMintedTokens(uint256 nftType) external view returns (uint256){
		return _nbMinted[nftType];
	}
	
	function getNbMintedTokens(uint256 nftType, uint256 nftSubType) external view returns (uint256){
	...
	function getBaseFees(uint256 nftType, uint256 nftSubType, uint256 borrowDuration, bool isMint) public view returns (uint256){
	...
	function getFees(uint256 tokenId) public view returns (uint256){
		...
		uint256 nftSubType = _nftSubTypes[tokenId];
		...
		if (_isFirstBuy[tokenId] && _withFirstBuyRebate[tokenId]){
			rebateMintFees = getBaseFees(nftType, nftSubType, _borrowDuration[tokenId], true);
		}
		
		uint256 baseFees = getBaseFees(nftType, nftSubType, _getCurrentBorrowDuration(tokenId), false);
	...
	function getInfos(uint256 tokenId) public view returns(uint256, uint256, uint256, uint256, bool, bool, uint256){
		return (_nftTypes[tokenId], _nftSubTypes[tokenId], _borrowDuration[tokenId], _getCurrentBorrowDuration(tokenId), _isFirstBuy[tokenId], _withFirstBuyRebate[tokenId], _lockTimeout[tokenId]);
	}
	
	function buy(uint256 tokenId) external payable {
		buyTo(_msgSender(), tokenId);
	}
	
	function buyTo(address to, uint256 tokenId) public payable {
		...
		uint256 fees = getFees(tokenId);
		
		require(msg.value >= bidPrice + fees, "Runbot: Not enough funds");
		
		
		if (_isFirstBuy[tokenId]){
			_borrowTimeout[tokenId] = block.timestamp + _borrowDuration[tokenId];
			_isFirstBuy[tokenId] = false;
		}
		
		address seller = ownerOf(tokenId);
		uint256 feesVal = msg.value - bidPrice;
		payable(owner()).transfer(feesVal);
		...
		emit Buy(to, seller, tokenId, bidPrice, feesVal);
	}
	...
	{
		require(_isBuying[tokenId], "Runbot: Direct transfer are not enabled");
		
		super._beforeTokenTransfer(from, to, tokenId, batchSize);
	}
	
	function _afterTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal override(ERC721)
	{
		_isBuying[tokenId] = false;
	}
