...
contract Mines is MinesBase {
   ...
   */
  function getState(address _player) external view returns (Game memory minesState_, uint256 currentPayout_) {
    minesState_ = games[_player];
    currentPayout_ = (minesState_.currentMultiplier * minesState_.wager) / 10000;
  ...
  function endGameByTeam(address _player) external onlyTeam nonReentrant {
    ...
    require(game_.startTime + 1 days < block.timestamp, "Mines: Game not endable yet");
    _endGame(_player);
  }
  /**
   * @dev Ends the player's current game and receives the payout.
  ...
  function randomizerFulfill(uint256 _requestId, uint256[] calldata _randoms) internal override {
    ...
    uint256 payout_ = (multiplier_ * game_.wager) / 10000;
...
abstract contract MinesBase is Core {
  ...
  function _refundGame(address _player) internal {
    ...
    require(_game.isFulfilled == false, "Mines: Game is fulfilled");
