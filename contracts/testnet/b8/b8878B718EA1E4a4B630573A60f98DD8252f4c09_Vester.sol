...
interface IesPEX is IERC20Upgradeable{
    function convertPEX(address _address,uint256 _amount) external;
    function convertEsPEX(address _address,uint256 _amount) external;
}
...
contract Vester is Initializable {
    ...
    mapping(address => VestInfo[MaxVestCount]) public AddressVestInfos;

    event Vest(address _sender, uint8 vestType, uint amount, uint blockTimestamp, uint GovPexCount);
    event Claim(address _sender, uint[MaxVestCount] _amount);
    ...
    function initialize(
        ...
        minVestAmount = 10 ** 18;
        GovPexCount = 0;
        freedSecond = 60 * 60 * 24;
        VestType = [60, 70, 80, 100];
    }

    function updateFreedSecond(uint _freedSecond) public onlyGov {
        freedSecond = _freedSecond;
    }

    function vest(uint8 vestType, uint amount) external {
        require(vestType < VestType.length, "Invalid vestType");
        require(amount > minVestAmount, "amount too small");
        require(esPEX.balanceOf(msg.sender) >= amount, "Insufficient amount");

        for (uint i = 0; i < MaxVestCount; i++) {
            if (AddressVestInfos[msg.sender][i].BeginTimestamp == 0) {
                AddressVestInfos[msg.sender][i] = VestInfo(vestType, amount, block.timestamp, 0);
                esPEX.convertPEX(msg.sender, amount);
                GovPexCount += amount - amount.mulDiv(VestType[vestType], 100);
                emit Vest(msg.sender, vestType, amount, block.timestamp, GovPexCount);
                return;
            }
        }
    ...
    function getVesting(address _vester) view public returns (uint[MaxVestCount] memory){
        uint[MaxVestCount] memory sum;
        ...
        for (uint i = 0; i < MaxVestCount; i++) {
            ...
            uint actualDay = (blockTimestamp - vestInfos.BeginTimestamp) / freedSecond;
            uint expectDay = (vestInfos.VestType + 1) * 30;
            if (actualDay > expectDay) {
                expectDay = actualDay;
            }

            uint vestType = VestType[vestInfos.VestType];
            uint256 amount = (vestInfos.Amount).mulDiv(vestType, 100).mulDiv(actualDay, expectDay);
    ...
    function claim() external {
        uint[MaxVestCount] memory vests = getVesting(msg.sender);
        uint8 emptyCount = 0;
        uint8 delCount = 0;
        uint sum = 0;
        for (uint i = 0; i < MaxVestCount; i++) {
            ...
            uint vestType = VestType[vestInfos.VestType];
            if (vestInfos.AlreadyClaim == vestInfos.Amount.mulDiv(vestType, 100)) {
                delCount++;
        ...
        if (emptyCount + delCount == MaxVestCount) {
            delete AddressVestInfos[msg.sender];
        }
