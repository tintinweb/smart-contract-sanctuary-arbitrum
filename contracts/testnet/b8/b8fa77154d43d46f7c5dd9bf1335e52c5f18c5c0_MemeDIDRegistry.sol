...
// File @openzeppelin/contracts/utils/cryptography/[email protected]0
...
// File contracts/IMemeDIDRegistry.sol



pragma solidity ^0.8.0;

interface IMemeDIDRegistry {
...
// File contracts/MemeDIDRegistry.sol
...
contract MemeDIDRegistry is Ownable, ReentrancyGuard, IMemeDIDRegistry, ONFT721 {
    using Address for address;
    ...
    constructor(string memory _name, string memory _symbol, string memory _baseTokenURI, bytes32 _root, 
            address _treasury, uint256 _minGasToTransfer, address _lzEndpoint) 
            ONFT721(_name, _symbol, _minGasToTransfer, _lzEndpoint) {
        baseTokenURI = _baseTokenURI;
        root = _root;
        treasury = _treasury;

        _safeMint(msg.sender, uint256(_root));
        records[_root].expiry = type(uint64).max;
    } 
