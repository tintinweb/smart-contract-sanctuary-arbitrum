...
contract AssetManager {
    ...
    // function approveForTransfer(address _approved, uint256 _tokenId) public {
    //     nftCollection.approve(_approved, _tokenId);
    // }
    ...
    function deployVault() public returns (address addr) {
        ...
        // return bytecode;
        // bytes memory bytecodeWithArguments = abi.encodePacked(
        //     bytecode,
        //     abi.encode()
        // );
        ...
        // Vault(vault).depositNFT(msg.sender, nftCollectionAddress, _tokenId);
        ...
        // emit DepositedNFT(msg.sender, vault, _tokenId);

        return vault;
    }

    function depositNFT(address _vault, uint256 _tokenId) public {
        // bytes memory bytecode = type(Vault).creationCode;
        // bytes memory bytecode = type(TestContract1).creationCode;
        // return bytecode;
        // bytes memory bytecodeWithArguments = abi.encodePacked(
        ...
        emit DepositedNFT(msg.sender, _vault, _tokenId);
...
contract Vault is IERC721Receiver {
    ...
    function depositNFT(
        ...
        address _NFTAddress,
        uint256 _TokenID
    ) public inProjectState(ProjectState.newEscrow) onlySeller {
        nftAddress = _NFTAddress;
        tokenID = _TokenID;
        ERC721(nftAddress).safeTransferFrom(_from, address(this), tokenID);
