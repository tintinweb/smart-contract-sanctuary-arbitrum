...
contract Router is BaseRouter, IRouter, ReentrancyGuard {
    ...
    function updateTrailingStop(
        ...
        require(executors[msg.sender] || msg.sender == _account, "IVLPO"); //Invalid positionOwner
    ...
    */
    ...
    function setPriceAndExecute(bytes32 _key, uint256 _txType, uint256[] memory _prices) external {
        require(executors[msg.sender] || msg.sender == address(positionHandler), "FBD"); //Forbidden
    ...
    function revertExecution(
        ...
        require(path.length > 0 && params.length > 0, "IVLAL"); //Invalid array length
    ...
    function _modifyPosition(
        ...
        if (_isTakeAssetRequired) {
            require(_params.length > 0, "IVLPL"); //Invalid params length
            _transferAssetToVault(
                _account,
                _path[1],
                _params[0],
        ...
        if (!_isFastExecute || _txType == ADD_POSITION) {
            ...
            bytes memory data;
            uint256 amountIn = _params.length == 0 ? 0 : _params[0];
            bool isSwapSuccess = true;
            ...
            if (executeTxType == ADD_COLLATERAL || executeTxType == REMOVE_COLLATERAL) {
                ...
                data = abi.encode(_params[0], position);
    ...
    function _setPriceAndExecute(
        ...
        if (_getTxTypeForExecuting(_txType) != TRIGGER_POSITION) {
        ...
        (uint256 deadline, ) = _extractDeadlineAndAmountOutMin(txn.txType, _getParams(_key, txn.txType), false);
        ...
        _modifyPosition(
            ...
            bonds[_key].isLong,
            bonds[_key].posId,
            txn.txType == ADD_POSITION ? CONFIRM_POSITION : txn.txType,
            false,
            _getParams(_key, txn.txType),
            _prices,
            _path,
...
contract BasePosition is BasePositionConstants, Ownable {
    ...
    function setExecutor(address _executor, bool _isActive) external onlyOwner {
        require(_executor != address(0), "IVLE/Z"); //Invalid executor, zero address
        executors[_executor] = _isActive;
        emit SetExecutor(_executor, _isActive);
    }
    //End config functions
