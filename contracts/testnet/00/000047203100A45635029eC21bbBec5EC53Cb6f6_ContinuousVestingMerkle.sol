  ...
  MerkleSet
{
  ...
  constructor(
    ...
    uint256 _votingFactor, // votes have this weight
    ...
    ContinuousVesting(_token, _total, _uri, _votingFactor, _start, _cliff, _end)
    MerkleSet(_merkleRoot)
  {}

  function NAME() external override pure returns (string memory) {
    return 'ContinuousVestingMerkleDistributor';
  }
...
abstract contract AdvancedDistributor is Ownable, Distributor, IAdjustable, IVotesLite {
    ...
    // Adjustable contracts must be sweepable
    function adjustTotal(address beneficiary, int256 amount) external onlyOwner {
        ...
        emit AdjustTotal(beneficiary, amount);
    }
...
interface IAdjustable {
  event AdjustTotal(address indexed beneficiary, int256 amount);
  ...
  function adjustTotal(address beneficiary, int256 amount) external;
  // Set the token being distributed
...
abstract contract Sweepable {
    using SafeERC20 for IERC20;

    event SetupSweepable(address recipient);
    event SweepToken(address indexed token, uint256 amount);
    event SweepNative(uint256 amount);

    address payable private immutable recipient;

    constructor(address payable _recipient) {
        recipient = _recipient;
        emit SetupSweepable(_recipient);
    }
