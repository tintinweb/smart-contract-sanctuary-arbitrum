...
contract OmnichainRouter is IOmnichainRouter, NonblockingLzApp, IAxelarExecutable {
    ...
    event AxReceived(string srcChain, string srcUA);
    ...
    */
     ...
     * @dev payload: Data passed to the omReceive() function of the destination UA
     ...
     * @dev srcUA: Address of the UA on the source chain
    ...
    */
    ...
    mapping(address => uint256) public uaRedirectBudget;
    mapping(address => uint256) public srcUARedirectBudget;
     ...
     *        Involved during cross-chain multi-protocol routing. For example, Optimism (LayerZero) to Moonbeam (Axelar).
     */
    function send(string memory dstChainName, address dstUA, bytes memory fnData, uint gas, address user, uint256 redirectFee) external override payable {
        bytes memory _payload = abi.encode(dstChainName, dstUA, fnData, gas, msg.sender, chainName, user, redirectFee);
        RouteCall memory params = RouteCall(dstChainName, _payload, gas, user, msg.sender, redirectFee);
        _route(params, false);
    }

    /**
     * @notice OmnichainRouter on the redirect chain is charged with redirection transaction fee. User Application
     *         needs to fund its budget to support redirections (multi-protocol messaging).
     *
     * @param remoteUA Address of the remote User Application ("UA") that calls the redirect chain Router.
     */
    function fundUA(address remoteUA) external payable {
        require(keccak256(bytes(chainName)) == keccak256(bytes(redirectChain)));
        uaRedirectBudget[remoteUA] += msg.value;
    }
     ...
     *
     * @param redirectionBudgetManager Address pointed by the srcUA (msg.sender) executing this function. Responsible for funding srcUA redirection budget.
     */
    function withdrawUARedirectFees(address redirectionBudgetManager) external {
        require(srcUARedirectBudget[msg.sender] > 0);
        srcUARedirectBudget[msg.sender] = 0;
        (bool sent,) = payable(redirectionBudgetManager).call{value: srcUARedirectBudget[msg.sender]}("");
    ...
    function _route(RouteCall memory params, bool isRedirected) internal {
        ...
        if (isRedirect) {
            _validateAndAssignRedirectFee(params.srcUA, params.redirectFee);
     ...
     * @param params See {RouteCall} struct.
     * @param isRedirect Used to set the destination UA considering possible redirection
     */
    ...
    function _processMessage(bytes memory payload) internal {
        (string memory dstChainName, address dstUA, bytes memory fnData, uint gas, address srcUA, string memory srcChain, address user, uint256 redirectFee)
        ...
        if (keccak256(bytes(dstChainName)) != keccak256(bytes(chainName))) {
            ...
            RouteCall memory params = RouteCall(dstChainName, payload, gas, user, srcUA, redirectFee);
            require(isDirectRoute(dstChainName), "NO_REDIRECTED_ROUTE");
            _validateAndChargeUA(params.srcUA, params.redirectFee);
            _route(params, true);

            return;
        }
        IOmniApp receiver = IOmniApp(dstUA);
        receiver.omReceive(fnData, srcUA, srcChain);
    }

    /**
     * @notice Validates the budget of the User Application ("UA"), Router's balance, and charges UA with redirection fee.
     *
     * @param remoteUA Address of the source UA
     * @param redirectFee Fee to be paid for the redirection by the OmnichainRouter contract. UA will be charged.
     */
    function _validateAndChargeUA(address remoteUA, uint256 redirectFee) internal {
        require(address(this).balance >= redirectFee, "ROUTER_NOT_FUNDED");
        require(uaRedirectBudget[remoteUA] >= redirectFee, "UA_NOT_FUNDED");
        uaRedirectBudget[remoteUA] -= redirectFee;
    }

    /**
     * @notice Validates the user's balance, and assigns redirection fee for UA redirections budget
     * @notice To be automated using native gas airdrop function in the future iteration
     *
     * @param srcUA Address of the User Application on the source chain
     * @param redirectFee Fee to be paid for the redirection by the OmnichainRouter contract. UA will be charged on destination.
     */
    function _validateAndAssignRedirectFee(address srcUA, uint256 redirectFee) internal {
        require(redirectFee > 0, "NO_REDIRECT_FEE");
        srcUARedirectBudget[srcUA] += redirectFee;
    }
...
interface IOmniApp {
    /**
     * @notice Function to be implemented by the User Application ("UA") utilizing Omnichain Router for receiving
     *         cross-chain messages.
     *
     * @param payload Encoded payload with a data for a target function execution.
     * @param srcUA Address of the remote User Application ("UA") that can be used for source validation.
     * @param srcChain Name of the source remote chain.
     */
    function omReceive(bytes calldata payload, address srcUA, string memory srcChain) external;
}
