...
//宠物铸造合约
contract PetFactory is Ownable, ReentrancyGuard, AccessControlEnumerable, IFactory, Pausable {
    ...
    struct Pet {
        ...
        uint256 weight;              //权重
        uint256 prop_sum;            //属性总和(capacity + scan + duration)
        uint256 count;               //实际数量
    ...
    constructor(address _mgge, address _serverSide, uint256 _salt) ReentrancyGuard() {
       ...
       //将部署者设置为超级管理员
        ...
        //初始化
        //宠物1 
        uint _type_id =  _addType(14101,100,130,"attack");
        _setPropValues(_type_id, "potential",1,5); 
        _setPropValues(_type_id, "duration",40,50); 
        _setPropValues(_type_id, "scan",10,20); 
        _setPropValues(_type_id, "capacity",50,60);      
        
        //第1次强化的区间
        _setUpgrades(1, "capacity", 2,5); 
        _setUpgrades(1, "scan", 1,3); 
        _setUpgrades(1, "duration", 0,0); 

        //第2次强化的区间
        _setUpgrades(2, "capacity", 4,8); 
        _setUpgrades(2, "scan", 2,4); 
        _setUpgrades(2, "duration", 0,0); 

        //第3次强化的区间
        _setUpgrades(3, "capacity", 8,15); 
        _setUpgrades(3, "scan", 3,6); 
        _setUpgrades(3, "duration", 0,0); 

        //第4次强化的区间
        _setUpgrades(4, "capacity", 12,20); 
        _setUpgrades(4, "scan", 4,7); 
        _setUpgrades(4, "duration", 0,0); 

        //第5次强化的区间
        _setUpgrades(5, "capacity", 15,25); 
        _setUpgrades(5, "scan", 5,10); 
    ...
    function _addType(
        ...
        uint256 _weight,              //权重
        uint256 _prop_sum,            //属性总和(capacity + scan + duration)
    ...
    function getTypeDesc(uint256 _type_id) public view returns(Pet memory){
        return pets[_type_id];
    }
  ...
  // MANAGING Authorization Contract Address for Call 
   ...
   // 合约部署者增加操作员
        ...
        returns(uint[] memory)
    {
        ...
        props[hash_capacity]   = 0;       //容量
        props[hash_scan]  = 0;            //扫码
        props[hash_duration]       = 0;   //耐久
        uint256 potential;           //潜力
        ...
        for (uint i = 0; i < _typeCount; i++) {
             if (pets[i].count < pets[i].prop_sum) {
                 _count ++;
             }
        }
        if (_count == 0) {
        ...
        bytes32 hash_prop_name = pets[_type_id].margin_name;
        require(
            hash_prop_name == hash_capacity ||  
            hash_prop_name == hash_scan ||  
            hash_prop_name == hash_duration, 
            "wrong margin_name"
        );
 
        Pet storage pet = pets[_type_id];
        pet.count +=1; //累加物品数量

        uint new_rand;
        if (settings[_type_id][hash_capacity].section > 0) {
            new_rand = (randomResult.sub(randomResult%10)).div(10);
            props[hash_capacity] = settings[_type_id][hash_capacity].base + new_rand % settings[_type_id][hash_capacity].section;
        } else {
            props[hash_capacity] = settings[_type_id][hash_capacity].base;
        }
        ...
        if (settings[_type_id][hash_scan].section > 0 ) {
            ...
            props[hash_scan] = settings[_type_id][hash_scan].base;
        }
        ...
        //不做溢出检测
        unchecked {
           uint margin = pets[_type_id].prop_sum - (props[hash_capacity] + props[hash_scan] + props[hash_duration]);
        ...
        uint[] memory ch =  new uint[](4);
        ch[0] =  props[hash_capacity];
        ch[1] =  props[hash_scan];
        ...
        returns(uint[] memory)
    {
       ...
       if (upgrades[_potential][hash_capacity].section > 0) {
             charactor[0] =  ch.capacity + upgrades[_potential][hash_capacity].base + randomResult % upgrades[_potential][hash_capacity].section;
       } else {
            charactor[0] =  ch.capacity + upgrades[_potential][hash_capacity].base;
       }
       ch.capacity = charactor[0];

       if (upgrades[_potential][hash_scan].section > 0) {
           charactor[1] =  ch.scan +  upgrades[_potential][hash_scan].base + randomResult % upgrades[_potential][hash_scan].section;
       } else {
           charactor[1] =  ch.scan +  upgrades[_potential][hash_scan].base;
       }
       ch.scan = charactor[1];

       if (upgrades[_potential][hash_duration].section > 0) {
            charactor[2] = ch.duration +  upgrades[_potential][hash_duration].base + randomResult % upgrades[_potential][hash_duration].section;
       } else {
            charactor[2] = ch.duration + upgrades[_potential][hash_duration].base;
       }
       ch.duration = charactor[2];
        ...
        requiresRevert(_to, _id, _orderid, signature)
    {
        ...
        ch.capacity    = revertcharacters[_id].capacity;      //容量
