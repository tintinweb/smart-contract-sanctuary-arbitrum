...
contract StrategySellFacet is IStrategySell {
    ...
    error InvalidRealizedPNL(uint256 realizedPNL,uint256 collactoralAmount);
    /**
    ...
    function executeSellStrategy(bytes32 takerRequestHash, bytes32 makerRequestHash) external {
        ...
        if (!strategyStatus) {
            ...
            //  revert InvalidRealizedPNL(uint256(-takerStrategy.realisedPnl), takerStrategy.collateralAmount);
            ...
            if (takerStrategy.realisedPnl > 0) {
                ...
                // if(takerStrategy.collateralAmount - uint256(-takerStrategy.realisedPnl) > 0) {
                    LibVault._marginIncrease(
                    takerStrategy.owner,
                    takerStrategy.collateralAmount - uint256(-takerStrategy.realisedPnl)
                    );
