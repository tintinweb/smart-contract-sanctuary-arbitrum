...
// File contracts/interfaces/IDexPool.sol
...
contract DexRouter is Ownable {
        ...
        internal
    {
        ...
        //(,, uint reserve0, uint reserve1) = IDexPool(_poolAddress).getReserves(_tokenIn);
        ...
        external
    {
        ...
        address poolAddress = IPoolFactory(factory).getPairAddress(token0, token1);
        require(poolAddress != address(0), "pool does not exist!");

        // we transfer here because otherwise you have to approve every 
        // smart contract pool. 
        ...
        require( (balanceAfter - balanceBefore) >= _minAmountOut, 'insufficient output amount');
    }
    ...
    function addLiquidity(
        ...
        external
    {
        require(_token0 != _token1, "tokens should be different!");
        require(_token0 != address(0) && _token1 != address(0), "tokens should not be zero!");
        require(_amount0 > 0, "amount in should not be zero");
        require(_amount0 > 0, "amount in should not be zero");
        ...
        IERC20(token0).safeTransferFrom(msg.sender, poolAddress, _amount0);
        IERC20(token1).safeTransferFrom(msg.sender, poolAddress, _amount1);

        IDexPool(poolAddress).addLiquidity(_amount0, _amount1);
    }
    ...
    {
        address poolAddress = IPoolFactory(factory).getPairAddress(_token0, _token1);
        require(poolAddress != address(0), "pool does not exist!");
    ...
    {
        address poolAddress = IPoolFactory(factory).getPairAddress(_tokenIn, _tokenOut);
        require(poolAddress != address(0), "pool does not exist!");
