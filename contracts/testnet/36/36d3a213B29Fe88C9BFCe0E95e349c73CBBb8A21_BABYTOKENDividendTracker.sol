...
contract StandardBabyTokenFactory is TokenFactoryBase {
    using Address for address payable;
    StandardBabyToken[] public babyTokens;
    ...
    function create(
        ...
        StandardBabyToken(payable(token)).initialize(
        owner,
        name,
        symbol,
        totalSupply,
        ...
        babyTokens.push(StandardBabyToken(payable(token)));
        assignTokenToOwner(owner, token, 4);
        emit TokenCreated(owner, token, TokenType.baby, 1);
    }

    function getTokens() public view returns(StandardBabyToken[] memory) {
        return babyTokens;
    }
}