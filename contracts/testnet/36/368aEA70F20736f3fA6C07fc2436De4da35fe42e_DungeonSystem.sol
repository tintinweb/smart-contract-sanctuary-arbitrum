...
import {ActionXpComponent, Layout as ActionXpComponentStruct, ID as ACTION_XP_COMPONENT_ID} from "../generated/components/ActionXpComponent.sol";
...
contract DungeonSystem is IDungeonSystem, GameRegistryConsumerUpgradeable {
    ...
    function _handleAccountXpGranting(
        ...
        ActionXpComponent actionXpComponent = ActionXpComponent(
            _gameRegistry.getComponent(ACTION_XP_COMPONENT_ID)
        );
        ActionXpComponentStruct memory actionXp = actionXpComponent.getValue(
            dungeonTriggerEntity
        );
        uint256 amountToGrant;
        if (success) {
            amountToGrant = actionXp.successAmount;
        } else {
            amountToGrant = actionXp.failAmount;
        }
        ...
        if (
            amountToGrant == 0 || currentAccruedXp >= actionXp.maxAmountAllowed
        ) {
            return;
        }
        if (currentAccruedXp + amountToGrant >= actionXp.maxAmountAllowed) {
            amountToGrant = actionXp.maxAmountAllowed - currentAccruedXp;
        }
...
struct EndDungeonBattleParams {
    ...
    uint256 dungeonTriggerEntity;
    uint256 node;
    uint256 totalDamageTaken; // TODO: REMOVE
    uint256 totalDamageDealt; // TODO: REMOVE
    uint256[] moves; // TODO: REMOVE
    bool success;
}
...
uint256 constant ID = uint256(
    keccak256("game.piratenation.actionxpcomponent.v1")
);

struct Layout {
    uint64 xpAmountToReward;
    uint64 successAmount;
    uint64 failAmount;
    uint64 maxAmountAllowed;
}

library ActionXpComponentStorage {
...
/**
 * @title ActionXpComponent
 * @dev Action Xp Component
 */
contract ActionXpComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Amount of Xp to reward for completing the action
        keys[0] = "xp_amount_to_reward";
        values[0] = TypesLibrary.SchemaValue.UINT64;

        // Amount of Xp to reward for successfully completing the action
        keys[1] = "success_amount";
        values[1] = TypesLibrary.SchemaValue.UINT64;

        // Amount of Xp to reward for failing the action
        keys[2] = "fail_amount";
        values[2] = TypesLibrary.SchemaValue.UINT64;

        // Max amount of Xp allowed to be rewarded for completing the action
        keys[3] = "max_amount_allowed";
        values[3] = TypesLibrary.SchemaValue.UINT64;
    }
    ...
    function getValue(
        ...
        value = ActionXpComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = ActionXpComponentStorage.layout().entityIdToStruct[
            entity
        ];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](4);
        values[0] = abi.encode(s.xpAmountToReward);
        values[1] = abi.encode(s.successAmount);
        values[2] = abi.encode(s.failAmount);
        values[3] = abi.encode(s.maxAmountAllowed);
    }
    ...
    function getBytes(
        ...
        Layout memory s = ActionXpComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function setBytes(
        ...
        (
            ...
            s.failAmount,
            s.maxAmountAllowed
        ) = abi.decode(value, (uint64, uint64, uint64, uint64));
    ...
    function remove(
        ...
        delete ActionXpComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete ActionXpComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.xpAmountToReward = value.xpAmountToReward;
