...
contract PositionRouter is Ownable, ReentrancyGuard {
    ...
    uint256 immutable expScale = 10**18;
    ...
    function updateManager(MarginManager newManager) public onlyOwner {
        manager = newManager;
    }
    ...
    function openShortPositionRequest(
        ...
        address token = isCall ? address(IOption(option).UNDERLYING()) : address(IOption(option).USDC());
        require(isCall ? lens._checkCallIsListed(callFactory, option) : lens._checkPutIsListed(putFactory, option), "Not a valid market");
        require(IOption(option).endingTime() != 0, "Invalid maturity");

        ERC20(token).transferFrom(msg.sender, address(this), collateral);

        uint256 currentIndex = indexPosition;
        bytes32 requestKey = getRequestKey(currentIndex);
        indexPosition++;
        ...
        positionRequest.assetToBorrow = token;
        positionRequest.isCall = isCall;
        positionRequest.amountOfUnderlying = amountOfUnderlying;
        positionRequest.endingTime = isCall ? AoriCall(option).endingTime() : AoriPut(option).endingTime();
        positionRequest.index = currentIndex;
    ...
    function executeOpenPosition(uint256 indexToExecute) public returns (bytes32) {
        ...
        ERC20 underlying = positionRequest.isCall ? (IOption(positionRequest.option).UNDERLYING()) : (IOption(positionRequest.option).USDC());
        ...
        bytes32 keyToEmit = manager.openShortPosition(positionRequest);
        lastPosition++;
    ...
    function rejectOrCancelIncreasePosition(uint256 indexToReject) public {
        ...
        ERC20 underlying = positionRequest.isCall ? (IOption(positionRequest.option).UNDERLYING()) : (IOption(positionRequest.option).USDC());
        underlying.transfer(positionRequest.account, positionRequest.collateral);
    ...
    /**
        ...
        @notice util is inputted in 1e18
     */
    function getBorrowRate(ERC20 token) public view returns (uint256) {
        require(manager.vaultAdd(token) != address(0x0), "Unsupported vault");
        address vault = manager.vaultAdd(token);
        uint256 util = Structs.mulDiv(IVault(vault).totalBorrows(), 10**token.decimals(), token.balanceOf(address(vault)) + IVault(vault).totalBorrows());
        IRM memory irm = interestRateModels[token];
        uint256 rate = (util <= irm.kinkUtil)
            ? irm.baseRate + Structs.mulDiv(util, irm.rateBeforeUtil, 10**token.decimals())
            : irm.baseRate + Structs.mulDiv(irm.kinkUtil, irm.rateBeforeUtil, 10**token.decimals()) + Structs.mulDiv((util - irm.kinkUtil), irm.rateAfterUtil, 10**token.decimals());
        return rate / secondsPerYear;
    }

    function getInterestOwed(bool isCall, ERC20 underlying, uint256 strikeInUSDC, uint256 optionSize, uint256 entryMarginRate, uint256 lastAccrueTime, uint256 newAccrueTime) public view returns (uint256) {
        if(newAccrueTime - lastAccrueTime == 0) {
            return 0;
        } else {
        uint256 interestFactor;
        uint256 decimalDiff;
        if (underlying.decimals() < 18) {
            decimalDiff = 18 - underlying.decimals();
        }
        uint256 borrowRate = getBorrowRate(underlying);
        interestFactor = ((borrowRate + entryMarginRate) / 2) * (newAccrueTime - lastAccrueTime);
        if (isCall) {
            return Structs.mulDiv(interestFactor, optionSize, expScale);
        } else {
            uint256 amountOfUnderlying = Structs.mulDiv(strikeInUSDC, optionSize, 10**underlying.decimals());
            return Structs.mulDiv(interestFactor, amountOfUnderlying, expScale);
        }
        }
    }
    ...
    function isLiquidatable(ERC20 token, uint256 fairValueOfOption, uint256 optionSize, uint256 collateral, bool isCall) public view returns(uint256, uint256, bool) {
        ...
        collateralVal = Structs.mulDiv(getPrice(manager.oracles(address(token))), collateral, 10**token.decimals());         
        uint256 positionVal = Structs.mulDiv(fairValueOfOption, optionSize, 10**token.decimals());
        uint256 liquidationThreshold = Structs.mulDiv(positionVal, liquidationRatio, BPS_DIVISOR);
...
contract MarginManager is Ownable {
    ...
    function openShortPosition(Structs.OpenPositionRequest memory params) public returns (bytes32) {
        require(msg.sender == address(positionRouter), "address is not position router");
...
contract AoriLens {
    ...
    function getUserActiveLongPositions(CallFactory callFactory, PutFactory putFactory, OrderbookFactory orderbookFactory, address user) public view returns (LensCollatPosition[] memory, LensCollatPosition[] memory) {
        AoriCall[] memory activeCalls = getActiveCallMarkets(callFactory);
        LensCollatPosition[] memory userCalls = new LensCollatPosition[](activeCalls.length);

        AoriPut[] memory activePuts = getActivePutMarkets(putFactory);
        LensCollatPosition[] memory userPuts = new LensCollatPosition[](activePuts.length);

        for(uint i; i < activeCalls.length; i++) {
            if(activeCalls[i].balanceOf(user) > 0) {
                userCalls[i].account = user;
                userCalls[i].isCall = true;
                userCalls[i].isLong = true;
                userCalls[i].option = address(activeCalls[i]);
                userCalls[i].name = activeCalls[i].name();
                userCalls[i].optionSize = activeCalls[i].balanceOf(user);
                userCalls[i].orderbook = orderbookFactory.getBookByOption(address(activeCalls[i]));
                
            }
        }
        for(uint j; j < activePuts.length; j++) {
            if(activePuts[j].balanceOf(user) > 0) {
                userPuts[j].account = user;
                userPuts[j].isCall = true;
                userPuts[j].isLong = true;
                userPuts[j].option = address(activePuts[j]);
                userPuts[j].name = activePuts[j].name();
                userPuts[j].optionSize = activePuts[j].balanceOf(user);
                userPuts[j].orderbook = orderbookFactory.getBookByOption(address(activePuts[j]));
            }
        }
        return (userCalls, userPuts);
    }

    function getUserActiveLeveredShortPositions(MarginManager manager, address user) public view returns (LensLeveredPosition[] memory) {
        bytes32[] memory keys = getLeveredPositions(manager, user);
        LensLeveredPosition[] memory leveredData = new LensLeveredPosition[](keys.length);
        for(uint i; i < leveredData.length; i++) {
            (address account, bool isCall, , address option, , uint256 optionSize, uint256 collateral, uint256 entryMarginRate, uint256 lastAccrueTime, address orderbook, ) = manager.getPositionWithKey(keys[i]);
            leveredData[i].account = account;
            leveredData[i].isCall = isCall;
            leveredData[i].isLong = false;
            leveredData[i].isLevered = true;
            leveredData[i].option = option;
            leveredData[i].name = AoriCall(option).name();
            leveredData[i].optionSize = optionSize;
            leveredData[i].collateral = collateral;
            leveredData[i].entryMarginRate = entryMarginRate;
            leveredData[i].lastAccrueTime = lastAccrueTime;
            leveredData[i].orderbook = orderbook;
        }
        return (leveredData);
    }

    function getUserActiveCollateralizedShortPositions(OrderbookFactory orderbookFactory, CallFactory callFactory, PutFactory putFactory, address user) public view returns (LensCollatPosition[] memory, LensCollatPosition[] memory) {
        AoriCall[] memory activeCalls = getActiveCallMarkets(callFactory);
        LensCollatPosition[] memory userCalls = new LensCollatPosition[](activeCalls.length);
        ...
        for(uint i; i < activeCalls.length; i++) {
            if(getShortCallBalance(activeCalls[i], user) > 0) {
                userCalls[callCount].account = user;
                userCalls[callCount].isCall = true;
                userCalls[callCount].isLong = false;
                userCalls[callCount].isLevered = false;
                userCalls[callCount].option = address(activeCalls[i]);
                userCalls[callCount].name = AoriCall(activeCalls[i]).name();
                userCalls[callCount].optionSize = getShortCallBalance(activeCalls[i], user);
                userCalls[callCount].orderbook = orderbookFactory.getBookByOption(address(activeCalls[i]));
                callCount++;
            }
        }
        for(uint j; j < activePuts.length; j++) {
            if(getShortPutBalance(activePuts[j], user) > 0) {
                userPuts[putCount].account = user;
                userPuts[putCount].isCall = true;
                userPuts[putCount].isLong = false;
                userPuts[putCount].isLevered = false;
                userPuts[putCount].option = address(activePuts[j]);
                userPuts[putCount].name = AoriPut(activePuts[j]).name();
                userPuts[putCount].optionSize = getShortPutBalance(activePuts[j], user);
                userPuts[putCount].orderbook = orderbookFactory.getBookByOption(address(activePuts[j]));
                putCount++;
            }
        }

        LensCollatPosition[] memory resizedUserCalls = new LensCollatPosition[](callCount);
        for(uint l = 0; l < callCount; l++) {
            resizedUserCalls[l] = userCalls[l];
        }
        
        LensCollatPosition[] memory resizedUserPuts = new LensCollatPosition[](putCount);
        for(uint l = 0; l < putCount; l++) {
            resizedUserPuts[l] = userPuts[l];
        }
        return (resizedUserCalls, resizedUserPuts);
    }

    function getOpenInterest(CallFactory callFactory, PutFactory putFactory) public view returns (uint256) {
        AoriCall[] memory activeCalls = getActiveCallMarkets(callFactory);
        AoriPut[] memory activePuts = getActivePutMarkets(putFactory);
        uint256 openInterest;
        for(uint i; i < activeCalls.length; i++) {
            openInterest += Structs.mulDiv(activeCalls[i].totalSupply(), getPrice(AggregatorV3Interface(activeCalls[i].oracle())), 10**activeCalls[i].decimals());
        }
        for(uint i; i < activePuts.length; i++) {
            openInterest += Structs.mulDiv(activePuts[i].totalSupply(), getPrice(AggregatorV3Interface(activeCalls[i].oracle())), 10**activePuts[i].decimals());
        }
        return openInterest;
    }

    function getCumVolCumFees(OrderbookFactory orderbookFactory) public view returns (uint256, uint256) {
        ...
        return (cumVol, cumFees);
    }

    function getLeveredPositions(MarginManager manager, address user) public view returns(bytes32[] memory) {
    ...
    function getActiveOrderbooks(OrderbookFactory orderbookFactory) public view returns (Orderbook[] memory) {
        ...
        Orderbook[] memory resizedOrderbooks = new Orderbook[](activeCount);
        for(uint i = 0; i < activeCount; i++) {
            resizedOrderbooks[i] = activeBooks[i];
        }
        return resizedOrderbooks;
    }
    ...
    function tableEndingTimes(OrderbookFactory orderbookFactory) public view returns (uint256[] memory) {
        ...
        for (uint256 i; i < orderbooks.length; i++) {
            endingTimes[i] = AoriCall(address(orderbooks[i].OPTION())).endingTime();
        }
        return(endingTimes);
    }
    ...
    function tableCumVol(OrderbookFactory orderbookFactory) public view returns (uint256[] memory) {
        ...
        for (uint256 i; i < orderbooks.length; i++) {
            cumVol[i] = orderbooks[i].cumVol();
        }
        return(cumVol);
    }

    // function tableTopBidBottomAsk(Orderbook orderbook) public view returns (Bid, Ask) {
    //     Ask[] memory activeAsks = getActiveAsks(orderbook);
    //     Bid[] memory activeBids = getActiveBids(orderbook);
    //     Ask bottomAsk;
    //     Bid topBid;
    //     for (uint256 i; i < activeAsks.length; i++) {
    //         if(i == 0) {
    //             bottomAsk = activeAsks[i];
    //         } else {
    //             if(activeAsks[i].USDCPerOPTION() <= bottomAsk.USDCPerOPTION()) {
    //                 bottomAsk = activeAsks[i];
    //             }
    //         }
    //     }
    //     for (uint256 i; i < activeBids.length; i++) {
    //         if(i == 0) {
    //             topBid = activeBids[i];
    //         } else {
    //             if(activeBids[i].OPTIONPerUSDC() >= topBid.OPTIONPerUSDC()) {
    //                 topBid = activeBids[i];
    //             }
    //         }
    //     }
    //     return(topBid, bottomAsk);
    // }

    // function getActiveAsksByOwner(Orderbook factory) public view returns (Ask[] memory) {
    //     Ask[] memory allAsks = factory.getAsks();
    ...
    // function getActiveBidsByOwner(Orderbook factory) public view returns (Bid[] memory) {
    //     Bid[] memory allBids = factory.getBids();
    ...
    // function getLongCallBalance(AoriCall call, address buyer) public view returns (uint256) {
    //     return call.balanceOf(buyer);
    // }
    
    // function getLongPutBalance(AoriPut put, address buyer) public view returns (uint256) {
    //     return put.balanceOf(buyer);
    // }

    function getShortCallBalance(AoriCall call, address seller) public view returns (uint256) {
        return call.getOptionsSold(seller);
    }
    
    function getShortPutBalance(AoriPut put, address seller) public view returns (uint256) {
    ...
    */
    ...
    function getIsActive(Orderbook orderbook) public view returns (bool) {
        if(AoriCall(address(orderbook.OPTION())).endingTime() < block.timestamp) {
            return false;
        } else {
            return true;
        }
...
contract Vault is Ownable, ERC4626 {
    ...
    function updateManagerAndRouter(address newManager) public onlyOwner {
        manager = newManager;
    }
...
interface IOrderbookFactory is IOwnable {
    ...
    event OrderMade(address maker, uint256 price, uint256 size, bool isAsk, address order);
    event OrderFilled(address taker, uint256 price, uint256 size, bool isAsk, bool hasEnded, address order);
    ...
    function eventEmit(address makerOrTaker, uint256 price, uint256 size, bool isAsk, bool isMake, bool isCancelled, bool hasEnded, address order) external;
}
...
contract OrderbookFactory is Ownable {
    ...
    event AoriOrderbookCreated(address orderbook, address option, address usdc);
    event OrderMade(address orderbook, address maker, uint256 price, uint256 size, bool isAsk, address order);
    event OrderFilled(address orderbook, address taker, uint256 price, uint256 size, bool isAsk, bool hasEnded, address order);
    event OrderCancelled(address orderbook, address maker, bool isAsk, address order);
    /**
        Set the keeper of the Optiontroller.
        The keeper controls and deploys all new markets and orderbooks.
    */
    ...
    */
    ...
    function eventEmit(address makerOrTaker, uint256 price, uint256 size, bool isAsk, bool isMake, bool isCancelled, bool hasEnded, address order) external {
        ...
        if(isMake) {
            emit OrderMade(msg.sender, makerOrTaker, price, size, isAsk, order);
        } else {
            if(isCancelled) {
                emit OrderCancelled(msg.sender, makerOrTaker, isAsk, order);
            }
            emit OrderFilled(msg.sender, makerOrTaker, price, size, isAsk, hasEnded, order);
        }
    }
}
...
contract Orderbook {
    ...
    function createAsk(uint256 _USDCPerOPTION, uint256 _OPTIONSize) public returns (Ask) {
        ...
        ask.fundContract();
        factory.eventEmit(msg.sender, _USDCPerOPTION, _OPTIONSize, true, true, false, false, address(ask));
        return ask;
    }
    ...
    function createBid(uint256 _OPTIONPerUSDC, uint256 _USDCSize) public returns (Bid) {
        require((_OPTIONPerUSDC * 10**18) % bidMinTick == 0, "Prices must match appropriate tick size");
        ...
        bid.fundContract();
        factory.eventEmit(msg.sender, _OPTIONPerUSDC, _USDCSize, false, true, false, false, address(bid));
    ...
    function fillAsks(address receiver, uint256 _USDCPerOPTION, uint256 USDCsize, uint256 seatId, address[] memory asks) public returns(uint256) {
        ...
        for (uint256 i; i < asks.length; i++) {
            ...
            if(toFill > USDCsize) {
                ...
                factory.eventEmit(receiver, _USDCPerOPTION, USDCsize, true, false, false, false, address(asks[i]));
                break;
            }
            USDC.approve(asks[i], toFill);
            Ask(asks[i]).fill(receiver, toFill, seatId);
            factory.eventEmit(receiver, _USDCPerOPTION, USDCsize, true, false, false, true, asks[i]);
            USDCsize -= toFill;
    ...
    function fillBids(address receiver, uint256 _OPTIONPerUSDC, uint256 OPTIONsize, uint256 seatId, address[] memory bids) public returns(uint256) {
        ...
        for (uint256 i; i < bids.length; i++) {
            ...
            factory.eventEmit(receiver, _OPTIONPerUSDC, OPTIONsize, false, false, false, true, address(bids[i]));
            OPTIONsize -= toFill;
    ...
    function cancelOrder(address order) public {
        require(msg.sender == IOrder(order).maker());
        IOrder(order).cancel();
        factory.eventEmit(IOrder(order).maker(), 0, 0, false, false, true, true, order);
    }
...
contract Bid {
    ...
    */
    function fill(address receiver, uint256 amountOfOPTION, uint256 seatId) public returns (uint256) {
        ...
        if(receiver == seatOwner) {
            USDCToReceive = OPTIONAfterFee / OPTIONPerUSDC;
        } else {
            USDCToReceive = amountOfOPTION / OPTIONPerUSDC;
            uint256 seatScoreFeeInBPS = mulDiv(fee, ((AORISEATSADD.getSeatScore(seatId) * 500) + 3500), 10000);
