...
/**
 * @title Miner game, players need to find occurrences of the selected item
 */

contract Miner is Common {
    ...
    mapping(address => MinerGame) minerGames;
    mapping(uint256 => address) minerIDs;
     ...
     * @param selectedItem item selected before the game
     */
    event Miner_Play_Event(
     ...
     * @param selectedItem item selected before the game
     * @param rockOutcomes the 10 items in the rock
     */
    event Miner_Outcome_Event(
        address indexed playerAddress,
        uint256 wager,
        uint256 payout,
        address tokenAddress,
    ...
    /**
     * @dev event emitted when a refund is done
     ...
     */
    event Miner_Refund_Event(address indexed player, uint256 wager, address tokenAddress);
     ...
     */
    function Miner_GetState(address player) external view returns (MinerGame memory) {
        return (minerGames[player]);
    }

    /**
     * @dev Function to play Miner, takes the user wager save  selected item and makes a request to the VRF
     * @param wager wager amount
     * @param tokenAddress address of token to bet, 0 address is considered the native coin
     * @param itemSelected item selected
     */
    function Miner_Play(uint256 wager, uint8 itemSelected, address tokenAddress) external payable nonReentrant {
        address msgSender = _msgSender();
        if (itemSelected < 0 || itemSelected > 4) {
            revert('Miner : selected item must be between 0 and 4');
        }
        if (minerGames[msgSender].requestID != 0) {
            revert('Range : waiting for VRF');
        }

        uint256 fee = _transferWager(tokenAddress, wager, 700000, 21, msgSender);

        uint256 id = _requestRandomWords(1);

        minerGames[msgSender] = MinerGame(
            wager,
            id,
            tokenAddress,
            uint64(block.number),
        ...
        minerIDs[id] = msgSender;

        emit Miner_Play_Event(msgSender, wager, tokenAddress, itemSelected, fee);
    }

    /**
     * @dev Function to refund user in case of VRF request failling
     */
    function Miner_Refund() external nonReentrant {
        address msgSender = _msgSender();
        MinerGame storage game = minerGames[msgSender];
        if (game.requestID == 0) {
        ...
        delete (minerIDs[game.requestID]);
        delete (minerGames[msgSender]);
        ...
        emit Miner_Refund_Event(msgSender, wager, tokenAddress);
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal {
        address playerAddress = minerIDs[requestId];
        if (playerAddress == address(0)) revert();
        MinerGame storage game = minerGames[playerAddress];

        uint256 payout;
        uint8[] memory rockOutcomes = new uint8[](10);
        uint32 winningBets;
        ...
        for (uint i = 0; i < 10; i++) {
            uint256 segment = (randomWords[0] >> (i * 25)) & ((1 << 25) - 1);
            rockOutcomes[i] = uint8(segment % 5);
            if (rockOutcomes[i] == game.selectedItem) {
                occurrences++;
            }
        }

        uint16 multiplier = _getMultiplier(occurrences);

        if (game.isveFlush) {
            payout = (game.wager * multiplier) - game.wager;
        } else {
        ...
        emit Miner_Outcome_Event(playerAddress, game.wager, payout, tokenAddress, game.selectedItem, rockOutcomes);

        _burnAndTransfer(tokenAddress, game.wager, 1, winningBets);

        delete (minerIDs[requestId]);
        delete (minerGames[playerAddress]);
        if (payout != 0) {
            _transferPayout(playerAddress, payout);
        }
    }

    function _getMultiplier(uint8 occurrence) internal pure returns (uint16 multiplier) {
        if (occurrence < 3) {
            return 0;
        }
        if (occurrence == 3) {
