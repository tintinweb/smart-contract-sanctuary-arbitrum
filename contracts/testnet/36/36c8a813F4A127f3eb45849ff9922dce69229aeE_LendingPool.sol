...
interface IPoissonIncentivesController {
...
interface IInitializableAToken {
   ...
   * @param treasury The address of the Poisson treasury, receiving the fees on this aToken
...
interface IAToken is IERC20, IScaledBalanceToken, IInitializableAToken {
   ...
   **/
  function getIncentivesController() external view returns (IPoissonIncentivesController);
...
interface IPoissonAPRDataProvider {
...
library ValidationLogic {
  ...
  function validateBorrow(
    ...
    //Poisson: place where stETH is blocked from being borrowed
 ...
 * @dev Main point of interaction with an Poisson protocol's market
 * - Users can:
 *   # Deposit
 *   # Withdraw
 *   # Borrow
...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  function depositYield(address asset, uint256 amount) external override whenNotPaused {
    ...
    if (aprDataProvider != address(0)) {
      IPoissonAPRDataProvider(aprDataProvider).updateAPR(asset, amount, totalAmount);
    }
