    ...
    PausableUpgradeable
{
    ...
    function __RadpieCCIPRouter_init(address router, address link) public initializer {
        __Ownable_init();
        __ReentrancyGuard_init();
        __Pausable_init();
        chainlinkRouter = router;
        linkToken = link;
        LinkTokenInterface(linkToken).approve(chainlinkRouter, type(uint256).max);
    }
    ...
    modifier tokenAllowedToTransfer(address token) {
     ...
     * @param isNative A boolean indicating whether the token is native to the current chain.
    ...
    function estimateMsgGasFee(
        ...
        // Determine the fee token based on whether the token is native or not
        address feeToken = isNative ? address(0) : linkToken;
    ...
    function tokenTransfer(
        ...
        tokenAllowedToTransfer(_token)
    {
        if (_receiver == address(0) || _token == address(0)) revert InvalidAddress();

        if (_amount == 0 || gasFee == 0) revert InvalidAmount();

        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        IERC20(_token).approve(chainlinkRouter, _amount);

        uint256 balance;

        if (isNative) {
            if (msg.value < gasFee) revert InvalidAmount();
            balance = msg.value;
        } else {
            if (msg.value > 0) revert InvalidAmount();
            IERC20(linkToken).safeTransferFrom(msg.sender, address(this), gasFee);
            balance = IERC20(linkToken).balanceOf(address(this));
        }
        ...
        if (fee > balance) revert NotEnoughBalance(balance, fee);
        ...
        if (isNative)
            ...
            messageId = IRouterClient(chainlinkRouter).ccipSend(
                destinationChainSelector,
                evm2AnyMessage
            );
    ...
    /// @dev Adds a new token to the list of allowed tokens.
    /// @param token The address of the token to add.
    function addToken(address token) external onlyOwner {
        if (allowedTokens[token]) revert AlreadyRegsitered();
