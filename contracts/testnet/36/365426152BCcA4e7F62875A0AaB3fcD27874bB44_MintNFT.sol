...
contract MintNFT is ERC721Enumerable, Ownable {
    ...
    uint256 public standardPrice = 0.02 * 10**18;  // 0.01 ARB  // 350 ARB
    uint256 public whitelistPrice = 0.01 * 10**18; // 250 ARB
    ...
    function mint(address recipient, string memory uri) external {
        ...
        if(price > 0) {
            require(arbToken.transferFrom(msg.sender, address(this), price), "Token transfer failed");
        }
    ...
    */
    ...
    function mint(address recipient, string memory uri) external payable {
        require(mintingEnabled, "Minting is currently disabled");
        ...
        if(msg.value > price) { // Refund any extra ETH sent
    ...
    function mintMultiple(address recipient, string[] memory uris) external payable {
        require(mintingEnabled, "Minting is currently disabled");

        uint256 numToMint = uris.length;
        uint256 totalPrice = getMintPrice(msg.sender) * numToMint;

        require(msg.value >= totalPrice, "Incorrect ETH value sent");

        if(msg.value > totalPrice) { // Refund any extra ETH sent
    ...
    function mintMultiple(address recipient, string[] memory uris) external payable {
        ...
        require(arbToken.transferFrom(msg.sender, address(this), totalPrice), "Token transfer failed");
