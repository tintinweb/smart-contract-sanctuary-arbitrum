...
contract TokenVault is ERC20Upgradeable, ERC721HolderUpgradeable {
    ...
    event UpdateReservePrice(uint256 reservePrice);
    ...
    function updateAdmin(address _admin, bool _isAdmin) external {
        require(msg.sender == Ownable(settings).owner(), "update admin:not gov");
    ...
    function start() external payable onlyAdmin {
        require(auctionState == State.inactive, "start:no auction starts");
        require(msg.value >= getReservePrice(), "start:too low bid");
