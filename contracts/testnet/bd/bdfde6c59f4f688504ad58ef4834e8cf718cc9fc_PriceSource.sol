...
contract PriceSource {
    ...
    function encode(uint8 decimals, address[] memory aggregators) external view returns (bytes memory) {
        ...
        returns (UFixed128[] memory result)
    {
        ...
        for (uint256 i; i < length;) {
            result[i] = _price(
                abi.decode(
                    PriceSourceLib.priceSourceOf(constituents[i].asset, constituents[i].chainId),
                    (PriceSourceLib.Source)
                )
            );
            unchecked {
                i = i + 1;
    ...
    function priceSources(ConstituentLib.Constituent[] calldata constituents)
        external
        pure
        returns (PriceSourceLib.Source[] memory result)
    {
        uint256 length = constituents.length;
        result = new PriceSourceLib.Source[](length);
        for (uint256 i; i < length;) {
            result[i] = abi.decode(
                PriceSourceLib.priceSourceOf(constituents[i].asset, constituents[i].chainId), (PriceSourceLib.Source)
            );
            unchecked {
                i = i + 1;
            }
    ...
    // function totalAssets(ConstituentLib.Constituent[] memory constituents)
    //     external
    //     view
    ...
    //     uint256 length = constituents.length;
    //     for (uint256 i; i < length;) {
    //         total += constituents[i].balance.convertToBase(_price(
    //             abi.decode(
    //                 PriceSourceLib.priceSourceOf(constituents[i].asset, constituents[i].chainId),
    //                 (PriceSourceLib.Source)
    //             )
    ...
    function _price(PriceSourceLib.Source memory source) internal view returns (UFixed128) {
        ...
        uint256 price = PriceLib.Q128.mulDivUp(
        ...
        if (aggregatorsCount != 1) {
            for (uint256 i = 1; i < aggregatorsCount;) {
                price = price.mulDivUp(10 ** source.aggregatorDecimals[i], _getChainlinkPrice(source.aggregators[i]));
...
library ConstituentLib {
    ...
    function updateBalance(ConstituentSet storage set, address asset, uint256 chainId, uint128 balance)
        internal
        returns (bool)
    {
        uint256 key = _key(asset, chainId);
        if (_contains(set, key)) {
            set._values[set._indexes[key] - 1].balance = balance;
            return true;
        } else {
            return false;
        }
