...
contract SupplySchedule is Owned, ISupplySchedule {
    ...
    uint256 public constant INFLATION_START_DATE = 1673740800; // 2023-01-15T00:00:00+00:00

    // The number of SYNTH rewarded to the caller of SYNTH.mint()
    uint256 public minterReward = 100 * 1e18;

    // The number of SYNTH minted per week
    uint256 public inflationAmount;

    uint256 public maxInflationAmount = 3e6 * 1e18; // max inflation amount 3,000,000

    // Address of the SynthToken for the onlySynthToken modifier
    address public synthToken;

    // Max SYNTH rewards for minter
    ...
    /**
     * @return The amount of SYNTH mintable for the inflationary supply
     */
    ...
    function weeksSinceLastIssuance() public view returns (uint256) {
        ...
        uint256 timeDiff = lastMintEvent > 0 ? block.timestamp.sub(lastMintEvent) : block.timestamp.sub(INFLATION_START_DATE);
    ...
    /**
     * @notice Record the mint event from SYNTH by incrementing the inflation
     * week counter for the number of weeks minted (probabaly always 1)
     * and store the time of the event.
     * @param supplyMinted the amount of SYNTH the total supply was inflated by.
     * @return minterReward the amount of SYNTH reward for caller
     * */
    function recordMintEvent(uint256 supplyMinted) external onlySynthToken returns (uint256) {
    ...
    /**
     * @notice Sets the reward amount of SYNTH for the caller of the public
     * function SYNTH.mint().
     * This incentivises anyone to mint the inflationary supply and the mintr
     * Reward will be deducted from the inflationary supply and sent to the caller.
     * @param amount the amount of SYNTH to reward the minter.
     * */
    ...
    /**
     * @notice Set the SynthrProxy should it ever change.
     * SupplySchedule requires SYNTH address as it has the authority
     * to record mint event.
     * */
    function setSynthToken(IERC20 _synthToken) external onlyOwner {
        require(address(_synthToken) != address(0), "Address cannot be 0");
        synthToken = address(_synthToken);
        emit SynthTokenUpdated(synthToken);
    }
    ...
    /**
     * @notice Only the SYNTH contract is authorised to call this function
     * */
    modifier onlySynthToken() {
        require(msg.sender == synthToken, "Only the synth token contract can perform this action");
        _;
    }
    ...
    /**
     * @notice Emitted when the SYNTH minter reward amount is updated
     * */
    ...
    event SynthTokenUpdated(address newAddress);
}
