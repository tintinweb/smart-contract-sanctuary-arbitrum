...
contract Skills {
     ...
     */
    function stake(uint endDate, uint256 submissionsAllotted, uint256 numberOfWinners) payable external returns(uint256) {
     ...
     */
    function finalizeOrder(uint256 orderId) internal {
        uint256 numberOfWinnersFound = 1;
        address[] memory winnerAddresses;

        // get the first place winner
        uint currentScoreKey = _scoreTrees[orderId].last();
        uint currentTimestampKey = _timestampTrees[orderId][currentScoreKey].first();
        winnerAddresses[0] = _addressLookup[orderId][currentScoreKey][currentTimestampKey];

        // find all the remaining winners
        while(numberOfWinnersFound < _orders[orderId].numberOfWinners) {

            // check if there is a next timestamp entry, if so add them to the winners
            currentTimestampKey = _timestampTrees[orderId][currentScoreKey].next(currentTimestampKey);
            if (_timestampTrees[orderId][currentScoreKey].exists(currentTimestampKey)) {
                winnerAddresses[numberOfWinnersFound] = _addressLookup[orderId][currentScoreKey][currentTimestampKey];
                numberOfWinnersFound++;
                continue;
            }

            // no more entries for the current score, grab the next lowest skill
            // no need to check if it exists since we can't start an order that has less submissions than winners.
            currentScoreKey = _scoreTrees[orderId].prev(currentScoreKey);
            currentTimestampKey = _timestampTrees[orderId][currentScoreKey].first();
            winnerAddresses[numberOfWinnersFound] = _addressLookup[orderId][currentScoreKey][currentTimestampKey];
            numberOfWinnersFound++;
        }
        ...
        // evenly distribute winnings to the users
        for (uint256 i=0; i < numberOfWinnersFound; i++) {
            weth.withdrawTo(winnerAddresses[i], _winnerRewardAmount);
        }
