...
contract DiscountedRound is Ownable, ReentrancyGuard {
    ...
    */
    ...
    /// @dev Accepts direct ETH deposits 
    ...
    /**
     * @dev Set the start and end timestamps of the sale
     * @param _startTimestamp - unix timestamp of sale start
     * @param _endTimestamp - unix timestamp of sale end
     * Note: only callable by contract owner
     */
    function setSaleTimestamps(uint _startTimestamp, uint _endTimestamp) public onlyOwner {

        require(
            saleStartTime == 0 && saleEndTime == 0,
            "Sale start and end times can only be set once"
        );
        
        require(
            _endTimestamp > _startTimestamp,
            "sale cannot end before it starts"
        );

        saleStartTime = _startTimestamp;
        saleEndTime = _endTimestamp;
    }
