...
contract WINRBlackJack is IWINRBlackJack, CoreLight {
    uint64 internal constant houseEdge = 98;
    uint64 internal constant dealerStandOn = 17;
    uint64 internal constant minWagerAmount = 1e17;
    uint8 internal constant maxHands = 5;
    uint64 internal randomness_;
    ...
    //  playerAddress => array of handIndexes
    // note contract overwrites the array, so it can store handIndexes of a previous game, but these will not be accessed by the contract since array is parsed with amountHands
    mapping(address => uint32[5]) private handIndexesGame;

    // playerAddress => array of bools indicating if a handIndex (of handIndexesGame) is active.
    // @note contract overwrites the array, so it can store handIndexes of a previous game, but these will not be accessed by the contract since array is parsed with amountHands
    mapping(address => bool[5]) private activeHandsInGame;

    // playerAddress => Game
    // note player can only play one game at a time. game struct is overwritten when a new game is started
    mapping(address => Game) internal games;

    // playerAddress => GameIndex active player to active game index
        ...
        returns (uint256 gameIndex_, uint32 handIndex_)
    {
        unchecked {
            ...
            // require that GameStatus is Finished or None
            require(
    ...
    function randomizerFulfill(uint256 _requestId, uint256[] calldata _randoms) internal override {
        ...
        require(games[_player].awaitingRandomness, "Blackjack: Game is not awaiting randomness");
        ...
        if (game_.status == GameStatus.TABLE_DEAL) {
            ...
            uint256[] memory topCards_ = HelperBlackJack.drawFirstCardsFromStack(_randoms[0], amountCards_);
            ...
            if (_randoms.length == 2) {
                ...
                uint256 _gameIndex = activePlayerMapping[_player];
                uint256 topCard1_ = HelperBlackJack.drawSingleCardFromStack(_randoms[0], drawnCards[_gameIndex]);
                unchecked {
                ...
                uint256 topCard2_ = HelperBlackJack.drawSingleCardFromStack(_randoms[1], drawnCards[_gameIndex]);
                unchecked {
                ...
                uint256 _gameIndex = activePlayerMapping[_player];
                uint256 topCard_ = HelperBlackJack.drawSingleCardFromStack(_randoms[0], drawnCards[_gameIndex]);
                unchecked {
            ...
            uint256 _gameIndex = activePlayerMapping[_player];
            uint256[] memory topCards_ = HelperBlackJack.drawCardsFromStack(_randoms[0], 6, drawnCards[_gameIndex]);
    ...
    function _dealCardsToDealer(address _player, uint256[] memory _cardsToDealer) internal {
        ...
        unchecked {
            ...
            Cards memory _dealerCards = cards[_gameIndex];
            ...
            for (uint256 i = 1; i < 6; ++i) {
                ...
                _dealerCards.cards[i + 1] = uint8(_cardsToDealer[i]);
                // Deal the next card from _cardsToDealer to the dealer
                _dealerCards = HelperBlackJack.hitDealerWithCard(_dealerCards, uint8(_cardsToDealer[i]));
                // Check again if the dealer has to draw another card
                if (_dealerCards.totalCount >= dealerStandOn) {
                    cards[_gameIndex] = _dealerCards;
                    if (_dealerCards.totalCount >= 22) {
                        // If the dealer's total count is 22 or more, the dealer busts.
                        // Update the dealer's cards, emit an event, and process the dealer's bust.
                        emit DealerCardsDealt(_gameIndex, _dealerCards);
                        _processDealerBust(_player);
                        break;
                    } else {
                        // If the dealer's total count is less than 22 and greater than or equal to dealerStandOn, the dealer stands.
    ...
    function _checkState(uint256 _handIndex) internal view {
        ...
        // require(_game.status == GameStatus.PLAYER_TURN, "Blackjack: Game is not in player turn"); // note this is a logical check but technically redundant
