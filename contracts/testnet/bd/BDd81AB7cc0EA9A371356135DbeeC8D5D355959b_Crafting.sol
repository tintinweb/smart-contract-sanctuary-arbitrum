...
/// @dev Farmland - Crafting Smart Contract
...
// EVENTS
    ...
    event recipeCrafted(address indexed account, uint256 recipeID, uint256 item, uint256 amount);
...
// FUNCTIONS
    ...
    function craftItem(uint256 recipeID, uint256[] memory itemIDs, uint256[] memory amounts)
        ...
        nonReentrant
        whenNotPaused
        // onlyAllowed TODO: Introduce external permissioned crafting in the future
    {
        Recipe storage recipe = recipes[recipeID];                                                 // Shortcut accessor for the recipe
        require (recipe.active,                                                                    "Recipe inactive");
        require (itemIDs.length == amounts.length,                                                 "Number of items & amounts should match");
        require (keccak256(abi.encode(recipe.recipeItems)) == keccak256(abi.encode(itemIDs)),      "Items should match the recipe");
        require (keccak256(abi.encode(recipe.recipeAmounts)) == keccak256(abi.encode(amounts)),    "Items Amounts should match the recipe");
        uint256 itemToMint = recipe.itemID;
        uint256 totalToMint = recipe.amount;
        emit recipeCrafted(_msgSender(), recipeID, itemToMint, totalToMint);                       // Write an event
        itemsContract.burnBatch(_msgSender(),itemIDs,amounts);                                     // Burn items
        itemsContract.mintItem(itemToMint, totalToMint, _msgSender());                             // Mint craft item
        if (recipe.paymentAddress != address(0)) {                                                 // Check if a payable recipe
            IERC20 paymentContract = IERC20(recipe.paymentAddress);                                // Setup the payment contract
            uint256 priceToPay = recipe.price ;                                                    // Calculate price to pay
            require( paymentContract.balanceOf(_msgSender()) >= priceToPay,                        "Balance too low");
            paymentContract.safeTransferFrom(_msgSender(), address(this), priceToPay);             // Take the payment for crafting
        }
    }
...
// INTERNAL FUNCTIONS
    ...
    function withdrawAll() 
        ...
        onlyOwner
    {
        uint256 total = lastRecipeID;                                           // Store total number of recipes into a local variable to save gas
        uint256 amount;                                                         // Instantiate local variable to store the amount to withdraw
        for (uint256 recipeID=0; recipeID < total; recipeID++) {                // Loop through all recipes
            IERC20 paymentContract = IERC20(recipes[recipeID].paymentAddress);  // Setup the payment contract
    ...
    /// @param amount of the item to craft
    /// @param price set the price for the recipe
    /// @param paymentAddress set the contract for recipe payment
    /// @param active set the status of the recipe
    /// @param recipeItems a list of items for the recipe
    /// @param recipeAmounts a list of amounts for the recipe
    function createRecipe(string calldata name, uint256 itemID, uint256 amount, uint256 price, address paymentAddress, bool active, uint256[] calldata recipeItems, uint256[] calldata recipeAmounts)
        external
        onlyOwner
    {
        uint256 totalItems = recipeItems.length;
        require(totalItems == recipeAmounts.length,                   "Number of Items & Amounts must match");
        Recipe storage recipe = recipes[lastRecipeID];                // Shortcut accessor for the recipe
        recipe.name = name;                                           // Set the name
        recipe.itemID = itemID;                                       // Set the item
        recipe.amount = amount;                                       // Set the amount
        recipe.price = price;                                         // Set the price
        recipe.paymentAddress = paymentAddress;                       // Set the payment contract
        recipe.active = active;                                       // Set recipe as active/inactive
        recipe.recipeItems = recipeItems;                             // Add items to array
        recipe.recipeAmounts = recipeAmounts;                         // Add amounts to array
        lastRecipeID++;                                               // Increment the recipe number
    }
    ...
    /// @param recipeItems a list of items for the recipe
    /// @param recipeAmounts a list of amounts for the recipe
    function updateRecipe(uint256 recipeID, string calldata name, uint256 itemID, uint256 amount, uint256 price, address paymentAddress, bool active, uint256[] calldata recipeItems, uint256[] calldata recipeAmounts)
        public
        onlyOwner
    {
        uint256 totalItems= recipeItems.length;
        ...
        recipe.paymentAddress = paymentAddress;                       // Set the payment contract
        recipe.active = active;                                       // Set recipe as active/inactive
        delete recipe.recipeItems;                                    // Delete previous items
        delete recipe.recipeAmounts;                                  // Delete previous amounts
        recipe.recipeItems = recipeItems;                             // Add items to array
        recipe.recipeAmounts = recipeAmounts;                         // Add amounts to array
    }
    ...
    {
        uint256 total = lastRecipeID;                           // Store total number of recipes into a local variable
        if ( total == 0 ) {
            return allRecipes;                                  // if no recipes added, return an empty array
        } else {
            Recipe[] memory _allRecipes = new Recipe[](total);
            for(uint256 i = 0; i < total; i++){                 // Loop through the recipes
                _allRecipes[i] = recipes[i];                    // Add recipe to the array
            }
            return _allRecipes;
        }
    }
