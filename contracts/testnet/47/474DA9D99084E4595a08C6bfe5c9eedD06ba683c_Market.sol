...
contract Market is ERC721Holder, Ownable, ReentrancyGuard {
     ...
     * @notice Buy token with TOKEN by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenIds: array of tokenId of the NFT purchased
     * @param _prices: array of price (must be equal to the askPrice set by the seller)
     */
    function buyTokens(
        address _collection,
        ...
        uint256[] memory _prices
         ...
         for (uint256 i = 0; i < _tokenIds.length; i++) {
            Ask memory askOrder = _askDetails[_collection][_tokenIds[i]];
            IERC20(askOrder.tokenBuy).safeTransferFrom(address(msg.sender), address(this), _prices[i]);

            _buyToken(_collection, _tokenIds[i], _prices[i]);
        }
    }
