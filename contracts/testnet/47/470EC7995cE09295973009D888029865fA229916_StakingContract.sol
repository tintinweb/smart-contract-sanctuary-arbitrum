...
import "./FallToken.sol";
import "./IFALL.sol";
import "./sFallToken.sol";
import "./ISFALL.sol";


contract StakingContract {
    IFALL public fallToken;
    ISFALL public sfallToken;
    ...
    constructor(
        ...
        fallToken = IFALL(_fallToken);
        sfallToken = ISFALL(_sfallToken);
    ...
    function stake(uint256 amount) external {
        ...
        // Transfer FALL tokens from the user to the staking contract
        fallToken.transferFrom(msg.sender, address(this), amount);

        // Mint sFALL tokens to the user (assuming it's a non-tradable token)
        sfallToken.mint(msg.sender, amount);

        // Update the staked balance and timestamp of the user
    ...
    function unstake(uint256 amount) external {
        ...
        // Burn the sFALL tokens from the user
        sfallToken.burn(msg.sender, amount);

        // Calculate the unstaking fee
        uint256 unstakingFee = amount * unstakingFeePercentage / 100;

        // Calculate the amount of FALL tokens to be returned to the user
        uint256 returnedAmount = amount - unstakingFee;

        // Transfer the returned FALL tokens to the user
        fallToken.transfer(msg.sender, returnedAmount);

        // Transfer the unstaking fee to the treasury wallet
        fallToken.transfer(treasuryWallet, unstakingFee);
    ...
    function getStakedDuration(address account) external view returns (uint256) {
        require(stakedBalances[account] > 0, "No staked balance for the given address");
        return block.timestamp - stakedTimestamps[account];
    }
}
...
contract FALLTOKEN is IFALL, ERC20, Ownable {
...
contract sFALLTOKEN is ISFALL, ERC20, Ownable {
    ...
    constructor( uint256 _supply) ERC20("sFALL", "sFALL") 
    {
        supply = _supply;
        _mint(msg.sender, supply);
    }

    //Mints $FALLNODE to a recipient
