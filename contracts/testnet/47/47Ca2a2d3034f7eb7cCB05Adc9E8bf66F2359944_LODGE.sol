...
contract LEVEL is ERC20Burnable, Operator {
  ...
  uint256 public constant PRESALE_DISTRIBUTION = 100000 ether;
  ...
  function transferFrom(
    ...
    if (
      ...
      _transfer(sender, taxOffice, taxAmount);
      ITaxOffice(taxOffice).handleMainTokenTax(taxAmount);
...
contract LODGE is ERC20Burnable, Operator {
  ...
  function transferFrom(
    ...
    if (
      ...
      uint256 amountAfterTax = amount.sub(taxAmount);
      _transfer(sender, address(taxOffice), taxAmount);
      ITaxOffice(taxOffice).handleShareTokenTax(taxAmount);
      _transfer(sender, recipient, amountAfterTax);
    }
    _approve(
      sender,
