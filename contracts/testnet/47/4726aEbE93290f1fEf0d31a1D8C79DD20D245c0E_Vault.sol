...
contract Vault is IVault, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _decreasePosition(
        ...
        uint256 accountDebtAmount = debtAmount[_trader] + borrowingFee;
        if (_amountOutAfterFeesUsd == 0) {
            debtAmount[_trader] += accountDebtAmount;
        } else if (accountDebtAmount >= _amountOutAfterFeesUsd) {
            accountDebtAmount -= _amountOutAfterFeesUsd;
            _amountOutAfterFeesUsd = 0;
        } else {
            _amountOutAfterFeesUsd -= accountDebtAmount;
            accountDebtAmount = 0;
        }
        debtAmount[_trader] = accountDebtAmount;
