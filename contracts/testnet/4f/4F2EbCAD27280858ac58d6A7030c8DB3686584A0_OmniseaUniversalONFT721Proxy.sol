...
import "./OmniseaONFT721Proxy.sol";
import "./IOmniseaUniversalONFT721Proxy.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import {BasicCollectionParams} from "../structs/erc721/ERC721Structs.sol";
import "@openzeppelin/contracts/utils/introspection/ERC165.sol";

contract OmniseaUniversalONFT721Proxy is NonblockingLzApp, ERC165, IOmniseaUniversalONFT721Proxy, ReentrancyGuard {
    struct LzParams {
        uint16 dstChainId;
    ...
    uint256 public oseaBuybackFee; // 0.006 ETH will be used to buyback OSEA each quarter
    ...
    constructor(uint16 chainId_, address _lzEndpoint, uint256 _minGasToTransferAndStore) NonblockingLzApp(_lzEndpoint) {
        ...
        oseaBuybackFee = 6000000000000000;
        _chainId = chainId_;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC165, IERC165) returns (bool) {
        return interfaceId == type(IOmniseaUniversalONFT721Proxy).interfaceId || super.supportsInterface(interfaceId);
    }

    function estimateSendFee(address _collection, uint16 _dstChainId, bytes memory _toAddress, uint _tokenId, bool _useZro, bytes memory _adapterParams, BasicCollectionParams memory _collectionParams) public view virtual override returns (uint nativeFee, uint zroFee) {
        bytes32 collectionId = keccak256(abi.encode(msg.sender, _chainId));
        bytes memory payload = abi.encode(_toAddress, _tokenId, _collectionParams, collectionId);
        (nativeFee, zroFee) = lzEndpoint.estimateFees(_dstChainId, address(this), payload, _useZro, _adapterParams);

        nativeFee += fixedFee;

        if (collectionToId[_collection] == bytes32(0) && oseaBuybackFee > 0) {
            nativeFee += oseaBuybackFee;
        }
    }

    function sendFrom(address _collection, uint16 _dstChainId, bytes calldata _toAddress, uint _tokenId, address payable _refundAddress, address _zroPaymentAddress, bytes calldata _adapterParams) public payable virtual override nonReentrant {
        LzParams memory lzParams = _getLzParams(_dstChainId, _refundAddress, _zroPaymentAddress, _adapterParams);

        _send(lzParams, _toAddress, _tokenId, _collection);
    }
    ...
    function _send(
        ...
        IOmniseaONFT721 erc721 = IOmniseaONFT721(collection);

        address owner_;
        {
            // Try calling owner() method
            (bool successOwner, bytes memory dataOwner) = collection.call(abi.encodeWithSignature("owner()"));
            if (successOwner) {
                owner_ = abi.decode(dataOwner, (address));
            }
        }

        BasicCollectionParams memory _collectionParams = BasicCollectionParams({
            tokenId: tokenId,
            name: erc721.name(),
        ...
        bytes32 collectionId = collectionToId[collection];
        bool isFirstTime = collectionId == bytes32(0);
        if (isFirstTime) {
            // First-time bridging this collection - Generate ID
            collectionId = keccak256(abi.encode(collection, _chainId));
            idToCollection[collectionId] = collection;
            collectionToId[collection] = collectionId;
        }

        // Workaround for Stack Too Deep
        LzParams memory lzParams = _lzParams;

        _debitFrom(msg.sender, lzParams.dstChainId, collection, _toAddress, tokenId);

        {
            bytes memory payload = abi.encode(_toAddress, tokenId, _collectionParams, collectionId);
            _checkGasLimit(lzParams.dstChainId, FUNCTION_TYPE_SEND, lzParams.adapterParams, dstChainIdToTransferGas[lzParams.dstChainId]);
            (uint nativeFee) = _payONFTFee(msg.value, isFirstTime);
            _lzSend(lzParams.dstChainId, payload, lzParams.refundAddress, lzParams.zroPaymentAddress, lzParams.adapterParams, nativeFee);
            emit SendToChain(lzParams.dstChainId, msg.sender, _toAddress, tokenId);
        }
    }
    ...
    function _nonblockingLzReceive(
        ...
        (bytes memory toAddressBytes, uint tokenId, BasicCollectionParams memory _collectionParams, bytes32 _collectionId) = abi.decode(_payload, (bytes, uint, BasicCollectionParams, bytes32));
        ...
        if (collection == address(0)) {
            OmniseaONFT721Proxy proxy = new OmniseaONFT721Proxy();
            collection = address(proxy);
            IOmniseaONFT721(collection).initialize(_collectionParams);
            idToCollection[_collectionId] = collection;
            collectionToId[collection] = _collectionId;
        }

        _creditTo(_srcChainId, collection, toAddress, tokenId, _collectionParams.tokenURI);
    ...
    function _payONFTFee(uint _nativeFee, bool _isFirstTransfer) internal virtual returns (uint amount) {
        uint oseaBuybackFee_ = (_isFirstTransfer && oseaBuybackFee > 0) ? oseaBuybackFee : 0;
        uint fee = fixedFee;
        amount = _nativeFee - fee - oseaBuybackFee_;
        ...
        if (oseaBuybackFee_ > 0) {
            (bool p2,) = payable(oseaRevenueManager).call{value : (oseaBuybackFee_)}("");
            require(p2);
        }
    }

    function _debitFrom(address _from, uint16, address _collection, bytes memory, uint _tokenId) internal virtual {
        IOmniseaONFT721 collection = IOmniseaONFT721(_collection);
...
contract OmniseaONFT721Proxy {
    fallback() external payable {
        _delegate(address(0xF4dd9aDEB988C0E143057a23499317Fc0c1994FE));
    }
 ...
 */
interface IOmniseaUniversalONFT721Proxy is IERC165 {
    /**
     ...
     */
    function sendFrom(address _collection, uint16 _dstChainId, bytes calldata _toAddress, uint _tokenId, address payable _refundAddress, address _zroPaymentAddress, bytes calldata _adapterParams) external payable;
     ...
     */
    function estimateSendFee(address _collection, uint16 _dstChainId, bytes calldata _toAddress, uint _tokenId, bool _useZro, bytes calldata _adapterParams, BasicCollectionParams memory _collectionParams) external view returns (uint nativeFee, uint zroFee);
}
