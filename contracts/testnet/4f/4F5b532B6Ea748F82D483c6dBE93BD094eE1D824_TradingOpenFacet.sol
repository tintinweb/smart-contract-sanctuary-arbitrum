...
contract TradingOpenFacet is ITradingOpen, OnlySelf {
    ...
    function limitOrderDeal(LimitOrder memory order, uint256 marketPrice) external onlySelf override {
        ...
        OpenTrade memory ot = OpenTrade(
            ...
            IPairsManager(address(this)).getPairHoldingFeeRate(order.pairBase, order.isLong), Constants.arbSys.arbBlockNumber()
        );
    ...
    function _marketTradeDeal(
        ...
        OpenTrade memory ot = OpenTrade(
            ...
            pt.qty, IPairsManager(address(this)).getPairHoldingFeeRate(pt.pairBase, pt.isLong), Constants.arbSys.arbBlockNumber()
        );
...
library LibTrading {
    ...
    // todo: Replace with "apollox.trading.storage" for mainnet deployment
    bytes32 constant TRADING_POSITION = keccak256("apollox.trading.storage.20230809");
