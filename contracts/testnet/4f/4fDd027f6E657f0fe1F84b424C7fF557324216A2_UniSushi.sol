...
contract UniSushi is Ownable {
    ...
    function swap(uint256 _amountToSwap, uint _threshold) external {
        // balance validation
        // uint balance = IERC20(USDC).balanceOf(address(this));

        // emit BalanceState(USDC, balance, _amountToSwap);
        ...
        pathInFirstLeg[0] = WETH;
        pathInFirstLeg[1] = USDC;

        address[] memory pathInSecondLeg = new address[](2);
        pathInSecondLeg[0] = USDC;
        pathInSecondLeg[1] = WETH;
        ...
        emit BalanceState(USDC, baseCurrencyAmount, _amountToSwap);
    ...
    function getBalance(
        ...
        uint256[] memory uniSimulationResponse = uniswapRouter.getAmountsOut(balance, pathInFirstLeg);
        uint256 baseCurrencyAmount = uniSimulationResponse[1];
