...
contract Skills {
    ...
    function claimCreatorReward(uint256 orderId) external {

        // check to see the order is valid
        require(orderId < _orderCounter.current(), "The order submitted for has not been created yet.");
        require(_orders[orderId].status == OrderStatus.COMPLETED, "The order submitted is not completed.");
        require(_orders[orderId].creatorClaimed == false, "The order's creator reward has already been claimed.");

        // mark the creator has claimed their reward
        _orders[orderId].creatorClaimed = true;

        // withdraw the funds to the creator
        IWETH9Ext weth = IWETH9Ext(_wxpAddress);
        weth.withdrawTo(_orders[orderId].creator, _orders[orderId].totalReturn);

        // remove creators reward from the allocated funds
