...
contract SynthrIssuer is Owned, MixinSystemSettings, IIssuer {
    ...
    function _collateralisationRatioCrossChain(address _issuer) internal view returns (uint256, bool) {
        uint256 totalOwnedCollateral = _collateralCrossChain(_issuer);
        (uint256 debtBalance, , bool anyRateIsInvalid) = _debtBalanceOfAndTotalDebt(
            synthrDebtShare().balanceOfCrossChain(_issuer),
            sUSD
        );
    ...
    function _collateralCrossChain(address account) internal view returns (uint256) {
        uint256 balance = IERC20(address(wrappedSynthr())).balanceOf(account);
    ...
    function _collateral(address account) internal view returns (uint256) {
        uint256 balance = wrappedSynthr().chainBalanceOf(account);

        if (synthrDebtShare().balanceOfCrossChain(account) > 0) {
            if (address(rewardEscrowV2()) != address(0)) {
                balance = balance.add(
                    rewardEscrowV2()
                        .balanceOfCrossChainInUSDC(account)
                        .multiplyDecimal(synthrDebtShare().balanceOf(account))
                        .divideDecimal(synthrDebtShare().balanceOfCrossChain(account))
                );
            }
    ...
    function debtBalanceOfCrossChain(address _issuer) external view returns (uint256 debtBalance) {
        // What was their initial debt ownership?
        uint256 debtShareBalance = synthrDebtShare().balanceOfCrossChain(_issuer);
    ...
    function checkFreeCollateral(address _issuer, bytes32 _collateralKey) external view returns (uint256 withdrawableSynthr) {
        ...
        uint256 currentCollateral = wrappedSynthr().chainBalanceOf(_issuer);
        uint256 currentCollateralForKey = wrappedSynthr().chainBalanceOfPerKey(_issuer, _collateralKey);
    ...
    function getSendBurnGasFee(
        ...
        (uint256 existingDebt, , ) = _debtBalanceOfAndTotalDebt(synthrDebtShare().balanceOf(account), sUSD);
        ...
        uint256 currentDebtShare = synthrDebtShare().balanceOf(account);
        ...
        uint256 sharesToRemove = totalSharesToRemove.multiplyDecimal(currentDebtShare).divideDecimal(
            synthrDebtShare().balanceOfCrossChain(account)
        );
    ...
    function getSendLiquidateGasFee(
        ...
        uint256 currentDebtShare = synthrDebtShare().balanceOf(_account);
        ...
        uint256 notifyAmount;
        if (wrappedSynthr().collateralCurrency(_collateralKey) == NULL_ADDRESS) {
            ...
            notifyAmount = exchangeAgent().getSYNTHExpectedAmount(
                wrappedSynthr().collateralCurrency(_collateralKey),
                collateralAmount
            );
        }
        bytes memory lzPayload = abi.encode(
        ...
        )
    {
        ...
        uint256 transferableBalance = wrappedSynthr().chainBalanceOfPerKey(_account, _collateralKey);
        bool removeFlag = true;
