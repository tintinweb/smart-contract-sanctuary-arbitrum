...
contract CollectionFactory is NonblockingLzApp {
    ...
    function create(string memory chSym, string memory tName, string memory tSym, string memory URI, uint256 price, uint256 from, uint256 to, uint256 supply, address destAddr, string[] memory URIs) public payable {
        require(bytes(tName).length > 0 && bytes(tSym).length > 0 && bytes(chSym).length > 0 && chainSymbolToId[chSym] > 0);
        address tokenOwner = destAddr != address(0) ? destAddr : msg.sender;
        if (chainSymbolToId[chSym] == chainId) {
            address _collectionAddress = _collectionsRepository.addCollection(tName, tSym, URI, tokenOwner, supply, URIs, price, from, to);
            emit Created(_collectionAddress, tokenOwner);
            return;
        }
        bytes memory payload = abi.encode(tName, tSym, URI, tokenOwner, price, from, to, supply, URIs);
        uint16 chId = uint16(chainSymbolToId[chSym]);
        uint256 v = 1;
        uint256 gas = 10000000;
        bytes memory adapter = abi.encodePacked(v, gas);
        (uint fee,) = lzEndpoint.estimateFees(chId, address(this), payload, false, adapter);
        require(msg.value >= fee, "fee");
        lzEndpoint.send{value : msg.value}(chId, this.getTrustedRemote(chId), payload, payable(msg.sender), address(0x0), adapter);
    }
    ...
    function estimateFees(string memory chSym, string memory tName, string memory tSymbol, string memory URI, uint256 price, uint256 from, uint256 to, uint256 supply, address destAddr, string[] memory URIs) external view returns (uint) {
        bytes memory payload = abi.encode(tName, tSymbol, URI, destAddr, price, from, to, supply, URIs);
        uint16 chId = uint16(chainSymbolToId[chSym]);
        uint16 v = 1;
        uint gas = 10000000;
        bytes memory adapter = abi.encodePacked(v, gas);
