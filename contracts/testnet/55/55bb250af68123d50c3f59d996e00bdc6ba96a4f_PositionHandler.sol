...
contract PositionHandler is PositionConstants, IPositionHandler, BaseExecutor {
    ...
    */
    ...
    function _triggerPosition(
        ...
        settingsManager.updateCumulativeFundingRate(_position.indexToken, _isLong);
    ...
    function _decreasePosition(
        ...
        settingsManager.updateCumulativeFundingRate(_position.indexToken, _position.isLong);
        ...
        //Collect vault fee
        if (posData[3] > 0) {
            vault.accountDeltaAndFee(hasProfit, posData[3], posData[1]);
        }
    ...
    function _increasePosition(
        ...
        vaultUtils.validateLiquidation(_isLong, true, _indexPrice, _position);
    ...
    function _validateExecutor(address _account) internal view {
        require(_isExecutor(_account), "Forbidden: Not executor"); //Forbidden, not executor 
    }

    function _validatePositionKeeper() internal view {
        require(Address.isContract(address(positionKeeper)), "Invalid positionKeeper");
    }

    function _validateVaultUtils() internal view {
        require(Address.isContract(address(vaultUtils)), "Invalid vaultUtils");
    }

    function _validateRouter() internal view {
        require(Address.isContract(address(positionRouter)), "Invalid positionRouter");
    }
