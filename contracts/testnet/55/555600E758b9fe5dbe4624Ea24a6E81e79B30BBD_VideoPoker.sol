...
contract VideoPoker is Core, VideoPokerLogic {
  ...
  event Dealt(address indexed player, uint32 cards);

  event Finalized(address indexed player, uint256 payout, uint256 result, uint32 cards);
  ...
  function dealt(address _player, Game storage _game, uint256 _randomness) internal {
    ...
    emit Dealt(_player, _game.cards);
  }
  ...
  function finalize(address _player, Game storage _game) private {
    ...
    // event for frontend
    emit Finalized(_player, payout_, result_, _game.cards);
  }
  ...
  function start(
    ...
    requestPair[uint64(_requestRandom(1))] = player_; //! unsafe type casting
