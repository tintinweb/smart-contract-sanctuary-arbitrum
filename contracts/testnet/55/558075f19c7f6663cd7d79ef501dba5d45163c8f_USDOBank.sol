...
abstract contract USDOOperation is USDOBankStorage {
    ...
    event UpdateUSDO(address oldUSDO, address newUSDO);
    ...
    ///@notice update USDO address
    function updateUSDO(address newUSDO) external onlyOwner {
        emit UpdateUSDO(USDO, newUSDO);
        USDO = newUSDO;
    }
...
contract USDOMulticall {
    ...
    function getDepositData(address from, address collateral, uint256 amount, address to) external pure returns (bytes memory) {
...
contract USDOBank is IUSDOBank, USDOOperation, USDOView, USDOMulticall {
    ...
    event Deposit(address indexed collateral, address indexed from, address indexed to, address operator, uint256 amount);
    event Borrow(address indexed from, address indexed to, address indexed operator, uint256 amount, bool isDepositToJOJO);
    event Repay(address indexed from, address indexed to, uint256 amount);
    event Withdraw(address indexed collateral, address indexed from, address indexed to, address operator, uint256 amount);
    event Liquidate(
        address indexed collateral,
    ...
    function deposit(address from, address collateral, uint256 amount, address to)
        external
        override
        nonReentrant
    ...
    function flashLoan(address receiver, address collateral, uint256 amount, address to, bytes memory param)
        external
    ...
    function _settleCollateralAndUSDO(
        ...
        */
        if (liquidateAmount <= USDOBorrowed) {
            ...
            insuranceFee = USDOBorrowed.decimalMul(reserve.liquidityInfo.insuranceFeeRate)
                         .decimalDiv(1e18-reserve.liquidityInfo.insuranceFeeRate);
    ...
    function _depositOrRepay(address depositCollateral, uint256 depositAmount, DataTypes.UserInfo storage liquidatorInfo, address liquidator, uint256 tRate) internal {
