...
contract SelectNFT is ERC721, Ownable {
    ...
    constructor() ERC721("SelectNFT", "SLCT") {}
        ...
        onlyOwner 
    {
        ...
        require(projectId < nextProjectId && !projectData.locked, "SelectNFT: non-existent or locked project");
    ...
    function setProjectFees(uint128 projectId, address[] calldata feeReceivers, uint16[] calldata feeBasisPoints) public {
        ProjectData memory projectData = projects[projectId];
        require(projectId < nextProjectId && !projectData.locked, "SelectNFT: non-existent or locked project");
    ...
    function mint(address to, uint128 projectId) public {
        require(msg.sender == owner() || msg.sender == mintContract[projectId], "SelectNFT: not authorized to mint");
    ...
    function signatureTransfer(address from, address to, uint256 tokenId, uint256 nonce, bytes memory sig) public onlyOwner {
        /* CHECKS */
        require(!_usedNonces[msg.sender][nonce], "SelectNFT: invalid nonce for signature");

        bytes32 msgHash = keccak256(abi.encodePacked(
            "\x19Ethereum Signed Message:\n32",
            keccak256(abi.encodePacked(from, to, tokenId, nonce)))
        );
        require(from == msgHash.recover(sig), "SelectNFT: invalid signature");
    ...
    function signatureApprovalForAll(address account, address operator, bool approved, uint256 nonce, bytes memory sig) public onlyOwner {
        ...
        require(account == msgHash.recover(sig), "SelectNFT: invalid signature");
    ...
    function tokenIdxFromTokenId(uint256 tokenId) 
        ...
        returns(uint128)
    {
        return uint128((tokenId << 128) >> 128);
    }
    ...
    {
        return (projectIdFromTokenId(tokenId), tokenIdxFromTokenId(tokenId));
    }
...
// File: contracts/SelectNFTMarket.sol
...
contract SelectNFTMarket is Ownable {
    SelectNFT private immutable selectNFT;
    ...
    WrappedEth public weth = WrappedEth(0xb2c0652Beb5D6c39D0cb4A33990A2cB49385e95D);
    ...
    constructor(SelectNFT _nftAddr) {
        selectNFT = _nftAddr;
    }
