...
contract BGT_UFM {
    ...
    function registeredFp(uint256 __fp_id, uint256 __fp_quantity, uint256 day, uint256 __annualInterestRate) external onlyAdmin {
        require(_fps[__fp_id].fpId == 0, "Financial package has been registered");
        _fps[__fp_id] = Fp(__fp_id, day, __annualInterestRate, __fp_quantity, 0);
        emit update_fp(__fp_id, day, __annualInterestRate, __fp_quantity, 0);
    }
    ...
    function expired(uint256 __order_id) external {
        ...
        _order.interest = this.getFpInterest(_order.orderId);
    ...
    function getFpInterest(uint256 __order_id) external view returns (uint256) {
        Order storage _order = _orders[__order_id];
        if (_order.orderId == 0) return 0;
