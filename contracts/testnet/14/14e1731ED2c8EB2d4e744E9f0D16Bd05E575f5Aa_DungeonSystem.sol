...
contract DungeonSystem is IDungeonSystem, GameRegistryConsumerUpgradeable {
    ...
    function getDungeonMap(
        ...
        return
            IDungeonMapSystem(_getSystem(DUNGEON_MAP_SYSTEM_ID)).getDungeonMap(
                _getDungeonMapEntity(dungeonTriggerEntity)
            );
    }
    ...
    function getDungeonNode(
        ...
        return
            _getDungeonNode(_getDungeonMapEntity(dungeonTriggerEntity), node);
    }
    ...
    function startDungeonBattle(
        ...
        // Start battle and update dungeon progress state
    ...
    function endDungeonBattle(
        ...
        // Setup
        address account = _getPlayerAccount(_msgSender());
        DungeonTriggerSystem dungeonTriggerSystem = DungeonTriggerSystem(
        ...
        uint256 mapEntity = dungeonTriggerSystem.getDungeonMapEntity(
        ...
        DungeonNode memory node = _getDungeonNode(mapEntity, params.node);
    ...
    function _getExtraTimeForDungeonCompletion() public view returns (uint256) {
        return
    ...
    function _validateStartDungeonBattle(
        ...
        // Setup
        DungeonTriggerSystem triggerSystem = DungeonTriggerSystem(
            _getSystem(DUNGEON_TRIGGER_SYSTEM_ID)
        );
        ITokenTemplateSystem tokenTemplateSystem = ITokenTemplateSystem(
            _getSystem(TOKEN_TEMPLATE_SYSTEM_ID)
        );
        uint256 mapEntity = triggerSystem.getDungeonMapEntity(
            params.dungeonTriggerEntity,
            tokenTemplateSystem
        );
        (uint256 startAt, uint256 endAt) = triggerSystem.getTimeRange(
            params.dungeonTriggerEntity,
            tokenTemplateSystem
        );
        ...
        if (params.node == 0) {
            ...
            if (
                (endAt + _getExtraTimeForDungeonCompletion() < block.timestamp)
            ) {
                revert DungeonExpired(params.dungeonTriggerEntity);
            }
        }
    ...
    function _validateEndDungeonBattle(
        ...
        uint256 dungeonMapEntity,
        EndDungeonBattleParams calldata params,
        DungeonTriggerSystem triggerSystem,
        IDungeonProgressSystem progressSystem,
        ...
        if (
            !IDungeonMapSystem(_getSystem(DUNGEON_MAP_SYSTEM_ID))
                .hasDungeonNodes(dungeonMapEntity)
        ) {
            revert DungeonMapNotFound(params.dungeonTriggerEntity);
        }

        if (
            startAt > block.timestamp || // Start is in the future
            (endAt + _getExtraTimeForDungeonCompletion() < block.timestamp) // End is in the past, with extra time
        ) {
            revert DungeonExpired(params.dungeonTriggerEntity);
        }
