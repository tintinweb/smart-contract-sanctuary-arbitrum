...
contract BGT_UFM {
    ...
    event buy(address indexed sender, uint256 indexed fpId, uint256 orderId, uint256 amount);
    event update_fp(uint256 indexed fpId, uint256 day, uint256 annualInterestRate, uint256 fp_quantity, uint256 fp_quantity_sold);
    event order_expired(uint256 indexed orderId, uint256 indexed fpId, address indexed sender, uint256 amount, uint256 interest);
    ...
    function registeredFp(uint256 day, uint256 __annualInterestRate, uint256 __fp_quantity) external onlyAdmin {
        ...
        emit update_fp(_aufoFpIds, day, __annualInterestRate, __fp_quantity, 0);
    }
    ...
    function addAllowance(uint256 __fp_id, uint256 __fp_quantity) external onlyAdmin {
        ...
        emit update_fp(__fp_id, _fps[__fp_id].day, _fps[__fp_id].annualInterestRate, _fps[__fp_id].quantity, _fps[__fp_id].quantity_sold);
    }
    ...
    function create(uint256 __fp_id, uint256 __amount) external {
        ...
        emit buy(msg.sender, __fp_id, _autoOrderIds, __amount);
        emit update_fp(__fp_id, _fps[__fp_id].day, _fps[__fp_id].annualInterestRate, _fps[__fp_id].quantity, _fps[__fp_id].quantity_sold);
    }

    function expired(uint256 __order_id) external {
        ...
        emit order_expired(__order_id, _order.fpId, _order.sender,  _order.amount, _order.interest);
    }
    ...
    function getFpInterest(uint256 __order_id, address sender) external view returns (uint256) {
        ...
        uint256 interest = _order.amount * (block.timestamp - _order.time) * _fp.annualInterestRate / 100 / (365 days);
        return interest;
    }
