...
contract QarbCodes is QarbCodesInterface, QrCode, ERC721URIStorage, Ownable {
     ...
     * @dev ERC721 token with storage based token URI management.
     */
    function mintAllowList() external payable override returns (uint256) {
        require(1 <= _allowList[msg.sender], "ALLOW_LIST_MAX_EXCEEDED");
    ...
    function mint(string memory qrCodeText) external payable override {
    ...
    function togglePublicSaleActive() external override onlyOwner {
        ...
        if (isPublicSaleActive) isPhase2Active = true;
...
contract QrCode {
    ...
    function _generateQRURIByteVersion(
        ...
        DynamicBuffer.appendSafe(
            svgBytes,
            "<svg viewBox='0 0 296 296' width='296' height='296' xmlns='http://www.w3.org/2000/svg'><style>.bg{fill:#FCF}.fg{fill:#000}</style><rect class='bg' x='0' y='0' width='296' height='296'></rect>" // solhint-disable-line
        );
...
interface QarbCodesInterface {
    ...
    function mintAllowList() external payable returns (uint256);

    function mintFree(string memory qrCodeText) external returns (uint256);

    function mint(string memory qrCodeText) external payable;
