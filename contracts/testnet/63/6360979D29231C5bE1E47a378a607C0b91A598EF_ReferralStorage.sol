    ...
    IReferralStorage
{
    ...
    // Vested GEMB rate is 5% for all tiers
    uint256 public vGEMBRate = 500;
    ...
    IGEMLPManager public gemLpManager; // vault contract address
    IERC20 public gemLp;
    ...
    /**
     *
     * @dev setTier allows the governance to set the GEMLP and vGEMB rates for a given tier
     * @param _tierId the identifier for the tier being updated
     * @param _GEMLPRate the new GEMLP rate, expressed as a percentage in basis points (1% = 100 basis points)
     * @dev setTier allows the governance to set the GEMLP and vGEMB rates for a given tier
     * @dev sets the vGEMB rate to default value for all tiers
     */
    function setTier(
        uint256 _tierId,
        uint256 _GEMLPRate
    ) external override onlyGovernance {
        // require that the GEMLP rate is not greater than 100%
        require(
            _GEMLPRate <= BASIS_POINTS_DIVISOR,
            "ReferralStorage: invalid GEMLP Rate"
        );

        // get the current tier object from storage
        Tier memory tier = tiers[_tierId];

        // update the GEMLP and vGEMB rates in the tier object
        tier.GEMLPRate = _GEMLPRate;
        tier.vGEMBRate = vGEMBRate;

        // write the updated tier object back to storage
        tiers[_tierId] = tier;

        // emit an event to notify listeners of the tier update
        emit SetTier(_tierId, _GEMLPRate, vGEMBRate);
    }
     ...
     * @param _tierId the identifier for the tier being updated
     * @param _GEMLPRate the new GEMLP rate, expressed as a percentage in basis points (1% = 100 basis points)
     * @param _vGEMBRate the new vGEMB rate, expressed as a percentage in basis points (1% = 100 basis points)
     * @dev updateTier allows the governance to update the GEMLP and vGEMB rates for a given tier
     */
    function updateTier(
        uint256 _tierId,
        uint256 _GEMLPRate,
        ...
        // require that the vGEMB rate is not greater than 100%
        require(
            _vGEMBRate <= BASIS_POINTS_DIVISOR,
            "ReferralStorage: invalid vGEMB Rate"
        );
        ...
        // update the GEMLP and vGEMB rates in the tier object
        tier.GEMLPRate = _GEMLPRate;
        tier.vGEMBRate = _vGEMBRate;

        // write the updated tier object back to storage
        tiers[_tierId] = tier;

        // emit an event to notify listeners of the tier update
        emit SetTier(_tierId, _GEMLPRate, _vGEMBRate);
    }

    /**
     *
     * @param _vGEMBRate the new vGEMB rate, expressed as a percentage in basis points (1% = 100 basis points)
     * @dev updateVestedGEMBRate allows the governance to update the default vGEMB rate
     */

    function updateVestedGEMBRate(uint256 _vGEMBRate) external onlyGovernance {
        // require that the vGEMB rate is not greater than 100%
        require(
            _vGEMBRate <= BASIS_POINTS_DIVISOR,
            "ReferralStorage: invalid vGEMB Rate"
        ...
        // update the vGEMB rate in storage
        vGEMBRate = _vGEMBRate;

        // emit an event to notify listeners of the vGEMB rate update
        emit SetVestedGEMBRate(_vGEMBRate);
    }
    ...
    /**
     * @notice Changes the address of the GEMLP Manager contract and updates the GEMLP token contract address.
     * @param gemLpManager_ The new address of the GEMLP Manager contract.
     * @dev This function can only be called by the Timelock governance contract.
     * @dev The new GEMLP Manager address must not be null.
     * @dev Updates the GEMLP token contract address to the new GEMLP Manager's GEMLP token contract address.
     * @dev Emits a `GEMLPManagerUpdated` event upon successful execution.
     */
    function setGemLpManager(
        address gemLpManager_
    ) public onlyTimelockGovernance {
        // Ensure that the new GEMLP Manager address is not null.
        _checkNotNull(gemLpManager_);

        // Update the GEMLP Manager address and the GEMLP token contract address.
    ...
    /**
     *
     * @dev Returns the vested GEMB rate of the player
     * @param _account Address of the player
     * @return uint256 Vested GEMB rate of the player
     * @notice If the player has no referrer, the rate is 0
     * @notice This function overrides the getPlayerVestedGEMBRate function in the IReferralSystem interface
     */
    function getPlayerVestedGEMBRate(
        address _account
        ...
        // If the player has no referrer, return a vested GEMB rate of 0
        if (code_ == bytes32(0)) {
            return 0;
        }

        // Return the vested GEMB rate of the player's referrer's tier
        return tiers[referrerTiers[codeOwners[code_]]].vGEMBRate;
    }
    ...
    function setReward(
        ...
        if (referrer_ != address(0)) {
            ...
            uint256 amountRebate_ = calculateRebate(
                _amount,
                tiers[referrerTiers[referrer_]].GEMLPRate
            );
    ...
    /**
     *
     * @dev Allows a referrer to claim their rewards in the form of GEMLP tokens.
     * @dev Referrers cannot be on the blacklist.
     * @dev Rewards can only be withdrawn once per withdrawInterval.
     * @dev Calculates the total GEMLP amount and updates the withdrawn rewards.
     * @dev Transfers the GEMLP tokens to the referrer.
     */
    function claim(
        uint256[] calldata _minGemLps
        ...
        // effects: calculate total GEMLP amount and update withdrawn rewards
        uint256 totalGemLpAmount_;
        ...
        for (uint256 i = 0; i < wlTokens_.length; ++i) {
            ...
            // interactions: convert token rewards to GEMLP
            if (amount_ > 0) {
                totalGemLpAmount_ += _convertReferralTokensToGEMLP(
                    token_,
                    amount_,
                    _minGemLps[i]
                );
        ...
        // transfer GEMLP tokens to referrer
        if (totalGemLpAmount_ > 0) {
            gemLp.transfer(referrer_, totalGemLpAmount_);
        }
        emit Claim(referrer_, totalGemLpAmount_);
    }

    /**
     *
     * @param _referrer address of the referrer
     * @dev returns the amount of GEMLP that can be claimed by the referrer
     * @dev this function is used by the frontend to show the amount of GEMLP that can be claimed
     */
    function getPendingGEMLPRewards(
        address _referrer
    ) public view returns (uint256 totalGemLpAmount_) {
        address[] memory wlTokens_ = allWhitelistedTokens;
        ...
        for (uint256 i = 0; i < wlTokens_.length; ++i) {
            ...
            // If the referrer can claim some of the current token, calculate the GEMLP amount
            if (amount_ != 0) {
                ...
                uint256 aumInUsdw_ = gemLpManager.getAumInUsdw(true);

                // Calculate the GEMLP amount of the current token without deducting GEMLP minting fees
                uint256 amountWithFee_ = aumInUsdw_ == 0
                    ? usdwAmount_
                    : ((usdwAmount_ * IERC20(gemLp).totalSupply()) /
                        aumInUsdw_);
                ...
                // Calculate the amount of GEMLP that can be claimed for the current token
                totalGemLpAmount_ +=
                    (amountWithFee_ *
     ...
     */
    function _convertReferralTokensToGEMLP(
        address _token,
        uint256 _amount,
        uint256 _minGemLp
    ) internal returns (uint256 gemLpAmount_) {
        uint256 currentGEMLPBalance_ = gemLp.balanceOf(address(this));

        // approve GEMLPManager to spend the tokens
        IERC20(_token).approve(address(gemLpManager), _amount);

        // GEMLPManager returns amount of GEMLP minted
        gemLpAmount_ = gemLpManager.addLiquidity(_token, _amount, 0, _minGemLp);

        // note: if we want to check if the mint was successful and the GEMLP actually sits in this contract, we should do it like this:
        require(
            gemLp.balanceOf(address(this)) ==
                currentGEMLPBalance_ + gemLpAmount_,
            "ReferralStorage: GEMLP mint failed"
        );
    }
