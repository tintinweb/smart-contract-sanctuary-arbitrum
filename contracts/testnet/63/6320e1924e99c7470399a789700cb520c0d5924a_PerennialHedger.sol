    ...
    AccessControlUpgradeable
{
    ...
    /// @param delta Delta that needs to be hedged meaning that if delta is negative we need positive hedge
    /// @return deltaDiff difference between the new delta and the old delta
    function hedge(int256 delta) external onlyPool returns (int256 deltaDiff) {
        _syncDelta();

        int256 targetDelta = -delta;
    ...
    function getRequiredCollateral() external returns (int256) {
        return
            ...
            _getRequiredCollateral(productShort);
    }
    ...
    function _depositCollateral(IProduct product, uint256 amount) internal {
        ...
        USDC.pull(hedgedPool, UFixed18.wrap(amount));
