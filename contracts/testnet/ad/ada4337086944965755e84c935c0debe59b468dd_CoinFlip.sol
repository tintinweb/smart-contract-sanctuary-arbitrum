...
pragma solidity ^0.8.4;
contract CoinFlip is OwnableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IERC20Upgradeable public token; // CoinFlip token
    ...
    uint256 public callbackGasLimit; //min flip amounts
    ...
    function __CoinFlip_init(
        ...
        callbackGasLimit = _callbackGasLimit; //5_000_000;
    ...
    function flip(uint256 _flipAmountAndFees, CoinSide _headsOrTails) external notContract returns (uint256, uint256) {
        ...
        IERC20Upgradeable(address(token)).approve(treasuryAddress, fees);
    ...
    function randomizerCallback(uint256 _id, bytes32 _value) external {
        uint256 flipId = flipIdByRequestId[_id]; //_id(requestId)
