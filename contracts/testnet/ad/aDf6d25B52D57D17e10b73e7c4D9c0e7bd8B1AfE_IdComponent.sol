...
uint256 constant ID = uint256(keccak256("game.piratenation.idcomponent"));

struct Layout {
    string value;
}

library IdComponentStorage {
 ...
 */
contract IdComponent is BaseStorageComponent {
    /** SETUP **/
    ...
    function getValue(
        ...
        value = IdComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = IdComponentStorage
           .layout()
    ...
    function getBytes(
        ...
        Layout memory s = IdComponentStorage.layout().entityIdToStruct[entity];
    ...
    function remove(uint256 entity) public virtual {
        ...
        delete IdComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(uint256[] calldata entities) public virtual {
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete IdComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
