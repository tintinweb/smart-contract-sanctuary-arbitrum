...
library CurveBabyJubJub {
    ...
    // Checks if a square root of `x` exists.
    function sqrtExist(uint256 x) internal view returns (bool) {
        if (x == 0) {
            return true;
        }
        uint256 status = expmod(x, (Q - 1) / 2, Q);
        if (status == 1) {
            return true;
        } else if (status == Q - 1) {
            return false;
        } else {
            revert("Unexpected output");
        }
    }

    /// Checks if `x` is possible to belong to a point on Baby Jubjub Curve.
    function isValidCoordinateX(uint256 x) internal view returns (bool flag) {
        uint256 xSquare = mulmod(x, x, Q);
        return
            sqrtExist(
                mulmod(
                    submod(mulmod(A, xSquare, Q), 1, Q),
                    inverse(submod(mulmod(D, xSquare, Q), 1, Q)),
                    Q
                )
            );
    }
...
contract Shuffle is IShuffle, Ownable {
    ...
    uint8 public constant INVALID_CARD_INDEX = 255;
    uint8 public constant UNREACHABLE_PLAYER_INDEX = 9;
    uint8 public constant ALLOWED_PROOF_SUBMISSION_DELAY = 40;
    ...
    mapping(uint256 => mapping(uint256 => Deck)) public decks;
    ...
    function register(
        ...
        if (playerIndexes[gameId] == numPlayers[gameId]) {
            ...
            for (uint8 i = 1; i < numPlayers[gameId]; i++) {
                (
    ...
    function queryCardFromDeck(
        uint8 index,
        uint256 gameId
    ) external view inDealingPhase(gameId) returns (uint256[4] memory card) {
        uint8 lastPlayerIdx = numPlayers[gameId] - 1;
    ...
    function queryCardInDeal(
        ...
        uint8 playerIdx = cardDeals[gameId].curPlayerIdx[index];
    ...
    function queryCardDealRecord(
        uint8 index,
        uint256 gameId
    ) public view inDealingPhase(gameId) returns (uint8) {
        return cardDeals[gameId].record[index];
    }

    // Queries card deal records.
    function batchQueryCardDealRecord(
        uint8[] memory indexes,
        uint256 gameId
    ) external view inDealingPhase(gameId) returns (uint8[] memory results) {
        results = new uint8[](indexes.length);
        for (uint8 i = 0; i < indexes.length; ++i) {
            results[i] = queryCardDealRecord(indexes[i], gameId);
        }
        return results;
    }
    ...
    function verifyShuffle(
        uint256 gameId,
        uint8 playerIdx
    ) external view onlyGameContract returns (bool) {
        if (playerIdx == UNREACHABLE_PLAYER_INDEX) {
            return false;
        } else if (decks[gameId][playerIdx].timestamp == 0) {
            return false;
        } else if (
            block.timestamp <
            decks[gameId][playerIdx].timestamp + ALLOWED_PROOF_SUBMISSION_DELAY
        ) {
            for (uint8 i = 0; i < 52; i++) {
                require(
                    CurveBabyJubJub.isValidCoordinateX(
                        decks[gameId][playerIdx].X0[i]
                    ),
                    "Invalid coordinate X"
                );
                require(
                    CurveBabyJubJub.isValidCoordinateX(
                        decks[gameId][playerIdx].X1[i]
                    ),
                    "Invalid coordinate X"
                );
            }
    ...
    function deal(
        ...
        uint256 gameId,
        bool shouldVerifyDeal
    ) internal inDealingPhase(gameId) {
        require(
            CurveBabyJubJub.isOnCurve(decryptedCard[0], decryptedCard[1]),
            "Not on baby jubjub curve"
        );
        require(
        ...
        cardDeals[gameId].record[cardIdx] |= (uint8(1) << curPlayerIdx);
    ...
    function dealBatch(
        ...
        for (uint8 i = 0; i < cardIdx.length; ++i) {
            deal(
                permanentAccount,
                cardIdx[i],
                curPlayerIdx,
    ...
    // Searches the value (between 0 ~ 51) of the `cardIndex`-th card in the `gameId`-th game.
    function search(
        uint8 cardIndex,
        uint256 gameId
    ) public view returns (uint8) {
        if (
            cardDeals[gameId].record[cardIndex] != (1 << numPlayers[gameId]) - 1
        ) {
    ...
    function batchSearch(
        uint8[] calldata cardIndexes,
        uint256 gameId
    ) external view returns (uint8[] memory cards) {
        cards = new uint8[](cardIndexes.length);
        for (uint8 i = 0; i < cardIndexes.length; ++i) {
            cards[i] = search(cardIndexes[i], gameId);
        }
    }

    // Gets data for verifying shuffle proof.
    function getShuffleVerificationData(
        uint256 gameId,
        uint8 playerIdx
    )
        public
        ...
        returns (uint256[8] memory proof, uint256[8] memory publicSignal)
    {
        ...
        uint8 prevPlayerIdx = cardDeals[gameId].prevPlayerIdx[cardIdx][
            playerIdx
        ];
        publicSignal = [
    ...
    function getCardValues(
        ...
        uint256 numCards = cardIndices.length;
        uint8[] memory actualCardValues = new uint8[](numCards);
        for (uint8 i = 0; i < numCards; i++) {
            actualCardValues[i] = search(cardIndices[i], gameId);
            require(
                actualCardValues[i] != INVALID_CARD_INDEX,
    ...
    function getUndealtCardIndices(
        ...
        indices = new uint8[](52);
        numUndealtCards = 0;
        uint8[52] memory record = cardDeals[gameId].record;
        for (uint8 i = 0; i < numCardsToDeal; i++) {
            if (i == 2 * playerIdx || i == 2 * playerIdx + 1) {
                continue;
            }
            if (((record[i] >> playerIdx) & 1) == 0) {
                indices[numUndealtCards] = i;
                numUndealtCards += 1;
            }
        }
    }
