...
contract Exchanger is IExchanger, TypeAndVersionInterface, ConfirmedOwner {
    ...
    function commitTrade(bytes32 commitment) external override {
        ...
        s_commitmentReceived[commitment] = block.number - 1;
    ...
    function resolveTradeWithReport(
        ...
        if (report.upperBlockhash != blockhash(report.blocknumberUpperBound))
            revert BlockhashMismatch(report.upperBlockhash, blockhash(report.blocknumberUpperBound));
