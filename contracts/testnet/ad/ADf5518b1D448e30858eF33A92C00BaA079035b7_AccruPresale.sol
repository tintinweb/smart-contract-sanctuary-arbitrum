...
contract AccruPresale is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    // uint256 private privateSaleMinPurchase = 33333333; 
    // uint256 private publicSale1MinPurchase = 33333333;
    // uint256 private publicSale2MinPurchase = 33333333;
    ...
    function verifyPrivateSalePurchase(
        ...
        uint256 purchasedAmount = privateSaleDeposits[account] / 10 ** 18;
        if(purchasedAmount >= minPurchase && purchasedAmount + amount <= maxPurchase) {
            return true;
        }
        return (amount >= minPurchase && amount <= maxPurchase);
    ...
    function verifyPublicSale1Purchase(
        address account,
        uint256 amount
    ) public view returns (bool isPurchasable) {
        uint256 purchasedAmount = publicSale1Deposits[account] / 10 ** 18;
        if(purchasedAmount >= minPurchase && purchasedAmount + amount <= maxPurchase) {
            return true;
        }
        return (amount >= minPurchase && amount <= maxPurchase);
    ...
    function verifyPublicSale2Purchase(
        ...
        uint256 purchasedAmount = publicSale2Deposits[account] / 10 ** 18;
        if(purchasedAmount >= minPurchase && purchasedAmount + amount <= maxPurchase) {
            return true;
        }
        ...
        returns (bool)
    {
        ...
        address sigRecover = keccak256(abi.encodePacked(_msgSender(), amount))
        ...
        emit TokensBought(_msgSender(), amount, usdPrice, block.timestamp);
        return true;
    }

    /**
     * @dev To buy into a private sale using ETH
     * @param amount No of tokens to buy
     */
        ...
        returns (bool)
    {
        ...
        require(
            verifyPublicSale1Purchase(_msgSender(), ethAmount),
            "Invalid purchased"
        );
        ...
        returns (bool)
    {
        ...
        uint256 excess = msg.value - ethAmount;
        require(
            verifyPublicSale2Purchase(_msgSender(), ethAmount),
            "Invalid purchased"
        );
    ...
    function setSignerWallet(address _signer) external onlyOwner {
     ...
     /**
     * @dev To change mininmum purchase amount of token
     * @param _minAmount mininmum purchase amount
     */
    function setMinPurchase(uint256 _minAmount) external onlyOwner {
        minPurchase = _minAmount;
    }
