...
contract NFTRenderer {
  ...
  constructor(address _vault721, address oracleRelayer, address taxCollector, address collateralJoinFactory) {
    ...
    IODSafeManager safeManager = IODSafeManager(vault721.safeManager());
    _safeEngine = ISAFEEngine(safeManager.safeEngine());
  ...
  function _renderParams(uint256 _safeId) internal view returns (VaultParams memory) {
    ...
    bytes32 cType;
    {
      IODSafeManager.SAFEData memory smData = IODSafeManager(_safeManager).safeData(_safeId);
    ...
    {
      IOracleRelayer.OracleRelayerCollateralParams memory cParams = _oracleRelayer.cParams(cType);
      params.ratio = cParams.safetyCRatio.toString();
    }

    {
      ITaxCollector.TaxCollectorCollateralParams memory cParams = _taxCollector.cParams(cType);
      params.stabilityFee = cParams.stabilityFee.toString();
    }
