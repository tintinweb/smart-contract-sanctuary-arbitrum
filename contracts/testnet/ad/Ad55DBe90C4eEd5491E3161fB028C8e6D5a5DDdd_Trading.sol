...
interface ITrading {
    ...
    function selfClosePosition(bytes32 id) external;
    ...
    // function selfLiquidatePosition(bytes32 id) external;
...
contract Trading is Roles, ERC2771Context, ITrading {
    ...
    bytes32 constant ORACLE = keccak256("ORACLE");
    bytes32 constant GOV = keccak256("GOV");
    ...
    uint256 public executionFee = 3e5; //0.2 USDC
    ...
    function openPositionGasLess(
        ...
        nonces[gasLess.owner]++;
        return _openPosition(gasLess.owner, order, executionFee);
    }
    ...
    function openPositionGasLessWithPermit(
        ...
        return _openPosition(gasLess.owner, order, executionFee + permitFee);
    }
    ...
    function selfClosePosition(bytes32 id) external virtual override {
        ...
        _excutePostion(id, chainlinkPrice, true, 1);
    }
        ...
        override
    {
        ...
        uint16 pairId = positionStorage.getPositionPairId(id);
        uint256 chainlinkPrice = chainlink.getPrice(pairId);
        _excutePostion(id, chainlinkPrice, false, 1);
    }
    ...
    function closePosition(
        ...
        //TODO verify price with chainlink
        require(closeType != 1, "!oracel-cant-close");
        _excutePostion(id, price, false, closeType);
    }

    // function selfLiquidatePosition(bytes32 id) external virtual override {
    //     Types.Position memory position = positionStorage.getPosition(id);
    //     require(position.owner == _msgSender(), "!owner-call");
    //     uint256 chainlinkPrice = chainlink.getPrice(position.pairId);
    //     _excutePostion(
    ...
    function _excutePostion(
        ...
        int256 finalAmount = int256(position.amount) +
            pnl -
            int256(feeClose) -
            (isSelf ? int256(0) : int256(executionFee));
        ...
        pool.creditFee(id, feeClose, isSelf ? 0 : executionFee);
    }
    ...
    // function updateMargin(bytes32 id, uint256 leverage)
    //     external
    //     virtual
    //     override
    // {}

    // function increasePosition(bytes32 id, uint256 price)
    //     external
    //     virtual
    //     override
    // {}

    // function decreasePosition(bytes32 id, uint256 price)
    //     external
    //     virtual
    //     override
    // {}
...
library Verify {
    ...
    bytes32 public constant CLOSE_POSITION_HASH =
        keccak256(
