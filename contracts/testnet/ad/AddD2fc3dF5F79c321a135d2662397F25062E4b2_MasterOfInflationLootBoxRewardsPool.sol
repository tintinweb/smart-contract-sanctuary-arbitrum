...
// Integration with the IMasterOfInflation contract to handle minting of rewards.
// A backup rewards pool to be used if the primary rewards pool is empty or fails to dispense rewards.
// Configuration of sub-pools, each with their own drop rate caps, master of inflation pool, mint amount, and item IDs.
  ...
  ManagerModifier
{
  ...
  address masterOfInflationTokenAddress;
  IRewardsPool backupRewardsPool;
  ...
  function rewardChance(uint64 _subPoolId) external returns (uint256) {
  ...
  function dispenseRewards(
    ...
    if (roll < dropRateCap[_subPoolId]) {
      ...
      mintedRewards = masterOfInflation.tryMintFromPool(
        MintFromPoolParams(
