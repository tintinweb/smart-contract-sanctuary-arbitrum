...
contract CockFight is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_fee <= BASERATE, "Platform fee should be greater less than or equal to 10,000");
    ...
    onlyAgent
    {
        ...
        if (_gameResult == Position.Cancelled || _gameResult == Position.Draw) {
    ...
    onlyDistributer
    {
        require(distributorContract != address(0), "The distributor address is zero address");
        uint256 currentTreasuryAmount = treasuryAmount;
        treasuryAmount = 0;
        tokenAddress.safeTransfer(address(distributorContract), currentTreasuryAmount);
    ...
    function _calculateRewards(uint256 roundID) internal {
        ...
        // White wins
        if (round.gameResult == Position.White) {
            if(round.RedAmount != 0 && round.WhiteAmount != 0)
            {
        ...
        uint256 treasuryRound = round.totalFee;

        emit RewardsCalculated(roundID, rewardAmount, treasuryRound);
    }
