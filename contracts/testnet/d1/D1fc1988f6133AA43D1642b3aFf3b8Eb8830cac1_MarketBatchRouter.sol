...
import "./interfaces/CloberBatchRouter.sol";
import "./Errors.sol";

contract MarketBatchRouter is CloberMarketSwapCallbackReceiver, CloberBatchRouter {
    ...
    function batchLimitOrder(
        LimitOrderParams[] calldata limitAskParamsList,
        LimitOrderParams[] calldata limitBidParamsList
    ) external payable {
        for (uint256 i = 0; i < limitAskParamsList.length; ++i) {
            _limitOrder(limitAskParamsList[i], _ASK);
        }
        for (uint256 i = 0; i < limitBidParamsList.length; ++i) {
            _limitOrder(limitBidParamsList[i], _BID);
        }
    }
    ...
    function batchLimitOrderAfterClaim(
...
interface CloberBatchRouter {
     ...
     * @param limitAskParamsList Array of limitOrderParams: The batch of parameters for the limitAsk.
     * @param limitBidParamsList Array of limitOrderParams: The batch of parameters for the limitBid.
     */
    function batchLimitOrder(
        LimitOrderParams[] calldata limitAskParamsList,
        LimitOrderParams[] calldata limitBidParamsList
