...
import "../lz/token/onft/ONFT721Core.sol";
import "./interfaces/IFarmlandCollectible.sol";

contract ProxyCitizens is ONFT721Core, IERC721Receiver {
    ...
    constructor(uint256 _minGasToTransfer, address _lzEndpoint, address _proxyToken) ONFT721Core(_minGasToTransfer, _lzEndpoint) {
        ...
        token = IFarmlandCollectible(_proxyToken);
    }

    IFarmlandCollectible public immutable token;
    ...
    function crossChain(uint16 dstChainId, uint256 tokenId) public payable {
        if (msg.sender != token.ownerOf(tokenId)) revert NotTokenOwner();

        // Retrieve the stats
        (,uint256 _stamina, uint256 _strength, uint256 _speed, uint256 _courage, uint256 _intelligence) = token.collectibleTraits(tokenId);

        // Remove NFT on current chain
        _debitFrom(msg.sender, dstChainId, abi.encode(msg.sender), tokenId);

        // Encode stats within the payload
        bytes memory payload = abi.encode(msg.sender, tokenId, _stamina, _strength, _speed, _courage, _intelligence);
