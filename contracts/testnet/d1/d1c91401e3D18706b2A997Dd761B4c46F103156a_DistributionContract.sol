...
contract DistributionContract {
    ...
    uint256 public totalUnclaimedRewardsArbot;
    uint256 public totalUnclaimedRewardsEth;
    uint256 public constant ELIGIBILITY_EPOCHS = 1;
    uint256 public constant ELIGIBILITY_PERIOD = 1 days;

    struct Holder {
        uint256 balance;
        uint256 lastClaimEpoch;
        uint256 lastClaimOrPurchaseTime;
        uint256 accumulatedArbot;
        uint256 accumulatedEth;
        uint256 pendingRewardsArbot;
        uint256 pendingRewardsEth;
    }
    ...
    receive() external payable {
        totalDistributionEth += msg.value / 2;
    }

    function distributeRewards() internal {
        payable(teamAddress).transfer((totalDistributionEth * 50) / 100);

        uint256 distributionEth = totalDistributionEth / 2;
        totalUnclaimedRewardsEth += distributionEth;

        distributeRewardsToHolders();
    }

    function distributeRewardsToHolders() internal {
        uint256 totalSupply = token.totalSupply();

        uint256 distributionArbot = token.balanceOf(address(this));
        totalDistributionArbot += distributionArbot;
        totalUnclaimedRewardsArbot += distributionArbot;

        for (uint256 i = 0; i < token.holdersCount(); i++) {
            address holder = token.holderAtIndex(i);
            if (isEligible(holder)) {
                Holder storage holderStruct = holders[holder];
                uint256 holderBalance = holderStruct.balance;

                uint256 holderShareArbot = (totalUnclaimedRewardsArbot * holderBalance) / totalSupply;
                uint256 holderShareEth = (totalUnclaimedRewardsEth * holderBalance) / totalSupply;

                holderStruct.pendingRewardsArbot += holderShareArbot;
                holderStruct.pendingRewardsEth += holderShareEth;

                totalUnclaimedRewardsArbot -= holderShareArbot;
                totalUnclaimedRewardsEth -= holderShareEth;
            }
        }
    }
    ...
    function claimArbot() external {
        ...
        Holder storage holderStruct = holders[msg.sender];

        uint256 availableArbot = holderStruct.pendingRewardsArbot;
        require(availableArbot > 0, "No available ARBOT to claim");

        holderStruct.pendingRewardsArbot = 0;
        holderStruct.lastClaimEpoch = epoch.getCurrentEpoch();
        holderStruct.lastClaimOrPurchaseTime = block.timestamp;
    ...
    function claimEth() external {
        ...
        uint256 availableEth = holderStruct.pendingRewardsEth;
        require(availableEth > 0, "No available ETH to claim");

        holderStruct.pendingRewardsEth = 0;
        holderStruct.lastClaimEpoch = epoch.getCurrentEpoch();
        holderStruct.lastClaimOrPurchaseTime = block.timestamp;
    ...
    function isEligible(address holder) public view returns (bool) {
        ...
        uint256 lastClaimEpoch = holderStruct.lastClaimEpoch;
        uint256 currentEpoch = epoch.getCurrentEpoch();

        if (currentEpoch >= lastClaimEpoch + ELIGIBILITY_EPOCHS) {
            if (block.timestamp >= holderStruct.lastClaimOrPurchaseTime + ELIGIBILITY_PERIOD) {
                return true;
            }
        }
    ...
    function getPendingArbotToClaim(address holder) public view returns (uint256) {
        return holders[holder].pendingRewardsArbot;
    }

    function getPendingEthToClaim(address holder) public view returns (uint256) {
        return holders[holder].pendingRewardsEth;
    }
