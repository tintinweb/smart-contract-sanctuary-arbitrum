...
contract WhitelistPaymaster is BasePaymaster {
    ...
    function verify(bytes calldata _paymasterAndData) external view returns (bool) {

        (,, address target) = abi.decode(_paymasterAndData, (uint48, uint48, address));

        bytes32 _hash = MessageHashUtils.toEthSignedMessageHash(
            keccak256(abi.encode(target))
        );

       bytes memory signature = _paymasterAndData[SIGNATURE_OFFSET:];
       require(ECDSA.recover(_hash, signature) == paymasterOwner, "Invalid signature");
    }
