...
contract L2UnicornEvolutor is BaseGame, BaseL2Unicorn, BaseL2UnicornRule, BaseRecipient, Pausable {
    ...
    function randomizerCallback(uint256 requestId_, bytes32 seed_) external {
        ...
        for (uint256 numberIndex_ = 0; numberIndex_ < evolveTokenIdCount_;) {
            uint256 randomNumber_ = getEvolveRandomNumber(seed_, numberIndex_);
            //根据进化TokenId级别和随机数，取进化到的TokenId
            IL2UnicornRule.HatchRule memory hatchRule = IL2UnicornRule(l2UnicornRule).getHatchRuleByEvolveTokenIdLevelRandomNum(evolveLevelArr_[numberIndex_], randomNumber_);
            uint8 newLevel = hatchRule.level;
            if(hatchRule.startTokenId != 0){
                uint256 newTokenId = IL2Unicorn(l2Unicorn).mintForLevel(user_, newLevel, hatchRule.startTokenId);
                emit EvolveCallback(user_, requestId_, seed_, numberIndex_, randomNumber_, evolveTokenIdArr_[numberIndex_], evolveLevelArr_[numberIndex_], newTokenId, newLevel, l2Unicorn);
            }
     ...
     * @notice 获取进化随机数
     */
    function getEvolveRandomNumber(bytes32 seed_, uint256 numberIndex_) public view returns (uint256){
    ...
    function viewEvolveResults(uint256 requestId_) external view returns (EvolveResult[] memory evolveResults){
        ...
        for (uint8 numberIndex_ = 0; numberIndex_ < length_;) {
            uint256 randomNumber_ = getEvolveRandomNumber(gameInfo_.seed, numberIndex_);
            IL2UnicornRule.HatchRule memory hatchRule = IL2UnicornRule(l2UnicornRule).getHatchRuleByEvolveTokenIdLevelRandomNum(evolveLevelArr_[numberIndex_], randomNumber_);
            //
