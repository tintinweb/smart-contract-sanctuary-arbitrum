...
import "./launch/VestingLaunch.sol";
import "./launch/LaunchTreasury.sol";

contract VestingLaunchFactory is Ownable {
    /* ======== STATE VARIABLS ======== */

    address public immutable MizuRegistry;

    /* ======== CONSTRUCTION ======== */

    constructor(address _registry) {
        require(_registry != address(0));
        MizuRegistry = _registry;
    }

    /* ======== OWNER FUNCTIONS ======== */

    /**
     * @notice deploys vesting token launch and bond contracts and returns address of both
     *     @param _payoutToken address the token paid out from the bond
     *     @param _payinToken address the token used to open a bond position and add liquidity
     *     @param _initialOwner address address of the admin of the bond and treasury
     *     @param _affiliate address need to be whitelisted by Mizu
     *     @param _quoteToken address used to calculate payin/payout ratio (I.E payin > WETH > payout)
     *     @param _router the router to get token prices from, used to calculate bond price
     *     @return _treasury address
     *     @return _bond address
     */
    function createLaunchAndTreasury(
        address _payoutToken,
        address _payinToken,
        address _initialOwner,
        address _affiliate,
        ...
        LaunchTreasury treasury = new LaunchTreasury(_payoutToken, _initialOwner);
        VestingLaunch launch =
        new VestingLaunch(address(treasury), _payoutToken, _payinToken, _initialOwner, MizuRegistry, _affiliate, _quoteToken, _router, _receiver);

        return IRegistry(MizuRegistry).pushBond(
            _payoutToken, _payinToken, address(treasury), address(launch), _initialOwner, address(this)
        );
    }

    /**
     * @notice returns the MizuDao address
     */
    function MizuDao() public view returns (address mizuDAO) {
        mizuDAO = IRegistry(MizuRegistry).MizuDao();
    }

    /**
     * @notice returns the MizuTreasury address used to collect bond fees
     */
    function MizuTreasury() public view returns (address mizuTreasury) {
        mizuTreasury = IRegistry(MizuRegistry).MizuTreasury();
    }
}