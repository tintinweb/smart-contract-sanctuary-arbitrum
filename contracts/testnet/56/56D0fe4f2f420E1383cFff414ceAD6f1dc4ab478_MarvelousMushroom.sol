...
contract MarvelousMushroom is ONFT, IERC2981 {
	...
	constructor(
		...
		uint _startMintId,
		uint _endMintId
	) ONFT("Marvelous Mushroom", "MMRs", _layerZeroEndpoint) {
	...
	function mint(uint _amount) external payable {
		...
		require(minting, "Not start");
		require(!minted[msg.sender], "Already minted");
		require(nextMintId <= maxMintId, "Limit reach");
		require(_amount <= 2, "Max 2 per address");
	...
	function setRoyaltyPercentage(uint royaltyPercentage) external onlyOwner {
...
contract ONFT is IONFT, NonblockingLzApp, ERC721 {
	...
	function _nonblockingLzReceive(
		...
		(bytes memory toAddress, uint tokenId) = abi.decode(
			_payload,
			(bytes, uint)
		);
