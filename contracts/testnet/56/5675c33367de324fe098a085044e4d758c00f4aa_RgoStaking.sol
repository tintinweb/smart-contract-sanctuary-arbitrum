...
import {IRlpRewardDistributor} from "../interfaces/IRlpRewardDistributor.sol";
import {ReentrancyGuardUpgradeable} from "openzeppelin-upgradeable/security/ReentrancyGuardUpgradeable.sol";

contract RgoStaking is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IERC20 public rgo;
    IRlpRewardDistributor public llpRewardDistributor;
    uint256 public lastUpdateRewardTime;
    uint256 public accRewardPerShare;
    mapping(address => UserInfo) public userInfo;

    function initialize(address _rgo, address _llpRewardDistributor) external initializer {
        require(_rgo != address(0), "RgoStaking:initialize: invalid RGO");
        require(_llpRewardDistributor != address(0), "RgoStaking:initialize: invalid reward distributor");
        __Ownable_init();
        __ReentrancyGuard_init();
        rgo = IERC20(_rgo);
        llpRewardDistributor = IRlpRewardDistributor(_llpRewardDistributor);
    }
    ...
    function pendingRewards(address _to) external view returns (uint256) {
        ...
        uint256 _totalStakedAmount = rgo.balanceOf(address(this));
    ...
    function stake(address _to, uint256 _amount) external nonReentrant {
        require(_amount > 0, "RgoStaking::stake: amount > 0");
        ...
        rgo.safeTransferFrom(sender, address(this), _amount);
        emit Staked(sender, _to, _amount);
    }

    function unstake(address _to, uint256 _amount) external nonReentrant {
        require(_amount > 0, "RgoStaking::unstake: amount > 0");
        address sender = msg.sender;
        UserInfo memory _userInfo = userInfo[sender];
        require(_userInfo.amount >= _amount, "RgoStaking::unstake: unstake amount > staked amount");
        updateRewards();
        _userInfo.amount -= _amount;
        _userInfo.rewardDebt -= int256((_amount * accRewardPerShare) / ACC_REWARD_PRECISION);
        userInfo[sender] = _userInfo;
        rgo.safeTransfer(_to, _amount);
    ...
    function claimRewardsToSingleToken(address _to, address _rewardTokenOut, uint256 _minAmountOut)
        ...
        nonReentrant
    {
        require(_rewardTokenOut != address(0), "RgoStaking::claimRewards: Invalid address!");
        updateRewards();
        address sender = msg.sender;
        UserInfo memory user = userInfo[sender];
        int256 _accumulatedReward = int256((user.amount * accRewardPerShare) / ACC_REWARD_PRECISION);
    ...
    function setRlpRewardDistributor(address _llpRewardDistributor) external onlyOwner {
        require(_llpRewardDistributor != address(0), "RgoStaking::setRlpRewardDistributor: Invalid address");
        llpRewardDistributor = IRlpRewardDistributor(_llpRewardDistributor);
        emit RlpRewardDistributorSet(_llpRewardDistributor);
    }
    ...
    event RlpRewardDistributorSet(address indexed _llpRewardDistributor);
}
...
interface IRlpRewardDistributor {
    function withdrawableTokens(address _token) external view returns (bool);
