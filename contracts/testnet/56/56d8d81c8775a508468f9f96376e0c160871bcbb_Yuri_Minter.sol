...
contract Yuri_Minter is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    event Change(address indexed user, uint cardId);
    ...
    function bond(address invitor) external {
        ...
        //        require(userInfo[invitor].invitor != address(0), 'invitor not bond');
        ...
        //invitor refer amount
        userInfo[msg.sender].invitor = invitor;
        userInfo[invitor].referList.push(msg.sender);
        emit Bond(invitor, msg.sender);
    }

    function mintYuri(string memory uri) external {
        //        require(USDT.balanceOf(msg.sender) >= mintPrice, 'not enough usdt');
        if (!_checkUserAmount(msg.sender)) {
            require(userInfo[msg.sender].mintAmount == 0, 'out of limit');
        }
    ...
    function changeToGS(uint cardId) external {
        //        require(yuri.ownerOf(cardId) == msg.sender, 'not owner');
        yuri.safeTransferFrom(msg.sender, address(this), cardId);
        uint id = rand(9);
        igs.mint(msg.sender, id);
        emit Change(msg.sender, id);
    }

    function changeToSBT(uint cardId, uint times, bytes32 r, bytes32 s, uint8 v) external {
    ...
    function _checkUserAmount(address addr) internal view returns (bool){
        if (userInfo[addr].referAmount >= 3) {
            return true;
        }
        return false;
    }
