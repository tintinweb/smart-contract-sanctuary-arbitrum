...
interface GNSStakingInterfaceV6_5 {
    // Structs
    struct Staker {
    ...
    function distributeReward(uint _amountToken, address rewardToken) external;
...
contract GNSTradingCallbacksV6_5 is Initializable {
    ...
    GNSStakingInterfaceV6_5 public staking;
    ...
    GNSBorrowingFeesInterfaceV6_5 public borrowingFees;
    ...
    event GovFeeCharged(address indexed trader, uint valueDai);
    ...
    // TODO: incrementing initialize version in order to have all chains/contracts at same version
    function initialize(
        StorageInterfaceV5 _storageT,
        GNSPairInfosInterfaceV6 _pairInfos,
        GNSReferralsInterfaceV6_2 _referrals,
        GNSStakingInterfaceV6_5 _staking,
    ...
    function initializeV2(GNSBorrowingFeesInterfaceV6_5 _borrowingFees) external reinitializer(2) {
    ...
    function claimGovFees() external onlyGov {
        uint amountDai = govFeesDai;
        govFeesDai = 0;
        transferFromStorageToAddress(storageT.gov(), amountDai);
    }
    ...
    function openTradeMarketCallback(AggregatorAnswer memory a) external onlyPriceAggregator notDone {
        ...
        if (cancelReason == CancelReason.NONE) {
            ...
            uint govFees = handleGovFees(t.trader, t.pairIndex, t.positionSizeDai * t.leverage);
            transferFromStorageToAddress(t.trader, t.positionSizeDai - govFees);
    ...
    function closeTradeMarketCallback(AggregatorAnswer memory a) external onlyPriceAggregator notDone {
        ...
        if (cancelReason != CancelReason.NO_TRADE) {
            ...
            if (cancelReason == CancelReason.NONE) {
                ...
                // Gov fee to pay for oracle cost
                uint govFee = handleGovFees(t.trader, t.pairIndex, v.levPosDai);
    ...
    function executeNftCloseOrderCallback(AggregatorAnswer memory a) external onlyPriceAggregator notDone {
        ...
        if (cancelReason == CancelReason.NONE) {
            ...
            if (cancelReason == CancelReason.NONE) {
                ...
                handleOracleRewards(triggeredLimitId, t.trader, (v.reward1 * 2) / 10, v.tokenPriceDai);
    ...
    function registerTrade(
        ...
        uint govFee = handleGovFees(trade.trader, trade.pairIndex, (v.posDai > 0 ? v.posDai : v.levPosDai));
        ...
        if (nftId < 1500) {
            ...
            sendToVault(v.reward3, trade.trader);
        ...
        distributeStakingReward(trade.trader, v.reward1 + v.reward2 - v.reward3);
    ...
    function unregisterTrade(
        ...
        // 6. Take DAI from vault if winning trade
    ...
    function _getBorrowingFeeAdjustedPercentProfit(
        ...
        borrowingFee = borrowingFees.getTradeBorrowingFee(
            GNSBorrowingFeesInterfaceV6_5.BorrowingFeeInput(
                trade.trader,
                trade.pairIndex,
                trade.index,
                trade.buy,
    ...
    function marketExecutionPrice(uint price, uint spreadP, bool long) private pure returns (uint) {
    ...
    function _openTradePrep(
        ...
        (priceImpactP, priceAfterImpact) = pairInfos.getTradePriceImpact(
            marketExecutionPrice(c.executionPrice, c.spreadP, c.buy),
            c.pairIndex,
            c.buy,
    ...
    function distributeStakingReward(address trader, uint amountDai) private {
        ...
        staking.distributeReward(amountDai, address(storageT.dai()));
    ...
    function handleGovFees(address trader, uint pairIndex, uint leveragedPositionSize) private returns (uint govFee) {
        govFee = (leveragedPositionSize * storageT.priceAggregator().openFeeP(pairIndex)) / PRECISION / 100;
        govFeesDai += govFee;

        emit GovFeeCharged(trader, govFee);
    }
