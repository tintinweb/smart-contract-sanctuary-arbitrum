...
contract ToroPool is Initializable, ERC20Upgradeable, ReentrancyGuard, IToroPool {
  ...
  /// @notice Track how much funds were sent FROM `ToroPool` TO `Market`s
  uint private _credits;

  /// @notice Track how much funds were sent TO `ToroPool` FROM `Market`s
  ...
  modifier onlyMarket() {
    require(_toroAdmin.hasRole(_toroAdmin.MARKET_ROLE(), msg.sender), "only market");
    _;
  }
  ...
  /// maximum payout. This is an access-controlled function - only the `Market`
  /// contracts may call this function
  function _transferToMarket(address market, uint amount) external onlyMarket {
    ...
    // Transfer the funds to the `Market`
    _currency.safeTransfer(market, amount);
  }

  /// @notice Accounting function to increase the amount credited to `ToroPool`
  /// i.e., How much is owed TO `ToroPool` FROM `Market`s
  /// @param amount Amount to increase `_credits`
  function _incrementCredits(uint amount) external onlyMarket {
    _credits += amount;
  }
  ...
  /// i.e., How much is owed TO `ToroPool` FROM `Market`s
  /// @param amount Amount to decrease `_credits`
  function _decrementCredits(uint amount) external onlyMarket {
    _credits -= amount;
  }
  ...
  /// i.e., How much is owed FROM `ToroPool` TO `Market`s
  /// @param amount Amount to increase `_debits`
  function _incrementDebits(uint amount) external onlyMarket {
    _debits += amount;
  }
  ...
  /// i.e., How much is owed FROM `ToroPool` TO `Market`s
  /// @param amount Amount to decrease `_debits`
  function _decrementDebits(uint amount) external onlyMarket {
    _debits -= amount;
  }
  ...
  /// @notice Conversion from underlying tokens to LP tokens, taking into
  /// account the net debt of every open `Market`
  /// @param amount Amount of underlying tokens
  /// @return uint Amount of LP tokens
  ...
  function _getNetBalance() internal view returns(uint) {
    ...
    // net debt to each open/pending `Market`
