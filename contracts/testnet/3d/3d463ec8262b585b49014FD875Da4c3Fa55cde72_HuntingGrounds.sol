...
contract HuntingGrounds is Initializable, HuntingGroundsSettings {
    ...
    function _claimBugz(uint256 _tokenId, address _to) private {
        ...
        tokenIdToTotalBugzClaimed[_tokenId] += _bugzAmount;

        _mintTrophiesIfNeeded(_to, _tokenId);
    ...
    function _mintTrophiesIfNeeded(address _to, uint256 _tokenId) private {
        for(uint256 i = 0; i < bugzTrophyAmounts.length; i++) {
            uint256 _bugzTrophyAmount = bugzTrophyAmounts[i];

            if(tokenIdToTotalBugzClaimed[_tokenId] >= _bugzTrophyAmount) {
                require(bugzAmountToTrophyId[_bugzTrophyAmount] > 0, "HuntingGrounds: Trophy id not set");

                trophiez.mintIfNeeded(_to, bugzAmountToTrophyId[_bugzTrophyAmount]);
            }
        }
    }
}
...
abstract contract HuntingGroundsContracts is Initializable, HuntingGroundsState {
    ...
    function setContracts(
        ...
        address _trophiezAddress)
    external
    onlyAdminOrOwner
    {
        world = IWorld(_worldAddress);
        bugz = IBugz(_bugzAddress);
        trophiez = ITrophiez(_trophiezAddress);
    }
...
abstract contract HuntingGroundsSettings is Initializable, HuntingGroundsContracts {
    ...
    function setBugzTrophyAmounts(
        uint256[] calldata _trophyAmounts,
        uint256[] calldata _trophyIds)
    external
    nonZeroLength(_trophyIds)
    onlyAdminOrOwner
    {
        require(_trophyAmounts.length == _trophyIds.length, "HuntingGrounds: Bad trophy array lengths");

        delete bugzTrophyAmounts;

        bugzTrophyAmounts = _trophyAmounts;

        for(uint256 i = 0; i < _trophyIds.length; i++) {
            require(_trophyIds[i] > 0, "HuntingGrounds: Bad trophy ids");
            bugzAmountToTrophyId[_trophyAmounts[i]] = _trophyIds[i];
        }
    }
}
...
abstract contract HuntingGroundsState is Initializable, IHuntingGrounds, AdminableUpgradeable {
    ...
    mapping(uint256 => uint256) tokenIdToTotalBugzClaimed;

    uint256 public bugzAmountPerDay;

    ITrophiez public trophiez;

    uint256[] public bugzTrophyAmounts;
    mapping(uint256 => uint256) public bugzAmountToTrophyId;

    function __HuntingGroundsState_init() internal initializer {
        AdminableUpgradeable.__Adminable_init();

        bugzAmountPerDay = 100 ether;

        bugzTrophyAmounts = [100 ether, 3000 ether, 36500 ether];
        bugzAmountToTrophyId[100 ether] = 2;
        bugzAmountToTrophyId[3000 ether] = 3;
        bugzAmountToTrophyId[36500 ether] = 4;
    }
}
...
interface ITrophiez is IERC1155Upgradeable {
