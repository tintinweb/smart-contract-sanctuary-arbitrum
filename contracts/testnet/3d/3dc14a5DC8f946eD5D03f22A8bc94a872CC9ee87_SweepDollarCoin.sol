...
contract SweepDollarCoin is BaseSweep {
    ...
    /**
     * @notice Get Maximum Sweep Amount
     * @return uint256 Sweep amount
     */
    function margin_sweep_amount() external view returns (uint256) {
        (uint256 sweep_amount, uint256 usdx_amount) = uniV3TWAPOracle.getLiquidity();
        uint256 usdx_sweep_amount = (usdx_amount * 10 ** decimals()) / target_price();
        
        return usdx_sweep_amount > sweep_amount ? usdx_sweep_amount - sweep_amount : 0;
    }
    ...
    function minter_mint(
        ...
        if (!is_minting_allowed(_amount)) revert MintNotAllowed();
