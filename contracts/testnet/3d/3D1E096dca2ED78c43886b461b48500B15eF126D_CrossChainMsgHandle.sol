...
contract CrossChainMsgHandle is Ownable, ICrossChainMsgSend {
    using ECDSA for bytes32;
    uint256 public required;
    uint256 public fee;
    string public version = "1.0";
    string public name = "CrossChainMsgHandle";
    bytes32 public DOMAIN_SEPARATOR;
    address[] public signers;

    mapping(uint16 => mapping(string => uint256)) public nonces;
    mapping(address => uint) public indexes;
    mapping(address => bool) public authorized;
    mapping(uint256 => mapping(string => mapping(address => bool)))
        public signed;
    mapping(uint256 => mapping(string => bool)) public txCompleted;

    constructor(address[] memory signers_, uint256 required_, uint256 _fee) {
        required = required_;

        for (uint i = 0; i < signers_.length; i++) {
            address _addr = signers_[i];
    ...
    event InvalidSigner(address signer);
    ...
    function send(
        ...
        emit SendMsg(msg.sender, _chainId, _toContract, _payload, msg.value);
    }

    function receiveMsg(
        uint16 _chainId,
        address _toContract,
        string calldata _fromHash,
        ...
        require(!txCompleted[_chainId][_fromHash], "tx already completed");
        require(
            r.length == v.length &&
                r.length == s.length &&
                r.length >= required,
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encode(
                        keccak256(
                            "ReceiveMsg(string fromHash,uint16 fromChainId,uint256 nonce,bytes fromContract,bytes payload)"
                        ),
                        keccak256(bytes(_fromHash)),
                        _chainId,
                        nonces[_chainId][_fromHash]++,
                        keccak256(_fromContract),
                        keccak256(_payload)
                    )
                )
            )
        ...
        for (uint256 i = 0; i < v.length; i++) {
            ...
            if (authorized[signer] && !signed[_chainId][_fromHash][signer]) {
                signed[_chainId][_fromHash][signer] = true;
                confirmations++;
                if (confirmations >= required) {
                    txCompleted[_chainId][_fromHash] = true;
                    ICrossChainMsgReceive(_toContract).receiveMsg(
                        _chainId,
                        _fromContract,
                        _payload
                    );
    ...
    function getNonce(
        ...
        return nonces[_chainId][_fromHash];
    }
