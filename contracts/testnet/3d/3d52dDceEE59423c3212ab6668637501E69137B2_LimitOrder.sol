...
import {Module, OrderType, ModuleData, TccItemV2, TcdItem, TaskConfig, SwapEventItem, OrderEventItem, CreateItem, ExactInputParams} from "./StructList.sol";
...
contract LimitOrder is OrderBase {
    ...
    function approveDex(IERC20 _token, address _routerAddress, uint256 _spendAmount) internal {
        if (_token.allowance(address(this), _routerAddress) < _spendAmount) {
            _token.approve(_routerAddress, approveAmount);
        }
    }
    ...
    function _dov3swap(
        ...
        return MultiDexRouter(V3Dex).swapExactTokensForToken(
            ExactInputParams({
                factory: factory,
                tokenList: tokenList,
                feeList: feeList,
    ...
    function _dov2swap(
        ...
        if (_eth) {
            return MultiDexRouter(multiDexRouterAddress).swapExactTokensForETHSupportingFeeOnTransferTokens(
                _routerAddressList,
                _amountIn,
                0,
                _path,
            ...
            return MultiDexRouter(multiDexRouterAddress).swapExactTokensForTokensSupportingFeeOnTransferTokens(
                _routerAddressList,
                _amountIn,
                0,
                _path,
    ...
    function _takeDevFee(address _user) internal {
        if (devToken != address(0) && devFee > 0) {
            require(userInfoList[_user].devAmount >= devFee, "L14");
    ...
    function limitOrder(address _user, bytes32 _md5) external nonReentrant {
        ...
        require(y.tcd._status && !y.tcd._completed, "L09");
        ...
        if (!y.tcc.isV3) {
            approveDex(IERC20(x.swapInToken), multiDexRouterAddress, y.tcd._maxSpendTokenAmount);
        } else {
            approveDex(IERC20(x.swapInToken), V3Dex, y.tcd._maxSpendTokenAmount);
        }
        x._in = y.tcd._maxSpendTokenAmount;
        if (x.swapOutToken == weth) {
            if (!y.tcc.isV3) {
                x.swapOutAmount = _dov2swap(true, y.tcc._swapPathConfig1._routerAddressListV2, y.tcd._maxSpendTokenAmount, y.tcc._swapPathConfig1._swapPathList, _user);
            } else {
                x.swapOutAmount = _dov3swap(y.tcc._swapPathConfig1._factoryV3[0], y.tcc._swapPathConfig1._swapPathList, y.tcc._swapPathConfig1._feeListV3, _user, y.tcd._maxSpendTokenAmount, false);
            }
        } else {
            if (!y.tcc.isV3) {
                x.swapOutAmount = _dov2swap(false, y.tcc._swapPathConfig1._routerAddressListV2, y.tcd._maxSpendTokenAmount, y.tcc._swapPathConfig1._swapPathList, _user);
            } else {
                x.swapOutAmount = _dov3swap(y.tcc._swapPathConfig1._factoryV3[0], y.tcc._swapPathConfig1._swapPathList, y.tcc._swapPathConfig1._feeListV3, _user, y.tcd._maxSpendTokenAmount, true);
            }
        }
        //验证成交价格是否合理
        require(x.swapOutAmount.mul(10 ** 18) > y.tcc._swapPathConfig1._swapPriceZone[0].mul(x._in) && x.swapOutAmount.mul(10 ** 18) < y.tcc._swapPathConfig1._swapPriceZone[1].mul(x._in), "L11");
        (x._fee, x._feeToken) = _getFeeDetails();
        x._poolFee = x._fee.mul(swapRate).div(swapAllRate);
        x._totalFee = x._fee.add(x._poolFee);
        require(x._totalFee <= y.tcc._maxFeePerTx, "L12");
        require(userInfoList[_user].ethAmount >= x._totalFee, "L13");
