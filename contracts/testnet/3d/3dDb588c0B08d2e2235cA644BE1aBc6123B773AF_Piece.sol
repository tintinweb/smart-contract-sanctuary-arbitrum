...
contract Piece is ERC1155LazyMint, PermissionsEnumerable {
    ...
    function burnSoulsGetPieces() public payable virtual nonReentrant {
        ...
        uint256 quantity = 0;
        uint256 sword1Balance = sword.balanceOf(_receiver, 1);
        uint256 sword2Balance = sword.balanceOf(_receiver, 2);
        uint256 sword3Balance = sword.balanceOf(_receiver, 3);
        uint256 sword4Balance = sword.balanceOf(_receiver, 4);
        uint256 bonus = 1;

        require(
                (sword1Balance > 0 || sword2Balance > 0 || sword3Balance > 0 || sword4Balance > 0),
        ...
        uint256 soulsBalance = souls.balanceOf(_receiver, 0);
        require(
                soulsBalance > 0,
        ...
        souls.burn(_receiver, 0, soulsBalance);

        super._transferTokensOnClaim(_receiver, 0, quantity);
        emit TokensClaimed(msg.sender, _receiver, 0, quantity);
    }
