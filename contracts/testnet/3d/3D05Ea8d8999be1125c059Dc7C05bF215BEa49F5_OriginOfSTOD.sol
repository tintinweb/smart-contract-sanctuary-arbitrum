...
interface IOriginOfSTOD is IERC1155 {
...
interface IOriginOfSTODPrinter is IERC721 {}
...
import { IOriginOfSTOD } from "./interfaces/IOriginOfSTOD.sol";
import { IOriginOfSTODPrinter } from "./interfaces/IOriginOfSTODPrinter.sol";

error OriginOfSTOD_ExceedsMaxPageNumber();
error OriginOfSTOD_InsufficientValue();
error OriginOfSTOD_InvalidConfiguration();
error OriginOfSTOD_InvalidSendData();
error OriginOfSTOD_MintClosed();
error OriginOfSTOD_NotAllowed();
error OriginOfSTOD_TurnClosed();
error OriginOfSTOD_ValueOutOfRange();
error OriginOfSTOD_WithdrawFailed();

contract OriginOfSTOD is ONFT1155Core, ERC1155, IONFT1155, IOriginOfSTOD {
    uint256 public immutable MAX_PAGE_NUMBER;
    uint256 public immutable TOTAL_CHAINS;
    uint256 public immutable CHAIN_OFFSET;

    IOriginOfSTODPrinter public pagePrinter;
    ...
    {
        if (_chainOffset >= _totalChains) revert OriginOfSTOD_InvalidConfiguration();
    ...
    modifier onlyPrinter() {
        if (_msgSender() != address(pagePrinter)) revert OriginOfSTOD_NotAllowed();
        _;
    }
    ...
    function pageNumber(uint256 _tokenId) public view returns (uint256) {
        if (_tokenId >= (MAX_PAGE_NUMBER * TOTAL_CHAINS) + TOTAL_CHAINS) revert OriginOfSTOD_ValueOutOfRange();
    ...
    function tokenId(uint256 _page) public view returns (uint256) {
        if (_page > MAX_PAGE_NUMBER) revert OriginOfSTOD_ValueOutOfRange();
     ...
     */
    function setPagePrinter(IOriginOfSTODPrinter _pagePrinter) public payable onlyOwner {
    ...
    function setCurrentMaxPage(uint256 _currentMaxPage) public payable onlyOwner {
        if (_currentMaxPage > MAX_PAGE_NUMBER) revert OriginOfSTOD_ExceedsMaxPageNumber();
    ...
    function mint(uint256 _amount) public payable {
        if (mintClosed) revert OriginOfSTOD_MintClosed();
        if (_amount * price != msg.value) revert OriginOfSTOD_InsufficientValue();
    ...
    function _mint(address _to, uint256 _page, uint256 _amount) internal {
        if (_page > MAX_PAGE_NUMBER) revert OriginOfSTOD_ExceedsMaxPageNumber();
    ...
    function turnPage(uint256 _tokenId, uint256 _amount) public payable {
        if (turnClosed) revert OriginOfSTOD_TurnClosed();

        uint256 page = pageNumber(_tokenId);

        if (page + 1 > currentMaxPage) revert OriginOfSTOD_ExceedsMaxPageNumber();
    ...
    {
        if (turnClosed) revert OriginOfSTOD_TurnClosed();
        if (pageNumber(_tokenId) + 1 > currentMaxPage) revert OriginOfSTOD_ExceedsMaxPageNumber();
        ...
        payable
    {
        ...
        for (uint256 i; i < _tokenIds.length;) {
            if (pageNumber(_tokenIds[i]) + 1 > currentMaxPage) revert OriginOfSTOD_ExceedsMaxPageNumber();
    ...
    function _debitFrom(
        ...
        override
    {
        if (_msgSender() != _from && !isApprovedForAll(_from, _msgSender())) revert OriginOfSTOD_NotAllowed();
        ...
        internal
    {
        if (_tokenIds.length == 0 || _tokenIds.length != _amounts.length)
            revert OriginOfSTOD_InvalidSendData();
    ...
    function withdraw(address _receiver) public payable onlyOwner {
        ...
        if (!success) revert OriginOfSTOD_WithdrawFailed();
    }
}