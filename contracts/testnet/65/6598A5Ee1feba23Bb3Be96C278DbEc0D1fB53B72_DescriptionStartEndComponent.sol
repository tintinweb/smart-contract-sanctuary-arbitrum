...
uint256 constant ID = uint256(
    keccak256("game.piratenation.descriptionstartendcomponent")
);
...
library DescriptionStartEndComponentStorage {
...
/**
 * @title DescriptionStartEndComponent
 * @dev A description for things that have a beginning and an end.
 */
contract DescriptionStartEndComponent is BaseStorageComponentV2 {
    /** SETUP **/
     ...
     *
     * @param entity Entity to get value for
     * @param start The starting description
     * @param end The ending description
     */
    function setValue(
        uint256 entity,
        ...
        _setValue(entity, Layout(start, end));
    }
    ...
    function getLayoutValue(
        ...
        value = DescriptionStartEndComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return start The starting description
     * @return end The ending description
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (string memory start, string memory end) {
        if (has(entity)) {
            Layout memory s = DescriptionStartEndComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (start, end) = abi.decode(_getEncodedValues(s), (string, string));
        }
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = DescriptionStartEndComponentStorage
            .layout()
            .entityIdToStruct[entity];

        // ABI Encode all fields of the struct and add to values array
        values = new bytes[](2);
        values[0] = abi.encode(s.start);
        values[1] = abi.encode(s.end);
    }
    ...
    function setBytes(
        ...
        Layout memory s = DescriptionStartEndComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.start, s.end) = abi.decode(value, (string, string));
    ...
    function remove(
        ...
        delete DescriptionStartEndComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete DescriptionStartEndComponentStorage
                .layout()
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.start, value.end));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.start, value.end);
    }
}