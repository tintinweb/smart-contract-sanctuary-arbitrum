...
import "./IProductionCapacity.sol";
import "../Utils/EpochConfigurable.sol";

contract ProductionCapacity is IProductionCapacity, Pausable, EpochConfigurable {
  error InsufficientCapacity(uint realmId, uint spentCapacity, uint currentCapacity);

  // Realm ID -> timestamp
  mapping(uint => uint) public LAST_USED_TIMESTAMP;
  // Realm ID -> capacity left
  mapping(uint => uint) public LAST_PRODUCTION_REMAINING_CAPACITY;

  uint256 public PRODUCTION_CAPACITY_EPOCHS_CAP;

  constructor(address _manager) EpochConfigurable(_manager, 24 hours, 0) {
    PRODUCTION_CAPACITY_EPOCHS_CAP = 6;
  }

  function productionCapacity(uint _realmId) external view returns (uint) {
    return _productionCapacity(_realmId);
  }
  ...
  function productionCapacityBatch(
    ...
    for (uint i = 0; i < _realmIds.length; i++) {
      result[i] = _productionCapacity(_realmIds[i]);
    }
  }

  function spendProductionCapacity(uint _realmId, uint _spentCapacity) external onlyManager {
    _spendCapacity(_realmId, _spentCapacity);
  }
  ...
  function spendProductionCapacityBatch(
    ...
    for (uint i = 0; i < _realmIds.length; i++) {
      _spendCapacity(_realmIds[i], _spentCapacity[i]);
    }
  }

  function _spendCapacity(uint _realmId, uint _spentCapacity) internal {
    uint currentCapacity = _productionCapacity(_realmId);
    if (currentCapacity < _spentCapacity) {
      revert InsufficientCapacity(_realmId, _spentCapacity, currentCapacity);
    }

    LAST_USED_TIMESTAMP[_realmId] = block.timestamp;
    LAST_PRODUCTION_REMAINING_CAPACITY[_realmId] = currentCapacity - _spentCapacity;
  }

  function _productionCapacity(uint _realmId) internal view returns (uint capacity) {
    uint lastTimestamp = LAST_USED_TIMESTAMP[_realmId];
    if (block.timestamp == lastTimestamp) {
      return 0;
    }

    uint currentEpoch = currentEpoch();
    uint elapsedEpochs = currentEpoch - epochAtTimestamp(lastTimestamp);
    capacity =
      LAST_PRODUCTION_REMAINING_CAPACITY[_realmId] +
      ((ONE_HUNDRED * elapsedEpochs) / PRODUCTION_CAPACITY_EPOCHS_CAP);
    if (capacity > ONE_HUNDRED) {
      capacity = ONE_HUNDRED;
    }
  }

  function updateEpochCap(uint256 epochCap) external onlyAdmin {
    PRODUCTION_CAPACITY_EPOCHS_CAP = epochCap;
  }
}
