...
interface IPosition {
    ...
    event CopyPosition(bytes32 id, bytes32 cid);
    ...
    function closePosition(
        ...
        int256 pnl,
        uint16 closeType
    ) external returns (Master memory);
...
contract Position is Roles, IPosition {
    ...
    function addPositionWithMaster(
        ...
        bytes32 id = _addPosition(position);
        masterInfo[id] = master;
        emit CopyPosition(id, cid);
        return id;
    }
    ...
    function addPosition(Types.Position calldata position)
        ...
        returns (bytes32)
    {
        return _addPosition(position);
    }

    function _addPosition(Types.Position calldata position)
        internal
        returns (bytes32)
    {
    ...
    function closePosition(
        ...
        int256 pnl,
        uint16 closeType
    ) external override hasRole(TRADING) returns (Master memory) {
