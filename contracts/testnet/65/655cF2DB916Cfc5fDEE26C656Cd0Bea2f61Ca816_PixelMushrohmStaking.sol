...
contract PixelMushrohmStaking is IPixelMushrohmStaking, PixelMushrohmAccessControlled {
    ...
    function levelUp(uint256 _tokenId) external override onlyPixelMushrohmOwner(_tokenId) {
        ...
        require(levelEarned(_tokenId) >= pixelMushrohm.getLevelCost(), "PixelMushrohm: Not eligible for a level up");
    ...
    function sporePowerEarned(uint256 _tokenId) public view override returns (uint256 sporePower) {
        ...
        sporePower = ((sporePowerPerWeek.mul(timeDelta).mul(pixelMushrohm.getTotalMultiplier(_tokenId))).div(1e9)).div(
            WEEK
        );
    }
    ...
    function levelEarned(uint256 _tokenId) public view override returns (uint256 level) {
        ...
        level = (levelPerWeek().mul(timeDelta).div(WEEK)).add(stakedTokenData[_tokenId].lastLevel);
    }

    function levelPerWeek() public view override returns (uint256) {
        require(address(pixelMushrohm) != address(0), "PixelMushrohm: Invalid pixelMushrohm address");
        return sporePowerPerWeek.div(pixelMushrohm.getMaxSporePowerLevel());
    }
