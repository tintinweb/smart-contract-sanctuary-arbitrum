...
// for performace, calculate yieid and claim from off-chain
contract ProfitShare is ReentrancyGuard {
    using SafeMath for uint256;
    address public lpToken;
    uint256 public totalUser;
    uint256 public devidend_rate_percent = 50;
    ...
    constructor(address wl, address _lpToken) {
        totalUser = 0;
        whitelists[wl] = true;
        lpToken = _lpToken;
    }
    ...
    function stake(uint256 amount) public nonReentrant {
        ...
        if (userAddressToID[msg.sender] == 0) {
            ...
            idToUserAddress[totalUser] = msg.sender;
            userFund[msg.sender] = amount;
        } else {
            userFund[msg.sender] = userFund[msg.sender].add(amount);
        }
        require(
        ...
        emit Stake(
            ...
            amount,
            userFund[msg.sender],
            userAddressToID[msg.sender]
        );
    }

    function unstake() public nonReentrant {
        require(userFund[msg.sender] > 0, "r1");
        uint256 amount = userFund[msg.sender];
        userFund[msg.sender] = 0;
        require(IERC20(lpToken).transfer(address(msg.sender), amount), "r2");
