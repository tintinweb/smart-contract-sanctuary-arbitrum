...
abstract contract IHelixFund {
  ...
  function cancelInvestment(address account, uint256 nonce) public virtual;

  function directDividendPayment(
    address issuer,
  ...
  function directDividendPaymentWithPermit(
    address issuer,
    address[] calldata investors,
    uint256[] calldata amounts,
    // USDC permit condition
  ...
  function indirectDividendPayment(
    bytes32 root,
    address issuer,
    uint256 totalAmount,
    uint256 uuid
  ...
  function indirectDividendPaymentWithPermit(
    bytes32 root,
    address issuer,
    uint256 totalAmount,
    uint256 deadline,
  ...
  function approveInterest(
    uint256 subscriptionDate,
    uint256 amount,
    address account
  ) public virtual;

  function toggleFund(bool status) public virtual;
}
...
//   }
  ...
  function getFundImplementationRepository(
    IHelixConfig config
  ) internal view returns (ImplementationRepository) {
    return
      ImplementationRepository(
        config.getAddress(uint256(HelixConfigOptions.Addresses.FundImplementationRepository))
      );
  }
...
//   }
  ...
  function getToggleIssuerPay(IHelixConfig config) internal view returns (bool) {
    return config.getBoolean(uint256(HelixConfigOptions.Booleanes.ToggleIssuerPay));
  }

  function getToggleInvestorsClaim(IHelixConfig config) internal view returns (bool) {
    return config.getBoolean(uint256(HelixConfigOptions.Booleanes.ToggleInvestorsClaim));
  }
}
...
contract HelixBorrower is BaseUpgradeablePausable, IHelixBorrower {
    IHelixConfig public config;
    using HelixConfigHelper for IHelixConfig;
    
    uint256 public nonces;

    function hasRole(bytes32 role, address account) public view virtual override(AccessControlUpgradeable, IHelixBorrower) returns (bool){
        return AccessControlUpgradeable.hasRole(role, account);
    }

    /// @notice Initialize a new borrower proxy contract
    /// @param owner Owner of borrower proxy contract
        ...
        initializer
    {
        ...
        address protocolAdmin = IHelixConfig(_config).protocolAdminAddress();
        
        __BaseUpgradeablePausable__init(owner);
        __BaseUpgradeablePausable__init(protocolAdmin);

        // IERC20withDec usdc = config.getUSDC();
        // usdc.approve(oneInch, type(uint256).max);
    }

    /// @notice Borrower cancel a loan
    /// @param poolAddress Pool's address
    function cancel(address poolAddress) external onlyAdmin {
        IHelixTranchedPool(poolAddress).cancel();
    }

    /// @notice Borrower close all investment actions, allows borrower to drawdown USDC
    /// @param poolAddress Pool's address
    function close(address poolAddress) external onlyAdmin {
        IHelixTranchedPool(poolAddress).close();
    }    

    /// @notice Borrower lock a loan
    /// @param poolAddress Pool's address
    function lockPool(address poolAddress) external onlyAdmin {
        IHelixTranchedPool(poolAddress).lockPool();
    }

    /// @notice Set hash of loan's documents
    /// @param poolAddress Pool's address
    /// @param fileHash The hash of loan's documents
    function setFileHash(address poolAddress, bytes32 fileHash) external onlyAdmin {
        require(fileHash != bytes32(0), "File's hash is not valid");
        IHelixTranchedPool pool = IHelixTranchedPool(poolAddress);
        pool.setFileHash(fileHash);
    }
    ...
    function drawdown(
        ...
        uint256 amountAvailable = IHelixMultiTranchedPool(poolAddress).drawdown();

        if (addressToSendTo == address(0) || addressToSendTo == address(this)) {
            addressToSendTo = msg.sender;
        }

        transferERC20(config.usdcAddress(), addressToSendTo, amountAvailable);
    }
     ...
     */
    function pay(address poolAddress, uint256 amount) public onlyAdmin {
        IERC20withDec usdc = config.getUSDC();
     ...
     */
    function payWithPermit(address poolAddress, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public onlyAdmin {
        IERC20PermitUpgradeable(config.usdcAddress()).permit(msg.sender, address(this), amount, deadline, v, r, s);
        pay(poolAddress, amount);
    }

    /**
     * @notice Allows a borrower to pay back loans by calling the `pay` function directly on many TranchedPools
     * @param pools An array of credit lines to be paid back
     * @param amounts An array of amounts, in USDC atomic units, that the borrower wishes to pay
     */
     ...
     * @param s s of signature
     */
    function payMultipleWithPermit(address[] calldata pools, uint256[] calldata amounts, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public onlyAdmin {
        uint256 totalAmount;
        for (uint256 i = 0; i < amounts.length; i++) {
            totalAmount = totalAmount + amounts[i];
        }
        IERC20PermitUpgradeable(config.usdcAddress()).permit(msg.sender, address(this), totalAmount, deadline, v, r, s);
        payMultiple(pools, amounts);
    }

    /**
     * @notice Allows a borrower to pay back loans by calling the `pay` function directly on a TranchedPool and check if they pay full
     * @param poolAddress The credit line to be paid back
     * @param amount The amount, in USDC atomic units, that the borrower wishes to pay
     */
    function payInFull(address poolAddress, uint256 amount) external onlyAdmin {
        ...
        _transferAndPay(usdc, poolAddress, amount);
        require(
            IHelixMultiTranchedPool(poolAddress).creditLine().balance() == 0,
    ...
    function _transferAndPay(
        ...
        IHelixMultiTranchedPool pool = IHelixMultiTranchedPool(poolAddress);
    ...
    /**
     * @notice Allows a borrower to drawdown on their credit line through a UniTranchedPool.
     * @param poolAddress The creditline from which they would like to drawdown
     * @param addressToSendTo The address where they would like the funds sent. If the zero address is passed,
     *  it will be defaulted to the contracts address (msg.sender). This is a convenience feature for when they would
     *  like the funds sent to an exchange or alternate wallet, different from the authentication address
     * @param signature The signature of admin to allow borrower to drawdown
     */
    function drawdown(
        address poolAddress,
        address addressToSendTo,
        bytes calldata signature
    ) external onlyAdmin allowDrawdownByOnlySigner(poolAddress, signature) {
        incrementNonce();
        uint256 amountAvailable = IHelixUniTranchedPool(poolAddress).drawdown();
    ...
    /**
     * @notice Allows a borrower to pay back loans by calling the `pay` function directly on a UniTranchedPool
     * @param poolAddress The credit line to be paid back
     * @param interestPayment The amount of interest, in USDC atomic units, that the borrower wishes to pay
     * @param principalPayment The amount of principal, in USDC atomic units, that the borrower wishes to pay
     * @param expiresAt The deadline of signature
     * @param signature The signature of system's admin
     */
    function pay(
        address poolAddress, 
        ...
        checkOnlySigner(msg.sender, poolAddress, interestPayment, principalPayment, expiresAt, signature);
        incrementNonce();
        _pay(poolAddress, interestPayment, principalPayment);
    }

    /**
     * @notice Allows a borrower to pay back loans by calling the `pay` function directly on a UniTranchedPool without executing approve transaction
     * @param poolAddress The credit line to be paid back
     * @param interestPayment The amount of interest, in USDC atomic units, that the borrower wishes to pay
     * @param principalPayment The amount of principal, in USDC atomic units, that the borrower wishes to pay
     * @param expiresAt The deadline of pay signature
     * @param paySignature The signature of system's admin
     * @param deadline The deadline of permit signature
     * @param v v of permit signature
     * @param r r of permit signature
     * @param s s of permit signature
     */
    function payWithPermit(
        ...
        checkOnlySigner(msg.sender, poolAddress, interestPayment, principalPayment, expiresAt, paySignature);
        incrementNonce();
        IERC20PermitUpgradeable(config.usdcAddress()).permit(msg.sender, address(this), interestPayment + principalPayment, deadline, v, r, s);
        _pay(poolAddress, interestPayment, principalPayment);
    }
    ...
    function _pay(
        ...
        IERC20withDec usdc = config.getUSDC();
        bool success = usdc.transferFrom(msg.sender, address(this), interestPayment + principalPayment);
        require(success, "Failed to transfer USDC");
        _transferAndPay(usdc, poolAddress, interestPayment, principalPayment);
    }
    ...
    function _transferAndPay(
        ...
        IHelixUniTranchedPool pool = IHelixUniTranchedPool(poolAddress);
        // We don't use transferFrom since it would require a separate approval per creditline
        bool success = usdc.transfer(address(pool.creditLine()), interestPayment + principalPayment);
        require(success, "USDC Transfer to creditline failed");
        pool.assess(interestPayment, principalPayment);
    }

    /**
     * @notice Check if system's admin signed to allow borrower pay for their loan
     * @param account Address of a borrower who want to pay for their loan
     * @param poolAddress The credit line to be paid back
     * @param interestPayment The amount of interest, in USDC atomic units, that the borrower wishes to pay
     * @param principalPayment The amount of principal, in USDC atomic units, that the borrower wishes to pay
     * @param expiresAt The deadline of pay signature
    ...
    function checkOnlySigner(
        ...
        bytes32 hash = keccak256(abi.encodePacked(account, poolAddress, interestPayment, principalPayment, expiresAt, address(this), nonces, block.chainid));
        bytes32 ethSignedMessage = ECDSAUpgradeable.toEthSignedMessageHash(hash);
        require(hasRole(OWNER_ROLE, ECDSAUpgradeable.recover(ethSignedMessage, signature)), "Invalid signer");
    }
    ...
    modifier allowDrawdownByOnlySigner(
        address poolAddress,
        bytes calldata signature
    ) {
        bytes32 hash = keccak256(abi.encodePacked(poolAddress, address(this), nonces, block.chainid));
        bytes32 ethSignedMessage = ECDSAUpgradeable.toEthSignedMessageHash(hash);
        require(hasRole(OWNER_ROLE, ECDSAUpgradeable.recover(ethSignedMessage, signature)), "Invalid signer");
        _;
    }
    ...
    /// @notice Issuer approve someone to invest in the fund by creating a subscription directly in the contract
    /// @param fund The fund contract's address
    /// @param subscriptionDate The subscription date of the investment
    /// @param amount The amount that investor will need to deposit
    /// @param account The address of account, which is allowed to invest
    function approveInterest(IHelixFund fund, uint256 subscriptionDate, uint256 amount, address account) external onlyAdmin {
        fund.approveInterest(subscriptionDate, amount, account);
    }

    /// @notice Issuer cancel an investment of an investor
    /// @param fund The fund contract's address
    /// @param account Investor's address
    /// @param nonce Index of investor's investment
    function cancelInvestment(IHelixFund fund, address account, uint256 nonce) external onlyAdmin{
        fund.cancelInvestment(account, nonce);
    }

    /// @notice Issuer directly pay dividend to investors of the fund
    /// @param fund The fund contract's address
    /// @param investors Array of fund's investors
    /// @param amounts Array of token amounts, that each investor will be received
    function directDividendPayment(
        IHelixFund fund,
        address[] calldata investors,
        uint256[] calldata amounts,
        ...
        fund.directDividendPayment(msg.sender, investors, amounts, uuid);
    }

    /// @notice Issuer directly pay dividend to investors of the fund without executing approve transaction
    /// @param fund The fund contract's address
    /// @param investors Array of fund's investors
    /// @param amounts Array of token amounts, that each investor will be received
    /// @param deadline The deadline of signature
    ...
    function directDividendPaymentWithPermit(
        ...
        fund.directDividendPaymentWithPermit(msg.sender, investors, amounts, deadline, v, r, s, uuid);
    }

    /// @notice Issuer indirectly pay dividend to investors of the fund
    /// @param fund The fund contract's address
    /// @param root The merkle's root in database
    /// @param totalAmount Total amount of token, that all investors will be received
    function indirectDividendPayment(
        IHelixFund fund,
        bytes32 root,
        uint256 totalAmount,
        ...
        fund.indirectDividendPayment(root, msg.sender, totalAmount, uuid);
    }

    /// @notice Issuer indirectly pay dividend to investors of the fund without executing approve transaction
    /// @param fund The fund contract's address
    /// @param root The merkle's root in database
    /// @param totalAmount Total amount of token, that all investors will be received
    /// @param deadline The deadline of signature
    ...
    function indirectDividendPaymentWithPermit(
        ...
        fund.indirectDividendPaymentWithPermit(root, msg.sender, totalAmount, deadline, v, r, s, uuid);
    }

    /// @notice Open or close fund
    /// @param fund The fund contract's address
    /// @param status True if issuer want close fund and vice versa
    function toggleFund(
        IHelixFund fund,
        bool status
    ) public onlyAdmin {
        fund.toggleFund(status);
    }
}
