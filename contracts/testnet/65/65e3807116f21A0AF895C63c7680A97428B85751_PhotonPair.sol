...
contract PhotonPair is ReentrancyGuard, NoDelegateCall {
	...
	/// @dev Get next initialized tick
	function getNextInitializedTick(int24 tick) external view returns (int24, bool) {
		(int24 nextInitializedTick, bool initialized) = tickBitmap.nextInitializedTickWithinOneWord(tick);
		return (nextInitializedTick, initialized);
	}
	...
	function getExactTokenYForTokenX(uint256 tokenXAmount) external view returns (uint256 tokenYAmountRequired) {
		...
		while (tokenXAmount > 0) {
			...
			if (tokenXAmount > 0) {
				(currTick, initialized) = tickBitmap.nextInitializedTickWithinOneWord(currTick);
			}
		}
	}
