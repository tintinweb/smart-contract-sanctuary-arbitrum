...
contract RoboTrustDeployer {
    ...
    function createTrust(uint256 _period, uint _maxPayouts, uint[] memory _paymentAmounts, uint _annuityPV, address _trustee, address _beneficiary, bool _showAccounting, string memory _disclaimer) public payable {
        //require(keccak256(abi.encodePacked(disclaimer)) == keccak256(abi.encodePacked(_disclaimer)), "You must sign our disclaimer");
        RoboTrust newTrust = (new RoboTrust){value: msg.value}(msg.sender, _trustee, _beneficiary, _period, _maxPayouts, _paymentAmounts, _annuityPV, priceFeedAddress, _showAccounting);
...
contract FinancialMathHelper {
    ...
    function getPaymentEthAmount(uint _usdAmountDegree18) public returns (Payment memory) {
