    ...
    UUPSUpgradeable
{
     ...
     * @param rabbitId token id of rider nft
     */
     ...
     * @param rabbitId token Id of rider nft which is transferred
     ...
     * @param rabbitId token id of nft that is received on dst chain
     */
    ...
    /**  
        ...
        * @param rabbitId this it tokenId of nft from which user participated in lottery
    */
    ...
    struct RiderMetadata {
        uint256 score;
        uint256 noOfRides; // total number of rides
    }

    struct RidingMetadata {
        uint256 rideNonce;
        uint256 rabbitId;
    }

    struct ChainRiderMetadata {
        bool riding; // if you left the chain, it means you are still riding wrt this chain
        uint256 lastRideTime;
        uint256 noOfRides; // no of rides to this chain
        uint256 lastRideIdx; //last ride index
    }
    ...
    uint256 public version; //cool down time b/w each cross chain transfer in sec
    uint64 public coolDownTime; //cool down time b/w each cross chain transfer in sec
    string public chainId; // current chain id
    uint64[5] public chainTravelThresholds; //this is threshold on number of visited to any chain
    uint64[6] public ccTravelRangeScore; //this is scoring array based on chain travel threshold
    mapping(uint256 => RiderMetadata) public riderMetadata; //maps token id to it's metadata
    mapping(uint256 => RidingMetadata) public ridingMetadata; // eventId -> id [riding]
    mapping(uint256 => ChainRiderMetadata) public chainRiderMetadata; // eventId -> id [riding]
    IGateway gateway; // gateway instance
    mapping(string => DestDetails) public remoteContractMp;
     ...
     * @param rabbitId token id
     */
    function getRiderMetadata(
        uint256 rabbitId
    ) public view returns (RiderMetadata memory) {
        return riderMetadata[rabbitId];
    }

    function setGateway(address _gatewayAddress) public onlyOwner {
        gatewayAddress = _gatewayAddress;
        gateway = IGateway(gatewayAddress);
    }

    function setTravelThresholds(
    ...
    function setCooldown(uint64 _coolDownTime) public onlyOwner {
        coolDownTime = _coolDownTime;
    }
    ...
    function enrollRouterRiderContract(
        ...
        if (_fee != 0) remoteContractMp[_chainId].fee = _fee;
        if (
            keccak256(abi.encodePacked(_contractAddress)) !=
            keccak256(abi.encodePacked(""))
        ) remoteContractMp[_chainId].entryPoint = _contractAddress;
    }
    ...
    function setChainColor(string memory _chainColor) public onlyOwner {
        chainColor = _chainColor;
    }
    ...
    function _safeMint(
        ...
        _safeMint(to, rabbitId);
        _setTokenURI(rabbitId, uri);
    }
    ...
    function _transfer(
        ...
        require(!chainRiderMetadata[tokenId].riding, "Rabbit Hopping");
    ...
    {
        string memory tokenUri = super.tokenURI(_rabbitId);
     ...
     */
    function _scoreOnChain(uint256 _rabbitId) internal view returns (uint256) {
        uint256 numTravels = chainRiderMetadata[_rabbitId].noOfRides;
        uint256 left = 0;
        ...
        return ccTravelRangeScore[left];
    }
     ...
     * @param _rabbitId  token Id of rider NFT that need to be transferred.
    ...
    function rideCrossChain(
        ...
        DestDetails memory destDetails = remoteContractMp[_dstChainId];
        require(
            keccak256(abi.encodePacked(destDetails.entryPoint)) !=
                keccak256(abi.encodePacked("")),
            "Invalid dst chain"
        );
        require(destDetails.fee <= msg.value, "fee > msg.value");

        require(_ownerOf(_rabbitId) == msg.sender, "RP: Caller Not Owner");
        require(!chainRiderMetadata[_rabbitId].riding, "Already Riding");
        require(
            chainRiderMetadata[_rabbitId].lastRideTime + coolDownTime <
                block.timestamp,
            "try again after cooldown"
        );
        chainRiderMetadata[_rabbitId].riding = true;
        ...
        bytes memory payload = abi.encode(
            _rabbitId,
            super.tokenURI(_rabbitId),
            riderMetadata[_rabbitId],
            receiver,
            block.timestamp
        );
        ...
        uint256 eventId = gateway.iSend(
            ...
            _dstChainId,
            _requestMetadata,
            abi.encode(destDetails.entryPoint, payload)
        );

        ridingMetadata[eventId].rabbitId = _rabbitId;
        ridingMetadata[eventId].rideNonce = block.timestamp;
    ...
    function submitTicket(
        ...
        require(
            remoteContractMp[lotteryChainId].fee <= msg.value,
            "fee > msg.value"
        );
        require(ownerOf(_rabbitId) == msg.sender, "Caller Not Owner Of Rabit");
        require(!chainRiderMetadata[_rabbitId].riding, "Rabbit Hopping");
        require(
            uint8(_requestMetadata[48]) == 0 &&
                keccak256(_requestMetadata[50:]) ==
                keccak256(abi.encodePacked("")),
            "invalid asm or ack"
        );
        bytes memory payload = abi.encode(
            _rabbitId,
            toBytes(msg.sender),
            riderMetadata[_rabbitId].score,
            _lotteryId,
            block.timestamp
        );
        uint256 requestIdentifier = gateway.iSend(
            1,
            0, //routeAmount
            "0x", //routeRecipient
            lotteryChainId,
    ...
    function iReceive(
        ...
        require(
            keccak256(abi.encodePacked(requestSender)) ==
                keccak256(
                    abi.encodePacked(remoteContractMp[srcChainId].entryPoint)
                ),
        ...
        (
            ...
            RiderMetadata memory _rabbitMetadata,
        ...
        riderMetadata[rabbitId] = _rabbitMetadata;
        RiderMetadata storage _riderMetadata = riderMetadata[rabbitId];
        uint256 scoreFrom = _riderMetadata.score;

        // taken this travel also for score calculation
        chainRiderMetadata[rabbitId].noOfRides += 1;
        chainRiderMetadata[rabbitId].riding = false;

        _riderMetadata.noOfRides += 1;
        uint256 score = _scoreOnChain(rabbitId);
        if (
            riderMetadata[rabbitId].noOfRides -
                chainRiderMetadata[rabbitId].lastRideIdx <
            5 &&
            chainRiderMetadata[rabbitId].lastRideIdx > 0
        ) {
            score /= 2;
        }

        _riderMetadata.score += score;
        chainRiderMetadata[rabbitId].lastRideTime = block.timestamp;
        // add this chain to last_five_chains
        chainRiderMetadata[rabbitId].lastRideIdx = _riderMetadata.noOfRides;

        // minting on dst chain to receiver
        _safeMint(receiver, rabbitId, tokenUri);
    ...
    function iAck(
        ...
        uint256 rabbitId = ridingMetadata[eventIdentifier].rabbitId;
        uint256 rideNonce = ridingMetadata[eventIdentifier].rideNonce;
        delete ridingMetadata[eventIdentifier];
        if (!execFlag) {
            // if failed on dst chain
            chainRiderMetadata[rabbitId].riding = false;
            emit CrossChainTransferFailed(eventIdentifier, rabbitId, rideNonce);
            return;
        }

        _burn(rabbitId);
        emit CrossChainReceived(eventIdentifier, rideNonce, rabbitId);
    }
