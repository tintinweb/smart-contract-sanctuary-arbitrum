...
contract OrderBook is ReentrancyGuard, IOrderBook {
    ...
    function getPrivateStatus() external view returns (bool) {
    ...
    function createSwapOrder(
        ...
        bool _shouldWrap,
        bool _shouldUnwrap
    ) external payable nonReentrant onlyPrivate {
        // require(!inPrivateMode,"OrderBook: action not enabled");
    ...
    function getUsdxMinPrice(address _otherToken) public view returns (uint256) {
        ...
        uint256 redemptionAmount = IVault(vault).getRedemptionAmount(_otherToken, USDX_PRECISION);
    ...
    function _vaultSwap(address _tokenIn, address _tokenOut, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        if (_tokenOut == usdx) { // buyUSDX
            amountOut = IVault(vault).buyUSDX(_tokenIn, _receiver);
        } else if (_tokenIn == usdx) { // sellUSDX
