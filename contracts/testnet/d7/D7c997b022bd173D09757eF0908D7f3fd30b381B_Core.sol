...
contract Core is ICore, ACLUser, Initializable, ReentrancyGuard, EmergencyModifier {
  ...
  function withdraw(string memory _lender, bytes calldata _txData, uint256 _loanId) external emergencyStop {
    ...
    emit CollateralWithdrawn(decodeData.market, decodeData.token, decodeData.amount, decodeData.to, _lender, _loanId);
  }
...
interface ICore {
  ...
  event CollateralWithdrawn(address market, address token, uint256 amount, address to, string lenderName, uint256 loanId);
...
interface IPositionToken is IERC721Metadata {
  ...
  event DepositIncreased(uint256 tokenId, uint256 amount, uint256 yeild_);
