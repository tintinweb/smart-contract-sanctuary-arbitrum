...
uint256 constant ID = uint256(keccak256("game.piratenation.traitcollectioncomponent.dev2"));
 ...
 * @dev Generated with component version 1.
 */
contract TraitCollectionComponent is BaseStorageComponent {
    /** SETUP **/
    ...
    function append(
        ...
        emitSetBytes(
            entity,
            abi.encode( s.componentIds, s.isAttribute)
        );
    }
   ...
   function removeValueAtIndexes(
       ...
       for (uint256 i = 0; i < indexes.length; i++) {
           ...
           if (indexToRemove != lastIndexInArray) {
               s.componentIds[indexToRemove] =  s.componentIds[lastIndexInArray];
    ...
    function batchSetValue(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            _setValueToStorage(entities[i], values[i]);
            encodedValues[i] = abi.encode(
                values[i].componentIds, values[i].isAttribute
            );
        }
    ...
    function getValue(
        ...
        // Get the struct from storage
        value = TraitCollectionComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getBytes(
        ...
        Layout memory s = TraitCollectionComponentStorage.layout().entityIdToStruct[entity];
        value = abi.encode(s.componentIds, s.isAttribute);
    }
    ...
    function setBytes(
        ...
        Layout memory s = TraitCollectionComponentStorage.layout().entityIdToStruct[entity];
        (s.componentIds, s.isAttribute) = abi.decode(value, (uint256[], bool[]));
        this.setValue(entity, s);
    }
    ...
    function remove(uint256 entity) public virtual {
        // Remove the entity from the component
        delete TraitCollectionComponentStorage.layout().entityIdToStruct[entity];
        emitRemoveBytes(entity);
    }
    ...
    function _setValueToStorage(
        ...
        s.componentIds = transform.componentIds;
        s.isAttribute = transform.isAttribute;
    }
}
...
uint256 constant ID = uint256(keccak256("game.piratenation.traitmetadatacomponent.dev2"));
 ...
 * @dev Generated with component version 1.
 */
contract TraitMetadataComponent is BaseStorageComponent {
    /** SETUP **/
    ...
    function setValue(
        ...
        emitSetBytes(
            entity,
            abi.encode(value.displayName, value.traitName)
        );
    }
    ...
    function batchSetValue(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            _setValueToStorage(entities[i], values[i]);
            encodedValues[i] = abi.encode(
                values[i].displayName, values[i].traitName
            );
        }
    ...
    function getBytes(
        ...
        Layout memory s = TraitMetadataComponentStorage.layout().entityIdToStruct[entity];
        value = abi.encode(s.displayName, s.traitName);
    }
    ...
    function _setValueToStorage(
        ...
        s.displayName = transform.displayName;
        s.traitName = transform.traitName;
    }
}
    ...
    ITokenURIHandler
{
    ...
    function tokenURI(
        ...
        ).getValue(EntityLibrary.addressToEntity(tokenContract));
        ...
        for (uint256 idx; idx < collection.componentIds.length; idx++) {
            ...
            ).getValue(collection.componentIds[idx]);
...
import {EntityLibrary} from "../../core/EntityLibrary.sol";
import {ImageUrlTraitComponent, ID as IMAGE_URL_TRAIT_COMPONENT_ID} from "../../generated/components/ImageUrlTraitComponent.sol";
        ...
        extraTraits[2] = TokenURITrait({
            ...
            value: abi.encode(
                ImageUrlTraitComponent(
                    _gameRegistry.getComponent(IMAGE_URL_TRAIT_COMPONENT_ID)
