...
contract MarkPriceOracle is AccessControlUpgradeable {
    ...
    function getCustomMarkTwapCopy(
        uint256 _index,
        uint256 _startTimestamp,
        uint256 _endTimestamp
    ) external view returns (uint256 priceCumulative, uint256 count, uint256 start, uint256 end) {
        (priceCumulative,,count,start,end) = _getCustomTwap(_index, _startTimestamp, _endTimestamp, true);
    }
    ...
    ) internal view returns (uint256 priceCumulative, uint256 lastUpdatedTimestamp, uint256 priceCount, uint256 start, uint256 end) {
        MarkPriceObservation[] memory observations = observationsByIndex[_index];
