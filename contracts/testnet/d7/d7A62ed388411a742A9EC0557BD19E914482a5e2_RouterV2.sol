 ...
 */
contract RouterV2 is AccessControl, IRouterV2 {
    ...
    constructor(address _publisher, address _accountRegistrar) {
    ...
    function createMarket(
        ...
        uint256 marketTradesLimit,
        uint256 marketStartTimeStamp,
        uint256 tradingDuration,
        uint256 waitTimeDuration,
        address targetContract
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        IMarketsContract gameContract = IMarketsContract(targetContract);
        gameContract.setMarketData(
            marketId,
            minTradeAmount,
            maxTradeAmount,
            marketTradesLimit,
        ...
        )
    {
        (
            ...
            tradingEndTimestamp,
            roundCloseTimestamp
        ) = IMarketsContract(targetContract).getRoundData(marketId);
    }
    ...
    function openTrades(OpenTxn[] calldata params) external {
        ...
        for (uint32 index = 0; index < params.length; index++) {
            ...
            ERC20 tokenX = IMarketsContract(currentParams.targetContract)
                .tokenX();
            if (tokenX.balanceOf(user) < currentParams.amount) {
                emit FailResolve(
            ...
            // if (
            //     tokenX.allowance(user, address(this)) < currentParams.amount &&
    ...
    function execute(
        ...
        uint256 batchSize,
        address targetContract,
        bytes memory startPricePublisherSignature,
        bytes memory endPricePublisherSignature,
        uint256 roundId
    ) public {
        _validateKeeper();
        IMarketsContract gameContract = IMarketsContract(targetContract);
        _verifyPrice(
            ...
            endPriceTimestamp,
            endPrice,
            startPricePublisherSignature,
            endPricePublisherSignature
        );
    ...
    function _verifyTrade(
        ...
        (address signer, ) = getAccountMapping(params.trader);
    ...
    function _openTrade(TradeParams memory params) internal {
        IMarketsContract gameContract = IMarketsContract(params.targetContract);
        IERC20(gameContract.tokenX()).transferFrom(
            params.trader,
            params.targetContract,
            params.amount
        );
        gameContract.placeTrade(
            params.marketId,
            params.amount,
            params.avatarUrl,
            params.countryCode,
    ...
    function _verifyPrice(
        ...
        bytes32 startPriceHashData = keccak256(
            abi.encodePacked(marketId, startPrice, startPriceTimestamp)
        );
        bytes32 endPriceHashData = keccak256(
            abi.encodePacked(marketId, endPrice, endPriceTimestamp)
        );
        require(
            _validate(
                startPriceHashData,
            ...
            "Router: Start price signature didn't match"
        );
        require(
            _validate(endPriceHashData, endPricePublisherSignature, publisher),
            "Router: Start price signature didn't match"
        );
    }
}
...
interface IMarketsContract {
    ...
    struct Round {
        ...
        uint256 tradingDuration;
        uint256 waitTimeDuration;
        TradeGroup upTradeGroup;
        TradeGroup downTradeGroup;
    }
