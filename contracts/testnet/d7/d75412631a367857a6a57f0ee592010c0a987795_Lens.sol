...
contract Pool is IPool, Initializable {
     ...
     * staking process.
     * This struc is used to reduce the number of state declarations in the contract, due to Solidity limitations on
    ...
    function _stake(address _staker, uint256 _amount) internal {
        ...
        if (balances[msg.sender] + _amount > maxStakePerAddress) revert Error.MaxStakePerAddressExceeded();
        if (totalSupply + _amount > maxStakePerPool) revert Error.MaxStakePerPoolExceeded();

        if (balances[msg.sender] == 0) stakersCount++;
