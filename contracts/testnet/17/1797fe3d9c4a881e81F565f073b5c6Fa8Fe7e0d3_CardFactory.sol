...
import {CardPieces} from './CardPieces.sol';

contract CardFactory is AccessControl, Ownable2Step {
  ...
  // Mapping from cardPieces to award
  mapping(address => Award) public awards;
  /// singer address
  mapping(address => bool) public signers;

  event UpdateSigner(address signer, bool status, address operator);
  event Redeemed(address cardPieces, address nftAddress, uint256 nftTokenId, bool redeemed);
  ...
  function deploy(
    address _targetNFTAddr,
    uint96 _targetTokenid,
    string memory _name,
    ...
    require(_targetNFTAddr != address(0), 'Invalid address');
    uint256 _target = (uint256(uint160(_targetNFTAddr)) << 96) | _targetTokenid;
    address _cardPieces = address(new CardPieces{salt: bytes32(_target)}(_name, _symbol, _baseURI, _maxSupply));
    awards[_cardPieces] = Award(_targetTokenid, _targetNFTAddr, false);

    CardPieces(_cardPieces).transferOwnership(_newOwner);
  }

  function batchMint(
    address[] calldata _cardPieces,
    uint256[] calldata _amounts,
    uint256[] calldata _randomWords,
    uint256 _deadline,
    bytes calldata _signature
  ) external {
    require(_cardPieces.length == _amounts.length, 'Invalid length');
    bool success = _verifySignature(msg.sender, _cardPieces, _amounts, _randomWords, _deadline, block.chainid, _signature);
    require(success, 'Invalid Signature');

    uint256 numIndex = 0;
    for (uint256 i = 0; i < _cardPieces.length; i++) {
      Award storage _award = awards[_cardPieces[i]];
      require(!_award.redeemed, 'Already redeemed');

      for (uint256 j = 0; j < _amounts[i]; j++) {
        CardPieces(_cardPieces[i]).mint(msg.sender, _randomWords[numIndex]);
        numIndex++;
      }
    }
  }

  function redeem(address _cardPieces) external onlyRole(OPERATOR_ROLE) {
    Award storage _award = awards[_cardPieces];
    require(!_award.redeemed, 'Already redeemed');
    _award.redeemed = true;

    emit Redeemed(_cardPieces, _award.nftAddress, _award.nftTokenId, _award.redeemed);
  }
  ...
  function _verifySignature(
    ...
    bytes32 ethSignedMessageHash = keccak256(
      abi.encodePacked(
        ...
        keccak256(abi.encodePacked(_to, _cardPieces, _amounts, _randomWords, _deadline, chainId, address(this)))
      )
    );
...
contract CardPieces is ERC721, AccessControl, Ownable2Step {
  bytes32 public constant OPERATOR_ROLE = keccak256('OPERATOR_ROLE');
  ...
  // Mapping from token ID to lottery
  ...
  event LotteryInfo(uint256 tokenId, uint64 timestamp, uint192 randomWords);

  constructor(string memory name_, string memory symbol_, string memory baseURI_, uint16 maxSupply_) ERC721(name_, symbol_) {
  ...
  function mint(address _to, uint256 _randomWords) external onlyRole(OPERATOR_ROLE) {
    uint256 _tokenId = _useRandomAvailableTokenId();
    _safeMint(_to, _tokenId);

    lotteries[_tokenId] = Lottery(uint64(block.timestamp), uint192(_randomWords));

    emit LotteryInfo(_tokenId, uint64(block.timestamp), uint192(_randomWords));
  }
  ...
  function _useRandomAvailableTokenId() internal returns (uint256) {
    ...
    require(numAvailableRemainingTokens > 0, 'No more tokenIds available');
  ...
  function _getRandomNum(uint256 numAvailableRemainingTokens) internal view returns (uint256) {
    return uint256(keccak256(abi.encode(block.timestamp, blockhash(block.number - 1), address(this), numAvailableRemainingTokens)));
  }
  ...
  function destruct(address payable _to) external onlyOwner {
    selfdestruct(_to);
  }
}