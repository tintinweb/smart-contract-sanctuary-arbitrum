...
contract SynthrBridge is BaseBridge {
    ...
    function sendREAppend(address account, uint amount) external payable {
        _sendREAppend(account, amount);
        emit REAppend(account, amount, _selfLZChainId);

        bytes memory lzPayload = abi.encode(PT_REWARD_ESCROW_APPEND, abi.encodePacked(account), amount);
    ...
    function _sendREAppend(
        ...
        ISynthrAggregator(synthrAggregator).append(account, amount);
    }
