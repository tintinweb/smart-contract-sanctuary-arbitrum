...
function withdrawTokenB(uint256 amount) public {
    require(isOwner(), "Only owner can withdraw Token B.");
    require(getTokenB().balanceOf(address(this)) >= amount, "Insufficient balance of Token B.");
    getTokenB().transfer(msg.sender, amount);
}
