...
import "../Adventurer/IAovTranscendenceTimer.sol";
...
contract Transcendence is ManagerModifier, ReentrancyGuard, Pausable {
  ...
  function transcend(
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      for (uint16 rewardIndex = 0;
        rewardIndex < rewardTranscendenceLevelCaps.length && temp > rewardTranscendenceLevelCaps[rewardIndex];
        rewardIndex++) {
      ...
      if (rewardTokenId > 0) {
        LOOT_BOX_DISPENSER.dispense(msg.sender, rewardTokenIds[rewardTokenId], 1);
      }
