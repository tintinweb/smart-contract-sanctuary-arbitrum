    ...
    IActivePool
{
    ...
    function balanceOf(address _asset) external view returns (uint256) {
        ...
        require(_assetVault != address(0), "ActivePool: Vault not set");
        ...
        callerIsBorrowerOpsOrStabilityPoolOrVesselMgrOrVesselMgrOps
    {
        ...
        if (_balance < _amount)
            ISmartVault(_assetVault).withdraw(_amount - _balance);
        ...
        if (_balance != 0) _depositERC20(_asset, _balance);
    ...
    function _depositERC20(address _asset, uint256 _amount) internal {
        ...
        IERC20Upgradeable(_asset).safeApprove(_assetVault, _amount);

        ISmartVault(_assetVault).deposit(_amount);
        emit CollateralDepositedIntoSmartVault(_asset, _amount, _assetVault);
    }
