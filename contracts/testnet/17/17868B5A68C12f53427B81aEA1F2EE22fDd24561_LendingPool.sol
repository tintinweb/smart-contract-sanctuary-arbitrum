...
interface INatriumIncentivesController {
...
interface IInitializableAToken {
   ...
   * @param treasury The address of the Natrium treasury, receiving the fees on this aToken
...
interface IAToken is IERC20, IScaledBalanceToken, IInitializableAToken {
   ...
   **/
  function getIncentivesController() external view returns (INatriumIncentivesController);
...
interface INatriumAPRDataProvider {
...
library ValidationLogic {
  ...
  function validateBorrow(
    ...
    //Natrium: place where stETH is blocked from being borrowed
 ...
 * @dev Main point of interaction with an Natrium protocol's market
 * - Users can:
 *   # Deposit
 *   # Withdraw
 *   # Borrow
...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  function depositYield(address asset, uint256 amount) external override whenNotPaused {
    ...
    if (aprDataProvider != address(0)) {
      INatriumAPRDataProvider(aprDataProvider).updateAPR(asset, amount, totalAmount);
    }
