...
contract PixelMushrohmERC721 is IPixelMushrohmERC721, ERC721Enumerable, PixelMushrohmAccessControlled, ReentrancyGuard {
    ...
    uint256 constant WHITELIST_MAX_MINT = 2031;
    ...
    function maxOutSporePower(uint256 _tokenId) external override onlyOwner {
        ...
        uint256 _newSporePower = _sporePowerCost.mul(_maxSporePowerLevel);
    ...
    function airdrop(address[] calldata _to, uint256[] calldata _amount) external override onlyOwner {
        require(
        ...
        for (uint256 i = 0; i < _to.length; i++) {
            for (uint256 j = 0; j < _amount[i]; j++) {
                _mint(_to[i], 0, MintType.APELIEN_AIRDROP);
            }
        }
    }
    ...
    {
        require(_amount > 0, "PixelMushrohm: Amount must be greater than 0");
        require(!staking.isStaked(_tokenId), "PixelMushrohm: Must not be staked");
        require(
    ...
    function _mint(
        ...
        if (_mintType == MintType.BRIDGE) {
            _maxMint = BRIDGE_MAX_MINT;
        } else if (_mintType == MintType.APELIEN_AIRDROP) {
            _tokenId = _apelienTokenIdTracker.current().add(BRIDGE_MAX_MINT).add(1);
            _apelienTokenIdTracker.increment();
            _maxMint = BRIDGE_MAX_MINT.add(APELIEN_MAX_MINT);
        } else {
            _tokenId = _standardTokenIdTracker.current().add(BRIDGE_MAX_MINT).add(APELIEN_MAX_MINT).add(1);
            _standardTokenIdTracker.increment();
            _maxMint = BRIDGE_MAX_MINT.add(APELIEN_MAX_MINT).add(FIRST_GEN_LEVEL_MAX_MINT).add(WHITELIST_MAX_MINT);
        }
    ...
    function getLevelMultiplier(uint256 _tokenId) public view override returns (uint256) {
        return (_baseLevelMultiplier.mul(tokenData[_tokenId].level)).add(1e9);
    }
...
interface IPixelMushrohmERC721 is IERC721Enumerable {
    ...
    function airdrop(address[] calldata _to, uint256[] calldata _amount) external;
