...
contract LootFinityToken is
    ...
    ERC721Royalty,
    NonblockingLzApp,
    // Open sea supported events for metadata update
    IERC4906
{
    bytes32 constant ORDINAL_TRAIT_KEY =
        keccak256(abi.encodePacked("Ordinal_ID"));
    mapping(SaleType => SaleRange) public saleRange;
    mapping(uint256 => string[]) public traits;
    mapping(uint256 => mapping(uint256 => bool)) public isTraitWrapped;
    ...
    function setRevenueReceiver(address _revenueReceiver) public onlyManager {
        revenueReceiver = _revenueReceiver;
    }
    ...
    function setMintAuthority(address _mintAuthority) public onlyManager {
        mintAuthority = _mintAuthority;
    }
    ...
    function setTraitManager(address _traitManager) public onlyManager {
        traitManager = _traitManager;
    }
    ...
    function containsOrdinal(
        uint256 _collectibleId
    ) public view returns (bool) {
        _requireMinted(_collectibleId);
        string memory ordinalId;
        bool isWrapped;
        uint256 ordinalIdIndex = traits[_collectibleId].length + 1;

        for (uint256 i = 0; i < traits[_collectibleId].length; i += 2) {
            string memory key = traits[_collectibleId][i];

            if (keccak256(abi.encodePacked(key)) == ORDINAL_TRAIT_KEY) {
                isWrapped = isTraitWrapped[_collectibleId][i];
                ordinalId = traits[_collectibleId][i + 1];
                ordinalIdIndex = i;
                break;
            }
        }

        if (ordinalIdIndex > traits[_collectibleId].length) {
            return false;
        }

        return StringUtils.strlen(ordinalId) > 0 && isWrapped;
    }

    function wrapTrait(
        uint256 _collectibleId,
        uint256 _traitIdx
    ) public onlyTraitManager {
        isTraitWrapped[_collectibleId][_traitIdx] = true;
    ...
    function unwrapTrait(
        ...
        isTraitWrapped[_collectibleId][_traitIdx] = false;
        emit MetadataUpdate(_collectibleId);
    }
    ...
    function removeTrait(
        ...
        uint256 totalTraits = traits[_collectibleId].length;
        uint256 keyIndex;
        bool found;

        for (uint i = 0; i < totalTraits; i += 2) {
            if (
        ...
        if (found) {
            if (keyIndex < totalTraits - 2) {
                traits[_collectibleId][keyIndex] = traits[_collectibleId][totalTraits - 2];
                traits[_collectibleId][keyIndex + 1] = traits[_collectibleId][totalTraits - 1];

                isTraitWrapped[_collectibleId][keyIndex] = isTraitWrapped[_collectibleId][totalTraits - 2];
                isTraitWrapped[_collectibleId][keyIndex + 1] = isTraitWrapped[_collectibleId][totalTraits - 1];
            }
            isTraitWrapped[_collectibleId][totalTraits - 2] = false;
            isTraitWrapped[_collectibleId][totalTraits - 1] = false;

            traits[_collectibleId].pop();
            traits[_collectibleId].pop();
            emit MetadataUpdate(_collectibleId);
        }
    }
    ...
    function sendCollectibleToChain(
        ...
        for (uint256 i = 0; i < traits[_collectibleId].length; i++) {
            wrapping[i] = isTraitWrapped[_collectibleId][i];
        }
    ...
    function _nonblockingLzReceive(
        ...
        for (uint i = 0; i < _traits.length; i++) {
            ...
            isTraitWrapped[collectibleId][i] = _wrapping[i];
        }
    ...
    function burn(uint256 _collectibleId) external onlyMintAuthority {
        _requireMinted(_collectibleId);
        require(canBeBurned[_collectibleId] == true);
        delete (traits[_collectibleId]);
        _burn(_collectibleId);
    }
...
function buildMetadata(
        ...
        if (traits[_collectibleId].length > 0) {
            for (uint256 i = 0; i < traits[_collectibleId].length; i += 2) {
                ...
                bool isLast = i + 2 >= traits[_collectibleId].length;
                bool isWrapped = isTraitWrapped[_collectibleId][i];
        ...
        metadataBytes = bytes(
            abi.encodePacked(
                '{"name":"Lootfinity Token", "description":"Lootfinity Token","image": "',
                imageURI,
                '", "attributes": [',
                attributes,
                "]}"
...
interface ICrossChainCollectible {
    event CollectibleReceived(
        address indexed to,
        uint256 srcChainId,
        uint256 collectibleId
    );
    event CollectibleSent(
...
interface ILootFinityToken {
    ...
    function containsOrdinal(uint256 tokenId) external returns (bool);

    function wrapTrait(uint256 _collectibleId, uint256 _traitIdx) external;

    function unwrapTrait(uint256 _tokenId, uint256 _traitIdx) external;
}
