...
contract CreatorHeroesStorage is Ownable, StatusHandler, ImageDataHandler {
    using Strings for uint256;

    address public specialCharacterAddress;

    function setTokenURIContractAddress(address newTokenURIContractAddress) public onlyOwner {
        specialCharacterAddress = newTokenURIContractAddress;
    }
    ...
    function createDataURI(uint256 tokenId, uint256 slot, uint256 iconCheck) public view returns (string memory) {
        ...
        imageURI = Base64.encode(
            bytes(
                string.concat(
                    _firstConcatCharacterRender(tokenId, slot),
                    ITokenURI(specialCharacterAddress).secondConcatCharacterRender_(tokenId, slot)
                )
            )
        );
