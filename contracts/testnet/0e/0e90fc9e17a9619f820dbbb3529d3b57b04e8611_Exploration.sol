...
contract Exploration is Initializable, ExplorationSettings {
    ...
    function ownerOfDonkey(uint256 _tokenId) external view override returns(address) {
        return tld.ownerOf(_tokenId);
    }

    function tldOwnerOfDonkey(uint256 _tokenId) private view returns(address) {
        return tld.ownerOf(_tokenId);
    }
    ...
    function getDonkeyInfo(uint256 _tokenId) external view returns(DonkeyStakeInfo memory) {
        
        bool isStaked = isDonkeyStaked(_tokenId);
        return DonkeyStakeInfo(
            isStaked ? tokenIdToInfo[_tokenId].owner : tldOwnerOfDonkey(_tokenId),
            isStaked,
            isStaked ? totalStakedTimeForDonkeyInSec(_tokenId) : 0,
            isStaked ? timeStakedForDonkey(_tokenId) : 0
            );
    }
    ...
    //Used for testing, for sure remove later
    function setLastStakedTimeForDonkey(uint256 _tokenId, uint256 _lastStakedTime) external override onlyAdminOrOwner {
        uint256 lastStakedTime = tokenToStakeInfo[_tokenId].lastStakedTime;
        require(lastStakedTime > 0, "Exploration: Donkey is not staked");
        require(_lastStakedTime > 0, "Exploration: Cannot set lastStakedTime to 0");
        tokenToStakeInfo[_tokenId].lastStakedTime = _lastStakedTime;
    }
}
...
// Expands on StakeInfo, can replace StakeInfo in Arb-one implementation
...
interface IExploration {
	...
	function ownerOfDonkey(uint256 _tokenId) external view returns(address);

	function setLastStakedTimeForDonkey(uint256 _tokenId, uint256 _lastStakedTime) external;

	//function totalStakedTimeForDonkeyInSec(uint256 _tokenId) external view returns (uint256);
}
