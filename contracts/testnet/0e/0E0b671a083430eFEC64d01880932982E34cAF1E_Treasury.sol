    ...
    AccessControlRolesUpgradeable
{
    ...
    modifier onlyPoolAction(
        uint256 poolId,
        address poolUser,
        SignedPoolAction action,
        bytes memory payload,
        bytes calldata signature
    ) {
        consumePoolActionSignature(
            poolId,
            poolUser,
            action,
            payload,
     ...
     * @param signature The user signature, with payload: `{id}{nonce}{role}`.
     */
    function validatePoolActionSignature(
        uint256 id,
        address poolUser,
        SignedPoolAction action,
        bytes memory payload,
        ...
        if (action == SignedPoolAction.Ownership) {
            require(
                pool.owner == poolUser,
     ...
     * @param signature The user signature, with payload: `{id}{nonce}{role}`.
     */
    function consumePoolActionSignature(
        uint256 id,
        address poolUser,
        SignedPoolAction action,
        bytes memory payload,
        ...
        validatePoolActionSignature(id, poolUser, action, payload, signature);
        // The signature is valid, so it is consumed by increasing the nonce.
    ...
    AccountRoleVerifier
{
    ...
    function setLiquidityPool(address _liquidityPool)
        external
        onlyOwner
    {
        liquidityPool = LiquidityPool(_liquidityPool);
