...
import {MINTER_ROLE, MANAGER_ROLE, IS_PIRATE_TRAIT_ID, DICE_ROLL_1_TRAIT_ID, DICE_ROLL_2_TRAIT_ID, RANDOMIZER_ROLE, ELEMENTAL_AFFINITY_TRAIT_ID, EXPERTISE_TRAIT_ID, GENERATION_TRAIT_ID, LEVEL_TRAIT_ID, XP_TRAIT_ID} from "../Constants.sol";
...
contract StarterPirateSystem is GameRegistryConsumerUpgradeable, ILootCallback {
    ...
    function fulfillRandomWordsCallback(
        ...
        if (request.account != address(0)) {
            ...
            // Get LootTableComponent, get the weights assigned to it, and use that to determine the templateId
            LootTableComponent lootTableComponent = LootTableComponent(
                _gameRegistry.getComponent(LOOT_TABLE_COMPONENT_ID)
            );
            ...
            uint256 total = 0;
            for (uint256 idx; idx < lootTable.lootIds.length; idx++) {
                total += lootTable.weights[idx];
                if (entropy < total) {
                    // Make sure token template exists
                    if (
                        tokenTemplateSystem.exists(lootTable.lootIds[idx]) ==
                        false
                    ) {
                        revert TemplateNotFound();
                    }
                    // Set random StarterPirate tokentemplate id
                    tokenTemplateSystem.setTemplate(
                        address(starterPirateNFT),
                        request.tokenId,
                        lootTable.lootIds[idx]
                    );
                    break;
                }
            }
     ...
     */
    function _grantLoot(address account, uint256 amount) internal {
