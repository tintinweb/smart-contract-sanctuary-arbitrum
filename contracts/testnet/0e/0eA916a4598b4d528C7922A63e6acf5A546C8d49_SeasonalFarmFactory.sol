...
error InvalidValue(string given);
...
contract SeasonalFarmFactory is
    IHasFeeInfo, IHasAssetInfo, IHasPausable,
    ...
    ETHFee
{
    ...
    event DefaultFundraisingTimeChanged(uint256 defaultFundraisingTime);
    ...
    // default fundraising time which can be used by the manager to raise funds (defaults - 3 days)
    uint256 public defaultFundraisingTime;
    ...
    function createFarm(
        ...
        ERC1967Proxy seasonalFarm = new ERC1967Proxy(
            ...
            abi.encodeWithSignature(
                "initialize((bool,string,string,uint256,uint256,uint256,uint256),address,uint256)",
                _info,
                _operator,
                defaultFundraisingTime
            )
        );
    ...
    /// @notice set the default fundraising time a manager can use when creating a farm
    /// @dev can only be called by the `owner`
    /// @param _defaultFundraisingTime default fundraising time when creating a farm
    function setDefaultFundraisingTime(uint256 _defaultFundraisingTime) external onlyOwner {
        if (_defaultFundraisingTime < 15 minutes) revert BelowMin(15 minutes, _defaultFundraisingTime);
        defaultFundraisingTime = _defaultFundraisingTime;
        emit DefaultFundraisingTimeChanged(_defaultFundraisingTime);
    }
