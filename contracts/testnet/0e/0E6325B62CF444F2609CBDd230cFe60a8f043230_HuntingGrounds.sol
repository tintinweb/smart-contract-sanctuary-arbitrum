...
interface IHousezManager {
    function numberOfFloorsUnlocked(address _user) external view returns(uint16);
}
...
contract HuntingGrounds is Initializable, HuntingGroundsSettings {
    ...
    contractsAreSet
    {
        uint32 _boost = _getBoostForUser(_owner);

        _claimBugz(_tokenId, _owner, _boost);
    ...
    {
        uint32 _boost = _getBoostForUser(msg.sender);
        ...
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            _claimBugz(_tokenIds[i], msg.sender, _boost);
        }
    }
    ...
    onlyAdminOrOwner
    {
        ...
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            require(world.ownerForStakedToad(_tokenIds[i]) == _owner, "HuntingGrounds: User does not own Toad");
            require(world.locationForStakedToad(_tokenIds[i]) == Location.HUNTING_GROUNDS, "HuntingGrounds: Toad is not at hunting grounds");

            _claimBugz(_tokenIds[i], _owner, _boost);
        }
    }

    function _claimBugz(uint256 _tokenId, address _to, uint32 _boost) private {
        ...
        uint256 _bugzAmount = (block.timestamp - tokenIdToLastClaimedTime[_tokenId]) * (bugzAmountPerDay * (100000 + _boost)) / 1 days / 100000;
    ...
    function _getBoostForUser(address _user) private view returns(uint32) {
        uint16 _floorsOwned = housezManager.numberOfFloorsUnlocked(_user);
        return floorsOwnedToBoost[_floorsOwned];
    }
}
...
abstract contract HuntingGroundsContracts is Initializable, HuntingGroundsState {
    ...
    onlyAdminOrOwner
    {
        ...
        housezManager = IHousezManager(_housezManagerAddress);
    }
    ...
    function areContractsSet() public view returns(bool) {
        return address(world) != address(0)
            ...
            && address(housezManager) != address(0);
    }
}
...
abstract contract HuntingGroundsSettings is Initializable, HuntingGroundsContracts {
    ...
    function setFloorsOwnedToBoost(
        uint16 _floorsOwned,
        uint32 _boost)
    external
    onlyAdminOrOwner
    {
        floorsOwnedToBoost[_floorsOwned] = _boost;

        emit FloorsOwnedToBoostUpdated(_floorsOwned, _boost);
    }
}
...
import "../housezmanager/IHousezManager.sol";
...
abstract contract HuntingGroundsState is Initializable, IHuntingGrounds, AdminableUpgradeable {
    ...
    event FloorsOwnedToBoostUpdated(uint16 _floorsOwned, uint32 _boost);
    ...
    mapping(uint16 => uint32) public floorsOwnedToBoost;

    IHousezManager public housezManager;
