    ...
    GovernorSettings
{
    ...
    constructor(IVotes _token, TimelockController _timelock, GovernorParams memory _params)
        ...
        /// prod settings: voting delay in blocks (1 day), voting period blocks (1 week), proposal threshold 1%
        /// GovernorSettings(6575, 46027, 1)
        /// dev settings: voting delay in blocks (5 mins), voting period blocks (1 hour), proposal threshold 1%
        /// GovernorSettings(60, 300, _params.proposalThreshold * 10 ** 18) // assumes 18 decimals in _token
        GovernorSettings(_params.votingDelay, _params.votingPeriod, _params.proposalThreshold * 10 ** 18) // assumes 18 decimals in _token
    ...
    /// @notice The quorum requied for a proposal to pass.
    /// @return The number of votes required for a proposal to pass.
    /// The number is computed based on token supply at a given block
    ...
    /// Proposal is executed as follows:
    /// targets[i].call{value: values[i]}(calldatas[i])
    /// @param targets The list of contract addresses to execute.
    /// @param values The list of values (ETH amount) to send with each call.
    /// @param calldatas The list of codes to execute on each contract.
    /// @param description The description of the proposal
    /// @return proposalId the id of created proposal
    function propose(
