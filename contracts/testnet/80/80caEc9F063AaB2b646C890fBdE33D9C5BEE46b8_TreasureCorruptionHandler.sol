...
interface ITreasureCorruptionHandler {
...
contract TreasureCorruptionHandler is Initializable, TreasureCorruptionHandlerContracts {
    ...
    onlyCorruptionRemoval
    {
        ...
        for(uint256 i = 0; i < _requestInfo.treasureIds.length; i++) {
            ...
            for(uint256 j = 0; j < _requestInfo.treasureAmounts[i]; j++) {

                if(_randomNumber % 100000 < _treasureMetadata.craftingBreakOdds) {
                    ...
                    if(_treasureMetadata.consumableIdDropWhenBreak > 0) {
                        consumable.mint(_user, _treasureMetadata.consumableIdDropWhenBreak, 1);
                    }
                }
