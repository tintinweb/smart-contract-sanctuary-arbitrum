...
contract XMY is IERC20, Ownable {
    ...
    mapping(address => uint256) public presaleListIndex;
    ...
    function presale(address invitor,uint256 piece) payable external  {
        ...
        _bindInvitor(msg.sender,deadAddress);
        ...
        presaleList.push(msg.sender);
        processPreSaleReward(msg.sender);
    ...
    function processPreSaleReward(address account) internal{
        uint256 totalPieces; 
        for (uint256 i; i < _binders[account].length;) {
            address lowAddress = _binders[account][i];
            totalPieces += presalePieces[lowAddress];
        }
        uint256 rewardPieces = totalPieces / 5;
        if(rewardPieces > 0){
    ...
    function launch() external onlyOwner {
        ...
        for (uint256 i; i < presaleList.length;) {
            uint256 totalPresaleAmount = presaleAmount[presaleList[i]] + presaleReward[presaleList[i]];
            leftPresale[presaleList[i]] = totalPresaleAmount;
        }
    }

    function getPresaleToken()external{
        require(leftPresale[msg.sender] > 0 || block.timestamp > startTradeTime );
