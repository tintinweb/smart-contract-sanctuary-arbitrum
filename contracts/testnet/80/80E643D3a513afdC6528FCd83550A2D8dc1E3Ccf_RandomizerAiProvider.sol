...
contract RandomizerAiProvider is Access, IRandomizerProvider, InternalRNG {
  ...
  mapping(uint256 => uint8) public counts; 
  ...
  function randomizerCallback(uint256 _requestId, bytes32 _value) external onlyRandomizer {
    uint8 count_ = counts[_requestId];
  ...
  function request(uint256 _routerRequestId, uint8 _count, uint256) external onlyRouter returns (uint256 requestId_) {
