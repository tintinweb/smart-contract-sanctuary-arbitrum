...
contract A3SQueue is IA3SQueue, Ownable {
    ...
    //The Threshold after which max queue length will be extened, Default is 200; NEED IMPLEMENTATION
...
contract A3SWallet is ERC721Holder, ERC1155Holder {
    ...
    function ownerOf() public view returns (address) {
        return IA3SWalletFactoryV3(factory).walletOwnerOf(address(this));
    }
}
...
library A3SQueueHelper {
    ...
    function _mint(
        ...
        if (
            ...
            token.mint(_referringAddr, _RewardForReferring);
            token.mint(_addr, _RewardForReferred);
            emit ReferralMint(
                _referringAddr,
                _addr,
                _RewardForReferring,
    ...
    function _steal(
        ...
        A3SToken token = A3SToken(_token);
        token.mint(_tailIdx, _balance.div(2));
        token.mint(_vault, _balance.div(2));
...
/*
总量10B（5条链，每条链2B）
其中 90%以que2earn的方式产出
9.5% A3S Treasury 950M 用于提供流动性、marketing等用途,初始解锁10%，之后3年每6个月线性解锁（每次15%）
0.5% 团队50M 初始解锁10%，之后4年每6个月线性解锁（每次11.25%）
*/

contract A3SToken is ERC20, Ownable {
    uint256 constant public totalq2earnSupply = 18 * (10**9) * (10**18);

    uint256 constant public total_TreasurySupply = 19 * (10**6) * (10**18);
    uint256 constant public initial_TreasurySupply = 19 * (10**5) * (10**18);
    uint256 constant public continue_TreasurySupply = 19 * 15 * (10**4) * (10**18); // every 6 month for 3 yrs
    

    uint256 constant public total_ProjectPartySupply = 1 * (10**6) * (10**18);
    uint256 constant public initial_ProjectPartySupply = 1 * (10**5) * (10**18);
    uint256 constant public continue_ProjectPartySupply = 1125 * (10**2) * (10**18) ; // every 6 month for 4 yrs
    ...
    function mint(address to, uint256 amount) public ONLY_GOV {
        require(queue2earnSupply + amount <= totalq2earnSupply, "A3S: Queue To Earn $AA token mint exceed the maximum supply");
        _mint(to, amount);
        queue2earnSupply += amount;
        emit Mint(to, amount);
    }
    
    function mint_Treasury() public ONLY_GOV {
    ...
    function mint_Project() public ONLY_GOV {
