...
contract FloatBottle is IQQBforFlotBottle {
    ...
    mapping(uint => Red) public redpockets; //index 1 start
    ...
    mapping(address => uint) redget_states;

    function getRed(uint rid, uint ticket, uint pw) external {
        require(!isContract(msg.sender), "address invaild");
        require(redget_num[rid][msg.sender] < 5, "b601:too many try");
        ...
        if (msgs[red_msg[rid]].ticket == ticket && pw == password[rid]) {
            ...
            IERC20(redpockets[rid].a).transfer(
                msg.sender,
                redpockets[rid].n * 10 ** 18
            );
            redget_states[msg.sender] = 200;
        } else {
            redget_num[rid][msg.sender] += 1;
            redget_states[msg.sender] = redget_num[rid][msg.sender];
        }
    }

    function getRedStauts() external view returns (uint) {
        return redget_states[msg.sender];
    }
