...
contract ListMintByMetadrop is PrimarySaleModule, IListMintByMetadrop {
  ...
  // The most recent previous merkletroot (if any)
  bytes32 public previousListMerkleRoot;

  // The timestamp of a change in merkleroot
  uint256 public merkleRootChangeTimestamp;
  ...
  function initialisePrimarySaleModule(
    ...
    _initialisePrimarySaleModuleBase(
      ...
      phaseId_,
      listMintConfig.reservedAllocationPhase,
      previousRootValidityInSeconds_
    );
  }
  ...
  function setList(bytes32 merkleRoot_) external onlyFactoryOrProjectOwner {
    // Save the previous merkleroot in previousListMerkleRoot. This will remain a valid root until the expiry of the
    // previous root validation period.
    previousListMerkleRoot = listMerkleRoot;
    merkleRootChangeTimestamp = block.timestamp;
  ...
  function merkleListValid(
    ...
    uint256 unitPrice_,
    bool paymentValid_
  ) public view returns (bool success_, address allowanceAddress_) {
    ...
    // First check using the current root:
    (success_, allowanceAddress_) = _performMerkleCheck(
      addressToCheck_,
      position_,
      quantityEligible_,
      proof_,
    ...
    if (
      !success_ &&
      block.timestamp <
      (merkleRootChangeTimestamp + previousRootValidityInSeconds)
    ) {
      // Try the previous root as we are within the changed root validity period:
      (success_, allowanceAddress_) = _performMerkleCheck(
        addressToCheck_,
        position_,
        quantityEligible_,
        ...
        unitPrice_,
        leaf,
        previousListMerkleRoot
      );
    }

    return (success_, allowanceAddress_);
  }

  /** ____________________________________________________________________________________________________________________
   *                                                                                                               -->MINT
   * @dev (function) _performMerkleCheck  Check the merkle root
   *
   * ---------------------------------------------------------------------------------------------------------------------
   * @param addressToCheck_        The address we are checking
   * ---------------------------------------------------------------------------------------------------------------------
   ...
   * @param unitPrice_             The individual unit price of NFTs being minted in this call
   * ---------------------------------------------------------------------------------------------------------------------
   * @param leaf_                  The leaf being checked
   * ---------------------------------------------------------------------------------------------------------------------
   * @param root_                  The root being used in the check
   * ---------------------------------------------------------------------------------------------------------------------
   * _____________________________________________________________________________________________________________________
   */
  function _performMerkleCheck(
    address addressToCheck_,
    uint256 position_,
    uint256 quantityEligible_,
    bytes32[] calldata proof_,
    ...
    bytes32 leaf_,
    bytes32 root_
  ) internal view returns (bool success_, address allowanceAddress_) {
    if (MerkleProof.verify(proof_, root_, leaf_)) {
    ...
    if (useEPS) {
      ...
      for (uint256 i = 1; i < allAddresses.length; ) {
        ...
        if (MerkleProof.verify(proof_, listMerkleRoot, leaf_)) {
