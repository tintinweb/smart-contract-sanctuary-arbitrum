...
import "./interfaces/IGambitTradingCallbacksV1.sol";
import "./interfaces/ITWAPPriceGetter.sol";
...
contract GambitPriceAggregatorV1 is Initializable, ITWAPPriceGetter, Multicall {
        ...
        )
    {
        ...
        // scope for `fee`
        {
            ...
            // note that update may fail if the transaction is mined too late, or
            // if other transaction has already updated the price
            //
            // In first case, pyth.getPriceNoOlderThan will be thrown
            // In second case, pyth.getPriceNoOlderThan will return the latest price
            //
            // So it is safe to use try-catch here.
        ...
        PythStructs.Price memory pythPrice1 = pyth.getPriceNoOlderThan(
            f.priceId1,
            PYTH_PRICE_AGE
        );
        ...
        // parse pyth price
        price =
            (uint(uint64(pythPrice1.price)) * PRECISION) /
            (10 ** uint(uint32(-pythPrice1.expo)));
        conf =
            (uint(pythPrice1.conf) * PRECISION) /
            (10 ** uint(uint32(-pythPrice1.expo)));

        // Price from Chainlink
        if (enableChainlinkFeed) {
            // fetch price
