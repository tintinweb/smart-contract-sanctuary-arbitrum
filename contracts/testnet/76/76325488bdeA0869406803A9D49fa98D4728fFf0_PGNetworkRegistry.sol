...
contract PGNetworkRegistry is IXReceiver, IPGRegistry, MemberRegistry, OwnableUpgradeable {
    ...
    mapping (uint256 => NetworkRegistry) networkRegistry;
    ...
    function updateNetworkRegistry(uint256 _chainId, NetworkRegistry memory _registry) external onlyOwner {
    ...
    function syncUpdateMember(address _member, uint32 _activityMultiplier, uint256[] memory _chainIds, uint256[] memory _relayerFees)
        external payable
        onlyOwner
    {
        require(_chainIds.length == _relayerFees.length, "PGNetworkRegistry: Wrong network params");
    ...
    function xReceive(
        ...
        if (functionSelector == IPGRegistry.setNewMember.selector) {
            (, address _member, uint32 _activityMultiplier, uint32 _startDate) = abi.decode(_callData, (bytes4, address, uint32, uint32));
            setNewMember(_member, _activityMultiplier, _startDate);
            return bytes("");
        }
        if (functionSelector == IPGRegistry.updateMember.selector) {
            (, address _member, uint32 _activityMultiplier) = abi.decode(_callData, (bytes4, address, uint32));
            updateMember(_member, _activityMultiplier);
            return bytes("");
        }
        return bytes("");
...
interface IMultichainMemberRegistry {
    function syncSetNewMember(address _member, uint32 _activityMultiplier, uint32 _startDate, uint256[] memory _chainIds, uint256[] memory _relayerFees) external payable;
    function syncUpdateMember(address _member, uint32 _activityMultiplier, uint256[] memory chainIds, uint256[] memory relayerFees) external payable;
}
