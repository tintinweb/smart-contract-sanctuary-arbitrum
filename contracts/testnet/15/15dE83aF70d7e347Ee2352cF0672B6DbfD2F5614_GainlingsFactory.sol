...
contract GainlingsFactory is ERC721Enumerable, RrpRequesterV0, ReentrancyGuard, Ownable
{
    ...
    function attackInternal(uint256 attackerTokenId, uint256 defenderTokenId,uint256 attackType) internal  canAttack(attackerTokenId,defenderTokenId)  {
    ...
    function attackWarrior(AttackRequest memory attackRequest,uint256 randomness)  internal  {
        ...
        else if(attackRequest.attackType == 1){
            transferBountyToStack(winnerToken,loserToken);
            _burn(loserToken);
        }
        else if(attackRequest.attackType == 2){
            _safeTransfer(ownerOf(loserToken), ownerOf(winnerToken), loserToken, "");
        }
    ...
    function transferBountyToStack(uint256 winnerTokenId, uint256 loserTokenId) internal{
        _tokenStacks[winnerTokenId] += _tokenBounties[loserTokenId];
        _tokenBounties[loserTokenId] = 0;
    }
