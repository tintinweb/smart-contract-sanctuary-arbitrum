...
contract StakingPoolFactory is IStakingPoolFactory, Ownable, Initializable {
    ...
    function setSecSpanPerUpdate(uint256 newSecSpanPerUpdate) external {
        secSpanPerUpdate = newSecSpanPerUpdate;
    }
    ...
    function syncYieldPriceOfWeight() external override returns (uint256) {
        ...
        lastUpdateTimestamp = lastTimeUpdatePriceOfWeight;
    ...
    function calPendingFactoryReward() external override returns (uint256 reward) {
        return _calPendingFactoryReward();
    }

    function calLatestPriceOfWeight() external override returns (uint256) {
    ...
    function _calPendingFactoryReward() internal returns (uint256 reward) {
        uint256 currentTimestamp = block.timestamp;
    ...
    function _calStakingPoolApeXReward(address pool) internal returns (uint256 reward, uint256 newPriceOfWeight) {
...
interface IStakingPoolFactory {
    ...
    function calStakingPoolApeXReward(address token) external returns (uint256 reward, uint256 newPriceOfWeight);

    function calPendingFactoryReward() external returns (uint256 reward);

    function calLatestPriceOfWeight() external returns (uint256);
...
contract StakingPool is IStakingPool, Reentrant, Initializable {
    ...
    function pendingYieldRewards(address _staker) external returns (uint256 pending) {
