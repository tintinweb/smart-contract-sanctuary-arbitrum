...
contract ItemsFacet is SolidStateERC1155, IERC2981, WithStorage, WithModifiers {
    ...
    function requestNftConversion(uint16 quantity) external payable pausable {
        uint256 totalPrice = _constants().nftConversionPrice * quantity;
        require(
            msg.value >= totalPrice,
            'Incorrect amount of ETH sent'
        );
        _token().requestedNftConversions[msg.sender] += quantity;
    }
    ...
    function mintGameItem(
        ...
        for (uint8 i = 0; i < amount; i++) {
            ...
            _token().requestedNftConversions[to]--;
        }

        _safeMintBatch(to, mintedIds, amounts, '');
    }

    function requestChestOpening(uint256 tokenId) external payable pausable {
        require(balanceOf(msg.sender, tokenId) > 0, 'Does not own token');
        require(
        ...
        require(
            msg.value >= _constants().chestOpeningPrice,
            'Incorrect amount of ETH sent'
        );
        require(_token().requestedChestOpening[tokenId] == address(0), 'Chest opening already requested');
        _token().requestedChestOpening[tokenId] = msg.sender;
    }
    ...
    function openTreasureChest(
        ...
        require(_token().requestedChestOpening[tokenId] == receiver, 'No chest opening requested');
        require(balanceOf(receiver, tokenId) > 0, 'Does not own token anymore');

        _token().requestedChestOpening[tokenId] = address(0);
