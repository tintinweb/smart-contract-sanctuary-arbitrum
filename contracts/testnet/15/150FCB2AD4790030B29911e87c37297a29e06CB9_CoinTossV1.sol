...
/// @title nekopachi CoinToss
/// @author munji ([email protected]) - Romuald Hog (based on Yakitori's Coin Toss)
...
contract CoinTossV1 is Game {
    ...
    /// @param face The chosen coin face.
    /// @param rolled The rolled coin face.
    /// @param _payouts The payout amount for each coin face.
    ...
    /// @param _supraRouterAddress of the Chainlink VRF Coordinator.
    ...
    function _getPayout(uint256 betAmount) private pure returns (uint256) {
        return betAmount * 2; //최대 두배
    }

    /// @notice Creates a new bet and stores the chosen coin face.
    /// @param face The chosen coin face.
    /// @param token Address of the token.
    /// @param betUnit The bet unit.
    /// @param rngCount The amount of random numbers to request.
    function wager(
        bool face,
        address token,
        uint256 betUnit,
    ...
    function _callback(
        ...
        uint256[] memory _payoutsToDisplay = new uint256[](
            _randomNumbers.length
        );
        uint256 totalPayOut = 0;
        coinTossBet.numOfToss = bets[_id].rngCount;
        for (uint256 i = 0; i < _randomNumbers.length; i++) {
            bool[2] memory _faces = [false, true];
            bool rolled = _faces[_randomNumbers[i] % 2];
            bool isWin = rolled == coinTossBet.face;
            uint256 _payout = _getPayout(bet.betUnit);
            _rolled[i] = rolled;
            _payouts[i] = isWin ? _payout : 0;
            _payoutsToDisplay[i] = isWin ? _payout - _getFees(_payout) : 0;
            totalPayOut += _payouts[i];
            coinTossBet.numOfWins += isWin ? 1 : 0;
        }
        uint256 payout = _resolveBet(bet, totalPayOut);
        emit Roll({
            id: bet.id,
            user: bet.user,
            token: bet.token,
            amount: bet.amount,
            face: coinTossBet.face,
            rolled: _rolled,
            _payouts: _payoutsToDisplay,
