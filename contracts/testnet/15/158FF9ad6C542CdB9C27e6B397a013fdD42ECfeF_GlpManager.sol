...
contract GlpManager is ReentrancyGuard, Governable, IGlpManager {
    ...
    constructor(address _vault, address _usdx, address _glp, uint256 _cooldownDuration) public {
        gov = msg.sender;
        vault = IVault(_vault);
        usdx = _usdx;
        glp = _glp;
    ...
    function setWhiltelistedWallet(address _wallet,bool _isActive) external {
        _validateHandler();
        whitelistedWallets[_wallet] = _isActive;
    }
    ...
    function addLiquidity(address _token, uint256 _amount, uint256 _minUsdx, uint256 _minGlp) external override nonReentrant returns (uint256) {
        if (inPrivateMode) { revert("GlpManager: action not enabled"); }
        require(whitelistedWallets[msg.sender],"GlpManager: Wallet is not Whitelisted");
        return _addLiquidity(msg.sender, msg.sender, _token, _amount, _minUsdx, _minGlp);
    }

    function addLiquidityForAccount(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdx, uint256 _minGlp) external override nonReentrant returns (uint256) {
        _validateHandler();
        require(whitelistedWallets[_fundingAccount],"GlpManager: Wallet is not Whitelisted");
        return _addLiquidity(_fundingAccount, _account, _token, _amount, _minUsdx, _minGlp);
    }
    ...
    function removeLiquidityForAccount(address _account, address _tokenOut, uint256 _glpAmount, uint256 _minOut, address _receiver) external override nonReentrant returns (uint256) {
        _validateHandler();
        require(whitelistedWallets[_account],"GlpManager: Wallet is not Whitelisted");
    ...
    function _addLiquidity(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdx, uint256 _minGlp) private returns (uint256) {
        ...
        require(usdxAmount >= _minUsdx, "GlpManager: insufficient USDX output");

        uint256 mintAmount = aumInUsdx == 0 ? usdxAmount : usdxAmount.mul(glpSupply).div(aumInUsdx);
        ...
        emit AddLiquidity(_account, _token, _amount, aumInUsdx, glpSupply, usdxAmount, mintAmount);
    ...
    function _removeLiquidity(address _account, address _tokenOut, uint256 _glpAmount, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        uint256 usdxAmount = _glpAmount.mul(aumInUsdx).div(glpSupply);
        ...
        emit RemoveLiquidity(_account, _tokenOut, _glpAmount, aumInUsdx, glpSupply, usdxAmount, amountOut);
...
interface IGlpManager {
    ...
    function addLiquidity(address _token, uint256 _amount, uint256 _minUsdx, uint256 _minGlp) external returns (uint256);
    function addLiquidityForAccount(address _fundingAccount, address _account, address _token, uint256 _amount, uint256 _minUsdx, uint256 _minGlp) external returns (uint256);
