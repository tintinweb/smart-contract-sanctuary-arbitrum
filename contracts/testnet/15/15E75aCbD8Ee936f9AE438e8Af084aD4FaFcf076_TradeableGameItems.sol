...
uint256 constant ID = uint256(
    keccak256("game.piratenation.tradelicenseexemptcomponent.v1")
);
...
library TradeLicenseExemptComponentStorage {
    bytes32 internal constant STORAGE_SLOT = bytes32(ID);
...
/**
 * @title TradeLicenseExemptComponent
 * @dev Whether an entity is trade license exempt
 */
contract TradeLicenseExemptComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getLayoutValue(
        ...
        // Get the struct from storage
        value = TradeLicenseExemptComponentStorage.layout().entityIdToStruct[
            entity
        ];
    }
    ...
    function getValue(
        ...
        if (has(entity)) {
            Layout memory s = TradeLicenseExemptComponentStorage
                .layout()
                .entityIdToStruct[entity];
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = TradeLicenseExemptComponentStorage
            .layout()
            .entityIdToStruct[entity];
    ...
    function remove(
        ...
        // Remove the entity from the component
        delete TradeLicenseExemptComponentStorage.layout().entityIdToStruct[
            entity
        ];
        _emitRemoveBytes(entity);
    }
