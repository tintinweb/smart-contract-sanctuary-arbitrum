...
import {IERC721EnumerableUpgradeable, IERC721Upgradeable} from "@openzeppelin/contracts-upgradeable/token/ERC721/extensions/ERC721EnumerableUpgradeable.sol";
...
contract ContractL2 {
    ...
    address public genesisNftSeries1;
    address public genesisNftSeries2;
    address public stakingGenesisNft;

    constructor(address nft1, address nft2, address staking) {
        genesisNftSeries1 = nft1;
        genesisNftSeries2 = nft2;
        stakingGenesisNft = staking;
    }

    function listNftsS1(address account) external returns (uint256[] memory) {
        uint256 balance = balanceOfS1(account);

        uint256[] memory tokens = new uint256[](balance);
        for (uint256 i = 0; i < balance; i++) {
            bytes memory tokenOfOwnerByIndexData = abi.encodeWithSelector(
                IERC721EnumerableUpgradeable.tokenOfOwnerByIndex.selector,
                account
            );
            uint256 tokenId = arbsys.sendTxToL1(genesisNftSeries1, tokenOfOwnerByIndexData);
            tokens[i] = tokenId;
        }
    ...
    function listNftsS2(address account) external returns (uint256[] memory) {
        uint256 balance = balanceOfS2(account);
        ...
        for (uint256 i = 0; i < balance; i++) {
            ...
            uint256 tokenId = arbsys.sendTxToL1(genesisNftSeries2, tokenOfOwnerByIndexData);
            tokens[i] = tokenId;
        }
    ...
    function balanceOfS1(address account) public returns (uint256) {
        bytes memory data = abi.encodeWithSelector(IERC721Upgradeable.balanceOf.selector, account);

        uint256 balance = arbsys.sendTxToL1(genesisNftSeries1, data);
        console.log("balanceOfS1", balance);

        return balance;
    }

    function balanceOfS2(address account) public returns (uint256) {
        bytes memory data = abi.encodeWithSelector(IERC721Upgradeable.balanceOf.selector, account);

        uint256 balance = arbsys.sendTxToL1(genesisNftSeries2, data);
        console.log("balanceOfS1", balance);

        return balance;
    }

    function getRewardLarge(address account) public returns (uint256) {
        bytes memory data = abi.encodeWithSelector(IStakingGenesisNFT.getRewardLarge.selector, account);

        uint256 reward = arbsys.sendTxToL1(stakingGenesisNft, data);
        console.log("getRewardLarge", reward);

        return reward;
    }

    function getRewardSmall(address account) public returns (uint256) {
        bytes memory data = abi.encodeWithSelector(IStakingGenesisNFT.getRewardSmall.selector, account);

        uint256 reward = arbsys.sendTxToL1(stakingGenesisNft, data);
        console.log("getRewardSmall", reward);
