...
contract AaveVault is Ownable, ReentrancyGuard {
  ...
  constructor(address _dusd, address _dai, address _aArbDai, address _augustusAddr, address _aavePool) {
    DUSD = IERC20(_dusd);
    DAI = IERC20(_dai);
    aArbDAI = IAToken(_aArbDai);
    augustus = _augustusAddr;
    // augustus = IAugustusSwapper(_augustusAddr);
    aavePool = IPool(_aavePool);
  }

  function depositAndSave(bytes memory swapCalldata) public nonReentrant {
  ...
  function userBalance() public view returns(uint256) {
    ...
    uint256 balance = balances[msg.sender].rayMul(reserve.liquidityIndex);
    return balance;
  }

  function withdraw(uint256 amount) public nonReentrant {
    DataTypes.ReserveData memory reserve = aavePool.getReserveData(address(DAI));
    uint256 userBalance = balances[msg.sender].rayMul(reserve.liquidityIndex);
    require(userBalance >= amount, "Not enough amount");
