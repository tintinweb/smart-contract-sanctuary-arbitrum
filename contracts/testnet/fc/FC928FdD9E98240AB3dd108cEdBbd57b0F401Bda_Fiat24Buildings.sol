...
import "./libraries/EnumerableUintToUintMap.sol";
...
contract Fiat24Buildings is Initializable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    mapping (uint256=>uint256) public buildingPriceList;
    ...
    function build(uint256 buildingType_) external {
        ...
        if(buildingPriceList[buildingType_] == 0 ) {
            revert Fiat24Buildings__BuldingNotAvailable();
        }
        if(f24.balanceOf(msg.sender) < buildingPriceList[buildingType_]) {
        ...
        if(buildingPriceList[buildingType_] > 0) {
            f24.burnFrom(msg.sender, buildingPriceList[buildingType_]);
        }
    ...
    function addBuilding(uint256 buildingType_, uint256 buildingPrice_) external {
        ...
        buildingPriceList[buildingType_] = buildingPrice_;
    }
...
library EnumerableUintToUintMap {
    ...
    */
    ...
    function get(UintToUintMap storage map, uint256 key) internal view returns (uint256) {
        return uint256(_get(map._inner, uint256(key)));
    }
    ...
    function get(UintToUintMap storage map, uint256 key, string memory errorMessage) internal view returns (uint256) {
        return uint256(_get(map._inner, uint256(key), errorMessage));
    }

    function getAll(UintToUintMap storage map) internal view returns( MapEntry[] memory) {
        return  map._inner._entries;
    }
}
