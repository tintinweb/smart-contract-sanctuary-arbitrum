    ...
    LiquidityPoolManagerImplementationPointerUpgradeable
{
  ...
  struct Tier {
    ...
    function initialize() external initializer {
        ...
        ipfsBaseURI = "ipfs://QmRixq9jye3NY6EgBuEPZuBmyVBV4wT7jfKrmKnSWHoEkh/";
    

        // Initialize contract
        changeNodeMinPrice(42_000 * (10**18)); // 42,000 CRIOS
        changeCompoundDelay(24800); // 8h - 3 compounds day/max
        changeTierSystem(
            [100000, 105000, 110000, 120000, 130000, 140000],  // TierLevel  
            [1000, 500, 100, 50, 10, 0] // TierSlope
        );
      ...
      Tier({
        ...
        compoundFee: 40,
        name: "Zeus",
        imageURI: string(abi.encodePacked(ipfsBaseURI, "zeus.jpg"))
      }),
      Tier({
        level: 4000,
        slope: 500,
        ...
        compoundFee: 20,
        name: "Poseidon",
        imageURI: string(abi.encodePacked(ipfsBaseURI, "poseidon.jpg"))
      }),
      Tier({
        level: 8000,
        slope: 500,
    ...
    // function mergeartifacts(uint256[] memory _artifactIds, string memory artifactName)
    //     external
    //     nonReentrant
    //     onlyArtifactOwner
    //     checkPermissionsMultiple(_artifactIds)
    //     whenNotPaused
    //     verifyName(artifactName)
    // {
    //     address account = _msgSender();
    //     require(
    //         _artifactIds.length > 1,
    //         "criosManager: At least 2 artifacts must be selected in order for the merge to work"
    //     );
    ...
    //         ArtifactEntity storage artifactFromIds = _artifacts[_artifactIds[i]];
    //         require(
    //             isProcessable(artifactFromIds),
    //             "criosManager: For the process to work, all selected artifacts must be compoundable. Try again later."
    //         );

    //         // Compound the artifact
    //         compoundReward(artifactFromIds.id);

    //         // Use this tier if it's lower than current
    //         if (lowestTier == 0) {
    //             lowestTier = artifactFromIds.rewardMult;
    //         } else if (lowestTier > artifactFromIds.rewardMult) {
    //             lowestTier = artifactFromIds.rewardMult;
    //         }

    //         // Additionate the locked value
    //         totalValue += artifactFromIds.artifactValue;

    //         // Burn the artifact permanently
    //         _burn(artifactFromIds.id);
    //     }

    //     require(
    //         lowestTier >= tierLevel[0],
    //         "criosManager: Something went wrong with the tiers"
    //     );
    ...
    //     zeus.accountReward(account, newArtifactValue);

    //     // Create the artifact (which will burn that amount)
    //     uint256 currentArtifactId = createArtifactWithTokens(
    //         artifactName,
    //         newArtifactValue
    //     );

    //     // Set tier, logTier and increase
    //     ArtifactEntity storage artifact = _artifacts[currentArtifactId];
    //     artifact.isMerged = true;
    //     if (lowestTier != tierLevel[0]) {
    //         logTier(artifact.rewardMult, -int256(artifact.artifactValue));
    //         artifact.rewardMult = lowestTier;
    //         logTier(artifact.rewardMult, int256(artifact.artifactValue));
    //     }

    //     emit Merge(_artifactIds, artifactName, totalValue);
    // }
    ...
    )
  {
   ...
   function getTier(uint256 mult) public view returns (Tier memory, uint256) {
    ...
    // This is the function who is called on frontEnd.
    function createArtifactWithTokens(
        ...
        logTier(tierLevel[0], int256(artifactValue));
    ...
    function cashoutRewardNoFees(uint256 _artifactId)
        external
        nonReentrant
        onlyArtifactOwner
        checkPermissions(_artifactId)
    ...
    function cashoutAllNoFees() external nonReentrant onlyArtifactOwner whenNotPaused {
        address account = _msgSender();
        uint256 rewardsTotal = 0;
        uint256[] memory artifactsOwned = getArtifactIdsOf(account);
        for (uint256 i = 0; i < artifactsOwned.length; i++) {
    ...
    function compoundAll() external nonReentrant onlyArtifactOwner whenNotPaused {
     ...
     function compoundAllNoFees() external nonReentrant onlyArtifactOwner whenNotPaused {
    ...
    function compoundRewardNoFees(uint256 _artifactId)
        public
        onlyArtifactOwner
        checkPermissions(_artifactId)
        whenNotPaused
        ...
        returns (uint256)
    {
        ...
        if (artifact.rewardMult != tierLevel[0]) {
            logTier(artifact.rewardMult, -int256(artifact.artifactValue));
            logTier(tierLevel[0], int256(artifact.artifactValue));
        }

        artifact.rewardMult = tierLevel[0];
    ...
    (Tier memory artifacts, ) = getTier(prevMult);
    return artifacts.slope;
  }
    ...
    function getTieredRevenues(uint256 mult) private view returns (uint256) {
    (Tier memory artifacts, ) = getTier(mult);
    return artifacts.dailyAPR;
  }
    ...
    function _calculateRewardsFromValue(
        ...
        return (rewardsMultiplicated * _artifactValue) / 100000;
    }
        ...
        returns (artifactInfoEntity[] memory)
    {
        ...
        for (uint256 i = 0; i < _artifactIds.length; i++) {
            ...
            uint256 amountToReward = calculateReward(artifact);
            uint256 amountToRewardDaily = rewardPerDayFor(artifact);
  ...
  function changeTiers(Tier[6] memory _newArtifs) public onlyOwner {
    ...
    function changeTierSystem(
        ...
        require(
            _tierLevel.length == 6,
            "artifacts: newTierLevels length has to be 6"
        );
        require(
            _tierSlope.length == 6,
            "artifacts: newTierSlopes length has to be 6"
        );
