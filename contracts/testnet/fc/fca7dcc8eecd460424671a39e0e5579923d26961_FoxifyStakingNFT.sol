...
contract FoxifyStakingNFT {
    ...
    uint256 public constant STUCK_AMOUNT = 125e18;
    uint256 public constant DISTRIBUTION_AMOUNT = 1250e18;
    uint256 public constant DIVIDER = 1e18;

    IERC721Enumerable public ownerToken;
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public userInfoAmount;
    mapping(uint256 => mapping(uint256 => mapping(address => uint256))) public userInfoRewardDebt;
    ...
    function availableToWithdraw(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            uint256 userAmount = userInfoAmount[_pid][tokenId];
            if (block.timestamp <= poolStartTime || userAmount <= STUCK_AMOUNT) continue;
            else {
                uint256 periodAmount = (DISTRIBUTION_AMOUNT * (block.timestamp - poolStartTime)) / pool.period;
                if (periodAmount > DISTRIBUTION_AMOUNT) periodAmount = DISTRIBUTION_AMOUNT;
                availableAmount += periodAmount - (DISTRIBUTION_AMOUNT - (userAmount - STUCK_AMOUNT));
            }
        }
    }
    ...
    function pendingShare(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            share +=
                ((userInfoAmount[_pid][tokenId] * accRewardSharePerShare_) / DIVIDER) -
                userInfoRewardDebt[_pid][tokenId][_token];
        }
    }
    ...
    event Deposit(uint256 indexed pid, uint256[] tokenIds);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 tokenId, uint256 amount);
    event RewardPaid(address indexed user, uint256 tokenId, IERC20 token, uint256 amount);
    ...
    error OwnerTokenZero();
    error ClaimNotOwner(uint256 tokenId, address caller, address owner);
    error WithdrawNotOwner(uint256 tokenId, address caller, address owner);
    error AlreadyDepositedForToken(uint256 tokenId);
    ...
    constructor(
        ...
        if (ownerToken_ == address(0)) revert OwnerTokenZero();
        ownerToken = IERC721Enumerable(ownerToken_);
    ...
    function add(
        ...
        if (_period == 0) revert PeriodZero();
    ...
    function deposit(uint256 _pid, uint256[] memory _tokenIds) external onlyOperator {
        PoolInfo storage pool = poolInfo[_pid];
        updatePool(_pid);
        uint256 amountPerId = STUCK_AMOUNT + DISTRIBUTION_AMOUNT;
        uint256 totalTransferAmount;
        if (_tokenIds.length > 0) {
            for (uint256 i = 0; i < _tokenIds.length; i++) {
                uint256 tokenId = _tokenIds[i];
                if (tokenId >= ownerToken.totalSupply()) revert InvalidTokenId(tokenId, ownerToken.totalSupply());
                if (userInfoAmount[_pid][tokenId] != 0) revert AlreadyDepositedForToken(tokenId);
                pool.totalAmount += amountPerId;
                totalTransferAmount += amountPerId;
                userInfoAmount[_pid][tokenId] += amountPerId;
                for (uint256 j = 0; j < _rewardToken.length; j++) {
                    address tokenAddress = address(_rewardToken[j]);
                    userInfoRewardDebt[_pid][tokenId][tokenAddress] =
                        (userInfoAmount[_pid][tokenId] * accRewardSharePerShare[_pid][tokenAddress]) /
                        DIVIDER;
                }
            }
            pool.token.safeTransferFrom(msg.sender, address(this), totalTransferAmount);
        }
        emit Deposit(_pid, _tokenIds);
    }

    function claim(uint256 _pid, uint256[] memory _tokenIds) public {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            if (tokenId >= ownerToken.totalSupply()) revert InvalidTokenId(tokenId, ownerToken.totalSupply());
            uint256 userAmount = userInfoAmount[_pid][tokenId];
            address to = ownerToken.ownerOf(tokenId);
            if (to != msg.sender) revert ClaimNotOwner(tokenId, msg.sender, to);
            if (userAmount > 0) {
                for (uint256 j = 0; j < _rewardToken.length; j++) {
                    IERC20 token = _rewardToken[j];
                    ...
                    if (_pending > 0) {
                        ...
                        token.safeTransfer(to, _pending);
                        emit RewardPaid(to, tokenId, token, _pending);
                    }
                }
            }
        }
    ...
    function withdraw(uint256 _pid, uint256[] memory _tokenIds) external {
        PoolInfo storage pool = poolInfo[_pid];
        updatePool(_pid);
        claim(_pid, _tokenIds);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            if (tokenId >= ownerToken.totalSupply()) revert InvalidTokenId(tokenId, ownerToken.totalSupply());
            address to = ownerToken.ownerOf(tokenId);
            if (to != msg.sender) revert WithdrawNotOwner(tokenId, msg.sender, to);
            uint256[] memory array = new uint256[](1);
            array[0] = tokenId;
            uint256 availableTokenAmount = availableToWithdraw(_pid, array);
            userInfoAmount[_pid][tokenId] -= availableTokenAmount;
            pool.token.safeTransfer(to, availableTokenAmount);
            emit Withdraw(msg.sender, _pid, tokenId, availableTokenAmount);
            for (uint256 j = 0; j < _rewardToken.length; j++) {
                address tokenAddress = address(_rewardToken[j]);
                userInfoRewardDebt[_pid][tokenId][tokenAddress] =
                    (userInfoAmount[_pid][tokenId] * accRewardSharePerShare[_pid][tokenAddress]) /
