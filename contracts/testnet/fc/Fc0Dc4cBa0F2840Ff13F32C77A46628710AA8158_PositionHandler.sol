...
contract PositionHandler is PositionConstants, IPositionHandler, Ownable {
    ...
    function _increaseMarketPosition(
        ...
        uint256 pendingCollateral = _fromTokenToUSD(_order.pendingCollateral, _prices[_prices.length - 1], tokenDecimals);
        uint256 pendingSize = _fromTokenToUSD(_order.pendingSize, _prices[_prices.length - 1], tokenDecimals);
        _increasePosition(
            _path[0],
            pendingCollateral,
            pendingSize,
    ...
    function modifyPosition(
        ...
        require(_path.length == _prices.length && _path.length > 0, "IVLAL");
        ...
        if (_txType == ADD_COLLATERAL || _txType == REMOVE_COLLATERAL) {
            ...
            _addTrailingStop(key, _isLong, params, order, _prices[0]);
        } else if (_txType == UPDATE_TRAILING_STOP) {
            (OrderInfo memory order) = abi.decode(_data, ((OrderInfo)));
            _updateTrailingStop(key, _isLong, _prices[0], order);
        } else if (_txType == CANCEL_PENDING_ORDER) {
            (Position memory position, OrderInfo memory order) = abi.decode(_data, ((Position), (OrderInfo)));
            _cancelPendingOrder(key, position, order);
            ...
            (address account, Position memory position, OrderInfo memory order) = abi.decode(_data, ((address), (Position), (OrderInfo)));
            require(account == position.owner, "IVLPO"); //Invalid positionOwner
            _triggerPosition(
                key,
                _isLong,
                _posId,
    ...
    function _addOrRemoveCollateral(
        ...
        if (_txType == ADD_COLLATERAL) {
            ...
            positionKeeper.increasePoolAmount(_path[0], _isLong, _amountIn);
            positionKeeper.unpackAndStorage(_key, abi.encode(_position), DataType.POSITION);
        } else {
            _position.collateral -= _amountIn;
            vaultUtils.validateSizeCollateralAmount(_position.size, _position.collateral);
            vaultUtils.validateLiquidation(_position.owner, _path[0], _isLong, true, _prices[0], _position);
            _position.reserveAmount -= _amountIn;
    ...
    function _triggerPosition(
        ...
        uint8 statusFlag = vaultUtils.validateTrigger(_isLong,_prices[0], _order);
    ...
    function _decreasePosition(
        ...
        require(_position.size <= _sizeDelta, "ISFPS/EXD"); //Insufficient position size, exceeded
        ...
        {
            (usdOut, usdOutFee) = _reduceCollateral(_position.owner, _indexToken, _sizeDelta, _isLong, _posId, _prices[0], _position);
        }
        ...
        if (_position.size != _sizeDelta) {
            ...
            vaultUtils.validateSizeCollateralAmount(_position.size, _position.collateral);
            vaultUtils.validateLiquidation(_position.owner, _indexToken, _isLong, true, _prices[0], _position);
    ...
    function _increasePosition(
        ...
        vaultUtils.validateLiquidation(_position.owner, _indexToken, _isLong, true, _indexPrice, _position);
    ...
    */
    ...
    function getLastPrice(address _token) external view returns (uint256, uint256) {
...
interface IRouter {
    ...
    Params length must between 4 to 6.
        For all orders: 
        ...
        Market order:
            ...
            params[4] is deadline (must be bigger than 0), if the transaction is delayed, check this deadline for executing or reverting
            params[5] is amount out min if the collateral token is not stable, we will swap to a stable following path, 
