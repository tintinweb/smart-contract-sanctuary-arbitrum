    ...
    ISmolRingStaking
{
    ...
    function claim(ICreatureOwnerResolverRegistry.Creature memory creature) external nonReentrant {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            if (currentRewards[i] > 0) {
                ...
                uint256 claimedFidelityFee = claimedFidelity[creature.ownerResolver][creature.tokenId][i];
                claimedFidelity[creature.ownerResolver][creature.tokenId][i] = 0;
                ...
                ITokenDistributor(rewardTokenStateMapping[rewardTokens[i]].tokenDistributor).payout(
                    msg.sender,
                    currentRewards[i] - currentFidelityFee
                );
            }
        }
        lastRewardPayoutTimestamp[creature.ownerResolver][creature.tokenId] = block.timestamp;
    ...
    function claimFidelity(ICreatureOwnerResolverRegistry.Creature memory creature, address account)
        public
        onlyOperator
        nonReentrant
    {
        require(!emergencyShutdown, "ABSTRACTSTAKING:SHUTDOWN");
        require(isOwner(account, creature), "ABSTRACTSTAKING:NOT_OWNER_OF_CREATURE");
        require(hasFidelity(creature), "ABSTRACTSTAKING:NO_FIDELITY_AMOUNT_AVAILABLE");
        _accrueRewards(creature, true);
        uint256[] memory toClaim = claimableFidelityOf(creature);
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            if (toClaim[i] > 0) {
                ITokenDistributor(rewardTokenStateMapping[rewardTokens[i]].tokenDistributor).payout(
                    account,
                ...
                fidelity[creature.ownerResolver][creature.tokenId][i] = 0;
