...
contract CheckinUpgradeable is
Pausable,
Initializable,
ReentrancyGuard
{
    ...
    uint256 public defaultCheckinPrice;
    ...
    function initialize() public initializer {
        ...
        /// Set default egg price is 0.1 USDT
        defaultCheckinPrice = 0.1 * 1e18;
        _createCheckin(msg.sender);
    ...
    function _createCheckin(
        ...
        Checkin memory newCheckin = Checkin(block.timestamp, _ownerAddress);
        checkin.push(newCheckin);
        uint256 _checkinId = checkin.length - 1;
        return _checkinId;
    }
    ...
    /// @param _newCheckinPrice - Set new value for egg price
    function setDefaultCheckinPrice(uint256 _newCheckinPrice) external onlyAdmin {
        defaultCheckinPrice = _newCheckinPrice;
    }

    ///@dev Get total existing egg on the market
    function getTotalCheckin() external view returns (uint256) {
        return checkin.length;
    }

    function getCheckinList() external view returns (Checkin[] memory) {
        return checkin;
    }
