...
contract StakingBGT is Ownable {
    ...
    function deposit(uint256 amount) external {
        ...
        recycleDPos(msg.sender);

        // Update the balance and the dpos
        poss[msg.sender] += amount;
        totalPos += amount;
        
        addDPosMoves(msg.sender, amount);
        updateSuperDPos(msg.sender);
    ...
    function doUpdateDPosToSuper() external {
        bool isUpdated = recycleDPos(msg.sender);
    ...
    function claimInterest() external {
        ...
        bgtToken.transfer(msg.sender, interest);
    ...
    // 变更前，先收回
    function recycleDPos(address account) internal returns (bool) {
        bool result = false;
        ...
        if (superFrom != address(0))
        {
            ...
            uint256 dposFrozen = dposMoves[account].dposFrozen;
            if (dpos > 0)
            {
            ...
            else if (dposFrozen > 0)
            {
                address super_ = dposMoves[account].super_;
                dpossFrozen[super_] -= dposMoves[account].dposFrozen;

                dposMoves[account].dpos = dposMoves[account].dposFrozen;
                dposMoves[account].dposFrozen = 0;
            }
            {
                // 添加有效上级
                uint256 dposDquity = getDPosDquity();
                address root = getSuperRoot(account, dposDquity);
                if (root != dposMoves[account].root)
                {
    ...
    // 变更后，更新DPOS信息
    function addDPosMoves(address account, uint256 amount) internal {
        address superFrom = invitation.getSuper(account);
        if (superFrom != address(0))
        {
            // 收回
            ...
            {
                dposMoves[account] = DPosMove(superFrom, address(0), dpos + amount, 0);
            }
        }
    }
    ...
    // 根据DPOS信息，添加DPOS
    function updateSuperDPos(address account) internal {
        uint256 dpos = dposMoves[account].dpos;
        if (dpos > 0)
        {
            if (dposMoves[account].root != address(0))
            {
                address root = dposMoves[account].root;
                settlementInterest(root);
                dposs[root] += dpos; 
            ...
            else 
            {
                ...
                {
                    dposMoves[account].dposFrozen = dposMoves[account].dpos;
                    dposMoves[account].dpos = 0;
                    dpossFrozen[super_] += dpos;
                }
            }
        }
    }
    ...
    function getSuperRoot(address account, uint256 dposDquity) internal view returns (address) {
        ...
        while (true)
        {
            ...
            super_ = invitation.getSuper(super_);
            if (super_ == address(0))
                break;
            if (poss[super_] >= dposDquity)
                break;
    ...
    function doSettlementInterest() external {
        ...
        for (uint i=userIndex; i<length; i++)
        {
            ...
            interest = annualInterestRateConfig[i].annualInterestRate * elapsedTime * (poss[account] + dposs[account] + fposs[account]) / (totalPos + totalFPos + totalDPos) / (365 days) / 100;
        }
