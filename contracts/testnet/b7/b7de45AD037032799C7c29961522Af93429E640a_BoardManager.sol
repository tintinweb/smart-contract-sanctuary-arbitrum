...
contract BoardManager is Ownable, IBoardManager {
    ...
    // Creates a board with `numPlayers` and `bigBlindSize` when starting a new game.
    // This also automatically calls `join` for `msg.sender`. Returns the newly created board id.
    function createBoard(
        ...
        boards[boardId].themeId = themeId;
    ...
    function fold(
        ...
        if (boards[boardId].numUnfoldedPlayers == 1) {
            ...
            uint256[] memory winnerChips = new uint256[](
                boards[boardId].numPlayers
            );
            winnerChips[winnerIndex] = chipManager.getPotSize(boardId);
            boards[boardId].winnerChips = winnerChips;
    ...
    function settleWinner(uint256 boardId) internal {
        ...
        boards[boardId].winnerChips = winningAmounts;
    ...
    function quit(uint256 boardId) external {
        ...
        Chip memory chip = chipManager.getChip(boardId);
        uint256[] memory amounts = new uint256[](players.length);
        for (uint8 i = 0; i < players.length; ++i) {
            // if it's the quitter, all his chips will not be returned
            amounts[i] = index != i
                ? chipManager.getChipSinglePlayer(boardId, i) + chip.bets[i]
                : 0;
        }
        accountManager.settle(boardId, players, amounts, collectVig, true);
        boards[boardId].stage = GameStage.Ended;
    }
}
...
contract ChipManager is Ownable, IChipManager {
    ...
    function bet(
        ...
        if (
            ...
            (betType == BetType.AllIn &&
                chips[boardId].bets[playerIndex] > chips[boardId].lastRaise)
        ) {
            chips[boardId].raiseBeforeLastRaise = chips[boardId].lastRaise;
    ...
    // `winningAmounts` indicates the amount of chips won by each player.
    // `totalAmounts` indicates the amount of chips won by each player plus remaining chips not bet yet.
    // Note: `scores` match the order of `chips[boardId].players`
    function settle(
        uint256 boardId,
        uint256[] memory scores
        ...
        uint256[] memory winningAmounts = getSettleAmount(
            chips[boardId].potSize,
            sortedBets,
            indices,
            groupCount
        );
        uint256[] memory totalAmounts = new uint256[](winningAmounts.length);
        for (uint8 i = 0; i < winningAmounts.length; i++) {
            totalAmounts[i] = winningAmounts[i] + chips[boardId].chips[i];
        }
        return (totalAmounts, winningAmounts);
    }
