    ...
    AccessControl
{
    ...
    function setConfigure(
        ...
        emit CreateContract(address(config), assetPair);
    }

    /**
     * @notice Pauses/Unpauses the option creation
     */
    function setCreation(bool _isPaused) public onlyRole(DEFAULT_ADMIN_ROLE) {
        isPaused = _isPaused;
        emit Pause(isPaused);
    }
    ...
    function createFromRouter(
        ...
        optionIdToTournament[optionID] = optionParams.tournamentId;
        _mint(optionParams.user, optionID);
        uint256 settlementFee = optionParams.totalFee - option.premium;

        emit Create(
            optionParams.user,
            optionID,
            optionParams.tournamentId,
            settlementFee,
            optionParams.totalFee
        );
    }
    ...
    function unlock(
        ...
        if (
            ...
            emit Expire(optionID, option.totalFee, priceAtExpiration);
        }

        ITournamentLeaderboard(tournamentManager.leaderboard()).updateUserRank(
            ownerOf(optionID),
            optionIdToTournament[optionID],
            option.state == State.Expired
                ? -int256(option.totalFee)
                : int256(option.lockedAmount - option.totalFee),
            option.totalFee
        );
        _burn(optionID);
    }
    ...
    function runInitialChecks(
        ...
        require(totalFee < config.maxFee(), "O36");
    }
    ...
    function getAmount(
        ...
        (settlementFeePercentage) = _getSettlementFeePercentage(
            optionParams.user,
        ...
        amount = (optionParams.totalFee * 10 ** decimals()) / unitFee;
    }
     ...
     */
    function _getbaseSettlementFeePercentage(
        bool isAbove
    ) internal view returns (uint16 baseSettlementFeePercentage) {
    ...
    function _exercise(
        ...
        emit Exercise(user, optionID, option.lockedAmount, priceAtExpiration);
    }
    ...
    function _getSettlementFeeDiscount(
        address user,
        uint256 traderNFTId
    ) public view returns (uint8 maxStep) {
    ...
    function _getSettlementFeePercentage(
        ...
        uint256 maxStep;
        maxStep = _getSettlementFeeDiscount(user, traderNFTId);
        settlementFeePercentage =
            settlementFeePercentage -
            (stepSize * maxStep);
    }
