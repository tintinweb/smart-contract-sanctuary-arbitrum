...
contract F24TimeLock is Initializable {
    struct LockedAmount{
    ...
    error Fiat24TimeLock__NotEnoughTokensLocked();
    error F24TimeLock__NoTokensLocked();
    error F24TimeLock__LockTransferFailed();
    error F24TimeLock__ClaimTransferFailed();

    function initialize(address tokenAddress_, address fiat24AccountAddress) public initializer {
    ...
    function lock(uint256 amount_) external {
        ...
        lockedAmounts[tokenId].unlockTime = block.timestamp + THIRTYDAYS;
    ...
    function claim(uint256 amount_) external {
        ...
        if(lockedAmounts[tokenId].lockedAmount < amount_) {
            revert Fiat24TimeLock__NotEnoughTokensLocked();
        }
        lockedAmounts[tokenId].lockedAmount -= amount_;
    ...
    function claimableAmount() external view returns(uint256) {
        uint256 tokenId = getTokenId(msg.sender);
        if(tokenId == 0
           || lockedAmounts[tokenId].unlockTime > block.timestamp) {
            return 0;
        } else {
            return lockedAmounts[tokenId].lockedAmount;
        }
    }
