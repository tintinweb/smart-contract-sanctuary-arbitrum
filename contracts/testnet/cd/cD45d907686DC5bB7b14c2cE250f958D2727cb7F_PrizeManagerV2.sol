...
contract PrizeManagerV2 is IPrizeManagerV2, ReentrancyGuard, Ownable {
    ...
    function _claim(uint256 _gameId, address _winner) internal {
        ...
        require(_prizeInfo.expire > block.timestamp , 'PrizeManagerV2: expire');
    ...
    function _liquidate(uint256 _gameId, address _winner) internal {
        PrizeInfo memory _prizeInfo = prizeInfo[_gameId];
        require(_prizeInfo.expire < block.timestamp , 'PrizeManagerV2: !expire');
        if (isClaimable[_winner][_gameId] != 0) {
            isClaimable[_winner][_gameId] = 0;
            _transfer(_prizeInfo.token, treasury, _prizeInfo.amount);
            SidePrizeInfo memory _sidePrize = sidePrizeInfo[_gameId];
            if (_sidePrize.exist) {
                for (uint256 i = 0; i < _sidePrize.tokens.length; i++) {
                    // transfer token
                    _transfer(_sidePrize.tokens[i], treasury, _sidePrize.amounts[i]);
                }
            }
        }
    ...
    function liquidate(uint256 _gameId, address[] memory _winners) external {
        for (uint256 i = 0; i < _winners.length; i++) {
            _liquidate(_gameId, _winners[i]);
        }
    }
