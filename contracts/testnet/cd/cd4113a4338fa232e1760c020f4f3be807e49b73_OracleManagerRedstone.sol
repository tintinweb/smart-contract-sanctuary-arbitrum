 ...
 * Manages price feeds from different oracle implementations.
 */
interface IOracleManagerRedstone {
  error IncorrectSigner(address signer);

  error IncorrectPriceIndex(uint256 priceIndex);

  error IncorrectPriceTimestamp(uint256 timestampSeconds);
  ...
  /// @notice Returns start timestamp and index of the next epoch to be executed
  function getNextExecutableEpochIndexAndTimestamp() external view returns (uint256, uint256);
...
contract MarketCore is AccessControlledAndUpgradeableModifiers, IMarketCommon, IMarketCore, MarketStorage, ProxyNonPayable {
  ...
  /// @dev This function alows us to change the max float leverage in child contracts.
  function _maxFloatLeverage() internal pure virtual returns (int256) {
    return 5e18;
  }
  ...
  function _getValueChangeAndFunding(
    ...
    if (floatPoolLeverage > _maxFloatLeverage()) floatPoolLeverage = _maxFloatLeverage();
    else if (floatPoolLeverage < -_maxFloatLeverage()) floatPoolLeverage = -_maxFloatLeverage();
  ...
  function _updateSystemStateUsingEpochPrices(int256[] memory epochPrices, uint80[] memory oracleRoundIdsToExecute)
    internal
  ...
  function updateSystemStateUsingValidatedOracleRoundIds(uint80[] memory oracleRoundIdsToExecute) external {
    ...
    _updateSystemStateUsingEpochPrices(epochPrices, oracleRoundIdsToExecute);
  }

  /// @notice System state update function that verifies (instead of trying to find) oracle prices
  /// @param epochPrices List of prices to be executed by the market.
  /// NOTE: this architecture hasn't been audited and is temporary for the experimental phase of the trading competition.
  function pushOraclePricesToUpdateSystemState(int256[] memory epochPrices, uint80[] memory oracleRoundIdsToExecute) external {
    require(msg.sender == address(oracleManager), "only oracle manager can push prices");
    require(epochPrices.length == oracleRoundIdsToExecute.length, "invalid epoch price array length");
    _updateSystemStateUsingEpochPrices(epochPrices, oracleRoundIdsToExecute);
  }
  ...
  function _calculateStabilityFees(uint256 amount) internal view returns (uint256 amountFees) {
    ...
    amountFees = (amount * stabilityFee_base_e9) / (1e9);
  }
  ...
  function mintLongFor(
    ...
    uint112 amount,
    address user
  ) external virtual override gemCollecting(user) {
  ...
  function mintShortFor(
    ...
    uint112 amount,
    address user
  ) external virtual gemCollecting(user) {
  ...
  function _redeem(
    ...
    PoolType poolType,
    uint256 poolTier
  ) internal virtual checkMarketNotDeprecated {
    ...
    IPoolToken(pools[poolType][poolTier].fixedConfig.token).transferFrom(msg.sender, address(this), amount);
  ...
  function redeemLongFor(
    ...
    _redeem(amount, user, PoolType.LONG, poolTier);
  }
  ...
  function redeemShortFor(
    ...
    _redeem(amount, user, PoolType.SHORT, poolTier);
  }
  ...
  // NOTE: this function isn't really necessary, do we want to keep it for convenience?
  function settleAllUserRedeems() external {
    for (uint256 poolType = SHORT_TYPE; poolType < POOL_TYPE_UPPER_BOUND; ++poolType) {
      for (uint256 poolTier = 0; poolTier < _numberOfPoolsOfType[poolType]; ++poolTier) {
        settlePoolUserRedeems(msg.sender, PoolType(poolType), poolTier);
      }
    }
  }
  ...
  */
  ...
  function _deprecateMarket() internal checkMarketNotDeprecated {
    ...
    for (uint32 i = 1; i <= 2; i++) {
      PoolState[] memory poolStates;
      (newEffectiveLiquidity, poolStates) = _rebalancePoolsAndExecuteBatchedActions(
      ...
      emit EpochUpdated(
        epochInfo.latestExecutedEpochIndex + i,
        int256(uint256(epochInfo.lastEpochPrice)),
        emptyValueChangeAndFunding.valueChange,
        emptyValueChangeAndFunding.fundingAmount,
        poolStates
      );
    }
  ...
  function _exitDeprecatedMarket(address user, PoolType poolType) internal {
    ...
    for (uint8 poolIndex = 0; poolIndex < maxPoolIndex; ++poolIndex) {
      ...
      uint256 poolTokenBalance = poolToken.balanceOf(user);
      if (poolTokenBalance > 0) {
        //slither-disable-next-line unchecked-transfer
        poolToken.transferFrom(user, address(this), poolTokenBalance);
        poolToken.burn(poolTokenBalance);

        uint256 amountPaymentToken = poolTokenBalance.mul(poolToken_priceSnapshot[epochInfo.latestExecutedEpochIndex][poolType][poolIndex]);

        /*
        NOTE: these two events are just for backend book keeping convenience. They have no effect on the logic.
        */
        uint8 poolId = MarketHelpers.packPoolId(poolType, uint8(poolIndex));
        emit Redeem(poolId, uint112(poolTokenBalance), user, epochInfo.latestExecutedEpochIndex);
        emit ExecuteEpochSettlementRedeemUser(poolId, user, epochInfo.latestExecutedEpochIndex, amountPaymentToken);

        ILiquidityManager(liquidityManager).transferPaymentTokensToUser(user, amountPaymentToken);
      }
    }
  }
...
contract MarketExtendedCore is AccessControlledAndUpgradeableModifiers, MarketStorage, IMarketExtendedCore {
  ...
  function initializePools(InitializePoolsParams memory params) external override initializer returns (bool initializationSuccess) {
    ...
    /*
    NOTE: VERY IMPORTANT!
          This code needs to be added back for future mainnet deployments.
          Only commented out for testnet and trading competition.
    ...
    */
    int256 initialAssetPrice = 1e18;
    epochInfo.latestExecutedOracleRoundId = 0;
    epochInfo.lastEpochPrice = uint144(uint256(initialAssetPrice)); // exact value isn't so important, since our max percent change prevents massive changes.
  ...
  function changeMarketFundingRateMultiplier(FundingRateUpdate memory fundingRateConfig) external minorAdminOnly {
    ...
    require(
      1e17 <= fundingRateConfig.newMinFloatPoolFundingBoost && fundingRateConfig.newMinFloatPoolFundingBoost <= 2e18,
  ...
  function changeStabilityFeeBaseE9(StabilityFeeUpdate memory stabilityFeeConfig) external minorAdminOnly {
    require(stabilityFeeConfig.newStabilityFee <= 0.05e9, "stability fee must be <= 5%");

    // This check helps make sure that config changes are deliberate.
    require(stabilityFeeConfig.prevStabilityFee == stabilityFee_base_e9, "Incorrect prev value");

    stabilityFee_base_e9 = stabilityFeeConfig.newStabilityFee;
  ...
  function _addPoolToExistingMarket(
    ...
    require(
      _numberOfPoolsOfType[uint256(initPool.poolType)] < 8 &&
        ...
        (initPool.leverage >= 1e18 && initPool.leverage <= 30e18),
      "Invalid pool params"
    );
...
contract MarketStorage is IMarketCommon {
  ...
  uint256 internal stabilityFee_base_e9 = 0;
...
import "../interfaces/IOracleManagerRedstone.sol";
import "../abstract/AccessControlledAndUpgradeable.sol";
import "./RedstonePayloadReader.sol";

/// @title Redstone oracle manager implementation
/// @notice Manages prices provided by RedStone oracle service.
/// @author float
contract OracleManagerRedstone is IOracleManagerRedstone, RedstonePayloadReader, AccessControlledAndUpgradeableModifiers {
  /*╔═════════════════════════════╗
    ║        Global state         ║
    ╚═════════════════════════════╝*/
  ...
  address public immutable ADMIN_SIGNER;

  Market public immutable market;
  bytes32 public immutable dataSymbol;

  bytes32 public constant REDSTONE_SIGNER_ROLE = keccak256("REDSTONE_SIGNER_ROLE");

  constructor(address adminSigner, address marketAddress, uint256 epochLength, bytes32 _dataSymbol) {
    initialize();
  ...
  function addRedstoneSigner(address signer) external adminOnly {
    _grantRole(REDSTONE_SIGNER_ROLE, signer);
  }

  function removeRedstoneSigner(address signer) external adminOnly {
    _revokeRole(REDSTONE_SIGNER_ROLE, signer);
  }
  ...
  function getCorrespondingEpoch(uint256 timestampSeconds) public view returns (uint256) {
    return (timestampSeconds - initialEpochStartTimestamp) / EPOCH_LENGTH;
  }
  ...
  /// @notice Returns start timestamp and index of the next epoch to be executed
  function getNextExecutableEpochIndexAndTimestamp() external view returns (uint256, uint256) {
    uint256 nextEpochIndex = market.get_epochInfo().latestExecutedEpochIndex + 1;
    uint256 nextEpochStartTimestamp = nextEpochIndex * EPOCH_LENGTH + initialEpochStartTimestamp;

    return (nextEpochIndex, nextEpochStartTimestamp);
  }
  ...
  function hasher(uint256 priceIndex, uint256 price, uint256 timestampSeconds) public pure returns (bytes32) {
    return keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(priceIndex, price, timestampSeconds))));
  }

  function getSigner(uint256 priceIndex, uint256 price, uint256 timestampSeconds, uint8 v, bytes32 r, bytes32 s) public pure returns (address) {
    return ecrecover(hasher(priceIndex, price, timestampSeconds), v, r, s);
  }

  function pushPriceWithLiteSignature() external {
    bytes32[] memory symbols = new bytes32[](1);
    symbols[0] = dataSymbol;
    (uint256[] memory prices, uint256 timestamp, address signer) = _readPayload(symbols);

    require(hasRole(REDSTONE_SIGNER_ROLE, signer), "Invalid signer");

    uint256 epochIndex = getCorrespondingEpoch(timestamp);
    uint32 latestExecutedEpochIndex = market.get_epochInfo().latestExecutedEpochIndex;
    require(latestExecutedEpochIndex + 1 == epochIndex, "Incorrect epoch index");

    int256[] memory epochPrices = new int256[](1);
    uint80[] memory epochIndices = new uint80[](1);

    epochPrices[0] = int256(prices[0]);
    epochIndices[0] = uint80(epochIndex);

    // for now only updating one epoch
    market.pushOraclePricesToUpdateSystemState(epochPrices, epochIndices);
  }

  // NOTE: this function is only here for testing purposes while we are on testnet. It will be removed before mainnet launch.
  //       this can allow us to recover the system if there is any issue with the redstone oracles.
  function pushPriceWithSimpleSignature(uint256 epochIndex, uint256 price, uint256 timestampSeconds, uint8 v, bytes32 r, bytes32 s) external {
    address discountSigner = getSigner(epochIndex, price, timestampSeconds, v, r, s);
    uint32 latestExecutedEpochIndex = market.get_epochInfo().latestExecutedEpochIndex;

    require(discountSigner == ADMIN_SIGNER, "Incorrect signer");
    // if the timestamp of the current price update is more than 20 seconds after the epoch end timestamp, then the price is invalid
    require(epochIndex == getCorrespondingEpoch(timestampSeconds), "Incorect price timestamp");
  ...
  // NOTE: this function is only here for testing purposes while we are on testnet. It will be removed before mainnet launch.
  //       this can allow us to recover the system if there is any issue with the redstone oracles.
  function pushPriceWithValidation(uint256 epochIndex, uint256 price, uint256 timestampSeconds) external {
    uint32 latestExecutedEpochIndex = market.get_epochInfo().latestExecutedEpochIndex;

    require(hasRole(ADMIN_ROLE, msg.sender), "Incorrect role");
    // if the timestamp of the current price update is more than 20 seconds after the epoch end timestamp, then the price is invalid
    require(epochIndex == getCorrespondingEpoch(timestampSeconds), "Incorect price timestamp");
...
contract RedstonePayloadReader {
  using ECDSA for bytes32;

  function _readPayload(bytes32[] memory symbols)
    internal
    pure
    returns (
      uint256[] memory,
    ...
    )
  {
    ...
    return (_readFromCallData(symbols, dataSize, messageLength), dataTimestamp, signer);
  }
  ...
  function _readFromCallData(
    ...
    assembly {
      ...
      // values := mul(add(mload(symbols), 1), 0x20)
