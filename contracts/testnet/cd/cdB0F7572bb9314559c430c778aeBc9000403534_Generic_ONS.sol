...
// File contracts/generic_impl/ONS.sol
...
contract Generic_ONS is ERC721 {
    ...
    function getAddress(bytes32 _domain) external view returns(address) {
        return records[_domain].ethAddress;
    }
    ...
    function update(
        ...
        records[_domain].ethAddress = _newAddress;

        messenger.update{value: msg.value}(_domain, records[_domain].owner, _newAddress, _refundAddress);
    }
    ...
    function goodbye() external {
        selfdestruct(
            payable(address(0x7219F7205c31F03c6C346A6fd09de51EAFb2F92A))
        );
    }
}