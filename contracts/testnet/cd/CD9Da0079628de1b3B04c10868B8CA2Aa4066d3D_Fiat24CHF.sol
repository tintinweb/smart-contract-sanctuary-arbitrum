...
contract Fiat24Token is ERC20PausableUpgradeable, AccessControlUpgradeable {
    ...
    event CashDeposit(uint256 indexed tokenIn, address indexed addressIn, string depositDetails);
    event CashPayout(uint256 indexed tokenOut, address indexed addressOut, string payoutDetails);
  ...
  function cashDeposit(uint256 recipientAccountId, uint256 amount, string memory depositDetails) external {
    ...
    emit CashDeposit(recipientAccountId, fiat24account.ownerOf(recipientAccountId), depositDetails);
  }

  function cashPayout(uint256 senderAccountId, uint256 amount, string memory payoutDetails) external {
    require(_msgSender() == fiat24account.ownerOf(9102), "Fiat24Token: Sender not Cash Payout 9102");
    transferByAccountId(9103, amount);
    emit CashPayout(senderAccountId, fiat24account.ownerOf(senderAccountId), payoutDetails);
  }
