...
interface IAviveGenesisNFT is IERC721 {
    function mint(address to, uint256 tokenId) external;
}

contract AviveNFTMinter is Ownable, ReentrancyGuard {
    struct MintRound {
        uint32 id;
        uint64 startTime;
        uint64 endTime;
    ...
    IAviveGenesisNFT public immutable GENESIS_NFT;
    ...
    modifier isNFTOwner(uint nftID) {
        require(GENESIS_NFT.ownerOf(nftID) == msg.sender, 'not owner');
        _;
    }

    constructor(address verifier_, address nft_) {
        verifier = verifier_;
        GENESIS_NFT = IAviveGenesisNFT(nft_);
    }
    ...
    function _mint(uint256 nftId) internal {
        GENESIS_NFT.mint(msg.sender, nftId);
    }
