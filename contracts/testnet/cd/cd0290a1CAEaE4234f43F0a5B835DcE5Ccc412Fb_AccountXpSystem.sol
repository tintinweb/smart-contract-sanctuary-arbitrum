...
import {AccountXpTrackerComponent, Layout as AccountXpTrackerComponentStruct, ID as ACCOUNT_XP_TRACKER_COMPONENT_ID} from "../generated/components/AccountXpTrackerComponent.sol";
...
contract AccountXpSystem is IAccountXpSystem, GameRegistryConsumerUpgradeable {
        ...
        onlyRole(GAME_LOGIC_CONTRACT_ROLE)
    {
        ...
        uint256 currentXp = accountXpTracker.getValue(entity).currentAccountXp;
        ...
        accountXpTracker.setValue(
            entity,
            AccountXpTrackerComponentStruct(newAccountXp)
        );
    }
    ...
    function _checkIfQualifiesForTradeLicense(
        ...
        // Get Account Xp threshold for TradeLicense
        uint256 tradeLicenseThreshold = IGameGlobals(
            _getSystem(GAME_GLOBALS_ID)
        ).getUint256(TRADE_LICENSE_THRESHOLD);
        // Check if new account xp is greater than max threshold and grant TradeLicense
        if (newAccountXp >= tradeLicenseThreshold) {
