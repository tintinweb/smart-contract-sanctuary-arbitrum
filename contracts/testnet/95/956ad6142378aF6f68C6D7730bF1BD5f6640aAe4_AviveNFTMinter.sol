...
contract AviveNFTMinter is AviveGenesisNFT, ReentrancyGuard {
    ...
    // mintHistory for each avive user, key is mintRoundId, then avive id
    // an avive user can only mint once in a round
    mapping(uint32 => mapping(uint256 => bool)) public mintHistory;
    ...
    function setupMintRound(
        ...
        if (latestRoundId != 0) {
            ...
            require(
                mintRounds[latestRoundId].endTime < block.timestamp,
            ...
            require(
                rewardRounds[latestRoundId].endTime < block.timestamp,
    ...
    function setupRewardRound(
        ...
        require(
            latestRoundId != 0 && mintRound.endTime < block.timestamp,
            'mintRound not ended'
        );
        ...
        require(
            winnerCount > 0 && winnerCount <= mintRound.mintCount,
            'winnerCount invalid'
        );
        ...
        bytes32 seed = keccak256(
            abi.encodePacked(blockhash(block.number - 1), mintRound.reserve)
        );
    ...
    function genesisMint(
        ...
        require(mintHistory[roundId][aviveId] == false, 'already minted');
        require(mintRound.mintCount < mintRound.total, 'reach max mint count');
        uint256 nftId = mintRound.startId + mintRound.mintCount;
        _safeMint(_msgSender(), nftId);
        mintRound.reserve += mintRound.fee;
        mintRound.mintCount += 1; // current round count
        totalMintCount += 1; // total count
        mintHistory[roundId][aviveId] = true;
    ...
    function claim(
        ...
        RewardRoud memory rewardRound = rewardRounds[roundId];
