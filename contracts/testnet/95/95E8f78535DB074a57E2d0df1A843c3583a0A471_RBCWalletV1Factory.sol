...
contract RBCWalletV1 is BaseAccount, SocialRecovery, TokenCallbackHandler, Initializable, PausableUpgradeable, UUPSUpgradeable {
    ...
    event RBCWalletInitialized(IEntryPoint indexed entryPoint, address indexed owner);

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(IEntryPoint anEntryPoint) SocialRecovery() {
    ...
    function getOwner() view internal override returns (address) {
        return owner;
    }
     ...
     */
    function initialize(address anOwner, bytes32[] memory guardianAddrHashes, uint256 _threshold) public virtual initializer {
        _initialize(anOwner, guardianAddrHashes, _threshold);
    }

    function _initialize(address anOwner, bytes32[] memory guardianAddrHashes, uint256 _threshold) internal virtual {
     ...
     */
    function execute(address dest, uint256 value, bytes calldata func) nonReentrant whenNotPaused external {
...
contract RBCWalletV1Factory {
     ...
     */
    function createAccount(address owner, uint256 salt, bytes32[] memory guardianAddrHashes, uint256 _threshold) public returns (RBCWalletV1 ret) {
        address addr = getAddress(owner, salt, guardianAddrHashes, _threshold);
        ...
        ret = RBCWalletV1(payable(new ERC1967Proxy{salt : bytes32(salt)}(
            ...
            abi.encodeCall(RBCWalletV1.initialize, (owner, guardianAddrHashes, _threshold))
        )));
    }
     ...
     */
    function getAddress(address owner, uint256 salt, bytes32[] memory guardianAddrHashes, uint256 _threshold) public view returns (address) {
 ...
 * @notice Implements smart contract wallet social recovery functions. 
 * Preserves privacy of guardian addresses until in recovery mode.
 */
abstract contract SocialRecovery is ReentrancyGuard {
    /************************************************
     *  Storage Variables
    ***********************************************/

    /**
     * @notice Stores list of guardian hashes.
     */
    mapping(bytes32 => bool) public isGuardian;

    /**
     * @notice Determines the guardian threshold required for completion of social recovery.
     */
    ...
    /**
     * @notice Current round of recovery the wallet is in.
     */
    uint256 public currRecoveryRound;

    /**
     * @notice <apping for bookkeeping when swapping guardians.
     */
    ...
    ***********************************************/
    ...
    modifier onlyGuardian {
        require(isGuardian[keccak256(abi.encodePacked(msg.sender))], "Not a guardian");
        _;
    }
    ...
    ***********************************************/
     ...
     */
    function getOwner() virtual internal returns (address);

    /**
     * Replaces the owner of the wallet with a new address.
     * @param newOwner The new owner's address.
     */
    function setOwner(address newOwner) virtual internal;
    ...
    function executeRecovery(address newOwner, address[] calldata guardianList) onlyGuardian onlyInRecovery external {
        ...
        for (uint i = 0; i < guardianList.length; i++) {
            ...
            require(recovery.recoveryRound == currRecoveryRound, "Recovery round mismatch");
    ...
    function transferGuardianship(bytes32 newGuardianHash) onlyGuardian notInRecovery external {
        ...
        require(guardianHashToRemovalTimestamp[keccak256(abi.encodePacked(msg.sender))] == 0, "Cannot transfer guardianship because guardian is queueud for removal");
