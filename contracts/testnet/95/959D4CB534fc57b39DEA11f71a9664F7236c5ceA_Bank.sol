...
contract Bank is AccessControlEnumerable, KeeperCompatibleInterface, Multicall {
    ...
    /// @param token Address of the token.
    /// @param amount Number of tokens.
    function deposit(address token, uint256 amount) external payable onlyTokenOwner(DEFAULT_ADMIN_ROLE, token) {
        if (_isGasToken(token)) {
            amount = msg.value;
        } else {
    ...
    /// @param token Address of the token.
    /// @param amount Number of tokens.
    function withdraw(address token, uint256 amount) public onlyTokenOwner(DEFAULT_ADMIN_ROLE, token) {
    ...
    function setBalanceRisk(address token, uint16 balanceRisk) external onlyTokenOwner(DEFAULT_ADMIN_ROLE, token) {
    ...
    function setPausedToken(address token, bool paused) external onlyTokenOwner(DEFAULT_ADMIN_ROLE, token) {
    ...
    /// @param token Address of the token.
    /// @param minHouseEdgeWithdrawAmount Minimum amount of token to allow transfer.
    function setMinHouseEdgeWithdrawAmount(address token, uint256 minHouseEdgeWithdrawAmount)
        external
    ...
    /// @param token Address of the token.
    /// @param partner Address of the partner.
    function setTokenPartner(address token, address partner) external onlyTokenOwner(DEFAULT_ADMIN_ROLE, token) {
        uint256 roleMemberCount = getRoleMemberCount(GAME_ROLE);
        for (uint256 i; i < roleMemberCount; i++) {
    ...
    function setHouseEdgeSplit(
        ...
        emit SetTokenHouseEdgeSplit(token, bank, dividend, partner, _treasury, team);
    }
    ...
    /// @param token Address of the token.
    /// @param subId Subscription ID.
    function setTokenVRFSubId(address token, uint64 subId) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _allocateHouseEdge(address token, uint256 fees) private {
        ...
        emit AllocateHouseEdgeAmount(token, bankAmount, dividendAmount, partnerAmount, treasuryAmount, teamAmount);
    }
    ...
    function performUpkeep(bytes calldata performData) external override {
        (UpkeepActions upkeepAction, address tokenAddress) = abi.decode(performData, (UpkeepActions, address));
        Token memory token = tokens[tokenAddress];

        if (
            upkeepAction == UpkeepActions.DistributePartnerHouseEdge &&
            token.houseEdgeSplit.partnerAmount > token.minHouseEdgeWithdrawAmount
        ) {
            withdrawPartnerAmount(tokenAddress);
        } else if (
            upkeepAction == UpkeepActions.DistributeOwnHouseEdge &&
            token.houseEdgeSplit.treasuryAmount + token.houseEdgeSplit.teamAmount > token.minHouseEdgeWithdrawAmount
        ) {
    ...
    {
        (UpkeepActions upkeepAction, address tokenAddressData) = abi.decode(checkData, (UpkeepActions, address));
        ...
        if (
            (upkeepAction == UpkeepActions.DistributePartnerHouseEdge &&
                token.houseEdgeSplit.partnerAmount > token.minHouseEdgeWithdrawAmount) ||
            (upkeepAction == UpkeepActions.DistributeOwnHouseEdge &&
                token.houseEdgeSplit.treasuryAmount + token.houseEdgeSplit.teamAmount >
    ...
    function setTeamWallet(address _teamWallet) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function withdrawHouseEdgeAmount(address tokenAddress) public {
        HouseEdgeSplit storage tokenHouseEdge = tokens[tokenAddress].houseEdgeSplit;
        ...
        if (treasuryAmount != 0 || teamAmount != 0) {
            emit HouseEdgeDistribution(tokenAddress, treasuryAmount, teamAmount);
        }
    }
    ...
    function getBalance(address token) public view returns (uint256) {
        ...
        HouseEdgeSplit memory tokenHouseEdgeSplit = tokens[token].houseEdgeSplit;
        return
            balance -
