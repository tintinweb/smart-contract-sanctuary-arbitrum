...
contract FuturXVoucher is ERC721Enumerable, Ownable {
    ...
    uint256 public maxVoucherDiscountValue = 500000000000000000000;
    ...
    function distributeVoucher(
        ...
        if (_voucherType == 1) {
            require(_maxDiscountValue <= maxVoucherDiscountValue, "max value exceeded");
        } else {
            require(_value <= maxVoucherDiscountValue, "max value exceeded");
        }
    ...
    function setMaxVoucherDiscountValue(uint256 _amount)
        external
        onlyOwner
    {
        maxVoucherDiscountValue = _amount;
    }
}