    ...
    LimitOrdersStorage
{
        ...
        returns (uint256 newAmountOfToken0, uint256 newAmountOfToken1)
    {
        ...
        if (_editParams.amountOfToken == 0) {
            cancelOrder(_editParams.poolAddress, _editParams.tokenId);
        } else {
            require(
                pathTokenToWETH[
                    order.orderType == OrderType.TAKE_PROFIT
                        ? order.poolData.token0
                        : order.poolData.token1
                ].minAmount <= _editParams.amountOfToken,
                "minAmount"
            );
            ...
            if (
                ...
                IERC20Upgradeable(order.poolData.token0).safeTransferFrom(
                    _msgSender(),
                    address(this),
                    _editParams.amountOfToken - order.amountOfToken0
                );
                IERC20Upgradeable(order.poolData.token0).safeIncreaseAllowance(
                ...
                IERC20Upgradeable(order.poolData.token1).safeTransferFrom(
                    _msgSender(),
                    address(this),
                    _editParams.amountOfToken - order.amountOfToken1
                );
                IERC20Upgradeable(order.poolData.token1).safeIncreaseAllowance(
                        address(nftPositionManager),
                        _editParams.amountOfToken - order.amountOfToken1
                    );
            }
            ...
            if (order.orderType == OrderType.TAKE_PROFIT) {
                if (_editParams.amountOfToken < order.amountOfToken0) {
                    ...
                    IERC20Upgradeable(order.poolData.token0).safeTransfer(
                        _msgSender(),
                        newAmountOfToken0
                    );
                    ...
                    order.liquidity -= LiquidityAmounts.getLiquidityForAmount0(
                        TickMath.getSqrtRatioAtTick(order.poolData.tickLower),
                        TickMath.getSqrtRatioAtTick(order.poolData.tickUpper),
                ...
                if (_editParams.amountOfToken < order.amountOfToken1) {
                    ...
                    // Send decreased tokens to user.
                    IERC20Upgradeable(order.poolData.token1).safeTransfer(
                        _msgSender(),
                        newAmountOfToken1
                    );
                    uint256 newReceive = LiquidityAmounts
                        .getAmount0ForLiquidity(
                            TickMath.getSqrtRatioAtTick(
                                order.poolData.tickLower
                            ),
                    ...
                    if (newReceive < order.receivedAmount)
                        order.receivedAmount -= newReceive;
        ...
        allowedOrder(_tokenId, _poolAddress)
    {
        ...
        (bool success, ) = cancellDelegator.delegatecall(call);
        if (success == false) {
            assembly {
                let ptr := mload(0x40)
...
import "./interfaces/ITriadaWETH.sol";
