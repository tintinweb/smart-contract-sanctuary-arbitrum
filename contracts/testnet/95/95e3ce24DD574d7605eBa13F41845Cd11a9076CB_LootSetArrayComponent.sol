...
uint256 constant ID = uint256(
    keccak256("game.piratenation.lootsetarraycomponent")
);

struct Layout {
    uint256[] lootSetEntities;
}

library LootSetArrayComponentStorage {
...
/**
 * @title LootSetArrayComponent
 * @dev Component for storing an array of lootset entity ids
 */
contract LootSetArrayComponent is BaseStorageComponentV2 {
    /** SETUP **/
        ...
        returns (string[] memory keys, TypesLibrary.SchemaValue[] memory values)
    {
        ...
        // Entity ids belonging to LootArray components
        keys[0] = "loot_set_entities";
     ...
     *
     * @param entity Entity to get value for
     * @param lootSetEntities Entity ids belonging to LootArray components
     */
    function setValue(
        uint256 entity,
        uint256[] memory lootSetEntities
    ) external virtual onlyRole(GAME_LOGIC_CONTRACT_ROLE) {
        _setValue(entity, Layout(lootSetEntities));
    }
    ...
    function append(
        ...
        Layout storage s = LootSetArrayComponentStorage
            .layout()
            .entityIdToStruct[entity];
        for (uint256 i = 0; i < values.lootSetEntities.length; i++) {
            s.lootSetEntities.push(values.lootSetEntities[i]);
        }

        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(s.lootSetEntities));
    }
    ...
    function removeValueAtIndexes(
        ...
        for (uint256 i = 0; i < indexes.length; i++) {
            ...
            // Get the last index
            uint256 lastIndexInArray = s.lootSetEntities.length - 1;
            // Move the last value to the index to pop
            if (indexToRemove != lastIndexInArray) {
                s.lootSetEntities[indexToRemove] = s.lootSetEntities[
                    lastIndexInArray
                ];
            }
            // Pop the last value
            s.lootSetEntities.pop();
        }
    ...
    function getLayoutValue(
        ...
        value = LootSetArrayComponentStorage.layout().entityIdToStruct[entity];
    }
     ...
     *
     * @param entity Entity to get value for
     * @return lootSetEntities Entity ids belonging to LootArray components
     */
    function getValue(
        uint256 entity
    ) external view virtual returns (uint256[] memory lootSetEntities) {
        if (has(entity)) {
            Layout memory s = LootSetArrayComponentStorage
                .layout()
                .entityIdToStruct[entity];
            (lootSetEntities) = abi.decode(_getEncodedValues(s), (uint256[]));
        }
    }
    ...
    function getByteValues(
        ...
        values = new bytes[](1);
        values[0] = abi.encode(s.lootSetEntities);
    }
    ...
    function setBytes(
        ...
        Layout memory s = LootSetArrayComponentStorage
            .layout()
            .entityIdToStruct[entity];
        (s.lootSetEntities) = abi.decode(value, (uint256[]));
    ...
    function remove(
        ...
        delete LootSetArrayComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete LootSetArrayComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
    ...
    function _setValueToStorage(uint256 entity, Layout memory value) internal {
        ...
        s.lootSetEntities = value.lootSetEntities;
    }
    ...
    function _setValue(uint256 entity, Layout memory value) internal {
        ...
        // ABI Encode all native types of the struct
        _emitSetBytes(entity, abi.encode(value.lootSetEntities));
    }

    function _getEncodedValues(
        Layout memory value
    ) internal pure returns (bytes memory) {
        return abi.encode(value.lootSetEntities);
    }
}