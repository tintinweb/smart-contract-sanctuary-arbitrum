...
interface IVault {
    ...
    function updateTokenBalance(address _token) external returns (uint256);
...
contract Vault is IVault, Ownable, ReentrancyGuard {
    ...
    function updateTokenBalance(address _token) public returns (uint256) {
        uint256 nextBalance = IERC20(_token).balanceOf(address(this));
        tokenBalances[_token] = nextBalance;
        return nextBalance;
    }
