...
import "../dex/gmx/interfaces/IRouter.sol";
...
contract GMXLimitOrderProxy is AbstractLimitOrderProxy {
    ...
    constructor(ERC20 _makerToken, ERC20 _takerToken, address _orderBook) AbstractLimitOrderProxy(_makerToken, _takerToken) {
      ...
      router = IRouter(orderBook.router());
      minOutPerc = 50; // 50%
    }
    ...
    function createOrder(uint256 optionID, uint256 makerAmount_, uint256 takerAmount_, LimitOrderType orderType_) external override nonReentrant {
        ...
        ERC20(makerToken).safeApprove(address(router), makerAmount_);
    ...
    /// @notice Approve GMX Order book as Plugin on GMX Router
    function approvePlugin(address plugin) external onlyOwner nonReentrant {
...
interface IOrderBook {
    ...
    /// @notice Get current index/counter of {account}
    function swapOrdersIndex(address account) external returns(uint256);
    /// @notice Execute an onder
    function executeSwapOrder(address account, uint256 orderIndex, address payable feeReceiver) external;
    /// @notice Router of GMX Order book
...
interface IRouter {
    ...
    function plugins(address plugin) external returns(bool);
    function approvePlugin(address _plugin) external;
    function approvedPlugins(address _account, address _plugin) external returns(bool);
}
