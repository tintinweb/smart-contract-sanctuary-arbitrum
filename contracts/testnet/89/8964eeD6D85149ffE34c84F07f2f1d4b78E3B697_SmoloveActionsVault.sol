...
import "./interfaces/ISmoloveActionsVault.sol";
    ...
    ISmoloveActionsVault
{
    ...
    function initialize(
        ...
        require(_atlasMine != address(0), "SMOLOVE_ACTIONS_VAULT:INVALID_ATLASMINE_ADDRESS");
        treasury = _treasury;
    ...
    function stake(
        ...
        _stake(user, uint256(amount), lock);
    }
    ...
    function _stake(
        ...
        require(validLock, "SMOLOVE_ACTIONS_VAULT:LOCK_TIME_NOT_ALLOWED");
        uint256 battleflyStakerDepositId = BattleflyStaker.deposit(amount, battleflyLock);
        ...
        s.amount = amount;
        uint64 internalLock = getCurrentEpoch() + 1 + (getLockPeriod(lock) / 1 days);
        s.unlockAt = vaultStake.unlockAt > internalLock ? vaultStake.unlockAt : internalLock;
        s.lock = lock;
        ...
        emit Stake(user, nextStakeId, amount, lock);
        nextStakeId++;
        totalStaked += amount;
    ...
    function claimAllToTreasury() public nonReentrant {
        uint256 totalReward = 0;
        uint256[] memory stakeIds = activeStakes.values();
        for (uint256 i = 0; i < stakeIds.length; i++) {
            if (_getStakeClaimableAmount(stakeIds[i]) > 0) {
                totalReward += _claim(stakeIds[i]);
            }
        }
        require(totalReward > 0, "SMOLOVE_ACTIONS_VAULT:NO_REWARDS_TO_CLAIM");
        magic.safeTransfer(treasury, totalReward);
    }

    function claimToTreasury(uint256[] memory stakeIds) external nonReentrant {
        uint256 totalReward = 0;
        for (uint256 i = 0; i < stakeIds.length; i++) {
            if (_getStakeClaimableAmount(stakeIds[i]) > 0) {
                totalReward += _claim(stakeIds[i]);
    ...
    function withdrawAll() external nonReentrant {
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            ...
            if (canWithdraw(stakeId)) {
                (uint256 received, uint256 claimed) = _withdraw(stakeId);
                receiveAmount += received;
                claimAmount += claimed;
            }
        }
        require(receiveAmount > 0 || claimAmount > 0, "SMOLOVE_ACTIONS_VAULT:NO_STAKES_TO_WITHDRAW");
        if (receiveAmount > 0) {
            magic.safeTransfer(msg.sender, receiveAmount);
        }
        if (claimAmount > 0) {
            magic.safeTransfer(treasury, claimAmount);
        }
    }
    ...
    function withdraw(uint256[] memory stakeIds) external nonReentrant {
        ...
        for (uint256 i = 0; i < stakeIds.length; i++) {
            ...
            (uint256 received, uint256 claimed) = _withdraw(stakeIds[i]);
            receiveAmount += received;
            claimAmount += claimed;
        }
        require(receiveAmount > 0 || claimAmount > 0, "SMOLOVE_ACTIONS_VAULT:NO_STAKES_TO_WITHDRAW");
    ...
    function requestWithdrawal(uint256[] memory stakeIds) public override {
        for (uint256 i = 0; i < stakeIds.length; i++) {
            UserStake memory userStake = userStakes[stakeIds[i]];
            require(userStake.owner == msg.sender, "SMOLOVE_ACTIONS_VAULT:ONLY_OWNER_CAN_REQUEST_WITHDRAWAL");
            BattleflyStaker.requestWithdrawal(userStake.battleflyStakerDepositId);
            emit RequestWithdrawal(stakeIds[i]);
        }
    }

    function _withdraw(uint256 stakeId) internal returns (uint256, uint256) {
        UserStake storage s = userStakes[stakeId];
        uint256 rewardAmount;
        uint256 returnAmount;
        require(canWithdraw(stakeId), "SMOLOVE_ACTIONS_VAULT:CANNOT_WITHRAW_BEFORE_RETENTION_PERIOD_HAS_PASSED");
        ...
        return (returnAmount, rewardAmount);
    }
    ...
    function canRequestWithdrawal(uint256 stakeId) public view override returns (bool requestable) {
        UserStake memory s = userStakes[stakeId];
        return (getCurrentEpoch() >= (s.unlockAt - 14) && BattleflyStaker.canRequestWithdrawal(s.battleflyStakerDepositId));
    }
    ...
    function canWithdraw(uint256 stakeId) public view override returns (bool withdrawable) {
        UserStake memory s = userStakes[stakeId];
        return (getCurrentEpoch() >= s.unlockAt && BattleflyStaker.canWithdraw(s.battleflyStakerDepositId));
    }
     ...
     */
    function initialUnlock(uint256 stakeId) public view override returns (uint64 epoch) {
        UserStake memory s = userStakes[stakeId];
        return s.unlockAt;
    }
     ...
     */
    function retentionUnlock(uint256 stakeId) public view override returns (uint64 epoch) {
        UserStake memory s = userStakes[stakeId];
        return BattleflyStaker.getVaultStake(s.battleflyStakerDepositId).retentionUnlock;
    }
    ...
    function getCurrentEpoch() public view override returns (uint64 epoch) {
        return BattleflyStaker.currentEpoch();
    }

    function getLockPeriod(IAtlasMine.Lock _lock) internal view returns (uint64) {
        if (_lock == IAtlasMine.Lock.twoWeeks) {
            return 14 days + 1 days + uint64(atlasMine.getVestingTime(_lock));
        }
        if (_lock == IAtlasMine.Lock.oneMonth) {
            return 30 days + 1 days + uint64(atlasMine.getVestingTime(_lock));
        }
        if (_lock == IAtlasMine.Lock.threeMonths) {
            return 90 days + 1 days + uint64(atlasMine.getVestingTime(_lock));
        }
        if (_lock == IAtlasMine.Lock.sixMonths) {
            return 180 days + 1 days + uint64(atlasMine.getVestingTime(_lock));
        }
        if (_lock == IAtlasMine.Lock.twelveMonths) {
            return 365 days + 1 days + uint64(atlasMine.getVestingTime(_lock));
        }
    ...
    function setBattleflyAtlasStaker(address _atlasStaker) external onlyAdmin {
        ...
        BattleflyStaker = IBattleflyAtlasStaker(_atlasStaker);
        // Approve the AtlasStaker contract to spend the magic
        magic.safeApprove(address(_atlasStaker), 2**256 - 1);
    }

    function setActiveBattleflyLock(IAtlasMine.Lock _battleflyLock) external onlyAdmin {
        require(
            _battleflyLock < allowedLocks[0],
            "SMOLOVE_ACTIONS_VAULT:BATTLEFLY_LOCK_MUST_NOT_BE_BIGGER_THAN_SMALLEST_ALLOWED_LOCK"
        );
        battleflyLock = _battleflyLock;
    }
...
interface ISmoloveActionsVault {
    ...
    function initialUnlock(uint256 stakeId) external view returns (uint64 epoch);

    function retentionUnlock(uint256 stakeId) external view returns (uint64 epoch);
