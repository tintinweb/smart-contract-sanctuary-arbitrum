...
import {PayWhatYouWantStorage} from "./PayWhatYouWantStorage.sol";

contract PayWhatYouWantFacet is AppFacet {
    function pwywAppEnabled() external view returns (bool) {
        return PayWhatYouWantStorage.layout()._pwywStatus;
    }

    function pwywStartSale()
        external
        onlyRolesOrOwner(BaseStorage.MANAGER_ROLE)
    {
        PayWhatYouWantStorage.layout()._pwywSaleActive = true;
    }

    function pwywStopSale()
        external
        onlyRolesOrOwner(BaseStorage.MANAGER_ROLE)
    {
        PayWhatYouWantStorage.layout()._pwywSaleActive = false;
    }

    function pwywSaleActive() external view returns (bool) {
        return PayWhatYouWantStorage.layout()._pwywSaleActive;
    }

    function updatePWYWStatus(
        bool payStatus_
    ) external onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        PayWhatYouWantStorage.layout()._pwywStatus = payStatus_;
    }

    function pwywStartEditionSale(
        string memory editionId_
    ) external onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        PayWhatYouWantStorage.Layout storage layout = PayWhatYouWantStorage
            .layout();
        if (layout._activeTokens[editionId_] == false) {
            layout._activeTokens[editionId_] = true;
            layout._pwywActiveTokensCount += 1;
        }

        if (layout._pwywSaleActive == false) {
            layout._pwywSaleActive = true;
        }
    }

    function pwywStopEditionSale(
        string memory editionId_
    ) external onlyRolesOrOwner(BaseStorage.MANAGER_ROLE) {
        PayWhatYouWantStorage.Layout storage layout = PayWhatYouWantStorage
            .layout();
        ...
        if (layout._pwywActiveTokensCount == 0) {
            layout._pwywSaleActive = false;
        }
    }

    function pwywIsEditionActive(
        string memory editionId_
    ) external view returns (bool) {
        return PayWhatYouWantStorage.layout()._activeTokens[editionId_] == true;
    }

    function pwywActiveTokensCount() external view returns (uint256) {
        return PayWhatYouWantStorage.layout()._pwywActiveTokensCount;
    }
}
...
library PayWhatYouWantStorage {
    // This slot should not be changed
    bytes32 private constant STORAGE_SLOT =
        keccak256("niftykit.apps.payWhatYouWant.storage");

    struct Layout {
        mapping(string => bool) _activeTokens;
        bool _pwywStatus;
        bool _pwywSaleActive;
        uint _pwywActiveTokensCount;
    }
