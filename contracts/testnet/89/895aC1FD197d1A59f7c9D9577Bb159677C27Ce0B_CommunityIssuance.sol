...
import "../Interfaces/IAGLWrapper.sol";
...
contract CommunityIssuance is ICommunityIssuance, OwnableUpgradeable, CheckContract, BaseMath {
	...
	IERC20Upgradeable public aglToken;
	IStabilityPoolManager public stabilityPoolManager;

	mapping(address => uint256) public totalAGLIssued;
	mapping(address => uint256) public lastUpdateTime;
	mapping(address => uint256) public AGLSupplyCaps;
	mapping(address => uint256) public aglDistributionsByPool;
	...
	function setAddresses(
		...
		checkContract(_aglTokenAddress);
		...
		aglToken = IERC20Upgradeable(_aglTokenAddress);
		stabilityPoolManager = IStabilityPoolManager(_stabilityPoolManagerAddress);

		emit AGLTokenAddressSet(_aglTokenAddress);
	...
	function issueAGL() external override onlyStabilityPool returns (uint256) {
		return _issueAGL(msg.sender);
	}

	function _issueAGL(address _pool) internal isStabilityPool(_pool) returns (uint256) {
		DistributionRewards storage distributionRewards = stabilityPoolRewards[_pool];
		...
		emit TotalAGLIssuedUpdated(_pool, totalIssuance);
	...
	function sendAGL(address _account, uint256 _aglAmount)
		external
		override
		onlyStabilityPool
		activeStabilityPoolOnly(msg.sender)
	{
		...
		uint256 safeAmount = balance >= _aglAmount ? _aglAmount : balance;
	...
	function setWeeklyAglDistribution(address _stabilityPool, uint256 _weeklyReward)
		external
	...
	function wrapRewardsToken(address _pool, IAGLWrapper _vestaWrappedToken)
		external
		onlyOwner
	{
