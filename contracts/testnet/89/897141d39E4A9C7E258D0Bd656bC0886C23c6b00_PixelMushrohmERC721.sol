...
contract PixelMushrohmERC721 is IPixelMushrohmERC721, ERC721Enumerable, PixelMushrohmAccessControlled {
    /* ========== DEPENDENCIES ========== */
    ...
    uint256 constant RESERVED_MAX_MINT = 1500;
    uint256 constant FIRST_GEN_LEVEL_MAX_MINT = 1500;
    uint256 constant UNRESERVED_MAX_MINT = 2050;

    mapping(address => uint256) private _numUserTokensMinted;

    Counters.Counter private _tokenIdTracker;
    Counters.Counter private _numUnreservedTokensMinted;
    string public baseURI;

    address public mintToken;
    uint256 public mintTokenPrice;
    uint256 public maxMintPerWallet = 1;
    bytes32 public merkleRoot;

    uint256 public maxSporePowerLevel = 4;
    uint256 public maxLevel = 10;
    /// @dev 18 decimals
    uint256 public sporePowerCost = 50000000000000000000;
    uint256 public levelCost = 50000000000000000000;
    /// @dev 18 decimals
    uint256 public totalSporePower;
    /// @dev 9 decimals
    uint256 public levelMultiplier = 20000000; // 2%
    ...
    function setSporePowerCost(uint256 _sporePowerCost) external override onlyPolicy {
        sporePowerCost = _sporePowerCost;
        emit SporePowerCost(_sporePowerCost);
    }

    function setMaxSporePowerLevel(uint256 _maxSporePowerLevel) external override onlyPolicy {
        maxSporePowerLevel = _maxSporePowerLevel;
        emit MaxSporePowerLevel(_maxSporePowerLevel);
    }

    function setLevelCost(uint256 _levelCost) external override onlyPolicy {
        levelCost = _levelCost;
        emit LevelCost(_levelCost);
    }

    function setMaxLevel(uint256 _maxLevel) external override onlyPolicy {
        maxLevel = _maxLevel;
        emit MaxLevel(_maxLevel);
    }

    function setLevelMultiplier(uint256 _levelMultiplier) external override onlyPolicy {
        levelMultiplier = _levelMultiplier;
        emit LevelMultiplier(_levelMultiplier);
    }
    ...
    function setRarityMultiplier(uint256 _rarityMultiplier, uint256[] calldata _tokenIds) external override onlyOwner {
        require(_rarityMultiplier > 0, "Staking: Rarity multiplier must be greater than 0");
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            tokenData[_tokenIds[i]].rarityMultiplier = _rarityMultiplier;
        }
    }
    ...
    function setMintToken(address _tokenAddr) external override onlyOwner {
        mintToken = _tokenAddr;
    }

    function setMintTokenPrice(uint256 _price) external override onlyOwner {
        mintTokenPrice = _price;
    }

    function setMaxMintPerWallet(uint256 _maxMintPerWallet) external override onlyOwner {
    ...
    /* NOTE: DEVELOPMENT ONLY FUNCTION, COMMENT OUT ON PROD DEPLOY */
    function maxOutSporePower(uint256 _tokenId) external override onlyOwner {
        require(!staking.isStaked(_tokenId), "PixelMushrohm: Unstake before maxing out spore power");
        uint256 _newSporePower = sporePowerCost.mul(maxSporePowerLevel);
        uint256 _sporePowerDiff = _newSporePower.sub(tokenData[_tokenId].sporePower);
        totalSporePower = totalSporePower.add(_sporePowerDiff);
        tokenData[_tokenId].sporePower = _newSporePower;
    }
    ...
    function withdraw(
        ...
        require(_to != address(0), "PixelMushrohm: Invalid to address");
        IERC20(_tokenAddr).safeTransferFrom(address(this), _to, _amount);
        emit Withdraw(_tokenAddr, _amount, _to);
    }

    function airdrop(address _to, uint256 _amount) external override onlyOwner {
        require(_to != address(0), "PixelMushrohm: Invalid to address");
        require(_amount > 0, "PixelMushrohm: Amount must be greater than 0");
        for (uint256 i = 0; i < _amount; i++) {
            _numUnreservedTokensMinted.increment();
            _mint(_to, 0, false);
        }
    }
    ...
    function whitelistMint(uint256 _amount, bytes32[] calldata _merkleProof) external override {
        require(!paused, "PixelMushrohm: Contract paused");
        require(_amount > 0, "PixelMushrohm: Amount must be greater than 0");
        require(_numUnreservedTokensMinted.current() < UNRESERVED_MAX_MINT, "PixelMushrohm: Mint complete");
        require(mintToken != address(0), "PixelMushrohm: Payment token not set");
        require(mintTokenPrice != 0, "PixelMushrohm: Payment price not set");
        require(
            _numUserTokensMinted[msg.sender].add(_amount) <= maxMintPerWallet,
            "PixelMushrohm: Max mint limit reached"
        );
        require(_isWhitelisted(msg.sender, _merkleProof), "PixelMushrohm: Not in whitelist");

        IERC20(mintToken).safeTransferFrom(msg.sender, address(this), mintTokenPrice.mul(_amount));

        for (uint256 i = 0; i < _amount; i++) {
            _numUnreservedTokensMinted.increment();
            _numUserTokensMinted[msg.sender] = _numUserTokensMinted[msg.sender].add(1);
            _mint(msg.sender, 0, false);
        }
    }

    function bridgeMint(address _to, uint256 _tokenId) external override onlyBridge {
        require(!_exists(_tokenId), "PixelMushrohm: Token already exists");
        _mint(_to, _tokenId, true);
    }

    function firstGenLevelMint(uint256 _tokenId) external override {
        require(!paused, "PixelMushrohm: Contract paused");
    ...
    function updateSporePower(uint256 _tokenId, uint256 _sporePowerEarned) external override onlyStaking {
        tokenData[_tokenId].sporePower = tokenData[_tokenId].sporePower.add(_sporePowerEarned);
        totalSporePower = totalSporePower.add(_sporePowerEarned);
    }

    function redeemSporePower(uint256 _tokenId, uint256 _amount) external override onlyRedeemer {
        require(_amount > 0, "PixelMushrohm: Amount must be greater than 0");
        require(
    ...
    function updateLevel(uint256 _tokenId) external override onlyStaking {
        require(tokenData[_tokenId].level < maxLevel, "PixelMushrohm: Level already maxxed");
    ...
    function _mint(
        ...
        if (_reserved) {
            _maxMint = RESERVED_MAX_MINT;
        } else {
            _tokenId = _tokenIdTracker.current().add(RESERVED_MAX_MINT).add(1);
            _tokenIdTracker.increment();
            _maxMint = UNRESERVED_MAX_MINT.add(RESERVED_MAX_MINT).add(FIRST_GEN_LEVEL_MAX_MINT);
        }
    ...
    function getSporePower(uint256 _tokenId) public view override returns (uint256 sporePower) {
        sporePower = tokenData[_tokenId].sporePower.add(staking.sporePowerEarned(_tokenId));
    }

    function getSporePowerLevel(uint256 _tokenId) public view override returns (uint256 sporePowerLevel) {
        sporePowerLevel = Math.min(getSporePower(_tokenId).div(sporePowerCost), maxSporePowerLevel);
    }
    ...
    function getMaxSporePowerLevel() public view override returns (uint256) {
        return maxSporePowerLevel;
    }

    function getLevel(uint256 _tokenId) public view override returns (uint256 level) {
        level = tokenData[_tokenId].level;
    }
    ...
    function getTotalMultiplier(uint256 _tokenId) public view override returns (uint256) {
        return tokenData[_tokenId].rarityMultiplier.add(levelMultiplier.mul(tokenData[_tokenId].level)).add(1e9);
    }

    function getLevelMultiplier(uint256 _tokenId) public view override returns (uint256) {
        return (levelMultiplier.mul(tokenData[_tokenId].level)).add(1e9);
    }

    function getRarityMultiplier(uint256 _tokenId) public view override returns (uint256) {
        return tokenData[_tokenId].rarityMultiplier.add(1e9);
    }
    ...
    function isEligibleForLevelMint(uint256 _tokenId) public view override returns (bool) {
        return tokenData[_tokenId].level >= firstGenLevelMintLevel && !firstGenLevelMintComplete[_tokenId];
    }
}
...
interface IPixelMushrohmERC721 is IERC721Enumerable {
    ...
    event LevelMultiplier(uint256 levelMultiplier);
    ...
    function setMaxSporePowerLevel(uint256 _maxSporePowerLevel) external;

    function setLevelCost(uint256 _levelCost) external;

    function setMaxLevel(uint256 _maxLevel) external;

    function setLevelMultiplier(uint256 _levelMultiplier) external;

    function setFirstGenLevelMintLevel(uint256 _level) external;

    function setRarityMultiplier(uint256 _rarityMultiplier, uint256[] calldata _tokenIds) external;
    ...
    function getSporePowerLevel(uint256 _tokenId) external view returns (uint256 sporePowerLevel);

    function averageSporePower() external view returns (uint256);

    function getMaxSporePowerLevel() external view returns (uint256);

    function getLevel(uint256 _tokenId) external view returns (uint256 level);

    function getLevelCost() external view returns (uint256);

    function getMaxLevel() external view returns (uint256);

    function getTotalMultiplier(uint256 _tokenId) external view returns (uint256);
...
interface IPixelMushrohmStaking {
    ...
    function levelEarned(uint256 _tokenId) external view returns (uint256 level);

    function levelPerWeek() external view returns (uint256);
