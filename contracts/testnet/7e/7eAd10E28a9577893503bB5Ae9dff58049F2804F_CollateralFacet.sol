...
contract CollateralFacet is ICollateral, ReentrancyGuard {
     ...
     * @param newCollaterals Information about the collateral to be added
     * @custom:event emits a StrategyIncrease event
     */
    function increaseStrategyCollateral(
        uint256 strategyId,
        ...
        // 抵押品查重
        LibCollateral._checkCollateralDuplicates(newCollaterals);

        uint256 newCollateralLen = newCollaterals.length;
        for (uint256 i; i < newCollateralLen; ) {
            StrategyTypes.CollateralInfo memory collateralInfo = newCollaterals[i];

            collateralInfo;
            // ToDo 更新总账户：可用余额减少，不可用余额增多
            // 验证未使用用余额是否充足
        ...
        // 调用nft合约更新抵押品
        LibCollateral._increaseStrategyCollateral(strategyId, newCollaterals);

        // 发出事件
        emit StrategyCollateralAdded(strategyId);
    }

    function decreaseStrategyCollateral(
        uint256 strategyId,
        StrategyTypes.CollateralInfo[] calldata collaterals
    ) external {
        ...
        // 抵押品查重和是否为已支持
        LibCollateral._checkCollateralDuplicates(collaterals);

        bytes32 requestHash = LibStrategyConfig._getRequestHashAndUpdateNonce(admin);

        // 设置请求参数
        l.userDSCRequest[requestHash].admin = admin;
        l.userDSCRequest[requestHash].strategyId = strategyId;
        LibCollateral._collateralsMemoryToStorage(l.userDSCRequest[requestHash].collaterals, collaterals);
    ...
    function executeDecreaseStrategyCollateral(bytes32 requestHash) external nonReentrant {
        ...
        // 验证owner和是否激活 ToDo 这里是否不需要验证是否激活？如果非激活，是否取走所有抵押品
        ...
        // 减少抵押品
        StrategyTypes.CollateralInfo[] memory processedCollaterals = LibCollateral._decreaseStrategyCollateral(
            requestParam.strategyId,
            requestParam.collaterals
        );

        // 更新总账户：可用和不可用余额

        // 循环计算抵押品总价值，并与维持保证金进行比较
        LibMarketPricer._checkCollateralSufficiency(processedCollaterals, requestHash, false);
...
library LibPositionCore {
    ...
    function _getStrategy(uint256 strategyId) external view returns (StrategyTypes.StrategyData memory) {
     ...
     */
    function _handleCreateAndMerge(StrategyTypes.Strategy memory requestParam) external returns (bool) {
        LibPositionCore.Layout storage l = LibPositionCore.layout();
    ...
    function _handleIncreasePositions(uint256 tokenId, StrategyTypes.Strategy memory strategy) external {
        LibPositionCore.Layout storage l = LibPositionCore.layout();
        StrategyTypes.StrategyData storage originalStrategy = l.strategies[tokenId];
        if (!originalStrategy.isActive) {
            revert StrategyIsNotActive(originalStrategy.strategyId);
