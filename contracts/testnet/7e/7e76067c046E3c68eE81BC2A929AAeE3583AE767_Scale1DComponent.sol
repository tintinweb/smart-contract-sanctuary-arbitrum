...
uint256 constant ID = uint256(
    keccak256("game.piratenation.scale1dcomponent.v1")
);
...
library Scale1DComponentStorage {
...
/**
 * @title Scale1DComponent
 * @dev The 1D scale of an entity
 */
contract Scale1DComponent is BaseStorageComponentV2 {
    /** SETUP **/
    ...
    function getValue(
        ...
        value = Scale1DComponentStorage.layout().entityIdToStruct[entity];
    }
    ...
    function getByteValues(
        ...
        // Get the struct from storage
        Layout storage s = Scale1DComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function getBytes(
        ...
        Layout memory s = Scale1DComponentStorage.layout().entityIdToStruct[
            entity
        ];
    ...
    function remove(
        ...
        delete Scale1DComponentStorage.layout().entityIdToStruct[entity];
    ...
    function batchRemove(
        ...
        for (uint256 i = 0; i < entities.length; i++) {
            delete Scale1DComponentStorage.layout().entityIdToStruct[
                entities[i]
            ];
        }
