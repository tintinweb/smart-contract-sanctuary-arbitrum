...
contract SmolRings is ERC721Enumerable, ReentrancyGuard, AccessControl, ISmolRings {
    ...
    uint128 public constant MAX_RINGS = 5000;
    uint128 public constant MAX_WHITELIST_AMOUNT = 3500;
    uint128 public constant RING_SMOL_AMOUNT = 1000;
    uint128 public constant RING_TEAM_AMOUNT = 200;
    ...
    event RingUpgraded(address sender, uint256 ringId, uint256 treasureId);
     ...
     */
    function mintRing(uint256 amount) external virtual nonReentrant nonContractCaller {
        require(regularMintEnabled, "SMOLRING:REGULAR_MINT_DISABLED");
        require(magic.balanceOf(msg.sender) >= 5e18, "SMOLRING:NOT_ENOUGH_MAGIC_IN_WALLET");
        require(amount <= 2, "SMOLRING:MAX_ALLOWANCE_PER_BATCH_REACHED");
        require(
    ...
    function mintRingSmolSwol(uint256[] calldata smolIds, uint256[] calldata swolIds) external virtual nonReentrant nonContractCaller {
        _mintRingSmol(smolIds);
        _mintRingSwol(swolIds);
    }
     ...
     */
    function mintRingSmol(uint256[] calldata smolIds) external virtual nonReentrant nonContractCaller {
        _mintRingSmol(smolIds);
    }
    
    function _mintRingSmol(uint256[] calldata smolIds) internal {
    ...
    */
    function mintRingSwol(uint256[] calldata swolIds) external virtual nonReentrant nonContractCaller {
        _mintRingSwol(swolIds);
    }

    function _mintRingSwol(uint256[] calldata swolIds) internal {
    ...
    function mintRingWhitelist(
        ...
        for (uint256 i = 0; i < rings.length; i++) {
            require(rings[i] == 0 || allowedForges[i].valid, "SMOLRING:TYPE_NOT_ALLOWED_FOR_FORGING");
        }
        require(
        ...
        for (uint256 i = 0; i < rings.length; i++) {
            for (uint256 j = 0; j < rings[i]; j++) {
                if (totalRingsPerType[i] == allowedForges[i].maxForges) {
                    ringProps[RING_TEAM_AMOUNT + ringCounter] = Ring(0);
                    totalRingsPerType[0]++;
                } else {
    ...
    function mintRingTeam(
        ...
        require(amount <= 32, "SMOLRING:MAX_ALLOWANCE_PER_BATCH_REACHED");
        require(allowedForges[ringType].valid, "SMOLRING:TYPE_NOT_ALLOWED_FOR_FORGING");
        for (uint256 i = 0; i < amount; i++) {
            if (totalRingsPerType[ringType] == allowedForges[ringType].maxForges) {
    ...
    function forgeRing(uint256 ringId, uint256 ringType) external virtual nonReentrant {
        require(forgeEnabled, "SMOLRING:FORGING_NOT_ENABLED");
        require(ownerOf(ringId) == msg.sender, "SMOLRING:NOT_OWNER_OF_RING");
        require(allowedForges[ringType].valid, "SMOLRING:TYPE_NOT_ALLOWED_FOR_FORGING");
        require(ringProps[ringId].ringType != ringType, "SMOLRING:CANNOT_FORGE_TO_CURRENT_TYPE");
        require(
            totalRingsPerType[ringType] <= allowedForges[ringType].maxForges,
            "SMOLRING:MAX_AMOUNT_FOR_TYPE_REACHED"
        );
        if (allowedForges[ringType].requiredAmount > 0) {
            if (allowedForges[ringType].tokenType == 0) {
                IERC1155(allowedForges[ringType].contractAddress).safeTransferFrom(
                    msg.sender,
                ...
                IERC20(allowedForges[ringType].contractAddress).transferFrom(
                    msg.sender,
                    treasury,
                    allowedForges[ringType].requiredAmount
                );
    ...
    function setAllowedForges(uint256[] calldata ringTypes, ForgeType[] calldata forgeTypes) public onlyOperator {
        require(ringTypes.length == forgeTypes.length, "SMOLRING:INVALID_ARRAY_LENGTH");
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        allowedForges[ringProps[_tokenId].ringType].imageURI,
                        '", "attributes": [{"trait_type": "Type", "value": "',
                        allowedForges[ringProps[_tokenId].ringType].name,
                        '"},{"trait_type": "Reward Factor", "value": "',
                        toString(allowedForges[ringProps[_tokenId].ringType].rewardFactor),
                        '"}]}'
                    )
                )
            )
    ...
    function ringRarity(uint256 ringId) public view returns (uint256) {
        if (ringProps[ringId].ringType > 0) {
            return allowedForges[ringProps[ringId].ringType].rewardFactor;
        }
