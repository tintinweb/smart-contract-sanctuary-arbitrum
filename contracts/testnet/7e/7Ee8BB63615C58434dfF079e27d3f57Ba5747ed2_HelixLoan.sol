...
contract HelixLoan is BaseUpgradeablePausable, EIP712Upgradeable, IHelixLoan, IVersioned {
   ...
   * @param _addresses An array of address includes: HelixConfig contract, Manager proxy contract, Manager address and token in this fund
  ...
  function initialize(
    ...
    for (uint i; i < _addresses.length - 1; ) {
      require(address(_addresses[i]) != address(0), Errors.NOT_ALLOWED_ZERO_ADDRESS);
      unchecked {
        i++;
      }
    }
  ...
  function updateRepaymentReceipts(RepaymentTransaction[] calldata txs) public onlyProtocolAdmin {
    IPoolTokens poolTokens = config.getHelixPoolTokens();
    ...
    for (uint i = 0; i < txs.length; ) {
      ...
      require(!repaymentReceiptExisted(investor, txHash), Errors.REPAYMENT_RECEIPT_ALREADY_EXISTED);
      ...
      poolTokens.redeem(tokenId, repaymentTx.principal, repaymentTx.interest);
      ...
      emit RepaymentReceiptsConfirmed(
        ...
        investor,
        repaymentTx.txType,
        repaymentTx.principal, 
        repaymentTx.interest,
        oldTVL,
        getTVL()
      );
  ...
  function invest(
    ...
    require(hasAllowedUID(msg.sender), Errors.NOT_SATISFY_KYC_CONDITION_FOR_LOAN);

    require(block.timestamp <= expirationTime, Errors.EXPIRED_INVESTMENT_PLACEHOLDER);

    bytes32 investmentHash = getInvestmentHash(_msgSender(), investmentDate, amount, salt, expirationTime);

    require(!_isCancelledOrFinalized(investmentHash), Errors.INVESTMENT_HAS_BEEN_FINALIZED_OR_CANCELLED);
    ...
    if (!investmentApprovedByManager) {
      require(
        investments[_msgSender()][investmentHash].status == InvestmentStatus.PendingPayment,
        Errors.INVALID_INVESTMENT_PLACEHOLDER
      );
    }
  ...
  function investWithPermit(
    ...
    invest(investmentDate, amount, signingTime, expirationTime, investSignature);
  }
  ...
  function getTVL() public view returns(uint256) {
    return config.getHelixPoolTokens().poolTVL(address(this));
  }
  ...
  modifier onlyProtocolAdmin() {
    require(
      _msgSender() == config.protocolAdminAddress() || _msgSender() == config.protocolAdminSubstitutionAddress(),
      Errors.ONLY_ALLOWED_TO_CALL_BY_NON_ADMIN
    );
    _;
  }

  modifier onlyManager() {
    require(hasRole(OWNER_ROLE, msg.sender), Errors.CALLER_NOT_MANAGER);
    _;
  }

  modifier isNotClosed() {
    require(!closed, Errors.POOL_IS_CLOSED);
    _;
  }
}
