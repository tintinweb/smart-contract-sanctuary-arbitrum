...
contract MarketTaker is IMarketTaker, BlockContext, ClearingHouseCallee, MarketTakerStorageV1 {
    ...
    function swap(SwapParams memory params) external override returns (SwapResponse memory) {
        ...
        if (params.isClose && takerPositionSize != 0) {
            // if trader is on long side, isSwapWithBase: true, isSell: true
            // if trader is on short side, isSwapWithBase: true, isSell: false
    ...
    /// @inheritdoc IMarketTaker
    function resetAccumulatedFee(address baseToken) external override {
        // MT_OAM: only Amm
        require(msg.sender == IAmmFactory(_ammFactory).getAmm(baseToken), "MT_OAM");
        _accumulatedFeeSinceLastRepegging[baseToken] = 0;
    }
    ...
    //
    /// @inheritdoc IMarketTaker
    function getAccumulatedFee(address baseToken) external view override returns (uint256) {
        return _accumulatedFeeSinceLastRepegging[baseToken];
    }
    ...
    function getMarkPrice(address baseToken) external view override returns (uint256) {
        return IAmm(IAmmFactory(_ammFactory).getAmm(baseToken)).getPriceCurrent();
    }
    ...
    function _swap(SwapParams memory params) internal returns (InternalSwapResponse memory) {
        ...
        uint256 feeForLp = swapAstp.exchangeFee - insuranceFundFee;
        _accumulatedFeeSinceLastRepegging[params.baseToken] += feeForLp;
        ...
        uint256 lpCalcFeeRatio = (feeForLp * Constant.IQ96) / totalLiquidity;
        ...
        if (timestamp == lastSettledTimestamp || lastSettledTimestamp == 0) {
            ...
            if (totalLiquidity != 0) {
                ...
                fundingGrowthGlobal.twPremiumWithLiquidity =
                    lastFundingGrowthGlobal.twPremiumWithLiquidity +
                    FairtradeMath.mulDiv(balanceBase.toInt256(), deltaTwPremium, totalLiquidity);
            }
        }
...
abstract contract MarketTakerStorageV1 {
    ...
    mapping(address => uint256) internal _accumulatedFeeSinceLastRepegging;
}