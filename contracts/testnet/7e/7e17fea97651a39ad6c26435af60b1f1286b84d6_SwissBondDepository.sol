...
interface AddressTracker {
    function isAddressSubmitted(address _address) external view returns (bool);
    function checkToggle() external view returns (bool);
}
...
contract SwissBondDepository is Ownable {
    ...
    IERC20 public immutable Swiss; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Swiss when receives principle
    ...
    AddressTracker public addressTracker;
    ...
    constructor ( 
        ...
        addressTracker = AddressTracker(_addressTracker);
    }
    ...
    function initializeBondTerms( 
        ...
        require( _controlVariable >= 1, "Can lock adjustment" );
    ...
    function deposit( 
        ...
        if (addressTracker.checkToggle()) {
            require(addressTracker.isAddressSubmitted(msg.sender), "Not Whitelisted");
        }
        ...
        treasury.mintRewards( address(this), payout.add(fee) );
        
        if ( fee != 0 ) { // fee is transferred to dao 
            Swiss.safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Swiss.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Swiss.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Swiss.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Swiss.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
