...
contract TreeNursery is Basic {
    ...
    event Spray(address owner, uint256 nftId, uint256 day, uint256 waterUsed);
    ...
    function spray() public {
        ...
        emit Spray(msg.sender, user.nftId, today, period[quality].waterPerDay);
    }
    ...
    function claim() public {
        ...
        if (getStatusNursery(msg.sender) == 1) {
            emit Claim(user.nftId, msg.sender);
        } else {
    ...
    function submit(address _user, uint256 _result) public onlyMod {
        ...
        if (_result < 10) {
            ...
            gene.change(user.nftId, [uint256(2), 1, 0, 1]);
            emit NurseryCompleted(user.nftId, 1);
        } else if (_result < 98) {
            gene.change(
                user.nftId,
                [uint256(2), 2, (performance * 102) / 100, 1]
            );
            emit NurseryCompleted(user.nftId, 2);
        } else {
            gene.change(
                user.nftId,
                [uint256(2), 3, (performance * 105) / 100, 1]
            );
