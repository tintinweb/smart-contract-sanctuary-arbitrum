...
contract Fiat24ClientApplication is Initializable, AccessControl {
    ...
    function addClientApplication(uint256 tokenId_, string memory clientApplicationData_) external {
        if(!(tokenExists(tokenId_) && fiat24Account.ownerOf(tokenId_) == msg.sender)) {
...
interface IFiat24Account is IERC721Enumerable {
    function historicOwnership() external;
    function nickNames() external;
    function isMerchant() external;
    function merchantRate() external;
