...
contract StargatePlugin is IPlugin, Ownable {
    ...
    /// @param _params - a custom bytes params to config plugin.
    function configPlugin(bytes calldata _params) public onlyOwner {
        (address _stgRouter, address _stgLPStaking, address _stargateToken) = abi.decode(_params, (address, address, address));
        StargatePluginConfig storage _config = _getConfig();
        _config.stgRouter = _stgRouter;
        _config.stgLPStaking = _stgLPStaking;
        _config.stargateToken = _stargateToken;
    }
    ...
    /// @dev the action function called by execute.
    function _getTotalAssetsMD(bytes calldata _payload) private returns (bytes memory result) {
    ...
    function _getStargatePoolFromToken(address _token) internal returns (address) {
        ...
        revert("StargatePlugin: Pool not found for token");
    }
    ...
    /// @notice assembly to initialize the storage variable by setting its slot to the slotAddress.
    function _getConfig() internal pure returns (StargatePluginConfig storage _config) {
    ...
    function _distributeReward() internal {
        address _stargateToken = _getConfig().stargateToken;
