...
contract MintingStation is AccessControl {
     ...
     * 2 flows: 1. mint from user ; 2. mint from DGG SC (the ticket redeem case)
     */
    function mintCollectible(string memory URI, bytes memory signature, address contractAddress, address to) external returns (uint256) {
        address signer = verify(URI, signature);
        require(whiteListSigner[signer], "Invalid signer");
        uint256 tokenId = NFT721(contractAddress).safeMint(to, URI);
...
contract Tournament is AccessControl, ReentrancyGuard {
    ...
    function createRedeemRequest(
        ...
        for(uint256 i =0 ; i< rewardClaimTickets.length; i++) {
            ...
            redeemRequest.createtor = msg.sender;
    ...
    function claimTicket(uint256 requestId, bytes memory signature) external {
        ...
        require(redeemRequest.createtor == msg.sender, "You are not the owner of the request");
    ...
    function getLisTicketCollectionAddress() public view returns(address[] memory) {
...
// contract Tournament2222 is AccessControl, ReentrancyGuard {
//     using SafeMath for uint256;
//     using SafeERC20 for IERC20;
