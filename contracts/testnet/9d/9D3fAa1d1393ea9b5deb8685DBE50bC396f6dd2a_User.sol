...
contract User is IUser, Initializable, DAOAccessControlled {
    ...
    ILoot8SignatureVerification public sigVerificationContract;
    ...
    // Mapping Users Loot8 wallet address => A nonce counter for user signatures
    ...
    function initializeV2(address _verifier) public reinitializer(2) {
        require(_verifier != address(0), "INVALID ADDRESS");
        sigVerificationContract = ILoot8SignatureVerification(_verifier);
    }
    ...
    */
    ...
    function linkExternalAccount(address _account, bytes memory _signature) external {
        ...
        require(sigVerificationContract.verify(
            _account,
            _account,
            user,
            sigVerificationContract.linkMessage(), 
            nonces[user].current(), 
            _signature
        ), "UNAUTHORIZED");
