...
import {ILT} from "@hopelend/core/contracts/interfaces/ILT.sol";
import {IVotingEscrow} from "@hopelend/core/contracts/interfaces/IVotingEscrow.sol";
import {IERC20, IERC20Detailed} from "@hopelend/core/contracts/dependencies/openzeppelin/contracts/IERC20Detailed.sol";
import {IHTokenRewards} from "@hopelend/core/contracts/interfaces/IHTokenRewards.sol";
import {IVariableDebtTokenRewards} from "@hopelend/core/contracts/interfaces/IVariableDebtTokenRewards.sol";
import {IACLManager, IPoolAddressesProvider} from "@hopelend/core/contracts/interfaces/IACLManager.sol";
import {IPool} from "@hopelend/core/contracts/interfaces/IPool.sol";
import {SafeCast} from "@hopelend/core/contracts/dependencies/openzeppelin/contracts/SafeCast.sol";
import {DataTypes} from "@hopelend/core/contracts/protocol/libraries/types/DataTypes.sol";
import {Errors} from "@hopelend/core/contracts/protocol/libraries/helpers/Errors.sol";
import {TransferHelper} from "light-lib/contracts/TransferHelper.sol";
import {IChildGaugeFactory} from "../interfaces/IChildGaugeFactory.sol";
import {IChildLendingGauge} from "../interfaces/IChildLendingGauge.sol";

contract ChildLendingGauge is IChildLendingGauge {
  ...
  constructor(address _pool, address _ltToken) {
    require(_pool != address(0), Errors.ZERO_ADDRESS_NOT_VALID);
    require(_ltToken != address(0), Errors.ZERO_ADDRESS_NOT_VALID);
  ...
  function initialize(address _underlyingAsset) public override {
    require(_underlyingAsset != address(0), Errors.ZERO_ADDRESS_NOT_VALID);
    require(factory == address(0), "GP002");
    factory = msg.sender;
    address votingEscrowDelegation = IChildGaugeFactory(factory).votingEscrow();
    votingEscrow = IVotingEscrow(votingEscrowDelegation);
  ...
  function hvCheckpoint(address _addr) public override {
    ...
    uint256 ltBalance = IERC20(address(ltToken)).balanceOf(address(this));
    if (ltBalance > 0) {
      uint256 currentWeek = block.timestamp / _WEEK;
      inflationRate[currentWeek] += ltBalance / ((currentWeek + 1) * _WEEK - block.timestamp);
      TransferHelper.doTransferOut(address(ltToken), factory, ltBalance);
    }
  ...
  function userCheckpoint(address _addr) external returns (bool) {
    require((msg.sender == _addr) || (msg.sender == address(factory)), "GP000");
...
interface IChildLendingGauge {
  ...
  function initialize(address _underlyingAsset) external;
