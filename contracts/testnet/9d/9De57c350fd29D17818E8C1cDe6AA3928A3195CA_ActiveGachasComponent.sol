...
uint256 constant ID = uint256(keccak256("game.piratenation.activegachascomponent.v2"));
...
contract ActiveGachasComponent is BaseStorageComponentV2 {
    ...
    function append(
        ...
        _emitSetBytes(
            entity,
            abi.encode( s.activeGachaEntities)
        );
    }
   ...
   function removeValueAtIndexes(
       ...
       for (uint256 i = 0; i < indexes.length; i++) {
           ...
           if (indexToRemove != lastIndexInArray) {
               s.activeGachaEntities[indexToRemove] =  s.activeGachaEntities[lastIndexInArray];
           }
           // Pop the last value
    ...
    {
        if (has(entity)) {
            Layout memory s = ActiveGachasComponentStorage.layout().entityIdToStruct[entity];
    ...
    function _setValue(
        ...
        _emitSetBytes(
            entity,
            abi.encode(value.activeGachaEntities)
        );
    }
