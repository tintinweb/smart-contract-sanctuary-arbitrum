...
contract NOXTokenSale is OwnableUpgradeable {
    ...
    modifier onlyWhitelisted() {
        require(
            whitelistAmounts[msg.sender].remainingAmount != 0,
            "Not whitelisted"
        );
        _;
    }
    ...
    function purchaseTokens(
        ...
        if (usdtAmount > 0) USDT.transferFrom(msg.sender, owner(), usdtAmount);
        if (usdcAmount > 0) USDC.transferFrom(msg.sender, owner(), usdcAmount);
    ...
    function calculateNOXAmount(
        ...
        uint256 totalUSDValue = usdtAmount + usdcAmount;

        uint256 noxAmount = totalUSDValue * noxPerUsd;
