...
contract ManagerFacet is Calculator {
    ...
    event BaseFeeChanged(uint fee);
    event FeeDiscountChanged(address user, uint discount);
    event CurrencyAddressChanged(address addr);
    event TemplateAddressChanged(address addr);
    event WithdrawServiceIncome(address indexed operator, address indexed to, uint amount);
    event AccountTypeChanged(address indexed user, uint indexed accountType);
    event TerminateService(bytes32 indexed id, address indexed operator, address buyer, address seller, uint amount, uint fee);

    function getUSDTAddress() external view returns (address) {
        return PaymentStorage.layout().usdt;
    }
    ...
    function getServiceIncome() external view returns (uint) {
        return PaymentStorage.layout().serviceIncome;
    }
    ...
    function setUSDTAddress(address _addr) external {
        ...
        PaymentStorage.layout().usdt = _addr;
        emit CurrencyAddressChanged(_addr);
    }
    ...
    function withdrawServiceIncome(address _to, uint _amount) external {
        if (_to == address(0)) revert AddressIsZero();
        if (_amount == 0) revert InvalidAmount();
        AccessControlStorage.enforceIsOwner();
        PaymentStorage.Layout storage layout = PaymentStorage.layout();
        if (_amount > layout.serviceIncome) revert InsufficientBalance();

        unchecked {
            layout.serviceIncome -= _amount;
        }
        IERC20(layout.usdt).transfer(_to, _amount);

        emit WithdrawServiceIncome(msg.sender, _to, _amount);
    }
    ...
    function _terminate(
        ...
        unchecked {
            if (_amount == 0) {
                layout.userAccounts[service.buyer].balance += deposit;
                service.security = 0;
            } else {
                uint remaining = deposit - _amount;
                fee = _getServiceFee(service.seller, _amount);
                layout.userAccounts[service.buyer].balance += remaining;
                layout.userAccounts[service.seller].balance += (_amount - fee);
