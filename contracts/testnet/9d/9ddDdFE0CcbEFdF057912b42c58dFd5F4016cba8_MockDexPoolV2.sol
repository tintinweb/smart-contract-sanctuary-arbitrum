 ...
 */
contract MockDexPoolV2 is IValidatedDataReciever, AccessControlEnumerable {
    ...
    event RequestReceivedV2(bytes32 reqId, uint256 data, uint256, uint256);
    ...
    function sendRequestTestV2(
        uint256 testData_,
        address[] memory receiveSide,
        uint256[] memory chainId,
        uint256 cursor
    ) public onlyRole(OPERATOR_ROLE) {
        for (uint i = cursor; i < receiveSide.length; i++) {
            require(receiveSide[i] != address(0), "MockDexPool: zero address");
        }
        uint256 nonce = IBridgeV2(bridge).nonces(msg.sender);
        bytes32 requestId = RequestIdLib.prepareRequestId(
            bytes32(uint256(uint160(receiveSide[receiveSide.length - 1]))),
            chainId[receiveSide.length - 1],
            bytes32(uint256(uint160(msg.sender))),
            block.chainid,
            nonce
        );
        bytes memory output = abi.encodeWithSelector(
            bytes4(keccak256(bytes("receiveRequestTest(uint256,bytes32,address[],uint256[],uint256)"))),
            testData_,
            requestId,
            receiveSide,
            chainId,
    ...
    function receiveRequestTest(uint256 newData, bytes32 reqId) public {
        ...
        emit RequestReceivedV2(reqId, newData, 1, 1);
    }


    function receiveRequestTest(
        uint256 newData, 
        bytes32 reqId, 
        address[] memory receiveSide, 
        uint256[] memory chainId, 
        ...
        if (cursor != receiveSide.length - 1) {
            cursor++;
            sendRequestTestV2(newData, receiveSide, chainId, cursor);
        } else {
            testData = newData;
        }
        emit RequestReceived(newData);
        emit RequestReceivedV2(reqId, newData, receiveSide.length, cursor);
    }
