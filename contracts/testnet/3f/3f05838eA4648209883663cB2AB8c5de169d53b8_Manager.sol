...
interface IStrategy {
  function name() external view returns (string memory);
  function signal(uint256) external view returns (bool);
}
...
import { State, KeeperRegistryInterface } from "./interfaces/chainlink/KeeperRegistryInterface.sol";
import "./interfaces/chainlink/IERC677.sol";
...
contract Manager is ERC721Enumerable, Ownable {
  ...
  function createVault(VaultType _vaultType, string memory _strategyId, address[] memory _inputs, bytes memory _config) external payable {
    ...
    // IKeeper(upkeepMap[msg.sender]).addVault(vault);
