...
contract Decoder {
    function decodeSingle(bytes32 _input) external pure returns (uint256[4] memory) {
        uint256[4] memory decoded = unpack(_input);
        return (decoded);
    }

    function decodeMultiplePair(bytes32[] memory val) external pure returns (uint256[4][] memory) {
