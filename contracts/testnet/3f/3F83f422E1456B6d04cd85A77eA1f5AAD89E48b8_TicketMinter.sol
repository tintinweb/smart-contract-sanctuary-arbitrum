    ...
    ERC1155Burnable
{
    ...
    function crossChainMint(
        ...
        require(_amount_USD_8DP + int(paymentTolerance) >= int(price[_id]));
    ...
    function mint(
        ...
        uint256 _percentage = feePercentage[_id];
    ...
    ) public payable onlyPermitted {
        forSale[_id] = true;
        capacity[_id] = _capacity;
        price[_id] = _amount_USD_8DP;
        revenueAddress[_id] = _revenueAddress;
        feePercentage[_id] = _feePercentage;

        for (uint256 i = 0; i < crosschainIds.length; i++) {
            uint256 fee = getTransferOutFee(crosschainIds[i]);
            bytes memory _target = abi.encodePacked(
                crosschainContracts[crosschainIds[i]]
            );
            bytes memory payload = abi.encodeWithSelector(
                TicketMinter.crossChainCreateTicket.selector,
                _id,
                _amount_USD_8DP,
                _revenueAddress,
                _feePercentage
            );
            bytes memory messageData = _getMessageData(payload, _target);
    ...
    function setCrossChainContracts(
        ...
        require(_chainIds.length == _addresses.length);
        crosschainIds = _chainIds;
        for (uint256 i = 0; i < _addresses.length; i++) {
            crosschainContracts[_chainIds[i]] = _addresses[i];
        }
    }

    function sendCrossChainSetForSale(
        uint256 _id,
        bool _forSale
    ) public onlyPermitted {
        ...
        for (uint256 i = 0; i < crosschainIds.length; i++) {
            uint256 fee = getTransferOutFee(crosschainIds[i]);
            bytes memory _target = abi.encodePacked(crosschainContracts[i]);
            bytes memory payload = abi.encodeWithSelector(
                TicketMinter.crossChainSetForSale.selector,
                _id,
                _forSale
            );
            bytes memory messageData = _getMessageData(payload, _target);
    ...
    function crossChainSetForSale(uint256 _id, bool _forSale) external {}

    function crossChainCreateTicket(
        uint256 _id,
        uint256 _amount_USD_8DP,
        address _revenueAddress,
        uint256 _feePercentage
    ...
    function _getMessageData(
        ...
        bytes memory messageByte = abi.encode(messageData);
        return messageByte;
    }
    ...
    function setCapacity(uint256 _id, uint256 _capacity) public onlyPermitted {
        capacity[_id] = _capacity;
    }
