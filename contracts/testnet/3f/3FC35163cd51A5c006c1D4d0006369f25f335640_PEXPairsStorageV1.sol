...
interface ITrading{
    function maxPosUsdt() external view returns(uint);
}
...
contract PEXPairsStorageV1 is Initializable {
    ...
    function updatePair(uint _pairIndex, Pair calldata _pair) external onlyGov feedOk(_pair.feed) feeListed(_pair.feeIndex){
        ...
        delete isPairListed[p.from][p.to];
        isPairListed[_pair.from][_pair.to] = true;

        p.from = _pair.from;    //dev版本目前特有的
        p.to = _pair.to;    //dev版本目前特有的
        p.feed = _pair.feed;
        p.spreadP = _pair.spreadP;
        p.groupIndex = _pair.groupIndex;    //dev版本目前特有的
        p.feeIndex = _pair.feeIndex;
        
        emit PairUpdated(_pairIndex);
    }
    
    function delPair(string calldata from, string calldata to) external onlyGov {
        delete isPairListed[from][to];
    }
    ...
    function pairMaxLevPosUsdt(uint _pairIndex) external view returns(uint){
        ITrading t = ITrading(storageT.trading());
        return (fees[pairs[_pairIndex].feeIndex].maxLexPosUsdt == 0) ?
            t.maxPosUsdt() : fees[pairs[_pairIndex].feeIndex].maxLexPosUsdt;
    }
    function pairMaxGainP(uint _pairIndex) external view returns(uint){
        ICallbacks c = ICallbacks(storageT.callbacks());    
        return (groups[pairs[_pairIndex].groupIndex].maxGainP == 0) ? 
            c.MAX_GAIN_P() : groups[pairs[_pairIndex].groupIndex].maxGainP;
    }
