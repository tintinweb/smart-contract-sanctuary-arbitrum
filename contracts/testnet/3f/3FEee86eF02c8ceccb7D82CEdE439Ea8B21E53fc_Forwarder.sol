...
contract Forwarder is NonblockingLzApp {
    ISemaphore public groupManager;
    error Forwarder__NoBalanceLeftForGas();
    ...
    constructor(
        ...
        groupManager = ISemaphore(groupManagerAddress);
    }

    function send(
        uint16 _dstChainId,
        address _dstAddress,
        ProofData calldata proofData // the number of pings
    ) public payable {
        if (address(this).balance <= 0) revert Forwarder__NoBalanceLeftForGas();

        bytes memory payload = abi.encode(proofData);

        // use adapterParams v1 to specify more gas for the destination
        uint16 version = 1;
        uint gasForDestinationLzReceive = 3_500_000;
    ...
    function lzReceive(
        ...
        ProofData memory proofData = abi.decode(_payload, (ProofData));

        groupManager.verifyProof(
            proofData.groupId,
