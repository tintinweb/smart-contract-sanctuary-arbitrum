...
contract AcidVault is ERC4626, Ownable {
    ...
    constructor(address _asset, address _rewardTracker, address _stakingYieldPool, address _wstETH, address _camelotDex)
        ERC4626(IERC20(_asset))
        ERC20("Staked sAcid", "ssAcid")
    {
    ...
    function redeem(uint256 shares, address receiver, address owner) public override returns (uint256) {
        ...
        if (requiredAssets > availableAssets) {
            rewardTracker.withdraw(requiredAssets.sub(availableAssets), 0);
    ...
    function withdraw(uint256 assets, address receiver, address owner) public override returns (uint256) {
        ...
        if (assets > availableAssets) {
            rewardTracker.withdraw(assets.sub(availableAssets), 0);
    ...
    function totalAssets() public view override returns (uint256) {
        uint256 earned = stakingYieldPool.earned(address(this));
    ...
    function _collectRewards() internal {
        ...
        if (reward > 0) {
            ...
            camelotDex.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                reward, 0, path, address(this), address(0), block.timestamp
            );
        }
    }
}
