...
// interface ITokenURI {
//     function secondConcatCharacterRender_(uint256 characterId, uint256 slot)
//         external
//         view
//         returns (string memory data);
// }

contract CreatorHeroesStorage is Ownable, DataURIHandler {
    ...
    function setBattleCharacterData(uint256 tokenId, uint256[] memory imageIds) external {
        _checkCategolyId(imageIds, 1);
        _setStructureData(tokenId, imageIds, 0);
    }

    // onlyowner
    function setPartsData(uint256 tokenId, uint256[] memory imageIds) external onlyOwner {
        _setStructureData(tokenId, imageIds, 1);
    }
    ...
    function createSpecialBattleCharacterDataURI(
        ...
        //_createDataURI(uint256 slot, uint256 characterData, bytes32 personalityData, uint256 status)
        return
            _createSpecialBattleCharacterDataURI(characterData, personalityData, status, pointer, specialCharacterData);
    }

    function characterRender(uint256 characterData) public view returns (string memory) {
        return _render(characterData, 0);
    }

    function partsRender(uint256 characterData) public view returns (string memory) {
        return _render(characterData, 1);
    }

    function firstRender(uint256 characterData) public view returns (string memory) {
        return _firstRender(characterData);
    }
    ...
    function getStructureData(uint256 tokenId, uint256 index, uint256 slot) public view returns (uint256 value) {
        return _getStructureData(tokenId, index, slot);
    }
}
...
import {PersonalityHandler} from "./PersonalityHandler.sol";

// import {Strings} from "@openzeppelin/contracts/utils/Strings.sol";
// import {Ownable} from "@openzeppelin/contracts/access/Ownable.sol";
// import {Base64} from "@openzeppelin/contracts/utils/Base64.sol";

abstract contract DataURIHandler is StatusHandler, ImageDataHandler, PersonalityHandler {
