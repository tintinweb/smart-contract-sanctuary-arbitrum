...
contract TestShib is ERC20, Ownable {
    ...
    function swapBack() internal isSwapping {
        ...
        uint256 amountOfTShibBurn = (taxAmount * burnFee) / (totalFee);
        uint256 amountOfTShibLp = (taxAmount * liquidityFee) / (totalFee);
        uint256 amountOfTShibForHolders = (taxAmount * holdersFee) / (totalFee);
        uint256 amountOfTShibForStaking = (taxAmount * stakingFee) / (totalFee);

        taxAmount -= amountOfTShibBurn;
        taxAmount -= amountOfTShibLp;
        taxAmount -= amountOfTShibForHolders;
        taxAmount -= amountOfTShibForStaking;
        ...
        _transfer(address(this), DEAD, amountOfTShibBurn);
        _transfer(address(this), holdersWallet, amountOfTShibForHolders);
        _transfer(address(this), stakingRewardWallet, amountOfTShibForStaking);
        ...
        emit SwapBack(amountOfTShibBurn, amountOfBackTokenMkt, amountOfTShibLp, amountOfTShibForStaking, amountOfTShibForHolders, amountOfBackTokenDev, block.timestamp);
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "ArbShib: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "ArbShib: index out of bounds");
