...
contract RwaxStaking is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _pool, address _rwax, address _llp, address _weth, address _ethUnwrapper)
        external
        initializer
    {
        require(_pool != address(0), "RwaxStaking:initialize: invalid address");
        require(_rwax != address(0), "RwaxStaking:initialize: invalid address");
        require(_llp != address(0), "RwaxStaking:initialize: invalid address");
        require(_weth != address(0), "RwaxStaking::initialize: invalid address");
        require(_ethUnwrapper != address(0), "RwaxStaking::initialize: invalid address");
        __Ownable_init();
        __ReentrancyGuard_init();
        pool = IPool(_pool);
        RWAX = IBurnableERC20(_rwax);
        LLP = IERC20(_llp);
        WETH = IWETH(_weth);
    ...
    function pendingRewards(address _to) external view returns (uint256) {
        ...
        uint256 totalStaked = RWAX.balanceOf(address(this));
    ...
    function stake(address _to, uint256 _amount) external nonReentrant {
        require(_amount > 0, "RwaxStaking::stake: invalid amount");
        update();
        ...
        RWAX.safeTransferFrom(msg.sender, address(this), _amount);
        RWAX.burn(_taxAmount);
    ...
    function unstake(address _to, uint256 _amount) external nonReentrant {
        ...
        require(_userInfo.amount >= _amount, "RwaxStaking::unstake: insufficient staked amount");
        update();
        ...
        RWAX.safeTransfer(_to, _amount);
        emit Unstaked(sender, _to, _amount);
    }

    function claimRewards(address _to) external nonReentrant {
        require(_to != address(0), "RwaxStaking::transferRewards: invalid address");
        update();
        address sender = msg.sender;
    ...
    function claimRewardsToSingleToken(address _to, address _tokenOut, uint256 _minAmountOut) external nonReentrant {
        require(_to != address(0), "RwaxStaking::claimRewardsToSingleToken: invalid address");
        require(withdrawableTokens[_tokenOut], "RwaxStaking::claimRewardsToSingleToken: !withdrawable");
        update();
        address sender = msg.sender;
        UserInfo memory user = userInfo[sender];
        int256 accumulatedReward = int256((user.amount * accRewardPerShare) / ACC_REWARD_PRECISION);
    ...
    function update() public {
        uint256 _totalStakedAmount = RWAX.balanceOf(address(this));
    ...
    function setController(address _controller) external onlyOwner {
        require(_controller != address(0), "RwaxStaking::setController: invalid address");
        controller = _controller;
        emit ControllerSet(_controller);
    }

    function setTokenWithdrawable(address _token, bool _allowed) external onlyOwner {
        require(_token != address(RWAX) && _token != address(0), "RwaxStaking::setTokenWithdrawable: invalid address");
    ...
    function setRewardsPerSecond(uint256 _rewardsPerSecond) external onlyController {
        require(_rewardsPerSecond <= MAX_REWARD_PER_SECOND, "RwaxStaking::setRewardsPerSecond: > MAX_REWARD_PER_SECOND");
        update();
    ...
    function swap(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _minAmountOut)
        ...
        onlyController
    {
        require(_tokenIn != address(RWAX) && _tokenOut != _tokenIn, "RwaxStaking::swap: invalid path");
        ...
        require(actualAmountOut >= _minAmountOut, "RwaxStaking::swap: !slippage");
    ...
    function convert(address _token, uint256 _amount, uint256 _minLlpAmount) external onlyController {
        require(_token != address(RWAX) && _token != address(0), "RwaxStaking::convertToLlp: invalid address");
        if (_amount > 0) {
            ...
            require(_amountOut >= _minLlpAmount, "RwaxStaking::convertToLlp: !slippage");
    ...
    function recoverFund(address _token, address _to, uint256 _amount) external onlyOwner {
        require(_token != address(RWAX) && _token != address(0), "RwaxStaking::recoverFund: invalid token address");
        require(_to != address(0), "RwaxStaking::recoverFund: invalid address");
        ...
        returns (uint256)
    {
        ...
        require(_amountOut >= _minAmountOut, "RwaxStaking::transferRewardsToSingleToken: !slippage");
