...
contract LIRASacrifice is Context {
     ...
     * @dev based on @openzeppelin/[email protected] ./contracts/access/Ownable.sol
     */
    ...
    modifier canRedeem {
        ...
        require(block.timestamp > _rounds[_rounds.length - 1].end, 'LIRA_SACRIFICE_SACRIFICE_NOT_ENDED');
        _;
    }
    ...
    // returns true if sacrifies phase is over, false otherwise
    function ended() public view returns (bool ended_) {
        ended_ = _rounds.length > 0 && block.timestamp > _rounds[_rounds.length - 1].end;
    }
    ...
    function quoteSacrifice(uint amount_) public view canSacrifice returns (uint) {
        return amount_.mul(10).div(round().bonus).mul(10 ** 8);
    }
