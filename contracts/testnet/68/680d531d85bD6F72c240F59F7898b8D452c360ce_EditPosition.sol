...
contract EditPosition is Roles, ERC2771Context, IEditPosition {
    ...
    function editParams(uint256 _basePrice, uint256 _minSize)
        external
        hasRole(GOV)
    {
        minSize = _minSize;
    ...
    function executeEditCopy(bytes32[] calldata copyIds)
        ...
        hasRole(ORACLE)
    {
        uint256 oracleFee = _getFeeExecution(200000, copyIds.length);
    ...
    function _incPosition(
        ...
        uint256 openFee = ((amount - oracleFee) *
            position.leverage *
            pair.openFee) / (BPS**2 + position.leverage * pair.openFee);
