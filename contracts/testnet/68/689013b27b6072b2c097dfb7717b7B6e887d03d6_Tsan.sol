...
contract Tsan is ERC20, Ownable {
    ...
    uint public dev_marketSupply = (900000000 * 10 ** decimals());
    uint public supplyLeft = (maxSupply - supply);
    uint public supply;

    constructor() ERC20("Tsan Token", "TSAN") {
        _mint(0xBB8A3435c6A42fF6576920805B36f578aeCa4b58, dev_marketSupply);
        supply = dev_marketSupply;
    }

    function mint(address _to, uint _amount) external {
        require(admins[msg.sender], "Cannot mint if not admin");
        require(_amount <= supplyLeft, "Amount exceed maxSupply");
        _mint(_to, _amount);
        supply += _amount;
    }
