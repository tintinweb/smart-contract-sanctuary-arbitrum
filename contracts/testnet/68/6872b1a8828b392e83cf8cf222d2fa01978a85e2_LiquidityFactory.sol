...
contract LiquidityFactory is ILiquidityFactory,Ownable {
    ...
    mapping(address=>bool) public wPair;
    address public wFactory;
    ...
    function addWPair(address pair,bool flag)external onlyOwner {
        wPair[pair] = flag;
    }

    function setWFactory(address factory)external onlyOwner {
        wFactory = factory;
    }
    ...
    function addLiquidity(
        ...
        address _to = to;
        bool flag = wPair[pairFor(wFactory,_tokenA,_tokenB)];
    ...
    function addLiquidityETH(
        ...
        bool flag = wPair[pairFor(wFactory,token,WETH)];
        ...
        address pair = ISwapFactory(_factory).pairFor(token, WETH);

        IApproveProxy(_APPROVE_PROXY_).claimTokens(token, msg.sender, pair, amountToken);
        IWETH(WETH).deposit{value : amountETH}();
        assert(IWETH(WETH).transfer(pair, amountETH));
        liquidity = IPancakePair(pair).mint(address(this));
        address lPair = pairFor(_factory,token,WETH);
    ...
    function removeLiquidity(
        ...
        bool flag = wPair[pairFor(wFactory,tokenA,tokenB)];
        address _factory ;
        address _tokenA = tokenA;
        address _tokenB = tokenB;
        uint _liquidity = liquidity;
