...
contract CrossChainMsgHandle is Ownable, ICrossChainMsgSend {
    ...
    string public name = "CrossChainMsgContract";
    bytes32 public DOMAIN_SEPARATOR;
    address[] public signers;

    mapping(uint16 => mapping(string => uint)) public nonces;
    mapping(address => uint) public indexes;
    mapping(address => bool) public authorized;
    mapping(uint => mapping(string => mapping(address => bool))) public signed;
    mapping(uint => mapping(string => bool)) public txCompleted;

    constructor(address[] memory signers_, uint required_) {
        required = required_;
    ...
    function send(
        ...
        emit SendMsg(msg.sender, _chainId, _destination, _payload, msg.value);
    }
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encode(
                        keccak256(
                            "Withdraw(string fromHash,uint16 fromChainId,uint nonce,string bytes destination,bytes payload)"
                        ),
                        _fromHash,
                        _chainId,
