    ...
    UXDControllerStorage
{
    ...
    /// @param tokenAddress the asset token address
    ...
    function whitelistAsset(
        ...
        if (!tokenAddress.isContract()) {
            revert CtrlAddressNotContract(tokenAddress);
        }
        whitelistedAssets[tokenAddress] = isWhitelisted;
        if (isWhitelisted) {
            _addAsset(tokenAddress);
        } else {
            _removeAsset(tokenAddress);
        }
    ...
    function _addAsset(address tokenAddress) private {
        for (uint256 i = 0; i < assetList.length; i++) {
            if (tokenAddress == assetList[i]) {
                return;
            }
        }
    ...
    function _removeAsset(address tokenAddress) private {
        ...
        if (foundIndex != type(uint256).max) {
            assetList[foundIndex] = assetList[assetList.length];
            assetList.pop();
        }
    }
    ...
    function mint(
        ...
        address depository = router.findDepositoryForAssetDeposit(assetToken, assetAmount);
        ...
        returns (uint256)
    {
        ...
        address depository = router.findDepositoryForAssetDeposit(collateral, amount);
...
contract UXDToken is Ownable, IRedeemable {
 /*//////////////////////////////////////////////////////////////
                                 EVENTS
    //////////////////////////////////////////////////////////////*/
    ...
    error InvalidSigner(address owner, address recovered);

    error NotController(address msgSender);

    error OverSupplyCap(uint256 supplyCap, uint256 newTotalSupply);

    error NotAllowed(uint256 allowance, uint256 amount);
    ...
    string public constant name = "UXD";

    // solhint-disable-next-line const-name-snakecase
    string public constant symbol = "UXD";
