...
contract Vault is Ownable, ERC4626 {
    ...
    address manager;
    address aori;
    uint256 public cooldown = 604800; //7 Day cooldown
    ...
    function depositAssets(uint256 assets, address receiver, uint256 lockUpTime) public {
        require(lockUpTime == 0 || lockUpTime == 2630000 || lockUpTime == 7890000 || lockUpTime == 15780000 || lockUpTime == 31560000);
        if(lockUpTime == 0) {
            cooldownTime[msg.sender] = block.timestamp + cooldown;
            deposit(assets, receiver);
            IAORI(aori).stakeLPTokens(Vault(address(this)), receiver, balanceOf(receiver), cooldown);
        } else {
            cooldownTime[msg.sender] = block.timestamp + lockUpTime;
            deposit(assets, receiver);
            IAORI(aori).stakeLPTokens(Vault(address(this)), receiver, balanceOf(receiver), lockUpTime);
        }
    }
...
interface IAORI is IERC20 {
    ...
    function whitelistVault(ERC4626 vault, address oracle) external;
    ...
    function claimAORI(address receiver) external ;

    function claimAccrual() external;
...
contract AoriLens {
    ...
    function tableData(OrderbookFactory orderbookFactory) public view returns (Orderbook[] memory, uint256[] memory, address[] memory, string[] memory, bool[] memory, uint256[] memory, uint256[] memory) {
        Orderbook[] memory uniqueCallBooks = doesCallOptionExistAtStrike(orderbookFactory);
        Orderbook[] memory uniquePutBooks = doesPutOptionExistAtStrike(orderbookFactory);
        Orderbook[] memory combinedBooks = new Orderbook[](uniqueCallBooks.length + uniquePutBooks.length);
        uint256[] memory strikes = new uint256[](combinedBooks.length);
        address[] memory options = new address[](combinedBooks.length);
        string[] memory names = new string[](combinedBooks.length);
        bool[] memory isCalls = new bool[](combinedBooks.length);
        uint256[] memory sizes = new uint256[](combinedBooks.length);
        uint256[] memory cumVols = new uint256[](combinedBooks.length);
        for(uint256 i = 0; i < uniqueCallBooks.length; i++) {
            combinedBooks[i] = uniqueCallBooks[i];           
        }
        for(uint256 i = 0; i < uniquePutBooks.length; i++) {
            combinedBooks[uniqueCallBooks.length + i] = uniquePutBooks[i];          
        }
        for (uint256 i; i < combinedBooks.length; i++) {
            strikes[i] = AoriCall(address(combinedBooks[i].OPTION())).strikeInUSDC();
            options[i] = address(combinedBooks[i].OPTION());
            names[i] = ERC20(address(combinedBooks[i].OPTION())).name();
            isCalls[i] = true;
            sizes[i] = AoriCall(address(options[i])).totalSupply();
            cumVols[i] = combinedBooks[i].totalVolume();
        }
        return(combinedBooks, strikes, options, names, isCalls, sizes, cumVols);
    }

function doesCallOptionExistAtStrike(OrderbookFactory orderbookFactory) public view returns (Orderbook[] memory) {
    Orderbook[] memory orderbooks = getActiveOrderbooks(orderbookFactory);
    Orderbook[] memory orderbooksClean = new Orderbook[](orderbooks.length);
    uint256[] memory strikes = new uint256[](orderbooks.length);
    ...
    for (uint256 i = 0; i < orderbooks.length; i++) {
        if(!AoriCall(address(orderbooks[i].OPTION())).isCall()) {
            break;
        }
        uint256 currentStrike = AoriCall(address(orderbooks[i].OPTION())).strikeInUSDC();
        bool strikeExists = false;
        for (uint256 j = 0; j < count; j++) {
            if (strikes[j] == currentStrike) {
                strikeExists = true;
                break;
            }
        }
        if (!strikeExists) {
            orderbooksClean[count] = orderbooks[count];
        }
    }
    Orderbook[] memory finalOrderbooks = new Orderbook[](count);
    for (uint256 i = 0; i < count; i++) {
        finalOrderbooks[i] = orderbooksClean[i];
    }
    return finalOrderbooks;
}

function doesPutOptionExistAtStrike(OrderbookFactory orderbookFactory) public view returns (Orderbook[] memory) {
    Orderbook[] memory orderbooks = getActiveOrderbooks(orderbookFactory);
    Orderbook[] memory orderbooksClean = new Orderbook[](orderbooks.length);
    uint256[] memory strikes = new uint256[](orderbooks.length);
    address[] memory options = new address[](orderbooks.length);
    ...
    for (uint256 i = 0; i < orderbooks.length; i++) {
        ...
        uint256 currentStrike = AoriPut(address(orderbooks[i].OPTION())).strikeInUSDC();
        bool strikeExists = false;
        for (uint256 j = 0; j < count; j++) {
            if (strikes[j] == currentStrike) {
                strikeExists = true;
        ...
        if (!strikeExists) {
            strikes[count] = currentStrike;
            orderbooksClean[count] = orderbooks[count];
            options[count] = address(orderbooks[count].OPTION());
            names[count] = ERC20(address(orderbooks[count].OPTION())).name();
            isCalls[count] = false;
            sizes[count] = AoriCall(address(options[count])).totalSupply();
            count++;
        }
    }
    Orderbook[] memory finalOrderbooks = new Orderbook[](count);
