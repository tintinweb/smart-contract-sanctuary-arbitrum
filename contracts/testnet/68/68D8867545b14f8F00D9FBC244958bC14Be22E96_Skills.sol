...
import "token-bridge-contracts/contracts/tokenbridge/libraries/aeWETH.sol";
...
contract Skills {
     ...
     * who can change this on the smart contract if it ever needs to change.
     */
    function stake(uint256 amount) payable external {
        require(amount <= _maximumStake, "This amount is beyond the maximum stake.");
        require(amount > 0, "You must specify an amount > 0.");
        require(msg.value == amount, "The amount of eth sent is not equal to the amount.");

        // convert the value into the WETH equivalent and deposit to this contract
        aeWETH(_wxpAddress).depositTo(address(this));
    }

    // just a helper function for now to get the currency back while testing
    function testWithdraw(uint256 amount) external {
        aeWETH(_wxpAddress).withdrawTo(msg.sender, amount);
    }
}
