...
contract PresaleArb is Ownable, ReentrancyGuard {
    ...
    function _simulateClaim(
        ...
        if (block.timestamp > endTime + LOCK_TIME) {
            uint256 tokensToReceive = userTotalTokensToReceive[msg.sender] -
                userClaimedAmount[msg.sender];

            return (userTotalTokensToReceive[msg.sender], tokensToReceive);
        } else {
            uint256 totalTokensToReceive = userTotalTokensToReceive[msg.sender];

            uint256 baseTokenToReceive = (totalTokensToReceive * 200) / 1000;
            ...
            return (userTotalTokensToReceive[msg.sender], currentAvailableAmt);
        }
    }
