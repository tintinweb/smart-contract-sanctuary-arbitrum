...
contract Processor is Roles, ERC2771Context, IProcessor {
    ...
    function selfClosePosition(
        ...
        _executePosition(id, chainlinkPrice, true, closeType, 0);
    }
    ...
    function closePositionGasLess(
        ...
        _executePosition(
            ...
            false,
            1,
            int256(_getFeeExecution(300000, 1))
        );
    }
    ...
    function closePositions(
        ...
        int256 oracleFee = int256(_getFeeExecution(300000, length));
    ...
    function executePosition(
        ...
        require(closeType != 1, "!oracle-cant-close");
        _executePosition(id, price, isSelf, closeType, oracleFee);
    }
    ...
    function _executePosition(
        ...
        if (
            closeType == 0 &&
            finalAmount > int256((position.amount * pair.liqThreshold) / BPS)
        ) {
            revert("!liquidate");
        }
        if (closeType == 2) {
        ...
        int256 amountShare = _getShareAmount(
            id,
            price,
            pnl,
            finalAmount,
    ...
    function _getShareAmount(
        ...
        if (masterAddress != address(0x0)) {
            openPosition.changeCopyAmount(-finalAmount, masterAddress, owner);
