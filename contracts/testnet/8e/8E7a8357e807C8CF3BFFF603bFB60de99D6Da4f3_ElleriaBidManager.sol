...
contract ElleriaBidManager is Ownable, ReentrancyGuard {
  ...
  // X $ELM + 80 USDC. Not possible to increase unless all bids refunded and contract migrated. 
  // If USDC cost is altered (reduced to increase $ELM burn), all remaining bids will be automatically refunded.
  uint256 public usdcCostInWEI = 80000000000000000000; 
  ...
  function SetMintingCycle(uint256 _max) external onlyOwner {
  ...
  function ConsumeBid(uint256 _bidId, uint256 quantity, uint256 _variant) external onlyOwner onlyIfBidValid(_bidId) {
    ...
    emit BidConsumed(bids[_bidId].owner, elmPrice, usdcPrice, quantity, _bidId);

    elleriumAbi.transferFrom(address(this), safeAddr, elmPrice);
    usdcAbi.transferFrom(address(this), safeAddr, usdcPrice);
  ...
  function CreateBid(uint256 _elmAmountInWEI, uint256 quantity) external nonReentrant {
    ...
    emit BidCreated(msg.sender, _elmAmountInWEI * quantity, usdcCostInWEI * quantity, quantity, bidCounter++);
  }

  function RefundBid(bytes memory _signature, uint256 _totalBalanceWorth, uint256 _bidId) external nonReentrant onlyIfBidValid(_bidId) {
    require(msg.sender == bids[_bidId].owner, "AuctionBridge: cannot refund for others");
    require((_totalBalanceWorth - bids[_bidId].elmBalance) == bids[_bidId].usdcBalance, "AuctionBridge: wrong balance");
    require(
      signatureAbi.verify(signerAddr, msg.sender, _totalBalanceWorth, "cancel bid", _bidId, _signature),
  ...
  // Events
  event BidCreated(address indexed owner, uint256 elmValue, uint256 usdcValue, uint256 quantity, uint256 bidId);
  event BidCancelled(address indexed owner, uint256 elmValue, uint256 usdcValue, uint256 quantity, uint256 bidId);
  event BidConsumed(address indexed owner, uint256 elmValue, uint256 usdcValue, uint256 quantity, uint256 bidId);
  event CycleReset(uint256 timestamp, uint256 quantity);
}
