...
pragma solidity 0.8.13;
contract Assetha is ERC721B, Ownable, NonblockingReceiver {

    using Strings for uint256;
    mapping (address => uint256) public preMintCountOfAddress;
    mapping (address => bool) public hasFreeMinted;
    mapping (address => bool) public hasEarlyQuit;
    uint256[] public burntTokens;
    uint256 public totalPreMints;
    uint256 public totalFreeMints;
    uint256 public totalBurnt;
    uint256 public maxTokens;
    uint256 public earlyQuitBalance;
    ...
    uint256 maxBatchSize_ = 20;

    bool public salePaused = false;
    bool public preSalePaused = false;
    bool public earlyQuitPaused = false;
    bool public revealed = true;

    uint256 public mintPrice = 0.03 ether;
    uint256 public preMintPrice = 0.02 ether;
    uint256 public earlyQuitPrice = 0.01 ether;
    uint256 public maxFreeMints = 10;
    uint256 public maxPreMints = 100;
    uint256 public tokensPerWhitelistedAddress = 2;

    uint256 private nextToken = 7001;
    uint256 private startIndex_ = 7001;
    uint256 private endIndex_ = 8000;

    constructor(string memory _initBaseURI, string memory _initNotRevealedURI, address _recipient) ERC721B("Assetha", "AST", maxBatchSize_, startIndex_, endIndex_) {
        ...
        _safeMint(address(_msgSender()), 4);
    }

    modifier checkQuantity(uint256 mintQuantity) {
        require(mintQuantity > 0 && mintQuantity <= maxBatchSize, "Quantity should be lower than 20.");
        require(nextToken + mintQuantity <= (maxTokens - (maxFreeMints - totalFreeMints)), "Exceeded total supply.");
        _;
    }

    function earlyQuit(uint256 tokenId) public {
        require(!earlyQuitPaused, "Early quitting is not enabled.");
        require(ownerOf(tokenId) == _msgSender() || getApproved(tokenId) == _msgSender(), "You are not the owner of this token.");
        // bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        // require(MerkleProof.verify(_merkleProof, whitelistMerkleRoot, leaf), "Invalid proof!");
        require(hasEarlyQuit[_msgSender()] == false, "You have already early quit.");
        require(address(this).balance >= earlyQuitPrice, "Not enough balance in the contract.");
            
        safeTransferFrom(_msgSender(), owner(), tokenId);
        payable(_msgSender()).transfer(earlyQuitPrice);
        hasEarlyQuit[_msgSender()] = true;
    }

    function freeMint() public payable {
        // bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        // require(MerkleProof.verify(_merkleProof, freeMerkleRoot, leaf), "Invalid proof!");
        require(!salePaused, "Sale has not started.");
        require(totalFreeMints < maxFreeMints, "Exceeded free mint supply.");
        require(hasFreeMinted[_msgSender()] == false, "You have already free minted.");

        _safeMint(_msgSender(), 1);
        hasFreeMinted[_msgSender()] = true;
        totalFreeMints++;
        nextToken++;
    }

    function whitelistMint(uint256 mintQuantity) public payable checkQuantity(mintQuantity) {
        // bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        // require(MerkleProof.verify(_merkleProof, whitelistMerkleRoot, leaf), "Invalid proof!");
        require(!preSalePaused, "Presale has not started.");
        // require(msg.value == preMintPrice * mintQuantity, "Invalid price.");
        require(preMintCountOfAddress[_msgSender()] + mintQuantity <= tokensPerWhitelistedAddress, "You can't mint more tokens.");
        require(totalPreMints + mintQuantity <= maxPreMints, "Exceeded total supply of presale.");

        _safeMint(_msgSender(), mintQuantity);

        totalPreMints = totalPreMints + mintQuantity;
        preMintCountOfAddress[_msgSender()] = preMintCountOfAddress[_msgSender()] + mintQuantity;
        nextToken = nextToken + mintQuantity;
    }

    function publicMint(uint256 mintQuantity) public payable checkQuantity(mintQuantity) {
        require(!salePaused, "Sale has not started.");
        // require(msg.value == mintPrice * mintQuantity, "Invalid price.");

        _safeMint(_msgSender(), mintQuantity);
        nextToken = nextToken + mintQuantity;
    }

    function transferToChain(uint16 _chainId, uint tokenId) public payable {
        ...
        require(msg.value >= messageFee, "Please send more gas for the message fees.");
        endpoint.send{value: msg.value}(_chainId, trustedRemoteLookup[_chainId], payload, payable(msg.sender), address(0x0), adapterParams);
    }
    ...
    // ONLY OWNER
    function burnTokens(uint256[] calldata tokenId) public virtual onlyOwner() {
        for (uint256 i; i < tokenId.length; i++) {
            _burn(tokenId[i]);
            burntTokens.push(tokenId[i]);
            totalBurnt++;
        }
    }
    ...
    function setPreMintPrice(uint256 _newPrice) public onlyOwner() {
        preMintPrice = _newPrice;
    }

    function setEarlyQuitPrice(uint256 _newPrice) public onlyOwner() {
        earlyQuitPrice = _newPrice;
    }
    ...
    function togglePreSaleState() public onlyOwner() {
        (preSalePaused == true ? preSalePaused = false : preSalePaused = true);
    }

    function toggleRevealedState() public onlyOwner() {
        (revealed == true ? revealed = false : revealed = true);
    }
    ...
    function withdrawAmount(uint amount) public payable onlyOwner {
