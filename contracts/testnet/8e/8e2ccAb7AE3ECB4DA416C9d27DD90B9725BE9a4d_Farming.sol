...
// File contracts/interfaces/IFarming.sol
...
interface IFarming{
    ...
    function getUserInfo(uint16 _pid, address _user) external view returns (uint256, uint256, uint256, uint256, bool);
}
...
contract Farming is Initializable, AccessControlUpgradeable, PausableUpgradeable, IFarming {
    /**
    ...
    */
    ...
    function setInvestorsUnlockTimestamp(uint256 _investorsUnlockTimestamp) public onlyManager {
        investorsUnlockTimestamp = _investorsUnlockTimestamp;
    }
    ...
    function addInvestorAddress(address _wallet) public onlyManager {
        investorWallets[_wallet]=true;
    }

    function removeInvestorAddress(address _wallet) public onlyManager {
    ...
    // Request withdrawal LP tokens
    function requestWithdrawal(uint16 _pid) public override whenNotPaused {
//
    ...
    function withdraw(uint16 _pid, uint256 _amount) public override whenNotPaused {
        ...
        require(user.requestedWithdrawalTimestamp!=0, "Request withdrawal first");
        require(block.timestamp >= user.requestedWithdrawalTimestamp + 604800, "Withdrawal is not active yet");
        require(block.timestamp <= user.requestedWithdrawalTimestamp + 1209600, "Withdrawal is expired");
        updatePool(_pid);
        user.pendingReward += user.amount * pool.accTokenPerShare / 1e12 - user.rewardDebt;
        if(_amount > 0) {
            user.amount -= _amount;
    ...
    function getUserInfo(uint16 _pid, address _user) external override view returns (uint256, uint256, uint256, uint256, bool) {
        PoolInfo storage pool = poolInfo[_pid];
