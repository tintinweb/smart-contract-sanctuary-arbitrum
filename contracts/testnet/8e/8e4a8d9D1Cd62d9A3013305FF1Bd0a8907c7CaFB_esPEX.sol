...
contract esPEX is ERC20Upgradeable,IesPEX {
    ...
    event ConvertPEX(address _address, uint256 _amount);
    event ConvertEsPEX(address _address, uint256 _amount);
    event UpdateAddress(string name,address _address,bool _allow);

    mapping(address => bool) public  ConvertPEXAddress;
    mapping(address => bool) public ConvertEsPEXAddress;
    modifier onlyConvertPEXAddress() {
        require(ConvertPEXAddress[msg.sender], "Not an allow address");
        _;
    }
    modifier onlyConvertEsPEXAddress() {
        require(ConvertEsPEXAddress[msg.sender], "Not an allow address");
        _;
    }
    ...
    function initialize(
        ...
        minVestAmount = 10 * decimals();
        VestType = [60, 70, 80, 100];
    }

    function setConvertPEXAddress(address _address, bool _allow) public onlyGov {
        require(_address != address(0), "address not 0");
        ConvertPEXAddress[_address] = _allow;
        emit UpdateAddress("setConvertPEXAddress",_address,_allow);
    }

    function setConvertEsPEXAddress(address _address, bool _allow) public onlyGov {
        require(_address != address(0), "address not 0");
        ConvertEsPEXAddress[_address] = _allow;
        emit UpdateAddress("setConvertEsPEXAddress",_address,_allow);
    }

    function convertPEX(address _address, uint256 _amount) external onlyConvertPEXAddress {
        require(balanceOf(_address) >= _amount, "Insufficient vester amount");
        require(PEX.balanceOf(address(this)) >= _amount, "Insufficient contract amount");

        _burn(_address, _amount);
        require(PEX.transfer(msg.sender, _amount));
        emit ConvertPEX(_address,_amount);
    }

    function convertEsPEX(address _address, uint256 _amount) external onlyConvertEsPEXAddress {
        require(PEX.allowance(msg.sender, address(this)) >= _amount, "please approve");
        require(PEX.balanceOf(msg.sender) >= _amount, "Insufficient contract amount");

        _mint(_address, _amount);
        require(PEX.transferFrom(msg.sender, address(this), _amount));
        emit ConvertEsPEX(_address,_amount);
    }
}
