...
contract PresaleSettings is Ownable {
    ...
    constructor(uint256 _creation_fee) {
        ...
        SETTINGS.MIN_PERCENT_PLATFORM = 500; // 50%
    }
    ...
    function getMinimumPercentToPlatform() external view returns (uint256) {
        return SETTINGS.MIN_PERCENT_PLATFORM;
    }
    ...
    function setMinPercentToPlatform(uint256 _minPercentToPlatform) external onlyOwner {
        require(_minPercentToPlatform <= 1000);
        SETTINGS.MIN_PERCENT_PLATFORM = _minPercentToPlatform;
    }
