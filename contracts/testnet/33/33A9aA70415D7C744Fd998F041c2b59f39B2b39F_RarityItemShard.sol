...
import "./IRealmBaseERC1155Token.sol";
import "@openzeppelin/contracts/token/ERC1155/extensions/ERC1155Supply.sol";
import "@openzeppelin/contracts/token/ERC1155/extensions/ERC1155Burnable.sol";

contract RealmBaseERC1155Token is
  IRealmBaseERC1155Token,
  ERC1155Supply,
  ERC1155Burnable,
  ReentrancyGuard,
  ManagerModifier
{
  ...
  // Constructor
  //=======================================
  constructor(string memory _name, address _manager) ERC1155(_name) ManagerModifier(_manager) {
    UNBOUND = false;
  }

  bool public UNBOUND;
  address public ALLOWED_TARGET;
  ...
  function mintBatchFor(
    ...
    require(_ids.length == _amounts.length, "ERC1155: _for/_ids/_amounts length mismatch");

    for (uint i = 0; i < _ids.length; i++) {
      _mintBatch(_for, _ids[i], _amounts[i], "");
    }
  }
  ...
  function burnBatch(uint256[][] memory ids, uint256[][] memory amounts) external nonReentrant {
    require(ids.length == amounts.length, "ERC1155: _ids/_amounts length mismatch");

    for (uint i = 0; i < ids.length; i++) {
      _burnBatch(msg.sender, ids[i], amounts[i]);
    }
  }
  ...
  function _beforeTokenTransfer(
    ...
    require(UNBOUND || to == ALLOWED_TARGET, "ERC1155: Token not unbound");
  }
  ...
  function updateAllowedAddress(address _allowedAddress) external onlyAdmin {
    ALLOWED_TARGET = _allowedAddress;
  }
...
import "../Manager/ManagerModifier.sol";
import "../ERC1155/RealmBaseERC1155Token.sol";

contract RarityItemShard is RealmBaseERC1155Token {
  //=======================================
  // Constructor
  //=======================================
  constructor(address _manager) RealmBaseERC1155Token("Shard", _manager) {}
}
