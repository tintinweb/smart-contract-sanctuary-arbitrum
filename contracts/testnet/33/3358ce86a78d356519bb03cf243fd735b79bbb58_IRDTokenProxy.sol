    ...
    UUPSUpgradeable
{
    ...
    uint public startConvertBlock;
    uint public startClaimBlock;
    uint public totalClaimBlock;
    ...
    modifier convertEnabled() {
        require(startConvertBlock != 0, "Convert disabled");
        _;
    }

    modifier claimEnabled() {
        require(startClaimBlock != 0, "Claim disabled");
        _;
    }
    ...
    function transfer(
        ...
        require(_whitelistTransferAddresses[msg.sender], "NOT_IN_WHITELIST");
    ...
    function startConvert() public onlyOwner {
        require(startConvertBlock == 0, "Cannot update start convert time");
        startConvertBlock = block.number;
    }
    ...
    function stopConvert() public onlyOwner {
        require(startConvertBlock != 0, "Cannot update stop convert time");
        startConvertBlock = 0;
    }
    ...
    function startClaim() public onlyOwner {
        require(startClaimBlock == 0, "Cannot update start claim time");
        startClaimBlock = block.number;
    }
    ...
    function stopClaim() public onlyOwner {
        require(startClaimBlock != 0, "Cannot update stop claim time");
        startClaimBlock = 0;
    }
    ...
    function getClaimable(address claimAddress) public view returns (uint256) {
        ...
        if (convertData[claimAddress].startClaimBlock == 0) {
            return 0;
        }

        if (block.number < startClaimBlock) {
            return 0;
        }
        uint256 tokenPerBlock = totalLockAmount / totalClaimBlock;
        uint progressBlock = block.number -
            convertData[claimAddress].startClaimBlock;
        uint256 fullClaimableAmount;
        if (progressBlock > totalClaimBlock) {
    ...
    function convertRD(uint256 amount) public convertEnabled nonReentrant {
        require(!_blacklistConvertAddresses[msg.sender], "IN_BLACKLIST");
        require(amount > 0, "Amount must be greater than zero");
        require(balanceOf(msg.sender) >= amount, "Balance is not enough");

        if (convertData[msg.sender].startClaimBlock == 0) {
            convertData[msg.sender].lockAmount = amount;
            convertData[msg.sender].claimedAmount = 0;
            convertData[msg.sender].startClaimBlock = block.number;
        } else {
            require(
                convertData[msg.sender].startClaimBlock + totalClaimBlock >=
                    block.number,
                "Cannot convert, please claim first"
            );
            convertData[msg.sender].startClaimBlock =
                (amount *
                    block.number +
                    convertData[msg.sender].lockAmount *
                    convertData[msg.sender].startClaimBlock) /
                (amount + convertData[msg.sender].lockAmount);
            convertData[msg.sender].lockAmount += amount;
        }
    ...
    function claimRD() public claimEnabled nonReentrant {
        require(convertData[msg.sender].startClaimBlock > 0, "Cannot claim, convert first");
        ...
        if (
            convertData[msg.sender].claimedAmount ==
            convertData[msg.sender].lockAmount
        ) {
            convertData[msg.sender].startClaimBlock = 0;
        }
