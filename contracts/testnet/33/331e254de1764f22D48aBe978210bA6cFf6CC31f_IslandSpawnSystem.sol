...
import {SceneObjectParent, SceneObjectParentComponent, ID as SCENE_OBJECT_PARENT_COMPONENT_ID} from "./SceneObjectParentComponent.sol";
import {SceneObjectGameItem, SceneObjectGameItemComponent, ID as SCENE_OBJECT_GAME_ITEM_COMPONENT_ID} from "./SceneObjectGameItemComponent.sol";
...
abstract contract SceneSystem is OwnerSystem, ISceneSystem {
    ...
    function addOrUpdateSceneObjects(
        ...
        for (uint256 i = 0; i < addedObjectParams.length; i++) {
            _addOrUpdateSceneObject(account, sceneEntity, addedObjectParams[i]);
        }
    }
