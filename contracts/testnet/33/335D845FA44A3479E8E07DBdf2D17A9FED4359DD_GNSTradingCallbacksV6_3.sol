...
contract GNSTradingCallbacksV6_3 is Initializable {
    ...
    function openTradeMarketCallback(
        ...
        uint maxSlippage = o.wantedPrice * o.slippageP / 100 / PRECISION;   //计算最大价格滑动量；

        //判断新的价格是否能够开仓；
        if(isPaused || a.price == 0
        || (t.buy ?
            t.openPrice > o.wantedPrice + maxSlippage :
            t.openPrice < o.wantedPrice - maxSlippage)  //最大滑点都无法满足价格；
        || (t.tp > 0 && (t.buy ?
            t.openPrice >= t.tp :
            t.openPrice <= t.tp))
        || (t.sl > 0 && (t.buy ?
            t.openPrice <= t.sl :
            t.openPrice >= t.sl))   //最新的开仓价格与tp,sl不相匹配；
        || !withinExposureLimits(t.pairIndex, t.buy, t.positionSizeDai, t.leverage) //防止即刻爆仓？
        || priceImpactP * t.leverage > pairInfos.maxNegativePnlOnOpenP())   //滑点*杠杆倍数要少于一个数？
        {  //市价开仓失败；
    ...
    function closeTradeMarketCallback(
        ...
        if(t.leverage > 0){
            ...
            v.tokenPriceDai = aggregator.tokenPriceDai();   //获取dai价格？

            if(a.price == 0){   //价格为0是啥意思？
...
interface StorageInterfaceV5{
    ...
    struct PendingMarketOrder{
        ...
        uint slippageP;             // PRECISION (%)    //允许的最大滑点？
        uint spreadReductionP;
        uint tokenId;               // index in supportedTokens
    }
