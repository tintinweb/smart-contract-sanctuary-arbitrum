...
// File contracts/games/CoinFlip.sol
...
contract CoinFlip is Game {
    ...
    /// @param bet The Bet struct information.
    /// @param diceBet The Coin Flip bet struct information.
    /// @dev Used to package bet information for the front-end.
    struct FullCoinFlipBet {
        Bet bet;
        CoinFlipBet coinFlipBet;
    }

    /// @notice Coin Flip bet information struct.
    ...
    mapping(uint256 => CoinFlipBet) public coinFlipBets;
    ...
    function fulfillRandomWords(uint256 id, uint256[] memory randomWords) internal override {
        ...
        CoinFlipBet storage coinFlipBet = coinFlipBets[id];
        Bet storage bet = bets[id];
        ...
        coinFlipBet.rolled = rolledCoinSide;
        uint256 payout = _resolveBet(bet, rolledCoinSide == coinFlipBet.face ? _getPayout(bet.amount) : 0);

        emit Roll(bet.id, bet.user, bet.token, bet.amount, coinFlipBet.face, rolledCoinSide, payout);
    ...
    /// @param user Address of the gamer.
    /// @param dataLength The amount of bets to return.
    /// @return A list of Coin Flip bet.
    function getLastUserBets(address user, uint256 dataLength) external view returns (FullCoinFlipBet[] memory) {
        Bet[] memory lastBets = _getLastUserBets(user, dataLength);
        FullCoinFlipBet[] memory lastCoinFlipBets = new FullCoinFlipBet[](lastBets.length);
        for (uint256 i; i < lastBets.length; i++) {
            lastCoinFlipBets[i] = FullCoinFlipBet(lastBets[i], coinFlipBets[lastBets[i].id]);
        }
        return lastCoinFlipBets;
    }
}