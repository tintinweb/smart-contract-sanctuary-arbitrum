...
contract NftMinning is OwnableUpgradeable, IERC721ReceiverUpgradeable {
    ...
    event Deposit(
        address indexed user,
        uint indexed unionsId,
        uint indexed tokenId
    );
    event Claim(address indexed user, uint indexed amount);
    event Withdraw(address indexed user, uint indexed tokenId);

    event ClaimTax(address indexed user, uint indexed amount);
    event ClaimGeTax(address indexed user, uint indexed amount);
    event ClaimDao(address indexed user, uint indexed amount);
    ...
    function init() public initializer {
        ...
        unions = IMSW(0x01b6923769a39f9aEF35c0880396421Cb4668e22);
    ...
    function setPool(uint dayOut_) public onlyOwner {
        pool.dayOut = dayOut_;
    }
    ...
    function upgradeUnion(uint uid_, uint toCardId_) public returns (bool) {
        // require(msg.sender == unionsInfo[uid_].owner, "not owner!");

        require(unionsInfo[uid_].lv < 3, "already max lv!");
        uint diff;
        {
    ...
    function upgradeProcess(uint uid_) internal {
        (, , uint lv_, , ) = unions.cardInfoes(unions.cardIdMap(uid_));
        ...
        // update dao info
        daoDebt.totalDao += dao[lv_ - 1] - dao[unionsInfo[uid_].lv - 1];
        daoDebt.debt = _debt;
        ...
        unionsInfo[uid_].lv = lv_;
    }

    function joinUnion(uint unionsId_) public {
    ...
    function deposit(uint unionsId_, uint kunId_) public {
        ...
        uint _power = baseNftPower[getKunId(kunId_)];
        ...
        kunIdList[msg.sender].push(kunId_);

        // kun
        kun.safeTransferFrom(msg.sender, address(this), kunId_);
        emit Deposit(msg.sender, unionsId_, kunId_);
    }
    ...
    function withdraw(uint kunID_) public {
    ...
    function claimGeTax() public {
        ...
        if (_tax != 0) {
            ...
            IERC20(MSW).transfer(msg.sender, _tax);
            emit ClaimGeTax(msg.sender, _tax);
        }
    }

    // dao logic -----------------------------------------------------------------------------------------------
    function updateDaoDebt() public view returns (uint, uint) {
        uint thisDay = (block.timestamp - pool.startTime) / 1 days;

        if (thisDay != daoDebt.updateDays) {
            uint amount = pool.dayOut;
            uint _debt = daoDebt.totalDao > 0 ? amount / daoDebt.totalDao : 0;
            return (_debt, thisDay);
        } else {
            return (daoDebt.debt, daoDebt.updateDays);
        }
    }

    function updateDaoBonus(uint uid_) internal view returns (uint _bonus) {
    ...
    function claimDaoBonus(uint uid_) public {
        ...
        if (_bonus > 0) {
            ...
            emit ClaimDao(msg.sender, _bonus);
        }
    }
