...
contract USDW is YieldToken, IUSDW {
    ...
    function addVault(address _vaultAddress) external override onlyGovernance {
...
contract YieldToken is ERC20, AccessControlBase, IYieldToken {
    ...
    function withdrawToken(address _token, address _account, uint256 _amount) external onlyGovernance {
        SafeTransferLib.safeTransfer(ERC20(_token), _account, _amount);
    }

    function setInWhitelistMode(bool _inWhitelistMode) external onlyGovernance {
        inWhitelistMode = _inWhitelistMode;
    }

    function setWhitelistedHandler(address _handler, bool _isWhitelisted) external onlyGovernance {
