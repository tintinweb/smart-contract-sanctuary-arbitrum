...
/// @custom:experimental This is an experimental contract version 1.0 on Arbitrum Testnet
contract bettingMarket is Ownable {
    ...
    function makeBets(uint256 amount, address player, bytes32[] calldata proof) external payable {
        ...
        uint256 timeMultiplier = (betTimeStamp - seasonStartTimestamp) * 3 / MILISECONDS_PER_WEEK;
        // uint256 wagerMultiplier = 3 - 3 * numBetter / numPlayer;
        uint256 newWeight = Math.min(timeMultiplier, 1);
        ...
        for (uint256 i = 0; i < betsTracking[player].length; i++) {
            if (betsTracking[player][i].bettor == msg.sender) {
                ...
                uint256 totalWeightedBet = betsTracking[player][i].weight * betsTracking[player][i].wager + newWeight * amount;
                betsTracking[player][i].weight = totalWeightedBet / (betsTracking[player][i].wager + amount);
    ...
    function startSeason(uint256 _seasonEndTimestamp) external onlyOwner {
        ...
        console.log("season start timestamp: %s", block.timestamp);
    ...
    function distributeRewards(address payable[] calldata winnerPlayers) external onlyOwner {
        ...
        // address [] memory winnerBettors = new address[](betsTracking[winnerPlayers[0]].length 
        //     + betsTracking[winnerPlayers[1]].length + betsTracking[winnerPlayers[2]].length);
        // address [] memory winnerPayouts = new address[](betsTracking[winnerPlayers[0]].length 
        //     + betsTracking[winnerPlayers[1]].length + betsTracking[winnerPlayers[2]].length);
        // uint256 bettorCount = 0;
        // uint256 bettorPayout = 0;

        for (uint256 i = 0; i < winnerPlayers.length; i++) {
            for (uint256 j = 0; j < betsTracking[winnerPlayers[i]].length; j++) {
                totalBetsonWinners += betsTracking[winnerPlayers[i]][j].wager;
                address _bettor = betsTracking[winnerPlayers[i]][j].bettor;
                winnerBettorsWeightedSum += betsTracking[winnerPlayers[i]][j].wager * betsTracking[winnerPlayers[i]][j].weight ;
                /// transfer the original bet amount to each successful bettor
                (bool sent, ) = _bettor.call{value:betsTracking[winnerPlayers[i]][j].wager}("");
                require(sent, "Failed to send Ether");
                // totalPayout[_bettor] += betsTracking[winnerPlayers[i]][j].wager;
            }
        }
        uint256 totalBetsonLosers = totalWager - totalBetsonWinners;
        /// distribute totalBetsonLosers to the successful bettors based on their weighted bets
        for (uint256 i = 0; i < winnerPlayers.length; i++) {
            for (uint256 j = 0; j < betsTracking[winnerPlayers[i]].length; j++) {
                address _bettor = betsTracking[winnerPlayers[i]][j].bettor;
                uint256 claimofPrizePool = betsTracking[winnerPlayers[i]][j].wager * betsTracking[winnerPlayers[i]][j].weight / winnerBettorsWeightedSum;
                /// transfer the proportional claim of the prize pool to each successful bettor
                (bool sent, ) = _bettor.call{value:claimofPrizePool}("");
                require(sent, "Failed to send Ether");
                // totalPayout[_bettor] += betsTracking[winnerPlayers[i]][j].wager;
            }
        }
