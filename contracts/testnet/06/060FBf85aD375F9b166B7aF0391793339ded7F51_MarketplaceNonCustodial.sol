    ...
    IMarketplaceNonCustodial
{
    ...
    function cancelListing(uint256 _orderId, uint256 _amountToCancel)
        external
        override
    {
        if (_orderId > orderCtr) {
        ...
        if (_amountToCancel == 0 || _amountToCancel > _order.amount) {
            revert InvalidAmountInput(_amountToCancel);
        }
        uint256 _newAmount = _order.amount - _amountToCancel;
...
interface IMarketplaceNonCustodial {
    ...
    function cancelListing(uint256 _orderId, uint256 _amountToCancel) external;
