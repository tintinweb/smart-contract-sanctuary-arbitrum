...
contract Bartender is IBartender, Ownable, Constant {
    ...
    uint96 private feeBPS = 0; // 500 / 0.5%
    uint256 private _totalSupply;
    uint256 private _currentId;

    // mapping of id to SAKE vault
    mapping(uint256 => address) public sakeVault;
    mapping(uint256 => UpdatedDebtRatio) public updatedDebtRatio;
    mapping(address => mapping(uint256 => UserDepositInfo)) public userDepositInfo;
    ...
    function settingManagerForVELA(address mintAndBurn, address stakingVault, address _vlp) external onlyOwner {
        velaMintBurnVault = mintAndBurn;
    ...
    function createSake(address _velaMintBurnVault, address _velaStakingVault, address _vlp) external onlyKeeper {
        ...
        // create new SAKE
        Sake newSake = new Sake(address(usdcToken), water, address(this), _velaMintBurnVault, _velaStakingVault, _vlp);
        sakeVault[lCurrentId] = address(newSake);
        // transfer _token into the newly created SAKE
        usdcToken.safeTransfer(address(newSake), _amount);
    ...
    function withdraw(uint256 _amount, uint256 id, address _receiver) external {
        ...
        if (sakeVaultInfo[id].startTime + COOLDOWN_PERIOD < block.timestamp) revert ThrowLockTimeOn();
    ...
    // #todo since console log is not working
    uint256 public test;
    ...
    function _transferAndRepayLoan(uint256 _withdrawnAmountinUSDC, address _receiver, uint256 share) private {
        ...
        usdcToken.safeTransfer(_receiver, share);
    }
    ...
    function takeFees(uint256 _amount) private returns (uint256 amount) {
        ...
        // feeRecipient share
        usdcToken.safeTransfer(feeRecipient, _protocolFee);
    }
    ...
    function getVLPPrice() private view returns (uint256) {
    ...
    function _updateShares(
        ...
        for (uint256 i = 0; i < totalUsers; ) {
            ...
            address user = sakeVaultInfo[id].users[i];
            if (user == sender) {
                uint256 subAmountFromMaxWithdrawal = beforeWithdrawal(id,previousValue, previousDebt, user) - withdrawableAmount;
    ...
    function updateDebtAndValueAmount(
        uint256 id,
        bool state
    ) public view returns (uint256 newDebt, uint256 Value, uint256 dvtRatio) {
    ...
    function maxWithdraw(uint256 id, uint256 currentShares) public view returns (uint256) {
        uint256 updatedDebt;
        uint256 value;
    ...
    // preview withdrawal
    function previewWithdraw(uint256 id, address user) public view returns (uint256) {
        uint256 shares;
        if (updatedDebtRatio[id].newValue == 0) {
 ...
 */
contract Sake is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    ...
    modifier onlyBartender() {
        if (address(bartender) != _msgSender())
    ...
    function executeMintAndStake() external nonReentrant onlyBartender returns (bool status, uint256 totalVLP) {
    ...
    function withdraw(
    ...
    ) external nonReentrant onlyBartender returns (bool status, uint256 usdcAmount) {
        vlp.approve(address(velaStakingVault), amountToWithdrawInVLP);
        // @todo add slippage tolerance
        // @notice there is no need for reentrancy guard Bartender will handle that
        // @todo the decision made from REFERENCE: 01 is what will determine the amount to withdraw
        ...
        // no convenient function to get the usdc return, will use balanceOf for now as every call is suppose to withdraw all USDC in the contract
