...
contract BidFomo is Caller, ReentrancyGuard {
    ...
    uint256 public initialPrice = 1 ** 6; // 10**6 usd
    ...
    function bid(string memory message) public {
        ...
        uint256 curBiddingAmount = getCurrentBidAmount();
        require(
    ...
    // USD, need / 1e8
    function getBOBOPrice() public view returns (uint256 usd) {
        ...
        usd = (getETHPrice() * amountOut) / 10 ** 18;
    }

    function getCurrentBidAmount() public view returns (uint256 amount) {
        uint256 curBiddingPrice = (pool.bidding * (1000 + bidPriceGrowthRate)) /
            1000;
        amount = (curBiddingPrice * (10 ** 8) * (10 ** 6)) / getBOBOPrice();
