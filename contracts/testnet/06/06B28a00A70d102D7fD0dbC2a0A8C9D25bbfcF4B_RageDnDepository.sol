    ...
    RageDnDepositoryStorage
{
    ...
    function depositInsurance(uint256 amount, address from) external onlyOwner {
        ...
        bool success = IERC20(insuranceToken).transferFrom(from, address(this), amount);
        if (!success) {
            revert ();
        }
    ...
    function withdrawInsurance(uint256 amount, address receiver)
        external
        onlyOwner
        nonReentrant
    {
    ...
    function getUnrealizedPnl() public view returns (int256) {
        return
            int256(getDepositoryAssets()) -
            int256(netAssetDeposits + insuranceDeposits);
    }
