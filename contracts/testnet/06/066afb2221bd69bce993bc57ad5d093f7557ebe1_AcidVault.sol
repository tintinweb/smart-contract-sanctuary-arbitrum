...
import "./interfaces/ICamelotDex.sol";
...
contract AcidVault is ERC4626, Ownable {
    ...
    uint256 private minRewardToSwap = 1000000000000000;
    ...
    constructor(
        ...
        address _camelotDex,
        address _wETH
    ) ERC4626(IERC20(_asset)) ERC20("Staked sAcid", "ssAcid") {
    ...
    function depositWithRewards(uint256 amount, address to, uint256 minAmountRewardRate) public returns (uint256) {
        require(amount > 0, "AcidVault: deposit amount must be greater than 0");
        require(minAmountRewardRate > 0, "AcidVault: deposit reward amount expected  must be greater than 0");

        _collectRewards(minAmountRewardRate);
    ...
    function redeemWithRewards(uint256 shares, address receiver, address owner, uint256 minAmountRewardRate)
        public
        returns (uint256)
    {
        require(shares > 0, "AcidVault: redeem shares must be greater than 0");
        require(minAmountRewardRate > 0, "AcidVault: redeem reward amount expected must be greater than 0");
    ...
    function withdrawWithRewards(uint256 assets, address receiver, address owner, uint256 minAmountRewardRate)
        public
        returns (uint256)
    {
        require(assets > 0, "AcidVault: withdraw assets must be greater than 0");
        require(minAmountRewardRate > 0, "AcidVault: withdraw reward amount expected must be greater than 0");
    ...
    function totalAssets() public view override returns (uint256) {
        ...
        if (earned > 0) {
            uint256[] memory rewards = camelotDex.getAmountsOut(earned, path);
    ...
    function collectRewards(uint256 minAmountRewardRate) public {
        _collectRewards(minAmountRewardRate);
        _depositRewards();
    }
    ...
    function _collectRewards(uint256 minAmountRewardRate) internal {
        ...
        if (reward > minRewardToSwap) {
            ...
            camelotDex.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                reward, minAmountRewardRate * reward, path, address(this), address(0), block.timestamp
            );

            if (treasury != address(0)) {
                uint256 acidBalanceAfter = IERC20(asset()).balanceOf(address(this));
                uint256 acidEarned = acidBalanceAfter - acidBalanceBefore;
                uint256 fee = acidEarned * treasuryFeePercentage / 100;
