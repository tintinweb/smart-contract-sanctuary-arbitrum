...
contract BaseURI is ERC721Enumerable {
    using Strings for uint256;

    string      public _name;
    string      public _symbol;
    string      public __baseURI;
    string      public _postFix;
    uint256     public _offset;
    uint256     public _max;
    bool        public _initialised;
    bool        public _baseZero;

     address     public _minter;

    modifier onlyMinter() {
        require(msg.sender == _minter,"unauthorised");
        _;
    }

    constructor() ERC721("GOLDEN","BOY") {
        _initialised = true;
    }
    ...
    function init(TokenStorageData calldata data) external {
        require(!_initialised,"BaseURI : contract already initialised");
        _minter = msg.sender;
        _name = data.name;
        _symbol = data.symbol;
        __baseURI = data.baseURI;
        _postFix = data.postFix;
        _offset  = data.offset;
        _max     = data.max;
        _baseZero = data.baseZero;
        _initialised = true;
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        uint256 newTokenId = tokenId + _offset;
        if (_baseZero) {
            newTokenId = newTokenId % _max;
        } else {
            if (newTokenId > _max) {
                newTokenId -= _max;
                newTokenId++;
    ...
    function mint(address owner, uint256 tokenId) external onlyMinter {
        _mint(owner,tokenId);
    }
