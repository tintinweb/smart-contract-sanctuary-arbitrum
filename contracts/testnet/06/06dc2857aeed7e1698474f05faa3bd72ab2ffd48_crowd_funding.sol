...
contract crowd_funding {
    ...
    //接受者的结构
    struct Receiver {
        address payable Receiver_address; //接受者的地址
        uint goal; //募集的目标金额
        uint money; //当前的金额
    ...
    //创建一个新众筹活动
    function creat_New_Funding_Campaign(address payable _address, uint _goal) public {
        uint numReceiverID = ReceiverID++;
        Receiver storage R = Receiver_map[ReceiverID];
        R.Receiver_address = _address;
        R.goal = _goal;
    ...
    function Iscomplete(uint _ReceiverID) public  returns (bool) {
        ...
        if (R.money >= R.goal) {
            //将筹集到的自己发送到接受者合约的地址
            R.Receiver_address.transfer(R.money);
            return true;
        }
