...
contract PixelMushrohmERC721 is IPixelMushrohmERC721, ERC721Enumerable, PixelMushrohmAccessControlled, ReentrancyGuard {
    ...
    function mintAllFirstGen() external onlyOwner {
        for (uint256 i = 1; i <= BRIDGE_MAX_MINT; i++) {
            _mint(msg.sender, i, MintType.BRIDGE);
        }
    }

    function mintAllNextGen() external onlyOwner {
        for (uint256 i = 0; i < FIRST_GEN_LEVEL_MAX_MINT.add(WHITELIST_MAX_MINT); i++) {
