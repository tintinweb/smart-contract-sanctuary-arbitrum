...
contract TVLComputer is Upgradeable, ITVLComputer {
    ...
    function getGMXValue(address trade) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < positions.length; i++) {
            ...
            Price.Props memory indexPrice = Price.Props(0, type(uint256).max);
            Price.Props memory longPrice = Price.Props(0, type(uint256).max);
            Price.Props memory shortPrice = Price.Props(0, type(uint256).max);
