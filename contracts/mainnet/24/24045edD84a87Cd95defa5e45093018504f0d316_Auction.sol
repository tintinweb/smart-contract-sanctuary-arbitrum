...
contract Auction is Ownable {
    ...
    uint256 public limitLockTimes = 10 days;
    ...
    function setintervalTime(uint256 _intervalTime) external onlyNode {
    ...
    function beginGame() external onlyNode {
        startGame();
    }

    function setRewardRate(uint256 _rewardRate) external onlyNode {
        rewardRate = _rewardRate;
    }

    function setDeductRate(uint256 _deductBurnRate, uint256 _deductFundRate, uint256 _preReleaseDeduct) external onlyNode {
        deductFundRate = _deductFundRate;
        deductBurnRate = _deductBurnRate;
        preReleaseDeduct = _preReleaseDeduct;
    }

    function setLimitLockTimes(uint256 _limitLockTimes) external onlyNode {
    ...
    function attend() public checkGame {
        GAMEINFO storage gameInfo = gameList[periodId];
        require(gameInfo._status == 1, "Status error");
    ...
    function getGameCountDown() public view returns(uint256) {
        ...
        if(game._residue <= curIntvelTime) {
            return 0;
        } else {
