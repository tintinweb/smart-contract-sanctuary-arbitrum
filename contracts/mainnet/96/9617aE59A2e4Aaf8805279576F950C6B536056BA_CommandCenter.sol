...
contract CommandCenter is Initializable, AccessControlEnumerableUpgradeable {
    ...
    event NewRate(uint256 _newRate);
    event NewGrowth(uint256 _newGrowth);
    event NewIncentivesSize(uint256 _newSize);
    ...
    function stale(uint256 _id, bool _status) external onlyRole(OPERATOR_ROLE) {
        if (!voter.partnerNFT(_id)) {
            voter.designatePartnerNFT(_id, true);
        }
    ...
    function setFeeProtocol(
        ...
        factoryCL.setFeeProtocol(_feeProtocol);
    }
    ...
    function setPoolFeeProtocol(
        ...
        factoryCL.setPoolFeeProtocol(pool, _feeProtocol);
    }
    ...
    function whitelistGaugeRewards(
        ...
        address[] memory _gauges = getGaugeArray(_pools);
    ...
    ///@dev distribute as RAM voting incentives for a pool
    function incentivizeVotesNative(
        ...
        for (uint256 i = 0; i < _pools.length; ++i) {
            ...
            _feeDist.bribe(emissionsToken, _emissionsTokenAmounts[i]);
        }
        emit VoteIncentives(_pools, _emissionsTokenAmounts);
    }

    ///@dev distribute as RAM LP incentives for a pool
    ...
    ///@dev operator gated call to get the underlying RAM
    ...
    function updateIncentivesSize(
        ...
        emit NewIncentivesSize(_newSize);
    }
    ...
    function getRatioByPair(address _pair) external view returns (uint256) {
        return voter.gaugeXRamRatio(getGauge(_pair));
    }
