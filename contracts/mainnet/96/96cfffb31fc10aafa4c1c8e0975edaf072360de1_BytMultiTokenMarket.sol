...
contract BytMultiTokenMarket is Ownable, ReentrancyGuard {
    ...
    address[] public paymentTokenArray; //Allows unlimited payment token types if ever necessary

    uint256 public fee;
    address payable public feeRecipient;

    bool public marketActive;
    bool public updateListingsActive;

    mapping(address => uint256) public royaltyFee; //Set by collection owner and paid to collection owner manually after validation
    ...
    constructor() {
        setFee(200);
        setFeeRecipient(payable(0xD14fD22c819F7948b76387e94Eaec9304f426e4B));
        setTransactionDictionary(0x1e5847150d8843Eb783B23673360232Cf4E8A733);
        addAllowedPaymentToken(address(0)); 
    }
    ...
    function createListing(
        ...
        uint256 _paymentTokenArrayIndex
    ) external {
        ...
        listings[_collectionAddress][_tokenId][_msgSender()] = Listing(
            ...
            _expirationDate,
            block.number,
            paymentTokenArray[_paymentTokenArrayIndex]
        );
    ...
    function updateListing(
        ...
        require(updateListingsActive, "Cannot update listings right now");
        require(_newQuantity > 0, "Listing amount must be > 0");

        Listing storage listedItem = listings[_collectionAddress][_tokenId][_msgSender()];
        if (IERC165(_collectionAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            IERC721 nft = IERC721(_collectionAddress);
            require(_newQuantity == 1, "ERC721 is not fungible");
            require(nft.ownerOf(_tokenId) == _msgSender(), "not owning item");
        } else if (IERC165(_collectionAddress).supportsInterface(INTERFACE_ID_ERC1155)) {
            IERC1155 nft = IERC1155(_collectionAddress);
        ...
        listedItem.listingBlockNumber = block.number;
        listedItem.paymentToken = paymentTokenArray[_paymentTokenArrayIndex];
        ...
        payable
    {
        ...
        require(listedItem.listingBlockNumber < block.number, "Cannot buy in same block the listing is set");
        ...
        BytPriceDictionary(transactionDictionary).reportSale(_collectionAddress, _tokenId, listedItem.paymentToken, listedItem.pricePerItem);
        _buyItem(listedItem.pricePerItem, _quantity, _collectionAddress, _owner, listedItem.paymentToken);
    }
    ...
    function _buyItem(
        ...
        uint256 feeAmount = totalPrice * (fee + royaltyFee[_collectionAddress]) / BASIS_POINTS;
        if(_paymentToken == address(0))
        {
            require(msg.value == totalPrice, "Incorrect message value");
            feeRecipient.transfer(feeAmount);
            payable(_owner).transfer(totalPrice - feeAmount);
        }
        else
        {
            IERC20(_paymentToken).safeTransferFrom(_msgSender(), feeRecipient, feeAmount);
            IERC20(_paymentToken).safeTransferFrom(_msgSender(), _owner, totalPrice - feeAmount);
        }
    }
    ...
    function setUpdateListingsActive() external onlyOwner {
        updateListingsActive = !updateListingsActive;
    }
    ...
    function withdrawERC20Funds(address receiver, address tokenAddress, uint256 amount) external onlyOwner {
        IERC20(tokenAddress).safeTransferFrom(address(this), receiver, amount);
    }
    ...
    function addAllowedPaymentToken(address _paymentToken) public onlyOwner {
        paymentTokenArray.push(_paymentToken);
        emit AddPaymentToken(_paymentToken);
    }

    function changeAllowedPaymentToken(uint256 index, address _paymentToken) public onlyOwner {
        require(index < paymentTokenArray.length, "Index invalid");
        paymentTokenArray[index] = _paymentToken;
    }

    function setRoyaltyFee(uint256 _fee, address _nftCollection) external {
        require(_fee < (BASIS_POINTS / 2), "Royalty cannot be >= 50%");
        if(_msgSender() != owner())
        {
            require(_msgSender() == Ownable(_nftCollection).owner());
        }

        royaltyFee[_nftCollection] = _fee;
    }
