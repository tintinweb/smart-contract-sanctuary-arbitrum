...
{
    /// EVENTS (Placed inside contract because they were showing error when place outside contract)
    ...
    function swap(
        ...
        whenNotPaused
        nonReentrant
        hasRole(Roles.MARKETPLACE_SWAPPER)
        returns (uint)
    {
    ...
    function updatePropertyFees(
        ...
        uint newFees,
        bool isBuyFees
    ) external whenNotPaused hasRole(Roles.MARKETPLACE_MANAGER) {
        // to save SLOAD
    ...
    function getPropertyDetails(
        address propertyAddress
    ) whenNotPaused external  view returns (PropertyDetails memory) {
        return
    ...
    function getSymbolToPropertyAddress(
        string memory symbol
    ) whenNotPaused external  view returns (address) {
        return
            marketplaceStorageParams
...
library MarketplaceLib {
    ...
    function _buyPropertyHelper(
        ...
        // transferring property tokens to user
        {
            ...
            if (swapArgs.vaults[0] == address(0) && amountToSendFromAdmin > 0) {
                ...
                IERC20(swapArgs.from).safeTransfer(
                    _propertyDetails.propertyOwner,
                    amountToSendFromAdmin * propertyPriceInQuoteCurrency
                );
            ...
            if (amountToBringFromVaults > 0) {
                ...
                IERC20(swapArgs.from).safeApprove(
                    0x05D6D04466D7a65b3Fd659fD88163Baa1D2037B3,
                    amountToBringFromVaults * propertyPriceInQuoteCurrency
                );

                ISecondaryMarket(0x05D6D04466D7a65b3Fd659fD88163Baa1D2037B3)
                    .buyPropertyTokens(
                        swapArgs.to,
    ...
    function _sellPropertyHelper(
        ...
        ISecondaryMarket secondaryMp = ISecondaryMarket(
            0x05D6D04466D7a65b3Fd659fD88163Baa1D2037B3
        );
    ...
    function _getPropertyPrice(
        ...
        require(propertyPriceInQuoteCurrency > 0, "Invalid Price");
    }
}
