...
contract Bet247L2 is Pausable {
    using SafeMath for uint256;
    using Address for address;
    uint256 public minAmout = 0.01 ether;
    uint256 public devFee = 250; // 5% all bet amount
    ...
    event ReopenMatch(string key, uint256 startTime);
    ...
    struct Bet {
        ...
        uint256 amount;
        uint256 numberOfTicket; // number of tickets bettor brought
        bool withdraw;
    }
    ...
    struct MatchBetInfo {
        ...
        uint256 amount;
        uint256 totalBettor;
        uint256 totalPlayer1Ticket;
        uint256 totalDrawTicket;
        uint256 totalPlayer2Ticket;
    }
    ...
    function betMatch(
        ...
        if (prediction == MATCH_RESULT.PLAYER1_WIN) {
            require(
                msg.value >= numberOfTicket.mul(matches[key].player1WinAmount),
                "insufficient funds"
            );
        }
        if (prediction == MATCH_RESULT.PLAYER2_WIN) {
            require(
                msg.value >= numberOfTicket.mul(matches[key].player2WinAmount),
                "insufficient funds"
            );
        }
        if (prediction == MATCH_RESULT.DRAW) {
            require(
                msg.value >= numberOfTicket.mul(matches[key].drawAmount),
                "insufficient funds"
            );
        }
        require(
        ...
        if (matches[key].bettors[msg.sender][prediction].numberOfTicket > 0) {
            matches[key].bettors[msg.sender][prediction].amount = matches[key]
                .bettors[msg.sender][prediction]
                .amount
                .add(msg.value);
            matches[key]
            .bettors[msg.sender][prediction].numberOfTicket = matches[key]
                .bettors[msg.sender][prediction]
                .numberOfTicket
                .add(numberOfTicket);
        } else {
    ...
    function withdrawMatch(string calldata key) external returns (uint256) {
        ...
        require(
            matches[key].bettors[msg.sender][matches[key].prediction].withdraw ==
                false,
            "withdrawn"
        );

        require(
            matches[key].startTime.add(60 * 24 * 3600) > block.timestamp,
            "expired"
        );

        uint256 payAmount = matches[key]
            .bettors[msg.sender][matches[key].prediction]
            .numberOfTicket
            .mul(matches[key].amount.div(matches[key].totalWinningTicket));

        require(matches[key].remainAmount >= payAmount, "fund runs out");

        matches[key].bettors[msg.sender][matches[key].prediction].withdraw = true;
        matches[key].remainAmount = matches[key].remainAmount.sub(payAmount);
        ...
        requireEditor
    {
        ...
        uint256 totalWinningTicket = getTotalTicketByPrediction(
            key,
            matches[key].prediction
        );
        uint256 devFeeAmount = devFee.mul(amount.div(10000));
        ...
        requireEditor
    {
        ...
        require(
            matches[key].startTime.add(30 * 24 * 3600) < block.timestamp,
            "_"
        );
    ...
    function getWinningAmount(
        ...
        uint256 winning = numberOfTicket.mul(amount.div(totalWinningTicket));
        return winning;
    }
}