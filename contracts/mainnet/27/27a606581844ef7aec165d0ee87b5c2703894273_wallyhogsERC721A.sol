...
// File: contracts/Test contract/kitty/wallyhogsERC721A.sol
...
contract wallyhogsERC721A is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer   {

  string public baseURI;
  string public notRevealedUri;
  uint256 public cost = 0.005 ether;
  uint256 public wlcost = 0.0005 ether;
  uint256 public maxSupply = 1000;
  uint256 public WlSupply = 20;
  ...
  function mint(uint256 tokens) public payable nonReentrant {
    require(!paused, "WHG: oops! contract is paused");
    require(!preSale, "WHG: Sale hasn't started yet");
    require(tokens <= MaxperWallet, "WHG: max mint amount per tx exceeded");
    require(totalSupply() + tokens <= maxSupply, "WHG: We sold out");
    require(PublicMintofUser[_msgSenderERC721A()] + tokens <= MaxperWallet, "WHG: Max NFT Per Wallet exceeded");
    require(msg.value >= cost * tokens, "WHG: insufficient funds");
...
/// @dev presale mint for whitelisted
    ...
    require(!paused, "WHG: oops! contract is paused");
    require(preSale, "WHG: Presale hasn't started yet");
    require(MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "WHG: You are not Whitelisted");
    require(WhitelistedMintofUser[_msgSenderERC721A()] + tokens <= MaxperWalletWl, "WHG: Max NFT Per Wallet exceeded");
    require(tokens <= MaxperWalletWl, "WHG: max mint per Tx exceeded");
    require(totalSupply() + tokens <= WlSupply, "WHG: Whitelist MaxSupply exceeded");
    require(msg.value >= wlcost * tokens, "WHG: insufficient funds");
