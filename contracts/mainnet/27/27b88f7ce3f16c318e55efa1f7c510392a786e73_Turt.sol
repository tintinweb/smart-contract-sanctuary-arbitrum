...
contract Turt is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_AIRDROP_WALLET_DISTRIBUTION = 600 ether;

    // Have the rewards been distributed to the pools
    bool public rewardPoolDistributed = false;

    address public turtOracle;
    ...
    /**
     * @notice Constructs the TURT ERC-20 contract.
     */
    constructor() public ERC20("DefiTurtle", "TURT") {
        // Mints 1 TURT to contract creator for initial pool setup
        _mint(msg.sender, 1 ether);   
        IUniswapV2Router _dexRouter = IUniswapV2Router(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
    ...
    function _getTurtPrice() internal view returns (uint256 _turtPrice) {
        try IOracle(turtOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Turt: failed to fetch TURT price from Oracle");
        }
    }

    function setTurtOracle(address _turtOracle) public onlyOperator {
        require(_turtOracle != address(0), "oracle address cannot be 0 address");
        turtOracle = _turtOracle;
    }

    /**
     * @notice Operator mints TURT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of TURT to mint to
