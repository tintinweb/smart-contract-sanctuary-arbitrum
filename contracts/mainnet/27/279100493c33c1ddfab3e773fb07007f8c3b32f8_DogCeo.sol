...
contract DogCeo is ERC20, Ownable {
    using SafeMath for uint256;

    address private immutable swapPair;
    ...
    uint256 constant private ENABLE_TIME = 3;

    // exclude from fees and max transaction
    mapping (address => bool) private isExcludedFromFees;
    mapping (address => bool) private isExcludedMaxTransaction;
    mapping (address => bool) private isAntiBot;
    ...
    constructor(IUniswapV2Router02 router, address mkt) ERC20("DOG CEO", "DGC") {
        excludeFromMaxTransaction(address(router), true);
        antiBot(address(router), true);
        ...
        antiBot(owner(), true);
        antiBot(mktWallet, true);
        antiBot(address(this), true);
        antiBot(deadAddr, true);
    ...
    function excludeFromMaxTransaction(address updAds, bool isEx) public onlyOwner {
        isExcludedMaxTransaction[updAds] = isEx;
    }

    function antiBot(address updAds, bool isEx) public onlyOwner {
        isAntiBot[updAds] = isEx;
        isExcludedMaxTransaction[updAds] = isEx;
        isExcludedFromFees[updAds] = isEx;
        emit ExcludeFromFees(updAds, isEx);
        
    }

    function updateBuyFees(uint256 _buyFee) external onlyOwner {
        buyFees = _buyFee;
        require(buyFees <= 25, "Must keep fees at 25% or less");
    }
    ...
    function updateLimits(uint256 _amount) external onlyOwner {
    ...
    function _transfer(
        ...
        uint256 transferTaxRate = (ammPairs[to] &&
        ...
        if(activeLimits){
            if (
                ...
                if(!activeTrading){
                    require((isExcludedFromFees[from] || isExcludedFromFees[to]) && timeStart > 0, "Trading is not active.");
                }

               if (timeStart == 0 && !isAntiBot[from] && !isAntiBot[to]) {
                    require(isAntiBot[from] || isAntiBot[to], "anti bot address.");
                    timeStart = block.timestamp;
                }


                if (block.timestamp - timeStart < ENABLE_TIME && !isAntiBot[from] && !isAntiBot[to]) {
                    require(isAntiBot[from] || isAntiBot[to], "anti bot address.");
                }
        ...
        if(takeFee){
            fees = amount.mul(transferTaxRate).div(100);
            if(fees > 0){
