...
contract FeeToken is IERC20, Ownable, ReentrancyGuard {
    error InsufficientARCBalance(uint256 _toStake, uint256 _balance);
    error InsufficientARCAllowance(uint256 _toStake, uint256 _allowance);
    error InsufficientsARCBalance(uint256 _toUnstake, uint256 _balance);
    error InsufficientsARCAllowance(uint256 _toUnstake, uint256 _allowance);
    ...
    constructor (address _USDT, address _ARC, string memory __name, string memory __symbol, uint256 __totalSupply) {
        feeTracker = new FeeTracker(_USDT);
        feeTracker.transferOwnership(_msgSender());
        underlyingToken = IERC20Burnable(_ARC);
