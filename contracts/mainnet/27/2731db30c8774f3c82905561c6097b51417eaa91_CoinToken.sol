...
contract CoinToken is ERC20, Ownable, ILiquidityManageable {
    ...
    ICoinSwapV2Router01 public swapFeesRouter;
    ...
    mapping(address => bool) internal _isCoinPair;
    mapping(uint256 => mapping(address => bool))
        internal _isCachedAutodetectedCoinPair;
    ...
    event CoinPairAdded(address _pair);
    event CoinPairRemoved(address _pair);
    ...
    ) ERC20("Coin Swap", "COIN") {
        ICoinSwapV2Router01 router = ICoinSwapV2Router01(_router);
        ICoinSwapV2Factory factory = ICoinSwapV2Factory(router.factory());
    ...
    function isCoinPair(address _pair) public returns (bool isPair) {
        if (_isCoinPair[_pair]) {
            return true;
        }
        ...
        if (_isCachedAutodetectedCoinPair[_currentCacheVersion][_pair]) {
            return true;
        }
        ...
        bool isVerifiedPair = isWhitelistedFactory[factory] &&
            ICoinSwapV2Factory(factory).isPair(_pair);
        ...
        if (token0 == address(this)) {
            if (isVerifiedPair) {
                _isCachedAutodetectedCoinPair[_currentCacheVersion][
                    _pair
                ] = true;
            }
    ...
    function _shouldTakeTransferTax(
        ...
        return
            ...
            (isCoinPair(sender) || isCoinPair(recipient));
    }
    ...
    function _transfer(
        ...
        bool isBuy = isCoinPair(sender);
        bool isSell = isCoinPair(recipient);
        bool isIndirectSwap = (_isCoinPair[sender] ||
            _isCachedAutodetectedCoinPair[_currentCacheVersion][sender]) &&
            (_isCoinPair[recipient] ||
                _isCachedAutodetectedCoinPair[_currentCacheVersion][recipient]);
    ...
    function isExcludedFromMaxWallet(
        ...
        return _isExcludedFromMaxWallet[account] || _isCoinPair[account];
    }
    ...
    function addCoinPair(address _pair) external onlyOwner {
        _isCoinPair[_pair] = true;
        emit CoinPairAdded(_pair);
    }

    function removeCoinPair(address _pair) external onlyOwner {
        _isCoinPair[_pair] = false;
        emit CoinPairRemoved(_pair);
    }
    ...
    function setSwapFeesRouter(address _swapFeesRouter) public onlyOwner {
        swapFeesRouter = ICoinSwapV2Router01(_swapFeesRouter);
    }
