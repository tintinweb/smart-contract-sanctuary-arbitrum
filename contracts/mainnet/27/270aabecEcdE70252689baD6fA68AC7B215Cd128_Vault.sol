...
library Roles {
    ...
    bytes32 public constant RENT_REDEEMER_VAULT =
        keccak256("RENT_REDEEMER_VAULT");
}
...
// @todo solve the issue where 1 is returned in no profit case
    ...
    ERC721HolderUpgradeable
{
    ...
    struct LpTokenDetails {
        uint depositRate; // conversion rate when made deposit
        uint depositAmount;
        uint createdAt;
        int pnl; // profit or loss made, for record purpose
    }
    ...
    event ProfitNotified(
        uint profitNotified,
        uint adminPerformanceFeesCollected
    );
    event LossNotified(uint amount);
    event FeesUpdated(uint newFees);
    event AdminPerformanceFeesUpdated(uint newFees);
    event AdminPerformanceFeesWithdrawn(address to, uint amountWithdrawn);
    event Deposit(
        uint assets,
        uint depositRate,
        uint lpTokenId,
    ...
    event RentBooked(string[] symbols, uint[] rentBooked);
    event ProfitBooked(uint[] lockNftTokenIds, int[] pnlAccrued);

    /// @notice Representation of 100%. Equals t0 10000 that means 100 = 1%
    uint256 public constant PERCENTAGE_BASED_POINT = 10000; // 100 %

    uint256 public constant TIME_SCALING_FACTOR = 8; // 8 decimals

    /// @notice percentage of fees to be deducted when SecondaryMarketplace is borrowing
    uint256 public fees; // e.g 500 = 5%

    uint public nftCounter;

    mapping(uint tokenId => LpTokenDetails lpTokenDetails)
        public lpTokenIdToDetails;

    /// @notice how much shares user will get against 1 asset currently
    uint public currentConversionRatePerAsset;

    // Underlying ERC20 tokens related stuff
    /// @notice address of underlying erc20 token
    ERC20Upgradeable public asset;
    /// @notice keeps track how many underlying tokens are deposited in this vault
    uint private _totalVirtualSharesSupply;

    /// @notice keeps track how many virtual lps are in circulation
    uint private _totalAssets;

    uint8 private _decimals;

    ///@notice  The duration (in seconds) that users must keep their deposits in the vault to be eligible to fully realize the rewards.
    ///         If a user tries to realize rewards before this period elapses, they will receive a reduced reward amount.
    uint public fullRewardDuration;

    /// @notice x% of `fees`
    uint public adminPerformanceFeesPercentage; // e.g 500 = 5%

    /// @dev fees in collected only in profit case.
    uint public adminPerformanceFeesCollected;
    ...
    function initialize(
        ...
        asset = ERC20Upgradeable(underlyingToken);
        _decimals = asset.decimals();
        currentConversionRatePerAsset = 10 ** decimals();
        fullRewardDuration = _fullRewardDuration;
        fees = 500; // 5%
        adminPerformanceFeesPercentage = fees;
    }

    /**
     * @param assets number of underlying token to deposit
     * @param receiver reciever of the Lp-NFT
     * @return Token id of Lp-NFT
     */

    // @todo add re-entrancy guards whereever possible
    function deposit(
        uint256 assets,
        address receiver
        ...
        uint virtualShares = _getVirtualSharesToMint(assets);
        console.log(virtualShares, "these are vs minted");
        console.log(currentConversionRatePerAsset, "this is current cr p");
        _totalVirtualSharesSupply += virtualShares;
        _totalAssets += assets;
        uint lpTokenId = _mintNFT(
            receiver,
            currentConversionRatePerAsset,
            assets,
            block.timestamp
        );
        emit Deposit(
    ...
    /**
     * @notice to burn Lp-NFT withdraw underlying
     * @param lpTokenId Token id of LP-NFT
     * @param receiver receiver of underlying
     */
    function withdraw(
        uint lpTokenId,
        address receiver
        ...
        require(
            _isApprovedOrOwner(msg.sender, lpTokenId),
            "Vault: !owner or !approved"
        );
        _burn(lpTokenId);
        (uint assetsToGive, int pnl) = previewWithdraw(lpTokenId);
        LpTokenDetails memory lpTokenDetails = lpTokenIdToDetails[lpTokenId];
        int coversionRateDiff = int(currentConversionRatePerAsset) -
            int(lpTokenDetails.depositRate);
        // 100 - 50 = 50 => in case of profit
        // 50 - 100 => = -50 in case of loss
        int diff = pnl - lpTokenIdToDetails[lpTokenId].pnl;
        if (diff >= 0) {
            // 1000 + 100  => 1050 => 50 is already claimed and is represented in 'diff'
            assetsToGive =
                assetsToGive -
                uint(lpTokenIdToDetails[lpTokenId].pnl); // @todo make sure int - stuff
            lpTokenIdToDetails[lpTokenId].pnl += diff;
        } else {
            lpTokenIdToDetails[lpTokenId].pnl += diff;
        }
        uint sharesToBurn = _getVirtualSharesToBurn(
            assetsToGive
            // lpTokenIdToDetails[lpTokenId].depositAmount, // @todo verify what to pass here
            // coversionRateDiff,
            // lpTokenDetails.createdAt
        );
        console.log(sharesToBurn, "shares to burn");
        console.log(assetsToGive, "assetsToGive");
        console.logInt(pnl);
        _totalAssets -= assetsToGive;
        _totalVirtualSharesSupply -= sharesToBurn;
        // maybe find an efficient way to do this
        if (_totalVirtualSharesSupply == 0) {
            currentConversionRatePerAsset = 10 ** decimals();
        }
        asset.safeTransfer(receiver, assetsToGive);
        emit Withdraw(lpTokenId, assetsToGive, pnl, msg.sender);
        return assetsToGive;
    }

    function previewWithdraw(
        uint lpTokenId
    ) public view returns (uint assetsUserWillReceive, int pnl) {
        LpTokenDetails memory lpTokenDetails = lpTokenIdToDetails[lpTokenId];
        pnl = _getPNLAccrued(lpTokenId);
        assetsUserWillReceive = uint(int(lpTokenDetails.depositAmount) + pnl);
    }

    function _getPNLAccrued(uint _lpTokenId) internal view returns (int) {
        LpTokenDetails memory lpTokenDetails = lpTokenIdToDetails[_lpTokenId];
        require(lpTokenDetails.createdAt != 0, "Invalid lp token id");
        int conversionRateDiff = int(currentConversionRatePerAsset) -
            int(lpTokenDetails.depositRate);
        if (conversionRateDiff == 0) return 0; // @todo check this if it is needed or not
        uint depositAmount = lpTokenDetails.depositAmount;
        int timeFactor = int(getTimeFactor(lpTokenDetails.createdAt));
        int scalingFactor = int256(10 ** (decimals() + TIME_SCALING_FACTOR));
        console.logInt(conversionRateDiff);
        console.log("This is coversion rate diff");
        return
            (
                (int(depositAmount) *
                    conversionRateDiff *
                    (
                        conversionRateDiff > 0 // true : profit , false : loss or breakeven
                            ? timeFactor // consider time factor when is in profit
                            : int(10 ** TIME_SCALING_FACTOR) // do not consider time factor and immediately realize loss in loss case
                    ))
            ) / scalingFactor;
    }
     ...
     * @param propertyToken property that the vault is buying(kind of)
     * @param noOfPropertyToken no of property tokens the vault is paying for
     * @param amountToBorrow value of `noOfPropertyToken` in vault `asset` currency
     * @return actualBorrowedAmount actual amount paid by vault after deducting fees
     */
    ...
    function borrow(
        ...
        external
        hasRole(Roles.MARKETPLACE_BORROWER_ROLE)
        nonReentrant
        returns (uint actualBorrowedAmount)
    {
        ERC20Upgradeable(propertyToken).safeTransferFrom(
            msg.sender,
            address(this),
            noOfPropertyToken
        );
        actualBorrowedAmount =
            amountToBorrow -
            ((amountToBorrow * fees) / PERCENTAGE_BASED_POINT);
        asset.safeTransfer(recipient, actualBorrowedAmount);

        emit TokensLentToMP(msg.sender, amountToBorrow);
    }
     ...
     * @param  _recipient address of property token receiver
    ...
    ) external hasRole(Roles.MARKETPLACE_BORROWER_ROLE) nonReentrant {
        // @todo change modifier name to onlySecondaryMp
        // _notifyRentAsRewardToGauge(_propertyToken); // @todo update this stuff
        ERC20Upgradeable(_propertyBasecurrency).safeTransferFrom(
            msg.sender,
            address(this),
            _repayAmount
        );
        ERC20Upgradeable(_propertyToken).safeTransfer(
            _recipient,
            _amountOfTokens
        );
    ...
    function updateFees(
        uint256 _newFees
    ) external hasRole(Roles.VAULT_MANAGER) {
    ...
    /**
     * @notice Updates Admin performance fees
     * @param _newFees percentage of fee ; 100 is 1%
     */
    function updateAdminPerformanceFees(uint256 _newFees) external {
        adminPerformanceFeesPercentage = _newFees;
        emit AdminPerformanceFeesUpdated(_newFees);
    }

    /**
     * @notice withdraw admin performance fees
     * @param _to receiver of fees
     */
    function withdrawAdminPerformanceFees(
        address _to
    ) external hasRole(Roles.VAULT_MANAGER) {
        uint adminPerformanceFeesCollectedCached = adminPerformanceFeesCollected;
        delete adminPerformanceFeesCollected;
        asset.safeTransfer(_to, adminPerformanceFeesCollectedCached);
        emit AdminPerformanceFeesWithdrawn(
            _to,
            adminPerformanceFeesCollectedCached
        );
    }

    // // @todo to be removed for production, this is for testing only
    // function cheatConversionRate(uint newConversionRate) external {
    //     if (newConversionRate > currentConversionRatePerAsset) {} else {}
    //     currentConversionRatePerAsset = newConversionRate;
    // }

    function _updateConversionRate() internal {
        uint256 supply = totalVirtualSharesSupply(); // Saves an extra SLOAD if totalSupply is non-zero.
        console.log(supply, "this is sol supply");
        console.log(totalAssets(), "this is total assets");
        uint newRate = supply == 0
            ? 10 ** decimals()
            : (uint256(10 ** decimals()) * totalAssets()) / supply;
        // @todo decrease totalSupply when withdrawing
        // currentConversionRatePerAsset =
        //     ((oldCachedConversionRatePerAsset * (pnlCount - 1)) + newRate) /
        //     pnlCount;
        console.log(newRate, "new rate");
        currentConversionRatePerAsset = newRate;
    }

    /// @notice when loss occurs
    function nofityLoss(
        uint256 _amount
    ) external hasRole(Roles.MARKETPLACE_BORROWER_ROLE) {
        _totalAssets -= _amount;
        _updateConversionRate();
        emit LossNotified(_amount);
    }

    /// @notice when profit occurs
    function notifyProfit(
        uint256 _amount
    ) public hasRole(Roles.MARKETPLACE_BORROWER_ROLE) {
        _notifyProfitInternal(_amount);
    }

    function _notifyProfitInternal(uint _amount) internal {
        uint profitAfterFees = _deductAdminPerformanceFees(_amount);
        _totalAssets += profitAfterFees;
        _updateConversionRate();
        console.log(_amount, "_amount");
        console.log(profitAfterFees, "profitAfterFees");
        emit ProfitNotified(profitAfterFees, _amount - profitAfterFees);
    }

    // @todo remove this for production
    function cheatHardReset() external {
        delete _totalAssets;
        delete _totalVirtualSharesSupply;
        currentConversionRatePerAsset = 10 ** decimals();
    }

    function bookProfit(
        uint[] memory lockNftTokenIds,
        address receiver
    ) external returns (int[] memory) {
        uint arrLength = lockNftTokenIds.length;
        int[] memory pnlAccrued = new int[](arrLength);
        uint totalPnl;
        for (uint i; i < arrLength; i++) {
            pnlAccrued[i] = _bookProfitInternal(lockNftTokenIds[i]);
            totalPnl += uint(pnlAccrued[i]);
        }
        if (totalPnl > 0) asset.safeTransfer(receiver, totalPnl);
        emit ProfitBooked(lockNftTokenIds, pnlAccrued);
        return pnlAccrued;
    }

    function _bookProfitInternal(uint _lockNftTokenId) internal returns (int) {
        require(
            _isApprovedOrOwner(msg.sender, _lockNftTokenId),
            "Vault: !owner or !approved"
        );
        LpTokenDetails storage lpTokenDetails = lpTokenIdToDetails[
            _lockNftTokenId
        ];
        int pnl = _getPNLAccrued(_lockNftTokenId);
        int diffPnl = pnl - lpTokenDetails.pnl;
        // realizing profit and updating depositRate
        if (diffPnl >= 0) {
            // lpTokenDetails.depositRate = currentConversionRatePerAsset;
            lpTokenDetails.pnl += diffPnl;
    ...
    function bookRent(
        ...
        uint[] memory rentBooked = new uint[](arrLength);
        string[] memory singleSymbol = new string[](1);
        address rentDistributor = address(systemRegistry.rentDistributor());
        uint totalRent;
        for (uint i; i < arrLength; i++) {
            singleSymbol[0] = symbols[i];
            uint lockNftTokenId = systemRegistry.rentShare().harvestRent(
                singleSymbol,
                address(this)
            );
            IERC721(address(systemRegistry.lockNft())).approve(
                address(systemRegistry.rentDistributor()),
                lockNftTokenId
            );
            // @todo do this tx with systemRegistry Style,
            // rn, not doing because systemRegistry do not have interface updated
            rentBooked[i] = IRentDistributor(rentDistributor)
                .redeemRentForVault(lockNftTokenId);
            totalRent += rentBooked[i];
        }
        _notifyProfitInternal(totalRent);
        emit RentBooked(symbols, rentBooked);
        return rentBooked;
    }

    function _deductAdminPerformanceFees(uint _amount) internal returns (uint) {
        uint adminPerformanceFees = (_amount * adminPerformanceFeesPercentage) /
            PERCENTAGE_BASED_POINT;
        adminPerformanceFeesCollected += adminPerformanceFees;
        return _amount - adminPerformanceFees;
    }

    /// @return returns tokenId of minted token
    function _mintNFT(
        address to,
        uint depositRate,
        uint depositAmount,
        ...
        _safeMint(to, tokenId);
        lpTokenIdToDetails[tokenId] = LpTokenDetails({
    ...
    function getTimeFactor(uint createdAt) internal view returns (uint) {
        uint fullRewardDuration_ = fullRewardDuration; // caching
        uint timeElapsed = block.timestamp - createdAt;
        timeElapsed = timeElapsed >= fullRewardDuration_
            ? fullRewardDuration_
            : timeElapsed;
        return (timeElapsed * 10 ** TIME_SCALING_FACTOR) / fullRewardDuration_;
    }

    /// @notice returns amount of virtual shares that will be assigned to user on deposit
    function _getVirtualSharesToMint(
        uint256 assets
    ) internal view returns (uint256) {
        // @todo check this stuff
        // for auditor : ignore round down issue in conversion rate
        return
            totalVirtualSharesSupply() == 0
                ? assets
                : (assets / currentConversionRatePerAsset) * (10 ** decimals());
    }

    function _getVirtualSharesToBurn(
        uint _assets
    )
        internal
        view
        returns (
            // ,
            // int _conversionRateDifference,
            // uint _createdAt
            uint
        )
    {
        // uint scalingFactor = 10 ** (decimals() + TIME_SCALING_FACTOR);
        // @todo check for over/underflow
        return ((_assets * 10 ** decimals()) / currentConversionRatePerAsset);
        // getTimeFactor(_createdAt)) / (scalingFactor);
    }

    function totalVirtualSharesSupply() public view returns (uint) {
        return _totalVirtualSharesSupply;
    }
    ...
    function rescueTokens(
        ...
        ERC20Upgradeable(tokenAddress).safeTransfer(recipient, amount);
    }
