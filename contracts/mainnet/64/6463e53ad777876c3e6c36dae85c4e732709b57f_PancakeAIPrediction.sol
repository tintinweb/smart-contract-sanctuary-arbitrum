...
// src/PancakeAIPrediction.sol

/**
 * @title PancakeAIPrediction
 */
contract PancakeAIPrediction is Ownable, Pausable {
     ...
     * @param epoch: epoch
     */
    function betBear(uint256 epoch) external payable whenNotPaused notContract {
     ...
     * @param epoch: epoch
     */
    function betBull(uint256 epoch) external payable whenNotPaused notContract {
        require(epoch == currentEpoch, "Bet is too early/late");
        Round storage round = rounds[epoch];
        require(_bettable(round), "Round not bettable");
        require(msg.value >= minBetAmount, "Bet amount must be greater than minBetAmount");
