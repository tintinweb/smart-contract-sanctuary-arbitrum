...
contract PerpPoolPositionManager is IPositionManager {
  ...
  uint256 private lastIntervalId;
	...
	constructor(address _poolPositionPurchaserAddress,  address _poolTokenAddress, address _priceUtilsAddress, address _leveragedPoolAddress, address _trackingTokenAddress, address _poolCommitterAddress, address _usdcAddress, address _perpPoolUtilsAddress) {
  ...
  function PositionWorth() public view returns (uint256) {
    uint256 committedUsdcWorth = perpPoolUtils.getCommittedUsdcWorth(address(poolPositionPurchaser));
    return committedUsdcWorth;
  }
