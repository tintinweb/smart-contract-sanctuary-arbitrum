...
contract Hedge is IERC20, Context, IPositionRouterCallbackReceiver, ReentrancyGuard, Initializable {
  ...
  struct PositionRequest {
    ...
    address indexToken;           //  token address to open position
    address collateralToken;      //  token used as collateral in long / short position
    uint256 sizeDelta;            //  position change in USD. decimal is 30. calculated by multiplying leverage to collateral usd value
    uint256 collateralDelta;      //  collateral change in USD, decimal is 30
    uint256 collateralAmount;     //  collateral amount to deposit, use decimal of `collateralToken` token
    uint256 acceptablePrice;      //  current `indexToken`(ETH) mark price
    uint256 minOut;               //  when opening long position, `collateralToken` should be swapped to `indexToken`. 
                                  //   this amount is minimum amount of output token when swap
    bytes32 referralCode;         //  GMX referral provides discount and rebates. this contract can benefit up to 10% discount for open/close fee 
                                  //   from referral code.
  }
  ...
  struct PositionRequestQueue {
    ...
    bool isDeposit;                     //  there may be a liquidate action. in that case, we can't use boolean type to check action type
  ...
  uint256 public hedgeFactor = 10; // 2 = 50% pricipal, 10 = 10% prinicpal required, etc
  ...
  address public positionRouter = address(0xb87a436B93fFE9D75c5cFA7bAcFff96430b09868);
   ...
   */
  // constructor(address lpAddress, address _hedgeToken, address gmxRouter) ERC20("HedgeVault", "HVT") {
  ...
  function deposit(uint256 lpAmount) external lock nonReentrant returns (uint256) {
  ...
  /**
   * @notice
   *  do gmx perp. (long & short)
   *  to be simple for now, assumes deposit USDC and open ETH position
   ...
   * @param pr position parameter structure variable
   */
  function updatePerp(PositionRequest calldata pr) external payable onlyManager lock nonReentrant {
    require(msg.value >= IPositionRouter(positionRouter).minExecutionFee(), "too low execution fee");
    require(
      pr.sizeDelta > pr.minOut.mul(gmxVault.getMinPrice(pr.indexToken)),
      "too low leverage"
    );
    require(
      pr.sizeDelta.div(gmxVault.tokenToUsdMin(pr.collateralToken, pr.collateralAmount)) < gmxVault.maxLeverage().div(BASIS_POINTS_DIVISOR),
      "exceed max leverage"
    );
    // check available amounts to open positions
    _checkPool(pr.isLong, pr.indexToken, pr.collateralToken, pr.sizeDelta);
    ...
    address[] memory path;
    if (pr.isLong) {
      if (address(hedgeToken) == pr.indexToken) {
        path = new address[](1);
        path[0] = address(hedgeToken);
      } else {
        path = new address[](2);
        path[0] = address(hedgeToken);
        path[1] = pr.indexToken;
      }
    } else {
      if (address(hedgeToken) == pr.collateralToken) {
        path = new address[](1);
        path[0] = address(hedgeToken);
      } else {
        path = new address[](2);
    ...
    if (pr.isOpen) {
      requestKey = IPositionRouter(positionRouter).createIncreasePosition{value: msg.value}(
        path,
        pr.indexToken,
        pr.collateralAmount,
        pr.minOut,       // it's better to provide minimum output token amount from a caller rather than calculate here
        pr.sizeDelta,    // we can set sizeDelta based on leverage value. need to decide which one is preferred
        pr.isLong,
        gmxVault.getMinPrice(pr.indexToken),   // current ETH mark price
        msg.value,
        pr.referralCode,
        address(this)
      );
    } else {
      requestKey = IPositionRouter(positionRouter).createDecreasePosition{value: msg.value}(
        path,
        pr.indexToken,
        pr.collateralDelta,
        pr.sizeDelta,
        pr.isLong,
        address(this),
        gmxVault.getMinPrice(pr.indexToken),
        pr.minOut,
        msg.value,
        false,
        address(this)
   ...
   */
  function triggerExecutePositionRequest(bytes32 positionKey, bool isOpen) external onlyManager nonReentrant {
    ...
    IncreasePositionRequest memory request = IPositionRouter(positionRouter).increasePositionRequests(positionKey);
    require(request.blockTime.add(minTimeDelayPublic) <= block.timestamp, "delay");

    if (isOpen) {
      IPositionRouter(positionRouter).executeIncreasePosition(positionKey, address(this));
    } else {
      IPositionRouter(positionRouter).executeDecreasePosition(positionKey, address(this));
    }
    _isLock = false;
  }
   ...
   */
  function gmxPositionCallback(bytes32 requestKey, bool isExecuted, bool isIncrease) override external nonReentrant {
    ...
    if (isExecuted) {
      ...
      if (queue.isDeposit == false && hedgeTokenBalance >= required) {
        withdraw(queue.beneficiary, queue.amount);
      }
    }
  ...
  function withdraw(address receipient, uint256 amount) public lock nonReentrant {
