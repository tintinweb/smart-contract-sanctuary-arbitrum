...
contract LiquidationAutomation is AutomationCompatibleInterface, Ownable {
    ...
    uint256 public gasRequired;

    constructor(address _rewardGateway, address _smartVaultManager, address _vaultIndex, uint256 _gasRequired) Ownable(msg.sender) {
    ...
    function checkUpkeep(bytes calldata) external view override returns (bool, bytes memory) {
        uint256 cursor = block.timestamp / 10;
        while(gasleft() > gasRequired) {
            uint256 _id = (cursor % ISmartVaultManager(smartVaultManager).totalSupply()) + 1;
            try ISmartVault(ISmartVaultIndex(vaultIndex).getVaultAddress(_id)).undercollateralised() returns (bool _undercollateralised) {
                if (_undercollateralised) {
                    return (true, abi.encodePacked(uint(_id)));
                }
            } catch {}
            console.log(_id);
            console.log(gasleft());
    ...
    function setGasRequired(uint256 _gasRequired) external onlyOwner() {
        gasRequired = _gasRequired;
    }
}
