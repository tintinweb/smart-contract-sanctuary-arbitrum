...
library RouterJuniorImp {
    ...
    function handleWithdrawJunior(RouterStateStore storage store, Ticket storage ticket) public {
        ...
        store.lastJuniorWithdrawalTimes[params.account] = block.timestamp;
...
library RouterSeniorImp {
    ...
    function handleWithdrawSenior(RouterStateStore storage store, Ticket storage ticket) public {
        ...
        if (params.account != address(0)) {
            ...
            increasePendingWithdrawal(store, params.account, params.shares);
            store.lastSeniorWithdrawalTimes[params.account] = block.timestamp;
        }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function lastJuniorWithdrawalTime(address account) external view returns (uint256) {
        return _store.lastJuniorWithdrawalTimes[account];
    }
    ...
    function lastSeniorWithdrawalTime(address account) external view returns (uint256) {
        return _store.lastSeniorWithdrawalTimes[account];
    }
    ...
    function beforeFillLiquidityOrder(
        ...
        console.log("beforeFillLiquidityOrder", isValid);
    }
    ...
    function afterFillLiquidityOrder(
        ...
        console.log("enter afterFillLiquidityOrder");
        MuxOrderContext memory context = MuxOrderContext({
            orderId: order.id,
            seniorAssetId: order.assetId,
        ...
        console.log("leave afterFillLiquidityOrder");
    }
    ...
    function claimJuniorReward() external returns (uint256) {
        _store.updateRewards();
        return _store.rewardController.claimJuniorRewardFor(msg.sender, msg.sender);
    }

    function claimSeniorReward() external nonReentrant returns (uint256) {
        _store.updateRewards();
        return _store.rewardController.claimSeniorRewardFor(msg.sender, msg.sender);
    }

    function debugRefund() external onlyRole(KEEPER_ROLE) {
        _store.debugRefund();
    }

    function debugSetStatus(RouterStatus status) external onlyRole(KEEPER_ROLE) {
