...
contract TokenPresale is Ownable {
    ...
    uint256 public cap = 40 * 10**6 * 10**18;
    uint256 public constant tokenPrice = 0.000005 * 10**18; // Price per token in eth
    uint256 public constant minInvestment = 0.005 * 10**18;
    ...
    function isPublicSaleActive() public view returns(bool){
      return (publicSaleEnabled && block.timestamp >= publicSaleStartTime);
   }

    function isWhitelistSaleActive() public view returns(bool){
      return (block.timestamp >= whitelistStartTime);
   }


    function buyTokens() external payable {
        require(whitelist[msg.sender] || (publicSaleEnabled && block.timestamp >= publicSaleStartTime), "Presale not started or not allowed");
        require(block.timestamp >= whitelistStartTime, "Whitelist presale not started");
        require(msg.value >= minInvestment, "Investment below minimum threshold");
        uint256 tokensToBuy = (msg.value * 10**18) / tokenPrice;
        require(totalTokensSold + tokensToBuy <= cap, "Exceeds presale cap");
    ...
    function claimTokens() external {
        ...
        require(allocations[msg.sender] > 0, "No tokens to claim");
        uint256 tokensToClaim = allocations[msg.sender];
