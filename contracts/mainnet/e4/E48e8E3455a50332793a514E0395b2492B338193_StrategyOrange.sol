...
interface IOrangeStrykeLPAutomator {
    function deposit(uint256 amount) external returns (bool);
    function redeem(uint256 shares,uint256 minAssets)external returns(bool);
...
contract StrategyFeeManagerInitializable is OwnableUpgradeable, PausableUpgradeable {
...
import "../../interfaces/orange/IAngleDistributor.sol";
import "../../interfaces/orange/IOrangeStrykeLPAutomator.sol";
import "../Common/StratFeeManager.sol";
import "../../interfaces/camelot/ICamelotRouter.sol";
import "../Common/StrategyFeeManagerInitializable.sol";

contract StrategyOrange is StrategyFeeManagerInitializable {
    ...
    address public want;
    address public swant;
    address public adist; //Anble Distributor
    ...
    function initialize(
        address _swant,
        address _native,
        address _angledistributor,
        address[] memory _rewardToWantRoute,
        CommonAddresses calldata _commonAddresses
    ) public initializer {
        __StratFeeManager_init(_commonAddresses);
        want = _rewardToWantRoute[_rewardToWantRoute.length-1];
        swant = _swant;
        reward = _rewardToWantRoute[0];
        native = _native;
        adist = _angledistributor;
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IOrangeStrykeLPAutomator(swant).deposit(wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IOrangeStrykeLPAutomator(swant).redeem( IOrangeStrykeLPAutomator(swant).convertToShares(_amount - wantBal),0);
    ...
    function managerHarvest(address[]  memory _tokens, uint256[] memory _amount, bytes32[][] memory _proofs) external onlyManager {
        address[] memory _user;
        _user[0] = address(this);
        IAngleDistributor(adist).claim(_user, _tokens, _amount, _proofs);
        _harvest(tx.origin);
    }
    ...
    function _swapRewardToWant() internal {
        if (reward != want) {
            ...
            if (amount > 0) {
                ICamelotRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(amount, 0, rewardToWantRoute, address(this), zeroAddress, block.timestamp);
            }
        }
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        uint256 _amount = IOrangeStrykeLPAutomator(swant).convertToAssets(IOrangeStrykeLPAutomator(swant).balanceOf(address(this)));
        return _amount;
    }
    ...
    function retireStrat() external {
        ...
        IOrangeStrykeLPAutomator(swant).redeem( IOrangeStrykeLPAutomator(swant).balanceOf(address(this)),0);
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(swant, type(uint).max);
        IERC20(swant).safeApprove(unirouter, type(uint).max);
    ...
    function _removeAllowances() internal {
        IERC20(want).safeApprove(swant, 0);
        IERC20(swant).safeApprove(unirouter, 0);
