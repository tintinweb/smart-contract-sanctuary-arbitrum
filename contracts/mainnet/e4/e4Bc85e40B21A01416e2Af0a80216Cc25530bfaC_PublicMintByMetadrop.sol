...
import {PrimarySaleModule} from "../PrimarySaleModule.sol";
...
contract PublicMintByMetadrop is PrimarySaleModule, IPublicMintByMetadrop {
  ...
  function initialisePrimarySaleModule(
    ...
    _initialisePrimarySaleModuleBase(
      ...
      publicMintConfig.metadropPerMintFee,
      publicMintConfig.metadropPrimaryShareInBasisPoints,
      metadropOracleAddress_,
      messageValidityInSeconds_,
      phaseId_,
      publicMintConfig.reservedAllocationPhase,
   ...
   * _____________________________________________________________________________________________________________________
   */
  function updatePublicMintPrice(
    uint256 newPublicMintPrice_
  ) external onlyFactoryDuringSupportRequest {
    uint256 oldPublicMintPrice = publicMintPrice;
    publicMintPrice = uint128(newPublicMintPrice_);
    emit PublicMintPriceUpdated(oldPublicMintPrice, newPublicMintPrice_);
  }
  ...
  function publicMint(
    ...
    if (msg.value != ((publicMintPrice + metadropFeePerMint) * quantityToMint_))
