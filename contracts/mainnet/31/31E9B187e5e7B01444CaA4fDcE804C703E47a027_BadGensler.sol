...
contract BadGensler is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BadGenslerDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Bad Gensler", "Gensler") {
        marketingWallet = payable(0x068A64a2D2D7e28602a59Cc5F6d72b65136FCfc1);
        devWallet = payable(0x49F1ef1B4113Bd2A0e560ffDEbc4734D91832f2C);
        ...
        dividendTracker = new BadGenslerDividendTracker(
            payable(this),
            router,
    ...
    function updateFees(
        ...
        require(
            totalSellFees <= 35 && totalBuyFees <= 35,
            "total fees cannot exceed 35 % sell or buy"
        );
...
contract BadGenslerDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BadGensler public BadGenslerContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BadGenslerContract = BadGensler(mainContract);
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "BadGensler_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BadGensler_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
    ...
    function _reinvestDividendOfUser(
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BadGenslerContract);

            uint256 prevBalance = BadGenslerContract.balanceOf(address(this));
            ...
            {
                uint256 received = BadGenslerContract.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    BadGenslerContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
