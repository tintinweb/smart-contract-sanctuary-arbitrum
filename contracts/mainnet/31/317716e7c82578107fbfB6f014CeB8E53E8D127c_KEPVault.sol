...
library KEPChecks {
  ...
  */
  ...
  /**
    * @notice Checks before reward tokens are claimed
    * @param self KEPTypes.Store
  */
  function beforeClaimRewardTokensChecks(
    KEPTypes.Store storage self
  ) external view {
    if (
      address(self.rewardToken) == address(self.WNT) ||
      address(self.rewardToken) == address(self.LRT) ||
      address(self.rewardToken) == address(self.LST) ||
      address(self.rewardToken) == address(self.tokenB)
...
library KEPEmergency {
  ...
  */
  function emergencyRepay(
    ...
    KEPManager.swapExactTokensForTokens(self, _sp);
...
contract KEPVault is ERC20, AccessManaged, ReentrancyGuard, IKEPVault, IKEPVaultEvents {
  ...
  */
  ...
  /**
    * @notice Function for claiming reward tokens sent to this vault
    * @dev Should be called by approved Owner (Timelock + MultiSig)
    * @dev Reward token will NOT be LRT/LST/WNT/tokenB
  */
  function claimRewardTokens() external restricted {
    KEPChecks.beforeClaimRewardTokensChecks(_store);

    IERC20(address(_store.rewardToken)).transfer(
      _store.treasury,
      IERC20(address(_store.rewardToken)).balanceOf(address(this))
    );
  }
  ...
  */
  function withdraw(
    ...
    if (address(self.tokenB) != address(self.WNT)) {
      ...
      _wntAmtInSwapped = KEPManager.swapTokensForExactTokens(self, _sp2);
    } else {
