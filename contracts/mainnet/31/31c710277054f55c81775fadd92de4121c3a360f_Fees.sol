...
contract Fees {
	...
	function autoCalculate(address _account, uint256 _amount) external view returns (uint256, uint256, uint256, uint256, bool) {
        ...
        if (accountMul != 0) {
			...
			return (mintAmount_, multiplierBribe, accountMul, parity, _refered);
		} else {
			...
			return (mintAmount_, multiplierBribe, multiplierDaily, parity, _refered);
		}
	}

    function calculateMint(address _account, uint256 _amountFees, uint256 _currentMultiplier) external view returns (uint256 amount_) {
        uint256 accountMul = IMiningStrat(miningStrat).accountMultipliers(_account);
        uint256 mintAmount_;
		bool _refered = IReferrer(referrerSC).isPlayerReferred(_account);
        if (accountMul != 0) {
