  ...
  IERC721Receiver
{
  ...
  IOreoBoosterConfig public shipboosterConfig;
  IERC20Upgradeable public oreo;
  ...
  constructor(
    IERC20Upgradeable _oreo,
    IMasterChef _masterChef,
    IOreoBoosterConfig _shipboosterConfig,
    IWNativeRelayer _wNativeRelayer,
    address _wNative
  ) public {
    require(_wNative != address(0), "OreoBooster::initialize:: _wNative cannot be address(0)");
    require(address(_oreo) != address(0), "OreoNFTOffering::initialize:: _oreo cannot be address(0)");
    require(address(_masterChef) != address(0), "OreoNFTOffering::initialize:: _masterChef cannot be address(0)");
    require(
      address(_shipboosterConfig) != address(0),
      "OreoNFTOffering::initialize:: _shipboosterConfig cannot be address(0)"
    );
    require(
  ...
  modifier isStakeTokenOK(address _stakeToken) {
    require(shipboosterConfig.stakeTokenAllowance(_stakeToken), "OreoBooster::isStakeTokenOK::bad stake token");
    _;
  }
  ...
  function pendingBoosterOreo(address _stakeToken, address _user) external view returns (uint256) {
    ...
    (, , uint256 boostBps) = shipboosterConfig.energyInfo(stakingNFT.nftAddress, stakingNFT.nftTokenId);
    // if (currentEnergy == 0) {
    //   return 0;
    // }
    return pendingOreo.mul(boostBps).div(1e4);
    // return MathUpgradeable.min(currentEnergy, pendingOreo.mul(boostBps).div(1e4));
  }
  ...
  function harvest(address[] calldata _stakeTokens) external whenNotPaused nonReentrant {
    for (uint256 i = 0; i < _stakeTokens.length; i++) {
      require(shipboosterConfig.stakeTokenAllowance(_stakeTokens[i]), "OreoBooster::harvest::bad stake token");
