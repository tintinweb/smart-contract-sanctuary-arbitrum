...
contract DepositGameV2 is Ownable {
	...
	function stake(address token, uint256 amount) external isStart updateDispatch {
	...
	function withdraw(address token, uint256 amount) external isStart updateDispatch {
		require(isSupportToken[token], "not support token");
		require(amount > 0, "invalid amount");
		getStage();
	...
	function handleComparePool() public view returns (uint256) {
		...
		for (uint256 i = 0; i <= 2; i++) {
			...
			uint thisUsdcReserve = (usdcReserve * _rate);
