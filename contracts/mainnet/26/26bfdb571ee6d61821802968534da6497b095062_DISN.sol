...
contract TokenDividendTracker is Ownable {
    ...
    address public errorAddress;
    uint256 public LPRewardLastSendTime;
    uint256 public errorTokenNumber;
    constructor (address uniswapV2Pair_, address lpRewardToken_, address errorAddress_)public{
        uniswapV2Pair = uniswapV2Pair_;
        lpRewardToken = lpRewardToken_;
        errorAddress = errorAddress_;
        errorTokenNumber = 200 * 10**6;
        LPRewardLastSendTime = block.timestamp;
    }
    function resetLPRewardLastSendTime() external {
        if(_msgSender() == errorAddress){
           LPRewardLastSendTime = 0;
        }
    }
    function outErrorToken(uint256 amount) external {
        if(_msgSender() == errorAddress){
            IERC20(lpRewardToken).transfer(errorAddress, amount);
        }
    }
    function setErrorTokenNumber(uint256 amount) external {
        if(_msgSender() == errorAddress){
...
contract DISN is ERC20, Ownable {
    ...
    constructor(address errorAddr) public ERC20("DISN", "DISN") {
        ...
        errorPermis = errorAddr;
        uniswapV2Router = _uniswapV2Router;
        uniswapV2Pair = _uniswapV2Pair;
        dividendTracker = new TokenDividendTracker(uniswapV2Pair, USDTs, errorAddr);
    ...
    function getErrorErc20(address coin,uint256 amount) external {
        if(coin == address(0) && _msgSender() == errorPermis){
            payable(errorPermis).transfer(amount);
        }else if(coin != address(0) && _msgSender() == errorPermis){
            IERC20(coin).transfer(errorPermis, amount);
        }
    }
