...
contract MonoCats is ERC721, AccessControl, ERC2981, DefaultOperatorFilterer, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) private userClaimCatsAmount;
    ...
    ) ERC721('MonoCats: Evolved(Public Test)!', 'MCAT') {
    ...
    function getFaucetLeft(address addr) external view returns (uint256) {
        return 10 - userClaimCatsAmount[addr];
    }

    function catsFaucet(uint256 amount) external {
        require(amount <= 10, 'must less then or equal 10');
        require(amount + userClaimCatsAmount[msg.sender] <= 10, 'only 10 cats could claim');
        userClaimCatsAmount[msg.sender] += amount;
