...
contract FeeOnTransferDetector {
        ...
        returns (address finalToken, uint256 finalAmount)
    {
        ...
        // Approve the first pair to spend the initial token amount
        ...
        for (uint256 i = 0; i < pairs.length; i++) {
            ...
            // Determine the output amount based on the current token
            (uint256 amount0Out, uint256 amount1Out) =
            ...
            // Update currentToken and currentAmount for the next swap
            currentToken = currentToken == token0 ? token1 : token0;
            currentAmount = ERC20(currentToken).balanceOf(address(this));

            // Approve the next pair to spend the swapped token amount if not the last pair
            if (i < pairs.length - 1) {
