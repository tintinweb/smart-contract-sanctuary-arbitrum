...
interface IOldgPLX {
    function rawStakedPLXInfo(
        address _user,
        uint8 _period
    ) external view returns (TokenStake memory);
}
...
contract gPLX is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    function migrate(address[] memory _user) external onlyOwner {
        for (uint i; i < _user.length; i++) {
            for (uint8 _period; _period <= 4; _period++) {
                uint256 period = stakingPeriod[_period];
                TokenStake memory tokenStake = IOldgPLX(oldContract)
                    .rawStakedPLXInfo(_user[i], _period);
                uint256 length = tokenStake.stakingList.length;
                if (length > 0) {
                    TokenStake storage userTokenStake = staked[_user[i]][
                        _period
                    ];
                    userTokenStake.startIdx = tokenStake.startIdx;
                    userTokenStake.unstakebleAmount = tokenStake
                        .unstakebleAmount;

                    for (uint j = 0; j < length; j++) {
                        userTokenStake.stakingList.push(
                            tokenStake.stakingList[j]
                        );
                        userDaysList[_user[i]][_period].push(
                            tokenStake.stakingList[j].startTime
                        );
                        uint256 amount = tokenStake.stakingList[j].plxAmount;
                        if (amount > 0) {
                            super._mint(_user[i], amount);
                        }
                    }
                }
            }
            PLXDistributor.migrateUser(_user[i]);
        }
    }
