...
import { CelerIMFacetBase, CelerIM } from "blinq/Helpers/CelerIMFacetBase.sol";
import { StandardizedCallFacet } from "blinq/Facets/StandardizedCallFacet.sol";
...
contract CalldataVerificationFacet {
    ...
    function extractBridgeData(
        bytes calldata data
    ) external pure returns (IBlinQ.BridgeData memory bridgeData) {
    ...
    function extractData(
        ...
        pure
        returns (
            IBlinQ.BridgeData memory bridgeData,
    ...
    {
        IBlinQ.BridgeData memory bridgeData = _extractBridgeData(data);
    ...
    function validateDestinationCalldata(
        ...
        if (selector == AmarokFacet.startBridgeTokensViaAmarok.selector) {
            (, AmarokFacet.AmarokData memory amarokData) = abi.decode(
                callData.slice(4, callData.length - 4),
                (IBlinQ.BridgeData, AmarokFacet.AmarokData)
            );
        ...
        if (
            ...
            (, , AmarokFacet.AmarokData memory amarokData) = abi.decode(
                callData.slice(4, callData.length - 4),
                (IBlinQ.BridgeData, LibSwap.SwapData[], AmarokFacet.AmarokData)
            );
            return
                keccak256(dstCalldata) == keccak256(amarokData.callData) &&
                abi.decode(callTo, (address)) == amarokData.callTo;
        ...
        if (selector == StargateFacet.startBridgeTokensViaStargate.selector) {
            (, StargateFacet.StargateData memory stargateData) = abi.decode(
                callData.slice(4, callData.length - 4),
                (IBlinQ.BridgeData, StargateFacet.StargateData)
            );
            return
        ...
        if (
            ...
            (, CelerIM.CelerIMData memory celerIMData) = abi.decode(
                callData.slice(4, callData.length - 4),
                (IBlinQ.BridgeData, CelerIM.CelerIMData)
            );
            return
        ...
        if (
            ...
            (, , CelerIM.CelerIMData memory celerIMData) = abi.decode(
                callData.slice(4, callData.length - 4),
                (IBlinQ.BridgeData, LibSwap.SwapData[], CelerIM.CelerIMData)
            );
            return
                keccak256(dstCalldata) == keccak256(celerIMData.callData) &&
                keccak256(callTo) == keccak256(celerIMData.callTo);
    ...
    function _extractBridgeData(
        bytes calldata data
    ) internal pure returns (IBlinQ.BridgeData memory bridgeData) {
        if (
            bytes4(data[:4]) == StandardizedCallFacet.standardizedCall.selector
        ) {
            // StandardizedCall
        ...
        // normal call
        bridgeData = abi.decode(data[4:], (IBlinQ.BridgeData));
    }
    ...
    function _extractSwapData(
        ...
        if (
            ...
            (, swapData) = abi.decode(
                unwrappedData.slice(4, unwrappedData.length - 4),
                (IBlinQ.BridgeData, LibSwap.SwapData[])
            );
            return swapData;
        }
        // normal call
...
contract AmarokFacet is IBlinQ, ReentrancyGuard, SwapperV2, Validatable {
    /// Storage ///
...
contract StargateFacet is IBlinQ, ReentrancyGuard, SwapperV2, Validatable {
    /// CONSTANTS ///
    ...
    bytes32 internal constant NAMESPACE =
        keccak256("com.blinq.facets.stargate");
    ...
    function startBridgeTokensViaStargate(
        IBlinQ.BridgeData calldata _bridgeData,
...
import { RelayerCelerIM } from "blinq/Periphery/RelayerCelerIM.sol";
...
import { CelerIM } from "blinq/Helpers/CelerIMFacetBase.sol";
...
/// @custom:version 2.0.0
contract RelayerCelerIM is IBlinQ, TransferrableOwnership {
    using SafeERC20 for IERC20;
...
/// @author モンキー・D・ルフィ
/// @notice A simple registry to track BlinQ periphery contracts
/// @custom:version 1.0.0
contract PeripheryRegistryFacet {
    /// Storage ///

    bytes32 internal constant NAMESPACE =
        keccak256("com.blinq.facets.periphery_registry");
