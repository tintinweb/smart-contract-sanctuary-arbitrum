...
contract UserAgent is ERC2771Context, ReentrancyGuard, Initializable {
    using SafeERC20 for IERC20;

    address public immutable EXCHANGE_ROUTER_GMX;
    ...
    constructor(
        ...
        address exchangeRouterGMX_,
        address roles_,
        address router_,
        address dataStore_
    ) ERC2771Context(trustedForwarder_) {
        require(trustedForwarder_ != address(0), "UserAgent: zero forwarder");
        require(exchangeRouterGMX_ != address(0), "UserAgent: zero exchange");
        require(roles_ != address(0), "UserAgent: zero roles");
        require(router_ != address(0), "UserAgent: zero router");
        require(dataStore_ != address(0), "UserAgent: zero data store");

        EXCHANGE_ROUTER_GMX = exchangeRouterGMX_;
        ROLES = roles_;
        ROUTER = router_;
        DATA_STORE = dataStore_;
    }

    // Fallback only allows sending ETH but not receiving it
    fallback() external payable {
        // GMX will wrap and send WETH if ETH transfer fails
        require(msg.data.length != 0, "UserAgent: empty data");

        bytes32 role = RolesLibrary.EXECUTOR_ROLE;
        require(
            IRoles(ROLES).hasRole(role, _msgSender()),
            "UserAgent: unauthorized"
        );
        _delegate(EXCHANGE_ROUTER_GMX);

        // useragent balance check is not required because we pass the entire msg.value to the exchange router
    }

    function initialize(address owner_) external initializer {
        require(owner_ != address(0), "UserAgent: zero owner");
        owner = owner_;
    }
    ...
    function withdraw(
        ...
        require(token != address(0), "UserAgent: zero token");
        require(to != address(0), "UserAgent: zero to");
        require(amount > 0, "UserAgent: zero amount");

        TokenTransfer.transfer("UAWD", DATA_STORE, ROUTER, token, to, amount);
    }

    // modified from OpenZeppelin's Proxy.sol
    function _delegate(address implementation) internal virtual {
        uint256 value = msg.value;
        uint256 datasize = _msgData().length;
        assembly {
            ...
            let result := call(gas(), implementation, value, 0, datasize, 0, 0)
