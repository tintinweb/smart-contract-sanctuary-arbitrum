...
import "./interfaces/IBridgeworldLegion.sol";
import "./interfaces/ILegion.sol";
...
contract VerifyLegionGenesis is OwnableUpgradeable {
    ...
    function atlasMineBalanceOf(address owner) external view returns (uint256) {
        return tokensToBalance(tokensInBridgeworld(atlas, owner));
    }

    function craftingBalanceOf(address owner) external view returns (uint256) {
        return tokensToBalance(tokensInBridgeworld(crafting, owner));
    }

    function questingBalanceOf(address owner) external view returns (uint256) {
        return tokensToBalance(tokensInBridgeworld(questing, owner));
    }

    function summoningBalanceOf(address owner) external view returns (uint256) {
        return tokensToBalance(tokensInBridgeworld(summoning, owner));
    }

    function walletBalanceOf(address owner) external view returns (uint256) {
        return tokensToBalance(tokensInWallet(owner));
    }
}
...
interface ILegion is IERC721EnumerableUpgradeable {}
...
interface ILegionMetadataStore {
    ...
    function metadataForLegion(uint256 tokenId)
        external
        view
