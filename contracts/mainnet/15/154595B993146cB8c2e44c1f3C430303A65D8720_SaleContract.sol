...
contract SaleContract is AccessControlEnumerable, ISaleContract, Ownable, BlackHolePrevention, UsesGalaxisRegistry, IVersionable { 
    ...
    uint256             _saleEnd;
    uint256             _fullPrice;
    uint256             _fullERC20Price;
    bool                _ethSaleEnabled;
    bool                _erc20SaleEnabled;
    ...
    function init(SaleConfiguration calldata config, address newOwner) external {
        ...
        _UpdateSaleConfiguration(config);
        _UpdateWalletsAndShares(config.wallets, config.shares);
        _transferOwnership(newOwner);

        // is membership card contract?
        isMembership = isMembershipCard(config);
        _initialized = true;
    }

    function isMembershipCard(SaleConfiguration calldata config) internal view returns(bool) {
        ...
        address firstTokenAddress = myCommunityRegistry.getRegistryAddress("TOKEN_1");
        if(config.token == firstTokenAddress) {
            return true;
        }
        return false;
    }
    ...
    function _UpdateSaleConfiguration(SaleConfiguration memory config) internal {
        ...
        _fullERC20Price     = config.fullERC20Price;
        _ethSaleEnabled     = config.ethSaleEnabled;
        _erc20SaleEnabled   = config.erc20SaleEnabled;
        _erc20tokenAddress  = config.erc20tokenAddress;
    ...
    function UpdateWalletsAndShares(
        ...
        _UpdateWalletsAndShares(_newWallets,_newShares);
    }

    function _UpdateWalletsAndShares(
        address payable[] memory _newWallets,
        uint16[] memory _newShares
    ) internal {
    ...
    function _internalMint(uint256 _numberOfCards, address _receiver, uint256 _value, uint8 _section) internal {
        ...
        if(_fullPrice == 0) {
            require(_value == 0, "Sale: ETH sent must be 0");
            ...
            if(_section == TRANSFER_TYPE_ETH) {
                ...
                require(_erc20SaleEnabled,                           "Sale: Token Sale is not enabled");
                checkPrice = _fullERC20Price;
    ...
    function buyWithERC20(uint256 amount, IERC20 _erc20Token, bytes memory userData) external {
        require(address(_erc20Token) == _erc20tokenAddress ,"Sale: Invalid Token");
        require(_erc20Token.allowance(msg.sender,address(this))>= amount,"Sale: Insufficient amount approved");
        require(_erc20Token.balanceOf(msg.sender) >= amount,"Sale: buyer has insufficient funds");
        require(_erc20Token.transferFrom(msg.sender,address(this),amount),"Sale: token transfer failed");
        ...
        if(buyType == BUY_TYPE_APSALE) {
            ...
            require(payload.receiver == msg.sender, "Payload Verify: Invalid receiver");
            ...
            emit ApprovedTokenPayloadSale(msg.sender, msg.sender, numberOfCards, amount);

        } else if(buyType == BUY_TYPE_SALE) {
            _internalMint(numberOfCards, msg.sender, amount, TRANSFER_TYPE_ERC20);
            emit TokenSale(msg.sender, msg.sender, numberOfCards, amount);
        }
    }
    ...
    function _mintCards(uint256 numberOfCards, address recipient) internal {
        ...
        // if this is a membership card contract, minter needs to hold GALAXIS tokens
        if(isMembership) {
            IERC20 galaxisToken = IERC20(galaxisRegistry.getRegistryAddress("GALAXIS_TOKEN"));
            uint256 requiredBalance = galaxisRegistry.getRegistryUINT("MEMBERSHIP_MINT_GALAXIS_AMOUNT");
            require(galaxisToken.balanceOf(recipient) >= requiredBalance, "Sale: Token Balance insufficient for membership mint");
        }
    ...
    function verify_payload_rules(SaleSignedPayload memory _payload, uint256 _value, uint256 _numberOfCards, uint8 _section) internal view {
        ...
        if(_payload.free) {
            ...
            if(_section == TRANSFER_TYPE_ETH) {
                ...
                checkPrice = _payload.erc20_price;
            }
    ...
    function _split(uint256 amount, uint8 transferType) internal {
        ...
        for (uint256 j = 0; j < _wallets.length; j++) {
            ...
            if(transferType == TRANSFER_TYPE_ETH) {
                ...
                sent = IERC20(_erc20tokenAddress).transfer(_wallets[j], _amount);
