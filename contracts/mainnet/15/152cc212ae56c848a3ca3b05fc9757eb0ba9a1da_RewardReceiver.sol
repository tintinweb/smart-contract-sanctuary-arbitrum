...
contract RewardReceiver is IRewardReceiver, UpgradeableGovernable {
    ...
    function receiveRewards(uint256 _rewards0, uint256 _rewards1) external override {
        ...
        if (_rewards0 > 0) {
            IManager(msg.sender).token0().transferFrom(msg.sender, address(this), _rewards0);
            ...
            for (uint256 i; i < length;) {
                ...
                if (i < length - 1) {
                    ...
                    sumOfRewards = sumOfRewards + trackerRewards;
                } else {
                    trackerRewards = _rewards0 - sumOfRewards;
                    sumOfRewards = 0;
                }
        ...
        if (_rewards1 > 0) {
            IManager(msg.sender).token1().transferFrom(msg.sender, address(this), _rewards1);
            ...
            for (uint256 i; i < length;) {
                ...
                if (i < length - 1) {
                    ...
                    trackerRewards = _rewards1 - sumOfRewards;
                    sumOfRewards = 0;
                }
    ...
    function distributeRewards(address _manager) external override returns (uint256, uint256) {
        ...
        if (rewards0 > 0) {
            data.rewardPool0 = 0;
            IManager(_manager).token0().transfer(msg.sender, rewards0);
        }
        ...
        if (rewards1 > 0) {
            data.rewardPool1 = 0;
            IManager(_manager).token1().transfer(msg.sender, rewards1);
        }
    ...
    function removeTracker(address _manager, uint256 _index) external onlyGovernorOrFactory {
        ...
        if (length != 1 && (length - 1 != _index)) {
            trackers[_manager][_index] = trackers[_manager][length - 1];
        }
