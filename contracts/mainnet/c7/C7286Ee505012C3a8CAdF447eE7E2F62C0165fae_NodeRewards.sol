...
/**
 * @title Node Rewards Contract
 * @author GAST-TEAM
 * @notice In this contract user can harvest Node Rewards
 */
contract NodeRewards is AccessControlEnumerable, ReentrancyGuard {
    ...
    event SetAddrs(address gastAddr);
    event AddRewards(address[] users, uint256[] amounts, uint256 totalAmount);
    event HarvestToken(address user, uint256 amount);
     ...
     * @dev Set Addrs
     */
    function setAddrs(address gastAddr) external onlyRole(MANAGER_ROLE) {
        require(gastAddr != address(0), "Addrs cannot be empty");

        GAST = IERC20(gastAddr);

        emit SetAddrs(gastAddr);
    }
