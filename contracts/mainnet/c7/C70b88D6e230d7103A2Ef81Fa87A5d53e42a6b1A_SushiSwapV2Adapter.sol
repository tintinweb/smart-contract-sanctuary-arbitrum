...
contract SushiSwapV2Adapter is SushiSwapV2Swap, OwnableUpgradeable {
    
    function initialize(address _defiRouter, address _weth, uint256 _liquiditySlippage, uint256 _swapSlippage) external initializer  {
        defiRouter = _defiRouter;
        weth = _weth;
        liquiditySlippage = _liquiditySlippage;
        swapSlippage = _swapSlippage;
        __Ownable_init();
    }

    function setLiquiditySlippage(uint256 _num) external onlyOwner {
        liquiditySlippage = _num;
    }

    function setSwapSlippage(uint256 _num) external onlyOwner {
        swapSlippage = _num;
    }
    ...
    function removeLiquidity(
        ...
        (uint256 tokenAAmount, uint256 tokenBAmount) = computedLP(liquidity, tokenA, tokenB, defiLP);
...
contract SushiSwapV2Swap {
    ...
    function swap(address tokenA, address tokenB, uint256 tokenAAmount) internal returns (uint256[] memory amounts) {
    ...
    function wethSwap(address tokenA, address tokenB, uint256 tokenAAmount) internal returns (uint256[] memory amounts) {
        TransferHelper.safeApprove(tokenA, defiRouter, tokenAAmount);
        ...
        amounts = ISushiSwapRouter02(defiRouter).swapExactTokensForTokens(
            tokenAAmount,
            FullMath.mulDiv(paramAmounts[2], swapSlippage, Constants.DENOMINATOR),
            path,
            address(this),
            block.timestamp + 5 minutes
        );
    ...
    function getAmountsOut(uint256 amountIn, address tokenA, address tokenB) public view returns (uint) {
        address[] memory path = new address[](2);
        path[0] = tokenA;
        path[1] = tokenB;
    ...
    function getWethAmountsOut(uint256 amountIn, address tokenA, address tokenB) public view returns (uint) {
        uint256 wethAmount = getAmountsOut(amountIn, tokenA, weth);
        if (wethAmount == 0) {
            return 0;
        }
        address[] memory path = new address[](3);
