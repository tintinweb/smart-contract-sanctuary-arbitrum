...
contract AnimaSpender is EpochConfigurable, ITokenSpender {
  IAnima private ANIMA;

  // epoch => deltas
  mapping(uint => uint) public EPOCH_SPENDING_STORAGE;
  // bucket => epoch => deltas
  mapping(uint => mapping(uint => uint)) public BUCKET_EPOCH_SPENDING_STORAGE;
  ...
  function getEpochValue(uint _epoch) external view returns (uint) {
    return EPOCH_SPENDING_STORAGE[_epoch];
  }
  ...
  function getEpochValueBatch(
    ...
    for (uint i = startEpoch; i <= endEpoch; i++) {
      result[i - startEpoch] = EPOCH_SPENDING_STORAGE[i];
    }
  }
  ...
  function getBucketEpochValueBatch(
    ...
    result = new uint[](_endEpoch - _startEpoch + 1);
    for (uint i = _startEpoch; i <= _endEpoch; i++) {
      result[i - _startEpoch] = BUCKET_EPOCH_SPENDING_STORAGE[_bucket][i];
    }
  }
  ...
  function getEpochValueBatchTotal(
    ...
    require(startEpoch <= endEpoch, "AnimaSpender: INVALID_EPOCH_RANGE");

    for (uint i = startEpoch; i <= endEpoch; i++) {
      result += EPOCH_SPENDING_STORAGE[i];
    }
  }
  ...
  function getBucketEpochValueBatchTotal(
    ...
    require(_startEpoch <= _endEpoch, "AnimaSpender: INVALID_EPOCH_RANGE");
    for (uint i = _startEpoch; i <= _endEpoch; i++) {
      result += BUCKET_EPOCH_SPENDING_STORAGE[_bucket][i];
    }
  }
  ...
  function spend(
    ...
    uint _amount,
    uint _bucket
  ) external whenNotPaused onlyTokenSpender {
    uint epoch = currentEpoch();
    ANIMA.burnFrom(_owner, _amount);
    EPOCH_SPENDING_STORAGE[epoch] += _amount;
    BUCKET_EPOCH_SPENDING_STORAGE[_bucket][epoch] += _amount;
  }
  ...
  function forceUpdateEpochSpending(
    uint _epoch,
    ...
    for (uint i = 0; i < _amounts.length; i++) {
      ...
      BUCKET_EPOCH_SPENDING_STORAGE[_buckets[i]][_epoch] = _amounts[i];
    }
    EPOCH_SPENDING_STORAGE[_epoch] = totalAmount;
  }
}
