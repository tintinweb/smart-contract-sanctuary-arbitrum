...
contract RangePositionManager {
    ...
    // 1 tick = 0.01% in price movement (1 basis point = 0.01%, 50 basis points = 0.5%, 100 basis points = 1%)
    int24 public tickMovement;
    ...
    function canMoveRange() public view returns (bool) {
        ...
        if (checkMoveRangeDisabled == true) {
            return true;
        }
    ...
    external
    {
        require(moveRangeAddresses[msg.sender] == true, "not allowed to move range");
        ...
        console.log(params.amount0);
        console.log(params.amount1);
        ...
        // case if input is 0
            ...
            returnFromSwap = (tokenForRatios == token0) ? params.amount0 : params.amount1;
        }
        ...
        console.log(amount0Before - ((returnFromSwap - amountToSwap) * slippagePercent / 10000)  );
        console.log(amount0After);
    ...
    function mint(
        ...
        if (contractCall) {
            ...
            console.log("mint check beforeafter");
            console.log("amount0 desire: %s", mintParams.amount0Desired);
            console.log("amount1 desire: %s", mintParams.amount1Desired);
            //console.log("amount0min desire: %s", mintParams.amount0Min);
            //console.log("amount1min desire: %s", mintParams.amount1Min);
            console.log("minter value check: %s", token0 == WETH ? mintParams.amount0Desired : (token1 == WETH ? mintParams.amount1Desired : 0));
...
//                console.log("token0Balance: %s", token0Balance);
        ...
        // user is owner
            ...
            if (token0 == WETH && (token0Balance > 0)) {
                payable(feeReceiver).transfer(token0Balance);
            }
            if (token1 == WETH && (token1Balance > 0)) {
                payable(feeReceiver).transfer(token1Balance);
            }
            if (token0 != WETH && token0Balance > 0) {
                IERC20(token0).safeTransfer(feeReceiver, token0Balance);

            }
            if (token1 != WETH && token1Balance > 0) {
                IERC20(token1).safeTransfer(feeReceiver, token1Balance);
            }
        }
    ...
    /// sets the new fee receiver
    /// @dev sets the value of the fee receiver
    /// @param _newFeeReceiver the new value for _newFeeReceiver
    function changeFeeReceiver(address _newFeeReceiver) external {
        require(msg.sender == feeReceiver, "not fee receiver");
        feeReceiver = _newFeeReceiver;
        emit NewFeeReceiver(feeReceiver);
    }
