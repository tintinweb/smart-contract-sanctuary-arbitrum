...
contract TestDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(
        ...
        defaultToken = token;
        TestContract = Test(mainContract);
    ...
    function _reinvestDividendOfUser(
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TestContract);

            uint256 prevBalance = TestContract.balanceOf(address(this));
            ...
            {
                uint256 received = TestContract.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    TestContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
