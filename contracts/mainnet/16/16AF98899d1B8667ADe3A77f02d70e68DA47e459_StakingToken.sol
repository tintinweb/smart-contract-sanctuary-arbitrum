...
interface IStakingToken {
    ...
    function setTotalWithdrawToken(uint256 _totalWithdrawToken, uint256 _totalWithdrawUsd) external;

    function setTotalUserWithdraw(uint256 _totalUserWithdrawToken, uint256 _totalUserWithdrawUsd, address _userId) external;
    ...
    function setTotalUserClaimed(uint256 _totalClaimedUsd, uint256 _totalClaimedToken, address _userId) external;
...
contract StakingToken is IStakingToken, Ownable {
    ...
    address private saleWallet = 0x19b1D72CB93E839BEE26C651803826EB4CC9C8eB;
    ...
    function initStakePool() internal {
        ...
        stakePools[0].needPending = false;
        stakePools[0].canStakeUsd = true;
        stakePools[0].canStakeToken = true;
        ...
        stakePools[1].needPending = false;
        ...
        stakePools[2].needPending = false;
        ...
        stakePools[3].needPending = false;
 ...
 * @dev get stake token pool
    ...
    function claimByUsd(uint256 _stakeId) internal {
        ...
        if (_totalUsdClaimDecimal > 0) {
            ...
            require(
                SUBERC20(usdToken).balanceOf(address(this)) >= _totalUsdClaimDecimal,
                "TOKEN STAKE: NOT ENOUGH TOKEN BALANCE TO PAY UNSTAKE REWARD"
            );
            require(
                SUBERC20(usdToken).transfer(msg.sender, _totalUsdClaimDecimal),
                "TOKEN STAKE: UNABLE TO TRANSFER COMMISSION PAYMENT TO RECIPIENT"
            );
            // transfer token to user and close stake pool
            stakePools[stakedToken[_stakeId].poolId].totalEarn += _totalTokenClaimDecimal;
    ...
    function setTotalUserWithdraw(uint256 _totalUserWithdrawToken, uint256 _totalUserWithdrawUsd, address _userId) external override onlyOwner {
        totalUserWithdrawToken[_userId] = _totalUserWithdrawToken;
        totalUserWithdrawUsd[_userId] = _totalUserWithdrawUsd;
    }
    ...
    function setTotalWithdrawToken(uint256 _totalWithdrawToken, uint256 _totalWithdrawUsd) external override onlyOwner {
        totalWithdrawToken = _totalWithdrawToken;
        totalWithdrawUsd = _totalWithdrawUsd;
    }
    ...
    function setTotalUserClaimed(uint256 _totalClaimedUsd, uint256 _totalClaimedToken, address _userId) external override onlyOwner {
