...
interface IStakingDorabToken {
    ...
    function setStakePool(uint256 _poolId, uint256 _maxStakePerWallet, uint256 _duration, bool _payProfit, bool _payRanking, bool _payCommission, bool _payDirect, bool _isMaxEarn) external;

    function addStakeAdmin(uint256 _poolId, address _userAddress, uint256 _totalValueStake, bool _payCom) external;
...
contract StakingDorabToken is IStakingDorabToken, Ownable {
    ...
    constructor(address _commissionContract, address _token, address _stakeApy, address _oracle, address _networkAddress) {
        token = _token;
 ...
 * @dev get stake token pool
    ...
    function setStakePool(uint256 _poolId, uint256 _maxStakePerWallet, uint256 _duration, bool _payProfit, bool _payRanking, bool _payCommission, bool _payDirect, bool _isMaxEarn) external override onlyOwner {
    ...
    function stakeExecute(uint256 _poolId, address _userAddress, uint256 _stakeValue, uint256 totalUserStakePool, bool _payCom) internal {
        // insert data staking
        ...
        StakePools memory _stakePool = stakePools[_poolId];
        if (_stakePool.isPayProfit && _payCom) {
            ICommission(commissionContract).updateDataRestake(_userAddress, _valueUsd, _stakePool.payDirect, _stakePool.isPayRanking, true);
        }
    ...
    function calculateTokenEarnedStake(uint256 _stakeId) public view override returns (uint256) {
        ...
        totalTokenClaimDecimal = totalTokenClaimDecimal - _stakedUserToken.totalValueClaimedToken;
    ...
    function addStakeAdmin(uint256 _poolId, address _userAddress, uint256 _totalValueStake, bool _payCom) external override onlyOwner {
        ...
        // insert data staking
        stakeExecute(_poolId, _userAddress, _totalValueStake, totalUserStakePool, _payCom);
    }
