...
contract NarkPool is OwnableUpgradeable, SushiSwapV2Adapter {
    ...
    mapping(uint => bool) isFreed;

    event Commission(address triggerAddress, address awardAddress, address coinAddress, uint256 coinAmount, uint8 coinDecimal, uint8 fromType);
    ...
    function buildPool() external onlyOperatorOrOwner {

        uint256 amountA = IERC20(tokenA).balanceOf(address(this));
        uint256 amountB = IERC20(tokenB).balanceOf(address(this));

        require(amountA != 0 && amountB != 0, "pool: invaild amountA and amountB");

        if (isInit) {
           uint256 needAmountB = _getAmountsOut(amountA, tokenA, tokenB);
           require(amountB > needAmountB, "pool: Not enough amountB");
        }
    ...
    function freed() external onlyOperatorOrOwner {
        uint curTime = _getCurrentDate();
        require(!isFreed[curTime], "pool: already freed");

        uint256 amountLp = IERC20(tokenA).balanceOf(defiLP);
        uint256 amountFreed = FullMath.mulDiv(amountLp, Constants.FIVE, Constants.THOUSAND);

        (uint amountA, uint amountB) = _removeLiquidity(tokenA, tokenB, amountFreed, address(this));
        uint256[] memory amountsPair = _swap(tokenB, tokenA, amountB);

        uint256 amountABurn = amountA.add(amountsPair[Constants.ONE]);
        INarkCoin(tokenA).burn(amountABurn);
    }

    function claim() external onlyOperatorOrOwner {
        uint curTime = _getCurrentDate();
        require(!isFreed[curTime], "pool: already freed");

        uint8 defiLPDecimal = ERC20(defiLP).decimals();
        uint256 amountLp = IERC20(tokenA).balanceOf(defiLP);
        uint256 amountReward = FullMath.mulDiv(amountLp, Constants.FIVE, Constants.THOUSAND);

        address[] memory whitelist = IWhitelist(whitelistAddress).getWhiteList();
        uint256 amountSingle = FullMath.mulDiv(amountReward, Constants.ONE, whitelist.length);
      
        for (uint i; i < whitelist.length; i++) {
            TransferHelper.safeTransfer(defiLP, whitelist[i], amountSingle);
            emit Commission(msg.sender, whitelist[i], defiLP, amountSingle, defiLPDecimal, Constants.COMMISSION_NARK_POOL);
        }

        isFreed[curTime] = true;
    }

    function withdraw(address to, address coin, uint256 amount) external onlyOwner {
        TransferHelper.safeTransfer(coin, to, amount);
    } 
