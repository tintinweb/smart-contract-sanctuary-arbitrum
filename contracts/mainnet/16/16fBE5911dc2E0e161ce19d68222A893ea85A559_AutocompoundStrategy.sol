...
contract AutocompoundStrategy is Ownable { 
  ...
  IRewardRouterV2 public constant GlpRewardRouterV2 = IRewardRouterV2(0xA906F338CB21815cBc4Bc87ace9e68c87eF8d8F1);
  ...
  uint256 public glpMintLockTime = 900; // in seconds
  ...
  function purchaseGlp() public {
    ...
    if(wethBalance == 0 || wethBalance < sensibleMinimumWethToCompound) {
  ...
  function sendGlpToVault() public {
    ...
    // GLP is only transferable 15min+ after last mint
    if(lastPurchaseTimestamp >= block.timestamp - glpMintLockTime) {
      return; // we can't transfer GLP yet and trying would revert
    }
  ...
  function setGlpMintLockTime(uint256 _newTimeInSeconds) public onlyOwner {
    glpMintLockTime = _newTimeInSeconds;
    emit MintLockTimeUpdated(msg.sender, glpMintLockTime);
  }
