...
contract Naruto is ERC20, Ownable {
    ...
    NarutoDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Naruto", "NARUTO") {
        ...
        dividendTracker = new NarutoDividendTracker(
            payable(this),
            router,
...
contract NarutoDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        NarutoContract = Naruto(mainContract);
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "Naruto_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Naruto_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
    ...
    function _reinvestDividendOfUser(
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(NarutoContract);

            uint256 prevBalance = NarutoContract.balanceOf(address(this));
            ...
            {
                uint256 received = NarutoContract.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    NarutoContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
