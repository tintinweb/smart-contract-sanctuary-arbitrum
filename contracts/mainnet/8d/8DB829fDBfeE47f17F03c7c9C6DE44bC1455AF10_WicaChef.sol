...
interface IWicaToken {
    ...
    function fragmentToWica(uint256 value) external view returns (uint256);

    function wicaToFragment(uint256 wica) external view returns (uint256);
...
contract WicaChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //   1. The pool's `accRewardPerShare` (and `lastRewardBlockTimestamp`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlockTimestamp; // Last block number that Rewards distribution occurs.
        uint256 accRewardPerShare; // Accumulated Rewards per share.
    }


    // WICA
    IWicaToken public wica;
    // WICA LP address
    IUniswapPair public wicaLp;
    // Uniswap V2 Router
    IUniswapV2Router public router;
    // WICA tokens reward per block.
    ...
    uint256 public constant epochPeriod = 12 seconds;
    ...
    // The block number when WICA mining starts.
    ...
    event LogRewardPerEpoch(uint256 amount);
    ...
    constructor(
        ...
        lastBlockTimestamp = _startBlockTimestamp;
    }
    ...
    function updateRewardPerEpoch(uint256 _rewardPerEpoch) external onlyOwner {
        massUpdatePools();
        rewardPerEpoch = _rewardPerEpoch;
        emit LogRewardPerEpoch(_rewardPerEpoch);
    }
    ...
    function add(
        ...
        uint256 lastRewardBlockTimestamp = block.timestamp > startBlockTimestamp
            ? block.timestamp
            : startBlockTimestamp;
    ...
    // Update the given pool's WICA allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending Wica on frontend.
        ...
        returns (uint256)
    {
        ...
        if (address(pool.lpToken) == address(wica)) {
            lpSupply = wica.balanceOfUnderlying(address(this));
        }
        if (block.timestamp > pool.lastRewardBlockTimestamp && lpSupply != 0) {
            uint256 wicaReward = ((block.timestamp - pool.lastRewardBlockTimestamp) / epochPeriod *
                rewardPerEpoch *
                pool.allocPoint) / totalAllocPoint;
            accRewardPerShare += (wicaReward * 1e12) / lpSupply;
        }
        return
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 wicaReward = ((block.timestamp - pool.lastRewardBlockTimestamp) / epochPeriod *
            rewardPerEpoch *
            pool.allocPoint) / totalAllocPoint;
        pool.accRewardPerShare += (wicaReward * 1e12) / lpSupply;
        pool.lastRewardBlockTimestamp = block.timestamp;
    ...
    // Deposit tokens to WicaChef for WICA allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _account
        ...
        if (address(pool.lpToken) == address(wica)) {
            _amount = wica.fragmentToWica(_amount);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
        if (address(pool.lpToken) == address(wica)) {
            _amount = wica.wicaToFragment(_amount);
        }
        ...
        returns (uint256)
    {
        ...
        if (lastBlockTimestamp + epochPeriod < block.timestamp) {
            wica.rebase(
                block.timestamp,
                compound(1e18, compoundRatio, (block.timestamp - lastBlockTimestamp) / epochPeriod) - 1e18,
                false
            );
    ...
    function compoundBig(
        ...
        IERC20(address(wica)).safeTransferFrom(
            msg.sender,
            address(this),
            _amountTokenDesired
        );
        IERC20(address(wica)).approve(address(router), _amountTokenDesired);
        ...
        IERC20(address(wica)).safeTransfer(
            msg.sender,
