...
// File: contracts/strategies/StabilizeStrategyGLPV4.sol

pragma solidity =0.6.6;

// This is a strategy is an absoluste stake strategy, fsGLP passively earns WETH, MPs + esGMX
// esGMX is persistently staked to earn more esGMX, MPs and WETH
...
contract StabilizeStrategyGLPV4 is Ownable {
    using SafeMath for uint256;
    ...
    address public gmxMigrator; // Address provided from GMX that allows movement of funds from old reward router
    ...
    address constant OLD_GMX_REWARD_ROUTER = address(0xA906F338CB21815cBc4Bc87ace9e68c87eF8d8F1);
    address constant GMX_REWARD_ROUTER = address(0x159854e14A862Df9E39E1D128b8e5F70B4A3cE9B);
    address constant GLP_REWARD_ROUTER = address(0xB95DB5B167D75e6d04227CfFFA61069348d271F5);
    ...
    */
    ...
    function redeemStakeProfit() internal {
        ...
        GMXRewardRouter(GMX_REWARD_ROUTER).handleRewards(false,false,false,false,true,false,false); // Will stake MPs
        ...
        if(_bal > 0){
            ...
            address glpManager = GMXRewardRouter(GLP_REWARD_ROUTER).glpManager();
            ...
            GMXRewardRouter(GLP_REWARD_ROUTER).mintAndStakeGlp(targetToken, sellAmount, 1, 1);
        }
    }
    ...
    function governanceFinishMoveEsGMXFromDeprecatedRouter(address _sender) external {
        // Obtain tokens from another address
        require(_msgSender() == gmxMigrator || _msgSender() == governance(), "Not authorized caller");
        GMXRewardRouter(OLD_GMX_REWARD_ROUTER).acceptTransfer(_sender); // Used to receive esGMX
    }

    function governanceSetGmxMigrator(address _migrator) external onlyGovernance {
        gmxMigrator = _migrator;
    }
