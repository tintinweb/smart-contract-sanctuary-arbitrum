...
contract OracleManager is IOracleManager, NameVersion, Admin {
    ...
    function getValueWithHistory(bytes32 symbolId) external returns (uint256 curValue, uint256 preTimestamp, uint256 preValue) {
        address oracle = _oracles[symbolId];
        require(oracle != address(0), 'OracleManager.getValueWithHistory: no oracle');
        return IOracle(oracle).getValueWithHistory();
    }
...
interface IOracle is INameVersion {
    ...
    function getValueWithHistory() external returns (uint256 curValue, uint256 preTimestamp, uint256 preValue);
