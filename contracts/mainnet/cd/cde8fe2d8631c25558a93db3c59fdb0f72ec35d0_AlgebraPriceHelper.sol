...
contract AlgebraPriceHelper is IPriceHelper, UpgradeableGovernable {
    ...
    {
        return AlgebraLiquidityLib.amountsForLiquidity(pool, tickLower, tickUpper, liquidity);
    }
    ...
    function liquidityForAmounts(address pool, int24 tickLower, int24 tickUpper, uint256 amount0, uint256 amount1)
        ...
        returns (uint128)
    {
        return AlgebraLiquidityLib.liquidityForAmounts(pool, tickLower, tickUpper, amount0, amount1);
    }
    ...
    function amountsForAmount0(address pool, int24 tickLower, int24 tickUpper, uint256 amount0)
        ...
        returns (uint256, uint256)
    {
        return AlgebraLiquidityLib.amountsForAmount0(pool, tickLower, tickUpper, amount0);
    }
    ...
    function amountsForAmount1(address pool, int24 tickLower, int24 tickUpper, uint256 amount1)
        ...
        returns (uint256, uint256)
    {
        return AlgebraLiquidityLib.amountsForAmount1(pool, tickLower, tickUpper, amount1);
    }
    ...
    function getPrices(address pool) external view override returns (uint256 price0, uint256 price1) {
        ...
        if (_data.referenceToken == IAlgebraPool(pool).token1()) {
            //  token1 decimals * oracle decimals
            price0 = spotPrice * oraclePrice;
            uint256 decimals = IERC20Metadata(IAlgebraPool(pool).token1()).decimals() + oracleDecimals;
            if (decimals < 18) {
            ...
            uint256 decimals = 48 - IERC20Metadata(IAlgebraPool(pool).token1()).decimals();
    ...
    function checkManipulation(address pool) external view override returns (uint256 spot) {
        ...
        uint256 mean = AlgebraLibrary.getPrice(pool, _data.secondsAgo);
    ...
    function _oraclePrice(PoolData memory _data) private view returns (uint256) {
        ...
        require(block.timestamp <= updatedAt + _data.stalePeriod, "Stale Price Update");
