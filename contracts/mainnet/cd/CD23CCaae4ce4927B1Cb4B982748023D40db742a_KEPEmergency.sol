...
interface IKEPVault {
  ...
  function deposit(KEPTypes.DepositParams memory dp) external payable;
  function depositNative(KEPTypes.DepositParams memory dp) external payable;

  function withdraw(KEPTypes.WithdrawParams memory wp) external payable;
  ...
  function compound(KEPTypes.CompoundParams memory cp) external payable;
  ...
  function updateChainlinkOracleWNT(address addr) external;
...
library KEPEmergency {
  ...
  */
  function emergencyRepay(
    ...
    // Swap ETH for tokenB
    uint256 _wntAmtInSwapped = KEPManager.swapTokensForExactTokens(self, _sp);
...
library KEPManager {
  ...
  */
  ...
  //   * @param self KEPTypes.Store
  //   * @param alp KEPTypes.AddLiquidityParams
  //   * @return depositKey
  // */
  // function addLiquidity(
  //   KEPTypes.Store storage self,
  //   KEPTypes.AddLiquidityParams memory alp
  // ) public returns (bytes32) {
  //   return KEPWorker.addLiquidity(self, alp);
  // }

  // /**
  //   * @notice Remove liquidity from yield source
  //   * @param self KEPTypes.Store
  //   * @param rlp KEPTypes.RemoveLiquidityParams
  //   * @return withdrawKey
  // */
  // function removeLiquidity(
  //   KEPTypes.Store storage self,
  //   KEPTypes.RemoveLiquidityParams memory rlp
  // ) public returns (bytes32) {
  //   return KEPWorker.removeLiquidity(self, rlp);
  // }
...
library KEPReader {
  ...
  */
  function assetValue(KEPTypes.Store storage self) public view returns (uint256) {
    (int256 _ratio, uint8 _decimals) = self.chainlinkOracleWNT.consult(address(self.LRT));

    return convertToUsdValue(
      self,
      address(self.WNT),
      lrtAmt(self) * uint256(_ratio) / uint256(_decimals)
    );
  }
  ...
  */
  function delta(KEPTypes.Store storage self) public view returns (int256) {
    // TODO: to check if this function really needed

    (int256 _ratio, uint8 _decimals) = self.chainlinkOracleWNT.consult(address(self.LRT));

    uint256 _lrtAmt = lrtAmt(self)
        * uint256(_ratio)
        / uint256(_decimals);
    uint256 _tokenBDebtAmt = debtAmt(self);
    uint256 equityValue_ = equityValue(self);
...
// import { IDepositPool } from "../../interfaces/protocols/kep/IDepositPool.sol";
import { ISwap } from "../../interfaces/swap/ISwap.sol";
...
library KEPTypes {
  ...
  struct Store {
    ...
    IChainlinkOracle chainlinkOracleWNT;

    // LRT native restaking contract address
