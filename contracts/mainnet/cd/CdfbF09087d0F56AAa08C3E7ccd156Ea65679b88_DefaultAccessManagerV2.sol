...
import "./DCAStrategyManagerV2.sol";
import "./DCATypes.sol";

contract DefaultAccessManagerV2 is IAccessManager {
    enum AccessResult {
        OK,
    ...
    DCAStrategyManagerV2 dcaStrategyManager;

    constructor(address dcaStrategyManager_) {
        dcaStrategyManager = DCAStrategyManagerV2(dcaStrategyManager_);
    }
    ...
    function hasAccess(
        ...
        DCATypes.StrategyDataV2 memory strategyData = dcaStrategyManager
...
contract DCAStrategyManagerV2 is Ownable, AccessControl {
    ...
    function _collectFromAsset(
        ...
        while (participantsIndex >= 0 && maxLoopIterations > 0) {
                ...
                )
            {
                ...
                uint256 fee = IFeeManager(strategyDataV2.feeManager).calculateFee(
                    strategyId,
                    msg.sender,
                    toCollect
                );
    ...
    function _distributeTargetAsset(
        ...
        if (strategyDataV2.executionData.received > 0) {
            while (
                ...
                if (toSend > 0) {
                    IERC20(strategyDataV2.path[strategyDataV2.path.length - 1]).transfer(participant, toSend);
                }
    ...
    function executeDCA(
        ...
        if (
            ...
            if (strategyDataV2.executionData.totalCollectedToExchange > 0) {
                ...
                uint256 amountIn = strategyDataV2.executionData.totalCollectedToExchange;
                uint256 amountOutMin = (strategyDataV2
                            .executionData
                            .totalCollectedToExchange * beliefPrice) /
                            DENOMINATOR;
                uint256[] memory amounts = uniswapV2Router.swapExactTokensForTokens(amountIn, amountOutMin, strategyDataV2.path, address(this), block.timestamp);
                received = amounts[amounts.length-1];
                uint256 toAssetBalance = IERC20(strategyDataV2.path[strategyDataV2.path.length-1]).balanceOf(address(this));
