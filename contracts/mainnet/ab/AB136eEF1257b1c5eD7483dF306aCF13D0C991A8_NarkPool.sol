...
interface IWhitelist {
    ...
    function whiteAlready(address user) external view returns (bool);
}
...
contract NarkPool is OwnableUpgradeable, SushiSwapV2Adapter {
    ...
    function freed() external onlyOperatorOrOwner {
        ...
        uint256 amountSingle = FullMath.mulDiv(amountReward, Constants.ONE, Constants.THOUSAND);
      
        for (uint i; i < Constants.THOUSAND; i++) {
    ...
    function burn() external onlyOperatorOrOwner {

        uint256 amountLp = IERC20(defiLP).balanceOf(address(this));
        uint256 amount = FullMath.mulDiv(amountLp, Constants.FIVE, Constants.THOUSAND);

        (uint amountA, uint amountB) = _removeLiquidity(tokenA, tokenB, amount, address(this));
        uint256[] memory amountsPair = _swap(tokenB, tokenA, amountB);
    ...
    function airdrop() external onlyOperatorOrOwner {
        uint curTime = _getCurrentDate();
        require(!isFreed[curTime], "pool: already freed");
        ...
        address[] memory whitelist = IWhitelist(whitelistAddress).getWhiteList();
        uint256 amountSingle = FullMath.mulDiv(amount, Constants.ONE, whitelist.length);
    ...
    function dailyBurn() external onlyOperatorOrOwner {

        uint curTime = _getCurrentDate();

        if (dailyFreed[curTime] == 0) {
            uint256 amountLp = IERC20(defiLP).balanceOf(address(this));
            dailyFreed[curTime] = FullMath.mulDiv(amountLp, Constants.ONE, Constants.DENOMINATOR);
        }

        uint256 amount = FullMath.mulDiv(dailyFreed[curTime], Constants.ONE, Constants.TWO);
     ...
     function dailyAirdrop(uint start, uint end) external onlyOperatorOrOwner {
