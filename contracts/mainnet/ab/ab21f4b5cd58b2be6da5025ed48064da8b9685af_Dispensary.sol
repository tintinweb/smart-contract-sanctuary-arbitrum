...
// File contracts/inventory/Dispensary.sol
interface IGenericToken {
    function addToWhitelist(address) external;
    function mint(address, uint256, uint256) external;
    function whitelistEnabled() external returns(bool);
    function whitelistBalance(address) external returns(uint256);
}
 ...
 * @custom:security-contact Telegram: zeroXshogun
 */
contract Dispensary is ERC1155,
                       Permissions
{
    using Counters for Counters.Counter;

    IGenericToken public bugs;
    ...
    /// Mapping of claimable tokens.
    /// @dev id => address => amount
     ...
     * Deploy contract.
     */
    constructor(address _bugs) ERC1155("") {
        bugs = IGenericToken(_bugs);
    }
    ...
    receive() external payable {
        // Genesis whitelist.
        if (bugs.whitelistEnabled()) {
            require(msg.value == 4200000000000000, "Send 0.0042 ETH");
            require(bugs.whitelistBalance(_msgSender()) == 0, "Already whitelisted");
            bugs.addToWhitelist(_msgSender());
            bugs.mint(_msgSender(), 0, 1000000000000000000000);
     ...
     */
    function addNewToken(string calldata name, string calldata uri)
        public
        onlyOwner
    {
        uint256 id = _currentId.current();
        tokens[id] = Token(id, name, uri);
     ...
     */
    function mint(address to, uint256 id, uint256 amount) external onlyApp {
        _mint(to, id, amount, "");
    }

    function setClaimableToken(address account, uint256 id, uint256 amount) external onlyApp {
        claimableTokens[id][account] = amount;
    }

    function claimToken(uint256 id) external {
        uint256 amount = claimableTokens[id][_msgSender()];
