...
contract PendleEzEth is Initializable, PendleGeneral, UUPSUpgradeable, AccessControlUpgradeable {
    using SafeERC20 for IERC20;
    ...
    function initialize(
        ...
        IERC20(mainInfo.underlyingAsset).forceApprove(UNISWAP_V3_ROUTER, type(uint256).max);
...
contract PendleGeneral is BaseStrategyPendle {
    ...
    function setPendleLogic(address _newPendleLogic) external onlyAuthorized {
        if(address(pendleLogic) != address(0)) {
            ...
            IERC20(ARB).forceApprove(address(pendleLogic), 0);
            IERC20(PENDLE).forceApprove(address(pendleLogic), 0);
        }
        pendleLogic = IPendleLogic(_newPendleLogic);
        IERC20(WETH).forceApprove(_newPendleLogic, type(uint256).max);
        IERC20(ARB).forceApprove(address(_newPendleLogic), type(uint256).max);
        IERC20(PENDLE).forceApprove(address(_newPendleLogic), type(uint256).max);
    }

    function setNewPendleRouter(address _newRouter)  external onlyAuthorized {
        IERC20(WETH).forceApprove(address(mainInfo.pendleRouter), 0);
        IERC20(mainInfo.underlyingAsset).forceApprove(address(mainInfo.pendleRouter), 0);
        IERC20(mainInfo.lpToken).forceApprove(address(mainInfo.pendleRouter), 0);
        IERC20(mainInfo.syToken).forceApprove(address(mainInfo.pendleRouter), 0);
        IERC20(mainInfo.ptToken).forceApprove(address(mainInfo.pendleRouter), 0);
        IERC20(mainInfo.ytToken).forceApprove(address(mainInfo.pendleRouter), 0);
    ...
    function _claimAndSellRewards() internal {
        ...
        (address[] memory stakingTokens, address[][] memory rewTokens) = pendleLogic.getRewardInfo();
