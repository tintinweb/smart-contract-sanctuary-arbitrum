...
import "../libraries/WithReward.sol";
    ...
    IDividendPayingToken
{
    ...
    /// Gets reward account information by index
    ...
    /// @return numInQueue The number of wallets left to process before this wallet
    /// @return withdrawableRewards The amount of rewards this account can withdraw
    /// @return totalRewards The total rewards this account has earned
    /// @return lastClaimTime The last time the account claimed rewards
    /// @return nextClaimTime The next time this account is eligible to claim rewards
    /// @return timeTillAutoClaim The number of seconds until this account is eligible for reward claims
    ...
    function updateRewardBalance(address account, int256 difference)
        public
        onlyRole(LibDiamond.VESTING_ROLE)
    {
        uint256 newBalance = SafeCast.toUint256(
            SafeCast.toInt256(rewardBalanceOf(account)) + difference
        );
        _setRewardBalance(account, newBalance);
    }
    ...
    function _processRewards() internal {
        ...
        while (gasUsed < gas && iterations < numHolders) {
            ...
            // skip if manual claim is enabled
            if (_rs().manualClaim[account]) return;

            // skip if cannot auto claim yet
            if (!_canAutoClaim(_rs().claimTimes[account])) return;
