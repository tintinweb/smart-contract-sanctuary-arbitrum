...
import "../token/interface/ITLD.sol";
import "../tool/interface/ISunswapV2Router02.sol";

/**
 * @title LP Staking Contract
 * @author TLD-TEAM
 * @notice In this contract user can stake LP and harvest TLD
 */
contract TLDPool is AccessControlEnumerable, ReentrancyGuard {
    ...
    uint256[7] public feeArr = [3000, 2000, 1500, 700, 700, 700, 700];
    ...
    event SetAddrs(address invitingAddr, address tldAddr);
    ...
    function setAddrs(
        ...
        TLD.approve(address(router), 2 ** 256 - 1);

        emit SetAddrs(invitingAddr, tldAddr);
    }
    ...
    function deposit(
        ...
        if (userStakes[user] > 0) {
            uint256 pendingToken = (userStakes[user] *
                (accTokenPerStake - userLastAccTokenPerStake[user])) / 1e18;
            if (pendingToken > 0) {
                userStoredToken[user] += pendingToken;
            }
        }
        ...
        userLastAccTokenPerStake[user] = accTokenPerStake;
    ...
    function withdraw(
        ...
        require(userStakes[user] >= amount, "Not enough token to withdraw");
    ...
    function harvest() external nonReentrant {
        ...
        TLD.mint(msg.sender, amount);
        TLD.mint(address(this), amount);

        sellTLD(amount);
        uint256 trxAmount = address(this).balance;

        address inviter = msg.sender;
        uint256 remainAmount = trxAmount;
        for (uint256 i = 0; i < 7; i++) {
            inviter = inviting.userInviter(inviter);
            uint256 transferAmount = (trxAmount * feeArr[i]) / 1e4;
    ...
    function updatePool() public {
        ...
        if (block.timestamp > lastRewardTime && stakes > 0) {
            uint256 amount = getTokenPerSecond() *
    ...
    /**
     * @dev Get Token Per Second
     */
    function getTokenPerSecond() public view returns (uint256) {
        uint256 pastDays = (block.timestamp - startTime) / 1 days;
        if (pastDays > 365) return 1446759259259259400;
        else if (pastDays > 182) return 2893518518518519000;
        else return 5787037037037038000;
    }

    function sellTLD(uint256 amount) internal {
 ...
 * @title TLD Interface
 * @author TLD-TEAM
 * @notice Interface of the TLD
 */
interface ITLD is IERC20 {
