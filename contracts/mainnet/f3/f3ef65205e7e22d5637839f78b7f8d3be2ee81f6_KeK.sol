...
contract KeK is ERC20, Ownable {
    ...
    KeKDividendTracker public dividendTracker;
    ...
    constructor() ERC20("KEK", "KEK") {
        marketingWallet = payable(0x4B3B6C1C324abf106E98156acF22FC15BBE6A2B3);
        devWallet = payable(0x257C3B3c229879aAed4a9062DB7f5A519B8166F8);
        ...
        dividendTracker = new KeKDividendTracker(
            payable(this),
            router,
...
contract KeKDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function _transfer(
        ...
        require(false, "KeK_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "KeK_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(KeKContract);

            uint256 prevBalance = KeKContract.balanceOf(address(this));
            ...
            {
                uint256 received = KeKContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    KeKContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
