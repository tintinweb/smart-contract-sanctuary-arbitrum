...
import "./interfaces/IUpshotConsumerV2.sol";
...
contract PriceFeed is Initializable, OwnableUpgradeable {
    ...
    IUpshotConsumerV2 public upshotConsumer;
    bytes32 public ucSpecId; // job spec id for floor price
    uint256 public ucPayment; // payment for executing job
    address public manualBot;
    address public oracleBot;
    mapping(bytes32 => string) public keyToAssetAddressMap; // price feed key => asset id
    ...
    mapping(bytes32 => PriceMode) public priceModes;
    ...
    event PriceModeUpdated(bytes32 indexed key, PriceMode indexed priceMode);
     ...
     */
    function initialize(
        IUpshotConsumerV2 _upshotConsumer,
        bytes32 _ucSpecId,
        uint256 _ucPayment
    ) public initializer {
        __Ownable_init();
        ...
        ucSpecId = _ucSpecId;
        ucPayment = _ucPayment;
    }

    function addPriceFeedKey(bytes32 _key, string calldata _assetAddress) public onlyOwner {
        keyToAssetAddressMap[_key] = _assetAddress;
    }

    function updatePriceOracle(bytes32 _key) public onlyOracleBot {
        require(priceFeedKeyExists(_key), "PriceFeed: invalid key");
        bytes32 requestId = upshotConsumer.requestStatisticsNumber(
            ucSpecId,
            ucPayment,
            keyToAssetAddressMap[_key]
        );
    ...
    function getPrice(bytes32 _key) public view returns (uint256) {
        require(priceFeedKeyExists(_key), "PriceFeed: invalid key");
        if (priceModes[_key] == PriceMode.Oracle) {
    ...
    function _getTwapOracle(bytes32 _key, uint256 interval) internal view returns (uint256) {
        ...
        while (currentTimestamp > baseTimestamp && index > 0) {
            ...
            price = upshotConsumer.requestIdStatisticsNumber(currentSnapshot.requestId);
            assert(price != 0);
        ...
        )
    {
        ...
        for (uint256 i = len - 1; i >= 0; ) {
            ...
            uint256 _price = upshotConsumer.requestIdStatisticsNumber(snapshot.requestId);
            if (_price != 0) {
    ...
    function setUpshotConsumer(IUpshotConsumerV2 _upshotConsumer) public onlyOwner {
        upshotConsumer = _upshotConsumer;
    }

    function priceFeedKeyExists(bytes32 _key) public view returns (bool) {
        return keccak256(bytes(keyToAssetAddressMap[_key])) != keccak256("");
    }
     ...
     * Set price mode
     */
    function setPriceMode(bytes32 _key, PriceMode _mode) public onlyOwner {
        priceModes[_key] = _mode;
        emit PriceModeUpdated(_key, _mode);
    }

    function setUcSpecId(bytes32 _specId) public onlyOwner {
        ucSpecId = _specId;
    }

    function setUcPayment(uint256 _payment) public onlyOwner {
        ucPayment = _payment;
    }
}
