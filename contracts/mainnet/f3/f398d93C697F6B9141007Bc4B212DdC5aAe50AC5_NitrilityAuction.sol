...
contract NitrilityAuction is Ownable(msg.sender), ReentrancyGuard {
    ...
    function acceptOffer(
        ...
        require(
            offerData.eventType == NitrilityCommon.EventTypes.OfferPlaced,
            "Invalid Sale Type"
        );

        uint256 offerPrice;
        if (offerData.accessLevel == NitrilityCommon.AccessLevel.NonExclusive) {
    ...
    // reject the offer
    function rejectOffer(uint256 offerId, bool isSeller) external {
        OfferData storage offer = idToOffer[offerId];
        // Check if the offer has the correct event type
        require(
            offer.eventType == NitrilityCommon.EventTypes.OfferPlaced,
            "Invalid Sale Type"
        );
        ...
        if (isSeller) {
            ...
            require(msg.sender == offer.buyerAddr, "Only bidder can call");
            eventType = NitrilityCommon.EventTypes.OfferRejected;
        }

        // Update the event type of the offer
        offer.eventType = eventType;
