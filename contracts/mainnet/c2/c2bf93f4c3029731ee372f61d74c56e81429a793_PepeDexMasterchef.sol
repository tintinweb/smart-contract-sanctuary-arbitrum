...
interface IStake {
  ...
  function safePepeDexTransfer(address _account, uint256 _amount) external;
}
...
interface IMasterChef {
  ...
  function pendingPepeDex(address _stakeToken, address _user) external view returns (uint256);
  ...
  function pepedexPerBlock() external view returns (uint256);
  ...
  function depositPepeDex(address _for, uint256 _amount) external;

  function withdrawPepeDex(address _for, uint256 _amount) external;
  ...
  function pepedex() external returns (IPEPEDEX);
}
...
// MasterChef is the master of PEPEDEX. He can make PEPEDEX and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once PEPEDEX is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract PepeDexMasterchef is IMasterChef, Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  ...
  struct PoolInfo {
    ...
    uint256 lastRewardBlock; // Last block number that PEPEDEX distribution occurs.
    uint256 accPepeDexPerShare; // Accumulated PEPEDEX per share, times 1e12. See below.
    uint256 depositFee;
  }

  // PEPEDEX token.
  IPEPEDEX public override pepedex;
  // Stake address.
  IStake public stake;
  // Dev address.
  ...
  // PEPEDEX per block.
  uint256 public override pepedexPerBlock;
  ...
  // The block number when PEPEDEX mining starts.
  ...
  event SetPepeDexPerBlock(uint256 prevPepeDexPerBlock, uint256 currentPepeDexPerBlock);
  ...
  constructor(
    ...
    devAddr = _devAddr;
    refAddr = _refAddr;
    pepedexPerBlock = _pepedexPerBlock;
    ...
    // add PEPEDEX pool
    pools.add(address(_pepedex));
    poolInfo[address(_pepedex)] = PoolInfo({
      allocPoint: 0,
      lastRewardBlock: startBlock,
      accPepeDexPerShare: 0,
      depositFee: 0
    });
    totalAllocPoint = 0;
  }
  ...
  // Set PEPEDEX per block.
  function setPepeDexPerBlock(uint256 _pepedexPerBlock) external onlyOwner {
    massUpdatePools();
    uint256 prevPepeDexPerBlock = pepedexPerBlock;
    pepedexPerBlock = _pepedexPerBlock;
    emit SetPepeDexPerBlock(prevPepeDexPerBlock, pepedexPerBlock);
  }
  ...
  function addPool(
    ...
    require(_depositFee <= 500, "add: max 5% fee");
  ...
  // Update the given pool's PEPEDEX allocation point. Can only be called by the owner.
  function setPool(
    address _stakeToken,
    uint256 _allocPoint,
    uint256 _depositFee
    ...
    require(_depositFee <= 500, "set: max 5% fee");
  ...
  function removePool(address _stakeToken) external override onlyOwner {
    require(_stakeToken != address(pepedex), "removePool: can't remove PEPEDEX pool");
    ...
    poolInfo[_stakeToken].accPepeDexPerShare = 0;
  ...
  // View function to see pending PEPEDEXs on frontend.
  function pendingPepeDex(address _stakeToken, address _user) external view override returns (uint256) {
    PoolInfo storage pool = poolInfo[_stakeToken];
    UserInfo storage user = userInfo[_stakeToken][_user];
    uint256 accPepeDexPerShare = pool.accPepeDexPerShare;
    ...
    if (block.number > pool.lastRewardBlock && totalStakeToken != 0) {
      ...
      uint256 pepedexReward = multiplier.mul(pepedexPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      accPepeDexPerShare = accPepeDexPerShare.add(pepedexReward.mul(1e12).div(totalStakeToken));
    }
    return user.amount.mul(accPepeDexPerShare).div(1e12).sub(user.rewardDebt);
  }
  ...
  function updatePool(address _stakeToken) public override {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 pepedexReward = multiplier.mul(pepedexPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    pepedex.mint(devAddr, pepedexReward.mul(devBps).div(10000));
    pepedex.mint(address(stake), pepedexReward.mul(refBps).div(10000));
    pepedex.mint(address(stake), pepedexReward);
    pool.accPepeDexPerShare = pool.accPepeDexPerShare.add(pepedexReward.mul(1e12).div(totalStakeToken));
    pool.lastRewardBlock = block.number;
  }

  // Deposit token to MasterChef for PEPEDEX allocation.
  function deposit(
    address _for,
    address _stakeToken,
    uint256 _amount
    ...
    require(_stakeToken != address(pepedex), "deposit: use depositPepeDex instead");
    ...
    user.rewardDebt = user.amount.mul(pool.accPepeDexPerShare).div(1e12);
  ...
  function withdraw(
    ...
    require(_stakeToken != address(pepedex), "withdraw: use withdrawPepeDex instead");
  ...
  // Deposit PEPEDEX to MasterChef.
  function depositPepeDex(address _for, uint256 _amount)
    external
    override
    onlyPermittedTokenFunder(_for, address(pepedex))
    nonReentrant
  {
    PoolInfo storage pool = poolInfo[address(pepedex)];
    UserInfo storage user = userInfo[address(pepedex)][_for];

    if (user.fundedBy != address(0)) require(user.fundedBy == _msgSender(), "depositPepeDex: bad sof");

    updatePool(address(pepedex));

    if (user.amount > 0) _harvest(_for, address(pepedex));
    if (user.fundedBy == address(0)) user.fundedBy = _msgSender();
    if (_amount > 0) {
      uint256 depositFeeAmount = _amount.mul(pool.depositFee).div(10000);
      if (depositFeeAmount > 0) {
        _amount = _amount.sub(depositFeeAmount);
        IERC20(address(pepedex)).safeTransferFrom(address(_msgSender()), devAddr, depositFeeAmount);
      }
      IERC20(address(pepedex)).safeTransferFrom(address(_msgSender()), address(this), _amount);
      user.amount = user.amount.add(_amount);
    }
    user.rewardDebt = user.amount.mul(pool.accPepeDexPerShare).div(1e12);
    emit Deposit(_msgSender(), _for, address(pepedex), _amount);
  }

  // Withdraw PEPEDEX
  function withdrawPepeDex(address _for, uint256 _amount) external override nonReentrant {
    PoolInfo storage pool = poolInfo[address(pepedex)];
    UserInfo storage user = userInfo[address(pepedex)][_for];

    require(user.fundedBy == _msgSender(), "withdrawPepeDex: only funder");
    require(user.amount >= _amount, "withdrawPepeDex: not good");

    updatePool(address(pepedex));
    _harvest(_for, address(pepedex));

    if (_amount > 0) {
      user.amount = user.amount.sub(_amount);
      IERC20(address(pepedex)).safeTransfer(address(_msgSender()), _amount);
    }
    user.rewardDebt = user.amount.mul(pool.accPepeDexPerShare).div(1e12);
    if (user.amount == 0) user.fundedBy = address(0);
    emit Withdraw(_msgSender(), _for, address(pepedex), user.amount);
  }

  // Harvest PEPEDEX earned from a specific pool.
  ...
  // Harvest PEPEDEX earned from pools.
  ...
  // Internal function to harvest PEPEDEX
  function _harvest(address _for, address _stakeToken) internal {
    ...
    uint256 pending = user.amount.mul(pool.accPepeDexPerShare).div(1e12).sub(user.rewardDebt);
    require(pending <= pepedex.balanceOf(address(stake)), "_harvest: wait what.. not enough PEPEDEX");
    stake.safePepeDexTransfer(_for, pending);
  ...
  function _referralCallee(address _for, uint256 _pending) internal {
    ...
    stake.safePepeDexTransfer(address(refAddr), _pending.mul(refBps).div(10000));
