...
import {IV3Pool} from "../interfaces/v3-pool/IV3Pool.sol";
import {IAlcorOptionPoolFactory} from "../interfaces/IAlcorOptionPoolFactory.sol";
import {IBaseAlcorOptionCore} from "../interfaces/IBaseAlcorOptionCore.sol";
...
import {VanillaOptionPool} from "../libraries/combo-pools/VanillaOptionPool.sol";
import {LPPosition} from "../libraries/LPPosition.sol";
...
abstract contract BaseAlcorOptionCore is IBaseAlcorOptionCore {
        ...
        )
    {
        ...
        // case of burn
            ...
            userOptionBalanceDelta = int256(_position.deposit_amount1)-int256(amount1Delta);
            amount0ToTransfer = amount0Delta;

            // transfer the collateral to the owner
            amount1ToTransfer = (amount1Delta >= _position.deposit_amount1 ?  _position.deposit_amount1 : amount1Delta);
    ...
    function _collectFeesLP(
        ...
        internal
        returns (
            uint128 amount0ToTransfer,
            uint128 amount1ToTransfer
        )
    {
        LPPosition.Info memory _position = LPpositionInfos.get(LPPosition.Key(
            {
                owner: msg.sender, 
                expiry: optionPoolKey.expiry, 
                strike: optionPoolKey.strike, 
                isCall: optionPoolKey.isCall, 
                tickLower: -tickUpper, 
                tickUpper: -tickLower
            }));
        ...
        // collect fees
        (amount0ToTransfer, amount1ToTransfer) = v3Pool.collect(
            msg.sender,
            optionPoolKey.hashOptionPool(),
            -tickUpper,
            -tickLower,
...
import {IAlcorOptionCore} from "../../interfaces/IAlcorOptionCore.sol";
import {IBaseComboOption} from "../../interfaces/IBaseComboOption.sol";
...
import {VanillaOptionPool} from "../../libraries/combo-pools/VanillaOptionPool.sol";
import {LPPosition} from "../../libraries/LPPosition.sol";
import {OptionBalanceMath} from "../../libraries/OptionBalanceMath.sol";
import {FullMath} from "../../libraries/FullMath.sol";
...
import {IV3Pool} from "../../interfaces/v3-pool/IV3Pool.sol";
import {IV3PoolActions} from "../../interfaces/v3-pool/IV3PoolActions.sol";
    ...
    IVanillaOption
{
    ...
    function mint(
        ...
        // console.log('balance token1', ERC20(token1).balanceOf(address(v3Pool)));
        ...
        )
    {
        ...
        // console.logInt(userOptionBalanceDelta);
        
       if (userOptionBalanceDelta < 0) {
            if (userOptionBalance < 0 && userOptionBalance - userOptionBalanceDelta > 0) {
                amount1ToTransfer+= uint256(-userOptionBalance); 
            } else if (userOptionBalance < 0 && userOptionBalance - userOptionBalanceDelta < 0) {
        ...
        // we emit amount{0,1}Delta for correct displaying on the frontend
        emit AlcorBurn(
            lpPositionKey.owner,
            amount0ToTransfer,
            amount1ToTransfer
    ...
    function collectFees(
       ...
       (amount0, amount1) = _collectFeesLP(optionPoolKey, tickLower, tickUpper);
        ...
        )
    {
        ...
        if(amount1 != amountSpecified) revert SpecifiedAndReturnedAmountNotRelated();

        if(!zeroForOne){
            additionalFee = FullMath.mulDiv(uint256(amountSpecified),v3Pool.fee(), 1e6);
            amountSpecified -= int256(additionalFee);
        }
        // console.log("Additional fee: %s", additionalFee);
        int256 userOptionBalance = usersBalances.getOptionBalance(
            owner,
            optionPoolKey.hashOptionPool()
        );
        ...
        v3Pool.updatePoolBalances(
            optionPoolKey.hashOptionPool(),
            -int256(amount0PoolShouldTransfer),
            -int256(amount1PoolShouldTransfer-int256(additionalFee))
        );
...
interface IBaseAlcorOptionCore {
    // function optionPoolInfos(bytes32 hash) external view returns (uint256 strike, uint256 expiry, bool isCall);
}
...
interface IV3PoolActions {
    ...
    /// @dev Price is represented as a sqrt(amountToken1/amountToken0) Q64.96 value
    /// @param sqrtPriceX96 the initial sqrt price of the pool as a Q64.96
    // function initialize(bytes32 optionPoolHash, uint160 sqrtPriceX96) external;
    function initialize(
        uint256 expiry,
        uint256 strike,
        bool isCall,
    ...
    // @ AlcorFinance
    // function setAlcorOptionCoreAddress(address _alcorOptionCoreAddress) external;
}
...
interface IV3PoolState {
    ...
    //     uint128 liquidityGross,
    //     // uint128 liquidityGrossBuying,
    //     // uint128 liquidityGrossSelling,
    //     // amount of net liquidity added (subtracted) when tick is crossed from left to right (right to left),
    //     int128 liquidityNet,
    //     // int128 liquidityNetBuying,
    //     // int128 liquidityNetSelling,
    //     // fee growth per unit of liquidity on the _other_ side of this tick (relative to the current tick)
    //     // only has relative meaning, not absolute — the value depends on when the tick is initialized
    //     uint256 feeGrowthOutside0X128,
    //     uint256 feeGrowthOutside1X128,
    //     // uint256 feeGrowthOutside0BuyingX128,
    //     // uint256 feeGrowthOutside1BuyingX128,
    //     // uint256 feeGrowthOutside0SellingX128,
    //     // uint256 feeGrowthOutside1SellingX128,
    ...
    //     // @ AlcorFinance:
    //     // bool isBuyingLP,
    //     // uint256 feeGrowthInside0BuyingLastX128,
    //     // uint256 feeGrowthInside1BuyingLastX128,
    //     // uint256 feeGrowthInside0SellingLastX128,
    //     // uint256 feeGrowthInside1SellingLastX128,
...
library VanillaOptionPool {
    ...
    function updatePoolBalances(
        ...
        // @dev TODO: fix bug with pool tokens balances:

        // token0
        if (token0Delta > 0) poolBalances.token0Balance += uint256(token0Delta);
        else if (poolBalances.token0Balance > uint256(-token0Delta)) {
            poolBalances.token0Balance -= uint256(-token0Delta);
        } else {
            poolBalances.token0Balance = 0;
        }
        // token1
        if (token1Delta > 0) poolBalances.token1Balance += uint256(token1Delta);
        else if (poolBalances.token1Balance > uint256(-token1Delta)) {
            poolBalances.token1Balance -= uint256(-token1Delta);
        } else {
            poolBalances.token1Balance = 0;
        }
        // console.log("PoolBalance amount0: %s , amount1: %s", poolBalances.token0Balance, poolBalances.token1Balance);
    }
...
import {EnumerableSet} from "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";
import {VanillaOptionPool} from "./combo-pools/VanillaOptionPool.sol";
...
library TickMath {
    ...
    int24 internal constant MIN_TICK = -300000;
