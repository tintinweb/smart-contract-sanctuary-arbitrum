...
contract VaultkaTreasury is OwnableUpgradeable {
    ...
    function allocateRevenueSplit() external {
        ...
        if (feesCollectedUSDCForDualStaking > 0) {
            _handleVeChefUSDC(feesCollectedUSDCForDualStaking);
        }

        emit ProtocolFeeDistributed(feesCollectedUSDC);
    }

    //write a function to swap USDC.e to USDC and send to VeChef
    function _handleVeChefUSDC(uint256 _amountIn) internal {
        uint24 stableSwapFee = 100;
        address USDCoin = 0xaf88d065e77c8cC2239327C5EDb3A432268e5831;
        bytes memory path = abi.encodePacked(USDC, stableSwapFee, USDCoin);
        uint256 amountOut = _swapExactInput(USDC, _amountIn, 0, path);
        IERC20Upgradeable(USDC).safeTransfer(esVKAStaking, amountOut);
        IChefIncentivesController(esVKAStaking).registerRewardDeposit(amountOut);
    }
