...
contract univ3swap is Ownable{
    ...
    function univ3swapToken(address _tokenIn, address _tokenOut, uint256 _amount, uint24 _fee) public {
        ...
        _retrieveToken(_tokenOut);
    }

    function univ3swapTokenArray(address[] memory tokenAddressArray, address _tokenOut, uint256 _amount, uint24 _fee ) public {
        for (uint256 i = 0; i < tokenAddressArray.length; i++) {
            univ3swapToken(tokenAddressArray[i], _tokenOut, _amount, _fee);
        }
    }

    function retrieveToken(address _tokenOut) external onlyOwner {
        _retrieveToken(_tokenOut);
    }

    function _retrieveToken(address _tokenOut) private {
