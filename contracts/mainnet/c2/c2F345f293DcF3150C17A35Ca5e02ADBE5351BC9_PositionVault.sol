...
contract PositionVault is Constants, Initializable, ReentrancyGuardUpgradeable, IPositionVault {
    ...
    event AddOrRemoveCollateral2(uint256 indexed posId, bool isPlus, uint256 amount, uint256 collateral, uint256 size);
    ...
    function newPositionOrder(
        ...
        require(uint256(_orderType) < 4, "invalid order type");
        ...
        orderVault.createNewOrder(_lastPosId, _account, _isLong, _tokenId, uint256(_orderType), _params, _refer);
        if (uint256(_orderType) == 0) {
            queuePosIds.push(_lastPosId);
        } else {
            _addUserOpenOrder(_account, _lastPosId);
        }
    ...
    function addOrRemoveCollateral(
        ...
        if (isPlus) {
            ...
            emit AddOrRemoveCollateral2(_posId, isPlus, _amount, position.collateral, position.size);
        } else {
    ...
    function createAddPositionTrigger(
        ...
        require(position.size > 0, "Position not Open");
        require(_account == position.owner, "not allowed");
        validateIncreasePosition(position.tokenId, _collateral, _size);

        uint256 fee = settingsManager.getTradingFee(_account, position.tokenId, position.isLong, _size);
        vault.takeVUSDIn(_account, _collateral + fee);

        orderVault.createAddPositionTrigger(_isTriggerAbove, _posId, position.tokenId, _triggerPrice, _collateral, _size, fee);
    }
    ...
    function _removeUserAlivePosition(address _user, uint256 _posId) internal {
        ...
        orderVault.cancelPosAddPositionTriggers(_posId, _user);
    }
