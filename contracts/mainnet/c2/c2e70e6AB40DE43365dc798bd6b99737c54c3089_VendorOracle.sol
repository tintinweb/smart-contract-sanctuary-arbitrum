...
contract VendorOracle is IErrors, IVendorOracle, Ownable {
    ...
    address private constant OHM = 0x6E6a3D8F1AfFAc703B1aEF1F43B8D2321bE40043;
    address private constant OHM_INDEX = 0x0000000000000000000000000000000000000001;
    ...
    {
        if (_base == gOHM) {
            int256 index = getPriceUSD(OHM_INDEX);
            int256 OHM_price = getPriceUSD(OHM);
            if (OHM_price == NOT_FOUND || index == NOT_FOUND) return NOT_FOUND;
            return (OHM_price * index) / 1e9;
        } //gOHM
        return NOT_FOUND;
    }
