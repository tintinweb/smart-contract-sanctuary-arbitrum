...
// File: IUniswapV3PositionsNFT.sol
...
interface IUniswapManager {
  ...
  function determineTicks(IUniswapV3Pool _pool, uint24 _twapTime, int24 tickRangeMultiplier) external view returns (int24, int24);
  function getLiquidity(uint256 _tokenId) external view returns (uint128 _liquidity);
  function getLpReserves(uint256 _tokenId) external view returns (uint256 _token0, uint256 _token1);
  function getCurrentTick(uint256 _tokenId) external view returns (int24 tick);
  ...
  function getPool(address _token0, address _token1, uint24 _fee) external view returns(IUniswapV3Pool);
  function getPriceAtTick(int24 tick) external view returns (uint256);
  function getSqrtTwapX96(IUniswapV3Pool uniswapV3Pool, uint32 twapInterval) external view returns (uint160 sqrtPriceX96);
  function getPriceX96FromSqrtPriceX96(uint160 sqrtPriceX96) external pure returns(uint256 priceX96);
  function getTwapPrice(IUniswapV3Pool _pool, uint32 _time) external view returns (uint256);
  function isUnbalanced(uint256 _tokenId) external view returns (bool _result);
  function setTwapTime(uint256 _tokenId, uint24 _twapTime) external;
  function setTickRangeMultiplier(uint256 _tokenId, int24 _tickRangeMultiplier) external;
  function newPosition(positionParameters calldata _params) external returns (uint256 _tokenId, uint256 _refund0, uint256 _refund1);
  function deposit(uint256 _tokenId, uint256 _amount0, uint256 _amount1, bool _balance) external returns (uint256 _refund0, uint256 _refund1);
...
abstract contract CoreStrategyAaveUni is BaseStrategy {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    ...
    function setTickRangeMultiplier(int24 _tickRangeMultiplier) external onlyAuthorized {
    ...
    function getLpPrice() public view returns (uint256) {
        return manager.getTwapPrice(wantShortLP, 3);
    }
    ...
    function estimatedTotalAssets() public view override returns (uint256) {
        //(uint256 amount0, uint256 amount1) = manager.getLpReserves(tokenId);
        //return amount0 + convertShortToWantLP(amount1);
        uint256 balShort = convertShortToWantLP(balanceShort());
        uint256 balShortInWant = convertShortToWantLP(balShort);
        return balanceOfWant().add(balanceDeployed()).add(balShortInWant);
    }
     ...
     */
    function balanceDebtOracle() public view returns (uint256) { // TODO: PUBLIC
        ...
        uint256 amountOut =
            router.exactInputSingle(
                ISwapRouter.ExactInputSingleParams({
                    ...
                    amountIn: _amount,
                    //amountOutMinimum: amountOutMin, // TODO: No slippage?
                    amountOutMinimum: 0,
                    sqrtPriceLimitX96: 0
                })
            );
        if( convertShortToWantLP(amountOut) < convertShortToWantLP(desired) ) {
            return convertShortToWantLP(desired).sub(convertShortToWantLP(amountOut));
        } else {
            return 0;
        }
        ...
        */
        ...
        _amountWant = router.exactInputSingle(
            ISwapRouter.ExactInputSingleParams({
                ...
                //amountOutMinimum: amountOutMin, //TODO: revist
                amountOutMinimum: 0,
                sqrtPriceLimitX96: 0
            })
        );
        if(desired > _amountWant)
            _slippageWant = desired.sub(_amountWant);
    }
...
contract USDCWETHAAVEUNI is CoreStrategyAaveUni {
    using SafeERC20 for IERC20;
    ...
    function claimHarvest() internal override {
        if(tokenId != 0)
            IUniswapManager(manager).collectPositionFees(tokenId);
    }
