...
contract MatchOrderFeature is IMatchOrderFeature, IERC721OrdersEvent, FixinTokenSpender, FixinERC721Spender  {
    ...
    function matchOrders(bytes[] calldata datas, bool revertIfIncomplete) external override {
        ...
        assembly {
            ...
            for { let ptr := datas.offset } lt(ptr, ptrEnd) { ptr := add(ptr, 0x20) } {
                ...
                switch selector
                // matchOrder
                case 0xdf57eb0a00000000000000000000000000000000000000000000000000000000 {
                    impl := implMatchOrder
                }
                // matchERC721Order
        ...
        returns (uint256 profit)
    {
        ...
        SellOrderInfo memory sellOrderInfo = _checkupSellOrder(sellOrderParam, buyOrder.maker);
    ...
    function _checkupBuyOrder(
        ...
        if (LibOracleSignature.isNeedValidateOracleSignature(order.expiry)) {
            extraData = LibOracleSignature.validateOracleSignature(orderHash, taker, extraData);
        }
        ...
        if (
            ...
            (validateHash, extraData) = _getBulkValidateHashAndExtraData(leaf, extraData);
        }
    ...
    function _checkupSellOrder(SellOrderParam memory param, address taker) internal returns (SellOrderInfo memory info) {
        ...
        // Set the validateOracleSignature flag.
        bool isNeedValidateOracleSignature = ((data3 >> 160) & 1) == 1;
        if (isNeedValidateOracleSignature) {
            info.expiryTime |= (1 << 248);
        }
        ...
        // Validate the oracle signature if needed.
        if (isNeedValidateOracleSignature) {
            LibOracleSignature.validateOracleSignature(info.orderHash, taker, param.extraData);
        }
        ...
        require(
            ...
            signatureType == LibSignature.SignatureType.ETHEREUM_PERSONAL_SIGN_1271 ||
            signatureType == LibSignature.SignatureType.BITCOIN_PERSONAL_SIGN_1271,
    ...
    function _validateOrderSignature(
        ...
        if (
            ...
            require(
                LibSignature.isValidSignature1271(maker, hash, v, r, s),
                "INVALID_SIGNATURE"
            );
        } else if (
            signatureType == LibSignature.SignatureType.ETHEREUM_PERSONAL_SIGN_1271 ||
            signatureType == LibSignature.SignatureType.ETHEREUM_PERSONAL_SIGN_BULK_1271
        ) {
            ...
            signatureType == LibSignature.SignatureType.BITCOIN_PERSONAL_SIGN_1271 ||
            signatureType == LibSignature.SignatureType.BITCOIN_PERSONAL_SIGN_BULK_1271
        ) {
            ...
            signatureType == LibSignature.SignatureType.BITCOIN_PERSONAL_SIGN_173 ||
            signatureType == LibSignature.SignatureType.BITCOIN_PERSONAL_SIGN_BULK_173
        ) {
