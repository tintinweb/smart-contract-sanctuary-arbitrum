...
import "./interfaces/IDefexaExchange.sol";

contract Arbitrage is Initializable,
    UUPSUpgradeable, OwnableUpgradeable, WardedLivingUpgradeable {

    event Swap(address indexed user, address indexed tokenFrom, address indexed tokenTo, uint256 amountIn, uint256 price);
    ...
    function arbitrage(
        ...
        // transfer from
        require(IERC20(swapToken).transferFrom(msg.sender, address(this), amount), "TF");
        ...
        //transfer to
        require(IERC20(receiveToken).transfer(msg.sender, endBalance - startBalance), "TF");
    }
...
//        require(hash == keccak256(data), "QHTrade/hash-mismatch");
    ...
    function swap(
        ...
        uint256 startBalance = _prepareSwap(signature, swapToken, receiveToken, amount);
        ...
        //transfer to
        uint256 result = endBalance - startBalance;
        if (result > 0) {
            if (receiveToken != address(0)) {
                require(IERC20(receiveToken).transfer(msg.sender, result), "TF");
            } else {
                payable(msg.sender).transfer(result);
            }
        }

        emit Swap(msg.sender, swapToken, receiveToken, amount, price);
    }
...
interface IDefexaExchange {
    ...
    event OrderFilled(uint256 orderId, uint256 amountSent, uint256 amountReceived, uint256 ts);
