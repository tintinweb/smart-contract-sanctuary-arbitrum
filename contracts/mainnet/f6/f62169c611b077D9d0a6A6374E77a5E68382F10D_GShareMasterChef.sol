...
contract GShareMasterChef is IRewardPool, OwnableUpgradeSafe, IERC721Receiver, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTime;    // Last block number that GSHARE distribution occurs.
        uint256 accGSharePerShare;  // Accumulated GSHARE per share, times 1e18. See below.
    ...
    uint256 public totalGShareTaxed;
    ...
    uint256 public constant DEFAULT_RUNNING_TIME = 365 days;
    uint256 public constant TOTAL_REWARDS = 600000 ether;
    ...
    function initialize(
        ...
        poolStartTime = _poolStartTime; // supposed to be 1680739200 Thursday, April 6, 2023 12:00:00 AM
        poolEndTime = poolStartTime.add(DEFAULT_RUNNING_TIME);
        rewardPerSecond = TOTAL_REWARDS.div(DEFAULT_RUNNING_TIME); 
    ...
    function pendingReward(uint256 _pid, address _user) public override view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 _gshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint_);
            accGSharePerShare = accGSharePerShare.add(_gshareReward.mul(1e18).div(lpSupply));
        }
    ...
    // Update the given pool's GSHARE allocation point and deposit fee. Can only be called by the owner.
    ...
    function depositNFT(address _nft, uint256 _tokenId, uint256 _slot, uint256 _pid) external nonReentrant checkContract {
        ...
        user.rewardDebt = user.amount.mul(poolInfo[_pid].accGSharePerShare).div(1e18);
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _gshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint_);
            pool.accGSharePerShare = pool.accGSharePerShare.add(_gshareReward.mul(1e18).div(lpSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function _harvestReward(uint256 _pid, address _account, bool _taxedReward) internal {
        ...
        if (_pending > 0) {
            if (_taxedReward) {
                ...
                _safeGShareTransfer(_account, _half_boosted_pending);
                emit RewardPaid(_account, _pid, _pending, _boost);
                _safeGShareTransfer(_account, _half_boosted_pending);
                totalGShareTaxed = totalGShareTaxed.add(_half_boosted_pending);
                emit RewardTaxed(_account, _pid, _half_boosted_pending);
            } else {
                uint256 _boost = _pending.mul(getBoost(_account, _pid)).div(10000);
                _safeGShareTransfer(_account, _pending.add(_boost));
                emit RewardPaid(_account, _pid, _pending, _boost);
            }
    ...
    // Deposit LP tokens to MasterChef for GSHARE allocation.
    ...
    function _safeGShareTransfer(address _to, uint256 _amount) internal {
        IERC20 _gshare = IERC20(gshare);
        uint256 _bal = _gshare.balanceOf(address(this));
        if (_amount > _bal) _amount = _bal;
        if (_amount > 0) {
            _gshare.transfer(_to, _amount);
        }
    }
    ...
    function governanceRecoverUnsupported(address _token, uint256 amount, address to) external onlyOwner {
        if (block.timestamp < poolEndTime + 180 days) {
            // do not allow to drain core token (GShare or lps) if less than 180 days after pool ends
