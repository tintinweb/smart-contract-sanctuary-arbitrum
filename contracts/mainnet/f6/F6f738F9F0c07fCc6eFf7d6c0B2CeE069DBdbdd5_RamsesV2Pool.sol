...

            {
                ...
                if (boostedLiquidityCache.newBoostedLiquidity > 0) {
                    ...
                    VeRamBoostCache memory veRamBoostCache;
                    {
                        ...
                        if (veRamBoostCache.veRamBoostUsedRatio > 1e18) {
                            veRamBoostCache.veRamBoostUsedRatio = 1e18;
                        }
                        uint256 veRamBoostAvailableRatio = 1e18 - veRamBoostCache.veRamBoostUsedRatio;

                        // make this cap take effect after epoch 2791, this if block can be taken out in subsequent upgrades
                        if (period < 2791) {
                        ...
                        if (boostedLiquidityCache.newBoostedLiquidity > veRamBoostAvailable) {
    ...
    function _switchAttached(
        ...
        if (veRamTokenId != oldAttached) {
            ...
            if (oldAttached != 0) {
                ...
                uint128 veRamBoostUsedRatio = states.boostInfos[period].veRamInfos[oldAttached].veRamBoostUsedRatio;

                if (veRamBoostUsedRatio > positionBoostUsedRatio) {
                    veRamBoostUsedRatio -= uint128(positionBoostUsedRatio);
                } else {
                    veRamBoostUsedRatio = 0;
                }

                states.boostInfos[period].veRamInfos[oldAttached].veRamBoostUsedRatio = veRamBoostUsedRatio;
