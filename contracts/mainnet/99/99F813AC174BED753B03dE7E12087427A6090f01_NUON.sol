...
contract NUON is ERC20, Ownable {
    ...
    modifier onlyPools() {
        require(controller.isAllowedToMint(msg.sender),'NUON: not an approved pool');
        _;
    }
    ...
    function setWhitelisted(address _toWhitelist, bool _status) public onlyOwner {
        whitelisted[_toWhitelist] = _status;
    }

    function setNUONController(address _controller) external onlyOwner {
        require(_controller != address(0), '0 address provided');
        controller = INUONController(_controller);
    }
    ...
    function transferFrom(
        ...
        address spender = _msgSender();
        _spendAllowance(from, spender, amount);
        _transferWithValidation(from, to, amount);
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        _transferWithValidation(_msgSender(), recipient, amount);
        return true;
    }
    ...
    function _transferWithValidation(
        ...
        if (isLiquidityPool(recipient) && sender == owner() || 
            isLiquidityPool(sender) && recipient == owner()) {
            _transfer(sender, recipient, amount);
        } else if (sender == owner() && whitelisted[recipient] ||
            whitelisted[sender] && recipient == owner()) {
            _transfer(sender, recipient, amount);
        } else if (swapEnabled) {
            _transfer(sender, recipient, amount);
        } else {
            revert("Not enabled yet");
        }
    ...
    function transferAnyERC20Token(address _tokenAddr, address _to) public onlyOwner {
        uint256 balOfToken = IERC20(_tokenAddr).balanceOf(address(this));
        require(IERC20(_tokenAddr).transfer(_to, balOfToken), "Could not transfer out tokens!");
    }
}