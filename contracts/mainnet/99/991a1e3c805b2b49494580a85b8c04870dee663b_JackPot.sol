...
JACKPOT PROTOCOL v1.0

FIRST DECENTRALIZED PROTOCOL
WITH AUTOMATIC LOTTERY SYSTEM FOR HOLDERS IN ARBITRUM

The contract accumulates a certain amount of ETH for each transaction of buy/sale, when the accumulated amount exceeds more than 0.06 ETH, contract executes the random lottery function and chooses a random wallet holder who holds a minimum amount of $JACK.
All users who hold more than 30,000 (0.03% of Total Supply) tokens have their assured participation and can be raffled every time. It should be noted that the greater the movement in the market, the greater the amount of automatic lotteries.


More information:
https://www.jackpotprotocol.com/
...
/**
* @title Jack Protocol ($JACK) Contract
* @author Dev Mexos
...
contract JackpotProtocol is ERC20, Ownable, VRFV2WrapperConsumerBase {
  using SafeMath for uint256;
  ...
  //Buy fees
  uint256 public _buy_SusFee;
  uint256 public _buy_LotoFee;
  uint256 public _buy_MktFee;
  uint256 public _buy_totalFees;

  //Sell fees
  uint256 public _sell_SusFee;
  uint256 public _sell_LotoFee;
  uint256 public _sell_MktFee;
  uint256 public _sell_totalFees;

  //Average fees
  uint256 public _average_SusFee;
  uint256 public _average_LotoFee;
  uint256 public _average_MktFee;
  uint256 public _average_totalFees;

  //Sustainability wallet fee address
  address payable private _susFeeAddress;
  ...
  //Jackpot Pot Contract
  JackPot public _jackpotContract;

  //Excluded from lottery
  mapping (address => bool) private _isExcludedFromJackpot;

  //Jackpot min ETH for execution
  uint256 public _jackpotExecuteAmount;

  //Min. amount to execute Lotto-Plus
  uint256 public _lottoPlusExecuteAmount;

  //Min. amount to participe in Jackpot
  ...
  address internal immutable wrapperAddress = 0x96A0F0Ef8DFa87d7bACF46412C64dC87a4109a22;
  ...
  event JackpotWinner(uint256 randomNumber, address wallet, uint256 prizeAmount, bool chainlinkGenerated);
  ...
  constructor() ERC20("Jackpot Protocol", "JACK", 18) VRFV2WrapperConsumerBase(linkAddress, wrapperAddress) {

    //Creation of a uniswap pair for this token for mainnet/testnet
    //Mainnet
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x38eEd6a71A4ddA9d7f776946e3cfa4ec43781AE6);
    //Testnet
    //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x38eEd6a71A4ddA9d7f776946e3cfa4ec43781AE6);
    ...
    // Creation of Jackpot Pot subcontract (the token contract is the creator and the one who manages it)
    // If the Jackpot Pot subcontract must be changed, the token contract must be established as the owner 
    // for the operation of the lottery dynamics, through the "setTokenAddress" function of the subcontract.
    _jackpotContract = new JackPot();
    ...
    //Initial fee wallets
    _susFeeAddress = payable(_msgSender());
    ...
    _jackpotExecuteAmount = 60000000000000000; //Min. default execution amount: 0.06 ETH (in wei).
    _minAmountToParticipate = 3000 * (10**decimals()); //Min. default amount to participate: 0.03% of Total Supply.
    _lottoPlusExecuteAmount = 1000000000000000000; //For the execution of Lotto-Plus, there must be 1 ETH (in wei) left over in the token contract.
    ...
    _buy_LotoFee = 3;
    _buy_MktFee = 2;
    _buy_totalFees = _buy_SusFee.add(_buy_LotoFee).add(_buy_MktFee);
    _sell_SusFee = 3;
    _sell_LotoFee = 3;
    _sell_MktFee = 2;
    _sell_totalFees = _sell_SusFee.add(_sell_LotoFee).add(_sell_MktFee);
    _average_SusFee = _buy_SusFee.add(_sell_SusFee);
    _average_LotoFee = _buy_LotoFee.add(_sell_LotoFee);
    _average_MktFee = _buy_MktFee.add(_sell_MktFee);
    _average_totalFees = _average_SusFee.add(_average_LotoFee).add(_average_MktFee);

    //Initial value at 0 for lottery rounds
    _jackpotRound = 0;

    //Initial minimum amount of LINK to allow the use of the Chainlink VRF system
    _minLinkBalanceToUseChainlink = 0; //Min. 0 LINK
  ...
  * If the Jackpot Pot subcontract must be changed, 
  ...
  *
  * @param addr New Jack Protocol Jackpot Pot Sub-Contract Address.
  *
  * Requirements:
  *
  ...
  */
  function updateJackpotContractAddress (address payable addr) public onlyOwner {
    _jackpotContract = JackPot(addr);
  }
  ...
  * - Must be executed by the contract owner.
  */
  function updateJackpotExecuteAmount(uint256 amount) public onlyOwner {
    _jackpotExecuteAmount = amount;
  }
  ...
  * - Must be executed by the contract owner.
  */
  function updateJackpotMinTokensAmount(uint256 amount) public onlyOwner {
  ...
  * - Must be executed by the contract owner.
  */
  function excludeFromJackpot(address account, bool state) public onlyOwner {
    _isExcludedFromJackpot[account] = state;
  ...
  * - Must be executed by the contract owner.
  */
  function setSusFeeAddress(address newAddress) public onlyOwner{
    _susFeeAddress = payable(newAddress);
  }
  ...
  * - Must be executed by the contract owner.
  */
  function setMktFeeAddress(address newAddress) public onlyOwner{
    _mktFeeAddress = payable(newAddress);
  }
  ...
  */
  function isExcludedFromJackpot(address account) public view returns(bool) {
    return _isExcludedFromJackpot[account];
  }
  ...
  * Requirements:
  *
  * - It can only be executed internally by the function `_transfer` or `excludeFromJackpot`.
  */
  ...
  * After obtaining the winning number, send the funds from the Jackpot
  * Pot subcontract to the corresponding wallet and add the event data
  * to a registry variable.
  *
  * Emits a {JackpotWinner} event.
  *
  * Requirements:
  *
  ...
  function awardRandom() private {
    if (_listOfHolders.length > 0){
      ...
      if(contractLinkBalance >= _minLinkBalanceToUseChainlink){
        ...
        fixedSeed = alternativePseudoRandom(100, 1000000, address(_jackpotContract).balance);
        chainlinkGenerated = false;
      }
      uint256 rndVal = fixedSeed % _listOfHolders.length;
      uint256 prizeAccumulated = address(_jackpotContract).balance;
      _jackpotContract.withdraw(_listOfHolders[rndVal]);
      _jackpotRound++;
      _winnerInfo[_jackpotRound].randomNumber = rndVal;
      _winnerInfo[_jackpotRound].wallet = _listOfHolders[rndVal];
      _winnerInfo[_jackpotRound].prizeAmount = prizeAccumulated;
      _winnerInfo[_jackpotRound].chainlink = chainlinkGenerated;
      emit JackpotWinner(rndVal, _listOfHolders[rndVal], prizeAccumulated, chainlinkGenerated);
    }
  }
  ...
  */
  function _transfer(
    ...
    if (!_isExcludedFromJackpot[from] && balanceOf(from) < _minAmountToParticipate && _addedHolderList[from]){
      removeHolder(from);
    }
    if (!_isExcludedFromJackpot[to] && balanceOf(to) >= _minAmountToParticipate && !_addedHolderList[to] && to != uniswapV2Pair){
      addHolder(to);
    }
    if (address(_jackpotContract).balance > _jackpotExecuteAmount && !_swapping && !justSwaped && !_pickingWinner){
      _pickingWinner = true;
      awardRandom();
      _pickingWinner = false; 
    }
  ...
  */
  function swapActualTokensAndSendDividends(uint256 tokens) private  {
    ...
    uint256 SusFee = newBalance.mul(_average_SusFee).div(_average_totalFees);
    transferToAddressETH(_susFeeAddress, SusFee);
    uint256 BuyBackFee = newBalance.mul(_average_MktFee).div(_average_totalFees);
    transferToAddressETH(_mktFeeAddress, BuyBackFee);
    uint256 LotoFee = newBalance.mul(_average_LotoFee).div(_average_totalFees);
    transferToAddressETH(payable(_jackpotContract), LotoFee);
    uint256 remainingBalance = address(this).balance;
    if(remainingBalance >= _lottoPlusExecuteAmount){
      transferToAddressETH(payable(_jackpotContract), remainingBalance);
...
/**
* @title Jack Protocol Jackpot Pot Sub-Contract
* @author Dev Mexos
...
* `updateJackpotContractAddress` function in the token contract.
*/
...
contract JackPot is Ownable, ReentrancyGuard {
  ...
  * `updateJackpotContractAddress` function of the token contract.
  *
  ...
  */
  ...
  /** @notice Deposit ETH/BNB into the Jackpot Pot subcontract.
  *
