...
contract BattleFrens is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public WinAmount;
    mapping(address => uint256) public LooseAmount;

    event Fren1joined(address indexed fren1, uint256 indexed battleid, string tokenURI, uint256 betamount);
    event Fren2joined(address indexed fren2, uint256 indexed battleid, string tokenURI, uint256 betamount);
    event BattleFinished(
        address indexed winner,
        uint256 indexed wonamount,
    ...
    function fren1join(uint256 _battleid, uint256 _amountNFA, string memory _tokenURI) public {
        ...
        fren1Joined = true;
        emit Fren1joined(fren1[_battleid], _battleid, tokenURI, _amountNFA);
    }
    ...
    function fren2join(uint256 _battleid, uint256 _amountNFA, string memory _tokenURI) public {
        ...
        emit Fren2joined(fren2[_battleid], _battleid, tokenURI, _amountNFA);
    }
    ...
    function startbattle(uint256 _battleid) public {
        ...
        uint256 burnAmount = ((betamount * burnperc) / 10);
        string memory winnerURI = frenImg[_battleid][winner];
        currBattle_id++;
        fren1Joined = false;
        fren2Joined = false;
        IFREN(NFA_ERC20).transfer(fren_grave, burnAmount * 10 ** 18);

        emit BattleFinished(WinnerFren[_battleid], frenRewardDebt[winner], _battleid, winnerURI, burnAmount);
    }
    ...
    function getReward() public nonReentrant {
        ...
        uint256 reward = (frenRewardDebt[msg.sender] - 1) * 10 ** 18;
