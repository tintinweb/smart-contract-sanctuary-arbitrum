...
/**
 * @title HelixDealTokens
 * @notice HelixDealTokens is an ERC721 compliant contract, which can represent
 *  investment shares of any of the borrower deal.
 * @author Helix
 */
contract HelixDealTokens is IHelixDealTokens, ERC721PresetMinterPauserdUpgradeSafe {  
  ...
  // dealAddress => DealStats
  mapping(address => DealStats) public deals;
  ...
  */
  ...
  function initialize(address owner, IHelixConfig _config) external initializer {
    ...
    __ERC721_init_unchained("Helix Deal Tokens", "HLX-DT");
    __Pausable_init_unchained();
  ...
  /// @notice Halting the helix investment share token minting process.
  function halt() external onlyRole(OWNER_ROLE) {
    paused() ? _unpause() : _pause();
  }

  /// @notice this function will only be called by admin to allow helix investment share token can be 
  /// transferred between investors.
  function toggleAllowToTransfer() external whenNotPaused onlyRole(OWNER_ROLE) {
    allowToTransfer = allowToTransfer ? false : true;
  }

  /// @inheritdoc IHelixDealTokens
  function mint(
    uint256 principalAmount,
    address to
  ) external virtual override onlyDeal whenNotPaused returns (uint256 tokenId) {
    address dealAddress = _msgSender();
  ...
  function updateDealStats(
    ...
    uint256 returnPaid
  ) external override onlyDeal whenNotPaused {
    address dealAddress = _msgSender();

    DealStats storage dealStats = deals[dealAddress];
    DealStats memory previousDealStats = dealStats;

    dealStats.principalInvested = dealStats.principalInvested.add(principalInvested);
    dealStats.principalRepaid = dealStats.principalRepaid.add(principalRepaid);
    dealStats.cumulativeReturns = dealStats.cumulativeReturns.add(returnPaid);

    require(dealStats.principalRepaid <= dealStats.principalInvested, "Principal repaid cannot exceed Principal invested");

    emit DealStatsUpdated(
      dealAddress, 
      previousDealStats.principalInvested, 
      previousDealStats.principalRepaid,
      previousDealStats.cumulativeReturns,
      dealStats.principalInvested, 
      dealStats.principalRepaid,
      dealStats.cumulativeReturns
    );
  }
  ...
  function updateProtocolStats(
    ...
    ProtocolStats memory previousProtocolStats = getProtocolStats();

    protocolStats.principalInvested = protocolStats.principalInvested.add(principalInvested);
    protocolStats.principalRepaid = protocolStats.principalRepaid.add(principalRepaid);
    protocolStats.cumulativeReturns = protocolStats.cumulativeReturns.add(returnPaid);

    require(protocolStats.principalRepaid <= protocolStats.principalInvested, "Principal repaid cannot exceed Principal invested");

    emit ProtocolStatsUpdated(
      previousProtocolStats.principalInvested, 
      previousProtocolStats.principalRepaid,
      previousProtocolStats.cumulativeReturns,
      protocolStats.principalInvested, 
      protocolStats.principalRepaid,
      protocolStats.cumulativeReturns
    );
  }

  /// @inheritdoc IHelixDealTokens
  function bookRepayment(
    uint256 tokenId, 
    uint256 principalRepaid, 
    uint256 returnPaid
  ) external virtual override onlyDeal whenNotPaused {
    TokenInfo storage token = tokens[tokenId];
    address dealAddr = token.deal;
    require(_msgSender() == dealAddr, "Only the token's deal can repay");

    token.principalRepaid = token.principalRepaid.add(principalRepaid);
    require(
      token.principalRepaid <= token.principalInvested,
      "Principal repaid cannot exceed Principal invested"
    );
    token.cumulativeReturns = token.cumulativeReturns.add(returnPaid);

    emit TokenRepaymentBooked(
      ownerOf(tokenId),
      dealAddr,
      tokenId,
      principalRepaid,
  ...
  function burn(uint256 tokenId) external virtual override whenNotPaused {
    ...
    bool fromTokenDeal = _validDeal(_msgSender()) && token.deal == _msgSender();
    address owner = ownerOf(tokenId);
    require(canBurn || fromTokenDeal, "ERC721Burnable: caller cannot burn this token");
    require(
      token.principalRepaid == token.principalInvested,
      "Can only burn fully repaid tokens"
    );
    _destroyAndBurn(owner, address(token.deal), tokenId);
  }

  function calculateTokenId(address dealAddress, address investor) public override view returns(uint256) {
    return uint256(keccak256(abi.encodePacked(dealAddress, investor, address(this))));
  }

  function getTokenInfo(uint256 tokenId) public view virtual override returns (TokenInfo memory) {
    return _getTokenInfo(tokenId);
  }

  function getDealStats(address deal) public view override returns (DealStats memory) {
    return deals[deal];
  }

  function getProtocolStats() public view override returns (ProtocolStats memory) {
    return protocolStats;
  }

  /// @inheritdoc IHelixDealTokens
  function onDealCreated(address newDeal) external override onlyHelixFactory {
    deals[newDeal].created = true;
  }
  ...
  /// @inheritdoc IHelixDealTokens
  function isValidDeal(address sender) public view virtual override returns (bool) {
    return _validDeal(sender);
  }
   ...
   * @param principalInvested token principal
   * @param dealAddress deal to which the token belongs
  ...
  function _createToken(
    ...
    tokenId = calculateTokenId(dealAddress, mintTo);
    TokenInfo storage userTokenInfo = tokens[tokenId];

    if (userTokenInfo.deal == address(0)) {
      userTokenInfo.deal = dealAddress;
    ...
    userTokenInfo.principalInvested +=  principalInvested;

    emit TokenPrincipalUpdated({
      owner: mintTo,
      deal: dealAddress,
      tokenId: tokenId,
      amount: userTokenInfo.principalInvested
    });
  }

  function _destroyAndBurn(address owner, address deal, uint256 tokenId) internal {
    delete tokens[tokenId];
    _burn(tokenId);
    emit TokenBurned(owner, deal, tokenId);
  }
  ...
  function _validDeal(address dealAddress) internal view virtual returns (bool) {
    return deals[dealAddress].created;
  }
  ...
  function dealTVL(address dealAddress) public view override returns(uint256) {
    DealStats storage deal = deals[dealAddress];
    return deal.principalInvested - deal.principalRepaid; 
  }
  ...
  function _beforeTokenTransfer(
    ...
    if (from != address(0)) {
      require(allowToTransfer, "Helix deal tokens is non-transferrable!");
    }
  ...
  modifier onlyDeal() {
    require(_validDeal(_msgSender()), "Invalid deal!");
    _;
  }
