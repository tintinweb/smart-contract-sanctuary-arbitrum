...
import {BeefyWrapper} from "../../external/beefy/BeefyWrapper.sol";
import {BeefyWrapperFactory} from "../../external/beefy/BeefyWrapperFactory.sol";
...
import {IBeefyStrategyAdapter} from "./adapters/IBeefyStrategyAdapter.sol";
    ...
    BeefyDepositoryStorage
{
    ...
    function setAdapter(address newAdapter) external onlyOwner {
        adapter = IBeefyStrategyAdapter(newAdapter);
    }
        ...
        returns (uint256)
    {
        ...
        // uint256 vaultAssets = vaultWrapper.convertToAssets(shares);
        uint256 redeemableAmount = poolTokenAmount.mulWadDown(adapter.getLpTokenPrice());
        ...
        returns (uint256)
    {
        ...
        redeemableUnderManagement -= redeemableAmount;
        // netAssetDeposits -= assetAmount;
        uint256 lpTokenAmount = adapter.calculateLpTokenAmountOneCoin(asset, assetAmount);
        ...
        uint256 coinAmount = adapter.removeLiquidityOneCoin(asset, lpTokenAmount, 0);
        // uint256 assetValue = tokenAmount.mulWadUp(adapter.getLpTokenPrice());
        IERC20Upgradeable(asset).safeTransfer(address(controller), coinAmount);
        emit Withdrawn(msg.sender, coinAmount, redeemableAmount, shares);
        return coinAmount;
    }
...
contract BeefyWrapper is ERC4626Upgradeable {
    ...
    function wrap(uint256 amount) public {
        IERC20Upgradeable(vault).safeTransferFrom(
            msg.sender,
            address(this),
            amount
        );
