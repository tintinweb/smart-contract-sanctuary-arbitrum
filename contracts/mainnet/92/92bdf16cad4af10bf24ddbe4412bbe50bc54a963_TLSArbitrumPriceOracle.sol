...
import {IAggregator} from "src/price-oracles/interfaces/IAggregator.sol";

import {BaseMixedPriceOracle} from "src/price-oracles/mixed/BaseMixedPriceOracle.sol";

contract TLSArbitrumPriceOracle is BaseMixedPriceOracle {
    function _getChainlinkOrApi3Aggregator(address currency)
        internal
        pure
        override
        returns (uint8 decimals, IAggregator.Aggregator[] memory aggregators)
    {
        if (currency == address(0)) {
            ...
            // wstETH/ETH
            // Deviation 0.5% | Heartbeat 86400s | Decimals 8
            ...
            // https://arbiscan.io/address/0x639Fe6ab55C921f74e7fac1ee960C0B6293ba612
            aggregators[1] = IAggregator.Aggregator(24 hours, 0x639Fe6ab55C921f74e7fac1ee960C0B6293ba612, 8);
        } else if (currency == 0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8) {
            // CHAINLINK
            // USDC (Stargate)
            ...
            // Deviation 0.1% | Heartbeat 86400s | Decimals 8
            // https://arbiscan.io/address/0x50834F3163758fcC1Df9973b6e91f0F0F0434aD3
            aggregators[0] = IAggregator.Aggregator(24 hours, 0x50834F3163758fcC1Df9973b6e91f0F0F0434aD3, 8);
    ...
    function _getRedstoneFeedId(address currency) internal pure override returns (uint8 decimals, bytes32 feedId) {
...
import {IPriceOracle} from "src/price-oracles/interfaces/IPriceOracle.sol";
import {IAggregator} from "src/price-oracles/interfaces/IAggregator.sol";
import {Currency} from "src/libraries/CurrencyLib.sol";
import {ChainlinkLib} from "src/price-oracles/libraries/ChainlinkLib.sol";
import {Api3Lib} from "src/price-oracles/libraries/Api3Lib.sol";
import {RedstoneLib} from "src/price-oracles/libraries/RedstoneLib.sol";

import {RedstoneConsumerBase} from "redstone/core/RedstoneConsumerBase.sol";

abstract contract BaseMixedPriceOracle is IPriceOracle, RedstoneConsumerBase {
    function prices(Currency currency0, Currency currency1, bytes calldata)
        external
        view
        override
        returns (uint256 currency0Price, uint256 currency1Price)
    {
        (currency0Price, currency1Price) =
            (_getPrice(Currency.unwrap(currency0)), _getPrice(Currency.unwrap(currency1)));

        if (currency0Price == 0 || currency1Price == 0) {
            bytes32[] memory feedIds = new bytes32[](currency0Price == currency1Price ? 2 : 1);
            if (currency0Price == currency1Price) {
                extractTimestampsAndAssertAllAreEqual();
                uint8 decimals0;
                uint8 decimals1;
                (decimals0, feedIds[0]) = _getRedstoneFeedId(Currency.unwrap(currency0));
                (decimals1, feedIds[1]) = _getRedstoneFeedId(Currency.unwrap(currency1));
                uint256[] memory redstonePrices = _securelyExtractOracleValuesFromTxMsg(feedIds);
                return
                    (RedstoneLib.price(decimals0, redstonePrices[0]), RedstoneLib.price(decimals1, redstonePrices[1]));
            } else {
                uint8 decimals;
                (decimals, feedIds[0]) =
                    _getRedstoneFeedId(Currency.unwrap(currency0Price == 0 ? currency0 : currency1));
                uint256[] memory redstonePrices = _securelyExtractOracleValuesFromTxMsg(feedIds);
                uint256 price = RedstoneLib.price(decimals, redstonePrices[0]);
                currency0Price == 0 ? currency0Price = price : currency1Price = price;
            }
        }
    }
    ...
    function _getPrice(address currency) internal view returns (uint256) {
        (uint8 decimals, IAggregator.Aggregator[] memory aggregators) = _getChainlinkOrApi3Aggregator(currency);
        if (aggregators.length != 0) {
            return aggregators[0].decimals != 0
                ? ChainlinkLib.price(decimals, aggregators)
                : Api3Lib.price(decimals, aggregators);
        }
    }
     ...
     */
    function _getRedstoneFeedId(address currency) internal pure virtual returns (uint8 decimals, bytes32 feedId);
}
