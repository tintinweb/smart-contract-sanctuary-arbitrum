...
contract ParticleMinter is ManagerModifier, ReentrancyGuard, Pausable {
  ...
  function capture(
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      (uint256 currentRealmId, bool isRealmSet) = PARTICLE_TRACKER.currentRealm(addr, adventurerId);
      ...
      // Remove explorer
      PARTICLE_TRACKER.removeExplorer(currentRealmId, addr, adventurerId, explorerWeight);
  ...
  function capture(
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      require(isRealmSet, "ParticleMinter: You must choose a realm to explore first");
  ...
  function getElapsedTime(address _addr, uint256 _adventurerId) external view returns (uint256) {
  ...
  // Internal
  //=======================================
  function _capture(address _addr, uint256 _adventurerId, uint256 _realmId) internal {
    // Check sender owns adventurer
    require(
    ...
    uint256 adventurerParticles = _totalParticlesForAdventurer(_addr, _adventurerId);

    // Mint particles for Adventurer
    PARTICLE.mintFor(msg.sender, adventurerParticles);

    // Get total particles for realmer
    uint256 realmerParticles = _totalParticlesForRealmer(_realmId, _addr, _adventurerId);
    ...
    emit Captured(_realmId, _addr, _adventurerId, adventurerParticles, realmerParticles);
  }
  ...
  function _totalParticlesForAdventurer(
    ...
    uint256 particlesPerSecond = (ADVENTURER_DATA.get(
      _addr,
      _adventurerId,
  ...
  function _totalParticlesForRealmer(
    ...
    uint256 particlesPerSecond = (realmerBaseParticle * _totalResources(_realmId)) / 24 / 60 / 60;
  ...
  function _elapsedTime(address _addr, uint256 _adventurerId) internal view returns (uint256) {
    // Calculate elapsed time
    uint256 elapsedTime = block.timestamp - PARTICLE_TRACKER.timer(_addr, _adventurerId);
  ...
  function _checkPopulation(uint256 _realmId) internal view {
    require(
      PARTICLE_TRACKER.getExplorerCount(_realmId) < POPULATION.getPopulation(_realmId),
      "ParticleMinter: Max Realm population reached"
    );
  }

  function _checkAddress(address _addr, bytes32[] calldata _proof) internal view {
    // Verify address
