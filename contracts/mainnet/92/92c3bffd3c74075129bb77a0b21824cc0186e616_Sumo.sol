...
contract Sumo is Permissions {
    ...
    constructor(
        ...
        address _dispensary,
        uint256 _cost,
        string memory _sumo0,
        string memory _sumo1
    ) {
        bugs = IBugs(_bugs);
    ...
    function placeBet(bool _choseZero, uint256 _betAmount) public {
        ...
        if (maxBet > 0){
            require(_betAmount <= maxBet, "Invalid bet size");
        }
    ...
    function defineMatch(uint256 _cost, string memory _sumo0, string memory _sumo1) external onlyApp {
    ...
    function runMatch() external onlyApp {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (bettors[bettor].choseZero == matches[currentMatchID].zeroWon) {
                ...
                uint256 bet = bettors[bettor].betAmount;
                ...
                if (stubs >= stubSetSize) {
                    ...
                    if (stubSets > 0) {
                        winnings += ((bet * (100 + stubSets)) / 100) - bet;
                    }
                }
    ...
    function setStubSetSize(uint256 _stubSetSize) external onlyOwner {
        stubSetSize = _stubSetSize;
    }
