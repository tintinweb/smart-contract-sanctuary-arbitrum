...
contract Potm is VRFConsumerBaseV2 {
    ...
    address private constant VRF_COORDINATOR = 0x41034678D6C633D8a95c75e1138A360a28bA15d1;
    address private constant LINK_TOKEN_ADDRESS = 0xf97f4df75117a78c1A5a0DBb814Af92458539FB4;
    bytes32 private constant KEY_HASH = 0x72d2b016bb5b62912afea355ebf33b91319f828738b111b723b78696b9847b63;
    uint64 private constant SUBSCRIPTION_ID = 202;
    ...
    mapping(uint256 => uint256) public prizeRequestIds;
    mapping(uint256 => uint256) public requestIdToPrizeIndex;
    ...
    function initializePrizes() private {
        prizes.push(Prize("Milady #3325", address(0), false));
        prizes.push(Prize("Remilio #1940", address(0), false));
        prizes.push(Prize("Remilio #7819", address(0), false));
        prizes.push(Prize("Remilio #318", address(0), false));
    ...
    function drawPrizes() public onlyOwner {
        ...
        for (uint256 i = 0; i < prizes.length; i++) {
            ...
            prizeRequestIds[s_requestId] = i;
            emit RandomnessRequested(s_requestId);
        }
    }

    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        require(hasDrawn, "Draw not initiated");
        uint256 prizeIndex = prizeRequestIds[requestId];
        uint256 totalTickets = 0;
        for (uint256 i = 0; i < participants.length; i++) {
            totalTickets += ticketsPerParticipant[participants[i]];
        }

        uint256 random = randomWords[0] % totalTickets;
        uint256 sum = 0;
        for (uint256 j = 0; j < participants.length; j++) {
            sum += ticketsPerParticipant[participants[j]];
            if (random < sum) {
                prizes[prizeIndex].winner = participants[j];
                ticketsPerParticipant[participants[j]] = 0;
                emit PrizeDrawn(prizes[prizeIndex].name, participants[j]);
                break;
            }
        }
