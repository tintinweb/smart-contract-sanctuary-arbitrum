    ...
    PausableUpgradeable
{
    ...
    uint256 public lockMDlpProportion;
    uint256 public treasuryFee;

    /* ============ Events ============ */

    event dlpConverted(address indexed _account, uint256 _dlpAmount, uint256 _mDlpAmount, uint256 _smartConvertProportion, uint256 _directConvertProportion);
    ...
    event lockMDlpProportionSet(uint256 _lockMDlpProportion);
    event treasuryFeeSet(uint256 _treasuryFee);
    event mDlpLocked(address indexed _account, uint256 _amount);
    ...
    function __DlpRush3_init(address _rdnt, address _rdntDlp, address _mdlp, address _ARB, address _mDlpSV, address _smartConvert) public initializer {
        __Ownable_init();
    ...
    function zapWithRadiant (
        ...
        uint256 _liquidity = IMDLP(mDLP).zapRadiantForDLP{ value: msg.value }(
            address(this),
            _rdntAmt
        );

        _convertAndEarn(msg.sender, _liquidity);
    }
    ...
    function convertLp (
        ...
        _convertAndEarn(msg.sender, _amount);
    }
    ...
    function _sendRewards(address _account, uint256 _amount) internal {
        ...
        emit ARBRewarded(_account, rewardToSend);
    }
    ...
    function _convert(address _account, uint256 _amount) internal {
        ...
        if (directAmount > 0) {
            IERC20(rdntDlp).safeApprove(address(mDLP), directAmount);
            IMDLP(mDLP).convertWithLp(address(this), directAmount, 0);
        }

        if (smartAmount > 0) {
            IERC20(rdntDlp).safeApprove(smartConvert, smartAmount);
            ISmartDlpConvert(smartConvert).smartConvert(smartAmount, 0);
        }
        emit dlpConverted(_account, _amount, IERC20(mDLP).balanceOf(address(this)), smartConvertProportion, DENOMINATOR - smartConvertProportion);
    }

    function _convertAndEarn(address _account, uint256 _amount) internal {
        _convert(_account, _amount);
        uint256 treasuryFeeAmount = (IERC20(mDLP).balanceOf(address(this)) - _amount) * treasuryFee / DENOMINATOR;
        uint256 mDlpToTransfer = _mDlpTransferAndLock(_account, IERC20(mDLP).balanceOf(address(this)) - treasuryFeeAmount);
        if (mDlpToTransfer > 0){
            IERC20(mDLP).safeTransfer(_account, mDlpToTransfer);
        }

        if (treasuryFeeAmount > 0){
            IERC20(mDLP).safeTransfer(owner(), treasuryFeeAmount);
        }
        _sendRewards(_account, _amount);
    }

    function _lock(address _account, uint256 _amount) internal {
        IERC20(mDLP).safeApprove(address(mDlpSV), _amount);
        mDlpSV.lockFor(_amount, _account);

        emit mDlpLocked(_account, _amount);
    }

    /* ============ Private Functions ============ */ 

    function _mDlpTransferAndLock(
        address _for,
        uint256 mDlp
    ) private returns (uint256) {
        uint256 mDlpToLock = mDlp * lockMDlpProportion / DENOMINATOR;
        uint256 mDlpToTransfer = mDlp - mDlpToLock;

        if (mDlpToLock > 0) {
            _lock(_for, mDlpToLock);
        }

        return mDlpToTransfer;
    }
    ...
    function setLockMDlpProportion(uint256 _lockMDlpProportion) external onlyOwner {
        require(_lockMDlpProportion <= DENOMINATOR, "Lock mDlp Proportion cannot be greater than 100%."); 
        lockMDlpProportion = _lockMDlpProportion;
        emit lockMDlpProportionSet(_lockMDlpProportion);
    }
