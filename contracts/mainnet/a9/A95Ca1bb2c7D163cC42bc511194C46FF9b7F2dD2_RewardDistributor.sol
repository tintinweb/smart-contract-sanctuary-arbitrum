    ...
    PausableUpgradeable
{
    ...
    function sendVeReward(
        ...
        for (uint256 i = 0; i < feeInfo.length; i++) {
            if (feeInfo[i].isActive) {
                ...
                if (!feeInfo[i].isAddress) {
                    IERC20(_rewardToken).safeApprove(feeInfo[i].to, feeTosend);
                    IBaseRewardPool(feeInfo[i].to).queueNewRewards(feeTosend, _rewardToken);
                } else {
                    IERC20(_rewardToken).safeTransfer(feeInfo[i].to, feeTosend);
                }

                emit VeRewardPaidTo(_rewardSource, feeInfo[i].to, _rewardToken, feeTosend);
            }
        }
            ...
            if (pancakeFeeInfos[i].isActive) {
                ...
                if(pancakeFeeInfos[i].isMCAKE) {
                    if(smartCakeConvert != address(0)) {
                        IERC20(CAKE).safeApprove(smartCakeConvert,feeAmount);
                        uint256 beforeBalance = IERC20(mCake).balanceOf(address(this));
                        ISmartCakeConvertor(smartCakeConvert).smartConvert(feeAmount, 0);
                        rewardToken = mCake;
                        feeTosend = IERC20(mCake).balanceOf(address(this)) - beforeBalance;
                    }
                    else
                    {
                        IERC20(CAKE).safeApprove(address(mCakeConvertor), feeAmount);
                        uint256 beforeBalance = IERC20(mCake).balanceOf(address(this));
                        mCakeConvertor.convert(address(this), feeAmount, 0);
                        rewardToken = mCake;
                        feeTosend = IERC20(mCake).balanceOf(address(this)) - beforeBalance;
                    }
                }

                if (!pancakeFeeInfos[i].isAddress) {
                    IERC20(rewardToken).safeApprove(pancakeFeeInfos[i].to, feeTosend);
                    IBaseRewardPool(pancakeFeeInfos[i].to).queueNewRewards(feeTosend, rewardToken);
                } else {
                    IERC20(rewardToken).safeTransfer(pancakeFeeInfos[i].to, feeTosend);
                }

                emit RewardPaidTo(_poolAddress, pancakeFeeInfos[i].to, _rewardToken, feeTosend);
            }
        }
    ...
    function setVeCakeOrRevenueFee(
        ...
        _setFee(_index, _value, _to, _isAddress, _isActive, feeInfo);
    ...
    function setPancakeFee(
        ...
        _setFee(_index, _value, _to, _isAddress, _isActive, pancakeFeeInfos);
