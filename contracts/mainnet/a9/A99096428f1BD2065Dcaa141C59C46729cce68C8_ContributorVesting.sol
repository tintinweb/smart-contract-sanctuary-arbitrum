...
contract ContributorVesting is Ownable {
    ...
    struct Vest {
        uint256 role; // the role of the beneficiary
        uint256 tier; // vesting tier of the beneficiary
    ...
    // Vesting tiers
    mapping(uint256 => mapping(uint256 => uint256)) public vestingTiers;
     ...
     * @return whether beneficiary was updated
     */
    function updateBeneficiaryTier(address _beneficiary, uint256 _tier)
        external
        onlyOwner
        returns (bool)
    {
        ...
        uint256 oldTier = vBeneficiary.tier;
    ...
    function vestTimeLeft(address beneficiary) public view returns (uint256) {
        uint256 vestingEnd = vestedBeneficiaries[beneficiary].startTime +
            threeYears;
        return
            block.timestamp < vestingEnd ? (vestingEnd - block.timestamp) : 0;
    }
