...
USD Rewards on Arbitrum
Welcome to ArbUSD #RBUSD
...
contract ArbUSD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ArbUSDDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ArbUSD", "RBUSD") {
        marketingWallet = payable(0x63cFDCA8Fd73B25a09FA7096630e155252B82140);
        devWallet = payable(0x8C518922593170eA0DCB6c803f56038737c66cb0);
        ...
        dividendTracker = new ArbUSDDividendTracker(
            payable(this),
            router,
...
contract ArbUSDDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        ArbUSDContract = ArbUSD(mainContract);
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "ArbUSD_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ArbUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
    ...
    function _reinvestDividendOfUser(
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ArbUSDContract);

            uint256 prevBalance = ArbUSDContract.balanceOf(address(this));
            ...
            {
                uint256 received = ArbUSDContract.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    ArbUSDContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
