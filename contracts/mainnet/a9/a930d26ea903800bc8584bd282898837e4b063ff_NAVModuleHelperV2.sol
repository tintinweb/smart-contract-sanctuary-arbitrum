...
contract NAVModuleHelperV2 is ReEntrancyGuard {
  ...
  function getIssueAmountToSwap(uint256 amount) public view returns (uint256) {
    (, uint256 usdcToSwap ) = calcIssueData(amount);
    return (usdcToSwap);
  }

  function getRedeemAmountToSwap(uint256 amount) public view returns (uint256) {
    uint256 wstethToSwap = 0;
    (, uint256 amount2) = calcRedeemData(amount);
    if (amount2 > 0) {
      wstethToSwap = INAVModule(NAVMODULE).getExpectedReserveRedeemQuantity(
        SETTOKEN,
        WSTETH,
        amount2
      );
