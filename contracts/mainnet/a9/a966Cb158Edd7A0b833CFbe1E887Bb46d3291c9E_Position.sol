...
library Position {
    ...
    function _updateBoostedPosition(
        ...
        require(secondsPerLiquidityPeriodIntX128 >= 0);
        require(secondsPerBoostedLiquidityPeriodIntX128 >= 0);
    ...
    function _updatePosition(
...

        {
            ...
            if (newLiquidity == 0) {
                _switchAttached(position, boostedPosition, 0, period);
                params.veRamTokenId = 0;
            }

            // type(uint256).max serves as a signal to not switch attachment
            if (params.veRamTokenId != type(uint256).max) {
                _switchAttached(position, boostedPosition, params.veRamTokenId, period);
            }

            {
                (uint256 veRamRatio, uint256 newBoostedLiquidity) = LiquidityMath.calculateBoostedLiquidity(
                    newLiquidity,
                    boostedPosition.veRamAmount,
                    states.boostInfos[period].totalVeRamAmount
                );

                if (newBoostedLiquidity > 0) {
                    uint160 lowerSqrtRatioX96 = TickMath.getSqrtRatioAtTick(params.tickLower);
                    uint160 midSqrtRatioX96 = TickMath.getSqrtRatioAtTick((params.tickLower + params.tickUpper) / 2);
                    uint160 upperSqrtRatioX96 = TickMath.getSqrtRatioAtTick(params.tickUpper);

                    uint256 balance0 = States.balance0();
                    uint256 balance1 = States.balance1();

                    {
                        ...
                        if (balance0 > maxBalance0) {
                            balance0 = maxBalance0;
                        }
                        if (balance1 > maxBalance1) {
                            balance1 = maxBalance1;
                        }
                    }
                    ...
                    uint256 boostedLiquidityCap = FullMath.mulDiv(hypotheticalLiquidity, veRamRatio, 1e18);
                    if (newBoostedLiquidity > boostedLiquidityCap) {
                        newBoostedLiquidity = boostedLiquidityCap;
                    }
                }

                boostedLiquidityDelta = int128(newBoostedLiquidity - boostedPosition.boostAmount);
            }
        }
    ...
    function _switchAttached(
        ...
        if (veRamTokenId != oldAttached) {
            ...
            if (oldAttached != 0) {
                ...
                // <= 1 because upgrade happens mid epoch
                // this can be changed to == 1 in a future upgrade
                if (timesAttached <= 1) {
                    veRamAmountDelta -= boostedPosition.veRamAmount;
                }

                // > 0 because upgrade happens mid epoch
                // this decrement can be outside of the if statement in a future upgrade
                if (timesAttached > 0) {
