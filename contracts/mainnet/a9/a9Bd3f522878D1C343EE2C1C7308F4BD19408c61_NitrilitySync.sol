...
contract NitrilitySync is Ownable(msg.sender), ReentrancyGuard {
    ...
    Counters.Counter private purchasingIds;

    EnumerableSet.UintSet purchasingDataSet;
    ...
    mapping(uint256 => PurchasingData) idToPurchasingData;
    ...
    function checkLicenseValid(
        ...
        NitrilityCommon.TemplateType memory templateType = INitrilityFactory(
            _nitrilityFactory
        ).getTemplateType(license, licensingType);

        require(templateType.listed, "License type not listed");
    ...
    // Function to calculate the total price with potential discounts
    function calculateTotalPrice(
        ...
        for (uint256 i = 0; i < licenseDatas.length; i++) {
            ...
            NitrilityCommon.TemplateType
                memory templateType = INitrilityFactory(_nitrilityFactory)
                    .getTemplateType(
                        licenseDatas[i].license,
                        licenseDatas[i].licensingType
                    );
            require(
                templateType.accessLevel == licenseDatas[i].accessLevel ||
                    templateType.accessLevel == NitrilityCommon.AccessLevel.Both
            );
            ...
            if (
                ...
                licensePrice = templateType.sPrice;
            } else {
                licensePrice = templateType.fPrice;
            }
            ...
            if (
                ...
                for (
                    uint256 j = 0;
                    j < licenseDatas[i].discountCodes.length;
                    j++
                ) {
                    if (
                        keccak256(bytes(licenseDatas[i].discountCode)) ==
                        keccak256(bytes(licenseDatas[i].discountCodes[j].code))
                    ) {
                        percentage = licenseDatas[i]
                            .discountCodes[j]
                            .percentage;
                        require(percentage <= 100, "InValid Discount Code");
                        break; // Exit loop early if discount found
                    }
                }
            }
            // Calculate total price based on license price and discount percentage
            totalPrice += (licensePrice * (100 - percentage)) / 100;
        }
    ...
    function purchaseLicenses(
        ...
        for (uint256 i = 0; i < licenseDatas.length; i++) {
            ...
            purchasingIds.increment();
            uint256 currentPurchasingId = purchasingIds.current();
            purchasingDataSet.add(currentPurchasingId);

            idToPurchasingData[currentPurchasingId] = PurchasingData(
                licenseDatas[i].license.listedId,
                licenseDatas[i].license.artistId,
                licenseDatas[i].newTokenURI,
                currentPurchasingId,
                licenseDatas[i].buyerAddr,
                msg.value,
                licenseDatas[i].licensingType,
    ...
    function approvePurchasing(
        ...
        require(
            idToPurchasingData[purchasingId].eventType ==
                NitrilityCommon.EventTypes.SalePending,
            "Invalid License"
        );

        idToPurchasingData[purchasingId].eventType = NitrilityCommon
            .EventTypes
            .SaleAccepted;

        PurchasingData memory purchasingData = idToPurchasingData[purchasingId];
    ...
    function rejectPurchasing(uint256 purchasingId) external onlyOwner {
        require(
            idToPurchasingData[purchasingId].eventType ==
                NitrilityCommon.EventTypes.SalePending,
    ...
    function removePurchasing(uint256 purchasingId) external {
        require(
            idToPurchasingData[purchasingId].buyerAddr == msg.sender,
    ...
    function fetchPurchasingDataOfBuyer(
        address _buyerAddr,
        uint256 _listedId
    ) public view returns (PurchasingData[] memory) {
        uint256 itemCount = 0;
        PurchasingData[] memory purchasingDatas;

        for (uint256 i = 0; i < purchasingDataSet.length(); i++) {
            PurchasingData storage purchasingData = idToPurchasingData[
                purchasingDataSet.at(i)
            ];
        ...
        purchasingDatas = new PurchasingData[](itemCount);
        itemCount = 0;
        ...
        for (uint256 i = 0; i < purchasingDataSet.length(); i++) {
            ...
            if (
                ...
                purchasingDatas[itemCount] = purchasingData;
                itemCount++;
            }
        }
    ...
    function fetchPurchasingDataOfSeller(
        address _sellerAddr,
        uint256 _listedId
    ) public view returns (PurchasingData[] memory) {
        uint256 itemCount = 0;
    ...
    function getTokenURI(
        ...
        return idToPurchasingData[purchasingId].tokenURI;
    }
}
