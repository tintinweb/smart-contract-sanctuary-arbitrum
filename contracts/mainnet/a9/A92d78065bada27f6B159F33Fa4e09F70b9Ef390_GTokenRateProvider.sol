...
contract GTokenRateProvider is IRateProvider {
    ...
    uint128 public immutable precisionDelta;

    error InvalidPrecisionDelta();

    constructor(IGToken _gToken) {
        gToken = _gToken;
        (, precisionDelta) = gToken.collateralConfig();

        // If `precisionDelta` is 0, it means the gToken has not been initialized
        if (precisionDelta == 0) {
            revert InvalidPrecisionDelta();
        }
    }

    /**
     * @dev Calculates and returns the price rate of a gToken
     * @return the value of gToken in collateral token
     */
    function getRate() external view override returns (uint256) {
        // `shareToAssetsPrice` is always scalled to 1e18 precision, so we need to divide by `precisionDelta` in case
        // decimals of the collateral token are different than 18
        return gToken.shareToAssetsPrice() / precisionDelta;
    }
}