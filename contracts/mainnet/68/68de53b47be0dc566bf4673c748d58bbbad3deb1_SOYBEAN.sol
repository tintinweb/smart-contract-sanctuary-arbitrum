...
contract SOYBEAN is ERC20, Ownable {
    ...
    uint256 public _maxWalletSize = 2 * (supply/100);
    ...
    constructor(IRouter router_, address marketing_) ERC20("SOYBEAN", "BEAN1") {
        router = router_;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (amount == 0) {
         ...
         if (
            ...
            if (to != address(router) && !_isExcludedFromTax[from] && !_isExcludedFromTax[to]) {
                require(amount <= _maxTxAmount, "Exceeds the maxTransaction.");
    ...
    function updateMaxTx(uint256 maxTx) external onlyOwner {
        _maxTxAmount = maxTx * (supply/100);
        emit MaxTxAmountUpdated(maxTx * (supply/100));
    }

    function updateWalletSize(uint256 maxSize) external onlyOwner {
        _maxWalletSize = maxSize * (supply/100);
    }

    function removeLimits() external onlyOwner{
        _maxTxAmount = supply;
        _maxWalletSize = supply;
        emit MaxTxAmountUpdated(supply);
    }
