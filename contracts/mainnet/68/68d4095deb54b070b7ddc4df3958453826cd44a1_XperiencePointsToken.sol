...
contract XperiencePointsToken is ERC20Upgradeable, UpgradeableOperable, ERC20PermitUpgradeable {
    /*´:°•.°+.*•´.*:˚.°*.˚•´.°:°•.°•.*•´.*:˚.°*.˚•´.°:°•.°+.*•´.*:*/
    /*                     STATE VARIABLES                        */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/

    /// @notice adjustable sell tax on $XP -> 350 = 3.5%
    uint16 public sellTax;

    /// @notice divisor for salesTax calculation
    uint16 private immutable DIVISOR = 10_000;

    /// @notice allow holder to bypass sell tax if true
    mapping(address => bool) public aMMPairs;

    /// @notice adjust fees for a given pair
    //NOTE: adding for future use in case we want to have a variable tax rate
    mapping(address => uint16) public taxRate;

    /// @notice Used to blacklist pools trying to avoid the sell retention
    ...
    event SetAMMPair(address indexed pair, bool indexed add);
    event SetSellTax(uint16 indexed sellTax);
    ...
    function init(address _treasuryRegistry) external initializer {
        __ERC20_init("XperiencePoints", "XP");
        __Governable_init(msg.sender);
        __ERC20Permit_init("XperiencePoints");

        if (_treasuryRegistry == address(0)) revert ZeroAddress();
    ...
    /**
     * @notice mint XP to user
     * @param _account to mint to
     * @param _amount to mint
     */
    function mint(address _account, uint256 _amount) external onlyOperator {
        _mint(_account, _amount);
    }

    /**
     * @notice burn XP tokens
    ...
    /**
     * @notice burn XP tokens from specified user
     * @param _account to burn XP tokens from
     * @param _amount to burn
     */
    function burnFrom(address _account, uint256 _amount) external onlyOperator {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (aMMPairs[to] && sellTax > 0) {
            ...
            if (roles.roleOfIndex(from) == 0) {
                ...
                percentage = roles.getUserRole(from).xpSellTax;
            }

            fees = (amount * percentage) / DIVISOR;
    ...
    /**
     * @notice add or remove a user from sell whitelist
     * @param _address to add/remove from whitelist
     * @param _isWhitelisted whether to add or remove from whitelist
     */

    function setSellWhitelist(address _address, bool _isWhitelisted) external onlyGovernor {
        if (_address == address(0)) revert ZeroAddress();
        aMMPairs[_address] = _isWhitelisted;
        emit SetAMMPair(_address, _isWhitelisted);
    }

    /**
     * @notice set a tax on XP sales
     * @dev should never be more than 5%
     * @param _tax the tax rate for XP sales
     */
    function setSellTax(uint16 _tax) external onlyGovernor {
        if (_tax > 500) revert InvalidTaxRate();
        sellTax = _tax;
        emit SetSellTax(_tax);
    }

    function setBlacklisted(address _pool, bool _status) external onlyGovernor {
        blacklisted[_pool] = _status;
    }

    function setTreasuryRegistry(address _newRegistry) external onlyGovernor {
        registry = ITreasuryRegistry(_newRegistry);
    }

    function setIsekaiRoles(address _roles) external onlyGovernor {
    ...
    /**
     * @dev override OZ ERC20 decimals to base 9
     */
    ...
    /**
     * @notice logic to burn from a specified account on their behalf
     * @param _account to burn from
     * @param _amount to burn
     */
