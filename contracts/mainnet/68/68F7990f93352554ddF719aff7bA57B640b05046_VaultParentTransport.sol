...
contract Accountant {
    ...
    function getVaultValue(address vault) external view returns (uint minValue, uint maxValue) {
        address[] memory activeAssets = VaultBaseExternal(vault).assetsWithBalances();
        for (uint i = 0; i < activeAssets.length; i++) {
            (uint minAssetValue, uint maxAssetValue) = _assetValueOfVault(activeAssets[i], vault);
    ...
    function _getUSDPrice(address asset) internal view returns (int256 price) {
        ...
        require(updatedAt + registry.chainlinkTimeout() >= block.timestamp, 'Price expired');
...
contract CPIT {
    ...
    function _updatePriceImpact(
        ...
        priceImpactBips = _calculatePriceImpact(preTransactionValue, postTransactionValue);
        ...
        uint cumulativePriceImpact = _calculateCumulativePriceImpact(currentWindow);
    ...
    function _calculatePriceImpact(
        ...
        priceImpactBips = ((deviation * Constants.BASIS_POINTS_DIVISOR) / oldValue);
    }
}
...
contract IntegrationDataTracker {
    ...
    function getData(
        ...
        return IntegrationDataTrackerStorage.layout().trackedData[_integration][_vault][_index];
    }
    ...
    function getAllData(
        ...
        return IntegrationDataTrackerStorage.layout().trackedData[_integration][_vault];
    }
     ...
     */
    function getDataCount(Integration _integration, address _vault) public view returns (uint256) {
        return IntegrationDataTrackerStorage.layout().trackedData[_integration][_vault].length;
    }
     ...
     * @param _data the data track data to be recorded in storage
     */
    function _pushData(Integration _integration, address _vault, bytes memory _data) private {
        IntegrationDataTrackerStorage.layout().trackedData[_integration][_vault].push(_data);
    }
     ...
     * @param _index data index to be removed from storage
     */
    function _removeData(Integration _integration, address _vault, uint256 _index) private {
        IntegrationDataTrackerStorage.Layout storage l = IntegrationDataTrackerStorage.layout();
        uint256 length = l.trackedData[_integration][_vault].length;
        require(_index < length, 'invalid index');

        l.trackedData[_integration][_vault][_index] = l.trackedData[_integration][_vault][
            length - 1
        ];
...
library IntegrationDataTrackerStorage {
    bytes32 internal constant STORAGE_SLOT = keccak256('valio.storage.IntegationDataTracker');
...
interface IRedeemer {
    ...
    function preWithdraw(address asset, address withdrawer, uint portion) external payable;

    function withdraw(address asset, address withdrawer, uint portion) external payable;
...
contract Registry is SafeOwnable, Pausable {
    ...
    function initialize(
        ...
        l.integrationDataTracker = IntegrationDataTracker(_integrationDataTracker);
    ...
    function setDeprecatedAsset(address asset, bool deprecated) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
    ...
    function setAssetType(address asset, RegistryStorage.AssetType assetType) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
    ...
    function setValuer(RegistryStorage.AssetType assetType, address valuer) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
        l.valuers[assetType] = valuer;
    }

    function setRedeemer(RegistryStorage.AssetType assetType, address redeemer) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
        l.redeemers[assetType] = redeemer;
    }

    function setPriceAggregator(address asset, address aggregator) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
    ...
    function setIntegrationDataTracker(address _integrationDataTracker) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
        l.integrationDataTracker = IntegrationDataTracker(_integrationDataTracker);
    }

    function setZeroXExchangeRouter(address _zeroXExchangeRouter) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
    ...
    function setZeroXMaximumSingleSwapPriceImpactBips(
        ...
        RegistryStorage.Layout storage l = RegistryStorage.layout();
        l.zeroXMaximumSingleSwapPriceImpactBips = _zeroXMaximumSingleSwapPriceImpactBips;
    }

    function setExecutor(ExecutorIntegration integration, address executor) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
    ...
    function setLivelinessThreshold(uint256 _livelinessThreshold) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
        l.livelinessThreshold = _livelinessThreshold;
    }

    function setMaxCpitBips(VaultRiskProfile riskProfile, uint256 _maxCpitBips) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
    ...
    function setCanChangeManagerFees(bool _canChangeManagerFees) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
        l.canChangeManagerFees = _canChangeManagerFees;
    }

    function setMaxSingleActionImpactBips(uint256 _maxSingleActionImpactBips) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
        l.maxSingleActionImpactBips = _maxSingleActionImpactBips;
    }

    function setDepositAsset(address _depositAsset, bool canDeposit) external onlyOwner {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
    ...
    function maxCpitBips(VaultRiskProfile riskProfile) external view returns (uint256) {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
    ...
    function executors(ExecutorIntegration integration) external view returns (address) {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
    ...
    function zeroXMaximumSingleSwapPriceImpactBips() external view returns (uint256) {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
    ...
    function integrationDataTracker() external view returns (IntegrationDataTracker) {
        RegistryStorage.Layout storage l = RegistryStorage.layout();
...
interface ITransport {
    ...
    function bridgeApprovalCancellation(BridgeApprovalCancellationRequest memory request) external;
    ...
    function createVaultChild(VaultChildCreationRequest memory request) external;
    ...
    function sendChangeManagerRequest(ChangeManagerRequest memory request) external payable;

    function sendAddSiblingRequest(AddVaultSiblingRequest memory request) external;

    function sendBridgeApproval(BridgeApprovalRequest memory request) external payable;
    ...
    function sendWithdrawRequest(WithdrawRequest memory request) external payable;

    function sendValueUpdateRequest(ValueUpdateRequest memory request) external payable;
...
// solhint-disable ordering
contract Transport is SafeOwnable, ITransport, ILayerZeroReceiver, IStargateReceiver {
    ...
    function trustedRemoteLookup(uint16 remoteChainId) public view returns (bytes memory) {
        TransportStorage.Layout storage l = TransportStorage.layout();
    ...
    function stargateAssetToSrcPoolId(address bridgeToken) public view returns (uint256) {
        TransportStorage.Layout storage l = TransportStorage.layout();
        return l.stargateAssetToSrcPoolId[bridgeToken];
    }

    function getGasUsage(GasFunctionType gasFunctionType) public view returns (uint) {
        TransportStorage.Layout storage l = TransportStorage.layout();
    ...
    function getLzFee(
        ...
        ChildVault memory childVault = ChildVault({ chainId: 0, vault: address(0) });
    ...
    function sendBridgeApprovalCancellation(
        ...
        _send(
            ...
            abi.encodeWithSelector(this.bridgeApprovalCancellation.selector, request),
            msg.value,
            getGasUsage(GasFunctionType.standard)
        );
    }
    ...
    function _sendValueUpdatedRequest(ValueUpdatedRequest memory request) internal {
        (uint fee, uint gas) = registry().transport().getLzFee(
            GasFunctionType.standard,
            request.parentChainId
        );
    ...
    function _sendSGBridgedAssetAcknowledment(
        ...
        _send(
            request.parentChainId,
            abi.encodeWithSelector(this.sgBridgedAssetReceived.selector, request),
            fee,
            gas
        );
    }

    function _sendChildCreatedRequest(ChildCreatedRequest memory request) internal {
        (uint fee, uint gas) = registry().transport().getLzFee(
            GasFunctionType.standard,
            request.parentChainId
        );
    ...
    function bridgeApprovalCancellation(
        ...
        VaultParent(request.parentVault).receiveBridgeApprovalCancellation(request.requester);
    }

    function bridgeApproval(BridgeApprovalRequest memory request) public onlyThis {
    ...
    function updateVaultValue(ValueUpdatedRequest memory request) public onlyThis {
    ...
    function createVaultChild(VaultChildCreationRequest memory request) public onlyThis {
        ...
        _sendChildCreatedRequest(
            ChildCreatedRequest({
                parentVault: request.parentVault,
                parentChainId: request.parentChainId,
                newChild: ChildVault({ chainId: registry().chainId(), vault: child })
            })
        );
    }
    ...
    function changeManager(ChangeManagerRequest memory request) public onlyThis {
        VaultChild(request.child.vault).receiveManagerChange(request.newManager);
    }
    ...
    function createParentVault(
        ...
        return _createParentVault(name, symbol, manager, streamingFee, performanceFee, riskProfile);
    }
    ...
    function createParentAndChildVault(
        ...
        VaultParent(parentVault).requestCreateChild{ value: msg.value }(childChainId, lzFee);
        return parentVault;
    }
    ...
    function sgReceive(
        ...
        SGReceivePayload memory payload = abi.decode(_payload, (SGReceivePayload));
    ...
    function setTrustedRemoteAddress(
        ...
        TransportStorage.Layout storage l = TransportStorage.layout();
        l.trustedRemoteLookup[_remoteChainId] = abi.encodePacked(_remoteAddress, address(this));
    }

    function setSGAssetToSrcPoolId(address asset, uint poolId) external onlyOwner {
        TransportStorage.Layout storage l = TransportStorage.layout();
        l.stargateAssetToSrcPoolId[asset] = poolId;
    }

    function setSGAssetToDstPoolId(uint16 chainId, address asset, uint poolId) external onlyOwner {
        TransportStorage.Layout storage l = TransportStorage.layout();
        l.stargateAssetToDstPoolId[chainId][asset] = poolId;
    }

    function setGasUsage(GasFunctionType gasUsage, uint gas) external onlyOwner {
        TransportStorage.Layout storage l = TransportStorage.layout();
    ...
    function _createParentVault(
        ...
        require(registry().parentVaultDiamond() != address(0), 'not parent chain');
        deployment = address(new VaultParentProxy(registry().parentVaultDiamond()));
    ...
    function _deployChild(
        ...
        deployment = address(new VaultChildProxy(registry().childVaultDiamond()));
...
library TransportStorage {
    bytes32 internal constant STORAGE_SLOT = keccak256('valio.storage.Transport');
...
contract VaultBaseExternal is IGmxPositionRouterCallbackReceiver, VaultBaseInternal {
    ...
    function getVaultValue() external view returns (uint minValue, uint maxValue) {
...
contract VaultBaseInternal is ReentrancyGuard, CPIT {
    ...
    event Withdraw(uint tokenId, address withdrawer, uint portion, address[] assets);
    ...
    modifier onlyTransport() {
        require(address(_registry().transport()) == msg.sender, 'not transport');
        _;
    }
    ...
    function initialize(Registry registry, address manager, VaultRiskProfile riskProfile) internal {
    ...
    function _withdraw(uint tokenId, address withdrawer, uint portion) internal {
        VaultBaseStorage.Layout storage l = VaultBaseStorage.layout();
    ...
    function _gmxPositionCallback(bytes32 positionKey, bool isExecuted, bool isIncrease) internal {
        VaultBaseStorage.Layout storage l = VaultBaseStorage.layout();
        require(msg.sender == address(l.registry.gmxConfig().positionRouter()), 'not gmx');
    ...
    function _addAsset(address asset) internal {
        VaultBaseStorage.Layout storage l = VaultBaseStorage.layout();
        require(l.registry.accountant().isSupportedAsset(asset), 'asset not supported');
        if (!l.enabledAssets[asset]) {
            l.enabledAssets[asset] = true;
            l.assets.push(asset);
            require(l.assets.length <= l.registry.maxActiveAssets(), 'too many assets');
    ...
    function _getVaultValue() internal view returns (uint minValue, uint maxValue) {
        return _registry().accountant().getVaultValue(address(this));
    }

    function _hasValue(address asset) internal view returns (bool) {
        (, uint maxValue) = _registry().accountant().assetValueOfVault(asset, address(this));
        return maxValue > 0;
    }
}
...
library VaultBaseStorage {
    bytes32 internal constant STORAGE_SLOT = keccak256('valio.storage.VaultBase');
...
contract VaultChild is VaultBaseInternal, VaultBaseExternal {
    ...
    function initialize(
        ...
        require(_vaultParentAddress != address(0), 'invalid _vaultParentAddress');

        bytes32 vaultId = keccak256(abi.encodePacked(_parentChainId, _vaultParentAddress));
        _setVaultId(vaultId);
        ...
        for (uint8 i = 0; i < _existingSiblings.length; i++) {
            ...
            l.siblings[_existingSiblings[i].chainId] = _existingSiblings[i].vault;
        }
    }
    ...
    function receiveAddSibling(uint16 siblingChainId, address siblingVault) external onlyTransport {
        VaultChildStorage.Layout storage l = VaultChildStorage.layout();
    ...
    function requestBridgeApprovalCancellation(uint lzFee) external payable whenNotPaused {
        ...
        if (msg.sender != _manager()) {
            require(l.bridgeApprovalTime + timeout < block.timestamp, 'cannot cancel yet');
        }
    ...
    function getLzFee(bytes4 funcHash, uint16 chainId) public view returns (uint fee) {
        if (funcHash == this.requestBridgeToChain.selector) {
            fee = _bridgeQuote(chainId);
        } else {
            (fee, ) = _registry().transport().getLzFee(GasFunctionType.standard, chainId);
        }
    }
...
library VaultChildStorage {
    bytes32 internal constant STORAGE_SLOT = keccak256('valio.storage.VaultChild');
...
contract VaultFees {
    ...
    function _announceFeeIncrease(uint256 newStreamingFee, uint256 newPerformanceFee) internal {
        require(newStreamingFee <= _MAX_STREAMING_FEE_BASIS_POINTS, 'streamingFee to high');

        VaultFeesStorage.Layout storage l = VaultFeesStorage.layout();

        require(
            newStreamingFee <= l.managerStreamingFee + _MAX_STREAMING_FEE_BASIS_POINTS_STEP,
            'streamingFee step exceeded'
        );
        require(newPerformanceFee <= _MAX_PERFORMANCE_FEE_BASIS_POINTS, 'performanceFee to high');
        require(
            newPerformanceFee <= l.managerPerformanceFee + _MAX_PERFORMANCE_FEE_BASIS_POINTS_STEP,
    ...
    function _renounceFeeIncrease() internal {
        ...
        require(l.announcedFeeIncreaseTimestamp != 0, 'no fee increase announced');
    ...
    function _commitFeeIncrease() internal {
        ...
        require(l.announcedFeeIncreaseTimestamp != 0, 'no fee increase announced');
        require(
            block.timestamp >= l.announcedFeeIncreaseTimestamp + _FEE_ANNOUNCE_WINDOW,
            'fee delay active'
        );
        ...
        emit FeeIncreaseCommitted(l.managerStreamingFee, l.managerPerformanceFee);
    }
    ...
    function _protocolFee(uint managerFees) internal pure returns (uint) {
        return (managerFees * _PROTOCOL_FEE_BASIS_POINTS) / Constants.BASIS_POINTS_DIVISOR;
    }
}
...
library VaultFeesStorage {
    bytes32 internal constant STORAGE_SLOT = keccak256('valio.storage.VaultFees');
    ...
    VaultFees
{
    ...
    event FeesLevied(uint tokenId, uint streamingFees, uint performanceFees, uint currentUnitPrice);
    ...
    function initialize(
        ...
        super.initialize(_managerStreamingFeeBasisPoints, _managerPerformanceFeeBasisPoints);
        ERC721MetadataStorage.Layout storage l = ERC721MetadataStorage.layout();
        l.name = _name;
        l.symbol = _symbol;
    ...
    function _createManagerHolding(address manager) internal {
        ...
        require(_exists(_MANAGER_TOKEN_ID) == false, 'manager holding already exists');
        require(l._tokenIdCounter == _MANAGER_TOKEN_ID, 'manager holding must be token 0');
        _mint(manager);
    }
    ...
    function _createProtocolHolding(address protocolTreasury) internal {
        ...
        require(_exists(_PROTOCOL_TOKEN_ID) == false, 'protcool holding already exists');
        require(l._tokenIdCounter == _PROTOCOL_TOKEN_ID, 'protocol holding must be token 1');
    ...
    function _burnShares(uint tokenId, uint shares, uint currentUnitPrice) internal {
        VaultOwnershipStorage.Layout storage l = VaultOwnershipStorage.layout();

        VaultOwnershipStorage.Holding storage holding = l.holdings[tokenId];
        require(block.timestamp >= holding.unlockTime, 'locked');
        // Enforces 1 withdraw per 24 hours
        require(block.timestamp >= holding.lastBurnTime + BURN_LOCK_TIME, 'burn locked');
    ...
    function _levyFees(uint tokenId, uint currentUnitPrice) internal {
        ...
        emit FeesLevied(tokenId, streamingFees, performanceFees, currentUnitPrice);
    ...
    function _levyFeesOnHolding(
        ...
        (streamingFees, performanceFees) = _calculateUnpaidFees(tokenId, currentUnitPrice);

        if (streamingFees > 0 || holding.streamingFee != newStreamingFee) {
            holding.lastStreamingFeeTime = block.timestamp;
        }

        if (performanceFees > 0 || holding.performanceFee != newPerformanceFee) {
            holding.lastPerformanceFeeUnitPrice = currentUnitPrice;
        }
        ...
        require(
            holding.totalShares + streamingFees + performanceFees == initialTotalShares,
            'check failed'
        );
    ...
    function _holdings(uint tokenId) internal view returns (VaultOwnershipStorage.Holding memory) {
        VaultOwnershipStorage.Layout storage l = VaultOwnershipStorage.layout();
...
library VaultOwnershipStorage {
    bytes32 internal constant STORAGE_SLOT = keccak256('valio.storage.VaultOwnership');
...
contract VaultParentInternal is VaultOwnershipInternal, VaultBaseInternal {
    ...
    function _getLzFee(bytes4 sigHash, uint16 chainId) internal view returns (uint fee) {
        if (sigHash == IVaultParentManager.requestBridgeToChain.selector) {
            ...
            (fee, ) = _registry().transport().getLzFee(GasFunctionType.createChild, chainId);
        } else if (sigHash == IVaultParentInvestor.requestTotalValueUpdateMultiChain.selector) {
            (fee, ) = _registry().transport().getLzFee(GasFunctionType.getVaultValue, chainId);
        } else if (
            sigHash == IVaultParentInvestor.withdrawMultiChain.selector ||
            sigHash == IVaultParentInvestor.withdrawAllMultiChain.selector
        ) {
            (fee, ) = _registry().transport().getLzFee(GasFunctionType.withdraw, chainId);
        } else {
            (fee, ) = _registry().transport().getLzFee(GasFunctionType.standard, chainId);
        }
    }
    ...
    function _totalValueAcrossAllChains() internal view returns (uint minValue, uint maxValue) {
        VaultParentStorage.Layout storage l = VaultParentStorage.layout();
        ...
        for (uint8 i = 0; i < l.childChains.length; i++) {
            require(_isNotStale(l.chainTotalValues[l.childChains[i]].lastUpdate), 'stale');
    ...
    function _timeUntilExpiry() internal view returns (uint) {
        ...
        uint timeTillExpiry;
        for (uint8 i = 0; i < l.childChains.length; i++) {
            uint expiryTime = _timeUntilExpiry(l.chainTotalValues[l.childChains[i]].lastUpdate);
    ...
    function _unitPrice(
        ...
        price = (totalValueAcrossAllChains * Constants.VAULT_PRECISION) / totalShares;
    }
}
...
contract VaultParentInvestor is VaultParentInternal {
    ...
    function deposit(
        ...
        address asset,
        uint amount
    ) external noBridgeInProgress noWithdrawInProgress isInSync whenNotPaused nonReentrant {
    ...
    function withdrawMultiChain(
        ...
        uint amount,
        uint[] memory lzFees
    ) external payable noWithdrawInProgress noBridgeInProgress whenNotPaused nonReentrant {
    ...
    function getLzFee(bytes4 sigHash, uint16 chainId) external view returns (uint fee) {
    ...
    function totalValueAcrossAllChains() external view returns (uint minValue, uint maxValue) {
    ...
    function requiresSyncForWithdraw(uint tokenId) external view returns (bool) {
    ...
    function _deposit(uint tokenId, address asset, uint amount) internal {
        ...
        (uint depositValueInUSD, ) = _registry().accountant().assetValue(asset, amount);
        ...
        if (tokenId == 0) {
            require(depositValueInUSD >= _registry().minDepositAmount(), 'min deposit not met');
        }
        ...
        emit Deposit(msg.sender, issuedToTokenId, asset, amount, currentUnitPrice, shares);
        _registry().emitEvent();
    }
    ...
    function _withdrawMultiChain(uint tokenId, uint amount, uint[] memory lzFees) internal {
        ...
        _sendWithdrawRequestsToChildrenMultiChain(tokenId, msg.sender, portion, lzFees);

        emit WithdrawMultiChain(msg.sender, tokenId, portion, minUnitPrice, amount);
        _registry().emitEvent();
    }
    ...
    function _sendWithdrawRequestsToChildrenMultiChain(
        ...
        for (uint8 i = 0; i < l.childChains.length; i++) {
            totalFees += lzFees[i];
            _sendWithdrawRequest(l.childChains[i], tokenId, withdrawer, portion, lzFees[i]);
        }
    ...
    function _requiresUnitPrice(uint tokenId) internal view returns (bool) {
        ...
        if ((_managerPerformanceFee() == 0 && _holdings(tokenId).performanceFee == 0)) {
            return false;
        }
    ...
    function _requiresSyncForWithdraw(uint tokenId) internal view returns (bool) {
...
contract VaultParentManager is VaultParentInternal, IVaultParentManager {
    ...
    modifier onlyManagerOrTransport() {
        require(
            msg.sender == _manager() || msg.sender == address(_registry().transport()),
    ...
    function setDiscountForHolding(
        ...
        _setDiscountForHolding(tokenId, streamingFeeDiscount, performanceFeeDiscount);
    }

    function levyFeesOnHoldings(uint[] memory tokenIds) external onlyManager whenNotPaused {
        for (uint i = 0; i < tokenIds.length; i++) {
            require(
                block.timestamp >= _holdings(tokenIds[i]).lastManagerFeeLevyTime + 24 hours,
    ...
    function sendBridgeApproval(
        uint16 dstChainId,
        uint lzFee
    ) external payable onlyManager noBridgeInProgress whenNotPaused nonReentrant {
        VaultParentStorage.Layout storage l = VaultParentStorage.layout();
        require(msg.value >= lzFee, 'insufficient fee');
        ...
        require(l.lastBridgeCancellation + 1 hours < block.timestamp, 'bridge approval timeout');
        address dstVault = l.children[dstChainId];
        require(dstVault != address(0), 'no dst vault');
    ...
    function requestCreateChild(
        ...
        ITransport.ChildVault[] memory existingChildren = new ITransport.ChildVault[](
    ...
    function holdings(uint tokenId) external view returns (VaultOwnershipStorage.Holding memory) {
    ...
    function performanceFee(
        ...
        return _performanceFee(fee, discount, _totalShares, tokenPriceStart, tokenPriceFinish);
    }
    ...
    function MAX_STREAMING_FEE_BASIS_POINTS_STEP() external pure returns (uint) {
    ...
    function MAX_PERFORMANCE_FEE_BASIS_POINTS_STEP() external pure returns (uint) {
...
library VaultParentStorage {
    bytes32 internal constant STORAGE_SLOT = keccak256('valio.storage.vaultParent');
...
contract VaultParentTransport is VaultBaseInternal {
    ...
    function receiveChildCreated(uint16 childChainId, address childVault) external onlyTransport {
        VaultParentStorage.Layout storage l = VaultParentStorage.layout();
    ...
    function receiveBridgeApprovalCancellation(address requester) external onlyTransport {
        VaultParentStorage.Layout storage l = VaultParentStorage.layout();
