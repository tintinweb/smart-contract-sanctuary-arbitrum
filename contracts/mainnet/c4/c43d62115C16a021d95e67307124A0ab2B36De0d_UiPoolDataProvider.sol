...
interface IMToken is IERC20, IScaledBalanceToken, IInitializableMToken {
    ...
    /**
     * @notice Returns the address of the Aave treasury, receiving the fees on this mToken.
...
/**
 * @title   ManekiProtocolDataProvider
 * @author  maneki.finance
 * @notice  Peripheral contract to collect and pre-process information from the Pool.
 * @dev     Based on AaveV3's AaveProtocolDataProvider
 */
contract ManekiProtocolDataProvider is IPoolDataProvider {
        ...
        )
    {
        ...
        return (
            ...
            IERC20Metadata(reserve.mTokenAddress).totalSupply(),
    ...
    function getMTokenTotalSupply(
        ...
        return IERC20Metadata(reserve.mTokenAddress).totalSupply();
    }
        ...
        )
    {
        ...
        currentMTokenBalance = IERC20Metadata(reserve.mTokenAddress).balanceOf(
            user
        );
...
import {ManekiProtocolDataProvider} from "../misc/ManekiProtocolDataProvider.sol";
import {WadRayMath} from "../protocol/libraries/math/WadRayMath.sol";
import {ReserveConfiguration} from "../protocol/libraries/configuration/ReserveConfiguration.sol";
import {UserConfiguration} from "../protocol/libraries/configuration/UserConfiguration.sol";
import {DataTypes} from "../protocol/libraries/types/DataTypes.sol";
...
contract UiPoolDataProvider is IUiPoolDataProvider {
    ...
    function getReservesData(
        ...
        returns (AggregatedReserveData[] memory, BaseCurrencyInfo memory)
    {
        IManekiOracle oracle = IManekiOracle(provider.getPriceOracle());
        IPool pool = IPool(provider.getPool());
        ManekiProtocolDataProvider poolDataProvider = ManekiProtocolDataProvider(
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            reserveData.availableLiquidity = IERC20Metadata(
                reserveData.underlyingAsset
            ).balanceOf(reserveData.mTokenAddress);
            (
    ...
    function getUserReservesData(
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            userReservesData[i].scaledMTokenBalance = IMToken(
...
library ReserveConfiguration {
    ...
    uint16 public constant MAX_RESERVES_COUNT = type(uint16).max;
    ...
    function calculateInterestRates(
        ...
        if (vars.totalDebt != 0) {
            ...
            vars.availableLiquidity =
                IERC20(params.reserve).balanceOf(params.mToken) +
