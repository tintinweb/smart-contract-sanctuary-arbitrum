...
contract PaalXSwapContract is Context, Ownable, ReentrancyGuard {
    ...
    event Buy(address indexed sender, uint256 amountIn, address indexed tokenAddr, uint256 amountOut);
    event Sell(address indexed sender, uint256 amountIn, address indexed tokenAddr, uint256 amountOut);        
    event SellTokensForTokens(address indexed sender, address indexed tokenIn, uint256 amountIn, address indexed tokenOut, uint256 amountOut);
     ...
     */
    function getAmountAfterFee(uint256 _amount) internal view returns (uint256) {
        return (_amount * (swappingFeeDivisior - swappingFee)) / swappingFeeDivisior;
    }
     ...
     */ 
    function swapExactETHForTokens(address _tokenOut, uint256 _amountOutMin, uint256 _deadline) external payable nonReentrant returns (uint256 amountOut){
     ...
     */    
    function swapExactTokensForETH(address _tokenIn, uint256 _amountIn, uint256 _amountOutMin, uint256 _deadline) external nonReentrant returns (uint256 amountOut){        
        require(!isStopped, "PAALXSWAP: Contract is not active");
     ...
     * @return amountOut the amount of tokens received.
     */
    function swapExactTokensForTokens(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin, uint256 _deadline, bool _hasPair) external nonReentrant returns (uint256 amountOut) {
        require(!isStopped, "PAALXSWAP: Contract is not active");
        require(_tokenIn != address(0) && _tokenOut != address(0), "PAALXSWAP: Invalid tokenIn/tokenOut address");
        require(_amountIn > 0, "PAALXSWAP: Invalid amountIn");
        ...
        uint256 initialBalanceTokenIn = tokenInContract.balanceOf(address(this));
        ...
        uint256 actualReceived = tokenInContract.balanceOf(address(this)) - initialBalanceTokenIn;    
        ...
        emit SellTokensForTokens(msg.sender, _tokenIn, _amountIn, _tokenOut, amountOut);
    }
    ...
    function setRouter(IUniswapV2Router02 _newRouter) external onlyOwner {
        require(address(_newRouter) != address(0), "PAALXSWAP: Invalid address");
        router = _newRouter;
    }
     ...
     */
    function withdrawERC20(address _tokenAddress) external onlyOwner returns (bool result){
    ...
    function setSwappingFee(uint256 _swappingFee, uint256 _swappingFeeDivisior) external onlyOwner {        
        ...
        require(_swappingFee * 100 / _swappingFeeDivisior < 5, "PAALXSWAP: Fee must be < 5%");
