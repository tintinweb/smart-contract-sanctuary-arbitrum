...
contract PerpetualVault is Initializable, Ownable2StepUpgradeable, ReentrancyGuardUpgradeable {
  ...
  struct DepositInfo {
    ...
    uint256 shares;
    address owner;
    uint256 blockNumber;      // this is unnecessary now because individual withdrawal is disabled
    ...
    if (orderResultData.orderType == Order.OrderType.MarketIncrease) {
      ...
      if (flow == FLOW.DEPOSIT) {
        ...
        if (callbackdata.length > 0) {
          (_leverage) = abi.decode(callbackdata, (uint256));
        } else {
          _leverage = leverage;
        }
      ...
      } else {      // same as if (flow == FLOW.SIGNAL_CHANGE || FLOW.COMPUND)
  ...
  function _createIncreasePosition(
    ...
    bytes memory callbackdata = abi.encode(_leverage);
    _gmxLock = true;
    gmxUtils.createOrder(orderType, orderData, marketPrices, callbackdata);
  }
  ...
  function _distribute(uint256 withdrawn, bool positionClosed) internal {
    ...
    if (positionClosed) {
      ...
      distributionAmount = withdrawn + balanceBeforeWithdrawal * sumOfShares / totalShares;
    }
