...
interface IFixtool {
    ...
    function getBridgeTokenAndAmount(bytes memory data) external pure returns (address, uint256, uint64, address, bytes memory);
}
...
contract ThetaV2 is Ownable {
    ...
    event EachBridge(address user, address srcToken, uint256 fromAmount);
    ...
    //setter
    function setSelector(bytes4 _selector, address _selectorAddr) external onlyOwner {
        functionSelectInfo[_selector].check = true;
        functionSelectInfo[_selector].selectorAddr = _selectorAddr;
    }
    ...
    //getter
    function getSelectorChecker(bytes4 _selector) external view returns (bool) {
        return functionSelectInfo[_selector].check;
    }
    ...
    function swapThetaV2Call(SwapData calldata _swap, ThetaValue[] memory thetas, SplitData memory splitData) public payable {
        ...
        for (uint256 i = 0; i < _swap.output.length; ) {
            ...
            if (_swap.output[i].dstToken != address(NATIVE_ADDRESS)) {
        ...
        if (_swap.output.length == 1) {
            ...
            for (uint256 i = 0; i < splitData.splitRate[0].length; ) {
                ...
                if (_swap.output[0].dstToken == address(NATIVE_ADDRESS)) {
            ...
            if (splitData.multiStandard == true) {
                ...
                for (uint256 i = 0; i < splitData.splitRate.length; i++) {
                    ...
                    for (uint256 j = 0; j < currentSplitRates.length; j++) {
    ...
    function _thetaV2Call(ThetaValue[] memory thetas) internal {
        for (uint256 i = 0; i < thetas.length; ) {
            (bool success, ) = PLEXUS.call{value: thetas[i].value}(thetas[i].callData);
            if (!success) revert THETA_V2_FAILED();
            unchecked {
                ++i;
            }
        }
    ...
    function _approvePlexus(address srcToken, uint256 amount) internal {
        ...
        if (srcToken != address(NATIVE_ADDRESS)) {
    ...
    function _bridgeSwapStart(SwapData calldata swap) internal returns (uint256[] memory) {
        ...
        if (succ) {
            for (uint i; i < length; ) {
                dstTokenBalance[i] = _getBalance(swap.output[i].dstToken) - initDstTokenBalance[i];
                unchecked {
                    ++i;
                }
            }
