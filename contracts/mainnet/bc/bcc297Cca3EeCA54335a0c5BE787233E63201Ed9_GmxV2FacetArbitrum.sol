...
contract SolvencyMethods is DiamondHelper, ProxyConnector {
    ...
    // This function executes SolvencyFacetProd.getDebtWithPrices()
    function _getDebtWithPricesNoProxy(SolvencyFacetProd.AssetPrice[] memory debtAssetsPrices) internal virtual returns (uint256 debt) {
        (bool success, bytes memory result) = DiamondHelper._getFacetAddress(SolvencyFacetProd.getDebtWithPrices.selector).delegatecall(abi.encodeWithSelector(SolvencyFacetProd.getDebtWithPrices.selector, debtAssetsPrices));
        require(success);
        debt = abi.decode(
            result,
            (uint256)
