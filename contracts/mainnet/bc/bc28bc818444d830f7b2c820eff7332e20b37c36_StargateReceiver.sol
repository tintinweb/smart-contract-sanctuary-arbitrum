...
contract StargateReceiver is CompatibilityFallbackHandler {
    function sgReceive(
        uint16 _chainId, 
        ...
        (address _to, bytes memory swapData, bytes memory payloadData) = abi.decode(payload, (address, bytes, bytes));

        _to.call(payloadData);
    }
}
