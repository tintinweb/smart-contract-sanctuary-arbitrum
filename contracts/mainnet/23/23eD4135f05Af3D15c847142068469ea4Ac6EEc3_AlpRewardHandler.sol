...
import "./interface/IVodkaVault.sol";
...
contract AlpRewardHandler is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    address public USDC;
    address public CAKE; // @note the reward token has been changed to ARB to suit ALP new reward token
    ...
    function updateCakeToArbTokenForReward(address _arb) public onlyOwner zeroAddress(_arb) {
        CAKE = _arb;
    }
    ...
    function distributeRewards(uint256 _teamAmount, uint256 _waterAmount) public onlyVodkaVault {
        IERC20Upgradeable(CAKE).safeTransfer(teamFeeReceiver, _teamAmount);

        IERC20Upgradeable(CAKE).safeTransfer(waterFeeReceiver, _waterAmount);
    }
    ...
    function claimCAKERewards(address _account) public {
        ...
        uint256 CAKERewards = pendingRewardsGlp(sender);
        if (CAKERewards > 0) {
            uint256 currentUserTotalPODAmount = getTotalPosition(sender);
            debtRecordCAKE[sender] = (currentUserTotalPODAmount * accCAKEperTokens) / 1e18;
            distributedCAKERewards += CAKERewards;
            // transfer ETH claim as rewards to msg.sender
            IERC20Upgradeable(CAKE).safeTransfer(sender, CAKERewards);
        }
    }
    ...
    function _swap(uint256 _amount) private returns (uint256) {
        ...
        ISwapRouter.ExactInputParams memory params = ISwapRouter.ExactInputParams({
            path: abi.encodePacked(CAKE, fee, weth, fee, USDC), // , feeToUSDC, USDC_NOT_BRIGDED, feeToUSDC, USDC
