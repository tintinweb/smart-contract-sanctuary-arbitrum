...
contract RewardDistributor is Ownable, ReentrancyGuard {
    ...
    address public constant xNFTE = address(0x28B19DEba0D67Efe1497dB1Cef0Bf63fC3ee04d3); // xNFTE contract address
        ...
        returns (uint256)
    {
        ...
        if (amount > 0) {
            ...
            if (restake) {
                ...
                IERC20(USDC).safeApprove(xNFTE, amount);
                IxNFTE(xNFTE).depositFor(addr, uint128(amount));
            } else {
                IERC20(USDC).safeTransfer(addr, amount);
            }
        }
