...
interface FullProtec{
    function totalStaked() external view returns(uint);
}

contract ArbChef is Ownable, ReentrancyGuard {
    ...
    uint256 public compoundRatio = 36e12;
    ...
    function massUpdatePools() public {
        ...
        uint newRewardPerBlock = fullProtec.totalStaked()*rewardRatio/1e18;
        if(newRewardPerBlock != rewardPerBlock){
            rewardPerBlock = newRewardPerBlock;
            emit LogRewardPerBlock(rewardPerBlock);
        }
    }
    ...
    function claim(uint256 _pid, address _account) external nonReentrant {
        ...
        if (start && lastBlock != block.number) {
            eggs.rebase(
                block.number,
    ...
    function changeRewardRatio(uint _ratio) public onlyOwner{
        rewardRatio = _ratio;
    }

    function toStart(uint blockNumber) public onlyOwner{
        if(blockNumber == 0){
            lastBlock = block.number;
        }else{
