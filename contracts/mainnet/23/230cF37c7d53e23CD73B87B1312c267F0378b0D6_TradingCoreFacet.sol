...
import {ITradingCoreError} from "../../utils/Errors.sol";
...
contract TradingCoreFacet is ITradingCore, ITradingCoreError, OnlySelf {
    ...
    function slippagePrice(
        ...
        if (isLong) {
            ...
            if (sc.slippageType == SlippageType.ONE_PERCENT_DEPTH) {
                ...
                slippage = ((pairQty.longQty + qty) * marketPrice - sc.longThresholdUsd) * 1e4 / sc.onePercentDepthAboveUsd + 1;
            }
            ...
            if (sc.slippageType == SlippageType.ONE_PERCENT_DEPTH) {
                ...
                slippage = ((pairQty.shortQty + qty) * marketPrice - sc.shortThresholdUsd) * 1e4 / sc.onePercentDepthBelowUsd + 1;
            }
            return marketPrice * (1e4 - slippage) / 1e4;
        }
    }
    ...
    function triggerPrice(
        ...
        if (isLong) {
            ...
            if (sc.slippageType == SlippageType.ONE_PERCENT_DEPTH) {
                ...
                slippage = s > sc.slippageLongP ? s : sc.slippageLongP;
            } else if (sc.slippageType == SlippageType.THRESHOLD && (pairQty.longQty + qty) * limitPrice >= sc.longThresholdUsd) {
                slippage = ((pairQty.longQty + qty) * limitPrice - sc.longThresholdUsd) * 1e4 / sc.onePercentDepthAboveUsd + 1;
            }
            ...
            if (sc.slippageType == SlippageType.ONE_PERCENT_DEPTH) {
                ...
                slippage = s > sc.slippageShortP ? s : sc.slippageShortP;
            } else if (sc.slippageType == SlippageType.THRESHOLD && (pairQty.shortQty + qty) * limitPrice >= sc.shortThresholdUsd) {
                slippage = ((pairQty.shortQty + qty) * limitPrice - sc.shortThresholdUsd) * 1e4 / sc.onePercentDepthBelowUsd + 1;
            }
            return limitPrice * (1e4 + slippage) / 1e4;
        }
    }
...
import "../interfaces/ITradingCore.sol";
import {PairMaxOiAndFundingFeeConfig, IPairsManager} from "../interfaces/IPairsManager.sol";
import "@openzeppelin/contracts/utils/math/SignedMath.sol";
...
library LibTradingCore {
    ...
    function fundingFeeRate(
        ...
        PairMaxOiAndFundingFeeConfig memory pairConfig = IPairsManager(address(this)).getPairConfig(pairBase);
        int256 fundingFeeR;
        if (ppi.longQty != ppi.shortQty) {
