...
/**
 * @dev Representation of each ArbiFLUX Lock-in
 */
struct AddressLock {
    /**
     * @dev ArbiFLUX locked-in amount
     */
    uint256 amount;

    /**
     * @dev How much Brain was burned
     */
...
/**
 * @dev Brainlet Finance - Brain Smart Contract
 */
contract BrainletFinanceToken is ERC777, IERC777Recipient {
    /**
    ...
    /**
     * @dev ArbiFLUX must be locked-in to execute this function
     */
    modifier requireLocked(address targetAddress, bool requiredState) {
        if (requiredState) {
            require(addressLocks[targetAddress].amount != 0, "You must have locked-in your ArbiFLUX tokens");
        }else{
            require(addressLocks[targetAddress].amount == 0, "You must have unlocked your ArbiFLUX tokens");
        }
    ...
    /**
     * @dev This will be ArbiFLUX token smart contract address
     */
    ...
    /**
     * @dev Decline some incoming transactions (Only allow Brain smart contract to send/recieve ArbiFLUX tokens)
     */
    function tokensReceived(
        address operator,
        address from,
        ...
        require(_msgSender() == address(_token), "You can only lock-in ArbiFLUX tokens");

        // Ensure someone doesn't send in some BRAIN to this contract by mistake (Only the contract itself can send itself BRAIN)
        require(operator == address(this) , "Only BRAIN contract can send itself ArbiFLUX tokens");
        require(to == address(this), "Funds must be coming into BRAIN token");
        require(from != to, "Why would BRAIN contract send tokens to itself?");
    }
    ...
    /**
     * @dev How long until you can lock-in any ArbiFLUX token amount
     */
    uint256 immutable private _failsafeTargetBlock;
	 
    constructor(address token, uint256 startTimeReward, uint256 maxTimeReward, uint256 failsafeBlockDuration) public ERC777("Brainlet Finance", "BRAIN", new address[](0)) {
    ...
    /**
     * @dev How much max ArbiFLUX can you lock-in during failsafe duration?
     */
     uint256 private constant _failsafeMaxAmount = 100 * (10 ** 18);

    /**
     * @dev 0.1 BRAIN minted/block/1 ArbiFLUX
     * @dev 10^18 / 10^8 = 10^10
     */
    uint256 private constant _mintPerBlockDivisor = 10 ** 1;

    /**
     * @dev To avoid small BRAIN/ArbiFLUX burn ratios we multiply the ratios by this number.
     */
    ...
    /**
     * @dev This is our max 100x BRAIN burn multiplier. It's multiplicative with the time multiplier.
     */
    uint256 private constant _maxBurnMultiplier = 1000000;

    /**
     * @dev This is our max 3x ArbiFLUX lock-in time multiplier. It's multiplicative with the burn multiplier.
     */
    ...
    /**
     * @dev PUBLIC FACING: Store how much locked in ArbiFLUX there is globally
     */
    uint256 public globalLockedAmount;
    
    /**
     * @dev PUBLIC FACING: Store how much is burned globally (only from the locked-in ArbiFLUX addresses)
     */
	...
	/**
	 * @dev PUBLIC FACING: Lock-in ArbiFLUX tokens with the specified address as the minter.
	 */
    function lock(address minterAddress, uint256 amount) 
        preventRecursion 
        preventSameBlock(_msgSender())
        requireLocked(_msgSender(), false) // Ensure ArbiFLUX is unlocked for sender
    public {
        require(amount > 0, "You must provide a positive amount to lock-in");

        // Ensure you can only lock up to 100 ArbiFLUX during failsafe period
        if (block.number < _failsafeTargetBlock) {
            require(amount <= _failsafeMaxAmount, "You can only lock-in up to 100 ArbiFLUX during failsafe.");
        }
        ...
        // Send [amount] of ArbiFLUX token from the address that is calling this function to BRAIN smart contract.
        IERC777(_token).operatorSend(_msgSender(), address(this), amount, "", ""); // [RE-ENTRANCY WARNING] external call, must be at the end
    }
    
	/**
	 * @dev PUBLIC FACING: Unlock any sender locked-in ArbiFLUX tokens
	 */
    function unlock() 
        preventRecursion 
        preventSameBlock(_msgSender())
        requireLocked(_msgSender(), true)  // Ensure ArbiFLUX is locked-in for sender
    public {
        AddressLock storage senderAddressLock = addressLocks[_msgSender()]; // Shortcut accessor
        ...
        // Send back the locked-in ArbiFLUX amount to person calling the method
        IERC777(_token).send(_msgSender(), amount, ""); // [RE-ENTRANCY WARNING] external call, must be at the end       
    }

	/**
	 * @dev PUBLIC FACING: Burn BRAIN tokens to a specific address
	 */
    function burnToAddress(address targetAddress, uint256 amount) 
        preventRecursion 
        requireLocked(targetAddress, true) // Ensure the address you are burning to has ArbiFLUX locked-in
    public {
        require(amount > 0, "You must burn > 0 BRAIN");
        ...
        // Call the normal ERC-777 burn (this will destroy BRAIN tokens). We don't check address balance for amount because the internal burn does this check for us.
        _burn(_msgSender(), amount, "", ""); // [RE-ENTRANCY WARNING] external call, must be at the end      
    }

	/**
	 * @dev PUBLIC FACING: Mint BRAIN tokens from a specific address to a specified address UP TO the target block
	 */
    function mintToAddress(address sourceAddress, address targetAddress, uint256 targetBlock) 
        preventRecursion 
        preventSameBlock(sourceAddress)
        requireLocked(sourceAddress, true) // Ensure the adress that is being minted from has ArbiFLUX locked-in
    public {
        ...
        // Call the normal ERC-777 mint (this will mint BRAIN tokens to targetAddress)
    ...
    function getMintAmount(address targetAddress, uint256 targetBlock) public view returns(uint256) {
        ...
        // Ensure this address has ArbiFLUX locked-in
        ...
        uint256 amount = targetAddressLock.amount; // Total of locked-in ArbiFLUX for this address
	...
	/**
	 * @dev PUBLIC FACING: Find out the current address ArbiFLUX lock-in time bonus (Using 1 block = 15 sec formula)
	 */
    ...
    /**
     * @dev PUBLIC FACING: Get ArbiFLUX/BRAIN burn ratio for a specific address
     */
    function getAddressRatio(address targetAddress) public view returns(uint256) {
        ...
        // Note that we multiply both ratios by the ratio multiplier before dividing. For tiny BRAIN/ArbiFLUX burn ratios.
        uint256 myRatio = addressBurnedAmount.mul(_ratioMultiplier).div(addressLockedAmount);
        return myRatio;
    }

    /**
     * @dev PUBLIC FACING: Get ArbiFLUX/BRAIN burn ratio for global (entire network)
     */
