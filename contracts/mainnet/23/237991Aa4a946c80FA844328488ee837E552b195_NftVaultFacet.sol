...
contract NftVaultFacet is BaseVault, INftVault {
    using SafeERC20 for IERC20;

    /// @notice Collects the fees associated with provided liquidity
    /// The contract must hold the erc721 token before it can collect fees
    function collectV3PositionFees(address tokenAddress, uint256 tokenId) external override onlyKeyHolder {
    ...
    /// @notice Collects the fees associated with provided liquidity
    /// The contract must hold the erc721 token before it can collect fees
    function reinvestV3PositionFees(address tokenAddress, uint256 tokenId, uint256 amount0Min, uint256 amount1Min) external override onlyKeyHolder {
    ...
    function partialNonFungibleTokenUnlock(address _tokenAddress, uint256 _tokenId) external override onlyKeyHolder onlyUnlockable {
        ...
        LibVaultStorage.DiamondStorage storage ds = LibVaultStorage.diamondStorage();
        IVaultFactory(ds.vaultFactory).notifyUnlock(_isCompletelyUnlocked());
    }

    function _isLockedNonFungibleAddress(address _tokenAddress) private view returns (bool) {
        LibVaultStorage.DiamondStorage storage ds = LibVaultStorage.diamondStorage();
        for (uint256 i = 0; i < ds.nonFungibleTokenDeposits[LibCommonConsts.INNER_STRUCT].length; i++) {
            if (_tokenAddress == ds.nonFungibleTokenDeposits[LibCommonConsts.INNER_STRUCT][i].tokenAddress) {
                return true;
            }
        }
