...
import "./interfaces/Gmx/callback/IDepositCallbackReceiver.sol";
import "./interfaces/Gmx/callback/IWithdrawalCallbackReceiver.sol";
...
contract TestGMX is IDepositCallbackReceiver, OwnableUpgradeable {
    ...
    uint256 public indexTokenPriceRecord;
    ...
    function getMarketTokenPrice() public returns (int256 marketTokenPrice, MarketPoolValueInfo.Props memory marketPoolValueInfo) {
        ...
        address _indexToken = 0xC4da4c24fd591125c3F47b340b6f4f76111883d8;

        Market.Props memory market = Market.Props({ marketToken: gmxasset, indexToken: _indexToken, longToken: weth, shortToken: usdc });

        Price.Props memory indexTokenPrice = IOracle(oracle).getPrimaryPrice(_indexToken);
        ...
        indexTokenPriceRecord = indexTokenPrice.min;

        bytes32 pnlFactorType = keccak256("MAX_PNL_FACTOR_FOR_DEPOSITS");

        bool maximize = true;

        //try using reader instaed of marketUtils

        (marketTokenPrice, marketPoolValueInfo) = IMarketUtils(0x38d91ED96283d62182Fc6d990C24097A918a4d9b).getMarketTokenPrice(
            dataStore,
            market,
            indexTokenPrice,
            longTokenPrice,
