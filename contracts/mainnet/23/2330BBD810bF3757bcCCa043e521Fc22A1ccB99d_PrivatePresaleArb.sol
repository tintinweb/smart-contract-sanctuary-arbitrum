...
contract PrivatePresaleArb is Ownable, ReentrancyGuard {
    ...
    uint public constant LOCK_TIME = 86400 * 30;
    ...
    function setTime(uint256 _startTime,uint256 _endTime,uint256 _claimTime) external onlyOwner {
        require(
        ...
        require(
            _claimTime > _endTime,
            "claim time must be greater than end time."
        );
    ...
    function commit() external payable nonReentrant {
        ...
        uint256 tokensToReceive = (tokensToSell * commitments[msg.sender]) /
            ethersToRaise;

        userTotalTokensToReceive[msg.sender] = tokensToReceive;
    ...
    function _simulateClaim(
        ...
        if (block.timestamp < endTime) return (userTotalTokensToReceive[account], 0);

        if (block.timestamp > endTime + LOCK_TIME) {
            ...
            uint256 baseTokenToReceive = (totalTokensToReceive * 300) / 1000;

            uint256 baseCalculationAmount = totalTokensToReceive - baseTokenToReceive;

            uint timeElapsed = block.timestamp - endTime;

            uint256 releaseAmt = (baseCalculationAmount * timeElapsed) / LOCK_TIME;

            uint256 totalReleaseAmt = baseTokenToReceive + releaseAmt;

            uint256 currentAvailableAmt = totalReleaseAmt -
                userClaimedAmount[account];
    ...
    function claim() external nonReentrant returns (uint256) {
        ...
        require(
            ...
            "Can only claim tokens after the claimTime."
        );

        (,uint256  currentAvailableAmt) = _simulateClaim(
            msg.sender
        );

        uint256 totalTobeClaim = currentAvailableAmt + userClaimedAmount[msg.sender];

        require(totalTobeClaim <= userTotalTokensToReceive[msg.sender], "error amount");

        salesToken.safeTransfer(msg.sender, currentAvailableAmt);

        emit Claim2(msg.sender, currentAvailableAmt);

        userClaimedAmount[msg.sender] += currentAvailableAmt;

        return currentAvailableAmt;
    }
