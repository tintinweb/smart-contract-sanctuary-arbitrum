...
contract TokenVesting_alphaEDE is Ownable {
    ...
    IERC20 immutable public payoutToken;
    IERC20 immutable public principalToken;
    IESBT public esbt;
    ...
    uint256[] public rankVestingDurationList = [365 days, 365 days, 295 days, 220 days, 170 days, 115 days, 90 days, 50 days];
    ...
    constructor(
        ...
        require(IERC20(_payOutToken).decimals() == IERC20(_principalToken).decimals(), "decimals mismatch");
    ...
    //set esbt
    function setESBT(address _esbt) external onlyPolicy {
        esbt = IESBT(_esbt);
    }

    //set rankVestingDurationList
    function setRankVestingDurationList(uint256[] memory _rankVestingDurationList) external onlyPolicy() {
    ...
    function setStart() external onlyPolicy returns (bool) {
    ...
    function deposit(uint _amount) external returns (uint) {
        ...
        uint256 rank = esbt.rank(msg.sender);
        require(rank > 0, "IESBT is needed before unlocking the tokens.");
        ...
        vesting : rankVestingDurationList[rank],
        lastBlockTimestamp : block.timestamp
        });

        //store currentVestingDuration
        currentVestingDuration[_depositor] = rankVestingDurationList[rank];
...
//        require(totalPayoutGiven <= payoutToken.balanceOf(address(this)), "Not enough tokens to pay");


        principalToken.safeTransferFrom(_depositor, address(this), _amount);
    ...
    //set price
    function setPrice(uint _price) external onlyPolicy() {
        price = _price;
    }

    //set price precision
    function setPricePrecision(uint _pricePrecision) external onlyPolicy() {
