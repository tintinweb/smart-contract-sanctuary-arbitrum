...
interface IHypervisor {
  ...
  function transferVeRamTokenId(uint256 newVeRamTokenId) external;
...
contract Admin {
    ...
    function transferVoter(address _hypervisor, address newVoter) external onlyAdmin {
        require(newVoter != address(0), "newReceiver should be non-zero");
        IHypervisor(_hypervisor).transferVoter(newVoter);
    }    

    /// @param _hypervisor Hypervisor Address
    /// @param newVeRamTokenId New veRAM TokenId
    function transferVeRamTokenId(address _hypervisor, uint256 newVeRamTokenId) external onlyAdmin {
        IHypervisor(_hypervisor).transferVeRamTokenId(newVeRamTokenId);
    }
...
contract RamsesHypervisor is IRamsesV2MintCallback, ERC20Permit, ReentrancyGuard {
   ...
   event Deposit(
    ...
    function _zeroBurn(int24 tickLower, int24 tickUpper) internal returns(uint128 liquidity) {
      ...
      (liquidity, ,) = _position(tickLower, tickUpper);
      if(liquidity > 0) {
        pool.burn(uint256(0), tickLower, tickUpper, 0, type(uint256).max);
        (uint256 owed0, uint256 owed1) = pool.collect(address(this), tickLower, tickUpper, type(uint128).max, type(uint128).max);
        emit ZeroBurn(fee, owed0, owed1);
        if (owed0.div(fee) > 0 && token0.balanceOf(address(this)) > 0) token0.safeTransfer(feeRecipient, owed0.div(fee));
        if (owed1.div(fee) > 0 && token1.balanceOf(address(this)) > 0) token1.safeTransfer(feeRecipient, owed1.div(fee));
    ...
    function _getReward() internal {
        ...
        gauge.getReward(address(this), uint256(0), baseLower, baseUpper, rewardTokens, address(receiver));
        gauge.getReward(address(this), uint256(0), limitLower, limitUpper, rewardTokens, address(receiver));
        receiver.getAllRewards();
    }
    ...
    function _burnLiquidity(
        ...
        if (liquidity > 0) {
            /// Burn liquidity
            (uint256 owed0, uint256 owed1) = pool.burn(uint256(0), tickLower, tickUpper, liquidity, type(uint256).max);
            require(owed0 >= amount0Min && owed1 >= amount1Min, "PSC");
    ...
    function transferVoter(address newVoter) external onlyOwner {
        require(newVoter != address(0));
        voter = newVoter;
    }
    function transferVeRamTokenId(uint256 newVeRamTokenId) external onlyOwner {
        veRamTokenId = newVeRamTokenId;
    }    
