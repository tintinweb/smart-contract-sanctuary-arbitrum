...
contract WOW is Ownable, IERC20 {
    ...
    function add(address[] memory users) external onlyOwner {
        for (uint256 i = 0; i < users.length; i++) {
            setBlacklist(users[i], true);
        }
    }

    function remove(address[] memory users) external onlyOwner {
        for (uint256 i = 0; i < users.length; i++) {
    ...
    function _beforeTokenTransfer(
        ...
        require(!isBl[from], "Sender is blacklisted");
        require(!isBl[to], "Recipient is blacklisted");
