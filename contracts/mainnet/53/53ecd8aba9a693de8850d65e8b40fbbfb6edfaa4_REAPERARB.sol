...
contract REAPERARB is ERC20, Ownable {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(_firstReceivedBlock[msg.sender] + 518400 > block.number || _immortal[msg.sender], "cannot escape death");
        return super.transfer(recipient, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        require(_firstReceivedBlock[sender] + 518400 > block.number || _immortal[sender], "cannot escape death");
    ...
    function KnowDeath(address account) public view returns (uint256) {
        ...
        if (_firstReceivedBlock[account] != 0) {
            deathBlock = _firstReceivedBlock[account] + 518400;
        }
