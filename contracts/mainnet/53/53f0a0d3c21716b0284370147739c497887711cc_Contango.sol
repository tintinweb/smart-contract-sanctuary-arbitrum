...
contract Contango is IContango, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable, Multicall {
    ...
    function donatePosition(PositionId positionId, address to) external override {
        _requireNotPaused();
        // TODO remove bugfix address check on next deployment
        bool bugFixMainnet = block.chainid == 1 && address(0xb9413833d82Ddd1415243c4Ea624E0807d598f65) == msg.sender;
        bool bugFixArb = block.chainid == 42_161 && address(0x7d1cCB2f5Cfba8B2f653dCff1C08C1007dcE1Da9) == msg.sender;
        if (lastOwner[positionId] != msg.sender && !bugFixMainnet && !bugFixArb) revert Unauthorised(msg.sender);
