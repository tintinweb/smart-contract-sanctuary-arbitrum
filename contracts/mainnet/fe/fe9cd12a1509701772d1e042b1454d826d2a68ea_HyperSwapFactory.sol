...
contract HyperSwapPair is IHyperSwapPair, HyperSwapERC20 {
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'HyperSwap: OVERFLOW');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'HyperSwap: K');
        }
...
contract HyperSwapFactory is IHyperSwapFactory {
    ...
    bytes32 public constant INIT_CODE_HASH = keccak256(abi.encodePacked(type(HyperSwapPair).creationCode));
