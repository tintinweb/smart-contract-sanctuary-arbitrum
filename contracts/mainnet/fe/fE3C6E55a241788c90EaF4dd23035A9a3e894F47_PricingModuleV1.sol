...
contract PricingModuleV1 is IPricingModule, AccessControlEnumerable {
    ...
    constructor(
        ...
        erc20TokenPrice = tokenPrice;
        erc721TokenPrice = nftTokenPrice;
        erc1155TokenPrice = multiTokenPrice;
        erc20VestedTokenPrice = vestedTokenPrice;
        lpTokenBasisPoints = _lpTokenBasisPoints;
    ...
    function setTokenPrice(uint256 newPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 oldPrice = erc20TokenPrice;
        erc20TokenPrice = newPrice;
    ...
    function setMultiTokenPrice(uint256 newPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 oldPrice = erc1155TokenPrice;
        erc1155TokenPrice = newPrice;
    ...
    function setVestedTokenPrice(uint256 newPrice) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 oldPrice = erc20VestedTokenPrice;
        erc20VestedTokenPrice = newPrice;
    ...
    function setLPBasisPoints(uint256 newBasisPoints) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 oldBasisPoints = lpTokenBasisPoints;
        lpTokenBasisPoints = newBasisPoints;
    ...
    function getPrice(
        ...
        for (uint256 i = 0; i < fungibleTokenDeposits.length; i++) {
            if (lpTokenProcessor.isV2LiquidityPoolToken(fungibleTokenDeposits[i].tokenAddress)) {
                ...
                lpV2Amounts[i] = (fungibleTokenDeposits[i].amount * lpTokenBasisPoints) / 10000;
            } else {
        ...
        for (uint256 i = 0; i < nonFungibleTokenDeposits.length; i++) {
            ...
            if (token0 != address(0)) {
                ...
                lpV3Tokens[i].liquidityToRemove = uint128((liquidity * lpTokenBasisPoints) / 10000);
    ...
    function _calculateFlatPrice(
        ...
        uint256 tokenPrice = isVested ? erc20VestedTokenPrice : erc20TokenPrice;
        ...
        if (hasNonFungibleToken) {
            usdCost += erc721TokenPrice;
        }
        if (hasMultiToken) {
            usdCost += erc1155TokenPrice;
        }
    }
