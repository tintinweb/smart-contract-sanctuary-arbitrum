...
import "./interfaces/IBabySpheres.sol";
...
contract BabySpheres is IBabySpheres, DefaultOperatorFilterer, ERC721Enumerable, ERC2981, Ownable {
    ...
    uint256 public nextTokenId = 101;
    ...
    constructor(address owner_, string memory baseURI_) ERC721("Baby Spheres Test", "Baby Sphere Test") {
    ...
    function mint() external {
        ...
        if (difference <= 1 hours) {
            if (!_privatePeriodAccounts.contains(msg.sender) || minted[msg.sender]) {
                revert ForbiddenToMintTokens();
            }
            ...
            if (
                !_whitelistPeriodAccounts.contains(msg.sender) || 
                !_privatePeriodAccounts.contains(msg.sender) || 
                minted[msg.sender]
            ) {
                revert ForbiddenToMintTokens();
            }
