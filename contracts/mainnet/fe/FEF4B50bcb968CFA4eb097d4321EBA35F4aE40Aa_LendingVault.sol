...
contract LendingVault is ERC20, ReentrancyGuard, Pausable, AccessManaged, ILendingVault {
  ...
  // Maximum allowed borrowers for this vault
  uint256 public maxBorrowers;
  ...
  event MaxCapacityUpdated(uint256 maxCapacity);
  event EmergencyPaused(address indexed guardian);
  event EmergencyResumed(address indexed guardian);
  ...
  event MaxBorrowersUpdated(uint256 maxBorrowers);
  ...
  */
  ...
  /**
    ...
    * @param _maxBorrowers  Maximum allowed borrowers
  ...
  */
  constructor(
    ...
    if (_performanceFee > SAFE_MULTIPLIER) revert Errors.PerformanceFeeExceeded();

    asset = _asset;
    isNativeAsset = _isNativeAsset;
    performanceFee = _performanceFee;
    maxCapacity = _maxCapacity;
    maxBorrowers = _maxBorrowers;
    treasury = _treasury;
  }
  ...
  */
  function borrowAPR() public view returns (uint256) {
    uint256 _borrowAPR;
    uint256 _approvedBorrowersLength = _approvedBorrowers.length();

    for (uint256 i = 0; i < _approvedBorrowersLength;) {
  ...
  */
  function depositNative(uint256 assetAmt, uint256 minSharesAmt) payable external nonReentrant whenNotPaused {
    ...
    if (assetAmt < MINIMUM_LIQUIDITY) revert Errors.InsufficientDepositAmount();
  ...
  */
  function withdraw(uint256 sharesAmt, uint256 minAssetAmt) external nonReentrant {
    ...
    if (isNativeAsset) {
      ...
      if (!success) {
        IWNT(address(asset)).deposit{value: _assetAmt}();
  ...
  */
  function _mintShares(uint256 assetAmt) internal returns (uint256) {
    ...
    if (totalSupply() == 0) {
      ...
      // Mint shares to dead address on first deposit to prevent inflation attack
      _mint(address(DEAD_ADDRESS), MINIMUM_LIQUIDITY);
    } else {
      _shares = assetAmt * totalSupply() / (totalAsset() - assetAmt);

      if (_shares == 0) revert Errors.InsufficientSharesMinted();
    }
  ...
  */
  function updateInterestRate(
    ...
    if (newInterestRate.kink1 > newInterestRate.kink2) revert Errors.InvalidInterestRateModel();
  ...
  */
  function updatePerformanceFee(uint256 newPerformanceFee) external restricted {
    if (newPerformanceFee > SAFE_MULTIPLIER) revert Errors.PerformanceFeeExceeded();
  ...
  */
  function approveBorrower(address borrower) external restricted {
    ...
    if (_approvedBorrowers.length() + 1 > maxBorrowers) revert Errors.ApprovedBorrowersExceededMaximum();

    bool _success = _approvedBorrowers.add(borrower);
    if (!_success) revert Errors.ApproveBorrowerFailure();
  ...
  */
  function revokeBorrower(address borrower) external restricted {
    if (!_approvedBorrowers.contains(borrower)) revert Errors.BorrowerAlreadyRevoked();

    bool _success = _approvedBorrowers.remove(borrower);
    if (!_success) revert Errors.RevokeBorrowerFailure();
  ...
  */
  function updateMaxCapacity(uint256 newMaxCapacity) external restricted {
    ...
    emit MaxCapacityUpdated(newMaxCapacity);
  }
  ...
  */
  ...
  /**
    * @notice Update maximum allowed borrowers for this vault
    * @param newMaxBorrowers Maximum borrowers allowed in uint256
  */
  function updateMaxBorrowers(uint256 newMaxBorrowers) external restricted {
    if (newMaxBorrowers < _approvedBorrowers.length()) revert Errors.ApprovedBorrowersExceededMaximum();

    maxBorrowers = newMaxBorrowers;

    emit MaxBorrowersUpdated(newMaxBorrowers);
  }
  ...
  */
  receive() external payable {
    if (msg.sender != address(asset)) revert Errors.OnlyNonNativeDepositToken();
  }
}
