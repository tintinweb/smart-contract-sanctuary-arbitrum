...
contract stakingmerge is Ownable {
    ...
    function unstaking() external {
        ...
        if (withdrawFee > 0) {
            IERC20(stakeTokenAddress).burn(amount.mul(10).div(1000));

            IERC20(stakeTokenAddress).transfer(
                marketingAddress,
                amount.mul(10).div(1000)
            );
        }
    ...
    function claimRewards() external {
        ...
        require(block.timestamp - stakers[stakerAddress].lastUpdateTime  >= 36000, "You can only claim once every 10 hours");
