    ...
    ReentrancyGuard
{
    ...
    /// silo incentive rewards controller address
    ISiloIncentivesController siloIncentivesController;

    /// vault address
    address public vault;

    /// fee recipient address that receives strategy fee
    address public feeRecipient;

    /// withdrawal fee
    uint256 public withdrawalFee;

    /// withdrawal precision for calculating fee
    ...
    event TransferToken(uint256 amount, bool _isDeposit);
    event SetWithdrawalFee(uint256 fee);
    ...
    /// @notice initializer
    /// @param _want deposit token address
    /// @param _router silo router address
    /// @param _silo silo pool address
    /// @param _siloIncentivesController silo incentive rewards controller address
    /// @param _vault vault(ALP) contract address
    /// @param _feeRecipient fee recipient address
    /// @param _withdrawalFee withdrawal fee
    function initialize(
        address _want,
        ISiloRouter _router,
        ISilo _silo,
        ISiloIncentivesController _siloIncentivesController,
        address _vault,
        address _feeRecipient,
        uint256 _withdrawalFee
    ) public initializer {
        require(_want != address(0), "Invalid want address");
        require(address(_router) != address(0), "Invalid siloRouter address");
        require(address(_silo) != address(0), "Invalid silo address");
        require(_feeRecipient != address(0), "Invalid fee recipient address");
        require(_vault != address(0), "Invalid vault address");
        require(
            address(_siloIncentivesController) != address(0),
            "Invalid silo incentives controller address"
        );
        ...
        vault = _vault;
        siloIncentivesController = _siloIncentivesController;

        IERC20(want).approve(address(_router), type(uint256).max);
    }
    ...
    function transferToken(
        ...
        uint256 wantBalance = IERC20(_token).balanceOf(vault);
        ...
        if (_isDeposit) {
            ...
            IERC20(_token).safeTransferFrom(vault, address(this), wantBalance);
        } else {
            wantBalance = IERC20(_token).balanceOf(address(this));

            if (wantBalance > _amount) {
                wantBalance = _amount;
            }

            IERC20(_token).safeTransfer(vault, wantBalance);
        }
        emit TransferToken(wantBalance, _isDeposit);
    }

    /// @notice get total amount of want token
    ///         strategy balance + pool staked balance
    /// @param token asset address
    /// @return balance token balance
    function getComponentAmount(address token) external view returns (uint256) {
        require(token == want, "Token is not want");

        ISilo.AssetStorage memory assetStorage = silo.assetStorage(token);
        uint256 tokenDeposits = assetStorage.totalDeposits;
        uint256 totalSupply = assetStorage.collateralToken.totalSupply();
        uint256 shareBalance = assetStorage.collateralToken.balanceOf(
            address(this)
        );

        uint256 poolBalance = (shareBalance * tokenDeposits) / totalSupply;
        uint256 wantBalance = IERC20(want).balanceOf(address(this));
        return poolBalance + wantBalance;
    }

    /// @notice get pending rewards against pool stake
        ...
        returns (uint256 pendingRewards)
    {
        ...
        pendingRewards = siloIncentivesController.getRewardsBalance(
            assets,
            address(this)
        );
    }
    ...
    function emergencyWithdraw(
        ...
        actions[0] = ISiloRouter.Action(
            ISiloRouter.ActionType.Withdraw,
            silo,
            IERC20(want),
            type(uint256).max,
            _collateralOnly
        ...
        uint256 wantBalance = IERC20(want).balanceOf(address(this));
        IERC20(want).safeTransfer(_msgSender(), wantBalance);

        emit EmergencyWithdraw(wantBalance);
    }
    ...
    function withdraw(
        ...
        _withdraw(_amount, _collateralOnly);
    }
    ...
    function pause() external onlyOwner {
        if (paused() == true) {
            IERC20(want).approve(address(siloRouter), type(uint256).max);
            _unpause();
        } else {
            _pause();
            IERC20(want).approve(address(siloRouter), 0);
        }
    }

    /// @notice _deposit internal
    /// @param _collateralOnly true if deposit should be used for collateral only. Otherwise false.
    function _deposit(bool _collateralOnly) internal {
        uint256 wantBalance = IERC20(want).balanceOf(address(this));

        if (wantBalance > 0) {
            ISiloRouter.Action[] memory actions = new ISiloRouter.Action[](1);
            actions[0] = ISiloRouter.Action(
                ISiloRouter.ActionType.Deposit,
                silo,
                IERC20(want),
                wantBalance,
                _collateralOnly
    ...
    /// @notice _withdraw internal
    /// @param _amount withdrawal token amount
    /// @param _collateralOnly true if collateral only tokens are to be withdrawn. Otherwise false.
    function _withdraw(uint256 _amount, bool _collateralOnly) internal {
        //uint256 wantBalance = IERC20(want).balanceOf(address(this));
        ...
        // if (withdrawalFee > 0) {
        //     uint256 fee = (wantBalance * withdrawalFee) / PRECISION;
        //     IERC20(want).safeTransfer(feeRecipient, fee);
        //     wantBalance = wantBalance - fee;
        // }

        emit Withdraw();
    }

    /// @notice _claim internal
    /// @param _amount claimable amount
    /// @return amount claimed amount
    function _claim(uint256 _amount) internal returns (uint256) {
        address[] memory assets = new address[](1);
        assets[0] = want;
        uint256 claimedRewards = siloIncentivesController.claimRewardsToSelf(
            assets,
            _amount
        );
        emit Claim(_amount);
