...
contract Main is AutomationCompatibleInterface, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function deployDraw(
        ...
        returns (string memory)
    {
        string memory cid = string(abi.encodePacked(keccak256(abi.encodePacked(numbers, range, algorithm, scheduledAt, drawCount, block.number))));
    ...
    function deployDraw(
        ...
        returns (string memory)
    {
        string memory cid = string(abi.encodePacked(keccak256(abi.encodePacked(scheduledAt, nbParticipants, nbWinners, algorithm, callbackAddress, drawCount, block.number))));
