...
contract GlmManager is IDepositCallbackReceiver, IWithdrawalCallbackReceiver, IGlmManager, OwnableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    address public glmVault;
    address public glmRouter;
    address public glmPriceReader;
    //@todo add setter for this variable
    address public glmFeeReceiver;

    address public roleStore;

    address public uniV3Router;
    uint24 public uniV3Fee; //fee = 500 = 0.05%
    ...
    uint256 public impactFeeMultiplier; // 1000 => 10% of the difference between target weight and current weight @todo may need to handle decimal
    //@todo need to change MAX_BPS
    uint16 internal constant MAX_BPS = 10_000;
    uint256 internal constant PRICE_PRECISION = 1e30;
    address public tempPayableAddress;

    GmPoolInfo[] public gmPools;
    //@todo consider delete this data structure and use a for loop for view func
    ...
    function setGlmAddresses(address _glmVault, address _glmRouter, address _glmPriceReader) external onlyOwner {
        glmVault = _glmVault;
        glmRouter = _glmRouter;
        glmPriceReader = _glmPriceReader;
        emit SetGlmAddresses(_glmVault, _glmRouter, _glmPriceReader);
    }
    ...
    function setTokenAddresses(address _WETH, address _USDC) external onlyOwner {
        WETH = _WETH;
        USDC = _USDC;
        emit SetTokenAddresses(_WETH, _USDC);
    }

    //function to set impactFeeMultiplier and isFeeActive
    function setImpactFeeMultiplier(uint256 _impactFeeMultiplier, bool _isFeeActive) external onlyOwner {
        //require it smaller than a certain amount
        impactFeeMultiplier = _impactFeeMultiplier;
        isFeeActive = _isFeeActive;
        emit SetImpactFeeMultiplier(_impactFeeMultiplier, _isFeeActive);
    }
    ...
    function setGmPool(
        ...
        require(_targetWeight <= PRICE_PRECISION, "Target weight is too high");
    ...
    //@todo can be maanged by setPool? it will change the order of pool?
    ...
    function getCurrentWeights() public view returns (uint256[] memory currentWeights) {
        ...
        for (uint256 i; i < gmPools.length; i++) {
            currentWeights[i] = getCurrentWeight(i);
        }
        return currentWeights;
    }

    //amount is in USDC
    function getRatioImpactFeeBps(uint256 _index, uint256 _usdcAmount, bool _isDeposit) public view returns (uint256 ratioImpactFeeBps) {
        uint256 weightAfterChange = getCurrentWeightWithChange(_index, _usdcAmount, _isDeposit);
        uint256 targetWeight = getTargetWeight(_index);

        if (_isDeposit) {
            //after deposit, the weight is higher than target weight => impact fee
            //handles an already overweighted pool?
            if (weightAfterChange > targetWeight) {
                return ((weightAfterChange - targetWeight) * impactFeeMultiplier) / 10000;
            } else {
                //@note the pool is underweighting or equal to traget weight => no fee
                return 0;
            }
        } else {
            //handle withdrawal
            if (weightAfterChange < targetWeight) {
                return ((targetWeight - weightAfterChange) * impactFeeMultiplier) / 10000;
            } else {
    ...
    function getPoolRatioAfterWithdrawal(uint256 _index, uint256 _glmAmount) public view returns (uint256 minimumRatio) {
        //get pool tvl
        uint256 poolTvl = getGmTokenValueInUsdc(
            gmPools[_index].marketToken,
            IERC20Upgradeable(gmPools[_index].marketToken).balanceOf(glmVault)
        );
        uint256 totalTvl = IERC4626Upgradeable(glmVault).totalAssets();
        uint256 glmPrice = IGlmPriceReader(glmPriceReader).getGlmPrice();
        uint256 glmAmountInUsdc = (glmPrice * _glmAmount) / 1e6;

        return ((poolTvl - glmAmountInUsdc) * PRICE_PRECISION) / (totalTvl - glmAmountInUsdc);
    }

    //@note amount in USDC
    function getCurrentWeightWithChange(uint256 _index, uint256 _usdcAmount, bool _isDeposit) public view returns (uint256 currentWeight) {
        if (IERC4626Upgradeable(glmVault).totalAssets() > 0) {
            uint256 currentGmTokenValue = (
                getGmTokenValueInUsdc(gmPools[_index].marketToken, IERC20Upgradeable(gmPools[_index].marketToken).balanceOf(glmVault))
            );
            return
                _isDeposit
                    ? ((currentGmTokenValue += _usdcAmount) * PRICE_PRECISION) / (IERC4626Upgradeable(glmVault).totalAssets() + _usdcAmount)
                    : ((currentGmTokenValue -= _usdcAmount) * PRICE_PRECISION) /
                        (IERC4626Upgradeable(glmVault).totalAssets() - _usdcAmount);
        } else {
            return 0;
        }
    }
    ...
    function getCurrentWeight(uint256 _index) public view returns (uint256 currentWeight) {
        if (IERC4626Upgradeable(glmVault).totalAssets() > 0) {
            ...
            uint256 assetValueInUsdc = getGmTokenValueInUsdc(marketToken, IERC20Upgradeable(marketToken).balanceOf(glmVault));

            uint256 totalAsset = IERC4626Upgradeable(glmVault).totalAssets();

            currentWeight = (assetValueInUsdc * PRICE_PRECISION) / totalAsset;
    ...
    function getMostOverWeightedPool() public view returns (uint256 index) {
        uint256 max;
        uint256 maxIndex;
        for (uint256 i; i < gmPools.length; i++) {
            address marketToken = gmPools[i].marketToken;
            uint256 balance = IERC20Upgradeable(marketToken).balanceOf(glmVault);

            if (balance == 0) {
                continue;
            }

            uint256 currentWeight = getCurrentWeight(i);
    ...
    function getMostUnderWeightedPool() public view returns (uint256 index) {
        uint256 max;
        uint256 maxIndex;
        for (uint256 i; i < gmPools.length; i++) {
            uint256 currentWeight = getCurrentWeight(i);
    ...
    function getGlmVaultGmTokenBalance(address _gmToken) public view returns (uint256) {
        return IERC20Upgradeable(_gmToken).balanceOf(glmVault);
    }

    function getGmTokenValueInUsdc(address _gmToken, uint256 _amount) public view returns (uint256) {
        uint256 gmTokenPrice = IGlmPriceReader(glmPriceReader).getMarketTokenPrice(_gmToken);
        return ((gmTokenPrice * _amount) * 1e6) / 1e30 / 1e18;
    }
    ...
    //function to get GmTokenAddresss by its long Token
    ...
    {
        for (uint256 i; i < gmPools.length; i++) {
            if (gmPools[i].longToken == _longToken) {
                return (
                    gmPools[i].marketToken,
                    gmPools[i].indexToken,
                    gmPools[i].shortToken,
                    gmPools[i].longToken,
                    gmPools[i].targetWeight,
                    gmPools[i].minimumWeight
                );
            }
        }
    }
    ...
    function depositFromRouter(address gmToken, uint256 gmTokenAmount, address user) external {
        require(msg.sender == glmRouter, "Request is not from GlmRouter");
        ...
        uint256 feeInGm;
        if (isFeeActive) {
            uint256 ratioImpactFeeBps = getRatioImpactFeeBps(poolId, GmTokenValueInUsdc, true);
            feeInUsdc = (GmTokenValueInUsdc * ratioImpactFeeBps) / MAX_BPS;
            feeInGm = (gmTokenAmount * ratioImpactFeeBps) / MAX_BPS;
            GmTokenValueInUsdc -= feeInUsdc;
            gmTokenAmount -= feeInGm;
            IERC20Upgradeable(gmToken).safeTransfer(glmFeeReceiver, feeInGm);
        }

        IERC4626Upgradeable(glmVault).deposit(GmTokenValueInUsdc, user);
        IERC20Upgradeable(gmToken).safeTransfer(glmVault, gmTokenAmount);

        emit DepositFromRouter(gmToken, gmTokenAmount, user, feeInUsdc);
    }
    ...
    function afterDepositExecution(bytes32 key, Deposit.Props memory deposit, EventUtils.EventLogData memory eventData) external {
        require(deposit.addresses.account == glmRouter, "Request is not from GlmRouter");
        require(IRoleStore(roleStore).hasRole(msg.sender, Role.CONTROLLER), "Not proper role");
        address user = IGlmRouter(glmRouter).getTransactionInfo(key).user;
        tempPayableAddress = user;
        ...
        IERC4626Upgradeable(glmVault).deposit(GmTokenValueInUsdc, user);
        IERC20Upgradeable(deposit.addresses.market).safeTransfer(glmVault, GmTokenAmount);
    }
    ...
    function afterWithdrawalExecution(bytes32 key, Withdrawal.Props memory withdrawal, EventUtils.EventLogData memory eventData) external {
        require(withdrawal.addresses.account == glmRouter, "Request is not from GlmRouter");
        ...
        address longTokenAddress = marketTokenToGmPoolInfo[withdrawal.addresses.market].longToken;

        if (IGlmRouter(glmRouter).getTransactionInfo(key).targetToken == USDC) {
            _usdcWithdraw(longTokenAddress, longAmountFromGMX, usdcAmountFromGMX, key, user);
        } else {
            _pairWithdraw(longTokenAddress, longAmountFromGMX, usdcAmountFromGMX, key, user);
        }
    }
    ...
    function _usdcWithdraw(
        ...
        if (longTokenAddress != WETH) {
            ISwapRouter.ExactInputParams memory params = ISwapRouter.ExactInputParams({
                path: abi.encodePacked(longTokenAddress, uniV3Fee, WETH, uniV3Fee, USDC),
        ...
        uint256 glmAmount = IGlmRouter(glmRouter).getTransactionInfo(key).glmAmount;

        IERC20Upgradeable(USDC).transfer(glmVault, totalUSDC);

        IGlmVault(glmVault).adjustPendingExecutionAmount(totalUSDC, false);
        //approve glmVault to use glm Token
        IERC20Upgradeable(glmVault).approve(glmVault, glmAmount);
        //@todo edit the function to use finalizeWithdrawal to keep the logic consistents
        IGlmVault(glmVault).userWithdraw(totalUSDC, glmAmount, user);
    }

    //@note only pair wihtdrawal has ratioImpact fee
    function _pairWithdraw(
        address longTokenAddress,
        uint256 longAmountFromGMX,
        uint256 usdcAmountFromGMX,
        bytes32 key,
        ...
        IGlmRouter.TransactionInfo memory transactionInfo = IGlmRouter(glmRouter).getTransactionInfo(key);

        tempPayableAddress = user;
        uint256 longTokenAmountOut;

        // case: user targetToken is ETH, if longTokenAddress is not WETH, swap longTokenAddress to WETH, else (so LTA = target, just add longAmountFromGMX to longTokenAmountOut)
        //         if (longTokenAddress != targetToken) {
        //             IERC20Upgradeable(longTokenAddress).approve(uniV3Router, longAmountFromGMX);
        //             ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
        //                 tokenIn: longTokenAddress,
        ...
        //             });
        //
        //             longTokenAmountOut = ISwapRouter(uniV3Router).exactInputSingle(params);
        //         } else {
        longTokenAmountOut += longAmountFromGMX;
        // }

        uint256 longTokenAmountValue = IGlmPriceReader(glmPriceReader).getMarketTokenPrice(transactionInfo.targetToken) *
            longTokenAmountOut;

        (uint256 longTokenAmountOutAfterFee, uint256 feeFromLongToken) = _afterRatioImpactFee(
            longTokenAmountOut,
            transactionInfo.ratioImpactFeeBps
        );

        (uint256 usdcAmountFromGMXAfterFee, uint256 feeFromUSDC) = _afterRatioImpactFee(
            longTokenAmountOut,
            transactionInfo.ratioImpactFeeBps
        );

        IERC20Upgradeable(transactionInfo.targetToken).transfer(glmFeeReceiver, feeFromLongToken);
        IERC20Upgradeable(transactionInfo.targetToken).transfer(user, longTokenAmountOutAfterFee);

        IERC20Upgradeable(USDC).transfer(glmFeeReceiver, usdcAmountFromGMXAfterFee);
        IERC20Upgradeable(USDC).transfer(user, feeFromUSDC);

        IGlmVault(glmVault).adjustPendingExecutionAmount(transactionInfo.pendingExecutionAmount, false);

        IERC20Upgradeable(glmVault).approve(glmVault, transactionInfo.glmAmount);
        //@note basically just perform a burn, check is enough
        IGlmVault(glmVault).finalizeWithdrawal(usdcAmountFromGMX + longTokenAmountValue, transactionInfo.glmAmount, user);
    }

    function _afterRatioImpactFee(
        uint256 _amount,
        uint256 _ratioImpactFeeBps
    ) internal pure returns (uint256 toUser, uint256 toFeeReceiver) {
        //@todo check if higher decimal is needed
        toFeeReceiver = (_amount * _ratioImpactFeeBps) / MAX_BPS;
        return (_amount - toFeeReceiver, toFeeReceiver);
    }

    //@todo delete in production
    function transferTokenToGlmVault(address _token) external {
        IERC20Upgradeable(_token).safeTransfer(glmVault, IERC20Upgradeable(_token).balanceOf(address(this)));
    }

    //withdraw token (all of its balance to owner)
