...
contract FactorLeverageSwitch is IFlashLoanRecipient, ReentrancyGuard, Ownable {
    ...
    function receiveFlashLoan(
        ...
        (
            ...
            address debtToken,
            uint256 debtBalance
        ) = abi.decode(params, (address, uint256, address, address, address, uint256, address, uint256));
        ...
        ILeverageStrategy(newPositionVault).borrow(debtBalance + feeAmount);

        // repay debt Flashloan
        IERC20(debtToken).safeTransfer(balancerVault, debtBalance + feeAmount);

        // send new vuault to user
        IERC721(newVault).transferFrom(address(this), msg.sender, newPositionId - 1);

        // send old vault to user
        IERC721(vault).transferFrom(address(this), msg.sender, positionId);
