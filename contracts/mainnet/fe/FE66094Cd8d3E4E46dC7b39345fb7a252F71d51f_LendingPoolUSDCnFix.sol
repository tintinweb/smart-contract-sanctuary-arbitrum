...
/**
 * @dev Temporary LendingPool implementation to reset "liquidityIndex" property of a deployed and configured AToken /reserve.
 * This contract was created in lieu of the following event:
 * - https://twitter.com/RDNTCapital/status/1742638364933714112
 * - Refer to changes with tag: @custom:usdcn-fix
 */
contract LendingPoolUSDCnFix is VersionedInitializable, ILendingPool, LendingPoolStorage {
	using SafeMath for uint256;
	using WadRayMath for uint256;
	using PercentageMath for uint256;
	using SafeERC20 for IERC20;
	...
	/**
	 * @custom:usdcn-fix Call to reset the liquidityIndex of the USDCn reserve to 1 ray (1e27).
	 * Index manipulated here: https://arbiscan.io/tx/0x1ce7e9a9e3b6dd3293c9067221ac3260858ce119ecb7ca860eac28b2474c7c9b
	 * @dev can only be called once.
	 */
	function resetOverrideUSDCnLiquidityIndex() external {
		// NOTE: Check this is the correct multisig
		require(msg.sender == owner, "only callable by radiant multisig");
		address USDCn = 0xaf88d065e77c8cC2239327C5EDb3A432268e5831;
		DataTypes.ReserveData storage reserveUSDCn = _reserves[USDCn];
		reserveUSDCn.liquidityIndex = 1e27;
		emit ReserveLogic.ReserveDataUpdated(
			reserveUSDCn.aTokenAddress,
			reserveUSDCn.currentLiquidityRate,
			reserveUSDCn.currentStableBorrowRate,
			reserveUSDCn.currentVariableBorrowRate,
			reserveUSDCn.liquidityIndex,
			reserveUSDCn.variableBorrowIndex
		);
		reserveUSDCn.lastUpdateTimestamp = uint40(block.timestamp);

		// As the burning of ATokens circumvents the usual tx flow, the config of users isn't update.
		// Specifically we set that the users now don't own any USDCn collateral.
		uint256 len = 4;
		address[] memory _accounts = new address[](len);
		_accounts[0] = 0x39519c027b503f40867548Fb0c890b11728faA8F;
		_accounts[1] = 0x78d13f1225EC1F5fD80BBFaA88490779Ba079De8;
		_accounts[2] = 0xd8B591abA7E5eDAc39f544e5d365CD65705Ea8f3;
		_accounts[3] = 0xFD76399B68E8521245b2cD42771B308c7164b0B3;

		for (uint256 i = 0; i < len; i++) {
			_usersConfig[_accounts[i]].setUsingAsCollateral(reserveUSDCn.id, false);
			emit ReserveUsedAsCollateralDisabled(USDCn, _accounts[i]);
		}
	}
