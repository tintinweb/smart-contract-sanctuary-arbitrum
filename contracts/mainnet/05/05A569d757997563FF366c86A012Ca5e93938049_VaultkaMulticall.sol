...
import { IMaotai } from "./interfaces/IMaotai.sol";
import { IRum } from "./interfaces/IRum.sol";
import { ISake } from "./interfaces/ISake.sol";
import { ISakeV1 } from "./interfaces/ISakeV1.sol";
import { ISakeV3 } from "./interfaces/ISakeV3.sol";
import { IVodka } from "./interfaces/IVodka.sol";
import { IVodkaV1a } from "./interfaces/IVodkaV1a.sol";
import { IVodkaV2 } from "./interfaces/IVodkaV2.sol";
import { IVodkaV2Dn } from "./interfaces/IVodkaV2Dn.sol";
import { IWhiskey } from "./interfaces/IWhiskey.sol";
import { IGin } from "./interfaces/IGin.sol";
import { ITequila } from "./interfaces/ITequila.sol";
...
contract VaultkaMulticall {
  ...
  function _getNumberOfPositions(string memory _vault, address _user) internal view returns (uint256) {
    ...
    if (areStringsEqual(_vault, "sakeV3")) return ISakeV3(vaultAddress).getTotalNumbersOfOpenPositionBy(_user);
    if (areStringsEqual(_vault, "vodka")) return IVodka(vaultAddress).getTotalNumbersOfOpenPositionBy(_user);
    if (areStringsEqual(_vault, "vodkaV1a")) return IVodkaV1a(vaultAddress).getTotalNumbersOfOpenPositionBy(_user);
    if (areStringsEqual(_vault, "vodkaV2")) return IVodkaV2(vaultAddress).getTotalOpenPosition(_user);
    if (areStringsEqual(_vault, "vodkaV2Dn")) return IVodkaV2Dn(vaultAddress).getTotalOpenPosition(_user);
    if (areStringsEqual(_vault, "whiskey")) return IWhiskey(vaultAddress).getTotalNumbersOfOpenPositionBy(_user);
    if (areStringsEqual(_vault, "gin")) return IGin(vaultAddress).getTotalNumbersOfOpenPositionBy(_user);
    if (areStringsEqual(_vault, "tequila")) return ITequila(vaultAddress).getTotalNumbersOfOpenPositionBy(_user);
  ...
  function _getDtv(
    ...
    address _user,
    uint256 _hlpPrice
  ) internal view returns (DtvValues memory) {
    address vaultAddress = vaultAddresses[_vault];
    DtvValues memory dtv;

    if (areStringsEqual(_vault, "maotai")) {
      (dtv.currentDTV, dtv.currentPosition, dtv.currentDebt) = IMaotai(vaultAddress).getUpdatedDebt(_positionId, _user);
    } else if (areStringsEqual(_vault, "rum")) {
      (dtv.currentDTV, dtv.currentDebt, dtv.currentPosition, , dtv.leverageAmountWithDA) = IRum(vaultAddress)
        .getPosition(_positionId, _user, _hlpPrice);
    } else if (areStringsEqual(_vault, "sake")) {
      (dtv.currentDTV, dtv.currentPosition, dtv.currentDebt) = ISake(vaultAddress).getUpdatedDebt(_positionId, _user);
    } else if (areStringsEqual(_vault, "sakeV1")) {
      (dtv.currentDTV, dtv.currentPosition, dtv.currentDebt) = ISakeV1(vaultAddress).getUpdatedDebtAndValue(
        _positionId,
        _user
      );
    } else if (areStringsEqual(_vault, "sakeV3")) {
      (dtv.currentDTV, dtv.currentPosition, dtv.currentDebt) = ISakeV3(vaultAddress).getUpdatedDebt(_positionId, _user);
    } else if (areStringsEqual(_vault, "vodka")) {
      (dtv.currentDTV, dtv.currentPosition, dtv.currentDebt) = IVodka(vaultAddress).getUpdatedDebt(_positionId, _user);
    } else if (areStringsEqual(_vault, "vodkaV1a")) {
      (dtv.currentDTV, dtv.currentPosition, dtv.currentDebt) = IVodka(vaultAddress).getUpdatedDebt(_positionId, _user);
    } else if (areStringsEqual(_vault, "vodkaV2")) {
      (dtv.currentDTV, dtv.currentPosition, dtv.currentDebt) = IVodkaV2(vaultAddress).getUpdatedDebt(
        _positionId,
        _user
      );
      (dtv.leverageAmountWithDA, ) = IVodkaV2(vaultAddress).getCurrentLeverageAmount(_positionId, _user);
    } else if (areStringsEqual(_vault, "vodkaV2Dn")) {
      (dtv.currentDTV, dtv.currentDebt, dtv.currentPosition) = IVodkaV2Dn(vaultAddress).getUpdatedDebt(
        _positionId,
        _user
      );
      IVodkaV2Dn.PositionDebt memory debt = IVodkaV2Dn(vaultAddress).positionDebt(_user, _positionId);
      dtv.longDebtValue = debt.longDebtValue;
      dtv.shortDebtValue = debt.shortDebtValue;
    } else if (areStringsEqual(_vault, "whiskey")) {
      (dtv.currentDTV, dtv.currentPosition, dtv.currentDebt) = IWhiskey(vaultAddress).getUpdatedDebtAndValue(
        _positionId,
        _user
      );
    } else if (areStringsEqual(_vault, "gin")) {
      (dtv.currentDTV, dtv.currentPosition, dtv.currentDebt) = IGin(vaultAddress).getUpdatedDebt(_positionId, _user);
    } else if (areStringsEqual(_vault, "tequila")) {
      (dtv.currentDTV, dtv.currentPosition, dtv.currentDebt) = ITequila(vaultAddress).getUpdatedDebt(
        _positionId,
        _user
      );
    }
  ...
  function fetchAllUsersDtv(
    ...
    for (uint256 i = 0; i < _users.length; i++) {
      ...
      for (uint256 j = 0; j < numberOfPositions; j++) {
        DtvValues memory dtvValues = _getDtv(_vault, j, _users[i], hlpPrice);

        dtv[i][j] = IVault.Dtv({
          currentDTV: dtvValues.currentDTV,
          currentPosition: dtvValues.currentPosition,
          currentDebt: dtvValues.currentDebt,
          leverageAmountWithDA: dtvValues.leverageAmountWithDA,
          longDebtValue: dtvValues.longDebtValue,
          shortDebtValue: dtvValues.shortDebtValue,
          leverageWithInterests: dtvValues.leverageWithInterests,
          totalInterests: dtvValues.totalInterests
        });
      }
    }
  ...
  function fetchSakeV3AllUsersPosition(address[] memory _users) external view returns (ISakeV3.UserInfo[][] memory) {
    string memory vault = "sakeV3";
    address vaultAddress = vaultAddresses[vault];
    require(vaultAddress != address(0), "dont have this vault");

    ISakeV3.UserInfo[][] memory positions = new ISakeV3.UserInfo[][](_users.length);

    for (uint256 i = 0; i < _users.length; i++) {
      uint256 numberOfPositions = _getNumberOfPositions(vault, _users[i]);
      positions[i] = new ISakeV3.UserInfo[](numberOfPositions);

      for (uint256 j = 0; j < numberOfPositions; j++) {
        ISakeV3.UserInfo memory positionInfo = ISakeV3(vaultAddress).userInfo(_users[i], j);
  ...
  function fetchVodkaV1aAllUsersPosition(address[] memory _users) external view returns (IVodka.UserInfo[][] memory) {
    string memory vault = "vodkaV1a";
    address vaultAddress = vaultAddresses[vault];
    require(vaultAddress != address(0), "dont have this vault");
  ...
  function fetchVodkaV2DnAllUsersPosition(
    address[] memory _users
  ) external view returns (IVodkaV2Dn.PositionInfo[][] memory) {
    string memory vault = "vodkaV2Dn";
    address vaultAddress = vaultAddresses[vault];
    require(vaultAddress != address(0), "dont have this vault");

    IVodkaV2Dn.PositionInfo[][] memory positions = new IVodkaV2Dn.PositionInfo[][](_users.length);

    for (uint256 i = 0; i < _users.length; i++) {
      uint256 numberOfPositions = _getNumberOfPositions(vault, _users[i]);
      positions[i] = new IVodkaV2Dn.PositionInfo[](numberOfPositions);

      for (uint256 j = 0; j < numberOfPositions; j++) {
        IVodkaV2Dn.PositionInfo memory positionInfo = IVodkaV2Dn(vaultAddress).positionInfo(_users[i], j);
  ...
  function fetchGinAllUsersPosition(address[] memory _users) external view returns (IGin.UserInfo[][] memory) {
    string memory vault = "gin";
    address vaultAddress = vaultAddresses[vault];
    require(vaultAddress != address(0), "dont have this vault");

    IGin.UserInfo[][] memory positions = new IGin.UserInfo[][](_users.length);

    for (uint256 i = 0; i < _users.length; i++) {
      uint256 numberOfPositions = _getNumberOfPositions(vault, _users[i]);
      positions[i] = new IGin.UserInfo[](numberOfPositions);

      for (uint256 j = 0; j < numberOfPositions; j++) {
        IGin.UserInfo memory positionInfo = IGin(vaultAddress).userInfo(_users[i], j);
  ...
  function fetchTequilaAllUsersPosition(address[] memory _users) external view returns (ITequila.UserInfo[][] memory) {
    string memory vault = "tequila";
    address vaultAddress = vaultAddresses[vault];
    require(vaultAddress != address(0), "dont have this vault");

    ITequila.UserInfo[][] memory positions = new ITequila.UserInfo[][](_users.length);

    for (uint256 i = 0; i < _users.length; i++) {
      uint256 numberOfPositions = _getNumberOfPositions(vault, _users[i]);
      positions[i] = new ITequila.UserInfo[](numberOfPositions);

      for (uint256 j = 0; j < numberOfPositions; j++) {
        ITequila.UserInfo memory positionInfo = ITequila(vaultAddress).userInfo(_users[i], j);
