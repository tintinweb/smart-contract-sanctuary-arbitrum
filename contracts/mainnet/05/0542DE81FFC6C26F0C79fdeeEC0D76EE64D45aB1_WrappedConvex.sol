...
interface IConvexBooster {
    ...
    function deposit(uint256 _pid, uint _lp) external returns(bool);
...
contract WrappedConvex is BaseWrapper {
    ...
    function initialize(address convexRewardPool_, string memory name_, string memory symbol_, address authority_) public initializer {
        __Wrapper_init(convexRewardPool_, name_, symbol_, authority_);
    ...
    function _invest() internal override {
        ...
        if (lpBalance > 0) {
            ...
            CVX_BOOSTER.deposit(CVX_POOL_ID, lpBalance);
        }
    }
