...
interface IStrategyUniswapV3 {
    ...
    function range() external view returns (int24 tickLower, int24 tickUpper);
...
contract StratFeeManagerInitializable is OwnableUpgradeable, PausableUpgradeable {
    ...
    /// @notice The address of the beefy fee recipient
    address public beefyFeeRecipient;

    /// @notice The address of the beefy fee config
...
import {StratFeeManagerInitializable, IFeeConfig} from "../../beefy/StratFeeManagerInitializable.sol";
import {IUniswapV3Pool} from "../../../interfaces/uniswap/IUniswapV3Pool.sol";
import {LiquidityAmounts} from "../utils/LiquidityAmounts.sol";
import {TickMath} from "../utils/TickMath.sol";
import {TickUtils, FullMath} from "../utils/TickUtils.sol";
import {UniV3Utils} from "../../../interfaces/exchanges/UniV3Utils.sol";
...
contract StrategyPassiveManagerUniswap is StratFeeManagerInitializable, IStrategyConcLiq, IStrategyUniswapV3 {
    ...
    address public pool;
    /// @notice The address of the wrapped native gas token.
    ...
    int56 public maxTickDeviationNegative;
    int56 public maxTickDeviationPositive;

    /// @notice The twap interval seconds we use for the twap check. 
    int32 public twapInterval;
    ...
    event SetDeviation(int56 maxTickDeviationNegative, int56 maxTickDeviationPositive);
    event SetTwapInterval(int32 oldInterval, int32 interval);
    ...
    function _onlyCalmPeriods() private view {
        ...
        if(twapTick - maxTickDeviationNegative > tick  || twapTick + maxTickDeviationPositive < tick) revert NotCalm();
    }
     ...
     * @param _native The wrapped native gas token.
    ...
    function initialize (
        ...
        if (fee == 100) {
            // .01% fee
            maxTickDeviationNegative = 4;
            maxTickDeviationPositive = 3;
        } else if (fee == 500) {
            // 0.05% fee
            maxTickDeviationNegative = 18;
            maxTickDeviationPositive = 17;
        } else if (fee == 3000) {
            // 0.3% fee
            maxTickDeviationNegative = 30;
            maxTickDeviationPositive = 29;
        } else if (fee == 10000) {
            // 1% fee
            maxTickDeviationNegative = 100;
            maxTickDeviationPositive = 99;
        } else {
            revert InvalidInput();
        }
    
        // Set the twap interval to 360 seconds.
        twapInterval = 360;
    ...
    function _addLiquidity() private whenNotPaused {
    ...
    function _harvest (address _callFeeRecipient) private onlyCalmPeriods {
        ...
        // Readjust our liquidity positions.
        _removeLiquidity();
        _setTicks();
        _addLiquidity();
    ...
    function _claimEarnings() private {
        ...
        (uint256 fee0, uint256 fee1) = IUniswapV3Pool(pool).collect(address(this), positionMain.tickLower, positionMain.tickUpper, type(uint128).max, type(uint128).max);
        (uint256 feeAlt0, uint256 feeAlt1) = IUniswapV3Pool(pool).collect(address(this), positionAlt.tickLower, positionAlt.tickUpper, type(uint128).max, type(uint128).max);
    ...
    */
    function range() external view returns (int24 tickLower, int24 tickUpper) {
        ...
        return (positionMain.tickLower, positionMain.tickUpper);
    }
    ...
    */
    ...
    function _setAltTick() private {
        ...
        if (bal0 > 0) {
            ...
            price1 = FullMath.mulDiv(uint256(sqrtPriceX96) ** 2, PRECISION, (2 ** 192));
            amount0 = bal0 * price1 / PRECISION;
        }
    ...
    */
     ...
     * @param _maxDeviationNegative The max negative deviation from twap we will allow.
     * @param _maxDeviationPositive The max positive deviation from twap we will allow.
    */
    function setDeviation(int56 _maxDeviationNegative, int56 _maxDeviationPositive) external onlyOwner {
        emit SetDeviation(_maxDeviationNegative, _maxDeviationPositive);
        maxTickDeviationNegative = _maxDeviationNegative;
        maxTickDeviationPositive = _maxDeviationPositive;
    }
    ...
    */
    ...
    function setTwapInterval(int32 _interval) external onlyOwner {
    ...
    */
     ...
     */
    function panic(uint256 _minAmount0, uint256 _minAmount1) external onlyManager {
        _claimEarnings();
        _removeLiquidity();
        _removeAllowances();
        _pause();
