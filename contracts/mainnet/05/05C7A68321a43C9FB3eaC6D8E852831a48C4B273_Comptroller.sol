    ...
    ExponentialNoError
{
     ...
     */
    function enterMarkets(address[] memory cTokens, address minter)
        public
        override
        returns (uint256[] memory)
    {
        require(isCtoken[msg.sender], "enterMarket: must ctoken");
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            results[i] = uint256(addToMarketInternal(cToken, minter));
        }
        ...
        returns (Error)
    {
        ...
        require(accountAssets[borrower].length + 1 <= 50, "enterMarket: max length need less than 50");
     ...
     */
    function exitMarket(address cTokenAddress, address borrower)
        external
        override
        returns (uint256)
    {
        require(isCtoken[msg.sender], "exitMarket: must ctoken");
        ...
        CToken[] memory userAssetList = accountAssets[borrower];
        ...
        CToken[] storage storedList = accountAssets[borrower];
    ...
    function _setFactory(address factory_) external {
        require(msg.sender == admin, "only admin can set dex factor");
        factory = factory_;
    }
    ...
    function _addMarketInternal(address cToken) internal {
        require(!isCtoken[cToken], "market already added");
        isCtoken[cToken] = true;
    ...
    function _setBorrowPaused(CToken cToken, bool state) public returns (bool) {
        ...
        require(
            msg.sender == pauseGuardian || msg.sender == admin || msg.sender == factory,
            "only pause guardian and admin can pause"
        );
        require(msg.sender == admin || state == true, "only admin can unpause");
...
abstract contract ComptrollerInterface {
    ...
    function enterMarkets(address[] calldata cTokens, address minter) virtual external returns (uint[] memory);
    function exitMarket(address cToken, address borrower) virtual external returns (uint);
...
contract CTokenStorage {
    ...
    /**
     * @notice Total number of underlying in circulation
     */
    uint public actualCash;
