...
//        require(hash == keccak256(data), "QHTrade/hash-mismatch");
    ...
    function wrapNative(address weth) external payable {
        ...
        require(IERC20(weth).transfer(msg.sender, msg.value), "TF");

        emit Swap(msg.sender, address(0), weth, msg.value, 1 ether);
    }

    function unwrapNative(address weth, uint256 amount) external {
        require(IERC20(weth).transferFrom(msg.sender, address(this), amount), "TF");

        safeApprove(weth, amount, weth);
        IWETH(weth).withdraw(amount);

        payable(msg.sender).transfer(amount);

        emit Swap(msg.sender, weth, address(0), amount, 1 ether);
    }
    ...
    function withdrawETH(uint256 amount, address to) external auth {
