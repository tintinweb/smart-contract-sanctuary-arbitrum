...
interface ICokeToken {
    ...
    function fragmentToCoke(uint256 value) external view returns (uint256);

    function cokeToFragment(uint256 coke) external view returns (uint256);
...
contract CokeChefV2 is Ownable, ReentrancyGuard {
    ...
    ICokeToken public coke;
    // COKE ETH LP address
    IUniswapPair public cokeEthLp;
    // Uniswap V2 Router
    IUniswapV2Router public router;
    // COKE tokens reward per block.
    ...
    // Update the given pool's COKE allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending Coke on frontend.
    function pendingReward(
        ...
        if (address(pool.lpToken) == address(coke)) {
            lpSupply = coke.balanceOfUnderlying(address(this));
        }
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 cokeReward = ((block.number - pool.lastRewardBlock) *
                rewardPerBlock *
                pool.allocPoint) / totalAllocPoint;
            accRewardPerShare += (cokeReward * 1e12) / lpSupply;
        }
        return
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 cokeReward = ((block.number - pool.lastRewardBlock) *
            rewardPerBlock *
            pool.allocPoint) / totalAllocPoint;
        pool.accRewardPerShare += (cokeReward * 1e12) / lpSupply;
        pool.lastRewardBlock = block.number;
    ...
    // Deposit tokens to CokeChef for COKE allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _account
        ...
        if (address(pool.lpToken) == address(coke)) {
            _amount = coke.fragmentToCoke(_amount);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
        if (address(pool.lpToken) == address(coke)) {
            _amount = coke.cokeToFragment(_amount);
        }
    ...
    function compoundBig(
        ...
        IERC20(address(coke)).safeTransferFrom(
            msg.sender,
            address(this),
            _amountTokenDesired
        );
        IERC20(address(coke)).approve(address(router), _amountTokenDesired);
        (uint256 token, , uint256 liq) = router.addLiquidityETH{
            value: msg.value
        }(
            address(coke),
            _amountTokenDesired,
        ...
        IERC20(address(coke)).safeTransfer(
            msg.sender,
