...
import {DuoToken} from "./DuoToken.sol";

contract DuoMaster is Ownable {
    ...
    //tracks historic deposits in hardcoded usd value of each address. depositsUSD[pid][user] is the total deposits for that user to that mono
    mapping(address => Whitelist) public whitelistInfo;
    // hardcoded usd value of each address.
    mapping(address => uint256) public lpPrice; // should be 1e8
    ...
    function depositWhitelist(
        ...
        if (amount > 0) {
            ...
            Whitelist storage whitelist = whitelistInfo[msg.sender];
            require(
                whitelist.isWhitelisted,
                "Whitelist: Not whitelisted address"
            );
            uint256 amountInUSD = (amount * lpPrice[address(pool.want)]) /
                (10 ** uint256(ERC20(address(pool.want)).decimals()));
            require(
                whitelist.usdValue >= amountInUSD,
                "Whitelist: Not enough amount to deposit"
            );
            whitelist.usdValue -= amountInUSD;
    ...
    function availableAmountToDeposit(
        uint256 pid,
        address _user
    ) public view returns (uint256 _amount) {
        Whitelist memory whitelist = whitelistInfo[_user];
        PoolInfo memory pool = poolInfo[pid];
        if (whitelist.isWhitelisted && whitelist.usdValue > 0) {
            _amount =
                (whitelist.usdValue *
                    (10 ** uint256(ERC20(address(pool.want)).decimals()))) /
                lpPrice[address(pool.want)];
            return _amount;
        }
        return 0;
    }
    ...
    function setWhitelist(
        ...
        uint256[] calldata _usdValue // in 1e8
    ) public onlyOwner {
        require(
            _user.length == _isWhitelisted.length &&
                _user.length == _usdValue.length,
            "input length mismatch"
        );
        for (uint256 i = 0; i < _user.length; i++) {
            whitelistInfo[_user[i]] = Whitelist({
                isWhitelisted: _isWhitelisted[i],
                usdValue: _usdValue[i]
            });
        }
    }

    function setLpPriceForWhitelist(
        address[] calldata _lp,
        uint256[] calldata _price // 1e8
    ) public onlyOwner {
        require(_lp.length == _price.length, "input length mismatch");
        for (uint256 i = 0; i < _lp.length; i++) {
            lpPrice[_lp[i]] = _price[i];
        }
    }
}
...
contract LPLockupDoubleReward is ERC20, Ownable, ReentrancyGuard {
    ...
    function initialize(
        ...
        approve(address(_duoMaster), type(uint256).max);
    }
