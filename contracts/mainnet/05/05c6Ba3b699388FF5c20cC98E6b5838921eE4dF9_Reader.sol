...
contract Reader is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping (address=>uint) public optionPremiumV2;
    ...
    function setOptionPremiumV2(address[] memory _userList, uint[] memory _premiumList) public onlyWriter {
        require(_userList.length == _premiumList.length, "_userList length mismatch _premiumList length");
        for (uint i = 0; i < _userList.length; i++) {
            optionPremiumV2[_userList[i]] = _premiumList[i];
        }
    }
    function getOptionPremiumV2(address[] memory _userList) public view returns(uint[] memory) {
        uint[] memory data =  new uint[](_userList.length);
         for (uint i = 0; i < _userList.length; i++) {
            data[i] =  optionPremiumV2[_userList[i]];
        }
        return data;
    }
