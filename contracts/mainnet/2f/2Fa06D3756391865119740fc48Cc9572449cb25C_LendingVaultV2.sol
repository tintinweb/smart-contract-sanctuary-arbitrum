...
interface ILendingVaultV2 {
  ...
  function borrowAPR(address borrower) external view returns (uint256);
  function lendingAPR(address borrower) external view returns (uint256);
...
contract LendingVaultV2 is ERC20, ReentrancyGuard, Pausable, Ownable2Step, ILendingVaultV2 {
  ...
  */
  ...
  /**
    * @notice Returns the current borrow APR for a borrower
    * @param borrower Address of borrower
    * @return borrowAPR   Current borrow rate in 1e18
  */
  function borrowAPR(address borrower) public view returns (uint256) {
    return _calculateInterestRate(borrower, totalBorrows, totalAvailableAsset());
  }

  /**
    * @notice Returns the current lending APR for a borrower; borrowAPR * utilization * (1 - performanceFee)
    * @param borrower Address of borrower
    * @return lendingAPR   Current lending rate in 1e18
  */
  function lendingAPR(address borrower) public view returns (uint256) {
    uint256 borrowAPR_ = borrowAPR(borrower);
  ...
  */
  ...
  /**
    ...
    * @param borrower Borrower's address
    * @return Boolean on whether borrower is approved in this lending vault or not
  */
  ...
  /**
    ...
    * @return Array of approved borrower addresses
  */
  ...
  */
  function _pendingInterest(uint256 assetAmt) internal view returns (uint256) {
    ...
    for (uint256 i = 0; i < approvedBorrowers.length(); i++) {
      _ratePerSec += _calculateInterestRate(
        approvedBorrowers.at(i),
        totalBorrows,
        _floating
      ) / SECONDS_PER_YEAR;
    }
