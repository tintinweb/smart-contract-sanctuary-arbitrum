...
contract LETH is ERC20, Ownable {
    ...
    function initialize(
        ...
        lastInitializeBinId = lastInitializeBinId + getActiveBinId() + uint256(seedLiquidityTimes * 100);
        ...
        for (uint256 i = 0; i < seedIndex; i++) {
            ...
            distributionX[i] = (Constants.PRECISION * 1) / 100;
        }
    ...
    function _addLiquidityForNewBin(
        uint256 tokenXbalance, uint24 binId
    ) internal {
        _addLiquidityForBin(tokenXbalance,0,binId);
    }

    function _addLiquidityForFloorBin(
        uint256 tokenXbalance, uint256 tokenYbalance, uint24 binId
    ) internal {
        _addLiquidityForBin(tokenXbalance,tokenYbalance,binId);
    }

    function _addLiquidityForBin(
        uint256 tokenXbalance, uint256 tokenYbalance, uint24 binId
    ) internal {
        ...
        // distribute 100%
        deltaIds[0] = 0;
        distributionX[0] = (Constants.PRECISION * 100) / 100;
        distributionY[0] = (Constants.PRECISION * 100) / 100;
    ...
    function rebalanceFloorBinLiquidity() external {
        ...
        if(tightLiqBin > 0){
            ...
            if(_tbtokenX > xPerBin){
                ...
                _addLiquidityForNewBin(xPerBin,uint24(tightLiqBin));
                tightLiqBin = 0;
            }
        }
        ...
        if(lastRecordedActiveBin > (getActiveBinId()+1)){
            ...
            if(_lrtokenX < xPerBin && lastRecordedActiveBin > getActiveBinId()){
                ...
                _addLiquidityForNewBin(xPerBin,uint24(lastRecordedActiveBin));
                addToNewBin = true;
            }
        }
        ...
        if(!addToNewBin){
            ...
            //tokenX to maxBin
            _addLiquidityForNewBin(xPerBin,maxBin);
            addToNewBin = true;
        }
        ...
        _addLiquidityForFloorBin(tokenXFloorBalance,tokenYbalance,floorLiquidityBin);
