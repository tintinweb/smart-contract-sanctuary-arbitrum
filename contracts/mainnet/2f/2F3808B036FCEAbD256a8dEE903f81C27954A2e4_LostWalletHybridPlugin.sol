...
/**
 * @title Lost Wallet Plugin
 * @author Josh Davis
 * @notice Provides the recoverMisplacedTokens()
 */

interface ILostWalletHybridPlugin {
    /**
...
contract LostWalletHybridPlugin is ViciERC20HybridPlugin {
    bytes4 constant RECOVER =
        ILostWalletHybridPlugin.recoverMisplacedTokens.selector;
    ...
    function _beforeHook(
        ...
        // _msg.sender permissions are validated in the ops contract
        if (_msg.sig == RECOVER) {
    ...
    function _execute(
        ...
        if (_msg.sig == RECOVER) {
            (address lostWallet, address toAddress) = abi.decode(
                _msg.data[4:],
                (address, address)
            );
            return _doRecover(_msg.sender, lostWallet, toAddress);
        }
    ...
    function _doRecover(
        ...
        (ISimplePluginExecutor(msg.sender)).executeFromPlugin(
            abi.encodeWithSelector(
                IModularERC20Ops.recoverMisplacedTokens.selector,
                operator,
                lostWallet,
                toAddress
            )
