...
contract Fees is Initializable, Upgradeable, IFees {
    ...
    modifier feederOnly() {
        require(msg.sender == address(registry.feeder()), "QF/AD"); // access denied
        _;
    }

    modifier factoryOnly() {
        require(msg.sender == address(registry.fundFactory()), "QF/AD"); // access denied
        _;
    }
    ...
    function setServiceFees(uint256 sf, uint256 pf, uint256 mf) external onlyOwner {
        require(sf <= MAX_SF, "QF/IS"); // invalid sf
        require(pf <= MAX_PF, "QF/IP"); // invalid pf
        serviceSf = sf;
        servicePf = pf;
        serviceMf = mf;
    ...
    // Must be called from the feeder
    function gatherEf(uint256 fundId, uint256 amount, address token) external override feederOnly {
        require(funds[fundId].live == 1, "FE/FNE");
    ...
    // Must be called from the feeder
    function gatherMf(uint256 fundId, uint256 pending, address token, address manager) external override feederOnly {
        require(funds[fundId].live == 1, "FE/FNE");
    ...
    function withdraw(address user, uint256 amount) external {
        ...
        gatheredServiceFees -= amount;
        registry.usdt().transfer(user, amount);
    ...
    function withdrawFund(uint256 fundId, address destination, uint256 amount) external {
        ...
        registry.usdt().transfer(destination, amount);
