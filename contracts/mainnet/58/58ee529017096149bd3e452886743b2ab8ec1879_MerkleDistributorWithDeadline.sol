...
contract MerkleDistributor is IMerkleDistributor, Ownable {
    ...
    function _setMerkleRoot(bytes32 newMerkleRoot) internal {
        if (newMerkleRoot == bytes32("")) revert InvalidMerkleRoot();
...
interface IMerkleDistributor {
    ...
    // This event is emitted when the merkle root is updated
    event MerkleRootSet(bytes32 newMerkleRoot);

    // This error is thrown when the merkle root is empty
