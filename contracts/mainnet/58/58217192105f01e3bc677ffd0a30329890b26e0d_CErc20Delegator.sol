...
contract TokenErrorReporter {
    ...
    error SetAddressOwnerCheck();
    error SetAutoCompoundOwnerCheck();
}

interface ICAPTokenFarm {
    function collectMultiple(address[] calldata assets) external; // address 0 is ETH
    function stake(uint256 amount) external;
    function unstake(uint256 amount) external;
    function stakingStore() external view returns (address);
}
...
contract CTokenStorage {
    ...
    /**
     * @notice Is the underlying token stakable
     */
    bool public isStakable;

    /**
     * @notice Wether or not the eth rewards should be autocompounded
     */
    bool public autocompound;

    /**
     * @notice CAP reward router for claiming rewards
     */
    ICAPTokenFarm public capTokenFarm;

    /**
     * @notice Adress to call transfer on
     */
    EIP20Interface public capToken;
    ...
    uint public lastCompoundBlock;
    uint public capBlockDelta;
...
abstract contract CTokenInterface is CTokenStorage {
    ...
    function _setTokenAddresses(EIP20Interface token_, ICAPTokenFarm tokenFarm_) virtual public returns (uint);
...
contract CErc20Delegator is CTokenInterface, CErc20Interface, CDelegatorInterface {
     ...
     * @param isStakable_ Wether or not the market being created is for a stakable token
    ...
    /**
     * @notice Updates the token contract addresses using _setTokenAddresses
     * @dev Admin function to set the token contract addresses
     * @param token_ the token contract to use
     * @param tokenFarm_ the rewardrouter contract address to use
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    
    function _setTokenAddresses(EIP20Interface token_, ICAPTokenFarm tokenFarm_) override public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("_setTokenAddresses(address,address)", token_, tokenFarm_));
        return abi.decode(data, (uint));
    }
    ...
    /**
     * @notice Toggle wether or not the token rewards should be autocompounded
     * @dev Admin function to set wether or not token rewards should be autocompounded
