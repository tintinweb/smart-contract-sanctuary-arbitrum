...
// File: contracts/pool/ArbPepeJackpot.sol


pragma solidity =0.8.19;

contract ArbPepeJackpot is Initializable, OwnableUpgradeable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    Tick[] public ticks2;
    uint256 public totalProbability2;
    ...
    function lottery() public onlyCaller {
        ...
        uint256 currentBonus = bal.div(3);
        ...
        for (uint256 i = 0; i < ticks.length; i++) {
            ...
            if (randomValue < cumulativeProbability) {
                uint256 bonus = currentBonus > maxBonus ? maxBonus : currentBonus;
    ...
    function lottery2() public onlyCaller {
        require(ticks2.length > 0 && totalProbability2 > 0, "No tickets");
        require(block.timestamp > 1682006400, "not good");
        uint256 bal = bonusToken.balanceOf(address(this));
        uint256 currentBonus = bal.div(3);
        if(currentBonus < minBonus) {
            return;
        ...
        uint256 randomValue = uint256(keccak256(abi.encodePacked(block.timestamp, block.number))) % totalProbability2;
        
        uint256 cumulativeProbability = 0;
        for (uint256 i = 0; i < ticks2.length; i++) {
            cumulativeProbability += ticks2[i].probability;
            if (randomValue < cumulativeProbability) {
                uint256 bonus = currentBonus > maxBonus ? maxBonus : currentBonus;
                bonusToken.transfer(ticks2[i].user, bonus);
                totalBonus += bonus;
                emit Winner(ticks2[i].id, ticks2[i].user, bonus, block.timestamp);
                break;
            }
        }
    ...
    function lottery0(uint256[] memory ids, address[] memory users, uint256[] memory probs, uint256 totalProb, uint256 ts) public onlyCaller {
        require(users.length == probs.length, "Invalid length");
        require(users.length > 0 && totalProb > 0, "Invalid params");
        ...
        latestBonusTimestamp = ts;

        uint256 randomValue = uint256(keccak256(abi.encodePacked(block.timestamp, block.number))) % totalProb;
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            if (randomValue < cumulativeProbability) {
                uint256 bonus = currentBonus > maxBonus ? maxBonus : currentBonus;
                bonusToken.transfer(users[i], bonus);
                totalBonus += bonus;
                emit Winner(ids[i], users[i], bonus, ts);
                break;
            }
        }
    }
    ...
    function _addTick2(address user, uint256 amount) internal {
        tickId++;
        (uint tick, uint256 amountUSD) = getTickTypeWithTradeAmount(amount);
        uint256 prob = tickProbability[tick];
        if (prob > 0) {
            ticks2.push(Tick({id:tickId, user: user, probability: prob}));
            totalProbability2 += prob;
        }
    ...
    function tradeEvent(address sender, uint256 amount) public onlyCaller {
        if (block.timestamp > 1682006400) {
            _addTick2(tx.origin, amount);
        } else {
    ...
    function setMinBouns(uint256 val) public onlyDev {
        require(val < maxBonus, "bad val");
        minBonus = val;
    }

    function setMaxBouns(uint256 val) public onlyDev {
        require(val > minBonus, "bad val");
        maxBonus = val;
    }

    function setLatestBonusTimestamp(uint val) public onlyDev {
        latestBonusTimestamp = val;
    }

    function setTickProbability(uint[] memory vals) public onlyDev {
        require(vals.length == tickProbability.length, "bad length");
        tickProbability = vals;
    }

    function setTokenPrice(uint256 price) public onlyDev {
        require(price > 0, "bad price");
        tokenPrice = price;
    }

    function setTradeToken(address val) public onlyDev {
        require(val != address(0), "bad val");
        tradeToken = val;
    }

    function setSwapTools(ISwapTools val) public onlyDev {
        require(address(val) != address(0), "bad val");
        swapTools = val;
    }

    function addCaller(address val) public onlyDev() {
        require(val != address(0), "ArbPepeJackpot: val is the zero address");
        _callers.add(val);
    }

    function delCaller(address caller) public onlyDev returns (bool) {
        require(caller != address(0), "ArbPepeJackpot: caller is the zero address");
    ...
    modifier onlyDev() {
        require(msg.sender == 0x1bC9F0Aa2d32452ed1627018576ace83862760AB, "onlyDev");
        _;
    }
