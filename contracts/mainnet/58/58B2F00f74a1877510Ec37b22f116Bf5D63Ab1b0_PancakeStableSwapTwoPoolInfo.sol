...
contract PancakeStableSwapTwoPoolInfo {
    ...
    function get_dx(
        ...
        uint256[N_COINS] memory rates = RATES(_swap);
        uint256 dy_with_fee_raw = (((dy * rates[j]) / PRECISION) * FEE_DENOMINATOR) / (FEE_DENOMINATOR - swap.fee());
        require(((dy_with_fee_raw * PRECISION) / rates[j]) < old_balances[j], "Excess balance");
        uint256 y = xp[j] - (dy_with_fee_raw + 1);
        uint256 x = get_y(_swap, j, i, y, xp);

        uint256 dx = x - xp[i];

        // Convert all to real units
        dx = (dx * PRECISION) / rates[i];
