...
contract PayoutVault is Initializable, OwnableUpgradeable {
    ...
    enum CampaignStatus {
        NONE,
        CREATED, // this is deprecated. we default to active now
        ACTIVE,
        PAUSED,
        COMPLETED
    }
    ...
    function initialize(address _owner, address _managerAddress, address _workerAddress) public initializer {
        __Ownable_init();
    ...
    function pushRecipientEarnings(
        ...
        if (
            campaigns[_campaignId].status == CampaignStatus.PAUSED ||
            campaigns[_campaignId].status == CampaignStatus.CREATED ||
            campaigns[_campaignId].status == CampaignStatus.NONE
        ) {
            revert Unauthorized();
        }
    ...
    function setCampaignStatus(uint32 _campaignId, CampaignStatus _status) external virtual onlyOwnerAndWorker {
        ...
        /// Campaign is set to status ACTIVE once via `createCampaign` function
        if (
...
contract ProxyFactory is Ownable {
    ...
    event WorkerAddressUpdated(address indexed oldAddress, address newAddress);
    event DeployedProxy(address indexed proxyAddress, address owner, address managerAddress, address workerAddress);
    ...
    function deployProxy(bytes4 _selector, address _managerAddress) external onlyOwnerAndWorker {
        ...
        emit DeployedProxy(address(proxy), owner(), _managerAddress, workerAddress);
