...
contract TheGOAT is ERC721, ERC721Enumerable, Ownable, EIP712, ERC721Votes {
    ...
    using Base64 for *;
    address private constant dev1 = 0x341345adF713a97934a9845a46b55BdCAA26e30e;
    ...
    // for migration from mainnet
    uint private constant forkStart = 174;
    mapping(uint256 => address) public ogs;

    function migrate(uint _id) public{
        require(day[_id].minted == false, "You already migrated your GOAT.");
        require(ogs[_id] == msg.sender, "You are not the owner of this GOAT.");
        safeMint(msg.sender, _id);
    }
    function migrateBatch(uint[] calldata _ids) public{
        for (uint i = 0; i < _ids.length; i++) {
            migrate(_ids[i]);
        }
    }

    function addOg(uint256 _id, address _og) public onlyOwner {
        require(_id < forkStart, "Can't add ogs for days after the fork.");
        ogs[_id] = _og;
    }
    function addBatchOg(uint256[] calldata _ids, address[] calldata _ogs) public onlyOwner {
        for (uint i = 0; i < _ids.length; i++) {
            ogs[_ids[i]] = _ogs[i];
        }
    }
    ...
    constructor() ERC721("The GOAT", "GOAT") EIP712("TheGOAT", "1") {
        ...
        art.skins = [
            [[102, 72, 33], [145, 103, 46]],
            [[79, 79, 79], [128, 128, 128]],
            [[119, 39, 23], [162, 70, 28]],
    ...
    function mintUnminted(uint256 goatId) public {
        require(day[goatId].minted == false, "You already minted your NFT.");
        require(
            goatId + 10 < getGoatsSinceDeployment() && goatId > forkStart  || goatId + 100 < getGoatsSinceDeployment(),
            "Day not reached yet."
        );
        if (goatId % 20 == 0) {
            safeMint(dev2, goatId);
        } else {
            safeMint(dev1, goatId);
        }
    }
    ...
    function tokenURI(
        ...
        bytes memory baseURI = (
            abi.encodePacked(
                ...
                '"}',
                "],",
                '"description": "Created by thegoat.bid","external_url": "https://thegoat.bid","image": "data:image/svg+xml;base64,',
