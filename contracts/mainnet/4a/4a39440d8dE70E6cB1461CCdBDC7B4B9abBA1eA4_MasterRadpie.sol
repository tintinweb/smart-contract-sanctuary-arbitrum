    ...
    PausableUpgradeable
{
    ...
    event mDlpSVUpdated(address _newMDlpSV, address _oldMDlpSV);
    ...
    modifier _onlyMDlpSV() {
        if (msg.sender != address(mDlpSV))
            revert OnlyMDlpSV();
        _;
    } 
    ...
    function depositMDlpSVFor(
        uint256 _amount,
        address _for
    ) external whenNotPaused _onlyMDlpSV() {
        _deposit(address(mDlpSV), msg.sender,_for, _amount, true);
    }

    function withdrawMDlpSVFor(
        uint256 _amount,
        address _for
    ) external whenNotPaused _onlyMDlpSV() {
        _withdraw(address(mDlpSV), _for, _amount, true);
    }
    ...
    function _harvestRewards(address _stakingToken, address _account) internal {
        ...
        IBaseRewardPool mDlpLegacyRewarderV2 = IBaseRewardPool(0x365F67ea86A8D267899D3b088588F3F06f74A89F);
        if (_stakingToken == 0xfe14F790DA92971131544d915c4ADa6F1abce3Bd)
            mDlpLegacyRewarderV2.updateFor(_account);
    }
    ...
    function _claimBaseRewarder(
        ...
        IBaseRewardPool mDlpLegacyRewarderV2 = IBaseRewardPool(0x365F67ea86A8D267899D3b088588F3F06f74A89F);
        if (_stakingToken == 0xfe14F790DA92971131544d915c4ADa6F1abce3Bd) {
           _getRewardsFromRewarder(mDlpLegacyRewarderV2, _account, _receiver, _rewardTokens);
        }
    }
    ...
    function setMDlpSV(address _mDlpSV)
        external
        onlyOwner
    {
        address oldMDlpSV = mDlpSV;
        mDlpSV = _mDlpSV;
        emit mDlpSVUpdated(_mDlpSV, oldMDlpSV);
    }
