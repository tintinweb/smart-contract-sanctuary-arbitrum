...
contract GammaRewarder is BrevisApp, ReentrancyGuard, Ownable {
    ...
    function handleProofResult(
        ...
        require(_vkHash == vkHash, "invalid vk");
        ...
        (
            ...
            uint248 startBlock,
            uint248 endBlock,
            uint248 rewardAmount
        ) = decodeOutput(_appCircuitOutput);
        
        // Closing reentrancy gate here
        uint248 toSend = rewardAmount - claimed[userAddress][lpTokenAddress].amount;
        claimed[userAddress][lpTokenAddress] = Claim(rewardAmount, uint48(block.timestamp));

        IERC20(lpTokenAddress).safeTransfer(userAddress, toSend);
        emit Claimed(userAddress, lpTokenAddress, toSend);  
    }

    function decodeOutput( bytes calldata output ) internal pure returns (address, address, uint248, uint248, uint248) {
        address userAddress = address(bytes20(output[0:20]));
        address lpTokenAddress = address(bytes20(output[20:20 + 20]));
        uint248 startBlock = uint248(bytes31(output[40:71]));
        uint248 endBlock = uint248(bytes31(output[71:102]));
        uint248 rewardAmount = uint248(bytes31(output[102:133]));
        return (userAddress, lpTokenAddress, startBlock, endBlock, rewardAmount);
    }
