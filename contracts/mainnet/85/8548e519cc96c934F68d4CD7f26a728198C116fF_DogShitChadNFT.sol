...
interface IDogeShitNFT is IERC721, IERC721Metadata, IERC721Enumerable {
  // getter
...
contract DogShitChadNFT is IDogeShitNFT, ERC721Pausable, Ownable, AccessControl {
  ...
  // Map the dogshitName for a tokenId
  mapping(uint256 => string) public override dogshitNames;

  mapping(uint256 => Category) public override categoryInfo;

  mapping(uint256 => uint256) private categoryCounter;

  mapping(uint256 => uint256) public override dogeshitNFTToCategory;

  mapping(uint256 => EnumerableSet.UintSet) private _categoryToDogeShitNFTList;
  ...
  event SetDogShitName(uint256 indexed tokenId, string prevName, string newName);
  ...
  modifier onlyGovernance() {
    require(hasRole(GOVERNANCE_ROLE, _msgSender()), "DogeShitNFT::onlyGovernance::only GOVERNANCE role");
    _;
  }

  /// @dev only the one having a MINTER_ROLE can continue an execution
  modifier onlyMinter() {
    require(hasRole(MINTER_ROLE, _msgSender()), "DogeShitNFT::onlyMinter::only MINTER role");
    _;
  }

  modifier onlyExistingCategoryId(uint256 _categoryId) {
    require(_categoryIds.current() >= _categoryId, "DogeShitNFT::onlyExistingCategoryId::categoryId not existed");
    _;
  }

  constructor(string memory _baseURI) public ERC721("DogShitChads", "DCHAD") {
  ...
  function categoryToDogeShitNFTList(uint256 _categoryId)
    external
    view
    override
  ...
  {
    uint256[] memory tokenIds = new uint256[](_categoryToDogeShitNFTList[_categoryId].length());
    for (uint256 i = 0; i < _categoryToDogeShitNFTList[_categoryId].length(); i++) {
      tokenIds[i] = _categoryToDogeShitNFTList[_categoryId].at(i);
    }
    return tokenIds;
  }
   ...
   */
  function tokenURI(uint256 _tokenId) public view virtual override(ERC721, IERC721Metadata) returns (string memory) {
    require(_exists(_tokenId), "DogShitNFT::tokenURI:: token not existed");

    string memory _tokenURI = _tokenURIs[_tokenId];
    string memory _categoryURI = categoryInfo[dogeshitNFTToCategory[_tokenId]].categoryURI;

    // If if category uri exists, use categoryURI as a tokenURI
    if (bytes(_categoryURI).length > 0 && bytes(_tokenURI).length > 0) {
      return string(abi.encodePacked(categoryInfo[dogeshitNFTToCategory[_tokenId]].categoryURI, _tokenURI));
    }
  ...
  {
    uint256 categoryIdToBeRemovedFrom = dogeshitNFTToCategory[_tokenId];
    dogeshitNFTToCategory[_tokenId] = _newCategoryId;
    require(
      _categoryToDogeShitNFTList[categoryIdToBeRemovedFrom].remove(_tokenId),
      "DogeShitNFT::updateTokenCategory::tokenId not found"
    );
    require(
      _categoryToDogeShitNFTList[_newCategoryId].add(_tokenId),
      "DogeShitNFT::updateTokenCategory::duplicated tokenId"
    );
  ...
  /**
   * @dev Get the associated dogshitName for a unique tokenId.
   */
  function getDogShitNameOfTokenId(uint256 _tokenId) external view override returns (string memory) {
    return dogshitNames[_tokenId];
  }
    ...
    returns (uint256)
  {
    ...
    _tokenIds.increment();
    dogeshitNFTToCategory[newId] = _categoryId;
    uint256 _tokenURI = categoryCounter[_categoryId]++;
    require(_categoryToDogeShitNFTList[_categoryId].add(newId), "DogeShitNFT::mint::duplicated tokenId");
    _mint(_to, newId);
  ...
  function _setTokenURI(uint256 _tokenId, string memory _tokenURI) internal virtual override {
    require(_exists(_tokenId), "DogeShitNFT::_setTokenURI::tokenId not found");
   ...
   */
  function setDogShitName(uint256 _tokenId, string calldata _name) external onlyGovernance {
    string memory _prevName = dogshitNames[_tokenId];
    dogshitNames[_tokenId] = _name;

    emit SetDogShitName(_tokenId, _prevName, _name);
  }
