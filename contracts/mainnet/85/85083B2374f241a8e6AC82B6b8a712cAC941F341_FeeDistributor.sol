...
contract FeeDistributor is Initializable, OwnableUpgradeable {
    ...
    // mapping for tracking which addresses were added to `feeTokens`
    mapping(address => bool) public seenFees;
    ...
    function setAddresses(ITokenBooster _tokenBooster) external onlyOwner {
        tokenBooster = _tokenBooster;
        startTime = _tokenBooster.startTime();
    }
    ...
    function depositFee(address _token, uint256 _amount) external {
        if (_amount > 0) {
            ...
            uint256 received = IERC20Upgradeable(_token).balanceOf(
                address(this)
            );
    ...
    /**
        @notice Claim accrued protocol fees according to a staked balance in `TokenBooster`.
        ...
        returns (uint256[] memory claimedAmounts)
    {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            IERC20Upgradeable(token).safeTransfer(_user, claimedAmounts[i]);
        ...
        returns (uint256, StreamData memory)
    {
        ...
        for (uint256 i = lastClaimWeek; i < claimableWeek; i++) {
            (uint256 userWeight, uint256 totalWeight) = tokenBooster
                .weeklyWeight(_user, i);
    ...
    function _buildStreamData(
        ...
        (uint256 userWeight, uint256 totalWeight) = tokenBooster.weeklyWeight(
            _user,
            _week
        );
        uint256 amount;
        ...
        if (userWeight > 0) {
            amount =
                (weeklyFeeAmounts[_token][_week] * userWeight) /
                totalWeight;
