...
// FrenChef is the master of wGM. He can make wGM and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once wGM is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract FrenChef is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of wGMs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. wGMs to distribute per block.
        uint256 lastRewardBlock; // Last block number that wGMs distribution occurs.
        uint256 accwGMPerShare; // Accumulated wGMs per share, times 1e12. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
    }

    // The wGM TOKEN!
    WrappedGM public wgm;
    // Dev address.
    address public devaddr;
    // wGM tokens created per block.
    uint256 public wGMPerBlock;
    // Bonus muliplier for early wgm makers.
    uint256 public constant BONUS_MULTIPLIER = 1;
    // Deposit Fee address
    address public feeAddress;
    address public NFAERC721 = 0x249bB0B4024221f09d70622444e67114259Eb7e8;
    address public NFAERC1155 = 0x5A8b648dcc56e0eF241f0a39f56cFdD3fa36AfD5;
    mapping(uint256 => uint256) public pid_to_LPID;
    // Info of each pool.
    ...
    constructor(WrappedGM _wgm, uint256 _wGMPerBlock, uint256 _startBlock) public {
        wgm = _wgm;
        devaddr = owner();
        feeAddress = owner();
        wGMPerBlock = _wGMPerBlock;
    ...
    // View function to see pending wGMs on frontend.
    function pendingwGM(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accwGMPerShare = accwGMPerShare.add(wGMReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accwGMPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accwGMPerShare = pool.accwGMPerShare.add(wGMReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 NFTAmount = IToken(NFAERC721).balanceOf(msg.sender);
        uint256 ERC1155Amount = IERC1155(NFAERC1155).balanceOf(msg.sender, pid_to_LPID[_pid]);

        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accwGMPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeWGMTransfer(msg.sender, pending);
                if (NFTAmount > 0 || ERC1155Amount > 0) {
                    uint256 bonusAmount = (pending * ERC1155Amount * NFTAmount * BONUS_MULTIPLIER) / 100;
                    if (bonusAmount > pending) {
                        bonusAmount = pending;
                    }
                    wgm.mint(msg.sender, bonusAmount);
                }
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accwGMPerShare).div(1e12);
    ...
    // Safe wgm transfer function, just in case if rounding error causes pool to not have enough wGMs.
    function safeWGMTransfer(address _to, uint256 _amount) internal {
        uint256 wgmBal = wgm.balanceOf(address(this));
        if (_amount > wgmBal) {
            wgm.safeWGMTransfer(_to, wgmBal);
        } else {
            wgm.safeWGMTransfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _wGMPerBlock) public onlyOwner {
        massUpdatePools();
        wGMPerBlock = _wGMPerBlock;
    }
}
...
contract WrappedGM is ERC20, Ownable, ReentrancyGuard {
    //wGM token
    address public NFA_ERC721 = 0x249bB0B4024221f09d70622444e67114259Eb7e8;
    address public GM_ERC20 = 0xC20ad20050151a3E542118559a5e90667fa710df;
