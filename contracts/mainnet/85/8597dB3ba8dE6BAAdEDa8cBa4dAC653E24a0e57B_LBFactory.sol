 ...
 * @author Trader Joe, Sam (543#3017, ELITE & Guru Network)
 * @notice Required interface of LBFactory contract
 */
interface ILBFactory is IPendingOwnable {
    ...
    event FeeTakerSet(address indexed lbPair, address indexed oldRecipient, address indexed newRecipient);
    ...
    event NewArt(address indexed newArt, address indexed oldArt);
    ...
    function getFeeTaker(address lbPair) external view returns (address);

    function getPairSupportsInterface(bytes4 iid) external view returns (bool);
    ...
    function setFeeTaker(address lbPair, address feeRecipient) external;

    function setPairSupportsInterface(bytes4 iid, bool status) external;
...
import {PendingOwnable} from "./PendingOwnable.sol";
...
contract LBFactory is PendingOwnable, ILBFactory {
    ...
    address private _feeRecipient; // Default
    mapping(address=>address) private _feeTaker; // Perp
    uint256 private _flashLoanFee;

    address private _lbPairImplementation;

    address private _artProxy;
    mapping(bytes4=>bool) private _pairSupportsInterface;
    ...
    /**
     * @notice Get the protocol fee taker
     * @return feeTaker
     */
    function getFeeTaker(address _p) external view override returns (address feeTaker) {
        address _ft = _feeTaker[_p];
        return _ft == address(0) ? _feeRecipient : _ft;
    }

    /**
     * @notice Function to get the PairSupportsInterface
     * @param iid Interface ID is first 4 bytes derived from Keccak hash of all its functions XOR'd
     * @return status Support for interface
     */
    function getPairSupportsInterface(bytes4 iid) external view override returns (bool status) {
        status = _pairSupportsInterface[iid];
    }
    ...
    /**
     * @notice Get the URI for a LBPair
     * @param lbPair The address of the LBPair
     * @param id The token ID to be seeked
     */
    function getUri(address lbPair, uint256 id) external view override returns (string memory) {
        return IArt(_artProxy).getUri(lbPair, id);
    }
     ...
     * @param feeRecipient The address of the recipient
     */
    function setFeeTaker(address lbPair, address feeRecipient) external override onlyOwner {
        _setFeeTaker(lbPair, feeRecipient);
    }

    /**
     * @notice Function to set the Art Proxy for ERC-1155 Tokens
     * @param art The address of the Art Proxy
     */
    function setArtProxy(address art) external onlyOwner {
        _setArtProxy(art);
    }

    /**
     * @notice Function to set the PairSupportsInterface
     * @param iid Interface ID is first 4 bytes derived from Keccak hash of all its functions XOR'd
     * @param status Support for interface
     */
    function setPairSupportsInterface(bytes4 iid, bool status)  external onlyOwner {
        _setPairSupportsInterface(iid, status);
    }
    ...
    /**
     * @notice Internal function to set the taker of the fee
     * @param feeRecipient The address of the taker
     */
    function _setFeeTaker(address lbPair, address feeRecipient) internal {
        //if (feeRecipient == address(0)) revert LBFactory__AddressZero();

        address oldFeeRecipient = _feeTaker[lbPair];
        if (oldFeeRecipient == feeRecipient) revert LBFactory__SameFeeRecipient(_feeRecipient);

        _feeTaker[lbPair] = feeRecipient;
        emit FeeTakerSet(lbPair, oldFeeRecipient, feeRecipient);
    }

    /**
     * @notice Internal function to set the Art Proxy
     * @param art The address of the Art Proxy
     */
    function _setArtProxy(address art) internal {
        emit NewArt(art, _artProxy);
        _artProxy = art;
    }
    /**
     * @notice Internal function to set the PairSupportsInterface
     * @param iid Interface ID is first 4 bytes derived from Keccak hash of all its functions XOR'd
     * @param status Support for interface
     */
    function _setPairSupportsInterface(bytes4 iid, bool status) internal {
        _pairSupportsInterface[iid] = status;
    }
