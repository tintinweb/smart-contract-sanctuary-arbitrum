...
contract SeparatePool is ERC20Permit, ISeparatePool {
     ...
     */
    function changeOwner(address _newOwner) external {
        require(msg.sender == owner || msg.sender == factory, "SeparatePool: Not owner/factory");
    ...
    function release(uint256 _id) external onlyOwner {
        ...
        _mint(ISeparatePoolFactory(factory).incomeMaker(), RELEASE_PUNISHMENT_AMOUNT);
    ...
    function _chargeFee(uint256 _amount) private {
        address FUR = ISeparatePoolFactory(factory).fur();

        // No fees before FUR is launched
        if (FUR == address(0)) return;
        else IERC20(FUR).transferFrom(msg.sender, ISeparatePoolFactory(factory).incomeMaker(), _amount);
    }
