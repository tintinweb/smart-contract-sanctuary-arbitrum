...
contract TokenIDOETH is Ownable, ReentrancyGuard {
    ...
    //amount
    uint256 public MaxIDOPayInAmount = 90900000000000000000; // 90.9 ETH
    uint256 public receivedIDOPayInAmount;

    uint256 public MinhitelistSinglePayInAmount = 3000; // 0.3 ETH
    uint256 public MaxWhitelistSinglePayInAmount = 900000000000000000; // 0.9 ETH
    ...
    //price
    uint256 public whiteListPrice = 600000000000000; // 0.0006 ETH per EDE
    ...
    function witheListSale(uint256 amount) internal {
        ...
        require(MaxWhitelistSinglePayInAmount >= amount + whitelistPayInAmountCounter[msg.sender], "Maximum buy amount: 0.9 ETH");
    ...
    function buy() nonReentrant payable public {
        ...
        if (whitelistPurchaseAmount[msg.sender] == 0) {
            require(payInEthAmount >= MinhitelistSinglePayInAmount, "Minimum buy amount: 0.3 ETH");
        } else if (whitelistPurchaseAmount[msg.sender] < MinhitelistSinglePayInAmount) {
            require(payInEthAmount >= MinhitelistSinglePayInAmount.sub(whitelistPurchaseAmount[msg.sender]), "Minimum buy amount: 0.3 ETH");
        }
