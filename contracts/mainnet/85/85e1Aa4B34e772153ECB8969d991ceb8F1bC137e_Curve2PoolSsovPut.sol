...
contract Curve2PoolSsovPut is ContractWhitelist, Pausable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_baseToken != address(0), 'E1');
        ...
        returns (uint256 pnl)
    {
        ...
        require(strike != 0, 'E12');
        require(
            IERC20(epochStrikeTokens[currentEpoch][strike]).balanceOf(
                msg.sender
            ) >= amount,
            'E15'
        );
        ...
        OptionsToken(epochStrikeTokens[currentEpoch][strike]).burnFrom(
            msg.sender,
            amount
        );
        ...
        returns (uint256 userQuoteTokenWithdrawAmount, uint256 rewards)
    {
        ...
        rewards =
            (crvToDistribute[currentEpoch] *
                (totalEpochStrikeDeposits[currentEpoch][strike] +
                    totalEpochStrikePremium[currentEpoch][strike])) /
            (totalEpochDeposits[currentEpoch] +
                totalEpochPremium[currentEpoch]);
...
contract OptionsToken is ERC20PresetMinterPauserUpgradeable {
    ...
    /// @dev The address of the base asset for the option
    address public baseToken;

    /// @dev The address of the quote asset for the option
     ...
     * @param _baseToken The address of the base token for the option
