...
contract CasinoYieldUSDT is Ownable, Pausable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount, address partner) external whenStarted nonReentrant {
    ...
    function reinvest() external whenStarted nonReentrant {
        _updateNotWithdrawn();
        require(stake[_msgSender()].notWithdrawn > 30000000, "Too low amount to reinvest");
        require(!left[_msgSender()], "Left");
    ...
    function sendReferralAmount(address investor, uint256 amount, bool isStakingReferral) internal {
        ...
        for(uint256 i = 0; i < 3; i++) {                
            ...
            IERC20(USDTContract).safeTransfer(_partner, refAmount);
