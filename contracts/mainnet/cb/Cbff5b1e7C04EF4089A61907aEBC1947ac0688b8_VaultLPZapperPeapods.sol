...
abstract contract PeapodsLPZapperBase {
    ...
    function _swapCamelotYak(
        ...
        CamelotYakRouter.Trade memory params = 
            CamelotYakRouter.Trade({
                amountIn: _amount,
                amountOut: 0,
                path: path,
                adapters: adapters,
    ...
    //     address tokenA = baseToken[pair.token0()]; 
    //     address tokenB = baseToken[pair.token1()]; 

    //     uint256 _amount = IERC20(tokenIn).balanceOf(address(this));

    //     if(_amount > 0 && tokenA == ohm){
    ...
    //         adapters[0] = usdc_usdcE_adapter;
    //         adapters[1] = weth_ohm_adapter;

    //         address[] memory recipients = new address[](2);
    //         recipients[0] = usdc_usdcE_adapter;
    //         recipients[1] = weth_ohm;

    //         _swapCamelotYak(tokenIn, path, adapters, recipients, _amount.div(2));
    //     }
            
    //     if(_amount > 0 && tokenA == peas){
    //         address[] memory path = new address[](2);
    //         path[0] = tokenIn;
    //         path[1] = tokenA;
    ...
    //         _swapCamelotYak(tokenIn, path, adapters, adapters, _amount.div(2));
    //     }
            
    //     if(_amount > 0 && tokenA == gmx) {
    //         _swapCamelot(tokenIn, weth, _amount.div(2));
    //         _swapCamelot(weth, tokenA, IERC20(weth).balanceOf(address(this)));
    //     }
        

    //     if(tokenB != weth){
    //         if(_amount > 0 && tokenB == ohm){
    //             address[] memory path = new address[](3);
    //             path[0] = tokenIn;
    //             path[1] = weth;
    //             path[2] = tokenB;
    ...
    //             _swapCamelotYak(tokenIn, path, adapters, adapters, _amount.div(2));
    //         }
    //         if(_amount > 0 && tokenB == gmx) {
    //             _swapCamelot(tokenIn, weth, _amount.div(2));
    //             _swapCamelot(weth, tokenB, IERC20(weth).balanceOf(address(this)));

    //         }
    //     }

    //     _swapAndStake(vault, tokenAmountOutMin, tokenA);
    // }
