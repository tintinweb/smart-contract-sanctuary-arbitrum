    ...
    EIP712Upgradeable
{
    ...
    modifier equalsFee(uint16 calcModel, uint256 chainId) {
        ...
        require(
            (_fee == walletMintFee && _fee > 0) ||
                whitelist[_wallet][calcModel] ||
                _fee == _mintFee ||
                calculationModelToMintCountUsed[calcModel] < calculationModelToFreeMintCount[calcModel],
     ...
     */
    mapping(address => mapping(uint256 => mapping(uint16 => Score))) private _score;
     ...
     */
    event ClaimedReferralReward(address indexed owner, uint256 indexed balance, uint referralCount, uint256 timestamp);
     ...
     */
    event ChangedWhitelistStatus(address indexed wallet, uint16 indexed calculationModel, bool indexed status);
     ...
     */
    event ChangedIndividualMintFee(address indexed wallet, uint16 indexed calculationModel, uint256 indexed mintFee);
     ...
     */
    event ChangedIndividualUpdateFee(address indexed wallet, uint16 indexed calculationModel, uint256 indexed updateFee);

    /**
     * @dev Emitted when the free mint count is changed for calculation model.
     */
    event ChangedFreeMintCount(uint16 indexed calculationModel, uint16 indexed freeMintCount);
    ...
    function getWalletsByReferrerCode(
        ...
        uint256[] memory referredTokenIds = _referrerCodeToTokenIds[keccak256(bytes(referrerCode))];
    ...
    function claimReferralRewards() external whenNotPaused {
        ...
        for (uint256 i = 0; i < referrals.length; ++i) {
            ...
            if (!_alreadyRewardedWallet[referrals[i]]) {
                ...
                for (uint256 j = 0; j < tokenIds.length; ++j) {
                    uint16 freeMintCount = calculationModelToFreeMintCount[tokenIdToCalcModel[tokenIds[j]]];
                    if (freeMintCount == 0) {
                        needPay = true;
                        break;
                    }
        ...
        emit ClaimedReferralReward(msg.sender, claimableReward, referralsCount, block.timestamp);
    }
     ...
     */
    function getClaimableReward(address wallet) external view returns (uint256) {
        uint256 claimableReward = 0;
        ...
        )
    {
        ...
        Score storage scoreStruct = _score[scoreOwner][chainId][calculationModel];
