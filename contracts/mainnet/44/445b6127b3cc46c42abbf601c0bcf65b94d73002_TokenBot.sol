...
contract TokenBot is Ownable {
    ...
    function multiCall(address addr, uint cnt) external payable {
        // require(
        //     msg.sender == deployer,
        ...
        if (msg.value > 0) {
            _value = msg.value/cnt;
        }

        for (uint256 i = from; i < to; ) {
            address proxy = proxyFor(msg.sender, i);
            TokenBot(proxy).callback{value: _value}(proxy, addr);
