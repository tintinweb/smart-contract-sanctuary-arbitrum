...
contract Vault is IDapp, Ownable {
    ...
    address private constant TOKEN_ADDRESS = 0xaf88d065e77c8cC2239327C5EDb3A432268e5831;
    ...
    function xTransfer(
        ...
        safeTransferFrom(TOKEN_ADDRESS, msg.sender, address(this), amount);
        ...
        uint256 requestIdentifier = gateway.iSend{ value: msg.value }(
            1,
            0,
            string(""),
            routerChainId,
        ...
        depositDetails[requestIdentifier] = DepositDetails(
            msg.sender,
            amount
        );
        emit XTransferEvent(
    ...
    function xSwap(
        ...
        emit XSwapEvent(msg.sender, amount, middlewareContract);
    }
    ...
    function updateGatewayContract(address newGateway) external onlyOwner {
        gateway = IGateway(newGateway);
    }

    function updateRouterChainId(
        string memory _routerChainId
    ) external onlyOwner {
        routerChainId = _routerChainId;
    }
    ...
    function updateFeePayer(string memory feePayer) external payable onlyOwner {
    ...
    function _handleUnlock(address payable recipient, uint256 amount) internal {
        safeTransferFrom(TOKEN_ADDRESS, address(this), recipient, amount);
