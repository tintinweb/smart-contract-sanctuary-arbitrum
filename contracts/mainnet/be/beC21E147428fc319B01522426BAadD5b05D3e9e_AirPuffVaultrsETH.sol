...
abstract contract AirPuffVault is IVault, OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC20BurnableUpgradeable {
    ...
    */
    ...
    /**
    * @notice Delete the lending vault address for a specific asset
    * @param _asset The asset for which to delete the vault
    * @dev Only callable by the contract owner. Emits SetLendingVault event.
    */
    function deleteLendingVault(address _asset) public onlyOwner {
        require(lendingVault[_asset] != address(0), "AirPuff: vault not set");
        for (uint256 i = 0; i < allLendingVaults.length; i++) {
            if (allLendingVaults[i] == lendingVault[_asset]) {
                allLendingVaults[i] = allLendingVaults[allLendingVaults.length - 1];
                allLendingVaults.pop();
                break;
            }
        }
        delete lendingVault[_asset];
        emit SetLendingVault(_asset, address(0));
    }
    ...
    */
    function cancelOpenPositionRequest(uint256 _requestPositionID, address _user) external onlyOwner {
        ...
        if (opr.asset == strategyAddresses.WETH) {
            ...
            payable(opr.user).transfer(opr.amount);
        } else {
        ...
        opr.done = true;
        payable(opr.user).transfer(feeConfiguration.executionFee);
    ...
    */
    function executeOpenPosition(
        ...
        opr.done = true;
        payable(IAirPuffHandler(strategyAddresses.AirPuffHandler).keeper()).transfer(feeConfiguration.executionFee);
    ...
    */
    function executeClosePosition(
        ...
        if (!cpr.isLiquidation) {
            ...
            if (kcd.amountOut < (_userInfo.leverageAmount+kcd.interestsAmount)) {
            ...
            IERC20Upgradeable(kcd.posBorrow).safeIncreaseAllowance(kcd.lenVault, _userInfo.leverageAmount);
            ...
            IERC20Upgradeable(strategyAddresses.LRTAsset).safeTransfer(strategyAddresses.AirPuffHandler, cpr.lendingRepayment);
            uint256 amountOut = IAirPuffHandler(strategyAddresses.AirPuffHandler).executeSwap(cpr.lendingRepayment,_data,_swapSimple, strategyAddresses.LRTAsset, kcd.posBorrow);
            IERC20Upgradeable(kcd.posBorrow).safeIncreaseAllowance(kcd.lenVault, amountOut);
            ILending(kcd.lenVault).repayDebt(_userInfo.leverageAmount, amountOut);
        ...
        emit Withdraw(_userInfo.user, cpr.toLeverageUser, cpr.toLeverageUser, block.timestamp, cpr.userPositionID, _userInfo.leverage, kcd.posBorrow);
    }
    ...
    */
    function closePosition(uint256 _positionID) external payable nonReentrant {
        ...
        //console.log("lendingDebt sub mfee",lendingDebt);

        //Position value in LRTAsset
        data.totalLRTAssetPosition = _userInfo.position - closeData.mFee;
        //console.log("totalLRTAssetPosition",data.totalLRTAssetPosition);
        ...
        //console.log("lendingDebt + slippage",lendingDebt);
        closeData.toLeverageUser = data.totalLRTAssetPosition - data.lendingRepayment;
        //console.log("toLeverageUser in eETH",closeData.toLeverageUser);
        ...
        //console.log("lendingRepayment",cpr.lendingRepayment);
        cpr.mFee = closeData.mFee;
        cpr.toLeverageUser = closeData.toLeverageUser - withdrawFee;
        //console.log("toLeverageUser",cpr.toLeverageUser);
        ...
        emit ClosePositionRequested(msg.sender, cpr.closeRequestID);
    }
    ...
    */
    function liquidatePosition(uint256 _positionID, address _user) external nonReentrant {
        ...
        IERC20Upgradeable(strategyAddresses.LRTAsset).safeTransfer(msg.sender, liquidatorReward);
    ...
    */
    function _getInputAssetValue(uint256 _amount, address _asset) internal view returns (uint256,uint256) {
        ...
        //uint256 LRTAssetPrice = 1.028636761518e18;
        //console.log("LRTAssetPrice",LRTAssetPrice);
        if (_asset == strategyAddresses.LRTAsset) {
