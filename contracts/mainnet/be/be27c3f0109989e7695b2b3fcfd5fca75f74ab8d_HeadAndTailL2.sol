...
contract HeadAndTailL2 is Pausable {
    ...
    address public devFeeAddress = 0x4B12158ED3b1fD0414742E35e5DEba85Bb7ebd66;
    uint256[] acceptAmounts = [
        0.005 ether,
        0.01 ether,
        0.02 ether,
        0.05 ether,
    ...
    address private weth = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1; //0x383a955F07EDB9de0b483a089C06dffc75D47620; //0xD7ACd2a9FD159E69Bb102A1ca21C9a3e3A5F771B; //mainnet = ;
    ...
    function _cancelGame(uint256 id) internal {
        ...
        Address.sendValue(payable(games[id].player1), games[id].amount);
    ...
    function joinGame(uint256 id, uint256 player2GameId)
        ...
        whenNotPaused
    {
        require(!Address.isContract(msg.sender) || !limitPlayer);
        ...
        Address.sendValue(
            payable(devFeeAddress),
            msg.value.sub(games[id].amount)
        );
        _joinGame(
            id,
            msg.sender,
    ...
    function flipCoin(
        ...
        require(_exist(amount, acceptAmounts), "wrong amount");
        require(games[id].player1 == address(0), "id exists");
        require(
            prediction == COIN_SIDE.TAIL || prediction == COIN_SIDE.HEAD,
            "wrong prediction"
        );

        uint256 oldid = _findGame(msg.sender, amount, prediction);
        //payable(devFeeAddress).transfer(msg.value.sub(amount));
        Address.sendValue(payable(devFeeAddress), msg.value.sub(amount));
        if (oldid > 0) {
    ...
    function getGame(uint256 id) external view returns (Game memory) {
        if (Address.isContract(msg.sender) && limitPlayer) {
            return games[0];
        }
    ...
    function _getRandomNumber() private returns (bool) {
        uint256 step = uint256(
            keccak256(
                abi.encodePacked(
                    block.number,
                    block.timestamp,
    ...
    function setDevFee(uint256 _devFee) external onlyOwner {
        ...
        require(_devFee >= 0, "_");
        require(openingGames.length == 0, "game not finished yet");
        devFee = _devFee;
    }
    ...
    function updateAcceptAmounts(uint256[] calldata _amounts)
        external
        onlyOwner
    {
        require(openingGames.length == 0, "game not finished yet");
        delete acceptAmounts;
        for (uint256 i = 0; i < _amounts.length; ) {
            acceptAmounts.push(_amounts[i]);
            unchecked {
                i++;
            }
        }
    ...
    function setLimitPlayer(bool _limitPlayer) external onlyOwner {
        limitPlayer = _limitPlayer;
    }
