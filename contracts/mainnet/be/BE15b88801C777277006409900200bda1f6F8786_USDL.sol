...
contract UFragments is Initializable, OwnableUpgradeable, ERC20Detailed {
  ...
  function transfer(address to, uint256 value) external override validRecipient(to) returns (bool) {
    ...
    if(_FeePool[to] == true || _FeePool[msg.sender] == true) {
      if(_exempt[msg.sender] == false){
        _gonBalances[msg.sender] = _gonBalances[msg.sender] - gonValue;
        _gonBalances[_treasury] = _gonBalances[_treasury] + ((gonValue * feeBasis) / 1000);
        uint256 inverse = 1000 - feeBasis;
        _gonBalances[to] = _gonBalances[to] + ((gonValue * inverse) / 1000);
        emit Transfer(msg.sender, to, ((value * inverse) / 1000));
      } else {
        _gonBalances[msg.sender] = _gonBalances[msg.sender] - gonValue;
  ...
  function transferFrom(
    ...
    if(_FeePool[to] == true || _FeePool[msg.sender] == true){
        if(_exempt[to] == true || _exempt[msg.sender] == true){
          ...
          uint256 inverse = 1000 - feeBasis;
          _gonBalances[to] = _gonBalances[to] + ((gonValue * inverse) / 1000);
          emit Transfer(from, to, ((value* inverse) / 1000));
        }
    } else {
        _gonBalances[from] = _gonBalances[from] - gonValue;
        _gonBalances[to] = _gonBalances[to] + gonValue;
  ...
  function transferAllFrom(address from, address to) external validRecipient(to) returns (bool) {
    ...
    if(_FeePool[to] == true || _FeePool[from] == true){
        if(_exempt[to] == true || _exempt[from] == true){
  ...
  function updateExempt(address _address, bool _bool) public {
    require(msg.sender == _admin);
    _exempt[_address] = _bool;
  }
}
/* solhint-enable */
