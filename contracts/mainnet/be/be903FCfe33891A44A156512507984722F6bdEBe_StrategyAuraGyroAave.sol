...
contract StrategyAuraGyroAave is StratFeeManagerInitializable {
    using SafeERC20 for IERC20;

    // Tokens used
    address public constant want = 0x125bC5a031B2Db6733bfa35d914ffa428095978B;
    address public constant output = 0x040d1EdC9569d4Bab2D15287Dc5A4F10F56a56B8;
    address public constant native = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1;
    address public constant lp0 = 0xaf88d065e77c8cC2239327C5EDb3A432268e5831;
    address public constant lp1 = 0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9;
    address public constant stakedLp0 = 0x7CFaDFD5645B50bE87d546f42699d863648251ad;
    address public constant stakedLp1 = 0xb165a74407fE1e519d6bCbDeC1Ed3202B35a4140;
    ...
    function addLiquidity() internal {
        ...
        if (nativeBal > 0) {
            ...
            IAaveWrapper(stakedLp0).deposit(IERC20(lp0).balanceOf(address(this)), address(this));
            IAaveWrapper(stakedLp1).deposit(IERC20(lp1).balanceOf(address(this)), address(this));

            uint256 stakedLp0Bal = IERC20(stakedLp0).balanceOf(address(this));
            uint256 stakedLp1Bal = IERC20(stakedLp1).balanceOf(address(this));
            
            BalancerActionsLib.multiJoin(unirouter, want, IBalancerPool(want).getPoolId(), stakedLp0, stakedLp1, stakedLp0Bal, stakedLp1Bal);
        }
    }

    function _calcSwapAmount() private returns (uint256 lp0Amt, uint256 lp1Amt) {
            uint256 nativeBal = IERC20(native).balanceOf(address(this));
            lp0Amt = nativeBal / 2;
            ...
            uint256 amountA = IAaveWrapper(stakedLp0).convertToAssets(balances[0]) * 1e18 / supply;
            uint256 amountB = IAaveWrapper(stakedLp1).convertToAssets(balances[1]) * 1e18 / supply;
    ...
    function _giveAllowances() internal {
        ...
        IERC20(lp0).safeApprove(stakedLp0, type(uint).max);
        IERC20(lp1).safeApprove(stakedLp1, type(uint).max);
        IERC20(stakedLp0).safeApprove(unirouter, type(uint).max);
        IERC20(stakedLp1).safeApprove(unirouter, type(uint).max);
    ...
    function _removeAllowances() internal {
        ...
        IERC20(lp0).safeApprove(stakedLp0, 0);
        IERC20(lp1).safeApprove(stakedLp1, 0);
        IERC20(stakedLp0).safeApprove(unirouter, 0);
        IERC20(stakedLp1).safeApprove(unirouter, 0);
