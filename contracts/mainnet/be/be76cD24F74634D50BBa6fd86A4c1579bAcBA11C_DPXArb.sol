/**
 *Submitted for verification at Arbiscan on 2022-06-08
*/
...
contract BKeeper {
    uint maxEthQty = 1e18;
    uint minQty = 1e10; // = 1e10;
    uint minProfitInEth = 1e15;

    address public admin;
    address[] public bamms;
    mapping(address => address) bamm2arb;
    ...
    function findSmallestQty() public returns(uint, address) {
        for(uint i = 0 ; i < bamms.length ; i++) {
            ...
            for(uint qty = maxEthQty ; qty > minQty ; qty = qty / 2) {
                ...
                ArbLike arb = ArbLike(bamm2arb[bamm]);
                try arb.swap(qty, bamm) {
                    uint balanceAfter = address(this).balance;
                    if(balanceBefore + minProfitInEth <= balanceAfter) return (qty, bamm);
                } catch {
    ...
    function checkUpkeep(bytes calldata /*checkData*/) external returns (bool upkeepNeeded, bytes memory performData) {
        ...
        upkeepNeeded = qty > 0;
        performData = abi.encode(qty, bamm);
    }
    
    function performUpkeep(bytes calldata performData) external {
        (uint qty, address bamm) = abi.decode(performData, (uint, address));
        require(qty > 0, "0 qty");
    ...
    function setParams(uint _minQty, uint _maxQty, uint _minProfit) external {
        require(msg.sender == admin, "!admin");
        
        minQty = _minQty;
        maxEthQty = _maxQty;
        minProfitInEth = _minProfit;
    }

    function addBamm(address newBamm, address arb) external {
        require(msg.sender == admin, "!admin");        
        bamms.push(newBamm);
        bamm2arb[newBamm] = arb;
    }
