...
import "../../core/flashloan/base/FlashLoanReceiverBase.sol";
    ...
    Pausable
{
    ...
    function leverageGlp(
        ...
        /* Check whether msg.sender has approved sufficient allowance */
        IERC20 collateral = IERC20(_collateral);
        require(
            collateral.allowance(msg.sender, address(this)) > _amount,
            "Not enough allowance for collateral transfer to leverager"
        );

        /* Transfer colletaral to contract */
        collateral.safeTransferFrom(msg.sender, address(this), _amount); // Collateral allowances: msg.sender => Leverager
        ...
        uint256 collateralUsdValue = _calculateCollateralUsdValue(
            address(glpToken),
            glpAmount
        );

        /* Calculate leveraged position value */
        uint256 totalFlashloanUsdValue = collateralUsdValue.mul(_leverage - 1); //8 decimals USD
        ...
        //     0
        // );
        flashLoanCall(totalFlashloanUsdValue, _borrowTokens, _ratio, _leverage);
    }

    function flashLoanCall(
        uint256 _totalFlashloanUsdValue,
        address[] calldata _borrowTokens,
        uint256[] calldata _ratio,
        uint256 _leverage //
    ) internal {
        ...
        for (uint256 i = 0; i < _borrowTokens.length; i++) {
            /* Calculate borrow amount for each token according to its ratio */
            uint256 flashloanUsdValue = _totalFlashloanUsdValue
                .mul(_ratio[i])
                .div(RATIO_DIVISOR); // 8 decimals USD
    ...
    function setValidBorrowAssets(
        ...
        for (uint256 i = 0; i < _borrowAssets.length; i++) {
            ...
            IERC20(_borrowAssets[i]).approve(address(POOL), type(uint256).max);
    ...
    function _calculateCollateralUsdValue(
        address _token,
        uint256 _amount
        ...
        uint256 collateralUsdValue = protocolOracle
            ...
            .mul(_amount)
            .div(10 ** ERC20(_token).decimals()); // 8 decimals USD
    ...
    function executeOperation(
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            if (amounts[i] != 0) {
                IERC20 borrowedAsset = IERC20(assets[i]);
                require(
                    borrowedAsset.allowance(
                        address(this),
                        address(gmxRewardRouter)
                    ) > amounts[i],
                    "Not enough allowance to stake in executeOperation"
                );
                require(
                    ...
                    "Failed to supply to gmx reward router"
                ); // Borrow tokens allowances: Leverager => Reward Router
            }
        }

        uint256 glpAmount = IERC20(glpToken).balanceOf(address(this));

        supplyAndRepayFlashloan(
            assets,
            amounts,
            premiums,
            originalCaller,
    ...
    function supplyAndRepayFlashloan(
        ...
        pool.supply(glpToken, _glpAmount, _originalCaller, 0);
        for (uint256 i = 0; i < _assets.length; i++) {
            pool.borrow(
                _assets[i],
                _amounts[i].add(_premiums[i]),
                2,
                0,
                _originalCaller
            );
