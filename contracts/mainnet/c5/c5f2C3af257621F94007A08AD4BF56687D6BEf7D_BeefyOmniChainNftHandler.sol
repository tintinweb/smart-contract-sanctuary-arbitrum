...
contract BeefyOmniChainNftHandler is NonblockingLzApp {
    ...
    mapping(uint16 => uint256) public chainGas; // gas by chain id
    ...
    event GasDeposit(uint256 amount);
    ...
    function setGas(uint16 _chainId, uint256 _gas) external onlyOwner {
        chainGas[_chainId] = _gas;
    }
    ...
    function _estimateFees(uint16 _dstChainId) private view returns (uint nativeFee, uint zroFee) {
        bytes memory adapterParams = abi.encodePacked(version, chainGas[_dstChainId]);
    ...
    function claimAllAndVote() external onlyAuth {
        for (uint i; i < chainIds.length; i++) {
            ...
            _sendMessage(chainIds[i]);
        }
    }

    function claimAllAndVote(uint16[] calldata _chainIds) external onlyAuth {
        for (uint i; i < _chainIds.length; i++) {
            if (_chainIds[i] == thisChainId) {
                _claimAndVote();
                continue;
            }

            _sendMessage(_chainIds[i]);
        }
    }

    function _sendMessage(uint16 _chainId) private {
            (uint nativeFee, ) = _estimateFees(_chainId);
            bytes memory adapterParams = abi.encodePacked(version, chainGas[_chainId]);
