...
interface IExchangeRouter {
    ...
    struct CreateWithdrawalParams {
        ...
        address[] longTokenSwapPath;
        address[] shortTokenSwapPath;
        // uint256 marketTokenAmount; // note not sure about it
...
contract VodkaVaultV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function getCurrentPosition(
        ...
        //@todo will not work as getGMPrice cannot be executed
    ...
    function fulfillOpenPosition(bytes32 key, uint256 _receivedTokens) public onlyHandler returns (bool) {
        ...
        PositionInfo memory _userInfo = PositionInfo({
            ...
            positionId: uint32(userInfo[user].length),
            closed: false
        });
        ...
        userInfo[user].push(_userInfo);
        // mint gmx shares to user
        _mint(user, dr.receivedMarketTokens);

        dr.success = true;
        address payable recipient = payable(dr.user);
        recipient.call{ value: address(this).balance }("");

        emit Deposited(user, _userInfo.deposit, block.timestamp, dr.receivedMarketTokens);
  ...
  */
    function requestClosePosition(
        ...
        //@note Assets IS GMTOKEN, not USDC
        uint256 withdrawAsssetAmount;
        if (_assetPercent == 0 || _assetPercent >= 900) {
            data.inFull = true;
            withdrawAsssetAmount = _userInfo.position;
            // inFull
            ...
            IExchangeRouter(gmxAddresses.exchangeRouter).sendWnt{ value: msg.value }(withdrawVault, msg.value);
            IExchangeRouter(gmxAddresses.exchangeRouter).sendTokens(gmxAddresses.gmxMarketToken, withdrawVault, _userInfo.position);
        } else {
            withdrawAsssetAmount = _userInfo.position.mulDiv(_assetPercent, DENOMINATOR);
            _burn(_userInfo.user, withdrawAsssetAmount);

            IERC20Upgradeable(gmxAddresses.gmxMarketToken).approve(gmxAddresses.gmxRouter, withdrawAsssetAmount);
            IExchangeRouter(gmxAddresses.exchangeRouter).sendWnt{ value: msg.value }(withdrawVault, msg.value);
            IExchangeRouter(gmxAddresses.exchangeRouter).sendTokens(gmxAddresses.gmxMarketToken, withdrawVault, withdrawAsssetAmount);
        }
        ...
        wr.gmTokenWithdrawnAmount = withdrawAsssetAmount;
    ...
    function fulfillClosePosition(bytes32 _key, uint256 _returnedUSDC) public onlyHandler returns (bool) {
        ...
        //@todo handle lost cases
        uint256 profit;
        if (_returnedUSDC > ((wr.gmTokenWithdrawnAmount * _userInfo.price) / 1e30)) {
            profit = _returnedUSDC - ((wr.gmTokenWithdrawnAmount * _userInfo.price) / 1e30);
        } else {
            profit = 0;
        }

        (uint256 currentDTV, , uint256 fulldebtValue) = getUpdatedDebt(wr.positionID, wr.user);
        ...
        if (!withdrawInFull[_key]) {
            ...
            uint256 userShares = ((wr.returnedUSDC - profit) * 1e3) / _userInfo.leverageMultiplier;
            uint256 waterShares = (wr.returnedUSDC - profit) - userShares;

            (waterProfits, toLeverageUser) = _getProfitSplit(profit, _userInfo.leverageMultiplier);
            if (waterProfits > 0) {
    ...
    function _getProfitSplit(uint256 _profit, uint256 _leverage) internal view returns (uint256, uint256) {
        ...
        uint256 toVodkaV2User = _profit > toWater ? _profit - toWater : 0;
    ...
    function _convertToShares(uint256 _positionId, uint256 _assetPercent, address _user) internal view returns (uint256, uint256, bool) {
        PositionInfo memory _userInfo = userInfo[_user][_positionId];
        uint256 positionValueInUSDC = _convertGMXMarketToUSDC(_userInfo.position, _userInfo.price);
        ...
        uint256 amountToWithdraw = positionValueInUSDC.mulDiv(percent, DENOMINATOR);
        ...
        return (_convertUSDCToGMXMarket(amountToWithdraw + profit, _userInfo.price), profit, inFull);
    }

    function _getProfitState(uint256 _positionId, address _user) internal view returns (uint256 profit) {
        (uint256 currentValueInUSDC, uint256 previousValueInUSDC) = getCurrentPosition(_positionId, 0, _user);
        if (currentValueInUSDC > previousValueInUSDC) {
            profit = currentValueInUSDC - previousValueInUSDC;
        } else {
            profit = 0;
        }
    }
    ...
    function _convertUSDCToGMXMarket(uint256 _amount, uint256 _GMXMarketPrice) internal pure returns (uint256) {
        return _amount.mulDiv((10 ** 18), _GMXMarketPrice) / 1e12;
    }
