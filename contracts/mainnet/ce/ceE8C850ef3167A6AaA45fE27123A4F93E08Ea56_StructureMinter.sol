...
contract StructureMinter is Pausable, ManagerModifier, ReentrancyGuard {
  ...
  mapping(address => bool) public stakeAfterMinting;
  ...
  // External
  //=======================================
  function mint(StructureMintRequest calldata _request) external whenNotPaused nonReentrant {
    ...
    // Add structures to be able to be randomly picked
    if (stakeAfterMinting[_request.structureAddress]) {
  ...
  function _checkCollectibleAmounts(
    ...
    if (cost == 0) {
      revert InvalidCollectibleType(_realmId, _structureId, _collectibleId);
    }
  ...
  function updateBlueprints(
    ...
    stakeAfterMinting[_structureAddress] = _stakeAfterMinting;
