...
import "../interfaces/IUniswapV3StrategyLogic.sol";
import "../interfaces/IUniswapV3StrategyData.sol";
 ...
 */
abstract contract UniswapV3BaseStrategy is AccessControl, UniswapV3StrategyStorage, IBorrower {
    ...
    function balance() public view returns (int) {
        return IUniswapV3StrategyData(provider.uniswapV3StrategyData()).balanceInTermsOf(getDepositToken(), address(this));
    }
    ...
    function getStableDebtFraction() external view returns (uint ratio) {
        ...
        uint stableDebt = lendVault.getDebt(addresses.stableToken, address(this));
        uint volatileDebt = oracle.getValueInTermsOf(addresses.volatileToken, lendVault.getDebt(addresses.volatileToken, address(this)), addresses.stableToken);
        for (uint i = 0; i<interchangeableTokens[addresses.stableToken].length; i++) {
            uint tokenDebt = lendVault.getDebt(interchangeableTokens[addresses.stableToken][i], address(this));
            stableDebt+=oracle.getValueInTermsOf(interchangeableTokens[addresses.stableToken][i], tokenDebt, addresses.stableToken);
        }
        for (uint i = 0; i<interchangeableTokens[addresses.volatileToken].length; i++) {
            uint tokenDebt = lendVault.getDebt(interchangeableTokens[addresses.volatileToken][i], address(this));
            volatileDebt+=oracle.getValueInTermsOf(interchangeableTokens[addresses.volatileToken][i], tokenDebt, addresses.stableToken);
        }
        return (PRECISION * stableDebt) / Math.max(1, stableDebt + volatileDebt);
    }
    ...
    /**
     * @notice Use migrator contract to copy the old contract state
     */
    function migrate(address oldStrategyAddress) external restrictAccess(GOVERNOR | CONTROLLER) {
        provider.uniswapV3StrategyMigrator().functionDelegateCall(abi.encodeWithSignature("migrate(address)", oldStrategyAddress));
    }

    /**
     * @notice Use migrator contrac to reset pnl trackers
     */
    function resetPnlData() external restrictAccess(GOVERNOR | CONTROLLER) {
        provider.uniswapV3StrategyMigrator().functionDelegateCall(abi.encodeWithSignature("resetPnlData()"));
    }

    /**
     * @notice Add a token that can be borrowed and swapped instead of a token
     * @param tokenA Token that can be replaced
     * @param tokenB The token to use instead of tokenA if tokenA can't be borrowed
     */
    function addInterchangeableToken(address tokenA, address tokenB) external restrictAccess(GOVERNOR | CONTROLLER) {
        if (!interchangeableTokens[tokenA].exists(tokenB)) {
            interchangeableTokens[tokenA].push(tokenB);
            IERC20(tokenB).safeIncreaseAllowance(provider.lendVault(), 2 ** 256 - 1 - IERC20(tokenB).allowance(address(this), provider.lendVault()));
            IERC20(tokenB).safeIncreaseAllowance(provider.swapper(), 2 ** 256 - 1 - IERC20(tokenB).allowance(address(this), provider.swapper()));
        }
    }

    /**
     * @notice Remove interchangeable token pair from storage
     */
    function removeInterchangeableToken(address tokenA, address tokenB) external restrictAccess(GOVERNOR | CONTROLLER) {
        uint index = interchangeableTokens[tokenA].findFirst(tokenB);
        if (index<interchangeableTokens[tokenA].length) {
            interchangeableTokens[tokenA][index] = interchangeableTokens[tokenA][interchangeableTokens[tokenA].length - 1];
            interchangeableTokens[tokenA].pop();
        }
    }
    ...
    function setLeverageAndTicks(
        ...
        int24 _multiplier0,
        int24 _multiplier1
    ) external virtual restrictAccess(KEEPER | GOVERNOR) requireAmmCheck trackPriceChangeImpact {
     ...
     */
    function siezeFunds() external requireAmmCheck restrictAccess(LENDVAULT) returns (address[] memory tokens, uint[] memory amounts) {
     ...
     */
    function delever(address token, uint amount) external requireAmmCheck restrictAccess(LENDVAULT | KEEPER | GOVERNOR) trackPriceChangeImpact {
        bytes memory returnData = provider.uniswapV3StrategyLogic().functionDelegateCall(abi.encodeWithSignature(
            "delever(address,uint256)",
            token, amount
        ));
        uint newLeverage = abi.decode(returnData, (uint));
        if (newLeverage!=parameters.leverage) {
            parameters.leverage = newLeverage;
            _withdraw(PRECISION);
            _harvest();
            _deposit();
        }
     ...
     */
    function deposit() external requireAmmCheck restrictAccess(GOVERNOR | CONTROLLER) trackPriceChangeImpact {
        _deposit();
    }

    /**
     * @notice Permissioned function called from controller or vault to withdraw to vault
     * @dev A fraction of the existing depositToken are used rather than all to ensure that all users
     * face the same slippage since withdrawing using only depositToken will result in no slippage
     */
    function withdraw(uint256 fraction) external requireAmmCheck restrictAccess(GOVERNOR | CONTROLLER) trackPriceChangeImpact {
        provider.uniswapV3StrategyLogic().functionDelegateCall(abi.encodeWithSignature(
            "swapTokensTo(address)",
            getDepositToken()
        ));
        uint startBalance = IERC20(getDepositToken()).balanceOf(address(this));
        uint toWithdraw = startBalance * fraction / PRECISION;
        _withdraw(fraction);
        provider.uniswapV3StrategyLogic().functionDelegateCall(abi.encodeWithSignature(
            "swapTokensTo(address)",
            getDepositToken()
        ));
        toWithdraw+=IERC20(getDepositToken()).balanceOf(address(this)) - startBalance;
        provider.uniswapV3StrategyLogic().functionDelegateCall(abi.encodeWithSignature(
...
contract UniswapV3DeltaNeutralStrategy is UniswapV3BaseStrategy {
    ...
    function getAddresses() external virtual view returns (address want, address stableToken, address volatileToken, address positionsManager) {
        want = addresses.want;
