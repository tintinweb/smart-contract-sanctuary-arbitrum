...
/**
 * @title NODE Staking Contract
 * @author TLD-TEAM
 * @notice In this contract user can stake NODE and harvest TLD
 */
contract NODEPool is AccessControlEnumerable, ReentrancyGuard {
    ...
    event SetAddrs(address tldAddr);
     ...
     * @dev Set Addrs
     */
    function setAddrs(address tldAddr) external onlyRole(MANAGER_ROLE) {
        TLD = IERC20(tldAddr);

        emit SetAddrs(tldAddr);
    }

    /**
     * @dev Add Rewards
     */
    function addRewards(uint256 amount) external onlyRole(MANAGER_ROLE) {
    ...
    function harvest() external nonReentrant {
        ...
        TLD.safeTransfer(msg.sender, amount);
    ...
    function updatePool() public {
        if (receivedToken <= releasedToken) {
            return;
        }

        if (receivedToken > releasedToken && stakes > 0) {
            accTokenPerStake +=
                ((receivedToken - releasedToken) * 1e18) /
                stakes;
            releasedToken = receivedToken;
        }
    }
