...
import { IDepositAutomatorRelay } from './IDepositAutomatorRelay.sol';
import { DepositAutomatorRelayLib } from './DepositAutomatorRelayLib.sol';
import { VaultParent } from '../vault-parent/VaultParent.sol';
import { SafeERC20 } from '@solidstate/contracts/utils/SafeERC20.sol';

import { Constants } from '../lib/Constants.sol';
import { TrustlessPermit } from '../lib/TrustlessPermit.sol';
import { Relayable } from '../relayable/Relayable.sol';

/**
 * @title   DepositAutomator (DEPRECATED) - USE RELAY
 * @dev     Allows users to queue deposits to a vault and execute them when the Vault is inSync.
 * @notice  .
 */

contract DepositAutomator is SafeOwnable, Relayable, IDepositAutomatorRelay {
    using DepositAutomatorRelayLib for RelayRemoveQueuedDeposit;
    using DepositAutomatorRelayLib for RelayQueueDepositAndSync;
    using DepositAutomatorRelayLib for RelayDeposit;
    using SafeERC20 for IERC20;
    using TrustlessPermit for address;

    string public constant override name = 'VALIO_DEPOSIT_AUTOMATOR';
    ...
    /**
     * @notice  Sets the Registry in the RelableStorage
     * @dev     Relayable requires access to the Valio Regitry
     * @param   __registry  .
     */
    function setRegistry(address __registry) external override onlyOwner {
        _setRegistry(__registry);
    }

    /**
     * @notice  Allows the owner to set the keeper fee
     * @param   _keeperFee  the new keeper fee
     */
    ...
    /**
     * @notice  Allows an atomic withdraw to be relayed by a keeper
     * @dev     The depositor needs to sign both the RelayRemoveQueuedDeposit and the ERC2612Permit
     * @param   permit  ERC2612Permit for the relay fee and the deposit amount
     * @param   relayDepositParams  RelayRemoveQueuedDeposit struct
     * @param   relayDepositSignature  EIP712 Signature of the RelayRemoveQueuedDeposit struct
     */
    function relayDeposit(
        ERC2612Permit memory permit,
        RelayDeposit memory relayDepositParams,
        bytes memory relayDepositSignature
    ) external {
        uint startingGas = gasleft();
        require(block.timestamp <= relayDepositParams.deadline, 'DAR: expired');

        // Verify that the signer of the QueueDepositAndSync struct
        // is the same signer as the ERC2612Permit
        _verifySigner(
            permit.owner,
            relayDepositParams.hash(),
            relayDepositSignature,
            relayDepositParams.signerNonce
        );
        ...
        IERC20(permit.token).safeTransferFrom(
            permit.owner,
            address(this),
            relayDepositParams.depositAmount
        );

        IERC20(permit.token).safeApprove(
            address(relayDepositParams.vault),
            relayDepositParams.depositAmount
        );

        // Stops the user from depositing into someone elses holding
        // DDOS causing extended lockup.
        require(
            relayDepositParams.tokenId == 0 ||
                permit.owner ==
                relayDepositParams.vault.ownerOf(relayDepositParams.tokenId)
        );

        relayDepositParams.vault.depositFor(
            permit.owner,
            relayDepositParams.tokenId,
            permit.token,
            relayDepositParams.depositAmount,
            relayDepositParams.maxUnitPrice,
            relayDepositParams.referrer
        );

        uint gasUsed = startingGas - gasleft();

        _calculateAndTakeFees(
            permit.owner,
            permit.token,
            gasUsed,
            0, // No additional msg.value,
            relayDepositParams.permitAmountForAllFees,
            msg.sender
        );
    }

    /**
     * @notice  Allows a deposit to be queued and a sync relayed by a keeper
     * @dev     The depositor needs to sign both the RelayQueueDepositAndSync and the ERC2612Permit
     * @dev     The sync is immediately executed and the deposit is queued to be executed later by a keeper
     * @param   permit  ERC2612Permit for the relay fee and the deposit amount
     * @param   queueDepositParams  RelayQueueDepositAndSync struct
     * @param   queueDepositParamsSignature  EIP712 Signature of the RelayQueueDepositAndSync struct
     */
    function relayQueueDepositAndSync(
        ERC2612Permit memory permit,
        RelayQueueDepositAndSync memory queueDepositParams,
        bytes memory queueDepositParamsSignature
    ) external payable {
        uint startingGas = gasleft();
        ...
        _verifySigner(
            permit.owner,
            queueDepositParams.hash(),
            queueDepositParamsSignature,
        ...
        uint totalLZSyncFees;

        for (uint i = 0; i < queueDepositParams.lzSyncFees.length; i++) {
            totalLZSyncFees += queueDepositParams.lzSyncFees[i];
        }

        uint additionalFees = totalLZSyncFees + queueDepositParams.keeperFee;

        require(msg.value >= additionalFees, 'insufficient eth for lz');

        _queueDepositAndSync(
            // The depositor is always the Permitter
            permit.owner,
            queueDepositParams.vault,
            queueDepositParams.tokenId,
            IERC20(permit.token),
            queueDepositParams.depositAmount,
            queueDepositParams.maxUnitPrice,
            queueDepositParams.expiry,
            queueDepositParams.lzSyncFees,
            totalLZSyncFees
        );
        ...
        _calculateAndTakeFees(
            ...
            gasUsed,
            additionalFees,
            queueDepositParams.permitAmountForAllFees,
            msg.sender
        );
    }

    /**
     * @notice  Allows the removal of a queued deposit to be relayed by a keeper
     * @dev     For use in the case where the queued deposit has expired
     * @dev     depositor receives lzFee and keeper refund
     * @param   permit  ERC2612Permit for the relay fee and the deposit amount
     * @param   relayRemoveQueuedDepositParams  RelayRemoveQueuedDeposit struct
     * @param   relayRemoveQueuedDepositSignature  EIP712 Signature of the RelayRemoveQueuedDeposit struct
     */
    function relayRemoveQueuedDeposit(
        ERC2612Permit memory permit,
        RelayRemoveQueuedDeposit memory relayRemoveQueuedDepositParams,
        bytes memory relayRemoveQueuedDepositSignature
    ) external {
        uint startingGas = gasleft();
        require(
            block.timestamp <= relayRemoveQueuedDepositParams.deadline,
            'DAR: expired'
        );

        // Verify that the signer of the RelayRemoveQueuedDeposit struct
        // is the same signer as the ERC2612Permit
        _verifySigner(
            permit.owner,
            relayRemoveQueuedDepositParams.hash(),
            relayRemoveQueuedDepositSignature,
            relayRemoveQueuedDepositParams.signerNonce
        );
        ...
        _removeQueuedDepositAndRefund(
            relayRemoveQueuedDepositParams.vault,
            relayRemoveQueuedDepositParams.index,
            permit.owner
        );
        ...
        _calculateAndTakeFees(
            ...
            gasUsed,
            0,
            relayRemoveQueuedDepositParams.permitAmountForAllFees,
            msg.sender
        );
    }

    /**
     * @notice  Allows the caller to request a sync and queue a deposit to be later executed by a keeper
     * @dev     The sync is immediately executed and the deposit is queued to be executed later by a keeper
     * @dev     The caller must provide enough ETH to cover the lz sync fees, and keeper fee
     * @param   vault  Valio Vault
     * @param   tokenId  holding tokenId
     * @param   depositAsset  the asset to deposit
     * @param   depositAmount  the amount of above to deposit
     * @param   maxUnitPrice  the maximum unit price the depositor is willing to pay
     * @param   expiry  the expiry of the queued deposit
     * @param   lzSyncFees  the fees to pay for the sync
     */
    ...
    /**
     * @notice  Allows the caller to queue a deposit to be later executed by a keeper
     * @dev     The caller must provide enough ETH to cover the keeper fee
     * @param   vault  Valio Vault
     * @param   tokenId  holding tokenId
     * @param   depositAsset  the asset to deposit
     * @param   depositAmount  the amount of above to deposit
    ...
    /**
     * @notice  Allows the keeper to execute a queued deposit
     * @param   vault  Valio Vault
     * @param   index  the index of the queued deposit
     */
    function executeDeposit(address vault, uint index) external {
        ...
        _removeQueuedDeposit(vault, index, qDeposit.depositor);

        qDeposit.depositAsset.safeTransferFrom(
            qDeposit.depositor,
            address(this),
            qDeposit.depositAmount
        );

        qDeposit.depositAsset.safeApprove(
    ...
    /**
     * @notice  Removes the queued deposit
     * @dev     Can only be called by the tokenOwner if not expired
     * @dev     Can be called by anyone if the queued deposit has expired
     * @dev     The depositor always receives the keeper fee refund
     * @param   vault  Valio vault
     * @param   index  the index of the queued withdraw to be removed
     */
    function removeQueuedDeposit(address vault, uint index) external {
        _removeQueuedDepositAndRefund(vault, index, msg.sender);
    }

    /**
     * @notice  Returns the currently configured keeperFee
     * @return  uint  the keeper fee to execute a queued deposit
     */
    ...
    /**
     * @notice  Gets the lz fees required to sync a vault, no buffer
     * @dev     This is just a helper method for the frontend
     * @param   vault  Valio Vault
     * @return  lzFees  lzFees for syncing a vault
     * @return  totalSendFee  sum of the above
     */
    ...
    /**
     * @notice  Checks if a queued deposit can be executed
     * @dev     This should be called by the keeper before executing a queued deposit
     * @param   vault  Valio Vault
     * @param   index  the index of the queued deposit to be executed
     * @return  bool  can execute
     * @return  string  if above == false, the reason
     */
    function canExecute(
        address vault,
        uint index
    ...
    /**
     * @notice  Returns the executed deposits for depositor for a vault
     * @param   vault  Valio Vault
     * @param   depositor  the depositor
     * @return  DepositAutomatorStorage.QueuedDeposit[] the queued deposits
     */
    ...
    /**
     * @notice  Returns the queued deposit (index) for depositor for a vault
     * @dev     Currently only one queued deposit per depositor per vault is supported
     * @dev     The index represents where the queued deposit struct is stored in the queuedDepositsByVault mapping
     * @param   vault  Valio Vault
     * @param   depositor  the depositor
     * @return  uint[] queued deposit indexes
     */
    ...
    /**
     * @notice  Returns the queued deposit for a vault at the index
     * @param   vault  Valio Vault
     * @param   index  the index of the queued deposit
     * @return  DepositAutomatorStorage.QueuedDeposit the queued deposit
     */
    ...
    /**
     * @notice  Returns the number of unexecuted queued deposits for a vault
     * @param   vault  Valio Vault
     * @return  uint  the number of queued deposits
     */
    ...
    /**
     * @notice  Returns all the queued deposits for a vault
     * @param   vault  Valio Vault
     * @return  DepositAutomatorStorage.QueuedDeposit[] the queued deposits
     */
    ...
    /**
     * @notice  Queues a deposit and syncs the vault
     * @dev     The sync is immediately executed and the deposit is queued to be executed later by a keeper
     * @param   depositor  the depositor
     * @param   vault  Valio Vault
     * @param   tokenId  holding tokenId
     ...
     * @param   expiry  the expiry of the queued deposit
     * @param   lzSyncFees  the fees to pay for the sync
     * @param   totalSyncFees  the total fees to pay for the sync
     */
    function _queueDepositAndSync(
        address depositor,
        VaultParent vault,
        uint tokenId,
        IERC20 depositAsset,
    ...
    /**
     * @notice  Queues a deposit to be executed later by a keeper
     * @dev     The deposit is not executed immediately, it is queued to be executed later by a keeper
     * @param   vault  Valio Vault
     * @param   depositor  the depositor
     * @param   tokenId  holding tokenId
     * @param   depositAsset  the asset to deposit
     ...
     * @param   maxUnitPrice  the maximum unit price the depositor is willing to pay
     * @param   expiry  the expiry of the queued deposit
     * @param   feesPaid  the fees paid for the deposit
     */
    function _queueDeposit(
        ...
        if (tokenId == 0) {
            uint numHoldings = vault.balanceOf(depositor);
            if (depositor == vault.manager()) {
            ...
            // Could ddos the holding by locking it up for extended periods
    ...
    /**
     * @notice  Removes already executed/removed queued deposit from depositors index storage
     * @dev     A bug where they didn't get removed from this storage existed. This was a patch
     * @param   indexes the depositors indexes to clean/check
     * @param   queuedDeposits  all queued deposits for a vault
     * @param   depositor  the depositor
     */
    ...
    /**
     * @notice  Removes a queued deposit and refunds the keeper fee
     * @dev     Can only be called by the tokenOwner if not expired
     * @dev     Can be called by anyone if the queued deposit has expired
     * @dev     The depositor always receives the keeper fee refund
     * @param   vault  Valio vault
     ...
     */
    function _removeQueuedDepositAndRefund(
        address vault,
        uint index,
        address caller
    ) internal {
        ...
        if (qDeposit.expiryTime > block.timestamp) {
            require(qDeposit.depositor == caller, 'not owner');
        }

        _removeQueuedDeposit(vault, index, caller);

        // Refund the tokenOwner the keeper fee, if its relayed the feeReceiver is the relayer
    ...
    /**
     * @notice  Removes a queued deposit from all storage
     * @param   vault  Valio vault
     * @param   index  the index of the queued withdraw to be removed
     * @param   depositor  the depositor
     */
    ...
    /**
     * @notice  Updates the index of a depositor in the vault -> depositor mapping
     * @dev     This is needed because the index of the depositor changes when a queued deposit is removed
     * @param   queuedDeposits  all queued deposits for a vault
     * @param   vault  Valio vault
     ...
     * @param   vault  Valio Vault
     * @param   index  the index of the queued deposit
     * @return  DepositAutomatorStorage.QueuedDeposit  .
     */
...
library DepositAutomatorRelayLib {
    bytes32 internal constant RELAY_QUEUE_DEPOSIT_AND_SYNC_EIP712_TYPE_HASH =
        keccak256(
            'RelayQueueDepositAndSync('
            'address vault,'
            'uint256 tokenId,'
            ...
            'uint256[] lzSyncFees,'
            'uint256 keeperFee,'
            'uint256 permitAmountForAllFees,'
            'uint256 signerNonce)'
        );

    bytes32 internal constant RELAY_DEPOSIT_AND_SYNC_EIP712_TYPE_HASH =
        keccak256(
            'RelayDeposit('
            'address vault,'
            'uint256 tokenId,'
    ...
    bytes32 internal constant RELAY_REMOVE_QUEUED_DEPOSIT_EIP712_TYPE_HASH =
        keccak256(
            'RelayRemoveQueuedDeposit('
            'address vault,'
            'uint256 index,'
            'uint256 deadline,'
            'uint256 permitAmountForAllFees,'
    ...
    function hash(
        IDepositAutomatorRelay.RelayQueueDepositAndSync
            memory queueDepositParams
    ) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    RELAY_QUEUE_DEPOSIT_AND_SYNC_EIP712_TYPE_HASH,
                    queueDepositParams.vault,
                    queueDepositParams.tokenId,
                    queueDepositParams.depositAmount,
                    queueDepositParams.maxUnitPrice,
                    ...
                    queueDepositParams.expiry,
                    keccak256(abi.encodePacked(queueDepositParams.lzSyncFees)),
                    queueDepositParams.keeperFee,
                    queueDepositParams.permitAmountForAllFees,
                    queueDepositParams.signerNonce
                )
            );
    ...
    function hash(
        IDepositAutomatorRelay.RelayRemoveQueuedDeposit
            memory relayRemoveQueuedDepositParams
    ) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    RELAY_REMOVE_QUEUED_DEPOSIT_EIP712_TYPE_HASH,
                    relayRemoveQueuedDepositParams.vault,
                    relayRemoveQueuedDepositParams.index,
                    relayRemoveQueuedDepositParams.deadline,
                    relayRemoveQueuedDepositParams.permitAmountForAllFees,
                    relayRemoveQueuedDepositParams.signerNonce
                )
            );
    ...
    function hash(
        IDepositAutomatorRelay.RelayDeposit memory relayDepositParams
    ) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    RELAY_DEPOSIT_AND_SYNC_EIP712_TYPE_HASH,
                    relayDepositParams.vault,
                    relayDepositParams.tokenId,
                    relayDepositParams.depositAmount,
                    relayDepositParams.maxUnitPrice,
                    relayDepositParams.referrer,
                    relayDepositParams.deadline,
                    relayDepositParams.permitAmountForAllFees,
                    relayDepositParams.signerNonce
                )
            );
...
interface IDepositAutomatorRelay {
    struct RelayQueueDepositAndSync {
        VaultParent vault;
        uint256 tokenId;
        uint256 depositAmount;
        uint256 maxUnitPrice;
        ...
        // Paid by relayer
        uint256 keeperFee;
        // No more than this amount can be taken by the relayer to cover fees
        uint256 permitAmountForAllFees;
        uint256 signerNonce;
    }
    ...
    struct RelayRemoveQueuedDeposit {
        address vault;
        uint256 index;
        uint256 deadline;
        // No more than this amount can be taken by the relayer to cover fees
...
library RelayLib {
    using SafeERC20 for IERC20;
    uint public constant CALCULATE_AND_TAKE_FEES_GAS = 55_000;

    // This is a safety net to ensure the relayer receives enough tokens to cover the gas cost
    // 5% protects against token price oracle lag
    uint public constant FEE_TOKEN_BUFFER_BIPS = 500;

    error Relay__NotEnoughGasTokenApproved();
    error Relay__FailedToTakeFee();

    /**
     * @notice  Calculates the cost of the relayed tx and takes the fee from the payer
     * @dev     To be called by the consumer to recoupe the relay cost
     * @param   payer  the address to take the fee from
     * @param   feeToken  the token used to pay the relay cost
     * @param   gasUsed  the amount of gas the tx used so far
     * @param   additionalFees  msgValue or additional fees
     * @param   feeReceiver  the address to send the fee to (the relayer)
     */
    function _calculateAndTakeFees(
        ...
        uint gasUsed,
        uint additionalFees,
        uint maxTokenAmountForFees,
        address feeReceiver
    ) internal returns (uint tokenCost) {
        tokenCost = _calculateTxCost(
            registry,
            tx.gasprice,
            feeToken,
            gasUsed,
        ...
        if (tokenCost > maxTokenAmountForFees)
            revert Relay__NotEnoughGasTokenApproved();

        IERC20(feeToken).safeTransferFrom(payer, feeReceiver, tokenCost);
    }

    /**
     * @notice  Calculates the cost of the relayed tx
     * @param   gasPrice  the price of gas when submitting the tx
     * @param   feeToken  the token used to pay the relay cost
     * @param   gasUsed  the amount of gas the tx will use
     * @param   additionalFees  msgValue or additional fees
     * @return  uint  the amount of the feetoken required to cover the cost
     */
    function _calculateTxCost(
        Registry registry,
        uint gasPrice,
        address feeToken,
        uint gasUsed,
        ...
        gasUsed = gasUsed + CALCULATE_AND_TAKE_FEES_GAS;
        uint gasCostEth = (gasUsed) * gasPrice;
        uint txCost = gasCostEth + additionalFees;

        require(address(registry.accountant()) != address(0), 'no accountant');
        require(
            address(registry.wrappedNativeToken()) != address(0),
            'no weth'
        );

        (, uint costUSD8) = registry.accountant().assetValue(
            address(registry.wrappedNativeToken()),
            txCost
        );

        uint feeTokenUnit = 10 ** IERC20Metadata(feeToken).decimals();
        (uint feeTokenValue8, ) = registry.accountant().assetValue(
            feeToken,
            feeTokenUnit
        );

        uint cost = _calculateTokenAmount(
            costUSD8,
            feeTokenValue8,
            feeTokenUnit
        );

        uint gasForValuing = startGas - gasleft();
        uint ethForValueing = gasForValuing * gasPrice;

        uint valuingCost = ((cost * ethForValueing) / txCost);

        return _adjustWithBuffer(cost + valuingCost, FEE_TOKEN_BUFFER_BIPS);
    }

    function _adjustWithBuffer(
        uint amount,
        uint bufferBIPs
    ) internal pure returns (uint) {
        return
            (amount * (Constants.BASIS_POINTS_DIVISOR + bufferBIPs)) /
            Constants.BASIS_POINTS_DIVISOR;
    }

    /**
     * @notice  Calculates the amount of the token required to cover the eth cost
     * @param   wethCost the cost of the transaction in $
     * @param   tokenPrice  token price in $
     * @param   tokenUnit  1 unit of the token
     * @return  uint  the amount of token to cover the wethCost
     */
    function _calculateTokenAmount(
        ...
        return (wethCost * tokenUnit) / tokenPrice;
    }
}
...
import { Registry } from '../registry/Registry.sol';
import { RelayableStorage } from './RelayableStorage.sol';
import { RelayLib } from '../relay/RelayLib.sol';
import { VaultParent } from '../vault-parent/VaultParent.sol';
import { VaultParentInvestor } from '../vault-parent/VaultParentInvestor.sol';
import { Constants } from '../lib/Constants.sol';

/**
 * @title   Relayable (DEPRECATED)
 * @notice  Extensible contract which allows the consumer to relay permitted operations in erc20 tokens.
 * @dev     ERC20 Tokens must be supported by the Valio Procotol (Registered)
 */
...
abstract contract Relayable {
    ...
    error Relay__InvalidSignature();
    error Relay__InvalidNonceUsed();

    /**
     * @notice  .Sets the Valio Registry in storage
     ...
     * @return  registry  Valio Registry
     */
    ...
    /**
     * @notice  .Calculates the amount of the feetoken required for relaying a tx
     * @dev     .
     * @param   gasPrice  the price of gas when submitting the tx
     * @param   feeToken  the token used to pay the relay cost
     * @param   gasUsed  the amount of gas the tx will use
    ...
    function _calculateAndTakeFees(
        ...
        RelayLib._calculateAndTakeFees(
            _registry(),
            payer,
            feeToken,
            gasUsed,
            additionalFees,
            maxTokenAmountForFees,
            feeReceiver
        );
    }

    /**
     * @notice  Recovers the signer from the signature and assets it matches the expected signer
     * @dev     Checks to make sure the payloads nonce has not be used
     * @param   expectedSigner  the expected signer of the payload
    ...
    function _verifySigner(
        ...
        if (
            RelayableStorage.layout().signerNonces[expectedSigner][payloadNonce]
        ) {
            revert Relay__InvalidNonceUsed();
        }
        ...
        // validate signer
        if (signer != expectedSigner) revert Relay__InvalidSignature();

        RelayableStorage.layout().signerNonces[signer][payloadNonce] = true;
    }

    /**
     * @notice  Sets the Valio Registry in storage
     * @param   __registry  Valio Registry
     */
    function _setRegistry(address __registry) internal {
        RelayableStorage.layout().registry = Registry(__registry);
    }
    ...
    function _DOMAIN_SEPARATOR() internal returns (bytes32) {
        RelayableStorage.Layout storage l = RelayableStorage.layout();
    ...
    /**
     * @dev     EIP712 Requires name for the domain
     * @return  string  name
     */
    function _name() internal view virtual returns (string memory);
    ...
    function _registry() internal view returns (Registry) {
        return RelayableStorage.layout().registry;
    }
}
...
library RelayableStorage {
    bytes32 internal constant STORAGE_SLOT =
        keccak256('valio.storage.Relayable');
