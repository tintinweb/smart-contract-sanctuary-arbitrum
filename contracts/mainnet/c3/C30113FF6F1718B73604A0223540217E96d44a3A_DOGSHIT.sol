...
contract DOGSHIT is ERC20Burnable, Ownable {
  using SafeMath for uint256;
  ...
  uint256 public TREASURY_FEE = 400;
  ...
  ) public ERC20("DogeShit", "DOGSHIT") {
    require(_initialSupply <= _maxSupply, "DOGSHIT: The _initialSupply should not exceed the _maxSupply");
  ...
  modifier onlyDev() {
    require(devAddress == _msgSender() || owner() == _msgSender(), "DOGSHIT: You don't have the permission!");
    _;
  }
  ...
  function updateUniswapV2Router(address newAddress) public onlyDev {
    require(newAddress != address(uniswapV2Router), "DOGSHIT: The router already has that address");
  ...
  function recoverToken(address _token) external onlyDev {
    ...
    require(tokenBalance > 0, "DOGSHIT: The contract doen't have tokens to be recovered!");
