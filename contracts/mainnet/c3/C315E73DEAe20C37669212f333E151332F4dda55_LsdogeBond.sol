...
contract LsdogeBond is Ownable {
    /* ======== DEPENDENCIES ======== */
    ...
    struct Note {
        uint256 payout; // LSDoge remaining to be paid
    ...
    constructor(address _lsd, address _treasury, uint _supply) {
        lsdoge = _lsd;
        treasury = _treasury;
        supply = _supply;
    }
     ...
     * @return payout_     the amount of LSDoge due
    ...
    function deposit(
         ...
         * price = quote tokens : LSDoge (i.e. 42069 DAI : LSDoge)
         *
         * 1e27 = LSDoge decimals (9) + price decimals (18)
         */
         ...
         *
         * this is either the number of LSDoge that the market can sell
        ...
        // purchased, and how much LSDoge has been sold
    ...
    function _decay(uint256 _id, uint48 _time) internal {
         ...
         * |------------------------------------| t
         */
        markets[_id].totalDebt = currentDebt(_id);
    ...
    function _tune(uint256 _id, uint48 _time) internal {
        ...
        if (meta.tuneEnabled && _time >= meta.lastTune + meta.tuneInterval) {
            ...
            // LSDoge decimals (9) + price decimals (18)
            uint256 capacity = market.capacityInQuote ? ((market.capacity * 1e27) / price) / (10 ** meta.quoteDecimals) : market.capacity;
             ...
             * is 10,000 LSDoge. max payout would be 1,000 LSDoge (10,000 * 1 / 10).
             */

            // derive a new control variable from the target debt and current supply

            //DEV : replace with LSDoge supply
            //uint64 newControlVariable = uint64((price * treasury.baseSupply()) / targetDebt);
            uint newControlVariable = (price * supply) / capacity;
    ...
    function _updateMinDebt(uint256 _id) internal {
        markets[_id].minDebt = (minMarketPrice[_id] * supply) / terms[_id].controlVariable;
    }
     ...
     * @param _id          ID of market
     * @param _minPrice    minimum price of the bond market, in 9 decimals (1e9 = 1 quote token per LSDoge)
     */
    ...
    function setCapacity(uint256 _id, uint256 _capacity, uint256 _maxDebt) external onlyOwner {
        markets[_id].capacity = _capacity;
        terms[_id].maxDebt = _maxDebt;
    }

    function setDebtDecayTerm(uint256 _id, uint48 _debtDecayTerm) external onlyOwner {
        terms[_id].debtDecayTerm = _debtDecayTerm;
    }
     ...
     * @param _market      [capacity (in LSDoge or quote), initial price / LSDoge (9 decimals), debt buffer (3 decimals), min price / LSDoge]
    ...
    function create(
         ...
         *
         * 1e27 = LSDoge decimals (9) + initial price decimals (18)
         */
        uint256 targetDebt = _booleans[0] ? ((_market[0] * 1e27) / _market[1]) / 10 ** decimals : _market[0];

        /*
         * max payout is the amount of capacity that should be utilized in a deposit
         * interval. for example, if capacity is 1,000 LSDoge, there are 10 days to conclusion,
         * and the preferred deposit interval is 1 day, max payout would be 100 LSDoge.
         */
         ...
         */
        uint256 controlVariable = (_market[1] * supply) / targetDebt;
        ...
        markets.push(
            Market({
                ...
                maxPayout: maxPayout,
                minDebt: (targetDebt * _market[3]) / _market[1],
                purchased: 0,
                sold: 0
            })
        );
        ...
        minMarketPrice[id_] = _market[3];

        emit CreateMarket(id_, address(lsdoge), address(_quoteToken), _market[1]);
    }
     ...
     * @param _id          ID of market
     * @return             price for market in LSDoge decimals
     *
     ...
     * @param _id          ID of market
     * @return             amount of LSDoge to be paid in LSDoge decimals
     *
     * @dev 1e27 = LSDoge decimals (9) + market price decimals (18)
     */
    function payoutFor(uint256 _amount, uint256 _id) external view returns (uint256) {
        Metadata memory meta = metadata[_id];
        return (_amount * 1e27) / marketPrice(_id) / 10 ** meta.quoteDecimals;
    }
    ...
    function debtRatio(uint256 _id) public view returns (uint256) {
        return (currentDebt(_id) * (10 ** metadata[_id].quoteDecimals)) / supply;
    }
     ...
     * @param _id          ID of market
     * @return debt        current debt for market in LSDoge decimals
     */
    function currentDebt(uint256 _id) public view returns (uint256 debt) {
        debt = markets[_id].totalDebt - debtDecay(_id);
        if (debt < markets[_id].minDebt) {
            debt = markets[_id].minDebt;
        }
    }
     ...
     */
    function debtDecay(uint256 _id) public view returns (uint256) {
        Metadata memory meta = metadata[_id];

        uint256 secondsSince = block.timestamp - meta.lastDecay;

        return (markets[_id].totalDebt * secondsSince) / terms[_id].debtDecayTerm;
    }
     ...
     * @param _id          ID of market
     * @return             control variable for market in LSDoge decimals
     */
    ...
    function _debtRatio(uint256 _id) internal view returns (uint256) {
        return (markets[_id].totalDebt * (10 ** metadata[_id].quoteDecimals)) / supply;
    }
     ...
     * @param _payout      the amount of LSDoge due to the user
    ...
    function redeem(address _user, uint256[] memory _indexes) public returns (uint256 payout_) {
        ...
        IERC20(lsdoge).transfer(_user, payout_);
    }
