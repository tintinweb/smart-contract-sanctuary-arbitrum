...
contract CreditUser is Initializable, ReentrancyGuardUpgradeable, ICreditUser {
    ...
    /// @param _recipient borrower
    /// @param _borrowedIndex position index
    function destroy(address _recipient, uint256 _borrowedIndex, address _liquidator) external override onlyCaller {
    ...
    /// @return bool value
    function isTimeout(address _recipient, uint256 _borrowedIndex, uint256 _duration) external view override returns (bool) {
    ...
    function getUserLendCredit(
        ...
        view
        override
        returns (address depositor, address token, uint256 amountIn, uint256 reservedLiquidatorFee, address[] memory borrowedTokens, uint256[] memory ratios)
    {
        UserLendCredit storage userLendCredit = userLendCredits[_recipient][_borrowedIndex];
    ...
    /// @notice end time of leverage
    /// @param _recipient borrower
    /// @param _borrowedIndex position index
    /// @param _duration maximum repay time
    /// @return bool value
    function getTimeoutTimestamp(address _recipient, uint256 _borrowedIndex, uint256 _duration) external view override returns (uint256) {
        UserBorrowed storage userBorrowed = userBorroweds[_recipient][_borrowedIndex];
        return userBorrowed.borrowedAt + _duration;
    }
}
...
interface ICreditUser {
    ...
    function destroy(address _recipient, uint256 _borrowedIndex, address _liquidator) external;
    ...
    function isTimeout(address _recipient, uint256 _borrowedIndex, uint256 _duration) external view returns (bool);
    ...
    function getUserLendCredit(
        ...
        external
        view
        returns (address depositor, address token, uint256 amountIn, uint256 _reservedLiquidatorFee, address[] memory borrowedTokens, uint256[] memory ratio);
    ...
    function getTimeoutTimestamp(address _recipient, uint256 _borrowedIndex, uint256 _duration) external view returns (uint256);
