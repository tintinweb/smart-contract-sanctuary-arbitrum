...
contract Viewer is IViewer, UpgradeableGovernable {
    ...
    /**
     * @notice Get user compound positions amounts. 18 decimals
     * @param _factory Factory address.
     * @param _pool pool address.
     * @param _user User address.
     */
    function userCompoundPositionUSD(address _factory, address _pool, address _user)
        external
        view
        override
        returns (uint256)
    {
        ...
        (uint256 amount0, uint256 amount1) = userCompoundPosition(_factory, _pool, _user);
        (uint256 price0, uint256 price1) = IPriceHelper(lpContracts.priceHelper).getPrices(_pool);

        amount0 =
            IPriceHelper(lpContracts.priceHelper).normalise(address(IManager(lpContracts.lpManager).token0()), amount0);

        amount1 =
            IPriceHelper(lpContracts.priceHelper).normalise(address(IManager(lpContracts.lpManager).token1()), amount1);

        return price0.mul(amount0).add(price1.mul(amount1)).div(1e18);
    }
     ...
     * @param _user User address.
     */
    function userCompoundPosition(address _factory, address _pool, address _user)
        public
        view
        override
        returns (uint256, uint256)
    {
        ...
        uint256 position = ICompounder(lpContracts.compounder).balanceOf(_user);
        ...
        if (position > 0) {
            ...
            if (vars.amount0 > 0 || vars.amount1 > 0) {
                ...
                // Add new assets
                vars.assets = vars.assets.add(vars.newAssets);
            }

            position = (vars.supply == 0) ? position : position.mulDivDown(vars.assets, vars.supply);
        } else {
            return (0, 0);
        }

        vars.supply = IManager(lpContracts.lpManager).totalSupply().add(vars.newAssets);

        vars.amount0 = vars.amount0.add(IManager(lpContracts.lpManager).token0().balanceOf(lpContracts.lpManager))
            .mulDivDown(position, vars.supply);
        vars.amount1 = vars.amount1.add(IManager(lpContracts.lpManager).token1().balanceOf(lpContracts.lpManager))
            .mulDivDown(position, vars.supply);
        ...
        for (uint256 i; i < vars.length;) {
            ...
            vars.liquidityToRemove = uint128(uint256(vars.liquidity).mulDivDown(position, vars.supply));
     ...
     * @param _user User address.
     */
    function userNonCompoundPositionUSD(address _factory, address _pool, address _user)
        external
        view
        override
        returns (uint256)
    {
        ...
        (uint256 amount0, uint256 amount1) = userNonCompoundPosition(_factory, _pool, _user);
        (uint256 price0, uint256 price1) = IPriceHelper(lpContracts.priceHelper).getPrices(_pool);
    ...
    /**
     * @notice Summ all user non compounded positions amounts.
     * @param _factory Factory address.
     * @param _pool pool address.
     * @param _user User address.
     */
    function userNonCompoundPosition(address _factory, address _pool, address _user)
        public
        view
        override
        returns (uint256, uint256)
    {
        ...
        uint256 position = IRewardTracker(lpContracts.doubleTracker).stakedAmount(_user);
        position = position.add(ISingleRewardTracker(lpContracts.singleTrackerZero).stakedAmount(_user));
        position = position.add(ISingleRewardTracker(lpContracts.singleTrackerOne).stakedAmount(_user));

        if (position == 0) return (0, 0);

        vars.supply = IManager(lpContracts.lpManager).totalSupply();
     ...
     * @param _user user address.
     */
    function userPositions(address _factory, address _pool, address _user)
        external
        view
        override
        returns (uint256 compoundPosition, uint256 doublePosition, uint256 zeroPosition, uint256 onePosition)
    {
        IFactory.LPContracts memory lpContracts = IFactory(_factory).getLPManagerContracts(_pool);
        require(lpContracts.lp == _pool, "LP Manager Not Found");

        compoundPosition = ICompounder(lpContracts.compounder).balanceOf(_user);
        doublePosition = IRewardTracker(lpContracts.doubleTracker).stakedAmount(_user);
        zeroPosition = ISingleRewardTracker(lpContracts.singleTrackerZero).stakedAmount(_user);
        onePosition = ISingleRewardTracker(lpContracts.singleTrackerOne).stakedAmount(_user);
    }
...
interface IViewer {
    ...
    function userCompoundPositionUSD(address _factory, address _pool, address _user) external view returns (uint256);
    function userCompoundPosition(address _factory, address _pool, address _user)
        external
        view
        returns (uint256, uint256);
