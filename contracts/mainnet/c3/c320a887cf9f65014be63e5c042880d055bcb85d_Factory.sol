...
contract Factory is IFactory, UpgradeableOperable {
    ...
    function _swapperPaths(
        ...
        if (_type == 1 || _type == 3) {
            // UNISWAP
            IUniswapV3Swapper.SwapPair[] memory routes = new IUniswapV3Swapper.SwapPair[](_routes);
            (asset0, asset1, routes) = abi.decode(_paths, (address, address, IUniswapV3Swapper.SwapPair[]));
            IUniswapV3Swapper(_swapper).upsertPathOverride(trackerZero, asset1, asset0, routes);
            IUniswapV3Swapper(_swapper).upsertPathOverride(trackerOne, asset0, asset1, routes);
        } else if (_type == 2) {
            // ALGEBRA
            IAlgebraSwapper.SwapPair[] memory routes = new IAlgebraSwapper.SwapPair[](_routes);
            (asset0, asset1, routes) = abi.decode(_paths, (address, address, IAlgebraSwapper.SwapPair[]));
            IAlgebraSwapper(_swapper).upsertPathOverride(trackerZero, asset1, asset0, routes);
            IAlgebraSwapper(_swapper).upsertPathOverride(trackerOne, asset0, asset1, routes);
        }
    }
}
