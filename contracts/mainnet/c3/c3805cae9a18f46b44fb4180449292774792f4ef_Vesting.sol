...
contract Vesting is AccessControl {
	...
	constructor(IWINR _Winr, IWINR _vWinr, address _admin, address _coAdmin, address _adminBackup) {
		WINR = _Winr;
		vWINR = _vWinr;

		categoryDetails[Category.INFLUENCERS] = CategoryDetail(0, 180);

		_grantRole(DEFAULT_ADMIN_ROLE, _admin);
		_grantRole(DEFAULT_ADMIN_ROLE, _coAdmin);
	...
	function _addInvestor(
		...
		investor.startTimestamp = uint128(block.timestamp);
	...
	function _calculateUnlockedTokens(
		...
		uint128 investorInitialTimestamp = investor.startTimestamp;
		if(investorInitialTimestamp < initialTimestamp) investorInitialTimestamp = uint128(initialTimestamp);

		CategoryDetail memory categoryDetail = categoryDetails[investor.category];
		uint256 cliffTimestamp = investorInitialTimestamp + categoryDetail.cliffDays * 1 days;
		uint256 vestingTimestamp = cliffTimestamp + categoryDetail.recurrence * 1 days;

		uint256 currentTimeStamp = block.timestamp;
		if (initialTimestamp == 0) return 0;
		if (currentTimeStamp > investorInitialTimestamp) {
