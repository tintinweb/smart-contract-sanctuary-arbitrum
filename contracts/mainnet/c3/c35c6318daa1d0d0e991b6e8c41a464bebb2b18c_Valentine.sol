...
contract Valentine is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 Valentine-WAVAX LP -> Valentine pool
    // uint256 public constant INITIAL_Valentine_POOL_DISTRIBUTION = 0 ether;
    ...
    address public valentineOracle;
    ...
    // Address of the Valentine-WAVAX LP
    ...
    /**
     * @notice Constructs the VALENTINE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress, uint256 _sellStartTime) public ERC20("Valentine Token", "VALENTINE") {
        // Mints 1 VALENTINE to contract creator for initial pool setup
    ...
    function _getValentinePrice() internal view returns (uint256 _valentinePrice) {
        try IOracle(valentineOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Valentine: failed to fetch VALENTINE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _valentinePrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_valentinePrice >= taxTiersTwaps[tierId]) {
    ...
    function setValentineOracle(address _valentineOracle) public onlyOperatorOrTaxOffice {
        require(_valentineOracle != address(0), "oracle address cannot be 0 address");
        valentineOracle = _valentineOracle;
    }
    ...
    /**
     * @notice Operator mints Valentine to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of Valentine to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentValentinePrice = _getValentinePrice();
            currentTaxRate = _updateTaxRate(currentValentinePrice);
            if (currentValentinePrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // address _valentinePool
        // address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        // require(_valentinePool != address(0), "!_valentinePool");
        // require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_valentinePool, INITIAL_VALENTINE_POOL_DISTRIBUTION);
    ...
    function getRemainingTimeForValentineSellingEnable() public view returns(uint256) {
