...
contract Vester is IVester, IERC20, ReentrancyGuard, Governable {
    ...
    address public override rewardTracker;  // contract of get unvested token info
    ...
    /// @notice deposit token to convert
    ...
    /// @notice claim all claimable converted token, withdraw all pair token and unconverted vestToken
    ...
    /// @notice calculate newly converted claimable token
    ...
    /// @notice calculate how much pair token needed to convert the vestToken
    function getPairAmount(address _account, uint256 _vestAmount) public view returns (uint256) {
        ...
        return _vestAmount.mul(combinedAverageStakedAmount).div(maxVestableAmount);
    }
    ...
    function _deposit(address _account, uint256 _amount) private {
        require(_amount > 0, "Vester: invalid amount");
    ...
    /// @notice update vesting global information
    ...
    /// @notice calculate the converted amount
    ...
    /// @notice claim the claimable converted token
    ...
    function getMaxVestAmount(address _account) public view returns (uint256) {
        ...
        _pairAmount = _pairAmount.add(pairAmounts[_account]);

        uint256 amount = 0;
        uint256 vestedAmount = getVestedAmount(_account);
        if (_pairAmount > combinedAverageStakedAmount){
            amount = maxVestableAmount; 
        } else {
            amount =  _pairAmount.mul(maxVestableAmount).div(
                combinedAverageStakedAmount
            ); 
        }
        return amount < vestedAmount ? 0 : amount.sub(vestedAmount);
    }
}