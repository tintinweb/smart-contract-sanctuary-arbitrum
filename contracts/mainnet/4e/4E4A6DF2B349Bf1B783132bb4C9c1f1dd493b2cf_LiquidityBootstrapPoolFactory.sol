...
import "../interfaces/ILiquidityBootstrapPool.sol";
import "../interfaces/ILiquidityBootstrapPoolDeployer.sol";
import "../interfaces/ILiquidityBootstrapPoolFactory.sol";
...
struct PoolSettings {
    ...
    uint88 maxTotalSharesOut;
    uint88 maxTotalAssetsIn;
    uint64 weightStart;
    uint64 weightEnd;
    uint40 saleStart;
    uint40 saleEnd;
...
contract LiquidityBootstrapPoolFactory is
    OwnableUpgradeable,
    TokensRescuer,
    ILiquidityBootstrapPoolFactory
{
    ...
    event PoolDeployerSet(address poolDeployer);
    ...
    modifier checkFee(uint48 fee) {
        if (fee > MAX_FEE_BIPS) revert("MaxFeeExceeded");
        _;
    }

    modifier checkSwapFeeAndReferrerFee(uint48 swapFee, uint48 referrerFee) {
        if (referrerFee > swapFee) {
            revert("referrerFee must be < swapFee");
        }
        _;
    }
    ...
    function __LiquidityBootstrapPoolFactory_init(
        address _owner,
        ...
        __LiquidityBootstrapPoolFactory_init_unchained(
            _feeRecipient,
            _platformFee,
            _referrerFee,
            _swapFee
    ...
    function createLiquidityBootstrapPool(
        ...
        if (
            args.share == args.asset ||
            args.share == address(0) ||
        ...
        if (
            args.weightStart < 0.01 ether ||
            args.weightStart > 0.99 ether ||
            args.weightEnd > 0.99 ether ||
        ...
        pool = ILiquidityBootstrapPoolDeployer(poolDeployer).createPool(
            _getSettings(args)
        );
        ...
        ILiquidityBootstrapPoolDeployer(poolDeployer).setAssetsStart(pool);
        isPool[pool] = true;
    ...
    function setPlatformFee(uint48 fee) external onlyOwner checkFee(fee) {
    ...
    function setReferrerFee(
        ...
        onlyOwner
        checkFee(fee)
        checkSwapFeeAndReferrerFee(factorySettings.swapFee, fee)
    {
    ...
    function setSwapFee(
        ...
        onlyOwner
        checkFee(fee)
        checkSwapFeeAndReferrerFee(fee, factorySettings.referrerFee)
    {
    ...
    function __LiquidityBootstrapPoolFactory_init_unchained(
        address _feeRecipient,
        uint48 _platformFee,
        uint48 _referrerFee,
        uint48 _swapFee
        ...
        internal
        initializer
        checkFee(_platformFee)
        checkFee(_referrerFee)
        checkFee(_swapFee)
        checkSwapFeeAndReferrerFee(_swapFee, _referrerFee)
    {
    ...
    function _getSettings(
        ...
        internal
        view
        returns (ILiquidityBootstrapPool.PoolDeploySettings memory)
    {
        FactorySettings memory settings = factorySettings;

        unchecked {
            return
                ILiquidityBootstrapPool.PoolDeploySettings({
                    platform: settings.feeRecipient,
                    platformFee: uint64(settings.platformFee) * 1e14,
                    referrerFee: uint64(settings.referrerFee) * 1e14,
                    swapFee: uint64(settings.swapFee) * 1e14,
                    asset: args.asset,
                    share: args.share,
                    manager: args.manager,
                    virtualAssets: args.virtualAssets,
                    virtualShares: args.virtualShares,
                    maxSharePrice: args.maxSharePrice,
                    maxTotalSharesOut: args.maxTotalSharesOut,
                    maxTotalAssetsIn: args.maxTotalAssetsIn,
                    weightStart: args.weightStart,
                    weightEnd: args.weightEnd,
                    saleStart: args.saleStart,
                    saleEnd: args.saleEnd,
                    sellingAllowed: args.sellingAllowed,
                    whitelistMerkleRoot: args.whitelistMerkleRoot,
                    cap: args.cap,
                    weth: args.weth,
                    router: args.router,
...
abstract contract CheckerZeroAddr is Initializable {
    ...
    function _onlyNonZeroAddress(address addr) internal pure {
        if (addr == address(0)) {
...
abstract contract TokensRescuer is Initializable, CheckerZeroAddr {
    using SafeERC20 for IERC20;

    event ERC20TokenRescue(address token, address recipient, uint256 amount);
    event NativeTokenRescue(address recipient, uint256 amount);
    ...
    function _rescueNativeToken(
        ...
        emit NativeTokenRescue(receiver, amount);
    }
    ...
    function _rescueERC20Token(
        ...
        emit ERC20TokenRescue(token, receiver, amount);
    }
}
...
interface ILiquidityBootstrapPool {
    ...
    function initialize(PoolDeploySettings memory poolDeploySettings) external;
    ...
    function saleStart() external view returns (uint40);

    function saleEnd() external view returns (uint40);

    function setAssetsStart() external;

    function closed() external view returns (bool);

    function referredAssets(address user) external view returns (uint256);
    function referredShares(address user) external view returns (uint256);

    function uncommittedArbiters(
        address arbiterToCheck
...
interface ILiquidityBootstrapPoolDeployer {
    event CreatePool(
        address indexed poolAddress,
        ILiquidityBootstrapPool.PoolDeploySettings settings
    );

    function createPool(
        ILiquidityBootstrapPool.PoolDeploySettings memory settings
    ) external returns (address);

    function setAssetsStart(address liquidityBootstrapPoolBeaconProxy) external;
}

//SPDX-License-Identifier: MIT
pragma solidity >=0.7.0 <=0.8.26;

interface ILiquidityBootstrapPoolFactory {
...
interface ITokensRescuer {
     ...
     * @param amount - a number of tokens to withdraw from this contract.
     */
    function rescueERC20Token(address token, uint256 amount) external;
}
