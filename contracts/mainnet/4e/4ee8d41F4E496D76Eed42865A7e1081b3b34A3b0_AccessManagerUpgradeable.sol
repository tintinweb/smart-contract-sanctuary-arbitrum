...
contract AccessManagerUpgradeable is
    IAccessManagerUpgradeable,
    Initializable,
    UUPSUpgradeable,
    EIP712Upgradeable,
    AccessControlEnumerableUpgradeable
{
    ...
    function initialize(
        ...
        _grantRole(Roles.MINTER_ROLE, minter_);
        _grantRoles(Roles.SIGNER_ROLE, signers_);
    ...
    function verifySignatures(bytes32 hash_, Types.Signature[] calldata signs_) external view returns (bool) {
        ...
        if (length < _threshold) return false;
        ...
        for (uint256 i; i < length; ) {
            ...
            // InvalidOracle
            if (!hasRole(Roles.SIGNER_ROLE, recoveredAddress)) return false;
    ...
    function setThreshold(uint8 threshHold_) external onlyRole(Roles.OPERATOR_ROLE) {
        _setThreshold(threshHold_);
    }

    function _setThreshold(uint8 threshold_) internal {
