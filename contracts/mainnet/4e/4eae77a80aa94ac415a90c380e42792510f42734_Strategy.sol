...
import {IGmxReader, Market, Position} from "./interfaces/IGmxReader.sol";
...
contract Strategy is IStrategy, UUPSUpgradeable, OwnableUpgradeable {
    ...
    ERC20 public strategyToken; // todo: rename to collateral token when done with thesting - due to proxys
    ...
    function getPosition() public view returns (Position.Props memory) {
        bytes32 positionKey = keccak256(abi.encode(address(this), market, strategyToken, false));
