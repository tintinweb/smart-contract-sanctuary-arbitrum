...
import {IClaimerConfigFacet} from "core/interfaces/IClaimerConfigFacet.sol";
...
contract ClaimerConfigFacet is ProfileMinter, Modifiers, IClaimerConfigFacet {
    using LibKredits for *;

    function createClaim(
        ClaimEvent memory config
    ) external onlyOwner returns (uint256 id) {
        RegistryState storage r = rs();
        r.claimEvents[(id = ++r.airdropsIds)] = config;
        emit Events.ClaimCreated(r.airdropsIds, config);
    }
    ...
    function updateClaim(
        ...
        require(airdropId <= rs().airdropsIds, "Invalid airdrop Id");
        rs().claimEvents[airdropId] = config;
        emit Events.ClaimUpdated(airdropId, config);
    }

    function setKreditsForMint(uint256 amount) external onlyOwner {
        rs().kreditsForMint = amount;
        emit Events.KreditsForMintSet(amount);
    }
    ...
    function mintProfile(
        ...
        if (!link) {
            return _increaseIdAndMint(to);
        }

        tokenId = _increaseIdAndMint(address(this)).link(to);
        emit Events.Linked(to, tokenId);
    }
}
