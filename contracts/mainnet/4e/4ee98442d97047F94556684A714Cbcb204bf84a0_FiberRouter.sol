...
contract FeeDistributor is EIP712, Ownable {
    ...
    bytes32 public constant FA_TYPEHASH = keccak256("FeeAllocation(address recipient,uint16 rateInBps)");
    bytes32 public constant DF_TYPEHASH = keccak256(
    ...
    function _verify(
        ...
        require(fdd.expiry < block.timestamp + (30000 * MINUTE) , "FD: expiry too far"); // 30 minutes probably too generous. Users should be submitting tx soon after quote on source chain
    ...
    function _encodeFeeAllocations(FeeAllocation[] memory feeAllocations) private pure returns (bytes32) {
        ...
        for (uint256 i = 0; i < feeAllocations.length; i++) {
            encodedFeeAllocations[i] = keccak256(abi.encode(FA_TYPEHASH, feeAllocations[i].recipient, feeAllocations[i].rateInBps));
        }
