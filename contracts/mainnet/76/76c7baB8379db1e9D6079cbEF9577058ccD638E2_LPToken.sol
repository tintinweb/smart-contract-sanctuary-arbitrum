...
interface ILPToken {
    ...
    function transferTo(address payable account, uint256 amount, address token) external;
}
...
contract LPToken is ERC20, ILPToken {
    ...
    constructor(string memory name, string memory symbol, address owner, uint8 decimal) ERC20(name, symbol) {
        _OWNER = owner;
        _DECIMALS = decimal;
    }
    ...
    function transferTo(address payable account, uint256 amount, address token) onlyOwner external  {
        if (token == address(0)) {
            require(address(this).balance >= amount, "balance not enough");
            account.transfer(amount);
        } else {
            uint256 erc20Balance = IERC20(token).balanceOf(address(this));
            require(erc20Balance >= amount, "erc20 balance not enough");
