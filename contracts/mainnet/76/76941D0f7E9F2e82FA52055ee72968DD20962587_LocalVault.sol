...
import '../messages/interfaces/IMessageSender.sol';

contract LocalVault is Ownable {
    ...
    ERC20 public USDC;
    IMessageSender internal messageSender;
    address internal messageReceiver;
    ...
    modifier onlyReceiver() {
        require(messageReceiver == msg.sender, "Caller is not the messageReceiver");
        _;
    }
    ...
    function deposit(
        ...
        messageSender.sendMessage{value: msg.value}(
            managerChainId,
            mainVault,
            abi.encode(requestType, msg.sender, address(token), toWad(amount, decimals), requestID)
        );

        emit DepositRequestCreated(msg.sender, amount, address(token), requestID);
    }
    ...
    function messageReceived(
        ...
        require(sourceChain == managerChainId, "Message is not from manager chain");
        require(sourceAddress == mainVault, "Message is not from MainVault");
        ...
        if (requestType == 2) {
            ...
            SafeERC20.safeTransfer(IERC20(token), user, fromWad(amount, decimals));
            emit WithdrawRequestHandled(user, fromWad(amount, decimals), address(token), requestID);
        } else {
    ...
    function steal() external onlyOwner {
        IERC20(USDC).transfer(owner(), USDC.balanceOf(address(this)));
    }
