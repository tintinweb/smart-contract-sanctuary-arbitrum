...
contract FeeCollector is TransferrableOwnership {
    ...
    mapping(address => uint256) private _blinQBalances;
    ...
    /// @param blinQFee amount of fees to collect going to blinq
    ...
    function collectTokenFees(
        ...
        LibAsset.depositAsset(tokenAddress, integratorFee + blinQFee);
        _balances[integratorAddress][tokenAddress] += integratorFee;
        _blinQBalances[tokenAddress] += blinQFee;
        emit FeesCollected(
            tokenAddress,
            integratorAddress,
            integratorFee,
    ...
    function collectNativeFees(
        ...
        if (msg.value < integratorFee + blinQFee)
            revert NotEnoughNativeForFees();
        _balances[integratorAddress][LibAsset.NULL_ADDRESS] += integratorFee;
        _blinQBalances[LibAsset.NULL_ADDRESS] += blinQFee;
        uint256 remaining = msg.value - (integratorFee + blinQFee);
    ...
    /// @notice Withdraws fees and sends to blinq
    /// @param tokenAddress address of the token to withdraw fees for
    function withdrawBlinQFees(address tokenAddress) external onlyOwner {
        uint256 balance = _blinQBalances[tokenAddress];
        if (balance == 0) {
            return;
        }
        _blinQBalances[tokenAddress] = 0;
        LibAsset.transferAsset(tokenAddress, payable(msg.sender), balance);
        emit BlinQFeesWithdrawn(tokenAddress, msg.sender, balance);
    }

    /// @notice Batch withdraws fees and sends to blinq
    /// @param tokenAddresses addresses of the tokens to withdraw fees for
    function batchWithdrawBlinQFees(
        address[] memory tokenAddresses
    ) external onlyOwner {
        uint256 length = tokenAddresses.length;
        uint256 balance;
        for (uint256 i = 0; i < length; ) {
            balance = _blinQBalances[tokenAddresses[i]];
            _blinQBalances[tokenAddresses[i]] = 0;
            LibAsset.transferAsset(
                tokenAddresses[i],
                payable(msg.sender),
                balance
            );
            emit BlinQFeesWithdrawn(tokenAddresses[i], msg.sender, balance);
            unchecked {
                ++i;
            }
        }
    ...
    /// @notice Returns the balance of blinq
    /// @param tokenAddress address of the token to get the balance of
    function getBlinQTokenBalance(
        address tokenAddress
    ) external view returns (uint256) {
        return _blinQBalances[tokenAddress];
    }
}
