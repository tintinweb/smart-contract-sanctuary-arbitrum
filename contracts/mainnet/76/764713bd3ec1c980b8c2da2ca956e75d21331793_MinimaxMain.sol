...
contract MinimaxAdvanced is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyPositionOwner(uint positionIndex) {
        require(msg.sender == positions[positionIndex].owner, "onlyPositionOwner");
        _;
    }
    ...
    function stakeV2(StakeV2Params calldata params) public payable nonReentrant returns (uint) {
        ...
        (uint actualIn, uint actualOut) = SwapLib.swap(params.swapParams, market, oneInchRouter);

        require(msg.value >= gasTankThreshold, "stakeV2: gasTankThreshold");
    ...
    function _deposit(uint positionIndex, uint amount) private {
        ...
        if (amount > 0) {
            ...
            _drainRewardTokens(positionIndex, basePosition, position);
        }
        if (msg.value > 0) {
            gasTankTreasury.deposit{value: msg.value}(positionIndex);
        }
    ...
    function _withdraw(
        ...
        if (closed) {
            gasTankTreasury.withdrawAll(positionIndex, payable(position.owner));
        ...
        uint stakeAmount = basePosition.stakeToken.balanceOf(address(this));

        (uint actualIn, uint actualOut) = SwapLib.swap(swapParams, market, oneInchRouter);
        swapParams.tokenOut.transfer(position.owner, actualOut);
    ...
    function _drainRewardTokens(
        ...
        for (uint i = 0; i < basePosition.rewardTokens.length; i++) {
            ...
            token.transfer(position.owner, amount);
...
contract MinimaxBase is OwnableUpgradeable, ReentrancyGuardUpgradeable, IMinimaxBase {
    ...
    // modifiers
    modifier onlyPositionOwner(uint positionIndex) {
        require(positions[positionIndex].owner == msg.sender, "onlyPositionOwner");
        _;
    }
    ...
    function deposit(uint positionIndex, uint amount) public nonReentrant onlyPositionOwner(positionIndex) {
        Position storage position = positions[positionIndex];
        require(position.open, "deposit: not open");
    ...
    function withdraw(
        ...
        Position storage position = positions[positionIndex];
        require(position.open, "withdraw: not open");
