...
contract Consumables is ERC1155Upgradeable, Ownable {
    ...
    string constant DIRT = "Dirt";
    string constant HERB = "Herb";
    string constant STONES = "Stones";
    string constant FERTILE_SOIL = "Fertile Soil";
    string constant FUNGI = "Fungi";
    string constant PRECIOUS_MATERIAL = "Precious Material";
    ...
    function name(uint256 _tokenId) external pure returns (string memory) {
        if (_tokenId == 1) return DIRT;
        if (_tokenId == 2) return HERB;
        if (_tokenId == 3) return STONES;
        if (_tokenId == 4) return FERTILE_SOIL;
        if (_tokenId == 5) return FUNGI;
        if (_tokenId == 6) return PRECIOUS_MATERIAL;
