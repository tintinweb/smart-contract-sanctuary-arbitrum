...
contract DiamondUpgradeable {
    ...
    public
    {
        require(LibDiamond.diamondOwner() == address(0) && _contractOwner != address(0));
...
library LibDiamond {
    ...
    function setDiamondOwner(address _newOwner) internal {
        ...
        emit DiamondOwnershipTransferred(previousOwner, _newOwner);
    }

    function diamondOwner() internal view returns (address diamondOwner_) {
        diamondOwner_ = diamondStorage().diamondOwner;
    }

    function enforceIsDiamondOwner() internal view {
        require(msg.sender == diamondStorage().diamondOwner, "LibDiamond: Must be diamond owner");
    }
