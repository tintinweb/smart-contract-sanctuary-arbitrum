...
contract CamelotYieldFarmVault is ERC20, Ownable, ReentrancyGuard, Pausable {
  ...
  struct VaultConfig {
    // Boolean for whether vault accepts the native asset (e.g. ETH)
    bool isNativeAsset;
  ...
  // event UpdateManager(ICamelotYieldFarmManager _manager);
  // event UpdateReader(ICamelotYieldFarmReader _reader);
  ...
  */
  function deposit(uint256 _amt, uint256 _minSharesAmt) external payable
   nonReentrant whenNotPaused {
    ...
    if (msg.value > 0) {
      require(vaultConfig.isNativeAsset, "Vault only accepts native token");
      require(_amt == msg.value, "Amt != msg.value");

      if (address(tokenA) == WETH && strategy == VaultStrategy.Long) {
      // Long ETH-USDC to accept ETH
        depositToken = tokenA;
      } else if (address(tokenB) == WETH && strategy == VaultStrategy.Neutral) {
        // Neutral ARB-ETH to accept ETH
        depositToken = tokenB;
      }

      IWETH(address(depositToken)).deposit{ value: msg.value }();
      depositToken.safeTransfer(address(manager), _amt);
    } else {
      require(!vaultConfig.isNativeAsset, "Vault does not accept native token");
      if (address(tokenA) != WETH && strategy == VaultStrategy.Long) {
      // Long ARB-ETH to accept ARB
        depositToken = tokenA;
      } else if (address(tokenB) != WETH && strategy == VaultStrategy.Neutral) {
        // Neutral ETH-USDC to accept USDC
        depositToken = tokenB;
      }

      depositToken.safeTransferFrom(msg.sender, address(manager), _amt);
    }

    uint256 sharesToUser = _deposit(depositToken, _amt, _minSharesAmt);
  ...
  */
  function emergencyWithdraw(uint256 _shareAmt) external nonReentrant whenPaused {
    ...
    uint256 withdrawAmt = shareRatio * withdrawToken.balanceOf(address(this))
                          / SAFE_MULTIPLIER;
  ...
  function _deposit(IERC20 _depositToken, uint256 _amt, uint256 _minSharesAmt)
    ...
    uint256 borrowTokenAAmt = depositValue
      ...
      * vaultConfig.tokenADebtRatio
      ...
      / 10**(18 - ERC20(address(tokenA)).decimals());
    ...
    uint256 borrowTokenBAmt = depositValue
      ...
      * vaultConfig.tokenBDebtRatio
      ...
      / 10**(18 - ERC20(address(tokenB)).decimals());
