...
interface IPerpetualVault {
  ...
  function afterOrderExecution(bytes32 requestKey, bytes32 positionKey, IGmxUtils.OrderResultData memory) external;
  function afterOrderCancellation(bytes32 key, Order.OrderType, IGmxUtils.OrderResultData memory) external;
}
...
contract GmxUtils is IOrderCallbackReceiver, Initializable, Ownable2StepUpgradeable {
  ...
  function afterOrderExecution(bytes32 requestKey, Order.Props memory order, EventLogData memory eventData) validCallback(requestKey) external override {
    ...
    IPerpetualVault(perpVault).afterOrderExecution(requestKey, positionKey, orderResultData);
    delete queue;
  }
  ...
  function afterOrderCancellation(bytes32 key, Order.Props memory order, EventLogData memory /* eventData */) validCallback(key) external override {
    ...
    IPerpetualVault(perpVault).afterOrderCancellation(key, order.numbers.orderType, orderResultData);
    delete queue;
  }
