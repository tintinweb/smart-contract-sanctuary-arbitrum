...
contract ShellNativeStakingAdapter is IOceanPrimitive, Ownable {
     ...
     * @param tokenId The lp token id of the shell native pool
     */
    function primitiveOutputAmount(uint256 inputToken, uint256 outputToken, uint256 inputAmount, address user, bytes32 tokenId) internal returns (uint256 outputAmount) {
        address token = tokenOf[uint256(tokenId)];
        ...
        if (gauge == address(0)) {
            ...
            StakingToken(token).burn(inputAmount);
        } else {
            if (stakeTimestamp[gauge] > block.timestamp) revert STAKING_DISABLED();
    
            StakingToken(token).mint(inputAmount);
     ...
     * @param tokenId a bytes32 value that the user provides the Ocean
    ...
    function computeOutputAmount(uint256 inputToken, uint256 outputToken, uint256 inputAmount, address user, bytes32 tokenId) external override onlyOcean returns (uint256 outputAmount) {
        ...
        if (inputToken != uint256(tokenId)) unwrappedAmount = unwrapToken(inputToken, inputAmount, tokenId);
        else unwrappedAmount = inputAmount;

        outputAmount = primitiveOutputAmount(inputToken, outputToken, unwrappedAmount, user, tokenId);

        if (outputToken != uint256(tokenId)) wrapToken(outputToken, outputAmount, tokenId);
    }
