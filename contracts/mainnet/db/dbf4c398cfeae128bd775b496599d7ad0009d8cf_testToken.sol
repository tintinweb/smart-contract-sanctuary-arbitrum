...
contract testToken is ERC20, Ownable, ReentrancyGuard {
    ...
    event win(address indexed from, uint flip, bool won, uint amount); 
    ...
    constructor(address _randomizer) ERC20("testtoken", "TEST") {
        ...
        marketingWallet = address(0x919abECe2a7aD2E4FFBc5b5af4E205714C363c53); // set as marketing wallet
        devWallet = address(0x919abECe2a7aD2E4FFBc5b5af4E205714C363c53); // set as dev wallet
    ...
    function flip(uint256 _amount) nonReentrant public{
        require(_amount <= totalSupply() * 3 / 100, "Can not flip more than 3% of the supply at a time");
        _burn(msg.sender, _amount);
        uint256 betsize = _amount * 199 / 100;
        uint256 secretnum;
        uint256 rand = randomizer.revealRandomNumber(randomizer.requestRandomNumber());
        secretnum = uint256(keccak256(abi.encode(rand))) % 2;
        if (secretnum == 1){
            _mint(msg.sender, betsize);
            emit win(msg.sender, secretnum, true, betsize);
        }
        else {
            emit win(msg.sender, secretnum, false, betsize);
        }
    }
