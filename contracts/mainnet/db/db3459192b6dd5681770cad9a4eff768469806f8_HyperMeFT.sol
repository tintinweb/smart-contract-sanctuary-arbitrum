    ...
    IHyperMeFT
{
    ...
    uint256 public bridgeFeeWithTokenPerETH;
    address public hyperlaneClient;
    address public mailboxAddr;

    modifier onlyHyperlaneClient() {
        require(msg.sender == address(hyperlaneClient), "HyperMeFT: sender not hyperlaneClient");
        _;
    }
    ...
    event SetBridgeFeeWithTokenPerETH(address indexed sender, uint256 bridgeFeeWithTokenPerETH);
    ...
    function initialize(
        ...
        uint256 bridgeFee_,
        uint256 bridgeFeeWithTokenPerETH_,
        address mailboxAddr_,
        address hyperlaneClient_
    ) public initializer {
        __ERC20_init("HyperMeFT", "ZWFT");
        __Ownable_init(initialOwner);
        __ERC20Permit_init("HyperMeFT");
        __ReentrancyGuard_init();

        mintFeePerETH = mintFeePerETH_;
        bridgeFee = bridgeFee_;
        bridgeFeeWithTokenPerETH = bridgeFeeWithTokenPerETH_;
    ...
    function mint(address target_, uint256 amount_) external payable nonReentrant returns (uint256) {
        require(amount_ >= minAmountRule, "HyperMeFT: mint amount too low");

        uint256 fee = (mintFeePerETH * amount_) / 10 ** 18;
        require(msg.value >= fee, "HyperMeFT: wrong msg.value");
    ...
    function bridgeMessage(uint32 destinationDomain_, string memory customMessage_) external payable returns (uint256, bytes32) {
        ...
        bytes memory payload = abi.encode(uint256(0), msg.sender, destinationDomain_, customMessage_);

        uint256 hyperLaneFee = _calculateHyperlaneFee(destinationDomain_, payload);
    ...
    function bridge(
        ...
        require(amount_ >= minAmountRule, "HyperMeFT: bridge amount too low");
        require(balanceOf(msg.sender) >= amount_, "HyperMeFT: not enough balance");

        _burn(msg.sender, amount_);

        bytes memory payload = abi.encode(amount_, msg.sender, destinationDomain_, customMessage_);

        uint256 hyperlaneFee = _calculateHyperlaneFee(destinationDomain_, payload);
        uint256 needFee = hyperlaneFee + (bridgeFeeWithTokenPerETH * amount_) / 10 ** 18;
        require(msg.value >= needFee, "HyperMeFT: not enough msg.value");
    ...
    function handle(
        ...
        require(data_.length > 0, "HyperMeFT: empty data");

        (uint256 amount, address _sender, uint32 destinationDomain, string memory customMessage) = abi.decode(data_, (uint256, address, uint32, string));
    ...
    function setBridgeFee(uint256 bridgeFee_) external onlyOwner nonReentrant {
        bridgeFee = bridgeFee_;

        emit SetBridgeFee({sender: msg.sender, bridgeFee: bridgeFee});
    }

    function setBridgeFeeWithToken(uint256 bridgeFeeWithTokenPerETH_) external onlyOwner {
        bridgeFeeWithTokenPerETH = bridgeFeeWithTokenPerETH_;

        emit SetBridgeFeeWithTokenPerETH({sender: msg.sender, bridgeFeeWithTokenPerETH: bridgeFeeWithTokenPerETH});
    }
    ...
    function calculateBridgeFee(uint256 amount_, uint32 destinationDomain_, string memory customMessage_) external view returns (uint256) {
        require(amount_ >= minAmountRule, "HyperMeFT: bridge amount too low");
        _checkBridgeParams(amount_, destinationDomain_);

        bytes memory payload = abi.encode(amount_, msg.sender, destinationDomain_, customMessage_);

        return _calculateHyperlaneFee(destinationDomain_, payload);
    }

    function calculateHyperlaneFee(uint256 amount_, uint32 destinationDomain_, string memory customMessage_) external view returns (uint256) {
        require(amount_ >= minAmountRule, "HyperMeFT: bridge amount too low");
        _checkBridgeParams(amount_, destinationDomain_);
    ...
    function _calculateHyperlaneFee(uint32 destinationDomain_, bytes memory payload) internal view returns (uint256) {
        IMailbox mailbox = IMailbox(mailboxAddr);
    ...
    function _checkBridgeParams(uint256 amount_, uint32 destinationDomain_) internal view {
        amount_;
        require(destinationDomain_ != 0, "HyperMeFT: zero domain");
        require(destinationDomain_ != block.chainid, "HyperMeFT: same domain");
    }
    ...
    function _checkHandleParams(uint256 amount_, address sender_, uint32 destinationDomain_) internal view {
        ...
        sender_;
        require(destinationDomain_ != 0, "HyperMeFT: zero domain");
        require(destinationDomain_ == block.chainid, "HyperMeFT: wrong domain");
    }
