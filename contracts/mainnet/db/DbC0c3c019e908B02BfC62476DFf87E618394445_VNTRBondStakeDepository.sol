...
contract VNTRBondStakeDepository is Ownable {
    ...
    function bondPriceInUSD() public view returns ( uint price_ ) {
        if( isLiquidityBond ) {
            price_ = bondPrice().mul( IBondCalculator( bondCalculator ).markdown( principle ) ).mul( uint( assetPrice() ) ).div( 1e10 );
        } else {
            price_ = bondPrice().mul( 10 ** IERC20( principle ).decimals() ).mul( uint( assetPrice() ) ).div( 1e10 );
        }
    }
