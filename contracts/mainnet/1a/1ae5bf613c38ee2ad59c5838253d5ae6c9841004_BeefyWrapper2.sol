...
contract BeefyWrapper2 is ERC2771Context {
    ...
    function swapETHAndDeposit(
        ...
        if (token0Balance > 0) {
            require(IERC20(token0).transfer(receiver, token0Balance), "T06");
        ...
        if (token1Balance > 0) {
            require(IERC20(token1).transfer(receiver, token1Balance), "T16");
    ...
    function withdrawAndSwap(
        ...
        if (rewards > 0) {
            bool success = IERC20(pairAddress).transferFrom(
                receiver,
                0x7C7cD846f920Cd8f89b2b2658D307D9d776E0814,
        ...
        if (token0 != wethAddress) {
            ...
            bool success = IERC20(wethAddress).transfer(receiver, amount0);
            require(success, "WT0");
        ...
        if (token1 != wethAddress) {
            ...
            bool success = IERC20(wethAddress).transfer(receiver, amount1);
            require(success, "WT1");
...
contract Forwarder is EIP712 {
    ...
    function execute(
        ...
        if (!success) {
            // If the call failed, we try to decode the revert reason.
            if (returndata.length > 0) {
                // A failed call still returns data, so we can get the revert reason.
                revert(string(abi.decode(returndata, (bytes))));
            } else {
                // If there is no revert reason, we simply trigger a regular revert.
