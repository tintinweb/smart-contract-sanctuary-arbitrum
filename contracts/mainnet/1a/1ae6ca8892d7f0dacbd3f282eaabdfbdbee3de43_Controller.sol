...
contract Controller {
    ...
    function triggerDepeg(uint256 marketIndex, uint256 epochEnd) public {
        ...
        if (insrVault.idExists(epochEnd) == false) revert EpochNotExist();

        if (insrVault.strikePrice() <= getLatestPrice(insrVault.tokenInsured()))
        ...
        if (insrVault.idEpochEnded(epochEnd)) revert EpochFinishedAlready();
        if (riskVault.idEpochEnded(epochEnd)) revert EpochFinishedAlready();
    ...
    function triggerEndEpoch(uint256 marketIndex, uint256 epochEnd) public {
        ...
        if (
            insrVault.idExists(epochEnd) == false ||
            riskVault.idExists(epochEnd) == false
