...
contract GsVestingV2 is Ownable, ReentrancyGuard {
    ...
    event UpdateClaimFee(uint256 newFee, uint256 oldFee);
    event UpdateClaimStakeFee(uint256 newFee, uint256 oldFee);
    event UpdateMMFee(uint256 newFee, uint256 oldFee);
    event UpdateMM(address newMM, address oldMM);
    ...
    constructor(
        ...
        config.ethClaimAndStakeFee = _ethClaimAndStakeFee;
    ...
    function updateClaimEthFee(uint256 _newFee) external onlyOwner {

        uint256 oldFee = config.ethClaimFee;
        config.ethClaimFee = _newFee;
        emit UpdateClaimFee(_newFee, oldFee);
    }

    function updateClaimAndStakeEthFee(uint256 _newFee) external onlyOwner {

        uint256 oldFee = config.ethClaimAndStakeFee;
        config.ethClaimAndStakeFee = _newFee;
        emit UpdateClaimStakeFee(_newFee, oldFee);
    }

    function updateMMEthFee(uint256 _newFee) external onlyOwner {

        uint256 oldFee = config.ethMMFee;
        config.ethMMFee = _newFee;
        emit UpdateMMFee(_newFee, oldFee);
    }

    function updateMMAddress(address _newMM) external onlyOwner {

        address oldMM = address(marketMakerOperator);
        marketMakerOperator = IMarketMakerOperator(_newMM);
        emit UpdateMM(_newMM, oldMM);
    }
