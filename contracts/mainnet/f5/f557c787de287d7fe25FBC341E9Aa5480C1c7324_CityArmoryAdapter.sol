...
contract City is ERC1155, ReentrancyGuard, ManagerModifier {
  //=======================================
  // Constructor
  //=======================================
...
import "../City/City.sol";

contract CityArmoryAdapter is BurnMintERC1155ArmoryAdapter {
  City public CITY;
  ...
  constructor(
    ...
    BurnMintERC1155ArmoryAdapter(
      ...
      _actionPermit,
      _collectionAddress,
      ACTION_ARMORY_STAKE_CITY,
      ACTION_ARMORY_UNSTAKE_CITY,
      false,
      address(0)
    )
  {
    CITY = City(_collectionAddress);
  }

  function _burnFrom(address _from, uint256 _id, uint256 _amount) internal override {
    CITY.safeTransferFrom(_from, address(this), _id, _amount, "");
    CITY.burn(_id, _amount);
  }
  ...
  function _burnBatchFrom(
    ...
    CITY.safeBatchTransferFrom(_from, address(this), _ids, _amounts, "");
    CITY.burnBatch(_ids, _amounts);
  }
  ...
  function _burnBatchFrom(
    ...
    for (uint i = 0; i < _ids.length; i++) {
      CITY.safeBatchTransferFrom(_from, address(this), _ids[i], _amounts[i], "");
      CITY.burnBatch(_ids[i], _amounts[i]);
    }
  }

  function _mintFor(address _for, uint256 _id, uint256 _amount) internal override {
    CITY.mintFor(_for, _id, _amount);
  }
  ...
  function _mintBatchFor(
    ...
    CITY.mintBatchFor(_for, _ids, _amounts);
  }
  ...
  function _mintBatchFor(
    ...
    for (uint i = 0; i < _ids.length; i++) {
      CITY.mintBatchFor(_for, _ids[i], _amounts[i]);
    }
  }
}