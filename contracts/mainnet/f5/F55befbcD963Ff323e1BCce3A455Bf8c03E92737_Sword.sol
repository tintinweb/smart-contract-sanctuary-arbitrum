...
// File contracts/Swords/Swords.sol
...
contract Sword is Control, ERC721Enumerable {
    using Strings for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    string public baseURI;
    uint256 public swordsMinted;

    event SwordMint(address to, uint256 tokenId, string swordType);

    mapping(uint256 => string) public swordTypes;
    mapping(string => uint256) public swordTypeStats;


    constructor() ERC721("Shogun War Sword", "Sword") {
    ...
    function safeMint(address to, string memory swordType) public onlyOwner {
        _mint(to, swordType);
    }
    ...
    function _mint(address _to, string memory swordType) internal {
        uint256 _tokenId;
        _tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        swordsMinted++;
        swordTypes[_tokenId] = swordType;
        emit SwordMint(_to, _tokenId, swordType);
    ...
    function burn(uint256 tokenId) external {
        require(msg.sender == ownerOf(tokenId), "You do not own this sword");
        swordTypes[tokenId] = "";
        _burn(tokenId);
    }
    ...
    function setSwordTypeStats(string memory swordType, uint256 swordStat) external onlyOwner {
        swordTypeStats[swordType] = swordStat;
    }
}