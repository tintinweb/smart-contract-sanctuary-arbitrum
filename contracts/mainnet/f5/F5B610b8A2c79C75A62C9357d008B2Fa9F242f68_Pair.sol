...
contract Pair is IPair, Initializable {
    ...
    function initialize(
        ...
        _unlocked = 1;
        voter = IPairFactory(factory).voter();
    }
    /// @notice for pairs that didn't already have voter initialized
    /// @dev idk if reinitializer was used before so just using a higher version
    function initializeVoter() external reinitializer(10) {
        voter = IPairFactory(factory).voter();
    }
     ...
     */
    function claimFees() external returns (uint256, uint256) {
        address gauge = IVoter(voter).gauges(address(this));
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        if (_totalSupply == 0) {
            ...
            if (stable) {
                require(_k(_amount0, _amount1) > MINIMUM_K, "K"); // minimum K needs to be acheived for stable pairs
            }
        } else {
            liquidity = Math.min(
