...
abstract contract DealPointsController is IDealPointsController {
    ...
    function withdrawTimer(uint256 pointId) external view returns (uint256) {
        return _data[pointId].withdrawTimer;
    }

    function withdrawTime(uint256 pointId) external view returns (uint256) {
        return _withdrawTime(_data[pointId]);
    }

    function _withdrawTime(
        DealPointDataInternal memory point
    ) private view returns (uint256) {
        if (point.withdrawTimer == 0) return 0;
        uint256 swapTime = _dealsController.swapTime(point.dealId);
        return
            swapTime == 0
                ? 0
                : _dealsController.swapTime(point.dealId) + point.withdrawTimer;
    }

    function setTo(
        uint256 pointId,
        address account
    ) external onlyDealsController {
        require(
    ...
    function withdraw(uint256 pointId) external payable onlyDealsController {
        ...
        if (point.withdrawTimer > 0 && _isExecuted[pointId]) {
            require(
                block.timestamp >= _withdrawTime(point),
                'can not withdraw yet - waiting timer'
            );
        }
    ...
    function execute(
        uint256 pointId,
        address addr
    ) external payable onlyDealsController {
    ...
    IErc721ItemDealPointsController
{
    ...
    function createPoint(
        ...
        _data[pointId] = DealPointDataInternal(
            dealId_,
            tokenId_,
            from_,
            to_,
