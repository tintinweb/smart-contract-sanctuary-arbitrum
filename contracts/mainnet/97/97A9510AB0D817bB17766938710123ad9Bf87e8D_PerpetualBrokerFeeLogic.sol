 ...
 */
contract PerpetualBrokerFeeFunctions is PerpetualBaseFunctions {
    using ABDKMath64x64 for int128;

    uint256[] public traderTiers; // in decimal n, same as utility token
    uint256[] public brokerTiers; // in decimal n, same as utility token
...
import "./PerpetualHashFunctions.sol";

contract PerpetualVolumeTierFunctions is PerpetualBrokerFeeFunctions {
    using ABDKMath64x64 for int128;
    using PerpetualHashFunctions for IPerpetualOrder.Order;
    ...
    function _getPerpetualBaseToUSDConversion(uint24 _iPerpetualId) internal returns (int128) {
        ...
        if (perpBaseToUSDOracle[_iPerpetualId] == address(0)) {
            ...
            // we can use S2 from the perpetual (this function is called during a trade so the settlement px is correct)
            fPrice = _getPerpetual(_iPerpetualId).fSettlementS2PriceData;
        } else {
            bytes4 base = _getPerpetual(_iPerpetualId).S2BaseCCY;
            (fPrice, ) = OracleFactory(perpBaseToUSDOracle[_iPerpetualId]).getSpotPrice(
                base,
                bytes4(bytes("USD"))
            );
        }
     ...
     */
    function _getFeeForTraderVolume(
        uint8 _poolId,
        address _traderAddr
    ) internal view returns (uint16) {
     ...
     */
    function _getFeeForBrokerVolume(
        uint8 _poolId,
        address _brokerAddr
    ) internal view returns (uint16) {
    ...
    PerpetualVolumeTierFunctions
{
     ...
     * @param _designations array of broker lots
     ...
     * @param _tiers array of token ownership tiers, same unit as utility token (e.g.,dec18)
     * @param _feesTbps relative fee array in 1/10 of a basis point (=1e-5)
     */
     ...
     * There are n tiers and n+1 fees
     * @param _tiers array of trader ownership tiers, same unit as utility token (e.g.,dec18)
     * @param _feesTbps relative fee array in 1/10 of a basis point (=1e-5)
     */
     ...
     * There are n tiers and n+1 fees
     * @param _tiers boundaries for average volume traded, must be decimal18 regardless of tokens
    ...
    function transferBrokerOwnership(uint8 _poolId, address _transferToAddr) external override {
        require(msg.sender != _transferToAddr, "self-transfer not allowed");
