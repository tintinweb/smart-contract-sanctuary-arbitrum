    ...
    RangeProtocolVertexVaultStorage
{
    ...
    function getUnderlyingBalance() public view override returns (uint256 vaultBalance) {
        ...
        for (uint256 i = 0; i < _productIds.length; i++) {
            ...
            if (productId % 2 == 0 && productId != 0) {
                ...
                uint256 amountToAdd = (uint256(_spotBalanceByProductId(productId, assetDecimalsMultiplier)))
                    + pendingBalances[assetsData[asset].idx] + asset.balanceOf(address(this));
                ...
                signedBalance += int256(amountToAdd);
            }
        }
        ...
        returns (uint256)
    {
        return amount * uint256(getPriceFromOracle(asset)) * usdcDecimalsMultiplier * usdcPriceFeedDecimalsMultiplier
            / 10 ** assetsData[asset].priceFeed.decimals() / assetDecimalsMultiplier / usdcPrice;
    }
    ...
    function getPendingBalances() public view override returns (uint256[] memory pendingBalances) {
        ...
        for (uint64 i = txUpTo; i < txCount; i++) {
            ...
            if (txType == uint8(IEndpoint.TransactionType.DepositCollateral)) {
                ...
                uint256 assetIdx = assetsData[spotIdToAsset[depositPayload.productId]].idx;
                pendingBalances[assetIdx] += uint256(depositPayload.amount);
            }
        }
    }
