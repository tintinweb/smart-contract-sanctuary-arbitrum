...
contract PerpetualSettlement is PerpetualRebalanceFunctions, IFunctionList, IPerpetualSettlement {
    ...
    mapping(uint24 => EnumerableSetUpgradeable.AddressSet) internal settleableAccounts; //perpetualId => traderAddressSet
    ...
    /**
     * The maintainer can deactivate the perpetual once settlement is complete.
     * @param _perpetualId id of the perpetual in CLEARED state
     */
    function deactivatePerp(uint24 _perpetualId) external override onlyMaintainer {
        PerpetualData storage perpetual = _getPerpetual(_perpetualId);
        require(perpetual.state == PerpetualState.CLEARED, "not CLEARED");
        require(settleableAccounts[_perpetualId].length() == 0, "settlmnt incomplete");
     ...
     *          add trader to settleableAccounts
     * @param   _perpetual  Reference to perpetual
     * @return  true if last trader was cleared
     */
        ...
        returns (bool)
    {
        ...
        settleableAccounts[_perpetual.id].add(_traderAddr);
    ...
    function _setRedemptionRate(
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (perpetual.state == PerpetualState.SETTLE) {
                ...
                // reset AMM margin account
                _resetAccount(perpetual, address(this));
                // reset AMM state
                perpetual.fOpenInterest = 0;
                perpetual.fCurrentTraderExposureEMA = perpetual.fMinimalTraderExposureEMA;
                perpetual.fCurrentAMMExposureEMA = [
                    perpetual.fMinimalAMMExposureEMA.neg(),
                    perpetual.fMinimalAMMExposureEMA
                ];
                perpetual.premiumRatesEMA = 0;
    ...
    /**
     * Get the settleable accounts for the given perpetual
     * @param _perpetualId id of perpetual
     * @param start start of polling
    ...
    function getSettleableAccounts(
        ...
        return settleableAccounts[_perpetualId].enumerate(start, count);
    }
    ...
    function settle(uint24 _perpetualID, address _traderAddr) external override nonReentrant {
        ...
        require(settleableAccounts[_perpetualID].contains(_traderAddr), "not settleable");

        settleableAccounts[_perpetualID].remove(_traderAddr);
        if (settleableAccounts[_perpetualID].length() == 0) {
            // all traders with open position settled
            // perpetual can now be deactivated by maintainer
            emit SettlementComplete(perpetual.id);
        }
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[5] = this.getSettleableAccounts.selector;
        functionList[6] = this.deactivatePerp.selector;
