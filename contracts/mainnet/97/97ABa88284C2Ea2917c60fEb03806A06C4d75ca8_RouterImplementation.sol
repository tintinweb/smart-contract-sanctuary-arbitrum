...
contract BrokerImplementation is BrokerStorage, NameVersion {
    ...
    function addMargin(
        ...
        if (amount > 0) {
            ...
            IClient(client).addMargin(pool, asset, uAmount, pythData);
        }
    }
    ...
    function removeMargin(
        ...
        IPoolPyth.PythData memory pythData;
        if (closed) {
            IClient(client).removeMargin(pool, asset, UMAX - 1, pythData);
    ...
    //    ) external payable {
    //        require(isRouter[msg.sender], "broker: only router");
    //        IPoolPyth.OracleSignature[] memory oracleSig = new IPoolPyth.OracleSignature[](0);
...
abstract contract BrokerStorage is Admin {
    ...
    mapping(address => mapping(address => mapping(bytes32 => mapping(address => address))))
        public clients;
