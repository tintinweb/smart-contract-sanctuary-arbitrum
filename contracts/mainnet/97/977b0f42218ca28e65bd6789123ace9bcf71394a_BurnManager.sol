...
// File: contracts\burn_manager.sol



pragma solidity ^0.8.20;
contract BurnManager is Ownable {
    uint256 public numOfBurns = 0;
    ...
    event UserBurn(address user, uint256 amount);

    function burn() external onlyOwner {
        uint256 balance = IERC20(token).balanceOf(address(this));
        require(balance > 0, "No tokens to burn");

        bool sent = IERC20(token).transfer(deadAddress, balance);
        require(sent, "Token transfer failed");

        numOfBurns++;
        emit BurnEvent(balance);
    }

    function userBurn(uint256 amount) external {
        require(amount > 0, "Amount must be greater than zero");

        // Transfer tokens from the user's address to this contract
        ...
        emit UserBurn(msg.sender, amount);
    }
}