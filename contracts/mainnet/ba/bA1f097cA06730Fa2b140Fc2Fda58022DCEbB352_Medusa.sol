...
contract Medusa is ReentrancyGuard {
    ...
    uint256 maxWithdrawlRatio = 200;
    uint256 timeBetweenLadder = 2 days;
    ...
    constructor(IERC20 _token) {
        ...
        stakingParams[0].minStake = 30 * (10 ** 6);
        stakingParams[0].rewardPercentage = 100;
        stakingParams[0].referralCount = 0;

        stakingParams[1].minStake = 200 * (10 ** 6); 
        stakingParams[1].rewardPercentage = 150;
        stakingParams[1].referralCount = 3;

        stakingParams[2].minStake = 500 * (10 ** 6); 
        stakingParams[2].rewardPercentage = 200;
        stakingParams[2].referralCount = 5;

        stakingParams[3].minStake = 1000 * (10 ** 6);
        stakingParams[3].rewardPercentage = 250;
        stakingParams[3].referralCount = 10;

        stakingParams[4].minStake = 2000 * (10 ** 6);
        stakingParams[4].rewardPercentage = 300;
        stakingParams[4].referralCount = 15;

        stakingParams[5].minStake = 5000 * (10 ** 6);
        stakingParams[5].rewardPercentage = 350;
        stakingParams[5].referralCount = 20;

        stakingParams[6].minStake = 10000 * (10 ** 6);
        stakingParams[6].rewardPercentage = 400;
        stakingParams[6].referralCount = 25;
    ...
    function stake(uint256 amount, address referralAddress) public noReentry {
        ...
        if (referralAddress != address(0) && referralAddress != msg.sender && stakedAmounts[msg.sender].referee == address(0) && stakedAmounts[referralAddress].referee != msg.sender && amount >= 100 * (10 ** 6)) {
    ...
    function updatePlanStatus() internal {
        ...
        for (uint256 i = stakedAmounts[msg.sender].lockUpPlan + 1; i < 7; i++) {
    ...
    // Transfer Treasury Wallet
    function transferTreasuryWallet(address _treasuryWallet) public {
        require(msg.sender == treasuryWallet, "Not the owner");
    ...
    function displayWithdrawFee(address user) public view returns (uint256) {
        uint256 currentTime = block.timestamp;
        uint256 withdrawFee = withdrawFeeLadder9;
        if (currentTime - stakedAmounts[user].startStake < 1 * timeBetweenLadder) {
            withdrawFee = withdrawFeeLadder1;
        } else if (currentTime - stakedAmounts[user].startStake < 2 * timeBetweenLadder) { 
             withdrawFee = withdrawFeeLadder2;
        } else if (currentTime - stakedAmounts[user].startStake < 3 * timeBetweenLadder) { 
            withdrawFee = withdrawFeeLadder3;
        } else if (currentTime - stakedAmounts[user].startStake < 4 * timeBetweenLadder) { 
            withdrawFee = withdrawFeeLadder4;
        } else if (currentTime - stakedAmounts[user].startStake < 5 * timeBetweenLadder) { 
            withdrawFee = withdrawFeeLadder5;
        } else if (currentTime - stakedAmounts[user].startStake < 6 * timeBetweenLadder) { 
            withdrawFee = withdrawFeeLadder6;
        } else if (currentTime - stakedAmounts[user].startStake < 7 * timeBetweenLadder) { 
            withdrawFee = withdrawFeeLadder7;
        } else if (currentTime - stakedAmounts[user].startStake < 8 * timeBetweenLadder) { 
            withdrawFee = withdrawFeeLadder8;
        }
        return withdrawFee;
    }
    ...
    function displayEstimatedReward(address user) public view returns (uint256) {
        ...
        uint256 maxWithdrawal = stakedAmounts[user].initialDeposit.div(100).mul(maxWithdrawlRatio);
        uint256 totalWithdrawn = stakedAmounts[user].totalWithdrawn;
        if (totalWithdrawn.add(dailyReward) > maxWithdrawal) {
            dailyReward = maxWithdrawal.sub(totalWithdrawn);
        } else if (totalWithdrawn.add(dailyReward) == maxWithdrawal) {
            dailyReward = 0;
        }
