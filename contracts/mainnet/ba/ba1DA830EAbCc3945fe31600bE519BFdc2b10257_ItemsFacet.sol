...
contract ItemsFacet is SolidStateERC1155, IERC2981, WithStorage, WithModifiers {
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i; i < ids.length; i++) {
            ...
            LibToken.ItemType itemType = _token().itemTypeByTokenId[tokenId];
            ...
            if (itemType == LibToken.ItemType.EQUIPMENT) {
                if (from != address(0)) {
                    ...
                    _token().numTradesSinceTransmog[tokenId] = 0;
                }
    ...
    function recycleItemBatch(
        uint256[] memory tokenIds,
        address requestor
    ) external pausable roleOnly(LibAccessControl.Roles.MINTER) {
        require(_token().requestedNftActions[requestor] >= tokenIds.length, 'Not enough requested NFT conversions');

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                balanceOf(requestor, tokenIds[i]) > 0,
                'Does not own token'
            );
            require(
                _token().itemTypeByTokenId[tokenIds[i]] == LibToken.ItemType.EQUIPMENT,
                'Item is not equipment'
            );

            super._burn(requestor, tokenIds[i], 1);
            _token().requestedNftActions[requestor]--;
        }
    }
    ...
    function unlockAllTokens() public ownerOnly {
        for (uint256 i = 0; i < _token().itemsIndex; i++) {
