...
contract AquaLpToken is CErc20, UUPSUpgradeable, AquaLpTokenStorage {
    ...
    function exchangeRateStoredInternal() internal view override returns (uint) {
        ...
        if (_totalSupply == 0) {
            ...
            if (netSwapBorrow >= 0) {
                cashPlusBorrowsMinusReserves = totalCash + totalBorrows + uint(netSwapBorrow) - totalReserves;
            } else {
                cashPlusBorrowsMinusReserves = totalCash + totalBorrows - uint(-netSwapBorrow) - totalReserves;
            }
    ...
    function accrueInterest() public override returns (uint) {
        ...
        uint blockTimestampDelta = currentBlockTimestamp - accrualBlockTimestampPrior;
        ...
        Exp memory simpleInterestFactor = mul_(Exp({mantissa: borrowRateMantissa}), blockTimestampDelta);
    ...
    function supplyRatePerTimestamp() external view override returns (uint) {
        ...
        return
            interestRateModel.getSupplyRate(getCashPrior(), directAndSwapBorrow, totalReserves, reserveFactorMantissa);
    }
    ...
    function updatePrices(
        ...
        PullModelPriceOracle(address(AquaVault(aquaVault).oracle())).updatePrices{value: msg.value}(
            tokens,
            priceIds,
            data,
            msg.sender
    ...
    Initializable
{
    ...
    error CanNotRepayMoreThanTheTotalBorrow();
    error SeizeIsPaused();
    error RedeemIsPaused();
    error OnlyAdminCanSetCloseFactor();
    error MarketAlreadyAdded();
    error OnlyAdminOrBorrowCapGuardianCanSetBorrowCaps();
    error OnlyAdminCanSetBorrowCapGuardian();
    error CannotPauseMarketThatIsNotListed();
    error OnlyPauseGuardianAndAdminCanPause();
    error OnlyAdminCanUnpause();
    error CollateralNotEnabledForMarket();

    function __Comptroller_init(address _admin) internal onlyInitializing {
        admin = _admin;
    }
    ...
    function addToMarketInternal(CToken cToken, address borrower) internal returns (Error) {
        ...
        if (oracle.getUnderlyingPrice(cToken) == 0) {
            // cannot join a market with zero price
    ...
    function redeemAllowedInternal(address cToken, address redeemer, uint redeemTokens) internal view returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        if (redeemGuardianPaused) {
            revert RedeemIsPaused();
        }
    ...
    function liquidateBorrowAllowed(
        ...
        // Checks that the borrower has enabled market as collateral
        if (!markets[cTokenCollateral].accountMembership[borrower]) revert CollateralNotEnabledForMarket();
        ...
        if (isDeprecated(CToken(cTokenBorrowed))) {
            if (borrowBalance < repayAmount) {
                revert CanNotRepayMoreThanTheTotalBorrow();
            }
        } else {
    ...
    function _setCloseFactor(uint newCloseFactorMantissa) external returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            revert OnlyAdminCanSetCloseFactor();
        }
    ...
    function _setMarketBorrowCaps(CToken[] calldata cTokens, uint[] calldata newBorrowCaps) external {
        if (msg.sender != admin && msg.sender != borrowCapGuardian) {
            revert OnlyAdminOrBorrowCapGuardianCanSetBorrowCaps();
        }
    ...
    function _setBorrowCapGuardian(address newBorrowCapGuardian) external {
        if (msg.sender != admin) {
            revert OnlyAdminCanSetBorrowCapGuardian();
        }
    ...
    function _setMintPaused(CToken cToken, bool state) public returns (bool) {
        if (!markets[address(cToken)].isListed) {
            revert CannotPauseMarketThatIsNotListed();
        }
        if (msg.sender != pauseGuardian && msg.sender != admin) {
            revert OnlyPauseGuardianAndAdminCanPause();
        }
...
abstract contract PriceOracle {
    ...
    mapping(CToken => bool) public isTokenOracleSupported;
