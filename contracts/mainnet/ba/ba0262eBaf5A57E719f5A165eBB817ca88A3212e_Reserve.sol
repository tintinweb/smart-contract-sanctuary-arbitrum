    ...
    LendVaultStorage 
{
    ...
    function initializeToken(address token, IRMData memory data) external restrictAccess(GOVERNOR) {
        require(data.optimalUtilizationRate<=PRECISION && data.optimalUtilizationRate>=0, "E25");
        require(data.baseBorrowRate<=PRECISION && data.baseBorrowRate>=0, "E26");
        require(data.slope1<PRECISION && data.slope1>=0, "E27");
        require(data.slope2<PRECISION && data.slope2>=0, "E28");
