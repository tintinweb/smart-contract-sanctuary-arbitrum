...
contract HegicStrategyInverseLongCondor is HegicInverseStrategy {
        ...
        returns (uint256 amount)
    {
        ...
        uint256 SELL_OTM_CALLStrike =
            ((ATMStrike * 110) / 100).round(roundedDecimals);
        uint256 SELL_OTM_PUTStrike =
            ((ATMStrike * 90) / 100).round(roundedDecimals);
        uint256 BUY_OTM_CALLStrike =
            ((ATMStrike * (100 + strikePercentage)) / 100).round(
                roundedDecimals
            );
        uint256 BUY_OTM_PUTStrike =
            ((ATMStrike * (100 - strikePercentage)) / 100).round(
                roundedDecimals
            );
