...
contract VestaEIR is CropJoinAdapter, IModuleInterest {
		...
		returns (uint256)
	{
		...
		uint256 crop = interestMinted - stock;
...
contract SavingModule is ISavingModule, OwnableUpgradeable, Shareable {
	...
	function _depositVST(uint256 _amount) internal {
		...
		uint256 minRewards = _getMaxRatePerSecond(_spSupply(), BPS) *
			secondDifference;
	...
	// _lockTime & _autoLock are no longer used
	function createLock(
		uint256 _lockTime,
		uint256 _amount,
		bool _autoLock
	...
	function _claim(uint256 _lockId, Lock storage _lock) internal {
		...
		IERC20(vst).transfer(msg.sender, rawInterest);
		...
		emit UserClaimed(msg.sender, _lockId, rawInterest);
	}
		...
		returns (uint256)
	{
		...
		return _getCurrentLockReward(_lockId);
	}

	function _getCurrentLockReward(uint256 _lockId)
		internal
		view
	...
	OwnableUpgradeable
{
	...
	mapping(address => mapping(uint128 => mapping(uint128 => uint256)))
		private epochToScaleToSumFixed;

	mapping(address => uint256) private lockSFixed;
		...
		returns (uint256 assetGainPerUnitStaked, uint256 vstLossPerUnitStaked)
	{
		...
		lastAssetError_Offset[_asset] =
			assetNumerator -
			(assetGainPerUnitStaked * _totalVSTDeposits);
	...
	function _updateRewardSumAndProduct(
		...
		uint256 currentS = epochToScaleToSumFixed[_asset][currentEpochCached][
			currentScaleCached
		];
		...
		epochToScaleToSumFixed[_asset][currentEpochCached][
			currentScaleCached
		] = newS;
	...
	function _getLockGainsFromSnapshot(
		...
		for (uint256 i = 0; i < totalAssets; ++i) {
			asset = assets[i];
			S_Snapshot = lockSFixed[asset];

			firstPortion =
				epochToScaleToSumFixed[asset][epochSnapshot][scaleSnapshot] -
				S_Snapshot;

			secondPortion =
				epochToScaleToSumFixed[asset][epochSnapshot][scaleSnapshot + 1] /
				SCALE_FACTOR;

			//prettier-ignore
			assetGains[i] =
                FullMath.mulDiv(_initialDeposit, firstPortion + secondPortion, P_Snapshot) / DECIMAL_PRECISION;
		}
		...
		internal
	{
		...
		for (uint256 i = 0; i < totalAsset; ++i) {
			asset = assets[i];
			currentS = epochToScaleToSumFixed[asset][currentEpoch][currentScale];
			lockSFixed[asset] = currentS;
		}
	...
	// There has been an issue with the reward function, so we have to drain it and manually give the rewards
	function drainReward(address _receiver) external onlyOwner {
		address asset;
		for (uint256 i = 0; i < assets.length; ++i) {
			...
			if (asset == address(0)) {
				...
				IERC20(asset).transfer(_receiver, balance);
			}
		}
	}
