...
contract ArbitHeroes is ERC20, Ownable {
 using SafeMath for uint256;
 ...
 constructor() ERC20("ARBITHEROES", "ABH") {
 
 router = IUniswapV2Router02(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
 uniswapV2Pair = IUniswapV2Factory(router.factory()).createPair(address(this), router.WETH());

 //Wallets
 devWallet = address(0x22Fc5c2171c9910b4A4b67AdD6C4Ffca831D691D);
 marketingWallet = address(0x22Fc5c2171c9910b4A4b67AdD6C4Ffca831D691D);
...
//Limits
 maxBuyAmount = totalSupply * 1 / 1;
 maxSellAmount = totalSupply * 5 / 5;
 maxWalletAmount = totalSupply * 5 / 5;
 ...
 require(((totalSupply() * newMaxBuy) / 1000) >= (totalSupply() / 10), "maxBuyAmount must be higher than 10%");
 require(((totalSupply() * newMaxSell) / 1000) >= (totalSupply() / 10), "maxSellAmount must be higher than 10%");
 maxBuyAmount = (totalSupply() * newMaxBuy) / 1000;
 maxSellAmount = (totalSupply() * newMaxSell) / 1000;
 }

 function updateMaxWalletAmount(uint256 newPercentage) external onlyOwner {
 require(((totalSupply() * newPercentage) / 1000) >= (totalSupply() / 10), "Cannot set maxWallet lower than 10%");
 ...
 _fees.sellTotalFees = _fees.sellMarketingFee + _fees.sellLiquidityFee + _fees.sellDevFee;
 require(_fees.buyTotalFees <= 10, "Must keep fees at 40% or less"); 
 require(_fees.sellTotalFees <= 10, "Must keep fees at 40% or less");
