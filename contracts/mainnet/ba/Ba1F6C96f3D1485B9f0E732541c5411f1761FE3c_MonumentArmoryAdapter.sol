  ...
  IDurabilityEnabledAdapter
{
  ...
  bool public SUPPORTS_DURABILITY;
  ...
  function _damagedEntityCount(
    ...
    return
      currentDurabilityStorage[_ownerAddress][_ownerTokenId][_entityAddress][
        _entityTokenId
      ] > 0
        ? 1
        : 0;
  ...
  function currentDurabilityBatch(
    ...
    mapping(uint => uint32) storage durabilities = currentDurabilityStorage[
      _ownerAddress
    ][_ownerId][_entityAddress];
  ...
  function currentDurability(
    ...
    return
      uint(
        currentDurabilityStorage[_ownerAddress][_ownerId][_entityAddress][
          _entityId
        ]
      );
  }
  ...
  function currentDurabilityPercentage(
    ...
    if (
      result == 0 &&
      _isNextAvailable(_ownerAddress, _ownerId, _entityAddress, _entityId)
    ) {
      result = ONE_HUNDRED;
    }
  }
  ...
  function currentDurabilityBatchPercentage(
    ...
    for (uint i = 0; i < _entityIds.length; i++) {
      ...
      if (
        result[i] == 0 &&
        _isNextAvailable(_ownerAddress, _ownerId, _entityAddress, _entityIds[i])
      ) {
  ...
  function reduceDurability(
    ...
    mapping(uint => uint32)
      storage _ownerEntityTokensDurability = currentDurabilityStorage[
        _ownerAddress
  ...
  function reduceDurabilityBatch(
    ...
    bool _startNewTokenIfNeeded,
    bool _ignoreAvailability
  ) external onlyManager returns (bool, uint[] memory) {
    if (!SUPPORTS_DURABILITY) {
      revert DurabilityNotSupported();
    }
    ...
    for (uint i = 0; i < _entityTokenIds.length; i++) {
      ...
      if (
        _reduceDurabilityInternal(_ownerEntityTokensDurability, mem.request)
      ) {
        mem.anyBroken = true;
    ...
    return (mem.anyBroken, mem.brokenAmounts);
  }
  ...
  function reduceDurabilityBatch(
    ...
    bool _startNewTokenIfNeeded,
    bool _ignoreAvailability
  ) external onlyManager returns (bool anyBroken, uint[] memory brokenAmounts) {
    if (!SUPPORTS_DURABILITY) {
      revert DurabilityNotSupported();
    }

    anyBroken = false;
    brokenAmounts = new uint[](_entityTokenIds.length);
    mapping(uint => uint32)
      storage _ownerEntityTokensDurability = currentDurabilityStorage[
        _ownerAddress
      ][_ownerTokenId][_entityAddress];
    ...
    return (anyBroken, brokenAmounts);
  }
  ...
  function reduceDurabilityBatch(
    ...
    for (uint i = 0; i < _entityTokenIds.length; i++) {
      ...
      _nestedReduceDurabilityInternal(mem, _entityTokenIds, i);
    }
    ...
    return (mem.anyBroken, mem.nestedBrokenAmounts);
  }
  ...
  function reduceDurabilityBatch(
    ...
    for (uint i = 0; i < _entityTokenIds.length; i++) {
      mem.request._ownerAddress = _ownerAddresses[i];
      mem.request._ownerTokenId = _ownerTokenIds[i];
      _nestedReduceDurabilityInternal(
        mem,
        _entityTokenIds,
        _durabilityLosses,
        i
  ...
  function _nestedReduceDurabilityInternal(
    ProcessingMemory memory mem,
    uint[][] calldata _entityTokenIds,
    uint i
  ) private {
    mapping(uint => uint32)
      storage _ownerEntityTokensDurability = currentDurabilityStorage[
        mem.request._ownerAddress
      ][mem.request._ownerTokenId][mem.request._entityAddress];
    ...
    if (durability == 0) {
      if (_request.startNewTokenIfUndamaged) {
        if (
          ...
          durability = _maximumTokenDurability(
            _request._entityAddress,
            _request._entityTokenId
          );
  ...
  function updateBreakHandler(address _handler) external onlyAdmin {
    SUPPORTS_DURABILITY = (_handler != address(0));
  ...
  AbstractDurabilityEnabledArmoryAdapter
{
  ...
  function stakeBatch(
    ...
    mapping(uint256 => uint256) storage memoryBalances = stakerBalances[
      _ownerAddress
    ][_ownerId];
    for (uint i = 0; i < _entityTokenIds.length; i++) {
  ...
  function stakeBatch(
    ...
    for (uint i = 0; i < _entityTokenIds.length; i++) {
      mapping(uint256 => uint256) storage balance = stakerBalances[
        _ownerAddress[i]
      ][_ownerId[i]];
      for (uint j = 0; j < _entityTokenIds[i].length; j++) {
  ...
  function unstake(
    ...
    _checkAndDecreaseAmountInternal(
      _ownerAddress,
      _ownerId,
      _entityTokenId,
      _entityAmount
  ...
  function mintBatch(
    ...
    mapping(uint256 => uint256) storage ownedEntities = stakerBalances[
      _ownerAddress
    ][_ownerId];
    for (uint i = 0; i < _entityTokenIds.length; i++) {
  ...
  function _checkAndDecreaseAmountsInternal(
    ...
    mapping(uint256 => uint32)
      storage ownedEntityDurabilities = _ownedEntityDurabilityMapping(
        _ownerAddress,
        _ownerId,
        COLLECTION_ADDRESS
      );
...
import "../Monument/Monument.sol";

contract MonumentArmoryAdapter is BurnMintERC1155ArmoryAdapter {
  Monument public MONUMENT;
  ...
  constructor(
    ...
    BurnMintERC1155ArmoryAdapter(
      ...
      _actionPermit,
      _collectionAddress,
      ACTION_ARMORY_STAKE_MONUMENT,
      ACTION_ARMORY_UNSTAKE_MONUMENT,
      true,
      _breakHandler
    )
  {
    MONUMENT = Monument(_collectionAddress);
  }
  ...
  function _burnFrom(
    ...
    MONUMENT.safeTransferFrom(_from, address(this), _id, _amount, "");
    MONUMENT.burn(_id, _amount);
  }
  ...
  function _burnBatchFrom(
    ...
    MONUMENT.safeBatchTransferFrom(_from, address(this), _ids, _amounts, "");
    MONUMENT.burnBatch(_ids, _amounts);
  }
  ...
  function _burnBatchFrom(
    ...
    for (uint i = 0; i < _ids.length; i++) {
      MONUMENT.safeBatchTransferFrom(
        _from,
        address(this),
        _ids[i],
        _amounts[i],
      ...
      MONUMENT.burnBatch(_ids[i], _amounts[i]);
    }
  }
  ...
  function _mintFor(
    ...
    MONUMENT.mintFor(_for, _id, _amount);
  }
  ...
  function _mintBatchFor(
    ...
    MONUMENT.mintBatchFor(_for, _ids, _amounts);
  }
  ...
  function _mintBatchFor(
    ...
    for (uint i = 0; i < _ids.length; i++) {
      MONUMENT.mintBatchFor(_for, _ids[i], _amounts[i]);
    }
  }
}