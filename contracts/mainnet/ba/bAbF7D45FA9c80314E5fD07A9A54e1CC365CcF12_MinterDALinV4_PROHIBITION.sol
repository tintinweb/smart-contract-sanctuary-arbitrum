 ...
 */
contract MinterDALinV4_PROHIBITION is ReentrancyGuard, MinterBase, IFilteredMinterDALinV1_PROHIBITION {
    ...
    function _onlyCoreAdminACL(bytes4 _selector) internal view {
        require(
            genArtCoreContract_Base.adminACLAllowed(msg.sender, address(this), _selector), "Only Core AdminACL allowed"
        );
    }
    ...
    function projectAuctionParameters(uint256 _projectId)
        external
        view
        returns (uint256 timestampStart, uint256 timestampEnd, uint256 startPrice, uint256 basePrice)
    {
     ...
     */
    function setMinimumAuctionLengthSeconds(uint256 _minimumAuctionLengthSeconds) external {
    ...
    function setAuctionDetails(
        ...
        _onlyArtistOrAdminACL(_projectId, this.setAuctionDetails.selector);
        // CHECKS
        ProjectConfig storage _projectConfig = projectConfig[_projectId];
        require(
            _projectConfig.timestampStart == 0 || block.timestamp < _projectConfig.timestampStart,
            "No modifications mid-auction"
        );
        require(block.timestamp < _auctionTimestampStart, "Only future auctions");
        require(_auctionTimestampEnd > _auctionTimestampStart, "Auction end must be greater than auction start");
        require(
            _auctionTimestampEnd >= _auctionTimestampStart + minimumAuctionLengthSeconds,
            "Auction length must be at least minimumAuctionLengthSeconds"
        );
        require(_startPrice > _basePrice, "Auction start price must be greater than auction end price");
        // EFFECTS
        ...
        emit SetAuctionDetails(_projectId, _auctionTimestampStart, _auctionTimestampEnd, _startPrice, _basePrice);
