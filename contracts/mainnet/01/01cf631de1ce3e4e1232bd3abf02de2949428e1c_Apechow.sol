...
contract Apechow is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 Apechow-WAVAX LP -> Apechow pool
    // uint256 public constant Apechow = 0 ether;
    ...
    // Address of the Apechow-WAVAX LP
    ...
    /**
     * @notice Constructs the APECHOW ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress, uint256 _sellStartTime) public ERC20("Apechow Token", "APECHOW") {
        // Mints 1 APECHOW to contract creator for initial pool setup
    ...
    function _getApechowPrice() internal view returns (uint256 _apechowPrice) {
        try IOracle(apechowOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Apechow: failed to fetch APECHOW price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _apechowPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_apechowPrice >= taxTiersTwaps[tierId]) {
    ...
    function setApechowOracle(address _apechowOracle) public onlyOperatorOrTaxOffice {
        require(_apechowOracle != address(0), "oracle address cannot be 0 address");
        apechowOracle = _apechowOracle;
    }
    ...
    /**
     * @notice Operator mints Apechow to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of Apechow to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentApechowPrice = _getApechowPrice();
            currentTaxRate = _updateTaxRate(currentApechowPrice);
            if (currentApechowPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // address _apechowPool
        // address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        // require(_apechowPool != address(0), "!_apechowPool");
        // require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_apechowPool, INITIAL_APECHOW_POOL_DISTRIBUTION);
    ...
    function getRemainingTimeForApechowSellingEnable() public view returns(uint256) {
        ...
        /// @dev overrides transfer function to meet tokenomics of Apechow
