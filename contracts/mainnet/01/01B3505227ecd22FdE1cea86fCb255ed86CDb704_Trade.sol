...
contract Trade is ITrade, TradeStorage, ReentrancyGuardUpgradeable {
    ...
    function transferOwnership(address to) external onlyOwner {
        ...
        uint256[] memory fundIds = new uint256[](1);
        fundIds[0] = _fundId;
        registry.fees().withdrawAll(fundIds, _owner);
        _owner = to;
    ...
    function setState(FundState newState) external override {
        require(
            msg.sender == registry.triggerServer() && (newState == FundState.Blocked || state == FundState.Blocked && newState == FundState.Opened)
    ...
    function lockTrade(address manager, uint256 duringTime) external override {
        require(
            isOwner(msg.sender)
            || msg.sender == address(registry.triggerServer())
    ...
    function setManager(address manager) external override {
        require(
            ...
            || msg.sender == address(registry.dripOperator())
            || (manager == _owner && msg.sender == registry.triggerServer()),
            "T/AD"
        ); // access denied
        if (managers[manager]) {
