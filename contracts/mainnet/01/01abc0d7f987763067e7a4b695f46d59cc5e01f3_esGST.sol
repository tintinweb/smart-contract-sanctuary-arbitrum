...
contract esGST is ERC20("esGST", "esGST"), Ownable, ReentrancyGuard{ 
    ...
    IERC20 public GST = IERC20(0x564B1C0971bB7dC0075C831b97E4117BB186303E);
    ...
    function claimableTokens(address _address) external view returns(uint256) {
        ...
        if (timePass >= userInfo[msg.sender].VestPeriod){
            ...
            claimable = userInfo[_address].totalVested.mul(timePass).div(userInfo[_address].VestPeriod);
        }
        return claimable;
    }
    ...
    function vest(uint256 _amount) external nonReentrant {
        ...
        require(this.balanceOf(msg.sender) >= _amount, "esGST balance too low");
        uint256 _amountin = _amount;
        uint256 amountOut = _amountin;
        
        userInfo[msg.sender].totalVested = userInfo[msg.sender].totalVested.add(amountOut);
    ...
    function lock(uint256 _amount) external nonReentrant {
        ...
        require(GST.balanceOf(msg.sender) >= _amount, "GST balance too low");
        uint256 amountOut = _amount;
        _mint(msg.sender, amountOut);
        GST.safeTransferFrom(msg.sender, address(this), _amount);
    }
    ...
    function claim() external nonReentrant {
        ...
        if (timePass >= userInfo[msg.sender].VestPeriod){
            ...
            claimable = userInfo[msg.sender].totalVested.mul(timePass).div(userInfo[msg.sender].VestPeriod);
            userInfo[msg.sender].VestPeriod = userInfo[msg.sender].VestPeriod.sub(timePass);
            
        }
        userInfo[msg.sender].totalVested = userInfo[msg.sender].totalVested.sub(claimable);
        userInfo[msg.sender].lastInteractionTime = block.timestamp;
