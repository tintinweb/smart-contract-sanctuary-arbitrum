 ...
 * Delta Neutral GLP farming strategy
    ...
    - borrow assets on Aave:
      ...
      -balance: 50U in GLP gives 25U-E, borrow 50U-E needs 70U as collat -> 120
        so out of 100 USDC, put 42 in GLP, 58 in Aave, borrow $42 worth of ETH in Aave and put in GLP too
 */
contract KamehaGlpUsd is ERC20, Ownable {
  ...
  /// EVENTS
  event Deposit(address user, uint usdcAmount, uint glpAmount);
  event Withdraw(address user, uint kamGlpAmount, uint usdcAmount);
  ...
  function deposit(uint usdcAmount) public {
    ...
    _mintDeltaNeutralGLP(usdcAmount, true);
  }  
  ...
  function _mintDeltaNeutralGLP (uint usdcAmount, bool mintTokens) private {
    require(isPaused == false, "Deposits paused");
    
    uint glpBalance = STAKED_GLP_TRACKER.balanceOf(address(this));
    // 1. mint GLP with 40% USDC 
    uint glpAmount = GMX_REWARD_ROUTER.mintAndStakeGlp(USDC, usdcAmount * 40 / 100, 0, 1);

    // 2. deposit remaing 58% in Aave
    AAVE_LP.deposit(USDC, usdcAmount * 60 / 100, address(this), 0);
    
    // 2. borrow usdcAmount*40% worth of ETH+WBTC (Aave LTV 80%, 40/60 ~ 66%), ETH:BTC ratio ~ 2:1, ETH=27% BTC =13%
    // ethamount * ethPrice / ethDecimals = usdcAmount * usdcPrice / usdcDecimals
    uint wethAmount = (usdcAmount * 27 / 100) * latestUsdcPrice() * 10**12 / latestEthPrice(); // USDC 6 to WETH 18 decimals
    uint wbtcAmount = (usdcAmount * 13 / 100) * latestUsdcPrice() * 10**2  / latestWbtcPrice();  // USDC 6 to WBTC 8 decimals
    ...
    if (mintTokens) {
      uint mintAmount = totalSupply() * glpAmount / glpBalance;
      _mint(msg.sender, mintAmount);
      emit Deposit(msg.sender, usdcAmount, mintAmount);
    }
  }
  ...
  function withdraw(uint kamGlpAmount) public {
    require (kamGlpAmount > 0, "Invalid Amount");
    // 0. Dont compound rewards or will reset cooldown
    //harvestAndCompound();
    uint glpBalance = STAKED_GLP_TRACKER.balanceOf(address(this));
    uint kamGlpSupply = totalSupply();
    
    // 1. Withdraw as ETH
    uint redeemedWETH = GMX_REWARD_ROUTER.unstakeAndRedeemGlp(WETH, glpBalance * kamGlpAmount / kamGlpSupply, 1, address(this) );
    
    // 2. Repay Aave ETH+WBTC debt in proportion
    uint debt = ERC20(VARDEBT_WETH).balanceOf(address(this));
    AAVE_LP.repay(WETH, debt * kamGlpAmount / kamGlpSupply, 2, address(this));
    
    // wBTC debt
    debt = ERC20(VARDEBT_WBTC).balanceOf(address(this)) * kamGlpAmount / kamGlpSupply;
    ...
    AAVE_LP.withdraw(USDC, usdcAave * kamGlpAmount / kamGlpSupply, msg.sender);
        
    // 4. There should be an excess WETH: convert, send all USDC remaining to user
    _swapAllWethToUsdc();
    uint uOut = ERC20(USDC).balanceOf(address(this));
    ERC20(USDC).transfer(msg.sender, uOut );
    
    // 5. burn
    _burn(msg.sender, kamGlpAmount);
    emit Withdraw(msg.sender, kamGlpAmount, usdcAave * kamGlpAmount / kamGlpSupply + uOut);
  }
  ...
  */
  ...
  function _swapAllWethToUsdc() private returns (uint amountOut){
    ...
    ISwapRouter.ExactInputSingleParams memory params =
      ISwapRouter.ExactInputSingleParams({
          ...
          amountIn: amountIn,
          amountOutMinimum: amountIn * latestEthPrice() / latestWbtcPrice() * 99 / 100, // tolerable slippage 5%
          sqrtPriceLimitX96: 0
      });
  ...
  function _swapEthForExactWbtc(uint amountOut) internal returns (uint amountIn){
    ISwapRouter.ExactOutputSingleParams memory params =
      ISwapRouter.ExactOutputSingleParams({
          ...
          deadline: block.timestamp,
          amountOut: amountOut,
          amountInMaximum:  amountOut * latestWbtcPrice() / latestEthPrice() * 101 / 100, // tolerable slippage 1
          sqrtPriceLimitX96: 0
      });
  ...
  function _swapExactEthForWbtc(uint amountIn) internal returns (uint amountOut){
    ISwapRouter.ExactInputSingleParams memory params =
      ISwapRouter.ExactInputSingleParams({
          ...
          deadline: block.timestamp,
          amountIn: amountIn,
          amountOutMinimum: amountIn * latestEthPrice() / latestWbtcPrice() * 99 / 100, // tolerable slippage 1%
          sqrtPriceLimitX96: 0
      });
    amountOut = SWAP_ROUTER.exactInputSingle(params);
  }
  ...
  function latestKamGlpPrice() public view returns (uint priceX8){
    uint glpBalance = STAKED_GLP_TRACKER.balanceOf(address(this));
