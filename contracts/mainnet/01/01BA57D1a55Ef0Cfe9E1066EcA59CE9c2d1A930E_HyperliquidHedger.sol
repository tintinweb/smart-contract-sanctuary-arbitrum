    ...
    AccessControlUpgradeable
{
    ...
    address public hyperliquidAccount;

    address public collateralAsset;

    uint256 public collateralWithdraw;
    bool public isHedging;

    mapping(address => int256) deltaCached;
    mapping(address => int256) public tempDeltas;
    ...
    /// @param _hyperliquidAccount Address of the hyperliquid account
    /// @param _collateralAsset collateral asset for trades
    function __HyperliquidHedger_init(
        address _hedgedPool,
        address _hyperliquidAccount,
    ...
    function _updateConfig(
        ...
        hyperliquidAccount = _hyperliquidAccount;
    ...
    function hedge() external onlyAuthorized returns (int256 deltaDiff) {

        if(!isHedging) {
            revert("Hedge is not initiated yet.");
        }

        if(collateralWithdraw > 0 && IERC20(collateralAsset).balanceOf(address(this)) == 0 ){
            revert("Check the hyperqliquid trade to make sure there are no errors hedging in progress");
        }

        address[] memory hedgedPoolUnderlyings = IHedgedPool(hedgedPool).getAllUnderlyings();

        for (uint256 i = 0; i < hedgedPoolUnderlyings.length; i++) {
            deltaCached[hedgedPoolUnderlyings[i]] = tempDeltas[hedgedPoolUnderlyings[i]];
            tempDeltas[hedgedPoolUnderlyings[i]] = 0;
        }
    ...
    function getDelta(address underlyingAddress ) external view returns (int256) {
       return deltaCached[underlyingAddress];
    }
    ...
    function resetHedger() external onlyOwner {
        isHedging = false;
        collateralWithdraw = 0;
        address[] memory hedgedPoolUnderlyings = IHedgedPool(hedgedPool).getAllUnderlyings();
        for (uint256 i = 0; i < hedgedPoolUnderlyings.length; i++) {
    ...
    function deposit(uint256 _depositAmount) external onlyAuthorized() {
    ...
    function updateDeltas(address[] memory _underlyings, int256[] memory _deltas) external onlyAuthorized() {
        for (uint256 i = 0; i < _underlyings.length; i++) {
            deltaCached[_underlyings[i]] = _deltas[i];
        }
    }

    /// @notice setCollateralValue()
    /// Talk to lukas ask if this is a function we need
