...
import { IFailureNotification } from '../../interfaces/IFailureNotification.sol';
import { IGateway } from '../interfaces/IGateway.sol';
import { IGatewayClient } from '../interfaces/IGatewayClient.sol';
 ...
 */
contract LayerZeroV2Gateway is SystemVersionId, GatewayBase, OAppMixin, TargetAppGasMixin {
    ...
    /**
     * @dev The address of the estimate service
     */
    address public estimateService;
    ...
    /**
     * @notice Emitted when the address of the estimate service is set
     * @param estimateService The address of the estimate service
     */
    event SetEstimateService(address indexed estimateService);
    ...
    /**
     * @notice Sets the address of the estimate service
     * @param _estimateService The address of the estimate service
     */
    function setEstimateService(address _estimateService) external onlyManager {
        estimateService = _estimateService;

        emit SetEstimateService(_estimateService);
    }
    ...
    function allowInitializePath(
        Origin calldata _origin
    ) external view override(OAppMixin) returns (bool) {
    ...
    function _isLzReceiveCaller() internal view override(OAppMixin) returns (bool) {
        return super._isLzReceiveCaller() || msg.sender == estimateService;
    }
    ...
    function _onLzReceiveFailure() private {
        if (
            msg.sender == estimateService &&
            estimateService.supportsInterface(type(IFailureNotification).interfaceId)
        ) {
            IFailureNotification(estimateService).notifyFailure();
        }
    }
 ...
 */
abstract contract OAppMixin is ManagerRole, ReentrancyGuard, ILayerZeroReceiver {
    /**
    ...
    /**
     * @dev See MAX_COUNT at UlnBase.sol (LayerZero V2 messagelib)
     */
    uint8 internal constant MAX_DVN_COUNT = (type(uint8).max - 1) / 2; // 127
    ...
    /**
     * @notice Emitted when the calling account is not an allowed caller
     */
    error OnlyLzReceiveCallerError();
     ...
     */
    modifier onlyLzReceiveCaller() {
        if (!_isLzReceiveCaller()) {
            revert OnlyLzReceiveCallerError();
        }
    ...
    function lzReceive(
        ...
        address _executor,
        bytes calldata _extraData
    ) external payable virtual override(ILayerZeroReceiver) nonReentrant onlyLzReceiveCaller {
     ...
     */
    function setEndpoint(address _endpointAddress) external virtual onlyManager {
        _setEndpoint(_endpointAddress);
    }
     ...
     */
    function setDelegate(address _delegate) external virtual onlyManager {
        _setDelegate(_delegate);
    }
     ...
     */
    function setDVNConfig(DVNConfigItem[] calldata _dvnConfig) external virtual onlyManager {
        _setDVNConfig(_dvnConfig);
    }
    ...
    function _setDVNConfig(DVNConfigItem[] calldata _dvnConfig) internal virtual {
    ...
    function _isLzReceiveCaller() internal view virtual returns (bool) {
        return msg.sender == address(endpoint);
    }
    ...
    function _dvnCount(uint256 _dvnArrayLength) private pure returns (uint8) {
        if (_dvnArrayLength == 0) {
            ...
            if (_dvnArrayLength > MAX_DVN_COUNT) {
...
/**
 * @title IFailureNotification
 * @notice Failure notification interface
 */
...
abstract contract SystemVersionId {
    ...
    uint256 public constant SYSTEM_VERSION_ID =
        uint256(keccak256('LZV2GatewayCheck-2024-06-12-Test-B')); // TODO
}