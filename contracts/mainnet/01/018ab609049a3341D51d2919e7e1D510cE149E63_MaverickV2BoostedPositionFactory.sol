 ...
 */
abstract contract BoostedPositionBase is ERC20, Multicall, Checks, IBoostedPositionBase {
    using Cast for uint256;
    ...
    function _checkAndUpdateBinBalances(
        uint32[] memory binIds_,
        uint128[] memory ratios_
    ) internal returns (uint128 deltaSupply) {
        uint128[] memory binBalances_ = binBalances;
        uint128[] memory newBinBalances = new uint128[](binIds_.length);
        uint128[] memory diffs = new uint128[](binIds_.length);
        uint256 totalSupply_ = totalSupply();
        ...
        for (uint256 i = 0; i < binIds_.length; i++) {
            newBinBalances[i] = pool.balanceOf(address(this), SUBACCOUNT, binIds_[i]);
            diffs[i] = newBinBalances[i] - binBalances_[i];
            if (i != 0) {
                uint128 required = Math.mulCeil(diffs[0], ratios_[i]).toUint128();
                if (required > diffs[i])
        ...
        binBalances = newBinBalances;
        deltaSupply = totalSupply_ == 0
            ? diffs[0]
            : Math.mulDivFloor(diffs[0], totalSupply(), binBalances_[0]).toUint128();
    }
}
...
contract MaverickV2BoostedPositionDynamic is IMaverickV2BoostedPosition, BoostedPositionBase {
    ...
    constructor(
        ...
        (binId, tokenAScale, tokenBScale) = (binId_, tokenAScale_, tokenBScale_);
    }
    ...
    modifier checkBinIsRoot() {
        if (pool.getBin(binId).mergeId != 0) revert BoostedPositionMovementBinNotMigrated();
        _;
    }
    ...
    function getTicks() public view checkBinIsRoot returns (int32[] memory ticks) {
        ticks = new int32[](1);
        ticks[0] = pool.getBin(binId).tick;
    }
