...
import {NoAccessFacet, LengthNotMatch, CannotBridgeToSameNetwork, MoreAmountThanFee, RegisterFirst, ReceiveMessageFailed, OnlyContractOwner} from "../Errors/GenericErrors.sol";
import "../interfaces/IBridge.sol";
import {ITokenMessenger, IMessageTransmitter} from "../interfaces/ICctp.sol";
...
contract CctpFacet is IBridge, Signers, VerifySigEIP712, ReentrancyGuard {
    using SafeERC20 for IERC20;

    ITokenMessenger private immutable TokenMessenger;
    IMessageTransmitter private immutable MessageTransmitter;

    bytes32 internal constant NAMESPACE = keccak256("com.plexus.facet.cctp");

    struct CctpStruct {
        mapping(uint64 => uint32) cctpChainMapping;
        mapping(uint64 => uint256) dstTxFee;
        address feeReceiver;
    }

    constructor(ITokenMessenger _TokenMessenger, IMessageTransmitter _MessageTransmitter) {
        TokenMessenger = _TokenMessenger;
        MessageTransmitter = _MessageTransmitter;
    }

    function setChainId(uint64[] memory _originId, uint32[] memory _cctpChainId) external {
        if (msg.sender != LibDiamond.contractOwner()) revert OnlyContractOwner();
        CctpStruct storage s = getStorage();
        for (uint i; i < _originId.length; i++) {
            s.cctpChainMapping[_originId[i]] = _cctpChainId[i];
        }
    }

    function getChainId(uint64 _originId) public view returns (uint32) {
        CctpStruct storage s = getStorage();
        return s.cctpChainMapping[_originId];
    }

    function setTxFee(uint64[] calldata _toChainIds, uint256[] calldata _fees) external {
        if (msg.sender != LibDiamond.contractOwner()) revert OnlyContractOwner();
        if (_toChainIds.length != _fees.length) revert LengthNotMatch();
        CctpStruct storage s = getStorage();
        for (uint256 i = 0; i < _toChainIds.length; i++) {
            if (block.chainid == _toChainIds[i]) revert CannotBridgeToSameNetwork();
            s.dstTxFee[_toChainIds[i]] = _fees[i];
        }
    }

    function getTxFee(uint64 _toChainId) public view returns (uint256) {
        if (block.chainid == _toChainId) revert CannotBridgeToSameNetwork();
        CctpStruct storage s = getStorage();
        if (s.dstTxFee[_toChainId] == 0) revert RegisterFirst();
        return s.dstTxFee[_toChainId];
    }

    function setCctpFeeReceiver(address _receiver) external {
        if (msg.sender != LibDiamond.contractOwner()) revert OnlyContractOwner();
        CctpStruct storage s = getStorage();
        s.feeReceiver = _receiver;
    }

    function getCctpFeeReceiver() public view returns (address) {
        CctpStruct storage s = getStorage();
        return s.feeReceiver;
    }

    function _convertAddr(address _input) internal pure returns (bytes32) {
        return bytes32(uint256(uint160(_input)));
    }

    //fromChain execute
    function bridgeToCctp(BridgeData memory _bridgeData) external payable nonReentrant {
        LibPlexusUtil._isTokenDeposit(_bridgeData.srcToken, _bridgeData.amount);
        _CctpStart(_bridgeData);
    }

    function swapAndBridgeToCctp(SwapData calldata _swap, BridgeData memory _bridgeData) external payable nonReentrant {
    ...
    function _CctpStart(BridgeData memory _bridgeData) internal {
        uint256 fee = getTxFee(_bridgeData.dstChainId);
        if (_bridgeData.amount <= fee) revert MoreAmountThanFee();
        uint256 bridgeAmt = _bridgeData.amount - fee;
        IERC20(_bridgeData.srcToken).safeTransfer(getCctpFeeReceiver(), fee);
        bool isNotNative = !LibPlexusUtil._isNative(_bridgeData.srcToken);
        if (isNotNative) {
            IERC20(_bridgeData.srcToken).safeApprove(address(TokenMessenger), bridgeAmt);
            TokenMessenger.depositForBurn(
                bridgeAmt,
                getChainId(uint64(_bridgeData.dstChainId)),
                _convertAddr(_bridgeData.recipient),
                _bridgeData.srcToken
            );
            IERC20(_bridgeData.srcToken).safeApprove(address(TokenMessenger), 0);
        } else {
            revert NoAccessFacet();
        }
        emit LibData.Bridge(msg.sender, _bridgeData.dstChainId, _bridgeData.srcToken, bridgeAmt, _bridgeData.plexusData, "CCTP");
    }
    ...
    function receiveMessage(
        ...
        address receiveUserAddr,
        address toChainUSDC,
        uint256 toChainReceiveAmount,
        bytes calldata _plexusData
    ) external {
        if (msg.sender != LibDiamond.contractOwner()) revert OnlyContractOwner();
        bool success = MessageTransmitter.receiveMessage(message, signature);
        if (!success) revert ReceiveMessageFailed();

        OutputToken[] memory output = new OutputToken[](1);
        output[0] = OutputToken(toChainUSDC);

        uint256[] memory returnAmount = new uint256[](1);
        returnAmount[0] = toChainReceiveAmount;

        emit LibData.Relayswap(receiveUserAddr, output, returnAmount, new DupToken[](0), _plexusData);
    }

    function receiveMessageAndRelayswap(
        ...
        bool success = MessageTransmitter.receiveMessage(cctpMessage, cctpSignature);
        if (!success) revert ReceiveMessageFailed();
        ...
        if (!LibPlexusUtil._isNative(_relay.feeTokenAddress)) {
        ...
        uint256[] memory dstAmount = LibPlexusUtil._relaySwapStart(swap);

        for (uint256 i = 0; i < swap.output.length; i++) {
            if (!LibPlexusUtil._isNative(swap.output[i].dstToken)) {
    ...
    function getStorage() private pure returns (CctpStruct storage s) {
...
contract VerifySigEIP712 is EIP712("Plexus", "1") {
    ...
    function relaySig(Input memory inputData, bytes[] memory signature) public view returns (bool) {
        ...
        require(!ds.txHashCheck[inputData.txHash], "already passed");
