...
contract Amm is IAmm, OwnableUpgradeable, BlockContext {
    ...
    Decimal.decimal private _partialLiquidationRatio;
     ...
     */
    function setInitMarginRatio(Decimal.decimal memory initMarginRatio_) public onlyOwner {
        _requireValidRatio(initMarginRatio_);
        _initMarginRatio = initMarginRatio_;
        emit InitMarginRatioChanged(initMarginRatio_.toUint());
    }
    ...
    function setMaintenanceMarginRatio(Decimal.decimal memory maintenanceMarginRatio_)
        ...
        onlyOwner
    {
        _requireValidRatio(maintenanceMarginRatio_);
        _maintenanceMarginRatio = maintenanceMarginRatio_;
        emit MaintenanceMarginRatioChanged(maintenanceMarginRatio_.toUint());
    }
     ...
     * @dev only owner can call
     */
    function setPartialLiquidationRatio(Decimal.decimal memory partialLiquidationRatio_)
        public
        onlyOwner
    {
        _requireValidRatio(partialLiquidationRatio_);
        _partialLiquidationRatio = partialLiquidationRatio_;
        emit PartialLiquidationRatioChanged(partialLiquidationRatio_.toUint());
    }
     ...
     * @dev only owner can call
     */
    function setLiquidationFeeRatio(Decimal.decimal memory liquidationFeeRatio_) public onlyOwner {
        _requireValidRatio(liquidationFeeRatio_);
        _liquidationFeeRatio = liquidationFeeRatio_;
        emit LiquidationFeeRatioChanged(liquidationFeeRatio_.toUint());
    }

    /**
     * as a workaround for not being able to set so many vars in constructor
     * @dev only owner can call
     */
    function setRatios(Ratios memory _ratios) external onlyOwner {
        setInitMarginRatio(_ratios.initMarginRatio);
        setMaintenanceMarginRatio(_ratios.maintenanceMarginRatio);
        setPartialLiquidationRatio(_ratios.partialLiquidationRatio);
        setLiquidationFeeRatio(_ratios.liquidationFeeRatio);
    }
    ...
    function _requireValidRatio(Decimal.decimal memory _ratio) private pure {
...
contract ClearingHouse is TransferHelper, OwnerPausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function liquidateWithSlippage(
        ...
        Decimal.decimal memory quoteAssetAmountLimit = isPartialClose
            ? _quoteAssetAmountLimit.mulD(ratios.partialLiquidationRatio)
            : _quoteAssetAmountLimit;
    ...
    modifier onlyRepegBotOrOwner() {
        require(repegBot == _msgSender() || repegBot == owner(), "caller is not repegBot or owner");
        _;
    }
    ...
    function repegAmmY(IAmm amm, Decimal.decimal memory _quoteAssetReserve) public onlyRepegBot {
    ...
    function repegAmmK(IAmm amm, Decimal.decimal memory k) public onlyOwner {
