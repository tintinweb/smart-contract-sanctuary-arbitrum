    ...
    IStrategyVault
{
    ...
    function _withdraw(uint256 _shares) internal virtual {
        ...
        uint fee = Math.min(_calculateFee(getPricePerFullShareOptimized())+withdrawalFee, maxFee);
        uint sharesAsFee = (_shares * fee) / PRECISION;
        ...
        uint vaultBalance = balanceOptimized();
        uint256 tokensToWithdraw = vaultBalance * _shares_calc / totalSupply();
        _burn(msg.sender, _shares);
        _mint(provider.governance(), governanceShares);
        
        uint256 balanceDepositToken = IERC20(depositToken).balanceOf(address(this));
        if (balanceDepositToken < tokensToWithdraw) {
            uint extraTokensNeeded = tokensToWithdraw - balanceDepositToken;
            uint amountReceived = _withdrawFromStrategies(extraTokensNeeded);
            tokensToWithdraw = Math.min(amountReceived + balanceDepositToken, tokensToWithdraw);
        }
        IERC20(depositToken).safeTransfer(msg.sender, tokensToWithdraw * userShares / Math.max(1, _shares_calc));
        IERC20(depositToken).safeTransfer(provider.reserve(), tokensToWithdraw * reserveShares / Math.max(1, _shares_calc));
        emit Withdraw(
            msg.sender,
            tokensToWithdraw,
            _shares,
    ...
    function _mintFeeShares(uint shares) internal {
        ...
        uint vaultBalance = balanceOptimized();
        uint256 tokensToWithdraw = (vaultBalance.mul(reserveShares)).div(totalSupply());

        uint256 balanceDepositToken = IERC20(depositToken).balanceOf(address(this));
        if (balanceDepositToken < tokensToWithdraw) {
            uint extraTokensNeeded = tokensToWithdraw - balanceDepositToken;
            _withdrawFromStrategies(extraTokensNeeded);
        }
        balanceDepositToken = IERC20(depositToken).balanceOf(address(this));
        IERC20(depositToken).safeTransfer(provider.reserve(), Math.min(balanceDepositToken, tokensToWithdraw));
    }

    /**
     * @notice Withdraws a desired amount of depositToken from the strategies
     * @param amount The amount of tokens that are needed to be withdrawn
     * @return amountWithdrawn The amount of depositToken that was retrieved from the strategies
     */
    function _withdrawFromStrategies(uint amount) internal returns (uint amountWithdrawn) {
        uint vaultBalance = balanceOptimized();
        for (uint256 i = 0; i < strategies.length; i++) {
            IBorrower strategy = IBorrower(strategies[i]);
            uint256 _toWithdraw = amount * uint(strategy.balanceOptimized()) / vaultBalance;
            if (_toWithdraw>0) {
                uint balanceBefore = IERC20(depositToken).balanceOf(address(this));
                IController(provider.controller()).withdraw(strategies[i], _toWithdraw);
                withdrawn[strategies[i]]+=IERC20(depositToken).balanceOf(address(this)) - balanceBefore;
                amountWithdrawn+=IERC20(depositToken).balanceOf(address(this)) - balanceBefore;
            }
        }
    }
