 ...
 * @title TLD Node NFT
 * @author TLD-TEAM
 * @notice This Contract Supply TLD Node NFT
 */
contract NODE is ERC721Enumerable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using Strings for uint256;

    string public baseURI = "https://nft.tronlending.io/";

    ITLDPool public TLDPool = ITLDPool(0xF3e215Cc61505bA9A18e084c7d891B1ebc170E68);
    ...
    address public marketWallet = 0xA30D18C731c9944F904fFB1011c17B75280d2A08;

    event BuyNFT(address indexed to, uint256 nftId);

    constructor() ERC721("TLD Node NFT", "NODE") {}

    /**
     * @dev Buy New NFT
     */
    function buyNFT() external nonReentrant {
        require(totalSupply() < 1000, "Total supply must < 1000");

        USDT.safeTransferFrom(msg.sender, marketWallet, 1000 * 1e18);

        uint256 newNFTId = totalSupply() + 1;

        _safeMint(msg.sender, newNFTId);

        NODEPool.deposit(msg.sender, TLDPool.userStakes(msg.sender));

        emit BuyNFT(msg.sender, newNFTId);
    }
    ...
    function tokenURI(
        ...
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, "nft.json"))
                : "";
    }
    ...
    function _afterTokenTransfer(
        ...
        NODEPool.withdraw(from, TLDPool.userStakes(from));
        NODEPool.deposit(to, TLDPool.userStakes(to));
    }
}