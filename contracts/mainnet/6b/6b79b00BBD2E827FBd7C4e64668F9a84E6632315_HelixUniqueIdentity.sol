...
import "../interfaces/IHelixUniqueIdentity.sol";
...
contract HelixUniqueIdentity is ERC1155PresetPauserUpgradeable, IHelixUniqueIdentity {
  // bytes32 public constant MINT_TYPEHASH = keccak256("Mint(address account,uint256 id,uint256 expiresAt)");
  bytes32 public constant MINT_TYPEHASH =
    0xdaf4d1aae6d1eb60b2ef7e52b91d41a981bdeba94cb44da6bba1b355fb4b0edb;

  // bytes32 public constant MINT_TO_TYPEHASH = keccak256("MintTo(address mintTo,address account,uint256 id,uint256 expiresAt)");
  bytes32 public constant MINT_TO_TYPEHASH =
    0xad0429117b2cbd550f7874169f7668e70abfafb39415501f3a183e528caf4de2;

  // bytes32 public constant BURN_TYPEHASH = keccak256("Burn(address account,uint256 id,uint256 expiresAt)");
  bytes32 public constant BURN_TYPEHASH =
    0x7f86fc42c1949807d76870083afed76824b6f66da851cae78ad10f94a943bd23;
  ...
  uint256 public mintCostPerToken;

  event MintCostPerTokenUpdated(uint256 newMintCostPerToken);

  constructor() {
    _disableInitializers();
  }

  /// @notice Initialize HelixUniqueIdentity contract
  /// @param owner Owner of contract
  /// @param uri The base uri of token
  ...
  /// @notice Set the cost to mint a token
  /// @dev Only be called by admin
  /// @param newMintCostPerToken The cost to mint a token
  function setMintCostPerToken(uint256 newMintCostPerToken) external onlyAdmin {
    mintCostPerToken = newMintCostPerToken;
    emit MintCostPerTokenUpdated(newMintCostPerToken);
  }

  /// @notice Set supported UID types
  /// @dev Only be called by admin
  /// @param ids An array of supported UID types
  /// @param values An array of boolean value, true if supported and vice versa
  ...
  /// @notice User mint a new UID token
  /// @param id UID of token
  /// @param expiresAt The expire time of signature
  /// @param signature The signature of admin to allow someone mint UID token
  function mint(
    uint256 id,
    uint256 expiresAt,
    ...
    payable
    override
    onlySignerMint(_msgSender(), id, expiresAt, signature)
    incrementNonce(_msgSender())
  {
    _mintTo(_msgSender(), id);
  }

  /// @notice Mint a new UID token to an user
  /// @param recipient Owner of new UID token
  /// @param id UID of token
  /// @param expiresAt The expire time of signature
  /// @param signature The signature of admin to allow someone mint UID token
  function mintTo(
  ...
  function _mintTo(address mintToAddress, uint256 id) private {
    require(msg.value >= mintCostPerToken, "Token mint requires to be paid");
  ...
  /// @notice Burn UID token of an user
  /// @param account The address of an user
  ...
  function burn(
    ...
    uint256 expiresAt,
    bytes calldata signature
  ) public override onlySignerBurn(account, id, expiresAt, signature) incrementNonce(account) {
    _burn(account, id, 1);
  ...
  function withdrawFee(address recipient) public onlyRole(OWNER_ROLE) {
    uint balance = address(this).balance;

    require(balance > 0, "Fee must be greater than 0");
    (bool success, ) = payable(recipient).call{value: balance}("");

    require(success, "Transfer money to recipient failed");

    emit NativeTokenWithdrawed(recipient, balance);
  }
  ...
  modifier onlySignerMint(
    ...
    bytes32 hash = keccak256(
      abi.encodePacked(MINT_TYPEHASH, account, id, expiresAt, address(this), nonces[account], block.chainid)
    );
  ...
  modifier onlySignerBurn(
    ...
    bytes32 hash = keccak256(
      abi.encodePacked(BURN_TYPEHASH, account, id, expiresAt, address(this), nonces[account], block.chainid)
    );
    bytes32 ethSignedMessage = ECDSAUpgradeable.toEthSignedMessageHash(hash);
    require(
      hasRole(SIGNER_ROLE, ECDSAUpgradeable.recover(ethSignedMessage, signature)),
...
interface IHelixUniqueIdentity is IERC1155Upgradeable {
  event NativeTokenWithdrawed(address indexed recipient, uint256 amount);
