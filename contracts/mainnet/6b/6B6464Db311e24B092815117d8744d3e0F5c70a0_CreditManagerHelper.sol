...
interface ICreditManager {
    ...
    function repay(address _recipient, uint256 _borrowedAmount, uint256 _repayAmountDuringLiquidation, bool _liquidating) external;
    ...
    event Harvest(uint256 _claimed, uint256 _accRewardPerShare, uint256 _totalShares);
...
import { ICreditManager as IOriginCreditManager } from "../credit/interfaces/ICreditManager.sol";
import { IAbstractVault } from "../vaults/interfaces/IAbstractVault.sol";
import { IShareLocker as IOriginShareLocker } from "../vaults/interfaces/IShareLocker.sol";
...
contract CreditManagerHelper {
    ...
    function pendingRewards(address _creditManager, address _recipient) public view returns (uint256) {
        ...
        uint256 rewards = ICreditManager(_creditManager).pendingRewards(_recipient);
        ...
        if (unclaimed > 0) {
            if (totalShares > 0) {
                ...
                (uint256 shares, , uint256 rewardPerSharePaid) = ICreditManager(_creditManager).users(_recipient);

                rewards += ((accRewardPerShare - rewardPerSharePaid) * shares) / PRECISION;
            }
        }
