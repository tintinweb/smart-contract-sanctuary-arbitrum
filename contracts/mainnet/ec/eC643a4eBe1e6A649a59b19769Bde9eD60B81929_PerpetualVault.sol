...
contract PerpetualVault is Initializable, Ownable2StepUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize(
    ...
    governanceFee = 500;  // 5%
    minCollateralAmount = 10_000_000;
    minEth = 0.002 ether;
    ...
    if (isOpen) {
      if (positionIsClosed == true) {
        ...
        if (_isLongOneLeverage(isLong, _leverage)) {
          bool completed = _runSwap(_swapData, true);
          if (completed) {
      ...
      if (positionIsClosed == false) {
        if (_isLongOneLeverage(beenLong, leverage)) {
          bool completed = _runSwap(_swapData, false);
          if (completed) _finalize();
        } else {
          _createDecreasePosition(0, 0, beenLong);
        }
      } else {
  ...
  function afterOrderExecution(
    ...
    if (orderResultData.orderType == Order.OrderType.MarketIncrease) {
      ...
      (leverage, ) = abi.decode(callbackdata, (uint256, bool));
  ...
  function afterOrderCancellation(bytes32 key, Order.OrderType orderType, bool, bytes memory /* callbackdata */) external nonReentrant {
    ...
    if (orderType == Order.OrderType.MarketSwap) {
      ...
      if (flow == FLOW.DEPOSIT) {
        ...
        nextAction.selector = NextActionSelector.WITHDRAW_ACTION;
      // } else if (flow == FLOW.SETTLE || flow == FLOW.COMPOUND) {
      //   _finalize();
      } else {
        _finalize();
      }
  ...
  function _doLifiSwap(bytes memory data, bool isCollateralToIndex) internal returns (uint256 outputAmount) {
    ...
    if (isCollateralToIndex) {
      balBefore = IERC20(indexToken).balanceOf(address(this));
    } else {
      balBefore = IERC20(collateralToken).balanceOf(address(this));
    }
    
    (bool success, ) = to.call(callData);
    require(success, "uniswap call reverted");
    outputAmount = IERC20(indexToken).balanceOf(address(this)) - balBefore;
  }
