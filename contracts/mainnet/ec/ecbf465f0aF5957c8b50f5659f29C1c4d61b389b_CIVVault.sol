...
contract CIVVault is Ownable, ReentrancyGuard {
    ...
    */
    ...
    function initializeStrategyWithState(
        ...
        bool successMint = strategy.fundRepresentToken.mint(sharesToMint);
        require(successMint, "ERR_V.8");

        // distribute shares to all depositors
        for (uint i = 0; i < _investors.length; i++) {
            if (_sharesToMint[i] > 0) {
                bool successShares = strategy.fundRepresentToken.transfer(
                    _investors[i],
                    _sharesToMint[i]
                );
                require(successShares, "Shares transfer failed");
            }
        }
...
// Reduce totDepositedAssets by entry fee and send fees to withdraw addresses
        ...
        if (feeAmount > 0) {
            bool transfer1 = strategy.assetToken.transfer(
                strategy.withdrawAddress[0],
                feeAmount / 2
            );
            require(transfer1, "entryFee transfer1 failed");
            bool transfer2 = strategy.assetToken.transfer(
                strategy.withdrawAddress[1],
                feeAmount / 2
            );
            require(transfer2, "entryFee transfer2 failed");
        }
*/
        ...
        if (
            ...
            if (netDeposits > 0) {
                ...
                bool success = strategy.assetToken.transfer(
                    strategy.investAddress,
                    netDeposits
                );
                require(success, "Funds to invest failed");
                emit Deposit(
                    address(this),
                    strategy.investAddress,
                    _id,
            ...
            bool success = strategy.assetToken.transferFrom(
                strategy.investAddress,
                address(this),
                offSet
            );
            require(success, "Funds to withdraw transfer failed");
        }
        ...
        checkEpochExistence(_id)
    {
        ...
        bool success = strategy.assetToken.transfer(_msgSender(), amount);
        require(success, "Cancel Deposit transfer failed");
        ...
        checkEpochExistence(_id)
    {
        ...
        bool success = _strategyInfo[_id].fundRepresentToken.transferFrom(
            _msgSender(),
            address(this),
            _amount
        );
        ...
        checkEpochExistence(_id)
    {
        ...
        bool success = strategy.fundRepresentToken.transfer(
            _msgSender(),
            amount
        );
        require(success, "Cancel Withdraw Transfer failed");
    ...
    function _claimWithdrawedTokens(
        ...
        if (availableToClaim > 0) {
            bool success = _strategyInfo[_id].assetToken.transfer(
                _user,
                availableToClaim
            );
            require(success, "Claim Withdraw Transfer failed");
        }
    ...
    function _distributeShares(uint _id) internal {
        ...
        for (uint i = lastProcessedIndex; i < loopLimit; i++) {
            ...
            uint depositInfo = _userInfoEpoch[_id][investor][lastProcessedEpoch]
                .depositInfo;
            uint netDepositInfo = (depositInfo * (feeBase - epoch.entryFee)) /
                feeBase;
            uint dueShares = (sharesToDistribute * netDepositInfo) /
                epoch.totDepositedAssets;
            ...
            if (dueShares > 0) {
                ...
                bool success = _strategyInfo[_id].fundRepresentToken.transfer(
                    investor,
                    dueShares
                );
                if (!success) {
    ...
    function sendPendingFees(
        ...
        bool success1 = strategy.assetToken.transfer(addr0, pendingFees / 2);
        require(success1, "Withdraw Fee transfer 1 failed");
        bool success2 = strategy.assetToken.transfer(addr1, pendingFees / 2);
        require(success2, "Withdraw Fee transfer 2 failed");
    }
    ...
    function withdrawERC20(IERC20 _tokenContract) external onlyOwner {
        ...
        require(success, "Withdraw ERC20 transfer failed");
    }
}
