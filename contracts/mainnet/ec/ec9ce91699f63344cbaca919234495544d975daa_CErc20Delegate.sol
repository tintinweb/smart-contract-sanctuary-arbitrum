...
contract CTokenStorage {
    ...
    /**
     * @notice Is the underlying token
     */
    bool public nonBorrowable;
    ...
    // used for calculating interest rate performance of vault market
    uint public prevExchangeRate;
    uint public zsBlockDelta;
    uint public lastZSGMUSDCDepositPrice; // This is used to determine price increases
...
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter {
     ...
     * @param nonBorrowable_ Wether or not the market being created is borrowable
     */
    ...
    function accrueInterest() virtual override public returns (uint) {
        /* Determine the amount of zsGMUSDC gain in price we will need to pull from contract to account for performance fee */
        if (nonBorrowable){
            uint256 price = GlobalPriceOracle(GLOBAL_PRICE_ORACLE).getUnderlyingPrice(address(this));
            address internalUnderlying = CErc20Storage(address(this)).underlying();
            if(lastZSGMUSDCDepositPrice == 0 || performanceFee == 0){
                // This is the initial accrual, do not pull anything out
                lastZSGMUSDCDepositPrice = price;
                return NO_ERROR;
            }
            if(price < lastZSGMUSDCDepositPrice){
            ...
            if(price > lastZSGMUSDCDepositPrice){
                ...
                uint256 percentIncrease = div_(mul_(sub_(price, lastZSGMUSDCDepositPrice),1e18),lastZSGMUSDCDepositPrice); // 1e18 decimals
                ...
                uint256 total_ = EIP20Interface(internalUnderlying).balanceOf(address(this));
    ...
    function mintFresh(address minter, uint mintAmount) internal {
        ...
        if (accrualBlockNumber != getBlockNumber() && !nonBorrowable) {
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal {
        ...
        if(nonBorrowable && !isRedeemerVip && withdrawFee > 0){
...
contract CErc20 is CToken, CErc20Interface {
     ...
     * @param nonBorrowable_ Whether or not the market being created is borrowable
     */
    function initialize(address underlying_,
        ...
        super.initialize(comptroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_, nonBorrowable_);
