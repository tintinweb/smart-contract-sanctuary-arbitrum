...
interface IArbVester {
    ...
    event VestingPositionAdded(address indexed user, uint256 amount, uint256 lpAmount, uint256 durationWeeks, uint256 start, uint256 vestId);
    event VestingPositionClosed(address indexed user, uint256 amount, uint256 vestId, uint256 usdtAmount);
    event VestingPositionUnlocked(address indexed user, uint256 amount, uint256 vestId);
    event Withdrawn(address indexed user, uint256 usdtAmount, uint256 amount);
    event ClaimedReward(address indexed user, address indexed asset, uint256 amount, uint256 totalClaimedAmount);
    ...
    function unlockLp(uint256 _vestId) external;
    ...
    function getPendingRewards(uint256 _vestId) external view returns (uint256[] memory);
    ...
    function calculateLpAmount(uint256 _arbAmount) external view returns (uint256);
}
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function addRewardToken(
        ...
        require(_rewardToken != address(0), "Vester : invalid _rewardToken");
        require(
            !rewardTokens.contains(_rewardToken),
            "Vester: reward token already added"
        );
        rewardTokens.add(_rewardToken);
    }

    function delRewardToken(
        address _rewardToken
    ) external onlyRole(EqbConstants.ADMIN_ROLE) {
        require(
            rewardTokens.contains(_rewardToken),
            "Vester: reward token not added"
        );
    ...
    function closeVestingPosition(
        ...
        require(
            _getTimeWeeksAfter(
                vestingPosition.start,
                vestingPosition.durationWeeks + expireWeeks
            ) > block.timestamp,
    ...
    function _withdrawNftPosition(
        ...
        INitroPool(nitroPool).withdraw(_vestingPosition.nftTokenId);
    ...
    function unlockLp(uint256 _vestId) external updateReward {
        require(_vestId < vestingPositions.length, "Vester: invalid _vestId");
        ...
        emit VestingPositionUnlocked(vestingPosition.user, _lpAmount, _vestId);
    }
