...
contract Core is ICore, Ownable, ReentrancyGuard {
    ...
    function autoResolve(uint256 positionId, bytes[] calldata updateData) external returns (bool) {
        ...
        bool canceled = false;
        uint256 cancelationTimestamp = 0;
        (canceled, position_.endPrice, cancelationTimestamp) = _immutableConfiguration.utils.getPriceForAutoResolve(
            order_.data.oracle,
            position_.endTime,
            updateData
        );
        order_.reserved -= position_.amountCreator;
        if (canceled) {
            require(cancelationTimestamp < block.timestamp, "Core: Cancelation period not passed");
            order_.available += position_.amountCreator;
...
interface ICoreUtilities {
    ...
    ) external returns (bool canceled, uint256 price, uint256 cancelationTimestamp);
