...
contract TestGMXVault {
  ...
  IExchangeRouter exchangeRouter = IExchangeRouter(0x11E590f6092D557bF71BaDEd50D81521674F8275);
  ...
  // IERC20 tokenA = IERC20(0x82aF49447D8a07e3bd95BD0d56f35241523fBab1); // WETH

  IERC20 tokenA = IERC20(0x2f2a2543B76A4166549F7aaB2e75Bef0aefC5B0f); // WBTC
  IERC20 tokenB = IERC20(0xaf88d065e77c8cC2239327C5EDb3A432268e5831);
  // IERC20 lpToken = IERC20(0x70d95587d40a2caf56bd97485ab3eec10bee6336); // WETH-USDC
  IERC20 lpToken = IERC20(0x7C11F78Ce78768518D743E81Fdfa2F860C6b9A77); // WBTC GM
  ...
  constructor() {
    ...
    tokenA.approve(address(depositVault), type(uint256).max);
    tokenB.approve(address(depositVault), type(uint256).max);

    lpToken.approve(address(withdrawalVault), type(uint256).max);
  }
  ...
  function addLiquidity(
    ...
    IExchangeRouter.CreateDepositParams memory _cdp =
      IExchangeRouter.CreateDepositParams({
        ...
        initialShortToken: address(tokenA),
