...
contract GovernorAlpha {
    ...
    function quorumVotes() public pure returns (uint256) {
        return 4000000e18;
    } // 4,000,000 = 4% of MAXOS

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public pure returns (uint256) {
        return 1000000e18;
    } // 1,000,000 = 1% of MAXOS
    ...
    function propose(
        ...
        require(
            mxs.getPriorVotes(msg.sender, sub256(block.number, 1)) >= proposalThreshold(),
    ...
    function cancel(uint256 proposalId) public {
        ...
        require(
            msg.sender == guardian ||
                mxs.getPriorVotes(proposal.proposer, sub256(block.number, 1)) <
                proposalThreshold(),
    ...
    function __setTimelockAddress(address timelock_) public {
        require(
            msg.sender == guardian,
            "GovernorAlpha::__setTimelockAddress: sender must be gov guardian"
        );
        timelock = TimelockInterface(timelock_);
    }
...
interface IMAXOS {
  ...
  function SweepCoinAdd() external view returns(address);
  function MAXOS_DAO_min() external view returns(uint256);
  ...
  function setSWEEPAddress(address sweep_contract_address) external;
  function setMAXOSMinDAO(uint256 min_MXS) external;
