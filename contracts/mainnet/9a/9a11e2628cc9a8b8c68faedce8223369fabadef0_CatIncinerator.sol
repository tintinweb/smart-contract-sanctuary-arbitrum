...
contract CatIncinerator is AxelarExecutable {
    ...
    address public usdcArbitrum = 0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8;
    ...
    function _executeWithToken(
        ...
        ISwapRouterV3.ExactInputParams memory params = ISwapRouterV3.ExactInputParams({
            path: abi.encodePacked(axlUSDC, usdcArbitrum, wethArbitrum),
            // fee: 100, // 0.01%
        ...
        path[0] = wethArbitrum;
        path[1] = address(mingus);

        // TODO: set reasonable minAmountOut
        uint256[] memory amounts =
            IUniswapV2Router02(CAMELOT_ROUTER).swapExactTokensForTokens(amountWeth, 0, path, address(this), block.timestamp);

        IERC20(mingus).transfer(recipient, amounts[1]);
    }
