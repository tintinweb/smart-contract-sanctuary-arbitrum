...
contract DCBNFTStaking is Initializable, OwnableUpgradeable, IStaking, IERC721ReceiverUpgradeable {
  ...
  event Stake(address indexed user, uint16 indexed _pid, uint256[] indexed ids, uint256 time);
  event ReStake(uint16 indexed _pid, address indexed user, uint256 timestamp);
  event Unstake(address indexed user, uint16 indexed _pid, uint256[] indexed ids, uint256 time);
  ...
  function add(
    ...
    require(_lockPeriodInDays > 0, "DCB: Lock period cannot be zero");
  ...
  function stake(uint16 _pid, uint256[] calldata _ids) external {
    PoolExtended storage pool = poolExt[_pid];
    UserExtended storage user = userExt[_pid][msg.sender];

    uint256 stopDepo = pool.common.endDate - (uint256(pool.common.lockPeriodInDays) * 1 days);
  ...
  function unStake(uint16 _pid, uint256[] calldata _ids) external {
    UserExtended storage user = userExt[_pid][msg.sender];
    PoolExtended storage pool = poolExt[_pid];
  ...
  function getDepositedIdsOfPool(uint16 _pid) external view returns (uint256[] memory) {
    return poolExt[_pid].depositedIds;
  }

  function getDepositedIdsOfUser(
    uint16 _pid,
    address _user
  ...
  function canClaim(uint16 _pid, address _addr) public view returns (bool) {
    ...
    return (block.timestamp >= user.depositTime + (uint256(pool.lockPeriodInDays) * 1 days));
  }
  ...
  function payout(uint16 _pid, address _addr) public view returns (uint256 value) {
    ...
    uint256 userTime = block.timestamp >
      (user.depositTime + (uint256(pool.lockPeriodInDays) * 1 days))
      ? (user.depositTime + (uint256(pool.lockPeriodInDays) * 1 days))
      : block.timestamp;
    uint256 to = userTime > pool.endDate ? pool.endDate : userTime;

    if (to > from) {
      value = ((to - (from)) * (pool.rewardRate) * (user.totalInvested)) / (1 days);
