...
contract PositionTracker is IERC20, ReentrancyGuard, IPositionTracker, Governable {
    using SafeMath for uint256;
    ...
    mapping (address => uint256) public override depositAmount; // deposit amount, not deflation
    ...
    function initialize(
        ...
        require(!isInitialized, "PositionTracker: already initialized");
        require(
            _deflationRatioPerSecond < PRECISION
            && _deflationRatioPerSecond > 1,
            "PositionTracker: invalid deflationRatioPerSecond"
        );
        isInitialized = true;
    ...
    function stake(address /** _depositToken */, uint256 _amount) external override nonReentrant {
        if (inPrivateStakingMode) { revert("PositionTracker: action not enabled"); }
    ...
    function transferFrom(address _sender, address _recipient, uint256 _amount) external override returns (bool) {
        ...
        uint256 nextAllowance = allowances[_sender][msg.sender].sub(_amount, "PositionTracker: transfer amount exceeds allowance");
    ...
    function claim(address _receiver) external override nonReentrant returns (uint256[] memory) {
        if (inPrivateClaimingMode) { revert("PositionTracker: action not enabled"); }
    ...
    function claimForAccount(address _account, address _receiver) external override nonReentrant returns (address[] memory, uint256[] memory) {
        ...
        uint256 len = distributors.length;
        uint256[] memory amounts = new uint256[](len);
        address[] memory rewards = new address[](len);
        for(uint256 i=0; i<len; i++) {
    ...
    function _mint(address _account, uint256 _amount) internal {
        require(_account != address(0), "PositionTracker: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal {
        require(_account != address(0), "PositionTracker: burn from the zero address");

        balances[_account] = balances[_account].sub(_amount, "PositionTracker: burn amount exceeds balance");
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) private {
        require(_sender != address(0), "PositionTracker: transfer from the zero address");
        require(_recipient != address(0), "PositionTracker: transfer to the zero address");

        if (inPrivateTransferMode) { _validateHandler(); }

        balances[_sender] = balances[_sender].sub(_amount, "PositionTracker: transfer amount exceeds balance");
    ...
    function _approve(address _owner, address _spender, uint256 _amount) private {
        require(_owner != address(0), "PositionTracker: approve from the zero address");
        require(_spender != address(0), "PositionTracker: approve to the zero address");
    ...
    function _validateHandler() private view {
        require(isHandler[msg.sender], "PositionTracker: forbidden");
    }

    /// @notice stake token for reward
    function _stake(address, address _account, uint256 _amount) private {
        require(_amount > 0, "PositionTracker: invalid amount");
        require(MAX_CDR > cumulativeDeflationaryRatio, "PositionTracker: stop stake");

        _updateRewards(_account);
        stakedAmounts[_account] = stakedAmounts[_account].add(_amount);
        _totalStaked = _totalStaked.add(_amount);
        depositAmount[_account] = depositAmount[_account].add(_amount);
        _mint(_account, _amount);
    }

    /// @notice unstake token
    function _unstake(address _account, uint256 _amount, address) private returns(uint256 amount) {
        uint256 _depositAmount = depositAmount[_account];
        ...
        if (_amount >= _depositAmount) {
            ...
            depositAmount[_account] = 0;
            amount = _depositAmount;
        }
        else {
            amount = _amount;
            depositAmount[_account] = _depositAmount.sub(_amount);
            _amount = stakedAmount.mul(_amount).div(_depositAmount);
        }       
    ...
    function _updateRewardsForDistributor(
        ...
        if (_account != address(0)) {
            ...
            if (accountReward > 0 && tmp.stakedAmount > 0) {
                tmp.cumulativeRewards = cumulativeRewards[_account][tmp.rewardToken];
                tmp.nextCumulativeReward = tmp.cumulativeRewards.add(accountReward);
                tmp.averageStakedAmounts = averageStakedAmounts[_account][tmp.rewardToken];
                ...
                cumulativeRewards[_account][tmp.rewardToken] = tmp.nextCumulativeReward;
                averageStakedAmounts[_account][tmp.rewardToken] = tmp.averageStakedAmounts;
            }
        }
    }
