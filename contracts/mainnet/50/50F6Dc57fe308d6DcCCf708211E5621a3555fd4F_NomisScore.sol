    ...
    EIP712Upgradeable
{
    ...
    function setScore(
        ...
        if (isNewScore) {
            ...
            if (referrerCodeBytes != 0) {
                address referrerWallet = _referralCodeToWallet[referrerCodeBytes];
                if (referrerWallet != address(0)) {
                    ...
                    (bool success, ) = payable(referrerWallet).call{ value: rewardValue }("");
                    require(success, "setScore: claim referral reward failed");

                    _alreadyRewardedWallet[msg.sender] = true;
    ...
    /**
     * @dev Claim referral rewards partially.
     * @param count The number of referrals to claim rewards for.
     * @param skip The number of referrals to skip.
     */
    function claimReferralRewardsPartially(
        uint256 count,
        uint256 skip
        ...
        require(count + skip <= referrals.length, "claimReferralRewardsPartially: count + skip should be less or equal then referrals length");

        // get claimableReward for all not claimed referrals
        uint256 referralsCount = 0;
        for (uint256 i = skip; i < count + skip; ++i) {
        ...
        require(
            claimableReward > 0,
            "claimReferralRewardsPartially: No rewards available"
        );
        require(
            claimableReward <= address(this).balance,
            "claimReferralRewardsPartially: Insufficient funds"
        );

        (bool success, ) = msg.sender.call{value: claimableReward}("");
        require(success, "claimReferralRewardsPartially: transfer failed");
