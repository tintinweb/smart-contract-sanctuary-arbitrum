...
contract FOMODuckIDO is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public constant  duckPrice = 10; //0.0000001 U * 100
    ...
    constructor(
        ...
        startTime = 1684843200;
        endTime  = 1685016000;
        offeringAmount =  500_000_000_000 * (10**18);
        raisingAmount= 50000* (10**6);

        adminAddress = msg.sender;
        finalleftWithAddr = 0x6023d0e894f6B3E8d3b9206DdbafdddF9929ba95;
        totalAmount = 0;
        calcDuration = 3600;
    }
    ...
    function finalWithdrawOfferingToken() public onlyAdmin {
        ...
        if(totalAmount < raisingAmount){
            ...
            offeringToken.safeTransfer(finalleftWithAddr, withDrawOfferAmount);
        }
    }

    function finalWithdrawOfferingTokenEx(uint256 _offerAmount) public onlyAdmin {
        require (block.timestamp > endTime.add(calcDuration), 'not withdraw OfferingToken time');
        if(totalAmount < raisingAmount){
            uint256 withDrawOfferAmount  = (raisingAmount.sub(totalAmount).mul(1e20).div(duckPrice));
            require(_offerAmount <= withDrawOfferAmount, 'offer amount wrong');
            offeringToken.safeTransfer(finalleftWithAddr, _offerAmount);
        }
    }
    ...
    function getIDOAmounts() public view returns(uint256 raiseAmount,uint256 offerAmount,uint256 userTotalAmout){
        ...
        offerAmount = offeringAmount; // DUCK
        userTotalAmout = totalAmount; //user USDC
    }

    function getDuckPriceX2() public pure returns(uint256){
        return duckPrice;
    }
