...
interface IDelegateV1 {
    ...
    /// @notice Retrieves the LockHolder contract for `partner_`.
    /// @param partner_ Partner address.
    /// @return LockHolder contract address.
    function lockHolderByPartner(address partner_) external view returns (address);
...
interface IMerklHelper {
    /// @notice Retrieves the total votes balance for `account_`.
    /// @param account_ Account address.
    /// @return votes The total votes balance for `account_`.
    function getVotes(address account_) external view returns (uint256 votes);
}
...
import "./interfaces/IMerklHelper.sol";
import "./interfaces/IVotingEscrow.sol";
import "./interfaces/IDelegateV1.sol";

contract MerklHelper is IMerklHelper {
    IVotingEscrow public immutable votingEscrow;
    IDelegateV1 public immutable partnersDelegate;
    IDelegateV1 public immutable holdersDelegate;

    /// @param votingEscrow_ VotingEscrow contract address.
    /// @param partnersDelegate_ Partners' delegate contract address.
    /// @param holdersDelegate_ Holders' delegate contract address.
    constructor(
        IVotingEscrow votingEscrow_,
        IDelegateV1 partnersDelegate_,
        IDelegateV1 holdersDelegate_
    ) {
        votingEscrow = votingEscrow_;
        partnersDelegate = partnersDelegate_;
        holdersDelegate = holdersDelegate_;
    }

    /// @inheritdoc IMerklHelper
    function getVotes(address account_) external view returns (uint256 votes) {
        votes = _retrieveVotesForAccount(account_);
        votes += _retrieveVotesForAccount(partnersDelegate.lockHolderByPartner(account_));
        votes += _retrieveVotesForAccount(holdersDelegate.lockHolderByPartner(account_));
    }

    /// @notice Retrieves the current votes balance for `account_`.
    /// @param account_ Account address.
    /// @return votes The current votes balance for `account_`.
    function _retrieveVotesForAccount(address account_) private view returns (uint256 votes) {
        if (account_ != address(0)) {
            votes = votingEscrow.getVotes(account_);
        }
    }
}