...
contract MerkleDistributor is IMerkleDistributor, Ownable {
    ...
    function claim(address account, uint256 cumulativeAmount, bytes32[] calldata merkleProof) public virtual override {
        ...
        if (claimed >= cumulativeAmount) revert AlreadyClaimed();
        uint256 remainingAmount = cumulativeAmount - claimed;

        // Mark it claimed and send the token.
        claimedAmount[account] = cumulativeAmount;
        IERC20(token).safeTransfer(account, remainingAmount);

        emit Claimed(account, remainingAmount);
    }
}