...
import "../toadstoolz/libraries/BBase64.sol";
...
abstract contract ERC1155OnChainBaseUpgradeable is Initializable, AdminableUpgradeable, ERC1155Upgradeable {
    ...
    event TokenAnimationUrlChanged(uint256 _tokenId, string _animationUrl);
    event TokenIsSoulboundChanged(uint256 _tokenId, bool _isSoulbound);
    ...
    override
    {
        ...
        for(uint256 i = 0; i < ids.length; i++) {
            ...
            if(tokenIdToTraitData[_id].isSoulBound) {
    ...
    onlyAdminOrOwner
    {
        ...
        emit TokenAnimationUrlChanged(_tokenId, _traitData.animationUrl);
        emit TokenIsSoulboundChanged(_tokenId, _traitData.isSoulBound);
    }
    ...
    returns(string memory)
    {
        ...
        string memory metadata = string(abi.encodePacked(
            ...
            _getAnimationUrlJSON(_traitData),
    ...
    function _getAnimationUrlJSON(TraitData storage _traitData) private view returns(string memory) {
        // Don't include animationUrl in the JSON if there is nothing set.
        //
        if(compareStrings(_traitData.animationUrl, "")) {
            return "";
        }

        return string(abi.encodePacked(
            '", "animation_url": "',
            _traitData.animationUrl
        ));
    }
...
contract Itemz is Initializable, ItemzContracts {
    ...
    function safeTransferFromSingle(
        address _from,
    ...
    whenNotPaused
    {
        ...
        for(uint256 i = 0; i < _tos.length; i++) {
            safeTransferFrom(_from, _tos[i], _id, _amount, "");
        }
    }
    ...
    whenNotPaused
    {
        require(_tos.length > 0 && _tos.length == _ids.length && _tos.length == _amounts.length);

        for(uint256 i = 0; i < _tos.length; i++) {
            safeTransferFrom(_from, _tos[i], _ids[i], _amounts[i], "");
        }
    }
...
import "./IItemz.sol";
import "../../shared/ERC1155OnChainBaseUpgradeable.sol";
