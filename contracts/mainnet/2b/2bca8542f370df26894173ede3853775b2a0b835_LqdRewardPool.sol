...
// Note that this pool has no minter key of LQD (rewards).
// Instead, the governance will call LQD distributeReward method and send reward to this pool at the beginning.
contract LqdRewardPool is IRewardPool, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. LQDs to distribute in the pool.
        uint256 lastRewardTime; // Last time that LQDs distribution occurred.
        uint256 accLqdPerShare; // Accumulated LQDs per share, times 1e18. See below.
    ...
    // The time when LQD mining starts.
    uint256 public poolStartTime;
    uint256 public poolEndTime;

    uint256 public rewardPerSecond;
    uint256 public constant RUNNING_TIME = 7 days;
    uint256 public constant TOTAL_REWARDS = 25000 ether;

    uint256 constant MAX_DEPOSIT_FEE = 2; // 2%
    ...
    constructor(address _lqd, uint256 _poolStartTime, address _treasuryFund) public {
        require(block.timestamp < _poolStartTime, "late");
        if (_lqd != address(0)) lqd = IERC20(_lqd);

        poolStartTime = _poolStartTime;
        poolEndTime = _poolStartTime.add(RUNNING_TIME);
        rewardPerSecond = TOTAL_REWARDS.div(RUNNING_TIME); 
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "LqdRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "LqdRewardPool: existing pool?");
        }
    }

    // Add a new token to the pool. Can only be called by the owner.
    function add(uint256 _allocPoint, IERC20 _token, uint256 _lastRewardTime, uint256 _depositFee) public onlyOperator {
        ...
        poolInfo.push(PoolInfo({token: _token, allocPoint: _allocPoint, lastRewardTime: _lastRewardTime, accLqdPerShare: 0, isStarted: _isStarted, depositFee: _depositFee }));
        if (_isStarted) {
            totalAllocPoint_ = totalAllocPoint_.add(_allocPoint);
        }
    }

    // Update the given pool's LQD allocation point. Can only be called by the owner.
    ...
    // View function to see pending LQDs on frontend.
    function pendingReward(uint256 _pid, address _user) public override view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accLqdPerShare = pool.accLqdPerShare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _lqdReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint_);
            accLqdPerShare = accLqdPerShare.add(_lqdReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accLqdPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _lqdReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint_);
            pool.accLqdPerShare = pool.accLqdPerShare.add(_lqdReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external override nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accLqdPerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                safeLqdTransfer(msg.sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accLqdPerShare).div(1e18);
    ...
    function _withdraw(address _account, uint256 _pid, uint256 _amount) internal {
        ...
        uint256 _pending = user.amount.mul(pool.accLqdPerShare).div(1e18).sub(user.rewardDebt);
        if (_pending > 0) {
            safeLqdTransfer(_account, _pending);
    ...
    // Safe lqd transfer function, just in case if rounding error causes pool to not have enough LQDs.
    function safeLqdTransfer(address _to, uint256 _amount) internal {
        uint256 _lqdBal = lqd.balanceOf(address(this));
        if (_lqdBal > 0) {
            if (_amount > _lqdBal) {
                lqd.safeTransfer(_to, _lqdBal);
            } else {
                lqd.safeTransfer(_to, _amount);
            }
        }
    }
