...
// File contracts/interfaces/IMoonbaseLock.sol

pragma solidity ^0.8.9;

interface IMoonbaseLock {
    function lock(
        address owner,
        address token,
        bool isLpToken,
    ...
    function transferLock(uint256 lockId, address receiverAddress) external;
...
// File contracts/MoonbaseTokenLocker.sol

pragma solidity ^0.8.9;

contract MoonbaseTokenLocker is IMoonbaseLock, Ownable {
    ...
    event LockAdded(uint256 indexed id, address token, address indexed owner, uint256 amount, uint256 unlockDate);
    event LockUpdated(
        uint256 indexed id,
        address token,
    ...
    event LockRemoved(uint256 indexed id, address token, address indexed owner, uint256 amount, uint256 unlockedAt);
    event LockTransfer(uint256 indexed id, address token, address indexed oldOwner, address indexed newOwner);
    ...
    constructor(uint256 _serviceFeeEth) {
        serviceFeeEth = _serviceFeeEth;
    ...
    function setServiceFee(uint256 _serviceFeeEth) public onlyOwner {
        serviceFeeEth = _serviceFeeEth;
    }

    function isFeeWhitelisted(address _user) public view returns (bool) {
    ...
    function lock(
        ...
        require(unlockDate < 10000000000, "TIMESTAMP INVALID"); // prevents errors when timestamp entered in milliseconds
        require(unlockDate > block.timestamp, "Unlock date should be after current time");
        require(amount > 0, "Amount should be greater than 0");
        if (!feeWhitelist.contains(msg.sender)) {
            require(msg.value == serviceFeeEth, "FEE NOT MET");
        }
        if (isLpToken) {
    ...
    function transferLock(uint256 lockId, address receiverAddress) external override validLock(lockId) {
        ...
        if (isLpToken) {
            ...
            _userLpLockIds[receiverAddress].add(lockId);
        } else {
            _userNormalLockIds[msg.sender].remove(lockId);

            _userNormalLockIds[receiverAddress].add(lockId);
        }

        userLock.owner = receiverAddress;

        emit LockTransfer(lockId, userLock.token, msg.sender, receiverAddress);
    }
    ...
    function getCumulativeLpTokenLockInfo(
        uint256 start,
        uint256 end
    ) public view returns (CumulativeLockInfo[] memory) {
