...
contract ERAShibes is ERC20, Ownable {
    ...
    constructor(
        ...
        uint256 _totalSupply = 400000000000000 * 10 ** 6;
    ...
    function swapBack() internal isSwapping {
        ...
        uint256 amountOfPunkBurn = (taxAmount * burnFee) / (totalFee);
        uint256 amountOfPunkLp = (taxAmount * liquidityFee) / (totalFee);
        uint256 amountOfPunkForHolders = (taxAmount * holdersFee) / (totalFee);
        uint256 amountOfPunkForStaking = (taxAmount * stakingFee) / (totalFee);

        taxAmount -= amountOfPunkBurn;
        taxAmount -= amountOfPunkLp;
        taxAmount -= amountOfPunkForHolders;
        taxAmount -= amountOfPunkForStaking;
        ...
        _transfer(address(this), DEAD, amountOfPunkBurn);
        _transfer(address(this), holdersWallet, amountOfPunkForHolders);
        _transfer(address(this), stakingRewardWallet, amountOfPunkForStaking);
        ...
        emit SwapBack(amountOfPunkBurn, amountOfBackTokenMkt, amountOfPunkLp, amountOfPunkForStaking, amountOfPunkForHolders, amountOfBackTokenDev, block.timestamp);
    }
