...
contract Herd is ERC20, Ownable {
    constructor() ERC20("The_Herd_Mentality", "HERD") {
        _mint(msg.sender, 1_000_000);
    }
    uint256 herdSize = 1; //this starts at 1 to account for the LP, and to make sure at least 1 is transferable
    ...
    function penTheSheep() public onlyOwner {
        require(herded >= block.timestamp + ONE_WEEK, 'lassie is still herding the sheep');
        pastured = false;
        emit sheepPastured(block.timestamp, pastured);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if(pastured == true){
            ...
            if(balanceOf(msg.sender) == 0){
                herdSize = herdSize - 1;
                emit sheepSlaughtered(block.timestamp, msg.sender, herdSize);
            }
