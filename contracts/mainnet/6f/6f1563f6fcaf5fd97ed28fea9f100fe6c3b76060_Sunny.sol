...
/// @notice Used to read older contracts
interface OLD_SUNNY {
...
contract Sunny {
    ...
    constructor(
        ...
        address _wrappedNativeTokenAggregatorAddress,
        address _mainStable,
        address _oldContract,
        address[] memory _vendors,
        uint256[] memory _vendorPlans,
        address[] memory _subscribers,
        address[] memory _subscriberVendors,
        uint256[] memory _subscriberPlans
    ) payable {
        if (
            (_wrappedNativeTokenAggregatorAddress == address(0)) || (_wrappedNativeToken == address(0))
                || (_mainStable == address(0))
        ) revert ZeroAddress();
        if (
            _vendors.length != _vendorPlans.length || _subscribers.length != _subscriberVendors.length
                || _subscribers.length != _subscriberPlans.length
        ...
        setMainStable(_mainStable);

        if (_oldContract != address(0)) {
            for (uint256 x; x < _vendors.length; ++x) {
                if (_vendors[x] == address(0)) revert ZeroAddress();

                (uint256 price, uint256 billingPeriod, uint256 timestamp, uint256 discount) =
                    OLD_SUNNY(_oldContract).getPlan(_vendors[x], _vendorPlans[x]);

                _setPlan(_vendors[x], price, billingPeriod, _vendorPlans[x], discount, timestamp);
            }
            for (uint256 x; x < _subscribers.length; ++x) {
                if (_subscribers[x] == address(0)) revert ZeroAddress();

                uint256 _planKey = uint256(uint160(_subscriberVendors[x]));
                _planKey |= _subscriberPlans[x] << 160;

                (uint256 lastPaymentTimestamp, uint256 periods) =
                    OLD_SUNNY(_oldContract).getLastPayment(_subscriberVendors[x], _subscriberPlans[x], _subscribers[x]);
                lastPayment[_planKey][_subscribers[x]] = lastPaymentTimestamp |= periods << 48;
            }
        }
    }
    ...
    modifier validPlan(uint256 _billingPeriod, uint256 _price, uint256 _discount) {
        ...
        if (_price < 10 * 10 ** decimals || _price > 1000000 * 10 ** decimals) {
            revert InvalidPrice();
        }
        if (_billingPeriod < 86400 || _billingPeriod >= 281474976710656) {
    ...
    function setMainStable(address _mainStable) public onlyOwner {
    ...
    function _isValidSubscriber(address _vendor, address _user, uint256 _planId) internal view returns (bool) {
        ...
        uint256 elapsedTime;
        unchecked {
            elapsedTime = block.timestamp - _lastPayment;
        }
        ...
        bool isWithinGracePeriod = elapsedTime <= billingPeriod + buffer;
    ...
    function setPlan(uint256 _price, uint256 _billingPeriod, uint256 _planId, uint256 _discount) external {
        return _setPlan(msg.sender, _price, _billingPeriod, _planId, _discount, 0);
    }

    function _setPlan(
        address _vendor,
        uint256 _price,
        uint256 _billingPeriod,
        uint256 _planId,
        uint256 _discount,
        uint256 _timestamp
    ) internal validPlan(_billingPeriod, _price, _discount) {
        ...
        plan[_getPlanKey(_vendor, _planId)] = _plan;

        // Emit an event indicating that the plan has been set or updated
        emit PlanSet(_vendor, _planId);
    }
    ...
    function _getDiscountPrice(uint256 _price, uint256 _periods, uint256 _cycleLength, uint256 _discount)
        internal
        pure
        returns (uint256)
    {
    ...
    function getAmount(address _token, uint256 _amountIn) external view returns (uint256) {
        return _getAmount(_token, _amountIn);
    }

    function _getAmount(address _token, uint256 _amountIn) internal view returns (uint256) {
        address tokenAggregatorAddress = aggregatorAddress[_token];
        AggregatorV3Interface priceFeed = AggregatorV3Interface(tokenAggregatorAddress);
    ...
    function _transferToken(address _token, address _vendor, address _user, uint256 amount, uint256 _gasless)
        internal
    {
        if (_gasless == 1) {
            // Collect 1% fee, with a $5 cap
            uint256 fee;
            uint256 cap;
            ...
            if (stableIsAllowed[_token] == 1) {
                ...
                fee = amount / 100;
                cap = (5 * (10 ** decimals) * 10 ** 8) / uint256(price);
            }

            if (fee > cap) fee = cap;

            TOKEN(_token).transferFrom(_user, _vendor, amount - fee);
            TOKEN(_token).transferFrom(_user, msg.sender, fee);
        } else {
            TOKEN(_token).transferFrom(_user, _vendor, amount);
        }
    }
    ...
    function subscribeWithAttestation(
        ...
        if (_isValidSubscriber(_vendor, subscriber, _planId)) {
            revert AlreadySubscribed();
        }

        (uint256 price, uint256 billingPeriod,, uint256 discount) = _getPlan(_vendor, _planId);

        _subscribeWithAttestation(subscriber, _vendor, _planId, _periods, _token, price, billingPeriod, discount);
    }

    function _getSubscriberFromAttestation(
    ...
    function _subscribeWithAttestation(
        address subscriber,
        address _vendor,
        uint256 _planId,
        uint256 _periods,
        ...
        if (stableIsAllowed[_token] == 1) {
            _subscribeStable(subscriber, _vendor, _planId, _periods, _token, price, billingPeriod, discount, 1);
        } else {
            _subscribeToken(subscriber, _vendor, _planId, _periods, _token, price, billingPeriod, discount, 1);
        }
    }
    ...
    function subscribeStable(address _vendor, uint256 _planId, uint256 _periods, address _stable) external {
        ...
        _subscribeStable(msg.sender, _vendor, _planId, _periods, _stable, price, billingPeriod, discount, 0);
    }

    function _subscribeStable(
        address _subscriber,
        address _vendor,
        uint256 _planId,
        uint256 _periods,
        ...
        uint256 _discount,
        uint256 _gasless
    ) internal checkPeriods(_periods) {
        if (stableIsAllowed[_stable] == 0) {
            revert InvalidToken();
        }

        _price = _getDiscountPrice(_price, _periods, _billingPeriod, _discount);

        if (_stable != mainStable) {
            uint256 decimals = TOKEN(_stable).decimals();
            uint256 mDecimals = mainStableDecimals;

            if (mDecimals < decimals) {
                _price = _price * (10 ** (decimals - mDecimals));
            } else {
                _price = _price / (10 ** (mDecimals - decimals));
            }
        }

        _confirmation(_subscriber, _vendor, _planId, _periods, _stable, _price);
        _transferToken(_stable, _vendor, _subscriber, _price, _gasless);
    }
    ...
    function subscribeToken(address _vendor, uint256 _planId, uint256 _periods, address _token) external {
        ...
        _subscribeToken(msg.sender, _vendor, _planId, _periods, _token, price, billingPeriod, discount, 0);
    }
    ...
    function _subscribeToken(
        ...
        _transferToken(_token, _vendor, _subscriber, amount, _gasless);
    }
    ...
    function _subscribeNative(
        ...
        payable(_vendor).transfer(amount);
    }
    ...
    function changePlan(address _vendor, uint256 _oldPlanId, uint256 _newPlanId, uint256 _periods, address _token)
        external
        payable
    {
        ...
        if (msg.value != 0) {
            _subscribeNative(_vendor, _newPlanId, _periods, reducedPrice, billingPeriod, discount);
        } else if (stableIsAllowed[_token] == 1) {
            _subscribeStable(
                msg.sender, _vendor, _newPlanId, _periods, _token, reducedPrice, billingPeriod, discount, 0
            );
        } else {
            _subscribeToken(msg.sender, _vendor, _newPlanId, _periods, _token, reducedPrice, billingPeriod, discount, 0);
        }
    }
    ...
    function collectPayment(address _user, address _vendor, uint256 _planId, address _token) external {
        if (stableIsAllowed[_token] == 0 && tokenIsAllowed[_token] == 0) {
            revert InvalidToken();
        }

        Misc memory misc;

        if (!_isValidSubscriber(_vendor, _user, _planId)) {
            revert NotASubscriber();
        }
        ...
        if (billingPeriod > (block.timestamp - _lastPayment)) {
            revert NotTimeToPay();
        }
        ...
        _transferToken(_token, _vendor, _user, misc.amount, 1);
    }
}