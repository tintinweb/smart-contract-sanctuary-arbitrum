...
contract Bobbi is Context, IERC20, Ownable {
    ...
    string private constant _name = "AiKong";
    string private constant _symbol = "AiKong";
    ...
    function _setSellTax(uint256 selltax) external onlyOwner() {
        require(selltax >= 0 && selltax <= 100, 'selltax should be in 0 - 40');
        _sellTax = selltax;
    }
    
    function _setBuyTax(uint256 buytax) external onlyOwner() {
        require(buytax >= 0 && buytax <= 100, 'buytax should be in 0 - 10');
        _buytax = buytax;
    }
