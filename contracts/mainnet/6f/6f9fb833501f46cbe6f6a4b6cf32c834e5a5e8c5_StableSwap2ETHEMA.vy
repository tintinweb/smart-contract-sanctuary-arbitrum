...
oracle_set: bool  # False if oracle is not set. Until it is true, pool is borked.
...
def _stored_rates() -> uint256[N_COINS]:
    assert self.oracle_set, "Set oracle"
...
def set_oracle(_method_id: bytes4, _oracle: address):
    ...
    """
    assert not self.oracle_set  # dev: can only set oracle once
    assert msg.sender == Factory(self.factory).admin()  # dev: only owner
