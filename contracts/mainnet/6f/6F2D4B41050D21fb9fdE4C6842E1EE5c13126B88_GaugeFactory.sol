...
abstract contract MultiRewardsPoolBase is Reentrancy, IMultiRewardsPool {
  ...
  constructor(address _stake, address _voter, address[] memory _allowedRewardTokens) {
    underlying = _stake;
    voter = _voter;
  ...
  function _notifyRewardAmount(address token, uint amount) internal lock virtual {
    ...
    if (!isRewardToken[token]) {
        require(IVoter(voter).isWhitelisted(token), "rewardTokens tokens must be whitelisted");
        require(rewardTokens.length < MAX_REWARD_TOKENS, "too many rewardTokens tokens");
    }
    if (rewardRate[token] == 0) {
  ...
  function swapOutRewardToken(uint i, address oldToken, address newToken) external {
      require(msg.sender == IVoter(voter).governor(), "only governor");
