...
contract Vester is ReentrancyGuardUpgradeable, OwnableUpgradeable, IVester {
    ...
    function getMaxVestableAmount(address _account) public view override returns (uint256) {
        ...
        uint256 maxVestableAmount = cumulativeReward.add(transferredCumulativeReward);
        uint256 cumulativeRewardDeduction = cumulativeRewardDeductions[_account];

        if (maxVestableAmount < cumulativeRewardDeduction) {
            return 0;
        }
        uint256 maxAmount = maxVestableAmount.sub(cumulativeRewardDeduction);
        if (hasVolumeLimit) {
            return
