...
contract NativeTokenFactory is NonblockingLzApp, ITokenFactory {

    event SetTokenOwner(address indexed token, address indexed owner);

    // Layer zero chain id.
    uint16 public immutable LZ_CHAIN_ID;
    // Token bridge fee in bips.
    uint256 public bridgeFee = 100; // 0.1%;

    mapping(address => uint256) public fee;
    mapping(address token => address owner) public owners;

    address public feeTo;
    address public tokenImplementation;

    constructor(address _endpoint, uint16 _lzChainId) NonblockingLzApp(_endpoint) {
        // lzEndpoint.getChainId() is inconsistent so we need to manually set the chain id.
        LZ_CHAIN_ID = _lzChainId;
        // Set deployer to fee recipient.
        feeTo = msg.sender;
        // Deploy token implementation contract.
        tokenImplementation = address(new NativeToken(address(lzEndpoint)));
    }

    function transferTokenOwnership(address token, address newOwner) external {
        require(owners[token] == msg.sender, "Only owner.");
        _setTokenOwner(token, newOwner);
    }

    function mintToken(address token, address recipient, uint256 amount) external {
        require(owners[token] == msg.sender, "Only owner.");
        NativeToken(token).mint(recipient, amount);
    }
    ...
    function estimateDeployBridgeTokensFee(
        ...
        bytes memory payload = abi.encode(msg.sender, name, symbol, decimals, destinationChains);
        ...
        for (uint256 i = 0; i < destinationChains.length; i++) {
            (nativeFees[i], zroFees[i]) = lzEndpoint.estimateFees(destinationChains[i], address(this), payload, useZro, adapterParams[i]);
    ...
    ) external payable returns (NativeToken token) {
        bytes memory payload = abi.encode(msg.sender, name, symbol, decimals, destinationChains);
        for (uint256 i = 0; i < destinationChains.length; i++) {
    ...
    function _deployToken(bytes memory payload, uint16 srcChainId) internal returns(NativeToken clone) {
        (
            address owner,
            string memory name,
            string memory symbol,
        ...
        bytes32 salt = keccak256(abi.encodePacked(owner, name, symbol, decimals));
        clone = NativeToken(_cloneDeterministic(tokenImplementation, salt));
        clone.init(name, symbol, decimals);
        _setTrustedRemotes(clone, chainIds, srcChainId);
        _setTokenOwner(address(clone), owner);
    }

    function _setTrustedRemotes(NativeToken token, uint16[] memory chainIds, uint16 srcChainid) internal {
        ...
        token.setTrustedRemote(srcChainid, path);
    }
    ...
    function _setTokenOwner(address token, address newOwner) internal {
        owners[token] = newOwner;
        emit SetTokenOwner(token, newOwner);
    }
...
contract NativeToken is OFT {
    ...
    // Mint function (called via the factory).
    ...
    function _creditTo(uint16 _srcChain, address _toAddress, uint256 _amount) internal override returns (uint256 creditedAmount) {
        uint256 feeAmount = _amount * ITokenFactory(owner()).bridgeFee() / 100_000;
        creditedAmount = _amount - feeAmount;
        super._creditTo(_srcChain, ITokenFactory(owner()).feeTo(), feeAmount);
