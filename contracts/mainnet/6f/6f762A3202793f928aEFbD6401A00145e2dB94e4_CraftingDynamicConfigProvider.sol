...
contract ActionPermit is IActionPermit, ManagerModifier {
  using Epoch for uint256;

  IBatchLastActionMarkerStorage public ACTION_STORAGE;
  ILockStorage public LOCK_STORAGE;
  ...
  function checkOwner(
    ...
    return _checkOwner(_tokenAddr, _tokenId, _proof);
  }
  ...
  function checkOwnerBatch(
    ...
    result = new address[](_tokenAddrs.length);
    for (uint i = 0; i < _tokenAddrs.length; i++) {
      result[i] = _checkOwner(_tokenAddrs[i], _tokenIds[i], _proofs[i]);
    }
  }
  ...
  function _checkOwner(
    ...
    return ERC721(_tokenAddr).ownerOf(_tokenId);
  }
  ...
  function getElapsedEpochs(
    ...
    result = new uint[](_tokenAddrs.length);
    ...
    for (uint i = 0; i < _tokenAddrs.length; i++) {
      result[i] = lastMarkers[i].toEpochNumber(_epochConfig);
    }
  }
  ...
  function _checkIfEnoughEpochsElapsed(
    ...
    uint256 _minEpochs,
    uint128 _epochConfig,
    uint256 _lastActionTimeEpoch
  ) private view {
    uint currentEpoch = block.timestamp.toEpochNumber(_epochConfig);
    _lastActionTimeEpoch = _lastActionTimeEpoch.toEpochNumber(_epochConfig);

    if (currentEpoch >= _lastActionTimeEpoch) {
      _lastActionTimeEpoch = currentEpoch - _lastActionTimeEpoch;
    } else {
      revert DuplicateActionAttempt(_tokenAddr, _tokenId);
    }

    if (_lastActionTimeEpoch < _minEpochs) {
      revert InsufficientEpochSpan(_minEpochs, _lastActionTimeEpoch, _tokenAddr, _tokenId);
    }
  }
}
...
contract CraftingDynamicConfigProvider is ICraftingDynamicConfigProvider, ManagerModifier {
  ...
  constructor(
    ...
    voidCostDynamicWeightFactor = 3 * ONE_HUNDRED; // 3x demand
  ...
  function _getConfig(
    ...
    result.voidCraftingCostPercentage =
      ...
      ((voidCostDynamicWeightFactor * _voidDemand) / (_realmDemand)); // base + (void_demand) / _realmDemand
    result.productivityPerCraft = dailyProductivityPool[_labId] / totalDemand; // 400
    result.durabilityLossPercentage = ((ONE_HUNDRED * _labSupply) / totalDemand) + 1;
