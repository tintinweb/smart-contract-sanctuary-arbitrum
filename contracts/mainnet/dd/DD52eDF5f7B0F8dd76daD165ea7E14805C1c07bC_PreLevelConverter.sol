...
import {IPreLevelConverter} from "../interfaces/IPreLevelConverter.sol";
...
contract PreLevelConverter is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, IPreLevelConverter {
        ...
        nonReentrant
    {
        ...
        USDT.safeTransfer(daoTreasury, taxAmountToTreasury);
        LVL.safeTransfer(_to, _preLvlAmount);
        emit Converted(msg.sender, _preLvlAmount, taxAmount - taxAmountToTreasury, taxAmountToTreasury);
    }

    /**
     * @notice send all collected USDT to pool, anyone with interest can call
     */
    function sendToLiquidityPool() external {
        uint256 amount = USDT.balanceOf(address(this));
        if (amount > 0) {
            _sendToLiquidityPool(amount);
        }
    }
