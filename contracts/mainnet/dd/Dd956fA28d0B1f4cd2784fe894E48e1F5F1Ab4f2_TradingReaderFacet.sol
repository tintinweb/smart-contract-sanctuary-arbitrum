    ...
    function traderAssets(address[] memory tokens) external view override returns (TraderAsset[] memory) {
        ...
        if (tokens.length > 0) {
            for (uint i; i < tokens.length; i++) {
                ...
                assets[i * 5] = TraderAsset(AssetPurpose.LIMIT, token, LibLimitOrder.limitOrderStorage().limitOrderAmountIns[token]);
                assets[i * 5 + 1] = TraderAsset(AssetPurpose.PENDING, token, LibTrading.tradingStorage().pendingTradeAmountIns[token]);
                assets[i * 5 + 2] = TraderAsset(AssetPurpose.POSITION, token, LibTrading.tradingStorage().openTradeAmountIns[token]);
                assets[i * 5 + 3] = TraderAsset(AssetPurpose.PREDICTION_PENDING, token, LibPredictUpDown.predictionUpDownStorage().pendingPredictionAmountIns[token]);
                assets[i * 5 + 4] = TraderAsset(AssetPurpose.PREDICTION, token, LibPredictUpDown.predictionUpDownStorage().openPredictionAmountIns[token]);
            }
        }
        return assets;
    }
