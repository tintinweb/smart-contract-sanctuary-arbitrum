...
interface IHarvester {
    function getUserGlobalDeposit(address _user)
        external
        view
        returns (
...
contract TreasureDAO is ERC20Upgradeable, OwnableUpgradeable {
    ...
    IUniswapV2Pair[] public magicswapLPs;
    IBeefyVaultV6 public beefyVaultV6;
    IERC20 public balancerToken;
    ...
    function initialize(address _magic, address _sushiLP, address _miniChefV2, address _harvesterFactory)
        public
        initializer
    {
        __Ownable_init();
    ...
    function initializeV2(
        address[] calldata _magicswapLPs,
        address _beefyVaultV6,
        address _balancerToken,
        address _balancerVault,
        bytes32 _balancerPoolId
    ) public reinitializer(2) {
        beefyVaultV6 = IBeefyVaultV6(_beefyVaultV6);
        balancerToken = IERC20(_balancerToken);
        balancerVault = IBalancerVault(_balancerVault);
        balancerPoolId = _balancerPoolId;

        IUniswapV2Pair[] memory _mlps = new IUniswapV2Pair[](_magicswapLPs.length);
        for (uint256 i = 0; i < _magicswapLPs.length; i++) {
            _mlps[i] = IUniswapV2Pair(_magicswapLPs[i]);
        }
        magicswapLPs = _mlps;
    }
    ...
    function balanceOf(address _account) public view override returns (uint256) {
        return getStakedBalance(_account) + getLPBalance(_account) + getMLPBalance(_account) * 0.5e18 / ONE
            + getBeefyBalance(_account) * 0.5e18 / ONE + getBalancerBalance(_account) * 0.5e18 / ONE
            + getHarvesterBalance(_account);
    }
    ...
    function getMLPBalance(address _account) public view returns (uint256) {
        uint256 _balance;
        for (uint256 i = 0; i < magicswapLPs.length; i++) {
            uint256 _lpBalance = magicswapLPs[i].balanceOf(_account);
            (uint112 _reserve0, uint112 _reserve1,) = magicswapLPs[i].getReserves();
            if (address(magic) == magicswapLPs[i].token0()) {
                _balance += _reserve0 * _lpBalance / magicswapLPs[i].totalSupply();
            } else {
                _balance += _reserve1 * _lpBalance / magicswapLPs[i].totalSupply();
            }
        }
    ...
    function getBeefyBalance(address _account) public view returns (uint256) {
        uint256 _lpBalance = beefyVaultV6.getPricePerFullShare() * beefyVaultV6.balanceOf(_account) / ONE;
        (uint112 _reserve0, uint112 _reserve1,) = sushiLP.getReserves();

        if (address(magic) == sushiLP.token0()) {
            return _reserve0 * _lpBalance / sushiLP.totalSupply();
        } else {
            return _reserve1 * _lpBalance / sushiLP.totalSupply();
        }
    }

    function getBalancerBalance(address _account) public view returns (uint256) {
        uint256 _lpBalance = balancerToken.balanceOf(_account);
        (uint256 _reserve,,,) = balancerVault.getPoolTokenInfo(balancerPoolId, IERC20(address(magic)));
        return _reserve * _lpBalance / balancerToken.totalSupply();
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        depositCur.lockedUntil = uint96(block.timestamp + 7 days);
        depositCur.lock = Lock.ONE_WEEK;

        magic.safeTransferFrom(msg.sender, address(this), _amount);

        emit Deposit(msg.sender, depositId, _amount, Lock.ONE_WEEK);
    }
    ...
    function _addDeposit(address _user)
        internal
        virtual
        returns (DepositInfo storage depositCur, uint256 newDepositId)
    {
