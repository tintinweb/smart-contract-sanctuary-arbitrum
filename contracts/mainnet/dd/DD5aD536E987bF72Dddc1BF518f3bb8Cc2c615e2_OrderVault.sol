...
import {OrderStatus, TriggerInfo, TriggerStatus, PositionTrigger, AddPositionOrder, AddPositionTrigger, DecreasePositionOrder} from "./structs.sol";
...
contract OrderVault is Constants, Initializable, ReentrancyGuardUpgradeable, IOrderVault {
    ...
    mapping(uint256 => AddPositionOrder) private addPositionOrders;
    mapping(uint256 => DecreasePositionOrder) private decreasePositionOrders;
    ...
    uint256 public lastAddPositionTriggerId;
    mapping(uint256 => AddPositionTrigger) private addPositionTriggers; // addPositionTriggerId => AddPositionTrigger
    mapping(uint256 => EnumerableSetUpgradeable.UintSet) private posAddPositionTriggerIds; // posId => addPositionTriggerIds[]

    uint256 private constant MAX_ADD_POSITION_TRIGGER = 5;
    ...
    event CreateAddPositionTrigger(
        uint256 indexed addPositionTriggerId,
        uint256 posId,
        uint256 tokenId,
        bool isTriggerAbove,
        uint256 triggerPrice
    );
    event RemoveAddPositionTrigger(uint256 indexed addPositionTriggerId, uint256 posId);
    ...
    function _onlyPositionVault() private view {
    ...
    function cancelMarketOrder(uint256 _posId) external override onlyPositionVault {
    ...
    function createAddPositionTrigger(
        ...
        require(
            EnumerableSetUpgradeable.length(posAddPositionTriggerIds[_posId]) < MAX_ADD_POSITION_TRIGGER,
            "max trigger exceeded"
        );
        uint256 _lastAddPositionTriggerId = lastAddPositionTriggerId;

        addPositionTriggers[_lastAddPositionTriggerId] = AddPositionTrigger({
            isTriggerAbove: _isTriggerAbove,
            posId: _posId,
        ...
        lastAddPositionTriggerId = _lastAddPositionTriggerId + 1;
        EnumerableSetUpgradeable.add(posAddPositionTriggerIds[_posId], _lastAddPositionTriggerId);
        emit CreateAddPositionTrigger(_lastAddPositionTriggerId, _posId, _tokenId, _isTriggerAbove, _triggerPrice);
    }

    function cancelAddPositionTriggers(uint256[] memory _addPositionTriggerIds) external nonReentrant {
        for (uint256 i; i < _addPositionTriggerIds.length; ++i) {
            uint256 addPositionTriggerId = _addPositionTriggerIds[i];

            AddPositionTrigger memory addPositionTrigger = addPositionTriggers[addPositionTriggerId];
            require(addPositionTrigger.size > 0, "inactive order");

            Position memory position = positionVault.getPosition(addPositionTrigger.posId);
            require(msg.sender == position.owner, "!positionOwner");

            _removeAddPositionTrigger(addPositionTrigger.posId, addPositionTriggerId);
            vault.takeVUSDOut(position.owner, addPositionTrigger.collateral + addPositionTrigger.fee);
        }
    }

    function cancelPosAddPositionTriggers(uint256 _posId, address _positionOwner) external override onlyPositionVault {
        uint256 len = EnumerableSetUpgradeable.length(posAddPositionTriggerIds[_posId]);
        if (len == 0) return;

        uint256 returnAmount;

        uint256[] memory addPositionTriggerIds = new uint256[](len);
        for (uint256 i; i < len; ++i) {
            addPositionTriggerIds[i] = EnumerableSetUpgradeable.at(posAddPositionTriggerIds[_posId], i);
        }
        for (uint256 i; i < len; ++i) {
            uint256 addPositionTriggerId = addPositionTriggerIds[i];
            returnAmount +=
                addPositionTriggers[addPositionTriggerId].collateral +
                addPositionTriggers[addPositionTriggerId].fee;
            _removeAddPositionTrigger(_posId, addPositionTriggerId);
        }

        vault.takeVUSDOut(_positionOwner, returnAmount);
    }
    ...
    function triggerForAddPositionTrigger(uint256 _addPositionTriggerId) external nonReentrant onlyOperator(1) {
        AddPositionTrigger memory addPositionTrigger = addPositionTriggers[_addPositionTriggerId];
        require(addPositionTrigger.size > 0, "inactive trigger");

        Position memory position = positionVault.getPosition(addPositionTrigger.posId);
        require(position.size > 0, "inactive position");

        uint256 price = priceManager.getLastPrice(position.tokenId);
        if (addPositionTrigger.isTriggerAbove) {
            require(price >= addPositionTrigger.triggerPrice, "lastPrice is below triggerPrice");
        } else {
            require(price <= addPositionTrigger.triggerPrice, "lastPrice is above triggerPrice");
        }

        positionVault.increasePosition(
            addPositionTrigger.posId,
            position.owner,
            position.tokenId,
            position.isLong,
            price,
            addPositionTrigger.collateral,
            addPositionTrigger.size,
            addPositionTrigger.fee
        );

        _removeAddPositionTrigger(addPositionTrigger.posId, _addPositionTriggerId);
    }
    ...
    function selfExecuteDecreasePositionOrder(uint256 _posId) external nonReentrant {
        ...
        DecreasePositionOrder memory decreasePositionOrder = decreasePositionOrders[_posId];
        require(
        ...
        positionVault.decreasePositionByOrderVault(_posId, price, decreasePositionOrder.size);
    ...
    function _removeAddPositionTrigger(uint256 _posId, uint256 _addPositionTriggerId) internal {
        delete addPositionTriggers[_addPositionTriggerId];
        EnumerableSetUpgradeable.remove(posAddPositionTriggerIds[_posId], _addPositionTriggerId);
        emit RemoveAddPositionTrigger(_addPositionTriggerId, _posId);
    }
    ...
    function getAddPositionTrigger(uint256 _addPositionTriggerId) external view returns (AddPositionTrigger memory) {
        return addPositionTriggers[_addPositionTriggerId];
    }
    ...
    function getPosAddPositionTriggerIds(
        ...
        uint256 length = EnumerableSetUpgradeable.length(posAddPositionTriggerIds[_posId]);
        _addPositionTriggerIds = new uint256[](length);

        for (uint256 i; i < length; ++i) {
            _addPositionTriggerIds[i] = EnumerableSetUpgradeable.at(posAddPositionTriggerIds[_posId], i);
        }
    }
