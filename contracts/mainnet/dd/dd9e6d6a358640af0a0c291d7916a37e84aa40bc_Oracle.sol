...
contract Oracle is IOracle, DefaultAccessControl {
    ...
    function quote(
        ...
        for (uint256 i = 0; i < length; i++) {
            if (tokenAmounts[i] > 0) nonZeroCount++;
        }

        while (nonZeroCount > 1 || (nonZeroCount == 1 && tokenAmounts[length - 1] == 0)) {
                ...
                for (uint256 j = 0; j < subTokens.length; j++) {
                    ...
                    for (uint256 index = 0; index < length; index++) {
                        if (tokensInOrderOfDifficulty[index] == subToken) {
                            ...
                            if (subTokenAmounts[j] > 0 && tokenAmounts[index] == 0) nonZeroCount++;
...
library CommonLibrary {
    function getContractBytecodeHash(address addr) external view returns (bytes32 bytecodeHash) {
        assembly {
    ...
    function sort(address[] calldata array) public pure returns (address[] memory) {
    ...
    function sortAndMerge(address[] calldata a, address[] calldata b) public pure returns (address[] memory array) {
