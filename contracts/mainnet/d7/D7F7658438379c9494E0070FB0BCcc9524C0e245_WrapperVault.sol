...
abstract contract ERC20Facet is Initializable {
    ...
    function _spendAllowance(address owner, address spender, uint256 amount) internal virtual {
        uint256 currentAllowance = ERC20Storage.layout().allowance[owner][spender];
...
contract WrapperVault is Initializable, ERC20Facet, ReentrancyGuardUpgradeable, FactorGauge, FactorBoostReward {
    ...
    function deposit(uint256 amount, address receiver) external nonReentrant {
        ...
        _mint(receiver, afterBalance - beforeBalance);
        emit Deposit(msg.sender, receiver, amount, afterBalance - beforeBalance);
    }

    function withdraw(uint256 shares, address receiver, address owner) external nonReentrant {
        if (shares > balanceOf(owner) || balanceOf(owner) == 0) revert INSUFFICIENT_BALANCE();
        if (msg.sender != owner) {
        ...
        emit Withdraw(msg.sender, receiver, owner, shares, afterBalance - beforeBalance);
    }
