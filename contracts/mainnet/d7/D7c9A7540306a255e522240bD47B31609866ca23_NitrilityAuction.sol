...
interface INitrilityFactory {
    ...
    function purchaseLicense(
        ...
        address buyerAddr,
        NitrilityCommon.TemplateType memory templateData,
        NitrilityCommon.LicensingType licensingType,
        NitrilityCommon.EventTypes eventType,
        NitrilityCommon.AccessLevel accessLevel,
        uint256 price,
        uint256 amount
    ) external;
...
contract NitrilityAuction is Ownable(msg.sender), ReentrancyGuard {
    ...
    // license id => offer id
    mapping(uint256 => EnumerableSet.UintSet) offerIdsOfLicense;
    ...
    // check if the target address in the array of addresses
    ...
    function checkLicenseValid(
        NitrilityCommon.TemplateType memory templateType
    ) internal view {
        require(
    ...
    function calculateTotalPrice(
        LicensesData[] memory licenseDatas
        ...
        for (uint256 i = 0; i < licenseDatas.length; i++) {
            ...
            NitrilityCommon.TemplateType memory templateData = getTemplateData(
                licenseDatas[i].license,
                licenseDatas[i].licensingType
            );
            require(
                templateData.accessLevel == licenseDatas[i].accessLevel ||
                    templateData.accessLevel == NitrilityCommon.AccessLevel.Both
            );

            if (
                licenseDatas[i].accessLevel ==
                NitrilityCommon.AccessLevel.Exclusive
            ) {
    ...
    function getTemplateData(
        ...
        NitrilityCommon.LicensingType licensingType
    ) public pure returns (NitrilityCommon.TemplateType memory) {
        NitrilityCommon.TemplateType memory templateData;
        if (licensingType == NitrilityCommon.LicensingType.Creator) {
            templateData = license.creator;
        } else {
            if (licensingType == NitrilityCommon.LicensingType.Advertisement) {
                templateData = license.advertisement;
            } else {
                if (licensingType == NitrilityCommon.LicensingType.TvSeries) {
                    templateData = license.tvSeries;
                } else {
                    if (licensingType == NitrilityCommon.LicensingType.Movie) {
                        templateData = license.movie;
                    } else {
                        if (
                            licensingType ==
                            NitrilityCommon.LicensingType.VideoGame
                        ) {
                            templateData = license.videoGame;
                        } else {
                            if (
                                licensingType ==
                                NitrilityCommon.LicensingType.AiTraining
                            ) {
                                templateData = license.aiTraining;
                            } else {
                                revert("Invalid LicensingType");
                            }
                        }
                    }
                }
    ...
    function createOfferData(
        ...
        uint256 offerCount = offerIdsOfBuyer[buyerAddr].length();
        bool bFound = false;
        for (uint256 i = 0; i < offerCount; i++) {
            uint256 offerId = offerIdsOfBuyer[buyerAddr].at(i);
            if (
                idToOffer[offerId].listedId == listedId &&
                idToOffer[offerId].eventType ==
                NitrilityCommon.EventTypes.OfferPending
            ) {
                bFound = true;
            }
        }
        require(!bFound, "Already placed a bid on this license");
        ...
        offerIdsOfLicense[listedId].add(currentOfferId);
    ...
    // adding extra funds on buyer side
    function editOffer(
        uint256 offerId,
        uint256 offerPrice,
        uint256 offerDuration,
        ...
        require(
            idToOffer[offerId].buyerAddr == msg.sender,
            "There is no offer"
        );

        if (offerPrice != idToOffer[offerId].offerPrice) {
            if (offerPrice < idToOffer[offerId].offerPrice) {
                INitrilityFactory(_nitrilityFactory).reFundOffer(
                    msg.sender,
                    idToOffer[offerId].offerPrice - offerPrice
                );
            } else {
                payable(_nitrilityFactory).transfer(
                    offerPrice - idToOffer[offerId].offerPrice
                );
            }
        }

        idToOffer[offerId].offerPrice = offerPrice;
        idToOffer[offerId].offerDuration = offerDuration;
        idToOffer[offerId].tokenURI = tokenURI;

        emit OfferEvent(
            offerId,
            idToOffer[offerId].listedId,
            idToOffer[offerId].buyerAddr,
            offerPrice,
            offerDuration,
            tokenURI,
            NitrilityCommon.EventTypes.OfferEdited,
            idToOffer[offerId].licensingType,
            idToOffer[offerId].accessLevel
        );
    }
    ...
    function acceptOffer(
        ...
        require(
            offerData.eventType == NitrilityCommon.EventTypes.OfferPending,
            "Invalid Sale Type"
        );

        offerData.eventType = NitrilityCommon.EventTypes.OfferAccepted;

        INitrilityFactory(_nitrilityFactory).purchaseLicense(
            offerData.listedId,
            artistId,
            offerData.tokenURI,
            offerData.buyerAddr,
    ...
    // decline the upcoming offer on buyer side
    function removeOffer(uint256 offerId) external {
        ...
        require(
            msg.sender == idToOffer[offerId].buyerAddr,
            "Only bidder can call"
        );

        idToOffer[offerId].eventType = NitrilityCommon.EventTypes.OfferDeleted;

        OfferData storage offer = idToOffer[offerId];
        emit OfferEvent(
            offerId,
            offer.listedId,
            offer.buyerAddr,
            ...
            offer.offerDuration,
            offer.tokenURI,
            NitrilityCommon.EventTypes.OfferDeleted,
            idToOffer[offerId].licensingType,
            idToOffer[offerId].accessLevel
        );
        INitrilityFactory(_nitrilityFactory).reFundOffer(
            offer.buyerAddr,
            idToOffer[offerId].offerPrice
        );
    }

    // decline the upcoming offer on seller side
    function denyOffer(uint256 offerId) external onlyOwner {
        require(
            idToOffer[offerId].eventType ==
                NitrilityCommon.EventTypes.OfferPending,
            "Invalid Sale Type"
        ...
        idToOffer[offerId].eventType = NitrilityCommon.EventTypes.OfferDeclined;
        ...
        emit OfferEvent(
            ...
            offer.offerDuration,
            offer.tokenURI,
            NitrilityCommon.EventTypes.OfferDeclined,
            offer.licensingType,
            offer.accessLevel
        );
        INitrilityFactory(_nitrilityFactory).reFundOffer(
    ...
    function fetchOffersOfSeller(
        ...
        uint256 itemCount = offerIdsOfLicense[_listedId].length();
        OfferData[] memory offers = new OfferData[](itemCount);
        uint256 currentIndex = 0;
        for (uint256 i = 0; i < itemCount; i++) {
            uint256 offerId = offerIdsOfLicense[_listedId].at(i);
    ...
    function fetchAllOffersOfSeller(
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            ...
            OfferData storage currentItem = idToOffer[offerId];
            if (
                currentItem.eventType == NitrilityCommon.EventTypes.OfferPending
            ) {
                offers[currentIndex] = currentItem;
                currentIndex++;
            }
    ...
    function fetchOfferOfBuyer(
        ...
        uint256 bidCount = offerIdsOfBuyer[_buyer].length();
        OfferData[] memory offers = new OfferData[](bidCount);
        uint256 currentIndex = 0;
        for (uint256 i = 0; i < bidCount; i++) {
            uint256 offerId = offerIdsOfBuyer[_buyer].at(i);
            OfferData storage currentItem = idToOffer[offerId];
            ...
            if (
                ...
                currentItem.eventType ==
                NitrilityCommon.EventTypes.OfferPending &&
                currentItem.listedId == _listedId
            ) {
                offers[currentIndex] = currentItem;
                currentIndex++;
            }
    ...
    function purchaseLicenses(
        ...
        for (uint256 i = 0; i < licenseDatas.length; i++) {
            ...
            if (
                licenseDatas[i].accessLevel ==
                NitrilityCommon.AccessLevel.NonExclusive
            ) {
                licensePrice = templateData.fPrice;
            } else {
                licensePrice = templateData.sPrice;
            ...
            if (
                ...
                INitrilityFactory(_nitrilityFactory).purchaseLicense(
                    licenseDatas[i].license.listedId,
                    licenseDatas[i].license.artistId,
                    licenseDatas[i].newTokenURI,
                    msg.sender,
                    templateData,
                    licenseDatas[i].licensingType,
                    ...
                    licenseDatas[i].accessLevel,
                    licensePrice,
                    licenseDatas[i].counts
                );
            } else {
                createOfferData(
                    licenseDatas[i].license.listedId,
                    licenseDatas[i].offerDuration,
                    licenseDatas[i].newTokenURI,
                    licenseDatas[i].licensingType,
                    licenseDatas[i].accessLevel,
                    msg.sender,
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "TemplateType(uint256 fPrice,uint256 sPrice,uint256 tPrice,uint256 listingFormatValue,bool infiniteSupply,bool infiniteListingDuration,bool infiniteExclusiveDuration,uint256 accessLevel,uint256 listingStartTime,uint256 listingEndTime,uint256 exclusiveEndTime,DiscountCode discountCode,uint256 listed)"
                        ),
                        template.fPrice,
                        template.sPrice,
                        template.tPrice,
