...
contract WanderingMerchant is Initializable, WanderingMerchantContracts {
    ...
    function _fulfillRecipe(FulfillRecipeParams calldata _params) private {
        ...
        if(_recipeInfo.inputType == InputType.CONSUMABLE) {
            ...
            consumable.adminBurn(msg.sender, _params.inputTokenId, _recipeInfo.inputAmount);
        } else if(_recipeInfo.inputType == InputType.AUXILIARY_LEGION) {
            ...
            require(_recipeInfo.inputAmount == 1, "Cannot require more than one aux legions");

            legion.adminBurn(msg.sender, _params.inputTokenId);
        } else if(_recipeInfo.inputType == InputType.CORRUPTION) {
            require(_params.inputTokenId == _recipeInfo.inputTokenId, "Input token id does not match");

            corruption.burn(msg.sender, _recipeInfo.inputAmount);
        } else if(_recipeInfo.inputType == InputType.UNIVERSAL_KEYS) {
            require(_params.inputTokenId == _recipeInfo.inputTokenId, "Input token id does not match");

            universalKeys.safeTransferFrom(msg.sender, address(0xdead), _params.inputTokenId, _recipeInfo.inputAmount, "");
        } else if(_recipeInfo.inputType == InputType.BALANCER_CRYSTAL) {
            require(_params.inputTokenId == _recipeInfo.inputTokenId, "Input token id does not match");

            balancerCrystal.adminBurn(msg.sender, _params.inputTokenId, _recipeInfo.inputAmount);
        } else {
        ...
        for(uint32 i = 0; i < _recipeInfo.numberOfOutputs; i++) {
            ...
            if(_output.outputType == OutputType.CONSUMABLE) {
                ...
                IERC1155Upgradeable(_output.outputAddress).safeTransferFrom(_output.transferredFrom, msg.sender, _output.tokenId, _output.amount, "");
            } else {
...
abstract contract WanderingMerchantContracts is Initializable, WanderingMerchantState {
    ...
    external onlyAdminOrOwner
    {
        ...
        universalKeys = IERC1155Upgradeable(_universalKeysAddress);
    ...
    function areContractsSet() public view returns(bool) {
        return address(consumable) != address(0)
            ...
            && address(universalKeys) != address(0)
...
abstract contract WanderingMerchantState is Initializable, IWanderingMerchant, AdminableUpgradeable {
    ...
    IERC1155Upgradeable public universalKeys;
...
struct RecipeInfo {
    ...
    // If legion or corruption, will be 0.
    //
    uint32 inputTokenId;
    InputType inputType;
