...
contract BondController is IBondController, Initializable, ACLedUUPS {
    ...
    function initialize(
        ...
        bondToken = IERC20Upgradeable(bondToken_);
    }

    /// @inheritdoc IBondController
    function deposit(uint256 amount, IPermit.Permit memory permit) public {
        if (permit.usePermit) {
            IERC20PermitUpgradeable(address(bondToken)).permit(
                msg.sender,
                address(this),
                amount,
                permit.deadline,
    ...
    function lockTokensFrom(
        ...
        if (incentivePool < incentiveAmount) {
            revert InsufficientIncentiveBalance();
        }

        balances[user] -= amount;
        lockedPool += (amount + incentiveAmount);
        incentivePool -= incentiveAmount;
    }

    /// @inheritdoc IBondController
    function unlockTokensTo(
        address user,
        uint256 amount
    ) public onlyBondingManager {
        if (amount == 0) {
        ...
        if (lockedPool < amount) {
            revert InvalidAmount();
        }

        lockedPool -= amount;

        bondToken.safeTransfer(user, amount);
    }

    // TODO : Add necessary checks
    function unlockIncentives(uint amount) public onlyBondingManager {
        if (amount == 0) {
            return;
        }

        if (amount > lockedPool) {
            revert InvalidAmount();
        }

        lockedPool -= amount;
        incentivePool += amount;
    }

    function depositIncentives(uint256 amount) public {
        if (amount == 0) {
            revert InvalidAmount();
        }
    ...
    function withdrawIncentives(
        ...
        if (amount == 0 || amount > incentivePool) {
            revert InvalidAmount();
        }
    ...
    function _deposit(address from, uint256 amount) internal {
        ...
        bondToken.safeTransferFrom(from, address(this), amount);
    }
}
