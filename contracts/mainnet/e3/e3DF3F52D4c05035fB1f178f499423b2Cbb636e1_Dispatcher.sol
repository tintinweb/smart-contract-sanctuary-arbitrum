...
abstract contract DAOAccessControlled is Context {
    ...
    modifier onlyDispatcher() {
        require(authority.dispatcher() == _msgSender(), "UNAUTHORIZED");
        _;
    }
    ...
    function isTrustedForwarder(address forwarder) public view returns (bool) {
        return address(authority) != address(0) && forwarder == authority.forwarder();
    }
...
contract Dispatcher is IDispatcher,Initializable, DAOAccessControlled {
    ...
    mapping(address => OfferContext) offerContext;
    ...
    */
    function updateActiveReservationsCount() public {
        ...
        for (uint256 i = earliestNonExpiredIndex; i < reservationIds.current() - 1; i++) {
    ...
    */
    function mintLinked( address _collectible, address _patron, uint256 _expiry, bool _transferable) public virtual override {
        ...
        for(uint256 i=0; i < linkedCollectibles.length; i++) {
            ...
            if(
                ICollectible(_collectible).balanceOf(_msgSender()) > 0 &&
                linkedCollectibe.isActive() &&
                linkedCollectibe.mintWithLinked() &&
                linkedCollectibe.balanceOf(_msgSender()) == 0
            ) {
