...
interface IContractMetadata {
     ...
     * @param contractURI The new contract metadata URI.
     */
    event ContractURIChanged(string contractURI);

    /**
     * @dev Sets a new URI for the contract metadata.
     * @param contractURI_ The URI to set for the contract metadata.
     */
    function setContractMetadata(string memory contractURI_) external;

    /**
     * @dev Retrieves the current URI for the contract metadata.
     * @return The URI representing the contract metadata.
     */
...
/**
 * @title IQRC721
 * @notice Interface for a QRC721 token contract. This contract is ERC721-compliant and supports additional features.
 */
interface IQRC721 is IERC721EnumerableUpgradeable {
    /**
     * @notice Constructor parameters for the IQRC721 implementation contract.
     * @param name The name of the QRC721 token.
     * @param symbol The symbol of the QRC721 token.
     * @param contractURI The URI for the contract metadata.
     * @param baseURI The base URI for the token metadata.
     * @param totalSupplyCap The maximum total supply for the QRC721 token.
     * @param isBaseURIToken If true, the BaseURI will be used as the tokenURI for any token.
     */
    ...
    /**
     * @notice Mints a new QRC721 token to a specified receiver address.
     * @param receiver_ The address of the receiver for the new QRC721 token.
     * @param tokenId_ The ID of the new QRC721 token.
     * @param tokenURI_ The URI for the metadata of the new QRC721 token.
     */
    function mintTo(address receiver_, uint256 tokenId_, string calldata tokenURI_) external;

    /**
     * @notice Burns an existing QRC721 token owned by a specified payer address.
     * @param payer_ The address of the payer that owns the QRC721 token to be burned.
     * @param tokenId_ The ID of the QRC721 token to be burned.
     */
    function burnFrom(address payer_, uint256 tokenId_) external;

    /**
     * @notice Sets the base URI for the QRC721 token metadata.
     * @param baseURI_ The new base URI for the token metadata.
     */
    function setBaseURI(string calldata baseURI_) external;

    /**
     * @notice Sets the token URI for an existing QRC721 token.
     * @param tokenId_ The ID of the QRC721 token to update.
...
abstract contract ContractMetadata is IContractMetadata, Initializable {
    ...
    modifier onlyChangeMetadataPermission() virtual {
        // Ensures only authorized changes to the contract metadata.
        _;
    }

    /**
     * @notice Initializes the contract with a specified metadata URI.
     * @param contractURI_ The URI to be set for the contract metadata.
     */
    ...
    ) external onlyChangeMetadataPermission {
        _setContractURI(contractURI_);
    }
...
import {IQRC721} from "../interfaces/tokens/IQRC721.sol";

import {ContractMetadata} from "../metadata/ContractMetadata.sol";

/**
 * @title QRC721
 * @notice Implements a standard ERC721 token with additional features:
 *      - Minting and burning capabilities
 *      - Total supply cap enforcement
 *      - Contract metadata extension
 *      - Uniform Base URI option for all tokens
 */
contract QRC721 is
    IQRC721,
    ...
    ContractMetadata
{
     ...
     * @dev Used as the default URI for all tokens if isBaseURIToken is true.
     */
    ...
    /**
     * @notice Flag indicating if the token uses a base URI for all tokens.
     * @dev When true, the base URI is returned for all token URIs.
     */
    bool public isBaseURIToken;

    /**
     * @notice Modifier to allow only the owner to change metadata.
     */
     ...
     */
    function __QRC721_init(ConstructorParams calldata params_) external initializer {
        __Ownable_init();
        ...
        isBaseURIToken = params_.isBaseURIToken;
    }
     ...
     * @inheritdoc IQRC721
     */
    function burnFrom(address account_, uint256 tokenId_) external override {
        require(
            ownerOf(tokenId_) == account_ &&
                (account_ == _msgSender() ||
     ...
     *
     * @dev If isBaseURIToken is true, the base URI is returned.
     */
    function tokenURI(
        uint256 tokenId
    )
