/**
 *Submitted for verification at Arbiscan on 2023-05-14
*/
...
contract CashbackLock is Ownable {
    ...
    constructor(IERC20 _PRISM, IxPRISM _xPRISM) {
        PRISM = _PRISM;
        xPRISM = _xPRISM;
    ...
    function lockDiscount(uint256 amount) external {
        ...
        PRISM.transferFrom(msg.sender, address(this), amount);
        PRISM.approve(address(xPRISM), amount);

        uint256 availableCashback = xPRISM.balanceOf(address(this));
        xPRISM.lock(amount);

        uint256 cashbackAmount = 0;

        if (amount > 1 * (10 ** 18)) {
            cashbackAmount = amount.mul(cashbackRate).div(1000); // 1000 is used instead of 100 due to the RATE_MULTIPLIER
            if (cashbackAmount > availableCashback) {
                cashbackAmount = availableCashback;
            }
            if (cashbackRate >= 10) { // Only decrease the rate if the rate is 1% or higher (10 * 0.1%)
                cashbackRate = cashbackRate.sub(10); // Decrease by 1% (10 * 0.1%)
            } else {
                cashbackRate = 0;
            }
        }
        ...
        lastLockTimestamp = block.timestamp;
        xPRISM.transfer(msg.sender, amount.add(cashbackAmount));
    }

    function supplyxPRISM(uint256 amount) external {
        xPRISM.transferFrom(msg.sender, address(this), amount);
    }

    function retrievexPRISM(uint256 amount) external onlyOwner {
        xPRISM.transfer(msg.sender, amount);
    }
}