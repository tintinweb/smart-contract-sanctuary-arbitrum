...
contract SteadefiLiquidityWarehouse is LiquidityWarehouse {
        ...
        returns (bool)
    {
        ...
        uint256 lendingVaultTotalSupply = IERC20(withdrawTarget).totalSupply();
        uint256 lendingVaultTotalAsset = lendingVault.totalAsset();
        uint256 liquidityWarehouseShareBalance = IERC20(withdrawTarget).balanceOf(address(this));
        uint256 maxBurnableShares =
            lendingVault.totalAvailableAsset() * lendingVaultTotalSupply / lendingVaultTotalAsset;

        // Required to handle the case where the liquidity warehouse cannot burn all of it's
        // shares due to there not being sufficient liquidity in the Steadefi Lending Vault
        uint256 shareAmt = Math.min(liquidityWarehouseShareBalance, maxBurnableShares);
        if (withdrawAmount == type(uint256).max) {
            lendingVault.withdraw(shareAmt, shareAmt * lendingVaultTotalAsset / lendingVaultTotalSupply);
            return false; // Continue withdrawing from next lending vault
        }

        uint256 withdrawAmtShares = withdrawAmount * lendingVaultTotalSupply / lendingVaultTotalAsset;
        shareAmt = Math.min(withdrawAmtShares, shareAmt);
        lendingVault.withdraw(shareAmt, shareAmt * lendingVaultTotalAsset / lendingVaultTotalSupply);

        // Only continue withdrawing from the next lending vault if we could not get
        // the amount of assets we wanted to withdraw
        return shareAmt == withdrawAmtShares;
    }

    /// @inheritdoc LiquidityWarehouse
    function _getDeployedAssetValue(address withdrawTarget) internal view override returns (uint256) {
        ILendingVault lendingVault = ILendingVault(withdrawTarget);
        return lendingVault.totalAsset() * IERC20(withdrawTarget).balanceOf(address(this))
            / IERC20(withdrawTarget).totalSupply();
    }
    ...
    AccessControlDefaultAdminRules
{
    ...
    function _withdrawFromTargets(uint256 withdrawAmount, bytes memory data) internal returns (uint256) {
        ...
        while (!shouldStopWithdrawing && idx < s_withdrawTargets.values().length) {
            address withdrawTarget = s_withdrawTargets.at(idx);
            shouldStopWithdrawing = _withdrawFromTarget(missingWithdrawableAmount, withdrawTarget, data);
