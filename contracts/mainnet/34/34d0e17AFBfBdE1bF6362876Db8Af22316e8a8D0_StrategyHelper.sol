 ...
 *
 * Risk:
 * - This strategy keep tracking current price with narrow range that potentially cause huge divergence loss depending on market condition.
 *
 * @dev some internal calls have "this." prefix, but this is for testing that utilize mockCall powered by Foundry.
...
contract StrategyHelper {
    ...
    IOrangeDopexV2LPAutomator public immutable automator;
    ...
    constructor(IOrangeDopexV2LPAutomator automator_) {
    ...
    function checkLiquidizePooledAssets() external view returns (bool, bytes memory) {
        ...
        (
            IOrangeDopexV2LPAutomator.RebalanceTickInfo[] memory _burnTicksInfo,
            ,
            uint256 _totalUnburnableValueInAssets
        ) = this.getBurnableTicksInfo(_burnTicks);

        IOrangeDopexV2LPAutomator.RebalanceTickInfo[] memory _stayTicksInfo = this.getActiveTicksInfo(_stayTicks);
        ...
        IOrangeDopexV2LPAutomator.RebalanceTickInfo[]
            memory _mintTicksInfo = new IOrangeDopexV2LPAutomator.RebalanceTickInfo[](_mintTicks.length);
        for (uint256 i; i < _mintTicks.length; i++) {
            _mintTicksInfo[i] = IOrangeDopexV2LPAutomator.RebalanceTickInfo(_mintTicks[i], _targetLiquidity);
        }

        //Classify the stay ticks
        (
            IOrangeDopexV2LPAutomator.RebalanceTickInfo[] memory newMintTicksInfo,
            IOrangeDopexV2LPAutomator.RebalanceTickInfo[] memory newBurnTicksInfo
        ...
        IOrangeDopexV2LPAutomator.RebalanceSwapParams memory _swapParams = automator
            .calculateRebalanceSwapParamsInRebalance(newMintTicksInfo, newBurnTicksInfo);
    ...
    function checkBurnOuterLiquidity() external view returns (bool, bytes memory) {
        ...
        (IOrangeDopexV2LPAutomator.RebalanceTickInfo[] memory _burnTicksInfo, , ) = this.getBurnableTicksInfo(
            _burnTicks
        ); //"this." is used for testing purpose. detail is above.

        if (_burnTicksInfo.length != 0) {
            //Prepare Gelato package
            IOrangeDopexV2LPAutomator.RebalanceTickInfo[] memory _mintTicksInfo; //0
            IOrangeDopexV2LPAutomator.RebalanceSwapParams memory _swapParams = automator
     ...
     * @dev This function first retrieves the current active ticks from the OrangeDopexV2LPAutomator contract. It then calculates
     ...
     *      it validates the mint ticks to ensure they meet criteria by validateMintTicks().
     ...
     *      tick according to the . For negative ticks, the calculation behaves similar to the `ceil()` function rather than `floor()`.
    ...
    function getActiveTicksInfo(
        int24[] memory _activeTicks
    ) public view returns (IOrangeDopexV2LPAutomator.RebalanceTickInfo[] memory _activeTicksInfo) {
        _activeTicksInfo = new IOrangeDopexV2LPAutomator.RebalanceTickInfo[](_activeTicks.length);

        for (uint256 i; i < _activeTicks.length; i++) {
            uint128 _tickLiquidity = automator.getTickAllLiquidity(_activeTicks[i]);
            _activeTicksInfo[i] = IOrangeDopexV2LPAutomator.RebalanceTickInfo(_activeTicks[i], _tickLiquidity);
        }
    }
    ...
    {
        IOrangeDopexV2LPAutomator.RebalanceTickInfo[]
            memory tempBurnTicksInfo = new IOrangeDopexV2LPAutomator.RebalanceTickInfo[](_burnTicks.length);
        ...
        for (uint256 i; i < _burnTicks.length; i++) {
            ...
            if (_burnableLiquidity > 0) {
                tempBurnTicksInfo[_burnTicksLength] = IOrangeDopexV2LPAutomator.RebalanceTickInfo(
                    _burnTicks[i],
                    _burnableLiquidity
                );
                _burnTicksLength++;
        ...
        _burnTicksInfo = new IOrangeDopexV2LPAutomator.RebalanceTickInfo[](_burnTicksLength);
    ...
    function classifyStayTicks(
        IOrangeDopexV2LPAutomator.RebalanceTickInfo[] memory _mintTicksInfo,
        IOrangeDopexV2LPAutomator.RebalanceTickInfo[] memory _burnTicksInfo,
        IOrangeDopexV2LPAutomator.RebalanceTickInfo[] memory _stayTicksInfo,
        uint128 _targetLiquidity
    )
        public
        pure
        returns (
            IOrangeDopexV2LPAutomator.RebalanceTickInfo[] memory,
            IOrangeDopexV2LPAutomator.RebalanceTickInfo[] memory
        )
    {
        ...
        IOrangeDopexV2LPAutomator.RebalanceTickInfo[]
            memory newMintTicksInfo = new IOrangeDopexV2LPAutomator.RebalanceTickInfo[](
                _mintTicksInfo.length + mintExtensionCounter
            );
        IOrangeDopexV2LPAutomator.RebalanceTickInfo[]
            memory newBurnTicksInfo = new IOrangeDopexV2LPAutomator.RebalanceTickInfo[](
        ...
        for (uint256 i; i < _stayTicksInfo.length; i++) {
            //prevent mint/burn 0 liquidity
            if (_targetLiquidity < _stayTicksInfo[i].liquidity) {
                newBurnTicksInfo[newBurnIndex++] = IOrangeDopexV2LPAutomator.RebalanceTickInfo(
                    _stayTicksInfo[i].tick,
                    _stayTicksInfo[i].liquidity - _targetLiquidity
                );
            } else if (_targetLiquidity > _stayTicksInfo[i].liquidity) {
                newMintTicksInfo[newMintIndex++] = IOrangeDopexV2LPAutomator.RebalanceTickInfo(
                    _stayTicksInfo[i].tick,
