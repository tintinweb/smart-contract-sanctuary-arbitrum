/**
 *Submitted for verification at Arbiscan on 2022-10-10
*/
...
// File: contracts/BIFI/interfaces/curve/IHelper.sol
...
// File: contracts/BIFI/strategies/Beethovenx/BeefyBalancerStructs.sol
...
// File: contracts/BIFI/strategies/Beethovenx/StrategyBalancerMultiRewardGaugeUniV3.sol
...
contract StrategyBalancerMultiRewardGaugeUniV3 is StratFeeManager {
    ...
    function swapRewardsToNative() internal {
        ...
        if (outputBal > 0) {
            IBalancerVault.BatchSwapStep[] memory _swaps = buildSwapStructArray(outputToNativeRoute, outputBal);
            balancerSwap(_swaps, outputToNativeAssets, int256(outputBal));
        }
        // extras
        for (uint i; i < rewardTokens.length; i++) {
            ...
            if (bal >= rewards[rewardTokens[i]].minAmount) {
                if (rewards[rewardTokens[i]].assets[0] != address(0)) {
                    ...
                    IBalancerVault.BatchSwapStep[] memory _swaps = buildSwapStructArray(swapInfo, bal);
                    balancerSwap(_swaps, rewards[rewardTokens[i]].assets, int256(bal));
                } else {
                    _swapV3(rewards[rewardTokens[i]].routeToNative, bal);
                }
            }
        }
    }
    ...
    function addLiquidity() internal {
        ...
        if (native != input.input) {
            IBalancerVault.BatchSwapStep[] memory _swaps = buildSwapStructArray(nativeToInputRoute, nativeBal);
            balancerSwap(_swaps, nativeToInputAssets, int256(nativeBal));
        }

        uint256 inputBal = IERC20(input.input).balanceOf(address(this));
        balancerJoin(IBalancerPool(want).getPoolId(), input.input, inputBal);
    }

    function balancerJoin(bytes32 _poolId, address _tokenIn, uint256 _amountIn) internal {
        (address[] memory lpTokens,,) = IBalancerVault(unirouter).getPoolTokens(_poolId);
    ...
    function _swapV3(bytes memory _path, uint256 _amount) internal returns (uint256 amountOut) {
        ...
        return IUniswapRouterV3WithDeadline(uniswapRouter).exactInput(swapParams);
    }
    ...
    function balancerSwap(IBalancerVault.BatchSwapStep[] memory _swaps, address[] memory _route, int256 _amountIn) internal returns (int256[] memory) {
        ...
        return IBalancerVault(unirouter).batchSwap(swapKind, _swaps, _route, funds, limits, block.timestamp);
    }
