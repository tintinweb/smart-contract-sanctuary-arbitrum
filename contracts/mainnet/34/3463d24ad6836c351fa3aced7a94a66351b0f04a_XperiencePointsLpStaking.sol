...
contract XperiencePointsLpStaking is UpgradeableOperable, ReentrancyGuardUpgradeable {
    /*´:°•.°+.*•´.*:˚.°*.˚•´.°:°•.°•.*•´.*:˚.°*.˚•´.°:°•.°+.*•´.*:*/
    /*                        STATE VARIABLES                     */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
    ...
    ICamelotPositionManager public cpm; //camelot position manager
    IRewardsCollectorFactory public rewardsCollectorFactory;
    ...
    mapping(uint256 => Redemption) public redeemInfo;

    /*´:°•.°+.*•´.*:˚.°*.˚•´.°:°•.°•.*•´.*:˚.°*.˚•´.°:°•.°+.*•´.*:*/
    /*                        EVENTS                              */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
    event Unstake(uint256 indexed tokenId, uint256 redemptionAmount);
    event UnstakeFinalized(uint256 indexed tokenId, uint256 redemptionAmount);
    event UnstakeCancelled(uint256 indexed tokenId);

    /*´:°•.°+.*•´.*:˚.°*.˚•´.°:°•.°•.*•´.*:˚.°*.˚•´.°:°•.°+.*•´.*:*/
    /*                        ERRORS                              */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
    error CallerIsNotOwner();
    error TokenNotInNitroPool();
    ...
    function init(ICamelotPositionManager _cpm, IRewardsCollectorFactory _rewardsCollectorFactory)
        external
        initializer
    {
        ...
        minRedeemRatio = 67e16; // 1:0.67
        maxRedeemRatio = 1e18; // 1:1
        minRedeemDuration = 0; // 0s
        maxRedeemDuration = 4 weeks; // 4 weeks

        cpm = _cpm;
        rewardsCollectorFactory = _rewardsCollectorFactory;
    }
    ...
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
    /**
     * @notice initiate the unstake process of xp-eth spnft nitroPool nfts
     * @param _tokenId the id of the spnft
     * @param _duration the duration of the vesting period
     */
    function unstake(uint256 _tokenId, uint256 _duration) external nonReentrant {
        if (msg.sender != IERC721Upgradeable(address(cpm)).ownerOf(_tokenId)) revert CallerIsNotOwner();
        ICamelotPositionManager.IsekaiCamelotNFT memory icp = cpm.nftToSpNFT(_tokenId);
        if (!icp.inNitroPool) revert TokenNotInNitroPool();

        (uint256 amount,,,,,,,) = ICamelotNFTPool(icp.NFTPool).getStakingPosition(icp.id);
        /// @notice redemption amount
        (uint256 redemptionAmount, uint256 incentive) = getIncentiveRedemptionByVestingDuration(amount, _duration);

        redeemInfo[_tokenId] =
            Redemption(redemptionAmount, incentive, block.timestamp + _duration, block.timestamp + _duration + 1 weeks);

        emit Unstake(_tokenId, redemptionAmount);
    }

    /**
     * @notice finalize the unstake process of xp-eth spnft nitroPool nfts
     * @param _tokenId the id of the spnft
     * @param _owner the owner of the spnft
     */
    function finalizeUnstake(uint256 _tokenId, address _owner) external nonReentrant returns (uint256) {
        if (msg.sender != address(cpm)) revert CallerUnauthorized();

        Redemption memory redemption = redeemInfo[_tokenId];
        if (redemption.endTime > block.timestamp) revert("Unstake not ready");
        if (redemption.endTime == 0) revert("Unstake not initiated");
        // Check if has expired
        if (redemption.expire < block.timestamp) {
            // If expired, delete the redemption info
            // User will need to unstake again
            delete redeemInfo[_tokenId];

            emit UnstakeCancelled(_tokenId);

            return 0;
        }

        /// @dev withdraw from rewards collector
        ICamelotPositionManager.IsekaiCamelotNFT memory spNFT =
            ICamelotPositionManager(address(cpm)).nftToSpNFT(_tokenId);
        uint256 tokenId = spNFT.id;
        address nftPool = spNFT.NFTPool;

        ///@notice check to make sure there is a published nitroPool
        address nitroPool = cpm.getValidNitroPool(nftPool, false);

        address collector = rewardsCollectorFactory.getUserCollector(_owner);
        ...
        emit UnstakeFinalized(_tokenId, redemption.redemptionAmount);

        return redemption.incentive;
    }

    /**
     * @notice cancel the unstake process of xp-eth spnft nitroPool nfts
     * @param _tokenId the id of the spnft
     */
    function cancelUnstake(uint256 _tokenId) external nonReentrant {
        if (msg.sender != IERC721Upgradeable(address(cpm)).ownerOf(_tokenId)) revert CallerIsNotOwner();
        delete redeemInfo[_tokenId];
        emit UnstakeCancelled(_tokenId);
    }
    ...
    /**
     * @notice get redeemability and incentive of a spnft
     * @param _tokenId the id of the spnft
     */
    function isRedeemable(uint256 _tokenId) external view returns (bool, uint256) {
        if (redeemInfo[_tokenId].endTime <= block.timestamp && redeemInfo[_tokenId].expire > block.timestamp) {
            return (true, redeemInfo[_tokenId].redemptionAmount);
        } else {
            return (false, 0);
        }
    }
    ...
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
    /**
     * @notice get the redemption amount and incentive of a spnft by vesting duration
     * @param _amount the amount of the spnft
     * @param _duration the duration of the vesting period
     */
    function getIncentiveRedemptionByVestingDuration(uint256 _amount, uint256 _duration)
        public
        view
        returns (uint256 redemptionAmount, uint256 incentive)
    {
        if (_duration < minRedeemDuration) {
            return (redemptionAmount, incentive);
        }

        // capped to maxRedeemDuration
        if (_duration > maxRedeemDuration) {
            redemptionAmount = (_amount * maxRedeemRatio) / 1e18;
            incentive = _amount - redemptionAmount;
            return (redemptionAmount, incentive);
        }
        ...
        redemptionAmount = (_amount * ratio) / 1e18;
        incentive = _amount - redemptionAmount;
    }
}
