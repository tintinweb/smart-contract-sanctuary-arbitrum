...
import {SingleOutputCRLib, SingleOutputCommand, SingleOutputExec, SingleOutputBatch} from "../lib/SOCRLib.sol";
...
/// @notice Control Room for Single Output Commands.
contract SOCR is Ownable, BaseControlRoom, ControlRoomAccessControl {
    using SafeTransferLib for ERC20;
    using SingleOutputCRLib for SingleOutputCommand;
    using SingleOutputCRLib for SingleOutputExec; // @review: can remove
    using SingleOutputCRLib for SingleOutputBatch;
    ...
    /// @notice bytes4 constant to determine order being routed through external or bungee.
    bytes4 public constant BUNGEE_BYTES = hex"ffffffff";

    /// @notice origin execution mapping against the commandHash
    mapping(bytes32 => OriginExec) internal originExecs;

    /// @notice destination execution mapping against the commandHash
    mapping(bytes32 => DestinationExec) internal destinationExecs;

    /// @notice external router address map
    mapping(uint32 => address) internal routerAddresses;
    ...
    /**
     * @notice Responsible for destructuring the command into single output batch.
     * @param signedBatch batch of single output commands that will be executed.
     *      The batch will first be authenticated for the bungee whitehouse signature.
     *      Funds will be pulled from the commander using his signature.
     */
    function engageCommands(SignedBatch calldata signedBatch) internal override {
        /// Decode the batch into Single Output Batch
        SingleOutputBatch memory batch = abi.decode(signedBatch.batch, (SingleOutputBatch));
        ...
        unchecked {
            for (uint256 i = 0; i < batch.execs.length; i++) {
                ...
                // Determine if the command is routed to an external or not
                if ((bytes4(batch.execs[i].execPayload)) == BUNGEE_BYTES) {
                    ...
                    _execExternal(batch.execs[i], commandHash);
                }
    ...
    /**
     * @notice Receive inbound message from destination chain
     * @dev Responsible for unlocking funds to the beneficiary if the order is fulfilled
     * @dev Responsible for reverting the order if the user revoked it on destination chain via `annulCommand`
     * @dev Called by the corresponding switchboardId
    ...
    function inboundMsg(uint32 switchBoardId, bytes calldata payload) external payable override {
        ...
        // Decode the payload.
        // Validation performed on the caller SwitchboardPlug
     ...
     * @dev sends back escrowed funds to commander
    ...
    /**
     * @dev Responsible for fulfilling the command on the destination.
     ...
     * @notice If the fulfillment is wrong the solver will not unlock tokens on the origin chain.
     ...
     * @param batch Encoded batch array of SingleOutputExec
     */
    function obeyCommands(bytes calldata batch) external payable override {
        // Decode batch into SingleOutputExec[]
        SingleOutputExec[] memory execs = abi.decode(batch, (SingleOutputExec[]));

        // Loop the executions and fulfill them
        unchecked {
            for (uint256 i = 0; i < execs.length; i++) {
                // Check if the chainId is correct for fulfillment.
                ...
                if (destinationExecs[commandHash].amount > 0) revert CommandAlreadyFulfilled();

                // Revert if promisedOutput is less than minOutput amount
                if (execs[i].promisedOutput < execs[i].command.minOutput) revert MinOutputNotMet();

                // Send the tokens in the command to the receiver.
                _sendFundsToReceiver(
                    execs[i].command.outputToken,
                    msg.sender,
                    execs[i].promisedOutput,
                    execs[i].command.info.receiver
                );

                uint256[] memory promisedAmounts = new uint256[](1);
                promisedAmounts[0] = execs[i].promisedOutput;

                address[] memory outputTokens = new address[](1);
                outputTokens[0] = execs[i].command.outputToken;
                ...
                destinationExecs[commandHash] = DestinationExec(execs[i].command.outputToken, execs[i].promisedOutput);
    ...
    function wrapUpAtBase(
        ...
        unchecked {
            for (uint256 i = 0; i < commandHashes.length; i++) {
                ...
                promisedAmounts[i] = destinationExecs[commandHashes[i]].amount;
     ...
     * @notice this function is used to revoke the command given before it is fulfilled
     * @dev sends outbound message to source chain to revoke the command.
     * @param data encoded data to be decoded into SingleOutputCommand
    ...
    function annulCommand(bytes calldata data, uint256 gasLimit, uint256 originChainId) external payable override {
        ...
        SingleOutputCommand memory command = abi.decode(data, (SingleOutputCommand));
        ...
        destinationExecs[commandHash] = DestinationExec(command.outputToken, command.minOutput);
     ...
     * @notice Responsible to revert the given command by the commander.
     * @param commandHash hash of the command that needs to be reverted.
     * @param amount amount sent to the receiver on the destination.
     */
    function _validateAndRelease(uint32 switchBoardId, bytes32 commandHash, uint256 amount) internal {
        // Get the origin execution of the command.
        OriginExec memory originExec = originExecs[commandHash];
        ...
        if (originExec.promisedOutput == 0) revert InvalidCommand();

        // Check if the soldier obeyed the command correctly.
        if (originExec.promisedOutput > amount) revert PromisedAmountNotMet();
    ...
    /**
     * @notice Responsible for validating the signature.
     * @param batch batch of single output commands.
     * @param sig signature of Bungee White House on the batch
     */
    function _validateBatch(SingleOutputBatch memory batch, bytes calldata sig) internal view {
     ...
     * @param exec single output exec.
     */
    function _validateExec(SingleOutputExec memory exec) internal view returns (bytes32 commandHash) {
        ...
        if (exec.promisedOutput < exec.command.minOutput) revert MinOutputNotMet();
    }

    /**
     * @dev
     * @param exec single output execution that is routed externally
     */
    function _execExternal(SingleOutputExec memory exec, bytes32 commandHash) internal {
        /// address of the external router;
        address router = routerAddresses[uint32(bytes4(exec.execPayload))];

        // Transfer Tokens to router address.
        _transferTokens(exec, commandHash, router);

        /// Call the router that follows the IExternalRouter interface
        /// Should revert if something revert externally.
        IExternalRouter(router).routeCommand{value: exec.execValue}(
            exec.command.info,
            exec.execPayload,
    ...
    /**
     * @dev
     * @param exec single output execution that will be routed through bungee protocol.
        Funds will be locked on the contract and will be claimed after soldier completes the execution.
        Message will be receiver via Socket DL to unlock these funds.
     */
    function _execBungee(SingleOutputExec memory exec, bytes32 commandHash, address beneficiary) internal {
     ...
     * @param exec single output execution that will be routed through bungee protocol.
        ...
        Message will be receiver via Socket DL to unlock these funds.
     */
    function _execBungeeSwap(SingleOutputExec memory exec, bytes32 commandHash, address beneficiary) internal {
        // Transfer Tokens to this address.
        _transferTokens(exec, commandHash, address(this));
     ...
     * @param exec single output execution.
     * @param commandHash hash of the command.
     * @param to address, funds will be transferred to this address.
     */
    function _transferTokens(SingleOutputExec memory exec, bytes32 commandHash, address to) internal {
        ...
        PERMIT2.permitWitnessTransferFrom(
            ...
            exec.command.info.commander,
            commandHash,
            SingleOutputCRLib.PERMIT2_ORDER_TYPE,
            exec.signature
        );
    }
    ...
    /// @notice gets the address for the routeId
    function getRouterAddress(uint32 routeId) external view returns (address) {
        return routerAddresses[routeId];
    }
    ...
    /// @notice sets the router address against the routeId.
    function setRouterAddress(uint32 routeId, address _routerAddress) external onlyOwner {
        routerAddresses[routeId] = _routerAddress;
    }
    ...
    function hashOriginBatch(SingleOutputBatch memory batch) public view returns (bytes memory, bytes32, bytes memory) {
...
/// @dev This is the command struct when there is only one output token.
struct SingleOutputCommand {
    // generic command info
    CommandInfo info;
    // the address of the output token
    address outputToken;
    // the minimum expected output to the receiver on the destination domain.
    uint256 minOutput;
...
struct SingleOutputExec {
    // Command to be executed.
    SingleOutputCommand command;
    // promised output of the commands by the soldier to Bungee White House.
    uint256 promisedOutput;
    // amount of value to be sent while using an external party approved by white house.
    uint256 execValue;
    ...
    bytes swapData;
    // extra payload to be sent to the contractor.
    bytes execPayload;
    // signature of the commander
    bytes signature;
}

struct SingleOutputBatch {
    // Batch of executions to follow.
    SingleOutputExec[] execs;
    // Address of the soldier who will receive the locked funds.
    address beneficiary;
}

/// @title Single Output Control Room Library.
/// @author bungee-dev
/// @notice The Single Output Control Room library is the helper for creating order hashes and batchhashes for
///     a single output object type. The single output object is when there is single output token
///     in the command.
/// @dev This library would mainly contain the Single Command Object and the
///     corresponding hashing functions with it.
library SingleOutputCRLib {
    using CommandInfoLib for CommandInfo;
    ...
    string internal constant PERMIT2_ORDER_TYPE =
        string(
            abi.encodePacked(
                "SingleOutputCommand witness)",
                abi.encodePacked(CommandInfoLib.COMMAND_INFO_TYPE, SINGLE_OUTPUT_COMMAND_TYPE),
                Permit2Lib.TOKEN_PERMISSIONS_TYPE
            )
        );

    // SingleOutputExec Type
    bytes internal constant SINGLE_OUTPUT_COMMAND_TYPE =
        abi.encodePacked(
            "SingleOutputCommand(",
            "CommandInfo info,",
            "address outputToken,",
    ...
    // @review review this lib again, make sure things are solid
    bytes internal constant SINGLE_OUTPUT_EXEC_TYPE =
        abi.encodePacked(
            "SingleOutputExec(",
            "SingleOutputCommand command,",
            "uint256 promisedOutput,",
    ...
    bytes internal constant COMMAND_TYPE =
        abi.encodePacked(SINGLE_OUTPUT_COMMAND_TYPE, CommandInfoLib.COMMAND_INFO_TYPE);

    // Keccak Hash of Command Type.
    bytes32 internal constant COMMAND_TYPE_HASH = keccak256(COMMAND_TYPE);

    // Exec Type.
    bytes internal constant EXEC_TYPE = abi.encodePacked(SINGLE_OUTPUT_EXEC_TYPE, SINGLE_OUTPUT_COMMAND_TYPE);

    // Keccak Hash of Exec Type.
    bytes32 internal constant EXEC_TYPE_HASH = keccak256(EXEC_TYPE);

    /// @notice Hash of single output command on the origin chain
    /// @param command command that is issued by the commander
    function hashOriginCommand(SingleOutputCommand memory command) internal view returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    COMMAND_TYPE_HASH,
    ...
    /// @notice Hash of single output command on the destination chain
    /// @param command command that is issued by the commander
    function hashDestinationCommand(SingleOutputCommand memory command) internal view returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    COMMAND_TYPE_HASH,
    ...
    /// @notice Hash of SingleOutputExec on the origin chain
    /// @param execution CommandExecution sent by the soldier to be hashed
    function hashOriginExec(SingleOutputExec memory execution) internal view returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    EXEC_TYPE_HASH,
                    hashOriginCommand(execution.command),
                    execution.promisedOutput,
                    execution.execValue,
                    execution.swapRoute,
                    keccak256(execution.swapData),
                    keccak256(execution.execPayload),
                    keccak256(execution.signature)
                )
            );
    }

    /// @notice hash a batch of single output execs
    /// @param batch batch of single output executions to be hashed
    function hashOriginBatch(SingleOutputBatch memory batch) internal view returns (bytes32) {
        unchecked {
            bytes32 outputHash = keccak256(
                "SingleOutputExec(SingleOutputCommand command,uint256 promisedOutput,uint256 execValue,address swapRoute,bytes swapData,bytes execPayload,bytes signature)"
            );
            for (uint256 i = 0; i < batch.execs.length; i++) {
...
/**
 * @notice interface to call external router.
 */
interface IExternalRouter {
    function routeCommand(
        CommandInfo calldata commandInfo,
        bytes calldata payloadWithExtraData,
        bytes calldata affiliateFees
    ) external payable;
}
