...
contract TokenDistributorCustom is Ownable {
    ...
    function setUserAlloc(
        ...
        uint256[] calldata _totalAmount,
        uint256[] calldata _claimed
        ) external onlyAdmin{
            require(_tokenDistribution[_fromPoolId].unlockDates.length != 0, "Token distribution not yet set");
            uint256[]memory _percentages = _tokenDistribution[_fromPoolId].unlockPercentages;
            uint256 _length = _user.length;
            require(_length == _totalAmount.length,"Lengths doesn't match");
            for(uint256 i ; i< _length ;){
                _userVesting[_user[i]][_fromPoolId].totalAmount = _totalAmount[i]; 
                _userVesting[_user[i]][_fromPoolId].totalClaimed = _claimed[i]; 
                _userVesting[_user[i]][_fromPoolId].initiated = true; 
                _userVesting[_user[i]][_fromPoolId].amounts = _calculateAmounts(_percentages,_totalAmount[i]);
