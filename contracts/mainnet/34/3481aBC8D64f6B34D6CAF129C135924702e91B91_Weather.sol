...
contract Weather is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the genesis pool & airdrop
    uint256 public constant INITIAL_WEATHER_GENESIS_POOL = 40000 ether;
    uint256 public constant INITIAL_WEATHER_AIRDROP = 2000 ether;
    ...
    /**
     * @notice Constructs the WEATHER ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Weather Tomb", "WEATHER") {
        // Mints 1 WEATHER to contract creator for initial pool setup
    ...
    function _getWeatherPrice() internal view returns (uint256 _weatherPrice) {
        try IOracle(weatherOracle).consult(address(this), 1e18) returns (
            uint144 _price
        ) {
            return uint256(_price);
        } catch {
            revert("Weather: failed to fetch WEATHER price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _WeatherPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_WeatherPrice >= taxTiersTwaps[tierId]) {
    ...
    function setWeatherOracle(address _weatherOracle) public onlyOperatorOrTaxOffice {
        require(_weatherOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints WEATHER to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of WEATHER to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentWeatherPrice = _getWeatherPrice();
            currentTaxRate = _updateTaxRate(currentWeatherPrice);
            if (currentWeatherPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        address _weatherGenesis,
        address _airdropReceiver
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_weatherGenesis != address(0), "!_weatherGenesis");
        require(_airdropReceiver != address(0), "!_airdropReceiver");
        rewardPoolDistributed = true;
        _mint(_weatherGenesis, INITIAL_WEATHER_GENESIS_POOL);
        _mint(_airdropReceiver, INITIAL_WEATHER_AIRDROP);
    }
