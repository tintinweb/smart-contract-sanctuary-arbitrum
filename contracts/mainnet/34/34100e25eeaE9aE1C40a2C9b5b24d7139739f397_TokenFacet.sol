	...
	Context
{
	...
	function mint(uint8 _v, bytes32 _r, bytes32 _s) external payable {
		require(totalSupply() + 1 <= getState().maxSupply, "Out of mint limits");
        require(verifySignedAddress(_v, _r, _s, msg.value), "Invalid signature");
	...
	function getRenewSubscriptionPriceUsd() public view returns(uint256) {
    ...
    function renewSubscription(uint256 tokenId, uint256 daysNum, uint8 _v, bytes32 _r, bytes32 _s) public payable {
        require(verifySignedAddressRenew(_v, _r, _s, tokenId, daysNum, msg.value), "Invalid signature");
    ...
    function freeRenewSubscription(uint256 tokenId, uint256 daysNum, uint8 _v, bytes32 _r, bytes32 _s) public {
        require(verifySignedAddressRenewFree(_v, _r, _s, tokenId, daysNum), "Invalid signature");
    ...
    function airDropSubscriptionForTokens(uint256[] calldata tokenIds, uint256[] calldata daysNum) public onlyOwner {
        require(tokenIds.length == daysNum.length);

        for(uint256 i = 0; i < tokenIds.length; i++) {
            renewer(tokenIds[i], daysNum[i]);
        }
    }

    function mintTokens(address[] calldata users) public onlyOwner {
    ...
    function mintTo(address sender) internal {
        getState().subscriptionDeadline[getState().currentIndex] = block.timestamp + (1 days * getState().afterMintSubscription);

        emit Minted(getState().currentIndex, sender, getState().subscriptionDeadline[getState().currentIndex]);

        getState().mintNonce[sender] += 1;
        _safeMint(sender, 1);
    }
    ...
    function verifySignedAddress(
        ...
        bytes32 prefixedHashMessage = keccak256(abi.encodePacked(Constants.PREFIX, keccak256(abi.encodePacked(value, msg.sender, getState().mintNonce[_msgSender()]))));
        address signer = ecrecover(prefixedHashMessage, _v, _r, _s);
        return signer == getState().publicKey;
    }

    function verifySignedAddressRenewFree(
        uint8 _v,
        bytes32 _r,
        bytes32 _s,
        uint256 tokenId,
    ...
    function verifySignedAddressRenew(
        ...
        bytes32 prefixedHashMessage = keccak256(abi.encodePacked(Constants.PREFIX, keccak256(abi.encodePacked(value, tokenId, daysNum))));
        address signer = ecrecover(prefixedHashMessage, _v, _r, _s);
        return signer == getState().publicKey;
    }

    function calculateEthAmount(uint usdAmount) internal view returns (uint) {

        (, int ethToUsdPrice, , , ) = AggregatorV3Interface(getState().ethPriceFeed).latestRoundData();

        uint ethAmount = (usdAmount * (10**uint(AggregatorV3Interface(getState().ethPriceFeed).decimals())) * 1 ether) / uint(ethToUsdPrice);
        return ethAmount;
    }
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                '"image":"', (isExpired) ? (string(bytes.concat(bytes(getState().baseURI), bytes("expired.gif")))) : (string(bytes.concat(bytes(getState().baseURI), bytes("activated.gif")))), '",',
