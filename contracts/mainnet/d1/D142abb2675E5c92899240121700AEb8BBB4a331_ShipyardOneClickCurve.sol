...
contract ShipyardOneClickCurve is Ownable, ReentrancyGuard, GasThrottler {
    ...
    bytes32 public constant USDC_TOKEN = "USDC";
    ...
    function deposit(address _shipyardVault, bytes32 _depositToken, uint256 _amount) external nonReentrant {
        ...
        } else if (_depositToken == USDC_TOKEN) {

            bytes32 preferredToken = strategy.preferredUnderlyingToken();
    ...
    function withdraw(address _shipyardVault, bytes32 _withdrawToken, uint256 _amount) external nonReentrant {
        ...
        require(withdrawTokenAddress != address(0) || _withdrawToken == USDC_TOKEN, 'Invalid withdraw token');
...
interface IStrategy {
    ...
    function preferredUnderlyingToken() external returns (bytes32) ;
    function underlyingTokenAddress(bytes32 _token) external returns (address);
    function underlyingTokenIndex(bytes32 _token) external returns (uint256);
}
