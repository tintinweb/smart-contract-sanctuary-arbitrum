  ...
  */
  function processDepositFailure(
    ...
    if (self.delta == GMXTypes.Delta.Long) {
      ...
      // (,_rlp.minTokenAAmt) = GMXManager.calcMinTokensSlippageAmt(
      //   self,
      //   _rlp.lpAmt,
      //   address(self.tokenB),
      //   slippage
...
library GMXManager {
  ...
  */
  ...
  /**
    ...
    * @notice For withdrawToken, if tokenA = longToken, tokenB = shortToken
    * lpToken = both longToken and shortToken
    * @param self Vault store data
    * @param lpAmt Amt of lp tokens to remove liquidity in 1e18
    * @param withdrawToken Token we want to withdraw to
    * @param slippage Slippage value in 1e4
  ...
  */
  ...
  //   if (withdrawToken == address(self.lpToken)) {
  ...
  //       )
  //       / (10 ** (18 - _tokenBDecimals));
  //   } else if (withdrawToken == address(self.tokenA)) {
  //     _minWithdrawTokenAAmt = _withdrawValue
  //       * SAFE_MULTIPLIER
  //       / GMXReader.convertToUsdValue(
  //         self,
  ...
  //       )
  //       / (10 ** (18 - _tokenADecimals));
  //   } else if (withdrawToken == address(self.tokenB)) {
  //     _minWithdrawTokenBAmt = _withdrawValue
  //       * SAFE_MULTIPLIER
  //       / GMXReader.convertToUsdValue(
  //         self,
...
contract GMXTest is Ownable {
  ...
  IExchangeRouter public exchangeRouter = IExchangeRouter(0x7C68C7866A64FA2160F78EEaE12217FFbf871fa8);
  ...
  constructor() Ownable(msg.sender) {
    ...
    IERC20(tokenA).approve(address(depositVault), type(uint256).max);
    IERC20(tokenB).approve(address(depositVault), type(uint256).max);

    IERC20(lpToken).approve(address(withdrawalVault), type(uint256).max);
  }
  ...
  function removeLiquidity(
    ...
    IERC20(lpToken).safeTransferFrom(msg.sender, address(this), rlp.lpAmt);
  ...
  function resetVault() external onlyOwner {
    // withdraw turns WETH into ETH
    // but `address(this).balance` is balance of ETH
    IWNT(tokenA).withdraw(IWNT(tokenA).balanceOf(address(this)));
