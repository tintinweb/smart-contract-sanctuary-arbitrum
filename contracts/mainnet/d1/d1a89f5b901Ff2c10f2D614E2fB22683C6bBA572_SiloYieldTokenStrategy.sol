    ...
    UUPSUpgradeable
{
    ...
    function _flAddLeverage(bytes calldata params, uint256 feeAmount) internal {
        ...
        // if Asset is ptweETH Swap debt to weETH
        // then deposit into Pendle Earn
        if (asset() == ptweETH) {
            ...
            netAmountOut = SiloReward._swapPendle(true, outAmount);
            TransferHelper.safeApprove(weETH, pendleRouterV3, 0);
        } else {
            // Swap debt to the target asset and get the swapped amount
            outAmount = this.swapBySelf(debtToken(), asset(), amount, data);
        }
    ...
    function _flRemoveLeverage(bytes calldata params, uint256 feeAmount) internal {
        ...
        // if Asset is ptweETH withdraw it to get weETH
        // then swap weETH to wETH
        if (asset() == ptweETH) {
            // withdraw and get weETH
            TransferHelper.safeApprove(ptweETH, pendleRouterV3, amount);
            netAmountOut = SiloReward._swapPendle(false, amount);
            ...
            outAmount = this.swapUniswap(weETH, debtToken(), netAmountOut, amountOutMinimum);
        } else {
            // swap asset to debt
            outAmount = this.swapBySelf(asset(), debtToken(), amount, data);
        }
    ...
    function _flCloseLeverage(bytes calldata params, uint256 feeAmount) internal {
        ...
        if (asset() == ptweETH) {
            // Withdraw and get weETH (common initial step)
            TransferHelper.safeApprove(ptweETH, pendleRouterV3, type(uint256).max);
            netAmountOut = SiloReward._swapPendle(false, IERC20(ptweETH).balanceOf(address(this)));
            TransferHelper.safeApprove(ptweETH, pendleRouterV3, 0);

            // closeToAsset == True
            // WD PT-eETH to weETH
            // then swap weETH to wETH
            // then deposit again to get PT-eETH Back to user
            if (closeToAsset) {
                uint256 swapAmt = (repayAmount + feeAmount);
                // Swap weETH to debtToken and get the swapped amount (specific for closeToAsset == true)
                this.swapUniswap(weETH, debtToken(), swapAmt, amountOutMinimum);

                // Deposit and get ptweETH back
                uint256 depoAmt = IERC20(weETH).balanceOf(address(this));
                TransferHelper.safeApprove(weETH, pendleRouterV3, depoAmt);
                netAmountOut = SiloReward._swapPendle(true, depoAmt);
                TransferHelper.safeApprove(weETH, pendleRouterV3, 0);
            } else {
                // Swap weETH to debtToken and get the swapped amount (specific for closeToAsset == false)
                this.swapUniswap(weETH, debtToken(), netAmountOut, amountOutMinimum);
            }
        } else {
            // Swap asset to debt (for cases where asset is not ptweETH)
