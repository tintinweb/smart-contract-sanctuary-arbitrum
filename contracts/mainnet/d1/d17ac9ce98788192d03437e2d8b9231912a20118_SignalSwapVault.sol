...
contract SignalSwapVault is Context, MessageApp, Initializable, Pausable, VaultFeeHandler {
    ...
    function signalSwapInitialize(
        ...
        if (_factory != address(0)) {
            initializeMessageBus(FactoryContract(factory).messageBus());
        }
    }
    ...
    /**
     * @notice Function to send token swap function call to router contract,
     ...
     */
    function signalTrade(bytes memory data) internal {
        address router = FactoryContract(factory).vaultTypeToRouterContract(vaultType);
        router.functionCall(data, "Vault: low-level call failed");
    }
    ...
    function executeMessage(
        ...
        signalTrade(_message);
        return ExecutionStatus.Success;
    }

    /**
     * @notice Sends token swap function call to router contract, sends tokens to given user's address after deducting the fees
     * and from the collected fees distributes a share to the Vault creator and the rest to the Treasury,only accessible by PLATFORM_ROLE.
    ...
    function withdrawTrade(
        ...
        uint256 txnFee = _calculateTotalFee(totalAmount, withdrawalFee);
...
abstract contract MessageApp is MessageSenderApp, MessageReceiverApp {
    function initializeMessageBus(address _messageBus) public {
