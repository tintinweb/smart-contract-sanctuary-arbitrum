...
contract RadiantArbitrumDai is DefiiWithParams {
    ...
    ILendingPoolAddressesProvider constant addressesProvider =
        ILendingPoolAddressesProvider(
            0xe21B295ED46528eFD5F3EF66E18BC6ad1c87f003
        );
    ...
    function _enterWithParams(bytes memory params) internal override {
        ...
        uint256 price = oracle.getAssetPrice(address(DAI));
        uint256 dec = 1e18;
        for (uint8 i = 0; i < loopCount; i++) {
            radiantPool.deposit(address(DAI), amount, address(this), 0);
            amount = (((((amount * price) / dec) * dec) / price) * ltv) / 100;
