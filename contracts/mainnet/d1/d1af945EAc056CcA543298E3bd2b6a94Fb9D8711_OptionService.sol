...
contract OptionService is  ModuleBase,IOptionService, Initializable,UUPSUpgradeable, ReentrancyGuardUpgradeable{
    ...
    function liquidateOption(
        ...
        if (IOptionFacet.OrderType.Call == _orderType) {
            IOptionFacet.CallOrder memory optionOrder = IOptionFacet(diamond).getCallOrder(_orderID);          
            require( optionOrder.holder != address(0), "OptionService:optionOrder not exist" );   
            ...
            if ( msg.sender == owner ||   (IPlatformFacet(diamond).getIsVault(msg.sender) &&  IOwnable(msg.sender).owner() == owner) ) {
                liquidateCall(optionOrder, _type, _incomeAmount, _slippage);
            ...
            IOptionFacet.PutOrder memory optionOrder = IOptionFacet(diamond).getPutOrder(_orderID); 
            require(optionOrder.holder != address(0),"OptionService:optionOrder not exist");    
            if ( optionOrder.liquidateMode == (IOptionFacet.LiquidateMode.PhysicalDelivery)  ) {     
                require(_type != LiquidateType.ProfitTaking, "OptionService:Unauthorized method of option settlement, type Error" );     
            }
            ...
            address owner = IVault(optionOrder.holder).owner();
            if ( msg.sender == owner || (IPlatformFacet(diamond).getIsVault(msg.sender) &&  IOwnable(msg.sender).owner() == owner) ) {  
                liquidatePut(optionOrder, _type, _incomeAmount, _slippage);
            } else if (block.timestamp > optionOrder.expirationDate) {
                liquidatePut(
                    optionOrder,
                    LiquidateType.NotExercising,
    ...
    function liquidateCall(
        IOptionFacet.CallOrder memory optionOrder,
        LiquidateType _type,
        uint256 _incomeAmount,
        uint256 _slippage
    ) internal {
        ...
        if (_type == LiquidateType.Exercising || _type == LiquidateType.ExercisingEOA) {
            //payLater time
            //traferFrom strikeAsset to writer
            uint256 strikeAmount=getParts(optionOrder.underlyingAsset, optionOrder.underlyingAmount, optionOrder.strikeAmount);
             if(_type == LiquidateType.ExercisingEOA && optionOrder.strikeAsset == eth ){
                 (bool success,) =address(optionOrder.writer).call{value:strikeAmount}("");
                 require(success,"OptionService:transfer eth fail");
             }else{
                address approveAsset= optionOrder.strikeAsset==eth?platformFacet.getWeth(): optionOrder.strikeAsset; 
                IVault(optionOrder.holder).invokeTransferFrom(
                    approveAsset,
                ...
                if(optionOrder.strikeAsset==eth){
                    IVault(optionOrder.writer).invokeUnwrapWETH(approveAsset,strikeAmount);
                }    
             }
            ...
            if( optionOrder.underlyingAssetType == IOptionFacet.UnderlyingAssetType.Original){
                    IVault(optionOrder.holder).invokeTransferEth(
                        optionOrder.writer,
                        optionOrder.underlyingAmount
                    );
            }
            else if ( optionOrder.underlyingAssetType == IOptionFacet.UnderlyingAssetType.Token) { 
                   uint256 balance = IERC20(optionOrder.underlyingAsset)
                        .balanceOf(optionOrder.holder);
            ...
            if (
                optionOrder.underlyingAssetType !=
                IOptionFacet.UnderlyingAssetType.Nft
            ) {
    ...
    function liquidatePut(
        IOptionFacet.PutOrder memory optionOrder,
        LiquidateType _type,
        uint256 _incomeAmount,
        uint256 _slippage
    ) internal {
        ...
        if (_type ==LiquidateType.Exercising || _type == LiquidateType.ExercisingEOA) {
            ...
            if(optionOrder.underlyingAssetType == IOptionFacet.UnderlyingAssetType.Original){
                IVault(optionOrder.holder).invokeTransferEth(   optionOrder.writer, optionOrder.underlyingAmount );
                    ...
                    IVault(optionOrder.holder).invokeTransfer(optionOrder.underlyingAsset,optionOrder.writer,balance);                     
                
            } else if ( optionOrder.underlyingAssetType ==IOptionFacet.UnderlyingAssetType.Nft) {  
                IVault(optionOrder.holder).invokeTransferNft( optionOrder.underlyingAsset,  optionOrder.writer,  optionOrder.underlyingNftID);           
            } else {
                revert("OptionService:underlyingAssetType error");
            }
            updatePosition(optionOrder.holder, optionOrder.underlyingAsset, 0);
        ...
        setFuncBlackList(optionOrder.writer, false);
    }
    ...
    function getEarningsAmountTest(
        address underlyingAsset, // ETH
        uint256 underlyingAmount, // 1
        address strikeAsset, // USDC
        uint256 strikeNotionalAmount
