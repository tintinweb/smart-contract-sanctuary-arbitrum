...
import { IChainlinkOracle } from "./IVotingEscrow/IChainlinkOracle.sol";

contract PriceProvider is IPriceProvider, OwnableUpgradeable {
    ...
    address public vkaTokenPoolAddress;
    ...
    function initialize(address _ethOracle, address _vkaTokenPool) public initializer {
        decimals = 18;
        twapInterval = 3600;
        timePoints = 7;
        ...
        vkaTokenPoolAddress = _vkaTokenPool;
        __Ownable_init();
    }
    ...
    function setVkaTokenPool(address _vkaTokenPoolAddress) external onlyOwner {
        if (_vkaTokenPoolAddress == address(0)) revert AddressZero();

        vkaTokenPoolAddress = _vkaTokenPoolAddress;

        emit VkaTokenPoolSet(_vkaTokenPoolAddress);
    }

    function updatePrice() public returns (uint256) {
        uint256 updatedPriceInEth = getPriceInEthFromUniswap(vkaTokenPoolAddress);
        price = getVkaPriceInUsd(updatedPriceInEth);
    ...
    function getVkaPriceInUsd(uint256 vkaPriceInEth) public view returns (uint256 vkaPriceInUsd) {
        uint256 ethPriceInUsd = _getLatestData(ethOracle); //usdc price in eth
        vkaPriceInUsd = (vkaPriceInEth * ethPriceInUsd) / 1e8; //decimal??
    }
    ...
    function getTokenPrice() external view returns (uint256) {
        // return price;
        uint256 updatedPriceInEth = getPriceInEthFromUniswap(vkaTokenPoolAddress);
        return getVkaPriceInUsd(updatedPriceInEth);
    }
    ...
    function getPriceInEthFromUniswap(address uniswapV3Pool) public view returns (uint256) {
        ...
        // how many vka token can be bought with 1 eth
        uint256 priceOfVkaInEth = (((_getPriceX96FromSqrtPriceX96(sqrtPriceX96))) * 1e18) / FixedPoint96.Q96;

        return priceOfVkaInEth;
    }
    ...
    function _getLatestData(address _oracle) internal view returns (uint256) {
        ...
        ) = IChainlinkOracle(_oracle).latestRoundData(); //in 1e8
    ...
    function _getSqrtTwapX96(
        ...
        if (_twapInterval == 0 || _timepoints == 1) {
            ...
            sqrtPriceX96Average = TickMath.getSqrtRatioAtTick(
                int24(cumulativeTick / int256(uint256(intervalBetweenObservations)))
            );
        }
    }
