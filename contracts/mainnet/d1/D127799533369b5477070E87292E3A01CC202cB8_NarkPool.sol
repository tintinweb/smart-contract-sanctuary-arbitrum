...
contract NarkPool is OwnableUpgradeable, SushiSwapV2Adapter {
    ...
    function freed() external onlyOperatorOrOwner {
        ...
        uint256 amountLp = IERC20(defiLP).balanceOf(address(this));
        uint256 amountFreed = FullMath.mulDiv(amountLp, Constants.ONE, Constants.DENOMINATOR);
        uint256 amountReward = FullMath.mulDiv(amountFreed, Constants.ONE, Constants.TWO);

        (uint amountA, uint amountB) = _removeLiquidity(tokenA, tokenB, amountFreed.sub(amountReward), address(this));
        uint256[] memory amountsPair = _swap(tokenB, tokenA, amountB);

        uint256 amountABurn = amountA.add(amountsPair[Constants.ONE]);
        INarkCoin(tokenA).burn(address(this), amountABurn);
