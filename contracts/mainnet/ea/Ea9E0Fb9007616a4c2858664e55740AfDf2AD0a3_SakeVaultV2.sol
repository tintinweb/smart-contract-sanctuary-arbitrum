...
contract SakeVaultV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function setUSDC(address _usdc) public onlyOwner zeroAddress(_usdc) {
        strategyAddresses.USDC = _usdc;
    }

    function setUSDCe(address _usdce) public onlyOwner zeroAddress(_usdce) {
        USDCe = _usdce;
    }

    function setVaultkaHandler(address _handler) public onlyOwner zeroAddress(_handler) {
        VaultkaHandler = _handler;
    }
    ...
    function closePosition(
        ...
        IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(VaultkaHandler, waterRepayment + closeData.toLeverageUser);
        uint256 receivedAmount = IHandler(VaultkaHandler).uniswapExecution(waterRepayment + closeData.toLeverageUser, strategyAddresses.USDC, USDCe, true);

        closeData.toLeverageUser = receivedAmount - waterRepayment;
        IERC20Upgradeable(USDCe).safeIncreaseAllowance(strategyAddresses.water, waterRepayment);
        ...
        if (feeConfiguration.withdrawalFee > 0) {
            ...
            IERC20Upgradeable(USDCe).safeTransfer(feeConfiguration.feeReceiver, fee);
        ...
        if (_sameSwap) {
            IERC20Upgradeable(USDCe).safeTransfer(_user, amountAfterFee);
        } else {
