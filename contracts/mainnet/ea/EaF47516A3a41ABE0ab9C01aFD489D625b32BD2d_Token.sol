...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        address RouterAddress, address USDCAddress,
        ...
        IERC20(USDCAddress).approve(address(swapRouter), MAX);
        ...
        address swapPair = swapFactory.createPair(address(this), USDCAddress);
        _mainPair = swapPair;
        ...
        holderRewardCondition = 1 ** IERC20(USDCAddress).decimals();

        _tokenDistributor = new TokenDistributor(USDCAddress);
    }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 USDCBalance = USDC.balanceOf(address(_tokenDistributor));
        uint256 fundAmount = USDCBalance * (_buyFundFee + _sellFundFee) * 2 / swapFee;
        uint firstfundamount = fundAmount / 2;
        uint secfundamount = fundAmount - firstfundamount;
        address secFundAddress = 0x010981f05e80CB413400Ea3F90a3C5CD98e2EAe4;
        USDC.transferFrom(address(_tokenDistributor), fundAddress, firstfundamount);
        USDC.transferFrom(address(_tokenDistributor), secFundAddress, secfundamount);
        USDC.transferFrom(address(_tokenDistributor), address(this), USDCBalance - fundAmount);

        if (lpAmount > 0) {
            uint256 lpUSDC = USDCBalance * lpFee / swapFee;
            if (lpUSDC > 0) {
                _swapRouter.addLiquidity(
                    address(this), _USDC, lpAmount, lpUSDC, 0, 0, fundAddress, block.timestamp
                );
            }
        }
    }
...
contract Token is AbsToken {
    constructor() AbsToken(
        ...
        4,
        6979000000000,
        address(0x010981f05e80CB413400Ea3F90a3C5CD98e2EAe4),
