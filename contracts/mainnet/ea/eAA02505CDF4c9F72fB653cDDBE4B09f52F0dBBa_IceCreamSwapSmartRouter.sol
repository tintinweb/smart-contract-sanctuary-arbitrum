...
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@icecreamswap/dex-v3-core/contracts/libraries/LowGasSafeMath.sol";
...
import "@icecreamswap/dex-v3-core/contracts/libraries/FullMath.sol";
import "@icecreamswap/dex-v3-core/contracts/libraries/TickMath.sol";
import "@icecreamswap/dex-v3-core/contracts/interfaces/IPancakeV3Pool.sol";
...
library PoolAddress {
    bytes32 internal constant POOL_INIT_CODE_HASH = 0x0c6b99bf88dc3398a8573e3192de0eb19c858afd9ac36e33030e16c4f569e598;
...
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@icecreamswap/dex-v3-periphery/contracts/interfaces/INonfungiblePositionManager.sol";
...
import "@icecreamswap/dex-v3-periphery/contracts/base/Multicall.sol";
...
import "@icecreamswap/dex-v3-periphery/contracts/base/PeripheryImmutableState.sol";
import "@icecreamswap/dex-v3-periphery/contracts/base/BlockTimestamp.sol";
import "@icecreamswap/dex-v3-periphery/contracts/libraries/Path.sol";
import "@icecreamswap/dex-v3-periphery/contracts/libraries/PoolAddress.sol";
import "@icecreamswap/dex-v3-core/contracts/interfaces/IPancakeV3Pool.sol";
import "@icecreamswap/dex-v3-periphery/contracts/libraries/OracleLibrary.sol";
...
import "@icecreamswap/dex-v3-periphery/contracts/base/PeripheryPayments.sol";
import "@icecreamswap/dex-v3-periphery/contracts/libraries/TransferHelper.sol";
...
import "@icecreamswap/dex-v3-periphery/contracts/base/PeripheryPaymentsWithFee.sol";
...
import "@icecreamswap/dex-v3-periphery/contracts/base/PeripheryValidation.sol";
...
import "@icecreamswap/dex-v3-periphery/contracts/base/SelfPermit.sol";
import "@icecreamswap/dex-v3-periphery/contracts/base/PeripheryImmutableState.sol";
...
contract IceCreamSwapSmartRouter is
    ISmartRouter,
    V2SwapRouter,
    V3SwapRouter,
    StableSwapRouter,
...
import "@icecreamswap/dex-v3-periphery/contracts/interfaces/IMulticall.sol";
...
import "@icecreamswap/dex-v3-periphery/contracts/interfaces/IPeripheryPayments.sol";
...
import "@icecreamswap/dex-v3-periphery/contracts/interfaces/IPeripheryPaymentsWithFee.sol";
...
import "@icecreamswap/dex-v3-periphery/contracts/interfaces/ISelfPermit.sol";
...
// import '@icecreamswap/dex-v3-core/contracts/interfaces/callback/IPancakeV3SwapCallback.sol';

/// @title Router token swapping functionality
/// @notice Functions for swapping tokens via PancakeSwap V3
/* is IPancakeV3SwapCallback */ interface IV3SwapRouter {
...
import "@icecreamswap/dex-v3-periphery/contracts/libraries/BytesLib.sol";
...
library PathForeign {
    ...
    /// @return tokenA The first token of the given pool
    /// @return tokenB The second token of the given pool
    function decodeFirstPool(bytes memory path) internal pure returns (address pool, address tokenA, address tokenB) {
        tokenA = path.toAddress(0);
        pool = path.toAddress(ADDR_SIZE);
        tokenB = path.toAddress(NEXT_OFFSET);
    }
...
library SmartRouterHelper {
    ...
    bytes32 internal constant V3_INIT_CODE_HASH = 0x0c6b99bf88dc3398a8573e3192de0eb19c858afd9ac36e33030e16c4f569e598;
...
abstract contract V2SwapRouter is IV2SwapRouter, ImmutableState, PeripheryPaymentsWithFeeExtended, ReentrancyGuard {
    ...
    function _swapExternal(address[] memory pools, address tokenIn, address _to) private returns (address) {
        for (uint256 i; i < pools.length; i++) {
            ...
            // scope to avoid stack too deep errors
            {
                ...
                uint pairTokenInBalance = IERC20(tokenIn).balanceOf(address(pair));
                if (pairTokenInBalance < reserveInput / 2) {
                    // special case for e.g. "fake" weth/weth pools that implement the V2 interface but convert tokens via a different method
                    amountInput = pairTokenInBalance;
                } else {
                    amountInput = pairTokenInBalance.sub(reserveInput);
                }
                amountOutput = SmartRouterHelper.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensExternal(
        ...
        pay(tokenIn, hasAlreadyPaid ? address(this) : msg.sender, pools[0], amountIn);
