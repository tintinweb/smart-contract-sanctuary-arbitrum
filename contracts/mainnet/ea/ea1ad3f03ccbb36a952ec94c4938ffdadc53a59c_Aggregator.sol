...
contract Aggregator is ChainlinkClient {
    ...
    IChainlinkFeed public sequencerUptimeFeed;
    ...
    event SequencerUptimeFeedUpdated(address value);
    ...
    error AggregatorSequencerDown();
    error AggregatorGracePeriodNotOver();
    ...
    function updateSequencerUptimeFeed(
        IChainlinkFeed value
    ) external onlyGov {
        if (address(value) == address(0)) {
            revert AggregatorInvalidAddress(address(0));
        ...
        sequencerUptimeFeed = value;
        emit SequencerUptimeFeedUpdated(address(value));
    }
    ...
    function sequencerCheck() private view {
        if(address(sequencerUptimeFeed) != address(0)) {
            ( ,int256 answer, uint256 startedAt, , ) = sequencerUptimeFeed.latestRoundData();
            bool isSequencerUp = answer == 0;
            if (!isSequencerUp) {
                revert AggregatorSequencerDown();
            }
            uint256 timeSinceUp = block.timestamp - startedAt;
            if (timeSinceUp <= GRACE_PERIOD_TIME || startedAt == 0) {
                revert AggregatorGracePeriodNotOver();
            }
        }
    }
