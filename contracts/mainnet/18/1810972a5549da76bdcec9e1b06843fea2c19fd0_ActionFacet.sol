...
import {IActionFacet} from "core/interfaces/IActionFacet.sol";
import {RegistryState, rs} from "core/RegistryState.sol";

contract ActionFacet is ERC721BaseInternal, Modifiers, IActionFacet {
    using LibKredits for uint256;
    ...
    function unlink() external {
        uint256 tokenId = LibKredits.unlink();
    ...
    function lock1155(
        ...
        require(
            nfts.length == tokenIds.length && tokenIds.length == amounts.length,
            "Invalid input"
        );
        RegistryState storage r = rs();

        uint256 tokenId721 = r.linkedId[msg.sender];
        for (uint256 i; i < nfts.length; ++i) {
            require(r.isValid[nfts[i]], "Not Kresko 1155 NFT");
            IERC1155(nfts[i]).safeTransferFrom(
                msg.sender,
                address(this),
                tokenIds[i],
                amounts[i],
            ...
            r.locked[tokenId721][nfts[i]][tokenIds[i]] += amounts[i];
            emit Events.Locked(
                msg.sender,
                tokenId721,
                nfts[i],
