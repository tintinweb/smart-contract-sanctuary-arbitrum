...
contract FeeDistributor {
    ...
    /// @dev Struct to store total fees collected and fees per NFT for a token
    struct FeeInfo {
        uint256 totalFees;
        uint256 feesPerNFT;
    }
    ...
    mapping(address => mapping(address => uint256)) private lastClaimedFeePerNFT;
    ...
    constructor(address _nftAddress) {
        ...
        registeredTokens.push(address(0));
    ...
    receive() external payable {
        distributeFees(address(0), msg.value);
    }
    ...
    function receiveERC20Fees(address _tokenAddress, uint256 _amount) external {
        ...
        distributeFees(_tokenAddress, _amount);
    }
    ...
    function distributeFees(address _tokenAddress, uint256 _amount) internal {
        uint256 totalNFTs = nft.totalSupply();
        require(totalNFTs > 0, "No NFTs in supply");

        feeInfo[_tokenAddress].feesPerNFT += _amount / totalNFTs;
        feeInfo[_tokenAddress].totalFees += _amount;
    }
    ...
    function claimFees(address _tokenAddress) external {
        uint256 holderShares = nft.balanceOf(msg.sender);
        require(holderShares > 0, "You do not own any NFTs");
        FeeInfo storage info = feeInfo[_tokenAddress];
        uint256 amountToClaim = (info.feesPerNFT - lastClaimedFeePerNFT[_tokenAddress][msg.sender]) * holderShares;
        require(amountToClaim > 0, "No fees to claim");

        // Update the last claimed fee
        lastClaimedFeePerNFT[_tokenAddress][msg.sender] = info.feesPerNFT;

        if (_tokenAddress == address(0)) {
            // Ether
            payable(msg.sender).sendValue(amountToClaim);
        } else {
            // ERC20 Token
            IERC20(_tokenAddress).safeTransfer(msg.sender, amountToClaim);
        }
    }
    ...
    /// @notice Provides total fees collected and fees per NFT for a given token
    /// @param _tokenAddress The address of the token to query
    /// @return totalFees The total fees collected for the token
    /// @return feesPerNFT The fees per NFT calculated for the token
    function getTotalAndFeesPerNFT(address _tokenAddress) external view returns (uint256 totalFees, uint256 feesPerNFT) {
        FeeInfo storage info = feeInfo[_tokenAddress];
        return (info.totalFees, info.feesPerNFT);
    }

    /// @notice Calculates the amount of fees claimable by an NFT holder for a specific token
    /// @param _tokenAddress The address of the token to query
    /// @param holder The address of the NFT holder
    /// @return The amount of fees claimable by the holder
    function getClaimableFees(address _tokenAddress, address holder) external view returns (uint256) {
        uint256 holderShares = nft.balanceOf(holder);
        FeeInfo storage info = feeInfo[_tokenAddress];
        uint256 amountToClaim = (info.feesPerNFT - lastClaimedFeePerNFT[_tokenAddress][holder]) * holderShares;
    ...
    function registerTokens(address[] memory _tokenAddresses) external onlyOwner {
        uint tokenLen = _tokenAddresses.length;
    ...
    function deRegisterTokens(address[] memory _tokenAddresses) external onlyOwner {
        ...
        for (uint i; i < tokenLen; i++) {
            ...
            uint registerLen = registeredTokens.length;
            for (uint j; j < registerLen; j++) {
                if (registeredTokens[j] == _tokenAddresses[i]) {
                    isTokenRegistered[_tokenAddresses[i]] = false;
                    // Move the last element into the place to delete
                    registeredTokens[j] = registeredTokens[registerLen - 1];
