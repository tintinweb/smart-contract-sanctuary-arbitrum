...
contract DealNFT is ERC721, IDealNFT, ReentrancyGuard {
    ...
    constructor(
        ...
        require(proxy_ != address(0), "proxy cannot be zero");
    ...
    function getTokenBoundAccount(uint256 tokenId) public view returns(address) {
        return _registry.account(_proxy, bytes32(abi.encode(0)), block.chainid, address(this), tokenId);
    }
    ...
    function _createTokenBoundAccount(uint256 tokenId) private returns(address) {
        ...
        address payable newWallet = payable(_registry.createAccount(_proxy, salt, block.chainid, address(this), tokenId));
        AccountProxy(newWallet).initialize(_implementation);
        AccountV3TBD newAccount = AccountV3TBD(newWallet);
...
// import {AccountV3Upgradable} from "tokenbound/src/AccountV3Upgradable.sol";
