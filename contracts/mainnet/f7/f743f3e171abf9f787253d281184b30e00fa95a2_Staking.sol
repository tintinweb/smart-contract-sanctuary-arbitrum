...
contract Staking is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(address name => uint256 balance) public balances;
    mapping(address name => uint256 time) public stakedTimestamps;
    mapping(address name => uint256 time) public lockedTimestamps;
    ...
    constructor(IERC20 contractAddress) Ownable(msg.sender) {
    ...
    /// @param timePeriodInSeconds amount of seconds to add to the initial timestamp i.e. we are essemtially creating the minimum staking period here
    ...
    function stakeTokens(
        IERC20 token,
        uint256 amount
    ) public timestampIsSet nonReentrant whenNotPaused {
        if (token != erc20Contract) {
            revert TokenNotMatch();
        }
        if (amount == 0) {
            revert ZeroAmount();
        }
        ...
        (bool success, uint256 result) = Math.tryAdd(
            balances[msg.sender],
            amount
        );
        if (!success) {
        ...
        if (stakedTimestamps[msg.sender] == 0) {
    ...
    function unstakeTokens(
        ...
        if (block.timestamp >= result) {
            (bool successSub, uint256 resultSub) = Math.trySub(
                balances[msg.sender],
                amount
            );
            if (!successSub) {
    ...
    /// how long is it still locked
    function checkTimeLocked(
        address stakingAddress
    ) public view returns (uint256) {
        if (lockedTimestamps[stakingAddress] == 0) {
            revert NoTokensLocked();
        }
        (bool success, uint256 result) = Math.tryAdd(
            lockedTimestamps[stakingAddress],
            timePeriod
        );
        if (!success) {
            revert MathFailed();
    ...
    /// @dev allows to check the timestamp of the staked tokens for a specific address
    /// @param stakingAddress - the address to check the timestamp for
    function checkTimeStaked(
        address stakingAddress
    ) public view returns (uint256) {
        if (stakedTimestamps[stakingAddress] == 0) {
            revert NoTokensLocked();
        }
        (bool successSub, uint256 resultSub) = Math.trySub(
            block.timestamp,
            stakedTimestamps[stakingAddress]
        );
        if (!successSub) {
            revert MathFailed();
        }
    ...
    /// @dev allows to get the staked ammount and timestamt at once
    /// @param stakingAddress - the address to check the balance for
    function checkStaked(
        ...
        return (balances[stakingAddress], resultSub);
    }
}