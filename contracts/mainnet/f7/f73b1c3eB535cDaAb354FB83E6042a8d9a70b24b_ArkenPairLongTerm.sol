...
import './interfaces/IArkenPairLongTermFactory.sol';
import './interfaces/IArkenPairLongTerm.sol';
import './interfaces/IERC721URIProvider.sol';
    ...
    IArkenPairLongTerm
{
        ...
        returns (uint256 liquidity, uint256 tokenId)
    {
        ...
        if (_totalSupply == 0) {
            ...
            UniswapV2ERC721._mint(address(0), MINIMUM_LIQUIDITY); // permanently lock the first MINIMUM_LIQUIDITY tokens
        } else {
            liquidity = Math.min(
...
import './interfaces/IUniswapV2ERC721.sol';

// import 'hardhat/console.sol';

contract UniswapV2ERC721 is Context, ERC165, IUniswapV2ERC721 {
    using Address for address;
    using Strings for uint256;

    string private constant _name = 'Uniswap V2 NFT';
    string private constant _symbol = 'UNI-V2-NFT';
    uint8 private constant _decimals = 18;

    // Total pool liquidity representation
    ...
    // Mapping from owner to total owned liquidity
    mapping(address => uint256) private _ownerLiquidity;

    // Mapping from token ID to liquidity
    mapping(uint256 => uint256) private _tokenLiquidity;
    ...
    function balanceOf(
        ...
        require(
            owner != address(0),
            'UniswapV2ERC721: address zero is not a valid owner'
        );
    ...
    function ownerOf(
        ...
        require(owner != address(0), 'UniswapV2ERC721: invalid token ID');
        return owner;
    }
    ...
    function liquidityOf(
        ...
        return _tokenLiquidity[tokenId];
    }

    function totalLiquidityOf(
        address owner
    ) public view virtual returns (uint256) {
        return _ownerLiquidity[owner];
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = UniswapV2ERC721.ownerOf(tokenId);
        require(to != owner, 'UniswapV2ERC721: approval to current owner');

        require(
            _msgSender() == owner || isApprovedForAll(owner, _msgSender()),
            'UniswapV2ERC721: approve caller is not token owner nor approved for all'
        );
    ...
    function transferFrom(
        ...
        require(
            ...
            'UniswapV2ERC721: caller is not token owner nor approved'
        );
     ...
     * are aware of the UniswapV2ERC721 protocol to prevent tokens from being forever locked.
     *
    ...
    function _safeTransfer(
        ...
        require(
            ...
            'UniswapV2ERC721: transfer to non ERC721Receiver implementer'
        );
    }
     ...
     * Emits a {Transfer} event.
     */
    function _safeMint(address to, uint256 value) internal virtual {
        _safeMint(to, value, '');
    }

    /**
     * @dev Same as {xref-UniswapV2ERC721-_safeMint-address-uint256-}[`_safeMint`], with an additional `data` parameter which is
    ...
    function _safeMint(
        ...
        uint256 tokenId = _mint(to, value);
        require(
    ...
    function _mint(
        ...
        require(!_exists(tokenId), 'UniswapV2ERC721: token already minted');
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        uint256 value = _tokenLiquidity[tokenId];
        totalSupply -= value;
    ...
    function _transfer(
        ...
        require(
            UniswapV2ERC721.ownerOf(tokenId) == from,
            'UniswapV2ERC721: transfer from incorrect owner'
        );
        require(
            to != address(0),
            'UniswapV2ERC721: transfer to the zero address'
        );
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(UniswapV2ERC721.ownerOf(tokenId), to, tokenId);
    }
    ...
    function _setApprovalForAll(
        ...
        require(owner != operator, 'UniswapV2ERC721: approve to caller');
    ...
    function _requireMinted(uint256 tokenId) internal view virtual {
        require(_exists(tokenId), 'UniswapV2ERC721: invalid token ID');
    }
    ...
    function _afterTokenTransfer(
        ...
        if (from != address(0)) {
            _ownerLiquidity[from] -= _tokenLiquidity[tokenId];
        }
        if (to != address(0)) {
            _ownerLiquidity[to] += _tokenLiquidity[tokenId];
        }
    }
}
