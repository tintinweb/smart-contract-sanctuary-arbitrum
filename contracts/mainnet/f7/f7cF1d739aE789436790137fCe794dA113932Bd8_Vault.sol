...
interface ISettingsManager {
    ...
    function executionGasUnits() external view returns (uint256);
}
...
contract Vault is Constants, Initializable, ReentrancyGuardUpgradeable, IVault {
    ...
    function withdraw(address _token, uint256 _amount) external payable nonReentrant preventBanners {
        _validateAndSendGasFee(settingsManager.executionGasUnits() / 2);
    ...
    function newPositionOrder(
        ...
        _validateAndSendGasFee(settingsManager.executionGasUnits());
    ...
    function newPositionOrderWithTPSL(
        ...
        _validateAndSendGasFee(settingsManager.executionGasUnits() * (1 + _prices.length));
        require(_refer != msg.sender, "Refer error");
        positionVault.newPositionOrder(msg.sender, _tokenId, _isLong, _orderType, _params, _refer);
    ...
    function addTPSL(
        ...
        _validateAndSendGasFee(settingsManager.executionGasUnits() * _prices.length);
    ...
    function _validateAndSendGasFee(uint256 _estimatedGasUnits) internal {
        uint256 estimatedGasUsed = _estimatedGasUnits * tx.gasprice;
        require(msg.value >= estimatedGasUsed, "insufficient gas fee");
        (bool success1, ) = payable(settingsManager.feeManager()).call{value: estimatedGasUsed}("");
        (bool success2, ) = payable(msg.sender).call{value: msg.value - estimatedGasUsed}("");
        require(success1 && success2, "failed to send");
    }
