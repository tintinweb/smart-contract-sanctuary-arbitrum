...
contract CashGameCashier is Ownable {
  ...
  event TakeProfitEmergency(address indexed token, uint256 amount);
  ...
  function setAllowedTokens() internal {
    ...
    allowedTokensPerChain[ETH_CHAIN_ID][ETH_USDT] = true;

    //POLYGON - chainID(137)
    uint256 POLYGON_CHAIN_ID = 137;
    address POLYGON_USDT = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F;
    allowedTokensPerChain[POLYGON_CHAIN_ID][POLYGON_USDT] = true;

    //POLYGON zkEVM - chainID(1101)
    uint256 POLYGON_ZKEVM_CHAIN_ID = 1101;
    address POLYGON_ZKEVM_USDT = 0x1E4a5963aBFD975d8c9021ce480b42188849D41d;
    allowedTokensPerChain[POLYGON_ZKEVM_CHAIN_ID][POLYGON_ZKEVM_USDT] = true;
  ...
  function takeProfitEmergency(address token, uint256 amount) public onlyOwner {
    require(IERC20(token).balanceOf(address(this)) >= amount, 'Insufficient balance');
    require(IERC20(token).transfer(msg.sender, amount), 'Transfer failed');
    emit TakeProfitEmergency(token, amount);
  }
