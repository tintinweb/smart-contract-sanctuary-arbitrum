...
contract Points2Manager is IPoints2Manager, Ownable {
    ...
    function validateToken(address token) public view returns (bool token0Valid, bool token1Valid) {
        ...
        // Validate that one of the underlying tokens is or USDC/USDT/WETH/USDM
        address token0 = pair.token0();
        address token1 = pair.token1();
        token0Valid = (token0 == USDC || token0 == USDT || token0 == WETH || token0 == USDM);
        token1Valid = (token1 == USDC || token1 == USDT || token1 == WETH || token1 == USDM);
    ...
    function setRates(address[] calldata tokens) external {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            // Validate the token is an LP token with one of the underlying tokens being USDC/USDT/WETH/USDM
