...
contract NitrilityAuction is Ownable(msg.sender), ReentrancyGuard {
    ...
    function calculateTotalPrice(
        ...
        for (uint256 i = 0; i < licenseDatas.length; i++) {
            ...
            NitrilityCommon.TemplateType
                memory templateData = INitrilityFactory(_nitrilityFactory)
                    .getTemplateData(
    ...
    function addExtraFunds(uint256 offerId) public payable {
        require(
    ...
    function acceptOffer(
        ...
        require(
            idToOffer[offerId].listedId == licenseData.listedId,
            "Listed Id Should be same"
        );
        address[] memory artistIdToAddresses = INitrilityFactory(
            _nitrilityFactory
        ).fetchArtistAddressForArtistId(licenseData.artistId);
        require(
            containsAddress(artistIdToAddresses, msg.sender),
            "only owner can accept the offer"
        );

        idToOffer[offerId].eventType = NitrilityCommon.EventTypes.OfferAccepted;
    ...
    function hashDiscountCode(
        ...
        return
            keccak256(
                abi.encode(
                    keccak256(
                        "TemplateType(string name,string code,uint256 discountType,uint256 percentage,uint256 fixedAmount,bool infinite,uint256 endTime)"
                    ),
                    discountcode.name,
                    discountcode.code,
