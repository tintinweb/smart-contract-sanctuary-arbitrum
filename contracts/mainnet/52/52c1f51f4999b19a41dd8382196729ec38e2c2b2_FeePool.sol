...
contract FeePool is IFeePool, UUPSUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    /// @inheritdoc IFeePool
    mapping(address => uint256) public override totalSharesOf;
    /// @inheritdoc IFeePool
    mapping(address => mapping(address => uint256)) public override shareOf;

    /// @notice Withdrawable amounts for accounts from indexes
    mapping(address => mapping(address => uint256)) internal withdrawableOf;
    /// @notice Accumulated rewards for accounts from indexes
    mapping(address => mapping(address => uint256)) internal lastAccumulatedTokenPerTotalSupplyInBaseOf;
    /// @notice Accumulated index rewards per total supply
    mapping(address => uint256) internal accumulatedTokenPerTotalSupplyInBaseOf;
    /// @notice Index token balances
    mapping(address => uint256) internal lastTokenBalanceOf;

    /// @notice Accumulates account rewards per index
    modifier accumulateRewards(address _index, address _account) {
        uint256 _totalShares = totalSharesOf[_index];
        if (_totalShares != 0) {
            uint256 tokenIncrease = IERC20(_index).balanceOf(address(this)) - lastTokenBalanceOf[_index];
            if (tokenIncrease != 0) {
                unchecked {
                    // overflow is desired
                    accumulatedTokenPerTotalSupplyInBaseOf[_index] += (tokenIncrease * MAX_BPs) / _totalShares;
                }
            }
        }
    ...
    /// @inheritdoc IFeePool
    function withdrawPlatformFeeOf(address _index, address _recipient) external override nonReentrant onlyOwner {
        _withdraw(_index, _index, _recipient);
    }

    /// @inheritdoc IFeePool
    function burn(address _index, MintBurnInfo calldata _burnInfo) external override onlyOwner {
        _burn(_index, _burnInfo.recipient, _burnInfo.share);
    }

    /// @inheritdoc IFeePool
    function mint(address _index, MintBurnInfo calldata _mintInfo) external override onlyOwner {
        _mint(_index, _mintInfo.recipient, _mintInfo.share);
    }

    /// @inheritdoc IFeePool
    function burnMultiple(address _index, MintBurnInfo[] calldata _burnInfo) external override onlyOwner {
        uint256 infosCount = _burnInfo.length;
        for (uint256 i; i < infosCount;) {
    ...
    /// @inheritdoc IFeePool
    function mintMultiple(address _index, MintBurnInfo[] calldata _mintInfo) external override onlyOwner {
        uint256 infosCount = _mintInfo.length;
        for (uint256 i; i < infosCount;) {
    ...
    /// @inheritdoc IFeePool
    function withdrawableAmountOf(address _index, address _account) external view override returns (uint256) {
        uint256 _accumulatedTokenPerTotalSupplyInBase = accumulatedTokenPerTotalSupplyInBaseOf[_index];
        uint256 _totalShares = totalSharesOf[_index];
        if (_totalShares != 0) {
            uint256 tokenIncrease = IERC20(_index).balanceOf(address(this)) - lastTokenBalanceOf[_index];
            if (tokenIncrease != 0) {
                unchecked {
                    // overflow is desired
                    _accumulatedTokenPerTotalSupplyInBase += (tokenIncrease * MAX_BPs) / _totalShares;
                }
            }
        }

        uint256 _lastAccumulatedTokenPerTotalSupplyInBase = shareOf[_index][_account] == 0
            ? _accumulatedTokenPerTotalSupplyInBase
            : lastAccumulatedTokenPerTotalSupplyInBaseOf[_index][_account];
        uint256 accumulatedTokenPerTotalSupplyInBaseIncrease;
        unchecked {
            // overflow is desired
            accumulatedTokenPerTotalSupplyInBaseIncrease =
                _accumulatedTokenPerTotalSupplyInBase - _lastAccumulatedTokenPerTotalSupplyInBase;
        }
        uint256 increase = (shareOf[_index][_account] * accumulatedTokenPerTotalSupplyInBaseIncrease) / MAX_BPs;
    ...
    function _accumulateAccountRewards(address _index, address _account) internal {
        ...
        unchecked {
            // overflow is desired
            accumulatedTokenPerTotalSupplyInBaseIncrease =
                accumulatedTokenPerTotalSupplyInBaseOf[_index] - _lastAccumulatedTokenPerTotalSupplyInBase;
        }
    ...
    function _withdraw(address _index, address _src, address _recipient) internal accumulateRewards(_index, _src) {
        uint256 amount = withdrawableOf[_index][_src];
        if (amount != 0) {
    ...
    function _mint(address _index, address _recipient, uint256 _share) internal accumulateRewards(_index, _recipient) {
    ...
    function _burn(address _index, address _recipient, uint256 _share) internal accumulateRewards(_index, _recipient) {
        shareOf[_index][_recipient] -= _share;
        totalSharesOf[_index] -= _share;
