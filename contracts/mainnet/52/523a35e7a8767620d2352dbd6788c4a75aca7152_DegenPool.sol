...
import "./interfaces/ISimpleAccountFactory.sol";
...
contract DegenPool is IDegenPool, AccessControlEnumerable {
  ...
  ISimpleAccountFactory public immutable accountFactory;
  ...
  FundFeeConfig public fundFeeConfig = FundFeeConfig(2, 180, 300);
  ...
  uint16 public minPositionDuration = 180; /// @notice The minimum position duration in seconds
  uint16 public minLeverage = 100; /// @notice The minimum leverage
  uint16 public maxLeverage = 1000; /// @notice The maximum leverage
  uint96 public minWager = 9e6; /// @notice The minimum wager amount 9$
  ...
  constructor(
    ...
    ISimpleAccountFactory accountFactory_,
  ...
  function updateLimits(uint96 newBudget, uint96 newMaxProfit, uint96 maxExpo) external onlyRole(CONTROLLER) {
    // Update the budget and maximum profit limit with the new values
    budget = newBudget;
    maxProfit = newMaxProfit;
    maxExposure = maxExpo;

    emit UpdateLimits(newBudget, newMaxProfit, maxExpo);
  }
  ...
  function updatePosConf(
    ...
    emit UpdatePosConf(newMinLeverage, newMaxLeverage, newMinPositionDuration, newMinWager);
  }
  ...
  function setSecondaryEnabled(bool isEnabled, uint64 _pairIndex) external onlyRole(CONTROLLER) {
    ...
    emit SecondaryEnabled(isEnabled, _pairIndex);
  }
  ...
  function verify(Order calldata order, bytes calldata signature_) public view returns (address) {
  ...
  function execute(bytes[] calldata priceData, Order calldata order, bytes calldata sig) external {
    // Verify the order's signature
    require(order.player == verify(order, sig), "Order is not verified");
    // Execute the position using the provided price data, order, and signature
    _execute(priceData, order, sig);
  }

  function executeByAA(
    bytes[] calldata priceData,
    Order calldata order,
    bytes calldata sig
  ) external {
    address signer = verify(order, sig);
    require(accountFactory.getAddress(signer, 0) == order.player, "Order is not verified");
