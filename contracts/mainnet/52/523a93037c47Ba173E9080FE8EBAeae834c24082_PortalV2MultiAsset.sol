...
contract PortalNFT is ERC721URIStorage {
    ...
    function getAccount(uint256 _tokenId) public view returns (uint256 stakedBalance, uint256 portalEnergy) {
        ...
        uint256 portalEnergyIncrease =
            (account.stakedBalance * (portal.maxLockDuration() - account.lastMaxLockDuration) * 1e18);
    ...
    function redeem(address ownerOfNFT, uint256 _tokenId)
        external
        onlyOwner
        returns (uint256 stakedBalance, uint256 portalEnergy)
    {
...
interface IPortalV2MultiAsset {
    function totalPrincipalStaked() external view returns (uint256 totalPrincipalStaked);
