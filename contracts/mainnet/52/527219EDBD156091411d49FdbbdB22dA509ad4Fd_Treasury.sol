...
contract Treasury is Operator, ReentrancyGuard {
    ...
    function openShort(uint256 _supplyAmount, uint256 _borrowAmount, uint256 _swapMinOut, uint16 _referralCode)external onlyGovernance {
        IRiskVault(riskOnPool).openShort(_supplyAmount, _borrowAmount, _swapMinOut, _referralCode);
    }
    
    
    function closeShort(uint256 _repayAmount, uint256 _swapMaxIn, uint256 _withdrawAmount)external onlyGovernance {
        IRiskVault(riskOnPool).closeShort(_repayAmount, _swapMaxIn, _withdrawAmount);
    }

    function swapAndSupplyAave(uint256 _swapAmount, uint256 _swapMinOut, uint16 _referralCode)external onlyGovernance {
        uint256 swapOut = swapPoolTokenToToken(usdt, _swapAmount, usdc, _swapMinOut, 100);
        IRiskVault(riskOnPool).supplyAave(usdc, swapOut, _referralCode);
    }

    function withdrawAaveAndSwap(uint256 _swapAmount, uint256 _swapMinOut)external onlyGovernance {
        IRiskVault(riskOnPool).withdrawAave(usdc, _swapAmount);
        swapPoolTokenToToken(usdc, _swapAmount, usdt, _swapMinOut, 100);
    }
...
interface IRiskVault {
    ...
    function openShort(uint256 _supplyAmount, uint256 _borrowAmount, uint256 _swapMinOut, uint16 _referralCode) external;
    function closeShort(uint256 _repayAmount, uint256 _swapMaxIn, uint256 _withdrawAmount)external;
