...
interface IrawBERRYToken is IERC20 {
...
import "./interfaces/tokens/IBERRYToken.sol";
import "./interfaces/tokens/IrawBERRYToken.sol";
import "./interfaces/IrawBERRYTokenUsage.sol";

/**
 * rawBERRY is BERRY's escrowed governance token.
 * It's non-transferable, except from/to whitelisted addresses.
 * It can be converted back to BERRY through a vesting process.
 * This contract is made to receive rawBERRY deposits from users in order to allocate them to Usages (plugins) contracts
 */
contract rawBERRY is Ownable, ReentrancyGuard, ERC20("rawBERRY", "rawBERRY"), IrawBERRYToken {
  using Address for address;
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;
  using SafeERC20 for IBERRYToken;

  struct rawBERRYBalance {
    uint256 allocatedAmount; // Amount of $rawBERRY allocated to a Usage
    uint256 redeemingAmount; // Total amount of $rawBERRY currently being redeemed
  }

  struct RedeemInfo {
    uint256 berryAmount; // $BERRY amount to receive when vesting has ended
    uint256 rawBERRYAmount; // $rawBERRY amount to redeem
    uint256 endTime;
    IrawBERRYTokenUsage dividendsAddress;
    uint256 dividendsAllocation; // Share of redeeming $rawBERRY to allocate to the Dividends Usage contract
  }

  IBERRYToken public immutable berryToken; // $BERRY token to convert to/from
  IrawBERRYTokenUsage public dividendsAddress; // $rawBERRY dividends contract

  EnumerableSet.AddressSet private _transferWhitelist; // addresses allowed to send/receive $rawBERRY

  mapping(address => mapping(address => uint256)) public usageApprovals; // Usage approvals to allocate $rawBERRY
  mapping(address => mapping(address => uint256)) public override usageAllocations; // Active $rawBERRY allocations to usages

  uint256 public constant MAX_DEALLOCATION_FEE = 200; // 2%
  mapping(address => uint256) public usagesDeallocationFee; // Fee paid when deallocating $rawBERRY
  ...
  // Adjusted dividends rewards for redeeming $rawBERRY
  uint256 public redeemDividendsAdjustment = 0; // 50%

  mapping(address => rawBERRYBalance) public rawBERRYBalances; // User's $rawBERRY balances
  mapping(address => RedeemInfo[]) public userRedeems; // User's redeeming instances


  constructor(IBERRYToken berryToken_) {
    berryToken = berryToken_;
  ...
  event Redeem(address indexed userAddress, uint256 rawBERRYAmount, uint256 berryAmount, uint256 duration);
  event FinalizeRedeem(address indexed userAddress, uint256 rawBERRYAmount, uint256 berryAmount);
  event CancelRedeem(address indexed userAddress, uint256 rawBERRYAmount);
  ...
  /*
   * @dev Returns user's $rawBERRY balances
   */
  function getRawBERRYBalance(address userAddress) external view returns (uint256 allocatedAmount, uint256 redeemingAmount) {
    rawBERRYBalance storage balance = rawBERRYBalances[userAddress];
    return (balance.allocatedAmount, balance.redeemingAmount);
  }

  /*
   * @dev returns redeemable $BERRY for "amount" of $rawBERRY vested for "duration" seconds
   */
  function getBERRYByVestingDuration(uint256 amount, uint256 duration) public view returns (uint256) {
   ...
   */
  function getUserRedeem(address userAddress, uint256 redeemIndex) external view validateRedeem(userAddress, redeemIndex) returns (uint256 berryAmount, uint256 rawBERRYAmount, uint256 endTime, address dividendsContract, uint256 dividendsAllocation) {
    RedeemInfo storage _redeem = userRedeems[userAddress][redeemIndex];
    return (_redeem.berryAmount, _redeem.rawBERRYAmount, _redeem.endTime, address(_redeem.dividendsAddress), _redeem.dividendsAllocation);
  }

  /**
   * @dev returns approved $rawBERRY to allocate from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev returns allocated $rawBERRY from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev returns if "account" is allowed to send/receive $rawBERRY
   */
   ...
   * Must only be called by owner
   */
  function updateDividendsAddress(IrawBERRYTokenUsage dividendsAddress_) external onlyOwner {
  ...
  function updateTransferWhitelist(address account, bool add) external onlyOwner {
    require(account != address(this), "updateTransferWhitelist: Cannot remove rawBERRY from whitelist");
  ...
  /**
   * @dev Approves "usage" address to get allocations up to "amount" of $rawBERRY from msg.sender
   */
  function approveUsage(IrawBERRYTokenUsage usage, uint256 amount) external nonReentrant {
  ...
  /**
   * @dev Convert caller's "amount" of $BERRY to $rawBERRY
   */
  ...
  /**
   * @dev Convert caller's "amount" of $BERRY to $rawBERRY to "to" address
   */
  ...
  /**
   * @dev Initiates redeem process ($rawBERRY to $BERRY)
   *
   * Handles dividends' compensation allocation during the vesting process if needed
   */
  function redeem(uint256 rawBERRYAmount, uint256 duration) external nonReentrant {
    require(rawBERRYAmount > 0, "redeem: rawBERRYAmount cannot be null");
    require(duration >= minRedeemDuration, "redeem: duration too low");

    _transfer(msg.sender, address(this), rawBERRYAmount);
    rawBERRYBalance storage balance = rawBERRYBalances[msg.sender];

    // get corresponding $BERRY amount
    uint256 berryAmount = getBERRYByVestingDuration(rawBERRYAmount, duration);
    emit Redeem(msg.sender, rawBERRYAmount, berryAmount, duration);

    // if redeeming is not immediate, go through vesting process
    if(duration > 0) {
      // add to SBT total
      balance.redeemingAmount = balance.redeemingAmount.add(rawBERRYAmount);

      // handle dividends during the vesting process
      uint256 dividendsAllocation = rawBERRYAmount.mul(redeemDividendsAdjustment).div(100);
      ...
      // add redeeming entry
      userRedeems[msg.sender].push(RedeemInfo(berryAmount, rawBERRYAmount, _currentBlockTimestamp().add(duration), dividendsAddress, dividendsAllocation));
    } else {
      // immediately redeem for $BERRY
      _finalizeRedeem(msg.sender, rawBERRYAmount, berryAmount);
    }
  }
  ...
  function finalizeRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.rawBERRYAmount);
    _finalizeRedeem(msg.sender, _redeem.rawBERRYAmount, _redeem.berryAmount);

    // handle dividends compensation if any was active
    if(_redeem.dividendsAllocation > 0) {
      // deallocate from dividends
      IrawBERRYTokenUsage(_redeem.dividendsAddress).deallocate(msg.sender, _redeem.dividendsAllocation, new bytes(0));
    }
  ...
  function cancelRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    // make redeeming $rawBERRY available again
    balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.rawBERRYAmount);
    _transfer(address(this), msg.sender, _redeem.rawBERRYAmount);
    ...
    emit CancelRedeem(msg.sender, _redeem.rawBERRYAmount);
  ...
  /**
   * @dev Allocates caller's "amount" of available $rawBERRY to "usageAddress" contract
   *
  ...
  function allocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    // allocates $rawBERRY to usageContract
    IrawBERRYTokenUsage(usageAddress).allocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Allocates "amount" of available $rawBERRY from "userAddress" to caller (ie usage contract)
   *
   * Caller must have an allocation approval for the required $rawBERRY from "userAddress"
   */
  ...
  /**
   * @dev Deallocates caller's "amount" of available $rawBERRY from "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function deallocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    // deallocate $rawBERRY into usageContract
    IrawBERRYTokenUsage(usageAddress).deallocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Deallocates "amount" of allocated $rawBERRY belonging to "userAddress" from caller (ie usage contract)
   *
   * Caller can only deallocate $rawBERRY from itself
   */
  ...
  /**
   * @dev Convert caller's "amount" of $BERRY into $rawBERRY to "to"
   */
  function _convert(uint256 amount, address to) internal {
    ...
    berryToken.safeTransferFrom(msg.sender, address(this), amount);
  }

  /**
   * @dev Finalizes the redeeming process for "userAddress" by transferring him/her "berryAmount" and removing "rawBERRYAmount" from supply
   *
   * Any vesting check should be ran before calling this
   * $BERRY excess is automatically burnt
   */
  function _finalizeRedeem(address userAddress, uint256 rawBERRYAmount, uint256 berryAmount) internal {
    uint256 berryExcess = rawBERRYAmount.sub(berryAmount);

    // sends due $BERRY tokens
    berryToken.safeTransfer(userAddress, berryAmount);

    // burns $BERRY excess if any
    berryToken.burn(berryExcess);
    _burn(address(this), rawBERRYAmount);

    emit FinalizeRedeem(userAddress, rawBERRYAmount, berryAmount);
  }

  /**
   * @dev Allocates "userAddress" user's "amount" of available $rawBERRY to "usageAddress" contract
   *
   */
  function _allocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    uint256 approvedRawBERRY = usageApprovals[userAddress][usageAddress];
    require(approvedRawBERRY >= amount, "allocate: non authorized amount");

    // remove allocated amount from usage's approved amount
    usageApprovals[userAddress][usageAddress] = approvedRawBERRY.sub(amount);
    ...
    // adjust user's $rawBERRY balances
  ...
  /**
   * @dev Deallocates "amount" of available $rawBERRY to "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function _deallocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    // check if there is enough allocated $rawBERRY to this usage to deallocate
    ...
    // burn corresponding $BERRY and $rawBERRY
    berryToken.burn(deallocationFeeAmount);
