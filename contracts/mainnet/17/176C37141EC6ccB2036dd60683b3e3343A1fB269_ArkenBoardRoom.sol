    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        lastEpochTime = _firstEpochTime - EPOCH_DURATION;
    ...
    modifier onlyAuthorizer() {
        require(
            ...
            'Authorization: caller is authorized'
        );
        _;
    }
    ...
    function setMaxCapacity(uint256 _amount) external onlyOwner {
        require(
            ...
            'Can only set max capacity greater than or equal to current balance'
        );
        maxCapacity = _amount;
    }
    ...
    function setPenaltyFeeWallet(address _penaltyFeeWallet) external onlyOwner {
        require(
            ...
            'Cannot set penalty fee wallet to the zero address'
        );
    ...
    function setReserveFund(address _reserveFund) external onlyOwner {
        require(
            ...
            'Cannot set reserve fund to the zero address'
        );
        reserveFund = _reserveFund;
    }
    ...
    function setNextEpochTime(uint256 _nextEpochTime) external onlyOwner {
        ...
        lastEpochTime = _nextEpochTime - EPOCH_DURATION;
    }
    ...
    function getMemberShareCurEpoch(
        ...
        uint256 totalEpoch = seat.totalEpoch;
        // avoid overflow
        if (firstEpoch + totalEpoch == 0) {
            return 0;
        }
        if (
            firstEpoch <= currentEpoch &&
            currentEpoch <= firstEpoch + totalEpoch - 1
        ) {
    ...
    function earned(address _member) public view returns (uint256) {
        uint256 _share = members[_member].share;

        uint lastSnapshotIndex = lastSnapshotIndexOf(_member);
        uint256 rewardEarned = 0;
        for (
            uint256 i = members[_member].currentSnapshotIndex;
            i < lastSnapshotIndex;
            ++i
        ) {
            uint256 epochReward = (boardroomHistory[i].rewardPerShare *
                _share) / (10 ** (rpsOffset + VE_DECIMALS));
    ...
    modifier checkEpoch() {
        uint256 _nextEpochTime = lastEpochTime + EPOCH_DURATION;
        require(
    ...
    function allocateRewardManually(
        uint256 _amount
    ) public checkEpoch whenNotPaused onlyAuthorizer {
        uint256 _totalShares = totalShares[currentEpoch - 1];

        require(_amount > 0, 'Cannot allocate 0 reward');
        require(_totalShares > 0, 'Cannot allocate when share is 0');
    ...
    function _stake(
        ...
        uint256 lockDuration = (_lockDuration / EPOCH_DURATION) *
            EPOCH_DURATION;
        ...
        uint256 totalEpoch = _lockDuration / EPOCH_DURATION;
        uint256 _currentEpoch = currentEpoch;

        members[_to].currentSnapshotIndex = _currentEpoch - 1;
        members[_to].balance = _value;
        members[_to].share = share;
        members[_to].firstEpoch = _currentEpoch;
        members[_to].totalEpoch = totalEpoch;
        members[_to].unlockTime = unlockTime;

        for (
            uint256 i = _currentEpoch - 1;
            i < _currentEpoch - 1 + totalEpoch;
            ++i
        ) {
    ...
    function resetEpochTimer() external whenNotPaused {
    ...
    function _increaseStake(address _to, uint256 _amount) internal {
        ...
        require(seat.balance > 0, 'The member does not exist');
        require(block.timestamp < seat.unlockTime, 'Lock is expired');
        require(
        ...
        uint256 lockDuration = EPOCH_DURATION * seat.totalEpoch;
    ...
    function withdrawFor(address _member) external whenNotPaused onlyOwner {
        _withdraw(_member);
    }

    function canWithdraw(address _member) external view returns (bool) {
        if (members[_member].unlockTime == 0) {
            return false;
        }
    ...
    function _withdraw(address _to) internal {
        ...
        require(seat.balance > 0, 'The member does not exist');
        require(block.timestamp > seat.unlockTime, 'Lock is not expired');
        ...
        IERC20(ve).safeTransfer(_to, seat.balance);
        totalBalance -= seat.balance;

        emit Withdrawn(_to, seat.balance, block.timestamp);
    }
    ...
    function governanceRecoverUnsupported(IERC20 _token) external onlyOwner {
        require(address(_token) != ve, 'Must not governance veARKEN');
        _token.safeTransfer(owner(), _token.balanceOf(address(this)));
    }

    function governanceRecoverUnsupportedWithAmount(
        address _token,
        uint256 amount
    ) external onlyOwner {
