...
contract PROXToken is ERC20, Ownable {
    ...
    function setAddress(address usdt_) external onlyOwner {
        ...
        address pair = factory.getPool(address(this), address(usdt), uint24(3000));
        if (pair == address(0)) {
            pair = factory.createPool(address(this), address(usdt), uint24(3000));
        }
        pairs[pair] = true;
        outOrder[pair] = true;
        outOrder[address(this)] = true;
