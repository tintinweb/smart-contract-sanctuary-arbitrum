...
contract LIQUIDITYADDER is Ownable {
	...
	function autoAddLiquidity(address contractA, address contractB, uint256 tokenAmountA) external onlyCaller() {
  
    
    }

    function addCaller(address val) external onlyOwner() {
        require(val != address(0), "LIQUIDITYADDER: val is the zero address");
        _callers.add(val);
    }

    function delCaller(address caller) external onlyOwner returns (bool) {
        require(caller != address(0), "LIQUIDITYADDER: caller is the zero address");
