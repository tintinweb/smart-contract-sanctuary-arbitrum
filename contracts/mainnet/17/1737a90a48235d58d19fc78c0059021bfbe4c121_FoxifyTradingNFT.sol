...
import "./interfaces/IFoxifyTradingNFT.sol";

contract FoxifyTradingNFT is IFoxifyTradingNFTBase, ERC721EnumerableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    Wave[] public waves;
    mapping(uint256 => uint256) public waveShare;
    ...
    IFoxifyTradingNFT[] public previousAffiliation;

    function initialize(
        string memory name_,
        string memory symbol_,
        string memory baseTokenURI_,
        IFoxifyTradingNFT[] memory previousAffiliation_
    ) initializer public {
        ...
        require(previousAffiliation_.length == 2, "FoxifyTradingNFT: Should be 2 versions of NFT");
        previousAffiliation = previousAffiliation_;
        _baseTokenURI = baseTokenURI_;
        // Updating using latest contract information
        _tokensCount = Counters.Counter(previousAffiliation[1].tokensCount()); 
        _updateMergeLevelRates(previousAffiliation[1].mergeLevelRates());
        _updateMergeLevelPermissions(previousAffiliation[1].mergeLevelPermissions());
    }
    ...
    function merge(uint256[] memory ids, Level from) external nonReentrant returns (bool) {
        ...
        require(fromIsBronze || from == Level.SILVER, "FoxifyTradingNFT: Invalid from level");
        ...
        Level to;
        if (fromIsBronze) {
            require(permissions.bronzeToSilver, "FoxifyTradingNFT: BronzeToSilver permission denied");
            to = Level.SILVER;
        } else {
            require(permissions.silverToGold, "FoxifyTradingNFT: SilverToGold permission denied");
            to = Level.GOLD;
        }
        require(mergeRate == ids.length, "FoxifyTradingNFT: Invalid params length");
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 id = ids[i];
            require(data[id].level == from, "FoxifyTradingNFT: Level is not match");
            require(ownerOf(id) == msg.sender, "FoxifyTradingNFT: Invalid id");
            _burn(id);
        }
    ...
    function migrate(uint256[] memory versions, uint256[] memory tokenIds) external returns (bool) {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 version = versions[i];
            uint256 tokenId = tokenIds[i];
            IERC721Upgradeable previousAffiliation_ = IERC721Upgradeable(address(previousAffiliation[version]));
            require(blackListIds[tokenId], "FoxifyTradingNFT: Black listed for migration");
            previousAffiliation_.transferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, tokenId);
            data[tokenId] = previousAffiliation[version].data(tokenId);
            _safeMint(msg.sender, tokenId);
            blackListIds[tokenId] = true;
        }
        emit Migrated(msg.sender, tokenIds);
        return true;
    }

    function processBatchMigration(uint256[] memory versions, uint256[] memory tokenIds) external onlyOwner {
        address[] memory userList = new address[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            uint256 version = versions[i];
            IERC721Upgradeable previousAffiliation_ = IERC721Upgradeable(address(previousAffiliation[version]));
            require(blackListIds[tokenId], "FoxifyTradingNFT: Black listed for migration");
            address user = previousAffiliation_.ownerOf(tokenId);
            userList[i] = user;
            data[tokenId] = previousAffiliation[version].data(tokenId);
            _safeMint(user, tokenId);
            blackListIds[tokenId] = true;
        }
        emit BatchMigrated(userList, tokenIds);
    }

    function mintRequest(bytes32[] calldata merkleProof) external returns (bool) {
        (uint256 waveIndex, Wave memory wave) = currentWave();
        require(block.timestamp >= wave.start && block.timestamp <= wave.end, "FoxifyTradingNFT: All waves expired");
        require(!claimed[waveIndex][msg.sender], "FoxifyTradingNFT: Already claimed");
        require(merkleProof.length > 0, "FoxifyTradingNFT: Invalid proofs length");
        require(
            MerkleProof.verify(merkleProof, wave.root, keccak256(bytes.concat(keccak256(abi.encode(msg.sender))))) ==
                true,
            "FoxifyTradingNFT: Invalid proofs"
        );
        bytes32 _value = keccak256(abi.encodePacked(blockhash(block.number - 1), msg.sender));
        uint256 randomValue = uint256(_value) % waveShare[waveIndex];
        Level level;
    ...
    function processMint(address user, Level level) external onlyOwner {
        bytes32 _value = keccak256(abi.encodePacked(blockhash(block.number - 1), user));
        uint256 tokenId = _safeMint(user);
        NFTData memory data_ = NFTData(level, _value, block.timestamp);
        data[tokenId] = data_;
        emit Minted(user, tokenId, data_);
    }


    function processBlackList(uint256[] memory tokenIds, bool status) external onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(!_exists(tokenIds[i]), "FoxifyTradingNFT: Already Migrated or Minted");
            blackListIds[tokenIds[i]] = status;
        }
    }

    function scheduleWave(Wave memory wave, uint256 totalShare) external onlyOwner returns (bool) {
        require(wave.root != bytes32(0), "FoxifyTradingNFT: Root is zero bytes");
        require(wave.start >= block.timestamp, "FoxifyTradingNFT: Current lt start");
        require(wave.end > wave.start, "FoxifyTradingNFT: End lte start");
        require(
            wave.distribution.bronze + wave.distribution.silver + wave.distribution.gold == totalShare,
            "FoxifyTradingNFT: Distribution ne Total Share"
        );
        if (waves.length > 0) {
            Wave memory lastWave = waves[waves.length - 1];
            require(wave.start > lastWave.end, "FoxifyTradingNFT: New wave start lt last wave end");
        }
        waves.push(wave);
        waveShare[waves.length - 1] = totalShare;
        emit WaveScheduled(waves.length - 1, wave);
        return true;
    }

    function unscheduleWave(uint256 index) external onlyOwner returns (bool) {
        require(index < waves.length, "FoxifyTradingNFT: Wave not exist");
        Wave memory targetWave = waves[index];
        require(targetWave.start > block.timestamp, "FoxifyTradingNFT: Wave already started");
    ...
    function updateUserActiveID(uint256 tokenId) external returns (bool) {
        if (tokenId > 0) require(ownerOf(tokenId) == msg.sender, "FoxifyTradingNFT: Incorrect owner");
    ...
    function _updateMergeLevelRates(MergeLevelRates memory rates) private {
        require(rates.bronzeToSilver > 0 && rates.silverToGold > 0, "FoxifyTradingNFT: Rate is not positive");
...
import "./IFoxifyTradingNFTBase.sol";

interface IFoxifyTradingNFT is IFoxifyTradingNFTBase {
...
interface IFoxifyTradingNFTBase {
    enum Level {
        UNKNOWN,
        BRONZE,
        SILVER,
    ...
    event BatchMigrated(address[] userList, uint256[] tokenIds);
    ...
    function scheduleWave(Wave memory wave, uint256 totalShare) external returns (bool);
