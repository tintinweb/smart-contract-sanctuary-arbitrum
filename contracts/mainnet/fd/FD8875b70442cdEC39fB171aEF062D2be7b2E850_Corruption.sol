...
contract Corruption is Initializable, CorruptionState {
    ...
    function setCorruptionStreamInfo(address _account, uint128 _newRatePerSecond, uint256 _generatedCorruptionCap, uint256 _corruptionMinimum) external onlyAdminOrOwner {
        ...
        addressToStreamInfo[_account].corruptionMinimum = _corruptionMinimum;

        // If a corruptionMinimum was set, this will mint the required tokens to reach the minimum.
        //
        _mintAccumulatedTokens(_account);

        emit CorruptionStreamModified(_account, _newRatePerSecond, _generatedCorruptionCap);
        emit CorruptionStreamMinimumModified(_account, _corruptionMinimum);
    }
    ...
    function _transfer(
        ...
        _mintAccumulatedTokens(from);
        _mintAccumulatedTokens(to);
    ...
    function _tokensAccumulated(address _account) private view returns(uint256, bool) {
        ...
        uint256 _corruptionMinimum = _streamInfo.corruptionMinimum;
        ...
        } else if(_tokenBalance + _rawAccumulated < _corruptionMinimum) {
            return (_corruptionMinimum - _tokenBalance, _hasStream);
        } else {
    ...
    function corruptionMinimum(address _account) external view returns(uint256) {
        return addressToStreamInfo[_account].corruptionMinimum;
    }
}
...
abstract contract CorruptionState is Initializable, ICorruption, ERC20Upgradeable, AdminableUpgradeable {
    ...
    event CorruptionStreamMinimumModified(address _account, uint256 _corruptionMinimum);
...
struct CorruptionStreamInfo {
    ...
    // Slot 4
    // The minimum amount of corruption that this stream has. Corruption cannot go below
    // this amount if it has already reached it.
    //
    uint256 corruptionMinimum;
}
