    ...
    UUPSUpgradeable
{
    ...
    function manualClaimBribes(
        ...
        for (uint i; i < rewarders.length; ++i) {
            ...
            IFeeDistributor(rewarders[i]).getReward(tokenID, _tokens);
            uint len = _tokens.length;
            for (uint j; j < len; ++j) {
                uint bal = IERC20Upgradeable(_tokens[j]).balanceOf(
                    address(this)
                );
                IERC20Upgradeable(_tokens[j]).transfer(msg.sender, bal);
            }
        }
    }
            ...
            for (uint i; i < tokens.length; ++i) {
                ...
                if (bal > 0) {
                    ...
                    if (tokens[i] == ram) {
                        ...
                        neadStake.notifyRewardAmount(ram, bal - fee);
                        emit ClaimBribe(
                            veDepositor,
                            msg.sender,
                            bal - fee,
                        ...
                        uint amountOut;
                        try swap.swapTokens(tokens[i], swapTo, bal) returns (
                            uint _amountOut
                        ) {
    ...
    function processPerformanceFees(
        ...
        if (bal > 0) {
            ...
            if (token == ram) {
                neadStake.notifyRewardAmount(ram, stakersShare);
                ...
                uint amountOut;
                try swap.swapTokens(token, veDepositor, stakersShare) returns (uint _amountOut) {
                    amountOut = _amountOut;
                } catch {
                    return;
                }
