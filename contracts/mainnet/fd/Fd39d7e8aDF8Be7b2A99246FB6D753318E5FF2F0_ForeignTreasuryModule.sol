...
/**
 * @title Cross-chain Treasury Module
 * @notice A cross-chain module that implements a treasury for a DAO, enabling financial transactions
 * on another chain based on DAO member voting on source chain.
 */
contract CrossChainTreasuryModule is
    IDAOResource,
    IDAOIntegration,
    ERC165,
    UUPSUpgradeable,
    ...
    Router
{
    ...
    function __CrossChainTreasuryModule_init(
    ...
    function __CrossChainTreasuryModule_initAndSetDeps(
        address registry_,
        uint32 remoteDomain_,
        address ism_,
        bytes32 remoteRouter_,
    ...
    /**
     * @notice Relays a message to send native currency to a specified address on another chain.
     * @dev Allows a DAO to distribute native currency from its Treasury.
     * @param to_ The address to send native currency to.
     * @param amount_ The amount of native currency to send.
     */
    function sendNative(address to_, uint256 amount_) external virtual onlyUpdatePermission {
        _relayMessageAction(ETHEREUM_ADDRESS, to_, amount_);
    }

    /**
     * @notice Relays a message to send ERC20 tokens to a specified address on another chain.
     * @dev Enables a DAO to distribute ERC20 tokens from its Treasury.
     * @param token_ The address of the ERC20 token to send.
     * @param to_ The address to send ERC20 tokens to.
     * @param amount_ The amount of ERC20 tokens to send.
     */
    function sendERC20(
        address token_,
        address to_,
        uint256 amount_
    ) external virtual onlyUpdatePermission {
        _relayMessageAction(token_, to_, amount_);
    }
    ...
    function redeemTokens(
        address newOwner_, uint256 value_
    ) public onlyUpdatePermission {
        (bool success_, ) = newOwner_.call{value: value_}("");

        require(success_, "CrossChainTreasuryModule: sending error.");
    }
    ...
    function getEncodedOperation(
        ...
        return abi.encode(MessageAction(token_, to_, amount_));
    }
    ...
    function _requirePermission(string memory permission_) internal view {
        require(
            checkPermission(msg.sender, permission_),
            "CrossChainTreasuryModule: permission denied."
        );
    }

    function _relayMessageAction(address token_, address to_, uint256 amount_) internal {
        MessageAction memory operation_ = MessageAction(token_, to_, amount_);
...
import {TokenBalance} from "@q-dev/gdk-contracts/libs/utils/TokenBalance.sol";

import {Router} from "@hyperlane-xyz/core/contracts/client/Router.sol";

import {MessageAction} from "./CrossChainTreasuryModule.sol";

/**
 * @title Foreign Treasury Module
 * @notice A cross-chain module for sending native currency and ERC20 tokens.
 * It allows the DAO to:
 *        - Send native currency to any address.
 *        - Send ERC20 tokens to any address.
 */
contract ForeignTreasuryModule is Router {
    using TokenBalance for address;
    ...
    /**
     * @notice Retrieves the native currency balance of the Treasury.
     * @dev Returns the amount of native currency held by the Treasury contract.
     * @return The balance of native currency in the Treasury.
     */
    ...
    /**
     * @notice Retrieves the balance of a specified ERC20 token in the Treasury.
     * @dev Returns the amount of the specified ERC20 token held by the Treasury contract.
     * @param token_ The address of the ERC20 token.
     * @return The balance of the specified ERC20 token in the Treasury.
     */
    function getERC20Balance(address token_) external view returns (uint256) {
    ...
    function _handle(uint32 origin_, bytes32 sender_, bytes calldata message_) internal override {
        ...
        require(operation_.amount != 0, "ForeignTreasuryModule: amount is zero");

        _sendFunds(operation_.token, operation_.to, operation_.amount);

        emit MessageReceived(origin_, mailbox.localDomain(), sender_, operation_);
    }

    function _sendFunds(address token_, address receiver_, uint256 amount_) internal {
        token_.sendFunds(receiver_, amount_);
    }
}