...
contract PlayerBook is OwnableUpgradeable {
        ...
        public
    {
        ...
        _plyr[addr].level = 1;
        _plyr[affCode].hasInvite += 1;
        addrExist[addr] = true;

        inviteAccountsTotal = inviteAccountsTotal.add(1);
        RegisteInfo memory itemRecord;
        itemRecord.addr = msg.sender;
        itemRecord.aff_addr = affCode;
        itemRecord._time = block.timestamp;
        inviteAccounts.push(itemRecord);
    }


    function settle(address accountAddr,uint _amount) 
        internal
    {
        address curaddr = accountAddr;
        address affID;
        uint curlvl = 1;

        _plyr[curaddr].reward = _plyr[curaddr].reward.add(_amount.mul(3000).div(_baseRate));
        
        while(curaddr != _teamWallet && curlvl < 7)
        {
            uint _tr = 0;
            affID = _plyr[curaddr].laff;

            if(_plyr[affID].level >= curlvl)
            {
                for(uint i = curlvl;i <= _plyr[affID].level;i++)
                    _tr += rankRate[i-1];
                if(stakeNFT(stakeNFTCon).balanceOf(affID) > 0)
                {
                    _plyr[affID].reward = _plyr[affID].reward.add(_amount.mul(_tr).div(_baseRate));
                }else{
                    _plyr[_teamWallet].reward = _plyr[_teamWallet].reward.add(_amount.mul(_tr).div(_baseRate));
                }
                
                curlvl = _plyr[affID].level + 1;
            }
        ...
        uint tr = 0;
        for(uint i = curlvl;i <= 7;i++)
            tr += rankRate[i-1];
        _plyr[_teamWallet].reward = _plyr[_teamWallet].reward.add(_amount.mul(tr).div(_baseRate));
    }

    function stake(address[] memory accountAddr,uint[] memory _amounts) 
        onlyOwner
        external
    {
        ...
        {
            settle(accountAddr[i],_amounts[i]);
        }
    }
    ...
    {
        return (_plyr[from].laff,_plyr[from].reward,_plyr[from].hasReward,_plyr[from].level);
    }
    ...
    function setStakeNFTCon(address new_stakeNFTCon)
        public onlyOwner
    {
        stakeNFTCon = new_stakeNFTCon;
    }


    function setWhiteList(address[] memory accountAddr,uint[] memory _lvls)
        onlyOwner
        public
    {
        require( _lvls.length >= accountAddr.length, "len err");

        for(uint i=0;i<accountAddr.length;i++)
        {
            require( _lvls[i] >= 1 && _lvls[i] <= 7, "lvl err");
            _plyr[accountAddr[i]].level = _lvls[i];
        }
    }
}