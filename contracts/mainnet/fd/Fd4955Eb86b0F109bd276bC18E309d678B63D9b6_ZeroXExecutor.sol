...
import { ExecutorIntegration } from './interfaces/IExecutor.sol';
...
contract VaultBaseInternal {
    modifier onlyTransport() {
        VaultBaseStorage.Layout storage l = VaultBaseStorage.layout();
        require(address(l.registry.transport()) == msg.sender, 'not transport');
        _;
    }
    ...
    modifier onlyManager() {
        VaultBaseStorage.Layout storage l = VaultBaseStorage.layout();
        require(l.manager == msg.sender, 'not manager');
        _;
    }

    function initialize(Registry registry, address manager) internal {
    ...
    function _gmxPositionCallback(
        ...
        VaultBaseStorage.Layout storage l = VaultBaseStorage.layout();
        console.log('gmxPositionCallback base', gasleft());
        require(
...
contract VaultOwnership is ERC721BaseInternal, VaultFees {
...
import { VaultBaseInternal } from '../vault-base/VaultBaseInternal.sol';
import { VaultOwnership } from '../vault-ownership/VaultOwnership.sol';
import { Registry } from '../registry/Registry.sol';
...
contract VaultParentManager is VaultParentInternal, IVaultParentManager {
    ...
    function requestBridgeToChain(
        ...
        uint amount,
        uint minAmountOut
    ) external payable onlyManager noBridgeInProgress {
        VaultParentStorage.Layout storage l = VaultParentStorage.layout();
    ...
    // Only 1 sibling can be created at a time
    // If two were requested one after the other the second one wouldn't know about the first ones existence
    function requestCreateSibling(
        uint16 newChainId
    ) external payable onlyManager {
        VaultParentStorage.Layout storage l = VaultParentStorage.layout();
        uint fee = _getSendQuote(
            this.requestCreateSibling.selector,
            newChainId
        );
        require(msg.value >= fee, 'insufficient fee');
        ...
        _registry().transport().sendVaultChildCreationRequest{ value: fee }(
    ...
    function changeManager(address newManager) external payable onlyManager {
        ...
        _sendChangeManagerRequestToChildren(newManager);
    }

    function _sendChangeManagerRequestToChildren(address newManager) internal {
        VaultParentStorage.Layout storage l = VaultParentStorage.layout();
        (uint[] memory fees, uint totalFees) = _getSendQuoteMultiChain(
            this.changeManager.selector,
            l.childChains
        ...
        require(msg.value >= totalFees, 'insufficient fee');
        for (uint8 i = 0; i < l.childChains.length; i++) {
            _sendChangeManagerRequest(l.childChains[i], newManager, fees[i]);
        }
    }
    ...
    function levyFeesOnHoldings(uint[] memory tokenIds) external onlyManager {
    ...
    function sendBridgeApproval(
        ...
        VaultParentStorage.Layout storage l = VaultParentStorage.layout();
        uint fee = _getSendQuote(this.sendBridgeApproval.selector, dstChainId);
        require(msg.value >= fee, 'insufficient fee');
        ...
        require(l.lastBridgeCancellation + 1 hours < block.timestamp);
        address dstVault = l.children[dstChainId];
        require(dstVault != address(0), 'no dst vault');
        l.bridgeInProgress = true;

        _registry().transport().sendBridgeApproval{ value: fee }(
