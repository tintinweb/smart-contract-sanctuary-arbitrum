...
library CallbackUtils {
    ...
    function afterDepositExecution(
        ...
        if (!isValidCallbackContract(deposit.callbackContract())) {
            return;
        }

        try
            IDepositCallbackReceiver(deposit.callbackContract()).afterDepositExecution{gas: deposit.callbackGasLimit()}(
                key,
                deposit,
                eventData
            )
    ...
    function afterDepositCancellation(
        ...
        try
            IDepositCallbackReceiver(deposit.callbackContract()).afterDepositCancellation{
                gas: deposit.callbackGasLimit()
            }(key, deposit, eventData)
        {} catch {
    ...
    function afterWithdrawalExecution(
        ...
        if (!isValidCallbackContract(withdrawal.callbackContract())) {
            return;
        }

        try
            IWithdrawalCallbackReceiver(withdrawal.callbackContract()).afterWithdrawalExecution{
                gas: withdrawal.callbackGasLimit()
            }(key, withdrawal, eventData)
        {} catch {
    ...
    function afterWithdrawalCancellation(
        ...
        try
            IWithdrawalCallbackReceiver(withdrawal.callbackContract()).afterWithdrawalCancellation{
                gas: withdrawal.callbackGasLimit()
            }(key, withdrawal, eventData)
        {} catch {
    ...
    function afterOrderExecution(
        ...
        if (!isValidCallbackContract(order.callbackContract())) {
            return;
        }

        try
            IOrderCallbackReceiver(order.callbackContract()).afterOrderExecution{gas: order.callbackGasLimit()}(
                key,
                order,
                eventData
            )
    ...
    function afterOrderCancellation(
        ...
        try
            IOrderCallbackReceiver(order.callbackContract()).afterOrderCancellation{gas: order.callbackGasLimit()}(
                key,
                order,
                eventData
            )
    ...
    function afterOrderFrozen(
        ...
        try
            IOrderCallbackReceiver(order.callbackContract()).afterOrderFrozen{gas: order.callbackGasLimit()}(
                key,
                order,
                eventData
            )
    ...
    function isValidCallbackContract(address callbackContract) internal view returns (bool) {
        ...
        if (!callbackContract.isContract()) {
            return false;
        }
...
library BaseOrderUtils {
    ...
    function getExecutionPriceForDecrease(
        ...
        if (
            (isLong && cache.executionPrice >= acceptablePrice) || (!isLong && cache.executionPrice <= acceptablePrice)
        ) {
