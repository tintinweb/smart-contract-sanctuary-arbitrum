...
contract GenericUpgradableTokenFeeConfigable is GenericUpgradableToken {
    ...
    bytes32 internal constant STORAGE_SLOT = keccak256("my.unique.namespace.genericupgradabletokenfeeconfigable");

    function contractStorage() private pure returns (ContractStorage storage cs) {
    ...
    //set tax distributer
    function setTaxDistributer(address initialTaxDistribAddress) public onlyOwner {
        ContractStorage storage cs = contractStorage();
        cs.paused = false;  // Default value for paused
        cs.taxDistribAddress = initialTaxDistribAddress;
    }
    ...
    modifier whenNotPaused() {
        require(!contractStorage().paused, "Contract is paused");
        _;
    }
    ...
    function configureFee(address targetAddr, uint256 transferType, uint256 sendingFee, uint256 receivingFee) public onlyOwner {
        require(transferType >= 1 && transferType <= 3, "Invalid transfer type");
        ContractStorage storage cs = contractStorage();
    ...
    function getFeeInfo(address addr) public view returns (uint256, uint256, uint256) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override whenNotPaused {
        ...
        if (senderFeeInfo.transferType == 2 || senderFeeInfo.transferType == 3) {
            senderFee = amount.mul(senderFeeInfo.sendingFee).div(10000);
            amount = amount.sub(senderFee);
        }

        if (receiverFeeInfo.transferType == 1 || receiverFeeInfo.transferType == 3) {
        ...
        if (senderFee > 0) {
            super._transfer(from, contractStorage().taxDistribAddress, senderFee);
        }
        if (receiverFee > 0) {
            super._transfer(to, contractStorage().taxDistribAddress, receiverFee);
        }
    }
}
