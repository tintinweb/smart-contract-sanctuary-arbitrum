...
library SiloRewardPT {
    ...
    function assetBalanceProtected(address assetPool) public view returns (uint256) {
        return IERC20(assetPool).balanceOf(address(this));
    }
     ...
     */
    function swapCamelotExactInputSingle(
        address tokenIn,
        address tokenOut,
        uint256 amount,
        uint256 amountOutMinimum
    ) public returns (uint256) {
        TransferHelper.safeApprove(tokenIn, address(camelotRouter), type(uint256).max);
        ...
        uint256 amountout = ICamelotRouter(camelotRouter).exactInputSingle(params);

        TransferHelper.safeApprove(tokenIn, camelotRouter, 0);
     ...
     * @return The amount of output token received after the swap.
     */
    function swapCamelotExactOutputSingle(
        address tokenIn,
        address tokenOut,
        uint256 amount,
        uint256 amountInMaximum
        ...
        ICamelotRouter.ExactOutputSingleParams memory params = ICamelotRouter.ExactOutputSingleParams({
            tokenIn: tokenIn,
            tokenOut: tokenOut,
            fee: 100,
            recipient: address(this),
        ...
        uint256 amountIn = ICamelotRouter(camelotRouter).exactOutputSingle(params);
    ...
    function supply(address asset, uint256 amount, address poolAddress, bool isProtected) public returns (uint256) {
        ...
        if (isProtected) {
            (collateralAmount, ) = ISiloStrategy(poolAddress).deposit(asset, amount, true);
        } else {
            (collateralAmount, ) = ISiloStrategy(poolAddress).deposit(asset, amount, false);
        }
    ...
    function borrow(address debtToken, uint256 amount, address poolAddress) public returns (uint256) {
        ...
        (debtAmount, ) = ISiloStrategy(poolAddress).borrow(debtToken, amount);
    ...
    function repay(address debtToken, uint256 amount, address poolAddress) public returns (uint256) {
        ...
        (uint256 repaidAmount, ) = ISiloStrategy(poolAddress).repay(debtToken, amount);
    ...
    function withdraw(
        ...
        // Determine the actual amount withdrawn based on protection status
        if (isProtected) {
            (withdrawnAmount, ) = ISiloStrategy(poolAddress).withdraw(asset, amount, true);
        } else {
            (withdrawnAmount, ) = ISiloStrategy(poolAddress).withdraw(asset, amount, false);
        }

        // Transfer the actual withdrawn amount back to the caller.
        IERC20(asset).safeTransfer(msg.sender, withdrawnAmount);

        // Return the actual amount withdrawn to the caller.
        return withdrawnAmount;
    }

    function _swapPendle(
        bool isDeposit,
        uint256 amount,
        address lrtToken,
...
import { Math } from '@openzeppelin/contracts/utils/math/Math.sol';
import { TransferHelper } from '@uniswap/v3-periphery/contracts/libraries/TransferHelper.sol';
import { SiloRewardPT } from '../../libraries/SiloRewardPT.sol';
...
contract SiloPTezETHStrategy is
    Initializable,
    ILeverageStrategy,
    IFlashLoanRecipient,
    ...
    UUPSUpgradeable
{
    ...
    address immutable PT_ADDRESS;

    address immutable LRT_TOKEN;

    address immutable PENDLE_MARKET;

    bool immutable IS_PROTECTED;
    ...
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(address _PT_ADDRESS, address _LRT_TOKEN, address _PENDLE_MARKET, bool _IS_PROTECTED) {
        _disableInitializers();
        PT_ADDRESS = _PT_ADDRESS;
        LRT_TOKEN = _LRT_TOKEN;
        PENDLE_MARKET = _PENDLE_MARKET;
    ...
    function initialize(
        ...
        require(_assetAddress == PT_ADDRESS);
    ...
    function getAssetTokens() public view returns (address[] memory allTokens) {
        return SiloRewardPT.getAssetTokens(getPoolAddress());
    }
    ...
    function assetBalance() public view returns (uint256) {
        return
            IS_PROTECTED
                ? SiloRewardPT.assetBalanceProtected(assetPool())
                : SiloRewardPT.assetBalance(getPoolAddress(), assetPool(), asset());
    }

    function debtBalance() public view returns (uint256) {
        return SiloRewardPT.debtBalance(getPoolAddress(), debtPool(), debtToken());
    }
    ...
    function addLeverage(uint256 amount, uint256 debt, bytes calldata data) external onlyOwner {
        ...
        if (amount > 0) {
            ...
            ISiloStrategy(poolAddress).deposit(asset(), amount, IS_PROTECTED);
        }
    ...
    function _flAddLeverage(bytes calldata params, uint256 feeAmount) internal {
        ...
        // Swap debt to LRT_TOKEN and get the swapped amount
        outAmount = this.swapBySelf(debtToken(), getLRTTokenAddress(), amount, data);
        // deposit and get PT Token
        TransferHelper.safeApprove(getLRTTokenAddress(), pendleRouterV3, outAmount);
        netAmountOut = SiloRewardPT._swapPendle(true, outAmount, getLRTTokenAddress(), getPendleMarket());
        TransferHelper.safeApprove(getLRTTokenAddress(), pendleRouterV3, 0);

        // Supply the asset to the Silo Vault
        TransferHelper.safeApprove(asset(), poolAddress, netAmountOut);
        ISiloStrategy(poolAddress).deposit(asset(), netAmountOut, IS_PROTECTED);
    ...
    function _flRemoveLeverage(bytes calldata params, uint256 feeAmount) internal {
        ...
        // withdraw
        ISiloStrategy(poolAddress).withdraw(asset(), amount, IS_PROTECTED);

        // swap LRT_TOKEN to wETH
        // withdraw and get LRT_TOKEN
        TransferHelper.safeApprove(getPTTokenAddress(), pendleRouterV3, amount);
        netAmountOut = SiloRewardPT._swapPendle(false, amount, getLRTTokenAddress(), getPendleMarket());
        TransferHelper.safeApprove(getPTTokenAddress(), pendleRouterV3, 0);

        // Swap LRT_TOKEN to debtToken and get the swapped amount
        uint256 amountOutMinimum = abi.decode(data, (uint256));
        outAmount = _swapCamelot(getLRTTokenAddress(), debtToken(), netAmountOut, amountOutMinimum, false);
    ...
    function closeLeverage(uint256 amount, bytes calldata data) external onlyOwner {
        ...
        uint256 lrtAmount = IERC20(getLRTTokenAddress()).balanceOf(address(this));
        ...
        IERC20(getLRTTokenAddress()).safeTransfer(owner(), lrtAmount);
    ...
    function _flCloseLeverage(bytes calldata params, uint256 feeAmount) internal {
        ...
        ISiloStrategy(poolAddress).withdraw(asset(), type(uint256).max, IS_PROTECTED);

        (uint256 amountOutMinimum, bool closeToAsset) = abi.decode(data, (uint256, bool));
        if (isExpired()) {
            IERC20(PT_ADDRESS).transfer(YT(), IERC20(PT_ADDRESS).balanceOf(address(this)));
            netSyOut = IPYieldTokenV2(YT()).redeemPY(SY());
            amountTokenOut = IPYieldTokenV2(SY()).redeem(address(this), netSyOut, LRT_TOKEN, 0, true);
            // When both expired and close to asset, swap some amounts to debtToken to repay.
            if (closeToAsset) {
                _swapCamelot(getLRTTokenAddress(), debtToken(), repayAmount + feeAmount, amountOutMinimum, true);
            } else {
                // If expired but not closing to asset, swap all amounts to debtToken to repay.
                _swapCamelot(getLRTTokenAddress(), debtToken(), amountTokenOut, amountOutMinimum, false);
            }
        } else {
            // Withdraw and get LRT_TOKEN (common initial step)
            TransferHelper.safeApprove(getPTTokenAddress(), pendleRouterV3, type(uint256).max);
            netAmountOut = SiloRewardPT._swapPendle(
                false,
                IERC20(getPTTokenAddress()).balanceOf(address(this)),
                getLRTTokenAddress(),
                getPendleMarket()
            );
            TransferHelper.safeApprove(getPTTokenAddress(), pendleRouterV3, 0);
            ...
            // closeToAsset == True
            // WD PT_TOKEN to LRT_TOKEN
            // then swap LRT_TOKEN to wETH
            // then deposit again to get PT_TOKEN Back to user
            if (closeToAsset) {
                // Swap LRT_TOKEN to debtToken and get the swapped amount (specific for closeToAsset == true)
                _swapCamelot(getLRTTokenAddress(), debtToken(), repayAmount + feeAmount, amountOutMinimum, true);

                // Deposit and get ptLRT_TOKEN back
                uint256 depoAmt = IERC20(getLRTTokenAddress()).balanceOf(address(this));
                TransferHelper.safeApprove(getLRTTokenAddress(), pendleRouterV3, depoAmt);
                netAmountOut = SiloRewardPT._swapPendle(true, depoAmt, getLRTTokenAddress(), getPendleMarket());
                TransferHelper.safeApprove(getLRTTokenAddress(), pendleRouterV3, 0);
            } else {
                // Swap LRT_TOKEN to debtToken and get the swapped amount (specific for closeToAsset == false)
                _swapCamelot(getLRTTokenAddress(), debtToken(), netAmountOut, amountOutMinimum, false);
            }
        }
    ...
    function _swapCamelot(
        ...
        if (isExactOutput) {
            inAmount = SiloRewardPT.swapCamelotExactOutputSingle(tokenIn, tokenOut, amountInput, amountInMaximum);
            feeCharge = leverageFeeCharge(inAmount, tokenIn);
        } else {
            inAmount = SiloRewardPT.swapCamelotExactInputSingle(tokenIn, tokenOut, amountInput, amountInMaximum);
            feeCharge = leverageFeeCharge(inAmount, tokenOut);
        }

        return inAmount - feeCharge;
    }
    ...
    function toAmountRoundUp(uint256 share, uint256 totalAmount, uint256 totalShares) internal pure returns (uint256) {
        return SiloRewardPT.toAmountRoundUp(share, totalAmount, totalShares);
    }
    ...
    function withdrawTokenInCaseStuck(address tokenAddress, uint256 amount) external onlyOwner {
        SiloRewardPT.withdrawTokenInCaseStuck(tokenAddress, amount, assetPool(), debtPool());

        emit WithdrawTokenInCaseStuck(tokenAddress, amount);
    }

    function supply(uint256 amount) external onlyOwner {
        SiloRewardPT.supply(asset(), amount, getPoolAddress(), IS_PROTECTED);
        emit Supply(amount);
    }

    function borrow(uint256 amount) external onlyOwner {
        SiloRewardPT.borrow(debtToken(), amount, getPoolAddress());
        emit Borrow(amount);
    }

    function repay(uint256 amount) external onlyOwner {
        SiloRewardPT.repay(debtToken(), amount, getPoolAddress());
        emit Repay(amount);
    }

    function withdraw(uint256 amount) external onlyOwner {
        SiloRewardPT.withdraw(asset(), assetPool(), amount, getPoolAddress(), IS_PROTECTED);
    ...
    function claimRewards(address rewardsToken) external onlyOwner {
        ...
        uint256 transferredAmount = SiloRewardPT.claimReward(
            incentiveAddress,
            rewardsToken,
            assetTokens,
            fee,
    ...
    function _getIncentiveAddress(address rewardsToken) internal pure returns (address) {
        if (rewardsToken == SiloRewardPT.siloToken) {
            return SiloRewardPT.siloIncentive;
        } else if (rewardsToken == SiloRewardPT.arb) {
            return SiloRewardPT.siloIncentiveSTIP;
        }
    ...
    function getPTTokenAddress() public view returns (address) {
        return PT_ADDRESS;
    }

    function getLRTTokenAddress() public view returns (address) {
        return LRT_TOKEN;
    }

    function getPendleMarket() public view returns (address) {
    ...
    function expiry() public view returns (uint256) {
        return IPYieldTokenV2(PT_ADDRESS).expiry();
    }

    function isExpired() public view returns (bool) {
        return IPYieldTokenV2(PT_ADDRESS).isExpired();
    }

    function SY() public view returns (address) {
        return IPYieldTokenV2(PT_ADDRESS).SY();
    }

    function YT() public view returns (address) {
        return IPYieldTokenV2(PT_ADDRESS).YT();
    }
