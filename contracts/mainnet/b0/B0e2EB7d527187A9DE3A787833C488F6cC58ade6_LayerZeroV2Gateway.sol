    ...
    LayerZeroV2TargetEstimateMixin
{
    ...
    // TODO
    function encodeTargetMessage(
        IActionDataStructures.TargetMessage calldata _targetMessage
    ) external pure returns (bytes memory) {
        return abi.encode(_targetMessage);
    }

    function _beforeEstimateTarget(
        bytes calldata _message
    ) internal override(LayerZeroV2TargetEstimateMixin) returns (uint256 controlValue) {
        peerMap[ESTIMATE_SOURCE_STANDARD_CHAIN_ID] = ESTIMATE_SOURCE_ADDRESS;
        layerZeroToStandardChainId[ESTIMATE_SOURCE_LZV2_EID] = ESTIMATE_SOURCE_STANDARD_CHAIN_ID;
    ...
    ) internal override(LayerZeroV2TargetEstimateMixin) returns (bool isSuccess) {
        delete peerMap[ESTIMATE_SOURCE_STANDARD_CHAIN_ID];
        delete layerZeroToStandardChainId[ESTIMATE_SOURCE_LZV2_EID];
...
abstract contract LayerZeroV2TargetEstimateMixin is Pausable, ILayerZeroReceiver {
    /**
     * @dev Contract self-reference for estimateTarget
     * @dev to account for differences in cold/warm storage access costs
     */
    ILayerZeroReceiver private __self = this;

    address internal constant ESTIMATOR_ADDRESS = 0xaAaAaAaaAaAaAaaAaAAAAAAAAaaaAaAaAaaAaaAa;
    uint32 internal constant ESTIMATE_SOURCE_LZV2_EID =
        uint32(uint256(keccak256('Source - LayerZero V2 Endpoint ID')));
    uint256 internal constant ESTIMATE_SOURCE_STANDARD_CHAIN_ID =
        uint256(keccak256('Source - EVM Chain ID'));
    uint64 internal constant ESTIMATE_NONCE = uint64(uint256(keccak256('Message Nonce')));
    address internal constant ESTIMATE_SOURCE_ADDRESS = 0xCcCCccccCCCCcCCCCCCcCcCccCcCCCcCcccccccC;
     ...
     */
    function estimateTarget(bytes calldata _message) external /*onlyEstimator*/ whenNotPaused {
        // TODO restore
        uint256 controlValue = _beforeEstimateTarget(_message);

        Origin memory estimateOrigin = Origin({
            srcEid: ESTIMATE_SOURCE_LZV2_EID,
            sender: bytes32(uint256(uint160(ESTIMATE_SOURCE_ADDRESS))),
            nonce: ESTIMATE_NONCE
        });
        ...
        bool isSuccess = _afterEstimateTarget(_message, controlValue);
...
abstract contract SystemVersionId {
    ...
    uint256 public constant SYSTEM_VERSION_ID =
        uint256(keccak256('LZV2GatewayCheck-2024-06-06-Open')); // TODO
}