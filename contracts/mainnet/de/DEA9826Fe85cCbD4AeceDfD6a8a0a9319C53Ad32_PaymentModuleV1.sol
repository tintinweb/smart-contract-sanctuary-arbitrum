...
contract PaymentModuleV1 is IDepositHandler, IPaymentModule, AccessControlEnumerable, IERC721Receiver {
    ...
    uint256 public constant REFERRER_BASIS_POINTS = 2500; // 25%
    uint256 public constant BURN_BASIS_POINTS = 2500; // 25%

    uint256 public feeCollectedLastBlock;
    uint256 public tokenBurnedLastBlock;
    ...
    function _processFee(uint256 usdAmount, address user, address referrer, address vault) private returns (uint256 weiToBeRefunded) {
        ...
        if (usdAmount > 0) {
            ...
            if (referrer != address(0)) {
                uint256 referrerUSDTShare = (usdAmount * REFERRER_BASIS_POINTS) / BASIS_POINTS;
            ...
            if (FLOKI != address(0)) {
                uint256 burnShare = (usdAmount * BURN_BASIS_POINTS) / BASIS_POINTS;
                uint256 flokiBalance = IERC20Metadata(FLOKI).balanceOf(BURN_ADDRESS);
                USDT.safeApprove(address(lpTokenProcessor), burnShare);
                bool success = lpTokenProcessor.swapTokens(address(USDT), burnShare, FLOKI, BURN_ADDRESS, mainRouter, _getV3PoolFees());
                treasuryUsdShare -= burnShare;
                require(success, "Swap failed");
                uint256 flokiBurned = IERC20Metadata(FLOKI).balanceOf(BURN_ADDRESS) - flokiBalance;
                emit FlokiBurned(tokenBurnedLastBlock, vault, burnShare, flokiBurned);
                tokenBurnedLastBlock = block.number;
            }
    ...
    function _processNativeFees(uint256 usdAmount, address referrer, address vault) private returns (uint256 weiToBeRefunded) {
        ...
        if (referrer != address(0)) {
            uint256 referrerWeiShare = (expectedWei * REFERRER_BASIS_POINTS) / BASIS_POINTS;
            ...
            uint256 usdReferrerShare = (usdAmount * REFERRER_BASIS_POINTS) / BASIS_POINTS;
    ...
    function notifyFlokiBurned(address _vault, uint256 _usdAmount, uint256 _flokiAmount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        emit FlokiBurned(tokenBurnedLastBlock, _vault, _usdAmount, _flokiAmount);
        tokenBurnedLastBlock = block.number;
    }

    function notifyReferrerSharePaid(address _vault, address _referrer, uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
