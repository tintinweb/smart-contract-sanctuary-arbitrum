...
contract ElectronVaultNFT is ERC721Enumerable {
    string public uri;
...
contract Vault is ReentrancyGuard, ElectronVaultNFT {
    ...
    constructor(
        ...
        address fees_,
        uint _maxPerVault
    ) ElectronVaultNFT(name, symbol, baseURI) {
    ...
    function payBackToken(uint256 vaultID, uint256 amount) external {
        require(usde.balanceOf(msg.sender) >= amount, "Token balance too low");
    ...
    function liquidateVault(uint256 vaultID) external {
        ...
        require(
            usde.balanceOf(msg.sender) >= debtValue,
        ...
        usde.safeTransferFrom(msg.sender, address(this), debtValue);
    ...
    returns(
        ...
        _debtAsset = address(usde);
        _vault = address(this);
    }
}

contract ElectronVault is Vault, Ownable {
    constructor(
        address CollateralPriceSourceAddress,
        uint256 minimumCollateralPercentage,
        string memory name,
    ...
    function recover(uint256 amountToken, address _address) public onlyOwner {
        usde.transfer(_address, amountToken);
    }
