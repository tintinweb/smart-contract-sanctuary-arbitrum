...
// File: contracts/PopFullProtec.sol
...
interface IPopToken {
    ...
    function fragmentToPop(uint256 value) external view returns (uint256);
    function popToFragment(uint256 pop) external view returns (uint256);
    function balanceOfUnderlying(address who) external view returns (uint256);
    function burn(uint256 amount) external;
}

contract PopFullProtec is Ownable {
    ...
    constructor(IPopToken _pop, uint256 _lockDuration, bool _depositsEnabled) {
        pop = _pop;
    ...
    function deposit(uint256 _amount) external {
        ...
        IERC20(address(pop)).safeTransferFrom(address(msg.sender), address(this), _amount);
        pop.burn(_amount);
    ...
    function withdraw(uint256 _amount) external {
        ...
        user.amount -= _amount;
        totalStaked -= _amount;
        pop.mint(address(msg.sender), _amount);
