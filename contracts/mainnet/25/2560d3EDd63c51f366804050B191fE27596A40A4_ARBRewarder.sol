    ...
    PausableUpgradeable
{
    ...
    event ARBRewadsSent(address _stakingToken, address _rewarder, uint256 _totalARBSent, uint256 lastRewardTimestamp, uint256 ARBPerSec);
    event registeredPool(address _stakingToken, uint256 _arbPersec, address _masterChef);
    event SetPool(address _stakingToken, address _masterChef, bool _isActive);
    ...
    function _calculateAndSendARB(address _stakingToken, address _rewarder) internal {
        ...
        if(!pool.isActive || pool.ARBPerSec == 0 )
            return ;
        ...
        tokenToPoolInfo[_stakingToken].lastRewardTimestamp = block.timestamp;
    }

    function _addPool(address _stakingToken, uint256 _arbPerSec, address _masterChef) internal {
    ...
    function addPools(
        ...
        if(_stakingToken.length != _arbPerSec.length || _stakingToken.length != _masterChefs.length)
            revert LengthMismatch();
        for(uint256 index = 0; index < _stakingToken.length; index++){
            _addPool(_stakingToken[index], _arbPerSec[index], _masterChefs[index]);
        }
    }

    function setPool(address _stakingToken, address _masterChef, bool _isActive) external onlyOwner {

        if(_masterChef == address(0))
            revert ZeroAddress();
        PoolInfo storage pool = tokenToPoolInfo[_stakingToken];

        if (pool.isActive) {// if setting an active pool to inactive, queue the pending arb rewards to rewarder
            if(!_isActive){
                address rewarder = IMasterPenpie(pool.masterChef).getRewarder(_stakingToken);
                _calculateAndSendARB(_stakingToken, rewarder);
            }
        } else if (_isActive) {// if setting an inactive pool as active, just set the current timestamp as lastRewardTimestamp
            pool.lastRewardTimestamp = block.timestamp;
}
        pool.masterChef = _masterChef;
        pool.isActive = _isActive;

        emit SetPool(_stakingToken, _masterChef, _isActive);
    }
