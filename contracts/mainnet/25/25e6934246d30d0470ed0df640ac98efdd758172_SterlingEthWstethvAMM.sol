...
// File contracts/SterlingEthWstEthvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract SterlingEthWstethvAMM is Ownable, Pausable {
    ...
    address public constant wsteth = address(0x5979D7b546E38E414F7E9822514be443A4800529);
    address public want;
    ...
    ISterlingRouter.Routes[] public wethToWstethPath;
    ...
    constructor(
        ...
        ISterlingRouter.Routes[] memory _wethToWstethPath
    ) {
        ...
        for (uint i; i < _wethToWstethPath.length; ++i) {
           wethToWstethPath.push(_wethToWstethPath[i]);
        }
   ...
   //Withdraws funds from masterchef
    ...
    function chargeFees(address caller) internal {
        uint256 toWeth = IERC20(str).balanceOf(address(this));

        ISterlingRouter(router).swapExactTokensForTokens(toWeth, 0, strToWethPath, address(this), block.timestamp);

        uint256 wethBal = IERC20(weth).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;
    ...
    function addLiquidity() internal {
        uint256 wethHalf = IERC20(weth).balanceOf(address(this)) / 2;
        ISterlingRouter(router).swapExactTokensForTokens(wethHalf, 0, wethToWstethPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(weth).balanceOf(address(this));
        uint256 t2Bal = IERC20(wsteth).balanceOf(address(this));

        ISterlingRouter(router).addLiquidity(weth, wsteth, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(wsteth).safeApprove(router, 0);
    ...
    function _addAllowance() internal {
        ...
        IERC20(wsteth).safeApprove(router, type(uint).max);
    }
