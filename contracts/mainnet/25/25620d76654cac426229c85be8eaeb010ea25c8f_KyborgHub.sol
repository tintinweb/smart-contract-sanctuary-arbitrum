...
import "./interfaces/hub/IKyborgHub.sol";
import "./interfaces/IKyborgHubCallbacks.sol";
import "./libraries/utils/SafeTransferLib.sol";
import "./libraries/utils/PathLib.sol";
import "./libraries/math/Math.sol";
import "./libraries/Pools.sol";
import "./KyborgHubBase.sol";

contract KyborgHub is IKyborgHub, KyborgHubBase {
    ...
    /// various view functions to a second contract (i.e. KyborgHubPositions.sol) and use delegatecall to call it.
    ...
    function deposit(
        ...
        IKyborgHubCallbacks(msg.sender).kyborgDepositCallback(token, amount, data);
    ...
    function _transferSwap(
        ...
        if (amountIn > 0) {
            ...
            IKyborgHubCallbacks(msg.sender).kyborgSwapCallback(tokenIn, tokenOut, amountIn, amountOut, data);
...
import "./interfaces/hub/IKyborgHubBase.sol";
import "./interfaces/common/IERC20Minimal.sol";
import "./libraries/Pools.sol";

abstract contract KyborgHubBase is IKyborgHubBase {
...
interface IKyborgHubView {
    ...
    /// @notice Return the value of a slot in KyborgHub contract
    function getStorageAt(bytes32 slot) external view returns (bytes32 word);
}
...
interface IKyborgHubActions {
    ...
    ///                         is still an outstanding balance, hub contract will call the sender's `kyborgSwapCallback` to request
