...
import {SelfManagedLogicWithUtils} from "../templates/SelfManagedLogicWithUtils.sol";
import {USDCe, USDT, MIM} from "../../constants/arbitrumOne.sol";

contract AbracadabraMIM2Crv is SelfManagedLogicWithUtils {
    using SafeERC20 for IERC20;

    address public constant _2pool = 0x7f90122BF0700F9E7e1F688fe926940E8839F353;
    address public constant mim3crv =
        0x30dF229cefa463e991e29D42DB0bae2e122B2AC7;
    address public constant abracadabra =
        0x6d2070b13929Df15B13D96cFC509C574168988Cd;

    // Infinite loop protection during claimRewards
    uint256 public constant MAX_REWARDS = 10;

    function enter() external payable override {
        _approveIfNeeded(MIM, mim3crv);
        _approveIfNeeded(mim3crv, abracadabra);

        uint256 usdtAmount = IERC20(USDT).balanceOf(address(this));
        uint256 usdcAmount = IERC20(USDCe).balanceOf(address(this));
        uint256 mimAmount = IERC20(MIM).balanceOf(address(this));

        uint256 _2poolAmount;
        if (usdcAmount != 0 || usdtAmount != 0) {
            _approveIfNeeded(USDCe, _2pool);
            _approveIfNeeded(USDT, _2pool);
            _approveIfNeeded(_2pool, mim3crv);

            _2poolAmount = ICurvePool(_2pool).add_liquidity(
                [usdcAmount, usdtAmount],
                0
            );
        }
        uint256 mim3crvAmount = ICurvePool(mim3crv).add_liquidity(
            [mimAmount, _2poolAmount],
            0
        );

        IMultiRewardsStaking(abracadabra).stake(mim3crvAmount);
    }

    function exit(uint256 liquidity) external payable override {
        IMultiRewardsStaking(abracadabra).withdraw(liquidity);

        uint256 mim3crvAmount = IERC20(mim3crv).balanceOf(address(this));

        uint256[2] memory amounts = ICurvePool(mim3crv).remove_liquidity(
            mim3crvAmount,
            [uint256(0), 0]
        );

        ICurvePool(_2pool).remove_liquidity(amounts[1], [uint256(0), 0]);
    }

    function claimRewards(address recipient) external payable override {
        IMultiRewardsStaking(abracadabra).getRewards();
        for (uint i = 0; i < MAX_REWARDS; i++) {
            try IMultiRewardsStaking(abracadabra).rewardTokens(i) returns (
                address token
            ) {
                _transferAll(token, recipient);
            } catch {
                break;
            }
        }
    ...
    function exitBuildingBlock(
        ...
        if (buildingBlockId == 0) {
            _exitAbracadabraBuildingBlock();
        } else if (buildingBlockId == 1) {
            _exitCurveArbitrumBuildingBlock();
        } else if (buildingBlockId == 2) {
            _exitMimBuildingBlock();
        } else {
    ...
    function accountLiquidity(
        ...
        return IMultiRewardsStaking(abracadabra).balanceOf(account);
    }

    function _exitAbracadabraBuildingBlock() internal {
        uint256 totalLiquidity = accountLiquidity(address(this));
        IMultiRewardsStaking(abracadabra).withdraw(totalLiquidity);
    }

    function _exitCurveArbitrumBuildingBlock() internal {
        _exitAbracadabraBuildingBlock();
        uint256 mim3CrvAmount = IERC20(mim3crv).balanceOf(address(this));
        uint256[2] memory amounts = ICurvePool(mim3crv).remove_liquidity(
            IERC20(mim3crv).balanceOf(address(this)),
            [uint256(0), 0]
        );
        ICurvePool(_2pool).remove_liquidity(amounts[1], [uint256(0), 0]);
    }

    function _exitMimBuildingBlock() internal {
        _exitAbracadabraBuildingBlock();
        uint256 mim3CrvAmount = IERC20(mim3crv).balanceOf(address(this));
        uint256 amount = ICurvePool(mim3crv).remove_liquidity_one_coin(
            mim3CrvAmount,
            1,
            0
        );
