...
interface IRewardDistributor {
    ...
    function whitelistRewarder(address _rewarder) external;
}
...
contract BaseRewardPool is Ownable, IBaseRewardPool {
    ...
    address public immutable operator; // master Campie
    ...
    event xGrailAndRewardDistributorUpdated(address indexed _xGrail, address indexed _rewardDistributor);
    ...
    modifier onlyMasterCampie() {
        if (msg.sender != operator) revert OnlyMasterCampie();
        _;
    }
    ...
    /// @notice Returns amount of staked tokens in master Campie by account
    ...
    function getReward(
        address _account,
        address _receiver
    ) public onlyMasterCampie updateReward(_account) returns (bool) {
        uint256 length = rewardTokens.length;
    ...
    function getRewards(
        ...
        address _receiver,
        address[] memory _rewardTokens
    ) external override onlyMasterCampie updateRewards(_account, _rewardTokens) {
    ...
    function emergencyWithdraw(address _rewardToken, address _to) external onlyMasterCampie {
    ...
    function _sendReward(
        ...
        _rewardToken == xGrail ? IRewardDistributor(rewardDistributor).sendUserXGrailRewards(_receiver, _amount) : IERC20(_rewardToken).safeTransfer(_receiver, _amount);
