...
interface ILendingVaultTreasury {
  function transferAsset(
    uint256 amt,
    address token,
    address receiver
  ) external;
  function updateWNT(address wnt) external;
...
import { ILendingVaultTreasury } from "../interfaces/treasury/ILendingVaultTreasury.sol";
import { IWNT } from  "../interfaces/tokens/IWNT.sol";
import { Errors } from  "../utils/Errors.sol";

/**
  * @title LendingVaultTreasury
  * @author Steadefi
  * @notice Treasury contract that receives fees from across all lending vaults.
  * This contract is made to receive assets. All assets can then be sent to approved contracts such as team MultiSig or Dividend module.
*/
contract LendingVaultTreasury is ILendingVaultTreasury, AccessManaged {
  ...
  */
  ...
  /**
    * @notice Transfer all native asset to an approved receiver
    * @param receiver Address of receiver
  */
  function transferAllNativeAsset(address receiver) external restricted {
    if (!receivers[receiver]) revert Errors.ReceiverNotApproved();

    (bool success, ) = receiver.call{value: address(this).balance}("");

    // if native transfer unsuccessful, send wrapped native token
    if (!success) {
      WNT.deposit{value: address(this).balance}();
      IERC20(address(WNT)).safeTransfer(
        receiver,
  ...
  /**
    * @notice Update wrapped native token address
    * @dev Should be called by approved Owner (Timelock + MultiSig)
    * @param wnt Address of wrapped native token
  */
  function updateWNT(address wnt) external restricted {
    WNT = IWNT(wnt);
    emit WNTUpdated(wnt);
  }
