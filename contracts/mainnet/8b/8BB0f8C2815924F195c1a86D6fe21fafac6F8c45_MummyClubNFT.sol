...
// File contracts/mummy/MummyClubARB.sol
...
contract MummyClubNFT is ONFT721 {
    ...
    function getTokenPower(uint256 tokenId) external view returns (uint256) {
        ...
        uint256 modulus = tokenId / 100;
        uint256 stepPower = 9900;
        if (tokenId % 100 == 0) modulus = modulus - 1;
        uint256 prePower = 5000;
        for (uint256 i = 0; i < modulus; i++) {
            prePower = (prePower * stepPower) / 10000;
        }
        return prePower;
    }
