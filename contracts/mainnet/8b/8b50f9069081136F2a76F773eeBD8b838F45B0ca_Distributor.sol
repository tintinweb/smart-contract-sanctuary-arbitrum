...
contract Distributor is Ownable {
    ...
    constructor(address _treasury, address _aohm, uint32 _epochLength, uint32 _nextEpochTime) {
        require(_treasury != address(0));
        treasury = ITreasury(_treasury);
        require(_aohm != address(0));
        AOHM = IERC20(_aohm);
    ...
    function nextRewardAt(uint _rate) public view returns (uint) {
        return AOHM.totalSupply().mul(_rate).div(1000000);
    }
