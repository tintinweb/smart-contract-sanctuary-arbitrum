...
contract FCCommunity_NFT is ERC721{
    ...
    constructor(string memory _name, string memory _symbol, string memory _domain, address addOwner) ERC721(_name, _symbol){
        domain = _domain;
        owner = addOwner;
    }
    ...
    function changeOwner(address newWallet) public checkOwner{
        require(newWallet!=address(0), "Wrong wallet address");
        owner = newWallet;
    }

    function mint_FCC_NFT_Item(address wallerReceiver) public checkOwner{
        require(wallerReceiver!=address(0), "Wrong wallet receiver.");
        _tokenIds.increment();
