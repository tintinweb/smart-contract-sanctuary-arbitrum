...
import {MetaProxy} from "contracts/utils/MetaProxy.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {IXGrailToken} from "contracts/interfaces/SynthChefs/Camelot/IXGrailToken.sol";

contract CamelotVEManager is MetaProxy {
    address public immutable grail;
    address public immutable xGrail;

    constructor(address _grail, address _xGrail) MetaProxy() {
        grail = _grail;
        xGrail = _xGrail;
    }

    function convert() external {
        uint256 amount = IERC20(grail).balanceOf(address(this));
        IERC20(grail).approve(xGrail, amount);
        IXGrailToken(xGrail).convertTo(amount, address(this));
    }

    function withdraw(uint256 _amount, address _to) external onlyAllowedCaller {
        IERC20(grail).transfer(_to, _amount);
    }
}
...
import {IMetaProxy} from "contracts/interfaces/IMetaProxy.sol";

contract MetaProxy is IMetaProxy, UUPSUpgradeable {
    address public admin;
    mapping(address => bool) public isCallerAllowed;

    event NewAdmin(address indexed);
    event CallerPermissionChanged(address indexed, bool);
    ...
    function initialize(address _admin, address[] calldata _callers) external initializer {
        __UUPSUpgradeable_init();
        admin = _admin;
        emit NewAdmin(_admin);

        for (uint256 i = 0; i < _callers.length; ++i) {
            isCallerAllowed[_callers[i]] = true;
            emit CallerPermissionChanged(_callers[i], true);
        }
    }
    ...
    function setCaller(address _caller, bool _allowed) external onlyAdmin {
        isCallerAllowed[_caller] = _allowed;
        emit CallerPermissionChanged(_caller, _allowed);
    }

    function call(address callee, bytes calldata callData)
        external
        onlyAllowedCaller
...
interface IMetaProxy {
    function call(address _callee, bytes memory _payload) external returns (bytes memory);
}
