...
contract MultiRewardsTimeLockNonTransferablePoolV3 is MultiRewardsBasePoolV3, ITimeLockNonTransferablePool {
    ...
    function batchMigrationDepositWithShares(
        uint256[] memory _amounts,
        ...
        require(
            _amounts.length == _starts.length &&
                ...
                _amounts.length == _shares.length &&
                _amounts.length == _receivers.length,
            "MultiRewardsTimeLockNonTransferablePoolV3.batchMigrationDepositWithShares: input arrays length mismatch"
        );

        for (uint256 i = 0; i < _receivers.length; i++) {
            _migrationDepositWithShares(_amounts[i], _starts[i], _ends[i], _shares[i], _receivers[i]);
        }
    }

    function _migrationDepositWithShares(
        uint256 _amount,
        uint64 _start,
        uint64 _end,
        ...
        require(
            migrationIsOn,
            "MultiRewardsTimeLockNonTransferablePoolV3._migrationDepositWithShares: only for migration"
        );
        require(
            _receiver != address(0),
            "MultiRewardsTimeLockNonTransferablePoolV3._migrationDepositWithShares: receiver cannot be zero address"
        );
        require(_amount > 0, "MultiRewardsTimeLockNonTransferablePoolV3._migrationDepositWithShares: cannot deposit 0");
        require(
            _end > _start,
            "MultiRewardsTimeLockNonTransferablePoolV3._migrationDepositWithShares: invalid duration"
        );

        depositToken.safeTransferFrom(_msgSender(), address(this), _amount);

        depositsOf[_receiver].push(Deposit({ amount: _amount, start: _start, end: _end, shareAmount: _shareAmount }));

        _mint(_receiver, _shareAmount);
        emit Deposited(_amount, _end - _start, _receiver, _msgSender());
    }
