 ...
 */
contract BufferRouterAB is AccessControl, IBufferRouterAB {
    using SafeERC20 for ERC20;

    uint16 public constant MAX_WAIT_TIME = 1 minutes;
    ...
    function initiateTrade(
        TradeInitiationParamas memory params
        ...
        IBufferBinaryOptionsAB optionsContract = IBufferBinaryOptionsAB(
            params.targetContract
        );

        optionsContract.runInitialChecks(params.strike, params.expiration);
        ERC20 tokenX = ERC20(optionsContract.tokenX());
        tokenX.safeTransferFrom(
            msg.sender,
            address(this),
            params.totalFee +
                IOptionsConfigAB(optionsContract.config()).platformFee()
        );
        lastSavedQueueId += 1;
        uint32 queueTimestamp = uint32(block.timestamp);
    ...
    function initiateAndResolve(
        TradeInitiationParamsV2[] memory trades
    ) external payable {
        uint256[] memory queueIds = new uint256[](trades.length);
        ResolveTradeParams[]
            memory resolveTradeParams = new ResolveTradeParams[](trades.length);

        for (uint32 i; i < uint32(trades.length); i++) {
            TradeInitiationParamsV2 memory allTradeInitiationParamas = trades[
                i
            ];

            lastSavedQueueId = allTradeInitiationParamas.queueId;

            TradeInitiationParamas
                memory tradeInitiationParamas = allTradeInitiationParamas
                    .tradeInitiationParamas;
            IBufferBinaryOptionsAB optionsContract = IBufferBinaryOptionsAB(
                tradeInitiationParamas.targetContract
            );
            IOptionsConfigAB config = IOptionsConfigAB(
                optionsContract.config()
            );

            try
                optionsContract.runInitialChecks(
                    tradeInitiationParamas.strike,
                    tradeInitiationParamas.expiration
                )
            {} catch Error(string memory reason) {
                emit FailResolve(allTradeInitiationParamas.queueId, reason);
                continue;
            }

            uint256 amountToPay = tradeInitiationParamas.totalFee +
                config.platformFee();
            ERC20 tokenX = ERC20(optionsContract.tokenX());

            (bool success, string memory errResaon) = _approveAndRegister(
                tradeInitiationParamas,
                allTradeInitiationParamas,
                amountToPay,
                tokenX
            );
            if (!success) {
                emit FailResolve(allTradeInitiationParamas.queueId, errResaon);
                continue;
            }
            (address signer, ) = getAccountMapping(
                allTradeInitiationParamas.user
            );

            (bool isValid, string memory errorResaon) = _verifyTrade(
                allTradeInitiationParamas.queueId,
                tradeInitiationParamas,
                allTradeInitiationParamas.user,
                signer,
                allTradeInitiationParamas.userSignInfo
            );
            ...
            _addQueueId(
                tradeInitiationParamas,
                allTradeInitiationParamas.queueTimestamp,
                allTradeInitiationParamas.queueId,
                optionsContract,
                allTradeInitiationParamas.user
            );
            prevSignature[
                allTradeInitiationParamas.userSignInfo.signature
            ] = true;
            queueIds[i] = allTradeInitiationParamas.queueId;
            resolveTradeParams[i] = allTradeInitiationParamas
                .resolveTradeParams;
        }

        resolveQueuedTrades(queueIds, resolveTradeParams);
    }
    ...
    function cancelQueuedTrade(uint256 queueId) external {
        ...
        _cancelQueuedTrade(queueId, "User Cancelled", admin);
    }
    ...
    function verifySignatures(
        ResolveTradeParams memory params,
        string memory assetPair
    ) public returns (bool, string memory) {
        SignInfo memory publisherSignInfo = params.publisherSignInfo;
        SignInfo memory sfSignInfo = params.sfSignInfo;

        bool isIvVerifed = Validator.verifyIv(
            assetPair,
        ...
        bool isSfVerified = Validator.verifySettlementFee(
            assetPair,
            params.sf,
            sfSignInfo.timestamp,
            sfSignInfo.signature,
            publisher
        );

        // Silently fail if the signature doesn't match
        if (!isIvVerifed || publisherSignInfo.timestamp < block.timestamp) {
            return (false, "Router: IV isn't valid");
        }
        if (!isSfVerified || sfSignInfo.timestamp < block.timestamp) {
            return (false, "Router: SF isn't valid");
        }
     ...
     */
    function resolveQueuedTrades(
        uint256[] memory queueIds,
        ResolveTradeParams[] memory resolveTradeParams
    ) public payable {
        _validateKeeper();
        for (uint32 i; i < uint32(queueIds.length); i++) {
            ResolveTradeParams memory params = resolveTradeParams[i];
            QueuedTrade storage queuedTrade = queuedTrades[queueIds[i]];
            IBufferBinaryOptionsAB optionsContract = IBufferBinaryOptionsAB(
            ...
            if (address(optionsContract) == address(0)) {
                continue;
            }
            (bool isValid, string memory errorResaon) = verifySignatures(
                params,
                optionsContract.assetPair()
            );
            if (!isValid) {
                _cancelQueuedTrade(queueIds[i], errorResaon, queuedTrade.user);
                continue;
            }

            if (block.timestamp - queuedTrade.queueTimestamp > MAX_WAIT_TIME) {
                _cancelQueuedTrade(
                    queueIds[i],
                    "Wait time too high",
                    queuedTrade.user
            ...
            (uint256 optionId, bool opened) = _evaluateOptionParams(
                queuedTrade,
                queueIds[i],
                params,
                tokenX,
            ...
            queuedTrade.optionId = optionId;
            optionIdMapping[queuedTrade.targetContract][optionId] = queueIds[i];
            tokenX.safeTransfer(
                admin,
                IOptionsConfigAB(optionsContract.config()).platformFee()
            );
    ...
    function executeOptions(
        ...
        for (uint32 i; i < arrayLength; i++) {
            ...
            (, , , , , uint256 expiration, , , bool isAbove) = optionsContract
            ...
            try
                optionsContract.unlock(params.optionId, priceAtExpiry)
            {} catch Error(string memory reason) {
    ...
    function _addQueueId(
        TradeInitiationParamas memory params,
        uint32 queueTimestamp,
        uint256 queueId,
        IBufferBinaryOptionsAB optionsContract,
        address user
    ) internal {
        uint256 contracts = (params.totalFee *
            (10 ** ERC20(optionsContract.tokenX()).decimals())) /
        ...
        queuedTrades[queueId] = QueuedTrade({
            ...
            maxFeePerContract: params.maxFeePerContract,
    ...
    function _verifyTrade(
        ...
        if (block.timestamp - userSignInfo.timestamp > MAX_WAIT_TIME) {
    ...
    function _approveAndRegister(
        TradeInitiationParamas memory tradeInitiationParamas,
        TradeInitiationParamsV2 memory params,
        uint256 amountToPay,
        ERC20 tokenX
    ) internal returns (bool, string memory) {
        Permit memory permit = params.permit;

        if (tokenX.balanceOf(params.user) < amountToPay) {
            return (false, "Router: Insufficient balance");
        }
        if (
            (tokenX.allowance(params.user, address(this)) < amountToPay) &&
            (!permit.shouldApprove)
        ) {
            return (false, "Router: Incorrect allowance");
            ...
            if (!success) return (false, "Router: Approval failed");
        }
        if (params.register.shouldRegister) {
            try
                accountRegistrar.registerAccount(
                    params.register.oneCT,
                    params.user,
                    params.register.signature
                )
            {} catch Error(string memory reason) {
                return (false, "Router: Registration failed");
            }
        }
        return (true, "");
    }

    function _evaluateOptionParams(
        ...
        IBufferBinaryOptionsAB.OptionParams
            memory optionParams = IBufferBinaryOptionsAB.OptionParams(
                queuedTrade.user,
                params.sf,
                params.iv,
                queuedTrade.allowPartialFill,
                queuedTrade.isAbove,
                queuedTrade.contracts,
                queuedTrade.strike,
                queuedTrade.expiration,
                0,
                queuedTrade.totalFee,
                queuedTrade.maxFeePerContract,
                getPrice(
                    queuedTrade.queueTimestamp,
                    params.priceUpdateData,
                    params.priceIds
                ),
                queuedTrade.referralCode
            );
        try optionsContract.evaluateParams(optionParams) returns (
            IBufferBinaryOptionsAB.RevisedData memory revisedData
        ) {
            optionParams.amount = revisedData.amount;
            optionParams.totalFee = revisedData.fee;
            optionParams.contracts = revisedData.revisedContracts;
            optionParams.sf = revisedData.revisedSf;

            if (revisedData.revisedContracts == 0) {
                _cancelQueuedTrade(queueId, "Insufficient liquidity", admin);
            ...
            if (optionParams.totalFee < queuedTrade.totalFee) {
                tokenX.safeTransfer(
                    queuedTrade.user,
                    queuedTrade.totalFee - optionParams.totalFee
                );
            }

            optionId = optionsContract.createFromRouter(
                optionParams,
                queuedTrade.queueTimestamp
            );
        } catch Error(string memory reason) {
            if (
                ...
                keccak256(abi.encodePacked("O29"))
            ) {
                _cancelQueuedTrade(queueId, reason, admin);
            } else {
                _cancelQueuedTrade(queueId, reason, queuedTrade.user);
            }
            return (0, false);
        }
    ...
    function _cancelQueuedTrade(
        uint256 queueId,
        string memory errorResaon,
        ...
        tokenX.safeTransfer(queuedTrade.user, queuedTrade.totalFee);
        emit CancelTrade(
            queuedTrade.user,
            queueId,
            errorResaon,
