...
abstract contract ModuleAuthUpgradableDuo is ModuleAuthUpgradable {
  ...
  */
  function updateExternalImageHash(bytes32 _imageHash) external onlySelf {
    ...
    require(_imageHash != bytes32(0), "ModuleAuthUpgradableDuo#updateExternalImageHash INVALID_IMAGE_HASH");
   ...
   *
   * @param _hash Digest of the signed data.
  ...
  function _signatureValidationExternal(
    ...
    // Copied from ModuleAuth.sol
    // Sequence v1 design doesn't allow for easy reuse of the original method
    (
      uint16 threshold,  // required threshold signature
      uint256 rindex     // read index
    ) = _signature.readFirstUint16();
    ...
    while (rindex < _signature.length) {
      ...
      if (flag == FLAG_ADDRESS) {
        ...
        bytes memory signature;
        (signature, rindex) = _signature.readBytes(rindex, size);
        require(isValidSignature(_hash, addr, signature), "ModuleAuthUpgradableDuo#_signatureValidation: INVALID_SIGNATURE");

        // Acumulate total weight of the signature
        totalWeight += addrWeight;
      } else {
        revert("ModuleAuthUpgradableDuo#_signatureValidation INVALID_FLAG");
      }
    ...
    return totalWeight >= threshold && _isValidExternalImage(imageHash);
  }
  ...
  function isValidSignature(
    ...
    // Validate signatures
    bool isValid = _signatureValidationExternal(_subDigest(keccak256(_data)), _signatures);
    if (isValid) {
      return SELECTOR_ERC1271_BYTES_BYTES;
    }
  ...
  function isValidSignature(
    ...
    // Validate signatures
    bool isValid = _signatureValidationExternal(_subDigest(_hash), _signatures);
    if (isValid) {
      return SELECTOR_ERC1271_BYTES32_BYTES;
    }
