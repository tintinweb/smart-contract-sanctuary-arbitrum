...
import "../oracle/interfaces/IPalmChainlinkClient.sol";
...
contract PositionRouter is BasePositionManager, IPositionRouter {
    ...
    IPalmChainlinkClient public chainlinkClient;
    ...
    function setWhitelistMode(
        bool _isWhitelistState
    ) external onlyAdmin {
        isWhitelistEnabled = _isWhitelistState;
    }

    function setWhitelistedTraders(
        address[] calldata user,
        bool[] calldata status
    ) external onlyAdmin {
        uint256 i;
        for (; i < user.length; ++ i) {
            whitelistedTraders[user[i]] = status[i];
        }
    }
    ...
    function setPalmChainlinkClient(address _client) external onlyOwner {
        chainlinkClient = IPalmChainlinkClient(_client);
    }
    ...
    ) external payable nonReentrant returns (bytes32 a) {
        require(_executionFee >= minExecutionFee, "fee");
        require(msg.value == _executionFee, "val");

        _validateWhitelister(msg.sender);
    ...
    function _validateWhitelister(
        address _trader
    ) private view {
        if (isWhitelistEnabled) {
            require(whitelistedTraders[_trader], "PositionRouter: not whitelisted");
        }
    }
    ...
    function _createIncreasePosition(
        ...
        if (address(chainlinkClient) != address(0)) {
            chainlinkClient.requestPositionExecution(
                _indexToken,
                requestKey,
                true
            );
    ...
    function _callRequestCallback(
        ...
        try
            IPositionRouterCallbackReceiver(_callbackTarget)
                .palmPositionCallback{gas: _gasLimit}(
                _key,
                _wasExecuted,
                _isIncrease
            )
