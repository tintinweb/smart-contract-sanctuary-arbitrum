...
interface IStake {
  ...
  function safeSpaceTransfer(address _account, uint256 _amount) external;
}
...
contract Stake is IStake, Ownable {
  ...
  /// @notice Safe SPACE transfer function, just in case if rounding error causes pool to not have enough SPACEs.
  /// @param _to The address to transfer SPACE to
  /// @param _amount The amount to transfer to
  function safeSpaceTransfer(address _to, uint256 _amount) external override onlyOwner {
    uint256 spaceBal = space.balanceOf(address(this));
    if (_amount > spaceBal) {
      space.transfer(_to, spaceBal);
    } else {
