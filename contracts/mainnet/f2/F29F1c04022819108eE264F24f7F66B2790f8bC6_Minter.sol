...
contract Minter is IMinter, Initializable {
    ...
    /** New variables */
    uint256 public incentivesSize;
    uint256 emission;
    uint256 internal maxGrowth;
    ...
    function initialize(
        ...
        weekly = 3086450704711440926680528; // copy current minter var

        msig = _msig;
        active_period = type(uint256).max / 2;
    }

    function initializeIncentivesSize() external reinitializer(2) {
        incentivesSize = 2_000_000 * 1e18;
        commandCenter = 0xAA2Fc285cf2b7be92F5cC7f049d233CdE161fa0D;
    }

    /// @notice returns emissions for epoch n+1
    function weekly_emission() public view returns (uint256) {
        return (weekly * emission) / PRECISION;
    }

    /// @notice calculate rebase
    function calculate_growth(uint256 _minted) public view returns (uint256) {
        return (maxGrowth * _minted) / PRECISION;
    }

    /// @notice deprecated after epoch 0 initialization
    ...
    function update_period() external returns (uint256) {
        ...
        if (block.timestamp > _period + WEEK) {
            ...
            if (_balanceOf < _required) {
                ...
                _ram.mint(commandCenter, incentivesSize);
            }
    ...
    function updateCommandCenter(address _newCommandCenter) external {
        require(msg.sender == RAMSES_TIMELOCK, "!TL");
        commandCenter = _newCommandCenter;
    }

    function updateIncentivesSize(uint256 _newSize) external {
        require(_newSize < weekly_emission(), "Too large");
        incentivesSize = _newSize;
    }

    function updateRate(uint256 _newRate) external {
        require(msg.sender == commandCenter, "!commandCenter");
        require(_newRate > 10 && _newRate <= 2000, "Emissions growth !valid");
        emission = _newRate;
    }

    function updateGrowth(uint256 _newGrowth) external {
        require(msg.sender == commandCenter, "!commandCenter");
        require(_newGrowth <= 1000, "Growth can't be greater than 100%");
        maxGrowth = _newGrowth;
    }
}