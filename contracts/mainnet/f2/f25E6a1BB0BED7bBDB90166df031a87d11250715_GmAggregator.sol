...
contract GmAggregator is Ownable {
    ...
    function _getMarketTokenPrice(GmxV2Market memory market) internal virtual view returns(int256 price, GmxV2MarketPoolValueInfo memory poolValueInfo) {
        return _getMarketTokenPrice(market, 1);
    }

    function _getMarketTokenPrice(GmxV2Market memory market, uint256 lgPrice) internal virtual view returns(int256 price, GmxV2MarketPoolValueInfo memory poolValueInfo) {
        try reader.getMarketTokenPrice(GMXV2_DATASTORE, market, GmxV2Price(lgPrice, lgPrice), GmxV2Price(lgPrice,lgPrice), GmxV2Price(1,1), keccak256(abi.encode("MAX_PNL_FACTOR_FOR_WITHDRAWALS")), true) returns (int256 pv, GmxV2MarketPoolValueInfo memory pvi) {
            return (pv, pvi);
        } catch {
    ...
    function getBalances(address user) public virtual view returns(Market[] memory markets) {
        return getBalances(user, 1);
    }

    function getBalances(address user, uint256 lgPrice) public virtual view returns(Market[] memory markets) {
        ...
        for (uint256 i=0;i<gms.length;i++) {
            ...
            (int256 price, GmxV2MarketPoolValueInfo memory poolValueInfo) = _getMarketTokenPrice(gms[i], lgPrice);
            (uint256 longBal, uint256 shortBal) = _calculateMarket(gms[i].marketToken, user, poolValueInfo);
            
            markets[i] = Market(gms[i].marketToken, string.concat(token.name()," ",longToken.name(),"/",shortToken.name()), token.symbol(), token.decimals(), token.balanceOf(user), token.totalSupply(), price, poolValueInfo.poolValue, poolValueInfo.netPnl, poolValueInfo.totalBorrowingFees, poolValueInfo.borrowingFeePoolFactor, poolValueInfo.impactPoolAmount, Balance(gms[i].longToken, longToken.name(), longToken.symbol(), longToken.decimals(), longBal, poolValueInfo.longTokenAmount, longToken.totalSupply()), Balance(gms[i].shortToken, shortToken.name(), shortToken.symbol(), shortToken.decimals(), shortBal, poolValueInfo.shortTokenAmount, shortToken.totalSupply()));
        }
        return markets;
    }
}
