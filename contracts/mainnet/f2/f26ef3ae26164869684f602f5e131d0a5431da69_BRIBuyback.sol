...
contract BRIBuyback is Ownable {
    using SafeERC20 for IERC20Metadata;

    uint256 constant PRICE_DIVIDER = 1 gwei;

    address public accountant;
    address public treasury;
    IERC20Metadata immutable bri;
    BRIX immutable brix;
    IERC20Metadata immutable usd;
    uint256 public price;

    event Sold(address seller, uint256 amount, uint256 price);
    event NewPrice(uint256 rate);
    event NewAccountant(address accountant);
    event NewTreasury(address treasury);

    modifier onlyAccountant {
        if (msg.sender != accountant) revert Restricted();
        _;
    }

    constructor(address owner_, address accountant_, address treasury_, IERC20Metadata bri_, BRIX brix_, IERC20Metadata usd_, uint256 price_) Ownable(owner_) {
        bri = bri_;
        brix = brix_;
        usd = usd_;
        price = price_;
    ...
    function sell(uint256 amount_) external {
        bri.safeTransferFrom(msg.sender, address(this), amount_);
        bri.safeTransfer(treasury, amount_);
        IERC20Metadata(brix).safeTransferFrom(msg.sender, address(this), amount_);
        
        uint256 fullPrice = amount_ * price / PRICE_DIVIDER; 

        brix.burnReward(amount_);
        usd.safeTransfer(msg.sender, fullPrice);

        emit Sold(msg.sender, amount_, fullPrice);
    }

    function changeAccountant(address newAccountant_) external onlyOwner {
        accountant = newAccountant_;
        emit NewAccountant(accountant);
    }

    function changeTreasury(address newTreasury_) external onlyOwner {
        treasury = newTreasury_;
        emit NewTreasury(treasury);
    }

    function payout(address rcpt_) external onlyAccountant {
        usd.safeTransfer(rcpt_, usd.balanceOf(address(this)));
    }

    function setPrice(uint256 price_) external onlyAccountant {
        price = price_;
...
contract BRIX is ERC20Permit, LedgerOwner {
    ...
    constructor(address owner_, address admin_, address warden_)
        ERC20("BrightPool Exchange Token", "BP0")
        ERC20Permit("BrightPool Token")
