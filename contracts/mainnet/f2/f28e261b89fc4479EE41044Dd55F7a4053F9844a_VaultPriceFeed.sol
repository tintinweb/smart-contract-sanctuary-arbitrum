...
import "@handle-fi/handle-oracles/contracts/H2SOFA.sol";
 ...
 * @dev This contract is the same as Chainlink's, with the exception of the
 * modifications listed below:
 * - A new function, submitInternal, was added which is equal to submit except for:
 *    - A new argument, bool _isOracle, was added
 *    - The function validateOracleRound is only called and checked if _isOracle == true
 *    - The function payOracle is only called if _isOracle == true
 * - The function submit calls the new submitInternal function, with _isOracle == true
 */
contract FluxAggregator is AggregatorV2V3Interface, Owned {
    ...
    function submit(uint256 _roundId, int256 _submission) external {
        submitInternal(_roundId, _submission, true);
    }

    /**
     * @notice may be called by oracles or an H2SO contract for updating quotes
     * @dev if _isOracle is false, oracle checks are bypassed and no oracle is paid
     * @param _roundId is the ID of the round this submission pertains to
     * @param _submission is the updated data that the oracle is submitting
     */
    function submitInternal(
        ...
        if (_isOracle) {
            bytes memory error = validateOracleRound(
                msg.sender,
                uint32(_roundId)
            );
