...
// 3,333 Omni Penguins NFTs on 7 blockchains

// https://twitter.com/0mnipenguins
...
contract OmniPenguins is Ownable, ERC721Burnable, ERC721Enumerable, LayerZeroable {
    using Strings for uint256;
    string public baseExtension = ".json";
    string private baseURI;

    uint256 public mintPrice = 0.01 ether;
    uint256 public lastTokenId = 1050;
    ...
    constructor(string memory baseURI_, address _layerZeroEndpoint)
        ERC721("Omni Penguins", "OMNIPENGUINS")
    {
        baseURI = baseURI_;
        layerZeroEndpoint = ILayerZeroEndpoint(_layerZeroEndpoint);
    }
    ...
    function mint(uint8 numTokens) public payable mintActive {
        require(mintedSupply < MAX_SUPPLY, "Max mint supply reached");
        require(mintPrice * numTokens <= msg.value, "Incorrect value sent"); 
        uint256 numAlreadyMinted = publicListMintCounts[msg.sender];
        require(numAlreadyMinted + numTokens <= MAX_MINT_PER_WALLET, "5 NFTs max per wallet");
    ...
    function transferToChain(
        ...
        (uint messageFee, ) = layerZeroEndpoint.estimateFees(_chainId, address(this), payload, false, adapterParams);
