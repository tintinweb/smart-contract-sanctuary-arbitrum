...
contract AirPuffLendingUSDT is ERC4626Upgradeable, OwnableUpgradeable {
    ...
    modifier onlyUSDTGifter() {
        require(allowedToGift[msg.sender], 'Not allowed to increment USDT');
        _;
    }
    ...
    event USDTGifterAllowed(address indexed gifter, bool status);
    ...
    function initialize(address _USDT) external initializer {
        ...
        __Ownable_init();
        __ERC4626_init(IERC20Upgradeable(_USDT));
        __ERC20_init("USDT-AirPuff", "USDT-AirPuff");
    }
    ...
    function getUtilizationRate() public view returns (uint256) {
        ...
        return totalAirPuffDebt == 0 ? 0 : totalAirPuffDebt.mulDiv(1e18, totalUSDT + totalAirPuffDebt);
    }
    ...
    function allowUSDTGifter(address _gifter,bool _status) external onlyOwner zeroAddress(_gifter) {
        allowedToGift[_gifter] = _status;
        emit USDTGifterAllowed(_gifter, _status);
    }
    ...
    /**
    * @notice Allow the Lending Vault to lend a certain amount of USDT to the protocol.
    * @dev The function allows the Lending Vault to lend a certain amount of USDT to the protocol. It updates the total debt and total USDT balances accordingly.
    * @param _borrowed The amount of USDT to lend.
    ...
    */
    function lend(uint256 _borrowed,address _receiver) external onlyLendingVault returns (bool status) {
        require(totalUSDT > _borrowed, "Not enough USDT to lend");
        ...
        IERC20(USDT).safeTransfer(_receiver, _borrowed);
     ...
     * @dev The function allows the Lending Vault to repay a certain amount of debt to the protocol. It updates the total debt and total USDT balances accordingly.
     * @param _debtAmount The amount of debt to repay.
     * @param _amountPaid The amount of USDT paid to repay the debt.
    ...
    function repayDebt(uint256 _debtAmount, uint256 _amountPaid) external onlyLendingVault returns (bool) {
        IERC20(USDT).safeTransferFrom(msg.sender, address(this), _amountPaid);
        totalDebt = totalDebt - _debtAmount;
        totalUSDT += _amountPaid;
     ...
     *      It updates the total USDT balance and sets a timelock for the receiver.
     ...
     *      The function then performs the actual withdrawal by calling the internal `_withdraw` function. It updates the total USDT balance after the withdrawal and returns the amount of shares returned to the user.
     * @param _assets The amount of assets (USDT) to withdraw.
     * @param _receiver The address of the receiver who will receive the corresponding shares.
     * @param _owner The address of the owner who is making the withdrawal.
     * @return The amount of shares returned to the user.
     */
    function withdraw(
        uint256 _assets, // Native (USDT) token amount
        address _receiver,
        address _owner
        ...
        require(balanceOfUSDT() > _assets, "Insufficient balance in vault");

        uint256 UR = getUtilizationRate();
        uint256 shares = previewWithdraw(_assets);
        uint256 feeAmount = (_assets * withdrawalFees) / DENOMINATOR;
        IERC20(USDT).safeTransfer(feeReceiver, feeAmount);
    ...
    //function that only allows a whitelisted address to call to increase totalUSDT 
    function increaseTotalUSDT(uint256 _amount) external onlyUSDTGifter {
