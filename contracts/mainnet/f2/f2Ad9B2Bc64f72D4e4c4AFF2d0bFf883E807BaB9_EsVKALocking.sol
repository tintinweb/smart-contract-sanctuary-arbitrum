...
contract EsVKALocking is VotingEscrowTokenBase, IPVotingEscrowMainchain, IDualStaking, OwnableUpgradeable {
    ...
    function initialize(IERC20 _esVKA) public initializer {
        if (address(_esVKA) == address(0)) revert Errors.ZeroAddress();
        esVKA = _esVKA;
        __Ownable_init();
    }
    ...
    function increaseLockPosition(
        ...
        if (additionalAmountToLock > 0) {
            esVKA.safeTransferFrom(user, address(this), additionalAmountToLock);
        }
    ...
    function withdraw(address user) external onlyVePeriphery returns (uint128 amount) {
        ...
        esVKA.safeTransfer(user, amount);
...
import { IPVeToken } from "./IVotingEscrow/IPVeToken.sol";
...
//@todo function to edit for emission (1)
...
//@todo function to adit for emssion (2) 
