/**
 *Submitted for verification at Arbiscan on 2023-03-15
*/
...
// File: contracts/final/glpPrice.sol
...
contract GLPPrice  is Ownable{
     ...
     function updateManager(GLPmanager newManager) external onlyOwner{
        GLPm = newManager;
     }

    function updatePool(GLPpool _pool) external onlyOwner{
        pool = _pool;
     }
     ...
     function getPrice(address _token) public view returns (uint256){
         if (_token == 0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9) {
            return pool.getMaxPrice(_token).mul(1e12).mul(slippage.add(10000)).div(10000);
         }
         else {
            return pool.getMaxPrice(_token).mul(slippage.add(10000)).div(10000);
         }
         
     }
    function setSlippage(uint256 _slippage) external onlyOwner{

        require(_slippage >= 0 && _slippage <= 600, "not in range");
        slippage = _slippage;
    }
