    ...
    ReentrancyGuardUpgradeable
{
    ...
    /**
     * @param marketplace address of marketplace to swap tokens from
     * @param from token to buy property tokens from
     * @param to property tokens address
     * @param vaults what marketplace should use to buy or sell,
     * @param amounts amount to buy/sell with wrt vaults
    ...
    function getPropertyPriceWithFees(
        ...
        uint amountOfTokens = getArraySum(amounts);
        uint propertyTokensPriceInBaseCurrency = systemRegistry
            .rootPriceOracle()
            .getTokenPrice(to);
        address propertyBaseCurrency = IMarketplace(marketplace)
            .getPropertyDetails(to)
            .baseCurrency;
        uint propertyPriceInQuoteCurrency = IMarketplace(marketplace)
            .getPropertyPriceInQuoteCurrency(
                propertyBaseCurrency,
                from,
                propertyTokensPriceInBaseCurrency
            );
        uint propertyTokensToBringFromAdmin;
        if (amounts[0] > 0 && vaults[0] == address(0)) {
            propertyTokensToBringFromAdmin = amounts[0];
        }

        uint amountToCharge = (propertyPriceInQuoteCurrency * amountOfTokens) +
            IMarketplace(marketplace).getFeesToCharge(
                to,
                propertyPriceInQuoteCurrency * propertyTokensToBringFromAdmin,
                true
            );
