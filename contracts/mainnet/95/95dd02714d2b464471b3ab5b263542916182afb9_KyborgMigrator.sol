...
contract KyborgMigrator is ReentrancyGuard {
    address public immutable weth;
    IManagerMinimal public immutable kyborgManager;
    ...
    constructor(address kyborgManager_, address uniV3PositionManager_) {
        kyborgManager = IManagerMinimal(kyborgManager_);
        uniV3PositionManager = INonfungiblePositionManagerMinimal(uniV3PositionManager_);
        weth = kyborgManager.WETH9();
    }
    ...
    /// @notice Migrate Uniswap V3 position to Kyborg position
    ...
    /// @param mintParams needCreatePool indicate the need of creating new Kyborg's pool,
    /// the amount of both burnt tokens need to exceed certain amount for creation.
    /// needAddTier indicate the need of adding new fee tier to the Kyborg's pool,
    /// the amount of both burnt tokens need to exceed certain amount for addition.
    /// sqrtPrice the sqrt price value for creating new Kyborg's pool.
    /// sqrtGamma the sqrt gamma value for adding new fee tier.
    /// ...others are subset of paramenters for Kyborg's `Manager.mint`
    ...
    function migrateFromUniV3WithPermit(
        ...
        // allow kyborg manager to use the tokens
        _approveTokenToKyborgManager(token0, amount0);
        _approveTokenToKyborgManager(token1, amount1);

        // mint kyborg position
    ...
    function _approveTokenToKyborgManager(address token, uint256 amount) internal {
        uint256 allowance = ERC20(token).allowance(address(this), address(kyborgManager));
        if (allowance >= amount) return;

        // revoke allowance before setting a new one, revert if unable to reset
        if (allowance != 0) SafeTransferLib.safeApprove(ERC20(token), address(kyborgManager), 0);

        // first try allow max amount
        if (!_trySafeApprove(ERC20(token), address(kyborgManager), type(uint256).max)) {
            // if failed, allow only exact amount
            SafeTransferLib.safeApprove(ERC20(token), address(kyborgManager), amount);
        }
    }

    function _mintPosition(address token0, address token1, MintParams calldata mintParams) internal {
        if (mintParams.needCreatePool) {
            kyborgManager.createPool(token0, token1, mintParams.sqrtGamma, mintParams.sqrtPrice, false);
        } else if (mintParams.needAddTier) {
            kyborgManager.addTier(token0, token1, mintParams.sqrtGamma, false, mintParams.tierId);
        }
