...
import {PendleLogicEzEth} from "./PendleLogicEzEth.sol";
 ...
 */
contract PendleGeneralEzEth is BaseStrategyPendle {
    using SafeERC20 for IERC20;
    ...
    PendleLogicEzEth public pendleLogic;
    ...
    function setPendleLogic(address _newPendleLogic) external onlyAuthorized {
        pendleLogic = PendleLogicEzEth(_newPendleLogic);
    }
...
contract PendleLogicEzEth {
    IAggregatorV3 internal dataFeed;
...
import {IPenpieMaster} from "../../integrations/penpie/IPenpieMaster.sol";
import {ISimpleVault} from "../../interfaces/ISimpleVault.sol";
import {PendleLogicRsEth} from "./PendleLogicRsEth.sol";
 ...
 * It provides functionality for managing assets and executing trades on the Pendle protocol.
 */
contract PendleGeneralRsEth is BaseStrategyPendle {
    using SafeERC20 for IERC20;
    ...
    PendleGeneralInfo public mainInfo;
    PendleLogicRsEth public pendleLogic;
    ...
    function setPendleLogic(address _newPendleLogic) external onlyAuthorized {
        if(address(pendleLogic) != address(0)) {
            IERC20(WETH).forceApprove(address(pendleLogic), 0);
        }
        pendleLogic = PendleLogicRsEth(_newPendleLogic);
        IERC20(WETH).forceApprove(_newPendleLogic, type(uint256).max);
    }
...
import {OracleLibrary} from "@uniswap/v3-periphery/contracts/libraries/OracleLibrary.sol";
import {IPendleRouter} from "../../integrations/pendle/IPendleRouter.sol";
import {PendleGeneralInfo} from "./PendleGeneralRsEth.sol";
import {IPendleGeneral} from "../../interfaces/IPendleGeneral.sol";
import {IAggregatorV3} from "../../integrations/chainlink/IAggregatorV3.sol";
import {IPenpieMaster} from "../../integrations/penpie/IPenpieMaster.sol";
import {SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {ICamelotRouter} from "../../integrations/externalDexes/ICamelotRouter.sol";

/**
 * @title PendleLogic
 * @dev Contract containing logic for handling various operations related to Pendle strategies.
 */
contract PendleLogicRsEth {
    using SafeERC20 for IERC20;
    IAggregatorV3 internal dataFeed;
    PendleGeneralInfo public pendleGeneralInfo;
    address constant public WETH = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1;
    ...
    constructor(PendleGeneralInfo memory _pendleGeneralInfo, address _dataFeed){
        pendleGeneralInfo = _pendleGeneralInfo;
        dataFeed = IAggregatorV3(_dataFeed);
        IERC20(WETH).forceApprove(pendleGeneralInfo.extraDex, type(uint256).max);
    }
     ...
     */
    function assetToWantRate(uint256 assetAmount) public view  returns (uint256) {
        uint256 rate = getChainlinkDataFeedLatestAnswer();
        return rate * assetAmount / 1e18;
    }
     ...
     * @return uint256 The equivalent amount in the want token.
     */
    function wantToAssetRate(uint256 wantAmount) public view  returns (uint256) {
        uint256 rate = getChainlinkDataFeedLatestAnswer();
        return wantAmount * 1e18 / rate;
    }
    ...
    function addLiquidity(uint256 amount, OffChainData calldata _offChainData) external returns(uint256 lpExpected, uint256 ytExpected, IPendleRouter.TokenInput memory inputData) {
        ...
        IPendleRouter.SwapData memory swapData;
        uint256 amountOut = _swapWethToRsEthViaCamelot(amount, msg.sender);
        inputData = IPendleRouter.TokenInput(pendleGeneralInfo.underlyingAsset, amountOut, pendleGeneralInfo.underlyingAsset, address(0), swapData);
        lpExpected = _offChainData.WantToLpYtProportion[0] * amountOut / 1e18;
        ytExpected = _offChainData.WantToLpYtProportion[1] * amountOut / 1e18;
    }
    ...
    function _swapWethToRsEthViaCamelot(uint256 amount, address rec) internal returns(uint256 amountOut) {
        ICamelotRouter.ExactInputSingleParams memory params;
        params.tokenIn = address(WETH);
        params.tokenOut = address(pendleGeneralInfo.underlyingAsset);
        params.recipient = rec;
        params.deadline = block.timestamp;
        params.amountIn = amount;
        params.amountOutMinimum = wantToAssetRate(amount) * pendleGeneralInfo.slippage / 10000;
        params.limitSqrtPrice = 0;
        amountOut = ICamelotRouter(pendleGeneralInfo.extraDex).exactInputSingle(params);
    }
}
...
import {PendleGeneralRsEth, PendleGeneralInfo} from "./PendleGeneralRsEth.sol";
...
contract PendleRsEth is Initializable, PendleGeneralRsEth, UUPSUpgradeable, AccessControlUpgradeable {
    using SafeERC20 for IERC20;
    ...
    function initialize(
        ...
        IERC20(PENDLE).forceApprove(UNISWAP_V3_ROUTER, type(uint256).max);
        IERC20(ARB).forceApprove(UNISWAP_V3_ROUTER, type(uint256).max);
        IERC20(WETH).forceApprove(address(mainInfo.pendleRouter), type(uint256).max);
        IERC20(mainInfo.underlyingAsset).forceApprove(address(mainInfo.pendleRouter), type(uint256).max);
        IERC20(mainInfo.lpToken).forceApprove(address(mainInfo.pendleRouter), type(uint256).max);
        IERC20(mainInfo.lpToken).forceApprove(address(mainInfo.pendleStaking), type(uint256).max);
        IERC20(mainInfo.lpToken).forceApprove(address(mainInfo.penpieStaking), type(uint256).max);
        IERC20(mainInfo.syToken).forceApprove(address(mainInfo.pendleRouter), type(uint256).max);
        IERC20(mainInfo.ptToken).forceApprove(address(mainInfo.pendleRouter), type(uint256).max);
        IERC20(mainInfo.ytToken).forceApprove(address(mainInfo.pendleRouter), type(uint256).max);
    ...
    function _swapAssetsViaAnotherDex(address _tokenIn, address _tokenOut, uint24 _fee, uint256 amount) internal override {
        ...
        params.limitSqrtPrice = 0;
        ICamelotRouter(mainInfo.extraDex).exactInputSingle(params);
    }
}