...
contract UniSwapLPManager is IManager, IUniswapV3MintCallback, UpgradeableOperableKeepable, ERC20Upgradeable {
    ...
    mapping(uint256 => mapping(uint256 => bool)) private alreadyRebalance;
    ...
    function initializeLPManager(
        ...
        token0.approve(_receiver, type(uint256).max);
        token1.approve(_receiver, type(uint256).max);
        ...
        if (length > 0) {
            ...
            for (uint256 i = 0; i < length;) {
                ...
                if (liq > 0) {
                    ...
                    (uint256 rewards0, uint256 rewards1) = pool.collect(
        ...
        onlyKeeper
    {
        ...
        if (length > 0) {
            for (uint256 i; i < length;) {
                require(!alreadyRebalance[block.number][_existingRanges[i].index], "Already Rebalanced");
                ...
                alreadyRebalance[block.number][_existingRanges[i].index] = true;
        ...
        if (length > 0) {
            for (uint256 i; i < length;) {
                ...
                if (_newRange[i].amount0 > 0 && _newRange[i].amount1 > 0) {
                    // Mint liquidity
                    mintLiquidity(address(this), ranges.length.sub(1), _newRange[i].amount0, _newRange[i].amount1);
                }
