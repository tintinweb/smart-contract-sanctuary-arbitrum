/**
 *Submitted for verification at arbiscan.io on 2022-02-20
*/
...
// File contracts/BeefyVaultV6.sol
...
// File contracts/StratManager.sol
...
// File contracts/SingleStakingStrategy.sol
...
contract SingleStakingStrategy is StratManager {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    ...
    // Jones Single Staking Contract
    address public immutable singleStaking;
    ...
    constructor(
        ...
        address _unirouter,
        address _vault
    ) StratManager(_unirouter, _vault) {
        want = _want;
        singleStaking = _singleStaking;
    }

    function balanceOfPool() public view returns (uint256) {
        return IStakingRewards(singleStaking).balanceOf(address(this));
    }
    ...
    function _deposit() internal whenNotPaused {
        IStakingRewards(singleStaking).stake(balanceOfWant());
    }

    function withdraw(uint256 amount) external {
        require(msg.sender == vault, "You don't have the permissions");

        IStakingRewards(singleStaking).withdraw(amount);
        IERC20(want).safeTransfer(vault, amount);
    }

    function _harvest() internal whenNotPaused {
        IStakingRewards(singleStaking).getReward();
    }
    ...
    function retireStrat() external {
        ...
        IStakingRewards(singleStaking).withdraw(balanceOfPool());
