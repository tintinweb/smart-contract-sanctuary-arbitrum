...
contract ApeSwap is ERC20Burnable, Ownable {
  using SafeMath for uint256;
  ...
  ) public ERC20("ApeSwap", "APE") {
    require(_initialSupply <= _maxSupply, "APE: The _initialSupply should not exceed the _maxSupply");
  ...
  modifier onlyDev() {
    require(devAddress == _msgSender() || owner() == _msgSender(), "APE: You don't have the permission!");
    _;
  }

  modifier hasMinterRole() {
    require(isMinter[_msgSender()], "APE: You don't have the permission!");
    _;
  }
  ...
  function mint(address _user, uint256 _amount) external hasMinterRole {
    ...
    require(_totalSupply.add(_amount) <= MAX_SUPPLY, "APE: No more minting allowed!");
  ...
  function updateFee(uint256 feeType, uint256 fee) external onlyDev {
    require(fee <= 900, "APE: The tax Fee cannot exceed 9%");
  ...
  function recoverToken(address _token) external onlyDev {
    ...
    require(tokenBalance > 0, "APE: The contract doen't have tokens to be recovered!");
