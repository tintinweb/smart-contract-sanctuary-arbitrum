...
contract StgComposed {
    ...
    function sgReceive(
        ...
        try vault.mint(_token, amountLD, amountLD / 10, deadline) {
            // success
            usds.transfer(address(toAddr), usds.balanceOf(address(this)) - usdsBefore);
        } catch {
            // fail
            revert("StgComposed: mint failed");
        }

        emit USDSMinted(toAddr, amountLD);
    }
}
