/**
 *Submitted for verification at Arbiscan on 2022-11-02
*/
...
contract zkSyno is Context, IBEP20, Ownable {
    ...
    address public marketingWallet = 0x91205bDBC826CF03962468982D4A97d6A08851a7;
    address public PrizePoolWallet = 0x21e98423C1F4a8D7A5AEe72fE76eB3b69831470F;

    string private constant _name = "zkSyno Token";
    string private constant _symbol = "SYNO";

    struct Taxes {
        uint256 rfi;
        uint256 marketing;
        uint256 liquidity;
        uint256 PrizePoolWallet;
    }

    Taxes public taxes = Taxes(2, 2, 1, 2);
    Taxes public sellTaxes = Taxes(2, 2, 1, 2);
    ...
    struct valuesFromGetValues {
        ...
        uint256 rMarketing;
        uint256 rLiquidity;
        uint256 rPrizePoolWallet;
        uint256 tTransferAmount;
        uint256 tRfi;
        uint256 tMarketing;
        uint256 tLiquidity;
        uint256 tPrizePoolWallet;
    }
    ...
    constructor() {
        ...
        _isExcludedFromFee[PrizePoolWallet] = true;
    ...
    function setTaxes(
        ...
        uint256 _marketing,
        uint256 _liquidity,
        uint256 _PrizePoolWallet
    ) public onlyOwner {
        require((_rfi + _marketing + _liquidity + _PrizePoolWallet ) <= 12, "Must keep fees at 12% or less");
        taxes = Taxes(_rfi, _marketing, _liquidity, _PrizePoolWallet);
        emit FeesChanged();
    }
    ...
    function setSellTaxes(
        ...
        require((_rfi + _marketing + _liquidity + _PrizePoolWallet ) <= 12, "Must keep fees at 12% or less");
        sellTaxes = Taxes(_rfi, _marketing, _liquidity, _PrizePoolWallet);
        emit FeesChanged();
    }
    ...
    function _takePrizePoolWallet(uint256 rPrizePoolWallet, uint256 tPrizePoolWallet) private {
        totFeesPaid.PrizePoolWallet += tPrizePoolWallet;

        if (_isExcluded[address(this)]) {
            _tOwned[address(this)] += tPrizePoolWallet;
        }
        _rOwned[address(this)] += rPrizePoolWallet;
    }
    ...
    function _getValues(
        ...
        (to_return.rPrizePoolWallet) = _getRValues2(
            to_return,
            takeFee,
            _getRate()
        );
    ...
    function _getTValues(
        ...
        s.tPrizePoolWallet = (tAmount * temp.PrizePoolWallet) / 100;
        s.tTransferAmount =
            tAmount -
            s.tRfi -
            s.tMarketing -
        ...
        )
    {
        ...
        uint256 rPrizePoolWallet = s.tPrizePoolWallet * currentRate;
        rTransferAmount =
            rAmount -
            rRfi -
            rMarketing -
    ...
    function _tokenTransfer(
        ...
        if (s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(
                sender,
                address(this),
                s.tLiquidity + s.tMarketing + s.tPrizePoolWallet
            );
        }
        if (s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if (s.rPrizePoolWallet > 0 || s.tPrizePoolWallet > 0) _takePrizePoolWallet(s.rPrizePoolWallet, s.tPrizePoolWallet);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap {
        ...
        uint256 PrizePoolWalletAmt = unitBalance * 2 * temp.PrizePoolWallet;
        if (PrizePoolWalletAmt > 0) {
            payable(PrizePoolWallet).sendValue(PrizePoolWalletAmt);
        }
    }
