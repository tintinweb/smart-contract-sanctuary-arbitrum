...
import "../rewardPool/AINodeReward.sol";

contract AINodeRewardGray is AINodeReward {
    constructor(
        string memory name,
        address validator
    ) AINodeReward(name, validator) {}
    function gray() public pure returns (bool) {
        return true;
    }
}
...
contract AINodeReward is Ownable, EIP712, ReentrancyGuard {
    ...
    bytes32 private constant CLAIM_REWARD_TYPEHASH =
        keccak256(
            "claimReward(uint256 amount,uint256 totalAmount,address to,uint256 expireTimestamp)"
        );
    ...
    function claimReward(
        ...
        require(block.timestamp < expireTimestamp, "Claim expired!");
        require(
            0 < amount && amount + _claimedBalances[to] <= totalAmount,
            "Invalid amount"
        );

        bytes32 signatureHash = keccak256(abi.encodePacked(signature));
        require(!_signatureUsed[signatureHash], "Signature used");
        _verifySignature(amount, totalAmount, to, expireTimestamp, signature);
        _signatureUsed[signatureHash] = true;

        require(amount <= address(this).balance, "Insufficient balance");

        _claimedBalances[to] += amount;
        _payout(to, amount);
    ...
    function getClaimedBalance(address account) public view returns (uint256) {
        return _claimedBalances[account];
    }
}