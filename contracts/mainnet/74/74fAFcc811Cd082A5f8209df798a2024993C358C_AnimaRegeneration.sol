...
import { ActionPermit } from "../Action/ActionPermit.sol";

struct AnimaRegenerationConfig {
  int64 minimumRenownPerLevel;
  int64 renownCapPerLevel;
  uint64 minLinearAnimaMultiplier;
  uint64 aboveCapLinearMultiplier;
  uint baseAnima;
  uint partnerAnimaDivisor;
}
...
contract AnimaRegeneration is ReentrancyGuard, ManagerModifier, Pausable {
  ...
  IBatchAdventurerData public ADVENTURER_DATA;
  ...
  constructor(
    ...
    AOV = _aov;
    ANIMA = IAnima(_anima);
    config.minLinearAnimaMultiplier = 60000;
    config.minimumRenownPerLevel = int64(SIGNED_ONE_HUNDRED);
    config.renownCapPerLevel = int64(300000);
    config.baseAnima = 13 ether;
    config.aboveCapLinearMultiplier = 125000;
    config.partnerAnimaDivisor = 5;
  }
  ...
  function regenerate(
    ...
    require(msg.sender == tx.origin, "Manifest is not allowed through another contract");
    ArrayUtils.checkForDuplicates(_adventurerAddresses, _adventurerIds);

    GATEWAY.checkAddressBatch(_adventurerAddresses, _proofs);
    ...
    mem.levels = ADVENTURER_DATA.getBatch(
      _adventurerAddresses,
      _adventurerIds,
    ...
    uint boost = BATTLE_BOOST.getAnimaBase(msg.sender);
  ...
  function estimateAnima(
    address[] calldata _adventurerAddresses,
    uint256[] calldata _adventurerIds
  ) external returns (uint[] memory availableAnima, int[] memory renownDelta) {
    availableAnima = new uint[](_adventurerAddresses.length);
    renownDelta = new int[](_adventurerAddresses.length);
    for (uint i = 0; i < _adventurerAddresses.length; i++) {
      uint level = ADVENTURER_DATA.get(
        _adventurerAddresses[i],
        _adventurerIds[i],
        traits.ADV_TRAIT_GROUP_BASE,
        traits.ADV_TRAIT_BASE_LEVEL
      );
      int renown = RENOWN.currentRenown(_adventurerAddresses[i], _adventurerIds[i], level);
      int focus = FOCUS.currentFocus(_adventurerAddresses[i], _adventurerIds[i]);
      (availableAnima[i], renownDelta[i]) = _calculateAvailableAnima(level, renown, focus, config);
    }
  }
  ...
  function estimateAnima(
    ...
    int _renown,
    int _focus
  ) public view returns (uint availableAnima, int renownDelta) {
  ...
  function _calculateAvailableAnima(
    ...
    if (_renown <= minRenown) {
      availableAnima = uint(((SIGNED_ONE_HUNDRED * _renown) / minRenown));
      if (availableAnima < uint(cfg.minLinearAnimaMultiplier)) {
        availableAnima = uint(cfg.minLinearAnimaMultiplier);
      }
      availableAnima =
        (_level * (availableAnima * (cfg.baseAnima * uint(_focus)))) /
        (ONE_HUNDRED * ONE_HUNDRED);
      return (availableAnima, 0);
    }
    renownDelta = (_renown - minRenown);
    uint cap = _level * uint(int(cfg.renownCapPerLevel));
    if (cap == 0) {
      return (0, 0);
    }
    cap -= uint(minRenown);

    availableAnima = ONE_HUNDRED - (ONE_HUNDRED * cap) / (uint(renownDelta) + cap);
    availableAnima =
      ONE_HUNDRED +
      (availableAnima * uint(cfg.aboveCapLinearMultiplier)) /
      ONE_HUNDRED;
    availableAnima = (_level * cfg.baseAnima * availableAnima) / (ONE_HUNDRED);
  ...
  function _regenerateInternal(
    ...
    uint lpBonus = (_lpBase * level * uint(focusSpent)) / 10_000;
    if (_adventurerAddress != AOV) {
      animaGained /= cfg.partnerAnimaDivisor;
      lpBonus /= cfg.partnerAnimaDivisor;
    }
  ...
  function updateBoost(address _boost) external onlyAdmin {
    BATTLE_BOOST = IBattleBoost(_boost);
  }
  ...
  function updateConfig(AnimaRegenerationConfig calldata _config) external onlyAdmin {
    config = _config;
  }
}
