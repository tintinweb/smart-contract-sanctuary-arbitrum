...
contract BlastPreDomains is ERC721Enumerable, Ownable {
    ...
    uint256 public price3Letter = 25 * 10 ** 16;
    uint256 public price4Letter = 1 * 10 ** 17;
    uint256 public price5Letter = 8 * 10 ** 15;

    mapping(bytes32 => Record) records;
    mapping(uint256 => Record) public nftRecords;
    ...
    constructor() ERC721("Blast Pre Reg", "BLASTPR") {
    ...
    function recordExists(string calldata name) public view returns (bool) {
        bytes32 node = keccak256(bytes(name));
    ...
    function register(
        ...
        uint256 _price = price(name, duration);
        if (msg.value < _price) {
            revert InsufficientValue();
        }
        require(
            records[keccak256(bytes(name))].owner == address(0x0),
            "Name Already Registered"
        );

        records[keccak256(bytes(name))].owner = _owner;
        records[keccak256(bytes(name))].ttl = duration;
        records[keccak256(bytes(name))].expiredAt = duration + block.timestamp;

        uint256 _tokenId = uint256(keccak256(bytes(name)));

        _mint(_owner, _tokenId);

        nftRecords[_tokenId].owner = _owner;
        nftRecords[_tokenId].ttl = duration;
        nftRecords[_tokenId].expiredAt = duration + block.timestamp;
        ...
        if (_referal != address(0)) {
            ...
            if (isSpecialReferal[_referal]) {
                uint256 _referalAmount = (msg.value *
                ...
                uint256 _referalAmount = (msg.value * referalPercentage) / 100;
