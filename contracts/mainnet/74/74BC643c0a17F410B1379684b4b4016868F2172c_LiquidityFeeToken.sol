...
contract LiquidityFeeToken is IERC20, Initializable, OwnableUpgradeable {
    using Address for address;
    ...
    function initialize(
        ...
        address[3] memory addrs_, // router, marketing wallet, marketing token
        uint16[3] memory feeSettings_ // tax, liquidity, marketing
    ) public initializer {
        require(
            addrs_[1] != address(0),
        ...
        require(
            feeSettings_[0] + feeSettings_[1] + feeSettings_[2] <= MAX_FEE,
        ...
        _marketingAddress = addrs_[1];
        _marketingToken = addrs_[2];
        _marketingFee = feeSettings_[2];
...
*/
    ...
    function _takeMarketingFee(uint256 tMarketing) private {
        ...
        // Send the tokens to this contract if we need to swap the tokens
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (
            ...
            uint256 _totalFees = _liquidityFee + _marketingFee;

            if (_marketingFee > 0) {
                uint256 marketingTokens = (contractTokenBalance *
                    _marketingFee) / _totalFees;
                sendMarketingFee(_marketingToken, marketingTokens);
            }

            if (_liquidityFee > 0) {
                uint256 swapTokens = (contractTokenBalance * _liquidityFee) /
                    _totalFees;
