...
abstract contract ExternalSwapRouterUpgradeable is Initializable {
    ...
    function prepareTokenIn(IERC20 tokenIn, address payer, uint256 tokenAmount, address externalRouter) internal {
        ...
        // ParaSwap special handling
        if (externalRouter == 0xDEF171Fe48CF0115B1d80b88dc8eAB59176FEe57) {
            tokenIn.safeIncreaseAllowance(0x216B4B4Ba9F3e719726886d34a177484278Bfcae, tokenAmount);
        }
        else {
...
interface INativeRouter is ISwapCallback {
    ...
    struct ExcatInputExecutionState {
        address sellerToken;
        address payer;
    ...
    NativePoolStorage
{
    ...
    function executeSwapFromTreasury(uint256 amount, Orders.Order memory _order, address recipient) internal {
        ...
        require(
            (treasuryBalanceInitial - treasuryBalanceFinal) >= FullMath.mulDiv(amount, 999990, 1000000),
    ...
    function executeSwapToTreasury(
        ...
        require(
            (treasuryBalanceFinal - treasuryBalanceInitial) >= FullMath.mulDiv(sellerTokenAmount, 999990, 1000000),
            "Swap amount not match"
        );
    ...
    NativeRfqPoolStorage
{
    ...
    // This follows the exsiting NativePool order singature format
    // keccak256("Order(uint256 id,address signer,address buyer,address seller,address buyerToken,address sellerToken,uint256 buyerTokenAmount,uint256 sellerTokenAmount,uint256 deadlineTimestamp,address caller,bytes16 quoteId)");
    bytes32 private constant ORDER_SIGNATURE_HASH = 0xcdd3cf1659a8da07564b163a4df90f66944547e93f0bb61ba676c459a2db4e20;
    ...
    function initialize(
        ...
        if (_owner == address(0) || _router == address(0) || bytes(_name).length == 0 || _weth == address(0) || _treasury == address(0) || _signer == address(0)) {
    ...
    ExternalSwapRouterUpgradeable
{
    ...
    function exactInput(
        ...
        if (address(this).balance > 0) TransferHelper.safeTransferETH(msg.sender, state.initialEthBalance);
