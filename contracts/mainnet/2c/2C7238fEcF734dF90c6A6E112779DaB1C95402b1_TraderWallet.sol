...
// File contracts/adapters/uniswap/interfaces/IQuoterV2.sol
...
// File contracts/adapters/uniswap/interfaces/IUniswapV3Adapter.sol
...
// File contracts/adapters/uniswap/interfaces/IUniswapV3Router.sol
...
// File contracts/adapters/uniswap/libraries/BytesLib.sol
...
// File contracts/adapters/uniswap/UniswapV3Adapter.sol
...
contract UniswapV3Adapter is OwnableUpgradeable {
    ...
    function _buy(
        ...
        if (isTraderWallet) {
            ...
            uint256 traderBalance = IERC20Upgradeable(tokenIn).balanceOf(traderWallet);
            uint256 vaultBalance = IERC20Upgradeable(tokenIn).balanceOf(usersVault);
            ratio_ = vaultBalance.mulDiv(
                ratioDenominator,
                traderBalance,
                MathUpgradeable.Rounding.Up
            ...
            uint256 amountInAvailable = IERC20Upgradeable(tokenIn).balanceOf(msg.sender);
        ...
        uint256 leftovers = IERC20Upgradeable(tokenIn).balanceOf(address(this));
        if (leftovers > 0) {
            IERC20Upgradeable(tokenIn).safeTransfer(msg.sender, leftovers);
        }
...
// File contracts/adapters/uniswap/interfaces/IUniswapV3Factory.sol
...
// File contracts/TraderWallet.sol
...
contract TraderWallet is BaseVault, ITraderWallet {
    ...
    EnumerableSet.UintSet private _traderSelectedProtocolIds;

    /// @notice array of tokens to make evaluation based on balances
    EnumerableSet.AddressSet private _allowedTradeTokens;
    ...
    function rollover() external override {
        ...
        if (
            _cumulativePendingDeposits == 0 &&
            _cumulativePendingWithdrawals == 0
        ...
        if (_currentRound != 0) {
            ...
            address _underlyingTokenAddress = underlyingTokenAddress;
            uint256 tokenBalance = IERC20(underlyingTokenAddress).balanceOf(
                address(this)
            );
