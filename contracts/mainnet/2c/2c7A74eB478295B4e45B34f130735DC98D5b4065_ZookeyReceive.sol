...
library ZookeyConstants {
    ...
    // GOERLI   B4FBF271143F4FBf7B91A5ded31805e42b2208d6
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _swap(
        ...
        bool success;
        // 1. prepare and swap
        if (_fromToken == ZookeyConstants.ETH) {
            //FROM NATIVE
            if (_toToken == ZookeyConstants.WETH) {
                //ETH => WETH
                success = _swapWrap(
                    address(this),
                    msg.sender,
            ...
            // FROM ERC20
            if (
                _fromToken == ZookeyConstants.WETH &&
                _toToken == ZookeyConstants.ETH
            ) {
                // WETH => ETH
                success = _swapWrap(
                    msg.sender,
    ...
    function swapBridge(
        ...
        (bool success, ) = bridgeAddr.call{
            value: toToken == address(0) ? toTokenBridgeBalance : 0
    ...
    function _bridge(
        ...
        uint256 bridgeAmount;
        bool success;
        if (_bridgeToken != ZookeyConstants.ETH) {
            _tokenRelayer(
                _bridgeToken,
                msg.sender,
                address(this),
    ...
    function _swapWrap(
        ...
        if (reversed) {
            IWETH(ZookeyConstants.WETH).transferFrom(
                from,
                address(this),
                amount
            );
            IWETH(ZookeyConstants.WETH).withdraw(amount);
            if (to != address(this)) {
            ...
            IWETH(ZookeyConstants.WETH).deposit{value: amount}();
            if (to != address(this)) {
                IERC20(ZookeyConstants.WETH).transferFrom(
                    address(this),
                    to,
                    amount
                );
