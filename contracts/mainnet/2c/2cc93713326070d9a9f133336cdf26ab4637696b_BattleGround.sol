...
import {
    ...
    TokensOnBattle,
    UITokenInfo,
    StartBattleBoosterParams
} from "./library/Types.sol";
import {
    Initializable
...
contract BattleGround is Ownable, Initializable {
    ...
    mapping(uint256 => Enemy) private enemies;
    mapping(uint256 => Location) private location;

    mapping(uint256 => uint256) public unlockTime;
    mapping(uint256 => bytes) public tokensBattle;

    mapping(bytes => TokensOnBattle) private tokensOnBattle;
    mapping(bytes => mapping(uint256 => uint256)) public animalsAmount;
    ...
    function initialize(
        ...
        setNeandersmolAddress(_neandersmol);
    ...
    function startBattle(
        ...
        StartBattleBoosterParams[] calldata _startParams
    ) external {
        bytes memory tokenIds = abi.encodePacked(_tokenIds);
        uint256 length = _tokenIds.length;
        uint256 totalAmount;
        uint256 totalLevel;

        if (length > MAX_SMOL_PER_ENTRY) revert AboveMaxEntry();
        for (uint256 i; i < length; i++)
            totalLevel += _startBattle(_tokenIds[i], tokenIds);
        if (_startParams.length != 0)
            totalAmount = boosterFunction(tokenIds, _startParams);

        if (totalAmount > length * MAX_BOOST_PER_NEANDERSMOL)
            revert BoostEntryIsAboveMaximum();

        uint256 maxLocationThreatIndex = totalLevel / length;

        uint64 locationIndex = calculateIndex(
            maxLocationThreatIndex,
            randomizer.revealRandomNumber(reqOne)
        );
        uint64 enemyIndex = calculateIndex(
            maxLocationThreatIndex,
            randomizer.revealRandomNumber(reqTwo)
        );

        tokensOnBattle[tokenIds].owner = msg.sender;
        tokensOnBattle[tokenIds].entryTime = uint64(block.timestamp);
        tokensOnBattle[tokenIds].requestId = randomizer.requestRandomNumber();
        tokensOnBattle[tokenIds].locationIndex = locationIndex;
        tokensOnBattle[tokenIds].enemyIndex = enemyIndex;

        IBones(bones).burn(msg.sender, (totalLevel + 1) * TO_WEI);
    ...
    function upgrade(bytes memory _tokenIds) external {
        TokensOnBattle memory t = tokensOnBattle[_tokenIds];
        if (block.timestamp < BATTLE_DURATION + t.entryTime)
            revert BattleIsStillOn();
        uint256[] memory tokenIds = decodeTokenIds(_tokenIds);
        Location memory l = location[t.locationIndex];
        Enemy memory e = enemies[t.enemyIndex];
        for (uint256 i; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            uint256 currentLevel = (
                skillController.tokenToProgressionStats(tokenId).level
            ) + 1;

            uint256 baseSuccessRate = (l.threat *
                e.threat *
                getSuccessChance(currentLevel)) / SCALE_FACTOR;

            uint256 success = t.booster != 0
                ? (t.booster * baseSuccessRate) / PERCENTAGE
                : baseSuccessRate;

            neandersmol.stakeHandler(tokenIds[i], false);
            bool isSuccessful = (randomizer.revealRandomNumber(t.requestId) %
                SCALE_FACTOR) < success;

            if (isSuccessful) {
                skillController.updateLevel(tokenId, 1);
                skillController.updateXp(tokenId, XP_PER_BATTLE);
            } else {
                unlockTime[tokenId] = block.timestamp + DEFAULT_WAIT_DURATION;
            }
            tokensBattle[tokenId] = "";
            emit LeaveBattle(tokenId, isSuccessful);
        }

        for (uint256 i; i < 6; i++) {
            uint256 amount = animalsAmount[_tokenIds][i];
            ...
            animalsAmount[_tokenIds][i] = 0;
        }

        delete tokensOnBattle[_tokenIds];

        emit Upgrade(_tokenIds, tokenIds);
    }
    ...
    function _startBattle(
        ...
        uint256 lockedAt = unlockTime[_tokenId];

        if (skillController.tokenToProgressionStats(_tokenId).level == 100)
        ...
        if (block.timestamp < lockedAt) revert TokenLocked();
        ...
        if (point == 0) revert ZeroPointsError();
        uint256 maxLevel = getMaxLevelForPoint(point);
        if (maxLevel < currentLevel + 1) revert MaxLevelForThisPointReached();

        neandersmol.stakeHandler(_tokenId, true);

        tokensBattle[_tokenId] = _tokenIds;

        emit EnterBattle(_tokenId);
        return currentLevel;
    }

    function bonesDivisor(uint256 _amount) internal pure returns (uint256) {
        if (_amount % AMOUNT_OF_BONE_ALLOWED_ONE != 0)
            revert InvalidBoosterAmount();

        if (_amount < AMOUNT_OF_BONE_ALLOWED_TWO) {
            return _amount / AMOUNT_OF_BONE_ALLOWED_ONE;
        } else {
            uint256 value = _amount % AMOUNT_OF_BONE_ALLOWED_TWO;

            return
                (_amount / AMOUNT_OF_BONE_ALLOWED_TWO) +
                (value / AMOUNT_OF_BONE_ALLOWED_ONE);
        }
    }

    function boosterFunction(
        bytes memory _tokenIds,
        StartBattleBoosterParams[] calldata _startParams
    ) internal returns (uint256) {
        if (_startParams.length > 0) {
            uint256 totalAmount;
            uint256 totalBoost;
            for (uint256 i; i < _startParams.length; i++) {
                StartBattleBoosterParams memory s = _startParams[i];
                totalAmount += s.amount;
                totalBoost += calculateBoost(
                    s.tokenAddress,
                    s.booster,
                    s.amount
                );

                if (s.tokenAddress == consumables) {
                    IERC1155(s.tokenAddress).burn(
                        msg.sender,
                        s.booster,
                        s.amount
                    );
                }
                if (s.tokenAddress == animals) {
                    IERC1155(s.tokenAddress).safeTransferFrom(
                        msg.sender,
                        address(this),
                        s.booster,
                        s.amount,
                    ...
                    animalsAmount[_tokenIds][s.booster] += s.amount;
                }

                if (s.tokenAddress == bones) {
                    if (s.amount != 1) revert InvalidBoosterAmount();
                    IBones(bones).burn(msg.sender, s.amount * TO_WEI);
                }
            }

            tokensOnBattle[_tokenIds].booster = uint96(totalBoost);
    ...
    function decodeTokenIds(
        ...
        uint256 count = _tokenIds.length / 32;
        uint256[] memory tokenIds = new uint256[](count);

        if (count == 1) {
            uint256 tokenId = abi.decode(_tokenIds, (uint256));
            tokenIds[0] = tokenId;
        }
        if (count == 2) {
            (uint256 tokenId1, uint256 tokenId2) = abi.decode(
                _tokenIds,
                (uint256, uint256)
            );
        ...
        if (count == 3) {
            (uint256 tokenId1, uint256 tokenId2, uint256 tokenId3) = abi.decode(
                _tokenIds,
    ...
    function calculateBoost(
        ...
        if (_boosterAddress == animals) {
            if (_booster == 0 || _booster == 1)
                boost = BABY_DINO_AND_WOLF_BOOSTER;
            if (_booster == 2) boost = SABERTOOTH_BOOSTER;
            if (_booster == 3 || _booster == 4)
                boost = TREX_AND_MAMMOTH_BOOSTER;
            if (_booster == 5) boost = WHALE_AND_5K_BONES_BOOSTER;
        }

        if (isBones) _boosterAmount = bonesDivisor(_booster);

        return boost == 0 ? 0 : boost * _boosterAmount;
    }
    ...
    function getConsumablesRarity(
        ...
        return _tokenId > RARE_CONSUMABLE_MAX_ID;
    }
    ...
    function getPoint(uint256 _tokenId) public view returns (uint256) {
        ...
        uint256 totalSkill = (skill.mystics + skill.farmers + skill.fighters) /
            SKILL_POINT;
        uint256 commondSense = (skillController.commonSense(_tokenId) *
            CS_DECIMAL) / CS_POINT;
        uint256 jobsCompleted = laborGround.getJobsCompleted(_tokenId) *
            JOB_POINT;
    ...
    function calculateIndex(
        ...
        if (_currLevel == 0) _currLevel = 1;

        uint256 index = _currLevel + 1 > LEVEL_CAP
            ? _randomSeed % LEVEL_CAP
            : _randomSeed % _currLevel;
    ...
    function getMaxLevelForPoint(
        uint256 _points
    ) internal pure returns (uint256) {
        uint256 discriminant = 1 + 8 * _points;

        uint256 sqrtDiscriminant = discriminant.sqrt();

        uint256 maxLevel = (sqrtDiscriminant - 1) / 2;
    ...
    function getIndex(uint256 _level) internal pure returns (uint64) {
        return uint64(_level % EVEN_CHECK != 0 ? _level : _level - 1);
    }
    ...
    function getAllEnemies() external view returns (Enemy[] memory) {
        ...
        uint256 enemyIndex;
        for (uint256 i = 1; i < LEVEL_ENTRIES * 2; i += 2) {
            Enemy memory currentEnemy = enemies[getIndex(i)];
            e[enemyIndex++] = currentEnemy;
        }
        return e;
    }
    ...
    function getAllLocations() external view returns (Location[] memory) {
        ...
        for (uint256 i = 1; i < LEVEL_ENTRIES * 2; i += 2) {
            ...
            l[locationIndex++] = currentLocation;
        }
    ...
    function getSuccessChance(uint _level) internal pure returns (uint) {
        uint256 chance = SCALE_FACTOR - (PERCENTAGE * _level);

        return
            _level > MAX_LEVEL_FOR_MINIMUM_CHANCE_OF_SUCCESS
    ...
    function getTokensOnBattleInfo(
        bytes memory _tokenIds
    ) external view returns (TokensOnBattle memory) {
        return tokensOnBattle[_tokenIds];
    }
    ...
    function getTokens(
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            if (tokensBattle[tokenIds[i]].length == 0) continue;
            count += 1;
        }
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            bytes memory id = tokensBattle[tokenId];
            TokensOnBattle memory t = tokensOnBattle[id];

            if (id.length > 0 && t.owner == _owner) {
                uiTokenInfo[uiIndex++] = UITokenInfo({
                    tokenId: uint64(tokenId),
                    location: location[t.locationIndex].name,
                    enemy: enemies[t.enemyIndex].name,
                    battleReaccessTimer: uint96(unlockTime[tokenId]),
                    id: id,
    ...
    function setNeandersmolAddress(address _neandersmol) public onlyOwner {
    ...
    event EnterBattle(uint256 indexed tokenId);
    event LeaveBattle(uint256 indexed tokenId, bool indexed outcome);
    ...
    event Upgrade(bytes indexed id, uint256[] indexed tokenIds);
...
struct StartBattleBoosterParams {
...
interface ILaborGround {
    function getJobsCompleted(uint256 _tokenId) external view returns (uint256);
}
...
error InvalidLevel();
error TokenLocked();
error ZeroPointsError();
error BattleIsStillOn();
error MaxLevelReached();
error BoostEntryIsAboveMaximum();
...
uint256 constant BATTLE_DURATION = 6 hours;

uint256 constant DEFAULT_WAIT_DURATION = 72 hours;
uint256 constant RARE_CONSUMABLE_WAIT_DURATION = 24 hours;
uint256 constant COMMON_CONSUMABLE_WAIT_DURATION = 60 hours;
...
uint256 constant CS_DECIMAL = 10 ** 9;
...
uint256 constant MINIMUM_CHANCE_OF_SUCCESS = 1000;
uint256 constant MAX_LEVEL_FOR_MINIMUM_CHANCE_OF_SUCCESS = 89;

uint256 constant LEVEL_ENTRIES = 8;

uint256 constant MAX_BOOST_PER_NEANDERSMOL = 5;
