...
contract BrokenEquipmentHandler is IBrokenTokenHandler, ManagerModifier {
  ...
  // item id -> shard obtained by breaking
  mapping(uint256 => ShardConfig) public shardConfigs;
  ...
  function handleBrokenToken(
    ...
    if (_amount <= 0) {
      ShardConfig memory config = shardConfigs[_entityTokenId];
      uint totalAmount = _amount * uint(config.amount);
      if (totalAmount > 0) {
        ARMORY.mint(
          _ownerAddress,
          _ownerTokenId,
  ...
  function handleBrokenTokenBatch(
    ...
    for (uint i = 0; i < _entityTokenIds.length; i++) {
      ...
      ShardConfig memory config = shardConfigs[_entityTokenIds[i]];
      if (config.amount == 0) {
        continue;
      }

      anyBroken = true;
      shardIds[i] = config.id;
      amounts[i] = _amounts[i] * uint(config.amount);
    }
  ...
  function handleBrokenTokenBatch(
    ...
    for (uint i = 0; i < _entityTokenIds.length; i++) {
      ...
      for (uint j = 0; j < _entityTokenIds[i].length; j++) {
        ...
        ShardConfig memory config = shardConfigs[_entityTokenIds[i][j]];
        if (config.amount == 0) {
          continue;
        }

        anyBroken = true;
        shardIds[i][j] = config.id;
        amounts[i][j] = _amounts[i][j] * uint(config.amount);
      }
    }
    if (anyBroken) {
  ...
  function configure(
    ...
    for (uint i = 0; i < _itemIds.length; i++) {
      shardConfigs[_itemIds[i]].id = _shardIds[i];
      shardConfigs[_itemIds[i]].amount = _amounts[i];
    }
  }
}
