...
contract VodkaVaultV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function setProtocolFee(
        ...
        require(_liquidatorsRewardPercentage <= MAX_BPS && _withdrawalFee <= 1000, "Fees !bounds");
    ...
    function requestClosePosition(uint256 _positionID, address _user) external payable InvalidID(_positionID, _user) nonReentrant {
        ...
        require(msg.sender == _positionInfo.user || allowedClosers[msg.sender], "VodkaV2: !allowed");
    ...
    function requestLiquidatePosition(address _user, uint256 _positionID) external payable nonReentrant onlyKeeper{
        PositionInfo storage _positionInfo = positionInfo[_user][_positionID];
    ...
    function fulfillLiquidation(bytes32 _key, uint256 _returnedUSDC) external onlyHandler returns (bool) {
        ...
        if (wr.returnedUSDC >= debtPortion) {
            ...
            if(leftovers > 0) {
                IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(wr.user, leftovers);
            }

            IERC20Upgradeable(strategyAddresses.USDC).safeIncreaseAllowance(strategyAddresses.WaterContract, debtPortion);
            IWater(strategyAddresses.WaterContract).repayDebt(debtPortion, debtPortion);
        } else {
            IERC20Upgradeable(strategyAddresses.USDC).safeIncreaseAllowance(strategyAddresses.WaterContract, wr.returnedUSDC);
