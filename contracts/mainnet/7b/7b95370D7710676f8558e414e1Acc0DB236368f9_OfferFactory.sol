...
abstract contract Collectible is ICollectible, NonblockingLzApp, ERC721, ERC721URIStorage, ERC721Enumerable, DAOAccessControlled {
    ...
    // Data for the collectible
    CollectibleData collectibleData;

    address public loot8Token; // ERC20 Token address
    ...
    mapping(string => string) private _errorMessages;
    ...
    constructor(
      ...
      NonblockingLzApp(_layerzeroEndpoint) {
        ...
        // Set the address for loot8 token
        loot8Token = _loot8Token;

        // Activate the collectible as soon as it is deployed
        collectibleData.isActive = true;

        collectibleIds.increment(); // Start from 1 as id == 0 is a check for existence

        _errorMessages["RETIRED"] = "COLLECTIBLE RETIRED";
        _errorMessages["INVALID"] = "INVALID COLLECTIBLE";
        _errorMessages["SUSPENDED"] = "COLLECTIBLE SUSPENDED";
        _errorMessages["NONTRF"] = "COLLECTIBLE NON TRANSFERABLE";
        _errorMessages["NONOWNER"] = "SENDER NOT OWNER";
        _errorMessages["GAS-SHORTAGE"] = "INSUFFICIENT MESSAGE VALUE FOR GAS";
    ...
    */
    ...
    ) internal virtual returns (uint256 newCollectibleId)
    {
        ...
        require(collectibleData.isActive, _errorMessages["RETIRED"]);
    ...
    */
    function _toggle(uint256 _collectibleId) internal virtual returns(bool _status) {
        ...
        require(collectibleDetails[_collectibleId].id != 0, _errorMessages["INVALID"]);
    ...
    */
    function _retire() internal virtual {
        require(collectibleData.isActive, _errorMessages["RETIRED"]);
        collectibleData.isActive = false;
    ...
    */
    function _creditRewards(address _patron, uint256 _amount) internal virtual {
        ...
        require(collectibleDetails[tokenOfOwnerByIndex(_patron, 0)].isActive, _errorMessages["SUSPENDED"]);
    ...
    */
    function _debitRewards(address _patron, uint256 _amount) internal virtual {
        ...
        // Check if the patrons collectible is active
        require(collectibleDetails[collectibleId].isActive, _errorMessages["SUSPENDED"]);
    ...
    */
    // function _addVisit(uint256 _collectibleId, bool _friend) internal virtual {

    //     // Check if collectible is active
    //     require(isActive, _errorMessages["RETIRED"]);

    //     // Check if collectible NFT with the given Id exists
    //     require(collectibleDetails[_collectibleId].id != 0, _errorMessages["INVALID"]);

    //     // Check if patron collectible is active or disabled
    //     require(collectibleDetails[_collectibleId].isActive, _errorMessages["SUSPENDED"]);
    ...
    */
    function toggleMintWithLinked() external onlyEntityAdmin(collectibleData.entity) {
        collectibleData.mintWithLinked = !collectibleData.mintWithLinked;
        emit MintWithLinkedToggled(collectibleData.mintWithLinked);
    }
    ...
    function _calculateRewards(uint256 _price) internal virtual returns(uint256) {
        return (_price * (10**ILoot8Token(loot8Token).decimals())) / 
                ITokenPriceCalculator(IDispatcher(IDAOAuthority(authority).getAuthorities().dispatcher).priceCalculator()).pricePerMint();
    }
    ...
    function safeTransferFrom(
        ...
        require(collectibleDetails[tokenId].transferable, _errorMessages["NONTRF"]);
        safeTransferFrom(from, to, tokenId, "");
    }

    function isRetired(address _patron) external virtual view returns(bool) {
    ...
    */
    function getNFTDetails(uint256 _nftId) external virtual view returns(CollectibleDetails memory) {
    ...
    // function dataURI() external virtual view returns(string memory) {
    ...
    function balanceOf(address _owner) public view override(ICollectible, IERC721, ERC721) returns (uint256) {
    ...
    function isLinkedCollectible(address _collectible) external view returns(bool) {
        for(uint256 i=0; i < linkedCollectibles.length; i++) {
            if(linkedCollectibles[i] == _collectible) {
                return true;
            }
    ...
    */
    function sendCollectibleToChain(uint16 _destinationChainId, uint256 _collectibleId) external payable {
        ...
        require(_msgSender() == ownerOf(_collectibleId), _errorMessages["NONOWNER"]);
        ...
        require(msg.value >= messageFee, _errorMessages["GAS-SHORTAGE"]);
