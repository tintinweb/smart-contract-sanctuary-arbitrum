...
// File contracts/registrar/DaoHallRegistryL2.sol



pragma solidity ^0.8.0;
contract DaoHallRegistryL2 is ERC721, Ownable, IDaoHallController, Operable{
  ...
  function setSubnodeRecord(
    ...
    uint64 expiry
      ...
      bytes32 subnode = _makeNode(node, keccak256(bytes(label)));
  ...
  function getNameData(bytes32 /*labelhash*/) public pure returns (address owner, uint64 expiry) {
      owner = address(0);
      expiry = 0;
  }
  
  function getNodeData(bytes32 node) public view returns (address owner, uint64 expiry) {
      owner = records[node].owner;
      expiry = records[node].expiry;
  }

  function getSubnodeRecord(bytes32 node) public view returns (Record memory) {
    return records[node];
  }

  function setResolverAndTTL(bytes32 node, address resolver, uint64 ttl) public {
    require(_isApprovedOrOwner(msg.sender, uint256(node)), "Not authorised");
    records[node].resolver = resolver;
    records[node].ttl = ttl;
  }

  function _setRecord(bytes32 node, Record memory record) internal {
    records[node] = record;

    // mint or transfer
    uint256 tokenId = uint256(node);
    address curOwner = _ownerOf(tokenId);
    if(curOwner != record.owner){
      if(curOwner == address(0)){
        _safeMint(record.owner, tokenId);
      }
      else if(record.owner == address(0)){
        _burn(tokenId);
      }
      else{
        safeTransferFrom(curOwner, record.owner, tokenId);
      }
    }
  }
  ...
  function _afterTokenTransfer(
      ...
      bytes32 node = bytes32(firstTokenId);
