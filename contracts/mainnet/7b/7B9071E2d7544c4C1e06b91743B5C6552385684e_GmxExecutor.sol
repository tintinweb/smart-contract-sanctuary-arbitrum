...
library GmxStoredData {
    ...
    function removePositionIfEmpty(
        ...
        if (size == 0) {
            VaultBaseExternal(address(this)).removeAssetLock(
                keyData._collateralToken
            );
            VaultBaseExternal(address(this)).updateActiveAsset(
                keyData._collateralToken
    ...
    function updatePositions(
        ...
        if (!positionIsTracked) {
            ...
            VaultBaseExternal(address(this)).addActiveAsset(_collateralToken);
            VaultBaseExternal(address(this)).addAssetLock(_collateralToken);
            dataTracker.pushData(
                Integration.GMXPositions,
                abi.encode(
                    GMXPositionData({
