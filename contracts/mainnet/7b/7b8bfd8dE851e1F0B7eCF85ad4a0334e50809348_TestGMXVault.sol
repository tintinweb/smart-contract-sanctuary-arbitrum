...
contract TestGMXVault {
  ...
  address orderVault = 0x31eF83a530Fde1B38EE9A18093A333D8Bbbc40D5;
  ...
  IERC20 USDC = IERC20(0xaf88d065e77c8cC2239327C5EDb3A432268e5831); // USDC
  ...
  constructor() {
    ...
    WETH.approve(address(orderVault), type(uint256).max);
    USDC.approve(address(orderVault), type(uint256).max);
  ...
  */
  ...
  function createOrder(address token, uint256 amt, uint256 executionFee) external payable returns (bytes32) {
    ...
    _swapPath[0] = address(WETH_USDC_GM);

    IExchangeRouter.CreateOrderParamsAddresses memory _addresses =
      IExchangeRouter.CreateOrderParamsAddresses({
        receiver: address(this),
    ...
    IExchangeRouter.CreateOrderParamsNumbers memory _numbers =
      IExchangeRouter.CreateOrderParamsNumbers({
        sizeDeltaUsd: 0,
    ...
    return exchangeRouter.createOrder(_cop);
  }

  event AfterOrderExecution(bytes32 orderKey, uint256 received);
  event AfterOrderCancellation(bytes32 orderKey, uint256 received);
  event AfterOrderFrozen(bytes32 orderKey, uint256 received);
  ...
  function afterOrderExecution(
    ...
    emit AfterOrderExecution(orderKey, eventData.uintItems.items[0].value);
  }
  ...
  function afterOrderCancellation(
    ...
    emit AfterOrderCancellation(orderKey, eventData.uintItems.items[0].value);
  }
  ...
  function afterOrderFrozen(
    ...
    emit AfterOrderFrozen(orderKey, eventData.uintItems.items[0].value);
  }
