...
contract NonFungibleFarm is Ownable {
    ...
    function changeDepositFee(uint256 fee) external onlyOwner {
        depositFee = fee;
    }
    ...
    function deposit(uint256 id, uint256 _amount) external {
        uint256 amount = (_amount * (100 - depositFee)) / 100;
        uint256 feeamount = _amount - amount;
        lpTokens[id].transferFrom(msg.sender, address(this), amount);
        lpTokens[id].transfer(address(this), amount);
        ...
        fren.amount = fren.amount.add(amount);
