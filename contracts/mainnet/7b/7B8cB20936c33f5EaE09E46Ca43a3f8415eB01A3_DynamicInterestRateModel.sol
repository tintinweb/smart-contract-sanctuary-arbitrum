...
contract DynamicInterestRateModel is IInterestRateModel {
  ...
  uint128 public immutable _baseRate;

  uint128 public immutable _kink;
  
  uint128 public immutable _slope0;
  
  uint128 public immutable _slope1;  

  constructor (IAddressRegistry registry, uint128 baseRate, uint128 kink, uint128 slope0, uint128 slope1) {
    _registry = registry;
    _baseRate = baseRate;
    _kink = kink;
    _slope0 = slope0;
  ...
  function calculateRate() external view returns (uint128) {
    IDepositorVault depositorVault = IDepositorVault(_registry.getDepositorVault());

    IDepositorVault.Snapshot[] memory snapshots = findSnapshots(depositorVault, block.timestamp.toUint32());

    // we ignore the most recent snapshot if we can as
    // it could be manipulated during same block operations
    uint256 i = snapshots.length == 1 ? 0 : 1;
    ...
    for (; i < snapshots.length; i++) {
      if (snapshots[i].timestamp > 0) {
        total = total + snapshots[i].totalAssets;
        count = count + 1;
      }
    }

    uint256 totalAssets = count == 0 ? 0 : total / count;

    return previewRate(getMarketUtilization(depositorVault, totalAssets));
  }

  function findSnapshots(IDepositorVault depositorVault, uint32 timestamp) private view returns (IDepositorVault.Snapshot[] memory snapshots) {
    snapshots = depositorVault.findSnapshots(timestamp, 4);

    if (snapshots.length == 0 || snapshots[0].timestamp == 0) {
      // we dont have any snapshots yet so use a default
      snapshots = new IDepositorVault.Snapshot[](1);
      snapshots[0] = IDepositorVault.Snapshot({
        timestamp: block.timestamp.toUint32(),
        totalAssets: depositorVault.getTotalAssets(),
        totalShares: depositorVault.getTotalShares(),
        totalLiabilities: depositorVault.getTotalLiabilities().toUint128()
      });
    }
  }

  function previewRate(uint256 utilization) public view returns (uint128) {
    if (utilization < _kink) {
      return _baseRate + _slope0.mulDiv(utilization, _kink).toUint128();
    }
    return _baseRate + _slope0 + _slope1.mulDiv(utilization - _kink, 1e18).toUint128();
  }

  function getMarketUtilization(IDepositorVault depositorVault, uint256 totalAssets) private view returns (uint256) {
    address marketAddress = address(_registry.getMarket());

    IDepositorVaultStorage depositorVaultStorage = _registry.getDepositorVaultStorage();
    IDepositorVaultStorage.BorrowLimit memory borrowLimit = depositorVaultStorage.getBorrowLimit(marketAddress);

    uint256 theoreticalLimit = borrowLimit.absoluteOrRelative == IDepositorVaultStorage.AbsoluteOrRelative.ABSOLUTE
      ? borrowLimit.amount
      : BPS.wrap(borrowLimit.amount.toUint16()).percentOf(totalAssets);

    if (theoreticalLimit == 0) {
      return 0;
    }

    uint8 decimals = depositorVault.getDecimals();

    uint256 liabilities = depositorVault.getLiabilities(marketAddress);

    uint256 effectiveLimit = Math.min(theoreticalLimit, totalAssets);

    uint256 utilization = effectiveLimit == 0 
      ? 0 
      : Math.mulDiv(liabilities.normalize(decimals, 18), 1e18, effectiveLimit.normalize(decimals, 18));
    
    return Math.min(1e18, utilization);
  }
}