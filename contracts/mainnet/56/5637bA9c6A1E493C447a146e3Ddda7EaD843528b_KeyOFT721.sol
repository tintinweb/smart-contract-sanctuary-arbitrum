    ...
    HeroOFTErrors
{
    ...
    function _lzReceive(
        ...
        (address to, uint64 idOrAmount) = abi.decode(
            _message,
            (address, uint64)
        );
        uint256 amountReceivedLD = _credit(
            to,
            _toLocalDecimals(idOrAmount),
            false
        );
    ...
    function estimateFee(
        ...
        return
            _estimateFee(
                _dstEid,
                abi.encode(_to, _toSharedDecimals(_tokenId)),
                defaultLzOption
            ).nativeFee;
    }
    ...
    function _estimateFee(
        ...
        bytes memory _message,
        bytes memory _options
    ) internal view returns (MessagingFee memory fee_) {
     ...
     */
    function updateLayerZeroGasLimit(
        uint32 _lzGasLimit
    ...
    function _updateLayerZeroGasLimit(uint32 _lzGasLimit) internal virtual {
        ...
        defaultLzOption = OptionsBuilder
            .newOptions()
...
abstract contract HeroOFTXCallbacks {
    function _debit(
        uint256 _amountOrId,
        uint256 _minAmount
    ) internal virtual returns (uint256 _amountSendingOrId_);

    function _credit(
        address _to,
        uint256 _value,
        bool _isFrozen
    ) internal virtual returns (uint256 amountReceived_);
}
...
interface IHeroOFTX {
    ...
    event OFTReceived(
        bytes32 indexed guid,
        uint32 indexed sourceEndpointId,
        address indexed to,
        uint256 amountOrId
    );
...
contract KeyOFT721 is OFT721, SendNativeHelper {
    ...
    function buy() external payable {
        ...
        if (address(inputToken) == address(0) && msg.value != cost)
            revert InvalidAmount();
        if (
            address(inputToken) != address(0) &&
            !inputToken.transferFrom(msg.sender, treasury, cost)
        ) {
            revert FailedToTransfer();
        }

        if (maxSupply != 0 && cacheTotalSupply > maxSupply)
...
abstract contract OFT721 is HeroOFTX, ERC721, IERC721Receiver {
    ...
    function _debit(
        uint256 _amountOrId,
        uint256
    ) internal override returns (uint256 _amountSendingOrId_) {
    ...
    function _credit(
        ...
        if (exists && _ownerOf(_value) != address(this))
            revert NFTOwnerIsNotContract();
