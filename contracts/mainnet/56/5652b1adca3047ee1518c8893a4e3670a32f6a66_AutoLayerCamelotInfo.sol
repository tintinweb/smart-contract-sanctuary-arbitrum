    ...
    function getUserLPs(address user) public view  returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            for (uint256 j = 0; j < nftAmount; j++) {
                ...
                (uint256 amount, uint256 amountWithMultiplier, uint256 startLockTime,uint256 lockDuration, uint256 lockMultiplier, uint256 rewardDebt, uint256 boostPoints, uint256 totalMultiplier ) = INFTPool(pools[i].nftContract).getStakingPosition(nftId);
                userLPs[i] += amount;
            }
        }
        return userLPs;
