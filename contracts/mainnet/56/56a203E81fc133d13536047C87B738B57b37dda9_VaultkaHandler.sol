...
contract VaultkaHandler is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function takeInterestsSnapshot(address[] memory _VaultkaVault) public onlyKeeper {
        // We assume that the vaults are not going to change across all strategies
        uint256 allVaultsLen = _VaultkaVault.length;
        for (uint256 i = 0; i < allVaultsLen; i++) {
            require(allowedVaults[_VaultkaVault[i]], 'Not an allowed vault');
            address lendingVault = _VaultkaVault[i];
    ...
    function getInterestRate(address _lendingVault) public view returns (uint256) {
        ...
        InterestRateConfig memory irc = vaultInterestRateConfig[_lendingVault];
