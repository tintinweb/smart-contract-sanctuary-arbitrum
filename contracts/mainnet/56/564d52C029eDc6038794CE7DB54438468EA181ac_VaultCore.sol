...
interface IPool {
    ...
    function quotePotentialWithdrawFromOtherAsset(
        ...
        address toToken,
        uint256 liquidity
    ) external view returns (uint256 amount, uint256 withdrewAmount);
  ...
  OwnableUpgradeable
{
  ...
  function increaseAllowance(
    ...
    _allowances[msg.sender][_spender] = _allowances[msg.sender][_spender].add(
      _addedValue
    );
  ...
  function changeSupply(
    ...
    _totalSupply = _rebasingCredits.divPrecisely(_rebasingCreditsPerToken).add(
      nonRebasingSupply
    );
  ...
  /**
   * @notice Crytical setter will be protected by the multisig
...
import { IWombatRouter } from "./interfaces/strategies/wombat/IWombatRouter.sol";
