...
contract BeefyStargateZapOneInch {
    ...
    function _swapAndStake(address _vault, address _inputToken, bytes calldata _token0, uint256 _minAmountOut) private {
        ...
        if (tokens[0] != SGETH) {
            ...
            IStargateRouter(stargateRouter).addLiquidity(uint256(pool.poolId()), inputBal, address(this));
        } else {
    ...
    receive() external payable {
        assert(msg.sender == WETH || msg.sender == SGETH);
    }
}
