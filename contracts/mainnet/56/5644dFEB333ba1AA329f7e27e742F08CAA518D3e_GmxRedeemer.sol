...
library GMXDecreaseOrdersStoredData {
    ...
    // Whenever the vault receives ETH, it will be converted to WETH
    // Therefore when we refund the execution fee for cancelled orders
    // We must convert the amount back to ETH
...
/**
 * @title   GmxStoredData
 * @notice  Utility library for storing GMX position and request data
 * @dev     Uses the IntegrationDataTracker to store GMX position and request data
 * @dev     The reason we use IntegrationDataTracker is so we don't have to make changes
 * @dev     To the core vault code to add storage for an integration
 * @dev     Be mindful to differentiate between Positions and Requests
 */
library GmxStoredData {
     ...
     * @dev     Can only be called by the vault, keyed by msg.sender
     * @param   key  the gmx request key (kekkak(account, index))
     * @param   requestData  the request data
     * @param   maxRequest  the maximum number of requests allowed by valio
     */
     ...
     * @dev     Can only be called by the vault as they're keyed by msg.sender
     * @param   registry  the valio registry
    ...
    /**
     * @notice  Removes the GMXPositionData at the given index for the calling vault
     * @dev     Only removes the position from storage if it's empty and there is no outstanding request
     * @param   keyData  the GMXPositionData to remove
     * @param   index  takes the index so it doesn't have to find it again
     */
    function removePositionIfEmpty(
        GMXPositionData memory keyData,
        uint index
    ) internal returns (uint size, uint collateral) {
        VaultBaseExternal vault = VaultBaseExternal(payable(address(this)));
        Registry registry = vault.registry();
        (size, collateral, , , , , , ) = IGmxVault(registry.gmxConfig().vault())
            .getPosition(
                address(this),
        ...
        if (size == 0) {
            vault.removeAssetLock(keyData._collateralToken);
            vault.updateActiveAsset(keyData._collateralToken);
     ...
     * @notice  Removes the GMXPositionData at the given index for the calling vault
     * @dev     Only removes the position from storage if it's empty and there is no outstanding request
     * @param   keyData  the GMXPositionData to remove, finds the index by looping through the stored data
     */
    ...
    /**
     * @notice  Given the composite key for a position, checks if it is tracked and if not adds it
     * @dev     It also removes any stale executed requests from storage.
     * @dev     We only allows a certain number of positions as no to exceed gas limits
     * @param   _indexToken  the index token (part of the composite key)
     * @param   _collateralToken  the collateral token (part of the composite key)
     * @param   _isLong  whether the position is long or short (part of the composite key)
     * @param   maxPositionsAllowed  the maximum number of positions allowed by valio
     */
    function updatePositions(
        ...
        if (!positionIsTracked) {
            // At anytime the vaults position can be partially liquidated
            // and some collateral may be returned to the vault
            // For the life of this position the vault must track this collateralToken
            VaultBaseExternal vault = VaultBaseExternal(payable(address(this)));
            vault.addActiveAsset(_collateralToken);
            vault.addAssetLock(_collateralToken);
            dataTracker.pushData(
                Integration.GMXPositions,
                abi.encode(
                    GMXPositionData({
    ...
    /**
     * @notice  Removes an executed request from storage
     * @dev     This should be uneccessary because the request should get removed when we receive
     * @dev     the callback from the GMXPositionRouter, but we have no guarantees on the callback
     * @dev     because it's wrapped in a try catch (on the gmx side)
     */
    ...
    /**
     * @notice  Checks if a request has not been executed
     * @dev     We don't want to stop tracking the position if a request is still open
     * @dev     This stops the vault from stopping tracking a position that is about to be open
     * @param   vault  the valio vault
     * @param   positionKeyData  the GMXPositionData to check
     * @return  bool  whether the request is still active
     */
     ...
     * @dev     This is used in the gmxCallback as we only receive the key
     * @param   vault  the valio vault
     * @param   key  the gmx request key to search for
     * @return  GMXRequestData  the request data
     * @return  index  the index of the request in storage
     */
    ...
    /**
     * @notice  Returns the number of tracked gmx positions in storage
     * @dev     Used to enforce the positions limit (for gas reasons)
     * @param   vault  the valio vault
     * @return  uint256  the number of tracked gmx positions
     */
    ...
    /**
     * @notice  Returns all tracked GMXPositionData in storage for the vault
     * @dev     We use this data to fetch the position information from gmx
     * @param   vault  the valio vault
     * @return  GMXPositionData[]  the tracked gmx positions
     */
    ...
    /**
     * @notice  Returns all the tracked GMXRequestData in storage for the vault
     * @dev     The are unexecuted requests to open a position, not positions
     * @param   vault  the valio vault
     * @return  GMXRequestData[]
     */
...
import { GmxStoredData } from '../lib/GmxStoredData.sol';
import { GMXDecreaseOrdersStoredData } from '../lib/GMXDecreaseOrdersStoredData.sol';
import { GmxHelpers } from '../lib/GmxHelpers.sol';
import { Constants } from '../lib/Constants.sol';

/**
 * @title   GmxRedeemer
 * @notice  Resizes gmx positions owned by the vault and provides proceeds to the withdrawer
 * @dev     Called via Delegate Call from the vault
 * @dev     In the case where the position cannot be resized, it is closed and all proceeds returned to the Vault
 * @dev     Note: that it implements preWithdraw, as this must happen before the erc20 redemption,
 * @dev     incase funds are redeemed to the vault
 */
contract GmxRedeemer is IRedeemer {
    bool public constant hasPreWithdraw = true;

    // Stake to deep struct
    struct CallResult {
        bool success;
        bytes returnData;
    }
     ...
     * @dev     All redemption logic is completed in preWithdraw
     */
    function withdraw(
        uint tokenId,
        address gmxVault,
    ...
    /**
     * @notice  Loops through the gmx positions held by the vault, resizes and redeems them
     * @dev     For Trigger Orders, after the resize this contract will attempt
     * @dev     to adjust the matching orders relative to the resize
     * @param   tokenId  TokenId of the holding, only for tracking
     * @param   gmxVault  The address of the GmxVault aka Asset
     * @param   withdrawer  The withdrawers address, where funds are sent
     * @param   portion  The portion of the positions to redeem
     */
    function preWithdraw(
        ...
        for (uint256 i = positions.length; i > 0; i--) {
            GmxStoredData.GMXPositionData memory keyData = positions[i - 1];
            _processPosition(tokenId, gmxVault, withdrawer, portion, keyData);
            // We need to stop tracking the position if its empty
            (uint size, uint collateral) = GmxStoredData.removePositionIfEmpty(
                keyData,
                i - 1
            );

            // If the manager has an open orders for the position
            // We resize it relative to the withdraw portion
            GMXDecreaseOrdersStoredData.adjustMatchingOrders(
                keyData,
                portion,
                size,
                collateral,
    ...
    /**
     * @notice  Resizes and Redeems a single gmx position
     * @dev     The implementation of gmxVault.decreasePosition is spagetti
     * @dev     In somecases they deduct fees from collateral in others pnl
     * @dev     So therefore we force fees to be charged to the position before we resize it
     * @param   tokenId  The holding tokenId, only used for tracking
     * @param   gmxVault  The Gmx Vault address
     * @param   withdrawer  The withdrawers address
     * @param   portion  The portion to redeem
     * @param   keyData  The position data
     */
    function _processPosition(
        ...
        (uint256 size, uint collateral, , , , , , ) = IGmxVault(gmxVault)
            .getPosition(
                address(this),
                keyData._collateralToken,
                keyData._indexToken,
        ...
        // This forces gmx to charge all accrued funding fees
        // To the position before we reduce it
        // If we don't do this the fees will be deducted from pnl of the withdrawer
        (callResult.success, ) = gmxVault.call(
            abi.encodeWithSelector(
                IGmxVault.decreasePosition.selector,
                address(this),
                keyData._collateralToken,
                keyData._indexToken,
                0,
                1, // sizeDelta 1 is the minimum
                keyData._isLong,
                address(this)
            )
        );

        // If we can't charge the funding then we close the whole position
        // This will only happen if the position exceeds max leverage
        // We return immediately as any proceeds from closing
        // the position will be returned to the vault
        ...
        // If there was funding owed the collateral will now be adjusted
        (size, collateral, , , , , , ) = IGmxVault(gmxVault).getPosition(
            address(this),
            keyData._collateralToken,
            keyData._indexToken,
            keyData._isLong
        ...
        // STD Scope
        {
            ...
            uint collateralPortion = ((collateral * portion) /
                Constants.PORTION_DIVISOR);
            // If the withdrawers PNL does not cover the fees we skip the redemption
            if (
                !_pnlGreaterThanFee(
                    gmxVault,
                    keyData,
            ...
            (callResult.success, callResult.returnData) = gmxVault.call(
                abi.encodeWithSelector(
                    IGmxVault.decreasePosition.selector,
                    address(this),
                    keyData._collateralToken,
        ...
        // If the above resize/redeem fails we close the entire position
        // And return all proceeds to the vault for later redemption
        if (!callResult.success) {
            _closeEntirePositionAndReturnProceedsToVault(
                tokenId,
                gmxVault,
        ...
        VaultBaseExternal(payable(address(this))).registry().emitEvent();
        emit Redeemed(
            tokenId,
            gmxVault,
            withdrawer,
            keyData._isLong ? keyData._indexToken : keyData._collateralToken,
            abi.decode(callResult.returnData, (uint))
        );
    }

    /**
     * @notice  Closes the position and returns all proceeds to the vault
     * @dev     In the case where the withdrawer portion == 100% or the position cannot be resized
     * @param   tokenId  The holding tokenId, only used for tracking
     * @param   gmxVault  The Gmx Vault address
     * @param   keyData  The position data
     */
    function _closeEntirePositionAndReturnProceedsToVault(
        ...
        emit Redeemed(
            ...
            address(this),
            keyData._isLong ? keyData._indexToken : keyData._collateralToken,
            amountOut // Returned to vault, for subsequent redemption
        );
    }

    /**
     * @notice  Checks if the valio vault has an open request
     * @dev     When submitting orders to gmx, first an intent is submitted
     * @dev     A few seconds later the order is executed.
     * @dev     If their is an unexcuted order we revert.
     */
    ...
    /**
     * @notice  Checks that the withdrawers PNL is greater than the fees
     * @dev     If this is not the case there is no point execising the redemption
     * @param   gmxVault  The Gmx Vault address
     * @param   keyData  The position data
     * @param   size  The size of the whole gmx position
     * @param   sizePortion  The withdrawers size portion
     * @param   collateralPortion  The withdrawers collateral portion
     * @return  bool  .
     */
    function _pnlGreaterThanFee(
        address gmxVault,
        GmxStoredData.GMXPositionData memory keyData,
        uint size,
        uint sizePortion,
        ...
        uint256 usdOut = collateralPortion;
        ...
        if (hasProfit) {
            usdOut += (sizePortion * delta) / size;
        }

        uint fee = IGmxVault(gmxVault).getPositionFee(sizePortion);
    ...
    VaultBaseInternal
{
    ...
    // When a gmx trigger order is executed and the collateral
    // token is Weth it automatically converts it to ETH
    // and sends it to the vault. Therefore we need to convert it.
    // The vault should never hold ETH.
    receive() external payable {
        // This if clause allows us to withdraw WETH to ETH to send in a call
        // We use this for refunding gmx execution fees
