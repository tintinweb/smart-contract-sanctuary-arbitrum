...
contract TVLComputer is Upgradeable, ITVLComputer {
    ...
    function setSyntheticTokens(address[] memory tokens) public onlyOwner {
        for (uint256 i = 0; i < tokens.length; i++) {
    ...
    function getGMXValue(address trade) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < positions.length; i++) {
            ...
            Price.Props memory indexPrice = Price.Props(getPrice(market.indexToken, 30), getPrice(market.indexToken, 30));
            Price.Props memory longPrice = Price.Props(getPrice(market.longToken, 30), getPrice(market.longToken, 30));
            Price.Props memory shortPrice = Price.Props(getPrice(market.shortToken, 30), getPrice(market.shortToken, 30));
            MarketUtils.MarketPrices memory prices = MarketUtils.MarketPrices(indexPrice, longPrice, shortPrice);
            (int256 pnl,,) = reader.getPositionPnlUsd(
                dataStore,
                market,
                prices,
                getPositionKey(trade, positions[i].addresses.market, positions[i].addresses.collateralToken, positions[i].flags.isLong),
                positions[i].numbers.sizeInUsd
            );
            int256 netValue = int256(convertToGMXUsd(positions[i].addresses.collateralToken, positions[i].numbers.collateralAmount))
                 + pnl / int256(10 ** (30 - registry.usdt().decimals()));
    ...
    function convertToBoolArray(bytes memory mask) public pure returns (bool[] memory) {
    ...
    function convertToUsdt(address token, uint256 amount) public view returns (uint256) {
        require(priceFeeds[token] != address(0), "TC/EF"); // empty feed
        uint256 tokenDecimals = IERC20MetadataUpgradeable(token).decimals();
        uint256 usdtDecimals = registry.usdt().decimals();
        if (token == usdc || token == address(registry.usdt())) {
            return amount * 10**(usdtDecimals) / 10**(tokenDecimals);
        }
        return amount * getPrice(token, usdtDecimals) / 10 ** (tokenDecimals);
    }

    function convertToGMXUsd(address token, uint256 amount) public view returns (uint256) {
        uint256 decimals = IERC20MetadataUpgradeable(token).decimals();
        if (token == usdc) {
            return amount * 10**30 / 10**(decimals);
        }
        return amount * getPrice(token, 30) / 10 ** (IERC20MetadataUpgradeable(token).decimals());
    }

    function getPrice(address token, uint256 decimals) public view returns (uint256) {
        require(priceFeeds[token] != address(0), "TC/EF"); // empty feed
