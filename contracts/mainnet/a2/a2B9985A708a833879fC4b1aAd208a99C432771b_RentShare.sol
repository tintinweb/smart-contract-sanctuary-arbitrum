...
library Roles {
    ...
    bytes32 public constant VAULT_PNL_NOTIFIER =
        keccak256("VAULT_PNL_NOTIFIER");
}
    ...
    ERC721Holder
{
    ...
    function getPropertyToGlobalEpoch(
        string memory tokenSymbol
    ) external view returns (uint) {
        uint poolId = rentShareStorageParams.symbolToPoolId[tokenSymbol];
        uint globalEpoch = rentShareStorageParams.poolIdToEpoch[poolId];
        return globalEpoch;
    }
    ...
    function withdraw(
        ...
        staker.epochToTokenBalance[epoch] = staker.lastEpoch != epoch && //ye true horhi hy
            staker.epochToTokenBalance[staker.lastEpoch] >= _amount // ye false horhi hy
            ? staker.epochToTokenBalance[staker.lastEpoch] - _amount // if true
            : staker.epochToTokenBalance[epoch] - _amount; // if false
        staker.epochToRentDebt[epoch] =
            (staker.epochToTokenBalance[epoch] * pool.accumulatedRentPerShare) /
            rentShareStorageParams.RENT_PRECISION;
    ...
    function cancelHarvestRewardsRequest(uint lockNftTokenId) external {
        ...
        //@todo i think we do not need it, it can be burned without the transfer
    ...
    function _isKycdAndNotSanctioned(
        ...
        // skip checks when addr is platform's address
        if (systemRegistry.whitelist().isAddressOnAllowList(addr)) return;

        // @todo may be here we need to do the stuff if it is protocol contract, then skip the kyc stuff
        require(
