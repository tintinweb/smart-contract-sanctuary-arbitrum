  ...
  */
  ...
  function oracleRequest(
    ...
    emit OracleRequest(
      ...
      payment,
      callbackAddress,
      // highjack `callbackFunctionId` since our custom nodes read that param from storage and
    ...
    returns (bool)
  {
    ...
    require(gasleft() >= MINIMUM_CONSUMER_GAS_LIMIT, "Must provide consumer enough gas");
    delete s_packedRequests[requestId];
   ...
   */
  function acceptOwnableContractsArr(address[] calldata ownable) public validateAuthorizedSenderSetter {
    for (uint256 i = 0; i < ownable.length; i++) {
  ...
  function acceptAuthorizedReceivers(address[] calldata targets, address[] calldata senders)
    ...
    validateAuthorizedSenderSetter
  {
    acceptOwnableContractsArr(targets);
   ...
   */
  function ownerForwardCall(address to, bytes calldata data) external onlyOwner validateNotToLINK(to) {
