...
import "./interfaces/ISmolWorld.sol";

contract TransferBlocker is SmolsAddressRegistryConsumer {

    address smolWorldAddress;
        ...
        returns (bool)
    {
        ...
        //If there is no set school address, skip.
        if (schoolAddress != address(0) && ISchool(schoolAddress).totalStatsJoinedWithinCollection(_collectionAddress, _tokenId) > 0) return false;

        //If there is no set smol world address, skip.
        if (smolWorldAddress != address(0) && ISmolsWorld(smolWorldAddress).isSmolCheckedIn(_tokenId)) return false;

        return true;
    }

    function setSmolWorldAddress(address _smolWorldAddress) external requiresEitherRole(OWNER_ROLE, ADMIN_ROLE) {
        smolWorldAddress = _smolWorldAddress;
    }
}
...
interface ISmolsWorld {
    function isSmolCheckedIn(uint256 tokenId) external view returns (bool);
}
...
contract AccessControlEnumerableV3 is AccessControlEnumerable, Roles {
