...
contract Orbitrum is ERC20, Ownable { 
    ...
    event UpdateFees(uint256 sellMarketingFees, uint256 buyMarketingFees,
                     uint256 buyNftFee, uint256 sellNftFee, uint256 buyLiquidityFee, uint256 sellLiquidityFee, 
                     uint256 buyStakingFee, uint256 sellStakingFee);
    ...
    constructor() ERC20("OrbTest", "OTest") { 
        ...
        stakingWallet = payable(0x7299336E094dd0f5a74f6bdCbfE7fECc401b81C4); 
        ...
        totalBuyFees = buyMarketingFees.add(buyNftFee).add(buyLiquidityFee).add(buyStakingFee);
        totalSellFees = sellMarketingFees.add(sellNftFee).add(sellLiquidityFee).add(sellStakingFee);
        ...
        maxWallet = 1_000_000_000 * (10**18); // 1%
    ...
    function setTrueDefiRouter(address _router, uint256 setmaxgwei, uint256 setdeadblocks) external onlyOwner {
        tefiRouter = _router;
    ...
    function updateFees(uint256 marketingBuy, uint256 marketingSell, uint256 nftBuy, uint256 nftSell,
                        uint256 liquidityBuy, uint256 liquiditySell, uint256 stakingBuy, uint256 stakingSell) public onlyOwner {
        ...
        emit UpdateFees(sellMarketingFees, sellNftFee, sellStakingFee, sellLiquidityFee, 
                        buyMarketingFees, buyNftFee, buyStakingFee, buyLiquidityFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (to == DEAD) {
            ...
            if (!isBuying && !isSelling) {
                ...
                require(tx.gasprice <= gasPriceLimit,"No FrontRunning");
            ...
            uint256 totalFees = marketingFees.add(nftFee).add(liquidityFee).add(stakingFee);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 _completeFees = sellMarketingFees.add(sellNftFee).add(sellStakingFee) + buyMarketingFees.add(buyNftFee).add(buyStakingFee);
        ...
        uint256 stakingPayout = buyStakingFee.add(sellStakingFee) * feePortions;
        ...
        if (stakingPayout > 0) {
            (successOp2, ) = address(stakingWallet).call{value: stakingPayout}("");
        }

        emit SendDividends(
            marketingPayout + nftPayout + stakingPayout,
