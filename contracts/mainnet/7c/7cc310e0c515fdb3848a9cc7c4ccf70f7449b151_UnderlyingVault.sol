...
import {ISequencer} from "src/interfaces/vaults/ISequencer.sol";
...
contract UnderlyingVault is IUnderlyingVault, ERC4626Upgradeable, UpgradeableOperableKeepable {
    ...
    /// @notice Amount loaned to strategies
    mapping(address => uint256) public loaned;

    /// @notice Cap of assets per strategy
    mapping(address => uint256) public cap;

    /// @notice Total Loaned
    uint256 public totalLoaned;

    /// @notice Contract that manage loans paybacks
    ...
    function initialize(address _asset, address _enforceHub, string memory _name, string memory _symbol)
        external
        initializer
    {
        if (msg.sender != gov) {
    ...
    /**
     * @dev Balance of shares (jUSDC) for users.
     */
    ...
    /**
     * @dev Calculate assets to be received if redeem shares.
     */
    ...
    /**
     * @dev Calculate shares to be received if deposit assets.
     */
    ...
    /**
     * @dev Calculate the strategies retention for for payback the debt.
     */
    ...
    /**
     * @dev Calculate the borrowable amount of some strategy.
     */
    function borrowableAmount(address strategy) external view returns (uint256) {
        uint256 _cap = cap[strategy].mulDivDown(totalAssets(), BASIS_POINTS);
        uint256 totalDebt = loaned[strategy];
        uint256 totalAvailable = _cap > totalDebt ? _cap - totalDebt : 0;
        uint256 vaultAmount = underlying.balanceOf(address(this));
        if (vaultAmount > totalAvailable) {
    ...
    /**
     * @notice Receive rewards from strategies.
     * @param amount amount to be transfered here.
     */
    function receiveRewards(uint256 amount) external {
        ...
        emit ReceiveRewards(msg.sender, amount, totalAssets(), totalSupply());
    }

    /**
     * @notice Lend udnerlying asset to strategies.
     * @param amount amount to be lend.
     */
    function borrow(uint256 amount) external onlyStrategy {
        uint256 totalDebt = loaned[msg.sender] + amount;
        _checkCap(msg.sender, totalDebt);
        loaned[msg.sender] = totalDebt;
        totalLoaned = totalLoaned + amount;

        underlying.transfer(msg.sender, amount);

        emit Borrowed(msg.sender, amount, totalLoaned);
    }

    /**
     * @notice Strategies payback amount borrowed.
     * @param amount amount to payback.
     * @param incentives cost of the payback. Have to be paid by the user.
     */
    function payBack(uint256 amount, uint256 incentives) external onlyStrategy {
        underlying.transferFrom(msg.sender, address(this), amount - incentives);
        uint256 debt = loaned[msg.sender];
        if (amount < debt) {
            loaned[msg.sender] = debt - amount;
        } else {
            loaned[msg.sender] = 0;
        }
        uint256 totalDebt = totalLoaned;
        if (amount < totalDebt) {
            totalLoaned = totalDebt - amount;
        } else {
            totalLoaned = 0;
        }

        emit PayBack(msg.sender, amount, incentives, totalLoaned);
    }
    ...
    /**
     * @notice Enforce debt payback.
     * @param amount amount to payback.
     * @param enforceData The data needed to enforce payback.
     */
    function enforcePayBack(uint256 amount, bytes memory enforceData) external onlyOperatorOrKeeper returns (uint256) {
        ...
        if (totalAssets_ < _totalAssets + amount - retention) {
        ...
        emit EnforcePayback(amount, retention, totalAssets_, totalLoaned);
     ...
     */
    function updateDebt(address strategy, uint256 amount, bool substract) external onlyGovernor {
        if (substract) {
            loaned[strategy] = loaned[strategy] > amount ? loaned[strategy] - amount : 0;
            totalLoaned = totalLoaned > amount ? totalLoaned - amount : 0;
        } else {
            loaned[strategy] = loaned[strategy] + amount;
            totalLoaned = totalLoaned + amount;
        }
    }
     ...
     */
    function addStrategy(address _newOperator, uint256 _cap) external onlyGovernor {
        _grantRole(STRATEGY, _newOperator);
        cap[_newOperator] = _cap;
     ...
     * @notice Grant Operator role to _newOperator.
     */
    function removeStrategy(address _operator) external onlyGovernor {
        _revokeRole(STRATEGY, _operator);
        cap[_operator] = 0;
    ...
    function _checkCap(address _strategy, uint256 _loaned) private view {
        uint256 _cap = cap[_strategy].mulDivDown(totalAssets(), BASIS_POINTS);
        if (_loaned > _cap) {
            revert CapReached();
        }
    }
