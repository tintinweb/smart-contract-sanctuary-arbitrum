...
// Coinflip contract
contract BruhCoinFlip is Ownable, ReentrancyGuard {
    struct UserInfo {
        address user;
        uint256 userBet;
        uint256 time;
    ...
    uint256 public callbackGasLimit = 250000;
    uint256 public minBet = 100_000_000 * 1e6;
    uint256 public rewardPool;
    uint256 public rewardPoolDenominator = 100; //1% from reward pool supply
    uint256 public refundDelay = 10;
    uint256 public minOracleBalance = 5 * 1e15; //0.005 eth
    ...
    event SettingsUpdated(uint256 newDenominator, uint256 newMinBet, uint256 newDelay, uint256 newMinOracleBalance);
    ...
    function flip(uint256 userBet, bool isTail) external payable returns (uint256) {
        ...
        uint256 oracleBalance = randomizer.clientBalanceOf(address(this));
        ...
        if (oracleBalance <= minOracleBalance && address(this).balance > 0) {
            randomizer.clientDeposit{value: address(this).balance}(address(this));
        }
    ...
    function changeSettings(uint256 newDenominator, uint256 newMinBet, uint256 newDelay, uint256 newMinOracleBalance) external onlyOwner {
        require(newDenominator > 0, "invalid new denominator");
        require(newDelay > 0, "invalid new delay");
        require(newMinOracleBalance > 0, "invalid new min oracle balance");

        rewardPoolDenominator = newDenominator;

        require(newMinBet > 0 && newMinBet < maxBet(), "invalid min bet");

        minBet = newMinBet;
        refundDelay = newDelay;
        minOracleBalance = newMinOracleBalance;

        emit SettingsUpdated(newDenominator, newMinBet, newDelay, newMinOracleBalance);
    }
