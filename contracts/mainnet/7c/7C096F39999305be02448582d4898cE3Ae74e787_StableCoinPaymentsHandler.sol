...
contract StableCoinPaymentsHandler is AccessControl, ReentrancyGuard, Pausable {
    ...
    event TokenPurchasedSuccessfully(
        address indexed user,
        address indexed token,
        address indexed rec,
        uint256 amount,
        IDataSalePoint.Type option,
        uint256 sold,
        uint256 turn
    );
    ...
    error StableCoinUndefined();

    constructor(address payable dataSalePoint_, address[] memory tokens_) {
        if (dataSalePoint_ == address(0)) {
            revert ZeroAddress();
        }

        for (uint256 index = 0; index < tokens_.length; index++) {
            if (tokens_[index] == address(0)) revert ZeroAddress();
            _stableCoin[tokens_[index]] = StableCoin({defined: true, totalLiquidity: 0});
        }
    ...
    function pay(
        address token_,
        uint256 amount_,
        IDataSalePoint.Type option_,
        address rec_
    ) external nonReentrant {
        _pay(token_, amount_, option_, _msgSender(), rec_, false);
    }
    ...
    function payFor(
        ...
        _pay(token_, amount_, option_, user_, rec_, true);
    }
    ...
    function isStableCoin(address token_) external view returns (bool) {
        return _stableCoin[token_].defined;
    }
    ...
    function getTotalLiquidity(address token_) external view returns (uint256) {
        return _stableCoin[token_].totalLiquidity;
    }
    ...
    function _pay(
        ...
        if (!_stableCoin[token_].defined) {
            revert StableCoinUndefined();
        }
        ...
        if (turn.supply < turn.sold + _getLiquidity(token_, amount_, option_)) {
        ...
        (address rec, uint256 fStableCoinFunds, uint256 sStableCoinFunds) = _getRec(
            user_,
            token_,
            rec_,
            option_,
        ...
        _purchase(_msgSender(), token_, amount_, fStableCoinFunds);

        _stableCoin[token_].totalLiquidity = _stableCoin[token_].totalLiquidity + amount_;
        uint256 liquidity = _getLiquidity(token_, amount_, option_);

        _dataSalePoint.setTurnStatus(user_, token_, funds, liquidity, rec, fStableCoinFunds, sStableCoinFunds);

        emit TokenPurchasedSuccessfully(
            user_,
            token_,
            rec,
            amount_,
    ...
    function _purchase(address user_, address token_, uint256 amount_, uint256 reward_) internal {
        ...
        if (reward_ > 0) {
            IERC20(token_).safeTransferFrom(user_, address(_dataSalePoint), reward_);
        }
    }
    ...
    function _getRec(
        ...
        (uint256 fReward_, uint256 secondaryReward_) = _dataSalePoint.getRecRates(rec);
        uint256 fStableCoinFunds = (amount_ * fReward_) / 1000;
        uint256 sStableCoinFunds = (amount_ * secondaryReward_) / 1000;
        uint256 liquidity = _getLiquidity(token_, sStableCoinFunds, option_);

        return (rec, fStableCoinFunds, liquidity);
    }
    ...
    function _getLiquidity(
        ...
        return ((amount_ * 10 ** ETH_DECIMALS * NUMERATOR) / 10 ** decimals) / _dataSalePoint.getPrice(option_);
    }
}
