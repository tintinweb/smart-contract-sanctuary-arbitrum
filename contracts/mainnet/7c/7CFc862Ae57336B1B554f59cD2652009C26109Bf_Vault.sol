...
contract Vault is Initializable, ReentrancyGuardUpgradeable, IVault {
    ...
    // usdgAmounts tracks the amount of USDhLP debt for each whitelisted token
    mapping(address => uint256) public override usdgAmounts;

    // maxUsdgAmounts allows setting a max amount of USDhLP debt for a token
    ...
    // this value is used to calculate the redemption values for selling of USDhLP
    ...
    event UpdateMaxLeverage(uint256 maxLeverage);
    event UpdateFees(
    ...
    function initialize() public initializer {
        ...
        // Set initial values for the parameters due to proxy.
        ...
        emit UpdateMaxLeverage(maxLeverage);
        emit UpdateFees(
            taxBasisPoints,
    ...
    function clearTokenConfig(address _token) external {
        ...
        emit UpdateTokenConfig(_token, false, 0, 0, 0, 0, false, false);
    }
    ...
    // deposit into the pool without minting USDhLP tokens
    ...
    function liquidatePosition(
        ...
        // Check private liquidation mode and validate msg.sender.
        if (inPrivateLiquidationMode) _validate(isLiquidator[msg.sender], 34);
        // Apply H2SO signed quote.
        IVaultPriceFeed(priceFeed).h2sofaApplySignedQuote(signedQuoteData);
    ...
    function getTargetUsdgAmount(address _token)
        ...
        returns (uint256)
    {
        uint256 supply = IERC20Upgradeable(usdg).totalSupply();
        if (supply == 0) {
            return 0;
        }
    ...
    function _decreaseUsdgAmount(address _token, uint256 _amount) private {
        ...
        // since USDhLP can be minted using multiple assets
        // it is possible for the USDhLP debt for a single asset to be less than zero
        // the USDhLP debt is capped to zero for this case
        if (value <= _amount) {
