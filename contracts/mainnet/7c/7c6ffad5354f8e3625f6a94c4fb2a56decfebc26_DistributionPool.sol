...
contract DistributionPool is OwnableUpgradeable {
    ...
    //uint256 public constant MAX_TOKEN = 210_000_000_000_000_000 * 1e18;
    //uint256 public constant INIT_CLAIM = 1_292_000_000_000 * 1e18;
    uint256 public constant MAX_TOKEN = 5 * 10 ** 15 * 1e9;
    uint256 public constant INIT_CLAIM =  30_761_904_761 * 1e9;

    uint256 public antiSybil = 0.0003 ether;
    ...
    event Claim(address indexed user, uint256 amount, address referrer, uint timestamp);
    ...
    function canClaimAmount() public view returns(uint256) {
        ...
        uint256 claimedPercent = curClaimedCount * 100e18 / MAX_ADDRESSES;
        uint256 curPercent = 5e18;
    ...
    function claim(bytes calldata signature, address referrer) public payable {
        require(_claimedUser[_msgSender()] == false, "Already claimed");
        require(msg.value >= antiSybil, "Invalid sybil fee!");

        _claimedUser[_msgSender()] = true;

        require(isSigner[recoverSigner(generateSignature(_msgSender()), signature)], "Invalid signature");
    ...
    function setSignerStatus(address account, bool newValue) public onlyOwner {
        isSigner[account] = newValue;
    }

    function generateSignature(address _user) public view returns (bytes32 sig) {
        sig = keccak256(abi.encodePacked(address(this), _user));
    }
    ...
    function recoverSigner(
        ...
        return ecrecover(toEthSignedMessageHash(_MessageHash), v, r, s);
    }
    ...
    function setSybilThreshold(uint256 _newSybilFee) external onlyOwner {
        antiSybil = _newSybilFee;
    }
}