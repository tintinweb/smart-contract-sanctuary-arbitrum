/**
 *Submitted for verification at Arbiscan on 2022-07-06
*/
...
// File: contracts/strategies/StabilizeStrategyFlashImpermaxWETHV3.sol
...
// It will attempt to put all the asset into the highest earning vaults. Also includes flash loan features

// Strategy updated to not affect small pool rates as much
...
contract StabilizeStrategyFlashImpermaxWETHV3 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public lastBestVaultPureAPRIndex = 0; // Vault that currently has most of the strategy liquidity / highest apr
    uint256 public lastBestVaultLiquidityAPRIndex = 0; // Vault that currently has the best liquidity x apr combo
    uint256 public lastVaultCurveAverageAPR = 0; // Curve average of all the vaults APRs
    uint256 public vaultAPRCurveFactor = 50000; // Average is skewed to the right by 50% (0% = no skew, 100% = average apr = top apr)
    ...
    function totalTokenInVault(uint256 _vaultID) public view returns (uint256) {
        require(_vaultID < VaultList.length, "ID not valid");
        address mintableToken = VaultList[_vaultID].mintableToken;
    ...
    function totalLockedValue() public view returns (uint256) {
        ...
        for(uint256 i = 0; i < _length; i++){
            tokenEquivalent = tokenEquivalent.add(totalTokenInVault(i));
        }
    ...
    function totalStrategyLiquidity() public view returns (uint256) {
        ...
        for(uint256 i = 0; i < _length; i++){
            ...
            uint256 stratToken = totalTokenInVault(i);
            if(stratToken < liq){
                liq = stratToken;
            }
    ...
    */
    ...
    function rebalance(address _executor) internal {
        ...
        // This function will determine the highest earning pool, highest apr x liquidty combo and move available liquidity into that pool
        uint256 _length = VaultList.length;
        uint256 _bestAPR = 0;
        uint256 _bestAPRIndex = 0;
        uint256 _bestCombo = 0;
        uint256 _bestComboIndex = 0;
        uint256 _aprAverage = 0;
        ...
        for(uint256 i = 0; i < _length; i++){
            ...
            // Update apr average
            _aprAverage = _aprAverage.add(change);
            VaultList[i].vaultAPR = change;

            // Check the best combo
            change = change.mul(mintToken.totalBalance()); // Get the combination of rate of change along with liquidity
            if(change > _bestCombo){
                _bestCombo = change;
                _bestComboIndex = i;
            }
        }

        if(_bestAPR == 0 || _aprAverage == 0){
            // There was no change
            return;
        }
        ...
        lastBestVaultPureAPRIndex = _bestAPRIndex;
        lastBestVaultLiquidityAPRIndex = _bestComboIndex;
        _aprAverage = _aprAverage.div(_length);
        if(vaultAPRCurveFactor == DIVISION_FACTOR){
            _aprAverage = _bestAPR;
        }else if(vaultAPRCurveFactor > 0){
            uint256 dist = _bestAPR.sub(_aprAverage);
            _aprAverage = dist.mul(vaultAPRCurveFactor).div(DIVISION_FACTOR).add(_aprAverage);
        }
    ...
    function depositIntoVaults(bool resetInterest) internal {
        ...
        tokenList[0].token.safeApprove(IMPERMAX_ROUTER, _bal); // Approve the entire balance to be moved to the router once
        
        uint256 maxDeposit = ImpermaxToken(VaultList[lastBestVaultPureAPRIndex].mintableToken).totalBalance(); // Max to put into the first pool
        uint256 depositAmount = _bal;
        if(depositAmount > maxDeposit){
            depositAmount = maxDeposit;
        }

        if(depositAmount > 0){
            router.mint(VaultList[lastBestVaultPureAPRIndex].mintableToken, depositAmount, address(this), now.add(60));
        }

        // Then split the rest among vaults that are greater than the average APR
        for(uint256 i = 0; i < _length; i++){
            if(i == lastBestVaultPureAPRIndex || i == lastBestVaultLiquidityAPRIndex){continue;} // Skip these vaults for now
            if(VaultList[i].vaultAPR < lastVaultCurveAverageAPR){continue;} // Skip vaults that have lower APR than average
            _bal = tokenList[0].token.balanceOf(address(this));
            if(_bal == 0){break;}
            depositAmount = _bal;
            maxDeposit = ImpermaxToken(VaultList[i].mintableToken).totalBalance();
            if(depositAmount > maxDeposit){
                depositAmount = maxDeposit;
            }
            if(depositAmount == 0){continue;}
            router.mint(VaultList[i].mintableToken, depositAmount, address(this), now.add(60));
        }

        // Finally put the funds into the pool that has the highest combo, irrespective of liquidity of pool
        _bal = tokenList[0].token.balanceOf(address(this));
        if(_bal > 0){
    ...
    function governanceChangeCurveFactor(uint256 _factor) external onlyGovernance {
        require(_factor <= DIVISION_FACTOR, "Outside of range");
        vaultAPRCurveFactor = _factor;
    }
