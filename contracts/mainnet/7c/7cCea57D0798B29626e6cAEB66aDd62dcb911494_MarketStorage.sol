...
contract MarketStorage is AuthorizedAccessControl, IMarketStorage {

  using SafeCast for uint256;
  
  IMarketStorage.MarketState private _marketState;

  IMarketStorage.Liability private _marketLiability;

  mapping (address => IMarketStorage.UserState) private _userState; 

  mapping (address => IMarketStorage.Liability) private _userLiabilities; 

  constructor(IAddressRegistry registry) AdminAccessControl(registry) {
    _marketState.borrowIndex = 1e18;
    _marketState.discountIndex = 1e18;
    _marketState.lastUpdate = block.timestamp.toUint32();
  }
  ...
  function setMarketState(MarketState calldata marketState) external onlyAuthorized {
  ...
  function setMarketLiability(Liability calldata marketLiability) external onlyAuthorized {
    _marketLiability = marketLiability;
  }

  function getUserState(address account) external view returns (UserState memory) {
    return _userState[account];
  }

  function setUserState(address account, UserState calldata userState) external onlyAuthorized {
    _userState[account] = userState;
  }

  function getUserLiability(address account) external view returns (Liability memory) {
    return _userLiabilities[account];
  }

  function setUserLiability(address account, Liability calldata userLiability) external onlyAuthorized {
    _userLiabilities[account] = userLiability;
  }
}
