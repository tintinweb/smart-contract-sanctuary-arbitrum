...
contract Claimer is IRewardTracker, OwnableUpgradeable, ReentrancyGuardUpgradeable, ImplementationGuard {
    ...
    function refund(uint256 id, uint256 amount) external onlyOwner {
        if (id == 0) {
            require(block.timestamp >= endTime, "NotEnd");
            require(amount <= 16366 * 1e18 - totalClaimed - refunded[0], "Exceeded");
        } else {
            Round storage round = rounds[id];
            require(block.timestamp >= round.endTime, "NotEnd");
            require(amount <= round.totalAmount - round.totalClaimed - refunded[id], "Exceeded");
        }
        refunded[id] += amount;
        IERC20Upgradeable(mcbToken).transfer(msg.sender, amount);
        emit Refund(id, amount);
    }
    ...
    function averageStakedAmounts(address) external pure virtual override returns (uint256) {
        return 0;
    }

    function cumulativeRewards(address _account) external view virtual override returns (uint256) {
    ...
    function claim(uint256 id, uint256 amount, bytes calldata signature) external nonReentrant {
        if (id == 0) {
            ...
            require(round.totalClaimed + amount <= round.totalAmount, "Exceeded");
