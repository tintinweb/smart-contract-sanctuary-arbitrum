...
import "./interfaces/IVodkaVault.sol";
...
contract Water is ERC4626Upgradeable, OwnableUpgradeable {
    ...
    address public vodkaVault; // vodka Vault address
    ...
    modifier onlyVodkaVault() {
        require(msg.sender == vodkaVault, "Not vodka vault");
        _;
    }
    ...
    event VodkaVaultChanged(address newVodkaVault);
    ...
    function initialize(address _USDC) external initializer {
        ...
        __ERC20_init("VodkaV1-WATER", "V1-WATER");
    }
    ...
    function getUtilizationRate() public view returns (uint256) {
        uint256 totalWaterDebt = totalDebt;
        return totalWaterDebt == 0 ? 0 : totalWaterDebt.mulDiv(1e18, totalUSDC + totalWaterDebt);
    }
    ...
    function setVodkaVault(address _vodka) external onlyOwner zeroAddress(_vodka) {
        vodkaVault = _vodka;
        emit VodkaVaultChanged(_vodka);
    }
    ...
    /**
    * @notice Allow the Vodka Vault to lend a certain amount of USDC to the protocol.
    * @dev The function allows the Vodka Vault to lend a certain amount of USDC to the protocol. It updates the total debt and total USDC balances accordingly.
    * @param _borrowed The amount of USDC to lend.
    * @return status A boolean indicating the success of the lending operation.
    */
    function lend(uint256 _borrowed) external onlyVodkaVault returns (bool status) {
    ...
    /**
     * @notice Allows the Vodka Vault to repay debt to the protocol.
     * @dev The function allows the Vodka Vault to repay a certain amount of debt to the protocol. It updates the total debt and total USDC balances accordingly.
     ...
     */
    function repayDebt(uint256 _debtAmount, uint256 _amountPaid) external onlyVodkaVault returns (bool) {
