...
contract SingleStakingV2 is ISingleStaking, OwnableUpgradeable {
    ...
    function batchUpdateBoostMultiplier(address[] memory _users) external onlyOwner {
        uint256 userLength = _users.length;
        uint256[] memory pids = new uint256[](8);
        pids[0] = 0;
        pids[1] = 1;
        pids[2] = 2;
        pids[3] = 4;
        ...
        unchecked {
            for (uint256 i; i < userLength; ++i) {
                ...
                for (uint256 j; j < 8; ++j) {
                    // There are 10 valid PIDs as per the hard-coded array
                    uint256 pid = pids[j];
                    if (userInfo[pid][user].amount > 0) {
                        updateBoostMultiplier(user, pid);
                    }
                }
            }
        }
    ...
    function unstakeAndLiquidate(uint256 _pid, address _user, uint256 _amount) public onlyLeverageVault {
        ...
        uint256 boostedAmount = (_amount * multiplier) / BOOST_PRECISION;
