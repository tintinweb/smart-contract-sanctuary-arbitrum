    ...
    IStrategyVault
{
    ...
    function withdraw(uint256 _shares) public whenNotPaused onlyEOA{
        ...
        if(poolExists){
            if(rewards.balanceOf(address(this), msg.sender, false, address(this)) > 0){
                try rewards.withdraw(address(this), _shares, msg.sender, false, address(this)){} catch {}
            }    
        }    
        _withdraw(_shares);
    }
    ...
    function withdrawAll() external whenNotPaused onlyEOA{
        ...
        if(poolExists){
            ...
            if(balanceInRewards > 0){
                try rewards.withdraw(address(this), balanceInRewards, msg.sender, false, address(this)){} catch {}
            }  
        }               
