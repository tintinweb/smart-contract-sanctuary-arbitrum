...
import {DodoV1DepositBase} from "../templates/DodoV1DepositBase.sol";

contract DodoV1ArbitrumUSDT is DodoV1DepositBase {
    constructor()
        DodoV1DepositBase(
...
import {LogicWithUtils} from "./LogicWithUtils.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";


contract DodoV1DepositBase is LogicWithUtils {
    IDodoV1Pool public immutable POOL;
    IDodoMining public immutable MINING;
    IERC20 public immutable BASE_TOKEN;
    IERC20 public immutable BASE_LP;
    ...
    constructor(address pool, address mining, address rewardToken) {
        ...
        BASE_TOKEN = IERC20(IDodoV1Pool(POOL)._BASE_TOKEN_());
        BASE_LP = IERC20(IDodoV1Pool(POOL)._BASE_CAPITAL_TOKEN_());
        DODO_TOKEN = IERC20(rewardToken);
    }

    function enter() external payable override {
        uint256 quoteAmount = BASE_TOKEN.balanceOf(address(this));
        uint256 lpAmount;
        if (quoteAmount > 0) {
            _approveIfNeeded(address(BASE_TOKEN), address(POOL));
            lpAmount = POOL.depositBase(quoteAmount);
        }
        _approveIfNeeded(address(BASE_LP), address(MINING));
        MINING.deposit(address(BASE_LP), lpAmount);
    }
    function exit(uint256 liquidity) public payable override {
        MINING.withdraw(address(BASE_LP), liquidity);
    ...
    function emergencyExit() public payable override {
    ...
    function withdrawFunds(
        ...
        _transferAll(address(BASE_TOKEN), recipient);
    }

    function accountLiquidity(
        address account
    ) public view override returns (uint256) {
        return MINING.getUserLpBalance(address(BASE_LP), account);
    }
}
...
interface IDodoV1Pool is IERC20 {
    ...
    function withdrawAllBase() external;
}
...
abstract contract LogicWithUtils is Logic {
