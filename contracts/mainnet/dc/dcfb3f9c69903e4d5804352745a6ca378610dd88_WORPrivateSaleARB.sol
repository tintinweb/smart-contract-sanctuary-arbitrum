...
contract WORPrivateSaleARB is ERC20, Ownable, ReentrancyGuard {
    ...
    address public addressUSDC      = 0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8;
    ...
    address public addressWOR       = 0x02747A5Bb6D6940D8fbC7F36aFeFBc1D284BF698;
    ...
    function maxUSDbuy() public view returns (uint256) {
        return convert(addressWETH, addressUSDC, maxARBbuy);
    }

    function minUSDbuy() public view returns (uint256) {
        return convert(addressWETH, addressUSDC, minARBbuy);
    }
    ...
    function getLimitToBuy_USD(address buyer) public view returns (uint256 limit) {
        ...
        uint256 maxARBbuyConverted = convert(addressWETH, addressUSDC, maxARBbuy);
        uint256 hardCapPrivateSaleConverted = convert(addressWETH, addressUSDC, hardCapPrivateSale);
        uint256 totalSoldInARBConverted = convert(addressWETH, addressUSDC, totalSoldInARB);
        //It is redundant and unnecessary to check, but we do these rechecks
    ...
    //Backend calls this function when there is a BNB purchase on the BSC network
    //BNB paid at BSC are deposited into a BSC account
    //This function is called passing on the value of amountARB that the quote in ARB of the paid BNB
    function buyPrivateSale(uint256 amountARB) 
    ...
    function buyPrivateSaleByUSDC(uint256 amountUSDC)
        ...
        uint256 amountARBconverted = convert(addressUSDC, addressWETH, amountUSDC);
