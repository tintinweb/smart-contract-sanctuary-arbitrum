...
import "../../utils/OpenMath.sol";
import "../../libraries/SafeToken.sol";
import "../../apis/pancake/IPancakeRouter02.sol";
import "../../apis/sushiswap/IMiniChefV2.sol";

/// @title SushiswapWorker is a reinvest-optimized Worker which deposit LP into MasterChefV2 introduced in the PancakeSwap's MasterChefV2 migration
contract SushiswapWorker is
    OwnableUpgradeSafe,
    ...
    IWorker02
{
    ...
    /// @notice Upgraded State Variables for SushiswapWorker
    ...
    function initialize(
        ...
        sushi = address(masterChefV2.SUSHI());
        ...
        require(
            (farmingToken == lpToken.token0() ||
                farmingToken == lpToken.token1()) &&
                (baseToken == lpToken.token0() ||
                    baseToken == lpToken.token1()),
    ...
    modifier onlyOperator() {
        require(
            msg.sender == operator,
            "SushiswapWorker::onlyOperator:: not operator"
        );
        _;
    }
    ...
    modifier onlyReinvestor() {
        require(
            ...
            "SushiswapWorker::onlyReinvestor:: not reinvestor"
        );
        _;
    }
    ...
    /// @param _reinvestThreshold - The threshold to be reinvested if pendingSushi pass over.
    function _reinvest(
        address _treasuryAccount,
        ...
        if (bounty > 0) {
            uint256 beneficialVaultBounty = bounty.mul(
    ...
    function work(
        ...
        require(
            okStrats[strat],
            "SushiswapWorker::work:: unapproved work strategy"
        );
        require(
    ...
    function getMktSellAmount(
        ...
        require(
            rIn > 0 && rOut > 0,
            "SushiswapWorker::getMktSellAmount:: bad reserve values"
        );
    ...
    function health(uint256 id) external view override returns (uint256) {
        ...
        (uint256 totalBaseToken, uint256 totalFarmingToken) = lpToken
            .token0() == baseToken
            ? (r0, r1)
            : (r1, r0);
        // 3. Convert the position's LP tokens to the underlying assets.
        uint256 userBaseToken = lpBalance.mul(totalBaseToken).div(lpSupply);
        uint256 userFarmingToken = lpBalance.mul(totalFarmingToken).div(
            lpSupply
        );
    ...
    /// @param _beneficialVaultBounty - The amount of SUSHI to be swapped to BTOKEN & send back to the Vault.
    /// @param _callerBalance - The balance that is owned by the msg.sender within the execution scope.
    function _rewardToBeneficialVault(
        ...
        if (beneficialVaultToken != baseToken) {
            ...
            buybackAmount = beneficialVaultToken.myBalance().sub(
                _callerBalance
            );
        }
    }
    ...
    function actualBaseTokenBalance() internal view returns (uint256) {
        if (baseToken == sushi) {
            return baseToken.myBalance().sub(pendingSushi).sub(buybackAmount);
        }
    ...
    function _addShare(uint256 id) internal {
        ...
        if (balance > 0) {
            ...
            // 3. Deposit balance to SushiMiniChefV2 and update pendingSushi
    ...
    function setReinvestConfig(
        ...
        require(
            ...
            "SushiswapWorker::setReinvestConfig:: _reinvestBountyBps exceeded maxReinvestBountyBps"
        );
        if (baseToken == sushi) {
            require(
                _reinvestPath.length == 1,
                "SushiswapWorker:: _reinvestPath length must == 1, if baseToken == reward"
            );
        } else {
            require(
                _reinvestPath.length >= 2,
                "SushiswapWorker::setReinvestConfig:: _reinvestPath length must >= 2"
            );
        }
        require(
            _reinvestPath[0] == sushi &&
                _reinvestPath[_reinvestPath.length - 1] == baseToken,
            "SushiswapWorker::setReinvestConfig:: _reinvestPath must start with SUSHI, end with BTOKEN"
        );
    ...
    function setMaxReinvestBountyBps(
        ...
        require(
            ...
            "SushiswapWorker::setMaxReinvestBountyBps:: _maxReinvestBountyBps exceeded 30%"
        );
    ...
    function setRewardPath(address[] calldata _rewardPath) external onlyOwner {
        require(
            ...
            "SushiswapWorker::setRewardPath:: rewardPath length must be >= 2"
        );
        require(
            _rewardPath[0] == sushi &&
                _rewardPath[_rewardPath.length - 1] == beneficialVault.token(),
            "SushiswapWorker::setRewardPath:: rewardPath must start with SUSHI and end with beneficialVault token"
        );
    ...
    function setCriticalStrategies(
        IStrategy _addStrat,
        IStrategy _liqStrat
    ) external onlyOwner {
        addStrat = _addStrat;
    ...
    function setTreasuryConfig(
        ...
        require(
            ...
            "SushiswapWorker::setTreasuryConfig:: _treasuryBountyBps exceeded maxReinvestBountyBps"
        );
    ...
    function setBeneficialVaultConfig(
        ...
        require(
            ...
            "SushiswapWorker::setBeneficialVaultConfig:: _beneficialVaultBountyBps exceeds 100%"
        );
        require(
            _rewardPath.length >= 2,
            "SushiswapWorker::setBeneficialVaultConfig:: rewardPath length must >= 2"
        );
        require(
            _rewardPath[0] == sushi &&
                _rewardPath[_rewardPath.length - 1] == _beneficialVault.token(),
            "SushiswapWorker::setBeneficialVaultConfig:: rewardPath must start with SUSHI, end with beneficialVault token"
        );
    ...
    /// @dev Deposit lp to masterChef and update pendingSushi
    /// @param _balance - Lp amount to deposit.
    function _masterChefV2Deposit(uint256 _balance) internal {
        uint256 _sushiBefore = sushi.balanceOf(address(this));
        masterChefV2.deposit(pid, _balance, address(this));
        pendingSushi = pendingSushi.add(
            sushi.balanceOf(address(this)).sub(_sushiBefore)
        );
    }

    /// @dev Withdraw lp from masterChef and update pendingSushi
    /// @param _balance - Lp amount to withdraw.
    function _masterChefV2Withdraw(uint256 _balance) internal {
        uint256 _sushiBefore = sushi.balanceOf(address(this));
        masterChefV2.withdrawAndHarvest(pid, _balance, address(this));
        pendingSushi = pendingSushi.add(
            sushi.balanceOf(address(this)).sub(_sushiBefore)
        );
    }
