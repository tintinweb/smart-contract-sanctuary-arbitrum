...
contract TEST is ERC20, Ownable {
    ...
    function _manualTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!canAddLiquidityBeforeLaunch[sender]) {
            require(launched(), "TEST: Trading not open yet");
        }
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'TEST: ETH_TRANSFER_FAILED');
    }
    ...
    function setBuyFees(
        ...
        devFeeBuy2 = _devFee2;
        burnFeeBuy = _burnFee;
        totalFeeBuy = _liquidityFee  + _bonusFee + _devFee1 + _devFee2 + _burnFee;
    }
    ...
    function setSellFees(
        ...
        totalFeeSell = _liquidityFee  + _bonusFee + _devFee1 + _devFee2 + _burnFee;
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "TEST: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "TEST: index out of bounds");
