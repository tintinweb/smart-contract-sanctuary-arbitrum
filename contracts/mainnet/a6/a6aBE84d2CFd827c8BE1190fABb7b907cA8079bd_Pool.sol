...
contract Pool is Roles {
    ...
    function link() external onlyGov {
        ...
        poolRewards = PoolRewards(DS.getAddress("PoolRewards"));
    }
    ...
    function getDepositTaxBps(address asset, uint256 amount) public view returns (uint256) {
        ...
        if (globalUPLs[asset] - int256(bufferBalance) < 0) {
            taxBps = uint256(
                (int256(BPS_DIVIDER) * (int256(bufferBalance) - globalUPLs[asset])) / (int256(balance) + int256(amount))
            );
        }
        return taxBps;
    }
    ...
    function getWithdrawalTaxBps(address asset, uint256 amount) public view returns (uint256) {
        ...
        uint256 bufferBalance = poolStore.getBufferBalance(asset); // TODO : check overflow on casting
        if (globalUPLs[asset] - int256(bufferBalance) > 0) {
            taxBps = uint256(
                (int256(BPS_DIVIDER) * (globalUPLs[asset] - int256(bufferBalance))) / (int256(balance) - int256(amount))
            );
        }
        return taxBps;
    }
    ...
    function creditTraderLoss(address user, address asset, string memory market, uint256 amount)
        external
        onlyContract
    {
    ...
    function debitTraderProfit(address user, address asset, string calldata market, uint256 amount)
        external
        onlyContract
    {
        // return if profit = 0
    ...
    function deposit(address asset, uint256 amount) public payable {
        ...
        // increment / mint LP token
        poolRewards.incrementPoolRewards(asset, user, amount);
    ...
    function withdraw(address asset, uint256 amount) public {
        ...
        // bool isPoolRewardsTrue = poolRewards.decrementPoolRewards(asset, user, clpAmount);
        // require(isPoolRewardsTrue, "PoolRewards must return true");
...
contract PoolStore is Roles {
    uint256 private constant swV = 1000102;
    // Libraries
    using SafeERC20 for IERC20;
...
import "../utils/ITLP.sol"; 


contract PoolRewards is Roles {
    uint256 private constant swV = 1000101;
    ...
    function link() external onlyGov {
        poolStore = PoolStore(DS.getAddress("PoolStore"));
        tokenLP = ITLP(DS.getAddress("TokenLP"));
    }

    function incrementPoolRewards(address asset, address user, uint256 amount) external onlyContract {
        // if asset isnt Eth, return;
        if (asset == address(0)) {
            tokenLP.mint(user, amount);
        }
    }

    function decrementPoolRewards(address asset, address user, uint256 amount) external onlyContract returns (bool) {
        // if asset isnt Eth, return;
        if (asset != address(0)) {return true;}

        uint256 userOPL1Balance = tokenLP.balanceOf(user);
        require(userOPL1Balance >= amount, "not enough LP");

        tokenLP.burnFrom(user, amount);
        return true;
    }
