...
import {CreateParams, Phase} from "../structs/dn404/DN404Structs.sol";
import "../util/ReentrancyGuard.sol";
import "../util/Ownable.sol";
import {LibString} from "../util/LibString.sol";
import "./DN404.sol";
import "./DN404Mirror.sol";

contract OmniseaDN404 is DN404, ReentrancyGuard, Ownable {
    ...
    function initialize(
        ...
        if (params.premintQuantity > 0) {
            ...
            minted += params.premintQuantity;
        }

        address mirror = address(new DN404Mirror(msg.sender));
        _initializeDN404(params.premintQuantity > 0 ? params.premintQuantity * _WAD : 0, _owner, mirror);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return string(abi.encodePacked("ipfs://", tokensURI, "/", LibString.toString(effectiveId), ".json"));
    }

    function mint(address _minter, uint24 _quantity, bytes32[] memory _merkleProof, uint8 _phaseId) external nonReentrant returns (uint256) {
        ...
        _mint(_minter, _quantity * _WAD);
        minted += _quantity;

        return minted;
    }

    function mintPrice(uint8 _phaseId) public view returns (uint256) {
    ...
    function preMintToPlatform(uint256 _quantity) external {
        ...
        _mint(_revenueManager, _quantity * _WAD);
        minted += _quantity;
    }
...
contract DN404Mirror {
    ...
    function owner() public view returns (address result) {
        address base = baseERC20(); // Ensure this is the address of a contract with an `owner()` function
        bytes4 selector = bytes4(keccak256("owner()"));
        /// @solidity memory-safe-assembly
        assembly {
            let ptr := mload(0x40) // Allocate memory for input (function selector)

            mstore(ptr, selector) // Place `owner()` function selector at start of allocated memory

        // Attempt to call `owner()` on `base` contract, placing return data in memory starting at `ptr`
        // Note: Adjusted staticcall to use ptr for input and output, ensuring correct data handling
            if iszero(staticcall(gas(), base, ptr, 0x04, ptr, 0x20)) {
            // If call fails, revert with returned data
                returndatacopy(ptr, 0, returndatasize())
                revert(ptr, returndatasize())
            }

            result := mload(ptr) // Load returned address into `result`
        // No need for further memory management as we're returning a single value
        }
    }
