...
contract SwitchHop is SwitchV2 {
        ...
        nonReentrant
    {
        ...
        } else if (transferArgs.fromToken == bridgedUsdc) {
        ...
        nonReentrant
    {
        ...
        } else if (bridgeToken == bridgedUsdc) {
...
contract SwitchV2 is Ownable, SwitchRoot, ReentrancyGuard {
    using UniswapExchangeLib for IUniswapExchange;
    using UniversalERC20 for IERC20;
    using SafeERC20 for IERC20;
    ...
    mapping(address => bool) public dexWhitelists;
    ...
    modifier onlyWhitelistedDEX(address dex, address spender) {
        if (!dexWhitelists[dex]) {
            revert Forbidden();
        }
        if (spender != address(0) && !dexWhitelists[spender]) {
            revert Forbidden();
        }
        _;
    }
    ...
    /**
     * @notice Emitted when dex whitelisted
     * @param dex Dex address
     * @param whitelisted to whitelist or not
     */
    ...
    function whitelistDEX(address _dex, bool _whitelisted) external onlyOwner {
        _whitelistDEX(_dex, _whitelisted);
    }
    ...
    function _callAggregator(
        ...
        internal
        onlyWhitelistedDEX(swapData.swapContract, swapData.spender)
    {
    ...
    function _whitelistDEX(address _dex, bool _whitelist) internal {
        dexWhitelists[_dex] = _whitelist;

        emit DEXWhitelisted(_dex, _whitelist);
    }
}
