...
contract TWAPOracleUpdater is ERC20Permit, VaultOwned {
  ...
  function addTWAPSource( address newTWAPSourceDexPool_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.add( newTWAPSourceDexPool_ ), "TridentERC20TOken: TWAP Source already stored." );
    emit TWAPSourceAdded( newTWAPSourceDexPool_ );
  }

  function removeTWAPSource( address twapSourceToRemove_ ) external onlyOwner() {
    require( _dexPoolsTWAPSources.remove( twapSourceToRemove_ ), "TridentERC20TOken: TWAP source not present." );
...
contract TridentBaseToken is TWAPOracleUpdater {

  using SafeMath for uint256;

    constructor(string memory name_, string memory ticker_) TWAPOracleUpdater(name_, ticker_, 9) {
    }
