...
contract OrderBook is Storage, ReentrancyGuardUpgradeable, Admin, Getter {
    ...
    event ReportBrokerGasRebatePrice(uint96 nativeTokenPrice); // 1e18
     ...
     *          - profitTokenId: take no effect yet.
     *          - isProfit: false for withdraw collateral. true is unsupported yet.
     */
     ...
     *         on how to calculate funding and borrowing.
     */
    function updateFundingState() external onlyRole(BROKER_ROLE) {
        IDegenPool(_storage.pool).updateFundingState();
    }
