...
contract tester is ERC20, Ownable {
    ...
    address public spodervestWallet;
    ...
    constructor(address _owner) ERC20("tester", "tester") Ownable(msg.sender) {
        ...
        spodervestWallet = address(0);

        marketingFee = 300;
        spodervestFee = 300;
        LPFee = 200;
        totalFee = marketingFee + spodervestFee + LPFee;
        ...
        swapThreshold = 69000 * (10 ** 18);
        tokenLimitPerWallet = 207000 * (10 ** 18);

        _mint(address(_owner), 6900000 * (10 ** 18));
        _transferOwnership(address(_owner));
    }

    receive() external payable {}

    function updateFees(uint256 newMarketingFee, uint256 newSpodervestFee, uint256 newLPFee) external onlyOwner {
        require(newMarketingFee + newSpodervestFee + newLPFee <= 1000, "Max fee is 10%");

        marketingFee = newMarketingFee;
        spodervestFee = newSpodervestFee;
        LPFee = newLPFee;
        totalFee = newMarketingFee + newSpodervestFee + newLPFee;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override(ERC20) {
        ...
        if (!swapping && canSwap && isLiquidityPair[recipient]) {
            ...
            uint256 liquidityShareInTokens = ((swapThreshold * LPFee) / totalFee);
            swapAndAddLP(liquidityShareInTokens);

            uint256 oldBalance = address(this).balance;
            swapTokensForETH(swapThreshold - liquidityShareInTokens);
            uint256 newBalance = (address(this).balance) - oldBalance;

            uint256 marketingShare = newBalance * marketingFee / (marketingFee + spodervestFee);
            uint256 spodervestShare = newBalance - marketingShare;

            if (marketingShare > 0) {
                payable(marketingWallet).transfer(marketingShare);
            }
            if (spodervestShare > 0) {
                payable(spodervestWallet).transfer(spodervestShare);
            }
            swapping = false;
        }
        ...
        if (isWalletTaxFree[sender] || isWalletTaxFree[recipient]) {
            ...
            if (!isWalletExemptFromLimit[recipient]) {
                require(
                    ((balanceOf(recipient) + amount) - fees) <= tokenLimitPerWallet,
                    "Transfer amount exceeds the `tokenLimitPerWallet`."
                );
            }
            if (fees > 0) {
    ...
    function swapAndAddLP(uint256 LPShareTokens) private {
        uint256 half = LPShareTokens / 2;
