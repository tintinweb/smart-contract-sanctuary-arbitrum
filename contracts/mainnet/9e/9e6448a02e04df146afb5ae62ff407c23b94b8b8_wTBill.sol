...
import {IOpenEdenVault} from "./interface/IOpenEdenVault.sol";
import {IOpenEdenOracle} from "./interface/IOpenEdenOracle.sol";

/**
 * @author 0xEkkila
 * @dev The initial 1000 shares of this contract should be burned immediately following deployment.
 * @notice There is a significant difference between previewDeposit amount and the actual shares got minted
 * @notice Redeem process does not execute immediately after function call. This may be applied for the deposit process
 */
contract wTBill is Ownable, ReentrancyGuard {
    using SafeTransferLib for ERC20;
    using FixedPointMathLib for uint256;

    IOpenEdenOracle public oracle;
    ERC20 public immutable token;

    mapping(address => uint256) public balances;
    mapping(address => mapping(address => uint256)) public allowance;

    //ERC-20 related variables
    ...
    constructor(address _token, address _oracle, address govController) Ownable(govController) {
        token = ERC20(_token);
        oracle = IOpenEdenOracle(_oracle);
        decimals = 6;

        name = "Wrapped_OpenEden_TBill";
        symbol = "wTBill";
    }
    ...
    function scaleUp(uint256 amount) public view returns (uint256) {
        uint256 rate = oracle.latestAnswer();
