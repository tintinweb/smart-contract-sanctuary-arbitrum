...
contract CrossChainMessageSender is HeadlessSwapBase {
    address public acrossFeeOracle;
    ...
    function initialize(
        address _acrossFeeOracle,
        address _bridge,
        address _originWeth
    ) public initializer {
        __Ownable_init();
        acrossFeeOracle = _acrossFeeOracle;
        bridge = _bridge;
        originWeth = _originWeth;
    }
    ...
    function sendToAcross(
        ...
        bytes memory message = abi.encode(pool, onBehalf, pType);
        int64 relayerFeePct = IAcrossFees(acrossFeeOracle).getHighRelayersFee(
            msg.value
        );
