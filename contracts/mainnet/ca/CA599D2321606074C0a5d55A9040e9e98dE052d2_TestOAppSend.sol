...
contract TestOAppSend is OApp {
    using OptionsBuilder for bytes;

    event TestLzReceive(string data);
    ...
    function send(
        ...
        bytes memory payload = abi.encode(_message); // Encodes message as bytes.
        bytes memory lzOptions = OptionsBuilder.newOptions().addExecutorLzReceiveOption(_dstGas, 0);

        _appSend(
            _dstEid, // Destination chain's endpoint ID.
            payload, // Encoded message payload being sent.
            lzOptions, // Message execution options (e.g., gas to use on destination).
            MessagingFee(msg.value, 0), // Fee struct containing native gas and ZRO token.
            payable(msg.sender), // The refund address in case the send call reverts.
            addressToBytes32(_dstApp)
        );
    }
    ...
    function quote(
        ...
        MessagingFee memory fee = _appQuote(_dstEid, payload, lzOptions, addressToBytes32(_dstApp));
    ...
    function _appSend(
        ...
        endpoint.send{ value: _fee.nativeFee }(
            MessagingParams(_dstEid, _peer, _message, _options, false),
            _refundAddress
        );
    }
