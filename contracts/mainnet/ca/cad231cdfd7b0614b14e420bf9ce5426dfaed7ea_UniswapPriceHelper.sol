...
contract UniswapPriceHelper is IPriceHelper, UpgradeableGovernable {
    using SafeMath for uint256;
    using MulDivMathLib for uint256;

    mapping(address => PoolData) private data;
    ...
    function initialize(address _factory, address _feeds) external initializer {
    ...
    {
        return UniLiquidityLib.amountsForLiquidity(pool, tickLower, tickUpper, liquidity);
    }
    ...
    function liquidityForAmounts(address pool, int24 tickLower, int24 tickUpper, uint256 amount0, uint256 amount1)
        ...
        returns (uint128)
    {
        return UniLiquidityLib.liquidityForAmounts(pool, tickLower, tickUpper, amount0, amount1);
    }
    ...
    function amountsForAmount0(address pool, int24 tickLower, int24 tickUpper, uint256 amount0)
        ...
        returns (uint256, uint256)
    {
        return UniLiquidityLib.amountsForAmount0(pool, tickLower, tickUpper, amount0);
    }
    ...
    function amountsForAmount1(address pool, int24 tickLower, int24 tickUpper, uint256 amount1)
        ...
        returns (uint256, uint256)
    {
        return UniLiquidityLib.amountsForAmount1(pool, tickLower, tickUpper, amount1);
    }

    /**
     * @notice Normalice token decimals to 18.
    ...
    function normalise(address _token, uint256 _amount) external view override returns (uint256 normalised) {
        ...
        if (_decimals < 18) {
            normalised = _amount.mul(10 ** (18 - _decimals));
        } else if (_decimals > 18) {
            normalised = _amount.div(10 ** (_decimals - 18));
        }
    }
     ...
     * @return price0 usd price of token0; 18 decimals.
     * @return price1 usd price of token1; 18 decimals.
     */
    function getPrices(address pool) external view override returns (uint256 price0, uint256 price1) {
        ...
        (, int256 oraclePrice,,,) = _data.feed.latestRoundData();
        uint8 oracleDecimals = _data.feed.decimals();

        // TWAP of token0 quoted in token1.
        uint256 spotPrice = _manipulationProtection(pool, _data);

        if (_data.referenceToken == IUniswapV3Pool(pool).token1()) {
            //  token1 decimals * oracle decimals
            price0 = spotPrice * uint256(oraclePrice);
            uint256 decimals = IERC20Metadata(IUniswapV3Pool(pool).token1()).decimals() + oracleDecimals;
            if (decimals < 18) {
                price0 = price0.mul(10 ** (18 - decimals));
            } else {
                price0 = price0.div(10 ** (decimals - 18));
            }

            price1 = uint256(oraclePrice);

            if (oracleDecimals < 18) {
                price1 = price1.mul(10 ** (18 - oracleDecimals));
            } else {
                price1 = price1.div(10 ** (oracleDecimals - 18));
            }
        } else {
            price0 = uint256(oraclePrice);
            if (oracleDecimals < 18) {
                price0 = price0.mul(10 ** (18 - oracleDecimals));
            } else {
                price0 = price0.div(10 ** (oracleDecimals - 18));
            }

            price1 = price0.mulDivDown(PRECISION, spotPrice); //24 decimals

            uint256 decimals = 48 - IERC20Metadata(IUniswapV3Pool(pool).token1()).decimals();

            if (decimals < 18) {
                price1 = price1.mul(10 ** (18 - decimals));
            } else {
                price1 = price1.div(10 ** (decimals - 18));
            }
        }
    }
    ...
    function checkManipulation(address pool) external view override returns (uint256 spot) {
        ...
        uint256 mean = UniV3Library.getPrice(pool, _data.secondsAgo);
...
library UniLiquidityLib {
        ...
        returns (uint256, uint256)
    {
        ...
        uint128 liquidity = LiquidityAmounts.getLiquidityForAmount1(
            TickMath.getSqrtRatioAtTick(tickLower), TickMath.getSqrtRatioAtTick(tickUpper), amount1
        );
