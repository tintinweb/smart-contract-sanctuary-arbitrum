...
import "./Pricing/interfaces/IBasePriceOracle.sol";
...
contract PriceFeed is IPriceFeed, OwnableUpgradeable, BaseMath {
    ...
    uint256 public constant MAX_PRICE_DEVIATION_BETWEEN_ROUNDS_LOWER_LIMIT =
        0.2 ether;
    uint256 public constant MAX_PRICE_DEVIATION_BETWEEN_ROUNDS_UPPER_LIMIT =
        0.5 ether;
    ...
    mapping(address => IBasePriceOracle) public lpPriceRecords;
    ...
    function setLpOracle(address _token, address _priceFeed) external override {
        if (address(lpPriceRecords[_token]) == address(0)) {
            _checkOwner();
        } else {
            _checkTimelock();
        }
        ...
        require(!oracleRecords[_token].exists, "!duplicate");
        require(_priceFeed != address(0), "!priceFeed");

        lpPriceRecords[_token] = IBasePriceOracle(_priceFeed);

        emit NewOracleRegistered(_token, _priceFeed, false);
    }
    ...
    function setOracle(
        ...
        require(address(lpPriceRecords[_token]) == address(0), "!duplicate");

        if (
            _maxDeviationBetweenRounds <
            MAX_PRICE_DEVIATION_BETWEEN_ROUNDS_LOWER_LIMIT ||
            _maxDeviationBetweenRounds >
            MAX_PRICE_DEVIATION_BETWEEN_ROUNDS_UPPER_LIMIT
        ) {
    ...
    function fetchPrice(address _token) public override returns (uint256) {
        if (address(lpPriceRecords[_token]) != address(0)) {
            return lpPriceRecords[_token].getPrice(_token);
        } else {
            OracleRecord storage oracle = oracleRecords[_token];
            ...
            (
                FeedResponse memory currResponse,
                FeedResponse memory prevResponse
            ) = _fetchFeedResponses(oracle.chainLinkOracle);
            return
                _processFeedResponses(
                    _token,
                    oracle,
    ...
    function _processFeedResponses(
        ...
        bool isValidResponse = _isFeedWorking(_currResponse, _prevResponse) &&
            ...
            !_isPriceChangeAboveMaxDeviation(
                oracle.maxDeviationBetweenRounds,
                _currResponse,
                _prevResponse
            );
        ...
        )
    {
        ...
        prevResponse = _fetchPrevFeedResponse(
            oracle,
            currResponse.roundId,
            currResponse.decimals
        );
    }
    ...
    function _isPriceChangeAboveMaxDeviation(
        ...
        uint256 minPrice = PreonMath._min(currentScaledPrice, prevScaledPrice);
        uint256 maxPrice = PreonMath._max(currentScaledPrice, prevScaledPrice);
