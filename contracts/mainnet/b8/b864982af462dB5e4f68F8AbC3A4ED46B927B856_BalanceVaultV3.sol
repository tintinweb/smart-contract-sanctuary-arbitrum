    ...
    OwnableUpgradeable
{
    ...
    /// @inheritdoc IBalanceVaultV3
    bool public override isEmergency;

    /// @inheritdoc IBalanceVaultV3
    mapping(address => bool) public override supportTokens;

    /// @notice proxy deployer address, only owner and this address can call addWhiteFactory
    ...
    /// @inheritdoc IBalanceVaultV3
    mapping(address => mapping(address => bool)) public override whiteFactories;

    /// @inheritdoc IBalanceVaultV3
    mapping(address => mapping(address => bool))
        public
        override userApproveFactories;

    /*=====================
     *       Events       *
     *====================*/

    event Deposit(address indexed account, address _token, uint256 amount);
    event Withdraw(address indexed account, address _token, uint256 amount);
    event TransferBalanceFromVault(
        address indexed account,
        address factory,
        address _token,
        uint256 amount
    );
    event TransferBalanceToVault(
        address indexed account,
        address factory,
        address _token,
        uint256 amount
    );
    event SetEmergencyState(address indexed account, bool state);
    event SetProxyDeployer(address indexed proxyDeployer);
    ...
    /**
     * @dev can only be executed in non-emergency state
     */
    modifier notEmergency() {
        require(!isEmergency, "Invalid operation when in emergency");
        _;
    }
    ...
    /**
     * @notice constructor to init this implementation contract
     */
    constructor() {
        init();
    }
    ...
    /*==========================
     *   BalanceVault Functions  *
     *==========================*/

    /// @inheritdoc IBalanceVaultV3
    function setEmergencyState(bool _isEmergency) external override onlyOwner {
        isEmergency = _isEmergency;
        emit SetEmergencyState(msg.sender, _isEmergency);
    }
    ...
    /// @inheritdoc IBalanceVaultV3
    function removeSupportToken(address _token) external override onlyOwner {
        require(
            IERC20(_token).balanceOf(address(this)) == 0,
            "non zero balance token"
        );
        supportTokens[_token] = false;
    }

    /// @inheritdoc IBalanceVaultV3
    function addWhiteFactory(address _factory, address _token)
        external
        override
    {
        require(
        ...
        require(
            _factory != address(0) &&
            supportTokens[_token],
            "Invalid address"
        );
        whiteFactories[_factory][_token] = true;
    }

    /// @inheritdoc IBalanceVaultV3
    function removeWhiteFactory(address _factory, address _token)
        external
        override
        onlyOwner
    {
        whiteFactories[_factory][_token] = false;
    }

    /// @inheritdoc IBalanceVaultV3
    function approve(address _factory, address _token) external override {
        require(whiteFactories[_factory][_token], "Invalid factory");
        userApproveFactories[msg.sender][_factory] = true;
    }

    /**
    * @notice set commitedProxyDeployer as proxyDeployer
    */
    function setProxyDeployer(address _proxyDeployer) external override onlyOwner {
        require(_proxyDeployer != address(0), "Invalid proxy deployer");
        proxyDeployer = _proxyDeployer;
        emit SetProxyDeployer(_proxyDeployer);
    }

    /// @inheritdoc IBalanceVaultV3
    function disapprove(address _factory, address _token) external override {
        require(whiteFactories[_factory][_token], "Invalid factory");
        userApproveFactories[msg.sender][_factory] = false;
    }
    ...
    {
        require(supportTokens[_token], "Invalid token");
        ...
        nonReentrant
    {
        ...
        uint256 receivedAmount = IERC20(_token).balanceOf(address(this)).sub(
            balanceBefore
        );
        require(receivedAmount == _amount, "Unsafe receive");
    ...
    {
        require(supportTokens[_token], "Invalid token");
        require(balances[_token][msg.sender] >= _amount, "Balance not enough");
        balances[_token][msg.sender] = balances[_token][msg.sender].sub(
            _amount
        );
        uint256 balanceBefore = IERC20(_token).balanceOf(address(this));
        IERC20(_token).safeTransfer(msg.sender, _amount);
        uint256 transferredAmount = balanceBefore.sub(
            IERC20(_token).balanceOf(address(this))
        );
        require(transferredAmount == _amount, "Unsafe transfer");
    ...
    function transferBalanceFromVault(
        ...
        address _token,
        uint256 _amount
    ) external override notEmergency nonReentrant {
        require(
            userApproveFactories[_account][msg.sender],
            "Disapproved factory"
        );
        require(supportTokens[_token], "Invalid token");
        require(whiteFactories[msg.sender][_token], "Invalid factory");
        require(balances[_token][_account] >= _amount, "Balance not enough");
        ...
        require(transferredAmount == _amount, "Unsafe transfer");
        emit TransferBalanceFromVault(_account, msg.sender, _token, _amount);
    }
    ...
    function transferBalanceToVault(
        ...
        emit TransferBalanceToVault(_account, msg.sender, _token, _amount);
    }
        ...
        onlyOwner
    {
        ...
        require(!supportTokens[_token], "unsupported token only");
        IERC20(_token).safeTransfer(msg.sender, _amount);
    }
}
