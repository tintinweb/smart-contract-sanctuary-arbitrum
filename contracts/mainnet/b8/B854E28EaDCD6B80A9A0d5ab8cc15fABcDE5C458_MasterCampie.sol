...
/// @title Campie
/// @author Campie Team
contract Campie is ERC20('Campie Token', 'CMP'), ERC20Permit('Campie Token') {
...
interface IVLCampie is ILocker {
    
    function campie() external view returns(IERC20);
}
...
interface IVLCampieBaseRewarder is IBaseRewardPool {
    ...
    function queueCampie(uint256 _amount, address _user, address _receiver) external returns(bool);
}
...
import "../libraries/CampieFactoryLib.sol";

import "../Campie.sol";
import "../interfaces/IBaseRewardPool.sol";
import "../interfaces/IMintableERC20.sol";
import "../rewards/BaseRewardPool.sol";
import "../interfaces/IVLCampieBaseRewarder.sol";
import "../interfaces/IVLCampie.sol";

/// @title A contract for managing all reward pools
/// @author Campie Team
/// @notice Master Campie emit `CMP` reward token based on Time. For a pool,

contract MasterCampie is
    Initializable,
    OwnableUpgradeable,
    ...
    PausableUpgradeable
{
    ...
    struct UserInfo {
        ...
        uint256 unClaimedCampie;
        //
        // We do some fancy math here. Basically, any point in time, the amount of Campies
        // entitled to a user but is pending to be distributed is:
        //
        // pending reward = (user.amount * pool.accCampiePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws staking tokens to a pool. Here's what happens:
        //   1. The pool's `accCampiePerShare` (and `lastRewardTimestamp`) gets updated.
    ...
    // The Campie TOKEN!
    IERC20 public campie;
    IVLCampie public vlCampie;

    // Campie tokens created per second.
    uint256 public campiePerSec;
    ...
    // The timestamp when Campie mining starts.
    ...
    event UpdateEmissionRate(
        address indexed _user,
        uint256 _oldCampiePerSec,
        uint256 _newCampiePerSec
    );
    ...
    event VlCampieUpdated(address _newvlCampie, address _oldvlCampie);
    event DepositNotAvailable(
        address indexed _user,
        address indexed _stakingToken,
        uint256 _amount
    );
    event CampieSet(address _campie);
    event updatedWhielistedAllocManager(
        address indexed _account,
        bool _allowed
    );
    ...
    function __MasterCampie_init(
        address _Campie,
        uint256 _campiePerSec,
        uint256 _startTimestamp
    ) public initializer {
        __Ownable_init();
        __ReentrancyGuard_init();
        __Pausable_init();
        campie = IERC20(_Campie);
        campiePerSec = _campiePerSec;
    ...
    modifier _onlyVlCampie() {
        if (msg.sender != address(vlCampie)) revert OnlyVlCampie();
        _;
    }
    ...
    /// @return emission - Emissions of Campie from the contract, allocpoint - Allocated emissions of Campie to the pool,sizeOfPool - size of Pool, totalPoint total allocation points
        ...
        returns (uint256 emission, uint256 allocpoint, uint256 sizeOfPool, uint256 totalPoint)
    {
        ...
        return (
            ((campiePerSec * pool.allocPoint) / totalAllocPoint),
            pool.allocPoint,
            pool.totalStaked,
            totalAllocPoint
        );
    ...
    /// @param _rewardToken Specific pending reward token, apart from Campie
    /// @return pendingCampie - Expected amount of Campie the user can claim, bonusTokenAddress - token, bonusTokenSymbol - token Symbol,  pendingBonusToken - Expected amount of token the user can claim
    function pendingTokens(
        address _stakingToken,
        address _user,
        address _rewardToken
    ...
    {
        PoolInfo memory pool = tokenToPoolInfo[_stakingToken];
        pendingCampie = _calCampieReward(_stakingToken, _user);
    ...
    /// @notice Withdraw staking tokens from Master Campie.
    /// @param _stakingToken Staking token of the pool
    ...
    function updatePool(address _stakingToken) public whenNotPaused {
        ...
        uint256 campieReward = (multiplier * campiePerSec * pool.allocPoint) / totalAllocPoint;

        pool.accCampiePerShare = pool.accCampiePerShare + ((campieReward * 1e12) / lpSupply);
        pool.lastRewardTimestamp = block.timestamp;

        emit UpdatePool(_stakingToken, pool.lastRewardTimestamp, lpSupply, pool.accCampiePerShare);
    }
    ...
    function multiclaimSpecCMP(
        address[] calldata _stakingTokens,
        address[][] memory _rewardTokens,
        bool _withCMP
    ) external whenNotPaused {
        _multiClaim(_stakingTokens, msg.sender, msg.sender, _rewardTokens, _withCMP);
    }
    ...
    /* ============ Campie receipToken interaction Functions ============ */
    ...
    function afterReceiptTokenTransfer(
        ...
        if (_from != address(0)) {
            ...
            from.rewardDebt = (from.amount * pool.accCampiePerShare) / 1e12;
        } else {
            // mint
        ...
        if (_to != address(0)) {
            ...
            to.rewardDebt = (to.amount * pool.accCampiePerShare) / 1e12;
        } else {
            // brun
            tokenToPoolInfo[_stakingToken].totalStaked -= _amount;
        }
    ...
    /* ============ vlCampie interaction Functions ============ */

    function depositVlCampieFor(uint256 _amount, address _for) external whenNotPaused nonReentrant _onlyVlCampie {
        _deposit(address(vlCampie), msg.sender, _for, _amount, true);
    }

    function withdrawVlCampieFor(uint256 _amount, address _for) external whenNotPaused nonReentrant _onlyVlCampie {
        _withdraw(address(vlCampie), _for, _amount, true);
    }
    ...
    function _deposit(address _stakingToken, address _from, address _for, uint256 _amount, bool _isLock) internal {
        ...
        user.rewardDebt = (user.amount * pool.accCampiePerShare) / 1e12;
    ...
    function _withdraw(address _stakingToken, address _account, uint256 _amount, bool _isLock) internal {
        ...
        updatePool(_stakingToken);
        _harvestCampie(_stakingToken, _account);
    ...
    function _multiClaim(
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            updatePool(_stakingToken);
            uint256 claimableCampie = _calNewCampie(_stakingToken, _user) + user.unClaimedCampie;

            // if claim with CMP, then unclamed is 0
            if (_withCmp) {
                if (_stakingToken == address(vlCampie)) {
                    vlCampiePoolAmount += claimableCampie;
                } else {
                    defaultPoolAmount += claimableCampie;
                }
                user.unClaimedCampie = 0;
            } else {
                user.unClaimedCampie = claimableCampie;
            }

            user.rewardDebt = (user.amount * tokenToPoolInfo[_stakingToken].accCampiePerShare) / 1e12;
            _claimBaseRewarder(_stakingToken, _user, _receiver, _rewardTokens[i]);
        }

        // if not claiming CMP, early return
        if (!_withCmp) return;

        if (vlCampiePoolAmount > 0) {
            _sendCampieForVlCampiePool(_user, _receiver, vlCampiePoolAmount);
        }

        if (defaultPoolAmount > 0) {
            _sendCampie(_user, _receiver, defaultPoolAmount);
        }
    }

    /// @notice calculate Campie reward based at current timestamp, for frontend only
    function _calCampieReward(
        address _stakingToken,
        address _user
    ) internal view returns (uint256 pendingCampie) {
        PoolInfo memory pool = tokenToPoolInfo[_stakingToken];
        UserInfo memory user = userInfo[_stakingToken][_user];
        uint256 accCampiePerShare = pool.accCampiePerShare;

        if (block.timestamp > pool.lastRewardTimestamp && pool.totalStaked != 0) {
            uint256 multiplier = block.timestamp - pool.lastRewardTimestamp;
            uint256 campieReward = (multiplier * campiePerSec * pool.allocPoint) / totalAllocPoint;
            accCampiePerShare = accCampiePerShare + (campieReward * 1e12) / pool.totalStaked;
        }

        pendingCampie = (user.amount * accCampiePerShare) / 1e12 - user.rewardDebt;
        pendingCampie += user.unClaimedCampie;
    }
    ...
    /// @notice Harvest Campie for an account
    /// only update the reward counting but not sending them to user
    function _harvestCampie(address _stakingToken, address _account) internal {
        // Harvest Campie
        uint256 pending = _calNewCampie(_stakingToken, _account);
        userInfo[_stakingToken][_account].unClaimedCampie += pending;
    }

    /// @notice calculate Campie reward based on current accCampiePerShare
    function _calNewCampie(
        address _stakingToken,
        address _account
    ) internal view returns (uint256) {
        UserInfo memory user = userInfo[_stakingToken][_account];
        uint256 pending = (user.amount * tokenToPoolInfo[_stakingToken].accCampiePerShare) /
            1e12 -
            user.rewardDebt;
        return pending;
    }
    ...
    function _sendCampieForVlCampiePool(address _account, address _receiver, uint256 _amount) internal {
        address vlCampieRewarder = tokenToPoolInfo[address(vlCampie)].rewarder;
        campie.safeApprove(vlCampieRewarder, _amount);
        IVLCampieBaseRewarder(vlCampieRewarder).queueCampie(_amount, _account, _receiver);
        emit HarvestCampie(_account, _receiver, _amount, false);
    }

    function _sendCampie(address _account, address _receiver, uint256 _amount) internal {
        campie.safeTransfer(_receiver, _amount);
    ...
    /// @param _account Pool Manager Address
    ...
    function setCampie(address _campie) external onlyOwner {
        if (address(campie) != address(0)) revert CampieSetAlready();

        if (!Address.isContract(_campie)) revert MustBeContract();

        campie = IERC20(_campie);
        emit CampieSet(_campie);
    }

    function setVlCampie(address _vlCampie) external onlyOwner {
        address oldvlCampie = address(vlCampie);
        vlCampie = IVLCampie(_vlCampie);
        emit VlCampieUpdated(_vlCampie, oldvlCampie);
    }
    ...
    /// @param _mainRewardToken Token that will be rewarded for staking in the pool
    /// @return address of the rewarder created
    function createRewarder(address _receiptToken, address _mainRewardToken, address _rewardDistributor, address _xGrailToken)
        external
        _onlyPoolManager
        returns (address)
    {
        address rewarder = CampieFactoryLib.createRewarder(
            _receiptToken,
            _mainRewardToken,
            address(this),
            _rewardDistributor,
    ...
    /// @notice Add a new Camelot exchange pool. Explicitly for Camelot Exchange pools and should be called from Grail Staking.
    function add(
        uint256 _allocPoint,
        address _stakingToken,
        address _receiptToken,
    ...
    /// @notice Add a new pool that does not mint receipt token. Mainly for locker pool such as vlCMP, mGrailSV
    ...
    function createPool(
        ...
        IERC20 newToken =IERC20(CampieFactoryLib.createReceipt(
            address(_stakingToken),
            address(this),
            // address(0),
            _receiptName,
    ...
    /// @notice Updates the given pool's Campie allocation point, rewarder address and locker address if overwritten. Can only be called by a Pool Manager.
    /// @param _stakingToken Staking token of the pool
    /// @param _allocPoint Allocation points of Campie to the pool
    /// @param _rewarder Address of the rewarder for the pool
    function set(address _stakingToken, uint256 _allocPoint, address _rewarder) external _onlyPoolManager {
        if (!Address.isContract(address(_rewarder)) && address(_rewarder) != address(0))
            revert MustBeContractOrZero();
    ...
    /// @notice Update the emission rate of Campie for MasterCampie
    /// @param _campiePerSec new emission per second
    function updateEmissionRate(uint256 _campiePerSec) public onlyOwner {
        massUpdatePools();
        uint256 oldEmissionRate = campiePerSec;
        campiePerSec = _campiePerSec;

        emit UpdateEmissionRate(msg.sender, oldEmissionRate, campiePerSec);
    }
    ...
    function updateWhitelistedAllocManager(address _account, bool _allowed) external onlyOwner {
        ...
        emit updatedWhielistedAllocManager(_account, _allowed);
    }
