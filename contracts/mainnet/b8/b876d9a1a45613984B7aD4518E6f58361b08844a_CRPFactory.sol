 ...
 *
 * @notice SPToken is the "Mayfair Smart Pool" token (transferred upon finalization)
 *
    ...
    ReentrancyGuard
{
    ...
    struct PoolParams {
        // Mayfair Pool Token (representing shares of the pool)
    ...
    /// Address of the core factory contract; for creating the core pool and enforcing $MAY
     ...
     */
    event CapChanged(address indexed caller, uint oldCap, uint newCap);
    ...
    constructor(
        ...
        require(
            poolParams.swapFee >= MayfairConstants.MIN_FEE &&
                poolParams.swapFee <= MayfairConstants.MAX_FEE,
        ...
        require(
            poolParams.constituentTokens.length >=
                MayfairConstants.MIN_ASSET_LIMIT,
            "ERR_TOO_FEW_TOKENS"
        );
        require(
            poolParams.constituentTokens.length <=
                MayfairConstants.MAX_ASSET_LIMIT,
            "ERR_TOO_MANY_TOKENS"
        );
        ...
        SmartPoolManager.verifyTokenCompliance(
            ...
            coreFactory.minimumMay(),
            coreFactory.mayToken()
        );
        ...
        tokenCap = MayfairConstants.MAX_UINT;
    }
    ...
    function setSwapFee(
        uint swapFee
    ) external virtual needsCorePool onlyOwner lock logs {
     ...
     */
    function setCap(uint newCap) external needsCorePool onlyOwner lock logs {
     ...
     */
    function setExitFeeCollector(address feeCollector) external onlyOwner logs {
     ...
     */
    function setStrategy(address updaterAddr) external onlyOwner logs {
    ...
    ) external virtual onlyOwner lock logs {
        require(
    ...
    function updateWeight(
        address token,
        uint newWeight
    ) external virtual override needsCorePool onlyStrategy lock logs {
     ...
     */
    function pokeWeights() external virtual override needsCorePool lock logs {
        require(rights.canChangeWeights, "ERR_NOT_CONFIGURABLE_WEIGHTS");
    ...
    function removeToken(
        address token
    ) external override needsCorePool onlyStrategy lock logs {
        ...
        require(gradualUpdate.startBlock == 0, "ERR_NO_UPDATE_DURING_GRADUAL");
        // can't remove $MAY (core pool also checks but we can fail earlier)
        require(token != coreFactory.mayToken(), "ERR_MIN_MAY");
    ...
    ) external needsCorePool lock lockUnderlyingPool logs {
        require(
    ...
    ) external logs needsCorePool lock returns (uint poolAmountOut) {
        require(
            !rights.canWhitelistLPs || _liquidityProviderWhitelist[msg.sender],
            "ERR_NOT_ON_WHITELIST"
        );
    ...
    function joinswapPoolAmountOut(
        ...
        uint poolAmountOut,
        uint maxAmountIn
    ) external logs needsCorePool lock returns (uint tokenAmountIn) {
        require(
            !rights.canWhitelistLPs || _liquidityProviderWhitelist[msg.sender],
            "ERR_NOT_ON_WHITELIST"
        );
    ...
    function exitswapPoolAmountIn(
        ...
        uint poolAmountIn,
        uint minAmountOut
    ) external logs needsCorePool lock returns (uint tokenAmountOut) {
        // Delegate to library to save space
        uint exitFee;
        uint pAiAfterExitFee;

        // Calculates final amountOut, and the fee and final amount in
        (exitFee, pAiAfterExitFee, tokenAmountOut) = SmartPoolManager
    ...
    ) external logs needsCorePool lock returns (uint poolAmountIn) {
        // Delegate to library to save space
        uint exitFee;
        uint pAiAfterExitFee;

        // Calculates final amounts in, accounting for the exit fee
        (exitFee, pAiAfterExitFee, poolAmountIn) = SmartPoolManager
    ...
    function hasPermission(
        RightsManager.Permissions permission
    ...
    function createPoolInternal(uint initialSupply) internal {
        ...
        require(
            initialSupply >= MayfairConstants.MIN_POOL_SUPPLY,
            "ERR_INIT_SUPPLY_MIN"
        );
        require(
            initialSupply <= MayfairConstants.MAX_POOL_SUPPLY,
            "ERR_INIT_SUPPLY_MAX"
        );
...
contract CRPFactory is IcrpFactory, Ownable {
    ...
    function newCrp(
    ...
    ) external onlyOwner returns (ConfigurableRightsPool crp) {
        require(
            poolParams.constituentTokens.length >=
                MayfairConstants.MIN_ASSET_LIMIT,
            "ERR_TOO_FEW_TOKENS"
        ...
        crp = new ConfigurableRightsPool(factoryAddress, poolParams, rights);
     ...
     */
    function isCrp(address addr) external view override returns (bool) {
        return _isCrp[addr];
    }
}
...
library RightsManager {
     ...
     */
    function constructRights(
        bool[] calldata a
    ) external pure returns (Rights memory) {
        if (a.length == 0) {
            return
                Rights(
...
/**
 * @author PieDAO (ported to Balancer Labs) (ported to Mayfair)
 *
...
abstract contract TokenBase is IERC20 {
    ...
    function transferFrom(
        ...
        require(
            msg.sender == sender || amount <= oldAllowance,
...
abstract contract SPToken is TokenBase {
    string public constant NAME = "Mayfair Smart Pool";
...
abstract contract CPToken is TokenBase {
    string public constant NAME = "Mayfair Core Pool";
...
/**
 * @author Mayfair (and Balancer Labs and OpenZeppelin)
 *
