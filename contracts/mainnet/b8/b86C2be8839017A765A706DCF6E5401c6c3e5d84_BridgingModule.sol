...
import { IBridgingModule } from "src/interfaces/accounts/IBridgingModule.sol";
...
import { SolanaUtils } from "src/libraries/SolanaUtils.sol";
...
contract BridgingModule is IBridgingModule, SecurityModifiers {
    using SafeERC20 for IERC20;

    /**
     * @notice Maximum amount that can be bridged in one transaction.
     * @return The maximum bridging amount.
     */
    function getBridgeMaxAmount() external view returns (uint256) {
        return BridgeIntegrations._getBridgeMaxAmount();
    }

    /**
     * @notice Process an attested Wormhole bridge and Circle CCTP message to redeem tokens that were bridged.
     * @param _redeemParams The parameters for redeeming tokens.
     * redeemParams: {
     ...
     * }
     */
    function processWormholeBridgeMessage(ICircleIntegration.RedeemParameters calldata _redeemParams) external {
        BridgeIntegrations._processWormholeBridgeMessage(_redeemParams);
    }

    /**
     * @notice Bridge USDC tokens from the current address to the same address on the destination chain, using Wormhole.
     * @param _amount is the amount to be sent in 18 decimals scaled to 6 for usdc.
     * @param _wormholeChainId is the allowlisted wormhole chain ID that usdc can be sent to.
     * @dev Requires the sender to be the sudo key or operation key.
     */
    function bridgeUSDCWithWormholeEVM(uint256 _amount, uint16 _wormholeChainId) external requiresAuthorizedOperationsParty {
        if (!Account._infinexProtocolConfig().isSupportedWormholeChainId(_wormholeChainId)) revert Error.InvalidWormholeChainId();

        _bridgeUSDCWithWormhole(_amount, bytes32(uint256(uint160(address(this)))), _wormholeChainId);
    }

    /**
     * @notice Bridge USDC tokens from the current chain to Solana, using CCTP.
     * @param _amount is the amount to be sent in 18 decimals scaled to 6 decimals for usdc.
     * @param _solanaAccount The Solana account.
     * @param _solanaTokenAccount The Solana token account.
     * @param _accountBump A number used to ensure the derived address for the account is unique.
     * @param _tokenAccountBump A number used to ensure the derived for the token account address is unique
     * @dev The amount defined in the amount will be sent as usdc to the specified destination chain.
     * @dev Requires the sender to be the sudo key or operation key.
     */
    function bridgeUSDCWithCCTPSolana(
        ...
        uint32 solanaDestinationDomain = Account._infinexProtocolConfig().getSolanaCCTPDestinationDomain();
        ...
        ) = Account._infinexProtocolConfig().getSolanaConfiguration();

        SolanaUtils.AccountArgs memory accountArgs = SolanaUtils.AccountArgs({
            account: address(this),
            solanaAccount: _solanaAccount,
            solanaWalletSeed: walletSeed,
            solanaFixedPDASeed: fixedPDASeed,
            solanaWalletProgramAddress: walletProgramAddress,
            accountBump: _accountBump
        });

        SolanaUtils.TokenAccountArgs memory tokenAccountArgs = SolanaUtils.TokenAccountArgs({
            solanaTokenAccount: _solanaTokenAccount,
            solanaTokenMintAddress: tokenMintAddress,
            solanaTokenProgramAddress: tokenProgramAddress,
            solanaAssociatedTokenProgramAddress: associatedTokenProgramAddress,
            tokenAccountBump: _tokenAccountBump
        });

        if (!SolanaUtils.isTokenAccountValid(accountArgs, tokenAccountArgs)) revert Error.InvalidSolanaAddress();

        _bridgeUSDCWithCCTP(_amount, _solanaTokenAccount, solanaDestinationDomain);
    }

    /**
     * @notice Internal function to bridge USDC tokens from the current chain to the destination chain.
     * @param _amount amount of USDC in 18 decimals.
     * @param _destinationAddress destination address to recieve the bridged tokens.
     * @param _destinationDomain the id of the destination chain.
     */
    function _bridgeUSDCWithCCTP(uint256 _amount, bytes32 _destinationAddress, uint32 _destinationDomain) internal {
        address USDC = Bridge._USDC();

        uint256 scaledAmount = _scaleAndCheckUSDCAmount(_amount, USDC);

        emit BridgeToDomainStartedCCTP(USDC, _destinationDomain, _destinationAddress, scaledAmount);

        BridgeIntegrations._CCTPBridgeUSDC(scaledAmount, _destinationAddress, _destinationDomain);
    }
     ...
     * @param _amount amount of USDC in 18 decimals.
     * @param _destinationAddress destination address to recieve the bridged tokens.
     * @param _wormholeChainId the id of the destination chain.
     */
    function _bridgeUSDCWithWormhole(uint256 _amount, bytes32 _destinationAddress, uint16 _wormholeChainId) internal {
        address USDC = Bridge._USDC();

        uint256 scaledAmount = _scaleAndCheckUSDCAmount(_amount, USDC);

        emit BridgeToChainIdStartedWormhole(USDC, _wormholeChainId, _destinationAddress, scaledAmount);

        BridgeIntegrations._wormholeBridgeUSDC(scaledAmount, _destinationAddress, _wormholeChainId);
    }

    /**
     * @notice scales and checks the USDC amount that can be sent to the bridge.
     * @param _amount The amount to send in 18 decimals.
     * @param _USDC Address of the USDC contract.
     * @return scaled amount in the denoted token decimals.
     */
    function _scaleAndCheckUSDCAmount(uint256 _amount, address _USDC) internal returns (uint256) {
        uint256 scaledAmount = uint256(DecimalScaling.scaleTo(int256(_amount), IERC20Metadata(_USDC).decimals()));

        if (scaledAmount > BridgeIntegrations._getBridgeMaxAmount()) revert Error.BridgeMaxAmountExceeded();

        if (scaledAmount > IERC20(_USDC).balanceOf(address(this))) revert Error.InsufficientBalance();
...
/**
 * @title Bridging funds from one chain to another to the same account.
 */
interface IBridgingModule {
    /*///////////////////////////////////////////////////////////////
                    			    EVENTS
    ///////////////////////////////////////////////////////////////*/

    event BridgeToChainIdStartedWormhole(
        address indexed tokenAddress, uint16 wormholeChainId, bytes32 indexed destinationAddress, uint256 amount
    );
    event BridgeToDomainStartedCCTP(
        address indexed tokenAddress, uint32 destinationDomain, bytes32 indexed destinationAddress, uint256 amount
    );
    ...
    /**
     * @notice Bridge USDC tokens from the same address on the current chain to the destination chain.
     * @param _amount is the amount to be sent in usdc.
     * @param _wormholeChainId is the allowlisted wormhole chain ID that usdc can be sent to.
     * @dev The amount defined in amount will be sent as usdc to the specified destination chain.
     * @dev This function takes 18 decimals 'amount' and scales to target token amount.
     * @dev Requires the sender to be an origin key or operation key.
     */
    function bridgeUSDCWithWormholeEVM(uint256 _amount, uint16 _wormholeChainId) external;

    /**
     * @notice Bridge USDC tokens from the current chain to Solana.
     * @param _amount is the amount to be sent in 18 decimals scaled to 6 decimals for usdc.
     * @param _solanaAccount The Solana account.
     * @param _solanaTokenAccount The Solana token account.
     * @param _accountBump A number used to ensure the derived address for the account is unique.
     * @param _tokenAccountBump A number used to ensure the derived for the token account address is unique
     * @dev The amount defined in whe amount will be sent as usdc to the specified destination chain.
     * @dev Requires the sender to be the origin key or operation key.
     */
    function bridgeUSDCWithCCTPSolana(
        uint256 _amount,
        bytes32 _solanaAccount,
...
library SolanaUtils {
    /**
     * @notice Struct containing the Solana account related arguments
     * @param account The Ethereum account
     * @param solanaAccount The Solana account
     * @param solanaWalletSeed The salt used to generate the Solana account (fixed seed "wallet")
     * @param solanaFixedPDASeed The salt used to generate the PDA (Program Derived Address)
     * @param solanaWalletProgramAddress The Solana Wallet Program Address
     * @param accountBump A number used to ensure the derived address for the account is unique
     */
    struct AccountArgs {
        address account;
    ...
    /**
     * @notice Struct containing the Solana token account related arguments
     * @param solanaTokenAccount The Solana token account
     * @param solanaTokenMintAddress The Solana token mint address
     * @param solanaTokenProgramAddress The Solana token address
     * @param solanaAssociatedTokenProgramAddress The Solana ATA program address
     * @param tokenAccountBump A number used to ensure the derived for the token account address is unique
     */
    struct TokenAccountArgs {
        bytes32 solanaTokenAccount;
        bytes32 solanaTokenMintAddress;
        bytes32 solanaTokenProgramAddress;
        bytes32 solanaAssociatedTokenProgramAddress;
        uint8 tokenAccountBump;
    }
    /**
     * @dev Verifies if a Solana account address is valid or not
     * @param _accountArgs Struct containing the Solana account related arguments
     */

    function isAccountValid(AccountArgs memory _accountArgs) internal pure returns (bool) {
        return sha256(
            abi.encodePacked(
                _accountArgs.solanaWalletSeed,
                _accountArgs.account,
                uint8(0), // We always bridge funds to the 0 index/main account
                _accountArgs.accountBump,
                _accountArgs.solanaWalletProgramAddress,
                _accountArgs.solanaFixedPDASeed
            )
        ) == _accountArgs.solanaAccount;
    }

    /**
     * @dev Verifies if a Solana token account address is valid or not
     * @param _accountArgs Struct containing the Solana account related arguments
     * @param _tokenAccountArgs Struct containing the Solana token account related arguments
     */
    function isTokenAccountValid(AccountArgs memory _accountArgs, TokenAccountArgs memory _tokenAccountArgs)
        internal
        pure
        returns (bool)
    {
        return isAccountValid(_accountArgs)
            && sha256(
                abi.encodePacked(
                    _accountArgs.solanaAccount,
                    _tokenAccountArgs.solanaTokenProgramAddress,
                    _tokenAccountArgs.solanaTokenMintAddress,
                    _tokenAccountArgs.tokenAccountBump,
                    _tokenAccountArgs.solanaAssociatedTokenProgramAddress,
                    _accountArgs.solanaFixedPDASeed
                )
            ) == _tokenAccountArgs.solanaTokenAccount;
    }
}