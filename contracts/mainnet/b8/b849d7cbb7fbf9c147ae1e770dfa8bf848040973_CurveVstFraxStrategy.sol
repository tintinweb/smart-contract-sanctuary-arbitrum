...
contract CurveVstFraxStrategy is BaseStrategy {

    uint256 public constant keepMax = 10000;
    uint256 public maxLocks = 10; //max # of locks to have at a time

    address private constant WETH = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1;
    address private constant FXS = 0x9d2F299715D94d8A7E6F5eaa8E654E8c74a988A7;
    ...
    address private constant VSTFRAX = 0x59bF0545FCa0E5Ad48E13DA269faCD2E8C886Ba4;
    address private constant FraxCCFarmV2 = 0x127963A74c07f72D862F2Bdc225226c3251BD117;
    ...
    constructor()
        ...
        BaseStrategy(
            ...
            FUND,
            FXS,
            0xCAAaB0A72f781B92bA63Af27477aA46aB8F653E7, //FraxswapRouter V2
            FraxCCFarmV2
        )
    {
        ...
        IERC20(VSTFRAX).safeApprove(FraxCCFarmV2, uint256(-1));
        IERC20(FXS).safeApprove(currentRouter, uint256(-1));
        IERC20(FRAX).safeApprove(VSTFRAX, uint256(-1));
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        //Convert all harvested FXS to FRAX
        uint256 amt_harvested = IERC20(FXS).balanceOf(address(this));
        if(amt_harvested > 0) {
            _swapUniswapWithPathPreapproved(fxs_frax_path, amt_harvested);
        }

        //Deposit FRAX in pool to get LP tokens
        uint256 fraxBalance = IERC20(FRAX).balanceOf(address(this));
        if (fraxBalance > 0) {
            ICurvePool(VSTFRAX).add_liquidity([0, fraxBalance], 0);
        }
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            ...
            //iterate through the list of locked stakes and count the number of stakes and also find the smallest nonzero stake
            for (uint256 i = 0; i < lockedStakes.length; i++){
                if (lockedStakes[i].liquidity > 0) {
                    if(arr_idx == uint256(-1) || lockedStakes[arr_idx].liquidity > lockedStakes[i].liquidity) {
                        arr_idx = i;
                    }
                    numStakes += 1;
                }
            ...
            //if balance is 0 or if there are less than `maxLocks` stakes, then create a new locked stake
            if(numStakes < maxLocks || balanceOfPool() == 0) {
                IFraxFarm(rewards).stakeLocked(_want, 86400);
            }
            //else, add to the smallest nonzero stake
            else {
        ...
        returns (uint256)
    {
        ...
        if(arr_idx != uint256(-1)) {
            ...
            IFraxFarm(rewards).withdrawLocked(lockedStakes[arr_idx].kek_id);
        }
        ...
        if(amtWithdrew == 0) {
            for (uint256 i = 0; i < lockedStakes.length; i++){
                if (lockedStakes[i].liquidity > 0 && lockedStakes[i].ending_timestamp <= now) {
                    IFraxFarm(rewards).withdrawLocked(lockedStakes[i].kek_id);
        ...
        //It's possible this results in going over `maxLocks` if it was lowered
        //this won't cost much more gas on Arbitrum, but on mainnet, it'd make withdrawing cost a lot more, so consider not relocking there
        if(amtWithdrew > _amount) {
            IFraxFarm(rewards).stakeLocked(amtWithdrew.sub(_amount), 86400);
        }
        return _amount;
    }
    ...
    function withdrawLocked(bytes32 kek_id) public onlyOwner {
        IFraxFarm(rewards).withdrawLocked(kek_id);
    }
