...
contract LPManagementContract {
    IUniswapV3Staker public staker;
    
    constructor() {
        staker = IUniswapV3Staker(0x32652827cc02f77585E7a1eDd83fd515Ae7B8331);
    }
    
    function unstakeStakeAndClaim(
        ...
        for (uint256 i = 0; i < tokenIDs.length; i++) {
            staker.unstakeToken(key, tokenIDs[i]);
            staker.stakeToken(key, tokenIDs[i]);
        }

        
        staker.claimReward(rewardToken, msg.sender, amountRequested);
    }
}