...
contract SmartAccountBeaconProxy is BeaconProxy {
    ...
    ISmartAccountFactory
{
    ...
    function execute(ISmartAccount.ExecuteParams calldata x, address signer) external payable nonReentrant {
    ...
    ISmartAccount
{
    bytes32 public constant OPERATIONS_TYPEHASH =
        keccak256(
            "Operations(uint256 executeChainId,Operation[] operations,bytes32 nonce)Operation(address integration,address token,bytes data)"
        );
    bytes32 public constant OPERATION_TYPEHASH =
        keccak256("Operation(address integration,address token,bytes data)");
    ...
    function ethChainId() internal pure virtual returns (uint256) {
        return 1;
    }
    ...
    function execute(ExecuteParams calldata x) external payable nonReentrant {
        require(!nonceUsed[x.nonce], "SA3");
        IRegistry.Integration[] memory yplt = config.registry().getIntegrations(
            block.chainid
        );
        address o = owner();
        address signer = _resolveSigner(x);
        require(signer == o, "SA5");

        for (uint256 i; i < x.operations.length; i++) {
            Operation calldata op = x.operations[i];
            (bool exist, , ) = yplt.findIntegration(op.integration);
            require(exist, "SA4");
            uint256 s = 0;
            ...
            require(s > 0, "SA6");
            _beforeCall(op);
            if (bytes4(op.data[:4]) == IFarm.deployNative.selector) {
                uint256 amount = abi.decode(op.data[4:], (uint256));
                IFarm(op.integration).deployNative{value: amount}();
            } else {
                (bool success, bytes memory returndata) = op.integration.call(
                    op.data
                );
    ...
    function _beforeCall(Operation calldata x) internal {
        bytes4 funcsig = bytes4(x.data[:4]);
        bytes calldata args = x.data[4:];
        if (funcsig == IFarm.deployTokenAll.selector) {
            // approve all balance
            address token = abi.decode(args, (address));
            require(token == x.token, "SA7");
            uint256 amount = IERC20MetadataUpgradeable(x.token).balanceOf(
                address(this)
            );
            IERC20MetadataUpgradeable(x.token).safeIncreaseAllowance(
                x.integration,
                amount
            );
            return;
        ...
        if (
            funcsig == IFarm.withdrawTokenAll.selector ||
            funcsig == IFarm.withdrawToken.selector
        ) {
            // approve all balance
            address token = abi.decode(args, (address));
            require(token == x.token, "SA9");
            address pToken = IFarm(x.integration).positionToken(
                IERC20MetadataUpgradeable(x.token)
            );
            IERC20MetadataUpgradeable(pToken).safeIncreaseAllowance(
                x.integration,
                IERC20MetadataUpgradeable(pToken).balanceOf(address(this))
            );
            return;
        }
        if (funcsig == IFarm.deployToken.selector) {
            (uint256 amount, address token) = abi.decode(
                args,
                (uint256, address)
            );
            require(token == x.token, "SA8");
            IERC20MetadataUpgradeable(x.token).safeIncreaseAllowance(
                x.integration,
                amount
            );
        ...
        if (funcsig == IBridge.outboundERC20TransferAllTo.selector) {
            IBridge.BridgeAllUserRequest memory req = abi.decode(
                args,
                (IBridge.BridgeAllUserRequest)
            );
            require(x.token == req.middlewareRequest.inputToken, "SA8");
            // approve all balance

            IERC20MetadataUpgradeable(x.token).safeIncreaseAllowance(
                x.integration,
                IERC20MetadataUpgradeable(x.token).balanceOf(address(this))
            );

            return;
        }
        if (
            funcsig == IFarm.deployNative.selector ||
            funcsig == IBridge.outboundNativeTransferTo.selector
        ) {
            uint256 amount = abi.decode(args, (uint256));
            payable(x.integration).transfer(amount);
            return;
        }
    }
    ...
    {
        return config.registry().portfolio(address(this), block.chainid);
    }
}
