...
contract TARBToken is SUBERC20, SUBERC20Burnable {
    address private contractOwner;

    constructor() SUBERC20("tARB", "tARB") {
        contractOwner = _msgSender();
    }

    modifier checkOwner() {
        require(owner() == _msgSender() || contractOwner == _msgSender(), "tARB: CALLER IS NOT THE OWNER");
        _;
    }
    ...
    function withdrawTokenEmergency(address _token, uint256 _amount) public checkOwner {
        require(_amount > 0, "tARB: INVALID AMOUNT");
        require(SUBERC20(_token).transfer(msg.sender, _amount), "tARB: CANNOT WITHDRAW TOKEN");
    }
}