...
/**
 * @title GAST Single Pool Contract
 * @author GAST-TEAM
 * @notice In this contract user can stake GAST and harvest GAST
 */
contract GASTSinglePool is AccessControlEnumerable, ReentrancyGuard {
    ...
    uint256 public taxDynamicRate = 30;
    ...
    event SetAddrs(address treasury, address gastAddr);
    ...
    function setTokenInfo(
        ...
        emit SetTokenInfo(_tokenPerBlock);
    }
    ...
    function setAddrs(
        ...
        emit SetAddrs(_treasury, gastAddr);
    }
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        if (userStakes[msg.sender] > 0) {
            uint256 pendingToken = (userStakes[msg.sender] *
        ...
        if (amount > 0) {
            GAST.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(uint256 amount) external nonReentrant {
        require(userStakes[msg.sender] >= amount, "Not enough token to withdraw");
        ...
        if (amount > 0) {
            ...
            GAST.safeTransfer(treasury, feeAmount);
            GAST.safeTransfer(msg.sender, withdrawAmount);
        }
    ...
    function harvestToken() external nonReentrant {
        ...
        GAST.safeTransfer(msg.sender, amount);
    ...
    function updatePool() public {
        ...
        if (block.number > lastRewardBlock && stakes > 0) {
            uint256 amount = tokenPerBlock * (block.number - lastRewardBlock);
            accTokenPerStake += (amount * 1e18) / stakes;
    ...
    function getTokenRewards(address user) public view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakes > 0) {
            accTokenPerStakeTemp +=
                (tokenPerBlock * (block.number - lastRewardBlock) * 1e18) /
                stakes;
        }
