...
import "../interfaces/IBorrowerBalanceCalculator.sol";
...
abstract contract UniswapV3BaseStrategy is UniswapV3StrategyStorage, AccessControl, IBorrower {
     ...
     */
    function withdraw(uint256 amount) external requireAmmCheck restrictAccess(GOVERNOR | CONTROLLER) trackPriceChangeImpact {
...
contract UniswapV3StrategyStorage {
    ...
    /**
     * @notice Cached data used to calculate the strategy's balance. The data represents the debts and leftovers after
     * attempting to repay all debts
     * @dev The data in the cache can be obtained by calling BorrowerBalanceCalculator.calculateDebtsAndLeftovers(strategyAddress)
     * @param borrowedTokens The tokens that the strategy has borrowed
     * @param borrowedAmounts The debt amount for each borrowed token
     * @param availableAmounts The tokens leftover with the strategy after repaying debts
     * @param avilableAmoutns The amount of each leftover token
     * @param prevCacheUpdateBlock The last block at which the cache was updated
     */
    struct Cache {
        address[] borrowedTokens;
        uint[] borrowedAmounts;
    ...
    /// @notice Cached debts and token amounts to reduce gas consumption
    Cache public cache;
