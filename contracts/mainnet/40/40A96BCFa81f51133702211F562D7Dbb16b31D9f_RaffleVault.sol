...
import "../../extentions/dex/PoolFeeStorage.sol";
...
contract ConcentratedLPVault is TokenizedVault, PoolFeeStorage {
    ...
    function _afterDeposit(uint256, uint256) internal virtual override {
        // Collect all pool fees after deposit.
        _collectPoolFees();
    }
...
import "../../extentions/governance/TimeBonus.sol";
...
import "../../extentions/governance/ProtocolFee.sol";
    ...
    ProtocolFee
{
    ...
    function _setInitialFeeCharge() internal virtual {
        addFeeInfo(0, Fee(FeeType.Bps, 0, 0, msg.sender));  // Management Fee: 0% - on rebalance (TVL)
        addFeeInfo(1, Fee(FeeType.Bps, 3_000, 0, msg.sender));  // Performance Fee: 30% - on rebalance (Fees)
        addFeeInfo(2, Fee(FeeType.Bps, 0, 0, msg.sender));  // Withdrawal Fee: 0% - on withdrawal (TVL)
        addFeeInfo(3, Fee(FeeType.Bps, 500, 0, msg.sender));  // Withdrawal Fee: 5% - on withdrawal (Fees)
    }
...
library FullMath {
    ...
    function mulDiv(
        ...
        // diff: original lib works under 0.8.19 with overflows enabled
        unchecked {
