...
import {BaseEscrowDeployer} from "./BaseEscrowDeployer.sol";
...
contract HomeEscrowDeployer is BaseEscrowDeployer {
    constructor(address _messenger) BaseEscrowDeployer(_messenger, address(new HomeEscrow(_messenger))) {}
}

// SPDX-License-Identifier: BUSL-1.1
pragma solidity ^0.8.23;

import {IHomeEscrow} from "./interfaces/IHomeEscrow.sol";
...
contract HomeEscrow is IHomeEscrow {
    ...
    function tradeAndWithdraw(TradeParams[] calldata trades, Params calldata params) external {
        ...
        for (uint256 i; i < tradesLength; ++i) {
            TradeParams calldata trade = trades[i];

            trade.currency.approve(trade.allowanceTarget, trade.currency.balanceOfSelf());

            (address target, uint96 value) = trade.target.unpackRaw();

            // Skips validation to prevent reverting for successful trades
            target.call{value: trade.currency.isNative() ? address(this).balance : value}(trade.data);

            trade.currency.approve(trade.allowanceTarget, 0);
        }

        uint256 balance = params.outputCurrency.balanceOfSelf();
        if (balance > 0) params.outputCurrency.transfer(params.recipient, balance);
    }
}
...
contract BaseEscrowDeployer is IEscrowDeployer {
    using Clones for address;

    /// @dev The address of the Messenger contract
    address public immutable messenger;

    /// @notice The address of the Escrow implementation contract
    address public immutable escrowImpl;

    constructor(address _messenger, address _escrowImpl) {
        messenger = _messenger;
    ...
    function deploy(address user) external override returns (address escrow) {
        escrow = escrowImpl.cloneDeterministic(keccak256(abi.encodePacked(user)));
    }
    ...
    function escrowOf(address user) external view override returns (address escrow, bool deployed) {
        escrow = escrowImpl.predictDeterministicAddress(keccak256(abi.encodePacked(user)));
