    ...
    UUPSUpgradeable
{
    ...
    constructor(
        ...
        // prettier-ignore
        address poolAddress;
        if (_noteETHPoolId != bytes32(0)) {
            (poolAddress, /* */) = _balancerVault.getPool(_noteETHPoolId);
        }
    ...
    function reinvestVaultReward(address vault, IStrategyVault.ReinvestRewardParams calldata params) 
        ...
        (rewardToken, soldAmount, poolClaimAmount) = IStrategyVault(vault).reinvestReward(params);
        emit VaultRewardReinvested(vault, rewardToken, soldAmount, poolClaimAmount);
    }
