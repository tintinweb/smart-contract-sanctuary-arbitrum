...
contract CompoundMoneyMarketView is IMoneyMarketView {
    ...
    // UD60x18 internal constant BLOCKS_PER_DAY = UD60x18.wrap(60 / 12 * 60 * 24 * 1e18);
    ...
    function _borrowingLiquidity(IERC20 asset) internal view virtual returns (uint256) {
        ...
        uint256 available = (asset == nativeToken ? address(cToken).balance : asset.balanceOf(address(cToken))) * 0.95e18 / 1e18;
    ...
    function _apy(uint256 ratePerSecond) internal pure returns (uint256) {
        ...
        return (ud(ratePerSecond) * ud(_blocksPerDay() * 1e18) + UNIT).pow(DAYS_PER_YEAR).sub(UNIT).unwrap();
    }
    ...
    function _assetUSDPrice(IERC20 asset) internal view virtual returns (uint256) {
        return IUniswapAnchoredView(comptroller.oracle()).price(_symbol(asset)) * 1e12;
    }
    ...
    function _asRewards(PositionId positionId, IERC20 asset, bool borrowing) internal view virtual returns (Reward memory rewards_) {
        IERC20 rewardsToken = IERC20(comptroller.getCompAddress());
        ICToken cToken = _cToken(asset);
        ...
        uint256 valueOfEmissions = emissionsPerYear * rewards_.usdPrice / 1e18;

        uint256 assetPrice = _assetUSDPrice(asset);
        if (borrowing) {
            uint256 assetsBorrowed = cToken.totalBorrows();
            uint256 valueOfAssetsBorrowed = assetPrice * assetsBorrowed / (10 ** (asset.decimals()));

            rewards_.rate = valueOfEmissions * 1e18 / valueOfAssetsBorrowed;
            ...
            uint256 valueOfAssetsSupplied = assetPrice * assetSupplied / 1e18;

            rewards_.rate = valueOfEmissions * 1e18 / valueOfAssetsSupplied;
    ...
    function _borrowerAccruedComp(ICToken cToken, address borrower) internal view returns (uint256) {
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint256 borrowAmount = cToken.totalBorrows() * 1e18 / marketBorrowIndex;
        ...
        uint256 borrowerAmount = cToken.borrowBalanceStored(borrower) * 1e18 / marketBorrowIndex;

        // Calculate COMP accrued: cTokenAmount * accruedPerBorrowedUnit
        uint256 borrowerDelta = borrowerAmount * deltaIndex / 1e36;

        return comptroller.compAccrued(borrower) + borrowerDelta;
    }
    ...
    function _supplierAccruedComp(ICToken cToken, address supplier) internal view returns (uint256) {
        ...
        return comptroller.compAccrued(supplier) + supplierDelta;
    }
