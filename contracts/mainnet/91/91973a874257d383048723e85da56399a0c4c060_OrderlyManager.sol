...
import {IOrderlyManager} from "src/interfaces/IOrderlyManager.sol";
import {IOrderly, VaultTypes} from "src/interfaces/IOrderly.sol";

import {OrderlyRouter} from "src/OrderlyRouter.sol";

/// @title Elixir pool manager for Orderly
/// @author The Elixir Team
/// @custom:security-contact [email protected]
/// @notice Pool manager contract to provide liquidity for market making on Orderly.
contract OrderlyManager is IOrderlyManager, Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuard {
    ...
    /// @notice The Orderly contract.
    IOrderly public orderly;
    ...
    /// @notice Emitted when withdrawal is less than withdrawal fee.
    error WithdrawalIsLessThanFee(uint256 amountToReceive, uint256 fee);
    ...
    /// @param _orderly The Orderly contract address.
    /// @param owner Owner of the contract
    function initialize(address _orderly, address owner) public initializer {
        __UUPSUpgradeable_init();
        __Ownable_init();

        // Set Orderly contract.
        orderly = IOrderly(_orderly);
    ...
    function deposit(uint256 id, uint256 amount, address receiver) external payable whenDepositNotPaused nonReentrant {
        ...
        // Check that the remaining balance is enough for the Orderly fee.
        if (address(this).balance < pool.orderlyDepositFee) {
            revert FeeTooLow(address(this).balance, pool.orderlyDepositFee);
        }
    ...
    /// @dev Requests are placed into a FIFO queue, which is processed by the Elixir market-making network and passed on to Orderly via the `unqueue` function.
    /// @dev After processed by Orderly, the user (or anyone on behalf of it) can call the `claim` function.
    ...
    function claim(address user, uint256 id) external whenClaimNotPaused nonReentrant {
        ...
        OrderlyRouter(pool.router).claimToken(address(pool.token), amount);

        // Transfers the tokens after to prevent reentrancy.
        IERC20Metadata(pool.token).safeTransfer(user, amount);
    ...
    function takeElixirGas(address router) private {
        ...
        (bool sent,) = payable(OrderlyRouter(router).externalAccount()).call{value: fee}("");
    ...
    function processSpot(Spot calldata spot, bytes memory response) public {
        ...
        if (spot.spotType == SpotType.Deposit) {
            ...
            OrderlyRouter router = OrderlyRouter(pool.router);

            // Transfer tokens from the caller to the router.
            IERC20Metadata(pool.token).safeTransferFrom(spot.sender, address(router), spotTxn.amount);

            bytes32 brokerHash = keccak256(abi.encodePacked("elixir"));

            uint256 orderlyDepositFee = orderly.getDepositFee(
                pool.router,
                VaultTypes.VaultDepositFE({
                    accountId: keccak256(abi.encode(address(pool.router), brokerHash)),
                    brokerHash: brokerHash,
                    tokenHash: keccak256(abi.encodePacked(IERC20Metadata(pool.token).symbol())),
                    tokenAmount: uint128(spotTxn.amount)
                })
            );

            // Deposit funds to Orderly through router.
            router.deposit{value: orderlyDepositFee}(spotTxn.amount);
            ...
            if (responseTxn.amountToReceive < pool.orderlyWithdrawalFee) {
                revert WithdrawalIsLessThanFee(responseTxn.amountToReceive, pool.orderlyWithdrawalFee);
            }
            ...
            pool.userPendingAmount[spot.sender] += responseTxn.amountToReceive - pool.orderlyWithdrawalFee;

            emit Withdraw(address(pool.router), spot.sender, responseTxn.amountToReceive - pool.orderlyWithdrawalFee);
        } else {
    ...
    function unqueue(uint128 spotId, bytes memory response) external {
        ...
        address externalAccount = OrderlyRouter(spot.router).externalAccount();
    ...
    /// @param depositFee The Orderly deposit fee for the pool
    /// @param withdrawalFee The Orderly withdrawal fee for the pool
    /// @param hardcap The hardcap for the pool.
    /// @param externalAccount The external account to market make on Orderly.
    /// @param broker The broker name for Orderly.
    function addPool(
        uint256 id,
        address _token,
        uint256 depositFee,
        ...
        OrderlyRouter router = new OrderlyRouter(address(orderly), externalAccount, broker);

        // Approve the fee token to the router.
        router.makeApproval(address(_token));
        ...
        pools[id].token = _token;

        // Set the pool's deposit fee
        pools[id].orderlyDepositFee = depositFee;

        // Set the pool's withdrawal fee
        pools[id].orderlyWithdrawalFee = withdrawalFee;
...
interface IOrderlyManager {
    ...
    struct Pool {
        ...
        // The pool type.
        PoolType poolType;
        // The collateral token of the pool
        address token;
        // The Orderly deposit fee of the pool in native token
        uint256 orderlyDepositFee;
        // The Orderly withdrawal fee of the pool in collateral token
        uint256 orderlyWithdrawalFee;
...
interface IOrderly {
    ...
    function getDepositFee(address receiver, VaultTypes.VaultDepositFE calldata data) external view returns (uint256);
}
...
import {IERC20Metadata} from "openzeppelin/token/ERC20/extensions/IERC20Metadata.sol";
import {SafeERC20} from "openzeppelin/token/ERC20/utils/SafeERC20.sol";
import {VaultTypes} from "src/interfaces/IOrderly.sol";

import {IOrderly} from "src/interfaces/IOrderly.sol";

/// @title Elixir pool router for Orderly
/// @author The Elixir Team
/// @custom:security-contact [email protected]
/// @notice Pool router contract to deposit funds on behalf of users to Orderly. Allows external account to market make for this contract.
contract OrderlyRouter {
    using SafeERC20 for IERC20Metadata;
    ...
    /// @notice The Orderly contract.
    IOrderly public immutable orderly;

    /// @notice The address of the external account to market make on Orderly.
    address public immutable externalAccount;

    /// @notice The Manager contract associated with this Router.
    address public immutable manager;

    /// @notice The hash of the broker.
    bytes32 public immutable brokerHash;

    /// @notice The accountId for this pool.
    bytes32 public immutable accountId;

    /// @notice The token hash for this pool.
    ...
    /// @notice Set the Manager, Orderly, and external account.
    /// @param _externalAccount The address of the external account.
    /// @param broker The broker for Orderly.
    constructor(address _orderly, address _externalAccount, string memory broker) {
        // Set the Manager as the owner.
        manager = msg.sender;

        // Set Orderly's endpoint address.
        orderly = IOrderly(_orderly);
        ...
        // Set the account ID.
        accountId = keccak256(abi.encode(address(this), brokerHash));
    ...
    /// @notice Deposits funds to Orderly on behalf of the user.
    /// @dev Only callable by the Manager contract.
    /// @param amount The amount to deposit.
    function deposit(uint256 amount) external payable onlyManager {
        orderly.deposit{value: msg.value}(
            VaultTypes.VaultDepositFE({
                accountId: accountId,
                brokerHash: brokerHash,
                tokenHash: tokenHash,
    ...
    /// @notice Sets up linked signer and approves Orderly to transfer a token.
    /// @dev Only callable by the Manager contract.
    /// @param token The token to approve.
    function makeApproval(address token) external onlyManager {
        // Set the linked signer.
        orderly.delegateSigner(VaultTypes.VaultDelegate({brokerHash: brokerHash, delegateSigner: externalAccount}));

        // Approve the token transfer.
        IERC20Metadata(token).safeApprove(address(orderly), type(uint256).max);
    }

    /// @notice Allow claims from OrderlyManager contract.
    /// @dev Only callable by the Manager contract.
