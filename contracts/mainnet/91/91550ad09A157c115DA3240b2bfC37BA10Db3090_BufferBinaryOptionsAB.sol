...
contract BufferBinaryOptionsAB is
    IBufferBinaryOptionsAB,
    ReentrancyGuard,
    ERC721,
    AccessControl
{
    ...
    IOptionsConfigAB public override config;
    ...
    function createFromRouter(
        ...
        if (optionParams.isAbove) {
            ...
            markets[marketId].premiumUp += int256(optionParams.totalFee);
        } else {
            markets[marketId].contractsDown += optionParams.contracts;
            markets[marketId].premiumDown += int256(optionParams.totalFee);
        }

        int256 skew = getSkew(marketId);
        require(skew <= getMaxSkew(), "Loss too high");
    ...
    function unlock(
        ...
        closingPrice =
            (closingPrice / config.pricePrecision()) *
            config.pricePrecision();
        uint256 strike = (option.strike / config.pricePrecision()) *
            config.pricePrecision();
        uint256 payout;
        if (
            (option.isAbove && closingPrice > strike) ||
            (!option.isAbove && closingPrice < strike)
        ) {
    ...
    function isExpirationValid(uint256 expiration) public view returns (bool) {
        ...
        bool dayend = (expiration - block.timestamp) < (3 days + 36 hours) &&
            (expiration + 16 hours) % 1 days == 0;
        bool weekend = ((expiration - 1 days - 8 hours) % 1 weeks == 0) &&
            (expiration - block.timestamp < 3 weeks + 36 hours); // Weekends at friday 8:00 AM UTC
    ...
    function getMaxSkew() public view returns (int256) {
        return (config.maxSkew() * int256(pool.availableBalance())) / 1e4;
    }
    ...
    function getMaxPermissibleContracts(
        ...
        int256 maxSkew = getMaxSkew();
        int256 loss = getLoss(marketId, isAbove);
        return
            maxSkew > loss
                ? (uint256(maxSkew - loss) * (10 ** decimals())) /
    ...
    function evaluateParams(
        OptionParams calldata optionParams
    ) external view override returns (RevisedData memory revisedData) {
        ...
        uint256 revisedSf = getSettlementFeePercentage(
        ...
        uint256 fee;
        uint256 revisedContracts = (optionParams.totalFee *
            (10 ** decimals())) / _baseFeePerContract;
        ...
        uint256 amount = (config.payout() * revisedContracts) /
            (10 ** decimals());

        revisedData = RevisedData(amount, fee, revisedContracts, revisedSf);
    }
    ...
    function ownerOf(
        ...
        view
        virtual
        override(ERC721, IBufferBinaryOptionsAB)
        returns (address)
    {
...
interface IOptionsConfigAB {
    ...
    event UpdatePricePrecision(uint32 pricePrecision);
