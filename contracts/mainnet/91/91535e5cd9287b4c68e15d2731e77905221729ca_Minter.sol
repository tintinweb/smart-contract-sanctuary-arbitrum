...
import {Token} from "../token/Token.sol";
import {WrappedToken} from "../token/WrappedToken.sol";
...
import {Allowlist} from "../management/Allowlist.sol";
import {Owned} from "solmate/auth/Owned.sol";
...
import {NonblockingLzApp, ExcessivelySafeCall, BytesLib} from "@layerzerolabs/lzApp/NonblockingLzApp.sol";

/// @notice Base functionalty for the controller.
abstract contract Base is NonblockingLzApp {
    using BytesLib for bytes;

    /// @notice LayerZero parameters.
    uint256 public constant NO_EXTRA_GAS = 0;
    uint16 public constant PT_SEND = 0;
    uint16 public constant PT_REBASE = 1;
    uint16 public constant PT_MERKLE = 2;
    uint256 internal immutable lzChainId;
    bool public useCustomAdapterParams;

    /// @notice Array of trusted remotes.
    ...
    /// @notice Set whether to use custom adapter params.
    function setUseCustomAdapterParams(bool _useCustomAdapterParams) public virtual onlyOwner {
        useCustomAdapterParams = _useCustomAdapterParams;
        emit SetUseCustomAdapterParams(_useCustomAdapterParams);
    }

    /// @notice Set the trusted remote for the given chain.
    function setTrustedRemote(uint16 _remoteChainId, bytes calldata _path) external virtual override onlyOwner {
    ...
    /// @notice Layer zero message handlers.
    ...
    /// @notice Propagate a message to all trusted remotes.
    ...
    /// @notice Estimate the fees for propagating a message to all trusted remotes.
...
import {Base, Initializable} from "./Base.sol";
...
/// @notice Solmate's ERC20 implementation modified for use with OpenZeppelin upgradable contracts.
/// @custom:oz-upgrades
abstract contract ERC20 is Initializable {
    ...
    function __ERC20_init_(
        uint8 _decimals
    ) onlyInitializing public {
        // Name and symbol are set by the child contract.
        decimals = _decimals;
        INITIAL_CHAIN_ID = block.chainid;
        INITIAL_DOMAIN_SEPARATOR = computeDomainSeparator();
    }
