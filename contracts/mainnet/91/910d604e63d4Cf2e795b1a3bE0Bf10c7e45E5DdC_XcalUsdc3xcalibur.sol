...
// File contracts/interfaces/I3xcaliburGauge.sol

pragma solidity 0.8.17;

interface I3xcaliburGauge {
...
// File contracts/interfaces/I3xcaliburRouter.sol

pragma solidity 0.8.17;

interface I3xcaliburRouter {
...
// File contracts/XcalUsdc3xcalibur.sol
...
contract XcalUsdc3xcalibur is Ownable, Pausable {
    using SafeERC20 for IERC20;

    // Tokens
    address public constant weth = address(0x82aF49447D8a07e3bd95BD0d56f35241523fBab1);
    address public constant xcal = address(0xd2568acCD10A4C98e87c44E9920360031ad89fCB);
    ...
    //Routes
    I3xcaliburRouter.Routes[] public xcalToUsdcPath;
    I3xcaliburRouter.Routes[] public customPath;
    ...
    constructor(
        ...
        I3xcaliburRouter.Routes[] memory _xcalToUsdcPath
    ) {
        ...
        for (uint i; i < _xcalToUsdcPath.length; ++i) {
           xcalToUsdcPath.push(_xcalToUsdcPath[i]);
        }

        rewardTokens.push(xcal);
    ...
    function _deposit() internal whenNotPaused {
        ...
        I3xcaliburGauge(gauge).deposit(wantBal, 0);
    }
   ...
   //Withdraws funds from masterchef
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            I3xcaliburGauge(gauge).withdraw(_amount - wantBal);
    ...
    function _harvest(address caller) internal whenNotPaused {
        ...
        I3xcaliburGauge(gauge).getReward(address(this), rewardTokens);
        uint256 outputBal = IERC20(xcal).balanceOf(address(this));
    ...
    function chargeFees(address caller) internal {
       uint256 xcalFee = IERC20(xcal).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;

        uint256 callFee = xcalFee * CALL_FEE / FEE_DIVISOR;
        IERC20(xcal).safeTransfer(caller, callFee);

        uint256 recipientFee = xcalFee * RECIPIENT_FEE / FEE_DIVISOR;
        IERC20(xcal).safeTransfer(grimFeeRecipient, recipientFee);

        uint256 treasuryFee = xcalFee * TREASURY_FEE / FEE_DIVISOR;
        IERC20(xcal).safeTransfer(treasury, treasuryFee);

        uint256 stratFee = xcalFee * STRAT_FEE / FEE_DIVISOR;
        IERC20(xcal).safeTransfer(strategist, stratFee);
    }


    // Converts to both sides of the LP token and builds the liquidity pair
    function addLiquidity() internal {
        uint256 xcalHalf = IERC20(xcal).balanceOf(address(this)) / 2;
        I3xcaliburRouter(router).swapExactTokensForTokens(xcalHalf, 0, xcalToUsdcPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(xcal).balanceOf(address(this));
        uint256 t2Bal = IERC20(usdc).balanceOf(address(this));

        I3xcaliburRouter(router).addLiquidity(xcal, usdc, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 wethOut;
        if (outputBal > 0) {
            (wethOut,) = I3xcaliburRouter(router).getAmountOut(outputBal, xcal, weth);
        }
    ...
    function rewardBalance() public view returns (uint256) {
        return I3xcaliburGauge(gauge).earned(xcal, address(this));
    }
    ...
    function balanceOfPool() public view returns (uint256) {
        return I3xcaliburGauge(gauge).balanceOf(address(this));
    }
    ...
    function retireStrat() external {
        require(msg.sender == vault, "!vault");
        _harvest(msg.sender);
        I3xcaliburGauge(gauge).withdraw(balanceOfPool());
    ...
    function _subAllowance() internal {
        ...
        IERC20(xcal).safeApprove(router, 0);
    ...
    function _addAllowance() internal {
        ...
        IERC20(xcal).safeApprove(router, type(uint).max);
        IERC20(weth).safeApprove(router, type(uint).max);
        IERC20(usdc).safeApprove(router, type(uint).max);
    }
    ...
    function makeCustomTxn(address [][] memory _tokens, bool[] calldata _stable) external onlyAdmin {
        require(_tokens[0][0] != xcal && _tokens[_tokens.length-1][1] == xcal, "path");
        for (uint i; i < _tokens.length; ++i) {
            customPath.push(I3xcaliburRouter.Routes({
                from: _tokens[i][0],
                to: _tokens[i][1],
                stable: _stable[i]
            }));
        ...
        I3xcaliburRouter(router).swapExactTokensForTokens(tokenBal, 0, customPath, address(this), block.timestamp + 600);
