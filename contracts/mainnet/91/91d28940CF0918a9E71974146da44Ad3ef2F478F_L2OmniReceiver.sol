...
contract L2OmniReceiver is IERC165 ,IAny2EVMMessageReceiver,AccessControlEnumerable, NewProxy {
    ...
    mapping(uint64 => bool)             public valid_chains;
    ...
    event RootSenderUpdated(address _old,address _new);
    ...
    address                     public rootSender;

   
    error InvalidSender(address expected, address actual);

    constructor(address _galaxisRegistry,address _rootCommunityList) NewProxy(_galaxisRegistry){
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(RECEIVER_ADMIN,msg.sender);
        _setupRole(COMMUNITY_REGISTRY_ADMIN,address(this)); // to allow setting of home chain
        rootSender = _rootCommunityList;
    }

    function updateRootSender(address _rootSender) external onlyRole(RECEIVER_ADMIN) {
        emit RootSenderUpdated(rootSender ,_rootSender);
        rootSender = _rootSender;
    }
        ...
        validChain(message.sourceChainSelector)  
    {
        ...
        require(message.data.length > 24,"invalid data length");
        uint32 community_id            = uint32(bytes4(message.data[0:4]));
        address community_admin         = address(bytes20(message.data[4:24]));
        string  memory community_name   = string(message.data[24:message.data.length]);
    ...
    function _createCommunity(
        ...
        address community_proxy = newProxy("GOLDEN_COMMUNITY_REGISTRY"); // all golden contracts should start with `GOLDEN_`
        ...
        address cl = galaxisRegistry.getRegistryAddress("COMMUNITY_LIST"); // one list so from galaxis registry
    ...
    function addIncomingChain(uint64 chainId, bool valid) external onlyRole(RECEIVER_ADMIN) {
        valid_chains[chainId] = valid;
    }

    modifier onlyRouter() {
        require(msg.sender == galaxisRegistry.getRegistryAddress("CCIP_ROUTER"),"L2_OmniReceiver : Unauthorised");
        _;
    }

    modifier validChain(uint64 chainSelector) {
        if (!valid_chains[chainSelector]) revert InvalidChain(chainSelector);
        _;
    }
...
contract OmniSender is Initializable, AccessControlEnumerableUpgradeable, UsesGalaxisRegistry {
    ...
    function sendDataPayFeeToken(
        ...
        bytes memory data
    ) external onlyRole(CONTRACT_ADMIN) returns (bytes32 messageId) {
        IRouterClient i_router = IRouterClient(galaxisRegistry.getRegistryAddress("CCIP_ROUTER"));
        ...
        Client.EVM2AnyMessage memory message = Client.EVM2AnyMessage({
            ...
            extraArgs: s_chains[destinationChainSelector],
            feeToken: galaxisRegistry.getRegistryAddress("CCIP_FEE_TOKEN")
        });
    ...
    function addChain(uint64 chainId, bytes calldata description) external onlyRole(CONTRACT_ADMIN) {
        s_chains[chainId] = description;
        emit ChainAdded(chainId,description);
    }
