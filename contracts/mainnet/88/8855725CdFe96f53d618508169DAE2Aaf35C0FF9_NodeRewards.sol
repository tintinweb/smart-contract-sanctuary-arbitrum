...
contract NodeRewards is NodeRewardsBase, RewardsKYC {
    ...
    constructor(
        ...
        address _kycAdmin
    ) NodeRewardsBase(_referee, _nodeKey, _rewardToken) RewardsKYC(_kycAdmin) {
...
abstract contract RewardsKYC {
    using EnumerableSet for EnumerableSet.AddressSet;

    error OnlyKycAdmin();
    error OnlyKycWallet();

    address public immutable kycAdmin;
    EnumerableSet.AddressSet internal _kycWallets;

    modifier onlyKycAdmin() {
        if (msg.sender != kycAdmin) {
            revert OnlyKycAdmin();
        }
        _;
    }

    constructor(address _kycAdmin) {
        kycAdmin = _kycAdmin;
    }

    function addKycWallets(address[] calldata _wallets) external onlyKycAdmin {
