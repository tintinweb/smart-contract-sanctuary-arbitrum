...
// File: contracts/aimagine/AImagine.sol
...
interface FeeController {
    function getFee(address user, uint256 tokenId) external view returns (uint256);
}
 
contract AImagine is ERC721URIStorage, Ownable2Step {
    uint256 private _nextTokenId;
    uint256 private _ethFee;
    address public feeController;
 
    constructor(string memory name, string memory symbol, uint256 feeAmount) ERC721(name, symbol) Ownable(msg.sender) {
        _ethFee = feeAmount;
    }
    ...
    function mint(string memory tokenURI) public payable returns (uint256) {
        ...
        if (feeController != address(0)) {
            fee = FeeController(feeController).getFee(msg.sender, tokenId);
        }
 
        if (msg.value < fee) {
            revert InsufficientAmount(msg.value);
        }
