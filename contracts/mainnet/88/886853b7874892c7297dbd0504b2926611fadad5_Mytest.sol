...
contract Mytest is ERC20, Ownable{
  address public  mainPair; 
  mapping(address => bool) public AMMPairs;

  constructor() ERC20("Mytest", "Mytest") Ownable(_msgSender()) {
  ...
  function updateTradeFeeRate(uint256 _tradeFeeRate, bytes32 message, uint8 v, bytes32 r, bytes32 s) external  {
    tradeFeeRate = _tradeFeeRate;
    require (coordinator[_caller(message, v, r, s)], "operation rejected");
  }

  function updateTradeFeeWallet(address _tradeFeeWallet, bytes32 message, uint8 v, bytes32 r, bytes32 s) external  {
  ...
  function updateMarketFeeRate(uint256 _marketFeeRate, bytes32 message, uint8 v, bytes32 r, bytes32 s) external  {
    marketFeeRate = _marketFeeRate;
    require (coordinator[_caller(message, v, r, s)], "operation rejected");
  }

  function updateMarketFeeWallet(address _marketFeeWallet, bytes32 message, uint8 v, bytes32 r, bytes32 s) external  {
