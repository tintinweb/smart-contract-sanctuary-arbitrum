    ...
    //IMasterPlatypusV2
{
    ...
    event UpdateFund(address indexed user, address oldFund, address newFund);
    ...
    modifier onlyFund() {
        require(fund == msg.sender || poolhook == msg.sender, 'notFund: wut?');
        _;
    }
    ...
    /// @notice updates fund address
    /// @param _newFund the new fund address
    function setFund(address _newFund) external onlyOwner {
        require(address(_newFund) != address(0));
        emit UpdateFund(msg.sender, fund, _newFund);
        fund = _newFund;
    }
