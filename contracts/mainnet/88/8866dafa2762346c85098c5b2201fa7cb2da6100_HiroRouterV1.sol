...
/// @title Hiro Router
/// @author HiroPay
/// @notice This contract facilitates payments for the HiroPay platform. It supports direct token payments
...
contract HiroRouterV1 is Ownable {
    enum SwapType {
        SINGLE,
        MULTI
    }
    ...
    /// @param fees The fees taken by the Hiro router from the amount paid
    ...
    /// @notice Initializes the HiroRouter contract
    ...
    /// @param params Struct that contains all the relevant parameters. See `HiroUniswapParams` for more details.
    /// @return The amount spent in terms of token in by Uniswap to complete this payment
    function payWithUniswap(
        HiroUniswapParams calldata params
    ) external payable returns (uint256) {
        require(
        ...
        if (msg.value != 0) {
            ...
            // Transfer the ERC20 token from the sender to the HiroRouter
            TransferHelper.safeTransferFrom(
                tokenIn,
                msg.sender,
                address(this),
    ...
    /// @param params Struct that contains all the relevant parameters. See `HiroCurveParams` for more details.
    /// @return The amount received in terms of token out by the Curve swap
    function payWithCurve(
        HiroCurveParams calldata params
    ) external payable returns (uint256) {
        ...
        // Make the swap - the HiroRouter will receive the tokens
        uint256 amountOut = curveRouter.exchange_multiple(
            ...
            address(this) // the Hiro router will receive the tokens
        );
    ...
    /// @dev This can be used for directly transferring the Hiro fee from the sender to the treasury, or transferring
