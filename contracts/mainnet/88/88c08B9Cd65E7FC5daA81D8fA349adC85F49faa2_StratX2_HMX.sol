...
contract StratX2_HMX is BaseStrategyV2 {
    ...
    function initialize(
        ...
        _addRewardToken(IERC20(usdc));
        _addRewardToken(IERC20(esHMX));
        _addRewardToken(IERC20(arb));
    }
    ...
    function _vaultPending() public override view returns (uint256[] memory pending) {
        pending = new uint256[](3);

        address[] memory rewarders = hLPStaking.getRewarders();
        pending[0] = IRewarder(rewarders[0]).pendingReward(address(this));
        pending[1] = IRewarder(rewarders[1]).pendingReward(address(this)) + IRewarder(rewarders[2]).pendingReward(address(this));
        pending[2] = IRewarder(rewarders[3]).pendingReward(address(this));
    }
}
...
interface IStrategy {
    ...
    function onReward(address _user, uint256 _amount) external;
    ...
    function pendingToken(address _user) external view returns (address[] memory, uint256[] memory);
}
