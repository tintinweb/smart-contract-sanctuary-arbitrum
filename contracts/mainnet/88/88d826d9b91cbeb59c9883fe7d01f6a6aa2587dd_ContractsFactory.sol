...
contract ContractsFactory is Ownable2StepUpgradeable, IContractsFactory {
    ...
    EnumerableSetUpgradeable.AddressSet private _allowedGlobalTokens;

    UpgradeableBeacon private _beaconProxyForTraderWallet;
    UpgradeableBeacon private _beaconProxyForUsersVault;

    EnumerableSetUpgradeable.UintSet private _allowedGlobalProtocols;
    EnumerableSetUpgradeable.AddressSet private _tokensForAllowance;
    address public defaultUnderlyingTokenAddress;
    ...
    function setDefaultUnderlyingTokenAddress(address _newUnderlyingTokenAddress) external override onlyOwner {
        if (_newUnderlyingTokenAddress == address(0)) {
            revert ZeroAddress("_newUnderlyingTokenAddress");
        }
        defaultUnderlyingTokenAddress = _newUnderlyingTokenAddress;

        emit DefaultUnderlyingTokenAddressSet(defaultUnderlyingTokenAddress);
    }

    function setUsersVaultImplementation(
        address newImplementation
    ) external override onlyOwner {
        _checkZeroAddress(newImplementation, "newImplementation");
    ...
    function addGlobalAllowedProtocols(
        uint256[] calldata _protocols
    ) external override onlyOwner {
        uint256 length = _protocols.length;
        for (uint256 i; i < length; ) {
            uint256 protocol = _protocols[i];

            _allowedGlobalProtocols.add(protocol);
    ...
    function removeGlobalProtocol(uint256 protocol) external override onlyOwner {
        if (!_allowedGlobalProtocols.remove(protocol)) {
            revert InvalidToken();
        }
    ...
    function addTokensForAllowance(
        address[] calldata _tokens
    ) external override onlyOwner {
        uint256 length = _tokens.length;
        for (uint256 i; i < length; ) {
            ...
            _tokensForAllowance.add(token);

            emit TokensForAllowanceAdded(token);
    ...
    function removeTokensForAllowance(address token) external override onlyOwner {
        if (!_tokensForAllowance.remove(token)) {
            revert InvalidToken();
        }

        emit TokensForAllowanceRemoved(token);
    }
    ...
    function deployTraderWallet(
        ...
        _deployTraderWallet(underlyingTokenAddress, traderAddress, _owner, false);
    }

    function _deployTraderWallet(
        address underlyingTokenAddress,
        address traderAddress,
        address _owner,
        ...
        if(!_publicCreation && !allowedTraders[traderAddress]) revert InvalidTrader();
    ...
    function deployUsersVault(
       ...
       _deployUsersVault(traderWalletAddress, _owner, sharesName, sharesSymbol);
    }

    function _deployUsersVault(
        address traderWalletAddress,
        address _owner,
        string memory sharesName,
        string memory sharesSymbol
        ...
        return newUsersVault;
    }

    function setupPublicTraderAccount(
        address traderAddress,
        address _owner,
        string memory sharesName,
        string memory sharesSymbol
    ) external {
        require(defaultUnderlyingTokenAddress != address(0), "default underyling token address has not been setup");

        address[] memory allowedTokens = getAllowedGlobalTokens();
        uint256[] memory protocols = getAllowedGlobalProtocols();
        address[] memory tokensForAllowance = getTokensForAllowance();

        return _setupPublicTraderAccount(defaultUnderlyingTokenAddress, traderAddress, _owner, sharesName, sharesSymbol, protocols, allowedTokens, tokensForAllowance);
    }

    function _setupPublicTraderAccount(
        address underlyingTokenAddress,
        address traderAddress,
        address _owner,
        string memory sharesName,
        string memory sharesSymbol,
        uint256[] memory protocols,
        address[] memory allowedTokens,
        address[] memory tokensForAllowance
    ) internal {
        address _traderWalletAddress = _deployTraderWallet(underlyingTokenAddress, traderAddress, _owner, true);
        address _usersVaultAddress = _deployUsersVault(_traderWalletAddress, _owner, sharesName, sharesSymbol);

        /** Rollover the trader wallet */
        ITraderWallet(_traderWalletAddress).rollover();

        /** Add protocol to use */
        for(uint256 i = 0; i < protocols.length; i++) {
            ITraderWallet(_traderWalletAddress).addProtocolToUse(protocols[i]);
        }

        /** Add token supported */
        ITraderWallet(_traderWalletAddress).addAllowedTradeIndexTokens(allowedTokens);
        
        /** set adapter allowance for uniswap adapter (TW & UV) */
        for(uint256 i = 0; i < tokensForAllowance.length; i++) {
            ITraderWallet(_traderWalletAddress).setAdapterAllowanceOnToken(
                2, // uniswap
                tokensForAllowance[i],
                false
            );

            IUsersVault(_usersVaultAddress).setAdapterAllowanceOnToken(
                2, // uniswap
                tokensForAllowance[i],
                false
            );
    ...
    function getAllowedGlobalProtocols()
        public
        view
        override
        returns (uint256[] memory)
    {
        return _allowedGlobalProtocols.values();
    }

    function getTokensForAllowance()
        public
        view
        override
        returns (address[] memory)
    {
        return _tokensForAllowance.values();
    }
