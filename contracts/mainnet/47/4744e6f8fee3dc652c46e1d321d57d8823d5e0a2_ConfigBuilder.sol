...
contract ConfigBuilder is IConfigBuilder, IConfigMigration {
    ...
    constructor(address _governance, address _messenger, address _priceOracleDeployer, address _index) {
        ...
        reserve = IBaseIndex(_index).reserve();
    ...
    function finishRebalancing(IVault.RebalancingResult[] calldata results, Config calldata config)
        external
        only(governance)
        returns (IBaseIndex.DepositConfig memory, IBaseIndex.RedemptionConfig memory)
    {
    ...
    function setState(IConfigMigration.State memory) external virtual only(governance) {
        // will be implemented in the new versions of ConfigBuilder
        revert Forbidden();
    }
        ...
        returns (RebalancingLib.RebalancingResult memory result)
    {
        ...
        uint96 reserveBalance = IBaseIndex(index).reserveBalance();
        result = RebalancingLib.previewRebalancingOrders(
            currenciesHashOf,
            _currentPricesAndValuations(IBaseIndex(index).kSelf(), reserveBalance, params.payload),
            params,
        ...
        returns (RebalancingLib.ChainOrders[] memory orders)
    {
        ...
        orders = RebalancingLib.previewReserveRebalancingOrders(
            currenciesHashOf, weights, params, IBaseIndex(index).reserveBalance(), reserve, chainRegistry.chainsHash
        );
    }
    ...
    function _buildConfigs(Config calldata config)
        ...
        returns (IBaseIndex.DepositConfig memory deposit, IBaseIndex.RedemptionConfig memory redemption)
    {
        IBaseIndex.Config memory shared = _buildSharedConfig(config);
        ...
        if (hasRemote) redemption.forwarder = messenger;
    }

    function _buildSharedConfig(Config calldata config) internal view returns (IBaseIndex.Config memory shared) {
...
/// @title A library for managing a registry of chains
library ChainRegistryLib {
    ...
    /// @dev Thrown when trying to register an already registered chainId
    error AlreadyRegistered(uint256);
    /// @dev Thrown when a requested chainId is not registered.
    error NotRegistered();
    /// @dev Thrown when max amount of chains reached.
