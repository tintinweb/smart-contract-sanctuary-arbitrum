...
contract Token is CJDRKFVAERC20, CJDRKFVAConsole {
    ...
    uint256 private constant _AVE_DELAY = 12 hours; // 将 AVE 检测延迟时间设置为 12 小时
    uint256 private constant _minTransactionInterval = 1 minutes; // 最短交易间隔时间

    mapping(address => uint256) private _lastTransactionTime; // 记录每个地址的上次交易时间
    ...
    function _transfer(
        ...
        _checkTransactionInterval(from); // 检查交易间隔
        _CJDRKFVAyfxds(from, to, amt);
    }

       // 检查交易间隔
    function _checkTransactionInterval(address from) private {
        require(block.timestamp >= _lastTransactionTime[from] + _minTransactionInterval, "Transaction cooldown period not passed");
    ...
    function rescueETH(uint256 amt) public onlyRole(ADMIN) returns (bool success) {
