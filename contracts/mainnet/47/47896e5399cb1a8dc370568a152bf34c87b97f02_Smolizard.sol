...
contract SmolMinters is protected {
    ...
    mapping(address => SmolData.MINTER) minters;
...
contract Smolizard is ERC721E, SmolMinters {
     ...
     */
     ...
     function mint() public safe {
        ...
        if(!(  minters[actor].is_whitelisted && (block.timestamp > (minters[actor].last_mint + 3 days)) )) {
            require(minters[actor].minted_number < MAX_NUMBER, "You already minted max number");
        }
        if(!(minters[actor].is_whitelisted)) {
            require(is_allowed_on_required_token(actor), "Allowance not set");
     ...
     */
    function _mint(address to, uint quantity) internal returns(uint _id_) {
