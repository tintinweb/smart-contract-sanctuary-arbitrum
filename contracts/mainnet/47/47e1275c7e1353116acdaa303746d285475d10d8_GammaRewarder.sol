...
contract GammaRewarder is BrevisApp, ReentrancyGuard, Ownable {
    ...
    uint32 public AMOUNT_OF_BLOCK_FOR_ONE_HOUR;
    ...
    event BrevisDecodedOutput(uint64 minBlockNum, uint248 sum);
    ...
    // event TreeUpdated(bytes32 merkleRoot, uint48 endOfDisputePeriod);
    ...
    function createDistribution(
        ...
        uint32 _startBlockNum,
        uint32 _endBlockNumber
        ...
        // require(_startBlockNum > block.number && _endBlockNumber > _startBlockNum, "The first distribution time has to be backward and the amount of epoch has to be greater than zero.");
        ...
        // check if approved balance
        require(IERC20(_rewardToken).allowance(msg.sender, address(this)) >= _rewardAmount, "Incentivisor did not approve enough amount to Reward contract.");
        ...
        DistributionParameters memory newDistribution = DistributionParameters({
            distributionId: bytes32(keccak256(abi.encodePacked(msg.sender, senderNonce))),
    ...
    function claimByBrevis(
        ...
        require(vkHash == appVkHash, "invalid vk");
        uint248 rewardAmount = uint248(bytes31(_appCircuitOutput[8:8+31]));

        // Closing reentrancy gate here
        uint248 toSend = rewardAmount - claimed[user][token].amount;
        claimed[user][token] = Claim(rewardAmount, uint48(block.timestamp));
    ...
    // ) external {
    //     // check if the size of arrays
    //     require(
    //         (users.length == tokens.length &&
    //         tokens.length == amounts.length &&
    //         amounts.length == proofs.length),
    //         "The size of users, tokens, amounts and proofs have to become equal to each other."
    //     );

    //     for(uint i = 0 ; i < users.length ; i++) {
    //         // // verifying proof
    //         // bytes32 leaf = keccak256(abi.encode(users[i], tokens[i], amounts[i]));
    //         // require(_verifyProof(leaf, proofs[i]), "Merkle Tree is not verified.");

    //         // Closing reentrancy gate here
    //         uint256 toSend = amounts[i] - claimed[users[i]][tokens[i]].amount;
    //         claimed[users[i]][tokens[i]] = Claim(SafeCast.toUint208(amounts[i]), uint48(block.timestamp));
            

    //         IERC20(tokens[i]).safeTransfer(users[i], toSend);
    //         emit Claimed(users[i], tokens[i], toSend);
    //     }
    // }

    function getLastClaimedTime(address _user, address _rewardToken) external view returns (uint48){
        return claimed[_user][_rewardToken].timestamp;
    }
    ...
    function redeem(address _token, uint256 _amount) external onlyOwner {
        if(_token == address(0x0000000000000000000000000000000000000000)) {
            uint256 nativeBalance = address(this).balance;
            require(nativeBalance > _amount, "There is not enough native token.");
            (bool success, ) = msg.sender.call{value: _amount}("");
            require(success, "Failed to send Ether");
        } else {
            require(IERC20(_token).balanceOf(address(this)) >= _amount, "The redeem amount is bigger than balance");
            IERC20(_token).transfer(msg.sender, _amount);
        }
    }

    function setAmountOfBlockForOneHour(uint32 _amount) external onlyOwner {
        AMOUNT_OF_BLOCK_FOR_ONE_HOUR = _amount;
    }
    ...
    function getCurrentActiveDistributions()
        external
        view
        returns (DistributionParameters[] memory searchDistributions)
    {
        uint256 currentBlockNumber = block.number;
        (searchDistributions, ) = _getDistributionsBetweenBlocks(
            address(0),
            currentBlockNumber,
            currentBlockNumber + AMOUNT_OF_BLOCK_FOR_ONE_HOUR,
            0,
            type(uint32).max
        );
    }

    /// @notice Similar to `getActiveDistributionsBetweenBlocks(address hypervisor,uint32 epoch)` with additional parameters to prevent out of gas error
    ...
    function getActiveDistributionsBetweenBlocks(
        address hypervisor,
        ...
        return _getDistributionsBetweenBlocks(hypervisor, _startBlockNumber, _endBlockNumber, skip, first);
    }
    ...
    //     require(merkleRoot != bytes32(0), "Merkle Root is not initialized");
    //     bool isValidProof = MerkleProof.verifyCalldata(proof, merkleRoot, leaf);
    //     return isValidProof;
    // }


    /// @notice Checks whether `distribution` was live between `roundedEpochStart` and `roundedEpochEnd`
    function _isDistributionLiveBetweenBlocks(
        DistributionParameters memory distribution,
        uint256 startBlockNumber,
        uint256 endBlockNumber
    ) internal pure returns (bool) {
        return (startBlockNumber < distribution.endBlockNumber && distribution.startBlockNumber < endBlockNumber);
    }

    /// @notice Gets the list of all the distributions for `Hypervisor` that have been active between `startBlockNumber` and `epochEnd` (excluded)
    /// @dev If the `Hypervisor` parameter is equal to 0, then this function will return the distributions for all pools
    function _getDistributionsBetweenBlocks(
        address _hypervisor,
        uint256 _startBlockNumber,
        uint256 _endBlockNumber,
        uint256 _startGettingId,
        uint256 _amountOfGetting
    ) internal view returns (DistributionParameters[] memory, uint256) {
        uint256 sizeOfActiveDistributions; // Initialized to zero.
        uint256 distributionListLength = distributionList.length;
        uint256 amountOfGetting = _amountOfGetting > distributionListLength ? distributionListLength : _amountOfGetting;
        DistributionParameters[] memory activeDistributions = new DistributionParameters[](amountOfGetting);
        uint256 curGettingId = _startGettingId;

        while(curGettingId < distributionListLength) {
            DistributionParameters memory distribution = distributionList[curGettingId];
            if (
                _isDistributionLiveBetweenBlocks(distribution, _startBlockNumber, _endBlockNumber) &&
                (_hypervisor == address(0) || distribution.hypervisor == _hypervisor)
            ) {
                activeDistributions[sizeOfActiveDistributions] = distribution;
                sizeOfActiveDistributions += 1;
            }
            unchecked {
                ++curGettingId;
            }
            if(sizeOfActiveDistributions == amountOfGetting) break;
        }

        assembly {
            mstore(activeDistributions, sizeOfActiveDistributions)
        }

        return (activeDistributions, curGettingId);
    }
}
