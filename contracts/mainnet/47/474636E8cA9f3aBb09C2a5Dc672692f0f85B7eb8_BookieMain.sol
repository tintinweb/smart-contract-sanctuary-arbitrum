...
contract BookieMain is IBookieMain, AccessHandler {
        ...
        whenNotPaused
    {
        ...
        BetData.Bet memory bet = BetData.Bet({
            ...
            bytes32 ethHash = ECDSA.toEthSignedMessageHash(betHash);

            if (!hasRole(SIGNER_ROLE, ECDSA.recover(ethHash, signature))) {
    ...
    function createBet(BetData.Bet memory bet) private {
        ...
        if (marketHistory.getMarketState(bet.marketHash) == IMarketHistory.MarketState.Undefined)
...
import "./utils/IRewardHandler.sol";
    ...
    Pausable
{
     ...
     */
    function removeAdmin(address inAdmin)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
