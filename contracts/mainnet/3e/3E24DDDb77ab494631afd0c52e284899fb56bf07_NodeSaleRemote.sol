...
import '../interface/INodeSaleRemote.sol';
import '../util/SaleLibrary.sol';

contract NodeSaleRemote is
  Initializable,
  INodeSaleRemote,
  PausableUpgradeable,
  OwnableUpgradeable,
  ReentrancyGuardUpgradeable
{
  ...
  struct Summary {
    uint256 received; // node received
   ...
   * @param _payment Token to raise
   * @param _dstPaymentDecimals Payment decimal used in target sale on destination chain
   * @param _targetSale Target sale on destination chain (polygon)
   */
  function init(
    uint128 _start,
    uint128 _duration,
    ...
    do {
      ...
      if (i < 2) booster[i].end = booster[i].start + _duration;
  ...
  function _payload(uint256 _amountIn, address _user) internal view virtual returns (bytes memory payload) {
    // change to `dstPaymentDecimals` decimal
  ...
  function calcUserAllocation(address _user, uint128 _boosterRunning) public view virtual returns (uint256 userAlloc) {
    if (_boosterRunning == 3) {
      if (
        (stakers.length > 0 && stakers[stakerIndex[_user]] == _user && userTotalStaked(_user) >= minKomStaked) ||
        whitelist[_user] > 0
   ...
   * @param _nodeReceived Node received amount
   * @param _user User address
  ...
  function _amountInCalc(
    uint256 _alloc,
    uint256 _nodeReceived,
    address _user,
    uint128 _running,
    uint256 _boosterPrice
  ) internal view virtual returns (uint256 amountInFinal, uint256 nodeReceivedFinal) {
    uint256 left = sale - sold;

    if (_nodeReceived > left) _nodeReceived = left;

    amountInFinal = _nodeReceived * _boosterPrice;

    if (_running == 3) {
      require(maxFCFSBuy > 0 && _nodeReceived >= minFCFSBuy, '<min');
    } else if (_running == 4) {
      require(maxComBuy > 0 && _nodeReceived >= minComBuy, '<min');
    }

    uint256 purchaseThisRound = purchasedPerRound[_user][_running];

    if (purchaseThisRound + _nodeReceived > _alloc)
      amountInFinal = (_alloc - purchaseThisRound) * _boosterPrice;

    require(purchaseThisRound < _alloc && amountInFinal > 0, 'nope');

    nodeReceivedFinal = amountInFinal / _boosterPrice;
  }
  ...
  /**
   * @dev Buy node project using token raise (keep using `buyToken` naming to ease saleGateway to interact)
  ...
  function buyToken(uint256 _amountIn) external payable virtual whenNotPaused nonReentrant {
    ...
    (uint256 amountInFinal, uint256 nodeReceivedFinal) = _amountInCalc(
      calcAllocation,
      _amountIn / boosterPrice,
      user,
      running,
      boosterPrice
    );
    ...
    revenue += feeCharged;
    sold += nodeReceivedFinal;
    if (running == 1) booster1Achieved += nodeReceivedFinal;

    summaries[user].received += nodeReceivedFinal;
    ...
    emit NodeBought(running, user, nodeReceivedFinal, amountInFinal, feeCharged);
  }
   ...
   * @param _chainID ChainID source
   * @param _users User address
   * @param _stakedAmount Staked amount in KOM decimal
   */
  function setStakers(
    uint240 _chainID,
    ...
    uint256 chainStakedTotal = chainStaked[_chainID];
    for (uint256 i = 0; i < _users.length; ++i) {
      if (
        userChainStaked[_chainID][_users[i]] > 0 ||
        stakers.length > 0 && stakers[stakerIndex[_users[i]]] == _users[i]
      ) continue;

      stakerIndex[_users[i]] = stakers.length;
      stakers.push(_users[i]);

      userChainStaked[_chainID][_users[i]] = _stakedAmount[i];
      chainStakedTotal += _stakedAmount[i];
    }

    chainStaked[_chainID] = chainStakedTotal;
  }

  function updateStakers(
    uint240 _chainID,
    address[] calldata _users,
    uint256[] calldata _newStakedAmount
  ) external virtual onlyOwner {
    _isNotStarted();

    uint240 chainID = uint240(factory.allChainsStaked(factory.getChainStakedIndex(_chainID)));

    require(_chainID == chainID && stakers.length > 0 , 'bad');

    uint256 chainStakedTotal = chainStaked[_chainID];
    for (uint256 i = 0; i < _users.length; ++i) {
      if (userChainStaked[_chainID][_users[i]] == 0 || stakers[stakerIndex[_users[i]]] != _users[i]) continue;

      uint256 oldStaked = userChainStaked[_chainID][_users[i]];
      userChainStaked[_chainID][_users[i]] = _newStakedAmount[i];
      chainStakedTotal = chainStakedTotal - oldStaked + _newStakedAmount[i];
    }
  ...
  /**
   * @dev Set whitelist allocation node
   * @param _user User address
   * @param _allocation_d0 Node allocation in 0 decimal
   */
  function setWhitelist(address[] calldata _user, uint256[] calldata _allocation_d0) external virtual onlyOwner {
    _isNotStarted();
    require(_user.length == _allocation_d0.length, 'bad');
    ...
    for (uint256 i = 0; i < _user.length; ++i) {
      ...
      whitelist[_user[i]] = _allocation_d0[i];
      whitelistTotal += _allocation_d0[i];
    }
  ...
  /**
   * @dev Update whitelist allocation node
   * @param _user User address
   * @param _allocation_d0 Node allocation in 0 decimal
   */
  function updateWhitelist(address[] calldata _user, uint256[] calldata _allocation_d0) external virtual onlyOwner {
    _isNotStarted();
    require(_user.length == _allocation_d0.length, 'bad');
    ...
    for (uint256 i = 0; i < _user.length; ++i) {
      ...
      whitelist[_user[i]] = _allocation_d0[i];
      whitelistTotal = whitelistTotal - oldAlloc + _allocation_d0[i];
    }
  ...
  function removePurchase(address _user) external virtual onlyOwner {
    require(boosterProgress() == 2 && paused(), 'bad');
   ...
   * @param _minMaxFCFSBuy Min and max node to buy
   */
   ...
   * @param _minMaxComBuy Min and max node to buy
   */
  ...
  function updateFee(uint256[2] memory _fee_d2) external virtual onlyOwner {
    _isNotStarted();
  ...
  function updateMinKomStaked(uint256 _minKomStaked) external virtual onlyOwner {
    _isNotStarted();
