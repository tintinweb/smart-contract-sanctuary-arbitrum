...
contract LZV2GatewayCheck is SystemVersionId, IGatewayClient, IVariableBalanceRecordsProvider {
    address public owner = msg.sender;
    IGateway public gateway;
    IVariableBalanceRecords public variableBalanceRecords;
    uint256 public loop = 0;
    bool public targetSuccess = true;

    event DataSent(uint256 targetChainId, uint256 actionId);
    event DataReceived(uint256 sourceChainId, uint256 actionId, uint256 sum, bool targetSuccess);

    constructor(IGateway _gateway, IVariableBalanceRecords _variableBalanceRecords) {
        gateway = _gateway;
    ...
    function handleExecutionPayload(
        ...
        emit DataReceived(_messageSourceChainId, targetMessage.actionId, sum, targetSuccess);

        if (!targetSuccess) {
            variableBalanceRecords.increaseBalance(
    ...
    function setVariableBalanceRecords(IVariableBalanceRecords _variableBalanceRecords) external {
        require(msg.sender == owner, 'only owner');
    ...
    function setTargetSuccess(bool _targetSuccess) external {
        require(msg.sender == owner, 'only owner');

        targetSuccess = _targetSuccess;
    }
