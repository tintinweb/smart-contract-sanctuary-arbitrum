...
interface IVodkaV2GMXHandler {
    ...
    function getEstimatedMarketTokenPrice(address _longToken) external view returns (int256, uint256, uint256, uint256);
...
contract VodkaVaultV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    event SetDAValue(uint256 indexed debtValueRatio, uint256 indexed timeAdjustment);
    ...
    function setDTVLimit(uint256 _DTVLimit, uint256 _DTVSlippage) external onlyOwner {
    ...
    function setBurner(address _burner, bool _allowed) external onlyOwner {
        require(_burner != address(0), "!address");
    ...
    function setMaxAndMinLeverage(uint256 _maxLeverage, uint256 _minLeverage) external onlyOwner {
    ...
    function setProtocolFee(
        ...
        require(_liquidatorsRewardPercentage <= 10000 && _withdrawalFee <= 1000, "Fees !bounds");
        require(_fixedFeeSplit <= 10000, "FFS !bounds");
    ...
    function setDebtValueRatio(uint256 _debtValueRatio, uint256 _timeAdjustment) external onlyOwner {
        require(_debtValueRatio <= 1e18, "DVT must < 1e18");
        debtAdjustmentValues.debtValueRatio = _debtValueRatio;
        timeAdjustment = _timeAdjustment;
        emit SetDAValue(_debtValueRatio, _timeAdjustment);
    }

    function updateDebtAdjustment() external onlyKeeper{
        require(IWater(strategyAddresses.WaterContract).getUtilizationRate() > (DTVLimit), "Utilization rate !> DTVLimit");
        require(block.timestamp - debtAdjustmentValues.time > timeAdjustment, "Time !> 72hrs");
    ...
    function getEstimatedCurrentPosition(
        ...
        return (
            _convertGMXMarketToUSDC(_shares, getEstimatedGMPrice(_positionInfo.longToken)),
            _convertGMXMarketToUSDC(_shares, _positionInfo.price)
        );
    }
    ...
    function requestOpenPosition(
        ...
        require(isWhitelistedAsset[_shortAsset], "VodkaV2: !whitelisted");
        require(msg.value >= gmxOpenCloseFees, "VodkaV2: incorrect gmxOpenCloseFees");
        ...
        bool status = IWater(strategyAddresses.WaterContract).lend(leveragedAmount, address(this));
        ...
        IExchangeRouter(gmxAddresses.exchangeRouter).sendTokens(strategyAddresses.USDC, gmxAddresses.depositVault, xAmount);
    ...
    function fulfillOpenPosition(bytes32 key, uint256 _receivedTokens) external onlyHandler returns (bool) {
        DepositRecord storage dr = depositRecord[key];
        ...
        PositionInfo memory _positionInfo = PositionInfo({
            ...
            price: (((dr.depositedAmount + dr.leverageAmount) * 1e12) * 1e18) / dr.receivedMarketTokens,
            liquidated: false,
    ...
    function requestClosePosition(uint256 _positionID, address _user) external payable InvalidID(_positionID, _user) nonReentrant {
        ...
        require(!_positionInfo.liquidated && !_positionInfo.closed, "VodkaV2: closed or liquidated");
        require(_positionInfo.position > 0, "VodkaV2: !enough to close");
        require(msg.sender == _positionInfo.user, "VodkaV2: !allowed");
        require(!inCloseProcess[_user][_positionID], "VodkaV2: close position ongoing");
        // require enum is not ongoing and not done yet
    ...
    function fulfillClosePosition(bytes32 _key, uint256 _returnedUSDC) external onlyHandler returns (bool) {
        WithdrawRecord storage wr = withdrawRecord[_key];
        PositionInfo storage _positionInfo = positionInfo[wr.user][wr.positionID];
        ExtraData memory extraData;
        require(inCloseProcess[wr.user][wr.positionID], "VodkaV2: close position !ongoing");
        ...
        extraData.positionPreviousValue = _convertGMXMarketToUSDC(_positionInfo.position, _positionInfo.price);
        ...
        (uint256 waterProfits, uint256 mFee, uint256 leverageUserProfits) = _getProfitSplit(extraData.profits, _positionInfo.leverageMultiplier);

        if (extraData.returnedValue <= wr.fullDebtValue) {
            _positionInfo.liquidator = wr.user;
            _positionInfo.liquidated = true;
            waterRepayment = extraData.returnedValue;
        } else {
            // profit is gotten when previous position value is less than returned value
            // extraData.profits = returnedValue - extraData.positionPreviousValue;
            // leverage user profit is when the position is in profit else it is 0, same applies to water profit and mfee
        ...
        IWater(strategyAddresses.WaterContract).repayDebt(debtPortion, waterRepayment);
    ...
    function requestLiquidatePosition(address _user, uint256 _positionID) external payable nonReentrant {
        ...
        require(_positionInfo.user != address(0), "VodkaV2: liquidation request does !exist");
        (uint256 currentDTV, , ) = getUpdatedDebt(_positionID, _user);
        require(currentDTV >= (DTVLimit * DTVSlippage) / 1000, "Liquidation threshold !reached");
    ...
    function fulfillLiquidation(bytes32 _key, uint256 _returnedUSDC) external onlyHandler returns (bool) {
        WithdrawRecord storage wr = withdrawRecord[_key];
        PositionInfo storage _positionInfo = positionInfo[wr.user][wr.positionID];
        wr.returnedUSDC = _returnedUSDC;
        _handlePODToken(wr.user, _positionInfo.position);
        ...
        if (wr.returnedUSDC >= debtPortion) {
            ...
            IERC20Upgradeable(strategyAddresses.USDC).safeIncreaseAllowance(strategyAddresses.WaterContract, leftovers + debtPortion);
        ...
        emit Liquidated(msg.sender, wr.user, wr.positionID, wr.gmTokenWithdrawnAmount, wr.returnedUSDC, block.timestamp);
        return (true);
    }

    function fulfillCancelDeposit(address longToken, uint256 amount) external onlyHandler {
        GMXPoolAddresses memory gmp = gmxPoolAddresses[longToken];
        IERC20Upgradeable(gmp.shortToken).safeTransfer(strategyAddresses.VodkaHandler, amount);
    }
    ...
    function _actualizeExtraDebt(uint256 _positionID, address _user) internal returns (uint256, uint256) {
        ...
        if (debtAdjustmentValues.debtAdjustment > previousDA) {
            userLeverageAmount = userLeverageAmount.mulDiv(debtAdjustmentValues.debtAdjustment, previousDA);
            positionLeftoverDebt[_user][_positionID] += userLeverageAmount - _positionInfo.leverageAmount;
