...
struct Versions {
    uint64 endpoint;
    uint64 clearinghouse;
    uint64 clearinghouseLiq;
    uint64 spotEngine;
    uint64 perpEngine;
    uint64 querier;
    uint64 exchange;
...
contract FQuerier is Version {
    ...
    function initialize(address _clearinghouse) external {
        require(address(clearinghouse) == address(0), "already initialized.");
    ...
    function _exchange() internal view returns (IOffchainExchange) {
        return IOffchainExchange(endpoint.getOffchainExchange());
    }

    function legacyRisk(RiskHelper.Risk memory r)
        public
        view
        returns (LegacyRisk memory l)
    {
        l.longWeightInitialX18 = r.longWeightInitialX18;
        l.shortWeightInitialX18 = r.shortWeightInitialX18;
        l.longWeightMaintenanceX18 = r.longWeightMaintenanceX18;
        l.shortWeightMaintenanceX18 = r.shortWeightMaintenanceX18;
        l.largePositionPenaltyX18 = 0;
    }
    ...
    function _getAllProductIds(uint32 isoGroup)
        internal
        view
        returns (uint32[] memory spotIds, uint32[] memory perpIds)
    {
        spotIds = spotEngine.getProductIds(isoGroup);
        perpIds = perpEngine.getProductIds(isoGroup);
    }

    function getCrossProducts() public view returns (ProductInfo memory) {
        return getAllProducts(0);
    }

    function getAllProducts(uint32 isoGroup)
        public
        view
        returns (ProductInfo memory)
    {
        (uint32[] memory spotIds, uint32[] memory perpIds) = _getAllProductIds(
            isoGroup
        );
        return
            ProductInfo({
        ...
        returns (SpotProduct memory)
    {
        ...
        RiskHelper.Risk memory risk = spotEngine.getRisk(productId);
        return
            SpotProduct({
                productId: productId,
                oraclePriceX18: risk.priceX18,
                risk: legacyRisk(risk),
                config: spotEngine.getConfig(productId),
                state: state,
                lpState: lpState,
                bookInfo: productId != 0
                    ? getBookInfo(productId)
                    : BookInfo(0, 0, 0, 0, 0)
            });
    }
        ...
        returns (PerpProduct memory)
    {
        ...
        RiskHelper.Risk memory risk = perpEngine.getRisk(productId);
        ...
        returns (SubaccountInfo memory)
    {
...

        {
            ...
            for (uint256 i = 0; i < spotIds.length; i++) {
                if (spotIds[i] > maxProductId) {
                    maxProductId = spotIds[i];
                }
            }
            for (uint256 i = 0; i < perpIds.length; i++) {
                if (perpIds[i] > maxProductId) {
                    maxProductId = perpIds[i];
                }
            }
            subaccountInfo.healthContributions = new int128[][](
                maxProductId + 1
            );
            for (uint256 i = 0; i <= maxProductId; i++) {
        ...
        for (uint32 i = 0; i < spotIds.length; ++i) {
            ...
            subaccountInfo.spotProducts[
                ...
                state: state,
                lpState: lpState,
                bookInfo: getBookInfo(productId)
            });
            ...
            for (uint8 healthType = 0; healthType < 3; ++healthType) {
                ...
                if (lpBalance.amount != 0) {
                    int128 ratioX18 = lpBalance.amount.div(lpState.supply);
                    healthContribution += RiskHelper
                        ._getLpRawValue(
                            lpState.base.amount.mul(ratioX18),
                            lpState.quote.amount.mul(ratioX18),
                            risk.priceX18
                        )
                        .mul(
                ...
                subaccountInfo.healthContributions[productId][
                        healthType
                    ] += healthContribution;
            }
        }
        ...
        for (uint32 i = 0; i < perpIds.length; ++i) {
            ...
            for (uint8 healthType = 0; healthType < 3; ++healthType) {
                int128 healthContribution = RiskHelper
                    ...
                    .mul(balance.amount)
                    .mul(priceX18) + balance.vQuoteBalance;
                ...
                if (lpBalance.amount != 0) {
                    ...
                    healthContribution +=
                        RiskHelper
                            ._getLpRawValue(
                                lpState.base.mul(ratioX18),
                                lpState.quote.mul(ratioX18),
                                risk.priceX18
                            )
                            .mul(
                                RiskHelper._getWeightX18(
        ...
        uint256 spreads = clearinghouse.getSpreads();
        while (spreads != 0) {
            uint32 spotId = uint32(spreads & 0xFF);
            spreads >>= 8;
            uint32 perpId = uint32(spreads & 0xFF);
            spreads >>= 8;
            ...
            for (uint8 healthType = 0; healthType < 3; healthType++) {
                ...
                int128 rebateAmount = basisAmount
                    .mul(spotCoreRisk.price + perpCoreRisk.price)
                    .mul(spreadPenalty - existingPenalty) / 2;

                subaccountInfo.healthContributions[spotId][
                    healthType
                ] += rebateAmount;
                subaccountInfo.healthContributions[perpId][
                    healthType
                ] += rebateAmount;
            }
        }
    ...
    function getBookInfo(uint32 productId)
        public
        view
        returns (BookInfo memory bookInfo)
    {
        IOffchainExchange.MarketInfo memory marketInfo = _exchange()
            .getMarketInfo(productId);

        IOffchainExchange.LpParams memory lpParams = _exchange().getLpParams(
            productId
        );
        ...
        return
            BookInfo({
                ...
                priceIncrementX18: 0,
                minSize: marketInfo.minSize,
                collectedFees: marketInfo.collectedFees,
                lpSpreadX18: lpParams.lpSpreadX18
            });
    }

    function getContractVersions() external view returns (Versions memory) {
        return
            Versions({
                endpoint: IVersion(address(endpoint)).getVersion(),
                clearinghouse: IVersion(address(clearinghouse)).getVersion(),
                clearinghouseLiq: IVersion(clearinghouse.getClearinghouseLiq())
                    .getVersion(),
                spotEngine: IVersion(address(spotEngine)).getVersion(),
                perpEngine: IVersion(address(perpEngine)).getVersion(),
                querier: IVersion(address(this)).getVersion(),
                exchange: IVersion(address(_exchange())).getVersion()
            });
    }
}
