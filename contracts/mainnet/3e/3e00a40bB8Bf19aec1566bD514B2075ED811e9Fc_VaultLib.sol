...
/// @author Enzyme Council <[email protected]yme.finance>
...
contract VaultLib is VaultLibBase2, IVault, GasRelayRecipientMixin {
    ...
    function getExternalPositionLibForType(
        uint256 _typeId
    ) external view override returns (address externalPositionLib_) {
        return
    ...
    function setSymbol(string calldata _nextSymbol) external override {
        require(msg.sender == owner || msg.sender == getFundDeployer(), "Unauthorized");
    ...
    ) external override onlyAccessor returns (bytes memory returnData_) {
        bool success;
    ...
    function withdrawAsset(address _asset, address _target, uint256 _amount) external onlyOwner {
        __withdrawAssetTo(_asset, _target, _amount);
    }
    ...
    function receiveValidatedVaultAction(
        VaultAction _action,
    ...
    function isActiveExternalPosition(
        address _externalPosition
    ) public view override returns (bool isActiveExternalPosition_) {
