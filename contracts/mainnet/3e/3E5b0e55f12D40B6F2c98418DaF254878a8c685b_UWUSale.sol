...
import "../token-wallet/ITokenWallet.sol";
...
contract UWUSale is Ownable, Readable, ReentrancyGuard {
    ...
    uint constant public MIN_DEPOSIT = 0.00085714285 ether;
    uint constant public MAX_DEPOSIT = 10 ether;
    uint constant public MAX_WHITELISTED_DEPOSIT = 3 ether;

    uint constant public SOFT_CAP = 750 ether;
    uint constant public HARD_CAP = 1500 ether;
    ...
    function _getSalePrice(uint _totalDeposited) private pure returns(uint) {
        return _totalDeposited >= HARD_CAP ? 0.00085714285 ether : _totalDeposited.div(TOTAL_DISTRIBUTED);
    }
