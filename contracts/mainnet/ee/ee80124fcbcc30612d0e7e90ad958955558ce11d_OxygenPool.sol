...
// File contracts/lib/crypto/SignatureChecker.sol
...
// File contracts/OxygenPool.sol



contract OxygenPool is OwnableUpgradeable {
    using EnumerableSet for EnumerableSet.AddressSet;
    using SignatureChecker for EnumerableSet.AddressSet;

    uint256 public constant MAX_ADDRESSES = 230000;
    uint256 public constant MAX_TOKEN = 167_540_338_000_000_000 * 1e6;
    uint256 public constant INIT_CLAIM = 2_774_334_100_000 * 1e6;
    ...
    function claim(uint128 nonce, bytes calldata signature, address referrer) public {
        require(_usedNonce[nonce] == false, "OxygenPool: nonce already used");
        require(_claimedUser[_msgSender()] == false, "OxygenPool: already claimed");
        ...
        require(supplyPerAddress >= 1e6, "OxygenPool: Airdrop has ended");
    ...
    function addSigner(address val) public onlyOwner() {
        require(val != address(0), "OxygenPool: val is the zero address");
        _signers.add(val);
    }

    function delSigner(address signer) public onlyOwner returns (bool) {
        require(signer != address(0), "OxygenPool: signer is the zero address");
