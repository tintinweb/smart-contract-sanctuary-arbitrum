...
/// @notice ERC20 token contract representing Neuron (N) tokens.
/// @dev The Neuron token is used for various functions within the AI Arena ecosystem.
contract Neuron is ERC20, AccessControl {
    ...
    /// @notice Role for transferring  tokens.
    ...
    /// @notice Grants roles to the Neuron contract.
    /// @notice Mints the initial supply to the ditribution address.
    /// @param ownerAddress_ The address of the owner for the Neuron contract
    ...
    constructor(
        ...
        address initialDistributionAddress
    )
        ERC20("Neuron", "NRN")
    {
    ...
    function transferOwnership(address newOwnerAddress) external {
        require(msg.sender == ownerAddress);
        _revokeRole(DEFAULT_ADMIN_ROLE, ownerAddress);
        _grantRole(DEFAULT_ADMIN_ROLE, newOwnerAddress);
        ownerAddress = newOwnerAddress;
    }

    /// @notice Adds a new address to the transfer role.
    /// @dev Only the owner address is authorized to call this function.
    /// @param newTransferorAddress The address to be added as an approved transferor
    function addTransferor(address newTransferorAddress) external {
        grantRole(TRANSFER_ROLE, newTransferorAddress);
    }
    ...
    /// @notice Approves the specified amount of tokens for a receiving address.
    /// @dev The caller must have the transfer role.
    /// @param owner The owner of the tokens.
    /// @param transferor The address for which to approve the allowance.
    /// @param amount The amount of tokens to be approved.
    function approveTransfer(address owner, address transferor, uint256 amount) public {
        require(
            hasRole(TRANSFER_ROLE, msg.sender), 
            "ERC20: must have transfer role to approve transferring"
        );
        _approve(owner, transferor, amount);
    }
