    ...
    OwnableUpgradeable
{
    ...
    uint256 public override cooldownPeriod;
    ...
    mapping(address => uint256) public numberOfRedeems;
    ...
    function getRedeemInfoByIndex(address account, uint256[] calldata indexes)
        external
        view
        returns (RedeemInfo[] memory info)
    {
        info = new RedeemInfo[](indexes.length);

        for (uint256 i; i < indexes.length; ++i) {
            info[i] = redeemInfo[account][indexes[i]];
        }
    }

    function getRedeemInfo(address account)
        external
        view
        returns (RedeemInfo[] memory info)
    {
        uint256 length = numberOfRedeems[account];

        info = new RedeemInfo[](length);

        for (uint256 i; i < length; ++i) {
            info[i] = redeemInfo[account][i];
        }
    }

    function getRedeemable(address account)
        external
        view
        returns (uint256 amount)
    {
        ...
        for (uint256 i; i < length; ++i) {
            RedeemInfo memory redeemItem = redeemInfo[account][i];

            if (redeemItem.amount != 0 && redeemItem.expire <= block.timestamp)
                amount += redeemItem.amount;
        }
    }

    function getRedeemableByIndex(address account, uint256[] calldata indexes)
        external
        view
        returns (uint256 amount)
    {
        for (uint256 i; i < indexes.length; ++i) {
            RedeemInfo memory redeemItem = redeemInfo[account][indexes[i]];
    ...
    function redeem(uint256[] calldata _indexes)
        external
        override
        nonReentrant
        returns (uint256)
    {
        ...
        return _redeem(msg.sender, _indexes, msg.sender);
    }
    ...
    function redeemForAccount(
        ...
        _validateHandler();
        return _redeem(_account, _indexes, _receiver);
    }
        ...
        returns (uint256)
    {
        ...
        if (tokenAmount != 0) {
            ...
            emit Claim(msg.sender, _account, tokenAmount);
        }
    ...
    function _stake(
        ...
        emit Stake(msg.sender, _account, _amount);
    }
    ...
    function _redeem(
        ...
        uint256 _numberOfRedeem = numberOfRedeems[_account];

        for (uint256 i; i < _indexes.length; ++i) {
            require(
                _indexes[i] < _numberOfRedeem,
                "RewardTracker: Redeem not exists"
            );

            RedeemInfo memory redeemItem = redeemInfo[_account][_indexes[i]];

            if (redeemItem.amount == 0) continue;

            require(
                redeemItem.expire <= block.timestamp,
                "RewardTracker: cooldown duration not yet passed"
            );
            ...
            delete redeemInfo[_account][_indexes[i]];
        }

        if (amount != 0)
            IERC20Upgradeable(want).safeTransfer(_receiver, amount);
    }
    ...
    function _unstake(
        ...
        emit Unstake(msg.sender, _account, _amount);

        uint256 _index = numberOfRedeems[_receiver];

        redeemInfo[_receiver][_index] = RedeemInfo({
            amount: _amount,
            expire: block.timestamp + cooldownPeriod
        });

        numberOfRedeems[_receiver] = _index + 1;
