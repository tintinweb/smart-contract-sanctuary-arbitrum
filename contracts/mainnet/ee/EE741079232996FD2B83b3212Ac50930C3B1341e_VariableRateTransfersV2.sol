...
contract VariableRateTransfersV2 is
    ...
    Pausable
{
    ...
    mapping(uint256 => uint256) public processedTransfers;
    ...
    function processTransfers(
        TransferParams[] calldata transfers_
    ) external onlyValidProcessorExt whenNotPaused {
        _processPlatformTransfers(transfers_);
    }

    function processTransfersV2(
        TransferV2Params[] calldata transfers_
    ) external onlyValidProcessorExt whenNotPaused {
        TransferV2Params calldata transferDetails;
        uint256 length = transfers_.length;
        for (uint256 i; i < length; ) {
            transferDetails = transfers_[i];
            bool _success;
            bytes memory _returnData;
            (_success, _returnData) = address(this).call(
                abi.encodeWithSelector(
                    this.processTransferV2.selector,
                    transferDetails
                )
            );
            ...
            bytes memory _err;
            uint256 _amount;
            uint256 _totalAdjustments;
            if (_success) {
                (_amount, _totalAdjustments) = abi.decode(
                    _returnData,
                    (uint256, uint256)
                );
            } else {
                _err = _returnData;
                _amount = transferDetails.data.amount;
            }
            ...
            emit TransferProcessed(
                ...
                _success,
                false,
                transferDetails.data.token,
                transferDetails.data.from,
                _amount,
                _totalAdjustments,
                _err
            );
    ...
    function processTransferV2(
        TransferV2Params calldata transferDetails_
    ) external returns (uint256 transferredAmount, uint256 totalAdjustments) {
        require(msg.sender == address(this), "LC:SELF_ONLY");

        Transfer calldata _transfer = transferDetails_.data;
        Adjustments calldata _adjustments = transferDetails_.adjustments;
        Signature calldata _signature = transferDetails_.signature;
        uint256 _exchangeRate = transferDetails_.exchangeRate * 10 ** 6;

        /* Validate Signature */
        address transferSigner = _getSigner(_transfer, _signature);
        require(transferSigner == certifiedSigner, "LC:INVALID_SIGNER");

        uint128 tokenDecimals = IERC20Metadata(_transfer.token).decimals();

        uint256 remainingAmount = _transfer.amount;

        uint256 length = _adjustments.amounts.length;
        for (uint256 i; i < length; ) {
            remainingAmount -= _adjustments.amounts[i];
            uint256 adjustmentAmount = _adjustments.amounts[i];
            if (_transfer.usd) {
                adjustmentAmount = TokenMath.convertUsdToTokenAmount(
                    adjustmentAmount,
                    _exchangeRate,
                    8,
                    tokenDecimals
            ...
            IERC20Metadata(_transfer.token).transferFrom(
                _transfer.from,
                _adjustments.recipients[i],
                adjustmentAmount
            );
        ...
        if (_transfer.usd) {
            remainingAmount = TokenMath.convertUsdToTokenAmount(
                remainingAmount,
                _exchangeRate,
                8,
                tokenDecimals
    ...
    function _processPlatformTransfers(
        TransferParams[] calldata transfers_
    ) internal {
        bool success;
        bytes memory returnData;
    ...
    function _hashTransfer(
        Transfer calldata transfer_
    ) internal pure returns (uint256 transferHash) {
