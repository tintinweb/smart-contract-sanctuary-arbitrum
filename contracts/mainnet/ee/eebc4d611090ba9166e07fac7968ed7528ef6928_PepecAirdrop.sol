...
// File: PepecAirdrop.sol

pragma solidity ^0.8.15;

contract PepecAirdrop is Ownable {
    using ECDSA for bytes32;

    IERC20 public token;

    address public signer;
    uint256 public constant MAX_ADDRESSES = 15_000;
    uint256 public constant MAX_TOKEN = 650_000_000_000 * 1e6;
    uint256 public constant INIT_CLAIM = 45_000_000 * 1e6;
    uint256 public constant MAX_REFER_TOKEN = 45_000_000 * 1e6;
    ...
    function claim(
        ...
        require(_usedNonce[nonce] == false, "Pepec: nonce already used");
        require(_claimedUser[_msgSender()] == false, "Pepec: already claimed");

        _claimedUser[_msgSender()] = true;
        require(isValidSignature(nonce, signature), "Pepec: only auth claims");

        _usedNonce[nonce] = true;

        uint256 supplyPerAddress = canClaimAmount();
        require(supplyPerAddress >= 1e6, "Pepec: airdrop has ended");
    ...
    function setSigner(address val) public onlyOwner {
        require(val != address(0), "Pepec: val is the zero address");
        signer = val;
    }
