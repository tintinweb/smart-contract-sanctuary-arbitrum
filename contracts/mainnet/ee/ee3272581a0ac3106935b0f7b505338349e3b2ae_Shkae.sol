...
contract Shkae is ERC20, Ownable {
    event TaxPaidFrom(address indexed from, address indexed to, uint256 taxAmount);
    event TaxPaidTransfer(address indexed from, address indexed to, uint256 taxAmount);
    event WindowUpdated(uint256 startTime, uint256 endTime);

    address public _dexRouter = 0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45; // DEX router address
    address public taxCollector; // Marketing wallet

    uint256 public buyTaxPercent = 10; // Made mutable
    uint256 public sellTaxPercent = 10; // Made mutable
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if (taxAmount > 0) {
            ...
            emit TaxPaidTransfer(msg.sender, taxCollector, taxAmount);  // Emit an event for the tax payment
        }
    ...
    function transferFrom(
        ...
        if (taxAmount > 0) {
            ...
            emit TaxPaidFrom(msg.sender, taxCollector, taxAmount);  // Emit an event for the tax payment
        }
    ...
    function getTaxRate(address sender, address recipient) private view returns (uint256) {
        if (recipient == _dexRouter) {
            // If the recipient of the tokens is the DEX router, it means the sender is selling tokens
            return sellTaxPercent;
        } else if (sender == _dexRouter) {
            // If the sender of the tokens is the DEX router, it means the sender (user) is buying tokens
            return buyTaxPercent;
        } else {
            // Any other case is considered a standard transfer
