...
contract MonumentHomage is IMonumentHomage, ManagerModifier, Pausable, ReentrancyGuard {
  ...
  function payHomage(HomageRequest[] calldata _requests) external nonReentrant whenNotPaused {
    ...
    for (i = 0; i < _requests.length; i++) {
      ...
      for (uint256 j = 0; j < request.destinations.length; j++) {
        structuresToBurn.totalStructures += request.destinations[j].structureAmount;
      }
    }

    // Allocate memory for storing realms and structures to burn
    structuresToBurn.realmIds = new uint256[](structuresToBurn.totalDestinations);
    structuresToBurn.structureIds = new uint256[](structuresToBurn.totalDestinations);
    structuresToBurn.amounts = new uint256[](structuresToBurn.totalDestinations);
    ...
    uint256 currentEpoch = block.timestamp.toEpochNumber(EPOCH_DURATION, EPOCH_OFFSET);
    uint256 totalAnima;
    uint256 destinationIterator;
    uint256 destinationCost;
    uint256 homageIndex = nextHomageId;
    uint256 randomBase = Random.startRandomBase(uint256(uint160(address(this))), nextHomageId);
    // Process each homage request
    for (i = 0; i < _requests.length; i++) {
      HomageRequest calldata request = _requests[i];
      address owner = ERC721(request.adventurerAddress).ownerOf(request.adventurerId);
      ...
      for (uint256 j = 0; j < request.destinations.length; j++) {
        ...
        MonumentHomageConfig storage homageConfig = HOMAGE_CONFIG[destination.structureId];
        require(homageConfig.animaCost > 0, "MonumentHomage: Structure not configured");
        ...
        structuresToBurn.structureIds[destinationIterator] = destination.structureId;
        structuresToBurn.amounts[destinationIterator] = destination.structureAmount;
        ...
        for (uint256 k = 0; k < destination.structureAmount; k++) {
          // Dispense rewards
          DispensedRewards memory dispensedRewards = REWARDS_POOL.dispenseRewards(
            homageConfig.rewardPoolId,
            randomBase,
            msg.sender
          );
  ...
  function _emitRewards(
    ...
    emittedRewards.tokenAddresses = new address[](dispensedRewards.rewards.length);
  ...
  function getEligibleStructureAmounts(
    ...
    for (uint256 i = 0; i < _realmIds.length; i++) {
      result[i] = BATCH_STAKER.stakerBalance(_realmIds[i], address(MONUMENT), _tokenIds[i]);
    }
    return result;
  }
  ...
  function getAdventurerHomagePoints(
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
      result[i] = _getHomagePoints(currentEpoch, _addresses[i], _adventurerIds[i]);
    }
  }
  ...
  function _getHomagePoints(
    ...
    uint256 epochsSinceLastAction = _epoch -
      ACTION_STORAGE.getActionMarker(_address, _tokenId, ACTION_ADVENTURER_HOMAGE).toEpochNumber(
        EPOCH_DURATION,
        EPOCH_OFFSET
      );
    return epochsSinceLastAction > MAX_HOMAGE_POINTS ? MAX_HOMAGE_POINTS : epochsSinceLastAction;
  }
