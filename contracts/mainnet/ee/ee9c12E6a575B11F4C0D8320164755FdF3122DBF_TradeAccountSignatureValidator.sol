...
import "../interfaces/ITradeAccount.sol";
...
contract TradeAccountSignatureValidator is IRebateSignatureValidator {
    using Signature for bytes32;

    ITradeAccount public account;

    constructor(address _account) {
        account = ITradeAccount(_account);
    }
    ...
    function validateSignature(
        ...
        address signer = message.signerOf(signature);
        bool isValid = account.doesUserHaveAccountRole(
            accountId,
            signer,
            AccountRole.Withdraw
        );

        require(isValid, "TradeAccountSignatureValidator: invalid signature");
    }
}