    ...
    PausableUpgradeable
{
    ...
    /**
     * @dev Withdraw liquid amount of Magic from the Treasury Flywheel Vault
     */
    function withdrawLiquidAmount(uint256 amount) public override nonReentrant onlyOwner {
        require(
            amount <= pendingTreasuryAmountToStake,
            "BattleflyTreasuryFlywheelVault: Not enough liquid funds available"
        );
        MAGIC.safeTransfer(msg.sender, amount);
        pendingTreasuryAmountToStake -= amount;
        emit WithdrawLiquidMagic(msg.sender, amount);
    }
    ...
    event WithdrawLiquidMagic(address sender, uint256 amount);
