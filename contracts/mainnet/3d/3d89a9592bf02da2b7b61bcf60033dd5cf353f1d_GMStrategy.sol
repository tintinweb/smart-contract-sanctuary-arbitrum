...
contract GMStrategy is IGMStrategy, IDepositCallbackReceiver, IWithdrawalCallbackReceiver, OwnableUpgradeable {
    ...
    mapping(bytes32 => Data) private gmxData;

    /// @notice Deposits ID => Hash(block.timestamp, address user)
    ...
    /**
     * @notice Get Callback User
     * @param callbackHash Callback Hash
     * @return User address
     */
    function getCallbackUser(bytes32 callbackHash) external view returns (address) {
        return gmxData[callbackHash].user;
    }
    ...
    /**
     * @notice Compound incentives to more GM tokens.
     * @param _token Token to swapped.
     * @param _amount Amount to be swapped.
     * @param _data Signed Data.
     */
    function compound(address _token, uint256 _amount, GMData[] memory _data) external payable {
        _onlyKeeper();
        ...
        info.swapper = rebalance.getSwapper(_token, USDC);

        IERC20(_token).approve(address(info.swapper), _amount);

        _amount = info.swapper.swap(_token, _amount, USDC, 0, abi.encode(rebalance.slippage(), BASIS));

        info.length = uint8(_data.length);
        info.thisAddress = address(this);
        info.opHash = keccak256(abi.encode(arbsys.arbBlockNumber(), info.thisAddress));
        Data storage data = gmxData[info.opHash];

        data.callbackCounter = info.length;

        operationOnGoing = true;
        compoundOnGoing = true;
