...
interface IRewards {
    function rewardToken() external view returns (IBaseRewards);
...
contract StratX2_Aura is BaseStrategyV2 {
    using SafeERC20 for IERC20;

    IAuraBooster public booster;
    IAuraRewardPool public rewardPool;
    ...
    function initialize(
        ...
        super.__initialize(_addresses, _pid, false, _isAutoComp);

        booster = IAuraBooster(_booster);
        (,,,address rewardPool_,,) = booster.poolInfo(_pid);

        rewardPool = IAuraRewardPool(rewardPool_);
        _addRewardToken(IERC20(address(IRewards(rewardPool_).rewardToken())));

        extraRewardsLength = rewardPool.extraRewardsLength();
        for (uint256 i = 0; i < extraRewardsLength; i++) {
            address r = rewardPool.extraRewards(i);
            _addRewardToken(IERC20(IRewards(r).rewardToken().baseToken()));
        } 
    }

    function _vaultDeposit(uint256 _amount) internal virtual override {
        wantAddress.safeIncreaseAllowance(address(booster), _amount);
        booster.deposit(pid, _amount, true);
    }

    function _vaultWithdraw(uint256 _wantAmt) internal virtual override {
        rewardPool.withdrawAndUnwrap(_wantAmt, false);
    }

    function _vaultHarvest() public virtual override {
    ...
    function _vaultPending() public virtual override view returns (uint256[] memory pending) {
        // 0. BAL, AURA=BAL*mintRate()
        // 1(extra). ARB
        // 2(extra). AURA
        pending = new uint256[](extraRewardsLength + 1);
        uint256 earned = rewardPool.earned(address(this)); // bal
        pending[0] = earned;

        address operator = rewardPool.operator();
        address minter = IAuraBooster(operator).minter();
        uint256 mintRate = IL2Coordinator(minter).mintRate();
        address auraOFT = IL2Coordinator(minter).auraOFT();
        uint256 mintAmount = (earned * mintRate) / 1e18; // aura

        for (uint256 i = 0; i < extraRewardsLength; i++) {
            address r = rewardPool.extraRewards(i);
            address rewardToken = IVirtualBalanceRewardPool(r).rewardToken();
            address baseToken = IBaseRewards(rewardToken).baseToken();
            pending[i + 1] = IRewards(r).earned(address(this));
            if(auraOFT == baseToken){
                pending[i + 1] += mintAmount; // aura
            }
        }
    }

    function _vaultEmergencyWithdraw() internal virtual override {
        rewardPool.withdrawAndUnwrap(_vaultUserInfo(), false);
    }
}
