...
contract BondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable GLA; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints GLA when receives principle
    ...
    struct Bond {
        uint256 payout; // GLA remaining to be paid
        uint256 vesting; // Blocks left to vest
        uint256 lastBlock; // Last interaction
        uint256 pricePaid; // In USDC, for front end viewing
    }
    ...
    function deposit(
        ...
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 GLA ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) GLA
         */
        ...
        if (fee != 0) {
            ...
            IERC20(GLA).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(GLA).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(GLA).approve(stakingHelper, _amount);
                IStakingHelper(stakingHelper).stake(_amount, _recipient);
            } else {
                IERC20(GLA).approve(staking, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(GLA).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to GLA supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(GLA).totalSupply();
    ...
    /**
     *  @notice calculate amount of GLA available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or GLA) to the DAO
     *  @return bo
     */
