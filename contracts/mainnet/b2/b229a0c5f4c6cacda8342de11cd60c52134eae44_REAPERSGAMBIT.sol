...
contract REAPERSGAMBIT is ERC20, Ownable {
    ...
    address private constant BLACKHOLE_ADDRESS = address(0x000000000000000000000000000000000000dEaD);
    mapping(address => uint256) private _firstReceivedBlock;
    mapping(address => bool) private _immortal;
    uint256[] private tokenIdUsed;
    address public revivalCardNft = 0x472d94E579FE7DE54B3aDF0e26f7Af2a0b51aE42;

    constructor() ERC20("REAPERS GAMBIT", "ARBRG") {
        _mint(msg.sender, 999999999999999999 * 10 ** decimals());
    }

    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(_firstReceivedBlock[msg.sender] + 15000 > block.number || _immortal[msg.sender], "cannot escape death");
        _immortal[msg.sender] = false;
        return super.transfer(recipient, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        require(_firstReceivedBlock[sender] + 15000 > block.number || _immortal[sender], "cannot escape death");
        _immortal[msg.sender] = false;
    ...
    function CheatDeath(uint256 tokenId) public {
        address account = msg.sender;
        IERC721A revivalCard =  IERC721A(revivalCardNft);
        address holderAddress = revivalCard.ownerOf(tokenId);
        require(holderAddress==account,"this card not owner you");
        require(revivalCard.balanceOf(account) > 0, "must hold the specific NFT to cheat death");
        require(!_immortal[account], "account is already immortal");
        require(!isTokenIdUsed(tokenId), "this resurrection card has already been used");
        _immortal[account] = true;
        tokenIdUsed.push(tokenId);
    }
    ...
    function isTokenIdUsed(uint256 tokenId) public view returns (bool) {
        for (uint256 i = 0; i < tokenIdUsed.length; i++) {
            if (tokenIdUsed[i] == tokenId) {
                return true;
            }
        }
        return false;
    ...
    function KnowDeath(address account) public view returns (uint256) {
        ...
        if (_firstReceivedBlock[account] != 0) {
            deathBlock = _firstReceivedBlock[account] + 15000;
        }
