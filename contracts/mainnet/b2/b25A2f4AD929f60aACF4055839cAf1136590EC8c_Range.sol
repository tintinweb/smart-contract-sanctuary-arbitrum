...
contract Range is CommonSolo {
  ...
  event UpdateHouseEdge(uint256 multiplier, uint64 houseEdge);
  ...
  modifier isChoiceInsideLimits(bytes memory _gameData) {
    ...
    require(choice_ >= 500 && choice_ <= 9500, "Choice isn't allowed");
    _;
  }

  /*==================================================== State Variables ====================================================*/

  uint32 public mod = 9901;
  uint32 public range = 10000;
  uint256 public houseEdgeMultiplier = 98e18;
  ...
  function encodeGameData(uint32 _choice, bool _over) public pure returns (bytes memory) {
  ...
  function decodeGameData(bytes memory _gameData) public pure returns (uint32, bool) {
    return abi.decode(_gameData, (uint32, bool));
  }
  ...
  function isWon(bool _over, uint32 _choice, uint256 _result) public pure returns (bool won_) {
    won_ = _over ? (_result < _choice) : (_result > _choice);
  }
  ...
  function calcSelectionRange(
    uint256 _choice,
    bool _over
  ) public view returns (uint256 selectionRange_) {
    if (_over) {
      selectionRange_ = _choice;
    } else {
      selectionRange_ = range - _choice;
    }
  }
  ...
  /// @param _choice players choice between 5-95
  /// @param _over if the value is true, the game inside 0-choice. if not the game inside choice-100
  function calcWinMultiplier(
    uint256 _choice,
    bool _over
  ) public view returns (uint256 winMultiplier_) {
  ...
  function calcReward(
    ...
    reward_ = (_wager * calcWinMultiplier(_choice, _over)) / 1e20;
  }
  ...
  function getResultNumbers(
    ...
    numbers_ = modNumbers(_randoms, mod);
  }
    ...
    returns (uint256 payout_, uint32 playedGameCount_, uint256[] memory payouts_)
  {
    ...
    (uint32 choice_, bool over_) = decodeGameData(_game.gameData);
    uint256 reward_ = calcReward(choice_, over_, _game.wager);

    for (uint8 i = 0; i < _game.count; ++i) {
      if (isWon(over_, choice_, _resultNumbers[i])) {
