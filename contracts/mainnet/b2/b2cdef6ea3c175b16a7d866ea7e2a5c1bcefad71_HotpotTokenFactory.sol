...
contract HotpotTokenFactory is IHotpotFactory, Initializable, AccessControl {
    ...
    modifier onlyProjectAdmin(address tokenAddr) {
        bytes32 projectAdminRole = IHotpotToken(tokenAddr)
            .getProjectAdminRole();
        require(
            IHotpotToken(tokenAddr).hasRole(projectAdminRole, msg.sender),
            "not project admin"
        );
        _;
    }
    ...
    function deployToken(
        TokenInfo calldata token,
        uint256 mintfirstAmount
    ) public payable {
        ...
        if (mintfirstAmount > 0) {
            (uint256 minReceive, , , ) = IHotpotToken(address(proxy))
                .estimateMint(msg.value);
            if (token.raisingTokenAddr != address(0)) {
                IERC20(token.raisingTokenAddr).transferFrom(
                    msg.sender,
                    address(this),
                    mintfirstAmount
                );
                IERC20(token.raisingTokenAddr).approve(
                    address(proxy),
                    mintfirstAmount
                );
            }
            IHotpotToken(address(proxy)).mint{
                value: token.raisingTokenAddr == address(0)
                    ? mintfirstAmount
                    : 0
            }(msg.sender, mintfirstAmount, minReceive);
        }
        emit LogTokenDeployed(
            token.tokenType,
            token.bondingCurveType,
            tokenId,
            address(proxy)
        );
    }
    ...
    function setPlatformTaxRate(
        uint256 platformMintTax,
        uint256 platformBurnTax
    ) public onlyRole(PLATFORM_ADMIN_ROLE) {
        require(
    ...
    function addBondingCurveImplement(
        address impl
    ) public onlyRole(PLATFORM_ADMIN_ROLE) {
        require(impl != address(0), "invalid implement");
        string memory bondingCurveType = IBondingCurve(impl).BondingCurveType();
        require(
            bytes(bondingCurveType).length != bytes("").length,
            "bonding curve type error"
        );
        require(
            _implementsMap[bondingCurveType] == address(0),
    ...
    function getBondingCurveImplement(
    ...
    function updateHotpotImplement(
    ...
    function requestUpgrade(
        ...
        require(
            tokenImpl !=
                _proxyAdmin.getProxyImplementation(
    ...
    ) external onlyProjectAdmin(proxyAddress) {
     ...
     */
    function upgradeTokenImplement(
        address proxyAddress
    ) external payable override onlyRole(PLATFORM_ADMIN_ROLE) {
        require(
            upgradeTimelock[proxyAddress] != 0 &&
                upgradeTimelock[proxyAddress] <= block.timestamp,
            "Upgrade Failed: timelock"
        );
        (address impl, bytes memory data) = abi.decode(
            upgradeList[proxyAddress],
            (address, bytes)
        );
        upgradeTimelock[proxyAddress] = 0;
        upgradeList[proxyAddress] = new bytes(0);
        ...
        emit LogTokenImplementUpgraded(
            proxyAddress,
            tokensType[proxyAddress],
    ...
    function removeAllHookForToken(
        address token
    ) external override onlyProjectAdmin(token) {
        address[] memory hooks = tokenHooks[token];
        for (uint256 i = 0; i < hooks.length; i++) {
...
interface IHotpotFactory {
    ...
    function createGovernorForToken(
        address proxyAddr,
        IGovernor.GovInfo calldata token
    ) external;
    ...
    event LogTokenUpgradeRejected(
        address proxyAddress,
        address rejecter,
        string reason
    );
    event LogTokenImplementUpgraded(
        address proxyAddress,
        string tokenType,
        address implementAddr
    );
