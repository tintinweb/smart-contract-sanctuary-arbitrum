/**
 *Submitted for verification at Arbiscan on 2022-04-06
*/
...
// File contracts/crosschainStablecoinSlim/crosschainStablecoinSlimv2.sol
...
contract crosschainStablecoinSlimV2 is ReentrancyGuard, VaultNFTv4 {
    ...
    constructor(
        ...
        if( collateral.decimals() < mai.decimals() ){
            cfDelta = (uint256(10) ** (uint256(mai.decimals()).sub(uint256(collateral.decimals()))) );
        }else{
            cfDelta=1;
        }
    ...
    function payBackToken(uint256 vaultID, uint256 amount) external {
        ...
        ).div(getEthPriceSource().mul(10000).mul(cfDelta));
    ...
    function checkExtract(uint256 vaultID) public view returns (uint256) {
        ...
        if((halfDebt).div(10**priceSourceDecimals)<=minDebt){
            ...
            return debtValue.mul(gainRatio).div(1000).div(getEthPriceSource()).div(cfDelta);
        }else{
            return halfDebt.mul(gainRatio).div(1000).div(getEthPriceSource()).div(cfDelta);
        }
    }
...
// File contracts/crosschainStablecoinSlim/crosschainQiStablecoinSlimv2.sol

pragma solidity 0.5.17;


contract crosschainQiStablecoinSlimV2 is crosschainStablecoinSlimV2, Ownable {
    constructor(
        address ethPriceSourceAddress,
        uint256 minimumCollateralPercentage,
        string memory name,
