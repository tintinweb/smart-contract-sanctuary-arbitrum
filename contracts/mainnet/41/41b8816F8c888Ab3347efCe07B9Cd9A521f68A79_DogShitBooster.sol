...
interface IMasterChef {
  ...
  function pendingDogShit(address _stakeToken, address _user) external view returns (uint256);
  ...
  function dogshitPerBlock() external view returns (uint256);
  ...
  function depositDogShit(address _for, uint256 _amount) external;

  function withdrawDogShit(address _for, uint256 _amount) external;
  ...
  function dogshit() external returns (IDOGSHIT);
}
...
contract DogShitBooster is Ownable, Pausable, ReentrancyGuard, AccessControl, IMasterChefCallback, IERC721Receiver {
  ...
  IDogShitBoosterConfig public dogshitboosterConfig;
  IERC20 public dogshit;
  ...
  constructor(
    ...
    IDogShitBoosterConfig _dogshitboosterConfig,
    address _wNative
  ) public {
    require(_wNative != address(0), "DogShitBooster::constructor:: _wNative cannot be address(0)");
    require(address(_dogshit) != address(0), "DogShitNFTOffering::constructor:: _dogshit cannot be address(0)");
    require(address(_masterChef) != address(0), "DogShitNFTOffering::constructor:: _masterChef cannot be address(0)");
    require(
      address(_dogshitboosterConfig) != address(0),
      "DogShitNFTOffering::constructor:: _dogshitboosterConfig cannot be address(0)"
    );
    ...
    dogshitboosterConfig = _dogshitboosterConfig;
    dogshit = _dogshit;
    wNative = _wNative;
  ...
  modifier inExec() {
    require(_IN_EXEC_LOCK == _NOT_ENTERED, "DogShitBooster::inExec:: in exec lock");
    require(address(masterChef) == _msgSender(), "DogShitBooster::inExec:: not from the master chef");
  ...
  modifier isStakeTokenOK(address _stakeToken) {
    require(dogshitboosterConfig.stakeTokenAllowance(_stakeToken), "DogShitBooster::isStakeTokenOK::bad stake token");
    _;
  }

  /// @dev validate whether a specified nft can be staked into a particular staoke token
  modifier isDogShitBoosterNftOK(
    address _stakeToken,
    address _nftAddress,
    uint256 _nftTokenId
  ) {
    require(
      dogshitboosterConfig.dogshitboosterNftAllowance(_stakeToken, _nftAddress, _nftTokenId),
      "DogShitBooster::isDogShitBoosterNftOK::bad nft"
    );
    _;
  }
  ...
  modifier permit(bytes calldata _sig) {
    ...
    require(recoveredAddress == _msgSender(), "DogShitBooster::permit::INVALID_SIGNATURE");
    _;
  }

  modifier onlyGovernance() {
    require(hasRole(GOVERNANCE_ROLE, _msgSender()), "DogShitBooster::onlyGovernance::only GOVERNANCE role");
    _;
  }

  /// @dev Require that the caller must be an EOA account to avoid flash loans.
  modifier onlyEOA() {
    require(msg.sender == tx.origin, "DogShitBooster::onlyEOA:: not eoa");
    _;
  }
  ...
  /// @dev View function to see pending booster DOGSHITs on frontend.
  function pendingBoosterDogShit(address _stakeToken, address _user) external view returns (uint256) {
    uint256 pendingDogShit = masterChef.pendingDogShit(_stakeToken, _user);
    NFTStakingInfo memory stakingNFT = userStakingNFT[_stakeToken][_user];
    if (stakingNFT.nftAddress == address(0)) {
      return 0;
    }
    (, , uint256 boostBps) = dogshitboosterConfig.energyInfo(stakingNFT.nftAddress, stakingNFT.nftTokenId);
    // if (currentEnergy == 0) {
    //   return 0;
    // }
    return pendingDogShit.mul(boostBps).div(1e4);
    // return Math.min(currentEnergy, pendingDogShit.mul(boostBps).div(1e4));
  }

  /// @dev Internal function for withdradogshit a boosted stake token and receive a reward from a master chef
  ...
  function _withdrawFromMasterChef(IERC20 _stakeToken, uint256 _shares) internal {
    if (_shares == 0) return;
    if (address(_stakeToken) == address(dogshit)) {
      masterChef.withdrawDogShit(_msgSender(), _shares);
    } else {
  ...
  function _harvestFromMasterChef(address user, IERC20 _stakeToken) internal {
    ...
    uint256 beforeReward = dogshit.balanceOf(user);
    masterChef.harvest(user, address(_stakeToken));

    emit Harvest(user, _stakeToken, dogshit.balanceOf(user).sub(beforeReward));
  }
  ...
  function stakeNFT(
    ...
    isDogShitBoosterNftOK(_stakeToken, _nftAddress, _nftTokenId)
    nonReentrant
    onlyEOA
  {
  ...
  function _stakeNFT(
    ...
    require(
      ...
      "DogShitBooster::stakeNFT:: nft already staked"
    );
  ...
  function _unstakeNFT(address _stakeToken) internal {
    NFTStakingInfo memory toBeSentBackNft = userStakingNFT[_stakeToken][_msgSender()];
    require(toBeSentBackNft.nftAddress != address(0), "DogShitBooster::stakeNFT:: no nft staked");
  ...
  function stake(IERC20 _stakeToken, uint256 _amount)
    ...
    nonReentrant
  {
    require(_amount > 0, "DogShitBooster::stake::nothing to stake");
    ...
    if (address(_stakeToken) == address(dogshit)) {
      masterChef.depositDogShit(_msgSender(), _amount);
    } else {
  ...
  function _unstake(IERC20 _stakeToken, uint256 _amount) internal {
    require(_amount > 0, "DogShitBooster::_unstake::use harvest instead");
  ...
  function harvest(address[] calldata _stakeTokens) external whenNotPaused nonReentrant {
    for (uint256 i = 0; i < _stakeTokens.length; i++) {
      require(dogshitboosterConfig.stakeTokenAllowance(_stakeTokens[i]), "DogShitBooster::harvest::bad stake token");
  ...
  /// @dev currently, the caller will be a master chef calling before a dogshit lock
  function masterChefCall(
    address stakeToken,
    address userAddr,
    uint256 unboostedReward
    ...
    (, uint256 currentEnergy, uint256 boostBps) = dogshitboosterConfig.energyInfo(
    ...
    // dogshitboosterConfig.consumeEnergy(stakingNFT.nftAddress, stakingNFT.nftTokenId, extraReward);
  ...
  function _safeWrap(IERC20 _quoteBep20, uint256 _amount) internal {
    if (msg.value != 0) {
      require(address(_quoteBep20) == wNative, "DogShitBooster::_safeWrap:: baseToken is not wNative");
      require(_amount == msg.value, "DogShitBooster::_safeWrap:: value != msg.value");
