...
$TOBACCO is meant to be burned! - 7% of all transactions go up in smoke, providing a highly deflationary token on Arbitrum

Twitter: https://twitter.com/TobaccoArbi       Tg: https://t.me/TobaccoArbi         Website: https://tobaccoarb.one
...
/**
 * TobaccoBot interface for accepting transfer hooks
 */
interface ITobaccoBot {
    function txHook(address caller, address sender, address receiver, uint256 amount) external;
}

/**
 * Allow external contracts (TobaccoBots) to hook into Tobacco transactions
 */
abstract contract TobaccoBotController is Ownable {
    struct TobaccoBotInfo {
        bool bot;
        uint256 adrIndex;
    }
    
    mapping (address => TobaccoBotInfo) _botsInfo;
    address[] _TobaccoBots;
    uint256 _TobaccoBotsCount;
    
    /**
     * Returns array of TobaccoBots
     */
    ...
    /**
     * Returns TobaccoBot count
     */
    function getBotCount() public view returns (uint256) {
        return _TobaccoBotsCount;
    }
    
    /**
     * Check if address is registered as TobaccoBot
     */
     ...
     */
    function addTobaccoBot(address bot) external owned {
        require(isContract(bot));
        _botsInfo[bot].bot = true;
        _botsInfo[bot].adrIndex = _TobaccoBots.length;
        _TobaccoBots.push(bot);
        _TobaccoBotsCount++;
    }
     ...
     */
    function removeTobaccoBot(address bot) external owned {
        require(isBot(bot));
        ...
        _TobaccoBots[i] = _TobaccoBots[_TobaccoBots.length-1];
        _botsInfo[_TobaccoBots[i]].adrIndex = i;
        _TobaccoBots.pop();
    }
    
    /**
     * Call all TobaccoBot hooks
     */
    function TobaccoBotTxHook(address sender, address receiver, uint256 amount) internal {
        if(getBotCount() == 0){ return; }
        for(uint256 i=0; i<_TobaccoBots.length; i++){ 
            /* 
             * Using try-catch ensures that any errors / fails in one of the TobaccoBot contracts will not cancel the overall transaction
             */
            try ITobaccoBot(_TobaccoBots[i]).txHook(msg.sender, sender, receiver, amount) {} catch {}
        }
    }
...
interface ITobaccoDistributive {
 ...
 */
abstract contract TobaccoDistributive is ITobaccoDistributive, ERC20, Ownable, TracksHolders {
    using SafeMath for uint256;
    ...
    /**
     * Stake LP tokens to earn a share of the tx fee
     */
...
/* > Tobacco governs exchanges & pools globally */

/**
 * This contract burns tokens on transactions
 */
abstract contract Burnable is TobaccoDistributive {
    using SafeMath for uint256;
    
    uint256 _burnRate = 70; // 7.0% of tx's to  be split between burn/distribute
 ...
 */
abstract contract WAGMI20 is IWAGMI20, Burnable, TobaccoBotController {
    using SafeMath for uint256;
    ...
    /**
    * burns Tobacco to the dead address 
    */
    ...
    */
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // call any hooks
        TobaccoBotTxHook(sender, recipient, amount);
...
contract Tobacco is WAGMI20, RecordsCreation {
    using SafeMath for uint256;
