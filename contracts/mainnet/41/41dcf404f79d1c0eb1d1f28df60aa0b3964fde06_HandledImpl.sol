...
import {CErc20Delegate} from "tender/compound/CErc20Delegate.sol";
...
contract CErc20 is CToken, CErc20Interface {
   ...
   * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
   */
  function redeemUnderlyingForUser(uint256 redeemAmount, address user) external override returns (uint256) {
  ...
  function doTransferIn(address from, uint256 amount) internal virtual override returns (uint256) {
    ...
    if (isGLP) {
      EIP20NonStandardInterface(stakedGLP).transferFrom(from, address(this), amount);
    } else {
  ...
  function doTransferOut(address payable to, uint256 amount) internal virtual override {
    ...
    if (isGLP) {
      EIP20NonStandardInterface(stakedGLP).transfer(to, amount);
    } else {
