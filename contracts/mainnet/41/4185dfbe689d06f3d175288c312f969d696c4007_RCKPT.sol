...
constructor(
...
) ERC20("RCKPT", "RCKPT") DividendDistributor(_routerAddress, _WETH) {
    ...
    ReflectionWallet = payable(address(this));  // Set the ReflectionWallet to the contract address
    ...
    function getEthAmount(uint256 RCKPTAmount) internal view returns (uint256) {
    address[] memory path = new address[](2);
    path[0] = address(this);
    path[1] = address(WETH);

    uint256[] memory amounts = swapRouter.getAmountsIn(RCKPTAmount, path);
    ...
    // Use Sushiswap factory to create the pair
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
    return _RCKPTTransfer(_msgSender(), recipient, amount);
}
...
function _RCKPTTransfer(
    ...
    // Trigger swapBack if conditions are met
    if (shouldSwapBack()) {
        swapBack();
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHReflection = (amountETH * ReflectionFee) / (totalFee);
        ...
        WETH.transfer(address(dividendDistributor), amountETHReflection);
