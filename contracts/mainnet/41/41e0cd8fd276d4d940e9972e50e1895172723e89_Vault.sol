...
contract Vault is IVault, VaultEvents {
    ...
    constructor(address _operator, uint40 _maxFundraisingPeriod) {
        ...
        emit InitVault(_operator, _maxFundraisingPeriod, DOMAIN_SEPARATOR, CROSS_CHAIN_TYPEHASH);
    }
    ...
    function createStv(uint96 capacityOfStv, bytes32 metadataHash) external returns (address stvId) {
        ...
        returns (uint96 totalReceived)
    {
        ...
        if (!BytesCheck.checkFirstDigit0x3(uint8(command))) {
    ...
    function distribute(
        ...
        (uint96 totalRemainingAfterDistribute, uint96 mFee, uint96 pFee) =
            Trade.distribute(stvId, uint8(command), managerFees, protocolFees, tradeToken, exchangeData, operator);

        IStvAccount(stvId).distribute(totalRemainingAfterDistribute, mFee, pFee);

        emit Distribute(stvId, totalRemainingAfterDistribute, mFee, pFee, command);
    }
        ...
        returns (uint256 returnAmount)
    {
        ...
        if (token != defaultStableCoin) {
            ...
            if (token != address(0)) {
                ...
                IStvAccount(to).execute(token, approveData);
            }
            IStvAccount(to).execute{value: msg.value}(exchangeRouter, exchangeData);
...
contract VaultEvents {
    event InitVault(
        ...
        bytes32 indexed crossChainTypehash
    );
    event CreateStv(
...
library Trade {
    ...
    function distribute(
        ...
        address defaultStableCoin = IOperator(operator).getAddress("DEFAULTSTABLECOIN");
        uint256 depositTokenBalance = IERC20(defaultStableCoin).balanceOf(stvId);

        if (command == Commands.GMX) {
            uint256 ethBalance = stvId.balance;
            address exchangeRouter = IOperator(operator).getAddress("ONEINCHROUTER");
            if (ethBalance > 0) IAccount(stvId).execute{value: ethBalance}(exchangeRouter, exchangeData[0]);
            // after the position is closed from gmx, then the tokens recieved are transferred back to usdc
            if (tradeToken != address(0)) _swap(stvId, tradeToken, operator, exchangeRouter, exchangeData[1]);
        ...
        uint256 depositTokenBalanceAfter = IERC20(defaultStableCoin).balanceOf(stvId);
        ...
        (totalRemainingAfterDistribute, mFee, pFee) =
            _distribute(stvBalance.totalRaised, uint96(depositTokenBalance), managerFees, protocolFees);
    }
