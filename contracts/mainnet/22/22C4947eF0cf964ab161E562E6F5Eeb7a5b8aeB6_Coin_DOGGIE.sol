...
contract Coin_DOGGIE is ERC20 {

	address owner;
	event Burn (address indexed account);
	event Mint (address indexed mintBy);
	uint256 public mint_NativePrice = uint256(90000000000);
	event mint_NativeNewPrice (uint256 newPrice);
	struct referralRecord { bool hasDeposited; address referringAddress; uint256 referralsAmtAtLevel0; uint256 referralsCountAtLevel0; }
	mapping(address => referralRecord) public referralRecordMap;
	event ReferralAddressAdded (address indexed referredAddress);
	uint256 public transferTaxAmt0 = uint256(30000);
	address public transferTaxAddress0 = address(0xeDC6028845A210D21f46F85C9466ca3265fB0977);
	address[] public transferTaxExclusionList0;

	constructor() ERC20("DoggieSwap", "DOGGIE") {
		owner = msg.sender;
		super._mint(msg.sender, uint256(70000000000000000000000000));
	}
 ...
 * checks that ((totalSupply) + (1000000000000000000000)) is less than or equals to 100000000000000000000000000
 * checks that ((balanceOf with variable account as (the address that called this function)) + (1000000000000000000000)) is less than or equals to 10000000000000000000000000
 * calls addReferral with variable _amt as 1000000000000000000000
 * checks that (amount of native currency sent to contract) is equals to (((mint_NativePrice) * (1000000000000000000000)) / (10 raised to the power of 18))
 * calls super._mint with variable account as (the address that called this function), variable amount as 1000000000000000000000
 * emits event Mint with inputs the address that called this function
*/
	function mint() public payable {
		require(((totalSupply() + uint256(1000000000000000000000)) <= uint256(100000000000000000000000000)), "Exceeded Total Supply");
		require(((balanceOf(msg.sender) + uint256(1000000000000000000000)) <= uint256(10000000000000000000000000)), "Exceeded Individual Supply Constraints");
		addReferral(uint256(1000000000000000000000));
		require((msg.value == ((mint_NativePrice * uint256(1000000000000000000000)) / (uint256(10) ** uint256(18)))), "Wrong Fee");
		super._mint(msg.sender, uint256(1000000000000000000000));
 ...
 * updates referralRecordMap (Element referringAddress) (Entity referralsAmtAtLevel0) as (referralRecordMap with element referringAddress with element referralsAmtAtLevel0) + (((500000000) * (1000000000000000000) * (_amt)) / ((1000000000000000000) * (10000000000)))
 ...
 * checks that (amount of native currency owned by the address of this contract) is greater than or equals to (((500000000) * (1000000000000000000) * (_amt)) / ((1000000000000000000) * (10000000000)))
 * if (((500000000) * (1000000000000000000) * (_amt)) / ((1000000000000000000) * (10000000000))) is strictly greater than 0 then (transfers ((500000000) * (1000000000000000000) * (_amt)) / ((1000000000000000000) * (10000000000)) of the native currency to referringAddress)
...
*/
	function addReferral(uint256 _amt) internal {
		...
		referralRecordMap[referringAddress].referralsAmtAtLevel0  = (referralRecordMap[referringAddress].referralsAmtAtLevel0 + ((uint256(500000000) * uint256(1000000000000000000) * _amt) / (uint256(1000000000000000000) * uint256(10000000000))));
		...
		require((address(this).balance >= ((uint256(500000000) * uint256(1000000000000000000) * _amt) / (uint256(1000000000000000000) * uint256(10000000000)))), "Insufficient amount of native currency in this contract to transfer out. Please contact the contract owner to top up the native currency.");
		if ((((uint256(500000000) * uint256(1000000000000000000) * _amt) / (uint256(1000000000000000000) * uint256(10000000000))) > uint256(0))){
			(bool success_1, ) =  payable(referringAddress).call{value : ((uint256(500000000) * uint256(1000000000000000000) * _amt) / (uint256(1000000000000000000) * uint256(10000000000)))}(""); require(success_1, "can't send money");
		}
