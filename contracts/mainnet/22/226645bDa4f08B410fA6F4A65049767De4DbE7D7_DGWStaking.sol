...
contract DGWStaking is IDChainStaking, DChainBase {
  ...
  function _validateMinimumStakingAmount(
    ...
    if (
      ...
      address baseToken = _stakeToken == token0 ? token0 : token1;
      address quoteToken = _stakeToken == token0 ? token1 : token0;

      amountOut = OracleLibrary.getQuoteAtTick(
        tick,
        uint128(_stakingAmount),
        baseToken,
        quoteToken
      );
    }
  ...
  function _convertTokenToUSD(
    ...
    if (
      ...
      totalStakingAmountInUSD = _calculatePriceUsingUniswapV3PoolOracle(
        address(rewardToken),
        _stakingAmount,
        false
      );
