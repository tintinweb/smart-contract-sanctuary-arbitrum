    ...
    ERC721HolderUpgradeable
{
    ...
    function deposit(
        ...
        _totalVirtualSharesSupply += _getVirtualSharesToMint(assets);
        _totalAssets += assets;
    ...
    function withdraw(
        ...
        lpTokenIdToDetails[lpTokenId].pnl = pnl;
     ...
     * @param _newFees percentage of fee ; 100 is 1%
     */
    function updateAdminPerformanceFees(
        uint256 _newFees
    ) external hasRole(Roles.VAULT_MANAGER) {
    ...
    function _updateConversionRate() internal {
        ...
        uint newRate = supply == 0
            ? 10 ** decimals()
            : uint256(10 ** decimals()).mulDivUp(totalAssets(), supply);
        ...
        return
            uint(
                int(_assets) -
                    (int(
                        int(_assets) *
                            _conversionRateDifference *
                            int(getTimeFactor(_createdAt))
                    ) / int(scalingFactor))
            );
    }
