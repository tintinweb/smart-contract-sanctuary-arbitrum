...
import "./CryptsZeeHandlerContracts.sol";

contract CryptsZeeHandler is
    Initializable,
    CryptsZeeHandlerContracts
{

    function initialize() external initializer {
        CryptsZeeHandlerContracts.__CryptsZeeHandlerContracts_init();

        _setZeeDiversionPoints(20);
        _setZeePercentOfPoolClaimed(200); // 0.2%
    }
    ...
    function setZeeDiversionPoints(uint24 _ZeeDiversionPoints) external onlyAdminOrOwner {
        _setZeeDiversionPoints(_ZeeDiversionPoints);
    }

    function setZeePercentOfPoolClaimed(uint32 _ZeePercentOfPoolClaimed) external onlyAdminOrOwner {
        _setZeePercentOfPoolClaimed(_ZeePercentOfPoolClaimed);
    }

    function _setZeeDiversionPoints(uint24 _ZeeDiversionPoints) private {
        zeeDiversionPoints = _ZeeDiversionPoints;
        emit ZeeDiversionPointsChanged(_ZeeDiversionPoints);
    }

    function _setZeePercentOfPoolClaimed(uint32 _ZeePercentOfPoolClaimed) private {
        zeePercentOfPoolClaimed = _ZeePercentOfPoolClaimed;
        emit ZeePercentOfPoolClaimedChanged(_ZeePercentOfPoolClaimed);
    }
        ...
        public
    {
        ...
        IERC721(zeeAddress).safeTransferFrom(
            _user,
            address(this),
...
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "./CryptsZeeHandlerState.sol";

abstract contract CryptsZeeHandlerContracts is Initializable, CryptsZeeHandlerState{

    function __CryptsZeeHandlerContracts_init() internal initializer {
        CryptsZeeHandlerState.__CryptsZeeHandlerState_init();
    }
    ...
    function setContracts(
        ...
        zeeAddress = _zeeAddress;
        corruptionCrypts = ICorruptionCrypts(_corruptionCryptsAddress);
    }

    modifier contractsAreSet() {
        require(areContractsSet(), "CryptsZeeHandlerContracts: Contracts aren't set");
        _;
    }

    function areContractsSet() public view returns(bool) {
        return zeeAddress != address(0)
...
abstract contract CryptsZeeHandlerState is
    Initializable,
    AdminableUpgradeable,
    ERC721HolderUpgradeable,
    ICryptsCharacterHandler
{
    event ZeeDiversionPointsChanged(uint24 _diversionPoints);
    event ZeePercentOfPoolClaimedChanged(uint32 _percent);

    ICorruptionCrypts public corruptionCrypts;
    address public zeeAddress;

    bool public stakingAllowed;

    uint24 public zeeDiversionPoints;
    uint32 public zeePercentOfPoolClaimed;

    function __CryptsZeeHandlerState_init() internal initializer {
