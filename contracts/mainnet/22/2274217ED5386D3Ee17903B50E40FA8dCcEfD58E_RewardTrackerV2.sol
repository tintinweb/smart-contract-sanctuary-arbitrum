    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(address => uint256) public cumulativeRewardPerTokens;
    ...
    mapping(address => mapping(address => uint256)) public cumulativeRewards;
    ...
    event Claim(address receiver, address rewardToken, uint256 amount);
    event SetEsNeu(address esNeu);
    ...
    function setEsNeu(address _esNeu) external onlyGov {
        esNeu = _esNeu;
        emit SetEsNeu(_esNeu);
    }
    ...
    function tokensPerInterval(address _rewardToken) external view returns (uint256) {
        return IRewardDistributor(distributor).getTokensPerIntervals(_rewardToken);
    }
    ...
    function claimable(address _account, address _token) public view returns (uint256) {
        ...
        if (stakedAmount == 0) {
            return claimableRewards[_token][_account];
        }

        uint256 supply = totalSupply;
        uint256 pendingReward = IRewardDistributor(distributor).pendingRewards(_token) * PRECISION;
        uint256 nextCumulativeRewardPerToken = cumulativeRewardPerTokens[_token] + (pendingReward / supply);

        return
            claimableRewards[_token][_account] +
            ((stakedAmount * (nextCumulativeRewardPerToken - previousCumulatedRewardPerToken[_token][_account])) /
                PRECISION);
    }

    function claimables(address _account) public view returns (uint256[] memory) {
        uint256[] memory _claimables;
        address[] memory _rewardTokens = rewardTokens();

        for (uint256 i = 0; i < _rewardTokens.length; i++) {
            _claimables[i] = claimable(_account, _rewardTokens[i]);
        }
        return _claimables;
    }

    function rewardTokens() public view returns (address[] memory) {
        return IRewardDistributor(distributor).getRewardTokens();
    }
    ...
    function _claim(address _account, address _receiver) private returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            tokenAmounts[i] = claimableRewards[rewardToken][_account];
            claimableRewards[rewardToken][_account] = 0;

            if (tokenAmounts[i] > 0) {
                IERC20(rewardToken).transfer(_receiver, tokenAmounts[i]);
                emit Claim(_account, rewardToken, tokenAmounts[i]);
            }
        }
        return tokenAmounts;
    }
    ...
    function _updateRewards(address _account) private {
        ...
        for (uint256 i = 0; i < _rewardTokens.length; i++) {
            ...
            address rewardToken = _rewardTokens[i];
            uint256 _cumulativeRewardPerToken = cumulativeRewardPerTokens[rewardToken];

            if (totalSupply > 0 && blockReward > 0) {
                _cumulativeRewardPerToken = _cumulativeRewardPerToken + ((blockReward * PRECISION) / totalSupply);
                cumulativeRewardPerTokens[rewardToken] = _cumulativeRewardPerToken;
            }
            ...
            if (_account != address(0)) {
                ...
                uint256 accountReward =
                    (stakedAmount *
                        (_cumulativeRewardPerToken - previousCumulatedRewardPerToken[rewardToken][_account])) /
                    PRECISION;
                uint256 _claimableReward = claimableRewards[rewardToken][_account] + accountReward;

                claimableRewards[rewardToken][_account] = _claimableReward;
                previousCumulatedRewardPerToken[rewardToken][_account] = _cumulativeRewardPerToken;

                if (_claimableReward > 0 && stakedAmounts[_account] > 0) {
                    uint256 nextCumulativeReward = cumulativeRewards[rewardToken][_account] + accountReward;

                    if (rewardToken == esNeu) {
                        averageStakedAmounts[_account] =
                            ((averageStakedAmounts[_account] * cumulativeRewards[rewardToken][_account]) /
                                nextCumulativeReward) +
                            ((stakedAmount * accountReward) / nextCumulativeReward);
                    }

                    cumulativeRewards[rewardToken][_account] = nextCumulativeReward;
                }
            }
        }
    }
