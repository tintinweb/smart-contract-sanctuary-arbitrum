...
contract Treasury is ITreasury, ContractGuard, ReentrancyGuard {
    ...
    function initialize(
        ...
        nextSupplyTarget = 1000000 ether; // 1B supply is the next target to reduce expansion rate
        allocateSeigniorageSalary = 1 ether; // 1 MAINTOKEN for allocateSeigniorage() calling
        

        REWARD_RATE_DENOMINATION = 1000000;
        REWARD_RATE_ESTOKEN = 9000000;
        initialized = true;
        operator = msg.sender;
    ...
    function setAllocateSeigniorageSalary(uint256 _allocateSeigniorageSalary)
        ...
        onlyOperator
    {
        require(_allocateSeigniorageSalary <= 1000 ether, "too much");
    ...
    function _sendToBoardroom(uint256 _amount, uint256 _expanded) internal {
        ...
        uint256 _mamount = _amount.mul(REWARD_RATE_DENOMINATION).div(REWARD_RATE_DENOMINATION.add(REWARD_RATE_ESTOKEN));
        uint256 _esamount = _amount.sub(_mamount);
        ...
        if (daoFundSharedPercent > 0) {
            _daoFundSharedAmount = _mamount.mul(daoFundSharedPercent).div(10000);
        ...
        if (marketingFundSharedPercent > 0) {
            _marketingFundSharedAmount = _mamount
        ...
        IERC20(_maintoken).safeIncreaseAllowance(boardroom, _mamount);
        IBoardroom(boardroom).allocateSeigniorage(_maintoken, _mamount);

        IERC20(_maintoken).transfer( convertContractAddress, _esamount);
