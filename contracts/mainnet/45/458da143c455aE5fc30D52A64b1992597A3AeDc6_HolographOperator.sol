...
contract HolographOperator is Admin, Initializable, HolographOperatorInterface {
  ...
  /**
   * @dev Deletes a job from the _operatorJobs mapping.
   * @param jobHash The hash of the job to delete.
   */
  function deleteOperatorJob(bytes32 jobHash) external onlyAdmin {
    if (_operatorJobs[jobHash] == 0) {
      revert JobDoesNotExist(jobHash);
    }
    delete _operatorJobs[jobHash];
  }

  /**
   * @dev Deletes a job from the _failedJobs mapping.
   * @param jobHash The hash of the job to delete.
   */
  function deleteFailedJob(bytes32 jobHash) external onlyAdmin {
    if (!_failedJobs[jobHash]) {
      revert JobDoesNotExist(jobHash);
    }
    delete _failedJobs[jobHash];
  }

  /**
   * @dev Deletes multiple jobs from the _operatorJobs mapping.
   * @param jobHashes An array of job hashes to delete.
   */
  function deleteMultipleOperatorJobs(bytes32[] memory jobHashes) external onlyAdmin {
    for (uint256 i = 0; i < jobHashes.length; i++) {
      if (_operatorJobs[jobHashes[i]] == 0) {
        revert JobDoesNotExist(jobHashes[i]);
      }
      delete _operatorJobs[jobHashes[i]];
    }
  }

  /**
   * @dev Deletes multiple jobs from the _failedJobs mapping.
   * @param jobHashes An array of job hashes to delete.
   */
  function deleteMultipleFailedJobs(bytes32[] memory jobHashes) external onlyAdmin {
    for (uint256 i = 0; i < jobHashes.length; i++) {
      if (!_failedJobs[jobHashes[i]]) {
        revert JobDoesNotExist(jobHashes[i]);
      }
      delete _failedJobs[jobHashes[i]];
    }
  }
  ...
  function operatorJobExists(bytes32 jobHash) external view returns (bool) {
    return _operatorJobs[jobHash] > 0;
  }
...
interface HolographOperatorInterface {
  ...
  /**
   * @dev Error thrown when a job does not exist
   */
  error JobDoesNotExist(bytes32 jobHash);
