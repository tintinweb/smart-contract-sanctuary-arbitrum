...
// File contracts/strategies/peapods/peapods-farm-bases/peapods-farm-lp-base.sol
...
abstract contract StrategyPeapodsLPFarmBase is StrategyBaseV3 {
  ...
  address public constant arb_usdc = 0xD40b7B05AB39A39658de1D7c9301A9ed765B3472;
...
// File contracts/strategies/peapods/strategy-peapods-peas-ohm.sol
...
contract StrategyPeapodsPeasOhm is StrategyPeapodsLPFarmBase{
  ...
  function harvest() public override onlyBenevolent {
    ...
    if (_peas > 0) {
      ...
      _swapCamelotYak(peas, path, adapters, adapters, _peas);
    }
    ...
    if (rewardToken != address(0)) {
      ...
      if (_reward > 0) {
        ...
        adapters[2] = usdc_usdcE_adapter;

        address[] memory recipients = new address[](3);
        recipients[0] = arb_usdc;
        recipients[1] = usdc_usdcE_adapter;
        recipients[2] = usdc_usdcE_adapter;

        _swapCamelotYak(rewardToken, path, adapters, recipients, _reward);
      }
    }

    uint256 _weth = IERC20(weth).balanceOf(address(this));

    // Swap half of WETH for token0
    if (_weth > 0) {
        address[] memory path = new address[](2);
        path[0] = weth;
        path[1] = token0;
        ...
        _swapCamelotYak(weth, path, adapters, adapters, _weth.div(2));
    }
    ...
    if (_weth > 0) {
      ...
      _swapCamelotYak(weth, path, adapters, recipients, _weth.div(2));
    }
