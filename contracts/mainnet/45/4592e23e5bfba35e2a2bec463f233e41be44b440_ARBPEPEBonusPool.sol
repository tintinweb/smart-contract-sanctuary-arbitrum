...
// File: contracts/pool/ARBPEPEBonusPool.sol
...
contract ARBPEPEBonusPool is Ownable {
    ...
    function stopReward() public onlyDev {
    ...
    function add(uint256 _allocPoint, address _token, bool _withUpdate) public onlyDev {
        require(_token != address(0), "ARBPEPEBonusPool: _token is the zero address");

        require(!EnumerableSet.contains(_pairs, _token), "ARBPEPEBonusPool: _token is already added to the pool");
    ...
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate) public onlyDev {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function setMaxStakingPerUser(uint256 amount) public onlyDev {
        maxStakingPerUser = amount;
    }

    function setStandardRewardInterval(uint256 interval) public onlyDev {
        require(interval > 0, "ARBPEPEBonusPool: interval must be greater than 0");
        standardRewardInterval = interval;
    }

    function pendingReward(uint256 _pid, address _user) public view returns (uint256) {
        require(_pid <= poolInfo.length - 1, "ARBPEPEBonusPool: Can not find this pool");
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require(_amount.add(user.amount) <= maxStakingPerUser, 'ARBPEPEBonusPool: exceed max stake');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "ARBPEPEBonusPool: Withdraw with insufficient balance");
    ...
    function emergencyRewardWithdraw(uint256 _amount) public onlyDev {
        require(_amount <= rewardToken.balanceOf(address(this)), 'ARBPEPEBonusPool: not enough token');
        rewardToken.safeTransfer(address(msg.sender), _amount);
    }

    function getPoolView(uint256 pid) public view returns (PoolView memory) {
        require(pid < poolInfo.length, "ARBPEPEBonusPool: pid out of range");
    ...
    function addCaller(address val) public onlyDev() {
        require(val != address(0), "ARBPEPEBonusPool: val is the zero address");
        _callers.add(val);
    }

    function delCaller(address caller) public onlyDev returns (bool) {
        require(caller != address(0), "ARBPEPEBonusPool: caller is the zero address");
