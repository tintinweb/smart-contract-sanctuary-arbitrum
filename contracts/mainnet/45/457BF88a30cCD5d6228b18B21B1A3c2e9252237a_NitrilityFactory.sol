...
interface INitrilityFactory {
    ...
    ) external pure returns (NitrilityCommon.TemplateType memory);
}
...
contract NitrilityFactory is Ownable, ReentrancyGuard {
    ...
    EnumerableSet.Bytes32Set private artistIdSet;
    ...
    uint256 public constant gasFee = 1e15; // 0.001 ether or 1e15 wei
    ...
    modifier onlyAuction() {
        require(
            msg.sender == auctionAddr || msg.sender == syncAddr,
    ...
    function setSyncAddr(address _syncAddr) external onlyCaller {
        syncAddr = _syncAddr;
    }

    function isCollection(address collection) internal view returns (bool) {
        for (uint256 i = 0; i < artistIdSet.length(); i++) {
            string memory artistId = bytes32ToString(artistIdSet.at(i));
            address collectionAddr = idToArtistCollectionAddress[artistId];
    ...
    function createCollection(string memory artistId) internal {
    ...
    function fetchArtistAddressForArtistId(
        ...
        return artistIdToAddresses[artistId].values();
    }
    ...
    function mintLicense(
        ...
        require(
            isCollection(msg.sender),
            "Caller is not a collection contract"
        );

        address collectionAddr = idToArtistCollectionAddress[license.artistId];
    ...
    function burnSoldLicense(uint256 tokenId) external {
        ...
        address collectionAddr = idToArtistCollectionAddress[
            idToSoldLicense[tokenId].artistId
        ];
        NitrilityLicense licenseContract = NitrilityLicense(collectionAddr);
    ...
    function reFundOffer(
        ...
        require(offerPrice >= gasFee, "Insufficient amount to cover gas fee");
    ...
    function hashTemplateType(
        ...
        return
            keccak256(
                abi.encode(
                    keccak256(
                        "TemplateType(uint256 fPrice,uint256 sPrice,uint256 tPrice,uint256 listingFormatValue,bool infiniteSupply,bool infiniteListingDuration,bool infiniteExclusiveDuration,uint256 accessLevel,uint256 listingStartTime,uint256 listingEndTime,uint256 exclusiveEndTime,bool listed)"
                    ),
                    template.fPrice,
                    template.sPrice,
                    template.tPrice,
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "License(string tokenURI,string artistId"
                            //,TemplateType creator,TemplateType advertisement,TemplateType tvSeries,TemplateType movie,TemplateType videoGame,TemplateType aiTraining)TemplateType(uint256 fPrice,uint256 sPrice,uint256 tPrice,uint256 listingFormatValue,bool infiniteSupply,bool infiniteListingDuration,bool infiniteExclusiveDuration,uint256 accessLevel,uint256 listingStartTime,uint256 listingEndTime,uint256 exclusiveEndTime,bool listed)
                        ),
                        keccak256(bytes(voucher.tokenURI)),
                        keccak256(bytes(voucher.artistId))
                        // hashTemplateType(voucher.creator),
                        // hashTemplateType(voucher.advertisement),
                        // hashTemplateType(voucher.tvSeries),
                        // hashTemplateType(voucher.movie),
                        // hashTemplateType(voucher.videoGame),
                        // hashTemplateType(voucher.aiTraining)
                    )
                )
            );
    }
