...
contract WrapperVault is Initializable, ERC20Facet, ReentrancyGuardUpgradeable, FactorGauge, FactorBoostReward {
    using SafeERC20 for IERC20;
    ...
    function initialize(
        ...
        __FactorBoostReward_init(_boostController, _rewardDuration);
        __ReentrancyGuard_init();
    ...
    function deposit(uint256 amount, address from) external nonReentrant {
        ...
        emit Deposit(msg.sender, msg.sender, amount, afterBalance - beforeBalance);
    }

    function withdraw(uint256 shares, address receiver, address owner) external nonReentrant {
        if (shares > balanceOf(msg.sender) || balanceOf(msg.sender) == 0) revert INSUFFICIENT_BALANCE();
        ...
        underlyingAsset.safeTransfer(msg.sender, afterBalance - beforeBalance);
        emit Withdraw(msg.sender, msg.sender, msg.sender, shares, afterBalance - beforeBalance);
    }
    ...
    function unstake(uint256 amount) external nonReentrant {
        if (balanceOf(msg.sender) < amount || balanceOf(msg.sender) == 0) revert INSUFFICIENT_BALANCE();
        // Burn WrapperVault tokens from the user
    ...
    /**
     * @notice redeems the user's scale reward
    ...
    /**
     * @notice returns the user's unclaimed scale reward
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 amount
    ) internal override(ERC20Facet, FactorGauge, FactorBoostReward) {
