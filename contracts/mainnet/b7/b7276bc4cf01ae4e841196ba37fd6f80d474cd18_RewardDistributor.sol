...
contract RewardDistributor is Ownable, ReentrancyGuard {
    ...
    address public constant EMERGENCY_RETURN = address(0); // Emergency return address
    address public constant xNFTE = address(0); // xNFTE contract address
    address public constant Token = address(0); // Native Arbitrum USDC Token address
    ...
    function recoverERC20(address _coin) external onlyOwner {
        ...
        require(_coin != Token, "Can't recover Token tokens");
        ...
        returns (uint256)
    {
        ...
        if (amount > 0) {
            ...
            if (restake) {
                ...
                IERC20(Token).safeApprove(xNFTE, amount);
                IxNFTE(xNFTE).depositFor(addr, uint128(amount));
            } else {
    ...
    function _checkpointReward() internal {
        ...
        for (uint256 i = 0; i < 20; i++) {
            ...
            xNFTESupply[thisWeek] = IxNFTE(xNFTE).totalSupply(thisWeek);
    ...
    function _findUserTimestampEpoch(
        ...
        for (uint256 i = 0; i < 128; i++) {
            ...
            if (IxNFTE(xNFTE).getUserPointHistoryTS(addr, mid) <= ts) {
                min = mid;
            } else {
                max = mid - 1;
            }
        ...
        returns (uint256 weekCursor)
    {
        ...
        uint256 maxUserEpoch = IxNFTE(xNFTE).userPointEpoch(addr);
        ...
        uint256 userPointTs = IxNFTE(xNFTE).getUserPointHistoryTS(
            addr,
            userEpoch
        );
        ...
        )
    {
        ...
        for (uint256 i = 0; i < maxIterations; i++) {
            ...
            uint256 balance = IxNFTE(xNFTE).balanceOf(addr, weekCursor);
            if (balance > 0) {
