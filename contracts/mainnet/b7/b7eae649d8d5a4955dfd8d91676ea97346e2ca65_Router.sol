...
contract Router is IRouter, UpgradeableOperableKeepable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function initializeRouter(
        ...
        manager.approve(_compounder, type(uint256).max);
        ...
        if (chainId == blastID) {
            ...
            //BLAST_YIELD_CONTRACT.configureAutomaticYield();
        }
    }
        ...
        returns (uint256 amount0, uint256 amount1, uint256 position, uint256 shares)
    {
        ...
        if (_deposit.compound) {
            // compound
            // Deposit position in componuder
            ...
            if (_deposit.rewardOption == 0) {
                vars.manager.transfer(address(tracker), position);
                tracker.stake(msg.sender, position);
            } else if (_deposit.rewardOption == 1) {
                vars.manager.transfer(address(trackerZero), position);
                trackerZero.stake(msg.sender, position);
            } else {
                vars.manager.transfer(address(trackerOne), position);
    ...
    function withdraw(
        ...
        if (_compound) {
            require(_position >= 0 && compounder.balanceOf(msg.sender) >= _position, "Not Enough Amount");
            _shares = _position;
            _position = compounder.redeem(_shares, msg.sender);
        } else {
            if (_rewardOption == 0) {
                require(_position >= 0 && tracker.stakedAmount(msg.sender) >= _position, "Not Enough Amount");
                tracker.withdraw(msg.sender, _position);
            } else if (_rewardOption == 1) {
                require(_position >= 0 && trackerZero.stakedAmount(msg.sender) >= _position, "Not Enough Amount");
                trackerZero.withdraw(msg.sender, _position);
            } else {
                require(_position >= 0 && trackerOne.stakedAmount(msg.sender) >= _position, "Not Enough Amount");
    ...
    function compoundDeposit(uint256 _amount0, uint256 _amount1) external override onlyOperator whenNotPaused {
        ...
        if (_amount0 > 0 && _amount1 > 0) {
            //  mint liquidty in default range
            (amount0, amount1) = _manager.mintLiquidity(msg.sender, 0, _amount0, _amount1);
        } else {
            amount0 = _amount0;
            amount1 = _amount1;
        ...
        // Calculate position
        uint256 _position = previewPosition(price0, price1, amount0, amount1, position0, position1);

        // Stake position in Reward
        _manager.mint(address(tracker), _position);
        tracker.stake(address(compounder), _position);

        emit CompoundDeposit(msg.sender, amount0, amount1, _position);
    }
    ...
    function unCompound(uint256 _shares, uint8 _rewardOption) external nonReentrant whenNotPaused returns (uint256) {
        ...
        if (_rewardOption == 0) {
            // Stake position in Reward
            manager.transfer(address(tracker), position);
            tracker.stake(msg.sender, position);
        } else if (_rewardOption == 1) {
            // Stake position in Reward
            manager.transfer(address(trackerZero), position);
            trackerZero.stake(msg.sender, position);
        } else {
            // Stake position in Reward
            manager.transfer(address(trackerOne), position);
            trackerOne.stake(msg.sender, position);
        }
    ...
    function claimAllYield() external onlyKeeper {
        BLAST_YIELD_CONTRACT.claimAllYield(address(this), msg.sender);
    }
    ...
    function updateInternalContracts(
        ...
        manager.approve(address(compounder), 0);
    ...
    function _executeOrder(
        ...
        if (diff > deviation || _order.stage != 1) {
            return 1; // Do nothing stage created
        }
