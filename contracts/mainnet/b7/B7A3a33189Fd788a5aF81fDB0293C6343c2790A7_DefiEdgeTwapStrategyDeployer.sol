...
contract UniswapV3TwapLiquidityManager is TwapStrategyBase, ReentrancyGuard, IRamsesV2MintCallback {
    using SafeMath for uint256;
    ...
    function _transferPerformanceFees(uint256 _fee0, uint256 _fee1) internal {
        ...
        _updateReserves(reserve0.sub(managerToken0Amount).sub(protocolToken0Amount), reserve1.sub(managerToken1Amount).sub(protocolToken1Amount));
...
library TwapOracleLibrary {
    ...
    function getTick(address _pool, uint32 _period) internal view returns (int24 timeWeightedAverageTick) {
        ...
        (int56[] memory tickCumulatives, , ) = IRamsesV2Pool(_pool).observe(secondAgos);
    ...
    function consult(address _pool, uint32 _period) internal view returns (uint256 price) {
        ...
        uint256 token0Decimals = IERC20Minimal(IRamsesV2Pool(_pool).token0()).decimals();
        uint256 token1Decimals = IERC20Minimal(IRamsesV2Pool(_pool).token1()).decimals();
