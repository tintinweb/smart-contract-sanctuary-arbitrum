...
import { SiloPTStrategyLib } from '../../libraries/SiloPTStrategyLib.sol';
import { IPYieldTokenV2 } from '../../interfaces/IPendle.sol';
import { IERC20 } from '@openzeppelin/contracts/token/ERC20/IERC20.sol';

contract SimulatePendleSwap {
    event PendleSwapAmount(uint256 netAmountOut);
    event PendleRedeemAmount(uint256 amountTokenOut);

    address public pendleRouterV3 = 0x00000000005BBB0EF59571E58418F9a4357b68A0;

    function simulateSwapPendle(
        uint256 amount,
        address ptToken,
        address lrtToken,
        address pendleMarket
    ) public returns (uint256) {
        IERC20(ptToken).approve(pendleRouterV3, type(uint256).max);
        uint256 netAmountOut = SiloPTStrategyLib._swapPendle(false, amount, lrtToken, pendleMarket);
        IERC20(ptToken).approve(pendleRouterV3, 0);

        emit PendleSwapAmount(netAmountOut);
        return netAmountOut;
    }

    function simulateRedeemPendle(uint256 amount, address ptToken, address lrtToken) public returns (uint256) {
        require(isExpired(ptToken), 'PT Token Not Expired');

        address ytAddress = YT(ptToken);
        address syAddress = SY(ptToken);

        require(IERC20(ptToken).transfer(ytAddress, amount), 'Transfer to YT failed');
        uint256 netSyOut = IPYieldTokenV2(ytAddress).redeemPY(syAddress);
        uint256 amountTokenOut = IPYieldTokenV2(syAddress).redeem(msg.sender, netSyOut, lrtToken, 0, true);

        emit PendleRedeemAmount(amountTokenOut);
        return amountTokenOut;
    }

    function simulateSwapPendlePT(uint256 amount, address lrtToken, address pendleMarket) public returns (uint256) {
        IERC20(lrtToken).approve(pendleRouterV3, type(uint256).max);
        uint256 netAmountOut = SiloPTStrategyLib._swapPendle(true, amount, lrtToken, pendleMarket);
        IERC20(lrtToken).approve(pendleRouterV3, 0);

        emit PendleSwapAmount(netAmountOut);
        return netAmountOut;
    }

    function expiry(address ptToken) public view returns (uint256) {
        return IPYieldTokenV2(ptToken).expiry();
    }

    function isExpired(address ptToken) public view returns (bool) {
        return IPYieldTokenV2(ptToken).isExpired();
    }

    function SY(address ptToken) public view returns (address) {
        return IPYieldTokenV2(ptToken).SY();
    }

    function YT(address ptToken) public view returns (address) {
        return IPYieldTokenV2(ptToken).YT();
    }
}