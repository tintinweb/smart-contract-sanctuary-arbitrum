...
import {NonblockingLzApp} from "@layerzerolabs/lzApp/NonblockingLzApp.sol";
...
contract TokenFactory is NonblockingLzApp, ITokenFactory {
    ...
    uint16 internal immutable chainId;
    uint256 public bridgeFee = 100; // 0.1%;
    address public feeTo;
    address public tokenImplementation;

    constructor(address _endpoint) NonblockingLzApp(_endpoint) {
        chainId = lzEndpoint.getChainId();
        feeTo = msg.sender;
    ...
    function estimateDeployBridgeTokensFee(
        ...
        for (uint256 i = 0; i < destinationChains.length; i++) {
            (nativeFees[i], zroFees[i]) = lzEndpoint.estimateFees(destinationChains[i], address(this), payload, useZro, adapterParams);
    ...
    function deployBridgeTokens(
        ...
        _deployToken(payload, chainId);
    }

    function _deployToken(bytes memory payload, uint16 nativeChainId) internal returns(BridgedToken clone) {
        (
            address nativeToken,
            string memory name,
        ...
        bytes32 salt = keccak256(abi.encodePacked(nativeToken, name, symbol, decimals, nativeChainId));
        clone = BridgedToken(_cloneDeterministic(tokenImplementation, salt));
        clone.init(name, symbol, decimals, nativeToken, nativeChainId);
        _setTrustedRemotes(clone, chainIds, nativeChainId);
    }
...
import {OFT} from "./token/OFT.sol";
import {OFTCore} from "@layerzerolabs/token/oft/OFTCore.sol";
import {ITokenFactory} from "./interfaces/ITokenFactory.sol";
...
import {OFTCore} from "@layerzerolabs/token/oft/OFTCore.sol";
import {IOFT} from "@layerzerolabs/token/oft/IOFT.sol";

// customized OFT implementation - remove token specific constructor parameters.
contract OFT is OFTCore, ERC20 {
    ...
    function _debitFrom(address _from, uint16, bytes memory, uint _amount) internal virtual override returns(uint) {
        ...
        if (_from != spender) allowance[_from][spender] -= _amount;
        _burn(_from, _amount);
        return _amount;
    }
...
/// @author Solmate (https://github.com/Rari-Capital/solmate/blob/main/src/tokens/ERC20.sol)
/// @dev Removed constructor parameters.
abstract contract ERC20 {
    /*//////////////////////////////////////////////////////////////
                                 EVENTS
    //////////////////////////////////////////////////////////////*/
