...
import { UniswapV2SwapRouter } from "./UniswapV2SwapRouter.sol";
import { UniswapV3SwapRouter } from "./UniswapV3SwapRouter.sol";
import { ISwapRouter } from "../interfaces/ISwapRouter.sol";

contract SwapRouter is UniswapV2SwapRouter, UniswapV3SwapRouter, ISwapRouter {
  function execute(bytes memory data) public virtual returns (uint256 amountOut) {
    (bool success, bytes memory result) = address(this).delegatecall(data); // solhint-disable-line avoid-low-level-calls
...
import { SwapRouterLib } from "../libs/SwapRouterLib.sol";
import { IUniswapV2SwapRouter } from "../interfaces/IUniswapV2SwapRouter.sol";

abstract contract UniswapV2SwapRouter is IUniswapV2SwapRouter {
  using SafeERC20 for IERC20Metadata;

  function swapUniswapV2(address router, uint256 deadline, SwapRouterLib.SwapParams memory params) public virtual returns (uint256) {
    IERC20Metadata(params.tokenIn).safeTransferFrom(msg.sender, address(this), params.amountIn);
    IERC20Metadata(params.tokenIn).forceApprove(address(router), params.amountIn);

    address[] memory path = new address[](2);
    path[0] = params.tokenIn;
		path[1] = params.tokenOut;

    uint256 balance1 = IERC20Metadata(params.tokenOut).balanceOf(address(this));
    ...
    require(amountOut >= params.amountOutMinimum, "output amount failed");

    uint256 balance2 = IERC20Metadata(params.tokenOut).balanceOf(address(this));
    require(balance2 >= balance1 + amountOut, "output balance failed");

    IERC20Metadata(params.tokenOut).safeTransfer(msg.sender, amountOut);
...
abstract contract UniswapV3SwapRouter is IUniswapV3SwapRouter {
  
  using SafeERC20 for IERC20Metadata;

  function swapUniswapV3(address router, bytes memory path, SwapRouterLib.SwapParams memory params) external returns (uint256 amountOut) {
    IERC20Metadata(params.tokenIn).safeTransferFrom(msg.sender, address(this), params.amountIn);

    IERC20Metadata(params.tokenIn).forceApprove(router, params.amountIn);

    IV3SwapRouter.ExactInputParams memory exactInputParams = IV3SwapRouter.ExactInputParams({
      path: path,
    ...
    amountOut = IV3SwapRouter(router).exactInput(exactInputParams);
    require(amountOut >= params.amountOutMinimum, "output amount failed");

    uint256 balance2 = IERC20Metadata(params.tokenOut).balanceOf(address(this));
    require(balance2 >= balance1 + amountOut, "output balance failed");

    IERC20Metadata(params.tokenIn).forceApprove(router, 0);
...
import { IUniswapV2SwapRouter } from './IUniswapV2SwapRouter.sol';
import { IUniswapV3SwapRouter } from './IUniswapV3SwapRouter.sol';

interface ISwapRouter is IUniswapV2SwapRouter, IUniswapV3SwapRouter { 
...
interface IUniswapV2SwapRouter {
  function swapUniswapV2(address router, uint256 deadline, SwapRouterLib.SwapParams memory params) external returns (uint256);
}
...
import { SwapRouterLib } from "@ambitlabs/swaprouter-contracts/libs/SwapRouterLib.sol";

import { AddressRegistryExtensions } from "../../libraries/AddressRegistryExtensions.sol";
import { IAddressRegistry } from "../../interfaces/core/IAddressRegistry.sol";
import { IDepositorVault } from "../../interfaces/vault/IDepositorVault.sol";

contract AUSDSwapRouter {

  using AddressRegistryExtensions for IAddressRegistry;
  using SafeERC20 for IERC20Metadata;

  function mintAUSD(IAddressRegistry registry, SwapRouterLib.SwapParams memory params) external returns (uint256) {
    IDepositorVault vault = registry.getDepositorVault();

    IERC20Metadata underlyingAsset = vault.getUnderlyingAsset();
    IERC20Metadata vaultToken = vault.getToken();

    require(params.tokenIn == address(underlyingAsset), "tokenIn mistmatch");
    require(params.tokenOut == address(vaultToken), "tokenOut mistmatch");

    underlyingAsset.safeTransferFrom(msg.sender, address(this), params.amountIn);
    underlyingAsset.forceApprove(address(vault), params.amountIn);

    uint256 shares = vault.deposit(params.amountIn, params.amountOutMinimum);
  ...
  function redeemAUSD(IAddressRegistry registry, SwapRouterLib.SwapParams memory params) external returns (uint256 amountOut) {
    IDepositorVault vault = registry.getDepositorVault();

    IERC20Metadata underlyingAsset = vault.getUnderlyingAsset();
    IERC20Metadata vaultToken = vault.getToken();

    require(params.tokenIn == address(vaultToken), "tokenIn mistmatch");
    require(params.tokenOut == address(underlyingAsset), "tokenOut mistmatch");

    vaultToken.safeTransferFrom(msg.sender, address(this), params.amountIn);
    vaultToken.forceApprove(address(vault), params.amountIn);

    uint256 redeemed = vault.redeem(params.amountIn, params.amountOutMinimum);

    underlyingAsset.safeTransfer(msg.sender, redeemed);
...
import { SwapRouter } from "@ambitlabs/swaprouter-contracts/contracts/SwapRouter.sol";
import { IMarketplaceAdapter } from "../../interfaces/marketplace/IMarketplaceAdapter.sol";
import { AUSDSwapRouter } from "./AUSDSwapRouter.sol";

contract SwapRouterAdapter is SwapRouter, AUSDSwapRouter, IMarketplaceAdapter {
  function execute(bytes memory data) public override(IMarketplaceAdapter, SwapRouter) returns (uint256 amountOut) {
