...
def setStaker(
    ...
    assert msg.sender == AUTHORIZER_ADAPTOR, "SENDER_NOT_ALLOWED"
    self.staker = _staker
...
def deposit(_value: uint256, _user: address = msg.sender):
    ...
    """
    assert msg.sender == self.staker, "DEPOSIT_NOT_ALLOWED"
...
def withdraw(_value: uint256, _user: address = msg.sender):
    ...
    """
    assert msg.sender == self.staker, "WITHDRAW_NOT_ALLOWED"
