    ...
    RecoverERC20
{
    ...
    uint256 private ACC_REWARD_PRECISION_UPDATED;
    ...
    function setPrecision(uint256 _precision) external onlyOwner {
        ACC_REWARD_PRECISION_UPDATED = _precision;
    }
     ...
     * @dev Its decimals count is ACC_REWARD_PRECISION_UPDATED
    ...
    function claim(address _user, address[] memory _tokens) public nonReentrant whenNotPaused {
        ...
        for (uint256 i; i < length; ) {
            ...
            uint256 rewardDebt = (user.amount * pool.accRewardPerShare) / ACC_REWARD_PRECISION_UPDATED;
    ...
    function pendingRewards(address _user, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i; i < length; ) {
            ...
            claimable[i] = (user.amount * accRewardPerShare) / ACC_REWARD_PRECISION_UPDATED - user.rewardDebt;
            unchecked {
                i++;
            }
        }
    ...
    function _handleActionAfterForToken(address _token, address _user, uint256 _balance) internal {
        ...
        uint256 accRewardPerShare = pool.accRewardPerShare;
        if (amount != 0) {
            uint256 pending = (amount * accRewardPerShare) / ACC_REWARD_PRECISION_UPDATED - user.rewardDebt;
            if (pending != 0) {
        ...
        user.rewardDebt = (_balance * accRewardPerShare) / ACC_REWARD_PRECISION_UPDATED;
    ...
    function _newRewards(
        ...
        if (lpSupply > 0) {
            ...
            newAccRewardPerShare = (newReward * ACC_REWARD_PRECISION_UPDATED) / lpSupply;
        }
    }
    ...
    //allow owner to withdraw the whole balance of any ERC20 
