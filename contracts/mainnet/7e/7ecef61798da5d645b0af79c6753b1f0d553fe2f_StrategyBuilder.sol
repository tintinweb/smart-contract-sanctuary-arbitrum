...
contract StrategyBuilder is StrategyBlocks {
    ...
    function process(StepCall[] memory steps) external returns (PositionId long, PositionId short) {
        ...
        returns (uint256 offset_, StackLib.Stack memory stack_)
    {
        ...
        for (offset_ = offset; offset_ < steps.length; offset_++) {
            ...
            if (step.step == Step.VaultDeposit) {
                ...
                (address token, EIP2098Permit memory permit, uint256 amount) = abi.decode(step.data, (address, EIP2098Permit, uint256));
                _pullFundsWithPermit(token, permit, amount, user);
            } else if (step.step == Step.PullFundsWithPermit2) {
                (IERC20 token, EIP2098Permit memory permit, uint256 amount) = abi.decode(step.data, (IERC20, EIP2098Permit, uint256));
                _pullFundsWithPermit2(token, permit, amount, user);
                ...
                stack_.storePositionId(_positionClose(stack_.loadPositionId(positionId, side)), side);
                ...
                _swap(swapData, tokenToSell, tokenToBuy, to);
...
abstract contract StrategyBlocks is IERC721Receiver {
    ...
    function _vaultWithdraw(IERC20 asset, uint256 amount, address to) internal {
        if (amount == ALL) amount = vault.balanceOf(asset, address(this));
        vault.withdraw(asset, address(this), amount, to);
    }
    ...
    function _positionClose(PositionId positionId) internal returns (PositionId positionId_) {
    ...
    function _swap(SwapData memory swapData, IERC20 tokenToSell, IERC20 tokenToBuy, address to) internal {
        ...
        vault.depositTo(tokenToBuy, address(this), output);
    }

    function _returnPositions(PositionId long, PositionId short, address owner) internal {
        if (positionNFT.exists(long)) positionNFT.safeTransferFrom(address(this), owner, long.asUint());
        if (positionNFT.exists(short)) positionNFT.safeTransferFrom(address(this), owner, short.asUint());
    }
    ...
    function _pullFundsWithPermit(address token, EIP2098Permit memory permit, uint256 amount, address owner) internal {
        ...
        IERC20(token).transferOut(owner, address(vault), amount);
    }

    function _pullFundsWithPermit2(IERC20 token, EIP2098Permit memory permit, uint256 amount, address owner) public {
        erc20Permit2.permitTransferFrom({
            ...
            transferDetails: IPermit2.SignatureTransferDetails({ to: address(vault), requestedAmount: amount }),
            owner: owner,
            signature: abi.encodePacked(permit.r, permit.vs)
        });
    }
