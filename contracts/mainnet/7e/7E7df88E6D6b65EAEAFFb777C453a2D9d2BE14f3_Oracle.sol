...
struct TokenConfig {
    ...
    uint256 uniswapPriceUnits;
    AggregatorV3Interface uniswapPriceFeed;
    bool isStableCoin;
}
...
contract Oracle is Ownable, IOracle {
    ...
    uint256 public constant PRICE_FEED_NORMAL_SPREAD = 2e3; // 0.2%
    ...
    // ============ View functions ============
    function getMultiplePrices(address[] calldata tokens, bool max) external override view returns (uint256[] memory) {
    ...
    function getMultipleChainlinkPrices(address[] calldata tokens) external override view returns (uint256[] memory) {
        uint256 len = tokens.length;
        uint256[] memory result = new uint[](len);

        for (uint256 i = 0; i < len;) {
            TokenConfig memory config = tokenConfig[tokens[i]];
            (, int256 guardPrice,,,) = config.chainlinkPriceFeed.latestRoundData();
			result[i] = uint256(guardPrice);
            unchecked {
                ++i;
            }
        }
    ...
    function getPrice(address token, bool max) external override view returns (uint256) {
    ...
    */
    function configToken(
        ...
        uint256 priceDecimals = AggregatorV3Interface(priceFeed).decimals();
        uint256 uniswapPriceDecimals = 0;
        if (uniswapPriceFeed != address(0)) {
            uniswapPriceDecimals = AggregatorV3Interface(uniswapPriceFeed).decimals();
        }
        ...
        tokenConfig[token] = TokenConfig({
            ...
            uniswapPriceUnits: 10 ** uniswapPriceDecimals,
            uniswapPriceFeed: AggregatorV3Interface(uniswapPriceFeed),
    ...
    function __getPrice(address token, bool max) internal view returns (uint256) {
    ...
    function _getPrice(address token, bool max) internal view returns (uint256) {
        return _getPriceSpread(__getPrice(token, max), PRICE_FEED_NORMAL_SPREAD, max);
    }
    ...
    function _getLastPrice(address token) internal view returns (uint256 price, uint256 timestamp) {
        if (lastAnswerTimestamp[token] + PRICE_FEED_INACTIVE < block.timestamp) {
            TokenConfig memory config = tokenConfig[token];
            if (address(config.uniswapPriceFeed) != address(0)) {
                int256 guardPrice;
                (, guardPrice,, timestamp,) = config.uniswapPriceFeed.latestRoundData();
                price = (uint256(guardPrice) * VALUE_PRECISION) / config.baseUnits / config.uniswapPriceUnits;
