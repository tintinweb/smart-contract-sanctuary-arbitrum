...
contract Staking is ReentrancyGuard {
	...
	uint256 public dailyInterestRate = uint256(120000);
	uint256 public minStakePeriod = (uint256(3000) * uint256(864));
	uint256 public totalWithdrawals = uint256(0);
	uint256 public unstakeClaimCoinPrice0 = uint256(1000000000000000000000);
 ...
 * checks that current time is less than or equals to 1718028000
...
*/
	function stake(uint256 _tokenId) public {
		require((block.timestamp <= uint256(1718028000)), "Date has passed");
 ...
 * creates an internal variable interestToRemove with initial value (thisRecord with element accumulatedInterestToUpdateTime) + ((((minimum of current time, 1718028000) - (thisRecord with element lastUpdateTime)) * (dailyInterestRate) * (1000000000000000000)) / (864000000))
...
*/
	function unstake(uint256 _tokenId) public nonReentrant {
		...
		uint256 interestToRemove = (thisRecord.accumulatedInterestToUpdateTime + (((minUIntPair(block.timestamp, uint256(1718028000)) - thisRecord.lastUpdateTime) * dailyInterestRate * uint256(1000000000000000000)) / uint256(864000000)));
	...
	function updateRecordsWithLatestInterestRates() internal {
		for (uint i0 = 0; i0 < (stakedAddresses).length; i0++){
			...
			for (uint i1 = 0; i1 < (nfts).length; i1++){
				...
				informationAboutStakeScheme[nfts[i1]]  = record (thisRecord.staker, thisRecord.stakeTime, minUIntPair(block.timestamp, uint256(1718028000)), (thisRecord.accumulatedInterestToUpdateTime + (((minUIntPair(block.timestamp, uint256(1718028000)) - thisRecord.lastUpdateTime) * dailyInterestRate * uint256(1000000000000000000)) / uint256(864000000))), thisRecord.amtWithdrawn);
			}
		}
	}
 ...
 * The function takes in 1 variable, (zero or a positive integer) _tokenId. It can be called by functions both inside and outside of this contract. It does the following :
 * creates an internal variable thisRecord with initial value informationAboutStakeScheme with element _tokenId
 * returns (thisRecord with element accumulatedInterestToUpdateTime) + ((((minimum of current time, 1718028000) - (thisRecord with element lastUpdateTime)) * (dailyInterestRate) * (1000000000000000000)) / (864000000)) as output
*/
	function interestEarnedUpToNowBeforeTaxesAndNotYetWithdrawn(uint256 _tokenId) public view returns (uint256) {
		record memory thisRecord = informationAboutStakeScheme[_tokenId];
		return (thisRecord.accumulatedInterestToUpdateTime + (((minUIntPair(block.timestamp, uint256(1718028000)) - thisRecord.lastUpdateTime) * dailyInterestRate * uint256(1000000000000000000)) / uint256(864000000)));
	}
 ...
 * creates an internal variable thisRecord with initial value informationAboutStakeScheme with element _tokenId
 * checks that (thisRecord with element staker) is equals to (the address that called this function)
 * updates informationAboutStakeScheme (Element _tokenId) as Struct comprising (thisRecord with element staker), (thisRecord with element stakeTime), (minimum of current time, 1718028000), ((totalInterestEarnedTillNow) - (_withdrawalAmt)), ((thisRecord with element amtWithdrawn) + (_withdrawalAmt))
...
*/
	function withdrawInterestWithoutUnstaking(uint256 _withdrawalAmt, uint256 _tokenId) public nonReentrant {
		...
		record memory thisRecord = informationAboutStakeScheme[_tokenId];
		require((thisRecord.staker == msg.sender), "You do not own this token");
		informationAboutStakeScheme[_tokenId]  = record (thisRecord.staker, thisRecord.stakeTime, minUIntPair(block.timestamp, uint256(1718028000)), (totalInterestEarnedTillNow - _withdrawalAmt), (thisRecord.amtWithdrawn + _withdrawalAmt));
