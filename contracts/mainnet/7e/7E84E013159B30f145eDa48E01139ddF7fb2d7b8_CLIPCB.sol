...
/**
 * @title CLIPCB
 * @dev The CLIPCB is non-transferable token.
 * It is possible to gradually convert from since to until period to CLIP.
 */
contract CLIPCB is ERC20 {
    /**********
     * Struct *
     **********/
    ...
    constructor(address clipAddress) ERC20("CLIP convertible bond", "CLIPCB") {
        clipToken = ERC20(clipAddress);
    }
    ...
    /**
     * Mint the CLIPCB by depositing the CLIP.
     * @param to Destination address for the CLIPCB.
     * @param since Unixtime to start converting the CLIPCB to the CLIP.
     * @param until Unixtime when all the CLIPCB can be converted to the CLIP
     * @param clipAmount Amount of CLIP tokens to deposit.
     */
    function mint(
        address to,
        uint64 since,
        ...
        if (clipAmount == 0) revert NoAmount();

        // Perform the transfer of CLIP tokens
        require(
            clipToken.transferFrom(msg.sender, address(this), clipAmount),
            "CLIP transfer failed"
        );

        _mint(to, clipAmount);
        claimInfo[to] = ClaimInfo(clipAmount, 0, since, until, msg.sender);
        originalClaimer[to] = to;

        emit Mint(to, clipAmount, since, until);
    }
    ...
    /**
     * Convert the CLIPCB to the CLIP.
    ...
    function claim(uint256 amount) external {
        ...
        uint256 currentClaimableCLIP = getClaimableTokens(
            originalClaimer[msg.sender]
        ) - originalClaimInfo.claimed;
        if (amount > currentClaimableCLIP) revert OverAmount();

        originalClaimInfo.claimed += amount;

        _burn(msg.sender, amount);
        require(clipToken.transfer(msg.sender, amount), "CLIP transfer failed");
    ...
    function transfer(
        ...
        require(
            ...
            "CLIPCB: bulk transfer args must be equals"
        );
        address owner = _msgSender();
    ...
    function transferFrom(
        ...
        require(
            ...
            "CLIPCB: bulk transferFrom args must be equals"
        );
    ...
    /**
     * Get current amount of the CLIPCB available for conversion.
     * @param original Holder of the CLIPCB token.
     */
    function getClaimableTokens(
        ...
        if (amount > originalClaimInfo.amount) {
            amount = originalClaimInfo.amount;
        }
        // Deduct already claimed tokens from the total claimable tokens
        return amount - originalClaimInfo.claimed;
    }
    ...
    /**
     * The CLIPCB is transferable to allowed addresses.
     */
