...
contract GnomeBooperTestV1 is IUniswapV3FlashCallback, PeripheryImmutableState, PeripheryPayments, Ownable {
    ...
    function uniswapV3FlashCallback(uint256 fee0, uint256 fee1, bytes calldata data) external override {
        ...
        uint256 restAmount = IERC20(boopToken).balanceOf(address(this));
        if (restAmount > 0) pay(boopToken, address(this), callback.payer, restAmount);
    }
    ...
    function getFeeFromBooper(address token, uint256 amount, address fren) internal {
        ...
        if (token == WETH9) {
            ...
            IGNOME(GNOME).approve(address(swapRouter), amountGnome);
            ...
            uint256 amounteth = swapRouter.exactInputSingle(sellparams);
