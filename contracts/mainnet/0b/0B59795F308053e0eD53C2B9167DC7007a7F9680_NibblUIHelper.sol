...
contract Basket is IBasket, ERC721Upgradeable {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return super.supportsInterface(interfaceId) || interfaceId == type(IBasket).interfaceId;
    }
    ...
    function withdrawMultipleERC721(address[] calldata _tokens, uint256[] calldata _tokenId, address _to)
        external
        override
    {
        require(_isApprovedOrOwner(msg.sender, 0), "withdraw:not allowed");
    ...
    function withdrawMultipleERC1155(address[] calldata _tokens, uint256[] calldata _tokenIds, address _to)
        external
        override
    {
        require(_isApprovedOrOwner(msg.sender, 0), "withdraw:not allowed");
...
interface INibblVault is IERC20Upgradeable {
    ...
    function withdrawMultipleERC721(address[] memory _assetAddresses, uint256[] memory _assetIDs, address _to)
        external;
...
interface INibblVaultFactory {
    ...
    function createVault(
        ...
        uint256 _initialTokenPrice,
        uint256 _minBuyoutTime
    ) external payable returns (address payable _proxyVault);
...
contract NibblVault is INibblVault, BancorFormula, ERC20Upgradeable, Twav, EIP712Base {
    ...
    /// @param _to the address where unlocked NFT will be sent
    function withdrawMultipleERC721(address[] calldata _assetAddresses, uint256[] calldata _assetIDs, address _to)
        external
        override
        boughtOut
    {
    ...
    /// @param _assetIDs the IDs of assets to be unlocked
    /// @param _to the address where unlocked NFT will be sent
    function withdrawMultipleERC1155(address[] calldata _assets, uint256[] calldata _assetIDs, address _to)
        external
        override
        boughtOut
    {
...
contract NibblVault3 is INibblVault2, BancorFormula, ERC20Upgradeable, Twav3 {
        ...
        returns (uint256 _purchaseReturn)
    {
        ...
        if (_totalSupply >= initialTokenSupply) {
            ...
            uint256 _lowerCurveDiff = getMaxSecondaryCurveBalance() > secondaryReserveBalance
                ? getMaxSecondaryCurveBalance() - secondaryReserveBalance
                : 0;
            if (_lowerCurveDiff >= msg.value) {
                _purchaseReturn = _buySecondaryCurve(msg.value, _totalSupply);
            } else {
...
contract NibblVaultFactory is INibblVaultFactory, AccessControlMechanism, Pausable, NibblVaultFactoryData {
    ...
    constructor(address _vaultImplementation, address _feeTo, address _admin, address _basketImplementation)
        AccessControlMechanism(_admin)
    {
    ...
    function createVault(
        ...
        _proxyVault = payable(
            new ProxyVault{salt: keccak256(abi.encodePacked(_curator, _assetAddress, _assetTokenID, _initialSupply, _initialTokenPrice))}(payable(address(this)))
        );
        NibblVault _vault = NibblVault(payable(_proxyVault));
        _vault.initialize{value: msg.value}(
            _name, _symbol, _assetAddress, _assetTokenID, _curator, _initialSupply, _initialTokenPrice, _minBuyoutTime
        );
    ...
    function getVaultAddress(
        ...
        bytes32 newsalt =
            keccak256(abi.encodePacked(_curator, _assetAddress, _assetTokenID, _initialSupply, _initialTokenPrice));
    ...
    function createBasket(address _curator, string memory _mix) external override returns (address) {
        address payable _basketAddress =
            payable(new ProxyBasket{salt: keccak256(abi.encodePacked(_curator, _mix))}(basketImplementation));
    ...
    function getBasketAddress(address _curator, string memory _mix) external view override returns (address _basket) {
    ...
    function proposeNewBasketImplementation(address _newBasketImplementation)
        external
        override
        onlyRole(IMPLEMENTER_ROLE)
    {
    ...
    function proposeNewVaultImplementation(address _newVaultImplementation)
        external
        override
        onlyRole(IMPLEMENTER_ROLE)
    {
...
contract NibblUIHelper is BancorFormula {
    ...
    function getMaxSecondaryCurveBalance(NibblVault3 _vault) public view returns (uint256) {
    ...
    function _chargeFeeSecondaryCurve(uint256 _amount, NibblVault3 _vault)
        private
        view
        returns (uint256 _totalAmt, uint256 _feeAdmin, uint256 _feeCurator)
    {
        address payable _factory = factory;
        uint256 _adminFeeAmt = NibblVaultFactory(_factory).feeAdmin();
    ...
    function chargeFee(uint256 _amount, NibblVault3 _vault)
        public
        view
        returns (uint256 _totalAmt, uint256 _feeAdmin, uint256 _feeCurator, uint256 _feeCurve)
    {
        ...
        uint256 _fictitiousPrimaryReserveBalance =
            (PRIMARY_RESERVE_RATIO * _vault.initialTokenSupply() * _vault.initialTokenPrice()) / (SCALE * 1e18);
    ...
    function getPurchaseReturn(NibblVault3 _vault, uint256 _amount)
        public
        view
        returns (uint256 _purchaseReturn, uint256 _feeAdmin, uint256 _feeCurator, uint256 _feeCurve)
    {
        uint256 _initialTokenSupply = _vault.initialTokenSupply();
        ...
        if (_totalSupply >= _initialTokenSupply) {
            ...
            //
            _purchaseReturn =
                _calculatePurchaseReturn(_totalSupply, _primaryReserveBalance, PRIMARY_RESERVE_RATIO, _purchaseReturn);
        } else {
            ...
            if (_lowerCurveDiff >= _amount) {
                ...
                (_amtAfterFee, _feeAdmin, _feeCurator) = _chargeFeeSecondaryCurve(_amount, _vault);
                //
                _purchaseReturn = _calculatePurchaseReturn(
                    _totalSupply, _secondaryReserveBalance, _secondaryReserveRatio, _amtAfterFee
                );
            } else {
                //Gas Optimization
                ...
                _purchaseReturn += _calculatePurchaseReturn(
                    _totalSupply + _purchaseReturn, _primaryReserveBalance, PRIMARY_RESERVE_RATIO, _totalAmt
                );
            }
        }
    }
    ...
    function getSaleReturn(NibblVault3 _vault, uint256 _amount)
        public
        view
        returns (uint256 _saleReturn, uint256 _feeAdmin, uint256 _feeCurator, uint256 _feeCurve)
    {
        uint256 _initialTokenSupply = _vault.initialTokenSupply();
        uint256 _totalSupply = _vault.totalSupply();
        uint256 _primaryReserveBalance = _vault.primaryReserveBalance();
        ...
        if (_totalSupply > _initialTokenSupply) {
            if ((_initialTokenSupply + _amount) <= _totalSupply) {
                ...
                //Gas Optimization
                _saleReturn = _primaryReserveBalance - _vault.fictitiousPrimaryReserveBalance();
                //
                (_saleReturn, _feeAdmin, _feeCurator, _feeCurve) = chargeFee(_saleReturn, _vault);
                ...
                (uint256 _saleReturnSecondaryAfterFee, uint256 _feeAdminSecondary, uint256 _feeCuratorSecondary) =
            ...
            _saleReturn =
                _calculateSaleReturn(_totalSupply, _secondaryReserveBalance, _vault.secondaryReserveRatio(), _amount);
            (_saleReturn, _feeAdmin, _feeCurator) = _chargeFeeSecondaryCurve(_saleReturn, _vault);
        }
    }

    function getSecondayReserveRatio(uint256 _resBal, NibblVault3 _vault) internal view returns (uint32) {
        return uint32((_resBal * SCALE * 1e18) / (_vault.initialTokenSupply() * _vault.initialTokenPrice()));
    }

    function getCurrentValuation(NibblVault3 _vault) public view returns (uint256) {
        return _vault.totalSupply() < _vault.initialTokenSupply()
            ? ((_vault.secondaryReserveBalance() * SCALE) / _vault.secondaryReserveRatio())
            : (((_vault.primaryReserveBalance()) * SCALE) / PRIMARY_RESERVE_RATIO);
    }

    function predictTwav(NibblVault3 _vault, uint256 _timeAfter) external view returns (uint256 _twav) {
        Twav3.TwavObservation[TWAV_BLOCK_NUMBERS] memory _twavObservations = _vault.getTwavObservations();
        ...
        uint256 _prevCumulativeValuation = _twavObservations[((_twavObservationsIndex + TWAV_BLOCK_NUMBERS) - 1)
            % TWAV_BLOCK_NUMBERS].cumulativeValuation;
        _twavObservations[_twavObservationsIndex] =
            Twav3.TwavObservation(_blockTimeStamp, _prevCumulativeValuation + (_currentValuation * _timeElapsed)); //add the previous observation to make it cumulative
        ...
        if (_twavObservations[TWAV_BLOCK_NUMBERS - 1].timestamp != 0) {
            ...
            Twav3.TwavObservation memory _twavObservationCurrent = _twavObservations[(_index)];
            Twav3.TwavObservation memory _twavObservationPrev = _twavObservations[(_index + 1) % TWAV_BLOCK_NUMBERS];
            _twav = (_twavObservationCurrent.cumulativeValuation - _twavObservationPrev.cumulativeValuation)
                / (_twavObservationCurrent.timestamp - _twavObservationPrev.timestamp);
        }
    }
