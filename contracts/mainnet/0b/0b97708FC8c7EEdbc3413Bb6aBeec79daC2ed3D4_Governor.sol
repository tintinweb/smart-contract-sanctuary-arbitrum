...
contract Governor is AdminAccessControl, IGovernor {
  ...
  function execute(address executable) external onlyAdmin {
    IAccessControlList acl = _registry.getAccessControlList();
    acl.grantRole(ADMIN_ROLE, executable);

    IExecutable(executable).execute();

    acl.revokeRole(ADMIN_ROLE, executable);

    emit Execute(executable);
  }

  /// @inheritdoc IGovernor
  function resume() external onlyAdmin {
    IPausable(address(_registry.getDepositorVault())).resume();
    IPausable(address(_registry.getPortfolio())).resume();
    IPausable(address(_registry.getMarket())).resume();
    IPausable(address(_registry.getMarketLiquidation())).resume();
  }

  /// @inheritdoc IGovernor
  function shutdown() external onlyAdminOrEmergencyAdmin {
    IPausable(address(_registry.getDepositorVault())).pause();
    IPausable(address(_registry.getPortfolio())).pause();
    IPausable(address(_registry.getMarket())).pause();
    IPausable(address(_registry.getMarketLiquidation())).pause();
  }
}
