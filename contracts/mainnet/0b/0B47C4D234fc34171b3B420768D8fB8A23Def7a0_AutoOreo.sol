...
/**
 * @title AutoOreo Staking Contract
...
contract AutoOreo is Ownable, ReentrancyGuard {
  ...
  function add(IVault _vault) public onlyOperator {
    ...
    require(_vault.autoOreo() == address(this), "add: invalid contract");
   ...
   */
  function deposit(uint256 _pid, uint256 _depositAmount, bool _flag) external nonReentrant {
   ...
   */
  function withdraw(uint256 _pid, uint256 _withdrawAmount, bool _flag) external {
...
interface IVault is IERC20 {
  ...
  function autoOreo() external view returns (address);
}