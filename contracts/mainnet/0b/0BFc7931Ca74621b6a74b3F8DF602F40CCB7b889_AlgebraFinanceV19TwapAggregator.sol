...
/// @notice Copied from https://github.dev/cryptoalgebra/AlgebraV1.9/tree/main/src and modified for solc ^0.8.0
library AlgebraFinanceDataStorageLibrary {
    ...
    function consult(
        ...
        int56 peroidInt56 = int56(int32(period));
        timeWeightedAverageTick = int24(tickCumulativesDelta / peroidInt56);

        // Always round to negative infinity
        if (
            tickCumulativesDelta < 0 &&
            (tickCumulativesDelta % peroidInt56 != 0)
...
import { IAlgebraPool, AlgebraFinanceDataStorageLibrary } from './AlgebraFinanceDataStorageLibrary.sol';

import { Registry, RegistryStorage } from '../registry/Registry.sol';
...
/**
 * @title   AlgebraFinanceV19TwapAggregator
 * @notice  Gets the unit price of an asset for a Algebra V1.9 pool (Camelot)
 * @dev  The pair token on the pool must be supported by valio and have a configured chainlink oracle
 */

contract AlgebraFinanceV19TwapAggregator is IValioCustomAggregator {
    ...
    constructor(address _VALIO_REGISTRY, uint32 _SECONDS_AGO) {
        VALIO_REGISTRY = Registry(_VALIO_REGISTRY);
        SECONDS_AGO = _SECONDS_AGO;
    }

    /**
     * @notice  Get the latest unit price from the twap for the given asset
     * @dev     The price 10**8
     * @param   mainToken  The erc20 asset
     * @return  answer  The unit price
     * @return  updatedAt  The last time the pair tokens chainlink oracle was updated
     */
    ...
    /**
     * @notice  The decimals/precision of the returned value
     * @dev     We only use USD chainlink oracles which return 8 decimals so this returns the same precision
     * @return  uint8  the precision/decimals of the unit price returned by this contract
     */
    ...
    /**
     * @notice  Returns the name of this aggregator contract
     * @return  string  the name
     */
    function description() external pure override returns (string memory) {
        return 'AlgebraFinanceV19TwapAggregator';
    }
    ...
    function _latestRoundData(
        ...
        RegistryStorage.PoolConfig memory v3PoolConfig = VALIO_REGISTRY
            .poolConfig(mainToken);
        ...
        int24 tick = AlgebraFinanceDataStorageLibrary.consult(
            v3PoolConfig.pool,
            SECONDS_AGO
        );

        uint256 quoteAmount = AlgebraFinanceDataStorageLibrary.getQuoteAtTick(
            tick,
