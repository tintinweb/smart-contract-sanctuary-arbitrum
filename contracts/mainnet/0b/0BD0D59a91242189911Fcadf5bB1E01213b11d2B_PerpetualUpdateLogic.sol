...
contract PerpetualUpdateLogic is PerpetualUpdateFunctions, IPerpetualUpdateLogic, IFunctionList {
    ...
    function updateFundingAndPricesBefore(
        uint24 _iPerpetualId,
        bool _revertIfClosed
    ) external override {
        require(msg.sender == address(this), "onlythis");

        PerpetualData storage perpetual1 = _getPerpetual(_iPerpetualId);
        _checkOracleStatus(perpetual1, _revertIfClosed);
        _accumulateFundingInPerp(perpetual1);
    }

    function updateFundingAndPricesAfter(uint24 _iPerpetualId) external override {
        require(msg.sender == address(this), "onlythis");
     ...
     * @param _index Index to emit with event. A value of zero removes the current delegate.
     */
    function setDelegate(address _delegate, uint256 _index) external override {
        if (_index > 0) {
            require((_delegate != address(0) && _delegate != msg.sender), "invalid delegate");
            delegates[msg.sender] = _delegate;
        } else {
            require(delegates[msg.sender] != address(0), "no delegate");
            delete delegates[msg.sender];
        }
        emit SetDelegate(msg.sender, delegates[msg.sender], _index);
    }
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[12] = this.setDelegate.selector;
