...
/**
 * @notice
 *  a vault contract that trades on GMX market and Lifi DEX Aggregator
...
contract PerpetualVault is Initializable, Ownable2StepUpgradeable, ReentrancyGuardUpgradeable {
  ...
  event LifiSwap(address inputToken, uint256 inputAmount, address outputToken, uint256 outputAmount);
  ...
  function afterOrderExecution(
    ...
    if (
      ...
      nextAction.selector = NextActionSelector.WITHDRAW_ACTION;
      // IGmxUtils.PositionData memory positionData = gmxUtils.getPositionInfo(positionKey, marketPrices);
  ...
  function setMaxWithdrawalLength(uint256 length) external onlyOwner {
    require(length > 0, "zero value");
    maxWithdrawalLength = length;
  }
   ...
   *  it could contain only lifi swap data, gmx swap data or both of them
   *  but in the case of containing both swap data, first element of array should be always lifi.
   * 
  ...
  function _doLifiSwap(bytes memory data, bool isCollateralToIndex) internal returns (uint256 outputAmount) {
    ...
    emit LifiSwap(address(inputToken), amount, address(outputToken), outputAmount);
  }

  // check callee contract address and recepient address of swap
