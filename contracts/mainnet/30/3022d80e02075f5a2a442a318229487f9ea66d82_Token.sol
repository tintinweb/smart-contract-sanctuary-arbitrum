...
contract Token is Ownable, ERC20{
    TokenVesting public fundToken;
    TokenVesting public teamLockToken;
    bool doOnce;
    uint256 public constant MAX_SUPPLY = 70 * 1e6 * 1e18;

    constructor(string memory name_, string memory symbol_) Ownable() ERC20(name_,symbol_){  

        fundToken = new TokenVesting(msg.sender, block.timestamp, 0 days, 0 days);
        teamLockToken = new TokenVesting(msg.sender, block.timestamp + 365 days, 0 days, 1460 days);
        __mint(msg.sender, 7 * 1e6 * 1e18);
        __mint(address(fundToken), 3.5 * 1e6 * 1e18);
        __mint(address(teamLockToken), 10.5 * 1e6 * 1e18);
