...
/**
 * @title OpenOceanAdapter
 * @author StakeEase
 * @notice OpenOcean Adapter contract
 */
contract OpenOceanAdapter is RouterIntentEoaAdapterWithoutDataProvider {
    using SafeERC20 for IERC20;
    ...
    function execute(
        ...
        if (address(this) == self())
            if (inputAsset != NATIVE_ADDRESS)
                IERC20(inputAsset).safeTransferFrom(msg.sender, self(), amount);
        ...
        if (inputAsset != NATIVE_ADDRESS)
            IERC20(inputAsset).safeIncreaseAllowance(openOceanExchange, amount);
        else value = amount;

        uint256 balBefore = getBalance(outputAsset);

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, ) = openOceanExchange.call{value: value}(swapData);

        require(success, Errors.SWAP_FAILED);
        uint256 amountReceived = getBalance(outputAsset) - balBefore;
        require(amountReceived != 0, Errors.AMOUNT_RECEIVED_CANNOT_BE_ZERO);

        address[] memory tokens = new address[](1);
        tokens[0] = outputAsset;
    ...
    function parseInput(
        bytes memory data
    ) internal pure returns (address, address, uint256, bytes memory) {
        (
            address inputAsset,
            address outputAsset,
            uint256 amount,
        ...
        return (inputAsset, outputAsset, amount, swapData);
    }
