...
interface ISolidlyVoter {
    ...
    function gauges(address _want) external view returns (address);
...
contract KeyChr is KolSolidlyManager, ReentrancyGuardUpgradeable, ERC20Upgradeable {
    ...
    function _deposit(uint256 _amount) internal nonReentrant whenNotPaused {
        ...
        if (!_isExcludedFromFee[msg.sender] && totalMintFees>0 && _amount>0) {
            uint256 fee = _amount * totalMintFees / 10000;
    ...
    function calcBuyFee() public view returns(uint256) { //dynamic buy fee based on the imbalance in pool
        ...
        if (ratio>=imbalanceThresold) //too much keyToken, buy fee need to be minimum
            return minTotalBuyFees;
        else //unhealthy
            return totalMintFees + ((imbalanceThresold-ratio) *  ( maxTotalBuyFees + minTotalBuyFees) / 10000);
    }
    ...
    function _transfer(
        ...
        bool _excluded = _isExcludedFromFee[to] || _isExcludedFromFee[from];

        if (automatedMarketMakerPairs[from] && amount>0 && !_excluded) {
            uint256 fees = amount * calcBuyFee() / 10000;
            super._transfer(from,treasury, fees); //1% buy tax
            amount -= fees;
        }
        if (automatedMarketMakerPairs[to] && amount>0 && !_excluded) {
            uint256 fees = amount * totalSellFees / 10000;
            super._transfer(from,treasury, fees); //1% buy tax
            amount -= fees;
        }
   ...
   */
    ...
    function setmintBalancer(address _mintBalancer) external onlyOwner {
        _isExcludedFromFee[_mintBalancer] = true;
    ...
    function setFees(uint256 _minTotalBuyFees, uint256 _maxTotalBuyFees, uint256 _totalMintFees,
        uint256 _totalSellFees, uint256 _imbalanceThresold) external onlyOwner {
        require(_minTotalBuyFees<=2000 && _maxTotalBuyFees<=2000
        && _totalMintFees<=2000 && _totalSellFees<=2000 && _imbalanceThresold<=10000,">20%");
        minTotalBuyFees = _minTotalBuyFees;
        maxTotalBuyFees = _maxTotalBuyFees;
        totalMintFees = _totalMintFees;
        totalSellFees = _totalSellFees;
        imbalanceThresold = _imbalanceThresold;
    }
    ...
    uint256[] public voteTokenIds;
    address[] public voteVaults;
    uint256[] public voteWeights;
    uint256 public totalMintFees;
    uint256 public totalSellFees;
    uint256 public imbalanceThresold;

    address[][] public voteVaults2;
    uint256[][] public voteWeights2;


    function lockInfoOfToken(uint256 _tokenId)
    public
    view
    returns (
        uint256 endTime,
    ...
    {
        endTime = veToken.locked__end(_tokenId);
        uint256 unlockTime = ((block.timestamp + MAX_LOCK) / 1 weeks) * 1 weeks;
        secondsRemaining = endTime > block.timestamp ? endTime - block.timestamp : 0;
        shouldIncreaseLock = unlockTime > endTime ? true : false;
    }

    function setVoteData(uint256[] calldata _tokenIds, address[][] calldata _vaultVotes, uint256[][] calldata _weights) external onlyOwner {
        require(_vaultVotes.length==_weights.length,"_vaultVotes.length");
        delete voteVaults2;
        voteVaults2 = new address[][](_vaultVotes.length);
        delete voteWeights2;
        voteWeights2 = new uint256[][](_weights.length);

        delete voteTokenIds;
        voteTokenIds = new uint256[](_tokenIds.length);

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            voteTokenIds[i] = _tokenIds[i];

            require(_vaultVotes[i].length==_weights[i].length,"_vaultVotes[i].length");

            delete voteVaults2[i];
            voteVaults2[i] = new address[](_vaultVotes[i].length);
            delete voteWeights2[i];
            voteWeights2[i] = new uint256[](_weights[i].length);

            for (uint256 j = 0; j < _vaultVotes[i].length; j++) {
                voteVaults2[i][j] = _vaultVotes[i][j];
                voteWeights2[i][j] = _weights[i][j];
            }
        }
    }

    function doVote() external authorized {
        for (uint256 i = 0; i < voteTokenIds.length; i++) {
            uint256 _tokenId = voteTokenIds[i];
            (, , bool shouldIncreaseLock) = lockInfoOfToken(_tokenId);
            if (shouldIncreaseLock) {
                veToken.increase_unlock_time(_tokenId, MAX_LOCK);
            }
            if (address(veDist)!=address(0)) veDist.claim(_tokenId);
            solidVoter.vote(_tokenId, voteVaults2[i], voteWeights2[i]);
        }
    }

    function resetAllVotes() external authorized {
        for (uint256 i = 0; i < voteTokenIds.length; i++) {
            uint256 _tokenId = voteTokenIds[i];
