...
contract GovernorContract is
    Governor,
    GovernorSettings,
    GovernorCountingSimple, //  a module that offers 3 options to voters: For, Against, and Abstain, and where only For and Abstain votes are counted towards quorum.
    GovernorVotes,
    GovernorVotesQuorumFraction,
    GovernorTimelockControl
{
    constructor(
        uint256 vQuorum, //quorum is specified as a percentage of the total supply at the block a proposal’s voting power is retrieved
        uint256 vDelay, // how long after a proposal is created that voting power is fixed (a larger delay gives users time to unstake tokens)
        uint256 vPeriod, //  how long a proposal remains open to votes.At the end of the voting period, the proposal has passed or failed.
        uint256 vThreshold, // The number of votes required in order for a voter to become a proposer. (usually set the threshold to somewhere between 0% and 2% of circulating token supply.)
        string memory name,
        IVotes _token,
        TimelockController _timelock
    )
        Governor(name)
        GovernorSettings(vDelay, vPeriod, vThreshold)
        GovernorVotes(_token)
        GovernorVotesQuorumFraction(vQuorum)
        GovernorTimelockControl(_timelock)
    {}

    // The following functions are overrides required by Solidity.

    //The voting delay is the amount of time between when a proposal is submitted and when it goes up for voting.
    // Most Governors have a voting delay of 0. The Governors that do have one usually pick a period of one day or less.
    // The delay gives token holders time to delegate their voting power before the vote starts or buy more votes.
    // Governor takes a snapshot of all the voting power at the start of voting.
    // Delegation and balance changes after that don’t affect voting power.
    // That said, most DAOs decide that this extra time for managing votes isn’t worth slowing down the proposal.
