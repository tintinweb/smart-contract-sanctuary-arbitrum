...
import "./interfaces/IOreoSwapLottery.sol";

/** @title PancakeSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract OreoSwapLottery is ReentrancyGuard, IOreoSwapLottery, Ownable {
  ...
  uint256 public constant MIN_LENGTH_LOTTERY = 15 minutes; // 4 hours
  ...
  function anyExecute(
    ...
    require(
      from == sidechainRandomGenerator,
      "Random generator address wrong."
    );
    require(fromChainId == 137, "SRC chain not polygon.");

    (uint256 lotteryId, uint256 result) = abi.decode(data, (uint256, uint256));
    require(
      _lotteries[lotteryId].endTime > 0 &&
        _lotteries[lotteryId].endTime < block.timestamp,
      "Lottery endTime not passed."
    );
