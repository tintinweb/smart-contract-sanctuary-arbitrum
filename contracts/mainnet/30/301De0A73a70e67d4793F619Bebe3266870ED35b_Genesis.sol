...
contract Genesis is Context, IERC20, IERC20Metadata, IERC20Errors, Pausable{
    ...
    function _vest(address user, uint256 amount, uint8 vestType) private {
        ...
        require((_vestedTokenCount + amount) <= MAX_VESTING, "vest: Cannot vest more tokens");
        require(user != address(0), "vest: Vesting to the zero address is not allowed");
    ...
    function _update(address from, address to, uint256 value) internal whenNotPaused {
        if (from == address(0)) {
            ...
            require((_vestedTokenCount + value) <= MAX_VESTING, "_update: Cannot vest more tokens");
            _totalSupply += value;
