...
// File contracts/interfaces/uniswapV3/IUniswapV3Factory.sol
...
// File contracts/libraries/uniswapV3/TransferHelper.sol
...
// File contracts/interfaces/uniswapV3/ISwapRouter.sol
...
contract ZapInitializer {
    ...
    function initializeSwapTradeFeeNumerator(bool isTestnet) internal {
        if (isTestnet) {
            address WETH = Constants.TESTNET_WETH_ADDRESS;
            address USDT = Constants.TESTNET_USDT_ADDRESS;
            address UNI = Constants.TESTNET_UNI_ADDRESS;
            address RXD = Constants.TESTNET_RXD_ADDRESS;

            /// @dev Goerli testnet initilaization
            swapTradeFeeNumerator[WETH][UNI] = 3000;
            swapTradeFeeNumerator[WETH][USDT] = 3000;
            swapTradeFeeNumerator[UNI][WETH] = 3000;
            swapTradeFeeNumerator[UNI][USDT] = 3000;
            swapTradeFeeNumerator[RXD][USDT] = 3000;
        } else {
    ...
    function initializeSwapPath(bool isTestnet) internal {
        if (isTestnet) {
            ...
            /// @dev Goerli testnet initialization
            swapPath[WETH][UNI] = [WETH, UNI];
            swapPath[WETH][USDT] = [WETH, USDT];
            swapPath[UNI][WETH] = [UNI, WETH];
            swapPath[UNI][USDT] = [UNI, USDT];
            swapPath[RXD][USDT] = [RXD, USDT];
        } else {
            address WETH = Constants.WETH_ADDRESS;
            address ARB = Constants.ARB_ADDRESS;
            address WBTC = Constants.WBTC_ADDRESS;
            ...
            swapPath[USDCE][WBTC] = [USDCE, WBTC];
...
contract Zap is IZap, IZapEvent, Ownable, ZapInitializer {
    ...
    constructor(bool isTestnet, uint24 _slippageToleranceNumerator) {
        ...
        if (isTestnet) {
            WETH = Constants.TESTNET_WETH_ADDRESS;
        } else {
    ...
    function getEstimateSwapOutAmount(
        ...
        uint256 calcAmount = inputAmount;
        ...
        estimateSwapOutAmount = calcAmount;
    }
