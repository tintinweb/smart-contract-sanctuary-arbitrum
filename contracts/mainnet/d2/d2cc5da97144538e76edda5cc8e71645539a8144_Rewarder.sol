...
contract Rewarder is IRewarder, AccessManaged {
    ...
    constructor(address _abra, address authority_) AccessManaged(authority_) {
        ABRA = IERC20(_abra);
    }
    ...
    function compound(
        ...
        uint balance = ABRA.balanceOf(address(this));
        require(balance >= minTokensBought, "Insufficient Tokens Bought");

        ABRA.forceApprove(address(gauge),balance);
        gauge.distributeReward(balance);
    }
