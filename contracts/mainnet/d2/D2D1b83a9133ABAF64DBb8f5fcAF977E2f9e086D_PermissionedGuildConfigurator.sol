...
/**
 * @title IGuildRoleManager
 * @author Covenant Labs
 * @notice Defines the basic interface for a role manager.
 **/
interface IGuildRoleManager {
    /**
     * @dev Emitted on addUserRole()
     * @param user The address of the impacted user
     * @param role The role being added
     **/
    event GuildRoleAdded(address indexed user, DataTypes.Roles indexed role);

    /**
     * @dev Emitted on removeUserRole()
     * @param user The address of the impacted user
     * @param role The role being added
     **/
    event GuildRoleRemoved(address indexed user, DataTypes.Roles indexed role);
    ...
    /**
     * @notice Checks if user is in a given role
     * @param user The address of the user
     * @param role The role
     **/
    function userIsInRole(address user, DataTypes.Roles role) external view returns (bool);

    /**
     * @notice Gets all the roles for a given user
     * @param user The address of the user
     **/
    function getUserRoles(address user) external view returns (uint256[] memory, uint256);

    /**
     * @notice Assigns a set of guild roles to a set of users
     * @param user The addresses of the user
     * @param role The roles
     **/
    function addUserRole(address user, DataTypes.Roles role) external;

    /**
     * @notice Removes a set of guid roles from a set of users
     * @param user The addresses of the user
     * @param role The roles
     **/
    function removeUserRole(address user, DataTypes.Roles role) external;
}
...
import {IGuildConfigurator} from "./IGuildConfigurator.sol";
import {DataTypes} from "../protocol/libraries/types/DataTypes.sol";

/**
 * @title IPermissionedConfigurator
 * @author Covenant Labs
 * @notice Defines the basic interface for a guild & protocol roles.
 **/
interface IPermissionedGuildConfigurator is IGuildConfigurator {
    /**
     * @notice Assigns a set of roles to a set of users
     * @param users The addresses of the user
     * @param roles The roles
     **/
    function addUsersRoles(address[] calldata users, DataTypes.Roles[] calldata roles) external;

    /**
     * @notice Removes a set of roles from a set of users
     * @param users The addresses of the user
     * @param roles The roles
     **/
    function removeUsersRoles(address[] calldata users, DataTypes.Roles[] calldata roles) external;
}
...
import {GuildConfigurator} from "./GuildConfigurator.sol";
import {IPermissionedGuildConfigurator} from "../../interfaces/IPermissionedGuildConfigurator.sol";
import {IGuildAddressesProvider} from "../../interfaces/IGuildAddressesProvider.sol";
import {IGuildRoleManager} from "../../interfaces/IGuildRoleManager.sol";
import {DataTypes} from "../libraries/types/DataTypes.sol";
import {Errors} from "../libraries/helpers/Errors.sol";

/**
 * @title PermissionedGuildConfigurator
 * @author Covenant Labs, inspired by AAVEv3
 * @dev Implements the configuration methods for the Covenant protocol
 **/
contract PermissionedGuildConfigurator is GuildConfigurator, IPermissionedGuildConfigurator {
    /**
     * @dev Constructor
     */
    constructor() GuildConfigurator() {}

    ///@inheritdoc IPermissionedGuildConfigurator
    function addUsersRoles(address[] calldata users, DataTypes.Roles[] calldata roles)
        external
        override
        onlyGuildAdmin
    {
        require(users.length == roles.length, Errors.ARRAY_SIZE_MISMATCH);
        IGuildRoleManager _roleManager = IGuildRoleManager(
            IGuildAddressesProvider(_addressesProvider).getGuildRoleManager()
        );

        for (uint256 i = 0; i < users.length; i++) {
            DataTypes.Roles role = roles[i];
            address user = users[i];
            _roleManager.addUserRole(user, role);
        }
    }

    ///@inheritdoc IPermissionedGuildConfigurator
    function removeUsersRoles(address[] calldata users, DataTypes.Roles[] calldata roles)
        external
        override
        onlyGuildAdmin
    {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            DataTypes.Roles role = roles[i];
            address user = users[i];
            _roleManager.removeUserRole(user, role);
        }
    }
}
