...
interface IRetroBridgeAdvancedAdventure {
...
import "./interfaces/IRetroBridgeAdvancedAdventure.sol";
import "@openzeppelin/contracts/access/Ownable2Step.sol";
import "@openzeppelin/contracts/token/ERC1155/ERC1155.sol";
import "@openzeppelin/contracts/token/ERC1155/extensions/ERC1155Supply.sol";
import "@openzeppelin/contracts/token/ERC1155/extensions/ERC1155URIStorage.sol";

contract RetroBridgeAdvancedAdventure is Ownable2Step, ERC1155Supply, ERC1155URIStorage, IRetroBridgeAdvancedAdventure {

    /// @dev true - whitelisted, false - not whitelisted
    mapping(uint256 nftId => mapping(address account => bool)) public whitelist;

    constructor(string memory _baseURI) Ownable(msg.sender) ERC1155("RetroBridgeAdvancedAdventure") {
    ...
    function setWhitelist(uint256 nftId, address account, bool whitelisted) public onlyOwner {
        require(whitelist[nftId][account] != whitelisted, "RetroBridgeAdvancedAdventure: not changing whitelist state");
    ...
    function safeTransferFrom(address, address, uint256, uint256, bytes memory) public pure override {
        revert("RetroBridgeAdvancedAdventure: safeTransferFrom is forbidden");
    }
    ...
    function safeBatchTransferFrom(
        ...
        revert("RetroBridgeAdvancedAdventure: safeBatchTransferFrom is forbidden");
    }
    ...
    function mint(address to, uint256 nftId, uint256 value) public {
        require(whitelist[nftId][msg.sender], "RetroBridgeAdvancedAdventure: msg.sender not whitelisted");
    ...
    function mintBatch(address to, uint256[] memory nftIds, uint256[] memory values) public {
        for(uint256 i = 0; i < nftIds.length; i++) {
            require(whitelist[nftIds[i]][msg.sender], "RetroBridgeAdvancedAdventure: msg.sender not whitelisted");
        }
        _mintBatch(to, nftIds, values, "");
        emit MintBatch(msg.sender, to, nftIds, values);
    }

    function uri(uint256 nftId) public view override(ERC1155, ERC1155URIStorage, IRetroBridgeAdvancedAdventure) returns (string memory) {
        return ERC1155URIStorage.uri(nftId);
    }

    function balanceOf(address account, uint256 nftId) public view override(ERC1155, IRetroBridgeAdvancedAdventure) returns (uint256) {
    ...
    function balanceOfBatch(
        address[] memory accounts,
        uint256[] memory ids
    ) public view override (ERC1155, IRetroBridgeAdvancedAdventure) returns (uint256[] memory) {
        return super.balanceOfBatch(accounts, ids);
    }

    function totalSupply(uint256 nftId) public view override(ERC1155Supply, IRetroBridgeAdvancedAdventure) returns (uint256) {
        return ERC1155Supply.totalSupply(nftId);
    }

    function totalSupply() public view override(ERC1155Supply, IRetroBridgeAdvancedAdventure) returns (uint256) {
        return ERC1155Supply.totalSupply();
    }

    function exists(uint256 nftId) public view override(ERC1155Supply, IRetroBridgeAdvancedAdventure) returns (bool) {
        return ERC1155Supply.exists(nftId);
    }

    function owner() public view override(Ownable, IRetroBridgeAdvancedAdventure) returns (address) {
