...
contract SmolsRenderer is AccessControlEnumerableV3, SmolsAddressRegistryConsumer  {
    ...
    function generateClothesPNGFromTraitId(uint256 _traitId, uint8 _gender, uint256 _dependencyLevel)
        public
        view
        returns (bytes memory)
    {
        ...
        if(_dependencyLevel == 0){
            //They have headsize 0
            return ISmolsTraitStorage(smolsTraitStorageAddress).getTraitImage(
                _traitId,
                _gender,
                _dependencyLevel
            );
        } else {
            //They have a custom headsize
            //Get trait image for this headsize
            bytes memory _thisImage = ISmolsTraitStorage(smolsTraitStorageAddress).getTraitImage(
                _traitId,
                _gender,
                _dependencyLevel
            );

            //If the image for their headsize is empty, return the 0 image 
            if((_thisImage).length == 0){
                return ISmolsTraitStorage(smolsTraitStorageAddress).getTraitImage(
                    _traitId,
                    _gender,
                    0
    ...
    function generateSVG(Smol memory _smol) public view returns (bytes memory) {
        ...
        return
            abi.encodePacked(
                '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" id="smol" width="100%" height="100%" version="1.1" viewBox="0 0 360 360" ',
                'style="background-color: transparent;background-image:url(',
                generatePNGFromTraitId(_smol.mouth, _smol.gender, _smol.headSize),
                "),url(",
                generatePNGFromTraitId(_smol.hat, _smol.gender, _smol.headSize),
                "),url(",
                generatePNGFromTraitId(_smol.glasses, _smol.gender, _smol.headSize),
                "),url(",
                generateClothesPNGFromTraitId(_smol.clothes, _smol.gender, _smol.headSize),
                "),url(",
    ...
    {//If skin
        ...
        return
            abi.encodePacked(
                ...
                ",",
                //Load the Glasses
                generateMetadataStringForTrait(_smol.glasses, _smol.headSize),
                ",",
                //Load the Hat
                ...
                ",",
                //Load the Mouth
                generateMetadataStringForTrait(_smol.mouth, _smol.headSize),
                ",",
                //Load the Gender
                generateMetadataString(
                    "Gender",
