...
contract CompoundLensMini is ExponentialNoError/*, CompoundLensMiniInterface*/ {
    ...
    function cTokenMetadata(CToken cToken) public view returns (CTokenMetadata memory) {
        ...
        if (isNetworkToken(cToken.symbol())) {
            underlyingAssetAddress = address(0);
            underlyingDecimals = 18;
            underlyingSymbol = removeFirstChar(cToken.symbol());
        } else {
    ...
    // doesn't take into account collateral amount
        ...
        for (uint i=0; i < accounts.length; i++) {
            ...
            for (uint j=0; j < account.markets.length; j++) {
                ...
                for (uint k=0; k < collaterals.length; k++) {
                    ...
                    if (comptroller.liquidateBorrowAllowed(address(account.markets[j]), address(collaterals[k]), msg.sender, account.account, 1) == 0) {
    ...
    // works only for ctoken
    function isLiquidationAllowedForAmount(
        address account,
        CToken borrowed, CToken collateral, uint amount
    ) public returns (bool) {
        CTokenMetadata memory borrowedMetadata = cTokenMetadata(borrowed);

        if (borrowedMetadata.underlying == address(0)) {
            // works only for ctoken
            return true;
        }

        if (borrowedMetadata.underlying != address(0) && CTokenLiquidation(address(borrowed)).liquidateBorrow(account, amount, collateral) == 0) {
            return true;
        }
    ...
    function isNetworkToken(string memory ctokenCymbol_) internal pure returns (bool) {
        return compareStrings(ctokenCymbol_, "bETH") || compareStrings(ctokenCymbol_, "bOKB");
    }

    function removeFirstChar(string memory str) public pure returns (string memory) {
...
interface CEtherLiquidation{
    function liquidateBorrow(address borrower, CToken cTokenCollateral) external payable;
}

interface CTokenLiquidation{
    function liquidateBorrow(address borrower, uint repayAmount, CTokenInterface cTokenCollateral) external  returns (uint);
}
