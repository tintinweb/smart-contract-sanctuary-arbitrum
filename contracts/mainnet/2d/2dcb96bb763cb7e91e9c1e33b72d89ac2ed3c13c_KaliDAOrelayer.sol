...
contract KaliDAOrelayer is Multicallable, Owned {
    event ProcessProposalFail(
        IKaliDAOrelayer indexed dao, 
        uint256 proposal, 
        bytes result
    );
    ...
    constructor(IKaliDAOrelayer _factory, address _owner) payable Owned(_owner) {
        factory = _factory;
    }
    ...
    function processProposal(IKaliDAOrelayer dao, uint256 proposal) external payable {
        (bool success, bytes memory result) = address(dao).call(abi.encodeWithSelector(dao.processProposal.selector, proposal));

        if (!success) emit ProcessProposalFail(dao, proposal, result);
    }
