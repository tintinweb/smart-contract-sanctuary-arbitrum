...
contract SixHeritageCognac is
    ...
    ERC721BurnableUpgradeable
{
    event LockApproveStatus(address[] accounts, bool status);
    error LockedAddress();
    ...
    mapping(address => bool) public lockedAddress;
    ...
    function initialize(address defaultAdmin, address minter, string memory url) public initializer {
        __ERC721_init("Six Heritage cognac", "SIX");
    ...
    function setLockApproveStatus(address[] calldata addresses, bool status) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < addresses.length; i++) {
            lockedAddress[addresses[i]] = status;
        }
        emit LockApproveStatus(addresses, status);
    }

    function approve(address to, uint256 tokenId) public override(IERC721, ERC721Upgradeable) {
        if (lockedAddress[to]) revert LockedAddress();

        super.approve(to, tokenId);
    }

    function setApprovalForAll(address operator, bool approved) public override(IERC721, ERC721Upgradeable) {
        if (lockedAddress[operator]) revert LockedAddress();
    ...
    function batchMint(address to, uint256 count) public onlyRole(MINTER_ROLE) {
        uint256 tokenId;
    ...
    function setBaseTokenURI(string calldata baseTokenURI_) external onlyRole(DEFAULT_ADMIN_ROLE) {
