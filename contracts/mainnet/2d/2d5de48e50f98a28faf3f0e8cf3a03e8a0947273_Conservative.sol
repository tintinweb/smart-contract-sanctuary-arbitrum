...
abstract contract DoubleOwnable is Context {
    ...
    modifier onlyOwner() {
        require(
            firstOwner == _msgSender() || secondOwner == _msgSender(),
...
contract Conservative is ERC721A, DoubleOwnable, ReentrancyGuard {
    ...
    uint256 public constant PRICE = 1000; // 1000 USDT
    ...
    ) ERC721A("CONSERVATIVE", "CONSERVATIVE") DoubleOwnable(_secondOwner) {
        isDividendExempt[address(this)] = true;
        isDividendExempt[0x000000000000000000000000000000000000dEaD] = true;
        isDividendExempt[0x0000000000000000000000000000000000000000] = true;
    ...
    function _mint(address to, uint256 quantity) internal virtual override {
        ...
        if (!isDividendExempt[to]) {
            bool success = dividendDistributor.setShare(to, balanceOf(to));
            require(success, "setShare failed");
        }
    }
    ...
    function transferFrom(
        ...
        if (!isDividendExempt[from]) {
            bool success = dividendDistributor.setShare(from, balanceOf(from));
            require(success, "setShare failed");
        }
    ...
    function withdrawStablecoin() public onlyOwner nonReentrant {
    ...
    function setDividendDistributor(address dd) public onlyOwner {
        dividendDistributor = IDividendDistributor(dd);
    }

    function changeStablecoin(address newStablecoin, uint8 decimals)
        public
        onlyOwner
    {
