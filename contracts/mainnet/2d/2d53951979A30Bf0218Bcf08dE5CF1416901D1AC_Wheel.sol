...
interface IWheel is IGameImplementation {
    /// @notice Wheel bet information struct.
    /// @param configId The config ID.
    /// @param rolled The rolled colors.
    struct WheelBet {
        uint32 configId;
        uint8[] rolled;
    }

    /// @notice Game configuration struct.
    /// @param weightRanges The weights for each color organized by interval (if black = 50, yellow = 30 and red = 20, then the table will look like this [50,80,100]).
    /// @param multipliers The mutlipliers for each color (10000 = x1).
    /// @param maxMultiplier The biggest multiplier.
    /// @param gameId ID of the game for which the config was initially created (used for frontend and subgraph).
    struct GameConfig {
        uint72[] weightRanges;
        uint64[] multipliers;
    ...
    /// @param configId The chosen game config ID.
    /// @param affiliate Address of the affiliate.
    /// @param betCount How many bets at maximum must be placed.
    /// @param stopGain Profit limit indicating that bets must stop after surpassing it (before deduction of house edge).
    /// @param stopLoss Loss limit indicating that bets must stop after surpassing it (before deduction of house edge).
    ...
    /// @param amount The bet amount.
    /// @param configId The chosen game config ID.
    /// @param rolled The rolled wheel numbers.
    /// @param payout The payout amount.
    event Roll(
        uint256 indexed id,
        address indexed receiver,
    ...
    /// @notice Emitted after a game config is added.
    /// @param configId The config ID.
    /// @param weights The weight of each color.
    /// @param multipliers The multiplier of each color.
    /// @param gameId ID of the game for which the config was initially created (used for frontend and subgraph).
    ...
    /// @notice Emitted after maxTotalColors is set.
    /// @param maxTotalColors Maximum number of colors per config.
    event SetMaxTotalColors(uint8 maxTotalColors);

    /// @notice Creates multiple bets and stores the chosen config ID.
    /// @param configId The chosen config ID.
    ...
    /// @notice Add a new game config.
    /// @param weights_ The weight of each color. The sum of the weights is not defined, and can be anything greater than > 1.
    /// @param multipliers_ The mutliplier of each color (10000 = x1).
    /// @param gameId ID of the game for which the config was initially created (used for frontend and subgraph).
    /// @return configId The created config ID.
    function addGameConfig(
        uint64[] calldata weights_,
        uint64[] calldata multipliers_,
        uint32 gameId
    ) external returns (uint32 configId);

    /// @notice Sets the maximum numbers of colors possible in a config.
    /// @param maxTotalColors_ Number of maximal colors.
    /// @dev If set to 0, it allows to pause new config creation
    function setMaxTotalColors(uint8 maxTotalColors_) external;

    function maxTotalColors() external view returns (uint8);
    ...
    function wheelBets(uint256 id) external view returns (WheelBet memory);

    /// @notice Emitted if the configuration is not valid.
    error InvalidConfig();

    /// @notice Emitted if the configuration doesn't exist.
    error ConfigNotExists(uint32 id);
}

/// @title BetSwirl's wheel game
/// @notice The game draws a color at random. Each color can have a different probability of being drawn,
/// as well as a different win multiplier. Several different configurations are possible, thanks to the game's configuration system.
/// @dev This contract can be used for several games at the same time (Wheel of Fortune, Plinko, etc.) by deploying a single contract.
/// @author Kinco
contract Wheel is Game, IWheel {
    /// @notice Maps bets IDs to the config ID and the rolled colors.
    mapping(uint256 => WheelBet) private _wheelBets;

    /// @notice Total configs count
    uint32 private _configsCount;

    // @notice Maximum colors a config can owns
    uint8 private _maxTotalColors;

    /// @notice All game configs
    ...
    /// @param maxColors Maximum number of colors per config
    constructor(
        address bankAddress,
        address chainlinkCoordinatorAddress,
        address chainlinkWrapperAddress,
    ...
    {
        _maxTotalColors = maxColors;
    }

    /// @notice Decode bytes into uint32 config ID.
    /// @param data Bytes to decode.
    /// @return configId The decoded chosen config ID.
    function _decodeBytesToUint32(
        bytes calldata data
    ) private pure returns (uint32 configId) {
        (configId) = abi.decode(data, (uint32));
    }

    /// @notice Calculates the target payout amount.
    /// @param betAmount Bet amount.
    /// @param configId The chosen config ID.
    /// @param rolled The rolled color.
    ...
    function _getPayout(
        ...
        return ((betAmount * _gameConfigs[configId].multipliers[rolled]) /
            BP_VALUE);
    }
    ...
    /// @return rolled The rolled color index (between 0 & config length).
    /// @return rolledPayout The payout based on the rolled color & the betAmount.
    function _roll(
        uint256 id,
        uint256 betAmount,
        uint256 randomWord
    ) internal view override returns (uint256 rolled, uint256 rolledPayout) {
        uint32 configId = _wheelBets[id].configId;
        // Get weightRanges in storage instead of memory saves gas, especially for large configurations.
        ...
        uint72 rolledWeight = uint72(
            (randomWord % weightRanges[totalColors - 1])
        );
        ...
        rolledPayout = _getPayout(betAmount, configId, uint8(rolled));
    }
    ...
    function wager(
        ...
        if (totalColors > _maxTotalColors) revert InvalidConfig();

        (Bet memory bet, uint256 chargedVRFCost) = _newBet(
            token,
            receiver,
            tokenAmount,
            _gameConfigs[configId].maxMultiplier,
            affiliate,
            multiBetData
        );

        _wheelBets[bet.id].configId = configId;
    ...
    /// @notice Creates mutliple bets and stores the chosen config ID.
    /// @param bet The chosen config ID.
    /// @param receiver Address of the receiver who will receive the payout.
    /// @param token Address of the token.
    /// @param tokenAmount The amount per bet.
    /// @param affiliate Address of the affiliate.
    ...
    function fulfillRandomWords(
        ...
        WheelBet storage wheelBet = _wheelBets[id];
        Bet storage bet = bets[id];
        ...
        if (bet.betCount == 1) {
            ...
            payout = _resolvePayout(bet, betAmount, rolledPayout);
        }
        // Multi Bet
        else {
        ...
        wheelBet.rolled = rolledBetsUint8;
        emit Roll(
            bet.id,
            bet.receiver,
            bet.token,
    ...
    function addGameConfig(
        ...
        if (
            totalColors == 0 ||
            totalColors > _maxTotalColors ||
            totalColors != multipliers_.length
        ...
        for (uint8 i; i < totalColors; ) {
            multiplier = multipliers_[i];
            weight = weights_[i];
            totalWeightAmount += weight;
            totalWeightedMultiplierAmount += weights_[i] * multiplier;
            if (maxMultiplier < multiplier) maxMultiplier = multiplier;
            weightRanges[i] = i == 0 ? weight : weightRanges[i - 1] + weight;
            unchecked {
                ++i;
            }
        }
        if (totalWeightAmount == 0) revert InvalidConfig();
        uint256 averageWeightedMultiplier = totalWeightedMultiplierAmount /
            totalWeightAmount;
        if (averageWeightedMultiplier > BP_VALUE) revert InvalidConfig();
        // For rounding issues
        if (
            averageWeightedMultiplier == BP_VALUE &&
            totalWeightedMultiplierAmount % totalWeightAmount != 0
        ) revert InvalidConfig();

        configId = _configsCount;
        _gameConfigs[configId] = GameConfig(
            weightRanges,
            multipliers_,
            maxMultiplier,
            gameId
    ...
    /// @param maxTotalColors_ Number of maximal colors.
    /// @dev If set to 0, it allows to pause new config creation
    function setMaxTotalColors(uint8 maxTotalColors_) external onlyOwner {
        _maxTotalColors = maxTotalColors_;
        emit SetMaxTotalColors(maxTotalColors_);
    }

    function maxTotalColors() external view returns (uint8) {
        return _maxTotalColors;
    }
