...
// File contracts/launchpad/interfaces/IVaultUtils.sol
...
// File contracts/launchpad/interfaces/IWETH.sol
...
// File contracts/launchpad/ZyberCappedSale.sol
...
contract ZyberCappedSale is Ownable, ReentrancyGuard {
    ...
    function getExpectedAllocation(
        ...
        if (noLimits) {
            userMaxCap = vaultUtils.isUserInVaults(account)
                ? type(uint256).max
                : 0;
        } else {
    ...
    function _buy(uint256 amount) internal {
        ...
        uint256 userMaxCap;
        if (noLimits) {
            userMaxCap = vaultUtils.isUserInVaults(msg.sender)
                ? type(uint256).max
                : 0;
        } else {
            userMaxCap =
                CAP_PER_WALLET *
                vaultUtils.getVaultUserInfo(msg.sender);
        }
        require(
