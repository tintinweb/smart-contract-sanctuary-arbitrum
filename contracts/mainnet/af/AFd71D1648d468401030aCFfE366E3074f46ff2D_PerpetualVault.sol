...
contract PerpetualVault is Initializable, ReentrancyGuardUpgradeable, Ownable2StepUpgradeable {
  ...
  bytes4 constant CREATE_DECREASE_ORDER = bytes4(
    keccak256("_createDecreaseOrder(uint256,uint256)")
  );
  ...
  // below `slKey` and `tpKey` is useless. should remove in the new deployment
  bytes32 slKey;
  bytes32 tpKey;
  ...
  mapping (address => mapping (address => bytes)) public swapMarketData;
  ...
  function totalAmount() public view returns (uint256) {
    ...
    IGmxUtils.PositionData memory positionData = gmxUtils.getPositionInfo(market, curPositionKey);
  ...
  function setSwapMarketData(address fromToken, address toToken, bytes memory data) external onlyOwner {
    swapMarketData[fromToken][toToken] = data;
  }
  ...
  function _createIncreasePosition(
    ...
    if (_isLong) {
      (PROTOCOL _protocol, address gPath, bytes memory uniPath) = abi.decode(
        swapMarketData[indexToken][address(collateralToken)],
        (PROTOCOL, address, bytes)
      );
      if (_protocol == PROTOCOL.UNISWAP) {
        ...
        requestKey = gmxUtils.createOrder(orderType, orderData);
      }
    } else {
      orderType = IGmxUtils.OrderType.MarketIncrease;
      collateralToken.safeTransfer(address(gmxUtils), amountIn);
      if (sizeDelta == 0) {
        sizeDelta = gmxUtils.tokenToUsdMin(address(collateralToken), amountIn) * _leverage / BASIS_POINTS_DIVISOR;
      }
      IGmxUtils.OrderData memory orderData = IGmxUtils.OrderData({
        market: market,
  ...
  function _createDecreasePosition(
    ...
    IGmxUtils.OrderType orderType;
    if (beenLong) {
      bytes memory swapMarketData = gmxUtils.getSwapMarketData(indexToken, address(collateralToken));
      (PROTOCOL _protocol, address gPath, bytes memory uniPath) = abi.decode(swapMarketData, (PROTOCOL, address, bytes));
      uint256 amountIn = collateralDelta == 0 ?
        IERC20(indexToken).balanceOf(address(this)) :
        gmxUtils.usdToTokenAmount(address(collateralToken), collateralDelta);
      if (_protocol == PROTOCOL.UNISWAP) {
        ISwapRouter.ExactInputParams memory param =
          ISwapRouter.ExactInputParams({
            path: uniPath,
  ...
  function _createDecreaseOrder(uint256 sl, uint256 tp) internal {
    gmxUtils.createDecreaseOrder(beenLong, sl, tp);
  }
  ...
  function _withdraw(address to, address recipient, uint256 _shares, bool forced) internal returns (bool) {
    ...
    if (want > positionData.netValueInCollateralToken) {
      sizeDelta = sizeInUsd;
      collateralDelta = gmxUtils.tokenToUsdMin(address(collateralToken), positionData.netValueInCollateralToken);      // can be set as 0
    } else {
      //  may need to consider fee deduction
      sizeDelta = want * sizeInUsd / positionData.netValueInCollateralToken;   //  want * size / netValue
      collateralDelta = gmxUtils.tokenToUsdMin(address(collateralToken), want);          // can be set as 0
    }
