...
contract TokenLauncherAdminFacet is ITokenLauncherAdmin {
    function updateTokenFiErc20Implementation(address newImplementation) external override onlyAdmin {
        LibTokenLauncherFactoryStorage.DiamondStorage storage ds = LibTokenLauncherFactoryStorage.diamondStorage();
        ds.tokenFiErc20Implementation = newImplementation;
    }

    function updateTokenFiErc721Implementation(address newImplementation) external override onlyAdmin {
        LibTokenLauncherFactoryStorage.DiamondStorage storage ds = LibTokenLauncherFactoryStorage.diamondStorage();
        ds.tokenFiErc721Implementation = newImplementation;
    }

    function updateTokenFiErc1155Implementation(address newImplementation) external override onlyAdmin {
        LibTokenLauncherFactoryStorage.DiamondStorage storage ds = LibTokenLauncherFactoryStorage.diamondStorage();
        ds.tokenFiErc1155Implementation = newImplementation;
    }

    function setTokenFiErc20Implementation(address token) external override {
        require(
            ...
            "TokenLauncherAdmin: Only admin or token owner can call this function"
        );

        LibTokenLauncherFactoryStorage.DiamondStorage storage ds = LibTokenLauncherFactoryStorage.diamondStorage();

        address _implementation = IDiamondProxy(token).implementation();
        require(_implementation != ds.tokenFiErc20Implementation, "TokenLauncherAdmin: Implementation is already set");

        IDiamondProxy(token).setImplementation(ds.tokenFiErc20Implementation);
    }

    function setTokenFiErc721Implementation(address token) external override {
        require(
            IAccessControl(address(this)).hasRole(AccessControlStorage.DEFAULT_ADMIN_ROLE, msg.sender) ||
                IAccessControl(token).hasRole(AccessControlStorage.DEFAULT_ADMIN_ROLE, msg.sender),
            "TokenLauncherAdmin: Only admin or token owner can call this function"
        ...
        address _implementation = IDiamondProxy(token).implementation();
        require(_implementation != ds.tokenFiErc721Implementation, "TokenLauncherAdmin: Implementation is already set");

        IDiamondProxy(token).setImplementation(ds.tokenFiErc721Implementation);
    }

    function setTokenFiErc1155Implementation(address token) external override {
        require(
            IAccessControl(address(this)).hasRole(AccessControlStorage.DEFAULT_ADMIN_ROLE, msg.sender) ||
                IAccessControl(token).hasRole(AccessControlStorage.DEFAULT_ADMIN_ROLE, msg.sender),
            "TokenLauncherAdmin: Only admin or token owner can call this function"
        ...
        address _implementation = IDiamondProxy(token).implementation();
        require(_implementation != ds.tokenFiErc1155Implementation, "TokenLauncherAdmin: Implementation is already set");

        IDiamondProxy(token).setImplementation(ds.tokenFiErc1155Implementation);
    }
