...
contract TradingOpenFacet is ITradingOpen, OnlySelf {
    ...
    function limitOrderDeal(LimitOrderDeal memory order, uint256 marketPrice) external onlySelf override {
    ...
    function marketTradeCallback(bytes32 tradeHash, uint upperPrice, uint lowerPrice) external onlySelf override {
        ...
        (bool successful, ITradingChecker.Refund refund, bytes memory data) = ITradingChecker(address(this)).marketTradeCallbackCheck(pt, marketPrice);
        if (!successful) {
    ...
    function _marketTradeDeal(
        ...
        bytes32 tradeHash, uint96 openFee, uint96 executionFee, uint256 marketPrice, uint64 entryPrice
    ) private {
        ...
        OpenTrade memory ot = OpenTrade(
            pt.user, uint32(tradeHashes.length), entryPrice, pt.pairBase, pt.tokenIn, margin, pt.stopLoss,
