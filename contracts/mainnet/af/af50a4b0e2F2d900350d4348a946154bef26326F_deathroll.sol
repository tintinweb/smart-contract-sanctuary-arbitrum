...
contract deathroll is ReentrancyGuard, Ownable {
    ...
    function createGame(uint256 _friendId, address _friendCollection, uint8 _stakeId, uint16 _buddyId, bytes32 _secretWish) external nonReentrant {
        require(opened, "Cannot create new games at this time");
        uint256 rollerId = ROLLERS.checkOwnership(msg.sender,_friendId,_friendCollection);
    ...
    function joinGame( uint256 _friendId, address _friendCollection, uint256 _gameId, bytes32 _secretWish) external nonReentrant isActive(_gameId) {
        require(!games[_gameId].started && games[_gameId].opened, "Cannot join this game");
        uint256 rollerId =ROLLERS.checkOwnership(msg.sender,_friendId,_friendCollection);
        if(games[_gameId].rollerIds[1]>0){
            require(rollerId == games[_gameId].rollerIds[1], "This game is private");
        }
    ...
    function rollTheDice(uint256 _friendId, address _friendCollection, uint256 _gameId, string memory _wish) external nonReentrant isActive(_gameId) {
        require(games[_gameId].started && !games[_gameId].opened, "Cannot roll this dice");
        Game storage game = games[_gameId];
        require(ROLLERS.checkOwnership(msg.sender,_friendId,_friendCollection)==game.rollerIds[0] || ROLLERS.checkOwnership(msg.sender,_friendId,_friendCollection)==game.rollerIds[1], "You're not in this game");
        require(game.currStage == Stage.FirstReveal || game.currStage == Stage.SecondReveal, "not at reveal stage");

        uint playerIndex;
        if(ROLLERS.checkOwnership(msg.sender,_friendId,_friendCollection)==game.rollerIds[0]) playerIndex = 0;
        else if (ROLLERS.checkOwnership(msg.sender,_friendId,_friendCollection)==game.rollerIds[1]) playerIndex = 1;
    ...
    function leaveGame(uint256 _gameId,uint256 _friendId ,address _friendCollection) external nonReentrant isActive(_gameId) {
        Game storage game = games[_gameId];
        require(ROLLERS.checkOwnership(msg.sender,_friendId,_friendCollection)==game.rollerIds[0] || ROLLERS.checkOwnership(msg.sender,_friendId,_friendCollection)==game.rollerIds[1], "You're not in this game");

        if(game.currStage == Stage.SecondCommit){
            MAGIC.safeTransfer(ROLLERS.getOwner(game.rollerIds[0]),stakes[game.stake]);
            game.winner=uint8(0);
    ...
    function getGamePlayers(uint256 _gameId) external view returns(uint256 rollerId0, uint256 rollerId1){
     ...
     function getGameCommits(uint256 _gameId) external view returns(string memory commitP1, string memory commitP2){
