...
contract DolomiteMoneyMarketView is BaseMoneyMarketView {
    ...
    {
        ltv = dolomite.getMarginRatio().value + WAD + dolomite.getMarketMarginPremium(dolomite.getMarketIdByTokenAddress(debtAsset)).value
            + dolomite.getMarketMarginPremium(dolomite.getMarketIdByTokenAddress(collateralAsset)).value;
        liquidationThreshold = ltv = Math.mulDiv(WAD, WAD, ltv);
    }
...
import "./dependencies/IDolomiteMargin.sol";
import "./dependencies/IIsolationVault.sol";
...
contract DolomiteMoneyMarket is BaseMoneyMarket {
    ...
    IDolomiteMargin public immutable dolomite;
    IIsolationVault public isolationVault;
    ...
    function _initialise(PositionId positionId, IERC20 collateralAsset, IERC20 debtAsset) internal virtual override {
        ...
        uint256 marketId = uint40(Payload.unwrap(positionId.getPayload()));
        if (marketId > 0) {
            isolationVault = IIsolationToken(address(dolomite.getMarketTokenAddress(marketId))).createVault(address(this));
            collateralAsset.forceApprove(address(isolationVault), type(uint256).max);
        } else {
            collateralAsset.forceApprove(address(dolomite), type(uint256).max);
        }

        debtAsset.forceApprove(address(dolomite), type(uint256).max);
    }

    function _collateralBalance(PositionId, IERC20 asset) internal view virtual override returns (uint256 balance) {
        return isolationVault == IIsolationVault(address(0))
            ? dolomite.getAccountWei(_self(), dolomite.getMarketIdByTokenAddress(asset)).value
            : isolationVault.underlyingBalanceOf();
    }

    function debtBalance(PositionId, IERC20 asset) public view virtual returns (uint256 balance) {
        return dolomite.getAccountWei(_selfOrAccount(), dolomite.getMarketIdByTokenAddress(asset)).value;
    }
    ...
    function _lend(PositionId, IERC20 asset, uint256 amount, address payer) internal virtual override returns (uint256 actualAmount) {
        ...
        if (isolationVault == IIsolationVault(address(0))) {
            __deposit(asset, actualAmount);
        } else {
            isolationVault.depositIntoVaultForDolomiteMargin({ toAccountNumber: 0, amountWei: actualAmount });
            isolationVault.openBorrowPosition({ fromAccountNumber: 0, toAccountNumber: uint160(address(this)), amountWei: actualAmount });
        }
    }
    ...
    function _borrow(PositionId, IERC20 asset, uint256 amount, address to) internal virtual override returns (uint256 actualAmount) {
        if (isolationVault != IIsolationVault(address(0))) {
            isolationVault.transferFromPositionWithOtherToken({
                borrowAccountNumber: uint160(address(this)),
                toAccountNumber: 0,
                marketId: 0,
                amountWei: amount,
                balanceCheckFlag: IIsolationVault.BalanceCheckFlag.None
            });
        }
        ...
        returns (uint256 actualAmount)
    {
        ...
        if (actualAmount > 0) {
            ...
            if (isolationVault != IIsolationVault(address(0))) {
                isolationVault.transferIntoPositionWithOtherToken({
                    fromAccountNumber: 0,
                    borrowAccountNumber: uint160(address(this)),
                    marketId: 0,
                    amountWei: actualAmount,
                    balanceCheckFlag: IIsolationVault.BalanceCheckFlag.None
                });
            }
        }
    ...
    function _withdraw(PositionId, IERC20 asset, uint256 amount, address to) internal virtual override returns (uint256 actualAmount) {
        if (isolationVault == IIsolationVault(address(0))) {
            ...
            isolationVault.transferFromPositionWithUnderlyingToken({
                borrowAccountNumber: uint160(address(this)),
                toAccountNumber: 0,
                amountWei: amount
            });
            isolationVault.withdrawFromVaultForDolomiteMargin({ fromAccountNumber: 0, amountWei: amount });
    ...
    function _selfOrAccount() internal view returns (IDolomiteMargin.Info memory self) {
        if (isolationVault != IIsolationVault(address(0))) {
            self.owner = address(isolationVault);
            self.number = uint160(address(this));
        } else {
            self.owner = address(this);
        }
    }
...
interface IIsolationVault {
    ...
    function openBorrowPosition(uint256 fromAccountNumber, uint256 toAccountNumber, uint256 amountWei) external payable;
    ...
    function transferFromPositionWithUnderlyingToken(uint256 borrowAccountNumber, uint256 toAccountNumber, uint256 amountWei) external;
