...
interface IOracle {
    function getPrimaryPrice(
        address token
    ) external view returns (Price.Props memory);
    ...
    PausableUpgradeable
{
    ...
    function afterWithdrawalExecution(bytes32 key, Withdrawal.Props memory withdrawal, EventUtils.EventLogData memory eventData) external {
        ...
        if (longAmountFromGMX > longDebtValue) {
            // Calculate long tokens available to swap for Long Token
            uint256 longAvailableToSwap = longAmountFromGMX - longDebtValue;
        ...
        /* if (longAmountFromGMX < longDebtValue && usdcAmountFromGMX > (shortDebtValue + pi.deposit)) */
            ...
            uint256 shortAvailableToSwap = usdcAmountFromGMX - shortDebtValue;
        ...
        // add returned from gmx to long token amount returned
        uint256 totalLongToken = data.longTokenAmountReturned + longAmountFromGMX;
        console.log("totalLongToken", totalLongToken);
        // add returned from gmx to short token amount returned
        uint256 totalShortToken = data.shortTokenAmountReturned + usdcAmountFromGMX;
        ...
        wr.isLiquidation
            ...
            : IVodkaV2(strategyAddresses.VodkaV2).fulfillClosePosition(key, totalLongToken, totalShortToken, data.longAmountValue);
    }
    ...
    function afterWithdrawalCancellation(
        ...
        or.feesRefunded = eventData.uintItems.items[0].value;
        or.gmTokensRefunded = dr.receivedMarketTokens;
        or.cancelled = true;
