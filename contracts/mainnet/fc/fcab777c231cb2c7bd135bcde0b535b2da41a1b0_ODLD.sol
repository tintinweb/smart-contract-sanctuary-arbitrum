...
contract ODLD is OFT{
    ...
    )  OFT(_name, _symbol, _layerzeroEndPoint, msg.sender) Ownable(msg.sender) {
    ...
    function setExcludeFee(address user, bool value) public onlyOwner{
    ...
    function transfer(address to, uint256 value) public override returns (bool) {
        require(!_blacklisted[msg.sender], "ERC20: account is blacklisted");
        require(!_blacklisted[to] , "ERC20: account is blacklisted");
        address owner = _msgSender();
        _transfer(owner, to, value);
        return true;
    }
    ...
    function transferFrom(address from, address to, uint256 value) public override returns (bool) {
        require(!_blacklisted[from], "ERC20: account is blacklisted");
        require(!_blacklisted[to] , "ERC20: account is blacklisted");
        address spender = _msgSender();
        _spendAllowance(from, spender, value);
        _transfer(from, to, value);
    ...
    function _update(address from, address to, uint256 value) internal override {
        ...
        if(tradingOpen){
            uint256 curTaxRate = isExcludedFromFees[from] ||
            isExcludedFromFees[to]
                ...
                : from == uniswapV2Pair
                    ...
                    : to == uniswapV2Pair
                            ? getTaxRate()
                            : 0;
            uint256 tax = (value * curTaxRate) / 100;
    ...
    function getTaxRate() public view returns(uint256){
        if(block.timestamp - poolInitTime < 120){
           return 30;
        }else {
            return 0;
        }
...
contract StakingRewards is OwnableUpgradeable{
    ...
    function init(
        ...
        rewardsToken = IesODLD(_rewardToken);
    }
