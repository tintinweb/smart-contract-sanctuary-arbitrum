...
// File: cobosafe-master/contracts/Types.sol
...
// File: cobosafe-master/interfaces/IRoleManager.sol
...
// File: cobosafe-master/interfaces/IAccount.sol
...
// File: cobosafe-master/interfaces/IAuthorizer.sol
...
// File: cobosafe-master/interfaces/IVersion.sol
...
// File: cobosafe-master/contracts/base/BaseVersion.sol
...
// File: cobosafe-master/contracts/Errors.sol
...
// File: cobosafe-master/contracts/base/BaseOwnable.sol
...
// File: cobosafe-master/contracts/base/BaseAuthorizer.sol
...
// File: cobosafe-master/contracts/base/BaseACL.sol
...
// File: cobosafe-master/contracts/acls/approves/ApprovsAuthorizer.sol
...
contract ApprovsAuthorizer is BaseACL{

    bytes32 public constant NAME = "ApproveAuthorizer";
    uint256 public constant VERSION = 1;

    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet approveAddressWhitelist;

    address constant BASE_APPROVER = 0x503e7088519769BdcEa21449Cf74A391a7Eca27C;
    address private _admin;

    event AddContractWhitelist(address indexed _contractAddress, address indexed user);
    event RemoveContractAddressWhitelist(address indexed _contractAddress, address indexed user);
    ...
    function addContractToApprove(address[] calldata _contractAddress) external onlyAdmin {
        for (uint256 i = 0; i < _contractAddress.length; i++) {
            if (approveAddressWhitelist.add(_contractAddress[i])) {
                emit AddContractWhitelist(_contractAddress[i], msg.sender);
            }
        }
    }

    function removePoolAddresses(address[] calldata _contractAddress) external onlyAdmin {
        for (uint256 i = 0; i < _contractAddress.length; i++) {
            if (approveAddressWhitelist.remove(_contractAddress[i])) {
                emit RemoveContractAddressWhitelist(_contractAddress[i], msg.sender);
            }
        }
    }

    function _checkAllowContractAddress(address _contractAddress) internal view {
        require(approveAddressWhitelist.contains(_contractAddress), "pool address not allowed");
    }
    ...
    function makeApprove(
        ...
        address _to,
        uint256 _amount 
        ) external view onlyContract (BASE_APPROVER){
            _checkAllowContractAddress(_to);
    }


    function contracts() public view override returns (address[] memory _contracts) {
        _contracts = new address[](1);
        _contracts[0] = BASE_APPROVER;
