...
contract TrancheMasterPerpetualUpgradeable is ITrancheMasterPerpetual, Initializable, CoreRefUpgradeable, ReentrancyGuardUpgradeable {
            ...
            if (u.isAuto) {
                for (uint j = inv.cycle; j < cycle; j++) {
                    ...
                    if (inv.rebalanced) {
                        ...
                        left = principal
                            .mul(PERCENTAGE_SCALE.sub(snapshot.validPercent))
                ...
                if (active && !inv.rebalanced) {
                    ...
                    left = principal
                        .mul(PERCENTAGE_SCALE.sub(tranches[i].validPercent))
                        .div(PERCENTAGE_SCALE);
                    if (left > 0) {
                        left -= 1;
                    }
    ...
    function _startCycle() internal checkNotActive {
        ...
        for (uint256 i = 0; i < tranches.length; i++) {
            Tranche storage t = tranches[i];
            t.validPercent = (t.principal.add(t.autoPrincipal)).mul(PERCENTAGE_SCALE).div(t.principal.add(t.autoPrincipal));
            t.autoValid = t.principal == 0 ? t.principal.add(t.autoPrincipal) : t.autoPrincipal.mul(t.validPercent).div(PERCENTAGE_SCALE);
    ...
    function _processExit(address[] memory _strategyAddresses) internal {
        ...
        p.seniorYieldDistribution = ITrancheYieldCurve(trancheYieldCurve).getSeniorYieldDistribution(p.seniorProportion);
        p.seniorYield = p.totalYield.mul(p.seniorYieldDistribution).div(PERCENTAGE_SCALE);
