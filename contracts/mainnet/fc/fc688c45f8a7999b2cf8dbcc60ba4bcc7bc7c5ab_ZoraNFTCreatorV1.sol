    ...
    ERC721RewardsStorageV1
{
    ...
    /// @return tokenId of the first token minted
    function purchaseWithComment(uint256 quantity, string calldata comment) external payable returns (uint256) {
    ...
    function _handleMintWithRewards(
        ...
        address mintReferral
    ) internal nonReentrant onlyPublicSaleActive returns (uint256) {
    ...
    /// @param quantity quantity to mint
    function adminMint(address recipient, uint256 quantity) external onlyRoleOrAdmin(MINTER_ROLE) returns (uint256) {
        _requireCanMintQuantity(quantity);
        _mintNFTs(recipient, quantity);
    ...
    function adminMintAirdrop(address[] calldata recipients) external override onlyRoleOrAdmin(MINTER_ROLE) returns (uint256) {
        _requireCanMintQuantity(recipients.length);
