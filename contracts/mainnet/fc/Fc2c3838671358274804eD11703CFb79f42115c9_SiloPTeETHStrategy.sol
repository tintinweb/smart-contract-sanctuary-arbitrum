...
contract SiloPTeETHStrategy is
    Initializable,
    ILeverageStrategy,
    IFlashLoanRecipient,
    ...
    UUPSUpgradeable
{
    ...
    function _flRemoveLeverage(bytes calldata params, uint256 feeAmount) internal {
        ...
        outAmount = _swapUniswap(weETH, debtToken(), outAmount, amountOutMinimum, false);
    ...
    function _flCloseLeverage(bytes calldata params, uint256 feeAmount) internal {
        ...
        (uint256 amountOutMinimum, bool closeToAsset, uint256 amountOO, bytes memory dataOO) = abi.decode(
            data,
            (uint256, bool, uint256, bytes)
        );
        ...
        if (isExpired()) {
            ...
            this.swapBySelf(weETH, debtToken(), amountOO, dataOO);
        } else {
            ...
            if (closeToAsset) {
                ...
                _swapUniswap(weETH, debtToken(), repayAmount + feeAmount, amountOutMinimum, true);
            } else {
                // If not closing to asset, perform swap with the original outAmount.
                _swapUniswap(weETH, debtToken(), outAmount, amountOutMinimum, false);
            }
        }
    ...
    function _swapUniswap(
        ...
        if (isExactOutput) {
            inAmount = SiloReward.swapUniswapExactOutputSingle(tokenIn, tokenOut, amountInput, amountInMaximum);
            feeCharge = leverageFeeCharge(inAmount, tokenIn);
        } else {
            inAmount = SiloReward.swapUniswapExactInputSingle(tokenIn, tokenOut, amountInput, amountInMaximum);
