...
contract PendleForwarder is Ownable {
    ...
    event PendleSwap(address indexed market, address indexed user, uint256 user_fid);
    ...
    function forwardCall(address target, bytes calldata data, address market, uint256 fid) public payable {
        uint256 swapValue;
        uint256 feeAmount;
        if (fee > 0) {
        ...
        if (success) {
            emit PendleSwap(market, msg.sender, fid);
            if (feeAmount > 0) {
