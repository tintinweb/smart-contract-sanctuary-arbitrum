...
interface IMarketLiquidation {
  ...
  event SetSmallAccountThreshold(uint256 threshold);
...
interface IPortfolio {
  ...
  event SupplyETH(address indexed account, uint256 amount, uint256 shares, address caller);

  event Withdraw(address indexed account, address indexed asset, uint256 amount, uint256 shares, address caller);

  event WithdrawETH(address indexed account, uint256 amount, uint256 shares, address caller);
  ...
  function getPortfolioValue(
    IPortfolioStorage.PortfolioAsset[] memory position
  ) external view returns (USD, USD[] memory);
}
...
interface IFlashLender is ISweepable, IERC3156FlashLender {
  event SetFlashLoanFee(address indexed caller, BPS bps, uint256 maxAmount);

  event SetFeeWhitelist(address indexed caller, address indexed account, bool excluded);
...
library Errors {
  ...
  // allow list
  error AllowList_NotAllowed(address account);

  // zappers
  error Zapper_SlippageExceeded(uint256 actual, uint256 expected);
}
...
import { IAddressRegistry } from "@ambit/finance-contracts/contracts/interfaces/core/IAddressRegistry.sol";
import { Migration } from "@ambit/finance-contracts/contracts/protocol/governance/Migration.sol";
import { AuthorizedAccessControl } from "@ambit/finance-contracts/contracts/protocol/security/AuthorizedAccessControl.sol";

contract Migration_ARB_Liquidator is
  Migration // solhint-disable-line contract-name-camelcase
{
  ...
  function migrate() internal override onlyAdmin {
    // grant authorization on FLASH_LENDER to the LIQUIDATOR
    grantAuthorization(0x76E031F27797d82FA8673Ff752759AB0e63BA67E, 0x9E40449fF25Da734243965CE97411ABA2C77DbA4);
  }

  function grantAuthorization(address source, address dest) private {
    AuthorizedAccessControl authorization = AuthorizedAccessControl(source);
    authorization.grantAuthorization(dest);
  }
}