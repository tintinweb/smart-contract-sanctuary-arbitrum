...
contract DefiEdgeStrategyFactory is IStrategyFactory {
    ...
    function createStrategy(CreateStrategyParams calldata params) external payable override {
        ...
        IRamsesV2Pool pool = IRamsesV2Pool(params.pool);
...
contract StrategyManager is AccessControl, IStrategyManager {
    ...
    constructor(
        ...
        IVoter voter = IVoter(pool.voter());
        gauge = IGaugeV2(voter.gauges(address(pool)));
    ...
    /**
     * Update ramses reward receiver address
     * @param _rewardReceiver New address where rewards should be received
     */
    function changeRewardReceiver(address _rewardReceiver) external {
        require(msg.sender == factory.governance() || hasRole(ADMIN_ROLE, msg.sender), "N");
        rewardReceiver = _rewardReceiver;
    }

    function getRewardParameters() external override view returns(IGaugeV2 _gauge, address _rewardReceiver, address[] memory _rewardTokens) {
        _gauge = gauge;
        _rewardReceiver = rewardReceiver;
        _rewardTokens = gauge.getRewardTokens();
    }
