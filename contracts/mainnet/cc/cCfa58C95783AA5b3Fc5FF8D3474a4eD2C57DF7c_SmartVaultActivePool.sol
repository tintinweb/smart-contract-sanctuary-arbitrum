...
/// @author belbix
contract SmartVault is ERC20Upgradeable, VaultStorage, ControllableV2 {
  ...
  function initializeSmartVault(
    ...
    VaultStorage.initializeVaultStorage(__underlying, _duration, false);
  ...
  /// @dev Only smart contracts will be affected by this restriction
  ///      If it is a contract it should be whitelisted
  function onlyAllowedUsers(address _sender) public view {
    _onlyAllowedUsers(_sender);
  }
  ...
  function doHardWork() external override {
    require(_isController(msg.sender), FORBIDDEN_MSG);
    _invest();
  ...
  function withdrawAllToVault() external override {
    require(_controller == msg.sender || IController(_controller).governance() == msg.sender, FORBIDDEN_MSG);
    IStrategy(_strategy()).withdrawAllToVault();
  }
  ...
  function _underlyingBalanceWithInvestment() internal view returns (uint256) {
    return VaultLibrary.underlyingBalanceWithInvestment(_strategy(), IERC20(_underlying()).balanceOf(address(this)));
  }
  ...
  function previewDeposit(uint256 assets) public view returns (uint256) {
    return
      totalSupply() == 0
        ? assets
        : assets.mulDiv(totalSupply(), _underlyingBalanceWithInvestment() + 1, MathUpgradeable.Rounding.Up);
  }
  ...
  function _getPricePerFullShare() internal view returns (uint256) {
    return
      totalSupply() == 0 ? _underlyingUnit() : (_underlyingUnit() * _underlyingBalanceWithInvestment()) / totalSupply();
  }
  ...
  function _availableToInvestOut() internal view returns (uint256) {
    return VaultLibrary.availableToInvestOut(_strategy(), _toInvest(), _underlyingBalanceInVault());
  }
  ...
  function _deposit(uint256 amount, address sender, address beneficiary) internal {
    ...
    uint256 toMint = totalSupply() == 0 ? amount : (amount * totalSupply()) / _underlyingBalanceWithInvestment();
  ...
  function _earned(address rt, address account) internal view returns (uint256) {
    return
      (balanceOf(account) * (_rewardPerToken(rt) - userRewardPerTokenPaidForToken[rt][account])) /
      1e18 +
  ...
  function _rewardPerToken(address rt) internal view returns (uint256) {
    ...
    return
      ...
      (((_lastTimeRewardApplicable(rt) - lastUpdateTimeForToken[rt]) * rewardRateForToken[rt] * 1e18) /
  ...
  function getRewardTokenIndex(address rt) external view override returns (uint256) {
  ...
  function _rebase() internal virtual {
    // no-op
  }

  function rebase() external {
    isActive();
    onlyAllowedUsers(msg.sender);
    _rebase();
  }
}
