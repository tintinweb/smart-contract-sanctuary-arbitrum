...
contract Treasury is Operator, ReentrancyGuard {
    ...
    function weth_price() internal view returns (uint256) {
        return IGlpVault(glpVault).getMinPrice(weth);
    }

    function weth_price_precision() internal view  returns (uint256) {
        return IGlpVault(glpVault).PRICE_PRECISION();
    }

    function wsteth_price() internal view returns (uint256) {
        return weth_price() * IRiskVault(riskOnPool).share_price() / IRiskVault(riskOnPool).share_price_decimals();
    }

    function wsteth_price_precision() internal view returns (uint256) {
    ...
    function getRequiredUsdCollateral() public view returns (uint256) {
        ...
        uint256 wstethValue = (vault.total_supply_wait() + vault.total_supply_staked() + vault.total_supply_withdraw()) * wsteth_price() / wsteth_price_precision();
    ...
    // allocate rewards
    function allocateReward(int256 _amount) external onlyGovernance {
        IRiskVault(riskOnPool).allocateReward(_amount);
    }
