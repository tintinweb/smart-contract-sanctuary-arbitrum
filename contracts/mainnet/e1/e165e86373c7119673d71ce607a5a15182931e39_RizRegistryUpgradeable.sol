...
import { RizLendingPoolAddressesProviderMinimalProxy } from
    "./riz-lending/RizLendingPoolAddressesProviderMinimalProxy.sol";
...
/// @title RizRegistryUpgradeable
/// @author Radiant
/// @notice This contract is an upgradeable alternative to `RizRegistry.sol`.
/// @dev This upgradeable contract does not require a ProxyAdmin, and it is compatible with `UUPSUpgradeable`.
contract RizRegistryUpgradeable is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function addPool(string calldata marketId_) external onlyOwner {
        ...
        RizLendingPoolAddressesProviderMinimalProxy addressProvider =
            RizLendingPoolAddressesProviderMinimalProxy(addressProviderAddr);
    ...
    /// @notice Set lendingPool enabled or not. If disabled, no Riz operations in LockZap will be allowed
    /// @param lendingPool address
    /// @param enabled bool
    function setLendingPoolEnabled(address lendingPool, bool enabled) external onlyOwner {
        _lendingPools[lendingPool] = enabled;
        emit LendingPoolToggled(lendingPool, enabled);
    }
...
import { Address } from "@openzeppelin/contracts/utils/Address.sol";
import { ILendingPoolAddressesProvider } from "@radiant-v2-core/interfaces/ILendingPoolAddressesProvider.sol";
import { RizLendingPoolAddressesProvider } from "./RizLendingPoolAddressesProvider.sol";
import { IRizLendingPoolAddressesProvider } from "../interfaces/Riz/IRizLendingPoolAddressesProvider.sol";
...
import { IFlashLoanReceiver } from "@radiant-v2-core/lending/flashloan/interfaces/IFlashLoanReceiver.sol";
...
contract RizLendingPool is VersionedInitializable, IRizLendingPool, RizLendingPoolStorage {
    ...
    function depositWithAutoDLP(address asset, uint256 amount, address onBehalfOf, uint16 referralCode)
        external
        virtual
        whenNotPausedOrShutdown
    {
        ...
        ILeverager leverager = ILeverager(IRizLendingPoolAddressesProvider(_addressesProvider).getLeverager());
        leverager.zapWETHWithBorrow(leverager.wethToZap(onBehalfOf), onBehalfOf);
    }

    /// @dev Emergency withdrawal NOTE: temporarily disabled to deploy to mainnet
    function _emergencyWithdraw(address owner, address to) internal {
        // BadDebtManager badDebtManager = BadDebtManager(_addressesProvider.getBadDebtManager());
        ...
        // uint256 _totalUserDepositsUSD;
        // uint256 _totalUserDebtsUSD;

        // uint256[] memory _cacheReserveRatios = new uint256[](_reservesCount);
        // for (uint256 i = 0; i < _reservesCount; i++) {
        //     address reserveAddress = _reservesList[i];
        //     DataTypes.ReserveData memory reserve = _reserves[reserveAddress];
        ...
        //     _totalUserDepositsUSD += userDeposit.mul(reserveUSDPrice).div(DEFAULT_DENOMINATOR);
        //     _totalUserDebtsUSD += userDebt.mul(reserveUSDPrice).div(DEFAULT_DENOMINATOR);
        //     _cacheReserveRatios[i] = badDebtManager.getAssetRatio(reserveAddress);
        // }
        // // If user allowance is 0 or they are responsible for bad debt, we can't proceed with the emergency
        // withdrawal
        // if (_totalUserDepositsUSD < _totalUserDebtsUSD || _totalUserDepositsUSD == 0) {
        //     revert RizErrors.UserAllowanceZero();
        // }
        // uint256 userAllowanceUSD = _totalUserDepositsUSD - _totalUserDebtsUSD;
        ...
        // // represented
        // // in BPS, so we need to divide them by PRECISION_DIVISOR to get the actual USD amount user will receive by
        // each
        // // token
        // uint256[] memory _userAllowances = new uint256[](_reservesCount);
        // for (uint256 i = 0; i < _reservesCount; i++) {
        ...
        //     );
        // }
        // // Ser emergency withdrawal status for user
    ...
    function flashLoan(
        ...
        for (vars.i = 0; vars.i < assets.length;) {
            ...
            premiums[vars.i] = amounts[vars.i].mul(_flashLoanPremiumTotal).div(BIPS_DIVISOR);
 ...
 *
 */
contract RizLendingPoolAddressesProviderMinimalProxy is
    Initializable,
    OwnableUpgradeable,
 ...
 *
 */
contract RizLendingPoolAddressesProvider is Ownable, IRizLendingPoolAddressesProvider {
    string private _marketId;
    mapping(bytes32 => address) private _addresses;
    address private _liquidationFeeTo;
    address public leverager;
