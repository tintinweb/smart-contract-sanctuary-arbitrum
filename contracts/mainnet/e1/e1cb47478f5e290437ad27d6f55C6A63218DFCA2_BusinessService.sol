...
contract BusinessService is IBusinessService, Initializable, UUPSUpgradeable {
    ...
    function businessHook(
        ...
        if (IVaultFacet(_diamond).getVaultType(_vault) != 3) {
            if (
                (IPriceOracle(0x60E974258eFCCEf5E7B9D579F47F600aC0a7064C)
                    .getUSDPrice(_token) * _amount) /
                    10 **
                        (
                            _token == IPlatformFacet(_diamond).getEth()
                                ? 1 ether
                                : IERC20(_token).decimals()
                        ) >=
                9.5 ether
            ) {
                uint ethAmount = (5 * 1 ether * 1 ether) /
                    IPriceOracle(0x60E974258eFCCEf5E7B9D579F47F600aC0a7064C)
                        .getUSDPrice(
                            0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE
                        );
                IPaymasterFacet(_diamond).setQuotaWhiteList(
                    2,
                    IVault(_vault).owner(),
                    ethAmount
                );
                emit BusinessHook(_vault, ethAmount, _token, _amount);
            }
        }
    }
}