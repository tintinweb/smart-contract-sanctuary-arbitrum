...
contract SamuraiPoker is Ownable, Pausable, ReentrancyGuard {
    ...
    function receiveRandomNumbers(
        ...
        if (winner == 0) {
            ...
            _depositToBankRoll(games[gameId].currency, houseEdgeAmount);
        } else if (winner == 2) {
            _transferOut(
                games[gameId].currency,
                prizeWithoutHouseEdge,
        ...
        _updateCurrencyStats(games[gameId].currency, games[gameId].bet * 2);
    ...
    function _updatePlayersStats(
        uint8 winner,
        address playerOne,
        address playerTwo
    ) internal {
        if (winner == 0) {
            playerStats[playerOne].draws += 1;
            playerStats[playerTwo].draws += 1;
        } else if (winner == 1) {
            playerStats[playerOne].wins += 1;
            playerStats[playerTwo].losses += 1;
        } else if (winner == 2) {
            playerStats[playerOne].losses += 1;
            playerStats[playerTwo].wins += 1;
        }
    }

    function _updateCurrencyStats(address currency, uint256 amount) internal {
        currencyStats[currency].games += 1;
