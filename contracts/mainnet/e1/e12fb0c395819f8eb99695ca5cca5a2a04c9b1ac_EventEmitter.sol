...
contract EventEmitter is IEventEmitter, UUPSUpgradeable, Initializable {
    address public owner;
    mapping(address => bool) public controllers;

    modifier onlyController() {
        require(controllers[msg.sender], "EventEmitter: caller is not a controller");
        _;
    }
    ...
    function grantController(address controller) external onlyOwner {
    ...
    /// @inheritdoc IEventEmitter
    function emitTransation(uint256 id, address customer, address merchant, uint256 paymentAmount)
        external
        override
        onlyController
    {
        emit Transaction(id, customer, merchant, paymentAmount);
    }

    /// @inheritdoc IEventEmitter
    function emitTransactionVerified(uint256 id, address verifier, uint256 paymentAmount)
        external
        override
        onlyController
    {
        emit TransactionVerified(id, verifier, paymentAmount);
    }
...
interface IEventEmitter {
     ...
     */
    event Transaction(uint256 indexed id, address indexed customer, address indexed merchant, uint256 paymentAmount);
     ...
     * @param paymentAmount The amount of the transaction
     */
    event TransactionVerified(uint256 indexed id, address indexed verifier, uint256 paymentAmount);
