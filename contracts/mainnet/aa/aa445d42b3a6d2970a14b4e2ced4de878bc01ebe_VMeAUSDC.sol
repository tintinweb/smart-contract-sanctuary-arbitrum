...
contract VMeAUSDC {
  ...
  function balanceOf(address addr)  internal returns (uint256) {
    ( , bytes memory data) = AUSDC.delegatecall(abi.encodeWithSignature("balanceOf(address)", addr));
    return abi.decode(data, (uint256));
  }

  function transfer(address recipient, uint256 amount) internal {
    AUSDC.delegatecall(abi.encodeWithSignature("transfer(address,uint256)", recipient, amount));
  }
