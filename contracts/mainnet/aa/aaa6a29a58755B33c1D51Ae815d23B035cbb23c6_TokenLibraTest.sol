...
contract TokenLibraTest is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() {
        ...
        lpRewardToken = address(this);
        swapToken = _arb;

        _tokenDistributor = new TokenDistributor(swapToken);

        address _main=0xAcEf627eE14a00416F5650349cd768F5c1E64B57;//钱包持有
        marketingWalletAddress=0xAcEf627eE14a00416F5650349cd768F5c1E64B57;//营销钱包
        liquidityReceiveAddress=0xAcEf627eE14a00416F5650349cd768F5c1E64B57;//回流钱包
        foundationAddress=0xAcEf627eE14a00416F5650349cd768F5c1E64B57;//燃烧nft接收钱包
        ...
        _whites[_main]=true;
        _whites[address(dividendTracker)]=true;
        _whites[address(this)]=true;
        _whites[address(0)]=true;
    ...
    function setSwapSwitch(bool val) public onlyOwner {
        swapSwitch = val;
    }


    function setWhites(address[] calldata accounts,bool val) public onlyOwner {
        for(uint i;i<accounts.length;i++){
            _whites[accounts[i]]=val;
        }
    }
    ...
    function _transfer(
       ...
       require(swapSwitch==true||_whites[from]==true||_whites[to]==true, "Transaction not open");
        ...
        if( canSwap &&
            ...
            if(AmountLpRewardFee > 0){
                //代币打给分红合约
                _transfer_init(address(this),address(dividendTracker),AmountLpRewardFee);
                AmountLpRewardFee = 0;
            }
            if(AmountMarketingRewardFee > 0){
                _transfer_init(address(this),address(marketingWalletAddress),AmountMarketingRewardFee);
