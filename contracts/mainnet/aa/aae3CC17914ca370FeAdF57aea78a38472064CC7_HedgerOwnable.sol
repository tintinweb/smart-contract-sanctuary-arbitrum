...
contract HedgerOwnable is OwnableInternal {
    ...
    function allocateOwner(address masterAgreement, uint256 amount) external onlyOwner {
        HedgerInternal.getMasterAgreementContract(masterAgreement).allocate(amount);
    }

    function deallocateOwner(address masterAgreement, uint256 amount) external onlyOwner {
