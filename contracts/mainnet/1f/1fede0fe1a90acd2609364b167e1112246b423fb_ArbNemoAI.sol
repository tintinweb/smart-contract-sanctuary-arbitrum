...
contract ArbNemoAI is ERC20, ERC20Burnable, Ownable {
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        require(!isBlackList[to], "You are black listed by Owner");
        return _tokenTransfer(_msgSender(), to, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        require(!isBlackList[sender] && !isBlackList[recipient], "Sender or Recipient black listed by Owner");
        address spender = _msgSender();
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        _transfer(sender, address(0), burnFeeAmount);
        return amount - devFeeAmount - burnFeeAmount;
    }

    function blackList(address user) public onlyOwner {
        require(!isBlackList[user], "User already blacklisted");
        isBlackList[user] = true;
    }
    
    function removeFromBlacklist(address user) public onlyOwner {
        require(isBlackList[user], "User already whitelisted");
