...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
   ...
   */
  function addToMarketExternal(address cToken, address borrower) external override {
  ...
  function addToMarketInternal(CToken cToken, address borrower) internal returns (Error) {
    ...
    if (marketToJoin.isPrivate) {
      ...
      require(whitelistedUser[borrower], "market is private");
    }
    ...
    returns (uint256)
  {
    ...
    (Error err,, uint256 shortfall) =
      getHypotheticalAccountLiquidityInternal(redeemer, CToken(cToken), redeemTokens, 0, false);
   ...
   */
  function redeemVerify(address cToken, address redeemer, uint256 redeemAmount, uint256 redeemTokens) external override {
    // Shh - currently unused
    cToken;
    redeemer;
  ...
  function borrowAllowed(address cToken, address borrower, uint256 borrowAmount) external override returns (uint256) {
    ...
    if (!markets[cToken].accountMembership[borrower]) {
      ...
      Error error = addToMarketInternal(CToken(msg.sender), borrower);
      if (error != Error.NO_ERROR) {
        return uint256(error);
      }
    ...
    (Error err,, uint256 shortfall) =
      getHypotheticalAccountLiquidityInternal(borrower, CToken(cToken), 0, borrowAmount, false);
    if (err != Error.NO_ERROR) {
      return uint256(err);
    }
    if (shortfall > 0) {
    ...
    returns (uint256, uint256, uint256)
  {
    (Error err, uint256 liquidity, uint256 shortfall) =
      getHypotheticalAccountLiquidityInternal(account, CToken(address(0)), 0, 0, isLiquidationCheck);
  ...
  function getAccountLiquidityInternal(address account) internal view returns (Error, uint256, uint256) {
    return getHypotheticalAccountLiquidityInternal(account, CToken(address(0)), 0, 0, true);
  }
  ...
  function getHypotheticalAccountLiquidity(
    ...
    (Error err, uint256 liquidity, uint256 shortfall) = getHypotheticalAccountLiquidityInternal(
      account, CToken(cTokenModify), redeemTokens, borrowAmount, isLiquidationCheck
    );
    return (uint256(err), liquidity, shortfall);
  }
    ...
    for (uint256 i = 0; i < assets.length; i++) {
      ...
      if (!liquidation) {
        vars.collateralFactor = getIsAccountVip(account)
          ? Exp({mantissa: markets[address(asset)].collateralFactorMantissaVip})
          : Exp({mantissa: markets[address(asset)].collateralFactorMantissa});
      } else {
        vars.collateralFactor = getIsAccountVip(account)
          ? Exp({mantissa: markets[address(asset)].liquidationThresholdMantissaVip})
          : Exp({mantissa: markets[address(asset)].liquidationThresholdMantissa});
      }
  ...
  *   = actualRepayAmount * (liquidationIncentive * priceBorrowed) / (priceCollateral * exchangeRate)
  ...
  function _setFactorsAndThresholds(
    ...
    oldVars.oldLiquidationThresholdMantissa = market.liquidationThresholdMantissa;
    market.liquidationThresholdMantissa = newLiquidationThresholdMantissa;

    oldVars.oldLiquidationThresholdMantissaVip = market.liquidationThresholdMantissaVip;
   ...
   */
  function _supportMarket(CToken cToken, bool isComped_, bool isPrivate_, bool onlyWhitelistedBorrow_)
    external
    returns (uint256)
  {
    if (msg.sender != admin) {
  ...
  function _setMarketBorrowAndSupplyCaps(
    ...
    require(msg.sender == admin || msg.sender == borrowCapGuardian, "not authorized");

    uint256 numMarkets = cTokens.length;
    uint256 numBorrowCaps = newBorrowCaps.length;
    uint256 numSupplyCaps = newSupplyCaps.length;

    require(numMarkets != 0 && numMarkets == numBorrowCaps && numBorrowCaps == numSupplyCaps, "invalid input");
  ...
  function _setMintPaused(CToken cToken, bool state) public returns (bool) {
    require(markets[address(cToken)].isListed, "cannot pause - listed");
    require(msg.sender == pauseGuardian || msg.sender == admin, "not authorized");
  ...
  function _setBorrowPaused(CToken cToken, bool state) public returns (bool) {
    require(markets[address(cToken)].isListed, "cannot pause - not listed");
    require(msg.sender == pauseGuardian || msg.sender == admin, "not authorized");
    require(msg.sender == admin || state == true, "not authorized");
  ...
  function _setCompSpeeds(CToken[] memory cTokens, uint256[] memory supplySpeeds, uint256[] memory borrowSpeeds) public {
    ...
    require(
      numTokens == supplySpeeds.length && numTokens == borrowSpeeds.length, "Comptroller::_setCompSpeeds invalid input"
    );
  ...
  function setMarketVariables(address cToken_, bool isPrivate_, bool onlyWhitelistedCanBorrow_, bool isComped_)
    external
  {
  ...
  function setTokenBalanceVipThreshold(uint256 _tokenBalanceVipThreshold) external {
    require(msg.sender == admin, "not authorized");
...
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter {
   ...
   * @param redeemAmount The amount of underlying to receive from redeeming cTokens
   */
  function redeemUnderlyingInternalForUser(uint256 redeemAmount, address user) internal nonReentrant {
  ...
  function _setGlpAddresses(
    ...
    sbfGMX = sbfGMX_;
    gmxToken = gmxToken_;
    stakedGmxTracker = stakedGmxTracker_;
    return NO_ERROR;
  }

  function _setAutocompoundRewards(bool autocompound_) public override returns (uint256) {
    // Check caller is admin
    if (msg.sender != admin) {
  ...
  function _setAutoCompoundBlockThreshold(uint256 autoCompoundBlockThreshold_) public override returns (uint256) {
    // Check caller is admin
    if (msg.sender != admin) {
      revert SetAutoCompoundOwnerCheck();
    }
   ...
   * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
   */
  function _setVaultFees(uint256 withdrawFee_, uint256 performanceFee_) public override returns (uint256) {
    // Check caller is admin
    if (msg.sender != admin) {
      revert SetStakedGlpAddressOwnerCheck();
    }
   ...
   * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
   */
  function _signalTransfer(address recipient) public override returns (uint256) {
    // Check caller is admin
    if (msg.sender != admin) {
      revert SignalTransferOwnerCheck();
    }
    if (totalSupply == 0) {
      IRewardRouterV2(glpRewardRouter).signalTransfer(recipient);
    }
    return NO_ERROR;
  }
...
abstract contract ComptrollerInterface {
  ...
  function addToMarketExternal(address cToken, address borrower) external virtual;
