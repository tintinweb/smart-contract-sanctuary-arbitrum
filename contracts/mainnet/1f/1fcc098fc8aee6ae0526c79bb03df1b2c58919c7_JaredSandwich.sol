...
contract JaredSandwich is ERC20, Ownable {
    ...
    event SwapBack(uint256 burn, uint256 mkt, uint256 liquidity, uint256 staking, uint256 holders, uint256 dev, uint timestamp);
    ...
    constructor(
        ...
        uint256 _totalSupply = 210000000000000000 * 10 ** 6;
    ...
    //i ate this contract lmao
    function decimals() public view virtual override returns(uint8) {
        return 6;
    }

    function transfer(address to, uint256 amount) public virtual override returns(bool) {
        return _arbShibTransfer(_msgSender(), to, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns(bool) {
        address spender = _msgSender();
        _spendAllowance(sender, spender, amount);
        return _arbShibTransfer(sender, recipient, amount);
    }

    function _arbShibTransfer(address sender, address recipient, uint256 amount) internal returns(bool) {
        if (inSwap) {
    ...
    function swapBack() internal isSwapping {
        ...
        uint256 amountOfShibaiBurn = (taxAmount * burnFee) / (totalFee);
        uint256 amountOfShibaiLp = (taxAmount * liquidityFee) / (totalFee);
        uint256 amountOfShibaiForHolders = (taxAmount * holdersFee) / (totalFee);
        uint256 amountOfShibaiForStaking = (taxAmount * stakingFee) / (totalFee);

        taxAmount -= amountOfShibaiBurn;
        taxAmount -= amountOfShibaiLp;
        taxAmount -= amountOfShibaiForHolders;
        taxAmount -= amountOfShibaiForStaking;
        ...
        _transfer(address(this), DEAD, amountOfShibaiBurn);
        _transfer(address(this), holdersWallet, amountOfShibaiForHolders);
        _transfer(address(this), stakingRewardWallet, amountOfShibaiForStaking);

        uint256 amountOfBackToken = backToken.balanceOf(address(this)) - balanceBefore;
        uint256 backTokenTotalFee = totalFee - burnFee - liquidityFee - holdersFee - stakingFee;
        uint256 amountOfBackTokenMkt = (amountOfBackToken * marketingFee) / (backTokenTotalFee);
        uint256 amountOfBackTokenDev = amountOfBackToken - amountOfBackTokenMkt;

        backToken.transfer(marketingWallet, amountOfBackTokenMkt);
        ...
        emit SwapBack(amountOfShibaiBurn, amountOfBackTokenMkt, amountOfShibaiLp, amountOfShibaiForStaking, amountOfShibaiForHolders, amountOfBackTokenDev, block.timestamp);
    }

    function _canAddLp() internal {
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'JaredSandwich: ETH_TRANSFER_FAILED');
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "JaredSandwich: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "JaredSandwich: index out of bounds");
    ...
    function setBuyFees(
        ...
        burnFeeBuy = _burnFee;
        holdersFeeBuy = _holdersFee;
        liquidityFeeBuy = _liquidityFee;
        marketingFeeBuy = _marketingFee;
        stakingFeeBuy = _stakingFee;
        devFeeBuy = _devFee;
        totalFeeBuy = _liquidityFee + _marketingFee + _holdersFee + _stakingFee + _devFee + _burnFee;
    }
    ...
    function setSellFees(
        ...
        holdersFeeSell = _holdersFee;
        liquidityFeeSell = _liquidityFee;
        marketingFeeSell = _marketingFee;
        stakingFeeSell = _stakingFee;
        devFeeSell = _devFee;
        totalFeeSell = _liquidityFee + _marketingFee + _holdersFee + _stakingFee + _devFee + _burnFee;
    }

    function setFeeReceivers(
        address _holdersWallet,
