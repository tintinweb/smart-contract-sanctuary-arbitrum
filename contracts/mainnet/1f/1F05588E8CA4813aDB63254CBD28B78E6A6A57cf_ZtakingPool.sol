...
contract ZtakingPool is IZtakingPool, Ownable2Step, Pausable, EIP712, Nonces {
    ...
    // List of tokens that can be staked
    ...
    // (referralCode => codeCreator)
    mapping(string => address) public codeCreators;

    // (depositor => referringDepositor)
    mapping(address => address) public depositorReferee;
    ...
    constructor(address[] memory _tokensAllowed, address _weth) Ownable(msg.sender) EIP712("ZtakingPool", "1"){
     ...
     */
    function depositFor(address _token, address _for, uint256 _amount, string memory _referralCode) whenNotPaused external {
        if (_amount == 0) revert DepositAmountCannotBeZero();
        if (_for== address(0)) revert CannotDepositForZeroAddress();
        if (_findIndex(tokens, _token)==tokens.length) revert TokenNotAllowedForStaking();
        ...
        _useReferralCode(_referralCode, _for);
        _generateReferralCode(_for);
    }

    function depositETHFor(address _for, string memory _referralCode) whenNotPaused payable external {
        if (msg.value == 0) revert DepositAmountCannotBeZero();
        if (_for== address(0)) revert CannotDepositForZeroAddress();
        if (_findIndex(tokens, WETH_ADDRESS)==tokens.length) revert TokenNotAllowedForStaking();
    ...
    function migrate() external whenNotPaused {
        ...
        IMigrator(migrator).migrate(msg.sender);
        emit Migrate (++eventId, msg.sender);
    }
    ...
    function setMigrator(address _migrator) external onlyOwner {
        for (uint i = 0; i<tokens.length; i++) {
            IERC20(tokens[i]).safeIncreaseAllowance(_migrator, 2**256-1);

            // Revoke old approvals for token
            if (migrator!=address(0)) {
                IERC20(tokens[i]).safeDecreaseAllowance(migrator, IERC20(tokens[i]).allowance(address(this), migrator));
            }
        }
        migrator = _migrator;
    ...
    function setStakable(address _token, bool _canStake) external onlyOwner {
        ...
        for (uint i = 0; i<tokens.length; i++) {
            if (tokens[i]==_token  && _canStake) revert TokenAlreadyConfiguredWithState();
        }
        if (_canStake) {
            tokens.push(_token);
        } else {
            uint index = _findIndex(tokens, _token);
    ...
    /**
     * @notice If referral code is provided, emit an event and set referee for new depositor
     */
    function _useReferralCode(string memory referralCode, address _for) internal {
        if (bytes(referralCode).length!=0 && depositorReferee[_for]==address(0)) {
            address referee = codeCreators[referralCode];
            if (referee==address(0)) revert InvalidReferral();
            if (referee==msg.sender) revert CannotReferSelf();
            depositorReferee[_for] = referee;
            emit ReferralCodeUsed(_for, referee);
        }
    }
     ...
     */
    function _generateReferralCode(address user) internal {
        if (bytes(referralCodes[user]).length != 0) return;

        // Create a hash of the user's address
        uint attempt;
        bytes32 addressHash = keccak256(abi.encodePacked(user, attempt));
        string memory code = Strings.toHexString(uint256(addressHash), 32);
        code = _substring(code, 2, 10);

        // Check for uniqueness and adjust if necessary
        while (codeCreators[code] != address(0)) {
            attempt+=1;
            bytes32 addressHash = keccak256(abi.encodePacked(user, attempt));
            string memory code = Strings.toHexString(uint256(addressHash), 32);
            code = _substring(code, 2, 10);
    ...
    /**
     * @notice Helper function to extract a substring
     */
    ...
    /**
     * @notice Find the index of an address in an array
     * @dev Returns the length of the array if address couldn't be found
     */
    function _findIndex(address[] storage array, address find) internal returns (uint) {
...
interface IZtakingPool {
    ...
    error MigratorNotSet(); //Thrown when the migrator is zero address
    error InvalidReferral(); //Thrown when providing a referral code that hasn't been generated by the staking contract
    error CannotReferSelf(); //Thrown when a user tries to use their own referral code when depositing
    ...
    ///@notice Emitted when a depositor deposits using another depositors referral code
    ///@param depositor New depositor
    ///@param referee The old depositor whose referral code was used for new deposit
    event ReferralCodeUsed(
        address indexed depositor,
        address indexed referee
    );

    ///@notice Emitted when a new user deposits, resulting in the creation of a new referral code
    ...
    ///@notice Emitted when migrator address is changed
    ///@param migrator The address of the contract that handles migration logic
    ...
    ///@param _referralCode Referral code to point out which address referred the current depositng address
    function depositFor(address _token, address _for, uint256 _amount, string memory _referralCode) external;

    ///@notice Stake a specified amount of ether into the Ztaking Pool
    ///@param _for The user to deposit/stake on behalf of
    ///@dev the amount deposited is specified by msg.value
    ///@param _referralCode Referral code to point out which address referred the current depositng address
    function depositETHFor(address _for, string memory _referralCode) payable external;
    ...
    ///@notice Set/Change the address to handle migrations
