    ...
    */
    function executeClosePosition(
        ...
        if (!cpr.isLiquidation) {
            require(_lendingRepaymentAmount <= _userInfo.position, "Too much repayment");
            if (_lendingRepaymentAmount == _userInfo.position) {
                cpr.lendingRepayment = _lendingRepaymentAmount;
                cpr.toLeverageUser = 0;
            } else {
            ...
            kcd.interestsAmount = closeRequestInterestsBorrowedAsset[cpr.user][cpr.closeRequestID];
            console.log("interestsAmount",kcd.interestsAmount);
