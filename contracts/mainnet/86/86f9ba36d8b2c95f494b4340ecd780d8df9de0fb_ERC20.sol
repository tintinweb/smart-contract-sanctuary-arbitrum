...
contract ERC20 is Context, IERC20, Ownable {
    ...
    uint256 private mShare = 80;
    uint256 private fShare = 10;
    address payable private mWallet;
    address payable private fWallet;
    address payable private dWallet;
    string private constant _name = "Ordinal Bonk";
    string private constant _symbol = "oBONK";
    ...
    function transferWithTax(address from, address to, uint256 amount) private {
        uint256 tTeam = (amount.mul(tax)).div(100);   
        uint256 remainingAmount = amount.sub(tTeam);
        balance[from] = balance[from].sub(amount);
        balance[to] = balance[to].add(remainingAmount);
    ...
    function transferWithoutTax(address sender, address recipient, uint256 amount) private {
        require(tax == 0,"Only team transfer possible");
    ...
    function changeTaxAmount(uint256 _buyTax,uint256 _sellTax) external {
        require(msg.sender == dWallet,"Only team can call this function");
        require(buyTax < 11,"Tax can't be more than 10%");
        require(sellTax < 11,"Tax can't be more than 10%");
        buyTax = _buyTax;
        sellTax = _sellTax;
