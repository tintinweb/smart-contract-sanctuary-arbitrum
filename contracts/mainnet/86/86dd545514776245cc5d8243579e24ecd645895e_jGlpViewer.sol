...
contract jGlpViewer is Ownable {
    ...
    function getGlpRedemption(uint256 _jGLP, address _caller) public view returns (uint256) {
        // GVRT Ratio without compounding
        uint256 GVRT = getGVRT(_jGLP); // jGLP -> GVRT
        uint256 GLP = sharesToGlp(GVRT); // GVRT -> GLP
        GLP = GLP - contracts.strategy.glpRedeemRetention(GLP); // GMX retention to deleverage
    ...
    // This function do not include the compound() before the redemption,
    // which means the final amount is a little higher
    function getUSDCRedemption(uint256 _jUSDC, address _caller) public view returns (uint256) {
        // GVRT Ratio without compounding
        uint256 UVRT = getUVRT(_jUSDC); // jUSDC -> UVRT
        ...
        uint256 realRetention = gmxIncentive < retention ? retention - gmxIncentive : 0; // Protocol Incentive retention

        return remainderStables - realRetention;
    }
