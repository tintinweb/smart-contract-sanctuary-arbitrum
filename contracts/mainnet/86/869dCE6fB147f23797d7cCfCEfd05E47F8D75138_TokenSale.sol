...
/// @title DecentraBet token sale contract
/// @notice ETH (WETH) and USDC can be deposited in this contract to get DecentraBet tokens once the sale ends. The rate depends on the dollar value of the tokens raised
contract TokenSale is Ownable, ReentrancyGuard {
    ...
    /// @notice Target raise amount in USD, if amount totalRaisedUSD is inferior then not all available tokens will be given out
    /// @dev Set by the `allocateTokensForSale` function
    uint256 public targetRaiseUSD;
    /// @notice Target raise amount in USD, if amount totalRaisedUSD is inferior then not all available tokens will be given out
    /// @dev Set by the `allocateTokensForSale` function
    uint256 public tokenPriceUSD;
    /// @notice If owner has withdrawn tokens leftover from sale
    /// @dev Set by the `withdrawUnsoldTokens` function
    bool public hasWithdrawn;

    /// @notice ETH price given by chainlink oracle at beginning of sale. Used to estimate the total value raised using a fixed ETH price
    /// @dev Set by the `setSaleSchedule` function
    uint256 public ETHPriceUSD;
    /// @notice USDC price given by chainlink oracle at beginning of sale. Used to estimate the total value raised using a fixed ETH price
    /// @dev Set by the `setSaleSchedule` function
    uint256 public constant USDC_PRICE_USD = 100_000_000;
    /// @notice Estimate raised amount in USD
    /// @dev Updated at every deposit call
    uint256 public estimateTotalRaised;
    /// @notice Maximum raise amount in USD, if amount estimateTotalRaised is superior to this no more tokens will be sold
    uint256 public constant MAX_RAISE_USD = 10_000_000 * 1e8;
    ...
    function allocateTokensForSale(
        uint256 _amount,
        ...
        availableTokens = IERC20(saleToken).balanceOf(address(this));
        targetRaiseUSD = (availableTokens * _tokenPriceUSD) / 10 ** 18;
    ...
    function setSaleSchedule(uint256 _start, uint256 _end) external onlyOwner {
        ...
        ETHPriceUSD = IAggregatorV3Interface(supportedTokensData[WETH].oracle)
            .latestAnswer()
            .toUint256();
        require(ETHPriceUSD > 0, "INVALID_ORACLE_ANSWER");
    }
    ...
    /// @notice Function used to transfer unsold tokens funds to the treasury. Can only be called after withdrawals have been enabled
    function withdrawUnsoldTokens() external onlyOwner {
        require(withdrawalsEnabled, "WITHDRAWALS_NOT_ENABLED");
        if (totalRaisedUSD >= targetRaiseUSD || hasWithdrawn) {
            revert();
        }
        hasWithdrawn = true;
        uint256 balance = (((targetRaiseUSD - totalRaisedUSD) * 10 ** 18) /
            tokenPriceUSD);
    ...
    function getUserClaimableTokens(
        ...
        if (totalRaise >= targetRaiseUSD) {
            ...
            return
                (((depositedValueWithPrecision * 10 ** 18) / tokenPriceUSD)) /
                10 ** ERC20(account.token).decimals();
        }
    }
    ...
    function _depositFor(
        ...
        if (_token == WETH) {
            ...
            estimateTotalRaised +=
                (_amount * USDC_PRICE_USD) /
                10 ** ERC20(_token).decimals();
        }

        require(estimateTotalRaised <= MAX_RAISE_USD, "MAX_AMOUNT_RAISED");
