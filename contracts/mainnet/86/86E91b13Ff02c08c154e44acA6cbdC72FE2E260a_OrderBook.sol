...
contract OrderBook is Storage, ReentrancyGuardUpgradeable, Admin, Getter {
    ...
    function cancelOrder(uint64 orderId) external nonReentrant {
        ...
        require(orderData.placeOrderTime + coolDown <= _blockTimestamp(), "CLD"); // CooLDown
     ...
     * @dev Broker can withdraw brokerGasRebate.
     */
    function claimBrokerGasRebate(uint96 nativeTokenPrice) external onlyRole(BROKER_ROLE) returns (uint256 rawAmount) {
        emit ReportBrokerGasRebatePrice(nativeTokenPrice);
        return IDegenPool(_storage.pool).claimBrokerGasRebate(_msgSender());
    }
