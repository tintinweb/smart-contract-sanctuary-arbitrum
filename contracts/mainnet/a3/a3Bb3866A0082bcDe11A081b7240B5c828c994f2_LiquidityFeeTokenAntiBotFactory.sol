...
import { ILiquidityFeeTokenAntiBot } from "../interfaces/ILiquidityFeeTokenAntiBot.sol";

contract LiquidityFeeTokenAntiBotFactory is TokenFactoryBase {
    ...
    function create(
        ...
        token = Clones.cloneDeterministic(implementation, keccak256(abi.encodePacked(msg.sender, name, symbol, totalSupply, feeSettings, enableAntiBot)));
        ILiquidityFeeTokenAntiBot(token).initialize(
            msg.sender,
            name,
            symbol,
            totalSupply,
        ...
        assignTokenToOwner(msg.sender, token, 8);
        emit TokenCreated(msg.sender, token, 8, implementationVersion);
    }
}
 ...
 * 🚀 Unleash the power of decentralized finances and tokenization with DEXTools Token Creator. Customize your token seamlessly. Manage your created tokens conveniently from your user panel - start creating your dream token today!
 */
interface ILiquidityFeeTokenAntiBot {
    function initialize(
        address owner_,
        string memory name_,
