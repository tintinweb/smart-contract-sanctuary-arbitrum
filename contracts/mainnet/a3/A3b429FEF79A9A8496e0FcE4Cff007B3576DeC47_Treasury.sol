...
contract Treasury is Operator, ReentrancyGuard {
    ...
    function open(uint256 _openAmount, uint256 _swapMaxIn, uint256 _repayAmount)external onlyGovernance {
        if (_openAmount > 0){
            require(IERC20(usdt).balanceOf(riskOnPool) >= _repayAmount, "insufficient usdt balance");
            ...
            if (totalDebtBase + _openAmount * wstETHPrice / 1e28 > _repayAmount * 1e2){
                require((totalCollateralBase + _openAmount * wstETHPrice / 1e28) *  currentLiquidationThreshold * 1e18 > 
                    (totalDebtBase + _openAmount * wstETHPrice / 1e28 - _repayAmount * 1e2) * 1e4 * IRiskVault(riskOnPool).minHealthFactor(), "insufficient");
            }
            IRiskVault(riskOnPool).open(_openAmount, _swapMaxIn, _repayAmount);
        }else if (_repayAmount > 0){
            if (IERC20(IAavePoolV3(aaveV3).getReserveData(usdt).variableDebtTokenAddress).balanceOf(riskOnPool) > 0){
                IRiskVault(riskOnPool).repayAave(usdt, _repayAmount);
            }
        }
    ...
    function close(uint256 _closeAmount, uint256 _swapMinOut, uint256 _withdrawAmount)external onlyGovernance {
        if (_closeAmount > 0){
            ...
            if (totalDebtBase  + _withdrawAmount * 1e2 > _closeAmount * wstETHPrice / 1e28){
                require((totalCollateralBase - _closeAmount * wstETHPrice / 1e28) *  currentLiquidationThreshold * 1e18 > 
                        (totalDebtBase  + _withdrawAmount * 1e2 - _closeAmount * wstETHPrice / 1e28) * 1e4 * IRiskVault(riskOnPool).minHealthFactor(), "insufficient");
            }
            IRiskVault(riskOnPool).close(_closeAmount, _swapMinOut, _withdrawAmount);
        }else{
            IRiskVault(riskOnPool).borrowAave(usdt, _withdrawAmount);
        }
