...
interface NFTId {
    ...
    function setOriginal(uint256 _original) external;
}

interface TokenSwapInfo {
    function setLP(address _addr) external;

    function changeSwap() external;
    ...
    function getAmountPrice() external returns (uint256);

    function checkOpenSwap() external view returns (bool);

    function checkSwapLimt(uint256 newPrice) external view returns (uint256);

    function calculateFluctuation(uint256 newPrice) external;

    function toSwap(uint256 tokenIn, bool isAir) external;

    function toAddLiquidity() external;
}
...
contract Token404 is ERC404, Pausable, ReentrancyGuard {
    ...
    constructor(address _owner)
        ERC404("OKK Walle LIU", "OKK Walle LIU", 18, 10000000000, _owner)
    {
    ...
    function _transfer(
        ...
        swapInfo.changeSwap();
        (liquidityAmount, airdropAmount, burnAmount) = swapInfo.geetFeeAmount(
            amount
        );
        ...
        if (from == uniswapV2Pair || to == uniswapV2Pair) {
            ...
            require(
                amount <= swapInfo.checkSwapLimt(newPrice),
                "exceed tx limax"
            );
    ...
    function getAddressNFTAmount(address user)
        public
        view
        returns (
            uint256,
    ...
    {
        uint256[] memory ids = _owned[user];
        uint256 types1;
        uint256 types2;
        uint256 types3;
        uint256 types4;
        ...
        return (types1, types2, types3, types4, types5, types6, types7);
    }
