...
interface ISettingsManager {
    ...
    function isWhitelistedFromWithdrawCooldown(address _addr) external view returns (bool);
...
import {Position, OrderStatus, OrderType, WithdrawalRequest} from "./structs.sol";
...
contract Vault is Constants, Initializable, ReentrancyGuardUpgradeable, IVault {
    ...
    mapping(uint256 => WithdrawalRequest) public withdrawalRequests; // withdrawalRequestId => WithdrawalRequest

    event Deposit(address indexed account, address indexed token, uint256 amount);
    event Withdraw(uint256 withdrawalRequestId, address indexed account, address indexed token, uint256 amount);
    ...
    event SupplyVusdToVault(address indexed account, uint256 vusdAmount);
    event Seize(address indexed account, uint256 vusdAmount);
    ...
    event CreateWithdrawalRequest(
        uint256 indexed withdrawalRequestId,
        address indexed account,
        address token,
        uint256 vusdAmount
    );
    event CancelWithdrawalRequest(
        uint256 indexed withdrawalRequestId,
        address indexed account,
        address token,
        uint256 vusdAmount
    ...
    modifier preventBanners() {
        _preventBanners();
        _;
    }

    function _preventBanners() private view {
        require(!settingsManager.checkBanList(msg.sender), "Account banned");
    }
    ...
    function deposit(address _account, address _token, uint256 _amount) public override nonReentrant preventBanners {
    ...
    // create withdrawal request and burn vusd
    function withdraw(address _token, uint256 _amount) external payable nonReentrant preventBanners {
        require(settingsManager.isWithdraw(_token), "withdraw not allowed");
        require(_amount > 0, "zero amount");

        require(msg.value == settingsManager.marketOrderGasFee(), "invalid gasFee");
        _sendGasFee();

        IVUSD(vusd).burn(msg.sender, _amount);

        uint256 withdrawalRequestId = nextWithdrawalRequestId;
        nextWithdrawalRequestId = withdrawalRequestId + 1;
    ...
    // execute withdrawal request to withdraw stablecoin
    function executeWithdrawal(uint256 _withdrawalRequestId) external nonReentrant {
        WithdrawalRequest memory req = withdrawalRequests[_withdrawalRequestId];
        require(req.isOpen, "invalid request");
        require(settingsManager.isWithdraw(req.token), "withdraw not allowed");
        require(!settingsManager.checkBanList(req.account), "Account banned");
        require(
            block.timestamp >= req.timestamp + settingsManager.withdrawalExecutionDelay() ||
            settingsManager.isWhitelistedFromWithdrawCooldown(req.account),
            "withdrawal delay not yet passed"
        );

        withdrawalRequests[_withdrawalRequestId].isOpen = false;

        uint256 withdrawFee = (req.vusdAmount * settingsManager.withdrawFee(req.token)) / BASIS_POINTS_DIVISOR;
        _distributeFee(withdrawFee, address(0), address(0));

        uint256 tokenAmount = priceManager.usdToToken(req.token, req.vusdAmount - withdrawFee);
        IERC20Upgradeable(req.token).safeTransfer(req.account, tokenAmount);

        emit Withdraw(_withdrawalRequestId, req.account, req.token, tokenAmount);
    }

    // cancel withdrawal request
    function cancelWithdrawal(uint256 _withdrawalRequestId) external nonReentrant {
        WithdrawalRequest memory req = withdrawalRequests[_withdrawalRequestId];
        require(req.isOpen, "invalid request");
        require(req.account == msg.sender, "not owner");

        withdrawalRequests[_withdrawalRequestId].isOpen = false;

        IVUSD(vusd).mint(req.account, req.vusdAmount);

        emit CancelWithdrawalRequest({
            withdrawalRequestId: _withdrawalRequestId,
            account: req.account,
            token: req.token,
            vusdAmount: req.vusdAmount
        });
    }

    // stake stablecoin to mint vlp
    function stake(address _account, address _token, uint256 _amount) public override nonReentrant preventBanners {
    ...
    function unstake(address _tokenOut, uint256 _vlpAmount) public nonReentrant preventBanners {
    ...
    function newPositionOrder(
        ...
        address _refer
    ) public payable nonReentrant preventBanners {
    ...
    function newPositionOrderWithTPSL(
        ...
        uint256[] memory _prices,
        uint256[] memory _amountPercents
    ) external payable nonReentrant preventBanners {
        if (_orderType == OrderType.MARKET) {
            require(
    ...
    // submit trigger order to increase size of exisiting position
    function createAddPositionTriggerOrder(
        uint256 _posId,
        uint256 _collateralDelta,
        uint256 _sizeDelta,
        uint256 _triggerPrice,
        ...
        positionVault.createAddPositionTrigger(
            msg.sender,
            isTriggerAbove,
            _posId,
            _triggerPrice,
    ...
    function addCollateral(uint256 _posId, uint256 _amount) public nonReentrant preventBanners {
        positionVault.addOrRemoveCollateral(msg.sender, _posId, true, _amount);
    }

    // remove collateral to increase leverage
    function removeCollateral(uint256 _posId, uint256 _amount) public payable nonReentrant preventBanners {
        require(msg.value == settingsManager.marketOrderGasFee(), "invalid triggerGasFee");
        _sendGasFee();
    ...
    function addTrailingStop(uint256 _posId, uint256[] memory _params) external payable nonReentrant preventBanners {
        require(msg.value == settingsManager.triggerGasFee(), "invalid triggerGasFee");
        _sendGasFee();
    ...
    function cancelPendingOrder(uint256 _posId) public nonReentrant preventBanners {
        orderVault.cancelPendingOrder(msg.sender, _posId);
    }

    // cancel multiple pending newPositionOrder / trailingStopOrder
    function cancelPendingOrders(uint256[] memory _posIds) external preventBanners {
    ...
    function supplyVusdToVault(uint256 _vusdAmount) external nonReentrant onlyOperator(3) {
        IVUSD(vusd).burn(msg.sender, _vusdAmount);
        totalUSD += _vusdAmount;

        emit SupplyVusdToVault(msg.sender, _vusdAmount);
    }

    function seize(address _exploiter) external nonReentrant onlyOperator(3) {
        require(settingsManager.checkBanList(_exploiter), "not exploiter");

        uint256 vusdBalance = IVUSD(vusd).balanceOf(_exploiter);
        IVUSD(vusd).burn(_exploiter, vusdBalance);
        totalUSD += vusdBalance;

        emit Seize(_exploiter, vusdBalance);
    }
