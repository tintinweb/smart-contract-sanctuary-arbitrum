...
contract AlbertEuro is ERC20, Ownable {
    ...
    event UpdateFees(uint256 DEVFeeOnBuy, uint256 DEVFeeOnSell, uint256 DEVFeeOnTransfer);
    ...
    constructor () ERC20("AlbertEuro", unicode"ALBERT") 
    {   
        ...
        DEVFeeOnSell = 3;
        DEVFeeOnTransfer = 8;
        DEVWallet = 0xc1fDCf656EBE628b7606E1389B79F0376635Ed3C;
        ...
        _isExcludedFromLimits[address(0x0)] = true;
    ...
    function _updateUniswapV2Router(address newAddress) external {
    ...
    function SetFees(uint256 _DEVFeeOnBuy, uint256 _DEVFeeOnSell, uint256 _DEVFeeOnTransfer) external onlyOwner {
        DEVFeeOnBuy = _DEVFeeOnBuy;
        DEVFeeOnSell = _DEVFeeOnSell;
        DEVFeeOnTransfer = _DEVFeeOnTransfer;
        emit UpdateFees(DEVFeeOnBuy, DEVFeeOnSell, DEVFeeOnTransfer);
    }
    ...
    function isExcludedFromFromLimits(address account) private view returns(bool) {
        return _isExcludedFromFees[account];
    }
    function setMaxTransferAmount(uint256 _maxTransferAmount) external onlyOwner {
        require(maxTransferAmount >= totalSupply() / 400, "maxTransferAmount can't be lower than 0.25% of the supply");
        maxTransferAmount = _maxTransferAmount;
    }
    function changeMaxHold(uint256 amount) public onlyOwner {
        require (amount >= totalSupply() / 200, "max Wallet amount must be greator than 0.5% of the supply");
        maxHold = amount;
    }
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (!_isExcludedFromLimits[from] || !_isExcludedFromLimits[to]) {
            require(amount <= maxTransferAmount , "Anti-whale: Transfer amount exceeds max limit");
            require(balanceOf(to) + amount <= maxHold, "Anti-whale: Wallet exceeds maxWallet limit");
        }      
        if (amount == 0) {
        ...
        if (canSwap &&
            ...
            swapping = true;
            swapAndSendDEV(contractTokenBalance);     
            swapping = false;
        }
        uint256 _totalFees;
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || swapping) {
            ...
            _totalFees = DEVFeeOnBuy;
        } else if (to == uniswapV2Pair) {
            _totalFees =  DEVFeeOnSell;
        } else {
            _totalFees = DEVFeeOnTransfer;
        }
        if (_totalFees > 0) {
