...
contract ConnextModule is Module, IXReceiver {
  ...
  event ModuleSetUp(
    address owner, address avatar, address target, address originSender, uint32 origin, address connext
  );
  ...
  function setUp(bytes memory initializeParams) public override initializer {
    __Ownable_init();
    (address _owner, address _avatar, address _target, address _originSender, uint32 _origin, address _connext) =
  ...
  function xReceive(
    ...
    // Decode message
    (address _to, uint256 _value, bytes memory _data, Enum.Operation _operation) =
      abi.decode(_callData, (address, uint256, bytes, Enum.Operation));
