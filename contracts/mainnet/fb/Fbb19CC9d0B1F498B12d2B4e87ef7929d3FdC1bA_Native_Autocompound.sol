...
contract Native_Autocompound is Ownable, ReentrancyGuard, Pausable {
    ...
    /*function totalUserBalanceWithoutPending(address _user) public view returns (uint256) {
        (uint256 stakedAmount, ) = INativeFarm(nativeFarm).userInfo(pid, address(this));
        uint256 totalBal = balanceOf().add(stakedAmount);
        return totalBal.mul(userInfo[_user].shares).div(totalShares);
    }*/
