    ...
    PausableUpgradeable
{
    ...
    mapping(address => bool) public tryGMXSwap;
    ...
    event AssetFeeChanged(address asset, uint24 fee);
    ...
    function setAssetFee(address _asset, uint24 _fee, bool _tryGMX) external onlyOwner {
        require(_asset != address(0), "Zero address");
        require(_fee <= 5000, "Fee too high");
        assetFee[_asset] = _fee;
        tryGMXSwap[_asset] = _tryGMX;
        emit AssetFeeChanged(_asset, _fee);
    }

    function setGMXRouter(address _GMXRouter) external onlyOwner {
        require(_GMXRouter != address(0), "Zero address");
        GMXRouter = _GMXRouter;
        emit RouterChanged(_GMXRouter);
    }
    ...
    function afterWithdrawalExecution(bytes32 key, Withdrawal.Props memory withdrawal, EventUtils.EventLogData memory eventData) external {
        ...
        if (wr.longToken != WETH) {
            if (tryGMXSwap[wr.longToken]) {
                address[] memory path = new address[](2);
                path[0] = wr.longToken;
                path[1] = strategyAddresses.USDC;

                IERC20Upgradeable(wr.longToken).approve(address(GMXRouter), longAmountFromGMX);
                uint256 balBefore = IERC20Upgradeable(strategyAddresses.USDC).balanceOf(address(this));
                try IGMXRouter(GMXRouter).swap(path, longAmountFromGMX, 0, msg.sender) {
                    uint256 balAfter = IERC20Upgradeable(strategyAddresses.USDC).balanceOf(address(this));
                    amountOut = balAfter - balBefore;
                } catch {
                    amountOut = _uniswapV3Swap(longAmountFromGMX, wr.longToken, strategyAddresses.USDC);
                }
            } else {
                amountOut = _uniswapV3Swap(longAmountFromGMX, wr.longToken, strategyAddresses.USDC);
            }
    ...
    function _uniswapV3Swap(
        ...
        ISwapRouter.ExactInputParams memory params = ISwapRouter.ExactInputParams({
            path: abi.encodePacked(_assetFrom, assetFee[_assetFrom], WETH, assetFee[WETH], _assetTo),
            recipient: address(this),
            deadline: block.timestamp,
            amountIn: _amount,
            amountOutMinimum: 0
        });
        uint256 out = ISwapRouter(strategyAddresses.univ3Router).exactInput(params);
        return out;
    }
