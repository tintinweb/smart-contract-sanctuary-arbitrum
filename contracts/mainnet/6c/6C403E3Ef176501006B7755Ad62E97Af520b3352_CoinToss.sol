...
interface IBank {
    ...
    function getBetRequirements(
        address token,
        uint256 multiplier
    ) external view returns (bool, uint64, uint256, uint256);
}
...
// import {Game} from "./oldBank/Game.sol";
...
contract CoinToss is Game, IGameImplementation {
    ...
    /// @param wrapPayout Whether the payout has been wrapped 
    event Roll(
        uint256 indexed id,
        address indexed user,
        address indexed token,
    ...
    function _decodeBytesToBool(bytes calldata data) private pure returns (bool face)
    {
    ...
    function _wager(
        ...
        Bet memory bet = _newBet(token, user, tokenAmount, _getPayout(10000), wrapPayout);
    ...
    function wager(
        ...
        _wager(face,msg.sender,token,tokenAmount,wrapPayout);
    }
    ...
    function wagerWithData(
        ...
        _wager(_decodeBytesToBool(bet),msg.sender,token,tokenAmount,wrapPayout);
    }

    /// @notice Creates a new bet and stores the chosen coin face for an user.
    /// @param bet The chosen coin face.
    /// @param user Address of the user
    /// @param token Address of the token.
    /// @param tokenAmount The number of tokens bet.
    function wagerWithDataFor(
        bytes calldata bet,
        address user,
        address token,
        uint256 tokenAmount,
        ...
        _wager(_decodeBytesToBool(bet),user,token,tokenAmount,wrapPayout);
    ...
    IGame
{
    ...
    /// @param payout The payout amount.
    /// @param chargedVRFCost The Chainlink VRF fee charged to the player.
    /// @param wrapPayout Whether the payout has to be wrapped 
    struct Bet {
        bool resolved;
        address user;
        address token;
    ...
    /// @param tokenAddress Address of the token (zero address if gas token).
    /// @param user Address of the user.
    /// @param tokenAmount The number of tokens bet.
    /// @param multiplier The bet amount leverage determines the user's profit amount. 10000 = 100% = no profit.
    /// @param wrapPayout Whether the payout has to be wrapped (only useful when token is gas token or wrapped gas token).
    ...
    /// Excess gas token will be refunded to the user via the withdrawOverchargedVRFCost function. 
    function _newBet(
        address tokenAddress,
        address user,
        uint256 tokenAmount,
        ...
        {
            (bool isAllowedToken, uint64 VRFSubId, uint256 minBetAmount, uint256 maxBetAmount) = bank
            ...
            if (tokenAmount > maxBetAmount) {
                if (!isWrappedGasToken && tokenAddress == address(0)) {
                    Address.sendValue(
                        payable(msg.sender),
        ...
        uint256 fee = isWrappedGasToken || tokenAddress != address(0) ? msg.value : msg.value - tokenAmount;
        {
        ...
        if (tokenAddress != address(0) || isWrappedGasToken) {
            IERC20(isWrappedGasToken ? address(wrapped) : tokenAddress).safeTransferFrom(
                msg.sender,
                address(this),
                tokenAmount
            );
            if (isWrappedGasToken) {
                wrapped.withdraw(tokenAmount);
            }
        }
    ...
    function _accountVRFCost(Bet storage bet, uint256 startGas) internal {
        ...
        if (actualVRFCost > bet.chargedVRFCost) {
            actualVRFCost = bet.chargedVRFCost;
        } else {
            // Otherwise credits it to his account.
            uint256 overchargedVRFCost = bet.chargedVRFCost - actualVRFCost;
    ...
    function _resolveBet(
        ...
        bool wrapPayout = bet.wrapPayout;
        ...
        if (payout > betAmount) {
            ...
            bank.payout{value: isGasToken ? betAmountFee : 0}(
                isGasToken && wrapPayout ? address(this) : user,
                token,
                profitPayout,
                fee
            );
            
            if (isGasToken && wrapPayout) {
                wrapped.deposit{value: profitPayout + betAmountPayout}();
                wrapped.transfer(user, profitPayout + betAmountPayout);
            }
            ...
            if (isGasToken) {
                if(wrapPayout) {
                    wrapped.deposit{value: payout}();
                    wrapped.transfer(user, payout);
                }
                else {
    ...
    function refundBet(uint256 id) external {
        ...
        uint256 chainlinkVRFCost = bet.chargedVRFCost;
