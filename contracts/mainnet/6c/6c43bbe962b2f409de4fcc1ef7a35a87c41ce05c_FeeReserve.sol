...
library LibCycle {
    ...
    function fulfilRequests() external {
        ...
        uint256[2] memory aggregateMintFees;
        uint256[2] memory aggregateBurnFees;
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            if (mintRequest.asset != address(0)) {
                uint256[2] memory mintFees = _fulfilMintRequest(mintBurnKey, mintRequest);
                aggregateMintFees[0] += mintFees[0];
                aggregateMintFees[1] += mintFees[1];
            }
            GMIBurnRequest memory burnRequest = _getCycleStorage().burnRequests[mintBurnKey];
            if (burnRequest.asset != address(0)) {
                uint256[2] memory burnFees = _fulfilBurnRequest(mintBurnKey, burnRequest);
                aggregateBurnFees[0] += burnFees[0];
                aggregateBurnFees[1] += burnFees[1];
            }
        }
        LibAggregateVaultUtils.pullFeeAmountsFromEscrow(aggregateMintFees);
        LibAggregateVaultUtils.pullFeeAmountsFromEscrow(aggregateBurnFees);
    }
        ...
        returns (uint256[2] memory feeAmounts)
    {
        ...
        for (uint256 i = 0; i < indexSize; i++) {
            if (_mintRequest.gmAmountsRequired[i] > 0 && _mintRequest.assetAmounts[i] > 0) {
                ...
                if (isUsdc) {
                    uint256 quotedRecieved = _previewGmMint(_mintRequest.markets[i], gmReceived[i], TOKEN_USDC_NATIVE);
                    feeAmounts[0] += _mintRequest.assetAmounts[i] > quotedRecieved
                        ? _mintRequest.assetAmounts[i] - quotedRecieved
                        : 0;
                } else {
                    uint256 quotedRecieved = _previewGmMint(_mintRequest.markets[i], gmReceived[i], TOKEN_WETH);
                    feeAmounts[1] += _mintRequest.assetAmounts[i] > quotedRecieved
                        ? _mintRequest.assetAmounts[i] - quotedRecieved
                        : 0;
                }
        ...
        for (uint256 i = 0; i < 4; i++) {
            if (_burnRequest.gmAmounts[i] > 0) {
                ...
                if (isUsdc) {
                    ...
                    gmValue = _previewGmMint(_burnRequest.markets[i], _burnRequest.gmAmounts[i], TOKEN_USDC_NATIVE);
                    feeAmounts[0] += gmValue > amountOutTotal ? gmValue - amountOutTotal : 0;
                } else {
                    amountOutTotal = _swap(false, usdcReceived[i]) + ethReceived[i];
                    gmValue = _previewGmMint(_burnRequest.markets[i], _burnRequest.gmAmounts[i], TOKEN_WETH);
                    feeAmounts[1] += gmValue > amountOutTotal ? gmValue - amountOutTotal : 0;
                }
            }
        }
