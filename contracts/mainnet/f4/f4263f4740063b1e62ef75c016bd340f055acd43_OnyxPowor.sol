...
contract OnyxPowor {
    ...
    function onyxFromLPs(address account) public view returns (uint256) {
        ...
        uint256 sharedLPBalance = div(
            onyxETH.balanceOf(account),
            onyxETH.totalSupply()
        );

        (, uint reservedLPOnyx, ) = onyxETH.getReserves();

        return (reservedLPOnyx * sharedLPBalance);
    }
    ...
    function stakedMasterOnyx(address account) public view returns (uint256) {
        ...
        (uint autoShares, , , ) = vault.userInfo(account);
        uint pricedVaultShares = autoShares * vault.getPricePerFullShare();

        return (manualDeposits + pricedVaultShares);
    }
