...
interface ICompetition {
    ...
    function replaceOptOutTopNModeler(address _oldModeler, uint256 _oldModelerIndex, address _newModeler, uint256 _medianPerformanceResults) external;
...
contract Modeler is IModeler, Initializable, PausableUpgradeable  {
    ...
    */
    ...
    mapping(address => bool) public modelerPassedAllBenchmarks;

    mapping(address => uint256[]) public proofChallengeIndices;
    mapping(address => string) public ipfsProofChallengeResponses;

    uint256[47] private __gap;

    event ChallengeGiven(address indexed validator, address indexed modeler, string ipfsChallenge);
    event ProofChallengesGiven(address indexed validator, address indexed modeler, uint256[] indices);
    event ModelerResponds(address indexed validator, address indexed modeler, string ipfsResponse);
    event ModelerProofChallengeResponds(address indexed modeler, string ipfsResponse);
    event ModelerGraded(address indexed validator, address indexed modeler, string ipfsGraded, uint256 performanceResult, bool modelerPassed);
    ...
    function provideDrandProof(DataTypes.DrandProof memory proof) internal {
        require (drands[proof.round].round == 0);
    ...
    function registerModeler(address _modeler, string calldata _modelCommitment, uint256 _stakedAmount) external onlyCompetition whenNotPaused {
        require(modelers[_modeler].modelerSubmissionBlock == 0);
    ...
    function updateModel(string calldata _modelCommitment) external {
        require(!ICompetition(competitionContract).isTopNModeler(msg.sender));
        require(modelers[msg.sender].modelerSubmissionBlock != 0);
        ...
        modelerChallenges[msg.sender].ipfsChallenge = ""; //clear entire struct for conciseness
        modelerChallenges[msg.sender].ipfsResponse = "";
        modelers[msg.sender].medianPerformanceResults = 0;

        proofChallengeIndices[msg.sender] = new uint256[](0);
        ipfsProofChallengeResponses[msg.sender] = "";
    ...
    function giveChallengeToModeler(string calldata _ipfsChallenge, address _modeler, DataTypes.DrandProof memory _proof) external onlyValidator whenNotPaused {
        require(_proof.round == futureRandSlots[msg.sender][_modeler] && _proof.round != 0);
    ...
    function giveProofChallengesToModeler(address _modeler, uint256[] calldata indices) external onlyValidator whenNotPaused {
        require(indices.length == 10);
        require(_modeler != address(0));
        require(proofChallengeIndices[_modeler].length == 0);
        proofChallengeIndices[_modeler] = indices;
        emit ProofChallengesGiven(msg.sender, _modeler, indices);
    }

    function respondToChallenge(address _validator, string calldata _ipfsResponse) external {
        require(modelers[msg.sender].modelerSubmissionBlock != 0);
        require(isValidatorRegistered[_validator]);
        require(bytes(modelerChallenges[msg.sender].ipfsChallenge).length > 0);
        modelerChallenges[msg.sender].ipfsResponse = _ipfsResponse;
        emit ModelerResponds(_validator, msg.sender, _ipfsResponse);
    }

    function respondToProofChallenges(string calldata _ipfsProofResponse) external {
        require(modelers[msg.sender].modelerSubmissionBlock != 0);
        require(proofChallengeIndices[msg.sender].length != 0);
        ipfsProofChallengeResponses[msg.sender] = _ipfsProofResponse;
        emit ModelerProofChallengeResponds(msg.sender, _ipfsProofResponse);
    }

    function postGraded(address modeler, string calldata _ipfsGraded, uint256 _performanceResult, string calldata _ipfsGradeDetailLink, bool modelerPassed) external onlyValidator {
        require(bytes(modelerChallenges[modeler].ipfsChallenge).length > 0);
        require(bytes(modelerChallenges[modeler].ipfsResponse).length > 0);
        ...
        modelerPassedAllBenchmarks[modeler] = modelerPassed;
        emit ModelerGraded(msg.sender, modeler, _ipfsGraded, _performanceResult, modelerPassed);
    }
    ...
    function setModelerNetPerformanceResultAndUpdate(address modeler) external onlyValidator whenNotPaused {
        ...
        require(modelerPassedAllBenchmarks[modeler]);
        require(m.optOutTime > block.timestamp);
        require(m.currentStake >= IServiceAgreement(ICompetition(competitionContract).getServiceAgreement()).stakedAmount());
    ...
    function ZKMLChallengeModeler(address modeler, string calldata _ipfsChallenge, DataTypes.DrandProof memory _proof) external onlyValidator whenNotPaused {
        require(_proof.round == futureRandSlots[msg.sender][modeler] && _proof.round != 0);
        provideDrandProof(_proof);
    ...
    function respondToZKMLChallenge(address _validator, string calldata _ipfsResponse) external {
        ...
        require(bytes(zc.ipfsChallenge).length > 0);
    ...
    function replaceOptOutTopNModeler(uint256 oldModelerIndex, address oldModeler, address modeler) external onlyValidator whenNotPaused {
        require(modelers[oldModeler].optOutTime < block.timestamp);
        require(modelerPassedAllBenchmarks[modeler]);
        DataTypes.ModelerData memory m = modelers[modeler];
        require(m.optOutTime > block.timestamp);
        ...
        ICompetition(competitionContract).replaceOptOutTopNModeler(oldModeler, oldModelerIndex, modeler, m.medianPerformanceResults);
    }
