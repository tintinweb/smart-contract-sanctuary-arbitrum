...
contract Farm is IFarmEvent {
    ...
    function depositLiquidity(
        ...
        uint256 shareBeforeDeposit = IStrategyInfo(_strategyContract).userShare(msg.sender);
        ...
        if (_isETH) {
            ...
            (increasedToken0Amount, increasedToken1Amount, sendBackToken0Amount, sendBackToken1Amount) = IStrategy(
                _strategyContract
            ).depositLiquidity{value: _inputAmount}(
            ...
            (increasedToken0Amount, increasedToken1Amount, sendBackToken0Amount, sendBackToken1Amount) = IStrategy(
                _strategyContract
            ).depositLiquidity(_isETH, msg.sender, _inputToken, _inputAmount, _swapInAmount, _minimumSwapOutAmount);
        }
    ...
    function claimReward(address _strategyContract) public {
        uint256 claimedRewardAmount = IStrategyInfo(_strategyContract).userWbtcReward(msg.sender);
