...
import "../core/interfaces/IPlpManagerPyth.sol";
    ...
    OwnableUpgradeable
{
    ...
    event StakePalm(address account, uint256 amount);
    event UnstakePalm(address account, uint256 amount);
    event RedeemPalm(address account, uint256 amount);

    event StakePlp(address account, uint256 amountIn, uint256 amountOut);
    event UnstakePlp(address account, uint256 amountIn);
    event RedeemPlp(address account, uint256 plpAmount, uint256 amountOut);

    function initialize(
        address _wrapped,
        address _feePalmTracker,
        ...
        feePalmTracker = _feePalmTracker;
    ...
    function batchStakePalmForAccount(
        address[] calldata _accounts,
        uint256[] calldata _amounts
    ) external nonReentrant onlyOwner {
        for (uint256 i; i < _accounts.length; ++i) {
            _stakePalm(msg.sender, _accounts[i], _amounts[i]);
        }
    }

    function stakePalmForAccount(
        address _account,
        uint256 _amount
    ) external nonReentrant onlyOwner {
        _stakePalm(msg.sender, _account, _amount);
    }

    function stakePalm(uint256 _amount) external nonReentrant {
        _stakePalm(msg.sender, msg.sender, _amount);
    }

    function unstakePalm(uint256 _amount) external nonReentrant {
        _unstakePalm(msg.sender, _amount);
    }

    function redeemPalm(
        uint256[] calldata _indexes
    ) external nonReentrant returns (uint256) {
        return _redeemPalm(msg.sender, _indexes, msg.sender);
    }

    function _redeemPalm(
        address _account,
        uint256[] calldata _indexes,
        address _receiver
    ) internal returns (uint256 amount) {
        amount = IRewardTracker(feePalmTracker).redeemForAccount(
            _account,
            _indexes,
            _receiver
        );

        emit RedeemPalm(_account, amount);
    }
    ...
    function _mintAndStakePlp(
        ...
        uint256 plpAmount = IPlpManagerPyth(plpManager).addLiquidityForAccount{
            value: msg.value
        }(
            priceUpdateData,
            _fundingAccount,
    ...
    function _redeemAndWithdrawPlp(
        bytes[] calldata priceUpdateData,
        address _account,
        uint256[] calldata _indexes,
        uint256 _minOut,
        ...
        uint256 _plpAmount = IRewardTracker(feePlpTracker).redeemForAccount(
            _account,
            _indexes,
            _account
        );

        uint256 amountOut = IPlpManagerPyth(plpManager)
            .removeLiquidityForAccount{value: msg.value}(
            priceUpdateData,
            _account,
            _plpAmount,
            _minOut,
        ...
        emit RedeemPlp(_account, _plpAmount, amountOut);

        return amountOut;
    }

    function unstakePlp(uint256 _plpAmount) external nonReentrant {
        _unstakePlp(msg.sender, _plpAmount);
    }

    function _unstakePlp(address _account, uint256 _plpAmount) internal {
        ...
        emit UnstakePlp(_account, _plpAmount);
    }

    function compoundPalm(
        uint256 _minOut,
        uint256 _deadline
    ) external nonReentrant {
        _compoundPalm(msg.sender, _minOut, _deadline);
    }

    function compoundPlp(
        bytes[] calldata priceUpdateData,
        uint256 _minUsdp,
        uint256 _minPlp
    ) external payable nonReentrant {
        _compoundPlp(priceUpdateData, msg.sender, _minUsdp, _minPlp);
    }

    function _compoundPalm(
        address _account,
        uint256 _minOut,
        uint256 _deadline
    ) private {
        if (feePalmTracker == address(0)) return;

        uint256 rewardAmount = IRewardTracker(feePalmTracker).claimForAccount(
            _account,
            address(this)
        );
        if (rewardAmount == 0) return;

        uint256 palmAmount = _swapExactTokensForTokens(
            rewardAmount,
            _minOut,
            _deadline
        );
        ...
        if (palmAmount > 0) {
            address palm = IRewardTracker(feePalmTracker).want();

            IERC20Upgradeable(palm).safeApprove(feePalmTracker, 0);
            IERC20Upgradeable(palm).safeApprove(feePalmTracker, palmAmount);

            _stakePalm(address(this), _account, palmAmount);
        }
    }

    function _compoundPlp(
        bytes[] calldata priceUpdateData,
        address _account,
        uint256 _minUsdp,
        uint256 _minPlp
    ) private {
        if (feePlpTracker == address(0)) return;

        uint256 rewardAmount = IRewardTracker(feePlpTracker).claimForAccount(
            _account,
            address(this)
        );

        if (rewardAmount > 0) {
            address rewardToken = IRewardTracker(feePlpTracker).rewardToken();

            IERC20Upgradeable(rewardToken).safeApprove(plpManager, 0);
    ...
    function claim() external nonReentrant {
        ...
        if (feePalmTracker != address(0))
            IRewardTracker(feePalmTracker).claimForAccount(account, account);

        if (feePlpTracker != address(0))
    ...
    function _stakePalm(
        ...
        IRewardTracker(feePalmTracker).stakeForAccount(
            _fundingAccount,
            _account,
            _amount
        );

        emit StakePalm(_account, _amount);
    }

    function _unstakePalm(address _account, uint256 _amount) private {
        require(_amount > 0, "RewardRouter: invalid _amount");

        IRewardTracker(feePalmTracker).unstakeForAccount(
            _account,
            _amount,
            _account
        );

        emit UnstakePalm(_account, _amount);
    }
    ...
    function _swapExactTokensForTokens(
        ...
        address rewardToken = IRewardTracker(feePalmTracker).rewardToken();
        address palm = IRewardTracker(feePalmTracker).want();
        ...
        IERC20Upgradeable(rewardToken).safeApprove(address(swapRouter), 0);
