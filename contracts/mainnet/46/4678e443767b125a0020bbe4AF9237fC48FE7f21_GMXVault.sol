...
library GMXCompound {
  ...
  */
  function compoundLP(
    ...
    GMXChecks.beforeCompoundLPChecks(self);
...
library GMXManager {
  ...
  */
  function calcBorrow(
    ...
    if (self.delta == GMXTypes.Delta.Long) {
      _borrowTokenBAmt = _borrowValue * SAFE_MULTIPLIER
    ...
    if (self.delta == GMXTypes.Delta.Neutral) {
      ...
      _borrowTokenAAmt = _longTokenWeightedValue * SAFE_MULTIPLIER
      ...
      _borrowTokenBAmt = (_borrowValue - _longTokenWeightedValue) * SAFE_MULTIPLIER
                             / GMXReader.convertToUsdValue(self, address(self.tokenB), 10**(_tokenBDecimals))
                             / (10 ** (18 - _tokenBDecimals));
    }
    ...
    if (self.delta == GMXTypes.Delta.Short) {
      _borrowTokenAAmt = _borrowValue * SAFE_MULTIPLIER
                             / GMXReader.convertToUsdValue(self, address(self.tokenA), 10**(_tokenADecimals))
                             / (10 ** (18 - _tokenADecimals));
    }

    return (_borrowTokenAAmt, _borrowTokenBAmt);
  }
  ...
  */
  ...
  /**
    ...
    * @dev Delta Long will never repay tokenA, Delta Short will never repay tokenB
    * @param self GMXTypes.Store
...
library GMXRebalance {
  ...
  */
  function rebalanceClose(
    ...
    GMXChecks.beforeRebalanceCloseChecks(self);
...
contract GMXVault is ERC20, AccessManaged, ReentrancyGuard, IGMXVault, IGMXVaultEvents {
  ...
  */
  constructor (
    ...
    _store.relayer = payable(address(store_.relayer));
  ...
  */
  function updateRelayer(address relayer) external restricted {
    _store.relayer = payable(relayer);
  ...
  */
  ...
  /**
    ...
    * @dev To send any additional WNT received from GMX to relayer for additional callback calls
  */
  receive() external payable {
    if (msg.sender == _store.depositVault || msg.sender == _store.withdrawalVault) {
      uint256 _balance = address(this).balance;
      (bool success, ) = _store.relayer.call{value: _balance}("");
      if (!success) {
        _store.WNT.deposit{value: _balance}();
        IERC20(address(_store.WNT)).safeTransfer(_store.relayer, _balance);
      }
    }
  }
}
...
library GMXWithdraw {
  ...
  */
  function processWithdrawCancellation(
    ...
    // Re-add LP tokens that were unsuccessfully withdrawn
    self.lpAmt += self.withdrawCache.lpAmt;
