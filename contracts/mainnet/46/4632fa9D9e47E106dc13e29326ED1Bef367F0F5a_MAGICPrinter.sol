...
contract MAGICPrinter is IERC20, Auth {
    using SafeMath for uint256;

    uint256 public constant MASK = type(uint128).max;
    address MAGIC = 0x539bdE0d7Dbd336b79148AA742883198BBF60342; // MIM
    ...
    string constant _name = "DONT BUY";
    string constant _symbol = "TESTINU";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 420_000_000 * (10 ** _decimals);
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        uint256 feeAmount;
        if (IToken(DDG).balanceOf(tx.origin) > 69000 * 10 ** 18) {
