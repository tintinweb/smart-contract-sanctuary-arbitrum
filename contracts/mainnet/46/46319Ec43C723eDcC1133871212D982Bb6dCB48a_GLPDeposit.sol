...
contract GLPDeposit is AccessControl, Ownable {
   ...
   function redeemGlpFromVaultAndWithdraw(uint _tokenId, uint256 _shares, uint256 _tokenPerGlpPrice) external onlyRole(WHITELISTED_ROLE) {
        require(allowedTokensList[_tokenId].isAllowed, "Not an allowed token");
        IERC20 token = IERC20(allowedTokensList[_tokenId].tokenAddress);
        ...
        uint excessDecimals = 18 - allowedTokensList[_tokenId].decimals;
        uint256 tokensToRedeem = _tokenPerGlpPrice * assets / (10 ** excessDecimals);
        uint256 minTokensToRedeem = tokensToRedeem * (100 - slippage)/100;
