...
contract Automation is Admin, NonDelegateMulticall {
    ...
    error AutomationNotSet(address user);
    ...
    function compoundFor(
        ...
        if (
            strategies.length != sickles.length
                || strategies.length != params.length
                || strategies.length != sweepTokens.length
        ) {
            revert InvalidInputLength();
        }

        address[] memory targets = new address[](strategies.length);
        ...
        for (uint256 i = 0; i < targets.length; i++) {
            ...
            if (rewardAutomation[user] != RewardAutomation.Compound) {
                revert AutomationNotSet(user);
            }
            targets[i] = address(strategies[i]);
            data[i] = abi.encodeCall(
                ICompoundable.compoundFor,
                (sickles[i], params[i], sweepTokens[i])
            );
            emit CompoundedFor(
                user,
                params[i].claimContractAddress,
                params[i].depositContractAddress
            );
        }
    ...
    function harvestFor(
        ...
        for (uint256 i = 0; i < targets.length; i++) {
            ...
            targets[i] = address(strategies[i]);
            data[i] = abi.encodeCall(
                IHarvestable.harvestFor, (sickles[i], params[i], sweepTokens[i])
            );
            emit HarvestedFor(user, params[i].stakingContractAddress);
        }
        this.multicall(targets, data);
    }
    ...
    function rebalanceFor(
        ...
        for (uint256 i = 0; i < targets.length; i++) {
            ...
            targets[i] = address(strategies[i]);
            data[i] = abi.encodeCall(
                IRebalanceable.rebalanceFor,
                (sickles[i], params[i], sweepTokens[i])
            );
            emit RebalancedFor(
                user,
                address(params[i].nftInfo.nftManager),
                params[i].nftInfo.tokenId
            );
        }
        this.multicall(targets, data);
    }
    ...
    function disableAutoCompound() external {
        if (rewardAutomation[msg.sender] != RewardAutomation.Compound) {
            revert AutomationNotSet(msg.sender);
        }
