...
contract gETH is ERC20("gETH", "gETH"), Ownable , ReentrancyGuard{ 
    ...
    mapping(address => CollateralProperties) private CollateralMap;
    ...
    function estimateRedeemWithCollaterals(uint256 _amount, IERC20 _token) public view returns(uint256) {
        require(isCollateral[address(_token)], "not collateral");
    ...
    function crosschainMint(uint256 _amount, IERC20 _token, uint16 dstChainId ) public nonReentrant{
       ...
       require(calWeight(address(_token), _amount) <= token.maxWeight, "tokens reached max weight");
       _token.safeTransferFrom(msg.sender, address(this), _amount);
       uint256 bridgeAmount = mintWithCollaterals(_amount, _token);
    ...
    function swap(uint256 _amount, IERC20 _from, IERC20 _to) public nonReentrant returns (uint256) {
       ...
       require(isCollateral[address(_from)], "not collateral");
       require(_from.balanceOf(msg.sender) >= _amount, "token balance too low");
       CollateralProperties memory token = CollateralMap[address(_from)];
       require(calWeight(address(_from), _amount) <= token.maxWeight, "tokens reached max weight");
       _from.safeTransferFrom(msg.sender, address(this), _amount);
       uint256 swapAmount = mintWithCollaterals(_amount, _from);
       uint256 amountOut = redeem(swapAmount, _to);
       _to.safeTransfer(msg.sender, amountOut);
    ...
    function estimateSwap(uint256 _amount, IERC20 _from, IERC20 _to) public view returns (uint256) {
       ...
       uint256 amountOut = estimateRedeemWithCollaterals(swapAmount, _to);
       return amountOut;
