    ...
    AvoForwarderErrors
{
    ...
    function _executeV1(
        ...
        (success_, revertReason_) = avocadoMultisig_.cast{ value: forwardParams_.value }(
            params_,
            forwardParams_,
            signaturesParams_
        );
    ...
    function _executeChainAgnosticV1(
        ...
        string memory revertReason_;
        (success_, revertReason_) = avocadoMultisig_.castChainAgnostic{ value: params_.forwardParams.value }(
            params_,
            signaturesParams_,
...
abstract contract AvoForwarderSimulateV1 is AvoForwarderCore {
        ...
        )
    {
        ...
        (avocadoMultisig_, deploymentGasUsed_, isDeployed_) = _getSimulateDeployedAvocado(from_, index_);

        {
            uint256 gasSnapshotBefore_;
            bytes32 avoVersion_ = keccak256(bytes(avocadoMultisig_.DOMAIN_SEPARATOR_VERSION()));
            if (avoVersion_ == keccak256(bytes("1.0.0")) || avoVersion_ == keccak256(bytes("1.0.1"))) {
                ...
                gasSnapshotBefore_ = gasleft();
                (success_, revertReason_) = avocadoMultisig_.simulateCast{ value: forwardParams_.value }(
                    params_,
                    forwardParams_,
                    signaturesParams_
                );
        ...
        )
    {
        ...
        // used gas here is cast gas + deployment gas + 10k gas for emit events instead of measuring to avoid
        // a stack too deep error.
        _wasteGas(((castGasUsed_ + deploymentGasUsed_ + 10_000) * SIMULATE_WASTE_GAS_MARGIN) / 100); // e.g. 10% of used gas
    }
