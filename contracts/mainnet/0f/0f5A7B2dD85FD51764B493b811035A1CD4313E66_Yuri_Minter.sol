...
contract Yuri_Minter is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    function mintYuri(string memory uri, bool isFreeMint) external onlyEOA {
        ...
        require(block.timestamp < mintYuriEndTime, 'mint yuri end');
        if (isFreeMint) {
    ...
    function claimArbAirDrop(uint timestamp, bytes32 r, bytes32 s, uint8 v) external onlyEOA {
        ...
        require(airDropNums < 2000, 'out of limit');
        ...
        uint amount = airDropTotal * airDropRate[1] / 100 / 2000;
    ...
    function claimAIGCAirDrop() external onlyEOA {
        require(airDropInfo[msg.sender].isYuriClaimed == false, 'already claimed');
        require(block.timestamp >= mintYuriEndTime, 'not claim time');
        require(airDropStatus[1],'not start');
        uint airDropAmount = calculateAirDropAmount(msg.sender);
        airDropInfo[msg.sender].isYuriClaimed = true;
    ...
    function calculateAirDropAmount(address addr) public view returns (uint){
        //        if (block.timestamp >= mintYuriEndTime) {
        //            return 0;
        //        }
        if (airDropInfo[addr].isYuriClaimed == true) {
            return 0;
        }
        uint yuriFreeAirDropAmount = airDropTotal * airDropRate[1] * 50 / 10000;
        uint yuriFreeAmount = yuriFreeAirDropAmount * userInfo[addr].freeMintAmount / totalFreeMint;
        uint yuriUsdtAirDropAmount = airDropTotal * airDropRate[1] * 30 / 10000;
        uint yuriUsdtAmount = yuriUsdtAirDropAmount * userInfo[addr].usdtMintAmount / totalUsdtMint;
        uint yuriBondAirDropAmount = airDropTotal * airDropRate[1] * 20 / 10000;
        uint yuriBondAmount = yuriBondAirDropAmount / totalBond;
        uint yuriAmount = yuriFreeAmount + yuriUsdtAmount + yuriBondAmount;
        return yuriAmount;
    }

    function claimKajamaAirDropAmount() external onlyEOA {
        require(airDropStatus[2],'not start');
        uint kajamaAmount = airDropTotal * airDropRate[2] / 100 / 20000;
    ...
    function claimSoulBondAirDrop() external onlyEOA {
        ...
        require(block.timestamp >= soulBondEndTime, 'not claim time');
    ...
    function calculateSBTAirDropAmount(address addr) public view returns (uint){
        ...
        uint level = sbt.checkUserCardId(msg.sender);
        uint sbtAmount = airDropTotal * airDropRate[3] / 100;
        uint airDropAmount = sbtAmount * soulBondRate[level] / soulBondMap[level];
        return airDropAmount;
    }
