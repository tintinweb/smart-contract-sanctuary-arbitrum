...
import "./contracts/GNSTradingV6_2.sol";
import "./contracts/GNSTradingStorageV5.sol";
import "./contracts/GNSNftRewardsV6.sol";
...
contract GainsReader {
    ...
    function config() external view returns (GainsConfig memory) {
        GNSTradingV6_2 trading = GNSTradingV6_2(tradingStorage.trading());
    ...
    function getLimitOrders(address trader)
        ...
        returns (
            ...
            GNSNftRewardsV6.OpenLimitOrderType[] memory openLimitOrderTypes
        )
    {
        GNSTradingV6_2 trading = GNSTradingV6_2(tradingStorage.trading());
        GNSNftRewardsV6 nftRewards = trading.nftRewards();
        ...
        openLimitOrderTypes = new GNSNftRewardsV6.OpenLimitOrderType[](total);
                ...
                if (limitOrderCounts[pairIndex] > 0) {
                    ...
                    for (uint256 orderIndex = 0; orderIndex < maxTradesPerPair; orderIndex++) {
                        if (tradingStorage.hasOpenLimitOrder(trader, pairIndex, orderIndex)) {
                            ...
                            openLimitOrderTypes[openLimitOrderIndex] = nftRewards.openLimitOrderTypes(
                                trader,
                                pairIndex,
                                orderIndex
                            );
