...
contract Core is ICore, Initializable, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, CoreStorageV1 {
	...
	modifier hedgeCoreAllowed() {
		require(hedgeCoreStatus(), "HC:LKD");
		_;
	}
	...
	function initialize(
		...
		// 1. param checks
		require(bytes(name_).length != 0, "HC:SINV");
		require(IAddressProvider(addressProvider_).getDAO() != address(0), "HT:AINV");
		require(IERC20(wToken_).totalSupply() > 0, "HC:GINV");
	...
	function initGnesisHedge(
		...
		// 1. para checks
		require(!initialized, "HC:ITD!");
		require(lastRebaseTime_ != 0, "HC:TINV");
		...
		require(hedgeInfo.hedgeTokenPrice != 0, "HC:WPINV");
		currSTokenIndex = index();
		require(currSTokenIndex != 0, "HC:IINV");
	...
	function withdrawAllWhenPaused() public override nonReentrant onlyInitialized whenPaused onlyWithdrawAllNotPaused isEligibleSender {
		...
		require(hgeBalance + levBalance + sponsorBalance != 0, "HC:0");

		// 2. withdraw all to user
		if (hgeBalance + levBalance != 0) {
	...
	function updatePriceBeforeRebase() external override nonReentrant onlyInitialized whenNotPaused returns (uint256 price_) {
		require(!_isSTokenRebased(index()), "HC:RBD"); // make sure not rebased
		require(!isPriceUpdatedBeforeRebase(), "HC:PUpd"); // make sure price updated before rebase
		require(block.timestamp >= lastPriceUpdateTimestamp + 8 hours, "HC:PNR");
		price_ = _updatePrice();
	}
	...
	function _depositFor(
		...
		require(user_ != address(0), "HC:A0");
		require(hgeAmount_ + levAmount_ != 0, "HC:0");
	...
	function _withdrawTo(
		...
		require(hgeAmount_ + levAmount_ != 0, "HC:0");
		require(recipient_ != address(0), "HC:A0");
	...
	function _startNewHedge() internal {
		require(hgeToken.totalSupply() > 0 && levToken.totalSupply() > 0, "HC:2t0");
		bool isLev;
	...
	function updateGaugeAndRatio(address newGauge_, uint256 ratio_) external onlyDAO {
		gauge = newGauge_;
		require(ratio_ <= RATIO_PRECISION, "HC:RINV"); // <= 100%
		toGaugeRatio = ratio_;
	}
	...
	function updatePriceRatio(bool isUp_, uint256 newRatio_) external onlyDAO {
		require(newRatio_ <= RATIO_PRECISION, "HC:PRINV");
	...
	function setWithdrawAllPause(bool paused_) external onlyEmergencyAdmin {
		if (paused_) {
			// pause withdraw all
			require(!withdrawAllPaused, "HC:WAP");
		} else {
			// unpause withdraw all
			require(withdrawAllPaused, "HC:WANP");
		}
	...
	function updateRestrictedPeriod(uint256 value_) external onlyDAO {
		require(value_ <= 8 hours, "HC:RPINV");
