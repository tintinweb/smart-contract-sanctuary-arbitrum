/**
 *Submitted for verification at arbiscan.io on 2022-01-29
*/
...
// File: contracts/strategies/StabilizeStrategyWBTCTraderV3.sol
...
contract StabilizeStrategyWBTCTraderV3 is Ownable {
    using SafeMath for uint256;
    ...
    uint256 public priceAverageMethod = 1; // 0 =  Average with chainlink price, 1 = Average to spot price
    ...
    uint256 public testTradeMultiplier = 1; // Used to determine spot price
    ...
    uint256 public withdrawPriceSlippage = 1000; // Percent difference allowed between chainlink oracle and spot price
    ...
    function exchangeUSDTForWBTC(uint256 amount, address receiver) internal {
        // Check to see if wBTC spot price is near chainlink price
        uint256 spotPrice = getSpotBTCPrice();
        uint256 chainPrice = getBTCUSDPrice();
        uint256 diff = 0;
        if(spotPrice > chainPrice){
            diff = spotPrice.sub(chainPrice).mul(DIVISION_FACTOR).div(chainPrice);
        }else{
            diff = chainPrice.sub(spotPrice).mul(DIVISION_FACTOR).div(spotPrice);
        }
        require(diff < withdrawPriceSlippage, "Price difference too high between spot and chainlink");
    ...
    */
    ...
    function checkAndSwapTokens(address _executor) internal {
        ...
        if(sellToken == 0){
            // We will sell wBTC for USDT when wbtc price is very high
            ...
            // We will sell USDT for wbtc when wbtc price is very low
    ...
    function _updateLastPrice(bool forced) internal {
    ...
    function changeTradingConditions(uint256 _cooldown, 
                                    ...
                                    uint256 _averageMethod,
                                    uint256 _slippage) external onlyGovernance {
        // Changes a lot of trading parameters in one call
        require( _percentSell <= 100000 && _slippage < 100000,"Percent cannot be greater than 100%");
        ...
        withdrawPriceSlippage = _slippage;
    }
    // --------------------
