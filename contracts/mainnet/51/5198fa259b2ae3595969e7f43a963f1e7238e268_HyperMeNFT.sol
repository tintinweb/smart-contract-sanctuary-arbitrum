...
import "./interface/IHyperMeNFT.sol";

contract HyperMeNFT is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable, UUPSUpgradeable, IHyperMeNFT {

    uint256 public override nextTokenId;
    uint256 public override mintFee;
    uint256 public bridgeFee;
    uint256 public bridgeFeeWithToken;
    address public override hyperlaneClient;
    address public override mailboxAddr;
    string public override uri;
    uint256 private placeholder;
    uint256 private placeholder2;

    modifier onlyHyperlaneClient() {
        require(msg.sender == address(hyperlaneClient), "HyperMeNFT: sender not hyperlaneClient");
        _;
    }
    ...
    event SetBridgeFeeWithToken(address indexed sender, uint256 bridgeFeeWithToken);
    ...
    function initialize(
        ...
        uint256 mintFee_,
        uint256 bridgeFee_,
        uint256 bridgeFeeWithToken_,
        address mailboxAddr_,
        address hyperlaneClient_,
        string memory uri_
    ) public override initializer {
        __ERC721_init("HyperMeNFT", "HM");
        __ERC721Enumerable_init();
        __Ownable_init(initialOwner_);
        __UUPSUpgradeable_init();

        require(initialTokenId_ >= 1, "HyperMeNFT: tokenId should be >= 1");
        nextTokenId = initialTokenId_;
        mintFee = mintFee_;
        bridgeFee = bridgeFee_;
        bridgeFeeWithToken = bridgeFeeWithToken_;
    ...
    function mint(address target_) external payable override returns (uint256) {
        require(msg.value >= mintFee, "HyperMeNFT: wrong msg.value");
    ...
    function bridgeMessage(uint32 destinationDomain_, string memory customMessage_) external payable returns (uint256, bytes32) {
        _checkBridgeParams(0, destinationDomain_);

        bytes memory payload = abi.encode(uint32(0), msg.sender, destinationDomain_, customMessage_);

        uint256 hyperLaneFee = _calculateBridgeFee(destinationDomain_, payload);
        uint256 totalFee = hyperLaneFee + bridgeFee;
        require(msg.value >= totalFee, "HyperMeNFT: not enough msg.value");
        ...
        return (totalFee, messageId);
    }

    function bridge(uint256 tokenId_, uint32 destinationDomain_, string memory customMessage_) external payable override returns (uint256, bytes32) {
        _checkBridgeParams(tokenId_, destinationDomain_);

        require(ownerOf(tokenId_) == msg.sender, "HyperMeNFT: user do not have NFT");
        _burn(tokenId_);

        bytes memory payload = abi.encode(tokenId_, msg.sender, destinationDomain_, customMessage_);

        uint256 hyperLaneFee = _calculateBridgeFee(destinationDomain_, payload);
        uint256 totalFee = hyperLaneFee + bridgeFeeWithToken;
        require(msg.value >= totalFee, "HyperMeNFT: not enough msg.value");
    ...
    ) external payable virtual override onlyHyperlaneClient {
        // linter suppress: UnusedLocalVariable
        origin_;
        sender_;

        require(data_.length > 0, "HyperMeNFT: empty data");

        (uint256 tokenId, address _sender, uint32 destinationDomain, string memory customMessage) = abi.decode(data_, (uint256, address, uint32, string));
    ...
    function setURI(string memory uri_) external override onlyOwner {
        emit SetURI({sender: msg.sender, oldUri: uri, newUri: uri_});

        uri = uri_;
    }

    function setMintFee(uint256 mintFee_) external override onlyOwner {
        mintFee = mintFee_;

        emit SetMintFee({sender: msg.sender, mintFee: mintFee});
    }

    function setBridgeFee(uint256 bridgeFee_) external override onlyOwner {
        bridgeFee = bridgeFee_;

        emit SetBridgeFee({sender: msg.sender, bridgeFee: bridgeFee});
    }

    function setBridgeFeeWithToken(uint256 bridgeFeeWithToken_) external override onlyOwner {
        bridgeFeeWithToken = bridgeFeeWithToken_;

        emit SetBridgeFeeWithToken({sender: msg.sender, bridgeFeeWithToken: bridgeFeeWithToken});
    }

    function setMailbox(address newMailbox_) external onlyOwner {
    ...
    function withdrawETH(address to_, uint256 amount_) external override onlyOwner {
        emit WithdrawETH({sender: msg.sender, to: to_, amount: amount_});

        payable(to_).transfer(amount_);
    }

    function calculateBridgeFee(uint256 tokenId_, uint32 destinationDomain_, string memory customMessage_) external view override returns (uint256) {
        _checkBridgeParams(tokenId_, destinationDomain_);

        bytes memory payload = abi.encode(tokenId_, msg.sender, destinationDomain_, customMessage_);

        return (_calculateBridgeFee(destinationDomain_, payload));
    }
    ...
    function _calculateBridgeFee(uint32 destinationDomain_, bytes memory payload) internal view returns (uint256) {
        IMailbox mailbox = IMailbox(mailboxAddr);
    ...
    function _checkBridgeParams(uint256 tokenId_, uint32 destinationDomain_) internal view {
        if (tokenId_ > 0) {
            require(ownerOf(tokenId_) != address(0), "HyperMeNFT: nft do not exist");
        }
        require(destinationDomain_ != 0, "HyperMeNFT: zero domain");
        require(destinationDomain_ != block.chainid, "HyperMeNFT: same domain");
    }
    ...
    function _checkHandleParams(uint256 tokenId_, address sender_, uint32 destinationDomain_) internal view {
        ...
        sender_;
        require(destinationDomain_ != 0, "HyperMeNFT: zero domain");
        require(destinationDomain_ == block.chainid, "HyperMeNFT: wrong domain");
    }
    ...
    function supportsInterface(
        ...
        return interfaceId == type(IHyperMeNFT).interfaceId || super.supportsInterface(interfaceId);
    }
}
...
import "./interface/IMailbox.sol";
import "./interface/IHyperMeNFT.sol";
import "./interface/IHyperMeFT.sol";
...
contract HyperlaneClient is Initializable, IHyperlaneClient {
    ...
    function handle(uint32 origin_, bytes32 sender_, bytes calldata data_) external payable onlyMailbox {
        ...
        IHyperMeNFT(user).handle{value: msg.value}(origin_, sender_, data_);
    }
}

contract HyperlaneClientFT is Initializable, IHyperlaneClient {
    event SetMailbox(address indexed sender, address indexed mailbox);
    event ReceivedMessage(address indexed msgSender, uint32 indexed origin, bytes32 indexed sender, bytes data);
    ...
    modifier onlyMailbox() {
        require(msg.sender == address(mailboxAddr), "HyperlaneClientFT: sender not mailbox");
        _;
    }

    modifier onlyUser() {
        require(msg.sender == address(user), "HyperlaneClientFT: sender not user");
        _;
    }
    ...
    function handle(uint32 origin_, bytes32 sender_, bytes calldata data_) external payable onlyMailbox {
        ...
        IHyperMeFT(user).handle{value: msg.value}(origin_, sender_, data_);
    }
}
...
interface IHyperMeNFT {
    ...
    function bridge(uint256 tokenId_, uint32 destinationDomain_, string calldata customMessage) external payable returns (uint256, bytes32);
    ...
    function setBridgeFeeWithToken(uint256 bridgeFeeWithToken_) external;

    function withdrawETH(address to_, uint256 amount_) external;

    function calculateBridgeFee(uint256 tokenId_, uint32 destinationDomain_, string memory customMessage_) external view returns (uint256);
}
