...
contract ExchangeHelper is IExchangeHelper {
    ...
    function addExchange(
        ...
        LibExchangeHelper.addExchange(exchangeType, name);
    }
    ...
    function getExchangeId(
        string memory name
    ) external view returns (uint48 id) {
    ...
    function swap(
        ...
        amount_ = LibExchangeHelper.swap(params);
    }
...
library LibExchangeHelper {
    ...
    function addExchange (ExchangeType exchangeType, string memory name) internal {
        ...
        ds.exchangesInfo[name] = ExchangeInfo({name: name, exchangeType: exchangeType, exchangeId: ds.exchangesCount});
    ...
    function getExchangeId (string memory name)  internal view returns(uint48 id) {
        ExchangeHelperStorage storage ds = diamondStorage();
        id = ds.exchangesInfo[name].exchangeId;
    }

    function swap (SwapParams memory params) internal view returns (uint amount_) {
...
enum ExchangeContractType {
    UNISWAP_V2_ROUTER, UNISWAP_V2_FACTORY, UNISWAP_V3_ROUTER, UINSWAP_V3_FACTORY
}
