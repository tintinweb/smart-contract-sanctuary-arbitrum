...
abstract contract NodeRewardsBase is Initializable, RewardsHelper {
    ...
    /**
     * @notice Called in Referee.batchClaimReward whenever someone claims their reward for multiple node keys. {See Referee.sol}
     *
     * @param _nodeKeyIds Ids of the node keys which are claiming the reward.
     * @param _batchNumbers Batch numbers of successful attestations made by each node key.
     *
     * @dev _batchNumbers can contain 0 (empty items). Make sure to skip over it.
     */
    function batchClaimReward(
        ...
        for (uint256 i; i < _nodeKeyIds.length; i++) {
            _claimReward(_nodeKeyIds[i], _batchNumbers[i]);
        }
    }
