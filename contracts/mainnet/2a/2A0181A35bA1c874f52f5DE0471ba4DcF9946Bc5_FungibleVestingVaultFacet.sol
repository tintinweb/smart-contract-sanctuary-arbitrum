...
contract FungibleVestingVaultFacet is BaseVault, IFungibleVestingVault, ReentrancyGuard {
    using SafeERC20 for IERC20;

    function vest() external override onlyKeyHolder nonReentrant {
        LibVaultStorage.DiamondStorage storage ds = LibVaultStorage.diamondStorage();
        for (uint256 i = 0; i < ds.fungibleTokenDeposits[LibCommonConsts.INNER_STRUCT].length; i++) {
            address tokenAddress = ds.fungibleTokenDeposits[LibCommonConsts.INNER_STRUCT][i].tokenAddress;
            uint256 amountAvailable = getTokenAvailability(tokenAddress);
            if (amountAvailable > 0) {
                ds.lastVestingTimestamp[tokenAddress] = block.timestamp;
                IERC20(tokenAddress).safeTransfer(msg.sender, amountAvailable);
            }
        }
        ds.isUnlocked = _isCompletelyUnlocked();
        IVaultFactory(ds.vaultFactory).notifyUnlock(ds.isUnlocked);
    }

    function partialVest(address _tokenAddress) external override onlyKeyHolder nonReentrant {
        ...
        LibVaultStorage.DiamondStorage storage ds = LibVaultStorage.diamondStorage();
        ds.lastVestingTimestamp[_tokenAddress] = block.timestamp;
    ...
    function getTokenAvailability(address tokenAddress) public view override returns (uint256) {
        ...
        LibVaultStorage.DiamondStorage storage ds = LibVaultStorage.diamondStorage();
        if (block.timestamp >= ds.unlockTimestamp) {
            return balance;
        }
        return (balance * (block.timestamp - ds.lastVestingTimestamp[tokenAddress])) / (ds.unlockTimestamp - ds.lastVestingTimestamp[tokenAddress]);
    }
