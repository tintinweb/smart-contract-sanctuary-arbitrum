...
contract NOADsPool is IERC721Receiver, ReentrancyGuard {
   ...
   struct Karma {
    ...
    constructor(
        ...
        floor = _floor;
        fee = _fee;
        //init karma (rarity multiplier)
        karma.push(Karma(10, 2));
    ...
    function buyNFT(uint256 tokenId) public payable onlyDao nonReentrant returns (bool) {
    ...
    function updateDao(address _daoAdmin) external onlyDao {
        daoAdmin = _daoAdmin;
    }
...
// NFT id + level check
uint id = karma.length;
bool minable = false;
for (uint256 i = 0; i < id; i++) {
Karma storage d = karma[i];
if (d.index == tokenId && d.level == 1) minable = true;
}

// check NFT is minable
require ( minable , "NFT not minable");
...
function viewKarma(uint256 _tokenId) public view returns (uint256) {
//rarity multiplier
uint256 out;
uint id = karma.length;
for (uint256 i = 0; i < id; i++) {
Karma storage d = karma[i];
if (d.index == _tokenId) out = d.level;
}
return (out);
}


    function addMultipleKarma(uint256[] calldata _tokenId, uint256[] calldata _level) public onlyDao returns (bool) {
...
//            karma.push(Karma(tokenId[i], level[i] ));

done = false;
id = karma.length;
for (uint256 j = 0; j < id; j++) {
Karma storage d = karma[j];
if (d.index == _tokenId[i] && _level[i] > 0 && _level[i] < 21) 
{
d.level = _level[i];
done = true;
}
...
if (!done) karma.push(Karma(_tokenId[i], _level[i]));
...
function addKarma(uint256 _tokenId, uint256 _level) public onlyDao {
        
bool done = false;
uint id = karma.length;
for (uint256 i = 0; i < id; i++) {
Karma storage d = karma[i];
if (d.index == _tokenId && d.level > 0 && d.level < 101) 
{
d.level = _level;
done = true;
}
...
if (!done) karma.push(Karma(_tokenId, _level));
