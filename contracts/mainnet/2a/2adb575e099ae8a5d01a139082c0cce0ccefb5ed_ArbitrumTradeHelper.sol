...
// Deployed at: 0x2AdB575e099aE8a5d01A139082C0cce0CcEfB5Ed (Arbitrum)
...
contract ArbitrumTradeHelper {
  ...
  address private constant wstETH_WETH_V3 = 0x35218a1cbaC5Bbc3E57fd9Bd38219D37571b3537;
  ...
  function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external {
    ...
    } else if (pool == wstETH_WETH_V3) {
      if (amount0Delta > 0) {
        _transferFrom(user, wstETH, wstETH_WETH_V3, uint256(amount0Delta));
      } else {
        _transferFrom(user, WETH, wstETH_WETH_V3, uint256(amount1Delta));
      }
    } else {
  ...
  function buyWstEth(uint256 packedParams) external returns (uint256 wethBalanceAfter) {
    ...
    _v3Swap(wstETH_WETH_V3, false, -int256(wstEthBuyAmount));
  ...
  function sellWstEth(uint256 packedParams) external returns (uint256 wethBalanceAfter) {
    ...
    _v3Swap(wstETH_WETH_V3, true, int256(wstEthSellAmount));
