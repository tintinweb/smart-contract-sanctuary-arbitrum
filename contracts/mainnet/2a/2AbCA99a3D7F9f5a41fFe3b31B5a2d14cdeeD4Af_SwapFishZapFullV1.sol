...
contract SwapFishZap is ISwapFishZap, ReentrancyGuard, Ownable {
    ...
    uint public zapFee = 100; // 1%
    bool public nativePromo = true;
    uint public constant ZAP_FEE_MAX = 500; // 5%
    address public constant FISH_TOKEN = 0xb348B87b23D5977E2948E6f36ca07E1EC94d7328;
    ...
    function _zapPrivate(
        ...
        if (zapFee > 0) {
            if (!(nativePromo && (lpTokens[0] == FISH_TOKEN || lpTokens[1] == FISH_TOKEN))) {
                uint fee = inputAmount * zapFee / 10000;
                inputToken.transfer(factory.feeTo(), fee);
                inputAmount -= fee;
            }
        }
    ...
    function setZapFee(uint _zapFee, bool _nativePromo) external onlyOwner {
        require (_zapFee <= ZAP_FEE_MAX, "Fee to high.");
        nativePromo = _nativePromo;
        zapFee = _zapFee;
    }
}
