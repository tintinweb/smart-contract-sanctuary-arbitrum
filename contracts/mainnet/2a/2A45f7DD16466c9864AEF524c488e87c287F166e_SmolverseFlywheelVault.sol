...
import "../interfaces/vaults/ISmolverseFlywheelVault.sol";
    ...
    ERC721HolderUpgradeable
{
    ...
    /**
     * @dev Stake Smolverse tokens
     */
    function stake(address[] memory tokenAddresses, uint256[] memory tokenIds) external override nonReentrant {
        require(
            tokenAddresses.length == tokenIds.length,
            "SmolverseFlywheelVault: tokenAddresses and tokenIds must be equal in size"
        );
        require(tokenAddresses.length > 0, "SmolverseFlywheelVault: required to stake at least 1 token");
        for (uint256 i = 0; i < tokenAddresses.length; i++) {
            require(
                smolverseTokens[tokenAddresses[i]].enabled,
                "SmolverseFlywheelVault: Token not allowed for staking"
            );
            require(
                IERC721Upgradeable(tokenAddresses[i]).ownerOf(tokenIds[i]) == msg.sender,
                "SmolverseFlywheelVault: Not owner of token"
            );
        }
        for (uint256 i = 0; i < tokenAddresses.length; i++) {
            IERC721Upgradeable(tokenAddresses[i]).safeTransferFrom(msg.sender, address(this), tokenIds[i]);
            tokensByUser[msg.sender][tokenAddresses[i]].add(tokenIds[i]);
            emit stakeToken(msg.sender, tokenAddresses[i], tokenIds[i]);
        }
    }
    ...
    function unstake(address[] memory tokenAddresses, uint256[] memory tokenIds) external override nonReentrant {
        ...
        uint256 removedStakeWeight = 0;
        for (uint256 i = 0; i < tokenAddresses.length; i++) {
            removedStakeWeight += smolverseTokens[tokenAddresses[i]].allowance;
        }
        require(
            remainingStakeableAmount(msg.sender) >= removedStakeWeight,
            "SmolverseFlywheelVault: Not allowed to withdraw tokens under the allowance amount"
        );
        for (uint256 i = 0; i < tokenAddresses.length; i++) {
        ...
        returns (uint256 atlasStakerDepositId)
    {
        if (!whitelistedUsers[msg.sender]) {
            require(
                ...
                "SmolverseFlywheelVault: amount exceeds stakeable amount"
            );
        }
     ...
     */
    function addSmolverseToken(bool _enabled, address _token, uint256 _allowance) external override onlyOwner {
...
interface ISmolverseFlywheelVault {
    ...
    function addSmolverseToken(bool _enabled, address _token, uint256 _allowance) external;
