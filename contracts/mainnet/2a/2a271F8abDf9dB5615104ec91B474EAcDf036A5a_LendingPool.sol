    ...
    ReentrancyGuardUpgradeable
{
    ...
    function collect() external nonReentrant {
        ...
        if (treasuryLend > 0) _safeTransfer(
            lendToken,
            treasury,
            treasuryLend
        );
        totalFees = 0;
        if (treasuryCol > 0) _safeTransfer(
            colToken,
            treasury,
            treasuryCol
        );
        ...
        if (lenderLend > 0) _safeTransfer(lendToken, owner, lenderLend);
        if (lenderCol > 0) _safeTransfer(colToken, owner, lenderCol);
