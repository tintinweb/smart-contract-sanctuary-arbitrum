...
contract PaalBet is Ownable, ReentrancyGuard {
  ...
  event PlaceBet(address account, bytes key, bytes position, uint256 amount, uint256 marketTvl, uint256 userTvl);
  ...
  function getMarketTVL(bytes calldata key, bytes calldata position) public view returns(uint256) {
    return marketInfo[key].tvl[position];
  }

  function getUserMarketTVL(address account, bytes calldata key, bytes calldata position) public view returns(uint256) {
    return userMarketInfo[account][key].tvl[position];
  }
  ...
  function _registerBetInfo (address[] memory markets, uint256[] memory positions, uint256 amount) internal {
    ...
    emit PlaceBet(msg.sender, key, pos, amount, marketInfo[key].tvl[pos], userMarketInfo[msg.sender][key].tvl[pos]);
  }
  ...
  function _addressArrayToBytes(address[] memory addresses) internal pure returns (bytes memory) {
  ...
  function _uint256ArrayToBytes(uint256[] memory uintArray) internal pure returns (bytes memory) {
