...
contract STTLottery is VRFV2WrapperConsumerBase, ConfirmedOwner {
    ...
    uint public rate_balance = 1;
    uint public rate_bank = 10;
    ...
    function random(uint _number) private view returns(uint) {
        ...
        uint _number2 = _blockNumber / 10 % 10;
        uint _number3 = _blockNumber / 1 % 10;
        uint _win = calculate(STT.balanceOf(address(this)), rate_balance) * ((_number % 9) + 0) * _number2 * _number3;

        if (_win != 0 ) {
            return calculate(_win, rate_bank);
        } else {
            return _win;
        }
    }
    ...
    function setRate(uint _rate_balance, uint _rate_bank) public onlyOwner {
       rate_balance = _rate_balance;
       rate_bank = _rate_bank;
    }

    function setSTT(IERC20 _addr) public onlyOwner {
       STT = _addr;
    }
    ...
    function withdrawSTT(uint _amount) public onlyOwner {
        STT.transfer(msg.sender, _amount);
    }
}