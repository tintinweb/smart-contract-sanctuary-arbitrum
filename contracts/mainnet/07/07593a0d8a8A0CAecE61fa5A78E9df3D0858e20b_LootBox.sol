...
contract LootBox is ILootBoxStorage, Storage, AccessControl, Nft, Prize, Price, Sign, VRF, Jackpot, IERC721Receiver, IERC1155Receiver, Balance {
    /**
     ...
     * @notice deprecated, use PrizeClaimed instead
     */
    ...
    /**
     * @dev More generalized event to support any kind of prizes.
     * @param prizeType might be 0 = ERC20, 1 = ERC721, 2 = ERC1155
     */
    event PrizeClaimed(uint indexed lootBoxId, address indexed user, uint prizeType, address tokenContract, uint tokenId, uint amount, uint chainId);

    /**
     * @dev received a free loot box
     */
    event FreeTicketUsed(address indexed user, uint64 indexed externalId);
    ...
    function init(string memory name_, string memory symbol_, string memory baseUri_, uint256 maxIncome, uint32 begin, uint32 end, address signer) initializer public virtual override(Storage) {
        ...
        Storage.init(name_, symbol_, baseUri_, maxIncome, begin, end, signer);
    }
     ...
     * @dev By a specified amount of LootBoxes for the specified tokens.
     */
    function buyFor(address user, address token, uint count, uint boost) public onlyInitialized {
        if (boost < MIN_NFT_BOOST || boost > _config().maxBoost) {
            revert OutOfRange("boost", boost, MIN_NFT_BOOST, _config().maxBoost);
        }
        ...
        _addTotalIncome(totalValue);

        // check scope
        (bool isEarly, bool isLate, bool isMaxIncomeReached) = _isInScope();
        if (isEarly || isLate || isMaxIncomeReached) {
            revert OutOfScope(isEarly, isLate, isMaxIncomeReached);
        }
    ...
    function acquireFree(
        ...
        // TODO: use correct price; rethink the price concept at all
        _addTotalIncome(_price(0xaf88d065e77c8cC2239327C5EDb3A432268e5831));
        // check scope
        (bool isEarly, bool isLate, bool isSoldOut) = _isInScope();
        if (isEarly || isLate || isSoldOut) {
            revert OutOfScope(isEarly, isLate, isSoldOut);
        }
        ...
        // emit the event of using a free ticket
        emit FreeTicketUsed(user, externalId);
     ...
     */
    function burnForRandomPrize(uint rarity) public onlyInitialized {
        if (rarity >= RARITIES) {
            revert OutOfRange("rarity", rarity, 0, RARITIES - 1);
        }

        if (rarity == RARITY_GRAND_PRIZE) {
            (bool tooEarly, bool saleNotOver) = _isOutOfScope();
            if (tooEarly && saleNotOver) {
                revert OutOfScope(tooEarly, false, saleNotOver);
            }
        }
    ...
    function _randomClaimResponseHandler(IdType id, Random.Seed memory random) internal override {
        ...
        (bool result, PrizeInfo memory info) = _playPrize(rarity, random, user, thisChainId);
        ...
        emit PrizeClaimed(id.toTokenId(), user, info.prizeType, info.contractAddress, info.tokenId, info.amount, info.chainId);
    ...
    function _isInScope() private view returns (bool tooEarly, bool tooLater, bool soldOut) {
        Scope storage scope = _scope();

        return (
            block.timestamp < scope.begin,
            block.timestamp >= scope.end,
            scope.totalIncome > scope.maxIncome
        );
    }

    function _isOutOfScope() private view returns (bool tooEarly, bool saleIsNotOver) {
        Scope storage scope = _scope();
        uint256 totalIncome = scope.totalIncome;
        return (
            block.timestamp < scope.end,
            totalIncome < scope.maxIncome
        );
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(AccessControl, Nft, IERC165) returns (bool) {
