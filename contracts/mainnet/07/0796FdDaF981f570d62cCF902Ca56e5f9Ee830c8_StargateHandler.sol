...
import {IStargateComposer, IFactory, IPool} from "../external/stargate/IStargateComposer.sol";
import {IStargateReceiver} from "../external/stargate/IStargateReceiver.sol";
import "./BridgeHandlerBase.sol";

contract StargateHandler is BridgeHandlerBase, IStargateReceiver {
    using UniversalERC20 for IERC20;
    ...
    function performBridgeAction(
        ...
        (
            ...
            uint16 dstChainId
                ...
                (uint256, uint256, uint256, uint256, uint16)
            );

        bytes memory stargatePayload = abi.encode(
            BridgeRequest({
        ...
        bytes memory callTo = abi.encodePacked(params.callTo);

        (uint256 bridgeNativeFee, ) = getLayerZeroFee(
            stargatePayload,
            dstChainId,
            dstGasForCall,
            dstNativeAmount,
        ...
        _validateBridgeNativeFee(
            params.bridgeNativeFee,
            bridgeNativeFee,
            params.user
        );
        ...
        IStargateComposer(stargateComposer).swap{value: bridgeNativeFee}(
            dstChainId,
            srcPoolId, // source pool id
            dstPoolId, // dest pool id
            payable(params.recipient), // refund adddress. extra gas (if any) is returned to this address
            params.amount, // quantity to swap
            params.dstSwap.minAmountIn, // the min qty you would accept on the destination
            IStargateComposer.lzTxObj(dstGasForCall, dstNativeAmount, callTo),
            callTo, // the address to send the tokens to on the destination
            stargatePayload // bytes param, if you wish to send additional payload you can abi.encode() them here
        );
    ...
    function sgReceive(
        ...
        if (msg.sender != stargateComposer) {
            revert Forbidden();
        }

        _executeAction(payload, token, amountLD);
    }
    ...
    function getLayerZeroFee(
        ...
        return
            IStargateComposer(stargateComposer).quoteLayerZeroFee(
                dstChainId,
