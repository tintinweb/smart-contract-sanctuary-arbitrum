/**
 *Submitted for verification at Arbiscan on 2022-09-06
*/
...
contract NPairPool_in is Ownable {
    ...
    event PairLocked(uint256 indexed _id, address _srcToken, address _destToken, uint256 _timestamp);
    event PairUnlocked(uint256 indexed _id, address _srcToken, address _destToken, uint256 _timestamp);
    /* WRITE METHODS*/
    /*
    ...
    */
    function listNewPair(address _srcToken, uint256 _srcDecimals, address _destToken, uint256 _destDecimals) public onlyOwner returns(bool) {
        ...
        for(i=1; i<=totalPairs; i++){
            if(pairs[i].srcToken == _srcToken && pairs[i].destToken == _destToken){
                error = true;
                i = totalPairs;
            }
        }
        require(!error, "NEON: Token pair already listed");
        pairs[totalPairs + 1].srcToken = _srcToken;
        pairs[totalPairs + 1].destToken = _destToken;
        pairs[totalPairs + 1].srcDecimals = _srcDecimals;
        pairs[totalPairs + 1].destDecimals = _destDecimals;
        pairs[totalPairs + 1].active = true;
        totalPairs = totalPairs.add(1);
    ...
    * @return bool successfully completed
    */
    function lockPair(uint256 _id) external onlyOwner returns(bool){
        ...
        pair.active = false;
        emit PairLocked(_id, pair.srcToken, pair.destToken, block.timestamp);
        return true;
    }
    /*
    * @dev Unlock a pair
    ...
    * @return bool successfully completed
    */
    function unlockPair(uint256 _id) external onlyOwner returns(bool){
        require(_id > 0, "NEON: ID must be > 0");
        require(_id <= totalPairs, "NEON: ID out of range");
        ...
        pair.active = true;
        emit PairUnlocked(_id, pair.srcToken, pair.destToken, block.timestamp);
        return true;
    }
    /*
    * @dev download pair list from another pool
    * () will be defined the unit of measure
    * @param _contract address of the old pool where pair list will be downloaded
    * @return bool successfully completed
    */
    function downloadPairList(address _contract) external onlyOwner returns(bool){
        INPairPool_in pairPool = INPairPool_in(_contract);
        uint256 poolTotalPairs = pairPool.totalPairs();
        uint256 i;
