...
contract UniWPool is TokenRecover {
    using SafeMath for uint256;
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if(depositFeeRate>0)
            {
                uint256 depositFeeAmount = _amount.mul(depositFeeRate).div(10000);
                _amount = _amount.sub(depositFeeAmount);
                pool.lpToken.safeTransfer(treasuryAddress, depositFeeAmount);

                emit DepositFee(treasuryAddress, depositFeeAmount);
            }
        }
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if(depositFeeRate>0)
            {
                uint256 depositFeeAmount = _amount.mul(depositFeeRate).div(1000);
                _amount = _amount.sub(depositFeeAmount);
                pool.lpToken.safeTransfer(treasuryAddress, depositFeeAmount);
    ...
    // Add a function to update TreasuryAddress. Can only be called by the owner.
