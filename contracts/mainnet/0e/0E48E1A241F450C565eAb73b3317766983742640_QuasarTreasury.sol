...
contract QuasarTreasury is Ownable {
    ...
    address public SQUA;
    uint public SQUAQueue; // Delays change to SQUA address
    ...
    /**
        @notice allow approved address to deposit an asset for QUA
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint QUA needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( QUA ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn QUA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IQUAERC20( QUA ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( SQUA ).balanceOf( msg.sender ); // Can only borrow against SQUA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with QUA
        @param _amount uint
     */
    function repayDebtWithQUA( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IQUAERC20( QUA ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, QUA, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( QUA ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( QUA ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns QUA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match QUA decimals
            value_ = _amount.mul( 10 ** IERC20( QUA ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SQUA ) { // 9
            SQUAQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
