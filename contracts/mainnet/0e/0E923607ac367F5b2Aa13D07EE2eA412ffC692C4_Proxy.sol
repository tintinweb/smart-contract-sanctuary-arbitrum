...
contract Proxy is IAccessControlHolder, ProxyOFT {
    using SafeERC20 for IERC20;

    error OnlyOFTManagerAccess();

    IAccessControl public override acl;

    bytes32 public constant OFT_MANAGER = keccak256("OFT_MANAGER");

    modifier onlyOFTManagerAccess() {
        if (!acl.hasRole(OFT_MANAGER, msg.sender)) {
            revert OnlyOFTManagerAccess();
        }
        _;
    }
    ...
    function withdraw(
        uint256 amount,
        address to
    ) external onlyOFTManagerAccess {
        innerToken.safeTransfer(to, amount);
    }

    function withdrawAll(address to) external onlyOFTManagerAccess {
        innerToken.safeTransfer(to, innerToken.balanceOf(address(this)));
    }

    function enforceClearaFailedMessage(
        uint16 _srcChainId,
        bytes calldata _srcAddress,
        uint16 _nonce
    ) external onlyOFTManagerAccess {
