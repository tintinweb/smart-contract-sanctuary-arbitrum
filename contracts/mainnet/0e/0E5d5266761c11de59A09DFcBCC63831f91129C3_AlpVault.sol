...
contract AlpVault is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable, IAlpVault {
    ...
    function initialize(address _water, address _usdc, address _vkHandler) external initializer {
        ...
        water = IWater(_water);
        USDC = _usdc;
        strategyAddresses.VaultkaHandler = _vkHandler;
    ...
    function setVaultkaHandler(address _vkHandler) external onlyOwner {
        strategyAddresses.VaultkaHandler = _vkHandler;
        emit SetHandler(_vkHandler);
    }
    ...
    function setFeeConfiguration(
        ...
        emit SetFeeConfiguration(_feeReceiver, _withdrawalFee, _waterFeeReceiver, _liquidatorsRewardPercentage);
    }
    ...
    function getUpdatedDebt(
        ...
        currentDTV = (leverageWithInterests * strategyMisc.DECIMAL) / currentPosition;
    ...
    function handleAndCompoundRewards() public returns (uint256) {
        ...
        if (balance > 0) {
            ...
            emit RewardDistributed(balance, toOwner, toWater, toVodkaUsers);
    ...
    function openPosition(address _token, uint256 _amount, uint256 _leverage) external whenNotPaused nonReentrant{
        ...
        pts.vaultSnapshotID = IHandler(strategyAddresses.VaultkaHandler).vaultCurrentSnapshotID(address(water));
    ...
    function closePosition(uint256 _positionID, address _user) external InvalidID(_positionID, _user) nonReentrant {
        ...
        if (closeData.returnedValue < _userInfo.leverageAmount) {
            ...
            IERC20Upgradeable(USDC).safeTransfer(feeConfiguration.waterFeeReceiver, closeData.totalInt);
            _userInfo.liquidator = msg.sender;
            _userInfo.liquidated = true;
        } else {
            closeData.waterRepayment = _userInfo.leverageAmount;
    ...
    function liquidatePosition(uint256 _positionID, address _user) external nonReentrant {
        ...
        if (closeData.returnedValue < closeData.fulldebtValue) {
            ...
            liquidatorReward = (closeData.toLeverageUser * feeConfiguration.liquidatorsRewardPercentage) / strategyMisc.MAX_BPS;
            ...
            IERC20Upgradeable(USDC).safeTransfer(_user, closeData.toLeverageUser);
            IERC20Upgradeable(USDC).safeTransfer(feeConfiguration.waterFeeReceiver, closeData.totalInt);
        }

        IERC20Upgradeable(USDC).safeIncreaseAllowance(address(water), lendingRepayAmount);
        bool success = water.repayDebt(_userInfo.leverageAmount, lendingRepayAmount);
        require(success, "Water: Repay failed");

        PositionTimestamps storage pts = positionTimestamps[_user][_positionID];
        pts.closeTimestamp = block.timestamp;

        emit Liquidated(_user, _positionID, msg.sender, closeData.returnedValue, liquidatorReward, block.timestamp);
    }
...
interface IAlpVault {
    ...
    event SetHandler(address indexed _handler);
