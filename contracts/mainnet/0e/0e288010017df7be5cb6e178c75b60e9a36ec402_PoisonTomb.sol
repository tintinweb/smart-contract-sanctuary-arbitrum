...
contract PoisonTomb is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 POISON-WETH LP -> POISON pool
    // uint256 public constant INITIAL_POISON_POOL_DISTRIBUTION = 0 ether;
    ...
    address public poisonTombOracle;
    ...
    /**
     * @notice Constructs the POISON ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress, uint256 _sellStartTime) public ERC20("Poison Tomb Token", "POISON") {
        // Mints 10 POISONTOMB to contract creator for initial pool setup
    ...
    function _getPoisonTombPrice() internal view returns (uint256 _poisonTombPrice) {
        try IOracle(poisonTombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("PoisonTomb: failed to fetch POISONTOMB price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _poisonTombPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_poisonTombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPoisonTombOracle(address _poisonTombOracle) public onlyOperatorOrTaxOffice {
        require(_poisonTombOracle != address(0), "oracle address cannot be 0 address");
        poisonTombOracle = _poisonTombOracle;
    }
    ...
    /**
     * @notice Operator mints POISONTOMB to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of POISONTOMB to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPoisonTombPrice = _getPoisonTombPrice();
            currentTaxRate = _updateTaxRate(currentPoisonTombPrice);
            if (currentPoisonTombPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // address _poisonTombPool
        // address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        // require(_poisonTombPool != address(0), "!_poisonTombPool");
        // require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_PoisonTombPool, INITIAL_POISONTOMB_POOL_DISTRIBUTION);
    ...
    function getRemainingTimeForPoisonTombSellingEnable() public view returns(uint256) {
        ...
        /// @dev overrides transfer function to meet tokenomics of PoisonTomb
