...
contract SwitchNxtp is Switch {
    ...
    event SetTransactionManager(address transactionManager);
    ...
    function setTransactionManager(address _newTransactionManager) external onlyOwner {
        ...
        transactionManager = ITransactionManager(_newTransactionManager);
        emit SetTransactionManager(_newTransactionManager);
    }
        ...
        returns (ITransactionManager.TransactionData memory)
    {
        ...
        if (!native) {
            IERC20(transferArgs.fromToken).safeApprove(transactionManagerAddress, 0);
            IERC20(transferArgs.fromToken).safeApprove(transactionManagerAddress, amountAfterFee);
        }
        ...
        returns (ITransactionManager.TransactionData memory)
    {
        ...
        uint256 amountAfterFee = _getAmountAfterFee(IERC20(transferArgs.srcSwap.srcToken), transferArgs.amount, transferArgs.partner, transferArgs.partnerFeeRate);
        ...
        require(returnAmount >= transferArgs.minReturn, "return amount was not enough");

        if (returnAmount > 0) {
            IERC20(transferArgs.srcSwap.dstToken).safeApprove(transactionManagerAddress, 0);
            IERC20(transferArgs.srcSwap.dstToken).safeApprove(transactionManagerAddress, returnAmount);
