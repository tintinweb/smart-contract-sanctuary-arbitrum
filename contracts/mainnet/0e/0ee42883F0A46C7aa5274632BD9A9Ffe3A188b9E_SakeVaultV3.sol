...
contract SakeVaultV3 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    mapping(address => mapping(uint256 => uint256)) private maturityDay;
    ...
    /** ----------- View functions ------------- */
    function getMaturityDay(address _user, uint256 _positionId) external view returns(uint256) {
        return maturityDay[_user][_positionId];
    }
    ...
    function openPosition(
        ...
        UserInfo memory _userInfo = UserInfo({
            ...
            cooldownPeriodElapse: block.timestamp +
                ISettingManager(strategyAddresses.velaStakingVault).cooldownDuration(),
        ...
        maturityDay[msg.sender][userPositionId] = block.timestamp + vaultMaturityTime;
    ...
    function closePosition(
        ...
        if (allowedClosers[msg.sender]) {
            require(block.timestamp >= maturityDay[_user][_positionID], "SAKE:Wait for Mature date");
        }
