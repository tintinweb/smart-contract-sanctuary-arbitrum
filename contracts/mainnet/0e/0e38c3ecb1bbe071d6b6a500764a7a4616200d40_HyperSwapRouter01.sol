...
library HyperSwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'4dedc36eef382578bf9f3bb38faba39ec2ea831428b99a4cf531ec5c3a657af8' // init code hash
            ))));
    }
...
contract HyperSwapRouter01 is IHyperSwapRouter01 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            IHyperSwapPair(HyperSwapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        uint amountIn,
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, HyperSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public pure override returns (uint amountIn) {
        return HyperSwapLibrary.getAmountOut(amountOut, reserveIn, reserveOut);
    }
