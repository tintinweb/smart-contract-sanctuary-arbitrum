...
contract PancakeSwapLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    address public immutable sidechainRandomGenerator;
    ...
    uint256 public constant MIN_LENGTH_LOTTERY = 9 minutes; // 4 hours
    uint256 public constant MAX_LENGTH_LOTTERY = 11 minutes; // 4 days
     ...
     * @param _sidechainRandomGenerator: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _cakeTokenAddress, address _sidechainRandomGenerator) {
        cakeToken = IERC20(_cakeTokenAddress);
        sidechainRandomGenerator = _sidechainRandomGenerator;
        ...
        nonReentrant
    {
        ...
        require(_lotteries[_lotteryId].finalNumber > 0, "Numbers not drawn");
    ...
    function startLottery(
        ...
        CallProxy(anyCallAddress).anyCall(
            sidechainRandomGenerator,
            abi.encode(currentLotteryId, _endTime),
            address(0), // we don't have fallback
            137, // polygon
            0 // fee paid on source chain
        );
    }

    function testAnyCall(uint lotteryId, uint flags) external payable {
        CallProxy(anyCallAddress).anyCall{value: msg.value}(
            sidechainRandomGenerator,
            abi.encode(lotteryId, block.timestamp),
            address(0), // we don't have fallback
            137, // polygon
            flags // fee paid on dest chain
        );
    ...
    function calcAnycallGas(uint lotteryId) public view returns (uint256) {
        bytes memory data = abi.encode(lotteryId, block.timestamp);
        uint256 gasFees = CallProxy(anyCallAddress).calcSrcFees("0", 137, data.length); // calculate gas fees
        return gasFees;
    }
    ...
    function setFinalNumber(uint256 _lotteryId, uint32 _number) external onlyOperator {
      _lotteries[_lotteryId].finalNumber = _number;
    }

    uint8 public testMode = 0;

    function setTestMode(uint8 _testMode) external onlyOwner {
      testMode = _testMode;
    }
      ...
      returns (bool success, bytes memory result)
    {
        ...
        require(from == sidechainRandomGenerator, "Random generator address wrong.");
        require(fromChainId == 137, "SRC chain not polygon.");
        ...
        require(_lotteries[lotteryId].endTime < block.timestamp, 'Lottery endTime not passed.');
        require(_lotteries[lotteryId].finalNumber == 0, 'FinalNumber already set.');
        
        _lotteries[lotteryId].finalNumber = uint32(result);
  }
