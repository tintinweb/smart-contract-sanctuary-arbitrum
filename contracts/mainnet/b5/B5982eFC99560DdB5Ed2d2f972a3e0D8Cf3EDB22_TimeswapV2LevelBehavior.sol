...
import {IFarmingLevelMasterV2} from "./interfaces/IFarmingLevelMasterV2.sol";
import {ITimeswapV2LevelBehavior} from "./interfaces/ITimeswapV2LevelBehavior.sol";
...
abstract contract TimeswapV2Behavior is ITimeswapV2Behavior, ERC1155, ERC20 {
  ...
  // function transferFromMsgSenderAndApprovePeriphery(address token, address periphery, uint256 amount) private {
  //   IERC20(token).safeTransferFrom(msg.sender, address(this), amount);
  //   uint256 allowance = IERC20(token).allowance(address(this), periphery);
  //   if (allowance < amount) IERC20(token).approve(periphery, type(uint256).max);
  // }
  ...
  function lendGivenPrincipal(
    ...
    // if (param.isToken0) {
    //   if (address(this) < param.token) _mint(address(this), param.tokenAmount);
    //   else {
    //     IERC20(param.token).safeTransferFrom(msg.sender, address(this), param.tokenAmount);
    //     uint256 allowance = IERC20(param.token).allowance(address(this), timeswapV2LendGivenPrincipal);
    //     if (allowance < param.tokenAmount) IERC20(param.token).approve(timeswapV2LendGivenPrincipal, type(uint256).max);
    //   }
    // } else {
    //   if (address(this) > param.token) _mint(address(this), param.tokenAmount);
    //   else {
    ...
    if ((param.isToken0 && (address(this) < param.token)) || (!param.isToken0 && (address(this) > param.token))) 
    ...
    if ((param.isToken0 && (address(this) < param.token)) || (!param.isToken0 && (address(this) > param.token)))
      _mintRequirement(param.tokenAmount);


    // if (param.isToken0) {
    //   if (address(this) < param.token) _mintRequirement(param.tokenAmount);
    // } else {
    //   if (address(this) > param.token) _mintRequirement(param.tokenAmount);
    // }
  }
  ...
  function borrowGivenPrincipal(
    ...
    if (param.isLong0 == param.isToken0) {
      if ((param.isLong0 && (address(this) < param.token)) || (!param.isLong0 && (address(this) > param.token)))
        ...
        uint256 allowance = IERC20(param.token).allowance(address(this), timeswapV2BorrowGivenPrincipal);
        if (allowance < param.maxPositionAmount - param.tokenAmount)
          IERC20(param.token).approve(timeswapV2BorrowGivenPrincipal, type(uint256).max);
      }
    } else {
      if ((param.isLong0 && (address(this) < param.token)) || (!param.isLong0 && (address(this) > param.token)))
        ...
        uint256 allowance = IERC20(param.token).allowance(address(this), timeswapV2BorrowGivenPrincipal);
        if (allowance < param.maxPositionAmount)
          IERC20(param.token).approve(timeswapV2BorrowGivenPrincipal, type(uint256).max);
      }
    }
    ...
    // if (param.isLong0 == param.isToken0) {
    //   if (param.isLong0) {
    //     if (address(this) < param.token) _mint(address(this), param.maxPositionAmount - param.tokenAmount);
    //     else {
    //       IERC20(param.token).safeTransferFrom(msg.sender, address(this), param.maxPositionAmount - param.tokenAmount);
    //       uint256 allowance = IERC20(param.token).allowance(address(this), timeswapV2BorrowGivenPrincipal);
    //       if (allowance < param.maxPositionAmount - param.tokenAmount)
    ...
    // } else {
    //   if (param.isLong0) {
    //     if (address(this) < param.token) _mint(address(this), param.maxPositionAmount);
    //     else {
    //       IERC20(param.token).safeTransferFrom(msg.sender, address(this), param.maxPositionAmount);
    //       uint256 allowance = IERC20(param.token).allowance(address(this), timeswapV2BorrowGivenPrincipal);
    //       if (allowance < param.maxPositionAmount)
    ...
    // if (param.isLong0) {
    //   if (address(this) < param.token) _burn(address(this), param.maxPositionAmount - positionAmount);
    ...
    if (param.isLong0 == param.isToken0) {
      ...
      if ((param.isToken0 && (address(this) < param.token)) || (!param.isToken0 && (address(this) > param.token)))
        ...
        IERC20(param.token).safeTransfer(param.tokenTo, param.tokenAmount);
    }
    ...
    // if (param.isLong0 == param.isToken0) {
    //   if (param.isLong0) {
    //     if (address(this) < param.token) _mintRequirement(positionAmount - param.tokenAmount);
    //   } else {
    //     if (address(this) > param.token) _mintRequirement(positionAmount = param.tokenAmount);
    //   }
    // } else {
    //   if (param.isLong0) {
    //     if (address(this) < param.token) _mintRequirement(positionAmount);
    //   } else {
    //     if (address(this) > param.token) _mintRequirement(positionAmount);
    //   }
  ...
  function closeBorrowGivenPosition(
    ...
    // if (param.isLong0 != param.isToken0) {
    //   if (param.isToken0) {
    //     if (address(this) < param.token) _mint(address(this), param.maxTokenAmount);
    //     else {
    //       IERC20(param.token).safeTransferFrom(msg.sender, address(this), param.maxTokenAmount);
    //       uint256 allowance = IERC20(param.token).allowance(address(this), timeswapV2CloseBorrowGivenPosition);
    //       if (allowance < param.maxTokenAmount)
    //         IERC20(param.token).approve(timeswapV2CloseBorrowGivenPosition, type(uint256).max);
    //     }
    //   } else {
    //     if (address(this) > param.token) _mint(address(this), param.maxTokenAmount);
    //     else {
    ...
    // if (param.isLong0 != param.isToken0) {
    //   if (param.isToken0) {
    //     if (address(this) < param.token) _burn(address(this), param.maxTokenAmount - tokenAmount);
    ...
    if (param.isLong0 == param.isToken0) {
      ...
      } else IERC20(param.token).safeTransfer(param.to, param.positionAmount);
    }

    // if (param.isLong0 == param.isToken0) {
    //   if (param.isLong0) {
    //     if (address(this) < param.token) _burnRequirement(param.to, param.positionAmount - tokenAmount);
    //     else IERC20(param.token).safeTransfer(param.to, param.positionAmount - tokenAmount);
    //   } else {
    //     if (address(this) > param.token) _burnRequirement(param.to, param.positionAmount - tokenAmount);
    //     else IERC20(param.token).safeTransfer(param.to, param.positionAmount - tokenAmount);
