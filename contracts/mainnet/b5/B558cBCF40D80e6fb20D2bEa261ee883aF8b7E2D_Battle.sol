...
contract Battle {
    ...
    function moveTo(uint256 _wizId, uint256[] memory _movesX, uint256[] memory _movesY) external {
        ...
        // action
        Action memory moveAction = Action(0,uint8(currX),uint8(currY),uint8(_movesX[_movesX.length-1]),uint8(_movesY[_movesY.length-1]),0,0);
        ActionList storage actionList = playerActionLists[_wizId];
        actionList.actions.push(moveAction);
        actionList.gameId = uint32(currGame.gameId);
    } 

    // called from CombatSpell.sol to register action
    function createSpellAction(uint256 _wizId, uint256 _originX, uint256 _originY, uint256 _spellId,uint256 _power, uint256 _gameId)public {
         // action
        ...
        ActionList storage actionList = playerActionLists[_wizId];
        actionList.actions.push(moveAction);
        actionList.gameId = uint32(_gameId);
    }
        ...
        uint256 counter=0;
         for(uint x = 0;x<mapSize;x++){
            for(uint y = 0;y<mapSize;y++){
                if(_shape[x][y]<1){
                    int offsetX = int(x+_originX)-halfSize; // 8
                    int offsetY = int(y+_originY)-halfSize; // 8
    ...
    function setSize(uint256 _newSize, int256 _newHalfSize) external{
        mapSize = _newSize;
        halfSize = _newHalfSize;
    }
