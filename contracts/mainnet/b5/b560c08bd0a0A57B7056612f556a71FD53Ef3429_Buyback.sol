/**
 *Submitted for verification at Arbiscan on 2022-10-04
*/

// SPDX-License-Identifier: MIT
// File: @openzeppelin/[email protected]/math/SafeMathUpgradeable.sol
...
// File: @openzeppelin/[email protected]/token/ERC20/SafeERC20Upgradeable.sol
...
// File: @openzeppelin/[email protected]/proxy/Initializable.sol
...
// File: @openzeppelin/[email protected]/utils/ReentrancyGuardUpgradeable.sol
...
// File: @openzeppelin/[email protected]/token/ERC20/ERC20BurnableUpgradeable.sol
...
interface IOracle {
    ...
    function getUSDsPrice() external view returns (uint256);

    function getSPAprice_prec() external view returns (uint256);

    function getUSDsPrice_prec() external view returns (uint256);
}
    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function distributeAndBurnSPA() external onlyOwner nonReentrant {
        uint256 balance = ERC20BurnableUpgradeable(SPA).balanceOf(
            address(this)
        );

        if (rewardPercentage == MAX_PERCENTAGE) {
            emit SPARewarded(balance);
            ERC20BurnableUpgradeable(SPA).safeTransfer(veSpaRewarder, balance);
        } else {
            // Calculating the amount to reward based on rewardPercentage
            uint256 toReward = balance.mul(rewardPercentage).div(
                MAX_PERCENTAGE
            );

            // Remaining balance will be burned
            uint256 toBurn = balance.sub(toReward);
     ...
     */
    function updateRewardPercentage(uint256 _newRewardPercentage)
        external
        onlyOwner
    {
     ...
     */
    function getSPAReqdForUSDs(uint256 _usdsAmount)
        external
        view
        returns (uint256)
    {
        ...
        (
            uint256 usdsPrice,
            uint256 spaPrice,
            uint256 usdsPrice_prec,
            uint256 spaPrice_prec
        ) = _getOracleData();

        // Calculates the total USDs value
        uint256 totalUsdsValue = _usdsAmount.mul(usdsPrice).div(usdsPrice_prec);

        // Calculates spa amount required
        uint256 spaAmtReqd = totalUsdsValue.mul(spaPrice_prec).div(spaPrice);
    ...
    function buyUSDs(
        ...
        require(usdsToSend > 0, "SPA Buyback: SPA Amount too low");
        require(
            usdsToSend >= _minUSDsOut,
            "SPA Buyback: Slippage more than expected"
        );
        require(
            usdsToSend <=
                ERC20BurnableUpgradeable(USDS).balanceOf(address(this)),
            "SPA Buyback: Insufficient USDs balance"
        );
     ...
     * @return Amount of USDs user will get
     */
    function _getUsdsOutForSpa(uint256 _spaIn)
        private
        view
        returns (uint256, uint256)
    {
        ...
        uint256 totalSpaValue = _spaIn.mul(spaPrice).div(spaPrice_prec);

        // Divides SPA Value by USDs price
        uint256 usdsOut = totalSpaValue.mul(usdsPrice_prec).div(usdsPrice);
        ...
        )
    {
        ...
        uint256 usdsPrice = IOracle(ORACLE).getUSDsPrice();
        uint256 spaPrice = IOracle(ORACLE).getSPAprice();

        // Fetches the precision for SPA and USDS from ORACLE
        uint256 usdsPrice_prec = IOracle(ORACLE).getUSDsPrice_prec();
        uint256 spaPrice_prec = IOracle(ORACLE).getSPAprice_prec();

        return (usdsPrice, spaPrice, usdsPrice_prec, spaPrice_prec);
    }

    function _isValidAddress(address _address) private pure {
        require(_address != address(0), "SPA Buyback: Invalid Address");
    }

    function _isValidAmount(uint256 _amount) private pure {
        require(_amount != 0, "SPA Buyback: Invalid Amount");
    }
