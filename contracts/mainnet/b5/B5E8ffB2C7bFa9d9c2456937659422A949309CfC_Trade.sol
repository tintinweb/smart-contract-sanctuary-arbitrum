...
library GMX {
    ...
    function gmxIncreasePosition(
        ...
        require(trade.servicesEnabled()[0], "T/FS"); // forbidden service
        WhitelistUtils.checkTokenAllowance(indexToken, trade);
    ...
    function gmxDecreasePosition(
        ...
        if (receiveToken != usdt) {
            ...
            WhitelistUtils.checkTokenAllowance(receiveToken, trade);
        } else {
...
library GMXV2 {
    ...
    function createOrder(
        ...
        bool isSwap = _params.orderType == Order.OrderType.MarketSwap
            || _params.orderType == Order.OrderType.LimitSwap;
        require(!isSwap, "T/OTNS");
        bool isIncrease = _params.orderType == Order.OrderType.MarketIncrease
            || _params.orderType == Order.OrderType.LimitIncrease;
        bool isAlwaysAllowedOperation = !isIncrease;
        require(isAlwaysAllowedOperation || _trade.servicesEnabled()[0], "T/FS");
        require(isAlwaysAllowedOperation || _trade.status() == FundState.Opened, "T/FC");
...
contract Trade is ITrade, TradeStorage, ReentrancyGuardUpgradeable {
    ...
    function gmxDecreasePosition(
        ...
        uint256 acceptablePrice, // usd amount [1e6]
        uint256 executionFee
    ) external payable saveDebt saveDebtPayable override {
        GMX.gmxDecreasePosition(
            collateralToken,
            indexToken,
            receiveToken,
            collateralDelta,
