...
contract TestDistributor is Ownable {
    ...
    uint256 public constant INIT_CLAIM = 80_397_000 * 1e6;
    ...
    function claim(uint128 nonce, bytes calldata signature, address referrer) external {
        require(isStarted, "TEST: claim not started");
        require(_usedNonce[nonce] == false, "TEST: nonce already used");
        require(_claimedUser[_msgSender()] == false, "TEST: already claimed");

        _claimedUser[_msgSender()] = true;
        require(isValidSignature(nonce, signature), "TEST: only auth claims");

        _usedNonce[nonce] = true;

        uint256 supplyPerAddress = canClaimAmount();
        require(supplyPerAddress >= 1e6, "TEST: airdrop has ended");
    ...
    function setSigner(address val) external onlyOwner() {
        require(val != address(0), "TEST: val is the zero address");
        signer = val;
    }

    function start(address _tokenAddress) external onlyOwner() {
        require(signer != address(0), "TEST: set singer before start");
        require(!isStarted, "TEST: already started");
    ...
    function withdraw(address _token, uint256 _amount) external onlyOwner {
        if (_token != address(0)) {
            if (_token == address(this)) {
                require(block.timestamp >= endTime, "TEST: claim not ended");
            }
