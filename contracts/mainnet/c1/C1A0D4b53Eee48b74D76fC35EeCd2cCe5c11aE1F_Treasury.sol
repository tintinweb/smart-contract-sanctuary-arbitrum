...
contract Treasury is Operators {
    ...
    address public sVNTR;
    uint public sVNTRQueue; // Delays change to sVNTR address
    ...
    constructor (
        ...
        isReserveToken[ _sVNTR ] = true;
        reserveTokens.push( _sVNTR );
        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for VNTR
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint VNTR needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( VNTR ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn VNTR for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IVNTRERC20( VNTR ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sVNTR ).balanceOf( msg.sender ); // Can only borrow against sVNTR held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with VNTR
        @param _amount uint
     */
    function repayDebtWithVNTR( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IVNTRERC20( VNTR ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, VNTR, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( VNTR ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( VNTR ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns VNTR valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match VNTR decimals
            value_ = _amount.mul( 10 ** IERC20( VNTR ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyOperator() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.sVNTR ) { // 9
            sVNTRQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
