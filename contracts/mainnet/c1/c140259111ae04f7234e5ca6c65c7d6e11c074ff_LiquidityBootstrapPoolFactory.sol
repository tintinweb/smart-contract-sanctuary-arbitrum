...
contract LiquidityBootstrapPoolFactory is Ownable {
    ...
    /// @dev Error thrown when the redemption delay is greater than 1 day.
    error InvalidDelay();

    /// @dev Error thrown when the fee recipient is invalid.
    ...
    constructor(
        ...
        factorySettings = FactorySettings(_feeRecipient, _platformFee, _swapFee);
    ...
    {
        // Check that share and asset address are not the same.
        // Check that share and asset address are not 0x.
        ...
        // Check the sale time duration must be at least 1 day.
        // Check the sale can only be ended after at least 1 day from current block time.
        if (
        ...
        if (args.saleEnd < args.vestEnd) {
            // saleEnd will consider as vestStart if pool has vesting.
            // vestCliff must be greater than or qual to saleEnd (vestStart).
            if (args.saleEnd > args.vestCliff) {
                revert InvalidVestCliff();
            }

            // vestEnd must greater than vestCliff.
            if (args.vestCliff >= args.vestEnd) {
                revert InvalidVestEnd();
            }
        }

        // Check that weightStart and weightEnd in range [0.01 ether, 0.99 ether]
        if (
        ...
        if (args.redemptionDelay > 1 days) {
            revert InvalidDelay();
        }

        // Check that both assets and virtualAssets can not be 0 at the same time.
        if (assets == 0 && args.virtualAssets == 0) revert InvalidAssetValue();

        if (assets == 0) {
            // Zero assets pool is called zero collateral pool.
            // Zero collateral pool will be created with only virtualAssets.
            // The virtualAssets is a parameter that will be used to calculate the share price.
            // In order to mimics the real assets balance, virtualAssets should have the same decimals as the real assets.
            uint8 decimal = ERC20(args.asset).decimals();
            // Do not allow Zero collateral pool to be created with less than 1 virtualAssets.
            if (args.virtualAssets < 10 ** decimal) revert InvalidAssetValue();
        }

        pool = implementation.cloneDeterministic(_encodeImmutableArgs(args), salt);

        // Transfer both shares and asset to create new pool.
        // Sender must require to approve both shares and asset before calling this function.
    ...
    function modifySettings(
        ...
        onlyOwner
    {
        // Both fees must be less than or equal to MAX_FEE_BIPS (~ 10%).
        ...
        factorySettings = FactorySettings(feeRecipient, platformFee, swapFee);
