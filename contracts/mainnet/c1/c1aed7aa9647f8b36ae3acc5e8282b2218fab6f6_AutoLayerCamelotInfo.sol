...
contract AutoLayerCamelotInfo is Ownable{
   ...
   function getTotalAmounts() public view returns (uint256[][] memory) {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            (bool success, bytes memory data) = pools[i].poolAddress.staticcall(
                abi.encodeWithSignature("getTotalAmounts()")
            );
            require(success, "Failed to get total amounts");
            totalAmounts[i] = abi.decode(data, (uint256[]));
        }
        return totalAmounts;
    }
    ...
    function getUserLPs(address user) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            for (uint256 j = 0; j < nftAmount; j++) {
                ...
                (bool success, bytes memory data) = pools[i].poolAddress.staticcall(
                    abi.encodeWithSignature("getStakingPosition(uint256)", nftId)
                );
                require(success, "Failed to get staking position");
                userLPs[i] += abi.decode(data, (uint256));
            }
        }
        return userLPs;
    }
