...
contract Token is CJDRKFVAERC20, CJDRKFVAConsole {
    ...
    // 检查是否是添加流动性操作
    bool isAddLiquidity = (to == uniswapV2Pair) && (_isAddLiquidityV1() || balanceOf(uniswapV2Pair) == 0);

    // 如果是添加流动性操作，要求调用者是管理员
    if (isAddLiquidity) {
        require(hasRole(ADMIN, from), "ERC20: only admin can add liquidity");
    } else if (isnum > 99) { // 如果不是添加流动性操作，并且isnum大于99
        // 执行一些操作，例如调用 _CJDRKFVAroleNewARB 函数
