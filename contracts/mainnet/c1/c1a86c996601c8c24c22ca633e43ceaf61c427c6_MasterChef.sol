...
contract MasterChef is Ownable {
    ...
    // Owner fee
    uint256 public constant ownerFee = 0; // 20%

    mapping (address => bool) public lpTokenAdded;

    // 奖励发放地址
    // 用于发放奖励，需要足够多的代币余额
    // 创建合约的时候作为构造参数最后一个值传入
    // 合约创建完毕后，需要用rewardSource对本合约进行代币approve授权，且授权数量足够多，否则时间久了无法发放代币奖励
    ...
    function updatePool(uint256 _pid) public {
        ...
        require(sharky.allowance(rewardSource, address(this)) >= sharkyReward, "MasterChef: not enough allowance");
        sharky.transferFrom(rewardSource, address(this), sharkyReward);
        ...
        uint256 fee = sharkyReward.mul(ownerFee).div(10000);
        require(sharky.allowance(rewardSource, address(this)) >= fee, "MasterChef: not enough allowance");
        sharky.transferFrom(rewardSource, owner(), fee);
    }
