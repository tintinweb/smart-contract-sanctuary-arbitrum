...
import "../../interfaces/external/gmd/IGmdVaultV2.sol";

import "../../libraries/TokenUtils.sol";
import "../../libraries/Checker.sol";

/// @title  GmdTokenAdapterV2
/// @dev this is for GMD v2 vaults (1 line change from v1 adapter)
/// @author Savvy DeFi
contract GmdTokenAdapterV2 is
  ITokenAdapter,
  Initializable,
  Ownable2StepUpgradeable
{
  ...
  function initialize(address _vaultAddress, uint256 _pid) public initializer {
    ...
    (baseToken, token, , , , , , , , , , ) = IGmdVaultV2(_vaultAddress)
      .poolInfo(_pid);
  ...
  function price() external view override returns (uint256) {
    uint256 gdPrice = IGmdVaultV2(vaultAddress).GDpriceToStakedtoken(pid);
  ...
  function _deposit(
    ...
    IGmdVaultV2(vaultAddress).enter(amount, pid);
  ...
  function _withdraw(
    ...
    IGmdVaultV2(vaultAddress).leave(amount, pid);
  ...
  function _checkPoolId() internal view {
    (address curBaseToken, address curToken, , , , , , , , , , ) = IGmdVaultV2(
      vaultAddress
    ).poolInfo(pid);
