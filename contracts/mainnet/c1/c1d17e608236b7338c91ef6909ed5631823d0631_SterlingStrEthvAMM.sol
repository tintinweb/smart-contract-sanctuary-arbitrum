...
// File contracts/SterlingStrEthvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract SterlingStrEthvAMM is Ownable, Pausable {
   ...
   //Withdraws funds from masterchef
    ...
    function chargeFees(address caller) internal {
        uint256 strBal = IERC20(str).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;

        uint256 callFee = strBal * CALL_FEE / FEE_DIVISOR;
        IERC20(str).safeTransfer(caller, callFee);

        uint256 recipientFee = strBal * RECIPIENT_FEE / FEE_DIVISOR;
        IERC20(str).safeTransfer(grimFeeRecipient, recipientFee);

        uint256 treasuryFee = strBal * TREASURY_FEE / FEE_DIVISOR;
        IERC20(str).safeTransfer(treasury, treasuryFee);

        uint256 stratFee = strBal * STRAT_FEE / FEE_DIVISOR;
    ...
    function addLiquidity() internal {
        ...
        ISterlingRouter(router).swapExactTokensForTokens(strHalf, 0, strToWethPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(str).balanceOf(address(this));
        uint256 t2Bal = IERC20(weth).balanceOf(address(this));

        ISterlingRouter(router).addLiquidity(str, weth, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
