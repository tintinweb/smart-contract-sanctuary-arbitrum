...
contract Gauge is IGauge, Initializable {
    ...
    event RewardInitialized(address reward);
        ...
        returns (uint256 claimed0, uint256 claimed1)
    {
        ...
        uint256 balanceAfter0 = IERC20(_token0).balanceOf(address(this));
        uint256 balanceAfter1 = IERC20(_token1).balanceOf(address(this));

        claimed0 = balanceAfter0 - balanceBefore0;
        claimed1 = balanceAfter1 - balanceBefore1;

        if (claimed0 > 0 || claimed1 > 0) {
            // check if the gauge is alive, then send fees
            if (IVoter(voter).isAlive(address(this))) {
    ...
    function notifyRewardAmount(address token, uint256 amount) external lock {
        ...
        uint256 _rewardRate = amount / DURATION;
    ...
    function removeExtraRewardToken(
        ...
        uint256 len = rewards.length;
        for (uint256 i = duplicateIndex; i < len - 1; ++i) {
            rewards[i] = rewards[i + 1];
        }
        rewards.pop();
    }
