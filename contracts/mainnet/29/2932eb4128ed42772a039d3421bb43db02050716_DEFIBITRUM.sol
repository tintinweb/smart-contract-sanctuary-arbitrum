...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, Ownable {
    ...
    function _withdrawDividendOfUser(
        ...
        if (_withdrawableDividend > 0) {
            ...
            if (rewardToken != router.WETH()) {
                bool success = swapARBForCustomToken(
                    user,
                    _withdrawableDividend
                );
                if (!success) {
    ...
    function swapARBForCustomToken(
        address user,
        uint256 amt
...
contract DEFIBITRUM is ERC20, Ownable {
    ...
    DFM_DividendTracker public dividendTracker;
    ...
    Taxes public buyTaxes = Taxes(400, 300, 200, 100);
    Taxes public sellTaxes = Taxes(400, 300, 200, 100);
    Taxes public transferTaxes = Taxes(400, 300, 200, 100);
    ...
    constructor() ERC20("Defibitrum", "DFM") {
        dividendTracker = new DFM_DividendTracker();
        ...
        _tokengeneration(owner(), 1e9 * (10 ** 18));
    }

    receive() external payable {}

    function updateDividendTracker(address newAddress) public onlyOwner {
        DFM_DividendTracker newDividendTracker = DFM_DividendTracker(
            payable(newAddress)
        );
    ...
    function rescueARB() external onlyOwner {
        uint256 ARBbalance = address(this).balance;
        payable(owner()).sendValue(ARBbalance);
    }
    ...
    function setBuyTaxes(
        ...
        buyTaxes = Taxes(_rewards, _marketing, _liquidity, _burn);
        require(
            (_rewards + _marketing + _liquidity + _burn) <= 1200,
    ...
    function setSellTaxes(
        ...
        sellTaxes = Taxes(_rewards, _marketing, _liquidity, _burn);
        require(
            (_rewards + _marketing + _liquidity + _burn) <= 1200,
            "Must keep fees at 12% or less"
        );
    ...
    function setTransferTaxes(
        ...
        transferTaxes = Taxes(_rewards, _marketing, _liquidity, _burn);
        require(
            (_rewards + _marketing + _liquidity + _burn) <= 1200,
            "Must keep fees at 12% or less"
        );
    ...
    function _transfer(
        ...
        if (takeFee) {
            bool useLaunchFee = block.number <=
                startTradingBlock + antiBotBlocks;
            uint256 swapAmt;
            uint burnTax;
            uint totalTax;
            swapTax += sellTaxes.burn;
            if (automatedMarketMakerPairs[to] && !useLaunchFee) {
                swapAmt = (amount * swapTax) / 10000;
                burnTax = sellTaxes.burn;
                totalTax = swapTax;
                ...
                totalTax =
                    ...
                    buyTaxes.marketing +
                    buyTaxes.rewards;
                swapAmt = (amount * (totalTax)) / 10000;
                burnTax = buyTaxes.burn;
            } else if (useLaunchFee) {
                swapAmt = (amount * launchtax) / 10000;
            } else {
                totalTax =
                ...
                swapAmt = (amount * (totalTax)) / 10000;
                burnTax = transferTaxes.burn;
            }
            if (swapAmt > 0) {
                amount = amount - (swapAmt);
                super._transfer(from, address(this), swapAmt);
                if (burnTax > 0) {
                    _burn(address(this), (swapAmt * burnTax) / totalTax);
                }
            }
        }
    ...
    function swapAndLiquify(uint256 tokens, uint256 swapTax) private {
        ...
        swapTokensForARB(toSwap);

        uint256 deltaBalance = address(this).balance - initialBalance;
        uint256 unitBalance = deltaBalance /
            (denominator - sellTaxes.liquidity);
        uint256 ARBToAddLiquidityWith = unitBalance * sellTaxes.liquidity;

        if (ARBToAddLiquidityWith > 0) {
            // Add liquidity to pancake
            addLiquidity(tokensToAddLiquidityWith, ARBToAddLiquidityWith);
        }
    ...
    function swapTokensForARB(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
...
contract DFM_DividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("DFM_Dividen_Tracker", "DFM_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "DFM_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "DFM_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "DFM_Dividend_Tracker: Cannot update claimWait to same value"
        );
