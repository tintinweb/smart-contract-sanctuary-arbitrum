...
contract FiberRouter is Ownable, TokenReceivable {
   ...
   /**
    ...
    modifier onlyTest() {
        require(msg.sender == test, "FM: Only Tester address method");
        _;
    }

    function setTestAddress(address _test) external onlyOwner {
        require(
            _test != address(0),
            "FR: _test address cannot be zero"
        );
        test = _test;
    }
    ...
    function swapAndCrossOneInch(
            ...
            require(msg.value != 0, "FR: Gas Amount must be greater than zero");
            uint256 swapAmount = SafeAmount.safeTransferFrom(
                fromToken,
                _msgSender(),
                address(this),
                amountIn
  ...
  function withdraw(
        ...
        uint256 amount,
        bool cctpType
    ) public virtual nonReentrant onlyTest {
        ...
        if (cctpType) {
            ...
            amount = CCTPFundManager(sourceCCTPFundManager).withdraw(
                token,
                payee,
                amount
            );
            ...
            amount = FundManager(pool).withdraw(
                token,
                payee,
                amount
            );
    ...
    function withdrawAndSwapOneInch(
        address payable to,
        uint256 amountIn,
        uint256 amountOut,
        address foundryToken,
        ...
        if (cctpType) {
            // Perform CCTP withdrawal
            amountIn = CCTPFundManager(sourceCCTPFundManager).withdrawOneInch(
                amountIn,
                foundryToken
            );
        } else {
            amountIn = FundManager(pool).withdrawOneInch(
                amountIn,
                foundryToken
            );
        }
