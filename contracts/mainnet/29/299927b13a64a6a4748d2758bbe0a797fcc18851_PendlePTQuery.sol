...
import "../PendlePTAdapter.sol";
import "../../interfaces/IQuoter.sol";

contract PendlePTQuery is IPoolQuery {
    PendlePTAdapter public immutable adapter;
    ...
    constructor(address adapter_, IPendleStaticRouter staticRouter_) {
        adapter = PendlePTAdapter(adapter_);
    ...
    function swapGivenInputAmount(uint256 inputToken, uint256 inputAmount) public view returns (uint256 outputAmount) {
        ...
        uint256 rawAmount;
        if (isX) {
            (rawAmount,,,,) = staticRouter.swapExactTokenForPtStatic(adapter.primitive(), adapter.underlying(xToken), rawInputAmount);
        } else {
            (rawAmount,,,,) = staticRouter.swapExactPtForTokenStatic(adapter.primitive(), rawInputAmount, adapter.underlying(xToken));
        }
