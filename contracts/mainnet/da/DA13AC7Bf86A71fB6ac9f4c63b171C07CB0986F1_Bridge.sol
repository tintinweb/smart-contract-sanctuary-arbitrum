...
contract Bridge is CCIPReceiver, OwnerIsCreator {
    ...
    function quoteAddLiquidity() public view returns (uint256 fees) {
        uint256 balance = IERC20(token).balanceOf(address(this));
        Client.EVM2AnyMessage memory evm2AnyMessage = Client.EVM2AnyMessage({
            receiver: abi.encode(destinationBridge),
            data: abi.encode(TYPE_REQUEST_ADD_LIQUIDITY, balance),
    ...
    function quoteSend() public view returns (uint256 fees) {
        uint256 amount;
        Client.EVM2AnyMessage memory evm2AnyMessage = Client.EVM2AnyMessage({
            receiver: abi.encode(destinationBridge),
    ...
    function addLiquidity(uint256 amount) external payable {
        ...
        // Refund excess Eth
        uint _excessEth = msg.value - fees;
        if (_excessEth > 0) {
