...
interface ID3Vault {
    ...
    function userDeposit(address user, address token) external;
...
contract D3Proxy is IDODOSwapCallback {
    ...
    function d3MMSwapCallBack(address token, uint256 value, bytes calldata _data) external override {
        ...
        // this method used for hardhat but fail in forge test
        (decodeData.data, decodeData.payer) = abi.decode(_data, (bytes, address));
