...
contract Escrow is ReentrancyGuard, IEscrow, EIP712 {
  ...
  function getOP() public view virtual returns (address) {
    return OP;
  }
  ...
  function updateOP(address _op) external onlyBridge nonReentrant {
    OP = _op;
  }
  ...
  function mint(WithdrawData memory Data, bytes memory signature) public nonReentrant {
...
library StructuredLinkedList {
  struct CancelWithdrawCheckPointData {
    uint256 accumulatedCanceledAmount;
  }
  struct Node {
    uint256 id;
  ...
  function insert(
    ...
    StructuredLinkedList.Node memory spot = getSortedSpot(self, _id);
    return insertBefore(self, spot, Node(_id, _value));
  }
  ...
  function get(List storage self, uint256 _id) internal view returns (Node memory node) {
    ...
    if (!found) {
      return Node(0, CancelWithdrawCheckPointData({ accumulatedCanceledAmount: 0 }));
    }
  ...
  function getAdjacent(
    ...
    if (!nodeExists(self, _node)) {
      return (false, Node(0, CancelWithdrawCheckPointData({ accumulatedCanceledAmount: 0 })));
    } else {
  ...
  function _insert(
    ...
    if (!nodeExists(self, _new.id) && nodeExists(self, _node.id)) {
      Node memory c = self.list[_node.id][_direction];
...
library Checkpoints {
  ...
  ) internal returns (ProcessWithdrawCheckPointData memory) {
    return _insert(self, LastID, value);
  }
  ...
  function upperLookup(
    ...
    return
      pos == len
        ? ProcessWithdrawCheckPointData({ Rate: 0, USDCAmount: 0 })
        : self._checkpoints[pos]._value;
  }
  ...
  function _insert(
    ...
    uint256 key,
    ProcessWithdrawCheckPointData memory value
  ) private returns (ProcessWithdrawCheckPointData memory) {
    uint256 pos = self._checkpointsLength;

    if (pos > 0) {
      // Copying to memory is important here.
      ProcessWithdrawCheckpointElement memory last = self._checkpoints[pos - 1];

      // Checkpoint keys must be non-decreasing.
      require(last.LastID <= key, 'Checkpoint: decreasing keys');

      // Update or push new checkpoint
      if (last.LastID == key) {
        self._checkpoints[pos - 1]._value = ProcessWithdrawCheckPointData({
          Rate: value.Rate,
        ...
        push(
          self,
          ProcessWithdrawCheckpointElement({
            LastID: key,
            _value: ProcessWithdrawCheckPointData({
              Rate: value.Rate,
              USDCAmount: value.USDCAmount
            })
          })
      ...
      push(
        ...
        ProcessWithdrawCheckpointElement({
          LastID: key,
          _value: ProcessWithdrawCheckPointData({ Rate: value.Rate, USDCAmount: value.USDCAmount })
        })
      );
      return (value);
    }
  ...
  function push(
    ProcessWithdrawTracer storage self,
    ProcessWithdrawCheckpointElement memory value
  ) private returns (ProcessWithdrawCheckpointElement memory) {
    uint256 pos = self._checkpointsLength;
    self._checkpoints[pos] = value;
   ...
   * @param LastID The last ID of the checkpoint.
   * @param value The value of the checkpoint.
   * @return result if the element was successfully added.
   * @notice The `LastID` of the checkpoint must be greater than the `LastID` of the last checkpoint.
   */
  function push(
    ...
    StructuredLinkedList.Node memory spot = self.getSortedSpot(LastID);
    result = self.insertBefore(spot, StructuredLinkedList.Node(LastID, value));
  }
  ...
  function upperLookup(
    ...
    uint256 len = self.sizeOf();
    uint256 pos = _upperBinaryLookup(self, key, 0, len);
    return pos == 0 ? StructuredLinkedList.CancelWithdrawCheckPointData(0) : self.get(pos).value;
  }

  /**
   * @dev Returns the index of the checkpoint with the `LastID` that is less than or equal to the given `key`.
   * @param self The StructuredLinkedList.
   * @param key The key to search for.
   * @param low The lower bound of the search.
   * @param high The upper bound of the search.
...
library FIFOQueue {
  ...
  function addToQueue(
    ...
    deque._data[backIndex].exchanges.push(Exchange({ LPAmount: LPAmount, processed: false }));
  ...
  function removeFromQueue(
    ...
    require(!emptyQueue(deque), 'Queue is empty');
  ...
  function moveBeginningOfQueue(WithdrawDeque storage deque, uint256 newValue) internal {
    ...
    require(_newValue <= deque.QueueEnd, 'Index out of bounds');
  ...
  ) internal returns (uint256 LeftOverLPAmount) {
    require(!emptyQueue(deque), 'Queue is empty');
    WithdrawItem storage item = deque._data[SafeCast.toInt128(int256(ID))];
    require(!item.exchanges[item.exchanges.length - 1].processed, 'Already processed');
    // If the item covers the amount, mark it as processed and return 0
    if (ItemNonProcessedLP == 0) {
      item.exchanges[item.exchanges.length - 1].processed = true;
      return 0;
    }

    if (item.exchanges[item.exchanges.length - 1].LPAmount > ItemNonProcessedLP) {
      // If the item covers more than the amount, split it into two items
      item.exchanges[item.exchanges.length - 1].LPAmount -= ItemNonProcessedLP;
      item.exchanges[item.exchanges.length - 1].processed = true;
      item.exchanges.push(Exchange({ LPAmount: ItemNonProcessedLP, processed: false }));
    } else if (item.exchanges[item.exchanges.length - 1].LPAmount == ItemNonProcessedLP) {
      // If the item covers exactly the amount, mark it as processed
      item.exchanges[item.exchanges.length - 1].processed = false;
    } else {
      // If the item covers less than the amount, mark it as processed and return the leftover amount
      LeftOverLPAmount = ItemNonProcessedLP - item.exchanges[item.exchanges.length - 1].LPAmount;
      item.exchanges[item.exchanges.length - 1].processed = true;
    }
  }
  ...
  function findLastCoveringItem(
    ...
    require(deque._data[high].accumulatedLPAmount >= accumulatedLPAmount, 'No covering item found');
    return uint128(high);
  }
  ...
  function at(
    ...
    require(index < deque.QueueEnd, 'Index out of bounds');
  ...
  function get(
    ...
    require(idx <= deque.QueueEnd, 'Index out of bounds');
  ...
  UUPSUpgradeable
{
  ...
  mapping(uint256 => bool) public usedNonces;
  modifier onlyAdmin() {
    require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), 'not authorized admin');
    _;
  }
  modifier onlyOperationWallet() {
    require(
  ...
  modifier whenNotPaused() {
    require(activeWithdraw, 'Withdraw not active');
    _;
  }
  ...
  function initialize(
    ...
    require(_usdcTokenAddress != address(0), ' zero address');
    require(_lpTokenAddress != address(0), 'zero address');
    __AccessControlEnumerable_init();
    __ReentrancyGuard_init();
    __UUPSUpgradeable_init();
    __EIP712_init_unchained('OPULOUS', '1');
  ...
  function enterQueue(
    ...
    require(LPAmount >= MinWithdrawAmount, 'LPAmount < MinWithdrawAmount');
    verifySignature(signature, Data);
  ...
  function exitQueue(
    ...
    require(Data.value >= LastProcessedID, 'Already processed');
    verifySignature(signature, Data);

    uint256 index = userRequestsIDs[msg.sender].length;
    for (uint256 i = 0; i < userRequestsIDs[msg.sender].length; i++) {
      if (userRequestsIDs[msg.sender][i] == Data.value) {
        index = i;
        break;
      }
    }
    require(index < userRequestsIDs[msg.sender].length, 'OOB');
    FIFOQueue.WithdrawItem storage item = _queue.get(Data.value);
    if (item.ID == LastProcessedID) {
      require(!item.exchanges[item.exchanges.length - 1].processed, 'Already processed');
    }
    require(item.userWallet == msg.sender, 'User!=msg.sender');
    ...
    uint256 previousCanceledAmount = cancelHistory
      .upperLookup(Data.value)
      .accumulatedCanceledAmount;
    cancelHistory.push(
      Data.value,
      StructuredLinkedList.CancelWithdrawCheckPointData({
        accumulatedCanceledAmount: previousCanceledAmount + LPAmount
      })
    );
    QueuedTokens -= LPAmount;
  ...
  function claimAll(
    ...
    for (
      ...
      if (item.ID < LastProcessedID && item.exchanges.length > 0) {
        for (uint256 j = 0; item.exchanges.length > 0; j++) {
          TotalUSDC +=
            (item.exchanges[0].LPAmount * rateHistory.upperLookup(item.ID).Rate) /
            10 ** 18;
          totalLPAmount += item.exchanges[0].LPAmount;
          item.exchanges[0] = item.exchanges[item.exchanges.length - 1];
          item.exchanges.pop();
        }
        ...
        uint256 j;
        uint256 length = item.exchanges.length;
        uint256 index;
        while (j < length) {
          if (item.exchanges[index].processed) {
            TotalUSDC +=
              (item.exchanges[index].LPAmount * rateHistory.upperLookup(item.ID).Rate) /
              10 ** 18;
            totalLPAmount += item.exchanges[index].LPAmount;
            item.exchanges[index] = item.exchanges[item.exchanges.length - 1];
            item.exchanges.pop();
          } else {
            index++;
          }
    ...
    require(TotalUSDC > 0, 'Nothing to claim');
  ...
  function SetOperationWallet(
    ...
    require(newOperationWallet.length == authorized.length, 'Arrays length mismatch');
    for (uint256 i = 0; i < newOperationWallet.length; i++) {
      if (authorized[i]) {
  ...
  function SetStakingContract(address newStakingContract) public onlyAdmin {
    require(newStakingContract != address(0), 'zero address');
  ...
  function ProcessQueue(uint256 LPAmount, uint256 maxIterations) public onlyOperationWallet {
    if (!processing) {
      require(LPAmount > 0, 'LPAmount == 0');
      require(QueuedTokens >= LPAmount, 'LPAmount>QueuedTokens');
      (lastUSDCReceived, lastRate) = stakingContract.withdrawFunds(LPAmount);
      processing = true;
      _processQueue(LPAmount, 0, maxIterations);
    } else {
      // continue previous processing if canceledAmount > 0
      if (canceledAmount > 0) _processQueue(canceledAmount, leftOverLPAmount, maxIterations);
      else {
        processing = false;
      }
    }
    ...
    if (!processing) {
      rateHistory.push(
        LastProcessedID,
        Checkpoints.ProcessWithdrawCheckPointData({ Rate: lastRate, USDCAmount: lastUSDCReceived })
      );
      QueuedTokens -= LPAmount;
    }
  }
  ...
  function getAllUserWithdrawRequests(
    ...
    for (uint256 i = 0; i < userRequests.length; i++) {
      ...
      if (item.ID > LastProcessedID && item.exchanges.length > 0) {
        ...
        bool Completed = true;
        if (!item.exchanges[item.exchanges.length - 1].processed) {
          Completed = false;
        }
        if (!Completed) requestsLength++;
      }
    }
  ...
  function totalClaimAmount(
    ...
    for (uint256 i = 0; i < iterations; i++) {
      FIFOQueue.WithdrawItem storage item = _queue.get(userClaimRequests[i]);
      if (item.ID > LastProcessedID) continue;
      uint256 rate = rateHistory.upperLookup(item.ID).Rate;
      if (item.ID < LastProcessedID && item.exchanges.length > 0) {
        for (uint256 j = 0; j < item.exchanges.length; j++) {
          TotalUSDC += (item.exchanges[j].LPAmount * rate) / 10 ** 18;
          totalLPAmount += item.exchanges[j].LPAmount;
        }
      } else if (item.ID == LastProcessedID && item.exchanges.length > 0) {
        for (uint256 index = 0; index < item.exchanges.length; index++) {
          if (item.exchanges[index].processed) {
            TotalUSDC += (item.exchanges[index].LPAmount * rate) / 10 ** 18;
            totalLPAmount += item.exchanges[index].LPAmount;
          }
        }
      }
   ...
   * @dev Processes the queue.
   * @param LPAmount The amount of LP tokens to be processed.
   * @param leftOver The amount of LP tokens left from the previous iteration.
   * @param maxIterations The maximum number of recursive iterations.
   */
  function _processQueue(uint256 LPAmount, uint256 leftOver, uint256 maxIterations) internal {
    FIFOQueue.WithdrawItem storage firstItem = _queue.at(_queue.QueueBegin);
    uint256 startCanceled = cancelHistory.upperLookup(firstItem.ID).accumulatedCanceledAmount;
    ...
    uint256 target = LPAmount + alreadyProcessed + leftOver;
    uint256 LastID = _queue.findLastCoveringItem(target);
    canceledAmount = cancelHistory.upperLookup(LastID).accumulatedCanceledAmount - startCanceled;
    FIFOQueue.WithdrawItem storage lastItem = _queue.get(LastID);

    uint256 nonProcessedLPAmount = lastItem.accumulatedLPAmount - target;
    leftOverLPAmount = _queue.updateItem(LastID, nonProcessedLPAmount);
    _queue.moveBeginningOfQueue(LastID);
    LastProcessedID = LastID;
    if (maxIterations == 0) return;
    if (canceledAmount + leftOverLPAmount > 0) {
      maxIterations--;
      _processQueue(canceledAmount, leftOverLPAmount, maxIterations);
    } else {
      processing = false;
    }
  }
   ...
   */
  function verifySignature(bytes memory signature, WithdrawData memory Data) internal {
    require(!usedNonces[Data.nonce], 'signature executed');
    require(block.timestamp <= Data.deadLine, 'signature expired');
    require(Data.receiver == msg.sender, 'User!=msg.sender');
    _checkRole(OPERATION_WALLET, Data.signer);
