...
library GMXV2Adapter {
    ...
    event CreateIncreasePosition(address sender, bytes32 requestKey);
    event CancelPosition(address sender, bytes32 orderKey);
    event CreateDecreasePosition(address sender, bytes32 requestKey);
    ...
    error CreateSwapOrderFail();
    error CreateIncreasePositionFail(string);
    error CancelPositionFail(string);
    error CreateDecreasePositionFail(string);
    error CreateIncreasePositionOrderFail(string);
    error CreateDecreasePositionOrderFail(string);
    error NotSupportedTokens(address, address);
    ...
    function executeOperation(
        ...
        } else if (uint256(traderOperation.operationId) == 20) {
            /** Decrease position by stoploss */
            (bool success) =  _cancelOrder(
                isTraderWallet,
                traderWallet,
                usersVault,
                ratio,
    ...
    function _cancelOrder(
        ...
        (bytes32 orderKeyTW, bytes32 orderKeyUV) = _decodeCancelData(tradeData);
        bytes32 orderKey = orderKeyTW;
        if (!isTraderWallet) {
            orderKey = orderKeyUV;
        }
        ...
        multicallData[0] = abi.encodeWithSelector(IGmxV2ExchangeRouter.cancelOrder.selector, orderKey);
        ...
        if (!success) {
            revert CancelPositionFail(_getRevertMsg(data));
        }

        emit CancelPosition(address(this), orderKey);
    ...
    function _decodeCancelData(bytes memory tradeData) private pure returns (bytes32 keyTW, bytes32 keyUV) {
        (
            keyTW,
            keyUV
        ) = abi.decode(
