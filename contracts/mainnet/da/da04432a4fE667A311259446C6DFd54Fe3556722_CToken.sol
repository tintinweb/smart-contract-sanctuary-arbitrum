...
// File interfaces/cTokens/ICToken.sol
...
/// @title Interface of CadmosPay CToken
interface ICToken {
    ...
    /// @dev Used by the Operator to transfer assets to the treasury
    function transferToTreasury(uint256 amount) external;

    function transferWithDescription(address to, uint256 value, string memory description) external returns (bool);

    function transferFromWithDescription(address from, address to, uint256 value, string memory description) external returns (bool);

    /// @dev Allows Operator to return wrongly sent ERC20 to its holder
    ...
    event TransferToTreasury(address indexed treasury, uint256 amount);
    event TransferDescription(string description);
...
contract CToken is ERC4626, Ownable2Step, Pausable, ICToken   {
    using SafeERC20 for IERC20;

    /* ========== CONSTANTS ========== */

    address public override(ICToken) treasury; //Safeguarding address to which the CToken underlying Assets can be transferred

    /* ========== CONSTRUCTOR ========== */


    constructor(IERC20 asset_, address treasury_, string memory name_, string memory symbol_) ERC20(name_, symbol_) ERC4626(asset_) {
        treasury = treasury_;
    }
    ...
    function deposit(
        uint256 assets,
        address receiver
    ) public override(ERC4626) whenNotPaused returns (uint256) {
    ...
    /// @dev Used by the Operator to transfer assets to the treasury
    function transferToTreasury(uint256 amount) external override(ICToken) onlyOwner{
        require(treasury != address(0), "Transfer to the nul address");
        IERC20(asset()).safeTransfer(treasury, amount);
        emit TransferToTreasury(treasury, amount);
    }

    function transferWithDescription(address to, uint256 value, string memory description) public override(ICToken) returns (bool) {
        emit TransferDescription(description);
        return transfer(to,value);
    }

    function transferFromWithDescription(address from, address to, uint256 value, string memory description) public override(ICToken) returns (bool) {
        emit TransferDescription(description);
        return transferFrom(from,to,value);
    }

    /// @dev Allows Operator to return wrongly sent ERC20 to its holder
    function recoverERC20(address token, address recipient, uint256 amount) external override(ICToken) onlyOwner{
        require(recipient != address(0), "Transfer to the nul address");
        uint256 balanceBefore = IERC20(asset()).balanceOf(address(this));
        IERC20(token).safeTransfer(recipient, amount);
        require(balanceBefore == IERC20(asset()).balanceOf(address(this)), "Transferred Reserve Asset");
        emit RecoveredERC20(token, recipient, amount);
    }


    function pause() external override(ICToken) onlyOwner{
        _pause();
    }

    function unpause() external override(ICToken) onlyOwner{
        _unpause();
    }
