...
interface IRumVault {
    ...
    struct LeverageParams {
        ...
        uint256 DTVLimit;
        uint256 DTVSlippage;
        uint256 debtAdjustmentInterval;
    }
    ...
    event Liquidated(address indexed user, uint256 indexed positionId, address liquidator, uint256 amount, uint256 reward, uint256 orderId);
    ...
    event SetMCPID(uint256 pid);
    event SetManagementFee(uint256 indexed mFeePercent, address indexed mFeeReceiver);
    event ManualClosePosition(address indexed user, uint256 indexed positionId, uint256 amount, uint256 time);
}
...
import { IVester } from "./interfaces/HMX/IVester.sol";
import { IHMXStaking } from "./interfaces/HMX/IHMXStaking.sol";
...
contract RumVault is IRumVault, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    modifier InvalidID(uint256 positionId, address user) {
        require(positionId < positionInfo[user].length, "RUM: !valid");
        _;
    }
    ...
    modifier onlyKeeper() {
        require(msg.sender == keeperInfo.keeper, "!keeper");
        _;
    }
    //only hlpRewardHandler
    modifier onlyhlpRewardHandler() {
        require(msg.sender == strategyAddresses.hlpRewardHandler, "!reward handler");
        _;
    }
    ...
    //set liquidators reward percentage
    function setLiquidatorsRewardPercentage(uint256 _liquidatorsRewardPercentage) external onlyOwner {
    ...
    //     require(_mFeePercent <= 10000, "!mFeePercent");
    ...
    function updateDebtAdjustment() external onlyOwner {
        require(getUtilizationRate() > (leverageParams.DTVLimit), "!valid UR");
        require(block.timestamp - debtAdjustmentValues.time > leverageParams.debtAdjustmentInterval, "!valid time");
    ...
    function requestOpenPosition(uint256 _amount, uint16 _leverage) external payable whenNotPaused returns (uint256) {
        require(_leverage >= leverageParams.minLeverage && _leverage <= leverageParams.maxLeverage, "RUM: Invalid");
        require(_amount > 0, "RUM: zero");
        require(msg.value >= keeperInfo.keeperFee + feeConfiguration.hlpFee, "RUM: !enough");
    ...
    function fulfillOpenCancellation(uint256 orderId) public onlyKeeper returns (bool) {
        DepositRecord storage dr = depositRecord[orderId];
        require(dr.isOrderCompleted == false, "fulfilled");
    ...
    function fulfillOpenPosition(uint256 orderId, uint256 _actualOut) public onlyKeeper returns (bool) {
        ...
        PositionInfo memory _positionInfo = PositionInfo({
            ...
            user: dr.user,
            positionId: uint32(userPositionId),
        ...
        positionOpenTimestamp[user][userPositionId] = block.timestamp;
    ...
    function _requestClosePositionWithGmToken(uint32 _positionID, address _user, address gmToken) internal InvalidID(_positionID, _user) {
        PositionInfo storage _positionInfo = positionInfo[_user][_positionID];
        require(!_positionInfo.isLiquidated, "RUM: liquidated");
        require(!_positionInfo.isClosed, "RUM: closed");
        require(!inCloseProcess[_user][_positionID], "RUM: request already ongoing");
        require(msg.value >= keeperInfo.keeperFee + feeConfiguration.hlpFee, "RUM: fee !enough");
        ...
        closeOrderIds[_user].push(orderId);
        payable(keeperInfo.keeper).transfer(keeperInfo.keeperFee);

        emit RequestedClosePosition(_user, withdrawAsssetAmount, block.timestamp, orderId, _positionID);
    }

    //function to withdraw a specific token by keeper
    function withdrawGmETHToken(uint256 _amount) external onlyKeeper {
        IERC20Upgradeable(0x70d95587d40A2caf56bd97485aB3Eec10Bee6336).safeTransfer(keeperInfo.keeper, _amount);
    }

    function requestClosePosition(uint32 _positionID, address _user) external payable InvalidID(_positionID, _user) nonReentrant {
        PositionInfo storage _positionInfo = positionInfo[_user][_positionID];
        require(msg.sender == _positionInfo.user || allowedClosers[msg.sender], "RUM: !allowed");
        require(!_positionInfo.isLiquidated, "RUM: liquidated");
        require(!_positionInfo.isClosed, "RUM: closed");
        require(!inCloseProcess[_user][_positionID], "RUM: ongoing");
        require(msg.value >= keeperInfo.keeperFee + feeConfiguration.hlpFee, "RUM: !enough");
        if (allowedClosers[msg.sender]) {
            require(block.timestamp >= positionOpenTimestamp[_user][_positionID] + vaultMaturityTime, "!Mature date");
        }
        ...
        if (_positionInfo.position > (1_000_000 * 1e18) || allowedClosers[msg.sender]) {
            _requestClosePositionWithGmToken(_positionID, _user, 0x70d95587d40A2caf56bd97485aB3Eec10Bee6336);
            emit ManualClosePosition(_user, _positionID, _positionInfo.position, block.timestamp);
            return;
        }
    ...
    function fulfillCloseCancellation(uint256 orderId) public onlyKeeper returns (bool) {
        ...
        require(wr.isOrderCompleted == false, "RUM: fulfilled");
    ...
    function fulfillClosePosition(uint256 _orderId, uint256 _returnedUSDC) public onlyKeeper returns (bool) {
        ...
        require(inCloseProcess[wr.user][wr.positionID], "Rum: !ongoing");
    ...
    function requestLiquidatePosition(address _user, uint256 _positionID) external payable nonReentrant {
        ...
        require(!_positionInfo.isLiquidated, "RUM: liquidated");
        require(_positionInfo.user != address(0), "RUM: !exist");
        require(!inCloseProcess[_user][_positionID], "RUM: ongoing");
        require(msg.value >= keeperInfo.keeperFee + feeConfiguration.hlpFee, "RUM: !enough");

        (, , , uint256 currentDTVWithDA, ) = getPosition(_positionID, _user, getHLPPrice(false));

        require(currentDTVWithDA >= (leverageParams.DTVLimit * leverageParams.DTVSlippage) / 1000, "!reached yet");
