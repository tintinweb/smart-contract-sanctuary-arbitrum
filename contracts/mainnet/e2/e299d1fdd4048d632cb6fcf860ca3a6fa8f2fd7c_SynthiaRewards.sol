...
contract SynthiaRewards is ERC721Enumerable, Ownable2Step {
    ...
    constructor() ERC721("Synthia Rewards", "SYNR") {}
    ...
    function _afterTokenTransfer(
        ...
        for (uint i = 0; i <= batchSize; i++) {
            uint tokenId = firstTokenId + i;
            // If not mint then check if soulbound
            if (
                from != address(0) && items[tokenIdToItemId[tokenId]].soulbound
            ) {
                revert ErrorMessage("NFT is soulbound");
            }
        }
