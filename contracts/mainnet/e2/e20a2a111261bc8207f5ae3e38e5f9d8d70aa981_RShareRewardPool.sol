...
contract RShareRewardPool {
    ...
    struct UserInfo {
        ...
        uint256 endLockTime; // End lock time
    }

    // Info of each pool.
    struct PoolInfo {
        IERC20 token; // Address of LP token contract.
        uint256 allocPoint; // How many allocation points assigned to this pool. RSHARE to distribute per block.
        uint256 lastRewardTime; // Last time that RSHARE distribution occurs.
        uint256 accRSharePerShare; // Accumulated RSHARE per share, times 1e18. See below.
    ...
    uint256 public rSharePerSecond = 0.00321502057 ether; // 50000 rShare / (180 days * 24h * 60min * 60s)
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint256 _lockDuration) public onlyOperator {
        massUpdatePools();
    ...
    // View function to see pending RSHARE on frontend.
    function pendingrShare(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accRSharePerShare = pool.accRSharePerShare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (address(pool.token) == address(rose)) {
            tokenSupply = rose.balanceOfUnderlying(address(this));
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.token) == address(rose)) {
                _amount = rose.fragmentToRose(_amount);
            }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {        
        ...
        UserInfo storage user = userInfo[_pid][_sender];
        if (address(pool.token) == address(rose)) {
            require(user.amount >= rose.fragmentToRose(_amount), "withdraw: not good");
        }
        else {
    ...
    function isWithdrawable(uint256 _pid) public view returns (bool){
        address _sender = msg.sender;
        UserInfo storage user = userInfo[_pid][_sender];
    ...
    // Safe rShare transfer function, just in case if rounding error causes pool to not have enough RSHARE.
