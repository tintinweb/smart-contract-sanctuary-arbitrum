...
*
*/
contract LiquidateLoan is Ownable {
        ...
        returns (bool)
    {
        ...
        
        {
            ...
            (collateral, userToLiquidate, data) = abi.decode(params, (address, address, bytes));
            //liquidate unhealthy loan
            liquidateLoan(ILendingPool(msg.sender), collateral, assets[0], userToLiquidate, amounts[0], false);
        }
        
        {
            (address swapper, address inputToken, bytes memory swapData, bytes memory other) = abi.decode(data, (address, address, bytes, bytes));
            
            
            IERC20(inputToken).safeTransfer(swapper, IERC20(inputToken).balanceOf(address(this)));
            ISwapper(swapper).swap(inputToken, swapData);

            (swapper, inputToken, swapData, other) = abi.decode(other, (address, address, bytes, bytes));
        ...
        IERC20(assets[0]).approve(address(msg.sender), amountOwing);
    ...
    function liquidateLoan(ILendingPool _lendingPool, address _collateral, address _liquidate_asset, address _userToLiquidate, uint256 _amount, bool _receiveaToken) public {
    ...
    */
    function executeFlashLoans(ILendingPool _lendingPool, address _assetToLiquidate, uint256 _flashAmt, address _collateral, address _userToLiquidate, bytes calldata _swapData) public {
