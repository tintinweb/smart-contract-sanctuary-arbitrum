...
Telegram: \ https://t.me/SuffraV2E
Website: \ https://suffra.xyz
...
contract Suffra is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Suffra Token", "SFRA") {
        ...
        DevWallet = 0xEeF3b253C5D8F77194E08822be84b63BaC38302B;
    ...
    function updateTFactory(uint256 newNum) external onlyOwner {
        require(newNum >= 1);
        transDigit = newNum;
        updateLimits();
    }

    function updateWFactory(uint256 newNum) external onlyOwner {
        require(newNum >= 1);
        walletDigit = newNum;
        updateLimits();
    }

    function updateDFactory(uint256 newNum) external onlyOwner{
        delayDigit = newNum;
    }
    ...
    function updateBuyFees(uint256 _burnFee, uint256 _DevFee) external onlyOwner {
        ...
        buyDevFee = _DevFee;
        buyTotalFees = buyBurnFee + buyDevFee;
        require(buyTotalFees <=  /*, /*"Must keep fees at 25% or less");
    ...
    function updateSellFees(uint256 _burnFee, uint256 _DevFee) external onlyOwner {
        ...
        sellDevFee = _DevFee;
        sellTotalFees = sellBurnFee + sellDevFee;
        require(sellTotalFees <= /*, /*"Must keep fees at 25% or less");
        sellBurnFee = _burnFee;
        sellDevFee = _DevFee;
        * Counterpart to Solidity's `%` operator. This function uses a `revert`
        * opcode (which leaves remaining gas untouched) while Solidity uses an
