...
contract PoolV2 {
	...
	function getPoolOI() external view returns (uint256,uint256) {
		return (openInterest,utilizationMultiplier);
	}
	...
	function deposit(uint256 amount) external payable { 
		...
		require(currency != address(0),"token must not equal dead wallet");
		require(currency != address(this),"token must not equal contract address");
        ...
        uint256 clpAmountToMint;
		// = currentBalance == 0 || totalSupply == 0 ? amount : amount * totalSupply / lastBalance;

		if (currentBalance <= 0 && totalSupply <= 0 ){
			clpAmountToMint = amount;
		}else{
			clpAmountToMint =  amount * totalSupply / currentBalance;
		}
	...
	function creditUserProfit(address destination, uint256 amount) external onlyTrading {
		...
		_transferOutForTrade(destination, amount);
	}
	...
	function _transferOutForTrade(address to, uint256 amount) internal {
		if (amount == 0 || to == address(0)) return;
		// amount = amount * (10**10) ;
		IERC20(currency).transfer(to, amount);
	}
