...
contract Amm is IAmm, OwnableUpgradeable, BlockContext {
    ...
    event BotStatusSet(address indexed bot, bool indexed status);
    ...
    modifier onlyRepegBot() {
        require(botsMap[_msgSender()], "caller is not repegBot");
        _;
    }
    ...
    mapping(address => bool) public botsMap; // all bots go here
     ...
     * only owner
     */
    function setRepegBot(address _bot, bool _status) public onlyOwner {
        botsMap[_bot] = _status;
        emit BotStatusSet(_bot, _status);
    }
        ...
        returns (Decimal.decimal memory, Decimal.decimal memory)
    {
        ...
        if (level2Amount.toUint() != 0 && divergence.toUint() > level2Amount.toUint()) {
            ...
            Decimal.decimal memory fee = level2DynamicFeePercent.divScalar(2);
            spread = fee;
            toll = fee;
            ...
            Decimal.decimal memory fee = level1DynamicFeePercent.divScalar(2);
            spread = fee;
            toll = fee;
        }
