...
contract AirdropContract {
    ...
    // Function to set the token holder address, only callable by the admin
    // the token holder is the address that holds the tokens to be given out as reward
    // the token holder needs to call the approve() method on the ERC20 token setting the spender 
    // as this contract's address
    function setTokenHolder(address _tokenHolder) external onlyAdmin {
    ...
    function claimReward(address recipient, uint256 tokenId, uint256 rewardId) external onlyAdmin {
        ...
        IERC20(tokenAddress).transferFrom(tokenHolder, recipient, amount);
    }
}