...
contract ZeroWayNFT is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable, IZeroWayNFT {
    ...
    function bridge(uint256 tokenId_, uint32 destinationDomain_) external payable override returns (uint256, bytes32) {
        _checkBridgeParams(tokenId_, destinationDomain_);
        require(ownerOf(tokenId_) == msg.sender, "ZeroWayNFT: user do not have NFT");

        _burn(tokenId_);

        uint256 fee = _calculateBridgeFee(tokenId_, destinationDomain_);
        require(msg.value >= fee, "ZeroWayNFT: not enough msg.value");
    ...
    function calculateBridgeFee(uint256 tokenId_, uint32 destinationDomain_) external view override returns (uint256) {
        ...
        return (_calculateBridgeFee(tokenId_, destinationDomain_));
    }
    ...
    function _calculateBridgeFee(uint256 tokenId_, uint32 destinationDomain_) internal view returns (uint256) {
        IMailbox mailbox = IMailbox(mailboxAddr);
