...
contract BaseFacet is StorageV2 {
 ...
 */
abstract contract VRFV2PlusWrapperConsumerBase is StorageV2 {
...
contract OpenFacet is BaseFacet {
    ...
    function _checkIfCallInOneDay(address user) internal {
        uint256 lastTime = userLastOpenTime[user];
        uint256 currentTime = block.timestamp;
        ...
        require(lastTime == 0 || lastTime + remainOneDayInSecond <= currentTime, 'Only Once a Day');
        userLastOpenTime[user] = currentTime;
    }
...
contract StorageV2 is Storage {
    mapping(address => uint256) public userLastOpenTime; // user => timestamp
}