...
contract Pool is Ownable, ReentrancyGuard, IPool {
    ...
    function mint(
        ...
        uint wethToCollect = _collateral_amount - unwrappedETHGiven;
        if (wethToCollect > 0) {
            ERC20(collateral).transferFrom(msg.sender, address(this), wethToCollect);
        }
    ...
    function redeem(
        ...
        if (_effective_collateral_ratio < COLLATERAL_RATIO_MAX) {
            uint _share_output_value = ArbiTen_amount_post_fee.sub(ArbiTen_amount_post_fee.mul(_effective_collateral_ratio).div(PRICE_PRECISION));
        ...
        if (_effective_collateral_ratio > 0) {
            uint _collateral_output_value = ArbiTen_amount_post_fee.mul(_effective_collateral_ratio).div(10**missing_decimals).div(PRICE_PRECISION);
