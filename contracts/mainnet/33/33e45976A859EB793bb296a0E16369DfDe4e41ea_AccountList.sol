...
contract AccountList is IAccountList, SafeOwnable {
  ...
  function set(address[] calldata _accounts, bool[] calldata _included) external override onlyOwner {
    ...
    for (uint256 i; i < _accounts.length; ++i) { _resetIndexToAccountToIncluded[_resetIndex][_accounts[i]] = _included[i]; }
  }

  function reset(address[] calldata _newIncludedAccounts) external override onlyOwner {
    _resetIndex++;
    for (uint256 i; i < _newIncludedAccounts.length; ++i) { _resetIndexToAccountToIncluded[_resetIndex][_newIncludedAccounts[i]] = true; }
  }

  function isIncluded(address _account) external view override returns (bool) { return _resetIndexToAccountToIncluded[_resetIndex][_account]; }
}
...
contract SafeOwnable is ISafeOwnable, Ownable {
  ...
  modifier onlyNominee() { require(_msgSender() == _nominee, "msg.sender != nominee"); _; }

  function transferOwnership(address _newNominee) public virtual override(ISafeOwnable, Ownable) onlyOwner { _setNominee(_newNominee); }
  ...
  function getNominee() public view virtual override returns (address) { return _nominee; }
