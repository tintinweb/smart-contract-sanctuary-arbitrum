...
contract Contango is IContango, IContangoAdmin, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
        ...
        returns (PositionId positionId_, InstrumentStorage memory instrument_, IMoneyMarket moneyMarket)
    {
        ...
        if (id == 0) {
            ...
            instrument_.base.forceApprove(address(moneyMarket), type(uint256).max);
            instrument_.quote.forceApprove(address(moneyMarket), type(uint256).max);
        ...
        returns (Trade memory trade_)
    {
        ...
        uint256 quantity = _myBalance(cb.instrument.base);
        ...
        uint256 repaid = _myBalance(cb.instrument.quote);
        ...
        returns (Trade memory trade_, address owner)
    {
        ...
        // TODO validate when fully closing a cashflow ccy must be specified
        ...
        returns (Trade memory trade_)
    {
        ...
        uint256 quoteBalance = _myBalance(cb.instrument.quote);
        uint256 repaid;
        if (!_bypassFlashLoanOnClose(cb)) {
        ...
        if (repayTo != address(0)) _transferOut(asset, address(this), repayTo, amountOwed);
        ...
        returns (Trade memory trade_, address owner)
    {
        ...
        if (tradeParams.cashflow > 0) {
            ...
            _repayFromMarket(moneyMarket, tradeParams.positionId, _instrument.quote, _myBalance(_instrument.quote));
    ...
    function _executeSwap(
        ...
        if (shouldRun && execParams.swapAmount != 0) {
            ...
            swap.input = _myBalanceI(tokenToSell);
            swap.output = _myBalanceI(tokenToBuy);

            // TODO would it make sense to approve only what swapAmount?
            tokenToSell.approveIfNecessary(dexes[execParams.dex].spender);
            Address.functionCall(dexes[execParams.dex].router, execParams.swapBytes);

            swap.input = _myBalanceI(tokenToSell) - swap.input;
            swap.output = _myBalanceI(tokenToBuy) - swap.output;

            // TODO check if swap.input == swapAmount
    ...
    function _applyFee(FlashLoanCallback memory cb, uint256 quantity) private returns (uint256 fee, Currency feeCcy) {
        cb.instrument.base.approveIfNecessary(address(feeManager));
        cb.instrument.quote.approveIfNecessary(address(feeManager));
        (fee, feeCcy) = feeManager.applyFee(cb.owner, cb.positionId, quantity);
    }

    // TODO revisit all these function wrappings if we have more free space on the contract
    ...
    function _myBalance(IERC20 token) private view returns (uint256) {
        return token.myBalance();
    }

    function _myBalanceI(IERC20 token) private view returns (int256) {
        return token.myBalanceI();
    }

    function _depositBalance(IERC20 token, address owner) internal returns (uint256 deposited) {
        deposited = token.transferBalance(address(vault), IWETH9(address(0)));
    ...
    function _handleNegativeCashflow(bool shouldRun, FlashLoanCallback memory cb) private {
        if (shouldRun && cb.cashflow < 0) {
            ...
            uint256 uCashflow = _transferOut(_cashflowToken, address(this), address(vault), uint256(-cb.cashflow));
    ...
    function _transferOut(IERC20 token, address payer, address to, uint256 amount) private returns (uint256) {
        return token.transferOut(payer, to, amount);
    }
    ...
    function createInstrument(Symbol symbol, IERC20 base, IERC20 quote) external override {
        ...
        instruments[symbol].baseUnit = base.unit();
        instruments[symbol].quote = quote;
        instruments[symbol].quoteUnit = quote.unit();
...
import "../libraries/DataTypes.sol";
import "../moneymarkets/interfaces/IMoneyMarket.sol";
...
library TransferLib {
    ...
    function transferOut(IERC20 token, address payer, address to, uint256 amount, IWETH9 nativeToken)
        internal
        returns (uint256)
    {
        if (payer == address(0)) revert ZeroPayer();
...
interface IFeeModel {
    ...
    function calculateFee(address trader, PositionId positionId, uint256 quantity)
        external
        view
...
interface IReferralManager is IReferralManagerEvents {
    ...
    // @dev The precision of the rewards and rebates is 1e18 == 100%
    ...
    function calculateRewardDistribution(address trader, uint256 amount)
        external
        view
