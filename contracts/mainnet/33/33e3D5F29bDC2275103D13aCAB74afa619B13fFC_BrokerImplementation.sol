...
contract BrokerImplementation is BrokerStorage, NameVersion {
    ...
    function trade(
        ...
        // Trade and withdraw
            ...
            } else if (asset == ISwapper(swapper).tokenWETH()) {
                IClient(client).transfer(tokenB0, address(this), balance);
                (, outAmount) = ISwapper(swapper).swapExactB0ForETH(balance);
                (bool success, ) = payable(msg.sender).call{value: outAmount}('');
                require(success, 'BrokerImplementation.trade: Send ETH fail');
            } else {
                IClient(client).transfer(tokenB0, address(this), balance);
...
interface ISwapper {
    ...
    function swapExactB0ForETH(uint256 amountB0) external returns (uint256 resultB0, uint256 resultBX);
    function swapExactB0ForBX(address tokenBX, uint256 amountB0) external returns (uint256 resultB0, uint256 resultBX);
}
