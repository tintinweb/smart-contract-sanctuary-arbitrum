...
/**
 * @title VeladashLending's Comptroller Contract
...
contract Comptroller is ComptrollerStorage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     */
    function enterMarkets(address[] memory vTokens) override public returns (uint[] memory) {
     ...
     */
    function exitMarket(address vTokenAddress) override external returns (uint) {
        /* Get sender tokensHeld and amountOwed underlying from the vToken */
        (uint oErr, uint tokensHeld, uint amountOwed, ) = VToken(vTokenAddress).getAccountSnapshot(msg.sender);
        ...
        Market storage marketToExit = markets[address(vTokenAddress)];
        ...
        uint assetIndex = len;
        for (uint i = 0; i < len; i++) {
            if (userAssetList[i] == VToken(vTokenAddress)) {
                assetIndex = i;
                break;
            }
        }
        ...
        emit MarketExited(VToken(vTokenAddress), msg.sender);
     ...
     */
    function mintAllowed(address vToken, address minter, uint mintAmount) override external view returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        require(!mintGuardianPaused[vToken], "mint paused");
     ...
     */
    function mintVerify(address vToken, address minter, uint actualMintAmount, uint mintTokens) override external {
        // Shh - currently unused
        vToken;
        minter;
        actualMintAmount;
     ...
     */
    function redeemAllowed(address vToken, address redeemer, uint redeemTokens) override external view returns (uint) {
     ...
     */
    function redeemVerify(address vToken, address redeemer, uint redeemAmount, uint redeemTokens) override external pure {
        // Shh - currently unused
        vToken;
        redeemer;
    ...
    function borrowAllowed(address vToken, address borrower, uint borrowAmount) override external returns (uint) {
        ...
        if (!markets[vToken].accountMembership[borrower]) {
            ...
            Error err_ = addToMarketInternal(VToken(msg.sender), borrower);
        ...
        uint err = comptrollerLens.checkPartialBorrowAllowedAndReturn(address(this), vToken, borrower, borrowAmount);
    ...
    function seizeAllowed(
        ...
        uint err = comptrollerLens.seizeAllowed(address(this), vTokenCollateral, vTokenBorrowed, seizeTokens);
        if (err != uint(Error.NO_ERROR)) {
            return err;
        }
     ...
     */
    function transferAllowed(address vToken, address src, uint transferTokens) override external view returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
    ...
    function getAccountLiquidity(address account) public view returns (uint, uint, uint) {
        (Error err, uint liquidity, uint shortfall) = getHypotheticalAccountLiquidityInternal(account, VToken(address(0)), 0, 0);
     ...
     */
    function liquidateCalculateSeizeTokens(address vTokenBorrowed, address vTokenCollateral, uint actualRepayAmount) override external view returns (uint, uint) {
    ...
    function _setMintPaused(VToken vToken, bool state) public returns (bool) {
        require(markets[address(vToken)].isListed, "market not listed");
    ...
    function accountHasMembership(address vToken, address account) external view returns(bool) {
        return markets[vToken].accountMembership[account];
    }
    ...
    function isDeprecated(VToken vToken) public view returns (bool) {
        return comptrollerLens.isDeprecated(address(this), vToken);
    }
...
/**
 * @title VeladashLending's InterestRateModel Interface
 * @author VeladashLending
 */
...
/**
 * @title VeladashLending's VToken Contract
...
abstract contract VToken is VTokenInterface, ExponentialNoError, TokenErrorReporter {
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        uint allowed = comptroller.transferAllowed(address(this), src, tokens);
        if (allowed != 0) {
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal {
        ...
        uint allowed = comptroller.seizeAllowed(address(this), seizerToken, seizeTokens);
        if (allowed != 0) {
...
abstract contract VErc20Interface is VErc20Storage {
    ...
    function borrowBehalf(address receiver, uint borrowAmount) virtual external returns (uint);
    function repayBorrow(uint repayAmount) virtual external returns (uint);
    function repayBorrowBehalf(address borrower, uint repayAmount) virtual external returns (uint);
    function liquidateBorrow(address borrower, uint repayAmount, VTokenInterface vTokenCollateral) virtual external returns (uint);
