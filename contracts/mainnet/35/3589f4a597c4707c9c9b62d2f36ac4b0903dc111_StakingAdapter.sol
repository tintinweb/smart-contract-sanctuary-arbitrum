...
contract StakingAdapter is IOceanPrimitive, Ownable {
    ...
    /// @notice The stake token address => gauge address
    ...
    function addGauge(address gauge_,  uint256 stakeTimestamp_) external onlyOwner {
        address stakingToken = IRewardsGauge(gauge_).lp_token();
        uint256 xToken = _calculateOceanId(stakingToken, 0);
        underlying[xToken] = stakingToken;
        decimals[xToken] = IERC20Metadata(stakingToken).decimals();
    ...
    function primitiveOutputAmount(uint256 inputToken, uint256 outputToken, uint256 inputAmount, address user, bytes32 minimumOutputAmount) internal returns (uint256 outputAmount) {
        ...
        address gauge = gauges[underlyingInputToken];

        if (gauge == address(0)) IRewardsGauge(underlyingInputToken).withdraw(rawInputAmount);
        else {
            if (stakeTimestamp[gauge] > block.timestamp) revert STAKING_DISABLED();
            IRewardsGauge(gauge).deposit(rawInputAmount, address(this));
        }

        rawOutputAmount = rawInputAmount;
     ...
     */
    function computeOutputAmount(uint256 inputToken, uint256 outputToken, uint256 inputAmount, address user, bytes32 metadata) external override onlyOcean returns (uint256 outputAmount) {
        uint256 unwrappedAmount = unwrapToken(inputToken, inputAmount, metadata);

        outputAmount = primitiveOutputAmount(inputToken, outputToken, unwrappedAmount, user, metadata);
