    ...
    ERC20Upgradeable
{
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        vaultState.unlockedAmount += uint104(newTokensReceived);
    ...
    function withdrawInstantly(uint256 amount) external nonReentrant {
        ...
        require( (depositReceipt.round == currentRound ||
                    depositReceipt.roundB == currentRound) , "Invalid round"); 
        

        if(depositReceipt.roundB == currentRound){
            receiptAmount = depositReceipt.amountB;
            depositReceipt.amountB = uint104(receiptAmount.sub(amount));
        }else if(depositReceipt.roundB == currentRound){
        ...
        vaultState.unlockedAmount = uint104(
                uint256(vaultState.unlockedAmount).sub(amount)
        );
    ...
    function _depositFor(uint256 amount, address creditor) private {        
        ...
        if ((receipt.round == 0) || //To manage first deposit 
            (receipt.round < (currentRound - 2))) //To manage deposit older than n-2 rounds
        {
            receipt.round =  uint16(currentRound);
            receipt.amount = uint104(depositAmount);
            receipt.roundB = 0;
            receipt.amountB = 0;
        }else if (receipt.roundB == 0){ //To manage deposit in consecutive round
        ...
        // ShareMathBeefy.assertUint128(newTotalPending);
