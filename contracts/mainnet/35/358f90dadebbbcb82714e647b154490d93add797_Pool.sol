...
address constant TARGET_TRANCHE = 0x5573405636F4b895E511C9C54aAfbefa0E7Ee458;
    ...
    IPool
{
    ...
    /* ======== INITIALIZERS ========= */
    function initialize(uint256 _maxLeverage, uint256 _maintainanceMargin, uint256 _interestAccrualInterval)
        external
        initializer
    {
        ...
        onlyListedToken(_token)
    {
        ...
        (uint256 daoFee_,) = _calcDaoFee(feeAmount);
        feeReserves[_token] += daoFee_;
        trancheAssets[_tranche][_token].poolAmount += (_amountIn - daoFee_);
        ...
        onlyAsset(_tokenOut)
    {
        ...
        (uint256 daoFee_,) = _calcDaoFee(feeAmount);
        feeReserves[_tokenOut] += daoFee_;
        _decreaseTranchePoolAmount(_tranche, _tokenOut, outAmountAfterFee + daoFee_);
        refreshVirtualPoolValue();
        ...
        nonReentrant
    {
        ...
        accrueInterest(_viaToken);
        _requireAmount(_lpAmount);

        uint256 viaTokenPrice = _getPrice(_viaToken, true);
        uint256 fromTrancheValue = liquidityCalculator.getTrancheValue(_fromTranche, true);
        uint256 targetTrancheValue = liquidityCalculator.getTrancheValue(TARGET_TRANCHE, true);
        ...
        if (targetLpOut < _minOut) revert SlippageExceeded();
        ...
        nonReentrant
    {
        ...
        {
            (uint256 _daoFee, uint256 _lpFee) = _calcDaoFee(swapFee);
            feeReserves[_tokenIn] += _daoFee;
            _rebalanceTranches(_tokenIn, amountIn - swapFee, _tokenOut, amountOutAfterFee, _lpFee);
        }
    ...
    function increasePosition(
        ...
        DataTypes.Side _side
    ) external whenNotPaused onlyOrderManager {
    ...
    // ========= ADMIN FUNCTIONS ========
    function setInterestRateModel(address _token, address _interestRateModel) external onlyOwner onlyAsset(_token) {
    ...
    function setSwapAllowed(address _sender, bool _allowed) external onlyOwner {
        if (_allowed != allowSwap[_sender]) {
            allowSwap[_sender] = _allowed;
            emit SwapAllowanceChanged(_sender, _allowed);
        }
    }
    ...
    function _rebalanceTranches(
        ...
        uint256[] memory outAmounts = _calcReduceTranchesPoolAmount(indexToken, _tokenOut, _amountOut);

        for (uint256 i = 0; i < outAmounts.length;) {
            address tranche = allTranches[i];
            uint256 lpFee = isStableCoin[_tokenIn]
                ? _lpFee / outAmounts.length
                : MathUtils.frac(_lpFee, riskFactor[_tokenIn][tranche], totalRiskFactor[_tokenIn]);
            trancheAssets[tranche][_tokenOut].poolAmount -= outAmounts[i];
            trancheAssets[tranche][_tokenIn].poolAmount +=
                MathUtils.frac(_amountInAfterFee, outAmounts[i], _amountOut) + lpFee;
            unchecked {
                ++i;
            }
        }
    ...
    function _validateMaxLiquidity(address _token) internal view {
        ...
        if (max > 0 && _getPoolAsset(_token).poolAmount > max) {
    ...
    function _updateGlobalShortPosition(
        ...
        globalShortSize = globalShortSize + nextSize - lastSize;
    }
    ...
    function _checkSwapAllowed() internal view {
