...
contract GemStoneFIPresale is Ownable, AccessControl, ReentrancyGuard{
    using SafeERC20 for IERC20;

    bytes32 constant public MANAGER_ROLE = keccak256('MANAGER_ROLE');

    IERC20 public GemStoneFIToken;
    AggregatorV3Interface internal priceFeed;

    address payable public treasury;
    address private constant CHAINLINK_MAINNET_ETH_USD_PRICEFEED_ADDRESS = 0x639Fe6ab55C921f74e7fac1ee960C0B6293ba612;
    ...
    uint256 public phase1TotalTokensToSell = 75_000_000 * 1e18;
    uint256 public phase2TotalTokensToSell = 100_000_000 * 1e18;
    uint256 public phase3TotalTokensToSell = 125_000_000 * 1e18;

    uint public maxTokenCapForPresale = 300_000_000 * 1e18;
    uint public maxTokensToPurchasePerWallet = 7_000_000 * 1e18;
    ...
    mapping(address => uint) public totalETHInvestmentsByIUser;
    ...
    event RecoverETH(address indexed caller, address indexed destinationWallet, uint indexed amount, uint timestamp);
    ...
    modifier claimsOpen(){
     ...
     modifier claimsNotOpen(){
        require(isOpenForClaims == false,"Claims are Open Presale Ended");
        _;
    }

    constructor(address _token, address payable _treasury, uint _durationInDays){
        require(_token != address(0),"Invalid Address");
        require(_durationInDays >= 1,"Presale Runs past 1 days");
        _grantRole(MANAGER_ROLE, _msgSender());
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());

        GemStoneFIToken = IERC20(_token); 
        treasury = _treasury;
        priceFeed = AggregatorV3Interface(CHAINLINK_MAINNET_ETH_USD_PRICEFEED_ADDRESS);
        presaleDurationInSec = block.timestamp + (_durationInDays * 1 * 60 * 60);
    }
    ...
    function EndPresale(bool _status) external onlyRole(MANAGER_ROLE){
        isOpenForClaims = _status;
           isPhase2Active = false;
           isPhase1Active = false;
           isPhase3Active = false;
    ...
    function setPresaleDurationInDays(uint256 _days) public onlyRole(MANAGER_ROLE){
        require(_days >= 1,"Presale Runs past 1 days");
        presaleDurationInSec = block.timestamp + (_days * 1 * 60 * 60);
    }
    ...
    function contribute() public payable nonReentrant presaleOpen claimsNotOpen presaleUnpaused{
        _checkInvestorCoolDownBeforeNextPurchase(msg.sender);
        uint256 pricePerToken = _getTokenPriceByPhase();
        (uint256 price, uint256 decimals) = _getLatestETHPriceInUSD();
        ...
        totalETHInvestmentsByIUser[msg.sender] += weiAmount;
    ...
    function claimContribution() public claimsOpen nonReentrant{
        ...
        require(
            IERC20(GemStoneFIToken).transfer(msg.sender, totalClaimableTokens),
    ...
    function recoverETH() public onlyRole(MANAGER_ROLE) nonReentrant{
        uint256 balance = getContractETHBalance();
        require(balance > 0,"Insufficient Contract Balance");
        treasury.transfer(balance);
        emit RecoverETH(msg.sender, treasury, balance, block.timestamp);
    }
    ...
    nonReentrant nonZeroAddress(_token) nonZeroAddress(_account){
        ...
        require(token != GemStoneFIToken,"Invalid Token");
        token.safeTransfer(_account, _amount);
        emit RecoverERC20Tokens(_msgSender(), _account, _amount, block.timestamp);
    }

    function getContractTokenBalance() public view returns(uint256){
        return GemStoneFIToken.balanceOf(address(this));
    }
    ...
    function updateMaxTokenCapForPresale(uint256 _amount) public onlyRole(MANAGER_ROLE){
        require(_amount >= 300_000_000,"Max Cap Must be above 30% of total supply");
    ...
    function _getLatestETHPriceInUSD() private view returns(uint256, uint256){
    ...
    function _refundUnsoldTokens(address _destination) private{
        ...
        require(GemStoneFIToken.transfer(_destination, refundableBalance),"Failed To Refund Tokens");
    }
