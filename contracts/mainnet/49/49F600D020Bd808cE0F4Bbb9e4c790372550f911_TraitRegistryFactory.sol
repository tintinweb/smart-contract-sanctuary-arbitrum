...
import "@openzeppelin/contracts/access/Ownable.sol";
import "../Implementers/Generic/GenericTrait.sol";

interface GenericTraitInterface {
    function setup( address _registry, uint16 _traitId, traitConfig memory _traitConfig, bytes[] memory _defaultPropValues ) external;
    function init() external;
    function TRAIT_TYPE() external returns (uint16);
}
...
contract TraitRegistryFactory is Ownable, BlackHolePrevention, NewProxy {
    ...
    function deploy(
        ...
        address tokenAddress = thisCommunityRegistry.getRegistryAddress(string(abi.encodePacked("TOKEN_", _tokenNum.toString())));
        ...
        address trait_proxy = newProxy("GOLDEN_TRAIT_REGISTRY");   // All golden contracts should start with `GOLDEN_`
        GTRegistry traitRegistry = GTRegistry(address(trait_proxy));
        traitRegistry.init(_communityId, _tokenNum);                // To initialise owner
        ...
        // load tokenAddress and find MAX_SUPPLY
        deployTraitZero(traitRegistry, thisCommunityRegistry, ERC721Token(tokenAddress).maxSupply());

        // grant creator TRAIT_REGISTRY_ADMIN
        thisCommunityRegistry.grantRole(TRAIT_REGISTRY_ADMIN, msg.sender);

        return address(traitRegistry);
    }

    function deployTraitZero(GTRegistry traitRegistry, CommunityRegistry thisCommunityRegistry, uint256 maxSupply) internal {
        // Launch new trait contract via proxy
        address trait_proxy = newProxy("GOLDEN_TRAIT_ZERO");
        GenericTraitInterface trait = GenericTraitInterface(trait_proxy);
        
        bytes[] memory _defaultPropValues = new bytes[](13);
        _defaultPropValues[0] = abi.encode(0);
        _defaultPropValues[1] = abi.encode(0);
        _defaultPropValues[2] = abi.encode(0);
        _defaultPropValues[3] = abi.encode(0);
        
        // modifier_lock
        _defaultPropValues[4] = abi.encode(true);
        // movement_permission
        _defaultPropValues[5] = abi.encode(MovementPermission.LOCKED);

        // activation
        _defaultPropValues[6] = abi.encode(block.timestamp);
        // cooldown
        _defaultPropValues[7] = abi.encode(3600 * 24 * 7);  // 7 days lock time
        // expiration
        _defaultPropValues[8] = abi.encode(0);  // never expires
        // counter
        _defaultPropValues[9] = abi.encode(1);  // this trait resets itself on use

        _defaultPropValues[10] = abi.encode(0);
        _defaultPropValues[11] = abi.encode(0);
        _defaultPropValues[12] = abi.encode(0);
        ...
        trait.setup(
            address(traitRegistry),
            1,
            traitConfig(true),  // inverted trait
            _defaultPropValues
        );
        trait.init();
        ...
        // TODO: fix start / end
        traits[0] = GTRegistry.traitStruct(0, uint8(trait.TRAIT_TYPE()), 1, maxSupply, true, address(trait), "ipfs://traitZERO", "TraitZero");
        traitRegistry.addTrait(traits);

        thisCommunityRegistry.renounceRole(TRAIT_REGISTRY_ADMIN, address(this));
    }
}
