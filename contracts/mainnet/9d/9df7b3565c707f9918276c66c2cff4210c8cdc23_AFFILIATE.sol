...
contract AFFILIATE is ERC20, Ownable {
    ...
    function _AffiTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		if (distributor != address(0)) {
                uint256 PurchasedAmount = readDistributor(distributor).getPresale(sender);
                require((balanceOf(sender) - amount) >= PurchasedAmount, "you can only transfer presale/airdrop tokens at the end of the presale/airdrop");
            }
