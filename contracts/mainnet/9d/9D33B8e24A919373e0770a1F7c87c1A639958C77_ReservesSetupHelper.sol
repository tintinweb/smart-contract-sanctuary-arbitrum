 ...
 *          Based on AaveV3's ReservesSetupHelper
 */
 ...
 * @dev     Based on AaveV3's IPoolConfigurator
 */
interface IPoolConfigurator {
    ...
    /**
     * @dev     Emitted when a flashloan privileged address is set
     * @param   contractAddress     The address of the contract
     * @param   isPrivileged        Set to true or false
     */
    event SetFlashLoanPrivilege(
    ...
    /**
     * @dev     Emitted when a new PublicLiquidator was set
     * @param   oldPublicLiquidator Address of the original PublicLiquidator or address(0) in case there is none
     * @param   newPublicLiquidator Address of the new PublicLiquidator or address(0) in case it is disabled
     */
    event PublicLiquidatorSet(
        address oldPublicLiquidator,
        address newPublicLiquidator
    );
     ...
     * @dev     If zero is provided as oracle address, the default asset oracles will be used
     *          to compute the overall debt and overcollateralization of the users using this category.
     * @dev     The new ltv and liquidation threshold must be greater than the base ltvs and liquidation
     *          thresholds of all assets within the eMode category
    ...
    /**
     * @notice      Set an address to be have flashloan privilege or not. A contract with flashloan privilege can
     *              call flashloan with 0 fees. Specially for Migrator, Leverager, PublicLiquidator and specific
     *              tokenomics design
     * @param _contract                 The address of the MToken to update its rebasing distributor
     * @param _flashloanPrivilege       The address of the rebasing distributor
     */

    function setFlashloanPrivilege(
        address _contract,
        bool _flashloanPrivilege
    ) external;

    /**
     * @notice  Updates the total flash loan premium.
     * @dev     Total flash loan premium consists of two parts:
    ...
    /**
     * @notice  Start the underlying incentivesController contract
     * @param   _incentivesController   The address of the incentivesController
     * @param   _rewardMinter           The address of the protocol token minter
     * @param   _maxMintable            The max amount of token allocated to the incentivesController to mint
     * @param   _startTimeOffset        The period that has passed since the starting time
     * @param   _rewardsPerSecond       The reward for the given _startTimeOffset period
     * @dev     _startTimeOffset and _rewardsPerSecond must have the same length
     */
    function startIncentivesController(
    ...
    /**
     * @notice  Add a new pool on IncentivesController
     * @param   _incentivesController     Address of the IncentivesController
     * @param   _asset                    The address of the asset to be incentivized (mToken, variableDebt and stableDebt)
     * @param   _allocPoint               Allocation points for the specific pool
     */
    ...
    /**
     * @notice  Batch update alloc points on incentives controller
     * @param   _incentivesController     Address of the IncentivesController
     * @param   _asset                    The address of the asset to be incentivized (mToken, variableDebt and stableDebt)
     * @param   _allocPoint               Allocation points for the specific pool
     */

    function batchUpdateIncentivesAllocPoints(
        address _incentivesController,
    ...
    /**
     * @notice  Set the onward incentives controller of an incentivized pool on incentives controller
     * @param   _asset                        The asset to set onward incentives controller on
     * @param   _onwardIncentivesController   The address onf the onward incentives controller
     */
    function setOnwardIncentivesController(
        address _incentivesController,
        address _asset,
        address _onwardIncentivesController
    ) external;
    ...
    /**
     * @notice  Set the on behald reiceiver of any tokens accumulated on ChefIncentivesController
     * @param   _incentivesController   Address of the IncentivesController
     * @param   _user                   The user that have earned protocol tokens from ChefIncentivesController
     * @param   _onBehalf               The receiver of the earned protocol tokens when _user claims
     */
    function setIncentivesControllerClaimReceiverOnBehalf(
        address _incentivesController,
        address _user,
        address _onBehalf
    ) external;
    ...
    /**
     * @notice  Delegates the call from PoolConfigurator to another contract
     * @param   _target Address of the target contract
     * @param   _data   The calldata to pass along with the call
     */
    function delegateCall(
 ...
 * @dev     Based on AaveV3's PoolConfigurator
 */
contract PoolConfigurator is VersionedInitializable, IPoolConfigurator {
    ...
    /// @inheritdoc IPoolConfigurator
    function setPublicLiquidator(
        address _publicLiquidator
    ) external override onlyRiskOrPoolAdmins {
        address oldPublicLiquidator = _pool.PUBLIC_LIQUIDATOR();
        _pool.setPublicLiquidator(_publicLiquidator);
        emit PublicLiquidatorSet(oldPublicLiquidator, _publicLiquidator);
    }
    ...
    function setFlashloanPrivilege(
        ...
        _pool.setFlashloanPrivilege(_contract, _flashloanPrivilege);
        emit SetFlashLoanPrivilege(_contract, _flashloanPrivilege);
    }
    ...
    function batchUpdateIncentivesAllocPoints(
        ...
        incentivesController.batchUpdateAllocPoint(_assets, _allocPoints);
    }
    ...
    function setOnwardIncentivesController(
        ...
        incentivesController.setOnwardIncentives(
            _asset,
    ...
    function setIncentivesControllerClaimReceiverOnBehalf(
        ...
        incentivesController.setClaimReceiver(_user, _onBehalf);
    }
    ...
    function setMTokenYieldDistributor(
        ...
        _pool.setMTokenYieldDistributor(_mToken, _yieldDistributor);
    }
    ...
    {
        (success, resultData) = _target.delegatecall(_data);
        require(success, Errors.DELEGATE_CALL_FAILED);
    }
    ...
    function _checkNoSuppliers(address asset) internal view {
        ...
        require(
            totalMTokens == 0 && accruedToTreasury == 0,
