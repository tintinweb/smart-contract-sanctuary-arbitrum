...
interface IBoostedMultiRewarder {
    ...
    function rewardInfo(uint256 index) external view returns(RewardInfo memory info);
...
import {IBoostedMultiRewarder} from "./IBoostedMultiRewarder.sol";
/**
...
interface IMasterWombatV3Reader {
    ...
    function boostedRewarders(uint256 poolId) external view returns (IBoostedMultiRewarder rewarder);
}
...
import {IBoostedMultiRewarder} from "./interfaces/wombat/IBoostedMultiRewarder.sol";
   ...
   // IWombatRouterReader constant public WombatRouter = IWombatRouterReader(0x19609B03C976CCA288fbDae5c21d4290e9a4aDD7);
    ...
    function getMagpiePoolInfo(uint256 poolId, address account) public view returns (MagpiePool memory) {
        ...
        if (magpiePool.stakingToken == mWom) {
            ...
            address boostedRewarder = address(masterWombatV3.boostedRewarders(magpiePool.wombatStakingPool.pid));
            if (magpiePool.wombatV3Pool.rewarder != address(0) || boostedRewarder != address(0)) {
                magpiePool.wombatV3Pool.rewardList = _getWombatV3PoolRewardList(magpiePool.wombatV3Pool.rewarder, boostedRewarder);
            }
        ...
        if (tokenRouter.token != address(0)) {
           if (tokenRouter.routerType == PancakeRouterType) {
            ...
            amountOut = prices[tokenRouter.paths.length - 1];
              ...
              try wombatRouter.getAmountOut(tokenRouter.paths, tokenRouter.pools, int256(10) ** tokenRouter.decimals) 
                returns (uint256 amountOutRes, uint256[] memory) {
                amountOut = amountOutRes;
    ...
    function _getWombatV3PoolRewardList(address rewarderAddress, address boostedRewarderAddress) internal view returns (WombatV3PoolReward[] memory) {

        WombatV3PoolReward[] memory rewardList = _addV3RewardsInfo(rewarderAddress);        
        WombatV3PoolReward[] memory newRewardList = _addV3BoostedRewardsInfo(boostedRewarderAddress, rewardList);
        return newRewardList;
    }

    function _addV3RewardsInfo(address rewarderAddress) internal view returns (WombatV3PoolReward[] memory) {
        if(rewarderAddress == address(0))
            return new WombatV3PoolReward[](0);
    ...
    function _addV3BoostedRewardsInfo(address boostedRewarderAddress, WombatV3PoolReward[] memory rewardList) internal view returns(WombatV3PoolReward[] memory newRewardList) {

        if(boostedRewarderAddress == address(0))
            return rewardList;

        IBoostedMultiRewarder boostedRewarder = IBoostedMultiRewarder(boostedRewarderAddress);
        bool[] memory isEmissionActive = boostedRewarder.isEmissionActive();
        uint256 rewardsLength = boostedRewarder.rewardLength();
        uint256 activeRewardsLength;
        for(uint256 i = 0; i< rewardsLength; i++)
            if(isEmissionActive[i])
                activeRewardsLength++;
        
        newRewardList = new WombatV3PoolReward[](activeRewardsLength + rewardList.length);
        uint256 n;
        for(n = 0; n < rewardList.length; ++n) 
            newRewardList[n] = rewardList[n];
        ...
        for(uint256 i = 0; i< rewardsLength; ++i){
            ...
            if(!isEmissionActive[i])
                continue;
            WombatV3PoolReward memory poolReward;
            IBoostedMultiRewarder.RewardInfo memory rewardInfo = boostedRewarder.rewardInfo(i);
            address rewardToken = address(rewardInfo.rewardToken);
            poolReward.rewardToken = rewardToken;
            poolReward.tokenPerSec = rewardInfo.tokenPerSec;
            poolReward.accTokenPerShare = rewardInfo.accTokenPerShare;
            poolReward.distributedAmount = rewardInfo.distributedAmount;
            poolReward.rewardTokenDecimals = ERC20(rewardToken).decimals();
            poolReward.rewardTokenSymbol = ERC20(rewardToken).symbol();

            newRewardList[n++] = poolReward;
        }
        return newRewardList;
    }
    ...
    function _addTokenRouteInternal(address tokenAddress, address [] memory paths, address[] memory pools) internal returns (TokenRouter memory tokenRouter) {
    ...
    // function addTokenRouter(uint16 routerType, address tokenAddress, address [] memory paths, address[] memory pools, address priceAddress) external onlyOwner {
    //     if(routerType > 5){
    //         revert("INVALID ROUTER");
    //     }
    //     TokenRouter memory tokenRouter = _addTokenRouteInternal(tokenAddress, paths, pools);
    //     if(routerType == PancakeRouterType){
    //         tokenRouter.routerType = PancakeRouterType;
    //     }else if(routerType == WombatRouterType){
    //         tokenRouter.routerType = WombatRouterType;
    //     }else if(routerType == ChainlinkType){
    //         tokenRouter.routerType = ChainlinkType;
    //         tokenRouter.chainlink = priceAddress;
    //     }else if(routerType == UniswapV3RouterType){
    //         tokenRouter.routerType = UniswapV3RouterType;
    //     }else if(routerType == TraderJoeV2Type){
