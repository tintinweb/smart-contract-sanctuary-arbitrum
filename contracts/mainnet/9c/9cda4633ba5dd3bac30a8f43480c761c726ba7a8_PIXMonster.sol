...
contract PIXMonster is Ownable, ReentrancyGuard {
    ...
    address public RESERVE = 0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address ROUTER = 0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45;
    address LEGACY_ROUTER = 0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45;
    address MAIN_ROUTER = ROUTER;
    address SELL_RECEIVER = 0x6ba52ce1E232f27e5Ba0EbA7Ab0a81C3d737A643;
    ...
    // GLOBAL TOKEN INFO
    uint24 public standardPoolFeeTier = 100;
    ...
    event BuyBack(uint256 amountIn, address receiver);
    ...
    receive() external payable {
        require(
            msg.value > 0,
            "BnbToUsdtExchange: BNB amount must be greater than zero"
        );
        uint256 amountToLiquify = msg.value;
        newSwap(
            WETH9,
            RESERVE,
            amountToLiquify,
    ...
    function newSwap(
        ...
        if (_swapType == SwapType.v3Single) {
            IV3SwapRouter.ExactInputSingleParams memory params = IV3SwapRouter
                .ExactInputSingleParams({
                    ...
                    recipient: _to,
                    amountIn: _amountIn,
                    amountOutMinimum: 0, // for now
                    sqrtPriceLimitX96: 0
                });

            router.exactInputSingle(params);
        }
        if (_swapType == SwapType.v3Multi) {
        ...
        if (_swapType == SwapType.v2) {
            if (_tokenOut == WETH9) {
                ...
                IWETH9(WETH9).withdraw(amountsOut);
            ...
            if (_tokenOut == RESERVE) {
                IERC20(RESERVE).transfer(address(_to), _amountIn);
            }
            if (_tokenOut == buyBackToken) {
                /// @notice We don't have NIX token in others Chains, so we will only save the event info of supposed buyback
                /// this will be sent to the other chain and the buyback will be done there
                /// @dev  We will need to create a function to check if the supposed buyback was done
                /// @param amountOut is the amount of tokens that will be bought
                /// @return  the amount of tokens that will be bought

                emit BuyBack(_amountIn, _to);
            }
            if (
                _tokenOut != WETH9 &&
                _tokenOut != RESERVE &&
                _tokenOut != buyBackToken
            ) {
                path = new address[](3);
                path[0] = _tokenIn;
                path[1] = _path;
