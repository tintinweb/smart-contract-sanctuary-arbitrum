...
import {ICCTP} from "../interfaces/external/ICCTP.sol";

contract CCTP is Bridge, DefiOp {
    using SafeERC20 for IERC20;

    ICCTP public immutable cctp;
    IERC20 public immutable usdc;

    constructor(ICCTP cctp_, IERC20 usdc_) {
        cctp = cctp_;
        usdc = usdc_;
    }
     ...
     */
    function useCCTP(uint64 chainId) external onlyOwner {
        uint256 tokenAmount = usdc.balanceOf(address(this));

        usdc.safeApprove(address(cctp), tokenAmount);
        cctp.depositForBurn(
            tokenAmount,
            _getDomain(chainId),
    ...
    function _getDomain(uint64 chainId) internal pure returns (uint32 domain) {
        if (chainId == ETHEREUM_CHAIN_ID) domain = 0;
        else if (chainId == AVALANCHE_CHAIN_ID) domain = 1;
        else if (chainId == OPTIMISM_CHAIN_ID) domain = 2;
        else if (chainId == ARBITRUM_ONE_CHAIN_ID) domain = 3;
        else if (chainId == BASE_CHAIN_ID) domain = 6;
        else if (chainId == POLYGON_CHAIN_ID) domain = 7;
        else revert UnsupportedDestinationChain(chainId);
    }
}
