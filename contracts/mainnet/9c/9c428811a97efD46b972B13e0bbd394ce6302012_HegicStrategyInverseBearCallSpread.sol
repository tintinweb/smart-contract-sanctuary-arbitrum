...
abstract contract HegicInverseStrategy is HegicStrategy {
    ...
    ) HegicStrategy(_priceProvider, _pricer, _limit, _spotDecimals, periodLimits, 0, _limitController) {
    ...
    ReentrancyGuard
{
    ...
    function connect() external override {
        ...
        require(limitPool == address(0) || limitPool == msg.sender, "OperationalTreasury only");
...
contract LimitController is AccessControl {
    ...
    function setLimit(uint256 value) onlyRole(DEFAULT_ADMIN_ROLE) external {
        _limit = value;
    }

    function setOperationalTreasury(IOperationalTreasury value) onlyRole(DEFAULT_ADMIN_ROLE) external {
    ...
    function limit() external view returns(uint) {
        ...
        uint operationalTreasuryLimit = operationalTreasury.totalBalance() - operationalTreasury.totalLocked() - operationalTreasury.lockedPremium();
        return _limit < operationalTreasuryLimit ? _limit : operationalTreasuryLimit;
    }
}