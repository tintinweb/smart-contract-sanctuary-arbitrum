...
contract Bullish is ReentrancyGuard, Pausable, XNFTHolder
{
	...
	event AdjustMintPeriodCB(address indexed user, address _address_token_reward, uint256 _start_block_id, uint256 _end_block_id);
	...
	{
		require(_address_token_reward != address(0), "extend_mint_period: Wrong period");
	...
	function adjust_mint_period(address _address_token_reward, uint256 _start_block_id, uint256 _end_block_id) external onlyOperator
	{
		require(_address_token_reward != address(0), "adjust_mint_period: Wrong period");
		require(_end_block_id > _start_block_id, "adjust_mint_period: Wrong period");

		RewardInfo storage reward = reward_info[_address_token_reward];
		reward.emission_start_block_id = (block.number > _start_block_id)? block.number : _start_block_id;
		reward.emission_end_block_id = _end_block_id;

		emit AdjustMintPeriodCB(msg.sender, _address_token_reward, _start_block_id, _end_block_id);
	}
