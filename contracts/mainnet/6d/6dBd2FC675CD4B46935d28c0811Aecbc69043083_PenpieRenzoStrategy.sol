...
interface IVault {
    ...
    function batchSwap(
        ...
        uint256 deadline) external payable returns (int256[] memory assetDeltas);
...
import { IPendleMarket, IPendleRouter } from '../../interfaces/IPendle.sol';
import { IPenpieDepositorHelper } from '../../interfaces/penpie/IPenpieDepositorHelper.sol';
import { IMasterPenpie } from '../../interfaces/penpie/IMasterPenpie.sol';
...
/**
 * @title PenpieRenzoStrategy
 * @notice This contract is a strategy that interacts with the Pendle protocol.
 *
 */
contract PenpieRenzoStrategy is IStrategy, StratManager, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    address public constant arbToken = 0x912CE59144191C1204E64559FE8253a0e49E6548;
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ...
        if (balancePenpie > 0) {
            _swapCamelot(penpieToken, weth, balancePenpie);
        }
        uint256 balancePendle = IERC20(pendleToken).balanceOf(address(this));
        if (balancePendle > 0) {
            _swapUniswap(pendleToken, weth, balancePendle, 0);
        }
        uint256 balanceArb = IERC20(arbToken).balanceOf(address(this));
        if (balanceArb > 0) {
            _swapUniswap(arbToken, weth, balanceArb, 0);
        }
        if (balancePenpie > 0 || balancePendle > 0 || balanceArb > 0) {
            chargeFees(callFeeRecipient);
            uint256 harvestAmount = IERC20(weth).balanceOf(address(this));
            _swapCamelot(weth, ezeth, IERC20(weth).balanceOf(address(this)));
            uint256 balanceEzETH = IERC20(ezeth).balanceOf(address(this));
            _deposit(balanceEzETH);
            IPenpieDepositorHelper(penpieDepositorHelper).depositMarket(
                address(_asset),
    ...
    function _giveAllowances() internal {
        IERC20(ezeth).safeApprove(pendleRouter, type(uint256).max);
    ...
    function _removeAllowances() internal {
        IERC20(ezeth).safeApprove(pendleRouter, 0);
     ...
     * @param _fromToken address token will take to swap.
     * @param _toToken address token will swap to.
     * @param _amountIn The amount of input tokens to be swapped.
     */
    function _swapCamelot(address _fromToken, address _toToken, uint256 _amountIn) internal {
        TransferHelper.safeApprove(_fromToken, penpieSwapRouter, _amountIn);
...
contract StratManager is OwnableUpgradeable, PausableUpgradeable {
    ...
    uint256 public beforeDepositBalance;
    uint256 public afterDepositBalance;
    ...
    function __StratFeeManager_init(StratFeeManagerParams calldata params) internal onlyInitializing {
        ...
        callFee = 1e16;
        slippage = 100;
        beforeDepositBalance = 0;
        afterDepositBalance = 0;
    }
