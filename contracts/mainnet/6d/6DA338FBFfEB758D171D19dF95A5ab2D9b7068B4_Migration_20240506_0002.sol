...
import { L2ChainlinkAggregatorPriceOracle } from "../../protocol/oracle/L2ChainlinkAggregatorPriceOracle.sol";

contract Migration_20240506_0002 is Migration { // solhint-disable-line contract-name-camelcase
  ...
  function migrate() internal override onlyAdmin {
    L2ChainlinkAggregatorPriceOracle oracle = new L2ChainlinkAggregatorPriceOracle(
    ...
    MigrationLib.updateOracle(_registry, Constants.WETH, address(oracle));
  }
}
...
contract L2ChainlinkAggregatorPriceOracle is IPriceOracle {

  AggregatorV3Interface immutable public _aggregator;
  ...
  constructor(AggregatorV3Interface aggregator, uint256 timeout, AggregatorV3Interface sequencerUptime, uint256 gracePeriod) {
  ...
  function getLatestPrice() external view returns (USD price, IPriceOracle.Confidence confidence) {
    require(_aggregator.decimals() == USDMath.DECIMALS, "invalid price feed decimals");

    ( , int256 answer, , uint256 timestamp, ) = _aggregator.latestRoundData();

    price = USD.wrap(answer < 0 ? 0 : uint256(answer));

    confidence = getConfidence(timestamp);
  }  

  function getConfidence(uint256 timestamp) private view returns (IPriceOracle.Confidence) {
    // check the sequencer status 
    (IPriceOracle.Status status, uint256 changedAt) = getStatus();

    if (status == IPriceOracle.Status.OFFLINE) {
      return IPriceOracle.Confidence.OFFLINE;
    }
    ...
    if (block.timestamp - timestamp > _timeout) {
      return IPriceOracle.Confidence.STALE;
    }

    return IPriceOracle.Confidence.OK;
  }

  /// @inheritdoc IPriceOracle
  function getStatus() public view returns (IPriceOracle.Status status, uint256 changedAt) {
    (, int256 answer, uint256 startedAt, , ) = _sequencerUptime.latestRoundData();

    status = answer == 0 ? IPriceOracle.Status.ONLINE : IPriceOracle.Status.OFFLINE;
    changedAt = startedAt;
  }
}
