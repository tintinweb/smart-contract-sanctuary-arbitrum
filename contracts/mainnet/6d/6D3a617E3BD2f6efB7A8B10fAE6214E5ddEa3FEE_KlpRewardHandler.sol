...
import {IKTCLeverageVault} from "./interfaces/water/IKTCLeverageVault.sol";
...
contract KlpRewardHandler is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    address public waterFeeReceiver;
    mapping(address => bool) public canClaimForUser;
    mapping(address => address) public chainlinkOracle;
    mapping(address => bool) public notAllowToSwap;
    mapping(address => mapping(address => uint24)) public assetToFees;
    ...
    function setSlippageAndSequencer(uint256 _slippage, address _sequencer) external onlyOwner {
        arbitrumSequencer = _sequencer;
        uniswapSlippage = _slippage;
    }

    function setAssetToFee(address _swapFrom, address _swapTo, uint24 _fee) external onlyOwner {
        assetToFees[_swapFrom][_swapTo] = _fee;
    }

    function allowToSwap(address _token, bool _allow) external onlyOwner {
        notAllowToSwap[_token] = _allow;    
    }
    ...
    function totalSupply() public view returns (uint256) {
        return IKTCLeverageVault(address(podToken)).totalSupply();
    }

    function getTotalPosition(address account) public view returns (uint256) {
        return IKTCLeverageVault(address(podToken)).getAggregatePosition(account);
    }

    function pendingRewardsKlp(address account) public view returns (uint256) {
        if (totalSupply() == 0) {
            return 0;
        }
    ...
    function getKLPSplit(uint256 _amount) public view returns (uint256, uint256, uint256) {
        uint256 profit;
    ...
    function distributeKlp(uint256 _amount) public onlyVodkaVault {
    ...
    function distributeRewards(
        ...
        uint256 amountOut = _swap(WETH, USDC, address(this), _waterAmount);
        // transfer to water fee receiver
        IERC20Upgradeable(USDC).safeTransfer(waterFeeReceiver, amountOut);
    }
    ...
    function claimETHRewards(address _account) public {
        ...
        IKTCLeverageVault(address(podToken)).handleAndCompoundRewards();

        // only vodka vault can call this function
        uint256 WETHRewards = pendingRewardsKlp(sender);
        if (WETHRewards > 0) {
    ...
    function swap(address _assetFrom, address _to, address _receiver, uint256 _amount) public  returns (uint256) {
        require(notAllowToSwap[msg.sender], "!allowed to swap");
        return _swap(_assetFrom, _to, _receiver, _amount);
    }

    function _swap(address _assetFrom, address _to, address _receiver, uint256 _amount) internal returns (uint256) {
        IERC20Upgradeable(_assetFrom).approve(address(uniRouter), _amount);
        uint256 amountOutMin = getAnySlippagedAmountOut(_assetFrom, _to, _amount);
        uint24 _fees = assetToFees[_assetFrom][_to];
        require(_fees > 0, "Invalid fee");
    ...
    function getLatestData(address _token) public view returns (uint256) {
        ...
        uint8 decimals = IAggregatorV3Interface(chainlinkOracle[_token]).decimals();
        if(decimals != 18) {
    ...
    function getAnySlippagedAmountOut(address _assetFrom, address _to, uint256 _amount) public view returns (uint256) {
        uint256 assetOutPrice = getLatestData(_to);
        uint256 convertToDecimals = assetOutPrice * (10 ** IERC20MetadataUpgradeable(_to).decimals()) / 1e18;
        uint256 pricedInAssetAmount = (_amount * convertToDecimals) / (10 ** IERC20MetadataUpgradeable(_assetFrom).decimals());
        uint256 amountOutMin = pricedInAssetAmount - (pricedInAssetAmount * uniswapSlippage / 1000);
