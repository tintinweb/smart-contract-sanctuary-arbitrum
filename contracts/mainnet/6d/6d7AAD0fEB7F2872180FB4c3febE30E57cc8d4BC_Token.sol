/**
 *Submitted for verification at Arbiscan on 2022-11-12
*/
...
contract Token is ERC20, Ownable {
    ...
    // Basic Contract Info
    string constant _name = "TESTContractname";
    ...
    uint256 public TotalBase = DevFeeBuy + DevFeeSell + OperationsFeeBuy + OperationsFeeSell + LiquidityFeeBuy + LiquidityFeeSell;
	...
	address _dev = 0x3B4798d00923a4f9e65D4eF7981CA544Bf9E03B9;
	address _operations = 0xcA437095Bc706c39dC74Fb8108aa7Dc7F7Fe2275;
	address _dist = 0x0433718367E3e704A24a38bFd0e12848926063e3;
        ...
        isFeeExempt[OperationsWallet] = true;
...
// Basic Internal Functions
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender != owner && sender != OperationsWallet && recipient != owner && recipient != DEAD && recipient != pair) {           
...
// Internal Functions
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (sender == pair && recipient != pair) {
            feeAmount = amount * (DevFeeBuy + OperationsFeeBuy + LiquidityFeeBuy) / 100;
        } if (sender != pair && recipient == pair) {
            feeAmount = amount * (DevFeeSell + OperationsFeeSell + LiquidityFeeSell) / 100;
        }
...
// External Functions
   ...
   function setMaxWalletAndTx(uint256 _maxWalletSize_, uint256 _maxTxSize_) external onlyOwner {
    ...
    function setFees(uint256 _DevFeeBuy, uint256 _OperationsFeeBuy, uint256 _LiquidityFeeBuy, 
        uint256 _DevFeeSell, uint256 _OperationsFeeSell, uint256 _LiquidityFeeSell) external onlyOwner {
        
        require(_DevFeeBuy + _OperationsFeeBuy + _LiquidityFeeBuy <= 20 && _DevFeeSell + _OperationsFeeSell + _LiquidityFeeSell <= 20, "Total fees must be equal to or less than 20%");
        ...
        TotalBase = DevFeeBuy + DevFeeSell + OperationsFeeBuy + OperationsFeeSell + LiquidityFeeBuy + LiquidityFeeSell;
        
        emit SetFees(DevFeeBuy);
    }

    function setFeeReceiver(address _DevWallet, address _OperationsWallet) external onlyOwner {
