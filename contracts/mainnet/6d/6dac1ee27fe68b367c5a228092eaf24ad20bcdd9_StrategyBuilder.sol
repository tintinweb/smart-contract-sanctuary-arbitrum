...
contract StrategyBuilder is StrategyBlocks {
    ...
    function process(StepCall[] memory steps, address returnPositionsTo) public payable returns (StepResult[] memory results) {
        ...
        _returnPositions(stack.long, stack.short, returnPositionsTo);
    }
    ...
    function _onPositionReceived(address, address from, uint256, bytes calldata data) internal override {
        ...
        (, stack, results) = _actionProcessor(steps, 0, stack, user, results);
        _returnPositions(stack.long, stack.short, user);
    }
    ...
    function _actionProcessor(
        ...
        for (offset_ = offset; offset_ < steps.length; offset_++) {
            ...
            if (step.step == Step.VaultDeposit) {
                ...
                Trade memory trade;
                (positionId, trade) = _positionDeposit(stack_.loadPositionId(positionId, side), amount);
                stack_.storePositionId(positionId, side);
                results[offset_].data = abi.encode(positionId, trade);
            } else if (step.step == Step.PositionBorrow) {
                (PositionId positionId, Side side, uint256 amount) = abi.decode(step.data, (PositionId, Side, uint256));
                Trade memory trade;
                (positionId, trade) = _positionBorrow(stack_.loadPositionId(positionId, side), amount);
                stack_.storePositionId(positionId, side);
                results[offset_].data = abi.encode(positionId, trade);
            } else if (step.step == Step.PositionWithdraw) {
                (PositionId positionId, Side side, uint256 amount) = abi.decode(step.data, (PositionId, Side, uint256));
                Trade memory trade;
                (positionId, trade) = _positionWithdraw(stack_.loadPositionId(positionId, side), amount);
                stack_.storePositionId(positionId, side);
                results[offset_].data = abi.encode(positionId, trade);
            } else if (step.step == Step.PositionRepay) {
                (PositionId positionId, Side side, uint256 amount) = abi.decode(step.data, (PositionId, Side, uint256));
                Trade memory trade;
                (positionId, trade) = _positionRepay(stack_.loadPositionId(positionId, side), amount);
                stack_.storePositionId(positionId, side);
                results[offset_].data = abi.encode(positionId, trade);
            } else if (step.step == Step.PositionClose) {
                (PositionId positionId, Side side) = abi.decode(step.data, (PositionId, Side));
                Trade memory trade;
                (positionId, trade) = _positionClose(stack_.loadPositionId(positionId, side), user);
                stack_.storePositionId(positionId, side);
                results[offset_].data = abi.encode(positionId, trade);
