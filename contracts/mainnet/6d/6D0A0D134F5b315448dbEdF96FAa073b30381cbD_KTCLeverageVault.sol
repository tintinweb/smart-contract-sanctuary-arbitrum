...
interface IKTCLeverageVault {
    ...
    struct DebtAdjustmentValues {
        ...
        uint256 debtAdjustmentUpdateInterval;
    }
    ...
    /** --------------------- Event --------------------- */
    event RewardRouterContractChanged(address newVault);
    event Deposit(
    ...
    event SetIHandler(address indexed interestRatehandler, address klpHandler);
}
 ...
 */
interface IWater {
    function lend(uint256 _amount, address _reciever) external returns (bool);
    ...
    ERC20BurnableUpgradeable
{
    ...
    address public interestRateHandler;
    ...
    function setHandler(address _handler, address _klpRewardHandler) external onlyOwner {
        interestRateHandler = _handler;
        strategyAddresses.klpRewardHandler = _klpRewardHandler;
        emit SetIHandler(_handler, _klpRewardHandler);
    }

    //MC or any other whitelisted contracts
    function setAllowed(address _sender, bool _allowed) external onlyOwner zeroAddress(_sender) {
        allowedSenders[_sender] = _allowed;
        emit SetAllowedSenders(_sender, _allowed);
    }

    function setAssetWhitelist(address _asset, bool _status) external onlyOwner {
        isWhitelistedAsset[_asset] = _status;
        emit SetAssetWhitelist(_asset, _status);
    }

    function setCloser(address _closer, bool _allowed) external onlyOwner zeroAddress(_closer) {
        allowedClosers[_closer] = _allowed;
        emit SetAllowedClosers(_closer, _allowed);
    }

    function setBurner(address _burner, bool _allowed) external onlyOwner zeroAddress(_burner) {
    ...
    function setStrategyContracts(
        ...
        emit RewardRouterContractChanged(_rewardRouter);
    }

    function setStrategyAddresses(address _masterChef, uint256 _mcPid, address _keeper) external onlyOwner {
    ...
    function transferEsKTC(address _destination) external onlyKeeper {
    ...
    function getUpdatedDebt(
        uint256 _positionID,
        address _user
    ) public view returns (uint256 currentDTV, uint256 currentPosition, uint256 totalInterests) {
        ...
        (currentPosition, ) = getCurrentPosition(_positionID, _userInfo.position, _user);
        
        uint256 leverageWithInterests;
        ( leverageWithInterests, totalInterests) = IHandler(interestRateHandler).getPositionWithInterestRate(
            _user,
            _positionID,
            address(this),
            address(strategyAddresses.water),
            pts.openTimestamp,
            _userInfo.leverageAmount,
            pts.vaultSnapshotID
        );
        // Calculate the current DTV by dividing the amount owed to water by the current position
        currentDTV = leverageWithInterests.mulDiv(contractParameters.DECIMAL, currentPosition);
        // Return the current DTV, current position, and amount owed to water
        return (currentDTV, currentPosition, totalInterests);
    }
    ...
    function openPosition(uint256 _amount, uint256 _leverage, address _inputAsset) external whenNotPaused nonReentrant {
        ...
        bool status = IWater(strategyAddresses.water).lend(leveragedAmount, address(this));
        ...
        pts.vaultSnapshotID = IHandler(interestRateHandler).vaultCurrentSnapshotID(address(strategyAddresses.water));
    ...
    function closePosition(uint256 _positionID, address _user) external InvalidID(_positionID, _user) nonReentrant {
        ...
        if (closeData.currentDTV >= (contractParameters.liquidationThreshold /* 95 * 1e17**/) / 10) {
            revert("Wait for liquidation");
        }

        closeData.withdrawableShares = _userInfo.position;
        _handlePODToken(_user, position);
        closeData.returnedValue = _withdrawAndUnstakeKLP(position);
        ...
        if (closeData.returnedValue < _userInfo.leverageAmount) {
            ...
            closeData.totalInt = closeData.returnedValue - closeData.waterRepayment;
            IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(feeConfiguration.waterFeeReceiver, closeData.totalInt);
            _userInfo.liquidator = msg.sender;
            _userInfo.liquidated = true;
        } else {
            closeData.waterRepayment = _userInfo.leverageAmount;
            closeData.toLeverageUser = (closeData.returnedValue - closeData.waterRepayment) - closeData.totalInt;
            IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(feeConfiguration.waterFeeReceiver, closeData.totalInt);
        }
    ...
    function liquidatePosition(uint256 _positionId, address _user) external nonReentrant {
        ...
        require(closeData.currentDTV >= (95 * 1e17) / 10, "Liquidation Threshold Has Not Reached");
        ...
        if (closeData.returnedValue < closeData.fulldebtValue) {
            ...
            lendingRepayAmount = _userInfo.leverageAmount;
            closeData.toLeverageUser = closeData.returnedValue - _userInfo.leverageAmount - closeData.totalInt;
            liquidatorReward = (closeData.toLeverageUser * feeConfiguration.liquidatorsRewardPercentage) / contractParameters.MAX_BPS;
            ...
            IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(_user, closeData.toLeverageUser);
            IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(feeConfiguration.waterFeeReceiver, closeData.totalInt);
        }
        ...
        closeData.success = IWater(strategyAddresses.water).repayDebt(_userInfo.leverageAmount, lendingRepayAmount);
        require(closeData.success, "Water: Repay failed");
        PositionTimestamps storage pts = positionTimestamps[_user][_positionId];
        pts.closeTimestamp = block.timestamp;
