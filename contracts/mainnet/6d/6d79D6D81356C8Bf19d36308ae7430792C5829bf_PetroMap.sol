...
import "../tokens/CrudeOIL.sol";
import "../types/PetroAccessControl.sol";
import "../interfaces/IPetroBank.sol";
import "../interfaces/IStrategy.sol";
import "../interfaces/IPetroRewardManager.sol";
...
contract PetroMap is PetroAccessControl, ERC721EnumerableUpgradeable{
  ...
  function mintPlot(uint256 _plotTypeID) public payable {
    ...
    require(balanceOf(msg.sender) < 5, "Max Supply of Plot reached");
  ...
  function getPlotPrice(uint256 _plotTypeID) public view returns (uint256){
    return plotTypeByID[_plotTypeID].price;
  }
  function getStrategyOfObject(uint256 _objectID) public view returns (address){
    return ObjectTypeByID[_objectID].strategy;
  }
  function getIsObjectNode(uint256 _objectTypeID) public view returns (bool)
  {
    return (ObjectTypeByID[_objectTypeID].strategy == RewardManagerAddress);
  }
  function getBoostOfPlot(uint256 _plotID) public view returns (uint256){
    return plotByID[_plotID].currentBoost;
  }
