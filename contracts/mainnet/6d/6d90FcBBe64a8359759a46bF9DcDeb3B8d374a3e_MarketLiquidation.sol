...
import { 
  ...
  ASSET_FIELD_LIQUIDATION_DISCOUNT 
...
library Constants {
  ...
  // the maximum liquidation discount to allow
  BPS internal constant MAX_LIQUIDATION_DISCOUNT_BPS = BPS.wrap(3000); // 30% maximum discount

  // the threshold in which an account is deemed unhealthy and subject to liquidation
  uint16 internal constant UNHEALTHY_HEALTH_SCORE_THRESHOLD = 99;
}

contract MarketLiquidation is AuthorizedAccessControl, Pausable, ReentrancyGuard, IMarketLiquidation {
  
  BPS private constant FIFTY_PERCENT = BPS.wrap(5000);

  struct LiquidationContext {
    /// @dev the params for the liquidation call.
    LiquidateParams params;
    /// @dev the asset that is being liquidated.
    IAssetStorage.Asset asset;
    /// @dev the total supply that the account has for the asset.
    uint256 totalSupply;
    /// @dev the total liabilities for the account.    
    uint256 liabilities;
    /// @dev the liquidation discount that will be applied.
    BPS liquidationDiscount;
    /// @dev the price of the asset.
    uint256 price;
    /// @dev the price discount according to the asset configuration.
    uint256 priceDiscount;
    /// @dev the amount being repaid on the liabilities.
    uint256 repayAmount;
    /// @dev the amount being repaid on the principal portion of the liabilities.
    uint256 repayPrincipalAmount;
    /// @dev the amount being repaid on the interest portion of the liabilities.
    uint256 repayInterestAmount;
    /// @dev the amount that should be refunded to the account in the case where a small account was liquidated.
    uint256 refundAmount;
    /// @dev the total supply that is being liquidated, calculated based on the discounted price.
    uint256 liquidatedSupply;   
    /// @dev the maxmimum decimals that should be used for normalizing amounts.
    uint8 maxDecimals;
    /// @dev the asset decimals.
    uint8 assetDecimals; 
    /// @dev the decimals for the liabilties denomination.
    uint8 liabilitiesDecimals;
  }
  ...
  uint256 private _smallAccountThreshold;

  constructor(IAddressRegistry registry) AdminAccessControl(registry) {
    _registry = registry;
  }
  ...
  /// @inheritdoc IMarketLiquidation
  function setSmallAccountThreshold(uint256 threshold) external onlyAdmin {
    if (threshold != _smallAccountThreshold) {
      _smallAccountThreshold = threshold;

      emit SetSmallAccountThreshold(threshold);
    }
  }

  /// @inheritdoc IMarketLiquidation
  function getSmallAccountThreshold() external view returns (uint256) {
    return _smallAccountThreshold;
  }

  /// @inheritdoc IMarketLiquidation
  function forceLiquidate(LiquidateParams memory params) 
    external 
      nonReentrant
      onlyAdmin 
      requireNonZeroAddress(params.account) 
  returns (address, uint256, uint256, uint256, uint256) {
    LiquidationContext memory context = beforeLiquidate(params);

    return liquidate(context);
  }

  /// @inheritdoc IMarketLiquidation
  function liquidate(LiquidateParams memory params) 
    external 
      nonReentrant
      onlyAuthorized 
      whenNotPaused 
  ...
  returns (address, uint256, uint256, uint256, uint256) {
    ...
    if (context.liabilities == 0) {
      revert Errors.MarketLiquidation_AccountNotLiquidatable(params.account);
    }

    if (isLiquidatable(context.params.account, context.liabilities) == false) {
      revert Errors.MarketLiquidation_AccountNotLiquidatable(context.params.account);
    }
  ...
  function isLiquidatable(address account) public view returns (bool) {
    uint256 liabilities = _registry.getMarket().getLiabilities(account);

    return isLiquidatable(account, liabilities);
  }

  function isLiquidatable(address account, uint256 liabilities) private view returns (bool) {
    // calculate the borrow limit using custom logic to take into account Oracle status
    (uint256 borrowLimit, bool isInvalid) = getBorrowLimit(account);

    uint256 health = borrowLimit * 100 / liabilities; 

    return isInvalid == false && health < Constants.UNHEALTHY_HEALTH_SCORE_THRESHOLD;
  }

  function getBorrowLimit(address account) private view returns (uint256 borrowLimit, bool isInvalid) {
    uint8 decimals = _registry.getDepositorVault().getDecimals();

    (IAssetStorage.Asset[] memory assets, IPortfolioStorage.PortfolioAsset[] memory portfolioAssets) = getPortfolioAssets(account);

    for (uint256 i = 0; i < portfolioAssets.length;) {
      (uint256 total, IPriceOracle.Confidence confidence) = getPortfolioAssetLimit(assets[i], portfolioAssets[i]);

      if (confidence == IPriceOracle.Confidence.OFFLINE && isOracleInvalid(assets[i])) {
        // the oracle is in an invalid state so we cant allow a liquidation
        isInvalid = true;
        break;
      }

      borrowLimit += total.normalize(USDMath.DECIMALS, decimals);

      unchecked { i++; }
    }
  }

  /// @dev if the Oracle is offline then we dont allow for liquidations until a threshold has passed
  /// then we can use a stale price and leave it to the liquidator if they want to liquidate
  function isOracleInvalid(IAssetStorage.Asset memory asset) private view returns (bool invalid) {
    (, uint256 changedAt) = IPriceOracle(asset.priceOracle).getStatus();

    invalid = block.timestamp - changedAt < 7200;
  }

  function getPortfolioAssets(address account) private view returns (IAssetStorage.Asset[] memory assets, IPortfolioStorage.PortfolioAsset[] memory portfolioAssets) {
    portfolioAssets = _registry.getPortfolioStorage().getPortfolioAssets(account, PORTFOLIO_ASSET_FIELD_SHARES);

    assets = _registry.getAssetStorage().getAssets(
      portfolioAssets.getTokenAddresses(), 
      ASSET_FIELD_CUSTODIAN | ASSET_FIELD_PRICE_ORACLE | ASSET_FIELD_MAX_LTV);
  }

  function getPortfolioAssetLimit(IAssetStorage.Asset memory asset, IPortfolioStorage.PortfolioAsset memory portfolioAsset) private view returns (uint256, IPriceOracle.Confidence) {
    require(portfolioAsset.token == asset.token, "token mismatch");
    require(asset.custodian != address(0), "no custodian");

    (USD price, IPriceOracle.Confidence confidence) = IPriceOracle(asset.priceOracle).getLatestPrice();

    ICustodian custodian = ICustodian(asset.custodian);
      
    uint256 amount = custodian.toAssets(portfolioAsset.shares);
    
    uint256 total = amount.mulDiv(USD.unwrap(price), 10 ** custodian.getDecimals());

    return (asset.maxLTV.percentOf(total), confidence);
  }

  function beforeLiquidate(LiquidateParams memory params) private view returns (LiquidationContext memory context) {
    context.params = params;
    
    IAssetStorage assetStorage = _registry.getAssetStorage();
    context.asset = assetStorage.getAsset(
      params.asset, 
      ASSET_FIELD_CUSTODIAN | ASSET_FIELD_PRICE_ORACLE | ASSET_FIELD_LIQUIDATION_DISCOUNT | ASSET_FIELD_MAX_LTV);
    
    if (BPS.unwrap(context.asset.maxLTV) == 0) {
      revert Errors.Validation_TokenNotAllowed(params.asset);
    }

    context.liquidationDiscount = PercentageMath.min(context.asset.liquidationDiscount, params.discount);

    IMarket market = _registry.getMarket();
    context.liabilities = market.getLiabilities(params.account);
  }

  function liquidate(LiquidationContext memory context) private returns (address, uint256, uint256, uint256, uint256) {
    require(context.asset.custodian != address(0), "no custodian");
    require(context.asset.priceOracle != address(0), "no price oracle");

    fetchDecimals(context);

    fetchPrice(context);

    fetchTotalSupply(context);

    context.params.maxAmount > 0 ? liquidateMaxAmount(context) : liquidateTotalSupply(context);

    repayLiabilities(context);

    transferCollateral(context);

    refundExcess(context);

    emit Liquidate({
      account: context.params.account,
      liquidator: msg.sender,
      token: context.asset.token,
      price: context.price,
      repayAmount: context.repayAmount,
      refundAmount: context.refundAmount,
      liquidatedSupply: context.liquidatedSupply
    });

    return (context.asset.token, context.liquidatedSupply, context.repayPrincipalAmount, context.repayInterestAmount, context.refundAmount);
  }

  function liquidateMaxAmount(LiquidationContext memory context) private view {
    // dont allow more than 50% of the liabilities to be repaid unless it is a small account
    context.repayAmount = context.liabilities <= _smallAccountThreshold 
      ? Math.min(context.params.maxAmount, _smallAccountThreshold)
      : Math.min(context.params.maxAmount, FIFTY_PERCENT.percentOf(context.liabilities));

    calculateLiquidatedSupply(context);
  }

  /// @dev calculate the total supply that can be liquidated for the asset.
  function calculateLiquidatedSupply(LiquidationContext memory context) private pure {
    // use the discounted price here    
    uint256 price = context.price - context.priceDiscount;

    // we are working with three different values all of which can have different 
    // precision so we need to normalize the values to the same precision 
    uint256 repayAmount = context.repayAmount.normalize(context.liabilitiesDecimals, context.maxDecimals);
    uint256 totalSupply = context.totalSupply.normalize(context.assetDecimals, context.maxDecimals);   
    price = price.normalize(USDMath.DECIMALS, context.maxDecimals);

    uint256 scalar = 10 ** context.maxDecimals;

    // truncate the repayAmount to ensure that the bonus doesnt exceed the total supply
    repayAmount = Math.min(repayAmount, totalSupply.mulDiv(price, scalar, Math.Rounding.Floor));

    uint256 liquidatedSupply = repayAmount.mulDiv(scalar, price);

    context.liquidatedSupply = liquidatedSupply.normalize(context.maxDecimals, context.assetDecimals);
    context.repayAmount = repayAmount.normalize(context.maxDecimals, context.liabilitiesDecimals);
  }

  function liquidateTotalSupply(LiquidationContext memory context) private view {
    calculateRepayAmount(context);

    if (context.liabilities > _smallAccountThreshold && context.repayAmount > FIFTY_PERCENT.percentOf(context.liabilities)) {
      // it is up to the caller to ensure that they arent trying to liquidate more than allowed
      revert Errors.MarketLiquidation_LiquidationAmountExceedsMaxAmount(
        context.repayAmount, 
        FIFTY_PERCENT.percentOf(context.liabilities));
    }
  }

  /// @dev calculate the repayment amount based on the supply that is to be liquidated.
  function calculateRepayAmount(LiquidationContext memory context) private pure {
    // calcuate the repayment amount based on the supply that is to be liquidated
    uint256 price = context.price - context.priceDiscount;
    context.liquidatedSupply = Math.min(context.totalSupply, context.params.totalSupply);

    // normalize the amounts for comparisson
    price = price.normalize(USDMath.DECIMALS, context.maxDecimals);
    uint256 liquidatedSupply = context.liquidatedSupply.normalize(context.assetDecimals, context.maxDecimals);

    context.repayAmount = liquidatedSupply.mulDiv(price, 10 ** context.maxDecimals).normalize(context.maxDecimals, context.liabilitiesDecimals);
  }

  /// @dev repay the liabilities from the liquidator.
  function repayLiabilities(LiquidationContext memory context) private {
    // transfer the amount from the caller so the current contract can repay the liabilities
    IERC20Metadata baseAsset = _registry.getDepositorVault().getUnderlyingAsset();
    baseAsset.safeTransferFrom(msg.sender, address(this), context.repayAmount);

    // repay the amount from the caller on behalf of the account
    IMarket market = _registry.getMarket();
    baseAsset.forceApprove(address(market), context.repayAmount);
    
    (context.repayPrincipalAmount, context.repayInterestAmount) = market.repay(context.params.account, context.repayAmount, address(this));
  }

  /// @dev withdraw and transfer the collateral to the liquidator.
  function transferCollateral(LiquidationContext memory context) private {
    IPortfolio portfolio = _registry.getPortfolio();
    portfolio.withdraw(context.params.account, msg.sender, context.asset.token, context.liquidatedSupply);
  }  

  /// @dev there is a case with small accounts whereby the liquidated amount can exceed liabilities
  /// so in these cases we only need to repay the actual liabilities and refund the difference.
  function refundExcess(LiquidationContext memory context) private {
    if (context.repayAmount > context.liabilities) {
      context.refundAmount = context.repayAmount - context.liabilities;

      IERC20Metadata baseAsset = _registry.getDepositorVault().getUnderlyingAsset();
      baseAsset.safeTransfer(context.params.account, context.refundAmount);
    }
  }

  function fetchTotalSupply(LiquidationContext memory context) private view  {
    try _registry.getPortfolioStorage().getPortfolioAsset(context.params.account, context.params.asset, PORTFOLIO_ASSET_FIELD_SHARES) returns (IPortfolioStorage.PortfolioAsset memory portfolioAsset) {
      if (portfolioAsset.shares == 0) {
        revert Errors.MarketLiquidation_NoTotalSupply(context.params.account, context.params.asset);
      }
      context.totalSupply = ICustodian(context.asset.custodian).toAssets(portfolioAsset.shares);
    } catch  {
      revert Errors.MarketLiquidation_NoTotalSupply(context.params.account, context.params.asset);
    }
  }

  function fetchDecimals(LiquidationContext memory context) private view {
    context.assetDecimals = ICustodian(context.asset.custodian).getDecimals();
    
    context.liabilitiesDecimals = _registry.getDepositorVault().getDecimals();

    context.maxDecimals = uint8(Math.max(
      Math.max(
        USDMath.DECIMALS,
        context.liabilitiesDecimals
      ),
      context.assetDecimals
    ));
  }

  function fetchPrice(LiquidationContext memory context) private view {
    (USD price, ) = IPriceOracle(context.asset.priceOracle).getLatestPrice();
    context.price = USD.unwrap(price);

    // each collateral has a liquidation discount that is to be applied to the 
    // oracle price when calculating the total supply that is to be claimed as a bonus
    BPS liquidationDiscount = PercentageMath.min(Constants.MAX_LIQUIDATION_DISCOUNT_BPS, context.liquidationDiscount);
    context.priceDiscount = liquidationDiscount.percentOf(context.price);
  }

  /// @inheritdoc IMarketLiquidation
  function findMostFavorablePosition(address account) public view returns (address, uint256) {
    FindParams memory params;
    params.account = account;

    findMostFavorable(params);

    return (params.asset.token, params.maxAmount);
  }
  ...
  function findMostFavorable(FindParams memory params) private view {
    IPortfolioStorage portfolioStorage = _registry.getPortfolioStorage();
    IPortfolioStorage.PortfolioAsset[] memory portfolioAssets = portfolioStorage.getPortfolioAssets(params.account, PORTFOLIO_ASSET_FIELD_SHARES);

    IAssetStorage assetStorage = _registry.getAssetStorage();
    IAssetStorage.Asset[] memory assets = assetStorage.getAssets(
      portfolioAssets.getTokenAddresses(), 
      ASSET_FIELD_CUSTODIAN | ASSET_FIELD_PRICE_ORACLE | ASSET_FIELD_LIQUIDATION_DISCOUNT | ASSET_FIELD_MAX_LTV);

    // find the index of the first asset that has a maxLTV > 0
    uint256 j;
    uint256 previousTotal;
    uint256 previous;
    for (uint256 i; i < portfolioAssets.length;) {
      require(portfolioAssets[i].token == assets[i].token, "token mismatch");
      require(assets[i].custodian != address(0), "no custodian");

      // can only compare the asset if it has an LTV
      if (BPS.unwrap(assets[i].maxLTV) > 0) {
        uint256 currentTotal = calculateTotal(assets[i], portfolioAssets[i].shares);
        
        uint256 current = currentTotal - assets[i].maxLTV.percentOf(currentTotal);
    ...
    require(j > 0, "could not find asset");

    // normalize the total to the same denomination as the pool
    params.maxAmount = USD.wrap(previousTotal).normalize(_registry.getDepositorVault().getDecimals());
    params.asset = assets[j - 1];
  }

  function calculateTotal(IAssetStorage.Asset memory asset, uint256 shares) private view returns (uint256) {
    ICustodian custodian = ICustodian(asset.custodian);
    uint256 amount = custodian.toAssets(shares);

    (USD price, ) = IPriceOracle(asset.priceOracle).getLatestPrice();
      
    return amount.mulDiv(USD.unwrap(price), 10 ** custodian.getDecimals());
  }  
}
