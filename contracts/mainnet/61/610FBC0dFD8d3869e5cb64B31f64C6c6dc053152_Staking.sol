...
/// @title Bonus Block Staking
/// @author Trivium (modified Oiler Staking Contract from oiler.network)
...
contract Staking {
   ...
   */
    ...
    mapping(address => mapping(uint16 => Stake)) public stakes;

    /// @notice Number of total stakes for each user
    mapping (address => uint16) public stakeCount;

    /// @notice Number of active stakes for each user
    mapping (address => uint16) public activeStakeCount;
    ...
    /// @notice Total token amount currently staked by all users
    ...
    /// @notice token that will be given as a reward
    ...
    constructor(address oilerToken_, uint256 stakingDurationInBlocks_, uint256 stakingFundAmount_, uint256 vestingDuration_, address owner_) {
        ...
        rewardToken = IERC20(oilerToken_);
     ...
     */
    function lockTokens(uint72 tokenAmount_, uint24 lockingPeriodInBlocks) public poolTokenSet {
        ...
        require(block.number <= stakingProgramEndsBlock - lockingPeriodInBlocks, "Your lock period exceeds Staking Program duration");

        // Increment user's total and active stake counts. Total stake count will be used as Stake ID
        stakeCount[msg.sender] = stakeCount[msg.sender] + 1;
        activeStakeCount[msg.sender] = activeStakeCount[msg.sender] + 1;

        // This is a locking reward - will be earned only after the full lock period is over - otherwise not applicable
        uint128 expectedStakingRewardPoints = calculateStakingRewardPoints(tokenAmount_, lockingPeriodInBlocks);

        Stake memory stake = Stake(tokenAmount_, lockingPeriodInBlocks, uint32(block.number), expectedStakingRewardPoints);
        stakes[msg.sender][stakeCount[msg.sender]] = stake;
        ...
        emit StakeLocked(msg.sender, tokenAmount_, lockingPeriodInBlocks, expectedStakingRewardPoints, stakeCount[msg.sender]);
    }
     ...
     */
    function unlockTokens(uint16 stakeId) public poolTokenSet {
        ...
        // Remove unlock amount from total staked amount
        totalStaked -= stake.tokenAmount;

        // Zero the Stake - to protect from double-unlocking
        delete stakes[msg.sender][stakeId];

        // Decrement User's active stake count
        activeStakeCount[msg.sender] = activeStakeCount[msg.sender] - 1;
    ...
    function getRewards() public {
        ...
        // require(stakes[msg.sender][stakeId].tokenAmount == 0, "You still have a stake locked - please unlock first, don't leave free money here");
    ...
    function release() public {
        ...
        require(rewardToken.transfer(msg.sender, releasable), "Reward oilers transfer failed");
    ...
    event StakeLocked(address recipient, uint256 tokenAmount, uint256 lockingPeriodInBlocks, uint256 expectedStakingRewardPoints, uint16 stakeId);
