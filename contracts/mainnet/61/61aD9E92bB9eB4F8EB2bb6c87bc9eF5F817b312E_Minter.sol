...
contract Minter {
    ...
    uint public constant LM_TARGET = 36900 * 27e24 / 1000;
    ...
    function setEmissions(uint _decay, uint _boost) public onlyAdmin {
        ...
        require(_decay <= 9990, "decay must be >= 0.1%");
        require(_decay >= 9500, "decay must <= than 5%");
        require(_boost <= boost + 1500 * LM_TARGET / 10000, "boost must be <= old boost + 0.15% of LM target");
        decay = _decay;
        boost = _boost;
        last_epoch = block.timestamp;
    }
    ...
    function circulating_emission() public view returns (uint) {
        return circulating_supply() * tail_emission / tail_base + boost;
    }
