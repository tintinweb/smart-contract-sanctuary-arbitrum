...
import "../market/MarketUtils.sol";
import "../market/Market.sol";
import "../reader/ReaderPricingUtils.sol";
...
contract ReaderLite {
        ...
        returns (uint256 collateralAmount, uint256 sizeInUsd, uint256 totalCostAmount, int256 pnlAfterPriceImpactUsd)
    {
        ...
        PositionPricingUtils.PositionFees memory fees = getPositionFees(cache, getPositionFeesParams, prices);
    ...
    function getPositionFees(
        ...
        PositionPricingUtils.PositionFees memory fees = PositionPricingUtils.getPositionFeesAfterReferral(
            params.dataStore,
            params.referralStorage,
            params.collateralTokenPrice,
            params.forPositiveImpact,
        ...
        MarketUtils.GetNextFundingAmountPerSizeResult memory nextFundingAmountResult = MarketUtils
            .getNextFundingAmountPerSize(params.dataStore, cache.market, prices);
        ...
        if (params.position.isLong()) {
            fees.funding.latestLongTokenClaimableFundingAmountPerSize +=
                nextFundingAmountResult.claimableFundingAmountPerSizeDelta.long.longToken *
                multiplier;
            fees.funding.latestShortTokenClaimableFundingAmountPerSize +=
                nextFundingAmountResult.claimableFundingAmountPerSizeDelta.long.shortToken *
                multiplier;

            if (params.position.collateralToken() == cache.market.longToken) {
                fees.funding.latestFundingFeeAmountPerSize +=
                    nextFundingAmountResult.fundingFeeAmountPerSizeDelta.long.longToken *
                    multiplier;
            } else {
                fees.funding.latestFundingFeeAmountPerSize +=
                    nextFundingAmountResult.fundingFeeAmountPerSizeDelta.long.shortToken *
                    multiplier;
            }
        } else {
            fees.funding.latestLongTokenClaimableFundingAmountPerSize +=
                nextFundingAmountResult.claimableFundingAmountPerSizeDelta.short.longToken *
                multiplier;
            fees.funding.latestShortTokenClaimableFundingAmountPerSize +=
                nextFundingAmountResult.claimableFundingAmountPerSizeDelta.short.shortToken *
                multiplier;

            if (params.position.collateralToken() == cache.market.longToken) {
                fees.funding.latestFundingFeeAmountPerSize +=
                    nextFundingAmountResult.fundingFeeAmountPerSizeDelta.short.longToken *
                    multiplier;
            } else {
                fees.funding.latestFundingFeeAmountPerSize +=
                    nextFundingAmountResult.fundingFeeAmountPerSizeDelta.short.shortToken *
                    multiplier;
            }
        }
