...
interface IDopexPositionManager {
  ...
  function withdrawAllFundsToUser(address _collateralToken, address _indexToken) external;
  ...
  event IncreaseOrderCreated(address[] _path, address _indexToken, uint256 _collateralDelta, uint256 _sizeDelta, uint256 _acceptablePrice);

  event DecreaseOrderCreated(address[] _path, address _indexToken, uint256 _collateralDelta, uint256 _sizeDelta, uint256 _acceptablePrice);
    ...
    ReentrancyGuard
{
    ...
    event EmergencyStrategyExit(uint256 _positionId, address _caller);
        ...
        nonReentrant
    {
        ...
        _safeApprove(indexToken, atlanticPool, unwindFees + amount);
    ...
    function emergencyStrategyExit(uint256 _positionId) external {
        ...
        emit EmergencyStrategyExit(_positionId, msg.sender);
    }
    ...
    function _exitStrategyKeepLongPosition(uint256 _positionId) private {
        ...
        _safeApprove(indexToken, atlanticPool, unwindAmount);
        IAtlanticPutsPool(atlanticPool).unwind(purchaseId);
    ...
    function _enableStrategy(uint256 _positionId) private {
        ...
        _safeTransferFrom(collateralToken, user, address(this), premiumFees);
        _safeApprove(collateralToken, atlanticPool, premiumFees);
