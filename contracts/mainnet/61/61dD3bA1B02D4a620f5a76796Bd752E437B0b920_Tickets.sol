  ...
  UUPSUpgradeable
{
  ...
  /**
   * @dev admin increases tickets supply.
   *
   * See {ERC20-_mint}.
   *
   * Requirements:
   ...
   * - the caller must have the `DEFAULT_ADMIN_ROLE`.
   */
  function increaseSupply(uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) {
    super._mint(escrowAddress, amount);
  }
  ...
  function _mint(address account, uint256 amount) internal override {
    if (balanceOf(escrowAddress) < amount) {
