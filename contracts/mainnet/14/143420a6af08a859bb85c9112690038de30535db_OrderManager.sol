...
contract OrderManager is AdminAbstract, Initializable, IOrderManager {
    address public orderBookImplAddress;
    ...
    function initialize(address _adminStructureAddress) external initializer {
        _setAdminStructure(_adminStructureAddress);
    }

    function setOrderBookImplementation(address _orderBookImplAddress) external onlyAdmin {
        if (_orderBookImplAddress == address(0)) revert Errors.WrongAddress();

        orderBookImplAddress = _orderBookImplAddress;

        emit OrderBookImplementationSet(_orderBookImplAddress);
    }

    function getOrderBookImplementation() external view override returns (address) {
        return orderBookImplAddress;
    }
}
