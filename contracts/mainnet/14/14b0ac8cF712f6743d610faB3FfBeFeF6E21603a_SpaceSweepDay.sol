...
error SpaceSweepDay__NotEnoughEth();
	...
	ReentrancyGuard
{
	...
	/* events */
	event SpaceSweepDayEnter(address indexed player, uint256 entryFee, uint256 entries);
	event RequestedSpaceSweepDay(uint256 indexed requestId);
	event WinnerPicked(
	...
	function enterSpaceSweepDaySpecial() public payable {
		...
		emit SpaceSweepDayEnter(msg.sender, msg.value, 1);
	}
	...
	function enterSpaceSweepDay(uint256 entries) public payable {
		...
		require(msg.value >= totalEntryFee, "SpaceSweepDay__NotEnoughEth");
		...
		emit SpaceSweepDayEnter(msg.sender, msg.value, entries);
	}
	 ...
	 * and it kicks off a Chainlink VRF call to get a randomWord.
	 */
	function performUpkeep(bytes calldata /* performData */) external override {
		...
		emit RequestedSpaceSweepDay(requestId);
	}
