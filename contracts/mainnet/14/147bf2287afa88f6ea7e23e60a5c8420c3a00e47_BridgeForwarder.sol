...
contract BridgeForwarder {
    ...
    function doOceanInteraction(address inputToken, address outputToken, bytes32 outputMetadata, address _receiver, bytes32 _minOutputAmount, address[] calldata _primitives, uint256[] calldata _oceanIds) external payable {
        ...
        if (inputToken == address(0)) {
            ...
            IERC20Metadata(inputToken).approve(ocean, convertedAmount);
        ...
        uint256 balanceBeforeInteraction =
            outputToken == address(0) ? address(this).balance : outputMetadata != bytes32(0) ? IERC1155(outputToken).balanceOf(address(this), uint256(outputMetadata)) : IERC20Metadata(outputToken).balanceOf(address(this));
        ...
        try IOceanInteractions(ocean).doMultipleInteractions{ value: msg.value }(interactions, _oceanIds) returns (uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory) {
            if (outputToken == address(0)) _handleEthTransfer(_receiver, address(this).balance - balanceBeforeInteraction);
            else if (outputMetadata != bytes32(0)) _handleERC1155Transfer(outputToken, _receiver, IERC1155(outputToken).balanceOf(address(this), uint256(outputMetadata)) - balanceBeforeInteraction, uint256(outputMetadata));
            else _handleERC20Transfer(outputToken, _receiver, IERC20Metadata(outputToken).balanceOf(address(this)) - balanceBeforeInteraction);
        } catch {
            // send funds to original owner in case of failure
            if (inputToken == address(0)) _handleEthTransfer(_receiver, msg.value);
            else _handleERC20Transfer(inputToken, _receiver, allowanceAmount);
        }
    }
    ...
    function _handleERC1155Transfer(address _token, address _receiver, uint256 _amount, uint256 _id) internal {
        try IERC1155(_token).safeTransferFrom(address(this), _receiver, _id, _amount, "") {} catch { revert(); }
    }
