...
import {IImplementationResolver} from "src/interfaces/IImplementationResolver.sol";
import {SmartAccountOwnerResolver} from "src/SmartAccountOwnerResolver.sol";
import {
    AddressNotContract,
    SenderNotSmartAccountOwnerOrMSAItself,
    WrongImplementation,
    ZeroAddress
} from "src/Errors.sol";

/// @author MassMoney
/// @notice Return the implementation used for Mass Smart Accounts.
///         Users can opt in or opt out from upgrades and revert to a specific
///         historical implementation.
/// @dev Update of implementation can only be done by the owner.
contract ImplementationResolver is Ownable2Step, IImplementationResolver {
    /* -------------------------------------------------------------------------- */
    /*                                 IMMUTABLE                                  */
    /* -------------------------------------------------------------------------- */

    /// @notice Smart account owner resolver contract address.
    ...
    /// @notice Latest Mass Smart Account implementation address.
    address public latestImplementation;

    /// @notice Mapping of historical implementations addresses to bool.
    ///         True if the implementation address is a historical one.
    mapping(address => bool) public historicalMSAImplementations;

    /// @notice Mapping of mass smart account to its chosen implementation address.
    ///         Owner of the mass smart account can choose an implementation address
    ///         for his mass smart account. This implementation address must be
    ///         a historical one.
    mapping(address => address) public msaImplementationOf;

    /* -------------------------------------------------------------------------- */
    /*                                   EVENTS                                   */
    /* -------------------------------------------------------------------------- */

    /// @dev Emitted when an implementation address is updated.
    /// @param implementation Address of the new implementation.
    event ImplementationUpdated(address indexed implementation);

    /// @dev Emitted when a mass smart account opt in for upgrades.
    /// @param massSmartAccount Mass Smart Account address.
    event OptInUpgrades(address indexed massSmartAccount);

    /// @dev Emitted when an owner set a specific implementation address for
    ///      its mass smart account and opt out from upgrades.
    /// @param massSmartAccount Mass Smart Account address.
    /// @param implementation Implementation address.
    event OptOutUpgrades(address indexed massSmartAccount, address indexed implementation);

    /* -------------------------------------------------------------------------- */
    /*                                 CONSTRUCTOR                                */
    /* -------------------------------------------------------------------------- */

    constructor(address owner, address _smartAccountOwnerResolver, address _firstImplementation)
        Ownable2Step()
    {
        ...
        if (_firstImplementation.code.length == 0) {
            revert AddressNotContract(_firstImplementation);
        }
        _transferOwnership(owner);
        smartAccountOwnerResolver = SmartAccountOwnerResolver(_smartAccountOwnerResolver);
        latestImplementation = _firstImplementation;
        historicalMSAImplementations[_firstImplementation] = true;
    }
    ...
    /// @dev Modifier to check if the sender is the owner of the mass smart account
    ///      or the mass smart account itself.
    modifier onlyMSAOwnerOrMSAItself(address massSmartAccount) {
        if (
            !(
                smartAccountOwnerResolver.ownerOf(massSmartAccount) == msg.sender
                    || massSmartAccount == msg.sender
            )
        ) {
            revert SenderNotSmartAccountOwnerOrMSAItself(msg.sender);
        }
        _;
    }
    ...
    /// @notice Update the latest implementation address for Mass Smart Accounts
    /// @dev This function can only be called by the owner of this contract.
    ///      The implementation address can be a historical one in case of rollback.
    /// @param newImplementation Address of the implementation.
    function updateImplementation(address newImplementation) external onlyOwner {
        if (newImplementation.code.length == 0) revert AddressNotContract(newImplementation);
        historicalMSAImplementations[newImplementation] = true;
    ...
    /// @notice Opt in upgrades for a mass smart account.
    /// @param massSmartAccount Mass Smart Account address.
    function optInUpgrades(address massSmartAccount)
        external
        override
        onlyMSAOwnerOrMSAItself(massSmartAccount)
    {
        delete msaImplementationOf[massSmartAccount];
        emit OptInUpgrades(massSmartAccount);
    }

    /// @notice Opt out from upgrades for a mass smart account by setting a
    ///         specific implementation.
    /// @dev The implementation address must be a historical one. If address(0) is
    ///      passed as implementation address, the latest implementation address
    ///      will be used.
    /// @param massSmartAccount Mass Smart Account address.
    /// @param implementation Implementation address.
    function optOutUpgrades(address massSmartAccount, address implementation)
        external
        override
        onlyMSAOwnerOrMSAItself(massSmartAccount)
    {
        ...
        if (!historicalMSAImplementations[implementation]) {
            revert WrongImplementation(implementation);
        }
        msaImplementationOf[massSmartAccount] = implementation;
        emit OptOutUpgrades(massSmartAccount, implementation);
    }
    ...
    /// @notice Get implementation for msg.sender.
    ///         msg.sender should be a mass smart account address.
    ///         This function is called from Proxy.huff.
    /// @return implementation address.
    function getImplementation() external view override returns (address) {
        // if the mass smart account has an implementation address set by its owner, return it
        address implementation = msaImplementationOf[msg.sender];
        if (implementation != address(0)) {
            return implementation;
        }
        // else return the latest implementation
...
/// @author MassMoney
/// @notice Interface for the ImplementationResolver contract.
interface IImplementationResolver {
    /// @notice Smart account owner resolver contract address.
    function smartAccountOwnerResolver() external view returns (SmartAccountOwnerResolver);
    ...
    /// @notice Mapping of historical implementations addresses to bool.
    ///         True if the implementation address is a historical one.
    function historicalMSAImplementations(address) external view returns (bool);

    /// @notice Mapping of mass smart account to its chosen implementation address.
    ///         Owner of the mass smart account can choose an implementation address
    ///         for his mass smart account. This implementation address must be
    ///         a historical one.
    function msaImplementationOf(address) external view returns (address);

    /// @notice Opt in upgrades for a mass smart account.
    /// @param massSmartAccount Mass Smart Account address.
    function optInUpgrades(address massSmartAccount) external;
    ...
    /// @param massSmartAccount Mass Smart Account address.
    /// @param implementation Implementation address.
    function optOutUpgrades(address massSmartAccount, address implementation) external;
