...
/**
 * @title ICovenantHelper
 * @author Covenant Labs
 * @notice Defines the user interface for the CovenantHelper function
 **/
interface ICovenantHelper {
    /**
     ...
     * @return userAccountData User variables as per IGuild.UserAccountDataStruct structure with the following parameters
    ...
    function quoteUserAccountData(address guild, address user)
        external
        returns (IGuild.UserAccountDataStruct memory userAccountData);
...
/**
 * @title GuildDataProvider
 * @author Covenant Labs
 * @notice Peripheral contract to collect and pre-process information from the Guild.
 */
contract GuildDataProvider {
    ...
    constructor(IGuildAddressesProvider addressesProvider) {
        ADDRESSES_PROVIDER = addressesProvider;
    }

    /**
     * @notice Returns the configuration data of the collateral
     * @param asset The address of the underlying collateral
     * @return decimals The number of decimals of the collateral
     * @return ltv The ltv of the collateral
     * @return liquidationThreshold The liquidationThreshold of the collateral
     * @return liquidationBonus The liquidationBonus of the collateral
     * @return isActive True if it is active, false otherwise
     * @return isFrozen True if it is frozen, false otherwise
     * @return isPaused True if it is paused, false otherwise
     * @return isNonMTM True if collateral is nonMarkToMarket liquidation based, false otherwise
     **/
    ...
    {
        DataTypes.CollateralConfigurationMap memory configuration = _getCollateralConfiguration(asset);

        (ltv, liquidationThreshold, liquidationBonus, decimals, isNonMTM) = configuration.getParams();

        (isActive, isFrozen, isPaused) = configuration.getFlags();
    }

    /**
     * @notice Returns the active flag for the collateral asset
     * @param asset The address of the underlying collateral
     * @return isActive flag
     **/
    function getCollateralActive(address asset) external view returns (bool isActive) {
        isActive = _getCollateralConfiguration(asset).getActive();
    }

    /**
     * @notice Returns the frozen flag for the collateral asset
     * @param asset The address of the underlying collateral
     * @return isFrozen flag
     **/
    function getCollateralFrozen(address asset) external view returns (bool isFrozen) {
        isFrozen = _getCollateralConfiguration(asset).getFrozen();
    }

    /**
     * @notice Returns the pasued flag for the collateral asset
     * @param asset The address of the underlying collateral
     * @return isPaused flag
     **/
    function getCollateralPaused(address asset) external view returns (bool isPaused) {
        isPaused = _getCollateralConfiguration(asset).getPaused();
    }

    /**
     * @notice Returns the caps parameters of the collateral
     * @param asset The address of the underlying collateral
     * @return supplyCap The supply cap of the collateral
     * @return userSupplyCap The user supply cap of the collateral
     **/
    function getSupplyCaps(address asset) external view returns (uint256 supplyCap, uint256 userSupplyCap) {
        (supplyCap, userSupplyCap) = _getCollateralConfiguration(asset).getCaps();
    }

    /**
     * @notice Returns the configuration for the collateral asset
     * @param asset The address of the underlying collateral
     * @return configuration for collateral asset
     **/
    function _getCollateralConfiguration(address asset)
        internal
        view
        returns (DataTypes.CollateralConfigurationMap memory configuration)
    {
        configuration = IGuild(ADDRESSES_PROVIDER.getGuild()).getCollateralConfiguration(asset);
    }

    /**
     * @notice Returns the protocol fee config
     * @return protocolServiceFee The protocol service fee
     * @return protocolDistributionFee The protocol distribution fee
     * @return protocolMintFee The protocol mint fee
     * @return protocolSwapFee The protocol swap fee
     * @return protocolServiceFeeAddress The protocol service fee address
     * @return protocolDistributionFeeAddress The protocol distribution fee address
     * @return protocolMintFeeAddress The protocol mint fee address
     * @return protocolSwapFeeAddress The protocol swap fee address
     **/
    function getProtocolFeeConfig()
        ...
        returns (
            ...
            uint256 protocolDistributionFee,
            uint256 protocolMintFee,
            uint256 protocolSwapFee,
            address protocolServiceFeeAddress,
            address protocolDistributionFeeAddress,
            address protocolMintFeeAddress,
            address protocolSwapFeeAddress
        )
    {
        DataTypes.PerpetualDebtData memory perpDebt = _getPerpetualDebt();
        DataTypes.PerpDebtConfigurationMap memory perpDebtConfig = _getPerpetualDebtConfig();

        protocolServiceFee = perpDebtConfig.getProtocolServiceFee();
        protocolDistributionFee = perpDebtConfig.getProtocolDistributionFee();
        protocolMintFee = perpDebtConfig.getProtocolMintFee();
        protocolSwapFee = perpDebtConfig.getProtocolSwapFee();

        protocolServiceFeeAddress = address(0); //perpDebt.protocolServiceFeeAddress;
        protocolDistributionFeeAddress = address(0); //perpDebt.protocolDistributionFeeAddress;
        protocolMintFeeAddress = address(0); //perpDebt.protocolMintFeeAddress;
        protocolSwapFeeAddress = address(0); //perpDebt.protocolSwapFeeAddress;
    }

    /**
     * @notice Returns the configuration data of the perpetual debt
     * @return notionalPriceMax Max notional price when refinancing
     * @return notionalPriceMin Min notional price when refinancing
     * @return isFrozen True if it is frozen, false otherwise
     * @return isPaused True if it is paused, false otherwise
     **/
    function getPerpDebtConfigurationData()
        external
        view
        returns (
            uint256 notionalPriceMax,
            uint256 notionalPriceMin,
            bool isFrozen,
            bool isPaused
        )
    {
        ...
        notionalPriceMax = perpDebt.notionalPriceMax;
        notionalPriceMin = perpDebt.notionalPriceMin;

        (isFrozen, isPaused) = perpDebt.configuration.getFlags();
    }

    /**
     * @notice Returns the debt mint mcaps parameter
     * @return mintCap The debt notional mint cap
     **/
    function getMintCap() external view returns (uint256 mintCap) {
        mintCap = _getPerpetualDebtConfig().getMintCap();
    }

    function getAPY() external view returns (uint256) {
        DataTypes.PerpetualDebtData memory localPerpetualDebt = _getPerpetualDebt();
        uint8 moneyTokenDecimals = IERC20Detailed(address(localPerpetualDebt.money)).decimals();
        uint256 assetPrice = _getFrontEndPrice(address(localPerpetualDebt.zToken));

        uint256 baseDecimals = 27;
        if (baseDecimals >= moneyTokenDecimals) {
            assetPrice = assetPrice * (10**(baseDecimals - moneyTokenDecimals));
        } else {
            assetPrice = assetPrice / (10**(moneyTokenDecimals - baseDecimals));
        }
        return localPerpetualDebt.getAPYgivenAssetPrice(assetPrice);
    }

    function getDebtNotionalPrice() external view returns (uint256) {
        DataTypes.PerpetualDebtData memory localPerpetualDebt = _getPerpetualDebt();
        uint256 assetPrice = _getFrontEndPrice(address(localPerpetualDebt.zToken));
        return localPerpetualDebt.getNotionalPriceGivenAssetPrice(assetPrice);
    }

    function _getPerpetualDebt() internal view returns (DataTypes.PerpetualDebtData memory) {
        return IGuild(ADDRESSES_PROVIDER.getGuild()).getPerpetualDebt();
    }

    function _getPerpetualDebtConfig() internal view returns (DataTypes.PerpDebtConfigurationMap memory) {
        return IGuild(ADDRESSES_PROVIDER.getGuild()).getPerpDebtConfiguration();
    }

    function _getFrontEndPrice(address asset) internal view returns (uint256) {
        return
            ICovenantPriceOracle(ADDRESSES_PROVIDER.getPriceOracle()).getAssetPrice(
                asset,
...
import {IGuildAddressesProvider} from "../interfaces/IGuildAddressesProvider.sol";
import {ICovenantPriceOracle} from "../interfaces/ICovenantPriceOracle.sol";
import {ICovenantHelper} from "../interfaces/ICovenantHelper.sol";
...
import {CollateralConfiguration} from "../protocol/libraries/configuration/CollateralConfiguration.sol";
import {GuildDataProvider} from "../misc/GuildDataProvider.sol";

/**
 * @title CovenantHelper
 * @author Covenant Labs
 */
contract CovenantHelper is ICovenantHelper {
        ...
        )
    {
        ...
        IGuild.UserAccountDataStruct memory userAccountData = _guild.getUserAccountData(user);
        ...
        )
    {
        ...
        IGuild.UserAccountDataStruct memory userAccountData = _guild.getUserAccountData(_onBehalfOf);
        ...
        //This should be equivalent to any remaining zToken in CovenantHelper contract.
        ...
        )
    {
        ...
        if (_exactMoney) {
            ...
            if ((vars.zTokensToBorrow > 0) && (vars.finalZTokenIn < _zTokenInTarget)) {
                //@dev zTokens currently held in CovenantHelper
        ...
        )
    {
        ...
        IGuild.UserAccountDataStruct memory userAccountData = _guild.getUserAccountData(msg.sender);
        ...
        //Validate repay
        if (vars.debtNotionalBurned > 0) _guild.validateRepay(vars.zTokenToBurn);
    ...
    function _quoteMaxWithdaw(
        ...
        uint256 assetPrice = ICovenantPriceOracle(IGuildAddressesProvider(_guild.ADDRESSES_PROVIDER()).getPriceOracle())
    ...
    function _quoteBufferFactor(address guild) internal view returns (uint256 bufferFactor) {
        address _guildDataProvider = IGuild(guild).ADDRESSES_PROVIDER().getGuildDataProvider();
        uint256 notionalPrice = GuildDataProvider(_guildDataProvider).getDebtNotionalPrice();
