...
contract Stabilizer is IStabilizer {
        ...
        validAmount(_usdx_call_amount)
    {
        ...
        call_amount = sweep_to_buy;
        
        if(sweep_balance < sweep_to_buy) {
            uint256 missing_sweep = sweep_to_buy - sweep_balance;
        ...
        amount_to_buy = _min(usdx_balance, missing_usdx);

        if(amount_to_buy > 0) _buy(amount_to_buy, 0);

        sweep_balance = sweep.balanceOf(address(this));
        if(sweep_to_buy <= sweep_balance) _repay(sweep_to_buy);
    }
    ...
    function _autorepays() internal view returns(bool) {
        return call_delay == 0;
    }
