...
contract CatchStaking is AccessControl {
    ...
    function getRemainingLockTime(
        ...
        if (stakes[_address][stakeIndex].amount == 0) return 0;
    ...
    function stake(uint256 _amount) external stakingRunning {
        ...
        stakes[msg.sender].push(
            Stake(_amount, block.timestamp, block.timestamp + LOCK_PERIOD)
        );
    ...
    function unstake() external stakingStarted {
        ...
        for (uint256 i = 0; i < stakes[msg.sender].length; i++) {
            ...
            if (
                ...
                stakes[msg.sender][i].amount = 0; // Mark the stake as withdrawn
            }
        }
    ...
    function unstakeByStakeIndex(uint256 stakeIndex) external stakingStarted {
        ...
        userStake.amount = 0; // Set the stake amount to 0 after withdrawal
        totalStaked -= amount;
