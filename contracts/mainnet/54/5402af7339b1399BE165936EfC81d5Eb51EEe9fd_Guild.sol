...
contract Guild is VersionedInitializable, GuildStorage, IGuild {
    ...
    uint256 public constant GUILD_REVISION = 0x2;
...
library BorrowLogic {
    ...
    function executeBorrow(
        ...
        for (uint16 i; i < params.collateralsCount; i++) {
            address currentCollateralAddress = collateralList[i];
            ...
            DataTypes.CollateralData storage currentCollateral = collateralData[currentCollateralAddress];
            ...
            if (currentCollateral.configuration.getNonMTMLiquidationFlag()) {
                collateralData[currentCollateralAddress].lastMintPrice[params.onBehalfOf] = ICovenantPriceOracle(
                    params.oracle
