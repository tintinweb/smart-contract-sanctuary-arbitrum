...
contract GnomeBooperTestV2 is IUniswapV3FlashCallback, PeripheryImmutableState, PeripheryPayments, Ownable {
    ...
    uint256 pricePerBoopGNOME = 4200 ether;
    ...
    function boopWethX() public payable returns (uint amountGnome, uint amountWeth) {
    ...
    function boopGnomeX(uint256 _gnomeAmount) public payable returns (uint amountGnome, uint amountWeth) {
        ...
        for (uint256 i = 0; i < boopAmount; i++) {
            ...
            // Sell
            IWETH(GNOME).approve(address(swapRouter), _gnomeAmount);
            ...
            IWETH(WETH9).approve(address(swapRouter), amountWeth);
            ISwapRouter.ExactInputSingleParams memory paramsBuy = ISwapRouter.ExactInputSingleParams({
                tokenIn: WETH9,
                tokenOut: GNOME,
                fee: 10000, // Assuming a 0.1% pool fee
