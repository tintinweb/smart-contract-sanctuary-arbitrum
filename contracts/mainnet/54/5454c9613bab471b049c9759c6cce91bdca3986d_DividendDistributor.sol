...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public reflectionsTracker;
    ...
    function deposit(uint256 amount) external onlyToken {
        ...
        reflectionsTracker += rewardAmount;
        //how many reward tokens were actually added
        dividendsPerShare = dividendsPerShare + ((dividendsPerShareAccuracyFactor * rewardAmount) / totalShares);
    }

    function addReflections() external onlyToken {
        uint256 rewardsBalance = rewardToken.balanceOf(address(this));
        // current balance of rewards token
        uint256 diff = rewardsBalance - reflectionsTracker;
        //what is the difference between what is there now, and what we started with (the reflections)

        totalDividends = totalDividends + diff;
        dividendsPerShare = dividendsPerShare + ((dividendsPerShareAccuracyFactor * diff) / totalShares);
    }
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            ...
            reflectionsTracker -= amount;
            //how many reward tokens were actually sent
...
contract CHAD is IERC20Extended, Context, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        setTxLimits(_totalSupply/100, _totalSupply/1000);
    ...
    function addLiquidity(uint256 amountSRGLiq) external {
        uint256 tokensToAdd = getTokenAmountOut(amountSRGLiq);
    ...
    function _sync() public {
        liqConst = liquidity * _balances[address(this)];
    }
    ...
    function getTokenAmountOut(uint256 amountSRGIn) public view returns (uint256) {
    ...
    function distributeTaxes() public {
        if (taxBalance > 0) {
    ...
    function processTaxes() public {
        try IReceiver(burnReceiver).process() {} catch {}
        try IReceiver(assetBackingReceiver).process() {} catch {}
        try IReceiver(liquidityReceiver).process() {} catch {}
        try IReceiver(stakingReceiver).process() {} catch {}
    }
