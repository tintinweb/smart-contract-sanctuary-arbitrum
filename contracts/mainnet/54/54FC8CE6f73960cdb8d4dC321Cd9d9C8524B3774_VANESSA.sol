...
contract VANESSA is ERC20Detailed, Ownable {
    struct Fee {
        uint256 liquidity;
        uint256 treasury;
        uint256 firepit;
    ...
    modifier initialDistributionLock() {
        require(
            initialDistributionFinished || allowTransfer[tx.origin],
    ...
    uint256 public rewardYield = 4571079149;
    ...
    constructor(
        ...
        address _autoLiquidityReceiver,
        address _treasuryReceiver
    ) ERC20Detailed("Undertale", "$UNDER", uint8(DECIMALS)) {
    ...
    function takeFee(
        ...
        if (_totalFee > 0) {
            ...
            uint256 firePitAmount = gonAmount.mul(_firePitFee).div(
                feeDenominator
            );
