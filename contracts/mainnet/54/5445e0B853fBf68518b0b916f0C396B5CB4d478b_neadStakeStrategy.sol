...
contract neadStakeStrategy is Initializable, AccessControlEnumerable {
    ...
    mapping(address => v1RouteInfo[]) public v1RouteForToken;
    mapping(address => bool) isV1Pair;
    bool toRam;
        ...
        unchecked {
            for (uint i; i < len; ++i) {
                ...
                if (_rewards[i] == asset) {
                    ...
                    if (bal > 0) {
                        if (toRam) {
                            IXRam(xRam).instantExit(bal);
                            bal = IERC20(ram).balanceOf(address(this));
                            amount += _swap(ram, bal);
                        } else {
                            uint id = IXRam(xRam).xRamConvertToNft(bal);
                            uint amountBefore = IERC20(asset).balanceOf(
                                address(this)
                            );
    ...
    function _swap(
        ...
        if (amountIn > 0) {
            if (isV1Pair[token]) {
                address tokenOut;
                (amountOut, tokenOut) = _swapV1(token, amountIn);
                // lazy swap logic
                if (tokenOut != asset) {
                    amountOut = _swap(tokenOut, amountOut);
                }
            } else {
                amountOut = _swapV2(token, amountIn);
            }
        }
    }
    ...
    function _swapV1(
        address token,
        uint amountIn
    ) internal returns (uint amountOut, address tokenOut) {
        v1RouteInfo[] memory _routes = v1RouteForToken[token];
        uint len = _routes.length;

        for (uint i; i < len; ) {
            amountOut = IPair(_routes[i].pair).getAmountOut(amountIn, token);
            IERC20(token).transfer(_routes[i].pair, amountIn);

            if (_routes[i].tokenA == token) {
                IPair(_routes[i].pair).swap(0, amountOut, address(this), "");
                token = _routes[i].tokenB;
            } else {
                IPair(_routes[i].pair).swap(amountOut, 0, address(this), "");
                token = _routes[i].tokenA;
            }
            amountIn = amountOut;
    ...
    function setV1Pair(
        ...
        isV1Pair[token] = _v1Pair;
    }

    function setToRam(bool _toRam) external onlyRole(SETTER_ROLE) {
        toRam = _toRam;
    }
    ...
    function setV1Route(
        address token,
        v1RouteInfo[] calldata _routes
    ) external onlyRole(SETTER_ROLE) {
        delete v1RouteForToken[token];
        for (uint i; i < _routes.length; ++i) {
            v1RouteForToken[token].push(_routes[i]);
        }
    }
