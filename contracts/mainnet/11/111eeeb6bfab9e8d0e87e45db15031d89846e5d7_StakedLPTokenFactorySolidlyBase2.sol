...
// File: contracts\interfaces\ISolidlyMAGauge.sol
...
// File: contracts\StakedLPTokenSolidlyBase2.sol

pragma solidity =0.5.16;
contract StakedLPTokenSolidlyBase2 is PoolToken {
	using SafeToken for address;
	
	bool public constant isStakedLPToken = true;
	string public constant stakedLPTokenType = "SolidlyBase2";
	...
	function maBalanceOf() internal view returns (uint) {
		if (tokenId == 0) return 0;
		return ISolidlyMAGauge(gauge).balanceOfToken(tokenId);
	}
	
	function maWithdrawAll() internal  {
		if (tokenId == 0) return;
		ISolidlyMAGauge(gauge).withdrawAndHarvest(tokenId);
	}
	
	function maDeposit() internal {
		maWithdrawAll();
		tokenId = ISolidlyMAGauge(gauge).deposit(underlying.myBalance());
	}
	
	function maWithdraw(uint withdrawAmount) internal {
		maWithdrawAll();
		tokenId = ISolidlyMAGauge(gauge).deposit(underlying.myBalance().sub(withdrawAmount));
	}
	
	/*** PoolToken Overrides ***/
	
	function _update() internal {
		uint256 _totalBalance = maBalanceOf();
	...
	function mint(address minter) external nonReentrant update returns (uint mintTokens) {
		...
		uint256 _totalBalanceBefore = maBalanceOf();
		maDeposit();
		uint256 _totalBalanceAfter = maBalanceOf();
	...
	function _getReward() internal returns (uint256) {
		ISolidlyMAGauge(gauge).getReward(tokenId);
...
// File: contracts\StakedLPTokenFactorySolidlyBase2.sol

pragma solidity =0.5.16;
contract StakedLPTokenFactorySolidlyBase2 {
	address public router;
	...
	function createStakedLPToken(address pair) external returns (address stakedLPToken) {
		...
		bytes memory bytecode = type(StakedLPTokenSolidlyBase2).creationCode;
		assembly {
			stakedLPToken := create2(0, add(bytecode, 32), mload(bytecode), pair)
		}
		StakedLPTokenSolidlyBase2(stakedLPToken)._initialize(pair, token0, token1, router, voter, rewardsToken, bridgeTokens);
