...
contract AICOREFactory is Ownable {
    ...
    function burn(uint256 amount) public {
        ...
        AIBBtoken.transferFrom(msg.sender, DEAD, burnShare);
        //40% treasury
     ...
     */
    function calculateRewards(address userAddress)
        public
        view
        returns (uint256)
    {
     ...
     * @param userAddress The address of user.
     */
    function estimateRewards(address userAddress)
        public
        view
        returns (uint256)
    {
        ...
        uint256 reward = ((user.latestBurnAmount * dailyReward) /
            totalBurnedByDay[user.lastBurnTime / 1 days]);
