...
import {OptionSpreadPool} from "../../libraries/combo-pools/OptionSpreadPool.sol";
...
contract OptionCallSpread is BaseComboOption {
    ...
    struct SpreadSwapCache {
        int256 swapLowAmount0;
        int256 swapLowAmount1;
        int256 swapHighAmount0;
        int256 swapHighAmount1;
    }
        ...
        )
    {
        ...
        uint256 additionalFee = FullMath.mulDiv(uint256(amountSpecified > 0 ? amountSpecified : -amountSpecified), v3Pool.fee(), 1e6);
        // swap1
        (
            spreadSwapCache.swapLowAmount0,
            spreadSwapCache.swapLowAmount1
        ) = v3Pool.swap(
            IV3PoolActions.SwapInputs({
                ...
                amountSpecified: zeroForOne ? amountSpecified + int256(additionalFee) : amountSpecified, 
                sqrtPriceLimitX96: sqrtPriceLimits.sqrtPriceLimitLowX96
            })
        );
        if(spreadSwapCache.swapLowAmount1 != (zeroForOne ? amountSpecified + int256(additionalFee) : amountSpecified)) revert SpecifiedAndReturnedAmountNotRelated();
        
        // should be changed due to next swap in reverse direction
        // warning: we take account of this below (in calculateNewOptionBalance())
        // amountSpecified = -amountSpecified;

        // swap2
        (
            spreadSwapCache.swapHighAmount0,
            spreadSwapCache.swapHighAmount1
        ) = v3Pool.swap(
            IV3PoolActions.SwapInputs({
                optionPoolKeyHash: VanillaOptionPool
                    .Key({
                ...
                zeroForOne: !zeroForOne,
                amountSpecified: !zeroForOne ? -amountSpecified+int256(additionalFee) : -amountSpecified,
                sqrtPriceLimitX96: sqrtPriceLimits.sqrtPriceLimitHighX96
            })
        );
        if(spreadSwapCache.swapHighAmount1 != (!zeroForOne ? -amountSpecified+int256(additionalFee) : -amountSpecified)) revert SpecifiedAndReturnedAmountNotRelated();

        amountSpecified > 0 ? amountSpecified -= int256(additionalFee) : amountSpecified += int256(additionalFee);
        ...
        (
            ...
            spreadSwapCache.swapLowAmount0 + spreadSwapCache.swapHighAmount0,
            amountSpecified
            // spreadSwapCache.swapLowAmount1
        );
        ...
        // просто изменить два пула на их amount{0,1} ?
        v3Pool.updatePoolBalances(
            ...
            spreadSwapCache.swapLowAmount0,
            // -int256(amount0PoolShouldTransfer),
            -int256(amount1PoolShouldTransfer) + (zeroForOne ? int256(0) : int256(additionalFee))
        );
    ...
    function withdraw(
        ...
        amount = uint256(userOptionBalance > 0 ? userOptionBalance : -userOptionBalance).mulDiv(
