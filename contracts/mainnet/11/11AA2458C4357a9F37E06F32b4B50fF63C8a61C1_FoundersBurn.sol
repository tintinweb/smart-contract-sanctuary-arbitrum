...
import "./interfaces/IFoundersBurn.sol";
...
contract FoundersBurn is IFoundersBurn, Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, IERC1271 {
    using AddressUpgradeable for address;
     ...
     */
    function claim(uint256 positionId) external override nonReentrant {
        _claim(positionId);
    }
    ...
    function treasuryOwnedMagicFromBurns() external view override returns (uint256) {
        ...
        return
            (baseAmountV2 * (burntV2 - nonRewardBurntV2)) +
            (baseAmountV1 * (burntV1 - nonRewardBurntV1)) -
            exceptionAddressBurntAmount;
    }
    ...
    /**
     * @dev Topup the contract with Magic
     */
    function topupMagic(uint256 amount) external override {
        MAGIC.transferFrom(msg.sender, address(this), amount);
    }

    /**
     * @dev Withdraw Magic from the contract
     */
    function withdrawMagic(uint256 amount) external override onlyAdmin {
        MAGIC.transfer(msg.sender, amount);
    }

    function _claimable(uint256 positionId) internal view returns (uint256) {
        uint256 endEpoch = burnPositions[positionId].end;
    ...
    function _claim(uint256 positionId) internal {
        ...
        burnPositions[positionId].lastClaimedEpoch = burnPositions[positionId].end;
        if (msg.sender == EXCEPTION_ADDRESS) {
    ...
    /**
     * @dev Sets the Voter address of this contract
     */
    function setVoter(address voter) external onlyAdmin {
    ...
    function isValidSignature(
        ...
        // Validate signatures
        if (ECDSAUpgradeable.recover(_hash, _signature) == VOTER) {
            return 0x1626ba7e;
        } else {
            return 0xffffffff;
        }
...
interface IBattleflyAtlasStakerV02 {
    ...
    function withdrawLiquidAmountForMigration(uint256 amount) external;

    function setAllowedForHarvestersDeploy(uint256 amount) external;

    function depositLiquidAmount(uint256 amount) external;
    ...
    function withdrawForFoundersBurn(uint256 amount) external;
...
interface IBattleflyFounderVault {
    ...
    function pendingStakeBackAmount() external view returns (uint256);

    function topupStakeBackAmount(uint256 amount) external;
}
...
interface IFlywheelEmissions {
    ...
    function setTreasury(address treasury2, address treasury3) external;
    ...
    function setFounderBurn(address founderBurn) external;

    function adjustFoundersBaseAmounts(uint256 toBurnV1, uint256 toBurnV2) external;
...
interface IFoundersBurn {
    ...
    function withdrawMagic(uint256 amount) external;
