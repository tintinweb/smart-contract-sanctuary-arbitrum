...
interface IGMXVault {
  ...
  function updateRelayer(address relayer) external;
...
interface IProtocolTreasury {
  ...
  function updateReceiver(address receiver, bool approved) external;
}
...
library GMXTypes {
  ...
  struct Store {
    ...
    // Relayer address that receives some execution fees for callback execution
...
library LRTTypes {
  ...
  struct Store {
    ...
    // Protocol rewards claim contract address (reserved)
...
import { IProtocolTreasury } from "../interfaces/treasury/IProtocolTreasury.sol";
import { IWNT } from  "../interfaces/tokens/IWNT.sol";
import { IGMXVault } from  "../interfaces/strategy/gmx/IGMXVault.sol";
import { ILRTVault } from  "../interfaces/strategy/lrt/ILRTVault.sol";
import { GMXTypes } from  "../strategy/gmx/GMXTypes.sol";
...
contract ProtocolTreasury is IProtocolTreasury, AccessManaged {
  ...
  // Approved receiver addresses
  mapping(address => bool) public receivers;

  /* ======================= EVENTS ========================== */

  event ReceiverUpdated(address addr, bool approved);
  event SVTokenWithdrawn(address vault, uint256 shareAmt);
  event AssetTransferred(address token, uint256 amt);

  /* ====================== CONSTRUCTOR ====================== */

  /**
    * @notice Initialize and configure wrapped native token for Treasury
  ...
  */
  ...
  /**
    * @notice Withdraw GMX strategy vault token for underlying assets
    * @dev Withdrawing from GMXv2 takes another transaction for assets to be received
    * @param vault Address of vault token
    * @param shareAmt Amount of vault tokens in 1e18
    * @param token Address of token asset to receive
    * @param slippage Slippage tolerance for removing liquidity; e.g. 3 = 0.03%
    * @param executionFee Execution fee sent to GMX for removing liquidity in 1e18
  */
  function withdrawGMXSVToken(
    address vault,
    uint256 shareAmt,
    ...
    _wp.shareAmt = shareAmt;
    _wp.token = token;
    _wp.slippage = slippage;
    _wp.executionFee = executionFee;

    IGMXVault(vault).withdraw{value: executionFee}(_wp);

    emit SVTokenWithdrawn(vault, shareAmt);
  }

  /**
    * @notice Withdraw LRT strategy vault token for underlying assets
    * @param vault Address of vault token
    * @param shareAmt Amount of vault tokens in 1e18
    * @param slippage Slippage tolerance for removing liquidity; e.g. 3 = 0.03%
    * @param deadline Timestamp for deadline for this transaction to complete
  */
  function withdrawLRTSVToken(
    address vault,
    uint256 shareAmt,
    ...
    LRTTypes.WithdrawParams memory _wp;

    _wp.shareAmt = shareAmt;
    _wp.slippage = slippage;
    _wp.deadline = deadline;

    ILRTVault(vault).withdraw(_wp);

    emit SVTokenWithdrawn(vault, shareAmt);
  }

  /**
    * @notice Transfer asset to an approved receiver
    * @param amt Amount of asset in asset decimals
    * @param token Address of asset to transfer
  ...
  */
  function transferAsset(
    ...
    if (!receivers[receiver]) revert Errors.ReceiverNotApproved();

    IERC20(token).safeTransfer(receiver, amt);

    emit AssetTransferred(token, amt);
  }

  /**
    * @notice Update address to be approved/unapproved as receivers
    * @dev Should be called by approved Owner (Timelock + MultiSig)
    * @param receiver Receiver address
    * @param approved Boolean to approve receiver or not
  */
  function updateReceiver(address receiver, bool approved) external restricted {
    receivers[receiver] = approved;

    emit ReceiverUpdated(receiver, approved);
  }
  ...
  /**
    ...
    * @dev We convert any native token to wrapped native
  */
  receive() external payable {
    WNT.deposit{value: address(this).balance}();
  }
}
