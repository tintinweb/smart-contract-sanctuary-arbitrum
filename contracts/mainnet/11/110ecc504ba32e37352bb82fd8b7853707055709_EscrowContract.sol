...
abstract contract EIP712 {
    ...
    function _buildDomainSeparator(
        ...
        return keccak256(abi.encodePacked(typeHash, nameHash, versionHash, block.chainid, address(this)));
    }
...
contract EscrowContract is AccessControl, EIP712 {
    ...
    IERC20 private USDC = IERC20(0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8);
    ...
    event OwnerClaimed(uint amount);
    ...
    event SetSigner(address indexed _signer);
    ...
    function buyerDeposit(Deal calldata newdeal) external returns (bytes32) {
        ...
        require(USDC.allowance(msg.sender, address(this)) >= newdeal.price, 'Allowance is too low');
        bool success = USDC.transferFrom(msg.sender, address(this), newdeal.price);
    ...
    function sellerDeposit(bytes32 _dealHash) external  returns (bool) {
        ...
        require(_deal.exists, "Deal not exists");
        require(msg.sender == _deal.seller, "Not a seller");
        require(_deal.buyer_deposited == _deal.price, 'Buyer not deposited yet');        
        require(USDC.allowance(msg.sender, address(this)) >= _deal.collateral, 'Allowance is too low');
        bool success =  USDC.transferFrom(msg.sender, address(this),  _deal.collateral);
        require(success==true, 'Transfer failed');
        deals[_dealHash].seller_deposited =  _deal.collateral;        
        // return true;
        emit SellerDeposited(_dealHash,  _deal.collateral);
        return success;
    ...
    function callArbitration(bytes32 _dealHash) external {
        EscrowDeal memory _deal = deals[_dealHash];
        require(_deal.exists, "Deal not exists");
        require(msg.sender == _deal.seller || msg.sender == _deal.buyer , "Not a party of the deal");
        deals[_dealHash].arbitration = true;
        
        if (msg.sender == _deal.seller )
        {
        emit ArbitrationCall(_dealHash, true);
        }

        else if (msg.sender == _deal.buyer) 
        {
    ...
    function sellerClaim(bytes32 _dealHash) external returns (bool) {
        ...
        require(msg.sender == _deal.seller, "Not a seller");
        require(_deal.seller_claim !=0, "Nothing to claim yet");
        require(_deal.claim_time < block.timestamp, "Too early to claim");
        uint amount = _deal.seller_claim; 
    ...
    function ownerClaim(uint256 amount) external returns (bool) {
        require(msg.sender == owner, "Not an owner");
        require(owner_claim <=amount, "Too much to claim");
        owner_claim-=amount;
        bool success = USDC.transfer(msg.sender, amount);
        require(success==true, 'Owner claim failed');
    ...
    function buyerClaim(bytes32 _dealHash) external returns (bool) {
        ...
        require(msg.sender == _deal.buyer, "Not a buyer");
        require(_deal.buyer_claim !=0, "Nothing to claim yet");
        require(_deal.claim_time < block.timestamp, "Too early to claim");
        uint amount = _deal.buyer_claim; 
    ...
    function buyerComplete(bytes32 _dealHash) external {
        ...
        require(msg.sender == _deal.buyer, "Not a buyer");
        require(_deal.buyer_deposited >= _deal.price, "Buyer not yet deposited");
        require(_deal.arbitration != true, "Arbitration in process");
        require(_deal.buyer_completed != true, "Already completed the deal");
        deals[_dealHash].buyer_completed = true;
        deals[_dealHash].claim_time = block.timestamp + 60;
        deals[_dealHash].seller_claim = _deal.price + _deal.collateral  - _deal.fee; 
        owner_claim += _deal.fee;
        emit BuyerCompleted(_dealHash);
       
    }

    function performArbitration(bytes32 _dealHash, uint _seller_amount, uint _buyer_amount) external  {
        EscrowDeal memory _deal = deals[_dealHash];

        require(_deal.exists, "Deal not exists");

        require(hasRole(ARBITER_ROLE, msg.sender), "Caller is not arbiter");
        require(_seller_amount <=  _deal.price + _deal.collateral  - _deal.fee, "Payment is too large for the seller");
        require(_buyer_amount <=  _deal.price, "Payment is too large for the buyer");
