...
contract maticzoomDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("maticzoom_Dividend_Tracker", "maticzoom_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "maticzoom_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "maticzoom_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main maticzoom contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "maticzoom_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"); 
        require(newClaimWait != claimWait, "maticzoom_Dividend_Tracker: Cannot update claimWait to same value");
...
contract maticzoom is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    maticzoomDividendTracker public dividendTracker;
    ...
    constructor() ERC20("maticzoom", "$maticzoom") 
    {
        ARBRewardsFee = 1;
        liquidityFee = 1;
        marketingFee = 4;
        ...
        marketingWallet = payable(0x68E5522A106e1faE14a8b6A1D879959725Ca763b);
        devWallet = payable(0xDF7320AA02AD37FE1f784EAAF6CFbFA262Eb6FcF);

        totalFees = ARBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy

        dividendTracker = new maticzoomDividendTracker();

        address _routerAddress = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506
;
    ...
    function updateAllFeeRates(uint256 _ARBRewardsFee,  uint256 _liquidityFee, uint256 _marketingFee, uint256 _devFee, uint256 _extraSellFee) 
    external returns(bool)
    {
        require(_extraSellFee<=4, "Extra Sell Fee Can be 10% or less");
        totalFees = _ARBRewardsFee.add(_liquidityFee).add(_marketingFee).add(_devFee);
        require(totalFees<=16, "Fee Can be 15% or less");
        ARBRewardsFee = _ARBRewardsFee;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "maticzoom: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "maticzoom: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "maticzoom: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "maticzoom: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "maticzoom: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "maticzoom: Cannot update gasForProcessing to same value");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        swapTokensForARB(swapableTokens, address(this)); 
        uint256 deltaBalance = address(this).balance-initialBalance;
        uint256 ARBToAddLiquidityWith = deltaBalance.mul(liquidityFee).div(totalFees).div(2);

        addLiquidity(halfLiquidityTokens, ARBToAddLiquidityWith);


        uint256 ARBForMarketing = deltaBalance.mul(marketingFee).div(totalFees);
        marketingWallet.transfer(ARBForMarketing);

        uint256 ARBForDev = deltaBalance.mul(devFee).div(totalFees);
        devWallet.transfer(ARBForDev);
    ...
    function swapTokensForARB(uint256 tokenAmount, address _to) public 
    {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ARBAmount) public 
    {
        uniswapV2Router.addLiquidityETH{value: ARBAmount}(
            address(this),
            tokenAmount,
