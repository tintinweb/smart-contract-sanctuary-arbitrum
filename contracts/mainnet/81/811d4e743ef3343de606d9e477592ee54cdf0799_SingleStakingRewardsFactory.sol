...
contract ReentrancyGuard {
    ...
    modifier nonReentrant() {
        uint256 localValue = _guardCounter;
        require(localValue == 1, "ReentrancyGuard: reentrant call");
        _guardCounter = 2;
        _;
        _guardCounter = 1;
    }
...
contract StakingSingleRewards is
    IStakingRewards,
    ...
    ReentrancyGuard
{
    ...
    //Constants
    uint256 public constant SECONDS_IN_1_DAY = 86400;
    ...
    constructor(
        ...
        require(
            ...
            "_rewardsDistribution address cannot be 0"
        );
        require(
            _rewardsToken != address(0),
            "_rewardsToken address cannot be 0"
        );
        require(_manager != address(0), "_manager address cannot be 0");
        require(
            _stakingToken != address(0),
            "_stakingToken address cannot be 0"
        );
    ...
    function rewardPerDay() external view returns (uint256) {
        return rewardRate * SECONDS_IN_1_DAY;
    }
    ...
    function withdraw(
        ...
        if (isLocked) {
            require(block.timestamp > periodFinish, "Period is not finished");
        }
    ...
    function notifyRewardAmount(
        ...
        rewardDuration = rewardsDuration;
        emit RewardAdded(reward, periodFinish);
    }

    function toggleWithdrawBeforeEnd() external {
        require(msg.sender == manager, "Unauthorized");
        isLocked = !isLocked;
        emit WithdrawLockToggled(!isLocked);
    }
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external {
        require(msg.sender == manager, "Unauthorized");
        if (address(stakingToken) == tokenAddress) {
            tokenAmount = tokenAmount.sub(_totalSupply);
        }
        IERC20(tokenAddress).safeTransfer(manager, tokenAmount);
    ...
    event WithdrawLockToggled(bool isLocked);
    event PauseToggled(bool isPaused);
}

contract SingleStakingRewardsFactory is Ownable {
    using SafeERC20 for IERC20;
    // immutables
    ...
    function update(
        ...
        address stakingPool = info.stakingRewards;
        require(
    ...
    function notifyRewardAmount(address stakingToken) public {
        ...
        if (info.rewardAmount > 0 && info.duration > 0) {
            ...
            IERC20(info.rewardsToken).safeTransfer(stakingPool, rewardAmount);
            StakingSingleRewards(stakingPool).notifyRewardAmount(
                rewardAmount,
                duration
            );
            emit NotifyPool(stakingPool, stakingToken, rewardAmount, duration);
        }
    }

    function pullExtraTokens(
        address token,
        uint256 amount
    ) external onlyOwner {
