...
contract CIVVault is Ownable, ReentrancyGuard {
    ...
    event SetInvestAddress(uint indexed id, address oldAddress, address newAddress);
    /// @notice Event emitted when send fee to our treasury
    event SendFeeWithOwner(uint indexed id, address treasuryAddress, uint feeAmount);
    ...
    event SharesDistributed(uint indexed id, uint epoch, address indexed investor, uint dueShares);
    /// @notice Event emitted when user claim Asset token
    event WithdrawedToken(uint indexed id, address indexed user, uint assetAmount);
    ...
    */
    ...
    function processFund(uint _id, uint _newVPS) private {
        ...
        uint feeAmount = (epoch.totDepositedAssets * strategy.entryFee) / 100; // Calculate the fee
        epoch.totDepositedAssets -= feeAmount; // Calculate net amount after fee deduction

        strategy.assetToken.safeTransfer(strategy.withdrawAddress[0], feeAmount / 2);
        strategy.assetToken.safeTransfer(strategy.withdrawAddress[1], feeAmount / 2);
    ...
    function deposit(
        ...
        require (block.timestamp <=
            epoch.epochStartTime + epoch.duration, "ERR_V.5"
        );
    ...
    {   
        ...
        uint curEpoch = getCurrentEpoch(_id);
        require (block.timestamp <=
            _epochInfo[_id][curEpoch].epochStartTime + _epochInfo[_id][curEpoch].duration, "ERR_V.5"
        );
        UserInfoEpoch storage userEpoch = _userInfoEpoch[_id][_msgSender()][
            curEpoch
        ];
    ...
    function _distributeShares(uint _id) internal {
        ...
        uint loopLimit = maxUsersToDistribute > 0 && maxUsersToDistribute < epoch.totDepositors
            ? maxUsersToDistribute
            : epoch.totDepositors;

        uint lastProcessedIndex = epoch.lastDepositorProcessed;
        // Distribute proportionally
        for (uint i = epoch.lastDepositorProcessed; i < loopLimit; i++) {
            ...
            if (dueShares > 0) {
                ...
                emit SharesDistributed(_id, lastProcessedEpoch, investor, dueShares);
            }

            lastProcessedIndex = i;
        }

        epoch.lastDepositorProcessed = lastProcessedIndex + 1;
    }
