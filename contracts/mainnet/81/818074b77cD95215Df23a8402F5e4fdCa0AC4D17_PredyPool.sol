...
contract PredyPool is IPredyPool, IUniswapV3MintCallback, Initializable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address uniswapFactory) public onlyOperator initializer {
    ...
    function uniswapV3MintCallback(uint256 amount0, uint256 amount1, bytes calldata) external onlyOperator override {
     ...
     * @param isQuoteToken Is quote or base
     */
    function withdrawCreatorRevenue(uint256 pairId, bool isQuoteToken) external onlyOperator onlyPoolOwner(pairId) {
        Perp.AssetPoolStatus storage pool = _getAssetStatusPool(pairId, isQuoteToken);
     ...
     */
    function take(bool isQuoteAsset, address to, uint256 amount) external onlyByLocker onlyOperator {
