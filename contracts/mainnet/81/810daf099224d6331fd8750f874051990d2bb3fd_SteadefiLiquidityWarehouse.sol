...
import {ILendingVault} from "./vendor/ISteadefiLendingVault.sol";
import {WadRayMath} from "aave-v3-core/contracts/protocol/libraries/math/WadRayMath.sol";
import {Math} from "openzeppelin/utils/math/Math.sol";
import {IERC20} from "openzeppelin/token/ERC20/IERC20.sol";
import {EnumerableSet} from "openzeppelin/utils/structs/EnumerableSet.sol";

contract SteadefiLiquidityWarehouse is LiquidityWarehouse {
    using EnumerableSet for EnumerableSet.AddressSet;
    using Math for uint256;

    constructor(LiquidityWarehouse.ConstructorParams memory params) LiquidityWarehouse(params) {}

    /// @inheritdoc LiquidityWarehouse
    function _withdrawFromTarget(uint256 withdrawAmount, address withdrawTarget, bytes memory) internal override {
        ILendingVault lendingVault = ILendingVault(withdrawTarget);

        // Determine the maximum amount of assets that can be withdrawn from the lending vault.  This is calculated
        // by taking the minimum of the available amount of assets in the lending vault and the missing amount to
        // withdraw
        uint256 maxWithdrawableAmount = Math.min(lendingVault.totalAvailableAsset(), withdrawAmount);

        // Determine the maximum amount of shares that the liquidity warehouse can burn.  Calculated
        // by taking the min between the LP balance of the liquidity warehouse and the amount of LP tokens
        // the maxWithdrawableAmount represents
        uint256 sharesAmt = Math.min(
            IERC20(withdrawTarget).balanceOf(address(this)),
            // Convert to 18DP as the LP tokens are denominated in 18dp
            (maxWithdrawableAmount * IERC20(withdrawTarget).totalSupply()).ceilDiv(lendingVault.totalAsset())
        );

        uint256 minAssetAmt = sharesAmt * lendingVault.totalAsset() / IERC20(withdrawTarget).totalSupply();

        lendingVault.withdraw(sharesAmt, minAssetAmt);
    }

    /// @inheritdoc LiquidityWarehouse
    function _getDeployedAssetValue(address withdrawTarget) internal view override returns (uint256) {
        ILendingVault lendingVault = ILendingVault(withdrawTarget);
        return lendingVault.lvTokenValue() * IERC20(withdrawTarget).balanceOf(address(this)) / WadRayMath.WAD;
    }
    ...
    AccessControlDefaultAdminRules
{
    ...
    function depositLender(uint256 depositAmount) external whenNotPaused nonReentrant {
        ...
        uint256 totalAssetAmount = getLenderNetAssetValue();
        s_assetData.lenderBalance += depositAmount.toUint216();

        uint256 shareAmt = _convertToShares(totalSupply(LENDER_LP_TOKEN_ID), totalAssetAmount, depositAmount);
    ...
    function depositBorrower(uint256 depositAmount) external whenNotPaused nonReentrant {
        ...
        uint256 borrowerTotalLPSupplyAmt = totalSupply(BORROWER_LP_TOKEN_ID);
        uint256 totalAssetAmount = getBorrowerNetAssetValue();
        uint256 shareAmt = _convertToShares(borrowerTotalLPSupplyAmt, totalAssetAmount, depositAmount);

        if (shareAmt == 0) revert ZeroShareAmt();
        if (borrowerTotalLPSupplyAmt == 0 && !hasRole(PRIMARY_BORROWER_ROLE, msg.sender)) revert AccessForbidden();
    ...
    /// @inheritdoc ILiquidityWarehouse
    function withdrawLender(uint256 shareAmount, address[] calldata withdrawTargets, bytes calldata data)
        external
        nonReentrant
    {
        ...
        _withdrawFromTargets(withdrawableAmount, withdrawTargets, data);
        ...
        s_assetData.lenderBalance -= withdrawableAmountAfterFee.toUint216();

        _safeTransferFrom(
            msg.sender, s_terms.feeRecipient, LENDER_LP_TOKEN_ID, withdrawalFee.wadMul(shareAmount), bytes("")
        );
        _burn(msg.sender, LENDER_LP_TOKEN_ID, shareAmount - withdrawalFee.wadMul(shareAmount));

        s_terms.asset.safeTransfer(msg.sender, withdrawableAmountAfterFee);
        emit AssetWithdrawn(withdrawableAmountAfterFee, false);
    }

    /// @inheritdoc ILiquidityWarehouse
    function withdrawLenderFees(address[] calldata withdrawTargets, bytes calldata data)
        external
        nonReentrant
        onlyFeeRecipient
    {
    ...
    /// @inheritdoc ILiquidityWarehouse
    function withdrawBorrowerFees(address[] calldata withdrawTargets, bytes calldata data)
        external
        nonReentrant
        onlyFeeRecipient
    {
    ...
    function _withdrawFromTarget(uint256 withdrawAmount, address withdrawTarget, bytes memory data) internal virtual;
    ...
    /// @param withdrawTargets The target addresses to try withdraw from
    /// @param data Arbitrary data that can be used by the underlying implementation
    function _withdrawFromTargets(uint256 withdrawAmount, address[] memory withdrawTargets, bytes memory data)
        internal
    {
        uint256 currentAssetBalance = s_terms.asset.balanceOf(address(this));
        if (currentAssetBalance > withdrawAmount) return;
        ...
        if (missingWithdrawableAmount > 0) revert InsufficientLiquidity(missingWithdrawableAmount);
    }
    ...
    function _isValidWithdrawTarget(address withdrawTarget) internal view returns (bool) {
        return s_withdrawTargets.contains(withdrawTarget);
    }
