...
abstract contract MultiRewardsPoolBase is Reentrancy, IMultiRewardsPool {
    ...
    function _rewardPerToken(address token) internal view returns (uint) {
        ...
        return
            ...
            (((_lastTimeRewardApplicable(token) - Math.min(lastUpdateTime[token], periodFinish[token])) * rewardRate[token]) / derivedSupply);
    }
    ...
    function removeRewardToken(address token) external onlyOperator {
        ...
        // assume it will be DTOKEN + pool tokens
        uint i = 3;
        bool found = false;
        for (; i < length; i++) {
    ...
    function _calcRewardPerToken(
        ...
        return (
            ((Math.min(endTime, _periodFinish) - Math.min(Math.max(lastSupplyTs0, startTimestamp), _periodFinish)) * rewardRate[token]) / supply,
            endTime
        );
    }
