...
import "./types/AccountRoles.sol";
import "./libraries/Signature.sol";
import "./interfaces/IReferrals.sol";
import "./interfaces/IFxToken.sol";
    ...
    AccessControlRolesUpgradeable
{
    ...
    function doesUserHaveAccountRole(
        ...
        bool hasExactRole = accounts[id].users[user].roles[role];
        if (hasExactRole) {
            return true;
        }
        bool isAccountOwner = accounts[id].users[user].roles[AccountRole.Owner];
        bool isAdminRole = role == AccountRole.ProtocolAdmin;
        bool isNoneRole = role == AccountRole.None;
        if (isAccountOwner && !isAdminRole && !isNoneRole) {
            // The owner role owns all other roles, EXCEPT ProtocolAdmin.
            // The None role is invalid.
            return true;
        }
        // Note that the `hasRole` check is for the contract access control,
        // and NOT for a trade account.
        // The admin must also own the trade account.
        if (isAdminRole && hasRole(ADMIN_ROLE, user) && isAccountOwner) {
            return true;
        }
        return false;
    }
    ...
    function validateAccountRoleSignature(
        ...
        validateAccountRoleSignatureWithNonce(
            id,
            accountUser,
            accountUserRole,
            signature,
     ...
     * @param accountUserRole The account role that is being validated.
     * @param signature The user signature, with payload: `{id}{nonce}{role}`.
     * @param nonce The nonce to use in the signature check.
     */
    function validateAccountRoleSignatureWithNonce(
        uint256 id,
        address accountUser,
        AccountRole accountUserRole,
        bytes calldata signature,
        ...
        bytes32 messageNon191 = keccak256(
            abi.encode(MESSAGE_SCOPE, nonce, id, uint8(accountUserRole))
        );
     ...
     * @param accountUserRole The account role that is being validated.
     * @param signature The user signature, with payload: `{id}{nonce}{role}`.
     * @param otherNonce An external nonce. The highest between this value
     *        and the user's nonce in this contract will be used.
     * @return the highest nonce
     */
    function validateAccountRoleSignatureWithHighestNonce(
        uint256 id,
        address accountUser,
        AccountRole accountUserRole,
        bytes calldata signature,
        ...
        uint256 nonce = userNonces[accountUser];
        uint256 highestNonce = otherNonce > nonce ? otherNonce : nonce;
        validateAccountRoleSignatureWithNonce(
            id,
            accountUser,
            accountUserRole,
    ...
    function validateAccountRolePermission(
        ...
        // Note that the owner needs to sign the role required for the
        // action they are doing, as they own all other roles (except for
        // the ProtocolAdmin role).
        // Otherwise, their signature check will fail in the
        // `validateRoleSignature` function.
        bool isPermissionlessRole = (accountUserRole == AccountRole.Deposit ||
            isOpeningAccount(id, accountUserRole));
        bool hasPermission = isPermissionlessRole ||
            doesUserHaveAccountRole(id, accountUser, accountUserRole);
        ...
        returns (bool)
    {
        ...
        // an ID yet.
        return accounts[id].isOpen || isOpeningAccount(id, accountUserRole);
    }

    function isOpeningAccount(uint256 id, AccountRole role)
        private
        pure
        returns (bool)
    {
        return id == 0 && role == AccountRole.Open;
    }
    ...
    function validateAccountRoleManagement(
        ...
        require(
            role != AccountRole.ProtocolAdmin,
            "Account: cannot use ProtocolAdmin role"
        );
    ...
    function _beforeTokenTransfer(
        ...
        // The previous owner retains trading permissions.
        grantAccountUserRoleUnchecked(firstTokenId, from, AccountRole.Trader);
    }
...
enum AccountRole {
    ...
    Open,
    // This role is used for signing protocol administration messages.
    // It is automatically managed and synchronised to users that have the
    // ADMIN_ROLE smart contract access control role.
    // For example, if an user has the ADMIN_ROLE permission on the smart
    // contract then it has this role on all of the user's trade accounts --
    // any of those accounts can be used for signing protocol admin messages.
    ProtocolAdmin
}