...
contract SpotMarket is IFillerMarket, ISpotMarket, Owned {
        ...
        returns (int256 quoteTokenAmount)
    {
        ...
        _validateSwap(spotOrder.baseTokenAmount, quoteTokenAmount, spotOrder.limitQuoteTokenAmount, spotOrder.auctionData);
    ...
    function _validateMarketOrder(
        ...
        AuctionParams memory auctionParams =
            abi.decode(auctionData, (AuctionParams));
...
contract SpotMarketL2 is SpotMarket {
    constructor(address permit2Address)
        SpotMarket(permit2Address)
    {
    }

    function executeMarketOrder(SpotMarketOrder memory marketOrder, bytes memory sig, SettlementParams memory settlementParams)
        external
        returns (int256 quoteTokenAmount)
    {
    ...
    function executeLimitOrder(SpotLimitOrder memory limitOrder, bytes memory sig, SettlementParams memory settlementParams)
        external
        returns (int256 quoteTokenAmount)
    {
        SpotOrder memory order = SpotOrder({
