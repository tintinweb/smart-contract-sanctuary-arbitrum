...
contract Miladyai is ERC20, Ownable {
    ...
    uint256 public holdersFeeBuy = 0;
    ...
    uint256 public holdersFeeSell = 0;
    ...
    function swapBack() internal isSwapping {
        ...
        uint256 amountOfMiladyaiBurn = (taxAmount * burnFee) / (totalFee);
        uint256 amountOfMiladyaiLp = (taxAmount * liquidityFee) / (totalFee);
        uint256 amountOfMiladyaiForHolders = (taxAmount * holdersFee) / (totalFee);
        uint256 amountOfMiladyaiForStaking = (taxAmount * stakingFee) / (totalFee);

        taxAmount -= amountOfMiladyaiBurn;
        taxAmount -= amountOfMiladyaiLp;
        taxAmount -= amountOfMiladyaiForHolders;
        taxAmount -= amountOfMiladyaiForStaking;
        ...
        _transfer(address(this), DEAD, amountOfMiladyaiBurn);
        _transfer(address(this), holdersWallet, amountOfMiladyaiForHolders);
        _transfer(address(this), stakingRewardWallet, amountOfMiladyaiForStaking);
        ...
        emit SwapBack(amountOfMiladyaiBurn, amountOfBackTokenMkt, amountOfMiladyaiLp, amountOfMiladyaiForStaking, amountOfMiladyaiForHolders, amountOfBackTokenDev, block.timestamp);
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "Milady: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "Milady: index out of bounds");
