...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
	...
	function _baseURI() internal view virtual returns (string memory) {
		return "https://ipfs.io/ipfs/QmRwovYueiAe3unRVpWmqiXRnexdmDPzkonmzzNfW1VDez/";
	}
...
contract Nft_ACNFT is ERC721Enumerable {

	address owner;
	event Mint (address indexed mintBy);
	uint256 public mint_NativePrice = uint256(10000000000000);
	...
	constructor() ERC721("ARBCORGINFT", "ACNFT") {
		owner = msg.sender;
	}
 ...
 * checks that ((totalSupply) + (_amt)) is less than or equals to 112
 * checks that ((balanceOf with variable owner as (the address that called this function)) + (_amt)) is less than or equals to 112
 * checks that (amount of native currency sent to contract) is equals to ((_amt) * ((mint_NativePrice) / (10 raised to the power of 0)))
 * checks that ((mintClaims with element the address that called this function) + (1)) is less than or equals to 10
 * checks that ((mintTotalAmt with element the address that called this function) + (_amt)) is less than or equals to 100
 ...
 * updates mintAmtInBank as (mintAmtInBank) + ((_amt) * ((mint_NativePrice) / (10 raised to the power of 0)))
 ...
 * updates mintTotalAmt (Element the address that called this function) as (mintTotalAmt with element the address that called this function) + (_amt)
 * emits event Mint with inputs the address that called this function
*/
	function mint(uint256 _amt) public payable {
		require(((totalSupply() + _amt) <= uint256(112)), "Exceeded Total Supply");
		require(((balanceOf(msg.sender) + _amt) <= uint256(112)), "Exceeded Individual Supply Constraints");
		require((msg.value == (_amt * (mint_NativePrice / (uint256(10) ** uint256(0))))), "Wrong Fee");
		require(((mintClaims[msg.sender] + uint256(1)) <= uint256(10)), "Exceed number of claims per address for this phase");
		require(((mintTotalAmt[msg.sender] + _amt) <= uint256(100)), "Exceed number of NFTs mintable per address for this phase");
		for (uint i0 = 0; i0 < _amt; i0++){
			uint256 supply = super.totalSupply();
			super._safeMint(msg.sender, supply);
		}
		mintAmtInBank  = (mintAmtInBank + (_amt * (mint_NativePrice / (uint256(10) ** uint256(0)))));
		mintClaims[msg.sender]  = (mintClaims[msg.sender] + uint256(1));
		mintTotalAmt[msg.sender]  = (mintTotalAmt[msg.sender] + _amt);
