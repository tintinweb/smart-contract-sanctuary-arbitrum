...
// File: src/rdLPStaking.sol

pragma solidity ^0.8.0;

contract rdLPstaking is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 totalRP;
        uint256 allocPoint; // How many allocation points assigned to this pool. esRADTs to distribute per block.
        uint256 lastRewardTime; // Last block time that esRADT distribution occurs.
        uint256 accesRADTPerShare; // Accumulated esRADT per share, times 1e12. See below.
    ...
    uint256 public totalesRADTdistributed = 0;

    // set a max esRADT per second, which can never be higher than 1 per second
    uint256 public constant maxesRADTPerSecond = 1e18;
    ...
    // The block time when esRADT mining starts.
    ...
    function supplyRewards(uint256 _amount) external onlyOwner {
        totalesRADTdistributed = totalesRADTdistributed.add(_amount);
        esRADT.transferFrom(msg.sender, address(this), _amount);
    }
    ...
    // Update the given pool's esRADT allocation point. Can only be called by the owner.
    ...
    // Deposit LP tokens to MasterChef for esRADT allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        user.rewardDebt = userPoint.mul(pool.accesRADTPerShare).div(1e12);
    ...
    // Safe esRADT transfer function, just in case if rounding error causes pool to not have enough esRADTs.
    function safeesRADTTransfer(address _to, uint256 _amount) internal {
        uint256 esRADTBal = esRADT.balanceOf(address(this));
        if (_amount > esRADTBal) {
            esRADT.transfer(_to, esRADTBal);
        } else {
            esRADT.transfer(_to, _amount);
        }
    }
}