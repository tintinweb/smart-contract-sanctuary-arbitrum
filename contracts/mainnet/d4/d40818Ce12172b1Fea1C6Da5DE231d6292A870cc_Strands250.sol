...
contract Strands250 is ERC721, DefaultOperatorFilterer, OwnableAdmins {
  uint256 public cap;
  ...
  function setIndividualize(bool i) public onlyAdmins {
    individualize = i;
  }

  function setTokenURI(uint256 tokenId, string memory _tokenURI) public onlyAdmins {
    require((mintCounter == 0 || tokenId <= mintCounter), "can't set URI for nonexistent token");
    _tokenURIs[tokenId] = _tokenURI;
  }

  function adminSelfBatchMint(uint8 num) public onlyAdmins {
  ...
  function setFeeRecipient(address _newRecipient) external onlyAdmins {
    feeRecipient = _newRecipient;
  }

  function setFeeAmount(uint256 _newAmount) external onlyAdmins {
    feeAmount = _newAmount;
  }

  function setFeeToken(address _newToken) external onlyAdmins {
    feeToken = _newToken;
  }
}
