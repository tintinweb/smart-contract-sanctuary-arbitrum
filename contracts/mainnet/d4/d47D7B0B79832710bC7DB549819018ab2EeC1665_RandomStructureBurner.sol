...
contract RandomStructureBurner is ManagerModifier, IRandomStructureBurner {
  ...
  function availableUsageCounts(
    ...
    for (uint i = 0; i < _structureIds.length; i++) {
      result[i] = _calculateCapacity(poolSizes[i], _lossPerUsage[i]);
    }
    return result;
  }
  ...
  function _reduceDurability(
    ...
    uint maxUsagesPerStructure = (HALF_DURABILITY + _lossPerUsage - 1) /
      _lossPerUsage;

    // Calculate batch sizes
    uint batchSize = (_usages + maxUsagesPerStructure - 1) /
      maxUsagesPerStructure;
    ...
    uint[][] memory durabilityLosses = new uint[][](batchSize);
    uint[][] memory structureIds = new uint[][](batchSize);
    for (uint j = 0; j < batchSize; j++) {
      structureIds[j] = ArrayUtils.toMemoryArray(_structureId, 1);

      if (_usages > maxUsagesPerStructure) {
        durabilityLosses[j] = ArrayUtils.toMemoryArray(
          maxUsagesPerStructure * _lossPerUsage,
          1
        );
        _usages -= maxUsagesPerStructure;
      } else if (_usages > 0) {
        durabilityLosses[j] = ArrayUtils.toMemoryArray(
          _usages * _lossPerUsage,
          1
        );
        _usages = 0;
      } else {
        durabilityLosses[j] = ArrayUtils.toMemoryArray(0, 1);
      }
    }

    address[] memory realmAddresses = ArrayUtils.toMemoryArray(
      REALM,
      batchSize
    );
    ...
    if (_durabilityLossHandler != address(0)) {
  ...
  function _calculateCapacity(
    ...
    uint singleStructureMaxDurability = (ONE_HUNDRED + _loss - 1) / _loss;
    return _poolSize * singleStructureMaxDurability;
  }
  ...
  function _ensurePoolSizesHaveEnoughCapacity(
    ...
    for (uint i = 0; i < poolSizes.length; i++) {
      ...
      if (_calculateCapacity(poolSizes[i], _durabilityLoss[i]) < _amounts[i]) {
