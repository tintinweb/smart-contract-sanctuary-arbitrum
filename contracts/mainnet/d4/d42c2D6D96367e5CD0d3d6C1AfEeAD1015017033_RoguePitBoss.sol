...
contract RoguePitBoss is ERC721Royalty, VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    ...
    // NFT mint variables
    uint256 startDate = 1714406400; // 4/29/24 16.00 pm GMT, must be in past
    uint256 currentDay = ((block.timestamp - startDate) + 600) / 600; // change to 3600 to test. solidity rounds down so add extra day before division.
    uint256 startPrice = 100000000000000; // 0.0001 ETH change back to 2370000000000000000; // 2.37 ETH
    uint256 currentPrice = startPrice + ((currentDay - 1) * 100000000000000); // add extra 2 zero to change back to 0.01, makes price rise by 0.0001 ETH per day
    ...
    {
        if (tokenId > 10) { // tokenId 1 - 104 are replacing existing DHRC NFTs so are being minted by admin. 105 - 114 are pre-sold so minted by admin
        ...
        if (tokenId < 10) { // take same rarity from DHRC NFT with same tokenId
        ...
        if (tokenId > 9) { // get random number from Chainlink VRF to determine rarity 
    ...
    function updateDayAndPrice() external {
        currentDay = ((block.timestamp - startDate) + 600) / 600; // change to 3600 to test
        currentPrice = startPrice + ((currentDay - 1) * 100000000000000);//  0.0001
