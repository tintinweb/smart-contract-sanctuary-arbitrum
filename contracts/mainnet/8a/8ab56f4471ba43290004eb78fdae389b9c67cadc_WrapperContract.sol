...
contract WrapperContract {
    ...
    event LogIt(uint _value);
    event LogIt2(uint _value, bytes result);
    ...
    constructor(address _fiat24ContractAddress, address _affiliate) {
        fiat24ContractAddress = _fiat24ContractAddress;
        owner = payable(msg.sender);
        affiliate = payable(_affiliate);
    }

    function depositTokenViaUsdc(address _inputToken, address _outputToken, uint256 _amountToDeposit, uint256 _feeAmount) external returns (uint256) {
        ...
        emit LogIt2(2, result);
        require(success, "Failed to call depositTokenViaUsdc() in the fiat 24 contract");
        emit LogIt(3);
        IERC20 tokenContract = IERC20(_inputToken);
        tokenContract.transferFrom(msg.sender, affiliate, _feeAmount);
        emit LogIt(4);
