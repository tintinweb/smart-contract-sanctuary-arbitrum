...
import "./interfaces/IAlgebraPoolV1_9.sol";
import "./interfaces/IAlgebraFactory.sol";
import "./interfaces/IAlgebraStateMulticallV1_9.sol";

contract AlgebraStateMulticallV1_9_02 is IAlgebraStateMulticall {
    ...
    function _calcTicksFromBitMap(
        ...
        for (uint256 i = 0; i < tickBitmap.length; i++) {
            ...
            for (uint256 j = 0; j < 256; j++) {
                if (bitmap & (1 << j) > 0) {
                    int24 populatedTick = ((int24(tickBitmap[i].index) << 8) + int24(j));
...
import "./pool/IAlgebraPoolStateV1_9.sol";
...
library AdaptiveFee {
    ...
    function getFee(
        ...
        return
            uint16(
                config.baseFee +
                    sigmoid(volumePerLiquidity, config.volumeGamma, uint16(sumOfSigmoids), config.volumeBeta)
