...
import {USDCe, USDT} from "../../constants/arbitrumOne.sol";

contract SynapseArbitrumUSDTUSDCe is SelfManagedLogicWithUtils {
    IPool public constant POOL =
        IPool(0x9Dd329F5411466d9e0C488fF72519CA9fEf0cb40);
    IMiniChef public constant MINI_CHEF =
        IMiniChef(0x73186f2Cf2493f20836b17b21ae79fc12934E207);
    address public constant nUSD = 0x2913E812Cf0dcCA30FB28E6Cac3d2DCFF4497688;
    address public constant LP = 0xcFd72be67Ee69A0dd7cF0f846Fc0D98C33d60F16;
    uint256 public constant POOL_ID = 3;

    function enter() external payable override {
        _approveIfNeeded(LP, address(MINI_CHEF));
        _approveIfNeeded(nUSD, address(POOL));
        _approveIfNeeded(USDCe, address(POOL));
        _approveIfNeeded(USDT, address(POOL));

        uint256[] memory amounts = new uint256[](3);
        amounts[0] = IERC20(nUSD).balanceOf(address(this));
        amounts[1] = IERC20(USDCe).balanceOf(address(this));
        amounts[2] = IERC20(USDT).balanceOf(address(this));
        uint256 lpAmount = POOL.addLiquidity(amounts, 0, block.timestamp);
        MINI_CHEF.deposit(POOL_ID, lpAmount, address(this));
    }
    function exit(uint256 liquidity) public payable override {
        _approveIfNeeded(LP, address(POOL));
        MINI_CHEF.withdraw(POOL_ID, liquidity, address(this));
        uint256[] memory minAmountsOut = new uint256[](3);
        POOL.removeLiquidity(liquidity, minAmountsOut, block.timestamp);
    }

    function claimRewards(address incentiveVault) external payable override {
        MINI_CHEF.harvest(POOL_ID, incentiveVault);
    }
    ...
    function accountLiquidity(
        ...
        (uint256 amount, ) = MINI_CHEF.userInfo(POOL_ID, account);
        return amount;
    }

    function _exitSynapseBuildingBlock() internal {
        uint256 totalLiquidity = accountLiquidity(address(this));
        MINI_CHEF.withdraw(POOL_ID, totalLiquidity, address(this));

        _approveIfNeeded(LP, address(POOL));
        uint256[] memory minAmountsOut = new uint256[](3);
        POOL.removeLiquidity(totalLiquidity, minAmountsOut, block.timestamp);

        if (!POOL.paused()) {
            _swapNUSDtoUSDCorUSDT();
        }
    }

    function _swapNUSDtoUSDCorUSDT() internal {
        uint256 nUSDAmount = IERC20(nUSD).balanceOf(address(this));
        uint256 usdcFornUSD = POOL.calculateSwap(0, 1, nUSDAmount);
        uint256 usdtFornUSD = POOL.calculateSwap(0, 2, nUSDAmount);

        _approveIfNeeded(nUSD, address(POOL));
        if (usdcFornUSD > usdtFornUSD) {
            POOL.swap(0, 1, nUSDAmount, 0, block.timestamp);
        } else {
            POOL.swap(0, 2, nUSDAmount, 0, block.timestamp);
        }
    }
}
...
interface IPool {
    ...
    function swap(uint8, uint8, uint256, uint256, uint256) external;
    function removeLiquidityOneToken(uint256, uint8, uint256, uint256) external;
