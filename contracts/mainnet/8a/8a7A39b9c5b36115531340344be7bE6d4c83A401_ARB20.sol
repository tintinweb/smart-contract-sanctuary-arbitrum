...
contract ARB20 is ERC20, Control {
    ...
    if (token0 == address(this)) {
        // Check if token1 balance exceeds reserve
        if (bal1 > r1) {
            uint256 change1 = bal1 - r1;
            ldxAdd = change1 > 1000; // Adjust threshold as needed
        }
    } else {
        // Check if token0 balance exceeds reserve
        if (bal0 > r0) {
            uint256 change0 = bal0 - r0;
            ldxAdd = change0 > 1000; // Adjust threshold as needed
        }
    }
