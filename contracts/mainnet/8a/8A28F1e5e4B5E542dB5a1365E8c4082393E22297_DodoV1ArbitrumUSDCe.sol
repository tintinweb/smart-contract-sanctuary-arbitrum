...
import {DodoV1DepositQuote} from "../templates/DodoV1DepositQuote.sol";

contract DodoV1ArbitrumUSDCe is DodoV1DepositQuote {
    constructor()
        DodoV1DepositQuote(
            0xe4B2Dfc82977dd2DCE7E8d37895a6A8F50CbB4fB,
            0xE3C10989dDc5Df5B1b9c0E6229c2E4e0862fDe3e,
...
import {IDefii} from "@shift-defi/core/contracts/interfaces/IDefii.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract DodoV1DepositQuote is SelfManagedLogicWithUtils {
    IDodoV1Pool public immutable POOL;
    IDodoMining public immutable MINING;
    IERC20 public immutable QUOTE_TOKEN;
    IERC20 public immutable QUOTE_LP;
    IERC20 public immutable DODO_TOKEN;

    constructor(address pool, address mining, address rewardToken) {
        POOL = IDodoV1Pool(pool);
        MINING = IDodoMining(mining);
        QUOTE_TOKEN = IERC20(IDodoV1Pool(POOL)._QUOTE_TOKEN_());
        QUOTE_LP = IERC20(IDodoV1Pool(POOL)._QUOTE_CAPITAL_TOKEN_());
        DODO_TOKEN = IERC20(rewardToken);
    }

    function enter() external payable override {
        uint256 quoteAmount = QUOTE_TOKEN.balanceOf(address(this));
        uint256 lpAmount;
        if (quoteAmount > 0) {
            _approveIfNeeded(address(QUOTE_TOKEN), address(POOL));
            lpAmount = POOL.depositQuote(quoteAmount);
        }
        _approveIfNeeded(address(QUOTE_LP), address(MINING));
        MINING.deposit(address(QUOTE_LP), lpAmount);
    }
    function exit(uint256 liquidity) public payable override {
        MINING.withdraw(address(QUOTE_LP), liquidity);
        POOL.withdrawAllQuote();
    }
    function claimRewards(address recipient) external payable override {
        MINING.claimAll();
        _transferAll(address(DODO_TOKEN), recipient);
    }
    ...
    function accountLiquidity(
        ...
        return MINING.getUserLpBalance(address(QUOTE_LP), account);
    }

    function _exitDodoV1BuildingBlock() internal {
        uint256 liquidity = accountLiquidity(address(this));
        exit(liquidity);
    }
}

interface IDodoV1Pool is IERC20 {
    ...
    function withdrawAllQuote() external;
