...
library UnitagLib {
    ...
    function hashTokenId(string memory chainPrefix, address nftContract, uint256 tokenId) internal pure returns (uint256) {
        return uint256(keccak256(abi.encode(chainPrefix, nftContract, tokenId)));
    }
}

interface IUnitagCallbackHandler {
    function tokenBinded(address to, uint256 id, uint256 value, uint256 amount) external returns (uint256);

    function tokenBindedBatch(address to, uint256 id, uint256[] calldata values, uint256[] calldata amounts) external returns (uint256);
}
