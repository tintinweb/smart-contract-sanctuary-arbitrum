...
contract AIAPE_IDO is Ownable, ReentrancyGuard {
    ...
    uint256 public airdropAmount = 30 * 10 ** investmentTokenDecimals;
    uint256 public airdropReferralRewardPercent = 10;
    address[] public airdropAddress;
    uint256 public maxQualifiedAddresses = 10000;
    uint256 public totalInvestment;
    uint256 public totalTokens;
    uint256 tokenPrice = 0.0000229 ether;
    ...
    function invest() external payable nonReentrant {
        ...
        uint256 tokens = (msg.value * 10 ** investmentTokenDecimals) /
            tokenPrice;
        ...
        require(
            IERC20(defaultInvestmentToken).transfer(msg.sender, tokens),
            "Transfer failed"
        );
        emit Invest(msg.sender, tokens);
    }
    ...
    function withdrawEth() external onlyOwner {
        (bool success, ) = payable(owner()).call{value: totalInvestment}("");
    ...
    function getAirdrop(address referrer) external nonReentrant {
        ...
        if ((referrer != address(0) && referrer != msg.sender)) {
            uint256 reward = (airdropAmount * airdropReferralRewardPercent) /
                100;
            if (referrers[msg.sender] != address(0)) {
                referrer = referrers[msg.sender];
                referrersRewards[msg.sender] += reward;
            } else {
            ...
            emit AirdropReferral(referrer, msg.sender, reward);
        }
    }

    function getAirdrop() external nonReentrant {
        require(
            airdropRecords[msg.sender] == 0,
        ...
        require(
            IERC20(defaultInvestmentToken).transfer(msg.sender, airdropAmount),
            "Transfer failed"
        );
        airdropRecords[msg.sender] = airdropAmount;
