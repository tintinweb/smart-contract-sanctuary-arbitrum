...
contract MultiChainSenderOrReceiver is BoringOwnable, IReceiver {
    ...
    function sendNFT (IERC721 nft, uint256 id, uint256 chainId, address recipient) external {
        ...
        if (address(original) == address(0)) {
            ...
            adapter.send(chainId, address(nft), id, nft.name(), nft.symbol(), nft.tokenURI(id), recipient,  royalty);
        } else {
            MultiChainNFT(address(nft)).burn(id);
            adapter.send(chainId, address(original), id, nft.name(), nft.symbol(), nft.tokenURI(id), recipient,  royalty);
        }
    }
...
contract MultiChainNFT is Ownable, ERC721, IERC2981, IMasterContract {
    ...
    function init(bytes calldata data) public payable override {
        ...
        require(bytes(symbol).length == 0 && bytes(symbol).length != 0);
        _transferOwnership(owner_);
        minter = minter_;
        name = _name;
        symbol = _symbol;
