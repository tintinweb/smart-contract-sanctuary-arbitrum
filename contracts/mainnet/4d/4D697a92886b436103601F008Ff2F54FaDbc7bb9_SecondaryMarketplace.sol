...
contract SecondaryMarketPositionManager {
    ...
    uint256[50] private __gap_sm_pm; // secondaryMarketplace positionManager = sm_pm
        ...
        while (remaining > 0) {
            ...
            if (flag) {
                ...
                if (totalProfit <= remaining) {
                    ...
                    IVault(poolToBorrowFrom).notifyProfit(totalProfit);
                } else {
                    // may be this else is not required
                    ...
                    remaining -= remaining;
                    IVault(poolToBorrowFrom).notifyProfit(remaining);
                }
            } else {
                // If there's a loss, update loss filled
                _positions.lossFilled += totalLoss;
                IVault(poolToBorrowFrom).nofityLoss(totalLoss);
            }
            // 2890
