...
contract LRTfiPowerFarmApiRouting is GenericPowerManager {
    ...
    address public immutable WISE_POOL;
        ...
        )
    {
        ...
        FARM_ASSET = block.chainid == 1
            ? WETH_ADDRESS_ETH_MAIN // for example we can read WETH from WiseLending
            : WETH_ADDRESS;

        // @TODO - get rid of ternary, parameter base
        POOL_ASSET_AAVE = block.chainid == 1
            ? AAVE_WETH_ADDRESS_ETH_MAIN // for example this can be defined in _powerFarmNFTs
            : AAVE_WETH_ADDRESS; // or read AaveHub from WiseLending and then AaveWeth

        _doApprovals(
            _wiseLendingAddress
        );

        // @TODO - get rid of ternary, parameter base DEPLOYMENT
        // abstract this to be on the deploy.script level instead
        if (block.chainid == 1) {
            WISE_POOL = 0x21b8065d10f73EE2e260e5B47D3344d3Ced7596E;
