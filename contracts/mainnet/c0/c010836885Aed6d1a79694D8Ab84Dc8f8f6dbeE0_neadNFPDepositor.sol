...
import "./interfaces/Ramses/INonfungiblePositionManager.sol";
import "./interfaces/Ramses/IGaugeV2.sol";
import "./interfaces/Ramses/IGaugeV2Factory.sol";
    ...
    UUPSUpgradeable
{
    ...
    function claimRewards(uint _tokenID) public {
        ...
        for (uint i; i < tokens.length; ) {
            ...
            if (bal > 0) {
                ...
                if (address(_token) == ram) {
                    bool state = ISwappoor(swappoor).priceOutOfSync();
                    if (state) {
                        uint amountOut = ISwappoor(swappoor).swapTokens(
                            ram,
                            neadRam,
                            owed
                        );
                        ...
                        emit claimReward(msg.sender, neadRam, amountOut);
                    } else {
                        IVeDepositor(neadRam).depositTokens(owed);
                        IERC20Upgradeable(neadRam).transfer(msg.sender, owed);
                        emit claimReward(msg.sender, neadRam, owed);
                    }
                } else {
                    _token.transfer(msg.sender, owed);
                    emit claimReward(msg.sender, tokens[i], owed);
     ...
     */
    function reattachAll() public onlyRole(OPERATOR_ROLE) {
