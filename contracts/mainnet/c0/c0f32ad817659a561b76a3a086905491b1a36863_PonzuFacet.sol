...
import {PonzuStorage, RoundWinner, ParticipantDeposit} from "../types/ponzu/PonzuStorage.sol";
...
contract PonzuFacet is WithRoles {
  ...
  function getCurrentRoundData() external view returns (PonzuRoundData memory) {
    ...
    return
      PonzuRoundData({
        ...
        totalParticipants: ps.newParticipantsList.length
      });
  }
  ...
  function totalParticipants() external view returns (uint256) {
    PonzuStorage storage ps = LibPonzu.DS();
    return ps.newParticipantsList.length;
  }

  function getParticipantList() external view returns (address[] memory) {
    PonzuStorage storage ps = LibPonzu.DS();
    return ps.newParticipantsList;
  }
...
import {PonzuStorage, PERCENTAGE_DENOMINATOR, RoundWinner, MAX_DEPOSIT, ParticipantDeposit} from "@contracts/ponzu/types/ponzu/PonzuStorage.sol";
...
library LibPonzu {
  ...
  function deposit(PonzuStorage storage ps, address user, uint256 amount) internal {
    ...
    uint256 participantCurDeposit = participantDeposit.amount;
    uint256 total = participantCurDeposit + amount;
    if (total > MAX_DEPOSIT) revert MaxDepositReached();

    if (participantCurDeposit == 0) {
      // has no deposits yet
      participantDeposit.timestamp = block.timestamp;
      // add to participants list
      ps.newParticipantsList.push(user);
    }
  ...
  function withdraw(PonzuStorage storage ps, address user, uint256 amount) internal {
    ...
    if (newBalance == 0) {
      ...
      ps.newParticipantsList.swapOut(user);
    }
  ...
  function selectWinner(PonzuStorage storage ps, uint256 randomNumIn) internal {
    ...
    if (winnerAmount > 0) {
      if (newWinnerTotalDeposit > MAX_DEPOSIT) {
        ...
        ps.totalDeposited += winnerAmount;
      }
    }
  ...
  function _selectWinner(
    ...
    uint256 totalParticipantsCount = ps.newParticipantsList.length;
    uint256 totalDeposited = ps.totalDeposited;
    // divide number by total deposited
    uint256 winnerNumber = randomNum % totalDeposited;

    console.log("totalParticipantsCount: %s", totalParticipantsCount);
    console.log("totalDeposited: %s", totalDeposited);
    console.log("winnerNumber: %s", winnerNumber);

    uint256 totalAmountsCalculated = 0;

    for (uint256 i = 0; i < totalParticipantsCount; ++i) {
      address participantAddress = ps.newParticipantsList[i];
      ...
      totalAmountsCalculated += pDeposit;
    ...
    console.log("totalAmountsCalculated: %s", totalAmountsCalculated);
...
struct PonzuStorage {
  ...
  address[] participantsList; // deprecating
  uint256 startTime;
...
library AddressArrayLibUtils {
  ...
  // efficient push many
  function pushMany(address[] storage array, address[] memory values) internal {
    uint256 valuesLength = values.length;
    uint256 arrayLength = array.length;
    uint256 newLength;
    unchecked {
      newLength = arrayLength + valuesLength;
    }
    assembly {
      sstore(array.slot, newLength)
    }
  ...
  function popMany(address[] storage array, uint256 count) internal {
    uint256 arrayLength = array.length;
    uint256 newLength;
    unchecked {
      newLength = arrayLength - count;
    }
    assembly {
      sstore(array.slot, newLength)
    }
