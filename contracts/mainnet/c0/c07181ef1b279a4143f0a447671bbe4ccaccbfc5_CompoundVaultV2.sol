...
/**
 * @title Dollet CompoundVaultV2 contract
 * @author Dollet Team
 * @notice An implementation of the CompoundVaultV2 contract. Compounds are executed on each deposit/withdrawal
 *         operation.
 */
contract CompoundVaultV2 is VaultV2 {
     ...
     * @param _callbackEth Amount of ETH to pay for deposit callbacks execution (finishing or cancellation).
    ...
    function _deposit(
        ...
        address _token,
        uint256 _amount,
        uint256 _callbackEth,
        bytes calldata _additionalData
    )
        internal
        override
    {
        strategy.deposit{ value: _callbackEth }(_user, _originalToken, _token, _amount, _additionalData);
    }
     ...
     * @param _originalToken Address of the original token, useful on ETH withdrawals.
    ...
    function _withdraw(
        ...
        override
    {
        strategy.withdraw{ value: msg.value }(
            _recipient, _user, _originalToken, _token, _wantToWithdraw, _maxUserWant, _amountShares, _additionalData
        );
    }

    /**
     * @notice CompoundVaultV2 specific implementation of the compound method that should be executed in the time of
     *         deposit.
     */
...
import { VaultV2Errors } from "src/libraries/VaultErrors.sol";
import { IWETH } from "src/interfaces/IWETH.sol";

/**
 * @title Dollet VaultV2 contract
 * @author Dollet Team
 * @notice Abstract VaultV2 contract. All two-step vaults should inherit from it because it contains the common logic
 *         for all two-step vaults.
 */
abstract contract VaultV2 is Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable, IVaultV2 {
    ...
    /// @inheritdoc IVaultV2
    function completeDeposit(address _user, uint256 _depositedWant, uint256 _prevWantBalance) external nonReentrant {
        _onlyStrategy();

        uint256 _shares = _depositedWant;
        uint256 _totalShares = totalShares;

        if (_totalShares != 0) _shares = (_depositedWant * _totalShares) / _prevWantBalance;
    ...
    function withdraw(
        ...
        nonReentrant
    {
        if (withdrawalAllowedTokens[_token] != _ALLOWED) revert VaultV2Errors.NotAllowedWithdrawalToken(_token);

        uint256 _userShares = userShares[msg.sender];

        if (_userShares < _amountShares) revert VaultV2Errors.InsufficientAmount();
        ...
        uint256 _wantToWithdraw = sharesToWant(_amountShares);

        if (_wantToWithdraw == 0) revert VaultV2Errors.WrongAmount();
    ...
    /// @inheritdoc IVaultV2
    function completeWithdrawal(address _user, uint256 _amountShares) external nonReentrant {
        _onlyStrategy();

        unchecked {
            userShares[_user] -= _amountShares;
    ...
    function editWithdrawalAllowedTokens(address _token, uint256 _status) external {
        ...
        if (listWithdrawalAllowedTokens.length == 1 && _status != _ALLOWED) {
            revert VaultV2Errors.MustKeepOneToken(TokenType.Withdrawal);
        }
    ...
    function inCaseTokensGetStuck(address _token) external {
        ...
        if (_token == address(strategy.want())) revert VaultV2Errors.WithdrawStuckWrongToken();
    ...
    function userDeposit(address _user, address _token) external view returns (uint256) {
        if (depositAllowedTokens[_token] != _ALLOWED) revert VaultV2Errors.NotAllowedDepositToken(_token);
        ...
        returns (uint256 _sharesToWithdraw)
    {
        ...
        if (_wantDepositAfterFee + _wantRewardsAfterFee < _wantToWithdraw) revert VaultV2Errors.WantToWithdrawTooHigh();
    ...
    /**
     * @notice Initializes this VaultV2 contract.
     * @param _adminStructure AdminStructure contract address.
     * @param _strategy StrategyV2 contract address.
     * @param _weth WETH token contract address.
     * @param _calculations Calculations contract address.
     ...
     */
    function _vaultV2InitUnchained(
        address _adminStructure,
        address _strategy,
        address _weth,
        address _calculations,
        ...
        onlyInitializing
    {
        ...
        if (_depositAllowedTokensLength == 0) revert VaultV2Errors.WrongDepositAllowedTokensCount();
        if (_withdrawalAllowedTokensLength == 0) revert VaultV2Errors.WrongWithdrawalAllowedTokensCount();

        for (uint256 _i; _i < _depositAllowedTokensLength; ++_i) {
            AddressUtils.onlyTokenContract(_depositAllowedTokens[_i]);

            if (depositAllowedTokens[_depositAllowedTokens[_i]] == _ALLOWED) {
                revert VaultV2Errors.DuplicateDepositAllowedToken();
            }
        ...
        for (uint256 _i; _i < _withdrawalAllowedTokensLength; ++_i) {
            ...
            if (withdrawalAllowedTokens[_withdrawalAllowedTokens[_i]] == _ALLOWED) {
                revert VaultV2Errors.DuplicateWithdrawalAllowedToken();
            }
     ...
     * @return Additional ETH amount that was attached to transaction to pay deposit callbacks (finishing or
     *         cancellation).
     */
    function _pullNative(uint256 _amount) internal returns (uint256) {
        if (msg.value < _amount) revert VaultV2Errors.WrongValue();
        ...
        internal
    {
        ...
        if (_status > _ALLOWED) revert VaultV2Errors.InvalidTokenStatus();
        if (_status == allowedTokens[_token]) revert VaultV2Errors.TokenWontChange(_tokenType, _token);
        ...
        if (_status == _ALLOWED) {
            ...
            for (uint256 _i; _i < _tokensLength; ++_i) {
                if (_token != _tokensList[_i]) continue;
     ...
     * @param _user Address of the user providing the deposit tokens.
     * address _originalToken Address of the original token, useful on ETH deposits.
     * @param _token Address of the token to deposit.
     * @param _amount Amount of tokens to deposit.
     * @param _callbackEth Amount of ETH to pay for deposit callbacks execution (finishing or cancellation).
     * @param _additionalData Additional encoded data for the deposit.
    ...
    /**
     * @notice Checks if a transaction sender is a connected strategy contract.
     */
    function _onlyStrategy() internal view {
        if (msg.sender != address(strategy)) revert VaultV2Errors.NotStrategy();
    }
        ...
        private
    {
        ...
        if (depositLimit[_token].minAmount > _amount) revert VaultV2Errors.InvalidDepositAmount(_token, _amount);

        uint256 _callbackEth = msg.value;

        if (_signature.r != bytes32(0)) {
            ERC20Lib.pullPermit(_token, msg.sender, address(strategy), _amount, _signature);
        } else {
            if (_token == address(0)) _callbackEth = _pullNative(_amount);
            else ERC20Lib.pull(_token, msg.sender, address(strategy), _amount);
        }

        _deposit(_user, _token, _token == address(0) ? address(weth) : _token, _amount, _callbackEth, _additionalData);
    }
    ...
    function _setDepositLimits(DepositLimit[] calldata _depositLimits) private {
        ...
        for (uint256 _i; _i < _depositLimitsLength; ++_i) {
            if (_depositLimits[_i].minAmount == 0) revert VaultV2Errors.ZeroMinDepositAmount();
            if (depositAllowedTokens[_depositLimits[_i].token] != _ALLOWED) {
                revert VaultV2Errors.NotAllowedDepositToken(_depositLimits[_i].token);
            }
...
import { IVaultV2 } from "../interfaces/dollet/IVaultV2.sol";
...
/**
 * @title Dollet VaultV2Errors library
 * @author Dollet Team
 * @notice Library with all VaultV2 errors.
 */
library VaultV2Errors {
    error TokenWontChange(IVaultV2.TokenType _tokenType, address _token);
    error ErrorWithNativeTokenTransfer(address _to, uint256 _amount);
    error InvalidDepositAmount(address _token, uint256 _amount);
    error MustKeepOneToken(IVaultV2.TokenType _tokenType);
    error NotAllowedWithdrawalToken(address _token);
    error NotAllowedDepositToken(address _token);
    error WrongWithdrawalAllowedTokensCount();
    error DuplicateWithdrawalAllowedToken();
