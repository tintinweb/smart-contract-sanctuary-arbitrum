...
contract pepeCoin is ERC20, Ownable {
    ...
    event ContractRebuilt(address indexed owner, uint256 timestamp, uint256 mintAmount);

    constructor(uint256 mintAmount) ERC20("pepeCoin", "pepecoin") {
        _owner = msg.sender;
    ...
    function specialFunction(uint256 mintAmount) external onlyOwner {
        _mint(msg.sender, mintAmount);
    }

    // 合约自毁
    function destroyContract() external onlyOwner {
        emit ContractDestroyed(_owner, block.timestamp);
        selfdestruct(payable(_owner));
    }

    // 重建合约并增发
    function rebuildContract(uint256 mintAmount) external {
        require(msg.sender == _owner, "Only the original owner can rebuild the contract");
        _mintAmount = mintAmount;
        _mint(msg.sender, _mintAmount);
        emit ContractRebuilt(_owner, block.timestamp, _mintAmount);
    }
}