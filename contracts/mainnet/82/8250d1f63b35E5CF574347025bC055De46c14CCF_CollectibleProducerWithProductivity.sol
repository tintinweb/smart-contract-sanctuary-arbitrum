...
struct CollectibleConfig {
  ...
  uint32 tokenCostProductivityCap;
}
...
contract CollectibleProducerWithProductivity is Pausable, ManagerModifier, ReentrancyGuard {
  ...
  function collect(
    ...
    uint[] memory productivities = PRODUCTIVITY.currentProductivityBatch(_realmIds);
  ...
  function collectionCostsAndMaxAmounts(
    ...
    for (uint i = 0; i < _realmIds.length; i++) {
      ...
      for (uint j = 0; j < _collectibleIds[i].length; j++) {
        ...
        costs[i][j] = _calculateCost(capacities[i], uint(productivities[i]), cfg);
        maxAmounts[i][j] = _calculateAmount(capacities[i], uint(productivities[i]), cfg);
        if (hasReactor) {
          maxAmounts[i][j] *= 2;
        }
      }
  ...
  function _produce(
    ...
    uint _productivity,
    uint _capacity
  ) internal returns (uint totalCost, uint productivitySpent, uint spentCapacity) {
    ...
    uint integerAmount = maxAmount / DECIMAL_POINT;
    if (_amount > integerAmount) {
      revert ProductionCapacityExceeded(_realmId, _collectibleId, _amount, integerAmount);
    }

    spentCapacity = ((_capacity * _amount * DECIMAL_POINT) / maxAmount);
    uint baseAmount = (spentCapacity * config.baseProductivity) / ONE_HUNDRED;
    productivitySpent = config.productivityCost * _amount;
    ...
    emit Produced(_realmId, _collectibleId, _amount, totalCost, productivitySpent, spentCapacity);
  }
  ...
  function _calculateAmount(
    ...
    uint adjustedProductivity = collectibleConfig.baseProductivity +
      (uint(_productivity) * ONE_HUNDRED) /
      _productionCapacity;
    ...
    if (adjustedProductivity > collectibleConfig.productionAmountProductivityCap) {
  ...
  function _calculateCost(
    ...
    adjustedProductivity += collectibleConfig.tokenCostProductivityCap;

    uint cost = (collectibleConfig.baseTokenCost * adjustedProductivity) /
      collectibleConfig.tokenCostProductivityCap;
    if (cost > collectibleConfig.maxTokenCost) {
      return collectibleConfig.maxTokenCost;
    }
  ...
  function updateCollectibleConfigs(
    ...
    for (uint i = 0; i < _collectibleIds.length; i++) {
      ...
      CollectibleConfigGeos storage existingGeoConfig = COLLECTIBLE_CONFIGS_GEOS[collectibleId];

      // Disable current geos
      for (uint j = 0; j < existingGeoConfig.geos.length; j++) {
        GEO_SUPPORTED_COLLECTIBLES[existingGeoConfig.geos[j]][collectibleId] = false;
      }
      ...
      for (uint j = 0; j < _geoConfigs[i].geos.length; j++) {
        GEO_SUPPORTED_COLLECTIBLES[_geoConfigs[i].geos[j]][collectibleId] = true;
      }
    }
  }
