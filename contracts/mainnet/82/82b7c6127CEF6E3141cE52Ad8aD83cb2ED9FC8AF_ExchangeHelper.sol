...
import { LibExchangeHelper, ExchangeInfo, ExchangePool } from "./LibExchangeHelper.sol";

interface IExchangeHelper {
    function initializeExchangeHelper () external;
}

contract ExchangeHelper {
    function EHInit () external {
        LibExchangeHelper.initialize();
    }
    function initializeExchangeHelper () external {
        LibExchangeHelper.initialize();
    }
    ...
    function addExchanges (ExchangeInfo[] memory  exchangeParams) external {
        LibExchangeHelper.addExchanges(exchangeParams);
    }
    ...
    function updateExchange (uint _id, ExchangeInfo memory _exchange) external {
        LibExchangeHelper.updateExchange(_id, _exchange);
    }
    function hasExchangePool (address poolAddress) external view returns(bool hasIt) {
        hasIt = LibExchangeHelper.hasExchangePool(poolAddress);
    }
    function getExchangePool (uint _id) external view returns (ExchangePool memory pool) {
        pool = LibExchangeHelper.getExchangePool(_id);
    }
    function getExchangePoolByAddress (address poolAddress) external view returns (ExchangePool memory pool) {
        pool = LibExchangeHelper.getExchangePool(poolAddress);
    }

    function addExchangePool (ExchangePool memory pool) external {
        LibExchangeHelper.addExchangePool(pool);
    }
    function addExchangePools(ExchangePool[] memory pools) external {
        LibExchangeHelper.addExchangePools(pools);

    }
    function getExchangePoolsCount () external view returns (uint count) {
        count = LibExchangeHelper.getExchangePoolsCount();
    }
    function getExchangePools (uint[] memory _ids) external view returns (ExchangePool[] memory exchangePools) {
        exchangePools = LibExchangeHelper.getExchangePools(_ids);
    }
    function getExchangePoolId (address poolAddress) external view returns(uint _id) {
        _id = LibExchangeHelper.getExchangePoolId (poolAddress);
    }

    function getExchangePoolsByRange (uint _start, uint _stop) external view returns (ExchangePool[] memory exchangePools) {
        exchangePools = LibExchangeHelper.getExchangePoolsByRange (_start, _stop);
    }
    function getExchangePoolsAll () external view returns (ExchangePool[] memory exchangePools) {
        uint _start = 0;
        uint _stop = LibExchangeHelper.getExchangePoolsCount();
        exchangePools = LibExchangeHelper.getExchangePoolsByRange(_start, _stop);
    }
}
...
library LibExchangeHelper {
    ...
    function  getExchangePoolReserves (uint id) internal view returns (uint poolId, uint reserve0, uint reserve1) {
        ...
        if (pool.poolType == ExchangePoolType.UNISWAP_V2_PAIR) {
            ReserveInfo memory reserves = LibUniswapV2Helper.getReserves(pool.pool);
            reserve0 = reserves.reserve0;
            reserve1 = reserves.reserve1;
            poolId = id;
        } else if (pool.poolType == ExchangePoolType.UNISWAP_V3_POOL) {
            ReserveInfo memory reserves = LibUniswapV3Helper.getReserves(IUniswapV3Pool(pool.pool));
            reserve0 = reserves.reserve0;
            reserve1 = reserves.reserve1;
            poolId = id;

        } else if (pool.poolType == ExchangePoolType.CAMELOT_V3_POOL) {
            ReserveInfo memory reserves = LibCamelotV3.getReserves(pool.pool);
            reserve0 = reserves.reserve0;
            reserve1 = reserves.reserve1;
            poolId = id;
        }
