...
interface IDO {
 ...
 contract  returnC {
     ...
     function claim(address ido,address ercAddress,address adr,address inviter) public  payable {
         IDO(ido).claim{value:msg.value}(inviter);
...
contract BatchClaim {
    ...
    function batchClaim (uint amount,address inviter) public payable{
        require(msg.value == idoEtherAmount*amount,"Insufficient quantity");
        for(uint a;a < amount;a++){
            returnC claimC = new returnC();
            claimC.claim{value:idoEtherAmount}(idoAddress,ercAddress,address(this),inviter);
