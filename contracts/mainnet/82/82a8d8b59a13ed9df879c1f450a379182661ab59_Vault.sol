 ...
 */
abstract contract PoolRegistry is VaultAuthorization {
...
abstract contract PoolTokens is PoolRegistry, AssetManagers {
    using BalanceAllocation for bytes32;
    using BalanceAllocation for bytes32[];
...
contract Whitelist is Ownable {
    ...
    mapping(address => bool) public poolBalances;

    // /**
    //  * @dev Throws if called by any account other than the whitlist.
    //  */
    // modifier forSwaps() {
    //     _require(swaps(msg.sender), Errors.ADDRESS_NOT_ALLOWLISTED);
    //     _;
    // }

    // modifier forPoolBalances() {
    //     _require(poolBalances(msg.sender), Errors.ADDRESS_NOT_ALLOWLISTED);
    //     _;
    // }

    // function swaps(address account) public view returns(bool) {
    //     return _swaps[account];
    // }

    // function poolBalances(address account) public view returns(bool) {

    // }

    /**
     * @dev Set whitelist of the contract.
     * Can only be called by the current owner.
     */
    function setForSwap(address account, bool isWhitelist) external onlyOwner {
        swaps[account] = isWhitelist;
    }
     ...
     * Can only be called by the current owner.
     */
    function setForPoolBalance(address account, bool isWhitelist) external onlyOwner {
        poolBalances[account] = isWhitelist;
    }
}
 ...
 */
abstract contract PoolBalances is Fees, PoolTokens, UserBalance {
    using Math for uint256;
    using SafeERC20 for IERC20;
    using BalanceAllocation for bytes32;
    using BalanceAllocation for bytes32[];
    ...
    modifier forPoolBalances() {
        _require(whitelist.poolBalances(msg.sender), Errors.ADDRESS_NOT_ALLOWLISTED);
        _;
    }
    ...
    ) private forPoolBalances nonReentrant withRegisteredPool(poolId) authenticateFor(sender) {
 ...
 * individual swaps.
 */
abstract contract Swaps is PoolBalances {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;
    using EnumerableMap for EnumerableMap.IERC20ToBytes32Map;
    ...
    modifier forSwaps() {
        _require(whitelist.swaps(msg.sender), Errors.ADDRESS_NOT_ALLOWLISTED);
        _;
    }
...
contract Vault is VaultAuthorization, FlashLoans, Swaps {
    ...
    ) VaultAuthorization(authorizer) AssetHelpers(weth) TemporarilyPausable(pauseWindowDuration, bufferPeriodDuration) PoolBalances(whitelist) {
        // solhint-disable-previous-line no-empty-blocks
    }
