...
contract PerpetualVault is Initializable, Ownable2StepUpgradeable, ReentrancyGuardUpgradeable {
  ...
  struct DepositInfo {
    ...
    address owner;
    uint256 blockNumber;
    address recipient;        // this is set at the moment of withdrawal request
  }
  ...
  // uint256 public withdrawalCycle;       // the cycle of withdrawal
  ...
  bool _gmxLock;                       // status value
  
  SwapProgress swapProgressData;      // in order to cope with gmx swap fails, should keep the amount of already uniswapped and prior-swapped
  ...
  modifier noneFlow() {
    require(flow == FLOW.NONE, "action in progress");
    _;
  }
  ...
  function initialize(
    ...
    governanceFee = 500;  // 5%
    minCollateralAmount = 10_000_000;               // should pass parameter in the `initialize` function
    maxCollateralAmount = 1_000_000_000_000;     // should pass parameter in the `initialize` function
    minEth = 0.002 ether;
  ...
  function deposit(uint256 amount) external nonReentrant noneFlow payable {
    flow = FLOW.DEPOSIT;
    require(amount > minCollateralAmount, "not enough amount");
    require(totalCollateralAmount + amount <= maxCollateralAmount, "exceeds collateral cap");
    _payExecutionFee();
    collateralToken.safeTransferFrom(msg.sender, address(this), amount);
    counter++;
    depositInfo[counter] = DepositInfo(amount, 0, msg.sender, block.number, address(0));
    ...
    nextAction.data = abi.encode(counter, beenLong, leverage);
  }

  /**
   * @notice
   *  register a withdrawal request, 
   *  all withdrawal requests are saved in the queue and it's run by keeper periodically
   * @param recipient address to receive tokens
   * @param depositId deposit ID of collateral token to withdraw
   */
  function withdraw(address recipient, uint256 depositId) public nonReentrant noneFlow {
    require(recipient != address(0), "zero address");
    require(EnumerableSet.contains(userDeposits[msg.sender], depositId), "not a valid user");
    require(pendingWithdrawalList.length < maxWithdrawalLength, "withdrawal request queue is full");
    pendingWithdrawalList.push(depositId);
  }
    ...
    if (isOpen) {
      if (positionIsClosed == true) {
        ...
        if (beenLong == isLong) {
          ...
          nextAction.selector = NextActionSelector.INCREASE_ACTION;
          nextAction.data = abi.encode(0, isLong, _leverage);
  ...
  function runNextAction(MarketPrices memory prices, bytes[] memory _swapData) external nonReentrant gmxLock onlyKeeper {
    ...
    if (_nextAction.selector == NextActionSelector.INCREASE_ACTION) {
      if (positionIsClosed == true && flow == FLOW.DEPOSIT) {
        (uint256 depositId, , ) = abi.decode(_nextAction.data, (uint256, bool, uint256));
        _mint(depositId, depositInfo[depositId].amount);
      } else {
        (
          ,
          bool _isLong,
          uint256 _leverage
        ) = abi.decode(_nextAction.data, (uint256, bool, uint256));
  ...
  /**
   * if current position is gmx position, settles all pending fees (borrowing fee, negative funding fee) 
   *  against collateral before withdrawal actions so that we could ingore fee deduction in the withdrawal process from gmx positions.
   */
  function settle() external nonReentrant noneFlow onlyKeeper {
    if (curPositionKey != bytes32(0)) {
      ...
      uint256 sumOfShares;
      for (uint8 i = 0; i < pendingWithdrawalList.length; ) {
        uint256 depositId = pendingWithdrawalList[i];
        sumOfShares = sumOfShares + depositInfo[depositId].shares;
        unchecked {
          i = i + 1;
        }
      }
  ...
  function afterOrderExecution(
    ...
    if (orderResultData.orderType == Order.OrderType.MarketIncrease) {
      ...
      (uint256 _leverage, ) = abi.decode(callbackdata, (uint256, bool));
      // beenLong = orderResultData.isLong;
      ...
      if (flow == FLOW.WITHDRAW) {
        _distribute(orderResultData.outputAmount, sizeInUsd == 0);
      } else {
      ...
      if (flow == FLOW.DEPOSIT) {
        ...
        _distribute(outputAmount + swapProgressData.swapped, false);
      } else {      // same as if (flow == FLOW.SIGNAL_CHANGE)
    ...
    // think of emitting event that shows current leverage value
  }

  function afterOrderCancellation(bytes32 key, Order.OrderType orderType, bool, bytes memory /* callbackdata */) external nonReentrant {
    require(msg.sender == address(gmxUtils), "invalid caller");
    _gmxLock = false;
  ...
  // check this function in mainnet
  ...
  function getExecutionFee() public view returns (uint256 minExecutionFee) {
    if (positionIsClosed) {
      minExecutionFee = callbackGasLimit * tx.gasprice;
    } else {
      minExecutionFee = gmxUtils.getExecutionGasLimit(callbackGasLimit) * tx.gasprice;
    }
  }
  ...
  function setMinCollateralAmount(uint256 _minCollateralAmount) external onlyOwner {
    minCollateralAmount = _minCollateralAmount;
  }

  function setMaxCollateralAmount(uint256 _maxCollateralAmount) external onlyOwner {
  ...
  function _payExecutionFee() internal {
    uint256 minExecutionFee = getExecutionFee();
  ...
  function _isFundIdle() internal view returns (bool) {
    ...
    if (balance > minCollateralAmount) {
      return true;
    } else {
      return false;
    }
  ...
  function _createDecreasePosition(
    ...
    if (
      sizeDeltaInUsd == 0 ||
      (sizeInUsd - sizeDeltaInUsd) * BASIS_POINTS_DIVISOR / leverage < minCollateralAmount * marketPrices.shortTokenPrice.min
    ) {
   ...
   *  it could contain only uniswap swap data, gmx swap data or both of them
   *  but in the case of containing both swap data, first element of array should be always gmx.
   * 
  ...
  function _runSwap(bytes[] memory _swapData, bool isCollateralToIndex) internal returns (bool completed) {
    ...
    if (_swapData.length == 2) {
      ...
      require(_protocol == PROTOCOL.UNISWAP, "invalid swap data");
      swapProgressData.swapped = swapProgressData.swapped + _doLifiSwap(data, isCollateralToIndex);
      ...
      if (_protocol == PROTOCOL.UNISWAP) {
        uint256 outputAmount = _doLifiSwap(data, isCollateralToIndex);
  ...
  function _doLifiSwap(bytes memory data, bool isCollateralToIndex) internal returns (uint256 outputAmount) {
    (address to, uint256 amount, bytes memory callData) = abi.decode(
      data,
      (address, uint256, bytes)
    );
    
    _validateLifiCallData(to, callData);
    ...
    inputToken.safeApprove(to, amount);
    (bool success, ) = to.call(callData);
    require(success, "lifiswap call reverted");
    outputAmount = IERC20(outputToken).balanceOf(address(this)) - balBefore;
  }

  function _validateLifiCallData(address to, bytes memory callData) internal view {
    require(to == address(0x1231DEB6f5749EF6cE6943a275A1D3E7486F4EaE), "invalid lifi callee");
    address receiver;
    assembly {
      receiver := mload(add(callData, 112))
    }
    require(receiver == address(this), "invalid lifi calldata");
  }
  ...
  function _withdraw() internal {
    ...
    require(sumOfShares > 0, "zero value");
    
    if (positionIsClosed == true) {
      _distribute(0, true);
    } else if (_isLongOneLeverage(beenLong, leverage)) {  // beenLong && leverage == BASIS_POINTS_DIVISOR
      uint256 swapAmount = IERC20(indexToken).balanceOf(address(this)) * sumOfShares / totalShares;
      nextAction.selector = NextActionSelector.SWAP_ACTION;
      // abi.encode(swapAmount, swapDirection): if swap direction is true, swap collateralToken to indexToken
      ...
      uint256 collateralDeltaAmount = positionData.collateralAmount * sumOfShares / totalShares;
      uint256 sizeDeltaInUsd = positionData.sizeInUsd * sumOfShares / totalShares;
   ...
   * @param positionClosed true if position is position is closed completely by withdrawing all funds, or false
   */
  function _distribute(uint256 withdrawn, bool positionClosed) internal {
    uint256 sumOfShares;
    for (uint8 i = 0; i < pendingWithdrawalList.length; ) {
      uint256 depositId = pendingWithdrawalList[i];
      sumOfShares = sumOfShares + depositInfo[depositId].shares;
    ...
    if (positionClosed) {
      distributionAmount = collateralToken.balanceOf(address(this)) * sumOfShares / totalShares;
    } else {
      distributionAmount = withdrawn + collateralToken.balanceOf(address(this)) * sumOfShares / totalShares;
    }
    if (distributionAmount == 0) return;
    for (uint8 i = 0; i < pendingWithdrawalList.length; ) {
      uint256 depositId = pendingWithdrawalList[i];
      uint256 amount = distributionAmount * depositInfo[depositId].shares / sumOfShares;
      if (amount > 0) {
  ...
  function _transferToken(uint256 depositId, uint256 amount) internal {
    ...
    totalCollateralAmount -= depositInfo[depositId].amount;
  }
