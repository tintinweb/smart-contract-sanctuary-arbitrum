...
contract IncentivesHelper is OwnableUpgradeable, UUPSUpgradeable {
    ...
    function getVolUsdPrice() public view virtual returns (uint256) {
    ...
    function setRewardsVault(address _rewardsVault) public onlyOwner {
        incentivesController.setRewardsVault(_rewardsVault);
    }
...
interface IIncentivesController {
    ...
    function assets(address)
        external
        view
        returns (uint104 emissionPerSecond, uint104 index, uint40 lastUpdateTimestamp);
    function claimRewards(address[] memory assets, uint256 amount, address to) external returns (uint256);
    function claimRewardsOnBehalf(address[] memory assets, uint256 amount, address user, address to)
        external
        returns (uint256);
