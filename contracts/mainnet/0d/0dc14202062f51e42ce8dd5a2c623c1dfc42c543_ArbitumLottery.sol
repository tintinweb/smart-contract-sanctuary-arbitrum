...
contract ArbitumLottery is ReentrancyGuard{
    ...
    address private buybackwallet = 0xE855Df7f41Cf8e3ed715C3F0117661a592e19C66;
    address private dev = 0xE855Df7f41Cf8e3ed715C3F0117661a592e19C66;
...
uint256 private maxParticipantNumbers1;
    ...
    constructor()  {  
        ...
        ticketPrice = 10 ether ;
         ...
         maxParticipantNumbers1 = 4;
        ticketPrice1 = 25 ether;

         maxParticipantNumbers2 = 2;
        ticketPrice2 = 50 ether;

        tokenAdress = 0x912CE59144191C1204E64559FE8253a0e49E6548; 
        TokenInterface = IERC20(tokenAdress);
    }
   ...
   function announceLottery() public onlyOwner(){
    ...
    function joinLottery(uint256 _amount) external notOwner() noReentrant{
        ...
        //TokenInterface.transferFrom(msg.sender,address(this),_amount);
          bool chk= TokenInterface.transferFrom(msg.sender,address(this),_amount);
       if(chk){
...
for (uint i=0; i<arrayLength2; i++) {
  ...
  if (participants[i]==ad){
    ...
    function pickwinner() internal {
        ...
        uint256 BuyBackFee = SafeMath.div(SafeMath.mul(contractBalance,0),100); //buyback fee
        uint256 devFee = SafeMath.div(SafeMath.mul(contractBalance,20),100); //set dev fee
      
        TokenInterface.transfer(buybackwallet,BuyBackFee);
        uint256 winnerAmount = SafeMath.sub(contractBalance,BuyBackFee);
        winnerAmount -= devFee;
  
       TokenInterface.transfer(dev,devFee);
        TokenInterface.transfer(participants[win],winnerAmount);
    ...
    function pickwinner1() internal {
        ...
        uint256 BuyBackFee = SafeMath.div(SafeMath.mul(contractBalance,0),100);
         uint256 devFee = SafeMath.div(SafeMath.mul(contractBalance,20),100);
     ...
     TokenInterface.transfer(participants1[win],winnerAmount);
    ...
    function pickwinner2() internal {
      ...
      TokenInterface.transfer(participants2[win],winnerAmount);
