...
contract BattleFrens is Ownable, ReentrancyGuard {
    FrensArmy frensNFT;
    address public NFA_ERC721 = 0x54cfe852BEc4FA9E431Ec4aE762C33a6dCfcd179;
    ...
    event Fren1joined(address indexed fren1, uint256 indexed battleid);
    event Fren2joined(address indexed fren2, uint256 indexed battleid);
    event BattleFinished(address indexed winner, uint256 indexed battleid);

    constructor() {
        frensNFT = FrensArmy(NFA_ERC721);
    }

    function setAddress(address _Fren_NFT, address _Fren_Token) public onlyOwner {
        NFA_ERC721 = _Fren_NFT;
        NFA_ERC20 = _Fren_Token;
        frensNFT = FrensArmy(NFA_ERC721);
    }

    function fren1join(uint256 _battleid, uint256 _amountNFA) public {
        ...
        frenBets[_battleid][msg.sender] = _amountNFA;
        IFREN(NFA_ERC20).transferFrom(msg.sender, address(this), _amountNFA);
        emit Fren1joined(fren1[_battleid], _battleid);
    }

    function fren2join(uint256 _battleid, uint256 _amountNFA) public {
        uint256 amount = _amountNFA * decimals;
        require(!btl_HasStarted[_battleid], "Battle already Begun");
        ...
        emit Fren2joined(fren2[_battleid], _battleid);
    }
    ...
    function startbattle(uint256 _battleid) public {
        ...
        frenRewardDebt[WinnerFren[_battleid]] =
            frenBets[_battleid][WinnerFren[_battleid]] +
            ((frenBets[_battleid][LoserFren[_battleid]] * 2) / 3);
        address loser = LoserFren[_battleid];
        IFREN(NFA_ERC20).transfer(fren_grave, ((frenBets[_battleid][loser] * 2) / 3));

        emit BattleFinished(WinnerFren[_battleid], _battleid);
    }

    function vitalityCalculator(address _user) public view returns (uint256) {
        uint256 vitality = 0;
        uint256 multiplier = 10 ** 18;
        uint256 nfa_erc20_balance = IFREN(NFA_ERC20).balanceOf(tx.origin);
        uint256 nfa_erc721_balance = IFREN(NFA_ERC721).balanceOf(tx.origin);
        uint256 gmAmount = frensNFT.user_GM(_user);
        uint256 userPoints = gmAmount * ((7 * nfa_erc721_balance) / 10 + (((3 * nfa_erc20_balance) / 10) * multiplier));
        if (userPoints > 0) {
            vitality = (ln(1) - ln(userPoints));
        } else {
            vitality = 1;
        }
