...
interface IToobFinanceExecutor {
...
contract ToobFinanceRouter is Ownable {
    ...
    function _swap(
        ...
        IToobFinanceExecutor(executor).executePath{value: msg.value}(
            pathDefinition,
            amountsIn,
            msg.sender
        );
        ...
        if (referralCode > REFERRAL_WITH_FEE_THRESHOLD) {
            ...
            _universalTransfer(
                ...
                (amountOut * thisReferralInfo.referralFee * 8) /
                    (FEE_DENOM * 10)
            );
            amountOut =
                (amountOut * (FEE_DENOM - thisReferralInfo.referralFee)) /
                FEE_DENOM;
        }
        ...
        returns (uint256[] memory amountsOut)
    {
        ...
        uint256 pos = 6;
        {
            ...
            assembly {
                ...
                valueOutMin := shr(
                    mul(sub(32, outputMinAmountLength), 8),
                    calldataload(pos)
                )
                ...
                for {
                    ...
                    mstore(
                        add(memPos, 0x20),
                        shr(
                            mul(sub(32, outputAmountLength), 8),
                            calldataload(pos)
                        )
                    )
            ...
            for (uint256 i = 0; i < outputs.length; i++) {
                ...
                if (referralCode > REFERRAL_WITH_FEE_THRESHOLD) {
                    _universalTransfer(
                        ...
                        thisReferralInfo.beneficiary,
                        (amountsOut[i] * thisReferralInfo.referralFee * 8) /
                            (FEE_DENOM * 10)
                    );
                    amountsOut[i] =
                        (amountsOut[i] *
                            (FEE_DENOM - thisReferralInfo.referralFee)) /
                        FEE_DENOM;
                }
                _universalTransfer(
                    outputs[i].tokenAddress,
    ...
    function swapRouterFunds(
        ...
        IToobFinanceExecutor(executor).executePath{value: 0}(
            pathDefinition,
            amountsIn,
            msg.sender
        );
