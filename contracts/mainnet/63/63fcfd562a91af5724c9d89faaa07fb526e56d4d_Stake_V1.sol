...
contract Stake_V1 is Ownable, ERC721Holder {
    ...
    */
    ...
    function stakeAll() public {
        uint256[] memory ids = farm_token.tokenids(msg.sender);
        stake(ids);
    }


    // stake tokens to Farm for ERC20 allocation.
    function stake(uint256[] memory _tokenIDs) public Open {
        ...
        for (uint i = 0; i < _tokenIDs.length; i++) {
            // require(depositInfo[_tokenIDs[i]] == address(0x0), "already stake");
    ...
    function unStake() public {
        ...
        for (uint i = 0; i < _amount; i++) {
            ...
            farm_token.safeTransferFrom(address(this), address(msg.sender), user.tokenids[i]);
