...
import "../interfaces/MMTStakingInterfaceV6_2.sol";
...
contract MMTTradingCallbacks is Initializable {
    ...
    MMTPairInfosInterfaceV6 public pairInfos;
    MMTReferralsInterfaceV6_2 public referrals;
    MMTStakingInterfaceV6_2 public staking;
    ...
    uint256 public lpFeeP; // % of closing fee going to MMT/DAI LPs (eg. 20)
    uint256 public sssFeeP; // % of closing fee going to MMT staking (eg. 40)
    ...
    function initialize(
        ...
        MMTStakingInterfaceV6_2 _staking,
    ...
    function openTradeMarketCallback(
        ...
        if (
            ...
            emit CancelReason(
                a.price,
                storageT.openInterestDai(t.pairIndex, t.buy ? 0 : 1),
                o.slippageP,
                o.wantedPrice,
                priceImpactP,
                priceAfterImpact,
    ...
    function registerTrade(
        ...
        if (nftId < 1500) {
            ...
            // Convert NFT bot fee from DAI to token value
            //old formular for convert dai to mmt token
    ...
    function unregisterTrade(
        ...
        uint256 daiKeepInStorage = !marketOrder ? nftFeeDai : 0;
        if (trade.positionSizeDai > 0) {
            ...
            if (daiSentToTrader > daiLeftInStorage) {
                storageT.vault().sendAssets(
                    daiSentToTrader - daiLeftInStorage + daiKeepInStorage,
                    trade.trader
                );
                storageT.transferDai(
                    address(storageT),
                    trade.trader,
                    daiLeftInStorage - daiKeepInStorage
                );
            } else {
                sendToVault(
                    daiLeftInStorage >= daiSentToTrader + daiKeepInStorage
                        ? daiLeftInStorage - daiSentToTrader - daiKeepInStorage
...
interface MMTStakingInterfaceV6_2 {
