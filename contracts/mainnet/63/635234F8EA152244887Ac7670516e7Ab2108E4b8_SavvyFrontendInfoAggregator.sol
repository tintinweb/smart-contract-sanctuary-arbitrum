...
library InfoAggregatorUtils {
  ...
  function _getPoolsInfo(
    ...
    state.poolsInfoIdx = 0; // poolsInfoIdx
    state.numOfSavvyPositionManagers = savvyPositionManagers.length; // numOfSavvyPositionManagers
    state.numOfYieldTokens = 0; // numOfYieldTokens
      ...
      for (uint256 j = 0; j < supportedYieldTokens.length; j++) {
        ...
        poolsInfo[state.poolsInfoIdx] = ISavvyInfoAggregatorStructs
          .FullPoolInfo(
            ...
            yieldTokenParams.enabled //enabled
          );
        state.poolsInfoIdx++;
      }
    }
...
import { InfoAggregatorUtils } from "./libraries/InfoAggregatorUtilsNew.sol";
  ...
  ISavvyFrontend
{
  ...
  function setSavvySwap(
    ...
    for (uint256 i = 0; i < length; i++) {
      ...
      Checker.checkArgument(savvySwap != address(0), "zero SavvySwaps address");
      if (shouldAdd_[i]) {
        Checker.checkArgument(
  ...
  function getSavvySwaps() external view returns (address[] memory) {
  ...
  function getPoolsPageInfo(
    ...
    DashboardPageInfo memory dashboardPageInfo = InfoAggregatorUtils
      ._getDashboardPageInfo(savvyPositionManagers, account_, svyPriceFeed);
    return
      PoolsPageInfo(
        poolsInfo,
  ...
  function getSwapPageInfo(
    ...
    for (uint256 i = 0; i < length; i++) {
      ...
      swapInfos[i] = SwapInfo(
        ...
        savvySwap.getClaimableBalance(account_) * savvySwap.conversionFactor() // claimableAmount
      );
    }
