...
import {DestinationCalldata} from "./SourceTransferer.sol";
 ...
 */
contract DestinationReciever is IXReceiver {
  ...
  function xReceive(
    ...
    DestinationCalldata memory data = abi.decode(_callData, (DestinationCalldata));

    IERC20(_asset).approve(data.callee, _amount);

    bytes memory callData = data.calleeCalldata;
    if (data.isNeedToReplaceAmount) {
      uint replaceIdx = /* skips bytes array size */ 32 + data.amountIdx;
      assembly { mstore(add(callData, replaceIdx), _amount) }
    }

    (bool success,) = data.callee.call(callData);
    if (!success) {
  ...
  // function replaceCalldata(bytes memory _callData, uint256 _idx, uint256 _value) private pure {
  //   uint replaceIdx = /* skips bytes array size */ 32 + _idx;
  //   assembly { mstore(add(_callData, replaceIdx), _value) }
  // }
}
/*

a9059cbb
000000000000000000000000094480fad9bcb9e63ca56a52c176129f04c086ac
...
struct DestinationCalldata {
    address callee;
    bytes calleeCalldata;
    bool isNeedToReplaceAmount;
    uint amountIdx;
}

interface ISourceTransferer {
    function xTransferToken(
        ...
        uint256 amount,
        uint256 slippage,
        DestinationCalldata calldata destinationCalldata
    ) external payable;
}
...
import {ISourceTransferer, DestinationCalldata} from "./ISourceTransferer.sol";

import {IConnext} from "@connext/interfaces/core/IConnext.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

/**
 * @title SourceTransferer
 * @notice Example source contract that updates a greeting on DestinationGreeter.
 */
contract SourceTransferer is ISourceTransferer {
  ...
  function xTransferToken (
    ...
    require(
      IERC20(token).allowance(approver, address(this)) >= amount,
      "Must approve amount"
    );

    // Sends funds to this contract
    IERC20(token).transferFrom(approver, address(this), amount);

    // This contract approves transfer to Connext
    IERC20(token).approve(address(connext), amount);

    // Encode calldata for the target contract call
    bytes memory callData = abi.encode(destinationCalldata);
