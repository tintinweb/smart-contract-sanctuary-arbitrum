...
library UniswapV2Library {
    ...
    function pairFor(
        ...
        return factoryContract.getPair(tokenA, tokenB);
    }
...
contract SpartaDexRouter is IUniswapV2Router02 {
    ...
    constructor(address _factory, IWNative _wNative) {
        factory = _factory;
        wNative = _wNative;
    }
        ...
        returns (uint amountToken, uint amountETH, uint liquidity)
    {
        ...
        wNative.deposit{value: amountETH}();
        assert(wNative.transfer(pair, amountETH));
        liquidity = IUniswapV2Pair(pair).mint(to);
    ...
    {
        if (path[path.length - 1] != address(wNative)) {
            revert InvalidPath();
        }
        ...
        wNative.withdraw(amounts[amounts.length - 1]);
        ...
        returns (uint[] memory amounts)
    {
        ...
        wNative.deposit{value: amounts[0]}();
        assert(
            wNative.transfer(
                UniswapV2Library.pairFor(factory, path[0], path[1]),
                amounts[0]
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        uint amountOut = wNative.balanceOf(address(this));
