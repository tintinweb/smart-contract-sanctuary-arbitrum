...
contract ERC20DAO is ERC20Upgradeable, AccessControl, IERC721Receiver, ReentrancyGuard, Helper {
    ...
    function getERC20DAOdetails() external view returns (ERC20DAOdetails memory) {
    ...
    function transferNft(address _nft, address _to, uint256 _tokenId) external onlyCurrentContract {
    ...
    function mintGTToAddress(uint256[] memory _amountArray, address[] memory _userAddress)
        external
        onlyCurrentContract
    {
        if (_amountArray.length != _userAddress.length) {
            revert ArrayLengthMismatch(_amountArray.length, _userAddress.length);
        }
        ...
        for (uint256 i; i < leng;) {
            ...
            Emitter(emitterContractAddress).newUser(
                ...
                block.timestamp,
                _amountArray[i],
                Safe(Factory(factoryAddress).getDAOdetails(address(this)).gnosisAddress).isOwner(_userAddress[i])
            );
        ...
        Emitter(emitterContractAddress).mintGTToAddress(address(this), _amountArray, _userAddress);
    }
    ...
    function updateGovernanceSettings(uint256 _quorum, uint256 _threshold) external onlyCurrentContract {
        ...
        Emitter(emitterContractAddress).updateGovernanceSettings(address(this), _quorum, _threshold);
    }
    ...
    function toggleOnlyAllowWhitelist() external payable onlyCurrentContract {
        erc20DaoDetails.onlyAllowWhitelist = !erc20DaoDetails.onlyAllowWhitelist;
    }

    /// @dev Function to change governance active
    /// @param _isGovernanceActive New governance active status
    function updateGovernanceActive(bool _isGovernanceActive) external payable onlyCurrentContract {
    ...
    function updateTokenTransferability(bool _isTokenTransferable) external payable onlyCurrentContract {
        erc20DaoDetails.isTransferable = _isTokenTransferable;

        Emitter(emitterContractAddress).updateTokenTransferability(address(this), _isTokenTransferable);
    }
    ...
    function mintToken(address to, uint256 amount, bytes32[] calldata _merkleProof) public {
        if (erc20DaoDetails.onlyAllowWhitelist) {
            require(
                MerkleProof.verify(
                    _merkleProof,
                    Factory(factoryAddress).getDAOdetails(address(this)).merkleRoot,
                    keccak256(abi.encodePacked(to))
                ),
                "Incorrect proof"
            );
        }
        require(msg.sender == factoryAddress || msg.sender == address(this), "Only factory or dao");
        _mint(to, amount);
    }
    ...
    function burn(address account, uint256 amount) external onlyRole(RefundModule) {
        _burn(account, amount);
    }

    function grantRefundModule(address _refundModule) external payable onlyCurrentContract {
...
contract ERC721DAO is IERC721Receiver, ERC721Upgradeable, AccessControl, ERC2981, Helper {
    using SafeERC20 for IERC20;
    ...
    /// @dev This function returns details of a particular dao
    function getERC721DAOdetails() external view returns (ERC721DAOdetails memory) {
    ...
    function mintNft(address _to, string memory _tokenURI, bytes32[] calldata _merkleProof)
        public
        onlyFactory(factoryAddress)
    {
        ...
        if (!erc721DaoDetails.isNftTotalSupplyUnlimited) {
            require(
                Factory(factoryAddress).getDAOdetails(address(this)).distributionAmount >= _tokenIdTracker,
                "Max supply reached"
            );
        }
        ...
        Emitter(emitterAddress).mintNft(_to, address(this), _tokenURI, _tokenIdTracker);
    }
    ...
    /// @param _userAddress array of address where the amount should be transferred
    function mintGTToAddress(uint256[] memory _amountArray, string[] memory _tokenURI, address[] memory _userAddress)
        external
        onlyCurrentContract
    {
        ...
        for (uint256 i; i < length;) {
            for (uint256 j; j < _amountArray[i];) {
                if (balanceOf(_userAddress[i]) == erc721DaoDetails.maxTokensPerUser) {
        ...
        Emitter(emitterAddress).mintGTToAddress(address(this), _amountArray, _userAddress);
    }
    ...
    function updateGovernanceSettings(uint256 _quorum, uint256 _threshold) external onlyCurrentContract {
        ...
        Emitter(emitterAddress).updateGovernanceSettings(address(this), _quorum, _threshold);
    }

    /// @dev Function to set whitelist to true for a particular token contract
    function toggleOnlyAllowWhitelist() external payable onlyCurrentContract {
        erc721DaoDetails.onlyAllowWhitelist = !erc721DaoDetails.onlyAllowWhitelist;
    }
    ...
    function updateTokenTransferability(bool _isNftTransferable) external payable onlyCurrentContract {
        erc721DaoDetails.isTransferable = _isNftTransferable;

        Emitter(emitterAddress).updateTokenTransferability(address(this), _isNftTransferable);
    }

    function updateMaxTokensPerUser(uint256 _maxTokensPerUser) external payable onlyCurrentContract {
        erc721DaoDetails.maxTokensPerUser = _maxTokensPerUser;
        Emitter(emitterAddress).updateMaxTokensPerUser(address(this), _maxTokensPerUser);
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override(ERC721Upgradeable) {
        require(balanceOf(to) <= erc721DaoDetails.maxTokensPerUser, "Max amount reached");
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: caller is not token owner or approved");
    ...
    function setRoyalty(address _receiver, uint96 _royaltyFeesInBips) public onlyCurrentContract {
...
contract Factory is Helper {
    ...
    address private constant wrappedTokenAddress = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    ...
    function getDAOdetails(address _daoAddress) public view returns (DAODetails memory) {
        return daoDetails[_daoAddress];
    }

    /// @dev This function returns token gating details of a particular dao
    /// @param _daoAddress address of token contract
    function getTokenGatingDetails(address _daoAddress) external view returns (TokenGatingCondition[] memory) {
    ...
    function changeMerkleRoot(address _daoAddress, bytes32 _newMerkleRoot)
        external
        payable
        onlyGnosisOrDao(address(this), _daoAddress)
    {
        ...
        Emitter(emitterAddress).changeMerkleRoot(address(this), _daoAddress, _newMerkleRoot);
    }
    ...
    function createERC20DAO(
        ...
        if (!(_ownerFeePerDepositPercent < FLOAT_HANDLER_TEN_4)) {
            revert AmountInvalid("_ownerFeePerDeposit", _ownerFeePerDepositPercent);
        }
        ...
        address _safe = Safe(safe).createProxyWithNonce(singleton, _initializer, block.timestamp);
        ...
        address _daoAddress = address(new ProxyContract(ERC20Implementation, _owner, data));
        ...
        for (uint256 i; i < _admins.length;) {
            Emitter(emitterAddress).newUser(_daoAddress, _admins[i], _depositTokenAddress, 0, block.timestamp, 0, true);
    ...
    function createERC721DAO(
        ...
        address _daoAddress = address(new ProxyContract(ERC721Implementation, _owner, data));
        ...
        onlyGnosisOrDao(address(this), _daoAddress)
    {
        ...
        Emitter(emitterAddress).updateMinMaxDeposit(_daoAddress, _minDepositPerUser, _maxDepositPerUser);
    }

    /// @dev Function to update DAO Owner Fee
    /// @param _ownerFeePerDeposit New Owner fee
    /// @param _daoAddress address of the token contract
    function updateOwnerFee(uint256 _ownerFeePerDeposit, address _daoAddress)
        external
        payable
        onlyAdmins(daoDetails[_daoAddress].gnosisAddress)
    {
        if (!daoDetails[_daoAddress].isDeployedByFactory) {
            revert AddressInvalid("_daoAddress", _daoAddress);
        }
        ...
        Emitter(emitterAddress).updateOwnerFee(_daoAddress, _ownerFeePerDeposit);
    }
    ...
    /// @param _daoAddress address of the token contract
    function updateTotalRaiseAmount(uint256 _newDistributionAmount, uint256 _newPricePerToken, address _daoAddress)
        external
        payable
        onlyGnosisOrDao(address(this), _daoAddress)
    {
        ...
        uint256 _distributionAmount = daoDetails[_daoAddress].distributionAmount;
        ...
        if (_distributionAmount != _newDistributionAmount) {
            ...
            Emitter(emitterAddress).updateDistributionAmount(_daoAddress, _newDistributionAmount);
        }

        if (daoDetails[_daoAddress].pricePerToken != _newPricePerToken) {
            daoDetails[_daoAddress].pricePerToken = _newPricePerToken;
            Emitter(emitterAddress).updatePricePerToken(_daoAddress, _newPricePerToken);
        }
    }
    ...
    /// @param _daoAddress address of the token contract
    function updateDepositTime(uint256 _depositTime, address _daoAddress)
        external
        payable
        onlyAdmins(daoDetails[_daoAddress].gnosisAddress)
    {
        ...
        Emitter(emitterAddress).startDeposit(_daoAddress, block.timestamp, daoDetails[_daoAddress].depositCloseTime);
    }
    ...
    function setupTokenGating(
        ...
        TokenGatingCondition memory _tokenGatingCondition =
            TokenGatingCondition(_tokenA, _tokenB, _operator, _comparator, _value);
    ...
    /// @param _daoAddress address of the token contract
    function disableTokenGating(address _daoAddress)
        external
        payable
        onlyAdmins(daoDetails[_daoAddress].gnosisAddress)
    {
        uint256 length = tokenGatingDetails[_daoAddress].length;
        for (uint256 i; i < length;) {
            tokenGatingDetails[_daoAddress].pop();
            unchecked {
                ++i;
            }
        }
    ...
    function buyGovernanceTokenERC20DAO(
        ...
        uint256 _totalAmount = (_numOfTokensToBuy * daoDetails[_daoAddress].pricePerToken) / 1e18;
        ...
        if (_totalDeposit == 0) {
            ...
            if (_totalDeposit + _totalAmount > daoDetails[_daoAddress].maxDepositPerUser) {
                revert AmountInvalid("_numOfTokensToBuy", _numOfTokensToBuy);
            }
        }
        ...
        if (daoBalance > (daoDetails[_daoAddress].pricePerToken * daoDetails[_daoAddress].distributionAmount) / 1e18) {
            revert AmountInvalid("daoBalance", daoBalance);
        }

        uint256 ownerShare = (_totalAmount * daoDetails[_daoAddress].ownerFeePerDepositPercent) / (FLOAT_HANDLER_TEN_4);
    ...
    function ifTokenGatingApplied(address _daoAddress) private view {
        TokenGatingCondition[] memory conditions = tokenGatingDetails[_daoAddress];

        for (uint256 i; i < conditions.length;) {
            address _tokenA = conditions[i].tokenA;
            address _tokenB = conditions[i].tokenB;
            uint256 _valueA = conditions[i].value[0];
            uint256 _valueB = conditions[i].value[1];
            uint256 _balanceA = IERC20(_tokenA).balanceOf(msg.sender);
            uint256 _balanceB = IERC20(_tokenB).balanceOf(msg.sender);
    ...
    function buyGovernanceTokenERC721DAO(
        ...
        uint256 _totalAmount = daoDetails[_daoAddress].pricePerToken * (_numOfTokensToBuy);
        ...
        if (daoDetails[_daoAddress].depositTokenAddress == NATIVE_TOKEN_ADDRESS) {
            require(msg.value == _totalAmount, "Insufficient value");
            payable(ERC721DAO(_daoAddress).getERC721DAOdetails().ownerAddress).call{value: ownerShare}("");
            payable(daoDetails[_daoAddress].assetsStoredOnGnosis ? daoDetails[_daoAddress].gnosisAddress : _daoAddress)
                .call{value: userShare}("");
        } else {
            IERC20(daoDetails[_daoAddress].depositTokenAddress).safeTransferFrom(
                msg.sender,
                daoDetails[_daoAddress].assetsStoredOnGnosis ? daoDetails[_daoAddress].gnosisAddress : _daoAddress,
                userShare
            );

            IERC20(daoDetails[_daoAddress].depositTokenAddress).safeTransferFrom(
                msg.sender, ERC721DAO(_daoAddress).getERC721DAOdetails().ownerAddress, ownerShare
            );
        }
    ...
    function checkFeeAndMintTokens(
        ...
        ERC20DAOdetails memory _details = ERC20DAO(_daoAddress).getERC20DAOdetails();

        if (daoDetails[_daoAddress].ownerFeePerDepositPercent > 0) {
            if (daoDetails[_daoAddress].depositTokenAddress == NATIVE_TOKEN_ADDRESS) {
                require(msg.value == _totalAmount, "Insufficient value");
                payable(_details.ownerAddress).call{value: ownerShare}("");
                payable(
                    daoDetails[_daoAddress].assetsStoredOnGnosis ? daoDetails[_daoAddress].gnosisAddress : _daoAddress
                ).call{value: userShare}("");
            } else {
                ...
                IERC20(_depositTokenAddress).safeTransferFrom(msg.sender, _details.ownerAddress, ownerShare);
            }
            ...
            Emitter(emitterAddress).newUser(
                _daoAddress, msg.sender, _depositTokenAddress, _totalAmount, block.timestamp, userGtTokens, false
            );
        } else {
            if (daoDetails[_daoAddress].depositTokenAddress == NATIVE_TOKEN_ADDRESS) {
                require(msg.value == _totalAmount, "Insufficient value");
...
contract Helper {
    ...
    error DepositAmountInvalid(uint256 _maxDepositPerUser, uint256 _minDepositPerUser);
    error DepositClosed();
    ...
    error RaiseAmountInvalid(uint256 _totalRaiseAmount, uint256 _maxDepositPerUser);
    ...
    modifier onlyGnosis(address _factory, address _daoAddress) {
        require(Factory(_factory).getDAOdetails(_daoAddress).gnosisAddress == msg.sender, "Only Gnosis");
        _;
    }

    modifier onlyGnosisOrDao(address _factory, address _daoAddress) {
        require(
            Factory(_factory).getDAOdetails(_daoAddress).gnosisAddress == msg.sender || _daoAddress == msg.sender,
            "Only Gnosis or Dao"
        );
        _;
    }
    ...
    modifier onlyFactoryDeployed(address _factory) {
        require(Factory(_factory).getDAOdetails(address(this)).isDeployedByFactory);
        _;
    }
...
contract Airdrop is ReentrancyGuard, Helper {
    ...
    function updateEmitterAddress(address _emitterContractAddress) external onlyOwner {
        if (address(0) == _emitterContractAddress) {
            revert AddressInvalid("_emitterContractAddress", _emitterContractAddress);
        }
        emitterContractAddress = _emitterContractAddress;
    }
        ...
        external
    {
        ...
        uint256 allowance = IERC20(_airdropTokenAddress).allowance(msg.sender, address(this));
        ...
        for (uint256 i = 0; i < totalMembers;) {
            IERC20(_airdropTokenAddress).safeTransferFrom(msg.sender, _members[i], _airdropAmountArray[i]);
            Emitter(emitterContractAddress).airDropToken(
                msg.sender, _airdropTokenAddress, _members[i], _airdropAmountArray[i]
            );
            unchecked {
                ++i;
            }
