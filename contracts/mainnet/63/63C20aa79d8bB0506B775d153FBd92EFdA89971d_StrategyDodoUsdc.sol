...
// File contracts/interfaces/dodoproxy.sol
...
abstract contract StrategyDodoBase is StrategyBase {
    ...
    function harvest() public override onlyBenevolent {
        ...
        uint256 _base = IERC20(wantBase).balanceOf(address(this));

        IERC20(wantBase).safeApprove(dodo_approve, 0);
        IERC20(wantBase).safeApprove(dodo_approve, _base);

        if(wantBase == usdt) {
            IDodoProxy(dodo_proxy).addLiquidityToV1(usdc_usdt, _base, 0, 0, 0, 0, block.timestamp.add(60));
        }else if (wantBase == usdc) {
            IDodoProxy(dodo_proxy).addLiquidityToV1(usdc_usdt, 0, _base, 0, 0, 0, block.timestamp.add(60));
        }
