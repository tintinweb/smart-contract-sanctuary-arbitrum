...
// File contracts/AiMoonDoge.sol

pragma solidity ^0.8.15;

contract AiMoonDoge is Ownable {
    ...
    uint256 public constant MAX_TOKEN = 3_500_000_000 * 1e18;

    uint256 public constant INIT_CLAIM = 21700 * 1e18;

    uint256 public constant MAX_REFER_TOKEN = 147_694_482 * 1e18;
    ...
    function claim(bytes calldata signature, address referrer) public {
        require(state, "MoonDoge: airdrop has ended");

        require(
            _claimedUser[_msgSender()] == false,
            "MoonDoge: already claimed"
        );

        _claimedUser[_msgSender()] = true;
        require(isValidSignature(signature), "MoonDoge: only auth claims");

        uint256 supplyPerAddress = canClaimAmount();
        require(supplyPerAddress >= 1e18, "MoonDoge: airdrop has ended");
        ...
        emit Claim(_msgSender(), 0, amount, referrer, block.timestamp);
    }
    ...
    function toggleState(bool _state) public onlyOwner {
        state = _state;
    }
    ...
    function isValidSignature(
        ...
        bytes32 data = keccak256(abi.encodePacked(_msgSender(), address(this)));
