...
error MoreMsgvalue();
error ThetaV2MultiFailed();

// SPDX-License-Identifier: MIT
pragma solidity 0.8.17;

import {LibDiamond} from "../libraries/LibDiamond.sol";
import {ThetaV2MultiFailed, NoAccessFacet} from "../Errors/GenericErrors.sol";
import "../interfaces/IThetaV2relayFacet.sol";
import "hardhat/console.sol";

contract ThetaV2relayFacet {
    //bytes4(keccak256("relaySwapRouter(((address,address,(address,uint256)[],(address)[],(address,uint256)[],bytes,address,bytes),address),(bytes,uint256,uint256,address,bytes32),bytes[],bytes)"));
    bytes4 private immutable relaySwapRouter_FS = 0x9dd54289;
    //bytes4(keccak256("receiveMessageAndRelayswap(bytes,bytes,((address,address,(address,uint256)[],(address)[],(address,uint256)[],bytes,address,bytes),address),(bytes,uint256,uint256,address,bytes32),bytes[],bytes)"))
    bytes4 private immutable receiveMessageAndRelayswap_FS = 0x62901e89;
    //bytes4(keccak256("receiveMessage(bytes,bytes,address,address,uint256,bytes)"));
    bytes4 private immutable receiveMessage_FS = 0x2ffe238c;

    function relaySwapMulti(ThetaValue[] memory thetas) external payable {
        require(thetas.length > 0, "No theta values provided.");
        if (thetas.length == 1) {
            if (!_handleDelegateCall(thetas[0])) revert ThetaV2MultiFailed();
        } else {
            for (uint256 i = 0; i < thetas.length; i++) {
                if (!_handleDelegateCall(thetas[i])) revert ThetaV2MultiFailed();
            }
        }
    }

    // Helper function to process delegate calls
    function _handleDelegateCall(ThetaValue memory theta) private returns (bool) {
        require(msg.sender == LibDiamond.contractOwner());

        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        address cctpFacet = ds.selectorToFacetAndPosition[receiveMessage_FS].facetAddress;
        address swapFacet = ds.selectorToFacetAndPosition[relaySwapRouter_FS].facetAddress;

        bytes4 calldataSignature = bytes4(theta.relayswapCalldata);

        address targetFacet = (calldataSignature == receiveMessage_FS || calldataSignature == receiveMessageAndRelayswap_FS)
            ? cctpFacet
            : (calldataSignature == relaySwapRouter_FS)
            ? swapFacet
            : address(0);

        if (targetFacet == address(0)) {
            revert NoAccessFacet();
        }

        (bool success, bytes memory result) = targetFacet.delegatecall(theta.relayswapCalldata);
        if (!success) {
    ...
    function _revertWithError(bytes memory result) private pure {
        if (result.length > 0) {
