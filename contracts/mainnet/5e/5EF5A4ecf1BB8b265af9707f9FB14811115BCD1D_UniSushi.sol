...
contract UniSushi is Ownable {
    ...
    function swap2(address _base, address _counter, uint256 _amountToSwap, uint _threshold) external {
        // balance validation
        uint balance = IERC20(_base).balanceOf(address(this));
        require(balance > _amountToSwap, "no execution: insufficent balance");
        ...
        TransferHelper.safeApprove(_base, address(SUSHISWAP_ROUTER_ADDRESS), _amountToSwap);

        uint256[] memory sushiResponse = sushiswapRouter.swapExactTokensForTokens(_amountToSwap, 0, pathInSecondLeg, address(this), block.timestamp);
        uint256 latestAmountWeHave = sushiResponse[1];
