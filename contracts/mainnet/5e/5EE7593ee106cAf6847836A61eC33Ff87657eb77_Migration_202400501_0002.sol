...
contract Migration_202400501_0002 is Migration { // solhint-disable-line contract-name-camelcase
  ...
  function migrate() internal override onlyAdmin {    
    MigrationLib.updateVaultMaxSupply(_registry, MigrationLib.scale(500_000, 6));
  }
}
...
library MigrationLib {
  ...
  function scale(uint256 amount, uint256 decimals) internal pure returns (uint256) {
    return amount * 10 ** decimals;
  }

  function updateMaxSupply(IAddressRegistry registry, address token, uint256 maxSupply) internal {
    IAssetStorage assetStorage = registry.getAssetStorage();
    IAssetStorage.Asset memory asset = assetStorage.getAsset(token, 0xFFFF);
  ...
  function updateVaultMaxSupply(IAddressRegistry registry, uint256 maxSupply) internal {
    IDepositorVault vault = registry.getDepositorVault();
    vault.setMaxSupply(maxSupply);
  }
}
