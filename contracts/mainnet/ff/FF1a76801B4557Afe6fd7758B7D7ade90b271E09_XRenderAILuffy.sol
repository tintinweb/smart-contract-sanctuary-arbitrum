...
contract XRenderAILuffy is ERC721, Ownable, Pausable {
    constructor() ERC721("XRender Luffy Collection", "XRENDER") Ownable() {}
    ...
    string private co1 = ", ";
    string private rl1 = '{"name": "XRenderLuffy #';
    string private rl3 = '"}';
    string private rl4 = "data:application/json;base64,";

    string private tr1 =
        '", "attributes": [{"trait_type": "hatStyle","value": "';
    ...
    function randomOne(uint256 tokenId) internal view returns (Arb memory) {
        ...
        uint256 seed = random(
            abi.encodePacked("XRenderAI", tokenId.toString())
        );
        Arb memory arb;
        ...
        arb.hatName = genHat(
            random(abi.encodePacked(ra1, (150000 - tokenId).toString()))
        )[2];

        uint256 randomValue = random(
            abi.encodePacked(rc1, (150000 - tokenId).toString())
        );
        if (randomValue % 9 == 0) {
        ...
        randomValue = random(
            abi.encodePacked(rd1, (150000 - tokenId).toString())
        );
        if (randomValue % 5 == 0) {
    ...
    function getTraits(Arb memory arb) internal view returns (string memory) {
        ...
        o = string(
            abi.encodePacked(
                o,
                tr4,
                uint256(arb.eyes).toString(),
                tr5,
                uint256(arb.clothes).toString()
            )
        );
        o = string(
            abi.encodePacked(
                o,
                tr6,
                uint256(arb.logoClothes).toString(),
                tr7,
                uint256(arb.trousers).toString()
            )
        );
        return
            string(
                abi.encodePacked(
                    o,
                    tr8,
                    uint256(arb.hatBg).toString(),
                    tr9,
                    uint256(arb.hatLine).toString(),
                    tr10
                )
            );
    }
    ...
    function genAccessory(uint256) internal view returns (string[2] memory) {
    ...
    function genImgSVG(uint256 tokenId) public view returns (string memory) {
        ...
        string memory output = string(
            abi.encodePacked(z[0], z[1], z[2], z[3], z[4], z[5])
        );
        output = string(
            abi.encodePacked(output, z[6], z[7], z[8], z[9], z[10], z[11])
        );
        output = string(abi.encodePacked(output, z[12]));
        output = string(
            abi.encodePacked(
                output,
                genHat(random(abi.encodePacked(ra1, tokenId.toString())))[0],
                color[arb.hatBg],
                sp1
            )
        );
        output = string(
            abi.encodePacked(
                output,
                genHat(random(abi.encodePacked(ra1, tokenId.toString())))[1],
                color[arb.hatLine],
                sp1
            )
        );
        output = string(
            abi.encodePacked(
                output,
                genEyes(random(abi.encodePacked(re1, tokenId.toString()))),
                color[arb.eyes],
                sp1
            )
        );
        ...
        output = string(
            abi.encodePacked(output, z[15], color[arb.trousers], sp1, z[16])
        );
        return output;
    }
    ...
    function getisMinted(address _addressUser) external view returns (bool) {
        return isMinted[_addressUser];
    }

    function getInfoMintUser(
        address user
    ) public view returns (InfoMintNFT memory) {
        return info[user];
    }

    function setPriceMint(uint256 _priceMint) public onlyOwner {
    ...
    function mint() external payable {
        ...
        require(msg.value == priceMint, "You don't have enough balance");
        if (msg.sender != owner()) {
            require(!isMinted[msg.sender], "You have already minted");
        }
