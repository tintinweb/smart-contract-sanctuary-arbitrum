    ...
    UUPSUpgradeable
{
    ...
    function simulatePendleSwap() external {
        TransferHelper.safeApprove(ptweETH, pendleRouterV3, IERC20(ptweETH).balanceOf(address(this)));
        uint256 outAmount = SiloReward._swapPendle(false, IERC20(ptweETH).balanceOf(address(this)));
        TransferHelper.safeApprove(ptweETH, pendleRouterV3, 0);

        emit PendleRedeemAmount(outAmount);
    }
