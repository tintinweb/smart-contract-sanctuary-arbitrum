...
contract TraderWallet is BaseVault, ITraderWallet {
    ...
    modifier onlyTraderOrFactory() {
        if (_msgSender() != traderAddress && _msgSender() != contractsFactoryAddress) revert CallerNotAllowed();
        _;
    }
    ...
    function addAllowedTradeIndexTokens(
        address[] calldata _indexTokens
    ) external onlyTraderOrFactory {
        address _contractsFactoryAddress = contractsFactoryAddress;
        for (uint256 i = 0; i < _indexTokens.length; i++) {
            address indexToken = _indexTokens[i];
            if (
                !IContractsFactory(_contractsFactoryAddress)
                    .isAllowedGlobalToken(indexToken)
            ) revert InvalidToken();

            _allowedTradeTokens.add(indexToken);
        }
    }
    ...
    ) external override onlyTraderOrFactory {
        address _contractsFactoryAddress = contractsFactoryAddress;
    ...
    function addProtocolToUse(uint256 protocolId) external override onlyTraderOrFactory {
