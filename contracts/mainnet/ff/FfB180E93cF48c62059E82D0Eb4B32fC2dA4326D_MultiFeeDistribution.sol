    ...
    OwnableUpgradeable
{
     ...
     */
    function _updateReward(address _onBehalf) internal {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i ++) {
            ...
            if (userInfo.lastTimeUpdated > 0) {
                // not first deposit
                claimable[rewardToken][_onBehalf] += (r.rewardPerToken - userInfo.rewardPerToken) * userInfo.tokenAmount / 1e50;
            }
    ...
    function _getReward(
        ...
        for (uint256 i; i < length; i ++) {
            ...
            if (claimable[token][_user] > 0) {
                uint claimableReward = claimable[token][_user];
                rewardData[token].amount = rewardData[token].amount - claimableReward;

                IERC20(token).safeTransfer(_user, claimableReward);
                claimable[token][_user] = 0;
                emit RewardPaid(_user, token, claimableReward);
            }
        }
    }
