    ...
    ReentrancyGuardUpgradeable
{
   ...
   /*   function initialize(
    ...
    function executeWithdrawAll(uint256 _position, uint256 _batchSize) external onlyBattleflyBot {
        ...
        uint256 endPosition = depositIds.length < (_position + _batchSize)
            ...
            : (_position + _batchSize);
        for (uint256 i = _position; i < endPosition; i++) {
            (uint256 amount, uint256 remaining, , uint256 lockedUntil, , , IAtlasMine.Lock lock) = ATLAS_MINE.userInfo(
                address(this),
                depositIds[i]
            );
            uint256 totalLockedPeriod = lockedUntil + ATLAS_MINE.getVestingTime(lock);

            // If the position is available to withdraw and does NOT contain V1,V2 or Digistrat amounts
            if (
                totalLockedPeriod <= block.timestamp && depositIds[i] != 23 && depositIds[i] != 29 /* keep V1,V2 and Digistrat deposits in atlasmine */
            ) {
            ...
            // get out V1,V2 and Digistrat deposits at epoch 31, so we can withdraw them at epoch 32 ==> sunday August 21 after 6pm UTC (23 = Digistrat, 29 = V1 + V2)
            else if (
                currentEpoch == 31 && ((depositIds[i] == 23) || (depositIds[i] == 29)) /* get V1,V2 and Digistrat deposits out of atlasmine */
            ) {
                uint256 withdrawAmount;
                if (depositIds[i] == 23) {
                    // withdraw digistrat original deposit amount
                    withdrawAmount = 3491842520000000000000000;
                } else {
                    // withdraw V1 + V2 original deposit amount
                    withdrawAmount = 3529883770000000000000000;
                }
                ATLAS_MINE.withdrawPosition(depositIds[i], type(uint256).max);
                // At this point the stake position should be empty because the rest was withdrawn & restaked earlier. Therefore we can remove it
                atlasPositionsToRemove.add(depositIds[i]);
                // Directly register for restaking, unless a withdrawal is requested (we correct this in executeDepositAll())
                unstakedAmount[IAtlasMine.Lock.twoWeeks] += uint256(withdrawAmount);
            }
            // Get out non digistrat, V1 and V2 amounts and set them up for restaking.
            else if (
                (totalLockedPeriod <= block.timestamp && ((depositIds[i] == 23) || (depositIds[i] == 29))) /* get non V1,V2 and Digistrat deposits out of atlasmine */
            ) {
                uint256 withdrawAmount;
                if (depositIds[i] == 23) {
                    //get everything out except for the Digistrat initial deposit amount
                    if(remaining > 3491842520000000000000000) {
                        withdrawAmount = remaining - 3491842520000000000000000;
                    }
                } else {
                    //get everything out except for the V1+V2 initial deposit amount
                    if(remaining > 3529883770000000000000000) {
                        withdrawAmount = remaining - 3529883770000000000000000;
                    }
                }
                if(withdrawAmount > 0) {
                    ATLAS_MINE.withdrawPosition(depositIds[i], withdrawAmount);
                    //Don't remove because partial amount is still in AtlasMine
                    //atlasPositionsToRemove.add(depositIds[i]);
                    // Directly register for restaking, unless a withdrawal is requested (we correct this in executeDepositAll())
                    unstakedAmount[IAtlasMine.Lock.twoWeeks] += uint256(withdrawAmount);
                }
 ...
 /*     function setAllowedLocks(IAtlasMine.Lock[] memory _locks) public onlyOwner {
   ...
   /*   function setTreasury(address _treasury) public onlyOwner {
    ...
    // Use this to manually set unstake amounts at epoch 32 for the respective Digistrat, V1 & V2 amounts
    function addUnstakeAmountAtEpoch(uint64 _epoch, uint256 _amount) external override onlyOwner {
        unstakeAmountAtEpoch[_epoch] += _amount;
    }

    // Use this to manually set the retention unlock to epoch 32 for the  Digistrat, V1 & V2 deposits (15, 16 & 159)
    function setRetentionUnlockForStake(uint256 _depositId, uint64 _retentionUnlock) external override onlyOwner {
        vaultStakes[_depositId].retentionUnlock = _retentionUnlock;
    }
...
interface IBattleflyAtlasStakerV02 {
    ...
    function addUnstakeAmountAtEpoch(uint64 _epoch, uint256 _amount) external;

    function setRetentionUnlockForStake(uint256 _depositId, uint64 _retentionUnlock) external;
