...
interface IRodeoEscrowToken {
    function burn(uint256 amount, uint256 time) external;
}

interface IRodeoLiquidityMining {
    function deposit(uint256 pid, uint256 amount, address to, uint256 lock) external;
    function withdraw(uint256 pid, uint256 amount, address to) external;
    function harvest(uint256 pid, address to, address plugin) external;
    function userInfo(uint256 _pid, address _user) external view returns (uint256, uint256, uint256, uint256, int256, uint256);
...
interface IRodeoVester {
    function getAvailable(address target, uint256 index) external view returns (uint256);
    function claim(uint256 index, address target) external;
    function schedulesCount(address target) external view returns (uint256);
    function schedules(address target, uint256 index) external view returns (uint256, address, uint256, uint256, uint256, uint256, uint256, uint256, bool);
}
...
import "../../interfaces/gmx/IPrfctVault.sol";
import "../../interfaces/gmx/IGMXStrategy.sol";
import "../Common/StrategyFeeManagerInitializable.sol";

contract StrategyGLP is StrategyFeeManagerInitializable {
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 prfctFeeAmount = feeBal * fees.prfct / DIVISOR;
    ...
    function retireStrat() external {
        ...
        IPrfctVault.StratCandidate memory candidate = IPrfctVault(vault).stratCandidate();
    ...
    function acceptTransfer() external {
        address prevStrat = IPrfctVault(vault).strategy();
...
import "../../interfaces/prfct/IPrfctSwapper.sol";
import "../../interfaces/common/IWrappedNative.sol";
import "../Common/StrategyFeeManagerInitializable.sol";

contract StrategyGM is StrategyFeeManagerInitializable {
    using SafeERC20 for IERC20;
    ...
    function _swapRewardsToNative() internal {
        for (uint i; i < rewards.length; ++i) {
            ...
            if (reward != native) {
                ...
                if (amount > 0) {
                    IPrfctSwapper(unirouter).swap(reward, native, amount);
                }
            }
        }
    }
    ...
    function _mintGm() internal {
        ...
        if (long != native) IPrfctSwapper(unirouter).swap(native, long, halfAmount);
        if (short != native) IPrfctSwapper(unirouter).swap(native, short, nativeBal - halfAmount);
...
contract StrategyGMX is StrategyFeeManagerInitializable {
    using SafeERC20 for IERC20;
...
contract StrategyOrange is StrategyFeeManagerInitializable {
    ...
    function deposit() public whenNotPaused {
        uint256 wantBal = IERC20(want).balanceOf(address(this));
        uint256 _minlimit = IOrangeStrykeLPAutomator(swant).minDepositAssets();
        if (wantBal >= _minlimit) {
    ...
    function managerHarvest(address[] memory _users, address[]  memory _tokens, uint256[] memory _amount, bytes32[][] memory _proofs) external onlyManager {
        IAngleDistributor(adist).claim(_users, _tokens, _amount, _proofs);
        _harvest(tx.origin);
    }
...
import "../../interfaces/camelot/ICamelotRouter.sol";
import "../../interfaces/rodeo/IRodeoPool.sol";
import "../../interfaces/rodeo/IRodeoVester.sol";
import "../../interfaces/rodeo/IRodeoEscrowToken.sol";
import "../../interfaces/rodeo/IRodeoLiquidityMining.sol";
import "../Common/StrategyFeeManagerInitializable.sol";

contract StrategyRodeo is StrategyFeeManagerInitializable {
    using SafeERC20 for IERC20;

    // Tokens used
    address public output;
    address public want;
    address public ribToken;
    address public escrowToken;

    // Third party contracts
    address public vester;
    address public mining;
    address public miningPlugin;
    uint256 public pid;

    bool public harvestOnDeposit;
    uint256 public swapOutputAtAmount;
    ...
    function initialize(
        ...
        pid = _pid;
        mining = _mining;
        ribToken = _ribToken; // RIBUSDC
        output = _outputToWantRoute[0]; // RDO
        want = _outputToWantRoute[_outputToWantRoute.length - 1]; // USDC
        outputToWantRoute = _outputToWantRoute;

        swapOutputAtAmount = 1 ether;
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IRodeoPool(ribToken).mint(wantBal, address(this));
            uint256 ribBal = IERC20(ribToken).balanceOf(address(this));
            IRodeoLiquidityMining(mining).deposit(pid, ribBal, address(this), 0);
            emit Deposit(balanceOf());
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            uint256 ribBal = (_amount - wantBal) * IRodeoPool(ribToken).totalSupply() / IRodeoPool(ribToken).getTotalLiquidity();
            IRodeoLiquidityMining(mining).withdraw(pid, ribBal, address(this));
            IRodeoPool(ribToken).burn(ribBal, address(this));
            wantBal = IERC20(want).balanceOf(address(this));
        }
    ...
    // compounds earnings and charges performance fee
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        IRodeoLiquidityMining(mining).harvest(pid, address(this), zeroAddress);
        uint256 outputBal = IERC20(output).balanceOf(address(this));
        if (outputBal > swapOutputAtAmount) {
            chargeFees(callFeeRecipient);
            _convertRewards();
            uint256 wantHarvested = balanceOfWant();
            deposit();
    ...
    function _convertRewards() internal {
        // convert any output to want
        uint256 outputBal = IERC20(output).balanceOf(address(this));
        ICamelotRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(outputBal, 0, outputToWantRoute, address(this), zeroAddress, block.timestamp);
    }

    // performance fees
    function chargeFees(address callFeeRecipient) internal {
        IOrigFeeConfig.FeeCategory memory fees = getFees();
        uint256 outputBal = IERC20(output).balanceOf(address(this)) * fees.total / DIVISOR;

        uint256 callFeeAmount = outputBal * fees.call / DIVISOR;
        IERC20(output).safeTransfer(callFeeRecipient, callFeeAmount);

        uint256 prfctFeeAmount = outputBal * fees.prfct / DIVISOR;
        IERC20(output).safeTransfer(prfctFeeRecipient, prfctFeeAmount);

        uint256 strategistFeeAmount = outputBal * fees.strategist / DIVISOR;
    ...
    // it calculates how much 'want' the strategy has working in the farm.
    function balanceOfPool() public view returns (uint256) {
        (uint256 amount,,,,,) = IRodeoLiquidityMining(mining).userInfo(pid, address(this));
        amount = amount * IRodeoPool(ribToken).getTotalLiquidity() / IRodeoPool(ribToken).totalSupply();
        return amount;
    }
    ...
    function retireStrat() external {
        ...
        IRodeoLiquidityMining(mining).emergencyWithdraw(pid, address(this));
        uint256 ribBal = IERC20(ribToken).balanceOf(address(this));
        IRodeoPool(ribToken).burn(ribBal, address(this));
    ...
    function _giveAllowances() internal {
        ...
        IERC20(want).safeApprove(ribToken, type(uint).max);
        IERC20(ribToken).safeApprove(mining, type(uint).max);
    }

    function _removeAllowances() internal {
        IERC20(output).safeApprove(unirouter, 0);
        IERC20(want).safeApprove(ribToken, 0);
        IERC20(ribToken).safeApprove(mining, 0);
    }

    function setSwapOutputAtAmount(uint256 _swapOutputAtAmount) external onlyManager {
        swapOutputAtAmount = _swapOutputAtAmount;
    }
...
import "../interfaces/prfct/IStrategyV7.sol";
 ...
 */
contract PrfctVaultV1 is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
...
contract PrfctVaultV1Factory {
  ...
  constructor(address _instance) {
    if (_instance == address(0)) {
      instance = new PrfctVaultV1();
    } else {
      instance = PrfctVaultV1(_instance);
    }
  }
  ...
  function cloneVault() external returns (PrfctVaultV1) {
    return PrfctVaultV1(cloneContract(address(instance)));
  }
