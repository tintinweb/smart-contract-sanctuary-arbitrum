...
contract BatchQueryHelper {
    ...
    function getV3Tickets(IUniswapV3Pool[] calldata pairs, int24[] calldata minTickets, int24[] calldata maxTickets) external view returns (TicketVars[][] memory results){
        uint length = pairs.length;
        require(minTickets.length == length && maxTickets.length == length, "length error");
        results = new TicketVars[][](length);
        for (uint i = 0; i < length; i++) {
            ...
            TicketVars[] memory tickets = new TicketVars[]((uint)(ticketsLength));
            for(int24 j = 0; j < ticketsLength; j++){
                TicketVars memory t;
                t.tick = minTickets[i] + j * tickSpacing;
                (,int128 liquidityNet,,,,,,) = pair.ticks(t.tick);
