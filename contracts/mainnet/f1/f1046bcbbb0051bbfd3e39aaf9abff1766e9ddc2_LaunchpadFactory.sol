...
// File @openzeppelin/contracts/access/[email protected]v4.5.0
...
// File contracts/AdminableUpgradeable.sol

pragma solidity =0.8.4;

abstract contract AdminableUpgradeable is
    Initializable,
    OwnableUpgradeable,
    AccessControlUpgradeable
{
    function __initialize() public virtual initializer {
...
// File contracts/LaunchpadIDO/WithLimits.sol

pragma solidity =0.8.4;

abstract contract WithLimits is AdminableUpgradeable {
...
// File contracts/LaunchpadIDO/Timed.sol

pragma solidity =0.8.4;

abstract contract Timed is AdminableUpgradeable {
    ...
    function _Timed_Init(uint256 _startTime, uint256 _saleDuration) internal {
...
// File contracts/LaunchpadIDO/GeneralIDO.sol

pragma solidity =0.8.4;

abstract contract GeneralIDO is AdminableUpgradeable, Timed {
    using SafeMath for uint256;
    // Actual rate is: rate / 1e18
    // 6.123456 actual rate = 6123456000000 specified rate
    uint256 public rate;
    ...
    function calculatePurchaseAmount(uint256 purchaseAmountWei)
        ...
        returns (uint256)
    {
        uint256 nativeRate = getRateNative();
        return purchaseAmountWei.mul(1e18).div(nativeRate);
    }

    function calculatePurchaseAmountUSD(uint256 purchaseAmountUSDWei)
        public
        view
        returns (uint256)
    {
        int256 price = getNativePriceUSD();
        return
            calculatePurchaseAmount(
                purchaseAmountUSDWei.mul(1e8).div(uint256(price))
            );
    }

    function calculateNativeRequiredAmount(uint256 _tokenAmount)
        public
        view
        returns (uint256)
    {
        ...
        return _tokenAmount.mul(nativeRate).div(1e18);
    }
    ...
    function setPriceAggregator(address _priceFeed) public onlyOwnerOrAdmin {
        priceFeed = AggregatorV3Interface(_priceFeed);
    }

    // returns the Native currency price of token
    function getRateNative() public view returns (uint256) {
        int256 price = getNativePriceUSD();
        uint256 cPrice = (rate.mul(1e8)).div(uint256(price));
        return cPrice;
    }

    function getNativePriceUSD() public view returns (int256) {
...
// File contracts/LaunchpadIDO/WithLevelsSale.sol
    ...
    WithLimits
{
    ...
    function setTierUsersRoot(bytes32 _root, string memory _dataUri)
        public
        onlyOwnerOrAdmin
    {
        require(!isLive(), "Sale: Cannot set, sale is live");
        require(_root != 0, "Sale: Cannot set empty root");
        merkleRoot = _root;
        dataUri = _dataUri;
    }

    /**
     * Return:  multiplier, allocation,.
     *
     ...
     */
    function getUserLevelState(uint256 userWeight)
        public
        view
        returns (uint256, uint256)
    {
        ...
        uint256 allocation = levelsOpen ? userWeight + maxSell : userWeight;
        return (userWeight, allocation);
    }
...
// File contracts/LaunchpadIDO/Withdrawable.sol

pragma solidity =0.8.4;

abstract contract Withdrawable is AdminableUpgradeable {
    ...
    function _Withdraw_Init(address _fundToken) internal {
    ...
    function withdrawAll() external onlyOwnerOrAdmin {
        ...
        if (fundByTokens && fundToken.balanceOf(address(this)) > 0) {
            fundToken.transfer(fundsWallet, fundToken.balanceOf(address(this)));
        }
    }
    ...
    function withdrawBalance(uint256 amount) external onlyOwnerOrAdmin {
        ...
        if (fundByTokens) {
            fundToken.transfer(fundsWallet, amount);
        } else {
...
// File contracts/LaunchpadIDO/WithCancelable.sol

pragma solidity =0.8.4;

abstract contract WithCancelable is AdminableUpgradeable, Timed {
    bool public isCanceled;

    modifier notCanceled() {
        require(!isCanceled, "Sale: sale is canceled");
        _;
    }

    function cancelIdo() public onlyOwnerOrAdmin {
        require(block.timestamp < startTime, "Sale: sale is live");
        isCanceled = true;
    }
}

// File contracts/LaunchpadIDO/WithClaimer.sol

pragma solidity ^0.8.0;

abstract contract WithClaimer is AdminableUpgradeable, GeneralIDO {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    struct Claim {
        uint256 unlockTime; // unix time
        uint256 percent; // 6 decimals: 1.783% = 1783000
    }
    ...
    event ClaimDelayed(uint256 percent, uint256 newTime, uint256 claimIdx);
    ...
    function setClaimInfo(
        ...
        for (uint256 i = 0; i < times.length; i++) {
            ...
            require(
                times[i] > getEndTime(),
                "Claimer: time must be after end time"
            );
            claims.push(Claim(times[i], percents[i]));
            totalPercent += percents[i];
        }
        require(totalPercent == 100e6, "Claimer: Invalid total percent");
    }
    ...
    function getRemainingAccountAmount(address account)
        ...
        returns (uint256)
    {
        return balances[account] - claimed[account];
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < claims.length; i++) {
            if (claims[i].unlockTime < block.timestamp) {
        ...
        returns (uint256)
    {
        ...
        return (balances[account] * claims[claimIdx].percent) / 100e6;
    }

    function pauseClaiming(bool status) external onlyOwnerOrAdmin {
        isPaused = status;
    ...
    function setToken(address _token) external onlyOwnerOrAdmin {
        require(
            claims[0].unlockTime > block.timestamp,
            "Claimer: Claim started"
        );
        token = IERC20(_token);
    }

    function delayClaim(uint256 claimIdx, uint256 newUnlockTime)
        external
        onlyOwner
    {
        ...
        require(
            ...
            "Claimer: Time must be in future"
        );
        require(newUnlockTime > _claim.unlockTime, "Claimer: Invalid new Time");
    ...
    function claim(address account, uint256 idx) external {
        ...
        require(
            claims[idx].unlockTime < block.timestamp,
            "Claimer: Not claimable"
        );
        require(balances[account] > 0, "Claimer: No Allocation");
...
// File contracts/LaunchpadIDO/LaunchpadIDO.sol
    ...
    WithClaimer
{
    ...
    function initialize(
        ...
        require(stringsEqual(id, ""), "already initialized.");
        AdminableUpgradeable.__initialize();
        _General_Ido_Init(_rate, _tokensForSale, _priceFeed);
        _Timed_Init(_startTime, _duration);
        _Withdraw_Init(_fundToken);
        id = _id;
        levelsEnabled = _levelsEnabled;
        fundsWallet = 0x5c26953712ffAb29ea7B2B416832d3724d26E6e2;
    }
    ...
    ) public payable ongoingSale nonReentrant notCanceled {
        require(
            !fundByTokens,
            "Sale: presale is funded by tokens but value is missing"
        );

        internalBuyTokens(msg.value, amount, userWeight, merkleProof);
    }
    ...
    function buyTokens(
        ...
        uint256 userWeight,
        bytes32[] memory merkleProof
    ) public ongoingSale nonReentrant notCanceled {
        ...
        internalBuyTokens(value, amount, userWeight, merkleProof);

        fundToken.safeTransferFrom(msg.sender, address(this), value);
    }

    function internalBuyTokens(
        uint256 value,
        uint256 amount,
        uint256 userWeight,
        bytes32[] memory merkleProof
        ...
        bytes32 node = keccak256(abi.encodePacked(account, userWeight));
        userWeight = MerkleProof.verify(merkleProof, merkleRoot, node)
            ? userWeight
            : 0;
        (
            uint256 maxAllocation,
            uint256 minAllocation
        ) = checkAccountAllowedToBuy(userWeight);

        require(value > 0, "Sale: value is 0");

        uint256 requiredValue = calculateNativeRequiredAmount(amount);
        require(amount > 0, "Sale: amount is 0");
        require(msg.value >= requiredValue, "Sale: low balance.");
        tokensSold += amount;
        balances[account] += amount;
        require(amount >= minAllocation, "Sale: amount is too small");
        require(
        ...
        uint256 extra = msg.value.sub(requiredValue);
        if (extra > 0) payable(msg.sender).transfer(extra);

        emit TokensPurchased(account, value, amount);
    }

    function checkAccountAllowedToBuy(uint256 userWeight)
        private
        view
        returns (uint256, uint256)
    {
        uint256 levelAllocation = userWeight;

        // Public sale with no levels
        if (!levelsEnabled) {
        ...
        if (levelsEnabled) {
            ...
            require(
                levelAllocation > 0,
                "Sale: user has no level allocation or level is too low"
            );

            return (levelAllocation, 0);
        }
    ...
    {
        address[] memory users = participants.values();
...
contract LaunchpadFactory is Adminable {
    ...
    IdoInfo[] public idos;
    event IDOCreated(address ido);

    constructor() {
        libraryAddress = address(new LaunchpadIDO());
    }
    ...
    function deployIDO(
        ...
        idos.push(IdoInfo(LaunchpadIDO(clone), dataUri));
        emit IDOCreated(clone);
    }

    function setIdo(LaunchpadIDO _ido, string memory _dataUri)
        external
        onlyOwnerOrAdmin
    {
        idos.push(IdoInfo(_ido, _dataUri));
    }

    function updateDataUri(uint256 index, string memory _dataUri)
        external
        onlyOwnerOrAdmin
    {
        require(index < idos.length, "Index out of bounds");
        idos[index].dataUri = _dataUri;
    }
