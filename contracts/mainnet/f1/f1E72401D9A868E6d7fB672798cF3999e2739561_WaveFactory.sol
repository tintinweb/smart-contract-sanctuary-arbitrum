...
contract WaveContract is ERC2771Context, Ownable, ERC721 {
  ...
  constructor(
    ...
    PERMIT_TYPEHASH = keccak256(
      "Permit(address spender,uint256 rewardId,uint256 nonce,uint256 deadline)"
    );
  }
  ...
  function claim(
    ...
    if (claimed[keccak256(abi.encode(_msgSender(), rewardId))]) revert RewardAlreadyClaimed();
    ...
    unchecked {
      address recoveredAddress = ecrecover(
        keccak256(
          abi.encodePacked(
            ...
            keccak256(
              abi.encode(PERMIT_TYPEHASH, _msgSender(), rewardId, verifierNonce++, deadline)
            )
          )
        ),
        v,
      ...
      if (recoveredAddress == address(0) || recoveredAddress != factory.verifier())
  ...
  function airdrop(ClaimParams[] memory params) public onlyOwner {
    ...
    for (uint256 i = 0; i < len; ++i) {
      ...
      claimed[keccak256(abi.encode(params[i].user, params[i].rewardId))] = true;
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    return
      customMetadata
        ...
        : string(
          abi.encodePacked(baseURI, "/", Strings.toString(tokenIdToRewardId[tokenId]), ".json")
        );
  }

  ///@dev used for changing rewardId associated to some tokens
  function award(uint256[] memory winnerIds, uint256 rewardId) public onlyOwner {
