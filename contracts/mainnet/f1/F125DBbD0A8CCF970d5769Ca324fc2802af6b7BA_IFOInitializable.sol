 ...
 */
contract IFOInitializable is IIFOV2, ReentrancyGuard, Ownable, Whitelist {
    using SafeMath for uint256;
    ...
    // The address of the treasury, where fees are sent to
    ...
    // The maximum deposit fee that can be associated to a pool
    uint256 public constant MAX_DEPOSIT_FEE = 1050; // 5%

    // The minimum deposit fee that can be associated to a pool
    uint256 public constant MIN_DEPOSIT_FEE = 1000; // 0%
    ...
    struct PoolCharacteristics {
        ...
        uint256 depositFee; // The depositFee associated to the pool
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        // If pid is zero, make sure the user is white-listed (private sale pool)
        if (_pid == 0) {
            require(_isQualifiedWhitelist(msg.sender), "Deposit: Not white listed");
        }

        uint256 feeAmount = ((_amount.mul(_poolInformation[_pid].depositFee)).div(1000)).sub(_amount);

        // Transfers funds to this contract
        lpToken.safeTransferFrom(address(msg.sender), address(this), (_amount.sub(feeAmount)));

        // Transfers fee to treasury
        if (feeAmount > 0) {
            lpToken.safeTransferFrom(address(msg.sender), treasury, feeAmount);
        }

        // Update the user status
        _userInfo[msg.sender][_pid].amountPool = _userInfo[msg.sender][_pid].amountPool.add((_amount.sub(feeAmount)));
        ...
        _poolInformation[_pid].totalAmountPool = _poolInformation[_pid].totalAmountPool.add((_amount.sub(feeAmount)));
     ...
     * @param _limitPerUserInLP: limit per user (in LP tokens)
     * @param _hasTax: if the pool has a tax
     * @param _depositFee: The deposit fee associated to the pool, can be between 0 and 5%
     * @param _pid: pool id
     * @dev This function is only callable by admin.
     */
    function setPool(
        ...
        require(
            ...
            _depositFee >= MIN_DEPOSIT_FEE
            , "Operations: Deposit fee invalid"
        );
        ...
        _poolInformation[_pid].depositFee = _depositFee;
     ...
     * @return depositFee: The fee that currently associated to depositing into the pool (1000 is 0% 1050 is 5%)
     */
    function viewPoolInformation(uint256 _pid)
        external
        view
        ...
        )
    {
        return (
            ...
            _poolInformation[_pid].depositFee
        );
    }
    ...
    /**
     * @notice Check if the user is whitelisted (callable externally)
     */
