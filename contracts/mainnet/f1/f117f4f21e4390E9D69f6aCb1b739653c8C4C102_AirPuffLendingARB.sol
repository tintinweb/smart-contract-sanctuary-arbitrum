...
contract AirPuffLendingARB is ERC4626Upgradeable, OwnableUpgradeable {
    using SafeERC20 for IERC20;
    using MathUpgradeable for uint256;

    address public ARB; // ARB
    ...
    modifier onlyARBGifter() {
        require(allowedToGift[msg.sender], 'Not allowed to increment ARB');
        _;
    }
    ...
    event ARBGifterAllowed(address indexed gifter, bool status);
    ...
    function initialize(address _ARB) external initializer {
        require(_ARB != address(0), "ZERO_ADDRESS");
        ARB = _ARB;
        ...
        __Ownable_init();
        __ERC4626_init(IERC20Upgradeable(_ARB));
        __ERC20_init("ARB-AirPuff", "ARB-AirPuff");
    }

    /** ---------------- View functions --------------- */

    function balanceOfARB() public view returns (uint256) {
        return totalARB;
    }

    function getUtilizationRate() public view returns (uint256) {
        uint256 totalAirPuffDebt = totalDebt;
        return totalAirPuffDebt == 0 ? 0 : totalAirPuffDebt.mulDiv(1e18, totalARB + totalAirPuffDebt);
    }
    ...
    function allowARBGifter(address _gifter,bool _status) external onlyOwner zeroAddress(_gifter) {
        allowedToGift[_gifter] = _status;
        emit ARBGifterAllowed(_gifter, _status);
    }
    ...
    /**
    * @notice Allow the Lending Vault to lend a certain amount of ARB to the protocol.
    * @dev The function allows the Lending Vault to lend a certain amount of ARB to the protocol. It updates the total debt and total ARB balances accordingly.
    * @param _borrowed The amount of ARB to lend.
    ...
    */
    function lend(uint256 _borrowed,address _receiver) external onlyLendingVault returns (bool status) {
        require(totalARB > _borrowed, "Not enough ARB to lend");
        ...
        IERC20(ARB).safeTransfer(_receiver, _borrowed);
     ...
     * @dev The function allows the Lending Vault to repay a certain amount of debt to the protocol. It updates the total debt and total ARB balances accordingly.
     * @param _debtAmount The amount of debt to repay.
     * @param _amountPaid The amount of ARB paid to repay the debt.
    ...
    function repayDebt(uint256 _debtAmount, uint256 _amountPaid) external onlyLendingVault returns (bool) {
        IERC20(ARB).safeTransferFrom(msg.sender, address(this), _amountPaid);
        totalDebt = totalDebt - _debtAmount;
        totalARB += _amountPaid;
     ...
     *      It updates the total ARB balance and sets a timelock for the receiver.
     ...
     *      The function then performs the actual withdrawal by calling the internal `_withdraw` function. It updates the total ARB balance after the withdrawal and returns the amount of shares returned to the user.
     * @param _assets The amount of assets (ARB) to withdraw.
     * @param _receiver The address of the receiver who will receive the corresponding shares.
     * @param _owner The address of the owner who is making the withdrawal.
     * @return The amount of shares returned to the user.
     */
    function withdraw(
        uint256 _assets, // Native (ARB) token amount
        address _receiver,
        address _owner
        ...
        require(balanceOfARB() > _assets, "Insufficient balance in vault");

        uint256 UR = getUtilizationRate();
        uint256 shares = previewWithdraw(_assets);
        uint256 feeAmount = (_assets * withdrawalFees) / DENOMINATOR;
        IERC20(ARB).safeTransfer(feeReceiver, feeAmount);
    ...
    //function that only allows a whitelisted address to call to increase totalARB 
    function increaseTotalARB(uint256 _amount) external onlyARBGifter {
