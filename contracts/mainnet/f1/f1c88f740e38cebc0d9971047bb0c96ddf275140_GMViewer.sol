...
contract RebalanceStrategy is IRebalance, IDepositCallbackReceiver, IWithdrawalCallbackReceiver, OwnableUpgradeable {
        ...
        initializer
    {
        ...
        gmxReader = IReader(0xf60becbba223EEA9495Da3f606753867eC10d139);
    ...
    function afterWithdrawalExecution(
        ...
        Market.Props memory market = gmxReader.getMarket(gmxRouter.dataStore(), withdrawal.addresses.market);

        uint256 swappedAmount = getSwapper(market.longToken, USDC).swap(
            market.longToken, eventData.uintItems.items[0].value, USDC, 0, abi.encode(slippage, BASIS)
        );
        ...
        Rebalance memory _rebalance = Rebalance({
            ...
            usdcAmount: eventData.uintItems.items[1].value + swappedAmount,
            purchase: false
        });
        ...
        returns (uint256)
    {
        ...
        return applySlippage(
            gmxReader.getDepositAmountOut(gmxRouter.dataStore(), market, prices, 0, usdcAmount, address(0))
        );
    }
}
