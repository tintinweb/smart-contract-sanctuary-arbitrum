...
import { IAverageProductivityCalculator } from "./IAverageProductivityCalculator.sol";

contract AverageProductivityCalculator is
  EpochConfigurable,
  IAverageProductivityCalculator
{
  ...
  constructor(
    address _manager,
    address _productivity
  ) EpochConfigurable(_manager, 1 days, 0 hours) {
    PRODUCTIVITY = IProductivity(_productivity);
  }

  function currentRealmProductivityGains(
    uint _realmId
  ) public view returns (uint) {
    return PRODUCTIVITY.currentProductivity(_realmId);
  }

  function realmProductivityGains(
    uint _epoch,
    uint _realmId
  ) public view returns (uint gains) {
    (gains, ) = PRODUCTIVITY.epochsProductivityTotals(
      _realmId,
      _epoch,
      _epoch + 1
    );
  ...
  function realmProductivityGainsBatch(
    ...
    result = new uint[](_endEpoch - _startEpoch);
    for (uint i = 0; i < result.length; i++) {
      result[i] = realmProductivityGains(_startEpoch + i, _realmId);
    }
  }
  ...
  function currentAverageRealmProductivityGains(
    ...
    return averageRealmProductivityGains(currentEpoch(), _realmId, _epochSpan);
  }
  ...
  function averageRealmProductivityGains(
    ...
    (uint gains, ) = PRODUCTIVITY.epochsProductivityTotals(
      _realmId,
      _epoch - _epochSpan,
      _epoch + 1
    );

    return gains / _epochSpan;
  }
  ...
  function averageRealmProductivityGainsBatch(
    ...
    uint[] memory result = new uint[](_endEpoch - _startEpoch);
    for (uint i = 0; i < result.length; i++) {
      result[i] = averageRealmProductivityGains(
        _startEpoch + i,
        _realmId,
        _epochSpan
      );
