...
contract GMXDepositor is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, IDepositor {
    ...
    function initialize(address _caller, address _wethAddress, address _rewardTracker, address _platform) external initializer {
    ...
    function mint(address _token, uint256 _amountIn) public payable override onlyCaller returns (address, uint256) {
        ...
        IRewardTracker(rewardTracker).execute();
    ...
    function withdraw(address _tokenOut, uint256 _amountIn, uint256 _minOut) public payable override onlyCaller returns (uint256) {
        IRewardTracker(rewardTracker).execute();
    ...
    function stakeGlp(address _spender, uint256 _glpAmountIn) external override nonReentrant onlyCaller returns (bool) {
        return IGMXExecutor(executor).stakeGlp(_spender, _glpAmountIn);
    }

    function unstakeGlp(address _recipient, uint256 _glpAmountOut) external override nonReentrant onlyCaller returns (bool) {
        return IGMXExecutor(executor).unstakeGlp(_recipient, _glpAmountOut);
    }
