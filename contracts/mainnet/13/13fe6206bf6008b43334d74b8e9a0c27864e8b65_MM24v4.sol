...
error OnlyForMinted();
error RegisterNotEnabled();
error AlreadyRegistered();
error NotRegistered();
error IncorrectSelection();
error NoMintIfRegisterIsEnabled();
error OnlyOneOwnedPerAddress();
...
/// @title A contract with a mint check for a previous generic selection
/// @author glbkst
    ...
    ILicenseInfo
{
    ...
    mapping (address => uint) private _selections;
    mapping (uint => bool) private _correctSelections;
    uint256[] private _correctSelectionsArray;
    bool private _canRegister;

    /// @notice Contract deployed as paused.
    /// @param initialOwner owner of the contract
    constructor(
        address initialOwner
    ) ERC721("Football", "MM24v4") Ownable(initialOwner) {
        _pause();
        _nextTokenId = 1;
    ...
    function setCanRegister(bool value) public onlyOwner {
        _canRegister = value;
    }

    function registerSelection(uint256 selection) public {
        if(!_canRegister) revert RegisterNotEnabled();
        if(_selections[msg.sender] != 0) revert AlreadyRegistered();
        _selections[msg.sender] = selection;
    }

    function unregisterSelection(address addr) public onlyOwner {
        _selections[addr] = 0;
    }

    function setCorrectSelections(uint256[] memory correctSelections) public onlyOwner {
        for(uint256 i=0;i < _correctSelectionsArray.length;++i) {
            _correctSelections[_correctSelectionsArray[i]] = false;
        }
        for(uint256 i=0;i < correctSelections.length;++i) {
            _correctSelections[correctSelections[i]] = true;
        }
        _correctSelectionsArray = correctSelections;
    }
    ...
    /// @notice mint a new NFT e.g. for an airdrop
    ...
    function mint() external nonReentrant {
        if(_canRegister) revert NoMintIfRegisterIsEnabled();
        uint256 selectionOfSender = _selections[msg.sender];
        if (selectionOfSender == 0) revert NotRegistered();
        if(!_correctSelections[selectionOfSender]) revert IncorrectSelection();
        if(balanceOf(msg.sender)>= 1) revert OnlyOneOwnedPerAddress();
