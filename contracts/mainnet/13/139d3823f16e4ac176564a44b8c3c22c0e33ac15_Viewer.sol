...
import {AlgebraLiquidityLib} from "src/libraries/AlgebraLiquidityLib.sol";

contract Viewer is IViewer, UpgradeableGovernable {
    using SafeMath for uint256;
    using AlgebraLiquidityLib for IAlgebraPool;
    ...
    function getFactory(string memory _strategy) external view override returns (address) {
        ...
        for (uint256 i; i < length; ++i) {
            address _factory = factories[i];

            if (keccak256(bytes(IFactory(_factory).strategy())) == keccak256(bytes(_strategy))) {
                return _factory;
            }
        }
        return address(0);
        ...
        returns (uint256 position, uint256 shares)
    {
        ...
        position = IRouter(lpContracts.router).previewPosition(price0, price1, _amount0, _amount1, position0, position1);

        if (_compound) {
            shares = ICompounder(lpContracts.compounder).previewDeposit(position);
        }
    }
        ...
        returns (uint256 amount0, uint256 amount1, uint256 amount0AfterRetention, uint256 amount1AfterRetention)
    {
        ...
        IPriceHelper(lpContracts.priceHelper).getPrices(_pool);

        if (_compound) {
            _position = ICompounder(lpContracts.compounder).previewRedeem(_position);
        }

        (amount0, amount1, amount0AfterRetention, amount1AfterRetention) =
            IManager(lpContracts.lpManager).previewRedeemLiquidity(_position);
    }
    ...
    function TVL(address _factory, address _pool) external view override returns (uint256) {
        ...
        return ((price0 * position0).add(price1 * position1)).div(1e18);
    }
...
interface IFactory {
    ...
    struct SetupParams {
        uint256 nonce;
        IPriceHelper.PoolInput _poolInput;
    }

    function strategy() external view returns (string calldata);

    // External
    function create(CreateParams calldata params) external returns (uint256);
    ...
    event Create(uint256 nonce, address indexed lp, Stage stage, LPContracts);
    event Setup(uint256 nonce, address indexed lp, Stage stage, LPContracts);
