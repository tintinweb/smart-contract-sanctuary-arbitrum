...
// File: interfaces/pre_ido/IPreIDOEvents.sol
...
// File: interfaces/pre_ido/IPreIDOState.sol
...
interface IPreIDOBase is IPreIDOState, IPreIDOEvents {
...
contract EBONDSELL is IPreIDOBase, Ownable {
    ...
    uint256 private constant MIN_LOCK = 1 seconds; // 1 year;
    ...
    receive() external payable inPresalePeriod {
        ...
        _order(msg.value, 6, price, priceFeed.decimals(), 10); // default to 10% discount rate
    }
    ...
    function order(uint8 discountsRate) external payable inPresalePeriod {
        int256 price = getPrice();
        _order(msg.value, 6, price, priceFeed.decimals(), discountsRate);
    }
...
function _order(
    ...
    require(
        amount.mul(uint256(price)).div(10**(_amountDecimals + _priceDecimals)) >= minInvestment,
    ...
    uint256 tokenPriceX4 = 11117; // 0.9 * 10^4
...
uint8 upperPow = token.decimals() + 4; // 4(token price decimals) => 10^4 = 22
    ...
    require(
        totalDistributed + distributeAmount <= token.balanceOf(address(this)),
        "there is not enough supply token to be distributed"
    ); // NET

    token.safeTransfer(msg.sender, distributeAmount); // Transfer tokens directly to the buyer
    orders[++latestOrderId] = OrderInfo(
        payable(msg.sender),
        distributeAmount,
        releaseOnBlock,
        true // Mark the order as already claimed
    );
