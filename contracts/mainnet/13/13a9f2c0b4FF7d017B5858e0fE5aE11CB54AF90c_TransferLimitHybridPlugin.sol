...
import "contracts/token/modular/hybrid/TransferValidationHybridPlugin.sol";

/**
 * @title Transfer Limit Plugin
 * @author Josh Davis
 * @notice Adds the ability to limit the amount an address can transfer in a given time period
 */

/// @dev timeBlockSize defines when the limit for each user resets
/// @dev limitPerTimeBlock is the amount a user can transfer in and out
/// @dev exemptions are addresses where the limit does not apply
/// @notice any Defi contracts or exchange vaults that touch the token contract MUST be included as exemptions
/// @notice if the UniSwap pool is not included in the exemptions, then the limit will restrict total volume, not just individual trades
struct TransferLimitIntrospection {
    uint256 timeBlockSize;
    uint256 limitPerTimeBlock;
    address[] exemptions;
}

interface TransferLimitsEventsAndErrors {
    event TransferLimitUpdated(address indexed token, uint256 amount);

    event TransferLimitExemptionAdded(
        address indexed token,
        address indexed exemption
    );

    event TransferLimitExemptionRemoved(
        address indexed token,
        address indexed exemption
    );
...
interface ITransferLimitHybridPlugin is TransferLimitsEventsAndErrors {
    /**
     * @notice Returns the amount that can be transferred per period
     */
    function transferLimit() external view returns (uint256);

    /**
     * @notice Sets the limit amount
     * @param _limit  the new limit
     *
     * Requirements:
     * - caller MUST be the token contract owner
     */
    function setTransferLimit(uint256 _limit) external returns (bool);

    /**
     * @notice Returns the amount that a user has remaning to spend within the time period
     * @param user the account address
     */
    function transferLimitAmountRemaining(
        address user
    ) external view returns (uint256);

    /**
     * @notice Marks an address as exempt from the transfer limits
     * @notice This is necessary for defi contracts and exchange vaults
     * @param exemption the exempted address
     */
    function transferLimitAddExemption(
        address exemption
    ) external returns (bool);

    /**
     * @notice Marks the addresses as exempt from the transfer limits
     * @param exemptions the exempted addresses
     */
    function transferLimitAddExemptions(
        address[] memory exemptions
    ) external returns (bool);

    /**
     * @notice Unmarks an address as exempt from the transfer limits
     * @notice This is necessary for defi contracts and exchange vaults
     * @param exemption the exempted address
     */
    function transferLimitRemoveExemption(
        address exemption
    ) external returns (bool);

    /**
     * @notice Unmarks the addresses as exempt from the transfer limits
     * @param exemptions the exempted addresses
     */
    function transferLimitRemoveExemptions(
        address[] memory exemptions
    ) external returns (bool);
}

contract TransferLimitHybridPlugin is
    TransferValidationHybridPlugin,
    TransferLimitsEventsAndErrors
{
    using EnumerableSet for EnumerableSet.AddressSet;
    using BytesLib for bytes;

    uint256 constant DEFAULT_TIMEBLOCK = 1 days;

    bytes4 constant GET_TRANSFER_LIMIT =
        ITransferLimitHybridPlugin.transferLimit.selector;
    bytes4 constant GET_AMOUNT_REMAINING_LIMIT =
        ITransferLimitHybridPlugin.transferLimitAmountRemaining.selector;
    bytes4 constant SET_TRANSFER_LIMIT =
        ITransferLimitHybridPlugin.setTransferLimit.selector;
    bytes4 constant ADD_EXEMPTION =
        ITransferLimitHybridPlugin.transferLimitAddExemption.selector;
    bytes4 constant ADD_EXEMPTIONS =
        ITransferLimitHybridPlugin.transferLimitAddExemptions.selector;
    bytes4 constant REMOVE_EXEMPTION =
        ITransferLimitHybridPlugin.transferLimitRemoveExemption.selector;
    bytes4 constant REMOVE_EXEMPTIONS =
        ITransferLimitHybridPlugin.transferLimitRemoveExemptions.selector;

    struct TransferLimitStruct {
        uint256 timeBlockSize;
        uint256 limitPerTimeBlock;
        EnumerableSet.AddressSet exempt;
        mapping(address => mapping(uint256 => uint256)) dailyTotals;
    }

    mapping(address => TransferLimitStruct) internal transferLimits;

    function _onInstall(bytes calldata data) internal virtual override {
        // 128 is minimum length with empty array
        if (data.length < 128 || data.length % 32 != 0) {
            revert InvalidInitCode("invalid abi encoding");
        }
        // [0x20 bytes Block Size][0x20 bytes Limit][array]
        (bool isValid, ) = validateArray(data, 0x40);
        if (!isValid) {
            revert InvalidInitCode("invalid abi encoding");
        }

        (uint256 _blocksize, uint256 _limit, address[] memory _exemptions) = abi
            .decode(data, (uint256, uint256, address[]));
        initLimits(msg.sender, _blocksize, _limit, _exemptions);
    }

    function _onUninstall(bytes calldata) internal virtual override {
        TransferLimitStruct storage limits = transferLimits[msg.sender];
        limits.timeBlockSize = 0;
        limits.limitPerTimeBlock = 0;
        for (uint256 i = limits.exempt.length(); i > 0; i--) {
            limits.exempt.remove(limits.exempt.at(i - 1));
        }
    }

    function initLimits(
        address _token,
        uint256 _timeBlockSize,
        uint256 _limitPerTimeBlock,
        address[] memory _exemptions
    ) internal virtual {
        TransferLimitStruct storage limits = transferLimits[_token];
        if (_timeBlockSize == 0) _timeBlockSize = DEFAULT_TIMEBLOCK;

        limits.timeBlockSize = _timeBlockSize;
        limits.limitPerTimeBlock = _limitPerTimeBlock;
        for (uint256 i = 0; i < _exemptions.length; i++) {
            limits.exempt.add(_exemptions[i]);
        }
    }
        ...
        returns (bytes4[] memory funcs)
    {
        funcs = new bytes4[](10);
        funcs[0] = TRANSFER;
        funcs[1] = TRANSFER_FROM;
        funcs[2] = TRANSFER_CALL;
        funcs[3] = RECEIVED_FROM_BRIDGE;
        funcs[4] = SENT_TO_BRIDGE;
        funcs[5] = SET_TRANSFER_LIMIT;
        funcs[6] = ADD_EXEMPTION;
        funcs[7] = ADD_EXEMPTIONS;
        funcs[8] = REMOVE_EXEMPTION;
        funcs[9] = REMOVE_EXEMPTIONS;
    }
    ...
    function _beforeHook(
        ...
        // if (
        //     (_msg.sig == RECEIVED_FROM_BRIDGE || _msg.sig == SENT_TO_BRIDGE) &&
        //     IModularViciERC20(msg.sender).isMain()
        // ) {
        //     // Validation will happen on transfer to/from vault, so don't count
        //     // the same transfer twice.
        //     return TRUE;
        // }

        if (_msg.sig == GET_AMOUNT_REMAINING_LIMIT) {
            if (_msg.data.length != 36) {
                revert InvalidCallData(_msg.sig);
            }
            return TRUE;
        }
        if (_msg.sig == SET_TRANSFER_LIMIT || _msg.sig == ADD_EXEMPTION || _msg.sig == REMOVE_EXEMPTION) {
            if (_msg.data.length != 36) {
                revert InvalidCallData(_msg.sig);
            }
        } else if (_msg.sig == ADD_EXEMPTIONS || _msg.sig == REMOVE_EXEMPTIONS) {
            // 96 = selector + array with one address
            if (_msg.data.length < 96 && (_msg.data.length - 4) % 32 != 0) {
                revert InvalidCallData(_msg.sig);
            }
            // array starts at beginning, offest by 4 bytes for selector
            (bool isValid, ) = validateArray(_msg.data[4:], 0);
            if (!isValid) {
                revert InvalidCallData(_msg.sig);
            }
        } else if (_msg.sig == RECEIVED_FROM_BRIDGE) {
            BridgeArgs memory args = abi.decode(_msg.data[4:], (BridgeArgs));
            return
                updateLimitsNoRevert(
                    msg.sender,
                    args.fromAddress,
                    args.toAddress,
                    args.amount
    ...
    {
        funcs = new bytes4[](7);
        funcs[0] = GET_TRANSFER_LIMIT;
        funcs[1] = GET_AMOUNT_REMAINING_LIMIT;
        funcs[2] = SET_TRANSFER_LIMIT;
        funcs[3] = ADD_EXEMPTION;
        funcs[4] = ADD_EXEMPTIONS;
        funcs[5] = REMOVE_EXEMPTION;
        funcs[6] = REMOVE_EXEMPTIONS;
    }

    function _execute(
        Message calldata _msg
    ) internal virtual override returns (bytes memory) {
        if (_msg.sig == GET_TRANSFER_LIMIT) {
            return abi.encode(transferLimit());
        }

        if (_msg.sig == GET_AMOUNT_REMAINING_LIMIT) {
            address user = abi.decode(_msg.data[4:], (address));
            return abi.encode(transferLimitAmountRemaining(user));
        }

        if (_msg.sig == SET_TRANSFER_LIMIT) {
            uint256 _limit = abi.decode(_msg.data[4:], (uint256));
            return setTransferLimit(_limit) ? TRUE : FALSE;
        }

        if (_msg.sig == ADD_EXEMPTION) {
            address exemption = abi.decode(_msg.data[4:], (address));
            return transferLimitAddExemption(exemption) ? TRUE : FALSE;
        }

        if (_msg.sig == ADD_EXEMPTIONS) {
            address[] memory exemptions = abi.decode(
                _msg.data[4:],
                (address[])
            );
            return transferLimitAddExemptions(exemptions) ? TRUE : FALSE;
        }

        if (_msg.sig == REMOVE_EXEMPTION) {
            address exemption = abi.decode(_msg.data[4:], (address));
            return transferLimitRemoveExemption(exemption) ? TRUE : FALSE;
        }

        if (_msg.sig == REMOVE_EXEMPTIONS) {
            address[] memory exemptions = abi.decode(
                _msg.data[4:],
                (address[])
            );
            return transferLimitRemoveExemptions(exemptions) ? TRUE : FALSE;
        }
    ...
    function checkTransfer(
        ...
        TransferLimitStruct storage limits = transferLimits[token];
        if (limits.limitPerTimeBlock == 0 || limits.timeBlockSize == 0) {
            return true;
        }

        uint256 today = currentTimestamp() / limits.timeBlockSize;

        if (!limits.exempt.contains(fromAddress)) {
            uint256 fromTotal = limits.dailyTotals[fromAddress][today] + amount;
            if (fromTotal > limits.limitPerTimeBlock) revert TransferTooLarge();
            limits.dailyTotals[fromAddress][today] = fromTotal;
        }

        if (fromAddress != toAddress && !limits.exempt.contains(toAddress)) {
            uint256 toTotal = limits.dailyTotals[toAddress][today] + amount;
            if (toTotal > limits.limitPerTimeBlock) revert TransferTooLarge();
            limits.dailyTotals[toAddress][today] = toTotal;
        }

        return true;
    }

    function updateLimitsNoRevert(
        address token,
        address fromAddress,
        address toAddress,
        uint256 amount
        ...
        address vault = IModularViciERC20(msg.sender).vault();

        uint256 today = currentTimestamp() / limits.timeBlockSize;

        if (fromAddress != vault && !limits.exempt.contains(fromAddress)) {
            limits.dailyTotals[fromAddress][today] += amount;
        }

        if (
            toAddress != vault &&
            fromAddress != toAddress &&
            !limits.exempt.contains(toAddress)
        ) {
            limits.dailyTotals[toAddress][today] += amount;
        }

        return true;
    }

    function transferLimit() internal view virtual returns (uint256) {
        return transferLimits[msg.sender].limitPerTimeBlock;
    }

    function setTransferLimit(uint256 _limit) internal virtual returns (bool) {
        TransferLimitStruct storage limits = transferLimits[msg.sender];
        if (_limit == limits.limitPerTimeBlock) return false;

        limits.limitPerTimeBlock = _limit;
        emit TransferLimitUpdated(msg.sender, _limit);
        return true;
    }
    ...
    function transferLimitAmountRemaining(
        ...
        TransferLimitStruct storage limits = transferLimits[msg.sender];
        if (
            limits.exempt.contains(user) ||
            limits.limitPerTimeBlock == 0 ||
            limits.timeBlockSize == 0
        ) {
            return userBalance(IERC20(msg.sender), user);
        }

        uint256 today = currentTimestamp() / limits.timeBlockSize;
        uint256 amountSoFar = limits.dailyTotals[user][today];
        return
            (amountSoFar >= limits.limitPerTimeBlock)
                ? 0
                : limits.limitPerTimeBlock - amountSoFar;
    }

    function _doAddExemption(
        EnumerableSet.AddressSet storage exemptionSet,
        address exemption
    ) internal virtual returns (bool result) {
        result = exemptionSet.add(exemption);
        if (result) emit TransferLimitExemptionAdded(msg.sender, exemption);
    }

    function transferLimitAddExemption(
        address exemption
    ) internal virtual returns (bool) {
        return _doAddExemption(transferLimits[msg.sender].exempt, exemption);
    }

    function transferLimitAddExemptions(
        address[] memory exemptions
    ) internal virtual returns (bool result) {
        EnumerableSet.AddressSet storage exemptionSet = transferLimits[
            msg.sender
        ].exempt;
        for (uint256 i = 0; i < exemptions.length; i++) {
            result = _doAddExemption(exemptionSet, exemptions[i]) || result;
        }
    }

    function _doRemoveExemption(
        EnumerableSet.AddressSet storage exemptionSet,
        address exemption
    ) internal virtual returns (bool result) {
        result = exemptionSet.remove(exemption);
        if (result) emit TransferLimitExemptionRemoved(msg.sender, exemption);
    }

    function transferLimitRemoveExemption(
        address exemption
    ) internal virtual returns (bool) {
        return _doRemoveExemption(transferLimits[msg.sender].exempt, exemption);
    }
    ...
    function transferLimitRemoveExemptions(
        ...
        for (uint256 i = 0; i < exemptions.length; i++) {
            result = _doRemoveExemption(exemptionSet, exemptions[i]) || result;
        }
    }

    function introspect(
        address token
    ) public view virtual returns (TransferLimitIntrospection memory) {
        TransferLimitStruct storage limits = transferLimits[token];
        return
            TransferLimitIntrospection(
                limits.timeBlockSize,
                limits.limitPerTimeBlock,
                limits.exempt.values()
            );
    }
...
abstract contract TransferValidationHybridPlugin is ViciERC20HybridPlugin {
    bytes4 constant TRANSFER = IERC20.transfer.selector;
    bytes4 constant TRANSFER_FROM = IERC20.transferFrom.selector;
    bytes4 constant TRANSFER_CALL = IERC677.transferAndCall.selector;
    bytes4 constant SENT_TO_BRIDGE = IBridgeable.sentToBridge.selector;
    bytes4 constant RECEIVED_FROM_BRIDGE =
        ...
        returns (bytes4[] memory funcs)
    {
        ...
        funcs[1] = TRANSFER_FROM;
        funcs[2] = TRANSFER_CALL;
        funcs[3] = SENT_TO_BRIDGE;
        // Received from bridge was omitted intentionally
        // Validation MUST happen on send side only
        // Reverting on the receive side is bad, requires manual intervention to get funds unstuck
    }

    function _beforeHook(
        Message calldata _msg
    ) internal virtual override returns (bytes memory result) {
        address caller = _msg.sender;
        address fromAddress;
        address toAddress;
        uint256 amount;

        if (_msg.sig == TRANSFER) {
            fromAddress = _msg.sender;
            (toAddress, amount) = abi.decode(_msg.data[4:], (address, uint256));
        } else if (_msg.sig == TRANSFER_FROM) {
            (fromAddress, toAddress, amount) = abi.decode(
                _msg.data[4:],
                (address, address, uint256)
            );
        } else if (_msg.sig == TRANSFER_CALL) {
            fromAddress = _msg.sender;
            (toAddress, amount, ) = abi.decode(
                _msg.data[4:],
                (address, uint256, bytes)
            );
        } else if (_msg.sig == SENT_TO_BRIDGE) {
            BridgeArgs memory args = abi.decode(_msg.data[4:], (BridgeArgs));
            (caller, fromAddress, toAddress, amount) = (
                args.caller,
                args.fromAddress,
                args.toAddress,
                args.amount
        ...
        return
            checkTransfer(msg.sender, caller, fromAddress, toAddress, amount)
                ? TRUE
                : FALSE;
    }
