...
// File: contracts/token/BabyAIDoge.sol
pragma solidity >=0.8.0;
 ...
 */
contract BabyAIDoge is ERC20, Ownable {
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, "WoofWoof: ETH_TRANSFER_FAILED");
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "WoofWoof: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "WoofWoof: index out of bounds");
