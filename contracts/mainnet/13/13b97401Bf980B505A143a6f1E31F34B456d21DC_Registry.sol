    ...
    IRegistry
{
        ...
        returns (Integration memory result)
    {
        result = Integration({
            name: bytes11(input),
            integrationType: uint8(bytes1(input << 88)) == 0 ? IntegrationType.Bridge :
                uint8(bytes1(input << 88)) == 1 ? IntegrationType.Farm :
                IntegrationType.Dex,
        ...
        returns (AccountPosition[] memory result)
    {
        ...
        for (uint8 i; i < itrxn.length; i++) {
            ...
            temp[count].integration = itrxn[i];
            temp[count].position = IFarm(itrxn[i].integration).position(user);
            count++;
        }
