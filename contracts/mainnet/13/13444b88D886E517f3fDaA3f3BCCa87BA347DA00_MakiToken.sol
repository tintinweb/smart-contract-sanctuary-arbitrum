...
contract MakiToken is ERC20Permit, VaultOwned {
    ...
    uint256 public maxHoldAmount = 1000_000_000_000;
    mapping(address => bool) public pair;
    mapping(address => bool) public tasteApproved;
    mapping(address => bool) public isExcludeFromMax;
    bool private tasteVerificationRequired = true;
    bool public enableMaxLimit;
    
    modifier checkMaxLimit(address _addr) {
      _;
      if(enableMaxLimit) {
        if(!isExcludeFromMax[_addr]) {
          require(balanceOf(_addr) <= maxHoldAmount, "exceed max hold amount");
        }
      }
    }
    ...
    constructor(address _dao, address headChef) ERC20("Uramaki", "MAKI", 9) {
      dao = _dao;
      _headChef = headChef;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override checkMaxLimit(recipient) returns (bool) {
      uint256 fee = 0;
      if(pair[recipient])
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override checkMaxLimit(recipient) returns (bool) {
      uint256 fee = 0;
      if(pair[recipient])
        fee = amount.mul(sellFee).div(10000);
      if(pair[sender]) {
    ...
    function setFee(uint256 _buy, uint256 _sell) public onlyOwner {
      require(buyFee <= 1000 && sellFee <= 1000, "invalid fee value");
      buyFee = _buy;
      sellFee = _sell;
    }
    ...
    function excludeFromMaxLimit(address _addr) public onlyOwner {
      isExcludeFromMax[_addr] = true;
    }

    function includeFromMaxLimit(address _addr) public onlyOwner {
      isExcludeFromMax[_addr] = false;
    }
