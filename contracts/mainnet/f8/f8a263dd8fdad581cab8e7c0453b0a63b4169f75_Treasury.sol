...
interface IMasonry {
    ...
    function getATOMBPrice() external view returns (uint256);
...
contract Treasury is ContractGuard {
    ...
    uint256 public daoFunAsharedPercent;

    address public devFund1;
    address public devFund2;
    address public devFund3;
    address public devFund4;   
    uint256 public devFunAsharedPercent;

    /* =================== Events =================== */

    event Initialized(address indexed executor, uint256 at);
    event BurnerBonds(address indexed from, uint256 bondAmount);
    event RedeemerBonds(address indexed from, uint256 ATOMBAmount, uint256 bondAmount);
    event BoughtBonds(address indexed from, uint256 ATOMBAmount, uint256 bondAmount);
    ...
    modifier checkEpoch {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getATOMBPrice() > ATOMBPriceCeiling) ? 0 : getATOMBCirculatingSupply().mul(maxSupplyContractionPercent).div(10000);
    }
    ...
    function getBurnableATOMBLeft() public view returns (uint256 _burnableATOMBLeft) {
        uint256 _ATOMBPrice = getATOMBPrice();
        if (_ATOMBPrice <= ATOMBPriceOne) {
            uint256 _ATOMBSupply = getATOMBCirculatingSupply();
            uint256 _bondMaxSupply = _ATOMBSupply.mul(maxDebtRatioPercent).div(10000);
            uint256 _bondSupply = IERC20(abond).totalSupply();
            if (_bondMaxSupply > _bondSupply) {
                uint256 _maxMintableBond = _bondMaxSupply.sub(_bondSupply);
                uint256 _maxBurnableATOMB = _maxMintableBond.mul(_ATOMBPrice).div(1e18);
                _burnableATOMBLeft = Math.min(epochSupplyContractionLeft, _maxBurnableATOMB);
            }
        }
    }
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_ATOMBPrice <= ATOMBPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _discountAmount = _bondAmount.sub(ATOMBPriceOne).mul(discountPercent).div(10000);
    ...
    function getBondPremiumRate() public view returns (uint256 _rate) {
        uint256 _ATOMBPrice = getATOMBPrice();
        if (_ATOMBPrice > ATOMBPriceCeiling) {
            uint256 _ATOMBPricePremiumThreshold = ATOMBPriceOne.mul(premiumThreshold).div(100);
            if (_ATOMBPrice >= _ATOMBPricePremiumThreshold) {
                //Price > 1.10
                uint256 _premiumAmount = _ATOMBPrice.sub(ATOMBPriceOne).mul(premiumPercent).div(10000);
    ...
    function setATOMBPriceCeiling(uint256 _ATOMBPriceCeiling) external onlyOperator {
        require(_ATOMBPriceCeiling >= ATOMBPriceOne && _ATOMBPriceCeiling <= ATOMBPriceOne.mul(120).div(100), "out of range"); // [$1.0, $1.2]
    ...
    function setExtraFunds(
        address _daoFund,
        uint256 _daoFunAsharedPercent,
        address _devFund1,
        address _devFund2,
        address _devFund3,
        address _devFund4,
        uint256 _devFunAsharedPercent
    ) external onlyOperator {
        require(_daoFund != address(0), "zero");

        require(_daoFunAsharedPercent <= 1000, "out of range"); // <= 30%
        ...
        require(_devFunAsharedPercent <= 500, "out of range"); // <= 10%
        daoFund = _daoFund;
        daoFunAsharedPercent = _daoFunAsharedPercent;
        devFund1 = _devFund1;
        devFund2 = _devFund2;
        devFund3 = _devFund3;
        devFund4 = _devFund4;

        devFunAsharedPercent = _devFunAsharedPercent;
    }
    ...
    function setPremiumThreshold(uint256 _premiumThreshold) external onlyOperator {
        require(_premiumThreshold >= ATOMBPriceCeiling, "_premiumThreshold exceeds ATOMBPriceCeiling");
    ...
    function buyBonds(uint256 _ATOMBAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        require(_ATOMBAmount > 0, "Treasury: cannot purchase bonds with zero amount");
        ...
        require(_ATOMBAmount <= epochSupplyContractionLeft, "Treasury: not enough bond left to purchase");
        ...
        require(newBondSupply <= ATOMBSupply.mul(maxDebtRatioPercent).div(10000), "over max debt ratio");

        IBasisAsset(ATOMB).burnFrom(msg.sender, _ATOMBAmount);
        IBasisAsset(abond).mint(msg.sender, _bondAmount);

        epochSupplyContractionLeft = epochSupplyContractionLeft.sub(_ATOMBAmount);
        _updateATOMBPrice();
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(IERC20(ATOMB).balanceOf(address(this)) >= _ATOMBAmount, "Treasury: treasury has no more budget");

        seigniorageSaved = seigniorageSaved.sub(Math.min(seigniorageSaved, _ATOMBAmount));
        ...
        emit RedeemerBonds(msg.sender, _ATOMBAmount, _bondAmount);
    }

    function _sendToMasonry(uint256 _amount) internal {
        IBasisAsset(ATOMB).mint(address(this), _amount);

        uint256 _daoFunAsharedAmount = 0;
        if (daoFunAsharedPercent > 0) {
            _daoFunAsharedAmount = _amount.mul(daoFunAsharedPercent).div(10000);
            IERC20(ATOMB).transfer(daoFund, _daoFunAsharedAmount);
            emit DaoFundFunded(now, _daoFunAsharedAmount);
        }

        uint256 _devFunAsharedAmount = 0;
        if (devFunAsharedPercent > 0) {
            _devFunAsharedAmount = _amount.mul(devFunAsharedPercent).div(10000);
            IERC20(ATOMB).transfer(devFund1, _devFunAsharedAmount.div(4));
            IERC20(ATOMB).transfer(devFund2, _devFunAsharedAmount.div(4));
            IERC20(ATOMB).transfer(devFund3, _devFunAsharedAmount.div(4));
            IERC20(ATOMB).transfer(devFund4, _devFunAsharedAmount.div(4));
            emit DevFundFunded(now, _devFunAsharedAmount);
        }

        _amount = _amount.sub(_daoFunAsharedAmount).sub(_devFunAsharedAmount);

        IERC20(ATOMB).safeApprove(masonry, 0);
        IERC20(ATOMB).safeApprove(masonry, _amount);
    ...
    function _sendToBondTreasury(uint256 _amount) internal {
        uint256 treasuryBalance = IERC20(ATOMB).balanceOf(bondTreasury);
        ...
        if (_amount > unspent) {
            IBasisAsset(ATOMB).mint(bondTreasury, _amount.sub(unspent));
            emit RebateTreasuryFunded(now, _amount.sub(unspent));
        }
    }

    function _calculateMaxSupplyExpansionPercent(uint256 _ATOMBSupply) internal returns (uint256) {
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        _sendToBondTreasury(ATOMBSupply.mul(bondSupplyExpansionPercent).div(10000));
        if (epoch < bootstrapEpochs) {
            // 28 first epochs with 4.5% expansion
            _sendToMasonry(ATOMBSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochATOMBPrice > ATOMBPriceCeiling) {
                // Expansion ($ATOMB Price > 1 $FTM): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(abond).totalSupply();
                uint256 _percentage = previousEpochATOMBPrice.sub(ATOMBPriceOne);
                uint256 _savedForBond;
                uint256 _savedForMasonry;
                uint256 _mse = _calculateMaxSupplyExpansionPercent(ATOMBSupply).mul(1e14);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    // saved enough to pay debt, mint as usual rate
                    _savedForMasonry = ATOMBSupply.mul(_percentage).div(1e18);
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = ATOMBSupply.mul(_percentage).div(1e18);
