...
// File: contracts/pool/ShibaJackpot.sol


pragma solidity =0.8.19;

contract ShibaJackpotV4 is Initializable, OwnableUpgradeable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function addCaller(address val) public onlyOwner() {
        require(val != address(0), "ShibaJackpot: val is the zero address");
        _callers.add(val);
    }

    function delCaller(address caller) public onlyOwner returns (bool) {
        require(caller != address(0), "ShibaJackpot: caller is the zero address");
