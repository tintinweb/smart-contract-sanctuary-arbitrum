    ...
    function combine(bytes memory inputData) public view returns (bytes memory, bytes memory, bytes memory) {
        ...
        assembly {
            ...
            for { let callNumber := 0 } lt(callNumber, callnum) { callNumber := add(callNumber, 1) } {
                ...
                if or( or( or( or( eq(callType, 2), eq(callType, 4)), eq(callType, 5)), eq(callType, 6)), eq(callType, 7)) {

                    {
                        ...
                        if or( eq(callType, 4), eq(callType, 5) ) {
                            ...
                            // replace call data with computed value
                        ...
                        if eq(callType, 6) {
                            ...
                            // replace call data with last result's value
                            mstore(add(ptr, 32),  mload(
                                    sub(thisOutputPointer, 32)
                                )
                            )
