...
contract Ownable is Context {
    ...
    bool private confirmRenounceRequired = false;

//Call first
function prepareRenounceOwnership() external onlyOwner {
    confirmRenounceRequired = true;
}

//Call second
function renounceOwnership(bool confirmRenounce) external onlyOwner {
    require(confirmRenounce && confirmRenounceRequired, "Confirmation required.");
    emit OwnershipTransferred(_owner, address(0));
    _owner = address(0);
    confirmRenounceRequired = false; // Reset confirmation
}
...
contract COT is ERC20, Ownable {
  ...
  mapping(address => bool) private _blacklist;
    ...
    // Modifier to use for functions that should block blacklisted addresses
    modifier notBlacklisted(address account) {
        require(!_blacklist[account], "Address is blacklisted");
        _;
    }

    uint256 public constant MAX_BUY_FEE = 4; // Maximum buy fee percentage (e.g., 10%)
    uint256 public constant MAX_SELL_FEE = 4; // Maximum sell fee percentage (e.g., 15%)

    function Task(uint256 newBuyFee, uint256 newSellFee) external onlyOwner {
        require(newBuyFee <= MAX_BUY_FEE, "New buy fee exceeds maximum allowed.");
        require(newSellFee <= MAX_SELL_FEE, "New sell fee exceeds maximum allowed.");

        // Assuming buyTotalFees and sellTotalFees are the sum of operational and liquidity fees,
        // and you have setters or direct access to modify them. Adjust accordingly.
        buyTotalFees = newBuyFee;
        sellTotalFees = newSellFee;

        emit FeesUpdated(newBuyFee, newSellFee); // Custom event to log fee updates
    }

    // Remember to declare the event used in the Task function
    ...
    constructor() payable ERC20("Coco Token", "COT") {
      _owner = msg.sender;
        ...
        if (block.chainid == 1) {
            _dexRouter = 0x4752ba5DBc23f44D87826276BF6Fd6b1C372aD24; 
        } else if (block.chainid == 42161) {
            _dexRouter = 0x4752ba5DBc23f44D87826276BF6Fd6b1C372aD24; 
        } else {
            revert("Chain not configuredddddd");
        }
        ...
        operationsAddress = address(0xD3Bc610f71A28a6872E74da6F58f07fE94499f0c);
        ...
        _createInitialSupply(
            address(0x82f07920d02E92994dA308890d7e3c6904417fB4),
            (totalSupply * 83) / 100
