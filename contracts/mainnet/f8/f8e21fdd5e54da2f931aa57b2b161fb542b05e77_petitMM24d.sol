 ...
 * {ERC721Enumerable}.
 */
abstract contract ERC721 is Context, IERC721, IERC721Metadata, IERC721Errors {
    // Token name
     ...
     * core ERC721 logic MUST check to keep balances
...
/// @title petitMM24d
/// @author glbkst
/// @notice ERC721 contract, metadata and payable can be modified
contract petitMM24d is ERC721, Ownable {
    ...
    constructor(address initialOwner, string memory nameIn, string memory symbolIn, uint256 maxTokensIn)
        ERC721(nameIn, symbolIn)
        Ownable(initialOwner)
    {
       _paused = true;
       _status = NOT_ENTERED;
       _mintPayable = 150000 gwei;
       _maxTokens = maxTokensIn;
    }
    ...
    function _nonReentrantBefore() private {
        ...
        if (_status == ENTERED) {
            revert ERC721InvalidOperator(address(this));
        }
    ...
    function mint() public payable {
        if (msg.value != _mintPayable) revert PayableWrong(msg.value, _mintPayable);
        mintInternal();       
    }

     function mintInternal() private nonReentrant {
        if( _nextTokenId + 1 > _maxTokens) revert ERC721NonexistentToken(_nextTokenId+1);
    ...
    {
        _requireOwned(tokenId);
        if(bytes(_uri).length == 0) return super.tokenURI(tokenId);
        uri = _uri;
    }
