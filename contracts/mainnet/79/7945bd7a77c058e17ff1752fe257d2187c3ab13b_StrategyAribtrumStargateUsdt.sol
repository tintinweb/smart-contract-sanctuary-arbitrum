/**
 *Submitted for verification at Arbiscan on 2022-03-28
*/
...
abstract contract StrategyBase {
    ...
    function _distributePerformanceFeesBasedAmountAndDeposit(uint256 _amount)
        internal
    {
        uint256 _want = IERC20(want).balanceOf(address(this));
...
// File src/interfaces/star-chef.sol
...
// File src/interfaces/balancer.sol
...
// File src/strategies/arbitrum/strategy-stargate-farm-base.sol
...
abstract contract StrategyStargateFarmBase is StrategyBase {
    ...
    address public constant starchef =
    ...
    mapping(address => PoolRoute[]) public poolRoutes;

    uint256 public poolId; // poolId on staking contract
    uint256 public lpPoolId; // poolId on Stargate router
    ...
    constructor(
        ...
        IERC20(stg).approve(vault, type(uint256).max);
        IERC20(want).approve(starchef, type(uint256).max);
    }

    function balanceOfPool() public view override returns (uint256) {
        (uint256 amount, ) = IStarchef(starchef).userInfo(
            poolId,
            address(this)
        );
        return amount;
    ...
    function harvest() public override {
        ...
        if (poolRoutes[underlying].length > 0) {
            for (uint256 i = 0; i < poolRoutes[underlying].length; i++) {
                address tokenIn = poolRoutes[underlying][i].tokenIn;
                uint256 _tokenIn = IERC20(tokenIn).balanceOf(address(this));
                //Allow Balancer to sell rewards
                IERC20(tokenIn).safeApprove(vault, 0);
                IERC20(tokenIn).safeApprove(vault, _tokenIn);

                IBalancerVault.SingleSwap memory swapParams = IBalancerVault
                    .SingleSwap({
                        poolId: poolRoutes[underlying][i].poolId,
                        kind: IBalancerVault.SwapKind.GIVEN_IN,
                        assetIn: IAsset(tokenIn),
                        assetOut: IAsset(poolRoutes[underlying][i].tokenOut),
                        amount: _tokenIn,
                        userData: "0x"
                    });
        ...
        IStarRouter(starRouter).addLiquidity(
            lpPoolId,
            _underlying,
            address(this)
        );
...
// File src/strategies/arbitrum/stargate/strategy-stargate-usdt.sol

pragma solidity ^0.8.6;

contract StrategyAribtrumStargateUsdt is StrategyStargateFarmBase {
    // Token addresses
    address public constant _lp = 0xB6CfcF89a7B22988bfC96632aC2A9D6daB60d641;
    address public constant _underlying =
        0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9;
    address public usdt = 0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9;
    address public usdc = 0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8;

    uint256 public constant _lpPoolId = 2;

    uint256 public constant _poolId = 1;
    ...
    function getName() external pure override returns (string memory) {
        return "StrategyAribtrumStargateUsdt";
    }
}