...
interface IMarket {
   ...
   */
  function getBorrowLimit(address account, IOracle.Confidence confidence) external view returns (uint256);
   ...
   * @return The borrowing limit that is available for the list of provided assets.
   */
  function getBorrowLimit(IPortfolioStorage.PortfolioAsset[] memory portfolioAssets, IOracle.Confidence confidence) external view returns (uint256);
...
interface IOracle {
  ...
  enum Confidence {
    /// @dev the answer should be accepted.
    OK,
    /// @dev the answer was available, but it was stale.
    STALE,
    /// @dev the answer was not available.
    OFFLINE
  }
...
library Constants {
  ...
  address public constant AUSD = 0x7d0Bb2a91507D96a4E77E37759603DFCcf4F8Cb2;
...
import { IAddressRegistry } from "../../interfaces/core/IAddressRegistry.sol";
import { IAssetStorage } from "../../interfaces/core/IAssetStorage.sol";
import { Migration } from "../../protocol/governance/Migration.sol";
import { Custodian } from "../../protocol/portfolio/Custodian.sol";
import { BPS } from "../../libraries/PercentageMath.sol";
import { Constants } from "./Constants.sol";

contract Migration_20240516_0001 is Migration { // solhint-disable-line contract-name-camelcase

  address private constant SWAPROUTER_ADAPTER = 0xB70093eb4D6F707F96A9883f145BaAcEf5F44dB1;
  address private constant WSTETH_ORACLE = 0x1432B7d3Df6F548a02603fA38Ec1652329479C46;

  bytes32 private constant AMBIT_ASSET_STORAGE = 0x7267fae8044d9c0f406ec1d6bfdfdb3a4afea229fceedb3c88cc26df2ac97809;
  bytes32 private constant AMBIT_PORTFOLIO = 0x9d8aacac4eddf5ce050e7031a356719ed884b6564e70750ad9b5329bbba04170; // ambit.portfolio
  
  constructor(IAddressRegistry registry) Migration(registry) { } // solhint-disable-line no-empty-blocks

  function migrate() internal override onlyAdmin {
    IAssetStorage.PortfolioHooks memory hooks = IAssetStorage.PortfolioHooks({
      callback: address(0x0),
      options: 0
    });

    Custodian custodian = new Custodian(_registry, IERC20Metadata(Constants.WSTETH));
    custodian.grantAuthorization(_registry.getAddress(AMBIT_PORTFOLIO));

    IAssetStorage.Asset memory asset = IAssetStorage.Asset({
      token: Constants.WSTETH,
      custodian: address(custodian),
      priceOracle: WSTETH_ORACLE,
      marketplaceAdapter: SWAPROUTER_ADAPTER,
      maxLTV: BPS.wrap(7000), // 70%
      liquidationDiscount: BPS.wrap(1200), // 5%
      maxSupply: 5 * 10 ** 18, // 5
      hooks: hooks
    });

    IAssetStorage assetStorage = IAssetStorage(_registry.getAddress(AMBIT_ASSET_STORAGE));
    assetStorage.setAsset(asset);
...
import { IAddressRegistry } from "../../interfaces/core/IAddressRegistry.sol";
import { IExecutable } from "../../interfaces/governance/IExecutable.sol";
...
abstract contract Migration is AuthorizedAccessControl, IExecutable {
  ...
  modifier onlyGovernor() {
    if (msg.sender != address(_registry.getGovernor())) {
  ...
  function execute() external override onlyGovernor {
    require(_complete == false, "migration already run");
    _complete = true;

    IAccessControlList acl = _registry.getAccessControlList();
    require (acl.hasRole(ADMIN_ROLE, address(this)) == true, "migration doesnt have permission");
    
    migrate();
  }

  function migrate() internal virtual onlyAdmin { } // solhint-disable-line no-empty-blocks
...
abstract contract BaseCustodian is Vault, AuthorizedAccessControl, Pausable, ICustodian {
  ...
  function getTotalShares() public virtual view returns (uint256) {
    return this.getTotalAssets();
  }

  /// @inheritdoc ICustodian
  function toShares(uint256 amount) public virtual view returns (uint256 shares) {
    return amount;
  }

  /// @inheritdoc ICustodian
  function toAssets(uint256 shares) public virtual view returns (uint256 assets) {
    return shares;
  }

  /// @inheritdoc ICustodian
  function previewSupply(uint256 amount) public virtual view returns (uint256 shares) {
    return toShares(amount);
  }

  /// @dev called before the assets are transferred from the supplier.
  function beforeSupply(address, uint256, uint256) internal virtual { } // solhint-disable-line no-empty-blocks

  /// @inheritdoc ICustodian
  function supply(address supplier, uint256 amount) external onlyAuthorized whenNotPaused requireNonZeroAmount(amount) returns (uint256 shares) {
    if (amount > _underlyingAsset.balanceOf(supplier)) {
      revert Errors.Custodian_BalanceExceeded({
        amount: amount,
        balance: _underlyingAsset.balanceOf(supplier)
      });
    }
    ...
    emit Supply(msg.sender, supplier, address(_underlyingAsset), amount, shares);
  }

  /// @inheritdoc ICustodian
  function previewWithdraw(uint256 amount) public virtual view returns (uint256 shares) {
    return toShares(amount);
  }

  /// @dev called before the assets are transferred to the receiver.
  function beforeWithdraw(address, uint256, uint256) internal virtual { } // solhint-disable-line no-empty-blocks

  /// @inheritdoc ICustodian
  function withdraw(address receiver, uint256 amount) external onlyAuthorized whenNotPaused requireNonZeroAmount(amount) returns (uint256 shares) {
    if (amount > this.getTotalAssets()) {
      revert Errors.Custodian_BalanceExceeded({
        amount: amount,
        balance: this.getTotalAssets()
      });
    }

    shares = previewWithdraw(amount);
    
    beforeWithdraw(receiver, amount, shares);

    _underlyingAsset.safeTransfer(receiver, amount);

    emit Withdraw(msg.sender, receiver, address(_underlyingAsset), amount, shares);
  }
}
...
import { Migratable } from "../utils/Migratable.sol";
import { Vault } from "../vault/Vault.sol";
import { BaseCustodian } from "./BaseCustodian.sol";

/// @dev The default Custodian implementation maintains its own internal accounting and treats 
/// shares and assets on a 1:1 and doesn't support yield donations. Given this it isnt susceptible
/// to share inflation attacks.
contract Custodian is BaseCustodian, Migratable, Sweepable {

  uint16 private constant MAGIC_NUMBER = 0xd34a;
  ...
  constructor(IAddressRegistry registry, IERC20Metadata underlyingAsset) 
    BaseCustodian() 
    Vault(underlyingAsset) 
    Sweepable(registry) 
    AdminAccessControl(registry) { }

  /// @dev returns the migration context that allows a new instance to be created.
  function getMigrationContext() public view override returns (bytes memory context) {
    // include a version to allow future migrations to be smarter when migrating
    uint16 version = 1;
    context = abi.encode(MAGIC_NUMBER, version, _totalAssets);
  }

  /// @dev sets the migration context.
  function setMigrationContext(bytes memory context) external override onlyAdmin onlyWhenNotMigrated {
    uint16 magic;
    uint16 version;    
    (magic, version, _totalAssets) = abi.decode(context, (uint16, uint16, uint256));

    require(magic == MAGIC_NUMBER, "magic mismatch");
    require(version == 1, "version mismatch");
  }
  ...
  /// @dev called before the assets are transferred from the supplier.
  function beforeSupply(address, uint256 amount, uint256) internal override {
    _totalAssets += amount;
  }

  /// @dev called before the assets are transferred to the receiver.
  function beforeWithdraw(address, uint256 amount, uint256) internal override {
    _totalAssets -= amount;
  }
}
...
abstract contract Migratable {
  ...
  modifier onlyWhenNotMigrated() {
    if (_migrated) {
      revert Errors.Migratable_AlreadyMigrated();
    }
    _migrated = true;
    _;
  }
  ...
  /// @dev returns the migration context that allows a new instance to be created.
  function getMigrationContext() external virtual returns (bytes memory) { } // solhint-disable-line no-empty-blocks

  /// @dev sets the migration context.
  function setMigrationContext(bytes memory context) external virtual { } // solhint-disable-line no-empty-blocks
}
...
abstract contract Vault is IVault {
  ...
  // the underlying asset that the deposits deposit into the vault
  IERC20Metadata internal immutable _underlyingAsset;

  uint8 internal immutable _decimals;

  constructor(IERC20Metadata underlyingAsset) {
    _underlyingAsset = underlyingAsset;
    _decimals = underlyingAsset.decimals();
  }

  /// @inheritdoc IVault
  function getUnderlyingAsset() public view returns (IERC20Metadata) {
