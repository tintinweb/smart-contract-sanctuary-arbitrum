...
contract XAICORPToken is Context, IERC20, Ownable {
    ...
    uint256 public liquidityFeeOnBuying = 0; // 1% will be added to the liquidity
    uint256 public marketingFeeOnBuying = 250; // 4% will be added to the marketing address

    uint256 public liquidityFeeOnSelling = 0; // 1% will be added to the liquidity
    uint256 public marketingFeeOnSelling = 250; // 4% will be added to the marketing address
    ...
    constructor() {
        ...
        marketingWallet = address(0xF101C9f1AcbdB61f1A8831483046657196661110);
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "$XAI: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "$XAI: decreased allowance or below zero"
            )
        );
        return true;
    }
    ...
    function setMinTokenToSwap(uint256 _amount) external onlyOwner {
        require(_amount > 0, "$XAI: can't be 0");
    ...
    function setMaxHoldLimit(uint256 _amount) external onlyOwner {
        require(
            ...
            "$XAI: should be greater than 0.1%"
        );
    ...
    function setBuyFeePercent(
        ...
        require(
            _lpFee.add(_marketingFee) <= percentDivider.mul(15).div(100),
            "$XAI: can't be more than 15%"
        );
    }
    ...
    function setIsBot(address holder)
        external
        onlyOwner
    {
        isBot[holder] = false;
    ...
    function enableTrading() external onlyOwner {
        require(!trading, "$XAI: already enabled");
        trading = true;
        feesStatus = true;
    ...
    function _approve(
        ...
        require(owner != address(0), "$XBAI: approve from the zero address");
        require(spender != address(0), "$XAI: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "$XAI: transfer from the zero address");
        require(to != address(0), "$XAI: transfer to the zero address");
        require(amount > 0, "$XAI: Amount must be greater than zero");
        require(!isBot[from],"Bot detected");

        if (!isExcludedFromMaxTxn[from] && !isExcludedFromMaxTxn[to]) {
            require(amount <= maxTxnLimit, "$XAI: max txn limit exceeds");

            // trading disable till launch
            if (!trading) {
                require(
                    dexPair != from && dexPair != to,
                    "$XAI: trading is disable"
                );
            }
        }
