...
contract Dice is Common {
     ...
     */
    function Dice_GetState(address player)
        external
        view
    ...
    function Dice_Play(
        ...
        require(
            diceGames[msgSender].requestID == 0,
        ...
        if (numBets == 0) {
            if (isOver) _depositBankroll(wager, tokenAddress);
    ...
    function Dice_Refund() external nonReentrant {
        address msgSender = msg.sender;
        DiceGame memory game = diceGames[msgSender];
        if (game.requestID == 0) {
        ...
        emit Dice_Refund_Event(msgSender, wagers, tokenAddress);
    }
        ...
        override
    {
        ...
        DiceGame memory game = diceGames[playerAddress];
