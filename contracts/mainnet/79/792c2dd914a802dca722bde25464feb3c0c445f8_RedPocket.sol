...
// File: SocialMining_V2/RedPocket.sol
...
contract RedPocket is Ownable {
    using SafeMath for uint256;

    /* Events */
    event sendRecord (address sender, address receiver, uint256 amount, uint256 burnAmount, uint256 time);

    /* Smart Contract */
    IERC20 public MetaX = IERC20(0xdc1B3F63944e0ff11dbaD513FE4175fc0466cE2E);
    ...
    struct _redPocket {
        uint256 accumSend;
        mapping (address => uint256) accumSendByUser;
        mapping (address => mapping (uint256 => uint256)) epochSendByUser;
    }

    mapping (address => _redPocket) public redPocket;

    function getAccumSend (address user) public view returns (uint256) {
        return redPocket[user].accumSend;
    }

    function getAccumSendByUser (address user0, address user1) public view returns (uint256) {
        return redPocket[user0].accumSendByUser[user1];
    }

    function getEpochSendByUser (address user0, address user1, uint256 epoch) public view returns (uint256) {
        return redPocket[user0].epochSendByUser[user1][epoch];
    }

    function getCurrentSendByUser (address user0, address user1) public view returns (uint256) {
        return redPocket[user0].epochSendByUser[user1][Epoch()];
    }
    ...
    function Send (address receiver, uint256 amount) public {
        ...
        MetaX.transferFrom(msg.sender, receiver, sendAmount);
        if (burnRate > 0) {
            MetaX.transferFrom(msg.sender, burnAddr, burnAmount);
        }
        redPocket[msg.sender].accumSend = redPocket[msg.sender].accumSend.add(amount);
        redPocket[msg.sender].accumSendByUser[receiver] = redPocket[msg.sender].accumSendByUser[receiver].add(amount);
        redPocket[msg.sender].epochSendByUser[receiver][Epoch()] = redPocket[msg.sender].epochSendByUser[receiver][Epoch()].add(amount);
        emit sendRecord (msg.sender, receiver, amount, burnAmount, block.timestamp);
    }
