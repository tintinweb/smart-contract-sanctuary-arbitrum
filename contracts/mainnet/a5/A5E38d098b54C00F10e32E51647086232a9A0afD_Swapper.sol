...
contract HTLC is EIP712 {
    ...
    modifier safeParams(
        ...
        require(expiry > 0, "HTLC: zero expiry");
    ...
    function refund(bytes32 orderID) external {
        ...
        require(!order.isFulfilled, "HTLC: order fulfilled");
        require(order.initiatedAt + order.expiry < block.timestamp, "HTLC: order not expired");
...
import "./htlc/HTLC.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

interface DeBridgeAdapter {
    function send(
...
contract Swapper {
    using SafeERC20 for IERC20;
    DeBridgeAdapter deBridge;

    constructor(address _deBridge) {
        deBridge = DeBridgeAdapter(_deBridge);
    }
    ...
    function RedeemAndSwap(
        ...
        HTLC htlc = HTLC(gardenHtlc);

        (, , address redeemer, , , uint amount) = htlc.orders(orderID);
        require(redeemer == msg.sender, "Swapper: can be swapped only by redeemer");

        htlc.redeem(orderID, secret);

        IERC20 token = htlc.token();

        token.safeTransferFrom(redeemer, address(this), amount);

        token.safeApprove(address(deBridge), amount);

        deBridge.send(address(token), amount, chainIdTo, receiver, permit, useAssetFee, referralCode, autoParams);
    }
}