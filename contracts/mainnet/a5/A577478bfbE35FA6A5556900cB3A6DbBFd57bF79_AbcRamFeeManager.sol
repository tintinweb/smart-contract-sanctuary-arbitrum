...
import "./interfaces/IRamsesBribes.sol";
import "./interfaces/IAbcRamChef.sol";

contract AbcRamFeeManager is OwnableUpgradeable {
    ...
    address public abcRam;

    mapping(address => IRouter01.route[]) public routes;

    uint256 public feeStaking;
    uint256 public feeBribe;
    uint256 public feeLqdr;
    uint256 public feeMultiplier;

    address public pairBribe;
    address public abcRamChef;

    address public ramsesPair;

    address public pairSecondRewarder;

    modifier keeper() {
        require(
            isKeeper[msg.sender] == true || msg.sender == owner(),
            "not keeper"
        );
        _;
    }
    ...
    modifier restricted() {
        require(
            msg.sender == abcRamManager || msg.sender == owner(),
            "not auth"
        );
        _;
    }
    ...
    function notifyRewards() external keeper {
        uint256 _before = IERC20(abcRam).balanceOf(address(this));
        uint256 i;

        for (i = 0; i < tokens.length; i++) {
            address _token = tokens[i];
            if (isToken[_token]) {
                _swapToAbcRam(_token);
            }
        }
        uint256 _after = IERC20(abcRam).balanceOf(address(this));
    ...
    function swapToAbcRam(address _token) public restricted {
        _swapToAbcRam(_token);
    }

    function _swapToAbcRam(address _token) internal {
        uint256 i;
    ...
    function setAbcRamChef(address _abcRamChef) external onlyOwner {
        require(_abcRamChef != address(0), "addr 0");
        abcRamChef = _abcRamChef;
    }

    function setPairBribe(address _pairBribe) external onlyOwner {
        require(_pairBribe != address(0), "addr 0");
        pairBribe = _pairBribe;
    }

    function setPairSecondRewarder(
        address _pairSecondRewarder
    ) external onlyOwner {
    ...
    function setAbcRamManager(address _abcRamManager) external onlyOwner {
        require(_abcRamManager != address(0), "addr 0");
        abcRamManager = _abcRamManager;
    }

    function setAbcRam(address _abcRam) external onlyOwner {
        require(_abcRam != address(0), "addr 0");
        abcRam = _abcRam;
    }

    function setRamsesPair(address _pair) external onlyOwner {
    ...
    function setFees(
        ...
        require(
            _feeStaking + _feeBribe + _feeLqdr == _feeMultiplier,
            "Invalid fee values"
        );
    ...
    function _distributeFees(uint256 _amount) internal {
        ...
        uint256 _amountBribe = _amount.mul(feeBribe).div(feeMultiplier);
        uint256 _amountLqdr = _amount.sub(_amountStaking).sub(_amountBribe);

        IERC20(abcRam).safeApprove(abcRamChef, 0);
        IERC20(abcRam).safeApprove(abcRamChef, _amountStaking);
        IAbcRamChef(abcRamChef).seedRewards(_amountStaking);

        if (treasury != address(0)) {
            IERC20(abcRam).safeTransfer(treasury, _amountLqdr);
        }

        if (pairBribe != address(0)) {
            // IERC20(abcRam).safeTransfer(pairBribe, _amountBribe);
            IERC20(abcRam).safeApprove(pairBribe, 0);
            IERC20(abcRam).safeApprove(pairBribe, _amountBribe);
            IRamsesBribes(pairBribe).notifyRewardAmount(abcRam, _amountBribe);
        }
    }
    ...
    function distributeRebaseFees(uint256 _amount) external restricted {
        uint256 _abcRamBalance = IRamsesPair(ramsesPair).reserve0();
        uint256 _abcRamStaked = IERC20(abcRam).balanceOf(abcRamChef);
        uint256 _abcRamTotal = _abcRamBalance.add(_abcRamStaked);
        uint256 _amountLqdr = _amount.mul(feeLqdr).div(feeMultiplier);
        uint256 _amountLeft = _amount;

        if (treasury != address(0)) {
            IERC20(abcRam).safeTransfer(treasury, _amountLqdr);
            _amountLeft = _amount.sub(_amountLqdr);
        }

        uint256 _amountBribe = _amountLeft.mul(_abcRamBalance).div(
            _abcRamTotal
        );
        uint256 _amountStaking = _amountLeft.mul(_abcRamStaked).div(
            _abcRamTotal
        );
        ...
        if (pairSecondRewarder != address(0)) {
            IERC20(abcRam).safeTransfer(pairSecondRewarder, _amountBribe);
        }
    }
