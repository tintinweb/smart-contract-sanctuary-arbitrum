...
import "../../interfaces/ennead/IEnneadDepositor.sol";
import "../../interfaces/ennead/IEnneadRewarder.sol";
import "../Common/StratFeeManagerInitializable.sol";
import "../../utils/GasFeeThrottler.sol";

contract StrategyEnnead is StratFeeManagerInitializable, GasFeeThrottler {
    ...
    function initialize(
        ...
        rewarder = IEnneadDepositor(depositor).tokenForPool(want);
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IEnneadDepositor(depositor).deposit(want, wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IEnneadDepositor(depositor).withdraw(want, _amount - wantBal);
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        IEnneadRewarder(rewarder).getReward(address(this));
    ...
    function balanceOfPool() public view returns (uint256) {
        return IEnneadRewarder(rewarder).balanceOf(address(this));
    }

    // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
        return IEnneadRewarder(rewarder).claimable(address(this), output);
    }
    ...
    function retireStrat() external {
        ...
        IEnneadDepositor(depositor).withdraw(want, balanceOfPool());
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(depositor, type(uint).max);
    ...
    function _removeAllowances() internal {
        IERC20(want).safeApprove(depositor, 0);
