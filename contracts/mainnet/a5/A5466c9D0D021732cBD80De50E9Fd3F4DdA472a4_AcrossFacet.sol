...
contract AcrossFacet is IBridge, ReentrancyGuard {
    ...
    function bridgeToAcross(IBridge.BridgeData memory _bridgeData, AcrossData memory _acrossData) external payable nonReentrant {
        LibPlexusUtil._isTokenDeposit(_bridgeData.srcToken, _bridgeData.amount);
        _bridgeData.amount = LibPlexusUtil._fee(_bridgeData.srcToken, _bridgeData.amount);
    ...
    function _acrossStart(BridgeData memory _bridgeData, AcrossData memory _acrossData) internal {
        ...
        emit LibData.Bridge(msg.sender, _bridgeData.dstChainId, _bridgeData.srcToken, _bridgeData.amount, "Across");
    }
}
