...
abstract contract TimeswapV2Behavior is ITimeswapV2Behavior, ERC1155, ERC20, Multicall {
  ...
  constructor(
    ...
    address _timeswapV2BorrowGivenPrincipal,
    address _timeswapV2CloseBorrowGivenPosition,
    string memory name_1155,
    string memory name_20,
    string memory symbol_20
  ) ERC1155(name_1155) ERC20(name_20, symbol_20) {
  ...
  function initialize() external override {
    ERC20(address(this)).approve(timeswapV2LendGivenPrincipal, type(uint256).max);
    ERC20(address(this)).approve(timeswapV2BorrowGivenPrincipal, type(uint256).max);
    ERC20(address(this)).approve(timeswapV2CloseBorrowGivenPosition, type(uint256).max);
    ERC1155(timeswapV2Token).setApprovalForAll(timeswapV2CloseLendGivenPosition, true);
    ERC1155(timeswapV2Token).setApprovalForAll(timeswapV2CloseBorrowGivenPosition, true);
    ERC1155(timeswapV2Token).setApprovalForAll(timeswapV2Withdraw, true);
  }
  ...
  function _handleTokenTransfer(address token, bool isToken0, uint256 amount, address spender) private {
    if (_check(token, isToken0)) {
  ...
  function _burnAndTransfer(
    ...
    // ... (previous code)
    _burnAndTransferToken(token, token0To, token0Amount);
    _burnAndTransferToken(token, token1To, token1Amount);
  }

  function _burnAndTransferToken(address token, address to, uint256 amount) private {
  ...
  function lendGivenPrincipal(
    ...
    _handleTokenTransfer(param.token, param.isToken0, param.tokenAmount, timeswapV2LendGivenPrincipal);
  ...
  function closeLendGivenPosition(
    ...
    _burnAndTransfer(param.token, param.token0To, param.token1To, token0Amount, token1Amount);

    // if (token0Amount != 0) {
    //   if (callParam.token0 < param.token) {
