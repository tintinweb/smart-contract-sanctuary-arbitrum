...
import "./IVeeClaimer.sol";

contract VeeClaimer is
IVeeClaimer,
EIP712Upgradeable,
UUPSUpgradeable,
AccessControlledUpgradeable
{
    bytes32 public constant VEE_MAINTANER_ROLE = keccak256("VEE_MAINTANER_ROLE");
    bytes32 public constant VEE_CLAIMER_ROLE = keccak256("VEE_CLAIMER_ROLE");
    bytes32 public constant VEE_CLAIM_REQUEST_TYPEHASH = keccak256("Claim(address claimer,uint256 requestId,uint256 amount)");

    address internal _token;
    mapping(uint256 => bool) internal _claimed;

    bytes32 public constant DAILY_VEE_CLAIMER_ROLE = keccak256("DAILY_VEE_CLAIMER_ROLE");

    mapping(address => uint256) private _lastClaimDate;
    mapping(address => uint256) private _lastClaimRequestId;
    ...
    function initialize(
        ...
        __EIP712_init("VeeClaimer", "V1");
    }

    function claim(bytes[] calldata signature, uint256[] calldata requestIds, uint256[] calldata amounts) external {
        require(signature.length > 0, "Invalid amount of data");
        require(signature.length == requestIds.length, "Invalid amount of data");
        require(amounts.length == requestIds.length, "Invalid amount of data");
        uint count = signature.length;
        for(uint i = 0; i < count; i++) {
            require(_claimed[requestIds[i]] == false, "Already claimed");
            bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(VEE_CLAIM_REQUEST_TYPEHASH, msg.sender, requestIds[i], amounts[i])));
            address signer = ECDSAUpgradeable.recover(digest, signature[i]);
            require(_getAcl().hasRole(VEE_CLAIMER_ROLE, signer), "Invalid signature");
            require(IERC20(_token).transfer(msg.sender, amounts[i]), "Failed to send ERC20 funds");
            _claimed[requestIds[i]] = true;
            emit VeeClaimed(msg.sender, amounts[i], requestIds[i]);
        }
    }

    function claimDaily(bytes calldata signature, uint256 requestId, uint256 amount) external {
        require(signature.length > 0, "Invalid signature");

        uint256 senderLastClaimDate = _lastClaimDate[msg.sender];
        uint256 senderLastRequestId = _lastClaimRequestId[msg.sender];
        uint256 todayStart = _startOfDay(block.timestamp);

        require(
            senderLastClaimDate == 0 || (todayStart - senderLastClaimDate) >= 1 days,
            "Today's reward already claimed"
        );

        require(
            senderLastRequestId == 0 || senderLastRequestId != requestId,
            "Already claimed"
        );

        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(VEE_CLAIM_REQUEST_TYPEHASH, msg.sender, requestId, amount)));
        address signer = ECDSAUpgradeable.recover(digest, signature);

        require(
            _getAcl().hasRole(DAILY_VEE_CLAIMER_ROLE, signer),
            "Invalid signature"
        );

        _lastClaimDate[msg.sender] = todayStart;
        _lastClaimRequestId[msg.sender] = requestId;

        require(
            IERC20(_token).transfer(msg.sender, amount),
            "Failed to send ERC20 funds"
        );

        emit DailyVeeClaimed(msg.sender, amount, requestId);
    }

    function airdropVee(address[] calldata targetAddresses, uint256[] calldata amounts) external onlyRole(VEE_MAINTANER_ROLE) {
        require(targetAddresses.length == amounts.length, "Invalid amount of data");
        uint count = targetAddresses.length;
        for(uint i = 0; i < count; i++) {
            require(IERC20(_token).transfer(targetAddresses[i], amounts[i]), "Cannot transfer funds");
            emit VeeAirdropped(targetAddresses[i], amounts[i]);
        }
    }

    function withdrawFunds(address targetAddress, uint256 amount) external onlyRole(VEE_MAINTANER_ROLE) {
        require(IERC20(_token).transfer(targetAddress, amount), "Cannot transfer funds");
    }

    function setTokenAddress(address token) external onlyRole(VEE_MAINTANER_ROLE) {
        _token = token;
    ...
    view
    virtual
    override(AccessControlledUpgradeable)
    returns (bool)
    {
        return interfaceId == type(IVeeClaimer).interfaceId ||
...
interface IVeeClaimer {
    event VeeClaimed(address claimer, uint256 amount, uint256 requestId);
    event DailyVeeClaimed(address claimer, uint256 amount, uint256 requestId);
    event TokenAddressSet(address token);
    event VeeAirdropped(address wallet, uint256 amount);

    function claim(bytes[] calldata signature, uint256[] calldata requestId, uint256[] calldata amount) external;
    function claimDaily(bytes calldata signature, uint256 requestId, uint256 amount) external;
    function withdrawFunds(address targetAddress, uint256 amount) external;
    function airdropVee(address[] calldata targetAddresses, uint256[] calldata amounts) external;
