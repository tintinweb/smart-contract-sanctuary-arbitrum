...
contract MuchoBadgeWrapper is AccessControl {
    ...
    IMuchoBadgeManager public badgeManager =
        IMuchoBadgeManager(0xC439d29ee3C7fa237da928AD3A3D6aEcA9aA0717);
    IMuchoRewardRouter public rewardRouter =
        IMuchoRewardRouter(0x96D395d088C8e053f759F97695Efac4D4b45407A);
    ...
    function subscribe(
        ...
        if (!badgeManager.isSubscribedAndActive(subscriber, planId))
            badgeManager.subscribe(planId, subscriber);
...
interface IMuchoRewardRouter {
    ...
    function getUserMultiplierAndPlan(
        address _user
...
interface IMuchoBadgeManager {
    ...
    function isSubscribedAndActive(
        address _user,
        uint256 _planId
    ) external view returns (bool);

    function isSubscribedAndExpired(
        address _user,
        uint256 _planId
    ) external view returns (bool);
