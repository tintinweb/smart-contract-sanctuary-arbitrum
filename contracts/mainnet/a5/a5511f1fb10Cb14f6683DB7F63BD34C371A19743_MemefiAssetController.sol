...
interface IMemefiBurnableAsset {
    function assetsBurn(
        uint256 amount,
...
interface IMemefiMintableAsset {
    function assetsMint(
        address to,
        uint256 amount,
        uint256 erc1155id
...
import {IMemefiMintableAsset} from "./IMemefiMintableAsset.sol";
import {IMemefiBurnableAsset} from "./IMemefiBurnableAsset.sol";
...
contract MemefiAssetController is EIP712 {
    enum AssetType {
        Undefined,
        ERC721,
        ERC1155,
    ...
    constructor(address _memefimManagement) EIP712("Memefi", "1") {
        memefiManagement = IMemefiManagement(_memefimManagement);
    }
    ...
    function userMint(
        ...
        uint256[] memory mintedIds = IMemefiMintableAsset(assetAddress)
            .assetsMint(to, amount, erc1155TokenId);
    ...
    function userBurn(
        ...
        bytes32 typedHash = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "Burn(address executor,address assetAddress,uint256 assetType,uint256 erc1155TokenId,uint256[] erc721TokenIds,uint256 amount,uint256 nonce,uint256 deadline)"
                    ),
                    msg.sender,
                    assetAddress,
                    assetType,
                    erc1155TokenId,
                    keccak256(abi.encodePacked(erc721TokenIds)),
                    amount,
                    nonce,
                    deadline
                )
        ...
        IMemefiBurnableAsset(assetAddress).assetsBurn(
            amount,
            erc1155TokenId,
            erc721TokenIds
        );
