  ...
  */
  function deposit(
    ...
    if (dp.token == address(self.LRT)) {
      ...
      // If deposited is not LRT, we swap it for ETH and get the depositValue
      ...
      uint256 _depositToSwappedWNTAmt = KEPManager.swapExactTokensForTokens(self, _sp);

      // Get value of swapped deposit to LRT in USD
      _dc.depositValue = KEPReader.convertToUsdValue(
        self,
        address(self.WNT),
        _depositToSwappedWNTAmt
      );
    }
    ...
    _dc.lrtAmt += KEPManager.swapExactTokensForTokens(self, _sp2);
