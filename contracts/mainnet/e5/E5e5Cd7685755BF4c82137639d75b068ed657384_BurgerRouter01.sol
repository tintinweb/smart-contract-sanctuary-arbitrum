...
contract BurgerRouter01 {
  ...
  modifier ensure(uint deadline) {
    require(deadline >= block.timestamp, 'BurgerRouter: EXPIRED');
    _;
  }
  ...
  receive() external payable {
    ...
    require(msg.sender == address(weth), "BurgerRouter: NOT_WETH");
  }
  ...
  function _sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
    require(tokenA != tokenB, 'BurgerRouter: IDENTICAL_ADDRESSES');
    (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
    require(token0 != address(0), 'BurgerRouter: ZERO_ADDRESS');
  }
  ...
  function _quoteLiquidity(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
    require(amountA > 0, 'BurgerRouter: INSUFFICIENT_AMOUNT');
    require(reserveA > 0 && reserveB > 0, 'BurgerRouter: INSUFFICIENT_LIQUIDITY');
  ...
  function _getAmountsOut(uint amountIn, Route[] memory routes) internal view returns (uint[] memory amounts) {
    require(routes.length >= 1, 'BurgerRouter: INVALID_PATH');
  ...
  function _addLiquidity(
    ...
    require(amountADesired >= amountAMin, "BurgerRouter: DESIRED_A_AMOUNT");
    require(amountBDesired >= amountBMin, "BurgerRouter: DESIRED_B_AMOUNT");
    // create the pair if it doesn't exist yet
    address _pair = IFactory(factory).getPair(tokenA, tokenB, stable);
    if (_pair == address(0)) {
    ...
    if (reserveA == 0 && reserveB == 0) {
      ...
      uint amountBOptimal = _quoteLiquidity(amountADesired, reserveA, reserveB);
      if (amountBOptimal <= amountBDesired) {
        require(amountBOptimal >= amountBMin, 'BurgerRouter: INSUFFICIENT_B_AMOUNT');
        (amountA, amountB) = (amountADesired, amountBOptimal);
      } else {
        uint amountAOptimal = _quoteLiquidity(amountBDesired, reserveB, reserveA);
        assert(amountAOptimal <= amountADesired);
        require(amountAOptimal >= amountAMin, 'BurgerRouter: INSUFFICIENT_A_AMOUNT');
        (amountA, amountB) = (amountAOptimal, amountBDesired);
      }
    }
  }
  ...
  function _removeLiquidity(
    ...
    require(amountA >= amountAMin, 'BurgerRouter: INSUFFICIENT_A_AMOUNT');
    require(amountB >= amountBMin, 'BurgerRouter: INSUFFICIENT_B_AMOUNT');
  }
  ...
  function swapExactTokensForTokensSimple(
    ...
    require(amounts[amounts.length - 1] >= amountOutMin, 'BurgerRouter: INSUFFICIENT_OUTPUT_AMOUNT');
  ...
  {
    require(routes[0].from == address(weth), 'BurgerRouter: INVALID_PATH');
  ...
  returns (uint[] memory amounts)
  {
    require(routes[routes.length - 1].to == address(weth), 'BurgerRouter: INVALID_PATH');
    amounts = _getAmountsOut(amountIn, routes);
    require(amounts[amounts.length - 1] >= amountOutMin, 'BurgerRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    IERC20(routes[0].from).safeTransferFrom(
      msg.sender, _pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]
  ...
  function swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    require(
      ...
      'BurgerRouter: INSUFFICIENT_OUTPUT_AMOUNT'
    );
  }
  ...
  ensure(deadline)
  {
    ...
    require(amountOut >= amountOutMin, 'BurgerRouter: INSUFFICIENT_OUTPUT_AMOUNT');
  ...
  function _safeTransferETH(address to, uint value) internal {
    ...
    require(success, 'BurgerRouter: ETH_TRANSFER_FAILED');
  }
}
