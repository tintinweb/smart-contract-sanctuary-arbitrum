// contracts/PowerTokenVesting.sol
...
import {ReentrancyGuard as solReentrancyGuard} from "solmate/utils/ReentrancyGuard.sol";
import { SD59x18, sd, convert } from "@prb/math/SD59x18.sol";

/**
 * @title PowerTokenVesting
 */
contract PowerTokenVesting is Owned, solReentrancyGuard {
    SD59x18 MONTH = sd(2_628_000e18);
    struct VestingSchedule {
        ...
        bool revoked;
        // alpha parameter for power law
        SD59x18 alpha;
        // beta parameter for power law
        SD59x18 beta;
    }

    // address of the solERC20 token
    solERC20 private immutable _token;
     ...
     * @param token_ address of the solERC20 token contract
     */
    constructor(address token_) Owned(msg.sender) {
        // Check that the token address is not 0x0.
        require(token_ != address(0x0));
        // Set the token address.
        _token = solERC20(token_);
    }
    ...
    function createVestingSchedule(
        ...
        require(
            ...
            "PowerTokenVesting: cannot create vesting schedule because not sufficient tokens"
        );
        require(_duration > 0, "PowerTokenVesting: duration must be > 0");
        require(_amount > 0, "PowerTokenVesting: amount must be > 0");
        require(
            _slicePeriodSeconds >= 1,
            "PowerTokenVesting: slicePeriodSeconds must be >= 1"
        );
        require(_duration >= _cliff, "PowerTokenVesting: duration must be >= cliff");
    ...
    function revoke(
        ...
        require(
            ...
            "PowerTokenVesting: vesting is not revocable"
        );
    ...
    function withdraw(uint256 amount) external nonReentrant onlyOwner {
        require(
            ...
            "PowerTokenVesting: not enough withdrawable funds"
        );
        /*
    ...
    function release(
        ...
        require(
            ...
            "PowerTokenVesting: only beneficiary and owner can release vested tokens"
        );
        uint256 vestedAmount = _computeReleasableAmount(vestingSchedule);
        require(
            vestedAmount >= amount,
            "PowerTokenVesting: cannot release tokens, not enough vested tokens"
        );
    ...
    function getVestingIdAtIndex(
        ...
        require(
            ...
            "PowerTokenVesting: index out of bounds"
        );
    ...
    /**
     * @dev Returns the address of the solERC20 token managed by the vesting contract.
     */
    ...
    function _computeReleasableAmount(
        ...
        // Otherwise, some tokens are releasable.
            ...
            int256 vestedSeconds = int256(vestedSlicePeriods * secondsPerSlice);
            // Compute the amount of tokens that are vested.

            // convert into fixed point decimal numbers
            SD59x18 t = sd(vestedSeconds * 1e18) / sd(int256(vestingSchedule.duration * 1e18)); // time between 0-1 for total duration
            SD59x18 vestedPerToken = t.pow(vestingSchedule.beta * t - (vestingSchedule.alpha * (t-SD59x18.wrap(1e18))));
            SD59x18 vestedAmount = vestedPerToken * sd(int256(vestingSchedule.amountTotal) * 1e18);
            // Subtract the amount already released and return.
            return uint256(convert(vestedAmount)) - vestingSchedule.released;
        }
    }
