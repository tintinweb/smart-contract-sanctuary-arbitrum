...
contract Cards is ERC1155, Ownable {
    ...
    function uri(uint256 tokenId) override public view returns (string memory) {
        require(bytes(cardAttributesMap[tokenId].ipfsHash).length > 0, "URI query for nonexistent token");
        return string(abi.encodePacked("ipfs://", cardAttributesMap[tokenId].ipfsHash));
    }

    
    // Getters for individual attributes
    function getHealth(uint256 tokenId) public view returns (uint256) {
        require(cardAttributesMap[tokenId].health > 0, "Card not minted");
        return cardAttributesMap[tokenId].health;
    }

    function getStrength(uint256 tokenId) public view returns (uint256) {
        require(cardAttributesMap[tokenId].strength > 0, "Card not minted");
        return cardAttributesMap[tokenId].strength;
    }

    function getProvision(uint256 tokenId) public view returns (uint256) {
        require(cardAttributesMap[tokenId].provision > 0, "Card not minted");
        return cardAttributesMap[tokenId].provision;
    }

    function getCardType(uint256 tokenId) public view returns (string memory) {
        require(bytes(cardAttributesMap[tokenId].cardType).length > 0, "Card not minted");
        return cardAttributesMap[tokenId].cardType;
    }

    function getIpfsHash(uint256 tokenId) public view returns (string memory) {
        require(bytes(cardAttributesMap[tokenId].ipfsHash).length > 0, "Card not minted");
