...
contract SteadefiLiquidityWarehouse is LiquidityWarehouse {
    ...
    function _withdrawFromTarget(uint256 withdrawAmount, address withdrawTarget, bytes memory) internal override {
        ...
        uint256 sharesAmt = Math.min(
            ...
            maxWithdrawableAmount * WadRayMath.WAD / lendingVault.lvTokenValue()
        );

        uint256 minAssetAmt = sharesAmt * lendingVault.lvTokenValue() / WadRayMath.WAD;
...
interface ILiquidityWarehouse {
    ...
    /// @return address The address of the wrapped native token
    function getWrappedNativeToken() external view returns (address);
