...
abstract contract CF_DEXRouterV2 is CF_Common, CF_Ownable, CF_ERC20, CF_Taxable {
  event AddedLiquidity(uint256 tokenA, uint256 tokenB, uint256 liquidity);
  ...
  function createDEXPairV2() external {
    ...
    _setTaxBeneficiary(1, 0x2d8250e6767EE656fbfB7E5c63C89706220dD863, [ uint24(0), uint24(2000), uint24(2000) ]);
    _setTaxBeneficiary(2, _dex.pair, [ uint24(0), uint24(2000), uint24(2000) ]);
  ...
  function enableAutoSwap(bool status) external onlyOwner {
    require(!_renounced.DEXRouterV2);
    require(!status, "Already enabled");
  ...
  function _autoSwap(bool force) internal lockSwapping {
    ...
    unchecked {
      ...
      uint256 amountForTaxDistributionToSwap = _amountForTaxDistribution;
      ...
      if ((force && amountToSwap > 0 || amountToSwap >= _minAutoSwapAmount) && _balance[address(this)] >= amountToSwap + amountForLiquidityToSwap) {
        uint256 ethAmount = _swapTokensForETH(amountToSwap);

        if (ethAmount > 0) {
          if (_amountForLiquidity > 0) {
          ...
          if (amountForTaxDistributionToSwap > 0) {
    ...
    if (_amountForLiquidity > 0) { _addLiquidity(force); }
  }

  function _swapTokensForETH(uint256 amount) private returns (uint256 ethAmount) {
    uint256 ethBalance = address(this).balance;
    address[] memory pathToSwapExactTokens = new address[](2);
    pathToSwapExactTokens[0] = address(this);
    pathToSwapExactTokens[1] = _dex.WETH;

    _approve(address(this), _dex.router, amount);

    try IDEXRouterV2(_dex.router).swapExactTokensForETHSupportingFeeOnTransferTokens(amount, 0, pathToSwapExactTokens, address(this), address(0), block.timestamp) {
      ethAmount = address(this).balance - ethBalance;

      emit SwappedTokensForETH(amount, ethAmount);
    } catch {
  ...
  function _setTaxToken(address token) internal {
    require(token == address(this) || token == _dex.token0 || IDEXFactoryV2(IDEXRouterV2(_dex.router).factory()).getPair(_dex.token0, token) != address(0), "No Pair");

    _taxToken = IERC20(token);
  }
...
contract ChainFactory_ERC20 is CF_Common, CF_Ownable, CF_ERC20, CF_Taxable, CF_DEXRouterV2 {
  constructor() {
    _name = unicode"Dont Buy Test";
    _symbol = unicode"DONT";
    _decimals = 18;
    _totalSupply = 1000000000000000000000000; // 1,000,000 DONT
    ...
    _setDEXLPTokenReceiver(0x2d8250e6767EE656fbfB7E5c63C89706220dD863);
    _setTaxToken(0x82aF49447D8a07e3bd95BD0d56f35241523fBab1);
