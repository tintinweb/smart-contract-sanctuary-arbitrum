...
contract GmxUtils is IOrderCallbackReceiver, Initializable, Ownable2StepUpgradeable {
  ...
  modifier validCallback(bytes32 key) {
    ...
    if (queue.requestKey != key) {
      ...
      bytes32[] memory orderKeys = IDataStore(dataStore).getBytes32ValuesAt(accountOrderListKey, 0, 10);
      bool condition = false;
      for (uint8 i = 0; i < orderKeys.length; i++) {
        if (orderKeys[i] == key) {
          condition = true;
        }
      }
      require(condition, "invalid request key");
    }
    _;
  }
  ...
  function getPositionInfo(
    ...
    if (positionInfo.pnlAfterPriceImpactUsd >= 0) {
      netValue = positionInfo.position.numbers.collateralAmount * prices.shortTokenPrice.min + 
        uint256(positionInfo.pnlAfterPriceImpactUsd)
      ...
      netValue = positionInfo.position.numbers.collateralAmount * prices.shortTokenPrice.min - 
        (uint256(-positionInfo.pnlAfterPriceImpactUsd) + positionInfo.fees.borrowing.borrowingFeeUsd)
        - positionInfo.fees.funding.fundingFeeAmount * prices.shortTokenPrice.min
        - positionInfo.fees.positionFeeAmount * prices.shortTokenPrice.min;
    }
  ...
  // check the case that other user use this function as his callback
  function afterOrderExecution(bytes32 key, Order.Props memory order, EventLogData memory /* eventData */) validCallback(key) external override {
    delete queue;
    if (order.numbers.orderType != Order.OrderType.MarketSwap) {
      ...
      try gExchangeRouter.claimFundingFees(markets, tokens, perpVault) {} catch {
  ...
  function afterOrderFrozen(bytes32 key, Order.Props memory order, EventLogData memory /* eventData */) validCallback(key) external override {}
