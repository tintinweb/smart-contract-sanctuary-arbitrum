...
import { ILiquidityFeeToken } from "../interfaces/ILiquidityFeeToken.sol";

contract LiquidityFeeTokenFactory is TokenFactoryBase {
    ...
    function create(
        ...
        token = Clones.cloneDeterministic(implementation, keccak256(abi.encodePacked(msg.sender, name, symbol, totalSupply, feeSettings)));
        ILiquidityFeeToken(token).initialize(
            msg.sender,
            name,
            symbol,
            totalSupply,
        ...
        emit TokenCreated(msg.sender, token, 2, implementationVersion);
    }
}
