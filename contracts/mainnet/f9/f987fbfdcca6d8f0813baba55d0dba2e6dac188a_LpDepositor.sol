...
interface IRewarder {
    ...
    function earned(address account, address[] calldata tokens) external view returns (uint256[] memory);
    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint feeNotifyThreshold; // deprecated
    ...
    function claim(address[] calldata pool, bool lp) external {
        uint len = pool.length;
        uint bal;
        bool state;
        uint amountOut;
        for (uint i; i < len; ++i) {
            ...
            bal =
                IERC20Upgradeable(ram).balanceOf(address(this)) -
                unclaimedFees[ram];
            ...
            if (!lp) {
                ...
                if (state) {
                    amountOut = swap.swapTokens(ram, swap.weth(), bal);
                    swap.zapIn(true, amountOut, msg.sender);
                } else {
                    IVeDepositor(neadRam).depositTokens(bal);
                    swap.zapIn(false, bal, msg.sender);
                }
            }
        }
    }
    ...
    function notifyPerformanceFees(
    ...
    function manualClaimBribes(
        ...
        for (uint i; i < rewarders.length; ++i) {
            address[] memory _tokens = tokens[i];
            IBribe(rewarders[i]).getReward(tokenID, _tokens);
            uint len = _tokens.length;
            for (uint j; j < len; ++j) {
                uint bal = IERC20Upgradeable(_tokens[j]).balanceOf(
                    address(this)
                );
                IERC20Upgradeable(_tokens[j]).transfer(
                    msg.sender,
                    bal - unclaimedFees[_tokens[j]]
                );
            }
        }
    }
        ...
        for (uint i; i < rewarders.length; i++) {
            ...
            for (uint j; j < tokens[i].length; j++) {
                ...
                if (_token != ram && _token != neadRam) {
                    ...
                    IERC20Upgradeable(swapTo).safeTransfer(msg.sender, fee);
                ...
                // if token is ram or neadRam bounty will be paid in neadRam
                    ...
                    IERC20Upgradeable(neadRam).safeTransfer(msg.sender, fee);
                    stake.notifyRewardAmount(neadRam, bal - fee);
                    emit FeesNotified(neadRam, bal - fee, fee);
                } else if (_token == neadRam) {
                    bal -= unclaimedFees[_token];
                    uint fee = (bal * callFee) / 10 ** 18;
                    IERC20Upgradeable(_token).safeTransfer(msg.sender, fee);
                    stake.notifyRewardAmount(neadRam, bal - fee);
                    emit FeesNotified(neadRam, bal - fee, fee);
                } else {
                    //means _token == swapTo
    ...
    function addRewardsPerPool(
        ...
        for (uint i; i < pools.length; ++i) {
            ...
            for (uint j; j < len; ++j) {
                ...
                IERC20Upgradeable(token[j]).approve(
                    address(swap),
                    type(uint).max
                );
            }
    ...
    function _notifyPerformanceFees(address token) internal {
        ...
        if (token == ram) {
            if (state) {
                uint maxAmount = swap.calculateImpact();
                if (stakers > maxAmount) {
                    uint amountOut = swap.swapTokens(ram, neadRam, maxAmount);
                    uint remaining = stakers - maxAmount;
                    IVeDepositor(neadRam).depositTokens(remaining);
                    INeadStake(neadStake).notifyRewardAmount(
                        neadRam,
                        remaining + amountOut
                    );
                } else {
            ...
            if (state) {
                uint amountOut = swap.swapTokens(token, neadRam, stakers);
                INeadStake(neadStake).notifyRewardAmount(swapTo, amountOut);
            } else {
                uint amountOut = swap.swapTokens(token, ram, stakers);
                IVeDepositor(neadRam).depositTokens(amountOut);
