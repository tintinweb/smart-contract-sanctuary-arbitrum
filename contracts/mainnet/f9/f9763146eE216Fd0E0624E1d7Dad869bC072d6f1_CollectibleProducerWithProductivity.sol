...
contract CollectibleProducerWithProductivity is Pausable, ManagerModifier, ReentrancyGuard {
  ...
  function _produce(
    ...
    totalCost += _calculateCost(_capacity, _productivity, config);
    uint reactorCoefficient = STRUCTURE_STAKER.hasStaked(_realmId, REACTOR, 0) ? 2 : 1;
    uint maxAmount = _calculateAmount(_capacity, _productivity, config) * reactorCoefficient;
    ...
    productivitySpent = (config.productivityCost * _amount) / reactorCoefficient;
