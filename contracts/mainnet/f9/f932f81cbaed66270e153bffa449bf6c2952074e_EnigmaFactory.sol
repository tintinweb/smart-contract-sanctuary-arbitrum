...
import {IAlgebraMintCallback} from "./algebra/interfaces/callback/IAlgebraMintCallback.sol";
import {FullMath} from "@uniswap/v3-core/contracts/libraries/FullMath.sol";

//Enigma imports
import {IEnigmaFactory} from "./interfaces/IEnigmaFactory.sol";
import {Range, Rebalance, WithdrawParams, Position, DepositParams, BurnParams} from "./types/EnigmaStructs.sol";
import {EnigmaStorage} from "./abstract/EnigmaStorage.sol";
import {EnigmaHelper} from "./libs/EnigmaHelper.sol";
...
contract Enigma is EnigmaStorage, IAlgebraMintCallback, ReentrancyGuard {
    ...
    constructor(IEnigmaFactory factory_) {
        //perform basic sanitization on factory
        if (address(factory_) == address(0)) revert Errors.Enigma_InvalidImplementationAddress(address(factory_));
    ...
    /// -----------------------------------------------------------
    /// Algebra Mint Callback
    /// -----------------------------------------------------------

    /// @notice implement the Algebra V3 callback fn, called when minting uniswap LP position
    function algebraMintCallback(uint256 amount0, uint256 amount1, bytes calldata /*data*/ ) external override {
        //function protected as checks msg.sender is one of the enigmas current v3 pools
        _uniswapV3CallBack(amount0, amount1);
    }
        ...
        returns (uint256 shares, uint256 amount0, uint256 amount1)
    {
        if (params.recipient == address(0)) {
            revert Errors.Enigma_InvalidToAddress(params.recipient);
        }

        if (params.amount0Desired == 0 && params.amount1Desired == 0) {
            revert Errors.Enigma_InvalidDesiredAmounts(params.amount0Desired, params.amount1Desired);
        }

        if (params.amount0Desired > deposit0Max || params.amount1Desired > deposit1Max) {
            revert Errors.Enigma_InvalidAmounts(params.amount0Desired, params.amount1Desired);
        }

        if (isPrivate && !privateList[msg.sender]) revert Errors.Enigma_Private();

        if (params.deadline < block.timestamp) revert Errors.Enigma_Deadline();

        //we should poke fee's to ensure that flash theft of yield is not possible
        _pokeFees();
        ...
        (uint256 _sharesToMint, uint256 amount0Actual, uint256 amount1Actual) =
            EnigmaHelper.calcSharesAndAmounts(ts, total0, total1, params.amount0Desired, params.amount1Desired);

        //check that shares received are expected amount
        if (_sharesToMint < params.minSharesToMint) revert Errors.Enigma_MinimumSharesToMint();

        //check that if first deposit we are minting the minimum amount of inital shares
        if (ts == 0 && _sharesToMint < MIN_INITIAL_SHARES) revert Errors.Enigma_MinInitialAmount();

        //set the amount returned
        amount0 = amount0Actual;
        amount1 = amount1Actual;

        //transfer the required amounts into this contract from sender
        if (amount0Actual > 0) {
            token0.safeTransferFrom(msg.sender, address(this), amount0Actual);
        }
        if (amount1Actual > 0) {
            token1.safeTransferFrom(msg.sender, address(this), amount1Actual);
        }

        //loop over the ranges in the strategy and add these too the pool
        if (ts > 0) {
            uint256 rangeLength = ranges.length;
            for (uint256 index; index < rangeLength;) {
                Range memory _range = ranges[index];

                //add liquidty into pools based on underlying liquidity in the ranges
                //get the applicable uniswap pool (safe as pool is checked when adding ranges)
                address _pool = factory.poolByPair(address(token0), address(token1));

                //get the scaled amount of liquidity for the range
                (uint128 _liquidity) = EnigmaHelper.getScaledLiquidityForRange(_pool, _range, ts, _sharesToMint);

                //increment loop before continue to prevent freeze
                unchecked {
                    index++;
                }
                ...
                //mint liquidity
                _mintLiquidity(_pool, _range.tickLower, _range.tickUpper, _liquidity);
            }
        }

        //mint the underlying receipt tokens
        _mint(params.recipient, _sharesToMint);
        shares = _sharesToMint;

        /// Check total supply cap not exceeded. A value of 0 means no limit.
        //cannot use the cached ts here as needs to be the updated amount
    ...
    /// @param shares Number of liquidity tokens to redeem as pool underlying assets
    ...
    {
        if (balanceOf(msg.sender) < shares || shares == 0) revert Errors.Enigma_WithdrawInvalid(shares);
        if (to == address(0)) revert Errors.Enigma_InvalidToAddress(to);
        if (deadline < block.timestamp) revert Errors.Enigma_Deadline();
        ...
        for (uint256 index; index < rangeLength;) {
            ...
            (uint256 liquidity,,) = EnigmaHelper.position(address(this), _pool, _range.tickLower, _range.tickUpper);
            ...
            BurnParams memory _params = BurnParams(
                _pool, _range.tickLower, _range.tickUpper, attributableLiquidity.toUint128(), address(this), false
            );
            WithdrawParams memory payload = _burnLiquidity(_params);

            //update the totals
            //amount0 and amount1 are the burn amounts
        ...
        uint256 idleAmount0 = FullMath.mulDiv(token0.balanceOf(address(this)) - amount0, shares, totalSupply);
        uint256 idleAmount1 = FullMath.mulDiv(token1.balanceOf(address(this)) - amount1, shares, totalSupply);

        //transfer amounts to the user.
        if (idleAmount0 > 0 || amount0 > 0) token0.safeTransfer(to, idleAmount0 + amount0);
        if (idleAmount1 > 0 || amount1 > 0) token1.safeTransfer(to, idleAmount1 + amount1);
    ...
    /// @notice collects fees from Enigma's underlying UniswapV3 positions
    /// distributes the fees to the fee collector
    function harvest() external onlyOperator {
        _pokeFees();
    }

    /// @notice rebalance Enigma's UniswapV3 positions
    /// removes liquidity from rebalance params, swaps and then adds liquidity
    /// @dev onlyOperator contract can call this function.
    function rebalance(Rebalance calldata rebalanceData) external onlyOperator nonReentrant {
        ...
        for (uint256 index; index < rebalanceData.burns.length;) {
            //burn the required liquidities
            Position memory _burn = rebalanceData.burns[index];

            //fetch the required uni pool
            address _pool = factory.poolByPair(address(token0), address(token1));

            (uint256 liquidity,,) =
                EnigmaHelper.position(address(this), _pool, _burn.range.tickLower, _burn.range.tickUpper);
            ...
            if (liquidity > 0) {
                ...
                BurnParams memory _params = BurnParams({
                    _pool: _pool,
                    tickLower: _burn.range.tickLower,
                    tickUpper: _burn.range.tickUpper,
                    liquidity: liquidityToRemove.toUint128(),
                    to: address(this),
                    isZeroBurn: false
                });
            ...
            //don't remove the range if we are just performing a 0 burn (i.e harvesting)
            //if burn uint128.max then we want to remove the range
            if (liquidityToRemove >= liquidity && liquidityToRemove != 0 || _burn.liquidity == type(uint128).max) {
                (bool rangeExists, uint256 i) = _rangeExists(rebalanceData.burns[index].range);
                if (!rangeExists) revert Errors.Enigma_InvalidBurnRange(i);
                //gas optimization
        ...
        if (burn0Amount < rebalanceData.minBurn0) {
            revert Errors.Enigma_RebalanceBurnAmounts(burn0Amount, rebalanceData.minBurn0);
        }
        if (burn1Amount < rebalanceData.minBurn1) {
            revert Errors.Enigma_RebalanceBurnAmounts(burn1Amount, rebalanceData.minBurn1);
        }
        ...
        if (rebalanceData.swap.amountIn > 0) {
            ...
            //check to see the router is allowed
            if (!_factory.isRouterAllowed(rebalanceData.swap.router)) {
                revert Errors.Enigma_InvalidSwapRouter(rebalanceData.swap.router);
            }
            //perform the swap
            (bool success,) = rebalanceData.swap.router.call(rebalanceData.swap.payload);
            if (!success) revert Errors.Enigma_RebalanceSwapError();

            //start perform swap checks
            uint256 balance0After = token0.balanceOf(address(this));
            uint256 balance1After = token1.balanceOf(address(this));

            //check that we have received at least the expected amounts
            if (rebalanceData.swap.zeroForOne) {
                if (balance1After < balance1Before + rebalanceData.swap.expectedMinReturn) {
                    revert Errors.Enigma_RebalanceInsufficientSwap();
                }
                if (balance0After < balance0Before - rebalanceData.swap.amountIn) {
                    revert Errors.Enigma_RebalanceInsufficientSwap();
                }
            } else {
                if (balance0After < balance0Before + rebalanceData.swap.expectedMinReturn) {
                    revert Errors.Enigma_RebalanceInsufficientSwap();
                }
                if (balance1After < balance1Before - rebalanceData.swap.amountIn) {
                    revert Errors.Enigma_RebalanceInsufficientSwap();
                }
            }

            //remove approvals for token to the router
            token0.safeApprove(address(rebalanceData.swap.router), 0);
            token1.safeApprove(address(rebalanceData.swap.router), 0);

            emit Log_Rebalance(rebalanceData, balance0After, balance1After);
        } else {
            //no swaps required log (0,0) and ignore this in offchain data
        ...
        for (uint256 index; index < rebalanceData.mints.length;) {
            //add the liquidity in the strategy to the new ranges
            Position memory position = rebalanceData.mints[index];
            ...
            Range memory range_ = Range(position.range.tickLower, position.range.tickUpper, position.range.feeTier);

            //fetch the required uni pool
            address _pool = factory.poolByPair(address(token0), address(token1));
            if (_pool == address(0)) revert Errors.Enigma_RebalanceNoPool();

            if (!EnigmaHelper.verifyTickSpacing(_pool, range_)) {
                revert Errors.Enigma_RebalanceInvalidTickSpacing();
            }

            //verify the pool was whitelisted upon creation otherwise revert
            bool _isWhitelisted = EnigmaHelper.poolWhitelisted(address(this), _pool);
            if (!_isWhitelisted) revert Errors.Enigma_RebalanceNotWLPool();

            if (position.liquidity == 0) continue;

            //check if the range exists if not push it to the array (ensure no empty ranges)
            (bool exists,) = _rangeExists(range_);
            if (!exists) ranges.push(range_);

            //mint liquidity
            (uint256 amount0Minted, uint256 amount1Minted) =
                _mintLiquidity(_pool, position.range.tickLower, position.range.tickUpper, position.liquidity);

            mint0Amount += amount0Minted;
            mint1Amount += amount1Minted;
        }

        if (mint0Amount < rebalanceData.minDeposit0) {
            revert Errors.Enigma_RebalanceMintAmounts(mint0Amount, rebalanceData.minDeposit0);
        }
        if (mint1Amount < rebalanceData.minDeposit1) {
            revert Errors.Enigma_RebalanceMintAmounts(mint1Amount, rebalanceData.minDeposit1);
        }

        //check if the cap has been reached on the number of positions in the enigma
        if (ranges.length > _factory.ENIGMA_MAX_POS()) revert Errors.Enigma_MaxCapReached(ranges.length);
    }
    ...
    /// @notice check if the range exists in the enigma
    /// @param range_ the range to check if exists in the enigma
    function _rangeExists(Range memory range_) internal view returns (bool ok, uint256 index) {
        for (uint256 i; i < ranges.length;) {
            //evaluate if the range exists
            if (
                range_.tickLower == ranges[i].tickLower && range_.tickUpper == ranges[i].tickUpper
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        if (liquidity > 0) {
            (amount0, amount1, liquidity) =
                IAlgebraPool(_pool).mint(address(this), address(this), tickLower, tickUpper, liquidity, "");
        }
    }
    ...
    function _burnLiquidity(BurnParams memory _burnParams) internal returns (WithdrawParams memory withdrawPayload) {
        /// Burn liquidity if required
        if (_burnParams.liquidity > 0 || _burnParams.isZeroBurn) {
            (withdrawPayload.burn0, withdrawPayload.burn1) = IAlgebraPool(_burnParams._pool).burn(
                _burnParams.tickLower, _burnParams.tickUpper, _burnParams.liquidity
            );
        }

        //collecting swap fees
        (uint256 collect0, uint256 collect1) = IAlgebraPool(_burnParams._pool).collect(
            _burnParams.to, _burnParams.tickLower, _burnParams.tickUpper, type(uint128).max, type(uint128).max
        );

        //apply manager fees
        //fee is the difference between collect and what was burnt.
        if (collect0 > 0) withdrawPayload.fee0 = collect0 - withdrawPayload.burn0;
        if (collect1 > 0) withdrawPayload.fee1 = collect1 - withdrawPayload.burn1;
    }

    /// @notice applies the enigma fees and distributes
    /// @return fee0 amount of token0 taken as fee
    /// @return fee1 amount of token1 taken as fee
    function _applyFeesDistribute(uint256 fee0_, uint256 fee1_) internal returns (uint256 fee0, uint256 fee1) {
        //assign storage vars to memory
        uint256 fl = FEE_LIMIT;
        uint256 sf = SELECTED_FEE;

        fee0 = (fee0_ * sf) / fl;
        fee1 = (fee1_ * sf) / fl;

        address enigmaFeeRecipient = _factory.enigmaTreasury();
        uint256 enigmaTreasuryFee = _factory.ENIGMA_TREASURY_FEE();

        uint256 enigmaFee0;
        uint256 operatorFee0;
        uint256 enigmaFee1;
        uint256 operatorFee1;

        if (fee0 > 0) {
            //enigmaShare of fees
            enigmaFee0 = FullMath.mulDiv(fee0, enigmaTreasuryFee, fl);

            //operatorShare
            operatorFee0 = fee0 - enigmaFee0;

            //transfer amounts if non zero
            if (enigmaFee0 > 0) {
                //transfer fee to enigma treasury
                token0.safeTransfer(enigmaFeeRecipient, enigmaFee0);
            }
            if (operatorFee0 > 0) {
                //transfer fee to enigma operator
        ...
        if (fee1 > 0) {
            //enigmaShare of fees
            enigmaFee1 = FullMath.mulDiv(fee1, enigmaTreasuryFee, fl);
            //operatorShare
            operatorFee1 = fee1 - enigmaFee1;
    ...
    /// @notice pokes the enigma fees and distributes to collector
    /// used by harvest() function for collecting fees
    /// used by deposit() to prevent flash theft of yield
    function _pokeFees() internal {
        uint256 fee0Accumulator;
        uint256 fee1Accumulator;

        //loop over the ranges in the strategy to harvest fees from underyling position
        uint256 rangeLength = ranges.length;
        for (uint256 index; index < rangeLength;) {
            Range memory _range = ranges[index];
            ...
            //get the applicable uniswap pool (safe as pool is checked when adding ranges)
            address _pool = factory.poolByPair(address(token0), address(token1));
            (uint128 liquidity,,) = EnigmaHelper.position(address(this), _pool, _range.tickLower, _range.tickUpper);

            if (liquidity == 0) continue;

            //burn 0 liquidity from the pools as required to collect fees
            BurnParams memory _params = BurnParams({
                _pool: _pool,
                tickLower: _range.tickLower,
                tickUpper: _range.tickUpper,
                liquidity: 0,
                to: address(this),
                isZeroBurn: true
            });
    ...
    /// @notice gets the factory stored
    /// @return factory_ enigma factory address
    ...
    /// all its liquidity from Uniswap. (does not include pending fees)
    function getTotalAmounts() public view returns (uint256 total0, uint256 total1) {
        //add any currently unused values contained in this contract
        (total0, total1) = EnigmaHelper.getTotalAmounts(address(this), ranges, factory);
    }
...
//uniswap
import "./algebra/interfaces/IAlgebraFactory.sol";

//enigma
import "./Enigma.sol";
import "./interfaces/IEnigmaFactory.sol";
...
contract EnigmaFactory is Ownable, IEnigmaFactory {
    ...
    IAlgebraFactory private immutable uniswapFactory;

    uint256 public constant FEE_LIMIT = 1e18; //100%
    uint256 public ENIGMA_TREASURY_FEE = 0.075e18; //enigma fee is initially 7.5%
    uint256 public ENIGMA_CAP = 100; //cap of the amount of enigma which can be created by this factory
    uint256 public ENIGMA_MAX_POS = 20; //cap of the amount of positions that can be deployed on an Enigma
    ...
    /// @notice set of all allowed routers that can be used for swaps
    EnumerableSet.AddressSet private _allAllowedRouters;
    /// @notice set if anyone can create new enigma
    ...
    constructor(address _uniswapFactory) {
        require(_uniswapFactory != address(0), "F:!0");
        uniswapFactory = IAlgebraFactory(_uniswapFactory);
    }
    ...
    function deployEnigmaPool(
        ...
        if (_allEnigmas.length() >= ENIGMA_CAP) revert Enigma__FactoryCap();
        require(msg.sender == owner() || isOpen, "!Open");
        ...
        require(_operator != address(0), "Factory: 0A");
        ...
        require(implementation != address(0), "A(0)");

        //we should run a check that the pools for feeTiers actually exist
        for (uint256 i; i < _feeTiers.length;) {
            address _pool = uniswapFactory.poolByPair(token0, token1);
            require(_pool != address(0), "Factory: Pool does not exist");
        ...
        pool_ = IEnigma(
            Clones.cloneDeterministic(
                implementation, keccak256(abi.encode(token0, token1, block.timestamp, msg.sender))
            )
        );
        ...
        //generate the mapping to store the deployed pools
        enigmaPools[token0][token1][msg.sender] = address(pool_);
        enigmaPools[token1][token0][msg.sender] = address(pool_);

        //push new enigma pool to the _allEnigmas set
        _allEnigmas.add(address(pool_));

        emit EnigmaCreated(address(pool_), token0, token1, _selectedFee, _feeTiers, _isPrivate, _operator);
        return pool_;
    }
    ...
    function setEnigmaImplementation(address newEnigmaImplementation) external onlyOwner {
        ...
        //set the implementation to new implementation
    ...
    /// @notice Internal function to set the recipient of the treasury fees
    ...
    /// @notice sets the fee that enigma can take for treasury
    /// this is a percent of the fee that the manager sets
    /// @param _newEnigmaFee new enigmaFee
    function setEnigmaFee(uint256 _newEnigmaFee) public onlyOwner {
        if (_newEnigmaFee >= FEE_LIMIT) revert EnigmaFactory__InvalidFee(_newEnigmaFee);
        ENIGMA_TREASURY_FEE = _newEnigmaFee;
        //emit an event
        emit SetEnigmaTreasuryFee(_newEnigmaFee);
    }
    ...
    function inCaseTokensGetStuck(address _token) external onlyOwner {
        ...
        emit Log__InCaseTokensStuck(amount, _token);
    }
    ...
    /// @notice adds a allowed router for rebalancing swaps
    ///
    function addAllowedRouter(address _router) external onlyOwner {
        if (_allAllowedRouters.contains(_router)) revert Enigma__RouterWhitelisted(_router);
        _allAllowedRouters.add(_router);
        emit RouterAllowed(_router, true);
    }
    ...
    /// @notice check to see if the router is allowed
    ///
    function isRouterAllowed(address _router) external view returns (bool isAllowed) {
        return _allAllowedRouters.contains(_router);
    }
    ...
    /// @notice set so anyone can create a new enigma pool
    /// @param _isOpen isOpen or otherwise
    function setIsOpen(bool _isOpen) public onlyOwner {
        require(isOpen != _isOpen, "!CH");
        isOpen = _isOpen;

        //emit an event
        emit Log__IsOpen(_isOpen);
    }

    /// @notice set the cap on enigma pools
    /// @param _factoryCap the new amount of pools which can be created
    function setFactoryCap(uint256 _factoryCap) public onlyOwner {
        ENIGMA_CAP = _factoryCap;

        //emit an event
        emit Log__FactoryCap(_factoryCap);
    }

    /// @notice set the maximum positions an enigma can contain
    /// @param _maxPositions the new amount of positions in a enigma
    function setMaxPositions(uint256 _maxPositions) public onlyOwner {
        ENIGMA_MAX_POS = _maxPositions;

        //emit an event
        emit Log__PositionCap(_maxPositions);
    }
...
import "../libs/ERC20Sol.sol";
import {Range} from "../types/EnigmaStructs.sol";
...
abstract contract EnigmaStorage is IEnigma, ERC20Sol, Ownable {
    ...
    function _onlyOperator() private view {
        if (operatorAddress != msg.sender && owner() != msg.sender) {
            revert Errors.Enigma_NotOperator(msg.sender);
        }
    }
    ...
    //tracks if the enigma has been initalized
    bytes32 internal _parameters;

    uint256 internal constant PRECISION = 1e18;
    uint256 internal constant MIN_INITIAL_SHARES = 1e6;
    ...
    //fee params
    uint256 public constant FEE_LIMIT = 1e18; //represents 100% fee
    uint256 public SELECTED_FEE = 0.1e18; //initial fee of 10%
    ...
    function _uniswapV3CallBack(uint256 amount0_, uint256 amount1_) internal {
        if (!_pools.contains(msg.sender)) revert Errors.Enigma_UniCallbackFail(msg.sender);
    ...
    function initialize(
        ...
        if (parameters != 0) revert Errors.Enigma_AlreadyInit();
        if (_uniFactory == address(0)) revert Errors.Enigma_InitInvalidFactory(_uniFactory);
        if (_owner == address(0)) revert Errors.Enigma_InvalidOwner(_owner);
        if (_operator == address(0)) revert Errors.Enigma_InvalidOperator(_operator);

        if (_token0 == address(0)) revert Errors.Enigma_InvalidAddress(_token0);
        if (_token1 == address(0)) revert Errors.Enigma_InvalidAddress(_token1);

        token0 = IERC20(_token0);
        token1 = IERC20(_token1);

        factory = IAlgebraFactory(_uniFactory);
        _setSelectedFee(_selectedFee);
        _transferOwnership(_owner);

        /// add the pools to the enumerable set only upon factory creation
        /// operators cannot add any further pools once init
        _addPools(_feeTiers, _token0, _token1);

        /// no cap on deposits or supply of enigma token
        maxTotalSupply = 0;
    ...
    /// @notice Returns all the addresses of the underlying uniV3 pool
    ...
    /// @notice get the length of the ranges array
    /// @return length of the ranges array
    function getRangeLength() external view returns (uint256) {
        return ranges.length;
    }

    /// @notice get all the ranges on the enigma
    ...
    /// @notice add a new pool to the mapping
    /// only ever called once upon enigma pool creation
    /// a new pool can only ever be added at init time
    function _addPools(uint24[] calldata feeTiers_, address token0Addr_, address token1Addr_) internal {
        for (uint256 i = 0; i < feeTiers_.length;) {
            address pool = factory.poolByPair(token0Addr_, token1Addr_);

            if (pool == address(0)) revert Errors.Enigma_InvalidPool(pool);
            if (_pools.contains(pool)) revert Errors.Enigma_PoolExists(pool);
    ...
    /// @notice set the max fee that can be taken by the enigma
    /// @notice should be < than 1e18 and > 0
    /// @dev the max is 1e18 so setting the _newSelectedFee to this
    /// @dev means that fees are 100% of the lp fees earned
    function _setSelectedFee(uint256 _newSelectedFee) internal {
        if (_newSelectedFee == 0) revert Errors.Enigma_InvalidFee(_newSelectedFee);
        if (_newSelectedFee > FEE_LIMIT) revert Errors.Enigma_InvalidFee(_newSelectedFee);

        SELECTED_FEE = _newSelectedFee;
        //emit update the fee
        emit Log_SetEnigmaFee(_newSelectedFee);
    }
    ...
    /// @notice sets restrictions around deposits for receipt token supply
    ...
    /// @notice sets restrictions around deposits
    /// @notice i.e the max amount of tokens that can exist in the enigma
    ...
    function setDepositMax(uint256 _deposit0Max, uint256 _deposit1Max) external onlyOperator {
        ...
        emit Log_DepositMaxSet(_deposit0Max, _deposit1Max);
    }

    /// @notice adds a set of addresses to the allowlist
    /// @param listed Array of addresses to be appended
    function appendList(address[] calldata listed) external onlyOperator {
        for (uint256 i; i < listed.length;) {
            privateList[listed[i]] = true;
            emit Log_SetIsWhitelisted(listed[i], true);
    ...
    /// @notice removes an addresses from the allowlist
    /// @param listed Address of listed to remove
    function removeListed(address listed) external onlyOperator {
        privateList[listed] = false;
        emit Log_SetIsWhitelisted(listed, false);
    }

    /// @notice sets if the enigma pool should use the whitelist
    /// users can always withdraw from the enigmaPool
    /// however only deposits will be restricted
    function togglePrivate() external onlyOperator {
        isPrivate = !isPrivate;
        emit Log_IsPrivate(isPrivate);
    }

    /// @notice set the max fee that can be taken by the enigma
    /// @notice should be < than 1e18 and > ENIGMA_TREASURY_FEE
    /// @dev the max is 1e18 so setting the _newSelectedFee to this
    /// @dev means that fees are 100% of the lp fees earned
    function setSelectedFee(uint256 _newSelectedFee) external onlyOperator {
    ...
    function setOperator(address _operator) external onlyOperator {
        ...
        emit Log_SetOperator(_operator);
    }

    /// @notice Rescues funds stuck or tokens not compatible with the enigma
    /// @param _token address of the token to rescue.
    function inCaseTokensGetStuck(address _token) external onlyOwner {
        if (_token == address(token0) || _token == address(token1)) revert Errors.Enigma_RescueInvalidTokens();
        uint256 amount = IERC20(_token).balanceOf(address(this));
        IERC20(_token).safeTransfer(msg.sender, amount);
    }
...
import {Range, Position} from "../types/EnigmaStructs.sol";
import "../interfaces/IEnigma.sol";

/// @title Core library for the Enigma Pools
/// @notice Assists in the calculation of Enigma minting based on underlying
/// @notice Provides functions for computing liquidity and ticks for token amounts and prices
library EnigmaHelper {
    using FullMath for uint256;
    ...
    function getLiquidityForAmounts(address _pool, int24 tickLower, int24 tickUpper, uint256 amount0, uint256 amount1)
        public
        view
        returns (uint128)
    {
        (uint160 sqrtRatioX96,,,,,,,) = IAlgebraPool(_pool).globalState();
    ...
    function getAmountsForLiquidity(address _pool, int24 tickLower, int24 tickUpper, uint128 liquidity)
        public
        view
    ...
    function position(address enigma, address _pool, int24 tickLower, int24 tickUpper)
        public
        view
        returns (uint128 liquidity, uint128 tokensOwed0, uint128 tokensOwed1)
    {
        bytes32 positionKey;
        address This = address(enigma);
        assembly {
            positionKey := or(shl(24, or(shl(24, This), and(tickLower, 0xFFFFFF))), and(tickUpper, 0xFFFFFF))
        }

        (liquidity,,,, tokensOwed0, tokensOwed1) = IAlgebraPool(_pool).positions(positionKey);
    }

    /// @return tick Uniswap pool's current price tick
    function currentTick(address _pool) public view returns (int24 tick) {
        (, tick,,,,,,) = IAlgebraPool(_pool).globalState();
    }

    /// @notice verified that the tick spacing on the pool is correct.
    function verifyTickSpacing(address pool_, Range memory range_) public view returns (bool) {
        int24 spacing = IAlgebraPool(pool_).tickSpacing();
        return range_.tickLower < range_.tickUpper && range_.tickLower % spacing == 0 && range_.tickUpper % spacing == 0;
    }
    ...
    function calcSharesAndAmounts(
        ...
        if (totalSupply == 0) {
            ...
            shares = (amount1Desired * totalSupply) / total1;
            amount1 = ((shares * total1) + (totalSupply - 1)) / totalSupply;
        } else if (total1 == 0) {
            shares = (amount0Desired * totalSupply) / total0;
            amount0 = ((shares * total0) + (totalSupply - 1)) / totalSupply;
        } else {
            ...
            unchecked {
                // Round up amounts
                amount0 = (cross) / (total1);
                amount1 = (cross) / (total0);
            }

            shares = FullMath.mulDiv(cross, totalSupply, total0) / total1;
        }
    }

    /// @notice check if the range exists in the enigma
    /// @param currentRanges_ current ranges in the enigma
    /// @param range_ the range to check if exists in the enigma
    ...
    {
        for (uint256 i; i < currentRanges_.length;) {
            //evaluate if the range exists
            if (
                range_.tickLower == currentRanges_[i].tickLower && range_.tickUpper == currentRanges_[i].tickUpper
    ...
    /// @notice check if the univ3 pool was initiilized upon creation
    /// @dev only pools that were added at init time can be added to the enigma
    /// @return isWhitelisted if the uniV3 pool was added at init time
    function poolWhitelisted(address _enigmaAddress, address _pool) public view returns (bool isWhitelisted) {
        address[] memory _pools = IEnigma(_enigmaAddress).getPools();
    ...
    /// @notice gets the amount of token0 and token1 in each range
    /// @dev this does not include any pending fees which have not been poked
    function getTokenAmountsByRange(address _enigma, address _uniFactory)
        external
        view
        returns (uint256[] memory token0Amounts, uint256[] memory token1Amounts, Range[] memory ranges)
    {
        ranges = IEnigma(_enigma).getRanges();
        address token0 = address(IEnigma(_enigma).token0());
        address token1 = address(IEnigma(_enigma).token1());

        token0Amounts = new uint256[](ranges.length);
        token1Amounts = new uint256[](ranges.length);

        for (uint256 i = 0; i < ranges.length;) {
            Range memory range = ranges[i];

            address _pool = IAlgebraFactory(_uniFactory).poolByPair(address(token0), address(token1));

            (uint128 liquidity, uint128 tokensOwed0, uint128 tokensOwed1) =
                position(_enigma, _pool, range.tickLower, range.tickUpper);

            (uint256 amount0, uint256 amount1) =
                getAmountsForLiquidity(_pool, range.tickLower, range.tickUpper, liquidity);

            token0Amounts[i] = tokensOwed0 + amount0;
            token1Amounts[i] = tokensOwed1 + amount1;
    ...
    /// @notice gets the amount of liquidity in each range
    function getTotalLiquidityForRanges(address enigma, address _uniFactory)
        external
        view
        returns (Position[] memory liquidities)
    {
        IEnigma _enigma = IEnigma(enigma);
        Range[] memory ranges = _enigma.getRanges();
        liquidities = new Position[](ranges.length);
        for (uint256 i; i < ranges.length;) {
            address pool = IAlgebraFactory(_uniFactory).poolByPair(address(_enigma.token0()), address(_enigma.token1()));

            (uint128 liquidity,,) = position(address(_enigma), pool, ranges[i].tickLower, ranges[i].tickUpper);

            liquidities[i] = Position({liquidity: liquidity, range: ranges[i]});
            unchecked {
                ++i;
            }
        }
    ...
    /* function not required for algebra integration
    function getUnderlyingFeeTiers(address enigma)
        external
        view
        returns (uint24[] memory feeTiers, address[] memory _underlyingPools)
    {
        IEnigma _enigma = IEnigma(enigma);

        _underlyingPools = _enigma.getPools();
        feeTiers = new uint24[](_underlyingPools.length);

        for (uint256 i = 0; i < _underlyingPools.length;) {
            feeTiers[i] = IAlgebraPool(_underlyingPools[i]).fee();
            unchecked {
                ++i;
            }
        }
    ...
    */
    ...
    function getMaxLiquidityForRange(
        address enigma,
        address _unifactory,
        int24 tickLower,
        int24 tickUpper,
        ...
        IEnigma _enigma = IEnigma(enigma);
        IERC20 token0 = _enigma.token0();
        IERC20 token1 = _enigma.token1();
        uint256 amount0 = token0.balanceOf(address(_enigma));
        uint256 amount1 = token1.balanceOf(address(_enigma));

        address pool = IAlgebraFactory(_unifactory).poolByPair(address(token0), address(token1));
        require(pool != address(0), "POOL(0)");

        maxLiquidity = getLiquidityForAmounts(pool, tickLower, tickUpper, amount0, amount1);
    }

    function getTotalUnusedAmounts(address enigma) external view returns (uint256 amount0, uint256 amount1) {
        IEnigma _enigma = IEnigma(enigma);
        //returns the total of unused tokens
        IERC20 token0 = _enigma.token0();
        IERC20 token1 = _enigma.token1();
        amount0 = token0.balanceOf(address(_enigma));
        amount1 = token1.balanceOf(address(_enigma));
    }
    ...
    /// @return token0 The sorted first token
    /// @return token1 The sorted second token
    function sortTokens(address tokenX, address tokenY) external pure returns (address token0, address token1) {
        require(tokenX != tokenY, "same token");
        (token0, token1) = tokenX < tokenY ? (tokenX, tokenY) : (tokenY, tokenX);
    ...
    /// other words, how much of each token the vault would hold if it withdrew
    /// all its liquidity from Uniswap. (does not include pending fees)
    function getTotalAmounts(address _enigma, Range[] memory _ranges, IAlgebraFactory factory)
        external
        view
        returns (uint256 total0, uint256 total1)
    {
        IERC20 token0 = IEnigma(_enigma).token0();
        IERC20 token1 = IEnigma(_enigma).token1();
        //add any currently unused values contained in this contract
        total0 = token0.balanceOf(_enigma);
        total1 = token1.balanceOf(_enigma);

        //loop over the ranges in the strategy
        uint256 rangeLength = _ranges.length;
        for (uint256 index; index < rangeLength;) {
            Range memory _range = _ranges[index];

            //get the applicable uniswap pool (safe as fee tier is checked when adding)
            address _pool = factory.poolByPair(address(token0), address(token1));

            (uint128 liquidity, uint256 owed0, uint256 owed1) =
                position(address(this), _pool, _range.tickLower, _range.tickUpper);

            (uint256 amount0, uint256 amount1) =
                getAmountsForLiquidity(_pool, _range.tickLower, _range.tickUpper, liquidity);

            //increment the index prior to the continue to prevent unlimted gas issue
            unchecked {
                ++index; // Increment in an unchecked block
            }

            if (amount0 == 0 && amount1 == 0 && owed0 == 0 && owed1 == 0) continue;

            total0 += amount0 + owed0;
            total1 += amount1 + owed1;
        }
    }

    /// @notice Calculates the amount of liquidity to add to a range based on existing amounts in the range
    /// used for direct depositing to underlying pools in enigma deposit fn.
    /// @param pool the uniswapV3 pool
    /// @param range the underlying enigma range
    /// @param totalShares the total amount of shares that already exist
    /// @param mintShares the new amount of shares being minted
    /// @return liquidity of the new position that is to be minted
    function getScaledLiquidityForRange(address pool, Range memory range, uint256 totalShares, uint256 mintShares)
        external
        view
        returns (uint128 liquidity)
    {
        //get the current liquidity in the univ3 position
        (uint128 _liquidity,,) = position(address(this), pool, range.tickLower, range.tickUpper);

        // get the underrlying amounts of token0 and token1 for that amount of liquidity
        (uint256 amt0, uint256 amt1) = getAmountsForLiquidity(pool, range.tickLower, range.tickUpper, _liquidity);

        //scale the amounts that are to be added based on the number of new shares to mint and round down.
        uint256 amt0Act = FullMath.mulDiv(amt0, mintShares, totalShares + 1);
        uint256 amt1Act = FullMath.mulDiv(amt1, mintShares, totalShares + 1);

        //get the liquidity for the scaled amounts from uniV3.
        liquidity = getLiquidityForAmounts(pool, range.tickLower, range.tickUpper, amt0Act, amt1Act);
    }

    /// -----------------------------------------------------------
    /// END EnigmaHelper by SteakHut Labs
    /// -----------------------------------------------------------
}
...
/// @title Errors
/// @notice Library containing all custom errors the core enigma may revert with.
library Errors {
    /*//////////////////////////////////////////////////////////////////////////
                                    GENERICS
    //////////////////////////////////////////////////////////////////////////*/
    /// @notice most generic errors
    // not an owner or operator of the enigma
    error Enigma_NotOperator(address operator);

    /*//////////////////////////////////////////////////////////////////////////
                                  ENIGMA CORE
    //////////////////////////////////////////////////////////////////////////*/

    /// @notice Errors relating to constructor
    // the address invalid or address(0)
    error Enigma_InvalidImplementationAddress(address implementation);

    /// @notice Errors relating to depositing into enigma
    // to address invalid or address(0)
    error Enigma_InvalidToAddress(address toAddress);
    // desired amounts less than required
    error Enigma_InvalidDesiredAmounts(uint256 amount0, uint256 amount1);
    // amounts inputted are incorred
    error Enigma_InvalidAmounts(uint256 amount0, uint256 amount1);
    //max total supply of tokens minted
    error Enigma_MaxTotalSupply();
    //user not on private list
    error Enigma_Private();
    //required to mint a minimum amount of shares
    error Enigma_MinInitialAmount();
    //required to mint a non 0 amount of shares
    error Enigma_ZeroSharesAmount();
    //cannot remove funds from enigma that are the pool tokens
    error Enigma_RescueInvalidTokens();
    //max positions cap reached
    error Enigma_MaxCapReached(uint256 amount);

    /// @notice Errors relating to withdrawing from enigma
    //user does not have enough shares to burn
    error Enigma_WithdrawInvalid(uint256 shares);
    error Enigma_Deadline();

    /// @notice Errors relating to rebalancing the enigma
    error Enigma_RebalanceBurnAmounts(uint256 burnAmount, uint256 minBurn);
    error Enigma_RebalanceSwapError();
    error Enigma_RebalanceRangeDistribution(uint256 dist);
    error Enigma_RebalanceNoPool();
    error Enigma_RebalanceNotWLPool();
    error Enigma_RebalanceInvalidTickSpacing();
    error Enigma_RebalanceMintAmounts(uint256 mintAmount, uint256 minDeposit);
    error Enigma_RebalanceInsufficientSwap();
    /// @notice swap router is not whitelisted by Enigma Controller
    error Enigma_InvalidSwapRouter(address router);
    /// @notice range does not exist to burn
    error Enigma_InvalidBurnRange(uint256 index);

    /// @notice Errors relating to the core of enigma
    error Enigma_UniCallbackFail(address caller);
    error Enigma_AlreadyInit();
    error Enigma_InitInvalidFactory(address factory);
    error Enigma_InvalidOperator(address operator);
    error Enigma_InvalidOwner(address owner);
    error Enigma_MinimumSharesToMint();

    /// @notice Errors relating to adding polls the the enigma
    error Enigma_InvalidPool(address pool);
    error Enigma_PoolExists(address pool);

    /// @notice Errors relating to the enigma fee
    error Enigma_InvalidFee(uint256 proposedFee);

    /// @notice Errors relating to sanitization of address
    error Enigma_InvalidAddress(address _address);
}
