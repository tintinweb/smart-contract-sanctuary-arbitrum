...
error ZeroAddress();
error InvalidRangeValues(uint start, uint stop);
...
library LibTokenHelper {
    ...
    function getTokens(uint start, uint stop) internal view returns (TokenInfo[] memory tokens) {
        if (start >= stop) revert InvalidRangeValues(start, stop);
...
contract TokenManager {
    ...
    function getTokensRange (uint start, uint stop) external view returns (TokenInfo[] memory tokens) {
        tokens = LibTokenHelper.getTokens(start, stop);
    }
