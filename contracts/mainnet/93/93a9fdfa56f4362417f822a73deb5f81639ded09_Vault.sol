...
contract Vault is ReentrancyGuard, IVault, VaultEvents {
    ...
    function distribute(
        ...
        (uint96 totalRemaining, uint96 mFee) =
            Trade.distribute(stvId, c, totalDepositTokenUsed, managerFees, tradeTokens, exchangeData, operator);

        uint96 totalRemainingAfterFees = totalRemaining - protocolFees;
        IStvAccount(stvId).distribute(totalRemainingAfterFees, mFee, protocolFees);

        emit Distribute(stvId, totalRemainingAfterFees, mFee, protocolFees, c);
    }
...
contract VaultEvents {
    ...
    event Distribute(address indexed stvId, uint96 totalRemainingAfterFees, uint96 mFee, uint96 pFee, uint256 command);
...
library Trade {
    ...
    /// @param exchangeData exchange data to swap, 0 - eth swap, 1 - tradeToken swap
    /// @param operator address of the operator
    /// @return totalRemaining amount of defaultStableCoin remaining after manager fees
    ...
    function distribute(
        ...
        bytes[] calldata exchangeData,
        address operator
    ) external returns (uint96 totalRemaining, uint96 mFee) {
        ...
        (totalRemaining, mFee) = _distribute(totalDepositTokenUsed, uint96(depositTokenBalance), managerFees);
    }
    ...
    function _distribute(uint96 totalRaised, uint96 totalReceivedAfterClose, uint96 managerFees)
        internal
        pure
        returns (uint96 totalRemaining, uint96 mFee)
    {
        if (totalReceivedAfterClose > totalRaised) {
            ...
            totalRemaining = totalReceivedAfterClose - mFee;
        } else {
            totalRemaining = totalReceivedAfterClose;
        }
    }
