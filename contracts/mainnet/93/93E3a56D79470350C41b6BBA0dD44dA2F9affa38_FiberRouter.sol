...
contract FiberRouter is Ownable, TokenReceivable {
   ...
   /**
    ...
    constructor() {
        bytes memory initData = IFerrumDeployer(msg.sender).initData();
        (WETH, oneInchAggregatorRouter, pool, gasWallet) = abi.decode(
            initData,
        ...
        require(gasWallet != address(0), "Pool address cannot be the zero address");
    }
    ...
    /**
     @notice Sets the gasPayer address.
     @param _gasWallet The gasPayer
     */
    function setGasPayer(address _gasWallet) external onlyOwner {
        require(
            _gasWallet != address(0),
            "gasWallet address cannot be zero"
        );
        gasWallet = payable(_gasWallet);
    }
    ...
    // Emitting the event with swap details
    emit Swap(
        token,
        targetToken,
        block.chainid,
