...
contract Keeper is IERC20TokenReceiver, PausableAccessControl, ReentrancyGuard {
	...
	/// @notice Factor to convert native token amount into synth token amount.
	uint256 public immutable CONVERSION_FACTOR;

	/// @notice The address of the synthetic token to convert to base token.
	/// @notice We suppose that synToken precision is the same as {FIXED_POINT_SCALAR}
	...
	/// @notice Total amount of synthethic tokens already converted.
	uint256 private _synthTokenConverted;
	...
	constructor(
		...
		uint8 _synthTokenDecimals = TokenUtils.expectDecimals(_synthToken);

		CONVERSION_FACTOR = 10**(_synthTokenDecimals - _nativeTokenDecimals);
	}
	...
	///
	/// @param _synthAmount the amount of synthetic tokens to stake.
	function stake(uint256 _synthAmount) external nonReentrant {
		_checkNotPaused();
		if (_synthAmount == 0) {
			revert ZeroValue();
		}
		...
		TokenUtils.safeTransferFrom(synthToken, msg.sender, address(this), _synthAmount);

		// Increases user stake amount
		_increaseStakeFor(msg.sender, _synthAmount);

		emit TokensStaked(msg.sender, _synthAmount);
	}
	...
	///
	/// @param _synthAmount The amount of synthetic tokens to unstake.
	function unstake(uint256 _synthAmount) external nonReentrant {
		_checkNotPaused();
		_distribute();
		_update(msg.sender);
		...
		uint256 _unstakeAmount = _getEffectiveUnstakeFor(msg.sender, _synthAmount);
	...
	function liquidate(address _toLiquidate) external nonReentrant {
		...
		uint256 _synthConvertedOverflow = _getOverflowFor(_toLiquidate);

		// Checks if valid liquidation
		if (_synthConvertedOverflow == 0) {
		...
		_increaseConvertedForLiquidator(msg.sender, _synthConvertedOverflow);

		emit TokensLiquidated(msg.sender, _toLiquidate, _synthConvertedOverflow);
	}
	...
	function emergencyExit() external nonReentrant {
		...
		uint256 _synthUnstakeAmount = _getMaxUnstakeFor(msg.sender);
		if (_synthUnstakeAmount == 0) {
		...
		_unstake(msg.sender, _synthUnstakeAmount);

		emit EmergencyExitCompleted(msg.sender, _synthUnstakeAmount);
	}
	...
	/// @notice Gets the total amount of native tokens to distribute to users.
	///
	...
	/// @notice Allows `_owner` to claim its converted tokens into native tokens.
	///
	...
	function _claim(address _owner) internal {
		...
		uint256 _convertedSynthAmount = _getMaxConvertedFor(_owner);

		if (_convertedSynthAmount > 0) {
			// Resets converted amount
			_resetConvertedFor(_owner);
			// Decreases user stake and burns synth tokens
			_decreaseStakeFor(_owner, _convertedSynthAmount);
			TokenUtils.safeBurn(synthToken, _convertedSynthAmount);
			// Transfers converted tokens to user
			uint256 _claimableNativeAmount = _normalizeSynthToNative(_convertedSynthAmount);
			nativeTokenReserve -= _claimableNativeAmount;
			TokenUtils.safeTransfer(nativeToken, _owner, _claimableNativeAmount);
		}

		emit TokensClaimed(_owner, _convertedSynthAmount);
	}

	/// @notice Allows `_owner` to unstake its synthethic tokens.
	///
	/// @param _owner The address of the account to claim converted native tokens for.
	/// @param _synthAmount The amount of tokens to unstake
	function _unstake(address _owner, uint256 _synthAmount) internal {
		if (_synthAmount > 0) {
			_decreaseStakeFor(_owner, _synthAmount);

			TokenUtils.safeTransfer(synthToken, _owner, _synthAmount);
		}

		emit TokensUnstaked(_owner, _synthAmount);
	}

	function _normalizeNativeToSynth(uint256 _amount) internal view returns (uint256) {
		return _amount * CONVERSION_FACTOR;
	}

	function _normalizeSynthToNative(uint256 _amount) internal view returns (uint256) {
		return _amount / CONVERSION_FACTOR;
	}
	...
	function _distribute() internal {
		...
		if (_totalStaked > 0) {
			...
			if (_toDistribute > 0) {
				uint256 _toConvertSynthAmount = _normalizeNativeToSynth(_toDistribute);
				_synthTokenConverted += _toConvertSynthAmount;
				_totalDividendPoints += (_toConvertSynthAmount * FIXED_POINT_SCALAR) / _totalStaked;
			}
		}
	...
	function _resetConvertedFor(address _owner) internal {
		UserInfo storage _userInfo = _userInfos[_owner];
		_synthTokenConverted -= _userInfo.totalConverted;
	...
	function _increaseConvertedForLiquidator(address _liquidator, uint256 _overflow) internal {
		...
		_synthTokenConverted += _overflow;
	}
	...
	function _getUserInfoFor(address _user) internal view returns (uint256, uint256) {
		...
		if (_totalStaked != 0) {
			uint256 _toDistribute = _getNativeTokenToDistribute();
			uint256 _toConvertSynthAmount = _normalizeNativeToSynth(_toDistribute);
			_userTotalConverted += (_toConvertSynthAmount * _userTotalStaked) / _totalStaked;
		}
	...
	function _getNativeTokenToDistribute() internal view returns (uint256) {
		...
		uint256 _nativeTokenAvailable = nativeTokenReserve - _normalizeSynthToNative(_synthTokenConverted);
	...
	function _getMaxConvertedFor(address _owner) internal view returns (uint256) {
		UserInfo storage _userInfo = _userInfos[_owner];

		uint256 _convertedAmount = Math.min(_userInfo.totalConverted, _userInfo.totalStaked);
