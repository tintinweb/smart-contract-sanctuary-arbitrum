...
contract PplMinter is Ownable, ReentrancyGuard {
  ...
  // DISHES NFT addresses
  address public immutable nft1;
  address public immutable nft2;
  address public immutable nft3;
  ...
  function canUserMint(address _user) public view returns(bool) {
    if (
      IERC721(nft1).balanceOf(_user) > 0 &&
      IERC721(nft2).balanceOf(_user) > 0 &&
      IERC721(nft3).balanceOf(_user) > 0
    ) {
      return true;
    }
  ...
  function mint(
    ...
    require(canUserMint(_msgSender()) || _msgSender() == owner(), "Not eligible for minting");
