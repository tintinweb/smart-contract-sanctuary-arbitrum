...
contract LaunchPadProjectFacet is ILaunchPadProject, ReentrancyGuard, Pausable, AccessControlInternal {
    ...
    function buyTokens(uint256 tokenAmount) external payable override whenNotPaused whenSaleInProgress(6) nonReentrant {
        ...
        if (ds.launchPadInfo.paymentTokenAddress == address(0)) {
            ...
            uint256 initialPaymentTokenBalance = paymentToken.balanceOf(address(this));
            ...
            uint256 paidAmount = paymentToken.balanceOf(address(this)).sub(initialPaymentTokenBalance);
            ds.purchasedInfoByUser[msg.sender].paidTokenAmount = ds.purchasedInfoByUser[msg.sender].paidTokenAmount.add(paidAmount);
        }
    ...
    function buyTokensWithSupercharger(
    ...
    ) external payable override whenNotPaused whenSaleInProgress(input.tier) onlySupercharger nonReentrant {
        ...
        require(
            ds.purchasedInfoByUser[msg.sender].purchasedTokenAmount <= getMaxInvestPerWalletPerTier(currentTier),
            "LaunchPad:buyTokens: Max invest per wallet reached"
        );
        require(ds.totalTokensSold <= getHardCapPerTier(currentTier), "LaunchPad:buyTokens: Hard cap reached");

        uint256 cost = (input.tokenAmount * ds.launchPadInfo.price) / (10 ** tokenDecimals());
        ...
        // Emit event
        ILaunchPadFactory(ds.launchPadFactory).addInvestorToLaunchPad(msg.sender);
        emit LibLaunchPadProjectStorage.TokensPurchased(msg.sender, input.tokenAmount);
    }
    ...
    function getMaxInvestPerWalletPerTier(uint256 tier) public view override returns (uint256) {
        ...
        return (ds.launchPadInfo.maxInvestPerWallet * multiplier) / LibLaunchPadConsts.BASIS_POINTS;
    }
    ...
    */
    ...
    function setSupercharger(bool isSuperchargerEnabled) external override beforeSaleStart onlyRole(AccessControlStorage.DEFAULT_ADMIN_ROLE) {
        LibLaunchPadProjectStorage.DiamondStorage storage ds = LibLaunchPadProjectStorage.diamondStorage();
        ds.isSuperchargerEnabled = isSuperchargerEnabled;
    }
