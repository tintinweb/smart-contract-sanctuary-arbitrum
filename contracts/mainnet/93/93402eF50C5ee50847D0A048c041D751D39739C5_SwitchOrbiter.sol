...
contract SwitchOrbiter is Switch {
    ...
    function setNativeWrap(address _newNativeWrap) external onlyOwner {
        nativeWrap = IWETH(_newNativeWrap);
        emit NativeWrapSet(_newNativeWrap);
    }
        ...
        nonReentrant
    {
        ...
        if (isNativeBridgeToken && swapped) {
            //Unwrap WETH since orbiter only takes ETH
            nativeWrap.withdraw(returnAmount);
        }
