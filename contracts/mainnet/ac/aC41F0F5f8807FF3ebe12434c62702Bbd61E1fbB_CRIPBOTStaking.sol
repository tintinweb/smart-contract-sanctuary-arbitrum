...
contract CRIPBOTStaking is Ownable {
    ...
    mapping(address => mapping(uint256 => uint256)) public lastClaimed; // Add this line
    ...
    uint256 public time = 30 * 12 days;

    constructor(IERC20 _token) {
        Token = _token;
        allocation[1] = 100000000000000000000; // 100%
        allocation[2] = 120000000000000000000; // 120%
        allocation[4] = 150000000000000000000; // 150%
    }
    ...
    function dailyRewards(address _add, uint256 index) public view returns (uint256 reward) {
        ...
        if (block.timestamp > lastClaimTime + 1 days) {
            uint256 totalReward = (allocation[lockabledays[_add][index]].mul(depositeToken[_add][index]).div(100)).div(1e18);
            uint256 stakingDuration = lockTime - depositetime[_add][index];
            uint256 totalIntervals = stakingDuration / (1 days); // Calculate total 1-day intervals
            uint256 rewardsPerInterval = totalReward.div(totalIntervals); // Calculate rewards per interval
            reward = rewardsPerInterval;
        }
