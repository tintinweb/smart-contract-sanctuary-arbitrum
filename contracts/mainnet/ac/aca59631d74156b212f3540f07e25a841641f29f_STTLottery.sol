...
contract STTLottery is VRFV2WrapperConsumerBase, ConfirmedOwner {
    ...
    event Lottery (uint ticketID, address player, uint chainLInkNumber1, uint chainLInkNumber2, uint blockNumber, uint blockNumber1, uint blockNumber2, uint win);
    ...
    function random(uint _number) private returns(uint) {
        ...
        uint _chainlinkNumber1 = (_number % 9) + 0;
        uint _chainlinkNumber2 = ((_number / block.number) % 9) + 0;

        uint _win = calculate(STT.balanceOf(address(this)), rate_balance) * _chainlinkNumber1 * _chainlinkNumber2 * _number2 * _number3;
        ...
        tickets[lastTicket].blockNumber1 = _number2;
        tickets[lastTicket].blockNumber2 = _number3;
    ...
    function setRate(uint _rate_balance, uint _rate_bank) public onlyOwner {
       require(_rate_balance <= 123 && _rate_bank <= 123);
