...
library LibWallet {
    ...
    bytes32 constant REQUEST_TYPEHASH = keccak256(bytes(REQUEST_TYPE));
    ...
    function hash(Request memory request) internal pure returns (bytes32) {
        return keccak256(abi.encode(REQUEST_TYPEHASH, request.to, request.token, request.id, request.amount, request.tokenType, request.start, request.end));
    }

    function validate(Request memory request) internal view {
...
contract BetterFanWallet is Ownable(msg.sender), EIP712('Better Fan Wallet', '1'), ERC1155Holder {
    mapping(bytes32 => bool) public processed;

    event Withdraw(LibWallet.Request request, bytes signature);
    event Deposit(address sender, uint256 value);

    function verify(LibWallet.Request memory request, bytes memory _signature) public view returns (address) {
    ...
    function withdraw(LibWallet.Request memory request, bytes memory signature) external {
        ...
        bytes32 hash = LibWallet.hash(request);
        require(!processed[hash], "This transaction already processed!");
