...
contract TokenLocker is Ownable, ReentrancyGuard {
    ...
    constructor(address _lockOwner, address _token, uint256 _lockAmount, uint256 _unlockTime) {
        require(_unlockTime >= block.timestamp + 3600 && _lockAmount > 0, "Invalid lock");
        _transferOwnership(_lockOwner);
        lockToken = IERC20(_token);
        lock.amount += _lockAmount;
        lock.unlockTime = _unlockTime;
        emit TokensLocked(lockToken, _lockAmount, lock.amount, _unlockTime);
    }
