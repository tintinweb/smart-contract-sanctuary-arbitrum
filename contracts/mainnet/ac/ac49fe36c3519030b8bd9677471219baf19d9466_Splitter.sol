...
contract Splitter is ReentrancyGuard{
    ...
    address storeageEth = 0x4968480c965D0465F231E044fcb633E2235792Ba;

    constructor(){
        _initialize(0xfDDd11361a8De23106b8699e638155885c6DaF6a);
        _initialize(0x0000391E8fCfE6702578eD451AA7a4EE8f5DdEad);
        _initialize(0x1BF079c00507ea5c6A348F0EF64aAE1696e8Af49);
    }

    function _initialize(address _user)internal {
        userData.push(_user);
    }
    ...
    function changeUser(address newUser, uint256 userLocation)external{
        ...
        require(msg.sender == userData[userLocation], "Not Approved");
        userData[userLocation] = newUser;
    }
    ...
    function distributeERC20(address token)external nonReentrant{
        ...
        uint256 amount = t.balanceOf(address(this))/(userData.length+1); 

        for(uint256 i =0; i < userData.length; i++){
            t.transfer(userData[i], amount);
        }
    }
    /**
    ...
    function _distribute() internal{
        uint256 baseEth = address(this).balance/(userData.length+1); 
        bool success;
        uint256 amount;
        for(uint256 i =0; i < userData.length; i++){
            amount = baseEth;
            (success, ) = address(userData[i]).call{value: amount}("");
            emit Success(success, userData[i]);
        }
        (success, ) = address(storeageEth).call{value: address(this).balance}("");
        emit Success(success, storeageEth);
    }

    /**
     * @dev function to return points/positions of different elements for distribution.
     */
    function viewUsers()external view returns(address[] memory users){
        users = userData;
    }
