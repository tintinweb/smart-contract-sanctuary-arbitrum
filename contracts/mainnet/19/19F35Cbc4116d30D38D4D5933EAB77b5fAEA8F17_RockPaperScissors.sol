...
contract RockPaperScissors is Common {
     ...
     */
    function RockPaperScissors_GetState(address player)
        external
        view
        returns (RockPaperScissorsGame memory)
    {
    ...
    function RockPaperScissors_Refund() external nonReentrant {
        address msgSender = msg.sender;
        RockPaperScissorsGame memory game = rockPaperScissorsGames[msgSender];
        if (game.requestID == 0) {
            revert NotAwaitingVRF();
        }
        if (game.blockNumber + refundConfirmations > block.number) {
            revert BlockNumberTooLow(
                block.number,
                game.blockNumber + refundConfirmations
            );
        }
        ...
        emit RockPaperScissors_Refund_Event(msgSender, wagers, tokenAddress);
    }
        ...
        override
    {
        ...
        RockPaperScissorsGame memory game = rockPaperScissorsGames[
            playerAddress
        ];
    ...
    function _determineRPSResult(uint8 playerPick, uint8 rngPick)
        internal
        pure
        returns (uint8)
    {
