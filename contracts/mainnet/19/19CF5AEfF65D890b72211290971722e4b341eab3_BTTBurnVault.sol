...
/// @title BETT (BBTT)
/// @notice This contract creates the token
contract BTT is ERC20 {
    /// The name of the token to be created upon deployment
    string private constant nameForDeploy = "BerrugaTieneToken";
    ...
    constructor(uint256 totalSupply) ERC20(nameForDeploy, symbolForDeploy) {
...
contract BTTBurnVault {
    ...
    using SafeERC20 for BTT;
    /// The address of the BTCb token
    address constant addrWbtc = 0x2f2a2543B76A4166549F7aaB2e75Bef0aefC5B0f;

    IERC20 constant wbtc = IERC20(addrWbtc);
    BTT immutable btt;

    event burnMade(uint256 amtBurned, uint256 wtcbWithdrew);

    /// @notice Constructor sets the AMT token
    /// @param _addrBtt The address of the AMT token
    constructor(address _addrBtt) {
        btt = BTT(_addrBtt);
    }
