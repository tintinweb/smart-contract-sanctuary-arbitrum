...
abstract contract UniMixin {
    ...
    function _getSwapAmountUni(
        ...
        uint256 denominator = IUniswapV2Router02(_router).quote(
            halfInvestment,
            _reserveA + halfInvestment,
            _reserveB - nominator
        );
 ...
 */
contract ReaperStrategyGranary is ReaperBaseStrategyv3_2, IFlashLoanReceiver, UniMixin {
    using ReaperMathUtils for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // 3rd-party contract addresses
    address public constant UNI_ROUTER = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
    ILendingPoolAddressesProvider public constant ADDRESS_PROVIDER =
        ILendingPoolAddressesProvider(0x642cc899652B068D1bED786c4B060Ec1027D1563);
    IAaveProtocolDataProvider public constant DATA_PROVIDER =
        IAaveProtocolDataProvider(0x96bCFB86F1bFf315c13e00D850e2FAeA93CcD3e7);
    IRewardsController public constant REWARDER = IRewardsController(0x7A191973Eaf8cdcC4dE683d10C1E11c5a5bc717D);
    ...
    function _harvestCore() internal override returns (uint256 feeCharged) {
        ...
        uint256 wantGeneratedFromRewards = balanceOfWant() - wantBalBefore;
        feeCharged = (wantGeneratedFromRewards * totalFee) / PERCENT_DIVISOR;
        if (feeCharged != 0) {
            want.safeTransfer(treasury, feeCharged);
        }
    }
    ...
    function _leverUpMax() internal {
        ...
        if (desiredBorrow > borrow + minLeverageAmount) {
            ...
            if (want.balanceOf(address(gWant)) >= borrowIncrease) {
    ...
    function _leverUpStep(uint256 _totalBorrowIncrease) internal returns (uint256) {
        ...
        allowance = MathUpgradeable.min(allowance, want.balanceOf(address(gWant)));
