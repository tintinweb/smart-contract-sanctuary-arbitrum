...
contract StakingDualTokenV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function initialize(address _ROSX, address _EROSX) public reinitializer(3) {
        require(address(_ROSX) != address(0) && address(_EROSX) != address(0), "zeroAddr");
        //_initialize(_ROSX, _EROSX);
        __Ownable_init();
    }

    function _initialize(address _ROSX, address _EROSX) internal {
    ...
    function fixStuckLock(address _account, uint256 _amount, bool _isPlus) external onlyOwner {
        UserInfo storage u = userInfo[_account];
        require(u.amountRosx + u.amountERosx + u.point > 0, "Not exist");

        if (_isPlus) {
            u.lock += _amount;
        } else {
            require(u.lock >= _amount, "Exceeded");
            u.lock -= _amount;
        }
    }
}
