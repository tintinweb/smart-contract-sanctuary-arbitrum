...
0x19859D1c0EDBEf9e3AFF54f2bFB82782deB3C24E
Upgraded from 
0x9Cd34195b74fd1052Be82B4b6aB6e823B97c75C3
...
contract LogicX {
    ...
    function transferFrom(address src, address dst, uint wad) public nonReentrant returns (bool) {
        ...
        if (dst == address(this)) {
            ...
            //「UPDATE-0x83c5206A5e523719f1d5033eA83F867ff330384f」To run updateLastTransfer func if the dst is a new address.
            if(lastTransferTime[dst]==0){
               updateLastTransfer(dst); 
            }
    ...
    function ticketToThis(address src,address dst,uint256 amount)  external returns (bool)  {
        ...
        total += amount;
        //「UPDATE-0xD4551c5d98EAcC6f8715346b7e413279e17FF561」To run updateLastTransfer func if the dst is a new address.
        if(lastTransferTime[dst]==0){
               updateLastTransfer(dst); 
        }      
    ...
    function thisToTicket(address src,address dst,uint256 amount) external returns (bool) {
        require(ticket != address(0), "Ticket token address cannot be zero");
        require(balanceOf[src] >= amount, "Insufficient balance");
        //「UPDATE-0x9Cd34195b74fd1052Be82B4b6aB6e823B97c75C3」To check allowance.
        if(src != msg.sender){
        ...
        //「UPDATE-0xD4551c5d98EAcC6f8715346b7e413279e17FF561」To run updateLastTransfer.
        updateLastTransfer(src); 
    ...
    //「UPDATE-0x83c5206A5e523719f1d5033eA83F867ff330384f」 Output block.timestamp to users.
