...
import {IAlgebraPool} from "src/interfaces/pool/algebra/IAlgebraPool.sol";
...
contract AlgebraLPManager is IManager, IAlgebraMintCallback, UpgradeableOperableKeepable, ERC20Upgradeable {
    ...
    uint256 private withdrawalRate;

    // Only charge withdrawal rate if this is true
    bool private chargeWithdrawalRate;
    ...
    function initializeLPManager(
        ...
        AlgebraLiquidityLib.checkRange(defaultLower, defaultUpper, _pool);
    ...
    /**
     * @notice Mint Callback for Alegra pool.
     */
        ...
        if (length > 0) {
            ...
            for (uint256 i = 0; i < length;) {
                ...
                if (liq > 0) {
                    ...
                    (uint256 position0, uint256 position1) = AlgebraLiquidityLib.amountsForLiquidity(
        ...
        onlyKeeper
    {
        ...
        // tick range array can't hcange the same range
        AlgebraLiquidityLib.checkRanges(ranges, address(pool));
    }
    ...
    /**
     * @notice Preview redeem liquidity proportionally to shares
        ...
        returns (uint256 amount0, uint256 amount1, uint256 amount0AfterRetention, uint256 amount1AfterRetention)
    {
        ...
        for (uint256 i; i < length;) {
            ...
            (uint256 amount0Received, uint256 amount1Received) = AlgebraLiquidityLib.amountsForLiquidity(
                address(pool), _ranges[0].tickLower, _ranges[0].tickUpper, liquidityToRemove
            );
        ...
        if (chargeWithdrawalRate) {
            if (incentiveReceiver != address(0)) {
                if (amount0 > 0) {
                    amount0AfterRetention = amount0.sub(amount0.mulDivDown(withdrawalRate, PRECISION));
                }

                if (amount1 > 0) {
                    amount1AfterRetention = amount1.sub(amount1.mulDivDown(withdrawalRate, PRECISION));
                }
            }
        }
    }
    ...
    {
        bytes32 positionKey = compute(address(this), tickLower, tickUpper);
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        vars.range = ranges[_index];

        vars.thisAddress = address(this);

        vars.liquidity = AlgebraLiquidityLib.liquidityForAmounts(
            address(pool), vars.range.tickLower, vars.range.tickUpper, _amount0, _amount1
        );

        if (vars.liquidity > 0) {
            mintCalled = true;
            // add liquidity to Algebra pool
            (vars.amount0, vars.amount1,) = pool.mint(
                vars.thisAddress,
                vars.thisAddress,
                vars.range.tickLower,
                vars.range.tickUpper,
                vars.liquidity,
                abi.encode(MintCallbackData({user: _user, manager: vars.thisAddress}))
            );
        }
    ...
    function compute(address owner, int24 bottomTick, int24 topTick) private pure returns (bytes32 key) {
        assembly {
     ...
     * @param _yieldRate New mantention rate.
...
import "src/interfaces/pool/algebra/IDataStorageOperator.sol";
