...
library ParaSwapUtils {
  ...
  function swap(address to, bytes memory callData) external {
    ...
    (SimpleData memory data) = abi.decode(callData, (SimpleData));
    IERC20(data.fromToken).safeApprove(approvalAddress, data.fromAmount);
  ...
  function _validateCallData(address to, bytes memory callData) internal view {
    ...
    (SimpleData memory data) = abi.decode(callData, (SimpleData));
    address receiver = data.beneficiary;
