...
contract UTPVester is IVester, IERC20, ReentrancyGuard, Governable {
    using SafeMath for uint256;
    ...
    uint256 public vestingDuration; // vest duration
    uint256 public pairFactor = 1e6;  // proportion of participation vest

    address public vestToken;       // vest token
    address public pairToken;       // token of participate vest 
    address public claimableToken;  // vested token

    address public override rewardTracker;  // contract of get unvested token info, for URP vesting, this is deflation manager address

    uint256 public override totalSupply;    // total unvested token
    uint256 public pairSupply;              // total staked pairToken

    bool public hasMaxVestableAmount;       // is limit vest amount

    mapping (address => uint256) public balances;                            // user => unvested amount
    mapping (address => uint256) public override pairAmounts;                // user => pairToken amount
    mapping (address => uint256) public override cumulativeClaimAmounts;     // user => total vested amount
    mapping (address => uint256) public override claimedAmounts;             // user => claimed amount
    mapping (address => uint256) public lastVestingTimes;                    // user => operate time
    ...
    event SetHasMaxVestableAmount(bool hasMaxVestableAmount);
    event SetPairFactor(uint256 pairFactor);
    ...
    function setHasMaxVestableAmount(bool _hasMaxVestableAmount) external onlyGov {
        ...
        emit SetHasMaxVestableAmount(_hasMaxVestableAmount);
    }

    function setPairFactor(uint256 _pairFactor) external onlyGov {
        pairFactor = _pairFactor;
        emit SetPairFactor(_pairFactor);
    }
    ...
    function withdraw() external nonReentrant {
        ...
        if (balance > 0) {
            IERC20(vestToken).safeTransfer(_receiver, balance);
    ...
    function getMaxVestableAmount(address _account) public override view returns (uint256) {
        ...
        return IUtpRewardTracker(rewardTracker).cumulativeRewards(_account, vestToken);
    }
    
    function getCombinedAverageStakedAmount(address _account) public override view returns (uint256) {
        return IUtpRewardTracker(rewardTracker).averageStakedAmounts(_account, vestToken);
    }
    
    function getPairAmount(address _account, uint256 _vestAmount) public view returns (uint256) {
        if (!hasRewardTracker()) { return 0; }
        ...
        return _vestAmount.mul(combinedAverageStakedAmount).mul(pairFactor).div(maxVestableAmount).div(1e6);
    }
    
    function getVestAmount(address _account, uint256 _pairAmount) public view override returns (uint256) {
        if (!hasRewardTracker()) { return 0; }
        ...
        if(_pairAmount > combinedAverageStakedAmount){
            return maxVestableAmount;
        }
        
        return _pairAmount.mul(maxVestableAmount).div(combinedAverageStakedAmount);
    }
    
    function getMaxVestAmount(address _account) public view returns (uint256) {
        if (!hasRewardTracker()) { return 0; }
        ...
        uint256 _pairAmount = IERC20(pairToken).balanceOf(_account);
        if (pairAmounts[_account] > 0 && balances[_account] > 0) {
            _pairAmount = _pairAmount.add(
                pairAmounts[_account].sub(
                    amount.mul(pairAmounts[_account]).div(balances[_account])
                )
            );
        }
        uint256 vestedAmount = getVestedAmount(_account);
        if (_pairAmount > combinedAverageStakedAmount){
            return maxVestableAmount > vestedAmount ? maxVestableAmount.sub(vestedAmount) : 0;
        }

        uint256 maxAmount = _pairAmount.mul(maxVestableAmount).div(
            combinedAverageStakedAmount
        );

        maxAmount = maxAmount > maxVestableAmount ? maxVestableAmount : maxAmount;
        return vestedAmount > maxAmount ? 0 : maxAmount.sub(vestedAmount);
    }
    ...
    function transfer(address /* recipient */, uint256 /* amount */) public override returns (bool) {
        revert("UTPVester: non-transferrable");
    }
    ...
    function _mint(address _account, uint256 _amount) private {
        require(_account != address(0), "UTPVester: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) private {
        require(_account != address(0), "UTPVester: burn from the zero address");

        balances[_account] = balances[_account].sub(_amount, "UTPVester: burn amount exceeds balance");
    ...
    function _burnPair(address _account, uint256 _amount) private {
        ...
        pairAmounts[_account] = pairAmounts[_account].sub(_amount, "UTPVester: burn amount exceeds balance");
    ...
    function _deposit(address _account, uint256 _amount) private {
        require(_amount > 0, "UTPVester: invalid amount");

        _updateVesting(_account);

        IERC20(vestToken).safeTransferFrom(_account, address(this), _amount);
        ...
        if (hasMaxVestableAmount) {
            ...
            require(getVestedAmount(_account) <= maxAmount, "UTPVester: max vestable amount exceeded");
        }
    ...
    function _updateVesting(address _account) private {
        ...
        if (pairAmounts[_account] > 0) {
            if (balances[_account] == 0) 
                _burnPair(_account, pairAmounts[_account]);
            else 
                _burnPair(_account, amount.mul(pairAmounts[_account]).div(balances[_account]));
        }
        _burn(_account, amount);
        cumulativeClaimAmounts[_account] = cumulativeClaimAmounts[_account].add(amount);
        IMintable(vestToken).burn(address(this), amount);
    }
    ...
    function _validateHandler() private view {
        require(isHandler[msg.sender], "UTPVester: forbidden");
    }
}