...
contract SrvTokenUSDCPool is Ownable,Pausable,ReentrancyGuard{
    using SafeMath for uint256;
    ...
    constructor(IERC20 _token,IERC20 _USDC)  {
        Token = _token;
        USDC=_USDC;
        allocation[30]  = 317590000; 
        allocation[120] = 635190000; 
        allocation[365] = 952690000; 

    }

    // 30days lock 10% apy  ==  0.00000000031759 per second == 317590000
    // 120days lock 20% apy ==  0.00000000063519 per second == 635190000
    // 365days lock 30% apy ==  0.00000000095269 per second == 952690000
    ...
    function farm(uint256 _amount, uint256 _lockableDays) external whenNotPaused nonReentrant	
    {
        ...
        require(_lockableDays==30||_lockableDays==120||_lockableDays==365, "Invalid day selection");
        ...
        function pendindRewards(address _add) public view returns(uint256 reward)
    {
        ...
        for(uint256 z=0 ; z< depositeToken[_add].length;z++){
           ...
           tokenperct=depositeToken[_add][z]*allocation[lockabledays[_add][z]];
            Reward += (sec*tokenperct).div(1e30);
        }
    ...
    function harvest(uint256 [] memory _index) public  {
        ...
        require(block.timestamp > lockTime,"unstake time not reached!");
          sec =(block.timestamp-lastclaimedtime[msg.sender][_index[z]]);
           tokenperct=depositeToken[msg.sender][_index[z]]*allocation[lockabledays[msg.sender][_index[z]]];
            Reward += (sec*tokenperct).div(1e30);
            ...
            for(uint256 i = _index[t]; i <  depositeToken[msg.sender].length - 1; i++) 
        {
          ...
          lastclaimedtime[msg.sender].pop();
             ...
             IERC20(Token).safeTransfer(msg.sender, Users[msg.sender].WithdrawDepositeAmount);
             IERC20(USDC).safeTransfer(msg.sender,  Users[msg.sender].WithdrawAbleReward);
      ...
      function emergencyWithdrawUSDC(uint256 _token) external onlyOwner {
         
         IERC20(USDC).safeTransfer(msg.sender,_token);
    }
   ...
   function emergencyWithdrawtokens(IERC20 _token,uint256 _amount) external onlyOwner {
    ...
    function changePercentages(uint256 _0dayspercent,uint256 _7dayspercent,uint256 _30dayspercent) external onlyOwner{
     allocation[0] = _0dayspercent;
