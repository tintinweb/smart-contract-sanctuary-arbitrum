...
contract PaalXSwapContract is Context, Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(address => uint256)) public referrals; // mapping(referrer => mapping(tokenAddress => feeAmount))
    mapping(address => mapping(address => bool)) public referredBy; // mapping(referrer => mapping(msg.sender => true))
    mapping(address => uint256) public totalReferralFeePerToken;
    ...
    function pauseRefferals() external onlyOwner {
        require(isReferralStopped == true, "Already Paused");
    ...
    function resumeRefferals() external onlyOwner {
        require(isReferralStopped == false, "Already Active");
        isReferralStopped = false;
