...
contract Timelock is Ownable, EditQueueInteractor {
    // Timelock constructor
    constructor(address _owner) Ownable(_owner) EditQueueInteractor() {}
    ...
    uint256 public sensitiveActionTimelock = 2 minutes;
    uint256 public timelockEditDelay = 14 minutes;

    /**
     * @dev Queue an edit for the timelock duration
     * @param _newTimelock The new timelock duration in seconds
     */
    function queueTimelockDurationEdit(uint256 _newTimelock) public onlyOwner {
        require(_newTimelock > 0, "Timelock !>= 0");
        registerQueuedEdit(block.timestamp + sensitiveActionTimelock, address(0), false, _newTimelock, "", "timelockDuration");
    }

    /**
     * @dev Commit the queued timelock edit
     */
    function commitQueuedTimelockEdit() public onlyOwner {
        // Get the indexes of the committable edits
        uint256[] memory committableEditIndexes = getCommittableEditIndexes("timelockDuration");

        // Loop through the committable edits, commit
        for (uint256 i = 0; i < committableEditIndexes.length; i++) {
            // Get the matching queue edit
            (, , , uint256 newTimelock, , ) = getQueuedEdit(committableEditIndexes[i]);

            sensitiveActionTimelock = newTimelock;
            removeEditFromQueue(committableEditIndexes[i]);
        }
    }
...
contract CommunityCommittee is Timelock {
    ...
    mapping(address => bool) isCommunityCommitteeMember;
    address[] communityCommittee;

    // Community committee modifier
    modifier onlyCommunityCommittee() {
        require(isCommunityCommitteeMember[msg.sender], "Sender !committee member");
        _;
    }
     ...
     */
    function queueCommunityCommitteeStatusChange(address _member, bool _isMember) public onlyOwner {
     ...
     */
    function commitCommunityCommitteeStatusChanges() public onlyOwner {
        ...
        for (uint256 i = 0; i < committableEditIndexes.length; i++) {
            ...
            (, address addressPayload, bool boolPayload, , , ) = getQueuedEdit(committableEditIndexes[i]);
            ...
            if (boolPayload) {
                bool isMember = false;
                for (uint256 j = 0; j < communityCommittee.length; j++) {
...
contract PausingCommittee is CommunityCommittee {
    ...
    mapping(address => uint8) hasPausingPrivileges;
    address[] pauseCommittee;

    // Pause committee modifier
    modifier onlyPausers() {
        require(hasPausingPrivileges[msg.sender] == 1 || hasPausingPrivileges[msg.sender] == 3, "Sender !pauser");
        _;
    }
    modifier onlyUnpausers() {
        require(hasPausingPrivileges[msg.sender] == 2 || hasPausingPrivileges[msg.sender] == 3, "Sender !unpauser");
        _;
    }
     ...
     */
    function queuePauseCommitteeStatusChange(address _member, uint8 _permissions) public onlyCommunityCommittee {
        // Queue edit
     ...
     */
    function commitPauseCommitteeStatusChanges() public onlyCommunityCommittee {
        // Get all committable index items
        ...
        for (uint256 i = 0; i < committableEditIndexes.length; i++) {
            ...
            (, address addressPayload, , uint256 uintPayload, , ) = getQueuedEdit(committableEditIndexes[i]);
            ...
            if (uint8(uintPayload) == 0) {
                uint index = 0;
                for (index = 0; index < pauseCommittee.length; index++) {
                    if (pauseCommittee[index] == addressPayload) {
                        pauseCommittee[index] = pauseCommittee[pauseCommittee.length - 1];
                        pauseCommittee.pop();
                        break;
                    }
                }
            ...
            if (uint8(uintPayload) != 0) {
                bool isMember = false;
                for (uint256 j = 0; j < pauseCommittee.length; j++) {
     ...
     */
    function pauseMinting() public onlyPausers {
        mintingEnabled = false;
        emit MintingEnabled(false);
    }

    // Unpausing timelock trackers
    uint256 unpauseTimelock;

    /**
     * @dev Function to queue a minting pause, this is timelocked
     */
    function queueUnpauseMinting() public onlyUnpausers {
     ...
     */
    function commitUnpauseMinting() public onlyUnpausers {
...
contract MintingLogic is ERC20Burnable, OFT, PausingCommittee {
    ...
    constructor(
        ...
        address _lzEndpoint,
        address _owner
    ) OFT(_name, _symbol, _lzEndpoint, _owner) PausingCommittee(_owner) {}
    ...
    /**
     * @dev Read the status of a specific minting status entry
     * @param _uid The unique identifier of the minting request.
     * @return The status of the minting request.
     */
    function readMintingStatus(uint256 _uid) public view returns (MintingStatus) {
        return mintingStatus[_uid];
    }
    ...
    modifier onlyMintingRequestCreator() {
        require(isMintingRequestCreator[msg.sender], "Caller !minting request creator");
        _;
    }
    modifier onlyMintingAdministrators() {
        require(isMintingAdministrator[msg.sender], "Caller !minting administrator");
        _;
    }
     ...
     */
    function addMintingRequestCreator(address _creator) public onlyOwner {
     ...
     * @notice This function is only called by the Taofu team.
     */
    function removeMintingRequestCreator(address _creator) public onlyOwner {
     ...
     * @notice This function is only called by the Taofu team.
     */
    function addMintingAdministrator(address _administrator) public onlyOwner {
     ...
     * @notice This function is only called by the Taofu team.
     */
    function removeMintingAdministrator(address _administrator) public onlyOwner {
     ...
     * @param _price The relative price of Taofu in Tao.
     * @notice This function is only called by the minting request creators.
     */
    function createMintingRequest(uint256 _uid, uint256 _amount, uint256 _price) public onlyMintingRequestCreator {
        // Check that amount is positive
        require(_amount > 0, "Amount !positive");

        // Add the minting request
        mintingRequests.push(MintingRequest(_uid, msg.sender, _amount, _price));
        emit MintingRequestCreated(msg.sender, _amount, _price, _uid);
     ...
     */
    function approveMintingRequest(uint256 _uid) public onlyMintingAdministrators {
        ...
        while (mintingRequests[i].uid != _uid) {
            ...
            require(i < mintingRequests.length, "Minting request !found");
        }
     ...
     * @notice This function is only called by the minting administrators.
     */
    function rejectMintingRequest(uint256 _uid) public onlyMintingAdministrators {
        // Find the minting request
        uint256 i = 0;
        while (mintingRequests[i].uid != _uid) {
            i++;
     ...
     */
    function queueEditMintingDelay(uint256 _mintingDelay) public onlyOwner {
     ...
     */
    function commitQueuedEditMintingDelay() public onlyOwner {
        ...
        for (uint256 i = 0; i < committableEditIndexes.length; i++) {
            ...
            (, , , uint256 newMintingDelay, , ) = getQueuedEdit(committableEditIndexes[i]);
     ...
     */
    function createMintingQueueItem(uint256 _uid, address _recipient, uint256 _amount) public onlyMintingAdministrators {
        // Check that amount is positive
        require(_amount > 0, "Amount !positive");
     ...
     * @notice Anyone can call this function, but we assume it is mostly called by the Taofu team.
     */
    function deliverMintingQueueItems(uint256 _max_amount_to_process) public {
        // If minting is disabled, do nothing
        require(mintingEnabled, "Minting !enabled");

        uint256 i = 0;
        uint256 processed = 0; // Counter for the number of items processed
     ...
     */
    function readMintingQueueItems() public view returns (MintingQueueItem[] memory) {
        return mintingQueue;
    }
     ...
     */
    function calculateTotalMintingQueueAmount() public view returns (uint256) {
        uint256 total = 0;
        for (uint256 i = 0; i < mintingQueue.length; i++) {
            total += mintingQueue[i].amount;
        }
        return total;
    }
...
contract VetoCommittee is MintingLogic {
    ...
    constructor(string memory _name, string memory _symbol, address _lzEndpoint, address _owner) MintingLogic(_name, _symbol, _lzEndpoint, _owner) {}
    ...
    modifier onlyVetoParticipants() {
        require(isVetoParticipant[msg.sender], "Caller !veto participant");
        _;
    }
     ...
     * @notice This function is only called by the Taofu team.
     */
    function addVetoParticipant(address _participant) public onlyOwner {
     ...
     * @notice This function is only called by the Taofu team.
     */
    function removeVetoParticipant(address _participant) public onlyOwner {
     ...
     */
    function vetoMintingQueueItem(uint256 _index) public onlyVetoParticipants {
        require(_index < mintingQueue.length, "Minting queue item !exist");
     ...
     * @notice This function can be called by any of the veto participants. It vetoes the minting queue item.
     */
    function vetoMintingQueueItemByRecipient(address _recipient) public onlyVetoParticipants {
        // Track index of the next item to be kept/protected from deletion
        uint256 index_of_next_keeper = 0;

        // Loop over items, find keepers, mark vetos for replacement
        for (uint256 i = 0; i < mintingQueue.length; i++) {
            // If this should be vetoed, do not protect it
            if (mintingQueue[i].recipient == _recipient) {
                emit VetoPerformed(mintingQueue[i].recipient, mintingQueue[i].amount);

                // Register the status of the minting request
                editMintingStatus(mintingQueue[i].uid, MintingStatus.Vetoed);
     ...
     * @notice This function can be called by any of the veto participants. It vetoes the minting queue item.
     */
    function vetoAllMintingQueueItems(bool _cheap_mode) public onlyVetoParticipants {
...
contract EditQueueInteractor {
     ...
     */
    function setEditQueue(address _edit_queue_address) public {
        require(address(editQueue) == address(0), "EditQueue already set");
     ...
     */
    function removeEditFromQueue(uint256 _index) public {
...
contract TaofuOFT is VetoCommittee {
    /**
     * @dev Taofu token constructor
     * @param _name The name of the token
     * @param _symbol The symbol of the token
     * @param _lzEndpoint The LayerZero Endpoint address, differs per chain: https://docs.layerzero.network/contracts/endpoint-addresses
     * @param _owner The owner of the token after the deployment is complete (this is not the deployment key)
     */
    constructor(string memory _name, string memory _symbol, address _lzEndpoint, address _owner) VetoCommittee(_name, _symbol, _lzEndpoint, _owner) {}
}