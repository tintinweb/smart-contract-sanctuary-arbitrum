...
contract PerpetualTradeFunctions is PerpetualRebalanceFunctions {
    using ABDKMath64x64 for int128;
    using ConverterDec18 for int128;
    ...
    function _trade(IPerpetualOrder.Order memory _order, bytes32 _digest) internal returns (bool) {
        ...
        require(perpetual.state == PerpetualState.NORMAL, "state should be NORMAL");
        {
            ...
            (fPrice, _order.fAmount) = _getTradeLogic().preTrade(_order);
            if (fPrice == 0) {
                return false;
            }
            // override trade amount in order
            ...
            int128 fTraderPos = marginAccounts[_order.iPerpetualId][_order.traderAddr].fPositionBC;
            ...
            (brkrFeeTbps, protocolFeeTbps) = _doMarginCollateralActions(
                perpetual,
                fTraderPos,
                fPrice,
                _order
            ...
            if (!isClose || _order.fAmount.abs() > fTraderPos.abs()) {
                // we are flipping the position sign or increasing the trade size
                // ensure resulting position not too large
                // (we no longer update k-star here, for gas reasons -> kstar is
                // updated in rebalance at the end of trade execution)
                _doOpeningTradeActions(fTraderPos, _order);
            }
        ...
        // distribute fees
        int128 fFee = _getTradeLogic().distributeFees(
            _order,
            brkrFeeTbps,
            protocolFeeTbps,
            !isClose
        ...
        // only rebalance for trade
        _getRebalanceLogic().rebalance(perpetual.id);
        // directly distribute fees
        {
            emit Trade(
                perpetual.id,
                _order.traderAddr,
                ...
                fFee,
                fPnLCC,
                _getBaseToCollateralConversionMultiplier(perpetual, false, false)
            );
        }
        if (fNewPos == 0) {
            // trader closed position, set position to zero
            marginAccounts[perpetual.id][_order.traderAddr].fLockedInValueQC = 0;
     ...
     * @param _order order struct
     * @return broker fee, exchange fee in tbps
     */
    function _doMarginCollateralActions(
        ...
        uint16 brkrFeeRateTbps;
        uint16 protocolFeeTbps;
        (brkrFeeRateTbps, protocolFeeTbps) = _setBrokerFeeAndUpdateVolumeEMA(_order);
        if (
            ...
            return (brkrFeeRateTbps, protocolFeeTbps);
        }
        int128 fDeposit;
        // determine target leverage
        {
            ...
            int128 fTargetLeverage = isFlip || isOpen
                ? (_order.leverageTDR).TDRToABDK()
                : int128(0);
            fDeposit = _getMarginViewLogic().calcMarginForTargetLeverage(
                _perpetual.id,
                _fTraderPos,
                _fPrice,
                _order.fAmount,
        ...
        int128 fTotalFee;
        {
            int128 fx = _getBaseToCollateralConversionMultiplier(_perpetual, false, false);
            int128 fTradeAmountCC = _order.fAmount.abs().mul(fx);
            uint16 fee = brkrFeeRateTbps + protocolFeeTbps;
            fTotalFee = fTradeAmountCC.mul(ConverterDec18.tbpsToABDK(fee));
            if (_order.executorAddr != address(0x0)) {
    ...
    function _executeCancelOrder(uint24 _perpetualId, bytes32 _digest) internal {
        if (executedOrCancelledOrders[_digest] & MASK_ORDER_CANCELLED != 0) {
            return;
        }
        executedOrCancelledOrders[_digest] = MASK_ORDER_CANCELLED;
     ...
     * @param _order            order struct
     * @return broker fee, exchange fee in tenth of a basis points
     */
    function _setBrokerFeeAndUpdateVolumeEMA(
        IPerpetualOrder.Order memory _order
    ) internal returns (uint16, uint16) {
        uint16 xchFee = _getBrokerFeeLogic().determineExchangeFee(_order);
        ...
        // signature was checked in determineExchangeFee. That is, if we are here,
        ...
        return (_order.brokerFeeTbps, xchFee);
    }
}
...
contract PerpetualLimitTradeManager is
    ...
    IPerpetualLimitTradeManager
{
    ...
    /**
     * Execute order sent via limit order book
     * @param _order Perpetual order
     * @param _executor Address of the executor that is rebated
     * @return true if the order execution was successful
     * or order is close only and there is no position to close; false otherwise
     */
    function tradeViaOrderBook(
        Order memory _order,
        address _executor
    ) external override whenNotPaused updateFundingAndPrices(_order.iPerpetualId) returns (bool) {
        // ensure trade was sent via order-book
        require(
            LimitOrderBookFactory(orderBookFactory).orderBooks(_order.iPerpetualId) == msg.sender,
            "only order-bk"
        );
        // ensure the order is not expired
        if (_order.iDeadline <= block.timestamp) {
            _rebateExecutor(_order);
            return false;
        }
        // ensure the order was submitted sufficiently long ago (>= trade delay)
        // or the executor has been pre-approved by the LOB
        require(
            block.timestamp >= _order.submittedTimestamp + iTradeDelaySec ||
                LimitOrderBookFactory(orderBookFactory).approvedExecutor(_executor),
            "delay required"
        );

        /* Order book contract must test the following conditions
            ...
            require(!isOrderExecuted(digest), "order executed");
            require(!isOrderCancelled(digest), "order canceled");
            require(_order.iDeadline >= block.timestamp, "deadline exceeded");
        */

        if (_order.flags.isStopOrder()) {
            // reverts if trigger not met
            _getTradeLogic().validateStopPrice(
                _order.fAmount > 0,
                _getPerpetualMarkPrice(_getPerpetual(_order.iPerpetualId), false),
                _order.fTriggerPrice
            );
        }

        bytes32 digest = _order._getDigest(address(this), true);
        // ensure that if the order is close only, there is an open position to close
        if (
            _order.flags.isCloseOnly() &&
            marginAccounts[_order.iPerpetualId][_order.traderAddr].fPositionBC == 0
        ) {
            _executeCancelOrder(_order.iPerpetualId, digest);
            _rebateExecutor(_order);
            return true;
        }
    ...
    /**
     * @notice Rebate executor for trying to execute a market order
     * @param _order Perpetual order
     */
    function _rebateExecutor(Order memory _order) internal {
        if (_order.executorAddr == address(0x0)) {
            return;
        }
        // market order failed due to limit price, we rebate the executor
        PerpetualData storage perpetual = _getPerpetual(_order.iPerpetualId);
        LiquidityPoolData storage liqPool = _getLiquidityPoolFromPerpetual(perpetual.id);
        _transferFromUserToVault(liqPool, _order.traderAddr, perpetual.fReferralRebateCC);
        _transferFromVaultToUser(liqPool, _order.executorAddr, perpetual.fReferralRebateCC);
    }
     ...
     */
    function executeCancelOrder(uint24 _perpetualId, bytes32 _digest) external {
        // ensure request was sent via order-book
        require(
            LimitOrderBookFactory(orderBookFactory).orderBooks(_perpetualId) == msg.sender,
            "only order-bk"
        );
        _executeCancelOrder(_perpetualId, _digest);
    }

    /**
     * Determine if an order has already been executed.
     * @param digest Order id/digest.
     * @return True if the order was executed.
     */
    function isOrderExecuted(bytes32 digest) external view override returns (bool) {
        return executedOrCancelledOrders[digest] & MASK_ORDER_EXECUTED != 0;
    }

    /**
     * Determine if an order has been cancelled.
     * @param digest Order id/digest.
     * @return True if the order was cancelled.
     */
    function isOrderCanceled(bytes32 digest) external view override returns (bool) {
        return executedOrCancelledOrders[digest] & MASK_ORDER_CANCELLED != 0;
    }
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[0] = this.tradeViaOrderBook.selector;
        functionList[1] = this.executeCancelOrder.selector;
        functionList[2] = this.isOrderExecuted.selector;
        functionList[3] = this.isOrderCanceled.selector;
