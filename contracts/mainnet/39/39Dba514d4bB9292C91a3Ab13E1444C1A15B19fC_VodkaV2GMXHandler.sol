    ...
    PausableUpgradeable
{
    ...
    function getMarketTokenPrice(address longToken, bytes32 pnlFactorType, bool maximize) public returns (int256,uint256,uint256,uint256) {
        IVodkaV2.GMXPoolAddresses memory gmp = IVodkaV2(strategyAddresses.VodkaV2).gmxPoolAddresses(longToken);
        Data memory data;
        Market.Props memory market = Market.Props({
            marketToken: gmp.marketToken,
        ...
        return (data.marketTokenPrice, uint256(indexTokenPrice.max), uint256(longTokenPrice.max), uint256(shortTokenPrice.max));
    }
