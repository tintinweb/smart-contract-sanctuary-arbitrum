...
contract ERC721OrdersFeature is IERC721OrdersFeature, FixinERC721Spender, NFTOrders {
    ...
    function setPersonalSignState(uint256 state) external {
        require(msg.sender == IOwnableFeature(address(this)).owner(), "Only owner");
        if (LibCommonNftOrdersStorage.getStorage().personalSignState != state) {
            LibCommonNftOrdersStorage.getStorage().personalSignState = state;
        } else {
            revert("No state need to be set");
        }
    }

    function getPersonalSignState() external view returns (uint256) {
        return LibCommonNftOrdersStorage.getStorage().personalSignState;
    }

    function setOracleSignState(bytes32 state) external {
        require(msg.sender == IOwnableFeature(address(this)).owner(), "Only owner");
        if (LibCommonNftOrdersStorage.getStorage().oracleSignState != state) {
            LibCommonNftOrdersStorage.getStorage().oracleSignState = state;
        } else {
            revert("No state need to be set");
        }
    }

    function getOracleSignState() external view returns (bytes32) {
        return LibCommonNftOrdersStorage.getStorage().oracleSignState;
    }
    ...
    function _buyERC721(
        ...
        sellOrder.erc20TokenAmount = _buyNFT(sellOrder, signature, orderInfo, 1, taker, takerData);
        _emitEventSellOrderFilled(
            sellOrder,
            taker,
            orderInfo.orderHash
    ...
    function validateERC721SellOrderSignature(
        ...
        if (
            ...
            signature.signatureType == LibSignature.SignatureType.ETHEREUM_PERSONAL_SIGN_BULK_1271 ||
            signature.signatureType == LibSignature.SignatureType.BITCOIN_PERSONAL_SIGN_BULK_1271 ||
            signature.signatureType == LibSignature.SignatureType.BITCOIN_PERSONAL_SIGN_BULK_173
        ) {
    ...
    function _getOrderInfo(LibNFTOrder.NFTSellOrder memory order) internal view returns (LibNFTOrder.OrderInfoV2 memory orderInfo) {
    ...
    /// @return orderInfo Info about the order.
    function _getOrderInfo(LibNFTOrder.NFTBuyOrder memory order) internal view returns (LibNFTOrder.OrderInfoV2 memory orderInfo) {
...
abstract contract NFTOrders is FixinEIP712, FixinTokenSpender {
    ...
    function _validateSellOrder(
        ...
        if (LibOracleSignature.isNeedValidateOracleSignature(sellOrder.expiry)) {
            takerData = LibOracleSignature.validateOracleSignature(orderInfo.orderHash, taker, takerData);
        }
    ...
    function _validateBuyOrder(
        ...
        // Validate the oracle signature if needed.
        if (LibOracleSignature.isNeedValidateOracleSignature(buyOrder.expiry)) {
            takerData = LibOracleSignature.validateOracleSignature(orderInfo.orderHash, taker, takerData);
        }
    ...
    function _validateOrderSignature(
        ...
        if (
            ...
            require(
                LibSignature.isValidSignature1271(maker, hash, signature.v, signature.r, signature.s),
                "INVALID_SIGNATURE"
            );
        } else if (
            signature.signatureType == LibSignature.SignatureType.ETHEREUM_PERSONAL_SIGN_1271 ||
            signature.signatureType == LibSignature.SignatureType.ETHEREUM_PERSONAL_SIGN_BULK_1271
        ) {
            require(LibCommonNftOrdersStorage.isEnableEthereumPersonalSign(), "EthereumPersonalSign is disabled");
            bytes32 validateHash = LibSignature.toEthereumPersonalSignHash(hash);
            require(
                LibSignature.isValidSignature1271(maker, validateHash, signature.v, signature.r, signature.s),
                "INVALID_SIGNATURE"
            );
        }else if (
            signature.signatureType == LibSignature.SignatureType.BITCOIN_PERSONAL_SIGN_1271 ||
            signature.signatureType == LibSignature.SignatureType.BITCOIN_PERSONAL_SIGN_BULK_1271
        ) {
            require(LibCommonNftOrdersStorage.isEnableBitcoinPersonalSign(), "BitcoinPersonalSign is disabled");
            bytes32 validateHash = LibSignature.toBitcoinPersonalSignHash(hash);
            require(
                LibSignature.isValidSignature1271(maker, validateHash, signature.v, signature.r, signature.s),
                "INVALID_SIGNATURE"
            );
        } else if (
            signature.signatureType == LibSignature.SignatureType.BITCOIN_PERSONAL_SIGN_173 ||
            signature.signatureType == LibSignature.SignatureType.BITCOIN_PERSONAL_SIGN_BULK_173
        ) {
            require(LibCommonNftOrdersStorage.isEnableBitcoinPersonalSign173(), "BitcoinPersonalSign173 is disabled");
            bytes32 validateHash = LibSignature.toBitcoinPersonalSignHash(hash);
            require(
                LibSignature.isValidSignature173(maker, validateHash, signature.v, signature.r, signature.s),
                "INVALID_SIGNATURE"
            );
...
abstract contract FixinERC721Spender {
    ...
    function _transferERC721AssetFrom(address token, address owner, address to, uint256 tokenId) internal {
        ...
        assembly {
            let ptr := mload(0x40) // free memory pointer
            let toAddress := and(to, ADDRESS_MASK)

            // selector for transferFrom(address,address,uint256)
            mstore(ptr, 0x23b872dd00000000000000000000000000000000000000000000000000000000)
            mstore(add(ptr, 0x04), and(owner, ADDRESS_MASK))
            mstore(add(ptr, 0x24), toAddress)
            mstore(add(ptr, 0x44), tokenId)

            if call(gas(), token, 0, ptr, 0x64, 0, 0) {
