...
contract TokenManager {
  ...
  function averagePrice(address user, address token) external view returns (uint256) {
    return accounts[user][token].avgPrice;
  }

  function amount(address user, address token) external view returns (uint256) {
    return accounts[user][token].amount;
  }
