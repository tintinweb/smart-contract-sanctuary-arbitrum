...
contract MultiLocks is Wallet, TaxCreationBlock {
    ...
    function deployContractMLPool(address aToken, uint256 _typeToken) public {
        require(mMLPools[aToken] == 0, "exists");
        require(_typeToken == 0 || _typeToken == 1, "_typeToken"); // 0 - simpleToken, 1 - LP Token
...
contract MultiLockPool {
    ...
    function addMultiLockData(
        ...
        uint256 bBefore = IERC20(aMLToken).balanceOf(address(this));
        uint256 SumTokens = 0;
        ...
        require(bBefore.add(SumTokens) >= IERC20(aMLToken).balanceOf(address(this)), "Balance");
    }
