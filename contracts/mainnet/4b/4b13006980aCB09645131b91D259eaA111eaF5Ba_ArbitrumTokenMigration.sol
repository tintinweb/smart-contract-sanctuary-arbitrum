...
/**
 * @title A TCR token to MYC token migration contract for Arbitrum
 * @author raymogg
 ...
 * @dev The major difference between the Arbitrum and Mainnet contracts is that the Arbitrum contract does not
 * directly mint tokens, but instead transfers pre minted tokens to migrators.
 */
contract ArbitrumTokenMigration is AccessControl {
    ...
    function _migrate(
        ...
        // will revert if there is insufficient balance
        IERC20(myc).transfer(to, amount);
