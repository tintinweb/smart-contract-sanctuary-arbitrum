...
contract SettingsManager is ISettingsManager, Initializable, ReentrancyGuardUpgradeable, Constants {
    ...
    mapping(address => bool) public override isWhitelistedFromCooldown; // whitelist from unstake cooldown
    ...
    event SetWithdrawalExecutionDelay(uint256 withdrawalExecutionDelay);
    ...
    uint256 public override withdrawalExecutionDelay;

    EnumerableSetUpgradeable.AddressSet private withdrawWhitelist; // wallet addresses who can bypass withdrawal cooldown period
    ...
    function setWithdrawalExecutionDelay(uint256 _withdrawalExecutionDelay) external onlyOperator(3) {
        require(_withdrawalExecutionDelay <= 3 days, "invalid withdrawalExecutionDelay");
        withdrawalExecutionDelay = _withdrawalExecutionDelay;
        emit SetWithdrawalExecutionDelay(_withdrawalExecutionDelay);
    }

    /* ========== VAULT WITHDRAW WHITELIST ========== */
    function addAccountsToWithdrawWhitelist(address[] memory _accounts) external onlyOperator(3) {
        for (uint256 i = 0; i < _accounts.length; i++) {
            EnumerableSetUpgradeable.add(withdrawWhitelist, _accounts[i]);
        }
    }

    function removeAccountsFromWithdrawWhitelist(address[] memory _accounts) external onlyOperator(3) {
        for (uint256 i = 0; i < _accounts.length; i++) {
            EnumerableSetUpgradeable.remove(withdrawWhitelist, _accounts[i]);
        }
    }

    function isWhitelistedFromWithdrawCooldown(address _account) public view override returns (bool) {
        return EnumerableSetUpgradeable.contains(withdrawWhitelist, _account);
    }
