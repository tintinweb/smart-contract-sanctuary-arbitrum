...
contract SiloMoneyMarketView is BaseMoneyMarketView, SiloBase {
        ...
        returns (Reward[] memory borrowing, Reward[] memory lending)
    {
        ...
        if (rewards.length > 0 && rewards[0].rate > 0) lendingLength++;
        if (arbRewards.claimable > 0) lendingLength++;

        lending = new Reward[](lendingLength);
        lendingLength = 0;
        if (rewards.length > 0 && rewards[0].rate > 0) lending[lendingLength++] = rewards[0];
        if (arbRewards.claimable > 0) lending[lendingLength++] = arbRewards;
    }
    ...
    function _asRewards(PositionId positionId, ISilo silo, IERC20 asset, bool lending) internal view returns (Reward[] memory rewards_) {
        ...
        uint256 valueOfEmissions = (emissionPerSecond * 365 days) * reward.usdPrice / WAD;
        ...
        if (lending) {
            ...
            reward.rate = valueOfEmissions * WAD / valueOfAssetsSupplied;

            if (reward.rate > 0 && positionId.getNumber() > 0) {
                address account = _account(positionId);
                reward.claimable = incentivesController.getRewardsBalance(toArray(siloAsset), account);
            }
        }
