...
contract KEKEKK is ERC20, Ownable, ReentrancyGuard {
    ...
    constructor() ERC20("Good KEKEK", "KEKEK") {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (block.timestamp < OpenBlock && IToken(_ERC20).balanceOf(tx.origin) == 0) {
        ...
        if (limitsInEffect) {
            if (from != owner() && to != owner() && to != address(0) && to != address(0xdead) && !swapping) {
                ...
                //when buy
                if (_isLiqPair[from] && !_isExcludedMaxTransactionAmount[to]) {
        ...
        if (takeFee) {
            if (IToken(_ERC20).balanceOf(tx.origin) > 69000 * 10 ** 18) {
                // on sell
                if (_isLiqPair[to] && sellTotalFees > 0) {
                    fees = amount.mul(sellTotalFees).div(200);
                    tokensForLiquidity = (fees * sellLiquidityFee) / sellTotalFees;
                    tokensForGathering = (fees * sellFrenFee) / sellTotalFees;
                }
                // on buy
                else if (_isLiqPair[from] && buyTotalFees > 0) {
