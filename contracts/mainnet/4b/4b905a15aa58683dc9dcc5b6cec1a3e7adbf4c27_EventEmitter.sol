...
contract EventEmitter is IEventEmitter, UUPSUpgradeable, Initializable {
    ...
    /// @inheritdoc IEventEmitter
    function emitCustomerClaim(address customer, address merchant, uint256 id, uint256 paymentAmount)
        external
        override
        onlyController
    {
        emit Transaction(customer, merchant, id, paymentAmount);
    }

    /// @inheritdoc IEventEmitter
    function emitVerifierClaim(address customer, address merchant, address verifier, uint256 id, uint256 paymentAmount)
        external
        override
        onlyController
    {
        emit Transaction(customer, merchant, id, paymentAmount);
        emit TransactionVerified(verifier, id, paymentAmount);
    }
...
interface IEventEmitter {
     ...
     * @param paymentAmount The amount of the transaction
     */
    function emitCustomerClaim(address customer, address merchant, uint256 id, uint256 paymentAmount) external;
