...
contract SmartRouterAnalyst {
    ...
    function parseRevertReason(bytes memory reason) private pure returns (TokenResponse memory) {
        if (reason.length != 64) {
            assembly {
                revert(add(32, reason), mload(reason))
            }
        } else {
            return abi.decode(reason, (TokenResponse));
        }
    }
    ...
    function smartRoute(address[] calldata pairs, uint256 amountIn, address initialToken) 
        external
        returns (TokenResponse memory result)
    {
        ...
        try IUniswapV2Pair(pairs[0]).swap(amount0Out, amount1Out, address(this), abi.encode(pairs, amountIn, initialToken)) {}
    ...
    function _performSwaps(address[] memory pairs, address initialToken, uint256 amountIn, address sender) internal {
        ...
        ERC20(currentToken).approve(pairs[0], currentAmount);
        ...
        for (uint256 i = 0; i < pairs.length; i++) {
            ...
            ERC20(currentToken).safeTransfer(pairs[i], ERC20(currentToken).balanceOf(sender));
        ...
        bytes memory finalTokenInfo = abi.encode(TokenResponse({finalToken: currentToken, finalAmount: currentAmount}));

        // revert with the abi encoded fees
        assembly {
            revert(add(32, finalTokenInfo), mload(finalTokenInfo))
        }
