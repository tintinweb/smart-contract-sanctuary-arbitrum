...
contract MultiChainNFT is ONFT721Core, ERC721, IERC2981, IMasterContract {
    ...
    function init(bytes calldata data) public payable override {
        (string memory _name, string memory _symbol, address owner_, uint16[] memory chains, bytes[] memory remotes) = abi.decode(data, (string, string, address, uint16[], bytes[]));
...
import "../NonBlockingLZMultichain.sol";

abstract contract ONFT721Core is NonBlockingLZMultichain, IERC165, IONFT721Core {
    ...
    constructor(uint256 _minGasToTransferAndStore, address _lzEndpoint) NonBlockingLZMultichain(_lzEndpoint) {
        require(_minGasToTransferAndStore > 0, "minGasToTransferAndStore must be > 0");
        minGasToTransferAndStore = _minGasToTransferAndStore;
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165) returns (bool) {
        return interfaceId == type(IONFT721Core).interfaceId || interfaceId == type(IERC165).interfaceId;
    }
