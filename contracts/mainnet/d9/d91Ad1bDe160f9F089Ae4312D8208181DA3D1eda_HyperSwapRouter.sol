...
import '../vvs-swap-core/contracts/interfaces/IHyperSwapFactory.sol';
...
contract HyperSwapRouter is IHyperSwapRouter02 {
    ...
    function addLiquidityETH(
        ...
        address pair = HyperSwapLibrary.pairFor(factory, token, WETH);
    ...
    {
        require(path[0] == WETH, 'HyperSwapRouter: INVALID_PATH');
        amounts = HyperSwapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'HyperSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(HyperSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'HyperSwapRouter: INVALID_PATH');
        amounts = HyperSwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'HyperSwapRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, HyperSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'HyperSwapRouter: INVALID_PATH');
        amounts = HyperSwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'HyperSwapRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(HyperSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(HyperSwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'HyperSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
...
import '../../vvs-swap-core/contracts/interfaces/IHyperSwapPair.sol';
...
library HyperSwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'74741af54dbb4c94c5625c3988723cc27228ad2fe81ce9b77e5333435dea825a' // init code hash
            ))));
    }
