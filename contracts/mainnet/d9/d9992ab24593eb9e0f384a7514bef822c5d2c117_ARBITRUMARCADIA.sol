...
contract ARBITRUMARCADIA is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "ARCADIA";
    string private _symbol = "ARCADIA";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x7e05a8A3Aab6526b6A185ECF684Ac477C04bB6a9); // Marketing Address
    address payable public teamWalletAddress = payable(0xF3201588D8b2A915F30D35e73059a4B37296b3d7); // Team Address
    ...
    constructor () {
        ...
        SushiswapPair = ISushiswapV2Factory(_SushiswapV2Router.factory())
            .createPair(address(this), _SushiswapV2Router.WETH());

        SushiswapV2Router = _SushiswapV2Router;
        _allowances[address(this)][address(SushiswapV2Router)] = _totalSupply;
        ...
        isWalletLimitExempt[address(SushiswapPair)] = true;
    ...
    function changeRouterVersion(address newRouterAddress) public onlyOwner returns(address newPairAddress) {
        ...
        ISushiswapV2Router02 _SushiswapV2Router = ISushiswapV2Router02(newRouterAddress); 

        newPairAddress = ISushiswapV2Factory(_SushiswapV2Router.factory()).getPair(address(this), _SushiswapV2Router.WETH());

        if(newPairAddress == address(0)) //Create If Doesnt exist
        {
            newPairAddress = ISushiswapV2Factory(_SushiswapV2Router.factory())
                .createPair(address(this), _SushiswapV2Router.WETH());
        }

        SushiswapPair = newPairAddress; //Set new pair address
        SushiswapV2Router = _SushiswapV2Router; //Set new router address

        isWalletLimitExempt[address(SushiswapPair)] = true;
        isMarketPair[address(SushiswapPair)] = true;
    }

     //to recieve ETH from SushiswapV2Router when swaping
