...
contract CreditRewardTracker is Initializable, IRewardTracker {
    ...
    uint256 private constant MAX_MANAGER_SIZE = 20;
    ...
    mapping(address => bool) private canExecute;

    error NotAuthorized();
    event Executed(address _sender, address _target, uint256 _claimed, uint256 _timestamp);
    event ToggleCanExecute(address _vault, bool _state);
    ...
    function toggleCanExecute(address _target) external onlyOwner {
        require(_target != address(0), "CreditRewardTracker: _target cannot be 0x0");

        canExecute[_target] = !canExecute[_target];

        emit ToggleCanExecute(_target, canExecute[_target]);
    }
