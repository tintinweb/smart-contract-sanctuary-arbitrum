...
interface IPioneerNFT is IERC721EnumerableUpgradeable {    
...
contract PioneerNFT3 is ERC721EnumerableUpgradeable, AccessControlUpgradeable, IPioneerNFT {
    ...
    // Mapping for locking transfer NFT
    mapping(uint256 => bool) private _mapLockedTokens;
    mapping(uint256 => bool) public mapLockedTokens;
    ...
    event PricingTierSetOrAdded(uint256 index, uint256 price, uint256 quantity, bool isNative);
    ...
    function supportsInterface(
        ...
        public
        view
        override(ERC721EnumerableUpgradeable, AccessControlUpgradeable, IERC165Upgradeable)
        returns (bool)
    {
        return
            super.supportsInterface(interfaceId) ||
    ...
    function setOrAddPricingTier(
        ...
        if (_index < pricingTiers.length) {
            require(_quantity >= pricingTiers[_index].soldItem, "New quantity must be greater than or equal sold items");
            // Subtract the quantity of the old tier from maxSupply
            maxSupply -= pricingTiers[_index].quantity;
            pricingTiers[_index] = Tier(_price, _value, _quantity, pricingTiers[_index].soldItem, _startTime, _endTime);
        } else if (_index == pricingTiers.length) {
            pricingTiers.push(Tier(_price, _value, _quantity, 0, _startTime, _endTime));
        } else {
        ...
        maxSupply += _quantity;
        emit PricingTierSetOrAdded(_index, _price, _quantity, false);
    }
    ...
    function getOnSaleTier() public view returns (Tier memory, uint256) {
        uint256 totalTier = pricingTiers.length;
        for (uint256 i = 0; i < totalTier; i++) {
            Tier memory tier = pricingTiers[i];
            if (tier.startTime <= block.timestamp && tier.endTime >= block.timestamp) {
                return (tier, i);
            }
        }
        revert("No tier is currently on sale");
    }
    ...
    function mint() public payable {
        ...
        (Tier memory currentTier, uint256 index) = getOnSaleTier();

        // check available token
        require(currentTier.quantity > currentTier.soldItem, "Sold out");

        // check time range
        require((block.timestamp >= currentTier.startTime) && (block.timestamp <= currentTier.endTime), "Invalid time");
        ...
        if (currentTier.value > 0) {
            uint256 remainder = msg.value - currentTier.value;
    ...
    // -- Whitelist + Redeem
    // ----------------------------
    function _updateWhitelistAmounts(
     ...
     */
    function updateWhitelistAmounts(
    ...
    function redeemFromWhitelist() external {
        uint256 startTime = 1717718400; // Friday, 7 June 2024 07:00:00 GMT+07:00
        ...
        require(_tokenIds.current() + toMint <= maxSupply, "Exceeds maxSupply");
    ...
    function lockTransfer(uint256 tokenId) external onlyRole(LOCK_ROLE) {
        mapLockedTokens[tokenId] = true;
    }

    function unlockTransfer(uint256 tokenId) external onlyRole(LOCK_ROLE) {
        mapLockedTokens[tokenId] = false;
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override(ERC721Upgradeable, IERC721Upgradeable) {
        // check for locking 
        require(mapLockedTokens[tokenId] != true, "The token is not allowed to be transferred");
