...
import {ClaimEvent, rs} from "core/RegistryState.sol";
import {Constants} from "core/Constants.sol";
import {IViewFacet} from "core/interfaces/IViewFacet.sol";
import {ERC721BaseInternal} from "lib/solidstate/contracts/token/ERC721/base/ERC721BaseInternal.sol";
import {ERC721EnumerableInternal} from "lib/solidstate/contracts/token/ERC721/enumerable/ERC721EnumerableInternal.sol";

contract ViewFacet is ERC721BaseInternal, ERC721EnumerableInternal, IViewFacet {
    function getAccountInfo(
        address _account
    ) external view returns (AccountInfo memory account) {
        account.linkedId = getLinkedId(_account);
        account.linked = account.linkedId != 0;
        account.points = getKredits(account.linkedId);
        account.currentClaimId = rs().airdropsIds;
        account.claimedCurrent = getClaimed(_account, account.currentClaimId);
        account.hasKreskian = Constants.BURN_NFT.balanceOf(_account, 0) > 0;

        uint256 bal = _balanceOf(_account);
        account.walletProfileId = bal > 0
            ? _tokenOfOwnerByIndex(_account, 0)
            : 0;

        account.lockedQFKs = new uint256[](8);

        if (account.linkedId == 0) return account;

        for (uint256 i; i < 8; i++) {
            account.lockedQFKs[i] = getLocked(
                account.linkedId,
    ...
    function getBurnNftAddress() external pure returns (address) {
        return address(Constants.BURN_NFT);
    }

    function getConfig(
        uint256 airdropId
    ) external view returns (ClaimEvent memory) {
        return rs().claimEvents[airdropId];
    }
    ...
    function getClaimed(
        ...
        return rs().claimed[user][airdropId];
    }

    function getIsValid(address nft) external view returns (bool) {
        return rs().isValid[nft];
    }

    function getKredits(uint256 tokenId) public view returns (uint256) {
        return rs().kredits[tokenId];
    }

    function getLinkedId(address user) public view returns (uint256) {
        return rs().linkedId[user];
    }
