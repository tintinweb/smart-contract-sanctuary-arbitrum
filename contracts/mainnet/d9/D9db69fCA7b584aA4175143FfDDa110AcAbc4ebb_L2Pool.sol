...
/**
 * @title IRebasingMToken
 * @author maneki.finance
 * @notice Defines the basic interface for the a rebasing token on Blast (usdb, weth)
 */

interface IRebasingMToken is IMToken {
    function rebasingDistributor() external view returns (address);

    function initializeRebasing(address _rebasingDistributor) external;

    function setRebasingDistributor(address _rebasingDistributor) external;
}
...
library FlashLoanLogic {
    ...
    function executeFlashLoan(
        ...
        for (vars.i = 0; vars.i < params.assets.length; vars.i++) {
            ...
            IMToken(reservesData[params.assets[vars.i]].mTokenAddress)
    ...
    function executeFlashLoanSimple(
        ...
        IMToken(reserve.mTokenAddress).transferUnderlyingTo(
            params.receiverAddress,
            params.amount
        );
...
library PoolLogic {
    ...
    function executeInitReserve(
        ...
        require(_isContract(params.asset), Errors.NOT_CONTRACT);
    ...
    function executeMintToTreasury(
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            if (accruedToTreasury != 0) {
                ...
                IMToken(reserve.mTokenAddress).mintToTreasury(
                    amountToMint,
                    normalizedIncome
                );
...
import {IRebasingMToken} from "../../interfaces/IRebasingMToken.sol";
...
contract Pool is VersionedInitializable, PoolStorage, IPool {
    ...
    address public PUBLIC_LIQUIDATOR;
    ...
    function finalizeTransfer(
        ...
        require(
            msg.sender == _reserves[asset].mTokenAddress,
    ...
    function setPublicLiquidator(
        ...
        PUBLIC_LIQUIDATOR = _publicLiquidator;
    }
    ...
    function liquidationCall(
        ...
        if (PUBLIC_LIQUIDATOR != address(0)) {
            require(
                msg.sender == PUBLIC_LIQUIDATOR,
                Errors.ONLY_PUBLIC_LIQUIDATOR_ALLOWED
            );
        }
        ...
        autoMintToTreasury(collateralAsset);
    ...
    function isFlashloanPrivilege(
        ...
        return _isFlashloanPrivilege[_flashloanCaller];
    }
    ...
    function setFlashloanPrivilege(
        ...
        _isFlashloanPrivilege[_callerAddress] = _isPrivileged;
    }
    ...
    function flashLoan(
        ...
        DataTypes.FlashloanParams memory flashParams = DataTypes
            .FlashloanParams({
                ...
                params: params,
                referralCode: referralCode,
                flashLoanPremiumToProtocol: _isFlashloanPrivilege[msg.sender]
                    ? 0
                    : _flashLoanPremiumToProtocol,
                flashLoanPremiumTotal: _isFlashloanPrivilege[msg.sender]
                    ? 0
    ...
    function setMTokenYieldDistributor(
        address _mToken,
        address _rebasingDistributor
    ) external virtual onlyPoolConfigurator {
        IRebasingMToken mToken = IRebasingMToken(_mToken);
        if (mToken.rebasingDistributor() == address(0)) {
            mToken.initializeRebasing(_rebasingDistributor);
        } else {
            mToken.setRebasingDistributor(_rebasingDistributor);
        }
    }
    ...
    /****************************************/
    /**
     * @dev     Function hook, called when user interact with the protocol
     * @param   _asset  The asset to mint the earned portion to treasury
     **/
...
contract PoolStorage {
    ...
    // Map of users address and their eMode category (userAddress => eModeCategoryId)
    mapping(address => bool) internal _isFlashloanPrivilege;
