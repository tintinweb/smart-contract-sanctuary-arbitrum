...
contract StgComposed {
    ...
    function sgReceive(
        ...
        (address toAddr, uint256 deadline) = abi.decode(
            payload,
            (address, uint256)
        );
        ...
        vault.mint(_token, amountLD, 0, deadline);

        usds.transfer(
            address(toAddr),
            usds.balanceOf(address(this)) - usdsBefore
        );

        emit USDSMinted(toAddr, usds.balanceOf(address(this)) - usdsBefore);
    }
}
