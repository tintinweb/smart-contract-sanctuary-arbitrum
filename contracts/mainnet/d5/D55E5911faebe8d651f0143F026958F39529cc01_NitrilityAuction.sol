...
contract NitrilityAuction is Ownable(msg.sender), ReentrancyGuard {
    ...
    // license id => offer id
    mapping(uint256 => EnumerableSet.UintSet) offerIdsForLicense;
    mapping(address => EnumerableSet.UintSet) offerIdsForBuyer;
    ...
    function makeOffer(
        ...
        require(
            ...
            "offer price should be same with your funds"
        );
        // make the license ids for buyer
        uint256 offerCount = offerIdsForBuyer[buyerAddr].length();
        bool bFound = false;
        for (uint256 i = 0; i < offerCount; i++) {
            uint256 offerId = offerIdsForBuyer[buyerAddr].at(i);
            if (
        ...
        offerIdsForLicense[listedId].add(currentOfferId);
        //  make the offer ids for buyer
        offerIdsForBuyer[buyerAddr].add(currentOfferId);
    ...
    function editOffer(
        ...
        idToOffer[offerId].offerTime = offerTime;
    ...
    function fetchOffersOfSeller(
        ...
        uint256 itemCount = offerIdsForLicense[_listedId].length();
        Offer[] memory offers = new Offer[](itemCount);
        uint256 currentIndex = 0;
        for (uint256 i = 0; i < itemCount; i++) {
            uint256 offerId = offerIdsForLicense[_listedId].at(i);
            Offer storage currentItem = idToOffer[offerId];
    ...
    function fetchOfferOfBuyer(
        ...
        for (uint256 i = 0; i < bidCount; i++) {
            uint256 offerId = offerIdsForBuyer[_buyer].at(i);
            Offer storage currentItem = idToOffer[offerId];
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "License(string artistId,string tokenURI,TemplateType creator,TemplateType advertisement,TemplateType tvSeries,TemplateType movie,TemplateType videoGame,TemplateType aiTraining)TemplateType(uint256 fPrice,uint256 sPrice,uint256 tPrice,uint256 listingFormatValue,bool infiniteSupply,bool infiniteListingDuration,bool infiniteExclusiveDuration,uint256 accessLevel,uint256 listingStartTime,uint256 listingEndTime,uint256 exclusiveEndTime,bool listed)"
                        ),
