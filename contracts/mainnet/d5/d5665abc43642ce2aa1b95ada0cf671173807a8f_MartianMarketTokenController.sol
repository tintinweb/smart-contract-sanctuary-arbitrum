...
/*
    ...
    - Add DefaultToken override for a project (Ownable only)
...
contract MartianMarketTokenController is Ownable {
    ...
    event GlobalTokenSet(address indexed contract_, address indexed operator_,
        address token_);
    
    event SetOverride(address indexed operator_, bool bool_);
    event SetGlobalOverride(address indexed operator_, bool bool_);
    ...
    IMartianMarket public MM = 
        IMartianMarket(0x1b0EfFE43fe9b2A9B8Ada6060C1431cC7E5d77a3);
    ...
    // Ownable Global Override Permissions
    bool public globalOverrideEnabled; // default as false

    function setGlobalOverrideEnabled(bool bool_) external onlyOwner {

        globalOverrideEnabled = bool_;

        emit SetGlobalOverride(msg.sender, bool_);
    }
    modifier onlyGlobalOverrideenabled {
        require(globalOverrideEnabled,
            "Global Token Overrides are not enabled!");
        _;
    }

    // Ownable Set Global Override
    mapping(address => address) public contractToGlobalToken;

    function setContractToGlobalToken(address contract_, address token_) external
    onlyOwner {
        
        contractToGlobalToken[contract_] = token_;

        emit GlobalTokenSet(contract_, msg.sender, token_);
    }
    ...
    returns (string memory) {
        ...
        address _tokenOverride = contractToIndexToToken[contract_][index_];

        if (_tokenOverride != address(0)) {
            // Token has been overridden
            return contractToTokenInfo[_tokenOverride].tokenName;
        }
        else {
            // Token has not been overridden, find out globalToken override
            address _globalToken = contractToGlobalToken[contract_];
            if (_globalToken != address(0)) {
                // Global override exists. Return override info
                return contractToTokenInfo[_globalToken].tokenName;
            } 
            else {
                // Global override doesn't exist. Return normal info.
                return MM.getDefaultTokenNameOfContract(contract_);
            }
        }
    }
    ...
    returns (string memory) {
        ...
        if (_tokenOverride != address(0)) {
            // Token has been overridden
            return contractToTokenInfo[_tokenOverride].tokenImageUri;
        }
        else {
            // Token has not been overridden, find out globalToken override
            address _globalToken = contractToGlobalToken[contract_];
            if (_globalToken != address(0)) {
                // Global override exists. Return override info
                return contractToTokenInfo[_globalToken].tokenImageUri;
            } 
            else {
                // Global override doesn't exist. Return normal info.
                return MM.getDefaultTokenImageOfContract(contract_);
            }
        }
    }
    ...
    returns (address) {
        ...
        if (_tokenOverride != address(0)) {
            // Token has been overridden
            return contractToTokenInfo[_tokenOverride].tokenAddress;
        }
        else {
            // Token has not been overridden, find out globalToken override
            address _globalToken = contractToGlobalToken[contract_];
            if (_globalToken != address(0)) {
                // Global override exists. Return override info
                return contractToTokenInfo[_globalToken].tokenAddress;
            } 
            else {
                // Global override doesn't exist. Return normal info.
                return MM.getDefaultTokenOfContract(contract_);
            }
        }
    }
}