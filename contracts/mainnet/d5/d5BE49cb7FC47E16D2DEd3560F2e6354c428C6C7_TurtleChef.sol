...
import "./ITurtleswapPair.sol";

//new distributor to handle multiple rewardooors

contract TurtleChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Turtl to distribute per block.
        uint256 lastRewardTimestamp; // Last block number that Turtl distribution occurs.
        uint256 accTurtlPerShare; // Accumulated Turtl per share, times 1e18. See below.
    ...
    IBoringERC20 public turtl;

    // Turtl tokens created per second
    uint256 public turtlPerSec;
    ...
    // The timestamp when Turtl mining starts.
    uint256 public startTimestamp;

    // Total locked up rewards
    uint256 public totalLockedUpRewards;

    // Total Turtl in Turtl Pools (can be multiple pools)
    uint256 public totalTurtlInPools;

    // xTURTL address.
    address public xTURTLAddress;
    ...
    event SetxTURTLPercent(uint256 oldPercent, uint256 newPercent);

    constructor(
        IBoringERC20 _turtl,
        uint256 _turtlPerSec,
        address _xTURTLAddress,
        uint256 _xTURTLPercent,
        address _feeAddress
    ) {
        require(
            _xTURTLPercent <= 1000,
            "constructor: invalid xTURTL percent value"
        );
        ...
        xTURTLAddress = _xTURTLAddress;
        xTURTLPercent = _xTURTLPercent;
    ...
    // Update the given pool's Turtl allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
        ...
        )
    {
        ...
        uint256 accTurtlPerShare = pool.accTurtlPerShare;
        uint256 lpSupply = pool.totalLp;

        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            uint256 multiplier = block.timestamp - pool.lastRewardTimestamp;
            uint256 total = 1000;
            uint256 lpPercent = total - xTURTLPercent;

            uint256 turtlReward = (multiplier *
                turtlPerSec *
                pool.allocPoint *
                lpPercent) /
                totalAllocPoint /
            ...
            accTurtlPerShare += (
                ((turtlReward * ACC_TOKEN_PRECISION) / lpSupply)
            );
        }

        uint256 pendingTurtl = (((user.amount * accTurtlPerShare) /
        ...
        addresses[0] = address(turtl);
        symbols[0] = IBoringERC20(turtl).safeSymbol();
        decimals[0] = IBoringERC20(turtl).safeDecimals();
        amounts[0] = pendingTurtl;
        ...
        )
    {
        ...
        rewardsPerSec[0] =
            (pool.allocPoint * turtlPerSec * lpPercent) /
            totalAllocPoint /
            total;
    ...
    // View function to see if user can harvest Turtl.
    ...
    function _updatePool(uint256 _pid) internal validatePoolByPid(_pid) {
        ...
        uint256 turtlReward = ((multiplier * turtlPerSec) * pool.allocPoint) /
            totalAllocPoint;
        ...
        if (xTURTLPercent > 0) {
            turtl.mint(xTURTLAddress, (turtlReward * xTURTLPercent) / total);
        }

        turtl.mint(address(this), (turtlReward * lpPercent) / total);

        pool.accTurtlPerShare +=
            (turtlReward * ACC_TOKEN_PRECISION * lpPercent) /
            pool.totalLp /
            total;
    ...
    function depositWithPermit(
        ...
        PoolInfo storage pool = poolInfo[pid];
        ITurtleswapPair pair = ITurtleswapPair(address(pool.lpToken));
        pair.permit(msg.sender, address(this), amount, deadline, v, r, s);
        _deposit(pid, amount);
    }

    // Deposit tokens for Turtl allocation.
        ...
        validatePoolByPid(_pid)
    {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(turtl)) {
                totalTurtlInPools += _amount;
            }
        }
        user.rewardDebt =
            (user.amount * pool.accTurtlPerShare) /
            ACC_TOKEN_PRECISION;
        ...
        for (
            ...
            pool.rewarders[rewarderId].onTurtlReward(
                _pid,
                msg.sender,
                user.amount
            );
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        for (
            ...
            pool.rewarders[rewarderId].onTurtlReward(_pid, msg.sender, 0);
        }

        if (address(pool.lpToken) == address(turtl)) {
            totalTurtlInPools -= amount;
        }
    ...
    // Pay or lockup pending Turtl.
    function payOrLockupPendingTurtl(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = ((user.amount * pool.accTurtlPerShare) /
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                user.rewardLockedUp = 0;
                user.nextHarvestUntil = block.timestamp + pool.harvestInterval;
                safeTurtlTransfer(msg.sender, pendingRewards);
            }
        } else if (pending > 0) {
    ...
    function safeTurtlTransfer(address _to, uint256 _amount) internal {
        if (turtl.balanceOf(address(this)) > totalTurtlInPools) {
            uint256 turtlBal = turtl.balanceOf(address(this)) -
                totalTurtlInPools;
            if (_amount >= turtlBal) {
                turtl.safeTransfer(_to, turtlBal);
            } else if (_amount > 0) {
                turtl.safeTransfer(_to, _amount);
            }
        }
    }
    ...
    function updateEmissionRate(uint256 _turtlPerSec) public onlyOwner {
        _massUpdatePools();

        emit EmissionRateUpdated(msg.sender, turtlPerSec, _turtlPerSec);
    ...
    // Update xTURTL address
    function setxTURTLAddress(address _xTURTLAddress) public onlyOwner {
        require(_xTURTLAddress != address(0), "invalid new xTURTL address");
        xTURTLAddress = _xTURTLAddress;
        emit SetxTURTLAddress(msg.sender, _xTURTLAddress);
    }

    function setxTURTLPercent(uint256 _newxTURTLPercent) public onlyOwner {
        require(_newxTURTLPercent <= 1000, "invalid percent value");
        emit SetxTURTLPercent(xTURTLPercent, _newxTURTLPercent);
        xTURTLPercent = _newxTURTLPercent;
    }
    ...
    function getTurtlPerSec() public view returns (uint256) {
        return turtlPerSec;
    }
}