...
interface IPausable {
  ...
  /** 
   * @notice Pauses operations on the contract.
   */
  function pause() external;

  /** 
   * @notice Resumes operations on the contract if it has been paused.
   */
  function resume() external;
...
library InterestMath {
  ...
  uint128 private constant HALF_WAD = 0.5e18;

  uint128 private constant SECONDS_PER_YEAR_IN_WAD = 31557600000000000000000000; // 365.25 * 86400

  function wadMul(uint256 a, uint256 b) private pure returns (uint256) {
    return ((a * b) + HALF_WAD) / WAD;
  }

  function wadDiv(uint256 a, uint256 b) private pure returns (uint256) {
    return ((a * WAD) + (b / 2)) / b;
  }
  ...
  /**
   * @dev Calculates the fractional per second interest rate from the given annual rate.
   *
   * @param rate The annual interest rate defined as a ray.
   * @return The interest rate on a per second basis to apply.
   **/
  function calculateFractionalRate(uint128 rate) private pure returns (uint128) {
    return add(WAD, wadDiv(rate, SECONDS_PER_YEAR_IN_WAD)).toUint128();
  }

  /**
   * @dev Calculates the compound interest rate based on the number of seconds that have elapsed.
   *
   * @param rate The annual rate to apply.
   * @param elapsed The elapsed time in seconds.
   * 
   * @return The compounded fractional per second rate to apply for the elapsed time.
   **/
  function calculateCompoundRate(uint128 rate, uint256 elapsed) private pure returns (uint128) {
    return pow(calculateFractionalRate(rate), elapsed).toUint128();
  }

  /**
   * @dev Calculates the updated market borrowing index.
   *
   * @param borrowIndex The interest index.
   * @param rate The annual rate to apply.
   * @param elapsed The elapsed time in seconds.
   * 
   * @return The current market borrowing index.
   **/
  function calculateIndex(uint128 borrowIndex, uint128 rate, uint256 elapsed) internal pure returns (uint128) {
    // we have an annual borrow rate which needs to be 
    // applied fractionally for the number of seconds that 
    // have ellapsed since the last time we updated
    uint128 factor = calculateCompoundRate(rate, elapsed);

    return wadMul(borrowIndex, factor).toUint128();
  }

  /**
   * @dev Calculates the total amount that has accrued according to the interest index.
   *
   * @param amount The amount to calculate the accrued value for.
   * @param index The index to calculate the total from.
   * @param marketIndex The current market index to calculate from.
   * 
   * @return The total accrued amount.
   **/
  function calculateAccrued(uint256 amount, uint128 index, uint128 marketIndex) internal pure returns (uint128) {
    if (index == marketIndex) {
      return 0;
    }
    return (wadDiv(wadMul(amount, marketIndex), index) - amount).toUint128();
  }
}
...
import { ReentrancyGuard } from "../../dependencies/@openzeppelin/contracts/utils/ReentrancyGuard.sol";
...
import { IDiscountModel } from "../../interfaces/market/IDiscountModel.sol";
import { IInterestRateModel } from "../../interfaces/market/IInterestRateModel.sol";
import { IPortfolioStorage, PORTFOLIO_ASSET_FIELD_SHARES } from "../../interfaces/portfolio/IPortfolioStorage.sol";
import { IPriceOracle } from "../../interfaces/oracle/IPriceOracle.sol";
import { IDepositorVault } from "../../interfaces/vault/IDepositorVault.sol";
...
import { AuthorizedAccessControl } from "../security/AuthorizedAccessControl.sol";
import { Pausable } from "../utils/Pausable.sol";
import { Sweepable } from "../utils/Sweepable.sol";
import { AdminAccessControl } from "../security/AdminAccessControl.sol";
import { Sanctionable } from "../utils/Sanctionable.sol";

contract Market is AuthorizedAccessControl, Sweepable, Pausable, Sanctionable, ReentrancyGuard, IMarket {
  ...
  uint16 private constant MAX_BORROWING_FEE = 3000; // 30%

  IAddressRegistry public immutable _registry;

  IDiscountModel private _discountModel;

  IInterestRateModel private _interestRateModel;

  // the parameters used to calculate the borrowing fee for the treasury
  Fees.Parameters private _borrowingFee;

  constructor(IAddressRegistry registry, IDiscountModel discountModel, IInterestRateModel interestRateModel) 
    Sweepable(registry) 
    AdminAccessControl(registry) 
    Sanctionable(registry) 
  {
    _registry = registry;
  ...
  modifier requireNonZeroAddress(address value) {
    if (value == address(0)) {
      revert Errors.Validation_ZeroAddressNotAllowed();
    }
    _;
  }

  modifier requireNonZeroAmount(uint256 value) {
    if (value == 0) {
      revert Errors.Validation_ZeroAmountNotAllowed();
    }
    _;
  }

  /// @inheritdoc IMarket
  function setBorrowingFee(Fees.Parameters memory fee) external onlyAdmin {
    _borrowingFee = fee;

    if (BPS.unwrap(_borrowingFee.bps) > MAX_BORROWING_FEE) {
      revert Errors.Validation_LimitExceeded(
        MAX_BORROWING_FEE,
        BPS.unwrap(_borrowingFee.bps)
      );
    }

    emit SetBorrowingFee(fee);
  }

  /// @inheritdoc IMarket
  function getBorrowingFee() external view returns (Fees.Parameters memory) {
    return _borrowingFee;
  }

  /// @inheritdoc IMarket
  function setDiscountModel(IDiscountModel discountModel) external onlyAdmin {
    if (address(discountModel) != address(_discountModel)) {
      accrueLiabilities();

      _discountModel = discountModel;
      
      emit SetDiscountModel(discountModel, msg.sender);
    }
  }
  ...
  /// @inheritdoc IMarket
  function setInterestRateModel(IInterestRateModel interestRateModel) external onlyAdmin {
    if (address(interestRateModel) == address(0)) {
      revert Errors.Validation_ZeroAddressNotAllowed();
    }
    
    if (address(_interestRateModel) != address(interestRateModel)) {
      accrueLiabilities();

      _interestRateModel = interestRateModel;

      emit SetInterestRateModel(interestRateModel, msg.sender);
    }
  }
  ...
  function beforePause() public override {
    accrueLiabilities();
  }

  function afterResume() public override {
    accrueLiabilities();
  }

  /// @inheritdoc IMarket
  function getBorrowLimit(address account, IPriceOracle.Confidence confidence) public view requireNonZeroAddress(account) returns (uint256) {
    IPortfolioStorage.PortfolioAsset[] memory portfolioAssets = _registry.getPortfolioStorage().getPortfolioAssets(account, PORTFOLIO_ASSET_FIELD_SHARES);

    return getBorrowLimit(portfolioAssets, confidence);
  }

  /// @inheritdoc IMarket
  function getBorrowLimit(IPortfolioStorage.PortfolioAsset[] memory portfolioAssets, IPriceOracle.Confidence confidence) public view returns (uint256) {

    uint8 decimals = _registry.getDepositorVault().getDecimals();
    
    IAssetStorage.Asset[] memory assets = _registry.getAssetStorage().getAssets(
      portfolioAssets.getTokenAddresses(), 
      ASSET_FIELD_CUSTODIAN | ASSET_FIELD_PRICE_ORACLE | ASSET_FIELD_MAX_LTV);

    uint256 borrowLimit = 0;

    for (uint256 i = 0; i < portfolioAssets.length;) {
      borrowLimit += getBorrowLimit(assets[i], portfolioAssets[i], confidence).normalize(decimals);
  ...
  function getBorrowLimit(IAssetStorage.Asset memory asset, IPortfolioStorage.PortfolioAsset memory portfolioAsset, IPriceOracle.Confidence confidence) private view returns (USD) {
    require(portfolioAsset.token == asset.token, "token mismatch");
    require(asset.custodian != address(0), "no custodian");

    ICustodian custodian = ICustodian(asset.custodian);      
    uint256 amount = custodian.toAssets(portfolioAsset.shares);
    
    uint8 decimals = custodian.getDecimals();
    
    (USD price, IPriceOracle.Confidence priceConfidence) = IPriceOracle(asset.priceOracle).getLatestPrice();

    // if the confidence that was returned is less than the confidence level 
    // that is required then this price can be assumed to be 0
    if (isConfidenceAcceptable(confidence, priceConfidence) == false) {
      return USD.wrap(0);
    }

    uint256 total = amount.mulDiv(USD.unwrap(price), 10 ** decimals);

    return USD.wrap(asset.maxLTV.percentOf(total));
  }

  function isConfidenceAcceptable(IPriceOracle.Confidence desired, IPriceOracle.Confidence confidence) private pure returns (bool) {
    return uint256(confidence) <= uint256(desired);
  }
  
  /// @inheritdoc IMarket
  function borrow(uint256 amount) external nonReentrant whenNotPaused requireNonZeroAmount(amount) notSanctioned(msg.sender) {
    borrowInternal(msg.sender, amount.toUint128(), msg.sender);
  }

  /// @inheritdoc IMarket
  function borrow(address account, uint256 amount, address receiver) external nonReentrant onlyAuthorized whenNotPaused requireNonZeroAmount(amount) {
    borrowInternal(account, amount.toUint128(), receiver);
  }

  function borrowInternal(address account, uint128 amount, address receiver) private {
    // ensure that the latest liabilities have been calculated for the borrower
    (IMarketStorage.Liability memory userLiability, uint128 accrued) = accrueLiabilities(account);

    uint256 borrowLimit = getBorrowLimit(account, IPriceOracle.Confidence.OK);
    ...
    IMarketStorage marketStorage = _registry.getMarketStorage();

    marketStorage.setUserLiability(
      account, 
      IMarketStorage.Liability({
        borrowed: userLiability.borrowed + amount,
        liabilities: userLiability.liabilities + amount
      }));

    IMarketStorage.Liability memory marketLiabilities = marketStorage.getMarketLiability();
    marketStorage.setMarketLiability(
      IMarketStorage.Liability({
        borrowed: marketLiabilities.borrowed + amount,
        // note, we include the accured interest here as that has already been added 
        // to the userLiability.liabilites in the call to accrueLiabilities() above
        liabilities: marketLiabilities.liabilities + accrued + amount
      }));
    
    // calculate the borrowing fee which is subtracted from 
    // the total amount that the user wants to borrow
    uint256 feeAmount = Fees.calculate(_borrowingFee, amount);

    IDepositorVault vault = _registry.getDepositorVault();
    vault.borrow(amount, receiver, feeAmount, address(_registry.getTreasury()));

    emit Borrow(account, receiver, amount, feeAmount);
  }

  /// @inheritdoc IMarket
  function repay(uint256 amount) external nonReentrant requireNonZeroAmount(amount) notSanctioned(msg.sender) returns (uint256 principal, uint256 interest) {
    return repayInternal(msg.sender, amount.toUint128(), msg.sender);
  }

  /// @inheritdoc IMarket
  function repay(address account, uint256 amount, address repayer) 
    external 
      nonReentrant 
      onlyAuthorized 
  ...
  {
    return repayInternal(account, amount.toUint128(), repayer);
  }

  /// @inheritdoc IMarket
  function repayOutstandingLiabilities() external nonReentrant notSanctioned(msg.sender) returns (uint256 principal, uint256 interest) {
    return repayInternal(msg.sender, MAX_UINT128, msg.sender);
  }
  
  function repayInternal(address account, uint128 amount, address repayer) private returns (uint256 principal, uint256 interest) {
    (IMarketStorage.Liability memory userLiability, uint128 accrued) = accrueLiabilities(account);

    if (userLiability.liabilities == 0) {
      revert Errors.Market_NoLiabilities();
    }

    amount = Math.min(amount, userLiability.liabilities).toUint128();

    IDepositorVault vault = _registry.getDepositorVault();
    IERC20Metadata underlyingAsset = vault.getUnderlyingAsset();

    // transfer the full amount from the borrower to the market
    underlyingAsset.safeTransferFrom(repayer, address(this), amount);

    IMarketStorage marketStorage = _registry.getMarketStorage();

    // when the account pays off their debts they need to pay off a portion which is
    // the borrowed amount and then the interest accrued is payed as yield
    (principal, interest) = calculateRepaymentComponents(amount, userLiability);

    principal = principal < userLiability.borrowed ? principal : userLiability.borrowed;

    marketStorage.setUserLiability(
      account, 
      IMarketStorage.Liability({
        borrowed: userLiability.borrowed - principal.toUint128(),
        liabilities: userLiability.liabilities - amount
      }));

    IMarketStorage.Liability memory marketLiability = marketStorage.getMarketLiability();
    marketStorage.setMarketLiability(
      IMarketStorage.Liability({
        borrowed: marketLiability.borrowed - principal.toUint128(),
        // note, we include the accured interest here as that has already been added 
        // to the userLiability.liabilites in the call to accrueLiabilities() above
        liabilities: marketLiability.liabilities + accrued - amount
      }));

    // repay the vault and donate the interest as yield
    underlyingAsset.forceApprove(address(vault), principal + interest);
    vault.repay(principal);
    
    if (interest > 0) {
      vault.donate(interest);
    }

    emit Repay(account, repayer, amount);
  }

  /// @dev given a repayment amount, split into the principal and interest portions
  function calculateRepaymentComponents(uint128 amount, IMarketStorage.Liability memory userLiability) private pure returns (uint128 principal, uint128 interest) {
    interest = uint256(uint256(amount) * (userLiability.liabilities - userLiability.borrowed) / userLiability.liabilities).toUint128();

    // the remaining amount is deducted from the principal component
    principal = amount - interest;
  }

  /// @inheritdoc IMarket
  function getLiabilities(address account) public view returns (uint256) {
    IMarketStorage marketStorage = _registry.getMarketStorage();

    IMarketStorage.Liability memory userLiability = marketStorage.getUserLiability(account);

    if (userLiability.liabilities == 0) {
      return 0;
    }

    IMarketStorage.UserState memory userState = marketStorage.getUserState(account);

    IMarketStorage.MarketState memory marketState = marketStorage.getMarketState();

    if (marketState.lastUpdate != block.timestamp) {
      marketState = calculateMarketState(marketState);
    }

    uint128 interest = calculateAccruedInterest(account, marketState, userState, userLiability);

    return userLiability.liabilities + interest;
  }
  ...
  function accrue(address account) external {
    ...
    marketStorage.setUserLiability(
      account, 
      IMarketStorage.Liability({
        borrowed: userLiability.borrowed,
        liabilities: userLiability.liabilities
      }));

    IMarketStorage.Liability memory marketLiabilities = marketStorage.getMarketLiability();
    marketStorage.setMarketLiability(
      IMarketStorage.Liability({
        borrowed: marketLiabilities.borrowed,
        liabilities: marketLiabilities.liabilities + accrued
      }));
  }

  /// @dev accrue the market liabilities
  function accrueLiabilities() private returns (IMarketStorage.MarketState memory) {
    IMarketStorage marketStorage = _registry.getMarketStorage();
    ...
    if (marketState.lastUpdate != block.timestamp) {
      marketState = calculateMarketState(marketState);
      marketStorage.setMarketState(marketState);
    }

    return marketState;
  }

  function accrueLiabilities(address account) private returns (IMarketStorage.Liability memory userLiability, uint128 interest) {
    IMarketStorage marketStorage = _registry.getMarketStorage();

    IMarketStorage.MarketState memory marketState = accrueLiabilities();

    IMarketStorage.UserState memory userState = marketStorage.getUserState(account);

    // calculate the updated user liabilities, note that we dont update the 
    // user liability here as the caller of this function is required to do it
    userLiability = marketStorage.getUserLiability(account);
    interest = calculateAccruedInterest(account, marketState, userState, userLiability);
    userLiability.liabilities += interest;

    // update the user state to track the market state
    userState.borrowIndex = marketState.borrowIndex;
    userState.discountIndex = marketState.discountIndex;
    marketStorage.setUserState(account, userState);

    emit AccrueLiabilities(account, userLiability.liabilities, userState.borrowIndex);
  }

  function calculateMarketState(IMarketStorage.MarketState memory marketState) internal view returns (IMarketStorage.MarketState memory) {
    // dont want to accrue liabilities if the market has been paused
    if (paused()) {
      marketState.lastUpdate = block.timestamp.toUint32();
      return marketState;
    }
    ...
    return calculateMarketState(marketState, elapsed);
  }

  function calculateMarketState(IMarketStorage.MarketState memory marketState, uint256 elapsed) internal view returns (IMarketStorage.MarketState memory) {
    // update the borrowing index
    uint128 borrowingRate = _interestRateModel.calculateRate();

    uint128 borrowIndex = InterestMath.calculateIndex(marketState.borrowIndex, borrowingRate, elapsed);
    require(borrowIndex >= marketState.borrowIndex, "invalid borrow index");

    marketState.borrowIndex = borrowIndex;

    // update the discount index
    if (address(_discountModel) != address(0)) {
      uint128 discountRate = _discountModel.calculateDiscountRate(borrowingRate);
      
      uint128 discountIndex = InterestMath.calculateIndex(marketState.discountIndex, discountRate, elapsed);
      require(discountIndex >= marketState.discountIndex, "invalid discount index");

      marketState.discountIndex = discountIndex;
    }
  ...
  function calculateAccruedInterest(
    address account,
    IMarketStorage.MarketState memory marketState, 
    IMarketStorage.UserState memory userState,
    IMarketStorage.Liability memory userLiability) private view returns (uint128 interest) {

    if (userLiability.liabilities > 0) {
      interest = address(_discountModel) != address(0) 
        ? calculateDiscountedLiabilities(account, marketState, userState, userLiability)
        : InterestMath.calculateAccrued(userLiability.liabilities, userState.borrowIndex, marketState.borrowIndex);
    }
  }

  function calculateDiscountedLiabilities(
    address account,
    IMarketStorage.MarketState memory marketState, 
    IMarketStorage.UserState memory userState,
    IMarketStorage.Liability memory userLiability) private view returns (uint128) {

    // calculate the liabilities with according to the rules of the discount model
    uint128 discount = _discountModel.calculateDiscountAmount(account, userLiability);

    uint256 percentage = discount.mulDiv(1e18, userLiability.liabilities);

    uint128 discountAmount = userLiability.liabilities.mulDiv(percentage, 1e18).toUint128();

    uint128 remaining = userLiability.liabilities - discountAmount;

    discountAmount = InterestMath.calculateAccrued(
      discountAmount, 
      userState.discountIndex == 0 ? 1e18 : userState.discountIndex, 
      marketState.discountIndex);

    remaining = InterestMath.calculateAccrued(remaining, userState.borrowIndex, marketState.borrowIndex);

    return discountAmount + remaining;
  }

  /// @inheritdoc IMarket
  function ensureHealthyAccount(address account) external view {
    ensureHealthyAccount(account, 100);
  }

  /// @inheritdoc IMarket
  function ensureHealthyAccount(address account, uint16 threshold) public view {
    uint256 liabilities = getLiabilities(account);
    ...
    for (uint256 i = 0; i < portfolioAssets.length;) {
      // if the price confidence is not ok then this is treated as no borrow limit for the asset
      borrowLimit += getBorrowLimit(assets[i], portfolioAssets[i], IPriceOracle.Confidence.OK).normalize(decimals);

      if (borrowLimit * 100 / liabilities >= threshold) {
        // check the borrow limit on each asset as it can save gas by short circuiting here
        return;
      }
    ...
    revert Errors.Market_UnhealthyAccount(account, borrowLimit * 100 / liabilities);
  }  
}
...
abstract contract AdminAccessControl {
  ...
  modifier onlyAdmin() {
    if (_registry.getAccessControlList().hasRole(ADMIN_ROLE, msg.sender) == false) {
  ...
  modifier onlyEmergencyAdmin() {
    if (_registry.getAccessControlList().hasRole(EMERGENCY_ADMIN_ROLE, msg.sender) == false) {
      revert Errors.Authorization_NotAuthorized({ caller: msg.sender });
    }
    _;
  }
  ...
  modifier onlyAdminOrEmergencyAdmin() {
    {
      IAccessControlList acl = _registry.getAccessControlList();
      bool isAuthorized = acl.hasRole(ADMIN_ROLE, msg.sender) || acl.hasRole(EMERGENCY_ADMIN_ROLE, msg.sender);
  ...
  /// @dev returns true if the current caller is an admin
  function isAdmin() internal view returns (bool) {
    IAccessControlList acl = _registry.getAccessControlList();
    return acl.hasRole(ADMIN_ROLE, msg.sender);
  }
}
...
abstract contract AuthorizedAccessControl is AdminAccessControl {
  ...
  EnumerableSet.AddressSet private _authorized;

  modifier onlyAuthorized() {
    if (_authorized.contains(msg.sender) == false) {
      revert Errors.Authorization_NotAuthorized({ caller: msg.sender });
    }
    _;
  }

  /// @dev grant authorization to privilleged functions to this component.
  function grantAuthorization(address account) external onlyAdmin {
    if (_authorized.contains(account) == false) {
      _authorized.add(account);
      emit GrantAuthorization(account, msg.sender);
    }
  }

  /// @dev revoke authorization to privilleged functions to this component.
  function revokeAuthorization(address account) external onlyAdmin {
    if (_authorized.contains(account) == true) {
      _authorized.remove(account);
      emit RevokeAuthorization(account, msg.sender);
    }
  }

  /// @dev returns a list of the authorized accounts for this component.
  function getAuthorized() external view returns (address[] memory) {
    return _authorized.values();
  }
}
...
import { Pausable as OpenZeppelinPausable } from "../../dependencies/@openzeppelin/contracts/security/Pausable.sol";
import { IPausable } from "../../interfaces/utils/IPausable.sol";
import { AdminAccessControl } from "../security/AdminAccessControl.sol";

abstract contract Pausable is AdminAccessControl, OpenZeppelinPausable, IPausable {

  /// @inheritdoc IPausable
  function pause() external onlyAdminOrEmergencyAdmin {
    beforePause();
  ...
  function beforePause() public virtual { } // solhint-disable-line no-empty-blocks

  function afterPause() public virtual { } // solhint-disable-line no-empty-blocks

  /// @inheritdoc IPausable
  function resume() external onlyAdminOrEmergencyAdmin {
    beforeResume();
  ...
  function afterResume() public virtual { } // solhint-disable-line no-empty-blocks

  /// @inheritdoc IPausable
  function paused() public view override(IPausable, OpenZeppelinPausable) returns (bool) {
    return super.paused();
  }
}
...
abstract contract Sweepable is AdminAccessControl, ISweepable {
  ...
  function sweep(IERC20 token, uint256 amount) external onlyAdminOrEmergencyAdmin {
    amount = Math.min(amount, token.balanceOf(address(this)));
    
    address treasury = address(_registry.getTreasury());

    token.safeTransfer(treasury, amount);

    emit Sweep(msg.sender, treasury, address(token), amount);
  }

  /// @inheritdoc ISweepable
  function sweepETH(uint256 amount) public onlyAdminOrEmergencyAdmin {
    ...
    emit SweepETH(msg.sender, treasury, amount);
  }
}