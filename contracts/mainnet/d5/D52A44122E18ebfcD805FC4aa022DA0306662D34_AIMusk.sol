...
// File contracts/interfaces/IElonMarsPool.sol



interface IElonMarsPool {
...
// File contracts/interfaces/IElonStarsPool.sol



interface IElonStarsPool {
    function deposit(uint256 _pid, uint256 _amount) external;
    function withdraw(uint256 _pid, uint256 _amount) external;
    function emergencyWithdraw(uint256 _pid) external;
    function injectRewards(uint256 amount) external;
...
interface IJackpot {
    ...
    function setStarsToken(IERC20 starsToken_) external;
}
...
// File contracts/AIMusk.sol




contract AIMusk is ERC20, Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;

    event SwapBack(uint256 burn, uint256 gov1, uint256 gov2, uint256 liquidity, uint256 jackpot, uint256 stars, uint256 mars, uint256 donate, uint timestamp);
    ...
    uint256 public swapBackMinimum = 1000;
    ...
    uint256 private starsFee;
    uint256 private donateFee;
    uint256 private marsFee;
    ...
    uint256 public starsFeeBuy = 300;
    uint256 public donateFeeBuy = 250;
    uint256 public marsFeeBuy = 50;
    ...
    uint256 public starsFeeSell = 300;
    uint256 public donateFeeSell = 250;
    uint256 public marsFeeSell = 50;
    uint256 public totalFeeSell = 1500;

    // Fees receivers
    address private gov1Wallet;
    address private gov2Wallet;
    address private starsWallet;
    address private marsWallet;
    IJackpot public jackpotWallet;
    address private donateWallet;
    ...
    constructor(
        ...
        uint256 _totalSupply = 197_106_280_000_000_000 * 1e6;
    ...
    function initializePair() external onlyOwner {
        require(!initialized, "AIMusk: Already initialized");
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        return _starTransfer(_msgSender(), to, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        address spender = _msgSender();
        _spendAllowance(sender, spender, amount);
        return _starTransfer(sender, recipient, amount);
    }

    function _starTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        if (inSwap) {
            _transfer(sender, recipient, amount);
            return true;
        }
        if (!canAddLiquidityBeforeLaunch[sender]) {
            require(launched(), "AIMusk: Trading not open yet");
        }
    ...
    function swapBack() internal swapping {
        ...
        sellFees();
        uint256 amountStarBurn = (taxAmount * burnFee) / (totalFee);
        uint256 amountStarLp = (taxAmount * liquidityFee) / (totalFee);
        uint256 amountElonStars = (taxAmount * starsFee) / (totalFee);
        uint256 amountElonMars = (taxAmount * marsFee) / (totalFee);
        taxAmount -= amountStarBurn;
        taxAmount -= amountStarLp;
        taxAmount -= amountElonStars;
        taxAmount -= amountElonMars;
        ...
        _transfer(address(this), DEAD, amountStarBurn);
        _doRewardsInject(amountElonStars, amountElonMars);

        uint256 amountBackToken = backToken.balanceOf(address(this)) - balanceBefore;
        uint256 backTokenTotalFee = totalFee - burnFee - liquidityFee - starsFee - marsFee;
        ...
        uint256 amountBackTokenDonate = amountBackToken - amountBackTokenGov1 - amountBackTokenGov2 - amountBackTokenJackpot;
        ...
        backToken.transfer(donateWallet, amountBackTokenDonate);

        if (addLiquidityEnabled) {
            _doAddLp();
        }

        emit SwapBack(amountStarBurn, amountBackTokenGov1, amountBackTokenGov2, amountStarLp, amountBackTokenJackpot, amountElonStars, amountElonMars, amountBackTokenDonate, block.timestamp);
    }

    function _doRewardsInject(uint256 amountElonStars, uint256 amountElonMars) internal {
        _approve(address(this), address(starsWallet), amountElonStars);
        _approve(address(this), address(marsWallet), amountElonMars);
        IElonStarsPool(starsWallet).injectRewards(amountElonStars);
        IElonMarsPool(marsWallet).injectRewards(amountElonMars);
    }
    ...
    function _doAddLp() internal {
        ...
        if(half < swapBackMinimum) return;
    ...
    function buyFees() internal {
        ...
        starsFee = starsFeeBuy;
        marsFee = marsFeeBuy;
        donateFee = donateFeeBuy;
    ...
    function sellFees() internal {
        ...
        starsFee = starsFeeSell;
        marsFee = marsFeeSell;
        donateFee = donateFeeSell;
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'AIMUSK: ETH_TRANSFER_FAILED');
    }

    function clearStuckBalance(address tokenAddr) external onlyOwner {
        IERC20 token = IERC20(tokenAddr);
        token.transfer(_msgSender(), token.balanceOf(address(this)));
    }

    function setBackToken(address _backToken) public onlyOwner {
        backToken = IERC20(_backToken);
        jackpotWallet.setStarsToken(backToken);
    }
    ...
    function launch() public onlyOwner {
        require(launchedAt == 0, "AIMusk: The rocket already launchhed!");
    ...
    function setBuyFees(
        ...
        starsFeeBuy = _starsFee;
        marsFeeBuy = _marsFee;
        donateFeeBuy = _donateFee;
        burnFeeBuy = _burnFee;
        // AIDGOE BUG fixed
        totalFeeBuy = _gov1Fee + _gov2Fee + _liquidityFee + _jackpotFee + _starsFee + _marsFee +  _donateFee + _burnFee;
    }
    ...
    function setSellFees(
        ...
        starsFeeSell = _starsFee;
        marsFeeSell = _marsFee;
        donateFeeSell = _donateFee;
        burnFeeSell = _burnFee;
        // AIDGOE BUG fixed
        totalFeeSell = _gov1Fee + _gov2Fee + _liquidityFee + _jackpotFee + _starsFee + _marsFee + _donateFee + _burnFee;
    }

    function setFeeReceivers(
        address _gov1Wallet,
        address _gov2Wallet,
        address _starsWallet,
        address _marsWallet,
        ...
        starsWallet = _starsWallet;
        marsWallet = _marsWallet;
        jackpotWallet = IJackpot(_jackpotWallet);
        donateWallet = _donateWallet;
        donateWallet = _donateWallet;
    }

    function setSwapBackMinimum(uint256 _minimum) external onlyOwner {
        swapBackMinimum = _minimum;
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "AIMUSK: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "AIMUSK: index out of bounds");
