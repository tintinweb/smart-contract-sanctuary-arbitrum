...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
	...
	function _baseURI() internal view virtual returns (string memory) {
		return "https://ipfs.io/ipfs/QmQ1CvnCBmMyuvUKgnBGMy6NFdPc4wJ8U3s79WYttgbMdk/";
	}
...
contract Nft_KINDSHIB is ERC721Enumerable {

	address owner;
	event Mint (address indexed mintBy);
	uint256 public mint_NativePrice = uint256(100000000000000);
	event mint_NativeNewPrice (uint256 newPrice);
	uint256 public mintAmtInBank = uint256(0);

	constructor() ERC721("KindShibaNFT", "KINDSHIB") {
		owner = msg.sender;
	}
 ...
 * checks that ((totalSupply) + (_amt)) is less than or equals to 20000
 * checks that (amount of native currency sent to contract) is equals to (((mint_NativePrice) * (_amt)) / (10 raised to the power of 0))
 ...
 * updates mintAmtInBank as (mintAmtInBank) + (((mint_NativePrice) * (_amt)) / (10 raised to the power of 0))
 * emits event Mint with inputs the address that called this function
*/
	function mint(uint256 _amt) public payable {
		require(((totalSupply() + _amt) <= uint256(20000)), "Exceeded Total Supply");
		require((msg.value == ((mint_NativePrice * _amt) / (uint256(10) ** uint256(0)))), "Wrong Fee");
		for (uint i0 = 0; i0 < _amt; i0++){
			uint256 supply = super.totalSupply();
			super._safeMint(msg.sender, supply);
		}
		mintAmtInBank  = (mintAmtInBank + ((mint_NativePrice * _amt) / (uint256(10) ** uint256(0))));
