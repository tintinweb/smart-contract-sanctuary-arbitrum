...
contract ChronosV3Vault is ERC20Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function earn() public {
        if (onlyHarvester) {
            require(allowedHarvester(msg.sender), "!harvester");
        }
    ...
    function allowedHarvester(address _harvester) public view returns (bool) {
        return
            IStrategyRebalanceStakerUniV3(address(IControllerV7(controller).strategies(address(pool)))).harvesters(
                _harvester
            );
    }
    ...
    function getProportion() public view returns (uint256) {
        uint160 sqrtRatioX96 = uniswapCalculator.getSqrtRatioAtTick(twapTick());
    ...
    function twapTick() public view returns (int24) {
        ...
        int56 _averageTick = (_cumulativeTicks[1] - _cumulativeTicks[0]) / int24(_twapTime);
        return int24(_averageTick);
    }
    ...
    function _refundUnused(bool _maticUsed) internal returns (uint256) {
        ...
        uint160 sqrtPriceX96 = uniswapCalculator.getSqrtRatioAtTick(twapTick());
        ...
        if (_cache.amount0Desired > _cache.amount0)
            if ((address(token0) == address(wNative)) && _maticUsed)
                _refundMatic(_cache.amount0Desired - (_cache.amount0));
            else {
                token0.safeTransfer(msg.sender, _cache.amount0Desired - (_cache.amount0));
            }

        if (_cache.amount1Desired > _cache.amount1)
            if ((address(token1) == address(wNative)) && _maticUsed)
                _refundMatic(_cache.amount1Desired - (_cache.amount1));
            else {
    ...
    function updateOnlyHarvester(bool _onlyHarvester) external onlyOwner {
        onlyHarvester = _onlyHarvester;
    }

    function updateTwapTime(uint24 _twapTime) external onlyOwner {
        twapTime = _twapTime;
    }
