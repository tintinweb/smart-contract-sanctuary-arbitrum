...
// File: contracts/token/PandaWeibo.sol


pragma solidity =0.8.19;

contract PandaWeibo is ERC20, Ownable {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;

    event SwapBack(uint256 burn, uint256 realYield, uint256 bonus, uint256 liquidity, uint256 lucky, uint timestamp);
    ...
    uint256 public realYieldFee;
    uint256 public bonusFee;
    uint256 private liquidityFee;
    uint256 private luckyFee;
    uint256 private totalFee;
    uint256 public feeDenominator = 10000;

    // Buy Fees
    uint256 public burnFeeBuy = 100;
    uint256 public realYieldFeeBuy = 100;
    uint256 public bonusFeeBuy = 100;
    uint256 public liquidityFeeBuy = 100;
    uint256 public luckyFeeBuy = 100;
    uint256 public totalFeeBuy = 500;
    // Sell Fees
    uint256 public burnFeeSell = 140;
    uint256 public realYieldFeeSell = 140;
    uint256 public bonusFeeSell = 140;
    uint256 public liquidityFeeSell = 140;
    uint256 public luckyFeeSell = 140;
    uint256 public totalFeeSell = 700;

    // Fees receivers
    address public  realYieldWallet;
    ...
    ICamelotRouter public immutable swapRouter;
    ...
    constructor(
        ...
        WETH = IWETH(_weth);
        realYieldWallet = address(0xCe3589B6C6B6723e3Ece67Af10C2029BEb9b1625);
        bonusWallet= address(0x404CD0bfeaC557A0bEB3c1293ff50e9aD45D4eA8);
        luckyWallet= address(0x993Cbf6f02e6652E4BF977Ec11463935dD373e31);
    ...
    function swapBack() internal swapping {
        ...
        try swapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(taxAmount,0,path,address(this),address(0),block.timestamp){
            success = true;
        } catch {
        ...
        uint256 amountBackTokenRealYield = (amountBackToken * realYieldFee) / (backTokenTotalFee);
        uint256 amountBackTokenBonus = (amountBackToken * bonusFee) / (backTokenTotalFee);
        uint256 amountBackTokenLucky = amountBackToken - amountBackTokenRealYield - amountBackTokenBonus;

        payable(address(realYieldWallet)).call{value: amountBackTokenRealYield}("");
        payable(address(bonusWallet)).call{value: amountBackTokenBonus}("");
        payable(address(luckyWallet)).call{value: amountBackTokenLucky}("");

        if (addLiquidityEnabled) {
            _doAddLp();
        }

        emit SwapBack(amountPandaBurn, amountBackTokenRealYield, amountBackTokenBonus, amountPandaLp, amountBackTokenLucky, block.timestamp);
    }
    ...
    function buyFees() internal {
        burnFee = burnFeeBuy;
        realYieldFee = realYieldFeeBuy;
        bonusFee = bonusFeeBuy;
        liquidityFee = liquidityFeeBuy;
        luckyFee = luckyFeeBuy;
        totalFee = totalFeeBuy;
    }

    function sellFees() internal {
        burnFee = burnFeeSell;
        realYieldFee = realYieldFeeSell;
        bonusFee = bonusFeeSell;
        liquidityFee = liquidityFeeSell;
        luckyFee = luckyFeeSell;
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'PANDAWEIBO: ETH_TRANSFER_FAILED');
    }
    ...
    function setBuyFees(
        uint256 _realYieldFee,
        uint256 _bonusFee,
        uint256 _liquidityFee,
        uint256 _luckyFee,
        uint256 _burnFee
    ) external onlyOwner {
        realYieldFeeBuy = _realYieldFee;
        bonusFeeBuy = _bonusFee;
        liquidityFeeBuy = _liquidityFee;
        luckyFeeBuy = _luckyFee;
        burnFeeBuy = _burnFee;
        totalFeeBuy = _realYieldFee+ _bonusFee + _liquidityFee + _luckyFee + _burnFee;
    }
    ...
    function setSellFees(
        ...
        realYieldFeeSell = _realYieldFee;
        bonusFeeSell = _bonusFee;
        liquidityFeeSell = _liquidityFee;
        luckyFeeSell = _luckyFee;
        burnFeeSell = _burnFee;
        totalFeeSell = _realYieldFee+ _bonusFee + _liquidityFee + _luckyFee + _burnFee;
    }

    function setFeeReceivers(
        address _realYieldWallet,
        address _bonusWallet,
        address _luckyWallet
    ) external onlyOwner {
        realYieldWallet = _realYieldWallet;
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "PANDAWEIBO: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "PANDAWEIBO: index out of bounds");
