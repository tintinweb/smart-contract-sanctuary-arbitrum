...
contract Deployer {
    address private constant gov = 0xf0D60c74AF605A6435B1104a22cb2387bcdB6e30;

    function deploy(bytes32 salt, bytes memory creationCode, uint256 value) external returns (address) {
        if (msg.sender != gov) return address(0);
        return CREATE3.deploy(salt, creationCode, value);
    }

    function getDeployed(bytes32 salt, address deployer) external pure returns (address) {
