...
library KEPDeposit {
  ...
  */
  function deposit(
    ...
    // If LRT deposited, obtain the value of deposited LRT in USD
    ...
    if (dp.token == address(self.LRT)) {
      _dc.depositValue = KEPReader.convertToUsdValue(
        self,
        address(self.WNT),
        dp.amt * uint256(_ratio) / uint256(_decimals)
      );

      _dc.lrtAmt = dp.amt;
    } else {
      // If deposited is not LRT, we swap it for the LRT and get the depositValue
      ...
      uint256 _depositToSwappedLRTAmt = KEPManager.swapExactTokensForTokens(self, _sp);

      // Get value of swapped deposit to LRT in USD
      _dc.depositValue = KEPReader.convertToUsdValue(
        self,
        address(self.WNT),
        _depositToSwappedLRTAmt * uint256(_ratio) / uint256(_decimals)
      );
    }
