...
contract NitroSetup {
  uint private constant gasPrice = 1e8;
  uint private constant deployGasLimit = 1e6;
  uint private constant runGasLimit = 1e6;
  ...
  uint public constant COST = (deployGasLimit + runGasLimit) * gasPrice;
  ...
  function configureRollup(
    ...
    if (nativeToken != address(0)) {
      ...
      IERC20(nativeToken).transferFrom(msg.sender, address(this), COST);
      IERC20(nativeToken).approve(inbox, COST);
    } else {
      require(msg.value > COST, "msg.value less than min cost");
    }
    uint fee = 0;
    fee += createRetryableTicket(inbox, nativeToken, ZOLTU, deployGasLimit, type(L2Setup).creationCode);
    fee += createRetryableTicket(inbox, nativeToken, upgradeExecutorL2, runGasLimit,
    ...
    if (nativeToken == address(0)) {
      (bool success, ) = msg.sender.call{value: msg.value - COST - fee}("");
      require (success == true, "refund failed");
    }
  }
  ...
  function createRetryableTicket(
    ...
    if (nativeToken == address(0)) {
      uint fee = IInbox(inbox).calculateRetryableSubmissionFee(code.length, 0);
      IInbox(inbox).createRetryableTicket{value: fee + gasLimit * gasPrice}(
        target, 0, fee, msg.sender, msg.sender, gasLimit, gasPrice, code);
      return fee;
    }
    IERC20Inbox(inbox).createRetryableTicket(
      target, 0, 0, msg.sender, msg.sender, gasLimit, gasPrice, gasLimit * gasPrice, code);
    return 0;
  }
}
