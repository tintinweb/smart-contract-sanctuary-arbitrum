...
import "./interfaces/IShareNFT.sol";
import "./modules/ShareLevelAmount.sol";
...
//        if(earningTime > epochDuration) earningTime = epochDuration;
    ...
    function updateRewardForUser(uint256 start, uint256 size, bool isDailyUpdate) public onlyAdmin {
        uint256 length = start+size;
        if (length > joinedUsers.length) {
            length = joinedUsers.length;
        }
        for(uint256 i = start; i < length; i++){
            _updateReward(joinedUsers[i],isDailyUpdate);
        }
    }

    // There is something wrong when operating the system; this function is designed to update the lost value.
    function updateLostEarningForUser(
        uint256 start,
        uint256 size,
        uint256 lostRate,
        uint256 lostTime
        ...
        for(uint256 i = start; i < length; i++){
            UserInfo storage user = users[joinedUsers[i]];
            if (!user.isActive){
                return;
            }
            uint256 lostPerOne = BASE_LEVEL_AMOUNT.mul(lostRate).div(
                DIVISION_FACTOR
            ).div(epochDuration);
            uint256 lostValue = user.amount.mul(lostPerOne).mul(lostTime).div(1e18);
            user.earnings = user.earnings.add(lostValue);
            emit RewardUpdated(joinedUsers[i], lostValue);
        }
    }
