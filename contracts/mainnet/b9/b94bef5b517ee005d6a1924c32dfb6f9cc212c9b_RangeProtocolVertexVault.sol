...
//        //        // 86400 is the seconds for    eats for the individual price feeds, 1800 seconds is the 30 minutes buffer
...
//    }
    ...
    function swap(address target, bytes calldata swapData, IERC20 tokenIn, uint256 amountIn) external onlyManager {
        ...
        if (lastSwapTimestamp + minSwapInterval > block.timestamp) revert VaultErrors.SwapTimeIntervalIsNotSatisfied();
    ...
    function changeMinSwapInterval(uint256 newSwapInterval) external override onlyUpgrader {
        // @note we are not adding a limit check on min swap interval optimistically assuming that the upgrader will
        // set a reasonable limit on the min swap interval.
        minSwapInterval = newSwapInterval;
        emit SwapIntervalChanged(newSwapInterval);
    }
...
library VaultErrors {
    ...
    error SwapTimeIntervalIsNotSatisfied();
}
...
interface IRangeProtocolVertexVault is IERC20 {
    ...
    event SwapIntervalChanged(uint256 swapInterval);
    ...
    function changeMinSwapInterval(uint256 newSwapInterval) external;
