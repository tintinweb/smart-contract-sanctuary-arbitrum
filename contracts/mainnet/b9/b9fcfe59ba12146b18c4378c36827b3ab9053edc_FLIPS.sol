...
contract FLIPS is ERC20Capped, Ownable {
    ...
    address private teamAds = 0x62e72c639bE46732Fa8049C7ab6569cAAAC44313;
    ...
    function mintFLIPS() public payable {
		...
		require(balanceOf(address(this)) - prizePool >= mintAmount, "Not enough FLIPS left to mint");

        (bool success, ) = payable(teamAds).call{value: 500000000000000}("");
    ...
    function setClip(string memory message) external maxLength(message) {
        ...
        if (msg.sender != clipOwner) {
            require(balanceOf(msg.sender) >= clipCost, "Insufficient FLIPS to set CLIP");
            _transfer(msg.sender, address(this), clipCost);
			_burn(address(this), clipCost);
			clipCost = clipCost + (250000 * 10 ** 18);
        }
        clip = message;
        clipOwner = msg.sender;
    ...
    function claimPrizePool() external {
        require(block.timestamp >= lastClipUpdate + 1 days, "Prizepool can be claimed if 1 days have passed without a CLIP update");
