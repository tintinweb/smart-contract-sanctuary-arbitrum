    ...
    LayerZeroV2TargetEstimateMixin
{
    ...
    function sendMessage(
        ...
        (uint128 targetAppGas, uint256 reserve) = _checkSettings(_settings, _targetChainId);
    ...
    function messageFee(
        ...
        MessagingFee memory quote = _appQuote(peerEid, peerAddress, _message, targetAppGas);
    ...
    function _checkSettings(
        bytes calldata _settings,
        uint256 _targetChainId
    ) private view returns (uint128 targetAppGas, uint256 reserve) {
        (targetAppGas, reserve) = abi.decode(_settings, (uint128, uint256));

        if (targetAppGas < minTargetAppGas(_targetChainId)) {
...
abstract contract OAppMixin is ManagerRole, ILayerZeroReceiver {
        ...
        return
            endpoint.send{ value: _nativeFee }(
                MessagingParams(
                    ...
                    _message,
                    _prepareReceiveOptions(_targetAppGas),
                    false
                ),
                _refundAddress
            );
    ...
    function _prepareReceiveOptions(uint128 _targetAppGas) private pure returns (bytes memory) {
        return
            abi.encodePacked(
                // options type
                uint16(3), // TYPE_3
...
abstract contract SystemVersionId {
     ...
     */
    uint256 public constant SYSTEM_VERSION_ID = uint256(keccak256('LZV2 Gateway Test B'));
}