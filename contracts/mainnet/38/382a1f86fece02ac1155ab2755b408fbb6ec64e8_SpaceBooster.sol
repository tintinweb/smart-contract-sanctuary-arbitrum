...
interface IMasterChef {
  ...
  function pendingSpace(address _stakeToken, address _user) external view returns (uint256);
  ...
  function depositSpace(address _for, uint256 _amount) external;

  function withdrawSpace(address _for, uint256 _amount) external;
  ...
  function space() external returns (ISPACE);
}
...
contract SpaceBooster is Ownable, Pausable, ReentrancyGuard, AccessControl, IMasterChefCallback, IERC721Receiver {
  ...
  ISpaceBoosterConfig public spaceboosterConfig;
  IERC20 public space;
  ...
  constructor(
    ...
    ISpaceBoosterConfig _spaceboosterConfig,
    IWNativeRelayer _wNativeRelayer,
    address _wNative
  ) public {
    require(_wNative != address(0), "SpaceBooster::constructor:: _wNative cannot be address(0)");
    require(address(_space) != address(0), "SpaceNFTOffering::constructor:: _space cannot be address(0)");
    require(address(_masterChef) != address(0), "SpaceNFTOffering::constructor:: _masterChef cannot be address(0)");
    require(
      address(_spaceboosterConfig) != address(0),
      "SpaceNFTOffering::constructor:: _spaceboosterConfig cannot be address(0)"
    );
    require(
      address(_wNativeRelayer) != address(0),
      "SpaceNFTOffering::constructor:: _wNativeRelayer cannot be address(0)"
    );
    ...
    spaceboosterConfig = _spaceboosterConfig;
    space = _space;
  ...
  modifier inExec() {
    require(_IN_EXEC_LOCK == _NOT_ENTERED, "SpaceBooster::inExec:: in exec lock");
    require(address(masterChef) == _msgSender(), "SpaceBooster::inExec:: not from the master chef");
  ...
  modifier isStakeTokenOK(address _stakeToken) {
    require(spaceboosterConfig.stakeTokenAllowance(_stakeToken), "SpaceBooster::isStakeTokenOK::bad stake token");
    _;
  }

  /// @dev validate whether a specified nft can be staked into a particular staoke token
  modifier isSpaceBoosterNftOK(
    address _stakeToken,
    address _nftAddress,
    uint256 _nftTokenId
  ) {
    require(
      spaceboosterConfig.spaceboosterNftAllowance(_stakeToken, _nftAddress, _nftTokenId),
      "SpaceBooster::isSpaceBoosterNftOK::bad nft"
    );
    _;
  }
  ...
  modifier permit(bytes calldata _sig) {
    ...
    require(recoveredAddress == _msgSender(), "SpaceBooster::permit::INVALID_SIGNATURE");
    _;
  }

  modifier onlyGovernance() {
    require(hasRole(GOVERNANCE_ROLE, _msgSender()), "SpaceBooster::onlyGovernance::only GOVERNANCE role");
    _;
  }

  /// @dev Require that the caller must be an EOA account to avoid flash loans.
  modifier onlyEOA() {
    require(msg.sender == tx.origin, "SpaceBooster::onlyEOA:: not eoa");
    _;
  }
  ...
  /// @dev View function to see pending booster SPACEs on frontend.
  function pendingBoosterSpace(address _stakeToken, address _user) external view returns (uint256) {
    uint256 pendingSpace = masterChef.pendingSpace(_stakeToken, _user);
    NFTStakingInfo memory stakingNFT = userStakingNFT[_stakeToken][_user];
    if (stakingNFT.nftAddress == address(0)) {
      return 0;
    }
    (, , uint256 boostBps) = spaceboosterConfig.energyInfo(stakingNFT.nftAddress, stakingNFT.nftTokenId);
    // if (currentEnergy == 0) {
    //   return 0;
    // }
    return pendingSpace.mul(boostBps).div(1e4);
    // return Math.min(currentEnergy, pendingSpace.mul(boostBps).div(1e4));
  }

  /// @dev Internal function for withdraspace a boosted stake token and receive a reward from a master chef
  ...
  function _withdrawFromMasterChef(IERC20 _stakeToken, uint256 _shares) internal {
    if (_shares == 0) return;
    if (address(_stakeToken) == address(space)) {
      masterChef.withdrawSpace(_msgSender(), _shares);
    } else {
  ...
  function _harvestFromMasterChef(address user, IERC20 _stakeToken) internal {
    ...
    uint256 beforeReward = space.balanceOf(user);
    masterChef.harvest(user, address(_stakeToken));

    emit Harvest(user, _stakeToken, space.balanceOf(user).sub(beforeReward));
  }
  ...
  function stakeNFT(
    ...
    isSpaceBoosterNftOK(_stakeToken, _nftAddress, _nftTokenId)
    nonReentrant
    onlyEOA
  {
  ...
  function _stakeNFT(
    ...
    require(
      ...
      "SpaceBooster::stakeNFT:: nft already staked"
    );
  ...
  function _unstakeNFT(address _stakeToken) internal {
    NFTStakingInfo memory toBeSentBackNft = userStakingNFT[_stakeToken][_msgSender()];
    require(toBeSentBackNft.nftAddress != address(0), "SpaceBooster::stakeNFT:: no nft staked");
  ...
  function stake(IERC20 _stakeToken, uint256 _amount)
    ...
    nonReentrant
  {
    require(_amount > 0, "SpaceBooster::stake::nothing to stake");
    ...
    if (address(_stakeToken) == address(space)) {
      masterChef.depositSpace(_msgSender(), _amount);
    } else {
  ...
  function _unstake(IERC20 _stakeToken, uint256 _amount) internal {
    require(_amount > 0, "SpaceBooster::_unstake::use harvest instead");
  ...
  function harvest(address[] calldata _stakeTokens) external whenNotPaused nonReentrant {
    for (uint256 i = 0; i < _stakeTokens.length; i++) {
      require(spaceboosterConfig.stakeTokenAllowance(_stakeTokens[i]), "SpaceBooster::harvest::bad stake token");
  ...
  /// @dev currently, the caller will be a master chef calling before a space lock
  function masterChefCall(
    address stakeToken,
    address userAddr,
    uint256 unboostedReward
    ...
    (, uint256 currentEnergy, uint256 boostBps) = spaceboosterConfig.energyInfo(
    ...
    // spaceboosterConfig.consumeEnergy(stakingNFT.nftAddress, stakingNFT.nftTokenId, extraReward);
  ...
  function _safeWrap(IERC20 _quoteBep20, uint256 _amount) internal {
    if (msg.value != 0) {
      require(address(_quoteBep20) == wNative, "SpaceBooster::_safeWrap:: baseToken is not wNative");
      require(_amount == msg.value, "SpaceBooster::_safeWrap:: value != msg.value");
