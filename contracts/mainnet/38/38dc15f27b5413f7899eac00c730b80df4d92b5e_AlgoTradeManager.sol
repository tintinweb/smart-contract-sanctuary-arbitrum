...
contract AlgoTradeManager is AlgoTradingHandler, GelatoRelayContext {
    ...
    */
    ...
    /**
     * @notice This function is used to create, update or cancel limit orders for gmx-v2
     * @dev It can be called by the creator
     * @param txData The transaction data object containing information of trader's trade
     * @param _swapArgs The structured data for a swap call on an extension of the enzyme contract
     */
    function executeLimitOrderV2FromCreator(
        ExecuteLimitOrderMetaDataV2 calldata txData,
        ExtensionArgs[] memory _swapArgs
    ) external isCreator {
        _executeLimitOrderInternalV2(txData, _swapArgs);
    }
    ...
    /**
     * @notice This function is used to cancel limit orders for gmx-v2
     * @dev It can be called by the creator
     * @param txData The transaction data object containing order keys
     */
    function executeCancelLimitOrderV2FromCreator(
        CancelLimitOrderMetaDataV2 calldata txData
    ) external isCreator {
        _executeCancelLimitOrderInternalV2(txData.orderKeys);
    }
     ...
     * @param v, @param r, and @param s, v, r, s parameters corresponding to transaction data object (Signed by backend signer)
     */
    function executeCancelLimitOrderV2(
        CancelLimitOrderMetaDataV2 calldata txData,
        uint8 v,
        bytes32 r,
        bytes32 s
    ...
    function executeForceTrade(
        ...
        (
            address comptrollerProxy_,
            address externalPosition
        ) = getAddressesFromVaultProxy();

        _swap(comptrollerProxy_, _swapArgs);
    ...
    /**
     * @notice This function is used to validate trader transaction details and determine
     * whether to process the limit order
     * @dev This function calls on TradingExtensionV2 contract to validate trade data
     * @param data encoded data containing fundmanager address and external position
     * @param addresses an array containing collateral token and market addresses
     * @param isLong position type
     * @return canExec This boolean variable indicates whether to copy this trade or not
     * @return message if canExec true, then success message will populate, otherwise, error codes will popultate
     * @return execPayload execution payload
     * @return action action enum id
     * @return orderCount no. of orders to process
     */
    function validateExecuteLimitOrderDataForGmxV2(
        bytes memory data, //abi.encode(_fundmanager, externalPosition)
        address[] memory addresses,
        bool isLong
    )
        public
    ...
    {
        (canExec, message, execPayload, action, orderCount) = tradingExtensionV2
            .validateExecuteLimitOrderDataForGmxV2(data, addresses, isLong);
    }

    /**
     * @notice This function is used to validate trader transaction details and determine
     * whether to cancel the order or not
     * @dev This function calls on TradingExtensionV2 contract to validate trade data
     * @param orderKeys encoded data containing fundmanager address, external position, and trader
     * @return canExec This boolean variable indicates whether to copy this trade or not
     * @return message if canExec true, then success message will populate, otherwise, error codes will popultate
     * @return execPayload execution payload
     */
    function validateCancelLimitOrderDataForGmxV2(
    ...
    {
        (canExec, message, execPayload) = tradingExtensionV2
            .validateCancelLimitOrderDataForGmxV2(orderKeys);
    }
     ...
     * @param _swapArgs The structured data for a swap call on an extension of the enzyme contract
     * @param orderKeys An array comprising order keys intended for the cancellation of specific limit orders
     */
    function closeOpenPositions(
        ...
        if (
            ...
            if (orderKeys.length > 0) {
                _executeCancelLimitOrderInternalV2(orderKeys);
            }
    ...
    function _executeTradeInternalV2(
        ...
        _callOnGmxV2(actionArgs, _comptrollerProxy, externalPosition, 0);
     ...
     * @param _swapArgs The structured data for a swap call on an extension of the enzyme contract
     */
    function _executeLimitOrderInternalV2(
        ExecuteLimitOrderMetaDataV2 calldata txData,
        ExtensionArgs[] memory _swapArgs
    ) private {
        (
        ...
        (
            ...
            uint256[] memory action,
            uint256 orderCount
        ) = validateExecuteLimitOrderDataForGmxV2(
                abi.encode(address(this), externalPosition),
                txData.addresses,
                txData.isLong
            );
    ...
    /**
     * @notice an internal function to cancel limit order
     * @param orderKeys The transaction object containing trader transaction data
     */
    function _executeCancelLimitOrderInternalV2(
        bytes32[] memory orderKeys
    ) private {
        (
        ...
        for (uint256 i; i < actionArgs.length; ++i) {
            _callOnGmxV2(actionArgs[i], comptrollerProxy, externalPosition, 5);
        }
    }
    ...
    function _callOnGmxV2(
        ...
        //action
        //0 - It refers to create order enum id
        //3 - It refers to create limit order enum id
        //4 - It refers to update limit order enum id
        //5 - It refers to cancel limit order enum id

        positionArgs = BaseStorage.ExtensionArgs({
            _extension: _externalPositionManager,
            _actionId: uint256(1), //Call On External Position
            _callArgs: abi.encode(externalPosition, action, actionArgs)
        });
    ...
    function _claimFundingFees(
        ...
        positionArgs = BaseStorage.ExtensionArgs({
            _extension: _externalPositionManager,
            _actionId: uint256(1), //Call On External Position
            _callArgs: abi.encode(externalPosition, 2, "") // '0' here refers to create order enum id
        });
    ...
    function _transferRelayFees() private {
    ...
    function getAddressesFromVaultProxy()
        private
        returns (address comptrollerProxy, address externalPosition)
    {
        comptrollerProxy = __getComptrollerProxyForVault(vaultProxy);
        externalPosition = IVault(vaultProxy).getActiveExternalPositions()[0];

        return (comptrollerProxy, externalPosition);
    }
    ...
    function validateSigner(
        ExecuteLimitOrderMetaDataV2 calldata metadata,
        uint8 v,
        bytes32 r,
        bytes32 s
    ) internal returns (bool) {
    ...
    function validateSigner(
        CancelLimitOrderMetaDataV2 calldata metadata,
        uint8 v,
        bytes32 r,
        bytes32 s
    ) internal returns (bool) {
...
import {Order} from "@gmx-synthetics/contracts/order/Order.sol";
...
interface ITradingExtensionV2 {
    ...
    function getSigner(
        AlgoTradingStorage.ExecuteLimitOrderMetaDataV2 calldata metadata,
        uint8 v,
        bytes32 r,
        bytes32 s,
        uint256 nonce,
    ...
    function getSigner(
        AlgoTradingStorage.CancelLimitOrderMetaDataV2 calldata metadata,
        uint8 v,
        bytes32 r,
        bytes32 s,
        uint256 nonce,
...
abstract contract AlgoTradingStorage is BaseStorage {
    ...
    struct ExecuteLimitOrderMetaDataV2 {
        address[] addresses; // array of collateralToken, marketAddress
        bool isLong;
        address account;
        bytes32 txHash;
    ...
    struct CancelLimitOrderMetaDataV2 {
        bytes32[] orderKeys;
        address account;
        bytes32 txHash;
        uint256 nonce;
