...
contract VNTRBondStakeDepository is Ownable {
    ...
    address public immutable VNTR; // intermediate reward token from treasury
    address public immutable sVNTR; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints VNTR when receives principle
    ...
    struct Bond {
        uint gonsPayout; // sVNTR gons remaining to be paid
        uint VNTRPayout; //VNTR amount at the moment of bond
    ...
    constructor ( 
        ...
        require( _VNTR != address(0) );
        VNTR = _VNTR;
        require( _sVNTR != address(0) );
        sVNTR = _sVNTR;
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 VNTR ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) VNTR
         */
        ...
        //TODO
        IERC20( VNTR ).approve( staking, payout );
        IStaking( staking ).stake( payout, address(this) );
        IStaking( staking ).claim( address(this) );
        uint stakeGons=ISVNTR(sVNTR).gonsForBalance(payout);

        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({ 
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            VNTRPayout: _bondInfo[ _depositor ].VNTRPayout.add( payout ),
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISVNTR(sVNTR).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sVNTR ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( VNTR ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice return bond info with latest sVNTR balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISVNTR(sVNTR).balanceForGons(info.gonsPayout);
    ...
    /**
     *  @notice calculate current ratio of debt to VNTR supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( VNTR ).totalSupply();
    ...
    /**
     *  @notice calculate amount of VNTR available for claim by depositor
     *  @param _depositor address
     *  @return pendingPayout_ uint
     */
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        uint percentVested = percentVestedFor( _depositor );
        uint payout = ISVNTR(sVNTR).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
