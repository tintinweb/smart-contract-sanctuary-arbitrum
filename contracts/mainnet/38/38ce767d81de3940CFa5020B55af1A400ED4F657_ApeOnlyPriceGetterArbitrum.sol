...
library ApeOnlyPriceGetterArbitrum {
    ...
    function getNativePrice() public view returns (uint) {
        (uint daiReserve, uint nativeReserve0, ) = IApePair(pairFor(WNATIVE, DAI)).getReserves();
        (uint nativeReserve1, uint usdcReserve, ) = IApePair(pairFor(WNATIVE, USDC)).getReserves();
        (uint usdtReserve, uint nativeReserve2, ) = IApePair(pairFor(WNATIVE, USDT)).getReserves();

        uint nativeTotal = nativeReserve0 + nativeReserve1 + nativeReserve2;
        uint usdTotal = daiReserve + usdcReserve + usdtReserve;
