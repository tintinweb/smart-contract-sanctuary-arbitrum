...
contract TradingContest is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, ITradingContest {
    ...
    IOmniStaking public lvlOmniStaking;
    ...
    function record(address _user, uint256 _value) external {
        ...
        if (
            ...
            uint256 _lvlOmniStakingAmount;
            if (lvlStaking != ILVLStaking(address(0))) {
                (_lvlAmount,) = lvlStaking.userInfo(_user);
            }
            if (daoStaking != IDaoStaking(address(0))) {
                (_lvlDAOAmount,,) = daoStaking.userInfo(_user);
            }
            if (lvlOmniStaking != IOmniStaking(address(0))) {
                _lvlOmniStakingAmount = lvlOmniStaking.stakedAmounts(_user);
            }

            uint256 _bonusRatio = (_lvlAmount + _lvlDAOAmount + _lvlOmniStakingAmount ) * uint256(BONUS_PER_TOKEN) / TOKEN_PRECISION;
            uint256 _bonusPoint = _value * _bonusRatio / BONUS_PRECISION;
            uint256 _point = _value + _bonusPoint;
            emit RecordAdded(_user, _value, _lvlDAOAmount, _lvlAmount + _lvlOmniStakingAmount, _point, currentBatch);
        }
    }
    ...
    function setLvlOmniStaking(address _lvlOmniStaking) external onlyOwner {
        require(_lvlOmniStaking != address(0), "Invalid address");
        lvlOmniStaking = IOmniStaking(_lvlOmniStaking);
        emit LvlOmniStakingSet(_lvlOmniStaking);
    }
