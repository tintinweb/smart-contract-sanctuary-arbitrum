...
contract CoinFlip is Common {
    ...
    uint32 constant houseEdgeFees = 10000;
    ...
    function CoinFlip_Play(
        ...
        if (numBets == 0) {
            if (isHeads) _depositBankroll(wager, tokenAddress);
            else _withdrawShares(wager, tokenAddress);
        } else {
    ...
    function CoinFlip_Refund() external nonReentrant {
        address msgSender = msg.sender;
        CoinFlipGame memory game = coinFlipGames[msgSender];
        if (game.requestID == 0) {
        ...
        emit CoinFlip_Refund_Event(msgSender, wagers, tokenAddress);
    }
        ...
        override
    {
        ...
        CoinFlipGame memory game = coinFlipGames[playerAddress];
        ...
        uint8[] memory coinFlip = new uint8[](numBets);
        uint256[] memory payouts = new uint256[](numBets);

        address tokenAddress = game.tokenAddress;
        uint256 multiplier;
        uint256 tempMultiplier;
        ...
        for (i = 0; i < numBets; i++) {
            ...
            if (coinFlip[i] == 1 && game.isHeads == true) {
                totalValue += int256((wager * tempMultiplier) / 10000);
                payout += (wager * multiplier) / 10000;
                payouts[i] = (wager * multiplier) / 10000;
                continue;
            }
...
Players store cryptocurrencies in their personal electronic wallets and are fully responsible for the security of their electronic wallets, as well as for signing transactions on blockchain networks.
