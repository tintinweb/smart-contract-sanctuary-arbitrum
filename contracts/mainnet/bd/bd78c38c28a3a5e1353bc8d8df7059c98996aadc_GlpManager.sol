...
contract GlpManager is ReentrancyGuard, Governable, IGlpManager {
    ...
    constructor() public {
        ...
        rewardTracker = IRewardTracker(0x4e971a87900b931fF39d1Aad67697F49835400b6);
    ...
    function getGlobalLongShortSize(address token) public view returns (bool, uint256) {
        uint256 longSize = 0;
        uint256 shortSize = 0;
        ...
        if (size > 0) {
            uint256 averagePrice = vault.globalShortAveragePrices(token);
            shortSize = size.div(averagePrice).mul(10 ** decimals);
        }

        longSize = vault.reservedAmounts(token);

        bool isNetLong = longSize > shortSize;
        uint256 netSize = isNetLong ? longSize.sub(shortSize) : shortSize.sub(longSize);

        return (isNetLong, netSize);
    }

    function getAumPerToken(address token) public view returns (uint256) {
        uint256 aum = 0;
        uint256 shortProfits = 0;
        ...
        uint256 price = false ? vault.getMaxPrice(token) : vault.getMinPrice(token);
        uint256 poolAmount = vault.poolAmounts(token);
        uint256 decimals = vault.tokenDecimals(token);
    ...
    function getNonStableAum(bool maximise) public view returns (uint256) {
        uint256 length = vault.allWhitelistedTokensLength();
        uint256 aum = aumAddition;
        uint256 shortProfits = 0;
    ...
    function getHedgePosition(address _account) public view returns (uint256) {
        address _collateralToken = 0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8;
        address _indexToken = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1;
        bool _isLong = false;

        (uint256 size, , uint256 averagePrice, , , , , ) = vault.getPosition(_account, _collateralToken, _indexToken, _isLong);

        uint256 hedgePositionETH = size.div(averagePrice);

        return hedgePositionETH;
    }

    function getLongPosition(address _account) public view returns (uint256) {
        address _indexToken = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1;

        uint256 nonStableAum = getNonStableAum(false);
        uint256 totalGlpSupply = IERC20(glp).totalSupply();
        uint256 stakedAmounts = rewardTracker.stakedAmounts(_account);

        uint256 price = vault.getMinPrice(_indexToken);
        uint256 longPositionETH = nonStableAum.div(price).mul(stakedAmounts).div(totalGlpSupply);

        return longPositionETH;
    }

    function getNetPosition(address _account) public view returns (bool, uint256) {
        uint256 longPositionETH = getLongPosition(_account);

        uint256 hedgePositionETH = getHedgePosition(_account);

        bool isNetLong = longPositionETH > hedgePositionETH;
        uint256 netPosition = isNetLong ? longPositionETH.sub(hedgePositionETH) : hedgePositionETH.sub(longPositionETH);

        return (isNetLong, netPosition);
    }
