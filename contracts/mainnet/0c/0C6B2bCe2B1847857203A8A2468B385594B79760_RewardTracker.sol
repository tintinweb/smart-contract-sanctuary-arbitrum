...
/**
    @notice Stake and reward pool for YFX.
*/
...
contract RewardTracker is IERC20, ReentrancyGuard, IRewardTracker, Governable {
    ...
    address[] public depositTokens;                                                         // list of allowed stake token
    mapping (address => bool) public isDepositToken;                                        // is allow stake
    // user => token => staked amount
    mapping (address => mapping (address => uint256)) public override depositBalances;      // user deposit balances
    mapping (address => uint256) public override stakedAmounts;
    // token => staked amount
    mapping (address => uint256) public totalDepositSupply;                                 // total deposited token amount
    ...
    bool public isInitialized;                                              // initialize flag
    bool public inPrivateTransferMode;
    bool public inPrivateStakingMode;
    bool public inPrivateClaimingMode;
    address public distributor;                                             // rewards distributor address
    mapping (address => bool) public isHandler;                             // handler whitelist

    uint256 public cumulativeRewardPerToken;
    mapping (address => uint256) public claimableReward;                    // user => claimable rewards
    mapping (address => uint256) public previousCumulatedRewardPerToken;    // user => available rewards base per staked

    mapping (address => uint256) public override cumulativeRewards;         // user => cumulative gained rewards, used for vest
    mapping (address => uint256) public override averageStakedAmounts;      // user => average Staked Amounts, used for vest

    event Claimed(address receiver, uint256 amount);
    event SetDepositToken(address depositToken, bool isDepositToken);
    ...
    /// @param _depositTokens, tokens allowed to deposit to stake
    /// @param _distributor, reward distributor contract address
    function initialize(
    ...
    /// @notice add or remove supported deposit token
    /// @param  _depositToken, token address
    /// @param  _isDepositToken, true for add, false for remove
    function setDepositToken(address _depositToken, bool _isDepositToken) external onlyGov {
        ...
        emit SetDepositToken(_depositToken, _isDepositToken);
    }
    ...
    /// @notice stake tokens for reward
    ...
    /// @notice stake tokens for reward for account
    ...
    /// @notice unstake and withdraw from the reward tracker
    /// @param _depositToken, deposit token
    /// @param _amount, amount
    function unstake(address _depositToken, uint256 _amount) external override nonReentrant returns(uint256){
        if (inPrivateStakingMode) { revert("RewardTracker: action not enabled"); }
        return _unstake(msg.sender, _depositToken, _amount, msg.sender);
    }

    /// @notice unstake and withdraw from the reward tracker for account
    ...
    function unstakeForAccount(address _account, address _depositToken, uint256 _amount, address _receiver) external override nonReentrant returns(uint256) {
        _validateHandler();
        return _unstake(_account, _depositToken, _amount, _receiver);
    }
    ...
    /// @notice claim reward for account
    /// @notice _account, account address
    /// @param _receiver, receiver address
    ...
    function claimable(address _account) public override view returns (uint256) {
        ...
        uint256 pendingRewards = IRewardDistributor(distributor).pendingRewards(supply);
        uint256 nextCumulativeRewardPerToken = cumulativeRewardPerToken.add(pendingRewards.mul(PRECISION).div(supply));
        return claimableReward[_account].add(
            stakedAmount.mul(nextCumulativeRewardPerToken.sub(previousCumulatedRewardPerToken[_account])
        ).div(PRECISION));
    }
    ...
    /// @notice claim reward, update reward information and send reward token to account
    /// @param _account, account address
    /// @param _receiver, receiver address
    ...
    /// @param _fundingAccount, send deposit token from which account
    /// @param _account, deposit for which account
    /// @param _depositToken, deposit token address
    /// @param _amount, amount
    function _stake(address _fundingAccount, address _account, address _depositToken, uint256 _amount) private {
        require(_amount > 0, "RewardTracker: invalid amount");
        require(isDepositToken[_depositToken], "RewardTracker: invalid deposit token");
    ...
    /// @param _amount, amount
    /// @param _receiver, receiver address
    function _unstake(address _account, address _depositToken, uint256 _amount, address _receiver) private returns(uint256){
        require(_amount > 0, "RewardTracker: invalid amount");

        _updateRewards(_account);

        uint256 depositBalance = depositBalances[_account][_depositToken];
        require(depositBalance >= _amount, "RewardTracker: amount exceeds depositBalance");
        depositBalances[_account][_depositToken] = depositBalance.sub(_amount);

        uint256 stakedAmount = stakedAmounts[_account];
        require(stakedAmounts[_account] >= _amount, "RewardTracker: amount exceeds stakedAmount");
    ...
    /// @notice update reward information
    /// @param _account, account address to update personal reward info if the address is not address(0)
    function _updateRewards(address _account) private {
        ...
        if (_account != address(0)) {
            ...
            if (accountReward > 0 && stakedAmounts[_account] > 0) {
