...
contract PolicyManager is IPolicyManager, ExtensionBase, GasRelayRecipientMixin {
    ...
    modifier onlyFundOwner(address _comptrollerProxy) {
        require(
            msg.sender == IVault(getVaultProxyForFund(_comptrollerProxy)).getOwner(),
    ...
    ) public ExtensionBase(_fundDeployer) GasRelayRecipientMixin(_gasRelayPaymasterFactory) {}
    ...
    function __policyHookRestrictsCurrentInvestorActions(
        PolicyHook _hook
