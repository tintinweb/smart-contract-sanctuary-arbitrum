...
import { AuthorizedAccessControl } from "../../protocol/security/AuthorizedAccessControl.sol";
...
contract Migration_20240515_0001 is Migration { // solhint-disable-line contract-name-camelcase

  address private constant SWAPROUTER_ADAPTER = 0xB70093eb4D6F707F96A9883f145BaAcEf5F44dB1;
  address private constant MARKETPLACE_PURCHASER = 0x98324ad145E23a75596b1b9ef6793BDBAff0a593;
  address private constant MARKETPLACE_VENDOR = 0x98442490F10F16e5F0e34FF5c127f3FD13E635d5;  
  ...
  function migrate() internal override onlyAdmin {
    MigrationLib.updateMarketplaceAdapter(_registry, Constants.AUSD, SWAPROUTER_ADAPTER);

    // revoke the old contracts
    address marketplaceVendor = address(_registry.getMarketplaceVendor());
    address marketplacePurchaser = address(_registry.getMarketplacePurchaser());

    address portfolio = address(_registry.getPortfolio());
    address market = address(_registry.getMarket());

    // update flash fee ignore list
    IFlashLender flashLender = _registry.getFlashLender();
    flashLender.setFeeWhitelist(marketplaceVendor, false);
    flashLender.setFeeWhitelist(marketplacePurchaser, false);    
    flashLender.setFeeWhitelist(MARKETPLACE_VENDOR, false);
    flashLender.setFeeWhitelist(MARKETPLACE_PURCHASER, false);

    // revoke the old contracts
    revokeAuthorization(portfolio, marketplaceVendor);
    revokeAuthorization(portfolio, marketplacePurchaser);
    revokeAuthorization(address(flashLender), marketplaceVendor);
    revokeAuthorization(address(flashLender), marketplacePurchaser);
    revokeAuthorization(market, marketplaceVendor);
    revokeAuthorization(market, marketplacePurchaser);

    // authorize the new contracts
    grantAuthorization(portfolio, MARKETPLACE_VENDOR);
    grantAuthorization(portfolio, MARKETPLACE_PURCHASER);
    grantAuthorization(address(flashLender), MARKETPLACE_VENDOR);
    grantAuthorization(address(flashLender), MARKETPLACE_PURCHASER);
    grantAuthorization(market, MARKETPLACE_VENDOR);
    grantAuthorization(market, MARKETPLACE_PURCHASER);

    // update the registry
    _registry.setAddress(keccak256("ambit.marketplace.vendor"), MARKETPLACE_VENDOR);
    _registry.setAddress(keccak256("ambit.marketplace.purchaser"), MARKETPLACE_PURCHASER);
  }
  ...
  function revokeAuthorization(address source, address dest) private {
    AuthorizedAccessControl authorization = AuthorizedAccessControl(source);
    authorization.revokeAuthorization(dest);
  }
}
...
library MigrationLib {
  ...
  function updateMarketplaceAdapter(IAddressRegistry registry, address token, address adapter) internal {
    IAssetStorage assetStorage = registry.getAssetStorage();
    IAssetStorage.Asset memory asset = assetStorage.getAsset(token, 0xFFFF);

    asset.marketplaceAdapter = adapter;

    assetStorage.setAsset(asset);
  }

  function updateOracle(IAddressRegistry registry, address token, address priceOracle) internal {
    IAssetStorage assetStorage = registry.getAssetStorage();
    IAssetStorage.Asset memory asset = assetStorage.getAsset(token, 0xFFFF);

    asset.priceOracle = priceOracle;
