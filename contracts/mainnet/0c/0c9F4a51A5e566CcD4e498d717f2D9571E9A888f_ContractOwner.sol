...
contract ContractOwner is EIP712Upgradeable, OwnableUpgradeable, Version {
    function initialize(
        address multisig,
        address _deployer,
        address _spotEngine,
    ...
    Verifier verifier;
    SpotAddProductCall[] spotAddProductCalls;
    PerpAddProductCall[] perpAddProductCalls;
    ...
    function addProducts() external onlyOwner {
        for (uint256 i = 0; i < spotAddProductCalls.length; i++) {
            SpotAddProductCall memory call = spotAddProductCalls[i];
        ...
        delete spotAddProductCalls;

        for (uint256 i = 0; i < perpAddProductCalls.length; i++) {
            PerpAddProductCall memory call = perpAddProductCalls[i];
        ...
        delete perpAddProductCalls;
    }

    function hasPendingAddProductCalls() public view returns (bool) {
        return perpAddProductCalls.length > 0 || spotAddProductCalls.length > 0;
    }
    ...
    function registerTransferableWallet(address wallet, bool _transferable)
        ...
        onlyOwner
    {
        endpoint.registerTransferableWallet(wallet, _transferable);
    }
