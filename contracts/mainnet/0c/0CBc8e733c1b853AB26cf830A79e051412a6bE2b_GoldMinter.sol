...
contract GoldMinter is GatewayGuarded, Ownable, ReentrancyGuard {
    ...
    function setGoldAddress(address _addr) external onlyOwner {
        goldAddress = _addr;
    }

    function withdrawTo(address _to) external onlyOwner returns (bool sent) {
    ...
    function claimWithSig(
        ...
        require(block.timestamp <= _deadline, "GoldMinter: signature expired");

        require(
            !openIdMintedStatus[_openId],
            "GoldMinter: openId has been used"
        );
        ...
        openIdMintedStatus[_openId] = true;
        mintedOpenIds[msg.sender].push(_openId);
