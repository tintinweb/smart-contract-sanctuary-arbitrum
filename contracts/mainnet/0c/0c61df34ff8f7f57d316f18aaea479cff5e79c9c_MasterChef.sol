...
interface IStake {
  ...
  function safeHokutoTransfer(address _account, uint256 _amount) external;
}
...
interface IMasterChef {
  ...
  function pendingHokuto(address _stakeToken, address _user) external view returns (uint256);
  ...
  function hokutoPerBlock() external view returns (uint256);
  ...
  function depositHokuto(address _for, uint256 _amount) external;

  function withdrawHokuto(address _for, uint256 _amount) external;
  ...
  function hokuto() external returns (IHOKUTO);
}
...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        // Gas optimization: this is chehokutor than requiring 'a' not being zero, but the
...
// MasterChef is the master of HOKUTO. He can make HOKUTO and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once HOKUTO is sufficiently
...
contract MasterChef is IMasterChef, Ownable, ReentrancyGuard {
  ...
  struct PoolInfo {
    ...
    uint256 lastRewardBlock; // Last block number that HOKUTO distribution occurs.
    uint256 accHokutoPerShare; // Accumulated HOKUTO per share, times 1e12. See below.
    uint256 depositFee;
  }

  // HOKUTO token.
  IHOKUTO public override hokuto;
  // Stake address.
  IStake public stake;
  // Dev address.
  ...
  // HOKUTO per block.
  uint256 public override hokutoPerBlock;
  ...
  // The block number when HOKUTO mining starts.
  ...
  event SetHokutoPerBlock(uint256 prevHokutoPerBlock, uint256 currentHokutoPerBlock);
  ...
  constructor(
    ...
    devAddr = _devAddr;
    refAddr = _refAddr;
    hokutoPerBlock = _hokutoPerBlock;
    ...
    // add HOKUTO pool
    pools.add(address(_hokuto));
    poolInfo[address(_hokuto)] = PoolInfo({
      allocPoint: 0,
  ...
  // Set HOKUTO per block.
  function setHokutoPerBlock(uint256 _hokutoPerBlock) external onlyOwner {
    massUpdatePools();
    uint256 prevHokutoPerBlock = hokutoPerBlock;
    hokutoPerBlock = _hokutoPerBlock;
    emit SetHokutoPerBlock(prevHokutoPerBlock, hokutoPerBlock);
  }
  ...
  // Update the given pool's HOKUTO allocation point. Can only be called by the owner.
  function setPool(
    address _stakeToken,
    uint256 _allocPoint,
    uint256 _depositFee
  ...
  function removePool(address _stakeToken) external override onlyOwner {
    require(_stakeToken != address(hokuto), "removePool: can't remove HOKUTO pool");
    ...
    poolInfo[_stakeToken].accHokutoPerShare = 0;
  ...
  // View function to see pending HOKUTOs on frontend.
  function pendingHokuto(address _stakeToken, address _user) external view override returns (uint256) {
    PoolInfo storage pool = poolInfo[_stakeToken];
    UserInfo storage user = userInfo[_stakeToken][_user];
    uint256 accHokutoPerShare = pool.accHokutoPerShare;
    ...
    if (block.number > pool.lastRewardBlock && totalStakeToken != 0) {
      ...
      uint256 hokutoReward = multiplier.mul(hokutoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      accHokutoPerShare = accHokutoPerShare.add(hokutoReward.mul(1e12).div(totalStakeToken));
    }
    return user.amount.mul(accHokutoPerShare).div(1e12).sub(user.rewardDebt);
  }
  ...
  function updatePool(address _stakeToken) public override {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 hokutoReward = multiplier.mul(hokutoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    hokuto.mint(devAddr, hokutoReward.mul(devBps).div(10000));
    hokuto.mint(address(stake), hokutoReward.mul(refBps).div(10000));
    hokuto.mint(address(stake), hokutoReward);
    pool.accHokutoPerShare = pool.accHokutoPerShare.add(hokutoReward.mul(1e12).div(totalStakeToken));
    pool.lastRewardBlock = block.number;
  }

  // Deposit token to MasterChef for HOKUTO allocation.
  function deposit(
    address _for,
    address _stakeToken,
    uint256 _amount
    ...
    require(_stakeToken != address(hokuto), "deposit: use depositHokuto instead");
    ...
    user.rewardDebt = user.amount.mul(pool.accHokutoPerShare).div(1e12);
  ...
  function withdraw(
    ...
    require(_stakeToken != address(hokuto), "withdraw: use withdrawHokuto instead");
  ...
  // Deposit HOKUTO to MasterChef.
  function depositHokuto(address _for, uint256 _amount)
    external
    override
    onlyPermittedTokenFunder(_for, address(hokuto))
    nonReentrant
  {
    PoolInfo storage pool = poolInfo[address(hokuto)];
    UserInfo storage user = userInfo[address(hokuto)][_for];

    if (user.fundedBy != address(0)) require(user.fundedBy == _msgSender(), "depositHokuto: bad sof");

    updatePool(address(hokuto));

    if (user.amount > 0) _harvest(_for, address(hokuto));
    if (user.fundedBy == address(0)) user.fundedBy = _msgSender();
    if (_amount > 0) {
      uint256 depositFeeAmount = _amount.mul(pool.depositFee).div(10000);
      if (depositFeeAmount > 0) {
        _amount = _amount.sub(depositFeeAmount);
        IERC20(address(hokuto)).safeTransferFrom(address(_msgSender()), devAddr, depositFeeAmount);
      }
      IERC20(address(hokuto)).safeTransferFrom(address(_msgSender()), address(this), _amount);
      user.amount = user.amount.add(_amount);
    }
    user.rewardDebt = user.amount.mul(pool.accHokutoPerShare).div(1e12);
    emit Deposit(_msgSender(), _for, address(hokuto), _amount);
  }

  // Withdraw HOKUTO
  function withdrawHokuto(address _for, uint256 _amount) external override nonReentrant {
    PoolInfo storage pool = poolInfo[address(hokuto)];
    UserInfo storage user = userInfo[address(hokuto)][_for];

    require(user.fundedBy == _msgSender(), "withdrawHokuto: only funder");
    require(user.amount >= _amount, "withdrawHokuto: not good");

    updatePool(address(hokuto));
    _harvest(_for, address(hokuto));

    if (_amount > 0) {
      user.amount = user.amount.sub(_amount);
      IERC20(address(hokuto)).safeTransfer(address(_msgSender()), _amount);
    }
    user.rewardDebt = user.amount.mul(pool.accHokutoPerShare).div(1e12);
    if (user.amount == 0) user.fundedBy = address(0);
    emit Withdraw(_msgSender(), _for, address(hokuto), user.amount);
  }

  // Harvest HOKUTO earned from a specific pool.
  ...
  // Harvest HOKUTO earned from pools.
  ...
  // Internal function to harvest HOKUTO
  function _harvest(address _for, address _stakeToken) internal {
    ...
    uint256 pending = user.amount.mul(pool.accHokutoPerShare).div(1e12).sub(user.rewardDebt);
    require(pending <= hokuto.balanceOf(address(stake)), "_harvest: wait what.. not enough HOKUTO");
    stake.safeHokutoTransfer(_for, pending);
  ...
  function _referralCallee(address _for, uint256 _pending) internal {
    ...
    stake.safeHokutoTransfer(address(refAddr), _pending.mul(refBps).div(10000));
  ...
  // This is a function for mining an extra amount of hokuto, should be called only by stake token caller contract (boosting purposed)
  function mintExtraReward(
    address _stakeToken,
    address _to,
    uint256 _amount
  ) external override onlyStakeTokenCallerContract(_stakeToken) {
    hokuto.mint(_to, _amount);
  }
}