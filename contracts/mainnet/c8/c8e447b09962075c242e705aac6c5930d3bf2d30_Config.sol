...
interface ITrainSwapNFT is IERC721Upgradeable, IERC721MetadataUpgradeable, IERC721EnumerableUpgradeable {
  // getter

  function trainNames(uint256 tokenId) external view returns (string calldata);
  ...
  function trainswapNFTToCategory(uint256 tokenId) external view returns (uint256);

  function categoryToTrainSwapNFTList(uint256 categoryId) external view returns (uint256[] memory);
  ...
  function getTrainNameOfTokenId(uint256 tokenId) external view returns (string memory);
...
interface ITrainBoosterConfig {
  ...
  function trainboosterNftAllowance(
...
contract Config is ITrainBoosterConfig, Ownable, ReentrancyGuard {
  using SafeERC20 for IERC20;
  using SafeMath for uint256;

  struct TrainBoosterNFTInfo {
    address nftAddress;
    uint256 tokenId;
  }

  struct TrainBoosterEnergyInfo {
    uint256 maxEnergy;
    uint256 currentEnergy;
    uint256 boostBps;
    uint256 updatedAt;
  ...
  struct TrainBoosterAllowance {
    address nftAddress;
    uint256 nftTokenId;
    bool allowance;
  }

  struct TrainBoosterAllowanceParams {
    address stakingToken;
    TrainBoosterAllowance[] allowance;
  }
  ...
  mapping(address => mapping(uint256 => TrainBoosterEnergyInfo)) public trainboosterEnergyInfo;
  ...
  mapping(address => mapping(address => mapping(uint256 => bool))) public trainboosterNftAllowanceConfig;
  ...
  event SetTrainBoosterNFTEnergyInfo(
    address indexed nftAddress,
    uint256 indexed nftTokenId,
    uint256 maxEnergy,
    uint256 currentEnergy,
  ...
  event SetTrainBoosterNFTAllowance(
  ...
  modifier onlyCaller() {
    require(callerAllowance[msg.sender], "TrainBoosterConfig::onlyCaller::only eligible caller");
    _;
  }

  /// @notice getter function for energy info
  /// @dev check if the trainbooster energy existed,
    ...
    )
  {
    ...
    // if there is no preset trainbooster energy info, use preset in category info
    ...
    TrainBoosterEnergyInfo memory trainboosterInfo = trainboosterEnergyInfo[_nftAddress][_nftTokenId];
    // if there is no preset trainbooster energy info, use preset in category info
    // presume that it's not a preminted nft
    if (trainboosterInfo.updatedAt == 0) {
      uint256 categoryId = ITrainSwapNFT(_nftAddress).trainswapNFTToCategory(_nftTokenId);
    ...
    return (trainboosterInfo.maxEnergy, trainboosterInfo.currentEnergy, trainboosterInfo.boostBps);
  }
  ...
  function consumeEnergy(
    ...
    require(_nftAddress != address(0), "TrainBoosterConfig::consumeEnergy::_nftAddress must not be address(0)");
    TrainBoosterEnergyInfo storage energy = trainboosterEnergyInfo[_nftAddress][_nftTokenId];

    if (energy.updatedAt == 0) {
      uint256 categoryId = ITrainSwapNFT(_nftAddress).trainswapNFTToCategory(_nftTokenId);
      CategoryEnergyInfo memory categoryEnergy = categoryEnergyInfo[_nftAddress][categoryId];
      require(categoryEnergy.updatedAt != 0, "TrainBoosterConfig::consumeEnergy:: invalid nft to be updated");
  ...
  /// @param _param a TrainBoosterNFTParams {nftAddress, nftTokenId, maxEnergy, boostBps}

  function setExternalTokenEnergyInfo(address _externalNft, CategoryEnergyInfo calldata _param) external onlyOwner {
    require(_externalNft != address(0), "TrainBoosterConfig::externalNft::_externalNft must not be address(0)");
  ...
  function setStakeTokenAllowance(address _stakeToken, bool _isAllowed) external onlyOwner {
    require(_stakeToken != address(0), "TrainBoosterConfig::setStakeTokenAllowance::_stakeToken must not be address(0)");
  ...
  function setCallerAllowance(address _caller, bool _isAllowed) external onlyOwner {
    require(_caller != address(0), "TrainBoosterConfig::setCallerAllowance::_caller must not be address(0)");
    callerAllowance[_caller] = _isAllowed;

    emit SetCallerAllowance(_caller, _isAllowed);
  }

  /// @notice A function for setting trainbooster NFT energy info as a batch
  /// @param _params a list of TrainBoosterNFTParams [{nftAddress, nftTokenId, maxEnergy, boostBps}]
  function setBatchTrainBoosterNFTEnergyInfo(TrainBoosterNFTParams[] calldata _params) external onlyOwner {
    for (uint256 i = 0; i < _params.length; ++i) {
      _setTrainBoosterNFTEnergyInfo(_params[i]);
    }
  }

  /// @notice A function for setting trainbooster NFT energy info
  /// @param _param a TrainBoosterNFTParams {nftAddress, nftTokenId, maxEnergy, boostBps}
  function setTrainBoosterNFTEnergyInfo(TrainBoosterNFTParams calldata _param) external onlyOwner {
    _setTrainBoosterNFTEnergyInfo(_param);
  }

  /// @dev An internal function for setting trainbooster NFT energy info
  /// @param _param a TrainBoosterNFTParams {nftAddress, nftTokenId, maxEnergy, boostBps}
  function _setTrainBoosterNFTEnergyInfo(TrainBoosterNFTParams calldata _param) internal {
    trainboosterEnergyInfo[_param.nftAddress][_param.nftTokenId] = TrainBoosterEnergyInfo({
      maxEnergy: _param.maxEnergy,
    ...
    emit SetTrainBoosterNFTEnergyInfo(
      _param.nftAddress,
      _param.nftTokenId,
      _param.maxEnergy,
      _param.maxEnergy,
  ...
  function setStakingTokenCategoryAllowance(CategoryAllowanceParams calldata _param) external onlyOwner {
    for (uint256 i = 0; i < _param.allowance.length; ++i) {
      require(
        ...
        "TrainBoosterConfig::setStakingTokenCategoryAllowance:: bad staking token"
      );
  ...
  /// @param _param a TrainBoosterAllowanceParams {stakingToken, [{nftAddress, nftTokenId,allowance;}]}
  function setStakingTokenTrainBoosterAllowance(TrainBoosterAllowanceParams calldata _param) external onlyOwner {
    for (uint256 i = 0; i < _param.allowance.length; ++i) {
      require(
        stakeTokenAllowance[_param.stakingToken],
        "TrainBoosterConfig::setStakingTokenTrainBoosterAllowance:: bad staking token"
      );
      trainboosterNftAllowanceConfig[_param.stakingToken][_param.allowance[i].nftAddress][
        _param.allowance[i].nftTokenId
      ] = _param.allowance[i].allowance;

      emit SetTrainBoosterNFTAllowance(
        _param.stakingToken,
        _param.allowance[i].nftAddress,
        _param.allowance[i].nftTokenId,
        _param.allowance[i].allowance
  ...
  /// @dev if not support when checking with token, need to try checking with category level (categoryNftAllowanceConfig) as well since there should not be trainboosterNftAllowanceConfig in non-preminted nft
  function trainboosterNftAllowance(
    address _stakeToken,
    address _nftAddress,
    uint256 _nftTokenId
  ) external view override returns (bool) {
    if (!trainboosterNftAllowanceConfig[_stakeToken][_nftAddress][_nftTokenId]) {
      CategoryEnergyInfo memory externalboosterInfo = externalboosterEnergyInfo[_nftAddress];
