...
contract BonusDistributor is IRewardDistributor, ReentrancyGuard, Governable {
    ...
    function updateRewardTracker(address _rewardTracker) external onlyGov {
        rewardTracker = _rewardTracker;
        emit UpdateRewardTracker(_rewardTracker);
    }
    ...
    function setBonusMultiplier(uint256 _bonusMultiplierBasisPoints) external onlyGov {
        require(lastDistributionTime != 0, "BonusDistributor: invalid value");
    ...
    function pendingRewards(uint256 supply) public view override returns (uint256) {
        if (block.timestamp == lastDistributionTime) {
            return 0;
        }

        uint256 timeDiff = block.timestamp.sub(lastDistributionTime);
        uint256 amount = timeDiff.mul(supply).mul(bonusMultiplierBasisPoints).div(BASIS_POINTS_DIVISOR).div(BONUS_DURATION);
    ...
    function distribute(uint256 supply) external override returns (uint256) {
        ...
        uint256 amount = pendingRewards(supply);
        if (lastDistributionTime < block.timestamp){
            lastDistributionTime = block.timestamp;
        }
