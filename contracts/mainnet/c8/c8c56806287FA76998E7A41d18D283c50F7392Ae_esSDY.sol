...
/**
  * @title esSDY
  * @author Steadefi
  * @notice Escrowed version of the SDY token, obtainable by converting SDY to esSDY.
  * Not transferrable except for whitelisted to/from addresses. Can be redeemed for SDY through
  * a locked redemption process. Can be allocated to approved plugins for various utilities.
*/
contract esSDY is ERC20, Ownable {
  ...
  // Addresses allowed to send/receive esSDY
  ...
  /**
   * @notice Only allow TokenManager
  */
  modifier onlyTokenManager() {
    if (msg.sender != tokenManager) revert Errors.OnlyTokenManagerAllowed();
    _;
  }

  /* ====================== CONSTRUCTOR ====================== */

  constructor() ERC20("Escrowed Steadefi", "esSDY") Ownable(msg.sender) {
  ...
  */
   ...
   * @param index index of whitelist to return
  ...
  */
  ...
  /**
   * @notice Returns if "account" is allowed to send/receive esSDY
  ...
  */
  ...
  /**
   * @notice Mints esSDY tokens
   * @param to address to mint tokens to
   * @param amount amount of tokens to mint in uint256
  */
  ...
  /**
   * @notice Burn caller's esSDY tokens
   * @param amount amount of tokens to burn in uint256
  */
  ...
  /**
   * @notice Transfer tokens to a specified address, checking against whitelist
    * @param to address to transfer tokens to
    * @param value amount of tokens to transfer in uint256
  */
  ...
  /**
   * @notice Transfer tokens from one address to another, checking against whitelist
  ...
  */
  function _beforeTokenTransfer(address from, address to, uint256 /* _amount */) internal view {
    if (from != address(0) && !_transferWhitelist.contains(from) && !_transferWhitelist.contains(to))
   ...
   * @param account address to be added or removed from transfer whitelist
   * @param add boolean to determine adding or removal of address to whitelist
  */
  function updateTransferWhitelist(address account, bool add) external onlyOwner {
    if (account == address(this)) revert Errors.InvalidUpdateTransferWhitelistAddress();
  ...
  /**
   * @notice Updates the tokenManager address
   * @param newTokenManager address of tokenManager
  */
  function updateTokenManager(address newTokenManager) external onlyOwner {
