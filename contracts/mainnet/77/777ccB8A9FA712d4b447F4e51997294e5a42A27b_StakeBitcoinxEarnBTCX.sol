...
contract StakeBitcoinxEarnBTCX is ReentrancyGuard {
    ...
    uint256 public baseFee = 500000000000000; //wei: 0.0005 ETH

    uint256 public penalty = 30;
    ...
    uint256 public maxLimitPerUser = 100000000000000000000000000;
    uint256 public rewardPerBlock = 120000000000000000; //wei BTCX Per 1M BTCX (2BTCX per block/1M): 40%= 40000000000000000
    ...
    function doStake(uint256 amount) external nonReentrant {
        ...
        require(
            stakeBalances[msg.sender] <= maxLimitPerUser,
            "Staking Limit per USER is reached"
        );
    ...
    function unStakeToken(uint256 amount) external payable nonReentrant {
        ...
        if (block.timestamp < lockedDurations[msg.sender]) {
            ...
            ERC20(stakeToken).transfer(
                msg.sender,
                stakeBalances[msg.sender] - penaltyAmount
            );
            emit UnStaked(
                msg.sender,
                stakeBalances[msg.sender] - penaltyAmount
    ...
    function getUserRewardPaidBalances(
        address _address
    ) public view returns (uint256) {
        return rewardPaidBalances[_address];
    }

    function getUserLockedDurations(
        address _address
    ) public view returns (uint256) {
        return lockedDurations[_address];
    }

    function getTotalRewardTokenInSmartContract()
        public
        view
        returns (uint256)
    {
    ...
    function earned(address account) public view returns (uint256) {
        ...
        require(
            block.timestamp > lastStakedDates[account],
            "Incorrect Timestamp"
        );
    ...
    function getAllRewardEarnedByUser(
        address account
    ) public view returns (uint256) {
