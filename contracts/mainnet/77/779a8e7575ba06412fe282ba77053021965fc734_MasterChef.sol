...
contract Whitelist is Ownable {
    ...
    function toggleNftWhitelist(address _address, bool _toggle) public onlyOwner {
...
contract MasterChef is Ownable, ReentrancyGuard, Whitelist {
    ...
    // Bonus muliplier for early lp makers.
    ...
    mapping(uint256 => uint256) public nftBoostRate;
    // mapping(uint256 => uint256) public nftBoostRateByTokenId;
    ...
    constructor(
        ...
        toggleNftWhitelist((address)(_boosterNFT), true);
    }
    ...
    function getBoost(address _account, uint256 _pid) public view returns (uint256) {
        ...
        return getNftBoostRate(nftDetail.tokenId);
    }
    ...
    function getNftBoostRate(uint256 _nftId) public view returns (uint256) {
        return nftBoostRate[_nftId] > 0 ? nftBoostRate[_nftId]: 10;
    }

    function setNftBoostRate(uint256 _nftId, uint256 _boostRate) public onlyOwner{
        require (_boostRate >= 10 && _boostRate <= 100, "Please enter correct boost value");

        nftBoostRate[_nftId] = _boostRate ;
    }

    function setPpPerBloc(uint256 _tokenPerBloc) external onlyOwner {
      tokenPerBlock = _tokenPerBloc;
    }
