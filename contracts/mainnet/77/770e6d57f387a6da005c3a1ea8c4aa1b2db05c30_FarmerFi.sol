...
contract FarmerFi is Context, IERC20, Ownable {
   ...
   constructor(address _router) public {
    ...
    // Set the Sushiswap Router v2 address for Arbitrum One
    IUniswapV2Router02 _sushiRouter = IUniswapV2Router02(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
    
    // Create a Uniswap pair for this new token
    uniswapV2Pair = IUniswapV2Factory(_sushiRouter.factory())
        .createPair(address(this), _sushiRouter.WETH());
    
    // Set the rest of the contract variables
    uniswapV2Router = _sushiRouter;
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) { // 100000000000
    ...
    /// @return tFee
    /// @return tLiquidity
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) { // 100000000000
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) { // 100000000000
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) { //100000000000
    ...
    function _transfer(
        address from,
        address to,
        uint256 amount // 100000000000 = 100 SALMOON
    ) private {
    ...
    // @param {tokenAmount}
    // @param {ethAmount}
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private { //100 tokens,1 eth
