...
import { ICamelotV3Factory, ICamelotV3Pool } from "../../../shared/interfaces/ICamelotV3.sol";
import {IERC20} from "../../../shared/interfaces/IERC20.sol";
import { LibExchangeHelper, ReserveInfo } from "./LibExchangeHelper.sol";

struct CamelotV3SwapParams {
    address router;
    address recipient;
    address tokenIn;
    address tokenOut;
...
library LibCamelotV3 {
    function getPoolByPair (address factory, address token0, address token1) internal view returns(address poolAddress) {
        poolAddress = ICamelotV3Factory(factory).poolByPair(token0, token1);
    }
    function getPool (address factory, address token0, address token1) internal view returns(address poolAddress) {
        poolAddress = getPoolByPair(factory, token0, token1);
    }
    function vaultAddress (address factory) internal view returns (address vault) {
        vault = ICamelotV3Factory(factory).vaultAddress();
    }
    function getReserves (address poolAddress) internal view returns (ReserveInfo memory reserves) {
        reserves.reserve0 = IERC20(ICamelotV3Pool(poolAddress).token0()).balanceOf(poolAddress);
        reserves.reserve1 = IERC20(ICamelotV3Pool(poolAddress).token1()).balanceOf(poolAddress);
        reserves.ts = block.timestamp;
    }
    function swap (CamelotV3SwapParams memory swapParams) internal returns (uint amount) {
        // amount = ICamelotV3Router(swapParams).swap()
    }
}
...
import {LibUniswapV2Helper, LibUniswapV3Helper, UniswapV2SwapParams, UniswapV3SwapParams, IUniswapV3Pool} from "./LibUniswapHelper.sol";

import {TokenInfo, LibTokenHelper } from "../TokenHelper/LibTokenHelper.sol";
import { LibInitializer } from "../../../shared/libraries/LibInitializer.sol";
import {CamelotV3SwapParams, LibCamelotV3 } from "./LibCamelotExchange.sol";
import {ICamelotV3Pool} from "../../../shared/interfaces/ICamelotV3.sol";


import "hardhat/console.sol";

error InvalidExchangePoolRangeValues(uint start, uint stop);
...
enum ExchangeType {
    UNISWAP_V2, UNISWAP_V3, BALANCER_V2, CAMELOT_V3
}
...
struct UniswapPoolParams {
    uint poolId;
    ReserveInfo reserves;
}
library LibExchangeHelper {
    bytes32 constant STORAGE_POSITION = keccak256("fraktal.protocol.exchange.helper.storage");
    ...
    function addExchange (ExchangeInfo memory _exchange) internal {
        ...
        exchange.quoter = _exchange.quoter;

        diamondStorage().exchangesCount++;
    }
    function addExchanges(ExchangeInfo[] memory exchanges) internal {
        uint len = exchanges.length;
    ...
    function getExchangePools (uint[] memory _ids) internal view returns (ExchangePool[] memory exchangePools) {
        uint len = _ids.length;
        exchangePools = new ExchangePool[](len);

        for (uint i = 0; i < len; i++) {
            exchangePools[i] = diamondStorage().exchangePools[_ids[i]];
        }
    }

    function getExchangePoolsByRange (uint start, uint stop) internal view returns (ExchangePool[] memory exchangePools) {
        if (start >= stop) revert InvalidExchangePoolRangeValues(start, stop);
        console.log("nothing");
        uint len = stop - start;
        exchangePools = new ExchangePool[](len);

        for (uint i = 0; i < len; i++) {
            exchangePools[i] = diamondStorage().exchangePools[start + i];
        }
    }
    ...
    function addExchangePools(ExchangePool[] memory ExchangePools) internal {
        uint len = ExchangePools.length;

        for (uint i = 0; i < len; i++) {
            addExchangePool(ExchangePools[i]);
        }
    }

    function getExchangePoolsCount () internal view returns (uint count) {
        count = diamondStorage().exchangePoolsCount;
    }

    function  getExchangePoolReserves (uint id) internal view returns (uint poolId, uint reserve0, uint reserve1) {
        ExchangePool memory pool = getExchangePool(id);
        if (pool.poolType == ExchangePoolType.UNISWAP_V2_PAIR) {
            LibUniswapV2Helper.getReserves(pool.pool);
        } else if (pool.poolType == ExchangePoolType.UNISWAP_V3_POOL) {
            LibUniswapV3Helper.getReserves(IUniswapV3Pool(pool.pool));

        } else if (pool.poolType == ExchangePoolType.CAMELOT_V3_POOL) {
            LibCamelotV3.getReserves(pool.pool);
        }
    }
    ...
    function swap (SwapParams memory params) internal returns (uint amount) {
        ...
        if (exchange.exchangeType == ExchangeType.UNISWAP_V2) {
        ...
        else if (exchange.exchangeType == ExchangeType.CAMELOT_V3) {
            CamelotV3SwapParams memory swapParams = CamelotV3SwapParams({
                router: exchange.router,
                recipient: params.recipient,
                tokenIn: LibTokenHelper.getToken(params.tokenIn).token,
                tokenOut: LibTokenHelper.getToken(params.tokenOut).token,
            ...
            return LibCamelotV3.swap(swapParams);
        } 
        else {
...
import {Util} from "../../../shared/libraries/Util.sol";
import {LibTokenHelper, TokenInfo} from "../TokenHelper/LibTokenHelper.sol";
import {IERC20} from "../../../shared/interfaces/IERC20.sol";
import {IQuoterV2} from "@uniswap/v3-periphery/contracts/interfaces/IQuoterV2.sol";

import {
    LibExchangeHelper, ExchangePool,ExchangePoolType, ExchangeInfo
} from "./LibExchangeHelper.sol";

import  {LibUniswapV2Helper, LibUniswapV3Helper, ReserveInfo } from "./LibUniswapHelper.sol";

struct MarketPoolParams {
    uint poolId;

}

struct QuoteInParams {
    address quoter;
    address tokenIn;
    address tokenOut;
    uint256 amountIn;
...
struct MassMarketsArbitrageParams {
    string ticker;
}
struct MarketData {
    string ticker;
...
error DifferentPools(uint _pool0, uint _pool1);

library LibMarketHelper {
    function getMarketData (ExchangePool memory pool) internal view returns(
        ReserveInfo memory reserves
    ) {
        // reserves and swap amounts
        ExchangeInfo memory exchange = LibExchangeHelper.getExchange(pool.exchangeId);
        if (pool.poolType == ExchangePoolType.UNISWAP_V2_PAIR ) {
            reserves = LibUniswapV2Helper.getReserves(pool.pool);
        } else if (pool.poolType == ExchangePoolType.UNISWAP_V3_POOL ) {
            reserves = LibUniswapV3Helper.getReserves(IUniswapV3Pool(pool.pool));
        }  else if (pool.poolType == ExchangePoolType.CAMELOT_V3_POOL) {
    ...
    function getMarketDataById (uint _poolId) internal view returns (uint poolId, ReserveInfo memory reserves) {
        ExchangePool memory pool = LibExchangeHelper.getExchangePool(_poolId);
        (reserves) = getMarketData(pool);
        poolId = _poolId;
    }
    // function checkArbitrageFromMarkets (MassMarketsArbitrageParams[] memory params) internal view returns (bool hasArb, bytes memory _arbData) {
        
    //     uint marketInc = 0;
    //     uint marketLen = params.markets.length;

    //     for (marketInc; marketInc < marketLen; marketInc) {

    //     }
    // }
    function getExchangeFromPoolId (uint _id) internal view returns(ExchangeInfo memory exchange) {
        ExchangePool memory pool = LibExchangeHelper.getExchangePool(_id);
        exchange = LibExchangeHelper.getExchange(pool.exchangeId);
    }

    function buildMarkets () internal view returns (MarketData[] memory markets) {
        // ExchangePool[] pools = LibExchangeHelper.getExchangePoolsAll();
        uint poolsLen = LibExchangeHelper.getExchangePoolsCount();
        uint i;
        
        ExchangePool[] memory pools = new ExchangePool[](poolsLen);

        markets = new MarketData[](poolsLen);
        ...
        for (i; i < poolsLen; i++) {
            ExchangePool memory pool = LibExchangeHelper.getExchangePool(i);
            
            uint poolId = i;
            uint token0Id = pool.token0;
            uint token1Id = pool.token1;
            
            token0 = LibTokenHelper.getToken(token0Id);
            token1 = LibTokenHelper.getToken(token1Id);

            string memory ticker = Util.getTicker(
                token0.token,
                token1.token
            );

            (uint reserve0, uint reserve1,) = LibExchangeHelper.getExchangePoolReserves(i);
            markets[i] = MarketData({
                ticker: ticker,
                marketId: keccak256(abi.encode(token0, token1)),
                poolId: poolId,
                reserve0: reserve0,
                reserve1: reserve1,
                price: 0
    ...
    function checkArbitrage (uint _pool0, uint _pool1, uint _tokenIn) internal returns (bool hasArb, FlashParams memory flashParams) {
        ExchangePool memory pool0 = LibExchangeHelper.getExchangePool(_pool0);
        ExchangePool memory pool1 = LibExchangeHelper.getExchangePool(_pool1);

        if (pool0.token0 == pool1.token0 && pool0.token1 == pool1.token1) revert DifferentPools(_pool0, _pool1);
        TokenInfo memory token0 = LibTokenHelper.getToken(pool0.token0);
        TokenInfo memory token1 = LibTokenHelper.getToken(pool0.token1);
        address tokenIn;
        address tokenOut;

        bool zeroForOne = _tokenIn == pool0.token0;

        if (_tokenIn == pool0.token0) {
            tokenIn = token0.token;
            tokenOut = token1.token;
        } else {
            tokenIn = token1.token;
            tokenOut = token0.token;
           
        }
        ExchangeInfo memory exchange0 = LibExchangeHelper.getExchange(pool0.exchangeId);
        ExchangeInfo memory exchange1 = LibExchangeHelper.getExchange(pool1.exchangeId);

        QuoteInParams memory quoteParams0;
        QuoteInParams memory quoteParams1;

        ReserveInfo memory reserves0;
        ReserveInfo memory reserves1;

        quoteParams0.tokenIn = tokenIn;
        quoteParams0.tokenOut = tokenOut;
        quoteParams0.quoter = exchange0.quoter;
        quoteParams0.fee = pool0.fee;

        quoteParams1.tokenIn = tokenIn;
        quoteParams1.tokenOut = tokenOut;
        quoteParams1.quoter = exchange1.quoter;
        quoteParams1.fee = pool1.fee;

        uint amountIn;

        if (pool0.poolType == ExchangePoolType.UNISWAP_V2_PAIR) {
            reserves0 = LibUniswapV2Helper.getReserves(pool0.pool);

            if (zeroForOne) amountIn = reserves0.reserve0 / 200;
            else amountIn = reserves0.reserve1 / 200;

            quoteParams0.amountIn = amountIn;

        } else if (pool0.poolType == ExchangePoolType.UNISWAP_V3_POOL) {
            reserves0 = LibUniswapV3Helper.getReserves(IUniswapV3Pool(pool1.pool));
            if (zeroForOne) amountIn = reserves0.reserve0 / 200;
            else amountIn = reserves0.reserve1 / 200;
        ...
        } else if (pool0.poolType == ExchangePoolType.CAMELOT_V3_POOL) {
            
        }
        (quoteParams1.amountIn,,,) = _quoteV3In(quoteParams0);

        (uint arbAmountOut,,,) = _quoteV3In(quoteParams1);

        hasArb = quoteParams0.amountIn < arbAmountOut;
        if (hasArb) {

            return (hasArb, flashParams);
        }
        flashParams = FlashParams({
            pool0: _pool0,
            pool1: _pool1,
            tokenIn: LibTokenHelper.getTokenId(tokenIn),
            tokenOut: LibTokenHelper.getTokenId(tokenOut), 
            amountIn: quoteParams0.amountIn,
            fee1: pool1.fee
        });
        // _data = abi.encode(flashParams, (FlashParams));

    }
    function _quoteV3In (QuoteInParams memory params) internal returns (
            uint256 amountOut,
            uint160 sqrtPriceX96After,
            uint32 initializedTicksCrossed,
            uint256 gasEstimate
        ...
        (amountOut, sqrtPriceX96After, initializedTicksCrossed, gasEstimate) = IQuoterV2(
            params.quoter).quoteExactInputSingle(quoterParams);

    }

    function _quoteOut (QuoteOutParams memory params) internal returns (
        uint256 amount,
        uint160 sqrtPriceX96After,
        uint32 initializedTicksCrossed,
        uint256 gasEstimate
    ) {
        IQuoterV2.QuoteExactOutputSingleParams memory quoterParams = IQuoterV2.QuoteExactOutputSingleParams({
            tokenIn: params.tokenIn,
            tokenOut: params.tokenOut,
        ...
        (amount, sqrtPriceX96After, initializedTicksCrossed, gasEstimate) = IQuoterV2(params.quoter).quoteExactOutputSingle(quoterParams);
  
    }
    function checkArbV3toV3 (
        address factory0,
        address factory1,
        address quoter0,
        address quoter1,
        ...
        uint24 fee0,
        uint24 fee1
    ) external returns(bool hasArb, uint inAmount, uint outAmount, bytes memory data) {
        QuoteInParams memory inParams;
        QuoteInParams memory outParams;
        address poolAddress = IUniswapV3Factory(factory0).getPool(tokenIn, tokenOut, fee0);
        IUniswapV3Pool pool0 = IUniswapV3Pool(poolAddress);
        poolAddress = IUniswapV3Factory(factory1).getPool(tokenIn, tokenOut, fee1);
        IUniswapV3Pool pool1 = IUniswapV3Pool(poolAddress);

        uint amountIn = IERC20(tokenIn).balanceOf(address(pool0));

        inParams.quoter = quoter0;
        inParams.tokenIn = tokenIn;
        inParams.tokenOut = tokenOut;
        inParams.amountIn = amountIn;
        inParams.fee = fee0;

        
        (uint amountOut,,,) = _quoteV3In(inParams);
        outParams.quoter = quoter1;
        outParams.tokenIn = tokenOut;
        outParams.tokenOut = tokenIn;
        outParams.amountIn = amountOut;
        outParams.fee = fee1;

        (uint amountOut1,,,) = _quoteV3In(outParams);
        inAmount = amountIn;
        outAmount = amountOut1;

        if (amountIn < amountOut1) {
            hasArb = true;
            abi.encode(
                pool0, pool1, amountIn
            );
        }
    }
...
import {LibExchangeHelper, ReserveInfo, ExchangePool, ExchangePoolType} from "./LibExchangeHelper.sol";
...
error UnkownPoolType(ExchangePoolType poolType);
...
library LibUniswapV3Helper {
    ...
    function getReserves (IUniswapV3Pool pool) internal view returns(ReserveInfo memory reserves) {
        IERC20 token0 = IERC20(pool.token0());
        IERC20 token1 = IERC20(pool.token1());

        reserves.reserve0 = token0.balanceOf(address(token0));
        reserves.reserve1 = token1.balanceOf(address(token1));
        reserves.ts = block.timestamp;
    }
...
library LibUniswapHelper {
    function getReserves (ExchangePool memory pool) internal returns(ReserveInfo memory reserves) {
        if (pool.poolType == ExchangePoolType.UNISWAP_V2_PAIR) {
            reserves = LibUniswapV2Helper.getReserves(pool.pool);
        } else if (pool.poolType == ExchangePoolType.UNISWAP_V3_POOL) {
            reserves = LibUniswapV3Helper.getReserves(IUniswapV3Pool(pool.pool));
            ...
            revert UnkownPoolType(pool.poolType);
        }
    }
...
import {LibExchangeHelper, ExchangePool, ReserveInfo} from "./LibExchangeHelper.sol";
import {MarketData, LibMarketHelper} from "./LibMarketHelper.sol";

contract MarketHelper {
    constructor () {

    }
    function getMarketData (ExchangePool memory pool) external view returns (ReserveInfo memory reserves) {
        reserves = LibMarketHelper.getMarketData(pool);
    }
    function getMarketDataById (uint _id) external view returns (uint id, ReserveInfo memory reserves) {
        (id, reserves) = LibMarketHelper.getMarketDataById(_id);
    }
    function buildMarkets () external view returns (MarketData[] memory marketData) {
        marketData = LibMarketHelper.buildMarkets();
    }
...
interface IAlgebraPool {
  ...
  function positions (bytes32) external returns(Position memory);
...
interface ICamelotV3Factory is IAlgebraFactory {

}

interface ICamelotV3Pool is IAlgebraPool {
...
import {IERC20Meta} from "../interfaces/IERC20.sol";
...
library Util {
    ...
    function sortTokens (address _t0, address _t1) internal pure returns (address token0, address token1) {
        if (_t0 < _t1) {
            token0 = _t0;
            token1 = _t1;
        } else {
    ...
    function getTicker (address _token0, address _token1) internal view returns (string memory ticker) {
        (address token0, address token1) = sortTokens(_token0, _token1);
        ticker = string.concat(string.concat(IERC20Meta(token0).symbol(), "_"), IERC20Meta(token1).symbol());
    } 
