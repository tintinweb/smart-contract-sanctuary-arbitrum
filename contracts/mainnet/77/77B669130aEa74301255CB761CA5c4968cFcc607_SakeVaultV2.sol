...
contract SakeVaultV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    uint256 private MCPID;
    uint256 private MAX_BPS;
    uint256 private MAX_LEVERAGE;
    uint256 private MIN_LEVERAGE;
    ...
    function liquidatePosition(uint256 _positionId, address _user) external nonReentrant {
        ...
        IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(VaultkaHandler, lendingRepayAmount);
        lendingRepayAmount = IHandler(VaultkaHandler).uniswapExecution(lendingRepayAmount, strategyAddresses.USDC, USDCe, true);

        IERC20Upgradeable(USDCe).safeIncreaseAllowance(strategyAddresses.water,lendingRepayAmount);
        IWater(strategyAddresses.water).repayDebt(_userInfo.leverageAmount, lendingRepayAmount);
