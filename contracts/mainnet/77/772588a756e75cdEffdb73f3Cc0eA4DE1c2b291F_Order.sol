...
contract Owner {
    ...
    function notContract(address account) view public returns(bool) {
        uint size;
        assembly {
...
interface ITemplar {
    ...
    function getTokenInfo(uint256 tokenId) external view returns (uint256 fightAt, uint256 energy, uint256 levelCode);
}
...
contract Order is Owner{
    ...
    event PlaceOrder(uint256 indexed tokenId, address indexed seller, uint256 indexed price, uint256 levelCode);
    ...
    constructor(address LOS20_,address LOS721_,address templar_,address pendingOwner_){
        LOS20 = IERC20(LOS20_);
    ...
    function placeOrder(uint256 _tokenId, uint256 _price) nonContract external
    {
        (, uint energy, uint levelCode) = templar.getTokenInfo(_tokenId);
        require(levelCode > 0,"open first");
        require(energy == templar.maxEnergy(),"full energy");
        require(LOS721.ownerOf(_tokenId) == msg.sender, "Only owner can sell");
        require(_price > 0, "Cannot sell without fee");
        LOS721.transferFrom(msg.sender, address(this), _tokenId);
    ...
    function tokenInfo(uint256 tokenId) public view returns (uint256 fightAt, uint256 energy, uint256 levelCode, uint256 price) {
    ...
    function cancelOrder(uint256 _tokenId) public nonContract {
        ...
        LOS721.transferFrom(address(this), msg.sender, _tokenId);
    ...
    function fillOrder(uint256 _tokenId,uint256 _price) public nonContract {
        ...
        LOS20.transferFrom(msg.sender, feeAccount, feeAmount);
        LOS20.transferFrom(msg.sender, itemSale.owner,itemSale.price - feeAmount);
    ...
    function myInventory(address owner) external view returns(uint256[] memory available, uint256[] memory marketplace) {
        return (LOS721.getTokens(owner), sellerTokens[owner].values());
    }
