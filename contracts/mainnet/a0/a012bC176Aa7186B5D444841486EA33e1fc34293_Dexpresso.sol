...
contract Dexpresso is IDexpresso, Pausable, ReentrancyGuard {
    ...
    modifier isDaoMember() {
        require(
            IAccessController(daoMultiSig).isDaoMember(msg.sender),
    ...
    function executeSwap(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            bool orderCancelleded = orderCanceledStatus[matchedOrder.makerUserAddress][
            ...
            if (
                (matchedOrder.makerValidUntil < block.number) ||
                (matchedOrder.takerValidUntil < block.number)
            ) {
                batchExecuteStatus[i] = SwapStatus(matchedOrder.matchID, VALID_UNTIL_ERROR_CODE);
                continue;
            }

            bool isPriceFair = (matchedOrder.makerTotalSellAmount *
            ...
            bool isPriceRelative = (matchedOrder.makerRatioSellArg *
            ...
            uint256 takerFee = (matchedOrder.makerTotalSellAmount * matchedOrder.takerFeeRatio) /
                1000;
            uint256 makerFee = (matchedOrder.takerTotalSellAmount * matchedOrder.makerFeeRatio) /
                1000;
     ...
     *
     */
    function proposeToUpdateDaoMultiSig(
        address _newDaoMultiSig
    ) external whenNotPaused isDaoMultiSig {
