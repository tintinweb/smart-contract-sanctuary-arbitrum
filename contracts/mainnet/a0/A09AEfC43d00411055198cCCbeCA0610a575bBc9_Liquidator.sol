    ...
    ReentrancyGuardUpgradeable
{
    ...
    // adding new variable for proxies instead of changing existing one
    ...
    function initialize(
        ...
        sushiSwapRouter = IUniswapV2Router02(_swapRouterAddress);
        ...
        for (uint256 i = 0; i < candidates.length; i++) {
            ...
            if (currentAssetPrice.mul(transferedAmount) > liquidationAmount) {
                ...
                if (transferedAmount - amountUsed > 0) {
                    IERC20Metadata(collateralTokenAddress).transfer(
                        candidates[i],
    ...
    function _swapPath() public view returns (address[] memory) {
        address[] memory path;
        path = new address[](3);
    ...
    function _swapExactOut(
        ...
        IERC20Metadata(collateralTokenAddress).approve(
            address(sushiSwapRouter),
            amountInMaximum
        );

        uint[] memory amounts = sushiSwapRouter.swapTokensForExactTokens(
            amountOut,
            amountInMaximum,
            _swapPath(),
            address(this),
