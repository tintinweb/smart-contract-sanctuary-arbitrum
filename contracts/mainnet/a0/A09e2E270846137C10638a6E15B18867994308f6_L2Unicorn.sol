...
contract L2Unicorn is IL2Unicorn, ERC721EnumerableUpgradeable, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    function batchBurn(uint256[] calldata tokenIdArr) external {
        ...
        for (uint256 i_ = 0; i_ < length;) {
            ...
            require(_isApprovedOrOwner(_msgSender(), tokenId), "L2Unicorn: caller is not token owner or approved");
            _burn(tokenId);
            unchecked {i_++;}
        }
    }
    ...
    function mintForLevel(address to_, uint8 level_, uint256 levelStartTokenId_) external returns (uint256) {
        require(hasRole(_ROLE_MINTER, _msgSender()), "L2Unicorn: caller access denied");
        require(levelStartTokenId_ != 0, "L2Unicorn: level start tokenId is zero");
