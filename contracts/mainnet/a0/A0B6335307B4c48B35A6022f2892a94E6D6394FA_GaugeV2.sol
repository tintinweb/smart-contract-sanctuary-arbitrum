...
interface IShortfallBuster {
    function requestRewards(address token, uint256 amount) external;
}
...
import "../interfaces/IShortfallBuster.sol";
...
contract GaugeV2 is Initializable, IGaugeV2 {
    ...
    mapping(address owner => address operator) public ownerToOperator;
    ...
    function initialize(
        ...
        address token0 = IRamsesV2Pool(_pool).token0();
        address token1 = IRamsesV2Pool(_pool).token1();
        ...
        isReward[ram] = true;
        isReward[xRam] = true;
        isReward[token0] = true;
        isReward[token1] = true;
    ...
    function setRewardFalse() external reinitializer(2) {
        require(
            address(this) == 0xB11aBCcCF6b97e64e695Ca27fa7383C59fBA9C6c ||
                address(this) == 0x3442510698c286449341646D2892e3dC205979e2
        );
        isReward[0x912CE59144191C1204E64559FE8253a0e49E6548] = false;
    }
    ...
    function rewardRate(
        address token
    ) external view returns (uint256 _rewardRate) {
        uint256 period = _blockTimestamp() / WEEK;
        if (period > 2830) {
            _rewardRate = tokenTotalSupplyByPeriod[period][token] / WEEK;
        } else {
            _rewardRate =
                (tokenTotalSupplyByPeriod[period][token] * 4) /
                (10 * WEEK);
        }
    }
    ...
    function notifyRewardAmount(
        ...
        IRamsesV2Pool(pool)._advancePeriod();
        uint256 period = _blockTimestamp() / WEEK;
        ...
        if (msg.sender == voter && !isReward[token]) {
            isReward[token] = true;
            rewards.push(token);
    ...
    function getReward(
        ...
        require(
            msg.sender == owner || msg.sender == ownerToOperator[owner],
            "Not authorized"
        );
    ...
    function _getReward(
        ...
        if (_reward > 0) {
            ...
            if (_reward > bal) {
                IShortfallBuster(0xAA2348Cb6372D69Be39deF46D2d51D983cad8a41)
                    .requestRewards(token, _reward - bal);
            }
    ...
    function retrieveLeftovers(
        ...
        for (uint256 i = 0; i < rewardsLength; ++i) {
            ...
            for (uint256 j = 0; j < periodsLength; ++j) {
                ...
                require(period > 2794 && period < 2830, "Nope");
    ...
    function approveOperator(address _operator) external {
        ownerToOperator[msg.sender] = _operator;
    }
}
