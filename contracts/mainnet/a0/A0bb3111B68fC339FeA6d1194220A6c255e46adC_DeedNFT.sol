...
error NotDeedOwner(uint256 deedId);
...
contract DeedNFT is ERC721Upgradeable, ERC721URIStorageUpgradeable, AccessManagerBase {
    ...
    modifier deedExists(uint256 _deedId) {
        if (_ownerOf(_deedId) == address(0)) revert DeedDoesNotExist(_deedId);
        _;
    }

    modifier onlyOwner(uint256 _deedId) {
        if (_msgSender() != _ownerOf(_deedId)) revert NotDeedOwner(_deedId);
