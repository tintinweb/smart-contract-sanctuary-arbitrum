...
contract Referee is ERC2771Context, Proxied, IReferee {
    ...
    function attest(
        ...
        _attest(_batchNumber, _l2StateRoot, _nodeKeyId, false);
    ...
    function batchAttest(
        ...
        for (uint256 i; i < _nodeKeyIds.length; i++) {
            _attest(_batchNumber, _l2StateRoot, _nodeKeyIds[i], true);
        }
    ...
    function claimReward(uint256 _nodeKeyId, uint256 _batches) external {
        _claimReward(_nodeKeyId, _batches);
    }
    ...
    function batchClaimReward(
        ...
        for (uint256 i; i < _nodeKeyIds.length; i++) {
            _claimReward(_nodeKeyIds[i], _batches);
        }
    }
    ...
    function _claimReward(uint256 _nodeKeyId, uint256 _batches) internal {
        uint256[] memory claimableBatchNumbers = new uint256[](_batches);
        ...
        _batches = _batches > maxClaimableBatches
            ? maxClaimableBatches
            : _batches;

        if (maxClaimableBatches == 0) {
            revert NoRewardsToClaim();
        }

        for (uint256 i; i < _batches; i++) {
