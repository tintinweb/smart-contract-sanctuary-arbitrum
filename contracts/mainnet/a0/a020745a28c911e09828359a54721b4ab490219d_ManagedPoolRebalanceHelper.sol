...
import {IManagedPool} from '@balancer-labs/v2-interfaces/pool-utils/IManagedPool.sol';
import {IVault, IERC20} from '@balancer-labs/v2-interfaces/vault/IVault.sol';
...
contract ManagedPoolRebalanceHelper is AccessManaged {

    using SafeERC20 for SIERC20;

    uint public immutable REBALANCE_COOLDOWN_BLOCKS = 0;
    uint public immutable MAX_BALANCE_DEVIATION;
    uint public immutable MAX_REBALANCE_SLIPPAGE;
    uint public immutable MAX_INITIAL_TOKEN_WEIGHT;
    ...
    constructor (
            ...
            uint maxInitialTokenWeight) AccessManaged(accessManager) {
    ...
    function addToken(
        ...
        IManagedPool pool = _getPool(vault, poolId);
    ...
    function rebalance(
        ...
        _deposit(vault, poolId, origTokens, origBalances);
        _ensureStateIsAcceptableAfterRebalance(vault, poolId, origBalances, origWeights, weights);
        ...
        for (uint i = 0; i < tokens.length; ++i) {
            if (weights[i] < origWeights[i]) {
                ...
                updateOps[index] = IVault.PoolBalanceOp({
                    ...
                    poolId: poolId,
                    token: IERC20(tokens[i]),
                    amount: origBalances[i] - withdrawAmount
                });
                ++index;
            }
        }
    ...
    function _ensureStateIsAcceptableAfterRebalance(
        IVault vault,
        bytes32 poolId,
        uint256[] memory origBalances,
        uint256[] memory origWeights,
        ...
        for(uint i = 0; i < origBalances.length; ++i) {
            ...
            require(absWeightedBalanceDiff <= MAX_BALANCE_DEVIATION * origWeights[i] * newBalances[i], "RATE1");
            require(absWeightedBalanceDiff <= MAX_BALANCE_DEVIATION * origBalances[i] * newWeights[i], "RATE2");
        }

        require(1E18 <= totalValue + MAX_REBALANCE_SLIPPAGE, "SLIPPAGE");
        // bptPrice = Math.div(Math.mul(virtualSupply, weight), balance, !isLowerBound)
        /* btPrice = s*w/b   totv = w */
    }
