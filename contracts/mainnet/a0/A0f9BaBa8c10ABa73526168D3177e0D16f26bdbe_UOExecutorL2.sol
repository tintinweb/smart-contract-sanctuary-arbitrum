...
contract UOExecutorL2 is GnosisSafeStorage{
    ...
    **/
    function withdrawFromWallet(bytes memory _path, uint _amount, uint _destChainId, address _destAddress, uint _bonderFee) public{
        ...
        if(_token == address(weth) && _balanceOfERC20 < _amount){
            weth.deposit{value: (_amount - _balanceOfERC20)}();
        }

        uint _bridgeTokenAmount = _amount;
        if(_token != _bridgeToken){
            IERC20(_token).safeIncreaseAllowance(address(uniRouter), _amount);
            _bridgeTokenAmount = uniRouter.exactInput(ISwapRouter.ExactInputParams(_path, address(this), block.timestamp + 100, _amount, UNISWAP_MIN_OUTPUT));
        }
        ...
        if(_destChainId == getChainId()){
            ...
            */
            if(_destChainId == L1CHAINID){
                ...
                _bridge.swapAndSend(_destChainId, _destAddress, _bridgeTokenAmount, _bonderFee, _bridgeTokenAmount * 98 / 100, block.timestamp + 100, (_bridgeTokenAmount - _bonderFee) * 98 / 100, 0);
            }
        }
    ...
    function depositToAVault(bytes memory _path, uint _amount) external{
        ...
        //transfer from ETH to WETH if WETH not enough
        uint _balanceOfERC20 = IERC20(_token).balanceOf(address(this));
        if(_token == address(weth) && _balanceOfERC20 < _amount){
            weth.deposit{value: (_amount - _balanceOfERC20)}();
        }

        uint _targetTokenAmount = _amount;
        if(_token != _targetToken){
            IERC20(_token).safeIncreaseAllowance(address(uniRouter), _amount);
            _targetTokenAmount = uniRouter.exactInput(ISwapRouter.ExactInputParams(_path, address(this), block.timestamp + 100, _amount, UNISWAP_MIN_OUTPUT));
        }
