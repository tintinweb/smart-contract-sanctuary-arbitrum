...
contract TriCryptoConvexHarvester is ERC20, ReentrancyGuard {
    ...
    constructor(
        ...
        if (_crvToNativePath.length > 0) addRewardV3(_crvToNativePath, 1000000);
        if (_cvxToNativePath.length > 0) addRewardV3(_cvxToNativePath, 300000);
    ...
    function chargeFees(uint nativeTokenBal) internal {
        ...
        IWETH(native).withdraw(senderFee + beneficiaryFee + developerFee);

        (bool successSender, ) = payable(msg.sender).call{value: senderFee}("");
        require(successSender, "Sender Transfer failed.");
        (bool successBeneficiary, ) = payable(beneficiary).call{value: beneficiaryFee}("");
        require(successBeneficiary, "Beneficiary Transfer failed.");
        (bool successDeveloper, ) = payable(developer).call{value: developerFee}("");
        require(successDeveloper, "Developer Transfer failed.");
