...
contract Treasury is ContractGuard {
    ...
    function initialize(
        ...
        dollarSupplyTarget = 50000 ether;
        ...
        seigniorageExpansionFloorPercent = 1500; // At least 15% of expansion reserved for boardroom
        maxSupplyContractionPercent = 25; // Upto 0.25% supply for contraction (to burn DOLLAR and mint BOND)
        maxDebtRatioPercent = 1500; // Upto 15% supply of BOND to purchase
        ...
        // First 0 epochs with 1% expansion
        bootstrapEpochs = 0;
        ...
        shareRewardPoolExpansionRate = 0.0001583320 ether; // 10000 share / (731 days * 24h * 60min * 60s)
        shareRewardPoolContractionRate = 0.0001583320 ether; // SAME RATE
    ...
    function setMaxSupplyExpansionPercents(uint256 _maxSupplyExpansionPercent) external onlyOperator {
        require(_maxSupplyExpansionPercent >= 3 && _maxSupplyExpansionPercent <= 500, "_maxSupplyExpansionPercent: out of range"); // [0.03%, 5%]
    ...
    function setMaxSupplyContractionPercent(uint256 _maxSupplyContractionPercent) external onlyOperator {
        require(_maxSupplyContractionPercent >= 30 && _maxSupplyContractionPercent <= 500, "out of range"); // [0.03%, 5%]
    ...
    function setBootstrap(uint256 _bootstrapEpochs, uint256 _bootstrapSupplyExpansionPercent) external onlyOperator {
        require(_bootstrapEpochs <= 1, "_bootstrapEpochs: out of range");
