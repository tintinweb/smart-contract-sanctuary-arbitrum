...
/**
 ...
 Website: https://taskrypto.com/ 
 TG: https://t.me/TasKryptoXPortal 
 TW: https://twitter.com/taskrypto
...
contract TasKrypto is ERC20, Ownable {
    ...
    uint256 public taxBuy = 4;
    uint256 public taxLPool = 1;
    uint256 public taxSell = 4;

    // limis
    bool public limitsOn = true;
    uint256 public maxWalletInPercent = 5;
    ...
    constructor() ERC20('TaskMan Crypto', 'TMC')  {
    ...
    function transferTokens(
        ...
        uint256 tax = 0;
        if (startTime != 0 && isSwap && !taxesOff && !(isTaxFree[sender] || isTaxFree[recipient])) {
    ...
    function checkLimits(
        ...
        if (startTime == 0) {
            require(isTaxFree[sender] || isTaxFree[recipient], "Contract is not started.");
        }
        bool isBuy = sender == uniPair && recipient != address(uniRouter);
        bool isSell = recipient == uniPair;
        bool isSwap = isBuy || isSell;
        ...
        if (limitsOn) {
            ...
            uint256 maxWallet = totalSupply() * maxWalletInPercent / 100;
            if (isSwap) {
                uint256 maxTx = totalSupply() * maxTxInPercent / 100;
    ...
    function swapTokens(
        ...
        if (swapOn && !swappingInProgress && !isOwner && overMin && startTime != 0 && isSell) {
            swap(minSwap);
        }
    }
    ...
    function isAccountBot(address account) external view returns (bool) {
        return isBot[account];
    }

    function excludeBot(address account) external onlyOwner {
        require(isBot[account], 'Account is not blacklisted');
        isBot[account] = false;
    }
    ...
    function sumTax(bool isSell) private returns (uint256) {
        if (isSell) {
            return taxLPool + taxSell;
        } else {
            return taxLPool + taxBuy;
        }
    }
