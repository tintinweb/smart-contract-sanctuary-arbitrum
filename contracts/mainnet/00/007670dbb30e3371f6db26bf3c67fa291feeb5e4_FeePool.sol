...
contract FeePool is IFeePool, UUPSUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    /// @notice Index token balance at the last update
    uint256 internal lastIndexBalance;
    /// @notice Accumulated index rewards per total supply
    uint256 internal accumulatedTokenPerTotalSupplyInBase;

    /// @inheritdoc IFeePool
    mapping(address => uint256) public override sharesOf;

    /// @notice Withdrawable amounts for accounts
    mapping(address => uint256) internal withdrawableOf;
    /// @notice Accumulated rewards for accounts
    mapping(address => uint256) internal lastAccumulatedTokenPerTotalSupplyInBaseOf;

    error NothingToWithdraw();

    /// @notice Accumulates account rewards per index
    modifier accumulateRewards(address _account) {
        address _index = index;
        try IIndex(_index).accrueFee(address(this)) {} catch {}

        uint256 _totalShares = totalShares;
        if (_totalShares != 0) {
            uint256 tokenIncrease = IERC20(_index).balanceOf(address(this)) - lastIndexBalance;
            if (tokenIncrease != 0) {
                unchecked {
                    // overflow is desired
                    accumulatedTokenPerTotalSupplyInBase += (tokenIncrease * MAX_BPS) / _totalShares;
                }
            }
        }
        ...
        if (_totalShares != 0) {
            lastIndexBalance = IERC20(_index).balanceOf(address(this));
        }
    }
    ...
    /// @inheritdoc IFeePool
    function initialize(address _index, address _owner) external override initializer {
    ...
    /// @inheritdoc IFeePool
    function withdraw() external override nonReentrant accumulateRewards(msg.sender) {
        uint256 amount = withdrawableOf[msg.sender];
        if (amount == 0) revert NothingToWithdraw();

        withdrawableOf[msg.sender] = 0;
        IERC20(index).safeTransfer(msg.sender, amount);
    }

    /// @inheritdoc IFeePool
    function burn(MintBurnInfo calldata _burnInfo) external override onlyOwner {
        _burn(_burnInfo);
    }

    /// @inheritdoc IFeePool
    function mint(MintBurnInfo calldata _mintInfo) external override onlyOwner {
        _mint(_mintInfo);
    }

    /// @inheritdoc IFeePool
    function burnMultiple(MintBurnInfo[] calldata _burnInfo) external override onlyOwner {
        for (uint256 i; i < _burnInfo.length; ++i) {
            _burn(_burnInfo[i]);
        }
    }

    /// @inheritdoc IFeePool
    function mintMultiple(MintBurnInfo[] calldata _mintInfo) external override onlyOwner {
    ...
    /// @inheritdoc IFeePool
    function withdrawableAmountOf(address _account) external view override returns (uint256) {
        uint256 _accumulatedTokenPerTotalSupplyInBase = accumulatedTokenPerTotalSupplyInBase;
        uint256 _totalShares = totalShares;
        uint256 _currentBalance = IERC20(index).balanceOf(address(this)) + IIndex(index).fees();
        if (_totalShares != 0) {
            uint256 tokenIncrease = _currentBalance - lastIndexBalance;
            if (tokenIncrease != 0) {
                unchecked {
                    // overflow is desired
        ...
        uint256 _lastAccumulatedTokenPerTotalSupplyInBase = sharesOf[_account] == 0
            ? _accumulatedTokenPerTotalSupplyInBase
            : lastAccumulatedTokenPerTotalSupplyInBaseOf[_account];
        ...
        uint256 increase = (sharesOf[_account] * accumulatedTokenPerTotalSupplyInBaseIncrease) / MAX_BPS;

        return increase + withdrawableOf[_account];
    }

    /// @notice Accumulates account rewards
    function _accumulateAccountRewards(address _account) internal {
        uint256 _lastAccumulatedTokenPerTotalSupplyInBase = sharesOf[_account] == 0
        ...
        unchecked {
            // overflow is desired
            accumulatedTokenPerTotalSupplyInBaseIncrease =
                accumulatedTokenPerTotalSupplyInBase - _lastAccumulatedTokenPerTotalSupplyInBase;
        }

        if (accumulatedTokenPerTotalSupplyInBaseIncrease != 0) {
            uint256 increase = (sharesOf[_account] * accumulatedTokenPerTotalSupplyInBaseIncrease) / MAX_BPS;
            if (increase != 0) {
                withdrawableOf[_account] += increase;
            }
        }

        lastAccumulatedTokenPerTotalSupplyInBaseOf[_account] = accumulatedTokenPerTotalSupplyInBase;
    }

    /// @dev Mints shares for `_recipient` address
    function _mint(MintBurnInfo calldata _mintInfo) internal accumulateRewards(_mintInfo.recipient) {
        sharesOf[_mintInfo.recipient] += _mintInfo.share;
        totalShares += _mintInfo.share;

        emit Mint(_mintInfo.recipient, _mintInfo.share);
    }

    /// @dev Burns shares for `_recipient` address
    function _burn(MintBurnInfo calldata _burnInfo) internal accumulateRewards(_burnInfo.recipient) {
        sharesOf[_burnInfo.recipient] -= _burnInfo.share;
        totalShares -= _burnInfo.share;

        emit Burn(_burnInfo.recipient, _burnInfo.share);
    }
...
interface IFeePool {
    ...
    event Mint(address indexed recipient, uint256 share);
    event Burn(address indexed recipient, uint256 share);

    /// @notice Initializes FeePool with the given params
    /// @param _index Address of Index
    /// @param _owner Address of owner
    function initialize(address _index, address _owner) external;
    ...
    function mint(MintBurnInfo calldata _mintInfo) external;
    ...
    function burn(MintBurnInfo calldata _burnInfo) external;
    ...
    function mintMultiple(MintBurnInfo[] calldata _mintInfo) external;
    ...
    function burnMultiple(MintBurnInfo[] calldata _burnInfo) external;

    /// @notice Withdraws sender fees from the Index
    function withdraw() external;

    /// @notice Total shares in the index
    /// @return Returns total shares in the Index
    function totalShares() external view returns (uint256);

    /// @notice Shares of specified recipient in the Index
    /// @return Returns shares of specified recipient in the Index
    ...
    function withdrawableAmountOf(address _account) external view returns (uint256);
}
