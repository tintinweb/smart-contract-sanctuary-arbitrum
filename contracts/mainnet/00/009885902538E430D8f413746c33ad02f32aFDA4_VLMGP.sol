...
import "./interfaces/IMGPBurnEventManager.sol";
...
contract VLMGP is IVLMGP, Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address public burnEventManager;
    ...
    event MgpBurn(address indexed user, uint256 indexed timestamp, uint256 amount);
    ...
    function startUnlock(uint256 _amountToCoolDown) external override whenNotPaused nonReentrant {
        _checkVlMgpUnlockEligibility(_amountToCoolDown);
    ...
    function burnVlmgp( uint256 _vlmgpAmountToBurn, uint256 _vlmgpBurnEventId ) external whenNotPaused nonReentrant 
    {
        _checkVlMgpUnlockEligibility(_vlmgpAmountToBurn);        

        if(burnEventManager == address(0))
                revert BurnEventManagerNotSet();

       _claimVlmgpRewards();

        IMasterMagpie(masterMagpie).withdrawVlMGPFor(_vlmgpAmountToBurn, msg.sender); // trigers update pool share, so happens before total amount reducing
        totalAmount -= _vlmgpAmountToBurn;

        IERC20(MGP).safeApprove(burnEventManager, _vlmgpAmountToBurn);
        IMGPBurnEventManager(burnEventManager).joinEventFor(msg.sender, _vlmgpBurnEventId, _vlmgpAmountToBurn);

        if (referralStorage != address(0)) IReferralStorage(referralStorage).updateTotalFactor(msg.sender);

        emit MgpBurn(msg.sender, block.timestamp, _vlmgpAmountToBurn);
    }
   ...
   function forceUnLock(uint256 _slotIndex) external whenNotPaused nonReentrant {
    ...
    function setReferralStorage(address _referralStorage) external onlyOwner {
     ...
     function setMgpBurnEventManager(address _burnEventMAnager) external onlyOwner {
        burnEventManager = _burnEventMAnager;
    }
    ...
    function _checkVlMgpUnlockEligibility( uint256 _vlmgpAmount ) internal view
    {
        if (_vlmgpAmount > getUserTotalLocked(msg.sender))
        revert NotEnoughLockedMPG();

        uint256 totalLockAfterStartUnlock = getUserTotalLocked(msg.sender) - _vlmgpAmount;
    ...
    function _claimVlmgpRewards() internal {
