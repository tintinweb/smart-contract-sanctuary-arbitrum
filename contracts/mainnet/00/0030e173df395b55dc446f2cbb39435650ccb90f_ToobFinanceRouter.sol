...
contract ToobFinanceRouter is Ownable {
    ...
    function swap(
        ...
        return _swapApproval(tokenInfo, pathDefinition, executor);
    }
    ...
    function _swapApproval(
        ...
        if (tokenInfo.inputToken == _ETH) {
            ...
            // Support rebasing tokens by allowing the user to trade the entire balance
            if (tokenInfo.inputAmount == 0) {
                tokenInfo.inputAmount = IERC20(tokenInfo.inputToken).balanceOf(
                    msg.sender
                );
            }
    ...
    function _swap(
        ...
        require(
            tokenInfo.inputToken != tokenInfo.outputToken,
        ...
        IToobExecutor(executor).executePath{value: msg.value}(
            pathDefinition,
            amountsIn,
            msg.sender
        );
    ...
    function _swapMultiApproval(
        ...
        for (uint256 i = 0; i < inputs.length; i++) {
            if (inputs[i].tokenAddress == _ETH) {
                ...
                if (inputs[i].amountIn == 0) {
                    inputs[i].amountIn = IERC20(inputs[i].tokenAddress)
        ...
        return
            _swapMulti(inputs, outputs, valueOutMin, pathDefinition, executor);
    }
    ...
    function swapMultiPermit2(
        ...
        {
            uint256 permit_length = msg.value > 0
                ? inputs.length - 1
                : inputs.length;
    ...
    function _swapMulti(
...

        {
            ...
            for (uint256 i = 0; i < outputs.length; i++) {
                ...
                amountsOut[i] =
                    _universalBalance(outputs[i].tokenAddress) -
                    balancesBefore[i];

                // Remove the swapMulti Fee (taken instead of positive slippage)
                amountsOut[i] =
                    (amountsOut[i] * (FEE_DENOM - _swapMultiFee)) /
                    FEE_DENOM;
        ...
        emit SwapMulti(msg.sender, amountsIn, tokensIn, amountsOut, tokensOut);
    }
    ...
    function swapRouterFunds(
        ...
        IToobExecutor(executor).executePath{value: 0}(
            pathDefinition,
            amountsIn,
            msg.sender
        );
