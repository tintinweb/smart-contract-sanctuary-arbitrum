...
interface IStake {
  ...
  function safeApeTransfer(address _account, uint256 _amount) external;
}
...
interface IMasterChef {
  ...
  function pendingApe(address _stakeToken, address _user) external view returns (uint256);
  ...
  function apePerBlock() external view returns (uint256);
  ...
  function depositApe(address _for, uint256 _amount) external;

  function withdrawApe(address _for, uint256 _amount) external;
  ...
  function ape() external returns (IAPE);
}
...
// MasterChef is the master of APE. He can make APE and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once APE is sufficiently
...
contract MasterChef is IMasterChef, Ownable, ReentrancyGuard {
  ...
  struct PoolInfo {
    ...
    uint256 lastRewardBlock; // Last block number that APE distribution occurs.
    uint256 accApePerShare; // Accumulated APE per share, times 1e12. See below.
    uint256 depositFee;
  }

  // APE token.
  IAPE public override ape;
  // Stake address.
  IStake public stake;
  // Dev address.
  ...
  // APE per block.
  uint256 public override apePerBlock;
  ...
  // The block number when APE mining starts.
  ...
  event SetApePerBlock(uint256 prevApePerBlock, uint256 currentApePerBlock);
  ...
  constructor(
    ...
    // add APE pool
    pools.add(address(_ape));
    poolInfo[address(_ape)] = PoolInfo({
      allocPoint: 0,
  ...
  // Set APE per block.
  function setApePerBlock(uint256 _apePerBlock) external onlyOwner {
    massUpdatePools();
    uint256 prevApePerBlock = apePerBlock;
    apePerBlock = _apePerBlock;
    emit SetApePerBlock(prevApePerBlock, apePerBlock);
  }
  ...
  function removePool(address _stakeToken) external override onlyOwner {
    require(_stakeToken != address(ape), "removePool: can't remove APE pool");
    ...
    poolInfo[_stakeToken].accApePerShare = 0;
  ...
  function pendingApe(address _stakeToken, address _user) external view override returns (uint256) {
    ...
    if (block.number > pool.lastRewardBlock && totalStakeToken != 0) {
      ...
      uint256 apeReward = multiplier.mul(apePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      accApePerShare = accApePerShare.add(apeReward.mul(1e12).div(totalStakeToken));
    }
    return user.amount.mul(accApePerShare).div(1e12).sub(user.rewardDebt);
  }
  ...
  function updatePool(address _stakeToken) public override {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 apeReward = multiplier.mul(apePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ape.mint(devAddr, apeReward.mul(devBps).div(10000));
    ape.mint(address(stake), apeReward.mul(refBps).div(10000));
    ape.mint(address(stake), apeReward);
    pool.accApePerShare = pool.accApePerShare.add(apeReward.mul(1e12).div(totalStakeToken));
    pool.lastRewardBlock = block.number;
  }

  // Deposit token to MasterChef for APE allocation.
  function deposit(
    address _for,
    address _stakeToken,
    uint256 _amount
    ...
    require(_stakeToken != address(ape), "deposit: use depositApe instead");
    ...
    user.rewardDebt = user.amount.mul(pool.accApePerShare).div(1e12);
  ...
  function withdraw(
    ...
    require(_stakeToken != address(ape), "withdraw: use withdrawApe instead");
  ...
  // Deposit APE to MasterChef.
  function depositApe(address _for, uint256 _amount)
    external
    override
    onlyPermittedTokenFunder(_for, address(ape))
    nonReentrant
  {
    PoolInfo storage pool = poolInfo[address(ape)];
    UserInfo storage user = userInfo[address(ape)][_for];

    if (user.fundedBy != address(0)) require(user.fundedBy == _msgSender(), "depositApe: bad sof");

    updatePool(address(ape));

    if (user.amount > 0) _harvest(_for, address(ape));
    if (user.fundedBy == address(0)) user.fundedBy = _msgSender();
    if (_amount > 0) {
      uint256 depositFeeAmount = _amount.mul(pool.depositFee).div(10000);
      if (depositFeeAmount > 0) {
        _amount = _amount.sub(depositFeeAmount);
        IERC20(address(ape)).safeTransferFrom(address(_msgSender()), devAddr, depositFeeAmount);
      }
      IERC20(address(ape)).safeTransferFrom(address(_msgSender()), address(this), _amount);
      user.amount = user.amount.add(_amount);
    }
    user.rewardDebt = user.amount.mul(pool.accApePerShare).div(1e12);
    emit Deposit(_msgSender(), _for, address(ape), _amount);
  }

  // Withdraw APE
  function withdrawApe(address _for, uint256 _amount) external override nonReentrant {
    PoolInfo storage pool = poolInfo[address(ape)];
    UserInfo storage user = userInfo[address(ape)][_for];

    require(user.fundedBy == _msgSender(), "withdrawApe: only funder");
    require(user.amount >= _amount, "withdrawApe: not good");

    updatePool(address(ape));
    _harvest(_for, address(ape));

    if (_amount > 0) {
      user.amount = user.amount.sub(_amount);
      IERC20(address(ape)).safeTransfer(address(_msgSender()), _amount);
    }
    user.rewardDebt = user.amount.mul(pool.accApePerShare).div(1e12);
    if (user.amount == 0) user.fundedBy = address(0);
    emit Withdraw(_msgSender(), _for, address(ape), user.amount);
  }

  // Harvest APE earned from a specific pool.
  ...
  // Internal function to harvest APE
  function _harvest(address _for, address _stakeToken) internal {
    ...
    uint256 pending = user.amount.mul(pool.accApePerShare).div(1e12).sub(user.rewardDebt);
    require(pending <= ape.balanceOf(address(stake)), "_harvest: wait what.. not enough APE");
    stake.safeApeTransfer(_for, pending);
  ...
  function _referralCallee(address _for, uint256 _pending) internal {
    ...
    stake.safeApeTransfer(address(refAddr), _pending.mul(refBps).div(10000));
  ...
  // This is a function for mining an extra amount of ape, should be called only by stake token caller contract (boosting purposed)
  function mintExtraReward(
    address _stakeToken,
    address _to,
    uint256 _amount
  ) external override onlyStakeTokenCallerContract(_stakeToken) {
    ape.mint(_to, _amount);
  }
}