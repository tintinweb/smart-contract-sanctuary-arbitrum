...
contract KeyChr is KolSolidlyManager, ReentrancyGuardUpgradeable, ERC20Upgradeable {
    ...
    function calcBuyFee() public view returns(uint256) { //dynamic buy fee based on the imbalance in pool
        ...
        if (ratio>=5000) //healthy
            ...
            return totalMintFees + ((5000-ratio) *  ( maxTotalBuyFees + minTotalBuyFees) / 10000);
    }
   ...
   */
    ...
    function setFees(uint256 _minTotalBuyFees, uint256 _maxTotalBuyFees, uint256 _totalMintFees, uint256 _totalSellFees) external onlyOwner {
        require(_minTotalBuyFees<=2000 && _maxTotalBuyFees<=2000 && _totalMintFees<=2000 && _totalSellFees<=2000,">20%");
    ...
    function setVoteData(uint256[] calldata _tokenIds, address[] calldata _vaultVotes, uint256[] calldata _weights) external onlyOwner {
        ...
        delete voteVaults;
        voteVaults = new address[](_vaultVotes.length);
        delete voteWeights;
        voteWeights = new uint256[](_weights.length);
        require(_vaultVotes.length==_weights.length,"_vaultVotes.length!=_weights.length");

        for (uint256 i = 0; i < _vaultVotes.length; i++) {
            voteVaults[i] = _vaultVotes[i];
    ...
    function doVote() external authorized {
        for (uint256 i = 0; i < voteTokenIds.length; i++) {
            ...
            solidVoter.vote(_tokenId, voteVaults, voteWeights);
        }
    }
