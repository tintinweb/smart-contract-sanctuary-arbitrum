...
contract LIONPresale is ReentrancyGuard {
	...
	// Price of presale LION, 1 ETH
	uint256 ETHPerToken = 1;
	// Amount of ETH received in presale
	uint256 busdReceived = 0;

    uint256 HardCap = 1 * 10 ** 18; // 1 ETH
	...
	function buy(uint256 _amount) public nonReentrant {
        ...
        require(_amount + (tokensOwned[msg.sender] * ETHPerToken) <= maxcontributionLimit, "high amount than max");
        
        
		address _buyer = msg.sender;
		uint256 tokens = _amount.div(ETHPerToken);   
