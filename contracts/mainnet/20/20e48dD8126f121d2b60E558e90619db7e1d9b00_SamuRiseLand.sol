...
contract SamuRiseLand is ERC721EnumerableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function mintPostClaim(bytes memory _signature, uint256 _nonce) 
        ...
        validateClaimPeriodActive 
        underMaxSupply(1) 
        hasValidSignature(_signature, abi.encode(msg.sender, _nonce, MintState.POST_CLAIM))
        numberMintedUnderAllocation(numberMinted[msg.sender], 1, MAX_LAND_PER_WALLET)
    {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, _tokenId.toString(), "?rarity=",  uint256(tokenIdToRarity[_tokenId]).toString())) : "";
    }
...
error ClaimPeriodIsNotActive();
error PostClaimPeriodIsNotActive();
