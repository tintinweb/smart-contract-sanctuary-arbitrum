...
contract StrategyAutomationAdapter is AutomateTaskCreator {
    ...
    function addStrategy(
        ...
        (Module[] memory additionalModule, bytes[] memory additionalPayload) = abi.decode(params, (Module[], bytes[]));
        ModuleData memory moduleData = ModuleData({
            modules: new Module[](2 + additionalModule.length),
            args: new bytes[](2 + additionalPayload.length)
        });
        ...
        for (uint i = 0; i < additionalModule.length; i++) {
            moduleData.modules[i + 2] = additionalModule[i];
            moduleData.args[i + 2] = additionalPayload[i];
        }
