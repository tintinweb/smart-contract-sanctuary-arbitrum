...
contract Seller {
    ...
    constructor(address maintokenAddress, address sourceAddress, address destinationWallet, address sellerAddress, uint256 price, uint256 leadTreshold ) {
    ...
    function sourceTokenDecimals() public view returns(uint8){
    ...
    function changePrice(uint256 price) external onlyOwner {
        require(price >= 250, "changePrice: Price must be grether than 0.2 (200) ");
        _tokenPrice = price;
    }

    function calculateAmount(uint256 sourceTokenAmount) public view returns(uint256){
        return uint256((sourceTokenAmount * 1000 )/ _tokenPrice );
    }
