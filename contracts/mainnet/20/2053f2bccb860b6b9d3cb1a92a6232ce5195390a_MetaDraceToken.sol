...
/**
 * Standard MetaDraceToken, stripped down to just add/sub/mul/div
 */
library MetaDraceToken {
    function add(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
        require(c >= a, "MetaDraceToken: addition overflow");

        return c;
    }
    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        return sub(a, b, "MetaDraceToken: subtraction overflow");
    }
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        uint256 c = a * b;
        require(c / a == b, "MetaDraceToken: multiplication overflow");

        return c;
    }
    function div(uint256 a, uint256 b) internal pure returns (uint256) {
        return div(a, b, "MetaDraceToken: division by zero");
    }
...
contract MetaDrace is IERC20, Auth {
    using MetaDraceToken for uint256;

    address private WETH;
    address private DEAD = 0x000000000000000000000000000000000000dEaD;
    address private ZERO = 0x0000000000000000000000000000000000000000;

    string private constant  _name = "MetaDrace";
    string private constant _symbol = "DRACE";
    uint8 private constant _decimals = 9;

    uint256 private _totalSupply = 1000000 * (10 ** _decimals);
    //max wallet holding of 2% 
    uint256 public _maxTokenPerWallet = ( _totalSupply * 2 ) / 100;
    ...
    address payable public marketingWallet = payable(0x816022c549c491027bd6047b8f7a01d6401aaD9a);
    ...
    uint256 private maxBuyTransaction = ( _totalSupply * 10 ) / 1000; // 1% max tx
    ...
    function setFee (uint256 _sellFeeRate, uint256 _buyFeeRate) external onlyOwner {
        require (_buyFeeRate <= 10, "Fee can't exceed 10%");
        require (_sellFeeRate <= 10, "Fee can't exceed 10%");
