...
/**
 * @dev The contract directly connected to BrightPool Backend managing exchanging assets
 * and deploying and executing orders.
 */
contract BrightPoolManager is LedgerOwner {
    using ECDSA for bytes32;

    /**
     * @dev Permit data structure to permit transfers without prior allowance
     */
    struct PermitData {
        uint256 deadline;
        uint256 value;
    ...
    /**
     * @dev Address of the cron wallet allowed to execute batched orders
     */
    address cron;

    /**
     * @dev Address of the backend wallet allowed to sign off transaction executed in managers context
     */
    address backend;

    /**
     * @dev The treasury of the BrightPool system
     */
    IBrightPoolTreasury treasury;

    /**
     * @dev The event emitted on new treasury address assignment
     */
    event NewTreasury(address indexed treasury);

    /**
     * @dev The event emitted upon new backend public address assignment
     */
    event NewBackend(address indexed backend);

    /**
     * @dev The event emitted upon new cron public address assignment
     */
    event NewCron(address indexed cron);

    /**
     * @dev A modifier restricting method to be called only by contract backend address
     */
    modifier onlyBackend() {
        if (_msgSender() != backend) revert Restricted();
        _;
    }
     ...
     */
    modifier onlyCron() {
        if (_msgSender() != cron) revert Restricted();
        _;
    }

    /**
     * @dev The contracts constructor.
     *
     * @param owner_ Owner of the contract
     * @param admin_ The admin of the contract (allowed to change backend and cron addresses)
     * @param backend_ The backend address connected to the manager
     * @param cron_ The cron address connected to the manager
     * @param ledger_ The ledger address connected to the manager. Only moment when it can be assigned as address(0)
     * @param treasury_ The treasury contract for BrightPool
     */
    constructor(
        address owner_,
        address admin_,
        ...
        address treasury_,
        address warden_
    ) LedgerOwner(owner_, admin_, warden_) {
        // slither-disable-next-line missing-zero-check
        _setLedger(IBrightPoolLedger(payable(ledger_)));
        // slither-disable-next-line missing-zero-check
        cron = cron_;
        // slither-disable-next-line missing-zero-check
        backend = backend_;
        // slither-disable-next-line missing-zero-check
        treasury = IBrightPoolTreasury(treasury_);
    }

    /**
     * @dev The method to set new treasury.
     * The method is restricted to contracts owner only.
     *
     * @param treasury_ The new treasury contract
     */
    function setTreasury(IBrightPoolTreasury treasury_) external onlyAdminOrOwner {
        if (treasury == treasury_) revert AlreadySet();
        if (address(0) == address(treasury_)) revert ZeroAddress();
        // slither-disable-start reentrancy-events
        // slither-disable-next-line reentrancy-no-eth
        if (_getWarden().changeValue(address(treasury_), "treasury", _msgSender())) {
            treasury = treasury_;
            emit NewTreasury(address(treasury));
        }
        // slither-disable-end reentrancy-events
    }

    /**
     * @dev Retrieving current address of the treasury
     */
    ...
    /**
     * @dev The method to set new cron address.
     * The method is restricted to contracts admin only.
     *
     * @param cron_ The cron_ address to be assigned. Cannot be address(0)
     */
    function setCron(address cron_) external onlyAdmin {
        if (cron == cron_) revert AlreadySet();
        if (address(0) == cron_) revert ZeroAddress();
        cron = cron_;
        emit NewCron(cron);
    }

    /**
     * @dev The method to set new backend address.
     * The method is restricted to contracts admin only.
     *
     * @param backend_ The backend address to be assigned. Cannot be address(0)
     */
    function setBackend(address backend_) external onlyAdmin {
        if (backend == backend_) revert AlreadySet();
        if (address(0) == backend_) revert ZeroAddress();
        backend = backend_;
     ...
     * The method has to be signed of by contracts backend key
     *
     * @param order_ The order struct. It's id has to be unique and cannot be 0. Order owner has to be the same as method caller.
     * @param deadline_ The deadline to which this order can be placed (verified on the blockchain)
     * @param executionTimeout_ The deadline after which this order can be executed
     * @param signature_ The signature of the call signed by current backend key.
     */
    function makeOrder(
        IBrightPoolLedger.Order calldata order_,
        uint256 deadline_,
        uint256 executionTimeout_,
        bytes calldata signature_
    ) external payable {
        if (_msgSender() != order_.owner) revert BadSender();
        if (executionTimeout_ < deadline_) revert Blocked();
        // slither-disable-next-line timestamp
        if (deadline_ < block.timestamp) revert Timeout();
        bytes32 digest = digestFromOrder(order_, deadline_, executionTimeout_);
        if (backend != MessageHashUtils.toEthSignedMessageHash(digest).recover(signature_)) revert InvalidSignature();

        ledger().makeOrder{value: msg.value}(order_, executionTimeout_);
    }
     ...
     * @param executionTimeout_ The deadline after which this order can be executed
     * @param signature_ The signature of the call signed by current backend key.
     * @param permit_ Permit data if order is made without allowance but with permit
     */
    function makeOrderWithPermit(
        IBrightPoolLedger.Order calldata order_,
        uint256 deadline_,
        uint256 executionTimeout_,
        bytes calldata signature_,
        PermitData calldata permit_
    ) external payable {
        if (_msgSender() != order_.owner) revert BadSender();
        if (executionTimeout_ <= deadline_) revert Blocked();
        // slither-disable-next-line timestamp
        ...
        // Check permits
        if (address(order_.bid.token) != address(0)) {
            if (order_.bid.token.allowance(order_.owner, address(ledger())) < order_.bid.amount) {
                if (permit_.deadline > 0) {
                    IERC20Permit(address(order_.bid.token)).permit(
                        order_.owner,
                        address(ledger()),
                        permit_.value,
                        permit_.deadline,
    ...
    /**
     * @dev The method for order cancellation.
     * The method has to be signed off by contracts backend key
     *
     * @param id_ The id of the order to be cancelled.
     * @param rewardConsumed_ The amount of the reward consumed for the cancellation
     * @param deadline_ The deadline to which this call is possible
     * @param signature_ The signature of the call signed by current backend key
     */
    function cancelOrder(uint256 id_, uint256 rewardConsumed_, uint256 deadline_, bytes calldata signature_) external {
        if (_msgSender() != ledger().orderOwner(id_)) revert BadSender();
        // slither-disable-next-line timestamp
        if (deadline_ < block.timestamp) revert Timeout();
        bytes32 digest = keccak256(abi.encode(block.chainid, _msgSender(), id_, rewardConsumed_, deadline_));
        if (backend != MessageHashUtils.toEthSignedMessageHash(digest).recover(signature_)) {
            revert InvalidSignature();
        }
        if (!ledger().executeOrder(id_, true, rewardConsumed_, treasury, 0)) {
            revert NotExists();
        }
    }
    ...
    function cancelOrderWithPermit(
        ...
        if (rewardConsumed_ > 0) {
            BRIX brix = ledger().rewardToken();
            if (brix.allowance(_msgSender(), address(ledger())) < rewardConsumed_) {
                if (permit_.deadline > 0) {
                    brix.permit(
                        _msgSender(),
                        address(ledger()),
    ...
    /**
     * @dev The method for order owners to claim their price
     *
     * @param id_ The id of the order to be executed by the owner
     */

    function executeOrder(uint256 id_) external {
        if (_msgSender() != ledger().orderOwner(id_)) revert Restricted();
        if (!ledger().executeOrder(id_, false, 0, treasury, 0)) revert NotExists();
    }

    /**
     * @dev The method for order execution in with possible assets swaps.
     * This method can be executed by the backend wallet only.
     *
     * @param orders_ The order list to be executed
     * @param treasuryCap_ The cap price for single transaction amount where treasury can be used
     */
    function executeOrders(uint256[] calldata orders_, uint256 treasuryCap_) external onlyCron {
        for (uint256 i = 0; i < orders_.length;) {
            // slither-disable-start unused-return,
            // slither-disable-start calls-loop
            ledger().executeOrder(orders_[i], false, 0, treasury, treasuryCap_);
    ...
    function digestFromOrder(IBrightPoolLedger.Order calldata order_, uint256 deadline_, uint256 executionTimeout_)
        internal
        view
        returns (bytes32)
    {
...
abstract contract LedgerOwner is Ownable {
    ...
    constructor(address owner_, address admin_, address warden_) Ownable(owner_) {
        if (owner_ == address(0)) revert ZeroAddress();
        if (admin_ == address(0)) revert ZeroAddress();
        if (warden_ == address(0)) revert ZeroAddress();
        _warden = BrightPoolWarden(warden_);
        _admin = admin_;
    }
     ...
     */
    function setLedger(IBrightPoolLedger ledger_) external virtual {
        if (address(_ledger) != address(0) && msg.sender != _admin && msg.sender != owner()) {
            revert Restricted();
        }
        ...
        // slither-disable-start reentrancy-events
        if (address(_ledger) == address(0) || _warden.changeValue(address(ledger_), "ledger", msg.sender)) {
            _ledger = ledger_;
...
abstract contract IBrightPoolLedger is Ownable {
    ...
    /**
     * @dev The event emitted upon order being rejected from the ledger
     */
    event RejectedOrder(uint256 indexed id);
    ...
    constructor(address owner_, address warden_) Ownable(owner_) {
        if (warden_ == address(0)) revert ZeroAddress();
        _warden = BrightPoolWarden(warden_);
    }
    ...
    function setManager(address manager_) external onlyOwner {
        ...
        // slither-disable-start reentrancy-events
        // slither-disable-next-line reentrancy-no-eth
        if (manager == address(0) || _warden.changeValue(manager_, "manager", address(0))) {
            manager = manager_;
    ...
    /**
     * @dev Reward token ledger is associated with
     */
    function rewardToken() external view virtual returns (BRIX);
     ...
     */
    function orderOwner(uint256 id_) external view virtual returns (address);
...
/**
 * @dev Action restricted. Sender is invalid
 */
error BadSender();

/**
 * @dev Action restricted. Signature is invalid
 */
 ...
 * @dev Missing vesting tokens
 */
error WrongAffiliatePlan();

/**
 * @dev Bad attributes passed to the call
 */
error WrongInput();
...
contract BRIX is ERC20Permit, LedgerOwner {
    uint256 constant BRIX_HARD_CAP = 888_888_888 gwei;
     ...
     * @param warden_ The warden contract
     */
    constructor(address owner_, address admin_, address warden_)
        ERC20("BrightPool Exchange Token", "BRIX")
        ERC20Permit("BrightPool Exchange Token")
        LedgerOwner(owner_, admin_, warden_)
    {}
    ...
    function reward(address recipient_, uint256 amount_) external onlyLedger {
        if (totalSupply() + amount_ > BRIX_HARD_CAP) revert CapExceeded();
     ...
     */
    function burnReward(uint256 amount_) external {
...
interface IBrightPoolConsumer {
    function consume(IBrightPoolLedger.Asset memory asset_) external payable;
}
...
interface IBrightPoolExchangeable {
    function exchange(IBrightPoolConsumer consumer_, IBrightPoolLedger.Order memory order_) external payable;
}
