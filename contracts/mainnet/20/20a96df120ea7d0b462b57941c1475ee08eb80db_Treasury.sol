...
contract Treasury is Ownable {
    ...
    address public sGLA;
    uint256 public sGLAQueue; // Delays change to sGLA address
    ...
    /**
        @notice allow approved address to deposit an asset for GLA
        @param _amount uint
    ...
    function deposit(
        ...
        // mint GLA needed and store amount of rewards for distribution
    ...
    /**
        @notice allow approved address to burn GLA for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        IGLAERC20(GLA).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sGLA).balanceOf(msg.sender); // Can only borrow against sGLA held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with GLA
        @param _amount uint
     */
    function repayDebtWithGLA(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        IGLAERC20(GLA).burnFrom(msg.sender, _amount);
    ...
    /**
        @notice returns GLA valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    {
        if (isReserveToken[_token]) {
            // convert amount to match GLA decimals
            value_ = _amount.mul(10**IERC20(GLA).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SGLA) {
            // 9
            sGLAQueue = block.number.add(blocksNeededForQueue);
        } else return false;
