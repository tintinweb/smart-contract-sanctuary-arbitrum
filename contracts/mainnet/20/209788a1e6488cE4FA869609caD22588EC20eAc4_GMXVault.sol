...
library GMXWithdraw {
  ...
  */
  function processWithdraw(
    ...
    if (_assetValueBefore > 0) {
      // Use existing tokens debt amount adding back repayed tokens amt
      (uint256 _tokenADebtAmt, uint256 _tokenBDebtAmt) = GMXReader.debtAmt(self);

      _hp.tokenADebtAmtBefore = _tokenADebtAmt + _wc.repayParams.repayTokenAAmt;
      _hp.tokenBDebtAmtBefore = _tokenBDebtAmt + _wc.repayParams.repayTokenBAmt;
