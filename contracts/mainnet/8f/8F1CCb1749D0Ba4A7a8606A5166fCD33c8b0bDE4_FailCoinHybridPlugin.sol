...
interface FailCoinEventsErrors {
    error WierdIntermittentBlockchainErrorOnERC20Transfer();
    error WierdIntermittentBlockchainErrorOnPermit();
}

interface IFailCoinHybridPlugin is FailCoinEventsErrors {
    /**
     * @dev returns the number of times tokens can be transferred before this
     * contract will start reverting on transfer.
     */
    function nextTransferFailure() external view returns (uint256);

    /**
     * @dev Sets the number of times tokens can be transferred before this
     * contract will start reverting on transfer.
     * @param successes the number that can be transferred.
     */
    function failAfterTransferring(uint256 successes) external;

    /**
     * @dev returns the number of permits that can be granted before this
     * contract will start reverting on permit.
     */
    function nextPermitFailure() external view returns (uint256);

    /**
     * @dev Sets the number of permits that can be granted before this
     * contract will start reverting on permit.
     * @param successes the number that can be transferred.
     */
    function failAfterPermitting(uint256 successes) external;
}

contract FailCoinHybridPlugin is TransferValidationHybridPlugin, FailCoinEventsErrors {
    bytes4 constant NEXT_TRANSFER_FAIL =
        IFailCoinHybridPlugin.nextTransferFailure.selector;
    bytes4 constant TRANSFER_FAIL_AFTER =
        IFailCoinHybridPlugin.failAfterTransferring.selector;
    bytes4 constant NEXT_PERMIT_FAIL =
        IFailCoinHybridPlugin.nextPermitFailure.selector;
    bytes4 constant PERMIT_FAIL_AFTER =
        IFailCoinHybridPlugin.failAfterPermitting.selector;
    bytes4 constant PERMIT = IERC20Permit.permit.selector;

    struct FailCoinStruct {
        uint256 transferFailIndex;
        uint256 transferCount;
        uint256 permitFailIndex;
        uint256 permitCount;
    }

    mapping(address => FailCoinStruct) internal failParams;

    function _onInstall(bytes calldata) internal virtual override {
        FailCoinStruct storage params = failParams[msg.sender];
        params
            .transferFailIndex = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
        params
            .permitFailIndex = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
    }
    ...
    {
        funcs = new bytes4[](8);
        funcs[0] = TRANSFER_FAIL_AFTER;
        funcs[1] = PERMIT_FAIL_AFTER;
        funcs[2] = PERMIT;
        funcs[3] = TRANSFER;
        funcs[4] = TRANSFER_FROM;
        funcs[5] = TRANSFER_CALL;
        funcs[6] = RECEIVED_FROM_BRIDGE;
        funcs[7] = SENT_TO_BRIDGE;
    }
    ...
    function _beforeHook(
        ...
        if (_msg.sig == PERMIT) {
            return checkPermit(msg.sender) ? TRUE : FALSE;
        }

        if (_msg.sig != TRANSFER_FAIL_AFTER && _msg.sig != PERMIT_FAIL_AFTER) {
    ...
    {
        funcs = new bytes4[](4);
        funcs[0] = NEXT_TRANSFER_FAIL;
        funcs[1] = NEXT_PERMIT_FAIL;
        funcs[2] = TRANSFER_FAIL_AFTER;
        funcs[3] = PERMIT_FAIL_AFTER;
    }
    ...
    function _execute(
        ...
        if (_msg.sig == NEXT_TRANSFER_FAIL) {
            if (params.transferCount >= params.transferFailIndex) {
                return abi.encode(0);
            }
            return abi.encode(params.transferFailIndex - params.transferCount);
        }

        if (_msg.sig == NEXT_PERMIT_FAIL) {
            if (params.permitCount >= params.permitFailIndex) {
                return abi.encode(0);
            }
            return abi.encode(params.permitFailIndex - params.permitCount);
        }

        if (_msg.sig == TRANSFER_FAIL_AFTER) {
            uint256 successes = abi.decode(_msg.data[4:], (uint256));
            params.transferFailIndex = params.transferCount + successes;
            return TRUE;
        }

        if (_msg.sig == PERMIT_FAIL_AFTER) {
            uint256 successes = abi.decode(_msg.data[4:], (uint256));
            params.permitFailIndex = params.permitCount + successes;
            return TRUE;
        }
    ...
    function checkPermit(address token) internal virtual returns (bool) {
        FailCoinStruct storage params = failParams[token];
        if (params.permitCount >= params.permitFailIndex) {
            revert WierdIntermittentBlockchainErrorOnPermit();
        }
        params.permitCount++;
        return true;
    }

    function checkTransfer(address token, address, address, address, uint256) internal virtual override returns (bool) {
        FailCoinStruct storage params = failParams[token];
        if (params.transferCount >= params.transferFailIndex) {
            revert WierdIntermittentBlockchainErrorOnERC20Transfer();
        }
