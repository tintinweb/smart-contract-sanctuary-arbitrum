...
contract TestDistributor is Initializable, AccessControlEnumerableUpgradeable, UUPSUpgradeable {
    event GameAdded(address indexed game);
    event GameRemoved(address indexed game);
    event LockIdUpdated(uint256 indexed lockId);
    event PinkLockUpdated(address indexed pinkLock);

    using EnumerableSet for *;

    Currency currency;
    address public pinkLock;
    uint256 public lockId;

    EnumerableSet.AddressSet private _games;
    mapping(address => uint256) public _rewardsAmounts;
    ...
    function initialize(
        ...
        _grantRole(Roles.OPERATOR_ROLE, initialAdmin);
        _games.add(game);
        _rewardsAmounts[game] = 180000000 * 10e18;
        pinkLock = pinkLock_;
        lockId = lockId_;
        currency = currency_;
    }

    // view game and rewards
    function games() external view returns (address[] memory, uint256[] memory result) {
        result = new uint256[](_games.length());
        for (uint256 i; i < _games.length(); i++) {
            result[i] = _rewardsAmounts[_games.at(i)];
        }

        return (_games.values(), result);
    }

    function addGame(address game, uint256 rewardAmounts) external onlyRole(Roles.OPERATOR_ROLE) {
        _games.add(game);
        _rewardsAmounts[game] = rewardAmounts;
        emit GameAdded(game);
    }

    function removeGame(address game) external onlyRole(Roles.OPERATOR_ROLE) {
        _games.remove(game);
        emit GameRemoved(game);
    }

    // function setLockId(uint256 lockId_) external onlyRole(Roles.OPERATOR_ROLE) {
    //     lockId = lockId_;
    //     emit LockIdUpdated(lockId);
    // }

    // function setPinkLock(address pinkLock_) external onlyRole(Roles.OPERATOR_ROLE) {
    //     pinkLock = pinkLock_;
    //     emit PinkLockUpdated(pinkLock);
    // }

    function transferLockOwnership(address newOwner) external onlyRole(Roles.OPERATOR_ROLE) {
        IPinkLock(pinkLock).transferLockOwnership(lockId, newOwner);
    }

    function unlock() external onlyRole(Roles.OPERATOR_ROLE) {
        IPinkLock(pinkLock).unlock(lockId);
    }

    function distributeIncentive() external onlyRole(Roles.OPERATOR_ROLE) {
        IPinkLock(pinkLock).unlock(lockId);
        _distribute();
    }
    ...
    function _distribute() internal {
        address game;
        uint256 amount;
        for (uint256 i; i < _games.values().length; ) {
            game = _games.at(i);
            amount = _rewardsAmounts[game];
            currency.transfer(game, amount);
            unchecked {
                ++i;
            }
        }
