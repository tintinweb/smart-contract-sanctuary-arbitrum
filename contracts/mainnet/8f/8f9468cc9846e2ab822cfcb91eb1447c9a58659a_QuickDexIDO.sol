...
contract QuickDexIDO is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public constant quickPrice = 1e4; //0.01 U
    ...
    // The offering token(QUICK & esQUICK)
    ...
    event Harvest(address indexed user, uint256 offeringAmount, uint256 quickOfferingAmount,uint256 excessAmount);
    ...
    constructor(
        ...
        offeringToken0 = _offeringToken0;//Quick & esQuick
        offeringToken1 =_offeringToken1;
        startTime = 1680714900;
        endTime  = 1680974100;
        offeringAmount =  100000000 * 10**18;// _offeringAmount;give to user token: 100,000,000（34,000,000QUICK + 66,000,000esQUICK）
        raisingAmount= 1000000* 10**6;  //_raisingAmount;raise USDC

        adminAddress = msg.sender;
        finalleftWithAddr = 0x71313c13DabFe05b17681708803dd58d6f93317e;
        totalAmount = 0;
        calcDuration = 3600*2;
    }
    ...
    function harvest() public nonReentrant {
        ...
        uint256 quickOfferingAmount;
        if(offeringTokenAmount > 0 ){
            quickOfferingAmount = offeringTokenAmount.mul(offer0Ratio).div(100);
            offeringToken0.safeTransfer(address(msg.sender), quickOfferingAmount);
            offeringToken1.safeTransfer(address(msg.sender), offeringTokenAmount.sub(quickOfferingAmount));
        }
        if (refundingTokenAmount > 0){
            raisingToken.safeTransfer(address(msg.sender), refundingTokenAmount);
        }
        userInfo[msg.sender].claimed = true;
        emit Harvest(msg.sender, offeringTokenAmount,quickOfferingAmount,refundingTokenAmount);
    }
    ...
    function finalWithdrawOfferingTokenEx(uint256 _offer0Amount,uint256 _offer1Amount) public onlyAdmin {
        require (block.timestamp > endTime.add(calcDuration), 'not withdraw OfferingToken time');
        if(totalAmount < raisingAmount){
            uint256 withDrawOfferAmount  = (raisingAmount.sub(totalAmount).mul(1e18).div(quickPrice));
    ...
    function getIDOAmounts() public view returns(uint256 raiseAmount,uint256 offerAmount0,uint256 offerAmount1,uint256 userTotalAmout){
        ...
        offerAmount0 = offeringAmount.mul(offer0Ratio).div(100); // QUICK
        offerAmount1 = offeringAmount.sub(offerAmount0); // esQUICK
        userTotalAmout = totalAmount; //user USDC
    }

    function getQuickPrice() public pure returns(uint256){
        return quickPrice;
    }
