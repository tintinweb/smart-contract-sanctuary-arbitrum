...
// File: contracts/BIFI/strategies/Beethovenx/StrategyBalancerComposableMultiRewardGaugeUniV3.sol
...
contract StrategyBalancerComposableMultiRewardGaugeUniV3 is StratFeeManager {
    ...
    function addLiquidity() internal {
        uint256 nativeBal = IERC20(native).balanceOf(address(this));
        IBalancerVault.BatchSwapStep[] memory _swaps = buildSwapStructArray(nativeToWantRoute, nativeBal);
        balancerSwap(_swaps, nativeToWantAssets, int256(nativeBal));
    }
    ...
    function _swapV3(bytes memory _path, uint256 _amount) internal returns (uint256 amountOut) {
        ...
        return IKyberElastic(uniswapRouter).swapExactInput(swapParams);
    }
