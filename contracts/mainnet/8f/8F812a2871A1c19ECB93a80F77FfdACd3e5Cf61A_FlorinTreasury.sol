...
/**
 * @title FlorinTreasury
 * @dev The FlorinTreasury is responsible for the minting and redeeming of Florin tokens, and for holding a deposit of EUR-pegged tokens.
 *
 * Important Considerations:
 * 1. The contract supports a pause and unpause feature. All core functionalities like minting, redeeming, and depositing are affected.
 * 2. It has role-based access control, governed by the OpenZeppelin's AccessControlUpgradeable contract.
 * 3. This contract allows the changing of the EUR token contract; only tokens pegged to the EUR should be used.
 * 4. Anyone can mint FLR against EUR tokens or redeem FLR for EUR tokens.
 *    Minting of Florin tokens without direct deposit of EUR token can only be done by an account with the LOAN_VAULT_ROLE. (See LoanVaults calls of FlorinTreasury.)
 * 5. Decimals for the EUR tokens and the Florin tokens might differ; conversion happens internally.
 *
 * Security Measures:
 * - The contract uses OpenZeppelin's PausableUpgradeable to implement circuit-breaker functionality.
 * - OpenZeppelin's SafeERC20Upgradeable is used for all ERC20 token interactions.
 */
...
contract FlorinTreasury is AccessControlUpgradeable, PausableUpgradeable {
    ...
    event SetEurToken(IERC20Upgradeable eurToken);
    event MintAgainstEUR(address from, uint256 eurTokens, uint256 florinTokens);

    FlorinToken public florinToken;

    IERC20MetadataUpgradeable public eurToken;
    ...
    /**
     * @dev Initializes the FlorinTreasury contract with the provided FlorinToken and EURToken addresses.
     * Grants the DEFAULT_ADMIN_ROLE to the account that deploys the contract.
     * This is an initializer function and replaces the constructor for upgradeable contracts.
     * It initializes AccessControl and Pausable functionalities and sets the initial Florin and EUR token contracts.
     *
     * Requirements:
     * - Can only be called once.
     *
     * @param florinToken_ The address of the FlorinToken contract to be set.
     * @param eurToken_ The address of the ERC20-compatible EUR-pegged token to be used.
     */
    function initialize(FlorinToken florinToken_, IERC20MetadataUpgradeable eurToken_) external initializer {
    ...
    /// @dev Sets the EUR token used for redemption. (Do not use tokens here that are not pegged to EUR.)
    /// @param eurToken_ the EUR token to be used
    function setEurToken(IERC20MetadataUpgradeable eurToken_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        eurToken = eurToken_;
        emit SetEurToken(eurToken_);
    }
    ...
    function mint(address receiver, uint256 florinTokens) external whenNotPaused onlyRole(LOAN_VAULT_ROLE) {
        florinToken.mint(receiver, florinTokens);
        emit Mint(msg.sender, receiver, florinTokens);
    }
    ...
    function redeem(uint256 florinTokens) external whenNotPaused {
        florinToken.burnFrom(msg.sender, florinTokens);
        uint256 eurTokens = Util.convertDecimalsERC20(florinTokens, florinToken, eurToken);
        eurToken.safeTransfer(msg.sender, eurTokens);
        emit Redeem(msg.sender, florinTokens, eurTokens);
    }

    /// @dev Mints Florin Token to msg.sender against EUR (only when not paused)
    /// @param eurTokens amount of EUR tokens to be minted Florin for (eurToken.decimals() decimals)
    function mintAgainstEUR(uint256 eurTokens) external whenNotPaused {
        if (eurTokens == 0) {
            revert Errors.TransferAmountMustBeGreaterThanZero();
        }

        uint256 flrTokens = Util.convertDecimals(eurTokens, eurToken.decimals(), 18);

        eurToken.safeTransferFrom(msg.sender, address(this), eurTokens);

        florinToken.mint(msg.sender, flrTokens);

        emit MintAgainstEUR(msg.sender, eurTokens, flrTokens);
    }
    ...
    function transferFlorinTokenOwnership(address newOwner) external onlyRole(DEFAULT_ADMIN_ROLE) {
