...
contract Accounts is ConfirmedOwner {
    ...
    function getRevenue() private view returns (uint256) {
        return address(this).balance - totalBalance;
    }

    function revenue() external view onlyOwner returns (uint256) {
        return getRevenue();
    }

    function balance() external view returns (uint256) {
        return balances[msg.sender] + testBalances[msg.sender];
    }

    function balanceUser(address user) external view onlyOwner returns (uint256) {
        return balances[user];
    }

    function balanceTestUser(address user) external view onlyOwner returns (uint256) {
        return testBalances[user];
    }

    function totalBalanceUser(address user) external view onlyOwner returns (uint256) {
        return balances[user] + testBalances[user];
    }

    function totalBalanceUsers() external view onlyOwner returns (uint256) {
        return totalBalance;
    }
    ...
    function withdrawMaximum() external view returns (uint256) {
        return balances[msg.sender];
    }

    function withdrawOwner(uint256 amount, address recipient) external onlyOwner {
    ...
    function removeTestBalance(address user, uint256 amount, address recipient) external onlyOwner {
        uint256 ethBalance = testBalances[user];
        _withdraw(amount, recipient, ethBalance);
        testBalances[msg.sender] -= amount;
        totalBalance -= amount;
    }
