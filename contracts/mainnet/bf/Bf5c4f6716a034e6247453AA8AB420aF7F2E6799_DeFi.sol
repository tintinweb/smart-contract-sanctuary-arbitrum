...
contract TradingStrategy is ERC721A, DoubleOwnable {
    ...
    uint256 public immutable PUBLIC_SOFT_CAP;
    uint8 public maxBuy = 20;
    uint256 public immutable PRICE;
    bool public mintingOn = false;
    ...
    constructor(
        ...
        uint256 _publicSoftCap,
        uint256 _price
    ) ERC721A(_name, _symbol) DoubleOwnable(_secondOwner) {
        isRewardExempt[address(this)] = true;
        isRewardExempt[0x000000000000000000000000000000000000dEaD] = true;
        isRewardExempt[0x0000000000000000000000000000000000000000] = true;
        protocolWallet = _protocolWallet;
        // stablecoin = IERC20(_stablecoin);
        // stablecoinDecimals = _stablecoinDecimals;
        stablecoin = IERC20(0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9); // USDT
        stablecoinDecimals = 6;
        PUBLIC_SOFT_CAP = _publicSoftCap;
        PRICE = _price;
    }
    ...
    function _afterTokenTransfers(
        ...
        if (!isRewardExempt[from]) {
            bool success = rewardDistributor.setAllocation(
                from,
                balanceOf(from)
            );
            require(success, "setAllocation failed");
        }

        if (!isRewardExempt[to]) {
            bool success = rewardDistributor.setAllocation(to, balanceOf(to));
            require(success, "setAllocation failed");
        }
    }
    ...
    function publicMint(uint256 quantity) public {
        ...
        require(quantity <= maxBuy, "quantity exceeds max buy");
        require(mintingOn, "Minting is currently disabled");
        require(
            totalSupply() + quantity <= PUBLIC_SOFT_CAP,
            "amount would exceed max supply"
        );
        uint256 finalPrice = (PRICE * (10 ** stablecoinDecimals)) * quantity;
        require(
            stablecoin.balanceOf(_msgSender()) >= finalPrice,
            "insufficient balance"
        );

        stablecoin.transferFrom(_msgSender(), protocolWallet, finalPrice);
    ...
    function burn(uint256 tokenId) public onlyOwner {
        ...
        // Reward tracker
        if (!isRewardExempt[previousOwner]) {
            bool success = rewardDistributor.setAllocation(
                previousOwner,
    ...
    function withdrawStablecoin() public onlyOwner {
        ...
        stablecoin.transfer(_msgSender(), balance);
    }
    ...
    function setRewardDistributor(address rd) public onlyOwner {
        rewardDistributor = IRewardDistributor(rd);
    }
    ...
    function changeMaxBuy(uint8 newMaxBuy) public onlyOwner {
        maxBuy = newMaxBuy;
    }
...
contract DeFi is TradingStrategy {
    constructor()
        TradingStrategy(
            0xb5c71301914302862d63f55DbC25eE48f4B097A4,
            0x6C2504E9538EDC2D4522FD84C637D2D379d4604a,
            "DEFI",
            "DEFI",
            4000,
            300
