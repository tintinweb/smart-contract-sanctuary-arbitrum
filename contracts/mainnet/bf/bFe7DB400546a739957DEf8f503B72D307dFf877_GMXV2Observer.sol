...
contract GMXV2Observer is IObserver, Ownable {
    ...
    function _evaluateIncreaseOrders(
        ...
        for(uint256 i = 0; i < orders.length; i++) {
            if(
                orders[i].numbers.orderType == IGmxV2Order.OrderType.Liquidation ||
                orders[i].numbers.orderType == IGmxV2Order.OrderType.LimitDecrease ||
                orders[i].numbers.orderType == IGmxV2Order.OrderType.StopLossDecrease
            ) continue;
