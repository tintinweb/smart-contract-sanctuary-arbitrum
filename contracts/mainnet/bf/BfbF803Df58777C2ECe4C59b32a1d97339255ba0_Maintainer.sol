...
contract Maintainer is Ownable {
    ...
    EnumerableSet.AddressSet private firstResponders;
    event FirstResponseAdded(address indexed newFirstResponse);
    event FirstResponseRemoved(address indexed newFirstResponse);
    event ManagerChanged(address newManager);
    event OracleFactoryChanged(address newFactory);
    event Delegated(string indexed func);
    event SetOracleMarketClosed(bytes4 baseCurrency, bytes4 quoteCurrency, bool marketClosed);

    /**
     * Constructor
     * @param _manager perpetual proxy manager address
     * @param _firstResponse first response address
     */
    constructor(address payable _manager, address _firstResponse) {
        require(_manager != address(0), "zero addr");
        _setMntnrManager(_manager);
        _setMntnrOracleFactory(IPerpetualManager(_manager).getOracleFactory());
        _addMntnrFirstResponse(_firstResponse);
    }

    /**
     * Set the address of first response
     * @param _firstResponse address
     * @dev Maintainer.test
     */
    function addMntnrFirstResponse(address _firstResponse) external onlyOwner {
        _addMntnrFirstResponse(_firstResponse);
    }

    /**
     * Remove first responder
     * @param _firstResponse address of first responder to remove
     * @dev Maintainer.test
     */
    function removeMntnrFirstResponse(address _firstResponse) external onlyOwner {
        _removeMntnrFirstResponse(_firstResponse);
    }

    function _addMntnrFirstResponse(address _firstResponse) internal {
        require(_firstResponse != address(0), "zero addr");
        firstResponders.add(_firstResponse);
        emit FirstResponseAdded(_firstResponse);
    }
     ...
     * @dev Maintainer.test
     */
    function firstReponders() external view returns (address[] memory) {
        return firstResponders.values();
    }

    function _removeMntnrFirstResponse(address _firstResponse) internal {
        require(_firstResponse != address(0), "zero addr");
        firstResponders.remove(_firstResponse);
        emit FirstResponseRemoved(_firstResponse);
    }

    /**
     * Set address of manager
     * @param _manager perpetual manager proxy address
     */
    function setMntnrManager(address payable _manager) external onlyOwner {
        _setMntnrManager(_manager);
    }

    function _setMntnrManager(address payable _manager) internal {
        require(_manager != address(0), "zro address");
        require(_manager != manager, "no change");
        manager = _manager;
        emit ManagerChanged(_manager);
    }

    /**
     * Set address of oracle factory
     * @param _oracleFactory oracle factory address
     */
    function setMntnrOracleFactory(address _oracleFactory) external onlyOwner {
        _setMntnrOracleFactory(_oracleFactory);
    }
     ...
     * @param _oracleFactory oracle factory address
     */
    function _setMntnrOracleFactory(address _oracleFactory) internal {
        require(_oracleFactory != address(0), "zro address");
        require(_oracleFactory != oracleFactory, "no change");
        oracleFactory = _oracleFactory;
        emit OracleFactoryChanged(_oracleFactory);
    }

    /**
     * Pause of Perpetual can be initiated by owner or first response address
     * @dev PerpetualManager.test, Maintainer.test
     */
    function pause() external {
        require(msg.sender == owner() || firstResponders.contains(msg.sender), "only owner/1resp");
        PerpetualManagerProxy(manager).pause();
        emit Delegated("pause");
    }

    /**
     * delegated unpause
     * @dev PerpetualManager.test
     */
    function unpause() external onlyOwner {
        PerpetualManagerProxy(manager).unpause();
        emit Delegated("unpause");
    }

    // @dev OracleFactory.test
    function setMarketClosed(
        ...
        require(msg.sender == owner() || firstResponders.contains(msg.sender), "only owner/1resp");
        OracleFactory(oracleFactory).setMarketClosed(_baseCurrency, _quoteCurrency, _marketClosed);
        emit SetOracleMarketClosed(_baseCurrency, _quoteCurrency, _marketClosed);
    }

    /**
     * Set the oracle factory address to convert the traded volume into
     * USD on BrokerFeeLogic
     * @dev PerpetualBrokerDeposit.test
     * @param _iPerpetualId id of perpetual
     * @param _oracleFactoryAddr address for oracle factory
     */
    function setOracleFactoryForPerpetual(
        uint24 _iPerpetualId,
        address _oracleFactoryAddr
    ) external onlyOwner {
        IPerpetualManager(manager).setOracleFactoryForPerpetual(_iPerpetualId, _oracleFactoryAddr);
        emit Delegated("setOracleFactoryForPerpetual");
    }

    // @dev PerpetualBrokerFee.test
    function setUtilityTokenAddr(address _tokenAddr) external onlyOwner {
        IPerpetualManager(manager).setUtilityTokenAddr(_tokenAddr);
        emit Delegated("setUtilityTokenAddr");
    }

    // @dev PerpetualBrokerFee.test
    function setFeesForDesignation(
        uint32[] calldata _designations,
        uint16[] calldata _feesTbps
    ) external onlyOwner {
        IPerpetualManager(manager).setFeesForDesignation(_designations, _feesTbps);
        emit Delegated("setFeesForDesignation");
    }

    // @dev PerpetualBrokerFee.test
    function setBrokerTiers(
        uint256[] calldata _tiers,
        uint16[] calldata _feesTbps
    ) external onlyOwner {
        IPerpetualManager(manager).setBrokerTiers(_tiers, _feesTbps);
        emit Delegated("setBrokerTiers");
    }

    // @dev PerpetualBrokerFee.test
    function setTraderTiers(
        uint256[] calldata _tiers,
        uint16[] calldata _feesTbps
    ) external onlyOwner {
        IPerpetualManager(manager).setTraderTiers(_tiers, _feesTbps);
        emit Delegated("setTraderTiers");
    }

    // @dev PerpetualBrokerFee.test
    function setTraderVolumeTiers(
        uint256[] calldata _tiers,
        uint16[] calldata _feesTbps
    ) external onlyOwner {
        IPerpetualManager(manager).setTraderVolumeTiers(_tiers, _feesTbps);
        emit Delegated("setTraderVolumeTiers");
    }

    // @dev PerpetualBrokerFee.test
    function setBrokerVolumeTiers(
        uint256[] calldata _tiers,
        uint16[] calldata _feesTbps
    ) external onlyOwner {
        IPerpetualManager(manager).setBrokerVolumeTiers(_tiers, _feesTbps);
        emit Delegated("setBrokerVolumeTiers");
    }

    // @dev PerpetualManager.test
    function depositToDefaultFund(uint8 _poolId, int128 _fAmount) external onlyOwner {
        uint8 count = IPerpetualManager(manager).getPoolCount();
        require(_poolId > 0 && _poolId <= count, "pool index out of range");
        PerpStorage.LiquidityPoolData memory lp = IPerpetualManager(manager).getLiquidityPool(
            _poolId
        );
        uint256 amountWei = ConverterDec18.toUDecN(_fAmount, lp.marginTokenDecimals);
        IERC20Upgradeable marginToken = IERC20Upgradeable(lp.marginTokenAddress);
        marginToken.approve(manager, amountWei);
        marginToken.safeTransferFrom(msg.sender, address(this), amountWei);
        IPerpetualManager(manager).depositToDefaultFund(_poolId, _fAmount);
        emit Delegated("depositToDefaultFund");
    }

    // @dev PerpetualManager.test
    function withdrawFromDefaultFund(uint8 _poolId, int128 _fAmount) external onlyOwner {
        PerpStorage.LiquidityPoolData memory lp = IPerpetualManager(manager).getLiquidityPool(
            _poolId
        );
        IERC20Upgradeable marginToken = IERC20Upgradeable(lp.marginTokenAddress);
        IPerpetualManager(manager).withdrawFromDefaultFund(_poolId, _fAmount);
        uint256 postBalance = marginToken.balanceOf(address(this));
        marginToken.safeTransferFrom(address(this), msg.sender, postBalance);
        emit Delegated("withdrawFromDefaultFund");
    }
    ...
    function createLiquidityPool(
        ...
        uint8 r = IPerpetualManager(manager).createLiquidityPool(
            _marginTokenAddress,
            _iTargetPoolSizeUpdateTime,
            _fBrokerCollateralLotSize,
            _fCeilPnLShare
        );
        emit Delegated("createLiquidityPool");
        return r;
    }

    // @dev PerpetualManager.test
    function setPerpetualPoolFactory(address _shareTokenFactory) external onlyOwner {
        IPerpetualManager(manager).setPerpetualPoolFactory(_shareTokenFactory);
        emit Delegated("setPerpetualPoolFactory");
    }

    // @dev PerpetualManager.test
    function runLiquidityPool(uint8 _liqPoolID) external onlyOwner {
        IPerpetualManager(manager).runLiquidityPool(_liqPoolID);
        emit Delegated("runLiquidityPool");
    }
    ...
    function createPerpetual(
        ...
        IPerpetualManager(manager).createPerpetual(
            _iPoolId,
            _baseQuoteS2,
            _baseQuoteS3,
            _baseParams,
        ...
        emit Delegated("createPerpetual");
    }
    ...
    function setPerpetualOracles(
        ...
        IPerpetualManager(manager).setPerpetualOracles(_iPerpetualId, _baseQuoteS2, _baseQuoteS3);
        emit Delegated("setPerpetualOracles");
    }

    // @dev PerpetualManager.test
    function setPerpetualBaseParams(
        uint24 _iPerpetualId,
        int128[7] calldata _baseParams
    ) external onlyOwner {
        IPerpetualManager(manager).setPerpetualBaseParams(_iPerpetualId, _baseParams);
        emit Delegated("setPerpetualBaseParams");
    }
    ...
    function setPerpetualRiskParams(
        ...
        IPerpetualManager(manager).setPerpetualRiskParams(
            _iPerpetualId,
            _underlyingRiskParams,
            _defaultFundRiskParams
        );
        emit Delegated("setPerpetualRiskParams");
    }

    // @dev PerpetualTradeIT.test
    function activatePerpetual(uint24 _perpetualId) external onlyOwner {
        IPerpetualManager(manager).activatePerpetual(_perpetualId);
        emit Delegated("activatePerpetual");
    }
    ...
    function setPerpetualParam(
        ...
        IPerpetualManager(manager).setPerpetualParam(_iPerpetualId, _name, _value);
        emit Delegated("setPerpetualParam");
    }
    ...
    function setPerpetualParamPair(
        ...
        IPerpetualManager(manager).setPerpetualParamPair(_iPerpetualId, _name, _value1, _value2);
        emit Delegated("setPerpetualParamPair");
    }

    // @dev PerpetualManager.test
    function setPoolParam(uint8 _poolId, string memory _name, int128 _value) external onlyOwner {
        IPerpetualManager(manager).setPoolParam(_poolId, _name, _value);
        emit Delegated("setPoolParam");
    }

    // @dev PerpetualTreasury.test
    function togglePerpEmergencyState(uint24 _perpetualId) external onlyOwner {
        IPerpetualManager(manager).togglePerpEmergencyState(_perpetualId);
        emit Delegated("togglePerpEmergencyState");
    }

    // @dev PerpetualTradeLogic.test
    function transferEarningsToTreasury(uint8 _poolId, int128 _fAmount) external onlyOwner {
        IPerpetualManager(manager).transferEarningsToTreasury(_poolId, _fAmount);
        emit Delegated("transferEarningsToTreasury");
    }

    function deactivatePerp(uint24 _perpetualId) external onlyOwner {
        IPerpetualManager(manager).deactivatePerp(_perpetualId);
        emit Delegated("deactivatePerp");
    }

    // @dev PerpetualTreasury.test
    function pauseLiquidityProvision(uint8 _poolId, bool _pauseOn) external onlyOwner {
        IPerpetualManager(manager).pauseLiquidityProvision(_poolId, _pauseOn);
        emit Delegated("pauseLiquidityProvision");
    }

    // @dev PerpetualManager.test
    function setBlockDelay(uint8 _delay) external onlyOwner {
        IPerpetualManager(manager).setBlockDelay(_delay);
        emit Delegated("setBlockDelay");
    }

    // @dev PerpetualManager.test
    function setNormalState(uint24 _iPerpetualId) external onlyOwner {
        IPerpetualManager(manager).setNormalState(_iPerpetualId);
        emit Delegated("setNormalState");
    }

    // @dev PerpetualManager.test
    function setEmergencyState(uint24 _iPerpetualId) external onlyOwner {
        IPerpetualManager(manager).setEmergencyState(_iPerpetualId);
        emit Delegated("setEmergencyState");
    }
}
