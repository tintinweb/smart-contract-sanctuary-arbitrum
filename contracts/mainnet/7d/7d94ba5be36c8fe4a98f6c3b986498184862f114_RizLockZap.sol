...
contract RizRegistry is Ownable {
    using SafeERC20 for IERC20;
    using Address for address;
    ...
    address private badDebtManagerImpl;
    ...
    event ImplementationSet(bytes32 id, address implementation, address addressProvider);
    ...
    function addPool(string calldata marketId_) external onlyOwner {
        RizLendingPoolAddressesProvider addressProvider = new RizLendingPoolAddressesProvider(marketId_);
        ...
        address badDebtManager = Clones.clone(badDebtManagerImpl);
    ...
    function setImplementation(bytes32 implId, address implAddress) external onlyOwner {
    ...
    /// @notice Sets bad debt manager implementation to be used by Clones library
    /// @param _badDebtManagerImpl The address of the bad debt manager implementation
    function setBadDebtManagerImpl(address _badDebtManagerImpl) external onlyOwner {
        // Make sure the bad debt manager is a contract
        if (!_badDebtManagerImpl.isContract()) {
            revert Errors.NotAContract();
        }
        badDebtManagerImpl = _badDebtManagerImpl;
        emit BadDebtManagerSet(_badDebtManagerImpl);
    }
