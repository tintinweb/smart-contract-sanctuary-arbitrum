...
contract PositionReader {
    ...
    function getUserPositions(address _vault, address _account) external view returns (DispPosition[] memory){
        ...
        for(uint256 i = 0; i < _keys.length; i++){
            ...
            _dps[i].indexTokenMinPrice = IVaultPriceFeed(priceFeed).getPriceUnsafe(position.indexToken, false, false, false);
            _dps[i].indexTokenMaxPrice = IVaultPriceFeed(priceFeed).getPriceUnsafe(position.indexToken, true, false, false);
        }
        return _dps;
    }
    ...
    function getTokenInfo(address _vault, address[] memory _fundTokens) external view returns (DispToken[] memory) {
        ...
        for(uint256 i = 0; i < _dispT.length; i++){
            ...
            _dispT[i].poolSize = vault.isFundingToken(_fundTokens[i]) ? IVaultPriceFeed(priceFeed).tokenToUsdUnsafe(_fundTokens[i], _tBase.poolAmount, false) : 0;
            _dispT[i].poolSize = _dispT[i].poolSize > _dispT[i].guaranteedUsd ? _dispT[i].poolSize.sub(_dispT[i].guaranteedUsd) : 0;
