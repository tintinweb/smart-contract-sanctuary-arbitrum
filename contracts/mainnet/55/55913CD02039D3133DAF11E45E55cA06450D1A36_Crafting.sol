...
contract Crafting is ICrafting, ManagerModifier, ReentrancyGuard, Pausable {
  ...
  function dynamicConfigCache() public returns (DynamicCraftingConfigCache memory) {
  ...
  function maximumRealmCraftsAvailableView() public view returns (uint[] memory) {
  ...
  function craftingCostsView() external view returns (CraftingCost[] memory result) {
    ...
    for (uint i = 0; i < result.length; i++) {
      ...
      result[i].void =
        (result[i].realm * uint(_dynamicConfigCache.voidCraftingCostPercentage[i])) /
        ONE_HUNDRED;
    }
  }
  ...
  function _affectRealms(uint[] memory realmCrafts, uint randomBase) internal {
    ...
    uint[] memory memLabIds = supportedLabIds;
    ...
    uint[][] memory labIds;
    for (uint i = 0; i < memLabIds.length; i++) {
      if (realmCrafts[i] == 0) {
        continue;
      }
      realmAddresses = ArrayUtils.toMemoryArray(REALM_ADDRESS, realmCrafts[i]);
      labIds = new uint[][](realmIds[i].length);
      for (uint j = 0; j < realmIds[i].length; j++) {
        labIds[j] = ArrayUtils.toMemoryArray(memLabIds[i], 1);
      }
      ...
      REALM_ARMORY.reduceDurabilityBatch(
        ...
        LAB_ADDRESS,
        labIds,
        _dynamicConfigCache.durabilityLossPercentage[i],
        true,
        true
      );
      ...
      PRODUCTIVITY_STORAGE.changeBatch(
        realmIds[i],
        int(uint(_dynamicConfigCache.productivityPerCraft[i])),
        true
      );

      uint animaCapacity = labConfig[memLabIds[i]].animaCapacity;
      for (uint j = 0; j < realmIds[i].length; j++) {
