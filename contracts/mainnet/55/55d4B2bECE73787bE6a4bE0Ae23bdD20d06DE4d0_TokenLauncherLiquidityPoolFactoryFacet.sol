...
import { LibTokenLauncherLiquidityPoolFactoryStorage } from "../libraries/TokenLauncherLiquidityPoolFactoryStorage.sol";
...
contract TokenLauncherLiquidityPoolFactoryFacet is ITokenLauncherLiquidityPoolFactory {
    ...
    function createTokenLauncherV2LiquidityPool(
        ITokenLauncherLiquidityPoolFactory.CreateV2LpTlInput memory input
    ) external payable override onlyCreateLiquidityPoolRole {
        ...
        emit TokenLiquidityCreated(msg.sender, input.createV2LpInput.liquidityPoolDetails);
    }

    function createV2LiquidityPool(CreateV2LpInput memory input) external payable override onlyCreateLiquidityPoolRole {
    ...
    function _registerLiquidityPool(RegisterLiquidityPoolInput memory input) private {
        ...
        emit LiquidityPoolCreated(ds.currentBlockLiquidityPoolCreated, input.liquidityPoolToken, input.tokenAddress);
        ds.currentBlockLiquidityPoolCreated = block.number;
    }
    ...
    function _lockLiquidity(CreateV2LpInput memory input, address pairAddress) private {
        ...
        require(
            input.lockLPDetails.lockLPTokenPercentage <= LibCommonConsts.MULTIPLIER_BASIS,
            "TokenLauncherLiquidityPoolFactoryFacet: INVALID_LOCK_PERCENTAGE"
        );

        uint256 lpBalanceToLock = (IERC20(pairAddress).balanceOf(address(this)) * input.lockLPDetails.lockLPTokenPercentage) / LibCommonConsts.MULTIPLIER_BASIS;
    ...
    modifier onlyCreateLiquidityPoolRole() {
        require(
            IAccessControl(address(this)).hasRole(LibTokenLauncherConsts.CREATE_LIQUIDITY_POOL_ROLE, msg.sender),
            "TokenLauncherLiquidityPoolFactoryFacet: Only token owner can call this function"
        );
        _;
    }
}
...
library LibTokenLauncherLiquidityPoolFactoryStorage {
    ...
    struct DiamondStorage {
        ...
        uint256 currentBlockLiquidityPoolCreated;
