...
/**
 * @title pALEO Pool Contract
 * @author GAST-TEAM
 * @notice In this contract user can harvest pALEO
 */
contract pALEOPool is AccessControlEnumerable, ReentrancyGuard {
    ...
    event SetAddrs(address paleoAddr);
    ...
    function setAddrs(
        ...
        pALEO = IERC20(paleoAddr);

        emit SetAddrs(paleoAddr);
    }
    ...
    function managerDeposit(
        ...
        for (uint256 i = 0; i < users.length; i++) {
            if (userStakes[users[i]] > 0) {
                uint256 pendingToken = (userStakes[users[i]] *
                    (accTokenPerStake - userLastAccTokenPerStake[users[i]])) /
                    1e28;
                if (pendingToken > 0) {
                    userStoredToken[users[i]] += pendingToken;
                }
            }

            if (amounts[i] > 0) {
                userStakes[users[i]] += amounts[i];
                stakes += amounts[i];

                userLastStakeTime[users[i]] = block.timestamp;
            }

            userLastAccTokenPerStake[users[i]] = accTokenPerStake;
    ...
    function harvest() external nonReentrant {
        ...
        uint256 pendingToken = (userStakes[msg.sender] *
            (accTokenPerStake - userLastAccTokenPerStake[msg.sender])) / 1e28;
        ...
        pALEO.safeTransfer(msg.sender, amount);
    ...
    function updatePool() public {
        ...
        if (block.number > lastRewardBlock && stakes > 0) {
            ...
            accTokenPerStake += (amount * 1e28) / stakes;
    ...
    function getTokenRewards(address user) public view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakes > 0) {
            accTokenPerStakeTemp +=
                (tokenPerBlock * (block.number - lastRewardBlock) * 1e28) /
                stakes;
        }
