...
contract ArgusAccountHelper is ArgusAuthorizerHelper, IVersion {
    ...
    function addFuncAuthorizer(
        ...
        // 2. Set variants
        addFuncAuthorizerVariants(authorizerAddress, _contracts, funcLists);
    ...
    function addTransferAuthorizer(
        ...
        // 2. Set variants
        addTransferAuthorizerVariants(authorizerAddress, tokenReceivers);
        // 3. Add authorizer to root authorizer set
        addAuthorizer(coboSafeAddress, authorizerAddress, isDelegateCall, roles);
    }
    ...
    function addDexAuthorizer(
        ...
        // 2. Set variants
        addDexAuthorizerVariants(authorizerAddress, _swapInTokens, _swapOutTokens);
        // 3. Add authorizer to root authorizer set
        addAuthorizer(coboSafeAddress, authorizerAddress, isDelegateCall, roles);
    }
}
...
abstract contract ArgusAuthorizerHelper {
    function addFuncAuthorizerVariants(
        address authorizerAddress,
        address[] calldata _contracts,
        string[][] calldata funcLists
    ) public {
    ...
    function removeFuncAuthorizerVariants(
        address authorizerAddress,
        address[] calldata _contracts,
        string[][] calldata funcLists
    ) external {
    ...
    function addTransferAuthorizerVariants(
        address authorizerAddress,
        TransferAuthorizer.TokenReceiver[] calldata tokenReceivers
    ) public {
    ...
    function removeTransferAuthorizerVariants(
        address authorizerAddress,
        TransferAuthorizer.TokenReceiver[] calldata tokenReceivers
    ) external {
        if (tokenReceivers.length == 0) return;
    ...
    function addDexAuthorizerVariants(
        address authorizerAddress,
        address[] calldata _swapInTokens,
        address[] calldata _swapOutTokens
    ) public {
        setDexAuthorizerVariants(authorizerAddress, _swapInTokens, _swapOutTokens, true);
    }

    function removeDexAuthorizerVariants(
        address authorizerAddress,
        address[] calldata _swapInTokens,
        address[] calldata _swapOutTokens
    ) external {
        setDexAuthorizerVariants(authorizerAddress, _swapInTokens, _swapOutTokens, false);
    }

    function setDexAuthorizerVariants(
        address authorizerAddress,
        address[] calldata _swapInTokens,
        address[] calldata _swapOutTokens,
        bool tokenStatus
...
abstract contract BaseAccount is IAccount, BaseOwnable {
    ...
    function execTransactions(
        ...
        for (uint256 i = 0; i < callDataList.length; i++) {
            transaction.to = callDataList[i].to;
            transaction.value = callDataList[i].value;
            transaction.data = callDataList[i].data;
            transaction.flag = callDataList[i].flag;
            transaction.hint = callDataList[i].hint;

            resultList[i] = _executeTransactionWithCheck(transaction);

            emit TransactionExecuted(
                callDataList[i].to,
                bytes4(callDataList[i].data),
                callDataList[i].value,
                transaction
            );
        }
    }
    ...
    function _executeTransactionWithCheck(
        ...
        AuthorizerReturnData memory preData = _preExecCheck(transaction);
        ...
        AuthorizerReturnData memory postData = _postExecCheck(transaction, result, preData);

        // 5. Do post process.
        if (flag.hasPostProcess()) _postExecProcess(transaction, result);

        // 6. Handle hint if supported.
        if (transaction.hint.length == 0) {
            // Only do this when no hint provided.
            if (flag.supportHint()) {
...
contract FlatRoleManager is IFlatRoleManager, BaseOwnable {
    ...
    function revokeRoles(bytes32[] calldata _roles, address[] calldata _delegates) external {
        require(_roles.length > 0 && _roles.length == _delegates.length, "FlatRoleManager: Invalid inputs");
...
contract ArgusRootAuthorizer is BaseAuthorizer, IAuthorizerSupportingHint {
    ...
    // Role + Authorizer address + sub hint >= 64 bytes.
    uint256 private constant HINT_SIZE = 64;
    ...
    function _preExecCheckWithHint(
        ...
        (address auth, bytes memory subHint) = _getAuthorizerFromHint(transaction);
    ...
    function _preExecCheck(
        ...
        if (transaction.hint.length >= HINT_SIZE) {
    ...
    function flag() external view returns (uint256) {
        // Force pre-check, post-check, hint flag set or our hint will stop working.
        return _unionFlag | AuthFlags.SUPPORT_HINT_MASK | AuthFlags.HAS_PRE_CHECK_MASK | AuthFlags.HAS_POST_CHECK_MASK;
    }
