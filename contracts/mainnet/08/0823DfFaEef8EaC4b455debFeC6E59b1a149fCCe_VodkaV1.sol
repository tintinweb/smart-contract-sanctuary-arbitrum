...
import "./interfaces/IGlpRewardHandler.sol";
...
contract VodkaV1 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    /** ----------- View functions ------------- */
    function getPositionTimestamp(address _user, uint256 _positionId) external view returns(uint256, uint256) {
        return (positionOpenTimestamp[_user][_positionId], positionOpenTimestamp[_user][_positionId] + vaultMaturityTime);
    }
    ...
    function openPosition(
        ...
        positionOpenTimestamp[msg.sender][userInfo[msg.sender].length] = block.timestamp;
    ...
    function closePosition(
        ...
        require(!_userInfo.liquidated && !_userInfo.closed, "VODKA: position is closed | liquidated");
        ...
        uint256 returnedValue = _withdrawAndUnstakeGLP(_userInfo.position);
        ...
        if (returnedValue > combinedDepositAndLeverage) {
            data.profits = returnedValue - combinedDepositAndLeverage;
            (closeData.waterProfits, closeData.mFee, data.leverageUserProfits) = _getProfitSplit(
                data.profits,
                _userInfo.leverage
            );
        }
        ...
        if (returnedValue < _userInfo.leverageAmount + closeData.waterProfits) {
            ...
            closeData.toLeverageUser = (returnedValue - closeData.waterRepayment) - closeData.mFee - closeData.waterProfits;
    ...
    function liquidatePosition(uint256 _positionId, address _user) external nonReentrant onlyKeeper{
        UserInfo storage _userInfo = userInfo[_user][_positionId];
        require(!_userInfo.liquidated && !_userInfo.closed, "VODKA: Already liquidated");
    ...
    function _mFeePayment(uint256 _mFee, uint256 _waterFee) internal {
        if (_mFee > 0) {
            IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(mFeeReceiver, _mFee);
        }
        if (_waterFee > 0) {
