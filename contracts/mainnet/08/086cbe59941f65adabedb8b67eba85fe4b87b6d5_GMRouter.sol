    ...
    PausableUpgradeable
{
    ...
    function initialize(address _strategy, address _vault, address _signer) external initializer {
        ...
        dataTypeHash = keccak256(
            "Data(uint256 nonce,uint8 action,uint256 amount,uint256 aum,uint256 executionAmount,IGMStrategy.GMData[] gmData,uint256 deadline)GMData(uint8 gmTokenIndex,uint256 indexPrice,uint256 longPrice,uint256 shortPrice,uint256 amount,uint256 executionFee,uint256 callbackGasLimit)"
        );
        ...
        for (uint256 i; i < length;) {
            ...
            gmDataHashes[i] = keccak256(
                abi.encode(
                    gmDataTypeHash,
                    gmData.gmTokenIndex,
                    keccak256(abi.encode(gmData.longPath)),
                    keccak256(abi.encode(gmData.shortPath)),
            ...
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        ...
                        keccak256(abi.encode(gmDataHashes)),
                        params.deadline
                    )
                )
            ),
