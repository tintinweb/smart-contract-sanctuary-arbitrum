...
abstract contract BaseStrategyPendle {
     ...
     */
    function __Base_Strategy_Initialize(address _vault, address _strategist, address _keeper) internal {
...
interface IBaseStrategyPendle {
    function adjustPosition(uint256 _debtOutstanding, OffChainData calldata _offChainData) external;

    function migrate(address _newStrategy, OffChainData calldata _offChainData) external;

    function withdraw(uint256 amount, OffChainData calldata _offChainData) external returns (uint256);
...
import {StrategyParams, IOnChainVaultPendle} from "./interfaces/IOnChainVaultPendle.sol";
import {IBaseStrategyPendle, OffChainData} from "./interfaces/IBaseStrategyPendle.sol";
...
/**
 * @title OnChainVaultPendle
 * @dev A vault contract managing strategies for the Locus protocol.
 */
contract OnChainVaultPendle is
    Initializable,
    IOnChainVaultPendle,
    UUPSUpgradeable,
    ...
    EIP712Upgradeable
{
    error Vault__WrongSigner();
    error Vault__NonceIsUsed();
    ...
    bytes32 public offChainDataTypeHash;
    address public backendDataSigner;
    uint256 constant deadValue = 1;


    mapping(address => StrategyParams) public strategies;
    mapping(address strategy => uint256 position)
        public strategyPositionInArray;
    mapping (uint256 nonce => bool isUsed) isNonceUsed;
    ...
    /**
     * @dev Initializes the OnChainVaultPendle contract.
     * @param _token The ERC20 token managed by the vault.
    ...
    function initialize(
        ...
        address _admin,
        address _treasury,
        address _backendDataSigner
    ) external initializer {
        __EIP712_init("Locus Pendle Vault", "1");
        ...
        token = _token;
        treasury = _treasury;
        backendDataSigner = _backendDataSigner;
        offChainDataTypeHash = keccak256("OffChainData(uint256 YtToWantRate,uint256[2] WantToLpYtProportion,uint256 PtToSyRate,uint256 SyToPtRate,uint256[2] LpToSyPtProportion,uint256 PyToAssetRate,uint256 SyToAssetRate,uint256 nonce)");
    }

    /**
     * @dev Authorizes a new implementation for upgrade.
     * @param newImplementation The address of the new implementation.
     */
    function _authorizeUpgrade(address newImplementation) internal override onlyRole(UPGRADER){}

    /**
     * @dev Modifier to restrict access to authorized roles.
     */
    modifier onlyAuthorized() {
        if (
     ...
     * @return uint256 The total supply of vault tokens.
     */
    ...
    /**
     * @notice Modifier to check the amount on deposit.
    ...
    function _verifySigner712(bytes memory signature, OffChainData memory _offChainData) internal {
    if(isNonceUsed[_offChainData.nonce]) revert Vault__NonceIsUsed();
    bytes32 digest = _hashTypedDataV4(
        keccak256(
            abi.encode(
                offChainDataTypeHash,
                _offChainData.YtToWantRate,
                keccak256(abi.encode(_offChainData.WantToLpYtProportion)),
                _offChainData.PtToSyRate,
                _offChainData.SyToPtRate,
                keccak256(abi.encode(_offChainData.LpToSyPtProportion)),
                _offChainData.PyToAssetRate,
                _offChainData.SyToAssetRate,
                _offChainData.nonce
            )
        )
    );
    if(backendDataSigner != ECDSA.recover(digest, signature)){
        revert Vault__WrongSigner();
    }
    isNonceUsed[_offChainData.nonce] = true;
}
    ...
    /**
     * @dev Revokes funds from the vault.
     */
    ...
    /**
     * @dev Sets the emergency shutdown state.
     * @param _emergencyShutdown The emergency shutdown state.
     */
    ...
    function setBackendSigner(
        address _newSigner
    ) external onlyAuthorized {
        backendDataSigner = _newSigner;
    }
    ...
    /**
     * @dev Sets the vault token contract.
     * @param _newToken The new vault token contract.
     */
    function setVaultToken(IVaultToken _newToken) external onlyAuthorized {
        vaultToken = _newToken;
    }

    /**
     * @notice Returns the total assets managed by the vault.
     * @return _assets The total assets managed by the vault.
     */
    function totalAssets() public view returns (uint256 _assets) {
        for (uint256 i = 0; i < OnChainStrategies.length; i++) {
            _assets += IBaseStrategyPendle(OnChainStrategies[i])
    ...
    /**
     * @notice Returns the total idle assets held by the vault.
     * @return uint256 The total idle assets held by the vault.
     */
    ...
    /**
     * @dev Updates the minimum debt per harvest for a strategy.
     * @param strategy The address of the strategy.
     * @param _minDebtPerHarvest The new minimum debt per harvest.
     */
    ...
    /**
     * @dev Updates the maximum debt per harvest for a strategy.
     * @param strategy The address of the strategy.
     * @param _maxDebtPerHarvest The new maximum debt per harvest.
     */
     ...
     * @return uint256 The number of shares issued.
     */
    function deposit(
        uint256 _amount,
        address _recipient
    ...
    /**
     * @dev Deposits funds into the vault for the sender.
     * @param _amount The amount to deposit.
     * @return uint256 The number of shares issued.
     */
    function deposit(
    ...
    /**
     * @dev Initiates a withdrawal from the vault.
     * @param _maxShares The maximum number of shares to withdraw.
     * @param _recipient The address of the recipient.
     * @param _maxLoss The maximum acceptable loss.
     * @param _offChainData Data related to the off-chain withdrawal process.
     */
    function withdraw(
        uint256 _maxShares,
        address _recipient,
        ...
        uint256 len = _offChainData.length;
        for (uint256 i; i < len; i++) {
            _verifySigner712(signatures[i], _offChainData[i]);
        }
        _initiateWithdraw(_maxShares, _recipient, _maxLoss, _offChainData);
    }

    /**
     * @dev Adds a new strategy to the vault.
     * @param _strategy The address of the strategy contract.
     * @param _debtRatio The debt ratio for the strategy.
    ...
    /**
     * @dev Retrieves the outstanding debt for a strategy.
     * @param _strategy The address of the strategy.
     * @return uint256 The outstanding debt for the strategy.
     */
    ...
    /**
     * @dev Retrieves the outstanding debt for the sender.
     * @return uint256 The outstanding debt for the sender.
     */
    function debtOutstanding() external view returns (uint256) {
    ...
    /**
     * @dev Retrieves the credit available for a strategy.
     * @param _strategy The address of the strategy.
     * @return uint256 The credit available for the strategy.
     */
    ...
    /**
     * @dev Initiates a withdrawal from the vault.
     * @param maxShares The maximum number of shares to withdraw.
     * @param recipient The address of the recipient.
     * @param maxLoss The maximum acceptable loss.
     * @param _offChainData Data related to the off-chain withdrawal process.
     */
    function _initiateWithdraw(
        ...
        if (shares == type(uint256).max) {
            shares = vaultToken.balanceOf(msg.sender);
        }
        if (shares > vaultToken.balanceOf(msg.sender)) revert Vault__NotEnoughShares();
        ...
        if (value > vaultBalance) {
            ...
            for (uint256 i = 0; i < OnChainStrategies.length; i++) {
                ...
                uint256 loss = IBaseStrategyPendle(OnChainStrategies[i]).withdraw(
                    amountNeeded, _offChainData[i]
                );
        ...
        vaultToken.burn(msg.sender, shares);
     ...
     * @return uint256 The price per share of the vault.
     */
    function pricePerShare() external view returns (uint256) {
    ...
    /**
     * @dev Revokes a strategy from the vault.
     * @param _strategy The address of the strategy.
     */
    ...
    /**
     * @dev Migrates a strategy to a new address.
     * @param _oldStrategy The address of the old strategy.
     * @param _newStrategy The address of the new strategy.
     * @param _offChainData Data related to the off-chain migration process.
     */
    function migrateStrategy(
        ...
        IBaseStrategyPendle(_oldStrategy).migrate(_newStrategy, _offChainData);
    ...
    /**
     * @dev Reports gains, losses, and debt payments for a strategy.
     * @param _gain The gain amount.
     * @param _loss The loss amount.
     * @param _debtPayment The debt payment amount.
     * @return uint256 The total assets managed by the strategy.
     */
    function report(
        uint256 _gain,
        uint256 _loss,
        ...
        if (strategies[msg.sender].debtRatio == 0 || emergencyShutdown) {
            return IBaseStrategyPendle(msg.sender).estimatedTotalAssets();
        } else {
            return debt;
        }
    }
    ...
    /**
 * @dev Calculates the locked profit of the vault.
 * @return uint256 The locked profit of the vault.
 */
    ...
    /**
 * @dev Reports a loss for a strategy.
 * @param _strategy The address of the strategy.
 * @param _loss The loss amount.
 */
    ...
    /**
 * @dev Returns the total free funds available in the vault.
 * @return uint256 The total free funds available in the vault.
 */
    function _freeFunds() internal view returns (uint256) {
        return totalAssets() - _calculateLockedProfit() + deadValue;
    }

    /**
 * @dev Returns the value of shares for a given number of shares.
 * @param _shares The number of shares.
 * @return uint256 The value of shares.
 */
    ...
    /**
 * @dev Calculates the number of shares for a given amount.
 ...
 * @return uint256 The maximum available shares for withdrawal.
 */
    ...
    /**
 * @dev Issues shares to an account for a given amount.
 * @param _to The recipient of the shares.
 * @param _amount The amount of shares to issue.
 * @return uint256 The number of shares issued.
 */
    ...
    /**
 * @dev Assess fees for a strategy based on the gain.
 * @param strategy The address of the strategy.
 * @param gain The gain amount.
 * @return uint256 The total fees assessed.
 */
