...
contract TYFSale is Ownable {
    ...
    uint256 public constant TYF_PRICE = 1 * (10 ** 14);
    ...
    function withdrawRewardTokens() external onlyOwner {
        ...
        if (_totalDepositBalance < SOFT_CAP) {
            ...
            uint256 tokensForCurrentDeposit = minDeposit.div(TYF_PRICE).mul(BASE);
    ...
    function increaseTotalRewardToken(uint256 amount) external onlyOwner {
        ...
        uint256 _totalRewardToken = _getTotalRewardToken();
        require(_totalRewardToken > 0 || amount >= HARD_CAP.div(TYF_PRICE).mul(BASE), "Not correct");
    ...
    function getTYF() external {
        ...
        uint256 reward = _userDeposit[_msgSender()].div(TYF_PRICE).mul(BASE);
    ...
    function getTYFAndDeposit() external {
        _requireSaleStarts();
        _requireDeadLineIsOver();
        ...
        uint256 reward = _userDeposit[_msgSender()].mul(HARD_CAP).div(_totalDepositBalance).div(TYF_PRICE).mul(BASE);
        _requireEnoughTYFBalance(reward);
        ...
        uint256 rewardInETH = reward.div(BASE).mul(TYF_PRICE);
        uint256 wad = 0;
