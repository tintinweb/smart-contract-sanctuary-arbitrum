...
contract PhysicalRedeemConsumer is GenericTraitConsumer {
    struct UserDataMessage {
        uint32 tokenId;
        uint16 traitId;
    }

    error PhysicalRedeemConsumerUserActionAndUserDataTokenIdDoNotMatch(
        uint32 communityId,
        uint32 collectionId,
    ...
    error PhysicalRedeemConsumerNotOwnerOfToken(
    ...
    error PhysicalRedeemConsumerTraitDoesNotExist(
        uint32 communityId,
        uint32 collectionId,
        uint32 tokenId,
        uint16 traitId
    ...
    error PhysicalRedeemConsumerInvalidTraitType(uint16 traitType);
    ...
    function handleAction(
        ...
        if (action.tokenId != message_.tokenId) {
            revert PhysicalRedeemConsumerUserActionAndUserDataTokenIdDoNotMatch(
                action.communityId,
                action.collectionId,
                action.tokenId,
                message_.tokenId
        ...
        if (ownerOfToken_ != from) {
            revert PhysicalRedeemConsumerNotOwnerOfToken(
                ownerOfToken_,
                action.communityId,
                action.collectionId,
                message_.tokenId
        ...
        GenericTrait trait = GenericTrait(
        ...
        if (address(trait) == address(0)) {
            revert PhysicalRedeemConsumerTraitDoesNotExist(
                action.communityId,
                action.collectionId,
                action.tokenId,
                message_.traitId
        ...
        if (traitType_ != 3) {
            revert PhysicalRedeemConsumerInvalidTraitType(traitType_);
        }

        if (trait.status(message_.tokenId) == uint8(TraitStatus.ACTIVE)) {
            // decrement counter value in source ( since we used this trait )
...
import {PhysicalRedeemConsumer} from "../../TraitConsumers/PhysicalRedeemConsumer.sol";

/**
 * @dev This factory is used to create new traits (perks) in a community
 */
contract UTPhysicalRedeemFactory is GenericTraitFactory {
        ...
        returns (address controllerCtrAddress, address storageCtrAddress)
    {
        ...
        if (controllerCtr == address(0)) {
            controllerCtr = address(new PhysicalRedeemConsumer(address(galaxisRegistry)));
