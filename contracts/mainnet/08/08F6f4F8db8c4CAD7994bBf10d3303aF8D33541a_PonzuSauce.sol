...
contract PonzuSauce is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Ponzu Sauce";
    string private _symbol = "Ponzu";
    uint8 private _decimals = 9;

    address payable public Marketing = payable(0x21fC45783d13E29D9d5e65280212A7110384275b);
    address payable public Treasury = payable(0x86DF84616da086e9fc7b17f621a60d0cA5914eb9);
    ...
    uint256 private _totalSupply = 8 * 10**14 * 10**9;
    uint256 public _maxTxAmount = 8 * 10**11 * 10**9;
    uint256 public _walletMax = 8 * 10**11 * 10**9;
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountETHLiquidity = amountReceived.mul(_liquidityShare).div(totalBNBFee).div(2);
        uint256 amountETHTreasury = amountReceived.mul(_treasuryShare).div(totalBNBFee);
        uint256 amountETHMarketing = amountReceived.sub(amountETHLiquidity).sub(amountETHTreasury);

        if(amountETHMarketing > 0)
            transferToAddressETH(Marketing, amountETHMarketing);

        if(amountETHTreasury > 0)
            transferToAddressETH(Treasury, amountETHTreasury);
