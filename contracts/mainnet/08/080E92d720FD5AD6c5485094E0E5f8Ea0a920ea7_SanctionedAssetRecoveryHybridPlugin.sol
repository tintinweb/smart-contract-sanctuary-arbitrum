...
/**
 * @title Sanctioned Assets Recover Plugin
 * @author Josh Davis
 * @notice Provides the recoverSanctionedAssets()
 */

interface ISanctionedAssetRecoveryHybridPlugin {
    /**
...
contract SanctionedAssetRecoveryHybridPlugin is ViciERC20HybridPlugin {
    bytes4 constant RECOVER =
        ISanctionedAssetRecoveryHybridPlugin.recoverSanctionedAssets.selector;
    ...
    function _beforeHook(
        ...
        if (_msg.sig == RECOVER) {
            if (_msg.data.length != 68) {
                revert InvalidCallData(RECOVER);
            }

            enforceOwnerOnly(msg.sender, _msg.sender);
            return TRUE;
        }
    ...
    function _execute(
        ...
        if (_msg.sig == RECOVER) {
            (address fromAddress, address toAddress) = abi.decode(
                _msg.data[4:],
                (address, address)
            );
            return _doRecover(_msg.sender, fromAddress, toAddress);
        }
    ...
    function _doRecover(
        ...
        (ISimplePluginExecutor(msg.sender)).executeFromPlugin(
            abi.encodeWithSelector(
                IModularERC20Ops.recoverSanctionedAssets.selector,
                operator,
                fromAddress,
                toAddress
            )
