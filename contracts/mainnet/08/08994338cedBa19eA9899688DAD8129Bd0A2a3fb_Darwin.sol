...
contract Darwin is  DarwinStore{
    ...
    event CharacterMinted(address indexed player, uint256 indexed tokenId, string metaHash);
    ...
    function mint721(uint256 orderId, uint256 tokenId, uint32 tokenTag, uint[] memory ids, uint[] memory amounts, uint8 v, bytes32 r, bytes32 s) public reentrancyGuard{
    ...
    function mintMine(uint256 orderId, uint256 markTag, uint32 actionTag, uint[] memory rewardIds, uint[] memory rewardAmounts, uint8 v, bytes32 r, bytes32 s) public reentrancyGuard {
        require(contractIsOpen, "Contract must active");
    ...
    function mintCharacter(uint256 tokenId, string memory tokenUri) public payable reentrancyGuard{
        require(contractIsOpen, "Contract must active");

        require(characterPrice() == msg.value, "character price unmatch");

        Character(characterNFT())._mintByAdmin(tokenId, 0, msg.sender);
        Character(characterNFT()).setTokenUri(tokenId, tokenUri);

        emit CharacterMinted(msg.sender, tokenId, tokenUri);
    }
    ...
    function withdrawal721(uint256 orderId, uint256 tokenId, uint32 tokenTag, uint[] memory ids, uint[] memory amounts, uint8 v, bytes32 r, bytes32 s) public reentrancyGuard{
        require(contractIsOpen, "Contract must active");
    ...
    function stack1155(uint256[] memory ids, uint256[] memory amounts) public reentrancyGuard{
        ...
        for(uint256 i=0; i<ids.length; ++i){
            uint256 oldAmount = _stack1155Map[ids[i]][msg.sender];

            _stack1155Map[ids[i]][msg.sender] = SafeMath.add(oldAmount, amounts[i]);
        }

        emit Stack1155(msg.sender, ids, amounts);
    }

    function withdrawal1155(uint256 orderId, uint256 markTag, uint32 actionTag, uint[] memory ids, uint[] memory amounts, uint8 v, bytes32 r, bytes32 s) public reentrancyGuard{
        require(contractIsOpen, "Contract must active");
        ...
        for(uint256 i=0; i<ids.length; ++i){
            uint256 oldAmount = _stack1155Map[ids[i]][msg.sender];           
            require(oldAmount >= amounts[i], "resource amount not enought");
        }

        Darwin1155(_NFT1155).safeBatchTransferFrom(address(this), msg.sender, ids, amounts, "withdrawal from game");
        ...
        for(uint256 i=0; i<ids.length; ++i){
            ...
            _stack1155Map[ids[i]][msg.sender] = SafeMath.sub(oldAmount, amounts[i]);
        }
        emit Withdrawal1155(msg.sender, orderId, ids, amounts);
    }
...
abstract contract  DarwinStore is ReentrancyGuarded, Ownable{
    ...
    mapping(uint256 => address) public _stackArmoryMap;

    mapping(uint256 => mapping(address => uint256)) public _stack1155Map;
...
abstract contract  Darwin721 is ReentrancyGuarded, ERC721, ProxyContractBase{
    ...
    function mint(address tokenOwner, uint256 tokenId, uint256 tag) internal {
        ...
        emit TokenMint(tokenOwner, tokenId, tag);
    }
    

    function _mintByAdmin(uint256 tokenId, uint256 tag, address tokenOwner) public virtual reentrancyGuard onlyNFTAdmin{
    }
...
contract Character is Darwin721{
    ...
    function setTokenUri(uint256 tokenId, string memory uri) public reentrancyGuard onlyNFTAdmin {
