...
contract ZookeyRouter is IZookeyRouter, OwnableUpgradeable, LockAndMsgSender {
        ...
        if (command <= Commands.BRIDGE_IF_BOUNDARY) {
            if (command <= Commands.SWAP_IF_BOUNDARY) {
                if (command == Commands.V3_SWAP_EXACT_IN) {
                    _swapV3ExactIn(_fromToken, _toToken, _inputs, _deadline);
                }
                if (command == Commands.V2_SWAP_EXACT_IN) {
                    _swapV2ExactIn(_fromToken, _toToken, _inputs, _deadline);
                }
            }
        } else {
            success = false;
    ...
    function _swapV3ExactIn(
        ...
        assembly {
            ...
            fromTokenAmount := calldataload(add(_inputs.offset, 0x20))
        ...
        bool needToETH = lastTokenBytes == ZookeyConstants.WETH &&
        ...
        ISwapRouter(ZookeyRouterConstants.UNI_SWAP_ROUTER_V3).exactInput(
            params
        );
        if (needToETH) {
            uint256 amount = IWETH(ZookeyConstants.WETH).balanceOf(
                address(this)
            );
    ...
    function _swapV2ExactIn(
        ...
        address[] calldata path = _inputs.toAddressArray(3);
        if (fromTokenAmount == ZookeyRouterConstants.CONTRACT_BALANCE) {
            _fromToken = path[0];
            fromTokenAmount = IERC20(_fromToken).balanceOf(address(this));
            onlyApprove = true;
        ...
        if (_fromToken == ZookeyConstants.ETH) {
            IUniswapV2Router02(ZookeyRouterConstants.UNI_SWAP_ROUTER_V2)
            ...
            // swap to ETH
            _toToken == ZookeyConstants.ETH &&
            path[path.length - 1] == ZookeyConstants.WETH
        ) {
            _tokenRelayer(
                _fromToken,
                fromTokenAmount,
    ...
    function _tokenRelayer(
        ...
        require(
            _fromToken != ZookeyConstants.ETH,
            ZookeyRouterErrors.FROM_TOKEN_ADDR_ERR
        );
