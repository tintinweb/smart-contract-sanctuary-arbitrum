...
/**
 * @title PenpieKelpStrategy
 * @notice This contract is a strategy that interacts with the Pendle protocol.
 *
 */
contract PenpieKelpStrategy is IStrategy, StratManager, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ...
        if (balancePendle > 0) {
            _swapUniswap(pendleToken, weth, balancePendle, 0, 3000);
        }
        uint256 balanceArb = IERC20(arbToken).balanceOf(address(this));
        if (balanceArb > 0) {
            _swapUniswap(arbToken, weth, balanceArb, 0, 3000);
        }
        if (balancePenpie > 0 || balancePendle > 0 || balanceArb > 0) {
            chargeFees(callFeeRecipient);
            uint256 harvestAmount = IERC20(weth).balanceOf(address(this));
            _swapUniswap(weth, rseth, IERC20(weth).balanceOf(address(this)), 0, 100);
            uint256 balanceRseth = IERC20(rseth).balanceOf(address(this));
            _deposit(balanceRseth);
    ...
    function _giveAllowances() internal {
        IERC20(rseth).safeApprove(pendleRouter, type(uint256).max);
    ...
    function _removeAllowances() internal {
        IERC20(rseth).safeApprove(pendleRouter, 0);
     ...
     * @param poolFee The pool fee to be used in the swap.
     ...
     * @param _fromToken The token to be swapped.
     * @param _toToken The token to be received.
    ...
    function _swapCamelot(address _fromToken, address _toToken, uint256 _amountIn) internal {
        TransferHelper.safeApprove(_fromToken, address(penpieSwapRouter), _amountIn);
