...
contract Zapper is Ownable {
    ...
    function quoteSellGBTIn(address gbt, uint256 fee, uint256 input, uint256 slippageTolerance, uint256 autoSlippage) external view returns (uint256 output, uint256 slippage, uint256 minOutput, uint256 autoMinOutput) {
        uint256 oldReserveGBT = IGBT(gbt).reserveGBT();
    ...
    function quoteBASEtoNFT(address gbt, uint256 gbtFee, uint256 input, uint256 slippageTolerance, uint256 autoSlippage) external view returns (uint256 output, uint256 slippage, uint256 autoOutput) {
     ...
     function quoteNFTtoBASE(address gbt, uint256 gbtFee, uint256 input, uint256 slippageTolerance) external view returns (uint256 output, uint256 slippage, uint256 minOutput) {
        ...
        uint256 newReserveGBT = IGBT(gbt).reserveGBT() + gbtToSell - (gbtToSell * gbtFee / DIVISOR);
        uint256 oldReserveBASE = IGBT(gbt).reserveVirtualBASE() + IGBT(gbt).reserveRealBASE();
        output = oldReserveBASE - (oldReserveBASE * IGBT(gbt).reserveGBT() / newReserveGBT);
        slippage = 100 * (1e18 - (output * 1e18 / ((gbtToSell - (gbtToSell * gbtFee / DIVISOR)) * IGBT(gbt).currentPrice() / 1e18)));
        minOutput = ((gbtToSell * IGBT(gbt).currentPrice() / 1e18) * slippageTolerance / DIVISOR) * (DIVISOR - IGNFT(gnft).bFee()) / DIVISOR;
    }
