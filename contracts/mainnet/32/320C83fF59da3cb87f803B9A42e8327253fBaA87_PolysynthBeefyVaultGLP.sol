...
library Beefy {
    ...
    struct VaultState {
        ...
        // // Total amount of beefy tokens availble for nextRound
        // uint128 assetBalanceForNextCycle;
    ...
    ERC20Upgradeable
{
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        console.log("NEW ROUND %s PPS %s", vaultState.round,  pricePerShare());
        ...
        // is not returned
        console.log("DEPOSIT GLP %s MooTokensReceived %s VaultBalance %s", amount, newTokensReceived, afterBalance);
    ...
    function withdrawInstantly(uint256 amount) external nonReentrant {
        ...
        uint256 balance =  depositReceipt.amountB + depositReceipt.amount;
        
        require(amount > 0 && amount <= balance , "!amount");
        require( (depositReceipt.round == currentRound ||
                    depositReceipt.roundB == currentRound ||
                    depositReceipt.round == (currentRound -1)) , 
                "Invalid round");

        if(depositReceipt.roundB == currentRound && amount <= depositReceipt.amountB){
            depositReceipt.amountB = uint104(uint256(depositReceipt.amountB).sub(amount));
            deductedFromUnLocked = amount;
        }else if( depositReceipt.round == currentRound && depositReceipt.amountB == 0){
            depositReceipt.amount = uint104(uint256(depositReceipt.amount).sub(amount));
            deductedFromUnLocked = amount;
        }else{
            deductedFromUnLocked = depositReceipt.amountB;
            deductedFromLocked = amount - deductedFromUnLocked;
            depositReceipt.amountB = 0;
            depositReceipt.amount = uint104(uint256(depositReceipt.amount).sub(deductedFromLocked));
        }

        receiptAmount = deductedFromUnLocked.add(deductedFromLocked);
        require(amount == receiptAmount, "!amount");

        vaultState.unlockedAmount = uint104(
                uint256(vaultState.unlockedAmount).sub(deductedFromUnLocked)
        );

        if (deductedFromLocked > 0 && vaultState.round > 1){
        ...
        console.log("Instant Withdraw %s AfterBalanceMoo %s ", amount,  vaultState.unlockedAmount);
    ...
    function _depositFor(uint256 amount, address creditor) private {        
        ...
        //         currentRound > 2 && (
        //             (receipt.roundB == 0 && receipt.round < (currentRound - 1)) ||  
        //             (receipt.roundB != 0 && receipt.roundB < (currentRound - 1))  //To manage deposit (Round 2 & 4) older than n-1 rounds
        //         ) 
        //     )
        // {
        ...
        //     receipt.roundB = 0;
        //     receipt.amountB = 0;
        // }else if(receipt.round == currentRound) { //To manage deposit in same round
        //     receipt.amount = uint104(depositAmount);
        // }else if (receipt.roundB == 0){ //To manage deposit in next round
        //     receipt.roundB =  uint16(currentRound);
        //     receipt.amountB = uint104(depositAmount);
        // }else if(receipt.roundB == currentRound) { //To manage deposit in same round
        //     receipt.amountB = uint104(depositAmount);
    ...
    //     uint256 currentRound = vaultState.round;
    //     Beefy.Withdrawal storage withdrawal = withdrawals[msg.sender];
    ...
    //     ShareMathBeefy.assertUint128(withdrawalShares);
    ...
    //     IBeefy bv = IBeefy(BEEFY_VAULT);
    //     //FIXME: This should be balance of GLP held by vault
    //     uint256 prevBalance = IERC20(vaultParams.asset).balanceOf(address(this));
    ...
    function _withdraw(uint256 numShares) internal returns (uint256) {
       require(numShares > 0, "!numShares");
        ...
        console.log("SHARESSS ", numShares);
        _transfer(msg.sender, address(this), numShares);
        ...
        )
    {
        ...
        uint256 _managementFeeInAsset =  uint256(deployedAmount)
                .mul(managementFeePercent)
                .div(100 * Beefy.FEE_MULTIPLIER);


        uint256 _vaultFee = _performanceFeeInAsset.add(_managementFeeInAsset);
        console.log("FEEE settledAmount %s deployedAmount %s ", optionState.settledAmount, deployedAmount);
    ...
    function pricePerShare() public view returns (uint256) {
        console.log("unlockedAmount %s prevRoundAmount %s lockedAmount %s", vaultState.unlockedAmount, vaultState.prevRoundAmount, vaultState.lockedAmount);
    ...
    function getNextMonday(uint256 timestamp) internal pure returns (uint256) {
    ...
    PolysynthKikoVaultStorage
{
    ...
    function close() external nonReentrant onlyKeeper {
       ...
       (uint256 totalFees, uint256 queuedWithdrawAmount) = _closeRound();
        ...
        if(currbalance.add(totalFees) >= uint256(vaultState.unlockedAmount).add(uint256(optionState.settledAmount))){
            // deployed balance must not have settle amount | Add fees to retrieve previous vaultbalance
            currentRound = vaultState.round;
            deployedbalance = currbalance.add(totalFees).sub(vaultState.unlockedAmount).sub(optionState.settledAmount);
            ...
            mooYieldShares = (accruedYield * deployedbalance)/bps;
            ...
            if(currentRound > 2  
                && mooYieldShares > 0){   
                glpYieldShares = (mooYieldShares *bps) / TOKEN_DECIMALS;
