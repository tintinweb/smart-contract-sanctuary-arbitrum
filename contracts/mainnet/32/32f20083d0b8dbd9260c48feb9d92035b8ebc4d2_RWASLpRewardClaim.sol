...
// File contracts/RWASLpRewardClaim.sol
// License-Identifier: MIT
    ...
    WithOperator
{
    ...
    bytes32 private constant _CLAIM_TYPEHASH =
        keccak256(
            "Claim(address operator,address to,uint256 rewardTotal,uint256 nonce,uint256 deadline)"
        );
    ...
    uint256 public claimedAllTotal;
    uint256 public count;
    mapping(uint256 => Claim) public claims;
    mapping(address => uint256[]) public addrClaims;
    ...
    function initialize(
        ...
        __EIP712_init("RWASLpRewardClaim", "1");
    ...
    function claim(
        ...
        require(
            ...
            "RWASLpRewardClaim: deadline expired"
        );

        address to = _msgSender();
        require(
            claimedTotal[to] < rewardTotal,
            "RWASLpRewardClaim: invalid total"
        );
        ...
        require(
            signer == operator(0),
            "RWASLpRewardClaim: invalid signature"
        );

        uint256 value = rewardTotal - claimedTotal[to];

        token.safeTransferFrom(wallet, to, value);
        claimedTotal[to] += value;
        claimedAllTotal += value;

        count++;
        claims[count].operator = operator(0);
        claims[count].to = to;
        claims[count].claimed = value;
        claims[count].rewardTotal = rewardTotal;
        claims[count].allTotal = claimedAllTotal;
        claims[count].claimTime = block.timestamp;
        claims[count].claimId = count;

        addrClaims[to].push(count);
    ...
    function addrClaimCount(address to) external view returns (uint256) {
        return addrClaims[to].length;
    }

    function getAddrClaimIds(
        address to,
        uint256 start,
        uint256 end
        ...
        for (uint256 i = start; i <= end; i++) {
            ids[i - start] = addrClaims[to][i];
        }
    }

    function getAddrClaims(
        address to,
        uint256 start,
        uint256 end
    ) external view returns (Claim[] memory result) {
        result = new Claim[](end - start + 1);
        for (uint256 i = start; i <= end; i++) {
            result[i - start] = claims[addrClaims[to][i]];
        }
    }
    ...
    function _splitSignature(
        ...
        require(
            sig.length == 65,
            "RWASLpRewardClaim: invalid signature length"
        );
