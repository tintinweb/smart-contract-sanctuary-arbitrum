...
contract TokenFiErc1155Facet is ITokenFiErc1155, SolidStateERC1155, AccessControlInternal, Pausable {
    ...
    function createToken(CreateTokenInput memory input) external onlyRole(AccessControlStorage.DEFAULT_ADMIN_ROLE) {
        ...
        IPlatformModule(ds.paymentModule).addService(platformId, service);
        ds.paymentServiceIndex[input.tokenId] = serviceIndex;
    }
    ...
    function setTokenPublicMintPrice(
        ...
        IPlatformModule(ds.paymentModule).updateService(platformId, ds.paymentServiceIndex[_tokenId], service);
    }
    ...
    function paymentServiceIndex(uint256 tokenId) external view override returns (uint256) {
        LibTokenFiErc1155Storage.DiamondStorage storage ds = LibTokenFiErc1155Storage.diamondStorage();
        return ds.paymentServiceIndex[tokenId];
    }
    ...
    function _processPayment(uint256 tokenId, uint256 amount, address paymentToken, address referrer) private {
        ...
        services[0] = uint32(ds.paymentServiceIndex[tokenId]);
        ...
        IPaymentModule(ds.paymentModule).processPayment{ value: msg.value }(paymentInput);

        emit MintPaymentProccessed(msg.sender, ds.paymentCount);
        ds.paymentCount++;
    }
