...
contract GlmManager is IDepositCallbackReceiver, IWithdrawalCallbackReceiver, IGlmManager, OwnableUpgradeable {
    ...
    function getRatioImpactFeeBps(uint256 _index, uint256 _usdcAmount, bool _isDeposit) public view returns (uint256 ratioImpactFeeBps) {
        ...
        require(weightAfterChange > 0, "Weight after change is 0");
        ...
        if (_isDeposit) {
            ...
            if (weightAfterChange > targetWeight) {
                return ((weightAfterChange - targetWeight) * impactFeeMultiplier) / PRICE_PRECISION;
            } else {
            ...
            //handle withdrawal
            if (weightAfterChange < targetWeight) {
                return ((targetWeight - weightAfterChange) * impactFeeMultiplier) / PRICE_PRECISION;
            } else {
    ...
    function withdrawFromRouter(address gmToken, uint256 gmAmount, uint256 glmAmount, address user) external {
        ...
        if (isFeeActive) {
            uint256 ratioImpactFeeBps = getRatioImpactFeeBps(poolId, glmUSDCValue, false);
            feeInUsdc = (glmUSDCValue * ratioImpactFeeBps) / MAX_BPS;
            feeInGm = (glmUSDCValue * ratioImpactFeeBps) / MAX_BPS;
        ...
        IGlmVault(glmVault).transferGmToUser(gmToken, gmAmount, user);
        IGlmVault(glmVault).finalizeWithdrawal(glmUSDCValue, glmAmount, user);
    }
    ...
    function _usdcWithdraw(
        ...
        uint256 pendingExecutionAmount = IGlmRouter(glmRouter).getTransactionInfo(key).pendingExecutionAmount;

        IERC20Upgradeable(USDC).transfer(glmVault, totalUSDC);
        IGlmVault(glmVault).adjustPendingExecutionAmount(pendingExecutionAmount, false);
