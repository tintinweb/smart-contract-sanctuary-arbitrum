...
library MathLib {
    ...
    function mulArbUp(uint256 a, uint256 b, uint256 scale) internal pure returns (uint256) {
        return MathUpgradeable.ceilDiv(a * b, scale);
    }

    function divArbUp(uint256 a, uint256 b, uint256 scale) internal pure returns (uint256) {
...
import {StorageSlotUpgradeable as StorageSlot} from
...
library ExecutionProcessorLib {
        ...
        internal
    {
        ...
        (int256 collateral, uint256 protocolFees, uint256 fee) =
            _applyFees(trader, positionId, cost.abs() + amount.abs());
    ...
    function _applyFees(address trader, PositionId positionId, uint256 cost)
        private
        view
