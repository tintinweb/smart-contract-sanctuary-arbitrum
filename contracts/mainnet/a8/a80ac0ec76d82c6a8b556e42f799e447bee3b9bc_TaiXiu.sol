...
contract TaiXiu is MultiPlayer, AutomationCompatibleInterface {

    event RoundisOpenNow_EVENT();
    event TaiXiu_Play_EVENT(
        address playerAddress,
        uint256 wager,
        address tokenAddress,
        bool isBig
    );
    event TaiXiu_Round_INFO_Event(
        uint256 id,
        uint256 tai,
        uint256 xiu,
        uint256 coutdown,
    ...
    event TaiXiu_Take_Balance_EVENT(
        uint256 tai,
        uint256 xiu,
        uint256 bankrollAccepted
    );
    event TaiXiu_VRF_Request_EVENT(uint8 xucxac1, uint8 xucxac2, uint8 xucxac3);
    ...
    mapping(address => TaiXiuGame) TaiXiuGames;
    mapping(uint256 => Result) Results;
    address[] big;
    address[] small;

    function TaiXiu_GetState(address player)
        external
        view
        returns (TaiXiuGame memory)
    {
        return (TaiXiuGames[player]);
    }
    ...
    function TaiXiu_Play(
        ...
        if (Rounds[lastId].status == 1) {
            require(
                ...
                "game is closed, waitting for new round"
            );
        }
        ...
        TaiXiuGame storage game = TaiXiuGames[msgSender];
        game.tokenAddress = tokenAddress;
        if (isBig) {
            game.tai += wager;
            Rounds[lastId].tai += wager;
        } else {
            game.xiu += wager;
            Rounds[lastId].xiu += wager;
        }
        if (!isPlayerListed(isBig, msgSender)) {
            if (isBig) big.push(msgSender);
            else small.push(msgSender);
        }
        if (isBig)
            emit TaiXiu_Play_EVENT(msgSender, game.tai, tokenAddress, isBig);
        else emit TaiXiu_Play_EVENT(msgSender, game.xiu, tokenAddress, isBig);

        emit TaiXiu_Round_INFO_Event(
            lastId,
            Rounds[lastId].tai,
            Rounds[lastId].xiu,
    ...
    function _TaiXiu_TakeBalance() internal {
        Rounds[lastId].status = 2;
        ...
        if (Rounds[lastId].tai > Rounds[lastId].xiu) {
            if (Rounds[lastId].xiu + bankrollAccepted >= Rounds[lastId].tai) {
                Rounds[lastId].bankrollAccepted =
                    Rounds[lastId].tai -
                    Rounds[lastId].xiu;
            } else {
                Rounds[lastId].bankrollAccepted = bankrollAccepted;
                uint256 tempTai = Rounds[lastId].xiu + bankrollAccepted;
                uint256 length = big.length;
                for (uint256 i = 0; i < length; i++) {
                    address _playerAddress = big[i];
                    uint256 tempPlayerBet = (TaiXiuGames[_playerAddress].tai *
                        tempTai) / Rounds[lastId].tai;
                    balance[Rounds[lastId].tokenAddress][
                        _playerAddress
                    ] += (TaiXiuGames[_playerAddress].tai - tempPlayerBet);
                    TaiXiuGames[_playerAddress].tai = tempPlayerBet;
                }
                Rounds[lastId].tai = tempTai;
            }
        } else {
            if (Rounds[lastId].tai + bankrollAccepted > Rounds[lastId].xiu) {
                Rounds[lastId].bankrollAccepted =
                    Rounds[lastId].xiu -
                    Rounds[lastId].tai;
            } else {
                Rounds[lastId].bankrollAccepted = bankrollAccepted;
                uint256 tempXiu = Rounds[lastId].tai + bankrollAccepted;
                uint256 length = small.length;
                for (uint256 i = 0; i < length; i++) {
                    address _playerAddress = small[i];
                    uint256 tempPlayerBet = (TaiXiuGames[_playerAddress].xiu *
                        tempXiu) / Rounds[lastId].xiu;
                    balance[Rounds[lastId].tokenAddress][
                        _playerAddress
                    ] += (TaiXiuGames[_playerAddress].xiu - tempPlayerBet);
                    TaiXiuGames[_playerAddress].xiu = tempPlayerBet;
                }
                Rounds[lastId].xiu = tempXiu;
            }
        }
        Rounds[lastId].blockNumber = uint64(block.number);
        Rounds[lastId].requestId = _requestRandomWords(3);
        emit TaiXiu_Take_Balance_EVENT(
            Rounds[lastId].tai,
            Rounds[lastId].xiu,
            Rounds[lastId].bankrollAccepted
        );
    ...
    function _TaiXiu_Refund() internal {
        Rounds[lastId].status = 5; // Refunding
        uint256 lengthXiu = small.length;
        for (uint256 i = 0; i < lengthXiu; i++) {
            address _playerAddress = small[i];
            balance[TaiXiuGames[_playerAddress].tokenAddress][
                _playerAddress
            ] += TaiXiuGames[_playerAddress].xiu;
            TaiXiuGames[_playerAddress].xiu = 0;
        }
        uint256 lengthTai = big.length;
        for (uint256 j = 0; j < lengthTai; j++) {
            address _playerAddress = big[j];
            balance[TaiXiuGames[_playerAddress].tokenAddress][
                _playerAddress
            ] += TaiXiuGames[_playerAddress].tai;
            TaiXiuGames[_playerAddress].tai = 0;
        }
        ...
        override
    {
        ...
        uint8 xucxac1 = uint8((randomWords[0] % 6) + 1);
        uint8 xucxac2 = uint8((randomWords[1] % 6) + 1);
        uint8 xucxac3 = uint8((randomWords[2] % 6) + 1);

        Results[lastId] = Result(xucxac1, xucxac2, xucxac3);

        Rounds[lastId].status = 3;
        emit TaiXiu_VRF_Request_EVENT(xucxac1, xucxac2, xucxac3);
    }
    ...
    function openBowl() internal {
        ...
        houseEdge[tokenAddress] +=
            (_round.tai + _round.xiu + _round.bankrollAccepted) /
            100;
        ...
        if (_openBowl(Results[lastId])) {
            ...
            for (uint256 j = 0; j < lengthTai; j++) {
                ...
                balance[tokenAddress][_playerAddress] +=
                    (TaiXiuGames[_playerAddress].tai * 198) /
                    100;
                ...
                if (refferal != address(0)) {
                    balance[tokenAddress][refferal] +=
                        TaiXiuGames[_playerAddress].tai /
                        1000;
                    houseEdge[tokenAddress] -= TaiXiuGames[_playerAddress].tai /
                        1000;
                }
                TaiXiuGames[_playerAddress].tai = 0;
            }
            ...
            for (uint256 i = 0; i < lengthXiu; i++) {
                ...
                if (refferal != address(0)) {
                    balance[tokenAddress][refferal] +=
                        TaiXiuGames[_playerAddress].xiu /
                        1000;
                    houseEdge[tokenAddress] -= TaiXiuGames[_playerAddress].xiu /
                        1000;
                }
                TaiXiuGames[_playerAddress].xiu = 0;
            }
            if (_round.tai > _round.xiu)
            ...
            for (uint256 i = 0; i < lengthXiu; i++) {
                address _playerAddress = small[i];
                balance[tokenAddress][_playerAddress] +=
                    (TaiXiuGames[_playerAddress].xiu * 198) /
                    100;
                // Send reward to Refferal.
                address refferal = Bankroll.getRef(_playerAddress);
                if (refferal != address(0)) {
    ...
    function _openBowl(Result memory mobat) private pure returns (bool) {
        uint8 kqua = mobat.xucxac1 + mobat.xucxac2 + mobat.xucxac3;
        if (kqua > 10) return true;
        else return false;
    }

    function isPlayerListed(bool _isBig, address playerAddress)
        private
        view
        returns (bool)
    {
        address[] storage players;
        if (_isBig) players = big;
        else players = small;
