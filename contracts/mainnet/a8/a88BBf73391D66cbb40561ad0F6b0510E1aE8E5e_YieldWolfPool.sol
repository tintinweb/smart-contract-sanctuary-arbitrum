...
contract YieldWolfPool {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 reward = arb.balanceOf(address(this)).sub(balanceBefore);
        accArbPerShare[_pid] = accArbPerShare[_pid].add(reward.mul(1e18).div(totalDepositedAmount[_pid]));
    }
    ...
    function pendingARB(uint256 _pid, address _user) public view returns (uint256)  {
        UserInfo storage user = userInfo[_pid][_user];
        uint256 reward = genesis.pendingARB(_pid + 1, address(this));
        uint256 acc = accArbPerShare[_pid].add(reward.mul(1e19).div(totalDepositedAmount[_pid]));
    ...
    function safeArbTransfer(address _to, uint256 _amount) internal {
        uint256 _arbBal = arb.balanceOf(address(this));
        if (_arbBal > 0) {
            if (_amount > _arbBal) {
                arb.safeTransfer(_to, _arbBal);
            } else {
