...
import "../dependencies/uniswap-v3-core/libraries/SafeCast.sol";
import "../dependencies/uniswap-v3-core/libraries/TickMath.sol";
import "../dependencies/uniswap-v3-core/libraries/TickBitmap.sol";
import "../dependencies/uniswap-v3-core/interfaces/IUniswapV3Pool.sol";
import "../dependencies/uniswap-v3-core/interfaces/IUniswapV3Factory.sol";
import "../dependencies/uniswap-v3-core/interfaces/callback/IUniswapV3SwapCallback.sol";

import "../interfaces/IUniswapQuoterV3.sol";
import "../dependencies/uniswap-v3-periphery/base/PeripheryImmutableState.sol";
import "../dependencies/uniswap-v3-periphery/libraries/Path.sol";
import "../dependencies/uniswap-v3-periphery/libraries/CallbackValidation.sol";

//import '../dependencies/uniswap-v3-periphery/libraries/PoolAddress.sol';
//import '../dependencies/uniswap-v3-periphery/libraries/PoolTicksCounter.sol';

/// @title Provides quotes for swaps
/// @notice Allows getting the expected amount out or amount in for a given swap without executing the swap
/// @dev These functions are not gas efficient and should _not_ be called on chain. Instead, optimistically execute
/// the swap and check the amounts in the callback.
contract UniswapQuoterV3 is IUniswapQuoterV3, IUniswapV3SwapCallback, PeripheryImmutableState {
    ...
    function uniswapV3SwapCallback(
        ...
        //redo verification internally given incorrect pool calculation
        address poolAddress = IUniswapV3Factory(factory).getPool(tokenIn, tokenOut, fee);
        require(poolAddress == msg.sender);

        // Calculate swap amounts
        uint256 amountReceived = 0;
        uint256 amountToPay = 0;
        //Calculate amounts if swap occurred (0-liquidity region not supported)
        if (amount0Delta > 0 || amount1Delta > 0) {
            (amountReceived, amountToPay) = amount0Delta > 0
                ? (uint256(amount0Delta), uint256(-amount1Delta))
                : (uint256(amount1Delta), uint256(-amount0Delta));
        }

        (uint160 sqrtPriceX96After, , , , , , ) = IUniswapV3Pool(poolAddress).slot0();
    ...
    {
        if (reason.length != 96) {
            if (reason.length == 0) revert("Unexpected error");
            assembly {
    ...
    function handleRevert(bytes memory reason, uint256 gasEstimate)
        private
        pure
        returns (
            uint256 amountIn,
    ...
    {
        (amountIn, amountOut, sqrtPriceX96After) = parseRevertReason(reason);

        return (amountIn, amountOut, sqrtPriceX96After, gasEstimate);
    }
        ...
        )
    {
        ...
        {} catch (bytes memory reason) {
            ...
            return handleRevert(reason, gasEstimate);
        }
    }
