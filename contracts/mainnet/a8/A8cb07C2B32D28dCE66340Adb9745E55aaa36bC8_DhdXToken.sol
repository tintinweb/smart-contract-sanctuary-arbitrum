...
contract DhdXToken is ERC20,ERC20Permit,ERC20Votes,Ownable {
    ...
    modifier onlyMinter() {
        require(isMinter(msg.sender), "DhdXToken: CALLER_IS_NOT_THE_MINTER");
        _;
    }

    constructor() ERC20("DhdX Token", "DHDX") ERC20Permit("")  Ownable(address(this))  {
    }
    ...
    function addMinter(address minter) external onlyOwner returns (bool) {
        require(minter != address(0), "DhdXToken.addMinter: ZERO_ADDRESS");
    ...
    function removeMinter(address minter) external onlyOwner returns (bool) {
        require(minter != address(0), "DhdXToken.delMinter: ZERO_ADDRESS");
    ...
    function getMinter(uint256 index) external view onlyOwner returns (address) {
        require(index <= EnumerableSet.length(minters) - 1, "DhdXToken.getMinter: OUT_OF_BOUNDS");
