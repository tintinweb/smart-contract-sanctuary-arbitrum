...
contract EnigmaFactory is Ownable, IEnigmaFactory {
    ...
    function deployEnigmaPool(address _token0, address _token1, uint24[] calldata _feeTiers, uint256 _selectedFee)
        external
        returns (IEnigma pool_)
    {
        ...
        Enigma(address(pool_)).initialize(address(uniswapFactory), token0, token1, _feeTiers, _selectedFee, msg.sender);
...
import "./interfaces/IEnigmaFactory.sol";
import {
    Range, Rebalance, WithdrawParams, PositionLiquidity, DepositParams, BurnParams
} from "./structs/EnigmaStructs.sol";

import "./abstract/EnigmaStorage.sol";
import {UniswapLiquidityManagement} from "./libs/UniswapLiquidityManagement.sol";
...
contract Enigma is EnigmaStorage, IUniswapV3MintCallback, ReentrancyGuard {
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        _applyFeesDistribute(fee0Accumulator, fee0Accumulator);
    ...
    function rebalance(Rebalance calldata rebalanceData) external onlyOperator nonReentrant {
        ...
        for (uint256 index; index < rebalanceData.burns.length; index++) {
            ...
            BurnParams memory _params = BurnParams(
                _pool, _burn.range.tickLower, _burn.range.tickUpper, liquidityToRemove.toUint128(), address(this), false
            );
...
import {Range, Rebalance, DepositParams} from "../structs/EnigmaStructs.sol";
...
import {Range} from "../structs/EnigmaStructs.sol";
