...
import {ILeveragedPool} from "mycelium/ILeveragedPool.sol";
...
contract MyceliumPoolPositionManager is IPositionManager {
  ...
  function PositionWorth() public view returns (uint256) {
    ...
    while (true) {
      try poolCommitter.unAggregatedCommitments(address(poolPositionPurchaser),currentIndex) returns (uint256 intervalId) {
        currentIndex += 1;
      } catch {
        break;
      }
...
import {IPoolStateHelper} from "mycelium/IPoolStateHelper.sol";
import {ILeveragedPool} from "mycelium/ILeveragedPool.sol";
import {ExpectedPoolState, ILeveragedPool2} from "mycelium/IPoolStateHelper.sol";
