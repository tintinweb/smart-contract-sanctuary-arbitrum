...
contract STESTHELPERV4 is Initializable, OwnableUpgradeable {
...
/// STAIK Token address 
    ...
    */
    ...
    function updateBaseValuesIfRequired() public {
        ...
        if (staikBaseValue == 0 && testingFunctions == true && staikPriceTestValue > 0) { // true
    ...
    function updateSTAIKBaseIfRequired() public {
        if (staikBaseGrowthWaitPeriod > 0 && staikBaseValue != 0) { // false
    ...
    */
    ...
    /// allows for testing with price override
    function staikPriceInUSD() public view returns (uint256) {    
        if(staikOracle.getRate(STAIK, USDC, true) == 0 && testingFunctions == true) {
            return staikPriceTestValue;
        }
        else return staikOracle.getRate(STAIK, USDC, true);
    }
