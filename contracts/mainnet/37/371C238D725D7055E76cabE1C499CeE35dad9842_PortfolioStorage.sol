...
contract PortfolioStorage is AuthorizedAccessControl, IPortfolioStorage {

  using EnumerableSet for EnumerableSet.AddressSet;

  mapping (address => EnumerableSet.AddressSet) private _assets;

  mapping (address => mapping (address => uint256)) private _shares;

  mapping (address => mapping (address => uint256)) private _lastUpdateBlocks;

  constructor(IAddressRegistry registry) AdminAccessControl(registry) { }

  /// @inheritdoc IPortfolioStorage
  function getCount(address account) external view returns (uint256) {
    return _assets[account].length();
  }

  /// @inheritdoc IPortfolioStorage
  function getPortfolioAssets(address account, uint16 mask) external view returns (IPortfolioStorage.PortfolioAsset[] memory) {
    EnumerableSet.AddressSet storage assets = _assets[account];

    uint256 length = assets.length();
    PortfolioAsset[] memory portfolioAssets = new PortfolioAsset[](length);

    for (uint256 i = 0; i < length;) {
      address token = assets.at(i);
      portfolioAssets[i] = load(account, token, mask);
  ...
  /// @inheritdoc IPortfolioStorage
  function getPortfolioAsset(address account, address token, uint16 mask) external view returns (IPortfolioStorage.PortfolioAsset memory) {
    EnumerableSet.AddressSet storage assets = _assets[account];

    // return a default in the case that it doesnt exist
    if (assets.contains(token) == false) {
      return IPortfolioStorage.PortfolioAsset({
        token: token,
        lastUpdateBlock: 0,
        shares: 0
      });
    ...
    return load(account, token, mask);
  }

  function load(address account, address token, uint16 mask) private view returns (IPortfolioStorage.PortfolioAsset memory portfolioAsset) {
    portfolioAsset.token = token;
    portfolioAsset.shares = mask & PORTFOLIO_ASSET_FIELD_SHARES != 0 ? _shares[account][token] : 0;
    portfolioAsset.lastUpdateBlock = mask & PORTFOLIO_ASSET_FIELD_LAST_UPDATE_BLOCK != 0 ? _lastUpdateBlocks[account][token] : 0;
  }

  /// @inheritdoc IPortfolioStorage
  function increaseShares(address account, address token, uint256 shares) external onlyAuthorized returns (uint256) {
    EnumerableSet.AddressSet storage assets = _assets[account];
    ...
    shares = _shares[account][token] + shares;

    _shares[account][token] = shares;
    _lastUpdateBlocks[account][token] = block.number;

    return shares;
  }

  /// @inheritdoc IPortfolioStorage
  function decreaseShares(address account, address token, uint256 shares) external onlyAuthorized returns (uint256) {
    EnumerableSet.AddressSet storage assets = _assets[account];

    if (assets.contains(token) == false) {
      revert Errors.PortfolioStorage_AssetNotFound(account, token);
    }
    ...
    if (shares > 0) {
      ...
      // no shares left so we can delete the asset
      delete _shares[account][token];
      delete _lastUpdateBlocks[account][token];
