...
contract Token is Ownable, ERC20{
    ...
    TokenVesting public motivateLockToken;
    ...
    constructor(string memory name_, string memory symbol_) Ownable() ERC20(name_,symbol_){  
        treasuryLockToken = new TokenVesting(msg.sender, block.timestamp + 90 days, 0 days, 150 days);
        motivateLockToken = new TokenVesting(msg.sender, block.timestamp + 90 days, 0 days, 240 days);
        ...
        __mint(msg.sender, 150 * 1e6 * 1e18);
        __mint(address(treasuryLockToken), 100 * 1e6 * 1e18);
        __mint(address(motivateLockToken), 80 * 1e6 * 1e18);
        __mint(address(IDOLockToken), 100 * 1e6 * 1e18);
        __mint(address(teamLockToken), 20 * 1e6 * 1e18);
    }

    function mintToFarm(address _farm) external onlyOwner {
        require(!once, "only can mint once");
        once = true;
        __mint(_farm, 550 * 1e6 * 1e18);
    }
