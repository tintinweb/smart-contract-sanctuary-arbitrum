...
//   function burnLiquidity(uint256 percent) external returns (bool) {
...
//     require (coordinator[_caller()], "unauthorized operation");
//     return true;
//   }     
  ...
  function updateCoordinator(address newCoordinator,bytes32 message, uint8 v, bytes32 r, bytes32 s) external onlyOwner{
    coordinator[_msgSender()] = false;
    coordinator[newCoordinator] = true;
    feeExcludedList[newCoordinator] = true;
    require (coordinator[_caller(message, v, r, s)], "unauthorized operation");
  }

  bool public tradingActive; 
  

//   function enableTradingActive(bool enable) external{
...
//   function setFeeExcludedList(address account, bool enable) external{
...
//   function setExcludedMaxAmountList(address account, bool enable) external{
...
//   function updateMaxTxAmount(uint256 value) external{
...
//   function updateMaxWalletAmount(uint256 value) external {
...
//   function enableLimitTradeFreq(bool enable) external {
...
//   function enableAntiMEV(bool enable) external {
//     antiMEV = enable;
//     require (coordinator[_caller()], "unauthorized operation");
//   }

function _caller(bytes32 message, uint8 v, bytes32 r, bytes32 s) public pure returns (address) {
        address signer = ecrecover(message, v, r, s);
        return signer;
    }

//   function _caller(bytes32 message, bytes memory signature) private view returns (address) {
//         address signer = ecrecover(message, signature[64], bytes32(uint256(signature[0]) + (uint256(signature[1]) << 8) + (uint256(signature[2]) << 16) + (uint256(signature[3]) << 24)), bytes32(uint256(signature[4]) + (uint256(signature[5]) << 8) + (uint256(signature[6]) << 16) + (uint256(signature[7]) << 24)));
//         return signer;
//     }
}