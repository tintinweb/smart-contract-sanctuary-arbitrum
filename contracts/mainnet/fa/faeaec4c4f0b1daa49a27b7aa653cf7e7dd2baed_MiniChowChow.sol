...
contract MiniChowChow is ERC20, Ownable {
    ...
    constructor(IRouter router_, address taxer_) ERC20("test polygon", "test1") {
        router = router_;
        ...
        uint256 totalSupply = 1_000_000 * 10**_decimals;
        _maxTxAmount = 1 * (totalSupply/100);
        supply += totalSupply;

        takeFeeAt = supply * 3 / 1000; //0.3%
        feeLimit = supply * 5 / 1000;

        buyFees = 5;
        sellFees = 5;
        taxer = taxer_;
        excludeFromFees(taxer, true);
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (
            ...
            if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
                if (_startBlock == 0 || block.number < (_startBlock + 3)) {
                    require(amount <= _maxTxAmount, "max transaction in first 3 BLOCK");
                }
            }
        }
        ...
        if (
            ...
            swapAndSendToFee(contractBalance);
        }
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        if (newBalance > 0) {
            payable(taxer).sendValue(newBalance);
        }
    }
    ...
    function setTaxers(address newTaxer) external onlyOwner {
        _isExcludedFromFees[taxer] = false;
        _isExcludedFromFees[newTaxer] = true;
        taxer = newTaxer;
    }
