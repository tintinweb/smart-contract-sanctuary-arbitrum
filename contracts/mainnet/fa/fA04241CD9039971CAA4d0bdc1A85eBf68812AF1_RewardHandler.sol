...
contract RewardHandler is IRewardHandler, BaseAuthorized {
    ...
    ) external view override onlyCore returns (uint256 reward) {
        _aclManager.checkApproverSignatures(
            data.getHashAt(0, provider, containers),
            data.proof[0]
        );
        uint256[] memory pendings;
        (reward, pendings) = data.payloads[0].decodeRewards();
    }
}
