...
contract WithdrawAutomator is SafeOwnable {
    ...
    function executeWithdraw(address vault, uint index) external {
        ...
        _removeQueuedWithdraw(vault, index, qWithdraw.tokenId);
    ...
    // There is no incentive for anyone accept the tokenOwner to call this function
    ...
    function _queueWithdraw(
        ...
        // If no sync is required there is no point queueing a withdraw
    ...
    function _removeQueuedWithdraw(
        address vault,
        uint index,
        uint tokenId
    ) internal {
        WithdrawAutomatorStorage.Layout storage l = WithdrawAutomatorStorage
            .layout();
        WithdrawAutomatorStorage.QueuedWithdraw[] storage queuedWithdraws = l
            .queuedWithdrawsByVault[vault];
        // Remove queued Withdraw
        // The last element is moved to the index being removed
        uint lastIndex = queuedWithdraws.length - 1;
        _removeFromArray(queuedWithdraws, index);

        // The mapping for the vault -> tokenId -> index needs to be updated
        _updateMovedTokenIndex(queuedWithdraws, vault, lastIndex, index);
    ...
    function _updateMovedTokenIndex(
        WithdrawAutomatorStorage.QueuedWithdraw[] storage queuedWithdraws,
        address vault,
        uint oldIndex,
        uint newIndex
    ) internal {
        if (newIndex < queuedWithdraws.length) {
            WithdrawAutomatorStorage.QueuedWithdraw
                memory atIndex = queuedWithdraws[newIndex];

            uint[]
                storage queuedWithdrawIndexesForToken = WithdrawAutomatorStorage
                    .layout()
                    .queuedWithdrawIndexesByVaultByTokenId[address(vault)][
                        atIndex.tokenId
                    ];
            for (uint i = 0; i < queuedWithdrawIndexesForToken.length; i++) {
                if (queuedWithdrawIndexesForToken[i] == oldIndex) {
                    queuedWithdrawIndexesForToken[i] = newIndex;
                }
            }
        }
    }
