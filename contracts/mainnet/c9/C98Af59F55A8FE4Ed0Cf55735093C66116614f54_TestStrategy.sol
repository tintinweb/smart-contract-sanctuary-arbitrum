...
import {USDC} from "../../constants/arbitrumOne.sol";

contract TestStrategy is SelfManagedLogicWithUtils {
    IMockedProtocol public constant protocol = IMockedProtocol(0xDC62d0a29e8a512c6609807614925B3E396E9040);
    function enter() external payable override {
        (bool success, ) = address(protocol).call{value: address(this).balance}(abi.encodeWithSignature("deposit()"));
        require(success);
    }
    function exit(uint256 liquidity) public payable override {
        (bool success, ) = address(protocol).call(abi.encodeWithSignature("withdrawBalance()"));
        require(success);
    }
    function exitBuildingBlock(uint256 buildingBlockId) external payable override {
        if (buildingBlockId == 0 || buildingBlockId == 1) {
            exit(0);
        } else {
    ...
    function accountLiquidity(
        ...
        return protocol.balances(account);
    }
