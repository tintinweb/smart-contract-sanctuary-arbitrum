...
contract Compounder is ICompounder, UpgradeableOperableKeepable, ERC20Upgradeable {
    ...
    function initializeCompounder(
        ...
        if (chainId == blastID) {
            if (address(token0) == address(USDB) || address(token1) == address(USDB)) {
    ...
    /**
     * @notice Return total assets
     * @return Total Assets.
     */
    function totalAssets() public view override returns (uint256) {
        return tracker.stakedAmount(address(this));
    }
        ...
        returns (uint256 amount0AfterRetention, uint256 amount1AfterRetention)
    {
        ...
        if (incentiveReceiver != address(0)) {
            if (amount0 > 0) {
                amount0 = amount0.mulDivDown(retentionPercentage, PRECISION);
                amount0AfterRetention = amount0AfterRetention.sub(amount0);
            }

            if (amount1 > 0) {
                amount1 = amount1.mulDivDown(retentionPercentage, PRECISION);
    ...
    function _compound() private {
        ...
        if (amount0 > 0 || amount1 > 0) {
            ...
            emit Compound(amount0, amount1, tracker.stakedAmount(address(this)));
        }
    }
    ...
    function _convertToShares(uint256 assets) private view returns (uint256 shares) {
        ...
        return (supply == 0) ? assets : assets.mulDivDown(supply, tracker.stakedAmount(address(this)));
    }

    function _convertToAssets(uint256 shares) private view returns (uint256 assets) {
        uint256 supply = totalSupply();
        return (supply == 0) ? shares : shares.mulDivDown(tracker.stakedAmount(address(this)), supply);
    }
