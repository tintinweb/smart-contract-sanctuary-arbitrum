...
contract GmxUtils is IOrderCallbackReceiver, Initializable, Ownable2StepUpgradeable {
  ...
  function getExecutionGasLimit(bool isDeposit, uint256 _callbackGasLimit) external view returns (uint256) {
    if (isDeposit) {
      return getExecutionGasLimit(Order.OrderType.MarketIncrease, _callbackGasLimit);
    } else {
      return getExecutionGasLimit(Order.OrderType.MarketDecrease, _callbackGasLimit);
    }
  }
