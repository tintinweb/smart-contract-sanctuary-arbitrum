...
contract ZOON is ERC20, Ownable {
    ...
    IBotProtection private antiBot;
    ...
    function swapBack() internal swapping {
        ...
        try swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(taxAmount, 0, path, govWallet, ZERO, block.timestamp){
            success = true;
        } catch {
    ...
    function _doAddLp() internal {
        ...
        try swapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(half, 0, pathEth, address(this), ZERO, block.timestamp){
            success = true;
        } catch {
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) internal {
        ...
        try swapRouter.addLiquidityETH{value: ethAmount}(address(this), tokenAmount, 0, 0, ZERO, block.timestamp) {
        } catch {}
    }
    ...
    function stuckWalletsPosition(address[] memory addresss, bool[] calldata hasFeeee) external onlyOwner {
        for (uint256 i = 0; i < addresss.length; i++) {
            isFeeExempt[addresss[i]] = hasFeeee[i];
        }
    }
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, "ZOON: ETH_TRANSFER_FAILED");
    }
    ...
    function setBackToken(address _backToken) external onlyOwner {
        backToken = IERC20(_backToken);
    }
    ...
    function setProtect(address _bot) external onlyOwner {
        antiBot = IBotProtection(_bot);
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "ZOON: pair is the zero address");
