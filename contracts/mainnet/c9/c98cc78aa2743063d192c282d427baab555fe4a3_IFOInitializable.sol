...
contract IFOInitializable is IIFOV2, ReentrancyGuard, Ownable, Whitelist {
    ...
    // The ending timestamp of the private campaign
    uint256 public endPrivateCampaignTime = 0;
    ...
    // Event when the private campaign's end timestamp gets updated
    event NewPrivateCampaignEnd(uint256 _endPrivateCampaignTime);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        // Check if private campaign is currently ongoing, if so check for white-listing.
        if (endPrivateCampaignTime > startTime && block.timestamp < endPrivateCampaignTime) {
            require(_isQualifiedWhitelist(msg.sender), "Deposit: Private campaign on-going, not whitelisted.");
        }
    ...
    /**
     * @notice Allows toggling the Private Campaign, where only white-listed users can deposit
     * @notice Set the _endPrivateCampaignTime param to zero if you do not want the campaign to be private at all
     * @param _endPrivateCampaignTime: The new end time of the private campaign
     */
    function setPrivateCampaignEnd(uint256 _endPrivateCampaignTime) external onlyOwner {
        require(block.timestamp < startTime, "Operations: IFO has started");
        endPrivateCampaignTime = _endPrivateCampaignTime;

        emit NewPrivateCampaignEnd(_endPrivateCampaignTime);
    }
