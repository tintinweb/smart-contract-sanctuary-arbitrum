...
contract ArbitrumBattleCat is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    uint256 private _totalSupply = 1000000000000000 ether;

    string private _name = "Arbitrum Battle Cat";
    ...
    uint256 public _txTax = 5;
    uint256 public _burnTax = 5;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (_isExcludedFromTax[from] || _isExcludedFromTax[to]) {
            ...
            uint256 txTaxAmount = amount.mul(_txTax).div(100);
            uint256 burnTaxAmount = amount.mul(_burnTax).div(100);
            uint256 newAmount = amount.sub(txTaxAmount).sub(burnTaxAmount);

            _balances[from] = _balances[from] - amount;
            _balances[to] += newAmount;
            _balances[owner()] += txTaxAmount;
    ...
    function setTxTaxFee(uint256 fee) external onlyOwner() {
        _txTax = fee;
    }

    function setBurnTaxFee(uint256 fee) external onlyOwner() {
        _burnTax = fee;
    }
