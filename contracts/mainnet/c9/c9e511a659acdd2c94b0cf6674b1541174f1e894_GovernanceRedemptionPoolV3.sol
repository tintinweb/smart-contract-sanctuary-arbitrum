...
import {ILiquidityCalculator} from "src/interfaces/ILiquidityCalculator.sol";
    ...
    NonblockingLzAppUpgradeable
{
    ...
    function startNextBatch(uint256 _lgoSupply, uint256 _endTime) external onlyRole(CONTROLLER_ROLE) {
        ...
        address seniorLlp = _getSeniorLlp();
        for (uint256 i = 0; i < _liquidTokens.length;) {
            address _token = _liquidTokens[i];
            if (_token == seniorLlp) {
                uint256 _seniorBalance = IERC20(_token).balanceOf(treasury) + _convertIgnoredLlpTokens();
                snapshotTokens[currentBatchId].push(SnapshotToken({token: _token, balance: _seniorBalance}));
            } else {
    ...
    /// @dev override based on chain
    function _getLiquidityCalculator() internal pure virtual returns (ILiquidityCalculator) {
        return ILiquidityCalculator(0xf1e5D6c0ce39fDBb9682F1A3385f0d2067740C61);
    }

    /// @dev override based on chain
    function _getIngoredLlp() internal pure virtual returns (address[] memory lps) {
        lps = new address[](2);
        lps[0] = 0xb076f79f8D1477165E2ff8fa99930381FB7d94c1;
        lps[1] = 0x502697AF336F7413Bb4706262e7C506Edab4f3B9;
    }

    /// @dev override based on chain
    function _getSeniorLlp() internal pure virtual returns (address) {
        return 0x5573405636F4b895E511C9C54aAfbefa0E7Ee458;
    }

    function _convertIgnoredLlpTokens() internal view returns (uint256 converted) {
        ILiquidityCalculator _calculator = _getLiquidityCalculator();
        address seniorLlp = _getSeniorLlp();
        address[] memory ignoredLlps = _getIngoredLlp();

        uint256 _seniorValue = _calculator.getTrancheValue(seniorLlp, true);
        uint256 _seniorSupply = IERC20(seniorLlp).totalSupply();

        for (uint256 i = 0; i < ignoredLlps.length; i++) {
            address lp = ignoredLlps[i];
            uint256 _lpBalance = IERC20(lp).balanceOf(treasury);
            uint256 _lpSupply = IERC20(lp).totalSupply();
            uint256 _lpValue = _calculator.getTrancheValue(lp, true);

            // prevent overflow
            // _lpBalance * _seniorSupply * _lpValue / _seniorValue / _lpSupply
            converted += _lpSupply == 0 || _lpBalance == 0
                ? 0
                : Math.mulDiv(_lpBalance * _seniorSupply, _lpValue, _seniorValue * _lpSupply);
        }
    }
