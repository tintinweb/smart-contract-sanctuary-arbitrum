...
abstract contract UpgradeableVault is ERC4626Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function depositWithRewards(uint256 amount, address to) public virtual returns (uint256) {
    ...
    function deposit(uint256 amount, address to) public virtual override returns (uint256) {
        require(amount > 0, "UpgradeableVault: deposit amount must be greater than 0");
        require(to != address(0), "UpgradeableVault: deposit to the zero address");
    ...
    function resetTwap(uint256 _twap) public onlyOwner {
        require(_twap > 0, "UpgradeableVault: twap must be greater than 0");

        for (uint256 i = 0; i < ratePoints.length; i++) {
            ratePoints[i].timestamp = block.timestamp;
            ratePoints[i].rate = _twap;
        }
    }
