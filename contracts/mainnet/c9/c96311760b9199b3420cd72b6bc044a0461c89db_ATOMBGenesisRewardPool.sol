...
contract ATOMBGenesisRewardPool {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ATOMB to distribute.
    ...
    // MAINNET
     uint256 public ATOMBPerSecond = 0.4629629 ether; // 80000 ATOMB / (24h * 60min * 60s * 2) 0.4629629
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "ATOMBGenesisPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "ATOMBGenesisPool: existing pool?");
        }
    }
    ...
    // Update the given pool's ATOMB allocation point. Can only be called by the owner.
    ...
    function pendingATOMB(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function safeATOMBTransfer(address _to, uint256 _amount) internal {
        uint256 _ATOMBBalance = ATOMB.balanceOf(address(this));
        if (_ATOMBBalance > 0) {
            if (_amount > _ATOMBBalance) {
                ATOMB.safeTransfer(_to, _ATOMBBalance);
            } else {
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 30 days) {
            // do not allow to drain core token (ATOMB or lps) if less than 30 days after pool ends
