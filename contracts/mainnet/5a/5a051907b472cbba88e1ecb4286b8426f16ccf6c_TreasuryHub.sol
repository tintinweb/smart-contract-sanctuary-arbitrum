...
contract TreasuryHub is Ownable {
    ...
    event USDCTransfered();
    event USDTTransfered();
    event DAITransfered();
    event TokenTransfered();
    event ETHTransfered();

    modifier onlyAdmin {
        require(msg.sender == address(sentry)
        || msg.sender == owner(),
        "FORBIDDEN: sender must be sentry.");
        _;
    }

    /**
     * @notice Set the contract for sending tokens
     * @param _sentry is the contract address
     */
    function setSentry(address _sentry) onlyOwner external {
        sentry = _sentry;
    }

    function approveSpecificToken(address _token,
                                uint _amount)
        public
        onlyOwner
    {
        IERC20(_token).approve(_token, _amount);
    }

    function transferUSDC(address _target, uint _amount)
        public
        onlyOwner
    {
    ...
    function transferUSDT(address _target, uint _amount)
        public
        onlyOwner
    {
        IERC20(USDT).transfer(_target, _amount);
        emit USDTTransfered();
    }

    function transferDAI(address _target, uint _amount)
        public
        onlyOwner
    {
        IERC20(DAI).transfer(_target, _amount);
        emit DAITransfered();
    }

    function transferETHToOwner(uint256 amount) public onlyOwner {
        (bool success, ) = owner().call{value: amount}("");
        require(success, "ERR: cannot boost reward");
        emit ETHTransfered();
    }
