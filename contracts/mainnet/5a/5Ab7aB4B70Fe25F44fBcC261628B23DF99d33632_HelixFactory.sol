...
contract HelixFactory is HelixBase, IHelixFactory {
     ...
     * @param _dealCurrency The token in which deal's investment and repayments happen
    ...
    function createDeal(
        address _borrower,
        address _dealWallet,
        address _dealCurrency, 
        uint256 _threshold, // 2 admins - 1 borrow -> 66% // 2 admins -> 100%
        uint256 _salt, 
        uint256[] calldata _allowedUIDTypes,
        uint256 dbDealId
    ) external override onlyProtocolAdmin returns (IHelixDeal) {
        ...
        {
            deal.initialize(
                [address(config), dealManager, _dealWallet, _dealCurrency, _borrower],
                _allowedUIDTypes
            );
