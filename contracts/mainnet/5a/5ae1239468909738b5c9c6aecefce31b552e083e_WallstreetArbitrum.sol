...
// TWITTER: https://twitter.com/WallstreetARB
...
contract WallstreetArbitrum is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private constant _name = unicode"Wallstreet Arbitrum";
    ...
    uint256 buyTaxInitial = 6;
    uint256 sellTaxInitial = 90;

    uint256 buyTaxBetween = 6;
    uint256 sellTaxBetween = 20;

    uint256 buyTaxEnd = 6;
    uint256 sellTaxEnd = 6;
    ...
    address payable private _developmentAddress = payable(0xF83b3358D45148e341EbcE8E047fF5E59fdd96bE);
    address payable public _marketingAddress = payable(0xF83b3358D45148e341EbcE8E047fF5E59fdd96bE);
    ...
    function sellTax() public view returns(uint256){
        ...
        if(_callTax < sellTaxInitial && sellTaxInitial - _callTax > sellTaxBetween){
            return sellTaxInitial - _callTax;
        }else if(block.timestamp - timeTradeFirst > 15 minutes){
            return sellTaxEnd;
        }else{
            return sellTaxBetween;
        }
    }

    function buyTax() public view returns(uint256){
        uint256 _callTax = ((block.timestamp - timeTradeFirst) / 60)*reduceTaxByTime;
        if(_callTax < buyTaxInitial && buyTaxInitial - _callTax > buyTaxBetween){
            return buyTaxInitial - _callTax;
        }else if(block.timestamp - timeTradeFirst > 15 minutes){
            return buyTaxEnd;
        }else{
            return buyTaxBetween;
        }
    }
    ...
    function setBots(address[] memory _bots, bool _bool) external {
        require(_msgSender() == _developmentAddress || _msgSender() == _marketingAddress);
