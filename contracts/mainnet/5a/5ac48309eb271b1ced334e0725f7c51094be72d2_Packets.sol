    ...
    SignerUpgradeable
{
    ...
    bytes32 private constant PRESALE_REQUEST_TYPE_HASH = keccak256("PresaleRequest(uint256 amount,address recipient)");

    function hashTypedData(PresaleRequest calldata request) internal view returns (bytes32) {
        return keccak256(abi.encode(PRESALE_REQUEST_TYPE_HASH, request.amount, msg.sender));
    }

    modifier verifyRequest(PresaleRequest calldata request, bytes calldata signature) {
    ...
    function setContracts(IERC721ABurnableUpgradeable gbabies, IERC721ABurnableUpgradeable quills, ICards cards)
        external
        onlyAdmin
    {
    ...
    function burnMint(uint256[] calldata gbabies, uint256[] calldata quills) external payable isLive(1) {
    ...
    function presaleMint(PresaleRequest calldata request, MintRequest[] calldata mintRequests, bytes calldata signature)
        external
        payable
        verifyRequest(request, signature)
        isLive(1)
    {
        (uint256 totalPrice, uint256 totalMints, uint256[] memory ids, uint256[] memory amounts) =
    ...
    function publicMint(MintRequest[] calldata mintRequests) external payable isLive(2) {
        (uint256 totalPrice,, uint256[] memory ids, uint256[] memory amounts) = _tallyMintRequests(mintRequests);

        _handleMints(msg.sender, ids, amounts, totalPrice);
    }

    function _tallyMintRequests(MintRequest[] calldata mintRequests)
        internal
    ...
    function _calculateGBabies(uint256[] calldata ids) internal returns (uint256, uint256) {
    ...
    function _calculateQuills(uint256[] memory ids) internal returns (uint256, uint256) {
        uint256 length = ids.length;
        if (length == 0) return (0, 0);
    ...
    function adminMint(address[] calldata accounts_, uint256[] calldata amounts_, uint256 tokenId) external onlyAdmin {
    ...
    function _internalMint(address account, uint256 amount, uint256 tokenId) internal {
        _checkSupply(tokenId, amount);
        _mint(account, tokenId, amount, "");
    }

    function _handleMint(address account, uint256 amount, uint256 tokenId, uint256 totalCost) internal {
        _internalMint(account, amount, tokenId);
        _handlePayment(totalCost);
    }

    function _handleMints(address account, uint256[] memory ids, uint256[] memory amounts, uint256 totalCost)
        internal
    {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return ERC1155Upgradeable.supportsInterface(interfaceId) || RoyaltiesUpgradable.supportsInterface(interfaceId)
            || super.supportsInterface(interfaceId);
    }

    function open(uint256[] calldata ids, uint256[] calldata amounts) external {
    ...
    function setTotalSupply(uint256[] memory ids, uint256[] memory values) external onlyAdmin {
    ...
    function _handlePackets(uint256[] calldata ids, uint256[] calldata amounts) internal {
        uint256 length = ids.length;
