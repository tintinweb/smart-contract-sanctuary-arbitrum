...
interface IReaperBoosterConfig {
  // getter
  ...
  function reaperboosterNftAllowance(
...
interface IMasterChef {
  ...
  function pendingReaper(address _stakeToken, address _user) external view returns (uint256);
  ...
  function reaperPerBlock() external view returns (uint256);
  ...
  function depositReaper(address _for, uint256 _amount) external;

  function withdrawReaper(address _for, uint256 _amount) external;
  ...
  function reaper() external returns (IREAPER);
}
...
contract ReaperBooster is Ownable, Pausable, ReentrancyGuard, AccessControl, IMasterChefCallback, IERC721Receiver {
  ...
  IReaperBoosterConfig public reaperboosterConfig;
  IERC20 public reaper;
  ...
  constructor(
    ...
    IReaperBoosterConfig _reaperboosterConfig,
    IWNativeRelayer _wNativeRelayer,
    address _wNative
  ) public {
    require(_wNative != address(0), "ReaperBooster::constructor:: _wNative cannot be address(0)");
    require(address(_reaper) != address(0), "ReaperNFTOffering::constructor:: _reaper cannot be address(0)");
    require(address(_masterChef) != address(0), "ReaperNFTOffering::constructor:: _masterChef cannot be address(0)");
    require(
      address(_reaperboosterConfig) != address(0),
      "ReaperNFTOffering::constructor:: _reaperboosterConfig cannot be address(0)"
    );
    require(
      address(_wNativeRelayer) != address(0),
      "ReaperNFTOffering::constructor:: _wNativeRelayer cannot be address(0)"
    );
    ...
    reaperboosterConfig = _reaperboosterConfig;
    reaper = _reaper;
  ...
  modifier inExec() {
    require(_IN_EXEC_LOCK == _NOT_ENTERED, "ReaperBooster::inExec:: in exec lock");
    require(address(masterChef) == _msgSender(), "ReaperBooster::inExec:: not from the master chef");
  ...
  modifier isStakeTokenOK(address _stakeToken) {
    require(reaperboosterConfig.stakeTokenAllowance(_stakeToken), "ReaperBooster::isStakeTokenOK::bad stake token");
    _;
  }

  /// @dev validate whether a specified nft can be staked into a particular staoke token
  modifier isReaperBoosterNftOK(
    address _stakeToken,
    address _nftAddress,
    uint256 _nftTokenId
  ) {
    require(
      reaperboosterConfig.reaperboosterNftAllowance(_stakeToken, _nftAddress, _nftTokenId),
      "ReaperBooster::isReaperBoosterNftOK::bad nft"
    );
    _;
  }
  ...
  modifier permit(bytes calldata _sig) {
    ...
    require(recoveredAddress == _msgSender(), "ReaperBooster::permit::INVALID_SIGNATURE");
    _;
  }

  modifier onlyGovernance() {
    require(hasRole(GOVERNANCE_ROLE, _msgSender()), "ReaperBooster::onlyGovernance::only GOVERNANCE role");
    _;
  }

  /// @dev Require that the caller must be an EOA account to avoid flash loans.
  modifier onlyEOA() {
    require(msg.sender == tx.origin, "ReaperBooster::onlyEOA:: not eoa");
    _;
  }
  ...
  /// @dev View function to see pending booster REAPERs on frontend.
  function pendingBoosterReaper(address _stakeToken, address _user) external view returns (uint256) {
    uint256 pendingReaper = masterChef.pendingReaper(_stakeToken, _user);
    NFTStakingInfo memory stakingNFT = userStakingNFT[_stakeToken][_user];
    if (stakingNFT.nftAddress == address(0)) {
      return 0;
    }
    (, , uint256 boostBps) = reaperboosterConfig.energyInfo(stakingNFT.nftAddress, stakingNFT.nftTokenId);
    // if (currentEnergy == 0) {
    //   return 0;
    // }
    return pendingReaper.mul(boostBps).div(1e4);
    // return Math.min(currentEnergy, pendingReaper.mul(boostBps).div(1e4));
  }

  /// @dev Internal function for withdrareaper a boosted stake token and receive a reward from a master chef
  ...
  function _withdrawFromMasterChef(IERC20 _stakeToken, uint256 _shares) internal {
    if (_shares == 0) return;
    if (address(_stakeToken) == address(reaper)) {
      masterChef.withdrawReaper(_msgSender(), _shares);
    } else {
  ...
  function _harvestFromMasterChef(address user, IERC20 _stakeToken) internal {
    ...
    uint256 beforeReward = reaper.balanceOf(user);
    masterChef.harvest(user, address(_stakeToken));

    emit Harvest(user, _stakeToken, reaper.balanceOf(user).sub(beforeReward));
  }
  ...
  function stakeNFT(
    ...
    isReaperBoosterNftOK(_stakeToken, _nftAddress, _nftTokenId)
    nonReentrant
    onlyEOA
  {
  ...
  function _stakeNFT(
    ...
    require(
      ...
      "ReaperBooster::stakeNFT:: nft already staked"
    );
  ...
  function _unstakeNFT(address _stakeToken) internal {
    NFTStakingInfo memory toBeSentBackNft = userStakingNFT[_stakeToken][_msgSender()];
    require(toBeSentBackNft.nftAddress != address(0), "ReaperBooster::stakeNFT:: no nft staked");
  ...
  function stake(IERC20 _stakeToken, uint256 _amount)
    ...
    nonReentrant
  {
    require(_amount > 0, "ReaperBooster::stake::nothing to stake");
    ...
    if (address(_stakeToken) == address(reaper)) {
      masterChef.depositReaper(_msgSender(), _amount);
    } else {
  ...
  function _unstake(IERC20 _stakeToken, uint256 _amount) internal {
    require(_amount > 0, "ReaperBooster::_unstake::use harvest instead");
  ...
  function harvest(address[] calldata _stakeTokens) external whenNotPaused nonReentrant {
    for (uint256 i = 0; i < _stakeTokens.length; i++) {
      require(reaperboosterConfig.stakeTokenAllowance(_stakeTokens[i]), "ReaperBooster::harvest::bad stake token");
  ...
  /// @dev currently, the caller will be a master chef calling before a reaper lock
  function masterChefCall(
    address stakeToken,
    address userAddr,
    uint256 unboostedReward
    ...
    (, uint256 currentEnergy, uint256 boostBps) = reaperboosterConfig.energyInfo(
    ...
    // reaperboosterConfig.consumeEnergy(stakingNFT.nftAddress, stakingNFT.nftTokenId, extraReward);
  ...
  function _safeWrap(IERC20 _quoteBep20, uint256 _amount) internal {
    if (msg.value != 0) {
      require(address(_quoteBep20) == wNative, "ReaperBooster::_safeWrap:: baseToken is not wNative");
      require(_amount == msg.value, "ReaperBooster::_safeWrap:: value != msg.value");
