...
AccessControlUpgradeable {
    ...
    IPresale.PresaleState public presaleState = IPresale.PresaleState.NOT_STARTED;
    ...
    TokensInfoPerPhase[15] public tokensInfoPerPhase;
    ...
    /**
 * @dev Executes a token purchase, allowing payments through either native cryptocurrency or ERC20 tokens.
     * This function handles various checks and operations, such as referral code usage, enforcing minimum purchase amounts,
     ...
     *
     * Emits a transaction event and adjusts the state to record the tokens bought and any bonuses awarded.
     * Throws if the sender is not authorized, if the payment conditions are not met, or if the payment token is unsupported.
     *
     * Important:
     * - Reverts with `InvalidSenderAddressError` if an unauthorized address attempts to buy on behalf of another user.
     * - Checks if the funds provided (either as msg.value for native currency or from an ERC20 token) are sufficient.
     * - Reverts with `MinimumPurchaseError` if the calculated USD amount is below the set minimum purchase threshold.
     * - Handles bonuses through `_processBonuses` and adjusts the user's token purchase record.
     * - Supports first-time purchase recognition, marking a user's first completed transaction.
     */
    function buyTokens(
    ...
    /**
     * @dev Calculates the cost and tokens receivable based on either the amount to pay or tokens to buy.
     * This function allows for a flexible exchange calculation where the user can specify either
    ...
    function setVolumeBuyBonusEnabled(bool _enabled) external onlyOwner {
    ...
    function setPresaleState(PresaleState _presaleState) external onlyOwner {
        presaleState = _presaleState;
        emit PresaleStateChangedEvent(_presaleState);
    }

    function setClaimingActive(bool active) external onlyOwner withClaimingSet(active) {
    ...
    function setPaymentToken(address _paymentToken, bool enabled) external onlyOwner {
    ...
    function updateTokenDataFeed(address _token, address _dataFeed) external onlyOwner {
        tokenDataFeed[_token] = AggregatorV3Interface(_dataFeed);
        emit TokenDataFeedUpdatedEvent(_token, _dataFeed);
    }

    function emergencyWithdrawToken(uint256 amountToWithdraw) external onlyOwner {
        address user = msg.sender;
        claimableToken.safeTransfer(user, amountToWithdraw);
        emit TokensWithdrawnEvent(user, amountToWithdraw);
    }

    function emergencyWithdrawNativeCoin(address payable _to, uint256 amountToWithdraw) external onlyOwner {
    ...
    function updateReferralMerkleRoot(bytes32 _referralMerkleRoot) external onlyOwner {
        referralMerkleRoot = _referralMerkleRoot;
        emit ReferralMerkleRootUpdatedEvent(_referralMerkleRoot);
    }

    function updateClaimingMerkleRoot(bytes32 _claimingMerkleRoot) external onlyOwner {
        claimingMerkleRoot = _claimingMerkleRoot;
        emit ClaimingMerkleRootUpdatedEvent(_claimingMerkleRoot);
    }

    function setHotWalletAddress(address _hotWalletAddress) external onlyOwner {
        wertWallets[_hotWalletAddress] = true;
        emit HotWalletAddressSetEvent(_hotWalletAddress);
    }

    function setEstimationAddress(address _estimationAddress) external onlyOwner {
    ...
    function setMinimumPurchaseUsdAmount(uint256 _minimumPurchaseUsdAmount) external onlyOwner {
        minimumPurchaseUsdAmount = _minimumPurchaseUsdAmount;
        emit MinimumPurchaseUsdAmountSetEvent(_minimumPurchaseUsdAmount);
    }

    function setClaimableToken(address _token) external onlyOwner {
        claimableToken = IERC20(_token);
        emit ClaimableTokenSetEvent(_token);
    }

    function initialize(
        address _usdcStableCoin,
        address _usdtStableCoin,
        ...
        _initializePayment(_usdcStableCoin, _usdtStableCoin, _nativeCoinDataFeed);
    ...
    function _getExchangeRate(address paymentToken) private view returns (uint256 exchangeRateDecimals) {
        int256 exchangeRate;
        uint8 decimals;
        if (paymentToken != address(0)) {
            (, exchangeRate,,,) = tokenDataFeed[paymentToken].latestRoundData();
            decimals = tokenDataFeed[paymentToken].decimals();
            if (decimals == 0) {
                revert InvalidTokenDataFeedError(address(tokenDataFeed[paymentToken]));
            }
        } else {
            (, exchangeRate,,,) = nativeCoinDataFeed.latestRoundData();
    ...
    function _processReferralBonus(
        ...
        if (referralMerkleProof.length > 0) {
    ...
    function _initializePayment(
        ...
        isSupportedPaymentToken[_usdcStableCoin] = true;
        isSupportedPaymentToken[_usdtStableCoin] = true;
        usdcStableCoinAddress = _usdcStableCoin;
        usdtStableCoinAddress = _usdtStableCoin;
        nativeCoinDataFeed = AggregatorV3Interface(_nativeCoinDataFeed);
        emit PaymentInitializedEvent(_usdcStableCoin, _usdtStableCoin, _nativeCoinDataFeed);
    }
...
interface IPresale {
    ...
    event PaymentInitializedEvent(
        address usdcStableCoin,
        address usdtStableCoin,
    ...
    error InsufficientAllowanceError(address token, uint256 required, uint256 allowance);
