...
/**
 * @title ACLManager
 * @notice Access Controll List Manager. Main registry of system roles and permissions
 */
contract ACLManager is AccessControl, IACLManager {
    ...
    uint8 private _approverThreshold = 2;
    address private _core;

    constructor(address governor) {
        require(governor != address(0), "Governor cannot be zero");
        _setupRole(DEFAULT_ADMIN_ROLE, governor);
        _setupRole(OPERATOR_ROLE, governor);
        _setupRole(EMERGENCY_ADMIN_ROLE, governor);
        _setupRole(RISK_ADMIN_ROLE, governor);

        // grant originator role to deployer account
        // originator role only used for initializing contracts and must be renounced after that
        _setupRole(ORIGINATOR_ROLE, msg.sender);
    }
    ...
    function addEmergencyAdmin(address account) external {
        grantRole(EMERGENCY_ADMIN_ROLE, account);
    }

    function removeEmergencyAdmin(address account) external {
        revokeRole(EMERGENCY_ADMIN_ROLE, account);
    }

    function addRiskAdmin(address account) external {
        grantRole(RISK_ADMIN_ROLE, account);
    }

    function removeRiskAdmin(address account) external {
        revokeRole(RISK_ADMIN_ROLE, account);
    }

    function setCoreAddress(address account) external {
        requireOriginator(msg.sender);
        require(address(account) != address(0), "Core cannot be zero");
        _core = account;
    }

    /// @inheritdoc IACLManager
    function isCore(address account) public view override returns (bool) {
        return _core == account;
    }

    /// @inheritdoc IACLManager
    function requireCore(address account) public view override {
        require(_core == account, "Core only");
    }

    /// @inheritdoc IACLManager
    function isOriginator(address account) public view override returns (bool) {
        return hasRole(ORIGINATOR_ROLE, account);
    }

    /// @inheritdoc IACLManager
    function requireOriginator(address account) public view {
        require(hasRole(ORIGINATOR_ROLE, account), "Originator only");
    }
    ...
    /// @inheritdoc IACLManager
    function requireOperator(address account) public view {
        require(hasRole(OPERATOR_ROLE, account), "Operator only");
    }

    /// @inheritdoc IACLManager
    function isMigrator(address account) public view override returns (bool) {
        return hasRole(MIGRATOR_ROLE, account);
    }

    /// @inheritdoc IACLManager
    function requireMigrator(address account) public view {
    ...
    /// @inheritdoc IACLManager
    function isApprover(address account) public view override returns (bool) {
        return hasRole(APPROVER_ROLE, account);
    }

    /// @inheritdoc IACLManager
    function requireApprover(address account) public view {
        require(hasRole(APPROVER_ROLE, account), "Approver only");
    }
    ...
    /// @inheritdoc IACLManager
    function requireEmergencyAdmin(address account) public view {
        require(hasRole(EMERGENCY_ADMIN_ROLE, account), "Emergency admin only");
    }
    ...
    /// @inheritdoc IACLManager
    function requireRiskAdmin(address account) public view {
        require(hasRole(RISK_ADMIN_ROLE, account), "Risk admin only");
    }
    ...
    // @inheritdoc IACLManager
    function getRequiredApproverSignatures()
        public
        view
        override
        returns (uint8)
    {
        return _approverThreshold;
    }

    /// @inheritdoc IACLManager
    function setRequiredApproverSignatures(
        uint8 value
    ) public override onlyRole(DEFAULT_ADMIN_ROLE) {
        _approverThreshold = value;
    }
    ...
    function checkApproverSignatures(
        ...
        require(
            signatures.length >= (_approverThreshold * 65),
            "Invalid signature len"
        );
        bytes32 hash = ECDSA.toEthSignedMessageHash(dataHash);
        address[] memory signers = new address[](_approverThreshold);
        for (uint256 i = 0; i < _approverThreshold; i++) {
            signers[i] = ECDSA.recover(hash, signatures[i * 65:(i + 1) * 65]);
            requireApprover(signers[i]);
            for (uint256 j = 0; j < i; j++) {
                require(signers[j] != signers[i], "Dupplicate signer");
            }
        }
