...
// File contracts/Harnesses/Harness.sol
...
contract Harness is Control, ERC721Enumerable {
    using Strings for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    string public baseURI;
    uint256 public harnessesMinted;

    event HarnessMint(address to, uint256 tokenId);

    constructor() ERC721("Shogun War Harness", "Harness") {
      _tokenIdCounter._value = 1;
    }
    ...
    function burn(uint256 tokenId) external {
        require(msg.sender == ownerOf(tokenId), "You do not own this harness");
        _burn(tokenId);
    }
...
// File contracts/Harnesses/HarnessManager.sol
...
contract HarnessManager is Control {

    // contract addresses
    address public paymentToken;
    string constant public requiredClass = "Artisan";
    ...
    // configs
    uint256 public costPerHarness;
    uint256 public cooldownDuration;

    event HarnessCrafted(uint256 shogunId);

    function compareStrings(string memory a, string memory b) public pure returns (bool) {
      return (keccak256(abi.encodePacked((a))) == keccak256(abi.encodePacked((b))));
    }

    function craftHarness(uint256[] memory shogunIds) external {

        // take payment
        IERC20(paymentToken).transferFrom(_msgSender(), 0x5d75D6A019dEeAB32Cc2Af685900F4a96aF22852, costPerHarness * shogunIds.length);
        ...
        for(uint256 i = 0; i < shogunIds.length; i++) {
            ...
            // check class
            require(compareStrings(classes.checkShogunClass(shogunIds[i]), requiredClass), "Only Artisan can craft harnesses");

            // set cooldown
            // onlyOwner call
            cooldownManager.setShogunCooldown(shogunIds[i], block.timestamp + cooldownDuration);
            // mint talisman to user
            // onlyOwner call
            harness.safeMint(msg.sender);
            // emit event
            emit HarnessCrafted(shogunIds[i]);
        }
    }
    ...
    function setCostPerHarness(uint256 _cost) external onlyOwner {
        costPerHarness = _cost;
    }
    ...
    function setHarness(address _harness) external onlyOwner {
