...
interface IMDLP is IERC20{
    ...
    function zapRadiantForDLP(address _for, uint256 _amount) external payable returns(uint256);
...
import "../interfaces/radiant/IAngelMerkl.sol";
    ...
    IRadiantStaking
{
    ...
    event RewardQueuerUpdated(address rewarder, address rewardQueuer);
    ...
    error InvalidTokenForClaimReward();
    ...
    function claimARBMerkl( 
        address angleMerkleDistributor,
        uint256[] calldata _amount, 
        bytes32[][] calldata proofs, 
        address rewardTokensForClaim,
        address rewardTokensForWithdraw    
    ) external whenNotPaused nonReentrant _onlyAllowedOperators {

        if( angleMerkleDistributor == address(0)) revert InvalidAddress();
        if ( _amount.length != proofs.length ) revert InvalidLengths();
        if( rewardTokensForClaim !=  0x2dADe5b7df9DA3a7e1c9748d169Cd6dFf77e3d01 
        && rewardTokensForClaim != 0x3a2d44e354f2d88EF6DA7A5A4646fd70182A7F55) revert InvalidTokenForClaimReward();

        address[] memory reciever = new address[](1);
        address[] memory rArbTokenAddress = new address[](1);
        reciever[0] = address(this);
        rArbTokenAddress[0] = rewardTokensForClaim; // rArb Token address

        uint256 beforeRArbBalance = IERC20(rArbTokenAddress[0]).balanceOf(address(this));

        IAngelMerkl(angleMerkleDistributor).claim(
            reciever,
            rArbTokenAddress,
            _amount,
            proofs
        ...
        uint256 afterRArbBalance = IERC20(rArbTokenAddress[0]).balanceOf(address(this));
        
        //address is of ARB Token.
        ILendingPool(lendingPool).withdraw(rewardTokensForWithdraw, afterRArbBalance - beforeRArbBalance, msg.sender);
    }
    ...
    function withdrawDlp(uint256 _amount) external onlyOwner {
        if(_amount == 0) return;
        IERC20(rdntWethLp).safeTransfer(msg.sender, _amount);
    }
    ...
    function setDelegate(address _delegateContract, address _delegate) external onlyOwner {
        IDelegateRegistry(_delegateContract).setDelegate("radiantcapital.eth", _delegate);
    }    
    ...
    function updateRewardQueuers(address[] calldata _poolAssets, address rewardQueuer) external onlyOwner {

        for(uint256 index = 0; index < _poolAssets.length; index++){
            address _asset = _poolAssets[index];
            address rewarder = pools[_asset].rewarder;
            if(rewarder != address(0)){
                IBaseRewardPool(rewarder).updateRewardQueuer(rewardQueuer, true);
                emit RewardQueuerUpdated(rewarder, rewardQueuer);
            }
        }
    }
}
