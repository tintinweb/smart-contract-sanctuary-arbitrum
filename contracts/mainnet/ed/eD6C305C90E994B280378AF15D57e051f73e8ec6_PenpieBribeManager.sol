...
contract PenpieBribeManager is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function addBribeNativeToEpoch(uint256 _epoch, uint256 _pid) external payable nonReentrant whenNotPaused onlyOperator {
        if (_epoch < exactCurrentEpoch() - 1) revert InvalidEpoch();
        if (_pid >= pools.length || !pools[_pid]._active) revert InvalidPool();

        (uint256 fee, uint256 afterFee) = _addBribe(_epoch, _pid, NATIVE, msg.value);

        // transfer the token to the target directly in one time to save the gas fee
        bool success;
        if (fee > 0) {
            if (feeCollector == address(0)) {
                unCollectedFee[NATIVE] += fee;
            } else {
                feeCollector.transfer(fee);
            }
        }
        (success, )  = distributor.call{value: afterFee}("");
        
        if (!success) revert InvalidBribeToken();
    }

    function addBribeERC20ToEpoch(uint256 _epoch, uint256 _pid, address _token, uint256 _amount) external nonReentrant whenNotPaused onlyOperator {
        if (_epoch < exactCurrentEpoch() - 1) revert InvalidEpoch();
        if (_pid >= pools.length || !pools[_pid]._active) revert InvalidPool();
        if (!allowedToken[_token] && _token != NATIVE) revert InvalidBribeToken();

        (uint256 fee, uint256 afterFee) = _addBribe(_epoch, _pid, _token, _amount);

        // transfer the token to the target directly in one time to save the gas fee
        if (fee > 0) {
            if (feeCollector == address(0)) {
                unCollectedFee[_token] += fee;
                IERC20(_token).safeTransferFrom(msg.sender, address(this), fee);
            } else {
                IERC20(_token).safeTransferFrom(msg.sender, feeCollector, fee);
            }
        }
