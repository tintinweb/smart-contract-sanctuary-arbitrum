...
contract MasterPenpie is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    using SafeERC20 for IERC20;
    ...
    event Add( uint256 _allocPoint, address indexed _stakingToken, address indexed _receiptToken, IBaseRewardPool indexed _rewarder );
    event Set( address indexed _stakingToken, uint256 _allocPoint, IBaseRewardPool indexed _rewarder );
    ...
    event UpdatePool( address indexed _stakingToken, uint256 _lastRewardTimestamp, uint256 _lpSupply, uint256 _accPenpiePerShare );
    ...
    modifier _onlyVlPenpie() {
        if (msg.sender != address(vlPenpie))
            revert OnlyVlPenpie();
        _;
    }
    ...
    function depositFor(address _stakingToken, address _for, uint256 _amount) external whenNotPaused nonReentrant {
        PoolInfo storage pool = tokenToPoolInfo[_stakingToken];
    ...
    function updatePool(address _stakingToken) public whenNotPaused {
        ...
        uint256 penpieReward = (multiplier * penpiePerSec * pool.allocPoint) / totalAllocPoint;
        
        pool.accPenpiePerShare = pool.accPenpiePerShare + ((penpieReward * 1e12) / lpSupply);
        pool.lastRewardTimestamp = block.timestamp;
    ...
    function beforeReceiptTokenTransfer(address _from, address _to, uint256 _amount) external _onlyReceiptToken() {
        address _stakingToken = receiptToStakeToken[msg.sender];
        updatePool(_stakingToken);

        if (_from != address(0))
            _harvestRewards(_stakingToken, _from);

        if (_to != address(0))
            _harvestRewards(_stakingToken, _to);
    }

    function afterReceiptTokenTransfer(address _from, address _to, uint256 _amount) external _onlyReceiptToken() {
        address _stakingToken = receiptToStakeToken[msg.sender];
        PoolInfo storage pool = tokenToPoolInfo[_stakingToken];
    ...
    function depositVlPenpieFor(
        uint256 _amount,
        address _for
    ) external whenNotPaused _onlyVlPenpie() {
    ...
    function _deposit(address _stakingToken, address _from, address _for, uint256 _amount, bool _isLock) internal {
        PoolInfo storage pool = tokenToPoolInfo[_stakingToken];
    ...
    function _multiClaim(address[] calldata _stakingTokens, address _user, address _receiver, address[][] memory _rewardTokens) internal nonReentrant {
        ...
        for (uint256 i = 0; i < length; ++i) {
            ...
            updatePool(_stakingToken);
            uint256 claimablePenpie = _calNewPenpie(_stakingToken, _user) + user.unClaimedPenpie;
            ...
            user.rewardDebt = (user.amount * tokenToPoolInfo[_stakingToken].accPenpiePerShare) / 1e12;
    ...
    function _calPenpieReward(address _stakingToken, address _user) internal view returns(uint256 pendingPenpie) {
        PoolInfo storage pool = tokenToPoolInfo[_stakingToken];
        UserInfo storage user = userInfo[_stakingToken][_user];
        ...
        if (block.timestamp > pool.lastRewardTimestamp && pool.totalStaked != 0) {
            ...
            totalAllocPoint;
            accPenpiePerShare = accPenpiePerShare + (penpieReward * 1e12) / pool.totalStaked;
        }

        pendingPenpie = (user.amount * accPenpiePerShare) / 1e12 - user.rewardDebt;
    ...
    function _calNewPenpie(address _stakingToken, address _account) view internal returns(uint256) {
        UserInfo storage user = userInfo[_stakingToken][_account];
        uint256 pending = (user.amount * tokenToPoolInfo[_stakingToken].accPenpiePerShare) /
            1e12 -
            user.rewardDebt;
        return pending;
    }
    ...
    function _sendPenpieForVlPenpiePool(address _account, address _receiver, uint256 _amount) internal {
        address vlPenpieRewarder = tokenToPoolInfo[address(vlPenpie)].rewarder;
        penpieOFT.safeApprove(vlPenpieRewarder, _amount);
        IVLPenpieBaseRewarder(vlPenpieRewarder).queuePenpie(_amount, _account, _receiver);

        emit HarvestPenpie(_account, _receiver, _amount, false);
    }

    function _sendPenpie(address _account, address _receiver, uint256 _amount) internal {
        penpieOFT.safeTransfer(_receiver, _amount);

        emit HarvestPenpie(_account, _receiver, _amount, false);
    }

    function _addPool(uint256 _allocPoint, address _stakingToken, address _receiptToken, address _rewarder) internal {
        if (!Address.isContract(address(_stakingToken)) || !Address.isContract(address(_receiptToken)))
    ...
    function setPenpie(address _penpieOFT) external onlyOwner {
        if(address(penpieOFT) != address(0))
            revert PenpieOFTSetAlready();

        if (!Address.isContract(_penpieOFT))
    ...
    function setVlPenpie(address _vlPenpie)
        external
        onlyOwner
    {
    ...
    function createRewarder(address _receiptToken, address mainRewardToken)
        external
        _onlyPoolManager
        returns (address)
    {
    ...
    function add(uint256 _allocPoint, address _stakingToken, address _receiptToken, address _rewarder) external _onlyPoolManager {
    ...
    function createNoReceiptPool(uint256 _allocPoint, address _stakingToken, address _rewarder) external onlyOwner {
        _addPool(_allocPoint, _stakingToken, _stakingToken, _rewarder);
    }

    function createPool(uint256 _allocPoint, address _stakingToken, string memory _receiptName, string memory _receiptSymbol) external onlyOwner {
        ...
        address rewarder = this.createRewarder(
            address(newToken),
            address(0)
        );
    ...
    function updatePoolsAlloc(address[] calldata _stakingTokens, uint256[] calldata _allocPoints) external _onlyWhiteListed() {
        massUpdatePools();
    ...
    function updateRewarderQueuer(address _rewarder, address _manager, bool _allowed) external onlyOwner {
