...
/**
 *Submitted for verification at Arbiscan on 2022-09-21
*/
...
contract FeeRecipient3 is Initializable, OwnableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IERC20Upgradeable public wNative;
    IERC20Upgradeable public tsu;
    ...
    address[] public wNativeToTsuRoute;
    address[] public wNativeToStableRoute;

    bool public splitTreasury;
    bool public sendHarvesterGas;

    event Harvest(uint256 totalHarvested, uint256 stablesHarvested, uint256 tsuHarvested, uint256 nativeNotified, uint256 time);
    ...
    event NewTsuRoute(address[] oldRoute, address[] newRoute);
    ...
    function initialize(
        ...
        wNativeToTsuRoute = _tsuRoute;
        wNativeToStableRoute = _stableRoute;
    }

    // Main function. Divides Kintsugi's profits.
    function harvest() public {
        ...
        if (splitTreasury) {
            ...
            tsuAmt = nativeAvailable - stableAmt;
            IUniswapRouter(unirouter).swapExactTokensForTokens(stableAmt, 0, wNativeToStableRoute, treasury, block.timestamp);
            IUniswapRouter(unirouter).swapExactTokensForTokens(tsuAmt, 0, wNativeToTsuRoute, treasury, block.timestamp);
        } else {
        ...
        emit Harvest(wNativeBal, stableAmt, tsuAmt, rewardPoolAmount, block.timestamp);
    }
    ...
    function setRoute(address[] calldata _route, bool _stableRoute) external onlyOwner {
        ...
        if(_stableRoute) {
            ...
            require(_route[_route.length - 1] == address(tsu), "!tsu");

            emit NewTsuRoute(wNativeToTsuRoute, _route);
            wNativeToTsuRoute = _route;
        }
    }
