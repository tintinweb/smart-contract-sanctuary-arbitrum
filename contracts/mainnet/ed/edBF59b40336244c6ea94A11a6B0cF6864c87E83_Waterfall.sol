...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    uint256 constant MAXSUPPLY = 80000 ether;
...
contract Waterfall is BEP20 {
    ...
    // Fee address
    address payable public constant feeAddr = 0x0568E084Dcb739EbAe4d1b547b84DFDEeC06ebbb;
    ...
    // Min amount to liquify. (default 10 Waterfalls)
    ...
    // The operator can call all functions except of minting Waterfall
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "WATERFALL::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the WaterfallSwapToken contract.
     */
    constructor() public BEP20("Waterfall", "WATERFALL") {
    ...
    /// @dev overrides transfer function to meet tokenomics of WATERFALL
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        transferTaxRate == 0 ) {
            ...
            require(taxAmount == burnAmount + liqAmount, "WATERFALL::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "WATERFALL::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the WATERFALL -> weth path for swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "WATERFALL::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnFee(uint16 _burnFee) public onlyOperator {
        require(_burnFee <= 100, "WATERFALL::updateBurnRate: Burn Fee must not exceed the maximum rate.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "WATERFALL::updateSwapRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "WATERFALL::transferOperator: new operator is the zero address");
...
// will be transferred to a governance smart contract once WATERFALL is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of WATERFALL
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accWaterfallPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accWaterfallPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. WATERFALLs to distribute per second.
        uint256 lastRewardSecond; // Last second that WATERFALLs distribution occurs.
        uint256 accWaterfallPerShare; // Accumulated WATERFALLs per share, times 1e18. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The WATERFALL TOKEN!
    Waterfall public waterfall;
    // Dev address.
    address public devaddr;
    // WATERFALL tokens created per second.
    ...
    // The block timestamp when Waterfall mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 WaterfallPerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    // Update the given pool's Waterfall allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // View function to see pending waterfall on frontend.
    function pendingWaterfall(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 waterfallReward = multiplier
                .mul(WaterfallPerSecond)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
                uint256 devReward = waterfallReward.div(10);
                uint256 totalRewards = waterfall.totalSupply().add(devReward).add(
                waterfallReward
            );
                if (totalRewards > waterfall.maxSupply()) {
                waterfallReward = waterfall.maxSupply().sub(waterfall.totalSupply());
            }
            accWaterfallPerShare = accWaterfallPerShare.add(
                waterfallReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return user.amount.mul(accWaterfallPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= waterfall.maxSupply()) {
            ...
            waterfall.mint(devaddr, waterfallReward.div(10));
            waterfall.mint(address(this), waterfallReward);
        } else {
            // mint the difference only to MC, update waterfallReward
            waterfallReward = waterfall.maxSupply().sub(waterfall.totalSupply());
            waterfall.mint(address(this), waterfallReward);
        }

        if (waterfallReward != 0) {
            // only calculate and update if waterfallReward is non 0
            pool.accWaterfallPerShare = pool.accWaterfallPerShare.add(
                waterfallReward.mul(1e18).div(pool.lpSupply)
            );
        }
    ...
    // Deposit LP tokens to MasterChef for Waterfall allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                .amount
                .mul(pool.accWaterfallPerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (pending > 0) {
                safeWaterfallTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accWaterfallPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeWaterfallTransfer(msg.sender, pending);
    ...
    // Safe Waterfall transfer function, just in case if rounding error causes pool to not have enough Waterfalls.
    ...
    function updateEmissionRate(uint256 _WaterfallPerSecond) external onlyOwner {
        require(_WaterfallPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        WaterfallPerSecond = _WaterfallPerSecond;
