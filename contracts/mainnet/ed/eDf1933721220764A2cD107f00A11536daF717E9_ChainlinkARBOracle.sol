...
contract ChainlinkARBOracle is AccessManaged, Pausable {
  ...
  // Mapping of LSRTs (Liquid Staking/Restaking Tokens)
  mapping(address => bool) public LSRTs;
  // Mapping of LSRTs ratio to denominator token
  mapping(address => address) public LSRTToDT;
  ...
  */
  ...
  /**
    ...
    * @dev Check if token is an LSRT; if so, we multiply the ratio WNT price
    * @param token Token address
  ...
  */
  function consultIn18Decimals(address token) external view whenNotPaused returns (uint256) {
    if (LSRTs[token]) {
      // Get LSRT ratio
      (int256 _lsrtAnswer, uint8 _lsrtDecimals) = consult(token);

      // Get token denominator price
      (int256 _tdAnswer, uint8 _tdDecimals) = consult(LSRTToDT[token]);

      return (
        (_lsrtAnswer.toUint256() * _lsrtDecimals / (10 ** _lsrtDecimals))
        * (_tdAnswer.toUint256() * _tdDecimals / (10 ** _tdDecimals))
        / SAFE_MULTIPLIER
      );
    } else {
  ...
  */
  ...
  /**
    * @notice Update token to LSRT mapping
    * @param token LSRT address
    * @param isLSRT Whether token is LSRT or not
  */
  function updateLSRT(address token, bool isLSRT) external restricted {
    if (token == address(0)) revert Errors.ZeroAddressNotAllowed();
  ...
  /**
    * @notice Update LSRT to denominator token
    * @param lsrt LSRT address
    * @param dt Denominator token address
  */
  function updateLSRTsRatioFeed(address lsrt, address dt) external restricted {
    if (lsrt == address(0)) revert Errors.ZeroAddressNotAllowed();
