...
abstract contract USDEngineAbstract is ReentrancyGuard, Ownable, IUSDEngine {
    ...
    function getBurnableUSD(address tokenCollateralAddress, address user, uint256 amountUSD) public view virtual returns (uint256, bool) {}
    ...
    function _burnUsd(uint256 amountUsdToBurn, address onBehalfOf, address usdFrom, address collateral) internal {
    ...
    function _getAccountInformation(
        address user,
        address collateral
    ) internal view virtual returns (uint256 totalUsdMinted, uint256 collateralValueInUsd, bool isLatestPrice) {}
    ...
    function _getUsdValue(address token, uint256 amount) internal view returns (uint256, bool) {
        uint256 usdValue;
        bool isLatestPrice;
        if (s_priceFeeds[token] == WSTETHPriceFeed) {
            (uint256 wstToETHPrice, bool isLatestPrice1) = validatePriceFeedAndReturnValue(WSTETHPriceFeed);
            (uint256 ethToUSDPrice, bool isLatestPrice2) = validatePriceFeedAndReturnValue(ETHPriceFeed);
            isLatestPrice = isLatestPrice1 && isLatestPrice2;
    ...
    function validatePriceFeedAndReturnValue(address _priceFeed) internal view returns (uint256, bool) {
    ...
    function getAccountInformation(
        address user,
        address collateral
    ) external view returns (uint256 totalUsdMinted, uint256 collateralValueInUsd, bool isLatestPrice) {
    ...
    function getAccountCollateralValue(address user, address collateral) public view virtual returns (uint256, bool) {}

    function getTokenAmountFromUsd(address token, uint256 usdAmountInWei) public view virtual returns (uint256, bool) {}
