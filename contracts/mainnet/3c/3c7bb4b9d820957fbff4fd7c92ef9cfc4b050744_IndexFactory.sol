...
/// @title IndexFactory
/// @notice Contract for deploying Index contracts
contract IndexFactory is Owned(msg.sender), IIndexFactory {
    /// @notice Deploys a new Index contract
    ///
    /// @param _name The name of the Index
    /// @param _symbol The symbol of the Index
    /// @param _reserve The reserve currency
    /// @param _governance The address of the Governance
    /// @param _messenger The address of the Messenger contract
    /// @param _maxDepositFeeInBPs The maximum deposit fee in basis points
    /// @param _maxRedemptionFeeInBPs The maximum redemption fee in basis points
    /// @param _maxAUMDilutionPerSecond The maximum AUM dilution per second
    ///
    /// @return index The address of the deployed Index contract
    function deploy(
        string memory _name,
        ...
        emit Deployed(index, _name, _symbol, _reserve, _governance);
    }
}
...
/// @title IIndexFactory
/// @dev Interface for the IndexFactory contract.
interface IIndexFactory {
    /// @dev Emitted when a new index is deployed.
    ///
    /// @param index The address of the deployed Index.
    /// @param name The name of the index.
    /// @param symbol The symbol of the index.
    /// @param reserve The reserve currency of the index.
    /// @param governance The governance of the index
    event Deployed(address index, string name, string symbol, Currency reserve, address governance);
}
...
import {IndexLib, FixedPointMathLib} from "./libraries/IndexLib.sol";
...
contract Index is Vault, IIndex, IERC20Metadata, IERC20Permit {
    ...
    /// @dev Initial chain ID when the contract was deployed
    uint256 private immutable INITIAL_CHAIN_ID;

    /// @dev Initial domain separator when the contract was deployed
    ...
    /// @param forwardedSender The address of the sender forwarded by the messenger contract
        ...
        returns (RedemptionInfo memory result)
    {
        ...
        if (msg.sender != params.config.forwarder) revert InvalidSender();
        ...
        for (uint256 i; i < params.config.homeCurrencies.length; ++i) {
            (Currency currency, uint96 value) = params.config.homeCurrencies[i].unpack();
    ...
    ///
    /// @dev sanitization is done externally. Ensure that home assets include reserve,
    ...
    ///
    /// @param withdrawals The currency withdrawals to process
...
library IndexLib {
    ...
    ///
    /// @return totalAssets The total assets of the index
    ...
    function totalAssets(IIndex.Config calldata config, bytes calldata payload, uint256 reserveBalance, uint256 kSelf)
        internal
        returns (uint256 totalAssets, uint256 reserveValuation, uint256 reservePrice)
    {
...
interface IIndexMetadata is IFeeQuoter {}
