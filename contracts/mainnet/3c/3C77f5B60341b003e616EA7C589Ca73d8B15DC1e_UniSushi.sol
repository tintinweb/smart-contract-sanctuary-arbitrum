...
contract UniSushi is Ownable {
    ...
    function u2s(address _base, address _counter, uint256 _amountToSwap, uint _threshold, uint24 _fee) external {
        // balance validation
    ...
    function s2u(address _base, address _counter, uint256 _amountToSwap, uint _threshold, uint24 _fee) external {
        // balance validation
        uint balance = IERC20(_counter).balanceOf(address(this));
        require(balance > _amountToSwap, "no execution: insufficent balance");
        ...
        // buy from sushiswap
        TransferHelper.safeApprove(_counter, address(SUSHISWAP_ROUTER_ADDRESS), _amountToSwap);
        
        address[] memory path = new address[](2);
        path[0] = _counter;
        path[1] = _base;

        uint256[] memory sushiResponse = sushiswapRouter.swapExactTokensForTokens(_amountToSwap, 0, path, address(this), block.timestamp);
        ...
        uint256 lastAmountWeHave = uniswapRouter.exactInputSingle(params);

        emit SwapExecuted(_base, _counter, lastAmountWeHave);
