...
contract GnomesNFT is ISJ741, NonblockingLzApp {
    /// @dev Token name
    ...
    bool public bridgeHasFee = false;
    bool public bridgeStats = false;
    bool public sleepRequirement = false;
    ...
    function setBridgeHasFee(bool _bridgeHasFee) public onlyDev {
        bridgeHasFee = _bridgeHasFee;
    }

    function setSleepRequirement(bool _sleepRequirement) public onlyDev {
        sleepRequirement = _sleepRequirement;
    }

    function setBridgeFee(uint256 _bridgeFee) public onlyDev {
        bridgeFee = _bridgeFee;
    }

    function setBridgeStats(bool _bridgeStats) public onlyDev {
        bridgeStats = _bridgeStats;
    }
    ...
    function _mintGnomes(address to, bool burnGnomes) internal virtual returns (uint32 tokenId) {
        ...
        libSJ721.emitTransfer(address(0), to, tokenId); // Emit an event for the token transfer
        if (burnGnomes && bridgeHasFee) {
            libSJ20.emitTransfer(address(0), to, (ONE * (100000 - bridgeFee)) / 100000);
            _totalSupply += (ONE * (100000 - bridgeFee)) / 100000;
            _balanceOf[to] += (ONE * (100000 - bridgeFee)) / 100000;
        } else {
    ...
    function _nonblockingLzReceive(uint16, bytes memory, uint64, bytes memory _payload) internal override {
        if (bridgeStats) {
            ...
            uint256 tokenId = _mintGnomes(toAddress, true);

            treasuryMintTimeStamp[toAddress] = usr_treasuryMintTimeStamp;

            gnomeGamePlay.setGnomeStats(
                tokenId,
                gnomeX,
                usr_xp,
                usr_hp,
    ...
    function bridgeGnome(string memory _chainTo, uint32 _tokenId) public payable {
        if (sleepRequirement) {
            require(
                block.timestamp > gnomeGamePlay.getMeditateTimeStamp(_tokenId),
                "Meditating Gnomes Can't be Bridged"
            );
            require(!gnomeGamePlay.getIsSleeping(_tokenId), "Sleeping Gnomes Can't be Bridged");
        }
        ...
        if (bridgeStats) {
            ...
            gnomeGamePlay.deleteGameStats(_tokenId);
            payload = abi.encode(msg.sender, _tokenId);
            _burn(msg.sender, _tokenId);
        }
