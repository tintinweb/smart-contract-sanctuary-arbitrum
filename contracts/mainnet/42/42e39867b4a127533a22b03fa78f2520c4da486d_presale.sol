...
contract presale is Ownable {
    ...
    function SetMinimumPresaleAllocation(uint256 _MinimumPresaleAllocation)
        external
        onlyOwner
    {
    ...
    function SetMaximumPresaleAllocation(uint256 _MaximumPresaleAllocation)
        external
        onlyOwner
    {
    ...
    function SetTotalPresaleAllocation(uint256 _TotalPresaleAllocation)
        external
        onlyOwner
    {
    ...
    function depositETH() external payable {
        ...
        require(msg.value >= MinimumPresaleAllocation, "Deposit is too low.");

        require(
            msg.value + addressDeposit[msg.sender] <= MaximumPresaleAllocation,
            "Deposit is too high."
        );

        require(
            msg.value + presaleTotal <= TotalPresaleAllocation,
