...
contract DefiEdgeTwapStrategy is UniswapV3TwapLiquidityManager {
    ...
    function airdropWithdraw(address _token, address _to) external {
        ...
        TransferHelper.safeTransfer(_token, _to, IERC20(_token).balanceOf(address(this)));
    }
}
...
contract UniswapV3TwapLiquidityManager is TwapStrategyBase, ReentrancyGuard, IUniswapV3MintCallback {
     ...
     */
    function burnLiquiditySingle(uint256 _tickIndex, uint256 _amount0Min, uint256 _amount1Min)
        public
        nonReentrant
        returns (
            uint256 amount0,
     ...
     * @param _amount1Min Minimum amount of token1 to be burned
     */
    function _burnLiquiditySingle(uint256 _tickIndex, uint256 _amount0Min, uint256 _amount1Min)
        internal
        returns (
            uint256 amount0,
            uint256 amount1,
    ...
    function _swap(bool zeroToOne, uint256 amountIn, bool isOneInchSwap, bytes calldata data) internal {
        ...
        IERC20 srcToken = zeroToOne ? token0 : token1;
        IERC20 dstToken = zeroToOne ? token1 : token0;

        srcToken.safeIncreaseAllowance(swapProxy, amountIn);
        ...
        srcToken.safeApprove(swapProxy, 0);
    ...
    function isInvalidTicks(Tick[] memory _ticks) internal pure returns (bool invalid) {
        for (uint256 i = 0; i < _ticks.length; i++) {
            ...
            for (uint256 j = 0; j < i; j++) {
                if ( _ticks[i].tickLower == _ticks[j].tickLower) {
                    if (_ticks[i].tickUpper == _ticks[j].tickUpper) {
                        invalid = true;
                        return invalid;
                    }
                }
