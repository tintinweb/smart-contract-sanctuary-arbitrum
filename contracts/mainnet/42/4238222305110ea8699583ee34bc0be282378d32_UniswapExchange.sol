...
/**
 * @dev The contract acting as an exchange proxy for any DEX implemented using UniswapExchange (V02) API
 */

contract UniswapExchange is IBrightPoolExchange {
    using SafeERC20 for IERC20;

    uint256 internal constant Q96 = 0x1000000000000000000000000;
    uint256 internal constant FEE_BASE = 1_000_000;
    uint256 internal constant PLATFORM_FEE = 1000;
    IUniswapV3Factory immutable factory;
    IUniswapV3Router immutable router;
    IWETH immutable weth;
    uint24 immutable fee;

    constructor(IUniswapV3Factory factory_, IUniswapV3Router router_, IWETH weth_, uint24 predictedFee_) {
        factory = factory_;
        router = router_;
        weth = weth_;
        fee = predictedFee_;
    ...
    function amountOut(IERC20 askToken_, IBrightPoolLedger.Asset calldata asset_) public view returns (uint256) {
        (address tok0, address tok1) = getAddresses(address(askToken_), address(asset_.token));
        IUniswapV3Pool pool = IUniswapV3Pool(factory.getPool(tok0, tok1, fee));

        // Pool might not exist
        if (address(pool) == address(0)) return 0;
        if (pool.token0() == address(0)) return 0;

        (uint160 sqrtPriceX96,,,,,,) = pool.slot0();

        uint256 amount0 = _mulDiv(pool.liquidity(), Q96, sqrtPriceX96);
        uint256 amount1 = _mulDiv(pool.liquidity(), sqrtPriceX96, Q96);

        uint256 out = 0;
        if (tok0 == pool.token0()) {
            out = (amount0 * asset_.amount) / amount1;
        } else {
            out = (amount1 * asset_.amount) / amount0;
        }
        if (out > IERC20(tok0).balanceOf(address(pool))) {
            return 0;
        } else {
            uint256 calculatedFee = out * (fee + PLATFORM_FEE) / FEE_BASE;
            return out - calculatedFee;
        }
    }

    function exchange(IBrightPoolConsumer consumer_, IBrightPoolLedger.Order calldata order_) public payable {
        if (address(order_.ask.token) == address(0)) {
            _exchangeTokenForETH(consumer_, order_);
        } else if (address(order_.bid.token) == address(0)) {
            _exchangeETHForToken(consumer_, order_);
        } else {
            _exchangeTokenForToken(consumer_, order_);
        }
    }

    function _exchangeTokenForToken(IBrightPoolConsumer consumer_, IBrightPoolLedger.Order calldata order_) private {
        order_.bid.token.safeTransferFrom(msg.sender, address(this), order_.bid.amount);
        order_.bid.token.safeIncreaseAllowance(address(router), order_.bid.amount);
        IUniswapV3Router.ExactInputSingleParams memory params;
        params.tokenIn = address(order_.bid.token);
        params.tokenOut = address(order_.ask.token);
        params.fee = fee;
        params.recipient = address(this);
        params.deadline = block.timestamp;
        params.amountIn = order_.bid.amount;
        params.amountOutMinimum = order_.ask.amount;
        params.sqrtPriceLimitX96 = 0;

        // slither-disable-next-line unused-return
        router.exactInputSingle(params);

        order_.ask.token.safeTransfer(order_.owner, order_.ask.amount);
        if (order_.ask.token.balanceOf(address(this)) > 0) {
            order_.ask.token.safeIncreaseAllowance(address(consumer_), order_.ask.token.balanceOf(address(this)));
            consumer_.consume(IBrightPoolLedger.Asset(order_.ask.token, order_.ask.token.balanceOf(address(this))));
        }
    }

    function _exchangeETHForToken(IBrightPoolConsumer consumer_, IBrightPoolLedger.Order calldata order_) private {
        // V3 requires to "change" eth to WETH9, so
        ...
        IERC20(weth).safeIncreaseAllowance(address(router), msg.value);
    ...
    function _exchangeTokenForETH(IBrightPoolConsumer consumer_, IBrightPoolLedger.Order memory order_) private {
        order_.bid.token.safeTransferFrom(msg.sender, address(this), order_.bid.amount);
        order_.bid.token.safeIncreaseAllowance(address(router), order_.bid.amount);
        // slither-disable-next-line unused-return
        IUniswapV3Router.ExactInputSingleParams memory params;
        ...
        (bool sentUser,) = payable(order_.owner).call{value: order_.ask.amount}("");
        if (!sentUser) revert InsufficientFunds();
        if (address(this).balance > 0) {
            // slither-disable-next-line arbitrary-send-eth
            consumer_.consume{value: address(this).balance}(
                IBrightPoolLedger.Asset(IERC20(address(0)), address(this).balance)
            );
        }
    }

    function getAddresses(address token0, address token1) private view returns (address, address) {
        if (address(token0) == address(0)) {
            return (address(weth), token1);
        } else if (address(token1) == address(0)) {
            return (token0, address(weth));
        } else {
