...
contract MemeCoin is ERC20, Ownable{
  ...
  uint256 constant _totalSupply = 30000 ether;
...
address constant deadAddress = address(0xdEaD);
  function burnLiquidity(uint256 percent) external onlyOwner returns (bool) {
    ...
    _update(address(mainPair), deadAddress, amountToBurn);
  ...
  function enableTradingActive(bool enable) external onlyOwner{
  ...
  function setFeeExcludedList(address account, bool enable) external onlyOwner{
  ...
  function setExcludedMaxAmountList(address account, bool enable) external onlyOwner{
  ...
  function enableLimitTradeFreq(bool enable) external onlyOwner {
