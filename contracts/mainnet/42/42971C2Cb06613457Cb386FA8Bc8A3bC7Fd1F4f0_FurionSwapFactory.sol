...
contract FurionSwapFactory is IFurionSwapFactory {
    ...
    constructor(address _incomeMaker, address _weth) {
        _owner = msg.sender;
    ...
    function createPair(
        ...
        bytes memory bytecode = abi.encodePacked(type(FurionSwapPair).creationCode, abi.encode(WETH));
...
import {IFurionSwapV2Callee} from "./interfaces/IFurionSwapV2Callee.sol";
import {IFurionTWAP} from "../oracles/interfaces/IFurionTWAP.sol";
    ...
    ReentrancyGuard
{
    ...
    function swap(
        ...
        // Update TWAP if is WETH pair, and TWAP contract is set
        {
            address twap = IFurionSwapFactory(factory).twap();
            if (twap != address(0)) {
                if (token0 == WETH) {
                    IFurionTWAP(twap).update(token1);
                } else if (token1 == WETH) {
                    IFurionTWAP(twap).update(token0);
                }
            }
        }
