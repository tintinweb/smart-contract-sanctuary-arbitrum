...
error SetParamsBorrowArraysLengthMismatch();
error SetParamsFundingArraysLengthMismatch();
    ...
    AccessControlRolesUpgradeable
{
    ...
    event SetPoolParamEffectBorrow(
        uint256 indexed poolId,
        string paramId,
        bytes32 paramValue,
        int256 sumFractionLong,
        int256 sumFractionShort,
        uint64 timestamp
    );

    event SetPoolParamEffectFunding(
        uint256 indexed poolId,
        string paramId,
        bytes32 paramValue,
        int256 sumFractionLong,
        int256 sumFractionShort,
        int256 lpFundingRateNotional,
        uint64 timestamp
    );
    ...
    /**
     * @dev Updates a pool param that is not effectful.
     *      Effectful pool params have their own designated functions.
     */
    function setParam(SetParamRequest calldata request)
        public
        onlyOperator
        onlyPoolOwner(
            request.poolId,
            request.poolUser,
            abi.encodePacked(request.paramId, request.paramValue),
            request.signature
        )
    {
        emit SetPoolParam(request.poolId, request.paramId, request.paramValue);
    }

    /**
     * @dev Updates a pool param that has a borrow fee charge effect.
     */
    function setParamWithBorrowEffect(
        SetParamRequest calldata request,
        SetParamEffectBorrow calldata effect
    )
        public
        onlyOperator
        onlyPoolOwner(
    ...
    {
        emit SetPoolParamEffectBorrow(
            request.poolId,
            request.paramId,
            request.paramValue,
            effect.sumFractionLong,
            effect.sumFractionShort,
            effect.timestamp
        );
    }

    /**
     * @dev Updates a pool param that has a funding fee charge effect.
     */
    function setParamWithFundingEffect(
        SetParamRequest calldata request,
        SetParamEffectFunding calldata effect
    )
        public
        onlyOperator
        onlyPoolOwner(
    ...
    {
        emit SetPoolParamEffectFunding(
            request.poolId,
            request.paramId,
            request.paramValue,
            effect.sumFractionLong,
            effect.sumFractionShort,
            effect.lpFundingRateNotional,
            effect.timestamp
        );
    }

    /**
     * @dev Updates multiple pool params in a single function call.
     */
    function setParams(SetParamsRequest calldata request)
        external
        onlyOperator
    {
        if (request.borrow.length != request.borrowEffects.length) {
            revert SetParamsBorrowArraysLengthMismatch();
        }
        if (request.funding.length != request.fundingEffects.length) {
            revert SetParamsFundingArraysLengthMismatch();
        }
        uint256 i;
        for (i = 0; i < request.nonEffectful.length; i++) {
            setParam(request.nonEffectful[i]);
        }
        for (i = 0; i < request.borrow.length; i++) {
            setParamWithBorrowEffect(
                request.borrow[i],
                request.borrowEffects[i]
            );
        }
        for (i = 0; i < request.funding.length; i++) {
            setParamWithFundingEffect(
                request.funding[i],
                request.fundingEffects[i]
            );
        }
    }
...
struct SetParamEffectBorrow {
    int256 sumFractionLong;
    int256 sumFractionShort;
    uint64 timestamp;
}

struct SetParamEffectFunding {
    int256 sumFractionLong;
    int256 sumFractionShort;
    int256 lpFundingRateNotional;
    uint64 timestamp;
...
struct SetParamsRequest {
    SetParamRequest[] nonEffectful;
    SetParamRequest[] borrow;
    SetParamRequest[] funding;
    SetParamEffectBorrow[] borrowEffects;
    SetParamEffectFunding[] fundingEffects;
}
