...
contract RewardRouterV2 is IRewardRouterV2, ReentrancyGuard, Governable {
    ...
    function initialize(
        ...
        // address _xlpManager,
        address _lexVester // address _xlpVester
    ) external onlyGov {
        weth = _weth;
    ...
    function handleRewards(
        ...
        uint256 lexAmount = 0;
        if (_shouldClaimLex) {
            lexAmount = IVester(lexVester).claimForAccount(account, account);
        ...
        if (_shouldClaimEsLex) {
            esLexAmount = IRewardTracker(stakedLexTracker).claimForAccount(account, account);
        ...
        if (_shouldClaimWeth) {
            if (_shouldConvertWethToEth) {
                uint256 wethAmount = IRewardTracker(feeLexTracker).claimForAccount(
                    account,
                    address(this)
                );
