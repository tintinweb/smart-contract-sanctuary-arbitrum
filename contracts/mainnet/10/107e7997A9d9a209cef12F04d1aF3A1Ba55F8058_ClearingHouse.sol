...
contract ClearingHouse is TransferHelper, OwnerPausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function partialClose(
        ...
        Side side = position.size.toInt() > 0 ? Side.BUY : Side.SELL;
        ...
        updateOpenInterestNotional(
            _amm,
            positionResp.realizedPnl.addD(positionResp.exchangedQuoteAssetAmount).mulScalar(-1),
            side
        );
    }
    ...
    function _internalPartialClose(
        IAmm _amm,
        address _trader,
        SignedDecimal.signedDecimal memory _sizeToClose,
        Decimal.decimal memory _quoteAssetAmountLimit
        ...
        positionResp.realizedPnl = unrealizedPnl.mulD(_sizeToClose).divD(oldPosition.size);
        ...
        (
            ...
            SignedDecimal.signedDecimal memory lastUpdatedCumulativePremiumFraction
                ...
                _amm,
                oldPosition,
                positionResp.realizedPnl.mulScalar(-1)
            );

        positionResp.exchangedPositionSize = _sizeToClose.mulScalar(-1);
        ...
        SignedDecimal.signedDecimal memory newSize = oldPosition.size.subD(_sizeToClose);
        SignedDecimal.signedDecimal memory remainOpenNotional = oldPosition.size.toInt() > 0
            ? MixedDecimal
                ...
                .subD(positionResp.realizedPnl)
            : positionResp.realizedPnl.addD(oldPositionNotional).subD(
                positionResp.exchangedQuoteAssetAmount
            );
