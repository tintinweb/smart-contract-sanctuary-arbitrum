...
contract SWISSSale is Ownable {

    using SafeERC20 for ERC20;
    using Address for address;

    uint constant DAIdecimals = 10 ** 18;
    uint constant SWISSdecimals = 10 ** 9;
    uint public constant MAX_SOLD = 50000 * SWISSdecimals;
    uint public constant PRICE = 10 * DAIdecimals / SWISSdecimals ;
    uint public constant MAX_SALE_PER_ACCOUNT = 70 * SWISSdecimals;
    uint public constant MAX_PRESALE_PER_ACCOUNT = 99 * SWISSdecimals + 9 * SWISSdecimals / 10;

    uint public sold;
    uint owed;
    address public SWISS;
    ...
    function buySWISS(uint256 amount) public onlyEOA {
    ...
    function claimSWISS() public onlyEOA {
      ERC20(SWISS).transfer(msg.sender, invested[msg.sender]);
      owed -= invested[msg.sender];
      invested[msg.sender] = 0;

    }
    function setClaimingActive(address swiss) public {
