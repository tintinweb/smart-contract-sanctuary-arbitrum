...
contract PlatformModuleDiamondInit is AccessControl {
    function init(IPlatformModule.Platform[] memory inputPlatforms) external {
        LibPlatformModuleStorage.DiamondStorage storage ds = LibPlatformModuleStorage.diamondStorage();

        for (uint256 i; i < inputPlatforms.length; i++) {
            bytes32 platformId = inputPlatforms[i].id;
            ds.platformById[platformId].name = inputPlatforms[i].name;
            ds.platformById[platformId].id = inputPlatforms[i].id;
            ds.platformById[platformId].owner = inputPlatforms[i].owner;
            ds.platformById[platformId].treasury = inputPlatforms[i].treasury;
            ds.platformById[platformId].referrerBasisPoints = inputPlatforms[i].referrerBasisPoints;
            ds.platformById[platformId].burnBasisPoints = inputPlatforms[i].burnBasisPoints;

            for (uint256 j = 0; j < inputPlatforms[i].services.length; j++) {
                ds.platformById[platformId].services.push(inputPlatforms[i].services[j]);
            }

            ds.platformIds.push(platformId);
        }

        _grantRole(AccessControlStorage.DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(LibPaymentModuleConsts.PLATFORM_MANAGER_ROLE, msg.sender);
    }
}