...
import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.4/contracts/utils/Strings.sol";
...
contract TCGInventory is ERC721Enumerable, AccessControl, ReentrancyGuard {
    ...
    function updateCardGameInformation(uint256 addWin, uint256 addPlayed, uint256 tokenId) public onlyRole(CONTRACT_ROLE) {
        require(_isApprovedOrOwner(msg.sender, tokenId) && _exists(tokenId), "Sender not approved or token doesn't exist.");
    ...
    function burn(uint256 tokenId) external {
        require(_isApprovedOrOwner(msg.sender, tokenId) && _exists(tokenId), "Sender not approved or owner or token doesn't exist.");
        _burn(tokenId);
    }

    function updateCardData(uint256 tokenId, uint8 top, uint8 left, uint8 right, uint8 bottom) public onlyRole(CONTRACT_ROLE) {
        require(_isApprovedOrOwner(msg.sender, tokenId) && _exists(tokenId), "Sender not approved or owner or token doesn't exist.");
        Card storage c = cardData[tokenId];
        c.top = top;
        c.left = left;
    ...
    function dataReturn(uint256 tokenId) public view returns(
        uint8 level, 
        uint8 top, 
        uint8 left, 
        uint8 right, 
        ...
        onlyRole(ADMIN_ROLE) 
    {
        ...
        templateLength++;
        template[templateLength] = Data(imageURL, name, description, "", level, top, left, right, bottom, levelSlots[level]);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns(string memory) {
        (uint8 level, uint8 top, uint8 left, uint8 right, uint8 bottom, uint256 winCount, uint256 playedCount, uint8 slot) = dataReturn(tokenId);
        ...
        string memory _json = Base64.encode(bytes(string(abi.encodePacked(template[templateId].jsonStorage,  attributes))));
    ...
    function _templateIdString(uint256 templateId) internal view returns(string memory) {
        return string(abi.encodePacked('{"name": "', template[templateId].name, '", "description": "', template[templateId].description,'","image": "', template[templateId].imageURL, '",'));
    }
        ...
        returns(string memory) 
    {
        ...
        return string(abi.encodePacked('"attributes": [{ "trait_type": "Name", "value": "', template[id].name, attribute,'" }, { "trait_type": "Win Count", "value": "', _wc, '" }, { "trait_type": "Played Count", "value": "', _pc, '" }, { "trait_type": "Slot", "value": "', _slot, '" }]}'));
    }

    function _attributes1(
        uint8 level, 
        uint8 top, 
        uint8 left, 
        uint8 right, 
        ...
        returns(string memory) 
    {
        ...
        return string(abi.encodePacked('" }, { "trait_type": "Level", "value": "', _level, '" }, { "trait_type": "Top", "value": "', _top, '" }, { "trait_type": "Left", "value": "', _left, '" }, { "trait_type": "Right", "value": "', _right,'" }, { "trait_type": "Bottom", "value": "', _bottom));
    }
