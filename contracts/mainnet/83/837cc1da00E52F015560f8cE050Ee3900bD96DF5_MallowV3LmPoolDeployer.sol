...
/// @notice Wraps MallowSwap V3 positions in a non-fungible token interface which allows for them to be transferred
/// and authorized.
...
interface IMasterChefV3 {
    ...
    function getMallowPerSecond(address _v3Pool) external view returns (uint256 poolMallowPerSecond);
}
...
import '@mallowswap/v3-core/contracts/libraries/LiquidityMath.sol';
...
import '@mallowswap/v3-core/contracts/libraries/LowGasSafeMath.sol';
import '@mallowswap/v3-core/contracts/libraries/SafeCast.sol';
import '@mallowswap/v3-core/contracts/libraries/FullMath.sol';
import '@mallowswap/v3-core/contracts/libraries/FixedPoint128.sol';
import '@mallowswap/v3-core/contracts/interfaces/IMallowV3Pool.sol';

import './libraries/LmTick.sol';

import './interfaces/IMallowV3LmPool.sol';
import './interfaces/IMasterChefV3.sol';

contract MallowV3LmPool is IMallowV3LmPool {
    ...
    IMallowV3Pool public immutable pool;
    ...
    function accumulateReward(uint32 currTimestamp) external override onlyPoolOrMasterChef {
        if (currTimestamp <= lastRewardTimestamp || msg.sender == address(pool)) {
            return;
        }

        if (lmLiquidity != 0) {
            uint256 rewardPerSecond = masterChef.getMallowPerSecond(address(pool));

            uint32 duration = currTimestamp - lastRewardTimestamp;
...
import '@mallowswap/v3-core/contracts/interfaces/IMallowV3Factory.sol';
import '@mallowswap/v3-periphery/contracts/interfaces/INonfungiblePositionManager.sol';
