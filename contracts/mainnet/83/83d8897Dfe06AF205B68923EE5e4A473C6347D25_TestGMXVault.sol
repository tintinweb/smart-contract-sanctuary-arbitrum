...
contract TestGMXVault {
  ...
  */
  ...
  function createOrder(uint256 amt, uint256 executionFee, bool isLong) external returns (bytes32) {
    address _token;

    if (isLong) {
      _token = address(WETH);
      WETH.safeTransferFrom(msg.sender, address(this), amt);
    } else {
      _token = address(USDC);
