...
contract EthWater is ERC4626Upgradeable, OwnableUpgradeable {
    ...
    function initialize(address _WETH) external initializer {
        ...
        __ERC20_init("ETH-WATER", "S-WATER");
    }
    ...
    function lend(uint256 _borrowed, address _receiver) external onlyAllowedVaults returns (bool status) {
        ...
        console.log("totalDebt", totalDebt, am);
        // totalDebt += _borrowed;
        // totalAsset -= _borrowed;
        console.log("totalAsset", totalAsset);

        require(ILeverageVault(_receiver).getUtilizationRate() <= utilRate, "Leverage ratio too high");
        console.log("util rate");

        // if (totalWETH < _borrowed) {
        // IWETH(WETH).deposit{ value: _borrowed }();
        IWETH(WETH).withdraw(_borrowed);
        // }
        console.log("weth deposit", WETH, _receiver);
        console.log("totalWETH", IERC20Upgradeable(WETH).balanceOf(address(this)));
        //
        (bool success, ) = payable(_receiver).call{ value: _borrowed }("");
        // IERC20(WETH).transfer(_receiver, am);
        // transfer _borrowed of eth to receiver
    ...
    function withdraw(
        ...
        require(balanceOfETH() >= _assets, "Insufficient balance in vault");

        uint256 shares = previewWithdraw(_assets);
        uint256 feeAmount = (_assets * withdrawalFees) / DENOMINATOR;

        // if (totalWETH < _assets) {
    ...
    function withdrawETH(
        uint256 _assets, // Native (ETH) token amount
        address _receiver,
        address _owner
    ) public noZeroValues(_assets) stillLocked returns (uint256) {
        require(_assets <= maxWithdraw(msg.sender), "ERC4626: withdraw more than max");
        require(balanceOfETH() >= _assets, "Insufficient balance in vault");
