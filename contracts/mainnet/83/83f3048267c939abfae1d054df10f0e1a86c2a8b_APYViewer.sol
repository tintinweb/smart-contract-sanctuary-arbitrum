...
contract APYViewer {
    ...
    function getEquivalentInMarket(
        ...
        returns (uint256)
    {
        ...
        uint256 lpAmount = Token(
            _pendleMarketAddress
        ).balanceOf(
            controllerAddress
        );

        return IWiseOracleHub(
            WISE_SECURITY.WISE_ORACLE()
        ).getTokensInETH(
            _pendleMarketAddress,
            lpAmount
        );
        ...
        )
    {
        ...
        for (i = 0; i < lenDeposit;) {
            ...
            uint256 lendingRate = FEE_MANAGER.isAaveToken(token) == false
    ...
    function getLendingRateAave(
        ...
        returns (uint256)
    {
        address underlyingAsset = FEE_MANAGER.underlyingToken(
            _poolToken
        );
