...
import "contracts/Protocol/protocol/tokens/MinterPauserClaimableERC20.sol";
import "contracts/Protocol/interfaces/apwine/IFutureVault.sol";
import "contracts/Protocol/interfaces/apwine/tokens/IFutureYieldToken.sol";
import "contracts/Protocol/protocol/tokens/ClaimableERC20.sol";
 ...
 */
contract FutureYieldToken is IFutureYieldToken, MinterPauserClaimableERC20 {
    using SafeMathUpgradeable for uint256;

    IFutureVault public override futureVault;
    uint256 public override internalPeriodID;
    ...
    function _beforeTokenTransfer(
        ...
        if (
            ...
            if (super.balanceOf(from) < amount)
                futureVault.claimFYT(from, amount.sub(super.balanceOf(from)));
        }
    }
    ...
    function mint(address to, uint256 amount)
        public
        override(IFutureYieldToken, MinterPauserClaimableERC20)
    {
    ...
    function burn(uint256 amount)
        public
        virtual
        override(IFutureYieldToken, ClaimableERC20)
    {
        super.burn(amount);
    }
    ...
    {
        if (
            msg.sender != futureVault.getFutureWalletAddress() &&
        ...
        returns (uint256)
    {
        return
            super.balanceOf(account).add(
                futureVault.getClaimableFYTForPeriod(account, internalPeriodID)
            );
    }
...
contract MinterPauserClaimableERC20 is
    Initializable,
    ContextUpgradeable,
...
import "contracts/Protocol/interfaces/apwine/tokens/IPT.sol";
import "contracts/Protocol/interfaces/apwine/IRegistry.sol";
import "contracts/Protocol/interfaces/apwine/IFutureWallet.sol";
import "contracts/Protocol/interfaces/apwine/IRewardsRecipient.sol";
    ...
    PausableUpgradeable
{
    ...
    function _transfer(
        ...
        require(
            sender != getFutureAddress(),
            "ERC20: transfer from the future vault address"
        );
        require(
            recipient != getFutureAddress(),
            "ERC20: transfer to the future vault address"
        );
    ...
    function getFutureAddress() internal virtual returns (address) {}
