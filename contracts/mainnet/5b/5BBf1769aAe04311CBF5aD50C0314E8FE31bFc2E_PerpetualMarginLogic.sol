    ...
    IPerpetualMarginLogic
{
     ...
     */
    function withdrawDepositFromMarginAccount(
        uint24 _iPerpetualId,
        address _traderAddr
    ) external override {
        require(msg.sender == address(this), "onlythis");
    ...
    function _depositMarginForOpeningTrade(
        ...
        _isDepositAllowed(pool, _order.traderAddr, _fDepositRequired);
        _updateTraderMargin(_perpetual, _order.traderAddr, _fDepositRequired);
        _transferFromUserToVault(pool, _order.traderAddr, _fDepositRequired);
    ...
    function _reduceMarginCollateral(
        ...
        if (account.fPositionBC == 0) {
            // opening - no negative cash
            _fAmountToWithdraw = _fAmountToWithdraw > account.fCashCC
                ? account.fCashCC
                : _fAmountToWithdraw;
        ...
        _transferFromVaultToUser(liquidityPool, _traderAddr, _fAmountToWithdraw);
    ...
    function _withdrawDepositFromMarginAccount(
        ...
        _transferFromVaultToUser(liqPool, _traderAddr, fCashToMove);
