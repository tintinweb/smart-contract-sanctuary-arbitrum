...
import "./IBatchAdventurerGateway.sol";
...
contract AdventurerTrainer is ReentrancyGuard, Pausable, ManagerModifier {
  ...
  function train(
    ...
    _train(sender, _addresses, _adventurerIds, _proofs, _traitIds, _traitAmounts);
  }
  ...
  function train(
    ...
    _train(msg.sender, _addresses, _adventurerIds, _proofs, _traitIds, _traitAmounts);
  }
  ...
  function _train(
    ...
    ACTION_PERMIT.checkPermissionsMany(
      ...
      _adventurerIds,
      _proofs,
      ACTION_ADVENTURER_TRAINING
    );
    GATEWAY.checkAddressBatch(_addresses, _proofs);

    uint256 totalAnima;
    // Load adventurer data
    uint24[10][] memory baseStats = ADVENTURER_DATA.getRawBatch(
      _addresses,
      _adventurerIds,
      traits.ADV_TRAIT_GROUP_BASE
    );
    uint256[] memory trainingPoints = ADVENTURER_DATA.getBatch(
      _addresses,
      _adventurerIds,
    ...
    for (uint256 j = 0; j < _adventurerIds.length; j++) {
      ...
      uint[] memory traitIds = _traitIds[j];
      uint[] memory traitAmounts = _traitAmounts[j];
      ...
      for (uint256 i = 0; i < traitIds.length; i++) {
        ...
        require(
          traitIds[i] >= traits.ADV_TRAIT_BASE_STRENGTH &&
            traitIds[i] <= traits.ADV_TRAIT_BASE_CHARISMA,
          "AdventurerTrainer: Trait ID is not valid"
        );

        // Add stats to adventurer
        totalTraits += traitAmounts[i];
        baseStats[j][traitIds[i]] += uint24(traitAmounts[i]);

        uint cost = _getTrainingCost(
          addr,
          adventurerId,
          baseStats[j][traits.ADV_TRAIT_BASE_LEVEL],
          traitAmounts[i]
        );
        totalAnima += cost;
      ...
      require(
        totalTraits <= trainingPoints[j],
        "AdventurerTrainer: Max amount for Traits exceeded"
      );
      // Decrease remaining stats from adventurer
      trainingPoints[j] -= totalTraits;
    }
  ...
  function _getTrainingCost(
    ...
    return _level * trainingCost * _traitAmount;
  }
