...
contract Vault is ReentrancyGuard, IVault, VaultEvents {
    ...
    function createStv(
        ...
        stvId = _createStv(capacityOfStv, metadataHash, msg.sender);

        if (subscriptionFundLimit > 0) {
            _investSubscribers(stvId, msg.sender, op, subscriptionFundLimit);
        }
    }
    ...
    function createStvWithDeposit(
        ...
        }
        {
            ...
            if (subscriptionFundLimit > 0) {
                totalDepositWithSubscription = _investSubscribers(stvId, msg.sender, op, subscriptionFundLimit);
            }
        ...
        returns (address tradeToken, uint256 totalReceived)
    {
        ...
        if (BytesCheck.checkFirstDigit0x1(uint8(command))) {
            ...
            bytes memory perpTradeData =
                abi.encodeWithSignature("execute(uint256,bytes,bool,address)", command, data, isOpen, stvId);
    ...
    function createAndExecute(
        ...
        _verifyData(createData, signature);
        (uint96 capacityOfStv, uint96 subscriptionFundLimit, bytes32 metadataHash, uint96 amount, address sender) =
            abi.decode(createData, (uint96, uint96, bytes32, uint96, address));

        address op = operator;
        address stvId;
        address traderAccount;
        address admin = IOperator(op).getAddress("ADMIN");
        if (msg.sender != admin) {
            if (sender != msg.sender) revert Errors.InputMismatch();
        }

        {
            stvId = _createStv(capacityOfStv, metadataHash, sender);
        }

        {
            address q = IOperator(op).getAddress("Q");
            address defaultStableCoin = IOperator(op).getAddress("DEFAULTSTABLECOIN");
            traderAccount = IOperator(op).getTraderAccount(sender);
            if (traderAccount == address(0)) traderAccount = IQ(q).createAccount(sender);
            uint96 traderAccountBalance = uint96(IERC20(defaultStableCoin).balanceOf(traderAccount));
            if (traderAccountBalance < amount) revert Errors.BalanceLessThanAmount();

            bytes memory transferData = abi.encodeWithSignature("transfer(address,uint256)", stvId, amount);
            IAccount(payable(traderAccount)).execute(defaultStableCoin, transferData, 0);
            IStvAccount(stvId).deposit(traderAccount, amount, true);
            emit Deposit(stvId, sender, sender, amount);
        }

        {
            uint96 totalDepositWithSubscription;
            if (subscriptionFundLimit > 0) {
                totalDepositWithSubscription = _investSubscribers(stvId, sender, op, subscriptionFundLimit);
            }
            if (totalDepositWithSubscription + amount > capacityOfStv) {
                revert Errors.TotalRaisedMoreThanCapacity();
            }
        }

        {
            bytes calldata tData = tradeData;
            uint256 c = command;
            address tradeToken;
            uint256 totalReceived;
            (, uint256 tradeAmount) = _getAmountAndStvId(tData);
            if (tradeAmount == 0) revert Errors.ZeroAmount();

            if (BytesCheck.checkFirstDigit0x1(uint8(c))) {
                tradeToken = _getTradeToken(tData);
                totalReceived = Trade.execute(c, tData, true, op);
            } else {
                address perpTrade = IOperator(op).getAddress("PERPTRADE");
                bytes memory perpTradeData =
                    abi.encodeWithSignature("execute(uint256,bytes,bool,address)", c, tData, true, stvId);
                (bool success,) = perpTrade.call{value: msg.value}(perpTradeData);
                if (!success) revert Errors.CallFailed(perpTradeData);
            }

            IStvAccount(stvId).execute(tradeAmount, tradeToken, totalReceived, true);

            emit Execute(stvId, tradeAmount, totalReceived, c, tData, msg.value, true);
        }
    }
    ...
    /// @param protocolFees protocol fees in defaultStableCoin decimals
    function distribute(address stvId, uint96 managerFees, uint96 protocolFees)
        external
        onlyAdmin
    {
        StvInfo memory stv = getStvInfo(stvId);
        if (stv.status != StvStatus.OPEN) revert Errors.StvNotOpen();
        StvBalance memory stvBalance = getStvBalance(stvId);
        uint96 totalDepositTokenUsed = stvBalance.totalRaised;
        ...
        if (depositTokenBalance > protocolFees) depositTokenBalance = depositTokenBalance - protocolFees;
        else revert Errors.InputMismatch();

        uint96 totalRemainingAfterFees;
        uint96 mFee;
        if (depositTokenBalance > totalDepositTokenUsed) {
            uint96 profits = depositTokenBalance - totalDepositTokenUsed;
            mFee = (profits * (managerFees / 1e18)) / 100;
            totalRemainingAfterFees = depositTokenBalance - mFee;
        } else {
            totalRemainingAfterFees = depositTokenBalance;
        }

        IStvAccount(stvId).distribute(totalRemainingAfterFees, mFee, protocolFees);

        emit Distribute(stvId, totalRemainingAfterFees, mFee, protocolFees);
    }
    ...
    function claimStvTradingReward(uint256[] calldata commands, bytes[] calldata data) external onlyAdmin {
        ...
        for (; i < data.length;) {
            ...
            bytes memory perpTradeData =
                abi.encodeWithSignature("execute(uint256,bytes,bool,address)", command, rewardData, false, stvId);
    ...
    function claimFutureManager(bytes32 futureManager, bytes calldata signature) external {
        bytes memory data = abi.encode(futureManager, msg.sender);
        _verifyData(data, signature);
        address subscriptions = IOperator(operator).getAddress("SUBSCRIPTIONS");
        ISubscriptions(subscriptions).claimFutureManager(futureManager, msg.sender);
        emit ClaimFutureManager(futureManager, msg.sender);
    }
    ...
    function _createStv(uint96 capacityOfStv, bytes32 metadataHash, address sender) internal returns (address stvId) {
        StvInfo memory stv = Generate.generate(capacityOfStv, sender, operator, maxFundraisingPeriod);
        stvId = stv.stvId;
        IStvAccount(stvId).createStv(stv);
        emit CreateStv(metadataHash, stvId, stv.manager, stv.endTime, stv.capacityOfStv);
    }

    function _investSubscribers(address stvId, address sender, address op, uint96 subscriptionFundLimit)
        internal
        returns (uint96 totalDepositWithSubscription)
    {
        address defaultStableCoin = IOperator(op).getAddress("DEFAULTSTABLECOIN");
        address[] memory subscribers = IOperator(op).getAllSubscribers(sender);
        ...
        for (; i < users.length;) {
            ...
            if (amountAfterRatio > 0) {
                ...
                IAccount(payable(traderAccount)).execute(defaultStableCoin, transferData, 0);
                emit DepositWithSubscription(stvId, sender, traderAccount, amountAfterRatio);
            }
...
contract VaultEvents {
    ...
    event Distribute(address indexed stvId, uint96 totalRemainingAfterFees, uint96 mFee, uint96 pFee);
    ...
    event ClaimFutureManager(bytes32 indexed futureManagerHash, address indexed sender);
}
...
library Trade {
    ...
    function _getParams(bytes calldata data)
        internal
        pure
        returns (address stvId, uint256 amount, address tradeToken)
    {
        assembly {
            stvId := calldataload(data.offset)
            amount := calldataload(add(data.offset, 0x20))
