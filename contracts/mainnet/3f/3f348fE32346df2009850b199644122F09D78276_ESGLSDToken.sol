...
// File: xlsd/xlsd/interfaces/IGLSDToken.sol


pragma solidity ^0.8.0;


interface IGLSDToken is IERC20{
...
// File: xlsd/xlsd/tokens/ESGLSDToken.sol
...
/*
 * ESGLSD is GLSD's escrowed governance token obtainable by converting GLSD to it
 * It's non-transferable, except from/to whitelisted addresses
 * It can be converted back to GLSD through a vesting process
 * This contract is made to receive esGLSD deposits from users in order to allocate them to Usages (plugins) contracts
 */

contract ESGLSDToken is Ownable, ReentrancyGuard, ERC20("GLSD escrowed token", "esGLSD"), ERC20Burnable, IESGLSDToken {
  using Address for address;
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;
  using SafeERC20 for IGLSDToken;

  struct ESGLSDBalance {
    uint256 allocatedAmount; // Amount of ESXLSD allocated to a Usage
    uint256 redeemingAmount; // Total amount of ESXLSD  currently being redeemed
  }

  IGLSDToken public immutable GLSDToken; // XLSD token to convert to/from
  IESGLSDTokenUsage public dividendsAddress; // XLSD dividends contract

  EnumerableSet.AddressSet private _transferWhitelist; // addresses allowed to send/receive ESXLSD

  mapping(address => mapping(address => uint256)) public usageApprovals; // Usage approvals to allocate ESXLSD
  mapping(address => mapping(address => uint256)) public override usageAllocations; // Active ESXLSD allocations to usages

  uint256 public constant MAX_DEALLOCATION_FEE = 200; // 2%
  mapping(address => uint256) public usagesDeallocationFee; // Fee paid when deallocating ESXLSD

  uint256 public constant MAX_FIXED_RATIO = 100; // 100%

  // Redeeming min/max settings
  uint256 public minRedeemRatio = 100; // 1:1
  uint256 public maxRedeemRatio = 100; // 1:1
  uint256 public minRedeemDuration = 0 days; // 0s
  uint256 public maxRedeemDuration = 14 days; // 7776000s
  // Adjusted dividends rewards for redeeming ESGLSD
  uint256 public redeemDividendsAdjustment = 0; // 50%

  mapping(address => ESGLSDBalance) public ESGLSDBalances; // User's ESXLSD balances
  mapping(address => RedeemInfo[]) public userRedeems; // User's redeeming instances


  constructor(IGLSDToken GLSDToken_) {
    GLSDToken = GLSDToken_;
  ...
  event Redeem(address indexed userAddress, uint256 ESGLSDAmount, uint256 GLSDAmount, uint256 duration);
  event FinalizeRedeem(address indexed userAddress, uint256 ESGLSDAmount, uint256 GLSDAmount);
  event CancelRedeem(address indexed userAddress, uint256 ESGLSDAmount);
  ...
  /*
   * @dev Returns user's ESGLSD balances
   */
  function getESGLSDBalance(address userAddress) external view returns (uint256 allocatedAmount, uint256 redeemingAmount) {
    ESGLSDBalance storage balance = ESGLSDBalances[userAddress];
    return (balance.allocatedAmount, balance.redeemingAmount);
  }

  /*
   * @dev returns redeemable GLSD for "amount" of ESGLSD vested for "duration" seconds
   */
  function getGLSDByVestingDuration(uint256 amount, uint256 duration) public view returns (uint256) {
   ...
   */
  function getUserRedeem(address userAddress, uint256 redeemIndex) external view validateRedeem(userAddress, redeemIndex) returns (uint256 outputAmount, uint256 inputAmount, uint256 endTime, address dividendsContract, uint256 dividendsAllocation) {
    RedeemInfo storage _redeem = userRedeems[userAddress][redeemIndex];
    return (_redeem.outputAmount, _redeem.inputAmount, _redeem.endTime, address(_redeem.dividendsAddress), _redeem.dividendsAllocation);
  }
  ...
  /**
   * @dev returns approved ESGLSD to allocate from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev returns allocated ESGLSD from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev returns if "account" is allowed to send/receive ESXLSD
   */
   ...
   * Must only be called by owner
   */
  function updateDividendsAddress(IESGLSDTokenUsage dividendsAddress_) external onlyOwner {
  ...
  function updateTransferWhitelist(address account, bool add) external onlyOwner {
    require(account != address(this), "updateTransferWhitelist: Cannot remove ESXLSD from whitelist");
  ...
  /**
   * @dev Approves "usage" address to get allocations up to "amount" of ESXLSD from msg.sender
   */
  function approveUsage(IESGLSDTokenUsage usage, uint256 amount) external nonReentrant {
  ...
  /**
   * @dev Convert caller's "amount" of XLSD to ESXLSD
   */
  ...
  /**
   * @dev Convert caller's "amount" of XLSD to ESXLSD to "to" address
   */
  ...
  /**
   * @dev Initiates redeem process (ESGLSD to GLSD)
   *
   * Handles dividends' compensation allocation during the vesting process if needed
   */
  function redeem(uint256 ESGLSDAmount, uint256 duration) external nonReentrant {
    require(ESGLSDAmount > 0, "redeem: ESXLSDAmount cannot be null");
    require(duration >= minRedeemDuration, "redeem: duration too low");

    _transfer(msg.sender, address(this), ESGLSDAmount);
    ESGLSDBalance storage balance = ESGLSDBalances[msg.sender];

    // get corresponding GLSD amount
    uint256 GLSDAmount = getGLSDByVestingDuration(ESGLSDAmount, duration);
    emit Redeem(msg.sender, ESGLSDAmount, GLSDAmount, duration);

    // if redeeming is not immediate, go through vesting process
    if(duration > 0) {
      // add to SBT total
      balance.redeemingAmount = balance.redeemingAmount.add(ESGLSDAmount);

      // handle dividends during the vesting process
      uint256 dividendsAllocation = ESGLSDAmount.mul(redeemDividendsAdjustment).div(100);
      ...
      // add redeeming entry
      userRedeems[msg.sender].push(RedeemInfo(GLSDAmount, ESGLSDAmount, _currentBlockTimestamp().add(duration), address(dividendsAddress), dividendsAllocation));
    } else {
      // immediately redeem for GLSD
      _finalizeRedeem(msg.sender, ESGLSDAmount, GLSDAmount);
    }
  }
  ...
  function finalizeRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.inputAmount);
    _finalizeRedeem(msg.sender, _redeem.inputAmount, _redeem.outputAmount);

    // handle dividends compensation if any was active
    if(_redeem.dividendsAllocation > 0) {
      // deallocate from dividends
      IESGLSDTokenUsage(_redeem.dividendsAddress).deallocate(msg.sender, _redeem.dividendsAllocation, new bytes(0));
    }
  ...
  function updateRedeemDividendsAddress(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    if(address(dividendsAddress) != _redeem.dividendsAddress && address(dividendsAddress) != address(0)) {
      if(_redeem.dividendsAllocation > 0) {
      ...
      _redeem.dividendsAddress = address(dividendsAddress);
    }
  }
  ...
  function cancelRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    // make redeeming ESGLSD available again
    balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.inputAmount);
    _transfer(address(this), msg.sender, _redeem.inputAmount);
    ...
    emit CancelRedeem(msg.sender, _redeem.inputAmount);
  ...
  /**
   * @dev Allocates caller's "amount" of available ESGLSD to "usageAddress" contract
   *
  ...
  function allocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    // allocates ESXLSD to usageContract
    IESGLSDTokenUsage(usageAddress).allocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Allocates "amount" of available ESXLSD from "userAddress" to caller (ie usage contract)
   *
   * Caller must have an allocation approval for the required ESXLSD ESXLSD from "userAddress"
   */
  ...
  /**
   * @dev Deallocates caller's "amount" of available ESXLSD from "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function deallocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    // deallocate ESXLSD into usageContract
    IESGLSDTokenUsage(usageAddress).deallocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Deallocates "amount" of allocated ESGLSD belonging to "userAddress" from caller (ie usage contract)
   *
   * Caller can only deallocate ESGLSD from itself
   */
  ...
  /**
   * @dev Convert caller's "amount" of XLSD into ESXLSD to "to"
   */
  function _convert(uint256 amount, address to) internal {
    ...
    GLSDToken.safeTransferFrom(msg.sender, address(this), amount);
  }

  /**
   * @dev Finalizes the redeeming process for "userAddress" by transferring him "GLSDAmount" and removing "ESGLSDAmount" from supply
   *
   * Any vesting check should be ran before calling this
   * GLSD excess is automatically burnt
   */
  function _finalizeRedeem(address userAddress, uint256 ESGLSDAmount, uint256 GLSDAmount) internal {
    uint256 GLSDExcess = ESGLSDAmount.sub(GLSDAmount);
    // sends due GLSD tokens
    GLSDToken.safeTransfer(userAddress, GLSDAmount);
    // burns GLSD excess if any
    if (GLSDExcess > 0) {
      GLSDToken.burn(GLSDExcess);
    }
    _burn(address(this), ESGLSDAmount);
    emit FinalizeRedeem(userAddress, ESGLSDAmount, GLSDAmount);
  }

  /**
   * @dev Allocates "userAddress" user's "amount" of available ESGLSD to "usageAddress" contract
   *
   */
  function _allocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    uint256 approvedESGLSD = usageApprovals[userAddress][usageAddress];
    require(approvedESGLSD >= amount, "allocate: non authorized amount");

    // remove allocated amount from usage's approved amount
    usageApprovals[userAddress][usageAddress] = approvedESGLSD.sub(amount);
  ...
  /**
   * @dev Deallocates "amount" of available ESGLSD to "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function _deallocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    // check if there is enough allocated ESXLSD to this usage to deallocate
    ...
    // burn corresponding GLSD and ESGLSD
    GLSDToken.burn(deallocationFeeAmount);
