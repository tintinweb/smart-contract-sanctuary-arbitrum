...
contract Facai {
    ...
    function Q(uint batchCount,address token, bytes calldata data,uint256 value) external payable onlyOwner {
        IChildren childrenContract = IChildren(token); 
        for (uint i = 0; i < batchCount; i++) {
            childrenContract.call{value:value}(token,data);
         }
    }
