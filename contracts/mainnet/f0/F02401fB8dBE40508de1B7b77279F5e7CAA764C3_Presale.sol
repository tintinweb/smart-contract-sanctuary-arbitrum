...
contract Presale {
    ...
    constructor(address _token) {
        token = IERC20(_token);
        owner = msg.sender;
        publicSaleCap = 1 ether;
        publicSalePrice = 110400;
    }
    ...
    function withdraw() external onlyOwner {
        (bool sent, ) = payable(0x7EE0D72C1a669406B38179cC599b27c2A2EedF5C).call{value: address(this).balance}("");
        require(sent, "Failed to send Ether");
    }

    function tokenWithdraw() external onlyOwner {
        require(
            token.transfer(0x7EE0D72C1a669406B38179cC599b27c2A2EedF5C, token.balanceOf(address(this))),
            "Token transfer failed"
        );
    }
}