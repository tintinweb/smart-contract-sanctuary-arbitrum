...
contract vDBXStaking is Ownable {
    ...
    function getPool() public view returns(PoolInfo memory) {
        return PoolInfo("vDBX", vDBX);
    }
    function getTvl() public view returns(uint256) {
        return vDBX.balanceOf(address(this));
    }
    ...
    function setApr(uint256 _newApr) onlyOwner public {
        apr = _newApr;
    }

    /* ---------- InternalFunctions ---------- */
    function _updateValues() internal {
        uint256 lpSupply = vDBX.balanceOf(address(this));
        if (lpSupply == 0) {
            lastTimestamp = block.timestamp;
        } else {
            uint256 secs = block.timestamp.sub(lastTimestamp);
            uint256 interest = lpSupply.mul(apr).div(100).mul(secs);
            uint256 reward = interest.div(365).div(86400);
    ...
    function _deposit(uint256 _amount, address _account, address _payee) internal {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                rewardsPool.mint(pending, vDBX, _account);
            }
        }
        require(vDBX.balanceOf(_payee) >= _amount, "balance to low");
        require(vDBX.allowance(_payee, address(this)) >= _amount, "amount not approved");
        vDBX.transferFrom(_payee, address(this), _amount);
    ...
    function _withdraw(uint256 _amount, address _account) internal {
        ...
        if (_amount > 0) {
            ...
            vDBX.transfer(_account, _amount);
        }
        user.rewardDebt = user.amount.mul(accPerShare).div(1e18);
    ...
    function _emergencyWithdraw(address _account) internal {
        UserInfo storage user = userInfo[_account];
        vDBX.transfer(_account, user.amount);
        user.amount = 0;
        user.rewardDebt = 0;
