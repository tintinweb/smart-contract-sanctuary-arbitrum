...
library Oracle {
    ...
    function transform(
        ...
        return
            Observation({
                ...
                secondsPerBoostedLiquidityPeriodX128: last.secondsPerLiquidityCumulativeX128 +
        ...
        )
    {
        if (secondsAgo == 0) {
            Observation memory last = self[index];
            if (last.blockTimestamp != time) last = transform(last, time, tick, liquidity, boostedLiquidity);
            return (
                last.tickCumulative,
    ...
    function newPeriod(Observation[65535] storage self, uint16 index, uint256 period) external {
        Observation memory last = self[index];
        self[index] = Observation({
            blockTimestamp: uint32(period),
...
library Tick {
    ...
    function cross(
        ...
        info.periodSecondsPerLiquidityOutsideX128[period] =
            secondsPerLiquidityCumulativeX128 -
            info.periodSecondsPerLiquidityOutsideX128[period];
        info.periodSecondsPerBoostedLiquidityOutsideX128[period] =
    ...
    function newPeriod(
        ...
        self[tick].periodSecondsPerLiquidityOutsideX128[period] = secondsPerLiquidityCumulativeX128;
        self[tick].periodSecondsPerBoostedLiquidityOutsideX128[period] = secondsPerBoostedLiquidityCumulativeX128;
    }
...
contract RamsesV2Pool is IRamsesV2Pool, Initializable {
    ...
    function _advancePeriod() private {
        ...
        if ((States._blockTimestamp() / 1 weeks) != _lastPeriod) {
            ...
            // start new period in ticks
            Tick.newPeriod(
                states._ticks,
                _slot0.tick,
                0, //secondsPerLiquidityCumulativeX128
                0, //secondsPerBoostedLiquidityPeriodX128
                period
            );
