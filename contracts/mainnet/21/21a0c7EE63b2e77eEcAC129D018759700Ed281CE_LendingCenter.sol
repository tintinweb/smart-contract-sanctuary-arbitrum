    ...
    PausableUpgradeable
{
    ...
    event LendingOrderCancel(uint256 indexed id, address indexed lender);
    ...
    function withdraw(
        ...
        emit LendingOrderWithdraw(order.id, order.lender, _amount);
        if (order.isClosed == true) {
            emit LendingOrderCancel(order.id, order.lender);
        }
    }
