...
/// @title Canonical MallowSwap V3 factory
/// @notice Deploys MallowSwap V3 pools and manages ownership and control over pool protocol fees
contract MallowV3Factory is IMallowV3Factory {
    uint8 public immutable VERSION = 1;

    /// @inheritdoc IMallowV3Factory
    ...
    function createPool(address tokenA, address tokenB, uint24 fee) external override returns (address pool) {
        ...
        pool = IMallowV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IMallowV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IMallowV3Pool(pool).collectProtocolFee(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IMallowV3Pool(pool).setLmPool(lmPool);
    }
}