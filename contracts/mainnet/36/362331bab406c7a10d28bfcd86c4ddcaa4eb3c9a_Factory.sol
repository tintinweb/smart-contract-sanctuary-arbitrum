...
contract Factory is IFactory, UpgradeableOperable {
     ...
     */
    function create(CreateParams memory params) external override returns (uint256) {
        _whitelisted();
        ...
        // Tracker Swapper
        _swapperPaths(params._type, trackerSwapper[params._type], params._paths, params._routes);
        ...
        stage[params._pool] = Stage.CREATED;

        poolByNonce[params._pool] = vars.nonce;

        lpContracts[vars.nonce] = vars.contracts;

        emit Create(vars.nonce, params._pool, Stage.CREATED, vars.contracts);
     ...
     */
    function setup(SetupParams memory params) external override {
        _whitelisted();

        LPContracts memory contracts = lpContracts[params.nonce];

        require(stage[contracts.lp] == Stage.CREATED, "Ivalid Stage");
        ...
        // Price Helper
        IPriceHelper(contracts.priceHelper).updatePoolData(contracts.lp, params._poolInput);

        stage[contracts.lp] = Stage.CONFIGURED;

        emit Setup(params.nonce, contracts.lp, Stage.CONFIGURED, contracts);
    }
     ...
     * @notice Verify if caller can call a function.
     */
    function _swapperPaths(uint256 _type, address _swapper, bytes[] memory _paths, uint8[] memory _routes) private {
        ...
        for (uint256 i; i < length; ++i) {
            if (_type == 1) {
                // UNISWAP
                IUniswapV3Swapper.SwapPair[] memory routes = new IUniswapV3Swapper.SwapPair[](_routes[i]);
                (asset0, asset1, routes) = abi.decode(_paths[i], (address, address, IUniswapV3Swapper.SwapPair[]));
                IUniswapV3Swapper(_swapper).upsertPathOverride(asset0, asset1, routes);
            } else if (_type == 2) {
                // ALGEBRA
                IAlgebraSwapper.SwapPair[] memory routes = new IAlgebraSwapper.SwapPair[](_routes[i]);
                (asset0, asset1, routes) = abi.decode(_paths[i], (address, address, IAlgebraSwapper.SwapPair[]));
                IAlgebraSwapper(_swapper).upsertPathOverride(asset0, asset1, routes);
            }
        }
    }
}
