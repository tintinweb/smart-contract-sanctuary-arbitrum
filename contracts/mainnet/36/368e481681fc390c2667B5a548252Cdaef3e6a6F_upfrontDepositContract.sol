...
contract upfrontDepositContract is ReentrancyGuard {
  string public constant VERSION = "1.0.9-arbitrum";
  ...
  function _openDepositWindow() internal {
    ...
    (uint256 year, uint256 month, ) = DateTime.getYearMonthDay(block.timestamp);

    require(block.timestamp >= DateTime.unixtimeFromDate(year, month, DEPOSIT_WINDOW_DAY, 0, 0, 0), "Unfinished");
  ...
  function _processRewards() internal {
    require(DEPOSIT_WINDOW_CLOSED == 0, "Unfinished");
  ...
  function _closeDepositWindow() internal {
    require(DEPOSIT_WINDOW_CLOSED == 0, "Deposit window closed");
    require(DEPOSIT_WINDOW_START + DEPOSIT_WINDOW_LENGTH <= block.timestamp, "Unfinished");
