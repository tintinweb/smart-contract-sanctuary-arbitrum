...
contract OpenEdenResonateOracle is ProviderAwareOracle {
    ...
    constructor(address _provider, address _USDC, address _openEdenTBillPriceOracle) ProviderAwareOracle(_provider) {
        USDC = _USDC;
        oracle = _openEdenTBillPriceOracle;
    }

    function getPrice(address) internal view returns (uint256 _amountOut) {
        return provider.getSafePrice(USDC) * IOpenEdenOracle(oracle).latestAnswer() / 1e6;
    }
