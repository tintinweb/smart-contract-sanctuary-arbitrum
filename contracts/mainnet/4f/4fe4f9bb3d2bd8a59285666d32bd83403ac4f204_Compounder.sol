...
contract Compounder is ICompounder, UpgradeableOperableKeepable, ERC20Upgradeable {
    ...
    function initializeCompounder(
        ...
        manager.approve(_tracker, type(uint256).max);
        token0.approve(_manager, type(uint256).max);
        token1.approve(_manager, type(uint256).max);
        token0.approve(_swapper, type(uint256).max);
        token1.approve(_swapper, type(uint256).max);
    }
    ...
    function setInternalContracts(address _swapper, address _router, address _tracker) external onlyGovernor {
        manager.approve(address(tracker), 0);
        router = IRouter(_router);
        tracker = IRewardTracker(_tracker);
        manager.approve(_tracker, type(uint256).max);

        address swaper_ = address(swapper);
        token0.approve(swaper_, 0);
        token1.approve(swaper_, 0);
        swapper = ITokenSwapper(_swapper);
        token0.approve(_swapper, type(uint256).max);
        token1.approve(_swapper, type(uint256).max);
    }
    ...
    function setManager(address _manager) external onlyGovernor {
        ...
        manager.approve(address(tracker), 0);
        token0.approve(_manager, 0);
        token1.approve(_manager, 0);
        token0.approve(_swapper, 0);
        token1.approve(_swapper, 0);
        ...
        manager.approve(address(tracker), type(uint256).max);
        token0.approve(_manager, type(uint256).max);
        token1.approve(_manager, type(uint256).max);
        token0.approve(_swapper, type(uint256).max);
        token1.approve(_swapper, type(uint256).max);
    ...
    function _compound() private {
        ...
        if (amount0 > 0 || amount1 > 0) {
            (int24 tickLower, int24 tickUpper) = manager.defaultRange();
            (uint256 depositAmount0, uint256 depositAmount1) =
                router.price().amountsForAmount0(manager.getPool(), tickLower, tickUpper, amount0);

            if (
                depositAmount0 > (10 ** IERC20Metadata(address(token0)).decimals()).div(100)
                    || depositAmount1 > (10 ** IERC20Metadata(address(token1)).decimals()).div(100)
            ) {
                router.compoundDeposit(depositAmount0, depositAmount1);
                totalAssets = tracker.stakedAmount(address(this));
            }
    ...
    function _convertToShares(uint256 assets) private view returns (uint256 shares) {
        ...
        return (assets == 0 || supply == 0)
            ? assets.mulDivDown(10 ** decimals(), 10 ** IERC20Metadata(address(manager)).decimals())
    ...
    function _convertToAssets(uint256 shares) private view returns (uint256 assets) {
        uint256 supply = totalSupply();
        return (supply == 0)
            ? shares.mulDivDown(10 ** IERC20Metadata(address(manager)).decimals(), 10 ** decimals())
