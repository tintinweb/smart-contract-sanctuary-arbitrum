...
import "../interfaces/IStakingV1FLY.sol";

abstract contract AirPuff1XStake is
    OwnableUpgradeable,
    ...
    ERC20BurnableUpgradeable
{
    ...
    uint256 public MAX_STAKING_POSITIONS;
    ...
    event SetStakingAddress(address indexed stakingContract);
    ...
    function setLRTAsset(address _LRTAsset) external onlyOwner {
        require(
            _LRTAsset != address(0),
            "AirPuff: LRTAsset address is invalid"
        );
        LRTAsset = _LRTAsset;
        emit LRTAssetChanged(_LRTAsset);
    }

    function setStakingContract(address _stakeFly) external onlyOwner {
        require(_stakeFly != address(0), "invalid address");
        stakingV1 = _stakeFly;
        emit SetStakingAddress(_stakeFly);
    }
    ...
    function getTotalStakePositionLen() external view returns(uint256) {
        return IStakingV1FLY(stakingV1).stakingPositionsLen(address(this));
    }

    function getTotalPositionStakedAndPoints() external view returns(uint256 staked, uint256 points) {
        return IStakingV1FLY(stakingV1).stakingDetails(address(this));
    }
    ...
    function openPosition(uint256 _amount) external whenNotPaused nonReentrant {
        ...
        // amount deposited should be greater than minimum required amount to stake
        require(_amount >= IStakingV1FLY(stakingV1).minFlyAmount(), "amount is less than required amount");
        IERC20(LRTAsset).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
        ...
        uint256 stakingId = IStakingV1FLY(stakingV1).stakingPositionsLen(address(this));
        // approve staking v1 for _amount
        IERC20(LRTAsset).approve(stakingV1, _amount);

        if (stakingId < MAX_STAKING_POSITIONS) IStakingV1FLY(stakingV1).stake(_amount);

        UserInfo memory _userInfo = UserInfo({
            user: msg.sender,
            deposit: _amount,
            positionId: posID,
            stakingId: stakingId < MAX_STAKING_POSITIONS ? stakingId : MAX_BPS, // MAX_BPS should be the default
    ...
    //     uint256 _positionID
    // ) external InvalidID(_positionID, msg.sender) whenNotPaused nonReentrant {
    ...
    //         uint256 fee = _userInfo.deposit.mulDiv(
    //             feeConfiguration.withdrawalFee,
    //             MAX_BPS
    //         );
    //         amount = _userInfo.deposit - fee;
    //         IERC20(LRTAsset).safeTransfer(
    ...
    //     IERC20(LRTAsset).safeTransfer(msg.sender, amount);

    //     emit Withdraw(
    //         msg.sender,
    //         _positionID,
    //         amount,
    //         _userInfo.closeTimestamp
    //     );
    // }
...
import "./abstracts/AirPuff1XStake.sol";

contract AirPuff1XFly is AirPuff1XStake {
    ...
    function initialize(address _pt_fly) external initializer {
        require(_pt_fly != address(0), "AirPuff: LRT address is invalid");

        LRTAsset = _pt_fly;
        MAX_BPS = 100_000;
        MAX_STAKING_POSITIONS = 100;

        __Ownable_init(msg.sender);
        __Pausable_init();
        __ReentrancyGuard_init();
        __ERC20_init("AirPuff-fluidity1X", "bffly1X");
    }
}
...
interface IStakingV1FLY {
    ...
    function stakingPositionsLen(address _account) external view returns(uint256);
