...
contract MIndexFeeStorage is Ownable, IFeeStorage {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    uint16 public split1 = 5000;
    address public receiver1 = 0x5C2Cbe1Efdcafe2e6DfFe20d19F54E6221A626A2; //Multisig owner
    address public receiver2 = 0x87A425F8CAe8867a874a9c7B2Fd5eE6768B2870f; //Multisig NFT rewards

    function setFeeSplit(uint16 newSplit) external onlyOwner {
        require(newSplit <= 10000);
        split1 = newSplit;
    }

    function setReceiver1(address newRec) external onlyOwner {
        require(newRec != address(0));
        receiver1 = newRec;
    }

    function setReceiver2(address newRec) external onlyOwner {
        require(newRec != address(0));
        receiver2 = newRec;
    }
    ...
    function sendFee(IERC20 token, uint256 amount) external {
        ...
        uint256 amount1 = amount.mul(split1).div(10000);
        uint256 amount2 = amount.sub(amount1);
        token.safeTransfer(receiver1, amount1);
        token.safeTransfer(receiver2, amount2);
    }
}
...
contract MIndexMarketLive is Ownable, ReentrancyGuard {
    //Libraries
    using SafeERC20 for IERC20;
    using SafeERC20 for IMintableBurnable;
    using SafeMath for uint256;
    ...
    uint256 public mTokenDecimals = 6;
    IMintableBurnable public mToken = IMintableBurnable(0x3971203DF7e4846B0f8d7e3a452d336bFBd9a0BF);
    IERC20 public buyToken = IERC20(0xaf88d065e77c8cC2239327C5EDb3A432268e5831); //USDC
    IERC20 public sellToken = IERC20(0x2eE1Fc67bEC816F3DAe2d2c9409483BEf49F181B); //mUSDC
    uint16 public depositFee = 1000;
    uint16 public withdrawFee = 1000;
    mapping(uint256 => bool) public depositFeeNftSet;
    mapping(uint256 => bool) public withdrawFeeNftSet;
    ...
    uint256 public priceSell;
    uint256 public priceValidUntil;
    uint256 public priceLastUpdate;

    //To avoid errors on pricing:
    uint256 public MIN_PRICE_BUY = 1000000;
    uint256 public MAX_PRICE_BUY = 1150000;
    uint256 public MIN_PRICE_SELL = 950000;
    uint256 public MAX_PRICE_SELL = 1100000;
    uint256 public SLIPPAGE = 30;

    //Events
    event PriceChanged(uint256 newPriceBuy, uint256 newPriceSell);
    event PriceBuyRangeChanged(uint256 min, uint256 max);
    event PriceSellRangeChanged(uint256 min, uint256 max);
    event DepositFeeStorageChanged(address newStorage);
    event WithdrawFeeStorageChanged(address newStorage);
    event BuyTokenChanged(address newBuyToken, uint8 decimals);
    event SellTokenChanged(address newBuyToken, uint8 decimals);
    ...
    //Constructor
    constructor() {
        setDepositFeeNft(1, 500);
        setWithdrawFeeNft(1, 500);
    }
    ...
    function setPrice(uint256 newPriceBuy, uint256 newPriceSell, uint256 duration) external onlyTrader {
        require(newPriceBuy >= MIN_PRICE_BUY, 'Price buy below minimum');
        require(newPriceBuy <= MAX_PRICE_BUY, 'Price buy above maximum');
        require(newPriceSell >= MIN_PRICE_SELL, 'Price sell below minimum');
        require(newPriceSell <= MAX_PRICE_SELL, 'Price sell above maximum');
        priceBuy = newPriceBuy;
        priceSell = newPriceSell;
        priceLastUpdate = block.timestamp;
        priceValidUntil = block.timestamp + duration;
        emit PriceChanged(newPriceBuy, newPriceSell);
    }
    ...
    function setPriceRangeBuy(uint256 min, uint256 max) external onlyOwner {
        MIN_PRICE_BUY = min;
        MAX_PRICE_BUY = max;
        emit PriceBuyRangeChanged(min, max);
    }

    function setPriceRangeSell(uint256 min, uint256 max) external onlyOwner {
        MIN_PRICE_SELL = min;
        MAX_PRICE_SELL = max;
        emit PriceSellRangeChanged(min, max);
    }
    ...
    function setDepositFeeNft(uint256 planId, uint16 fee) public onlyOwner {
        require(fee < 5000, 'Fee not in range');
        depositFeeNft[planId] = fee;
        depositFeeNftSet[planId] = true;
        emit DepositFeeNftChanged(planId, fee);
    ...
    function setWithdrawFeeNft(uint256 planId, uint16 fee) public onlyOwner {
        require(fee < 5000, 'Fee not in range');
        withdrawFeeNft[planId] = fee;
        withdrawFeeNftSet[planId] = true;
        emit WithdrawFeeNftChanged(planId, fee);
    ...
    //PUBLIC Methods
    function sell(uint256 amount) external nonReentrant onlyEnabled {
        require(amount > 0, 'Not valid amount');
        require(priceSell > 0, 'Not valid price');
        require(block.timestamp <= priceValidUntil, 'Price expired');
        ...
        //Amount after fees of the order
        uint256 amountOrder = amount.sub(wFeeAmount);

        //Apply slippage
        uint256 priceCalc = priceSell.mul(10000 - SLIPPAGE).div(10000);

        uint256 amountLiq = amountOrder.mul(priceCalc).div(10 ** DECIMALS_SELL);
        require(amountLiq <= sellToken.balanceOf(address(this)), 'Not enough liquidity. Try again later.');

        //Send tokens back to seller
        sellToken.safeTransfer(msg.sender, amountLiq);

        //Burn mTokens
        mToken.burn(address(this), amountOrder);

        emit Sold(msg.sender, priceSell, amountOrder, amountLiq);
    }

    function buy(uint256 amountIn) external nonReentrant onlyEnabled {
        require(amountIn > 0, 'Not valid amount');
        require(priceBuy > 0, 'Not valid price');
        require(block.timestamp <= priceValidUntil, 'Price expired');
        ...
        if (dFee > 0) {
            dFeeAmount = amountIn.mul(dFee).div(10000);
            buyToken.safeIncreaseAllowance(address(depositFeeStorage), dFeeAmount);
            depositFeeStorage.sendFee(buyToken, dFeeAmount);
        }

        //Amount after fees of the order
        uint256 amountOrder = amountIn.sub(dFeeAmount);

        //Apply slippage
        uint256 priceCalc = priceBuy.mul(10000 + SLIPPAGE).div(10000);

        //Mint new index tokens to buyer
        uint256 amountIndex = amountOrder.mul(10 ** DECIMALS_BUY).div(priceCalc);
        mToken.mint(msg.sender, amountIndex);

        emit Bought(msg.sender, priceBuy, amountOrder, amountIndex);
    }
...
// File: interfaces/IMuchoIndexOrderQueue.sol
...
// File: contracts/mIndexOrderQueue.sol
...
contract MIndexRewardRouter is Ownable, ReentrancyGuard {
    //Libraries
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    //Variables
    IERC20 public tokenReward = IERC20(0x3971203DF7e4846B0f8d7e3a452d336bFBd9a0BF);
    address public keeper = 0x850598B7Cf66858e3743994338A6f6eF89076406;
    uint256 public LIMIT_LOOP = 100;

    //Setters
    function setTokenReward(IERC20 newTokenReward) external onlyOwner {
        //Cannot be changed to avoid inconsistency with deposited rewards
        require(allUsers.length() == 0, 'Cannot change when rewards are pending');
    ...
    function setLimitLoop(uint256 newLimit) external onlyOwner {
        LIMIT_LOOP = newLimit;
    }

    //Modifiers
    modifier onlyKeeper() {
        require(msg.sender == keeper, 'Only for keeper');
        _;
    }
    ...
    //Events
    event Harvested(IERC20 token, uint256 amount, address user);
    event Rewarded(Reward reward);
    event BulkRewarded(RewardBulk reward);

    /*---------------------------------------VIEWS--------------------------------------*/
    function expiredAmount(uint256 loop) external view returns (uint256 totalAmount) {
        uint256 init = loop * LIMIT_LOOP;
        uint256 end = loop * LIMIT_LOOP + LIMIT_LOOP;
        uint256 usersLength = allUsers.length();
        require(usersLength >= init, 'Loop out of range');
        ...
        for (uint256 i = init; i < end; i++) {
            ...
            for (uint256 j = 0; j < userRewards[user].length; j++) {
                if (userRewards[user][j].expirationDate < block.timestamp) {
                    totalAmount += userRewards[user][j].amount;
                }
            }
        }
    }
    ...
    /*---------------------------------------METHODS--------------------------------------*/
    function depositReward(Reward calldata reward) external onlyKeeper {
        //console.log('---depositReward');
        require(reward.amount > 0, 'Invalid amount');
        require(reward.user != address(0), 'Invalid user');

        tokenReward.safeTransferFrom(msg.sender, address(this), reward.amount);
        allUsers.add(reward.user);
        userRewards[reward.user].push(reward);
        userTotalReward[reward.user] += reward.amount;

        emit Rewarded(reward);
    }

    function depositRewardBulk(RewardBulk calldata reward) external onlyKeeper returns (uint256 amount) {
        require(reward.amount.length > 0, 'Invalid amount length');
        require(reward.user.length > 0, 'Invalid users length');
        require(reward.user.length < 1000, 'Too much users');
        require(reward.user.length == reward.amount.length, 'Different lenght for amount and users');

        for (uint16 i = 0; i < reward.amount.length; i++) {
            amount += reward.amount[i];
            allUsers.add(reward.user[i]);
            userRewards[reward.user[i]].push(
                Reward({date: reward.date, expirationDate: reward.expirationDate, amount: reward.amount[i], user: reward.user[i]})
            );
            userTotalReward[reward.user[i]] += reward.amount[i];
        }

        tokenReward.safeTransferFrom(msg.sender, address(this), amount);
        emit BulkRewarded(reward);
    }

    function harvest() external nonReentrant returns (uint256 amount) {
        require(userTotalReward[msg.sender] > 0, 'No rewards pending');

        amount = userTotalReward[msg.sender];

        userTotalReward[msg.sender] = 0;
        allUsers.remove(msg.sender);
        delete userRewards[msg.sender];

        tokenReward.safeTransfer(msg.sender, amount);
        emit Harvested(tokenReward, amount, msg.sender);
    }

    function harvestExpired(uint256 loop) external onlyOwner returns (uint256 totalAmount) {
        uint256 init = loop * LIMIT_LOOP;
        uint256 end = loop * LIMIT_LOOP + LIMIT_LOOP;
        uint256 usersLength = allUsers.length();
        require(usersLength >= init, 'Loop out of range');
        ...
        for (uint256 i = init; i < end; i++) {
            ...
            for (uint256 j = 0; j < userRewards[user].length; j++) {
                if (userRewards[user][j].expirationDate < block.timestamp) {
                    totalAmount += userRewards[user][j].amount;
                    userTotalReward[user] -= userRewards[user][j].amount;
                    userRewards[user][j].amount = 0;
                }
            }
        }

        tokenReward.safeTransfer(msg.sender, totalAmount);
    }
}
...
contract MuchoBadgeManagerMock is IMuchoBadgeManager {
    mapping(address => Plan[]) userToPlan;

    function addPlan(address user, uint256 planId) external {
        userToPlan[user].push(
            Plan({
                id: planId,
                name: 'Mock Plan',
                uri: '',
                subscribers: 0,
                subscriptionPrice: Price({token: address(0), amount: 1}),
                renewalPrice: Price({token: address(0), amount: 1}),
                time: 10000000,
                exists: true,
                enabled: true
            })
    ...
    function activePlansForUser(address _user) external view returns (Plan[] memory planList) {
        return userToPlan[_user];
    }
}
