...
contract Vw {
    ...
    function createWallet(address _VWManager,address[] calldata _eoas) public returns(address[] memory) {
        address[] memory vws = new address[](_eoas.length);
        for (uint32 i = 0; i<_eoas.length; i++) {
            vws[i] = createWalletIfNotExists(_VWManager, _eoas[i]);
        }
        return vws;
    }
