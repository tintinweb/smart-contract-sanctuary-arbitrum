...
contract RoguePitBoss is ERC721Royalty, VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    ...
    //bytes32 keyHash = 0x83d1b6e3388bed3d76426974512bb0d270e9542a765cd667242ea26c0cc0b730; // Arb Goerli 50 gwei gas lane
    ...
    // NFT mint variables
    uint256 startDate = 1714338000; // 4/28/24 21.00 pm GMT, must be in past
    uint256 currentDay = ((block.timestamp - startDate) + 86400) / 86400; // change to 3600 to test. solidity rounds down so add extra day before division.
    uint256 startPrice = 1000000000000000; // 0.001 ETH change back to 2370000000000000000; // 2.37 ETH
    uint256 currentPrice = startPrice + ((currentDay - 1) * 1000000000000000); // add extra zero to change back to 0.01, makes price rise by 0.001 ETH per day
    ...
    Ownable(msg.sender)
    {
        COORDINATOR = VRFCoordinatorV2Interface(
            //0x6D80646bEAdd07cE68cab36c27c626790bBcf17f // Arbitrum Goerli
    ...
    address nftOwnerAdmin = 0xD57369D563A6c75c1C55b6C5C17e90203211cdEE;
    ...
    {
        if (tokenId > 114) { // tokenId 1 - 104 are replacing existing DHRC NFTs so are being minted by admin. 105 - 114 are pre-sold so minted by admin
        ...
        if (tokenId < 105) { // take same rarity from DHRC NFT with same tokenId
        ...
        if (tokenId > 104) { // get random number from Chainlink VRF to determine rarity 
    ...
    function updateDayAndPrice() external {
        ...
        currentPrice = startPrice + ((currentDay - 1) * 1000000000000000);// 0.001, add extra zero to make it 0.01
