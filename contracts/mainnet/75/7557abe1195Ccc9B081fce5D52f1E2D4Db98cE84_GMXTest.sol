...
contract GMXTest {
  ...
  uint256 public minLongTokenAmount;
  uint256 public minShortTokenAmount;
  ...
  address public gmxOracle = 0x1C16094d426Ab406a72Cc49b4171d1254D1Ddf18;
  ...
  function calcMinTokensSlippageAmt(
    ...
    uint256 _withdrawValue = lpAmt
      * IGMXOracle(gmxOracle).getLpTokenValue(
        address(lpToken),
        address(tokenA),
        address(tokenA),
        address(tokenB),
  ...
  */
  function tokenWeights() public view returns (uint256, uint256) {
    return (569050404731856419, 430949595268143580);
  }
  ...
  function convertToUsdValue(
    ...
    return amt * 10**(18 - IERC20Metadata(token).decimals())
                * IChainlinkOracle(chainlinkOracle).consultIn18Decimals(token)
                / SAFE_MULTIPLIER;
  }
