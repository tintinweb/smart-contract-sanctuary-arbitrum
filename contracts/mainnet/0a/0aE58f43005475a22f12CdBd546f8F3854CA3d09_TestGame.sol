...
/**
 * @title SicBoErrors
 * @dev Interface for defining custom errors used in the SicBo contract.
 */
interface SicBoErrors {
     ...
     */
    error SicBo__LockRoundBeforeLockAt();

    /**
     * @dev Error indicating that the round locked outside the buffer period.
     */
    error SicBo__LockRoundOutsideBuffer();
...
import { ISicBo } from "./interfaces/ISicBo.sol";
import { ITreasury } from "./interfaces/ITreasury.sol";
import { LibRoles as Roles } from "./libraries/LibRoles.sol";
import { Pausable } from "@openzeppelin/contracts/utils/Pausable.sol";
import { ReentrancyGuard } from "@openzeppelin/contracts/utils/ReentrancyGuard.sol";
import { SicBoErrors } from "./interfaces/SicBoErrors.sol";

contract TestGame is ISicBo, SicBoErrors, Pausable, ReentrancyGuard, AccessControlEnumerable {
    Currency currency;
    address nftAddress;
    address treasury;
    ...
    constructor(
        ...
        limitProtocolAmount = 100_000_000 ether;
        limitTreasuryAmount = 1_000_000 ether;
    ...
    function executeRound() public whenNotPaused onlyRole(Roles.OPERATOR_ROLE) {
        if (!genesisStartOnce) {
            revert SicBo__GenesisStartRoundNotTriggered();
        }
        if (!genesisLockOnce) {
            revert SicBo__GenesisLockRoundNotTriggered();
        }
    ...
    function genesisLockRound() external whenNotPaused onlyRole(Roles.OPERATOR_ROLE) {
        ...
        if (genesisLockOnce) {
            revert SicBo__GenesisLockRoundAlreadyTriggered();
        }
    ...
    function genesisStartRound() external whenNotPaused onlyRole(Roles.OPERATOR_ROLE) {
        if (genesisStartOnce) {
            revert SicBo__GenesisStartRoundAlreadyTriggered();
        }
        currentEpoch = currentEpoch + 1;
        _startRound(currentEpoch);
    ...
    function setProtocolFee(uint256 protocolFee_) external whenPaused onlyRole(DEFAULT_ADMIN_ROLE) {
        if (protocolFee_ > MAX_PERCENTAGE) {
            revert SicBo__InvalidAmount(protocolFee_);
        }
    ...
    function _safeLockRound(uint256 epoch_) internal {
        ...
        if (block.timestamp < rounds[epoch].lockAt) {
            revert SicBo__LockRoundBeforeLockAt();
        }
        if (block.timestamp > rounds[epoch].lockAt + bufferSeconds) {
            revert SicBo__LockRoundOutsideBuffer();
        }
    ...
    function _safeStartRound(uint256 epoch_) internal {
        ...
        if (rounds[epoch - 2].closeAt == 0) {
            revert SicBo__RoundNotStarted(epoch - 2);
        }
        if (block.timestamp < rounds[epoch - 2].closeAt) {
            revert SicBo__RoundNotEnded(epoch - 2);
        }
    ...
    /**
     * @dev Simulates rolling three dice for the SicBo game.
    ...
    /**
     * @dev Checks if the total score falls within the "Low" range in the SicBo game.
    ...
    /**
     * @dev Checks if the total score falls within the "High" range in the SicBo game.
     * @param totalScore_ The total score obtained from rolling the dice.
