...
interface IClearinghouseEventEmitter {
    ...
    event ClearinghouseInitialized(address endpoint, address quote, address fees);

    /// @notice Emitted when collateral is modified for a subaccount
    event ModifyCollateral(int128 amount, bytes32 indexed subaccount, uint32 productId);
...
interface IClearinghouseState {
    ...
    function getRisk(uint32 productId) external view returns (RiskHelper.Risk memory);
}
