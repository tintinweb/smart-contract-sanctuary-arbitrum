...
/**
 * @dev Basic implement for RiskManager
 * In this version, we only limit how many token can be tranferred each request.
 */
contract RiskManager is IRiskManager, BaseAuthorized {
    constructor(IACLManager aclManager) BaseAuthorized(aclManager) {}

    uint256 public maxAmount = type(uint256).max;

    function setMaxAmount(uint256 value) external onlyRiskAdmin {
        maxAmount = value;
    }
    ...
    function tryTransfer(
        ...
        require(amount <= maxAmount, "InvalidTransfer");
    }
}
