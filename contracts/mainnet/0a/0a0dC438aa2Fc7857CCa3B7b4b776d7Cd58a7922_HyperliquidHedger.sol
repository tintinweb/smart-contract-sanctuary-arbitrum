    ...
    AccessControlUpgradeable
{
    ...
    function hedge() external onlyAuthorized returns (int256 deltaDiff) {
        deltaCached = tempDelta;
        deltaDiff =  tempDelta - deltaCached;
    ...
    function initiateHedge(
        int256 _targetDelta,
        int256 _collateralMovment
    ) external onlyAuthorized returns (int256 deltaDiff) {

        if(tempDelta != 0) {
            revert("Temp delta is set there is a hedge running");
        }

        if(collateralDeposit != 0) {
            revert("Deposit is already set there is a hedge running");
        }

        if(collateralWithdraw != 0) {
            revert("Withdraw is already set there is a hedge running");
        }

        if(_collateralMovment > 0) {
            collateralDeposit = uint256(_collateralMovment);
        ...
        if(_collateralMovment < 0){
            collateralWithdraw = uint256(-1 * _collateralMovment);
        }

        deltaDiff =  _targetDelta - deltaCached;
        tempDelta = _targetDelta;
