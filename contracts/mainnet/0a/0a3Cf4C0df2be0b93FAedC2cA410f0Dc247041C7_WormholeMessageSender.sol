...
contract WormholeMessageSender is Ownable {
    uint256 constant GAS_LIMIT = 50_000;
    IWormholeRelayer public immutable wormholeRelayer;
    address public vault;
    mapping(uint16 => uint16) public chainIdToWormholeId;
    ...
    constructor(
        ...
        chainIdToWormholeId[1] = 2; // Ethereum
        chainIdToWormholeId[42161] = 23; // Arbitrum One
        chainIdToWormholeId[43114] = 6; // Avalanche
        chainIdToWormholeId[8453] = 30; // Base
        chainIdToWormholeId[56] = 4; // BSC
        chainIdToWormholeId[10] = 24; // Optimism
        chainIdToWormholeId[137] = 5; // Polygon
    }

    modifier onlyVault() {
        require(vault == msg.sender, "Caller is not the vault or vault is not registered");
        _;
    }
    ...
    function quoteGasPrice(
        ...
        require(chainIdToWormholeId[targetChain] != 0, 'Unsupported chain');
        (cost, ) = wormholeRelayer.quoteEVMDeliveryPrice(
            chainIdToWormholeId[targetChain],
            0,
            GAS_LIMIT
        );
    }
...
//        require(chainIdToWormholeId[destinationChain] != 0, 'Unsupported chain');
        uint256 cost = quoteGasPrice(chainIdToWormholeId[destinationChain]);
        require(msg.value >= cost, "Not enough funds to pay for the gas");

        wormholeRelayer.sendPayloadToEvm{value: cost}(
            chainIdToWormholeId[destinationChain],
            destinationAddress,
            payload,
            0,
            GAS_LIMIT
