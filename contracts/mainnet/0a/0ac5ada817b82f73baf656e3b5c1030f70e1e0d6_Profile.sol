 ...
 *      Avatar #0 of address(0) is the default avatar for every profile.
 */
contract Profile is AccessControl {
  ...
  event SetValidAvatarAddress(address avatarAddress, bool valid);
  ...
  function updateProfile(
    ...
    // Update
    updateUsername(msg.sender, _username);
    updateAvatar(_avatarAddress, _avatarId);
    updateInfo(msg.sender, _infoKeys, _infoValues);
  }
  ...
  /**
   * @dev Allows an admin to set validity of an Avatar contract address.
   * @param _avatarAddress The address of the Avatar contract to set validity.
   * @param _valid The validity of the Avatar contract.
   */
  function setValidAvatarAddress(address _avatarAddress, bool _valid) public onlyRole(PROFILE_ADMIN_ROLE) {
    ...
    // Event
    emit SetValidAvatarAddress(_avatarAddress, _valid);
  }
