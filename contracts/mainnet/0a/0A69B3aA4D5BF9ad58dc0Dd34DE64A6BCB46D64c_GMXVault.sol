...
library GMXDeposit {
  ...
  */
  function processDeposit(
    ...
    // Calculate equityBefore using current asset values
    uint256 _assetValueBefore = _dc.healthParams.lpAmtBefore
    ...
    uint256 _debtValueBefore = GMXReader.convertToUsdValue(
      self,
      address(self.tokenA),
      _tokenADebtAmt - _dc.borrowParams.borrowTokenAAmt
    )
    + GMXReader.convertToUsdValue(
      self,
      address(self.tokenB),
      _tokenBDebtAmt - _dc.borrowParams.borrowTokenBAmt
    );

    uint256 _equityBefore = _assetValueBefore - _debtValueBefore;

    self.depositCache.healthParams.equityBefore = _equityBefore;

    self.depositCache.sharesToUser = GMXReader.valueToShares(
      self,
      _dc.healthParams.equityAfter - _equityBefore,
      _equityBefore
    );
...
contract GMXVault is ERC20, AccessManaged, ReentrancyGuard, IGMXVault, IGMXVaultEvents {
  ...
  */
  ...
  /**
    * @notice Approve or revoke address to be blocked for this vault
    * @dev Should be called by approved Keeper
    * @param addr Address
    * @param blocked Boolean on whether address is blocked or not
  */
  function updateBlocklist(address addr, bool blocked) external restricted {
