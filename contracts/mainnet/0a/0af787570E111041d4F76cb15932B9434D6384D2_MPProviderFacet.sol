...
import {Governable, ECDSA} from "../Governable.sol";
import {ICommonErrors} from "../interfaces/IDiamondErrors.sol";

contract MPProviderFacet is IProvider, Governable {
    /**
     * @dev Modifier to restrict access to functions only to the Teleport facet.
     * The teleport is another facet in this same diamond, so we can check that the caller is this same diamond address.
     */
    modifier onlyTeleport() {
        if (msg.sender != address(this)) revert OnlyTeleportCalls();
        _;
    }
     ...
     * @param dappTranmissionInfo Information related to the dapp tranmission
     * @dev The address of the teleport param is ignored on this provider, as we only interact with the teleport contract.
     */
    function sendMsg(
        uint8 targetChainId_,
        bytes calldata,
        IProvider.DappTransmissionInfo calldata dappTranmissionInfo,
        bytes calldata
    ) external payable override onlyTeleport {
        if (msg.value > 0) {
            revert ICommonErrors.NoValueAllowed();
        }
        // The messaging protocol provider just emits the Tranmissions event. No need to validate the information, as we trust the teleport.
        emit Transmission(
            dappTranmissionInfo.dappTransmissionSender,
            targetChainId_,
            dappTranmissionInfo.dappTransmissionReceiver,
            dappTranmissionInfo.dAppId,
            dappTranmissionInfo.dappPayload
        );
    }

    function manualClaim(bytes calldata args) external onlyTeleport {
        // get all needed args and call _deliver method
        DeliverArgs memory params = abi.decode(args, (DeliverArgs));
        _deliver(params);
    }

    /**
     * @notice Delivers a message to an ITeleportDApp receiver contract, after verifying that:
     *         1. the transmission has not already been delivered
     *         2. the transmission is authorized by a majority of `signatures` from the `members` set
     *  @param params The parameters of the transmission, including:
     *        1. transmissionId_ The ID of the transmission, generated by the validators
     *        2. sourceChainId_ The chainID where the transmission originated
     *        3. transmissionSender_ The address that invoked `transmit()` on the source chain
     *        4. transmissionReceiver_ The address of the contract to receive the message
     *        5. dAppId_ ID for the dApp that the message belongs to
     *        6. payload_ The dApp-specific message data
     *        7. signatures_ The array of signatures from the members, authorising the operation
     */
    function _deliver(
        DeliverArgs memory params
    )
        private
        onlyConsensusHash(
            computeTransmissionHash(
                ...
                params.transmissionSender,
                params.transmissionReceiver,
                params.dAppId,
                params.payload
            ),
            params.signatures
    ...
    {
        emit Delivery(params.transmissionId);

        ITeleport(address(this)).onProviderReceive(
            ITeleport.DappTransmissionReceive({
                teleportSender: LibTeleport.teleportStorage().teleportAddressByChainId[params.sourceChainId], // We can use this address directly from this provider, as we are part of the same diamond.
                sourceChainId: params.sourceChainId,
                dappTransmissionSender: params.transmissionSender,
                dappTransmissionReceiver: params.transmissionReceiver,
    ...
    /// @notice Computes the bytes32 ethereum signed message hash of the transmission signatures
    /// @dev transmissionReceiver_ must be encoded as bytes to ensure cross-chain compatibility
    function computeTransmissionHash(
        bytes32 transmissionId_,
        uint8 sourceChainId_,
        bytes memory transmissionSender_,
        address transmissionReceiver_,
        bytes32 dAppId_,
            ...
            ECDSA.toEthSignedMessageHash(
                keccak256(
                    abi.encode(
                        ...
                        _addressToBytes(transmissionReceiver_),
                        dAppId_,
                        payload_
                    )
                )
    ...
    function fee(
        uint8,
        bytes calldata,
        IProvider.DappTransmissionInfo calldata,
        bytes calldata
    ) external pure override returns (uint256) {
        // This provider don't charge any extra fee for the transmissions, fees are being charged by the teleport contract.
        return 0;
    }
    ...
    /**
     * @dev Encodes the DeliverArgs struct into bytes.
     * @param args The DeliverArgs struct to be encoded.
     * @return The encoded bytes.
     */
    function deliverArgsToBytes(DeliverArgs calldata args) external pure returns (bytes memory) {
    ...
    function config(bytes calldata configData) external override onlyTeleport {
        // This provider doesn't need to be configured, but we still need to implement this method to satisfy the interface.
    }

    function supportedChains(uint256 index) external view override returns (uint8) {
        return LibTeleport.teleportStorage().supportedChainIds[index];
    }

    function supportedChainsCount() external view override returns (uint256) {
        return LibTeleport.teleportStorage().supportedChainIds.length;
    }
}
