...
contract RRSWallet is Ownable, ReentrancyGuard {
    

    uint256 public coinPrice;
    bool public isCoinActive;
    ...
    event BuyTicket(address indexed walletAddress, uint256 indexed quantity, address indexed tokenAddress, uint256 amount);
    event BuyTicketETH(address indexed walletAddress, uint256 indexed quantity, uint256 amount);
    ...
    /// @dev buy ticket by token
    /// @param _tokenAddress deposit token address
    /// @param _quantity ticket quantity
    function buyTicket(
        ...
        uint256 amount = _quantity*tokenPrice[_tokenAddress];
        require(IERC20(_tokenAddress).balanceOf(msg.sender) >= amount, "Insufficient funds");

        IERC20(_tokenAddress).transferFrom(msg.sender, address(this), amount);

        emit BuyTicket(msg.sender, _quantity, _tokenAddress, amount);
    }

    /// @dev buy ticket by coin
    /// @param _quantity ticket _quantity
    function buyTicketETH(
        ...
        uint256 amount = _quantity*coinPrice;
        require(amount <= msg.value, "Insufficient funds");

        emit BuyTicketETH(msg.sender, _quantity, msg.value);
    }
    ...
    /// @dev set ticket price of coin
    /// @param _price coin price
    function setRaceTicketPriceETH(
        uint256 _price
    ) external onlyWhiteList{
        coinPrice = _price;
    }    
    ...
    /// @dev set coin active
    /// @param _active active type
    function setCoinActive(
        bool _active
    ) external onlyOwner{
        isCoinActive = _active;
    }
