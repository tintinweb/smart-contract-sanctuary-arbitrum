...
contract LPStakingHold is DAPP, Auth {
    address address1;
    address address2;
    address address3;
    ...
    function distributeAllotment() external override {
        uint256 amountETH = address(this).balance * 333333 / 1000000;
    ...
    function distributeTokenAllotment(address token) external override {
        uint256 tokenAmt = IERC20(token).balanceOf(address(this)) * 333333 / 1000000;
        IERC20(token).transfer(address1, tokenAmt);
        IERC20(token).transfer(address2, tokenAmt);
        IERC20(token).transfer(address3, IERC20(token).balanceOf(address(this)));
