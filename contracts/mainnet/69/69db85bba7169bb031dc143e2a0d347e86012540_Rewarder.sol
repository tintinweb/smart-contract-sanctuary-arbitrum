...
contract Rewarder is IRewarder, AccessManaged {
    ...
    function compound(
        ...
        wrapper.claimSimple(address(this));
        ...
        uint balance = IERC20(poolToken).balanceOf(address(this));
        IERC20(poolToken).safeTransfer(address(SWAP_EXECUTOR), balance);
        SWAP_EXECUTOR.defaultSwap(poolToken, address(ABRA), minTokensBought);

        tokensBought = ABRA.balanceOf(address(this));
        ABRA.forceApprove(address(gauge), tokensBought);
        gauge.distributeReward(tokensBought);
    }
