...
import { IMoonpassVault, TokenWithdrawRequest, TokenDepositRequest } from "../interfaces/finance/IMoonpassVault.sol";
  ...
  MoonpassContract
{
  ...
  bytes32 internal constant TOKEN_DEPOSIT_TYPEHASH = keccak256("TokenDepositRequest(address token,address from,uint256 amount,RequestContext context)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  ...
  {
    amount = Token.wrap(tokenAddress).balanceOfSelf();
  }
  ...
  {
    amount = balance(address(0));
  }

  function deposit(
    TokenDepositRequest calldata request,
    ...
    whenNotPaused
  {
    ...
    emit VaultDeposit(tokenAddress, from, amount);
  }
    ...
    internal
  {
    ...
    emit VaultWithdraw(Token.unwrap(token), to, amount);
  }
}
