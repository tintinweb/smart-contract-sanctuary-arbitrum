...
contract FlexibleERC721 is ERC721, ERC2981, MintVoucherContract, CloneableNFT {
    constructor() ERC721("", "") CloneableNFT() {}
    ...
    **********************************************************************************************************/
    function initialize(
        ...
        _initialize(admin, owner_, signer, maxSupply_, soulBound_, name_, symbol_, baseURI_, baseContractURI_);
    }
    ...
    function mintWithVoucher(MintVoucher calldata voucher, bytes calldata signature) external payable {
        ...
        if (currentSupply + voucher.quantity > maxSupply) {
    ...
    /**
     * @dev Mint an NFT by an ADMIN or Contract owner
     * @param to The address to send the NFT token to
     * @param tokenId The ID of the NFT token
     */
    function adminMint(address to, uint256 tokenId) external adminOrOwnerOnly {
    ...
    /**
     * @dev Set permanent baseURI to the NFT collection
     * @param baseURI_ The baseURI to set permanent to
     */
    function setPermanentBaseURI(string memory baseURI_) external adminOrOwnerOnly {
        setBaseURI(baseURI_);
        baseURIPermanent = true;
    }

    /**
     * @dev Burn an NFT by the contract owner or the token owner
     * @param tokenId The ID of the NFT token
     */
    ...
    **********************************************************************************************************/
    ...
    function setBaseURI(string memory baseURI_) public adminOrOwnerOnly {
        if (baseURIPermanent) {
            revert CommonError.CannotUpdatePermanentURI();
        }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(AccessControl, ERC721, ERC2981) returns (bool) {
    ...
    function approve(address operator, uint256 tokenId) public override isTransferAllowed {
        super.approve(operator, tokenId);
    }

    function transferFrom(address from, address to, uint256 tokenId) public override isTransferAllowed {
        super.transferFrom(from, to, tokenId);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId) public override isTransferAllowed {
...
import "../platform/nft/common/Helper.sol";
...
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import "../mint-voucher/MintVoucherVerification.sol";
import "../platform/nft/common/Helper.sol";
