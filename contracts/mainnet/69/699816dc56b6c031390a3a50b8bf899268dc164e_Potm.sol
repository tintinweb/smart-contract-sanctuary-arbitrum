...
contract Potm is VRFConsumerBaseV2 {
    ...
    function fulfillRandomWords(uint256, uint256[] memory _randomWords) internal override {
        ...
        for (uint i = 0; i < prizes.length; i++) {
            uint256 winnerIndex = getRandomWinner(_randomWords[i]);
    ...
    function getRandomWinner(uint256 randomWord) private view returns (uint256) {
        uint256 totalTickets = 0;
        for (uint256 i = 0; i < participants.length; i++) {
            totalTickets += ticketsPerParticipant[participants[i]];
        }

        uint256 random = randomWord % totalTickets;
        uint256 sum = 0;
        for (uint256 i = 0; i < participants.length; i++) {
