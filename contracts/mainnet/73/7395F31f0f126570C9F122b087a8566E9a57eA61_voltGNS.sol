...
contract voltGNS is ERC4626, Ownable {
    ...
    constructor(IERC20 gns_, IERC20 dai_, address gnsVault_, address treasury_, address fees_, string memory name_, string memory symbol_) ERC20(name_, symbol_) ERC4626(gns_) {
    ...
    function withdraw(
		...
		assets = sendWithdrawFees(assets);
	...
	function sendWithdrawFees(uint assets) internal returns(uint) {
		...
		super._transfer(_msgSender(), treasury, fees);
		return assets - feesAssets;
	}

	function sendRedeemFees(uint shares) internal returns(uint) {
		uint withdrawFees = feesContract.getWithdrawFees(msg.sender);
	...
	//	maxGNSDeposits = 100_000 * 1e18; //100k GNS
