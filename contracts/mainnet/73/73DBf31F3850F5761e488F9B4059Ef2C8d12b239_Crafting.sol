  ...
  Pausable
{
  ...
  function handleDurabilityLoss(
    ...
    int[] memory productivityChanges = new int[](realmIds.length);
    for (uint i = 0; i < realmIds.length; i++) {
      for (uint j = 0; j < structureIds[i].length; j++) {
        uint configIndex = labConfig[structureIds[i][j]].labIndex;
        productivityChanges[i] += int(
          (uint(cache.productivityPerCraft[configIndex]) *
            durabilityLoss[i][j]) /
            uint(cache.durabilityLossPercentage[configIndex])
        );
      }
    }
    PRODUCTIVITY.changeBatch(realmIds, productivityChanges, true);
  }
  ...
  function handleDurabilityLoss(
    ...
    revert("Unsupported operation"); // Contract size exceeded when implemented
  }
