...
interface IBaseV1Factory {
    ...
    function arbiMaker() external returns (address);
...
contract BaseV1Fees {
    ...
    function claimFeesFor(address recipient, uint amount0, uint amount1) external returns (uint256 claimed0, uint256 claimed1) {
        ...
        // send 25% to arbiMaker
        address arbiMaker = IBaseV1Factory(factory).arbiMaker();
        if (arbiMaker != address(0x0)) {
            if (amount0 > 0) _safeTransfer(token0, arbiMaker, amount0 / 4);
            if (amount1 > 0) _safeTransfer(token1, arbiMaker, amount1 / 4);
            counter--;
        }
...
contract BaseV1Pair {
    ...
    function arbiMaker() external returns (address) {
        return IBaseV1Factory(factory).arbiMaker();
    }
...
contract BaseV1Factory {
    ...
    function setArbiMaker(address _arbiMaker) external {
        require (msg.sender == owner);
        arbiMaker = _arbiMaker;
    }
