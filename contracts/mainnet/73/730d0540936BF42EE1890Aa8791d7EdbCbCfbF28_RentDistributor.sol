...
import "./../interfaces/rent/IRentShare.sol";
import "./../interfaces/rent/IRentDistributor.sol";
 ...
 * @notice To redeem property rent to USDC/Jtry.
 */
...
contract RentDistributor is
    ...
    ERC721Holder
{
    // @todo add whenNotPaused modifier all over the protocol wherever needed
    ...
    mapping(string => uint) public propertyToRentAvailable;

    bool public rentWrapperToogle;

    /// @notice Role for vault to redeem rent instantly
    bytes32 public constant RENT_REDEEMER_VAULT_ROLE =
        keccak256("RENT_REDEEMER_VAULT");

    mapping(address propertyToken => uint rentClaimDuration)
        public propertyToRentClaimDurationForVault;
    //----------------------------------------
    // Events
    //----------------------------------------
    ...
    event RentRedeemed(
        ...
        uint indexed lockNftId,
        uint amount
    );
    event RentRedeemedByVault(
        address indexed recipient,
        uint indexed lockNftId,
        uint amount
    );
    event RentWrapperToogleUpdated(bool flag);
    event RentUpdated(string[] propertySymbols, uint[] amounts);
    event RentClaimDurationForVaultUpdated(
        address propertyAddress,
        uint oldDuration,
        uint newDuration
    );
     ...
     * @param symbols symbol of property whose rent is to remove
     * @param amounts amount of liquidity to remove
    ...
    function withdrawLiquidity(
        ...
        uint[] memory amounts,
        address to
    ) external whenNotPaused nonReentrant hasRole(Roles.RENT_MANAGER_ROLE) {
        uint arrLength = symbols.length;
        require(amounts.length == arrLength, "Array length miss-match");
        uint totalAmountToWithdraw;
        for (uint i; i < arrLength; i++) {
            propertyToRentAvailable[symbols[i]] -= amounts[i];
            totalAmountToWithdraw += amounts[i];
        }
        address USDC = address(systemRegistry.USDC());
        IERC20(USDC).safeTransfer(to, totalAmountToWithdraw);
        emit LiquidityWithdrawed(to, totalAmountToWithdraw);
    }

    function toogleRentWrapper(
        bool flag
    ) external whenNotPaused hasRole(Roles.RENT_MANAGER_ROLE) {
        // for auditor : deliberatley ignoring checks to check if desired state is already set or not
        rentWrapperToogle = flag;
        emit RentWrapperToogleUpdated(flag);
    }

    /**
     * @notice Initializes the contract.
     * @param _systemRegistry address of sytem registry
     */
    ...
    function redeem(
        uint lockNftTokenId,
        address recipient
    ) public whenNotPaused nonReentrant verifyCaller returns (uint) {
        ILockNFT lockNft = systemRegistry.lockNft();
        _isKycdAndNotSanctioned(
            recipient,
            lockNft.nftToRentShareLockDetails(lockNftTokenId).propertySymbols
        );
        address USDC = address(systemRegistry.USDC());
        uint rentAmount = getTotalRentAmount(lockNftTokenId);
        _redeemLockNft(lockNftTokenId);

        uint256 toTransfer = rentAmount - _calcFees(rentAmount);
        IERC20(USDC).safeTransfer(recipient, toTransfer);
        emit RentRedeemed(recipient, lockNftTokenId, toTransfer);
        return toTransfer;
    }

    function _redeemLockNft(uint lockNftTokenId) internal {
        require(_isLockNftMature(lockNftTokenId), "Too early to redeem Lock");
        _nftBurnAndStateUpdateHelper(lockNftTokenId);
    }

    // it does not actuallu burns the NFT, it transfers to PropertyToken address where this will be used
    // for offchain accounting
    function _nftBurnAndStateUpdateHelper(uint lockNftTokenId) internal {
        ILockNFT lockNftMinter = systemRegistry.lockNftMinter();
        address lockNft = address(systemRegistry.lockNft());
        // just wanted 1st symbol, as the array will contain 1 element only for now.
        string memory symbol = ILockNFT(lockNft)
            .nftToRentShareLockDetails(lockNftTokenId)
            .propertySymbols[0];
        address propertyAddress = systemRegistry
            .rentShare()
            .getSymbolToPropertyAddress(symbol);
        ...
        lockNftMinter.updateLockNftStatus(
            lockNftTokenId,
            ILock.LockStatus.EXECUTED
        );
    }

    function redeemRentForVault(
        uint lockNftTokenId
    ) external hasRole(Roles.RENT_REDEEMER_VAULT) returns (uint) {
        require(
            _isLockNftForVaultIsMature(lockNftTokenId),
            "Too early to redeem"
        );
        ...
        emit RentRedeemedByVault(msg.sender, lockNftTokenId, toTransfer);
        return toTransfer;
    }

    function _isLockNftForVaultIsMature(
        uint lockNftTokenId
        ...
        address propertyAddress = systemRegistry
            .rentShare()
            .getSymbolToPropertyAddress(symbol);
        uint nftCreatedAt = ILockNFT(lockNft)
            .nftToRentShareLockDetails(lockNftTokenId)
            .createdAt;
        return
            block.timestamp >=
            nftCreatedAt + propertyToRentClaimDurationForVault[propertyAddress];
    }

    function _isLockNftMature(
        uint lockNftTokenId
    ) internal view returns (bool) {
        IRentShare rentShare = systemRegistry.rentShare();
        return rentShare.isLockNftMature(lockNftTokenId);
    }
    ...
    function depositRentAgainstProperty(
        string[] memory symbols,
        uint[] memory amounts
    ) public whenNotPaused hasRole(Roles.RENT_MANAGER_ROLE) {
        uint arrLength = symbols.length;
        require(amounts.length == arrLength, "Array length miss-match");
        uint totalAmount;
        for (uint i; i < arrLength; i++) {
        ...
        emit RentUpdated(symbols, amounts);
    }

    function updatePropertyToRentClaimDurationForVault(
        address propertyToken,
        uint newDuration
    ) external hasRole(Roles.RENT_MANAGER_ROLE) {
        uint oldDuration = propertyToRentClaimDurationForVault[propertyToken];
        propertyToRentClaimDurationForVault[propertyToken] = newDuration;
        emit RentClaimDurationForVaultUpdated(
            propertyToken,
            oldDuration,
            newDuration
        );
    ...
    function getTotalRentAmount(uint lockNftTokenId) internal returns (uint) {
        ILockNFT lockNft = systemRegistry.lockNft();
        ILock.RentShareLock memory lockDetails = lockNft
            .nftToRentShareLockDetails(lockNftTokenId);
        uint arrLength = lockDetails.propertySymbols.length;
        uint totalRentAmount;
        string memory propertySymbol;
        uint propertyRent;
        for (uint i; i < arrLength; i++) {
            propertySymbol = lockDetails.propertySymbols[i];
            propertyRent = _normalizedAmount(lockDetails.amounts[i]);
            require(
                propertyToRentAvailable[propertySymbol] >= propertyRent,
                "Insufficient Rent available"
            );
            propertyToRentAvailable[propertySymbol] -= propertyRent;
            totalRentAmount += propertyRent;
        }

        return totalRentAmount;
    }

    function _normalizedAmount(uint _amount) internal pure returns (uint) {
        // because the amount coming from rentshare is in 18 decimals
