...
// File: contracts/pool/AISCodeBonusPool.sol


pragma solidity =0.8.19;

contract AICodeBonusPool is Ownable {
    ...
    function add(uint256 _allocPoint, address _token) public onlyOwner {
        require(_token != address(0), "AISCodeBonusPool: _token is the zero address");

        require(!EnumerableSet.contains(_pairs, _token), "AISCodeBonusPool: _token is already added to the pool");
    ...
    function setStandardRewardInterval(uint256 interval) public onlyOwner {
        require(interval > 0, "AISCodeBonusPool: interval must be greater than 0");
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            require(_amount.add(user.amount) <= maxStakingPerUser, 'AISCodeBonusPool: exceed max stake');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "AISCodeBonusPool: Withdraw with insufficient balance");
    ...
    function emergencyRewardWithdraw(uint256 _amount) public onlyOwner {
        require(_amount <= rewardToken.balanceOf(address(this)), 'AISCodeBonusPool: not enough token');
        rewardToken.safeTransfer(address(msg.sender), _amount);
    }

    function getPoolView(uint256 pid) public view returns (PoolView memory) {
        require(pid < poolInfo.length, "AISCodeBonusPool: pid out of range");
    ...
    function addCaller(address val) public onlyOwner() {
        require(val != address(0), "AISCodeBonusPool: val is the zero address");
        _callers.add(val);
    }

    function delCaller(address caller) public onlyOwner returns (bool) {
        require(caller != address(0), "AISCodeBonusPool: caller is the zero address");
