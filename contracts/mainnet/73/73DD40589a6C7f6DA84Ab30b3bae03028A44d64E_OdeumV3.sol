...
abstract contract OdeumCoreV3 is
    Ownable,
    ERC20,
    IOdeum
{
    ...
    address public constant dexRouter = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506; //SUSHISWAP
    /// @notice Token for which the commission is exchanged when it is withdrawn
    address public constant taxWithdrawToken = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1; //WETH
    uint256 public constant taxFee = 500;

    uint256 public collectedFee;

    mapping (address => bool) internal _isPairIncludedInFee;
    /// @notice Map of addresses to which the commission does not apply
    mapping(address => bool) internal _isAccountExcludedFromFee;
    ...
    event FeeCollected(address account, uint256 feeAmount);
    ...
    /// @notice Only owner can take fees
    ...
    function _transfer(
        ...
        if (recipient == address(0)) {
            ...
            require(amount > 0, "Odeum: transfer amount must be greater than zero");
...
/// @title A custom ERC20 token
contract OdeumV3 is OdeumCoreV3 {
    ...
    constructor(
        ...
        _mint(_owner, (INITIAL_CAP - poolWalletAmount) * (10 ** decimals()));
        _mint(poolWallet, poolWalletAmount * (10 ** decimals()));

        for(uint256 i=0; i<excluded.length; i++) {
            _isAccountExcludedFromFee[excluded[i]] = true;
            emit AccountExcludedFromFee(excluded[i]);
        }

        transferOwnership(_owner);
        _isPairIncludedInFee[_calcPair()] = true;
        emit PairIncludedInFee(_calcPair());
    }
    ...
    /// @notice Calculates ODEUM/WETH Sushiswap pair address
    function _calcPair() internal returns(address) {
        IUniswapV2Router02 router = IUniswapV2Router02(dexRouter);
        address factory = router.factory();
        (address token0, address token1) = address(this) < taxWithdrawToken
            ? (address(this), taxWithdrawToken)
            : (taxWithdrawToken, address(this));
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));

        bytes32 bytecodeHash = 0xe18a34eb0e04b04f7a0ac29a6e80748dca96319b42c54d679cb821dca90c6303;
        bytes32 rawAddress = keccak256(abi.encodePacked(bytes1(0xff), address(factory), salt, bytecodeHash));

        // The last 20 bytes form the address
