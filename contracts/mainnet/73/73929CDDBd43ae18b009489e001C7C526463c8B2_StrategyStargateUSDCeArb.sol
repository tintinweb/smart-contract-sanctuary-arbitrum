...
import "../common/BaseStrategyV2.sol";

// @dev Only works if the reward is ARB token.
contract StrategyStargateUSDCeArb is BaseStrategyV2 {
    using SafeERC20 for IERC20;

    address public unirouter = 0xE592427A0AEce92De3Edee1F18E0157C05861564; // uniswap v3 router
    // default constants
    address public constant ARB = 0x912CE59144191C1204E64559FE8253a0e49E6548; // ARB
    address public constant NATIVE = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1; // WETH
    ...
    // ARB ==> native
    bytes public rewardTokenRoute;

    // native ==> stargateAsset underlying
    bytes public nativeRoute;

    // ARB/stargateAsset Pool fees e.g ARB/USDCe
    uint24 public arbNativePoolFees;

    // NATIVE/stargateAsset Pool Fees e.g WETH/USDCe
    uint24 public nativeStargateAssetPoolFees;
    ...
    constructor(
        ...
        uint24 _arbNativePoolFees,
        uint24 _nativeStargateAssetPoolFees
    )
        BaseStrategyV2(
            _keeper,
            _strategist,
        ...
        )
    {
        ...
        // setting pool fees
        arbNativePoolFees = _arbNativePoolFees;
        nativeStargateAssetPoolFees = _nativeStargateAssetPoolFees;

        // default route
        rewardTokenRoute = abi.encodePacked(ARB, arbNativePoolFees, NATIVE);
        nativeRoute = abi.encodePacked(
            NATIVE,
            nativeStargateAssetPoolFees,
            stargateAsset
        );
    ...
    function _giveAllowanceRouter() internal {
        ...
        IERC20(ARB).safeApprove(unirouter, type(uint256).max);
    ...
    function _removeAllowanceRouter() internal {
        ...
        IERC20(ARB).safeApprove(unirouter, 0);
    ...
    function pendingRewardToken() public view returns (uint256) {
        return IStargateChef(chef).pendingEmissionToken(chefId, address(this));
    }

    function setRewardRoute(
        bytes calldata _rewardTokenRoute
    ) public onlyManager {
        (address firstToken, address lastToken) = _getFirstAndLastTokens(
            _rewardTokenRoute
        );
        _checkSwapPath(firstToken, lastToken, ARB, NATIVE);
        rewardTokenRoute = _rewardTokenRoute;
    }

    function setNativeRoute(bytes calldata _nativeRoute) public onlyManager {
        (address firstToken, address lastToken) = _getFirstAndLastTokens(
            _nativeRoute
        );
        _checkSwapPath(firstToken, lastToken, NATIVE, stargateAsset);
        nativeRoute = _nativeRoute;
    }

    /// @dev Pass 0 if don't want to change
    function setPoolsFee(
        uint24 _arbNativePoolFees,
        uint24 _nativeStargateAssetPoolFees
    ) public onlyManager {
        arbNativePoolFees = _arbNativePoolFees;
    ...
    function _checkSwapPath(
        ...
        require(_fromToken == _firstToken, "illegal path!");
        require(_destinationToken == _lastToken, "illegal path!");
    }

    /// @notice returns firstToken(from) and lastToken(to) from the provided path
    function _getFirstAndLastTokens(
        bytes calldata _swapPath
    ) internal pure returns (address firstToken, address lastToken) {
        uint calldataLength = _swapPath.length;
        require(
            calldataLength >= 40,
            "Input data length must be at least 40 bytes"
        );
        firstToken = address(bytes20(_swapPath));
        lastToken = address(bytes20(_swapPath[calldataLength - 20:])); // slices last 20 bytes and coverts to address
    }
    ...
    function _checkForWant(
        ...
        return address(pool.lpToken) == _want;
    }
    ...
    function _harvest(
        uint minUnderlyingAmountOutAfterFees,
        uint deadline
    ) internal override returns (uint256, uint256) {
        _claimRewards();
        uint256 outputBal = IERC20(ARB).balanceOf(address(this));
        if (outputBal > 0) {
            uint256 fee;
            if (publicHarvest) {
                fee = _chargeHarvestFee(outputBal, ARB, msg.sender);
                outputBal = outputBal - fee;
                require(outputBal > 0, "ZERO_AMOUNT");
            }
            _chargeFees(outputBal);
            _addLiquidity(minUnderlyingAmountOutAfterFees, deadline);
    ...
    function _chargeFees(
        ...
        IUniswapV3Router(unirouter).exactInput(
            IUniswapV3Router.ExactInputParams({
                path: rewardTokenRoute,
                recipient: address(this),
                deadline: block.timestamp,
                amountIn: _rewardToken,
                amountOutMinimum: 0
            })
        );
    ...
    function _addLiquidity(
        uint _minUnderlyingAmountOutAfterFees,
        uint _deadline
    ) internal {
        uint256 nativeBal = IERC20(NATIVE).balanceOf(address(this));
        ...
        IUniswapV3Router(unirouter).exactInput(
            IUniswapV3Router.ExactInputParams({
                ...
                deadline: _deadline,
                amountIn: nativeBal,
                amountOutMinimum: _minUnderlyingAmountOutAfterFees
            })
        );
...
// Same as BaseStBaseStrategy in common, just changed interface of harvest() to avoid MEVs
abstract contract BaseStrategyV2 is Ownable, Pausable {
    using Checks for *;
    using SafeERC20 for IERC20;
    ...
    function withdraw(
        uint256 _amount
    ) external onlyVault returns (uint256 wantWithdrawn) {
    ...
    function _harvest(
        uint minUnderlyingAmountOutAfterFees,
        uint deadline
    ) internal virtual returns (uint256 wantHarvested, uint256 harvesterFee);

    //  min underlying asset amount that should be deposited to chef
    function harvest(
        uint minUnderlyingAmountOutAfterFees,
        uint deadline
    ) external {
        ...
        (uint256 _wantHarvested, uint256 _harvesterFee) = _harvest(
            minUnderlyingAmountOutAfterFees,
            deadline
        );
    ...
    function _chargeFees(
        uint256 _tokensForFee
    ) internal virtual returns (uint256 feeTaken);
