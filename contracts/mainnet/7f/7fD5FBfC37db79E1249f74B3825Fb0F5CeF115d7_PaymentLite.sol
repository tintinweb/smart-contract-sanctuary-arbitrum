...
abstract contract PaymentTransfersAdapter is PaymentConstants {
     ...
     */
    function _deposit(address account, IERC20Upgradeable erc20, uint256 amount, address recipient) internal {
        require(currencySupport[erc20] == CurrencySupport.OPERATIVE, "BP103");
        emit Deposit(recipient, erc20, ERC20Utils.strictTransferFrom(erc20, account, address(this), amount));
    }
     ...
     */
    function _claim(address account, IERC20Upgradeable erc20, uint256 amount) internal {
...
contract PaymentLite is AccessControlUpgradeable, PausableUpgradeable, PaymentTransfersAdapter {
     ...
     */
    function rescueTokens(address account, IERC20Upgradeable erc20, uint256 amount) external onlyRole(OWNER_ROLE) {
        erc20.safeTransfer(account, amount);
    }
    ...
    function depositFunds(IERC20Upgradeable erc20, uint256 amount) external whenNotPaused {
        _deposit(_msgSender(), erc20, amount, _msgSender());
    }

    /**
     * @notice Deposit `amount` of supported token `erc20` to increase owned platform funds of `recipient`.
     * @param erc20 currency token to use for payment operation
     * @param amount amount of tokens to deposit
     * @param recipient recipient of the credits
     */
    function depositFundsWithRecipient(
        IERC20Upgradeable erc20,
        uint256 amount,
        address recipient
    ) external whenNotPaused {
        _deposit(_msgSender(), erc20, amount, recipient);
    }
