...
/**
 * A SpotOracle based on the stUSD and Transmuter smart contracts.
 */
contract STUSDUSDCOracle is AbstractOracle {
    using ABDKMath64x64 for int128;
    ...
    function getSpotPrice() public view virtual override returns (int128, uint256) {
        ...
        int128 fPrice;
        {
            uint256 stUSDUSDC = ITransmuter(SwapLib.transmuter).quoteOut(
                IERC4626(SwapLib.STUSD).previewMint(10 ** 18), // 1 stUSD = 1e18
                SwapLib.USDC,
                SwapLib.USDA
            ) + 1;
            fPrice = ConverterDec18.fromDecN(int256(stUSDUSDC), 6);
            // can't overflow: stUSD-USDC * ONE_64x64 ~ 1e27 << 2^255
            ts = block.timestamp;
        }
        return (fPrice, ts);
    }
     ...
     * Get base currency symbol.
     */
    function getBaseCurrency() external pure override returns (bytes4) {
        return bytes4("stUS");
    }
     ...
     * Get quote currency symbol.
     */
    function getQuoteCurrency() external pure override returns (bytes4) {
        return bytes4("USDC");
    }

    function priceId() external view override returns (bytes32) {}

    function priceFeed() external view override returns (address) {}

    function feedPeriod() external view override returns (uint256) {}
}