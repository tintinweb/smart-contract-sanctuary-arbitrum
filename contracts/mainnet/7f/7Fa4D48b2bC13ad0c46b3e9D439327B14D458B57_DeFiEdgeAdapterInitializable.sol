...
import "../interfaces/IDefiEdgeManager.sol";
import "../interfaces/IDefiEdgeAUMWrapper.sol";
...
contract DeFiEdgeAdapterInitializable is IAdapter, Initializable, Ownable, ReentrancyGuard {
    ...
    event SetVaultWrapper(address indexed oldVaultWrapper, address indexed newVaultWrapper);
    ...
    function tokenPerShare(
        ...
        if (totalSupply_ > 0) {
            (uint256 amount0, uint256 amount1) = IDefiEdgeAUMWrapper(vaultWrapper).getAUMWithoutFees(vault);
    ...
    function managerFee(
        ...
        return IDefiEdgeManager(manager()).performanceFeeRate();
    }
    ...
    function setVaultWrapper(
        address _newVaultWrapper
    ) external onlyOwner {
        require(_newVaultWrapper != address(0), "setWrapper: address cant be zero");

        address oldVaultWrapper = vaultWrapper;

        vaultWrapper = _newVaultWrapper;

        emit SetWrapper(oldVaultWrapper, vaultWrapper);
    }
...
interface IDefiEdgeAUMWrapper {
     ...
     */
    function getAUMWithoutFees(address _strategy) external view returns(uint256 reserve0, uint256 reserve1);
}
