...
contract HomechainOmnichainMessenger is IHomechainOmnichainMessenger, OmnichainMessenger {
    ...
    function redeem(
        ...
        IIndex.RedeemedInfo memory redeemed = IIndex(index).redeem(params, commandParams, msg.sender);
        if (redeemed.k != 0) {
            if (redeemed.reserveValuation == params.expectedReserveValuation && redeemed.k >= params.AUMScaledK) {
                _sendSnapshotTransferAndTrades(redeemed.k, sendParams);
            } else {
                _sendSnapshotTransfer(redeemed.k, sendParams);
            }
        }
    ...
    function retryRedeem(
        IIndex.RedeemKParams[] calldata params,
        ...
        for (uint256 i; i < retries.length; ++i) {
            ...
            for (uint256 j; j < retry.callbacks.length; ++j) {
                validateOptions(retry.options[j], minGasAmountForChain[retry.eid]);
    ...
    function _sendSnapshotTransfer(uint256 kRedeemed, SendParams calldata sendParams) internal {
        for (uint256 i; i < sendParams.config.eIds.length(); ++i) {
            uint16 eid = sendParams.config.eIds.at(i);

            bytes memory message = abi.encode(TRANSFER_SNAPSHOT, msg.sender, kRedeemed);
            bytes memory options = gasOptions(sendParams.config.minGas[i]);

            _lzSend(eid, _getPathOrRevert(eid), message, payable(address(this)), sendParams.zroPaymentAddress, options);
        }
    }

    function _sendSnapshotTransferAndTrades(uint256 kRedeemed, SendParams calldata sendParams) internal {
        for (uint256 i; i < sendParams.batches.length; ++i) {
            bytes memory path = _getPathOrRevert(sendParams.config.eIds.at(i));

            uint256 length = sendParams.batches[i].callbacks.length;
            for (uint256 j; j < length; ++j) {
                bytes memory message = j == 0
                    ? abi.encode(TRANSFER_AND_TRADE_SNAPSHOT, msg.sender, kRedeemed, sendParams.batches[i].callbacks[j])
                    : abi.encode(TRADE_SNAPSHOT, msg.sender, sendParams.batches[i].callbacks[j]);
                ...
                _lzSend(
                    ...
                    airdropOptions(
                        sendParams.config.minGas[i] + sendParams.batches[i].additionalGas[j],
                        j == length - 1 ? sendParams.batches[i].airdropAmount : 0,
    ...
    /// @dev version 1 options
    function gasOptions(uint256 gas) internal pure returns (bytes memory) {
        return abi.encodePacked(uint16(1), gas);
    }

    /// @dev version 2 options
    function airdropOptions(uint256 gas, uint256 dstNative, address dstAddress) internal pure returns (bytes memory) {
        return abi.encodePacked(uint16(2), gas, dstNative, dstAddress);
    }

    function validateOptions(bytes memory options, uint256 minGasAmountForChain) internal pure {
    ...
    IPhutureOnConsumeCallback
{
    ...
    function setHomeEid(uint16 _homeEid) external onlyOwner {
        homeEid = _homeEid;
    }
...
abstract contract BlockingApp is IBlockingApp, Owned {
    ...
    // empty slot for stateOverrides during gas estimation
