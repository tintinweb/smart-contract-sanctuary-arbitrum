...
contract Distributor is Initializable {
    ...
    IERC20Upgradeable public disputeToken;
    ...
    modifier onlyTrustedOrUser(address user) {
        if (user != msg.sender && canUpdateMerkleRoot[msg.sender] != 1 && msg.sender != guardian)
            revert NotTrusted();
        _;
    }
    ...
    function claim(
        ...
        for (uint256 i; i < usersLength; ) {
            ...
            claimed[user][token] = Claim(SafeCastUpgradeable.toUint208(amount), uint48(block.timestamp), getMerkleRoot());
    ...
    function toggleTrusted(address eoa) external onlyGuardian {
    ...
    function disputeTree(string memory reason) external {
        ...
        IERC20Upgradeable(disputeToken).safeTransferFrom(msg.sender, address(this), disputeAmount);
        disputer = msg.sender;
    ...
    function resolveDispute(bool valid) external onlyGuardian {
        if (disputer == address(0)) revert NoDispute();
        if (valid) {
            IERC20Upgradeable(disputeToken).safeTransfer(disputer, disputeAmount);
            // If a dispute is valid, the contract falls back to the last tree that was updated
            _revokeTree();
        } else {
            IERC20Upgradeable(disputeToken).safeTransfer(msg.sender, disputeAmount);
    ...
    /// immediately
    function revokeTree() external onlyGuardian {
        if (disputer != address(0)) revert UnresolvedDispute();
        _revokeTree();
    }
    ...
    function recoverERC20(address tokenAddress, address to, uint256 amountToRecover) external onlyGuardian {
    ...
    function setDisputePeriod(uint48 _disputePeriod) external onlyGuardian {
    ...
    function setDisputeToken(IERC20Upgradeable _disputeToken) external onlyGuardian {
        if (disputer != address(0)) revert UnresolvedDispute();
    ...
    function setDisputeAmount(uint256 _disputeAmount) external onlyGuardian {
        if (disputer != address(0)) revert UnresolvedDispute();
    ...
    function isGuardian(address _user) internal returns (bool) {
        return guardian == _user;
    }
}
