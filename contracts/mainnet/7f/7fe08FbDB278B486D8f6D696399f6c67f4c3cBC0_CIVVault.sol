...
contract CIVVault is Ownable, ReentrancyGuard {
    ...
    event TransferFailed(uint indexed strategyId, uint indexed epoch, address indexed investor, uint256 dueShares, bytes errorData);
    ...
    function _distributeShares(uint _id) internal {
        ...
        for (uint i = lastProcessedIndex; i < loopLimit; i++) {
            ...
            uint depositInfo = _userInfoEpoch[_id][investor][lastProcessedEpoch]
                .depositInfo;
            uint netDepositInfo = depositInfo * (feeBase - epoch.entryFee) / feeBase;
            uint dueShares = (sharesToDistribute * netDepositInfo) /
                epoch.totDepositedAssets;

            // Transfer shares if dueShares is greater than 0
            if (dueShares > 0) {
                // Use low-level call to handle potential errors
                (bool success, bytes memory data) = address(
                    _strategyInfo[_id].fundRepresentToken
                ).call(
