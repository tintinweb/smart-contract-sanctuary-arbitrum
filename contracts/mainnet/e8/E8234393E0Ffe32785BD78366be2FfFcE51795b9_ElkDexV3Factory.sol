...
import './ElkDexV3PoolDeployer.sol';
import './NoDelegateCall.sol';

import './ElkDexV3Pool.sol';

/// @title Canonical ElkDex V3 factory
/// @notice Deploys ElkDex V3 pools and manages ownership and control over pool protocol fees
contract ElkDexV3Factory is IElkDexV3Factory, ElkDexV3PoolDeployer, NoDelegateCall {
    /// @inheritdoc IElkDexV3Factory
...
contract ElkDexV3Pool is IElkDexV3Pool, NoDelegateCall {
    ...
    modifier onlyFactoryOwner() {
        require(msg.sender == IElkDexV3Factory(factory).owner());
        _;
    }

    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = IElkDexV3PoolDeployer(msg.sender).parameters();
...
contract ElkDexV3PoolDeployer is IElkDexV3PoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    /// @inheritdoc IElkDexV3PoolDeployer
    ...
    function deploy(
        ...
        pool = address(new ElkDexV3Pool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}
...
/// @title Callback for IElkDexV3PoolActions#flash
/// @notice Any contract that calls IElkDexV3PoolActions#flash must implement this interface
interface IElkDexV3FlashCallback {
    /// @notice Called to `msg.sender` after transferring to the recipient from IElkDexV3Pool#flash.
    ...
    /// @param data Any data passed through by the caller via the IElkDexV3PoolActions#flash call
    function elkDexV3FlashCallback(
        uint256 fee0,
        uint256 fee1,
        bytes calldata data
    ) external;
...
/// @title Callback for IElkDexV3PoolActions#mint
/// @notice Any contract that calls IElkDexV3PoolActions#mint must implement this interface
interface IElkDexV3MintCallback {
    /// @notice Called to `msg.sender` after minting liquidity to a position from IElkDexV3Pool#mint.
    /// @dev In the implementation you must pay the pool tokens owed for the minted liquidity.
    /// The caller of this method must be checked to be a ElkDexV3Pool deployed by the canonical ElkDexV3Factory.
    /// @param amount0Owed The amount of token0 due to the pool for the minted liquidity
    /// @param amount1Owed The amount of token1 due to the pool for the minted liquidity
    /// @param data Any data passed through by the caller via the IElkDexV3PoolActions#mint call
    function elkDexV3MintCallback(
        uint256 amount0Owed,
        uint256 amount1Owed,
        bytes calldata data
    ) external;
...
/// @title An interface for a contract that is capable of deploying ElkDex V3 Pools
/// @notice A contract that constructs a pool must implement this to pass arguments to the pool
/// @dev This is used to avoid having constructor arguments in the pool contract, which results in the init code hash
/// of the pool being constant allowing the CREATE2 address of the pool to be cheaply computed on-chain
interface IElkDexV3PoolDeployer {
...
/// @title Minimal ERC20 interface for ElkDex
/// @notice Contains a subset of the full ERC20 interface that is used in ElkDex V3
