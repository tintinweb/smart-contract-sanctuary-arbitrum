...
interface IReservesDistributor {
	function ibex() external view returns (address);
	function xIbex() external view returns (address);
...
contract ReservesDistributor is IReservesDistributor, Ownable {
	...
	function claim() external returns (uint amount) {
		...
		uint balance = IERC20(ibex).balanceOf(address(this));
		...
		if (amount > 0) {
			IERC20(ibex).transfer(xIbex, amount);
			IPoolToken(xIbex).sync();
		}
	...
	function withdraw(uint amount) external onlyOwner {
		...
		IERC20(ibex).transfer(msg.sender, amount);
