...
contract StrategyInfoQuerier {
    ...
    /// @dev Uniswap-Transaction-related Variable
    function getTransactionDeadlineDuration(address _strategyContract) external view returns (uint256) {
        return IStrategyInfo(_strategyContract).transactionDeadlineDuration();
    }

    /// @dev get Liquidity-NFT-related Variable
    function getLiquidityNftId(address _strategyContract) public view returns (uint256) {
        return IStrategyInfo(_strategyContract).liquidityNftId();
    }

    function getTickSpacing(address _strategyContract) external view returns (int24) {
        return IStrategyInfo(_strategyContract).tickSpacing();
    }

    /// @dev get Pool-related Variable
    function getPoolAddress(address _strategyContract) public view returns (address) {
        return IStrategyInfo(_strategyContract).poolAddress();
    }

    function getPoolFee(address _strategyContract) external view returns (uint24) {
        return IStrategyInfo(_strategyContract).poolFee();
    }

    function getToken0Address(address _strategyContract) external view returns (address) {
        return IStrategyInfo(_strategyContract).token0Address();
    }

    function getToken1Address(address _strategyContract) external view returns (address) {
        return IStrategyInfo(_strategyContract).token1Address();
    }

    /// @dev get Tracker-Token-related Variable
    function getTrackerTokenAddress(address _strategyContract) external view returns (address) {
        return IStrategyInfo(_strategyContract).trackerTokenAddress();
    }

    /// @dev get User-Management-related Variable
    function getIsInUserList(address _strategyContract, address _userAddress) external view returns (bool) {
        return IStrategyInfo(_strategyContract).isInUserList(_userAddress);
    }

    function getUserIndex(address _strategyContract, address _userAddress) external view returns (uint256) {
        return IStrategyInfo(_strategyContract).userIndex(_userAddress);
    }

    function getAllUsersInUserList(address _strategyContract) external view returns (address[] memory userList) {
    ...
    function getTotalUserShare(address _strategyContract) public view returns (uint256) {
        return IStrategyInfo(_strategyContract).totalUserShare();
    }

    /// @dev get Reward-Management-related Variable
    function getRewardToken0Amount(address _strategyContract) external view returns (uint256) {
        return IStrategyInfo(_strategyContract).rewardToken0Amount();
    }

    function getRewardToken1Amount(address _strategyContract) external view returns (uint256) {
        return IStrategyInfo(_strategyContract).rewardToken1Amount();
    }

    function getDistributeRewardAmount(address _strategyContract) external view returns (uint256) {
        return IStrategyInfo(_strategyContract).distributeRewardAmount();
    }

    /// @dev get User-Reward-Management-related Variable
    function getUserDistributeReward(address _strategyContract, address _userAddress)
        external
        view
        returns (uint256 userWETHReward)
    {
        return IStrategyInfo(_strategyContract).userDistributeReward(_userAddress);
    }

    function getTotalUserDistributeReward(address _strategyContract) external view returns (uint256) {
        return IStrategyInfo(_strategyContract).totalUserDistributeReward();
    }

    /// @dev get Buyback-related Variable
    function getBuyBackToken(address _strategyContract) external view returns (address) {
        return IStrategyInfo(_strategyContract).buyBackToken();
    }

    function getBuyBackNumerator(address _strategyContract) external view returns (uint24) {
        return IStrategyInfo(_strategyContract).buyBackNumerator();
    }

    /// @dev get Fund-Manager-related Variable
    function getAllFundManagerVaults(address _strategyContract)
        external
        view
    ...
    /// @dev get Earn-Loop-Control-related Variable
    function getEarnLoopSegmentSize(address _strategyContract) external view returns (uint256) {
        return IStrategyInfo(_strategyContract).earnLoopSegmentSize();
    }

    function getEarnLoopDistributedAmount(address _strategyContract) external view returns (uint256) {
        return IStrategyInfo(_strategyContract).earnLoopDistributedAmount();
    }

    function getEarnLoopStartIndex(address _strategyContract) external view returns (uint256) {
        return IStrategyInfo(_strategyContract).earnLoopStartIndex();
    }

    function getIsEarning(address _strategyContract) external view returns (bool) {
        return IStrategyInfo(_strategyContract).isEarning();
    }

    /// @dev get Rescale-related Variable
    function getDustToken0Amount(address _strategyContract) external view returns (uint256) {
        return IStrategyInfo(_strategyContract).dustToken0Amount();
    }

    function getDustToken1Amount(address _strategyContract) external view returns (uint256) {
        return IStrategyInfo(_strategyContract).dustToken1Amount();
    }

    /// @dev get Constant Variable
    function getBuyBackDenominator(address _strategyContract) external pure returns (uint24) {
        return IStrategyInfo(_strategyContract).getBuyBackDenominator();
    }

    function getFundManagerProfitVaultDenominator(address _strategyContract) external pure returns (uint24) {
        return IStrategyInfo(_strategyContract).getFundManagerProfitVaultDenominator();
    }

    function getFarmAddress(address _strategyContract) external view returns (address) {
        return IStrategyInfo(_strategyContract).getFarmAddress();
    }

    function getControllerAddress(address _strategyContract) external view returns (address) {
        return IStrategyInfo(_strategyContract).getControllerAddress();
    }

    function getSwapAmountCalculatorAddress(address _strategyContract) external view returns (address) {
        return IStrategyInfo(_strategyContract).getSwapAmountCalculatorAddress();
    }

    function getZapAddress(address _strategyContract) external view returns (address) {
        return IStrategyInfo(_strategyContract).getZapAddress();
    }

    /// @dev get tick info
    function getTickAndPrice(address _strategyContract) external view returns (int24, uint256) {
        // get poolAddress
        ...
        uint256 tokenPriceWithDecimals = getTokenPriceWithDecimalsByPoolAndTick(poolAddress, tick);

        return (tick, tokenPriceWithDecimals);
    }

    function getTickLowerAndPrice(address _strategyContract) external view returns (int24, uint256) {
        // get poolAddress
        address poolAddress = getPoolAddress(_strategyContract);
        ...
        // calculate tokenPrice
        uint256 tokenPriceWithDecimals = getTokenPriceWithDecimalsByPoolAndTick(poolAddress, tickLower);

        return (tickLower, tokenPriceWithDecimals);
    }

    function getTickUpperAndPrice(address _strategyContract) external view returns (int24, uint256) {
        // get poolAddress
        address poolAddress = getPoolAddress(_strategyContract);
        ...
        // calculate tokenPrice
        uint256 tokenPriceWithDecimals = getTokenPriceWithDecimalsByPoolAndTick(poolAddress, tickUpper);
    ...
    tokenPriceWithDecimalAdj = tokenPrice * (10^decimalPrecision)
        = (sqrtPriceX96 * (10^decimalPrecision) / (2^96))^2 
            / 10^(decimalPrecision + decimal1 - decimal0)
    */
    function getTokenPriceWithDecimalsByPoolAndTick(address poolAddress, int24 tick)
        internal
        view
        returns (uint256 tokenPriceWithDecimals)
    {
        (,,,,, uint256 decimal0, uint256 decimal1) = PoolHelper.getPoolInfo(poolAddress);
    ...
    function getUserLiquidityTokenBalance(address _strategyContract, address _userAddress)
        external
        view
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        uint256 userLiquidity = uint256(liquidity).mul(userShare).div(totalShare);

        // calculate token amount
        (amount0, amount1) =
            LiquidityAmounts.getAmountsForLiquidity(sqrtPriceX96, sqrtRatioAX96, sqrtRatioBX96, uint128(userLiquidity));
    }

    function getStrategyLiquidityTokenBalance(address _strategyContract)
        external
        view
        returns (uint256 amount0, uint256 amount1)
    {
    ...
    function getSqrtPriceAndLiquidityInfo(address _strategyContract)
        internal
        view
    ...
    function getLiquidity(address _strategyContract) public view returns (uint128 liquidity) {
        (,,, liquidity) = getSqrtPriceAndLiquidityInfo(_strategyContract);
    }

    /// @dev get ratio for share and liquidity : share / liquidity

    function getRationForShareAndLiquidity(address _strategyContract) external view returns (uint256) {
        uint256 totalShare = getTotalUserShare(_strategyContract);
        uint128 liquidity = getLiquidity(_strategyContract);
        return totalShare.div(uint256(liquidity));
    }

    function verifyLiquidityNftIdIsNotZero(uint256 liquidityNftId) internal pure {
