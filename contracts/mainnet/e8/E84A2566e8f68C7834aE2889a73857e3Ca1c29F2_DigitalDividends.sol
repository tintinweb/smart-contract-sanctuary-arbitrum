...
contract DigitalDividends is Ownable{

    com_Contract public com;
    IBEP20 public USDTToken;
    IERC20 public DDTUSDToken;
    IERC20 public DDTToken;
    ...
    struct UserInfo {
        ...
        uint256 DDTUSD_1stLCommission;
        uint256 USDT_Com_FromBuyer;
        uint256 DDTUSD_Com_FromBuyer;
        uint256 USDT_Com_FromSeller;
        uint256 DDTUSD_Com_FromSeller;
    ...
    mapping(address => uint256 ) public totalDDTUSDSpent;
    mapping(address => uint256 ) public TotalUSDTEarned;
    mapping(address => uint256 ) public TotalDDTUSDEarned;
    mapping(uint256 => uint256 ) public TotalTokenInRound;
    mapping(address => uint256 ) public totalUSDTCommission;
    mapping(address => uint256 ) public totalDDTUSDCommission;
    ...
    mapping(address => mapping(uint256 => uint256)) public withdrawHistoryOfDDTUSD;
    ...
    constructor()
    {
        com = com_Contract(0xAB199CB191C49677db808EbB3c1FEac74BAE550c);
        USDTToken = IBEP20(0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9);
        DDTUSDToken = IERC20(0x9887834D57A6C21D65Ea0c545DE34e669751fF5f);
        DDTToken = IERC20(0xe807773Ca16A608A4D370aC3928d275F1119Ca7C);
        defaultRefer = 0x2e1015Ee2536e1304E94D9848180dcbbda66F47d;
        anotheraddress = 0x2e1015Ee2536e1304E94D9848180dcbbda66F47d;
        round = 0;
        cycle[0] = 0;
    }
    ...
    public
    {
        ...
        require(IBEP20(tokenAddress) == USDTToken || IERC20(tokenAddress) == DDTUSDToken,"Invalid token address");
        UserInfo storage user = userInfo[msg.sender];
        ...
        if(roundbool == false)
        {
            ...
            { 
                ...
                if(IBEP20(tokenAddress) == USDTToken)
                {
                    ...
                    DDTToken.transferFrom(owner(),address(this),tokentransfer);
                }
                else if(IERC20(tokenAddress) == DDTUSDToken)
                {
                    totalDDTUSDSpent[msg.sender] = totalDDTUSDSpent[msg.sender]+(Rem_Amount1);
                    DDTUSDToken.transferFrom(msg.sender, address(this), Rem_Amount1);
                    DDTToken.transferFrom(owner(),address(this),tokentransfer);
                    DDTUSDToken.burn2(address(this),Rem_Amount1);
                }
            } 
            if(roundDetails[2] > 1)
            {
                ...
                remainingbuyerToken = 0;
                    ...
                    if(seller_Count[PreviousRound] >= buyer_Count[PreviousRound]){
                        ...
                        if(PreviousRound > endRound){
                            ...
                            {
                            DDTToken.minting(address(this),totalTokenMint);
                            totalTokenMint = 0;
    ...
    private{
        ...
        if(_totalamount > 0)
        {
            ...
            }else if(IERC20(_tokenAddress) == DDTUSDToken)
            {
                totalDDTUSDSpent[msg.sender] = totalDDTUSDSpent[msg.sender]+(_totalamount);
                DDTUSDToken.transferFrom(msg.sender, address(this), _totalamount);
                DDTUSDToken.burn2(address(this),_totalamount);
    ...
    private{
        ...
        info.DDTUSD_Commission = info.DDTUSD_Commission+(per_20);
    }
    ...
    private{
        ...
        userInfo[upline].DDTUSD_1stLCommission = userInfo[upline].DDTUSD_1stLCommission+(per_20);
        for(uint256 count= 0; count < referDepth; count++){
            if(upline != address(0))
            {
                ...
                userInfo[upline].DDTUSD_Com_FromBuyer = userInfo[upline].DDTUSD_Com_FromBuyer+(per20);
                if(upline == defaultRefer) break;
                upline = userInfo[upline].referrer;
            }else{
                break;
    ...
    private{
        ...
        for(uint256 count= 0; count < referDepth; count++){
            if(upline != address(0))
            {
                ...
                userInfo[upline].DDTUSD_Com_FromSeller = userInfo[upline].DDTUSD_Com_FromSeller+(per20);
                if(upline == defaultRefer) break;
                upline = userInfo[upline].referrer;
            }else{
                break;
    ...
    {
        uint256 values = DDTUSDToken.balanceOf(to);
        return values;
    }
    function totalEarned(address _address) 
    public view returns(uint256 _TotalUSDTEarned,uint256 _TotalDDTUSDEarned)
    {
        (uint256 _totalUSDT,uint256 _totalDDTUSD ) = TotalClaimed(_address);
        _TotalUSDTEarned = _totalUSDT+(TotalUSDTEarned[_address]);
        _TotalDDTUSDEarned = _totalDDTUSD+(TotalDDTUSDEarned[_address]);
        return (_TotalUSDTEarned,_TotalDDTUSDEarned);
    }
    ...
    public view returns(uint256,uint256)
    {
        ...
        totalDDTUSD = totalDDTUSD+(user.DDTUSD_1stLCommission)+(user.DDTUSD_Com_FromBuyer)+(user.DDTUSD_Com_FromSeller);

        if(_address == owner())
        {
            CommissionInfo storage info = commissionInfo[_address];
            totalUSDT = totalUSDT+(info.USDT_Commission);
            totalDDTUSD = totalDDTUSD+(info.DDTUSD_Commission);
        }
    return (totalUSDT, totalDDTUSD);
    }
    ...
    public{
        ...
        require(totalUSDT > 0 && totalDDTUSD > 0, "Balance less than zero");
        ...
        totalDDTUSDCommission[msg.sender] += totalDDTUSD;

        USDTToken.transfer(msg.sender,totalUSDT);
        DDTUSDToken.mint2(msg.sender);

        TotalUSDTEarned[msg.sender] += totalUSDT;
        TotalDDTUSDEarned[msg.sender] += totalDDTUSD;

        withdrawHistoryOfUSDT[msg.sender][w_count[msg.sender]] = totalUSDT;
        withdrawHistoryOfDDTUSD[msg.sender][w_count[msg.sender]] = totalDDTUSD;
        ...
        user.DDTUSD_1stLCommission = 0;
        user.DDTUSD_Com_FromBuyer = 0; 
        user.DDTUSD_Com_FromSeller = 0;
        info.USDT_Commission = 0;
        info.DDTUSD_Commission = 0;
    }
    ...
    function userWithdrawDDTToken() 
    public{
        address _user = msg.sender;
        require(getETAWithdraw(_user) > 0, "Sorry!, the amount is less than zero");
        DDTToken.transfer(msg.sender, getETAWithdraw(_user));
