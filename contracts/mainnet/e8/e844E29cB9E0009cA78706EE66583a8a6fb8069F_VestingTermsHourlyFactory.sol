...
import {VestingTermsHourly} from "./VestingTermsHourly.sol";
import {AccessHelper} from "../utils/AccessHelper.sol";

import {ADMIN_ROLE} from "../utils/constants.sol";

contract VestingTermsHourlyFactory is AccessHelper {
    mapping(bytes32 => address) public vestingTermsAddresses;

    struct VestingTermsParameters {
        string featuredProjectName;
        string currentFundingRoundName;
    ...
    event VestingTermsCreation(
        bytes32 indexed fundingTermsId,
        address _tokenForSaleAddress,
        uint256 _vestingPeriod,
        uint8 _startVestingPercentage,
        address _marketAddress,
        address indexed newVestingTermsAddress,
        address _treasuryAddress
    );
    ...
    /**
     * @notice Get `vestingTermsId` unique double which identifies a instance.
     * @dev `abi.encodePacked` is effectively same as `abi.encode` for these full word (bytes32, uint256) arg types.
     * @return vestingTermsId bytes32 hash.
     **/
    function getVestingTermsId(
    ...
    /**
     * @notice New {VestingTerms} creation.
     **/
    function createVestingTerms(
        VestingTermsParameters memory parameters
    ) external onlyAuthorized(ADMIN_ROLE) {
        bytes32 custodyVestingTermsId = getVestingTermsId(
            parameters.featuredProjectName,
            parameters.currentFundingRoundName
        );

        require(
            vestingTermsAddresses[custodyVestingTermsId] == address(0),
            "vesting terms configuration instance exists"
        );

        address newVestingTermsAddress = address(
            new VestingTermsHourly{salt: custodyVestingTermsId}(
                msg.sender,
                parameters._tokenForSaleAddress,
                parameters._vestingPeriod,
                parameters._startVestingPercentage,
                parameters._marketAddress,
                parameters._treasuryAddress
            )
        );

        vestingTermsAddresses[custodyVestingTermsId] = newVestingTermsAddress;

        emit VestingTermsCreation(
            custodyVestingTermsId,
            parameters._tokenForSaleAddress,
            parameters._vestingPeriod,
            parameters._startVestingPercentage,
            parameters._marketAddress,
            newVestingTermsAddress,
            parameters._treasuryAddress
        );
    }
}
...
contract VestingTermsHourly is AccessHelper, ReentrancyGuard {
    ...
    uint256 public immutable vestingHours;
    uint8 public immutable startVestingPercentage;
    uint256 public vestingStartTime;

    uint256 public constant VESTING_STEP = 1 hours;
    ...
    constructor(
        ...
        require(_vestingDays != 0, "Vesting period cannot be 0");
        require(_treasuryAddress != address(0), "Treasury Address cannot be 0");

        tokenForSaleAddress = _tokenForSaleAddress;
        vestingPeriod = _vestingDays * 24 * 60 * 60;
        vestingHours = _vestingDays * 24;
