...
contract DefiEdgeStrategy is UniswapV3LiquidityManager {
    ...
    function mint(
        ...
        uint256 _amount1Min,
        uint256 _minShare
    ) external nonReentrant onlyValidStrategy returns (uint256 amount0, uint256 amount1, uint256 share) {
    ...
    function rebalance(
        ...
        if (_swapData.length > 0) {
            _swap(zeroToOne, amountIn, _swapData);
        }
    ...
    function emergencyWithdraw(address _token, address _to, uint256 _amount, NewTick[] calldata _newTicks) external {
        ...
        if (_newTicks.length > 0) {
            for (uint256 tickIndex = 0; tickIndex < _newTicks.length; tickIndex++) {
                ...
                (uint256 currentLiquidity, , , , ) = pool.positions(PositionKey.compute(address(this), tick.tickLower, tick.tickUpper));
                pool.burn(tick.tickLower, tick.tickUpper, currentLiquidity.toUint128());
...
contract UniswapV3LiquidityManager is StrategyBase, ReentrancyGuard, IAlgebraMintCallback {
    ...
    ) internal onlyHasDeviation returns (uint256 tokensBurned0, uint256 tokensBurned1, uint256 fee0, uint256 fee1) {
        uint256 collect0;
        uint256 collect1;

        if (_shares > 0) {
            uint256 currentLiquidity256;
            (currentLiquidity256, , , , ) = pool.positions(PositionKey.compute(address(this), _tickLower, _tickUpper));
            if (currentLiquidity256 > 0) {
                uint256 liquidity = FullMath.mulDiv(currentLiquidity256, _shares, totalSupply());
    ...
    function addPerformanceFees(uint256 _fee0, uint256 _fee1) internal {
        ...
        uint256 _protocolPerformanceFeeRateByStrategy = factory.protocolPerformanceFeeRateByStrategy(address(this));
        uint256 _protocolPerformanceFeeRateByPool = factory.protocolPerformanceFeeRateByPool(address(pool));

        if (_protocolPerformanceFeeRateByStrategy > 0) {
            _protocolPerformanceFee = _protocolPerformanceFeeRateByStrategy;
        } else if (_protocolPerformanceFeeRateByPool > 0) {
            _protocolPerformanceFee = _protocolPerformanceFeeRateByPool;
        } else {
            _protocolPerformanceFee = factory.defaultProtocolPerformanceFeeRate();
        }
    ...
    function burnAllLiquidity() internal {
        for (uint256 _tickIndex = 0; _tickIndex < ticks.length; _tickIndex++) {
            ...
            if (currentLiquidity > 0) {
                burnLiquidity(tick.tickLower, tick.tickUpper, 0, currentLiquidity.toUint128());
            }
        }
    }
    ...
    function burnLiquiditySingle(
        uint256 _tickIndex
    ) public nonReentrant onlyHasDeviation returns (uint256 amount0, uint256 amount1, uint256 fee0, uint256 fee1) {
    ...
    function _burnLiquiditySingle(uint256 _tickIndex) internal returns (uint256 amount0, uint256 amount1, uint256 fee0, uint256 fee1) {
        ...
        if (currentLiquidity > 0) {
            (amount0, amount1, fee0, fee1) = burnLiquidity(tick.tickLower, tick.tickUpper, 0, currentLiquidity.toUint128());
        }
    }
     ...
     */
    function swap(bool zeroToOne, uint256 amountIn, bytes calldata data) public onlyOperator onlyValidStrategy nonReentrant {
        _swap(zeroToOne, amountIn, data);
    }
     ...
     * @param data Swap data to perform exchange from 1inch
     */
    function _swap(bool zeroToOne, uint256 amountIn, bytes calldata data) internal onlyHasDeviation {
