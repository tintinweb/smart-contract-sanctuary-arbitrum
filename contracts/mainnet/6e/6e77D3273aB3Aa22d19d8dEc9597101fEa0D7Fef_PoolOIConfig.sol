...
contract PoolOIConfig is Ownable {
    uint256 public _maxPoolOI; //Percentage of pool available balance
    IPoolOIStorage public _poolOIStorage;
    ILiquidityPool public _pool;

    constructor(
        uint256 maxPoolOI,
        IPoolOIStorage poolOIStorage,
        ILiquidityPool pool
    ) {
    ...
    function getMaxPoolOI() external view returns (uint256) {
        ...
        if (currentPoolOI >= (_maxPoolOI * _pool.availableBalance()) / 1e4) {
            return 0;
        } else {
            return
                ((_maxPoolOI * _pool.availableBalance()) / 1e4) - currentPoolOI;
        }
    }

    function getPoolOICap() external view returns (uint256) {
        return (_maxPoolOI * _pool.availableBalance()) / 1e4;
    }
}
