...
contract VeDist is IVeDist, Initializable/* , UUPSUpgradeable, OwnableUpgradeable */ {
    ...
    function initialize(address _votingEscrow) public initializer {
        ...
        uint256 _t = _timestampToRoundedEpoch(block.timestamp);
        startTime = _t;
        lastTokenTime = _t;
        timeCursor = _t;
        ...
        IERC20Upgradeable(_token).safeIncreaseAllowance(_votingEscrow, type(uint256).max);
    }
        
    function timestamp() external view returns (uint256) {
        return _timestampToRoundedEpoch(block.timestamp);
    }
    ...
    function _checkpointToken() internal {
        ...
        uint256 thisWeek = _timestampToRoundedEpoch(t);
        uint256 nextWeek = 0;
    ...
    function veForAt(uint256 _tokenId, uint256 _timestamp) external view returns (uint256) {
        ...
        return uint256(int256(Math.positiveInt128(pt.bias - pt.slope * (int128(int256(_timestamp - pt.ts))))));
    }

    function _checkpointTotalSupply() internal {
        address ve = votingEscrow;
        uint256 t = timeCursor;
        uint256 roundedTimestamp = _timestampToRoundedEpoch(block.timestamp);
        IVe(ve).checkpoint();
    ...
    function _calculateClaim(
        ...
        if (weekCursor == 0) {
            weekCursor = _timestampToRoundedEpoch(userPoint.ts + WEEK - 1);
        }
        ...
        IVe.Point memory oldUserPoint;
        {
            for (uint256 i = 0; i < 50; i++) {
                ...
                if (weekCursor >= userPoint.ts && userEpoch <= maxUserEpoch) {
                    ...
                    uint256 balanceOf = uint256(int256(Math.positiveInt128(oldUserPoint.bias - dt * oldUserPoint.slope)));
    ...
    function claimable(uint256 _tokenId) external view returns (uint256) {
        uint256 _lastTokenTime = _timestampToRoundedEpoch(lastTokenTime);
    ...
    function claim(uint256 _tokenId) external returns (uint256) {
        ...
        _lastTokenTime = _timestampToRoundedEpoch(_lastTokenTime);
