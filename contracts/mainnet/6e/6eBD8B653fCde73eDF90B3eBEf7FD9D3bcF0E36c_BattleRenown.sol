...
contract BattleRenown is IBattleRenown, ManagerModifier {
  ...
  mapping(address => mapping(uint256 => DiminishingReturns))
    public opponentDiminishingReturns;

  IRenown public immutable RENOWN;

  int public immutable K_BASELINE = 45000;
  ...
  function _updateRenown(
    ...
    for (uint i = 0; i < fightResult.rounds; i++) {
      ...
      uint opponentRenownBaseLevel = fightResult.opponentLevel <=
        fightResult.adventurerLevel
  ...
  function _opponentMultiplier(
    ...
    DiminishingReturns storage opponentBattles = opponentDiminishingReturns[
      _opponentAddress
    ][_opponentTokenId];

    if (
      uint256(opponentBattles.epochHash) != (_battleEpoch % type(uint16).max)
    ) {
    ...
    uint counter = _battleResult
      ? opponentBattles.winCounter++
      : opponentBattles.lossCounter++;
    if (counter > 20) {
      counter = 20;
    }
  ...
  function _calculateRenownChange(
    ...
    // Halve renown gains/losses when out of range
    if (
      _adventurerRenown > (_opponentRenown * 2) ||
      _opponentRenown > (_adventurerRenown * 2)
    ) {
      calcBase = calcBase / 2;
    }
    ...
    delta =
      (calcBase *
        (delta -
          calculateRenownWinProbability(_adventurerRenown, _opponentRenown))) /
      SIGNED_ONE_HUNDRED;
  }
  ...
  function calculateRenownWinProbability(
    ...
    result +=
      (SIGNED_ONE_HUNDRED * (_attackerRenown)) /
      (_attackerRenown + _defenderRenown);
    _attackerRenown =
      (_attackerRenown * _attackerRenown) /
      SIGNED_DECIMAL_POINT;
    _defenderRenown =
      (_defenderRenown * _defenderRenown) /
      SIGNED_DECIMAL_POINT;
    result +=
      5000 +
      (int(90000) * (_attackerRenown)) /
      (_attackerRenown + _defenderRenown);
    result /= 2;
    return result;
  }
