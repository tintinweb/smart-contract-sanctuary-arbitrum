...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function grantTUK(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = true;
        }
    }

    function proceedTUK(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
    ...
    function isTUK(address _rewardee_) public view returns (bool) {
...
contract Tuk is ERC20 {
    constructor() ERC20("TUK", "TUK") {
