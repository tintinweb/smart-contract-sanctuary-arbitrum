/**
 *Submitted for verification at Arbiscan on 2022-10-05
*/
...
contract MegaArb {
    address payable constant gohmArb = 0xEDCE1C21ad6cb55D647671682b35766EA4a440f1;
    address payable constant btcArb = 0x951E1d6936DeEF30D2d28a8F2393aecE7361e5B9;
    address payable constant dpxArb = 0x0ec6aB4AD7Ff136B9020E105f78fCc25a1997F6a;

    address constant REDEEMER = 0xf9A0e641c98f964b1c732661FaB9D5B96af28D49;

    address payable constant WALLET = 0xb69Dd1e9Fe391E1F36b01F00bb6a9d9303EE3E13;

    function run(uint qty, address payable arb, address redeemer, uint minProfit) payable public {
        GOHMArb(arb).swap{value:msg.value}(qty, redeemer);
        require(address(this).balance >= minProfit);
        WALLET.transfer(address(this).balance);
    }

    function arbGOHM(uint qty, uint minProfit) public {
        run(qty, gohmArb, REDEEMER, minProfit);        
    }

    function arbDPX(uint qty, uint minProfit) public {
        run(qty, dpxArb, REDEEMER, minProfit);        
    }

    function arbBTC(uint qty, uint minProfit) public {
        run(qty, btcArb, REDEEMER, minProfit);        
    }        
