...
contract DopexAdapter is IOption, UpgradeableOperableKeepable {
    ...
    // @notice Redeployment cheat not to have 
    ...
    function settle(SettleParams calldata params) public onlyOperator returns (uint256) {
        ...
        for (uint256 i = 0; i < strikeTokens.length;) {
            ...
            // Get how many options we bought at the strike, for one option strategy
            IOptionStrategy.Strike memory _optionStrike = _getBoughtStrike(msg.sender, params.currentEpoch, strike);
            uint256 strikeTokenBalance = _optionStrike.currentOptions;
            ...
            if (strikeTokenBalance > 0 && strikePnl > 0) {
                ...
                IOptionStrategy(msg.sender).settleOptionStrike(params.currentEpoch, IOption(thisAddress), strike);
            }
    ...
    function position(address _compoundStrategy) external view returns (uint256) {
        uint256 pnl_ = _pnl(msg.sender, _compoundStrategy);
    ...
    function _executeOptionsPurchase(
        ...
        for (uint256 i; i < _strikes.length; ++i) {
            ...
            if (_optionStrike.currentOptions == 0) {
                ...
                _optionStrike = _getBoughtStrike(msg.sender, _epoch, _strikes[i]);
            } else {
