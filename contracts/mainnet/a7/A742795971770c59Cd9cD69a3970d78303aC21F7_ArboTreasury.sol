...
contract ArboTreasury is Ownable {
  ...
  address public sARBO;
  uint public sARBOQueue; // Delays change to sARBO address

  uint public totalReserves; // Risk-free value of all assets
  uint public totalDebt;

  constructor(address _ARBO, address _USDC, uint _blocksNeededForQueue) {
  ...
  /**
        @notice allow approved address to deposit an asset for ARBO
        @param _amount uint
  ...
  function deposit(uint _amount, address _token, uint _profit) external returns (uint send_) {
    ...
    // mint ARBO needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(ARBO).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn ARBO for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint _amount, address _token) external {
    ...
    uint value = valueOf(_token, _amount);
    IARBOERC20(ARBO).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint _amount, address _token) external {
    ...
    uint maximumDebt = IERC20(sARBO).balanceOf(msg.sender); // Can only borrow against sARBO held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with ARBO
        @param _amount uint
     */
  function repayDebtWithARBO(uint _amount) external {
    require(isDebtor[msg.sender], "Not approved");

    IARBOERC20(ARBO).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, ARBO, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint _amount) external {
    ...
    IERC20Mintable(ARBO).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint) {
    return totalReserves.sub(IERC20(ARBO).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns ARBO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  function valueOf(address _token, uint _amount) public view returns (uint value_) {
    if (isReserveToken[_token]) {
      // convert amount to match ARBO decimals
      value_ = _amount.mul(10 ** IERC20(ARBO).decimals()).div(10 ** IERC20(_token).decimals());
  ...
  function queue(MANAGING _managing, address _address) external onlyManager returns (bool) {
    ...
    } else if (_managing == MANAGING.SARBO) {
      // 9
      sARBOQueue = block.number.add(blocksNeededForQueue);
    } else return false;
