...
contract LiquidityAdder is Ownable {
    ...
    function setLongTermPauser(
        address target,
        address pauser
    ) external onlyOwner {
        IArkenPairLongTerm(target).setPauser(pauser);
    }
        ...
        returns (address pair, uint256 liquidity, uint256 positionTokenId)
    {
        ...
        IERC20(tokenA).safeTransferFrom(msg.sender, pair, amountA);
        IERC20(tokenB).safeTransferFrom(msg.sender, pair, amountB);
        IArkenPairLongTerm(pair).unpause();
        (liquidity, positionTokenId) = IArkenPairLongTerm(pair).mint(
            msg.sender,
            lockTime
        );
    ...
    function swapLongTerm(
        ...
        (uint256 reserveIn, uint256 reserveOut) = ArkenLPLibrary.getReserves(
            factoryLongTerm,
            tokenIn,
            tokenOut
        );
        ...
        require(
            ...
            'LiquidityAdder: INSUFFICIENT_OUTPUT_AMOUNT'
        );
        IERC20(tokenIn).safeTransferFrom(msg.sender, pair, amountIn);
        uint256 balOutBefore = IERC20(tokenOut).balanceOf(msg.sender);
        IArkenPairLongTerm(pair).unpause();
        if (tokenOut == IArkenPairLongTerm(pair).token0()) {
            IArkenPairLongTerm(pair).swap(amountOut, 0, msg.sender, '');
        } else {
            IArkenPairLongTerm(pair).swap(0, amountOut, msg.sender, '');
        }
        IArkenPairLongTerm(pair).pause();
        IArkenPairLongTerm(pair).setPauser(msg.sender);
        uint256 balOutAfter = IERC20(tokenOut).balanceOf(msg.sender);
        require(
            balOutAfter - balOutBefore >= amountOutMin,
            'LiquidityAdder: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
