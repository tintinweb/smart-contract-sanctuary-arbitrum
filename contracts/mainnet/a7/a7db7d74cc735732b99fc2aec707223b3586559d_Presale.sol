...
contract Presale is Ownable {
    ...
    modifier duringPreSale() {
        require(!preSaleEnded, "Pre-sale has been ended.");
        _;
    }
   ...
   function buy() public payable returns(bool) {
       ...
       require(_tokenValue <= tokenForSale(), "Remaining token less than buying value");
       ...
       //tokenAddress.transferFrom(seller, _msgSender(), _tokenValue);
        ...
        // Store the tokens in the contract until the pre-sale ends
        tokensPurchased[msg.sender] += _tokenValue;

        // Add the buyer to the array if they haven't purchased before
        if (tokensPurchased[msg.sender] == _tokenValue) {
    ...
    function endPreSale() external onlyOwner {
        require(!preSaleEnded, "Pre-sale has already ended.");

        // Transfer the tokens to buyers
        for (uint256 i = 0; i < buyers.length; i++) {
            address buyer = buyers[i];
            uint256 tokensToTransfer = tokensPurchased[buyer];
            tokensPurchased[buyer] = 0; // Clear the buyer's token balance
            tokenAddress.transferFrom(seller, buyer, tokensToTransfer);
        }
