...
contract BondDepositoryStable is Ownable {
    ...
    address public immutable Token; // token given as payment for bond
    ...
    constructor(address _Token, address _principle, address _treasury, address _DAO, address _bondCalculator) {
     ...
     */
    function initializeBondTerms(uint _controlVariable, uint _minimumPrice, uint _maxPayout, uint _fee, uint _maxDebt, uint _initialDebt, uint32 _vestingTerm) external onlyPolicy {
        require(terms.controlVariable == 0, "Bonds must be initialized from 0");
        terms = Terms({controlVariable: _controlVariable, minimumPrice: _minimumPrice, maxPayout: _maxPayout, fee: _fee, maxDebt: _maxDebt, vestingTerm: _vestingTerm});
     ...
     *  @param _buffer uint
     */
    function setAdjustment(bool _addition, uint _increment, uint _target, uint32 _buffer) external onlyPolicy {
    ...
    function toggle() external onlyPolicy {
        available = !available;
    }
    ...
    function deposit(uint _amount, uint _maxPrice, address _depositor) external returns (uint) {
        require(available || msg.sender == _owner, "Bond not available yet");
    ...
    function redeem(address _recipient, bool _stake) external returns (uint) {
        ...
        if (percentVested >= 10000) {
            ...
            bondInfo[_recipient] = Bond({payout: info.payout.sub(payout), vesting: info.vesting.sub32(uint32(block.timestamp).sub32(info.lastTime)), lastTime: uint32(block.timestamp), pricePaid: info.pricePaid});
    ...
    function stakeOrSend(address _recipient, bool _stake, uint _amount) internal returns (uint) {
        if (!_stake) {
            ...
            if (useHelper) {
                ...
                IERC20(Token).approve(stakingHelper, _amount);
    ...
    function maxPayout() public view returns (uint) {
        return IERC20(Token).totalSupply().mul(terms.maxPayout).div(100000);
    }
