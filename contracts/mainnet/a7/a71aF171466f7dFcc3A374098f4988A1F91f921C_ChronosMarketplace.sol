...
interface IChronosMarketPlace {
    ...
    ///      auction maker can finish anytime.
    ...
    event PlaceBid(address bidder, uint256 auctionId, uint256 bidPrice);
    ...
    event AcceptOffer(uint256 offerId, address user);
...
interface IVoter {
    ...
    function isGauge(address _pair) external view returns (bool);
    ...
    function weights(address _pair) external view returns(uint);
    ...
    IChronosMarketPlace
{
    ...
    function listNftForFixed(
        ...
        require(allowedTokens[_paymentToken] || _paymentToken == address(0), Errors.INVALID_TOKEN);
    ...
    function buyNow(
        ...
        require(saleInfo.startTime <= currentTime && currentTime < saleInfo.endTime, Errors.NOT_SALE_PERIOD);
    ...
    function buyNowWithETH(
        ...
        require(msg.value >= saleInfo.price, "Low Eth Balance");
    ...
    function cancelListNftForAuction(
        ...
        if (auctionInfo.highestBidder != address(0)) {
            if(auctionInfo.paymentToken == address(0)) {
                payable(auctionInfo.highestBidder).transfer(auctionInfo.highestBidPrice);
            } else{
    ...
    function finishAuction(uint256 _auctionId) external override whenNotPaused {
        ...
        if (auctionInfo.highestBidder != address(0)) {
            ...
            if(auctionInfo.paymentToken == address(0)){
                payable(auctionInfo.seller).transfer(price - fee);
                payable(treasury).transfer(fee);
            } else {
                IERC20(auctionInfo.paymentToken).safeTransfer(auctionInfo.seller, price - fee);
    ...
    function makeOfferWithETH(
        ...
        require(msg.value >= _offerPrice && _offerPrice > 0, Errors.INVALID_PRICE);
