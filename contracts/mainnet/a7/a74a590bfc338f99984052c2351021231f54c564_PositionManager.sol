...
contract PositionManager is Initializable,OwnableUpgradeable,ReentrancyGuardUpgradeable,IPositionManager{
  ...
  function increasePosition(
    ...
    position.entryFundingRate = dipxStorage.cumulativeFundingRates(_indexToken, _collateralToken);
    position.fundingFactor = dipxStorage.currentFundingFactor(_account, _indexToken, _collateralToken, _isLong);
  ...
  function _collectFee(address _account, address _token, uint256 _amount) private{
    if(_amount>0){
      uint256 feeToLpAmount = dipxStorage.getLpPositionFee(_token, _amount);
      address referral = dipxStorage.referral();
      uint256 burnAmount = feeToLpAmount;
      IVault vault = IVault(dipxStorage.vault());
      if(referral != address(0)){
        uint256 rebateAmount = IReferral(referral).calculateRebateAmount(_account, feeToLpAmount);
        if(rebateAmount>0){
          burnAmount = burnAmount - rebateAmount;
