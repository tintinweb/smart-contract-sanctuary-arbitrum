...
contract ManagedTreasury is AccessControl,Pausable {
    ...
    function transferMarketingWallet(address to,uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        payable(to).transfer(amount);
    }
    ...
    function treasuryBuyBack(uint256 amountEth) external onlyCaller returns (uint256 amountToken) {
        if(!(amountEth >= minimumEthToSwap && amountEth <= maximumEthToSwap)) revert InvalidAmount();

        return swapEthForTokens(amountEth);
    }

    function treasuryAddLiquidityEth(uint256 tokenAmount, uint256 ethAmount) external onlyCaller {
        addLiquidity(tokenAmount, ethAmount);
    }

    function treasurySell(uint256 tokenAmount) external onlyCaller {
    ...
    function treasurySellOtherToken(ERC20 otherToken,uint256 tokenAmount) external onlyCaller {
        if(!(tokenAmount >= minimumTokenToSwap && tokenAmount <= maximumTokenToSwap)) revert InvalidAmount();

        swapTokensForEth(otherToken,tokenAmount);
    }

    function treasurySwapManagedToken(ERC20 toToken,uint256 tokenAmount) external onlyCaller {
        if(!(tokenAmount >= minimumTokenToSwap && tokenAmount <= maximumTokenToSwap)) revert InvalidAmount();

        swapTokensForToken(toToken, tokenAmount);
    }

    function treasuryBuyBackAndBurn(uint256 amountEth) external onlyCaller returns (bool) {
        if(!(amountEth >= minimumEthToSwap && amountEth <= maximumEthToSwap)) revert InvalidAmount();

        burnDeadWallet(swapEthForTokens(amountEth));
        return true;
    }

    function treasuryBurn(uint256 tokenAmount) external onlyCaller {        
    ...
    function swapEthForTokens(uint256 ethAmount) private returns (uint256 tokenAmount) {
     ...
     function swapTokensForToken(ERC20 toToken,uint256 tokenAmount) private returns (uint256 ethAmount) {
        address[] memory path = new address[](2);
