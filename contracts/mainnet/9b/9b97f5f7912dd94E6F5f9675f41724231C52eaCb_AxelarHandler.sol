...
contract AxelarHandler is BridgeHandlerBase, AxelarExecutable {
    ...
    constructor(
        ...
        gasReceiver = IAxelarGasService(_axelarGasReceiver);
    }
    ...
    function performBridgeAction(
        ...
        (
            ...
            bytes memory nonEvmPayload
        ) = abi.decode(params.bridgePayload, (string, string, bytes));

        string memory callTo;

        if (params.bridgeToken != gateway.tokenAddresses(tokenSymbol)) {
            revert InvalidAxelarToken(params.bridgeToken, tokenSymbol);
        }

        if (params.callTo == address(0)) {
            // EVM -> Non EVM bridge
            if (nonEvmPayload.length == 0) {
                revert InvalidBridgePayload();
            }
            if (bytes(params.nonEvmCallTo).length == 0) {
                revert InvalidNonEvmCallTo();
            }

            callTo = params.nonEvmCallTo;
            axelarPayload = nonEvmPayload;
        } else {
            if (nonEvmPayload.length != 0) {
                revert InvalidBridgePayload();
            }

            callTo = Strings.toHexString(params.callTo);
            axelarPayload = abi.encode(
                BridgeRequest({
        ...
        if (params.amount == 0) {
            ...
            }(address(this), dstChain, callTo, axelarPayload, params.user);

            gateway.callContract(dstChain, callTo, axelarPayload);
        } else {
    ...
    function _executeWithToken(
        ...
        _executeAction(payload, gateway.tokenAddresses(tokenSymbol), amount);
    }
    ...
    function _execute(
        ...
        _executeAction(payload, address(0), 0);
    }

    error InvalidAxelarToken(address tokenAddr, string symbol);
}
