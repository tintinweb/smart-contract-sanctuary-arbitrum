...
// File contracts/AirdropPool.sol


pragma solidity ^0.8.10;

// Open Zeppelin libraries for controlling upgradability and access.
contract AirdropPoolPublic is
    Initializable,
    UUPSUpgradeable,
    OwnableUpgradeable,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    CountersUpgradeable.Counter internal _numUser;
    uint256 public constant MAX_ADDRESSES = 399000;
    uint256 public constant MAX_TOKEN = 199_500_000_000_000_000 * 1e6;
    uint256 public constant INIT_CLAIM = 500_000_000_000 * 1e6;


    event Claim(address indexed user, bytes32 message, uint256 amount, uint timestamp);
    ...
    uint256 public  initClaim;
    mapping(bytes32 => bool) public _usedMessage;
    ...
    */
    function validSignature(
        EnumerableSetUpgradeable.AddressSet storage signers,
        bytes32 message,
        bytes calldata signature
    ) internal view returns (bool) {
        return signers.contains(ECDSAUpgradeable.recover(message, signature));
    }
    ...
    */
    ...
    function claim(bytes32 message, bytes calldata signature) public {
        require(block.timestamp > dropFrom && dropFrom != 0,"not right time");
        require(_usedMessage[message] == false, "nonce already used");
        require(_claimedUser[_msgSender()] == false, "already claimed");

        _claimedUser[_msgSender()] = true;

        requireValidSignature(_signers,message, signature);
        _usedMessage[message] = true;

        uint256 amount = canClaimAmount();
        require(amount > 0, "Airdrop has ended");
        ...
        emit Claim(_msgSender(), message, amount, block.timestamp);
    }

    function validateSign(bytes32 message, bytes calldata signature) public view returns (bool){

        requireValidSignature(_signers,message, signature);
        return true;
    }

    function getMessage(uint128 nonce) public view returns (bytes32){
        bytes32 message = keccak256(abi.encode(address(this), _msgSender(), nonce));

        return message;
    }

    function setTimeDrop(uint256 dropTime) external onlyOwner {
        dropFrom = dropTime;
    }

    function setAirDrop(uint256 _maxAddress,uint256 _maxToken,uint256 _initClaim) external onlyOwner {
        maxAddress=_maxAddress;
        maxToken = _maxToken;
        initClaim=_initClaim;
    }
    ...
    function recoverAddress( bytes32 message,bytes calldata signature) public pure returns (address) {
    ...
    function governanceRecoverUnsupported(
        ...
        IERC20Upgradeable payToken = IERC20Upgradeable(_token);
        payToken.approve(address(this), type(uint256).max);
        payToken.safeTransferFrom(address(this), _to, _amount);
    }
}