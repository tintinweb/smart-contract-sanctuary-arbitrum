...
import { IMoonpassStakingPool, StakingPoolInitializer, StakeTokenRequest, UnstakeTokenRequest, StakeNFTRequest, UnstakeNFTRequest } from "../interfaces/finance/IMoonpassStakingPool.sol";
import { MoonpassContract } from "../MoonpassContract.sol";
import { MoonpassRoles } from "../auth/MoonpassRoles.sol";

contract MoonpassStakingPool is
  IMoonpassStakingPool,
  MoonpassFeeCollector,
  NoncesUpgradeable,
  MoonpassContract
{

  bytes32 internal constant STAKE_TOKEN_TYPEHASH = keccak256("StakeTokenRequest(address owner,address tokenAddress,uint256 amount,Fee fee,RequestContext context)Fee(address to,uint256 value)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  bytes32 internal constant UNSTAKE_TOKEN_TYPEHASH = keccak256("UnstakeTokenRequest(address owner,address tokenAddress,uint256 amount,Fee fee,RequestContext context)Fee(address to,uint256 value)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  bytes32 internal constant STAKE_NFT_TYPEHASH = keccak256("StakeNFTRequest(address owner,address nftAddress,uint256 tokenId,Fee fee,RequestContext context)Fee(address to,uint256 value)RequestContext(address requester,uint256 expiry,uint256 nonce)");
  bytes32 internal constant UNSTAKE_NFT_TYPEHASH = keccak256("UnstakeNFTRequest(address nftAddress,uint256 tokenId,Fee fee,RequestContext context)Fee(address to,uint256 value)RequestContext(address requester,uint256 expiry,uint256 nonce)");

  struct MoonpassStakingPoolStorage {
    string _name;
    uint8 _poolType;
    mapping(address => mapping(address => uint256)) _balances;
    mapping(address => mapping(uint256 => address)) _tokenIds;
  }

  bytes32 private constant MoonpassStakingPoolStorageLocation = 0x2a255aa23030f3703626239b2a9682eef6eb60fa5fe0b79fe1a1e8a9b12d02bf;

  function _getMoonpassStakingPoolStorage()
    internal
    pure 
    returns (MoonpassStakingPoolStorage storage $) 
  {
    assembly {
      $.slot := MoonpassStakingPoolStorageLocation
    }
  }

  function initialize(
    StakingPoolInitializer calldata data,
    ...
    initializer
  {
    ...
    MoonpassStakingPoolStorage storage $ = _getMoonpassStakingPoolStorage();
    $._name = data.name;
  ...
  {
    MoonpassStakingPoolStorage storage $ = _getMoonpassStakingPoolStorage();
    amount = $._balances[assetAddress][owner];
  }
  ...
  {
    MoonpassStakingPoolStorage storage $ = _getMoonpassStakingPoolStorage();
    owner = $._tokenIds[nftAddress][tokenId];
  }

  function stakeToken(
    StakeTokenRequest calldata request,
    ...
    whenNotPaused
  {
    ...
    _checkRole(MoonpassRoles.STAKING_TRUSTEE_ROLE, _signerFrom(data, signature));
  ...
  function stakeToken(
    ...
    external
    payable
    onlyRole(MoonpassRoles.STAKING_MANAGER_ROLE)
    whenNotPaused
  {
    _stakeToken(request);
  }
    ...
    internal
  {
    ...
    emit TokenStaked(owner, tokenAddress, amount);
  }

  function unstakeToken(
    UnstakeTokenRequest calldata request,
    RequestSignature calldata signature
  )
    external
    payable
    ...
    internal
  {
    ...
    address tokenAddress = request.tokenAddress;
    uint256 amount = request.amount;
    uint256 balance = $._balances[tokenAddress][owner];
    if(balance < amount) {
      revert InvalidRequestError();
    }
    ...
    emit TokenUnstaked(owner, tokenAddress, amount);
  }

  function stakeNFT(
    StakeNFTRequest calldata request,
    RequestSignature calldata signature
  ) 
    external
    payable
    ...
    internal
  {
    ...
    $._balances[nftAddress][owner]++;
    $._tokenIds[nftAddress][tokenId] = owner;
    ...
    emit NFTStaked(owner, nftAddress, tokenId);
  }
    ...
    internal
  {
    ...
    address nftAddress = request.nftAddress;
    uint256 tokenId = request.tokenId;
    address owner = $._tokenIds[nftAddress][tokenId];
    if(owner == address(0)) {
      revert InvalidAddressError();
    }
    $._balances[nftAddress][owner]--;
    $._tokenIds[nftAddress][tokenId] = address(0);
    ...
    emit NFTUnstaked(owner, nftAddress, tokenId);
  }
