...
// File: contracts/token/AISCode.sol


pragma solidity =0.8.19;

contract AISCode is ERC20Capped, Ownable {
    ...
    constructor(
        ...
        address _swapRouter,
        address _weth
    ) ERC20Capped(MAX_CAP) ERC20("AISCODE", "AISCODE") {
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'AISCODE: ETH_TRANSFER_FAILED');
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "AISCODE: pair is the zero address");
    ...
    function addMinter(address minter) public onlyOwner returns (bool) {
        require(minter != address(0), "AISCODE: minter is the zero address");
    ...
    function getMinter(uint256 index) public view returns (address) {
        require(index <= getMinterLength() - 1, "AISCODE: index out of bounds");
        return _minters.at(index);
    }

    // modifier for mint function
    modifier onlyMinter() {
        require(isMinter(msg.sender), "AISCODE: caller is not the minter");
        _;
    }
