...
contract LaunchPadFactoryDiamondInit is AccessControl {
    ...
    function init(InitDiamondArgs memory _input) external {
        ...
        // Deprecated fields
        ds0._tokenfiToken = address(0);
        ds0._usdToken = address(0);
        ds0._router = address(0);
        ds0._treasury = address(0);

        // Supercharger setup
        if (_input.overrideSuperchargerTiers) {
            if (ds0.superChargerMultiplierByTier.length > 0) {
                delete ds0.superChargerMultiplierByTier;
                delete ds0.superChargerHeadstartByTier;
                delete ds0.superChargerTokensPercByTier;
            }
