...
contract SakeVaultV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function closePosition(
        ...
        uint256 profit;
        if (closeData.toLeverageUser > _userInfo.deposit) {
            profit = closeData.toLeverageUser - _userInfo.deposit;
            uint256 profitMfee = profit.mulDiv(mFeePercent, 10000);
            IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(feeConfiguration.feeReceiver, profitMfee);
            closeData.toLeverageUser -= profitMfee;
        }
