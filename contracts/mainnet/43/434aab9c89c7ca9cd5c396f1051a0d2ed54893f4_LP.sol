...
contract LP is Ownable, Pausable, ERC20 {
    ...
    function withdraw() external whenNotPaused() updateReward(msg.sender) {
        ...
        totalDeposit -= depositAmt[msg.sender];
    ...
    function getLPOut(uint _amt) external view returns(uint) {
        uint _lpAmt;
        if ((totalSupply() != 0) && (getCurrentReserve() != 0)) {
    ...
    function getAmountOut(uint _lpAmt) external view returns(uint) {
        uint _amt = totalSupply() > 0 ? getCurrentReserve() * _lpAmt / totalSupply() : 0;
        return _amt;
    }
