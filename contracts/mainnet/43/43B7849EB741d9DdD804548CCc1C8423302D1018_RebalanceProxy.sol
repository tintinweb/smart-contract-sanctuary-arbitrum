...
contract RebalanceProxy {
	...
	uint256 public maxTranslation = 100;
	uint256 public maxWidth = 10;
	...
	constructor(address _owner, address _rebalancer, address _admin) {
		owner = _owner;
	...
	function isWithinRange(
		...
		console.log(abs(lastMidPoint), abs(newMidPoint), uint256(diff));
	...
	function isWidthChangeWithinRange(
		...
		int256 allowedWidthDiff = oldWidth.mul(int256(maxWidth)).div(100);
	...
	function rebalance(
		...
		if(lastRebalance[hypervisor] != 0) require(block.timestamp >= lastRebalance[hypervisor] + _minInterval, "too soon");

		if(!exempted[hypervisor]) {
		...
		Admin(admin).rebalance(hypervisor, _baseLower, _baseUpper, _limitLower, _limitUpper, _feeRecipient, inMin, outMin);
	...
	function transferRebalancer(address newRebalancer) external onlyOwner {
		require(newRebalancer != address(0), "newRebalancer should be non-zero");
