...
contract TodT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    TodTDividendTracker public dividendTracker;
    ...
    event UpdaTodTakingAmounts(uint256 duration, uint256 amount);
    ...
    constructor() ERC20("TodT", "$TTT") {
        ...
        dividendTracker = new TodTDividendTracker(
            payable(this),
            router,
    ...
    function updaTodTakingAmounts(
        ...
        emit UpdaTodTakingAmounts(duration, bonus);
    }
...
contract TodTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        TodTContract = TodT(mainContract);
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "TodT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "TodT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TodT contract."
        );
    }
    ...
    function _reinvestDividendOfUser(
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TodTContract);

            uint256 prevBalance = TodTContract.balanceOf(address(this));
            ...
            {
                uint256 received = TodTContract.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    TodTContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
