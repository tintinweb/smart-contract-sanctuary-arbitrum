...
contract LPTokenProcessorV2 is ILPTokenProcessorV2, KeeperCompatible, AccessControlEnumerable {
    ...
    uint256 public constant GLOBAL_BASIS_POINTS = 10_000;
    uint256 public constant BURN_BASIS_POINTS = 2_500; // 25%
    uint256 public constant REFERRER_BASIS_POINTS = 2_500; // 25%
    uint256 public slippageBasisPoints = 300; // 3%
    mapping(address => uint256) public perTokenSlippage;
    bool public requireOraclePrice;
    uint24 public wethToUsdV3PoolFee = 3_000; // 0.3%
     ...
     */
    function _swapV2TokenByFeeToken(address tokenAddress, uint256 lpBalance, address routerAddress) private returns (uint256) {
    ...
    function _burnFloki(uint256 feeAmount, address vault) private returns (uint256) {
        // Burn floki
        if (FLOKI != address(0)) {
            uint256 burnShare = (feeAmount * BURN_BASIS_POINTS) / GLOBAL_BASIS_POINTS;
            feeAmount -= burnShare;
            ...
            emit FlokiBurned(tokenBurnedLastBlock, vault, feeAmount, flokiBurned);
    ...
    function _processFees(TokenSwapInfo memory info, uint256 feeBalance) private {
        ...
        if (info.referrer != address(0)) {
            uint256 referrerShare = (feeBalance * REFERRER_BASIS_POINTS) / GLOBAL_BASIS_POINTS;
    ...
    function _swapTokensWithV3Router(
        ...
        try router.exactInput(params) returns (uint256 amountOut) {} catch (bytes memory /* lowLevelData */) {
            return false;
        }
        return true;
    }
    ...
    function _getPriceInUSDWithSlippage(address token) private returns (uint256) {
        ...
        return price - ((price * slippage) / GLOBAL_BASIS_POINTS);
    }

    function adminWithdraw(address tokenAddress, uint256 amount, address destination) external onlyRole(DEFAULT_ADMIN_ROLE) {
