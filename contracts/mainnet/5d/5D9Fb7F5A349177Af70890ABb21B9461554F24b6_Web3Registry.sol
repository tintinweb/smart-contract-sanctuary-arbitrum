...
contract Web3Registry is IWeb3Registry, Initializable {
    ...
    function setRecord(bytes32 node, address _owner, address _resolver) external {
        setOwner(node, _owner);
        _setResolver(node, _resolver);
    }

    function setSubnodeRecord(bytes32 node, bytes32 label, address _owner, address _resolver) external {
        bytes32 subnode = setSubnodeOwner(node, label, _owner);
        _setResolver(subnode, _resolver);
    }

    function setSubnodeOwner(bytes32 node, bytes32 label, address _owner) public authorized(node) returns (bytes32) {
        bytes32 subnode = keccak256(abi.encodePacked(node, label));
        records[subnode].owner = _owner;
        emit NewOwner(node, label, _owner);
        // check if it's an addr reverse node
        // if (node != ADDR_REVERSE_NODE && address(ens) != address(0x0)) {
        //     // ens, set owner to this contract
        //     bytes32 ensSubnode = ens.setSubnodeOwner(ensBaseNode, label, address(this));
        //     // record node mapping
    ...
    function setResolver(bytes32 node, address _resolver) public authorized(node) {
        _setResolver(node, _resolver);
    }
