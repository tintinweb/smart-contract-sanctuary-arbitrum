    ...
    ReentrancyGuard
{
    ...
    /// _exists checkIn
    mapping(uint256 => bool) private userCheckInEntries;
    /// _exists transId
    ...
    function checkIn(uint256 date,uint256 amount,bytes memory signature, string memory transId) external {

        require(status, "check-in status is false");
        require(haveCheckedIn(msg.sender,date) == false, "Already Checked In!");
        require(existsTransId(transId) == false, "Exists transId");
        require(verifySignature(transId, "CHECKIN", amount, msg.sender, signature), "Signature is invalid");
        require(UXUY.balanceOf(address(this)) >= amount,"UXUY Balance is insufficient!");

        setTransId(transId);
        setCheckIn(msg.sender,date);
        // send token to sender
        TransferHelper.safeTransfer(UXUYTOKEN, msg.sender, amount);
        emit CheckIn(msg.sender, date, amount);
    }

    function setRewardTokens(address _uxuytoken) external isSuperOperator {
        UXUYTOKEN = _uxuytoken;
        UXUY = IERC20(UXUYTOKEN);
    }

    function setCheckIn(address user, uint256 date) internal {
        bytes32 label = keccak256(abi.encode(user, date));
        uint256 id = uint256(label);
        userCheckInEntries[id] = true;
    }

    function haveCheckedIn(address user, uint256 date) public view returns (bool){
        bytes32 label = keccak256(abi.encode(user, date));
        uint256 id = uint256(label);
        if(userCheckInEntries[id]){
            return true;
        }else{
            return false;
        }
