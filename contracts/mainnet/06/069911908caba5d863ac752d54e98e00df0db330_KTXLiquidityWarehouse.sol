...
import {KTXHelperLibrary} from "./KTXHelperLibrary.sol";

contract KTXLiquidityWarehouse is LiquidityWarehouse {
    /// @notice This error is thrown when setting a zero address
    error InvalidZeroAddress();

    /// @notice Chainlink Sequencer Uptime feed
    address internal immutable i_sequencerUptimeFeed;

    /// @notice Map for asset configuration
    mapping(address asset => KTXHelperLibrary.AssetConfig config) internal s_assetConfigs;

    constructor(
        LiquidityWarehouse.ConstructorParams memory params,
        KTXHelperLibrary.AssetConfigParams[] memory assetConfigs,
        address sequencerUptimeFeed
    ) LiquidityWarehouse(params) {
        if (sequencerUptimeFeed == address(0)) {
            revert InvalidZeroAddress();
        }
        i_sequencerUptimeFeed = sequencerUptimeFeed;
        KTXHelperLibrary.setAssetConfigs(assetConfigs, s_assetConfigs);
    }

    /// @notice Sets asset configurations
    /// @param assetConfigs Array of assets with their configuration
    function setAssetConfigs(KTXHelperLibrary.AssetConfigParams[] calldata assetConfigs)
        external
    ...
    function _withdrawFromTarget(uint256 withdrawAmount, address rewardRouterAddr, bytes memory)
        ...
        returns (bool)
    {
        return KTXHelperLibrary.withdrawAndSwap(
            KTXHelperLibrary.WithdrawAndSwapParams({
                rewardRouter: rewardRouterAddr,
                baseAsset: address(s_terms.asset),
                sequencerUptimeFeed: i_sequencerUptimeFeed,
    ...
    function _getDeployedAssetValue(address rewardRouterAddr) internal view override returns (uint256) {
        return KTXHelperLibrary.getDeployedAssetValue(
            rewardRouterAddr,
            address(s_terms.asset),
            i_sequencerUptimeFeed,
            s_assetConfigs[address(s_terms.asset)].chainlinkPriceFeed
        );
    }
...
import {IVault} from "./vendor/ktx/IVault.sol";
import {IRewardTracker} from "./vendor/ktx/IRewardTracker.sol";
import {IKLPManager} from "./vendor/ktx/IKLPManager.sol";
import {Math} from "openzeppelin/utils/math/Math.sol";

library KTXHelperLibrary {
    ...
    /// @notice USDG decimal
    uint8 internal constant USDG_DECIMAL = 18;

    /// @notice KLP decimal
    uint8 internal constant KLP_DECIMAL = 30;

    /// @notice KTX fee precision
    uint24 internal constant KTX_FEE_PRECISION = 1e4;

    /// @notice Dex fee precision
    uint24 internal constant DEX_FEE_PRECISION = 1e6;

    /// @notice Liquidity warehouse standard decimal precision
    uint256 internal constant WAD = 1e18;

    /// @notice KLP price decimal precision
    uint256 internal constant KLP_PRICE_PRECISION = 1e30;
    ...
    /// @notice Asset configuration
    struct AssetConfig {
        /// @notice Asset swap path on DEX
        bytes swapPath;
        /// @notice Asset swap fees from swapping on a DEX
        uint24[2] fees;
        /// @notice Asset swap deadline
        uint128 deadline;
        /// @notice Asset swap slippage
        uint128 slippage;
        /// @notice Swap router address
        /// @dev Uniswap or Camelot swap router will be used
        address swapRouter;
    ...
    struct WithdrawAndSwapParams {
        ...
        address baseAsset;
        ///@notice Chainlink Sequencer Uptime feed address
        address sequencerUptimeFeed;
        ///@notice Withdraw amount
        uint256 withdrawAmount;
    }

    /// @notice BurnKLP parameters
    struct BurnKLPParams {
        /// @param asset Asset address
        address asset;
        /// @param vault Address of KTX vault
        address vault;
        /// @param klpBurnAmount Amount of KLP that want to be burn
        uint256 klpBurnAmount;
        /// @param klpPrice KLP Price
        uint256 klpPrice;
        /// @param rewardRouter KTX reward router
        IRewardRouter rewardRouter;
    }

    /// @notice Sets assets with their configuration
    /// @param params Array of assets with config
    /// @param assetConfigs Configs for each asset
    function setAssetConfigs(
        AssetConfigParams[] memory params,
        mapping(address asset => KTXHelperLibrary.AssetConfig config) storage assetConfigs
    ) public {
        for (uint256 i; i < params.length; ++i) {
            AssetConfigParams memory assetConfigParams = params[i];
            if (
                assetConfigParams.asset == address(0)
                    || address(assetConfigParams.assetConfig.chainlinkPriceFeed) == address(0)
                    || assetConfigParams.assetConfig.swapRouter == address(0)
            ) {
                revert InvalidAssetConfig();
            }
            assetConfigs[assetConfigParams.asset] = assetConfigParams.assetConfig;
        }
    }

    /// @notice Sets assets with their configuration
    /// @param params withdrawAndSwap parameters
    /// @param assetConfigs Configs for each asset
    function withdrawAndSwap(
        WithdrawAndSwapParams memory params,
        mapping(address asset => KTXHelperLibrary.AssetConfig config) storage assetConfigs
    ) public returns (bool) {
        IKLPManager klpManager = IKLPManager(IRewardRouter(params.rewardRouter).klpManager());
        address ktxVault = klpManager.vault();
        uint256 klpPrice = klpManager.getPrice(false);
        uint256 klpToBurn;

        /// If withdraw amount is max (deactive is triggered) set klpToBurn to balance of the liquidity warehouse
        if (params.withdrawAmount == type(uint256).max) {
            klpToBurn = IRewardTracker(IRewardRouter(params.rewardRouter).feeKlpTracker()).stakedAmounts(address(this));
        } else {
            /// set amount of klp to burn based on USDG value
            klpToBurn =
                _convertToUSDG(ktxVault, params.baseAsset, params.withdrawAmount) * KLP_PRICE_PRECISION / klpPrice;
            /// Check if the amount of liquidity warehouse KLP balance is sufficient for the withdraw amount
            if (
                klpToBurn
                    > IRewardTracker(IRewardRouter(params.rewardRouter).feeKlpTracker()).stakedAmounts(address(this))
            ) {
                klpToBurn =
                    IRewardTracker(IRewardRouter(params.rewardRouter).feeKlpTracker()).stakedAmounts(address(this));
            }
        ...
        /// Burn the amount of KLP that needed to fulfill withdraw amount
        klpToBurn = _burnKLP(
            BurnKLPParams({
                asset: params.baseAsset,
                vault: ktxVault,
                klpBurnAmount: klpToBurn,
                klpPrice: klpPrice,
                rewardRouter: IRewardRouter(params.rewardRouter)
            }),
            assetConfigs
        );

        /// Check that there is no more KLP to burn to cover withdrawals
        if (klpToBurn == 0) return true;
        ...
        for (uint256 i; i < numWhitelistedTokens; ++i) {
            ...
            if (asset == params.baseAsset || !IVault(ktxVault).whitelistedTokens(asset)) continue;
            klpToBurn = _burnKLP(
                BurnKLPParams({
                    asset: asset,
                    vault: ktxVault,
            ...
            /// Convert withdrawn asset to base asset
            _swapToBaseAsset(asset, params.baseAsset, params.sequencerUptimeFeed, assetConfigs);
            if (klpToBurn == 0) break;
        }
        return klpToBurn == 0;
    }

    /// @notice Return value of deployed native asset
    /// @param rewardRouterAddr  Reward router address
    /// @param asset Base asset address
    /// @param sequencerUptimeFeed Address of Chainlink Sequencer Uptime Feed
    /// @param chainlinkPriceFeed Address of asset's chainlink price feed
    /// @return uint256 value of deployed asset
    function getDeployedAssetValue(
        address rewardRouterAddr,
        address asset,
        address sequencerUptimeFeed,
        address chainlinkPriceFeed
    ) public view returns (uint256) {
        IKLPManager klpManager = IKLPManager(IRewardRouter(rewardRouterAddr).klpManager());

        uint256 klpPriceUSD30DP = klpManager.getPrice(false);
        uint256 klpBalance18DP =
            IRewardTracker(IRewardRouter(rewardRouterAddr).feeKlpTracker()).stakedAmounts(address(this));
        uint256 navUSD18DP = klpPriceUSD30DP * klpBalance18DP / KLP_PRICE_PRECISION;

        uint256 baseAssetOraclePrice =
            _getAssetOraclePrice(AggregatorV3Interface(sequencerUptimeFeed), AggregatorV3Interface(chainlinkPriceFeed));
        return navUSD18DP * 10 ** IERC20Metadata(asset).decimals() / baseAssetOraclePrice;
    }

    /// @notice Burns KLP tokens
    /// @param params BurnKLP parameters
    /// @param assetConfigs Configs for each asset
    /// @return uint256 Amount of leftover KLP that needs to be burnt
    function _burnKLP(
        BurnKLPParams memory params,
        mapping(address asset => KTXHelperLibrary.AssetConfig config) storage assetConfigs
    ) internal returns (uint256) {
        uint256 maxBurnableKLPAmount;
        {
            uint256 ktxVaultBalance =
                IVault(params.vault).poolAmounts(params.asset) - IVault(params.vault).reservedAmounts(params.asset);
            maxBurnableKLPAmount =
                _convertToUSDG(params.vault, params.asset, ktxVaultBalance) * KLP_PRICE_PRECISION / params.klpPrice;
        }
        uint256 klpAmount = Math.min(params.klpBurnAmount, maxBurnableKLPAmount);
        uint256 counterSwapFee = _counterSwapFee(klpAmount, assetConfigs[params.asset].fees);
        uint256 klpRedeemFee = _counterKtxFee(
            params.vault,
            params.asset,
            klpAmount + counterSwapFee,
            (klpAmount + counterSwapFee) * params.klpPrice / KLP_PRICE_PRECISION,
            params.klpPrice
        );
        uint256 klpAmountWithFee = klpAmount + klpRedeemFee + counterSwapFee;

        if (klpAmountWithFee >= IRewardTracker(params.rewardRouter.feeKlpTracker()).stakedAmounts(address(this))) {
            if (maxBurnableKLPAmount < IRewardTracker(params.rewardRouter.feeKlpTracker()).stakedAmounts(address(this)))
            {
                params.rewardRouter.unstakeAndRedeemKlp(params.asset, maxBurnableKLPAmount, 0, address(this));

                return IRewardTracker(params.rewardRouter.feeKlpTracker()).stakedAmounts(address(this));
            }

            params.rewardRouter.unstakeAndRedeemKlp(
                params.asset,
                IRewardTracker(params.rewardRouter.feeKlpTracker()).stakedAmounts(address(this)),
                0,
                address(this)
            );
        ...
        if (klpAmountWithFee != 0) {
            /// check if ktx vault and staked klp have enough asset to cover the withdraw fee
            if ((klpAmountWithFee) <= maxBurnableKLPAmount) {
                params.rewardRouter.unstakeAndRedeemKlp(params.asset, klpAmountWithFee, 0, address(this));
                return params.klpBurnAmount - klpAmount;
            }

            /// unstake and redeem max burnable klpAmount
            params.rewardRouter.unstakeAndRedeemKlp(params.asset, maxBurnableKLPAmount, 0, address(this));

            uint256 maxBurnableCounterKLP = _getKtxFee(
                params.vault,
                params.asset,
                maxBurnableKLPAmount,
                maxBurnableKLPAmount * params.klpPrice / KLP_PRICE_PRECISION
            );

            uint256 maxKLPBurnAmountWithoutFees = maxBurnableKLPAmount - maxBurnableCounterKLP
                - _getSwapFee((maxBurnableKLPAmount - maxBurnableCounterKLP), assetConfigs[params.asset].fees);

            return params.klpBurnAmount < maxKLPBurnAmountWithoutFees
                ? 0
                : params.klpBurnAmount - maxKLPBurnAmountWithoutFees;
        }

        /// return if staked amount is 0
        if (IRewardTracker(params.rewardRouter.feeKlpTracker()).stakedAmounts(address(this)) == 0) {
            return 0;
        }
    ...
    /// @notice Returns amount of klp to counter redemption fee
    /// @param vault KTX vault address
    /// @param asset Asset address
    /// @param klpAmount Amount of klp
    /// @param usdgAmount Amount of usdg
    /// @param klpPrice Price of KLP
    /// @return uint256 Return KLP amount to counter KTX fee
    function _counterKtxFee(address vault, address asset, uint256 klpAmount, uint256 usdgAmount, uint256 klpPrice)
        internal
        view
        returns (uint256)
    {
        uint256 mintBurnFeeBasisPoints = IVault(vault).mintBurnFeeBasisPoints();
        uint256 taxBasisPoints = IVault(vault).taxBasisPoints();
        uint256 ktxFeeBasisPoint =
            _getFeeBasisPoints(vault, asset, usdgAmount, mintBurnFeeBasisPoints, taxBasisPoints, false);
        uint256 klpWithFee = (
            klpAmount * (KTX_FEE_PRECISION * KLP_PRICE_PRECISION / (KTX_FEE_PRECISION - ktxFeeBasisPoint))
        ) / KLP_PRICE_PRECISION;

        if (
            _getFeeBasisPoints(
                vault, asset, klpWithFee * klpPrice / KLP_PRICE_PRECISION, mintBurnFeeBasisPoints, taxBasisPoints, false
            ) == ktxFeeBasisPoint
        ) {
            return klpWithFee - klpAmount;
        }
                            ...
                            KTX_FEE_PRECISION
                                - (
                                    _getFeeBasisPoints(
                                        vault,
                                        asset,
                                        klpWithFee * klpPrice / KLP_PRICE_PRECISION,
    ...
    /// @notice Returns amount of ktx redeem fee
    /// @param vault KTX's vault address
    /// @param asset Address of asset withdrawal target
    /// @param klpAmount Amount of klp that want to be redeemed
    /// @param usdgAmount Amount of USDG that want to be withdrawn
    /// @return uint256 Amount of ktx withdrawal fee
    function _getKtxFee(address vault, address asset, uint256 klpAmount, uint256 usdgAmount)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 ktxFeeBasisPoint =
            _getFeeBasisPoints(vault, asset, usdgAmount, mintBurnFeeBasisPoints, taxBasisPoints, false);
        return klpAmount * ktxFeeBasisPoint / KTX_FEE_PRECISION;
    }

    /// @notice Returns amount of klp to counter swap fee
    /// @param klpAmount Amount of klp
    /// @param fees Fees of asset
    /// @return uint256 KLP amount to counter swap fee
    function _counterSwapFee(uint256 klpAmount, uint24[2] memory fees) internal pure returns (uint256) {
        uint256 counterKlpAmount = klpAmount * DEX_FEE_PRECISION / (DEX_FEE_PRECISION - fees[0]) - klpAmount;

        if (fees[1] > 0) {
            counterKlpAmount = (klpAmount * DEX_FEE_PRECISION / (DEX_FEE_PRECISION - fees[1]) - klpAmount)
                + (klpAmount * DEX_FEE_PRECISION / (DEX_FEE_PRECISION - fees[0]) - klpAmount) * DEX_FEE_PRECISION
                    / (DEX_FEE_PRECISION - fees[1]);
        }

        return counterKlpAmount;
    }

    /// @notice Returns amount of swap fee in klp amount
    /// @param klpAmount Amount of klp
    /// @param fees Fees of asset
    /// @return uint256 Amount of fees from swaps
    function _getSwapFee(uint256 klpAmount, uint24[2] memory fees) internal pure returns (uint256) {
        uint256 klpAmountAfterFee = klpAmount * (DEX_FEE_PRECISION - fees[0]) / DEX_FEE_PRECISION;

        if (fees[1] > 0) {
            klpAmountAfterFee = klpAmountAfterFee * (DEX_FEE_PRECISION - fees[1]) / DEX_FEE_PRECISION;
        }

        return klpAmount - klpAmountAfterFee;
    }

    /// @notice Returns the asset price in USDG value with 18 decimal precision
    /// @param vault KTX vault address
    /// @param asset Asset address
    /// @param amount Amount of asset
    /// @return uint256 Asset value in USDG with 18 decimal precision
    function _convertToUSDG(address vault, address asset, uint256 amount) internal view returns (uint256) {
        /// KTX return price based on chainlink price feed with some customizable logic (e.g USDC always have 1 to 1 ratio with USDG )
        uint256 ktxUsdRate = IVault(vault).getMaxPrice(asset);
        return amount * ktxUsdRate / 10 ** (KLP_DECIMAL - (USDG_DECIMAL - IERC20Metadata(asset).decimals()));
    }

    /// @notice Swap asset to base asset
    /// @param asset Asset address
    ...
    function _swapToBaseAsset(
        ...
        uint256 assetBalance = IERC20Metadata(asset).balanceOf(address(this));
        if (assetBalance > 0) {
            uint256 assetOutExpected = _convertToAssetValue(
                asset,
                assetBalance,
                sequencerUptimeFeed,
                assetConfigs[asset].chainlinkPriceFeed,
                assetConfigs[baseAsset].chainlinkPriceFeed
            );
            uint256 slippage = assetConfigs[asset].slippage;

            ISwapRouter(assetConfigs[asset].swapRouter).exactInput(
                ISwapRouter.ExactInputParams({
                    path: assetConfigs[asset].swapPath,
                    recipient: address(this),
                    deadline: block.timestamp + assetConfigs[asset].deadline,
                    amountIn: assetBalance,
                    amountOutMinimum: assetOutExpected * slippage
                        / (WAD * 10 ** (18 - IERC20Metadata(baseAsset).decimals())) //calculate slippage and convert to base asset decimal value
                })
            );
        }
    }

    /// @notice Returns the asset price in USD value with 18 decimal precision
    /// @param sequencerUptimeFeed Address of Chainlink Sequencer Uptime Feed
    /// @param assetOutPriceFeed Price feed address of target asset
    /// @param asset Asset address
    /// @param amount Amount of asset
    /// @return uint256 Asset value in USD with 18 decimal precision
    function _convertToUSD18DP(address sequencerUptimeFeed, address assetOutPriceFeed, address asset, uint256 amount)
        internal
        view
        returns (uint256)
    {
        uint256 usdPrice18DP =
            _getAssetOraclePrice(AggregatorV3Interface(sequencerUptimeFeed), AggregatorV3Interface(assetOutPriceFeed));
        return amount * 10 ** (18 - IERC20Metadata(asset).decimals()) * usdPrice18DP / WAD; //convert to 18 decimals and multiply with its price
    }

    /// @notice Calculate the amount of expected received asset based on paid asset dollar value
    /// @param assetIn Address of paid asset
    /// @param assetInAmount Amount of paid asset
    /// @param sequencerUptimeFeed Address of Chainlink Sequencer Uptime Feed
    /// @param assetInPriceFeed Price feed address of initial asset
    /// @param assetOutPriceFeed Price feed address of target asset
    /// @return uint256 Amount of expected received asset
    function _convertToAssetValue(
        address assetIn,
        uint256 assetInAmount,
        address sequencerUptimeFeed,
        address assetInPriceFeed,
        address assetOutPriceFeed
    ) internal view returns (uint256) {
        uint256 assetInUSD = _convertToUSD18DP(sequencerUptimeFeed, assetInPriceFeed, assetIn, assetInAmount);
        uint256 assetOutUSD =
            _getAssetOraclePrice(AggregatorV3Interface(sequencerUptimeFeed), AggregatorV3Interface(assetOutPriceFeed));
        return assetInUSD * WAD / assetOutUSD;
    }

    /// @notice Returns the Chainlink oracle price of an asset
    /// @param i_sequencerUptimeFeed Chainlink Sequencer Uptime Feed
    /// @param chainlinkPriceFeed Asset's chainlink price feed
    /// @return uint256 Asset price from chainlin pricefeed in 1e18 decimal precision
    function _getAssetOraclePrice(AggregatorV3Interface i_sequencerUptimeFeed, AggregatorV3Interface chainlinkPriceFeed)
        internal
        view
        returns (uint256)
    {
        AggregatorV3Interface priceFeed = chainlinkPriceFeed;
    ...
    /// @dev this function is a modified _getFeeBasis on KTX's vaultUtils by tweaking the initialAmount of vault USDG amounts
    /// @notice Returns get fee basis points for KTX withdrawal
    /// @param _vault Vault address
    /// @param _token Asset address
    /// @param _usdgDelta Amount of asset in usdg
    /// @param _feeBasisPoints KTX's vault fee basis points
    /// @param _taxBasisPoints KTX's vault tax basis points
    /// @param _increment KTX's vault interaction buy status
    /// @return uint256 Fee basis point
    function _getFeeBasisPoints(
        ...
        /// @dev substract _usdgDelta from vault's usdgAmounts in order to get actual fee value
        uint256 initialAmount = vault.usdgAmounts(_token) > _usdgDelta ? vault.usdgAmounts(_token) - _usdgDelta : 0;
        uint256 nextAmount = initialAmount + _usdgDelta;
        if (!_increment) {
            nextAmount = _usdgDelta > initialAmount ? 0 : initialAmount - _usdgDelta;
        }

        uint256 targetAmount = _getTargetUsdgAmount(_vault, _token, _usdgDelta);
        if (targetAmount == 0) {
            return _feeBasisPoints;
        }
    ...
    function _getTargetUsdgAmount(address ktxVault, address asset, uint256 usdgDelta) internal view returns (uint256) {
        uint256 supply = IERC20Metadata(IVault(ktxVault).usdg()).totalSupply() > usdgDelta
            ? IERC20Metadata(IVault(ktxVault).usdg()).totalSupply() - usdgDelta
            : 0;
        if (supply == 0) {
            return 0;
        }
        uint256 weight = IVault(ktxVault).tokenWeights(asset);
        return weight * supply / IVault(ktxVault).totalTokenWeights();
    }
}
...
interface IVault {
    ...
    function allWhitelistedTokens(uint256 idx) external view returns (address);
