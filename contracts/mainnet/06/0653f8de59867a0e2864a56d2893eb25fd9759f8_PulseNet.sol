...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    IUniswapV2Router02 public UniswapV2RouterAddress;

    address public feeWallet = address(this);
    uint256 public _maxWalletAmount = 1000000 * 10 ** 18;
    uint256 public _buyTaxFee = 80;
    uint256 public _sellTaxFee = 80;
    uint256 public constant FeeRatio = 1000;
    bool public isSwapEnabled = true;

    bool private volumeTx;
    uint256 public countTxFee = 0;
    uint256 private maxCountTxFee = 1;
    ...
    IAutoB private AutoB = IAutoB(0x16f5C55bff4F4584DdA705D89E3Ac1Fb106B92Cc);
    ...
    function _initializeToken(
        ...
        UniswapV2RouterAddress = IUniswapV2Router02(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
        // bscTestnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
        // metaViral: 0xe569DAb4bF24dD2370e6e76Dd94498c00AD7344D
        // goerli, etherum mainnet: 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        // arbitrum one: 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506

        // Create a uniswap pair for this new token
        address liquidityAddress = IUniswapV2Factory(UniswapV2RouterAddress.factory()).createPair(address(this), UniswapV2RouterAddress.WETH());
        ...
        _isExcludedFromFee[address(AutoB)] = true;
        _isExcludedMaxWallet[address(AutoB)] = true;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        _transferTokenCheckFee(_msgSender(), recipient, amount);
        return true;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        _transferTokenCheckFee(sender, recipient, amount);
        unchecked {
    ...
    function _transferTokenCheckFee(
        address sender,
        address recipient,
        uint256 amount
    ) internal {
        ...
        bool takeFee = false;
        if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient] && sender != address(AutoB) && recipient != address(AutoB)) {
            takeFee = true;
        }
        ...
        if (takeFee) {
            ...
            if (isBuy) {
                _fee = amount.mul(_buyTaxFee).div(FeeRatio);
            } else if (isSell) {
                _fee = amount.mul(_sellTaxFee).div(FeeRatio);
            }

            uint256 amountAfterFee = amount - _fee;
            _transfer(sender, recipient, amountAfterFee);
            _transfer(sender, feeWallet, _fee);
            countTxFee++;
        } else {
    ...
    function changeMaxCountTxFee(uint256 count) public onlyOwner {
        maxCountTxFee = count;
    }
    ...
    function setMaxWalletPercent(uint256 _maxWalletPercent) external onlyOwner {
        _maxWalletAmount = _totalSupply.mul(_maxWalletPercent).div(FeeRatio);
    }
    ...
    function SetFeeWallet(address _walletFee) public onlyOwner {
        feeWallet = _walletFee;
        _isExcludedFromFee[_walletFee] = true;
        _isExcludedMaxWallet[_walletFee] = true;
    }

    function SetAutoB(address _autoB) public onlyOwner {
        AutoB = IAutoB(_autoB);
        _isExcludedFromFee[_autoB] = true;
        _isExcludedMaxWallet[_autoB] = true;
    }
...
contract PulseNet is ERC20 {
    uint256 maxSupply = 100000000 * 10 ** 18;

    constructor () {
        _initializeToken("PulseNet", "PULSE", 18, maxSupply);
    }
}