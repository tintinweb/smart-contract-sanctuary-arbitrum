...
contract AdvancedQuestingDiamond is Initializable, AdvancedQuestingDiamondState {
    ...
    function endTimeForLegions(uint256[] calldata _legionIds) external view returns(uint256[] memory _endTimes, uint8[] memory _statisHitCounts) {
        _endTimes = new uint256[](_legionIds.length);
        _statisHitCounts = new uint8[](_legionIds.length);

        for(uint256 i = 0; i < _legionIds.length; i++) {
           (uint256 _endTime, uint8 _stasisHitCount) = endTimeForLegion(_legionIds[i]);
           _endTimes[i] = _endTime;
           _statisHitCounts[i] = _stasisHitCount;
        }
    }
    ...
    // If the random is not seeded, both numbers will return 0.
    function endTimeForLegion(uint256 _legionId) public view returns(uint256, uint8) {
        ...
        if(!appStorage.randomizer.isRandomReady(_requestIdForLegion(_usingOldSchema, _legionId))) {
            return (0, 0);
        }
