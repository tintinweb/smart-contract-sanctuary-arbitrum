...
contract JBC_HUB_2_0_FEES is AccessControl, ReentrancyGuard {
    ...
    function depositTokens(
        ...
        require(_swapAmount > 0, "Swap fee must be greater than zero");
        require(tokenAddress != address(this), "Invalid token address");

        if (tokenAddress == address(0)) {
            bool adminTransferSuccess = payable(admin).send(_adminFee);
            require(adminTransferSuccess, "Admin transfer failed");

            bool swapTransferSuccess = payable(swap).send(_swapAmount);
            require(swapTransferSuccess, "Swap transfer failed");

            emit NativeTransfer(msg.sender, swap, _swapAmount);
            emit NativeAdminFeeTransfer(msg.sender, admin, _adminFee);
        } else {
            require(
            ...
            bool adminTransferSuccess = payable(admin).send(_adminFee);
            require(adminTransferSuccess, "Admin transfer failed");
            emit ERC20Transfer(tokenAddress, msg.sender, swap, _swapAmount);
            emit NativeAdminFeeTransfer(msg.sender, admin, _adminFee);
        }
    }
