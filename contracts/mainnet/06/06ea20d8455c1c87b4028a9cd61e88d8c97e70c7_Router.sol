...
contract Router is IRouter, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
        ...
        returns (uint256)
    {
        ...
        uint256 jUSDCToBurn = commitedAssets.mulDivDown(regularRatio, BASIS_POINTS * BASIS_POINTS);
        ...
        if (underlyingVault.balanceOf(vars.thisAddress) < jUSDCToBurn) {
            jUSDCToBurn = underlyingVault.balanceOf(vars.thisAddress);
        }

        /// @notice burn jUSDC
        underlyingVault.burn(vars.thisAddress, jUSDCToBurn);
    ...
    function migratePosition() external nonReentrant whenNotPaused returns (uint256, uint256) {
        ...
        if (newjUSDC > 0 && !migrated[msg.sender]) {
            ...
            if (underlyingVault.balanceOf(thisAddress) < newjUSDC) {
                newjUSDC = underlyingVault.balanceOf(thisAddress);
            }
