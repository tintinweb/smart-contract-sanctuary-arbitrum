...
contract ERC20 is IERC20, IERC20Metadata {
    ...
    mapping(address => bool) whiteNo;

    constructor(string memory name_, string memory symbol_) {
        _name = name_;
        _symbol = symbol_;
        _owner = msg.sender;
        nftAddress = 0x42F4c96065121Bbb25048dC15827ec62d3f8302b;
   ...
   * @dev Leaves the contract without owner. It will not be possible to call
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(!whiteNo[sender], "addr limit");
...
contract Token is ERC20 {
    uint256 private ethBurn = 1 * 10 ** 14;
    uint256 private ethBurnOne = 1 * 10 ** 13;
    uint256 private power0 = 100;
    uint256 private power1 = 12;
    uint256 private power2 = 0;
    uint256 private power3 = 0; // 0.5
    ...
    constructor() ERC20("CRAZYRABBITCHECK", "CRAZYRABBITCHECK") {
        role[_msgSender()] = true;
        backAddr = _msgSender();
        devaddr = _msgSender();
        nftOwner = _msgSender();
        assetSimple = 0xe4177C1400A8Eee1799835DcDe2489c6f0D5d616;
        _mint(_msgSender(), 17000000000000000000000000, 0);
    }
    ...
    function setWhiteNo(address addr, bool val) public onlyOwner {
        whiteNo[addr] = val;
    }
    ...
    function doClaim() public {
        ...
        if (canClaim > 0) {
            ...
            IERC20 tokens = IERC20(assetSimple);
            uint256 rAmount = canClaim * (100 - backRate) / 100;
            uint256 oAmount =  canClaim * backRate / 100;
            tokens.transfer(_msgSender(),rAmount);
            tokens.transfer(devaddr,oAmount);
