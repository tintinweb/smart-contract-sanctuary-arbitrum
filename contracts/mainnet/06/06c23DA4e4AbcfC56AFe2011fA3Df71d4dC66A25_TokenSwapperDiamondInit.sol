...
contract TokenSwapperDiamondInit is AccessControl {
    struct InitDiamondArgs {
        address flokiToken;
        address feeToken;
        uint256 sellDelay;
        address dexMainRouter;
        address dexFactory;
        bool isV2Router;
        address treasury;
        uint24 wethToUsdV3PoolFee;
    ...
    function init(InitDiamondArgs memory _input) external {
        require(_input.dexMainRouter != address(0), "LockerDiamondInit::init::ZERO: Router cannot be zero address.");
        require(_input.treasury != address(0), "LockerDiamondInit::init::ZERO: Treasury cannot be zero address.");

        // TokenSwapper
        LibTokenSwapperStorage.DiamondStorage storage ds1 = LibTokenSwapperStorage.diamondStorage();

        ds1.slippageBasisPoints = 300; // 3%
        ds1.wethToUsdV3PoolFee = 3_000; // 0.3%
        ds1.sellDelay = _input.sellDelay;
        ds1.routerForFloki = _input.dexMainRouter;
        if (_input.dexMainRouter != address(0) && _input.dexFactory != address(0)) {
            ds1.routerByFactory[_input.dexFactory] = _input.dexMainRouter;
            ds1.v2Routers[_input.dexMainRouter] = _input.isV2Router;
        }
        ds1.priceOracle = _input.priceOracle;
        ds1.wethToUsdV3PoolFee = _input.wethToUsdV3PoolFee;
        ds1.treasury = _input.treasury;
        ds1.feeToken = _input.feeToken;
        ds1.flokiToken = _input.flokiToken;
