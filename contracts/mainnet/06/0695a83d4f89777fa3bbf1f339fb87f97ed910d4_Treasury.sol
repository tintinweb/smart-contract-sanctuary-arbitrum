...
interface ISeigniorageCalculator {
    function calculateSeigniorage(
        uint previousEpochEmpyrealPrice,
        uint previousEpochFirmamentPrice,
        uint empyrealSupply,
        uint // _epoch
    ) external returns (uint _savedForHorizon);
}
...
contract Treasury is ContractGuard, AccessControlled {
    ...
    function _sendToHorizon(uint256 _amount) internal {
        ...
        IHorizon(_horizon).allocateSeigniorage(_amount);
