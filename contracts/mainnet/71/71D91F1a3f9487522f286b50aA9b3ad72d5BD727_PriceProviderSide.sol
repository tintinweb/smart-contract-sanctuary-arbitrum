...
contract PriceProviderSide is IPriceProvider, OwnableUpgradeable, MsgReceiverAppUpg {
    ...
    //so it can only support one type of messsage?
    function _executeMessage(bytes memory message) internal override {
        uint256 updatedPrice = abi.decode(message, (uint256));
        price = updatedPrice;
