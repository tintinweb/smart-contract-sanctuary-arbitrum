...
interface IKlpRewardHandler {
    ...
    function getAnySlippagedAmountOut(address _assetFrom, address _to, uint256 _amount) external view returns (uint256);
    function swap(address _assetFrom, address _to, address _receiver, uint256 _amount) external  returns (uint256);
}
    ...
    ERC20BurnableUpgradeable
{
    ...
    function closePosition(uint256 _positionID, address _user) external InvalidID(_positionID, _user) nonReentrant {
        ...
        require(returnedValue > 0, "Out of range");
        console.log("Returned value: ", returnedValue);

        // Struct to store intermediate data during calculation
        IERC20Upgradeable(USDC).safeTransfer(strategyAddresses.klpRewardHandler, returnedValue);
        returnedValue = IKlpRewardHandler(strategyAddresses.klpRewardHandler).swap(USDC, strategyAddresses.USDC, address(this), returnedValue);
        require(returnedValue > 0, "Cannot swap");
    ...
    function liquidatePosition(uint256 _positionId, address _user) external nonReentrant onlyKeeper{
        ...
        IERC20Upgradeable(USDC).safeTransfer(strategyAddresses.klpRewardHandler, outputAmount);
        outputAmount = IKlpRewardHandler(strategyAddresses.klpRewardHandler).swap(USDC, strategyAddresses.USDC, address(this), outputAmount);
        require(outputAmount > 0, "Cannot swap");
