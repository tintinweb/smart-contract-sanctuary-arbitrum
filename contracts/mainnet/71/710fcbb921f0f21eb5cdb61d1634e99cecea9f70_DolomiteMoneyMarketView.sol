...
contract DolomiteMoneyMarket is BaseMoneyMarket {
    ...
    function debtBalance(PositionId positionId, IERC20 asset) public view returns (uint256 balance) {
        IDolomiteMargin.Info memory self = _isIsolationMode() ? IDolomiteMargin.Info(address(vault), positionId.asUint()) : _self();
    ...
    function _lend(PositionId positionId, IERC20 asset, uint256 amount, address payer) internal override returns (uint256 actualAmount) {
        ...
        if (_isIsolationMode()) {
            ...
            vault.openBorrowPosition(0, positionId.asUint(), actualAmount);
        } else {
    ...
    function _borrow(PositionId positionId, IERC20 asset, uint256 amount, address to) internal override returns (uint256 actualAmount) {
        if (_isIsolationMode()) vault.transferFromPositionWithOtherToken(positionId.asUint(), 0, 0, amount, BalanceCheck.None);
    ...
    function _repay(PositionId positionId, IERC20 asset, uint256 amount, address payer) internal override returns (uint256 actualAmount) {
        ...
        if (actualAmount > 0) {
            ...
            if (_isIsolationMode()) vault.transferIntoPositionWithOtherToken(0, positionId.asUint(), 0, actualAmount, BalanceCheck.None);
        }
    }
    ...
    function _withdraw(PositionId positionId, IERC20 asset, uint256 amount, address to) internal override returns (uint256 actualAmount) {
        if (_isIsolationMode()) {
            vault.transferFromPositionWithUnderlyingToken(positionId.asUint(), 0, amount);
