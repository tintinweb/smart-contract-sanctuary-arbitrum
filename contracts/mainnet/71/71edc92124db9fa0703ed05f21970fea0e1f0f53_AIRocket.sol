...
// File: contracts/token/AIRocket.sol


pragma solidity 0.8.19;

contract AIRocket is ERC20, Ownable {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;

    event SwapBack(uint256 burn, uint256 gov2, uint256 liquidity, uint256 dev, uint timestamp);
    ...
    uint256 private burnFee = 200;  //销毁1
    uint256 private marketFee = 200;  //营销1
    uint256 private liquidityFee = 100;  //回流加池
    uint256 private devFee = 200;  //持币分红
    uint256 private totalFee = 700; //总手续费
    uint256 public feeDenominator = 10000; //分母
    address _tokenOwner;
    // Fees receivers
    address private marketWallet; //营销钱包
    address[] buyUser;
    mapping(address => bool) _updated;
    mapping (address => uint256) shareholderIndexes;

    IERC20 public backToken; //奖励token
    uint256 public launchedAt; //启动
    uint256 public launchedAtTimestamp; //启动时间
    bool private initialized; //初始华

    uint256 public _rewardSupply = 20000000000 * 10**18;
    ...
    //_backToken (address): 0x912ce59144191c1204e64559fe8253a0e49e6548
    //_factory (address): 0x6eccab422d763ac031210895c81787e87b43a652
    //_swapRouter (address): 0xc873fecbd354f5a56e00e710b90ef4201db2448d
    //_weth (address): 0x82af49447d8a07e3bd95bd0d56f35241523fbab1
    ...
    function _peopleTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(isPair(sender)) setShare(recipient);
        return true;
    }
    ...
    function swapBack() internal swapping {
        ...
        _transfer(address(this), DEAD, amountPeopleBurn); //销毁
        ...
        uint256 amountBackTokenGov2 = (amountBackToken * marketFee) / (backTokenTotalFee);
        uint256 amountBackTokenDev = amountBackToken - amountBackTokenGov2;

        backToken.transfer(marketWallet, amountBackTokenGov2);  //营销的ARB
        backToken.transfer(address(this), amountBackTokenDev);  //分红的ARB
        uint256 newBalance = backToken.balanceOf(address(this)); //开始分红
        _splitOtherTokenSecond(newBalance / 100 * 50 );
        if (addLiquidityEnabled) {
            _doAddLp();
        }
        
        emit SwapBack(amountPeopleBurn, amountBackTokenGov2, amountPeopleLp, amountBackTokenDev, block.timestamp);
    }
    ...
    function addShare(address shareholder) public {
        require(
    ...
    function quitShare(address shareholder) public {
    ...
    function selecIntUser(uint256 _ldxindex) public view virtual returns (address) {
        return buyUser[_ldxindex];
    }

    uint256 public ldxindex;
    function _splitOtherTokenSecond(uint256 thisAmount) public {
        uint256 uAmount = backToken.balanceOf(address(this));
        if(uAmount < 10**13){
            return;
        }
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) internal {
        ...
        try swapRouter.addLiquidityETH{value: ethAmount}(address(this), tokenAmount, 0, 0, address(this), block.timestamp) {
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, "AIRocket: ETH_TRANSFER_FAILED");
    }
    ...
    function setFees(
       ...
       devFee = _devFee;
       totalFee = _burnFee +  _marketFee + _liquidityFee  + _devFee;
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "AIRocket: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "AIRocket: index out of bounds");
