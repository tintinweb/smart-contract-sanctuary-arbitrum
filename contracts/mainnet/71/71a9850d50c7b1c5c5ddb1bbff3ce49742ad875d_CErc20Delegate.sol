...
contract CTokenStorage {
    ...
    /**
     * @notice Is the underlying token VLP
     */
    bool public isVLP;

    /**
     * @notice Wether or not the eth rewards from vlp market should be autocompounded
     */
    bool public autocompound;

    /**
     * @notice VLP reward router for claiming rewards
     */
    IVLPTokenFarm public vlpTokenFarm;

    /**
     * @notice VLP Adress to call transfer on
     */
    IVLP public vlpToken;

    /**
     * @notice address of the Vela token
     */
    address public velaToken;

    /**
     * @notice address of the esVela token
     */
    address public esVelaToken;

    /**
     * @notice Staked VLP Adress to call transfer on
     */
    address public immutable WETH = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1;

    /**
     * @notice VLP Orderbook contract to approve transfers on for autocompounding
     */
    address public vlpVault;
    ...
    // used for calculating interest rate performance of VLP vault market
...
abstract contract CTokenInterface is CTokenStorage {
    ...
    function _setVlpAddresses(IVLP vlp_, IVLPTokenFarm vlpTokenFarm_, address vlpVault_, address velaToken_, address esVela_) virtual public returns (uint);
...
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter {
     ...
     * @param isVLP_ Wether or not the market being created is for the VLP token
     */
    ...
    function compoundFresh() internal {
        if(totalSupply == 0 || !isVLP) {
            return;
        }

        accrueInterest();

        // This is only performed on VLP tokens

        /* Remember the initial block number */
        uint currentBlockNumber = getBlockNumber();
        uint accrualBlockNumberPrior = accrualBlockNumber;
        uint _vlpBlockDelta = sub_(currentBlockNumber, accrualBlockNumberPrior);

        if(_vlpBlockDelta < autoCompoundBlockThreshold){
            return;
        }

        vlpBlockDelta = _vlpBlockDelta;
    ...
    /**
     * @notice Restaking VLP
     */
    function stakeVLPFresh() internal {
        uint256 _balance = vlpToken.balanceOf(address(this));
        if(_balance > 0){
    ...
    address internal constant GLOBAL_PRICE_ORACLE = address(0xC0da48abD727CD7f582700C8ca7bc8cd2aAd4657);
    ...
    function accrueInterest() virtual override public returns (uint) {
        ...
        if (isVLP){
            ...
            if(price > lastVlpDepositAmount){
                ...
                feeOut = div_(mul_(total_, feeOut),1e18);
                if(feeOut > 0){
                    doTransferOut(admin, feeOut);
                }
                stakeVLPFresh();
            }
            return NO_ERROR;
    ...
    function mintFresh(address minter, uint mintAmount) internal {
        ...
        if (accrualBlockNumber != getBlockNumber() && !isVLP) {
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal {
        ...
        if(isVLP && !isRedeemerVip && withdrawFee > 0){
    ...
    /**
     * @notice Updates the addresses for the VLP contracts using _setVlpAddresses
     * @dev Admin function to update the stakedVLP contract address
     * @param vlp_ the vlp contract to use
     * @param vlpTokenFarm_ the rewardrouter contract address to use
     * @param vlpVault_ the vlpVault contract address to use
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setVlpAddresses(IVLP vlp_, IVLPTokenFarm vlpTokenFarm_, address vlpVault_, address velaToken_, address esVela_) override public returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            revert SetVlpAddressOwnerCheck();
        }
        vlpToken = vlp_;
        vlpTokenFarm = vlpTokenFarm_;
        vlpVault = vlpVault_;
        esVelaToken = esVela_;
        velaToken = velaToken_;
        return NO_ERROR;
    }
...
contract CErc20 is CToken, CErc20Interface {
     ...
     * @param isVLP_ Whether or not the market being created is for the VLP token
     */
    function initialize(address underlying_,
        ...
        super.initialize(comptroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_, isVLP_);
    ...
    function isGLP() external view returns (bool) {
        return isVLP; // Used to maintain compatibility
    }
