...
contract LaborGrounds is Initializable, Ownable {
    ...
    mapping(address => uint256) private jobsCompleted;
...
*/
    ...
    function getJobsCompleted(address _player) external view returns (uint256) {
        return jobsCompleted[_player];
    }

    function setPreviousJobsCompleted(
        address[] calldata _players,
        uint256[] calldata _amount
    ) external onlyOwner {
        for (uint256 i; i < _players.length; i++)
            jobsCompleted[_players[i]] = _amount[i];
    }
