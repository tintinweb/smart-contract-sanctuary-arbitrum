...
// File: contracts/pulsedropSnapshot.sol
...
contract pulseDropSnapshot is Ownable {
    ...
    mapping(bytes32 => bool) processedAddresses;
    ...
    function snapshot() external onlyOwner {
        ...
        uint256 numberOfParticipants = 342; // Assuming there are 341 active participants

        for (uint256 i = 0; i < numberOfParticipants; i++) {
            address participant = staker.participants(i);
            bytes32 addressHash = keccak256(abi.encodePacked(participant));

            if (!processedAddresses[addressHash]) {
                ...
                activeFarmersLength++;
                processedAddresses[addressHash] = true;
            }
        }
    }
