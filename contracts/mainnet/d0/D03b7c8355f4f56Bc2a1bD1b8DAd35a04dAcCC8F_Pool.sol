...
contract Pool is IgnitionList, IPool, PoolStorage, BasePausable, EIP712Upgradeable {
    ...
    modifier afterLockupTime() {
        (uint64 _TGEDate, , , , ) = vesting.getVestingInfo();
        if (!isPrivateRaise && block.timestamp < (ignitionFactory.getLockupDuration() + _TGEDate)) {
            revert Errors.NotAllowedToTransferBeforeLockupTime();
        }
        if (isPrivateRaise && block.timestamp < communityCloseTime) {
            revert Errors.NotAllowedToTransferBeforeCommunityClose();
        }
        _;
    }
    ...
    function initialize(
        ...
        }
        {
            ...
            feeType = SafeCast.toUint8(uints[18]);
            if (feeType > 1) {
                feeType = 1;
            }
        }
    ...
    function buyTokenInCrowdfundingPool(
        ...
        _internalNormalUserBuyToken(proof, _purchaseAmount, _poolLimit);
    }
    ...
    */
    ...
    function withdrawRedundantIDOToken(
        ...
        if (isFailBeforeTGEDate()) {
            ...
            (uint64 _TGEDate, , , , ) = vesting.getVestingInfo();
            if (block.timestamp < _TGEDate) {
                revert Errors.NotAllowedToTransferBeforeTGEDate();
            }
            redundantAmount =
    ...
    function _internalNormalUserBuyToken(
        ...
        if (_poolLimit > 0 && _purchaseAmount > _poolLimit) {
            revert Errors.ExceedPoolPurchaseAmountForUser();
        }
    ...
    function _internalBuyToken(
        ...
        if (feeType == 1) {
            ...
            uint feeTokenAmount = getIDOTokenAmountByOfferedCurrency(
                participationFee
            );
            vesting.createVestingSchedule(buyer, IDOTokenAmount);
            vesting.createVestingSchedule(ignitionFactory.ifowner(), feeTokenAmount);
...
interface IIgnitionFactory {
    function ifowner() external view returns (address);
