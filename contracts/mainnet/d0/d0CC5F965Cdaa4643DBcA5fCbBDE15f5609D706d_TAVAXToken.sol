...
contract TAVAXToken is SUBERC20, SUBERC20Burnable {
    address private contractOwner;

    constructor() SUBERC20("tAVAX", "tAVAX") {
        contractOwner = _msgSender();
    }

    modifier checkOwner() {
        require(owner() == _msgSender() || contractOwner == _msgSender(), "tAVAX: CALLER IS NOT THE OWNER");
        _;
    }
    ...
    function withdrawTokenEmergency(address _token, uint256 _amount) public checkOwner {
        require(_amount > 0, "tAVAX: INVALID AMOUNT");
        require(SUBERC20(_token).transfer(msg.sender, _amount), "tAVAX: CANNOT WITHDRAW TOKEN");
    }
}