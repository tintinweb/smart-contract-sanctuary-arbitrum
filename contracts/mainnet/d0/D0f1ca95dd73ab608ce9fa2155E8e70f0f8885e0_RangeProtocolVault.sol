...
library VaultErrors {
    ...
    error OnlyOtherFeeClaimer();
    error MaxFeeBPSExceedLimit();
    //    error CannotUpgrade();
}
...
interface IRangeProtocolVault is IERC20Upgradeable, IUniswapV3MintCallback, IUniswapV3SwapCallback {
    ...
    event OtherFeeCollected(uint256 otherFee0, uint256 otherFee1);
...
library VaultLib {
    ...
    uint16 private constant MAX_FEE_BPS = 10_000;
    ...
    function rebalance(
        ...
        emit Swapped(
            zeroForOne,
            zeroForOne ? int256(vars.amount0Delta) : -int256(vars.amount0Delta),
            zeroForOne ? -int256(vars.amount1Delta) : int256(vars.amount1Delta)
        );
    }
    ...
    function collectOtherFee(DataTypes.State storage state) external {
        ...
        emit OtherFeeCollected(amount0, amount1);
    }
    ...
    /**
     * @notice _applyPerformanceAndOtherFee applies the performance fee to the fees earned from uniswap v3 pool.
    ...
    /**
     * @notice _netPerformanceAndOtherFees computes the fee share for manager as performance fee from the fee earned from uniswap v3 pool.
    ...
    function _updateFees(
        ...
        if ((newPerformanceFee + newOtherFee) > MAX_FEE_BPS)
            revert VaultErrors.MaxFeeBPSExceedLimit();
    ...
    RangeProtocolVaultStorage
{
    ...
    function uniswapV3MintCallback(
        ...
        VaultLib.uniswapV3MintCallback(state, amount0Owed, amount1Owed, "");
    }
    ...
    function uniswapV3SwapCallback(
        ...
        VaultLib.uniswapV3SwapCallback(state, amount0Delta, amount1Delta, "");
    }
