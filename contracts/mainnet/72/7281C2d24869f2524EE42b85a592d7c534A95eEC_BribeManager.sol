...
contract BribeManager is IBribeManager, OwnableUpgradeable {
  ...
  function setCastVoteCoolDown(uint256 _castVotesCooldown) external onlyOwner {
  ...
  function vote(
    ...
    for (uint256 i; i < length; i++) {
      ...
      if (delta != 0) {
        if (delta > 0) {
          poolTotalVote[pool.lpToken] = poolTotalVote[pool.lpToken].add(
            uint256(delta)
          );
          userTotalVote[msg.sender] = userTotalVote[msg.sender].add(
            uint256(delta)
          );
          userVoteForPools[msg.sender][pool.lpToken] = userVoteForPools[
            msg.sender
          ][pool.lpToken].add(uint256(delta));
          ...
          userVoteForPools[msg.sender][pool.lpToken] = userVoteForPools[
            msg.sender
          ][pool.lpToken].sub(uint256(-delta));
    ...
    if (msg.sender != delegatePool) {
      ...
      if (totalUserVote > 0) {
        ...
        totalVlQuoInVote = totalVlQuoInVote.sub(uint256(-totalUserVote));
      }
    }
    require(
  ...
  function unvote(address _lp) external override {
    ...
    poolTotalVote[pool.lpToken] = poolTotalVote[pool.lpToken].sub(currentVote);
    ...
    )
  {
    ...
    for (uint256 i; i < length; i++) {
      ...
      uint256 targetVote = poolTotalVote[pool.lpToken].mul(totalVotes()).div(
        totalVlQuoInVote
      );
    ...
    if (_swapForNative) {
      for (uint256 i = 0; i < length; i++) {
        ...
        finalFeeAmounts[i][0] = finalFeeAmounts[i][0].add(
          _swapFeesForNative(rewardTokens[i], feeAmounts[i], msg.sender)
        );
      }
    } else {
      for (uint256 i = 0; i < length; i++) {
  ...
  function getRewardForPools(address[] calldata _lps) external {
    ...
    for (uint256 i; i < length; i++) {
      if (_lps[i] == delegatePool) {
        ...
        IVirtualBalanceRewardPool(poolInfos[_lps[i]].rewarder).getReward(
          msg.sender
        );
      }
    }
    ...
    returns (address[][] memory rewardTokens, uint256[][] memory earnedRewards)
  {
    ...
    if (userVoteForPools[msg.sender][delegatePool] > 0) {
      (delegatePoolRewardTokens, delegatePoolRewardAmounts) = IDelegateVotePool(
        delegatePool
    ...
    for (uint256 i; i < length; i++) {
      ...
      if (userVoteForPools[msg.sender][pool.lpToken] > 0) {
        ...
        for (uint256 j = 0; j < rewardTokens[i].length; j++) {
          earnedRewards[i][j] = IVirtualBalanceRewardPool(pool.rewarder).earned(
            msg.sender,
            rewardTokens[i][j]
          );
        }
  ...
  function previewNativeAmountForCast(
    ...
    (address[][] memory rewardTokens, uint256[][] memory amounts) = voterProxy
      ...
      for (uint256 j = 0; j < rewardTokens[i].length; j++) {
        ...
        if (rewardTokens[i][j] == AddressLib.PLATFORM_TOKEN_ADDRESS) {
          ...
          feeAmount = feeAmount.add(
            nativeZapper.getAmountOut(rewardTokens[i][j], amounts[i][j])
          );
        }
      }
    }
  ...
  {
    Pool memory pool = poolInfos[_lp];
    rewardTokens = IVirtualBalanceRewardPool(pool.rewarder).getRewardTokens();
    ...
    for (uint256 i; i < length; i++) {
      ...
      if (AddressLib.isPlatformToken(rewardTokens[i])) {
        ...
        nativeAmount = nativeAmount.add(
          nativeZapper.zapInToken(rewardTokens[i], feeAmounts[i], _receiver)
        );
      }
    }
  }
