...
contract HorizaZap is OdosZap {
    ...
    function zapIn(SwapData memory swapData, HorizaZapInParams memory horizaData) external {
        _prepareSwap(swapData);
        _swap(swapData);

        IUniswapV3Pool pair = IUniswapV3Pool(horizaData.pair);
        ...
        for (uint256 i = 0; i < tokensOut.length; i++) {
            ...
            if (horizaData.amountsOut[i] > 0) {
                asset.transferFrom(msg.sender, address(this), horizaData.amountsOut[i]);
            }
    ...
    function getProportion(
        ...
        uint256 reserve1 = token1.balanceOf(address(pair));
    ...
    function _addLiquidity(IUniswapV3Pool pair, address[] memory tokensOut, uint256[] memory amountsOut) internal {
        uint256 reserve0 = IERC20Metadata(tokensOut[0]).balanceOf(address(pair));
        uint256 reserve1 = IERC20Metadata(tokensOut[1]).balanceOf(address(pair));
        ...
        INonfungiblePositionManager.MintParams memory params = 
            INonfungiblePositionManager.MintParams(tokensOut[0], tokensOut[1], pair.fee(),
                tick-4*tickSpacing, tick+4*tickSpacing, tokensAmount0, tokensAmount1, 0, 0, msg.sender, block.timestamp);
