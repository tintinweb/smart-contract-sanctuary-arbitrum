...
/// @title Controller of all vaults in the USDl borrow/lend system
  ...
  OwnableUpgradeable
{
  ...
  ///@notice any function with this modifier can be paused by USDL._pauser() in the case of an emergency
  modifier onlyPauser() {
    require(_msgSender() == _usdl.pauser(), "only pauser");
    _;
  }
  ...
  /// @notice register the USDl contract
  /// @param usdl_address address to register as USDl
  function registerUSDl(address usdl_address) external override onlyOwner {
    _usdl = IUSDL(usdl_address);
  }
  ...
  function checkVault(uint96 id) public view override returns (bool) {
    ...
    uint256 usdl_liability = truncate((vault.baseLiability() * _interest.factor));
    // if the LTV >= liability, the vault is solvent
    return (total_liquidity_value >= usdl_liability);
  }

  /// @notice borrow USDl from a vault. only vault minter may borrow from their vault
  /// @param id vault to borrow against
  /// @param amount amount of USDl to borrow
  function borrowUsdl(uint96 id, uint192 amount) external override {
    _borrowUSDl(id, amount, _msgSender());
  }

  /// @notice borrow USDl from a vault and send the USDl to a specific address
  /// @notice Only vault minter may borrow from their vault
  /// @param id vault to borrow against
  /// @param amount amount of USDl to borrow
  /// @param target address to receive borrowed USDl
  function borrowUSDLto(
    uint96 id,
    uint192 amount,
    address target
  ) external override {
    _borrowUSDl(id, amount, target);
  }

  /// @notice business logic to perform the USDl loan
  /// @param id vault to borrow against
  /// @param amount amount of USDl to borrow
  /// @param target address to receive borrowed USDl
  /// @dev pays interest
  function _borrowUSDl(
    uint96 id,
    uint192 amount,
    address target
    ...
    // the base amount is the amount of USDl they wish to borrow divided by the interest factor
    ...
    uint256 usdl_liability = truncate(uint256(_interest.factor) * base_liability);
    // now get the LTV of the vault, aka their borrowing power, in usdl
    uint256 total_liquidity_value = get_vault_borrowing_power(vault);
    // the LTV must be above the newly calculated usdl_liability, else revert
    require(total_liquidity_value >= usdl_liability, "vault insolvent");
    // now send usdl to the target, equal to the amount they are owed
    _usdl.vaultControllerMint(target, amount);
    // emit the event
    emit BorrowUSDl(id, address(vault), amount);
  }

  /// @notice borrow USDC directly from reserve
  /// @notice liability is still in USDl, and USDl must be repaid
  /// @param id vault to borrow against
  ...
  function borrowUSDCto(
    ...
    _usdl.vaultControllerTransfer(target, usdc_amount);
  }

  /// @notice repay a vault's USDl loan. anyone may repay
  /// @param id vault to repay
  /// @param amount amount of USDl to repay
  /// @dev pays interest
  function repayUSDl(uint96 id, uint192 amount) external override paysInterest whenNotPaused {
    // grab the vault by id if part of our system. revert if not
    IVault vault = getVault(id);
    // the base amount is the amount of USDl entered divided by the interest factor
    ...
    // burn the amount of USDl submitted from the senders vault
    _usdl.vaultControllerBurn(_msgSender(), amount);
    // emit the event
    emit RepayUSDl(id, address(vault), amount);
  }

  /// @notice repay all of a vaults USDl. anyone may repay a vaults liabilities
  /// @param id the vault to repay
  /// @dev pays interest
  function repayAllUSDl(uint96 id) external override paysInterest whenNotPaused {
    // grab the vault by id if part of our system. revert if not
    IVault vault = getVault(id);
    // get the total USDl liability, equal to the interest factor * vault's base liabilty
    //uint256 usdl_liability = truncate(safeu192(_interest.factor * vault.baseLiability()));
    uint256 usdl_liability = uint256(safeu192(truncate(_interest.factor * vault.baseLiability())));
    // decrease the vaults liability by the vauls base liability
    vault.modifyLiability(false, vault.baseLiability());
    // burn the amount of USDl paid back from the vault
    _usdl.vaultControllerBurn(_msgSender(), usdl_liability);

    emit RepayUSDl(id, address(vault), usdl_liability);
  }
  ...
  function liquidateVault(
    ...
    // the USDl to repurchase is equal to the bad fill price multiplied by the amount of tokens to liquidate
    uint256 usdl_to_repurchase = truncate(badFillPrice * tokens_to_liquidate);
    // get the vault that the liquidator wishes to liquidate
    IVault vault = getVault(id);

    //decrease the vault's liability
    vault.modifyLiability(false, (usdl_to_repurchase * 1e18) / _interest.factor);

    //decrease liquidator's USDl balance
    _usdl.vaultControllerBurn(_msgSender(), usdl_to_repurchase);
    ...
    // emit the event
    emit Liquidate(id, asset_address, usdl_to_repurchase, tokens_to_liquidate);
  ...
  ///@notice amount of USDl needed to reach even solvency
  ...
  /// @param id id of vault
  /// @return amount of USDl the vault owes
  ...
  /// @notice accrue interest to borrowers and distribute it to USDl holders.
  ...
  function pay_interest() private returns (uint256) {
    ...
    uint256 ui18 = uint256(_usdl.reserveRatio());
    // cast the reserve ratio now to an int in order to get a curve value
    int256 reserve_ratio = int256(ui18);

    // calculate the value at the curve. this vault controller is a USDl vault and will refernce
    ...
    // this donation is what pays out interest to USDl holders
    _usdl.vaultControllerDonate(valueAfter - valueBefore - protocolAmount);
    // send the protocol's fee to the owner of this contract.
    _usdl.vaultControllerMint(owner(), protocolAmount);
    // emit the event
