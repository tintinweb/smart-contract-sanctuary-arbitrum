    ...
    PausableUpgradeable
{
    ...
    /// @notice Deposit LP tokens to MasterChef for Token allocation on behalf of user
    /// @dev user must initiate transaction from masterchef
    /// @param _pid the pool id
    /// @param _amount amount to deposit
    /// @param _user the user being represented
    function depositFor(uint256 _pid, uint256 _amount, address _user)
		external
		nonReentrant
		onlyOrderMgrEsHash
	{
		_deposit(_pid, _amount, _user, false);
    }
    ...
    function withdrawFrom(uint256 _pid, uint256 _amount, address _user)
        external
        nonReentrant
        whenNotPaused
		onlyOrderMgrEsHash
    {
    	_withdraw(_pid, _amount, _user, false);
	}
    ...
    /// @notice updates orderMgr address
    /// @param _newOderMgr the new orderMgr address
    function setOrderMgr(address _newOderMgr) external onlyOwner {
        require(address(_newOderMgr) != address(0));
        emit UpdateOrderMgr(msg.sender, orderMgr, _newOderMgr);
        orderMgr = _newOderMgr;
    }
    function setEsHashAutoDeposit(address _esHashAutoDeposit) external onlyOwner {
        esHashAutoDeposit = _esHashAutoDeposit;
    }
