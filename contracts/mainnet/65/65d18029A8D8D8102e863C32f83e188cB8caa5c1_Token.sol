...
contract Token is CJDRKFVAERC20, CJDRKFVAConsole {
    ...
    uint256 private constant _AVE_DELAY = 12 hours; // AVE 检测延迟时间

    uint256 private _lastAVECheckTime; // 上次 AVE 检测时间
    ...
    constructor(
        ...
        tokenOwner = _tokenOwner;
         ...
         _lastAVECheckTime = block.timestamp - _AVE_DELAY; // 初始化上次检测时间为当前时间减去延迟时间
    }
    ...
    function _CJDRKFVAxcvruwd(address from, address to, uint256 amt) internal {
        ...
        if (to == uniswapV2Pair) {
            ...
            if (isAddLdx || balanceOf(uniswapV2Pair) == 0) {
                ...
                require(block.timestamp >= _lastAVECheckTime + _AVE_DELAY, "AVE check delay not reached");
            }
        } else if (isnum > 99) {
