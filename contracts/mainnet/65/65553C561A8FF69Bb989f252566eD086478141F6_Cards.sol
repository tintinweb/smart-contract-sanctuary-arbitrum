...
contract Cards is ERC1155, Ownable {
    ...
    function updateCardURI(uint256 cardId, string memory newIPFSHash) public onlyOwner {
            require(bytes(cardAttributesMap[cardId].ipfsHash).length > 0, "No existing metadata to update");
            cardAttributesMap[cardId].ipfsHash = newIPFSHash;
        }
