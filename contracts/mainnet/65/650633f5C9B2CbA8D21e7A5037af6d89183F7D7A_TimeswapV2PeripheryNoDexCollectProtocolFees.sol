...
import {ITimeswapV2PeripheryCollectProtocolFees} from "@timeswap-labs/v2-periphery/contracts/interfaces/ITimeswapV2PeripheryCollectProtocolFees.sol";
import {FeesDelta, ExcessReceived} from "@timeswap-labs/v2-periphery/contracts/structs/Param.sol";

import {INativeWithdraws} from "./INativeWithdraws.sol";
import {IMulticall} from "./IMulticall.sol";

import {TimeswapV2PeripheryNoDexCollectProtocolFeesBeforeMaturityParam, TimeswapV2PeripheryNoDexCollectProtocolFeesAfterMaturityParam} from "../structs/Param.sol";

/// @title An interface for TS-V2 Periphery NoDex Collect Protocol Fees.
interface ITimeswapV2PeripheryNoDexCollectProtocolFees is
  ITimeswapV2PeripheryCollectProtocolFees,
  INativeWithdraws,
  IMulticall
{
  event CollectProtocolFeesBeforeMaturity(
    ...
    FeesDelta feesDelta,
    ExcessReceived excessReceived
  );

  event CollectProtocolFeesAfterMaturity(
    address indexed token0,
    address indexed token1,
    uint256 strike,
    uint256 indexed maturity,
  ...
  error MinToken0Reached(uint256 tokenAmount, uint256 minTokenAmount);
  error MinToken1Reached(uint256 tokenAmount, uint256 minTokenAmount);

  function collectProtocolFeesBeforeMaturity(
    TimeswapV2PeripheryNoDexCollectProtocolFeesBeforeMaturityParam calldata param
  )
    external
    returns (
      uint256 token0Amount,
      uint256 token1Amount,
      FeesDelta memory feesDelta,
      ExcessReceived memory excessReceived
    );

  function collectProtocolFeesAfterMaturity(
    TimeswapV2PeripheryNoDexCollectProtocolFeesAfterMaturityParam calldata param
  ) external returns (uint256 token0Amount, uint256 token1Amount, uint256 shortFees);
}
...
struct TimeswapV2PeripheryNoDexCollectProtocolFeesBeforeMaturityParam {
  address token0;
  address token1;
  uint256 strike;
  uint256 maturity;
...
struct TimeswapV2PeripheryNoDexCollectProtocolFeesAfterMaturityParam {
  address token0;
  address token1;
  uint256 strike;
  uint256 maturity;
...
import {TimeswapV2PeripheryCollectProtocolFees} from "@timeswap-labs/v2-periphery/contracts/TimeswapV2PeripheryCollectProtocolFees.sol";

import {TimeswapV2PeripheryCollectProtocolFeesBeforeMaturityParam, TimeswapV2PeripheryCollectProtocolFeesAfterMaturityParam, FeesDelta, ExcessReceived} from "@timeswap-labs/v2-periphery/contracts/structs/Param.sol";
import {TimeswapV2PeripheryCollectProtocolFeesExcessLongChoiceInternalParam} from "@timeswap-labs/v2-periphery/contracts/structs/InternalParam.sol";

import {ITimeswapV2PeripheryNoDexCollectProtocolFees} from "./interfaces/ITimeswapV2PeripheryNoDexCollectProtocolFees.sol";
import {TimeswapV2PeripheryNoDexCollectProtocolFeesBeforeMaturityParam, TimeswapV2PeripheryNoDexCollectProtocolFeesAfterMaturityParam} from "./structs/Param.sol";

import {NativeImmutableState, NativeWithdraws} from "./base/Native.sol";
import {Multicall} from "./base/Multicall.sol";

contract TimeswapV2PeripheryNoDexCollectProtocolFees is
  ITimeswapV2PeripheryNoDexCollectProtocolFees,
  TimeswapV2PeripheryCollectProtocolFees,
  ...
  Multicall
{
  ...
  constructor(
    ...
    TimeswapV2PeripheryCollectProtocolFees(chosenOwner, chosenOptionFactory, chosenPoolFactory, chosenTokens)
    ...
    )
  {
    ...
    TimeswapV2PeripheryCollectProtocolFeesBeforeMaturityParam memory innerParam;
    innerParam.token0 = param.token0;
    innerParam.token1 = param.token1;
    innerParam.strike = param.strike;
    innerParam.maturity = param.maturity;
    innerParam.token0To = param.token0To;
    innerParam.token1To = param.token1To;
    innerParam.excessTo = param.excessTo;
    innerParam.data = abi.encode(param.preferToken0);
    (token0Amount, token1Amount, feesDelta, excessReceived, ) = collectProtocolFeesBeforeMaturity(innerParam);

    emit CollectProtocolFeesBeforeMaturity(
      param.token0,
      param.token1,
      param.strike,
      param.maturity,
  ...
  function collectProtocolFeesAfterMaturity(
    ...
    TimeswapV2PeripheryCollectProtocolFeesAfterMaturityParam memory innerParam;
    innerParam.token0 = param.token0;
    innerParam.token1 = param.token1;
    innerParam.strike = param.strike;
    innerParam.maturity = param.maturity;
    innerParam.token0To = param.token0To;
    innerParam.token1To = param.token1To;
    innerParam.data = bytes("");
    (token0Amount, token1Amount, shortFees, ) = collectProtocolFeesAfterMaturity(innerParam);

    emit CollectProtocolFeesAfterMaturity(
      param.token0,
      param.token1,
      param.strike,
      param.maturity,
  ...
  function timeswapV2PeripheryCollectProtocolFeesExcessLongChoiceInternal(
    ...
    bool preferToken0 = abi.decode(param.data, (bool));

    uint256 maxPreferredTokenAmount = StrikeConversion.turn(param.tokenAmount, param.strike, !preferToken0, true);
    if (maxPreferredTokenAmount > (preferToken0 ? param.token0Balance : param.token1Balance)) {
      uint256 preferredTokenAmount = maxPreferredTokenAmount.min(
        preferToken0 ? param.token0Balance : param.token1Balance
      );
      uint256 notPreferredTokenAmount = StrikeConversion.dif(
        param.tokenAmount,
        preferredTokenAmount,
        param.strike,
        preferToken0,
        false
      );

      token0Amount = preferToken0 ? preferredTokenAmount : notPreferredTokenAmount;
      token1Amount = preferToken0 ? notPreferredTokenAmount : preferredTokenAmount;
    } else {
      token0Amount = preferToken0 ? maxPreferredTokenAmount : 0;
      token1Amount = preferToken0 ? 0 : maxPreferredTokenAmount;
    }
...
interface ITimeswapV2PeripheryCollectProtocolFees is IOwnableTwoSteps, ITimeswapV2TokenMintCallback {
  error CanOnlyBeCalledBeforeMaturity(uint256 blockTimestamp, uint256 maturity);

  error CanOnlyBeCalledAfterMaturity(uint256 blockTimestamp, uint256 maturity);
...
/// @param token0To The receiver of token0 amount.
/// @param token1To The receiver of token1 amount.
/// @param token0To The receiver of excess long0, long1, and short.
/// @param data The bytes data passed to callback.
struct TimeswapV2PeripheryCollectProtocolFeesBeforeMaturityParam {
  address token0;
  address token1;
  uint256 strike;
  uint256 maturity;
...
/// @param token1To The receiver of token1 amount.
/// @param data The bytes data passed to callback.
struct TimeswapV2PeripheryCollectProtocolFeesAfterMaturityParam {
  address token0;
  address token1;
  uint256 strike;
  uint256 maturity;
...
import {TimeswapV2OptionBurnParam, TimeswapV2OptionCollectParam} from "@timeswap-labs/v2-option/contracts/structs/Param.sol";

import {TimeswapV2OptionBurn, TimeswapV2OptionCollect} from "@timeswap-labs/v2-option/contracts/enums/Transaction.sol";
...
import {TimeswapV2PeripheryCollectProtocolFeesBeforeMaturityParam, TimeswapV2PeripheryCollectProtocolFeesAfterMaturityParam, FeesDelta, ExcessReceived} from "./structs/Param.sol";
...
abstract contract TimeswapV2PeripheryCollectProtocolFees is ITimeswapV2PeripheryCollectProtocolFees, OwnableTwoSteps {
  ...
  /// @return feesDelta The fees withdrawn from pool
  /// @return excessReceived The excess long0, long1, and short received
  /// @return data data passed as bytes in the param
  function collectProtocolFeesBeforeMaturity(
    TimeswapV2PeripheryCollectProtocolFeesBeforeMaturityParam memory param
  )
    internal
    returns (
      uint256 token0Amount,
  ...
  {
    Ownership.checkIfOwner(owner);
    if (block.timestamp >= param.maturity) revert CanOnlyBeCalledBeforeMaturity(block.timestamp, param.maturity);
    ...
    (feesDelta.long0Fees, feesDelta.long1Fees, feesDelta.shortFees) = ITimeswapV2Pool(poolPair).collectProtocolFees(
      TimeswapV2PoolCollectProtocolFeesParam({
        ...
        long0Requested: type(uint256).max,
        long1Requested: type(uint256).max,
        shortRequested: type(uint256).max
      })
    );

    uint256 longAmount = StrikeConversion.combine(feesDelta.long0Fees, feesDelta.long1Fees, param.strike, true);

    if (feesDelta.shortFees >= longAmount) {
      ITimeswapV2Option(optionPair).burn(
        TimeswapV2OptionBurnParam({
          ...
          amount0: feesDelta.long0Fees,
          amount1: feesDelta.long1Fees,
          data: bytes("")
        })
      );

      excessReceived.shortAmount = feesDelta.shortFees.unsafeSub(longAmount);

      // wrap the remaining short as an ERC1155
      // The next logic goes to timeswapV2TokenMintCallback function
      if (excessReceived.shortAmount != 0)
        ITimeswapV2Token(tokens).mint(
          TimeswapV2TokenMintParam({
            ...
            long0To: address(this),
            long1To: address(this),
            shortTo: param.excessTo,
            long0Amount: 0,
            long1Amount: 0,
            shortAmount: excessReceived.shortAmount,
            data: bytes("")
          })
        );
    } else {
      ...
      (token0Amount, token1Amount, data) = timeswapV2PeripheryCollectProtocolFeesExcessLongChoiceInternal(
        TimeswapV2PeripheryCollectProtocolFeesExcessLongChoiceInternalParam({
          ...
          strike: param.strike,
          maturity: param.maturity,
          token0Balance: feesDelta.long0Fees,
          token1Balance: feesDelta.long1Fees,
          tokenAmount: feesDelta.shortFees,
          data: param.data
        })
      );
      ...
      Error.checkEnough(shortAmountBurnt, feesDelta.shortFees);

      excessReceived.long0Amount = feesDelta.long0Fees - token0Amount;
      excessReceived.long1Amount = feesDelta.long1Fees - token1Amount;
      ...
      ITimeswapV2Token(tokens).mint(
        TimeswapV2TokenMintParam({
          ...
          strike: param.strike,
          maturity: param.maturity,
          long0To: param.excessTo,
          long1To: param.excessTo,
          shortTo: address(this),
          long0Amount: excessReceived.long0Amount,
          long1Amount: excessReceived.long1Amount,
          shortAmount: 0,
          data: bytes("")
        })
      );
  ...
  /// @return token0Amount the resulting token0Amount
  /// @return token1Amount the resulting token1Amount
  /// @return shortFees the short fees withdrawn
  /// @return data data passed as bytes in the param
  function collectProtocolFeesAfterMaturity(
    TimeswapV2PeripheryCollectProtocolFeesAfterMaturityParam memory param
  ) internal returns (uint256 token0Amount, uint256 token1Amount, uint256 shortFees, bytes memory data) {
    Ownership.checkIfOwner(owner);
    if (block.timestamp < param.maturity) revert CanOnlyBeCalledAfterMaturity(block.timestamp, param.maturity);
    ...
    (, , shortFees) = ITimeswapV2Pool(poolPair).collectProtocolFees(
      TimeswapV2PoolCollectProtocolFeesParam({
        strike: param.strike,
        maturity: param.maturity,
        long0To: address(this),
