...
// Deposit Eth, Liquidity Generation on Uniswap V3, Emergency Withdraw
...
contract PoisonSale {
    ...
    event InitializePool(address pool); 
    ...
    event ERC721Received(address operator, address from,uint tokenid);
    ...
    function depositEth() public payable {
        ...
        if (privateSaleClosed == false) {
            ...
            require(block.timestamp >= publicSaleOpenedAt && block.timestamp <= publicSaleOpenedAt + 5 days, "Time was reached");
            ...
            ISwapRouter.ExactInputSingleParams({
                ...
                amountIn: msg.value,
                amountOutMinimum: amount - (amount/33),
                sqrtPriceLimitX96: 0
            });

        // The call to `exactInputSingle` executes the swap.
            amount= uniswapRouter.exactInputSingle{ value: msg.value }(params);
            tokens = amount * multiplier / publicSaleRate;
            
            require(tokenBalances[msg.sender] + tokens >= 200000000000000000000 && tokenBalances[msg.sender] + tokens <= 30000000000000000000000, "Public sale limit");
            
            require(publicSaleSold + tokens <= publicSaleCap, "Cap reached");
            publicSaleSold = publicSaleSold + tokens;
            
            stableCoinContributed[msg.sender] = stableCoinContributed[msg.sender] + amount;
        }
    ...
    function closePublicSale(bool _initialize) external {

        require(publicSaleOpenedAt !=0, "Private sale open");
        require(publicSaleClosedAt == 0, "Public sale closed");
        require(block.timestamp > publicSaleOpenedAt + 7 days || (publicSaleSold >= publicSaleCap - 200000000000000000000 && publicSaleSold <= publicSaleCap), 'Too early');

        publicSaleClosedAt = block.timestamp;

        if (_initialize == true) {
        uint sqrtX96 = sqrt((uniswapRate / 100000000) / (multiplier / 100000000)) * 2 ** 96;
        uint160 sqrtX96160 = uint160(sqrtX96);
        
        pool = nonfungiblePositionManager.createAndInitializePoolIfNecessary(address(poison), address(stableCoin), 3000, sqrtX96160);
        }
    }
    ...
    function generateLiquidity() external {
        ...
        stableCoin.safeApprove(address(nonfungiblePositionManager), stableCoinBalance);
        ...
        poison.safeApprove(address(nonfungiblePositionManager), amountPoison);
        require(poison.balanceOf(address(this)) >= amountPoison, "Not enough Poison balance");

            INonfungiblePositionManager.MintParams memory params =
            INonfungiblePositionManager.MintParams({
                token0: address(poison),
                token1: address(stableCoin),
                fee: 3000,
                tickLower: (TickMath.MIN_TICK / TickMath.TICK_SPACING) * TickMath.TICK_SPACING,
                tickUpper: (TickMath.MIN_TICK / TickMath.TICK_SPACING) * TickMath.TICK_SPACING,
                amount0Desired: amountPoison,
                amount1Desired: stableCoinBalance,
                amount0Min: 0,
                amount1Min: 0,
                recipient: address(this),
                deadline: block.timestamp + 15 minutes
        ...
        // Note that the pool defined must already be created and initialized in order to mint
        ...
        emit LiquidityGenerated(amount0, amount1, liquidity);
    }
    ...
    function emergencyWithdrawCoins() external {
        ...
        require(publicSaleClosedAt != 0, "Public sale open");
        require(liquidityGeneratedAt == 0, "Liquidity generated");
        require(block.timestamp > publicSaleClosedAt + 30 minutes + 2 days, "Too early");
    ...
    function recoverLpToken(address _lpToken) external {
    require(msg.sender == developer, "Developer only");
    
        require(liquidityGeneratedAt != 0, "Liquidity not generated");
        require(block.timestamp >= liquidityGeneratedAt + 720 days, "Too early");

        IERC721 lpToken = IERC721(_lpToken);
        
        lpToken.safeTransferFrom(address(this), developer, ERC721TokenId);

        emit LpRecovered(developer, ERC721TokenId);
    }
    
    function addPrivateInvestors(address[] memory _address, uint[] memory _tokens) external {
    require(msg.sender == developer, "Developer only");
    
        require(privateSaleClosed == false, "Private sale closed");
        
        for (uint8 i = 0; i < _address.length; i++){
        privateSaleSold = privateSaleSold + _tokens[i];
        tokenBalances[_address[i]] = tokenBalances[_address[i]] + _tokens[i];
        }
    }
