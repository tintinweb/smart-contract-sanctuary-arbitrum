...
interface ISkillbook {
    function createBook(uint256[5] memory _startSkills, uint256 _wizId) external;  
}
...
contract Arcane is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    event WizardBorn(string name, string race, string archetype, string affinity, string identity1, string identity2);
   ...
   function getWizardInfosIds(uint _wizId) external view  returns(uint256[5] memory) {
       return _sliceDna(wizards[_wizId].dna);
   }

    function getWizardInfosString(uint256 _wizId) external view returns(string[6] memory){
    ...
    function getBirthday(uint _wizId) external view returns(uint256){
    ...
    function tokenURI(uint256 _wizId) public view override returns (string memory) {
        require (_exists(_wizId));
        return string(abi.encodePacked(BASE_URI,_wizId.toString()));
    }
    ...
    // ------------------------------------------------------
     ...
     function _mintWizard(uint256 _id, string memory _name) internal {
        ...
        uint256 dna = _createDNA(race,archetype, affinity,identity1,identity2);
        if(_id==0){ dna = 2021204; }
        wizards[_id].dna=dna;
        ...
        emit WizardBorn(_name,races[race],archetypes[archetype],affinities[affinity],identities[identity1], identities[identity2]);
    }

     function _randomize(string memory _salt, uint256 _limit) internal returns (uint256){
        randCounter++;
        if(randCounter>=1000) { randCounter = 0; }
        return uint256(keccak256(abi.encodePacked(_salt,block.timestamp,block.number, randCounter))) % _limit;
    }

   

    function _isContractConnected(address _caller) internal view returns(bool) {
     ...
     function _createDNA(uint256 _race, uint256 _archetype, uint256 _affinity, uint256 _identity1,uint256 _identity2) internal pure returns(uint256){
    ...
    function setConnected(uint256 _connectedId, address _address) external onlyOwner {
