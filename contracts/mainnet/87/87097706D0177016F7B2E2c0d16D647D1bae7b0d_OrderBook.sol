    ...
    IOrderBook
{
    ...
    function executeIncreaseOrder(
        ...
        IERC20Upgradeable(underlying).safeTransfer(vault, order.tokenAmount);
    ...
    function executeDecreaseOrder(
        ...
        (uint256 size, uint256 collateral, , , , , , ) = IVault(vault).getPosition(
            _address,
            order.indexToken,
            order.isLong
        );
        ...
        uint256 amountOut = IRouter(router).pluginDecreasePosition(
            order.account,
            order.indexToken,
            collateral < order.collateralDelta ? collateral: order.collateralDelta,
            size < order.sizeDelta ? size : order.sizeDelta,
            order.isLong,
            address(this)
        );
