...
contract BankV2 is Ownable {
    ...
    mapping(address => bool) gameIsAllowed;
    ...
    /// @notice Emitted after a game is allowed.
    /// @param game Address of the game.
    /// @param allowed Whether the token is allowed for betting.
    event SetAllowedGame(address indexed game, bool allowed);
    ...
    function updatePoolInterest(
        uint256 amount,
        address currency,
        bool isDecrease
    ) external payable 
     {
        require(gameIsAllowed[msg.sender] == true,"!game");
    ...
    function setAllowedGame(address game, bool allowed) external onlyOwner {
        gameIsAllowed[game] = allowed;
        emit SetAllowedGame(game, allowed);
    }
    ...
    function setTokenMinBetAmount(
        address token,
        uint256 tokenMinBetAmount
    ) external onlyOwner {
    ...
    function accountFees(
        address token,
        uint256 fees
    ) external payable onlyOwner // onlyRole(GAME_ROLE)
    {
    ...
    function setTreasury(address tAddress) public onlyOwner {
    ...
    /// @param tokenAddress Address of the token.
    function withdrawHouseEdgeAmount(address tokenAddress) public onlyOwner {
