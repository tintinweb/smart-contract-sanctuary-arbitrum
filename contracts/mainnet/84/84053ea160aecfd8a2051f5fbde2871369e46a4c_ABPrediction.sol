...
* @title ABPrediction
*/
contract ABPrediction is Ownable, Pausable, ReentrancyGuard {
    ...
    event GuessA(address indexed sender, uint256 indexed roundId, uint256 amount);
    event GuessB(address indexed sender, uint256 indexed roundId, uint256 amount);
    ...
    event NewMinGuessAmount(uint256 indexed roundId, uint256 minGuessAmount);
    event NewTreasuryFee(uint256 indexed roundId, uint256 treasuryFee);
    ...
    */
    function guessA(uint256 _roundId) external payable whenNotPaused nonReentrant notContract {
        ...
        require(msg.value >= minGuessAmount, "Guess amount must be greater than minGuessAmount");    
    ...
    * @param _roundId: roundId
    */
    function guessB(uint256 _roundId) external payable whenNotPaused nonReentrant notContract {
        require(rounds[_roundId].roundStatus == RoundStatus.Active, "round is not active");
        require(_guessable(_roundId), "Round not guessable");
        require(msg.value >= minGuessAmount, "guess amount must be greater than minGuessAmount");
        require(ledger[_roundId][msg.sender].amount == 0, "Can only guess once per round");
    ...
    */
    function activateRound(string memory _description, uint256 _h, uint256 _m) external whenNotPaused onlyOperator {
    ...
    */
    function lockRound(uint256 _roundId) external whenNotPaused onlyOperator {
    ...
    */
    function endRound(uint256 _roundId, Position _finalPosition) external whenNotPaused onlyOperator {
    ...
    */
    ...
    /**
    * @notice Allows users to lock an active round if a market has been left unresolved for two years
    * @param _roundId: id of round to lock
    */
    function emergencyLockRound(uint256 _roundId) external {
        require(_roundId <= currentRound, "round doesn't exist");
        require(rounds[_roundId].startTimestamp != 0, "round must have started first");
        require(rounds[_roundId].startTimestamp > (rounds[_roundId].startTimestamp + 730 days), "wait longer");
        require(rounds[_roundId].roundStatus == RoundStatus.Active, "round is not active");
        Round storage round = rounds[_roundId];
        round.roundStatus = RoundStatus.Locked;
        round.lockTimestamp = block.timestamp;
    ...
    * @dev Callable by admin
    */
    function setMinGuessAmount(uint256 _minGuessAmount) external whenPaused onlyAdmin {
        require(_minGuessAmount != 0, "Must be superior to 0");
        minGuessAmount = _minGuessAmount;
    ...
    */
    ...
    /**
    * @notice Returns round IDs and guess information for a user that has participated
    ...
    */
    ...
    /**
    * @notice Returns round Ids length
