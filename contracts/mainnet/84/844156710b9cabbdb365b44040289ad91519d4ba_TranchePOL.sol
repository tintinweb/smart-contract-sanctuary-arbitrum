...
contract TranchePOL is Initializable, OwnableUpgradeable {
    ...
    address public seniorRewardToken;

    function initialize(IRouterV1 trancheRouter_, address seniorRewardToken_) external initializer {
        __Ownable_init();
        trancheRouter = trancheRouter_;
        seniorRewardToken = seniorRewardToken_;
    }
    ...
    function claimSeniorRewards() external {
        require(msg.sender == owner() || maintainers[msg.sender], "must be maintainer or owner");
        IERC20Upgradeable rewardToken = IERC20Upgradeable(seniorRewardToken);
        uint256 amount1 = rewardToken.balanceOf(address(this));
        trancheRouter.claimSeniorRewards();
        uint256 amount2 = rewardToken.balanceOf(address(this));
        if (amount2 > amount1) {
            uint256 amount = amount2 - amount1;
            SafeERC20Upgradeable.safeTransfer(rewardToken, msg.sender, amount);
            emit ClaimSeniorReward(seniorRewardToken, amount);
        }
    }
