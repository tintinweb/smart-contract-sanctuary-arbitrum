...
import "./interfaces/IOreoV3Pool.sol";
...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @notice Address of OREO contract.
    IOreo public immutable OREO;
    ...
    /// @notice Address of the OreoVault.
    OreoVault public oreoVault;

    uint256 public oreoPerSecond;
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, IOreoV3Pool indexed v3Pool, ILMPool indexed lmPool);
    ...
    event SetOreoPerBlock(uint256 prevOreoPerSecond, uint256 oreoPerSecond);
    ...
    /// @param _OREO The OREO token contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(address _OREO, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        OREO = IOreo(_OREO);
        nonfungiblePositionManager = _nonfungiblePositionManager;
        WETH = _WETH;
        oreoVault = new OreoVault(_OREO);
    }

    /// @notice Returns the oreo per second.
    /// @param _pid The pool pid.
    /// @return poolOreoPerSecond Oreo reward per second.
    function getOreoPerSecondByPid(uint256 _pid) public view returns (uint256 poolOreoPerSecond) {
        if (totalAllocPoint > 0) {
            poolOreoPerSecond = (oreoPerSecond * poolInfo[_pid].allocPoint) / totalAllocPoint;
        }
    }

    /// @notice Returns the oreo per second. This is for liquidity mining pool.
    /// @param _v3Pool Address of the V3 pool.
    /// @return poolOreoPerSecond Oreo reward per second.
    function getOreoPerSecond(address _v3Pool) public view returns (uint256 poolOreoPerSecond) {
        if (totalAllocPoint > 0) {
            poolOreoPerSecond = (oreoPerSecond * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) / totalAllocPoint;
        }
    }

    /// @notice View function for checking pending OREO rewards.
    /// @dev The pending oreo amount is based on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest.
    /// @param _tokenId Token Id of NFT.
    /// @return reward Pending reward.
    function pendingOreo(uint256 _tokenId) external view returns (uint256 reward) {
    ...
    function setOreoPerSecond(uint256 _oreoPerSecond) external onlyOwner {
        massUpdatePools();
        uint256 prevOreoPerSecond = oreoPerSecond;
        oreoPerSecond = _oreoPerSecond;
        emit SetOreoPerBlock(prevOreoPerSecond, oreoPerSecond);
    }
    ...
    function add(uint256 _allocPoint, IOreoV3Pool _v3Pool, bool _withUpdate) external onlyOwner {
    ...
    /// @notice Update the given pool's OREO allocation point. Can only be called by the owner.
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotOreoNFT();
    ...
    /// @notice harvest oreo from pool.
    /// @param _tokenId Token Id of NFT.
    /// @param _to Address to.
    /// @return reward Oreo reward.
    ...
    function harvestOperation(
        ...
        if (reward > 0) {
            if (_to != address(0)) {
                ...
                _safeTransferOreoFromVault(_to, reward);
    ...
    function unwrapWETH9(uint256 amountMinimum, address recipient) external nonReentrant onlyOwner {
    ...
    function sweepToken(address token, uint256 amountMinimum, address recipient) external nonReentrant onlyOwner {
    ...
    function getTimeMultiplier(uint32 _lastRewardTimestamp, uint32 _currentTimeStamp) private pure returns (uint256) {
        return _currentTimeStamp - _lastRewardTimestamp;
    }

    /// @notice Update oreo reward for all the liquidity mining pool.
    ...
    /// @notice Update oreo reward for the liquidity mining pool.
    ...
    function updatePool(uint256 _pid) internal {
        ...
        if (pool.allocPoint != 0 && address(LMPool) != address(0)) {
            ...
            if (currentTimeStamp > lastRewardTimestamp) {
                uint256 multiplier = getTimeMultiplier(lastRewardTimestamp, currentTimeStamp);
                uint256 oreoReward = (multiplier * oreoPerSecond) * (pool.allocPoint / totalAllocPoint);
                OREO.mint(address(oreoVault), oreoReward);
                // TODO: Add devBps and referralBps
                LMPool.accumulateReward(currentTimeStamp);
            }
        }
    }
    ...
    /// @notice Safe Transfer OREO.
    /// @param _to The OREO receiver address.
    /// @param _amount Transfer OREO amounts.
    function _safeTransferOreoFromVault(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = OREO.balanceOf(address(oreoVault));
            if (balance < _amount) {
                _amount = balance;
            }

            oreoVault.safeTransferOreo(_to, _amount);
        }
    }
...
import "./interfaces/IOreoVault.sol";
