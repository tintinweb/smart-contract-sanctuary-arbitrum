...
contract UVISale is UVI {
    
    uint256 public constant PRICE_PER_UVI = 2e-8 ether; // Set the price to 0.00000002 ETH

    bool public saleActive = false;

    event SaleActivated(uint256 timestamp, address owner);
    event SaleDeactivated(uint256 timestamp, address owner);
    event UVISold(address buyer, uint256 amount, uint256 ethAmount);
    ...
    constructor(
        string memory _iconIpfsUrl,
        uint256 _initialSupply,
        uint256 _maxSupply
    ) UVI(_iconIpfsUrl, _initialSupply, _maxSupply) {}

    function activateSale() external onlyOwner {
        require(!saleActive, "Sale is already active");
        saleActive = true;
        emit SaleActivated(block.timestamp, msg.sender);
    }

    function deactivateSale() external onlyOwner {
        require(saleActive, "Sale is already inactive");
        saleActive = false;
        emit SaleDeactivated(block.timestamp, msg.sender);
    }

    function buyUVI(uint256 amount) public payable onlyWhileSaleActive {
        uint256 ethAmount = amount * PRICE_PER_UVI;
        require(msg.value >= ethAmount, "Insufficient ETH sent");
        require(balanceOf(msg.sender) >= amount, "Insufficient UVI tokens for sale");
    
        _transfer(address(this), msg.sender, amount); // Transfer tokens from contract to buyer
    
        // Transfer received Ether to the contract owner
        payable(owner()).transfer(ethAmount);
    
        emit UVISold(msg.sender, amount, ethAmount);
...
contract UVI is ERC20, Ownable {
    ...
    constructor(
        ...
        uint256 _initialSupply,
        uint256 _maxSupply
    ) ERC20('UVI', 'UVI') Ownable(msg.sender) {
        iconIpfsUrl = _iconIpfsUrl;
        maxSupply = _maxSupply;
        require(_initialSupply <= _maxSupply, "Initial supply exceeds max supply");
        _mint(msg.sender, _initialSupply * 10**18);
    }

    function updateIconIpfsUrl(string memory newIpfsUrl) external onlyOwner {
        iconIpfsUrl = newIpfsUrl;
    }

    function tokenURI() public view returns (string memory) {
        string memory baseURI = "https://cf-ipfs.com/ipfs/bafkreicka3pmgn3vabwtva2qro5dslhwsyvag7tei2s4tsvbidr4y2wl6q";
        return string(abi.encodePacked(baseURI, iconIpfsUrl));
    }
