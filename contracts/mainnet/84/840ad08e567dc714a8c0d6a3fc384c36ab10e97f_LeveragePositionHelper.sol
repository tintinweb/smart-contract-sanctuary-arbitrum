...
contract LeveragePositionHelper {
    using SafeTransferLib for ERC20;

    address public immutable _0xExchangeProxy = 0xDef1C0ded9bec7F1a1670819833240f027b25EfF;
    address internal _lendingPool;

    /// @notice Opens up a leveraged position for msg.sender.
    /// @param lendingPool The address of the lending pool.
    /// @param longToken The token we are longing (using as collateral).
    /// @param shortToken The token we are shorting (borrowing and selling).
    /// @param payWithLongToken Which token the contract transfers in.
    /// @param amountIn The amount of the token to transfer in.
    /// @param debtAmountToOpen The amount of the short token to borrow.
    /// @param swapData0x The 0x swap data to be used to swap the short token for the long token.
    function openPosition(
        ...
        if (payWithLongToken) {
            ERC20(longToken).safeTransferFrom(msg.sender, address(this), amountIn);
        } else {
            ERC20(shortToken).safeTransferFrom(msg.sender, address(this), amountIn);
        }
        _approve(shortToken, _0xExchangeProxy);
        _approve(longToken, address(lendingPool));
        address[] memory assets = new address[](1);
        uint256[] memory amounts = new uint256[](1);
        uint256[] memory modes = new uint256[](1);
        assets[0] = shortToken;
        amounts[0] = debtAmountToOpen;
        modes[0] = 2;
        // Store the lending pool address so that it can be checked against msg.sender in the executeOperation callback function.
        _lendingPool = address(lendingPool);
        bytes memory data = abi.encode(longToken, msg.sender, swapData0x);
        lendingPool.flashLoan(address(this), assets, amounts, modes, msg.sender, data, 0);
        _lendingPool = address(0);
    }

    /// @notice Function to be called by the lending pool when the flash loan is executed.
    function executeOperation(
        ...
        (address longAsset, address sender, bytes memory swapData0x) = abi.decode(params, (address, address, bytes));
        console.log("short asset balance: %e", ERC20(0xaf88d065e77c8cC2239327C5EDb3A432268e5831).balanceOf(address(this)));
        (bool success,) = _0xExchangeProxy.call(swapData0x);
        require(success, "Swap failed.");
        uint256 balance = ERC20(longAsset).balanceOf(address(this));
        ILendingPool(msg.sender).deposit(longAsset, balance, sender, 0);
        return true;
    }
