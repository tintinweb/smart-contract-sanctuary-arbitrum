...
// File: xlsd/interfaces/IESXLSDTokenUsage.sol


pragma solidity ^0.8.0;

interface IESXLSDTokenUsage {
...
// File: xlsd/interfaces/IESXLSDToken.sol
...
struct RedeemInfo {
  uint256 XLSDAmount; // XLSD amount to receive when vesting has ended
  uint256 ESXLSDAmount; // ESXLSD amount to redeem
  uint256 endTime;
  IESXLSDTokenUsage dividendsAddress;
  uint256 dividendsAllocation; // Share of redeeming ESXLSD to allocate to the Dividends Usage contract
}

interface IESXLSDToken is IERC20 {
...
// File: xlsd/interfaces/IXLSDToken.sol


pragma solidity ^0.8.0;


interface IXLSDToken is IERC20{
...
/*
 * ESXLSD is XLSD's escrowed governance token obtainable by converting XLSD to it
 * It's non-transferable, except from/to whitelisted addresses
 * It can be converted back to XLSD through a vesting process
 * This contract is made to receive esXLSD deposits from users in order to allocate them to Usages (plugins) contracts
 */

contract ESTLSDToken is Ownable, ReentrancyGuard, ERC20("TLSD escrowed token", "esTLSD"), IESXLSDToken {
  using Address for address;
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;
  using SafeERC20 for IXLSDToken;

  struct ESXLSDBalance {
  ...
  IXLSDToken public immutable XLSDToken; // XLSD token to convert to/from
  IESXLSDTokenUsage public dividendsAddress; // XLSD dividends contract
  ...
  // Adjusted dividends rewards for redeeming ESXLSD
  uint256 public redeemDividendsAdjustment = 0; // 50%

  mapping(address => ESXLSDBalance) public ESXLSDBalances; // User's ESXLSD balances
  mapping(address => RedeemInfo[]) public userRedeems; // User's redeeming instances


  constructor(IXLSDToken XLSDToken_) {
    XLSDToken = XLSDToken_;
  ...
  event Redeem(address indexed userAddress, uint256 ESXLSDAmount, uint256 XLSDAmount, uint256 duration);
  event FinalizeRedeem(address indexed userAddress, uint256 ESXLSDAmount, uint256 XLSDAmount);
  event CancelRedeem(address indexed userAddress, uint256 ESXLSDAmount);
  ...
  /*
   * @dev Returns user's ESXLSD balances
   */
  function getESXLSDBalance(address userAddress) external view returns (uint256 allocatedAmount, uint256 redeemingAmount) {
    ESXLSDBalance storage balance = ESXLSDBalances[userAddress];
    return (balance.allocatedAmount, balance.redeemingAmount);
  }

  /*
   * @dev returns redeemable XLSD for "amount" of ESXLSD vested for "duration" seconds
   */
  function getXLSDByVestingDuration(uint256 amount, uint256 duration) public view returns (uint256) {
   ...
   */
  function getUserRedeem(address userAddress, uint256 redeemIndex) external view validateRedeem(userAddress, redeemIndex) returns (uint256 XLSDAmount, uint256 ESXLSDAmount, uint256 endTime, address dividendsContract, uint256 dividendsAllocation) {
    RedeemInfo storage _redeem = userRedeems[userAddress][redeemIndex];
    return (_redeem.XLSDAmount, _redeem.ESXLSDAmount, _redeem.endTime, address(_redeem.dividendsAddress), _redeem.dividendsAllocation);
  }
  ...
  /**
   * @dev returns approved ESXLSD to allocate from "userAddress" to "usageAddress"
   */
  ...
  /**
   * @dev returns allocated ESXLSD from "userAddress" to "usageAddress"
   */
   ...
   * Must only be called by owner
   */
  function updateDividendsAddress(IESXLSDTokenUsage dividendsAddress_) external onlyOwner {
   ...
   */
  function approveUsage(IESXLSDTokenUsage usage, uint256 amount) external nonReentrant {
  ...
  /**
   * @dev Initiates redeem process (ESXLSD to XLSD)
   *
   * Handles dividends' compensation allocation during the vesting process if needed
   */
  function redeem(uint256 ESXLSDAmount, uint256 duration) external nonReentrant {
    require(ESXLSDAmount > 0, "redeem: ESXLSDAmount cannot be null");
    require(duration >= minRedeemDuration, "redeem: duration too low");

    _transfer(msg.sender, address(this), ESXLSDAmount);
    ESXLSDBalance storage balance = ESXLSDBalances[msg.sender];

    // get corresponding XLSD amount
    uint256 XLSDAmount = getXLSDByVestingDuration(ESXLSDAmount, duration);
    emit Redeem(msg.sender, ESXLSDAmount, XLSDAmount, duration);

    // if redeeming is not immediate, go through vesting process
    if(duration > 0) {
      // add to SBT total
      balance.redeemingAmount = balance.redeemingAmount.add(ESXLSDAmount);

      // handle dividends during the vesting process
      uint256 dividendsAllocation = ESXLSDAmount.mul(redeemDividendsAdjustment).div(100);
      ...
      // add redeeming entry
      userRedeems[msg.sender].push(RedeemInfo(XLSDAmount, ESXLSDAmount, _currentBlockTimestamp().add(duration), dividendsAddress, dividendsAllocation));
    } else {
      // immediately redeem for XLSD
      _finalizeRedeem(msg.sender, ESXLSDAmount, XLSDAmount);
    }
  }
  ...
  function finalizeRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.ESXLSDAmount);
    _finalizeRedeem(msg.sender, _redeem.ESXLSDAmount, _redeem.XLSDAmount);

    // handle dividends compensation if any was active
    if(_redeem.dividendsAllocation > 0) {
      // deallocate from dividends
      IESXLSDTokenUsage(_redeem.dividendsAddress).deallocate(msg.sender, _redeem.dividendsAllocation, new bytes(0));
    }
  ...
  function cancelRedeem(uint256 redeemIndex) external nonReentrant validateRedeem(msg.sender, redeemIndex) {
    ...
    // make redeeming ESXLSD available again
    balance.redeemingAmount = balance.redeemingAmount.sub(_redeem.ESXLSDAmount);
    _transfer(address(this), msg.sender, _redeem.ESXLSDAmount);
    ...
    emit CancelRedeem(msg.sender, _redeem.ESXLSDAmount);
  ...
  /**
   * @dev Allocates caller's "amount" of available ESXLSD to "usageAddress" contract
   *
  ...
  function allocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    IESXLSDTokenUsage(usageAddress).allocate(msg.sender, amount, usageData);
  }
  ...
  function deallocate(address usageAddress, uint256 amount, bytes calldata usageData) external nonReentrant {
    ...
    IESXLSDTokenUsage(usageAddress).deallocate(msg.sender, amount, usageData);
  }

  /**
   * @dev Deallocates "amount" of allocated ESXLSD belonging to "userAddress" from caller (ie usage contract)
   *
   * Caller can only deallocate ESXLSD from itself
   */
  ...
  function _convert(uint256 amount, address to) internal {
    ...
    XLSDToken.safeTransferFrom(msg.sender, address(this), amount);
  }

  /**
   * @dev Finalizes the redeeming process for "userAddress" by transferring him "XLSDAmount" and removing "ESXLSDAmount" from supply
   *
   * Any vesting check should be ran before calling this
   * XLSD excess is automatically burnt
   */
  function _finalizeRedeem(address userAddress, uint256 ESXLSDAmount, uint256 XLSDAmount) internal {
    uint256 XLSDExcess = ESXLSDAmount.sub(XLSDAmount);

    // sends due XLSD tokens
    XLSDToken.safeTransfer(userAddress, XLSDAmount);

    // burns XLSD excess if any
    XLSDToken.burn(XLSDExcess);
    _burn(address(this), ESXLSDAmount);

    emit FinalizeRedeem(userAddress, ESXLSDAmount, XLSDAmount);
  }

  /**
   * @dev Allocates "userAddress" user's "amount" of available ESXLSD to "usageAddress" contract
   *
   */
  function _allocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    uint256 approvedESXLSD = usageApprovals[userAddress][usageAddress];
    require(approvedESXLSD >= amount, "allocate: non authorized amount");

    // remove allocated amount from usage's approved amount
    usageApprovals[userAddress][usageAddress] = approvedESXLSD.sub(amount);
  ...
  /**
   * @dev Deallocates "amount" of available ESXLSD to "usageAddress" contract
   *
   * args specific to usage contract must be passed into "usageData"
   */
  function _deallocate(address userAddress, address usageAddress, uint256 amount) internal {
    ...
    // burn corresponding XLSD and ESXLSD
    XLSDToken.burn(deallocationFeeAmount);
