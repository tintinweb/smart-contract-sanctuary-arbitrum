...
interface IRewardTracker {
    ...
    function redeemDebtWithProt(address _account, address _receiver, uint256 _amount) external;
}
...
contract ComptrollerV3Storage is ComptrollerV2Storage {
    ...
    /// @notice The address of the PROT token
    address public protAddress;
    ...
    ExponentialNoError
{
        ...
        returns (bool)
    {
        ...
        if (compAddress != address(0) && tokenBalanceVipThreshold > 0) {
            if (EIP20Interface(compAddress).balanceOf(_account) >= tokenBalanceVipThreshold || EIP20Interface(protAddress).balanceOf(_account) >= tokenBalanceVipThreshold) {
                return true;
            } 
        }
    ...
    function setCompAddress(address compAddress_) external {
     ...
     function setProtAddress(address protAddress_) external {
        require(msg.sender == admin, "only admin can set protAddress");
        protAddress = protAddress_;
    }
