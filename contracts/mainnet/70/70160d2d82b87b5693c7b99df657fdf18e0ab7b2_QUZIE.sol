...
contract QUZIE is ERC20, Ownable, DividendDistributor {
...
constructor(
...
) ERC20("QUZIE", "QUZIE") DividendDistributor(_routerAddress, _WETH) {
    ...
    function getEthAmount(uint256 QUZIEAmount) internal view returns (uint256) {
    address[] memory path = new address[](2);
    path[0] = address(this);
    path[1] = address(WETH);

    uint256[] memory amounts = swapRouter.getAmountsIn(QUZIEAmount, path);
    ...
    function transferFrom(
        ...
        return _QUZIETransfer(sender, recipient, amount);
    }
