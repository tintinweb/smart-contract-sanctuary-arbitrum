    ...
    AccessControlUpgradeable
{
    ...
    int256[] public deltaCached;
    int256[] public tempDeltas;
    ...
    function initiateHedge(
        int256[] memory _targetDeltas,
        address[] memory _underlyings,
        int256 _collateralMovment
    ) external onlyAuthorized returns (bool) {
        underlyingAssets = _underlyings;
        tempDeltas = _targetDeltas;
    ...
    function getDelta(address underlyingAddress ) external view returns (int256) {
        for(uint i = 0; i < underlyingAssets.length; i++) {
            if(underlyingAssets[i] == underlyingAddress) {
                return deltaCached[i];
            }
        }
        return 0;
    }
