...
contract AOGTToken is ERC20,Ownable{
    mapping(address => bool) private keeperMap;
    event SetKeeper(address sender,address addr,bool active);

    constructor() ERC20("Assembly Of Gods Token", "AOGT",18){
        uint256 initSupply = 43351143 * (10 ** super.decimals());
    ...
    function mintTo(address to, uint256 amount) public onlyKeeper(msg.sender) returns (bool){
        _mint(to, amount);
        return true;
    }
}