...
contract LP_USDT is Ownable, Pausable, ERC20 {
    ...
    constructor(address _stor, address _usdt) Ownable() Pausable() ERC20("Vortex USDT LP", "VX-ULP"){ 
        depositFee = 20;
        withdrawFee = 50;
        usdt = IERC20(_usdt);
    ...
    function deposit(uint _amt) external payable whenNotPaused() updateReward(msg.sender) {
        require(_amt != 0, "value is 0");
        usdt.transferFrom(msg.sender, revenue, _amt * depositFee / denominator);
        ...
        usdt.transferFrom(msg.sender, vault, _depositAmt);
    ...
    function withdraw() external whenNotPaused() updateReward(msg.sender) {
        ...
        if (block.timestamp - depositTime[msg.sender] <= 24 hours) {
            ...
            IVault(vault).withdrawUSDT(revenue, _fee);
            _amt = _amt - _fee;
        }

        IVault(vault).withdrawUSDT(msg.sender, _amt);
    ...
    function getTotalIncome() public view returns (uint){
        uint total = IRevenue(revenue).usdt_lp_revenue();
...
interface IRevenue {
    ...
    function lpReward(address _account, uint _amt) external;
    function lpRewardUSDT(address _account, uint _amt) external;
    function stake_revenue() view external returns (uint);
    function usdt_stake_revenue() view external returns (uint);
    function lp_revenue() view external returns (uint);
    function usdt_lp_revenue() view external returns (uint);
}
