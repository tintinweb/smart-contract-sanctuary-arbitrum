 ...
 * @author  .
 * @title   GmxExecutor
 * @notice  Contains the logic for allowing managers to interact with the GMX position router
 * @dev     The functions in contract are delegate called by the Vault (ie they execute as the vault)
 */
...
contract GmxExecutor is IExecutor, IGmxPositionRouterCallbackReceiver {
    ...
    function clean() external {
        GmxStoredData.cleanLiquidatedPositions();
    }

    /**
     * @notice  Allows the manager to create increasePosition request
     * @dev     For simplicity we don't support swaps on the way in
     * @dev     Usually the manager will get a better swap using 0x anyway
     * @param   _path  Currently only supports [collateralToken]
     * @param   _indexToken The token the position is against (eth/wbtc etc)
     * @param   _amountIn  The amount of collateral to use
     * @param   _minOut  Currently not used (no swaps on the way in)
     * @param   _sizeDelta  The size of the position
     * @param   _isLong  True if the position is long
     * @param   _acceptablePrice  The index token price at execution the manager is willing to accept
     */
    function createIncreasePosition(
        ...
        registry.emitEvent();
        emit GmxV1CreateIncreasePosition(
            _isLong,
            _indexToken,
            collateralToken,
            _sizeDelta,
    ...
    /**
     * @notice  Allows the manager to create decreasePosition request
     * @dev     We don't support swaps on the way out
     * @dev     CollateralToken+IndexToken+IsLong act as composite key for the position
     * @param   _path  This needs to be [collateralToken]
     * @param   _indexToken  The indexToken of the existing position
     * @param   _collateralDelta  The amount of collateral to withdraw
     * @param   _sizeDelta  The amount of the position to close
     * @param   _isLong  True if the position is long
     * @param   _acceptablePrice  The index token price at execution the manager is willing to accept
     * @param   _minOut  Currently not used (no swaps on the way out)
     */
    function createDecreasePosition(
        ...
        GmxStoredData.removeExcutedRequests();
        GmxStoredData.pushRequest(
            key,
            GmxStoredData.GMXRequestData({
        ...
        VaultBaseExternal(payable(address(this))).addActiveAsset(outputToken);
        registry.emitEvent();
        emit GmxV1CreateDecreasePosition(
            _isLong,
            _indexToken,
            _path[0],
            _sizeDelta,
    ...
    /**
     * @notice  This is called by GMX when a request is executed or cancelled
     * @dev     We need to update the active assets if the request was executed
     * @dev     And remove the request from the stored data
     * @param   _requestKey  This what gmx uses to identify the request (keccak(account,requestIndex))
     * @param   wasExecuted  If the request was executed
     * @param   isIncrease  If the request was an increase position request
     */
    function gmxPositionCallback(
        ...
        // ^^(this is what we also get if a increasePosition fails)
        ...
        GmxStoredData.removeRequest(registry, uint(index));

        // If the request was exec

        // If it's a decrease position we need to update the collateralAsset for shorts,
        // indexToken for longs aka outputToken
        ...
        uint256 markPrice = requestData._isLong
            ? registry.gmxConfig().vault().getMaxPrice(requestData._indexToken)
            : registry.gmxConfig().vault().getMinPrice(requestData._indexToken);

        registry.emitEvent();
        emit GmxV1Callback(
            isIncrease,
            requestData._isLong,
            requestData._indexToken,
            requestData._collateralToken,
            wasExecuted,
            markPrice
        );
    }
    ...
    /**
     * @notice  Checks if the acceptable price set by the manager is within an acceptable range of the chainlink price
     * @param   gmxVault  the gmx vault
     * @param   isIncrease  if the order is a increase or decrease
     * @param   indexToken  the index token
     * @param   isLong  if the position is long
     * @param   acceptablePrice  the price the manager is willing to accept
     * @param   acceptableDeviationBasisPoints  the acceptable deviation from the chainlink price
     */
...
library GmxStoredData {
    ...
    /**
     * @notice  Cleans any liquidated positions from storage
     * @dev     It also removes any stale executed requests from storage.
     * @dev     We only allow a certain number of positions as no to exceed gas limits
     * @dev     And we lock collateral assts, this must be run to free up asset slots
     */
    function cleanLiquidatedPositions() internal {
        Registry registry = VaultBaseExternal(payable(address(this)))
            .registry();
        IntegrationDataTracker dataTracker = registry.integrationDataTracker();
        require(
    ...
    function removeExcutedRequests() internal {
        ...
        for (uint256 i = positionData.length; i > 0; i--) {
            ...
            (address account, , ) = getIncreasePositionRequestsData(
    ...
    /**
     * @notice  Uses the request key to get the deails of a GMX increase position request from GMX
     * @dev     This function reads from GMX and not the integrationDataTracker
     * @dev     Gmx Takes the input token when the order is created, we must include this value in the vaults value
     * @param   gmxPositionRouter  the GMXPositionRouter
     * @param   key  the request key
     * @return  account  the account that made the request (the vault)
     * @return  inputToken  the input token for the request
     * @return  amountIn  the amount of the above in the request
     */
...
contract Registry is SafeOwnable, Pausable {
    ...
    /**
     * @notice  Returns the uniswap v3PoolConfig for the given asset
     * @param   asset  The address of the asset
     * @return  RegistryStorage.V3PoolConfig  The uniswap v3 pool config(pool, pairToken)
     */
...
abstract contract TransportStargate is TransportReceive, IStargateReceiver {
    ...
    function sgReceive(
        ...
        IERC20(_token).safeTransfer(payload.dstVault, amountLD);
