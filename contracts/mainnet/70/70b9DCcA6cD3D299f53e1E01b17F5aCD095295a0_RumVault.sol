...
contract RumVault is IRumVault, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function requestClosePosition(uint32 _positionID) external payable InvalidID(_positionID, msg.sender) nonReentrant {
        ...
        (, , , uint256 currentDTVWithDA, uint256 LeverageAmountWihtDA) = getPosition(_positionID, msg.sender);
    ...
    function fulfillCloseCancellation(uint256 orderId) public onlyKeeper returns (bool) {
        ...
        inCloseProcess[msg.sender][wr.positionID] = false;
    ...
    function requestLiquidatePosition(address _user, uint256 _positionID) external payable nonReentrant {
        ...
        (, , , uint256 currentDTVWithDA, uint256 LeverageAmountWihtDA) = getPosition(_positionID, _user);
    ...
    function fulfillLiquidation(uint256 _orderId, uint256 _returnedUSDC) external nonReentrant onlyKeeper {
        ...
        if (wr.returnedUSDC >= _positionInfo.leverageAmount) {
            wr.returnedUSDC -= _positionInfo.leverageAmount;
            ...
            IERC20Upgradeable(strategyAddresses.USDC).safeIncreaseAllowance(
                ...
                leftovers + _positionInfo.leverageAmount
            );
            IWater(strategyAddresses.water).repayDebt(_positionInfo.leverageAmount, leftovers + _positionInfo.leverageAmount);
        } else {
