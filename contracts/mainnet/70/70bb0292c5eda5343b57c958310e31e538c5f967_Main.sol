...
contract Main is AutomationCompatibleInterface, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function deployDraw(
        ...
        returns (string memory)
    {
        return createDraw(msg.sender, "", scheduledAt, nbParticipants, numbers, algorithm, defaultRequestConfirmations, callbackAddress);     
    }
    ...
    function isCompleted(string memory cid) external view returns (bool) {
        return draws[cid].completed;
    }

    function checkRawRandomness(string memory cid) external view returns (bytes memory) {
        return draws[cid].entropy;
    }

    function checkRandomNumbers(string memory cid) external view returns (uint32[] memory) {

        require(draws[cid].completed, "This random draw has not occured yet. Come back later.");
        ...
        uint8 algorithm = draws[cid].algorithm;
