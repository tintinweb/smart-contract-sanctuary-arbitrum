...
abstract contract Auth {
     ...
     */
    modifier authorized() {
        require(kurwa(msg.sender), "!AUTHORIZED"); _;
    }
     ...
     */
    function kurwa(address account) private view returns (bool) {
...
contract DividendDistributor is IDividendDistributor {
    ...
    // EARN
    IBEP20 public ARB = IBEP20(0x2f2a2543B76A4166549F7aaB2e75Bef0aefC5B0f);
...
/* Token contract */
contract BTC_REWARD is IBEP20, Auth {
    using SafeMath for uint256;

    // Addresses
    address BTCA = 0x2f2a2543B76A4166549F7aaB2e75Bef0aefC5B0f;
    address WETH = 0x82aF49447D8a07e3bd95BD0d56f35241523fBab1;
    ...
    address public sunFeeReceiver;
    // Name and symbol
    string constant _name = "ARB BTC REWARD";
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x9AC1d848a449aBE225F31D1a66D75fa1565A0E89;
        sunFeeReceiver = 0x9AC1d848a449aBE225F31D1a66D75fa1565A0E89;
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if (!authorizations[sender] && !isTxLimitExempt[sender] && !isTxLimitExempt[recipient] && recipient != owner && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver && recipient != sunFeeReceiver){
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountWETHsun = amountWETH.mul(sunFee).div(totalWETHFee); 
        ...
        (bool successsun, /* bytes memory data */) = payable(sunFeeReceiver).call{value: amountWETHsun, gas: 30000}(""); 
        require(successMarketing, "marketing receiver rejected ETH transfer");
        require(successsun, "sun receiver rejected ETH transfer");
    ...
    // Set our buy fees
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _sunFeeBuy, uint256 _reflectionFeeBuy, uint256 _marketingFeeBuy, uint256 _feeDenominator) external onlyOwner {
        liquidityFeeBuy = _liquidityFeeBuy;
        sunFeeBuy = _sunFeeBuy;
        reflectionFeeBuy = _reflectionFeeBuy;
        marketingFeeBuy = _marketingFeeBuy;
        totalFeeBuy = _liquidityFeeBuy.add(_sunFeeBuy).add(_reflectionFeeBuy).add(_marketingFeeBuy);
        feeDenominator = _feeDenominator;
    }

    // Set our sell fees
    function setSellFees(uint256 _liquidityFeeSell, uint256 _sunFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell, uint256 _feeDenominator) external onlyOwner {
        liquidityFeeSell = _liquidityFeeSell;
        sunFeeSell = _sunFeeSell;
        reflectionFeeSell = _reflectionFeeSell;
        marketingFeeSell = _marketingFeeSell;

        totalFeeSell = _liquidityFeeSell.add(_sunFeeSell).add(_reflectionFeeSell).add(_marketingFeeSell);
        feeDenominator = _feeDenominator;
    }

    // Set the marketing and liquidity receivers
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _sunFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        sunFeeReceiver = _sunFeeReceiver;
    }
