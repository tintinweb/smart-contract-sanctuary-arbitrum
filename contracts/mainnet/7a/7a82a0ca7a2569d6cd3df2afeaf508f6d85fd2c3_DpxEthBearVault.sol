...
abstract contract LPVault is ILPVault, ReentrancyGuard, AccessControl {
    ...
    function signalFlip() external nonReentrant {
        ...
        if (currentEpochDeposit != 0) {
            ...
            depositToken.approve(address(destination), currentEpochDeposit);
            destination.deposit(msg.sender, currentEpochDeposit);

            // There is nothing else on the vault
            if (userFlipingPercentage == 0) {
    ...
    function signalExit() external nonReentrant {
        ...
        userEpochs[msg.sender].exitPercentage = userExitPercentage;
        totalDeposited -= userEpochs[msg.sender].deposited;
    ...
    function cancelSignalExit() external nonReentrant {
        ...
        if (epoch > userEpochData.end) {
            revert CANNOT_CANCEL_EXIT();
        }

        exitingPercentage -= userEpochData.exitPercentage;

        userEpochs[msg.sender].end = 0;
        userEpochs[msg.sender].exitPercentage = 0;
    ...
    function withdraw() external nonReentrant {
        ...
        if (userEpochData.exitPercentage == 0) {
            // Deletes the user data to avoid double withdraws
            _deleteUserData(msg.sender);
            return;
        }
        ...
        uint256 rewards = epochs[userEpochData.end].endAmount.mulDivDown(userEpochData.exitPercentage, ACCURACY);
    ...
    function startEpoch(
        ...
        uint256 unusableExited = endBalance.mulDivUp(exitingPercentage, ACCURACY);
