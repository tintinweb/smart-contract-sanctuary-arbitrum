...
contract Vault is ReentrancyGuardUpgradeable, OwnableUpgradeable, IVaultPyth {
    ...
    function getNextFundingRate(address _token, bool _isLong)
        ...
        returns (uint256)
    {
        uint256 collateralDecimals = tokenDecimals[collateralToken];
        uint256 poolSize;
        uint256 intervals;

        if (positionRouter == address(0)) {
            if (
            ...
            if (_isLong) {
                poolSize = (IBasePositionManager(positionRouter)
                .maxGlobalLongSizes(_token) / 1e30) * 10 ** collateralDecimals;
            } else {
                poolSize = (IBasePositionManager(positionRouter)
                .maxGlobalShortSizes(_token) / 1e30) * 10 ** collateralDecimals;
            }
