    ...
    IRewardable
{
    ...
    bool private _isRewardable;
    uint8 private _currentLevel;
    ...
    {
        uint strpBalance = strpToken.balanceOf(address(this));
        if (strpBalance > 0){
            SafeERC20.safeTransfer(IERC20(address(strpToken)), 
                                    _to, 
                                    strpBalance);
        }
    }
    ...
    function withdraw(address _to) external override minimumLevel(2) onlyOwner
    {
        ...
        uint tradingBalance = tradingToken.balanceOf(address(this));
        if (tradingBalance > 0){
        ...
        uint stakingBalance = stakingToken.balanceOf(address(this));
        if (stakingBalance > 0){
            SafeERC20.safeTransfer(IERC20(address(stakingToken)), 
                                    _to, 
                                    stakingBalance);
        }
    ...
    function borrow(address _to, int256 _amount) external override nonReentrant onlyStrips {
        int256 oldBalance = calcTradingLiqudity();

        SafeERC20.safeTransfer(tradingToken, 
                                _to, 
                                uint(_amount));

        int256 newBalance = calcTradingLiqudity();
        emit BalanceChanged(
            block.timestamp,
            address(this),
            msg.sender,
    ...
    function depositTradingToken(address _from, int256 _amount) external ownerOrAdmin
    {
        ...
        SafeERC20.safeTransferFrom(tradingToken, 
                                            _from, 
                                            address(this), 
                                            uint(_amount));
         ...
         */
        slpToken.changeTradingPnl(_amount);
    }
    ...
    */
    ...
    function changeTradingToken(IERC20 _newTradingToken) external ownerOrAdmin
    {
        tradingToken = _newTradingToken;
    }


    function ensureFunds(int256 amount) external override nonReentrant onlyStrips {
    ...
    function externalLiquidityChanged(int256 diff) external override nonReentrant onlyStrips{
        int256 currentBalance = calcTradingLiqudity();
    ...
    function changeTradingPnl(int256 amount) external override virtual nonReentrant onlyStrips{
        slpToken.changeTradingPnl(amount);
    }
    
    function changeStakingPnl(int256 amount) external override virtual nonReentrant onlyStrips{
    ...
    function changeStrp(IERC20 _strpToken) external ownerOrAdmin
    {
        strpToken = _strpToken;
    }
