...
import {FixedPoint} from "src/univ2/FixedPoint.sol";
import {FixedPointMathLib} from "solmate/src/utils/FixedPointMathLib.sol";
import {SafeMath} from "@openzeppelin/contracts/utils/math/SafeMath.sol";
import {IUniswapV2Pair} from "./interfaces/IUniswapV2Pair.sol";
import {ICamelotPair} from "./interfaces/ICamelotPair.sol";

contract BondingTreasury is UpgradeableOperable, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    mapping(address => AMM) public amm;
    ...
    /* ========== CONSTRUCTOR ========== */
    function init(address _xp) external initializer {
        require(_xp != address(0), "Zero address: XP");
    ...
    /**
     * @notice allow approved address to deposit an asset for XP
     * @param _amount uint256
     * @param _token address
     * @param _profit uint256
     * @return send_ uint256
     */
    function deposit(uint256 _amount, address _token, uint256 _profit, bool _isLiquidityPool)
        external
        override
        onlyOperator
        returns (uint256 send_)
    {
        ...
        uint256 value = tokenValue(_token, _amount, _isLiquidityPool);

        // mint XP needed and store amount of rewards for distribution
        send_ = value.sub(_profit);

        XP.mint(msg.sender, send_);
     ...
     * @param _token address
     * @param _amount uint256
     * @param _toHarvester If we are sending to a harvester we dont decrease the amount of reserves
     */
    function manage(address _token, uint256 _amount, bool _toHarvester, bool _isLp) external override onlyOperator {
        if (!_toHarvester) {
            uint256 value = tokenValue(_token, _amount, _isLp);
     ...
     * @param _amount uint256
     */
    function mint(address _recipient, uint256 _amount) external override onlyOperator {
        XP.mint(_recipient, _amount);
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(XP.totalSupply());
    }

    /**
     * @notice returns LP value quoted in XP
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount, bool _isLiquidityPool)
        public
        view
        override
        returns (uint256 value_)
    {
        // This normalizes the deposited token decimals to same decimals as XP
        value_ = _amount.mul(10 ** IERC20Metadata(address(XP)).decimals()).div(10 ** IERC20Metadata(_token).decimals());

        if (_isLiquidityPool) {
            value_ = valuation(_token, _amount);
        }
    }
    ...
    function baseSupply() external view override returns (uint256) {
        return XP.totalSupply();
    }

    function setAMM(address _pair, AMM _amm) external onlyOperator {
        amm[_pair] = _amm;
    }
    ...
    function getKValue(address _pair) public view returns (uint256 k_) {
        ...
        if (_amm == AMM.UNISWAP) {
            return getKValueUniswap(_pair);
        } else if (_amm == AMM.CAMELOT) {
            return getKValueCamelot(_pair);
        } else {
            revert("Invalid AMM");
        }
    }

    function getKValueUniswap(address _pair) private view returns (uint256) {
        ...
        return reserve0.mul(reserve1).div(10 ** decimals);
    }

    function getKValueCamelot(address _pair) private view returns (uint256) {
        uint256 token0 = IERC20Metadata(IUniswapV2Pair(_pair).token0()).decimals();
        uint256 token1 = IERC20Metadata(IUniswapV2Pair(_pair).token1()).decimals();
        uint256 decimals = token0.add(token1).sub(IERC20Metadata(_pair).decimals());

        (uint256 reserve0, uint256 reserve1,,) = ICamelotPair(_pair).getReserves();
    ...
    function valuation(address _pair, uint256 amount_) private view returns (uint256 _value) {
...
interface ITreasury {
    function deposit(uint256 _amount, address _token, uint256 _profit, bool _isLp) external returns (uint256);

    function tokenValue(address _token, uint256 _amount, bool _isLp) external view returns (uint256 value_);

    function mint(address _recipient, uint256 _amount) external;

    function manage(address _token, uint256 _amount, bool _toHarvester, bool _isLp) external;
...
import "src/common/FullMath.sol";

import "./Babylonian.sol";
import "./BitMath.sol";

// Taken from https://github.com/Uniswap/solidity-lib/blob/master/contracts/libraries/FixedPoint.sol
...
// Taken from https://github.com/Uniswap/solidity-lib/blob/master/contracts/libraries
...
// Taken from https://github.com/Uniswap/solidity-lib/blob/master/contracts/libraries/BitMath.sol
library BitMath {
