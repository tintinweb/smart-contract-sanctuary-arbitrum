...
import "../../mint-voucher/V2/MintVoucherV2.sol";
import "../../cloneable-helper/CloneableNFT.sol";

error InvalidBaseURI();

contract FrozenERC721 is ERC721, ERC721URIStorage, ERC2981, MintVoucherContractV2, CloneableNFT {
    ...
    **********************************************************************************************************/
     ...
     */
    function mintWithVoucher(MintVoucherV2 calldata voucher, bytes calldata signature) external payable {
        ...
        emit PermanentURI(voucher.tokenURI, voucher.tokenId);
    }
     ...
     * @param uri The metadata uri of the NFT token
     */
    function adminMintWithURI(address to, uint256 tokenId, string memory uri) external adminOrOwnerOnly {
    ...
    **********************************************************************************************************/
     ...
     * @param voucher The MintVoucher that contains the specific mint details
     */
    function _handleMint(address to, MintVoucherV2 calldata voucher) internal override {
...
import "./MintVoucherVerificationV2.sol";
import "../../platform/nft/common/Helper.sol";

/**
 * @dev Base functions for MintVoucher verification and payment handling
 *      Revert with an error if zero address or insufficient funds
 */

abstract contract MintVoucherContractV2 is MintVoucherVerificationV2 {
    ...
    **********************************************************************************************************/
     ...
     * @param signature The signature that must originate from an authorized signer
     */
    function _mintWithVoucher(MintVoucherV2 calldata voucher, bytes calldata signature) internal virtual {
     ...
     * @param voucher The MintVoucher that contains the specific mint details
     */
    function _handleMint(address to, MintVoucherV2 calldata voucher) internal virtual;

    /**
     * @dev Handle ETH payments
     * @param voucher The MintVoucher that contains the specific mint details
     */
    function _handleEthPayment(MintVoucherV2 calldata voucher) internal {
     ...
     * @param voucher The MintVoucher that contains the specific mint details
     */
    function _handleERC20Payment(MintVoucherV2 calldata voucher) internal {
...
/**
 * @title  MintVoucherVerificationV2
 * @notice MintVoucherVerificationV2 is intended to be used with an off chain mint limiting server
 *         that issues vouchers. This contract handles the validation of the V2 voucher and signature.
 * @dev    Non-voucher related validation logic such as checking available supply and funds should
 *         be implemented in the caller functions.
 */
abstract contract MintVoucherVerificationV2 is MintVoucherVerification {
    /**
     * @dev The MintVoucherV2 struct contains the specific mint details. This data
     *      along with the contract address and the msg sender will be included in
     *      the signed message. Currency should be zero address if payment is in
     *      native currency. tokenId is necessary for ERC1155 implementations but
     *      for ERC721 it is up to the implementation.
     */
    struct MintVoucherV2 {
     ...
     * @param initialRecipientAmount The amount to send to the initialRecipient
     ...
     * @param currency The currency address for payment
     * @param tokenURI The uri for the tokenId
    ...
    /**
     * @dev Hash the mint voucher data, msg sender, contract address and network ID.
     *      The lastNonce needs to be updated with the voucher nonce prior to hashing.
     * @param netRecipient The address to transfer the net funds minus initial payout, if any
     * @param initialRecipient The address to transfer an initial payout
     * @param initialRecipientAmount The amount to send to initialRecipient
     * @param quantity The quantity to mint
     * @param nonce The mint voucher nonce used for replay protection
     * @param expiry The mint voucher expiration
     * @param price The price per unit
