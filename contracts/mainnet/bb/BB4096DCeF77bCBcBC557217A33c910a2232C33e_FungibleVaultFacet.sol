...
contract FungibleVaultFacet is BaseVault, IFungibleVault {
    using SafeERC20 for IERC20;

    function partialFungibleTokenUnlock(address _tokenAddress, uint256 _tokenAmount) external override onlyKeyHolder onlyUnlockable {
    ...
    function _isLockedFungibleAddress(address _tokenAddress) private view returns (bool) {
        LibVaultStorage.DiamondStorage storage ds = LibVaultStorage.diamondStorage();
        for (uint256 i = 0; i < ds.fungibleTokenDeposits[LibCommonConsts.INNER_STRUCT].length; i++) {
            if (_tokenAddress == ds.fungibleTokenDeposits[LibCommonConsts.INNER_STRUCT][i].tokenAddress) {
                return true;
            }
        }
