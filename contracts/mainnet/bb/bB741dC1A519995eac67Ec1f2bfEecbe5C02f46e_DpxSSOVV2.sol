...
contract DpxSSOVV2 is ContractWhitelist, Pausable, IERC20SSOV {
    ...
    constructor(
        ...
        require(_dpxPriceOracle != address(0), 'E1');
        ...
        addresses['DpxPriceOracle'] = _dpxPriceOracle;
    ...
    function purchase(
        ...
        if (totalFee > 0) {
            ...
            IERC20(dpx).safeTransfer(getAddress('FeeDistributor'), totalFee);
        }
    ...
    /**
     * @notice Settle calculates the PnL for the user and withdraws the PnL in DPX to the user. Will also the burn the option tokens from the user.
    ...
    function settle(
        ...
        if (totalFee > 0) {
            // Transfer fee to FeeDistributor
            dpx.safeTransfer(getAddress('FeeDistributor'), totalFee);
        }

        // Transfer PnL to user
        dpx.safeTransfer(msg.sender, pnl - totalFee);
        ...
        returns (uint256[2] memory)
    {
        ...
        return [userDpxAmount, userRdpxAmount];
    }
    ...
    /**
     * @notice Returns DPX price in USD
     */
    function getUsdPrice() public view returns (uint256) {
        return IPriceOracle(getAddress('DpxPriceOracle')).getPriceInUSD();
    }
