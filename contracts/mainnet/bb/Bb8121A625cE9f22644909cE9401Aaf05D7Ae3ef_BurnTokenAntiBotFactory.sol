...
import { IStandardBurnERC20AntiBot } from "../interfaces/IStandardBurnERC20AntiBot.sol";

contract BurnTokenAntiBotFactory is TokenFactoryBase {
    ...
    function create(
        ...
        token = Clones.cloneDeterministic(implementation, keccak256(abi.encodePacked(msg.sender, name, symbol, decimals, totalSupply, burnFeeBps, enableAntiBot)));
        IStandardBurnERC20AntiBot(token).initialize(
            msg.sender,
            name,
            symbol,
            decimals,
        ...
        assignTokenToOwner(msg.sender, token, 10);
        emit TokenCreated(msg.sender, token, 10, implementationVersion);
    }
}
 ...
 * 🚀 Unleash the power of decentralized finances and tokenization with DEXTools Token Creator. Customize your token seamlessly. Manage your created tokens conveniently from your user panel - start creating your dream token today!
 */
interface IStandardBurnERC20AntiBot {
    function initialize(
        address owner_,
        string memory name_,
