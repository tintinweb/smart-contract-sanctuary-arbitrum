...
contract V3Utils is IERC721Receiver, Common {
    ...
    function onERC721Received(address, address from, uint256 tokenId, bytes calldata data)  whenNotPaused() external override returns (bytes4) {
        ...
        if (instructions.whatToDo == WhatToDo.COMPOUND_FEES) {
            ...
            if (instructions.targetToken == token0) {
                ...
                (liquidity,amount0, amount1) = _swapAndIncrease(SwapAndIncreaseLiquidityParams(instructions.protocol, nfpm, tokenId, amount0, amount1, instructions.recipient, instructions.deadline, IERC20(address(0)), 0, 0, "", 0, 0, "", instructions.amountAddMin0, instructions.amountAddMin1, 0), IERC20(token0), IERC20(token1), instructions.unwrap);
            }
            ...
            uint256 token1Added;
            if (instructions.targetToken == token0) {
                (newTokenId, newLiquidity, token0Added, token1Added) = _swapAndMint(SwapAndMintParams(instructions.protocol, nfpm, IERC20(token0), IERC20(token1), fee, instructions.tickLower, instructions.tickUpper, 0, amount0, amount1, instructions.recipient, instructions.deadline, IERC20(token1), instructions.amountIn1, instructions.amountOut1Min, instructions.swapData1, 0, 0, "", instructions.amountAddMin0, instructions.amountAddMin1), instructions.unwrap);
            } else if (instructions.targetToken == token1) {
                (newTokenId, newLiquidity, token0Added, token1Added) = _swapAndMint(SwapAndMintParams(instructions.protocol, nfpm, IERC20(token0), IERC20(token1), fee, instructions.tickLower, instructions.tickUpper, 0, amount0, amount1, instructions.recipient, instructions.deadline, IERC20(token0), 0, 0, "", instructions.amountIn0, instructions.amountOut0Min, instructions.swapData0, instructions.amountAddMin0, instructions.amountAddMin1), instructions.unwrap);
            } else {
                // no swap is done here
                (newTokenId, newLiquidity, token0Added, token1Added) = _swapAndMint(SwapAndMintParams(instructions.protocol, nfpm, IERC20(token0), IERC20(token1), fee, instructions.tickLower, instructions.tickUpper, 0, amount0, amount1, instructions.recipient, instructions.deadline, IERC20(address(0)), 0, 0, "", 0, 0, "", instructions.amountAddMin0, instructions.amountAddMin1), instructions.unwrap);
            }
    ...
    function swapAndMint(SwapAndMintParams calldata params)  whenNotPaused() external payable returns (uint256 tokenId, uint128 liquidity, uint256 amount0, uint256 amount1) {
        ...
        uint256 feeAmount0;
        uint256 feeAmount1;
        // Calldata structs are read-only. so we need to clone them
        SwapAndMintParams memory _params = params;
        (_params.amount0, _params.amount1, feeAmount0, feeAmount1) = _takeFee(params.amount0, params.amount1, params.protocolFeeX64, FeeType.PROTOCOL_FEE);

        IWETH9 weth = _getWeth9(_params.nfpm, _params.protocol);
        _prepareSwap(weth, _params.token0, _params.token1, _params.swapSourceToken, _params.amount0, _params.amount1, _params.amountIn0 + _params.amountIn1);
        (tokenId, liquidity, amount0, amount1) = _swapAndMint(params, msg.value != 0);
        emit TakeFees(address(_params.nfpm), tokenId, _params.recipient, _params.amount0, _params.amount1, feeAmount0, feeAmount1, _params.protocolFeeX64, FeeType.PROTOCOL_FEE);
    }
    ...
    function swapAndIncreaseLiquidity(SwapAndIncreaseLiquidityParams calldata params)  whenNotPaused() external payable returns (uint128 liquidity, uint256 amount0, uint256 amount1) {
        ...
        SwapAndIncreaseLiquidityParams memory _params = params;
         ...
         (_params.amount0, _params.amount1, feeAmount0, feeAmount1) = _takeFee(params.amount0, params.amount1, params.protocolFeeX64, FeeType.PROTOCOL_FEE);
        emit TakeFees(address(_params.nfpm), _params.tokenId, _params.recipient, _params.amount0, _params.amount1, feeAmount0, feeAmount1, _params.protocolFeeX64, FeeType.PROTOCOL_FEE);
        (address token0,address token1,,,,) = _getPosition(_params.nfpm, _params.protocol, _params.tokenId);
        IWETH9 weth = _getWeth9(_params.nfpm, _params.protocol);
        _prepareSwap(weth, IERC20(token0), IERC20(token1), _params.swapSourceToken, _params.amount0, _params.amount1, _params.amountIn0 + _params.amountIn1);
