...
contract Handler is IHandler,Initializable,OwnableUpgradeable{
  ...
  function getPoolVolumes(address _pool, uint256 beginDate,uint256 delta) public view returns(PoolVolume[] memory){
    uint256 beginYear = beginDate/10000;
    uint256 beginMonth = (beginDate - beginYear*10000)/100;
    uint256 beginTs = timestampFromDate(beginYear,beginMonth,beginDate-beginYear*10000-beginMonth*100);

    PoolVolume[] memory volumes = new PoolVolume[](delta+1);
    for (uint256 i = 0; i < delta+1; i++) {
      uint256 dateNum = timestampToDateNumber(beginTs+60*60*24*i);
      volumes[i] = dailyPoolVolumes[_pool][dateNum];
    }
    return volumes;
  }
  function getPoolSupplies(address _pool, uint256 beginDate,uint256 delta) public view returns(PoolLiquidity[] memory){
    uint256 beginYear = beginDate/10000;
    uint256 beginMonth = (beginDate - beginYear*10000)/100;
    uint256 beginTs = timestampFromDate(beginYear,beginMonth,beginDate-beginYear*10000-beginMonth*100);
    
    PoolLiquidity[] memory supplies = new PoolLiquidity[](delta+1);
    for (uint256 i = 0; i < delta+1; i++) {
      uint256 dateNum = timestampToDateNumber(beginTs+60*60*24*i);
      supplies[i] = dailyPoolLiquidity[_pool][dateNum];
    }
    return supplies;
  }
  ...
  function afterRebate(
    ...
    PoolVolume storage volume = dailyPoolVolumes[collateralToken][date];
    volume.rebateFee = volume.rebateFee + amount;
    volume.rebateFeeInUsd = volume.rebateFeeInUsd + amount*poolPrice;
  }
}
