...
// SPDX-License-Identifier: BUSL-1.1 AND MIT AND UNLICENSED AND Unlicense
...
// File @uniswap/v2-core/contracts/interfaces/IUniswapV2Pair[email protected]
...
interface IOnlyHumans {
    function isUserAllowed(address contractAddress, address user) external view returns (bool);
}
...
contract Testtoken is ERC20, Ownable, BaseOFTV2 {
    IOnlyHumans public antibot;
    uint256 public contractStartTime;
    uint256 public constant TIME_LIMIT = 60 * 0;
    bool antibotEnabled = false;

    address private WETH;
    address public constant uniswapV2Router02 = 0x8cFe327CEc66d1C090Dd72bd0FF11d690C33a2Eb;
    ...
    uint16 public feeOwnerInitialPercentageBuy = 2000;
    uint16 public feeOwnerInitialPercentageSell = 2000;
    uint16 public feeOwnerPercentageBuy = 200;
    uint16 public feeOwnerPercentageSell = 200;
    uint16 public burnFeePercentage = 0;
    uint256 private collectedAdmin = 0;

    uint256 public maxTokenAmountPerWallet = 20000 * 10 ** decimals();
    uint256 public maxTokenAmountPerTransaction = 10000 * 10 ** decimals();
    
    uint256 private buyCount = 0;
    uint256 private initialBuyCountTreshold = 30;
    ...
    constructor(
        ...
        ) ERC20(unicode"Test Token", unicode"TEST") BaseOFTV2(_sharedDecimals, _lzEndpoint)  {

        router = IUniswapV2Router02(uniswapV2Router02);
        WETH = router.WETH();
        pair = IUniswapV2Factory(router.factory()).createPair(WETH, address(this));
        pairContract = IUniswapV2Pair(pair);
        uint256 ownerReceiverSupply = (1000000 * 200) / 10000;
        ...
        if (ownerReceiverSupply > 0) {
            _mint(msg.sender, (1000000 - ownerReceiverSupply) * 10 ** decimals());
            _mint(0x3566c48004749DBaEFbA82529Df45244deaA8c5E, ownerReceiverSupply * 10 ** decimals());
        } else {
            _mint(msg.sender, (1000000) * 10 ** decimals());
        }

        if (_antibotAddress != address(0)) {
            antibotEnabled = true;
            antibot = IOnlyHumans(_antibotAddress);
    ...
    function checkWhitelist(address user) public view returns (bool) {
        if (block.timestamp <= contractStartTime + TIME_LIMIT) {
            return antibot.isUserAllowed(address(this), user);
        }
        return true;
    }
    ...
    function _transfer(
        ...
        || inSwap) {
            ...
            if(antibotEnabled)
                require(checkWhitelist(tx.origin), "Address not whitelisted");
