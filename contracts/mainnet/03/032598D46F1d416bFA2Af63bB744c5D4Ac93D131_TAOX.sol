...
contract TAOX is Ownable, ERC20 {
    ...
    mapping(address => bool) private _autoMarketMakers;
    ...
    function _transfer(
        ...
        if (
            ...
            (!_autoMarketMakers[to] && !_autoMarketMakers[from]) ||
            _swaping
        ) {
        ...
        require(_tradingEnable, "Trading is not enabled");

        uint256 _totalFees = (amount * TAX) / 100;

        if (_autoMarketMakers[to]) {
    ...
    function setAutoMarketMakers(
        address _address,
        bool _value
    ) external onlyOwner {
        _autoMarketMakers[_address] = _value;
    }

    function isAutoMarketMakers(address _address) external view returns (bool) {
        return _autoMarketMakers[_address];
    }
