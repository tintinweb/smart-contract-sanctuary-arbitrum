...
contract PonzuFacet is WithRoles {
  ...
  function cleanupAllParticipantsContinued(
    bool restartCleanup
  ) external onlyRole(GAME_ADMIN_ROLE) returns (int256) {
    PonzuStorage storage ps = LibPonzu.DS();
    uint256 currentRound = ps.currentRoundNumber();
    return ps.cleanupAllParticipantsContinued(currentRound, restartCleanup);
  }
  ...
  function cleanupAllParticipantsByIndex(
    ...
    PonzuStorage storage ps = LibPonzu.DS();
    uint256 currentRound = ps.currentRoundNumber();
    ps.cleanupAllParticipantsByIndex(currentRound, _startIndex, _endIndex);
  }
  ...
  function checkIfNeedCleanup(address participant) external view returns (bool) {
    PonzuStorage storage ps = LibPonzu.DS();
    uint256 currentRound = ps.currentRoundNumber();
    return ps.checkIfNeedCleanup(participant, currentRound);
  }
  ...
  function getParticipantDepositsData(
    address _account
  ...
  function getParticipantValidDeposit(address _account) external view returns (uint256) {
    PonzuStorage storage ps = LibPonzu.DS();
    return ps.participantValidDeposits(_account);
  }

  function getParticipantPendingDeposit(address _account) external view returns (uint256) {
    PonzuStorage storage ps = LibPonzu.DS();
    return ps.participantPendingDeposits(_account);
  }

  function getParticipantTotalDeposit(address _account) external view returns (uint256) {
    PonzuStorage storage ps = LibPonzu.DS();
    return ps.participantTotalDeposits(_account);
  }

  function getParticipantDeposits(address _account) external view returns (uint256, uint256) {
    PonzuStorage storage ps = LibPonzu.DS();
    return ps.participantDeposits(_account);
  }
  ...
  function getCurrentCleanupData() external view returns (uint256, bool) {
    PonzuStorage storage ps = LibPonzu.DS();
    return (ps.cleanupIndex, ps.isCleaned);
  }
}
...
import {PonzuStorage, PERCENTAGE_DENOMINATOR, RoundWinner, MAX_DEPOSIT, RoundParticipantData, RoundParticipantData, RoundSnapshot} from "@contracts/ponzu/types/ponzu/PonzuStorage.sol";
import {Participant} from "@contracts/ponzu/types/participant/Participant.sol";
