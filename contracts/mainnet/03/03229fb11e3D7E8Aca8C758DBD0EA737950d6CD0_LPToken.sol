...
contract LPToken is Context, Ownable, ILPToken {
    ...
    modifier onlyKeeper(address addr) {
        require(
            isKeeper(addr),
            "caller is not LP keeper"
        );
        _;
    }

    function setKeeper(address addr,bool active) public onlyOwner {
        require(addr.isContract(), "LPToken: address must be LPToken contract");
     ...
     * Requirements
     *
     * - `msg.sender` must be the token contract keeper
     */
    function mint(uint256 amount) public onlyKeeper(msg.sender) returns (bool) {
