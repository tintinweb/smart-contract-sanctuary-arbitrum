...
import "./utils/IOFTUpgradeable.sol";
import "./utils/OFTCoreUpgradeable.sol";
...
contract Rumi is Initializable, OFTUpgradeable {
    ...
    function mintTokensToMasterChef(address _masterChefAddress, uint256 _amount) external onlyOwner {        
        _mint(_masterChefAddress, _amount);       
    }
...
import "./interfaces/IRumiToken.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
...
import "./utils/lzApp/NonblockingLzAppUpgradeable.sol";
...
contract RumiPresale is ReentrancyGuardUpgradeable, OwnableUpgradeable, NonblockingLzAppUpgradeable {
    ...
    // USDC Circle in Arbitrum
    address public USDCc;
    ...
    function buyTokensWithStable(
        ...
        address[] memory stableArray = new address[](4); 
        stableArray[0] = USDC;
        stableArray[1] = DAI;
        stableArray[2] = USDT;
        stableArray[3] = USDCc; //enabled for arbitrum              
    ...
    function forwardFunds(
        ...
        require(balanceOfToken >= _amount, "Insufficient balance");
       _transferERC(treasury, _amount, _token);       
        return true;
    }
    ...
    function _endPresale() internal returns (bool) {
        ...
        if(allLiquidity>0) _TOKEN.transfer(treasury, allLiquidity);       
        if(address(this).balance>0) _transferEth(payable(treasury), address(this).balance);        
        _transferERC(treasury, IERC20(USDC).balanceOf(address(this)), USDC);
    ...
    function setUSDCc(address _usdcc) public onlyOwner {
        USDCc = _usdcc;
        priceFeeds[USDCc] = AggregatorV3Interface(PRICE_FEED_USDC);
    }    
...
interface IOFTCore is IERC165 {
     ...
     */
    function sendFromToken(address _from, uint16 _dstChainId, bytes calldata _toAddress, uint _amount, address payable _refundAddress, address _zroPaymentAddress, bytes calldata _adapterParams) external payable;
...
import "../interfaces/IOFTReceiver.sol";
...
abstract contract LzAppUpgradeable is Initializable, OwnableUpgradeable, ILayerZeroReceiverUpgradeable, ILayerZeroUserApplicationConfigUpgradeable {
    ...
    mapping(uint16 => bytes) public trustedRemoteLookupToken;
    ...
    function getTrustedRemoteAddressToken(uint16 _remoteChainId) external view returns (bytes memory) {
        bytes memory path = trustedRemoteLookup[_remoteChainId];
        require(path.length != 0, "LzApp: no trusted path record");
        return path.slice(0, path.length - 20); // the last 20 bytes should be address(this)
    }
