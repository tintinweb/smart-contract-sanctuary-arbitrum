...
// File: contracts/gethstaking.sol
...
contract gethstaking is Ownable,ReentrancyGuard {
    ...
    IERC20 public WETH = IERC20(0x373217e8DAEE6744D4470E2b8955F4f86E70eCf3);
    IERC20 public xGND = IERC20(0x40EA7f6D6964413D4a26A0a268542DAE9F55768E);
    ...
    function pendingRP(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 XGNDSupply = xGND.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && XGNDSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 RPReward = multiplier.mul(RPPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accRPPerShare = accRPPerShare.add(RPReward.mul(1e12).div(XGNDSupply));
        }
        uint256 userXGNDPoint = user.xGNDamount;
        return userXGNDPoint.mul(accRPPerShare).div(1e12).sub(user.RPrewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accRPPerShare = pool.accRPPerShare.add(RPReward.mul(1e12).div(xGNDSupply));
        pool.lastRewardTime = block.timestamp;
    }

    // Deposit LP tokens to MasterChef for WETH allocation.
    function deposit(uint256 _pid, uint256 _amount, uint256 _xGNDamount) public nonReentrant {
        ...
        uint256 RPpending = userXGNDPoint.mul(pool.accRPPerShare).div(1e12).sub(user.RPrewardDebt);

        user.amount = user.amount.add(_amount).sub(fee);
        user.xGNDamount = user.xGNDamount.add(_xGNDamount);
        user.RPamount = user.RPamount.add(RPpending);

        userPoint = user.amount.add(user.RPamount); 
        userXGNDPoint = user.xGNDamount; 

        user.rewardDebt = userPoint.mul(pool.accWETHPerShare).div(1e12);
        user.RPrewardDebt = userXGNDPoint.mul(pool.accRPPerShare).div(1e12);
        ...
        xGND.safeTransferFrom(address(msg.sender), address(this), _xGNDamount);
        pool.lpToken.safeTransfer(owner(), fee);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from MasterChef.
    function withdraw(uint256 _pid, uint256 _amount, uint256 _xgndAmount) public nonReentrant {  
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        user.xGNDamount = user.xGNDamount.sub(_xgndAmount);
        ...
        xGND.safeTransfer(address(msg.sender), _xgndAmount);
