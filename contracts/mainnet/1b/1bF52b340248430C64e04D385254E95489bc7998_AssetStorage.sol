...
import { BPS, PercentageMath } from  "../../libraries/PercentageMath.sol";
import { Errors } from  "../../libraries/Errors.sol";

contract AssetStorage is AdminAccessControl, IAssetStorage {

  mapping (address => IAssetStorage.Asset) private _assets;

  // stores the indices for the assets such that we can return all
  mapping (uint256 => address) private _indexes;

  // the number of assets that have been registered
  uint256 private _count;

  constructor(IAddressRegistry registry) AdminAccessControl(registry) { }

  function validate(Asset memory asset) private pure {
    if (asset.token == address(0)) {
      revert Errors.Validation_ZeroAddressNotAllowed();
    }

    if (asset.custodian == address(0)) {
      revert Errors.Validation_ZeroAddressNotAllowed();
    }
    ...
    if (BPS.unwrap(asset.maxLTV) > BPS.unwrap(PercentageMath.ONE_HUNDRED_PERCENT)) {
      revert Errors.Validation_MaxLTVOutOfRange({ 
        maxLTV: asset.maxLTV 
      });
    }
  }
   ...
   * @notice Adds or updates the asset.
   */
  function setAsset(IAssetStorage.Asset memory asset) external onlyAdmin {
    validate(asset);

    IAssetStorage.Asset storage found = _assets[asset.token];

    if (found.custodian == address(0)) {
      // the asset doesnt yet exist so we need to track it 
      // to enable us to return all assets when needed
      _indexes[_count++] = asset.token;
    }

    found.token = asset.token;
    found.custodian = asset.custodian;
    found.priceOracle = asset.priceOracle;
    found.marketplaceAdapter = asset.marketplaceAdapter;
    found.maxSupply = asset.maxSupply;
    found.hooks.callback = asset.hooks.callback;
    found.hooks.options = asset.hooks.options;
    found.maxLTV = asset.maxLTV;
    found.liquidationDiscount = asset.liquidationDiscount;

    emit SetAsset(
      asset.token, 
      asset.custodian, 
      asset.priceOracle, 
      asset.marketplaceAdapter, 
      asset.maxLTV, 
      asset.liquidationDiscount,
      asset.maxSupply,
      asset.hooks);
  }

  /// @inheritdoc IAssetStorage
  function getAssets() external view returns (IAssetStorage.Asset[] memory assets) {
    uint256 count = _count;

    assets = new IAssetStorage.Asset[](count);

    for (uint256 i = 0; i < count;) {      
      assets[i] = _assets[_indexes[i]];
  ...
  /// @inheritdoc IAssetStorage
  /// @dev This specifically doesnt revert upon an asset not being found as this would mean an additional 
  /// SLOAD that we can avoid by having the calling functions assert that they have the data they needed.
  function getAsset(address token, uint16 mask) public view returns (IAssetStorage.Asset memory) {
    Asset storage found = _assets[token];

    return Asset({
      token: token,
      custodian: mask & ASSET_FIELD_CUSTODIAN != 0 ? found.custodian : address(0),
      marketplaceAdapter: mask & ASSET_FIELD_MARKETPLACE_ADAPTER != 0 ? found.marketplaceAdapter : address(0),
      priceOracle: mask & ASSET_FIELD_PRICE_ORACLE != 0 ? found.priceOracle : address(0),
      maxSupply: mask & ASSET_FIELD_MAX_SUPPLY != 0 ? found.maxSupply : 0,
      hooks: mask & ASSET_FIELD_HOOKS != 0 ? found.hooks : PortfolioHooks(address(0), 0),
      maxLTV: mask & ASSET_FIELD_MAX_LTV != 0 ? found.maxLTV : BPS.wrap(0),
      liquidationDiscount: mask & ASSET_FIELD_LIQUIDATION_DISCOUNT != 0 ? found.liquidationDiscount : BPS.wrap(0)
    });
  }

  /// @inheritdoc IAssetStorage
  function getAssets(address[] calldata tokens, uint16 mask) external view returns (IAssetStorage.Asset[] memory assets) {
    assets = new Asset[](tokens.length);
    
    for (uint256 i = 0; i < tokens.length;) {
      assets[i] = getAsset(tokens[i], mask);
