...
contract ChainlinkARBOracleWNT is AccessManaged, Pausable {
  ...
  int256 public constant SAFE_MULTIPLIER = 1e18;
  ...
  // Mapping of token to Chainlink ETH price feed
  mapping(address => address) public feeds;
  // Mapping of token to maximum delay allowed (in seconds) of last price update
  mapping(address => uint256) public maxDelays;
  // Mapping of token to price feed approximates
  mapping(address => int256) public approximates;
  ...
  */
  ...
  /**
    * @notice Get token ratio to ETH from Chainlink feed
    * @param token Token address
    * @return price Amount in ETH in int256
    * @return decimals Ratio decimals in uint8
  */
  function consult(address token) public view whenNotPaused returns (int256, uint8) {
    ...
    // As feed data may differ quite consistently across ETH and ARB,
    // we utilise a multiplier to approximate the given data on ARB feeds to ETH
    return (
      chainlinkResponse.answer * approximates[token] / SAFE_MULTIPLIER,
  ...
  */
  ...
  /**
    * @notice Update multiplier for feed data approximation
    * @dev This is used to normalize the feed data from chainlink across ETH to ARB.
    * E.g. if it is consistently 2% lower, we use a multiplier of 1.02e18 to approximate
    * @param token Token address
    * @param multiplier  Multiplier in 1e18
  */
  function updateApproximate(address token, int256 multiplier) external restricted {
    if (token == address(0)) revert Errors.ZeroAddressNotAllowed();
    if (feeds[token] == address(0)) revert Errors.NoTokenPriceFeedAvailable();

    approximates[token] = multiplier;
  }
