    ...
    ERC20Upgradeable
{
    ...
    event StakingLockedCanceled(bool state, uint256 timestamp);
    ...
    function cancelLock(bool status) external onlyOwner {
        lockCanceled = status;
        emit StakingLockedCanceled(status, block.timestamp);
    }
