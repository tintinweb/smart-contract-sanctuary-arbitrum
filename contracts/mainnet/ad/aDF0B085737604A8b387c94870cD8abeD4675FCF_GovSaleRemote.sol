  ...
  ReentrancyGuardUpgradeable
{
  ...
  // function totalStaked() public view virtual returns (uint256 totalStakedAmount) {
  //   uint256 chainStakedLength = factory.allChainsStakedLength();
  //   for (uint256 i = 0; i < chainStakedLength; ++i) {
  //     totalStakedAmount += chainStaked[uint240(factory.allChainsStaked(i))];
  //   }
  // }
  ...
  function calcUserAllocation(address _user, uint128 _boosterRunning) public view virtual returns (uint256 userAlloc) {
    ...
    if (_boosterRunning == 1) {
      ...
      if (uint128(block.timestamp) >= booster[2].start) {
        // userAlloc = SaleLibrary.calcAllocFromKom(candidateStakedToken, totalStaked(), saleAmount - booster1Achieved);
  ...
  function finalize(bytes memory _salePayload) external virtual {
    ...
    for (uint256 i = 0; i < remoteUsers.length; ++i) {
      ...
      if (summary.bought > remoteBought) {
        ...
        uint256 payback = (summary.bought - remoteBought) + surplusFee;
        summaries[user] = Summary(remoteUsersReceived[i], remoteBought, remoteFee);

        _releaseToken(user, payback);
      }
    }
  ...
  //  */
  // function setRefund(uint256 _refund_d2) external virtual onlyOwner {
  //   require(boosterProgress() == 4 && paused(), 'bad');
