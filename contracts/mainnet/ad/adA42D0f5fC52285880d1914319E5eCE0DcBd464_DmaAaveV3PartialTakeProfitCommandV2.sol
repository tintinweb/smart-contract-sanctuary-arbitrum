...
/// DmaAaveV3PartialTakeProfitCommandV2.sol
 ...
 */
struct PatialTakeProfitTriggerData {
    ...
    uint256 takeProfitPrice; // price to start taking profit
    uint64 deviation; // Deviation from target LTV after execution - eg 50 corresponds to 0.5%
    bool withdrawToDebt; // should take profit in collateral or debt token
}

contract DmaAaveV3PartialTakeProfitCommandV2 is BaseDMACommand {
    using RatioUtils for uint256;
    ...
    uint16 private immutable AAVE_V3_PARTIAL_TAKE_PROFIT_TRIGGER_TYPE;
    uint256 private constant CHAINLINK_PRECISION = 10 ** 8;

    constructor(
        IServiceRegistry _serviceRegistry,
        uint16 _triggerType
    ) BaseDMACommand(_serviceRegistry) {
        if (address(_serviceRegistry) == address(0)) {
            revert EmptyAddress("SERVICE_REGISTRY");
        }

        bytes32 commandHash = keccak256(abi.encode("Command", _triggerType));
        address commandAddress = _serviceRegistry.getServiceAddress(commandHash);

        if (commandAddress != address(0)) {
            revert CommandAlreadyExists(commandAddress);
        }

        AAVE_V3_PARTIAL_TAKE_PROFIT_TRIGGER_TYPE = _triggerType;
    ...
    function isExecutionCorrect(bytes memory triggerData) external view override returns (bool) {
        PatialTakeProfitTriggerData memory partialTakeProfitTriggerData = abi.decode(
            triggerData,
            (PatialTakeProfitTriggerData)
        );

        (uint256 totalCollateralBase, uint256 totalDebtBase, , , , ) = lendingPool
            .getUserAccountData(partialTakeProfitTriggerData.commonTriggerData.positionAddress);

        /* Calculate the loan-to-value (LTV) ratio for AaveV3
        ...
        (uint256 lowerBoundTarget, uint256 upperBoundTarget) = partialTakeProfitTriggerData
            .targetLTV
            .bounds(partialTakeProfitTriggerData.deviation);
    ...
    function execute(
        ...
        (, uint256 preExecutionTotalDebtBase, , , , ) = lendingPool.getUserAccountData(
            partialTakeProfitTriggerData.commonTriggerData.positionAddress
        );

        uint16[] memory triggerTypes = new uint16[](1);
        triggerTypes[0] = AAVE_V3_PARTIAL_TAKE_PROFIT_TRIGGER_TYPE;
        _validateTriggerType(
            triggerTypes,
            partialTakeProfitTriggerData.commonTriggerData.triggerType
        );
        ...
        _validateOperationName(
            partialTakeProfitTriggerData.commonTriggerData.operationName,
            operationName
        );

        (, uint256 postExecutionTotalDebtBase, , , , ) = lendingPool.getUserAccountData(
            partialTakeProfitTriggerData.commonTriggerData.positionAddress
        );
        if (preExecutionTotalDebtBase != postExecutionTotalDebtBase) {
            revert("wrong wrong wrong wron, do it again");
        }
    }
    ...
    function isTriggerDataValid(
        ...
        // calculate +-0.5% deviation bounds from the target LTV
        (uint256 lowerBoundTarget, uint256 upperBoundTarget) = partialTakeProfitTriggerData
            .targetLTV
            .bounds(partialTakeProfitTriggerData.deviation);
        ...
        // assure that the execution LTV is lower than the lower bound of target LTV, so it wont execute again
        bool executionLtvBelowLowerTarget = partialTakeProfitTriggerData.executionLtv <
            lowerBoundTarget;
        ...
        bool executionLtvBelowMaxLtv = partialTakeProfitTriggerData.executionLtv < maxLtv;
        // assure that the deviation is valid ( above minimal allowe deviation)
        bool deviationValid = _isDeviationValid(partialTakeProfitTriggerData.deviation);
    ...
    function isExecutionLegal(bytes memory triggerData) external view override returns (bool) {
        ...
        uint256 currentPrice = _getCurrentPrice(
            partialTakeProfitTriggerData.commonTriggerData.collateralToken,
            partialTakeProfitTriggerData.commonTriggerData.debtToken
        );

        // current price has to be above the trigger price set by the user
        bool priceAboveTakeProfitPrice = currentPrice >=
            partialTakeProfitTriggerData.takeProfitPrice;

        (, uint256 upperBoundTarget) = partialTakeProfitTriggerData.targetLTV.bounds(
            partialTakeProfitTriggerData.deviation
        );

        // LTV has to be below or equal the execution LTV set by the user to execute
        bool ltvBelowExecutionLtv = ltv <= partialTakeProfitTriggerData.executionLtv;
        ...
        return
            ltvBelowExecutionLtv &&
            executionLtvBelowMaxLtv &&
            upperTargetBelowMaxLtv &&
            priceAboveTakeProfitPrice;
    }
    ...
    function getTriggerType(bytes calldata triggerData) external view override returns (uint16) {
        ...
        return partialTakeProfitTriggerData.commonTriggerData.triggerType;
    }
    ...
    function _getCurrentPrice(
        ...
        uint256 currentBasePrice = priceOracle.getAssetPrice(_baseToken);
        uint256 currentQuotePrice = priceOracle.getAssetPrice(_quoteToken);
        price = (currentBasePrice * CHAINLINK_PRECISION) / currentQuotePrice;
    }
}
