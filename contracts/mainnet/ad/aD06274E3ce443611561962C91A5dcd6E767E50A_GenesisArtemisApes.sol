...
contract GenesisArtemisApes is ERC721, Ownable, Pausable, ReentrancyGuard {
    ...
    constructor(string memory name, string memory symbol, uint256 _maxSupply, uint256 _price, address _paymentToken) ERC721(name, symbol) {
    ...
    function mint() public whenNotPaused nonReentrant {
        ...
        if (!isPublicSaleEnabled) {
            ...
            require(addressToMintCount[msg.sender] < maxPerWhitelist, "You have already minted a token during the whitelist period");
        }
    ...
    function listOwners() external view returns (address[] memory) {
        address[] memory owners = new address[](totalMinted);
    ...
    function increaseMaxSupply(uint256 additionalSupply) external onlyOwner {
        require(maxSupply.add(additionalSupply) <= 10000, "Cannot exceed maximum supply limit");
    ...
    function setMaxPerWhitelist(uint256 _maxPerWhitelist) external onlyOwner {
