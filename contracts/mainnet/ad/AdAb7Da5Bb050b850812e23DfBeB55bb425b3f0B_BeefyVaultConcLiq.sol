...
contract BeefyVaultConcLiq is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    */
    ...
    function balances() public view returns (uint amount0, uint amount1) {
        ...
        (uint bal0, uint bal1) = IStrategyConcLiq(strategy).balances();
        return (IERC20Upgradeable(token0).balanceOf(address(this)) + bal0, IERC20Upgradeable(token1).balanceOf(address(this)) + bal1);
    }

    /**
     * @notice Balance of tokens sitting in this vault contract.
     * @return amount0 the amount of token0
     * @return amount1 the amount of token1
     */
    function available() public view returns (uint, uint) {
        (address token0, address token1) = wants();
        return (IERC20Upgradeable(token0).balanceOf(address(this)), IERC20Upgradeable(token1).balanceOf(address(this)));
    }
     ...
     */
    function depositAll(uint256 _minShares) external {
        (address token0, address token1) = wants();
        deposit(IERC20Upgradeable(token0).balanceOf(msg.sender), IERC20Upgradeable(token1).balanceOf(msg.sender), _minShares);
    }
    ...
    function deposit(uint256 _amount0, uint256 _amount1, uint256 _minShares) public nonReentrant {
        ...
        emit Deposit(msg.sender, shares, _amount0, _amount1);
    }
