...
contract GLPLeverageStrategy is Initializable, OwnableUpgradeable, UUPSUpgradeable, GLPLeverageStrategyStorage {
    ...
    function initializeV2() external reinitializer(2) {
        _claimRewards();
        targetCollateralRatio = type(uint256).max;
        minimumCollateralRatio = type(uint256).max;

        (uint256 glpCollateral, uint256 usdcDebt) = lendingPool.getAccountBalances(address(this));

        (uint256 glpToRedeem, ) = _usdcToGLPWithCost(usdcDebt + 1e6);
        lendingPool.removeCollateral(glpToRedeem, address(this));
        wrappedGLP.withdrawTo(address(this), glpToRedeem);
        ...
        SafeERC20Upgradeable.safeApprove(IERC20Upgradeable(lendingPool.asset()), address(lendingPool), 0);
        ...
        SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(lendingPool.asset()), address(lendingPool), IERC20Upgradeable(lendingPool.asset()).balanceOf(address(this)));

        (glpCollateral, usdcDebt) = lendingPool.getAccountBalances(address(this));

        require(usdcDebt == 0, "Debt not fully repaid");

        _withdrawTo(vault, glpCollateral);
    }
