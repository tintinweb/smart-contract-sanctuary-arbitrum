...
import {MixedPriceSourceMapper} from "./MixedPriceSourceMapper.sol";
import {MixedPriceOracle} from "./MixedPriceOracle.sol";
import {PriceOracleDeployer} from "../PriceOracleDeployer.sol";

/// @title MixedPriceOracleDeployer
/// @dev A contract for deploying Mixed Price Oracles.
contract MixedPriceOracleDeployer is PriceOracleDeployer {
    constructor(address _owner, address _priceSourceMapper, address _sequencerUptimeFeed)
        PriceOracleDeployer(_owner, _priceSourceMapper, _sequencerUptimeFeed)
    {}

    /// @notice Deploys a Mixed Price Oracle contract
    ///
    /// @param chainIds The array of chain IDs
    /// @param currencyBalances The array of currency balances
    ///
    /// @return staticPriceOracle The address of the deployed static price oracle contract
    function deploy(uint256[] calldata chainIds, a160u96[][] calldata currencyBalances)
        external
        override
        returns (address staticPriceOracle)
    {
        staticPriceOracle = address(
            new MixedPriceOracle(
                abi.encode(MixedPriceSourceMapper(priceSourceMapper).getSources(chainIds, currencyBalances)),
                sequencerUptimeFeed
            )
        );
    }
...
/// @notice Abstract contract for mapping price sources using Chainlink or Redstone feed aggregators
...
abstract contract MixedPriceSourceMapper {
    /// @dev Thrown when an unsupported asset is encountered
    ...
    /// @dev Internal function to retrieve the Chainlink or Api3 aggregator and decimals for a given chain ID and currency.
    ///      Decimals for Api3 aggregator is 0
    ///
...
import {BaseStaticPriceOracle, SSTORE2} from "../BaseStaticPriceOracle.sol";
...
contract MixedPriceOracle is RedstoneConsumerBase, BaseStaticPriceOracle {
    ...
    constructor(bytes memory pointerData, address _sequencerUptimeFeed)
        BaseStaticPriceOracle(pointerData, _sequencerUptimeFeed)
    {}
...
import {IPriceOracleDeployer} from "./interfaces/IPriceOracleDeployer.sol";

import {Owned} from "solmate/auth/Owned.sol";

abstract contract PriceOracleDeployer is Owned, IPriceOracleDeployer {
    address public priceSourceMapper;
    address public sequencerUptimeFeed;

    constructor(address _owner, address _priceSourceMapper, address _sequencerUptimeFeed) Owned(_owner) {
        priceSourceMapper = _priceSourceMapper;
        sequencerUptimeFeed = _sequencerUptimeFeed;
    }

    function setPriceSourceMapper(address _priceSourceMapper) external onlyOwner {
        priceSourceMapper = _priceSourceMapper;
    }
...
/// @title BaseStaticPriceOracle
/// @dev A base price oracle contract that uses Chainlink, Redstone and Api3 price feeds.
abstract contract BaseStaticPriceOracle is IStaticPriceOracle {
    uint256 private constant GRACE_PERIOD_TIME = 3600;

    address internal immutable dataPointer;
