...
/// @title PrivateVaultAccessDeployer
/// @author Teahouse Finance

// arb: 0x7923FE4Ce61b23b4E4C5aD80034f5403Fc2Cd64E
// op: 
contract PrivateVaultAccessDeployer {

    event AccessNFTCreated(address indexed nft);

    function deploy(string memory _name, string memory _symbol, address initMint) external {
        PrivateVaultAccess nft = new PrivateVaultAccess(_name, _symbol);
        nft.devMint(1, initMint);
        nft.transferOwnership(msg.sender);

        emit AccessNFTCreated(address(nft));
    }
}
