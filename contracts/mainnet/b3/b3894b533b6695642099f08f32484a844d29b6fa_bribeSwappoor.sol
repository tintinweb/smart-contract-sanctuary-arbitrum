    ...
    AccessControlEnumerableUpgradeable
{
    ...
    function setImpactRate(uint rate) external onlyRole(SETTER_ROLE) {
        impactRate = rate;
    }
    ...
    /// @notice estimate the maximum amount of eth that can be traded for neadRam while maintaining a roughly 10% or less price impact
    function calculateImpact() public view returns (uint max) {
        (, , uint reserve0, ) = getMetadata(weth, neadRam, neadRamWeth);
        uint rate = 10000 - impactRate;
        uint num = (reserve0 * impactRate);
        uint den = (rate * (10000 - factory.pairFee(neadRamWeth))) / 10000;
        uint maxWeth = num / den;
        max = getAmountsOut(weth, ram, false, maxWeth);
    }

    function getAmountsOut(address tokenA, address tokenB, bool stable, uint amount) public view returns (uint) {
        ...
        return getAmountOut(amount, stable, reserve0, reserve1, decimals0, decimals1);
    }
