...
contract PampdaDistributorIncentive is Initializable, AccessControlEnumerableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        _games.add(game_);
        _rewardsAmounts[game_] = 180_000_000 * 10e18;
        pinkLock = pinkLock_;
        lockId = lockId_;
        currency = currency_;
    }
    ...
    function addGame(address game, uint256 rewardAmounts) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _games.add(game);
    ...
    function setLockId(uint256 lockId_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        lockId = lockId_;
        emit LockIdUpdated(lockId);
    }

    function setPinkLock(address pinkLock_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        pinkLock = pinkLock_;
        emit PinkLockUpdated(pinkLock);
    }

    function transferLockOwnership(address newOwner) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function distribute() external onlyRole(Roles.OPERATOR_ROLE) {
        _distribute();
    }
