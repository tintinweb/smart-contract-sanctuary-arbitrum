/**
 *Submitted for verification at Arbiscan.io on 2023-09-07
*/
...
interface IVault {
    ...
    function buyUSDZ(address _token, address _receiver) external returns (uint256);
    function sellUSDZ(address _token, address _receiver) external returns (uint256);
...
contract Router is IRouter {
    ...
    constructor(address _vault, address _usdz, address _weth) public {
        vault = _vault;
        usdz = _usdz;
        weth = _weth;
    ...
    function _vaultSwap(address _tokenIn, address _tokenOut, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        if (_tokenOut == usdz) { // buyUSDZ
            amountOut = IVault(vault).buyUSDZ(_tokenIn, _receiver);
        } else if (_tokenIn == usdz) { // sellUSDZ
            amountOut = IVault(vault).sellUSDZ(_tokenOut, _receiver);
        } else { // swap
