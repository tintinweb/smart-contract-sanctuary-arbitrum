...
import {AtomicSwap} from "./htlc/HTLC.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/utils/Address.sol";
import "./DeBridgeAdapter.sol";

contract DeBridgeExecutor {
    using Address for address;
    ...
    function redeemAndSwap(
        ...
        AtomicSwap htlc = AtomicSwap(gardenHtlc);

        (address redeemer, , , , uint amount, ) = htlc.atomicSwapOrders(orderID);

        require(redeemer == msg.sender, "DeBridgeExecutor: only redeemer can swap");

        htlc.redeem(orderID, secret);

        IERC20 token = htlc.token();

        token.transferFrom(redeemer, address(this), amount);

        token.approve(deBridgeAdapter, amount);

        DeBridgeAdapter(deBridgeAdapter).send{value: msg.value}(
            _tokenAddress,
            _amount,
            _chainIdTo,
            _receiver,
