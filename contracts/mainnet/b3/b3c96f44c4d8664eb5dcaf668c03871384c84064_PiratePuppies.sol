...
/*
 *       _____       _     _              _______ _     _               _____ _           _           _____                   _
 *      / ____|     | |   | |            |__   __(_)   | |             |  __ (_)         | |         |  __ \                 (_)
 *     | |  __  ___ | | __| | ___ _ __      | |   _  __| | ___  ___    | |__) | _ __ __ _| |_ ___    | |__) |   _ _ __  _ __  _  ___  ___
 *     | | |_ |/ _ \| |/ _` |/ _ \ '_ \     | |  | |/ _` |/ _ \/ __|   |  ___/ | '__/ _` | __/ _ \   |  ___/ | | | '_ \| '_ \| |/ _ \/ __|
 *     | |__| | (_) | | (_| |  __/ | | |    | |  | | (_| |  __/\__ \   | |   | | | | (_| | ||  __/   | |   | |_| | |_) | |_) | |  __/\__ \
 *      \_____|\___/|_|\__,_|\___|_| |_|    |_|  |_|\__,_|\___||___/   |_|   |_|_|  \__,_|\__\___|   |_|    \__,_| .__/| .__/|_|\___||___/
 *                                                                                                               | |   | |
 *                                                                                                               |_|   |_|
 */
...
contract PiratePuppies is ERC721AQueryable, ERC2981, Ownable, ReentrancyGuard {
    ...
    uint256 public constant _MAX_SUPPLY = 8000;
    ...
    event AirdropInitiated(address[] users, string message);
     ...
     * @param _fundsReceiver The address to receive collected contract funds.
    ...
    constructor(
        ...
        _setHiddenMetadataURI(_hiddenMetadataUri);
        _setRoyalty(address(this), _royaltyFeesInBips);
        _setFundsReceiver(_fundsReceiver);
    }
    ...
    /**
     * @dev Function to allow the contract owner to reserve mint a specified number of tokens.
     * @param _mintAmount The number of tokens to be minted.
     * @notice Checks if the total supply after minting does not exceed the maximum limit.
     *         Mints the specified number of tokens to the owner's address.
     */
    function reserveMint(uint256 _mintAmount) public onlyOwner {
        require(totalSupply() + _mintAmount <= _MAX_SUPPLY, "Max supply exceeded");
        _safeMint(msg.sender, _mintAmount);
    }

    /**
     * @dev Function to allow the contract owner to airdrop tokens to multiple users.
     * @param _users An array of addresses to receive the airdropped tokens.
     * @notice Requires that the array of users is not empty.
     *         Calls internal _airdrop function to execute the airdrop.
     */
    function airdrop(address[] calldata _users) public onlyOwner {
        require(_users.length > 0, "Users length should be greater than zero!");
        require(totalSupply() + _users.length <= _MAX_SUPPLY, "Airdrop exceeds max supply");
        _airdrop(_users);
    }

    /**
     * @dev Internal function for executing an airdrop to multiple users.
     * @param _users An array of addresses to receive the airdropped tokens.
     * @notice Mints a single token to each address in the users array and emits an event.
     *         Only callable by the contract owner to prevent reentrancy issues.
     */
    function _airdrop(address[] calldata _users) internal onlyOwner {
        for (uint256 i = 0; i < _users.length; i++) {
            _safeMint(_users[i], 1);
        }
        emit AirdropInitiated(_users, "Airdrop done successfully");
    }
    ...
    /**
     * @dev Function to set the revealed state of the contract.
     * @param _state The new revealed state.
     * @notice Allows the contract owner to set whether the metadata is revealed or hidden.
     */
    ...
    /**
     * @dev Function to set the default royalty for the contract.
     * @param _receiver The address to receive royalty fees.
     * @param _royaltyFeesInBips The royalty fees in basis points.
     * @notice Allows the contract owner to set the default royalty fees and recipient address.
     *         Emits an event indicating the successful setting of the default royalty.
     */
    ...
    /**
     * @dev Internal function to set the default royalty for the contract.
     * @param _royaltyRecipient The address to receive royalty fees.
     * @param _royaltyFeesInBips The royalty fees in basis points.
     * @notice Sets the default royalty fees and recipient address.
     *         Emits an event indicating the successful setting of the default royalty.
     */
     ...
     * @param _receiver The address to receive royalty fees for the token.
     * @param _royaltyFeesInBips The royalty fees in basis points for the token.
     * @notice Allows the contract owner to set the royalty fees and recipient address for a specific token.
     *         Emits an event indicating the successful setting of the token royalty.
     */
    ...
    /**
     * @dev Internal function for setting the hidden metadata URI.
     ...
     */
    function _setHiddenMetadataURI(string memory _hiddenMetadataUri) internal {
        hiddenMetadataURI = _hiddenMetadataUri;
    }

    /**
     * @dev Function to set the URI prefix used for constructing token URIs.
     * @param _uriPrefix The new URI prefix.
     * @notice Allows the contract owner to set the URI prefix used for constructing token URIs.
     */
    ...
    /**
     * /**
     * @dev Function to set the URI suffix used for constructing token URIs.
     * @param _uriSuffix The new URI suffix.
     * @notice Allows the contract owner to set the URI suffix used for constructing token URIs.
     */
    ...
    /**
     * @dev Function to set the address to receive funds.
     * @param _receiver The new address to receive funds.
     * @notice Allows the contract owner to set the address to receive funds.
     *         Emits an event indicating the successful setting of the funds receiver.
     */
    function setFundsReceiver(address _receiver) public onlyOwner {
        _setFundsReceiver(_receiver);
    }

    /**
     * @dev Internal function to set the address to receive funds.
     * @param _receiver The new address to receive funds.
     * @notice Sets the address to receive funds internally.
     *         Requires that the new receiver address is valid and different from the current receiver.
     */
    function _setFundsReceiver(address _receiver) internal {
        require(_receiver != address(0), "Invalid address");
        require(_receiver != fundsReceiver, "Same as current receiver");
        fundsReceiver = _receiver;
    }

    /**
     * @dev Function to transfers a token to a specified address as a giveaway.
     * @param winner The address of the winner receiving the token.
     * @param _tokenIdToGiveaway The ID of the token to be given away.
     * @notice Allows the contract owner to transfer a token to a specified address as a giveaway.
     *         Emits an event indicating the successful token giveaway.
     */
    ...
    /**
     * @dev Function to check if a contract supports the given interface.
     * @param interfaceId The interface identifier.
     * @return A boolean indicating whether the contract supports the interface.
     * @notice Checks if the contract supports the ERC721, IERC721, and ERC2981 interfaces.
     */
    ...
    /**
     * @dev Function to allow the contract owner to withdraw the contract's balance.
     * @notice Allows the contract owner to withdraw the contract's balance to the funds receiver address.
     *         Emits an event indicating the successful withdrawal.
     */
    function withdraw() public onlyOwner {
        require(address(this).balance > 0, "No balance to withdraw");
        (bool success,) = payable(fundsReceiver).call{value: address(this).balance}("");
        require(success, "Withdrawal failed");
        emit Withdrawal(msg.sender, fundsReceiver, address(this).balance);
    }

    /**
     * @dev Function to return the base URI.
     * @return The base URI used for constructing token URIs.
     * @notice Returns the base URI used for constructing token URIs.
     *         Overrides the internal function in ERC721A to use the URI prefix.
     */
     ...
     * @notice This function is automatically called when the contract receives
     *         Ether without any specific function call.
     * @notice Handles incoming Ether transactions.
     * @notice This function is marked as 'external' to allow it to be called only
     *         from outside the contract.
     * @notice This function is marked as 'payable' to indicate that it can receive
     *         Ether along with the function call.
     */
