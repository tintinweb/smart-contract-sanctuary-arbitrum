...
interface IOracleFactory {
    function existsRoute(bytes4 _base, bytes4 _quote) external view returns (bool);
}

contract PerpetualSetterFunctions is PerpetualBaseFunctions {
    using ABDKMath64x64 for int128;
    ...
    function _setUnderlyingRiskParams(
        ...
        if (isQuanto) {
            _setExponentialParams(
                _perpetual,
                _underlyingRiskParams[2],
                _underlyingRiskParams[3],
                _underlyingRiskParams[4]
            );
        } else {
    ...
    function _setExponentialParams(
        PerpetualData storage _perpetual,
        int128 _fSigma2,
        int128 _fSigma3,
        int128 _fRho23
    ) internal {
        // we store exponential transformed values:
        // Esig^2 = exp(sig^2) - 1, Erho = (exp(rho*sig2*sig3) - 1) / (Esig2*Esig3)
        int128 fESigma2 = _fSigma2.mul(_fSigma2).exp().sub(ONE_64x64).sqrt();
        int128 fESigma3 = _fSigma3.mul(_fSigma3).exp().sub(ONE_64x64).sqrt();
        int128 fERho23 = _fRho23.mul(_fSigma2).mul(_fSigma3).exp().sub(ONE_64x64).div(
            fESigma2.mul(fESigma3)
        );
        _perpetual.fSigma2 = int32(fESigma2 >> 35);
        _perpetual.fSigma3 = int32(fESigma3 >> 35);
        _perpetual.fRho23 = int32(fERho23 >> 35);
    }
    ...
    function _setDefaultFundRiskParams(
        ...
        _perpetual.fAMMTargetDD = _defaultFundRiskParams[7];
        _perpetual.fAMMMinSizeCC = _defaultFundRiskParams[8];
        _perpetual.fMinimalTraderExposureEMA = _defaultFundRiskParams[9];
        _perpetual.fMinimalAMMExposureEMA = _defaultFundRiskParams[10];
        _perpetual.fMaximalTradeSizeBumpUp = int32(_defaultFundRiskParams[11] >> 35);
    }
    ...
    function _validateOracles(
        ...
        IOracleFactory factory = IOracleFactory(oracleFactoryAddress);
    ...
    function _validateDefaultFundRiskParams(
        ...
        require(_defaultFundRiskParams[8] > 0, "minimal AMM size>0");
        require(_defaultFundRiskParams[9] > 0, "minimal trader exposure EMA>0");
        require(_defaultFundRiskParams[10] > 0, "minimal AMM exposure EMA>0");
        require(_defaultFundRiskParams[11] > ONE_64x64, "trade size bump>1");
    }
    ...
    IPerpetualFactory
{
    ...
    function _activatePerpetual(uint24 _perpetualId) internal {
        ...
        require(perpetual.state == PerpetualState.INVALID, "state should be INVALID");
        ...
        if (pool.isRunning) {
            ...
            emit SetNormalState(_perpetualId);
        } else {
