...
abstract contract BaseLPVault is
    Initializable,
    UUPSUpgradeable,
    ERC20Upgradeable,
    ...
    PausableInitializable
{
    ...
    uint256[50] private __gap_baseVault;
    ...
    /// @notice how much shares user got against 1 asset when last profit/loss notified
    uint public oldConversionRatePerAsset;

    /// @notice how much shares user will get against 1 asset currently
    uint public currentConversionRatePerAsset;

    /// @notice number of times profit/loss notified
    uint public pnlCount;
     ...
     */
    function _initializeBaseLPVault(
        address _asset,
        string memory _name,
        ...
        // pnlCount = 0; // by default it should be one
        _decimals = asset.decimals();
        oldConversionRatePerAsset = currentConversionRatePerAsset =
            10 ** decimals();
    }

    /**
     * @notice transferFrom asset from caller and mints receipt tokens to recevier
     * @param assets number of asset user wants to deposit
     * @param receiver reciever of receipt tokens
     * @return shares amount of receipt tokens minted to the reciever
     */
    function deposit(
        uint256 assets,
        address receiver
    ...
    /**
     * @param assets amount of underlying token to be withdrawn
     * @param receiver address to receive underlying erc20 tokens
     * @param owner owner of shares
     * @return shares amount of shares withdrawn from user
     */
    function withdraw(
        uint256 assets,
        address receiver,
        ...
        console.log(
            shares,
            "preview me shareeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeees"
        );
        if (msg.sender != owner) {
    ...
    /// @notice returns amount of shares that will be minted on deposit
    /// @dev should be implemented in the child acc to strategy
    ...
    /// @notice returns amount of shares withdraw from user
    /// @dev should be implemented in the child acc to strategy
    function previewWithdraw(uint256 assets) public view returns (uint256) {
        // for auditor: ignore roundUp issue in coversion rate
        return
            totalSupply() == 0
                ? assets
                : assets.mulDivUp(
    ...
    function _updateConversionRate() internal {
        pnlCount++;
        uint oldCachedConversionRatePerAsset = oldConversionRatePerAsset;
        oldConversionRatePerAsset = currentConversionRatePerAsset; // to update the conversion rate
        uint256 supply = totalSupply(); // Saves an extra SLOAD if totalSupply is non-zero.
        uint newRate = supply == 0
            ? 10 ** decimals()
            : uint256(10 ** decimals()).mulDivUp(supply, totalAssets());
        console.log(
            newRate,
            "this is new rate-------------------------------------"
        );
        console.log(supply, "this is supply in its decimals");
        console.log(10 ** decimals(), "10**decimals");
        console.log(totalAssets(), "totalAssets()");

        console.log(
            uint256(10 ** decimals()).mulDivDown(supply, totalAssets()),
            "uint256(10 ** decimals()).mulDivDown(supply, totalAssets())"
        );
        currentConversionRatePerAsset =
            ((oldCachedConversionRatePerAsset * (pnlCount - 1)) + newRate) /
            pnlCount;
        // uint cac = (oldCachedConversionRatePerAsset * pnlCount - 1) + newRate;

        console.log(
            currentConversionRatePerAsset,
            "this is currentConversionRatePerAsset-------------"
        );
    }
...
import {BaseLPVault, ERC20Upgradeable, SafeERC20Upgradeable, Roles} from "./BaseLPVault.sol";
import {IGauge} from "./../interfaces/rent/IGauge.sol";
...
contract LPVault is BaseLPVault, ERC721Holder {
    /// -----------------------------------------------------------------------
    /// Library usage
    /// -----------------------------------------------------------------------
    using SafeERC20Upgradeable for ERC20Upgradeable;
    using FixedPointMathLib for uint256; // @todo may be keep it to base
    ...
    event RentClaimRequested(string[] propertiesSymbols, uint lockNftTokenId);
    ...
    function initialize(
        ...
        _initializeBaseLPVault(_stakeToken, _name, _name, _systemRegistry);
    ...
    modifier onlyAllowedMarketplaceBorrower() {
        if (!allowedMarketPlaceBorrowers[msg.sender]) {
            revert("CALLER_NOT_MARKETPLACE_BORROWER");
        }
        _;
    }
    ...
    function setAllowedMarketPlaceBorrower(
        ...
        require(_addr != address(0x00), "ZERO_ADDRESS");
     ...
     */
    // TODO : check which role to use throughout this contract
    function removeMarketplaceBorrower(
        address _addr
    ) external hasRole(Roles.MARKETPLACE_MANAGER) {
    ...
    function _notifyRentAsRewardToGauge(address _propertyAddress) internal {
        ...
        string memory propertySymbol = ERC20Upgradeable(_propertyAddress)
            .symbol();

        string[] memory propertySymbols = new string[](1);
        propertySymbols[0] = propertySymbol;

        uint lockNftTokenId = rentShare.harvestRent(
            propertySymbols,
            address(this)
        );
        address lockNft = address(systemRegistry.lockNft());
        address rentDistributor = address(systemRegistry.rentDistributor());
        IERC721(lockNft).approve(rentDistributor, lockNftTokenId);
        uint rentAmount = IRentDistributor(rentDistributor).redeemRentForVault(
            lockNftTokenId
        );

        if (rentAmount > 0) {
            ERC20Upgradeable(USDC).safeApprove(gaugeAddress, rentAmount);

            IGauge(gaugeAddress).notifyRewardAmount(USDC, rentAmount);
    ...
    function _settleLossIfAny(
        ...
        // if Deposit token > lp token, then means nofity profit : else settle loss
    ...
    function afterRepay(
        ...
        if (amount > 0) {
            // TODO: change this to a reason able address.
            // asset.safeTransfer(
    ...
    function rescueToken(
        ...
        ERC20Upgradeable(_tokenAddress).safeTransfer(msg.sender, _amount);
    }

    function requestRentClaim(
        string[] memory propertiesSymbols
    ) external hasRole(Roles.MARKETPLACE_MANAGER) returns (uint) {
        IRentShare rentShare = systemRegistry.rentShare();
        uint lockNftTokenId = rentShare.harvestRent(
            propertiesSymbols,
            address(this)
        );
        emit RentClaimRequested(propertiesSymbols, lockNftTokenId);
        return lockNftTokenId;
    }

    function redeemRentAndTransferToGauge(
        uint lockNftTokenId,
        address usdcAddress
    ) external returns (uint) {
        address rentDistributor = address(systemRegistry.rentDistributor());
        ...
        IERC721(lockNft).approve(rentDistributor, lockNftTokenId);
        return
            IRentDistributor(rentDistributor).redeem(
                lockNftTokenId,
                usdcAddress,
                gaugeAddress,
                address(0), // not allowing other token to send to gauge except USDC
                0
            );
    }
    ...
    function cheatIncreaseSupply(uint amount) external {
        increaseAssetTotalSupply(amount);
    }

    function cheatDecreaseSupply(uint _amount) external {
        assetTotalSupply -= _amount;
        _updateConversionRate();
        emit SupplyDecreased(_amount);
    }
