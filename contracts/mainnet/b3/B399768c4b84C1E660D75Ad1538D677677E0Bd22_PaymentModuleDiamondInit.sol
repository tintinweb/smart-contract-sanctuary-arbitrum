...
contract PaymentModuleDiamondInit is AccessControl {
    struct InitPaymentModuleDiamondArgs {
        IPaymentModule.AcceptedToken[] acceptedTokenByAddress;
        address usdToken;
        address burnToken;
        address router;
    }

    function init(InitPaymentModuleDiamondArgs memory _input) external {
        _isAcceptedTokensValid(_input.acceptedTokenByAddress);

        LibPaymentModuleStorage.DiamondStorage storage ds = LibPaymentModuleStorage.diamondStorage();

        ds.usdToken = _input.usdToken;
        ds.burnToken = _input.burnToken;
        ds.router = IUniswapV2Router02(_input.router);

        for (uint256 index = 0; index < _input.acceptedTokenByAddress.length; index++) {
            ds.acceptedTokenByAddress[_input.acceptedTokenByAddress[index].token] = _input.acceptedTokenByAddress[index];
            ds.acceptedTokens.push(_input.acceptedTokenByAddress[index].token);
        }

        _grantRole(AccessControlStorage.DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function _isAcceptedTokensValid(IPaymentModule.AcceptedToken[] memory acceptedTokenByAddress) private pure {
        require(acceptedTokenByAddress.length > 0, "PaymentModule::constructor::ZERO: acceptedTokenByAddress cannot be empty.");

        for (uint256 i = 0; i < acceptedTokenByAddress.length; i++) {
            require(acceptedTokenByAddress[i].router != address(0), "PaymentModule::constructor::ZERO: router cannot be zero address.");
            require(bytes(acceptedTokenByAddress[i].name).length > 0, "PaymentModule::constructor::ZERO: name cannot be empty.");
        }
    }
}