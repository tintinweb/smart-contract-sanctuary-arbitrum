...
interface IGmxUtils {
  ...
  function getExecutionGasLimit(bool isDeposit, uint256 callbackGasLimit) external view returns (uint256 executionGasLimit);
...
contract PerpetualVault is Initializable, Ownable2StepUpgradeable {
  ...
  function deposit(uint256 amount) external nonReentrant payable {
    ...
    _transferOutNative(true);
    collateralToken.safeTransferFrom(msg.sender, address(this), amount);
    counter++;
    depositInfo[counter] = DepositInfo(amount, 0, msg.sender, block.number);
   ...
   */
  function withdraw(address recipient, uint256 depositId) public nonReentrant payable {
    ...
    require(block.number - depositInfo[depositId].blockNumber > lockTime, "locked");
    _transferOutNative(false);
   ...
   */
  function run(bool isOpen, bool isLong, uint256 sl, uint256 tp, uint256 _leverage, MarketPrices memory prices, bytes[] memory _swapData) external nonReentrant onlyKeeper {
    require(address(gmxUtils).balance > minEth, "low than minium eth balance");
    marketPrices = prices;
    swapData = _swapData;
    ...
    if (isOpen) {
      if (closed == true) {
        ...
        leverage = _leverage;
        _createIncreasePosition(false, isLong, sl, tp);
      } else {
        if (beenLong == isLong) {
          ...
          nextAction.data = abi.encode(isLong, sltp, _leverage);
  ...
  function runNextAction(MarketPrices memory prices, bytes[] memory _swapData) external gmxLock onlyKeeper {
    ...
    if (nextAction.selector == NextActionSelector.DEPOSIT_ACTION) {
      ...
      if (closed == false) {
        ...
        _createIncreasePosition(true, beenLong, sl, tp);
      } else {
        _issueShares(depositId, true, depositInfo[depositId].amount);
        _entered = false;
        delete nextAction;
      }
      ...
      (
        ...
        bytes memory sltp,
        uint256 _leverage
      ) = abi.decode(nextAction.data, (bool, bytes, uint256));
      delete nextAction;
      (uint256 sl, uint256 tp) = abi.decode(sltp, (uint256, uint256));
      leverage = _leverage;
      _createIncreasePosition(false, _isLong, sl, tp);
      ...
      _entered = true;
      (uint256 sl, uint256 tp) = _cancelPendingOrders();
      _createIncreasePosition(false, beenLong, sl, tp);
    } else {
  ...
  function afterOrderExecution(bytes32 key, IGmxUtils.OrderType orderType, bool isLong, bytes32 positionKey) external {
    ...
    if (orderType == IGmxUtils.OrderType.MarketIncrease) {
      ...
      beenLong = isLong;
      closed = false;
      if (queue.depositId > 0) {
        IGmxUtils.PositionData memory temp = gmxUtils.getPositionInfo(curPositionKey, marketPrices);
        uint256 increased = temp.collateralAmount - queue.amountBefore;
        _issueShares(queue.depositId, true, increased);
      }
      ...
      if (nextAction.selector != NextActionSelector.WITHDRAW_ACTION) {
        closed = true;
      }
    } else if (orderType == IGmxUtils.OrderType.MarketSwap) {
      if (nextAction.selector == NextActionSelector.DEPOSIT_ACTION) {
        uint256 increased = IERC20(indexToken).balanceOf(address(this)) - queue.amountBefore;
        _issueShares(counter, true, increased);
        delete nextAction;
      } else if (nextAction.selector != NextActionSelector.WITHDRAW_ACTION) {
  ...
  function _issueShares(uint256 depositId, bool isMint, uint256 increased) internal {
    if (isMint) {
      uint256 _shares;
      if (totalShares > 0) {
        if (leverage == BASIS_POINTS_DIVISOR && beenLong) {
          uint256 _totalAmountBefore = IERC20(indexToken).balanceOf(address(this)) - increased;
          _shares = increased * totalShares / _totalAmountBefore;
        } else {
          uint256 _totalAmountBefore = _totalAmount(marketPrices) - increased;
          _shares = increased * totalShares / _totalAmountBefore;
        }
      } else {
  ...
  function _transferOutNative(bool isDeposit) internal {
    if (closed == true && isDeposit == true) return;
    uint256 minExecutionFee = gmxUtils.getExecutionGasLimit(isDeposit, callbackGasLimit) * tx.gasprice;
  ...
  function _isFundIdle(MarketPrices memory prices) internal view returns (bool) {
    ...
    if (prices.shortTokenPrice.min * balance > minCollateralUsd) {
      return true;
    } else {
      return false;
    }
   ...
   * @param _isDeposit: true when user deposit action triggers this function, false when run action triggers
  ...
  function _createIncreasePosition(
    ...
    if (leverage == BASIS_POINTS_DIVISOR && _isLong) {
      ...
      for (uint8 i = 0; i < swapData.length; i++) {
        ...
        if (_protocol == PROTOCOL.UNISWAP) {
          ...
          uint256 balBefore = IERC20(indexToken).balanceOf(address(this));
          (bool success, ) = to.call(callData);
          require(success, "uniswap call reverted");
          // last `depositId` equals with `counter` because another deposit is possible only after previous deposit is completely processed
          if (_isDeposit) {
            uint256 increased = IERC20(indexToken).balanceOf(address(this)) - balBefore;
            _issueShares(counter, true, increased);
          }
          beenLong = true;
          closed = false;
          ...
          queue.amountBefore = IERC20(indexToken).balanceOf(address(this));
        }
      }
      delete swapData;
    } else {
      if (_isDeposit) {
        queue.depositId = counter;
        IGmxUtils.PositionData memory temp = gmxUtils.getPositionInfo(curPositionKey, marketPrices);
        queue.amountBefore = temp.collateralAmount;
      }
      nextAction.selector = NextActionSelector.CREATE_DECREASE_ORDER;
      ...
      collateralToken.safeTransfer(address(gmxUtils), amountIn);
      uint256 sizeDelta = marketPrices.shortTokenPrice.max * amountIn * leverage / BASIS_POINTS_DIVISOR;
      IGmxUtils.OrderData memory orderData = IGmxUtils.OrderData({
        market: market,
  ...
  function _createDecreasePosition(
    ...
    if (leverage == BASIS_POINTS_DIVISOR && _isLong) {
      require(swapData.length > 0, "swap data missing");
      for (uint8 i = 0; i < swapData.length; ) {
        (PROTOCOL _protocol, bytes memory data) = abi.decode(swapData[i], (PROTOCOL, bytes));
        if (_protocol == PROTOCOL.UNISWAP) {
          (address to, bytes memory callData) = abi.decode(
            data,
          ...
          (bool success, ) = to.call(callData);
          require(success, "uniswap call reverted");
          if (sizeDeltaInUsd == 0) {    // if (it's not withdraw action)
            closed = true;
          }
          if (nextAction.selector == NextActionSelector.NO_ACTION) {
            _entered = false;
      ...
      if (
        sizeDeltaInUsd == 0 ||
        (sizeInUsd - sizeDeltaInUsd) * BASIS_POINTS_DIVISOR / leverage < minCollateralUsd
      ) {
  ...
  function _cancelPendingOrders() internal returns (uint256 sl, uint256 tp) {
    (sl, tp) = gmxUtils.cancelOrder(marketPrices.indexTokenPrice.min);
  ...
  function _withdraw(address recipient, uint256 depositId, bool forced, uint256 amount) internal returns (bool) {
    ...
    if (forced || balance >= amount) {
      ...
      _issueShares(depositId, false, 0);
      delete nextAction;
      if (forced == false) {
        _entered = false;
      }
    ...
    if (want * marketPrices.shortTokenPrice.min >= positionData.netValue) {
      ...
      sizeDeltaInUsd = want * marketPrices.shortTokenPrice.min * leverage / BASIS_POINTS_DIVISOR;
      collateralDeltaAmount = want;
    }

    nextAction.selector = NextActionSelector.WITHDRAW_ACTION;
    nextAction.data = abi.encode(recipient, depositId, true, amount);
