 ...
 * 
 */
contract Main is AutomationCompatibleInterface, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    struct Draw {
        ...
        uint16 requestConfirmations; // number of block confirmations the VRF service will wait to respond
    ...
    uint16 private defaultRequestConfirmations;
        ...
        ConfirmedOwner(msg.sender)
    {
        ...
        accounts = AccountsContract(_accountsContractAddress);
        s_subscriptionId = _subscriptionId;
        maxWinners = _maxWinners;
        link_token_contract = _link_token_contract;
        keyHash = _keyHash;
        callbackGasLimit = _callbackGasLimit;
        defaultRequestConfirmations = _requestConfirmations;
    ...
    {
        return createDraw(owner, cid, scheduledAt, nbParticipants, nbWinners, algorithm, defaultRequestConfirmations, callbackAddress);      
    }
    ...
    function deployDraw(
        ...
        returns (string memory)
    {
        return createDraw(msg.sender, "", scheduledAt, nbParticipants, nbWinners, algorithm, defaultRequestConfirmations, callbackAddress);     
    }
    ...
    function deployDraw(
        ...
        returns (string memory)
    {
        return createDraw(msg.sender, "", scheduledAt, range, numbers, algorithm, defaultRequestConfirmations, address(0));   
    }
    ...
    function deployDraw(
        ...
        returns (string memory)
    {
        return createDraw(msg.sender, "", scheduledAt, range, numbers, algorithm, requestConfirmations, address(0));   
    }


    function createDraw(
        address _owner,
        string memory tempCid,
        uint64 scheduledAt,
        uint32 nbParticipants,
        uint32 nbWinners,
        uint8 algorithm,
        ...
        returns (string memory)
    {
        ...
        string memory cid = tempCid;

        if (bytes(cid).length == 0) {
            cid = bytesToHex(abi.encodePacked(keccak256(abi.encodePacked(_owner, scheduledAt, nbParticipants, nbWinners, algorithm, requestConfirmations, callbackAddress, drawCount, block.number))));
        }
        ...
        draws[cid] = Draw(_owner, publishedAt, scheduledAt, occuredAt, nbParticipants, nbWinners, entropyNeeded, entropy, false, false, algorithm, requestConfirmations, callbackAddress);
        drawCount++;
        cids[drawCount] = cid;
        ...
        private
    {
        ...
        for (uint32 i = 0; i < _cids.length; i++) {
            ...
            uint16 requestConfirmations = draws[cid].requestConfirmations;
    ...
    function setDefaultRequestConfirmations(uint16 requestConfirmations) external onlyOwner {
        defaultRequestConfirmations = requestConfirmations;
    }
    ...
    function nbValuesBetween(uint32[] memory arr, uint32 min, uint32 max, uint32 imax) private pure returns (uint32) {
        uint32 count = 0;
    ...
    function removeIndexesFromArray(string[] storage arr, uint32[] memory idx) private {
    ...
    function bytesToHex(bytes memory buffer) private pure returns (string memory) {
