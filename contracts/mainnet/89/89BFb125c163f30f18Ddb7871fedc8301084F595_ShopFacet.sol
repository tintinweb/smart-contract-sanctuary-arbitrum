...
import {WithStorage, WithModifiers} from "../lib/LibStorage.sol";
...
import {LibMeta} from "../vendor/lib/LibMeta.sol";
...
struct PriceStorage {
    ...
    uint256 nativeTokenPriceInUsdFixed;
}
...
struct TokensStorage {
    ...
    mapping(address => uint256) foundingCharactersCountByAddress;
    mapping(address => uint256) gen0EggUsdCreditsByAddress;
    mapping(address => uint16) purchasedGen0EggsCountByAddress;
    mapping(address => bool) verifiedFoundingCharactersCountByAddress;
    uint32 gen0EggUsdCostWhitelist;
    uint32 gen0EggUsdCostCommunity;
    uint32 gen0EggUsdCostLastChance;
    uint256 gen0EggGasOffset;
    LibAccessControl.Gen0EggMintStatus gen0EggMintStatus;
    uint16 firstStageUserGen0EggLimit;
    uint16 lastStageUserGen0EggLimit;
    uint16 reservedGen0EggCount;
}
...
struct AccessControlStorage {
    ...
    mapping(address => bool) waitlisted;
}
...
contract WithModifiers is WithStorage {
    ...
    modifier whitelisted(address addr) {
        require(_acs().whitelisted[addr], "Whitelist required");
        _;
    }

    modifier allowlisted(address addr) {
        require(
            _acs().whitelisted[addr] || _acs().waitlisted[addr],
            "Allowlist required"
        );
        _;
    }
