...
contract HatsPoster {
    ...
    event PostRecord(bytes32 nonce, uint256 hatId, Metadata content);
    ...
    function postUpdate(string memory _tag, uint256 _hatId, Metadata memory _content) external onlyWearer(_hatId) {
        emit PostEvent(_tag, _hatId, _content);
    }

    // Posts a record with a nonce and content. Statefully stored onchain.
    function postRecord(bytes32 _nonce, uint256 _hatId, Metadata memory _content) external onlyWearer(_hatId) {
        require(records[_nonce].exists == false, "HatPoster: record exists");

        records[_nonce] = Record(_content.protocol, _content.pointer, true);
        emit PostRecord(_nonce, _hatId, _content);
    }
}
