...
contract LendingVaultV2 is ERC20, ReentrancyGuard, Pausable, Ownable2Step, ILendingVaultV2 {
  ...
  */
  function borrowAPR() public view returns (uint256) {
    ...
    for (uint256 i = 0; i < _approvedBorrowers.length(); i++) {
      _borrowAPR += borrowAPRPerBorrower(_approvedBorrowers.at(i));
    }

    return _borrowAPR / _approvedBorrowers.length();
  }
  ...
  */
  function _pendingInterest(address borrower, uint256 assetAmt) internal view returns (uint256) {
    ...
    // TODO totalBorrow to use individual borrowers borrow; else borrowAPR is not fair.
