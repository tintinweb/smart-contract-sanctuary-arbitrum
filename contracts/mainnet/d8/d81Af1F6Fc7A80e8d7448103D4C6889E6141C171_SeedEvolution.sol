...
contract SeedEvolution is Initializable, SeedEvolutionSettings {
    ...
    onlyEOA
    {
        ...
        for(uint256 i = 0; i < _solsToStake.length; i++) {
            ...
            if(_solToStake.treasureIds.length > 0) {
                require(timeBCAndTreasureStakingShutdown == 0, "Treasures no longer stakable");
        ...
        if(_totalMagicNeeded > 0) {
            magic.safeTransferFrom(msg.sender, lifeDaoAddress, _totalMagicNeeded);
        }
        if(_totalBCNeeded > 0) {
            require(timeBCAndTreasureStakingShutdown == 0, "BC no longer stakable");
    ...
    function _distributePotionsAndTreasures(LifeformInfo storage _info, uint256 _randomNumber) private returns(uint256) {
        ...
        if(timeBCAndTreasureStakingShutdown == 0 || _info.startTime < timeBCAndTreasureStakingShutdown) {
            if(_odds >= 100000) {
    ...
    onlyEOA
    {
        ...
        // Old treasury address. Used as a burn spot. Don't need to worry about transferring to life DAO address.
...
abstract contract SeedEvolutionContracts is Initializable, SeedEvolutionState {
    ...
    onlyAdminOrOwner
    {
        ...
        lifeDaoAddress = _lifeDaoAddress;
    }
...
abstract contract SeedEvolutionSettings is Initializable, SeedEvolutionContracts {
    ...
    function shutdownBCAndTreasureStaking()
    external
    onlyAdminOrOwner
    {
        timeBCAndTreasureStakingShutdown = block.timestamp;
    }
