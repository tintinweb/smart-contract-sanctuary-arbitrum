...
/// @title StopLossPolicy Contract
/// @notice A policy that restricts the collateral and size of the limit order
contract StopLossPolicy is PolicyBase {
    event TradeSettingsSet(address _fundManager, uint256 stopLoss);

    mapping(address => uint256) private fundManagerToStopLoss;
        ...
        returns (IPolicyManager.PolicyHook[] memory implementedHooks_)
    {
        ...
        implementedHooks_[0] = IPolicyManager.PolicyHook.TrailingStopLoss;
    ...
    function passesRule(
        ...
        uint256 stopLoss = fundManagerToStopLoss[_fundManager];
        if (stopLoss != 0) {
            uint256 grossPnlInToken = ((_collateralAmountInToken /
                powerOfTen(_collateralTokenDecimals)) * stopLoss) / 100;
    ...
    function __setTradeSettings(
        ...
        uint256 stopLoss = abi.decode(_encodedSettings, (uint256));

        fundManagerToStopLoss[_fundManager] = stopLoss;

        emit TradeSettingsSet(_fundManager, stopLoss);
    }
    ...
    function getTradeSettings(
        ...
        return abi.encode(fundManagerToStopLoss[_fundManager]);
    }
}
