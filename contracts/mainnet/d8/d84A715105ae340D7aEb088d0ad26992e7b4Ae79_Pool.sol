...
contract Pool {
    ...
    uint256 public maxSafx = 1000000 ether;
	...
	function setLiquidityMiningPeriod(uint _endTime) external onlyOwner {
		endTime = _endTime;
	}
	...
	function deposit(uint256 amount) external payable {
		...
		require(amount + lastBalance <= maxSafx, "!max-safx");
		...
		//IMining(mining).deposit(msg.sender, msg.value); //is msg.value because i have yet to remove the erc20 functionality
	...
	function referrerClaim(address _referrer, uint _amount) external onlyTrading {
		_transferOut(_referrer, _amount);
	}
	...
	function withdraw(uint256 currencyAmount) external {
		...
		//IMining(mining).withdraw(msg.sender, currencyAmount);
