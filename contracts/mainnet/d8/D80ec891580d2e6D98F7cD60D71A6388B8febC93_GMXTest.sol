...
contract GMXTest {
  ...
  function postWithdraw() external {
    ...
    if (_wc.withdrawParams.token == tokenA) {
      _tokenOut = tokenB;
      tokenOutToReceive = tokenB;
    }
    if (_wc.withdrawParams.token == tokenB) {
      _tokenOut = tokenA;
      tokenOutToReceive = tokenA;
    }

    tokenOutToReceiveAmt = IERC20(tokenOutToReceive).balanceOf(address(this));
    swapParamsExecutionFee = _wc.withdrawParams.swapExecutionFee;

    // Send native token for execution fee
    IExchangeRouter(exchangeRouter).sendWnt{value: _wc.withdrawParams.swapExecutionFee}(
      orderVault,
      _wc.withdrawParams.swapExecutionFee
    );
    ...
    _numbers.executionFee = _wc.withdrawParams.swapExecutionFee;
  ...
  function swap(address tokenOut) payable external {
    callbackCalled2 = false;
    // Send native token for execution fee
    IExchangeRouter(exchangeRouter).sendWnt{value: msg.value}(
      orderVault,
      msg.value
    );
    ...
    IExchangeRouter(exchangeRouter).sendTokens(
      ...
      orderVault,
      IERC20(_wc.withdrawParams.token).balanceOf(address(this))
    );
    ...
    IExchangeRouter.CreateOrderParamsAddresses memory _addresses;
    _addresses.receiver = address(this);
    _addresses.initialCollateralToken = _wc.withdrawParams.token;
    _addresses.callbackContract = callback;
    _addresses.market = address(0);
    _addresses.swapPath = _swapPath;
    _addresses.uiFeeReceiver = address(0);
    ...
    _numbers.triggerPrice = 0;
    _numbers.acceptablePrice = 0;
    _numbers.executionFee = msg.value;
    _numbers.callbackGasLimit = 2000000;
    _numbers.minOutputAmount = 0; // TODO
  ...
  receive() external payable {
    ...
    if (msg.sender == depositVault) {
      (bool success, ) = payable(_dc.user).call{value: _dc.depositParams.executionFee}("");
      require(success, "Transfer failed.");
    } else if (msg.sender == withdrawalVault) {
      (bool success, ) = payable(_wc.user).call{value: _wc.withdrawParams.executionFee}("");
      require(success, "Transfer failed.");
    } else if (msg.sender == orderVault) {
      (bool success, ) = payable(_wc.user).call{value: _wc.withdrawParams.swapExecutionFee}("");
      require(success, "Transfer failed.");
    }
