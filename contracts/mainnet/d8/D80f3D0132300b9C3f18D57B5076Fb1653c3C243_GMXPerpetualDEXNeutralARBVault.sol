...
interface IGMXPerpetualDEXNeutralManager {
  struct WorkData {
    address token;
    uint256 lpAmt;
...
interface IGMXPerpetualDEXNeutralReader {
...
import "../../interfaces/vaults/gmx/IGMXPerpetualDEXNeutralManager.sol";
import "../../interfaces/vaults/gmx/IGMXPerpetualDEXNeutralReader.sol";
...
contract GMXPerpetualDEXNeutralARBVault is ERC20, Ownable, ReentrancyGuard, Pausable {
  ...
  */
  ...
  function deposit(address _token, uint256 _amt, uint256 _minSharesAmt) external nonReentrant whenNotPaused {
    ...
    require(_token == USDC || _token == fsGLP, "Invalid token");
    mintMgmtFee();
    ...
    if (_token == USDC) {
      ...
      equityBefore = reader.assetValueWithPrice(glpPrice) - reader.debtValues();
      (bool success) = IStakedGLP(fsGLP).transferFrom(msg.sender, address(manager), _amt);
  ...
  */
  function withdraw(address _token, uint256 _shareAmt, uint256 _minWithdrawAmt) external nonReentrant whenNotPaused {
    ...
    if (_token == USDC) {
      ...
      withdrawAmt = IStakedGLP(fsGLP).balanceOf(address(this));
      (bool success) = IStakedGLP(fsGLP).transfer(msg.sender, withdrawAmt);
      require(success, "Transfer sGLP failed");
    }
