...
contract wGM is ERC20, Ownable, ReentrancyGuard {
    //wGM token
    ...
    address public GM_ERC721 = 0x249bB0B4024221f09d70622444e67114259Eb7e8;
    uint256 public ID = 3;
    GM frensGM;
    ...
    function unWrapAmount(address fren) public view returns (uint256) {
        ...
        uint256 GMERC721 = IFREN(GM_ERC721).balanceOf(fren);
        uint256 max_unwrapAmount = (wGMBalance * gmAmount * multiplier) / divisor;
        uint256 timelapsed;

        if (GMERC721 > 0) {
            uint256 preTime = block.timestamp - unWrapTimeStamp[fren];
            timelapsed = (preTime * GMERC721 * gm_mul) / gm_div;
        } else {
