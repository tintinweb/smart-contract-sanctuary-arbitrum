...
import "./interfaces/Oracle/IAggregatorV3Interface.sol";
...
contract WaterLendingHandler is OwnableUpgradeable, PausableUpgradeable {
    ...
    function previewBorrowedAmounts(
        ...
        address _longToken
    ) public view returns (uint256 longTokenOutputAmount, uint256 shortTokenAmount) {
        ...
        shortTokenAmount = poolProportion - _amountIn;
    ...
    function borrow(
        ...
        uint256 _leverageSize,
        address _longToken
    ) external onlyVodkavault returns (uint256 longTokenOutputAmount, uint256 shortTokenAmount) {
        require(_amountIn > 0, "Amount must be greater than 0");
        // Get GMX pool addresses
        IVodkaV2.GMXPoolAddresses memory gmp = IVodkaV2(msg.sender).gmxPoolAddresses(_longToken);

        (longTokenOutputAmount, shortTokenAmount) = previewBorrowedAmounts(_amountIn, _leverageSize, _longToken);
        ...
        bool usdcBorrowed = IWater(gmp.shortTokenVault).lend(shortTokenAmount, msg.sender);
        // require both borrow to be successful
        require(ethBorrowed && usdcBorrowed, "Borrow failed");
        return (longTokenOutputAmount, shortTokenAmount);
    }
}