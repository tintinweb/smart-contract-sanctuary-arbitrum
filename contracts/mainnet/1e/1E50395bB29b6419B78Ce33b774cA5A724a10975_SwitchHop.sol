...
contract SwitchHop is Switch {
        ...
        nonReentrant
    {
        ...
        if (block.chainid == 1) {
        ...
        } else if (transferArgs.fromToken == usdcToken) {
            ...
            IHopBridge.CCTPSwapParams memory swapParams = IHopBridge.CCTPSwapParams({
                path: abi.encodePacked(transferArgs.fromToken, nativeWrap),
        ...
        nonReentrant
    {
        ...
        if (!isNativeFromToken) {
            // Give hop bridge approval
            IERC20(swapArgs.bridgeToken).universalApprove(swapArgs.router, returnAmount);
        }
        ...
        } else if (swapArgs.fromToken == usdcToken) {
            require(amountAfterFee >= swapArgs.bonderFee, "Bonder fee cannot exceed amount");
            IHopBridge.CCTPSwapParams memory swapParams = IHopBridge.CCTPSwapParams({
                path: abi.encodePacked(swapArgs.fromToken, nativeWrap),
