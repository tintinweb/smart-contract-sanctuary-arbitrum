...
contract MultiClaiming {
    HeroglyphAttestation public heroglyphAttestation;

    constructor(address _attestation) {
        heroglyphAttestation = HeroglyphAttestation(_attestation);
    }

    function multiClaim(uint32[] calldata _validatorIds) external {
        for (uint256 i = 0; i < 70; ++i) {
            try heroglyphAttestation.createAttestationRequest(_validatorIds) { }
            catch (bytes memory) {
                return;
            }
        }
