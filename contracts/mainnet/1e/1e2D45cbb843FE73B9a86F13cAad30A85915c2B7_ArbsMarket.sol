...
interface IArbsSwapNFT is IERC721, IERC721Metadata, IERC721Enumerable {
  // getter

  function arbsNames(uint256 tokenId) external view returns (string calldata);
  ...
  function arbsswapNFTToCategory(uint256 tokenId) external view returns (uint256);

  function categoryToArbsSwapNFTList(uint256 categoryId) external view returns (uint256[] memory);
  ...
  function getArbsNameOfTokenId(uint256 tokenId) external view returns (string memory);
...
contract ArbsMarket is ERC721HolderUpgradeable, OwnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable {
  using SafeMathUpgradeable for uint256;
  ...
  struct ArbsSwapNFTMetadataParam {
    address nftAddress;
  ...
  // arbs original nft related
  mapping(address => mapping(uint256 => ArbsSwapNFTMetadata)) public arbsswapNFTMetadata;
  ...
  function initialize(
    ...
    require(_wNative != address(0), "ArbsMarket::initialize:: _wNative cannot be address(0)");
    require(_feeAddr != address(0), "ArbsMarket::initialize:: _feeAddr cannot be address(0)");
  ...
  modifier onlySupportedNFT(address _nft) {
    require(isNFTSupported[_nft], "ArbsMarket::onlySupportedNFT::unsupported nft");
    _;
  }

  /// @notice only GOVERNANCE ROLE (role that can setup NON sensitive parameters) can continue the execution
  modifier onlyGovernance() {
    require(hasRole(GOVERNANCE_ROLE, _msgSender()), "ArbsMarket::onlyGovernance::only GOVERNANCE role");
    _;
  }
  ...
  modifier withinBlockRange(address _nftAddress, uint256 _categoryId) {
    require(
      block.number >= arbsswapNFTMetadata[_nftAddress][_categoryId].startBlock &&
        block.number <= arbsswapNFTMetadata[_nftAddress][_categoryId].endBlock,
      "ArbsMarket::withinBlockRange:: invalid block number"
    );
    _;
  }
  ...
  modifier permit(bytes calldata _sig) {
    ...
    require(recoveredAddress == _msgSender(), "ArbsMarket::permit::INVALID_SIGNATURE");
    _;
  }

  /// @dev Require that the caller must be an EOA account to avoid flash loans.
  modifier onlyEOA() {
    require(msg.sender == tx.origin, "ArbsMarket::onlyEOA:: not eoa");
    _;
  }

  modifier onlyBiddingNFT(address _nftAddress, uint256 _categoryId) {
    require(
      arbsswapNFTMetadata[_nftAddress][_categoryId].isBidding,
      "ArbsMarket::onlyBiddingNFT::only bidding token can be used here"
    );
    _;
  }
  ...
  modifier onlyNonBiddingNFT(address _nftAddress, uint256 _categoryId) {
    require(
      !arbsswapNFTMetadata[_nftAddress][_categoryId].isBidding,
      "ArbsMarket::onlyNonBiddingNFT::only selling token can be used here"
    );
    _;
  }

  /// @dev set ARBS NFT metadata consisted of cap, startBlock, and endBlock
  function setArbsSwapNFTMetadata(ArbsSwapNFTMetadataParam[] calldata _params) external onlyGovernance {
    for (uint256 i = 0; i < _params.length; i++) {
      require(isNFTSupported[_params[i].nftAddress], "ArbsMarket::setArbsSwapNFTMetadata::unsupported nft");
      _setArbsSwapNFTMetadata(_params[i]);
    }
  }

  function _setArbsSwapNFTMetadata(ArbsSwapNFTMetadataParam memory _param) internal {
    require(
      _param.startBlock > block.number && _param.endBlock > _param.startBlock,
      "ArbsMarket::_setArbsSwapNFTMetadata::invalid start or end block"
    );
    ArbsSwapNFTMetadata storage metadata = arbsswapNFTMetadata[_param.nftAddress][_param.nftCategoryId];
  ...
  function buyBatchNFT(
    ...
    ArbsSwapNFTMetadata memory metadata = arbsswapNFTMetadata[_nftAddress][_categoryId];
    ...
    require(
      ...
      "ArbsMarket::buyBatchNFT:: invalid block number"
    );
  ...
  function _decreaseCap(
    ...
    require(
      arbsswapNFTMetadata[_nftAddress][_categoryId].cap >= _size,
      "ArbsMarket::_decreaseCap::maximum mint cap reached"
    );
    arbsswapNFTMetadata[_nftAddress][_categoryId].cap = arbsswapNFTMetadata[_nftAddress][_categoryId].cap.sub(_size);
  }
  ...
  function _buyNFTTo(
    ...
    IArbsSwapNFT(_nftAddress).mintBatch(_to, _categoryId, _size);
    emit Trade(
  ...
  /// @dev set a current price of a nftaddress with the folloarbs categoryId
  ...
  function _setCurrentPrice(
    ...
    require(address(_quoteToken) != address(0), "ArbsMarket::_setCurrentPrice::invalid quote token");
    arbsswapNFTMetadata[_nftAddress][_categoryId].price = _price;
    arbsswapNFTMetadata[_nftAddress][_categoryId].quoteBep20 = _quoteToken;
  ...
  function readyToStartAuction(
    ...
    arbsswapNFTMetadata[_nftAddress][_categoryId].isBidding = true;
    _readyToSellNFTTo(
      _nftAddress,
      _categoryId,
      _price,
  ...
  function _readyToSellNFTTo(
    ...
    require(
      arbsswapNFTMetadata[_nftAddress][_categoryId].startBlock == 0,
      "ArbsMarket::_readyToSellNFTTo::duplicated entry"
    );
    tokenCategorySellers[_nftAddress][_categoryId] = _to;
    _setArbsSwapNFTMetadata(
      ArbsSwapNFTMetadataParam({
        cap: _cap,
    ...
    onlyBiddingNFT(_nftAddress, _categoryId)
  {
    ...
    require(bidEntry.bidder == address(0), "ArbsMarket::cancelBiddingNFT::auction already has a bidder");
  ...
  function _cancelSellNFT(address _nftAddress, uint256 _categoryId) internal {
    ...
    delete arbsswapNFTMetadata[_nftAddress][_categoryId];
  }
  ...
  function setFeePercent(uint256 _feePercentBps) external onlyOwner {
    require(feePercentBps != _feePercentBps, "ArbsMarket::setFeePercent::Not need update");
    require(feePercentBps <= 1e4, "ArbsMarket::setFeePercent::percent exceed 100%");
    emit SetFeePercent(_msgSender(), feePercentBps, _feePercentBps);
    feePercentBps = _feePercentBps;
  }

  /// @notice use for only bidding token, this method is for bidding the folloarbs nft
  /// @param _nftAddress - nft address
  ...
  function _bidNFT(
    ...
    require(_seller != _to, "ArbsMarket::_bidNFT::Owner cannot bid");
    require(
      arbsswapNFTMetadata[_nftAddress][_categoryId].price < _price,
      "ArbsMarket::_bidNFT::price cannot be lower than or equal to the starting bid"
    );
    if (tokenBid[_nftAddress][_categoryId].bidder != address(0)) {
      require(
        tokenBid[_nftAddress][_categoryId].price < _price,
        "ArbsMarket::_bidNFT::price cannot be lower than or equal to the latest bid"
      );
    }
    ...
    if (prevBid.bidder != address(0)) {
      _safeUnwrap(arbsswapNFTMetadata[_nftAddress][_categoryId].quoteBep20, prevBid.bidder, prevBid.price);
    }
    _safeWrap(arbsswapNFTMetadata[_nftAddress][_categoryId].quoteBep20, _price);
  ...
  function _concludeAuction(address _nftAddress, uint256 _categoryId) internal {
    require(
      block.number >= arbsswapNFTMetadata[_nftAddress][_categoryId].endBlock,
      "ArbsMarket::_concludeAuction::Unable to conclude auction now, bad block number"
    );
    address _seller = tokenCategorySellers[_nftAddress][_categoryId];
    _decreaseCap(_nftAddress, _categoryId, 1);
    BidEntry memory bidEntry = tokenBid[_nftAddress][_categoryId];
    require(bidEntry.price != 0, "ArbsMarket::_concludeAuction::Bidder does not exist");
    uint256 price = bidEntry.price;
    uint256 feeAmount = price.mul(feePercentBps).div(1e4);
    _delBidByCompositeId(_nftAddress, _categoryId);
    if (feeAmount != 0) {
      arbsswapNFTMetadata[_nftAddress][_categoryId].quoteBep20.safeTransfer(feeAddr, feeAmount);
    }
    arbsswapNFTMetadata[_nftAddress][_categoryId].quoteBep20.safeTransfer(_seller, price.sub(feeAmount));
    IArbsSwapNFT(_nftAddress).mint(bidEntry.bidder, _categoryId);
    emit Trade(
      tokenCategorySellers[_nftAddress][_categoryId],
      bidEntry.bidder,
      _nftAddress,
  ...
  function _safeWrap(IERC20Upgradeable _quoteBep20, uint256 _amount) internal {
    if (msg.value != 0) {
      require(address(_quoteBep20) == wNative, "arbsMarket::_safeWrap:: baseToken is not wNative");
      require(_amount == msg.value, "arbsMarket::_safeWrap:: value != msg.value");
  ...
  function _safeUnwrap(
    ...
    if (address(_quoteBep20) == wNative) {
      _quoteBep20.safeTransfer(address(feeAddr), _amount);
  ...
  /// @notice get all bidding entries of the folloarbs nft
