    ...
    AccessControl
{
    ...
    function borrow(
        ...
        require(
            IQualifier(qualifierAddress).isAllowListed(msg.sender),
            "AssetManager: not on allow list"
        );
        ...
        // check if the token is acceptable type
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
                IQualifier(qualifierAddress).isAcceptableNFT(_tokenIds[i]),
                "AssetManager: incorrect token type"
            );
        }
...
interface IQualifier {
    ...
    function isAllowListed(address _userAddress) external view returns (bool);

    function addToAllowList(address _userAddress) external;

    function removeFromAllowList(address _userAddress) external;
