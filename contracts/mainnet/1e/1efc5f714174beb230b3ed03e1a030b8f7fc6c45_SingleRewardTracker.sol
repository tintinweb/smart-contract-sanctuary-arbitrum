...
contract SingleRewardTracker is ISingleRewardTracker, UpgradeableOperableKeepable {
    ...
    function initializeSingleTracker(
        ...
        if (_tokenOption) {
            ...
            manager_.token0().approve(_rewardsSwapper, type(uint256).max);
        } else {
            rewardToken = manager_.token0();
            manager_.token1().approve(_rewardsSwapper, type(uint256).max);
        }
        ...
        if (length > 0) {
            for (uint256 i; i < length;) {
                if (_swap[i].tokenOut == address(rewardToken)) {
                    IERC20(_swap[i].tokenIn).approve(address(incentivesSwapper), _swap[i].amountIn);
                    amount = amount.add(
    ...
    function setManager(address _manager) external onlyGovernor {
        ...
        manager.token0().approve(_swapper, 0);
        manager.token1().approve(_swapper, 0);
        IManager manager_ = IManager(_manager);
        manager = manager_;
        if (tokenOption) {
            rewardToken = manager_.token1();
            manager_.token0().approve(_swapper, type(uint256).max);
        } else {
            rewardToken = manager_.token0();
            manager_.token1().approve(_swapper, type(uint256).max);
        }
    }
        ...
        onlyGovernor
    {
        ...
        manager_.token0().approve(swaper_, 0);
        manager_.token1().approve(swaper_, 0);
    ...
    function _withdraw(address _account, uint256 _amount) private {
        ...
        manager.transfer(msg.sender, _amount);
