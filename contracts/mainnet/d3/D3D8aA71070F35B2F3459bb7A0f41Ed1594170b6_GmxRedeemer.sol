...
contract VaultParentInternal is VaultOwnershipInternal, VaultBaseInternal {
    ...
    function _setHasNotBridged(bool hasNotBridged) internal {
        VaultParentStorage.Layout storage l = VaultParentStorage.layout();

        l.hasNotBridged = hasNotBridged;
    }
        ...
        returns (uint minValue, uint maxValue)
    {
        ...
        // If the manager has never bridged then there is no need to check the child chains
        if (_hasNotBridged()) {
            return (minVaultValue, maxVaultValue);
        }
    ...
    function _hasNotBridged() internal view returns (bool) {
        VaultParentStorage.Layout storage l = VaultParentStorage.layout();

        return l.hasNotBridged;
    }
...
contract VaultParentInvestor is VaultParentInternal {
    ...
    function hasNotBridged() external view returns (bool) {
        return _hasNotBridged();
    }
    ...
    function _requiresSyncForWithdraw(
        ...
        if (_hasNotBridged() || !_requiresUnitPrice(tokenId)) {
            return false;
        }
        return true;
    }
...
library VaultParentStorage {
    ...
    struct Layout {
        ...
        // We need this to default to false;
        bool hasNotBridged;
    }
