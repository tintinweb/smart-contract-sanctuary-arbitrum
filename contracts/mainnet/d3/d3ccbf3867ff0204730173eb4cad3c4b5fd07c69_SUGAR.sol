...
contract SUGAR is ERC20Burnable, Ownable {
  using SafeMath for uint256;
  ...
  ) public ERC20("SugarSwap", "SUGAR") {
    require(_initialSupply <= _maxSupply, "SUGAR: The _initialSupply should not exceed the _maxSupply");
  ...
  modifier onlyDev() {
    require(devAddress == _msgSender() || owner() == _msgSender(), "SUGAR: You don't have the permission!");
    _;
  }

  modifier hasMinterRole() {
    require(isMinter[_msgSender()], "SUGAR: You don't have the permission!");
    _;
  }
  ...
  function updateUniswapV2Router(address newAddress) public onlyDev {
    require(newAddress != address(uniswapV2Router), "SUGAR: The router already has that address");
  ...
  function mint(address _user, uint256 _amount) external hasMinterRole {
    ...
    require(_totalSupply.add(_amount) <= MAX_SUPPLY, "SUGAR: No more minting allowed!");
  ...
  function updateFee(uint256 feeType, uint256 fee) external onlyDev {
    require(fee <= 900, "SUGAR: The tax Fee cannot exceed 9%");
  ...
  function recoverToken(address _token) external onlyDev {
    ...
    require(tokenBalance > 0, "SUGAR: The contract doen't have tokens to be recovered!");
