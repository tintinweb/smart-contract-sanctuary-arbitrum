...
// File: contracts/token/AIsifloki.sol


pragma solidity =0.8.19;

contract Aishki is ERC20, Ownable {
    ...
    constructor(
        ...
        uint256 _totalSupply = 100000000000000000 * 10 ** 6;
    ...
    function swapBack() internal isSwapping {
        ...
        uint256 amountOfAishkiBurn = (taxAmount * burnFee) / (totalFee);
        uint256 amountOfAishkiLp = (taxAmount * liquidityFee) / (totalFee);
        uint256 amountOfAishkiForHolders = (taxAmount * holdersFee) / (totalFee);
        uint256 amountOfAishkiForStaking = (taxAmount * stakingFee) / (totalFee);

        taxAmount -= amountOfAishkiBurn;
        taxAmount -= amountOfAishkiLp;
        taxAmount -= amountOfAishkiForHolders;
        taxAmount -= amountOfAishkiForStaking;
        ...
        _transfer(address(this), DEAD, amountOfAishkiBurn);
        _transfer(address(this), holdersWallet, amountOfAishkiForHolders);
        _transfer(address(this), stakingRewardWallet, amountOfAishkiForStaking);
        ...
        emit SwapBack(amountOfAishkiBurn, amountOfBackTokenMkt, amountOfAishkiLp, amountOfAishkiForStaking, amountOfAishkiForHolders, amountOfBackTokenDev, block.timestamp);
    }
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'AISHKI: ETH_TRANSFER_FAILED');
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "AISHKI: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "AISHKI: index out of bounds");
