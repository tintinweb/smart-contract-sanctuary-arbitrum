...
contract CoinBook is ICoinBook, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        require(_multiSig != address(0), "Can not use address(0)");
        require(_weth != address(0), "Can not use address(0)");
        require(_priceFeed != address(0), "Can not use address(0)");
        require(_taxWallet != address(0), "Can not use address(0)");
        require(_tax <= 200, "Tax must be less than or equal to 2%");
        require(_fees <= 500000000, "Fees must be less than $5.00");
        __Ownable_init();
        ...
        restrictedTokens[IERC20(address(0))] = true;
        restrictedUsers[address(0)] = true;
    ...
    receive() external override payable {
        require(msg.sender == wETH, "Only wETH contract allowed to send ETH");
    ...
    function createOrderERC20(
        ...
        require(_buyTokens.length == _buyAmounts.length, "Buy token and amounts length mismatch");
    ...
    function editOrderPricesAll(
        ...
        for(uint i = 0; i < lt; i++) {
            ...
            require(_buyAmounts[i] > 0, "Can not set buy amount to 0");
        ...
        orderId[_id].startTime = uint32(block.timestamp) + 60;
    }
    ...
    function editOrderPriceSingle(
        ...
        require(_buyAmount > 0, "Can not set buy amount to 0");
    ...
    function claimRefundOnExpire(uint256 _id) external override nonReentrant {
        ...
        require(lister != address(0), "_id provided for non-existent listing");
    ...
    function executeOrderERC20(
        ...
        require(_orderStatus(_id) == 1, "Order has settled or canceled");
    ...
    function setPaused(bool _flag) external override onlyOwner {
        require(isPaused != _flag, "Contract paused status alread set to _flag");
        isPaused = _flag;
        if(_flag) {
    ...
    function setUpdater(address _updater, bool _flag) external override onlyOwner {
        require(_updater != address(0), "Can not use address(0)");
        require(!restrictedUsers[_updater], "User is restricted");
        updaters[_updater] = _flag;
        emit UpdaterSet(_updater, _flag);
    }

    /**
     * @notice Set the LibraryBuilder Address.
     * @dev Only callable by owner
     * @param _library Address of the Library Builder
     */
    function setLibraryBuilder(address _library) external override onlyOwner {
        require(_library != address(0), "Can not use address(0)");
        libraryBuilder = ILibraryBuilder(_library);
    }
    ...
    function setRestrictedToken(IERC20 token, bool flag) external override onlyUpdaters {
        require(address(token) != address(0), "Can not use address(0)");
        require(address(token) != wETH, "Can not restrict wETH");
    ...
    function setRestrictedUser(address user, bool flag) external override onlyUpdaters {
        require(user != address(0), "Can not use address(0)");
    ...
    function updatePriceFeed(address newPriceFeed) external override onlyOwner {
        require(newPriceFeed != address(0), "Can not use address(0)");
    ...
    function updateListingFee(uint256 newFee) external override onlyOwner {
        require(newFee <= 500000000, "Fees must be less than $5.00");
    ...
    function updateMaxPriceAge(uint256 newMaxAge) external override onlyOwner {
        require(newMaxAge <= 1800 && newMaxAge >= 60, "Age must be between 1 and 30 minutes");
    ...
    function updateTax(address payable _taxWallet, uint16 _tax) external override onlyOwner {
        require(_taxWallet != address(0), "Can not use address(0)");
        require(_tax <= 400, "New tax set too high");
        taxWallet = _taxWallet;
        tax = _tax;
        emit TaxUpdated(_tax, _taxWallet);
    }
    ...
    function getCurrentListingFee() external override view returns (uint256 listingFeeETH, uint80 round) {
        ...
        round = r;
        listingFeeETH = ((listingFeeUSD * 10**18) / toUint256(ethPrice));
    }
    ...
    function getCurrentCancelFee() public override view returns (uint256 cancelFeeETH, uint80 round) {
        (uint80 r,int256 ethPrice,,,) = priceFeed.latestRoundData();
        round = r;
        cancelFeeETH = ((cancelFeeUSD * 10**18) / toUint256(ethPrice));
    }
    ...
    function _orderStatus(uint256 _id) internal view returns (uint8) {
        require(orderId[_id].lister != address(0), "Status check for non-existent order");
        ...
        if (block.timestamp > orderId[_id].endTime) {
    ...
    function getListingFee(uint80 r) internal view returns (uint256 listingFeeETH) {
        ...
        return ((listingFeeUSD * 10**18) / toUint256(ethPrice));
    }

    function getCancelFee(uint80 r) internal view returns (uint256 cancelFeeETH) {
        (,int256 ethPrice,,uint256 roundTime,) = priceFeed.getRoundData(r);
        require(block.timestamp - roundTime <= maxPriceAge, "Price too old");
        return ((cancelFeeUSD * 10**18) / toUint256(ethPrice));
    }
    ...
    function _fulfillFullOrderERC20(address _taker, uint256 _id, IERC20 _token, uint256 _amount) internal {
        ...
        (bool sellTokenHasLibrary, address sellTokenLibrary, bool buyTokenHasLibrary, address buyTokenLibrary) =
            libraryBuilder.getLibraryPool(address(sellToken), address(_token));
        ...
        if (_token == IERC20(wETH)) {
            ...
            if (buyTokenHasLibrary) {
                _token.safeTransferFrom(_taker, buyTokenLibrary, taxAmount1);
        ...
        if (sellToken == IERC20(wETH)) {
            ...
            if (sellTokenHasLibrary) {
                sellToken.safeTransfer(sellTokenLibrary, taxAmount2);
    ...
    function _fulfillPartialOrderERC20(
        ...
        uint256 _partialSellAmount = ((_fullSellAmount * (_amount * 10**18)) / (_fullBuyAmount * 10**18));
        ...
        for(uint i = 0; i < length; i++) {
            ...
            n = (b - (b * ((_amount * 10**36) / _fullBuyAmount) / 10**36));
    ...
    function _fulfillFullOrderEth(address _taker, uint256 _id, IERC20 _token, uint256 _amount) internal {
        ...
        (bool sellTokenHasLibrary, address sellTokenLibrary) = libraryBuilder.getLibraryPool(address(sellToken));
...
interface ICoinBook {
    ...
    event UpdaterSet(address updater, bool isActive);

    event PriceFeedUpdated(address newPriceFeed);

    event MaxPriceAgeUpdated(uint256 newMaxAge);

    event TaxUpdated(uint16 newTax, address newTaxWallet);
    ...
    function setLibraryBuilder(address _library) external;
