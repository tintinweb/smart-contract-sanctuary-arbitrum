...
contract IlkRegistry {
    ...
    struct Ilk {
        ...
        uint96  class;   // Classification code (1 - clip, 2+ - other)
    ...
    constructor(address vat_, address dog_, address spot_) public {
    ...
    function add(address adapter) external {
        ...
        require(ilks.length < type(uint96).max, "IlkRegistry/too-many-ilks");
        ilks.push(_ilk);
    ...
    function remove(bytes32 ilk) external {
        ...
        require(_class == 1, "IlkRegistry/invalid-class");
    ...
    function file(bytes32 ilk, bytes32 what, uint256 data) external auth {
        if      (what == "class") { require(data <= type(uint96).max && data != 0); ilkData[ilk].class = uint96(data); }
        else if (what == "dec")   { require(data <= type(uint8).max);  ilkData[ilk].dec   = uint8(data); }
    ...
    // Public function to update an ilk's pip if the ilk has been updated.
    ...
    function put(
        ...
        external auth {
            ...
            require(_class != 0 && _class <= type(uint96).max, "IlkRegistry/invalid-class");
            require(_dec <= type(uint8).max, "IlkRegistry/invalid-dec");
            uint96 _pos;
