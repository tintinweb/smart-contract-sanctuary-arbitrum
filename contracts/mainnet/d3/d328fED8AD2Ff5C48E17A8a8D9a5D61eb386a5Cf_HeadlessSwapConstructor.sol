...
import "./implementations/HeadlessBridgeSwapImplementation.sol";
import "./implementations/HeadlessSwapImplementation.sol";
...
contract HeadlessSwapConstructor is HeadlessSwapBase {
    ...
    // chainId => DestRouter
    mapping(uint256 => DestRouter) public destRouters;
    // chainId => Bridge Depositors
    mapping(uint256 => address) public bridgeDepositors;
    ...
    modifier SupportedChainId(uint256 chainId) {
        if (destRouters[chainId].router == address(0)) {
            revert NotSupportedChainId(chainId);
        }
        _;
    }

    function addNewDestRouter(
        DestRouter calldata _destRouter,
        uint256 _cId
    ) external onlyOwner {
        destRouters[_cId] = _destRouter;
    }

    function addBridgeDepositors(uint256 _cId, address depositor) public onlyOwner {
        bridgeDepositors[_cId] = depositor;
    }
    ...
    function createShortcut(
        ...
        PoolType pType,
        uint256 _cId
    ) public SupportedChainId(_cId) {
        address instance;
        if (cId == _cId) {
            instance = payable(Clones.clone(headlessSwapImplementaion));
            HeadlessSwapImplementation(payable(instance)).initialize(
                pType,
                pool,
                destRouters[_cId].router
            );
            HeadlessSwapImplementation(payable(instance)).transferOwnership(
                this.owner()
            );
        } else {
            instance = payable(
                Clones.clone(0x155C0B0123f92f615d61Fd573EA37Caa84f741f6)
            );
            HeadlessBridgeSwapImplementation(payable(instance)).initialize(
                pType,
                pool,
                0xC22FB7c71A52506dC74Fe865dd2Bbe050DD208F5,
                0xD8d4cf2F09A721e951AD061cC7FC27e1a1A22bbC,
                _cId
            );
            HeadlessBridgeSwapImplementation(payable(instance))
...
import "../structs/Structs.sol";
import "../interfaces/IOnthisDepositor.sol";
import "../HeadlessSwapBase.sol";
// https://onthis.xyz
/*
 .d88b.  d8b   db d888888b db   db d888888b .d8888. 
...
contract HeadlessBridgeSwapImplementation is HeadlessSwapBase {
    ...
    receive() external payable {
        ...
        IOnthisDepositor(depositor).sendToAcross{value: msg.value - fees}(
            pType,
            pool,
            msg.sender,
            destRouter,
...
contract HeadlessSwapImplementation is HeadlessSwapBase {
    ...
    receive() external payable {
        ...
        IHeadlessSwapHandler(swapRouter).handleSimpleSwap{
...
interface IHeadlessSwapHandler {
