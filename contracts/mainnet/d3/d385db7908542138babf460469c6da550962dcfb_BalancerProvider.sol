...
interface IBalancerFlashLoanRecipient {
    function receiveFlashLoan(
        Token18[] memory tokens,
        UFixed18[] memory amounts,
        UFixed18[] memory feeAmounts,
...
contract BalancerProvider is IERC3156FlashLender, IBalancerFlashLoanRecipient {
    ...
    function flashFee(Token18, UFixed18) external pure returns (UFixed18) {
    ...
    function flashLoan(
        ...
        Token18[] memory tokens = new Token18[](1);
        tokens[0] = token;

        UFixed18[] memory amounts = new UFixed18[](1);
        amounts[0] = amount;

        vault.flashLoan(IBalancerFlashLoanRecipient(this), tokens, amounts, abi.encode(receiver, data));
    ...
    function receiveFlashLoan(
        ...
        (IERC3156FlashBorrower receiver, bytes memory data) = abi.decode(userData, (IERC3156FlashBorrower, bytes));

        tokens[0].push(address(receiver), amounts[0]);
        IERC3156FlashBorrower(receiver).onFlashLoan(address(receiver), tokens[0], amounts[0], UFixed18Lib.ZERO, data);
        tokens[0].pullTo(address(receiver), address(vault), amounts[0]);
    }
}
