...
contract RouterHelper is Ownable {
    ...
    function getPairInfo(fatory Fatory, address tokenA, address tokenB, address _account) public view returns (address pair_, tokenInfoItem[] memory tokenInfoList, pairItem2[] memory PairInfo) {
        ...
        if (pair_ != address(0)) {
            ...
            PairInfo[0] = pairItem2(address(Fatory),pair_, token0, token1, reserve0, reserve1, IERC20(token0).decimals(), IERC20(token1).decimals(), pair(pair_).decimals(), pair(pair_).totalSupply(), pair(pair_).balanceOf(_account));
        }
    }
