...
contract Migration_20240517_0001 is Migration { // solhint-disable-line contract-name-camelcase
  ...
  function migrate() internal override onlyAdmin {
    MigrationLib.updateMarketplaceAdapter(_registry, Constants.WBTC, SWAPROUTER_ADAPTER);
    MigrationLib.updateMarketplaceAdapter(_registry, Constants.WETH, SWAPROUTER_ADAPTER);
    MigrationLib.updateMarketplaceAdapter(_registry, Constants.ARB, SWAPROUTER_ADAPTER);

    MigrationLib.updateMaxSupply(_registry, Constants.WSTETH, 100 * 10 ** 18);
  }
}
