...
import './OasisSwapERC20.sol';
import './libraries/Math.sol';
import './libraries/UQ112x112.sol';
import './interfaces/IERC20.sol';
import './interfaces/IOasisSwapFactory.sol';
import './interfaces/IOasisSwapCallee.sol';
...
contract OasisSwapPair is OasisSwapERC20 {
    ...
    // fee customizability
    uint64 public fee;
    uint64 public oasisFeeProportion;
    ...
    modifier lock() {
        require(unlocked == 1, 'OasisSwap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'OasisSwap: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1, uint64 _fee, uint64 _oasisFeeProportion) external {
        require(msg.sender == factory, 'OasisSwap: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;

        setFee(_fee, _oasisFeeProportion);
    }
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IOasisSwapFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'OasisSwap: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'OasisSwap: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function _swap(uint amount0Out, uint amount1Out, address to, uint64 feeRebate, bytes calldata data) internal returns (uint, uint, uint, uint){
        ...
        require(amount0Out < _reserve[0] && amount1Out < _reserve[1], 'OasisSwap: INSUFFICIENT_LIQUIDITY');

        uint balance0;
        uint balance1;
        { // avoids stack too deep errors
        require(to != token0 && to != token1, 'OasisSwap: INVALID_TO');
        ...
        if (data.length > 0) IOasisSwapCallee(to).uniswapV2Call(msg.sender, amount0Out, amount1Out, data);
        ...
        oasisFee0 = feeTaken0.div(FEE_DIVISOR).mul(_oasisFeeProportion).div(FEE_DIVISOR);
        oasisFee1 = feeTaken1.div(FEE_DIVISOR).mul(_oasisFeeProportion).div(FEE_DIVISOR);
        }
        balance0 = balance0.sub(oasisFee0);
        balance1 = balance1.sub(oasisFee1);
        feeCache0 = uint(feeCache0).add(oasisFee0);
        feeCache1 = uint(feeCache1).add(oasisFee1);
        }
    ...
    function setFee(uint64 _fee, uint64 _oasisFeeProportion) public {
        require(msg.sender == factory || IOasisSwapFactory(factory).isFeeManager(msg.sender), 'OasisSwap: FORBIDDEN');
...
contract OasisSwapERC20 {
    using SafeMathUniswap for uint;

    string public constant name = 'OasisSwap LP Token';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'OasisSwap: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'OasisSwap: INVALID_SIGNATURE');
