...
/// @title Callback for IMallowV3PoolActions#flash
/// @notice Any contract that calls IMallowV3PoolActions#flash must implement this interface
interface IMallowV3FlashCallback {
    /// @notice Called to `msg.sender` after transferring to the recipient from IMallowV3Pool#flash.
    /// @dev In the implementation you must repay the pool the tokens sent by flash plus the computed fee amounts.
    /// The caller of this method must be checked to be a MallowV3Pool deployed by the canonical MallowV3Factory.
    /// @param fee0 The fee amount in token0 due to the pool by the end of the flash
    /// @param fee1 The fee amount in token1 due to the pool by the end of the flash
    /// @param data Any data passed through by the caller via the IMallowV3PoolActions#flash call
    function mallowV3FlashCallback(uint256 fee0, uint256 fee1, bytes calldata data) external;
}

// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;

/// @title Callback for IMallowV3PoolActions#mint
/// @notice Any contract that calls IMallowV3PoolActions#mint must implement this interface
interface IMallowV3MintCallback {
    /// @notice Called to `msg.sender` after minting liquidity to a position from IMallowV3Pool#mint.
    /// @dev In the implementation you must pay the pool tokens owed for the minted liquidity.
    /// The caller of this method must be checked to be a MallowV3Pool deployed by the canonical MallowV3Factory.
    /// @param amount0Owed The amount of token0 due to the pool for the minted liquidity
    /// @param amount1Owed The amount of token1 due to the pool for the minted liquidity
    /// @param data Any data passed through by the caller via the IMallowV3PoolActions#mint call
    function mallowV3MintCallback(uint256 amount0Owed, uint256 amount1Owed, bytes calldata data) external;
}

// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;

/// @title Callback for IMallowV3PoolActions#swap
/// @notice Any contract that calls IMallowV3PoolActions#swap must implement this interface
interface IMallowV3SwapCallback {
    /// @notice Called to `msg.sender` after executing a swap via IMallowV3Pool#swap.
    ...
    /// @param data Any data passed through by the caller via the IMallowV3PoolActions#swap call
    function mallowV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external;
}

// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;

/// @title Minimal ERC20 interface for MallowSwap
/// @notice Contains a subset of the full ERC20 interface that is used in MallowSwap V3
...
/// @title The interface for the MallowSwap V3 Factory
/// @notice The MallowSwap V3 Factory facilitates creation of MallowSwap V3 pools and control over the protocol fees
interface IMallowV3Factory {
...
/// @title An interface for a contract that is capable of deploying MallowSwap V3 Pools
/// @notice A contract that constructs a pool must implement this to pass arguments to the pool
/// @dev This is used to avoid having constructor arguments in the pool contract, which results in the init code hash
/// of the pool being constant allowing the CREATE2 address of the pool to be cheaply computed on-chain
interface IMallowV3PoolDeployer {
...
interface IMallowV3PoolActions {
    ...
    /// @dev The caller of this method receives a callback in the form of IMallowV3MintCallback#mallowV3MintCallback
    ...
    /// @dev The caller of this method receives a callback in the form of IMallowV3SwapCallback#mallowV3SwapCallback
    ...
    /// @dev The caller of this method receives a callback in the form of IMallowV3FlashCallback#mallowV3FlashCallback
...
interface IMallowV3PoolDerivedState {
...
interface IMallowV3PoolImmutables {
    /// @notice The contract that deployed the pool, which must adhere to the IMallowV3Factory interface
...
interface IMallowV3PoolOwnerActions {
...
import './interfaces/IMallowV3Pool.sol';
...
import './interfaces/IMallowV3PoolDeployer.sol';
import './interfaces/IMallowV3Factory.sol';
import './interfaces/IERC20Minimal.sol';
import './interfaces/callback/IMallowV3MintCallback.sol';
import './interfaces/callback/IMallowV3SwapCallback.sol';
import './interfaces/callback/IMallowV3FlashCallback.sol';

import '@mallowswap/v3-lm-pool/contracts/interfaces/IMallowV3LmPool.sol';

contract MallowV3Pool is IMallowV3Pool {
    using LowGasSafeMath for uint256;
    using LowGasSafeMath for int256;
    using SafeCast for uint256;
    using SafeCast for int256;
    ...
    /// @inheritdoc IMallowV3PoolImmutables
    ...
    modifier onlyFactoryOrFactoryOwner() {
        require(msg.sender == factory || msg.sender == IMallowV3Factory(factory).owner());
        _;
    }

    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = IMallowV3PoolDeployer(msg.sender).parameters();
    ...
    /// @inheritdoc IMallowV3PoolDerivedState
    function snapshotCumulativesInside(
        int24 tickLower,
        int24 tickUpper
    )
    ...
    /// @inheritdoc IMallowV3PoolActions
    ...
    function mint(
        ...
        IMallowV3MintCallback(msg.sender).mallowV3MintCallback(amount0, amount1, data);
    ...
    function swap(
        ...
        if (zeroForOne) {
            ...
            IMallowV3SwapCallback(msg.sender).mallowV3SwapCallback(amount0, amount1, data);
    ...
    function flash(address recipient, uint256 amount0, uint256 amount1, bytes calldata data) external override lock {
        ...
        IMallowV3FlashCallback(msg.sender).mallowV3FlashCallback(fee0, fee1, data);
    ...
    /// @inheritdoc IMallowV3PoolOwnerActions
    ...
    function setLmPool(address _lmPool) external override onlyFactoryOrFactoryOwner {
        lmPool = IMallowV3LmPool(_lmPool);
...
import './MallowV3Pool.sol';

contract MallowV3PoolDeployer is IMallowV3PoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    /// @inheritdoc IMallowV3PoolDeployer
    ...
    /// @param factory The contract address of the MallowSwap V3 factory
    /// @param token0 The first token of the pool by address sort order
    /// @param token1 The second token of the pool by address sort order
    /// @param fee The fee collected upon every swap in the pool, denominated in hundredths of a bip
    /// @param tickSpacing The spacing between usable ticks
    function deploy(
        ...
        pool = address(new MallowV3Pool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}