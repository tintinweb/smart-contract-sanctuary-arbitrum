...
contract LendBorrowLendStrategy is UpgradeableBase, IStrategy {
    ...
    function __Strategy_init(address _comptroller) public initializer {
    ...
    function destroyAll() external override onlyOwnerAndAdmin {
        ...
        // swap rewardsToken to StrategyToken
        {
            ...
            // Get Rewards amount
            uint256 amountRewardsToken = _getLogicBalance(
                _rewardsToken,
                _logic
            );

            // Swap if RewardsToken Price/Amount Kill Switch is false
            if (
                _rewardsPriceKillSwitch(
                    _strategyStatistics,
                    _rewardsToken,
                    amountRewardsToken
                ) ==
                false &&
                amountRewardsToken > 0
            ) {
                _multiSwap(
                    _logic,
                    amountRewardsToken,
        ...
        if (_supplyXToken != _strategyXToken) {
            ...
            if (totalSupply > 0) {
                ILendingLogic(_logic).redeem(
                    _strategyXToken,
                    _getLogicBalance(_strategyXToken, _logic)
                );
            }

            // Repay borrow & calcuate supplyX remainded
            uint256 supplyXAmountRemainded;
            {
                uint256 balanceStrategyToken = _getLogicBalance(
                    strategyToken,
                    _logic
                );
                if (balanceStrategyToken > borrowAmount) {
                    // If balance > borrowAmount, Swap StrategyToken -> SupplyToken
                    _multiSwap(
                        _logic,
                        balanceStrategyToken - borrowAmount,
                        swapStrategyTokenToSupplyTokenInfo
                    );
                } else if (balanceStrategyToken < borrowAmount) {
                    // Calculate supplyX Amount remained
                    uint256 mantissa = IXToken(_supplyXToken)
                        .exchangeRateStored();
                    (
                        ,
                        uint256 collateralFactorSupplyApplied
                    ) = _getCollateralFactor(_supplyXToken);

                    supplyXAmountRemainded =
                        ((((((borrowAmount - balanceStrategyToken) * BASE) /
                            collateralFactorSupplyApplied) * BASE) / mantissa) *
                            strategyPriceUSD) /
                        supplyPriceUSD;

                    // If balance < borrowAmount, repay only for balance
                    borrowAmount = balanceStrategyToken;
                }
            ...
            // SupplyXToken : Redeem except collateral
            ILendingLogic(_logic).redeem(
                _supplyXToken,
                _getLogicBalance(_supplyXToken, _logic) - supplyXAmountRemainded
            );
        }

        // Get tokenTaken from MultiLogic, current balance of supply
        uint256 amountStrategy = IMultiLogicProxy(multiLogicProxy)
            .getTokenTaken(_supplyToken, _logic);
        uint256 balanceSupplyToken = _getLogicBalance(_supplyToken, _logic);

        // If we have extra supply, swap SupplyToken to BLID
        uint256 amountBLID = 0;
        if (balanceSupplyToken > amountStrategy) {
            _multiSwap(
                _logic,
                balanceSupplyToken - amountStrategy,
            ...
            amountStrategy = balanceSupplyToken;
        }
    ...
    function claimRewards() public override onlyOwnerAndAdmin {
        ...
        // Get Rewards amount
        amountRewardsToken = _getLogicBalance(_rewardsToken, _logic);
        ...
        if (diffStrategy > 0 && !rewardsTokenKill) {
            ...
            // Repay StrategyToken
            uint256 repayAmount = _getLogicBalance(_strategyToken, _logic);
            if (repayAmount > uint256(diffStrategy))
                repayAmount = uint256(diffStrategy);

            // RepayBorrow
            ILendingLogic(_logic).repayBorrow(_strategyXToken, repayAmount);
        }
        ...
        if (amountRewardsToken > 0 && rewardsTokenKill == false) {
            ...
            require(
                (amountRewardsToken * minimumBLIDPerRewardToken) / BASE <=
                    _getLogicBalance(blid, _logic),
                "C5"
            );
        }

        // If we have Strategy Token, swap StrategyToken to BLID
        uint256 balanceStrategyToken = _getLogicBalance(_strategyToken, _logic);
        ...
        onlyMultiLogicProxy
    {
        ...
        if (
            ...
            uint256 amountRewardsToken = _getLogicBalance(rewardsToken, _logic);
        ...
        uint256 balance = _getLogicBalance(token, _logic);
    ...
    function destructCircles(
        ...
        for (uint256 i = 0; i < iterateCount; ) {
            ...
            // Get BorrowBalance, Total Supply
            {
                ...
                xTokenBalance = _getLogicBalance(xToken, _logic);
            ...
            uint256 repayAmount = _getLogicBalance(strategyToken, _logic);
    ...
    function _multiSwap(
        ...
        for (uint256 i = 0; i < swapInfo.swapRouters.length; ) {
            if (i > 0) {
                amount = _getLogicBalance(swapInfo.paths[i][0], _logic);
            }
    ...
    function _getLogicBalance(address token, address _logic)
        private
        view
        returns (uint256)
    {
        return
            token == ZERO_ADDRESS
                ...
                : IERC20MetadataUpgradeable(token).balanceOf(_logic);
    }
}
