...
contract NogemHyperErc20 is HypERC20, ReentrancyGuard {
    ...
    function setTokenPrice(uint256 _tokenPrice) external onlyOwner {
        weiTokenPrice = _tokenPrice;
    }

    function getTokenPrice() external view returns (uint256) {
        return weiTokenPrice;
    }
    ...
    function mint() external payable nonReentrant {
        uint256 tokenQuantity = (msg.value * 10 ** 18) / weiTokenPrice;

        _validate(tokenCounter + (tokenQuantity / 10 ** 18) <= maxMintId, ERROR_MINT_EXCEEDS_LIMIT);
        _validate(tokenQuantity > 0, ERROR_MINT_INVALID_FEE);

        tokenCounter += (tokenQuantity / 10 ** 18);

        feeEarnedAmount += msg.value;

        _mint(_msgSender(), tokenQuantity);
        emit ONFTMinted(_msgSender(), tokenCounter - (tokenQuantity / 10 ** 18), msg.value, address(0), 0);
    }
    ...
    function mint(address referrer) public payable nonReentrant {
        ...
        uint256 referrerShareBips = referrerBips == 0 ? referralEarningBips : referrerBips;
        uint256 referrerEarnings = (msg.value * referrerShareBips) / DENOMINATOR;
        uint256 feeEarnings = msg.value - referrerEarnings;

        referrersEarnedAmount[referrer] += referrerEarnings;
        referredTransactionsCount[referrer] += (tokenQuantity / 10 ** 18);
