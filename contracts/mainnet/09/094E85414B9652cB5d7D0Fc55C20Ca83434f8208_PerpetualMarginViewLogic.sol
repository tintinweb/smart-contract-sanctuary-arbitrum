    ...
    PerpetualRebalanceFunctions
{
    ...
    function calcMarginForTargetLeverage(
        ...
        int128 fC2Q = _getCollateralToQuoteConversionMultiplier(perpetual, false);

        int128 fS2Mark = _getPerpetualMarkPrice(perpetual, false);

        int128 b0 = _ignorePosBalance
            ? int128(0)
            : _getMarginBalance(perpetual, _traderAddr, false);

        if (_fTargetLeverage == 0) {
            // leverage is to be set to position leverage
            _fTargetLeverage = _fTraderPos.abs().mul(fS2Mark);
            _fTargetLeverage = _fTargetLeverage.div(fC2Q).div(b0);
    ...
    {
        PerpStorage.PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        return _getMarginBalance(perpetual, _traderAddr, msg.sender != address(this));
    }

    function _getMarginBalance(
        ...
        int128 fConversionB2C = _getBaseToCollateralConversionMultiplier(
            _perpetual,
            atMark,
            _useOracle
        );
        // quote to collateral currency conversion
        int128 fConversionC2Q = _getCollateralToQuoteConversionMultiplier(_perpetual, _useOracle);
        ...
        returns (int128)
    {
        ...
        return _getAvailableCash(perpetual, _traderAddr);
    }
     ...
     */
    function _getAvailableCash(PerpetualData storage _perpetual, address _traderAddr)
        internal
        view
        returns (int128)
    {
        ...
        returns (bool)
    {
        ...
        bool bUseOracle = msg.sender != address(this);
        int128 threshold = _getMaintenanceMargin(perpetual, _traderAddr, bUseOracle);
        return (_getMarginBalance(perpetual, _traderAddr, bUseOracle) >= threshold);
    }
    ...
    function _getAvailableMargin(
        ...
        if (marginAccounts[_perpetual.id][_traderAddr].fPositionBC != 0) {
            ...
            if (_isInitialMargin) {
                fInitialMarginCC = _getInitialMargin(_perpetual, _traderAddr, _bUseOracle);
            } else {
                fInitialMarginCC = _getMaintenanceMargin(_perpetual, _traderAddr, _bUseOracle);
            }
        }

        int128 fAvailableMargin = _getMarginBalance(
            _perpetual,
            _traderAddr,
            msg.sender != address(this)
        );
    ...
    {
        PerpStorage.PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        return (_getAvailableMargin(perpetual, _traderAddr, true, msg.sender != address(this)) >=
            0);
    }
    ...
    {
        PerpStorage.PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        return _getInitialMargin(perpetual, _traderAddr, msg.sender != address(this));
    }

    function _getInitialMargin(
        PerpStorage.PerpetualData storage _perpetual,
        address _traderAddr,
        bool _bUseOracle
    ) internal view returns (int128) {
    ...
    {
        PerpStorage.PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        return _getMaintenanceMargin(perpetual, _traderAddr, msg.sender != address(this));
    }
        ...
        returns (bool)
    {
        ...
        return _getMarginBalance(perpetual, _traderAddr, msg.sender != address(this)) >= 0;
    }
     ...
     */
    function _getTraderPosition(PerpStorage.PerpetualData storage _perpetual, address _traderAddr)
        internal
        view
        returns (int128)
    {
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[8] = this.getAvailableCash.selector;
