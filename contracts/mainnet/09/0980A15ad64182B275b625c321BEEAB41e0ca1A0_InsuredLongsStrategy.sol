    ...
    IInsuredLongsStrategy
{
    ...
    uint256 private constant STRATEGY_FEE_KEY = 1;
    ...
    /**
     * Max leverage allowed for insuring long positions.
     */
    ...
    function createExitStrategyOrder(
        ...
        if (
            position.state == ActionState.Increased ||
            position.state == ActionState.IncreasePending
        ) {
    ...
    function _validteNotWithinExerciseWindow(
        ...
        _validate(
            !IAtlanticPutsPool(_atlanticPool).isWithinBlackoutWindow(),
            34
        );
    }
