...
// File contracts/FlexIRAv2u6.sol
...
contract FlexIRAv2u6 is ERC20Upgradeable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, OwnableUpgradeable {
    ...
    function initializeV2u6(uint8 tokenDec_) public reinitializer(4) {
        tokenDec = tokenDec_;
        tokenDecCoef = 10 ** (18 - tokenDec_);
    }

    function set_tokenDec(uint8 value) public onlyOwner {
        tokenDec = value;
        tokenDecCoef = 10 ** (18 - value);
    }

    function set_token(IERC20 value) public onlyOwner {
        token = value;
    }
    ...
    function deposit(uint256 amountUsd) public {
        ...
        depositWithRef(amountUsd, address(0));
    }
    ...
    function depositWithRef(uint256 amountUsd, address referrer) public {
        ...
        if (userVersion[msg.sender] == 0) {
            require(amountUsd >= minUserFirstDeposit, "FlexIRA: first deposit less than required");
        ...
        require(amountUsd >= minUserDeposit, "FlexIRA: deposit is less than min user's deposit");
        require(amountUsd <= maxUserDeposit, "FlexIRA: deposit is greater than max user's deposit");
    ...
    function _processDeposit(uint256 amountUsd, bool ignoreMaxTotalSupply) internal {
        ...
        // Fees
        uint256 devFeeUsd = amountUsd * devFeePct100 / 100;
        uint256 depositFeeUsd = amountUsd * depositFeePct100 / 100;
        if (userVersion[msg.sender] > 1) {
            depositFeeUsd = amountUsd * depositFeeV2u1Pct100 / 100;
        }

        uint256 referrerFeeUsd = 0;
        uint256 lotteryFeeUsd = 0;
        if (ref != address(0)) {
            referrerFeeUsd = amountUsd * referralFeePct1000 / 1000;
            lotteryFeeUsd = amountUsd * lotteryFeePct1000 / 1000;

            _safeTransferTokens(address(lottery), lotteryFeeUsd);
            lottery.addTickets(ref, usdToE18(lotteryFeeUsd));

            uint256 referrerFeeE18 = usdToE18(referrerFeeUsd);
            referralBonus[ref] += referrerFeeE18;
            _mintWithLimit(ref, referrerFeeE18, true);
        }

        _safeTransferTokens(devWallet, devFeeUsd);
        _safeTransferTokens(depositFeeWallet, depositFeeUsd);

        uint256 netAmountUsd = amountUsd.sub(devFeeUsd).sub(depositFeeUsd).sub(referrerFeeUsd).sub(lotteryFeeUsd);

        _safeTransferTokens(assetFundWallet, netAmountUsd.add(referrerFeeUsd));

        _mintWithLimit(msg.sender, usdToE18(netAmountUsd), ignoreMaxTotalSupply);
    }
    ...
    function withdrawDeposit(uint256 amountE18) public {
        require(!paused, "FlexIRA: contract is paused");

        uint256 myBalance = balanceOf(msg.sender);
        require(myBalance > 0, "FlexIRA: balance is empty");
        require(myBalance >= amountE18, "FlexIRA: amount exceed balance");
        ...
        withdrawRequestsTotal += amountE18;
        withdrawRequestTotalByDay[todayMidnight] += amountE18;
        withdrawRequestCountByDay[todayMidnight]++;
        withdrawRequestsBalanceByUser[msg.sender] += amountE18;

        _burn(msg.sender, amountE18);
    }

    function fillWithdrawRequests(uint256 amountE18, uint256 maxWithdrawRequests) public {
        require(!paused, "FlexIRA: contract is paused");

        if (amountE18 > 0) {
            _safeTransferTokensFrom(msg.sender, address(this), e18ToUsd(amountE18));
            withdrawPoolBalance += amountE18;
        }
        ...
        while (true) {
            ...
            if (wr.amount <= withdrawPoolBalance) {
                ...
                _safeTransferTokens(wr.user, e18ToUsd(wr.amount));
    ...
    function withdrawDividendAndCompoundRest(uint256 withdrawAmountE18) public {
        ...
        require(withdrawAmountE18 <= _withdrawableDividendE18, "FlexIRA: withdrawAmount more than user's dividends");
        if (_withdrawableDividendE18 > 0) {
            ...
            if (withdrawAmountE18 > 0) {
                ...
                emit DividendWithdrawn2(msg.sender, _withdrawableDividendE18);
            }
            if (compoundAmountE18 > 0) {
                ...
                emit DividendCompounded2(msg.sender, _withdrawableDividendE18);
            }
    ...
    function runLottery() public onlyOwner {
        ...
        lottery.transfer(assetFundWallet, e18ToUsd(lotteryResult.amount));
        _mintWithLimit(lotteryResult.winner, lotteryResult.amount, true);
    }


    //////////////////////////////////////////// Migration
    function transferTokensFromContract(IERC20 token, address to, uint amountUsd) public onlyOwner {
    ...
    function restore_balances(address[] calldata user, uint256[] calldata values) public onlyOwner {
        for (uint i = 0; i < user.length; i++) {
            ...
            if (userBalance > value) {
                _burn(user[i], userBalance - value);
            }
            if (userBalance < value) {
