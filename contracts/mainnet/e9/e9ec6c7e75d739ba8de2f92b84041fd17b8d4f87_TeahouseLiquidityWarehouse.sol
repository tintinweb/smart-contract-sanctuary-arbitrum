...
contract TeahouseLiquidityWarehouse is LiquidityWarehouse {
    ...
    uint256 internal s_priceThreshold;
    /// @notice Addresses
    HelperAddresses internal s_addresses;
    ...
    constructor(
        ...
        if (uniswapRouter == address(0) || sequencerUptimeFeed == address(0)) revert InvalidZeroAddress();
        s_addresses.uniswapRouter = uniswapRouter;
        s_addresses.sequencerUptimeFeed = sequencerUptimeFeed;
        s_priceThreshold = priceThreshold;
        ...
        returns (bool)
    {
        return TeahouseHelperLibrary.withdrawAndSwap(
            TeahouseHelperLibrary.WithdrawAndSwapParams({
                ...
                priceThreshold: s_priceThreshold,
                withdrawTarget: withdrawTarget,
                swapData: swapData,
                baseAsset: address(s_terms.asset),
                uniswapRouter: s_addresses.uniswapRouter,
                sequencerUptimeFeed: s_addresses.sequencerUptimeFeed
            }),
            s_assetConfigs
        );
    }
    ...
    function _getDeployedAssetValue(address withdrawTarget) internal view override returns (uint256) {
        return TeahouseHelperLibrary.convertToTeahouseAssets(
            withdrawTarget, address(s_terms.asset), s_priceThreshold, s_addresses.sequencerUptimeFeed, s_assetConfigs
        );
    }

    /// @inheritdoc LiquidityWarehouse
    function _afterExecute(bytes memory data) internal override {
        TeahouseHelperLibrary.swap(
            data,
            address(s_terms.asset),
            s_withdrawTargets,
            s_addresses.uniswapRouter,
            s_addresses.sequencerUptimeFeed,
            s_priceThreshold,
            s_assetConfigs
        );
    }
    ...
    AccessControlDefaultAdminRules
{
    ...
    function depositBorrower(uint256 depositAmount) external whenNotPaused nonReentrant {
        ...
        uint256 shareAmt = _convertToShares(totalSupply(BORROWER_LP_TOKEN_ID), getBorrowerNetAssetValue(), depositAmount);

        if (shareAmt == 0) revert ZeroShareAmt();
        if (totalSupply(BORROWER_LP_TOKEN_ID) == 0 && !hasRole(PRIMARY_BORROWER_ROLE, msg.sender)) revert AccessForbidden();
...
library TeahouseHelperLibrary {
        ...
        for (uint256 i; i < withdrawTargets.length(); ++i) {
            ...
            ISwapRouter(uniswapRouter).exactInput(
                ISwapRouter.ExactInputParams({
                    ...
                    amountIn: IERC20(counterAsset).balanceOf(address(this)),
    ...
    function _validatePoolPrice(
        ...
        uint256 oraclePrice = _getOraclePrice(assetConfig, sequencerUptimeFeed);
        uint256 absPriceDif = oraclePrice < spotPrice ? spotPrice - oraclePrice : oraclePrice - spotPrice;

        uint256 percentPriceDif = absPriceDif * WadRayMath.WAD / oraclePrice;
        if (percentPriceDif > priceThreshold) {
            revert PriceDiscrepancy(oraclePrice, spotPrice, percentPriceDif);
        }
    }
    ...
    function _getSpotPrice(IUniswapV3Pool pool) private view returns (uint256) {
        ...
        console.log("SPOT", spotPrice / 10 ** IERC20Metadata(pool.token1()).decimals());
        ...
        returns (uint256)
    {
        ...
        console.log("ORACLE", scaledOraclePrice);
