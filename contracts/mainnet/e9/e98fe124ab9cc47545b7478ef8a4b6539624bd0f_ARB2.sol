...
interface ITrueDefiSwap {
    ...
    contract ARB2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ARB2DividendTracker public dividendTracker;
    ...
    constructor() ERC20("USDC Rain", "ARB2") {
        marketingWallet = payable(0xF31A6BAD18e9E3221d851051EF6d5CC0496bbd25);
        devWallet = payable(0x18eF65ac93c00baD89677aC89fb8DA9284661731);
        ...
        dividendTracker = new ARB2DividendTracker(
            payable(this),
            router,
...
contract ARB2DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        ARB2Contract = ARB2(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ARB2_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ARB2_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ARB2Contract);

            uint256 prevBalance = ARB2Contract.balanceOf(address(this));
            ...
            {
                uint256 received = ARB2Contract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ARB2Contract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
