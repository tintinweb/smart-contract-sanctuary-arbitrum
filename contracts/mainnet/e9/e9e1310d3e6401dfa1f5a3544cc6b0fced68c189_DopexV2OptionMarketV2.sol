...
pragma solidity <0.9.0 >=0.4.0 >=0.5.0 >=0.8.0 ^0.8.0 ^0.8.1 ^0.8.13;
...
// src/interfaces/ITokenURIFetcher.sol

interface ITokenURIFetcher {
    function onFetchTokenURIData(uint256 id) external view returns (string memory);
}

// src/libraries/tokens/ERC721.sol
...
// src/pricing/IOptionPricingV2.sol
...
// src/pricing/fees/IDopexV2ClammFeeStrategyV2.sol
...
// src/interfaces/IDopexV2PositionManager.sol
...
contract DopexV2OptionMarketV2 is ReentrancyGuard, Multicall, Ownable, ERC721 {
    ...
    event LogExerciseOption(address user, uint256 tokenId, uint256 totalProfit, uint256 totalAssetRelocked);
    event LogSettleOption(address user, uint256 tokenId);
    event LogSplitOption(address user, uint256 tokenId, uint256 newTokenId, address to);
    ...
    event LogUpdateAddress(address tokeURIFetcher, address dpFee, address optionPricing);
    ...
    function mintOption(OptionParams calldata _params) external nonReentrant {
        ...
        for (uint256 i; i < _params.optionTicks.length; i++) {
            ...
            bytes memory usePositionData = abi.encode(
                ...
                abi.encode(address(this), _params.ttl, _params.isCall, opTick.pool, opTick.tickLower, opTick.tickUpper)
            );
        ...
        for (uint256 i; i < _params.optionTicks.length; i++) {
            ...
            bytes memory donatePositionData =
                abi.encode(opTick.pool, opTick.hook, opTick.tickLower, opTick.tickUpper, liquidityToDonate);
        ...
        emit LogMintOption(msg.sender, optionIds, _params.isCall, premiumAmount, totalAssetWithdrawn, protocolFees);
    }
     ...
     */
    function exerciseOption(ExerciseOptionParams calldata _params)
        external
        nonReentrant
        returns (AssetsCache memory ac)
    {
        if (
            ownerOf(_params.optionId) != msg.sender && exerciseDelegator[ownerOf(_params.optionId)][msg.sender] == false
        ...
        emit LogExerciseOption(ownerOf(_params.optionId), _params.optionId, ac.totalProfit, ac.totalAssetRelocked);
    }
     ...
     */
    function settleOption(SettleOptionParams calldata _params) external nonReentrant {
        ...
        for (uint256 i; i < oData.opTickArrayLen; i++) {
            ...
            if ((amount0 > 0 && amount1 == 0) || (amount1 > 0 && amount0 == 0)) {
                if (isAmount0 && amount0 > 0) {
                    ...
                    ac.assetToGet.transfer(msg.sender, currentBalance - (prevBalance + amountReq));
                }
            } else {
                if (isAmount0) {
                    ac.assetToUse.approve(address(positionManager), amount0);
    ...
    function positionSplitter(PositionSplitterParams calldata _params) external nonReentrant {
        ...
        emit LogSplitOption(ownerOf(_params.optionId), _params.optionId, optionIds, _params.to);
    }
     ...
     */
    function getCurrentPricePerCallAsset(IUniswapV3Pool _pool) public view returns (uint256) {
        (, bytes memory result) = address(_pool).staticcall(abi.encodeWithSignature("slot0()"));
     ...
     */
    function getPremiumAmount(bool isPut, uint256 expiry, uint256 strike, uint256 lastPrice, uint256 amount)
        external
        view
        returns (uint256)
    {
     ...
     */
    function _getPremiumAmount(bool isPut, uint256 expiry, uint256 strike, uint256 lastPrice, uint256 amount)
        internal
        view
        returns (uint256 premiumAmount)
    {
        uint256 premiumInQuote = (amount * optionPricing.getOptionPrice(isPut, expiry, strike, lastPrice))
