...
contract StrategyInfoQuerier {
    ...
    function getRewardWbtcAmount(address _strategyContract) external view returns (uint256) {
        return IStrategyInfo(_strategyContract).rewardWbtcAmount();
    }

    /// @dev get User-Reward-Management-related Variable
    function getUserWbtcReward(address _strategyContract, address _userAddress)
        external
        view
        returns (uint256 userWbtcReward)
    {
        return IStrategyInfo(_strategyContract).userWbtcReward(_userAddress);
    }

    function getTotalUserWbtcReward(address _strategyContract) external view returns (uint256) {
    ...
    function getFarmAddress(address _strategyContract) external pure returns (address) {
        return IStrategyInfo(_strategyContract).getFarmAddress();
    }

    function getControllerAddress(address _strategyContract) external pure returns (address) {
        return IStrategyInfo(_strategyContract).getControllerAddress();
    }

    function getSwapAmountCalculatorAddress(address _strategyContract) external pure returns (address) {
        return IStrategyInfo(_strategyContract).getSwapAmountCalculatorAddress();
    }

    function getZapAddress(address _strategyContract) external pure returns (address) {
