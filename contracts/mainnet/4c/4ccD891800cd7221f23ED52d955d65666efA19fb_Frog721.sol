...
contract Frog721  is ERC721, Owner {

      string public uri;
      uint256 private latestTokenId;
      uint256 private maxTokenId = 20000;
     ...
     constructor() ERC721("WorldMeme NFT", "WorldMeme NFT") {
     }
    ...
    function mint(address recipient_) external returns (uint256) {
        ...
        require(tokenId <= maxTokenId, "exceed");
        _safeMint(recipient_,tokenId);
        _setTokenURI(tokenId,string(abi.encodePacked(uri,uint2str(tokenId),".json")));
        _incrementTokenId();
        return tokenId;
    }

    function nftTotalAmount() external view returns(uint256) {
        return maxTokenId;
    }

    function nftRemainAmount() external view returns(uint256) {
        return maxTokenId - latestTokenId;
    }
