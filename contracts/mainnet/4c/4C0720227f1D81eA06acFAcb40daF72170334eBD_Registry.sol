    ...
    IStargateReceiver
{
    ...
    function initialize(
        ...
        l.gasUsage[GasFunctionType.standard] = 400_000;
        // This requires the vault to update it's balance of the received asset and send a message back to the parent
        l.gasUsage[GasFunctionType.sgReceive] = 600_000;
        // This requires the vault to calculate its balance and send a message back to the parent
        l.gasUsage[GasFunctionType.getVaultValue] = 600_000;
        // This requires the transport to deploy the child vault and send a message back to the parent
        l.gasUsage[GasFunctionType.createChild] = 1_000_000;
        // This requires the vault to redeem all of the assets in the vault to the withdrawer and send a message back to the parent
...
contract VaultParentInvestor is VaultParentInternal {
    ...
    function _withdrawMultiChain(
        ...
        if (!_requiresSyncForWithdraw(tokenId)) {
            currentUnitPrice = 0;
        } else {
