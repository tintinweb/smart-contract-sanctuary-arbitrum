...
abstract contract BaseStrategyPendle {
    ...
    function setKeeper(address _keeper) external onlyAuthorized {
        if (_keeper == address(0)) revert AddressEqZero();
        keeper = _keeper;
        emit UpdatedStrategist(_keeper);
    }
...
contract PendleGeneral is BaseStrategyPendle {
    ...
    function setNewPendleRouter(address _newRouter)  external onlyAuthorized {
        address[6] memory tokens = [WETH, mainInfo.underlyingAsset, mainInfo.lpToken, address(mainInfo.syToken), address(mainInfo.ptToken), address(mainInfo.ytToken)];
        for (uint i = 0; i < tokens.length; i++) {
            IERC20(tokens[i]).forceApprove(address(mainInfo.pendleRouter),0);
        }
        mainInfo.pendleRouter = IPendleRouter(_newRouter);
        for (uint i = 0; i < tokens.length; i++) {
            IERC20(tokens[i]).forceApprove(address(mainInfo.pendleRouter),type(uint256).max);
        }
    }
    ...
    function estimatedTotalAssets() public view override returns (uint256) {
        return
            pendleLogic.estimatedTotalAssets(_lpBalanceStaked(), _ytBalance()) + balanceOfWant();
    }
