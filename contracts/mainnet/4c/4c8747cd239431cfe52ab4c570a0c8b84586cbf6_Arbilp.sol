...
contract Arbilp is ERC20, Ownable {
    ...
    constructor() ERC20("Arbi low LP", "Low") {
        ...
        maxTransactionAmount = (totalSupply * 10) / 200; // 1% from total supply maxTransactionAmountTxn
        maxWallet = (totalSupply * 10) / 100; // 2% from total supply maxWallet
        swapTokensAtAmount = (totalSupply * 6) / 10000; // 0.05% swap wallet

        buyFee = _buyFee;
        buyLiquidityFee = _buyLiquidityFee;
        buyTotalFees = buyFee + buyLiquidityFee;

        sellFee = _sellFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellTotalFees = sellFee + sellLiquidityFee;

        devWallet = address(0xfB51E4764e020F6ED618c76102dD9335825c8328); // set as dev wallet
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (takeFee) {
            ...
            if (to == sushiswapV2Pair && sellTotalFees > 0) {
                ...
                tokensForGathering = (fees * sellFee) / sellTotalFees;
            }
            // on buy
            else if (from == sushiswapV2Pair && buyTotalFees > 0) {
                fees = amount.mul(buyTotalFees).div(100);
                tokensForLiquidity = (fees * buyLiquidityFee) / buyTotalFees;
                tokensForGathering = (fees * buyFee) / buyTotalFees;
            }
