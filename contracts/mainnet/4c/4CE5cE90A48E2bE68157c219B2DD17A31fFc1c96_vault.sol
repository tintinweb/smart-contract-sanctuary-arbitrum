...
contract vault is Ownable, ReentrancyGuard {
    ...
    function cycleRewards() external {
        ...
        uint256 compoundAmount = rewards.div(2);
    ...
    function updatePoolRate(uint256 _pid) internal {
        poolInfo[_pid].EarnRateSec =  poolInfo[_pid].totalStaked.mul(15000000).div(100000000).div(365).div(24).div(3600);
    }
    ...
    function addPool(IERC20 _lptoken, GDtoken _GDlptoken) external onlyOwner{
    ...
    function leaveETH(uint256 _amountin, uint256 _pid, address payable receiver) external nonReentrant{
    ...
    function enter(uint256 _amountin, uint256 _pid) public nonReentrant {
        ...
        uint256 totalUSDC = poolInfo[_pid].totalStaked;
        // Gets the amount of gdUSDC in existence
        uint256 totalShares = GDT.totalSupply();

        uint256 amountAfterFee = _amount.mul(99500).div(100000);
        ...
        // Calculate and mint the amount of gdUSDC the USDC is worth. The ratio will change overtime
            ...
            uint256 what = amountAfterFee.mul(totalShares).div(totalUSDC);
