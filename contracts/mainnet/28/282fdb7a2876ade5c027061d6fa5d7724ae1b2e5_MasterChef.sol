...
contract MasterChef is Ownable, IMasterChef {
    ...
    struct PoolInfo {
        ...
        uint256 reserveWETH;
        uint256 allocPointsARX; // % of dummy pools ARX allocation for this pool
        uint256 allocPointsWETH; // % of dummy pools WETH allocation for this pool
    }

    // Total allocation points. Must be the sum of all allocation points in all pools
    uint256 public totalAllocPointsARX;
    ...
    IERC20 private immutable _arxToken;
    IERC20 public immutable WETH;
    IERC20 public immutable dummyToken;
    IArbidexMasterChef public immutable mainChef;
    ...
    event PoolAdded(address indexed poolAddress, uint256 allocPointsARX, uint256 allocPointsWETH);
    event PoolSet(address indexed poolAddress, uint256 allocPointsARX, uint256 allocPointsWETH);
    ...
    event TokenWithdraw(IERC20 token, uint256 amount);
    ...
    function start() external onlyOwner {
        ArbidexPoolUserInfo memory poolInfo = getOldChefUserInfo();
        require(poolInfo.amount == 0, "Already deposited in dummy pool");
        uint256 callerBalance = dummyToken.balanceOf(msg.sender);
    ...
    function harvest() public {
        ...
        (uint256 pendingArx, uint256 pendingWETH) = getPendingRewardsFromOldChef();

        if (pendingArx == 0 && pendingWETH == 0) return;

        uint256 arxBalanceBefore = _arxToken.balanceOf(thisAddress);
        uint256 wethBalanceBefore = WETH.balanceOf(thisAddress);

        // Trigger harvest
        mainChef.deposit(poolId, 0);

        uint256 arxReceived = _arxToken.balanceOf(thisAddress) - arxBalanceBefore;
    ...
    /**
     * @dev Returns ARX and WETH token emission rate from main chef (allocated to this contract)
     */
    function emissionRates() public view returns (uint256 mainRate, uint256 wethRate) {
        ArbidexPoolInfo memory oldChefPoolInfo = getMainChefPoolInfo();

        uint256 arxPerSecond = mainChef.arxPerSec();
        uint256 oldChefTotalAllocationPointsARX = mainChef.arxTotalAllocPoint();
        uint256 wethPerSecond = mainChef.WETHPerSec();
        uint256 oldChefTotalAllocationPointsWETH = mainChef.WETHTotalAllocPoint();

        // Determine ratio of the dummy pools allocationS revelative to the overall emissions from old chef
        mainRate = (arxPerSecond * oldChefPoolInfo.arxAllocPoint) / oldChefTotalAllocationPointsARX;
        wethRate = (wethPerSecond * oldChefPoolInfo.WETHAllocPoint) / oldChefTotalAllocationPointsWETH;
    }

    function getPendingRewardsFromOldChef() public view returns (uint256 pendingARX, uint256 pendingWETH) {
        uint256 poolId = mainChefPoolId;
        address thisAddress = address(this);
    ...
    function getOldChefUserInfo() public view returns (ArbidexPoolUserInfo memory poolInfo) {
        poolInfo = mainChef.userInfo(mainChefPoolId, address(this));
    }

    function getRewardTokenBalances() public view returns (uint256 amountARX, uint256 amountWETH) {
        amountARX = _arxToken.balanceOf(address(this));
    ...
    function claimRewards() external override returns (uint256 rewardAmount, uint256 amountWETH) {
        ...
        _safeRewardsTransfer(_arxToken, msg.sender, rewardAmount);
    ...
    function _updatePool(address poolAddress) internal {
        // Pull any pending rewards while we're here
        harvest();
        ...
        uint256 allocPointsARX = pool.allocPointsARX; // gas saving
        uint256 allocPointsWETH = pool.allocPointsWETH; // gas saving

        // do not allocate rewards if pool is not active
        if ((allocPointsARX > 0 || allocPointsWETH > 0) && INFTPool(poolAddress).hasDeposits()) {
            ...
            uint256 mainRewards = (duration * mainRate * allocPointsARX) / totalAllocPointsARX;
    ...
    function _depositToPool() internal {
        uint256 contractBalance = dummyToken.balanceOf(address(this));
        if (contractBalance > 0) {
            dummyToken.safeApprove(address(mainChef), contractBalance);
            mainChef.deposit(mainChefPoolId, contractBalance);
        }
    }
    ...
    function add(
        ...
        if (allocPointsARX > 0 || allocPointsWETH > 0) {
            if (withUpdate) {
                // Update all pools if new pool allocPoint(s) > 0
                _massUpdatePools();
            }
        ...
        // update totalAllocPoint(s) with the new pool's points
        totalAllocPointsARX += allocPointsARX;
        totalAllocPointsWETH += allocPointsWETH;

        // add new pool
        _poolInfo[poolAddress] = PoolInfo({
            allocPointsARX: allocPointsARX,
        ...
        emit PoolAdded(poolAddress, allocPointsARX, allocPointsWETH);
    }
    ...
    function set(
        ...
        // Stash before pool update
        uint256 prevAllocPointsARX = pool.allocPointsARX;
        ...
        totalAllocPointsARX = (totalAllocPointsARX - prevAllocPointsARX) + allocPointsARX;
        totalAllocPointsWETH = (totalAllocPointsWETH - prevAllocPointsWETH) + allocPointsWETH;

        pool.allocPointsARX = allocPointsARX;
        pool.allocPointsWETH = allocPointsWETH;

        // if request is activating the pool
        if ((prevAllocPointsARX == 0 && allocPointsARX > 0) || (prevAllocPointsWETH == 0 && allocPointsWETH > 0)) {
            _activePools.add(poolAddress);
        }
        // if request is deactivating the pool
        else if ((prevAllocPointsARX > 0 && allocPointsARX == 0) && (prevAllocPointsWETH > 0 && allocPointsWETH == 0)) {
    ...
    /**
     * @dev Helper for migrating to new chef while keeping old chef state for dummy pool intact
     */
    ...
    function updateTreasury(address _treasury) external onlyOwner {
        require(_treasury != address(0), "updateTreasury: cannot be set to zero address");
    ...
    /**
     * @dev In the event of needed to extract tokens
     *
     * Funds are automatically sent to treasury address.
     */
    function withdrawToken(IERC20 token) external onlyOwner {
        uint256 amount = token.balanceOf(address(this));
        _safeRewardsTransfer(token, treasury, amount);
