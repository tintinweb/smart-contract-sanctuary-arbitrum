...
/**
 * @title PoroPredictions
 */
contract PoroPredictions is Ownable, Pausable, ReentrancyGuard {
  ...
  address public relayerAddress; // address of the operator
  ...
  modifier onlyRelayer() {
    require(msg.sender == relayerAddress, "Caller is not relayerAddress");
    _;
  }
   ...
   * @param _relayerAddress: relayer address
  ...
  function _genesisLockRound() internal {
  ...
  function _genesisStartRound() internal {
  ...
  function intervalCall() external onlyRelayer {
    if (!genesisStartOnce) {
      _genesisStartRound();
  ...
  function absoluteDifference(uint256 a, uint256 b) private pure returns (uint256) {
    return a > b ? a - b : b - a;
  }

  function _isTimeReached(uint256 _deadline) internal view returns (bool) {
    return absoluteDifference(_deadline, block.timestamp) <= 5;
  }
  ...
  function _safeEndRound(
    ...
    require(_isTimeReached(rounds[epoch].closeTimestamp), "Can only end round after closeTimestamp");
  ...
  function _safeLockRound(
    ...
    require(_isTimeReached(rounds[epoch].lockTimestamp), "Can only lock round after lockTimestamp");
  ...
  function _safeStartRound(uint256 epoch) internal {
    ...
    require(
      _isTimeReached(rounds[epoch - 2].closeTimestamp),
      "Can only start new round after round n-2 closeTimestamp"
    );
    _startRound(epoch);
  }

  function _getBlockTimestampOrStartTimestamp(uint256 epoch) internal view returns (uint256) {
    if (epoch > 0 && rounds[epoch - 1].startTimestamp != 0) {
      return rounds[epoch].startTimestamp + intervalSeconds;
    }
  ...
  function _startRound(uint256 epoch) internal {
    Round storage round = rounds[epoch];
    uint256 startTimestamp = _getBlockTimestampOrStartTimestamp(epoch);
