...
contract AutoCompound is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        } else if (rewardToken != USDC && IArbiDexFactory(factory).getPair(rewardToken, stakedToken) != address(0)) {
            // There is a pair directly between the reward token and the staked token
            path = new address[](2);
