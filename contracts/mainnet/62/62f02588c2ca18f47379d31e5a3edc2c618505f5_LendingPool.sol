...
abstract contract AbstractLender is OwnableLiquidityPool {
    ...
    function changeOracle(
        ...
        IPeerToPeerOpenTermLoan(loanAddr).changeOracle(newOracle);
    }
    ...
    function seizeCollateral(
        ...
        _ensureValidLoan(loanAddr);
        IPeerToPeerOpenTermLoan(loanAddr).seizeCollateral(amount);
    }
    ...
    function returnCollateral(
        ...
        address collateralTokenAddr = IPeerToPeerOpenTermLoan(loanAddr).collateralToken();
        SafeERC20.safeApprove(IERC20(collateralTokenAddr), loanAddr, depositAmount);

        IPeerToPeerOpenTermLoan(loanAddr).returnCollateral(depositAmount);
...
abstract contract BaseLendingPool is HookableLender {
    ...
    function deployLoan(
        ...
        /*
        // Make sure we are cloning a loan
        require(address(ILender(referenceImplementationAddress)) != address(0), "Invalid clone reference");

        // Clone the bytecode (but not the data) available at the address specified.
        // The clone is created via the op-code CREATE so it generates a new non-deterministic address for the clone.
        address newStandaloneImplAddr = _cloneBytecode(referenceImplementationAddress);
        require(newStandaloneImplAddr != address(0) && newStandaloneImplAddr != address(referenceImplementationAddress), "Invalid clone");

        // Double-check
        ILender loanInstance = ILender(newStandaloneImplAddr);
        require(address(loanInstance) != address(0), "Invalid loan interface");

        // This should never happen because the clone was forged via CREATE rather than CREATE2
        require(!_deployedLoans[newStandaloneImplAddr].isWhitelisted, "Invalid deployment address");

        // Initialize the loan
        loanParams.lenderAddr = address(this);
        loanInstance.initLoan(loanParams);

        uint256 effectiveLoanAmount = ILender(newStandaloneImplAddr).effectiveLoanAmount();

        _deployedLoans[newStandaloneImplAddr] = LoanDeploymentRecord({
            effectiveLoanAmount: effectiveLoanAmount,
            activeDelta: 0,
            isWhitelisted: true
        });

        loansDeployed.push(newStandaloneImplAddr);

        emit NewLoanDeployedByPool(newStandaloneImplAddr);

        return newStandaloneImplAddr;
        */
    }
    ...
    function fundLoan(address loanAddr) external override onlyIfInitialized nonReentrant ifConfigured onlyLoansOperator {
        ...
        // Late checks
        require(IPeerToPeerOpenTermLoan(loanAddr).loanState() == ACTIVE, "Funding check failed");
...
/**
 * @title Represents a lending pool. All withdrawals are time-locked. The lending pool is fully compliant with the ERC-4626 standard.
 */
