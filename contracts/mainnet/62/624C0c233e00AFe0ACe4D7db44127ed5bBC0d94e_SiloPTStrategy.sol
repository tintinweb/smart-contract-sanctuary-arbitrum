    ...
    UUPSUpgradeable
{
    ...
    function getAssetTokens() public view returns (address[] memory allTokens) {
        return SiloPTStrategyLib.getAssetTokens(getPoolAddress());
    }
    ...
    function assetBalance() public view returns (uint256) {
        return
            IS_PROTECTED
                ? SiloPTStrategyLib.assetBalanceProtected(assetPool())
                : SiloPTStrategyLib.assetBalance(getPoolAddress(), assetPool(), asset());
    }

    function debtBalance() public view returns (uint256) {
        return SiloPTStrategyLib.debtBalance(getPoolAddress(), debtPool(), debtToken());
    }
    ...
    function _flAddLeverage(bytes calldata params, uint256 feeAmount) internal {
        ...
        uint256 outAmount = this.swapBySelf(debtToken(), getLRTTokenAddress(), amount, data);
        // deposit and get PT Token
        TransferHelper.safeApprove(getLRTTokenAddress(), pendleRouterV3, outAmount);
        outAmount = SiloPTStrategyLib._swapPendle(true, outAmount, getLRTTokenAddress(), getPendleMarket());
        ...
        ISiloStrategy(poolAddress).deposit(asset(), outAmount, IS_PROTECTED);
    ...
    function _flRemoveLeverage(bytes calldata params, uint256 feeAmount) internal {
        ...
        SiloPTStrategyLib._swapPendle(false, amount, getLRTTokenAddress(), getPendleMarket());
        TransferHelper.safeApprove(getPTTokenAddress(), pendleRouterV3, 0);

        // Swap LRT_TOKEN to debtToken and get the swapped amount
        (uint256 amountOO, bytes memory dataOO) = abi.decode(data, (uint256, bytes));
        uint256 outAmount = this.swapBySelf(getLRTTokenAddress(), debtToken(), amountOO, dataOO);
    ...
    function _flCloseLeverage(bytes calldata params, uint256 feeAmount) internal {
        ...
        if (isExpired()) {
            ...
            uint256 netSyOut = IPYieldTokenV2(YT()).redeemPY(SY());
            IPYieldTokenV2(SY()).redeem(address(this), netSyOut, LRT_TOKEN, 0, true);

            this.swapBySelf(getLRTTokenAddress(), debtToken(), amountOO, dataOO);
        } else {
    ...
    function toAmountRoundUp(uint256 share, uint256 totalAmount, uint256 totalShares) internal pure returns (uint256) {
        return SiloPTStrategyLib.toAmountRoundUp(share, totalAmount, totalShares);
    }
    ...
    function withdrawTokenInCaseStuck(address tokenAddress, uint256 amount) external onlyOwner {
        SiloPTStrategyLib.withdrawTokenInCaseStuck(tokenAddress, amount, assetPool(), debtPool());

        emit WithdrawTokenInCaseStuck(tokenAddress, amount);
    }

    function supply(uint256 amount) external onlyOwner {
        SiloPTStrategyLib.supply(asset(), amount, getPoolAddress(), IS_PROTECTED);
        emit Supply(amount);
    }

    function borrow(uint256 amount) external onlyOwner {
        SiloPTStrategyLib.borrow(debtToken(), amount, getPoolAddress());
        emit Borrow(amount);
    }

    function repay(uint256 amount) external onlyOwner {
        SiloPTStrategyLib.repay(debtToken(), amount, getPoolAddress());
        emit Repay(amount);
    }

    function withdraw(uint256 amount) external onlyOwner {
        SiloPTStrategyLib.withdraw(asset(), assetPool(), amount, getPoolAddress(), IS_PROTECTED);
    ...
    function claimRewards(address rewardsToken) external onlyOwner {
        ...
        uint256 transferredAmount = SiloPTStrategyLib.claimReward(
            incentiveAddress,
            rewardsToken,
            assetTokens,
            fee,
    ...
    function _getIncentiveAddress(address rewardsToken) internal pure returns (address) {
        if (rewardsToken == SiloPTStrategyLib.siloToken) {
            return SiloPTStrategyLib.siloIncentive;
        } else if (rewardsToken == SiloPTStrategyLib.arb) {
            return SiloPTStrategyLib.siloIncentiveSTIP;
        }
