...
contract StrategyKyber is Strategy, ERC721TokenReceiver {
    ...
    function _earn() internal override {
        ...
        uint256 val0 = strategyHelper.value(address(token0), bal0);
        uint256 val1 = strategyHelper.value(address(token1), bal1);
        uint256 valRatio = val0 * 1e18 / (val0 + val1);
        (uint256 a0, uint256 a1) = LiquidityAmounts.getAmountsForLiquidity(sqrtP, minSqrtRatio, maxSqrtRatio, 1e18);
        uint256 liqRatio = a0 * 1e18 / (a0 + a1);

        if (valRatio > liqRatio) {
            uint256 targetVal = (val0 + val1) * liqRatio / 1e18;
            uint256 targetBal = targetVal * (10 ** token0.decimals()) / strategyHelper.price(address(token0));
            uint256 amt = (bal0 - targetBal) * (10000+(2*slippage)) / 10000;
            ...
            uint256 targetVal = (val0 + val1) * (1e18 - liqRatio) / 1e18;
            uint256 targetBal = targetVal * (10 ** token1.decimals()) / strategyHelper.price(address(token1));
            uint256 amt = (bal1 - targetBal) * (10000+(2*slippage)) / 10000;
...
interface IKyberStaking {
     ...
     *
     */
    function emergencyWithdrawForOwner(address[] calldata rewards, uint256[] calldata amounts) external;
