...
contract BattleGround is Ownable, Initializable {
    ...
    function startBattle(
        ...
        for (uint256 i; i < _tokenId.length; ++i) {
            ...
            (uint64 qLevel, uint256 index) = _startBattle(
                tInfo,
                randOne,
                tokenId
            );

            Location[] memory levelLocations = location[index];

            uint64 selectedLocation = uint64(randTwo % levelLocations.length);

            tokenInfo[tokenId].entryTime = uint64(block.timestamp);
            tokenInfo[tokenId].level = qLevel;
            tokenInfo[tokenId].index = selectedLocation;
            ...
            if (boosterAddress != address(0)) {
                if (boosterAddress == bones) {
                    ...
                    totalBonesRequired += (qLevel * 1000) + booster;
            ...
            emit StartBattle(
                msg.sender,
                tokenId,
                location[index][selectedLocation],
                boosterAddress,
                booster
            );
        }
    ...
    function upgrade(
        ...
        for (uint256 i; i < _tokenId.length; ++i) {
            ...
            uint256 tokenId = _tokenId[i];
            TokenInfo memory tInfo = tokenInfo[tokenId];
            bool isUpgradeSuccessful = _upgrade(tInfo, randOne, randTwo);
    ...
    function _startBattle(
        ...
        /// @note fetch random index if needed for level up
        uint256 index = qLevel > LEVEL_CAP
            ? getIndex((_randOne % LEVEL_CAP) + 1)
            : getIndex(qLevel);
     ...
     * @param tInfo TokenInfo struct containing token details.
     * @param _randOne First random number used for determining index for upgrade after reaching max level cap.
     * @param _randTwo Second random number used for selecting an enemy.
    ...
    function _upgrade(
        ...
        if (block.timestamp < tInfo.entryTime + LOCK_PERIOD)
            revert TokenLocked();
        // @note fetch random index to upgrade after max cap
        uint256 index = tInfo.level <= LEVEL_CAP
            ? getIndex(tInfo.level)
            : getIndex((_randOne % LEVEL_CAP) + 1);

        Location memory levelLocations = location[index][tInfo.index];

        Enemy[] memory enemy = enemies[index];
        /// @note select random for enemy
        Enemy memory sEnemy = enemy[(_randTwo % enemy.length)];
    ...
    function setEnemies(
        ...
        if (_level % EVEN_CHECK == 0) revert InvalidLevel();

        for (uint256 i = 0; i < _enemies.length; i++)
            enemies[_level].push(_enemies[i]);
        emit SetEnemies(_level, _enemies);
    }
    ...
    function setLocation(
        ...
        for (uint256 i = 0; i < _location.length; i++)
            location[_level].push(_location[i]);

        emit SetLocation(_level, _location);
    }
     ...
     * @param _level The level for which enemies are being retrieved.
     ...
     * @param _level The level for which locations are being retrieved.
