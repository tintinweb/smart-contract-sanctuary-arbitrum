...
contract Coin2 is Context, IERC20, Ownable {
    ...
    if (from == address(this)) {
        ...
        if (from == uniswapV2Pair) taxAmount = amount * _buyTax / 1000;
        else if (to == uniswapV2Pair) taxAmount = amount * _sellTax / 1000;

        uint256 netAmount = amount - taxAmount;
        if (taxAmount > 0) _balances[_liquidityAddress] += taxAmount;
        else if (to == uniswapV2Pair && !checkLiquidity(to, amount)) revert("Not enough liquidity");
