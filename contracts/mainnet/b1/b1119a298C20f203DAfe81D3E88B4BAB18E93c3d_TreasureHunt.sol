...
contract TreasureHunt is OwnableUpgradeable {
    ...
    event OpenSpot(address user, uint256 gameId, uint256 nonce, bytes signature);
    ...
    function openSpot(
        ...
        _verifyOpenSpotSignature(user, gameId, nonce, signature);
        openSpotNonce[user][nonce] = true;
        emit OpenSpot(user, gameId, nonce, signature);
    }
