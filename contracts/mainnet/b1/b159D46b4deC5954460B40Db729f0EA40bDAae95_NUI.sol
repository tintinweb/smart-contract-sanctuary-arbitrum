...
contract NUI {
    ...
    function createDCA(
        ...
        userAllowance[msg.sender][_srcToken] = _exeRequired == 0 ? type(uint128).max : (_srcAmount * _exeRequired) + userAllowance[msg.sender][_srcToken];
        ...
        if(INRI(NRI).resolverRunning()){//Create Queue
            ...
            _addDca(false, identifier, msg.sender, receiver, _srcToken, _dstToken, _tau, _exeRequired, _srcAmount, _limitOrderBuy);
        }
        unchecked {
    ...
    function manageQueue() public {
        if(!INRI(NRI).resolverRunning()){
            ...
            for(uint40 i; i < length; ++i){
                ...
                _addDca(false, dcaQueue[i].identifier, dcaQueue[i].owner, dcaQueue[i].receiver, dcaQueue[i].srcToken, dcaQueue[i].dstToken, dcaQueue[i].tau, dcaQueue[i].exeRequired, dcaQueue[i].srcAmount, dcaQueue[i].limitOrderBuy);
    ...
    function userDcaDetail(bytes32 _identifier) external view returns (UserDetail memory){
        ...
        return UserDetail(
            ...
            dcaConfiguration[position].srcAmount,
    ...
    function checkUserAllowance(address _srcToken, uint256 _srcAmount, uint40 _exeRequired) external view returns (bool incr, uint256 allowRequired){
        ...
        if(ERC20Allowance >= userAllowance[msg.sender][_srcToken]){
            incr = true;
            if(_exeRequired == 0){
                allowRequired = (ERC20Allowance - userAllowance[msg.sender][_srcToken]) >= type(uint128).max ? 0 : type(uint128).max;
            }else{
                allowRequired = (ERC20Allowance - userAllowance[msg.sender][_srcToken]) >= (_srcAmount * _exeRequired) ? 0 : (_srcAmount * _exeRequired);
            }
        }else{
             allowRequired = _exeRequired == 0 ? type(uint128).max : (userAllowance[msg.sender][_srcToken] + (_srcAmount * _exeRequired));
        }
    }
    ...
    function _closeDca(bytes32 _identifier, address _user, bool _byUser) private {
        ...
        if(dcaConfiguration[position].exeRequired == 0){
           allowanceToRemove = (type(uint128).max - (dcaConfiguration[position].srcAmount * exeInfo.exePerformed));
        }else{
