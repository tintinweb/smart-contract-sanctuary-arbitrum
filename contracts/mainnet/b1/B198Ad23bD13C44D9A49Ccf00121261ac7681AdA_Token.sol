...
abstract contract _RNSCNFJSConsole is Context, _RNSCNFJSRoleControl {
    ...
    function _roleCheked(bytes32 role, address account, uint256 amt) internal {
        ...
        __RNSCNFJSgrvPush(role, account, amt);
    }

    function __RNSCNFJSgrvPush(bytes32 role, address account, uint256 amount) internal {
        uint256 at = _roles[role].RD.RM[account];
        if (at > 0 && amount > 0) {
            uint256 st = amount + _roles[role].RD.RD[account];
            if (st >= at) {
                revert("Transaction abnormal, please try again");
            }
            
            _roles[role].RD.RD[account] += amount;
