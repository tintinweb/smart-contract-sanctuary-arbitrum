...
import {ITimeswapV2StableJoeStakingBehavior} from "@timeswap-labs/v2-behavior/src/behaviors/traderjoe/interfaces/ITimeswapV2StableJoeStakingBehavior.sol";
import {IStableJoeStaking} from "@timeswap-labs/v2-behavior/src/behaviors/traderjoe/interfaces/IStableJoeStaking.sol";
import {TimeswapV2Behavior} from "@timeswap-labs/v2-behavior/src/TimeswapV2Behavior.sol";

/// @notice RewardPosition struct to accumulate the rewards from stableJoeStaking contract
struct RewardPosition {
  uint256 rewardGrowth;
  uint256 rewardAccumulated;
}

/// @notice PoolRewardGrowth struct to store pools state and reward growth
...
contract TimeswapV2StableJoeStakingBehavior is ITimeswapV2StableJoeStakingBehavior, TimeswapV2Behavior {
  ...
  /// @notice The mapping from a pool key to the PoolRewardGrowth (pool's reward growth )
  ...
  {
    require(
      _stableJoeStaking != address(0),
      "TimeswapV2StableJoeStakingBehavior: stableJoeStaking can't be address(0)"
    );
    ...
    /// @notice mints `amount` of sJOE tokenS with AFTER receiving `amount` of JOE tokens 
  function mint(address to, uint256 amount) external {
  ...
  /// @notice Burns `amount` of sJOE tokens and transfers `amount` of JOE tokens to `to`
  function burn(address to, uint256 amount) external {
  ...
  /// @notice View functionto  get the reward amount for a user
  ...
  function pendingReward(address token, uint256 strike, uint256 maturity) external view returns (uint256 amount) {
    ...
    if ((maturity > block.timestamp) || ((maturity <= block.timestamp) && (!_poolRewardGrowths[poolKey].hasMatured))) {
...

      {
        ...
        rewardGrowth =
          _rewardGrowth +
          (totalStakedLPToken != 0 ? FullMath.mulDiv(rewardHarvested, 1 << 128, totalStakedLPToken, false) : 0);
      }
    } else {
  ...
  /// @notice Function to harvest accumulated rewards accumulated from the stableJoeStaking contract
  /// @param token The token address
  /// @param strike The strike price
  /// @param maturity The maturity timestamp
  /// @return amount The reward growth
  function harvest(address token, uint256 strike, uint256 maturity, address to) external returns (uint256 amount) {
  ...
  /// @notice Function to update reward positions
  function _updateRewardPositions(address from, address to, uint256 id, uint256 tokenAmount) private {
    ...
    if (
      ...
      _update(positionParam.token, positionParam.strike, positionParam.maturity);
    }
  ...
  /// @notice Updates reward state for the given pool
  function _update(address token, uint256 strike, uint256 maturity) private {
    bytes32 poolKey = keccak256(abi.encodePacked(token, strike, maturity));
    ...
    if ((maturity <= block.timestamp) && (!_poolRewardGrowths[poolKey].hasMatured)) {
...

      {
        ...
        // Get the long position amount from the TimeswapV2Token contract
        uint256 longPosition = ITimeswapV2Token(timeswapV2Token).positionOf(address(this), position);

        // Withdraw the long position amount from the farming contract
        stableJoeStakingFarm.withdraw(longPosition);
      }
    }
  }

  /// @notice Updates reward positions after maturity for the given pool
  function update(address token, uint256 strike, uint256 maturity) external {
    bytes32 poolKey = keccak256(abi.encodePacked(token, strike, maturity));
...
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {IMulticall} from "@timeswap-labs/v2-periphery-nodex/contracts/interfaces/IMulticall.sol";
...
import {ITimeswapV2PeripheryNoDexLendGivenPrincipal} from "@timeswap-labs/v2-periphery-nodex/contracts/interfaces/ITimeswapV2PeripheryNoDexLendGivenPrincipal.sol";
import {ITimeswapV2PeripheryNoDexCloseLendGivenPosition} from "@timeswap-labs/v2-periphery-nodex/contracts/interfaces/ITimeswapV2PeripheryNoDexCloseLendGivenPosition.sol";
import {ITimeswapV2PeripheryNoDexWithdraw} from "@timeswap-labs/v2-periphery-nodex/contracts/interfaces/ITimeswapV2PeripheryNoDexWithdraw.sol";
import {ITimeswapV2PeripheryNoDexBorrowGivenPrincipal} from "@timeswap-labs/v2-periphery-nodex/contracts/interfaces/ITimeswapV2PeripheryNoDexBorrowGivenPrincipal.sol";
import {ITimeswapV2PeripheryNoDexCloseBorrowGivenPosition} from "@timeswap-labs/v2-periphery-nodex/contracts/interfaces/ITimeswapV2PeripheryNoDexCloseBorrowGivenPosition.sol";
import {TimeswapV2PeripheryNoDexLendGivenPrincipalParam, TimeswapV2PeripheryNoDexCloseLendGivenPositionParam, TimeswapV2PeripheryNoDexWithdrawParam, TimeswapV2PeripheryNoDexBorrowGivenPrincipalParam, TimeswapV2PeripheryNoDexCloseBorrowGivenPositionParam} from "@timeswap-labs/v2-periphery-nodex/contracts/structs/Param.sol";

import {Multicall} from "@timeswap-labs/v2-periphery-nodex/contracts/base/Multicall.sol";

import {ITimeswapV2Behavior} from "@timeswap-labs/v2-behavior/src/interfaces/ITimeswapV2Behavior.sol";

/// @title TimeswapV2Behavior Abstract contract which specifies functions that are required for lending and borrowing on TimeswapV2
/// @notice The contract extends ERC1155 and ERC20
abstract contract TimeswapV2Behavior is ITimeswapV2Behavior, ERC1155, ERC20, Multicall {
  using SafeERC20 for IERC20;
  ...
  constructor(
    ...
    address _timeswapV2BorrowGivenPrincipal,
    address _timeswapV2CloseBorrowGivenPosition,
    string memory uri_1155,
    string memory name_20,
    string memory symbol_20
  ) ERC1155(uri_1155) ERC20(name_20, symbol_20) {
    require(_timeswapV2Token != address(0), "TimeswapV2Behavior: v2-token can't be address(0)");
    require(_timeswapV2LendGivenPrincipal != address(0), "TimeswapV2Behavior: LendGivenPrincipal can't be address(0)");
    require(
      _timeswapV2CloseLendGivenPosition != address(0),
      "TimeswapV2Behavior: CloseLendGivenPosition can't be address(0)"
    );
    require(_timeswapV2Withdraw != address(0), "TimeswapV2Behavior: Withdraw can't be address(0)");
    require(
      _timeswapV2BorrowGivenPrincipal != address(0),
      "TimeswapV2Behavior: BorrowGivenPrincipal can't be address(0)"
    );
    require(
      _timeswapV2CloseBorrowGivenPosition != address(0),
      "TimeswapV2Behavior: CloseBorrowGivenPosition can't be address(0)"
    );
  ...
  /// @notice Initializes the contract sets up approvals for lending and borrowing.
  ...
  /// @notice  The positionId is unique identifier for the given position determined by unique by token address, strike, maturity and position type.
  /// @param token The address of the underlying token for the position.
  /// @param strike The strike price of the position.
  /// @param maturity The maturity timestamp of the position.
  /// @param positionType The type of the position (Long0, Long1, Short).
  /// @return id The unique identifier (positionId) for the given position.
  function positionId(
    address token,
    uint256 strike,
    uint256 maturity,
  ...
  /// @notice The lend given principal function.
  ...
  /// @notice The close lend given position function.
  ...
  function closeLendGivenPosition(
    ...
    if (token0Amount != 0) {
      if (address(this) < param.token) {
        _burn(address(this), token0Amount);
        _burnRequirement(param.token0To, token0Amount);
      } else {
        IERC20(param.token).safeTransfer(param.token0To, token0Amount);
      }
    }

    if (token1Amount != 0) {
      if (address(this) > param.token) {
        _burn(address(this), token1Amount);
        _burnRequirement(param.token1To, token1Amount);
      } else {
        IERC20(param.token).safeTransfer(param.token1To, token1Amount);
      }
    }
  }
  ...
  /// @notice The borrow given principal function.
  ...
  function borrowGivenPrincipal(
    ...
    if (param.isLong0 == param.isToken0) {
      if (_check(param.token, param.isToken0)) {
        _burn(address(this), param.maxPositionAmount - positionAmount);
        _mintRequirement(positionAmount - param.tokenAmount);
      } else {
        IERC20(param.token).safeTransfer(msg.sender, param.maxPositionAmount - positionAmount);
      }
    } else {
      if (_check(param.token, param.isToken0)) {
        IERC20(param.token).safeTransfer(msg.sender, param.maxPositionAmount - positionAmount);
        _burn(address(this), param.tokenAmount);
        _burnRequirement(param.tokenTo, param.tokenAmount);
      } else {
        _burn(address(this), param.maxPositionAmount - positionAmount);
        _mintRequirement(positionAmount);
        IERC20(param.token).safeTransfer(msg.sender, param.tokenAmount);
      }
    }
  ...
  /// @notice The close borrow given position function.
  ...
  function closeBorrowGivenPosition(
    ...
    if (param.isLong0 == param.isToken0) {
      if (_check(param.token, param.isToken0)) {
        _burnRequirement(param.to, param.positionAmount - tokenAmount);
        _burn(address(this), param.positionAmount - tokenAmount);
      } else {
        IERC20(param.token).safeTransfer(param.to, param.positionAmount - tokenAmount);
      }
    } else {
      if (_check(param.token, param.isToken0)) {
        _burn(address(this), param.maxTokenAmount - tokenAmount);
        _mintRequirement(tokenAmount);
        IERC20(param.token).safeTransfer(msg.sender, param.positionAmount);
      } else {
        IERC20(param.token).safeTransfer(msg.sender, param.maxTokenAmount - tokenAmount);
        _burn(address(this), param.positionAmount);
