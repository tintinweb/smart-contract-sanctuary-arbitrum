...
contract FireRewards is IFireRewards, Ownable {
...
contract CrackFire is Context, IERC20, Ownable {
    using SafeMath for uint256;

    IDEXRouter private _IDEXRouter;

    FireRewards private _rewards;
    ...
    string private constant _name = "Crack Fire";
    string private constant _symbol = "FYA";
    ...
    constructor (address liquidityWalletAddy, address treasuryWalletAddy, address developmentWalletAddy) {
        ...
        _rewards = new FireRewards(address(this));
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && to != ZERO && to != DEAD && !swapping) {
            ...
            if (cooldownEnabled) {
                if (to != address(_IDEXRouter) && to != address(_uniswapV2Pair)){
            ...
            if (from == _uniswapV2Pair && to != address(_IDEXRouter) && !_isExcludedMaxTransactionAmount[to]) {
            ...
            if (to == _uniswapV2Pair && from != address(_IDEXRouter) && !_isExcludedMaxTransactionAmount[from]) {
    ...
    function swapTokensForTokens(uint256 tokenAmount) private {
        ...
        _IDEXRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
            address(_receiver),
    ...
    function addLiquidity(uint256 tokenAmount, uint256 usdcAmount) private {
        _approve(address(this), address(_IDEXRouter), tokenAmount);
        IERC20(USDC).approve(address(_IDEXRouter), usdcAmount);
        _IDEXRouter.addLiquidity(
            address(this),
            USDC,
            tokenAmount,
            usdcAmount,
    ...
    //     IDEXRouter uniswapV2Router = IDEXRouter(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506);
    //     _IDEXRouter = uniswapV2Router;
    //     _approve(address(this), address(_IDEXRouter), _tTotal);
    //     IERC20(USDC).approve(address(_IDEXRouter), IERC20(USDC).balanceOf(address(this)));
    //     _uniswapV2Pair = IDEXFactory(uniswapV2Router.factory()).createPair(address(this), USDC);
    //     _IDEXRouter.addLiquidity(address(this), USDC, balanceOf(address(this)), IERC20(USDC).balanceOf(address(this)), 0, 0, owner(), block.timestamp);
    //     swapEnabled = true;
    //     _swapTokensAtAmount = 5e5 * (10**_decimals);
    //     tradingOpen = true;
    //     tradingActiveBlock = block.number;
    //     IERC20(_uniswapV2Pair).approve(address(_IDEXRouter), type(uint).max);
    // }
