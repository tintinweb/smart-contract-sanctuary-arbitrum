...
contract GenericBridgeFacet is IBlinQ, ReentrancyGuard, SwapperV2, Validatable {
    /// Storage ///

    bytes32 internal constant NAMESPACE = keccak256("com.blinq.facets.generic");
    ...
    event BridgeConfigUpdated(address indexed bridge, bool whitelist);

    /// External Methods ///

    /// @notice Register token and bridge
    /// @param bridge Address of bridge for asset
    /// @param whitelist true or false for whitelist
    function updateBridgeWhitelist(address bridge, bool whitelist) external {
        ...
        s.allowedBridges[bridge] = whitelist;

        emit BridgeConfigUpdated(bridge, whitelist);
    }

    /// @notice Bridges tokens via Hop Protocol
    /// @param _bridgeData the core information needed for bridging
    /// @param _genericData data specific to Hop Protocol
    function startBridgeTokensViaGeneric(
        IBlinQ.BridgeData memory _bridgeData,
        GenericData calldata _genericData
    )
        external
        payable
        nonReentrant
    ...
    /// @param _bridgeData the core information needed for bridging
    /// @param _genericData generic data for the bridging
    function _startBridge(
        IBlinQ.BridgeData memory _bridgeData,
        GenericData memory _genericData
    ) private {
        Storage storage s = getStorage();
		require(s.allowedBridges[_genericData.bridge], "Bridge not allowed");

        // Give approval to bridge tokens
        LibAsset.maxApproveERC20(
            IERC20(_bridgeData.sendingAssetId),
            address(_genericData.bridge),
            _bridgeData.minAmount
        );

        uint256 value = LibAsset.isNativeAsset(address(_bridgeData.sendingAssetId))
            ? _genericData.nativeFee + _bridgeData.minAmount
            : _genericData.nativeFee;

		(bool success, ) = _genericData.bridge.call{ value: value }(_genericData.data);
    ...
    function handleMessage(
        ...
        (IBlinQ.BridgeData memory bridgeData, GenericData memory genericData) = abi.decode(message, (IBlinQ.BridgeData, GenericData));
        require(LibAsset.getOwnBalance(bridgeData.sendingAssetId) >= bridgeData.minAmount, "Insufficient balance");
        _startBridge(bridgeData, genericData);
    }
}
