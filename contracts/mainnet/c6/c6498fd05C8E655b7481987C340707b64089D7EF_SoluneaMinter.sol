...
contract SoluneaMinter is IMinter {
  ...
  uint internal constant _START_BASE_WEEKLY_EMISSION = 90_000_00e18;
  ...
  function updatePeriod() external override {
    ...
    if (block.timestamp >= activePeriod && initializer == address(0)) {
      ...
      * _WEEKLY_EMISSION_DECREASE
      ...
      _safeTransfer(address(token), IController(controller).governance(), toGovernance);

      IVeDist veDist = _veDist();
      IVoter voter = _voter();

      _safeTransfer(address(token), address(veDist), _growth);
  ...
  function _safeTransfer(address paramToken, address to, uint256 value) internal {
        require(paramToken.code.length > 0);
        (bool success, bytes memory data) = paramToken.call(
