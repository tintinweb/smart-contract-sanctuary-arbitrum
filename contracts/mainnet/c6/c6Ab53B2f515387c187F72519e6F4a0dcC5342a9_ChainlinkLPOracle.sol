...
contract ChainlinkLPOracle is AggregatorV3Interface {
    ...
    // function priceFor(uint256 amount) external view returns (int256) {
    //     return (int256(amount) * _fetchPrice()) / 1e18;
    // }
    ...
    function _fetchPrice() internal view returns (uint256) {
        ...
        uint256 price18 = (uint256(totalInLPQuote()) * 1e18) / totalSupply;
        return
            uint256(_scalePriceByDigits(int256(price18), 18, oracleDecimals));
    }

    function totalInLPQuote() public view returns (int256) {
        return tokenAInLPQuote() + tokenBInLPQuote();
    }

    function tokenAInLPQuote() public view returns (int256) {
        int256 price = tokenAPrice();
        uint256 supply = tokenAInLP();

        int256 priceScaled = _scalePriceByDigits(
            int256(price),
            tokenAoracle.decimals(),
            8
        );

        int256 reserve0Scaled = _scalePriceByDigits(
            int256(supply),
            tokenA.decimals(),
            8
        );

        return (priceScaled * reserve0Scaled) / 1e8;
    }

    function tokenBInLPQuote() public view returns (int256) {
        int256 price = tokenBPrice();
        uint256 supply = tokenBInLP();

        int256 priceScaled = _scalePriceByDigits(
            int256(price),
            tokenBoracle.decimals(),
            8
        );
    ...
    function tokenAInLP() public view returns (uint256) {
        int256 price = tokenAPrice();
        (uint256 reserve0, ) = lp.getReserves();
        return reserve0;
    }
    ...
    function tokenBInLP() public view returns (uint256) {
        ...
        //     8
        // );
        // int256 reserve1Scaled = _scalePriceByDigits(
        //     int256(reserve1),
        //     tokenA.decimals(),
        //     8
        // );
        // return (priceScaled * reserve1Scaled) / 1e8;
    }

    function tokenAPrice() public view returns (int256) {
        return _fetchChainlinkPrice(tokenAoracle);
    }

    function tokenBPrice() public view returns (int256) {
        return _fetchChainlinkPrice(tokenBoracle);
    }
    ...
    function _scalePriceByDigits(
        ...
        if (_answerDigits >= _targetDigits) {
            ...
            price = _price / int256(10 ** (_answerDigits - _targetDigits));
        } else if (_answerDigits < _targetDigits) {
            // Scale the returned price value up to Liquity's target precision
            price = _price * int256(10 ** (_targetDigits - _answerDigits));
        }
        return price;
    }
