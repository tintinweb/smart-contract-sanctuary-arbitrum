...
contract MINU is ERC20, Ownable {
    ...
    constructor() ERC20("Milly Inu", "MINU") {
        marketingWallet = payable(0xbE13b6C7a2EE5807D7F277EfA3044aa902Add0Cb);
        devWallet = payable(0x49030C0d77402373c634B1b2cDC04067e1caae40);
...
contract DividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(
        ...
        defaultToken = token;
        MINUContract = MINU(mainContract);
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(MINUContract);

            uint256 prevBalance = MINUContract.balanceOf(address(this));
            ...
            {
                uint256 received = MINUContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MINUContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
