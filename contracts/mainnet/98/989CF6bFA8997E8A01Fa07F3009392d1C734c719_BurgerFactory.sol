...
contract BurgerPair is IERC20, IPair, Reentrancy {
  ...
  function mint(address to) external lock override returns (uint liquidity) {
    ...
    require(liquidity > 0, 'BurgerPair: INSUFFICIENT_LIQUIDITY_MINTED');
    _mint(to, liquidity);
  ...
  function burn(address to) external lock override returns (uint amount0, uint amount1) {
    ...
    require(amount0 > 0 && amount1 > 0, 'BurgerPair: INSUFFICIENT_LIQUIDITY_BURNED');
  ...
  function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override lock {
    require(!IFactory(factory).isPaused(), "BurgerPair: PAUSE");
    require(amount0Out > 0 || amount1Out > 0, 'BurgerPair: INSUFFICIENT_OUTPUT_AMOUNT');
    (uint _reserve0, uint _reserve1) = (reserve0, reserve1);
    require(amount0Out < _reserve0 && amount1Out < _reserve1, 'BurgerPair: INSUFFICIENT_LIQUIDITY');
    uint _balance0;
    uint _balance1;
    {// scope for _token{0,1}, avoids stack too deep errors
      (address _token0, address _token1) = (token0, token1);
      require(to != _token0 && to != _token1, 'BurgerPair: INVALID_TO');
    ...
    require(amount0In > 0 || amount1In > 0, 'BurgerPair: INSUFFICIENT_INPUT_AMOUNT');
    {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
      (address _token0, address _token1) = (token0, token1);
      if (amount0In > 0) _update0(amount0In * IFactory(factory).getFee(stable) / 10000); // accrue fees for token0 and move them out of pool
      if (amount1In > 0) _update1(amount1In * IFactory(factory).getFee(stable) / 10000); // accrue fees for token1 and move them out of pool
      // since we removed tokens, we need to reconfirm balances,
      // but doing balanceOf again as safety check
      _balance0 = IERC20(_token0).balanceOf(address(this));
      _balance1 = IERC20(_token1).balanceOf(address(this));
      // The curve, either x3y+y3x for stable pools, or x*y for volatile pools
      require(_k(_balance0, _balance1) >= _k(_reserve0, _reserve1), 'BurgerPair: K');
    }
  ...
  function getAmountOut(uint amountIn, address tokenIn) external view override returns (uint) {
    (uint _reserve0, uint _reserve1) = (reserve0, reserve1);
    amountIn -= amountIn * IFactory(factory).getFee(stable) / 10000; // remove fee from amount received
  ...
  function approve(address spender, uint amount) external override returns (bool) {
    require(spender != address(0), "BurgerPair: Approve to the zero address");
  ...
  function permit(
    ...
    require(deadline >= block.timestamp, 'BurgerPair: EXPIRED');
    ...
    require(recoveredAddress != address(0) && recoveredAddress == owner, 'BurgerPair: INVALID_SIGNATURE');
  ...
  function transferFrom(address src, address dst, uint amount) external override returns (bool) {
    ...
    if (spender != src && spenderAllowance != type(uint).max) {
      require(spenderAllowance >= amount, "BurgerPair: Insufficient allowance");
    unchecked {
  ...
  function _transferTokens(address src, address dst, uint amount) internal {
    require(dst != address(0), "BurgerPair: Transfer to the zero address");
    ...
    require(srcBalance >= amount, "BurgerPair: Transfer amount exceeds balance");
  unchecked {
