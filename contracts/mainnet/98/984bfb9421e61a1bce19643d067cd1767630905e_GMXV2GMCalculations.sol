...
import { IGMXV2GMCalculations } from "src/calculations/gmx-v2/interfaces/IGMXV2GMCalculations.sol";
import { IGMXV2GMEstimations } from "src/calculations/gmx-v2/interfaces/IGMXV2GMEstimations.sol";
import { IGMXV2GMStrategy } from "src/strategies/gmx-v2/interfaces/IGMXV2GMStrategy.sol";
import { IReader, IMarket } from "src/strategies/gmx-v2/interfaces/IGMXV2GM.sol";
...
/**
 * @title Dollet GMXV2GMCalculations contract
 * @author Dollet Team
 * @notice Contract for doing GMXV2GMStrategy calculations.
 */
contract GMXV2GMCalculations is Calculations, IGMXV2GMCalculations {
    using AddressUtils for address;

    bytes32 private constant MAX_PNL_FACTOR_FOR_DEPOSITS = keccak256(abi.encode("MAX_PNL_FACTOR_FOR_DEPOSITS"));

    IGMXV2GMEstimations public estimations;
    address public usd;
    ...
    /**
     * @notice Initializes this GMXV2GMCalculations contract.
     * @param _adminStructure AdminStructure contract address.
     * @param _usd USD token contract address (USDC/USDT/DAI/etc.).
     * @param _estimations Strategy estimations contract address.
     */
    function initialize(address _adminStructure, address _usd, address _estimations) external initializer {
        _calculationsInitUnchained(_adminStructure);
        _setUsd(_usd);
        _setEstimations(_estimations);
    }

    /// @inheritdoc IGMXV2GMCalculations
    ...
    /// @inheritdoc IGMXV2GMCalculations
    function setEstimations(address _newEstimations) external {
        _onlyAdmin();
        _setEstimations(_newEstimations);
    }

    /// @inheritdoc IGMXV2GMCalculations
    function estimateDepositExecutionFee(uint256 _gasPrice) external view returns (uint256) {
        return estimations.estimateDepositExecutionFee(_gasPrice, IGMXV2GMStrategy(strategy));
    }

    /// @inheritdoc IGMXV2GMCalculations
    function estimateWithdrawalExecutionFee(uint256 _gasPrice) external view returns (uint256) {
        return estimations.estimateWithdrawalExecutionFee(_gasPrice, IGMXV2GMStrategy(strategy));
    }
    ...
    function _userDeposit(address _user, address _token) internal view override returns (uint256) {
        return strategyHelper.convert(usd, _token, _convertWantToUsd(IStrategyV2(strategy).userWantDeposit(_user)));
    }
    ...
    function _totalDeposits(address _token) internal view override returns (uint256) {
        return strategyHelper.convert(usd, _token, _convertWantToUsd(IStrategyV2(strategy).totalWantDeposits()));
    }

    /**
     * @notice Estimates the want balance after a compound operation.
     * @dev This strategy doesn't support compound operation. Because of this current want token balance is returned.
     * @return The strategy current want tokens balance.
     */
    function _estimateWantAfterCompound(uint16, bytes memory) internal view override returns (uint256) {
        return IStrategyV2(strategy).balance();
    }
    ...
    /**
     * @notice Sets a new strategy estimations contract address.
     * @param _newEstimations A new strategy estimations contract address.
     */
    function _setEstimations(address _newEstimations) private {
        AddressUtils.onlyContract(_newEstimations);

        emit EstimationsSet(address(estimations), _newEstimations);

        estimations = IGMXV2GMEstimations(_newEstimations);
    }

    /**
     * @notice Retrieves the GMX V2 market token price from the GMX V2 Reader contract and downscales it to 1e18.
     * @return The GMX V2 market token price downscaled to 1e18.
     */
    function _getMarketTokenPrice() private view returns (uint256) {
        IGMXV2GMStrategy _strategy = IGMXV2GMStrategy(strategy);
        IReader _reader = _strategy.reader();
        address _dataStore = _strategy.dataStore();
        IMarket.Props memory _marketInfo = _reader.getMarket(_dataStore, IStrategyV2(address(_strategy)).want());
        (int256 _price,) = _reader.getMarketTokenPrice(
            _dataStore,
            _marketInfo,
            _strategy.priceToken(_marketInfo.indexToken),
            _strategy.priceToken(_marketInfo.longToken),
            _strategy.priceToken(_marketInfo.shortToken),
            MAX_PNL_FACTOR_FOR_DEPOSITS,
            false
        );

        // The market token price is returned as 1e30, we are downscaling it to 1e18
        return _price < 0 ? 0 : uint256(_price) / 1e12;
    }
    ...
    function _convertUsdToWant(uint256 _usdAmount) private view returns (uint256) {
        uint256 _gmTokenPrice = _getMarketTokenPrice();

        if (_gmTokenPrice == 0) return 0;

        // 1e36 = 1e18 (GM token price precision) + 1e18 (want token precision)
        return _usdAmount * 1e36 / _gmTokenPrice / (10 ** ERC20Upgradeable(usd).decimals());
    }
    ...
    function _convertWantToUsd(uint256 _wantAmount) private view returns (uint256) {
        // 1e36 = 1e18 (GM token price precision) + 1e18 (want token precision)
        return _getMarketTokenPrice() * _wantAmount * (10 ** ERC20Upgradeable(usd).decimals()) / 1e36;
    }
}
...
/**
 * @title Dollet GMXV2GMCalculations interface
 * @author Dollet Team
 * @notice An interface for GMXV2GMCalculations contract.
 */
interface IGMXV2GMCalculations {
    /**
    ...
    /**
     * @notice Event emmited when a new estimations contract is set.
     * @param _oldEstimations Old estimations contract address.
     * @param _newEstimations New estimations contract address.
     */
    event EstimationsSet(address _oldEstimations, address _newEstimations);
    ...
    /**
     * @notice Sets a new strategy estimations contract address by an admin.
     * @param _newEstimations A new strategy estimations contract address.
     */
    function setEstimations(address _newEstimations) external;

    /**
     * @notice Estimates fee (in ETH) for deposit execution.
     * @param _gasPrice A gas price that should be used for deposit transaction execution. Since the gas price value may
     *                  fluctuate based on blockchain usage, some buffer could be added to ensure that the creation
     *                  transaction does not revert.
     * @return A fee (in ETH) for deposit execution.
     */
    function estimateDepositExecutionFee(uint256 _gasPrice) external view returns (uint256);

    /**
     * @notice Estimates fee (in ETH) for withdrawal execution.
     * @param _gasPrice A gas price that should be used for withdrawal transaction execution. Since the gas price value
     *                  may fluctuate based on blockchain usage, some buffer could be added to ensure that the creation
     *                  transaction does not revert.
     * @return A fee (in ETH) for withdrawal execution.
     */
    function estimateWithdrawalExecutionFee(uint256 _gasPrice) external view returns (uint256);
}
...
/**
 * @title Dollet GMXV2GMEstimations interface
 * @author Dollet Team
 * @notice An interface for GMXV2GMEstimations contract.
 */
interface IGMXV2GMEstimations {
    /**
     * @notice Estimates fee (in ETH) for deposit execution.
     * @param _gasPrice A gas price that should be used for deposit transaction execution. Since the gas price value may
     *                  fluctuate based on blockchain usage, some buffer could be added to ensure that the creation
     *                  transaction does not revert.
     * @param _strategy Strategy contract address to estimate for.
     * @return A fee (in ETH) for deposit execution.
     */
    function estimateDepositExecutionFee(
        uint256 _gasPrice,
        IGMXV2GMStrategy _strategy
    )
        external
        view
        returns (uint256);
     ...
     * @return A fee (in ETH) for withdrawal execution.
     */
    function estimateWithdrawalExecutionFee(
        uint256 _gasPrice,
        IGMXV2GMStrategy _strategy
    )
        external
...
/**
 * @title Dollet CalculationsErrors library
 * @author Dollet Team
 * @notice Library with all Calculations Errors.
 */
library CalculationsErrors {
    error WantToWithdrawIsTooHigh();
...
interface IVault {
    ...
    /**
     * @notice Deposit to the strategy using permit.
     * @param _user Address of the user providing the deposit tokens.
     * @param _token Address of the token to deposit.
     * @param _amount Amount of tokens to deposit.
     * @param _additionalData Additional encoded data for the deposit.
