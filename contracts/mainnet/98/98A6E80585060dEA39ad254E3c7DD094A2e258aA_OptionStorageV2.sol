...
contract OptionStorageV2 is AccessControl {
    event UpdateRouter(address indexed router, string routerType);
    event UpdateIncentivePoolContract(address indexed incentivePoolContract);
    ...
    address public incentivePoolContract;
    mapping(address => mapping(uint256 => mapping(address => uint256)))
        public userVolume; // userVolume[user][weekId][tokenX]
    mapping(uint256 => mapping(address => uint256)) public weeklyVolume; // weeklyVolume[weekId][tokenX]
    mapping(string => RouterInfo) private routers;

    bytes32 public constant OPTION_ROLE = keccak256("OPTION_ROLE");

    function _setRouter(string memory routerType, address _router) private {
        routers[routerType] = RouterInfo({
            contractAddress: _router,
            isSet: true
        });
        emit UpdateRouter(_router, routerType);
    }

    constructor(
        address _upDownContract,
        address _aboveBelowContract,
        address _incentivePoolContract
    ) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setRouter("UpDown", _upDownContract);
        _setRouter("AboveBelow", _aboveBelowContract);
        incentivePoolContract = _incentivePoolContract;
    }

    function _getRouter(
        string memory routerType
    ) private view returns (address) {
        require(routers[routerType].isSet, "Router not set");
        return routers[routerType].contractAddress;
    }
    ...
    function save(
        ...
        address user
    ) external onlyRole(OPTION_ROLE) {
        IOptions optionsContract = IOptions(optionsContractAddress);
        (, , uint256 tradeSize, , , , , ) = optionsContract.options(optionId);
        uint256 weekId = getWeekId(block.timestamp);
        address tokenX = address(optionsContract.tokenX());
        userVolume[user][weekId][tokenX] += tradeSize;
        weeklyVolume[weekId][tokenX] += tradeSize;
        if (
            IERC20(IIncentivePool(incentivePoolContract).jackpotToken())
    ...
    function getWeekId(uint256 timestamp) internal view returns (uint256) {
        uint256 weekTimestamp = (timestamp - 6 * 86400 - 16 * 3600) /
            (86400 * 7);
    ...
    function triggerJackpot(
        ...
        address optionsContractAddress,
        uint256 tradeSize,
        IOptions optionsContract,
        address user,
        address tokenX
    ) internal {
        address router = address(0);
        if (incentivePoolContract != address(0)) {
            router = _getRouter("UpDown");
            if (
                !optionsContract.hasRole(optionsContract.ROUTER_ROLE(), router)
            ) {
                router = _getRouter("AboveBelow");
            }
            require(router != address(0), "Router not found");
            IIncentivePool(incentivePoolContract).executeIncentives(
                user,
                tradeSize,
                optionsContractAddress,
                optionId,
    ...
    function setUpDownContract(
        address _upDownContract
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setRouter("UpDown", _upDownContract);
    }

    function setAboveBelowContract(
        address _aboveBelowContract
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setRouter("AboveBelow", _aboveBelowContract);
    }
