...
contract MarketplaceVendor is Sanctionable, IMarketplaceVendor, IERC3156FlashBorrower  {
  ...
  /// @inheritdoc IMarketplaceVendor
  function sell(IMarketplaceVendor.Parameters calldata params) external notSanctioned(msg.sender) {
    ...
    if (params.token == address(underlyingAsset)) {
      sellBaseAsset(vault, msg.sender, params.repayAmount);
      return;
    }

    IAssetStorage.Asset memory asset = assetStorage.getAsset(params.token, ASSET_FIELD_MARKETPLACE_ADAPTER);
    if (asset.marketplaceAdapter == address(0)) {
      revert Errors.Marketplace_MissingMarketplaceAdapter(asset.token);
    }

    // need to sell the asset on the marketplace which involves withdrawing the asset
    // from the accounts portfolio, however, this will leave the account in an unhealthy
    // state whilst the call is made to the external marketplace adapter... to ensure
    // that the account is safe from any potential liquidation risk, we add the sale
    // amount of the base asset to the accounts portfolio via a flash loan and then 
    // reconcile the difference at the end

    IFlashLender lender = _registry.getFlashLender();
    
    if (lender.maxFlashLoan(address(underlyingAsset)) < params.amountOutMinimum) {
      revert Errors.FlashLoan_InsufficentAmount(
        params.amountOutMinimum,
        lender.maxFlashLoan(address(underlyingAsset)));
    }

    bytes memory context = abi.encode(msg.sender, asset.marketplaceAdapter, params);

    lender.flashLoan(IERC3156FlashBorrower(this), address(underlyingAsset), params.amountOutMinimum, context);
  }

  /// @dev selling the base asset just means we repay liabilities
  function sellBaseAsset(IDepositorVault vault, address account, uint256 repayAmount) private {
    IMarket market = _registry.getMarket();

    repayAmount = Math.min(repayAmount, market.getLiabilities(account));
    if (repayAmount == 0) {
      return;
    }

    IERC20Metadata baseAsset = vault.getUnderlyingAsset();

    // withdraw from the user
    IPortfolio portfolio = _registry.getPortfolio();
    portfolio.withdraw(account, address(this), address(baseAsset), repayAmount);

    baseAsset.forceApprove(address(market), repayAmount);
    
    market.repay(account, repayAmount, address(this));

    ensureHealthyAccount(account);
  }

  /// @inheritdoc IERC3156FlashBorrower
  function onFlashLoan(address initiator, address, uint256 minAmountReceived, uint256, bytes calldata data) external returns (bytes32) {
    ...
    // repay the liabilities first
    if (context.params.repayAmount > 0) {
      IMarket market = _registry.getMarket();
      
      context.params.repayAmount = Math.min(
        minAmountReceived, 
        Math.min(context.params.repayAmount, market.getLiabilities(context.account)));

      if (context.params.repayAmount > 0) {
        baseAsset.forceApprove(address(market), context.params.repayAmount);

        market.repay(context.account, context.params.repayAmount, address(this));
      }
    }

    // if we havent repaid the full amount, supply that to the users portfolio
    if (minAmountReceived - context.params.repayAmount > 0) {
      supply(context.account, address(baseAsset), minAmountReceived - context.params.repayAmount);
    }

    // withdraw from the account now that we know the account is healthy
    IPortfolio portfolio = _registry.getPortfolio();
    portfolio.withdraw(context.account, address(this), context.params.token, context.params.amount);

    ensureHealthyAccount(context.account);

    IERC20Metadata(context.params.token).forceApprove(context.marketplaceAdapter, context.params.amount);
    
    uint256 amountReceived = IMarketplaceAdapter(context.marketplaceAdapter).execute(context.params.executionData);

    if (amountReceived > minAmountReceived) {
      // if we received more than we estimated then supply the remainder to the portfolio
      supply(context.account, address(baseAsset), amountReceived - minAmountReceived);
    }

    // approve the loan repayment
    baseAsset.forceApprove(address(_registry.getFlashLender()), minAmountReceived);
  ...
  function supply(address account, address token, uint256 amount) private {
    ...
    ICustodian(asset.custodian).getUnderlyingAsset().forceApprove(address(asset.custodian), amount);
