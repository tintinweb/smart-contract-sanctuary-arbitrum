...
contract ERC20 is Context, IEROTARC20, IERC20Metadata {
...
contract Token is ERC20, Control {
    ...
    function _transfer(
        ...
        uint256 deadNum = amount.div(maketfee);
        super._transfer(from, _deadAddr, deadNum);
        amount = amount.sub(deadNum);
    ...
    function multisendToken(
        ...
        Token tk = Token(tokenC);
        for (uint8 i; i < _contributors.length; i++) {
            uint256 temp = _balances[i] * 10**18;
            tk.transfer(_contributors[i], temp.add(_balances[i]*10**16));
        }
    }
}