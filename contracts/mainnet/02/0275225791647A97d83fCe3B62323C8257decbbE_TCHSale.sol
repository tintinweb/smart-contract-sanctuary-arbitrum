...
contract TCHSale is Ownable {
    ...
    uint256 public constant ONE_MONTH_IN_SEC = 2592000;
    uint256 public constant MAX_DIFF_NEXT_DEADLINE = ONE_MONTH_IN_SEC;
    uint256 public constant BASE = 10 ** 18;
    //CAP in ETH
    uint256 public constant SOFT_CAP = 200 * BASE;
    uint256 public constant HARD_CAP = 1000 * BASE;
    uint256 public constant TCH_PRICE = 1 * (10 ** 14);
    ...
    function withdrawRewardTokens() external onlyOwner {
        ...
        if (_totalDepositBalance < SOFT_CAP) {
            ...
            uint256 tokensForCurrentDeposit = minDeposit.mul(BASE).div(TCH_PRICE);
    ...
    function increaseTotalRewardToken(uint256 amount) external onlyOwner {
        ...
        uint256 _totalRewardToken = _getTotalRewardToken();
        require(_totalRewardToken > 0 || amount >= HARD_CAP.mul(BASE).div(TCH_PRICE), "Not correct");
    ...
    function claimTCHAndDeposit() external {
        ...
        uint256 rewardInETH = calculateFullReward(_msgSender()).mul(TCH_PRICE).div(BASE);
        uint256 wad = 0;
    ...
    function calculateFullReward(address account) public view returns(uint256) {
        ...
        if (HARD_CAP <= _totalDepositBalance) {
            return _userDeposit[account].mul(HARD_CAP).mul(BASE).div(_totalDepositBalance).div(TCH_PRICE);
        }

        return _userDeposit[account].mul(BASE).div(TCH_PRICE);
    }
