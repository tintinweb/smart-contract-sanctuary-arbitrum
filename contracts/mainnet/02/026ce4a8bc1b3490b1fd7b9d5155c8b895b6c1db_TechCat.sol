...
abstract contract CaptureTheFlag is Ownable, ReentrancyGuard {
    ...
    constructor(address _tokenAddress, uint256 _prize) {
    ...
    function captureFlag() external nonReentrant {
        ...
        require(token.balanceOf(msg.sender) >= capturePrice, "Insufficient tokens");

        // Transfer tokens from sender to contract
        token.transferFrom(msg.sender, address(this), capturePrice);
    ...
    function deleteData() external onlyOwner {
        ...
        for (uint256 i = 0; i < flagHolders.length; i++) {
...
contract TechCat is ERC20, CaptureTheFlag {
    ...
    constructor(uint256 _prize) ERC20("TechCat", "STC") CaptureTheFlag(address(this), _prize) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        require(cantrade || _nofee[from] || _nofee[to], "Trading is not allowed");
