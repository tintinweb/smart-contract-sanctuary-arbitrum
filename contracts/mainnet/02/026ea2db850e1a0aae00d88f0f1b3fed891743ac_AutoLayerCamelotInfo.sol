...
contract AutoLayerCamelotInfo is Ownable{
    ...
    struct TotalAmountPool {
        uint256 total0;
        uint256 total1;
    }

    struct PoolResult{
        TotalAmountPool totalAmounts;
        uint256 totalSupply;
        uint256 userLPs;
        address poolAddress;
        address nftContract;
   ...
   function getTotalAmounts() public view returns (TotalAmountPool[] memory) {
        TotalAmountPool[] memory totalAmounts = new TotalAmountPool[](pools.length);
        for (uint256 i = 0; i < pools.length; i++) {
            (uint256 total0, uint256  total1) = IHipervisor(pools[i].poolAddress).getTotalAmounts();
            TotalAmountPool memory totalAmount = TotalAmountPool({
                total0: total0,
                total1: total1
            });
    ...
    function getUserLPs(address user) public view  returns (uint256[] memory) {
    ...
    function getCamelotPoolsData (address user) public view  returns(PoolResult[] memory) {
        uint256[] memory totalSupplies = getTotalSupply();
        TotalAmountPool[] memory totalAmounts = getTotalAmounts();
