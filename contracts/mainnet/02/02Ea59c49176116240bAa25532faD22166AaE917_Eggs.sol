...
contract Eggs is ERC20Burnable, Ownable, IEGGS {
	...
	function rebase() public onlyMinter {
		...
		if (lastRebaseTimestamp >= block.timestamp) {
			return;
		}
		uint timeInterval = block.timestamp - lastRebaseTimestamp;
		uint factor = getFactor(timeInterval);
	...
	function setLastRebaseTimestamp(uint _lastRebaseTimestamp) public onlyOwner {
		lastRebaseTimestamp = _lastRebaseTimestamp;
	}
}