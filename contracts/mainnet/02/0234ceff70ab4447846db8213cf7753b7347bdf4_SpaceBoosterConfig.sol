...
interface ISpaceSwapNFT is IERC721Upgradeable, IERC721MetadataUpgradeable, IERC721EnumerableUpgradeable {
  // getter

  function spaceNames(uint256 tokenId) external view returns (string calldata);
  ...
  function spaceswapNFTToCategory(uint256 tokenId) external view returns (uint256);

  function categoryToSpaceSwapNFTList(uint256 categoryId) external view returns (uint256[] memory);
  ...
  function getSpaceNameOfTokenId(uint256 tokenId) external view returns (string memory);
...
interface ISpaceBoosterConfig {
  ...
  function spaceboosterNftAllowance(
...
contract SpaceBoosterConfig is ISpaceBoosterConfig, Ownable, ReentrancyGuard {
  using SafeERC20 for IERC20;
  using SafeMath for uint256;

  struct SpaceBoosterNFTInfo {
    address nftAddress;
    uint256 tokenId;
  }

  struct SpaceBoosterEnergyInfo {
    uint256 maxEnergy;
    uint256 currentEnergy;
    uint256 boostBps;
    uint256 updatedAt;
  ...
  struct SpaceBoosterAllowance {
    address nftAddress;
    uint256 nftTokenId;
    bool allowance;
  }

  struct SpaceBoosterAllowanceParams {
    address stakingToken;
    SpaceBoosterAllowance[] allowance;
  }
  ...
  mapping(address => mapping(uint256 => SpaceBoosterEnergyInfo)) public spaceboosterEnergyInfo;
  ...
  mapping(address => mapping(address => mapping(uint256 => bool))) public spaceboosterNftAllowanceConfig;
  ...
  event SetSpaceBoosterNFTEnergyInfo(
    address indexed nftAddress,
    uint256 indexed nftTokenId,
    uint256 maxEnergy,
    uint256 currentEnergy,
  ...
  event SetSpaceBoosterNFTAllowance(
  ...
  modifier onlyCaller() {
    require(callerAllowance[msg.sender], "SpaceBoosterConfig::onlyCaller::only eligible caller");
    _;
  }

  /// @notice getter function for energy info
  /// @dev check if the spacebooster energy existed,
    ...
    )
  {
    ...
    // if there is no preset spacebooster energy info, use preset in category info
    ...
    SpaceBoosterEnergyInfo memory spaceboosterInfo = spaceboosterEnergyInfo[_nftAddress][_nftTokenId];
    // if there is no preset spacebooster energy info, use preset in category info
    // presume that it's not a preminted nft
    if (spaceboosterInfo.updatedAt == 0) {
      uint256 categoryId = ISpaceSwapNFT(_nftAddress).spaceswapNFTToCategory(_nftTokenId);
    ...
    return (spaceboosterInfo.maxEnergy, spaceboosterInfo.currentEnergy, spaceboosterInfo.boostBps);
  }
  ...
  function consumeEnergy(
    ...
    require(_nftAddress != address(0), "SpaceBoosterConfig::consumeEnergy::_nftAddress must not be address(0)");
    SpaceBoosterEnergyInfo storage energy = spaceboosterEnergyInfo[_nftAddress][_nftTokenId];

    if (energy.updatedAt == 0) {
      uint256 categoryId = ISpaceSwapNFT(_nftAddress).spaceswapNFTToCategory(_nftTokenId);
      CategoryEnergyInfo memory categoryEnergy = categoryEnergyInfo[_nftAddress][categoryId];
      require(categoryEnergy.updatedAt != 0, "SpaceBoosterConfig::consumeEnergy:: invalid nft to be updated");
  ...
  /// @param _param a SpaceBoosterNFTParams {nftAddress, nftTokenId, maxEnergy, boostBps}

  function setExternalTokenEnergyInfo(address _externalNft, CategoryEnergyInfo calldata _param) external onlyOwner {
    require(_externalNft != address(0), "SpaceBoosterConfig::externalNft::_externalNft must not be address(0)");
  ...
  function setStakeTokenAllowance(address _stakeToken, bool _isAllowed) external onlyOwner {
    require(_stakeToken != address(0), "SpaceBoosterConfig::setStakeTokenAllowance::_stakeToken must not be address(0)");
  ...
  function setCallerAllowance(address _caller, bool _isAllowed) external onlyOwner {
    require(_caller != address(0), "SpaceBoosterConfig::setCallerAllowance::_caller must not be address(0)");
    callerAllowance[_caller] = _isAllowed;

    emit SetCallerAllowance(_caller, _isAllowed);
  }

  /// @notice A function for setting spacebooster NFT energy info as a batch
  /// @param _params a list of SpaceBoosterNFTParams [{nftAddress, nftTokenId, maxEnergy, boostBps}]
  function setBatchSpaceBoosterNFTEnergyInfo(SpaceBoosterNFTParams[] calldata _params) external onlyOwner {
    for (uint256 i = 0; i < _params.length; ++i) {
      _setSpaceBoosterNFTEnergyInfo(_params[i]);
    }
  }

  /// @notice A function for setting spacebooster NFT energy info
  /// @param _param a SpaceBoosterNFTParams {nftAddress, nftTokenId, maxEnergy, boostBps}
  function setSpaceBoosterNFTEnergyInfo(SpaceBoosterNFTParams calldata _param) external onlyOwner {
    _setSpaceBoosterNFTEnergyInfo(_param);
  }

  /// @dev An internal function for setting spacebooster NFT energy info
  /// @param _param a SpaceBoosterNFTParams {nftAddress, nftTokenId, maxEnergy, boostBps}
  function _setSpaceBoosterNFTEnergyInfo(SpaceBoosterNFTParams calldata _param) internal {
    spaceboosterEnergyInfo[_param.nftAddress][_param.nftTokenId] = SpaceBoosterEnergyInfo({
      maxEnergy: _param.maxEnergy,
    ...
    emit SetSpaceBoosterNFTEnergyInfo(
      _param.nftAddress,
      _param.nftTokenId,
      _param.maxEnergy,
      _param.maxEnergy,
  ...
  function setStakingTokenCategoryAllowance(CategoryAllowanceParams calldata _param) external onlyOwner {
    for (uint256 i = 0; i < _param.allowance.length; ++i) {
      require(
        ...
        "SpaceBoosterConfig::setStakingTokenCategoryAllowance:: bad staking token"
      );
  ...
  /// @param _param a SpaceBoosterAllowanceParams {stakingToken, [{nftAddress, nftTokenId,allowance;}]}
  function setStakingTokenSpaceBoosterAllowance(SpaceBoosterAllowanceParams calldata _param) external onlyOwner {
    for (uint256 i = 0; i < _param.allowance.length; ++i) {
      require(
        stakeTokenAllowance[_param.stakingToken],
        "SpaceBoosterConfig::setStakingTokenSpaceBoosterAllowance:: bad staking token"
      );
      spaceboosterNftAllowanceConfig[_param.stakingToken][_param.allowance[i].nftAddress][
        _param.allowance[i].nftTokenId
      ] = _param.allowance[i].allowance;

      emit SetSpaceBoosterNFTAllowance(
        _param.stakingToken,
        _param.allowance[i].nftAddress,
        _param.allowance[i].nftTokenId,
        _param.allowance[i].allowance
  ...
  /// @dev if not support when checking with token, need to try checking with category level (categoryNftAllowanceConfig) as well since there should not be spaceboosterNftAllowanceConfig in non-preminted nft
  function spaceboosterNftAllowance(
    address _stakeToken,
    address _nftAddress,
    uint256 _nftTokenId
  ) external view override returns (bool) {
    if (!spaceboosterNftAllowanceConfig[_stakeToken][_nftAddress][_nftTokenId]) {
      CategoryEnergyInfo memory externalboosterInfo = externalboosterEnergyInfo[_nftAddress];
