...
contract JonesSSOVCallV3Strategy is JonesSSOVV3StrategyBase {
     ...
     */
    function sellRewardTokensForBaseToken(address[] memory _rewardTokens, uint256[] memory _minBaseOutputAmounts)
        public
        onlyRole(KEEPER)
    {
        ...
        for (uint256 i = 0; i < _rewardTokens.length; i++) {
            ...
            IERC20(rewardToken).safeApprove(address(sushiRouter), IERC20(rewardToken).balanceOf(address(this)));
        }

        sushiRouter.sellTokens(_minBaseOutputAmounts, _rewardTokens, address(this), routes);
    }
