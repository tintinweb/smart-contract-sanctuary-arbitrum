...
contract PerpetualLiquidator is PerpetualRebalanceFunctions, IFunctionList, IPerpetualLiquidator {
    ...
    uint16 internal constant PROTOCOL_FEE_ON_LIQUIDATION_TBPS = 20;
    ...
    function _liquidateByAMM(
        ...
        address _liquidatorAddr,
        address _traderAddr
    ) internal updateFundingAndPrices(_perpetual.id) returns (int128 fLiquidatedAmount) {
        if (_getMarginViewLogic().isMaintenanceMarginSafe(_perpetual.id, _traderAddr)) {
        ...
        }
        {
            ...
            if (fTraderPos == fLiquidatedAmount.neg()) {
                // trader was fully liquidated, set position to zero
                account.fPositionBC = 0;
                account.fLockedInValueQC = 0;
                if (account.fCashCC == 0) {
                    // account.fCashCC<0 -> remains active
    ...
    {
        fPrice = _getPerpetualMarkPrice(_perpetual, false);
        {
            ...
            if (fRemainingMargin.sub(fPenaltyCC) > 0) {
                ...
                fRealizedPnL = fRealizedPnL.sub(
                    _getTradeLogic().distributeFeesLiquidation(
                        ...
                        _traderAddr,
                        _fLiquidatedAmount,
                        PROTOCOL_FEE_ON_LIQUIDATION_TBPS
                    )
                );
            }
        }
    ...
    function _payLiquidationPenalty(
        ...
        int128 fPenaltyCC;
        {
            int128 fB2C = _getBaseToCollateralConversionMultiplier(_perpetual, false, false);
        ...
        returns (int128)
    {
        ...
        int128 fS3 = _getCollateralToQuoteConversionMultiplier(_perpetual, false);
        ...
        int128 fTotalFeeRate = int128(
            uint128(PROTOCOL_FEE_ON_LIQUIDATION_TBPS) +
    ...
    function _getBaseToQuoteConversionMultiplier(
        ...
        if (_isMarkPriceRequest) {
            return _getPerpetualMarkPrice(_perpetual, false);
        } else {
            return _perpetual.fSettlementS2PriceData;
        }
    }
