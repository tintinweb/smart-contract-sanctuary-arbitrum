...
import {IVault, IERC20 as BALANCER_ERC20} from '@balancer-labs/v2-interfaces/vault/IVault.sol';
import {IManagedPool} from '@balancer-labs/v2-interfaces/pool-utils/IManagedPool.sol';
import {IAsset} from '@balancer-labs/v2-interfaces/vault/IAsset.sol';
...
contract BalancerRouter is IBalancerRouter {
    ...
    function dustEstimatingInvest(
        ...
        for (uint i = 0; i < wrapRequests.length; ++i) {
            ...
            address[] memory _dustTokens = IWrapper(wrapRequest.wrapper).depositTokens();
        ...
        sharesAdded = invest(gauge, tokenIn, amountIn, wrapRequests, minShareAmount, deadline);
    ...
    function _createJoinPoolRequest(address pool) private returns (IVault.JoinPoolRequest memory joinPoolRequest) {
        ...
        bytes memory data = abi.encode(WeightedPoolUserData.JoinKind.EXACT_TOKENS_IN_FOR_BPT_OUT, amountsWithoutBpt, 0);
...
interface IWrapper is IERC20, IAccessManaged {
    ...
    function lp() external returns (uint lpAmount);
