...
contract MembershipNFT is ERC721Drop {
    using Strings for uint256;

    uint256 public membershipDurationBlocks;
    mapping(uint256 => uint256) private _membershipStartBlocks;
    ...
    function setMembershipDurationBlocks(uint256 _durationBlocks) public {
        require(msg.sender == owner(), "Only the contract owner can update the duration");
        membershipDurationBlocks = _durationBlocks;
    }
        ...
        returns (uint256 startTokenId)
    {
        ...
        for(uint256 i = 0; i < _quantityBeingClaimed; i++) {
            ...
            _membershipStartBlocks[tokenId] = block.number;
    ...
    function isMembershipActive(uint256 tokenId) public view returns (bool) {
        if (_membershipStartBlocks[tokenId] == 0) {
            return false;
        }
        return block.number < _membershipStartBlocks[tokenId] + membershipDurationBlocks;
    }

    function getMembershipStartBlock(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "Membership: Query for nonexistent token");
        return _membershipStartBlocks[tokenId];
    }

    function getMembershipEndBlock(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "Membership: Query for nonexistent token");
        uint256 startBlock = _membershipStartBlocks[tokenId];
        return startBlock > 0 ? startBlock + membershipDurationBlocks : 0;
    }
