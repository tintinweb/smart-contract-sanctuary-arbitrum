...
contract SAClientERC20 is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    mapping(address => uint216) internal transferAmountMax;
    ...
    function setTransferAmountMax(address token, uint216 amount) external onlyOwner {
        transferAmountMax[token] = amount;
    }
    ...
    function _calcFee(address contractAddress, uint256 actionId, address tokenAddress, uint216 amount) internal view returns (uint216 fee) {
        require(amount <= transferAmountMax[tokenAddress], "Amount exceeds max");
