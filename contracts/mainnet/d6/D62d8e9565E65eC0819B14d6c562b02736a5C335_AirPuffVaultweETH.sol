...
abstract contract AirPuffVault is IVault, OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC20BurnableUpgradeable {
    ...
    function executeClosePosition(
        ...
        if (!cpr.isLiquidation) {
            ...
            if (amountOut >= _userInfo.leverageAmount) {
                toLending = _userInfo.leverageAmount;
                uint256 leftovers = amountOut - _userInfo.leverageAmount;
                uint256 liquidatorReward = leftovers * feeConfiguration.liquidatorsRewardPercentage / MAX_BPS;
                IERC20Upgradeable(kcd.posBorrow).safeTransfer(_userInfo.liquidator, liquidatorReward);
                IERC20Upgradeable(kcd.posBorrow).safeTransfer(_userInfo.user, leftovers - liquidatorReward);
            } else {
                toLending = amountOut;
            }

            IERC20Upgradeable(kcd.posBorrow).safeIncreaseAllowance(kcd.lenVault, toLending);
            ILending(kcd.lenVault).repayDebt(_userInfo.leverageAmount, toLending);
    ...
    */
    function liquidatePosition(uint256 _positionID, address _user) external nonReentrant {
        ...
        emit Liquidated(_user, _positionID, msg.sender, cpr.closeRequestID, cpr.lendingRepayment, block.timestamp);
    }
...
contract AirPuffVaultweETH is AirPuffVault {
