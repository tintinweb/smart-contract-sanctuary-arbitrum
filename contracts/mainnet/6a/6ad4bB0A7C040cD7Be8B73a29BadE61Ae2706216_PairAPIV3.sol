...
interface IDysonVault {
    ...
    function getLiquidity(address) external view returns (uint,uint);
}
contract PairAPIV3 is Initializable {
    ...
    function getAllCLPair(address _user, uint _amounts, uint _offset) external view returns(pairInfo[] memory Pairs){
        ...
        for(i; i < _offset + _amounts; i++){
            ...
            if (_pair != address(0xd416063128bf5A81D20547adaDf29b74440D6699)) {
    ...
    function _pairAddressToCLInfo(address _pair, address _account) internal view returns(pairInfo memory _pairInfo) {
        ...
        address _clPool = IDysonVault(_pair).pool();
        address _strategy = IDysonVault(_pair).strategy();
        
        (_pairInfo._a0Expect, _pairInfo._a1Expect) = IDysonVault(address(0xb26Bb0F38064798aC6a1e15679F2640361DF66e0)).getLiquidity(_strategy);

        _pairInfo.dysonStrategy = _strategy;
        _pairInfo.clPool = _clPool;
        _pairInfo.dysonPool = _pair;
        _pairInfo.feeAmount = IDysonVault(_clPool).fee();
