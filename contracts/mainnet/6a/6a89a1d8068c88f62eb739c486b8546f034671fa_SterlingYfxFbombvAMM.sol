...
// File contracts/interfaces/ISterlingGauge.sol
...
// File contracts/SterlingYfxFbombvAMM.sol

//SPDX-License-Identifier: MIT

pragma solidity 0.8.17;
contract SterlingYfxFbombvAMM is Ownable, Pausable {
    ...
    address public constant fbomb = address(0x74ccbe53F77b08632ce0CB91D3A545bF6B8E0979);
    address public constant yfx = address(0xaaE0c3856e665ff9b3E2872B6D75939D810b7E40);
    address public want;
    ...
    ISterlingRouter.Routes[] public strToYfxPath;
    ISterlingRouter.Routes[] public strToFbombPath;
    ...
    constructor(
        ...
        ISterlingRouter.Routes[] memory _strToYfxPath,
        ISterlingRouter.Routes[] memory _strToFbombPath
    ) {
        ...
        for (uint i; i < _strToYfxPath.length; ++i) {
           strToYfxPath.push(_strToYfxPath[i]);
        }

        for (uint i; i < _strToFbombPath.length; ++i) {
           strToFbombPath.push(_strToFbombPath[i]);
        }
   ...
   //Withdraws funds from masterchef
    ...
    function chargeFees(address caller) internal {
        uint256 toWrapped = IERC20(str).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;

        ISterlingRouter(router).swapExactTokensForTokens(toWrapped, 0, strToWethPath, address(this), block.timestamp);


        uint256 wethBal = IERC20(weth).balanceOf(address(this));

        uint256 callFee = wethBal * CALL_FEE / FEE_DIVISOR;
        IERC20(weth).safeTransfer(caller, callFee);

        uint256 recipientFee = wethBal * RECIPIENT_FEE / FEE_DIVISOR;
        IERC20(weth).safeTransfer(grimFeeRecipient, recipientFee);

        uint256 treasuryFee = wethBal * TREASURY_FEE / FEE_DIVISOR;
        IERC20(weth).safeTransfer(treasury, treasuryFee);

        uint256 stratFee = wethBal * STRAT_FEE / FEE_DIVISOR;
        IERC20(weth).safeTransfer(strategist, stratFee);
    }

    // Converts to both sides of the LP token and builds the liquidity pair
    function addLiquidity() internal {
        uint256 strHalf = IERC20(str).balanceOf(address(this)) / 2;
        ISterlingRouter(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(strHalf, 0, strToYfxPath, address(this), block.timestamp);
        ISterlingRouter(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(strHalf, 0, strToFbombPath, address(this), block.timestamp);

        uint256 t1Bal = IERC20(yfx).balanceOf(address(this));
        uint256 t2Bal = IERC20(fbomb).balanceOf(address(this));

        ISterlingRouter(router).addLiquidity(yfx, fbomb, stable, t1Bal, t2Bal, 1, 1, address(this), block.timestamp);
    }
    ...
    function _subAllowance() internal {
        ...
        IERC20(fbomb).safeApprove(router, 0);
        IERC20(yfx).safeApprove(router, 0);
    ...
    function _addAllowance() internal {
        ...
        IERC20(fbomb).safeApprove(router, type(uint).max);
        IERC20(yfx).safeApprove(router, type(uint).max);
    }
