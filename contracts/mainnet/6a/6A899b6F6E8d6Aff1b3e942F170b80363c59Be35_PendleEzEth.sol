...
abstract contract BaseStrategyPendle {
    ...
    function __Base_Strategy_Initialize(address _vault, address _strategist, address _rewards, address _keeper) internal {
        ...
        keeper = _keeper;
        creditThreshold = 1_000_000 * 10 ** ERC20(address(vault.vaultToken())).decimals(); // set this high by default so we don't get tons of false triggers if not changed
    }
...
pragma solidity 0.8.23;
import {PendleGeneral, PendleGeneralInfo} from "../strategies/PendleGeneral.sol";
...
contract PendleLogic {
    ...
    IPendleGeneral public pendleLocusModule;
    ...
    constructor(address _pendleGeneralInfo, address _dataFeed){
        pendleLocusModule = IPendleGeneral(_pendleGeneralInfo);
        dataFeed = AggregatorV3Interface(_dataFeed);
    }

    function ytToWant(uint256 ytAmount) public view returns(uint256){
        return (ytAmount * assetToWantRate(10 ** IERC20Metadata(pendleLocusModule.mainInfo().underlyingAsset).decimals()) * 10 ^ 18 / pendleLocusModule.mainInfo().syToken.exchangeRate() - pendleLocusModule.mainInfo().ptOracle.getPtToAssetRate(address(pendleLocusModule.mainInfo().market), TWAP_RANGE_SECS)/10**18);
    }

    function lpToWant(uint256 lpAmount) public view returns(uint256){
        return assetToWantRate(pendleLocusModule.mainInfo().ptOracle.getLpToAssetRate(address(pendleLocusModule.mainInfo().market), TWAP_RANGE_SECS) * lpAmount);
    }
...
//  1 ez eth = answer WETH
    ...
    function assetToWantRate(uint256 assetAmount) public view  returns (uint256) {
        ...
        return ezETHToWantRate * assetAmount;
    }
    ...
    function getRewardsInWantToken() public view returns (uint256 amount) {
        (, address[] memory bonusRewardTokens, , uint256[] memory bonusTokensAmounts) = pendleLocusModule.mainInfo().penpieMaster.allPendingTokens(pendleLocusModule.mainInfo().lpToken, msg.sender);
        uint256 len = bonusRewardTokens.length;
        for (uint256 i; i < len; i++) {
            if (bonusRewardTokens[i] == ARB) {
                amount += smthToSmth(pendleLocusModule.mainInfo().poolArbToWeth, ARB, WETH, bonusTokensAmounts[i]);
            } else if(bonusRewardTokens[i] == PENDLE){
                amount += smthToSmth(pendleLocusModule.mainInfo().poolPendleToWeth, PENDLE, WETH, bonusTokensAmounts[i]);
            }
        }
    }
}
...
contract PendleGeneral is BaseStrategyPendle {
    ...
    function adjustPosition(uint256 _debtOutstanding, OffChainData calldata _offChainData) internal override {
        ...
        if (excessWant > 0) {
            ...
            (uint256 lpOut, , , ) = mainInfo.pendleRouter.addLiquiditySingleTokenKeepYt(address(this), address(mainInfo.market), lpExpected, ytExpected, inputData);
    ...
    function _swapAssets(address _tokenIn, address _tokenOut, uint24 _fee, uint256 amount, address pool) internal {
        ...
        params.deadline = block.timestamp;
        params.amountIn = amount;
        params.amountOutMinimum = pendleLogic.smthToSmth(pool, _tokenIn, _tokenOut, amount) * mainInfo.slippage / 10000;
        params.sqrtPriceLimitX96 = 0;
    ...
    function _exitPosition(uint256 _stakedAmountInWant, OffChainData calldata _offChainData) internal {
        uint256 lpAmount = _offChainData.WantToLpYtProportion[0] * _stakedAmountInWant / 1e18;
        uint256 ytAmount = _offChainData.WantToLpYtProportion[1] * _stakedAmountInWant / 1e18;
