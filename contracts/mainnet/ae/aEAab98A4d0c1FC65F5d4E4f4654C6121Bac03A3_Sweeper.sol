...
contract Sweeper is IERC721Receiver, ERC1155Holder {
    ...
    uint256 feePerListingBuy = 0.4 ether;
    ...
    function buyMany(
        ...
        for(uint i = 0; i < _nftAddress.length; i++) {
            ...
            if (approved == MarketplaceContract.TokenApprovalStatus.ERC_721_APPROVED) {
                ...
                IERC1155Upgradeable(_nftAddress[i]).safeTransferFrom(address(this), msg.sender, _tokenId[i], _quantity[i], bytes(""));
            } else {
