...
contract VestingFacet is WithOwnership, WithStorage, ReentrancyGuard {
    ...
    function pauseVesting(bool _pause) external {
        LibDiamond.enforceIsContractOwner();

        if (_pause == true) {
            _vs().pausedAt = block.timestamp;
        } else {
            // calculate time since pause
            _vs().totalPauseTime = block.timestamp - _vs().pausedAt;
            _vs().pausedAt = 0;
        }
        _vs().paused = _pause;
    }
    ...
    function createVestingSchedule(
        ...
        IHamachi hamachi = IHamachi(_vs().token);

        hamachi.updateRewardBalance(
            _beneficiary,
            hamachi.balanceOf(_beneficiary)
        );
    }
    ...
    function _computeReleasableAmount(
        ...
        uint256 timeSincePause = 0;
        if (_vs().paused) {
            timeSincePause = block.timestamp - _vs().pausedAt; 
        }
        
        uint256 correctedTime = block.timestamp - _vs().totalPauseTime - timeSincePause;
        uint256 startWithCliff = vestingSchedule.start + vestingSchedule.cliff;
        if (correctedTime < startWithCliff) {
            return 0;
        } else if (correctedTime >= startWithCliff + vestingSchedule.duration) {
            return vestingSchedule.amountTotal - vestingSchedule.released;
        } else {
            uint256 timeFromEndCliff = correctedTime - startWithCliff;
...
interface IHamachi is IERC20Upgradeable {
    function updateRewardBalance(address account, uint256 balance) external;
}
