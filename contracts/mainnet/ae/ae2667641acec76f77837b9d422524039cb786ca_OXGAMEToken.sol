...
contract OXGAMEToken is ERC20, ERC20Burnable, Pausable, Ownable {
    using SafeMath for uint256;
    uint256 private _maxCap = 1000000000 * 10**decimals();
    uint256 public constant D_ZERO = 10000;
    uint256 public sellTax; 
    uint256 public buyTax;
    address public feeWallet = 0x95dDF67D893A9FCAB56D637323E6bE25580d2da5;
    mapping(address => bool) public lps;
    mapping(address => bool) public blacklisted;
    mapping(address => bool) public whitelisted;
    event SetLiquidPair(address LP, bool Status);

    uint256 public prizePool = 0 * 10**18; // 
    uint256 public teamSupply = 200000000 * 10**18; 
    uint256 public initialMintAmount = 8000 * 10**18; 
    uint256 public investAmount = 800 * 10**18; // 2m
    uint256 public clipCost = 100000 * 10**18; // 25000k
    uint256 public remainmintnum = 1000000000 * 10**18; 
    // uint256 public value = 0.001 * 10**18; 


    uint256 public lastClipUpdate;
    address public clipOwner;
    address private _fund = 0x95dDF67D893A9FCAB56D637323E6bE25580d2da5;
    string public clip = "[email protected]";
    ...
    constructor() ERC20('OX GAME TOKEN', 'OX') {
        sellTax = 500;
        buyTax = 500;
    ...
    function mintOX(address invester) payable external {

        require(msg.value == 0.0005 ether, "Please send 0.0005 ether as gas fee");
        uint256 mintAmount;
     ...
     function setOX(string memory message) external maxLength(message) {
        require(bytes(message).length > 0, "Message cannot be empty");
        if (msg.sender != clipOwner) {
            require(
                balanceOf(msg.sender) >= clipCost,
                "Insufficient OX to set OX"
            );
    ...
    function claimPrizePool() external {
        require(
            ...
            "Prizepool can be claimed if 3 days have passed without a OX update"
        );
        require(
