...
contract Q is ReentrancyGuard {
    ...
    event ChangeTraderAccount(address indexed oldTrader, address indexed newTrader, address indexed traderAccount);
    ...
    function execute(uint256 command, bytes calldata data, bool isOpen) public payable onlyAdmin {
        (address stvId,) = _getAmountAndStvId(data);
    ...
    function multiExecute(
        ...
        for (; i < data.length;) {
            ...
            (address stvId,) = _getAmountAndStvId(tradeData);

            bytes memory perpTradeData =
                abi.encodeWithSignature("execute(uint256,bytes,bool,address)", command, tradeData, openOrClose, stvId);
