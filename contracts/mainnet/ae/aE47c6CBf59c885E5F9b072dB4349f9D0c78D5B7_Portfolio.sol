...
uint8 constant FLAGS_BEFORE_SUPPLY = 0x01;
uint8 constant FLAGS_AFTER_SUPPLY = 0x02;
uint8 constant FLAGS_BEFORE_WITHDRAW = 0x04;
uint8 constant FLAGS_AFTER_WITHDRAW = 0x08;

interface IPortfolioHooks {

  struct PortfolioHookParams {
    address account;
    address token;
    uint256 amount;
    uint256 shares;
  ...
  /**
   * @notice Returns a keccak256 hash to indicate the hook implementation for other runtime querying.
   * 
   * @return bytes32 The keccak256 hash that represents the type of hook.
   */
  function getType() external view returns (bytes32);

  /**
   * @notice Called before an asset has been supplied to the portfolio.
   * 
   * @param params The parameters that the operation is being executed with.
   * 
   * @return bytes4 The function selector for the hook.
   */
  function beforeSupply(PortfolioHookParams memory params) external returns (bytes4);

  /**
   * @notice Called after an asset has been supplied to the portfolio.
   * 
   * @param params The parameters that the operation is being executed with.
   * 
   * @return bytes4 The function selector for the hook.
   */
  function afterSupply(PortfolioHookParams memory params) external returns (bytes4);

  /**
   * @notice Called before an asset has been withdrawn from the portfolio.
   * 
   * @param params The parameters that the operation is being executed with.
   * 
   * @return bytes4 The function selector for the hook.
   */
  function beforeWithdraw(PortfolioHookParams memory params) external returns (bytes4);

  /**
   * @notice Called after an asset has been withdrawn from the portfolio.
   * 
   * @param params The parameters that the operation is being executed with.
   * 
   * @return bytes4 The function selector for the hook.
   */
  function afterWithdraw(PortfolioHookParams memory params) external returns (bytes4);
}
...
import { 
  FLAGS_BEFORE_SUPPLY, 
  FLAGS_AFTER_SUPPLY, 
  FLAGS_BEFORE_WITHDRAW, 
  FLAGS_AFTER_WITHDRAW,
  IPortfolioHooks
} from "../interfaces/portfolio/IPortfolioHooks.sol";
import { IAssetStorage } from "../interfaces/core/IAssetStorage.sol";
import { Errors } from "../libraries/Errors.sol";

/// @dev manages the portfolio hooks
library PortfolioHooks {

  function shouldCallBeforeSupply(IAssetStorage.PortfolioHooks memory hook) internal pure returns (bool) {
    return hook.options & FLAGS_BEFORE_SUPPLY != 0;
  }

  function shouldCallAfterSupply(IAssetStorage.PortfolioHooks memory hook) internal pure returns (bool) {
    return hook.options & FLAGS_AFTER_SUPPLY != 0;
  }

  function shouldCallBeforeWithdraw(IAssetStorage.PortfolioHooks memory hook) internal pure returns (bool) {
    return hook.options & FLAGS_BEFORE_WITHDRAW != 0;
  }

  function shouldCallAfterWithdraw(IAssetStorage.PortfolioHooks memory hook) internal pure returns (bool) {
    return hook.options & FLAGS_AFTER_WITHDRAW != 0;
  }

  function ensureBeforeSupply(bytes4 selector) internal pure {
    ensureHookResponse(selector, IPortfolioHooks.beforeSupply.selector);
  }

  function ensureAfterSupply(bytes4 selector) internal pure {
    ensureHookResponse(selector, IPortfolioHooks.afterSupply.selector);
  }

  function ensureBeforeWithdraw(bytes4 selector) internal pure {
    ensureHookResponse(selector, IPortfolioHooks.beforeWithdraw.selector);
  }

  function ensureAfterWithdraw(bytes4 selector) internal pure {
    ensureHookResponse(selector, IPortfolioHooks.afterWithdraw.selector);
  }

  function ensureHookResponse(bytes4 actual, bytes4 expected) private pure {
    if (actual != expected) {
      revert Errors.Portfolio_UnknownHookResponse();
    }
  }
}
...
import { 
  IPortfolioStorage, 
  PORTFOLIO_ASSET_FIELD_SHARES, 
  PORTFOLIO_ASSET_FIELD_LAST_UPDATE_BLOCK 
} from "../../interfaces/portfolio/IPortfolioStorage.sol";
import { IPortfolioHooks } from "../../interfaces/portfolio/IPortfolioHooks.sol";
import { IPriceOracle } from "../../interfaces/oracle/IPriceOracle.sol";
...
import { PortfolioHooks } from "../../libraries/PortfolioHooks.sol";
...
contract Portfolio is AuthorizedAccessControl, Sweepable, Pausable, Sanctionable, ReentrancyGuard, IPortfolio {
  ...
  using PortfolioHooks for IAssetStorage.PortfolioHooks;
  using PortfolioHooks for bytes4;

  // set an upper limit to the number of assets that can be added 
  // to a users portfolio as each asset adds additional gas
  uint256 private constant MAX_PORTFOLIO_ASSETS = 20;
  
  IAddressRegistry public immutable _registry;

  constructor(IAddressRegistry registry) Sweepable(registry) AdminAccessControl(registry) Sanctionable(registry) {
    _registry = registry;
  }
  ...
  /// @dev this is needed so the WETH contract can unwrap and send here
  receive() external payable { }

  /// @inheritdoc IPortfolio
  function supply(address asset, uint256 amount) external whenNotPaused requireNonZeroAmount(amount) notSanctioned(msg.sender) returns (uint256 shares) {
    shares = supplyTo(msg.sender, msg.sender, asset, amount);

    emit Supply(msg.sender, msg.sender, asset, amount, shares, msg.sender);
  }

  /// @inheritdoc IPortfolio
  function supply(address account, address asset, uint256 amount) 
    external 
      onlyAuthorized 
      whenNotPaused 
      requireNonZeroAmount(amount) returns (uint256 shares) {

    shares = supplyTo(account, msg.sender, asset, amount);

    emit Supply(account, msg.sender, asset, amount, shares, msg.sender);
  }

  /// @inheritdoc IPortfolio
  function supplyETH() external payable whenNotPaused notSanctioned(msg.sender) returns (uint256 shares) {
    IWETH9 weth = _registry.getWETH();

    IAssetStorage assetStorage = _registry.getAssetStorage();
    
    IAssetStorage.Asset memory asset = assetStorage.getAsset(
      address(weth), 
      ASSET_FIELD_CUSTODIAN | ASSET_FIELD_MAX_SUPPLY | ASSET_FIELD_HOOKS);
    require(asset.custodian != address(0), "no custodian");

    // wrap the native and supply that
    weth.deposit{ value: msg.value }();
    weth.approve(address(asset.custodian), msg.value);

    shares = supplyTo(msg.sender, address(this), asset, msg.value);

    emit SupplyETH(msg.sender, msg.value, shares, msg.sender);
  }

  function supplyTo(address account, address supplier, address assetAddress, uint256 amount) private returns (uint256 shares) {
    IAssetStorage assetStorage = _registry.getAssetStorage();
    ...
    return supplyTo(account, supplier, asset, amount);
  }

  function supplyTo(address account, address supplier, IAssetStorage.Asset memory asset, uint256 amount) private nonReentrant returns (uint256 shares) {
    ICustodian custodian = ICustodian(asset.custodian);

    if (custodian.getTotalAssets() + amount > asset.maxSupply) {
      revert Errors.Portfolio_MaximumSupplyExceeded(asset.maxSupply);
    }

    address token = address(custodian.getUnderlyingAsset());

    shares = custodian.previewSupply(amount);

    if (asset.hooks.shouldCallBeforeSupply()) {
      IPortfolioHooks.PortfolioHookParams memory params = IPortfolioHooks.PortfolioHookParams({
        account: account,
        token: token,
        amount: amount,
        shares: shares
      });
      IPortfolioHooks(asset.hooks.callback).beforeSupply(params).ensureBeforeSupply();
    }

    require(custodian.supply(supplier, amount) == shares, "shares mismatch");
    
    IPortfolioStorage portfolioStorage = _registry.getPortfolioStorage();    
    portfolioStorage.increaseShares(account, asset.token, shares);

    if (portfolioStorage.getCount(account) > MAX_PORTFOLIO_ASSETS) {
      revert Errors.Portfolio_MaxmimumLimitReached(MAX_PORTFOLIO_ASSETS);
    }

    if (asset.hooks.shouldCallAfterSupply()) {
      IPortfolioHooks.PortfolioHookParams memory params = IPortfolioHooks.PortfolioHookParams({
        account: account,
        token: token,
        amount: amount,
      ...
      IPortfolioHooks(asset.hooks.callback).afterSupply(params).ensureAfterSupply();
    }
  }
  ...
  function withdraw(address asset, uint256 amount) 
    external 
      whenNotPaused 
      requireNonZeroAddress(asset) 
      requireNonZeroAmount(amount) 
      notSanctioned(msg.sender) 
  returns (uint256 shares) {

    shares = withdrawFrom(msg.sender, msg.sender, asset, amount, false);

    ensureHealthyAccount(msg.sender);

    emit Withdraw(msg.sender, asset, amount, shares, msg.sender);
  }

  /// @inheritdoc IPortfolio
  function withdrawETH(uint256 amount) external whenNotPaused requireNonZeroAmount(amount) notSanctioned(msg.sender) returns (uint256 shares) {
    IWETH9 weth = _registry.getWETH();

    shares = withdrawFrom(msg.sender, address(this), address(weth), amount, false);
    ...
    emit WithdrawETH(msg.sender, amount, shares, msg.sender);
  }

  /// @inheritdoc IPortfolio
  function withdraw(address account, address receiver, address asset, uint256 amount) 
    external
      onlyAuthorized
      whenNotPaused 
      requireNonZeroAmount(amount) returns (uint256 shares) {

    shares = withdrawFrom(account, receiver, asset, amount, true);
    
    emit Withdraw(account, asset, amount, shares, msg.sender);
  }

  function withdrawFrom(address account, address receiver, address asset, uint256 amount, bool isAuthorizedBroker) private nonReentrant returns (uint256 shares) {
    IAssetStorage assetStorage = _registry.getAssetStorage();
    
    IAssetStorage.Asset memory a = assetStorage.getAsset(asset, ASSET_FIELD_CUSTODIAN | ASSET_FIELD_HOOKS);
    require(a.custodian != address(0), "no custodian");

    IPortfolioStorage portfolioStorage = _registry.getPortfolioStorage();
    IPortfolioStorage.PortfolioAsset memory portfolioAsset = portfolioStorage.getPortfolioAsset(
      account, 
      asset, 
      PORTFOLIO_ASSET_FIELD_SHARES | PORTFOLIO_ASSET_FIELD_LAST_UPDATE_BLOCK);
    ...
    if (isAuthorizedBroker == false) {
      ensureWithdrawAvailability(portfolioAsset);
    }

    ICustodian custodian = ICustodian(a.custodian);
    address token = address(custodian.getUnderlyingAsset());

    shares = custodian.previewWithdraw(amount);
    ...
    if (a.hooks.shouldCallBeforeWithdraw()) {
      IPortfolioHooks.PortfolioHookParams memory params = IPortfolioHooks.PortfolioHookParams({
        account: account,
        token: token,
        amount: amount,
      ...
      IPortfolioHooks(a.hooks.callback).beforeWithdraw(params).ensureBeforeWithdraw();
    }

    portfolioStorage.decreaseShares(account, a.token, shares);

    require(custodian.withdraw(receiver, amount) == shares, "shares mismatch");
    
    if (a.hooks.shouldCallAfterWithdraw()) {
      IPortfolioHooks.PortfolioHookParams memory params = IPortfolioHooks.PortfolioHookParams({
        account: account,
        token: token,
        amount: amount,
      ...
      IPortfolioHooks(a.hooks.callback).afterWithdraw(params).ensureAfterWithdraw();
    }    
  }

  /// @dev supplying and withdrawing in the same block is not something that would happen  
  /// by an account with good intentions so we only allow this for authorized brokers
  function ensureWithdrawAvailability(IPortfolioStorage.PortfolioAsset memory portfolioAsset) private view {
    if (portfolioAsset.lastUpdateBlock >= block.number) {
      revert Errors.Portfolio_WithdrawUnavailableInCurrentBlock(block.number, portfolioAsset.lastUpdateBlock);
    }
  }
  ...
  // @inheritdoc IPortfolio
  function getPortfolioValue(address account) external view requireNonZeroAddress(account) returns (USD total) {
    IPortfolioStorage.PortfolioAsset[] memory assets = _registry.getPortfolioStorage().getPortfolioAssets(account, PORTFOLIO_ASSET_FIELD_SHARES);

    (total, ) = getPortfolioValue(assets);
  }

  /// @inheritdoc IPortfolio
  function getPortfolioValue(IPortfolioStorage.PortfolioAsset[] memory position) public view returns (USD, USD[] memory) {
    // this should return the assets in the same order as the assets were requested
    IAssetStorage.Asset[] memory assets = _registry.getAssetStorage().getAssets(
      position.getTokenAddresses(), 
      ASSET_FIELD_CUSTODIAN | ASSET_FIELD_PRICE_ORACLE);

    uint256 total;
    USD[] memory totals = new USD[](position.length);

    for (uint256 i = 0; i < position.length;) {
      // the getAssets method should return the assets 
      // in the same order so this is a safety check
      require(position[i].token == assets[i].token, "token mismatch");
      require(assets[i].custodian != address(0), "no custodian");

      ICustodian custodian = ICustodian(assets[i].custodian);      
      uint256 amount = custodian.toAssets(position[i].shares);

      uint8 decimals = custodian.getDecimals();

      (USD price, ) = IPriceOracle(assets[i].priceOracle).getLatestPrice();
      
      totals[i] = USD.wrap(amount.mulDiv(USD.unwrap(price), 10 ** decimals));
            
      total += USD.unwrap(totals[i]);

      unchecked { i++; }
    }

    return (USD.wrap(total), totals);
  }
}
