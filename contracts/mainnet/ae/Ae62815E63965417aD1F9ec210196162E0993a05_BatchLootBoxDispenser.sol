...
contract BatchLootBoxDispenser is ReentrancyGuard, Pausable, ManagerModifier {
  //=======================================
  // Immutable
  //=======================================
  ...
  function dispenseBatch(
    ...
    lootBox.mintBatchFor(_address, _ids, _amounts);
  }

  function dispenseSameIdsBatch(
    address[] calldata _address,
    uint256[] calldata _ids,
    uint256[][] calldata _amounts
  ) external onlyManager nonReentrant whenNotPaused {
    for (uint256 j = 0; j < _address.length; j++) {
      lootBox.mintBatchFor(_address[j], _ids, _amounts[j]);
    }
  }
  ...
  function dispenseBatch(
    ...
    for (uint256 j = 0; j < _address.length; j++) {
      lootBox.mintBatchFor(_address[j], _ids[j], _amounts[j]);
    }
  }
