...
contract Compounder is ICompounder {
    error Compounder__E1();

    address public immutable masterSynthChef;

    constructor(address _masterSynthChef) {
        masterSynthChef = _masterSynthChef;
    }
    ...
    function compound(
        ...
        if (msg.sender != masterSynthChef) revert Compounder__E1();
        IProtocolSynthChef(protocolSynthChef).harvest(poolId);
        uint256 callConfigLen = callConfigs.length;

        for (uint256 i; i < callConfigLen; ++i) {
        ...
        uint256 underlyingsLen = underlyings.length;
        for (uint256 i; i < underlyingsLen; ++i) {
