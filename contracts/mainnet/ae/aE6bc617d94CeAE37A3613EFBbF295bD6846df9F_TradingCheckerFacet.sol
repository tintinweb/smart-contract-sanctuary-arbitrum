...
contract TradingCheckerFacet is ITradingChecker {
    ...
    function executeLimitOrderCheck(
        ...
        return (true, Refund.NO, _buildExecuteLimitOrderResult(order, tuple, openFeeUsd, leverage_10000));
    }

    function _buildExecuteLimitOrderResult(
        LimitOrder memory order, ExecuteLimitOrderCheckTuple memory tuple,
        ...
        return abi.encode(
            ...
            uint64(_availableTakeProfit(order.pairBase, order.isLong, order.takeProfit, order.limitPrice, leverage_10000))
        );
    }

    function availableTakeProfit(ITrading.OpenTrade calldata ot) external view returns (uint64 takeProfit) {
        ...
        return uint64(_availableTakeProfit(ot.pairBase, ot.isLong, ot.takeProfit, ot.entryPrice, leverage_10000));
    }
    ...
    function marketTradeCallbackCheck(
        ...
        if (pt.blockNumber + Constants.FEED_DELAY_BLOCK < Constants.arbSys.arbBlockNumber()) {
