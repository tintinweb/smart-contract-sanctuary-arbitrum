...
contract PendleLogic {
    ...
    function addLiquidity(uint256 amount, OffChainData calldata _offChainData) external view returns(uint256 lpExpected, uint256 ytExpected, IPendleRouter.TokenInput memory inputData) {
        IPendleRouter.SwapData memory swapData = _offChainData.swapData;
        inputData = IPendleRouter.TokenInput(WETH, amount, address(pendleGeneralInfo.underlyingAsset), address(0), swapData);
...
contract PendleRsEth is Initializable, PendleGeneral, UUPSUpgradeable, AccessControlUpgradeable {
    using SafeERC20 for IERC20;
    ...
    function initialize(
        ...
        __Base_Strategy_Initialize(vault, strategist, strategist, keeper);
