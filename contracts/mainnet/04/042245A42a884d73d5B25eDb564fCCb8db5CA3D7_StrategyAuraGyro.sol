...
contract StrategyAuraGyro is StratFeeManagerInitializable {
    ...
    BeefyBalancerStructs.BatchSwapStruct[] public nativeToLp1Route;

    address[] public outputToNativeAssets;
    address[] public nativeToLp0Assets;
    address[] public nativeToLp1Assets;
    ...
    function initialize(
        ...
        BeefyBalancerStructs.BatchSwapStruct[] memory _nativeToLp1Route,
        address _booster,
        uint256 _pid,
        address[] memory _outputToNative,
        address[] memory _nativeToLp0,
        address[] memory _nativeToLp1,
        ...
        for (uint j; j < _nativeToLp1Route.length; ++j) {
            nativeToLp1Route.push(_nativeToLp1Route[j]);
        }
        ...
        nativeToLp1Assets = _nativeToLp1;
        output = outputToNativeAssets[0];
        native = nativeToLp0Assets[0];
        lp0 = nativeToLp0Assets[nativeToLp0Assets.length - 1];
        lp1 = nativeToLp1Assets[nativeToLp1Assets.length - 1];
    ...
    function addLiquidity() internal {
        ...
        if (nativeBal > 0) {
            ...
            IBalancerVault.BatchSwapStep[] memory _swaps = BalancerActionsLib.buildSwapStructArray(nativeToLp0Route, lp0Amt);
            BalancerActionsLib.balancerSwap(unirouter, swapKind, _swaps, nativeToLp0Assets, funds, int256(lp0Amt));

            IBalancerVault.BatchSwapStep[] memory _swaps1 = BalancerActionsLib.buildSwapStructArray(nativeToLp1Route, lp1Amt);
            BalancerActionsLib.balancerSwap(unirouter, swapKind, _swaps1, nativeToLp1Assets, funds, int256(lp1Amt));
            
            BalancerActionsLib.multiJoin(unirouter, want, IBalancerPool(want).getPoolId(), lp0, lp1, IERC20(lp0).balanceOf(address(this)), IERC20(lp1).balanceOf(address(this)));
        }
    }
