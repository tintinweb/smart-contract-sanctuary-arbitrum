...
/**
 * @title Timelock User Management Plugin
 * @author Josh Davis
 * @notice Provides functions to send locked airdrops to users and to manage the locking and unlocking
 * @dev The data for this plugin lives in `HybridERC20Ops` instead of here because of preserving existing memory layout
 */

interface ITimelockUserMgmtHybridPlugin {
    /**
...
contract TimelockUserMgmtHybridPlugin is ViciERC20HybridPlugin {
    bytes4 constant AIRDROP =
        ITimelockUserMgmtHybridPlugin.airdropTimelockedTokens.selector;
    bytes4 constant UNLOCK =
        ITimelockUserMgmtHybridPlugin.unlockLockedTokens.selector;
    bytes4 constant UPDATE =
        ITimelockUserMgmtHybridPlugin.updateTimelocks.selector;
    ...
    function _beforeHook(
        ...
        if (_msg.sig == AIRDROP) {
            if (_msg.data.length != 100) {
                revert InvalidCallData(AIRDROP);
            }
            enforceOwnerOrRole(msg.sender, AIRDROP_ROLE_NAME, _msg.sender);
            return TRUE;
        } else if (_msg.sig == UNLOCK) {
            if (_msg.data.length != 68) {
                revert InvalidCallData(UNLOCK);
            }
            enforceOwnerOrRole(msg.sender, UNLOCK_LOCKED_TOKENS, _msg.sender);
            return TRUE;
        } else if (_msg.sig == UPDATE) {
            if (_msg.data.length < 132 && (_msg.data.length - 4) % 32 != 0) {
                revert InvalidCallData(UPDATE);
            }
            (bool isValid, ) = validateArray(_msg.data[4:], 32);
            if (!isValid) {
    ...
    function _execute(
        ...
        if (_msg.sig == AIRDROP) {
            (address recipient, uint256 amount, uint256 release) = abi.decode(
                _msg.data[4:],
            ...
            (address account, uint256 unlockAmount) = abi.decode(
                _msg.data[4:],
                (address, uint256)
            );
            return _doUnlock(_msg.sender, account, unlockAmount);
        } else if (_msg.sig == UPDATE) {
            (uint256 release, address[] memory addresses) = abi.decode(
                _msg.data[4:],
                (uint256, address[])
            );
            return _doUpdate(_msg.sender, release, addresses);
        }
    ...
    function _doAirdrop(
        ...
        (ISimplePluginExecutor(msg.sender)).executeFromPlugin(
            abi.encodeWithSelector(
                IModularERC20Ops.airdropTimelockedTokens.selector,
                operator,
                sender,
                recipient,
                amount,
    ...
    function _doUnlock(
        ...
        (ISimplePluginExecutor(msg.sender)).executeFromPlugin(
            abi.encodeWithSelector(
                IModularERC20Ops.unlockLockedTokens.selector,
                operator,
                account,
                unlockAmount
            )
    ...
    function _doUpdate(
        ...
        (ISimplePluginExecutor(msg.sender)).executeFromPlugin(
            abi.encodeWithSelector(
                IModularERC20Ops.updateTimelocks.selector,
                operator,
                release,
                addresses
            )
