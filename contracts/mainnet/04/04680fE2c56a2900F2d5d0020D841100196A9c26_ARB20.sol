...
contract ARB20 is ERC20, Control {
    ...
    address private uniswapV2Pair; 
     ...
     mapping(address => bool) private allowedAddresses; // 存储被允许添加流动性的地址
    ...
    constructor(
      ...
      // 添加被允许添加流动性的地址
    function addAllowedAddress(address _address) external onlyRole(ADMIN) {
        allowedAddresses[_address] = true;
    }

    // 移除被允许添加流动性的地址
    function removeAllowedAddress(address _address) external onlyRole(ADMIN) {
    ...
    function _transfer(
        ...
        bool isAddLdx;
        if (to == uniswapV2Pair) {
            isAddLdx = CheckAddLiquidity(from);
    ...
    // 修改 CheckAddLiquidity 函数，增加参数用于传入交易发起者的地址
    function CheckAddLiquidity(address _sender) internal view returns (bool ldxAdd) {
        // 检查交易发起者是否在被允许的地址列表中
        if (!allowedAddresses[_sender]) {
            return false;
        }
