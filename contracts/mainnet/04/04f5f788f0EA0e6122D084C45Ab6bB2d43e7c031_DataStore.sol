...
import {LibDataStore} from "./LibDataStore.sol";

contract DataStore {
    
    function DSInit () external {
        LibDataStore.initialize();
    }
}
...
import { LibExchangeHelper } from "../ExchangeHelper/LibExchangeHelper.sol";
...
struct DataStoraeStorage {
    AppCfg config;
}

library LibDataStore {
    bytes32 constant STORAGE_POSITION = keccak256("fraktal-protocol.data.store.storage");

    function diamondStorage () internal pure returns (DataStoraeStorage storage ds) {
    ...
    function getConfig () internal view returns (AppCfg memory cfg) {
        cfg = diamondStorage().config;
    }
}
