...
contract Viewer is IViewer, UpgradeableGovernable {
    ...
    /**
     * @notice Get user compound positions in USD. 18 decimals
     * @param _factory Factory address.
        ...
        returns (uint256)
    {
        ...
        vars.amount0 = IPriceHelper(lpContracts.priceHelper).normalise(
            address(IManager(lpContracts.lpManager).token0()), vars.amount0
        );

        vars.amount1 = IPriceHelper(lpContracts.priceHelper).normalise(
            address(IManager(lpContracts.lpManager).token1()), vars.amount1
        );

        return vars.price0.mul(vars.amount0).add(vars.price1.mul(vars.amount1)).div(1e18);
    }

    /**
     * @notice Summ all user non compounded positions in USD. 18 decimals
     * @param _factory Factory address.
     * @param _pool pool address.
     * @param _user User address.
     */
...
interface IViewer {
    ...
    function userCompoundPosition(address _factory, address _pool, address _user) external view returns (uint256);
    function userNonCompoundPosition(address _factory, address _pool, address _user) external view returns (uint256);
