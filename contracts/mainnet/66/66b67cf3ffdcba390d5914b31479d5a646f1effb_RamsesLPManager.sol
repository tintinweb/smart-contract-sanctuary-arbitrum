...
contract RamsesLPManager is IManager, IRamsesV2MintCallback, UpgradeableOperableKeepable, ERC20Upgradeable {
        ...
        returns (uint256 amount0, uint256 amount1, uint256 amount0AfterRetention, uint256 amount1AfterRetention)
    {
        ...
        for (uint256 i; i < length;) {
            ...
            (amount0Received, amount1Received) =
                pool.burn(_ranges[i].tickLower, _ranges[i].tickUpper, liquidityToRemove);
        ...
        returns (uint256 amount0, uint256 amount1, uint256 amount0AfterRetention, uint256 amount1AfterRetention)
    {
        ...
        for (uint256 i; i < length;) {
            ...
            (uint256 amount0Received, uint256 amount1Received) = RamsesLiquidityLib.amountsForLiquidity(
                address(pool), _ranges[i].tickLower, _ranges[i].tickUpper, liquidityToRemove
            );
...
import {IRamsesV2PoolImmutables} from "src/interfaces/pool/ramses/IRamsesV2PoolImmutables.sol";
import {IRamsesV2PoolState} from "src/interfaces/pool/ramses/IRamsesV2PoolState.sol";
import {IRamsesV2PoolDerivedState} from "src/interfaces/pool/ramses/IRamsesV2PoolDerivedState.sol";
import {IRamsesV2PoolActions} from "src/interfaces/pool/ramses/IRamsesV2PoolActions.sol";
import {IRamsesV2PoolOwnerActions} from "src/interfaces/pool/ramses/IRamsesV2PoolOwnerActions.sol";
import {IRamsesV2PoolEvents} from "src/interfaces/pool/ramses/IRamsesV2PoolEvents.sol";
...
import {LiquidityAmounts} from "src/libraries/ramses/LiquidityAmounts.sol";
import {TickMath} from "src/libraries/ramses/TickMath.sol";
import {IRamsesV2Pool} from "src/interfaces/pool/ramses/IRamsesV2Pool.sol";
import {IManager} from "src/interfaces/manager/IManager.sol";

library RamsesLiquidityLib {
    /**
     * @notice Get the amounts of the given numbers of liquidity tokens
     * @param pool RamsesV2 liquidity pool contract
     * @param tickLower The lower tick of the position
     * @param tickUpper The upper tick of the position
     ...
     * @param pool Ramses liquidity pool contract
     * @param tickLower The lower tick of the position
     * @param tickUpper The upper tick of the position
    ...
    function checkRanges(IManager.Range[] calldata _ranges, address _pool) external view {
        IRamsesV2Pool pool = IRamsesV2Pool(_pool);
        uint256 length = _ranges.length;
        for (uint256 i; i < length;) {
...
interface IRamsesV2PoolActions {
    ...
    /// @return amount0 The amount of token0 sent to the recipient
    /// @return amount1 The amount of token1 sent to the recipient
    function burn(uint256 index, int24 tickLower, int24 tickUpper, uint128 amount, uint256 veRamTokenId)
        external
        returns (uint256 amount0, uint256 amount1);
...
import "src/libraries/ramses/FullMath.sol";
import "src/libraries/ramses/FixedPoint96.sol";
