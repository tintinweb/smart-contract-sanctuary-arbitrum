...
contract SakeVaultV3 is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function getVLPCooldownPeriod() external view returns(uint256) {
        return ISettingManager(strategyAddresses.velaStakingVault).cooldownDuration();
    }
    ...
    function openPosition(
        ...
        uint256 userPositionId = userInfo[msg.sender].length;
        ...
        positionOpenTimestamp[msg.sender][userPositionId] = block.timestamp;
    ...
    function closePosition(
        ...
        require(!_userInfo.closed, "Sake: position is Closed");
        ...
        if (allowedClosers[msg.sender]) {
            require(block.timestamp >= positionOpenTimestamp[_user][_positionID] + vaultMaturityTime, "SAKE:Wait for Mature date");
        }
        ...
        emit Withdraw(_user, amountAfterFee, block.timestamp, closeData2.withdrawableShares);
    }
    ...
    function liquidatePosition(uint256 _positionId, address _user) external nonReentrant {
        ...
        if (returnedValue >= _userInfo.leverageAmount) {
            returnedValue -= _userInfo.leverageAmount;

            liquidatorReward = returnedValue.mulDiv(feeConfiguration.liquidatorsRewardPercentage, MAX_BPS);
            IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(msg.sender, liquidatorReward);

            uint256 leftovers = returnedValue - liquidatorReward;
            if(leftovers > 0) {
                IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(_userInfo.user, leftovers);
            }

            IERC20Upgradeable(strategyAddresses.USDC).safeIncreaseAllowance(strategyAddresses.water, _userInfo.leverageAmount);
            IWater(strategyAddresses.water).repayDebt(_userInfo.leverageAmount, _userInfo.leverageAmount);
        } else {
            IERC20Upgradeable(strategyAddresses.USDC).safeIncreaseAllowance(strategyAddresses.water, returnedValue);
            IWater(strategyAddresses.water).repayDebt(_userInfo.leverageAmount, returnedValue);
        }
