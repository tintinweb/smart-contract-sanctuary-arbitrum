...
contract ProductionCapacity is IProductionCapacity, EpochConfigurable {
  ...
  function resetProductionCapacity(uint _realmId) external onlyManager {
    _spendCapacity(_realmId, _productionCapacity(_realmId));
  }

  function resetProductionCapacityBatch(uint[] calldata _realmIds) external onlyManager {
    for (uint i = 0; i < _realmIds.length; i++) {
      _spendCapacity(_realmIds[i], _productionCapacity(_realmIds[i]));
    }
  }
  ...
  function _productionCapacity(uint _realmId) internal view returns (uint capacity) {
    ...
    if (block.timestamp == lastTimestamp) {
      return LAST_PRODUCTION_REMAINING_CAPACITY[_realmId];
    }
    ...
    if (elapsedEpochs > PRODUCTION_CAPACITY_EPOCHS_CAP / 2) {
      capacity++;
    }
