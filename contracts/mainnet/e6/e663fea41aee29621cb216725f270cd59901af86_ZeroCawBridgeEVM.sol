...
contract ZeroBridgedCaw is ERC20, Ownable {

    string public earlycrow = "greetings from chain id 25, block 946";
    
    constructor(string memory __name, string memory __symbol, address targetOwner) ERC20(__name, __symbol) Ownable(targetOwner){
...
import {ZeroBridgedCaw} from "./ZeroBridgedCaw.sol";
import {IWETH} from "./IWETH.sol";

contract ZeroCawBridgeEVM is MessageClient {
    event CawTokenBridged(address from, address to, uint256 amount, uint256 destChainId);
    event CawTokenReceived(address to, uint256 amount);

    using SafeERC20 for IERC20;

    ZeroBridgedCaw public immutable BRIDGED_CAW_TOKEN;
    address public immutable UNISWAP_ROUTER_ADDRESS;
    address public immutable WETH;

    uint256 public wrappedFee;

    constructor(ZeroBridgedCaw bridgedCawToken_, address owner, address uniswapRouterAddress, uint256 wrappedFee_) {
        BRIDGED_CAW_TOKEN = bridgedCawToken_;
        MESSAGE_OWNER = owner;
        UNISWAP_ROUTER_ADDRESS = uniswapRouterAddress;
        wrappedFee = wrappedFee_;
        IUniswapV2Router02 router = IUniswapV2Router02(UNISWAP_ROUTER_ADDRESS);
        WETH = router.WETH();
    }

    function bridge(address to, uint256 amount, uint256 destChainId) external payable returns (uint256 txId) {
        require(msg.value >= wrappedFee, "Insufficient ETH for wrapped fee");
        IWETH(WETH).deposit{value: wrappedFee}();
        BRIDGED_CAW_TOKEN.burn(msg.sender, amount);
        uint256 requiredUsdcFee = MESSAGEv3.getSourceFee(destChainId, false);
        uint256 ethUsedInSwap = _swapFeeToken(requiredUsdcFee);
        if (msg.value > (ethUsedInSwap + wrappedFee)) {
            payable(msg.sender).transfer(msg.value - (ethUsedInSwap + wrappedFee));
        }
        emit CawTokenBridged(msg.sender, to, amount, destChainId);
        uint256 amountWithDecimals = amount * 1e18;
        return _sendMessage(destChainId, abi.encode(to, amountWithDecimals));
    }
    ...
    function _swapFeeToken(uint256 requiredUsdcAmount) private returns (uint256) {
        ...
        uint256[] memory swapAmounts = router.swapETHForExactTokens{value: (msg.value - wrappedFee)}(
            requiredUsdcAmount,
            path,
            address(this),
            block.timestamp
    ...
    function getRequiredEthForFee(uint256 destChainId) public view returns (uint256) {
        uint256 requiredUsdcFee = MESSAGEv3.getSourceFee(destChainId, false);
        IUniswapV2Router02 router = IUniswapV2Router02(UNISWAP_ROUTER_ADDRESS);
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        ...
        return amounts[0] + wrappedFee;
    }


    function messageProcess(uint256, uint256 sourceChainId_, address sender_, address, uint256, bytes calldata data_) external override onlySelf(sender_, sourceChainId_) {
        (address to, uint256 amount) = abi.decode(data_, (address, uint256));
        uint256 amountWithoutDecimals = amount / 1e18;
        BRIDGED_CAW_TOKEN.mint(to, amountWithoutDecimals);
        emit CawTokenReceived(to, amountWithoutDecimals);
    }
    
    function setWrappedFee(uint256 newFee) external onlyMessageOwner {
        wrappedFee = newFee;
    }
