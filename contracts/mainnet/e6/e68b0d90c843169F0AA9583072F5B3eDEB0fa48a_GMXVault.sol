...
import { GMXReader } from  "./GMXReader.sol";
import { GMXCallback } from  "./GMXCallback.sol";
...
contract GMXVault is ERC20, Ownable2Step, ReentrancyGuard, Pausable, IGMXVault {
  ...
  // Approved callback or keepers
  mapping(address => bool) public callbackOrKeepers;
  ...
  */
  constructor (
    ...
    _store.vault = IGMXVault(store_.vault);
    // TODO
    // _store.queue = IGMXQueue(store_.queue);
    _store.queue = store_.queue;

    // Deploy callback contract for this vault
    GMXCallback callback = new GMXCallback(address(this));
    _store.callback = address(callback);

    _store.chainlinkOracle = IChainlinkOracle(store_.chainlinkOracle);
    _store.gmxOracle = IGMXOracle(store_.gmxOracle);

    _store.exchangeRouter = IGMXExchangeRouter(store_.exchangeRouter);
    ...
    _store.depositHandler = store_.depositHandler;
    _store.withdrawalHandler = store_.withdrawalHandler;
    _store.orderHandler = store_.orderHandler;
  ...
  */
  function deposit(
    GMXTypes.DepositParams memory dp
  ) payable external nonReentrant whenNotPaused onlyQueue {
  ...
  */
  ...
  /**
    ...
    * @param depositKey Deposit key hash to find deposit info
  */
  function processMint(bytes32 depositKey) external whenNotPaused onlyKeeper {
    GMXDeposit.processMint(_store, depositKey);
  }
  ...
  */
  ...
  /**
    ...
    * @param orderKey Swap key hash to find orderKey hash
  */
  function processSwapForRepay(bytes32 orderKey) external whenNotPaused onlyKeeper {
    GMXWithdraw.processSwapForRepay(_store, orderKey);
  }
  ...
  */
  function processRepay(
    ...
    GMXWithdraw.processRepay(_store, withdrawKey, orderKey);
  }

  /**
    * @dev Process transfer of assets to user after swapping and burn shares
    * @param orderKey Swap key hash to find orderKey hash
  */
  function processSwapForWithdraw(bytes32 orderKey) external whenNotPaused onlyKeeper {
    GMXWithdraw.processSwapForWithdraw(_store, orderKey);
  }
  ...
  /**
    ...
    * @param orderKey Swap key hash to find orderKey hash
  */
  function processBurn(bytes32 withdrawKey, bytes32 orderKey) external whenNotPaused onlyKeeper {
    GMXWithdraw.processBurn(_store, withdrawKey, orderKey);
  }

  /**
    * @dev Emergency withdraw function, enabled only when vault is paused, burns svToken from user
    * @param withdrawParams WithdrawParams struct of withdraw parameters
  */
  function emergencyWithdraw(
    GMXTypes.WithdrawParams memory withdrawParams
  ) external nonReentrant whenPaused {
    // GMXEmergency.emergencyWithdraw(_store, withdrawParams);
  }
  ...
  */
  function compound(address token, uint256 slippage, uint256 deadline) external {
    GMXCompound.compound(_store, token, slippage, deadline);
  }
  ...
  */
  function _onlyQueue() internal view {
    if (address(_store.queue) != address(0)) revert Errors.OnlyQueueAllowed();
  }
  ...
  */
  ...
  /**
    ...
    * @param depositKey Deposit key hash to find deposit info
  */
  function processRebalanceAdd(bytes32 depositKey) external whenNotPaused onlyKeeper {
    GMXRebalance.processRebalanceAdd(_store, depositKey);
  }
  ...
  */
  ...
  /**
    ...
    * @param withdrawKey Withdraw key hash to find withdrawal info
  */
  function processRebalanceRemoveSwapForRepay(bytes32 withdrawKey) external whenNotPaused onlyKeeper {
    GMXRebalance.processRebalanceRemoveSwapForRepay(_store, withdrawKey);
  }
  ...
  /**
    ...
    * @param orderKey Swap key hash to find withdrawKey hash
  */
  function processRebalanceRemoveRepay(bytes32 withdrawKey, bytes32 orderKey) external whenNotPaused onlyKeeper {
    GMXRebalance.processRebalanceRemoveRepay(_store, withdrawKey, orderKey);
  }
  ...
  /**
    ...
    * @param depositKey Deposit key hash to find deposit info
  */
  function processRebalanceRemoveAddLiquidity(bytes32 depositKey) external whenNotPaused onlyKeeper {
    GMXRebalance.processRebalanceRemoveAddLiquidity(_store, depositKey);
  }
  ...
  /**
    ...
    * @param deadline Timestamp of deadline for swap to go through
  */
  function emergencyShutdown(uint256 slippage, uint256 deadline) external onlyKeeper {
    _pause();

    GMXEmergency.emergencyShutdown(_store, slippage, deadline);
  }
  ...
  /**
    ...
    * @param deadline Timestamp of deadline for swap to go through
  */
  function emergencyResume(uint256 slippage, uint256 deadline) external whenPaused onlyOwner {
    _unpause();

    GMXEmergency.emergencyResume(_store, slippage, deadline);
  }
  ...
  */
  ...
  /**
    * @dev Update allowed GMX callback handlers
    * @param depositHandler address of GMX's deposit callback handler
    * @param withdrawalHandler address of GMX's withdrawal callback handler
    * @param orderHandler address of GMX's order callback handler
  */
  function updateMinExecutionFee(
    address depositHandler,
    address withdrawalHandler,
    ...
    _store.depositHandler = depositHandler;
    _store.withdrawalHandler = withdrawalHandler;
    _store.orderHandler = orderHandler;
  }
  ...
  */
  ...
  function resetVault() external onlyOwner {
    _store.status = GMXTypes.Status.Open;
    // _store.isExecuting = false;
    // _store.isRebalancing = false;
    // _store.isSwapping = false;
    _store.vault.burn(msg.sender, IERC20(address(_store.vault)).balanceOf(msg.sender));
  }

  /* ========== CALLBACK FUNCTIONS FOR GMX HANDLERS ========== */
  ...
  //   IGMXEvent.Props memory eventData
  // ) external {
  //   GMXCallback.afterDepositExecution(_store, depositKey, depositProps, eventData);
  // }
  ...
  //   IGMXEvent.Props memory eventData
  // ) external {
  //   GMXCallback.afterWithdrawalExecution(_store, withdrawKey, withdrawProps, eventData);
  // }
  ...
  //   IGMXEvent.Props memory eventData
  // ) external {
  //   GMXCallback.afterWithdrawalCancellation(_store, withdrawKey, withdrawProps, eventData);
  // }
  ...
  //   IGMXEvent.Props memory eventData
  // ) external {
  //   GMXCallback.afterOrderExecution(_store, orderKey, orderProps, eventData);
  // }
  ...
  //   IGMXEvent.Props memory eventData
  // ) external {
  //   GMXCallback.afterOrderCancellation(_store, orderKey, orderProps, eventData);
  // }
  ...
  //   IGMXEvent.Props memory eventData
  // ) external {
  //   GMXCallback.afterOrderFrozen(_store, orderKey, orderProps, eventData);
  // }
  ...
  */
  receive() external payable {
    require(msg.sender == address(_store.WNT), "msg.sender != WNT");
  }
}
...
library GMXWithdraw {
  ...
  */
  function processBurn(
    ...
    // Refund any left over execution fees to user
    // If one of the token is WNT, then it will be returned to the user here as well
    self.WNT.withdraw(self.WNT.balanceOf(address(this)));
