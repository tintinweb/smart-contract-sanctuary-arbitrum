...
contract DragonCrown is ERC404 {
    string public dataURI;
    string public baseTokenURI;
    string[] private colors = ['Barbie', 'Cyber blue', 'Sprited away', 'Jade', 'Eva01', 'Eva001', 'Golden', 'Ancient monster', 'Cyberpunk', 'Megatron'];
    ...
    constructor(
        address _owner
    ) ERC404("DragonCrown", "DCON", 18, 30000, _owner) {
        balanceOf[_owner] = 30000 * 10 ** 18;
        feeWallet = _owner;
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        if (bytes(baseTokenURI).length > 0) {
            ...
            string memory color = colors[seed % 10];
            uint imageId;
                
            if (seed <= 50) {
                imageId = 1 + seed % 10;
                image = string.concat(Strings.toString(imageId), '.png');
                dragon = "ASTRA";
            } else if (seed <= 125) {
                imageId = 11 + seed % 10;
                image = string.concat(Strings.toString(imageId), '.png');
                dragon = "PRINCE ARGENTUM";
            } else if (seed <= 255) {
                imageId = 21 + seed % 10;
                image = string.concat(Strings.toString(imageId), '.png');
                dragon = "Rexxar";
            ...
            string memory jsonPreImage = string.concat(
                string.concat(
                    string.concat('{"name": "DCON #', Strings.toString(id)),
                    '","description":"A collection enabled by ERC404, an experimental token standard.","external_url":"https://dragoncrown.org","image":"'
                ),
            ...
            string memory jsonPostImage = string.concat(
                ...
                string.concat('"}, {"trait_type":"Color","value":"', 
                    color)
            );
