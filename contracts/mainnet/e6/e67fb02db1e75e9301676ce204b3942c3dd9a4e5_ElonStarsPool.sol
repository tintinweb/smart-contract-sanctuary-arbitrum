...
// File contracts/ElonStarsPool.sol



contract ElonStarsPool is Ownable {
    ...
    uint256 public STARS_MULTIPLIER = 1;
    ...
    constructor(
        ...
        uint256 _startTime,
        uint256 _starsEndTime,
        uint256 _maxStakingPerUser
    ) {
        rewardToken = _rewardToken;
        rewardPerSecond = _rewardPerSecond;
        startTime = _startTime;
        starsEndTime = _starsEndTime;
        maxStakingPerUser = _maxStakingPerUser;
    }

    function stopReward() public onlyOwner {
        starsEndTime = block.timestamp;
    }

    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (_to <= starsEndTime) {
            return _to.sub(_from).mul(STARS_MULTIPLIER);
        } else if (_from >= starsEndTime) {
            return 0;
        } else {
            return starsEndTime.sub(_from).mul(STARS_MULTIPLIER);
        }
    }

    function add(uint256 _allocPoint, address _token, bool _withUpdate) public onlyOwner {
        require(_token != address(0), "ElonStarsPool: _token is the zero address");

        require(!EnumerableSet.contains(_pairs, _token), "ElonStarsPool: _token is already added to the pool");
    ...
    function updateMultiplier(uint256 multiplierNumber) public onlyOwner {
        massUpdatePools();
        STARS_MULTIPLIER = multiplierNumber;
    }
    ...
    function setStandardRewardInterval(uint256 interval) public onlyOwner {
        require(interval > 0, "ElonStasPool: interval must be greater than 0");
        standardRewardInterval = interval;
    }

    function pendingReward(uint256 _pid, address _user) public view returns (uint256) {
        require(_pid <= poolInfo.length - 1, "ElonStasPool: Can not find this pool");
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require(_amount.add(user.amount) <= maxStakingPerUser, 'ElonStasPool: exceed max stake');
    ...
    function _injectRewardsWithTime(uint256 amount, uint256 rewardsSeconds) internal {
        ...
        uint256 remainingSeconds = starsEndTime > block.timestamp ? starsEndTime.sub(block.timestamp) : 0;
        ...
        if(block.timestamp >= startTime) {
            starsEndTime = block.timestamp.add(rewardsSeconds);
        } else {
            starsEndTime = startTime.add(rewardsSeconds);
        }
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "ElonStasPool: Withdraw with insufficient balance");
    ...
    function emergencyRewardWithdraw(uint256 _amount) public onlyOwner {
        require(_amount <= rewardToken.balanceOf(address(this)), 'ElonStarsPool: not enough token');
        rewardToken.safeTransfer(address(msg.sender), _amount);
    }

    function getPoolView(uint256 pid) public view returns (PoolView memory) {
        require(pid < poolInfo.length, "ElonStasPool: pid out of range");
    ...
    function addCaller(address val) public onlyOwner() {
        require(val != address(0), "ElonStasPool: val is the zero address");
        _callers.add(val);
    }

    function delCaller(address caller) public onlyOwner returns (bool) {
        require(caller != address(0), "ElonStasPool: caller is the zero address");
