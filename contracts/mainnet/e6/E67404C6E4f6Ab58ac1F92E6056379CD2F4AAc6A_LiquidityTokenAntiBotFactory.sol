...
import { ILiquidityTokenAntiBot } from "../interfaces/ILiquidityTokenAntiBot.sol";

contract LiquidityTokenAntiBotFactory is TokenFactoryBase {
    ...
    function create(
        ...
        token = Clones.cloneDeterministic(implementation, keccak256(abi.encodePacked(msg.sender, name, symbol, totalSupply, taxFeeBps, liquidityFeeBps, enableAntiBot)));
        ILiquidityTokenAntiBot(token).initialize(
            msg.sender,
            name,
            symbol,
            totalSupply,
        ...
        assignTokenToOwner(msg.sender, token, 7);
        emit TokenCreated(msg.sender, token, 7, implementationVersion);
    }
}
 ...
 * 🚀 Unleash the power of decentralized finances and tokenization with DEXTools Token Creator. Customize your token seamlessly. Manage your created tokens conveniently from your user panel - start creating your dream token today!
 */
interface ILiquidityTokenAntiBot {
    function initialize(
        address owner_,
        string memory name_,
