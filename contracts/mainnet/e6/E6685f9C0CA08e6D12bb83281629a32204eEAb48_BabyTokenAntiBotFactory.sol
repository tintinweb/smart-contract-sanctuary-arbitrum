...
import { IBabyTokenAntiBot } from "../interfaces/IBabyTokenAntiBot.sol";

contract BabyTokenAntiBotFactory is TokenFactoryBase {
    ...
    function create(
        ...
        token = Clones.cloneDeterministic(implementation, keccak256(abi.encodePacked(msg.sender, name, symbol, totalSupply, feeSettings, minimumTokenBalanceForDividends, enableAntiBot)));
        IBabyTokenAntiBot(token).initialize(
            msg.sender,
            name,
            symbol,
            totalSupply,
        ...
        assignTokenToOwner(msg.sender, token, 11);
        emit TokenCreated(msg.sender, token, 11, implementationVersion);
    }
}
