...
contract PromotionCodeNFT is ERC165, IERC721, IERC721Metadata, IBaseNFT {
    using Strings for uint256;
    using Address for address;

    // Promotion code owner
    address private _owner; 
    ...
    // Array of all mint timestamp
    uint256[] private _timestamps;

    // Array of all usage info
    bool[] private _isUsed;
    ...
    modifier onlyOwner() {
        require(msg.sender == _owner, "BaseNFT: must be owner");
        _;
    }

    modifier onlyMinter() {
        require(_minters[msg.sender], "BaseNFT: must be minter");
        _;
    }

    modifier onlyTransferable() {
        require(_transferable, "BaseNFT: must transferable");
        _;
    }

    function initialize(bytes memory _config) public {
        require(_owner == address(0), "BaseNFT: already initialized");

        (_owner, _name, _symbol) = abi.decode(_config, (address, string, string));
    ...
    returns (bool)
    {
        ...
        interfaceId == type(IBaseNFT).interfaceId ||
    ...
    /**
     * @dev Returns the mint timestamp of tokenId.
     */
    ...
    returns (address)
    {
        ...
        return address(_tokens[tokenId]);
    }

    function ownerOfWithZeroAddress(uint256 tokenId)
    public
    view
    returns (address)
    {
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        return tokenId > 0 && tokenId <= totalMinted() && _tokens[tokenId] != address(0);
    }
    ...
    returns (uint256)
    {
        require(account != address(0), "BaseNFT: mint to the zero address");

        uint256 tokenId = _tokens.length;

        _balances[account] += 1;
        _tokens.push(account);
        _timestamps.push(block.timestamp);
        _isUsed.push(false);

        require(
            _checkOnERC721Received(address(0), account, tokenId, ""),
            "BaseNFT: transfer to non ERC721Receiver implementer"
        );
    ...
    function burnExpiredPromotionCode(address account, uint256 id) external onlyOwner {
        // Clear approvals
        _approve(address(0), id);
        _burnCount++;
        _balances[account] -= 1;
        _tokens[id] = address(0);
        _timestamps[id] = 0;
    ...
    function burn(address account, uint256 id) external override onlyMinter {
        require(
            ...
            "BaseNFT: caller is not approved or owner"
        );
        require(isOwnerOf(account, id), "BaseNFT: not owner");
