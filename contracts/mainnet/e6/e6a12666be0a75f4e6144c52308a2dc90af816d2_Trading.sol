...
contract Trading {
	...
	struct Product {
		...
		uint32 liquidationThreshold; // in bps. 8000 = 80%
		uint32 liquidationFee; // In sbps (10^6). 0.5% = 5000. 0.025% = 250
        uint32 longFee; // In sbps (10^6). 0.5% = 5000. 0.025% = 250
        uint32 shortFee; // In sbps (10^6). 0.5% = 5000. 0.025% = 250
		uint64 interest; // For 360 days, in bps. 10% = 10000000000
		uint64 maxProfit; //in bps. 500% = 500000000000
	}
	...
	uint256 public keeperFee = 0.000125 ether;
	...
	function setFeeKeeper(address feeKeeper) external onlyOwner {
		feekeeper = feeKeeper;
	}
	...
	function addProduct(bytes32 productId, Product memory _product) external onlyOwner {
		...
		require(products[productId].liquidationThreshold == 0, "!product-exists");
		require(
			_product.liquidationThreshold > 0 &&
			_product.liquidationThreshold> 0,
			"!liqThreshold"
		);
        require(
            _product.liquidationFee <= 10**6 &&
            _product.longFee <= 10**6 &&
            _product.shortFee <= 10**6,
            "!fees"
        );
	...
	function updateProduct(bytes32 productId, Product memory _product) external onlyFeeKeeper {

		require(products[productId].liquidationThreshold > 0, "!product-does-not-exist");
		require(
			_product.liquidationThreshold > 0 &&
			_product.liquidationThreshold> 0,
			"!liqThreshold"
	...
	function submitOrder(
        ...
        payable(darkoracle).transfer(keeperFee);

		if (currency == address(0)) { // User is sending ETH
			margin = (msg.value - keeperFee) / 10**(18 - UNIT_DECIMALS);
		} else {
		...
		{
        uint256 fee = _getFee(size, product, isLong);
	...
	function cancelOrder(
		...
		Product memory product = products[productId];
        uint256 fee = _getFee(order.size, product, isLong);
...
function _settleCloseOrder(
		...
		if (pnl > int256(order.margin * uint256(product.maxProfit) / 10**4)) {
    		pnl = int256(order.margin * uint256(product.maxProfit) / 10**4);
		}
    	...
    	if (margin > fee) {
        	margin -= fee;
       		...
       		fee = margin; // take whatever margin is available as fee
        	margin = 0;
   		}
	...
	function liquidatePosition(
		...
		if (pnl <= -1 * int256(threshold)) {
			...
			uint256 fee = position.margin * product.liquidationFee / 10**4;
			address pool = IRouter(router).getPool(currency);

			_transferOut(currency, pool, position.margin * (10**4 - product.liquidationFee) / 10**4);
	...
	function releaseMargin(
		...
		if (includeFee) {
			Product memory product = products[productId];
            uint256 fee = _getFee(position.size, product, isLong);
			margin += fee;
		}
	...
	function migratePosition(
		...
		bytes32 key = _getPositionKey(trader, productId, currency, isLong);
    ...
    function _getFee(
        ...
        uint32 fee = isLong ? product.longFee : product.shortFee;
        return size * fee / 10**6;
    }
	...
	modifier onlyFeeKeeper() {
		require(msg.sender == feekeeper, "!feekeeper");
		_;
	}
