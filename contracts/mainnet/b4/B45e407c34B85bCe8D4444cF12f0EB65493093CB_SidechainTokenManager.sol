...
contract SidechainTokenManager is ITokenDistributionManager, AccessControl {
    using SafeERC20 for IERC20;

    bytes32 public constant EXTERNAL_SERVICE_ROLE = keccak256("EXTERNAL_SERVICE_ROLE");
    address internal constant OLD_TOKEN = 0x0000000000000000000000000000000000000001;

    IStorage private _storage;

    mapping(address => uint256) private _withdrawnBalances;
    mapping(address => uint256) private _withdrawnRefBalances;

    event Erc20Recovered(address token, uint256 amount);
    event BalanceWithdrawn(address indexed user, uint256 amount);
    event RefBalanceWithdrawn(address indexed ref, uint256 amount);

    error ErrNullAddress();
    error ErrOutOfBalance();

    constructor(IStorage storage_) {
        if (address(storage_) == address(0)) {
            revert ErrNullAddress();
        }

        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _grantRole(EXTERNAL_SERVICE_ROLE, _msgSender());
    ...
    function withdrawUserBalance(
        address user_,
        uint256 amount_
    ) external onlyRole(EXTERNAL_SERVICE_ROLE) {
        uint256 totalBalance = getPurchasedUserBalance(user_);
        if (totalBalance < amount_) {
            revert ErrOutOfBalance();
        }

        _withdrawnBalances[user_] += amount_;

        emit BalanceWithdrawn(user_, amount_);
    }
    ...
    function withdrawRefBalance(
        ...
        uint256 totalBalance = getRefUserBalance(user_);
        if (totalBalance < amount_) {
            revert ErrOutOfBalance();
        }

        _withdrawnRefBalances[user_] += amount_;

        emit RefBalanceWithdrawn(user_, amount_);
    }
    ...
    function getRefUserBalance(address user_) public view returns (uint256) {
        return _storage.refBalanceOf(OLD_TOKEN, user_) - _withdrawnRefBalances[user_];
    }

    function getPurchasedUserBalance(address user_) public view returns (uint256 totalBalance) {
        for (uint256 i; i < _storage.getRoundsCount(); i++) {
            totalBalance += _storage.balanceOf(i, user_);
        }

        totalBalance -= _withdrawnBalances[user_];
    }
}
