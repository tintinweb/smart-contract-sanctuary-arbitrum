...
interface ITokenManager {
  ...
  function withdrawAmbBalance(address user_, uint256 amount_) external;

  function recoverErc20(address token_, uint256 amount_) external;

  function getAmbUserBalance(address user_) external view returns (uint256);
...
contract TokenManagerMultichain is ITokenManager, AccessControl {
  using SafeERC20 for IERC20;
  bytes32 public constant EXTERNAL_SERVICE_ROLE = keccak256("EXTERNAL_SERVICE_ROLE");
  address internal constant OLD_TOKEN = 0x9000000000000000000000000000000000000009;

  ISaleProcessor private _sale;

  mapping(address => uint256) private _withdrawnBalances;
  mapping(address => uint256) private _withdrawnAmbBalances;

  event Erc20Recovered(address token, uint256 amount);
  event BalanceWithdrawn(address indexed user, uint256 amount);
  event AmbBalanceWithdrawn(address indexed ref, uint256 amount);

  error ErrNullAddress();
  error ErrOutOfBalance();

  constructor(ISaleProcessor sale_) {
    if (address(sale_) == address(0)) {
  ...
  function withdrawUserBalance(
    ...
    uint256 totalBalance = getUserBalance(user_);
  ...
  function withdrawAmbBalance(
    address user_,
    uint256 amount_
  ) external onlyRole(EXTERNAL_SERVICE_ROLE) {
    uint256 totalBalance = getAmbUserBalance(user_);
    if (totalBalance < amount_) {
      revert ErrOutOfBalance();
    }

    _withdrawnAmbBalances[user_] += amount_;

    emit AmbBalanceWithdrawn(user_, amount_);
  }
  ...
  function getAmbUserBalance(address user_) public view returns (uint256) {
    return _sale.ambasadorBalanceOf(OLD_TOKEN, user_) - _withdrawnAmbBalances[user_];
  }

  function getUserBalance(address user_) public view returns (uint256 totalBalance) {
    for (uint256 i; i < _sale.getStagesAmount(); i++) {
      totalBalance += _sale.balanceOf(i, user_);
    }
