...
contract NodeSaleRemoteFactory is
  Initializable,
  UUPSUpgradeable,
  OwnableUpgradeable,
  PausableUpgradeable,
  ...
  IGovFactory
{
   ...
   * @param _sale Amount node project to sell
   * @param _price Node project price in payment decimal
   * @param _fee_d2 Fee project percent in each rounds in 2 decimal
   * @param _minKomStaked Min kom to staked
   * @param _payment Tokens to raise
   * @param _dstPaymentDecimals Payment decimal used in target sale on destination chain
  ...
  function createProject(
    ...
    uint256 _sale,
    uint256 _price,
    uint256[2] memory _fee_d2,
    uint256 _minKomStaked,
    address _payment,
    // string[3] calldata _nameVersionMsg,
    // uint128[2] calldata _voteStartEnd,
    uint128 _dstPaymentDecimals, // decimal from token payment used at polygon sc (ex: 6 in usdc polygon)
    address _targetSale // sc sale at polygon
