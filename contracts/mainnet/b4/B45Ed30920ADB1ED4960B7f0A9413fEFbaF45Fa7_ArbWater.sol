...
contract ArbWater is ERC4626Upgradeable, OwnableUpgradeable {
    ...
    function initialize(address _ARB) external initializer {
        ...
        __ERC20_init("ARB-WATER", "ARB-WATER");
    }
    ...
    function totalAssetInPrice() external view returns(uint256) {
        ...
        uint256 getPrice = IHandler(vodkaHandler).getLatestData(ARB, false);
    ...
    /**
     * @notice Allow the VodkaV2 Vault to lend a certain amount of ARB to the protocol.
     * @dev The function allows the VodkaV2 Vault to lend a certain amount of ARB to the protocol. It updates the total debt and total ARB balances accordingly.
    ...
    function lend(uint256 _borrowed, address _receiver) external onlyAllowedVaults returns (bool status) {
        ...
        require(totalAsset > am, "Not enough ARB to lend");

        totalDebt += _borrowed;
        totalAsset -= _borrowed;

        require(getUtilizationRate() <= utilRate, "Leverage ratio too high");
        IERC20(ARB).safeTransfer(_receiver, am);
     ...
     * @dev The function allows the VodkaV2 Vault to repay a certain amount of debt to the protocol. It updates the total debt and total ARB balances accordingly.
    ...
    function deposit(uint256 _assets, address _receiver) public override noZeroValues(_assets) returns (uint256) {
        IERC20(ARB).transferFrom(msg.sender, address(this), _assets);
