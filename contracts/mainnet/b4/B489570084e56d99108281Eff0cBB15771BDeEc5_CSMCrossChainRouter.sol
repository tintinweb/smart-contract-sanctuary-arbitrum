...
contract CSMCrossChainRouter is ICSMCrossChainRouterL0, AccessControl, ReentrancyGuard {
    ...
    /// @notice received parameters per chain per swap id
    mapping(uint16 => mapping(bytes => CCReceiveParams)) private receivedParamsPerChain;

    /// @notice maps a dst chain id to a nonce
    mapping(uint16 => uint256) private noncePerRouter;

    ILayerZeroEndpoint public immutable layerZeroEndpoint;

    address payable public refundAddress;

    uint256 public gasForSRCReceiver = 600000;

    event MessageRouted(uint16 destinationChain, address indexed destinationAddress, bytes id);
    ...
    event GasForSRCReceiverUpdated(uint256 _oldGas, uint256 _newGas);
    ...
    function getMessageReceived(uint16 srcChain_, bytes calldata id_)
        external
        view
        override
        returns (CCReceiveParams memory)
    {
        return receivedParamsPerChain[srcChain_][id_];
    }
    ...
    function route(
        ...
        emit MessageRouted(dstChain_, dstAddress_, id_);
    }
    ...
    function handleReceive(
        ...
        (
            ...
            params.signature,
            params.id
        ) = abi.decode(payload_, (address, address, address, uint256, uint256, bytes, bytes));
        params.srcChainId = srcChainId_;

        require(isApprovedAsset(srcChainId_, params.srcAsset), "ASSET_NOT_APPROVED");
        receivedParamsPerChain[srcChainId_][params.id] = params;
    ...
    function setGasForSRCReceiver(uint256 _newGas) external onlyRole(DEFAULT_ADMIN_ROLE) {
        emit GasForSRCReceiverUpdated(gasForSRCReceiver, _newGas);
        gasForSRCReceiver = _newGas;
    }
