...
contract mspCoin is ERC20, Ownable {
    ...
    uint256 public _totalSupply = 69420000000 * 10 ** 6;
    ...
    function transfer(address to, uint256 amount) public virtual override returns(bool) {
        return mspCoinTransfer(_msgSender(), to, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns(bool) {
        address spender = _msgSender();
        _spendAllowance(sender, spender, amount);
        return mspCoinTransfer(sender, recipient, amount);
    }

    function mspCoinTransfer(address sender, address recipient, uint256 amount) internal returns(bool) {
        if (inSwap) {
    ...
    function swapBack() internal isSwapping {
        ...
        uint256 amountOfBackTokenDev = amountOfBackToken;
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "mspCoin: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "mspCoin: index out of bounds");
    ...
    function setSellFees(uint256 _devFee) external onlyOwner {
        devFeeSell = _devFee;
        totalFeeSell = _devFee;
    }

    function setFeeReceiver(address _devWallet) external onlyOwner {
