...
interface ITreasury {
  ...
  event AcceptedFundsDistributed(uint amountPpk, uint amountTournaments);

	function getAfterdeathAddress() external view returns(address);
	function getLadderAddress() external view returns(address);
...
interface ISnookGame is IDescriptorUser {
  ...
  struct ExtendedDescriptor {
	...
	function getSnookPriceInUSDC() external pure returns (uint);
  function getSNOOKAddress() external view returns (address);
  function getSnookStateAddress() external view returns (address);
  function getAfterdeathAddress() external view returns (address);
...
// File @openzeppelin/contracts-upgradeable/utils/ContextUpgradeabl[email protected]
...
contract Treasury is ITreasury, Initializable {
  ...
  uint public constant MINT_DIV            = 1000000000000000000000;
  uint public constant MINT_PPK_MUL        =  833333333333333333333; // 0.60 * (amountSentToTreasury / 0.72)
  uint public constant MINT_TOURNAMENTS_MUL=  166666666666666666667; // 0.12 * (amountSentToTreasury / 0.72)
  ...
  address private _game;
  address private _afterdeath;
  uint private _ppkBalance; 
  uint private _tournamentsBalance; 
  ...
  function _distributeAcceptedFunds(uint amount) internal {
    ...
    uint amountPpk = amount - amountTournaments; 
    _ppkBalance += amountPpk;
    emit AcceptedFundsDistributed(amountPpk, amountTournaments);
  }

  function acceptMintFunds(uint amount) external override onlySnookGame {
    require(_usdc.transferFrom(_game, address(this), amount), 'Treasury: transfer from SnookGame failed');
  ...
  function acceptResurrectionFunds(uint amount) external override onlyAfterdeath {
    require(_usdc.transferFrom(_afterdeath, address(this), amount), 'Treasury: transfer from Afterdeath failed');
  ...
  { 
	...
	function initialize2(address ladder) public
  { 
		require(_ladder == address(0), 'Already inited');
		_ladder = ladder;
  }
	...
	function getLadderAddress() external override view returns(address) {
		return _ladder;
	}

	function getGameAddress() external override view returns(address) {
		return _game;
	}

	function getUsdcAddress() external override view returns (address) {
  ...
  function _claimPrizeFor(address account, uint seasonNumber) internal {
    ...
    _usdc.transfer(winner.account, winner.prizeAmount);
