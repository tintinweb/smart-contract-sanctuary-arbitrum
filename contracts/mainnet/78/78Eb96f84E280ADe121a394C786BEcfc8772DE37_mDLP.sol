    ...
    PausableUpgradeable
{
    ...
    function convertWithLp(
        ...
        // IRadiantStaking(radiantStaking).stakeLp(_amount); // let's not stake right away to avoid lock position increase too much
    ...
    function initZap(address _weth, address _dlpPoolHelper, address _ethOracle, uint256 _ethLPRatio) external onlyOwner {
        weth = IWETH(_weth);
