...
contract Positions is Roles {
    ...
    event MarginIncreased(
        address indexed user, address indexed asset, string market, uint256 marginDiff, uint256 positionMargin
    );
    ...
    function increasePosition(uint256 orderId, uint256 price, address keeper) public onlyContract {
        ...
        int256 tempUsdAmount = _getUsdAmount(order.asset, int256(order.fee));

        // Increment reward
        rewards.incrementReward(order.user, uint256(tempUsdAmount));
    ...
    function decreasePosition(uint256 orderId, uint256 price, address keeper) external onlyContract {
        ...
        require(
            lastIncreased[order.user][order.market] < block.timestamp - DS.getUint("minPositionHoldTime"),
            "!min-hold-time"
        );
    ...
    function closePositionWithoutProfit(address _asset, string calldata _market) external {
        ...
        (int256 pnl,) =
            getPnL(_asset, _market, position.isLong, price, position.price, position.size, position.fundingTracker);
    ...
    function removeMargin(address asset, string calldata market, uint256 margin) external ifNotPaused {
        ...
        (int256 upl,) =
            getPnL(asset, market, position.isLong, price, position.price, position.size, position.fundingTracker);

        if (upl < 0) {
            uint256 absUpl = uint256(-1 * upl);
            require(
                absUpl < (remainingMargin * (BPS_DIVIDER - positionStore.removeMarginBuffer())) / BPS_DIVIDER, "!upl"
            );
        }
 ...
 *         anyone can run, execute orders as they are submitted to 
...
contract Processor is Roles, ReentrancyGuard {
    ...
    event UserSkipped(
        address indexed user, address indexed asset, string market, uint256 price, uint256 publishTime, string reason
    );
    ...
    function executeOrders(uint256[] calldata orderIds, bytes[] calldata priceUpdateData)
        external
        payable
        nonReentrant
        ifNotPaused
    ...
    function _executeOrder(uint256 orderId, uint256 price, bool withChainlink, address keeper)
        internal
    ...
    function selfLiquidatePosition(address user, address asset, string memory market)
        external
        nonReentrant
        ifNotPaused
    {
    ...
    function liquidatePositions(
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            (bool status, string memory reason) =
                _liquidatePosition(users[i], assets[i], markets[i], price, false, msg.sender);
            if (!status) {
    ...
    function _liquidatePosition(
        ...
        (int256 pnl,) = positions.getPnL(
            asset, market, position.isLong, price, position.price, position.size, position.fundingTracker
        );
    ...
    function _boundPriceWithChainlink(uint256 maxDeviation, uint256 chainlinkPrice, uint256 price)
        internal
        pure
        returns (bool)
    {
...
contract Rewards is Roles {
    ...
    uint256 public maxDailyReward = 9000 * UNIT;

    address public tokenPoint;
    ...
    function link() external onlyGov {
        ...
        tokenPoint = DS.getAddress("TokenPoint");
    }
    ...
    function incrementReward(address user, uint256 feeUsd) public onlyContract {
        ...
        // uint256 thisPrice = 1 * UNIT; //1eth
        // uint256 amount = (UNIT * feeUsd * feeBps) / (thisPrice * BPS_DIVIDER);
        uint256 amount = (feeUsd * feeBps) / BPS_DIVIDER;
        ...
        dailyRewards += amount;
        rewardStore.incrementReward(user, tokenPoint, amount);

        emit RewardIncremented(user, tokenPoint, amount);
    }

    function claimReward() external {
        address user = msg.sender;
        uint256 userReward = rewardStore.getReward(user, tokenPoint);
        if (userReward == 0) return;
        rewardStore.decrementReward(user, tokenPoint, userReward);
        fundStore.transferOut(tokenPoint, user, userReward);
        emit RewardClaimed(user, tokenPoint, userReward);
    }
}
...
/**
 * @title  Staking
 * @notice Stake GOVTOKEN to receive rewards
 */
contract Staking is Roles {
    ...
    // Events
    event GOVTOKENStaked(address indexed user, uint256 amount);
    event GOVTOKENUnstaked(address indexed user, uint256 amount);
    ...
    function link() external onlyGov {
        ...
        fundStore = FundStore(payable(DS.getAddress("FundStore")));
        stakingStore = StakingStore(DS.getAddress("StakingStore"));
        govToken = DS.getAddress("GOVTOKEN");
    }

    /// @notice Stake `amount` of GOVTOKEN to receive rewards
    function stake(uint256 amount) external {
        ...
        fundStore.transferIn(govToken, msg.sender, amount);

        emit GOVTOKENStaked(msg.sender, amount);
    }

    /// @notice Unstake `amount` of GOVTOKEN
    function unstake(uint256 amount) external {
        ...
        fundStore.transferOut(govToken, msg.sender, amount);

        emit GOVTOKENUnstaked(msg.sender, amount);
    }
    ...
    function getClaimableReward(address asset, address account) public view returns (uint256) {
        ...
        uint256 govTokenSupply = stakingStore.getTotalSupply();
        if (govTokenSupply == 0) return currentClaimableReward;

        uint256 _rewardPerTokenStored =
            stakingStore.getRewardPerTokenSum(asset) + (stakingStore.getPendingReward(asset) * UNIT) / govTokenSupply;
        if (_rewardPerTokenStored == 0) return currentClaimableReward; // no rewards yet

        uint256 govTokenBalance = stakingStore.getBalance(account);

        return currentClaimableReward
            + (govTokenBalance * (_rewardPerTokenStored - stakingStore.getPreviousReward(asset, account))) / UNIT;
    }
...
contract TokenLP is ERC20, ERC20Burnable, AccessControl {
    uint256 private constant swV = 1000101;
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    constructor(address defaultAdmin, address minter) ERC20("SpaceWhale LP", "S-WHALE-LP") {
        _grantRole(DEFAULT_ADMIN_ROLE, defaultAdmin);
        _grantRole(MINTER_ROLE, minter);
    }
...
contract PoolFarmEth is ReentrancyGuard {
    uint256 private constant swV = 1000101;
	...
	function timeLeft() public view returns (uint) {
		// require(finishAt > block.timestamp, "times up");
    ...
    function exitRewards() external {
		uint thisRewardTokenBal = rewardsToken.balanceOf(address(this));
        rewardsToken.transfer(owner, thisRewardTokenBal);
    }
