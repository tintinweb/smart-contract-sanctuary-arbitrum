...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _transferTokenCheckFee(
        ...
        if (sender != address(AutoB) && recipient != address(AutoB) && antiMEV && !isContractExempt[sender] && !isContractExempt[recipient]) {
            address human = ensureOneHuman(sender, recipient);
    ...
    function setMaxWalletPercent(uint256 _maxWalletPercent) external onlyOwner {
        _maxWalletAmount = _totalSupply.mul(_maxWalletPercent).div(1000);
    }
    ...
    function setContractExempt(address account, bool value) public onlyOwner {
        isContractExempt[account] = value;
    }

    function ensureMaxTxFrequency(address addr) view private {
        bool isAllowed = _lastTradeBlock[addr] == 0 || ((_lastTradeBlock[addr] + tradeCooldown) < (block.number + 1));
        require(isAllowed, "Max tx frequency exceeded!");
    }

    function ensureOneHuman(address _from, address _to) private view returns (address) {
    ...
    function toggleAntiMEV(bool toggle) public onlyOwner {
        antiMEV = toggle;
    }

    function setAutoBContract(address _autoB) public onlyOwner {
        AutoB = IAutoB(_autoB);
        feeWallet = _autoB;
