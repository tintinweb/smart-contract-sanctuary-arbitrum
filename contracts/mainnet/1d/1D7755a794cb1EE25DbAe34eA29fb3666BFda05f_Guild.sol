...
contract Guild is VersionedInitializable, GuildStorage, IGuild {
    ...
    uint256 public constant GUILD_REVISION = 0x5;
...
library BorrowLogic {
    ...
    function _zTokenForMoneyAmount(
        ...
        uint256 assetDecimals = IERC20Detailed(address(zToken)).decimals();
        if (moneyDecimals >= assetDecimals) {
            moneyAmount = zToken.baseToNotional(zTokenAmount * (10**(moneyDecimals - assetDecimals)));
        } else {
            moneyAmount = zToken.baseToNotional(zTokenAmount) / (10**(assetDecimals - moneyDecimals));
        }
    }
    ...
    function _moneyForZTokenAmount(
        ...
        uint256 assetDecimals = IERC20Detailed(address(zToken)).decimals();
        if (moneyDecimals <= assetDecimals) {
            zTokenAmount = zToken.notionalToBase(moneyAmount * (10**(assetDecimals - moneyDecimals)));
        } else {
            zTokenAmount = zToken.notionalToBase(moneyAmount) / (10**(moneyDecimals - assetDecimals));
        }
    }
}
