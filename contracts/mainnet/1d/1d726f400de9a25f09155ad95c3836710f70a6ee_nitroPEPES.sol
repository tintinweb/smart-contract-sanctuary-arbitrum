...
The fastest, most based Pepe on Arbitrum. Are you ready, anon?
...
/**
 * NitroPepeBot interface for accepting transfer hooks
 */
interface INitroPepeBot {
    function txHook(address caller, address sender, address receiver, uint256 amount) external;
}

/**
 * Allow external contracts (NitroPepeBots) to hook into NitroPepe transactions
 */
abstract contract NitroPepeBotController is Ownable {
    struct NitroPepeBotInfo {
        bool bot;
        uint256 adrIndex;
    }
    
    mapping (address => NitroPepeBotInfo) _botsInfo;
    address[] _NitroPepeBots;
    uint256 _NitroPepeBotsCount;
    
    /**
     * Returns array of NitroPepeBots
     */
    function getBots() public view returns (address[] memory) {
        return _NitroPepeBots;
    }
    
    /**
     * Returns NitroPepeBot count
     */
    function getBotCount() public view returns (uint256) {
        return _NitroPepeBotsCount;
    }
    
    /**
     * Check if address is registered as NitroPepeBot
     */
     ...
     */
    function addNitroPepeBot(address bot) external owned {
        require(isContract(bot));
        _botsInfo[bot].bot = true;
        _botsInfo[bot].adrIndex = _NitroPepeBots.length;
        _NitroPepeBots.push(bot);
        _NitroPepeBotsCount++;
    }
     ...
     */
    function removeNitroPepeBot(address bot) external owned {
        require(isBot(bot));
        ...
        _NitroPepeBots[i] = _NitroPepeBots[_NitroPepeBots.length-1];
        _botsInfo[_NitroPepeBots[i]].adrIndex = i;
        _NitroPepeBots.pop();
    }
    
    /**
     * Call all NitroPepeBot hooks
     */
    function NitroPepeBotTxHook(address sender, address receiver, uint256 amount) internal {
        if(getBotCount() == 0){ return; }
        for(uint256 i=0; i<_NitroPepeBots.length; i++){ 
            /* 
             * Using try-catch ensures that any errors / fails in one of the NitroPepeBot contracts will not cancel the overall transaction
             */
            try INitroPepeBot(_NitroPepeBots[i]).txHook(msg.sender, sender, receiver, amount) {} catch {}
        }
    }
...
interface INitroPepeDistributive {
 ...
 */
abstract contract NitroPepeDistributive is INitroPepeDistributive, ERC20, Ownable, TracksHolders {
    using SafeMath for uint256;
...
/* > NitroPepe governs exchanges & pools globally */

/**
 * This contract burns tokens on transactions
 */
abstract contract Burnable is NitroPepeDistributive {
    using SafeMath for uint256;
 ...
 */
abstract contract WAGMI20 is IWAGMI20, Burnable, NitroPepeBotController {
    using SafeMath for uint256;
    ...
    /**
    * burns NitroPepe to the dead address 
    */
    ...
    */
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // call any hooks
        NitroPepeBotTxHook(sender, recipient, amount);
...
contract nitroPEPES is WAGMI20, RecordsCreation {
    using SafeMath for uint256;
    
    constructor() WAGMI20(4200000) {
        _name = "nitroPEPES";
        _symbol = "nitroPEPES";
    }
}