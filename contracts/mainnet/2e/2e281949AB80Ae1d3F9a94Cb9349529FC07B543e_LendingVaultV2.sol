...
interface ILendingVaultV2 {
  ...
  function approvedBorrowersList() external view returns (address[] memory);
  ...
  function withdraw(uint256 sharesAmt) external;
...
import { ILendingVaultV2 } from "../interfaces/lending/ILendingVaultV2.sol";
import { IWNT } from "../interfaces/tokens/IWNT.sol";
import { Errors } from "../utils/Errors.sol";

contract LendingVaultV2 is ERC20, ReentrancyGuard, Pausable, Ownable2Step, ILendingVaultV2 {
  ...
  EnumerableSet.AddressSet private approvedBorrowers;
  ...
  */
  function borrowAPRPerBorrower(address borrower) public view returns (uint256) {
    return _calculateInterestRate(borrower, totalBorrows, totalAvailableAsset());
  }

  /**
    * @notice Returns the aggregated average borrow APR for this vault
    * @return Current borrow rate on aggregate/average in 1e18
  */
  function borrowAPR() public view returns (uint256) {
    uint256 _borrowAPR;

    for (uint256 i = 0; i < approvedBorrowers.length(); i++) {
      _borrowAPR += borrowAPRPerBorrower(approvedBorrowers.at(i));
    }

    return _borrowAPR / approvedBorrowers.length();
  }
  ...
  */
  function approvedBorrower(address borrower) public view returns (bool) {
    return approvedBorrowers.contains(borrower);
  }
  ...
  */
  function approvedBorrowersList() public view returns (address[] memory) {
    return approvedBorrowers.values();
  }
  ...
  */
  function withdraw(uint256 sharesAmt) public nonReentrant {
  ...
  */
  function _onlyBorrower() internal view {
    if (!approvedBorrowers.contains(msg.sender)) revert Errors.OnlyBorrowerAllowed();
  }
  ...
  */
  function _pendingInterest(address borrower, uint256 assetAmt) internal view returns (uint256) {
    ...
    uint256 _ratePerSec = _calculateInterestRate(borrower, totalBorrows, _floating) / SECONDS_PER_YEAR;
  ...
  */
  function _pendingInterests(uint256 assetAmt) internal view returns (uint256) {
    ...
    for (uint256 i = 0; i < approvedBorrowers.length(); i++) {
      pendingInterests_ += _pendingInterest(approvedBorrowers.at(i), assetAmt);
    }
  ...
  */
  function approveBorrower(address borrower) external onlyOwner {
    if (approvedBorrowers.contains(borrower)) revert Errors.BorrowerAlreadyApproved();

    approvedBorrowers.add(borrower);
  }
  ...
  */
  function revokeBorrower(address borrower) external onlyOwner {
    ...
    approvedBorrowers.remove(borrower);
  }
  ...
  */
  function emergencyPause() external whenNotPaused onlyKeeper {
    _pause();
