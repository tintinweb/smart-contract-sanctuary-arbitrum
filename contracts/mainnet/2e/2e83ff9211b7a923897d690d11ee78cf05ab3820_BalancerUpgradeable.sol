...
contract BalancerUpgradeable is IBalancer, ERC20Upgradeable, AccessManagedUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
     ...
     * 
     */
    function invest(address targetAdapter, address receiver) external override whenNotPaused nonReentrant returns (uint sharesWithChargedFee) {
        return _invest(targetAdapter, receiver, receiver);
    }

    function invest(address targetAdapter, address receiver, address dustReceiver) external override whenNotPaused nonReentrant returns (uint sharesWithChargedFee) {
        return _invest(targetAdapter, receiver, dustReceiver);
    }
    ...
    function _invest(address targetAdapter, address receiver, address dustReceiver) internal returns (uint sharesWithChargedFee) {
        if (!$isActiveAdapter[targetAdapter]) {
            revert InvalidAdapter(targetAdapter);
        }

        (uint sharesAdded, uint valuePrior, uint valueAdded) = _processInvestOrCompound(targetAdapter, dustReceiver, false, new SwapInfo[](0));
    ...
    function _processInvestOrCompound(
        ...
        for (uint i=0; i < l; i++) {
            ...
            if (targetAdapter == address(adapter)) {
                ...
                if (isCompound) {
                   ...
                   (valueBefore, valueAfter) = adapter.invest(dustReceiver);
                }
        ...
        if (isNewAdapter) {
            ...
            if (isCompound) {
                ...
                (, valueAfter) = adapter.invest(dustReceiver);
            }
    ...
    function donate(uint amount) external {
        ABRA.safeTransferFrom(msg.sender, address(this), amount);
        _lockToken(amount);
    ...
    function rebalance(
        ...
        uint lpsLeft = totalLp - lpAmount;
        if (lpsLeft == 0) {
            $chargedAdapters.remove(address(fromAdapter));
        }
        ...
        if (vb == 0 && va > 0) {
            $chargedAdapters.add(address(toAdapter));
        }
    ...
    function upgradeAdapter(
        ...
        $chargedAdapters.remove(address(sourceAdapter));
        ...
        if (targetValueBefore == 0 && targetValueAfter > 0) {
            $chargedAdapters.add(address(targetAdapter));
        }
    ...
    function compound(
        ...
        (uint sharesToMint, uint valuePrior, uint valueAdded) = _processInvestOrCompound(adapter, address(0), true, swaps);
    ...
    function compoundToExternalBalancer(
        ...
        for (uint i = 0; i < transfers.length; i++) {
            TransferInfo calldata transfer = transfers[i];
            SafeERC20.safeTransfer(IERC20(transfer.token), address(externalAdapter), transfer.amount);
        }

        IBalancer(externalBalancer).invest(externalAdapter, address(SWAP_EXECUTOR), externalBalancer);

        tokensBought = SWAP_EXECUTOR.defaultSwap(externalBalancer, address(ABRA), minTokensBought);

        uint fee = tokensBought * performanceFee / PERCENTAGE_COEFFICIENT;
        ABRA.safeTransfer($feeReceiver, fee);

        uint tv = totalValue();        
        emit Compound(adapter, tv, 0, tokensBought, fee);
        _lockToken(tokensBought - fee);
    }

    function takePerformanceFee(uint112, uint256, uint32) 
        external 
        override 
        restricted
        nonReentrant 
