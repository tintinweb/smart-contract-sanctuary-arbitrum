...
interface IFeeStrategy {
  ...
  function calculateUnwindFees(uint256 amount)
    external
    view
    returns (uint256 _unwindFees);
}
...
contract DopexAtlanticPutsPool is
    Pausable,
    ReentrancyGuard,
    AccessControl,
    ContractWhitelist
{
    ...
    uint256 public constant OPTION_TOKEN_DECIMALS = 18;

    /// @dev Number of decimals for max strikes
    uint256 public constant STRIKE_DECIMALS = 8;

    /// @dev Max strike weights divisor/multiplier
    uint256 public immutable WEIGHTS_MUL_DIV = 1e18;
    ...
    /**
     * @notice Total liquidity in a max strike in a epoch
     * @dev    epoch => maxStrike => liquidity
     */
    mapping(uint256 => mapping(uint256 => uint256))
        public totalEpochMaxStrikeLiquidity;
    ...
    /**
     * @notice Total collateral used for writing options from a max strike in a epoch
     * @dev    epoch => max strike => active collateral
     */
    mapping(uint256 => mapping(uint256 => uint256))
        public totalEpochMaxStrikeActiveCollateral;

    /**
     * @notice Total unlocked collateral from a max strike in a epoch
     * @dev    epoch => maxstrike => unlocked amount
     */
    mapping(uint256 => mapping(uint256 => uint256))
        public totalEpochMaxStrikeUnlockedCollateral;

    /**
     * @notice Total collateral unlocked in a epoch
     * @dev    epoch => unlocked amount
     */
    mapping(uint256 => uint256) public totalEpochUnlockedCollateral;

    /**
     * @notice Total collateral used for writing options in a epoch
     * @dev    epoch => active collateral
     */
    mapping(uint256 => uint256) public totalEpochActiveCollateral;
    ...
    uint256 public fundingRateUpdateTimes;

    uint256 public fundingRate = 50;
    ...
    event VaultConfigurationSet(VaultConfiguration config);
    ...
    function setManagedContract(address _managedContract, bool _setAs)
        external
    ...
    function expireEpoch() external nonReentrant {
        ...
        _validate(
            block.timestamp <=
                epochExpiry + vaultConfiguration.expireDelayTolerance,
            2
        );
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit VaultConfigurationSet(_vaultConfig);
    }
    ...
    function updateExpireDelayTolerance(uint256 _expireDelayTolerance)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        vaultConfiguration.expireDelayTolerance = _expireDelayTolerance;
    ...
    function deposit(
        ...
        totalEpochMaxStrikeLiquidity[epoch][_maxStrike] += _liquidity;
     ...
     * @param _user   Address of the user to purchase options for
     */
    function purchase(
        uint256 _strike,
        uint256 _amount,
        ...
        uint256 fees = calculatePurchaseFees(_strike, _amount);
        ...
        totalEpochCummulativeLiquidity[epoch] -= collateralRequired;
        totalEpochActiveCollateral[epoch] += collateralRequired;
    ...
    function _newPurchasePosition(
        ...
        userOptionsPurchases[purchaseId] = OptionsPurchase(
            _epoch,
            _putStrike,
            _amount,
            0,
    ...
    function _squeezeMaxStrikes(
        ...
        while (liquidityFromMaxStrikes != collateralRequired) {
            ...
            totalEpochMaxStrikeActiveCollateral[epoch][
                nextStrike
            ] += liquidityProvided;
    ...
    function _squeezeMaxStrikeCheckpoints(
        ...
        uint256 premium,
        uint256 purchaseId
    ) private returns (uint256 liquidityProvided) {
        ...
        uint256 endIndex = epochMaxStrikeCheckpointsLength[epoch][maxStrike] -
            1;
        ...
        while (
            ...
            _requiredLiquidity = collateralRequired - liquidityProvided;
            ...
            if (availableLiquidity >= _requiredLiquidity) {
                ...
                // if (liquidityProvidedFromCurrentMaxStrike <= _requiredLiquidity)
     ...
     * @return collateralWithFee Amount of collateral unlocked plus fees
     */
    function unlockCollateral(uint256 purchaseId, address to)
        external
        nonReentrant
        returns (uint256 collateralWithFee)
    {
        _isEligibleSender();
        _whenNotPaused();
        _isManagedContract();
        ...
        _validate(to != address(0), 15);
        _validate(_userOptionsPurchase.user == msg.sender, 12);
        _validate(_userOptionsPurchase.fundingRate == 0, 16);

        uint256 funding = calculateFunding(collateralAccess);

        _userOptionsPurchase.fundingRate = fundingRate;

        for (uint256 i = 0; i < _userOptionsPurchase.strikes.length; ) {
            _unlockCollateral(
                epoch,
                 ...
                 */
                (_userOptionsPurchase.weights[i] * collateralAccess) /
                    WEIGHTS_MUL_DIV,
                /**
                 *     contribution% * funding
                 */
                (funding * _userOptionsPurchase.weights[i]) / WEIGHTS_MUL_DIV,
        ...
        /// @dev Deduct funding before transfering out collateral
        collateralWithFee = collateralAccess - funding;

        userOptionsPurchases[purchaseId] = _userOptionsPurchase;

        totalEpochUnlockedCollateral[epoch] += collateralAccess;

        /// @dev Transfer out collateral
        _safeTransfer(addresses.quoteToken, to, collateralWithFee);
    }
    ...
    function _unlockCollateral(
        ...
        totalEpochMaxStrikeUnlockedCollateral[epoch][
            maxStrike
        ] += collateralAmount;
     ...
     */
    function relockCollateral(uint256 purchaseId)
        external
        returns (uint256 collateralToCollect)
    {
        ...
        _validate(_userOptionsPurchase.fundingRate != 0, 17);
        _validate(_userOptionsPurchase.epoch == epoch, 17);

        uint256 collateralBorrowed = strikeMulAmount(
            _userOptionsPurchase.optionStrike,
            _userOptionsPurchase.optionsAmount
        );
        uint256 funding = calculateFunding(
            collateralBorrowed,
            _userOptionsPurchase.fundingRate
        );
        uint256 fundingToDeduct;

        if (privateMode) {
            fundingToDeduct = calculateFunding(collateralBorrowed);
            funding = 0;
        }
        ...
        for (uint256 i = 0; i < _userOptionsPurchase.strikes.length; ) {
            _relockCollateral(
                epoch,
                _userOptionsPurchase.strikes[i],
                ((collateralBorrowed * _userOptionsPurchase.weights[i]) /
                    WEIGHTS_MUL_DIV),
                (_userOptionsPurchase.weights[i] * fundingToDeduct) /
                    WEIGHTS_MUL_DIV,
                (_userOptionsPurchase.weights[i] * funding) / WEIGHTS_MUL_DIV,
                _userOptionsPurchase.checkpoints[i]
            );
        ...
        userOptionsPurchases[purchaseId].fundingRate = 0;
        collateralToCollect = collateralBorrowed - fundingToDeduct + funding;
        totalEpochUnlockedCollateral[epoch] -= collateralBorrowed;
     ...
     * @param refund           Refund amount to deduct from funding earned
        ...
        returns (uint256 pnlToUser)
    {
        ...
        for (uint256 i = 0; i < _userOptionsPurchase.strikes.length; ) {
            ...
            if (_userOptionsPurchase.fundingRate != 0) {
                _relockCollateral(
                    ...
                    (settlement * _userOptionsPurchase.weights[i]) /
                        WEIGHTS_MUL_DIV,
                    0,
                    (funding * _userOptionsPurchase.weights[i]) /
                        WEIGHTS_MUL_DIV,
                    _userOptionsPurchase.checkpoints[i]
                );
            } else {
     ...
     */
    function unwind(uint256 purchaseId)
        external
        returns (uint256 unwindAmount)
    {
        _whenNotPaused();
        ...
        unwindAmount =
            ...
            calculateUnwindFees(_userOptionsPurchase.optionsAmount);
    ...
    function getNextFundingRate() public view returns (uint256) {
        uint256 currentFundingRate = fundingRate;
        VaultConfiguration storage config = vaultConfiguration;
        uint256 nextFundingUpdateTime = fundingRateUpdateTimes +
            config.fundingInterval;
        if (block.timestamp <= nextFundingUpdateTime) {
            return currentFundingRate;
        }
        uint256 intervals = (block.timestamp - nextFundingUpdateTime) /
            config.fundingInterval;
        if (intervals == 0) {
            return currentFundingRate;
        }
        if (intervals == 1) {
            return currentFundingRate + config.fundingRateIncrement;
        } else {
            return
                currentFundingRate + (config.fundingRateIncrement * intervals);
        }
    }
    ...
    function updateFundingRate() public {
        ...
        if (intervals == 1) {
            fundingRate += config.fundingRateIncrement;
            fundingRateUpdateTimes += config.fundingInterval;
        } else {
            fundingRate += (config.fundingRateIncrement * intervals);
            fundingRateUpdateTimes += (config.fundingInterval * intervals);
        }
    }
        ...
        )
    {
        ...
        (userWithdrawableAmount, premium, funding, underlying) = _withdraw(
            depositId
        );
        ...
        returns (bool)
    {
        ...
        fundingRateUpdateTimes = block.timestamp;
    ...
    {
        return
            IFeeStrategy(addresses.feeStrategy).calculateUnwindFees(
                underlyingAmount
            );
    }
    ...
    {
        return
            IFeeStrategy(addresses.feeStrategy).calculateSettlementFees(pnl) /
            1e12;
    }
    ...
    /**
     * @notice Caclulate collateral utilization rate
     * @param  epoch           Epoch of the pool
     * @return utilizationRate
     */
    function getUtilizationRate(uint256 epoch) public view returns (uint256) {
        uint256 activeCollateral = totalEpochActiveCollateral[epoch];
        uint256 unlockedCollateral = totalEpochUnlockedCollateral[epoch];
    ...
    {
        funding =
            ((totalCollateral * (100000 + vaultConfiguration.baseFundingRate)) /
                100000) -
            totalCollateral;
    }
    ...
    function calculateFunding(
        uint256 totalCollateral,
        uint256 unlockFundingRate
    ) public view returns (uint256 funding) {
        funding =
            ((totalCollateral * (100000 + (fundingRate - unlockFundingRate))) /
                100000) -
            totalCollateral;
    }
     ...
     */
    function _isValidMaxStrike(uint256 maxStrike, uint256 epoch)
        private
        view
        returns (bool)
    {
    ...
    /**
     * @notice Creates a new checkpoint if checkpointIntervalTime has passed or adds
     *         current checkpoint
     * @param  epoch     Epoch of the pool
     ...
     */
        ...
        emit NewCheckpoint(epoch, maxStrike, block.timestamp, index);
    }
     ...
     */
    function _withdraw(uint256 _depositId)
        private
        returns (
        ...
        )
    {
        ...
        _validate(_userDeposit.depositor != address(0), 21);
        ...
        funding +=
            (_userDeposit.liquidity * _depositCheckpoint.fundingAccrued) /
     ...
     * @param _epoch  Epoch of the pool
     */
    function _addMaxStrike(uint256 _strike, uint256 _epoch) private {
        ...
        returns (uint256)
    {
        ...
        while (
            (next != 0) && ((_value < _getMaxStrikeValue(next, _epoch)) == true)
        ) {
    ...
    /**
     * @notice Get value of a valid max strike
     * @param _maxStrike Max strike value
     * @param _epoch     Epoch of the pool
     * @param _value     Value of the max strike (confirmed validity if value is returned)
     */
    function _getMaxStrikeValue(uint256 _maxStrike, uint256 _epoch)
        private
        view
        returns (uint256 _value)
    {
        if (isValidStrike[_epoch][_maxStrike]) {
            return _maxStrike;
        } else {
            return 0;
        }
    ...
    function whitelistUser(address _user, bool _whitelist)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function setWhitelistUserMode(bool _mode)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    modifier whitelistCheck() {
        if (isWhitelistUserMode) {
            require(whitelistedUsers[msg.sender], "AP: Not whitelisted");
        }
        _;
    }
}