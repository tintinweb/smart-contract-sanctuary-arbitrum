...
contract RumVault is IRumVault, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    modifier InvalidID(uint256 positionId, address user) {
        require(positionId < positionInfo[user].length, "RUM: Id !valid");
        _;
    }
    ...
    modifier onlyhlpRewardHandler() {
        require(msg.sender == strategyAddresses.hlpRewardHandler, "!hlp reward handler");
        _;
    }
    ...
    function setLeverageParams(
        ...
        // require(_DTVSlippage <= 1000, "DTVSlippage > 1000");
        // require(_debtValueRatio <= 1e18, "DVR > 1e18");
    ...
    function updateDebtAdjustment() external onlyKeeper {
        require(getUtilizationRate() > (leverageParams.DTVLimit), "UR !greater than DTVLimit");
        require(
            block.timestamp - debtAdjustmentValues.time > leverageParams.debtAdjustmentInterval,
            "Invalid time for DA"
        );
    ...
    function fulfillOpenPosition(uint256 orderId, uint256 _actualOut) public onlyKeeper returns (bool) {
        ...
        require(isWithinSlippage(_actualOut, expectedOut, feeConfiguration.slippageTolerance), "RUM:  !within slippage tolerance");
    ...
    function requestClosePosition(uint32 _positionID, address _user) external payable InvalidID(_positionID, _user) nonReentrant {
        ...
        if (allowedClosers[msg.sender]) {
            require(block.timestamp >= positionOpenTimestamp[_user][_positionID] + vaultMaturityTime, "RUM: !Mature date");
        }
    ...
    function fulfillClosePosition(uint256 _orderId, uint256 _returnedUSDC) public onlyKeeper returns (bool) {
        ...
        // wr.fullDebtValue  is the leverageAmountWihtDA;
        (, , , , wr.fullDebtValue) = getPosition(wr.positionID, wr.user, getHLPPrice(false));

        wr.returnedUSDC = _returnedUSDC;

        extraData.positionPreviousValue = wr.fullDebtValue + _positionInfo.deposit; //_convertHLPToUSDC(_positionInfo.position, _positionInfo.buyInPrice);
        ...
        uint256 debtFromDA = wr.fullDebtValue > _positionInfo.leverageAmount ? wr.fullDebtValue - _positionInfo.leverageAmount : wr.fullDebtValue;
        waterRepayment = waterRepayment - debtFromDA;
        _transferProfit(waterProfits + debtFromDA, mFee);
    ...
    function _transferProfit(uint256 _waterProfit, uint256 _mFee) internal {
        if (_waterProfit > 0) {
            IERC20Upgradeable(strategyAddresses.USDC).safeTransfer(feeConfiguration.waterFeeReceiver, _waterProfit);
        }
        if (_mFee > 0) {
    ...
    function requestLiquidatePosition(address _user, uint256 _positionID) external payable nonReentrant {
        ...
        require(!_positionInfo.isLiquidated, "RUM: liquidated");
        require(_positionInfo.user != address(0), "RUM: request !exist");
        require(!inCloseProcess[_user][_positionID], "RUM: request ongoing");
        require(msg.value >= keeperInfo.keeperFee + feeConfiguration.hlpFee, "RUM: !fee");

        (, , , uint256 currentDTVWithDA, ) = getPosition(_positionID, _user, getHLPPrice(false));

        require(currentDTVWithDA >= (leverageParams.DTVLimit * leverageParams.DTVSlippage) / 1000, "!reached");
    ...
    function transferFrom(address from, address to, uint256 amount) public override returns (bool) {
        address spender = _msgSender();
        require(allowedSenders[from] || allowedSenders[to] || allowedSenders[spender], "ERC20: transfer !allowed");
        _spendAllowance(from, spender, amount);
        _transfer(from, to, amount);
        return true;
    }
    ...
    function transfer(address to, uint256 amount) public override returns (bool) {
        ...
        require(allowedSenders[ownerOf] || allowedSenders[to], "ERC20: transfer !allowed");
    ...
    function withdrawArb(address _arbToken, address _receiver, uint256 _amount) external onlyOwner {
        require(_arbToken != strategyAddresses.USDC, "!Withdraw USDC");

        uint256 withdrawAmount = _amount == 0 ? IERC20Upgradeable(_arbToken).balanceOf(address(this)) : _amount;

        IERC20Upgradeable(_arbToken).safeTransfer(_receiver, withdrawAmount);
    }
