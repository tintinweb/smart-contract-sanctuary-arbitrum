...
contract VaultSteerBase is ERC20, IVaultSteerBase {
  ...
  function deposit(uint256 amount0, uint256 amount1) external override {
    ...
    uint256 beforeBal = IERC20(steerVault).balanceOf(address(this));

    //deposit to Steer Periphery contract
    ISteerPeriphery(steerPeriphery).deposit(steerVault, amount0, amount1, 0, 0, address(this));

    // extract after deposit balance of steerVault
    uint256 afterBal = IERC20(steerVault).balanceOf(address(this));
...
contract VaultSteerSushiUsdcUsdce is VaultSteerBase {
  constructor(
    address _governance,
    address _timelock
  ) VaultSteerBase(0x3eE813a6fCa2AaCAF0b7C72428fC5BC031B9BD65, _governance, _timelock) {}
}