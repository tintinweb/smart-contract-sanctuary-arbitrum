...
import {IAlgebraPoolV2} from "../../interfaces/algebra/IAlgebraPoolV2.sol";
...
/// @title Beefy Passive Position Manager. Version: Camelot
/// @author weso, Beefy
/// @notice This is a contract for managing a passive concentrated liquidity position on Algebra / Camelot.
contract StrategyPassiveManagerCamelot is StratFeeManagerInitializable, IStrategyConcLiq {
    ...
    /// @notice Stored tick spacing for the pool.
    ...
    function initialize (
        ...
        pool = _pool;
        quoter = _quoter;
        lpToken0 = IAlgebraPoolV2(_pool).token0();
        lpToken1 = IAlgebraPoolV2(_pool).token1();
        ...
        twapInterval = 120;
        storedTickSpacing = _tickDistance();
    ...
    function _addLiquidity() private {
        ...
        if (liquidity > 0 && amountsOk) {
            minting = true;
            IAlgebraPoolV2(pool).mint(address(this), address(this), positionMain.tickLower, positionMain.tickUpper, liquidity, "Beefy Main");
        }
        ...
        if (liquidity > 0) {
            minting = true;
            IAlgebraPoolV2(pool).mint(address(this), address(this), positionAlt.tickLower, positionAlt.tickUpper, liquidity, "Beefy Alt");
        }
    }
    ...
    function _removeLiquidity() private {
        ...
        (uint128 liquidity,,,,,) = IAlgebraPoolV2(pool).positions(keyMain);
        (uint128 liquidityAlt,,,,,) = IAlgebraPoolV2(pool).positions(keyAlt);

        // If we have liquidity in the positions we remove it and collect our tokens.
        if (liquidity > 0) {
            IAlgebraPoolV2(pool).burn(positionMain.tickLower, positionMain.tickUpper, liquidity);
            IAlgebraPoolV2(pool).collect(address(this), positionMain.tickLower, positionMain.tickUpper, type(uint128).max, type(uint128).max);
        }

        if (liquidityAlt > 0) {
            IAlgebraPoolV2(pool).burn(positionAlt.tickLower, positionAlt.tickUpper, liquidityAlt);
            IAlgebraPoolV2(pool).collect(address(this), positionAlt.tickLower, positionAlt.tickUpper, type(uint128).max, type(uint128).max);
        }
    }
    ...
    function _claimEarnings() private returns (uint256 fee0, uint256 fee1, uint256 feeAlt0, uint256 feeAlt1) {
        ...
        if (liquidity > 0) IAlgebraPoolV2(pool).burn(positionMain.tickLower, positionMain.tickUpper, 0);
        if (liquidityAlt > 0) IAlgebraPoolV2(pool).burn(positionAlt.tickLower, positionAlt.tickUpper, 0);

        // Collect fees from the pool. 
        (fee0, fee1) = IAlgebraPoolV2(pool).collect(address(this), positionMain.tickLower, positionMain.tickUpper, type(uint128).max, type(uint128).max);
        (feeAlt0, feeAlt1) = IAlgebraPoolV2(pool).collect(address(this), positionAlt.tickLower, positionAlt.tickUpper, type(uint128).max, type(uint128).max);
    ...
    */
    function balancesOfPool() public view returns (uint256 token0Bal, uint256 token1Bal, uint256 mainAmount0, uint256 mainAmount1, uint256 altAmount0, uint256 altAmount1) {
        ...
        (uint128 liquidity,,,,uint256 owed0, uint256 owed1) = IAlgebraPoolV2(pool).positions(keyMain);
        (uint128 altLiquidity,,,,uint256 altOwed0, uint256 altOwed1) = IAlgebraPoolV2(pool).positions(keyAlt);
    ...
    */
    function currentTick() public view returns (int24 tick) {
        (,tick,,,,,,) = IAlgebraPoolV2(pool).globalState();
    }
    ...
    */
    function sqrtPrice() public view returns (uint160 sqrtPriceX96) {
        (sqrtPriceX96,,,,,,,) = IAlgebraPoolV2(pool).globalState();
    }
    ...
    */
    function swapFee() external override view returns (uint256 fee) {
        (,,uint16 _fee0,uint16 _fee1,,,,) = IAlgebraPoolV2(pool).globalState();
        uint _fee = _fee0 >= _fee1 ? _fee0 : _fee1;
    ...
    */
    function _tickDistance() private view returns (int24) {
        return IAlgebraPoolV2(pool).tickSpacing();
    }
    ...
    */
    ...
    function _setTicks() private onlyCalmPeriods {
        if (storedTickSpacing != _tickDistance()) revert InvalidTickSpacing();
        int24 tick = currentTick();
    ...
    */
    function twap() public view returns (int56 twapTick) {
        ...
        (int56[] memory tickCuml,,,) = IAlgebraPoolV2(pool).getTimepoints(secondsAgo);
