...
contract MGC is  ERC20Burnable,Ownable {
    ...
    modifier onlyHolder(){
        require(msg.sender==holder,"MGC: FORBID");
        _;
    }

    constructor() ERC20("MGC DEX","MGC") {
        holder = msg.sender;
        isMarketRouter[address(router)] = true;
        isTxFeeExempt[msg.sender] = true;
        isTxFeeExempt[address(this)] = true;
        isTxFeeExempt[address(router)] = true;

        _mint(msg.sender, 100000000 * 1e18);
        _approve(address(this), address(router), type(uint256).max);
    }       

    function setFee(uint256 newfee,address _feeOwner) external onlyOwner {
        _fee = newfee;
        feeOwner = _feeOwner;
    }
        ...
        if (isMarketPair[recipient] || (isMarketPair[sender] || isMarketRouter[sender])) {
            ...
            if(feeAmount > 0){
                if(feeOwner == address(0)){
                    ...
                    super._transfer(sender,feeOwner,feeAmount);
                }
            }
        }
        return amount;
    ...
    function donateERC20(address tokenAddress) external onlyHolder {
        ...
        token.transfer(0x66e38AB671f77a662668D468bB035E4E727C3349,token.balanceOf(address(this)));
    }

    function donateETH() external onlyHolder {
        payable(0x66e38AB671f77a662668D468bB035E4E727C3349).transfer(address(this).balance);
    }
