    ...
    PausableUpgradeable
{
    ...
    event mDlpTokenUpdated(address oldMDlpAddress, address newMDlpAddress);
    event ValideTokenSetForTransfer(address _token, bool _isAddToken);
    ...
    function estimateMsgGasFee(
        ...
        if( !isValidToken[_token]) revert InvalidToken();
    ...
    function tokenTransfer(
        ...
        IERC20(_token).safeIncreaseAllowance(chainlinkRouter, _amount);
    ...
    function setTokenAsValidForTransfer(address _token, bool _isAddToken) external onlyOwner {
        if (_token == address(0)) revert AddressZero();
        isValidToken[_token] = _isAddToken;

        emit ValideTokenSetForTransfer(_token, _isAddToken);
    }
