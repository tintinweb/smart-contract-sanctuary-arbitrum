...
contract RubicStaking is IRubicStaking, ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getAmountWithMultiplier(uint256 amount, uint128 lockTime) private pure returns (uint256) {
        if (lockTime == 1 minutes) return (10 * amount) / 10;
        if (lockTime == 5 minutes) return (10 * amount) / 10;
        if (lockTime == 10 minutes) return (12 * amount) / 10;
        if (lockTime == 15 minutes) return (15 * amount) / 10;
        if (lockTime == 20 minutes) return (20 * amount) / 10;
