...
library PoolAddress {
    bytes32 internal constant POOL_INIT_CODE_HASH = 0x09c178be473df44d1de6970978a4fdedce1ce52a23b2b979754547f6b43a19a5;
...
contract VoterV3 is ReentrancyGuardUpgradeable {
    ...
    function _createGauge(address _pool, uint256 _gaugeType) internal returns (address _gauge, address _internal_bribe, address _external_bribe) {
        ...
        if(_gaugeType == 1) {
            /*address _pool_factory = PoolAddress.computeAddress();
            address _pool_address = PoolAddress.computeAddress();
        ...
        address _owner = governance;
        string memory _type =  string.concat("Chronos LP Fees: ", IERC20(_pool).symbol() );
        _internal_bribe = IBribeFactory(bribefactory).createBribe(_owner, tokenA, tokenB, _type, true);

        _type = string.concat("Chronos Bribes: ", IERC20(_pool).symbol() );
        _external_bribe = IBribeFactory(bribefactory).createBribe(_owner, tokenA, tokenB, _type, false);
