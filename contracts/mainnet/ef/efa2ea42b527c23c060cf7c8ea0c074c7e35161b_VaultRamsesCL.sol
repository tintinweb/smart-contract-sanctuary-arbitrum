...
import "../../ammPositions/RamsesCLPosition.sol";
import "../../interfaces/IVault.sol";


/// @notice Vault with specific parameters for handling Uniswap v3 positions
contract VaultRamsesCL is IVault, RamsesCLPosition, VaultBase {
  ...
  function withdrawAmm() internal override(RamsesCLPosition, VaultBase) returns (uint baseAmount, uint quoteAmount) {
    (baseAmount, quoteAmount) = RamsesCLPosition.withdrawAmm();
  }
  
  
  /// @notice Deposit in Amm
  function depositAmm(uint baseAmount, uint quoteAmount) internal override(RamsesCLPosition, VaultBase) returns (uint256 liquidity) {
    liquidity = RamsesCLPosition.depositAmm(baseAmount,  quoteAmount);
  }
  ...
  /// @notice Get Amm type 
  function ammType() public pure override(RamsesCLPosition, IVault, VaultBase) returns (bytes32 _ammType){
    return RamsesCLPosition.ammType();
  }
}
...
contract VaultBase is Commons, VaultConfigurator, ERC20("", ""), ReentrancyGuard, IVault, FeeStreamer {
  ...
  function name() public view virtual override returns (string memory) { 
    return string(abi.encodePacked("TaoTrade ", baseToken.symbol(), "-", quoteToken.symbol()));
  }
...
import "../../interfaces/IRamsesNonfungiblePositionManager.sol";
...
abstract contract RamsesCLPosition is AmmPositionBase {
  ...
  //Using RAMSES CL 
  IRamsesNonfungiblePositionManager private constant _nonFungiblePositionManager = IRamsesNonfungiblePositionManager( 0xAA277CB7914b7e5514946Da92cb9De332Ce610EF);
  IUniswapV3Factory private constant uniswapV3Factory = IUniswapV3Factory(0xAA2cd7477c451E703f3B9Ba5663334914763edF8);
  ...
  function claimFees() internal virtual returns (uint baseAmount, uint quoteAmount) {
    ...
    (uint fee0, uint fee1) = IRamsesNonfungiblePositionManager(nonFungiblePositionManager()).collect( 
  ...
  function withdrawAmm() internal virtual override returns (uint baseAmount, uint quoteAmount) {
    ...
    claimFees();
    (uint removed0, uint removed1) = IRamsesNonfungiblePositionManager(nonFungiblePositionManager()).decreaseLiquidity(
    ...
    if (removed0 > 0 || removed1 > 0){
      IRamsesNonfungiblePositionManager(nonFungiblePositionManager()).collect( 
        IRamsesNonfungiblePositionManager.CollectParams({
          tokenId: tokenId,
          recipient: address(this),
  ...
  function depositAmm(uint256 baseAmount, uint256 quoteAmount) internal virtual override returns (uint256 newLiquidity) {
    ...
    if (tokenId > 0){
      (uint128 nl,,) = IRamsesNonfungiblePositionManager(nonFungiblePositionManager()).increaseLiquidity(
  ...
  function _mint(uint amount0Desired, uint amount1Desired) internal virtual returns (uint _tokenId, uint newLiquidity){
    ...
    (_tokenId, newLiquidity,,) = IRamsesNonfungiblePositionManager(nonFungiblePositionManager()).mint( 
      IRamsesNonfungiblePositionManager.MintParams({
        ...
        recipient: address(this),
        deadline: block.timestamp,
        veRamTokenId: 0 // no veRAM support
      })
    );
  }
  ...
  function getLiquidity() internal virtual view returns (uint128 liquidity) {
    if (tokenId > 0) (,,,,,,, liquidity,,,,) = IRamsesNonfungiblePositionManager(nonFungiblePositionManager()).positions(tokenId);
  }
...
contract PositionManager is Commons, ERC721Enumerable, IPositionManager {
  ...
  // Pnl fee X4, default 5%
  uint public constant PNL_FEE = 500;

  constructor() ERC721("TaoTrade Positions", "FP") {}
  ...
  function closePosition(uint tokenId) external {
    ...
    if (posPnl > 0) {
      uint pnlFee = posPnl * PNL_FEE / 1e4;
      ERC20(positionToken).safeTransfer(IController(IVaultConfigurator(vault).controller()).treasury(), pnlFee);
      ERC20(positionToken).safeTransfer(owner, posPnl - pnlFee);
    }
  ...
  function name() public view virtual override(ERC721, IERC721Metadata) returns (string memory) { 
    return string(abi.encodePacked("TaoTrade Positions ", baseToken.symbol(), "-", quoteToken.symbol()));
  }
  /// @notice Get the symbol of this contract token
...
abstract contract AmmPositionBase is Commons {
  ...
  function poolPriceMatchesOracle() public virtual returns (bool isMatching) {
    ...
    isMatching = baseValue >= quoteValue * 90 / 100 && quoteValue >= baseValue * 90 / 100;
  }
