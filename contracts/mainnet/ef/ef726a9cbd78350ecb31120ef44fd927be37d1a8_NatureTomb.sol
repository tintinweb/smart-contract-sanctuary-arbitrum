...
contract NatureTomb is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 NATURETOMB-WETH LP -> NATURETOMB pool
    // uint256 public constant INITIAL_NATURETOMB_POOL_DISTRIBUTION = 0 ether;
    ...
    address public naturetombOracle;
    ...
    // Address of the NATURETOMB-WETH LP
    ...
    /**
     * @notice Constructs the NatureTomb ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress, uint256 _sellStartTime) public ERC20("Nature Tomb Token", "NATURETOMB") {
        // Mints 1 NatureTomb to contract creator for initial pool setup
        require(_taxRate < 100000, "tax equal or bigger to 100%");
    ...
    function _getNatureTombPrice() internal view returns (uint256 _naturetombPrice) {
        try IOracle(naturetombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("NatureTomb: failed to fetch NATURETOMB price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _naturetombPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_naturetombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setNatureTombOracle(address _naturetombOracle) public onlyOperatorOrTaxOffice {
        require(_naturetombOracle != address(0), "oracle address cannot be 0 address");
        naturetombOracle = _naturetombOracle;
    }
    ...
    /**
     * @notice Operator mints NATURETOMB to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of NATURETOMB to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentNatureTombPrice = _getNatureTombPrice();
            currentTaxRate = _updateTaxRate(currentNatureTombPrice);
            if (currentNatureTombPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // address _NatureTombPool
        // address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        // require(_NatureTombPool != address(0), "!_NatureTombPool");
        // require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_NatureTombpool, INITIAL_NATURE_POOL_DISTRIBUTION);
    ...
    function getRemainingTimeForNatureTombSellingEnable() public view returns(uint256) {
        ...
        /// @dev overrides transfer function to meet tokenomics of NatureTomb
