...
/*
THE WHITE LOTUS -> Developed in 5 days for a hackaton, use caution.
...
LOTUS was released in a fair way via Trader Joe, The purchase of LOTUS tokens does not constitute an investment contract or any form of investment advice. WHITE LOTUS makes no guarantees or promises about the future value or performance of the token, and the value of the token may fluctuate significantly.
*/
...
contract Lotus is ERC20, Ownable {
    using SafeTransferLib for address payable;
    using SafeMath for uint256;
    using PriceHelper for uint256;

    address MUSE_DAO = 0xd7f52f7ff142b94B8deFc7Da8d34a7C65407533a;
    ...
    function chargeTax(
        ...
        if (feesEnabled && !isRebalancing) {
            ...
            if (from == address(pair) && to != (address(this))) {
                uint256 devFee = calculateFee(_amount, 22); //0.22
                uint256 daoFee = calculateFee(_amount, 3); // 0.3

                balanceOf[owner()] += devFee;
                emit Transfer(from, owner(), devFee);

                balanceOf[MUSE_DAO] += daoFee;
                emit Transfer(from, MUSE_DAO, daoFee);
