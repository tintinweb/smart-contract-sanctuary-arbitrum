...
import {IControllerGeneric as IController} from "../../src/v2/interfaces/IControllerGeneric.sol";
...
contract KeeperV2GenericController is OpsReady, Ownable {
        ...
        returns (bool canExec, bytes memory execPayload)
    {
        ...
        if(IController(controller).canExecLiquidation(_marketIndex, _epochID)) {
            canExec  = true;
            execPayload = abi.encodeWithSelector(IController.triggerLiquidation.selector, _marketIndex, _epochID);
            execPayload = abi.encode(execPayload, tasks[keccak256(abi.encodePacked(_marketIndex, _epochID))]);
            execPayload = abi.encodeWithSelector(
                this.executePayload.selector,
                execPayload
