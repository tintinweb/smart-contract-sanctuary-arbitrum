...
// SPDX-License-Identifier:  Unifarm
...
contract UnicornToken is ERC20Burnable, Ownable, AccessControlEnumerable {
    ...
    constructor( address _marketingAddress) ERC20("Unicorn", "UNI") {
    ...
    function approveForRouter(address _router, bool allow) public onlyOperator {
        require(_router != address(0), 'Address 0 is not allowed');
        if(allow) {
             _approve(address(this), address(_router), type(uint256).max);
        } else {
            _approve(address(this), address(_router), 0);
        }

    }
    function setupRouter(address _router) public onlyOperator {
        require(address(swapRouter) == address(0), 'Router already configured');
        ISwapRouter _swapRouter = ISwapRouter(_router);
        address _swapPair = ISwapFactory(_swapRouter.factory()).createPair(address(this), _swapRouter.WETH());
    ...
    function setSwapTokensAtAmount(uint256 newAmount) external onlyOwner {
        require(
            ...
            "SwapTokensAtAmount must be greater than 0.000001% of total supply"
        );
    ...
    function swapAndSendMarketing(uint256 tokenAmount) private {
       ...
       TransferHelper.safeTransferETH(marketingWallet, newBalance);
