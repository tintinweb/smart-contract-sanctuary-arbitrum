    ...
    IPerpetualPoolFactory
{
    ...
    function setPerpetualPoolFactory(address _shareTokenFactory) external override onlyMaintainer {
     ...
     * @param   _fBrokerCollateralLotSize lot size for broker
     * @return Pool id
     */
    function createLiquidityPool(
        address _marginTokenAddress,
        ...
        int128 _fBrokerCollateralLotSize,
        int128 _fCeilPnLShare
    ) external override onlyMaintainer returns (uint8) {
        _validateTargetPoolSizeUpdateTime(_iTargetPoolSizeUpdateTime);
        _validateCeilPnLShare(_fCeilPnLShare);
        require(_fBrokerCollateralLotSize > 0, "invalid lot");

        iPoolCount++;
        LiquidityPoolData storage pool = liquidityPools[iPoolCount];
        pool.id = iPoolCount;

        pool.marginTokenDecimals = _validateMarginToken(_marginTokenAddress);
        pool.marginTokenAddress = _marginTokenAddress;
        pool.shareTokenAddress = shareTokenFactory.createShareToken(pool.id, _marginTokenAddress);
        pool.iTargetPoolSizeUpdateTime = _iTargetPoolSizeUpdateTime;
        pool.fBrokerCollateralLotSize = _fBrokerCollateralLotSize;
        pool.fCeilPnLShare = int32(_fCeilPnLShare >> 35);
     ...
     */
    function runLiquidityPool(uint8 _liqPoolID) external override onlyMaintainer {
        require(_liqPoolID > 0 && _liqPoolID <= iPoolCount, "pool index out of range");
        LiquidityPoolData storage liquidityPool = liquidityPools[_liqPoolID];
        require(!liquidityPool.isRunning, "pool already running");
    ...
    /**
     * Set the address of the 'library-contract' AMM Perp Logic
     * @param _AMMPerpLogic address of AMM Perp Logic contract
     */
    function setAMMPerpLogic(address _AMMPerpLogic) external override onlyGovernance {
        require(_AMMPerpLogic != address(0), "invalid address");
        ammPerpLogic = _AMMPerpLogic;
    }

    /**
     * Set parameter of pool
     * @param _poolId   id of the pool
     * @param _name     name of the parameter
     * @param _value    value of the parameter (int128)
     */
    function setPoolParam(
        uint8 _poolId,
        string memory _name,
        ...
        if (_isStringEqual(_name, "iTargetPoolSizeUpdateTime")) {
            //uint16
            uint16 iTargetPoolSizeUpdateTime = uint16(uint128(_value));
            _validateTargetPoolSizeUpdateTime(iTargetPoolSizeUpdateTime);
            pool.iTargetPoolSizeUpdateTime = iTargetPoolSizeUpdateTime;
        } else if (_isStringEqual(_name, "fBrokerCollateralLotSize")) {
            //int128
            int128 fBrokerCollateralLotSize = _value;
            require(fBrokerCollateralLotSize > 0, "invalid fBrokerCollateralLotSize");
            pool.fBrokerCollateralLotSize = fBrokerCollateralLotSize;
        } else if (_isStringEqual(_name, "fCeilPnLShare")) {
            _validateCeilPnLShare(_value);
            pool.fCeilPnLShare = int32(_value >> 35);
        } else {
            revert("parameter not found");
        }
        emit SetPoolParameter(_poolId, _name, _value);
    }
    ...
    function _validateTargetPoolSizeUpdateTime(uint16 _iTargetPoolSizeUpdateTime) internal pure {
        require(
            ...
            "invalid pool updt time"
        );
    }

    function _validateMarginToken(address _marginTokenAddress) internal view returns (uint8) {
        require(
            _marginTokenAddress != address(this) && _marginTokenAddress != address(0),
            "invalid marginTokenAddress"
        );
        return IERC20Metadata(_marginTokenAddress).decimals(); // reverts if not defined
    }

    function _validateCeilPnLShare(int128 _fCeilPnLShare) internal pure {
        require(_fCeilPnLShare >= 0, "ceil pnl share >=0");
        require(_fCeilPnLShare < ONE_64x64, "ceil pnl share <1");
    }
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[4] = this.setPoolParam.selector;
