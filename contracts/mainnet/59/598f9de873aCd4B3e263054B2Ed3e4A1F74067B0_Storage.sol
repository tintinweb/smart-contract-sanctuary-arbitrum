...
contract Storage is AccessControl, ReentrancyGuard, IStorage {
    ...
    function setRefRate(
        uint256 firstRefRate_,
        uint256 secondRefRate_
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (isInactive()) {
            revert ErrClosed();
        }
    ...
    function openRound(uint256 index_) external onlyRole(OPERATOR_ROLE) {
        if (!isActive()) {
            revert ErrClosed();
        }
        if (!_rounds[index_].defined) {
    ...
    function closeRound(uint256 index_) external onlyRole(OPERATOR_ROLE) {
        if (!_rounds[index_].defined) {
            revert ErrRoundUndefined(index_);
        }
    ...
    function setAuth(address user_, bool value_) external onlyRole(OPERATOR_ROLE) {
        _auth[user_] = value_;

        emit AuthUserUpdated(user_, value_);
    }

    function setAuthBatch(
    ...
    function setMax(uint256 amount_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (amount_ > MAX) {
    ...
    function setMin(uint256 amount_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (amount_ < MIN) {
    ...
    function setAuthLimit(uint256 amount_) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function getRefRates(address ref_) external view returns (uint256, uint256) {
        ...
        if (ref.defined) {
            return (
                Math.max(ref.firstRefRate, _firstRefRate),
                Math.max(ref.secondRefRate, _secondRefRate)
            );
        }
        return (_firstRefRate, _secondRefRate);
    }
    ...
    function getPrice(Option option_) public view returns (uint256) {
        if (_rounds[_currentRound].state == State.Opened) {
            return
                option_ == Option.Short
                    ? _rounds[_currentRound].sPrice
                    : _rounds[_currentRound].lPrice;
        }
        return 0;
    }
