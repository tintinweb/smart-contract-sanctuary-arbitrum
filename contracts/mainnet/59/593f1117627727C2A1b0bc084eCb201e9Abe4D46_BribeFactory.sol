...
contract Bribe is IBribe, MultiRewardsPoolBase {
    ...
    function getRewardForOwner(uint256 tokenId, address[] memory tokens) external override {
    ...
    function _tokenIdToAddress(uint256 tokenId) internal pure returns (address) {
...
abstract contract MultiRewardsPoolBase is Reentrancy, IMultiRewardsPool {
    ...
    mapping(address => mapping(uint256 => CheckpointLib.Checkpoint)) public checkpoints;
    ...
    /// @notice A record of balance checkpoints for each token, by index
    mapping(address => mapping(uint256 => CheckpointLib.Checkpoint)) public rewardPerTokenCheckpoints;
    ...
    function derivedBalance(address account) external view override returns (uint256) {
    ...
    function removeRewardToken(address token) external onlyOperator {
        ...
        // assume it will be Solunea + pool tokens
        uint256 i = 3;
        bool found = false;
        for (; i < length; i++) {
    ...
    function _earned(address token, address account) internal view returns (uint256) {
        ...
        if (_endIndex > 0) {
            for (uint256 i = _startIndex; i <= _endIndex - 1; i++) {
                ...
                (uint256 _rewardPerTokenStored0, ) = _getPriorRewardPerToken(token, cp0.timestamp);
                (uint256 _rewardPerTokenStored1, ) = _getPriorRewardPerToken(token, cp1.timestamp);
        ...
        (uint256 _rewardPerTokenStored, ) = _getPriorRewardPerToken(token, cp.timestamp);
        reward +=
            (cp.value *
                (_rewardPerToken(token) - Math.max(_rewardPerTokenStored, userRewardPerTokenStored[token][account]))) /
            PRECISION;
        return reward;
    }

    function _derivedBalance(address account) internal view virtual returns (uint256) {
    ...
    function _notifyRewardAmount(address token, uint256 amount) internal virtual lock {
    ...
    function _getPriorBalanceIndex(address account, uint256 timestamp) internal view returns (uint256) {
    ...
    function _getPriorSupplyIndex(uint256 timestamp) internal view returns (uint256) {
    ...
    function _getPriorRewardPerToken(address token, uint256 timestamp) internal view returns (uint256, uint256) {
        uint256 nCheckpoints = rewardPerTokenNumCheckpoints[token];
        if (nCheckpoints == 0) {
            return (0, 0);
        }
        mapping(uint256 => CheckpointLib.Checkpoint) storage cps = rewardPerTokenCheckpoints[token];
