...
library TickMath {
    ...
    function getSqrtRatioAtTick(int24 tick) internal pure returns (uint160 sqrtPriceX96) {
        ...
        uint256 maxTickUint = MAX_TICK < 0 ? uint256(-int256(MAX_TICK)) : uint256(int256(MAX_TICK));
        require(absTick <= maxTickUint, 'T');
...
abstract contract Helpers is DSMath, Basic {
        ...
        )
    {
        ...
        uint256 _amount0 = params.amtA == type(uint256).max
            ? getTokenBal(TokenInterface(params.tokenA))
            : params.amtA;
        uint256 _amount1 = params.amtB == type(uint256).max
...
contract ConnectUniswapV3Arbitrum is UniswapResolver {
