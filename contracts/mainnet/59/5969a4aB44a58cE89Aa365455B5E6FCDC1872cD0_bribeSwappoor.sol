...
contract bribeSwappoor is Initializable, AccessControlEnumerableUpgradeable {
    ...
    IFactory constant factory = IFactory(0xAAA20D08e59F6561f242b08513D36266C5A29415);
    ...
    ILpDepositor constant depositor = ILpDepositor(0x1863736c768f232189F95428b5ed9A51B0eCcAe5);
    ...
    function addBridgeToken(address token, address bridge, bool stable) external onlyRole(SETTER_ROLE) {
        require(token != weth, "Nope");
        tokenBridge[token] = bridge;
        checkStable[token][bridge] = stable;
    }

    function addBridgeTokenBulk(address[] calldata token, address[] calldata bridge, bool[] calldata stable) external onlyRole(SETTER_ROLE) {
    ...
    function removeBridge(address token, address bridge) external onlyRole(SETTER_ROLE) {
        require(token != weth);
    ...
    */
    function zapIn(bool isWeth, uint amountA) external {
        address tokenA;
        address tokenB;
        (tokenA, tokenB) = isWeth ? (weth, neadRam) : (neadRam, weth);
        address pair = _pairFor(tokenA, tokenB, false);

        IERC20(tokenA).transferFrom(msg.sender,address(this),amountA);
        (uint amountB, uint swapAmount) = _swapToLp(tokenA, tokenB, pair, amountA);

        IERC20(tokenA).transfer(pair,amountA - swapAmount);
        IERC20(tokenB).transfer(pair,amountB);
        uint liquidity = IPair(pair).mint(msg.sender);
        address pool = depositor.tokenForPool(pair);
        depositor.deposit(pool, liquidity);
        IERC20(pool).transfer(msg.sender, liquidity);
    }

    // @notice separate swap function specifically for zap() _swap is not compatible
    function _swapToLp(address tokenA, address tokenB, address pair, uint amount) internal returns (uint, uint) {
        uint fee = factory.pairFee(pair) * 10000;
        (,,uint reserve0, uint reserve1) = getMetadata(tokenA, tokenB, pair);
        (reserve0, reserve1) = tokenA < tokenB ? (reserve0, reserve1) : (reserve1, reserve0);
    ...
    function _swap(address tokenA, address tokenB, uint amountA, bool stable) internal returns(uint) {
        ...
        uint amountOut;
        amountOut = getAmountOut(amountA * fee, stable, reserve0, reserve1, decimals0,decimals1);
        // honestly ffs ppl shouldnt have to go through this, i hate external calls
        if(isTax[tokenA]) {
            uint pairBal = IERC20(tokenA).balanceOf(pair);
            amountOut = getAmountOut((pairBal - reserve0) * fee, stable, reserve0, reserve1, decimals0,decimals1);
        }
    ...
    function swapOptimal(address tokenA, address tokenB, uint amount) internal returns (uint) {
        ...
        if (bridge == nextBridge) {
            ...
            uint nextBridgeAmount = swapOptimal(bridge, nextBridge, bridgeAmountOut);
            stable = checkStable[nextBridge][tokenB];
    ...
    function _calcSwap(uint reserve0, uint amountA, uint fee) internal pure returns (uint){
        uint x = 20000 - (fee / 10000);
        uint y = (4 * (10000 - (fee / 10000)) * 10000);
        uint z = 2 * (10000 - (fee / 10000));
        return (MathUpgradeable.sqrt(reserve0 * (x * x * reserve0 + amountA * y)) - reserve0 * x) / z;
    }
