...
contract LPStaking is Ownable, IERC721Receiver {
    ...
    //Mapping to store the referral relationship between users.
    mapping(address user => address referral) private _referrals;
    ...
    */
     ...
     * @param _refRewardPer The percentage of rewards to be distributed to referral participants
    ...
    function stakeNFT(uint256 _tokenID, address _referral) external {
        ...
        address userRef = _referrals[sender];
        if (_helperNFT.ownerOf(_tokenID) != sender) revert NotOwner();
        if(_referral != address(0) && userRef == address(0)) {
            _referrals[sender] = _referral;
            userRef = _referral;
        ...
        tvl = tvl + liquidity;
        totalLPs = totalLPs + 1;
        if(userRef != address(0)) _sendRefRewards(userRef, liquidity);
