...
contract GLOBALDistributor is Ownable {
    ...
    address public signer = 0x033A4FefFD0d78EE7e4c10dFC2D2D945dA9CD53E;

    uint256 public GENERAL = 1_260_000_000 * 1e18;
    uint256 public constant REFFERALS = 126_000_000 * 1e18;
    ...
    function claim(uint256 nonce, bytes calldata signature, address referrer) payable public {
        ...
        require(_usedNonce[nonce] == false, "GLOBAL: nonce already used");
        require(_claimedUser[_msgSender()] == false, "GLOBAL: already claimed");
        require(isClaimingEnabled == true, "GLOBAL: claiming paused");
        require(path[msg.sender] >= route, "GLOBAL: cannot claim");

        _claimedUser[_msgSender()] = true;
        require(isValidSignature(signature), "GLOBAL: only auth claims");

        _usedNonce[nonce] = true;
        require(token.balanceOf(address(this)) >= GENERAL, "GLOBAL: airdrop has ended");
    ...
    function setSigner(address val) public onlyOwner() {
        require(val != address(0), "GLOBAL: val is the zero address");
        signer = val;
    }
