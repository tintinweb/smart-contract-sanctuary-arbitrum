...
contract AccountLiquidity is AccessControlEnumerableUpgradeable, ExponentialNoErrorNew, SumerErrors {
  ...
  ) public view returns (AccountGroupLocalVars[] memory, AccountGroupLocalVars memory) {
...
contract AccountLiquidityHarness is ExponentialNoErrorNew, SumerErrors {
  error AssetsAndMarketsShouldHaveSameLength();
  error AssetsAndIsCTokensShouldHaveSameLength();
  error AssetsAndSnapshotsShouldHaveSameLength();
  ...
  mapping(address => AccountSnapshot) public snapshots;

  constructor(
    AssetGroup[] memory groups,
    address[] memory _assets,
    Market[] memory _markets,
    bool[] memory _isCToken,
    AccountSnapshot[] memory _snapshots
  ) {
    for (uint256 i = 0; i < groups.length; i++) {
      setAssetGroup(groups[i]);
    }
    if (_assets.length != _markets.length) {
      revert AssetsAndMarketsShouldHaveSameLength();
    }
    for (uint256 i = 0; i < _markets.length; i++) {
      setMarket(_assets[i], _markets[i]);
    }

    setAssets(_assets);

    if (_assets.length != _isCToken.length) {
      revert AssetsAndIsCTokensShouldHaveSameLength();
    }
    for (uint256 i = 0; i < _isCToken.length; i++) {
      setIsCToken(assets[i], _isCToken[i]);
    }

    if (_assets.length != _snapshots.length) {
      revert AssetsAndSnapshotsShouldHaveSameLength();
    }
    for (uint256 i = 0; i < _snapshots.length; i++) {
      setSnapshot(assets[i], _snapshots[i]);
    }
  }
  ...
  function setMarket(address ctoken, Market memory market) public {
    markets[ctoken] = market;
  }

  function setAssetGroup(AssetGroup memory group) public {
    // append new group
    assetGroupIdToIndex[group.groupId] = uint8(assetGroups.length);
    assetGroups.push(group);
  }

  function setSnapshot(address ctoken, AccountSnapshot memory _snapshot) public {
    snapshots[ctoken] = _snapshot;
  }

  function setIsCToken(address ctoken, bool _isCToken) public {
    isCToken[ctoken] = _isCToken;
  }

  function emptyAssetGroup() external {
    for (; assetGroups.length > 0; ) {
      AssetGroup memory group = assetGroups[0];
      delete (assetGroupIdToIndex[group.groupId]);
      delete assetGroups[0];
    }
  }

  function getBool(bool val) public view returns (bool) {
    return val;
  }

  function getBoolFromMarket(Market memory m) public view returns (bool) {
    return m.isListed;
  }
  ...
  function getGroupSummary(
    ...
    for (uint256 i = 0; i < assetsGroupNum; i++) {
      AssetGroup memory g = assetGroups[i];
      groupVars[i] = AccountGroupLocalVars(
        g.groupId,
        0,
        0,
    ...
    for (uint256 i = 0; i < assets.length; ++i) {
      ...
      AccountSnapshot memory snapshot = snapshots[asset];

      uint256 depositBalance = snapshot.depositBalance;
      uint256 borrowBalance = snapshot.borrowBalance;
      uint256 exchangeRateMantissa = snapshot.exchangeRate;

      // Get price of asset
      uint256 oraclePriceMantissa = markets[asset].normalizedPrice;
      // normalize price for asset with unit of 1e(36-token decimal)
      Exp memory oraclePrice = Exp({mantissa: oraclePriceMantissa});
      ...
      // tokensToDenom = oraclePrice * exchangeRate * discourntRate
      Exp memory exchangeRate = Exp({mantissa: exchangeRateMantissa});
      Exp memory discountRate = Exp({mantissa: 1e18});
      Exp memory tokensToDenom = mul_(mul_(exchangeRate, oraclePrice), discountRate);
      ...
      uint8 index = assetGroupIdToIndex[assetGroupId];
  ...
  function getHypotheticalSafeLimit(
    ...
    Exp memory intraSafeLimit = Exp({mantissa: intraSafeLimitMantissa});
    Exp memory interSafeLimit = Exp({mantissa: interSafeLimitMantissa});
    bool targetIsSuToken = (cTokenModify != address(0)) && !isCToken[cTokenModify];
    uint256 interGroupLiquidity = sumLiquidity;
    uint256 intraGroupLiquidity = mul_ScalarTruncate(targetGroup.intraSuRate, targetGroup.suDepositVal);
