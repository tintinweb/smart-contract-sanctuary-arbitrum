...
contract GMXOracle {
  ...
  */
  function getAmountsOut(
    ...
    // TODO should we bother with zero address checks? since it costs gas
    // and if the addresses dont match the whole call will revert
    ...
    if (address(tokenIn) == address(0)) revert Errors.ZeroAddressNotAllowed();
    if (address(tokenIn) != address(longToken) &&
        address(tokenIn) != address(shortToken)) revert Errors.InvalidTokenInLPPool();
