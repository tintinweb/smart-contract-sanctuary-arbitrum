...
contract StrategyJonesUsdc is Strategy {
    ...
    function _rate(uint256 sha) internal view override returns (uint256) {
        ...
        uint256 ast = (ast0 + ast1) * (10000 - redeemFee) / 10000;
            ...
            if (signaledStablesEpoch > 0) {
                proxy.call(
                    ...
                    abi.encodeWithSelector(
                        vaultRouter.cancelStableWithdrawalSignal.selector, signaledStablesEpoch, false
                    )
                );
        ...
        if (bal < tar) {
            ...
            if (signaledStablesEpoch == 0) {
                ...
                bytes memory dat = proxy.call(
                    address(vaultRouter),
                    0,
                    abi.encodeWithSelector(vaultRouter.stableWithdrawalSignal.selector, sha, false)
                );
