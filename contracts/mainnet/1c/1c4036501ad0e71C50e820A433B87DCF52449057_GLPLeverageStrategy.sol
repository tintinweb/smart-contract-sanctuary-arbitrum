...
contract GLPLeverageStrategy is Initializable, OwnableUpgradeable, UUPSUpgradeable, GLPLeverageStrategyStorage {
    ...
    function _leverDown(uint256 collateralValue, uint256 borrowValue) internal returns (uint256) {
        ...
        uint256 maxRedeem = lendingPool.userCollateralAmount(address(this));
        ...
        uint256 borrowValueGLP = _usdToGLP(borrowValue);
        uint256 collateralAmount = lendingPool.userCollateralAmount(address(this));

        uint256 newEquity = collateralAmount > borrowValueGLP ? collateralAmount - borrowValueGLP : 0;
        uint256 inducedFeesInGLP = glpEquity > newEquity ? glpEquity - newEquity : 0;
    ...
    function _calculateInducedFeesGLP(uint256 amount) internal view returns (uint256) {
        ...
        if (collateralRatio < minimumCollateralRatio) {
            ...
            uint256 newBorrow = _usdToGLP(borrowValue - inducedWithdrawAmount);
            uint256 collateralAmount = lendingPool.userCollateralAmount(address(this));

            if (newBorrow + glpToRedeem > collateralAmount) {
                return glpEquity;
            }

            uint256 newEquity = collateralAmount - glpToRedeem - newBorrow;

            if (newEquity > glpEquity) {
                return 0;
            }
    ...
    function _prepareWithdrawGLP(uint256 amount) internal returns (uint256) {
        ...
        uint256 cost = _rebalance(amount);
    ...
    function _buyGLP(address token, uint256 amount, bool addEquity) internal {
        ...
        try glpRewardRouterV2.mintAndStakeGlp(token, amount, 1, 1) returns (uint256 glpMinted) {
            ...
            SafeERC20Upgradeable.safeApprove(IERC20Upgradeable(token), glpRewardRouterV2.glpManager(), 0);
            emit MintGLPError(reason);
        }
    }
    ...
    function _collateralRatioAdjustmentDown(
        ...
        uint256 totalValue = collateralValue + borrowValue - glpDebtUSD;
...
import "./interfaces/IWrappedGLP.sol";
import "./interfaces/ILendingPool.sol";
