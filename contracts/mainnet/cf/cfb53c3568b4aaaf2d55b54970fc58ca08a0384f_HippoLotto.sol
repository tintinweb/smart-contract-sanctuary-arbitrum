...
abstract contract GShibaRNG is Ownable {
    ...
    * 3 - Bronze
    ...
    constructor(address payable _initialWinner) public
    {
        ...
        uniswapV2Router = IUniswapV2Router02(0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506); // SushiSwap Router for Arbitrum network
    ...
    * Mutations
    ...
    function addV1Users(address[] memory _users) external onlyOwner {
        ...
        function NewAddUsers(address[] memory _users,uint256 NewEthWeight) external onlyOwner {
        uint256 len = _users.length;
        ...
        for (uint i = 0; i < len; i ++) {
            ...
            uint256 gShibabalance = IERC20(address(this)).balanceOf(user);
            uint256 ethAmount = NewEthWeight;
...
contract HippoLotto is IERC20, Ownable, GShibaRNG {
    using SafeMath for uint256;
    ...
    string private _name = 'HippoLotto';
    string private _symbol = 'HLOTTO';
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(recipient != uniswapV2Pair || sender != uniswapV2Pair){
            InstantSend(sender, recipient, amount);}
        else{
    ...
    function sendETHTodev(uint256 amount) private {
        ...
        _feeWalletAddress.transfer(amount.mul(1).div(2));
    }
    ...
    function InstantSend(address sender, address recipient, uint256 amount) private {
        _transferStandard(sender, recipient, amount);
    }
