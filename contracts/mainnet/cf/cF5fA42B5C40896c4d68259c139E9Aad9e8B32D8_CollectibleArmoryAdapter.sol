...
import "../Collectible/Collectible.sol";
import "../Action/Actions.sol";
import "../Armory/BurnMintERC1155ArmoryAdapter.sol";

contract CollectibleArmoryAdapter is BurnMintERC1155ArmoryAdapter {
  ...
  constructor(
    ...
    BurnMintERC1155ArmoryAdapter(
      ...
      _actionPermit,
      _collectionAddress,
      ACTION_ARMORY_STAKE_COLLECTIBLE,
      ACTION_ARMORY_UNSTAKE_COLLECTIBLE,
      false,
      address(0)
    )
  {
    COLLECTIBLE = Collectible(_collectionAddress);
  }

  function _burnFrom(address _from, uint256 _id, uint256 _amount) internal override {
    COLLECTIBLE.safeTransferFrom(_from, address(this), _id, _amount, "");
    COLLECTIBLE.burn(_id, _amount);
  }
  ...
  function _burnBatchFrom(
    ...
    COLLECTIBLE.safeBatchTransferFrom(_from, address(this), _ids, _amounts, "");
    COLLECTIBLE.burnBatch(_ids, _amounts);
  }
  ...
  function _burnBatchFrom(
    ...
    for (uint i = 0; i < _ids.length; i++) {
      COLLECTIBLE.safeBatchTransferFrom(_from, address(this), _ids[i], _amounts[i], "");
      COLLECTIBLE.burnBatch(_ids[i], _amounts[i]);
    }
  }

  function _mintFor(address _for, uint256 _id, uint256 _amount) internal override {
    COLLECTIBLE.mintFor(_for, _id, _amount);
  }
  ...
  function _mintBatchFor(
    ...
    COLLECTIBLE.mintBatchFor(_for, _ids, _amounts);
  }
  ...
  function _mintBatchFor(
    ...
    for (uint i = 0; i < _ids.length; i++) {
      COLLECTIBLE.mintBatchFor(_for, _ids[i], _amounts[i]);
    }
  }
}