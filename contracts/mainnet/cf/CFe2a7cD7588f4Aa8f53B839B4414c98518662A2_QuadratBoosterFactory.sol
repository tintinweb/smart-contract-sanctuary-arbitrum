    ...
    OwnableUpgradeable
{
    ...
    /// @notice withdraws fee ny owner
    /// @param to An address withdraw to
    /// @dev only callable by owner
    function withdrawFee(address to) external override onlyOwner {
        require(to != address(0) && to != address(this));
        uint256 balance = address(this).balance;
        require(balance != 0, "ZA");
        (bool success, ) = to.call{value: balance}(new bytes(0));
        require(success, "ETH_TRANSFER_FAILED");
        emit FeeWithdrawn(_msgSender(), to, balance);
    }
