...
contract StreamRewarder is  Initializable, OwnableUpgradeable {
    ...
    event MasterRadpieUpdated(address oldMasterRadpie, address newMasterRadpie);
    ...
    ///@notice For Radpie, the stakingToken from masterRadpie must be used as Receipt Token . 
    function __StreamRewarder_init(address _masterRadpie, address _rewardQueuer, address _receiptToken, uint256 _duration) public initializer {
    ...
    function getReward(
        address _account,
        address _receiver
    ) external onlyMasterRadpie returns(bool) {
        updateFor(_account);
    ...
    function setMasterRadpie(address _masterRadpie) external onlyOwner {
        address oldMasterRadpie = _masterRadpie;
        masterRadpie = _masterRadpie;

        emit MasterRadpieUpdated(oldMasterRadpie, _masterRadpie);
    }
}