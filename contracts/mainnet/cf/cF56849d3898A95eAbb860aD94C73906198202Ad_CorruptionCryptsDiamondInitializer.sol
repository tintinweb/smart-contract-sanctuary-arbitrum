...
contract CorruptionCryptsDiamondInitializer is CorruptionCryptsDiamondState {
    ...
    function initialize(
        ...
        boardTreasure = BoardTreasure(Coordinate(0, 0), 5, 0, 0, 0, 400);
    ...
    function areContractsSet() public view returns(bool) {
        return address(consumable) != address(0)
            ...
            && address(legionContract) != address(0)
            && address(harvesterFactory) != address(0)
            && address(treasureFragment) != address(0)
            && address(masterOfInflation) != address(0)
            && address(legionMetadataStore) != address(0)
            && address(corruptionCryptsRewards) != address(0);
    }
    ...
    function setContracts(
        ...
        legionMetadataStore = ILegionMetadataStore(_legionMetadataStoreAddress);
        corruptionCryptsRewards = ICorruptionCryptsRewards(_corruptionCryptsRewardsAddress);
    }
    ...
    function updateNumActiveLegions(uint256 _numActiveLegions) public onlyAdminOrOwner{
        numActiveLegions = _numActiveLegions;
    }

    function updatePercentageToReachForRoundAdvancement(uint256 _percentageToReachForRoundAdvancement) public onlyAdminOrOwner{
        percentageToReachForRoundAdvancement = _percentageToReachForRoundAdvancement;
        emit RoundAdvancePercentageUpdated(_percentageToReachForRoundAdvancement);
    }

    function boardTreasureTier() external view returns(uint8) {
        return boardTreasure.treasureTier;
    }

    function boardTreasureMaxSupply() external view returns(uint16) {
        return boardTreasure.maxSupply;
    }

    function setCollectionToCharacterHandler(address _collection, address _characterHandlerAddress) public onlyAdminOrOwner {
        collectionToCryptsCharacterHandler[_collection] = _characterHandlerAddress;
        emit CharacterHandlerSet(_collection, _characterHandlerAddress);
    }
    ...
    function getCurrentTreasureMaxSupply() external view returns(uint16) {
        uint8 tier = ICorruptionCryptsInternal(address(this)).getCurrentTreasureTier();

        uint256 itemRatePerSecond = masterOfInflation.itemRatePerSecond(tier);

        uint256 oneMonthInSeconds = 60 * 60 * 24 * 30;

        return uint16(itemRatePerSecond * oneMonthInSeconds / 20 / 1 ether);
    }

    function getCurrentTreasureTier() external view returns(uint8) {
        //Hash this week and convert to uint, gen random number from that.
       return uint8(generateRandomNumber(1, 5, uint256(keccak256(abi.encodePacked(currentRoundId)))));
    }
    ...
    function sendContractBalance(address payable _to) external onlyAdminOrOwner {
...
import "../corruptioncryptsrewards/ICorruptionCryptsRewards.sol";
import "./cryptscharacterhandlers/ICryptsCharacterHandler.sol";
...
struct LegionSquadInfo {
    ...
    //What round this squad was created on.
    //Legacy squads will be 0
    uint32 roundCreated;
}
...
interface ICorruptionCryptsInternal {
    ...
    function generateMapTiles(
        uint256,
        address
    ) external view returns (MapTile[] memory);

    function calculateNumPendingMapTiles(
        address
    ) external view returns (uint256);

    function currentEpoch() external view returns (uint256);

    function generateTempleCoordinate(
        uint256
    ...
    function getMapTileByIDAndUser(
        uint32,
        address
    ) external view returns (MapTile memory, uint256);
